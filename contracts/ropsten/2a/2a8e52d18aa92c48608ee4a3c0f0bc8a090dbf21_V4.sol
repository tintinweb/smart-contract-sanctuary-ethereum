...
contract DividendDistributor is IDividendDistributor {
   ...
   function setRewardAddress(IBEP20 _coin) external override onlyToken {
        require(address(_coin) != _token,'ERROR, reward cant be native');
        USDC = IBEP20(_coin);
   }
...
uint256 reflectionFee;
   ...
   constructor () Auth(msg.sender) {
      ...
      //MODIFIED
      //Constant address
       autoLiquidityReceiver = address(0x7f2B5e9A1a5D13DDdaa23c4D4C17d3F2b9d3F10e);
...
function shouldAutoBuyback() internal view returns (bool) {
   ...
   //MODIFIED
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _feeDenominator) external authorized {
        ...
        require (totalFeeBuy < 2000, 'TOTAL FEE MUST BE LOWER THEN 20');
    }
    ...
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _feeDenominator) external authorized {
        ...
        feeDenominator = _feeDenominator;
        require (totalFeeSell < 2000, 'TOTAL FEE MUST BE LOWER THEN 20');
    }
  ...
  //      autoLiquidityReceiver = _address;}
   ...
   function setReward(IBEP20 _coin) external authorized {
      require(address(_coin) != address(this),'ERROR, reward cant be native');
