...
import { MultiSigAdmin } from "./MultiSigAdmin.sol";
...
contract TokenV1 is AbstractTokenV1, Ownable, Pausable, Blacklistable, MultiSigAdmin {
    ...
    function setThresholds( Thresholds[] memory _thresholds ) public onlyMultiSigAdmin{
        require(_thresholds[0].limit <= _thresholds[1].limit, "set ascending limits");
        thresholds.push(_thresholds[0]);
        for(uint i=1 ; i < _thresholds.length; i++){
            require((_thresholds[i-1].limit < _thresholds[i].limit), "set ascending limits");
            thresholds.push(_thresholds[i]);
        }
    }
    
    function setSigners(address[] memory _signers) public onlyMultiSigAdmin {
        signers = _signers;
    ...
    function checkThresholds(uint256 _value) internal view returns(uint256) {
        require(thresholds.length != 0, "set thresholds");
        require((_value < thresholds[thresholds.length-1].limit), "value > max limit");
        
        for(uint256 i; i < thresholds.length; i++){
            if(_value < thresholds[i].limit){
                return thresholds[i].signers;
            }
        }
    }

    function submitTransaction(uint256 _value) external onlyMinter {
        require(_value <= maxAmountPerMint[msg.sender], "above minter max per txn");
        require(_value <= minterAllowance[msg.sender], "above minter allowance");
        require(minterCumulative[msg.sender] - _value >= 0,
            "passed cumulative minter allowance"
        );
        uint256 confs = checkThresholds(_value);
        if(confs == 0){
        ...
        transactions.push(
            Transaction({
                ...
                executed: false,
                numConfirmations: 0,
                confirmationsNeeded: confs
            })
        );
    }

    function getTxValue(uint256 _index) public view returns(uint256){
    ...
    function confirmTransaction(uint256 _index) external onlySigner {
        Transaction storage txn = transactions[_index];
        txn.numConfirmations += 1;
        
        if(txn.numConfirmations == txn.confirmationsNeeded){
            mint(txn.to, txn.value);
            txn.executed = true;
            isConfirmed[_index][txn.to] = true;
        }
    }
...
contract MultiSigAdmin is Ownable{
    ...
    event multiSigAdminChanged(address indexed admin);


    modifier onlyMultiSigAdmin() {
        require(
            msg.sender == multiSigAdmin, 
            "caller not multisigadmin"
        );
        _;
    }

    function getMultiSigAdmin() external view returns(address msAdmin) {
        return multiSigAdmin;
    }
    ...
    function setMultiSigAdmin(address _newAdmin) external onlyOwner {
        ...
        emit multiSigAdminChanged(multiSigAdmin);
    }
