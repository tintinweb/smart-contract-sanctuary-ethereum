...
contract Satan2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x13410232ee034c73DaE2f44483Bb9C50CF3cF8cD); // Marketing Address
    ...
    string private _name = "Satan2";
    string private _symbol = "Satan2";
    ...
    uint256 public maxTX =100000000000000e9;
  ...
  function ExcludeLimitcheck(address _addr,bool _status) public onlyOwner() {
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
            ...
            if (block.timestamp == launchTime) {
             ...
             if(!Limtcheck[to]){ require(balanceOf(to).add(amount) <= maxTX,"Tx Limit Exceed");
         }
    ...
    function setmaxTX(uint256 amount) external {
        require(msg.sender==owner() || msg.sender==marketingAddress,"Accessable");
        maxTX = amount;
    }
