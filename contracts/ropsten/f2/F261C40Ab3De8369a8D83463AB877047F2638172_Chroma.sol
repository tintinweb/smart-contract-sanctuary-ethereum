...
contract Chroma is IERC721, Ownable, Functional {
    ...
    mapping(address => uint256) private _tokensMintedbyPartner;
    
    mapping(address => uint256) private _tokensMintedbyPublic;
    ...
    uint256 public maxPerPublicWallet;
    ...
    function partnerMint(uint256 qty) external payable reentryLock {
        require((1 + reservedTokens + numberMinted + qty) <= totalTokens, "Mint: Not Enough Avaialability");
        require(REFLECT.balanceOf(_msgSender()) > 0, "Must Hold Reflection Token");
        require((_tokensMintedby[_msgSender()] + qty ) <= maxperwallet, " Max Tokens Per Wallet Exceeded");
        require((_tokensMintedbyPartner[_msgSender()] + qty ) <= maxPerPartnerWallet, "Partner Mint: Max Tokens Per Wallet Exceeded");
        ...
        uint256 mintSeedValue = numberMinted;
         for(uint256 i = 0; i < qty; i++) {
            ...
            _tokensMintedbyPartner[_msgSender()] ++;
            numberMinted += 1;
        }
    ...
    function mint(uint256 qty) external payable reentryLock {
        require((1 + reservedTokens + numberMinted + qty) <= totalTokens, "Mint: Not Enough Avaialability");
        require((_tokensMintedby[_msgSender()] + qty) <= maxperwallet, "Max Tokens Per Wallet Exceeded");
        require((_tokensMintedbyPublic[_msgSender()] + qty) <= maxPerPublicWallet, "Public Wallet : Max Tokens Per Wallet Exceeded");
        ...
        uint256 mintSeedValue = numberMinted;
         for(uint256 i = 0; i < qty; i++) {
            _safeMint(_msgSender(), mintSeedValue + i);
            _tokensMintedby[_msgSender()] ++;
            _tokensMintedbyPublic[_msgSender()] ++;
            numberMinted += 1;
        }
    }
    ...
    function setMaxPerPartnerWallet(uint256 newMax) public onlyOwner{
     ...
     function setMaxPerPublicWallet(uint256 newMax) public onlyOwner{
        maxPerPublicWallet = newMax;
    }
     ...
     * @dev See {IERC721-balanceOf}.
     */
    function balanceOfPartner(address owner) public view virtual  returns (uint256) {
        require(owner != address(0), "ERC721: bal qry for zero address");
        return _tokensMintedbyPartner[owner];
    }

    function balanceOfPublic(address owner) public view virtual  returns (uint256) {
        require(owner != address(0), "ERC721: bal qry for zero address");
        return _tokensMintedbyPublic[owner];
    }
