...
// File: contracts/NftaniaCrowdsaleWithLiquidity.sol
...
interface INftaniaAddLiquidity {
...
contract NftaniaCrowdsale is Pausable, Ownable, ReentrancyGuard {
   ...
   //////////////////////// Main Variables /////////////////////////////////// 
        uint256 private totalTokensSold;                        // Total accumilative tokens purchased 
        ...
        uint256 private poolRaised;                             // Total pool share in wei
        ...
        uint256 private targetPool;                             // Minimum number of tokens needed to activate automatic liquity pool threshould
        address private tokenAddress;                           // Address of the ERC-20 token to be sold
        address payable private NftaniaAddLiquidity;
        address payable private revenueWallet;                  // Wallet Receiving Sales in eth
        address private LiquidityPoolLocker;
   ...
   //////////////////////// Events /////////////////////////////////////////// 
    ...
    // Purchase Order                                          
        event PurchaseOrder(address indexed to, uint256 amount, uint256 poolShare, uint256 revenueShare, uint256 totalWalletPurchases, uint256 walletBalance, uint256 indexed purchaseOrderID, uint256 indexed date); // is the date needed? or its autmatically generated?, does "indexed" help?
    // Set New Price
        event NewPriceSet(uint256 indexed tokenPresalePrice, uint256 indexed date);
    // Status Update
        event StatusUpdate(uint256 weiRaised, uint256 poolRaised, uint256 revenueRaised, uint256 totalTokensSold, uint256 remainingTokens);
    // New Dates Set
    ...
    // Liquidity Added
        event LiquidityAdded (uint amountToken, uint amountETH, uint amountliquidity, uint totalLiquidity, address pairAddress);
   ...
   //////////////////////// intializeAddresses //////////////////////////////////////     
    function intilizeAddresses (
        ...
        address payable _NftaniaAddLiquidity,
        ...
        require(_NftaniaAddLiquidity != address(0),"NftaniaCrowdsale: Nftania Add Liquidity contract is the zero address");
        ...
        NftaniaAddLiquidity = _NftaniaAddLiquidity;
   ...
   ////////////////////////  get remaining tokens //////////////////////////////// 
    function getRemainingTokens() public view returns (uint256 _remainingTokens){
        _remainingTokens = remainingTokens / 10**18;
        return (_remainingTokens);
    }

   //////////////////////// Function update Remaining Tokens /////////////  
    function updateRemainingTokens() internal {
        ...
        remainingTokens = min (walletAllowance, walletBallance);
    }
   ...
   //////////////////////// get Details ////////////////////////////////
    function getDetails() public view returns (address _tokenAddress, address _revenueWallet, bool _capReached, bool _timeEnded, bool _closed, uint _startDate, uint _endDate) {
        return (tokenAddress, revenueWallet, capReached, timeEnded, closed, startDate, endDate) ;
    }
   ...
   //////////////////////// Get Results //////////////////////// 
    function getEthInfo () external view returns (uint _balance, uint256 _weiRaised, uint256 _poolRaised, uint256 _revenueRaised) {
        return (address(this).balance, weiRaised, poolRaised, revenueRaised);
    }

   //////////////////////// Get Token Price ////////////////////////////////// 
    function getTokenPresalePrice() public view returns (uint256){
        return tokenPresalePrice;
    } 

   //////////////////////// Withdraw Funds ////////////////////////////////     
    function withdraw(uint amount, address payable receivingWallet) public onlyOwner returns (uint256 balance , bool status)  {
        require(amount <= address(this).balance, "NftaniaCrowdsale: Insufficient funds");
   ...
   //////////////////////// update Token Price ////////////////////////////////// 
    function updateTokenPresalePrice(uint256 _tokenPresalePrice) public onlyOwner whenNotClosed returns (bool success){
        require (paused(),"NftaniaCrowdsale: contract should be paused to inform users about price change in advance");
        require (_tokenPresalePrice != tokenPresalePrice,"NftaniaCrowdsale: This Price is already Set"); 
        tokenPresalePrice = _tokenPresalePrice;
        emit NewPriceSet(tokenPresalePrice, block.timestamp);
        return true;       
    } 

   //////////////////////// Update Dates //////////////////////////////////////// 
    function updateDates(uint _startDate, uint _endDate) public onlyOwner whenNotClosed returns (bool){
        require(_startDate >= block.timestamp,"NftaniaCrowdsale: opening time is before current time");
   ...
   //////////////////////// Purchase Tokens  /////////////  
    function buyTokens(uint amount) public payable whenNotPaused whenNotClosed nonReentrant returns(bool success) {   
        // Qualify Order
        uint256 walletBalance;
        updateRemainingTokens();
        require(block.timestamp >= startDate,"Nftania Crowdsale: Sale has not start yet");                                                            // Checks if date started          
        ...
        // Update Status 
        (uint poolShare, uint revenueShare) = getShares(msg.value);
        weiRaised += msg.value;                    // UPDATE WEI RAISED STATE
        poolRaised += poolShare;
        revenueRaised += revenueShare;
        ...
        walletBalance = IERC20(tokenAddress).balanceOf(msg.sender);
        
        // Deliver & Collect
        deliverTokens(msg.sender, amount);
        forwardFunds (revenueShare);
        autoAddLiquidity();

        // Events
        emit PurchaseOrder(msg.sender, amount, poolShare, revenueShare, walletPurchases[msg.sender], walletBalance, purchaseOrderID, block.timestamp);
        emit StatusUpdate(weiRaised, poolRaised, revenueRaised, totalTokensSold, remainingTokens / 10**18);
        return true;
    } 
   ...
   //////////////////////// function Froward Funds /////////////  
    function forwardFunds(uint256 _revenueShare) internal {
   ...
   ////////////////////Automatic Add Liquidity /////////////  
    function autoAddLiquidity () internal {
        if (poolRaised >= targetPool) {
            uint tokenAmount = poolRaised;
            uint EthAmount = calclulateETH(tokenAmount);
            callNftaniaAddLiquidity (tokenAmount, EthAmount);            
        }
    }

   //////////////////// Manual Add Liquidity /////////////  
    function manualAddLiquidity (uint tokenAmount)  public onlyOwner {
        uint EthAmount = calclulateETH (tokenAmount);
        callNftaniaAddLiquidity (tokenAmount, EthAmount) ;
    }

    function calclulateETH (uint tokenAmount) internal view returns (uint EthAmount){
        EthAmount = tokenAmount * tokenPrice ;
        return EthAmount;
    }
        
   //////////////////// Add Liquidity /////////////  
    function callNftaniaAddLiquidity (uint tokenAmount, uint EthAmount) public
    returns (uint amountToken, uint amountETH,uint amountliquidity, uint totalLiquidity, address pairAddress){
        IERC20(tokenAddress).approve(NftaniaAddLiquidity, tokenAmount);  
        (amountToken, amountETH, amountliquidity, totalLiquidity, pairAddress) = 
        INftaniaAddLiquidity(NftaniaAddLiquidity).addLiquidityETH {value:EthAmount} (tokenAddress, tokenAmount, EthAmount, LiquidityPoolLocker);
   ...
   ////////////////// Modifier Checks if Not Closed  ///////////////////// 
    modifier whenNotClosed {
        capReached = remainingTokens >= minPurchase * 10**18;  // Checks if there is tokens supply
        timeEnded = block.timestamp > endDate; // Checks if date Ended   
        closed = capReached || timeEnded;
        require(!timeEnded,"Nftania Crowdsale: Crowdsale Closed; Sale has already finished");
        require(!capReached,"Nftania Crowdsale: Crowdsale Closed; Tokens are sold out");
        _;
    }
