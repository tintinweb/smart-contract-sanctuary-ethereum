...
contract LuumInDapp{
    ...
    function ComprarProducto (uint256 _id, uint _Cantidad) public {
        ...
        {  
            ...
            MappingContratos[idContract] = Contrato (contractsCounter, _id, _Cantidad, MappingProductos[_id].price, CostoTotal, msg.sender, true, block.timestamp);
            MapContracts.push(Contrato (idContract, _id, _Cantidad, MappingProductos[_id].price, CostoTotal, msg.sender, true, block.timestamp));
            buyer.MapContracts.push(idContract);

            emit contratoCreado(contractsCounter, _id, _Cantidad, MappingProductos[_id].price, CostoTotal, msg.sender, true, block.timestamp);
