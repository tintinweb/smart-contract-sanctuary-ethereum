...
/**
 * @title Illuvitar D1sk
 * @dev inherit BaseIlluvitar
 * @author Yuri Fernandes
 */
contract D1skImpl is BaseIlluvitar {
    using StringsUpgradeable for uint256;

    /// @dev Accessory metadata
    mapping(uint256 => D1skMetadata) public metadata;

    function initialize(
        string memory name_,
        string memory symbol_,
        address imxMinter_,
        address d1skMetadataBuilder_
    ) external initializer {
        // initialize BaseIlluvitar storage variables
        __BaseIlluvitar_init(name_, symbol_, imxMinter_, d1skMetadataBuilder_);
    }
    ...
    function contractLevelImageURI() public pure override returns (string memory) {
        // TODO: Define D1sk contract-level image
        return "";
    }

    function description() public pure override returns (string memory) {
        // TODO: Description for the D1sk collection need to be set
        return "";
    }

    function tokenImageURI(uint256 _tokenId) public view override returns (string memory) {
    ...
    /**
     * @dev Mint D1sk with blueprint.
     * @dev blueprint has format of `a,b,c`
     *      a : set
     *      b : batch
     *      c : Pack Type
     * @param to Recipient address
     * @param tokenId Token id
     * @param blueprint D1sk blueprint
     */
    function _mint(
        address to,
        uint256 tokenId,
    ...
    /// @dev Parse blueprint
    function _parseBlueprint(bytes memory blueprint) private pure returns (D1skMetadata memory _metadata) {
        uint8 p;
        uint8 placeholder;

        p = _skipNonDecimal(blueprint, p);
        (_metadata.set, p) = _atoi8(blueprint, p);
        (_metadata.batch, p) = _atoi8(blueprint, p);
        (placeholder, p) = _atoi8(blueprint, p);
        _metadata.packType = PackType(placeholder);
        p = _skipNonDecimal(blueprint, p);
    ...
    function _buildMetadata(uint256 _tokenId) internal view override returns (string memory) {
        ...
        return IlluvitarsMetadataBuilder(metadataBuilder).buildD1skMetadata(_tokenId);
    }
}
...
abstract contract BaseIlluvitar is ERC721EnumerableUpgradeable, UUPSUpgradeable, OwnableUpgradeable, IMintable {
     ...
     */
    function _atoi8(bytes memory a, uint8 index) internal pure returns (uint8 i, uint8 p) {
