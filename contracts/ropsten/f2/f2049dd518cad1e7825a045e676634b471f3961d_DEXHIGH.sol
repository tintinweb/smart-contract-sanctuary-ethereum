...
contract DEXHIGH is RedBlackTree, SafeMath
{
    ...
    function GetMyOrders() public constant returns (address[] memory tokens, uint[] memory amounts, uint[] memory prices, bool[] memory sells, uint64[] memory timestamps)
    {
        OpenOrder storage openOrder = holder_OpenOrder[msg.sender];
        ...
        address[] memory _tokens = new address[](N);
        uint[] memory _amounts = new uint[](N);
        uint[] memory _prices = new uint[](N);
        bool[] memory _sells = new bool[](N);
        uint64[] memory _timestamps = new uint64[](N);
