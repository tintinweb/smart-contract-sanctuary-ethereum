...
contract UrinalDotNetNft is ERC721Enumerable, Ownable {
    ...
    uint16 private reserveUrinalsId;
    uint16 private urinalsId;
    ...
    uint256 public constant maxMint = 1000;

    function _baseURI() internal view virtual override returns (string memory) {
        return baseURI;
    }

    constructor() ERC721("Urinal Dot Net NFT", "UDNNFT") {
        reserveUrinalsId = 1; // item 1-60
        urinalsId = 61; // item 51-1893
    }
    ...
    function mint(bytes32[] memory proof, bytes32 leaf) public returns (uint256) {
        ...
        require(urinalsId <= maxMint, "Mint limit reached");

        _safeMint(msg.sender, urinalsId++);

        return urinalsId;
    }

    function reserveUrinals(address to, uint8 amount) public onlyOwner {
        require(reserveUrinalsId + amount <= 51, "Out of stock");

        for (uint8 i = 0; i < amount; i++) _safeMint(to, reserveUrinalsId++);
    }
