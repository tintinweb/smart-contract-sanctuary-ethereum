...
contract Market is ERC721Holder, Ownable {
	...
	function placeAsk(uint256 nftId, address payable recipient, uint256 price) external virtual ownerOnly(nftId) returns (bytes32 askId) {
		bytes32 askId = getAskId(nftId, _msgSender());
		marketOperable(askId);

		if (listings[askId].listTime == 0) {
			listings[askId] = Ask(price, nftId, indexerLength, recipient, _msgSender(), block.timestamp, 0);
