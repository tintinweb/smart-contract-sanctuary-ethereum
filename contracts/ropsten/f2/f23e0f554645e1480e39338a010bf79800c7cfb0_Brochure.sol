...
contract Brochure is Single1155 {
    constructor() Single1155("https://raw.githubusercontent.com/kevinbeauregard/public/main/sample1155.json") {}
...
contract Single1155 is ERC1155, Ownable, IBasicMint {
    ...
    uint256 public mintPrice = 42069000000000000; // 0.042069 ETH
    ...
    function sweepErc20(address fromToken) external onlyOwner {
    ...
    function mintActive() public view returns (bool) {
        return active && (totalSupply(tokenId) < maxSupply);
    }

    function mintData() external view returns (uint256, uint256, uint256) {
        return (mintPrice, mintLimit, maxSupply);
    }

    function totalSupply(uint256 id) public view returns (uint256) {
        require(tokenId == id, "Token does not exist");
        return _totalSupply;
    }
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(quantity + totalSupply(tokenId) <= maxSupply, "Maximum supply reached");
        mintedByAccount[_msgSender()] += quantity;
        require(mintedByAccount[_msgSender()] <= mintLimit, "Exceeds mint limit per account");
    ...
    function _beforeTokenTransfer(
        ...
        if (to == address(0)) {
            require(_totalSupply >= quantities[0], "ERC1155: burn amount exceeds totalSupply");
            unchecked {
                _totalSupply -= quantities[0];
