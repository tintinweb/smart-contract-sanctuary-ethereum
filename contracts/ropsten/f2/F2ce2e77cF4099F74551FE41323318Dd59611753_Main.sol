...
contract Main is Treatment, RequestInfo {
        ...
        )
    {
        ...
        LapSchema[] memory _lapsInfo = new LapSchema[](
        ...
        TreatmentSchema[] memory _treatments = new TreatmentSchema[](treatmentPatient[_request.patientId].length);
        ...
        if (_request.history == true && _request.approved == 1) {
            for (
                ...
                // treatment
                _treatments[i] = treatment[treatmentPatient[_request.patientId][i]];

                // medicines info
                MedicineSchema[] memory _getMedicines = getMedicines(treatmentPatient[_request.patientId][i]);

                for (
                    uint256 c = 0;c < _getMedicines.length; c++ ) {
                    _medicineInfo[c] = _getMedicines[c];
                }
                // end medicine;

                // laps
                LapSchema[] memory _getLaps = getLaps(treatmentPatient[_request.patientId][i]);
                for (
                    uint256 l = 0; l <_getLaps.length; l++
                ) {
                    _lapsInfo[l] = _getLaps[l];
                }
                
                // medical procedure
                MedicalProcesureSchema[] memory _getMedocal = getMedicalProcesure(treatmentPatient[_request.patientId][i]);
