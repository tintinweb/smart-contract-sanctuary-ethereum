...
contract ERC721 is IERC721, IERC721Metadata, ERC165{
    ...
    function isApprovedForAll(address _owner, address oper) public view override returns (bool) {

        return operatorAprovals[_owner][oper];    
    }

    // функция трансфера
    function transferFrom( address _from, address _to, uint256 _tokenId) external override {
        require(msg.sender == owners[_tokenId] || operatorAprovals[owners[_tokenId]][msg.sender] == true || tokenAprovals[_tokenId] == msg.sender, "ERC721: You are not owner");
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId) external override {
        require( _checkOnERC721Received(_from, _to,_tokenId, "") == true,"ERC721: transfer to non ERC721Receiver implementer");
        require(msg.sender == owners[_tokenId] || operatorAprovals[owners[_tokenId]][msg.sender] == true || tokenAprovals[_tokenId] == msg.sender, "ERC721: You are not owner");
        approve(address(0),_tokenId);
        balances[_from] -=1;
        balances[_to] +=1;        
    ...
    function mint(address to) public returns(uint256)
    {
        ...
        uint256 toktok = ++tokenId;
        balances[to] += 1;
        owners[toktok] = to;
        emit Transfer(address(0), to, toktok);
        return toktok;
    }
