...
contract JoydomCreatorPass is Ownable, ERC721A, ReentrancyGuard {
    constructor(
    ) ERC721A("Joydom", "JDM", 10, 10000) {}

    // For marketing etc.
    function reserveMint(uint256 quantity, address to) external onlyOwner {
        require(
            totalSupply() + quantity <= collectionSize,
            "Joydom Creator Pass: Too many already minted before dev mint."
        );
    ...
    uint256 public mintPrice = 0.025 * 10 ** 18;
    address public immutable recipient = 0x2B0229D36662979a15EC6d0a77468C336a13fA57;
    ...
    function allowListMint(uint256 quantity, bytes32[] memory proof) external payable callerIsUser {
        require(allowListStatus, "Joydom Creator Pass: Allow List Mint Not Start Yet.");
        require(totalSupply() + quantity <= collectionSize, "Joydom Creator Pass: Reached the Max.");
        require(allowListMintAmount >= quantity, "Joydom Creator Pass: Reached the Max For Allow List Mint.");
        require(isInAllowList(proof), "Joydom Creator Pass: Invalid Merkle Proof.");
        if(!allowListAppeared[msg.sender]) {
            allowListAppeared[msg.sender] = true;
            allowListStock[msg.sender] = maxPerAddressDuringMint;
        }
        require(allowListStock[msg.sender] >= quantity, "Joydom Creator Pass: Reached the Max Per Address During Allow List Mint.");
        uint256 totalCost = uint256(mintPrice) * quantity;
        require(msg.value >= totalCost, "Joydom Creator Pass: Need to send more ETH.");
    ...
    function isInAllowList2(bytes memory addr, bytes32[] memory proof) public view returns (bool) {
