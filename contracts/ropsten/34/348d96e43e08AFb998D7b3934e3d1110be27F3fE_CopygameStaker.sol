...
contract CopygameLeader is CopycatLeader, ERC721Holder, ERC1155Holder {
  ...
  function initializeGame(
    ...
    // Add initial token
    if (address(_baseToken) != address(WETH) && S.getTradingRouteEnabled(address(_baseToken))) {
      _addToken(_baseToken, 1);
    } else {
      _addToken(_baseToken, 999);
    }
  ...
  function depositTo(address to, uint256 percentage, IERC20 refToken, uint256 maxRefAmount) payable public virtual override nonReentrant onlyEOA returns(uint256 totalShare) {
  ...
  function withdrawTo(address to, uint256 shareAmount, IERC20 refToken, uint256 minRefAmount, bool asWeth) public virtual override returns(uint256 percentage) {
    require(false, "W");
  }

  // Tokens managing system
  function _addToken(IERC20 _token, uint256 _type) override internal virtual {
  ...
  function addToken(IERC20 token) nonReentrant onlyOwner override public virtual {
  ...
  function distributeReward(IERC20 token, uint256 amount) public virtual onlyOwner nonReentrant {
  ...
  function setDepositLimit(uint256 newDepositLimit) external virtual onlyOwner nonReentrant {
...
interface ICopygameStaker {
  function updatePool(address sender, uint256 newAmount) external;
