...
contract CNSController {
    ...
    uint256 internal _maxPolicyId = 2;
     ...
     * @param _domain The domain name.
     * @param _node The node of the domain.
     * @param _policy The policy of the domain.
     * @param _allowlist The allowlist of the domain. [Required if choose Policy ID 1]
     * @param _tokenAddress The address of the token. [Required if choose Policy ID 2].
     * @param _tokenAmount The amount of the token. [Required if choose Policy ID 2].
     */
    function registerWithConfig(
        ...
        require(_policy <= _maxPolicyId && _policy > 0, "Invalid policy");
        if (_policy == 1) {
            require(_allowlist.length > 0, "Invalid allowlist");
        } else if (_policy == 2) {
            require(_tokenAddress != address(0), "Invalid token address");
            require(_tokenAmount > 0, "Invalid token amount");
        }
        _domainRegister(_tokenId, _domain, _node);
        // TO DO add create policy internal
    }
    ...
    function getTestDrive() public view returns (uint256) {
        return testDrive;
    }
}
