...
contract Token is ERC721A, Ownable {
    ...
    function mint(uint256 _amount) external payable {
        ...
        require(
            ...
            "Higher than available supply"
        );
        require(_amount <= maxPerTX, "Higher than available per transaction");
        require(
            mintedAmount[msg.sender] + _amount <= maxPerWallet,
            "Higher than available per wallet"
        );
        require(_amount > 0, "Amount must be above zero");
        require(_amount * mintPrice <= msg.value, "Not enough ETH received");
    ...
    function changeMaxPerWallet(uint256 _newValue) external onlyOwner {
        maxPerWallet = _newValue;
    }
