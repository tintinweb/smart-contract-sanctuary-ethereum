...
contract KillKountDynamicReflector is INoBSDynamicReflector, AuthorizedListExt, LPSwapSupport, LockableFunction {
    ...
    constructor(address _lpRouter, address _controlToken, address _rewardsToken) LPSwapSupport(true) payable {
    ...
    function updateShare(address shareholder, uint256 amount, bool shouldProcess) external override onlyOwner {
        _setShareAndProcess(shareholder, amount, shouldProcess);
    }
    ...
    function _setShare(address shareholder, uint256 amount) internal {
        _setShareAndProcess(shareholder, amount, true);
    }

    function _setShareAndProcess(address shareholder, uint256 amount, bool shouldProcess) internal {
        Share memory holderShares = shares[shareholder];
        if(holderShares.amount > 0 && shouldProcess){
