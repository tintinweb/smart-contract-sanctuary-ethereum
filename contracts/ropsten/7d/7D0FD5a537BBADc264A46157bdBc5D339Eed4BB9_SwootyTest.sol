...
contract SwootyTest is ERC1155, Ownable, ERC1155Supply {
    ...
    function setTokenUrisBatch(uint256[] memory tokenIdBatch, string[] memory uriBatch) public onlyOwner {
        for (uint i=0; i<tokenIdBatch.length; i++) {
            setTokenUri(tokenIdBatch[i],uriBatch[i]);
        }
    }
