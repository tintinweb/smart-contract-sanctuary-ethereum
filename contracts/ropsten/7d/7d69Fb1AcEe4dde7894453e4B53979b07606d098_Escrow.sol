...
contract Escrow is ReEntrancyGuard, Refereed {
    ...
    function createOrderChain(address payable _seller, bytes12 _customizeId)
        public
        payable
        condition(msg.value > 0)
    {
        ...
        order.customizeId = _customizeId;
...
contract Refereed {
    mapping(address => bool) public referee;
    address public adminReferee;

    error AccessIsOnlyForReferee();
    error AccessIsOnlyForAdminReferee();

    event RefereeGranted(address indexed account);
    event RefereeRevoked(address indexed account);
    event AdminRefereeTransfered(
        address indexed oldAccount,
        address indexed newAccount
    );

    modifier onlyAdminReferee() {
        if (msg.sender != adminReferee) revert AccessIsOnlyForAdminReferee();
        _;
    }

    modifier onlyReferee() {
        if (referee[msg.sender] == false) {
            revert AccessIsOnlyForReferee();
        }
        _;
    }

    constructor() {
        adminReferee = msg.sender;
    }

    function transferAdminReferee(address _newAdminReferee)
        public
        onlyAdminReferee
    {
        adminReferee = _newAdminReferee;
        emit AdminRefereeTransfered(msg.sender, _newAdminReferee);
    }

    function grantReferee(address _account) public onlyAdminReferee {
        referee[_account] = true;
        emit RefereeGranted(_account);
    }

    function revokeReferee(address _account) public onlyAdminReferee {
        referee[_account] = false;
        emit RefereeRevoked(_account);
    }
}
