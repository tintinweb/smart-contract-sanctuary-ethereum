...
contract THIRDTIME is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private constant _totalSupply = 100 * 1e9 * 1e9;
    ...
    constructor() ERC20("thirdtime", "ttime") {
        ...
        maxTx = 1500 * 1e6 * 1e9; // 0.25% of supply max transaction allowed at launch
        maxWallet = 2000* 1e6 * 1e9; // 2% of supply max wallet
        tokenToEth = 500 * 1e6 * 1e9; // 0.05% is the amount of tokens collected before selling for eth
        ...
        theBank = payable(0x7637ed57cD0453E31EDc7De608edA0085e7Eb0cC); 
    ...
    function updateMaxTxAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 1 / 1000)/1e9, "Cannot set maxTx lower than 0.1%");
        maxTx = newNum * (1e9);
    }
