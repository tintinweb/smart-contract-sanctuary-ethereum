...
abstract contract AccessControl is Context, IAccessControl, ERC165 {
    ...
    bytes32 public constant DEFAULT_SIGNER_ROLE = keccak256("SIGNER_ROLE");

    constructor (){
        _setRoleAdmin(DEFAULT_ADMIN_ROLE, DEFAULT_ADMIN_ROLE);
        _setRoleAdmin(DEFAULT_SIGNER_ROLE, DEFAULT_ADMIN_ROLE);        
    }
...
contract MyToken is AccessControl, ERC721, ERC721Enumerable, Pausable, Ownable {
    using Address for address payable;

    string private baseURI;
    uint256 public maxTotalSupply;
    uint256 public currentSupplyA = 1;
    uint256 public currentSupplyB = 5000;
    ...
    constructor() ERC721("MyToken", "MKT") {
        ...
        _setupRole(DEFAULT_SIGNER_ROLE, _msgSender());
    }
    ...
    function signDecodeParams(bytes memory params) public pure returns (storeParam memory) {
        (
    ...
    function signVerify(storeParam memory sign) internal {
    ...
    function signValidition(bytes32 hash,bytes memory signature) public view returns (bool) {
        ...
        return hasRole(DEFAULT_SIGNER_ROLE,recovered);
    }

    function safeMint(address to,bytes memory param) external payable whenNotPaused{
        storeParam memory store = signDecodeParams(param);
        signVerify(store);
        require(totalSupply() + 1 <= maxTotalSupply, "Supply Exceed");
        require(msg.value >= mintFee, "invalid mint fee");
        if(store.group == 1){
            _safeMint(to, currentSupplyA);
            currentSupplyA++;

            require (currentSupplyA != 5000, "Group A Supply is reached");
        }else if(store.group == 2){
            _safeMint(to, currentSupplyB);
            currentSupplyB++;

            require (currentSupplyB != 10001, "Group B Supply is reached");
        }else {
