...
contract BugXToken is StandardToken {
    ...
    uint256 public tokenExchangeRate = 9000;             // 9000 BUGX tokens per 1 ETH
    uint256 public tokenExchangeRateTwo = 9900;             // 9000 * 1.1 BUGX tokens per 1 ETH
    uint256 public tokenExchangeRateThree = 11250;             // 9000 * 1.25 BUGX tokens per 1 ETH
    ...
    function deductionToken (address _addr, uint256 _eth) onlyOwner external {
        ...
        emit DeductionToken(_addr, tokens);  // logs token issued
    }
    ...
    function () public payable {
        ...
        emit RaiseToken(msg.sender, tokens); // logs token raised // TokenRaise 为充值总数置换成 token
    }
}