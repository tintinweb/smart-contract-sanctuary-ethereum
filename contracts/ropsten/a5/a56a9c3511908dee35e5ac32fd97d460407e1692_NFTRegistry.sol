...
interface IEventNFTAdded {
    event NFTAdded(address indexed addr);
}
contract NFTRegistry is IEventNFTAdded, Ownable {
    function addNFT(address _NFTContractAddr) public onlyOwner {
        emit NFTAdded(_NFTContractAddr);
    }
}
...
contract OrderStorage is IOrderStorage {
    ...
    struct OrderJSONFragment2 {
        string target;
        uint howToCall;
        string ordercalldata; //calldataのこと。名称変えました
        ...
        string basePrice;
        string extra;
        string listingTime; // number | string; のユニオン型。stringで置いてseaport側でstringで投げてもらいます
        string expirationTime; // number | string;　のユニオン型。stringで置いてseaport側でstringで投げてもらいます
    }
    ...
    struct OrderJSONFragment3 {
        ...
        string quantity;
        string englishAuctionReservePrice; // string | undefined;　のユニオン型。stringで置いてseaport側でstringで投げてもらいます
        string createdTime; // number | string;　のユニオン型。stringで置いてseaport側でstringで投げてもらいます
        uint nonce;
    }
