...
contract ERC20 {
   ...
   /**
    ...
    function mint(address accountId, uint256 amount, bytes calldata message) public onlyMinter() returns (bool) {
        ...
        require(msg.sender == minter(), "Minter must have authorisation");
    ...
    /**
     * @dev Make `accountId` an operator of the caller.
     *
     * Emits an {AuthorizedOperator} event.
     *
     * Requirements:
     *
     * - `accountId` must not equal the caller.
     */
    function authorizeOperator(address accountId) public {
        require(msg.sender != accountId, "Authorizing self as operator");

        _operators[msg.sender][accountId] = true;

        emit AuthorizedOperator(accountId, msg.sender);
    }

    /**
     * @dev Revoke `accountId`'s operator status for the caller.
     *
     ...
     * - `accountId` must not equal the caller.
     */
    function revokeOperator(address accountId) public {
        require(msg.sender != accountId, "Revoking self as operator");

        delete _operators[msg.sender][accountId];

        emit RevokedOperator(accountId, msg.sender);
    }
     ...
     * - if the hold has not expired, only the notary can release the hold
     */
     ...
     * - the caller must be the hold's creator or payer
     */
    function renewHold(string memory operationId, uint256 additionalTimeToExpiry) public returns (bool) {
        ...
        require(
            renewableHold.payer == msg.sender || renewableHold.creator == msg.sender,
