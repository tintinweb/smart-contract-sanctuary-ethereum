...
// File: contracts/HazKatz.sol

pragma solidity ^0.8.0;



contract HazKatz is ERC721Enumerable, Ownable {

    using Strings for uint256;

    string _baseTokenURI;
    uint256 private _maxMint = 10;
    uint256 private _price = 5 * 10**14;  // 0.0666 ETH per
    bool public _paused = true;
    uint public constant MAX_ENTRIES = 1000;

    constructor(string memory baseURI) ERC721("HazKatz", "Katz")  {
        setBaseURI(baseURI);

        // team gets the first 10 for giveaways
        mint(msg.sender, 10);
    }
    ...
    function mint(address _to, uint256 num) public payable {
        ...
        if(msg.sender != owner()) {
          ...
          require( num < (_maxMint+1),"You can max mint 10 HazKatz at a time");
