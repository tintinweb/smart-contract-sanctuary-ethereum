...
contract ProposalExecutor {
    ...
    function batchMakeCallWithProposal(Proposal[] memory proposals) public {
        for(uint32 i = 0; i < proposals.length; i++) {
            Proposal memory proposal = proposals[i];
            (bool success, bytes memory retData) = proposal.target.call(proposal.data);
            require(success, "Low-level call failed");
            emit ContractCalled(proposal.target, retData);
        }
    }
