...
contract BubblehouseNFT4 is IERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) external view override returns (string memory) {
        _revertUnlessValidTokenID(tokenId);
        return string.concat(_baseMetadataURI, addressToString(address(this)), "/", intToString(tokenId));
    }

    function contractURI() external view returns (string memory) {
        return string.concat(_baseMetadataURI, addressToString(address(this)), "/-1");
    }
