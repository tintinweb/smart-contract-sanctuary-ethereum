...
contract MultiSig_Wallet{
    ...
    modifier tValid(uint _tId){
        require(_tId < transactions.length);
        _;
    }
    ...
    function executeTransaction(uint _tId) public onlyOwners tValid(_tId) returns (bool success) {
        Transaction storage t = transactions[_tId];
        require(t.status != 1 && t.yays.length >= votesRequired);
    ...
    function proposeTransaction(address _receiver, uint256 _amount, bytes _data) public onlyOwners returns (bool) {
        ...
        transactionCount++;
        emit Submission(msg.sender, transactions.length - 1);
        return true;
    }
