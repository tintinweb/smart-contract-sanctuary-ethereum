...
abstract contract Crowdsale is ReentrancyGuard, Ownable {
    ...
    */
    ...
    function currentRate() external returns (uint256) {
        if(stage == CrowdsaleStage.PreICO)
        {
            uint256 preIcoAward = _currentRate.mul(bonus()).div(100000000000000000000);
            _currentRate = _currentRate+preIcoAward;
            return _currentRate;
        }
        else{
    ...
    function _getTokenAmount(uint256 weiAmount) internal view returns (uint256) {
         if(stage == CrowdsaleStage.PreICO){
             uint256 preIcoAward = weiAmount.mul(bonus()).div(100000000000000000000);
             uint256 totalTokens = weiAmount.mul(_currentRate);
            // _currentRate = _currentRate + preIcoAward;
