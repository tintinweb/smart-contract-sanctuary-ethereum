...
contract CenturiesCollective is ERC721, Ownable, Mintable {
    string public baseURI;

    bytes32 public merkleRoot;
    uint256 public constant MAX_SUPPLY = 4444;
    uint256 public constant MAX_PRE_SUPPLY = 1111;
    uint256 public constant MAX_PER_TX_PRESALE = 8;
    uint256 public constant MAX_PER_TX_PUBLIC = 25;
    uint256 public constant MAX_PER_WALLET = 3;
    uint256 public constant MAX_PER_ = 3;
    uint256 public constant RESERVES = 94;
    uint256 public constant PRICE_PER_PRE = 40000000000000000; // 0.04 eth
    uint256 public constant PRICE_PER_PUBLIC = 50000000000000000; // 0.05 eth
    ...
    constructor(address _owner, address _imx)
        ERC721("Centuries Collective", "CC")
        ...
        payable
    {
        ...
        require(
            count <= MAX_PER_TX_PRESALE,
            "Count cannot be more than presale maximum"
        );
        require(count * PRICE_PER_PRE <= msg.value, "Not enough ether sent");
        require(
            totalSupplied + count <= MAX_PRE_SUPPLY,
            "Purchase would exceed max tokens for presale"
        );
        require(
            mintedLog[msg.sender] + count <= MAX_PER_WALLET,
            "Purchase exceeds max tokens/wallet"
        );
        ...
        totalSupplied += count;
        mintedLog[msg.sender] += count;
        emit Deposit(msg.sender, msg.value, count);
    }

    function publicMint(uint256 count) public payable {
        require(saleIsActive, "Sale not on");
        require(count != 0, "Req count cant be 0");
        require(
            count <= MAX_PER_TX_PUBLIC,
            "Req qty > maximum per transaction"
        );
        require(count * PRICE_PER_PUBLIC <= msg.value, "Not enough ether sent");
        require(
            totalSupplied + count <= MAX_SUPPLY,
            "Purchase exceeds max pub sale tokens"
        );
        require(
            mintedLog[msg.sender] + count <= MAX_PER_WALLET,
