...
contract HowdyBox is ReentrancyGuard, Ownable {
    ...
    function sendMsg(uint _amount,string memory _msgHash, uint _deadline, address _receipient, bool _isNativeAntispamFee, uint _version ) public nonReentrant payable {
        ...
        if(_isNativeAntispamFee){
             require(_amount >= nativeAntiSpamFee, 'Please provide antispam fee to send msg');
        }else {
            require(_amount >= stableAntiSpamFee, 'Please provide antispam fee to send msg');
        }
    ...
    function readMsg(uint _id) public nonReentrant returns(string memory) {
    ...
    function reportSpamming(address _sender, address _receipient, bool _status) public {
        require(_receipient == msg.sender,'only receiver of the message can report the sender as spamming');
        isSpam[_sender][_receipient] = _status ;
    }

    function isSpamming(address _sender, address _receipient) public view returns(bool){
        return isSpam[_sender][_receipient];
    }

    function msgHistories(address _sender, address _receipient) public view returns(uint[] memory){
     ...
     function msgMetaData(uint _id) public view returns(address,address,uint,uint,uint,bool){
         MsgInfo memory response = createdMsgs[_id];
     ...
     function msgMetaDatas(uint[] memory _ids) public view returns(returnType[] memory){ 
     ...
     function sentMsgs(address _sender) public view returns(uint[] memory){
         return senders[_sender];
     }

     function receivedMsgs(address _receipient) public view returns (uint[] memory){
