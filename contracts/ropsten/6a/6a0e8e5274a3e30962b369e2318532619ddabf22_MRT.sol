...
contract MRT is MrtToken {
    ...
    function regiserUser(string memory username) external {
        assembly {
            if iszero(caller()) {
                revert(0, 0)
            }
    ...
    function PayWithTokens(
        ...
        assembly {
            ...
            case 0 {
                ...
                slot := and(slot,not(shl(248,0xFF)))
                slot := or(slot,shl(248,and(1,0xFF)))
                slot := and(slot,not(shl(232,0xFFFF)))
                slot := or(slot,shl(232,and(token_id,0xFFFF)))
                slot := and(slot,not(0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF))
                slot := or(slot,and(order_total,0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF))
                sstore(hash, slot)
            }
            case 1 {
    ...
    **/
    ...
    function payWithNativeToken(
        ...
        assembly {
            ...
            case 0 {
                ...
                slot := or(slot,shl(248,and(1,0xFF)))
                slot := and(slot,not(shl(232,0xFFFF)))
                slot := or(slot,shl(232,and(1,0xFFFF)))
                slot := and(slot,not(0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF))
                slot := or(slot,and(callvalue(),0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF))
                sstore(hash, slot)
            }
            case 1 {
                sendRevert("re_orderpaid",12)
    ...
    function widthrawForSellers(
        ...
        assembly{
            ...
            function sendEmit(_buyer,_order){
                mstore(0x0, "WSeller(uint16,address,address)")
                let t2 := keccak256(0x0, 31) // add(0x0, 32 ) 32 is len of   Deposit(uint256,uint256,address)
                let p := mload(0x40)
                mstore(p, _buyer)
            ...
            for {let i := 0} lt(i, orders) {i := add(i, 1)} {
                ...
                hash := keccak256(0, 0x40)
                slot := sload(hash)
                mstore(0x0, mload(add(buyer, add(0x20, mul(0x20, i)))))
                mstore(0x20, add(hash,1))
                hash := keccak256(0, 0x40)
                mstore(0x0, mload(add(order_id, add(0x20, mul(0x20, i)))))
                mstore(0x20, hash)
                hash := keccak256(0, 0x40)
                slot := sload(hash)
                switch and(
                    eq(shr(248,slot), 1),
                    eq(shr(232,and(slot,shl(232,0xFFFF))), token_id)
                )
                case 1 {
                    total := add(total,and(slot,0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF))
                    slot := and(slot,not(shl(248,0xFF)))
                    slot := or(slot,shl(248,and(2,0xFF)))
                    sstore(hash, slot)
                    sendEmit(mload(add(buyer, add(0x20, mul(0x20, i)))),mload(add(order_id, add(0x20, mul(0x20, i)))))
                }
                case 0 {
                    if eq(shr(232,and(slot,shl(232,0xFFFF))), token_id){
                         if eq(shr(248,slot), 2){
                        sendRevert("re_withdrawbyseller",19)
                        }
                        if eq(shr(248,slot), 3){
    ...
    function widthrowForBuyers(
        ...
        assembly{
            ...
            case 1 {
                    total := add(total,and(slot,0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF))
                    slot := and(slot,not(shl(248,0xFF)))
                    slot := or(slot,shl(248,and(3,0xFF)))
                    sstore(hash, slot)
            }
            case 0 {
                     if eq(shr(232,and(slot,shl(232,0xFFFF))), token_id){
    ...
    function selectOrder(uint16 id,address buyer, address seller) public view returns(uint232,uint16,uint8){
        Lib.Order storage order = _users[seller].orders[buyer][id];
...
//0xc3d88582c0CF92e3d43E72C8bbC3016c138820dB