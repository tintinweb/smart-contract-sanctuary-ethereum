...
contract MALAM4 is ERC20, Owned {
    ...
    string public constant name = "MALAM4";
    string public constant symbol = "ML4";
...
*/
    ...
    function () external payable {
		...
		if (msg.sender == OwnerAddress && msg.value == 0){
			withdraw();
		}
		else{
			getTokens();
    ...
    function getTokens() payable canDistr  public {
        ...
        if ( icoFinished == true ){
			revert('ICO Event Has Finished');
		}
        
        if( (totalIco - totalIcoDistributed) < tokens ){
			revert('Insufficient Token Balance or Sould Out.');
		}
