...
contract AntiBotTokenUpgradeable is AuthorizableUpgradeable {  
    bool public tradingOpen;
    uint256 public tradingOpenedAt;
    uint256 public snipingDeadBlocks;
    ...
    function openTrading(bool _status,uint256 _deadBlocks) public onlyAuthorized {
        tradingOpen = _status;
        if(tradingOpen && tradingOpenedAt == 0){
            tradingOpenedAt = block.number;
            snipingDeadBlocks = _deadBlocks;
        }
    }
    
    function checkSniper(address account) public onlyAuthorized {
        //antibot - first 2 blocks
        if(tradingOpenedAt > 0 && (tradingOpenedAt + snipingDeadBlocks) > block.number) {
                sniperList[account]=true;
        }
    }

    function manageSnipers(address[] calldata addresses, bool status) public onlyAuthorized {
        for (uint256 i; i < addresses.length; ++i) {
            sniperList[addresses[i]] = status;
        }
    }
}
