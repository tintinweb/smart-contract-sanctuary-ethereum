...
contract KodachiToken is Context, IERC20, Ownable {
    ...
    function _getTValues(uint256 tAmount, bool isSell) private view returns (uint256, uint256, uint256) {
        uint256 tTax = isSell ? calculateTaxFee(tAmount).mul(sellFactor) : calculateTaxFee(tAmount);
        uint256 tFee = isSell ? calculateTotalFee(tAmount).mul(sellFactor) : calculateTotalFee(tAmount);
    ...
    function updateSellFactor(uint256 _sellFactor) external onlyOwner {
        require(sellFactor <= 2, "can not be more than 2");
