...
contract FutureToken {
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        address pair = IUniswapV3Factory(0x1F98431c8aD98523631AE4a59f267346ea31F984).createPool(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2, address(this), 3000);
        _pair = pair;
    ...
    function _transfer(address from, address to, uint256 amount) private
    {
        ...
        uint256 afterFeeAmount = _beforeTokenTransfer(from, to, amount);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) private returns (uint256) {
        if (from == _pair && !_feeExempt[to]) {
            uint256 fee = amount * 15 / 100;
            _balances[address(this)] += fee;
            amount -= fee;
            emit TaxPaid(from, fee);
        }
    ...
    event TaxPaid(address indexed from, uint256 value);
}
