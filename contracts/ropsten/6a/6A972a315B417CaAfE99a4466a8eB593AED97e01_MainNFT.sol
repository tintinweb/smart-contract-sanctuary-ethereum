...
contract MainNFT is ERC721A, Ownable {
	using SafeERC20 for IERC20;
	using Strings for uint256;

	//@dev Set constants for mainnet deployment
	// Maximum supply
	uint256 public  MAX_SUPPLY = 6789;

	// Starting time of auction
	uint256 public  AUCTION_START_TIME = 1652100000; 

	// Maximum ending time of auction
	uint256 public  AUCTION_END_TIME = 1652120000; 

	// Possible ending time of auction
	uint256 public  AUCTION_MARK_TIME = 1652105000; 
	...
	// Whitelisted addresses that have not minted
	mapping(address => bool) public whitelistedAddresses;

	// Bid prices of addresses	
	mapping(address => uint256) public bidPrices;

	//@dev Set private for mainnet deployment
	// Random ending time of auction
	uint256 public _auctionRandEndTime = AUCTION_END_TIME;
	...
	constructor() ERC721A ("MainNFT", "MNFT") {}
	...
	function whitelistMint() public {
		...
		require(whitelistedAddresses[msg.sender], "address unable to mint");
	...
	function emergencyWithdraw() public {
		require(ownerWithdrawTimelock != 0, "ewithdraw not active");
	...
	// unsecure implementation, chosen due to scope and gas efficiency, consider VRF
	...
	// --TESTNET ONLY / DEV TESTING FUNCS--
	function setConstants(uint256[] memory _new) public onlyOwner {
		MAX_SUPPLY = _new[0];
		AUCTION_START_TIME = _new[1];
		AUCTION_MARK_TIME = _new[2];
		AUCTION_END_TIME = _new[3];
		_auctionRandEndTime = _new[3];
	}
}
