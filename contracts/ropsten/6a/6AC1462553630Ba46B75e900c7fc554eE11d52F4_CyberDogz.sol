...
contract CyberDogz is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAXIMUM_SUPPLY = 8600;
    uint256 public constant MAXIMUM_GIFT = MAXIMUM_SUPPLY;

    uint256 public constant MAXIMUM_MINT_WL = 8;
    uint256 public constant MAXIMUM_MINT_RAFFLE = 6;
    ...
    function presaleMint(uint256 ammount, bytes32[] calldata _merkleProof) external payable nonReentrant
    {
        ...
        require(workflow == WorkflowStatus.Presale, "CYBER DOGZ: Presale is not started yet!");

        require(tokensPerWalletWhitelist[msg.sender] + ammount <= MAXIMUM_MINT_WL, string(abi.encodePacked("CYBER DOGZ: Presale mint is ", MAXIMUM_MINT_WL, " token only.")));

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "CYBER DOGZ: You are not whitelisted");

        require(msg.value >= price * ammount, "CYBER DOGZ: Not enough ETH sent");
    ...
    function raffleMint(uint256 ammount) public payable nonReentrant {
        ...
        require(workflow != WorkflowStatus.SoldOut, "CYBER DOGZ: SOLD OUT!");
        require(workflow == WorkflowStatus.Sale, "CYBER DOGZ: Raffle is not started yet");
        require(msg.value >= price * ammount, "CYBER DOGZ: Not enough ETH sent");
        require(ammount <= MAXIMUM_MINT_RAFFLE, "CYBER DOGZ: You can only mint up to 6 token at once!");
        require(tokensPerWalletRaffle[msg.sender] + ammount <= MAXIMUM_MINT_RAFFLE, "CYBER DOGZ: You can't mint more than 6 tokens!");
        require(supply + ammount <= MAXIMUM_SUPPLY, "CYBER DOGZ: Mint too large!");
    ...
    function setUpSale() external onlyOwner {
        require(workflow == WorkflowStatus.Presale, "CYBER DOGZ: Unauthorized Transaction");
