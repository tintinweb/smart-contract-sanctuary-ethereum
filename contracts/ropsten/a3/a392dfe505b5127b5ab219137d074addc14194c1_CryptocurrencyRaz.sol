...
contract CryptocurrencyRaz is Ownable {
    ...
    {
        require(razNumber>=1 && razNumber<=numberOfRazzes);
        participationFeeForEachRaz[razNumber] = participationFee * 10 ** 10;
    }
    
     function changeWinnerPrizeMoneyForRaz(uint razNumber,uint prizeMoney) public onlyOwner 
     {
        require(razNumber>=1 && razNumber<=numberOfRazzes);
        winnerPrizeMoneyForEachRaz[razNumber] = prizeMoney * 10 ** 10;
    }
    ...
    function addNewRaz(uint maxBets, uint winningAmount, uint ownerAmount, uint particFee) public onlyOwner returns (uint) 
    {
        ...
        participationFeeForEachRaz[numberOfRazzes] = particFee * 10 ** 10;
        winnerPrizeMoneyForEachRaz[numberOfRazzes] = winningAmount * 10 ** 10;
        ownerPrizeMoneyForEachRaz[numberOfRazzes] = ownerAmount * 10 ** 10;    
    ...
    function updateExistingRaz(uint razNumber, uint maxBets, uint winningAmount, uint ownerAmount, uint particFee) public onlyOwner returns (uint) 
    {
        ...
        participationFeeForEachRaz[razNumber] = particFee * 10 ** 10;
        winnerPrizeMoneyForEachRaz[razNumber] = winningAmount * 10 ** 10;
        ownerPrizeMoneyForEachRaz[razNumber] = ownerAmount * 10 ** 10;    
    }
}