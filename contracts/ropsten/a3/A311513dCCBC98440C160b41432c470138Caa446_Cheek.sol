...
contract Cheek is ERC721A, Pausable, Ownable
{
    ...
    string constant public PROVENANCE = '';
    ...
    mapping (address => uint16) public mintsLeft;
    ...
    function setMintsLeft(AddressWithAmount[] calldata values) external onlyOwner
    {
        ...
        {
            mintsLeft[values[i].addr] = values[i].amount;
        }
    }
    ...
    function mint(uint16 numberOfTokens, bytes32[] calldata proof) external payable
    {
        ...
        else if (timeNow >= raffleMintStartTimestamp && timeNow < raffleMintStopTimestamp)
        {
            _raffleMint(msg.sender, numberOfTokens, proof);
        }
        else if (timeNow >= ticketMintStartTimestamp && timeNow < ticketMintStopTimestamp)
        {
    ...
    {
        uint16 maxMintCount = mintsLeft[addr];
