...
contract ReferrioEscrow {
    ...
    event Cancelled(string opportunityId, address recipient, uint256 amount);
    ...
    function deposit(string memory _opportunityId) external payable {
        ...
        balances[msg.sender][_opportunityId].hasDeposited = true;
        opportunityOwner[_opportunityId] = msg.sender; // gets override if same opportunity id - although has a check at the top
    ...
    // desposite money to address 'A' -> opportunity changed to address 'B' -> mapping broken?
    function cancel(string memory _opportunityId) external {
        Escrow memory account = balances[opportunityOwner[_opportunityId]][
            _opportunityId
        ];
