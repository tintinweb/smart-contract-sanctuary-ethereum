...
contract DragQueenDivas is ERC721A, Ownable {
    ...
    uint256 public _mintPrice = 10000000000000000; // 0.01 ETH
    uint256 public _discountMintPrice = 5000000000000000; // 0.005 ETH
    ...
    event NewDragQueenDivasNFTMinted(address sender, uint256 tokenId);
    ...
    function mint(address to, uint256 mintAmount) public payable {
        ...
        emitNftMintedEvents(to, mintAmount);
    }

     function emitNftMintedEvents(address to, uint256 mintAmount) internal {
        for (uint256 i = 1; i < mintAmount; i++) {
            uint256 tokenId = totalSupply() + i;
            emit NewDragQueenDivasNFTMinted(to, tokenId);
        }
    }
