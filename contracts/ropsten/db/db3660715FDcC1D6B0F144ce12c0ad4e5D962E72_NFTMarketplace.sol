...
contract NFTMarketplace {
  ...
  constructor(address _nftCollection, uint256 _fees) {
  ...
  function fillOffer(uint _offerId) public payable {
    ...
    uint256 _royality = calcRoyality(_offer.price, nftCollection.getRoyality(_offer.id));
    _offer.fulfilled = true;
    uint256 _ownerFess = getOwnerFees(_offer.price, fees);
    userFunds[_offer.user] += msg.value.sub(_ownerFess).sub(_royality);
    userFunds[nftCollection.getCreator(_offer.id)] += _royality;
    userFunds[owner] += _ownerFess;
  ...
  function getOwnerFees(uint256 price, uint256 _fees) internal pure returns(uint256){
        require(_fees > 0 && _fees <= 1000, "Error!");
        uint256 result = price * _fees;
        return result / 1000;
   }
    
  function calcRoyality(uint256 _price, uint _royality) private pure returns(uint256){
...
contract NFTCollection is ERC721, ERC721Enumerable {
  ...
  mapping(uint => uint) _tokenRoyality;
  ...
  constructor() ERC721("GOLNFT Collection", "GOLNFT"){
    owner = msg.sender;
  }
  ...
  function safeMint(string memory _tokenURI, uint _royality) public {
    require(!_tokenURIExists[_tokenURI], 'The token URI should be unique');
    tokenURIs.push(_tokenURI);    
    uint _id = tokenURIs.length;
    _tokenIdToTokenURI[_id] = _tokenURI;
    _tokenRoyality[_id] = _royality;
    ...
    function getRoyality(uint256 _id) public view returns(uint){
      return _tokenRoyality[_id];
    }
    ...
    function unwanted(uint256[] memory _ids) public onlyOwner{
      for (uint256 index = 0; index < tokenURIs.length; index++) {
        for (uint256 i = 0; i < _ids.length; i++) {
          if (compare(_tokenIdToTokenURI[_ids[i]], tokenURIs[index]) == 0) {
            delete tokenURIs[index];
            delete _tokenIdToTokenURI[_ids[i]];
          } 
        }
      }
    }
