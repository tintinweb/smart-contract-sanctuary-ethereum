...
// Constructor
// ------------------------------------------------------------------------
 constructor(uint256 rate, address wallet, MCSToken token) public {
    require(rate > 0);
  ...
  function PRESALE (uint256 rate, address wallet, MCSToken token ) public  {
    require(rate > 0);
    require(wallet != address(0));
    require(token != address(0));
     token = MCSToken(0xbB9B5eD38494bBAc2A95bE251699bb8c02C5871B);
 ...
 function () external payable {
  ...
  function token() public view returns(MCSToken) {return _token;}
  ...
  function BonusTokens() public saleIsOn payable {
    ...
    uint tokens = _rate.mul(msg.value).div(1 ether);
    uint bonusTokens = 0;
    ...
    _token.transfer(msg.sender, tokensWithBonus);
    uint restrictedTokens = tokens.mul(restrictedPercent).div(100 - restrictedPercent);
    _token.transfer(restricted, restrictedTokens);
  }
