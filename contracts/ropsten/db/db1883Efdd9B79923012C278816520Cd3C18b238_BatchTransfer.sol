...
contract BatchTransfer is Ownable {
    ...
    function batchTransfer(address[] memory addrArr, uint256[] memory amountArr) public payable returns(bool){
    ...
    function batchTransferToken(address token,address[] memory addrArr, uint256[] memory amountArr) public payable{
        require(addrArr.length > 0,"array's length must more than 0");
        require(addrArr.length <= _arrayLimit,"array's length must less than _arrayLimit");
        require(addrArr.length == amountArr.length,"wrong length");
