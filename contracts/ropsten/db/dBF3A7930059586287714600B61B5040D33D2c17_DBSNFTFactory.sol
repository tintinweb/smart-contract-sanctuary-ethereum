 ...
 * tokens.
 */
contract DBSNFTFactory is FactoryERC721, Ownable, ReentrancyGuard {
    ...
    event Minted(address indexed minter, address indexed account, uint256 tokenId, uint256 tableID);
    event Batchminted(address indexed minter, address indexed account, uint256[] tokens, uint256[] tableIDs);
    ...
    function _canMint(
        ...
        return (_fromAddress == owner()||
        (_tableId > 0 ) && isApprovedForAll(owner(), _fromAddress));
    }

    function _canMintBatch(
        address _fromAddress,
        uint256[] memory _tablesId
    ) internal view returns (bool) {
        for(uint256 i=0; i<_tablesId.length; i++){
            if(_tablesId[i] == 0){
                return false;
            }
        }
        return ((_fromAddress == owner())||isApprovedForAll(owner(), _fromAddress));
    }
    ...
    function canMint(uint256 _tableId)
        override
        external
        view
        returns (bool)
    {
        return _canMint(_msgSender(), _tableId);
    }
    ...
    function mint(
        ...
        require( _canMint(_msgSender(), _tableId),"DBSNFTFactory#_mint: CANNOT_MINT_MORE");
        uint256 tokenId = _mint(_tableId, _toAddress);

        emit Minted(_msgSender(), _toAddress, tokenId, _tableId);
        return tokenId;
    }

    /**
     * @dev minting in DBSNFTTokens!
     */
    function _mint(
        uint256 _tableId,
        address _toAddress
    ) internal returns (uint256){
        DBSNFTTokens tradable = DBSNFTTokens(nftAddress);
        uint256 tokenId = tradable.mint(_toAddress, _tableId);
        return tokenId;
    }

    /**
     * @dev batch minting in DBSNFTTokens!
     */
    function mintBatch(
        uint256[] memory _ids,
        address _toAddress
    ) override external nonReentrant() returns (uint256[] memory, uint256[] memory){
        require(_canMintBatch(_msgSender(), _ids),"DBSNFTFactory#_mint: CANNOT_MINTBATCH_MORE");
        (uint256[] memory tokens, uint256[] memory tableids) = _batchMint(_ids, _toAddress);

        emit Batchminted(_msgSender(), _toAddress, tokens, tableids);
    ...
    function _batchMint(
        ...
        DBSNFTTokens tradable = DBSNFTTokens(nftAddress);
        (uint256[] memory tokens, uint256[] memory tableids) = tradable.batchMint(_toAddress, _ids);
        return (tokens, tableids);
    }
}
...
contract DBSNFTTokens is ERC721Tradable{
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        string memory json = string(abi.encodePacked(
            '{"name":"', 
            tokenProperty[tokens[_tokenId]].name, 
            '","image":"',
            baseuri, 
            tokenProperty[tokens[_tokenId]].image, 
            '","description":"',
            tokenProperty[tokens[_tokenId]].description, 
            '","attributes":',
            tokenProperty[tokens[_tokenId]].attributes,
            '}'
            ));
        return json;
    }
    ...
    function checkId(uint256 _tableId) public override view returns(bool){
        require(_tableId > 0);
        return tokenProperty[_tableId].id > 0;
    }
}
...
abstract contract ERC721Tradable is ERC721Burnable, AccessControl, Ownable {
    ...
    */
    ...
    function burn(
        ...
        require(_exists(_id), "burn: token is not exists");
        _burn(_id);
        delete tokens[_id];
    }
    ...
    function mint(
        ...
        require(checkId(_id), "mint checkId error");
    ...
    function checkId(uint256 _tableId) public view virtual returns(bool);
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _tableIds.length; i++) {
            require(checkId(_tableIds[i]), "batchMint checkId error");
        }
