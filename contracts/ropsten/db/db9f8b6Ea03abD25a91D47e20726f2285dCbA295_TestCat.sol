...
contract TestCat is ERC721A, Ownable {
    bytes32 private _whitelistHash;

    uint256 public constant maxSupply = 4444;
    uint256 public WhitelistSupply = 200;
    uint256 public FCFSsupply = 200;
    uint256 public bonusSupply = 200;

    uint256 private constant maxPerAddress = 20;
    uint256 private constant maxPerFCFS = 5;
    uint256 private constant maxPerWhitelist = 10;

    uint256 public constant publicMintPrice = 0.015 ether;
    ...
    uint256 public nonPublicEndDate = 1647685209;
    uint256 public saleStartDate = 1646402400;

    string private baseUri =
        "https://gateway.pinata.cloud/ipfs/QmTMtE2msBEWfmAU37f268b125PhTsxAbwFRj6eX3aLt4q/";
    string private baseExtension = ".json";

    mapping(address => uint32) public WhitelistMintedByAddress;
    mapping(address => uint32) public FreeMintedByAddress;
    bool private revealed = false;
    bool public isBonusOpen = true;

    constructor() ERC721A("TESTCAT", "TESTCAT") {}
    ...
    function isNonPublicOpen() public view returns (bool) {
        return
            ...
            block.timestamp <= nonPublicEndDate;
    }
    ...
    function isBonusAvailable() public view returns (bool) {
        return bonusMintCounter < bonusSupply && isBonusOpen;
    }

    function setNonPublicSession(uint256 startDate, uint256 endDate)
        external
        onlyOwner
    {
        nonPublicStartDate = startDate;
        nonPublicEndDate = endDate;
    }
    ...
    function setFCFSSupply(uint256 amount) external onlyOwner {
        FCFSsupply = amount;
    }
    ...
    function setBonusState(bool state) external onlyOwner {
        isBonusOpen = state;
    }
    ...
    function _claimWithBonus(uint32 amount) private {
        ...
        require(
            bonusMintCounter + amount <= bonusSupply,
    ...
    function saleMint(uint32 amount) external payable onlyEOA {
        ...
        isBonusAvailable() ? _claimWithBonus(amount) : _claimSale(amount);
    }

    function FCFSMint(uint32 amount) private {
        require(isNonPublicOpen(), "Session Closed");
        require(totalSupply() + amount <= maxSupply, "Max Supply reached");
        require(
            (amount > 0) && (FCFSCounter + amount) <= FCFSsupply,
        ...
        require(
            FreeMintedByAddress[msg.sender] + amount <= maxPerFCFS,
            "Max Free per Address Reached"
        );
        FCFSCounter += amount;
