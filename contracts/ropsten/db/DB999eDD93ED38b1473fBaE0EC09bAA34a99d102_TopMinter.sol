...
;
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId));
        return append(_name,".topbidder.com/",tokenId);
    }
    function append(string memory a, string memory b, uint c) internal pure returns (string memory) {

        return string(abi.encodePacked(a, b, uintToString(c)));
