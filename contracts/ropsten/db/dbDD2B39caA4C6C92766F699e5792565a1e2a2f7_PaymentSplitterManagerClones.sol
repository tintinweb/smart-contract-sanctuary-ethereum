...
contract PaymentSplitterManagerClones is Ownable {
    ...
    * at `splitter`.
    */
    // function balanceOf(address splitter, address account) public view returns (uint256, uint256) {
    //     PaymentSplitterCloneable psc = PaymentSplitterCloneable(payable(splitter));

    //     uint256 ethBalance_ = ((psc.ethBalance() + psc.totalEthReleased()) * psc.shares(account)) / psc.totalShares() - psc.ethReleased(account);
    //     uint256 tokenBalance_ = ((psc.tokenBalance() + psc.totalTokenReleased()) * psc.shares(account)) / psc.totalShares() - psc.tokenReleased(account);
