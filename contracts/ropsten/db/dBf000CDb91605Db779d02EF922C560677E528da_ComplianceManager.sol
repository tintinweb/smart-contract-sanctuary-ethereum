...
contract ComplianceManager is IComplianceManager, AccessControl{
 ...
 */
    ...
    constructor(){
     ...
     function addBlackList(address addr) external onlyRole(INPUT_ROLE) {
        require(!checkBlackList(addr), "Address already in the BlackList");
        if(checkWhiteList(addr)) removeFromwhiteList(addr);
        blackList.push(addr);
        emit AddedToBlackList(addr, "Successfully added to BlackList");
    }
    ...
    function removeFromBlackList(address addr) external onlyRole(INPUT_ROLE) {
        require(checkBlackList(addr), "Address not in the BlackList");
        for(uint i=0; i < blackList.length; i++){
            if(blackList[i] == addr) delete blackList[i];
            emit RemovedFromBlackList (addr, "Successfully removed from BlackList");    
        }
    }
/**
 ...
 */
    function addwhiteList(address addr) external onlyRole(INPUT_ROLE) {
        require(!checkBlackList(addr),"Address in the BlackList. Remove it first");
        require(!checkWhiteList(addr),"Address already in the WhiteList");
        whiteList.push(addr);
        emit AddedToWhiteList(addr, "Successfully added to WhiteList");
    }
    ...
    function removeFromwhiteList(address addr) public onlyRole(INPUT_ROLE) {
        require(checkWhiteList(addr), "Address not in the whiteList");
        for(uint i=0; i < whiteList.length; i++){
            if(whiteList[i] == addr) delete whiteList[i];
            emit RemovedFromWhiteList (addr, "Successfully removed from WhiteList");
        } 
    }
}
