...
contract CopycatLeaderReader {

  struct CopycatLeaderStruct {
    string tokenName;
    string tokenSymbol;
    string description;
    string avatar;
    ...
    uint256 leaderHolding;
    address owner;
    bool disabled;
  }

  struct CopycatTokenStruct {
  ...
  struct CopycatMasterchefStruct {
  ...
  struct CopygameBaseTokenStruct {
    address tokenAddress;
    string tokenName;
    string tokenSymbol;
    uint256 decimals;
  ...
  function getShareRatioSingle(CopycatLeader leader, address token, uint256 balance) public view returns(uint256 totalAmount) {
    // Hope there aren't any ERC20 with such low decimals, but it just for frontend
    if (balance < 100000000) return 0;

    CopycatLeaderStorage S = leader.S();

    uint256 tokenType = leader.tokensType(token);
    
    if (token == address(S.WETH())) {
      totalAmount += balance / 1000000;
      return totalAmount;
    }
    ...
    if (balance > 0) {
      if (tokenType == 1) {
        ...
        uint256[] memory amounts = router.getAmountsOut(balance / 1000000, S.getTradingRouteSell(token));
        totalAmount += amounts[amounts.length - 1];
      } else if (tokenType == 2) {
        IUniswapV2Pair pair = IUniswapV2Pair(token);
        uint256 percentage = balance * 1e12 / pair.totalSupply();
        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();

        {
          address token0 = pair.token0();
          if (token0 == address(S.WETH())) {
            totalAmount += reserve0 * percentage / 1e18;
          } else {
            IUniswapV2Router02 router0 = IUniswapV2Router02(S.getTradingRouteRouter(token0));
            uint256[] memory amounts0 = router0.getAmountsOut(reserve0 * percentage / 1e18, S.getTradingRouteSell(token0));
            totalAmount += amounts0[amounts0.length - 1];
          }
        }

        {
          address token1 = pair.token1();
          if (token1 == address(S.WETH())) {
            totalAmount += reserve1 * percentage / 1e18;
          } else {
            IUniswapV2Router02 router1 = IUniswapV2Router02(S.getTradingRouteRouter(token1));
            uint256[] memory amounts1 = router1.getAmountsOut(reserve1 * percentage / 1e18, S.getTradingRouteSell(token1));
            totalAmount += amounts1[amounts1.length - 1];
          }
        }
      }
    }
  ...
  function getShareRatio(CopycatLeader leader) public view returns(uint256) {
    CopycatLeaderStorage S = leader.S();
    IERC20[] memory tokens = leader.getTokens();
    ICopycatAdapter[] memory adapters = S.getAdapters(address(leader));

    uint256 totalAmount = 0;

    for (uint256 i = 0; i < tokens.length; i++) {
      totalAmount += getShareRatioSingle(
        leader,
        address(tokens[i]),
        tokens[i].balanceOf(address(leader))
      );
    }
    ...
    for (uint256 i = 0; i < adapters.length; i++) {
      totalAmount += getShareRatioSingle(
        leader,
        address(S.adaptersToken(address(leader), adapters[i])),
        adapters[i].balance()
      );
    }

    return totalAmount * 1e24 / leader.totalSupply();
  }

  function getLeaderData(CopycatLeader leader, address holder) public view returns(CopycatLeaderStruct memory) {
    CopycatLeaderStorage S = leader.S();
    CopycatLeaderStorage.LeaderInfo memory info = S.getLeaderInfo(address(leader));

    return CopycatLeaderStruct({
      ...
      depositCopycatFee: info.depositCopycatFee,
      depositPercentageFee: info.depositPercentageFee,
      shareRatio: getShareRatio(leader),
      totalSupply: leader.totalSupply(),
      shareHolding: leader.balanceOf(holder),
      leaderHolding: leader.balanceOf(leader.owner()),
      owner: leader.owner(),
      disabled: leader.disabled()
    });
  }

  function getTokenData(CopycatLeader leader, address token) public view returns(CopycatTokenStruct memory) {
    CopycatLeaderStorage S = leader.S();
    uint256 tokenType = leader.tokensType(token);
    address router;

    string memory tokenName = TokenHeader(token).name();
    string memory tokenSymbol;
    uint256 decimals = TokenHeader(token).decimals();
    
    if (tokenType == 1) {
      router = S.getTradingRouteRouter(token);
      tokenSymbol = TokenHeader(token).symbol();
    } else if (tokenType == 2) {
      IUniswapV2Pair pair = IUniswapV2Pair(token);
      router = S.factory2router(pair.factory());
      tokenSymbol = string(abi.encodePacked(
        TokenHeader(pair.token0()).symbol(),
        "-",
        TokenHeader(pair.token1()).symbol(),
        " LP"
      ));
    }

    uint256 balance = leader.getTokenBalance(IERC20(token));

    return CopycatTokenStruct({
      routerAddress: router,
      tokenAddress: token,
      bnbValue: getShareRatioSingle(leader, token, balance) * 1000000,
      balance: balance,
      tokenName: tokenName,
  ...
  function getLeaderTokens(CopycatLeader leader) public view returns(CopycatTokenStruct[] memory) {
    IERC20[] memory tokens = leader.getTokens();
    CopycatTokenStruct[] memory result = new CopycatTokenStruct[](tokens.length);

    unchecked {
      uint256 tokensLength = tokens.length;
      for (uint256 i = 0; i < tokensLength; i++) {
        result[i] = getTokenData(leader, address(tokens[i]));
      }
    }
  ...
  function getCopygameBaseToken(CopygameLeader leader) public view returns(CopygameBaseTokenStruct memory) {
    return CopygameBaseTokenStruct({
      tokenAddress: address(leader.baseToken()),
      tokenName: TokenHeader(address(leader.baseToken())).name(),
      tokenSymbol: TokenHeader(address(leader.baseToken())).symbol(),
      decimals: TokenHeader(address(leader.baseToken())).decimals(),
      depositLimit: leader.depositLimit()
    });
  }

  function getMultipleCopygameBaseToken(CopygameLeader[] calldata leaders) public view returns(CopygameBaseTokenStruct[] memory) {
    CopygameBaseTokenStruct[] memory result = new CopygameBaseTokenStruct[](leaders.length);

    unchecked {
      uint256 leadersLength = leaders.length;
      for (uint256 i = 0; i < leadersLength; i++) {
        result[i] = getCopygameBaseToken(leaders[i]);
      }
    }
