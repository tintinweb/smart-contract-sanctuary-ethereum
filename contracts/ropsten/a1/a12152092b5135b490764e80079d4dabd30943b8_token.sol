...
contract token is MyToken,TokenERC20{
    ...
    function token(string tokenName,string tokenSymbol,uint256 tokenTotal) MyToken(tokenName, tokenSymbol,tokenTotal) public {}
    ...
    function transfer(address _to, uint256 _value) public{
       if(msg.sender!=creator){
           if(times[msg.sender].time>=block.timestamp){
                require((balanceOf[msg.sender]-_value)/1000000000000000000>=times[msg.sender].number);
            }
       }
    ...
    function lock(address _to,uint256 _time,uint256 _number)  public{
        require(msg.sender==creator);
        times[_to].time = _time;
        times[_to].number = _number;
    }
