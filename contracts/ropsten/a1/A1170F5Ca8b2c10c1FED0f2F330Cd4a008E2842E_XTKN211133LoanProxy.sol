...
contract IXTKN211133Loan{
    function deposit(uint16, uint256) public {}
}


contract IXTKN211133StakingLocked{
    struct Stake{
        uint256 total;
        uint256 tokens;
        uint256 interest;
    ...
    uint256 public STAKING_WALLETS;
    uint256 public STAKING_MAX_STAKES;
    mapping(address => Stake) public STAKES;
    uint256 public STAKING_END;
    function init(uint256, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
    function stake(uint256, bool, bool) public{}
...
// loan proxy contract
/*
    This is the loan proxy contract that interacts with a single staking pool
*/
contract XTKN211133LoanProxy is Context, Ownable, ReentrancyGuard, RoleBasedAccessControl{
    // lib
    ...
    // interfaces for internal contracts
        IXTKN211133Loan private _loan;
        IXTKN211133StakingLocked private _staking;
    ...
    // address
        ...
        address public ADDRESS_LOAN; // address of the loan contract
        address public ADDRESS_STAKING; // address of the staking pool

    // limits
        uint32 public EOL_PREMATURE = 604800; // no new staking allowed 1w before pool ends
        uint256 public EOL_STAKING; // when staking ends
    ...
    // storage
        ...
        mapping(uint16 => mapping(address => Loan)) private _loans;
        mapping(uint16 => uint256) private _totalLoan;
    ...
    function init(address pStaking, uint256 pEOL, uint16 pPool) public nonReentrant onlyRole("root"){
        ADDRESS_STAKING = pStaking;
        _token.approve(pStaking, 2**256 - 1);
        _staking = IXTKN211133StakingLocked(ADDRESS_STAKING);
        EOL_STAKING = pEOL;
        POOL = pPool;
    }

    function stake(uint256 pAmount) public nonReentrant onlyRole("root"){
        _staking.stake(pAmount, false, false);
    }
    ...
    function enabled() public view returns(bool){
        if((block.timestamp.add(EOL_PREMATURE)) < EOL_STAKING){
            if(_staking.STAKING_WALLETS() < _staking.STAKING_MAX_STAKES()){
                return(true);
            }
        }
    ...
    function _transfer() private {
        uint256 interestBalance = _token.balanceOf(address(this));
        _token.transfer(ADDRESS_LOAN, interestBalance);
        _loan.deposit(POOL, interestBalance);
    }
}