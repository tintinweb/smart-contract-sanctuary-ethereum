...
contract Staking{
    ...
    // 1% interst for each 15 minutes
    uint256 public interestRate = 1; 
    uint public interestPeriod = 15 minutes;
    // minimum stake amount;
    uint public minimum = 1e6;
    ...
    event ClaimInterst(address user, uint interstValue);

    function stake(uint amount) external {
        _checkRequire(amount);
        address user = msg.sender;

        _transferFromtoken(user,amount);

        if(stakerInfo[user].amount > 0){
            _claimInterst(user,  block.timestamp,  stakerInfo[user].lastInterestClaimed);
        }   

        stakerInfo[user].amount += amount;
        stakerInfo[user].lastInterestClaimed = block.timestamp;
    ...
    function unstake() external {
        address user = msg.sender;
        uint256 amount = stakerInfo[user].amount;
        require(amount > 0, "Not staked");
        _withdraw(user, amount, block.timestamp);
        emit UnStake(user, amount);
    }

    function withdraw(uint amount) external {
        _checkRequire(amount);
        _withdraw(msg.sender, amount, block.timestamp);
    }


    function claimInterest() external {
        address user = msg.sender;
        require(stakerInfo[user].amount > 0, "Not staked");
        _claimInterst(user,  block.timestamp,  stakerInfo[user].lastInterestClaimed);
    }

    function _checkRequire(uint amt) internal view {
        require(amt >= minimum, "Amount less than min");
    }

    function _withdraw(address user, uint amount, uint blocktimeStamp) internal {
        _claimInterst(user, blocktimeStamp, stakerInfo[user].lastInterestClaimed);
        stakerInfo[user].amount -= amount;
        stakerInfo[user].lastInterestClaimed = blocktimeStamp;
        _transfertoken(user,amount);
        emit Withdraw(user, amount);
    }

    function _claimInterst(address user, uint256 blockTimeStamp, uint lastInterestClaimed) internal {
        uint intrs;
        if((lastInterestClaimed + interestPeriod ) > blockTimeStamp ){ 

            intrs = (lastInterestClaimed - blockTimeStamp) / interestPeriod;
            intrs = ((intrs * interestRate)  * stakerInfo[user].amount) / 100;
            
            stakerInfo[user].totalAmount +=  intrs;
            _transfertoken(user,intrs);
        }
        emit ClaimInterst(user, intrs);
    }

    function _transfertoken(address to, uint amount) internal {
        USDT.safeTransfer(to, amount);
    }

    function _transferFromtoken(address from, uint amount) internal {
...
contract BorrowNFT is Ownable, ERC721Holder{
    ...
    function withdrawUSDT(address receiver, uint256 amount) external onlyOwner {
