...
contract DonationCenter is Ownable {
    ...
    /**
     * @dev Required to be used with upgradable pattern
     *   for initializing contract instead of constructor
     *
     * @param commissionRate_ commission rate for donations
     * @param beneficiary_ address that will collect the commission
     */
    constructor(uint commissionRate_, address beneficiary_) {
    ...
    function donate(address receiver_, string memory nickname_, string memory message_) external payable {
        ...
        _balances[beneficiary] = _balances[beneficiary].add(commissionAmount);
        _balances[receiver_] = _balances[receiver_].add(donationAmount);
    ...
    function withdraw(uint amount_) external {
        ...
        require(balanceOf(receiver) >= amount_, "Insufficient balance");

        _balances[receiver] = _balances[_msgSender()].sub(amount_);

        (bool success, ) = payable(receiver).call{value: amount_}("");
     ...
     */
    function balanceOf(address address_) internal view returns(uint) {
        return _balances[address_];
    }

    /**
     * @dev Returns available balance of sender..
     *
     * @return amount of wei available to withdraw
     */
