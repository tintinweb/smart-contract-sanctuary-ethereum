...
contract GRInsurance is Ownable {
    ...
    function addInsurance(uint _id, string memory _currencyName, uint _oneCoinPrice, uint _insurancePrice, string memory _buyEndDateAt, string memory _endDateAt) external onlyOwner {
        Insurance memory new_insurance = Insurance(_id, _currencyName, _oneCoinPrice, _insurancePrice, _buyEndDateAt, _endDateAt);
        insurances.push(new_insurance);
    }

    function getInsuranceById(uint _id) public view returns (uint id, string memory currencyName, uint oneCoinPrice, uint insurancePrice, string memory buyEndDateAt, string memory endDateAt) {
        for (uint i = 0; i <= insurances.length; i++) {
            Insurance storage insurance = insurances[i];
            if (_id == insurance.id) {
                return (insurance.id, insurance.currencyName, insurance.oneCoinPrice, insurance.insurancePrice, insurance.buyEndDateAt, insurance.endDateAt);
            }
        }
        return (0, '', 0, 0, '', '');
    }

    function addInsuranceUser(uint _id) public {
