...
contract Bond is Ownable, Pausable {
  ...
  function calculateDrip(uint256 poolID) internal virtual returns(uint256, uint256){
      ...
      if(claimedAt[poolID]> stakingStartTime[poolID] || restakingAt[poolID]> stakingStartTime[poolID]){
        if(claimedAt[poolID] > restakingAt[poolID]){
            stakingPeriodInDaysAfterClaim = block.timestamp.sub(claimedAt[poolID]).div(120);
        }else{
            stakingPeriodInDaysAfterClaim = block.timestamp.sub(restakingAt[poolID]).div(120);
        }
      }else{
  ...
  function restake(uint256 poolID) public whenNotPaused {
      ...
      require(stakeTime < 365, "can't restake after 365 days");
