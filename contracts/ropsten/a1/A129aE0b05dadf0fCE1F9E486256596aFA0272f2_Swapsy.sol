...
contract Swapsy is ReentrancyGuard, Ownable {
    ...
    function _buy(uint256 id) internal {
        ...
        _revenue[_swaps.tokenIn] += sellerFeeAmt;
        if (_swaps.tokenOut == address(0)) {
            (bool sent, ) = payable(_swaps.seller).call{value: sellerTransAmt}("");
            require(sent, "Swapsy: payment failed");
        } else {
            require(
    ...
    function getRevenue(address token) external view onlyOwner returns (uint256 amount) {
