...
contract LockLPTokens is Ownable {
    ...
    function stakeLP(
        ...
        address token0Pfan = uniPair.token0();
        address token1 = uniPair.token1();

        if (
            msg.value > 0 &&
            (token0Pfan == router.WETH() || token1 == router.WETH())
        ) {
            //keep token0Pfan pfan and not eth. add Eth liquidity separately
            token0Pfan = token0Pfan == router.WETH() ? token1 : token0Pfan;
            //check if _amountA is for pfan token(token0). PFAN amount should be larger than eth
            _amountA = _amountA > _amountB ? _amountA: _amountB;
            IERC20(token0Pfan).transferFrom(msg.sender, address(this), _amountA);
        ...
        } else {// other pairs that is not eth, as long as amounts match, sequence does not matter.
    ...
    function getRewardBundle(uint256 amount, uint256 _lockingDays)
        ...
        returns (uint256)
    {
        return InterestMath.getInterest(amount, _lockingDays, rewardAdjustPercent);
    }
    ...
    function calculateDays(uint256 _depositTime) public view returns (uint256) {
        ...
        //  Update to minutes if running some test suites
    ...
    function adjustRewardPercent(uint256 _rewardAdjustPercent) public onlyOwner {
...
library InterestMath {
    ...
    function getInterest(uint256 pFanAmount, uint256 daysStaked, uint256 rewardAdjustPercent)
        internal
        pure
        returns (uint256)
    {
        ...
        uint256 eq_1 = r_numerator.mul(1e18).div(r_denominator).mul(rewardAdjustPercent).div(100);
