...
contract FixedStablecoins is Constants {
    ...
    function underlyingTokens()
        internal
        view
        returns (address[N_COINS] memory tokens)
    {
        tokens[0] = DAI;
        tokens[1] = USDC;
        tokens[2] = USDT;
    ...
    function decimals()
        internal
        view
        returns (uint256[N_COINS] memory _decimals)
    {
...
interface ICurve3Pool {
    ...
    function calc_token_amount(uint256[3] calldata inAmounts, bool deposit)
        external
        view
        returns (uint256);
...
interface ICurve3Deposit {
    ...
    function remove_liquidity(uint256 amount, uint256[3] calldata min_uamounts)
        external;
...
interface ICurveZap {
    ...
    function remove_liquidity(uint256 amount, uint256[4] calldata min_uamounts)
        external;
    ...
    function calc_token_amount(uint256[4] calldata inAmounts, bool deposit)
        external
        view
        returns (uint256);
...
interface IBuoy {
    ...
    function updateRatiosWithTolerance(uint256 tolerance)
        external
        returns (bool);
    ...
    function stableToLp(uint256[3] calldata inAmount, bool deposit)
        external
        view
        returns (uint256);
...
contract Buoy3Pool is FixedStablecoins, Controllable, IBuoy, IChainPrice {
    ...
    function healthCheck(uint256 tolerance)
        external
        view
        ...
        returns (bool)
    {
        require(
            ...
            "updateRatiosWithTolerance: !authorized"
        );
    ...
    function lpToUsd(uint256 inAmount)
        external
        view
        override
        returns (uint256)
    ...
    function usdToLp(uint256 inAmount)
        external
        view
        override
        returns (uint256)
        ...
        returns (uint256[N_COINS] memory balances)
    {
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            _balances[i] = (
                IERC20(getToken(i)).balanceOf(address(curvePool)).mul(inAmount)
            )
            .div(totalBalance);
        }
        balances = _balances;
    ...
    function _lpToUsd(uint256 inAmount) internal view returns (uint256) {
        return
            inAmount.mul(curvePool.get_virtual_price()).div(
    ...
    function _singleStableFromLp(uint256 inAmount, int128 i)
        internal
        view
        returns (uint256)
    {
    ...
    function _usdToLp(uint256 inAmount) internal view returns (uint256) {
        return
            inAmount.mul(DEFAULT_DECIMALS_FACTOR).div(
    ...
    function getPriceFeed(uint256 i) external view override returns (uint256) {
        int256 _price;
        (, _price, , , ) = AggregatorV3Interface(getAggregator(i))
        .latestRoundData();
    ...
    /// @param i Token in
    function getTokenRatios(uint256 i)
        private
        view
        returns (uint256[N_COINS] memory _ratios)
    {
        int256[N_COINS] memory _prices;
        (, _prices[0], , , ) = AggregatorV3Interface(getAggregator(0))
        .latestRoundData();
        (, _prices[1], , , ) = AggregatorV3Interface(getAggregator(1))
        .latestRoundData();
        (, _prices[2], , , ) = AggregatorV3Interface(getAggregator(2))
        .latestRoundData();
        _ratios[0] = uint256(_prices[0])
        .mul(CHAINLINK_PRICE_DECIMAL_FACTOR)
        .div(uint256(_prices[1]));
        _ratios[1] = uint256(_prices[0])
        .mul(CHAINLINK_PRICE_DECIMAL_FACTOR)
        .div(uint256(_prices[2]));
        _ratios[2] = uint256(_prices[2])
        .mul(CHAINLINK_PRICE_DECIMAL_FACTOR)
        .div(uint256(_prices[1]));
        return _ratios;
    }
    ...
    function _updateRatios(uint256 tolerance) private returns (bool) {
        ...
        for (uint256 i = 1; i < N_COINS; i++) {
            _ratio = curvePool.get_dy(int128(0), int128(i), getDecimal(0));
            check = abs(
                int256(_ratio) - int256(chainRatios[i - 1].div(CHAIN_FACTOR))
            );
            if (check > tolerance) {
                return false;
            } else {
