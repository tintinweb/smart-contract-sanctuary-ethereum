...
contract ERC721_QUOKKA is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    //Sale start times
    uint public preSaleStart = 1632672000;    // TODO UPDATE DATE: Early Access Time for Friends and Family: 9AM (PST)
    
    uint public publicSaleStart = 1632672000;
    ...
    function withdraw(address payable target, uint amount) public onlyOwner {
        target.send(amount);
    }
    ...
    function publicSale(uint purchaseAmmt) payable public freezable {
        ...
        require(purchaseAmmt * publicPrice <= msg.value, "Not enough ETH sent");
    ...
    function whitelistMint(uint index, uint memberType, uint claimAmmt, uint claimTotal, bytes memory signature) payable public freezable {
        ...
        require(claimSigner == owner(), "Invalid Message Signer.");
