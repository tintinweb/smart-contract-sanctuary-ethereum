...
contract StreetCredLedger is Ownable {
  ...
  function getAvailableContracts() public view returns (address[] memory) {
    return availableContracts;
  }

  function fillArray(address[] memory _addresses) public onlyOwner {
    for (uint256 i = 0; i < _addresses.length; i++) {
      availableContracts.push(_addresses[i]);
    }
  }
}
