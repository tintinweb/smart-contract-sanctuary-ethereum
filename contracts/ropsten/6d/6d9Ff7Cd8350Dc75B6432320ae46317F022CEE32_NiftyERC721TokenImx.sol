...
contract NiftyERC721TokenImx is ERC721Mintable, Royalties, Signable, Withdrawable, Ownable {    
    ...
    // Immutable X Minting Address For L2 -> L1 Bridging
    address public imx;
    ...
    modifier onlyAdminOrIMX() {
        require(_msgSender() == imx || _isAdminOrValidNiftySender(), "Function can only be called by admin or IMX");
        _;
    }
    ...
    function setImx(address imx_) public {
    ...
    function prepareMintImx(uint256[] calldata amounts, string[] calldata ipfsHashes) external {
        _requireOnlyValidSender();
    ...
    ) external onlyAdminOrIMX {
        require(quantity == 1, "Mintable: invalid quantity");
        (uint256 id, bytes memory blueprint) = _split(mintingBlob);        
        _safeMint(user, id);
    ...
    function _isAdminOrValidNiftySender() private view returns (bool) {
        address currentAdmin = admin;     
        if(currentAdmin == address(0)) {
            return permissionsRegistry.isValidNiftySender(_msgSender());
        } else {
            return _msgSender() == currentAdmin;
        }
    }

    // Split the minting blob into token_id and blueprint portions
    // {token_id}:{blueprint}
    function _split(bytes calldata blob) private pure returns (uint256, bytes memory)
    {
        int256 index = BytesImx.indexOf(blob, ":", 0);
        require(index >= 0, "Separator must exist");
        // Trim the { and } from the parameters
        uint256 tokenID = BytesImx.toUint(blob[1:uint256(index) - 1]);
...
abstract contract ERC721Mintable is ERC721 {    
    ...
    function _safeMint(
        ...
        _mint(to, tokenId);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), ERROR_NOT_AN_ERC721_RECEIVER);
    }
