...
contract FL {
    ...
    function upload(update[5] calldata updates) public {
        bool flag;
        for (uint i = 0; i < 5; i++) {
            flag = true;
            for (uint j = 0; j < updatesCnt; j++) {
                if (updateList[j].i == updates[i].i && updateList[j].j == updates[i].j)
                    if(stringsEqual(updateList[j].key, updates[i].key)) {
