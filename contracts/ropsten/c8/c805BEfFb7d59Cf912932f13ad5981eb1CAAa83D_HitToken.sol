...
contract HitToken is Lock {
    ...
    */
    function HitToken(string _name, string _symbol, uint8 _decimals, uint256 _initialSupply,address _teamAccount,address _subFundAccount) public {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
        //定义公募账户
        ...
        //将私募总值，基金会总值初始放到owner账户
        balances[msg.sender] = mainFundBalance.add(hitFoundationBalance); 
        //将团队总值放到团队账户
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        if(teamLockTime[msg.sender] > 0){
             return super.teamLockTransfer(_to,_value);
                ...
                if(msg.sender==owner)
                {
                    require(_value<mainFundBalance);
                    super.transfer(_to, _value);
                    hitFoundationBalance = hitFoundationBalance.sub(_value);
                }
          else 
              return super.transfer(_to, _value);
