...
contract ERC721Claimable is ERC721, Ownable {
    ...
    function getPrice(uint256 tokenId) public view  returns (uint256){
        uint256 nonPricedElements = (reservedAmount + freeAmount);
        if (tokenId < nonPricedElements)
            return 0;
        return startPrice + (((endPrice - startPrice) / (totalSupply - nonPricedElements)) * (tokenId - nonPricedElements));
    }
