...
contract NFT{
   ...
   function uint_to_str(uint256 _i) public pure returns (string memory str){
    ...
    function a(string calldata morse_code) public returns(string memory){
        ...
        while (remain >= number_of_newline){
            line += 1;
            now_morse_text_list.push(morse_code[first:end]);
            remain -= number_of_newline;
            first = end;
            end = first + number_of_newline;
        }

        now_morse_text_list.push(morse_code[first:]);

        // uint x = 380 -(15/2)*(line);
        uint x = uint(380 -7*uint(line));
        uint y = first_y;
        x += add_x;

        string memory svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="794" height="488"> <rect width="794" height="488" style="fill:none"></rect>';

        for (uint i=0;i <= now_morse_text_list.length;i++){

            // for (w in now_morse_text_list[secondary]){ 
            for (uint y=0;y<=bytes(now_morse_text_list[i]).length;y++){ 
                // context.fillText(now_morse_text_list[secondary][w], x, y);
                // now_morse_text_list[i][y]
                svg = splicing(svg,"<text x=");
                svg = splicing(svg,uint_to_str(x));
                svg = splicing(svg," y=");
                svg = splicing(svg,uint_to_str(y));
                // svg = splicing(svg,string(x));
                y += line_spacing_y;
            }
