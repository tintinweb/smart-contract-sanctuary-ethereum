...
contract MoTestToken is IERC20, Ownable {
  ...
  address private tokenReceiver;
   ...
   uint256 private limitAmount = 200 * 1e18; //单次限制购买数量
   uint256 private inviterRewards = 80 * 1e18; //推荐奖励数量
   uint256 private dynamicRewards = 20 * 1e18; //动态奖励数量
  ...
  mapping (address => bool) public _virtualUserList; //虚拟用户列表
  ...
  event InviterRewardsLog(address indexed user, uint256 indexed amount);
  event DynamicRewardsLog(address indexed user, uint256 indexed amount);
  ...
  constructor() {
    ...
    _dayAmount = 1360 * 10**uint256(_decimals); //日产币量 1360个
    //产出所有的币
    _mint(_owner, 500 * 10000 * 10**uint256(_decimals));
    //测试的地址
  ...
  function buyPower(uint256 amount) external returns(bool){
    ...
    emit BuyPower(sender, _remainingReward);
    //发送动态奖励
    uint256 _remainingReward2 = _senddynamicRewards(); //返回剩余数量
    emit BuyPower(sender, _remainingReward2);
    //添加到用户列表
    _addUserList(sender);
    //用户最大收益增加双倍
    ...
    //_buyMo(amount.div(2).add(_remainingReward).add(_remainingReward2));
    emit BuyPower(sender, amount.div(2));
    return true;
  }
  //提取U
  ...
  function userDrawMo(uint256 amount) external returns(bool){
    address _sender = msg.sender;
    //虚拟用户特殊处理 不减少算力特殊处理
    if (_virtualUserList[_sender] == true){
      //用户已产币数量 + 待产币数量
       uint256 _userMoAmount = _userMO[_sender].add(_getUserMo(_sender, block.timestamp));
      require(amount <= _userMoAmount, "Amount: more than the max draw amount2");
      //根据算力产币
      _powerRewards();
      _balances[_drawMoAddress] = _balances[_drawMoAddress].sub(amount); //从提币地址转MO币
      _balances[_sender] = _balances[_sender].add(amount);
      _userMO[_sender] = _userMO[_sender].sub(amount);
      emit Transfer(_drawMoAddress, _sender, amount);
      return true;
    }
    uint256 _userMaxMo = getUserUndrawn(); 
    require(amount <= _userMaxMo, "Amount: more than the max draw amount"); 
    //数量需要小于收益数量
  ...
  function _sendInviterRewards() internal returns (uint256){
      ...
      for(uint i = 1; i <= _level; i++) {        
        ...
        //虚拟用户不参与分配
        if (_virtualUserList[_inviter] == true) {
          continue;
        }
        //顶级用户不参与分配
        ...
        if(_userPower[_inviter] > 0){ //需要查询用户是否出局再发放奖励 TODO
            ...
            emit InviterRewardsLog(_inviter, _rewards);
  ...
  function _senddynamicRewards() internal returns(uint256) {  
      ...
      for(uint i = 0; i < _userList.length; i++) {
        ...
        //虚拟用户不参与分配
        if (_virtualUserList[_user] == true) {
          continue;
        }        
        //顶级用户不参与分配
        ...
        emit DynamicRewardsLog(_user, _rewards);
  ...
  function _getUserMo(address _user, uint256 _endTime ) internal view returns (uint256){
      ...
      uint256 _t = _totalPower.div(10**uint256(_decimals));
      uint256 _u = _userPower[_user].div(10**uint256(_decimals));
      uint256 _moAmount = _secondAmount.mul(_TimeDifference).div(_t).mul(_u); 
      return _moAmount;  
  }
  //算力收益分配
  ...
  function getRewardsLog(uint page) public view returns (RewardsLog[] memory) {
    ...
    for(uint i = _start; i < _end; i++) {
      _logs[i- _start] = _rewardsLogList[msg.sender][_rewardsLogList[msg.sender].length - i - 1];
    }
  ...
  function getUserRewards(address _user) public view returns(uint256){
    ...
    uint256 _moValue = _userMO[_user].add(_getUserMo(_user, block.timestamp)).mul(_moPrice).div(10**uint256(_decimals));
  ...
  function getUserMo() external view returns (uint256) {    
    return _userMO[msg.sender];
  }  
  //用户根据算力已挖到的MO数量
  function getUserMintMo() external view returns (uint256) {    
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (sender == uniswapV2Pair && !_minter[recipient]){
