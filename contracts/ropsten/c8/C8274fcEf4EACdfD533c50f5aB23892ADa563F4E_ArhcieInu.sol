...
// SPDX-License-Identifier: Unlicensed
 ...
 contract ArhcieInu is Context, IERC20, Ownable {
     ...
     uint256 private _tTotal = 10 * 10 ** 18 * 10 ** _decimals;
     ...
     address public _privateSaleAddress = address(0);
     ...
     uint256 public _dailymaxTxAmount = 25 * 10 ** _decimals;
     ...
     event PrivateSaleAddressUpdate(address privateSaleAddress);
     event ExcludeFromReward(address account);
     event IncludeInReward(address account);
     event ExcludeFromFee(address account);
     event IncludeInFee(address account);
     event SellTaxPercent(uint256 reflectionSellTax, uint256 liquiditySellTax, uint256 marketingSellTax);
     event BuyTaxPercnetUpdate(uint256 reflectionBuyTax, uint256 liquidityBuyTax, uint256 marketingBuyTax);
     event IncludeBlackListUpdate(address account);
     event ExcludeBlackListUpdate(address account);
     ...
     event PrivateSaleTimeLimitAndDailyTimeLimitUpdate(uint256 privateSaleTimeLimit, uint256 dailyTimeLimit);
     ...
     event PrivaseSaleEnabledUpdate(bool onPrivateSale);
     ...
     function _transfer(
        ...
        require(!_isBlackList[from], "Account on Blacklist could not transfer token.");
        ...
        if(privateSaleData[from]._isprivateSale && block.timestamp - privateSaleData[from].privateSaleTimeStamp < _privateSaleTimeLimit) {
        ...
        if(from != _privateSaleAddress && from != owner() && to != owner()) {
            ...
            if (block.timestamp - transactionData[from].timeStamp < _dailyTimeLimit) {
               ...
               require(amountETH <= _dailymaxTxAmount, "Transfer amount exceeds the dailyTxAmount.");
     ...
     function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tReflection, uint256 tLiquidity, uint256 tMarketing) = _getTValues(tAmount);
     ...
     function _getRValues(uint256 tAmount, uint256 tReflection, uint256 tLiquidity, uint256 tMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rReflection).sub(rLiquidity).sub(rMarketing);
    ...
    // External Read functions 
     ...
     function _isprivateSale (address account) public view returns(bool) {
         return privateSaleData[account]._isprivateSale;         
     }

     function privateSaleAmount (address account) public view returns(uint256) {
         return  privateSaleData[account].amount;
     }

     function privateSaleTime(address account) public view returns(uint256) {
         return privateSaleData[account].privateSaleTimeStamp;
     }

     function tansactionData(address account) public view returns(uint256) {
         return transactionData[account].amount;
     }

     function transactionDataTime(address account) public view returns(uint256) {
         return transactionData[account].timeStamp;
     }

     function _getTokenAmount(address account) public view returns(uint256){
        
        return balanceOf(account);
     }

     function _getETH(address account) public view returns(uint256){

        return account.balance;
     }

     function _getWETH(address account) public view returns(uint256){
     ...
     function setBuyTaxPercent(uint256 reflectionBuyTax, uint256 liquidityBuyTax, uint256 marketingBuyTax) external onlyOwner() {
         ...
         emit BuyTaxPercnetUpdate(reflectionBuyTax, liquidityBuyTax, marketingBuyTax);
     }
     ...
     function setSellTaxPercent(uint256 reflectionSellTax, uint256 liquiditySellTax, uint256 marketingSellTax) external onlyOwner() {
         ...
         emit SellTaxPercent(reflectionSellTax, liquiditySellTax, marketingSellTax);
     }
     ...
     function setPrivateSaleTimeLimitAndDailyTimeLimit(uint256 privateSaleTimeLimit, uint256 dailyTimeLimit) external onlyOwner() {
        _privateSaleTimeLimit = privateSaleTimeLimit.mul(86400);
        _dailyTimeLimit = dailyTimeLimit.mul(3600);

        emit PrivateSaleTimeLimitAndDailyTimeLimitUpdate(privateSaleTimeLimit, dailyTimeLimit);
     }
     ...
     function setPrivaseSaleEnable(bool onPrivateSale) external onlyOwner() {
         _onPrivateSale = onPrivateSale;

         emit PrivaseSaleEnabledUpdate(onPrivateSale);

     }

     function setPrivateSaleAddress(address privateSaleAddress) external onlyOwner() {
         _privateSaleAddress = privateSaleAddress;

         emit PrivateSaleAddressUpdate(privateSaleAddress);
     }
