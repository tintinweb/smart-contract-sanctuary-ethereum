...
contract SpaceBotLowerGas is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 constant PRESALE_MAX_Wallet = 1;
  uint256 constant PUBLIC_MAX_Wallet = 5;
  ...
  constructor() ERC721("NAME", "SYMBOL") {
    setHiddenMetadataUri("ipfs://ipfs://QmeYW66uGTJ6ay1hyvesv4E3QA8oVmKj4r9UqResLmUwFx/hidden.json");
  ...
  function publicMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(publicMintsPerAddress[msg.sender] + _mintAmount <= PUBLIC_MAX_Wallet, "only 5 per address");
  ...
  function presale(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(preMintsPerAddress[msg.sender] + _mintAmount <= PRESALE_MAX_Wallet, "only 1 per address");
