...
import "./interfaces/IScribePriceOracle.sol";


contract Scribe is IScribe, ReflectToken {
    using SafeMath for uint256;
    using Percent for Percent.Percent;
    using Set for Set.AddressSet;

    uint256 public constant initialTotalSupply = 985000000 * 10**18;
    ...
    IScribePriceOracle public priceOracle;
    uint256 public initialPrice;
    
     /**
     * @dev Initializes the contract excluding itself and the owner from dex fee and limits.
     */
    constructor() public ReflectToken("Scribe Token", "SCRIBE", initialTotalSupply) {
    ...
    function initializePriceOracle(address priceOracleAddress) external onlyOwner {
        priceOracle = IScribePriceOracle(priceOracleAddress);
        if (initialPrice == 0) {
    ...
    */
    ...
    function setSellAccumulationFee(uint128 numerator, uint128 denominator) external onlyOwner {
        ...
        require(sellAccumulationFee.lte(initialSellAccumulationFee), "Scribe: fee too high");
    }
    ...
    function setSellAtSmallPriceAccumulationFee(uint128 numerator, uint128 denominator) external onlyOwner {
        ...
        require(sellAtSmallPriceAccumulationFee.lte(initialSellAtSmallPriceAccumulationFee), "Scribe: fee too high");
    }
    ...
    function setSellReflectionFee(uint128 numerator, uint128 denominator) external onlyOwner {
        ...
        require(sellReflectionFee.lte(initialSellReflectionFee), "Scribe: fee too high");
    }
    ...
    function setBuyAccumulationFee(uint128 numerator, uint128 denominator) external onlyOwner {
        ...
        require(buyAccumulationFee.lte(initialBuyAccumulationFee), "Scribe: fee too high");
    }
    ...
    function _swapAndLiquifyOrCollect(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (feeDestination == FeeDestination.Liquify) {
            ...
            revert("Scribe: invalid feeDestination");
        }
    }
