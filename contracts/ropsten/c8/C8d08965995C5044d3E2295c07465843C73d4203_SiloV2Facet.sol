...
contract SiloV2Facet is TokenSilo {
    ...
    function claimWithdrawals(address[] calldata tokens, uint32[] calldata withdrawals) public {
        AppStorage storage s = LibAppStorage.diamondStorage();
        for (uint256 i = 0; i < tokens.length; i++) {
            claimWithdrawal(tokens[i], withdrawals[i]);
        }
    }
    
    function claimWithdrawal(address token, uint32 _s) public returns (uint256 amount) {
        amount = removeTokenWithdrawal(msg.sender, token, _s);
        IERC20(token).transfer(msg.sender, amount);
        emit ClaimWithdrawal(msg.sender, token, _s, amount);
    }
...
contract TokenSilo {
    ...
    event ClaimWithdrawal(address indexed account, address token, uint32 withdrawals, uint256 amount);
    ...
    **/
    ...
    function getDeposit(address account, address token, uint32 _s) public view returns (uint256, uint256) {
        return LibTokenSilo.tokenDeposit(account, token, _s);
    }

    function getWithdrawal(address account, address token, uint32 _s) public view returns (uint256) {
        return LibTokenSilo.tokenWithdrawal(account, token, _s);
    }
    ...
    **/
    ...
    function season() private view returns (uint32) {
