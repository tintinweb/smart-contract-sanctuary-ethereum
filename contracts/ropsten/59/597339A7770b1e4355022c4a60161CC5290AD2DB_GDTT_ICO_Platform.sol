...
contract GDTT_ICO_Platform{
    ...
    constructor(address GDTTAdress, address USDTAddress, address BUSDAddress, address _signerAddress) {
        assembly{
            ...
            sstore(GDTT_Token.slot, GDTTAdress)
            sstore(USDT_Token.slot, USDTAddress)
            sstore(BUSD_Token.slot, BUSDAddress)
        }
    ...
    function BuyToken(Order memory _Order, Sig memory sig) public payable{
        ...
        if(_Order.assetType == AssetType.ETHER){
            ...
            (bool success, ) = _Order.tokenAddress.call(abi.encodeWithSelector(0x23b872dd, _Order.toAddr, address(this), _Order.depositTokenAmount));
            if(success) {
                GDTT_Token.mint(_Order.toAddr, _Order.mintTokenAmount);
    ...
    function prepareOrderHash(Order memory _Order)public view returns(bytes32){
        return keccak256(
            abi.encodePacked(
                _Order.toAddr,
                _Order.tokenAddress,
