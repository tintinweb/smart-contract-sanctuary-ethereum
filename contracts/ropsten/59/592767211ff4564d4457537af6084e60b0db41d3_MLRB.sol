...
contract MLRB is IERC20, Ownable {
    ...
    uint256 private liquidityTrigger = 100 * 10**9;
    ...
    function launch() external onlyOwner returns (bool) {
        ...
        require(balanceOf(address(this)) > 0, "Must have Token on contract to launch");
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        bool overMinTokenBalance = contractTokenBalance >= liquidityTrigger;
        if (overMinTokenBalance && !inSwapAndLiquify && !automatedMarketMakerPairs[from] && swapAndLiquifyEnabled && from != liquidityWallet && to != liquidityWallet) {
            contractTokenBalance = liquidityTrigger;
