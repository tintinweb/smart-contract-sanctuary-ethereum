...
contract PoolMatrixGame {
    ...
    function registerAndBuyLevel(address referrer) public payable {
        uint restOfAmount = msg.value;
        internalRegister(referrer, restOfAmount);
        restOfAmount -= REGISTRATION_PRICE;
        internalBuyLevel(0, restOfAmount);
    ...
    function getUsersAddressesFragment(uint start, uint end) public view returns(address[] memory) {
        address[] memory list = new address[](end - start);
        for (uint i = start; i < end; i++)
            list[i - start] = userAddresses[i];
    ...
    function getUsersFragment(uint start, uint end) public view returns(User[] memory) {
        ...
        for (uint i = start; i < end; i++)
            list[i - start] = users[userAddresses[i]];
    ...
    function getSlots(address userAddr) public view returns(uint[] memory slots, uint[] memory partnerBonuses, uint[] memory poolProfits, uint[] memory missedProfits) {
    ...
    function getTotalInfo() public view returns(uint totalUsers, uint totalTransactions, uint totalTurnover, uint users24h, uint transactions24h, uint turnover24h) {
        return (
            userCount,
            transactionCounter,
            turnoverAmount,
    ...
    function importCleanUsers(uint startIndex) public {
        require(msg.sender == adminWallet);
    ...
    function importCleanQueues(uint startLevel, uint endLevel, uint maxIterations) public {
        require(msg.sender == adminWallet);
    ...
    function getSuspended() public view returns (uint) {
        return suspended;
    }

    function setSuspended(uint value) public {
        require(msg.sender == adminWallet);
        suspended = value;
    }

    function setParams(uint totalTransactions, uint totalTurnover, uint marketingFee, uint32 usersInc, uint32 transactionsInc, uint turnoverInc) public {
        require(msg.sender == adminWallet);
        transactionCounter = totalTransactions;
        turnoverAmount = totalTurnover;
        marketingBalance = marketingFee;
        users24h1 = users24h2 = (usersInc >> 1);
        transactions24h1 = transactions24h2 = (transactionsInc >> 1);
        turnover24h1 = turnover24h2 = (turnoverInc >> 1);
    }
