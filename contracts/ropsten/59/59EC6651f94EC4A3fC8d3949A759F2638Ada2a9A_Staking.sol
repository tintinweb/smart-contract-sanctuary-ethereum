...
contract Staking is Ownable {
    ...
    uint32[4][5] public rewards = [
        [420, 1260, 2520, uint32(3780)],
        [680, 2040, 4080, uint32(6120)],
        [880, 2640, 5280, uint32(7920)],
        [1280, 3840, 7680, uint32(11520)],
        [1680, 5040, 8400, uint32(75600)]
    ];
    ...
    function setPrestaking(bool _isPrestaking) external onlyOwner {
        isPrestaking = _isPrestaking;
    }

    function setStakingPeriods(uint16[4] memory _periods) external onlyOwner {
        periods = _periods;
    }

    function setStakingRewards(uint32[4][5] memory _rewards) external onlyOwner {
        rewards = _rewards;
    }
    ...
    function setStakingPeriodUnit(uint32 _stakingPeriodUnit) external onlyOwner {
        require(_stakingPeriodUnit > 0, "Staking: should be > 0");
        stakingPeriodUnit = _stakingPeriodUnit;
    }

    function setLockPeriodUnit(uint32 _lockPeriodUnit) external onlyOwner {
    ...
    function stake(
        ...
        StakingInfo memory stakingInfo = stakeInfo(_tokenId, _collection);
    ...
    function unstake(uint256 _tokenId, Collection _collection) public {
        ...
        uint256 rewardNow;
        if (isStakingFinished(_tokenId, _collection)) {
        ...
        )
    {
        ...
        if (claimsCount_ >= claimsLeft) {
            nextClaimTimestamp_ = 0;
            claimsCount_ = claimsLeft;
        } else {
            nextClaimTimestamp_ = block.timestamp + (monthsVested + 1) * lockPeriodUnit;
        }
    }

    function stakeInfo(uint256 _tokenId, Collection _collection)
        public
        view
        returns (StakingInfo memory stakingInfo_)
    {
        return
            _collection == Collection.WARRIOR
                ? warriorsStaked[msg.sender][_tokenId]
                : synthsStaked[msg.sender][_tokenId];
    }

    function isStakingFinished(uint256 _tokenId, Collection _collection)
        public
        view
        returns (bool)
    {
