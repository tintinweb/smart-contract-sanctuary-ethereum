...
contract SaleNFT {
    ...
    function purchaseToken(uint256 _tokenId) external payable {
        ...
        require(_pay(msg.sender, payable(tokenOwner), price), "token transfer error for owner");      
        mintNftTokenAddress.safeTransferFrom(tokenOwner, msg.sender, _tokenId);

        require(_pay(msg.sender, payable(feeAddress), premium), "token transfer error for feeaddress");        
