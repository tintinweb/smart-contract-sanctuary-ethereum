...
contract RftExample is ERC20, ERC721Holder, ReentrancyGuard, Trustable {
  ...
  function getPercents() external view returns (uint256[] memory) {
    ...
    address[] memory holdersList = holders.values();

    uint256[] memory percents = new uint256[](holdersList.length);

    for(uint i = 0; i < holdersList.length; i++) {
      address holder = holders.at(i);

      uint256 balance = balanceOf(holder);
      uint256 percent = balance.mul(100).div(total);
  ...
  receive() external payable {
    ...
    for(uint i = 0; i < holdersList.length; i++) {
      ...
      uint256 balance = balanceOf(holder);
      uint256 percent = balance.mul(100).div(total);
      uint256 amount = balanceThis.mul(percent).div(100);

      (bool success, ) = payable(holder).call{value: amount, gas: 2300}("");
      // bool sent = payable(holder).send(amount);
...
contract NftExample is ERC721Enumerable, Trustable {
	...
	constructor() public ERC721("NftExample", "NFT") {}
