...
contract Voting {
	...
	*/
	function communityVoteCast(uint256 commId, uint256 pollId, bool vote) external {
		CommunityPoll memory poll = pollFetch(commId, pollId);
		//check wheather a member have a voting rights or not voted before
		require(isAlreadyVoted(pollId) != true, "You already voted.");
		require(community.votingRight(poll._type, commId, msg.sender) != false, "You already voted.");
	...
	*/
	function _computePollResult(uint256 communityId, uint256 pollId) internal view returns(bool){
	...
	*/
	function _doProposedChanges(uint256 commId, uint256 pollId) external view returns (bool) { 
		if(_computePollResult(commId, pollId)){
			// get the proposed changes and call
			CommunityPoll memory commPoll = pollFetch(commId, pollId);
