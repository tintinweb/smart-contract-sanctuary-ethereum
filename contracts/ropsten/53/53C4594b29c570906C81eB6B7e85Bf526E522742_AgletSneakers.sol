...
contract AgletSneakers is ERC721URIStorage, Ownable, IMintable, ERC2981 {
    ...
    public override onlyMinters {
        ...
        (len, index) = Minting.getLength(mintingBlob, index);
        (tokenURI, index) = Minting.getURI(mintingBlob, index, len);
        (len, index) = Minting.getLength(mintingBlob, index);
        tokenDetails = Minting.getDetails(mintingBlob, index, len);
...
library Minting {
    ...
    function getURI(bytes calldata blob, uint256 startIndex, uint96 length)
    internal
    pure
    returns (
        string memory,
        ...
        bytes calldata uriBytes = blob[startIndex : end];
    ...
    function getDetails(bytes calldata blob, uint256 startIndex, uint96 length)
    internal
    pure
    returns (
        string memory) {
        ...
        uint256 end = startIndex + uint256(length);
        require(end <= blob.length -1, "Bad format of mintingBlob");
        bytes calldata detailsBytes = blob[startIndex : end];
