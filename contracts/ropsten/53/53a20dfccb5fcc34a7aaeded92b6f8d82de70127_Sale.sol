...
contract WhitelistOperatorable is Operatorable {
   ...
   * @param _accounts array of adding accounts
   */
   ...
   * Requirements:
   * - Only operator can call
   * @param _accounts array of removing accounts
   */
...
contract Sale is WhitelistOperatorable {
  using Address for address;
  ...
  function purchase(
    ...
    if (step == Step.PRESALE) {
      if (!whitelist[msg.sender]) revert NoWhitelist();
