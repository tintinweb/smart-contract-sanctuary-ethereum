...
contract Service is Ownable {
    ...
    mapping(uint256 => string) public  _serviceNames;

    event AddService(uint256 indexed id, string name);

    function setServiceName(uint256 id, string memory name) public onlyOwner {
        _serviceNames[id] = name;
        emit AddService(id, name);
    }
}