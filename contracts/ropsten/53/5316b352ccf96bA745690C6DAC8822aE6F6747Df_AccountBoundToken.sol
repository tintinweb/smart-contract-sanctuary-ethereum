...
contract AccountBoundToken is IERC4973, IERC721Upgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => string) public Credential;
    ...
    function _burnCredential(uint256 tokenId) internal {
        ...
        Owner[tokenId] = address(1); // burned credentials are set to owned by address(1) to avoid replay attacks
    ...
    /// @notice The contract owner issues a credential to the receiver
    /// @param tokenId the tokenId of the credential
    ...
    function proxyIssueCredential(
        ...
        bytes32 digest = keccak256(
            abi.encode(tokenId, msg.sender, credentialURL)
        );
        address issuer = _recoverSigner(digest, proof);
        require(issuer == owner(), "Invalid issuer");
        _updateCredential(tokenId, credentialURL);
        _issueCredential(tokenId, msg.sender);
    }
