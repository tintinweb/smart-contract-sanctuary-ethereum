...
//UPDATE THIS    
 ...
 //UPDATE THIS
    ...
    uint256 public constant VIP_PRICE_PER_TOKEN = 0.005 ether;
    ...
    function mintAllowList(uint8 numberOfTokens) external payable {
     ...
     function mintVIPList(uint8 numberOfTokens) external payable {
        uint256 ts = totalSupply();
        bool callerHasMoreThanFiveHatchlingz = Hatchlingz.balanceOf(msg.sender) >= 5;

        require(isVIPListActive, "Allow list is not active");
        require(numberOfTokens <= _VIPList[msg.sender] || callerHasMoreThanFiveHatchlingz, "Exceeded max available to purchase");
