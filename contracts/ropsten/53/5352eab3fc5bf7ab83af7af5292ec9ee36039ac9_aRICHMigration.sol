...
contract aRICHMigration is Ownable {
    ...
    function migrate() external onlyInitialized() {
        uint256 amount = aRICH.balanceOf(msg.sender);
        require(amount <= maxAmt,"Exceed the specified amount");
        require(startTimestamp < block.timestamp,"Not started yet");
        require(block.timestamp < endTimestamp, "swapping of aRICH has ended");

        aRICH.transferFrom(msg.sender, address(this), amount);
        senderInfo[msg.sender] = senderInfo[msg.sender].add(amount);
        RICH.transfer(msg.sender, amount);
    }

    function reclaim() external {
        require(senderInfo[msg.sender] > 0, "user has no aRICH to withdraw");
        require(
            block.timestamp > endTimestamp,
            "aRICH swap is still ongoing"
        );

        uint256 amount = senderInfo[msg.sender];
        senderInfo[msg.sender] = 0;
        aRICH.transfer(msg.sender, amount);
    }

    function withdraw() external onlyOwner() {
        require(block.timestamp > endTimestamp, "swapping of aRICH has not ended");
        uint256 amount = RICH.balanceOf(address(this));
