...
contract NFT_MintingPlatform {
    ...
    event TransferUSDC(address indexed to, bool result);
    event TransferUSDCFeeToRefferer(address reffererAddress,uint reffererFee);
  ...
  constructor(address SignerAddress,address OwnerAddr, address _ERC721,address _USDCToken) {
    ...
    function mintNFT(Order memory _Order, Sig memory sig,uint USDCTokenAmount,address reffererAddress)public payable{
        ...
        if(_Order.assetType == AssetType.ETHER){
                require(msg.value == USDCTokenAmount,"Deposit ether for Minting NFT!!!");
        ...
        }else if(_Order.assetType == AssetType.USDC){
                require(USDCTokenAmount > 0,"USDC amount invalid or should not give ether while exchanging asset with USDC!!!");
                require(validateOrderHash(_Order,sig),"Incorrect signature while depositying USDC!!!");
                uint refferrerFee = getReffererFee(USDCTokenAmount);
                (bool successs,) = address(USDC).call(abi.encodeWithSignature("transferFrom(address,address,uint256)",msg.sender,Mint_Owner,USDCTokenAmount.sub(refferrerFee,"Subtraction overflow!!!")));
                (bool success,) = address(USDC).call(abi.encodeWithSignature("transferFrom(address,address,uint256)",msg.sender,reffererAddress,refferrerFee));
                    if(success && successs){
                        totalUsersMintedTokenId = incrermentUsingAssemblyLanguageOpcodes.increment(totalUsersMintedTokenId);
                        mintedUserDetails storage MintedUserdetails = MintedNFTsUserDetails[_Order.toAddr];
                        MintedUserdetails.userAddr = _Order.toAddr;
                        ...
                        ERC721.safeMint(_Order.toAddr,totalUsersMintedTokenId,_Order.uri);
                        emit TransferUSDC(_Order.toAddr,success);
                        emit TransferUSDCFeeToRefferer(reffererAddress,refferrerFee);
                    }else{
