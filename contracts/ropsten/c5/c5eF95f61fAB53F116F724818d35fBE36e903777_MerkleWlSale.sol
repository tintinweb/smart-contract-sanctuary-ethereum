...
contract MerkleWlSale is MerkleTree, Ownable, ReentrancyGuard {
    bytes32 public merkleRoot;
    uint256 public duration = 1 days;
    uint256 public startTime;
    uint256 public raised = address(this).balance;
    mapping(address => uint256) balance;
    bool public started = false;

    modifier isStarted() {
        require(started == true, "Sale hasnt started yet");
        _;
    }
    ...
    function changeSaleStatus(bool status) 
        external
        isActive
        onlyOwner 
    {
    ...
    function contribute(bytes32[] calldata _merkleProof)
        external
        payable
        isActive
        isStarted
    ...
    event Contribution(uint256 value, address contributor);
