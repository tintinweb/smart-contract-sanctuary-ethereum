...
*/
contract Chicken is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 160000 ether;
    // Initial distribution for the day 2-5 CHICKEN-WFTM LP -> CHICKEN pool
    uint256 public constant INITIAL_CHICKEN_POOL_DISTRIBUTION = 1 ether;
    ...
    /**
     * @notice Constructs the Chicken ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("CHICKEN", "CHICKEN") {
        // Mints 1 CHICKEN to contract creator for initial pool setup
    ...
    function _getChickenPrice() internal view returns (uint256 _chickenPrice) {
        try IOracle(chickenOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Chicken: failed to fetch CHICKEN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _chickenPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_chickenPrice >= taxTiersTwaps[tierId]) {
    ...
    function setChickenOracle(address _chickenOracle) public onlyOperatorOrTaxOffice {
        require(_chickenOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CHICKEN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CHICKEN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentChickenPrice = _getChickenPrice();
            currentTaxRate = _updateTaxRate(currentChickenPrice);
            if (currentChickenPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _chickenPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_chickenPool != address(0), "!_chickenPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_chickenPool, INITIAL_CHICKEN_POOL_DISTRIBUTION);
