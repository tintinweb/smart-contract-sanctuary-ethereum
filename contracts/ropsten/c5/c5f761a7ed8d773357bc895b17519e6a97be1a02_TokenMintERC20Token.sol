...
interface IcryptoPredictionPool {
    function Register(string memory _username) external returns(bool);
    // function callRebase(bool rebaseIsNegative, uint256 _rebasePoolId) external;
    function PlaceBet(uint256 index,uint256 _prices,uint256 _percent,uint256 _poolId,uint256 _amount) external returns(bool);
    function updatebal(address _user,uint256 _poolId,uint256 _reward,bool _isPositive) external returns(bool);
    function createRebase() external returns(bool);
    function createPool(uint256[15] memory _prices) external returns(bool);
    function updateowner(address new_owner) external returns(bool);
    function updatetime(uint256 _timestamp) external returns(bool);
    function fetchUser(address _user) external view returns(uint256[] memory _pools,string memory username,uint256 claimable,uint256 staked_balance, bool active);
    function fetchRebasePool(uint _RebasePoolId) external view returns( uint _NumberOfOccurances, uint _numberOfNegativeDates, uint _createdTime, uint _endingTime);
    function fetchPool(uint256 _poolId) external view returns(uint256[15] memory _prices,uint256 _start,uint256 _end,uint256 _staking_ends);
    function fetchUserBets(address _user, uint256 _poolId) external view returns(uint256[15] memory _pools,uint256[15] memory _prices,uint256[15] memory _amounts,uint256 balance,uint256 totalbet);
    function fetchUserInPool(uint256 _poolId) external view returns(address[] memory _betters);
}


contract cryptoPredictionPool is ReentrancyGuard, IcryptoPredictionPool {
