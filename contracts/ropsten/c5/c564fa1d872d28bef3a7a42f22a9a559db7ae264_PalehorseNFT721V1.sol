...
contract PalehorseNFT721V1 is ERC721 {
    ...
    constructor (string memory name, string memory symbol, string memory tokenURIPrefix) ERC721 (name, symbol){
        tokenCounter = 1;
        owner = msg.sender;
        _setBaseURI(tokenURIPrefix);
        setmaximumRoyality(1000);
    }
    ...
    // Create modifier for whitelisting
    
    // modifier onlyWhiteListUser() {
        
    //     require(whitelist[msg.sender]==true, "User is not whiteListed user");
    //     _;
    // }
    ...
    // Adding functionality of whitelist users

    function addWhiteList(address[] memory userList) public onlyOwner {
        
        for(uint256 i=0; i<userList.length; i++){
            
            address entry = userList[i];
...
// Removing functionality of whitelist users
    function removeWhiteList(address[] memory userList) public onlyOwner{
    ...
    //Adding lazyMint() function 

    /**
     * @dev This function will set maxRoyaltiy , access type is onlyOwner
     * @notice This Function will set maxRoyaltiy, minter should input royality under maxRoyaltiy
     * @param _maxroyality Uint input value.
     */
    ...
    function LazyMint(address payable creator, NFTVoucher calldata voucher, uint256 fee)
        public
        payable
        returns (uint256)
    {
        ...
        uint256 newItemId=tokenCounter;
        tokenCounter = tokenCounter + 1;
        creators[tokenCounter] = creator;


        require(whitelist[signer], "User is not whiteListed user");
        // make sure that the signer is authorized to mint NFTs
        require(creator == signer, "Signature invalid or unauthorized");

        // make sure that the redeemer is paying enough to cover the buyer's cost
        require(msg.value >= voucher.minPrice, "Insufficient funds to redeem");
        require(
            voucher.royality <= maximumRoyality,
            "royality of creater is too high"
        );

        // first assign the token to the signer, to establish provenance on-chain

        _safeMint(signer, newItemId, fee);

        royalities[newItemId] = voucher.royality;
        _setTokenURI(newItemId, voucher.tokenUri);
        // transfer the token to the redeemer
        _transfer(signer, msg.sender, newItemId);
        creator.transfer(msg.value);
        mintedNonce[voucher.vNonce] = true;
        nonceCount = nonceCount + 1;
        // record payment to signer's withdrawal balance
        // pendingWithdrawalsCreator[signer] += msg.value;
      ...
      function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_nonce, minPrice, tokenUri));
    }
