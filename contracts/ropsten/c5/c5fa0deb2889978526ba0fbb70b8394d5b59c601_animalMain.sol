...
contract animalMain is  ERC721, Ownable {
  ...
  function transfer(address _to, uint256 _tokenId) public {
    ...
    require(animalToOwner[_tokenId] == msg.sender, "該物件不是轉移者的");
  ...
  function createAnimal() public payable returns(uint) {
      //動手玩創意，可以限制每次建立動物需要花費多少ETH
      require(msg.value == 0.0005 ether, "建立動物需要 0.0005 ETH，請確認，請確認 ETH數量");
      ...
      //TO DO 
      //使用亂數來產生DNA, 星級(1-100), 動物種類(1-4)
      dna = keccak256(abi.encodePacked(msg.sender, block.coinbase, now));
      star = uint(dna) % 100 + 1;
