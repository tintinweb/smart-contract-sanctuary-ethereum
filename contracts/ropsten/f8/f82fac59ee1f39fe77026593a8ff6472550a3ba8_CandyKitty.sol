    ...
    SignerManager
{
    ...
    function preSaleMint(
        ...
        require(_quantity <= 5, "exceed max mint amount");
        require(tokenIdx + _quantity <= 2000, "exceed max presale amount");
        signers.requireValidSignature(
            signaturePayload(_to, _nonce),
            _sig,
            usedMessages
        );
        _mint(_to, _quantity, preSalePrice);
    }
    ...
    function publicMint(
        ...
        require(status == Status.PublicSale, "not in PublicSale mint period");
        require(tokenIdx + _quantity <= 10000, "exceed max mint amount");
        _mint(_to, _quantity, publicSalePrice);
    }

    function publicSaleMint(address _to, uint256 _quantity) external onlyOwner {
        require(tokenIdx + _quantity <= 10000, "exceed max mint amount");
        _mint(_to, _quantity, 0);
    }

    function _mint(address _to, uint256 _quantity, uint256 _price) internal {
        require(msg.value >= _quantity*_price, "insufficient value");
    ...
    function whitelistMint(
        ...
        for (uint256 idx = 0; idx < _tokenIds.length; idx++) {
            ...
            assembly {
                mstore(add(bNonce, add(0x20, mul(idx, 0x20))), tokenId)
            }
        }
        assembly {
            mstore(add(bNonce, mul(add(_tokenIds.length, 1), 32)), _nonce)
        }

        signers.requireValidSignature(
            signaturePayload(_to, keccak256(bNonce)),
            _sig,
            usedMessages
        );
        for (uint256 idx = 0; idx < _tokenIds.length; idx++) {
     ...
     */
    function alreadyMinted(address _to, bytes32 _nonce)
        external
        view
        returns (bool)
    {
        return
            usedMessages[
                SignatureChecker.generateMessage(signaturePayload(_to, _nonce))
            ];
    }
    ...
    signature.
     */
    function signaturePayload(address _to, bytes32 _nonce)
        internal
        pure
        returns (bytes memory)
    {
        return abi.encodePacked(_to, _nonce);
    }
    ...
    */
    ...
    /**
    @notice Changes the CandyKitty' nesting statuss (what's the plural of status?
    statii? statuses? status? The plural of sheep is sheep; maybe it's also the
    plural of status).
    @dev Changes the CandyKitty' nesting sheep (see @notice).
     */
    ...
    function withdraw(address payable _recipient, IERC20 _token) onlyOwner public returns(bool) {
