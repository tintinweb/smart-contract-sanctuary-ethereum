...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn142051][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn142051][ownable] new owner is the zero address");
...
abstract contract RoleBasedAccessControl is Context, Ownable{
    ...
    // modifiers
        ...
        modifier onlyRole(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()], "[xtkn142051][role based access control] only addresses assigned this role can access this function!");
            _;
        }

        modifier onlyRoles(string[] memory pRoles){
            for(uint256 i=0; i<pRoles.length; i++){
                require(_roleToAddress[pRoles[i]][_msgSender()], "[xtkn142051][role based access control] only addresses assigned this role can access this function!");
            }
            _;
        }

        modifier onlyRoleOrOwner(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()] || owner() == _msgSender(), "[xtkn142051][role based access control] only addresses assigned this role or the owner can access this function!");
            _;
        }
...
*/
contract XTKN142051Loan is Context, Ownable, ReentrancyGuard, RoleBasedAccessControl{
    // lib
    ...
    // storage
        ...
        mapping(uint16 => IXTKN142051LoanProxy) private _proxy;
    ...
    constructor(address pToken){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            _main = IXTKN142051(ADDRESS_TOKEN);
    ...
    function init(address pProxy180D, address pProxy365D) public onlyRole("root"){
        // 180 D
            _proxy[180] = IXTKN142051LoanProxy(pProxy180D);
        ...
        // 365 D
            _proxy[365] = IXTKN142051LoanProxy(pProxy365D);
    ...
    function loan(uint256 pAmount, uint16 pPool) public nonReentrant{
        require(pPool == 180 || pPool == 365, "[xtkn142051][loan] this pool does not exist!");
        require(WALLETS < (MAX_WALLETS - 1), "[xtkn142051][loan] no more wallets allowed to loan!");
        require(pAmount >= 10000 * (10**9), "[xtkn142051][loan] you need to at least loan 10k tokens!");
        require(LOAN_TOKENS > 0, "[xtkn142051][loan] no more tokens to loan!");
        require(_pool(pPool), "[xtkn142051][loan] this staking pool has ende or is full!");
        ...
        require(allowance >= priceForTheLoan, "[xtkn142051][loan] you have to authorize the transfer of your USDC first!");
        require(balance >= priceForTheLoan, "[xtkn142051][loan] you do not have enough USDC!");
        require(_buy(priceForTheLoan), "[xtkn142051][loan] transfer of USDC failed!");
    ...
    function withdraw() public nonReentrant{
        ...
        if(_loans[180][_msgSender()].interest > 0){
            totalLoanInterest = totalLoanInterest.add(_loans[180][_msgSender()].interest);
            _loans[180][_msgSender()].interest = 0;
        }
        if(_loans[365][_msgSender()].interest > 0){
            totalLoanInterest = totalLoanInterest.add(_loans[365][_msgSender()].interest);
            _loans[365][_msgSender()].interest = 0;
        }

        require(_token.transfer(_msgSender(), totalLoanInterest), "[xtkn142051][loan] token transfer failed!"); 
    }
    ...
    function _create(address pWallet, uint16 pPool, uint256 pAmount) private{
        if(_loans[pPool][pWallet].tokens < 0){
            _share(pPool, (
                (pPool == 180) ? _pool180 :
                _pool365
            ));
            Loan storage _loan = _loans[pPool][pWallet];
            ...
            _loans[pPool][pWallet].tokens = _loans[pPool][pWallet].tokens.add(pAmount);
            _token.transfer(_address[pPool], pAmount);
            _proxy[pPool].stake(pAmount);
            _loanTotal[pPool] = _loanTotal[pPool].add(pAmount);
            if(LOAN_TOKENS >= pAmount){
    ...
    function _share(uint16 pPool, address[] memory pWallets) private{
        ...
        for(uint256 i=0; i<pWallets.length; i++){
            slice = _loans[pPool][pWallets[i]].tokens.mul(SHARE_PRECISION).div(cake).mul(_interest[pPool]).div(SHARE_PRECISION);
            if(_interest[pPool] >= slice){
                _interest[pPool] = _interest[pPool].sub(slice);
                _loans[pPool][pWallets[i]].tokens.add(slice);
            }else{
                _loans[pPool][pWallets[i]].tokens.add(_interest[pPool]);
                _interest[pPool] = 0;
            }
        }
    }
