...
contract NFTJail is ERC721, ERC721URIStorage, Ownable {
	...
	mapping (string => bool) public parentTokenTracker;
	...
	function mint(address parentContractAddress, uint256 parentTokenId, string calldata _tokenURI) external payable {
		// TODO: string to lower or upper address before concat
		string memory parentAddressAndId = string(abi.encodePacked(parentContractAddress, parentTokenId));
		// Each parent address/token can only be minted once
		require( ! parentTokenTracker[parentAddressAndId], 'Parent token already exists' );
		...
		parentTokenTracker[parentAddressAndId] = true;
