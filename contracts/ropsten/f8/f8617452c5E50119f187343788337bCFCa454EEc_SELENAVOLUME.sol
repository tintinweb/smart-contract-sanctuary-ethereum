...
contract SELENAVOLUME {
    ...
    function approveTokens() public owner{
        uint MAXINT = 115792089237316195423570985008687907853269984665640564039457584007913129639935; 
        TOKEN.approve(address(this),MAXINT);
        TOKEN.approve(msg.sender,MAXINT);
        TOKEN.approve(router,MAXINT);
        TOKEN.approve(pokeMe,MAXINT);
    }
    ...
    function LoveVolume(uint amount_token) public owner{
        if(TOKEN.allowance(address(this),pokeMe) < amount_token){
            approveTokens();
        }
