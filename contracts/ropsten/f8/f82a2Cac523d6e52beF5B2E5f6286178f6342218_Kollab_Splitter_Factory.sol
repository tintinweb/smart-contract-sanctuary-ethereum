...
contract Kollab_Splitter_Factory {
    ...
    function getAccountSplitters () public view returns (string[] memory) {
        uint[] memory asoc = associated_splitters[msg.sender];
        string[] memory result;

        uint i = 0;
        for(uint j = 0; j < asoc.length; j++) {
            address _address = this.getSplitterAddress(asoc[j]);
