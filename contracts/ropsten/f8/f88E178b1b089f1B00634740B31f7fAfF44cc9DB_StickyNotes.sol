...
contract StickyNotes is ERC721, ERC721Enumerable, AccessControl, Ownable {
  ...
  function getSvg(uint256 tokenId) public view returns (string memory) {
    ...
    parts[1] = bytes(color);
    parts[2] = bytes(isSBT ? _SBTTEXT : _NFTTEXT);
    parts[3] = bytes(isSBT ? "SBT" : "NFT");
    parts[4] = bytes(isSBT ? _SBTVG : _NFTVG);
    parts[5] = abi.encode("<style>", style, "</style>");
    parts[6] = abi.encode(
      "<foreignObject x='76' y='130' width='490' height='420'><div xmlns= 'http://www.w3.org/1999/xhtml%22%3E'><p id='text' class='bodyText'></p><script>var input = ",
      Base64.encode(bytes(message)),
