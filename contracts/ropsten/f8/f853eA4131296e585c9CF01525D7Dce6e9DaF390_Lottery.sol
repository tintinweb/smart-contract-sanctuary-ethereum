...
contract Lottery is Ownable {
    ...
    function createRaffle(
        uint256 lotteryPrice_,
        uint256 price_,
        uint256 endIn_,
        uint256 totalTickets_
    ) public onlyOwner {
        require(
            IERC20(USDC).allowance(owner(), address(this)) >= lotteryPrice_
        );
        require(IERC20(USDC).balanceOf(owner()) >= lotteryPrice_);
        IERC20(USDC).transferFrom(owner(), address(this), lotteryPrice_);
        _raffleId.increment();
    ...
    function cancelRaffle(uint256 id) public onlyOwner {
        require(id <= _raffleId.current() && id > 0);
        Raffle storage raffle = raffles[id];
        require(raffle.status == RaffleStatus.Active);
    ...
    function purchaseTicket(uint256 id) public {
        require(id <= _raffleId.current() && id > 0);
        Raffle storage raffle = raffles[id];
        require(totalTicketSold[id] <= raffle.totalTickets);
        require(
            IERC20(USDC).allowance(msg.sender, address(this)) >= raffle.price
        );
        require(IERC20(USDC).balanceOf(msg.sender) >= raffle.price);
        IERC20(USDC).transferFrom(msg.sender, address(this), raffle.price);
    ...
    function selectRandomWinner(uint256 id) public onlyOwner {
        require(id <= _raffleId.current() && id > 0);
        Raffle storage raffle = raffles[id];
        require(block.timestamp >= raffle.endIn);
    ...
    function claim(uint256 id) public {
        ...
        Raffle storage raffle = raffles[id];
        require(block.timestamp >= raffle.endIn);
        require(ticketPurchased[id][msg.sender] > 0);
        require(raffle.winner == msg.sender);
