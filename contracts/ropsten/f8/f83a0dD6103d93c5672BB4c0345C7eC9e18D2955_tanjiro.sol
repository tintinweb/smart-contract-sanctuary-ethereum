...
contract tanjiro is Context, IERC20, Ownable {
    ...
    string private _name = "Tanjiro Inu";
    string private _symbol = "TANJIRO";
    ...
    uint256 private _previousRefTax = _taxFee;
    uint256 private _marTax; // tax for marketing
    uint256 private _previousMarTax;
    uint256 private _liqTax; // tax for liquidity
    uint256 private _previousLiqTax;

    uint256 private _totMar;
    uint256 private _totLiq;
    uint256 private _totSwap;
    ...
    uint256 public _maxHoldings = 15000000 * 10**9;
    ...
    constructor(address tokenOwner) {
      ...
      _marTax = _buyMarketingFee;
      _previousMarTax = _marTax;
      _liqTax = _buyLiquidityFee;
      _previousLiqTax = _liqTax;
      _totMar = _buyMarketingFee + _sellMarketingFee;
      _totLiq = _buyLiquidityFee + _sellLiquidityFee;
      _totSwap = _totMar + _totLiq;
      ...
      initialTransfer(_msgSender(), tokenOwner , _tTotal);

        //exclude owner and this contract from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;

        _marketingWallet = payable(0x5008068b88F39Cf71C6D3DbE75c11a39C00ccd93); //payable(0x782f303d1fE1aaAa44798d0B3c8A26CF63ae4034);
    ...
    function showTransactionSize() external view returns (uint256) {
        return _maxTxAmount;
    }    
    function showBuyMarketingTax() external view returns (uint256) {
        return _buyMarketingFee;
    }
    function showSellMarketingTax() external view returns (uint256) {
        return _sellMarketingFee;
    }        
    function showBuyLiquidityTax() external view returns (uint256) {
        return _buyLiquidityFee;
    }
    function showSellLiquidityTax() external view returns (uint256) {
        return _sellLiquidityFee;
    }     
    function showReflectionTax() external view returns (uint256) {
        return _taxFee;
    }    
    ...
    function setBuyMarketingTax(uint256 marTax) external onlyOwner() { 
      _buyMarketingFee = marTax;
    }
    function setSellMarketingTax(uint256 marTax) external onlyOwner() { 
      _sellMarketingFee = marTax;
    }
    function setBuyLiquidityTax(uint256 liqTax) external onlyOwner() { 
      _buyLiquidityFee = liqTax;
    }
    function setSellLiquidityTax(uint256 liqTax) external onlyOwner() { 
      _sellLiquidityFee = liqTax;
    }
    function setBuyReflectionTax(uint256 refTax) external onlyOwner() { 
      _taxFee = refTax;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = tAmount.mul(_totSwap).div(100);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _buyMarketingFee == 0 && _sellMarketingFee == 0 && _buyLiquidityFee == 0 && _sellLiquidityFee == 0 ) return;
        
        _previousRefTax = _taxFee;
        _previousMarTax = _marTax;
        _previousLiqTax = _liqTax;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if (from == uniswapV2Pair) { // Buy
                ...
                _taxFee = _taxFee;
                _liqTax = _sellLiquidityFee;
                _marTax = _sellMarketingFee;                
            } else { // other
                _taxFee = _taxFee;
                _liqTax = _buyLiquidityFee;
                _marTax = _buyMarketingFee;                
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensForMar = (contractTokenBalance.mul(_totMar)).div(_totSwap);
        uint256 tokensForLiq = (contractTokenBalance.sub(tokensForMar));
        ...
        swapTokensForEth(tokensForMar);
        uint256 newestBalance = address(this).balance.sub(nextBalance);        

        transferOutETH(_marketingWallet, newestBalance);
    ...
    function transferOutETH(address payable recipient, uint256 amount) private {
