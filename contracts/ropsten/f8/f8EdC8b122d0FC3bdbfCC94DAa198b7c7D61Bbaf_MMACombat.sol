...
contract MMACombat is Context, IERC20, Ownable {
    ...
    address public treasuryAddress = 0x68f30F0BaEEA8141d74beCA3A23Af115957e9171;
    address public charityAddress = 0x888608B2813B2ca4584de4df798B9Ba4C32CC8B3;
    address public kirpAddress = 0x42D6DC7B15f11C9BE2C2cf35Fc002746958697e4;
    address public lpRecipient = 0x7a38E94Fd69bB45f4A2c1E7A1B88bD64fC445882;
    ...
    Taxes public taxes = Taxes(35,20,20,20,20,5);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[kirpAddress] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _treasury, uint256 _charity, uint256 _kirp, uint256 _liquidity, uint256 _burn) public onlyOwner {
        taxes.rfi = _rfi;
    ...
    function _takeKirp(uint256 rKirp, uint256 tKirp) private{
        totFeesPaid.kirp +=tKirp;
        if(_isExcluded[charityAddress])_tOwned[charityAddress]+=tKirp;
        _rOwned[charityAddress] +=rKirp;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rTreasury,to_return.rCharity, to_return.rKirp, to_return.rLiquidity, to_return.rBurn ) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTreasury = tAmount*taxes.treasury/100;
        s.tCharity = tAmount*taxes.charity/100;
        s.tKirp = tAmount*taxes.kirp/100;
        s.tLiquidity = tAmount*taxes.liquidity/100;
        s.tBurn = tAmount*taxes.burn/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tTreasury-s.tLiquidity-s.tCharity-s.tKirp-s.tBurn;
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rTreasury,uint256 rCharity,uint256 rKirp,uint256 rLiquidity, uint256 rBurn) {
        ...
        rKirp = s.rKirp*currentRate;
        rBurn = s.rBurn*currentRate;
        rTransferAmount =  rAmount-rRfi-rTreasury-rLiquidity-rCharity-rKirp-rBurn;
        return (rAmount, rTransferAmount, rRfi,rTreasury,rCharity,rKirp,rLiquidity,rBurn);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if(s.rKirp > 0 || s.tKirp > 0){
            _takeKirp(s.rKirp, s.tKirp);
            emit Transfer(sender, kirpAddress, s.tKirp);
        }
    ...
    function updateKirpWallet(address newWallet) external onlyOwner{
        require(kirpAddress != newWallet ,'Wallet already set');
        kirpAddress = newWallet;
        _isExcludedFromFee[kirpAddress];
    }
