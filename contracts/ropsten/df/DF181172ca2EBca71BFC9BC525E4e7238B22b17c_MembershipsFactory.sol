...
import { IMembershipsV1 } from "./interfaces/IMembershipsV1.sol";
import { IMembershipsFactory } from "./interfaces/IMembershipsFactory.sol";

/// @title MembershipsFactory
/// @notice Factory contract that can deploy Memberships clones
/// @author Coinvise
contract MembershipsFactory is Ownable, IMembershipsFactory {
    ...
    /// @notice Emitted when a Memberships clone is deployed
    /// @param membershipsClone address of the newly deployed clone
    /// @param owner owner of the newly deployed clone
    /// @param implementation implementation contract used for the newly deployed clone
    event MembershipsDeployed(address indexed membershipsClone, address indexed owner, address indexed implementation);

    /// @notice Emitted when Memberships implementation contract is changed
    /// @param oldImplementation old implementation contract address
    /// @param newImplementation new implementation contract address
    event MembershipsImplSet(address indexed oldImplementation, address indexed newImplementation);
    ...
    /// @notice Memberships implementation contract address
    ...
    /// @notice Set Memberships implementation contract
    /// @dev Callable only by `owner`.
    ///      Reverts if `_memberships` param is address(0).
    ///      Emits `MembershipsImplSet`
    /// @param _memberships address of Memberships implementation contract
    function setMembershipsImplAddress(address _memberships) external onlyOwner {
        if (_memberships == address(0)) revert InvalidMemberships();

        emit MembershipsImplSet(memberships, _memberships);
    ...
    /// @notice Deploys and initializes a new Membership clone
    ...
    /// @param _membership membership parameters including price, validity, cap, airdropToken, airdropAmount
    /// @param _saltNonce nonce that will be used to generate the salt that determines clone address
    ...
    function deployMemberships(
        ...
        address membershipsClone = Clones.cloneDeterministic(
            memberships,
            keccak256(abi.encode(_owner, _name, _symbol, _saltNonce))
        );

        IMembershipsV1(membershipsClone).initialize(
            _owner,
            _treasury,
            _name,
            _symbol,
        ...
        emit MembershipsDeployed(membershipsClone, _owner, memberships);

        return membershipsClone;
    }

    /// @dev Computes the address of a clone deployed using Clones.cloneDeterministic()
    function predictDeterministicAddress(address _implementation, bytes32 _salt) external view returns (address) {
        return Clones.predictDeterministicAddress(_implementation, _salt, address(this));
    }
}
...
interface IMembershipsFactory {
    ...
    function setMembershipsImplAddress(address _memberships) external;
