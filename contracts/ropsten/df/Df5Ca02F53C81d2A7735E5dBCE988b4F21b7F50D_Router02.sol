...
import "./libraries/RythmLibrary.sol";
...
contract Router02 is IRouter02, IRythmCallee {
	...
	modifier checkETH(address poolToken) {
		require(WETH == IPoolToken(poolToken).underlying(), "RythmRouter: NOT_WETH");
		_;
	}
	...
	function mintCollateral(
		...
		if (isStakedLPToken(underlying)) {
			address rythmPair = IStakedLPToken01(underlying).underlying();
			_permit(rythmPair, amount, deadline, permitData);
			TransferHelper.safeTransferFrom(rythmPair, msg.sender, underlying, amount);
	...
	function leverage(
		...
		address rythmPair = getRythmPair(underlying);
		(uint amountA, uint amountB) = _optimalLiquidity(rythmPair, amountADesired, amountBDesired, amountAMin, amountBMin);
	...
	function _addLiquidityAndMint(
		...
		TransferHelper.safeTransfer(IBorrowable(borrowableA).underlying(), rythmPair, amountA);
		TransferHelper.safeTransfer(IBorrowable(borrowableB).underlying(), rythmPair, amountB);
		// mint LP token
		if (isStakedLPToken(underlying)) IRythmPair(rythmPair).mint(underlying);
		IRythmPair(underlying).mint(collateral);
		// mint collateral
	...
	function _removeLiqAndRepay(
		...
		address rythmPair = getRythmPair(underlying);
		// removeLiquidity
		IRythmPair(underlying).transfer(underlying, redeemAmount);
		//TransferHelper.safeTransfer(underlying, underlying, redeemAmount);
		if (isStakedLPToken(underlying)) IStakedLPToken01(underlying).redeem(rythmPair);
		(uint amountAMax, uint amountBMax) = IRythmPair(rythmPair).burn(address(this));
	...
	function rythmBorrow(address sender, address borrower, uint borrowAmount, bytes calldata data) external virtual override {
		...
		require(msg.sender == declaredCaller, "RythmRouter: UNAUTHORIZED_CALLER");
	...
	function rythmRedeem(address sender, uint redeemAmount, bytes calldata data) external virtual override {
		redeemAmount;
		CalleeData memory calleeData = abi.decode(data, (CalleeData));
	...
	function _optimalLiquidity(
		...
		(uint reserveA, uint reserveB,) = IRythmPair(rythmPair).getReserves();
		uint amountBOptimal = RythmLibrary.quote(amountADesired, reserveA, reserveB);
		if (amountBOptimal <= amountBDesired) {
			require(amountBOptimal >= amountBMin, "RythmRouter: INSUFFICIENT_B_AMOUNT");
			(amountA, amountB) = (amountADesired, amountBOptimal);
		} else {
			uint amountAOptimal = RythmLibrary.quote(amountBDesired, reserveB, reserveA);
			assert(amountAOptimal <= amountADesired);
			require(amountAOptimal >= amountAMin, "RythmRouter: INSUFFICIENT_A_AMOUNT");
	...
	function getRythmPair(address underlying) public virtual override view returns (address) {
	...
	function getBorrowable(address underlying, uint8 index) public virtual override view returns (address borrowable) {
		...
		borrowable = address(uint(keccak256(abi.encodePacked(
			...
			hex"0b7ea66c1925dab1b10af3b861c98710c4f3478eef6296af3d00f68e0ef17f28" // Borrowable bytecode keccak256
		))));
	}
	function getCollateral(address underlying) public virtual override view returns (address collateral) {
		collateral = address(uint(keccak256(abi.encodePacked(
			...
			hex"86d21afff2721abe9431bd35c4aaac47388f9c72c0e297d9b648c98c92fb1e06" // Collateral bytecode keccak256
		))));
	}
...
interface IRouter02 {
	...
	function leverage(
		address rythmPair, uint amountADesired, uint amountBDesired, uint amountAMin, uint amountBMin,
		address to, uint deadline, bytes calldata permitDataA, bytes calldata permitDataB
	) external;
	function deleverage(
		address rythmPair, uint redeemTokens, uint amountAMin, uint amountBMin, uint deadline, bytes calldata permitData
	) external;
	
	function isStakedLPToken(address underlying) external view returns(bool);
	function getRythmPair(address underlying) external view returns (address);
	function getBorrowable(address rythmPair, uint8 index) external view returns (address borrowable);
	function getCollateral(address rythmPair) external view returns (address collateral);
	function getLendingPool(address rythmPair) external view returns (address collateral, address borrowableA, address borrowableB);
}
...
interface IRythmCallee {
    function rythmBorrow(address sender, address borrower, uint borrowAmount, bytes calldata data) external;
    function rythmRedeem(address sender, uint redeemAmount, bytes calldata data) external;
}
...
import 'contracts/interfaces/IRythmPair.sol';

import "./SafeMath.sol";

library RythmLibrary {
    using SafeMath for uint;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'RythmLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'RythmLibrary: ZERO_ADDRESS');
    }
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'b2b68b93478b479bbfcfd69e8811363ea13cb7bfbed4fb21a9b72f2d992f7baf' // init code hash
            ))));
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IRythmPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'RythmLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'RythmLibrary: INSUFFICIENT_LIQUIDITY');
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'RythmLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'RythmLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'RythmLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'RythmLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'RythmLibrary: INVALID_PATH');
...
contract Router01 is IRouter01, IRythmCallee {
	using SafeMath for uint;
	...
	function mintCollateral(
		...
		address rythmPair = IPoolToken(poolToken).underlying();
		_permit(rythmPair, amount, deadline, permitData);
		return _mint(poolToken, rythmPair, amount, msg.sender, to);
	}
	...
	function _leverage(
		...
		address borrowableA = getBorrowable(rythmPair, 0);
		// mint collateral
		bytes memory borrowBData = abi.encode(CalleeData({
			callType: CallType.ADD_LIQUIDITY_AND_MINT,
			rythmPair: rythmPair,
	...
	function leverage(
		...
		_borrowPermit(getBorrowable(rythmPair, 0), amountADesired, deadline, permitDataA);
		_borrowPermit(getBorrowable(rythmPair, 1), amountBDesired, deadline, permitDataB);
		(uint amountA, uint amountB) = _optimalLiquidity(rythmPair, amountADesired, amountBDesired, amountAMin, amountBMin);
		_leverage(rythmPair, amountA, amountB, to);
	}
	...
	function _addLiquidityAndMint(
		...
		(address collateral, address borrowableA, address borrowableB) = getLendingPool(rythmPair);
		// add liquidity to uniswap pair
		TransferHelper.safeTransfer(IBorrowable(borrowableA).underlying(), rythmPair, amountA);
		TransferHelper.safeTransfer(IBorrowable(borrowableB).underlying(), rythmPair, amountB);
		IRythmPair(rythmPair).mint(collateral);
		// mint collateral
		ICollateral(collateral).mint(to);
	}
	...
	function deleverage(
		...
		address collateral = getCollateral(rythmPair);
		uint exchangeRate = ICollateral(collateral).exchangeRate();
		require(redeemTokens > 0, "RythmRouter: REDEEM_ZERO");		
		uint redeemAmount = (redeemTokens - 1).mul(exchangeRate).div(1e18);
		_permit(collateral, redeemTokens, deadline, permitData);
	...
	function _removeLiqAndRepay(
		...
		address tokenB = IBorrowable(borrowableB).underlying();
		// removeLiquidity
		TransferHelper.safeTransfer(rythmPair, rythmPair, redeemAmount);
		(uint amountAMax, uint amountBMax) = IRythmPair(rythmPair).burn(address(this));
		require(amountAMax >= amountAMin, "RythmRouter: INSUFFICIENT_A_AMOUNT");
		require(amountBMax >= amountBMin, "RythmRouter: INSUFFICIENT_B_AMOUNT");
		// repay and refund
	...
	function rythmBorrow(address sender, address borrower, uint borrowAmount, bytes calldata data) external virtual override {
		borrower; borrowAmount;
		CalleeData memory calleeData = abi.decode(data, (CalleeData));
		address declaredCaller = getBorrowable(calleeData.rythmPair, calleeData.borrowableIndex);
		// only succeeds if called by a borrowable and if that borrowable has been called by the router
		require(sender == address(this), "RythmRouter: SENDER_NOT_ROUTER");
		require(msg.sender == declaredCaller, "RythmRouter: UNAUTHORIZED_CALLER");
		if (calleeData.callType == CallType.ADD_LIQUIDITY_AND_MINT) {
			AddLiquidityAndMintCalldata memory d = abi.decode(calleeData.data, (AddLiquidityAndMintCalldata));
			_addLiquidityAndMint(calleeData.rythmPair, d.amountA, d.amountB, d.to);
		}
		else if (calleeData.callType == CallType.BORROWB) {
			BorrowBCalldata memory d = abi.decode(calleeData.data, (BorrowBCalldata));
			address borrowableB = getBorrowable(calleeData.rythmPair, 1);
			IBorrowable(borrowableB).borrow(d.borrower, d.receiver, d.borrowAmount, d.data);
		}
		else revert();
	}
	...
	function rythmRedeem(address sender, uint redeemAmount, bytes calldata data) external virtual override {
		redeemAmount;
		CalleeData memory calleeData = abi.decode(data, (CalleeData));
		address declaredCaller = getCollateral(calleeData.rythmPair);
		// only succeeds if called by a collateral and if that collateral has been called by the router
		require(sender == address(this), "RythmRouter: SENDER_NOT_ROUTER");
		require(msg.sender == declaredCaller, "RythmRouter: UNAUTHORIZED_CALLER");
		if (calleeData.callType == CallType.REMOVE_LIQ_AND_REPAY) {
			RemoveLiqAndRepayCalldata memory d = abi.decode(calleeData.data, (RemoveLiqAndRepayCalldata));
			_removeLiqAndRepay(calleeData.rythmPair, d.borrower, d.redeemTokens, d.redeemAmount, d.amountAMin, d.amountBMin);
		}
		else revert();
	}
	...
	function getBorrowable(address rythmPair, uint8 index) public virtual override view returns (address borrowable) {
		require(index < 2, "RythmRouter: INDEX_TOO_HIGH");
		borrowable = address(uint(keccak256(abi.encodePacked(
			hex"ff",
			bDeployer,
			keccak256(abi.encodePacked(factory, rythmPair, index)),
			hex"0b7ea66c1925dab1b10af3b861c98710c4f3478eef6296af3d00f68e0ef17f28" // Borrowable bytecode keccak256
		))));
	}
	function getCollateral(address rythmPair) public virtual override view returns (address collateral) {
		collateral = address(uint(keccak256(abi.encodePacked(
			hex"ff",
			cDeployer,
			keccak256(abi.encodePacked(factory, rythmPair)),
			hex"86d21afff2721abe9431bd35c4aaac47388f9c72c0e297d9b648c98c92fb1e06" // Collateral bytecode keccak256
		))));
	}
	function getLendingPool(address rythmPair) public virtual override view returns (address collateral, address borrowableA, address borrowableB) {
		collateral = getCollateral(rythmPair);
		borrowableA = getBorrowable(rythmPair, 0);
		borrowableB = getBorrowable(rythmPair, 1);
	}
}
