...
contract SPPayer is ISPPayer {
  ...
  function pay(uint256 _projectId, address _token, uint256 _tokenPrice, uint256 _amount) external override {
    ...
    // Add this user to whitelists
    uint256[] memory whitelistByAddress = whitelists[msg.sender];
    bool flag = false;
    for (uint i = 0; i < whitelistByAddress.length; i++) {
      if(whitelistByAddress[i] == _projectId) flag = true;
    }

    if(flag) whitelists[msg.sender].push(_projectId);
