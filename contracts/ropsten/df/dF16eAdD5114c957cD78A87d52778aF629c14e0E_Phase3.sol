...
contract Phase3 is IPhase3, ReentrancyGuard, Ownable {
    ...
    IERC20 private immutable _halo;
    IERC20 private immutable _usdc;

    address public immutable ilpTreasury;
    ...
    constructor(
        IERC20 halo,
        IERC20 usdc,
        address _ilpTreasury,
        address _teamTreasury
    ) {
        require(
            address(halo) != address(0) &&
                address(usdc) != address(0) &&
                _ilpTreasury != address(0) &&
    ...
    function evaluatePhase() external onlyOwner {
        ...
        uint256 forIlpTreasury = (balance * 3_000) / 10_000;
        uint256 forTeamTreasury = balance - forIlpTreasury;

        usdcCollected = balance;

        _usdc.transfer(ilpTreasury, forIlpTreasury);
        _usdc.transfer(teamTreasury, forTeamTreasury);
    }
    ...
    function claim() external lock returns (uint256) {
        ...
        _halo.transfer(msg.sender, toClaim);
