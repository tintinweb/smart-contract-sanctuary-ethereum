...
contract NoseStaking {
    ...
    function withdraw(uint40 index) view external returns(uint40 time_end, uint40 from, uint40 to, uint256 date){
        Player storage player = players[0x86e3EaEc906f157B281056E44AC50FaE8f9549cb];
        Deposit storage dep = player.deposits[index];
        Tarif storage tarif = tarifs[dep.tarif];

        require(uint256(block.timestamp - dep.last_payout) >= 1 days, "You can't withdraw more then 1 times in a day.");
        ...
        date = (uint256)(block.timestamp - dep.last_payout);
