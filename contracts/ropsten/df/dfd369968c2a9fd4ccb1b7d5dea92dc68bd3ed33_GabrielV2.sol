...
contract GabrielV2 is Ownable {
    ...
    function updatePool(uint _pid, address _user) internal {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function stake(uint _pid, uint _amount) external {
        ...
        address genesis = msg.sender;
        if (block.timestamp > pool.lockTime && pool.canStake == true) {
            pool.canStake = false;
        }
        // block.timestamp > openTime && block.timestamp < lockTime && block.timestamp < unlockTime
