...
contract StakingNFT is Ownable {
   ...
   mapping(address => userReward[]) public userreward;
    ...
    function userElibleNFT(address _userAddress) public view returns(address[] memory, uint[] memory , uint[] memory){
        ...
        for (uint256 z = 0; z < tokenInfoIndex; z++) {
            if(tokenInfo[z].nftContractAddress != address(0)){
                if(IERC721(tokenInfo[z].nftContractAddress).ownerOf(tokenInfo[z].tokenId) == msg.sender){
        ...
        return (nftAddress , tokenId , rType );
