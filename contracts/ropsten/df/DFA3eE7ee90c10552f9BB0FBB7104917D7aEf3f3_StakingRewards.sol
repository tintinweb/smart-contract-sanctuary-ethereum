...
contract StakingRewards is Ownable, ReentrancyGuard {
    ...
    function _getCampaignInfoID(address _token) internal view returns (uint256) {
        for (uint256 i = 0 ; i < campaignInfo.length; i++) {
            if (address(campaignInfo[i].stakingToken) == _token)
            {
                return i + 1;
            }
        }
    ...
    function depositWithPermit(
        ...
        _deposit(_campaignID, _msgSender(), _amount);
    }

    // @notice Stake Staking tokens to TokenFarm
    function deposit(address _token, address _beneficiary, uint256 _amount)
    external
    nonReentrant
    {
        console.log("Staking Rewards Deposit::::");
        uint256 _campaignID = _getCampaignInfoID(_token);
        if (_campaignID > 0)
        {
            _deposit(_campaignID - 1, _beneficiary, _amount);
        }
    }

    // @notice Stake Staking tokens to TokenFarm
    function _deposit(uint256 _campaignID, address _beneficiary, uint256 _amount) internal {
        CampaignInfo storage campaign = campaignInfo[_campaignID];
        UserInfo storage user = userInfo[_campaignID][_beneficiary];
        _updateCampaign(_campaignID);
        if (user.amount > 0) {
            uint256 pending = ((user.amount * campaign.accRewardPerShare) /
            campaign.precision) - user.rewardDebt;
            if (pending > 0) {
                campaign.rewardToken.safeTransfer(_beneficiary, pending);
            }
        }
        if (_amount > 0) {
        ...
        campaign.precision;
        emit Deposit(_beneficiary, _amount, _campaignID);
    }

    // @notice Withdraw Staking tokens from STAKING.
    function withdraw(address _token, address _beneficiary, uint256 _amount)
    external
    nonReentrant
    {
        uint256 _campaignID = _getCampaignInfoID(_token);
        if (_campaignID > 0)
        {
            _withdraw(_campaignID - 1, _beneficiary, _amount);
        }
    }

    // @notice internal method for withdraw (withdraw and harvest method depend on this method)
    function _withdraw(uint256 _campaignID, address _beneficiary, uint256 _amount) internal {
        CampaignInfo storage campaign = campaignInfo[_campaignID];
        UserInfo storage user = userInfo[_campaignID][_beneficiary];
        ...
        emit Withdraw(_beneficiary, _amount, _campaignID);
    }
    ...
    function harvest(uint256[] calldata _campaignIDs) external nonReentrant {
        for (uint256 i = 0; i < _campaignIDs.length; ++i) {
            _withdraw(_campaignIDs[i], _msgSender(), 0);
        }
    }
    ...
    function emergencyWithdraw(uint256 _campaignID) external nonReentrant {
        CampaignInfo storage campaign = campaignInfo[_campaignID];
        UserInfo storage user = userInfo[_campaignID][_msgSender()];
        uint256 _amount = user.amount;
        campaign.totalStaked -= _amount;
        user.amount = 0;
        user.rewardDebt = 0;
        //        campaign.stakingToken.safeTransfer(address(msg.sender), _amount);
        emit EmergencyWithdraw(_msgSender(), _amount, _campaignID);
    }
