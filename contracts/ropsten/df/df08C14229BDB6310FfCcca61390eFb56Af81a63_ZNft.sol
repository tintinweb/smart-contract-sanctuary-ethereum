    ...
    ZNftInterface
{
    function initialize(
        ...
        require(
            ...
            "ZNft: asset should not be address(0)"
        );
        require(
            ComptrollerInterface(_comptroller).isComptroller(),
            "ZNft: _comptroller is not a Comptroller contract"
        );
        __Ownable_init();
        ...
        returns (uint256)
    {
        ...
        ComptrollerInterface(comptroller).mintAllowed(
            address(this),
            msg.sender,
            0
        );
        ...
        //             tokenIds[i]
        //         );
        //         (bool checkSuccess, bytes memory result) = underlying
        ...
        //             tokenIds[i]
        //         );
        //         (bool buyPunkSuccess, bytes memory resultData) = underlying
        //             .call(data);
        //         require(
        //             buyPunkSuccess,
        //             "ZNFT: Calling buyPunk was unsuccessful"
        //         );
        //     }
        // } else {
        for (uint256 i; i < length; ++i) {
    ...
    function seize(
        ...
        ComptrollerInterface(comptroller).seizeAllowed(
            address(this),
            msg.sender,
            liquidator,
            borrower,
        ...
        for (uint256 i = 0; i < length; i++) {
            _transfer(borrower, liquidator, seizeIds[i]);
        }
    }
        ...
        returns (uint256)
    {
        ...
        //             tokenIds[i]
        //         );
        //         (bool transferPunkSuccess, bytes memory returnData) = underlying
        //             .call(data);
        //         require(
        //             transferPunkSuccess,
        //             "ZNFT: Calling transferPunk was unsuccessful"
        //         );
        //     }
        // } else {
        for (uint256 i; i < length; ++i) {
    ...
    function safeTransferFrom(
        ...
        ComptrollerInterface(comptroller).transferAllowed(
            address(this),
            from,
            to,
            1
    ...
    modifier validReceive(address operator) {
        require(
            ...
            "ZNft: This contract can only receive the underlying NFT"
        );
        require(
            operator == address(this),
            "ZNft: only the ZNft contract can be the operator"
        );
        _;
    }
    ...
    function setZBond(ZBond zBond) public override {
        require(
            msg.sender == comptroller,
            "Znft: only comptroller can set ZBond for ZNft"
        );
        zBonds.push(zBond);
    }

    function getZBonds() public view override returns (ZBond[] memory) {
        return zBonds;
    }
}
