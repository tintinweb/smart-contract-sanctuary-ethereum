...
contract StakeSealsv2 is UntransferableERC721, IERC721Receiver {
    ...
    event DepositSeal(address person, uint256[] sealIds);

    event WithdrawSeal(address person, uint256[] sealIds);

    event ClaimedPixl(address person, uint256[] sealIds, uint256 pixlAmount, uint256 claimedAt);
    ...
    uint256[] public defaultRates = [50, 60, 75, 100, 150, 500, 0];
    ...
    constructor(address _erc721Address, address _erc20Address, address _stakeSealsV1)
    UntransferableERC721("StakedSealsV2", "sSEAL")
    {
        erc721Address = _erc721Address;
        erc20Address = _erc20Address;
        stakeSealsV1 = _stakeSealsV1;
    ...
    unchecked {
            ...
            uint256 rewards = rate * (Math.min(block.number, EXPIRATION) - staked[tokenId].claimedAt);
            return rewards;
        }
    }
    ...
    */
    function findRate(uint256 tokenId) public view returns (uint256 rate) {
        uint256 rarity = IStakeSeals(stakeSealsV1).tokenRarity(tokenId);
    ...
    function deposit(uint256[] calldata tokenIds) public {
        ...
        emit DepositSeal(msg.sender, tokenIds);
        }
    }
    ...
    function withdraw(uint256[] calldata tokenIds) public {
        ...
        _withdraw(tokenIds);
        emit WithdrawSeal(msg.sender, tokenIds);
    }
    ...
    function _updateClaimedAt(uint256 tokenId) private {
    ...
    * @dev Modify the Staking contract address
     * @param tokenAddress - the new Staking contract
     */
    function setStakedSealsAddress(address tokenAddress) external onlyOwner {
        stakeSealsV1 = tokenAddress;
    }
