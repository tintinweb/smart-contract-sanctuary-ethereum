...
contract SatoruEnterprise is Context, IERC20, Ownable {
    ...
    function setRewardFee(uint256 rewardFee) external onlyOwner() {
        require(rewardFee <= 5, "Max reward fee cannot exceed more than 5%");
        defaultTaxFee = rewardFee;
    }

    function setDefaultMarketingFee(uint256 marketingFee) external onlyOwner() {
        require(marketingFee <= 15, "Max marketing fee cannot exceed more than 15%");
        defaultMarketingFee = marketingFee;
    }

    function setMarketingFee4Sellers(uint256 marketingFee4Sellers) external onlyOwner() {
        require(marketingFee4Sellers <= 15, "Max marketing fee cannot exceed more than 15%");
