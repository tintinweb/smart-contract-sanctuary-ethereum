...
contract FootBall is Ownable,ERC721{
    ...
    function playerAttack(uint _playerA,uint _playerB) external{
        require(playerToOwner[_playerA] == msg.sender && players[_playerB].isDraw == 1);
    ...
    function sendPlayerBack(uint[] _id) public {
        for(uint i=0;i<=_id.length;i++){ 
            require(playerToOwner[i] == msg.sender);
            uint fee = drawFee * backFee/100;
            //init player info 
            players[i].isDraw = 0;
            players[i].isSell = 0;
            players[i].readytime = 0;
            players[i].sellPrice = 0 ether;
            playerToOwner[i] = 0;
            ownerPlayerCount[msg.sender] = ownerPlayerCount[msg.sender].sub(1);
            if(address(this).balance >= fee){ 
                msg.sender.transfer(fee);    
            }  
            emit playerBack(i,msg.sender);
        }
