...
contract TodoList{
    ...
    modifier onlyUser(address _addr) {
         require(msg.sender == _addr, "not owner of the task");
         _;
    }
    ...
    function updateTask(string memory _task, uint taskID, address _addr) external onlyUser(_addr) {
        Todo storage td = _Todo[_addr][taskID];
        td.task = _task;
    }

   //function to return a user tasks
    function getTask(address _addr) public view onlyUser(_addr) returns(Todo[] memory) {
        Todo[] storage td = _Todo[_addr];
        return td;
    }

    //function to checktask
    function completeTask(uint taskID, address _addr) external onlyUser(_addr)  {
      Todo storage td = _Todo[_addr][taskID];
      td.completed = true;
