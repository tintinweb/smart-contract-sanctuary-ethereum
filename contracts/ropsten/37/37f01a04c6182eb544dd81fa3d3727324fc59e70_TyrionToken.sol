 ...
 */
contract TyrionToken is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private constant _tTOTAL = 500 * 1e6 * 10**_DECIMALS;
     ...
     * construction.
     */
    string private constant _NAME = "TYRION";
    string private constant _SYMBOL = "TYON";
    ...
    /**
     * @dev Percentage of the Growthxfee.
     */ 
    uint256 public _growthxFee = 25;
    address public growthxWallet = 0xac6a0fE8F105485F66f957FF383E8400E88Ef986;
    uint256 private _previousGrowthxFee = _growthxFee;


     /**
     * @dev Percentage of the TYONShield.
     */ 
    uint256 public _tyonshieldFee = 25;
    address public tyonshieldWallet = 0xac6a0fE8F105485F66f957FF383E8400E88Ef986;
    uint256 private _previousTyonshieldFee = _tyonshieldFee;

    /**
     * @dev Percentage of the FundMe.
     */ 
    uint256 public _fundFee = 25;
    address public fundWallet = 0xac6a0fE8F105485F66f957FF383E8400E88Ef986;
    uint256 private _previousFundFee = _fundFee;

    /**
     * @dev Percentage of the EcosystemGrowth.
     */ 
    uint256 public _ecosystemFee = 25;
    address public ecosystemWallet = 0xac6a0fE8F105485F66f957FF383E8400E88Ef986;
    ...
    constructor () {
        ...
        IPancakeRouter02 _pancakeswapV2Router =
            IPancakeRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);//ropsten uniswap routerv02
    ...
    function excludeFromReward(address account) external onlyOwner() {
        require(
            account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, //ropsten uniswap routerv02
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //Calculate all fess amount
        uint256 growthxtAmt = amount.mul(_growthxFee).div(percentDivider);
        uint256 tyonshieldAmt = amount.mul(_tyonshieldFee).div(percentDivider);
        uint256 fundAmt = amount.mul(_fundFee).div(percentDivider);
        uint256 ecosystemAmt = amount.mul(_ecosystemFee).div(percentDivider);




        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(growthxtAmt).sub(tyonshieldAmt).sub(fundAmt).sub(ecosystemAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(growthxtAmt).sub(tyonshieldAmt).sub(fundAmt).sub(ecosystemAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(growthxtAmt).sub(tyonshieldAmt).sub(fundAmt).sub(ecosystemAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(growthxtAmt).sub(tyonshieldAmt).sub(fundAmt).sub(ecosystemAmt)));
        }
        
        //Temporarily remove fees to transfer all fees
        _taxFee = 0;
        _liquidityFee = 0;

        //Send transfers to all fees wallet
        _transferStandard(sender, growthxWallet, growthxtAmt);
        _transferStandard(sender, tyonshieldWallet, tyonshieldAmt);
        _transferStandard(sender, fundWallet, fundAmt);
        _transferStandard(sender, ecosystemWallet, ecosystemAmt);
    ...
    function disableAllFees() external onlyOwner() {
        ...
        _growthxFee = 0;
        _previousGrowthxFee = _growthxFee;
        _tyonshieldFee = 0;
        _previousTyonshieldFee = _tyonshieldFee;
        _fundFee = 0;
    ...
    /**
     * @dev Call this function if required to set a different Growthx 
     * wallet address.
     *
     * Requirements:
     *
     * The growthx wallet cannot be the zero address.
     */
    function setGrowthxWallet(address newWallet) external onlyOwner() {
        require(newWallet != address(0), 
            "BEP20: the new wallet cannot be the zero address."
        );
        growthxWallet = newWallet;
    }

        /**
     * @dev Call this function if required to set a different TYONShield 
     * wallet address.
     *
     * Requirements:
     *
     * The tyonshield wallet cannot be the zero address.
     */
    function setTyonshieldWallet(address newWallet) external onlyOwner() {
        require(newWallet != address(0), 
            "BEP20: the new wallet cannot be the zero address."
        );
        tyonshieldWallet = newWallet;
    }

        /**
     * @dev Call this function if required to set a different FundMe 
     * wallet address.
     *
     * Requirements:
     *
     * The fundme wallet cannot be the zero address.
     */
    function setFundWallet(address newWallet) external onlyOwner() {
        ...
        /**
     * @dev Call this function if required to set a different Ecosystem Reserve 
     * wallet address.
     *
     * Requirements:
     *
     * The wallet cannot be the zero address.
     */
