...
contract OwnbitMultiSig {
  ...
  function spend(address destination, uint256 value, uint8[] vs, bytes32[] rs, bytes32[] ss) external {
    require(destination != address(this), "不能转给自己");
    require(_validSignature(address(0x0), destination, value, vs, rs, ss), "无效签名");
    require(address(this).balance >= value && value > 0, "余额或转账值无效");
