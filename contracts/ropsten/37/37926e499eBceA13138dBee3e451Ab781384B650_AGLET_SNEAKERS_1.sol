...
contract AGLET_SNEAKERS_1 is ERC721URIStorage, Ownable, IMintable, ERC2981 {
    ...
    mapping(uint256 => string) public sneakerName;

    mapping(uint256 => string) public sneakerBrand;

    mapping(uint256 => string) public sneakerBatchNumber;

    mapping(uint256 => string) public sneakerEdition;

    mapping(uint256 => string) public sneakerDrop;

    mapping(uint256 => string) public sneakerWorld;
    ...
    public onlyMinters {
        ...
        _setTokenAttributes(tokenId, name, brand, batchNumber, edition, drop, world);
    }
    ...
    function safeMintWithRoyalty(
        ...
        string memory world)
    public onlyMinters {
        safeMint(to, tokenId, tokenUri, name, brand, batchNumber, edition, drop, world);
    ...
    public override onlyMinters {
        ...
        index = _setTokenRoyaltyFromBlob(mintingBlob, index,tokenId);
        index = _setTokenUriFromBlob(mintingBlob, index,tokenId);
        _setTokenAttributesFromBlob(mintingBlob, index,tokenId);
        emit TokenMintedFor(tokenId, user);
    }

    function _setTokenRoyaltyFromBlob(
        bytes calldata blob, uint256 startIndex,uint256 tokenId
    ) internal
    returns (
        uint256){
        uint96 royaltyNumerator;
        address royaltyReceiver;
        uint256 index;
        (royaltyNumerator, index) = Minting.getRoyaltyFraction(blob, startIndex);
        (royaltyReceiver, index) = Minting.getRoyaltyReceiver(blob, index);
        _setTokenRoyalty(tokenId, royaltyReceiver, royaltyNumerator);
        return index;
    }

    function _setTokenUriFromBlob(
        bytes calldata blob, uint256 startIndex, uint256 tokenId
    ) internal
    returns (
        uint256){
        string memory tokenURI;
        uint256 index;
        (tokenURI, index) = Minting.getNextString(blob, startIndex);
        _setTokenURI(tokenId, tokenURI);
        return index;
    }

    function _setTokenAttributesFromBlob(
        bytes calldata blob, uint256 startIndex, uint256 tokenId)
    internal {
        ...
        string memory world;
        uint256 index;
        (name, index) = Minting.getNextString(blob, startIndex);
        (brand, index) = Minting.getNextString(blob, index);
        (batchNumber, index) = Minting.getNextString(blob, index);
        (edition, index) = Minting.getNextString(blob, index);
        (drop, index) = Minting.getNextString(blob, index);
        (world, index) = Minting.getNextString(blob, index);
        sneakerName[tokenId] = name;
        sneakerBrand[tokenId] = brand;
        sneakerBatchNumber[tokenId] = batchNumber;
        sneakerEdition[tokenId] = edition;
        sneakerDrop[tokenId] = drop;
        sneakerWorld[tokenId] = world;
    }
...
library Minting {
    ...
    function getNextString(bytes calldata blob, uint256 startIndex)
    internal
    pure
    returns (
        string memory,
        ...
        bytes calldata nextStringBytes = blob[startIndex : uint256(index)];
        string memory nextString = string(
            nextStringBytes
        );
        return (nextString, uint256(index) + 1);
    }
