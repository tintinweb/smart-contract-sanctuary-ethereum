...
//        if (amount > _txLimit) revert ErrorExceedTransactionLimit();
        ...
        if (amount + _totalMinted() > _maxSupply) revert("max supply");
        if (amount > _txLimit) revert("transaction limit");
...
//        if (msg.value < requiredValue) revert ErrorInsufficientFund();
        ...
        if (userMinted > _walletLimit) revert("wallet limit");

        if (msg.value < requiredValue) revert("insufficient fund");
