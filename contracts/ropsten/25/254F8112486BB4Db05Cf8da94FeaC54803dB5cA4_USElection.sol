...
contract USElection is Ownable {
    event LogStateResult(uint8 winner, uint8 stateSeats, string name);
    event LogElectionEnded(uint8 winner);

    uint8 public constant TRUMP = 1;
    uint8 public constant BIDEN = 2;
    
    bool public electionEnded;

    mapping(uint8 => uint8) public seats;
    mapping(string => bool) public submittedStates;
    ...
    function submitStateResult(StateResult calldata result) public onlyOwner onlyActiveElection {
        require(result.votesTrump != result.votesBiden, "There can't be a tie");
        require(result.stateSeats > 0, "State must have at least 1 seat");
        require(!submittedStates[result.name], "This state was already submitted");
        ...
        submittedStates[result.name] = true;
    ...
    function currentLeader() public view returns (uint8) {
        if(seats[BIDEN] > seats[TRUMP]) {
            return BIDEN;
        } else if(seats[TRUMP] > seats[BIDEN]) {
            return TRUMP;
        }
