...
contract BurnTokenX is ERC20Burnable, Ownable {
    ...
    uint constant _initial_supply = 1 * (10**12) * (10**18); // 1 trillion tokens, 18 decimals
    ...
    uint256 public swapThreshold = _initial_supply * 30 / 10000; // .3% of totalsupply
    ...
    constructor(string memory name__, string memory symbol__, address treasury, address development, address buyback, address[8] memory teamWallets) ERC20(name__, symbol__) {
        // set fees to max
        ...
        _mint(0x000000000000000000000000000000000000dEaD, _initial_supply * 80 / 100); // burn baby burnnnnn it's a crypto inferno
    ...
    function setSellFee(
        ...
        require(liquidity + treasury + dev + buyback < FEE_DENOM, "invalid fee structure");
    ...
    function excludeFromMaxTransaction(address account) external onlyOwner {
        require(!_isExcludedFromMaxTransaction[account], "Account is already excluded from max transaction");
    ...
    function excludeFromMaxWallet(address account) external onlyOwner {
        require(!_isExcludedFromMaxWallet[account], "Account is already excluded from max wallet");
    ...
    function includeInMaxTransaction(address account) external onlyOwner {
        require(_isExcludedFromMaxTransaction[account], "Account is already included in max transaction");
    ...
    function includeInMaxWallet(address account) external onlyOwner {
        require(_isExcludedFromMaxWallet[account], "Account is already included in max wallet");
        _isExcludedFromMaxWallet[account] = false;

        emit includedInMaxWallet(account);
    }

    function checkTransferAllowed(address from, address to, uint256 amount, bool takeFee) internal view {
        require(!_isBlackListedBot[from] || !_isBlackListedBot[to], "You are blacklisted");
        if (to != uniswapV2Pair) {
            require(balanceOf(to) + amount < maxWalletSize || _isExcludedFromMaxWallet[to], "Exceeds receivers maximum wallet size");
        }
        if(takeFee){
            require(
                amount <= maxTransactionSize || 
                (_isExcludedFromMaxTransaction[from] || _isExcludedFromMaxTransaction[to]),
    ...
    function checkFeeRequired(address from, address to) internal view returns (bool, FeeType) {
    ...
    function calculateFee(uint256 amount, FeeType feeType) internal view returns (uint256 fee) {
        uint256 feePercentage = 0;
        if (feeType == FeeType.Buy){
            feePercentage = buyFee.treasury + buyFee.liquidity + buyFee.dev + buyFee.buyback;
        } else if (feeType == FeeType.Sell){
            feePercentage = sellFee.treasury + sellFee.liquidity + sellFee.dev + sellFee.buyback;
        }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        uint256 feeDenominator = buyFee.treasury + buyFee.liquidity + buyFee.dev + buyFee.buyback;

        //commented lines are vestigial
        // uint256 treasuryFee = (tokens * buyFee.treasury) / feeDenominator;
        uint256 liquidityFee = (tokens * buyFee.liquidity) / feeDenominator;
        // uint256 devFee = (tokens * buyFee.dev) / feeDenominator;
        // uint256 buybackFee = (tokens * buyFee.buyback) / feeDenominator;
        ...
        uint256 ethDenominator = buyFee.treasury + (buyFee.liquidity / 2) + buyFee.dev + buyFee.buyback;

        uint256 treasuryEth = (contractEth * buyFee.treasury) / ethDenominator;
        uint256 liquidityEth = (contractEth * (buyFee.liquidity / 2)) / ethDenominator;
        uint256 buybackEth = (contractEth * buyFee.buyback) / ethDenominator;
        uint256 devEth = (contractEth * buyFee.dev) / ethDenominator;
        ...
        _developmentWallet.transfer(address(this).balance); // send all remaining eth to dev wallet (in case of rounding)
    ...
    function updateLiquidityLock(uint256 lockTime) external onlyOwner {
        require(block.timestamp > liquidityLockTime, "New liquidity lock time must be after the current lock time");
    ...
    function removeLiquidityETH (
        ...
        require(block.timestamp > liquidityLockTime, "Liquidity removal is locked");
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        ...
        //check for swapAndLiquify available
            ...
            if (contractBalance >= swapThreshold && !inSwapAndLiquify && swapAndLiquifyEnabled){
