...
contract Airdrop is Pausable, Ownable, ReentrancyGuard {
    ...
    function setInvestor(address _investor, uint _amount) external checkKeeper {
        if (invests[_investor] > 0) {
            totalInvested -= invests[_investor];
    ...
    function setInvestors(address[] memory _investors, uint[] memory _amounts) external checkKeeper {
        ...
        for (uint i = 0; i < _investors.length; i++) {
            ...
            if (!investors.contains(_investor)) investors.add(_investor);
    ...
    function claim() external nonReentrant checkInvestor {
        require (startedClaim == true, "!available");
        require (claimed[msg.sender] == false, "already claimed");

        uint claimAmount = invests[msg.sender];
    ...
    function emergencyWithdraw(uint256 _amount) external checkKeeper {
