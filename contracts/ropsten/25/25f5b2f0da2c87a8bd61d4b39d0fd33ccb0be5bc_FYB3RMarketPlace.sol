...
contract FYB3RMarketPlace is ERC721URIStorage, Ownable {
    ...
    function acceptOffer(uint256 tokenId) external onlyNftOwner(tokenId){
        ...
        uint256 serviceFee = biddingInfo[tokenId].bidPrice.mul(market_fee.div(1000));
        uint256 royaltyFee = biddingInfo[tokenId].bidPrice.mul(royalty.div(1000));
        buyer.transfer(biddingInfo[tokenId].bidPrice - serviceFee - royaltyFee);
    ...
    function BuyFixedPriceNFTs(uint256 tokenId)payable public returns(bool){
        ...
        uint256 royalty = royaltyDatas[tokenId].royalty;
        uint256 royaltyFee = msg.value.mul(royalty.div(1000));
        uint256 serviceFee = msg.value.mul(market_fee.div(1000));
        payable(Fixedprices[tokenId].owner).transfer(msg.value - royaltyFee - serviceFee);
        payable(royaltyDatas[tokenId].creator).transfer(royaltyFee);
        payable(owner()).transfer(serviceFee);
