...
contract LR4ChainContract is LR4Ownable,  LR4Sealed, LR4Common, LR4Killable {
   ...
   //add new parcel
    function addParcel(string memory _code, string memory _name, string memory  _urlPdf, string memory _urlTranslatedPdf, 
      string  memory _hashPdf, string memory _hashTranslatedPdf, string memory _cropType, string memory _translatedCropType,
      uint256  _timestamp ) onlyOwner onlyIfNotSealed external{
            ...
            parcelList[_code] = Parcel({urlPdf: _urlPdf, hashPdf:_hashPdf, datePdf: _timestamp, code: _code,
             ...
             urlTranslatedPdf: _urlTranslatedPdf, hashTranslatedPdf: _hashTranslatedPdf});
    }
    //add new crop operation to existing parcel
    function addCropOperation(string memory _parcelCode, string memory _cropCode, string memory _urlTranslatedPdf, 
        string memory _urlPdf, string  memory _hashPdf, string memory _hashTranslatedPdf, uint256 _timestamp) onlyOwner onlyIfNotSealed external {
            ...
            cropOperations[keyCrop].hashTranslatedPdf = _hashTranslatedPdf;
            ...
            cropOperations[keyCrop].hashTranslatedPdfAnalysis='';
    ...
    //add harvest associated to parcel
     function addHarvest(string memory _parcelCode, string memory _harvestCode, string memory _harvestName, string memory _recipientCompany,
        string memory _urlPdf, string  memory _hashPdf,  string memory _urlTranslatedPdf, string memory _hashTranslatedPdf,
         uint256 _timestamp, uint256 _dateHarvest) onlyOwner onlyIfNotSealed external {
            ...
            harvestList[keyParcelHarvest].hashTranslatedPdf = _hashTranslatedPdf;
    ...
    function addCropAnalysis(string memory _parcelCode,  string memory _cropCode,  
         string memory _urlPdf, string  memory _hashPdf, string memory _urlTranslatedPdf, string  memory _hashTranslatedPdf,
         uint256 _timestamp ) onlyOwner onlyIfNotSealed external {
            //check if parcel exists
             require(parcelList[_parcelCode].exists==1, strNotExtParcel);
             ...
             cropOperations[keyParcelCrop].hashTranslatedPdfAnalysis = _hashTranslatedPdf;
    ...
    function addHarvestAnalysis(string memory _parcelCode, string memory  _harvestCode, 
        string memory _analysisCode,  string memory _urlPdf, 
        string memory _urlTranslatedPdf, string memory _hashTranslatedPdf,
    ...
    function getHarvest(string memory _parcelCode, string memory _harvestCode) external view  returns (string memory, string memory,  Parcel memory, CropOperation [] memory, Harvest memory, Analysis memory) {
        string memory compositeKey = concat(_parcelCode,"|", _harvestCode);
