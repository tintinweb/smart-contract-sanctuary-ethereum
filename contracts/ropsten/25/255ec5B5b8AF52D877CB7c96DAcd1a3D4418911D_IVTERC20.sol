...
contract IVTERC20V2 is IVTERC20 {
    using SafeMath for uint256;
    ...
    function increaseAllowance(address spender, uint256 addedValue) external returns(bool) {
        ...
        uint256 amount =   allowance(owner, spender).add(addedValue);
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external returns(bool) {
        ...
        uint256 amount = currentAllowance.sub(subtractedValue);
        unchecked {
            _allowed[owner][spender] = amount;
            emit Approve(owner, spender, amount);
        }
    ...
    function IVTversion() external pure returns(uint32)
    {
        return 2;
    }
}