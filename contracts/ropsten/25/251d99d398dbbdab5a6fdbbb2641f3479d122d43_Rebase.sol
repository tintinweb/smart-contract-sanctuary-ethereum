...
contract Rebase is IERC20, Ownable, ReentrancyGuard {
    ...
    function _convert(uint256 _contractTokenAmount) private {
        uint256 stableBalanceBefore = IERC20(stableCoinAddress).balanceOf(address(staking));
        _swapTokensForStable(_contractTokenAmount, address(staking));
        uint256 stableBalanceAfter = IERC20(stableCoinAddress).balanceOf(address(staking)).sub(stableBalanceBefore);
