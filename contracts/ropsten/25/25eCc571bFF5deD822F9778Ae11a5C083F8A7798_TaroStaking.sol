...
abstract contract TaroStakingTokensStorage {
    using SafeMath for uint256;
    ...
    mapping(address => uint256) public _unStakeUnlockingTime;
    ...
    function getUnstakeTimeOf(address account) public view returns (uint256) {
        return _unStakeUnlockingTime[account];
    }

    function getClaimingFeePercent(address account) internal view returns(uint256) {
      if(block.timestamp - _unStakeUnlockingTime[account]<3*86400)
      return 200;
      else if ((block.timestamp - _unStakeUnlockingTime[account]>=3*86400) && (block.timestamp - _unStakeUnlockingTime[account]<604800))
      return 100;
      else if (block.timestamp - _unStakeUnlockingTime[account]>=604800 && (block.timestamp - _unStakeUnlockingTime[account]<2*604800))
      return 50;
      else if (block.timestamp - _unStakeUnlockingTime[account]>=2*604800 && (block.timestamp - _unStakeUnlockingTime[account]<4*604800))
      return 40;
      else if (block.timestamp - _unStakeUnlockingTime[account]>=4*604800 && (block.timestamp - _unStakeUnlockingTime[account]<6*604800))
      return 30;
      else if (block.timestamp - _unStakeUnlockingTime[account]>=6*604800 && (block.timestamp - _unStakeUnlockingTime[account]<8*604800))
      return 20;
      else if (block.timestamp - _unStakeUnlockingTime[account]>=8*604800 && (block.timestamp - _unStakeUnlockingTime[account]<10*604800))
      return 10;
      else if (block.timestamp - _unStakeUnlockingTime[account]>=10*604800 && (block.timestamp - _unStakeUnlockingTime[account]<12*604800))
      return 5;
      else
      return 1;
    }

    constructor(IERC20 rewardsToken_, IERC20 stakingToken_,address feeCollector_) public {
    ...
    function _stake(address account, uint256 amount) internal {
        _stakingToken.safeTransferFrom(account, address(this), amount);
        uint256 privBalance = _balances[account];
        _balances[account] = _balances[account].add(amount);
        if(_unStakeUnlockingTime[account]==0 && privBalance==0)
        _unStakeUnlockingTime[account]= 60 + block.timestamp;
        else
        _unStakeUnlockingTime[account] = (privBalance + amount)/(privBalance/_unStakeUnlockingTime[account] + amount/(block.timestamp+60));
        _totalSupply = _totalSupply.add(amount);
        _onMint(account, amount);
    }

    function _unstake(address account, uint256 amount) internal {
     uint256 fee = amount.mul(getClaimingFeePercent(account)).div(10000);
     uint256 netUnstake = amount.sub(fee);
     _feePool = _feePool.add(fee);
     _stakingToken.safeTransfer(account, netUnstake);
      _balances[account] = _balances[account].sub(amount);
     _stakingToken.safeTransfer(_feeCollector, fee);
...
contract TaroStaking is TwoStageOwnable, TaroStakingTokensStorage {
    using SafeMath for uint256;
    ...
    uint256 public constant MAX_DISTRIBUTION_DURATION = 7 days;
    ...
    constructor(
        ...
        address owner_
    ) public TwoStageOwnable(owner_) TaroStakingTokensStorage(rewardsToken_, stakingToken_,feeCollector) {
    }
    ...
    // }
      ...
      function unstake(uint256 amount) public onlyPositiveAmount(amount) {
        address sender = msg.sender;
        require(amount <= balanceOf(sender), "Unstaking amount exceeds staked balance");
        require(getTimestamp() >= getUnstakeTimeOf(sender), "Unstake time not passed");
        _lockRewards(sender);
        _unstake(sender, amount);
        emit Unstaked(sender, amount);
    }
...
contract TaroStakingSyntheticToken is TaroStaking {
    ...
    constructor(
        ...
        address feeCollector,
        address owner_
    ) public TaroStaking(rewardsToken_, stakingToken_,feeCollector, owner_) {
        name = name_;
        symbol = symbol_;
        decimals = decimals_;
    }
