...
contract TOKEN_NFT is ERC721URIStorage {
    ...
    function getTokens(uint256 n)
            ...
            returns (uint256[] memory nTokens)
        {
            ...
            uint256 arrLength = mintedIdsArray.length;
            if (n > arrLength) {
                nTokens = new uint256[](arrLength);
                for (uint256 i = 0; i < arrLength; i++) {
                    nTokens[i] = mintedIdsArray[arrLength - 1];
                    arrLength--;
                }
            } else {
