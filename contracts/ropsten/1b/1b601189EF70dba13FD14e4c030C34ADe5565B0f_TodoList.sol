...
contract TodoList {
    ...
    function setTask(string memory name_, uint256 daysTodo_) external stageNext(id[msg.sender]) {
        tasks[msg.sender][id[msg.sender]].name = name_;
        tasks[msg.sender][id[msg.sender]].endTask = block.timestamp + daysTodo_ * 1 days;
    ...
    function getTask(uint256 id_) external view returns (Task memory) {
       // require(id[msg.sender] != 0, "invalid id");
