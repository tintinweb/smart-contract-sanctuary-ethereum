 ...
 * @notice It vests the ORBIT tokens to an owner over a linear schedule.
...
contract VestingContractWithFeeSharing is Ownable, ReentrancyGuard {
     ...
     * @param _maxAmountToWithdraw maximum amount in ORBIT to withdraw per period
    ...
    /**
     * @notice Unlock ORBIT tokens
     * @dev It includes protection for overstaking
     */
    function unlockOrbitToken() external nonReentrant onlyOwner {
        require(
            (numberPastUnlocks == NUMBER_UNLOCK_PERIODS) || (block.number >= nextBlockForUnlock),
            "Unlock: Too early"
        );

        uint256 balanceToWithdraw = orbitToken.balanceOf(address(this));
        ...
        // Transfer ORBIT to owner
        orbitToken.safeTransfer(msg.sender, balanceToWithdraw);
    ...
    function withdrawOtherCurrency(address _currency) external nonReentrant onlyOwner {
        require(_currency != address(orbitToken), "Owner: Cannot withdraw ORBIT");
