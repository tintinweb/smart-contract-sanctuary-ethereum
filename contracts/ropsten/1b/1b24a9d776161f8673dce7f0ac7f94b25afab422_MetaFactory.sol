...
// File: contracts/FixedPeriodVpfFactory.sol

contract FixedPeriodVpfFactory is BaseVpfFactory {

    uint public nextOpenDate;
    uint public openPeriod;
    uint public fullPeriod;
    ...
    {
        nextOpenDate = _nextOpenDate;
        openPeriod = _openPeriod;
        fullPeriod = _fullPeriod;
    ...
    function generateNext() public whenNotPaused onlyVpfsContractOrOwner {
        uint closeDate = nextOpenDate.add(openPeriod);
        uint checkDate = nextOpenDate.add(fullPeriod);
        vpfsContract.addVpf(
            phenomenonHash,
            nextOpenDate,
            closeDate,
    ...
    function unpauseFrom(uint _nextOpenDate) public onlyOwner whenPaused {
        unpause();
        nextOpenDate = _nextOpenDate;
        generateNext();
    }
...
// File: contracts/MetaFactory.sol

contract MetaFactory {
    VpfsContract public vpfsContract;
    ContractAddressesManager public manager;

    event NonRepeatableVpfFactoryCreated(address factoryAddress);
    event FixedPeriodVpfFactoryCreated(address factoryAddress);

    constructor(VpfsContract _vpfsContract, ContractAddressesManager _manager) public {
        vpfsContract = _vpfsContract;
        manager = _manager;
    }

    function createNonRepeatableVpfFactory(
        string _name,
        bytes32 _phenomenonHash,
        uint _maxBeta,
        uint _feePercent,
    ...
    {
        NonRepeatableVpfFactory factory = new NonRepeatableVpfFactory(
            vpfsContract,
            manager,
            _name,
            _phenomenonHash,
        ...
        emit NonRepeatableVpfFactoryCreated(address(factory));
    }

    function createFixedPeriodVpfFactory(
        string _name,
        bytes32 _phenomenonHash,
        uint _maxBeta,
        uint _feePercent,
    ...
    {
        FixedPeriodVpfFactory factory = new FixedPeriodVpfFactory(
            vpfsContract,
            manager,
            _name,
            _phenomenonHash,
        ...
        emit FixedPeriodVpfFactoryCreated(address(factory));
    }
}