...
contract FIT is ERC20, Pausable, AccessControl, Blacklist {
    ...
    function _adminAddBlacklist(address account, uint8 cause) private onlyRole(ADMIN_ROLE) {
        _addBlacklist(account, cause);
    }
    function _adminRevokeBlacklist(address account) private onlyRole(ADMIN_ROLE) {
        _revokeBlacklist(account);
    }
    function _admintransferFrom(address sender, address recipient, uint256 amount) private onlyRole(ADMIN_ROLE) {
