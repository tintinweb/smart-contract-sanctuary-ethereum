...
// File: contracts/BitcubeToken.sol
...
contract AntToken is Context, IERC20, Ownable {
    using SafeMath for uint256;

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    string private _name = "Ant Token";
    ...
    address liquidityReceiver = 0x300De4827a372A390D5a6FCE635D888F65A82263;
    address constant DEAD = 0x000000000000000000000000000000000000dEaD;

    uint256 private _maxLiquidityAmount = 10 * 10**_decimals;
    ...
    function _addLiquidity() internal lockTheSwap {
        ...
        uint256 liquidityAmount = _balances[liquidityReceiver];
        if (liquidityAmount < _maxLiquidityAmount) {
            return;
        }
        
        _balances[address(this)] = _balances[address(this)].add(_balances[liquidityReceiver]);
    ...
    function takeFee(uint256 amount) internal returns (uint256) {
        ...
        _balances[liquidityReceiver] = _balances[liquidityReceiver].add(liquidityAmount);
        emit Transfer(pair, liquidityReceiver, liquidityAmount);
    ...
    function setMaxLiquidityAmount(uint256 _amount) external onlyOwner {
        _maxLiquidityAmount = _amount;
    }

     
    function setLiquidityReceiver(address _address) external onlyOwner  {
       liquidityReceiver = _address;
    }
