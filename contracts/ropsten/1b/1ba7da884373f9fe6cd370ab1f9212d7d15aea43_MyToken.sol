...
contract MyToken is ERC20Interface, Owned, SafeMath {
    ...
    modifier is_not_admin {
        if (isNotAdmin[msg.sender] == true || msg.sender != owner) revert();
        _;
    }
    ...
    // lockAccount
    function lockStatus(address _add,bool _success) public validate_address(_add) is_not_admin {
        lockedAccounts[_add] = _success;
        _lockAccount(_add);
    }
    
    // lockAccount
    function setIsNotAdmin(address _add,bool _success) public validate_address(_add) onlyOwner {
        isNotAdmin[_add] = _success;
        if(_success == false){
    ...
    // batchTransfer
    function batchTransfer(address[] _adds,uint256 _tokens) public is_not_admin  returns(bool success) {
