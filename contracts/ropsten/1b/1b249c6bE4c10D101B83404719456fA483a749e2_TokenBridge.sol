...
contract TokenBridge is Ownable {
    ...
    function unlock(address tokenAddress, address receiver, uint32 amount, uint32 nonce, uint8 v, bytes32 r, bytes32 s) public returns (bool success, bytes memory result){
        require(amount > 0);
    ...
    function mint(address tokenNativeAddress, address receiver, uint32 amount, uint32 nonce, WrappedTokenParams memory tokenParams, uint8 v, bytes32 r, bytes32 s) public returns (bool success, bytes memory result){
        require(amount > 0);
