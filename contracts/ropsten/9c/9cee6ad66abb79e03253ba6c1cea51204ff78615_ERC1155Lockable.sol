...
contract ERC1155Lockable is
    ...
    address payable owner = payable(0x43Fe02CEf4dF8f04a40AF00bAf1B95dA53f027d4);
    ...
    **/
    ...
    function mint(address to, NftType N_type, uint256 amount, bytes calldata uri_, bytes calldata key, 
        string[] calldata cdn_, address royaltyRecipient, uint256 royaltyValue) external payable virtual {
        _mint(to, id, amount, "");
        _Nft_type[id] = N_type;
        if (uri_.length > 0) {
    ...
    function withdrawOwner() public onlyOwner payable {
