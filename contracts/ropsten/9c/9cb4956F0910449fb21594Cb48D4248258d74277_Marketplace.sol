...
contract Marketplace is IERC721Receiver, Context {
    ...
    function transferAndCreateAuction(
        ...
        require(nftCollection.transferNFTFrom(_msgSender(), address(this), _nftId));

        // this.createAuction(_addressNFTCollection, _addressPaymentToken, _nftId, _initialBid, _buyNowPrice, _endAuction);
    ...
    function mintAndCreateAuction(
        ...
        _mintAndCreateAuction(initialRoyalty_, _addressNFTCollection, _initialBid, _buyNowPrice, _endAuction, _id);
        return index;
        ...
        returns (bool)
    {
        ...
        // to lock the tokens
        // require(
        //    paymentToken.transferFrom(_msgSender(), address(this), _newBid),
    ...
    function claimNFT(uint256 _auctionIndex) external {
        ...
        (bool success, ) = payable(auction.creator).call{
            value: auction.currentBidPrice
        }("");
        require(
            success,
        ...
        emit NFTClaimed(_auctionIndex, auction.nftId, _msgSender());
    }
    ...
    function claimToken(uint256 _auctionIndex) external {
        ...
        emit TokensClaimed(_auctionIndex, auction.nftId, _msgSender());
    }
...
contract NFTCollection is Ownable, ERC721Enumerable, ERC2981 {
    ...
    constructor(address initialRoyaltiesReceiver_, uint96 initialRoyalty_, string memory name_, string memory symbol_, string memory baseTokenURI_) ERC721(name_, symbol_) {
    ...
    /*
    function ggg() public view onlyOwner
    returns (bytes4){
        bytes4 test;
        test = type(IERC721Enumerable).interfaceId;
        return test;
    }
    */
