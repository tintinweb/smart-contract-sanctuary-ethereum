...
contract Oracle is ERC20, Ownable{
    ...
    address public marketingWallet = 0x499b61b070de86E8d62A66d8747D2381469C2D4C;
    address public lpRecipient = 0x499b61b070de86E8d62A66d8747D2381469C2D4C;
    ...
    Taxes public buyTaxes = Taxes(80, 0);
    Taxes public sellTaxes = Taxes(50, 10);
 
    uint256 public increaseFactor = 10;
    uint256 public decreaseFactor = 5;
    ...
    function setDynamicLPTax(bool isSell) internal {
        if(isSell){
            if(sellTaxes.lp == 100) return;
            else if(sellTaxes.lp + increaseFactor > 100) sellTaxes.lp = 100;
            else sellTaxes.lp += increaseFactor;
        }
        else if(!isSell){
            if(sellTaxes.lp == 10) return;
            else if(sellTaxes.lp - decreaseFactor < 10) sellTaxes.lp = 10;
            else sellTaxes.lp -= decreaseFactor;
        }
    }
 
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(amount > 0, "Transfer amount must be greater than zero");
        require(!isBlacklisted[sender] && !isBlacklisted[recipient], "Freezy says: Bye Bye Bots");
        ...
        //calculate fee
            ...
            if(recipient == pair){
                setDynamicLPTax(true);
                fee = amount * (sellTaxes.marketing + sellTaxes.lp) / 1000;
            } 
            else if(sender == pair) {
                setDynamicLPTax(false);
                fee = amount * (buyTaxes.marketing + buyTaxes.lp) / 1000;
            }
        }
    ...
    function swapForFees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            uint256 denominator = (sellTaxes.lp + sellTaxes.marketing) * 2;
            uint256 tokensToAddLiquidityWith = swapThreshold * sellTaxes.lp / denominator;
    ...
    function setBuyTaxes(uint256 _marketing, uint256 _lp) external onlyOwner{
        require(_marketing + _lp <= 150, "Max Fee is 15%");
        buyTaxes = Taxes(_marketing, _lp);
    }
 
    function setSellTaxes(uint256 _marketing, uint256 _lp) external onlyOwner{
        require(_marketing + _lp <= 150, "Max Fee is 15%");
        sellTaxes = Taxes(_marketing, _lp);
    }
 
    function setDynamicTaxFactors(uint256 _increase, uint256 _decrease) external onlyOwner{
        increaseFactor = _increase;
        decreaseFactor = _decrease;
    }
