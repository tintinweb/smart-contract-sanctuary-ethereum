...
contract CryptoCraftNFT is ERC721, ERC721Enumerable, Ownable {
    // Private 
    ...
    uint256 public constant MAX_PER_TXN = 3;
    ...
    function publicMint(uint256 _amount) external payable {
        ...
        require(supply + _amount + reserveSupply <= MAX_SUPPLY, "CC: Max token supply exceeded");
    ...
    function whitelistMint(uint8 _amount) external payable {
        ...
        require(_amount <= vipList[msg.sender], "CC: Exceeded your max vip mint");
        require(supply + _amount + reserveSupply <= MAX_SUPPLY, "CC: Max token supply exceeded");
        require(TOKEN_PRICE * _amount <= msg.value, "CC: Ether value sent is not correct");
    ...
    function reserve(uint256 n, address mintReceiver) public onlyOwner {
      ...
      require(supply + n + reserveSupply <= MAX_SUPPLY, "CC: Max token supply exceeded");
