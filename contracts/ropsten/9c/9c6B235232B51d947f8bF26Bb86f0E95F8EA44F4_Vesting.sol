...
contract Vesting is Ownable, ReentrancyGuard {
    ...
    function released(address beneficiary) external view returns (uint256) {
        ...
        if(_upfrontReleased[beneficiary]) {
            released = released.add(_beneficiaries[beneficiary].upfront);
        }
    ...
    /**
     * @return the amount of the token unlocked.
     */
    function unlocked(address beneficiary) external view returns (uint256) {
        uint256 unlocked = _vestedAmount(beneficiary);

        if(!_upfrontReleased[beneficiary]) {
            unlocked = unlocked.add(_beneficiaries[beneficiary].upfront);
        }
     ...
     */
    function locked(address beneficiary) external view returns (uint256) {
        return _beneficiaries[beneficiary].amount.sub(_vestedAmount(beneficiary));
    }
