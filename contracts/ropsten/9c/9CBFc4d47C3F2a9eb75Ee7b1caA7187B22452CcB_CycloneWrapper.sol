...
contract CycloneWrapper {
  ...
  function deposit(ICycloneV2dot2 _cyclone, bytes32 _commitment, bool _buyCYC) external payable {
    ...
    uint256 cycDeno;
    uint256 fee;
    (commitment, insertedIndex, blocktime, cycDeno, fee) = _cyclone.deposit.value(coinAmount)(_commitment);
    emit Deposit( commitment, insertedIndex, blocktime, cycDeno, fee );
    if (remainingCoin > 0) {
