...
contract Sorbettiere is Ownable {
    ...
    function changeEndTime(uint32 addSeconds) external onlyOwner {
        endTime += addSeconds;//To-do: Currently expires on April 21 2022. See if this deadline is extended before then on FTMSCAN
    }
...
// How many pools are in the contract
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.stakingTokenTotalAmount != 0) {
            ...
            uint256 iceReward =
                multiplier * icePerSecond * pool.allocPoint / totalAllocPoint;//total pending reward for the pool
            accIcePerShare += iceReward * 1e12 / pool.stakingTokenTotalAmount;//total pending reward per share in the pool
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 iceReward =
            multiplier * icePerSecond * pool.allocPoint / totalAllocPoint;//total pending reward for the pool
        pool.accIcePerShare += iceReward * 1e12 / pool.stakingTokenTotalAmount;//total pending reward per share in the pool; continues to increment
        pool.lastRewardTime = uint32(block.timestamp);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount * pool.accIcePerShare / 1e12;// keeps track of already paid out rewards
