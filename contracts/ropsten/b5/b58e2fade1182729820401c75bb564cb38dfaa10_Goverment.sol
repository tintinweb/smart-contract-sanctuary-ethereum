...
contract Goverment {
    ...
    function changeMail(bytes32 newMailAddress) public {
        if (isCitizien(msg.sender)) {
            Person storage person = citiziens[msg.sender];
            person.mailAddress = newMailAddress;
        } else revert nope();
    }
    ...
    function withdrawBalance(uint256 amount) public {
        if (balances[msg.sender] < 10000000000000) revert lowBalance();
            ...
            if (amount < 10000000000000) revert belowMinimumAmount();
