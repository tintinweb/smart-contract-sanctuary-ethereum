...
contract NooneStaking is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    struct UserInfo {
        ...
        uint96 claimableNooneDividendToken;
		uint96 claimableETH;
        ...
        uint[] NFTTokenIDs;              
        //
        // We do some fancy math here. Basically, any point in time, the amount of NooneDividendTokens and ETH 
        // entitled to a user but is pending to be distributed is:
		//  weightedBalance = (user.amount * (1000+boostPointsBP * 1000+lockTimeBoost)) / 1000000
        //   pending NooneDividendToken reward = ((user.weightedBalance * pool.accRwtPerShare) - user.rewardDebt)
    ...
    struct PoolInfo {
        ...
        uint112 accRwtPerShare;     // Accumulated NooneDividendTokens per share, times 1e12.
    ...
    INoone public Noone;
	// The reward token 
	IRewardToken public nooneDividendToken;
    ...
    // NooneDividendToken tokens created per block.
    uint256 public nooneDividendTokenPerBlock;
	...
	event RewardTokenSet(address indexed tokenAddress, uint256 indexed nooneDividendTokenPerBlock, uint256 timestamp);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event UpdateEmissionRate(address indexed user, uint256 nooneDividendTokenPerBlock);
...
//--------------------------------------------------------VIEW FUNCTIONS --------------------------------------------------------
    ...
    // View function to see pending rewards (NooneDividendToken and ETH) on frontend.
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256, uint256) {
        ...
        uint256 PendingNooneDividendToken;
        uint256 PendingETH;
        if (block.number > pool.lastRewardBlock && weightedBalance != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 nooneDividendTokenReward = multiplier * nooneDividendTokenPerBlock * pool.allocPoint / totalAllocPoint;
            accRwtPerShare = accRwtPerShare + nooneDividendTokenReward * 1e12 / weightedBalance;
            ...
            PendingNooneDividendToken = (userWeightedAmount * accRwtPerShare / 1e12) - user.rewardDebt + user.claimableNooneDividendToken;
            PendingETH = (userWeightedAmount * accETHPerShare / 1e12) - user.ETHrewardDebt + user.claimableETH;
        }
        return(PendingNooneDividendToken, PendingETH);
    }
...
//--------------------------------------------------------PUBLIC FUNCTIONS --------------------------------------------------------
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(tokenReleased) {
            uint256 nooneDividendTokenReward = multiplier * nooneDividendTokenPerBlock * pool.allocPoint / totalAllocPoint;
            pool.accRwtPerShare = uint112(pool.accRwtPerShare + nooneDividendTokenReward * 1e12 / lpSupply);
        }
        uint256 ETHReward = multiplier * ETHPerBlock * pool.allocPoint / totalAllocPoint;
    ...
    function withdraw(uint32 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.weightedBalance > 0){
            ...
            if(tokenReleased) {
                if (user.claimableNooneDividendToken > 0) {
                    safeNooneDividendTokenTransfer(msg.sender, user.claimableNooneDividendToken);
                    user.claimableNooneDividendToken = 0;
                }
            }
	...
	//Adds Noone/ETH liquidity without transfer fees and stakes the amount. Refund any unspent ETH to the user.
    
	//@dev The Noone/ETH pool MUST be pool 0
    function addLiquidityNoFeeAndStake(uint256 amountTokensIn, uint256 amountETHMin, uint256 amountTokenMin, uint256 lockTime) public payable nonReentrant {
        INoone.LiquidityETHParams memory params;
        ...
        Noone.transferFrom(msg.sender, address(this), amountTokensIn);
        ...
        (,uint256 ETHUsed ,uint256 numLiquidityAdded) = Noone.addLiquidityETH{value: msg.value}(params);
        //noFeeAddLiquidityETH refunds unused ETH, but not Noone, we're forced to do the same here (any unused Noone is locked in the token contract and will be treated as transfer fees).
	...
	//Buy Noone with the ETH rewards from the Noone single asset pool to compound
	// @dev Noone single asset pool MUST be pool 1
    ...
    function buyAndStakeETH(uint256 amountOutMin, uint256 lockTime) public payable nonReentrant{
        ...
        uint256 _amount = Noone.balanceOf(address(this)) - balanceBefore;
        user.amount = uint112(user.amount + _amount);
        if (lockTime > 0){
...
//--------------------------------------------------------RESTRICTED FUNCTIONS --------------------------------------------------------	
    ...
    function rescueToken(address tokenAddress) public onlyOwner {
        require((tokenAddress != address(nooneDividendToken)) && !poolExistence[IERC20(tokenAddress)], "rescueToken : wrong token address");
    ...
    // Updates NooneDividendToken emision rate. Can only be called by the owner
    function updateEmissionRate(uint256 _nooneDividendTokenPerBlock) public onlyOwner {
        require(tokenReleased == true, "updateEmissionRate: Reward token not set");
		massUpdatePools();
        nooneDividendTokenPerBlock = _nooneDividendTokenPerBlock;
        emit UpdateEmissionRate(msg.sender, _nooneDividendTokenPerBlock);
    }
    // Sets the reward token address and the initial emission rate. Can only be called by the owner. 
    function setRewardToken(address _NooneDividendToken, uint _nooneDividendTokenPerBlock) public onlyOwner {
        require(tokenReleased == false, "Reward token already set");
        nooneDividendToken = IRewardToken(_NooneDividendToken);
        nooneDividendTokenPerBlock = _nooneDividendTokenPerBlock;
		tokenReleased = true;
        emit RewardTokenSet(_NooneDividendToken, _nooneDividendTokenPerBlock, block.timestamp);
    }
...
//--------------------------------------------------------INTERNAL FUNCTIONS --------------------------------------------------------
    function _deposit(address sender, uint256 _pid, uint256 _amount, uint256 lockTime) internal {
        ...
        if (user.weightedBalance > 0) {
            if (_amount == 0 && lockTime == 0) {
                if(tokenReleased) {
                    uint256 pending = (user.weightedBalance * pool.accRwtPerShare / 1e12) - user.rewardDebt + user.claimableNooneDividendToken;
                    if (pending > 0) {
                        safeNooneDividendTokenTransfer(sender, pending);
                    }
    ...
    function _addToClaimable(uint256 _pid, address sender) internal {
        ...
        if(tokenReleased) {
                ...
                if (pending > 0) {
                    user.claimableNooneDividendToken += uint96(pending);
                    user.rewardDebt = user.weightedBalance * pool.accRwtPerShare / 1e12;
                }
            }
    ...
    // Safe transfer function, just in case if rounding error causes pool to not have enough NooneDividendTokens.
    function safeNooneDividendTokenTransfer(address _to, uint256 _amount) internal {
        uint256 nooneDividendTokenBal = nooneDividendToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > nooneDividendTokenBal) {
            transferSuccess = nooneDividendToken.transfer(_to, nooneDividendTokenBal);
        } else {
            transferSuccess = nooneDividendToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeNooneDividendTokenTransfer: transfer failed");
    }
