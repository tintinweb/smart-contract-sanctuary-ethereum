...
contract TokenStake {
    ...
    uint256[5] public Duration = [30 days, 60 days, 90 days, 180 days, 360 days];  // these are locking periods of token we can also place number of seconds here
    uint256[5] public Bonus = [12, 37, 72, 121, 505]; // these bonus variables are related to duration and the amount will be multiply by 10 for example 10 percent is equals to 100
    ...
    constructor(address payable _owner, address token1) {
        ...
        stakeToken = IERC20(token1); // address of the token 
        totalFee = 0; // fee percantage if the token is getting any fee on transaction (multiply by 10)
        maxStakeableToken = 1e25; // this is the the maximum limit user can stake
        percentDivider = 1000;
        minimumStakeToken = 1e18; // this is the minimum user can stake
    }
    ...
    function stake(uint256 amount1, uint256 timeperiod) public {
        ...
        uint256 amount = amount1.sub((amount1.mul(totalFee)).div(percentDivider));  // calculates the amount that goes in contract if the token has fees 
    ...
    /** using this function the owner of this contract will change the minimum and maximum stake limit we can also remove this method this doesn't affect the calculation */
    ...
    /** these two method will help owner to withdraw any wrongly deposited token
