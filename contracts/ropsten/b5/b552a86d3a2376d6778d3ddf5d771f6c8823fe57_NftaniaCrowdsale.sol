...
contract NftaniaCrowdsale is Pausable, Ownable, ReentrancyGuard {
   ...
   //////////////////////// Main Variables /////////////////////////////////// 
        ...
        bool private closed;                                    // Flag ("true" ifcrowdsale is closed)
   ...
   //////////////////////// Events /////////////////////////////////////////// 
    ...
    // Wei Raised
        event TokensUpdate (uint256 weiRaised, uint256 totalTokensSold, uint256 remainingTokens);
    // New Dates Set
        event NewDatesSet(uint startDate, uint endDate);
    // New Purchase Range
        event NewPurchaseRange  (uint256 _minPurchase, uint256 _maxPurchase);
    // Remaining Tokens
        event RemainingTokens(uint256 _remainingTokens);
   ...
   //////////////////////// Initialize Contract //////////////////////////////////////   
    function initializeContract (address  _tokenAddress) public onlyOwner{
        initializeTokenAddress(_tokenAddress);
        initializeRemainingTokens(_tokenAddress);
    }

   //////////////////////// Set Token Address ////////////////////////////////
    function initializeTokenAddress (address  _tokenAddress)  internal {
        require (_tokenAddress != address(0), "NftaniaCrowdsale: token is the zero address");
        tokenAddress = _tokenAddress;  
    }

   //////////////////////// initialize remaining tokens //////////////////////////////// 
    function initializeRemainingTokens (address  _tokenAddress)  internal {
        remainingTokens = IERC20(_tokenAddress).allowance(owner(), address(this)); 
        emit RemainingTokens(remainingTokens/10**18);
    }
   ...
   //////////////////////// check if cap reached ////////////////////////////////
    function purchaseCapReached() public view returns (bool capReached) {
        uint256 walletAllowance;
        walletAllowance =  IERC20(tokenAddress).allowance(owner(), address(this));
        capReached = walletAllowance < minPurchase;
        return capReached;
    }

   //////////////////////// Check if crowdsale time ended //////////////////////////////////////// 
    function crowdsaleHasClosed() public view returns (bool _closed) {
        _closed = purchaseCapReached() || block.timestamp > endDate;
        return _closed;
    }
   ...
   //////////////////////// Get Wei Raised //////////////////////// 
    function getWeiRaised() public view returns ( uint256 _weiRaised) {
        return weiRaised;
    }
   ...
   //////////////////////// Purchase Tokens  /////////////  
    function buyTokens(uint amount) public payable whenNotPaused() nonReentrant() returns(bool success) {   
        ...
        require(remainingTokens >= minPurchase * 10**18, "Tokens are sold out");              
        require(amount<=maxPurchase,"The tokens amount for this order is larger than the max tokens amount allowed per order");                                   // Checks is remaining tokens are below minimum order quantity
        require(amount+walletPurchases[msg.sender]<=walletLimit,"the total tokens requested is more than max qouta per wallet"); // Checks "qouta amount" limit if allowed.
        require(amount* 10**18 <= remainingTokens, "Amount requested is lager than remaining tokens");                           // Checks "Purchase supply" availability.
        ...
        emit PurchaseOrder (msg.sender, amount, walletPurchases[msg.sender], purchaseOrderID, block.timestamp);
        emit TokensUpdate (weiRaised, totalTokensSold, remainingTokens/10**18);
        return true;
    }
    
   //////////////////////// Function update Remaining Tokens /////////////  
    function updateRemainingTokens() internal returns(uint256 _remainingTokens) {
        uint256 walletAllowance;
