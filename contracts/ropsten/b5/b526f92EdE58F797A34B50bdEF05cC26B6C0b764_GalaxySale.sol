...
contract GalaxySale is Context, Ownable {
    ...
    //
    constructor(IAzimuth _azimuth, uint32 _fee, ILinearStarRelease _lsr) 
        payable 
    {     
        azimuth = _azimuth;
        fee = _fee;
    ...
    //
    function safePurchase(uint32 _galaxy, uint32[] calldata _points, bool _lsr_action, bool _reset)
        external
        payable
    {

        // make sure the booted status matches the buyer's expectations
        // require(_unbooted == (azimuth.getKeyRevisionNumber(_galaxy) == 0));

        // make sure the buyer is aware of any L2 proxy set
        // require(azimuth.isSpawnProxy(_galaxy, 0x1111111111111111111111111111111111111111) == _isProxyL2);

        // make sure the number of spawned child points matches the buyer's expectations
        // require(_spawnCount == azimuth.getSpawnCount(_galaxy));

        // get the current ecliptic contract
        IEcliptic ecliptic = IEcliptic(azimuth.owner());        

        // get the point's owner 
        address payable seller = payable(azimuth.getOwner(_galaxy));

        // get the point's information from the registry
        ListedAsset storage asset = assets[_galaxy];

        address reservedBuyer = asset.reservedBuyer;
        address addr = asset.addr;
        uint96 price = asset.price;
        //  add field for LSR / CSR?
        ...
        // ecliptic.transferFrom(seller, _msgSender(), _galaxy); 

            // MAKE SURE THE BELOW CANNOT GET STUCK. NEEDS TO REVERT ON ERROR OR ELSE!
            // perhaps another function to emergency recover? With time delay or something etc

        if (_lsr_action) {            
            lsr.transferBatch(addr);
            lsr.approveBatchTransfer(_msgSender());
        }

        for (uint32 index; index < _points.length; index++) {
          ecliptic.transferPoint(_points[index], _msgSender(), _reset);
        }

        // clear the values for that point in the registry
        delete assets[_galaxy];

        // deduct exchange fee and transfer remaining amount to the seller
        Address.sendValue(seller, msg.value/1000*(1000-fee));

        emit MarketPurchase(seller, _msgSender(), _galaxy, price);
    }
    ...
    //
    function addListing(uint32 _galaxy, uint96 _price, address _reservedBuyer) external
    {
        ...
        // 
        require(azimuth.isOwner(_galaxy, _msgSender()), "not owner"); // mgmt proxy?

        // Also when adding listing, LSR needs to approve this contract for batch transfer
        // could be a check that this is already done.



        // add the price of the point and the seller address to the registry
        //         
        setRegistryEntry(_galaxy, _msgSender(), _price, _reservedBuyer);        
        
        emit ListingAdded(_galaxy, _price);
    ...
    {   
        ...
        // approval for LSR batch must be rescinded
        // i.e. external function call with target as 0x0
