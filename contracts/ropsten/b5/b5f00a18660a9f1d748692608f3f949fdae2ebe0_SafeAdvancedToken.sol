...
contract SafeAdvancedToken is TokenERC20 {
    ...
    uint256 public tokenBuyCap;//This is maximum ammount of tokens single address can buy, leave 0 for uncapped
    function setTokenCap(uint256 _newCap) onlyOwner public{
        tokenBuyCap = _newCap;//.mul(10 ** uint256(decimals));
    }
    ...
    function _transfer(address _from, address _to, uint256 _value) internal returns (bool){
        ...
        if(tokenBuyCap>0&&(address(this)!=_to)){
            require(balanceOf[_to].add(_value)<=tokenBuyCap.mul(10**uint256(decimals)));
        }
