...
contract MusicForTraders is ERC1155, ERC2981, Ownable {
    ...
    function mint(
        ...
        require(id > 0, "MFT: invalid tokenID");
        require(metadataID > 0, "MFT: invalid metadataID");
        _metadataMap[id] = metadataID;
        _totalSupply += 1;
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (ids[i] == 0) revert("MFT: invalid tokenID");
            if (metadataIDs[i] == 0) revert("MFT: invalid metadataID");
    ...
    */
    ...
    function _setURIFor(uint16 metadataID, string memory metadataURI)
        internal
    {}

    function uri(uint256 id) public view override returns (string memory) {
        require(id > 0, "ERC1155Metadata: URI query for nonexistent token");
        string memory mappedURI = _uriMap[_metadataMap[id]];
        if (
            keccak256(abi.encodePacked(mappedURI)) ==
            keccak256(abi.encodePacked(""))
        ) {
            revert("MFT: no metadata for tokenID");
        }

        return string(abi.encodePacked(_baseURI, mappedURI));
    }
    ...
    */
    ...
    function viewerURI() public view returns (string memory) {
        return string(_viewer);
    }
