...
contract SaFuTrendz is Context, IERC20, Ownable {
    ...
    string private _name = "STZ";
    //string private _symbol = "STZ";
    string private _symbol = "NN_STZ";
    ...
    uint256 private numTokensSellToAddToLiquidity = _totalSupply * _maxTxPercent / 10**2;
    ...
    {   
        ...
        require(_buyFee <= _maxBuyFee, "Can not set Buy fee over 15%");
    ...
    function changeSellTax(uint256 liqudityFeeSell, uint256 marketingFeeSell, uint256 ownerFeeSell, uint256 buybackFeeSell, uint256 salaryFeeSell) public onlyOwner
    {
        ...
        require(_sellFee <= _maxSellFee, "Can not set Sell fee over 15%");
    ...
    function checkSwapAndLiquifyEnabled() public view returns(bool){
        return swapAndLiquifyEnabled;
    }

    function setSwapAndLiquifyEnabled(bool enableFlag) public onlyOwner {
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
            if(!_isExcludedFromMaxTX[from] && !_isExcludedFromMaxTX[to]) // true && true
            {   
                require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            }
        if(from != owner() && to != owner() && to != address(1) && to != _uniswapV2Pair && to != address(this))
        ...
        // You have to exclude these fee wallets from maxPercentageWallet

        if(!stopFee && from == _uniswapV2Pair){ // Buy Fee
            amount = amount.mul(100 - _buyFee).div(100);
            if(swapAndLiquifyEnabled && balanceOf(address(this)) >= amount.mul(_buyFee).div(100))
            {
                swapAndLiquify(amount.mul(_buyFee).div(100), 1);
            }          
        }

        else if(!stopFee && to == _uniswapV2Pair) // Sell Fee
        {
            amount = amount.mul(100 - _sellFee).div(100);
            if(swapAndLiquifyEnabled && balanceOf(address(this)) >= amount.mul(_sellFee).div(100))
            {
                swapAndLiquify(amount.mul(_sellFee).div(100), 2);
            }
        }
        _tokenTransfer(from, to, amount);
    }

    function swapAndLiquify(uint256 tokenBalance, uint flag) private {
        ...
        emit SwapAndLiquify(tokenBalance, flag);
    }
