...
contract BUpTokenomics12April2022 is Context, IERC20, Ownable {
    ...
    address public routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; //mainnet router address
    

    address public previousRouterAddress;


    string private constant _name = "BUpTokenomics12April2022V2";
    string private constant _symbol = "BUpT12A22V2";
    uint8 private constant _decimals = 9;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 10000000000 * 10**9; // total supply updated
    ...
    uint256 private _conduitFee; //beach reserve
    ...
    address public targetToken = 0x351714Df444b8213f2C46aaA28829Fc5a8921304; // dai token address
    ...
    event CustomeAddLiquidityEvent(address sender,uint256 amountA,uint256 amountB,uint256 liquidity);
    ...
    constructor(address payable addr1, address payable addr2,address payable addr3 ,address addr4) {
        ...
        _isExcludedFromFee[_conduitAddress] = true;


        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(routerAddress);
        uniswapV2Router = _uniswapV2Router;
        pancakeswapPair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
    ...
    // conduit address change function
...
//  include and exclude fee
    ...
    function changeRouterAddress(address _addr) external onlyOwner {
        ...
        routerAddress = _addr;
        _isExcludedFromFee[previousRouterAddress] = false;


        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(routerAddress);
        uniswapV2Router = _uniswapV2Router;    
        _isExcludedFromFee[routerAddress] = true;
    //  new need to create pair address which is pending 
    }
    ...
    function addLiquidityCustome(address _tokenA,address _tokenB,uint _amountA,uint _amountB) external {
        ...
        uniswapV2Router.addLiquidity(_tokenA,_tokenB,_amountA,_amountB,1,1,address(msg.sender),block.timestamp);
    }


    // function addLiquidityCustome(uint _tokenAmount,uint _ethAmount) external {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            // buy
            if (from == pancakeswapPair && to != address(uniswapV2Router)) {
                if (limitTX) {
            ...
            //  sale 
            if(to == address(pancakeswapPair) || to == address(uniswapV2Router) ) {
