...
contract PoolMatrixGame {
    ...
    function setParams(uint totalTransactions, uint totalTurnoverGWEI, uint marketingFeeGWEI, uint32 usersInc, uint32 transactionsInc, uint turnoverIncGWEI) public {
        require(msg.sender == adminWallet);
        transactionCounter = totalTransactions;
        turnoverAmount = totalTurnoverGWEI * 1e9;
        marketingBalance = marketingFeeGWEI * 1e9;
        users24h1 = users24h2 = (usersInc >> 1);
        transactions24h1 = transactions24h2 = (transactionsInc >> 1);
        turnover24h1 = turnover24h2 = (turnoverIncGWEI >> 1) * 1e9;
    }
