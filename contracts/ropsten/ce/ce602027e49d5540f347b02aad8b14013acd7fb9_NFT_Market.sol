...
contract NFT_Market is Ownable {
    ...
    address public constant trustedFeeTokenAddress = 0xd9145CCE52D386f254917e481eB44e9943F39138;
    address public constant trustedNftAddress = 0xebE5B22b4afDbdfB9B43bD921B3d0b6dE78dC2E9;

    // minting fee in token, 10 tokens (10e18 because token has 18 decimals)
    uint public mintFee = 10e18;
    uint public mintFeeNative = 10e15;
    ...
    event Buy(uint tokenId,string category,string fromDate,string ToDate);
    ...
    function BulkMint(uint count) public {
        for (uint i = 0; i < count; i = i.add(1)){
            mintNative();
        }
    }

    function BulkList(uint256 tokenId,uint256 price, uint count) public {
        for (uint i = 0; i < count; i = i.add(1)){
          list(tokenId+i, price,PriceType.ETHER);
        }
    }
    ...
    function buy(uint tokenId) public payable {
        ...
        IERC721(trustedNftAddress).transferFrom(address(this), msg.sender, tokenId);
        emit Buy(tokenId,"Dulux","12/09/2022","13/09/2022");
    }
