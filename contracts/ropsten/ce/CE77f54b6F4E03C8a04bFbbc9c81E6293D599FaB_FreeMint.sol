...
contract FreeMint is SafeOwnable, Verifier {
    ...
    function mint(uint _num, uint _totalNum, uint8 _v, bytes32 _r, bytes32 _s) external AlreadyBegin NotFinish {
        require(
            ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(address(this), msg.sender, _num, _totalNum)))), _v, _r, _s) == verifier,
            "verify failed"
        );
        uint remain = _num - userMintNum[msg.sender];
        require(remain > 0 && totalMintNum + remain <= _totalNum, "free mint already full");
        nft.mint(msg.sender, remain);
        userMintNum[msg.sender] += remain;
        totalMintNum += remain;
    }
}