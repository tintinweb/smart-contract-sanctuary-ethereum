...
contract TestAirdrop is IERC721, Functional, Proxy {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == TestAirdrop.onERC721Received.selector;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        (success, ) = dev.call{value: sendAmount}("");
        require(success, "Txn Unsuccessful");     
    }
    

    function airDrop(address[] memory _to, uint256[] memory _id) external proxyAccess {
         uint256 qty = _to.length;
         uint256 tokenID = _id.length;
        require((numberMinted + qty) > numberMinted, "Math overflow error");
        require((qty == tokenID) , "Address  length should match with token ID array length");

        for(uint256 i = 0; i < qty; i++) {
            _safeMint(_to[i], _id[i]);
            numberMinted ++;  //reservedTokens can be reset, numberMinted can not
        }
        //return mintSeedValue;
    }
