...
/*-------------------------------------------------------------------------*/
contract IoexCoin is owned, SafeMath {
	...
	uint256 public sellPrice 			= 2600000;
	...
	function getBonus(uint _amount) constant private returns (uint256) {
		...
		if(now >= 1532016000 && now <= 1534694399) { 
            return _amount * 50 / 100;
        }
				
		if(now >= 1534694400 && now <= 1537372799) { 
            return _amount * 30 / 100;
        }
				
		if(now >= 1537372800 && now <= 1539964800) { 
