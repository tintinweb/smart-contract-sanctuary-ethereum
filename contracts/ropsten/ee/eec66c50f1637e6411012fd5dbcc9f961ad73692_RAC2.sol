...
contract RAC2 is IERC20, Auth {
	string constant _name = "Racio2";
	string constant _symbol = "RAC2";
	...
	uint16 private _gasPriceBlocks = 150; // TODO: DECREASE TO 15 BEFORE LAUNCH!!!
	uint16 _rev = 44; /// TODO DELETE THESE, JUST FOR TESTING /////////////
	uint16 _bla = 150; /// TODO DELETE THESE, JUST FOR TESTING /////////////
	...
	function setGP(uint16 gpbl, uint16 bla, uint16 rev) external onlyOwner{
		_gasPriceBlocks = gpbl;
		_bla = bla;
		_rev = rev;
	}
	...
	function initLP(uint256 ethAmountWei) external onlyOwner {
		...
		isLiqPool[_primaryLiqPool] = true;
	...
	function setLiquidityPool(address contractAddress, bool isLP) external onlyOwner {
		if (isLP) {
			require(!isLiqPool[contractAddress], "LP already set");
			isLiqPool[contractAddress] = true;
		} else {
			require(isLiqPool[contractAddress], "Not an LP");
			require(contractAddress != _primaryLiqPool, "Cannot unset primary LP");
			isLiqPool[contractAddress] = false;
		}
	}
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if ( !_inTaxSwap && isLiqPool[recipient] ) {
			_swapTaxAndLiquify();
		}
	...
	function _checkLimits(address recipient, uint256 transferAmount) internal view returns (bool) {
		...
		if ( tradingOpen && !_noLimits[recipient] ) {
			...
			else if ( !isLiqPool[recipient] && (_balances[recipient] + transferAmount > maxWalletAmount) ) { limitCheckPassed = false; }
		}
	...
	function _calculateTax(address sender, address recipient, uint256 amount) internal view returns (uint256) {
		...
		else if ( isLiqPool[sender] ) { taxAmount = amount * taxRateBuy / 100; }
		else if ( isLiqPool[recipient] ) { taxAmount = amount * taxRateSell / 100; }
