...
contract HashtagNFTCollectiveNFTVault is
    ERC721HolderUpgradeable,
    IHashtagNFTCollectiveNFTVault
{
    ...
    mapping(uint256 => mapping(address => bool)) private tokenIdMapped;

    mapping(address => NftDetail[]) private nftDetailMap;

    uint256 public assetCount;

    bool private multiTokenManager;

    uint256 public multiTokenItemId;

    struct NftDetail {
        address token;
        uint256 tokenId;
        uint256 timeOfTransfer;
    }
    ...
    function initialize(
        ...
        if (_token != address(0x0))
            _storeNftDetail(_token, _tokenId);
        else {
            multiTokenManager = true;
            multiTokenItemId = _tokenId;
        }
    ...
    function storeNftDetail(address _from, uint256 _tokenId)
        external
        override
        onlyProxyFactory
    {
        _storeNftDetail(_from, _tokenId);
    }

    function _storeNftDetail(address _from, uint256 _tokenId) private {
        require(tokenIdMapped[_tokenId][_from] == false);

        if (!tokenExists[_from]) {
            tokens.push(_from);
            tokenExists[_from] = true;
        }

        nftDetailMap[_from].push(NftDetail(_from, _tokenId, block.timestamp));
        tokenIdMapped[_tokenId][_from] = true;
    }

    function setAssetCount(uint256 _count)
        external
        override
        onlyProxyFactory
        returns (uint256)
    ...
    function onERC721Received(
        ...
        virtual
        override(ERC721HolderUpgradeable)
        returns (bytes4)
    {
        assetCount += 1;
    ...
    function afterBuyout(address _curator) external {
        for (uint256 i = 0; i < tokens.length; i++)
            IHashtagNFTCollectiveERC721(tokens[i]).setRightsOwner(_curator);
    }
    ...
    function upgradeRightsOwnerToManager() external onlyCurator {
        for (uint256 i = 0; i < tokens.length; i++)
            IHashtagNFTCollectiveERC721(tokens[i]).upgradeRightsOwnerToManager();
    }

    function unWrapToken(uint256 tokenId) external onlyCurator {
        for (uint256 i = 0; i < tokens.length; i++)
            IHashtagNFTCollectiveERC721(tokens[i]).unWrapToken(tokenId);
    }

    function setRoyalties(uint256[] memory tokenIds, uint256[] memory royalties) external onlyCurator {
        for (uint256 i = 0; i < tokens.length; i++)
           IHashtagNFTCollectiveERC721(tokens[i]).setRoyalties(tokenIds, royalties);
    }
}
