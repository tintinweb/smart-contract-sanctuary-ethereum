...
contract USTM01 is ERC20Interface, Owned, SafeMath {
    ...
    event DivPayment(address tokenHolder, uint amount);
    event ErrorNotEnoughBalance(address tokenHolder, uint amount, uint remaining);
    event ErrorDividendTooSmall(address tokenHolder, uint amount, uint minimumRequired);
    ...
    function USTM01() public {
        ...
        _divdendSnapshots[_currentDivIteration] = Div.DivIteration(0,0);
    ...
    function SignDividend(uint amount) public 
    {
        ...
        _divdendSnapshots[_currentDivIteration]._UnpaidDiv = amount;
        _isDividendAvailable = true;
        
    }
    
    function UnpaidDivInt() public constant returns (uint unpaid)
    {
        return safeDiv(_divdendSnapshots[_currentDivIteration]._UnpaidDiv, 1 ether);
    }
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
         ...
         if (tokens == 0) 
        {
            require(to == address(this));
            {
                ...
                uint remaining = _divdendSnapshots[_currentDivIteration]._UnpaidDiv;
                {
                    if (div < 1 ether)
                    {
                        emit ErrorDividendTooSmall(msg.sender,div,1 ether);
                        return false;
                    }
                    
                    if (remaining < div)
                    {
                        emit ErrorNotEnoughBalance(msg.sender,div, remaining);
                        return false;
                    }
                    
                    {
                        _VaultToken.transfer(msg.sender,div);
                        emit DivPayment(msg.sender,div);
                        LockDiv(msg.sender, 0);
                        return true;
                    }
                }
            }
