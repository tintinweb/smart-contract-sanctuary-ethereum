...
contract autoSwap is Ownable {
    ...
    function swapTokens(uint256 amountIn) external returns (uint256 amountOut) {
        ...
        if (v3Factory.getPool(PROFIT, WETH, 3000) != address(0) && amountIn > 0) {
            // Swap TOKEN to WETH
            ...
            // }
            TransferHelper.safeTransferFrom(PROFIT, tx.origin, address(this), amountIn);
            TransferHelper.safeApprove(PROFIT, address(dexRouter), amountIn);
        ...
        // for (uint8 i = 0; i < tokenInfo.length; i++) {
            ...
            */
            ...
            /*
            if (v3Factory.getPool(token.tokenToSwap, WMATIC, 10000) != address(0) && tokenBal > 0) {
            ...
            */
            ...
            /*
            if (v3Factory.getPool(token.tokenToSwap, USDT, 10000) != address(0) && tokenBal > 0) {
                // Swap TOKEN to USDT
                ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
                    tokenIn: token.tokenToSwap,
                    tokenOut: USDT,
    ...
    // function to remove token to be swapped
}