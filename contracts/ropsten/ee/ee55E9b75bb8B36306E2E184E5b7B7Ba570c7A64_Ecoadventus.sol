...
import "../vault/IVaultAdmin.sol";

import "hardhat/console.sol";

contract Ecoadventus is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, ERC20SnapshotUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ERC20PermitUpgradeable, UUPSUpgradeable,OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  uint256 private constant BASIS_POINTS_MULTIPLIER = 10000;
    ...
    // Initial basis points for transfer fee (1 = 0.1%). Defaults to 1%
  ...
  event RemoveFromWhitelist(address indexed _removedAddress);
    ...
    function initialize(string memory name_, string memory symbol_, uint256 initialSupply_, address vaultAddress_) external initializer {
        ...
        _transferFeeBasisPoints = 10;
        whitelistEnabled = true;
    }

    modifier onlyVaultOrMinter(address caller) {
        require(
            IVaultAdmin(_vaultAddress).isVault(caller) || hasRole(MINTER_ROLE, caller),
            "Caller is not the Minter or Vault"
        );
        _;
    }
    ...
    function mint(address _accountTo, uint256 _amount) public whenNotPaused nonReentrant onlyVaultOrMinter(_msgSender()) {
        require(isWhitelisted(_accountTo), "Receiver is not whitelisted");
        require(_accountTo != address(0), "Mint to the zero address");
        super._mint(_accountTo, _amount);
    }
    ...
    function transfer(address _to, uint256 _value)
        ...
        returns (bool)
    {
        uint256 transferFee = calcTransferFee(_msgSender(), _value);
        _executeTransfer(_msgSender(), _to, _value.add(transferFee));
        uint256 amountToTransfer = _value.add(transferFee);
        console.log(amountToTransfer, transferFee);

        require((address(IVaultAdmin(_vaultAddress).getFeeAddress()) != address(0)), "Fee Transfer address is zero");
        require(isWhitelisted(address(IVaultAdmin(_vaultAddress).getFeeAddress())), "Transfers disabled");

        if (transferFee >0) {
            super.transfer(payable(IVaultAdmin(_vaultAddress).getFeeAddress()), transferFee);
            super.transfer(_to, amountToTransfer.sub(transferFee));
        } else {
            super.transfer(_to, amountToTransfer);
        }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal whenNotPaused override(ERC20Upgradeable, ERC20SnapshotUpgradeable) {
        require(isWhitelisted(from), "Transfers disabled");
        require(isWhitelisted(to), "Transfers disabled");
        super._beforeTokenTransfer(from, to, amount);
    }

    function _executeTransfer(address from, address to, uint256 value) internal {
        _transferRestrictions(to, from, value);
  ...
  */
  function _transferRestrictions(address to, address from, uint256 value) internal view {
  ...
  */
  ...
  /**
   ...
   /**
     ...
     */
    function enableWhitelist(bool _enabled) external onlyRole(MINTER_ROLE) {
     ...
     * @param _newAddress address to be added to the whitelist
     */
    function addToWhitelist(address _newAddress) external onlyRole(MINTER_ROLE) {
...
interface IVaultAdmin {
    function setFeeAddress(address newFeeAddress) external returns (bool);
    function getFeeAddress() external view returns (address);
    function isVault(address caller) external view returns (bool);
}
