...
contract Ownable {
  ...
  modifier onlyCXO() {
    require(msg.sender == cto || msg.sender == cfo || msg.sender == cfo);
    _;
  }
  ...
  function transferCEO(address newCEO) public {
    ...
    // 如果原来的CEO没有设置 则sender必须是owner 否则必须是原来的CEO
    if(ceo == address(0)) {
        require(msg.sender == owner);
    }else{
        require(msg.sender == ceo);
  ...
  function transferCFO(address newCFO) public {
    ...
    emit OwnershipTransferred(cfo, newCFO);
    cfo = newCFO;
  }

  function transferCTO(address newCTO) public {
    require(newCTO != address(0));

    // 如果原来的CTO没有设置 则sender必须是owner 否则必须是原来的CTO
    if(cto == address(0)) {
        require(msg.sender == owner);
    }else{
        require(msg.sender == cto);
    }
    emit OwnershipTransferred(cto, newCTO);
    cto = newCTO;
  }

  function whoisCEO() public view returns (address){
      return ceo;
  }
  function whoisCTO() public view returns (address){
      return cto;
  }
  function whoisCFO() public view returns (address){
      return cfo;
  }
...
contract ECRC {
  ...
  //验签数据入口函数
  function checkSign(bytes32 hashValue, string signValue, address addr) internal returns (bool){
    bytes memory signedString = hexStr2bytes(signValue);
    bytes32  r = bytesToBytes32(slice(signedString, 0, 32));
    bytes32  s = bytesToBytes32(slice(signedString, 32, 32));
    byte  v = slice(signedString, 64, 1)[0];
    return ecrecoverDecode(hashValue, r, s, v)==addr;
  }
...
contract MultiSigAcount is Ownable, ECRC{
    ERC20 private  _erc20;
    mapping(address=>bool) private canSenders;
    mapping(string=>bool) private isExistTx;
    uint256 private minSenderNum;
    bool private isPause;
    using strings for *;
    //event CheckTransferFail(string _txid, uint256 _value, address _from, address _to);
    //event SuccessTransfer(string _txid, uint256 _value, address _from, address _to);
    event TransferError(string _txid, uint256 _errno, address _to, uint256 _value);
    /*
    errno:
        200: 转账成功
    ...
    */
    ...
    constructor(address _erc20addr) public{
        _erc20 = ERC20(_erc20addr);
        minSenderNum = 1;
        isPause = false;
    }
    
    function addSender(address _addr) onlyCFO() public{
        canSenders[_addr] = true;
    }
    
    function removeSender(address _addr) onlyCFO() public{
        delete canSenders[_addr];
    }
    
    function setMinSenderNum(uint256 _num) onlyCFO() public {
        require(_num>0);
        minSenderNum = _num;
    }
    
    function pauseTransfer() onlyCXO() public {
        isPause = true;
    }
    
    function startTransfer()  onlyCTO() public {
        isPause = false;
    }
    ...
    function isSender(address _addr) public view returns (bool) {
        return canSenders[_addr];
    }    
    ...
    modifier onlyLtSenderNum(uint256 _num) {
        require(_num>=minSenderNum);
        _;
    }
