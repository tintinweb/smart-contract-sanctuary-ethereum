...
interface ISPPayer {
  ...
  function claimReward(uint256 _projectId, address _token) external;
}
...
contract SPPayer is ISPPayer {
  ...
  function pay(uint256 _projectId, address _token, uint256 _tokenPrice, uint256 _amount, bool _shouldClaim) external override {
    require(!ISPProjects(projects).crowdfundingEndState(_projectId));

    uint256 mintTokenAmount = _amount / _tokenPrice;

    paymentsOfHolders[msg.sender][_projectId] += _amount;
    payments[_projectId][address(_token)] += _amount;

    IERC20(_token).transferFrom(msg.sender, address(this), _amount * 10 ** decimals);
    
    //Add invested rate
    investedRate[msg.sender][_projectId] = paymentsOfHolders[msg.sender][_projectId] * 10 ** decimals / ISPProjects(projects).crowdfundingAmount(_projectId);
    
    // Set the last claimed date as last paid date
    lastClaimedDateBy[msg.sender][_projectId] = block.timestamp;

    store.mintFor(msg.sender, _projectId, mintTokenAmount * 10 ** decimals, _shouldClaim);
  }
  ...
  function claimReward(uint256 _projectId, address _token) external override {
    ...
    IERC20(address(_token)).transfer(msg.sender, investedRate[msg.sender][_projectId] * ISPProjects(projects).crowdfundingAmount(_projectId));
    lastClaimedDateBy[msg.sender][_projectId] = block.timestamp;
  }

  function refund(uint256 _projectId, address _tokenForRefund) public {
    ISPTokenStore tokenStore = ISPTokenStore(store);
    ISPToken _token = tokenStore.tokenOf(_projectId);
    uint256 refundAmount = paymentsOfHolders[msg.sender][_projectId] * 10 ** decimals;
    uint256 projectTokenAmount = _token.balanceOf(msg.sender, _projectId);
    _token.transferFrom(_projectId, msg.sender, address(this), projectTokenAmount);
    IERC20(_tokenForRefund).transfer(msg.sender, refundAmount);
  }

  // function refund(uint256 _projectId, address _tokenForRefund, uint256 _amount) public {
  //   ISPToken _token = ISPTokenStore(store).tokenOf(_projectId);
  //   uint256 refundAmount = paymentsOfHolders[msg.sender][_projectId] * 10 ** decimals;
  //   _token.transferFrom(_projectId, msg.sender, address(this), refundAmount);
  //   IERC20(_tokenForRefund).transfer(msg.sender, refundAmount);
  // }
}