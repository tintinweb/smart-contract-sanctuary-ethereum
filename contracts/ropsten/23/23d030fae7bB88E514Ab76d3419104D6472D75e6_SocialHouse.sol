...
contract SocialHouse is ERC1155, ReentrancyGuard, Ownable {
    ...
    uint16[5] public supplyCaps = [4000, 2200, 1100, 400, 77];
    ...
    function uri(uint256 typeId) public view override returns (string memory) {
        require( typeId < 5, "LL420GreenPass: ID isn't in the range" );
        require(bytes(baseURI).length > 0, "LL420GreenPass: base URI is not set");

        return string(abi.encodePacked(baseURI, uint8(typeId), ext));
    }
    ...
    function mint() public onlyStarted payable {
    ...
    function supply() public onlyStarted view returns (uint16[] memory) {
        uint16[] memory suppliesRes = new uint16[](5);
        for(uint8 i = 0; i < 5; i++) {
            suppliesRes[i] = supplies[i];
        }
        return suppliesRes;
    }
    ...
    function random(uint8 maxValue) public view returns (uint) {
