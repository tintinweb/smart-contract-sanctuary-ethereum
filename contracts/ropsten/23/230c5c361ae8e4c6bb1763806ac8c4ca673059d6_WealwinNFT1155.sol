    ...
    ERC1155BurnableUpgradeable
{
    ...
    uint256 private serviceValueEsc;
    ...
    mapping(address => bool) private whiteListuserArray;

    function initialize() public initializer {
        ERC1155Upgradeable.__ERC1155_init("", "WealwinNFT", "WealwinNFT");
        __Ownable_init();
        _tid = 1;
        serviceValue = 2500000000000000000;
        serviceValueEsc = 2500000000000000000;
    ...
    function getServiceFeeESC() public view returns (uint256) {
        return serviceValueEsc;
    }

    function serviceFunctionESC(uint256 _serviceValueEsc) public onlyOwner {
        serviceValueEsc = _serviceValueEsc;
    }
    ...
    function setTokenLimit(uint256 _amount) public onlyOwner {
        minimumTokenLimit = _amount;
    }

    function setDiscountPercentage(uint256 _discount) public onlyOwner {
    ...
    function mint(
        ...
        if (whitelistedUser) {
            require(
                whiteListuserArray[msg.sender] == true,
                "User is not White Listed"
            );
        } else {
            whiteListuserArray[msg.sender] = true;
        }
    ...
    function enableWuser(bool approved) public returns (bool) {
        require(whitelistAdmin == msg.sender, "User Not Authorized");
        whitelistedUser = approved;
        return approved;
    }
    ...
    function addWuser(address useraddress) public returns (bool) {
        require(whitelistAdmin == msg.sender, "User Not Authorized");
        whiteListuserArray[useraddress] = true;
        return true;
    }

    function removeWuser(address useraddress) public returns (bool) {
        require(whitelistAdmin == msg.sender, "User Not Authorized");
        whiteListuserArray[useraddress] = false;
        return true;
    }

    function getWUser(address userAddress) external view returns (bool) {
        return whiteListuserArray[userAddress];
    }
    ...
    function setWhiteListAdmin(address userAdd) public onlyOwner {
        whitelistAdmin = userAdd;
    }
    ...
    function saleWithToken(
        ...
        emit Buy(owner(), msg.sender, tokenId, amount, nooftoken);
    }
    ...
    function saleToken(
        ...
        _saleToken(from, tokenId, amount, "ETH");
        _setApprovalForAll(from, msg.sender, true);
        saleTokenTransfer(from, tokenId, nooftoken);
    }
    ...
    function _saleToken(
        ...
        if (keccak256(bytes(bidtoken)) == keccak256(bytes("WIN"))) {
            serviceVal = serviceValueEsc;
        }
        ...
        if (
            ...
            uint256 buyerBalance = t.balanceOf(msg.sender);

            cc  = buyerBalance;
            if (buyerBalance >= minimumTokenLimit) {
                uint256 disc = pERCent(amount, discountPercentage);
                cc1 = disc;
                amount = amount.sub(disc);
            }

            uint256 val = pERCent(amount, serviceVal).add(amount);
            require(
                approveValue >= val.div(10**Tokendecimals) && val > 0,
                "Insufficient Balance"
            );
            (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(
                amount,
            ...
            if (_adminfee != 0) {
                t.transferFrom(
                    msg.sender,
                    owner(),
                    _adminfee.div(10**Tokendecimals)
                );
            }
            if (_royal[tokenId] != 0) {
                t.transferFrom(
            ...
            if (netamount != 0) {
                t.transferFrom(
                    msg.sender,
                    from,
                    netamount.div(10**Tokendecimals)
                );
            }
        }
    }
    ...
    function acceptBId(
        ...
        emit Acceptbid(msg.sender, bidaddr, tokenId, amount, NOFToken);
    }
    ...
    function _acceptBId(
        ...
        uint256 serviceVal = serviceValue;
        if (keccak256(bytes(tokenAss)) == keccak256(bytes("WIN"))) {
            serviceVal = serviceValueEsc;
        }
    ...
    function orderPlace(uint256 tokenId, uint256 _price) public {
        require(
            _balances[tokenId][msg.sender] > 0 && _price > 0,
            "Is Not a Owner"
        );
        Order memory order;
