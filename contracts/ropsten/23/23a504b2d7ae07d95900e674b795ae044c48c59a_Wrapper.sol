...
contract Wrapper is Utils {
    ...
    function test_third(KyberNetwork network, ERC20 token) public view returns(uint) {
        uint count = 0;
        while (true) {
            if (network.reservesPerTokenDest(token, count) == address(0)) { break; }
            count++;
        }
        return count;
    }

    function test_fourth(KyberNetwork network, ERC20 token) public view returns(uint) {
        uint count = 0;
        while (true) {
            if (network.reservesPerTokenSrc(token, count) == address(0)) { break; }
            count++;
        }
        return count;
    }
    ...
    function getListReserves(KyberNetwork network, ERC20 token) public view returns(address[]) {
        ...
        // count = 0;
        // for(i = 0; i < numReserves; i++) {
        //     data = network.reservesPerTokenDest(token, i);
        //     if (data == address(0)) { break; }
        //     else { count++; }
        // }
        ...
        // }
        // return (numReserves, network.reservesPerTokenSrc(token, 0), network.reservesPerTokenDest(token, 0), src, dest);
        return src;
    }
}