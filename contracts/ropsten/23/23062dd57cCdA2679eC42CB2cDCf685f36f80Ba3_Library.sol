...
contract Library is Ownable {
    event BooksAdded(BookAdd[] _books);
    ...
    event BookRentPriceChanged(uint256 _newPrice);
    event LibTokensWithdrawn(uint256 _amount, address _admin);
    ...
    uint256 public bookPriceLibWei = 1 * 10**17;
    LibraryToken public libToken;
    LIBWrapper public libWrapper;
    ...
    constructor(address libTokenAddress, address payable libWrapperAddress) {
        require(libTokenAddress != address(0), "Invalid Library Token Address");
        require(libWrapperAddress != address(0), "Invalid LIB Wrapper Address");

        libToken = LibraryToken(libTokenAddress);
        libWrapper = LIBWrapper(libWrapperAddress);
    }

    function setBookPriceInLibWei(uint256 price) external onlyOwner {
        require(price > 0, "Price cannot be negative");
        bookPriceLibWei = price;

        emit BookRentPriceChanged(price);
    }

    function addBooks(BookAdd[] calldata books) external onlyOwner {
    ...
    function borrowBooks(uint256[] calldata bookIds) external {
        ...
        libToken.transferFrom(
            msg.sender,
            address(this),
            bookIds.length * bookPriceLibWei
        );
    ...
    // Using Pagination pattern for listing items, avoiding large queries
    function getAvailableBooks(uint256 startFrom, uint256 count)
        public
        view
        inQueryLimit(count)
        returns (BookView[] memory, uint256 total)
    {
    ...
    function getActiveBorrowersForBookId(
        uint256 bookId,
        uint256 startFrom,
        uint256 count
    ) public view inQueryLimit(count) returns (address[] memory) {
        ...
        for (uint256 i = 0; i < length; i++) {
            if (
                userToBorrowedBookId[bookBorrowers[startFrom + i]][bookId]
                    .isCurrentlyBorrowed
            ) {
                result[counter++] = bookBorrowers[startFrom + i];
            }
        }
    ...
    function booksBorrowedByUser(uint256[] calldata bookIds)
        public
        view
        returns (bool)
    {
        require(bookIds.length < MAX_ITEMS_PER_REQUEST, "Too many book ids");
        for (uint256 i = 0; i < bookIds.length; i++) {
            if (
                !userToBorrowedBookId[msg.sender][bookIds[i]]
                    .isCurrentlyBorrowed
    ...
    function withdrawLibTokens() external onlyOwner {
        uint256 amount = libToken.balanceOf(address(this));
        require(amount > 0, "Nothing to withdraw !");

        // Marto, is this the way to do the below (i.e. for the contract to approve the tokens spending) ?
        ...
        libToken.approve(address(libWrapper), amount);
        libWrapper.unwrap(amount);
        (bool sent, ) = payable(msg.sender).call{value: amount}("");
        require(sent, "Could not swap LIB and send ETH");

        emit LibTokensWithdrawn(amount, owner());
    }

    receive() external payable {}

    // If larger length is given, max possible is returned.
    // arr [1,2,3,4,5]   5
...
contract LIBWrapper {
    event LogLibWrapped(address _sender, uint256 _value);
    event LogLibUnwrapped(address _sender, uint256 _value);

    LibraryToken public libToken;

    constructor(address libTokenAddress) {
        require(
            libTokenAddress != address(0),
            "invalid Library token address !"
        );
        libToken = LibraryToken(libTokenAddress);
    }
    ...
    function unwrap(uint256 value) public payable {
        require(value > 0, "LIB should be > 0");
