...
contract BridgeAssistBSC {
     ...
     * @param _feeAddr BSC wallet address to send fee
     */
    constructor(IERC20 _erc20, address _backend, address _feeAddr) {
        erc20 = _erc20;
        backend = _backend;
        feeAddress = _feeAddr;
    }
     ...
     * @dev Tokens with the exception of the fee are transferred to the user
    ...
    function dispense(address _account, uint256 _amount) public onlyBackend {
        ...
        erc20.transfer(_account, _amount - fee);
        erc20.transfer(feeAddress, fee);
        emit Dispense(_account, _amount - fee);
    }

    /**
     * @notice Backend function to clear user lock in the swap tokens process
     * @param _account BSC wallet address
     */
    ...
    /**
     * @notice Calculation of the fee for swapping tokens
     * @param _amount Number of tokens to dispense from the bridge
     * @return fee Fee amount
     */
    function _calculateFee(uint256 _amount) internal view returns (uint256 fee) {
        fee = (_amount * feeNumerator) / feeDenominator;
    }
