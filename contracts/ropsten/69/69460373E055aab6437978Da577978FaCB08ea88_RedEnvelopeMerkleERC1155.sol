...
contract RedEnvelopeMerkleERC1155 is IEnvelope, ERC1155Holder {
    using Bits for uint8;

    struct MerkleERC1155Envelope {
    ...
    mapping(string => MerkleERC1155Envelope) private idToEnvelopes;

    function returnEnvelope(string calldata envelopeID) public nonReentrant {
        MerkleERC1155Envelope storage env = idToEnvelopes[envelopeID];
        require(env.numTokens > 0, "numTokens should be larger than zero");
        require(
            env.creator == msg.sender,
            "We will only return to the creator!"
        );
        IERC1155 token = IERC1155(env.tokenAddr);
        address receiver = payable(env.creator);
        uint256 tokenID = env.tokenID;
        uint256 oldnumTokens = env.numTokens;
        delete idToEnvelopes[envelopeID];
        bytes memory empty;
        token.safeTransferFrom(address(this), receiver, tokenID, oldnumTokens, empty);
    }
    ...
    function addEnvelope(
        ...
        require(idToEnvelopes[envelopeID].numTokens == 0, "numTokens not zero");
        require(value > 0, "Trying to create zero numTokens envelope");

        // First try to transfer the ERC1155 token
        IERC1155 token = IERC1155(tokenAddr);
        token.safeTransferFrom(msg.sender, address(this), tokenID, value, "");

        MerkleERC1155Envelope storage envelope = idToEnvelopes[envelopeID];
        ...
        envelope.tokenAddr = IERC1155(tokenAddr);
        envelope.tokenID = tokenID;
        envelope.numTokens = value;
    }
    ...
    function openEnvelope(
        ...
        currentEnv.numTokens -= 1;
        IERC1155 token = IERC1155(currentEnv.tokenAddr);
        token.safeTransferFrom(address(this), msg.sender, currentEnv.tokenID, 1, "");
    }
}
...
function _burnStr(string[] storage array, uint32 index) {
    require(index < array.length);
    array[index] = array[array.length - 1];
    array.pop();
}
...
contract IEnvelope is Ownable, ReentrancyGuard {
    ...
    function getRand(address receiver) internal virtual returns (uint16) {
