...
abstract contract ERC20 is IERC20, Auth {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        require(uint256(balanceOf(sender)) >= uint256(amount), "ERC20: Balance not great enough");        
        if(address(sender) != msg.sender){
            require(_allowances[sender][msg.sender] >= amount, "Request exceeds sender token allowance.");
        }
        uint256 mFee = (amount * mp) / bp;
        amount -= mFee;
        IERC20(address(this)).transfer(_marketingWallet, mFee);
