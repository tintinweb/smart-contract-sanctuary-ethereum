...
contract Dapplication is Ownable {
    ...
    mapping(address => uint) public usersDaiBalance;
    // mapping(address => uint) public faultyBalance;
    ...
    function sendApplication(uint _amount) public {
        ...
        // Important, Increasing the allowance
        ...
        usersDaiBalance[msg.sender] += _amount;
    ...
    function acceptApplication(uint _applicationNo, address _withdrawTo) external onlyOwner {
        uint balance = usersDaiBalance[msg.sender];

        // balance should be > 0
        require (balance > price, "staking balance cannot be less then the price");

        // Transfer Mock Dai tokens to this contract
        daiToken.transfer(_withdrawTo, balance);

        // reset staking balance to 0
        usersDaiBalance[msg.sender] = 0;
    ...
    //     faultyBalance[msg.sender] += msg.value;
    //     emit TransferReceived(msg.sender, msg.value);
    // }

    // Function to transfer ERC20 from the contract to a wallet
    function withdrawDAI(address _to, uint _amount)public{
        uint _daiBalance = daiToken.balanceOf(address(this));
        require(_amount <= _daiBalance,"Not enough amount");
