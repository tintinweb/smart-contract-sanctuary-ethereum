...
contract Moonshot is Context, IERC20, Ownable {
  ...
  /*  function setBuyBackMinAmount(uint256 amountMin) external onlyOwner() {
        require( amountMin > MIN_BUY_AMOUNT, "Amount must be greater than MIN_BUY_AMOUNT");
        require( amountMin <= _buyBackMaxTokenAmount, "Amount must be smaller than buyBackMaxAmount");
    ...
    function setBuyBackMaxAmount(uint256 amountMax) external onlyOwner() {
        require( amountMax > MIN_BUY_AMOUNT, "Amount be greater than MIN_BUY_AMOUNT");
        require( amountMax <= MAX_BUY_AMOUNT, "Amount must be smaller than MAX_BUY_AMOUNT");
    ...
    function setBuyBackCooldownInterval(uint256 value) external onlyOwner() {

        _buyBackCooldownInterval = value;
        _buyBackCooldownTimestamp  = block.timestamp;

        emit SetBuyBackCooldownInterval(_buyBackCooldownInterval, _buyBackCooldownTimestamp);
    }

    function setBuyBackSize(uint256 percentage) external onlyOwner() {
        require( percentage > 0, "Must be greater than 0");
        require( percentage <= (10 ** 4) , "Must be smaller than 10,000");
        _buyBackSize = percentage;

        emit SetBuyBackSize(percentage);
    }
    */
