...
contract InfinityMintApi is InfinityMintObject {
	...
	function allKazooPaged(
		...
		uint64 currentTokenId = totalMints();
		...
		if (currentTokenId != 0) {
			...
			while (kazooId < currentTokenId && count < pageMax) {
				if (
		...
		returns (uint64[] memory kazoos)
	{
		...
		if (mainContract.balanceOf(owner) > 2500)
	...
	function getRaw(uint64 kazooId) public view returns (bytes memory) {
		...
		ReturnObject memory data = storageContract.get(kazooId);
	...
	function getWalletContract(uint64 kazooId)
		public
		view
		returns (address result)
	{
		return storageContract.get(kazooId).wallet;
	}

	function getStickerContract(uint64 kazooId)
		public
		view
		returns (address result)
	{
