...
contract Inferno is ERC721, ERC721URIStorage, Pausable, AccessControl {
    ...
    uint256 private _price = 25000000000000000; // 0.025 Ether
    uint256 private _maxSupply = 120;
    uint256 private _maxZodiacSupply = 10;

    bool private _isSalePublic = false;

    string private base_uri = "";

    constructor() ERC721("Poopie", "POO") {
        _pause();
    ...
    function addWhitelists(address[] calldata addresses) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for(uint i = 0; i < addresses.length; i++) {
            _grantRole(MINTER_ROLE, addresses[i]);
        }
    }

    function mintKey(address to, string calldata sign) public payable {
        if(paused()) {
            revert(
                string(
                    abi.encodePacked(
        ...
        if(!hasRole(MINTER_ROLE, to) && !_isSalePublic) {
            revert(
                string(
                    abi.encodePacked(
                        "AccessControl: account ",
