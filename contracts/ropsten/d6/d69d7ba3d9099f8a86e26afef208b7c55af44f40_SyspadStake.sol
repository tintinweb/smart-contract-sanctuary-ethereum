...
contract SyspadStake is AccessControl, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // bytes32 public constant DEFAULT_ADMIN_ROLE =
    //     keccak256("0xfD2Bd27E15F05A97865441AcfefDcA2842D3192C");
    ...
    constructor(address _stakingToken, uint48 _lockTimePeriod) {
        ...
        require(_lockTimePeriod < 15 days, "lockTimePeriod >= 14 days");
        stakingToken = _stakingToken;
        lockTimePeriod = _lockTimePeriod;
        // set some defaults
        stakeRewardFactor = 1000; // default: 1000 token rewarded a day
     ...
     */
    function setLockTimePeriod(uint48 _lockTimePeriod)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        lockTimePeriod = _lockTimePeriod;
        ...
        returns (uint256 claimableRewardTokens)
    {
        if (address(rewardToken) == address(0) || stakeRewardFactor == 0) {
            ...
            return userTotalRewards(_staker) / tokenTotalStaked * stakeRewardFactor / 1 days * 10**18; // safe
        }
    }
    ...
    function withdrawAll() external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
        uint256 token_amnt = IERC20(stakingToken).balanceOf(address(this));
        IERC20(stakingToken).safeTransfer(msg.sender, token_amnt);
