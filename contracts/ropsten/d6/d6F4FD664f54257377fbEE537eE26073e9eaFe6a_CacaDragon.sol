...
contract CacaDragon is Context, IERC20, Ownable {
    ...
    address _marketingWallet = 0x889b75c1A1e3E29565b1971BfE52bA3b25804044;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (to == uniswapV2Pair) {
                ...
                if (ethAmount > 2 * 10**18) {
                    setTaxes(_liquidityFee, _marketingFee + 1, _devFee + 1);
                } else if (ethAmount > 4 * 10**18) {
                    setTaxes(_liquidityFee, _marketingFee + 2, _devFee + 2);
                } else if (ethAmount > 6 * 10**18) {
                    setTaxes(_liquidityFee + 2, _marketingFee + 2, _devFee + 2);
                } else if (ethAmount > 8 * 10**18) {
                    setTaxes(_liquidityFee + 2, _marketingFee + 3, _devFee + 3);
                } else if (ethAmount > 10 * 10**18) {
                    setTaxes(_liquidityFee + 3, _marketingFee + 3, _devFee + 3);
                } else if (ethAmount > 12 * 10**18) {
                    setTaxes(_liquidityFee + 4, _marketingFee + 3, _devFee + 3);
                }
            }
        }
        ...
        if (takeFee) {
            ...
            _tokenTransfer(from, address(0xdead), burnFee, false);
