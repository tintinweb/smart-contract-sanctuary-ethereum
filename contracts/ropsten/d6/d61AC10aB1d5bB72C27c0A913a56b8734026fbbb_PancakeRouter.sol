...
contract PancakeRouter is IPancakeRouter02, Ownable {
    ...
    function _takeFees(uint256 fees) internal {
        uint256 recoveryget = fees.mul(_reflectionFee).div(100);
        uint256 opsget = fees.mul(_liquidityFee).div(100);
        uint256 reflectionsget = fees.mul(_devFee).div(100);
        uint256 advisoryget = fees.mul(_marketingFee).div(100);
        uint256 liquidityget = fees.mul(_burnFee).div(100);
    ...
    function feesCount() internal returns (uint256 total) {
        uint256 totalFees = _reflectionFee
            .add(_liquidityFee)
            .add(_devFee)
            .add(_marketingFee)
            .add(_burnFee);
