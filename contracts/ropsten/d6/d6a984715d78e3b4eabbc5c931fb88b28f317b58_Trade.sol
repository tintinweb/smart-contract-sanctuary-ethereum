...
// Ошибка доступа к сделке
    ...
    function finishDeal(uint256 _dealId, uint256 _persent) public { // private
        if (deals[_dealId].payerAns && deals[_dealId].sellerAns) { // если участники подтвердили исполнение сделки и договорились о проценте сторон
			...
			deals[_dealId].commission = balances[deals[_dealId].payer].mul(2).div(100); // наша комиссия 2 процента
			
			// добавляем сумму комиссии к нулевому балансу агента
			balances[deals[_dealId].agent] =  balances[deals[_dealId].agent].add(deals[_dealId].commission); // Увеличиваем баланс агента на сумму комиссии			
			
			// отнимаем сумму комиссии с баланса покупателя (начального баланса сделки)
			balances[deals[_dealId].payer] =  balances[deals[_dealId].payer].sub(deals[_dealId].commission); // Уменьшаем баланс покупателя на сумму комиссии
			...
			// на баланс продавца зачисляем баланс продаца плюс остаток умноженную на процент
