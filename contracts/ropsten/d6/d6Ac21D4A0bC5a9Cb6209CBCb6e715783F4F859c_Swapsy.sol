...
contract Swapsy is ReentrancyGuard, Ownable {
    ...
    /**
     * Selling Ethereum with ERC20Token price
     * @param amountOut: amount out the transaction
     * @param tokenOut: address of ERC20Token
     * @param deadline: block timestamp for expires time in the pool
     */
    function sellETH(
        uint256 amountOut,
        address tokenOut,
    ...
    /**
     * Selling ERC20Token with ERC20Token price
     * @param amountIn: amount in the transaction
     * @param amountOut: amount out the transaction
     * @param tokenIn: address of ERC20Token
     * @param tokenOut: address of ERC20Token
     * @param deadline: block timestamp for expires time in the pool
     */
    function sellToken(
    ...
    function _buy(uint256 id) internal {
        ...
        if (_swaps.tokenOut == address(0)) {
            ...
            require(sent, "Swapsy: buy order failed");
        } else {
            require(
                ...
                "Swapsy: buy order failed"
            );
        }

        /* Get the product */
        uint256 buyingFee = ISwapsyManager(swapsyManager).getFeeForBuyer();
        uint256 buyingFeeAmt = (_swaps.amountIn * buyingFee) / 1000;
        uint256 buyingTransAmt = _swaps.amountIn - buyingFeeAmt;
        if (_swaps.tokenIn == address(0)) {
            (bool sent, ) = payable(msg.sender).call{value: buyingTransAmt}("");
            require(sent, "Swapsy: tx failed");
        } else {
            require(
                IERC20(_swaps.tokenIn).transfer(msg.sender, buyingTransAmt),
                "Swapsy: tx failed"
            );
        }

        _revenue[_swaps.tokenIn] += buyingTransAmt;
    ...
    function _claim(uint256 id) internal {
        ...
        bool _isExpired = (_swaps.status == STATUS.OPEN) &&
            (_swaps.deadline < block.timestamp);
        bool _isSoldOut = (_swaps.status == STATUS.SUCCESS);
        require(_isExpired || _isSoldOut, "Swapsy: cannot be claimed");
        ...
        if (_isExpired) {
            ...
            /* Selling fee */
            uint256 sellingFee = ISwapsyManager(swapsyManager)
                .getFeeForSeller();
            uint256 sellingFeeAmt = (_swaps.amountOut * sellingFee) / 1000;
            uint256 sellingTransAmt = _swaps.amountOut - sellingFeeAmt;

            if (_swaps.tokenOut == address(0)) {
                (bool sent, ) = payable(msg.sender).call{
                    value: sellingTransAmt
                }("");
                require(sent, "Swapsy: claim failed");
            } else {
                require(
                    IERC20(_swaps.tokenOut).transfer(
                        msg.sender,
                        sellingTransAmt
                    ),
                    "Swapsy: refund failed"
            ...
            _revenue[_swaps.tokenOut] += sellingFeeAmt;
            _swaps.status = STATUS.CLAIMED;
            emit Claim(
                _swaps.seller,
                _swaps.tokenOut,
                _swaps.amountOut,
