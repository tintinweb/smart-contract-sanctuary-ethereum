...
contract GabrielV2 is Ownable {
    ...
    function reusePool(uint _pid, ExtraArgs memory extraArgs, uint _NORT, address[] memory _rewardTokens, uint[] memory _totalRewards) external onlyOwner {
        ...
        pool.totalValLocked = pool.totalStaked;
        ...
        returns (uint[] memory unclaimedReward)
    {
        ...
        uint NORT = pool.NORT;
        if (block.timestamp > pool.lockTime && block.timestamp < pool.unlockTime && pool.totalStaked != 0) {
    ...
    function withdraw(uint _pid, bool forfeited, address tokenAddress, uint amount) external onlyOwner {
        // Calculate LOR.
        uint onePercent = amount.div(100);
        uint devShare = devPercent.mul(onePercent);
        uint gfShare = amount.sub(devShare);
        IERC20(tokenAddress).safeTransfer(devaddr, devShare);
        IERC20(tokenAddress).safeTransfer(growthFundAddr, gfShare);
        emit ReflectionsClaimed(_pid, tokenAddress, amount);
        if (!forfeited) {
            return;
        }
        PoolInfo storage pool = poolInfo[_pid];
        ...
        for (uint i = 0; i < NORT; i++) {
            ...
            if (rewardsInPool > 0 && block.timestamp > pool.unlockTime && pool.totalValLocked == 0) {
