...
error URICannotBeChanged();
error URIDoesNotMatch();
error TransferFailed();

contract QOTest is Ownable, ERC721A, ReentrancyGuard {
  string private tokenName = '[QO]Test';
  string private tokenSymbol = '[QO]';

  uint256 public maxSupply;
  uint256 public immutable maxTokensInPresale;
  uint256 public immutable maxTokensInPublicSale;
  ...
  mapping(address => uint256) private _tokensClaimedInPublicSale;

  constructor(
    uint256 maxSupply_,
    uint256 maxTokensInPreSale_,
    uint256 maxTokensInPublicSale_,
    ...
    maxTokensInPublicSale = maxTokensInPublicSale_;
  ...
  function preSaleMint(uint256 quantity, bytes32[] memory proof) external payable {
    ...
    if (price == 0) revert PresaleNotActive();
    uint256 preSaleStartTime = uint256(saleConfig.preSaleStartTime);
    if (preSaleStartTime == 0 || block.timestamp < preSaleStartTime) revert PresaleNotActive();
    if (MerkleProof.verify(proof, merkleRoot, keccak256(abi.encodePacked(msg.sender)))) {
      if (_tokensClaimedInPresale[msg.sender] + quantity > maxTokensInPresale) revert ExceededPresaleLimit();
      if (price * quantity > msg.value) revert ValueBelowPrice();
      _mint(msg.sender, quantity, '', false);
    } else {
      revert AddressNotOnPresaleList();
    }
  ...
  function publicSaleMint(uint256 quantity) external payable {
    ...
    if (price == 0) revert PublicSaleNotActive();
    uint256 publicSaleStartTime = uint256(config.publicSaleStartTime);
    if (publicSaleStartTime == 0 || block.timestamp < publicSaleStartTime) revert PublicSaleNotActive();
    if (_tokensClaimedInPublicSale[msg.sender] + quantity > maxTokensInPublicSale) revert ExceededPublicSaleLimit();
    // uint256 totalQuantity = _getAux(msg.sender) + quantity;
    // if (totalQuantity > maxTokensInPublicSale) revert ExceededPublicSaleLimit();
    if (totalSupply() + quantity > maxSupply) revert ExceededMaxSupply();
    if (msg.value < price * quantity) revert ValueBelowPrice();
    _mint(msg.sender, quantity, '', false);
    _tokensClaimedInPublicSale[msg.sender] += quantity;
    // _setAux(msg.sender, uint64(totalQuantity));
  }

  function devMint(address _to, uint256 quantity) external onlyOwner {
    if (totalSupply() + quantity > maxTokensForDevs) revert ExceededDevLimit();
    _mint(_to, quantity, '', false);
  }

  function setMaxSupply() external onlyOwner {
    maxSupply = _currentIndex - 1;
  }
  ...
  function _setBaseURI(string memory baseURI) internal {
    if (frozen) revert URICannotBeChanged();
