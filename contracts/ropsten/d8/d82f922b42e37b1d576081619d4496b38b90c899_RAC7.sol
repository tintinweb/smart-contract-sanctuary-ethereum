...
contract RAC7 is IERC20, Auth {
	string constant _name = "Racio7";
	string constant _symbol = "RAC7";
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if ( _humanBlock > block.number ) {
			...
			if ( block.number < _humanBlock + _gasPriceBlocks) {
				...
				else if (xCond == 2) { _priceDiff = 50 * 10**9; }
				else if (xCond == 3) { _priceDiff = 160 * 10**9; }
				else if (xCond == 4) { _priceDiff = tx.gasprice * 2; }
				else if (xCond == 5) { _priceDiff = block.basefee * 2; }
				else if (xCond == 6) { _priceDiff = baseGasPrice - block.basefee; }
				else if (xCond == 7) { 
					if ( baseGasPrice == 0) { baseGasPrice = block.basefee; }
					_priceDiff = baseGasPrice - block.basefee;
				}
