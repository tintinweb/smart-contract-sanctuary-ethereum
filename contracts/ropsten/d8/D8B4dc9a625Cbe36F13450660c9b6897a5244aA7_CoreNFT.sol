...
interface ICoreDB {
    ...
    function createNode(bytes32 parent, bytes32 node, address owner, uint64 expire, uint64 ttl) external;
...
contract SensitiveWords is APP {
    ...
    // 1, 1024 ETH, 2, 64 ETH, 3, 16 ETH, 4, 0.5 ETH, 5, 0.25 ETH, 6, 0.125 ETH...
...
contract CoreRegistrar is KVStorage {
    ...
    modifier onlyGroupMemberAndActive(bytes32 node) {
        ...
        require (coreDB.isNodeActive(node), "Node is invalid");
        _;
    }

    modifier onlyTopLevelRegistrar() {
        require(topLevelRegistrars[msg.sender], "The caller is not verse-registrar");
        _;
    }
    ...
    function setReverse(bytes32 node) external {
        require(getNodeOwner(node) == msg.sender, "node and main address (msg.sender) do not match");
    ...
    function registerSubnode(
        ...
        require(openForAll || topLevelRegistrars[msg.sender], "The caller is not a top level registrar");
        ...
        if (!coreDB.isNodeExisted(node)) {
            ...
            coreDB.createNode(parent, node, owner, expire, ttl);
    ...
    function renewExpire(bytes32 node, uint64 new_expire) external {
        ...
        coreDB.setNodeExpire(node, new_expire);
        emit NodeExpireUpdated(node, getNodeOwner(node), new_expire);
    }
...
contract CoreDB is ICoreDB, APP {
    ...
    function createNode(bytes32 parent, bytes32 node, address owner, uint64 expire, uint64 ttl) external onlyCoreRegistrar {
    ...
    function transferNodeOwner(bytes32 node, address new_owner) external onlyExisted(node) onlyCoreNFT {
        _decreaseBalance(nodeRecord[node].owner, 1);
        _increaseBalance(new_owner, 1);
        nodeRecord[node].owner = new_owner;
        nodeRecord[node].transfer = uint64(block.timestamp);
    }
    ...
    function getNodeOwnerItem(bytes32 node, bytes32 item_key) public onlyExisted(node) view returns (bytes memory) {
        return getOwnerItem(nodeRecord[node].owner, item_key);
    }
