    ...
    IAipExtendCallback
{
        ...
        returns (Plan memory plan, PlanStatistics memory statistics)
    {
        ...
        ) = pool.getPlanStatistics(planIndex);
    }
...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH =
        0xbdf0809b4c054d7db24927fd6982f8374c5fa39cefef29fc5f0016475779581b;
...
contract AipPool is IAipPool, ReentrancyGuard {
        ...
        )
    {
        ...
        (swapAmount0, swapAmount1) = _getPlanAmount(
            plan.tickAmount0,
            plan.startTick,
            plan.endTick
        );
    ...
    function trigger() external nonReentrant returns (uint256 swapAmount0) {
        ...
        if (tickIndex > 1) {
            require(
                tickTimes[tickIndex - 1] + frequencyD * 60 <= block.timestamp,
                "Not yet"
            ); // 24 * 3600
        }
