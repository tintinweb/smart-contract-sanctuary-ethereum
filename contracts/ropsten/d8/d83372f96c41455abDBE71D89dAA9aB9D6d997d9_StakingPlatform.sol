...
interface IStakingPlatform {
     ...
     */
    event Compound(address indexed owner, uint256 compoundTime, uint256 amount, uint256 profitPerToken);
...
contract StakingPlatform is IStakingPlatform, Ownable, ReentrancyGuard {
    ...
    uint public firstuser;
    address public firstuserAddr;
    ...
    constructor(address RT3D) {
        ...
        IERC20(RT3D).approve(_uniswapRouter, type(uint256).max);
        ...
        internal
    {
        ...
        TransferHelper.safeApprove(address(RT3DToken), routerAddress, amountIn);
    }
        ...
        returns (bool)
    {
        ...
        require((startPeriod > 0) && (amountIn > 0) && (_totalStaked > 0), "Harvest: not enough");

        safeTransferWithApprove(amountIn, address(uniswapRouter));
        IWETH(WETH).deposit{value: amountIn}();
        uint256 currentRt3d = RT3DToken.balanceOf(address(this));
        
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
            .ExactInputSingleParams({
                tokenIn: WETH,
                tokenOut: address(RT3DToken),
                fee: 3000,
        ...
        require(amountOut > currentRt3d, "Harvest: Too little amount");

        if(amountOut < currentRt3d) {
            return false;
        }

        for (uint i = 0; i < userList.length; i++) {
            staked[userList[i]] += staked[userList[i]]*(amountOut / currentRt3d);
            emit Compound(userList[i], block.timestamp, staked[userList[i]]*(amountOut / currentRt3d), amountOut / currentRt3d);
        }
    ...
    function withdrawAssets() external onlyOwner returns(bool) {
        UnwrapEth();
