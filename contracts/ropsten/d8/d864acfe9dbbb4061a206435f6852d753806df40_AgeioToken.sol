...
contract AgeioToken is Context, ITNT20 {
    ...
    string public _name = "Ageio Token";
    string public _symbol = "AGT1";
    uint8 public _decimals = 18;

    constructor () {
        require(msg.sender != address(0x0), "cannot deploy the token from address 0x0");
        _admin = msg.sender;
        _mint(_admin, 5*10**9*10**_decimals);
        _mintable = true;
    }
    ...
    function mint(uint256 amount) public returns (bool) {
        ...
        uint256 _amount = amount*10**uint256(_decimals);
        _mint(_admin, _amount);
        return true;
    }
