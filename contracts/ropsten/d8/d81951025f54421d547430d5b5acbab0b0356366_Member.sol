...
contract Member {
    ...
    mapping (address => VideoMetaInfo[]) private userVideos;

    function createVideo(string memory v, string memory title, string memory desc) external {
        userVideos[msg.sender].push(VideoMetaInfo({VAddress: v, VTitle:title, VDesc:desc, VAuthor:msg.sender}));
    }
    function videoInfo(string memory v) public view returns (string memory VTitle, string memory VDesc, address VAuthor) {
        for (uint p = 0; p < userVideos[msg.sender].length; p++) {
            if (keccak256(bytes(v)) == keccak256(bytes(userVideos[msg.sender][p].VAddress))){
                VideoMetaInfo memory vi = userVideos[msg.sender][p];
                VTitle = vi.VTitle;
                VDesc = vi.VDesc;
                VAuthor = vi.VAuthor;
            }
    ...
    mapping(address => string[]) private  subscribeAcls;
    function isSubscribe(string memory v) public view returns (bool result){
        result = false;
        for (uint p = 0; p < subscribeAcls[msg.sender].length; p++) {
            if (keccak256(bytes(v)) == keccak256(bytes(subscribeAcls[msg.sender][p]))){
                result = true;
            }
        }
    }
    ...
    function subscribe(string memory v) external payable{
