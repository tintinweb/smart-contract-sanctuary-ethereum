...
contract Tweeter_Token is Context, IERC20, Ownable {
    ...
    string private _name = "Tweeter";
    string private _symbol = "Tweeter";
    uint8 private _decimals = 9;
    

    uint256 public _devFee = 5;    
    address public DevWallet = 0xC686cCed29f3015FF97Bc37aC1FD992f7A34Dd07;
    uint256 private _previousDevFee = _devFee;

    uint256 public _marketingFee = 5;
    address public MarketingWallet = 0xC686cCed29f3015FF97Bc37aC1FD992f7A34Dd07;
    ...
    function removeAllFee() private {
        if( _devFee==0 && _marketingFee==0) return;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(marketingAmt).sub(devAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(marketingAmt).sub(devAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(marketingAmt).sub(devAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(marketingAmt).sub(devAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(marketingAmt).sub(devAmt)));
        }
    ...
    function setMarektingFeePercent(uint256 marketingFee) external onlyOwner() {
