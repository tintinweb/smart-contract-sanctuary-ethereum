...
contract YoInu is Context, IERC20, ERC20Ownable {
    using SafeMath for uint256;

    string private constant _nomenclature = "Yo Inu";
    ...
    bool private _initateLiqTrans = true;
    ...
    uint256 private _maxWalletSize = 150000000001 * 10**18;
    ...
    uint8 private _marTax = 6; // tax for marketing
    uint8 private _previousMarTax = _marTax;
    uint8 private _charTax = 2; // tax for charity
    uint8 private _previousCharTax = _charTax;
    uint8 private _liqTax = 2; // tax for liquidity
    uint8 private _previousLiqTax = _liqTax;
    uint8 private _refTax = 0; //tax for reflections
    uint8 private _previousRefTax = _refTax;
    uint8 private _liqDiv = _marTax + _charTax + _liqTax;
    ...
    constructor() {
        ...
        charityAddress = payable(0x5B320E1380108031F13a744052693514b7eB52A7);
        marketingAddress = payable(0x640e7d989211baD399B2292734A530d5aF4822eC);
    ...
    function taxTokensForCharity() external view returns (uint256) {
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        tokensForCharityToSwap += tLiquidity * _charTax / _liqDiv;
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_marTax + _charTax + _liqTax).div(10**2);
    }
    function removeAllFee() private {
        if (_refTax == 0 && _liqTax == 0 && _marTax == 0 && _charTax == 0) return;

        _previousRefTax = _refTax;
        _previousLiqTax = _liqTax;
        _previousMarTax = _marTax;
        _previousCharTax = _charTax;
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if (_buyLimits == true && from == uniV2Pair) {
			require(amount <= 40000000001 * 10**18, "Limits are in place, please lower buying amount");
		}
        if(_initateLiqTrans == true) {
	...
	function addBot(address _user) public onlyOwner {
        ...
        require(!_isBot[_user], "user already add as bot");
        _isBot[_user] = true;
    }
	function removeBot(address _user) public onlyOwner {
        require(_isBot[_user], "user already removed");
        _isBot[_user] = false;
    }
    ...
    function InitiateLiqAdd() external onlyOwner {
        _initateLiqTrans = true; //already delcared true above. Only call if declared false and are ready to add LP lock bool = 0xe70CFb2E1Fe3A1feA6fc0124A72585eB1A8c3984
    }
