...
contract DocuPay {
  ...
  function getCommentOnDocument(uint256 docId, uint256 commentIndex) public view returns(uint256) {
  ...
  function downvoteDocument(uint256 docId) public {
    ...
    // Remove the upvote from the document first, if any
  ...
  function sendComment(uint256 docId, uint256 date, string memory commentMsg) public {
    ...
    return commentsOnDocument[docId].push(comments.length + 1);
  }
}