  ...
  ERC721EnumerableUpgradeable
{
  ...
  function mint(
    ...
    uint256 total = computePrice(category, _options);
    uint256 tmpCredit = credits[to];
    if (credits[to] > 0 && overridePrice == 0) {
      credits[to] = tmpCredit > total ? tmpCredit - total : 0;
      total = tmpCredit > total ? 0 : total - tmpCredit;
    }
    if (hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) total = overridePrice;
    if (total > 0) {
      if (referrer != address(0) && tmpCredit < 10 * 10**6) {
        uint256 txRefReward = referrals[referrer].refRewards != 0
          ? referrals[referrer].refRewards
          : globalRefRewards;
        IERC20(currency).transferFrom(to, address(this), txRefReward < 100
          ? ((total * txRefReward) / 100)
          : txRefReward);
        IERC20(currency).transferFrom(to, splitWallet, total - txRefReward < 100
          ? ((total * txRefReward) / 100)
          : txRefReward);
        referrals[referrer].amount += txRefReward < 100
          ? ((total * txRefReward) / 100)
          : txRefReward;
      } else {
  ...
  function computePrice(string[] memory category, uint256[] memory selection)
    public
    view
    returns (uint256)
  {
