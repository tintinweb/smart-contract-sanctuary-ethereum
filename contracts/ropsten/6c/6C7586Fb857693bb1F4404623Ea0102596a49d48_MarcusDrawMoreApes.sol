...
contract MarcusDrawMoreApes is ERC721, ERC721Enumerable, IERC2981, Ownable {
  ...
  uint16 private _royaltyFeeNumeratorBasisPoints = 250; // 2.5%
  ...
  uint16 private constant _royaltyFeeDenominator = 10000;
  ...
  function supportsInterface(bytes4 interfaceId)
    ...
    returns (bool)
  {
    return ERC721Enumerable.supportsInterface(interfaceId) || interfaceId == type(IERC2981).interfaceId;
  }

  modifier supplyCheck(uint256 quantity) {
    require(totalSupply() + quantity <= maxSupply, "Purchase would exceed max supply of Painted Apes");
    _;
  }
  ...
  function setRoyaltyFeeNumerator(uint16 feeNumeratorBasisPoints) public onlyOwner {
    require(feeNumeratorBasisPoints <= _royaltyFeeDenominator, "Royalty fee must not exceed exceed sale price");
    _royaltyFeeNumeratorBasisPoints = feeNumeratorBasisPoints;
  }
  ...
  function royaltyInfo(uint256, uint256 salePrice) external view returns (address, uint256) {
    uint256 royaltyAmount = (salePrice * _royaltyFeeNumeratorBasisPoints) / _royaltyFeeDenominator;
