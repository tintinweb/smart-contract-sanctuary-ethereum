...
contract DBSNFTLootBoxTokens is ERC721Burnable, Ownable{
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        DBSNFTLootBox box = DBSNFTLootBox(lootBox);
        string memory json = string(abi.encodePacked("{ \"name\":" , box.getBox(tokens[_tokenId].boxId).name, "\"image\":", baseuri, box.getBox(tokens[_tokenId].boxId).image, "\"description\":", box.getBox(tokens[_tokenId].boxId).description, "\"attributes\":", box.getBox(tokens[_tokenId].boxId).attributes,"}"));
        return json;
    }
