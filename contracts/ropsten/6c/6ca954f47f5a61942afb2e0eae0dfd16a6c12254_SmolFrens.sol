...
contract SmolFrens is Ownable, ERC721, NonblockingReceiver {
    ...
    function mint(bytes32[] calldata _merkleProof, uint _amount) external payable {
        uint supply = 8889;
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));

        require(status == 30, 'Whitelist Mint Not Live');
    ...
    function Whitelist(bytes32[] calldata _merkleProof, uint _amount) external payable {
        uint supply = 8889;
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
