...
interface IOracle {
    ...
    function getMultiAssetPriceData(Asset[] calldata assets) external view returns (uint256[] memory priceArr,uint256 updateAt);
...
contract AssetPriceOracle is IOracle, Ownable {
    ...
    event AddAsset(address indexed addr,uint256 indexed subId,uint256 indexed index);
    event AssetPriceDataUpdated(uint256 indexed roundId, uint256 indexed updateAt, bytes data);
    struct AssetPriceData {
        uint256 updateAt;
        bytes data;
    }
    bool public constant isMomaOracle = true;
    ...
    uint256 private constant minUpdateTime = 1200; // 20 minutes 20 * 60 = 1200
    ...
    function getMultiAssetPriceData(Asset[] calldata _assets) external override view returns (uint256[] memory priceArr,uint256){
      uint256 len = _assets.length;
      priceArr = new uint256[](len);
      uint256 price;
      uint256 updateAt;
      for(uint256 i=0; i < len; i++){
        (price, updateAt) = this.getAssetPriceData(_assets[i]);
        priceArr[i] = price;
      }
      return (priceArr,updateAt);
    }
    ...
    function getAssetPriceDataByPeriod(Asset calldata asset,uint256 period) external override view checkRoundId  returns(uint256[2][] memory) {
       require(isExistedAsset(asset),"Oracle: asset not existed");
       uint256 assetIndex = assetIndexMap[asset.addr][asset.subId];
       uint256 t = block.timestamp - period * 3600;
