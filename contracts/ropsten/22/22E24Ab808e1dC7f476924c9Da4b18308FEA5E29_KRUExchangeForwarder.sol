...
interface IKRUExchangeForwarder {
    ...
    event Forward(address indexed sender_, address indexed recipient_, address indexed token_, uint256 amount_);
...
contract KRUExchangeForwarder is IKRUExchangeForwarder, ManagedUpgradeable, EIP712Upgradeable {
    ...
    bytes32 private constant _CONTAINER_TYPEHASE =
        keccak256(
            "Container(address sender,address token,address destination,uint256 amount,uint256 deadline,uint256 nonce)"
        );
    ...
    function forward(
        ...
        _forward(_msgSender(), token_, amount, deadline_, nonce_, v_, r_, s_);
    }
    ...
    function _forward(
        ...
        require(_isValidSigner(sender_, token_, amount_, deadline_, nonce_, v_, r_, s_), ERROR_INVALID_SIGNER);
        ...
        emit Forward(sender_, destinationAddress, token_, amount_);
    }
    ...
    function _isValidSigner(
        ...
        bytes32 structHash = keccak256(
            abi.encode(_CONTAINER_TYPEHASE, sender_, token_, destinationAddress, amount_, deadline_, nonce_)
        );
