...
░╚═════╝░╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝╚═╝░░╚═╝░░░╚═╝░░░╚═╝░░░░░╚═╝░░╚═╝╚═════╝░         
    ...
    This token has a 1% burn fee on each transaction, except buying.
...
contract GalaxyPad is ERC20, Ownable {
     ...
     */
    constructor(address _factoryAddress) ERC20("GalaxyPad", "GXPAD") {
        require(
            _factoryAddress != address(0),
            "Factory address cannot be null"
        );
    ...
    function setBurnFee(uint256 _fee) external onlyOwner {
        require(_fee <= 1000, "Fee must be lower than 10%");
        burnFee = _fee;
    }
    ...
    //Exemptions
    function exemptFromFee(address _address, bool _exempted)
        external
        onlyOwner
    {
    ...
    //Blacklist
    function setBlacklistAddress(address _address, bool _blacklisted)
        external
        onlyOwner
    {
    ...
    function _checkTransfer(
        ...
        address pairAddress = IUniswapV2Factory(factoryAddress).allPairs(0);

        // Check if pair is created
        if (pairAddress != address(0)) {
            
            // Check if transaction is a sell and fees are activated/exempted
            if (feeEnabled && !feeExemption[_from] && pairAddress != _from) {
                // Check Fee
