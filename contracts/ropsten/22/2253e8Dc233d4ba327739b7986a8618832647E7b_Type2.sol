...
contract Type2 is ERC1155Burnable, Ownable {
    address public typeOneAddress;
    ...
    /**
     * @dev set type one address
     */
    function setTypeOneAddress(address firstNFTAddress) public onlyOwner {
        typeOneAddress = firstNFTAddress;
    }

    /**
     * @dev mint new token calling by type one
     */
    function mintTokenByOne(address owner, uint256 typeId, uint256 mintAmount) external {
        require(_msgSender() == typeOneAddress, "Should be call by type one address");

        for (uint256 i = 0; i < mintAmount; i += 1) {
            _mint(owner, typeId, mintAmount, "");
        }
    }
