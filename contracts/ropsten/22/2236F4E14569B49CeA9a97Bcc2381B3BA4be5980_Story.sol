...
contract Story {
     ...
     */
    mapping (address => mapping (address => storyDetails[])) public tokenStories;

    // a mapping of contract addresses and their contribution stats
    mapping (address => Stats) public contributionStats;
    ...
    modifier commitHelper (address tokenAddress) {
        ...
        require(contributionStats[tokenAddress].lastContributor != msg.sender, "You can't contribute to the same story twice in a roll");
    ...
    */
    function createStoryCommit (
        ...
        tokenStories[tokenAddress][msg.sender].push(
            storyDetails(storyHash, bytes(""), false, block.timestamp)
        );
    }
    ...
    * @param tokenAddress address of the token contract
    ...
    function addStory (
        ...
        bytes memory story
            ...
            bytes32 storyReveal = sha256(abi.encodePacked(msg.sender, story));
            tokenStories[tokenAddress][msg.sender].push(
            storyDetails(storyReveal, story, true, block.timestamp)
        );
    }
