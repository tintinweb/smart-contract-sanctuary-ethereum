...
contract DAO{
    ...
    function withdrawDeposit(uint256 _amount) external {
        // Так и не придумал, как реализовать оптимизированную проверку, что закончены все голосования
        require(_amount <= deposits[msg.sender].amount, "Error: Sender cannot withdraw more tokens than he has");
        TOD.transferFrom(address(this), msg.sender, _amount);
        deposits[msg.sender].amount -= _amount;
    }
    ...
    function addProposal(bytes calldata _pCallData, address _pCallAddress) external {
        ...
        newProposal.time = block.timestamp;
    ...
    function vote(uint _pId, bool _choice) external {
        ...
        require(block.timestamp < (allProposals[_pId].time + time), "Error: Proposals time has finished");
    ...
    function finishProposal(uint _pId) external {
        ...
        require(block.timestamp <= (proposal.time + time), "Error: Proposals time has not yet finished");
        require(!proposal.Finished, "Error: Proposal is already finished");
        proposal.Finished = true;
        bool quorum = ((proposal.tokenYes + proposal.tokenNo) >= (TOD.getSupply() / 2)) && (proposal.tokenYes > proposal.tokenNo);
        bool success = false;
        if (quorum){
    ...
    function getProposalByID(uint _pId) external view returns(Proposal memory){
        require(allProposals[_pId].time != 0, "This proposal does not exist");
