...
contract Voting {
	...
	mapping(uint256 => CommunityPoll) private _commpolls; //pollId -> CommunityPoll
	mapping(uint256 => PollVote[]) private _pollVotes; // _PollIds -> PollVotes 
	mapping(uint256 => PollVote) private _commPollVotes; // __voteIds -> PollVote 
	...
	*/
	function communityVoteCast(uint256 commId, uint256 pollId, bool vote) external {
		...
		if(poll.endTimeStamps < block.timestamp){
			...
			_commPollVotes[voteId] = pollvote; 
	 ...
	 */
	function isAlreadyVoted(uint256 pollId) external view returns(PollVote[] memory){
		PollVote[] memory pollVotes = _pollVotes[pollId];
		// uint256 voteId = _voteIds.current();
		// bool isVoted = false;
		// for (uint256 index = 0; index < pollVotes.length ; index++) {
		// 	if(pollVotes[index].voter == msg.sender){ //&& _commPollVotes[index].pollId == pollId
		// 		isVoted = true;
		// 	}
		// }
		// return isVoted;
