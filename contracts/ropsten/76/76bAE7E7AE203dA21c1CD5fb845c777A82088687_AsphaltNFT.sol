...
contract AsphaltNFT is ERC721, Ownable {
    ...
    // IMX address event
    event ImxAddressChanged(address imx);

    // IMX status event
    event ImxEnableStatusChanged(bool status);
    ...
    */
    constructor(
        ...
        require(_imx != address(0), "AsphaltNFT: _imx must be a non-zero address");
        baseURI = _uri;
        imx = _imx;
        imxEnabled = true;
    }
    ...
    * @param mintingBlob The minting data from L2 with format {tokenId}:{blueprint}
    */
    function mintFor(
        address user,
        uint256 quantity,
    ...
    * @param isEnabled Enabled value to be set
    */
    function setEnableImx(bool isEnabled) external onlyOwner {
        imxEnabled = isEnabled;

        emit ImxEnableStatusChanged(isEnabled);
    }

    /** 
    * @dev Set IMX L2 contract address
    * @param newImx The IMX L2 contract address to be set
    */
    function setImx(address newImx) external onlyOwner {
        require(newImx != address(0), "AsphaltNFT: newIMX must be a non-zero address");
        imx = newImx;

        emit ImxAddressChanged(newImx);
    }
}
