...
contract AMAv2 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function ask(uint questionId, address to, uint amount) external {
        ...
        emit Asked(questionId, msg.sender, amount);
    }
    ...
    function refund(uint questionId) external {
        Question storage q = questions[questionId];
        require(q.questioner == msg.sender, "Not questioner");
        require(!q.answered, "Already answered");
        require(block.timestamp > q.timestamp + TIME_LIMIT_OF_ANSWER , "Please wait util timeout");
        require(q.reward > 0, "Insufficient balance");
        ...
        emit Refunded(questionId, msg.sender, reward);
    }
    ...
    function withdraw(uint questionId) external {
        ...
        require(msg.sender == q.questioner || msg.sender == q.answerer, "Not a questioner or answerer");

        uint withdrawn = q.bonus / 2 - q.withdrawn[msg.sender];
