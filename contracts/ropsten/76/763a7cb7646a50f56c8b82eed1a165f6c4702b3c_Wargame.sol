...
contract Wargame {
        ...
        internal
    {
		...
		if (now > roundEnd_ && roundEnd_ > 0) {
			revert();				// return card has not been flipped
		}
        ...
        public
    {
		...
		// if contract is deactivated, payout winner and send balance to fund
		if (!contractActivated_) {
			// make sure a winner is set.
			...
			// round is over, start delivery service
			if (now > roundEnd_ && roundEnd_ > 0) {
				currentWinner_.transfer(_winnings);
