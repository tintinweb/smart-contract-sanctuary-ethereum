...
interface decentralizedStorage{
    function addProject(address _contractAddress, string memory _name) external;
...
contract CUSTOMDIVIDENDTOKENDAPP is Ownable {
    ...
    function CreateCustomToken(address _rewardToken, string memory _tokenName, string memory _tokenSymbol, uint8 _decimals, uint256 _totalSupply, uint8 _rewardFee, uint8 _liqFee, address _router) public payable {
        ...
        payable(decentralizedStorage(storageMint).getBurnContractAddress()).transfer(creationFees);

        address TokenCreation = address (new DividendGeneratorToken(_rewardToken, _tokenName, _tokenSymbol, _decimals,_totalSupply,_rewardFee,_liqFee,_router));
        Ownable(TokenCreation).transferOwnership(msg.sender);

        decentralizedStorage(storageMint).addProject(TokenCreation, "dividendContract");
