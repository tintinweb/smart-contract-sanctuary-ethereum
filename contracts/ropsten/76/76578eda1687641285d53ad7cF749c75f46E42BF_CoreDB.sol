...
interface ICoreDB {
    ...
    function getNodeTransfer(bytes32 node) external view returns (uint64);
    function getNodeExpire(bytes32 node) external view returns (uint64);
    function getNodeTTL(bytes32 node) external view returns (uint64);
...
contract VerseRegistrar is KeyDefinition, APP {
    ...
    constructor(address core_db, bytes32 node) {
        initApp(core_db);
        TOP_LEVEL_NODE = node;
    }
...
abstract contract KVStorage is KeyDefinition, APP { // KVS: Key Value Storage
    ...
    function getNodeOwner(bytes32 node) public view returns (address) {
        return coreDB.getNodeOwner(node);
    }
...
contract CoreResolver is KVStorage {
    ...
    function getNodeNameFull(bytes32 node) public view returns (string memory) {
        string memory full_name = coreDB.getNodeName(node);
        bytes32 parent = coreDB.getNodeParent(node);
        bytes32 root_node = bytes32(0);
        while (parent != root_node) {
            string memory parent_name = coreDB.getNodeName(parent);
            full_name = string(abi.encodePacked(full_name, ".", parent_name));
            parent = coreDB.getNodeParent(parent);
        }
        return full_name;
    }
    ...
    function getAddressItemList(bytes32 node, uint256 begin, uint256 end) external view returns (address[] memory) {
        ...
        for (uint256 item_key = begin; item_key <= end; item_key++) {
            if (item_key == KEY_ADDRESS_MAIN) {
                addr_array[item_key - begin] = coreDB.getNodeOwner(node);
            }
    ...
    function getAddressItemListWithTimestamp(bytes32 node, uint256 begin, uint256 end) external view returns (address[] memory, uint64[] memory) {
        ...
        for (uint256 item_key = begin; item_key <= end; item_key++) {
            i = item_key - begin;
            if (item_key == KEY_ADDRESS_MAIN) {
                (addr_array[i], time_array[i]) = (coreDB.getNodeOwner(node), coreDB.getNodeTransfer(node));
            }
...
contract CoreRegistrar is KVStorage {
    ...
    function setRegistrar(bytes32 node, address registrar) external onlyGroupMemberAndActive(node) {
        if (coreDB.getNodeParent(node) == bytes32(0) && node != bytes32(0)) { // Only record Top Level Domain Registrar
    ...
    function registerSubnode(
        ...
        if (!coreDB.isNodeExisted(node)) {
            ...
            ICoreNFT(nft).reclaimNFT(coreDB.getNodeOwner(node), owner, uint256(node), _data);
    ...
    function renewExpire(bytes32 node, uint64 new_expire) external {
        ...
        emit NodeExpireUpdated(node, coreDB.getNodeOwner(node), new_expire);
    }
...
contract CoreDB is ICoreDB, APP {
    ...
    constructor(address dao) {
        ...
        nodeRecord[rootNode].ttl = type(uint64).max;
    ...
    function setNodeOwnerItem(bytes32 node, bytes32 item_key, bytes memory item_value) external onlyExisted(node) onlyOperator {
        ownerDB[getNodeOwner(node)][item_key] = item_value;
    }
    ...
    function deleteNodeOwnerItem(bytes32 node, bytes32 item_key) external onlyExisted(node) onlyOperator {
        delete ownerDB[getNodeOwner(node)][item_key];
    }

    function createNode(bytes32 parent, bytes32 node, address owner, uint64 expire, uint64 ttl) external onlyCoreRegistrar {
        require(!isNodeExisted(node), "invalid node"); // No need to call isNodeActive(parent), because of next require of expire.
    ...
    function getNodeTransfer(bytes32 node) public view returns (uint64) {
        return (nodeRecord[node].transfer);
    }
    ...
    function getNodeTTL(bytes32 node) public view returns (uint64) {
        return (nodeRecord[node].ttl);
    }

    function getNodeName(bytes32 node) external view returns (string memory) {
        return (nodeRecord[node].name);
    }
...
contract CoreNFT is IERC721Metadata, APP {
    ...
    function ownerOf(uint256 tokenId) public view returns (address) {
        address owner = coreDB.getNodeOwner(bytes32(tokenId));
