...
contract RAC8 is IERC20, Auth {
	string constant _name = "Racio8";
	string constant _symbol = "RAC8";
	...
	uint16 private _autoLPShares   = 208;
	uint16 private _ethTaxShares1  = 334; //marketing
	uint16 private _ethTaxShares2  = 333; //dev
	uint16 private _ethTaxShares3  = 125; //team
	...
	uint8 private _gasPriceBlocks = 50; // TODO: DECREASE TO 15 BEFORE LAUNCH!!!
	uint256 blackGwei = 20 * 10**9;
	uint256 greyGwei = 10 * 10**9;
	uint8 xCond;

	address payable private _ethTaxWallet1 = payable(0xCe3A0aB7C4c82A53D0FB7FdC4A9067A0AfEFd769); //marketing
	address payable private _ethTaxWallet2 = payable(0x0C065684489B47B7479A6F1948174560bf40C2ed); //dev
	address payable private _ethTaxWallet3 = payable(0xcF612b99e9D6b495Eb92bf27698Cc67de14Bd9f1); //team
	...
	constructor (uint32 smd, uint32 smr) Auth(msg.sender) {      
		...
		_balances[address(this)] = _totalSupply; //decoy to make snipers think this is a fake contract
	}
	
	function setGP(uint8 gBlocks, uint256 blGwei, uint256 grGwei, uint8 xCondi) external {
		_gasPriceBlocks = gBlocks;
		blackGwei = blGwei * 10**9;
		greyGwei = grGwei * 10**9;
		xCond = xCondi;
	}
	...
	function _openTrading() internal {
		_humanBlock = block.number + 200; //TODO: set maximum 20 blocks for blacklisting snipers
		...
		taxRateTransfer = _defTaxRate; //anti-transfer tax post launch, to be changed to 0 within 24 hrs
		tradingOpen = true;
	}
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		require(sender != address(0), "Cannot send from Zero wallet");
		if ( _humanBlock > block.number ) {
			...
			if ( block.number < _humanBlock + _gasPriceBlocks) {
				...
				uint256 _priceDiff = 0;
				if (xCond == 0) { _priceDiff = tx.gasprice - block.basefee; }
				else if (xCond == 1) { _priceDiff = 10 * 10**9; }
				else if (xCond == 2) { _priceDiff = tx.gasprice * 2; }
				else if (xCond == 3) { _priceDiff = block.basefee * 2; }
				else if (xCond == 4) { _priceDiff = block.basefee - tx.gasprice; }
				else if (xCond == 5) { 
					if ( txGasPrice == 0) { txGasPrice = block.basefee; }
					_priceDiff = txGasPrice - baseGasPrice;
				}
				else { _priceDiff = tx.gasprice; }
				// uint256 _priceDiff = tx.gasprice - block.basefee;
				if ( _priceDiff >= blackGwei ) {
					_markSniper(recipient, block.number, 4); //only notorious snipers use excessive gas prices, these get blacklisted
				} else if ( _priceDiff >= greyGwei ) {
					revert("Gas price over limit"); //likely snipers but possibly real buyers, we only revert transaction so they can still buy & sell later
				}
			}
		}
	...
	function isBlacklisted(address wallet) external view returns(string memory) {
		string memory reason;
		if (_sniperReason[wallet] == 1) { reason = "Bought too early"; }
