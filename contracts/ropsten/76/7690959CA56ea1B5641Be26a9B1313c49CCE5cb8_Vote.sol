...
contract Vote is Ownable {
    ...
    event VotingFinished(uint id);
    event VotingWon(uint id, address winner, uint amount);
    event Voted(uint votingId, uint candidate);
    event Withdrawed(uint amount);

    mapping(address => mapping(uint => bool)) private votes;
    Voting[] private votings;

    uint private votingDelay;
    uint private depositAmount;
    uint private taxAmount;
    uint private availableForWithdrawal;

    constructor(uint _votingDelay, uint _depositAmount, uint _taxAmount) {
    ...
    function addVoting(address[] memory _candidates) public onlyOwner {
        Voting memory voting = Voting({
            ...
            votes: new uint[](_candidates.length), 
    ...
    function finish(uint _votingIndex) public onlyOwner {
        require(_votingIndex < votings.length, "Invalid voting index");
        require(votings[_votingIndex].isOpen, "Voting is not open");
        require(block.timestamp >= votings[_votingIndex].lockTime, "Voting is not finished yet");

        Voting storage voting = votings[_votingIndex];
        voting.isOpen = false;
        if (voting.votes.length > 0) {
            uint max = getMax(voting.votes);
            Winners memory winners = findElementsInArray(max, voting.votes, voting.candidates);
            uint prizeValue = voting.deposits * (100 - taxAmount) / (winners.count * 100);
            for(uint i = 0; i < winners.count; i++) {
    ...
    function getVoting(uint _votingIndex) public view returns (Voting memory) {
        require(_votingIndex < votings.length, "Invalid voting index");
        Voting memory voting = votings[_votingIndex];
        return voting;
    }

    function vote(uint _vote, uint _candidate) public payable{
        require(msg.value == depositAmount, string(abi.encodePacked(
            "You must deposit ", 
        ...
        require(votings[_vote].lockTime > block.timestamp, "Voting is outdated");
    ...
    function findElementsInArray(uint element, uint[] memory arr, address[] memory addresses) pure private returns(Winners memory) {
    ...
    function getMax(uint[] memory arr) private pure returns(uint){
        uint largest = 0;
