...
contract TestCham is IERC721, Ownable, Functional {
    ...
    */
    ...
    function PublicMint(uint256 qty) external payable reentryLock {
        ...
        require(numberMinted >=10 && numberMinted <20,"Cannot Mint token id greater than 20");
        callerAdd = msg.sender;
        ...
        uint256 mintSeedValue = numberMinted;
        uint mintqty = qty;
        if(numberMinted > 17 && numberMinted < 20)
        {
            require(qty == 1, "Quantity cannot be more than 1 after token ID 17 to 20");
        ...
        for(uint256 i = 0; i < qty; i++) {
          ...
          uint256 cashIn = msg.value;
          uint256 cashChange = cashIn - (mintqty * price);
           if (cashChange > 0){
            (bool success, ) = msg.sender.call{value: cashChange}("");
            require(success, "Mint: unable to send change to user");
        }
    ...
    function MintPublicDiscount(uint256 qty) external payable reentryLock {
        require(qty <= maxPerTxn, "max 50 per txn");
        require(numberMinted >= 20 && numberMinted <= 25, "Cannot call this mint with ID greater than 25");
        require(mintActive);
        uint256 refundPrice;
        callerAdd = msg.sender;
        if(qty >= 10)
        ...
        for(uint256 i = 0; i < qty; i++) {
         ...
         uint256 cashIn = msg.value;
         uint256 cashChange = cashIn - (qty * refundPrice);
           if (cashChange > 0){
            (bool success, ) = msg.sender.call{value: cashChange}("");
            require(success, "Mint: unable to send change to user");
        }
