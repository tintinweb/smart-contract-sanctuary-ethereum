...
contract BrandingOrder is Adminer {
  ...
  function fulfillOrder(Order calldata order, uint256 amount) external payable {
    ...
    if (info.orderType == 0) {
      ...
      ERC1155(info.contractAddress).safeTransferFrom(msg.sender, owner, info.tokenId, amount, "Branding NFT trade");
    ...
    if (_fulfillMap[orderId].length == 0) {
      _orderIds.push(orderId);
    }

    OrderFulfill memory fulfill;
    fulfill.id = orderId;
    fulfill.owner = msg.sender;
    fulfill.time = block.timestamp;
    fulfill.amount = amount;
