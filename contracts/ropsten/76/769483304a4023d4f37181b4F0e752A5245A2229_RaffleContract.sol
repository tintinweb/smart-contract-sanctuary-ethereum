...
contract RaffleContract is Ownable {
    ...
    function createRaffle(
        uint256 rafflePrize_,
        uint256 price_,
        uint256 endIn_,
        uint256 totalTickets_,
        TicketsLimit ticketsLimit_,
        uint256 mininumTickets_
    ) public onlyOwner {
        require(
            IERC20(USDC).allowance(owner(), address(this)) >= rafflePrize_,
            'Contract not approved'
        );
        require(
            IERC20(USDC).balanceOf(owner()) >= rafflePrize_,
            'Not enough USDC tokens'
        );
        IERC20(USDC).transferFrom(owner(), address(this), rafflePrize_);
        _raffleId.increment();
    ...
    function cancelRaffle(uint256 id) public onlyOwner {
        require(id <= _raffleId.current() && id > 0, 'Raffle id not exist');
        ...
        IERC20(USDC).transferFrom(address(this), owner(), raffle.price);
    }

    function fetchCompletedRaffle() public view returns (Raffle[] memory) {
        uint256 raffleCount = _raffleId.current();
        uint256 leftCompletedRaffles = _raffleCancelled.current();
    ...
    function purchaseTicket(uint256 id, uint256 quantity) public {
        ...
        if (raffle.ticketsLimit == TicketsLimit.Finite) {
            require(
    ...
    function selectRandomWinner(uint256 id) public onlyOwner {
        ...
        require(block.timestamp >= raffle.endIn, 'Raffle is not ended yet');
        uint256 randomNumber = random(totalTicketSold[id]);
    ...
    function claimFees(uint256 id) public {
        ...
        IERC20(USDC).transferFrom(address(this), msg.sender, feesToReturn);
    }
}
