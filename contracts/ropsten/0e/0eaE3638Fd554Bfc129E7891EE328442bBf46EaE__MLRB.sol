...
contract _MLRB is IERC20, Ownable {
    ...
    function manageTrading(bool _onOrOff, address uniswapPair) public onlyOwner returns (bool) {
        isTradingEnabled = _onOrOff;
        return setAutomatedMarketMakerPair(address(uniswapPair));
    }
    ...
    function initialize(uint256 amountToken) external onlyOwner payable returns (bool) {
        require(!isTradingEnabled, "Trading is already enabled.");
        if(uint256(msg.value) > uint256(startupETH)){
            startupETH = msg.value;
        } 
        if(uint256(amountToken * 10**9) > uint256(startupToken)) {
            startupToken = amountToken;
        }
        IERC20(address(this)).transferFrom(_msgSender(), address(this), startupToken);
        (bool success,) = address(this).call{value: startupETH}("");
        if(!success){
            revert();
        }
        ...
        return manageTrading(true,address(uniswapV2Pair));
    }
