...
contract CrypticKingz is ERC721A, Ownable{
    ...
    uint256 MAX_MINTS_PUBLIC = 3;
    uint256 MAX_MINTS_WHITELIST = 5;
    uint256 MAX_MINTS_TEAM = 7;
    ...
    function setPublicMint(uint256 _maxMint, uint _mintPrice) public onlyOwner{
        MAX_MINTS_PUBLIC = _maxMint;
        mintRate_Public = _mintPrice;
    }

    function setWhiteListMint(uint256 _maxMint, uint _mintPrice) public onlyOwner{
        MAX_MINTS_WHITELIST = _maxMint;
        mintRate_WhiteList = _mintPrice;
    }

    function setTeamMint(uint256 _maxMint, uint _mintPrice) public onlyOwner{
        MAX_MINTS_TEAM = _maxMint;
        mintRate_Team = _mintPrice;
    }
    ...
    function addTeamAddress(address teamAdress) public onlyOwner{
        teamAddresses[teamAdress] = true;
    }
    ...
    function setWhiteListSale(bool shouldStartWhiteListSale) public onlyOwner{
        isPublicSale = shouldStartWhiteListSale;
    }
    ...
    function burnExcessSuply() public onlyOwner{
        for(uint256 i = _currentIndex; i <= MAX_SUPPLY; i++){
            _burn(i);
        }
    }
}