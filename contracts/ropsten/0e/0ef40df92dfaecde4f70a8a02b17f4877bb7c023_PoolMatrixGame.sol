...
contract PoolMatrixGame {
    ...
    function buyLevel(uint level, uint investAmount) public payable {
        ...
        require(users[msg.sender].id > 0); // Check if user is exists
        ...
        // Updating increments
        updateLevelIncrements(investAmount);
        ...
        // Activating level
        if (users[msg.sender].levels[level].openState != 2) {
    ...
    function updateLevelIncrements(uint investAmount) private {
        uint half = (block.timestamp / SECONDS_IN_DAY_HALF) % 2;
        stats24hNormalize(half);
        if (half == 0) {
    ...
    function getUserByID(uint id) public view returns(User memory) {
        return getUser(userAddresses[id - 1]);
    }
