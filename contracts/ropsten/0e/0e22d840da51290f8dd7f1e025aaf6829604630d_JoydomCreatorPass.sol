...
contract JoydomCreatorPass is Ownable, ERC721A, ReentrancyGuard {
    ...
    function isInAllowList2() public view returns (bytes32) {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        return leaf;
    }

    function isInAllowList3(bytes memory a) public pure returns (bytes32) {
        bytes32 leaf = keccak256(abi.encodePacked(a));
        return leaf;
    }

    function isInAllowList4(bytes32[] memory proof) public pure returns (uint256) {
        return proof.length;
    }

    function isInAllowList5(bytes32[] memory proof) public pure returns (bytes32) {
        uint256 i = 0;
        bytes32 a = proof[i];
        return a;
    }
