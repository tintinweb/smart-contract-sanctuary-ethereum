 ...
 */
contract TurtleTown is ERC721, Ownable, PaymentSplitter {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    Counters.Counter private _tokenIdWhitelistCounter;
    ...
    uint256 public constant maxTokens = 9500;

    // uint256 public constant maxWhitelistTokens = 1;
    uint256 public constant maxWhitelistTokens = 1000;

    uint256 public devTokenCount = 9501;
    ...
    bytes32 public merkleRoot =
        0x8f721c2ef396833f5fbecf00499e66ec12382d4d777f6690803477680f0f2a50;

    mapping(address => bool) public whitelistClaimed;
    mapping(address => uint256) public addressMinted;

    constructor() ERC721("Turtle Town", "TRTL") {}

    function mintDev(uint256 quantity) external onlyOwner {
        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(msg.sender, devTokenCount++);
        }
    }
    ...
    function whitelistMint(bytes32[] calldata _merkleProof) public payable {
        ...
        require(
            totalWhitelistSupply() + 1 <= maxWhitelistTokens,
            "Minting too many"
        );

        whitelistClaimed[msg.sender] = true;

        whitelistSafeMint(msg.sender);
    }
    ...
    function preSaleMint(
        ...
        require(
            addressMinted[msg.sender] < maxMintsPerWallet,
            "Max 5 per address"
        );
    ...
    function safeMint(address to) internal {
        ...
        _safeMint(to, tokenId + 1000);
    }

    function whitelistSafeMint(address to) internal {
        uint256 tokenId = _tokenIdWhitelistCounter.current();
        _tokenIdWhitelistCounter.increment();
        _safeMint(to, tokenId);
    }

    function totalSupply() public view override returns (uint256) {
        return _tokenIdCounter.current() + devTokenCount - 9501;
    }

    function totalWhitelistSupply() public view returns (uint256) {
        return _tokenIdWhitelistCounter.current();
    }
