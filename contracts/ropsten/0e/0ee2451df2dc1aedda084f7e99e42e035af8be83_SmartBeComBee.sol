...
contract SmartBeComBee {
    ...
    function registration(address referrerAddress, address tknAddress) external payable {
        registrationbase(msg.sender, referrerAddress, tknAddress);
    }
    
    function registrationbase(address userAddress, address referrerAddress, address tknAddress) private {
    ...
    function buyLevel(uint8 matrix, uint8 level, address tknAddress) external payable {
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if (tokenAddress != 0x0000000000000000000000000000000000000000){
            ...
            if (!address(uint160(receiver)).send(levelCost[level]*1e13)) {
