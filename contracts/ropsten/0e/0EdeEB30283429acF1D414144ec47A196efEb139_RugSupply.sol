  ...
  ERC721EnumerableUpgradeable
{
    ...
    if (total > 0) {
      if (referrer != address(0) && overridePrice == 0) {
        uint256 txRefReward = referrals[referrer].refRewards != 0
          ? referrals[referrer].refRewards * 10**6
          : globalRefRewards * 10**6;
  ...
  function viewComputePriceAfterDiscount(
    address to,
    address referrer,
    uint256 currentPrice
  ) public view returns (uint256) {
    uint256 tmpCredit = credits[to];
    uint256 txRefDiscount = referrals[referrer].refRewards != 0
  ...
  function computePriceAfterDiscount(
    ...
    currentPrice *= (100 - txRefDiscount) / 100;
    return currentPrice;
  }
  ...
  /**
   * @notice Set the custom values for a referee, _newDiscount must be between 0 and 100
   * @param _newRewards How much the referer will receive per referral in $
   * @param _newDiscount % discount that the referee will receive on the order
