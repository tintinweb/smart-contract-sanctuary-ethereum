...
contract Retract is Ownable {
    ...
    function claim(
        ...
        uint256 realAmount = amount.sub(claimed[token][user]);
        IERC20(token).safeTransferFrom(vault, user, realAmount);
        claimed[token][user] = claimed[token][user].add(realAmount);
