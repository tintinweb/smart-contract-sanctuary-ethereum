...
contract MaramaoFeeManager is Ownable, IFeeRecipient {
	using SafeERC20 for IERC20;

	struct PoolInfo {
		IERC20 stakingToken;
		uint256 stakingTokenTotalAmount;
		uint256 conventionThreshold;
		uint256 conventionDiscount;
	}
	...
	event FundsTransferred(address indexed governanceTreasury, bool native, address token, uint256 amount);
	...
	constructor(address _governanceTreasury, address _licenseManager) {
	...
	function receiveZeroExFeeCallback(
		...
		if (discount > 0) {
			uint256 rebateAmount = (amount * discount) / 100;
	...
	function sendFundsToGovernanceTreasury(address token, bool native) external {
		uint256 balance;
		if (native) {
			balance = address(this).balance;
			payable(governanceTreasury).transfer(balance);
		} else {
			balance = IERC20(token).balanceOf(address(this));
			IERC20(token).safeTransfer(governanceTreasury, balance);
		}
		emit FundsTransferred(governanceTreasury, native, token, balance);
	}
