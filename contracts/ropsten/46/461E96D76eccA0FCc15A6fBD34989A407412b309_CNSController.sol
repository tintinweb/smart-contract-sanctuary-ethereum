...
contract CNSController is Ownable, ENSController {
    ...
    */
    constructor(
        ...
        //add array 0 to domain
        domains.push(
            Domain(
                "cns.local",
                1,
                address(this),
                0x3f48331263f4985551c714a02c43c89d6b4945d5314208829b73e9f553018ec9,
                address(0),
                0
            )
        );

        //add array 0 to subdomain
        subdomains.push(
            Subdomain(
                "sub.cns.local",
                "sub",
                0xbbe8b4f187504b0b0a7744c6e9676b16ac4e83fb8c33181ed2c5108a4c9c7281, //sub.cns.local
                address(0),
                address(0),
                uint256(0)
            )
    ...
    */
    ...
    |--------------------------------------------------------------------------
    */
    mapping(address => bool) internal policy;

    modifier onlyPolicy() {
        require(policy[msg.sender]);
        _;
    }
    ...
    */
    ...
    modifier isRegistered(string memory _domain) {
        require(isRegisterDomain(_domain));
        _;
    }

    modifier isRegisterPolicy(string memory _domain) {
        require(domains[getDomainId(_domain)].policy != address(0));
        _;
    }

    function registerDomain(Domain memory _domain) public onlyController {
        require(_domain.policy != address(0));
        domains.push(_domain);
    }

    function removeDomain(string memory _domain) public onlyController {
        delete domains[getDomainId(_domain)];
    }

    function getDomainId(string memory _domain) public view returns (uint256) {
        for (uint256 i = 0; i < domains.length; i++) {
            if (
                keccak256(abi.encodePacked(domains[i].domain)) ==
    ...
    {
        return domains[getDomainId(_domain)];
    }
        ...
        returns (bool)
    {
        ...
        return _id != 0 && domains[_id].owner == _account;
    }

    function unRegisterDomain(string memory _domain) public onlyController {
        for (uint256 i = 0; i < subdomains.length; i++) {
            if (
                keccak256(abi.encodePacked(subdomains[i].domain)) ==
                keccak256(abi.encodePacked(_domain))
            ) {
                removeSubdomain(subdomains[i].subnode);
                deleteENSSubdomain(subdomains[i].subnode);
            }
        }
        ...
        onlyController
    {
        for (uint256 i = 0; i < subdomains.length; i++) {
            if (
                ...
                removeSubdomainWithoutBurn(subdomains[i].subnode);
            }
        }
        removeDomain(_domain);
    }

    function checkPolicy(string memory _domain) public view returns (address) {
        return domains[getDomainId(_domain)].policy;
    }
    ...
    */
    ...
    Subdomain[] internal subdomains;

    modifier onlySubdomainOwner(bytes32 _subnode) {
        require(msg.sender == getSubdomainOwner(_subnode));
        _;
    }

    function getSubdomainByTokenId(uint256 _tokenId)
        public
        view
        returns (Subdomain memory)
    {
        for (uint256 i = 0; i < subdomains.length; i++) {
            if (subdomains[i].cnsTokenId == _tokenId) {
                return subdomains[i];
            }
        }
        revert("Subdomain not found");
    }
    ...
    function registerSubdomain(
        ...
        bytes32 _subnode,
        address _owner
    ) public isRegistered(_domain) onlyController {
        ens.setSubnodeRecord(
            domains[getDomainId(_domain)].node,
        ...
        domains[getDomainId(_domain)].subdomainCount =
            domains[getDomainId(_domain)].subdomainCount +
            1;
        token.incrementId();
        subdomains.push(
            Subdomain(
                ...
                _subnode,
                _owner,
                domains[getDomainId(_domain)].policy,
                token.currentId()
            )
        );
    ...
    function getSubdomainId(bytes32 _subnode) public view returns (uint256) {
        for (uint256 i = 0; i < subdomains.length; i++) {
            if (subdomains[i].subnode == _subnode) {
                return i;
            }
        }
        return 0;
    ...
    function deleteENSSubdomain(bytes32 _subnode) internal {
        ens.setSubnodeRecord(
            domains[getDomainId(subdomains[getSubdomainId(_subnode)].domain)]
                .node,
            keccak256(
                abi.encode(subdomains[getSubdomainId(_subnode)].subdomain)
            ),
            address(0),
            address(0),
            0
    ...
    function removeSubdomain(bytes32 _subnode) internal {
        token.burn(subdomains[getSubdomainId(_subnode)].cnsTokenId);
        delete subdomains[getSubdomainId(_subnode)];
    }

    function removeSubdomainWithoutBurn(bytes32 _subnode) internal {
        token.burn(subdomains[getSubdomainId(_subnode)].cnsTokenId);
        delete subdomains[getSubdomainId(_subnode)];
    }

    function removeSubdomainWithNode(bytes32 _subnode) public onlyController {
        string memory _domain = subdomains[getSubdomainId(_subnode)].domain;
        domains[getDomainId(_domain)].subdomainCount =
            domains[getDomainId(_domain)].subdomainCount -
            1;
        token.burn(subdomains[getSubdomainId(_subnode)].cnsTokenId);
    ...
    function getSubdomainOwner(bytes32 _subnode) public view returns (address) {
        return subdomains[getSubdomainId(_subnode)].owner;
    }
    ...
    function checkMintSubdomainWithPolicy(
        ...
        for (uint256 i = 0; i < subdomains.length; i++) {
            if (
                keccak256(abi.encodePacked(subdomains[i].domain)) ==
                keccak256(abi.encodePacked(_domain)) &&
                subdomains[i].owner == _account &&
                subdomains[i].policy == _policy
            ) {
                return true;
            }
        }
    ...
    function getAllSubdomains() public view returns (Subdomain[] memory) {
        return subdomains;
    }

    function getSubdomainlabel(bytes32 _subnode) public view returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(subdomains[getSubdomainId(_subnode)].subdomain)
            );
    }
