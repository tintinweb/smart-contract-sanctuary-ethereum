...
contract CSMCSPTEST1 is ERC721A_CS, Ownable, Pausable, PullPayment {
    ...
    string[MAX_CUSTOM] public customTokenURI; // URIs for the custom tokens
    string public launchTokenURI; // URI for the launch tokens
    string public hiddenTokenURI; // URI for the pre-reveal place holder
    bool public collectionHidden = true; // If the collection is hidden

    constructor() ERC721A_CS("CsmcSpTestv1", "CSMCSPTEST1") {
        _pause();
        for (uint256 i = 0; i < MAX_CUSTOM; i++) {
            customTokenURI[i] = "";
        }
        launchTokenURI = "";
        hiddenTokenURI = "ar://AR_LINK/hidden.json";
    }

    // Mint tokens (user)
    function mintUser(uint256 quantity) public payable whenNotPaused {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory baseURI = tokenId > tokensLaunch ? customTokenURI[tokenId-tokensLaunch-1] : launchTokenURI;
    ...
    // <--<--<--<--<--<--<--<--<-- OnlyOwner -->-->-->-->-->-->-->-->-->-->
    // Mint tokens (owner)
    function mintOwner(address recipient, uint256 tokenId) public onlyOwner {
        ...
        _safeMintToken(recipient, tokenId);
    }

    // Total number of tokens
    function setNumTokensTotal(uint256 newSize) public onlyOwner {
        tokensTotal = newSize;
    }

    // Number of launch tokens
    function setNumTokensLaunch(uint256 newSize) public onlyOwner {
        tokensLaunch = newSize;
    }

    // Number of base tokens
    function setNumTokensBase(uint256 newSize) public onlyOwner {
        tokensBase = newSize;
    }

    // Mint price for tokens, price in wei (1 ether = 10^18 wei = 10^9 GWei)
    function setMintPrice(uint256 quantity, uint256 newPriceWei) public onlyOwner {
        unchecked { require(quantity > 0 && quantity <= MAX_MINT, 'ERC721A_CS: invalid quantity'); }
        mintPrice[quantity-1] = newPriceWei;
    }

    // Maximum number of tokens per wallet (should be a multiple of MAX_MINT)
    function setWalletMax(uint256 newWalletMax) public onlyOwner {
        walletMax = newWalletMax;
    }

    // URIs for the custom tokens [customId = 0->15]
    function setCustomTokenURI(uint256 customId, string memory newURI) public onlyOwner {
        unchecked { require(customId < MAX_CUSTOM, 'ERC721A_CS: invalid customId'); }
        customTokenURI[customId] = newURI;
    }

    // URI for the launch tokens
    function setLaunchTokenURI(string memory newURI) public onlyOwner {
        launchTokenURI = newURI;
    }

    // URI for the pre-reveal place holder
    function setHiddenTokenURI(string memory newURI) public onlyOwner {
        hiddenTokenURI = newURI;
    }
...
// SPDX-License-Identifier: MIT
// Creator: Chiru Labs (modified to ERC721CS for Cosmic Soup to add direct mint of specific non-launch tokens)
...
contract ERC721A_CS is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _safeMintToken(
        address to,
        uint256 tokenId,
        bytes memory _data
    ) internal {
        _mintToken(to, tokenId, _data, true);
    }
