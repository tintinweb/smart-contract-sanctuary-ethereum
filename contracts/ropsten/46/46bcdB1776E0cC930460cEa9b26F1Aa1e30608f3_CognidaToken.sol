...
// File: contracts/CognidaToken.sol

contract ApproveAndCallFallBack {
    function receiveApproval(address from, uint256 _amount, address _token, bytes _data) public;
}

contract CognidaToken is Ownable, Claimable, StandardToken, BurnableToken {
    event Transfer(address indexed from, address indexed to, uint value, bytes data);
    event RestrictionSet(address recipient, uint256 expiration);
    event Multisended();

    string public name = "Cognida Token";
    string public symbol = "COG";
    uint8 public decimals = 18;
    address public cognidaAddress;
    uint256 public multiSendLimit = 150;

    mapping(address => uint256) public restrictedAddresses;
    ...
    modifier onlyUnrestricted(address _from, address _to) {
        if(restrictedAddresses[_from] > currentTime()){
            require(_to == cognidaAddress, "recipient is not congnidaAddress");
        }
        _;
    }
    ...
    constructor(address _cognidaAddress) public {
        require(_cognidaAddress != address(0), "congidaAddress cannot be empty");
        // 550,000,000 tokens
        totalSupply_ = 10 ** 18 * 550000000;
        balances[msg.sender] = totalSupply();
        cognidaAddress = _cognidaAddress;
    }
    ...
    function superTransfer(address _to, uint256 _value) 
        internal 
        onlyUnrestricted(msg.sender, _to)
        validRecipient(_to)
    ...
    function transferFrom(
        ...
        returns (bool)
    {
        require(superTransferFrom(_from, _to, _value), "transferFrom failed");
        if (isContract(_to)) {
    ...
    function transfer(address _to, uint256 _value) public returns (bool)
    {
        require(superTransfer(_to, _value), "transfer failed");
        if (isContract(_to)) {
            contractFallback(msg.sender, _to, _value, new bytes(0));
        }
        return true;
    }
    ...
    function setRestriction(address _recipient, uint256 _expirationDate)
        external 
        onlyOwner 
        validRecipient(_recipient) 
    {
        require(_recipient != cognidaAddress, "cannot be cognidaAddress");
        require(currentTime() < _expirationDate, "cannot set expiration in the past");
        restrictedAddresses[_recipient] = _expirationDate;
        emit RestrictionSet(_recipient, _expirationDate);
    }
