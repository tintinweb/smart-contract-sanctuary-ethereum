...
contract DividendTracker is IERC20, Context, Ownable {
    ...
    string private _name = "TSUNAMI TRACKER";
    string private _symbol = "TSUNAMI_T";
    uint8 private _decimals = 9;
    ...
    function setTokenBalance(address account) public {
        uint256 balance = tsunami.balanceOf(account);
        if (!tsunami.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        tsunami = TSUNAMI(payable(contractAddr));
    }
