 ...
 */
contract ERC1155BridgeToken is ERC1155, Ownable, IBurnableMintableERC1155Token {
    using Strings for uint256;

    // Base URI
    string private baseURI;
    address public mediatorContract;

    // Optional mapping for token URIs
    mapping(uint256 => string) private tokenURIs;

    constructor(address _mediatorContract, string memory _baseURI) {
     ...
     */
    function burn(uint256[] memory _tokenIds, uint256[] memory _values) external override onlyMediator {
     ...
     * @param _mediatorContract address of the new bridge contract.
     */
    function setMediatorContract(address _mediatorContract) external onlyOwner {
        require(_mediatorContract != address(0));
        mediatorContract = _mediatorContract;
    }
