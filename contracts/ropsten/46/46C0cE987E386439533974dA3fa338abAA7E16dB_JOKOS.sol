...
contract JOKOS is ERC1155, Ownable, ERC1155Burnable {
    using Counters for Counters.Counter;

    uint256 public constant MAX_JOKOS = 10000;
    uint256 public constant MAX_JOKOS_PER_TX = 20;
    uint256 public constant PRICE = 0.001 ether;

    uint256 public presaleMaxPerWallet = 4;

    address public constant creator1Address =
        0x62FD1b3B4F6AaAEd8F206547dCcfE995AD0856f3;
    address public constant creator2Address =
        0xB203627d91C07B7C07474d95d9E80dc5aD8a0048;
    ...
    modifier saleIsOpen() {
        require(totalSupply() <= MAX_JOKOS, "Soldout!");
    ...
    modifier verify(address _buyer, bytes memory _sign) {
        ...
        bytes32 _hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encodePacked("jokosnft", addressBytes))
            )
        );
    ...
    function _mintJoko(address _to, uint256 _tokenId) private {
        ...
        verify(msg.sender, _sign)
    {
        ...
        require(_numberOfTokens > 0, "You can't mint 0 Joko");
        require(
        ...
        require(
            total + _numberOfTokens <= MAX_JOKOS,
            "Purchase would exceed max supply of JOKOS"
        );
    ...
    function mintSale(uint256 _numberOfTokens) public payable saleIsOpen {
        ...
        require(_numberOfTokens > 0, "You can't mint 0 Joko");
        require(
            _numberOfTokens <= MAX_JOKOS_PER_TX,
            "You can't mint more than the allowed amount"
        );
        require(
            total + _numberOfTokens <= MAX_JOKOS,
    ...
    function reserveJoko(uint256 _numberOfTokens) public onlyOwner {
        uint256 total = totalSupply();
        require(total + _numberOfTokens <= MAX_JOKOS);
        for (uint8 i = 0; i < _numberOfTokens; i++) {
            uint256 tokenToMint = totalSupply();
            _mintJoko(owner(), tokenToMint);
        }
    }
    ...
    function withdrawAll() public onlyOwner {
        ...
        require(balance > 0);
        _widthdraw(creator2Address, (balance * 30) / 100);
