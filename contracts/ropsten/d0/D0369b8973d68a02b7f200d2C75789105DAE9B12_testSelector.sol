...
contract testSelector {
    ...
    //     return abi.encode(_swapParams);
    // }

    // function abiDecode (
    //     bytes memory data
    // ) public view returns (uint) {
    //     SwapParams[] memory _swapParams = abi.decode(data, (SwapParams[]));
    //     return _swapParams[0].dexId;
    // }
    // address owner;
    // uint256 deadline;
    // mapping (bytes4 => address) internal selectAddress;
    ...
    // ) public onlyOwner {
    //     _trade(_selectorOne, _fromToken, _amount, _dataOne);
    // }
    ...
    //     bytes memory _dataOne
    // ) internal {
    //     address routerAddress = getSelectAddress(_selectorOne);
    //     IERC20 fromIERC20 = IERC20(_fromToken);
    //     fromIERC20.approve(routerAddress, _amount);

    //     (bool success, ) = routerAddress.delegatecall(_dataOne); 
