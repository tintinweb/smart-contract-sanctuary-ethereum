...
contract Shintaro1155 is ERC1155Supply, Ownable {
  struct Config {
    uint16 MAX_SUPPLY;
    uint16 TEAM_SUPPLY;
    uint256 PRICE;
  ...
  string public hashKey = "japan_tokens_^.^";
  address public signer = 0xe24A0e2A3EbB52A5df69DA55805a7ffb27f99cE8;
  ...
  constructor() ERC1155("") {
    ...
    tokens[uint8(TOKEN.G)].config = Config({ MAX_SUPPLY: 444, TEAM_SUPPLY: 0, PRICE: 0.1 ether });
  ...
  function claim(TOKEN _tokenId, uint16 _maxAmount, bytes calldata _signature) external payable {
    ...
    require(tokens[tokenId].config.MAX_SUPPLY > totalSupply(tokenId) + tokens[tokenId].config.TEAM_SUPPLY, "Exceed max");
  ...
  function eligibleByPrerequisite(uint8 _tokenId, uint16, address _account, bytes memory, uint8) internal view returns (bool) {
    uint256 preTokenBalance = IDRPM(0xaeEcA4B8084227F35346e78FEf2d04A4C88788AB).balanceOf(_account, 0) +
      IDRPM(0xaeEcA4B8084227F35346e78FEf2d04A4C88788AB).balanceOf(_account, 1);
