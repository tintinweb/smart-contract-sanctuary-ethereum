...
contract MoonLockVesting is ReentrancyGuard, Ownable{    
    ...
    function createLock(address _tokenAddress, address[] calldata _withdrawAddress, uint[] calldata _depositAmount, uint[] calldata _endDate,  uint[] calldata _startDate) external payable returns(bool){
        ...
        iDEXRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value}(
            0,
            path,
            address(this),
            block.timestamp + 30
        ...
        tokenToBurn.safeTransfer(0x000000000000000000000000000000000000dEaD, tokenToBurn.balanceOf(address(this)));
    ...
    // Claim tokens stuck in the contract
    function claimStuckTokens(address _tokenAddress, uint _amount) external onlyOwner{
        IERC20(_tokenAddress).safeTransfer(msg.sender, _amount);
    }

    // Delete vesting instance 
    function deleteVestingInstance(uint _index) external onlyOwner{
