  ...
  IERC721Enumerable
{
    ...
    returns (string memory)
  {
    ...
    //see the tokens in goldlist
    function tokensGoldlist() external virtual override view returns(uint) {}
...
contract nft is ERC721A, Ownable {
    ...
    uint private lastGoldlistId;
    uint private lastWhitelistId;
    
    //declaring initial values for variables
    constructor() ERC721A("Wing Wars", "WW", 10, 10000){
        unrevealedURI = "ipfs://.../";
    }
    ...
    */
    ...
    function goldlistMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        State saleState_ = saleState();
        require(saleState_ == State.GoldlistSale, "Sale in not open!");
    ...
    function reservedMints(uint256 number, address recipient) public onlyOwner {
        ...
        State saleState_ = saleState();
        if (saleState_ == State.NoSale || saleState_ == State.GoldlistSale) {
            lastGoldlistId = totalSupply();
        }
    ...
    function switchToGoldlistSale() public onlyOwner{
        State saleState_ = saleState();
        require(saleState_ == State.NoSale, "Current State must be No Sale!");
        goldlistSaleLaunchTime_ = block.timestamp;
    }
    
    //begins the whitelist sale
    function switchToWhitelistSale() public onlyOwner {
        State saleState_ = saleState();
        require(saleState_ == State.GoldlistSale, "Current State must be Goldlist Sale!");
    ...
    function saleState() public view returns(State){
        if (goldlistSaleLaunchTime_ == 0) {
            return State.NoSale;
        }
        else if (whitelistSaleLaunchTime_ == 0) {
            return State.GoldlistSale;
        }
    ...
    //see the tokens in goldlist
    function tokensGoldlist() external override view returns(uint) {
        return lastGoldlistId;
    }
    ...
    function goldlistSaleLaunchTime() external view returns(uint256) {
        return goldlistSaleLaunchTime_;
    }
