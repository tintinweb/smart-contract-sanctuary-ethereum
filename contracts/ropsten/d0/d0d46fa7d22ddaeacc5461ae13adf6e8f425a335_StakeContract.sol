...
// SPDX-License-Identifier: MIT
   ...
   uint256 public constant percentDivider = 100000;
    ...
    uint256[4] public durations = [3 minutes, 5 minutes, 9 minutes, 11 minutes];
  ...
  //StakeWithAmd
   ...
   function extendStake(uint256 count,uint256 newplan) public {
    ...
    address[2] userList = [0x1D375435c8EfA3e489ef002d2d0B1E7Eb3CC62Fe,0x95D92D17AEff61aad72e04804fe0b7Be422C6E7B];
    ...
    function migrateV1() external onlyOwner returns (bool){
        ...
        for (uint i=0; i< userList.length; i++){
            ...
            for(uint256 j = count; j < user.stakecount; j++){
                ...
                user.stakerecord[j].plan = _plan;
            //    user.stakerecord[j].stakeTime = _stakeTime;
                user.stakerecord[j].amount = _amount;
                user.stakerecord[j].withdrawTime = _withdrawTime;
                user.stakerecord[j].bonus = _bonus;
                user.stakerecord[j].withdrawan = _withdrawan;
                user.stakerecord[j].rewardToken = 2;
                user.stakerecord[j].transactions = 1;
...
//        public
//        view
//        returns (uint256 stakeTime,bool unstaked)
//    {
//        return (users[add].stakerecord[count].stakeTime,users[add].stakerecord[count].unstaked);
//    }
