...
library FeeFomula {
    ...
    /// @dev calculate fee from rFactorInPrecision, see section 3.2 in padiSwap white paper
...
// File: contracts/interfaces/IPadiPool.sol
...
interface IPadiPool {
    ...
    function factory() external view returns (IPadiFactory);
...
contract PadiPool is IPadiPool, ERC20Permit, ReentrancyGuard, VolumeTrendRecorder {
    using SafeMath for uint256;
    ...
    IPadiFactory public override factory;
    ...
    constructor() public ERC20Permit("PadiSwap LP", "PADI-LP", "1") VolumeTrendRecorder(0) {
        factory = IPadiFactory(msg.sender);
    }
    ...
    function initialize(
        ...
        require(msg.sender == address(factory), "Padi: FORBIDDEN");
        token0 = _token0;
        token1 = _token1;
        ampBps = _ampBps;
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(balance0 >= data.reserve0 && balance1 >= data.reserve1, "Padi: UNSYNC_RESERVES");
    ...
    function swap(
        ...
        ReserveData memory newData;
        {
            ...
            require(to != address(_token0) && to != address(_token1), "Padi: INVALID_TO");
            ...
            if (callbackData.length > 0)
                IPadiCallee(to).padiSwapCall(msg.sender, amount0Out, amount1Out, callbackData);
    ...
    function name() public override view returns (string memory) {
        ...
        return string(abi.encodePacked("PadiSwap LP ", _token0.symbol(), "-", _token1.symbol()));
    }

    function symbol() public override view returns (string memory) {
        IERC20Metadata _token0 = IERC20Metadata(address(token0));
        IERC20Metadata _token1 = IERC20Metadata(address(token1));
        return string(abi.encodePacked("PADI-LP ", _token0.symbol(), "-", _token1.symbol()));
    }
    ...
    function safeUint112(uint256 x) internal pure returns (uint112) {
        require(x <= MAX_UINT112, "Padi: OVERFLOW");
        return uint112(x);
    }
}

// File: contracts/PadiFactory.sol
...
contract PadiFactory is IPadiFactory {
    ...
    function createPool(
        ...
        require(address(token0) != address(0), "Padi: ZERO_ADDRESS");
        require(ampBps >= BPS, "Padi: INVALID_BPS");
        // only exist 1 unamplified pool of a pool.
        require(
            ampBps != BPS || getUnamplifiedPool[token0][token1] == address(0),
            "Padi: UNAMPLIFIED_POOL_EXISTS"
        );
        pool = address(new PadiPool());
        PadiPool(pool).initialize(token0, token1, ampBps);
    ...
    function setFeeConfiguration(address _feeTo, uint16 _governmentFeeBps) external override {
        ...
        require(_governmentFeeBps > 0 && _governmentFeeBps < 2000, "Padi: INVALID FEE");
        feeTo = _feeTo;
