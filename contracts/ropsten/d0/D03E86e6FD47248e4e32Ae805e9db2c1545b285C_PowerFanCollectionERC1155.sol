...
contract PowerFanCollectionERC1155 is ERC1155Tradable {
    ...
    OwnableUpgradeable
{
    ...
    address payable public rootOwner;

    /* All token ids */
    uint256[] private allTokenIds;
    string[] public nftUUIDArray;
    ...
    // Contract URI https://docs.opensea.io/docs/contract-level-metadata
    ...
    /**
    * @dev Require smart contract should not be paused
    */
    modifier notPaused() {
        require(!paused, "ERC1155Tradable:PAUSED");
        _;
    }
    ...
    /**
     * @dev Require _msgSender() to be the rootOwner of PowerFan Factory
     */
    modifier rootOnly() {
        require(
            rootOwner == _msgSender() || rootOwner == tx.origin,
            "ERC1155Tradable#creatorOnly: ONLY_ROOT_ALLOWED"
        );
        _;
    }
    ...
    function initialize_erc1155(
        address payable _rootOwner,
        address creator,
        string memory _name,
        string memory _symbol,
        string memory _uri
    ...
    // buyerAddress is msg.sender
  function mintTokenAndPurchase (address payable authorAddress,
                                 string memory _UUID,
                                 string memory tokenURI) public payable {


    // Mint the nft to new owner
    require(msg.value > 0, "Insufficient funds to buy");

    uint feePercent = uint(25);
    uint256 powerFee = (msg.value/1000).mul(feePercent);

    //send the fee to the DAO
    rootOwner.transfer(powerFee);

    uint256 authorPayment = msg.value.sub(powerFee);
    authorAddress.transfer(authorPayment);
    //check if NFT exists
    //require(nftUUIDArray[_UUID], "NFT exists");
    
    nftUUIDArray.push(_UUID);
    uint256 _id = nftUUIDArray.length;

    allTokenIds.push(_id);

    uint256 tokenId = _id;
    //set uri
    customUri[_id] = tokenURI;
    _mint(msg.sender, tokenId, 1, "0x");
  }

    function mintPowerNFT(POWERFAN_NFT calldata nft, bytes memory signature)
        public
        payable
    {
        address signer = _verify(nft, signature);
        require(msg.value >= nft.minPrice, "Insufficient funds to buy");
        uint256 powerFee = nft.minPrice.mul(25).div(1000);

        artists[nft.tokenId] = signer;
        artistsNFTs[signer].push(nft.tokenId);

        _mint(msg.sender, nft.tokenId, 1, "0x");
        customUri[nft.tokenId] = nft.uri;
        payable(rootOwner).transfer(powerFee);
        require(
            payable(signer).send(nft.minPrice.sub(powerFee)),
    ...
    function getNFTByArtist(address artist)
        public
        view
        returns (uint256[] memory)
    {
        return artistsNFTs[artist];
    }
     ...
     */
    function setURI(string memory _newURI) public notPaused onlyOwner {
        _setURI(_newURI);
    }
    ...
    /**
     * @dev Get contract metadata uri for opensea compatibility
     */
    ...
    /**
     * @dev Set contract metadata uri for opensea compatibility
     */
     ...
     * NOTE: If supply is only 1, treat it as NFT.
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) public virtual notPaused creatorOnly(_id) {
     ...
     */
    function setCreator(address _to, uint256[] memory _ids) public notPaused {
        require(
            _to != address(0),
    ...
    /**
     * @dev Only Factory owner can pause the smart contract for security reasons,
     */
    function setPaused() public rootOnly {
        paused = true;
    }

    /**
     * @dev Only Factory owner can unpause the smart contract for security reasons,
     */
    function setUnpaused() public rootOnly {
        paused = false;
    }
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "POWERFAN_NFT(uint256 tokenId,uint256 minPrice,string uri)"
                        ),
                        nft.tokenId,
                        nft.minPrice,
    ...
    function _verify(POWERFAN_NFT calldata nft, bytes memory signature)
        internal
        view
        returns (address)
    {
