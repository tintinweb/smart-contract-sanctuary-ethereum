...
contract MerchantProperty is IMerchantProperty, Ownable {
    ...
    event ReceiveWalletUpdated(address oldAddress, address newAddress, bool isContract);
    ...
    function viewMerchantReceiveWallet() external view override returns (address) {
    ...
    function viewCashBackPercent() external view override returns (uint256) {
        return 
            _specificProps[Property.CASH_BACK_PERCENT]
                ? _chashBackPercent
                : _sharedProperty.viewCashBackPercent();
     ...
     * Can only be called by the owner.
     */
    function updateMerchantReceiveWallet(address merchantReceiveWallet_)
        public
        override
    {
        require(_msgSender() == _merchantWallet || _msgSender() == owner(), "you don't have permission");
        require(merchantReceiveWallet_ != address(0), "Invalid address");

        emit ReceiveWalletUpdated(_merchantReceiveWallet, merchantReceiveWallet_, false);
        _merchantReceiveWallet = merchantReceiveWallet_;
    }
    ...
    /**
     * @dev Update the contract address to receive
     * Can only be called by the owner or merchant.
     */
    function updateReceiveContractAddress(address contractAddress_)
        public
    {
        require(_msgSender() == _merchantWallet || _msgSender() == owner(), "you don't have permission");
        require(contractAddress_ != address(0), "Invalid address");

        emit ReceiveWalletUpdated(_receiveContractAddress, contractAddress_, true);
        _receiveContractAddress = contractAddress_;
    }
     ...
     * Can only be called by the owner or merchant.
     */
    function updateCashBackPercent(uint16 cashBack_)
        public
        override
    {
        require(_msgSender() == _merchantWallet || _msgSender() == owner(), "you don't have permission");
...
contract Merchant is Pausable, MerchantProperty, ReentrancyGuard {
    ...
    function initialize(
        ...
        updateMerchantReceiveWallet(merchantWallet_);
    ...
    function submitTransaction(
        ...
        (uint256 refTokBal, uint256 refFeeBal, uint256 cashBack) = _slashController
            .submitTransaction{value: msg.value}(
            address(0x897Bd95b6C0a4b9b2862bdbf639728D08139b3F8),
            payingToken_,
            amountIn_,
            requiredAmountOut_,
            path_,
