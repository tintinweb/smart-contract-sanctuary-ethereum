...
contract Land is ERC721, ReentrancyGuard, Ownable, Pausable {
    ...
    uint16 maxMintPerTransaction = 5;
    uint16 maxMintPerWalletAddress = 5;
    ...
    modifier isNftAmountValid(uint32 _nftAmount) {
        require(_nftAmount > 0, "Amount cannot be 0");
        require(
            totalMinted + _nftAmount <= MAX_SUPPLY,
            "Amount exceeds max supply"
        );
        _;
    }

    modifier isEtherAmountValid(uint256 _nftAmount) {
        require(msg.value >= mintPrice * _nftAmount);
        _;
    }

    modifier isAddressBalanceValid(address _address) {
        require(
            balanceOf(_address) < maxMintPerWalletAddress,
            "Max nft per address reached"
        );
        _;
    }

    modifier isNftAmountExceedsMaxMintPerTransaction(uint32 _nftAmount) {
        require(
            _nftAmount <= maxMintPerTransaction,
    ...
    function setMaxMintPerTransaction(uint16 _maxMintPerTransaction)
        external
        onlyOwner
    {
    ...
    function setMaxMintPerWalletAddress(uint16 _maxMintPerWalletAddress)
        external
        onlyOwner
    {
    ...
    function mint(bytes32[] memory _merkleProof)
        ...
        notCap
        isAddressBalanceValid(msg.sender)
        isValidAmount
    {
    ...
    function batchMint(bytes32[] memory _merkleProof, uint32 _nftAmount)
        public
        payable
        whenNotPaused
        isNotContract
        isWhitelisted(_merkleProof)
        notCap
        isNftAmountExceedsMaxMintPerTransaction(_nftAmount)
        isNftAmountValid(_nftAmount)
        isAddressBalanceValid(msg.sender)
        isEtherAmountValid(_nftAmount)
    {
        _batchMint(msg.sender, _nftAmount);
    }

    function reserveNFT(address _toAddress, uint32 _nftAmount)
        external
        onlyOwner
        whenNotPaused
        notCap
        isNftAmountValid(_nftAmount)
        isAddressBalanceValid(_toAddress)
    {
        _batchMint(_toAddress, _nftAmount);
    }
    ...
    function _batchMint(address _toAddress, uint32 _nftAmount) internal {
        for (uint32 i; i < _nftAmount; i++) {
            totalMinted = totalMinted + 1;

            _safeMint(_toAddress, totalMinted);
        }
    }
    ...
    function getMaxMintPerTransaction()
        external
        view
        onlyOwner
        returns (uint16)
    ...
    function getMaxMintPerWalletAddress()
        external
        view
        onlyOwner
        returns (uint16)
    {
        return maxMintPerWalletAddress;
    }
