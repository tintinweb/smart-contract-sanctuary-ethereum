...
contract Eboogotchi is Ownable {
    ...
    function getHungry() public view returns (uint256) {
        return hungry + ((block.number - feedBlock) / speed);
    }

    function getDirty() public view returns (uint256) {
        return dirty + ((block.number - cleanBlock) / speed);
    }

    function getBored() public view returns (uint256) {
        return bored + ((block.number - playBlock) / speed);
    }
    ...
    function setSpeed(uint256 _speed) public onlyOwner {
        require(_speed > 0, "speed needs to be strictly positive");
        speed = _speed;
    }
}
