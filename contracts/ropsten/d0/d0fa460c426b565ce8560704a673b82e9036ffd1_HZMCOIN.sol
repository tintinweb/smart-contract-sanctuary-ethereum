...
contract HZMCOIN is Ownable,IERC20 {
    ...
    uint256 public lockTime = block.timestamp; // show unlock time
    ...
    // Only owner can lock 
    function TimeLockedWallet(uint256 _timeInHour ) public onlyOwner {
        lockTime = _timeInHour * 1 hours;
    }
    ...
    function transfer(address _to, uint256 _amount) public override returns (bool success) {
        ...
        require(locked[msg.sender] <= block.timestamp,string(abi.encodePacked("The wallet is locked for ",Strings.toString(locked[msg.sender])," seconds")));     // check unlock time
        locked[msg.sender] = block.timestamp + lockTime;
    ...
    function transferFrom(address _spenderOwner, address _recipient, uint256 _amount) public override returns (bool) {
        require(!frozenAccount[_msgSender()],"Your account is frozen account"); // Check for frozen account
        require(!frozenAccount[_spenderOwner] ,"_spenderOwner's account is frozen account"); // Check for frozen account
        ...
        require(locked[_spenderOwner] <= block.timestamp,string(abi.encodePacked("The wallet is locked for ",Strings.toString(locked[_spenderOwner])," seconds")));  // check unlock time
      
        locked[_spenderOwner] = block.timestamp + lockTime;
