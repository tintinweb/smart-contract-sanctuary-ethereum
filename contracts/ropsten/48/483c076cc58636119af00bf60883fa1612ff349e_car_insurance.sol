...
contract car_insurance{
    ...
    event addCar_log( uint80 license, address sourceAddr, uint fee, string msg);
    event compensation_log ( uint80 license, address destAddr, uint compensation);
    event vote_log ( uint80 license, uint voteYES, uint voteNO, string msg);
     ...
     //store new info into data
         if(msg.value>=1 ether){
            if(cars[_license].license ==0){ //check if it is existing in the data
                ...
                emit addCar_log (_license, msg.sender, msg.value, "SUCCESSFUL");
            }
            else{
             ...
             emit addCar_log (_license, msg.sender, msg.value, "FAILED");
         }
    }
       ...
       {    current_state = "No vote";
            ...
            {   compensation=2*cars[voting_license].total_fee;
                if(compensation<=address(this).balance)
                {
                    ...
                    emit vote_log ( voting_license, count_yes, count_no, "voting passed, compensated");
                    emit compensation_log ( voting_license, cars[voting_license].account_address , compensation);
                }
                else
                {
                    cars[voting_license].account_address.transfer(address(this).balance);
                    emit vote_log ( voting_license, count_yes, count_no, "voting passed, compensated");
                    emit compensation_log ( voting_license, cars[voting_license].account_address , address(this).balance);
                }
            ...
            emit vote_log ( voting_license, count_yes, count_no, "majority failed");
            voting_license=0;
            count_yes=0;
            count_no=0;
            count_vote=0;
