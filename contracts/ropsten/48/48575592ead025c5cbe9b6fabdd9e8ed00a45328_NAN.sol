...
// File: YLD/interfaces/INANVester.sol


pragma solidity ^0.8.9;

/**
 * @dev NAN token vester interface
 */

interface INANVester {
...
contract StakeRewards is IStakeRewards, Ownable {
  address public ydf;
  IERC721 private sNAN;
  IERC721 private slNAN;
  ...
  modifier onlyToken() {
    require(
      msg.sender == address(sNAN) || msg.sender == address(slNAN),
      'must be stake token'
    );
    _;
  }
  ...
  function getsNAN() external view returns (address) {
    return address(sNAN);
  }

  function getslNAN() external view returns (address) {
    return address(slNAN);
  }
  ...
  function setsNAN(address _sNAN) external onlyOwner {
    sNAN = IERC721(_sNAN);
  }

  function setslNAN(address _slNAN) external onlyOwner {
    slNAN = IERC721(_slNAN);
  }
}
...
/**
 * @dev NAN token interface
 */

interface INAN is IERC20 {
...
contract NANVester is Ownable {
  ...
  function getNAN() external view returns (address) {
    return address(_ydf);
  }
...
// File: YLD/NANStake.sol

/******************************************************************************************************
NANStake Inheritable Contract for staking NFTs
...
contract NANStake is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function _stake(
    ...
    _amountStaked = _amountStaked == 0
      ...
      : _amountStaked;
    _amountNANBaseEarn = _amountNANBaseEarn == 0
      ? _amountStaked
      : _amountNANBaseEarn;
    require(
      _amountStaked > 0 && _amountNANBaseEarn > 0,
    ...
    stakes[_ids.current()] = Stake({
      ...
      amountNANBaseEarn: _amountNANBaseEarn,
    ...
    returns (uint256)
  {
    ...
    return
      (_tokenStake.amountNANBaseEarn * _tokenStake.apr * _secondsStaked) /
      PERCENT_DENOMENATOR /
      ONE_YEAR;
  }
  ...
  function _afterTokenTransfer(
    ...
    if (_from != address(0)) {
      ...
      rewards.setShare(_from, _tokenStake.amountNANBaseEarn, true);
    }
    ...
    if (_to != address(0)) {
      ...
      rewards.setShare(_to, _tokenStake.amountNANBaseEarn, false);
    }
...
/******************************************************************************************************
Staked Yieldification Liquidity (slNAN)
...
contract slNAN is NANStake {
  ...
  function zapAndStakeETHOnly(uint256 _lockOptIndex) external payable {
    ...
    _returnExcessNAN(msg.sender, _ydfBalanceBefore);

    emit ZapETHOnly(msg.sender, msg.value, _lpBalanceToStake);
  }

  function zapAndStakeETHAndNAN(uint256 _amountNAN, uint256 _lockOptIndex)
    external
    payable
  {
    require(msg.value > 0, 'need to provide ETH to zap');
    ...
    ydf.transferFrom(msg.sender, address(this), _amountNAN);
    ...
    _returnExcessETH(msg.sender, _ethBalBefore);
    _returnExcessNAN(msg.sender, _ydfBalBefore);

    emit ZapETHAndNAN(msg.sender, msg.value, _amountNAN, _lpBalanceToStake);
  }

  function zapAndStakeNANOnly(uint256 _amountNAN, uint256 _lockOptIndex)
    external
  {
    require(
      _aprLockOptions[_lockOptIndex].lockTime > 0,
      'cannot zap and stake NAN only without lockup period'
    );
    ...
    emit ZapNANOnly(msg.sender, _amountNAN, _lpBalanceToStake);
  }
  ...
  function _getETHToReceiveOnSellNoSlippage(uint256 _amountNAN)
    internal
    view
    returns (uint256)
  {
    ...
    (uint112 _r0, uint112 _r1, ) = pair.getReserves();
    if (pair.token0() == IUniswapV2Router02(_uniswapRouter).WETH()) {
      return (_amountNAN * _r0) / _r1;
    } else {
      return (_amountNAN * _r1) / _r0;
    }
  }
  ...
  function _returnExcessNAN(address _user, uint256 _initialBal) internal {
...
contract NAN is INAN, ERC20, Ownable {
  ...
  sNAN private _stakedNAN;
  slNAN private _stakedNANLP;
  NANVester private _vester;
  ...
  modifier onlyStake() {
    require(
      address(_stakedNAN) == _msgSender() ||
        address(_stakedNANLP) == _msgSender(),
  ...
  constructor() ERC20('Yieldification', 'NAN') {
    ...
    _vester = new NANVester(address(this));
    ...
    _stakedNAN.setPaymentAddress(msg.sender);
    _stakedNAN.setRoyaltyAddress(msg.sender);
    _stakedNAN.transferOwnership(msg.sender);

    _stakedNANLP = new slNAN(
      uniswapV2Pair,
    ...
    _stakedNANLP.setPaymentAddress(msg.sender);
    _stakedNANLP.setRoyaltyAddress(msg.sender);
    _stakedNANLP.transferOwnership(msg.sender);

    _vester.addStakingContract(address(_stakedNAN));
    _vester.addStakingContract(address(_stakedNANLP));
    _vester.transferOwnership(msg.sender);

    _rewards.setsNAN(address(_stakedNAN));
    _rewards.setslNAN(address(_stakedNANLP));
    ...
    isTaxExcluded[address(_stakedNANLP)] = true; // allow zapping without taxes
  ...
  function _transfer(
    ...
    // handles transferring to a fresh wallet or wallet that hasn't bought NAN before
  ...
  function getStakedNAN() external view returns (address) {
    return address(_stakedNAN);
  }

  function getStakedNANLP() external view returns (address) {
    return address(_stakedNANLP);
  }
