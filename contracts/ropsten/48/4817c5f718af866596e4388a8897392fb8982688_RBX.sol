...
contract RBX is IERC20, Ownable, Pausable {
    ...
    */
    ...
    // Whether a previous call of swapTokens is still in process.
    ...
    constructor () {
        ...
        address _stakingPool = address(0xC01a48cF618F07f16bD834193E0C5aF99cAB2B9E);

        enableTreasuryTax(buyTax_, sellTax_, _taxDecimals, _treasury);
        changeMaxSellAmountPercent(_maxSellAmount);

        // Ethereum: 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48 (Uniswap)
     ...
     *
     * This is private function is equivalent to {transfer}, and can be used to
    ...
    function _beforeTokenTransfer(address sender) private {
        if (_treasuryRewardEnabled && 
            ...
            // uint256 tokensToSwap = getReservePercent(maxSellAmountPercent);
            uint256 tokensToSwap = _minTokensBeforeSwap;
            bool overMinTokensBeforeSwap = _totalTaxCollected >= tokensToSwap && tokensToSwap > 0;
            if (overMinTokensBeforeSwap) {
                swapTokensForUsdc(tokensToSwap, address(this));
    ...
    function _afterTokenTransfer(ValuesFromAmount memory values, bool selling, bool buying) private {    
     ...
     *
     * This is private function is equivalent to `approve`, and can be used to
    ...
    function taxTreasuryDecimals() external view returns (uint8) {
        return _taxTreasuryDecimals;
    }

    function treasuryRewardEnabled() external view returns (bool) {
    ...
    /**
     * @dev Swap `amount` tokens for USDC and send to `to`
     *
     * Emits {Transfer} event. From this contract to the token and USDC Pair.
     */
    function swapTokensForUsdc(uint256 amount, address to) private lock {
        ...
        uint256 currentAllowance = _allowances[address(this)][address(_uniswapV2Router)];
        ...
        // Figure out the exact amount of ETH received from swapping.
        uint256 ethReceived = address(this).balance - initialEthBalance;
        
        // Generate the uniswap pair path of ETH -> USDC
        address[] memory route = new address[](2);
        route[0] = _uniswapV2Router.WETH();
        route[1] = address(usdc);

        // Contract's current USDC balance.
        uint256 initialUsdcBalance = usdc.balanceOf(address(this));

        // Swap ETH to USDC
        _uniswapV2Router.swapExactETHForTokens{value: ethReceived}( 
            0, 
            route, 
            to,
            block.timestamp + 60 * 1000
        ...
        // Figure out the exact amount of USDC received from swapping.
        uint256 usdcReceived = usdc.balanceOf(address(this)) - initialUsdcBalance;

        uint256 receivedx20 = usdcReceived * 20;
        uint256 percentx20 = receivedx20 / 100;
        uint256 percentx80 = usdcReceived - percentx20;
        usdc.transfer(treasury, percentx80);

        if (address(stakingContract) == address(0)) { return; }
        uint256 stakedTotal = stakingContract.totalStaked();
        if (stakedTotal == 0) { return; }

        // approve usdc for transfer to staking contract
        usdc.approve(address(stakingContract), percentx20);

        stakingContract.fund(percentx20);
    }
    ...
    function initSwap(address routerAddress) public onlyOwner {
        ...
        address uniswapV2Pair_ = IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(this), address(usdc));
        ...
        Whitelist(address(uniswapV2Router));
        // exclude WETH and this Token Pair from paying fees.
        // Whitelist(uniswapV2Pair_);
    ...
    function recoverETH(uint256 ethAmount) public onlyOwner returns (bool success) {
