...
contract MyFlickyNFT is ERC721A, Ownable, IERC2981 {
    ...
    mapping (uint256 => bool) private metadataIdAvailable;

    string public baseURI = "ipfs://HIDDEN_CID/";
    ...
    string private constant WHITELIST_MINT = "WHITELIST_MINT";
    ...
    uint256 private constant RESERVED_TOTAL = 240;
    uint256 private constant MAP_TOTAL = 2000;
    uint256 private constant WHITELIST_TOTAL_POSSIBLE = 8160;
    uint256 private constant PUBLIC_MINT_TOTAL = 9760;
    ...
    returns (uint256, uint256, uint256, uint256) {
        return (reservedCount, mapCount, whitelistCount, publicMintCount);
    }

    function setWhitelistStatus(bool _whitelistOnly) public onlyOwner {
    ...
    function _setGateKeeper(address _gateKeeper)
        public
        onlyOwner
        returns (bool)
    {
    ...
    constructor() ERC721A("MyFlickyNFT", "NFT") {}
    ...
    function incrementTypeCount(string memory _nftType)
        private
    {
        if (compareStrings(_nftType, WHITELIST_MINT)) {
            publicMintCount++;
            whitelistCount++;
        }
    ...
    function canTypeBeMinted(string memory _nftType)
        private
        view
        returns (bool canMinted)
     {
        if (compareStrings(_nftType, WHITELIST_MINT)) {
            if (whitelistCount < WHITELIST_TOTAL_POSSIBLE && publicMintCount < PUBLIC_MINT_TOTAL) {
                canMinted = true;
            } else {
                canMinted = false;
            }
        }
        require(!whitelistOnly, "Only whitelisted participants may mint");
        if (compareStrings(_nftType, GENERAL_MINT)) {
    ...
    function mint(uint256 _metadataIndex, string memory _nftType, uint8 v, bytes32 r, bytes32 s)
        public
        returns (uint256)
    {
        require(!metadataIdAvailable[_metadataIndex], "A token with this id has already been minted");
        require(wasSignedByGateKeeper(_metadataIndex, _nftType, v, r, s), "May only mint with approved signature from gatekeeper.");        
        require(canTypeBeMinted(_nftType), "The maximum amount of that type of Flicky have already been minted");
        ...
        tokenIdsToMetadataIds[tokenId] = _metadataIndex;
        metadataIdAvailable[_metadataIndex] = true;

        incrementTypeCount(_nftType);
