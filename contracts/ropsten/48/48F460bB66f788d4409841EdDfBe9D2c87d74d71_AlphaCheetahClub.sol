...
contract AlphaCheetahClub is ERC721, Ownable {
    ...
    uint256 public cheetahReserve = 71; // Reserve 1% Cheetahs for team & community (Used in giveaways, events etc...)

    uint256 public constant SALE_START_DATE = 1648807200;

    bool public saleIsActive = true;

    uint private constant MAX_MINTS_PER_ADDRESS_SALE = 10;

    string public baseTokenURI = "https://gateway.pinata.cloud/ipfs/QmZKFJsWkgPkg5hwFyvfWrS9VHYJRYQ3nsaK4ADphCGRbA/";
    ...
    constructor() ERC721("AlphaCheetahClub", "ACC") {}
    ...
    function reserveCheetahs(address receiver, uint amount) public onlyOwner {
        require(amount > 0 && amount <= cheetahReserve, "Not enough Chetaahs left to reserve");
        for (uint256 i = 0; i < amount; i++) {
            safeMint(receiver);
        }
        cheetahReserve = cheetahReserve.sub(amount);
    }
    ...
    function mintCheetah(uint256 numberOfTokens) public payable {
        ...
        require(totalMinted().add(numberOfTokens) <= mintableSupply(), "Max Supply reached");
        require(mintedTokensByAddress[msg.sender] + numberOfTokens <= MAX_MINTS_PER_ADDRESS_SALE, "max NFT per address exceeded");
        require(MAX_CHEETAH_PURCHASE.mul(numberOfTokens) == msg.value, "Ether value sent is not correct");
    ...
    function mintableSupply() private view returns(uint) {
        return MAX_SUPPLY - cheetahReserve;
    }
