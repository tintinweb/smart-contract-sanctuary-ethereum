...
/// @title Dinoworld NFT smart contract
...
contract Dinoworld is ERC721A, Ownable, IERC2981, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint16 public royaltyBasisPoints = 5;
 
    /// @notice minting price
    uint256 public mintingPrice = 8800000000000000;
 
    /// @notice free batch size
    uint256 public freeBatch = 2000;
    uint256 public maxFree = 750;
    uint256 public freeCounter = 0;
    mapping(address => bool) public freeClaimed;
    mapping(address => uint256) public mintBlock;
 
    /// @notice supply
    uint256 public maxSupply = 8888;
    uint256 public teamNFTCounter = 0;
    ...
    function mintNFT(uint256 _mintAmount) public payable nonReentrant() {
        ...
        require(msg.sender == tx.origin, "Reverting. Method only calable by user");
 
        require(
            tokenCounter + _mintAmount <= maxSupply - freeBatch,
        ...
        if (msg.sender != owner()) {
            if(freeCounter >= maxFree) {
                require(msg.value >= mintingPrice * _mintAmount, "Payed Ether is too less for minting.");
            } else {
                if(!freeClaimed[msg.sender]) {
                    require(msg.value >= mintingPrice * (_mintAmount - 1), "One free, but payed ETH still too less.");
                } else {
                    require(msg.value >= mintingPrice * _mintAmount, "You already claimed the free mint. Payed ETH too less.");
                }
            }
            if(freeCounter < maxFree && !freeClaimed[msg.sender]) {
                require(mintBlock[msg.sender] + _mintAmount <= maxmintAmount, "Max mint per wallet reached");
            } else {
                require(mintBlock[msg.sender] + _mintAmount < maxmintAmount, "Max mint per wallet reached");
            }
        ...
        if(msg.sender != owner()) {
            if(!freeClaimed[msg.sender]) {
                if(_mintAmount > 1) {
                    mintBlock[msg.sender] += _mintAmount - 1;
                }
            } else {
                mintBlock[msg.sender] += _mintAmount;
            }
            ...
            teamNFTCounter += _mintAmount;
        }        
    }
 
    /// @notice public minting to msg.sender. Free for contract owner
    /// @param _merkleProof proof of whitelist
    function mintWhitelist(bytes32[] calldata _merkleProof) public nonReentrant() {
        ...
        require(whitelistCounter <= freeBatch, "Maximum of whitelist NFTs reached!");
        require(whitelistActive, "Whitelist minting is deactivated!");
        require(whitelisted(_merkleProof), "You are not whitelisted!");
        require(!claimed[msg.sender], "You have already claimed your free mint!");
    ...
    /// @notice verifies if wallet is whitelisted
    /// @param _merkleProof proof of whitelist
    /// @return bool flag if whitelisted or not
    function whitelisted(bytes32[] calldata _merkleProof) public view returns (bool) {
    ...
    /// @notice sets the free batch border
    /// @param _newHurdle new amount of maximal whitelist NFTs
    function setFreeBatch(uint256 _newHurdle) public onlyOwner {
        freeBatch = _newHurdle;
    }
 
    /// @notice sets the free batch border
    /// @param _newHurdle new maximum of free
    function setMaxFree(uint256 _newHurdle) public onlyOwner {
        maxFree = _newHurdle;
    }
    ...
    /// @dev normal and whitelist minting is stopped.
    function switchPaused() public onlyOwner {
        paused = !paused;
    }
 
    /// @notice switches whitelistActive
    /// @dev when whitelist is active, public minting is deactivated
    ...
    /// @param _newMerkle new merkleroot
    function setMerkle(bytes32 _newMerkle) public onlyOwner {
    ...
    /// @notice overrides starting token ID from ERC721A and sets it to 1
