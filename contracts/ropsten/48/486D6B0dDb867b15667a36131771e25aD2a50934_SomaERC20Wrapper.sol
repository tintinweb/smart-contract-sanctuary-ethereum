 ...
 */
contract SomaERC20Wrapper is SomaERC20 {
    using ERC20Helper for IERC20;

    IERC20 public underlying;

    bytes32 public constant GLOBAL_RECOVER_ROLE = keccak256('SomaERC20Wrapper:global-recover-role');
    bytes32 public LOCAL_RECOVER_ROLE;

    constructor(address networkAddress) SomaERC20(networkAddress) {}

    function initialize(IERC20 underlyingToken) external {
        LOCAL_RECOVER_ROLE = keccak256(abi.encodePacked(address(this), 'SomaERC20Wrapper:local-recover-role'));
        underlying = underlyingToken;
        super.initialize(
            string(abi.encodePacked("SOMA Wrapped: ", underlyingToken.tryName("ERC20"))),
            string(abi.encodePacked("SW", underlyingToken.trySymbol("ERC20"))),
            0,
            address(0)
        );
    }
    ...
    function recover(address account) public virtual returns (uint256) {
        require(hasRole(LOCAL_RECOVER_ROLE, _msgSender()) || hasRole(GLOBAL_RECOVER_ROLE, _msgSender()), 'SomaERC20Wrapper: invalid role requirements');
