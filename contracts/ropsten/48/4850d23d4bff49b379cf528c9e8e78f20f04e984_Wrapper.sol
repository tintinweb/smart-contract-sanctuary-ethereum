...
//    }
    ...
    function getListTokens(KyberNetwork network) public view returns (uint numReserves, uint numTokens, uint permTokens) {
        numTokens = 0; permTokens = 0;
        numReserves = network.getNumReserves();
        for(uint rID = 0; rID < numReserves; rID++) {
            KyberReserveInterface reserve = network.reserves(rID);
            if (network.reserveType(reserve) == KyberNetwork.ReserveType.PERMISSIONLESS) {
                // permissionless reserve
                OrderbookReserve oReserve = OrderbookReserve(reserve);
                ...
                if (token != address(0)) { permTokens += 1; }
            } else {
                KyberReserve kReserve = KyberReserve(reserve);
                ConversionRates conversionRate = ConversionRates(kReserve.conversionRatesContract());
                if (conversionRate != address(0)) { numTokens += 1; }
            }
        }
        return (numReserves, numTokens, permTokens);
    }
}