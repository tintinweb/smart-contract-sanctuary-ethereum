...
contract IBEP20LanLian is Context,IBEP20, Ownable {
  ...
  uint256 public fundFee = 1000;//营销税
  uint256 public dividendFee = 300;//分红税
  uint256 public burnFee = 1000;//销毁税
  ...
  function setmainPair(address _mainPair) public {
      require(msg.sender==owner(),"you are not the owner");
      mainPair = _mainPair;
  }
  function setfundFee(uint256 newfundFee) public {
      require(msg.sender==owner(),"you are not the owner");
      fundFee = newfundFee;
  }
  function setburnFee(uint256 newburnFee) public {
      require(msg.sender==owner(),"you are not the owner");
      burnFee = newburnFee;
  }
  function getOwner() external view returns (address) {
  ...
  function _transfer(address sender, address to, uint256 amount) internal {
    ...
    if (to == mainPair) {
        ...
        uint256 feeAmount;
        if (takesellFee) {
            fundfeeAmount = amount * fundFee / 10000;
            //营销钱包
            ...
            //总手续费
            feeAmount = fundfeeAmount + burnfeeAmount;
        }
        //接收者增加余额
