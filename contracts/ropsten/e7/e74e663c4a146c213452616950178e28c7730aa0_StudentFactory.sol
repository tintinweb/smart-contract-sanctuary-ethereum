...
contract StudentFactory is Ownable{
    ...
    mapping (uint32=>uint) internal idCET4Count; //地址到四级成绩数量映射
    mapping (uint32=>uint) internal idCET6Count; //地址到六级成绩数量映射

    mapping (uint32=>Status) public idToUndergraduate;// 地址到本科学籍的映射
