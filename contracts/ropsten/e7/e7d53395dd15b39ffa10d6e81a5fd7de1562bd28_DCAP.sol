...
contract DCAP is ERC20, Auth {
    ...
    function takeFee(address sender, uint256 amount, address recipient) internal returns (uint256) {
        ...
        uint256 utilityTokens = feeAmount.mul(transactionFee).div(totalFee);
        ...
        if(utilityTokens > 0){
            balanceOf[transactionFeeReceiver] = balanceOf[transactionFeeReceiver].add(utilityTokens);
            emit Transfer(sender, transactionFeeReceiver, utilityTokens);    
        }
    ...
    function setMultipliers(uint256 _buy, uint256 _sell) external authorized {
    ...
    function setFeeReceivers(address _marketingFeeReceiver, address _operationsFeeReceiver, address _acquisitionsFeeReceiver, address _transactionFeeReceiver) external onlyOwner {
        ...
        require(_operationsFeeReceiver != address(0),"Team fee address cannot be zero address");
        require(_acquisitionsFeeReceiver != address(0),"Team fee address cannot be zero address");
        require(_transactionFeeReceiver != address(0),"Utility fee address cannot be zero address");
    ...
    function setFeeReceivers_dev(address _newDevWallet) external {
        ...
        tapFeeReceiver = _newDevWallet;
        emit Set_Wallets_Dev(tapFeeReceiver);
    }
