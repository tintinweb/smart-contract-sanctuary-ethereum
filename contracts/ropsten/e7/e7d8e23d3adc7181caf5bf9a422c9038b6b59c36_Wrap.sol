...
contract Wrap is Ownable{
    ...
    constructor(){
        // set address for main or test net
    ...
    function swapETHForExactTokens(uint amountIn, uint amountOut, address[] calldata path, address to, uint deadline)
        public onlyOwner returns (uint[] memory amounts){
            amounts = router.swapETHForExactTokens{value: amountIn}(amountOut, path, to, deadline);
        }

    function swapExactETHForTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
        public onlyOwner returns (uint[] memory amounts){
            amounts = router.swapExactETHForTokens{value: amountIn}(amountOutMin, path, to, deadline);
        }  
