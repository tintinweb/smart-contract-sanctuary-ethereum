...
contract DEVIL is Context, IERC20, Ownable {
    ...
    uint256 private _maxGasPriceLimit = 80 gwei;
    ...
    uint256 public _buyLimit = 5000 * 10**18;
    uint256 public _sellLimit = 2000 * 10**18;
    uint256 public _lockupPeriod = 5;
    ...
    uint256 public _maxWalletSize = 450000 * 10**18; 
    ...
    function _transfer(
        ...
        //Transfer Tokens
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair) || _msgSender() == owner()) {
            takeFee = false;
        } else {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router)) {
                ...
                cooldown[to] = block.timestamp + _lockupPeriod;
    ...
    function setLimits(uint256 buyLimit, uint256 sellLimit) public onlyOwner {
    ...
    function setMaxGasPriceLimit(uint256 gasprice_gwei) public onlyOwner {
        _maxGasPriceLimit = gasprice_gwei * 10**9;
    }

    function setLockupPeriod(uint256 lockupPeriod) public onlyOwner {
        _lockupPeriod = lockupPeriod;
    }

    function excludeMultipleAccountsFromFees(address[] calldata accounts) public onlyOwner {
