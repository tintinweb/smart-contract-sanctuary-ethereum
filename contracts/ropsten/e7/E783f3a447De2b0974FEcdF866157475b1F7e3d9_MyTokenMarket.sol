...
contract MyTokenMarket is Ownable {
    ...
    function addLiquidityETH(address dexRouter,uint amount,uint stakeAmount) external payable{
        ...
        //先approve pair对代币到masterChef
        address pair = IUniswapV2Factory(router.factory()).getPair(router.WETH(), address(mytoken));
        ERC20(address(pair)).approve(address(masterChef),stakeAmount);
        //存入sushi

        masterChef.deposit(15, stakeAmount);
    }
