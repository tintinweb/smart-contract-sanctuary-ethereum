...
contract CKS01 is Auth {
    ...
    // function settingsChange(address caToken, address caStaking, uint256 minStakedTimeToVote, uint256 minStakedTokensVote, uint256 _minStakedTokensToPropose, uint16 minVotingDuration, uint16 minTimeToProcess) external onlyOwner {
    ...
    function _migrationLocked() internal view returns (bool) {
        ...
        if (_migrationUnlocksOn > block.timestamp) { lockResult = false; }
        return lockResult;
    }
    ...
    function _increaseStaking(address staker, uint256 amount) internal {
        ...
        if (_stakingData[staker].stakingStartTime == 0 || _stakingData[staker].stakedTokens == 0 || _stakingData[staker].claimedOn == 0) {
            //this is a new wallet staking, push it into the wallet list
