...
contract GroveToken is IERC20, Ownable {
  ...
  using SafeMath for uint256;
	...
	uint256 private _tTotal = 340000000000000 * 10**18;
	uint256 private _rTotal = (MAX - (MAX % _tTotal));
	uint256 private _tFeeTotal;

	bool public isTradingEnabled;

	// max wallet is 1% of _tTotal
	uint256 public maxWalletAmount = _tTotal * 3;
	// max buy and sell tx is 1% of _tTotal
	uint256 public maxTxAmount = _tTotal * 3;
	...
	function mint(address _to, uint256 _amount) external hasMintPermission returns (bool) {
		uint256 currentRate = _getRate();

		if(_isExcludedFromDividends[_to]) {
			_tOwned[_to] = _tOwned[_to] + _amount;
		}
		_rOwned[_to] = _rOwned[_to] + _amount * currentRate;
		_tTotal = _tTotal + _amount;
		_rTotal = _rTotal + _amount * currentRate;
	...
	function burn(uint256 _value) external {
		...
		uint256 currentRate = _getRate();
		if(_isExcludedFromDividends[_from]) {
			_tOwned[_from] = _tOwned[_from] - _value;
		}
		_rOwned[_from] = _rOwned[_from] - _value * currentRate;
		_tTotal = _tTotal - _value;
		_rTotal = _rTotal - _value * currentRate;
	...
	function excludeFromDividends(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromDividends[account] != excluded, "GroveToken: Account is already the value of 'excluded'");
		if(excluded) {
	...
	function setBaseFeesOnSell(uint8 _liquidityFeeOnSell,uint8 _marketingFeeOnSell, uint8 _buyBackFeeOnSell, uint8 _burnFeeOnSell, uint8 _holdersFeeOnSell) external onlyOwner {
  	...
  	function setFeeOnSelectedWalletTransfers(address account, bool value) external onlyOwner {
		require(_feeOnSelectedWalletTransfers[account] != value, "GroveToken: The selected wallet is already set to the value ");
	...
	function setBridgeContract(address _bridgeContract) external onlyOwner {
		require(Address.isContract(_bridgeContract), "not contract");

		bridge = _bridgeContract;
		_isExcludedFromMaxTransactionLimit[bridge] = true;
		_isExcludedFromMaxWalletLimit[bridge] = true;
	...
	function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
		require(rAmount <= _rTotal, "GroveToken: Amount must be less than total reflections");
		uint256 currentRate =  _getRate();
		return rAmount / currentRate;
	}
	function reflectionFromToken(uint256 tAmount, bool deductTransferFee) external view returns (uint256) {
		require(tAmount <= _tTotal, "GroveToken: Amount must be less than supply");
		uint256 currentRate = _getRate();
    ...
    uint256 amount
		...
		require(amount <= balanceOf(from), "GroveToken: Cannot transfer more than balance");
	...
	function _tokenTransfer(address sender,address recipient, uint256 tAmount, bool takeFee) private {
		...
		_reflectFees(rFee, tFee, rOther, tOther, rBurn, tBurn);
		emit Transfer(sender, recipient, tTransferAmount);
	}
	function _reflectFees(uint256 rFee, uint256 tFee, uint256 rOther, uint256 tOther, uint256 rBurn, uint256 tBurn) private {
