...
// The goal is to create a token that will distribute fees to holders when a user makes a transaction. 
// One way is to individually add the reward to all user’s balance. However, such transaction could easily fail due to network gas limit. 
// A different method is to create a deflationary mechanism so that tokens one holds are worth more.
...
contract ApolloVentures is Context, IERC20Metadata, Ownable {
    ...
    // Max value that is less than MAX and dividable by _tTotal
    // _rTotal is max reflection token
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    // thresold at which tax will be divided
    ...
    // to lock the swapping while function
    bool private swapping;
    bool public swapEnabled;
    // trading to and from swaping pool for user
    ...
    feeRateStruct public buyFeeRates = feeRateStruct(
        {
            // Reflection will be on every txn
            reflection: 200,
            // All other will be after thresold
            liquidity: 300,
            marketing: 400,
            lottery: 200,
            nftApp: 200
