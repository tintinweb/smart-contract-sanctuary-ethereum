...
contract DataStorage
{
    //event MyEvent1(
        //uint data
    //);
    mapping(string=>mapping(uint=>mapping(string=>string))) private data;
    mapping(string=>string[]) private orderFieldNames;
    mapping(uint=>string) private names;
    uint private nameIndex;
    function saveData(string memory name,string memory records,uint fieldCount,uint recordCount) public{
        uint returnValue;
        JsmnSolLib.Token[] memory tokens;
        uint actualNum;
        (returnValue,tokens,actualNum)=JsmnSolLib.parse(records,fieldCount*(recordCount+1)+1);
       // emit MyEvent1(actualNum);
        string[] memory fieldnames=new string[](fieldCount);
        orderFieldNames[name]=new string[](fieldCount);
        uint i;
        for(i=1;i<=fieldCount;i++)
        {
            string memory fieldName=JsmnSolLib.getBytes(records,tokens[i].start,tokens[i].end);
            fieldnames[i-1]=fieldName;
            orderFieldNames[name][i-1]=fieldName;
        }
            uint recordIndex=0;
            uint fieldIndex=0;
            for(i=fieldCount+1;i<actualNum;i++)
            {
                string memory value=JsmnSolLib.getBytes(records,tokens[i].start,tokens[i].end);
                data[name][recordIndex][fieldnames[fieldIndex]]=value;
                fieldIndex+=1;
    ...
    function left(string memory _a,uint n) public pure returns(string memory)
    {
        bytes memory _ba=bytes(_a);
        uint len=_ba.length;
        if(n<len){
    ...
    function getData(string memory name,uint top) public view returns(uint,uint,string memory)
    {
        mapping(uint=>mapping(string=>string)) storage records =data[name];
        string memory result="[";
        string[] memory fieldNames=orderFieldNames[name];
        ...
        uint recordCount=0;
        for(uint i=0;i<top||top==0;i++)
        {
            if(keccak256(abi.encodePacked(records[i][fieldNames[0]]))==keccak256(abi.encodePacked("")))
            {
                break;
            }
            uint fieldCount=orderFieldNames[name].length;
            recordCount++;
            for(uint j=0;j<fieldCount;j++)
            {
                result=strConcat(result,strConcat(strConcat("\"",records[i][fieldNames[j]]),"\","));
            }
        }
        bytes memory r=bytes(result);
        result=left(result,r.length-1);
        result=strConcat(result,"]");
        return(fieldNames.length,recordCount,result);
    }
}