...
contract PlagueNFT is ERC721Enumerable, Ownable {
...
contract PlagueToken is ERC20, ERC20Burnable, ReentrancyGuard, Ownable {



    constructor()

        ERC20("PlagueSquad Token", "PLS")
...
contract PlagueVaultNFT is Ownable, IERC721Receiver {



  mapping (uint256 => mapping (uint => Stake) ) public vault; 
...
function addVault(
  ...
  function _unstakeMany(address account, uint256[] calldata tokenIds, uint256 _pid) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      Stake memory staked = vault[_pid][tokenId];

      require(staked.owner == msg.sender, "not an owner");



      delete vault[_pid][tokenId];
  ...
  function _claim(address account, uint256[] calldata tokenIds, uint256 _pid, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      earned += (vaultid.daily * 1000000000000000000) * (block.timestamp - stakedAt) / 86400 ;
    ...
    if (earned > 0) {
      ...
      VaultInfo[_pid].token.mint(account, earned);
 ...
 function earningInfo(uint256 _pid,address account, uint256[] calldata tokenIds) external view returns (uint256[1] memory info) {
