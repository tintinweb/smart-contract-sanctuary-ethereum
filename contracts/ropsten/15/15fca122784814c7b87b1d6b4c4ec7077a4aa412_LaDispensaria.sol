...
contract LaDispensaria is Ownable, ReentrancyGuard {
	...
	constructor(address _pg, address _ap) {
		PG = IWWNFT(_pg);
		AP = IWWNFT(_ap);
	}
	...
	function _getTotalEtherOwed(address account) public view returns (uint256) {
		uint256 etherOwed = ((PG.balanceOf(account) * _getTotalEtherPG())/800) + ((AP.balanceOf(account) * _getTotalEtherAP())/1600);
		return etherOwed;
	}
