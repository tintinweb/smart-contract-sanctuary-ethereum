...
contract WatchChain is ERC1155Holder,
    ...
    event NewFundingStartTime(uint256 startTime,uint256 watchId);
    ...
    function setFundingStartTime(uint256 startTime,uint256 watchId) 
        external 
        status(watchId,Status.Waiting) 
        onlyOwner{
            require(startTime>block.timestamp,"startTime cannot be in the past");
            watches[watchId].fundingStartTime=startTime;
            emit NewFundingStartTime(startTime,watchId);
    }
