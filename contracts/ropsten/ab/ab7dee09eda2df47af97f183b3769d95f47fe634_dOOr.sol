...
Tax: 4/4%
2% Marketing & 2% LP Fee BUY
2% Marketing & 2% LP Fee SELL
...
contract dOOr is ERC20, Ownable {
    ...
    constructor() ERC20("dOOr", "\xF0\x9F\x9A\xAA") {
    ...
    ; uint256 totalSupply = 2 * 1e6 * 1e18;
 
        maxTransactionAmount = totalSupply * 10 / 1000; // 1% maxTransactionAmountTxn 20,000 tokens max
        maxWallet = totalSupply * 30 / 3000; // 3% maxWallet - 30,000 tokens
        swapTokensAtAmount = totalSupply * 1 / 2000; // 0.05% swap wallet - 1000 tokens swappable
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
        require(newAmount >= totalSupply() * 1 / 10000, "Swap amount cannot be lower than 0.01% of total supply."); // cannot set Swap amount lower than 200 Tokens
        require(newAmount <= totalSupply() * 2 / 200, "Swap amount cannot be higher than 1% of total supply."); // cannot set swap higher than 20,000 tokens
        swapTokensAtAmount = newAmount;
        return true;
    }
 
    function updateMaxTxnAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 1 / 4000)/1e18, "Cannot set maxTransactionAmount lower than 0.025%"); // cannot set maxTX less than 500 Tokens
        maxTransactionAmount = newNum * (10**18);
    }
 
    function updateMaxWalletAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 1 / 1000)/1e18, "Cannot set maxWallet lower than 0.1%"); // cannot set maxWallet lower than 2000 tokens
