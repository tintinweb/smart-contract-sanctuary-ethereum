...
contract Dreadfulz3D is ERC721Enumerable, ERC721Burnable, Ownable, ReentrancyGuard, Pausable, IERC721Receiver {
    ...
    function swap(uint256[] memory tokenIds) external whenNotPaused nonReentrant {
        ...
        for(uint256 i; i < tokenIds.length; i++) {
            ...
            IERC721 nft = hasToken(current) ? dreadfulz2D : this;
            if(!hasPaid(current)) {
                requiredBurn += cost;
            }
            _paid[current] = true;
    ...
    function hasToken(uint256 id) public view returns (bool) {
        return (!_exists(id) || ownerOf(id) == address(this)) && hasPaid(id);
    }
