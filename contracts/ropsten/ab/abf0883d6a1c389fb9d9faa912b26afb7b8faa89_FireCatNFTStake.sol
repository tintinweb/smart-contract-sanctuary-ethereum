...
contract FireCatNFTStake is IFireCatNFTStake, ModifyControl, FireCatTransfer {
    ...
    modifier modifyLocked() {
        require(_totalStaked == 0, "total staked is not zero");
        _;
    }
    ...
    function setPool(address pool_) external modifyLocked onlyRole(DATA_ADMIN) {
        require(pool_ != address(0), "nft is zero");
        pool = pool_;
        emit SetPool(pool_);
    }
    ...
    function setRegistryProxy(address registryProxy_) external modifyLocked onlyRole(DATA_ADMIN) {
        require(registryProxy_ != address(0), "nft is zero");
    ...
    function stake(uint256 tokenId_) external beforeStake nonReentrant returns (bool) {
        require(stateOf(msg.sender) == State.stakeable, "user can not stake");
        require(fireCatNFT.ownerOf(tokenId_) == msg.sender, "token is not owned by user");

        fireCatNFT.transferFrom(msg.sender, address(this), tokenId_);
        require(fireCatNFT.ownerOf(tokenId_) == address(this), "token is not owned by this contract");
        _totalStaked += 1;
    ...
    function claim() external beforeClaim nonReentrant returns (uint256) {
        ...
        require(fireCatNFT.ownerOf(tokenId) == msg.sender, "token is not owned by user");   
