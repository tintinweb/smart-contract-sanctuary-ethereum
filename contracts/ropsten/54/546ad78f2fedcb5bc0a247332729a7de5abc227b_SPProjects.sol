...
contract SPProjects is ISPProjects, Ownable {
  ...
  mapping(uint256 => SPProfitDistribution) internal profitDistributionOf;

  mapping(uint256 => bool) public override crowdfundingEndState; 

  mapping(uint256 => uint256) public override crowdfundingAmount;

  mapping(uint256 => address) public override creators;

  mapping(uint256 => uint) public override createdAtOf;
  ...
  */
    ...
    returns (uint256 projectId)
  {
    ...
    createdAtOf[projectId] = block.timestamp;
    
    profitDistributionOf[projectId] = _profitDistribution;

    crowdfundingAmount[projectId] = _metadata.totalPrice / 20;

    creators[projectId] = msg.sender;
  ...
  */
  ...
  function endCrowdFunding(uint256 _projectId) external {
    require(!crowdfundingEndState[_projectId]);
    crowdfundingEndState[_projectId] = true;
  }

  function getProfitDistributionOf(uint256 _projectId) external view override returns(SPProfitDistribution memory) {
    return profitDistributionOf[_projectId];
  }
}