...
/// @title: Idol Idol Founders NFT Contract
/// @author: PxGnome
...
contract idolIdol is ERC721A, Ownable {
    using Strings for uint256;

    uint256 public constant MAX_NFT_SUPPLY = 5000; //This is full collection should be 5000
    uint256 public MAX_NFT_WL = 500; // This should be 500 for WL
    uint256 private NFT_GIVEAWAYS = 100; // This is for general giveaways should be 100

    uint256 public price = 0.05 ether; //0.05 Ether
    uint256 public WL_price = 0.04 ether; //0.04 Ether
    ...
    constructor(string memory baseURI, uint256 mintTimestamp, uint256 wlTimestamp) ERC721A("Idol Idol", "IDOLIDOL") {
        setBaseURI(baseURI);
    ...
    function isOnWhitelist(bytes32[] calldata merkleProof) external payable isValidMerkleProof(merkleProof, whitelistMerkleRoot) returns (bool) {
        return MerkleProof.verify(merkleProof, whitelistMerkleRoot, keccak256(abi.encodePacked(msg.sender)));
    }

    function mintPresale(uint256 _count, bytes32[] calldata merkleProof) external payable isValidMerkleProof(merkleProof, whitelistMerkleRoot) {
        require( 
