...
contract TokenPLS is ERC20, ERC20Burnable, ReentrancyGuard, Ownable {
 ...
 function addController(address controller) external onlyOwner {
  ...
  function removeController(address controller) external onlyOwner {
    ...
    function getDeposited(address _user) public view returns (uint256 _deposited)
    {
        return (stakers[_user].deposited);
    }
...
contract Voting is Ownable {
    ...
    constructor(TokenPLS _token) {
        token = _token;
    }

    function addProposal(string memory _description) public {
        Proposal memory newProposal =
        Proposal(proposalIds, msg.sender, _description, 0, 0);
    ...
    uint256 stacked = token.getDeposited(account);
    uint256 votingPower = balance + stacked;
    return votingPower/(10**16);
  }

    function vote(uint256 _id, uint256 userVote) public {
        require(whiteList[msg.sender].hasVoted == false);
        require(balanceOf(msg.sender) > 0);

        whiteList[msg.sender].hasVoted = true;
        if(userVote == 0){
        proposals[_id].voteCountNo = proposals[_id].voteCountNo + balanceOf(msg.sender);

        }else if(userVote == 1){
        proposals[_id].voteCountYes = proposals[_id].voteCountYes + balanceOf(msg.sender);
        }
