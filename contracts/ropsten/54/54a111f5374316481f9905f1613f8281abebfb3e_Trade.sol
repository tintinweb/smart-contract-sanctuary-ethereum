...
contract Trade is Ownable {
    ...
    function confirm(uint256 _dealId, uint256 _persent) public {
        ...
        if (msg.sender == deals[_dealId].payer) // Подтверждение покупателем
            {
// Если продавец подтвердил сделку и сумму процента, то завершаем ее
    		...
    		// Если продавец подтвердил сделку но изменил сумму процента
        ...
        else if (msg.sender == deals[_dealId].seller) // Подтверждение продавцом
            {
// Если покупатель подтвердил сделку и сумму процента, то завершаем ее
    		...
    		// Если покупатель подтвердил сделку но изменил сумму процента
