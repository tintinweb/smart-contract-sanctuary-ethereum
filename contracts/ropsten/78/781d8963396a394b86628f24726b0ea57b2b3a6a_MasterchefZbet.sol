...
contract MasterchefZbet is Ownable, ReentrancyGuard, Whitelist {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // Bonus muliplier for early zbet makers.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. ZBET to distribute per block.
        uint256 lastRewardBlock;  // Last block number that ZBET distribution occurs.
        uint256 accZbetPerShare;   // Accumulated ZBET per share, times 1e18. See below.
        bool isPrivatePool;        // priavte pool only be accessable by particular address
    }
    ...
    // The ZBET TOKEN!
    IZbetToken public zbet;
    // ZBET tokens created per block.
    uint256 public zbetPerBlock;
    ...
    // The block number when ZBET mining starts.
    ...
    // Zbet referral contract address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 zbetPerBlock);
    ...
    constructor(
        ...
        uint256 _zbetPerBlock,
        uint256 _startBlock
    ) public {
        zbet = _zbet;
        boosterNFT = _boosterNFT;
        zbetPerBlock = _zbetPerBlock;
    ...
    function getNFTAddress(address _account, uint256 _pid) public view returns (address) {
        return _depositedNFT[_account][_pid].nftAddress;
    }

    function getNftTokenId(address _account, uint256 _pid) public view returns (uint256) {
        return _depositedNFT[_account][_pid].tokenId;
    }
    ...
    // View function to see pending ZBET on frontend.
    function pendingZbet(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accZbetPerShare = pool.accZbetPerShare;
        uint256 lpSupply = lpTokenAmount[_pid];
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 zbetReward = multiplier.mul(zbetPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accZbetPerShare = accZbetPerShare.add(zbetReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accZbetPerShare).div(1e18).sub(user.rewardDebt);
    }

    /* ========== Owner Functions ========== */

    // Add a new lp to the pool. Can only be called by the owner.
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _isPrivatePool, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's ZBET allocation point and deposit fee. Can only be called by the owner.
    ...
    // Depositing of NFTs
    function depositNFT(address _nft, uint256 _tokenId, uint256 _pid) public nonContract {
    ...
    // Withdrawing of NFTs
    function withdrawNFT(uint256 _pid) public nonContract {
        NFTDetails memory nftDetail = _depositedNFT[msg.sender][_pid];
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 zbetReward = multiplier.mul(zbetPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        zbet.mint(address(this), zbetReward);

        pool.accZbetPerShare = pool.accZbetPerShare.add(zbetReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for ZBET allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant nonContract {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accZbetPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeZbetTransfer(msg.sender, pending, _pid);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accZbetPerShare).div(1e18);
    ...
    // Safe ZBET transfer function, just in case if rounding error causes pool to not have enough ZBET.
    function safeZbetTransfer(address _to, uint256 _amount, uint256 _pid) internal {
        uint256 boost = 0;
        zbet.transfer(_to, _amount);
        boost = getBoost(_to, _pid).mul(_amount).div(10000);
        payReferralCommission(msg.sender, _amount);
        if (boost > 0) zbet.mint(_to, boost);
    }

    /* ========== Set Variable Functions ========== */

    function updateEmissionRate(uint256 _zbetPerBlock) public onlyOwner {
        massUpdatePools();
        zbetPerBlock = _zbetPerBlock;
        emit UpdateEmissionRate(msg.sender, _zbetPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                zbet.mint(referrer, commissionAmount);
