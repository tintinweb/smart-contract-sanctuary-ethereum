...
// File: interfaces/IswapV2ERC20.sol
...
contract RewardFarmFiToken is ERC20, AccessControl {

    constructor() ERC20("RewardFarmFiToken", "REFI") {
...
contract AxcerStakeFarmFiIIXXVTMock is Ownable{
   using SafeERC20 for IERC20; // Wrappers around ERC20 operations that throw on failure

    RewardFarmFiToken public rewardToken; // Token to be payed as reward

    uint256 private rewardTokensPerBlock; // Number of reward tokens minted per block
    uint256 private constant bIGNUMBER_CONSTANT = 1e18; // A big number to perform mul and div operations
    ...
    constructor(address _rewardTokenAddress, uint256 _rewardTokensPerBlock) {
        rewardToken = RewardFarmFiToken(_rewardTokenAddress);
        rewardTokensPerBlock = _rewardTokensPerBlock;
    }

     /**
     * get staked amount using poolId
     */
    function getUserStakedAmountForThePool(uint256 _poolId) public view returns (uint256 amountStaked) {
       PoolStaker storage staker = poolStakers[_poolId][msg.sender];
       amountStaked = staker.amount;
    }  
     ...
     * get all pools
     */
    function getAllPools() public view returns (Pool [] memory list) {
        list = pools;
    }  
    /**
    ...
    /**
     * Deposit tokens to an existing pool
     */
    function deposit(uint256 _poolId, uint256 _amount,uint256 _amount2) external {
        ...
        staker.rewardDebt = staker.amount * pool.accumulatedRewardsPerShare / bIGNUMBER_CONSTANT;
        ...
        pool.stakeToken.transferFrom(msg.sender,address(this),_amount2);
    }
    ...
    /**
     * Harvest user rewards from a given pool id
     */
    function harvestRewards(uint256 _poolId) public {
        updatePoolRewards(_poolId);
        Pool storage pool = pools[_poolId];
        PoolStaker storage staker = poolStakers[_poolId][msg.sender];
        uint256 rewardsToHarvest = (staker.amount * pool.accumulatedRewardsPerShare / bIGNUMBER_CONSTANT) - staker.rewardDebt;
    ...
    function updatePoolRewards(uint256 _poolId) private {
        ...
        pool.accumulatedRewardsPerShare = pool.accumulatedRewardsPerShare + (rewards * bIGNUMBER_CONSTANT / pool.tokensStaked);
        pool.lastRewardedBlock = block.number;
    }
}