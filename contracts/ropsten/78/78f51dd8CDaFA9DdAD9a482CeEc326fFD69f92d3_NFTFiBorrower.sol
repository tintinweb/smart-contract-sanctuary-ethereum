...
contract NFTfiAdmin is Ownable, Pausable, ReentrancyGuard {
    ...
    event SetWhitelistNFTContract(
        address nftContract,
        bool setAsWhitelisted
    );
    ...
    constructor() internal {
        //
        erc20CurrencyIsWhitelisted[address(0xdAC17F958D2ee523a2206206994597C13D831ec7)] = true;
    ...
    function whitelistNFTContract(address _nftContract, bool _setAsWhitelisted) external onlyOwner {
        ...
        emit SetWhitelistNFTContract(_nftContract,_setAsWhitelisted);
...
contract NFTFiBorrower is NFTfiAdmin, NFTfiSigningUtils, ERC721 {
    ...
    event PledgeCreated(
        ...
        address indexed loanERC20Denomination,
        uint256 nftCollateralId,
        uint256 loanPrincipalAmount,
        uint256 loanInterestRateForDurationInBasisPoints,
        uint256  loanAdminFeeInBasisPoints ,
        uint256 loanDuration,
        uint256 maximumRepaymentAmount,
        uint256 apy,
        uint256 orderId,
    ...
    event PledgeCancelled(uint256 orderId , uint256 orderCancelTime);
    uint256 public totalOrderId = 0;
    mapping (uint256 => Order) public OrderIdToLender;
    ...
    function pledge(address _nftCollateralContract, uint256 _nftCollateralId, address _loanERC20Denomination, uint256 _loanPrincipalAmount,uint256 _maximumRepaymentAmount, uint256 _loanInterestRateForDurationInBasisPoints, uint256 _loanDuration,uint256 _apy)public{
        console.log("pledgepledge");
        require(getOwnerByIdAr(_nftCollateralId,_nftCollateralContract) == msg.sender , "caller is not the token owner");
        require(isContract(_loanERC20Denomination), "token: implementation is not a contract");
        require(_loanInterestRateForDurationInBasisPoints != 0,"interest can not be zero ");

        require(_maximumRepaymentAmount >= _loanPrincipalAmount, 'Negative interest rate loans are not allowed.');
        require(uint256(_loanDuration) <= maximumLoanDuration, 'Loan duration exceeds maximum loan duration');
        require(uint256(_loanDuration) != 0, 'Loan duration cannot be zero');
        require(erc20CurrencyIsWhitelisted[_loanERC20Denomination], 'Currency denomination is not whitelisted to be used by this contract');
        require(nftContractIsWhitelisted[_nftCollateralContract], 'NFT collateral contract is not whitelisted to be used by this contract');
        ...
        loanInterestRateForDurationInBasisPoints: (_loanInterestRateForDurationInBasisPoints), //_loanStartTime
        ...
        borrower: msg.sender,
        apy:_apy,
        orderStartTime: uint64(now)
        });

        OrderIdToLender[totalOrderId] = order;
        totalOrderId = totalOrderId.add(1);
        IERC721(address(_nftCollateralContract)).transferFrom(msg.sender, address(this), _nftCollateralId);
        emit PledgeCreated(
            order.borrower,
            _nftCollateralContract,
            _loanERC20Denomination,
            ...
            _loanPrincipalAmount,
            _loanInterestRateForDurationInBasisPoints,
            order.loanAdminFeeInBasisPoints,
            _loanDuration,
            order.maximumRepaymentAmount,
            order.apy,
            order.orderId,
            now
        );
    ...
    function withdrawPledge(uint256 _orderId) public {

        Order memory order = OrderIdToLender[_orderId];
        require(address(0) != order.borrower, 'orderId borrower is null ');
        require(msg.sender == order.borrower, 'Only the borrower can withdrawPledge  NFT');
        delete OrderIdToLender[_orderId];
        IERC721(address(order.nftCollateralContract)).transferFrom(address(this), msg.sender, order.nftCollateralId);

        emit PledgeCancelled(_orderId,now);
    }
    ...
    function beginLoan(
        ...
        console.log("beginLoan" );

        Order memory order = OrderIdToLender[_orderId];
        console.log("beginLoan" );
        require(address(0) != order.borrower, 'orderId borrower is null ');
        require(msg.sender != order.borrower, 'orderId borrower is you self ');
        console.log(msg.sender );
        console.log(order.borrower );


        Loan memory loan = Loan({
        loanId: totalNumLoans, //currentLoanId,
        loanPrincipalAmount: order.loanPrincipalAmount,
        maximumRepaymentAmount: order.maximumRepaymentAmount,
        nftCollateralId: order.nftCollateralId,
        loanStartTime: uint64(now), //_loanStartTime
        loanDuration: uint32(order.loanDuration),
        loanInterestRateForDurationInBasisPoints: uint32(order.loanInterestRateForDurationInBasisPoints),
        loanAdminFeeInBasisPoints: uint32(order.loanAdminFeeInBasisPoints),
        nftCollateralContract: order.nftCollateralContract,
        loanERC20Denomination: order.loanERC20Denomination,
        ...
        IERC20(loan.loanERC20Denomination).transferFrom(msg.sender, loan.borrower, loan.loanPrincipalAmount);
    ...
    function payBackLoan(uint256 _loanId) external nonReentrant {
        ...
        IERC721(address(loan.nftCollateralContract)).transferFrom(address(this), msg.sender, loan.nftCollateralId);
    ...
    function liquidateOverdueLoan(uint256 _loanId) external nonReentrant {
        ...
        totalActiveLoans = totalActiveLoans.sub(1);
        IERC721(address(loan.nftCollateralContract)).transferFrom(address(this), lender, loan.nftCollateralId);
    ...
    function _computeInterestDue(uint256 _loanPrincipalAmount, uint256 _maximumRepaymentAmount, uint256 _loanDurationSoFarInSeconds, uint256 _loanTotalDurationAgreedTo, uint256 _loanInterestRateForDurationInBasisPoints) internal view returns (uint256) {
