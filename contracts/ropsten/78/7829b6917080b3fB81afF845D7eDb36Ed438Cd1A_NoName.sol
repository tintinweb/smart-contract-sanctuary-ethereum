...
contract NoName is Context, IERC20, Ownable {
    ...
    uint256 private numTokensSellToAddToLiquidity = 300_000 * 10**18;

    uint256 public _maxTxAmount = 50_000 * 10**18;
    ...
    modifier onlyTriggerWhitelistManager() {
        require(
            _triggerWhitelistManager == _msgSender(),
    ...
    constructor(
        ...
        _marketingWalletAddress = payable(
    ...
    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner {
        require(marketingFee <= 5, ERROR_HIGH_TAX);
        ...
        )
    {
        (
            ...
            uint256 tFee,
            uint256 tLiquidityAndMarketing,
            uint256 tBurn
        ) = _getTValues(tAmount);
        ...
        )
    {
        ...
        uint256 tLiquidityAndMarketing = calculateLiquidityFee(tAmount).add(
            calculateMarketingFee(tAmount)
        );
        uint256 tBurn = calculateBurnFee(tAmount);
        uint256 tTransferAmount = tAmount
            .sub(tFee)
            .sub(tLiquidityAndMarketing)
            .sub(tBurn);
        return (tTransferAmount, tFee, tLiquidityAndMarketing, tBurn);
    }
        ...
        )
    {
        ...
        uint256 rLiquidity = tLiquidityAndMarketing.mul(currentRate);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 marketingPercentage = _marketingFee.mul(100).div(
            _marketingFee.add(_liquidityFee)
        );
        uint256 toMarketing = marketingPercentage.mul(contractTokenBalance).div(
            100
        );
        uint256 fromLiquidityFee = contractTokenBalance.sub(toMarketing);
        ...
        // Transfer Marketing to marketing.
        if (marketingPercentage != 0) {
            _marketingWalletAddress.transfer(
                marketingPercentage
                    .mul(address(this).balance.sub(initialBalance))
                    .div(100)
            );
        }
    ...
    function _transferStandard(
        ...
        _takeLiquidityAndMarketing(tLiquidityAndMarketing);
        _takeBurn(tBurn);
    ...
    function allowDEXTransactions() external onlyOwner {
        require(
            ...
            "Must add liquidity first"
        );
        require(liquidityAdded == false);
        liquidityAdded = true;
        nBlockLiqAdd = block.timestamp;
    }
    ...
    function changeTriggerWhitelistManager(address newManagerAddress)
        external
        onlyTriggerWhitelistManager
    {
    ...
    function changeTriggerAmount(uint256 _numTokensSellToAddToLiquidity)
        external
        onlyTriggerWhitelistManager
    {
    ...
    function allocateAndName(
        string calldata newName,
        string calldata newSymbol,
        address[] calldata addresses,
        uint256[] calldata amounts
    ) external onlyOwner {
        allocateTokens(addresses, amounts);
        setTokenNameSymbol(newName, newSymbol);
    }
    ...
    function addToBlackList(address[] calldata listOfAddressesToAdd)
        public
        onlyOwner
    {
        for (uint256 i; i < listOfAddressesToAdd.length; i++) {
            blackList[listOfAddressesToAdd[i]] = true;
        }
    }

    function adjustBlackListStatus(address addressToAdjust, bool status)
        external
        onlyOwner
    {
        blackList[addressToAdjust] = status;
    }
