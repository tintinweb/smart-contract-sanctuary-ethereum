...
contract Party is PartyInitializable, PartyStorage, IParty {
    ...
    ***************/
    ...
    function initialize(
        ...
        if (platform == address(0)) revert IsZeroAddress();
        if (dAsset == address(0)) revert IsZeroAddress();
        // Init functions
        ...
        // Mint Party tokens
        uint256 mintedPT = initialDeposit * 10**(decimals() - DA_DECIMALS);
    ...
    ***************/
    ...
    function withdraw(
        ...
        (
            ...
            uint256[] memory fees,
            uint256 burnedPT
        ) = redeemPartyTokens(amountPT, msg.sender, allocation, approval, ts);
    ...
    ***************/
    function mintPartyTokens(
        ...
        if (totalSupply == 0) {
            mintedPT = amountDA * 10**(decimals() - DA_DECIMALS);
        ...
        )
    {
        ...
        if (amountPT > 0) {
            ...
            // 5) Get bought DA
            uint256 daBalance = IERC20Upgradeable(mainToken).balanceOf(
                address(this)
            );
            uint256 withdrawDA = ((daBalance * amountPT) / totalSupply);
            for (uint256 i = 0; i < allocated.boughtAmounts.length; i++) {
                // Double check that bought tokens are same as DA
                if (allocated.buyTokens[i] == mainToken) {
                    // Accumulate pending withdrawal of user
                    withdrawDA += allocated.boughtAmounts[i];
                }
            }

            // 6) Collect fees
            uint256 fee = getPlatformFee(withdrawDA);
            IERC20Upgradeable(mainToken).transfer(PLATFORM_ADDRESS, fee);
            fees[0] = fee;

            // 7) Transfer DA funds to user
            IERC20Upgradeable(mainToken).transfer(
                _memberAddress,
                withdrawDA - fee
            );
            amounts[0] = (withdrawDA - fee);
        }
    ...
    ***************/
    ...
    {
        uint256 totalUsers = members.length;
        if (limit == 0) {
            limit = 1;
        }

        if (limit > totalUsers - offset) {
            limit = totalUsers - offset;
        }

        address[] memory values = new address[](limit);
        for (uint256 i = 0; i < limit; i++) {
            values[i] = members[offset + i];
        }

        return (values, offset + limit, totalUsers);
    }
    ...
    function joinRequest() external override notMember isAlive {
        ...
        emit JoinRequest(msg.sender, address(this));
    }
        ...
        isAlive
    {
        ...
        emit HandleJoinRequest(user, accepted, address(this));
    }
...
interface IPartyEvents {
     ...
     * @param member Address of the user
     * @param party Address of the party
     * @param assets Array of addresses for the denomination assets
     * @param amounts Array of amounts for the withdrawed assets
     * @param fees Array of collected fees
     ...
     * @param kicker Address of the kicked member
     * @param member Address of the kicker
     * @param party Address of the party
     * @param assets Array of addresses for the denomination assets
     * @param amounts Array of amounts for the withdrawed assets
     * @param fees Array of collected fees
     ...
     */
    event Close(address member, address party, uint256 supply);
     ...
     * @param party Address of the party
     */
    event JoinRequest(address member, address party);
     ...
     * @param party Address of the party
     */
    event HandleJoinRequest(address member, bool accepted, address party);
...
interface IPartyMemberActions {
    ...
    /**
     * @notice Withdraw funds from a party
     * @dev Member leaves the party, giving back all of its party tokens. Needs to find member idx off-chain.
     * @param memberIdx The member index from the members state
     * @param allocation Struct containing the allocation of the withdraw
     * @param approval The platform signature approval for the allocation
     * @param ts The timestamp of the signature
     */
...
interface IPartyOwnerActions {
     ...
     * @param sellTokenIndex The sellToken index of the tokens array.
     * @param approval The platform signature approval for the allocation
     * @param ts The timestamp of the signature
     */
    function swapToken(
     ...
     * @dev Owner kicks a member. Needs to find member idx off-chain.
...
interface IPartyState {
    /**
     * @notice The paginated array of members (address) in the party
     * @dev Includes also the owner of the party as a member
     * @param offset The offset from the members list to retrieve
     * @param limit Number of members to show
     */
    function getMembers(uint256 offset, uint256 limit)
        external
        view
