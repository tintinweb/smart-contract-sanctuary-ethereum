...
contract DogeArmyWarzone is Ownable, Pausable, ReentrancyGuard {
    IERC721 public DOGEARMYNft;
    ...
    event WithdrawRewards(address indexed staker, uint256 tokens, uint256 tax);
    ...
    constructor(
      ...
      uint256 _baserate
        ...
        DOGEARMYNft = IERC721(_dogearmy);
        _contractTypes[_dogearmy] = ContractTypes.DOGEARMY;
        _baseRates[_dogearmy] = _baserate;
        treasury = _treasury;
    ...
    */
    ...
    function deposit(
      ...
      require(
        ...
        contractAddress == address(DOGEARMYNft)
        || contractAddress == address(VAULTNft)
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        if (contractType == ContractTypes.DOGEARMY) { user.stakedCBC.push(tokenIds[i]); }
        if (contractType == ContractTypes.ARMS) { user.stakedARMS.push(tokenIds[i]); }
        if (contractType == ContractTypes.VAULT) { user.stakedVAULT.push(tokenIds[i]); }
      }
    ...
    function withdraw(
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        if (contractType == ContractTypes.DOGEARMY) {
        ...
        if (contractType == ContractTypes.VAULT) {
      ...
      if (user.stakedCBC.length == 0 && user.stakedVAULT.length == 0) {
        newYield = 0;
      }
    ...
    function registerDeposit(address owner, address contractAddress, uint256 tokenId) public authorised {
      ...
      if (contractType == ContractTypes.VAULT) { user.stakedVAULT.push(tokenId); }
    ...
    function getStakerTokens(address staker) public view returns (uint256[] memory, uint256[] memory, uint256[] memory) {
      return (_stakers[staker].stakedCBC, _stakers[staker].stakedARMS, _stakers[staker].stakedVAULT);
    }
    ...
    */
    ...
    function setVAULTContract(address _VAULT, uint256 _baseReward) public onlyOwner {
      VAULTNft = IERC721(_VAULT);
      _contractTypes[_VAULT] = ContractTypes.VAULT;
      _baseRates[_VAULT] = _baseReward;
    }

    function setShibaDogeAddress(address shib) external onlyOwner {
      ShibaDoge = IERC20(shib);
    }
    ...
    */
    function withdrawRewards(uint256 amount) public nonReentrant whenNotPaused {
      ...
      ShibaDoge.transferFrom(treasury, _msgSender(), (amount - tax));
    ...
    */
    function spendRewards(address user, uint256 amount) external authorised nonReentrant {
    ...
    /**
    * @dev Returns current spendable balance of a specific user.
    */
    ...
    function rescueTokens(address _stuckToken, uint256 _amount) external onlyOwner {
      require(_stuckToken != address(DOGEARMYNft));
      require(_stuckToken != address(ARMSNft));
      require(_stuckToken != address(VAULTNft));
