...
// Dependency file: contracts/RecoverableByOwner.sol
...
// import "contracts/RecoverableByOwner.sol";

/// @custom:security-contact [email protected]
contract WebMasonCoin is ERC20Votes, RecoverableByOwner {
    uint256 public constant lockupTime = 6 * 30 * 24 * 60 * 60; // 6 months
    uint256 public constant cliffTime = 1 * 30 * 24 * 60 * 60; // 1 month
    uint256 public constant vestingTime = 5 * 12 * 30 * 24 * 60 * 60; // 5 years

    mapping(address => bool) public isVested;
    mapping(address => uint256) public vestedAt;
    mapping(address => uint256) public vestedTotal;

    mapping(address => bool) public isAirdropper;

    event Airdrop(address indexed recipients, uint256 amount, uint256 time);

    constructor() ERC20("WMC", "WMC") ERC20Permit("WMC") {
        ...
        onlyAirdropper
    {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            if (!isVested[recipients[i]]) {
                isVested[recipients[i]] = true;
                vestedAt[recipients[i]] = block.timestamp;
            }
            vestedTotal[recipients[i]] += amounts[i];
            transfer(recipients[i], amounts[i]);
    ...
    function lockedOf(address account) public view returns (uint256) {
        ...
        if ((block.timestamp - vestedAt[account]) >= (lockupTime + vestingTime))
            return 0;

        if ((block.timestamp - vestedAt[account]) <= lockupTime + cliffTime)
            return vestedTotal[account];

        return
            vestedTotal[account] -
            (vestedTotal[account] *
                (block.timestamp - vestedAt[account] - lockupTime)) /
            vestingTime;
    }
