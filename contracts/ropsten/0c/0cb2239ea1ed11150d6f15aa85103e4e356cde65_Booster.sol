    ...
    ReentrancyGuard
{
    ...
    {    
        ...
        (uint128 liquidityTotal, uint128 liquidityDeposit) = (0,0);
        (liquidityTotal, liquidityDeposit, amount0, amount1) = _calcLiquidityForDeposit(amount0Desired, amount1Desired);
        emit Total(liquidityTotal, liquidityDeposit);
        // If BP total supply > 0, vault can't be empty
        assert(BPtotalSupply == 0 || liquidityTotal > 0 );
        if (BPtotalSupply == 0) {
            // For first deposit, just use the liquidity desired      
            shares = liquidityDeposit;
        } else {
            shares = uint256(liquidityDeposit).mul(BPtotalSupply).div(liquidityTotal);        
        }
    }
    ...
    /**
    * @notice Calculates liquidity to be added to Uniswap v3 pool 
    * taking into account desired amounts for deposit, liquidity before the deposit,
    * accrued fees, protocol commission and unused  assets on the contact balance.
    * @param amount0Desired desired deposit amount token 0
    * @param amount1Desired desired deposit amount token 1
    * @return liquidityTotal Total liquidity in the Uniswap V3 pool + fees in Uniswap V3 + dust on the this contract
    * @return liquidityDeposit liquidity in the deposit
    * @return depositAmount0 amount of tokens 0 that can be deposited
    * @return depositAmount1 amount of tokens 1 that can be deposited
    */
    function _calcLiquidityForDeposit(uint256 amount0Desired, uint256 amount1Desired) internal view returns (uint128 liquidityTotal, uint128 liquidityDeposit, uint256 depositAmount0, uint256 depositAmount1) {
        liquidityDeposit = _liquidityForAmounts(baseLower, baseUpper, amount0Desired, amount1Desired);
        (depositAmount0, depositAmount1) = _amountsForLiquidity(baseLower, baseUpper, liquidityDeposit);
        //adding one penny due to loss during conversion 
        (depositAmount0, depositAmount1) = (depositAmount0.add(1), depositAmount1.add(1));
        ...
        uint256 amount0 = getBalance0().add(depositAmount0).add(uint256(tokensOwed0).mul(oneMinusFee).div(1e6));
        uint256 amount1 = getBalance1().add(depositAmount1).add(uint256(tokensOwed1).mul(oneMinusFee).div(1e6));

        //liquidity in position add liquidity from fees and contract balance
        liquidityTotal = liquidityInPosition + _liquidityForAmounts(baseLower, baseUpper, amount0, amount1) - liquidityDeposit;
    }
