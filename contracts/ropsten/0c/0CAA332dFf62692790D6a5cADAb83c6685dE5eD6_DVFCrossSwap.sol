...
abstract contract Swap is Storage, UserWallet, EIP712Upgradeable {
  ...
  event SwapPerformed(bytes32 swapId, bytes32 swapGateway, address indexed user,
   ...
   uint256 amountTo, bool fundsBridged);
  ...
  function executeSwapWithSignature(
    ...
    bool fundsBridged = postSwap(swapConstraints.swapGateway, tokenToAmount, swapConstraints.tokenTo, swapConstraints.user);

    emit SwapPerformed(swapId, swapConstraints.swapGateway, swapConstraints.user, swapConstraints.tokenFrom,
     swapConstraints.amountFrom, swapConstraints.tokenTo, tokenToAmount, fundsBridged);
  }
  ...
  function executeSwapWithSelfLiquidity(
    ...
    (,uint256 tokenToAmount) = performSwap(swapConstraints.user, swapConstraints.tokenFrom, swapConstraints.tokenTo,
     swapConstraints.amountFrom, swapConstraints.minAmountTo, data);

    emit SwapPerformed(swapId, bytes32(0), swapConstraints.user, swapConstraints.tokenFrom, swapConstraints.amountFrom,
     swapConstraints.tokenTo, tokenToAmount, false);
  }
