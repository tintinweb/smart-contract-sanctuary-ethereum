...
abstract contract GamesCore is Ownable {
    ...
    function withdraw() public onlyOwner {
        if (houseProfitEther > 0) {
            token.safeTransfer(
                owner(),
                token.balanceOf(address(this)) - uint256(houseProfitEther)
            );
    ...
    function _receiveBet(uint256 betAmount, bool useToken) internal {
        if (useToken) {
    ...
    function _payoutWinnings(address recipient, uint256 amount, bool useToken) internal {
        if (useToken) {
            ...
            token.safeTransfer(payable(recipient), amount);
        } else {
