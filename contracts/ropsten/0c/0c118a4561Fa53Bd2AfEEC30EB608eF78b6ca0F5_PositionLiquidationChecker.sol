...
contract PositionLiquidationChecker is Ownable{
    ...
    constructor(address _balanceManager) {
    ...
    function checker() external view returns (bool canExec, bytes memory execPayload) {
        ...
        for (uint i = 0; i < positionIds.length; i++) {
            ...
            if (positionColleteralValue < positionMarginValue) {
                ...
                canExec = true;
                execPayload = abi.encodeWithSelector(IClearingHouseExtended.liquidatePosition.selector,positionIds[i], 0, block.timestamp + 600);
                break;
            }
        }
