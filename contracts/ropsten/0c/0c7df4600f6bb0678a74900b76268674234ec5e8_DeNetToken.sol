...
contract Ownable {
  ...
  event MasterNodeTransferred(address indexed previousOwner, address indexed newOwner);
  ...
  function setMasterNode(address newMasterNode) external onlyOwner {
    MasterNodeTransferred(masterNode, newMasterNode);
...
contract DeNetToken is ERC827Token, MintableToken, BurnableToken, PausableToken, RecoverableToken {
    ...
    uint256 public defaultMint = 100000000000000000000;
    function DeNetToken() public {
        pause();
    }

    function mintMany(address[] addresses, uint256[] amounts) external onlyOwner canMint {
    ...
    function newUser(address user) external onlyMasterNode {
        require(mint(user, defaultMint));
    }
    
    function setDefaultMint(uint256 _value) external onlyOwner {
        defaultMint = _value;
    }
