...
contract UtilToken is ERC721AUpgradeable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IUtilToken{
    ...
    function mint() external payable override whenNotPaused {
        ...
        _safeMint(callerAddress, tokenId);
        emit NewNFT(tokenId);
    }
    ...
    ) internal virtual whenNotPaused airDropCheck(from, startTokenId, quantity) override (ERC721AUpgradeable) {
        super._beforeTokenTransfers(from, to, startTokenId, quantity);
    }

    modifier airDropCheck(address from, uint256 startTokenId, uint256 quantity) {
        for (uint256 tokenStartLength = 0; tokenStartLength < quantity; tokenStartLength++) {
            require(
                !airDropTokens[tokenStartLength],
...
interface IUtilToken {
     ...
     */
    function airDrop(address assigned, uint256 quantity) external;
