...
contract Reveal is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function redeem(uint256 tokenId) external {
        ...
        require(dropSupply[drop] >= tokenId, "Invalid TokenId");

        uint256 dropId; 
        for (uint256 index = 0; index <= drop ; index++) {
            if(dropSupply[index] >= tokenId ){
                dropId = index;
                break;
            }
        }
    ...
    function tokenURI(uint256 tokenId)
        ...
        {
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, bytes32(tokenId), ".json")) : "";
        }
