...
contract ICO is AllowanceCrowdsale, TimedCrowdsale {
	constructor(
		uint256 _rate,
		...
		Crowdsale(_rate, _wallet, _token, _minRate, _maxRate)
 ...
 */
abstract contract TimedCrowdsale is Crowdsale, Ownable {
	 ...
	 */
	event ChangedClosignTime(
		uint256 prevClosingTime,
		uint256 newClosingTime
	);

	event ChangedOpeningTime(
		uint256 prevOpeningTime,
	 ...
	 */
	function _setClosingTime(uint256 newClosingTime) onlyOwner() external {
	
		emit ChangedClosignTime(_closingTime, newClosingTime);
		_closingTime = newClosingTime;
	}
	
	function _setOpeningTime(uint256 newOpeningTime) onlyOwner() external {
	
		emit ChangedOpeningTime(_openingTime, newOpeningTime);
