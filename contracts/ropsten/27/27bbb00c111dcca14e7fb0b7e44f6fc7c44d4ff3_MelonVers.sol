...
interface MLM {
    function checkuserregister(address _a) external view returns(bool);
    function Distribution(address _a,uint256 _u,uint256 _usd,uint256 totalbuy,uint256 _txValue) external payable returns(bool);
    function Register(address _user,address ref) external returns(bool);
    function changeposition(address _a,string memory _p)external returns(bool);
...
contract MelonVers is ERC721, Ownable{
    ...
    MLM public mlmaddress;
    AggregatorV3Interface internal priceFeed;
    address public Directpurchase;
    uint256 public TXFEE;
    address public Admin;
    constructor(address _mlmaddress,address _Directpurchase,address _plateformFee,address _admin,string[] memory countryName,uint256[] memory _Totalblock) ERC721("AOJUS", "OJS"){
        ...
        lockdays = lockdays + 360;
        starttime = block.timestamp;
        mlmaddress = MLM(_mlmaddress);
        Directpurchase = _Directpurchase;
        Admin = _admin;
        newcountry(countryName,_Totalblock);
        plateformFee = _plateformFee;
        TXFEE = TXFEE + 10000000000000000;
        amount.push(usd);
    }
    ...
    function Getcurrentrate()public view returns(uint256){
        if(block.timestamp > starttime + (1 days * aafterchangerate)  && usd < lastusd){
    ...
    function changeUSD(uint256 _a) public onlyOwner returns(bool)  {
        usd = _a;
        emit upd(usd,usd);
        amount.push(usd);
        return true;
    ...
    function checkuserregister(address _a)public view returns(bool){
        return mlmaddress.checkuserregister(_a);
    }
    function userregister(address reff)public returns(bool){
        require(isMinter[reff],"reffer not buy any land");
        return mlmaddress.Register(msg.sender,reff);
    }
    function changeposition(string memory _a)public returns(bool){
        return mlmaddress.changeposition(msg.sender,_a);
    }
    function AssignToken(int[] memory x,int[] memory y,address[] memory _a,string memory _countryname) public onlyAdmin returns(bool){
        ...
        for(uint256 i=0;i<x.length;i++){
            ...
            History[_a[i]].tokenid.push(id);
            History[_a[i]].timeforbuy.push(block.timestamp);
            History[_a[i]].location.push(parseldata[id]);
    ...
    function multibuyparsel(uint256 _nonce ,bytes memory signature,int256[] memory _x,int256[] memory _y,string memory _countryname)public payable returns(bool) {
        require(_x.length == _y.length ,"order not same");
        require(_x.length <= 100,"its more then 100");
        require(verify(msg.sender,address(this),msg.value,_nonce,signature),"is change the user");
        require(iscountry[_countryname],"country not minted");

        mintedblock[_countryname] = mintedblock[_countryname] + _x.length ;
        require(mintedblock[_countryname] <= totalblock[_countryname],"end of the minting block");


        bool user_ = mlmaddress.checkuserregister(msg.sender);
        if(block.timestamp > starttime + (1 days * aafterchangerate) && usd < lastusd){
                usd = usd + incrementAmount;
                starttime = block.timestamp ;
                amount.push(usd);
            }
        mintedamountblock[_countryname][usd] = mintedamountblock[_countryname][usd] + _x.length;

        uint256 totausd = _x.length * usd;

        for(uint256 i=0;i<_x.length;i++){
            require(!axis[_x[i]][_y[i]],"alreday mint");
            plentblockMint(_x[i],_y[i],_countryname);
        }
        ...
        address ad = mlmaddress.getadmin();
        payable(ad).transfer(TXFEE);

        if(user_){
            mlmaddress.Distribution{value:(msg.value.sub(TXFEE))}(msg.sender,msg.value.sub(TXFEE),totausd,_x.length,TXFEE);
            // emit mainlog(msg.sender,_x.length,totausd,msg.value,user_);
            emit main(msg.sender);

        }
        else{
            payable(Directpurchase).transfer(msg.value.sub(TXFEE));
            emit mainevent(msg.sender,msg.value.sub(TXFEE));
        }
        return true;
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_contractaddress, _amount, _nonce));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(_contractaddress, _amount, _nonce);
