...
contract Auction is IERC721Receiver, Ownable {
    bool public isWhitelistBidsOnly = false;
    bool public isWhitelistOpen = false;

    uint256 public increasement = 0; // 4 decimals,  1000000 = 100%
    struct tokenDetails {
        address seller;
        uint128 price;
        uint256 duration;
    ...
    salePending[] public createdAuctions;
    ...
    /**
       Owner can toggle whitelist bids only, of on then bids can only be accespted if the user is on the whitelist
       Owner :Allow / prevent people not on the whitelist from bidding
    */
    function setWhitelistBidsOnly(bool _isWhitelistBidsOnly)
        external
        onlyOwner
    {
        isWhitelistBidsOnly = _isWhitelistBidsOnly;
    }

    /**
       Owner can set whitelist open/closed, if open users can add themselves to the whitelist
       Owner: Allow / prevent users from adding themselves to the white list
    */
    function setWhitelistOpen(bool _isWhitelistOpen) external onlyOwner {
        isWhitelistOpen = _isWhitelistOpen;
    }

    /**
       Buyer can add them self to the whitelist only if whitelist is open / true
       User: Add themselves to the white list
    */
    function addMeToWhitelist() public {
        require(isWhitelistOpen, "Whitelist is closed");
        require(msg.sender != address(0), "Invalid Address");
        whitelistedAddresses[msg.sender] = true;
    }

    /* 
    Owner: new function:  Add any address to the whitelist at any time
    */
    ...
    /* 
    Owner: new function: Set the % that each bid must increase from the previous
    */
    function setIncreasement(uint256 _increasement) external onlyOwner {
    ...
    */
    modifier isWhitelisted(address _address) {
        if (isWhitelistBidsOnly) {
            require(
                whitelistedAddresses[_address],
                "Not on whitelist cant bid"
            );
        }
        _;
    }
    ...
    /**
       ...
       User :Start an auction for an nft
    */
    function createTokenAuction(
        ...
        createdAuctions.push(salePending({nftAddress: _nft, nftId: _tokenId}));
    }

    /* 
    Owner: new function:  Increase the duration of an auction
    */
    function incraseDuration(
        ...
        require(
            ...
            "The new duration is less than the current duration"
        );
        auction.duration = _duration;
    }
    ...
    /**
       ...
       User: Make a bid for an NFT
    */
    function bid(address _nft, uint256 _tokenId)
        external
        payable
        isWhitelisted(msg.sender)
    {
        ...
        require(
            auction.maxBid + (auction.maxBid * increasement) / 1000000 <
                msg.value,
            "The bid price should be higher than increasement"
        );
        auction.maxBid = msg.value;
    ...
    */
    ...
    /**
     User:See which NFT’s are for sale
    */
    function getSaleAuctions() external view returns (salePending[] memory) {
        return createdAuctions;
    }

    /** 
    User: See a history of bids, amounts address’s
    User: If an NFT is for sale, what is the curent bid, when will the auction end
    */
