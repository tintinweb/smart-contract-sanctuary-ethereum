...
contract _MLRB is IERC20, Ownable {
    ...
    function fundLiquidity() public onlyOwner returns(bool){
        require(address(this).balance >= startupToken, "Contract must be prefunded with tokens.");
        fundingLiquidity = true;
        injectLiquidity(startupToken, address(this).balance, true);
    ...
    function initialize() external onlyOwner payable returns (bool) {
    ...
    function _transfer(address from,address to,uint256 amount) public virtual {
        ...
        if(!isTradingEnabled && !fundingLiquidity){
            if(_msgSender() != owner()){
                revert();
            }
        }
