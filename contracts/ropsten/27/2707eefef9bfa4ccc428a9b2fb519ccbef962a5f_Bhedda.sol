...
contract Bhedda is Context, IERC20 {
    ...
    struct TaxWallets {
        ...
        address payable giveaway;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x590b26612033FE5d679aee9cDC5bfB8F9fFaFeB4),
        giveaway: payable(0x590b26612033FE5d679aee9cDC5bfB8F9fFaFeB4)
        });
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint16 reflect, uint16 liquidity, uint16 marketing, uint16 giveaway) external onlyOwner {
        uint16 check = reflect + liquidity + marketing + giveaway;
        require(check <= maxBuyTaxes);
        _buyTaxes.liquidity = liquidity;
        _buyTaxes.reflect = reflect;
        _buyTaxes.marketing = marketing;
        _buyTaxes.giveaway = giveaway;
        _buyTaxes.totalSwap = check - reflect;
    }

    function setTaxesSell(uint16 reflect, uint16 liquidity, uint16 marketing, uint16 giveaway) external onlyOwner {
        uint16 check = reflect + liquidity + marketing + giveaway;
        require(check <= maxBuyTaxes);
        _sellTaxes.liquidity = liquidity;
        _sellTaxes.reflect = reflect;
        _sellTaxes.marketing = marketing;
        _sellTaxes.giveaway = giveaway;
        _sellTaxes.totalSwap = check - reflect;
    }

    function setTaxesTransfer(uint16 reflect, uint16 liquidity, uint16 marketing, uint16 giveaway) external onlyOwner {
        uint16 check = reflect + liquidity + marketing + giveaway;
        require(check <= maxBuyTaxes);
        _transferTaxes.liquidity = liquidity;
        _transferTaxes.reflect = reflect;
        _transferTaxes.marketing = marketing;
        _transferTaxes.giveaway = giveaway;
        _transferTaxes.totalSwap = check - reflect;
    }

    function setRatios(uint16 liquidity, uint16 marketing, uint16 giveaway) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.giveaway = giveaway;
        _ratios.total = liquidity + marketing + giveaway;
    }
    ...
    function setWallets(address payable marketing, address payable giveaway) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.giveaway = payable(giveaway);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 giveawayBalance = (amtBalance * ratios.giveaway) / ratios.total;
        uint256 marketingBalance = amtBalance - giveawayBalance;
        if (ratios.giveaway > 0) {
            _taxWallets.giveaway.transfer(giveawayBalance);
        }
