...
contract DexManager is IDexManager, DexOwner, ReentrancyGuard {
  ...
  function emitStatusUpdate(bytes32 _tradePairId, bytes32 _orderId) private {
    ...
    emit OrderStatusChanged(_order.traderAddress, _tradePairId, _order.oId,
  ...
  function addExecution(bytes32 _tradePairId, ITradePairs.Order memory _marketOrder, ITradePairs.Order memory _takerOrder, uint _price, uint _quantity) private {
    ...
    uint mlastFee = handleExecution(_tradePairId, _marketOrder.oId, _price, _quantity, _tradePair.makerFee);
    uint tlastFee = handleExecution(_tradePairId, _takerOrder.oId, _price, _quantity, _tradePair.takerFee);
    portfolio.addExecution(
      ...
      mlastFee,
      tlastFee);
    emit Executed(_tradePairId, _price, _quantity, _marketOrder.oId, _takerOrder.oId, mlastFee, tlastFee, _marketOrder.side == ITradePairs.Side.BUY ? true : false);

    emitStatusUpdate(_tradePairId, _marketOrder.oId); // EMIT maker order's status update
  }
  ...
  function addLimitOrder(
    ...
    if (_side == ITradePairs.Side.BUY) {
      ...
      if (takerRemainingQuantity > 0) {
        orderBooks.addOrder(_tradePair.buyBookId, _order.oId, _order.price);
      ...
      if (takerRemainingQuantity > 0) {
        orderBooks.addOrder(_tradePair.sellBookId, _order.oId, _order.price);
    ...
    emitStatusUpdate(_tradePairId, _order.oId);  // EMIT order status. if no fills, the status will be NEW, if any fills status will be either PARTIAL or FILLED
  }
  ...
  function addMarketOrder(
    ...
    tradePairs.updateOrder(orderId, _order);
    emitStatusUpdate(_tradePairId, _order.oId);  // EMIT taker(potential) order status. if no fills, the status will be NEW, if not status will be either PARTIAL or FILLED
  }

  function getOrders() public view returns (bytes32[] memory){
    return orderArray;
  }
}
...
import "./interfaces/IDexOwner.sol";


contract DexOwner is Ownable, IDexOwner{
  ...
  event Initialize(address orderBookAddress, address portofolioAddress, address tradePairsAddress);
  ...
  function setAdmin(string memory setting, bool boolean) external onlyOwner {
    adminSettings[setting] = boolean;
    emit ChangedSetAdmin(setting, boolean);
  }

  function initialize(address orderBookAddress, address portofolioAddress, address tradePairsAddress) external onlyOwner {
    orderBooks = IOrderBooks(orderBookAddress);
    portfolio = IPortfolio(portofolioAddress);
    tradePairs = ITradePairs(tradePairsAddress);
    emit Initialize(orderBookAddress, portofolioAddress, tradePairsAddress);
  }
...
interface ITradePairs {
    ...
    function setDecimals(bytes32 _tradePairId, uint256 _decimals, bool _isBase) external;
