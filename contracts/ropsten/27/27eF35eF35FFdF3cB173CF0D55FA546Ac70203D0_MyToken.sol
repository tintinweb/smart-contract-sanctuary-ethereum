...
contract Stakeable {
    ...
    uint256 internal stakingBase = 10;
  ...
  /**
    ...
    */
    ...
    function _stake(uint256 _amount) internal{
        ...
        uint _stakingAmount = _getStakingAmount(_amount);
        // Use the index to push a new Stake
        // push a newly created Stake with the current block timestamp.
        stakeholders[index].stake = Stake(msg.sender, _stakingAmount, timestamp, _amount, 0);
        // Emit an event that the stake has occured
        emit Staked(msg.sender, _stakingAmount, index,timestamp, _amount);
    }


    function _getStakingAmount(uint256 _amount) internal view returns(uint)
    {
        uint _amountToEther = _amount / 1 ether;
        uint _stakingAmount = stakingBase;

        if(_amountToEther != 0){
            _stakingAmount = _stakingAmount + (_amountToEther + 1);
        }
        else{
            _stakingAmount = _stakingAmount + 1;
        }
