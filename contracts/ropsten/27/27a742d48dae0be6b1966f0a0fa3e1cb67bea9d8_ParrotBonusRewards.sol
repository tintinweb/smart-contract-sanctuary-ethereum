 ...
 */
contract ParrotBonusRewards is IBonusRewards, Ownable, ReentrancyGuard {
    ...
    modifier notPaused() {
        require(!paused, "ParrotBonusRewards: paused");
        _;
    }
    ...
    function addBonus(
        ...
        require(
            ...
            "ParrotBonusRewards: not authorized caller"
        );
        require(
            _startTime >= block.timestamp,
            "ParrotBonusRewards: startTime in the past"
        );

        // make sure the pool is in the right state (exist with no active bonus at the moment) to add new bonus tokens
        Pool memory pool = pools[_lpToken];
        require(pool.lastUpdatedAt > 0, "ParrotBonusRewards: pool does not exist");
        ...
        for (uint256 i = 0; i < bonuses.length; i++) {
            if (bonuses[i].bonusTokenAddr == _bonusTokenAddr) {
                ...
                require(
                    ...
                    "ParrotBonusRewards: last bonus period hasn't ended"
                );
                require(
                    bonuses[i].remBonus == 0,
                    "ParrotBonusRewards: last bonus not all claimed"
                );
            }
        }
    ...
    function extendBonus(
        ...
        require(
            ...
            "ParrotBonusRewards: bonus and id dont match"
        );
        require(
            bonus.endTime > block.timestamp,
            "ParrotBonusRewards: bonus program ended, please start a new one"
        );
    ...
    function addPoolsAndAllowBonus(
        ...
        for (uint256 i = 0; i < _lpTokens.length; i++) {
            ...
            require(
                ...
                "ParrotBonusRewards: lptoken decimals > 18"
            );
    ...
    function collectDust(
        ...
        require(
            ...
            "ParrotBonusRewards: lpToken, not allowed"
        );
        ...
        if (_token == address(0)) {
            ...
            if (bonusTokenAddrMap[_token] == 1) {
                ...
                require(
                    ...
                    "ParrotBonusRewards: wrong pool"
                );
                require(
                    bonus.endTime + WEEK < block.timestamp,
                    "ParrotBonusRewards: not ready"
                );
    ...
    function setPaused(bool _paused) external override {
        require(
            ...
            "ParrotBonusRewards: caller not responder"
        );
        paused = _paused;
    }
