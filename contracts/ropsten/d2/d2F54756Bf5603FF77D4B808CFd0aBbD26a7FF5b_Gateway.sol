...
contract Gateway is ReentrancyGuard {
    ...
    mapping(string => mapping(uint256 => uint8)) outboundNonceMapping;
    ...
    function requestFromRouter(
        ...
        // can store keccak256(abi.encodePacked(chainId)) as state at contract level
        require(
        ...
        (bool success, bytes memory data) = address(this).call(
            abi.encodeWithSignature(
                "executeHandlerCalls(bytes[], bytes[], bool, bytes32)",
                requestPayload.handlers,
                requestPayload.payloads,
                requestPayload.isAtomic,
                messagehash
    ...
    function executeHandlerCalls(
        ...
        for (uint8 i = 0; i < handlers.length; i++) {
            ...
            if (isAtomic && !success) {
                revert("Operation Failed, reverting the batch request");
            }
            emit RequestFromRouterEvent(handler, messageHash, success);
        }
        return false;
    }
