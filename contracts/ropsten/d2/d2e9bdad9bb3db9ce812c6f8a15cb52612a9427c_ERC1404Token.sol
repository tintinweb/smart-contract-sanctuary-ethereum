...
contract MaxNumShareholdersToken is Ownable, MessagedERC1404 {
    ...
    bool public maxNumShareholdersActive = false;
    ...
    constructor (uint256 _maxNumShareholders) public {
      if (_maxNumShareholders > 0) {
          maxNumShareholders = _maxNumShareholders;
          maxNumShareholdersActive = true;
      }
        ...
        returns (uint256)
    {
        ...
        bool shareholderReplaced = (balanceOf(from) - value) == 0;
...
contract ERC1404Token is Pausable, ManagedWhitelist, MaxNumShareholdersToken {
   ...
   */
  constructor (string _name, string _symbol, uint256 _tokens, uint8 _decimals, uint256 _maxNumShareholders) MaxNumShareholdersToken(_maxNumShareholders) public {

    name = _name;
    symbol = _symbol;
    decimals = _decimals;

    totalSupply_ = _tokens * 10**uint256(decimals);
    balances[msg.sender] = totalSupply();
    emit Transfer(0x0, msg.sender, totalSupply());

    addToBothSendAndReceiveAllowed(msg.sender);
      ...
      returns (uint8 restrictionCode)
  {
      ...
      } else if (maxNumShareholdersActive && calcNextNumShareholders(from, to, value) > maxNumShareholders) {
