...
contract MerkleVesting is Ownable, TokenVesting {
    ...
    constructor(address token_, bytes32 merkleRoot_, bool startNow) TokenVesting(token_) {
        merkleRoot = merkleRoot_;
        
        if (startNow) vestingStartTimestamp = block.timestamp;
    }
    ...
    function claim(
        ...
        _token.safeTransfer(beneficiary, instantReleaseAmount);
...
contract TokenVesting is Ownable, ReentrancyGuard {
    ...
    event Release(address indexed beneficiaryPayable, bytes32 indexed vestingScheduleId, uint indexed amount);
    ...
    uint private vestingSchedulesTotalAmount;
     ...
     */
    function getVestingSchedulesCountByBeneficiary(address _beneficiary) external view returns (uint) {
     ...
     */
    function getVestingIdAtIndex(uint index) external view returns (bytes32) {
     ...
     */
    function getVestingScheduleByAddressAndIndex(address holder, uint index)
        external
        view
     ...
     */
    function getVestingSchedulesTotalAmount() external view returns (uint) {
    ...
    function revoke(bytes32 vestingScheduleId) public onlyOwner onlyIfVestingScheduleNotRevoked(vestingScheduleId) {
        ...
        uint vestedAmount = _computeReleasableAmount(vestingSchedule);
        if (vestedAmount > 0) {
            release(vestingScheduleId, vestedAmount);
        }
        uint unreleased = vestingSchedule.amountTotal.sub(vestingSchedule.released);
     ...
     */
    function release(bytes32 vestingScheduleId, uint amount)
        public
        nonReentrant
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        emit Release(beneficiaryPayable, vestingScheduleId, amount);
    }
     ...
     * @return the number of vesting schedules
     */
    function getVestingSchedulesCount() public view returns (uint) {
     ...
     */
    function computeVestingScheduleIdForAddressAndIndex(address holder, uint index) public pure returns (bytes32) {
     ...
     */
    function _computeReleasableAmount(VestingSchedule memory vestingSchedule) internal view returns (uint) {
        ...
        if ((currentTime < vestingSchedule.cliff) || vestingSchedule.revoked == true) {
            ...
            uint timeFromStart = currentTime.sub(vestingSchedule.start);
            uint secondsPerSlice = vestingSchedule.slicePeriodSeconds;
            uint vestedSlicePeriods = timeFromStart.div(secondsPerSlice);
            uint vestedSeconds = vestedSlicePeriods.mul(secondsPerSlice);
            uint vestedAmount = vestingSchedule.amountTotal.mul(vestedSeconds).div(vestingSchedule.duration);
