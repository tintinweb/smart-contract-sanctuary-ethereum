 ...
 * @notice It is a staking pool for Uniswap V2 LP tokens (stake Uniswap V2 LP tokens -> get ORBIT).
 */
contract StakingPoolForUniswapV2Tokens is Ownable, Pausable, ReentrancyGuard {
    ...
    // ORBIT token (token distributed)
    IERC20 public immutable orbitToken;

    // The staked token (i.e., Uniswap V2 WETH/ORBIT LP token)
    ...
    // Tokens distributed per block (in orbitToken)
     ...
     * @param _orbitToken reward token address
     * @param _rewardPerBlock reward per block (in ORBIT)
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (userInfo[msg.sender].amount > 0) {
            ...
            if (pendingRewards > 0) {
                orbitToken.safeTransfer(msg.sender, pendingRewards);
            }
        }
     ...
     * @param amount amount to withdraw (in orbitToken)
     * @dev Only callable by owner.
     */
    function adminRewardWithdraw(uint256 amount) external onlyOwner {
        orbitToken.safeTransfer(msg.sender, amount);
...
import "./interfaces/IOrbitToken.sol";

import "hardhat/console.sol";

/**
 * @title TokenDistributor
 * @notice It handles the distribution of ORBIT token.
 * It auto-adjusts block rewards over a set number of periods.
 */
contract TokenDistributor is ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeERC20 for IOrbitToken;
    ...
    IOrbitToken public immutable orbitToken;
    ...
    /**
     * @notice Constructor
     * @param _orbitToken ORBIT token address
    ...
    constructor(
        ...
        uint256 nonCirculatingSupply = IOrbitToken(_orbitToken).SUPPLY_CAP() -
            IOrbitToken(_orbitToken).totalSupply();
        ...
        console.log("Amount of tokens to be minted is:  ", amountTokensToBeMinted);
        console.log("Non circulating supply is: ", nonCirculatingSupply);
     ...
     * @param amount amount to deposit (in ORBIT)
     */
    function deposit(uint256 amount) external nonReentrant {
        require(amount > 0, "Deposit: Amount must be > 0");
        ...
        // Transfer ORBIT tokens to this contract
        orbitToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        // Transfer ORBIT tokens to the sender
        orbitToken.safeTransfer(msg.sender, amount);
    ...
    function withdrawAll() external nonReentrant {
        ...
        // Transfer ORBIT tokens to the sender
        orbitToken.safeTransfer(msg.sender, amountToTransfer);
    ...
    function _updatePool() internal {
        ...
        if (tokenRewardForStaking > 0) {
            ...
            bool mintStatus = orbitToken.mint(address(this), tokenRewardForStaking);
            if (mintStatus) {
                accTokenPerShare = accTokenPerShare + ((tokenRewardForStaking * PRECISION_FACTOR) / totalAmountStaked);
            }

            orbitToken.mint(tokenSplitter, tokenRewardForOthers);
        }
...
/**
 * @title TokenSplitter
 * @notice It splits ORBIT to team/treasury/trading volume reward accounts based on shares.
 */
...
contract TokenSplitter is Ownable, ReentrancyGuard {
    ...
    // Total ORBIT tokens distributed across all accounts
     ...
     * @param _orbitToken address of the ORBIT token
     */
    constructor(
    ...
    /**
     * @notice Release ORBIT tokens to the account
    ...
    function releaseTokens(address account) external nonReentrant {
        ...
        uint256 totalTokensReceived = orbitToken.balanceOf(address(this)) + totalTokensDistributed;
        ...
        orbitToken.safeTransfer(account, pendingRewards);
    ...
    /**
     * @notice Retrieve amount of ORBIT tokens that can be transferred
     * @param account address of the account
     */
...
interface IOrbitToken is IERC20 {
