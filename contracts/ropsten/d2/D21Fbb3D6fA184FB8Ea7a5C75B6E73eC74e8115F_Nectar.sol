    ...
    Whitelist
{
     ...
     */
    function setBloomNodes(address _newBloomNodes) external onlyOwner zeroAddressCheck(_newBloomNodes) {
     ...
     */
    function setBloomReferral(address _newBloomReferral) external onlyOwner zeroAddressCheck(_newBloomReferral) {
     ...
     */
    function setLiquidityManager(address _liquidityManager) public onlyOwner zeroAddressCheck(_liquidityManager) {
    ...
    function allowSwap(address _addr, bool _value) external onlyOwner zeroAddressCheck(_addr) {
        _setSwapAllowed(_addr, _value);
    }

    function swapUsdcForToken(uint256 _amountIn, uint256 _amountOutMin) external {
        _setSwapAllowed(_msgSender(), true);
        liquidityManager.swapUsdcForToken(_msgSender(), _amountIn, _amountOutMin);
        _setSwapAllowed(_msgSender(), false);
    }

    function swapTokenForUsdc(uint256 _amountIn, uint256 _amountOutMin) external {
        _setSwapAllowed(_msgSender(), true);
        liquidityManager.swapTokenForUsdc(_msgSender(), _amountIn, _amountOutMin);
        _setSwapAllowed(_msgSender(), false);
    }

    function _setSwapAllowed(address _addr, bool _value) private zeroAddressCheck(_addr) {
