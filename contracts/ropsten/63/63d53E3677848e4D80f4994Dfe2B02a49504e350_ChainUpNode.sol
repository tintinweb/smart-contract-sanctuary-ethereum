...
contract ChainUpNode is Ownable, ERC721AQueryable, ReentrancyGuard {
  ...
  function authorizedTrade(ChainUpTrade calldata trade) external payable onlyMinter {
    require(trade.expiredHeight > block.number, "Trade has expired");

    // change this in the future
    uint256 sum = 0;
    uint256 i = 0;
    for (i = 0; i < trade.price.length; i++) {
      sum += trade.price[i];
    }
    ...
    for (i = 0; i < trade.userListings.length; i++) {
      UserListing calldata userListing = trade.userListings[i];
      ...
      safeTransferFrom(ownerOf(userListing.tokenId), trade.receiver, userListing.tokenId);
    }

    masterHash = keccak256(abi.encodePacked(trade.price, trade.expiredHeight, trade.receiver, masterHash));
    signercheck(trade.signature.s, trade.signature.r, trade.signature.v, masterHash, trade.signature.signer);
  }
  ...
  function _getRewards(uint256 tokenId) private view returns (uint256) {
    require(totalSupply() * block.number - totalHeight > 0, "No rewards to claim");

    // change this is in the future
  ...
  function getRewards(uint256 tokenId) external view returns (uint256) {
    return _getRewards(tokenId);
  }

  function getBlockRewards() external view returns (uint256) {
    require(totalSupply() * block.number - totalHeight > 0, "No rewards to claim");
    uint256 totalReward = address(this).balance;
    return totalReward / (totalSupply() * block.number - totalHeight);
  }
