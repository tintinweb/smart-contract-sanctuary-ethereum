...
contract CollateralModule is Ownable {
    ...
    function addCollateral(
        ...
        require(conversionAmount > 0, "Zero conversion rate");

        if (minimum == 0) {
            minimum = 10**(IERC20Metadata(token).decimals());
        }

        collateralAssest[token] = Collateral(minimum, conversionAmount, true);
        totalCollateralAssest.push(token);
        emit AddCollateral(token, conversionAmount);
    }
    ...
    {
        require(conversionAmount > 0, "Zero conversion rate");
        collateralAssest[token].rate = conversionAmount;

        emit ConversionRate(conversionAmount);
    }
...
contract VaultBase is CollateralModule {
    ...
    mapping(uint256 => CDP) public CDPInfo;
    ...
    event Deposit(uint256 cdp, address user, uint256 amount);
    event Withdraw(uint256 cdp, address user, uint256 amount);
    event MintDD(uint256 cdp, address user, uint256 ddamount);
    event BurnDD(uint256 cdp, address user, uint256 ddamount);
    event TransferVault(uint256 cdp, address user, address to);
    event CloseVault(uint256 cdp, address user);

    // Internal method
    function _required(uint256 cdp, address user) internal view {
    ...
    function convertcollateralAssestToDD(address token, uint256 amount)
        public
        view
        returns (uint256)
    {
    ...
    function convertDDToCollateralAssest(address token, uint256 ddamount)
        public
        view
        returns (uint256)
    {
        uint256 amount = ((1 * 10**IERC20Metadata(token).decimals()) *
            (10**IERC20Metadata(DD).decimals()));
        amount = amount / collateralAssest[token].rate;

        return ((amount * ddamount) / (10**IERC20Metadata(DD).decimals()));
    }
    ...
    function getUserCDPByIndex(address user, uint256 index)
        external
        view
        returns (uint256)
    {
    ...
    function getUserCDP(
        ...
        uint256 j = 0;
        for (uint256 i = strIndex; i <= endIndex; i++) {
    ...
    function getUserCDPIndex(address user, uint256 cdp)
        external
        view
...
contract Vault is VaultBase {
    ...
    function create(
        ...
        emit CreateVault(token, user, amount, ddAmount);
    ...
    function mintDD(
        ...
        _required(cdp, user);
        require(ddamount > 0, "Zero amount");
    ...
    function burnDD(
        ...
        uint256 amount = convertDDToCollateralAssest(
            CDPInfo[cdp].token,
            CDPInfo[cdp].deposited
        );
        CDPInfo[cdp].deposited += amount;
    ...
    function closeVault(uint256 cdp, address user) external onlyDDMinter {
        ...
        require(CDPInfo[cdp].amount == 0, "Invalid amount");
