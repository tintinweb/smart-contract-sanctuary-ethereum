...
contract CreepzInvasionGrounds is Ownable, ReentrancyGuard {
    address public PGNft;
    address public APNft;
    address public BONft;
    address public HONft;
    ...
    constructor(
      ...
      address _signer
        ...
        PGNft = _pregame;
        APNft = _afterparty;
        _baseRates[_pregame] = 4200 ether;
        _baseRates[_afterparty] = 1800 ether;
    ...
    function deposit(
      ...
      require(
        ...
        contractAddress == PGNft
        || contractAddress == BONft
        || contractAddress == APNft
        || contractAddress == HONft,
        "Unknown contract"
      );
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        // if (user.currentYield != 0 || contractType != APNft) {
        //   newYield += getTokenYield(contractAddress, tokenIds[i]);
        // }
        newYield += getTokenYield(contractAddress, tokenIds[i]);

        if (contractAddress == PGNft) { user.stakedPG.push(tokenIds[i]); }
        if (contractAddress == APNft) { user.stakedAP.push(tokenIds[i]); }
        if (contractAddress == BONft) { user.stakedBO.push(tokenIds[i]); }
      }

      // if (user.currentYield == 0 && newYield != 0 && user.stakedAP.length > 0) {
      //   for (uint256 i; i < user.stakedAP.length; i++) {
      //     uint256 tokenYield = getTokenYield(address(APNft), user.stakedAP[i]);
      //     newYield += tokenYield;
      //   }
      // }
    ...
    function withdraw(
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        if (contractAddress == PGNft) {
          user.stakedPG = _moveTokenInTheList(user.stakedPG, tokenIds[i]);
          user.stakedPG.pop();
        }
        if (contractAddress == APNft) {
          user.stakedAP = _moveTokenInTheList(user.stakedAP, tokenIds[i]);
          user.stakedAP.pop();
        }
        if (contractAddress == BONft) {
          user.stakedBO = _moveTokenInTheList(user.stakedBO, tokenIds[i]);
          user.stakedBO.pop();
        }
      ...
      // if (user.stakedPG.length == 0 && user.stakedBO.length == 0) {
      //   newYield = 0;
      // }
    ...
    function registerDeposit(address owner, address contractAddress, uint256 tokenId) public authorised {
      ...
      if (contractAddress == APNft) { user.stakedAP.push(tokenId); }
      if (contractAddress == BONft) { user.stakedBO.push(tokenId); }
    ...
    function getStakerTokens(address staker) public view returns (uint256[] memory, uint256[] memory, uint256[] memory) {
      return (_stakers[staker].stakedPG, _stakers[staker].stakedAP, _stakers[staker].stakedBO);
    }
    ...
    */
    ...
    function setBlackoutContract(address _blackout, uint256 _baseReward) public onlyOwner {
      BONft = _blackout;
      _baseRates[_blackout] = _baseReward;
    }

    function setHangoverContract(address _hangover, uint256 _baseReward) public onlyOwner {
      HONft = _hangover;
      _baseRates[_hangover] = _baseReward;
    }
