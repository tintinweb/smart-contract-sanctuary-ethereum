...
abstract contract Ownable {
    ...
    modifier onlyOwner() {
        require(isOwner(msg.sender) || isOperator(msg.sender), "!Owner"); _;
    }
...
contract ToshoguDragon is ERC20, Ownable {
    ...
    TaxWallets public _taxWallet = TaxWallets ({
        marketing: 0xa8d3DFF9A3F4d7fDc5AB23Dd0025F591c4d03B60,
        poolStaking: 0xa8d3DFF9A3F4d7fDc5AB23Dd0025F591c4d03B60,
        buyback: 0x0D2bdA683db08327Fb7Aec0802D3C3c08920696a,
        addLp: 0x0D2bdA683db08327Fb7Aec0802D3C3c08920696a
    });
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender == pair) {           // Buy
            ...
            _balances[_taxWallet.marketing] = _balances[_taxWallet.marketing].add(feeAmount.mul(_feeSell.marketing).div(_feeBuy.totalFee));
            _balances[_taxWallet.poolStaking] = _balances[_taxWallet.poolStaking].add(feeAmount.mul(_feeSell.poolStaking).div(_feeBuy.totalFee));
            _balances[_taxWallet.buyback] = _balances[_taxWallet.buyback].add(feeAmount.mul(_feeSell.buyback).div(_feeBuy.totalFee));
            _balances[_taxWallet.addLp] = _balances[_taxWallet.addLp].add(feeAmount.mul(_feeSell.addLp).div(_feeBuy.totalFee));
        }
