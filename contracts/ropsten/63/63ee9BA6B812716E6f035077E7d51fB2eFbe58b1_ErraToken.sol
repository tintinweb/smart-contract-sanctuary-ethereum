...
contract ErraToken is OwnableUpgradeable, ERC20Upgradeable {
    ...
    function initialize() public initializer {
        __ERC20_init("Monster Era Token 1", "MET1"); // "Monster Era Token", "MET"
        __Ownable_init();

        // Initial local parameters
        _totalSupplyAtBirth = 1000000000 * 10**uint256(decimals());
        _periodInSeconds = 600; //2628000;
        _startTime = 1651413600;

        numConfirmationsRequired = 3;

        coreTeam = 0xF3E5337377EE08fA124c6eDa602348963f1B4dB6;
        playToEarn = 0x951e74E9b858D881B898dE79F6De4103E7DcE471;
        mkt = 0xde1dA257Ba2A858edB81764397D82eAAe5bFdFF9;
        lp = 0x4348605676D33848448f95F947ea6F8FE81FA9c5;
        seedSale = 0x5c5D2fc28982e11D83ebEFD9F77bc5774d87F168;
        privateSale = 0x08E7A9fc682312cFbe0f724affBe20e5896458C4;
        publicSale = 0x3De01c8685e66142e8360C118Df6059684FDD66d;
        advisor = 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db;
        fundReverse = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;
        staking = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
        ...
        lockSystemWallet(
            coreTeam,
            (_totalSupplyAtBirth * 10) / 100,
            0,
            13,
            24,
            (_totalSupplyAtBirth * 10) / 100 / 24
        );
        lockSystemWallet(
            playToEarn,
            (_totalSupplyAtBirth * 20) / 100,
            0,
            1,
            24,
            (_totalSupplyAtBirth * 20) / 100 / 24
        );
        lockSystemWallet(
            mkt,
            (_totalSupplyAtBirth * 20) / 100,
            (((_totalSupplyAtBirth * 20) / 100) * 5) / 100,
            1,
            24,
            (_totalSupplyAtBirth * 20) / 100 / 24
        );
        lockSystemWallet(
            lp,
            (_totalSupplyAtBirth * 8) / 100,
            (((_totalSupplyAtBirth * 8) / 100) * 25) / 100,
            1,
            12,
            (_totalSupplyAtBirth * 8) / 100 / 12
        );
        lockSystemWallet(
            seedSale,
            (_totalSupplyAtBirth * 3) / 100,
            (((_totalSupplyAtBirth * 3) / 100) * 5) / 100,
            4,
            20,
            (_totalSupplyAtBirth * 3) / 100 / 20
        );
        lockSystemWallet(
            privateSale,
            (_totalSupplyAtBirth * 12) / 100,
            (((_totalSupplyAtBirth * 12) / 100) * 8) / 100,
            4,
            15,
            (_totalSupplyAtBirth * 12) / 100 / 15
        );
        lockSystemWallet(
            publicSale,
            (_totalSupplyAtBirth * 2) / 100,
            (((_totalSupplyAtBirth * 2) / 100) * 12) / 100,
            2,
            3,
            (_totalSupplyAtBirth * 2) / 100 / 3
        );
        lockSystemWallet(
            advisor,
            (_totalSupplyAtBirth * 3) / 100,
            ...
            13,
            24,
            (_totalSupplyAtBirth * 3) / 100 / 24
        );
        lockSystemWallet(
            fundReverse,
            (_totalSupplyAtBirth * 7) / 100,
            0,
            7,
            15,
            (_totalSupplyAtBirth * 7) / 100 / 15
        );
        lockSystemWallet(
            staking,
            (_totalSupplyAtBirth * 15) / 100,
            (((_totalSupplyAtBirth * 15) / 100) * 8) / 100,
            1,
            36,
            (_totalSupplyAtBirth * 15) / 100 / 36
        );
    }

    function lockSystemWallet(
        address _wallet,
        uint256 _amountSum,
        uint256 gpeAmount,
        uint8 _startPeriod,
    ...
    function lockFund(
        ...
        LockItem memory item = LockItem({
            ...
            numOfPeriod: _numOfPeriod,
            amountInPeriod: _amountInPeriod
        });
    ...
    modifier requiredTransfer(
        ...
        require(
            _amount <= availableBalance(_from),
            "not enough enough fund to transfer"
        );
        _;
    }
    ...
    function indexInBlackList(address _address)
        public
        view
        onlyOwner
        returns (uint256)
    {
        require(
            _address != address(0) && isBlackList[_address],
            "invalid address"
        );
    ...
    ) public notInBlackList returns (bool) {
        require(!isOwner[msg.sender], "must be not owner");
        uint256 startBalance = balanceOf(msg.sender);
        uint256 totalTransfer = 0;
        for (uint256 i = 0; i < _addresses.length; i++) {
            require(_addresses[i] != address(0));
            ERC20Upgradeable.transfer(_addresses[i], _amounts[i]);
    ...
    function transfer(address _receiver, uint256 _amount)
        ...
        returns (bool)
    {
        ERC20Upgradeable.transfer(_receiver, _amount);
        return true;
    }
    ...
    function transferFrom(
        ...
        returns (bool)
    {
        ERC20Upgradeable.transferFrom(_from, _receiver, _amount);
        return true;
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 j = 0; j < items.length; j++) {
            if (block.timestamp >= items[j].releaseTime) {
                uint256 remaining = items[j].remainingAmount -
                    items[j].amountInPeriod;
    ...
    function availableBalance(address lockedAddress)
        private
        notInBlackList
        returns (uint256)
    {
    ...
    function addSigner(address[] memory _signers) public onlyOwner {
        ...
        for (uint256 i = 0; i < _signers.length; i++) {
            ...
            require(
                _signer != address(0) && !isSigner[_signer],
                "invalid signer"
            );
    ...
    function indexOfSigner(address _signer)
        public
        view
        onlyOwner
        returns (uint256)
    ...
    function submitTransaction(
        ...
        uint256 _value,
        bytes memory _data
    ) public onlySigner returns (uint256 _txIndex) {
