...
contract Items is ERC721, Mintable {
    ...
    constructor(address _imx) ERC721("Items", "ITEMS") Mintable(_imx) {}
    ...
    function devMint(uint256 quantity, uint256 offset) external onlyOwner {
        for (uint256 i = 0; i < quantity; i++) {
            safeMint(msg.sender, offset);
        }
    }

    function mintItems(uint256 quantity, uint256 offset) external {
    ...
    function safeMint(address to, uint256 offset) internal {
        uint256 tokenId = _itemCount.current() + offset;
