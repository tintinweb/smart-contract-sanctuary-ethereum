...
contract TwapRelayer is ITwapRelayer {
    ...
    function sell(Orders.SellParams calldata sellParams) external payable override lock returns (uint256) {
        ...
        address tokenIn = sellParams.tokenIn == ETH_ADDRESS ? weth : sellParams.tokenIn;
        address tokenOut = sellParams.tokenOut == ETH_ADDRESS ? weth : sellParams.tokenOut;
        ...
        emit Swap(msg.sender, tokenIn, tokenOut, _amountIn, _amountOut, fee, sellParams.to, orderId);
    ...
    function buy(Orders.BuyParams calldata buyParams) external payable override lock returns (uint256) {
        ...
        address tokenIn = buyParams.tokenIn == ETH_ADDRESS ? weth : buyParams.tokenIn;
        address tokenOut = buyParams.tokenOut == ETH_ADDRESS ? weth : buyParams.tokenOut;
        (address pair, bool inverted) = getPair(tokenIn, tokenOut);
        require(isPairEnabled[pair], 'TR54');
        ...
        emit Swap(msg.sender, tokenIn, tokenOut, _amountIn, _amountOut, fee, buyParams.to, orderId);
    ...
    function calculatePrepay() internal returns (uint256) {
        ...
        uint256 gasPrice = tx.gasprice - (tx.gasprice % 1e6);
        if (gasPrice == 0) {
            gasPrice = ITwapDelay(delay).gasPrice();
        }
        return ((gasPrice * gasPriceMultiplier) / PRECISION) * executionGasLimit;
    }
