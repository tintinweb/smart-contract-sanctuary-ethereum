...
contract MyRefundVault is RefundVault {
  ...
  function refundWhenNotClosed(address investor) returns(uint256 weiRefunded) {
  	require(!paused);
  ...
  function pauseRefund(bool _paused) {
  	paused = _paused;
  }
}
...
contract MyRefundableCrowdsale is FinalizableCrowdsale {
  ...
  function pauseRefund(bool _paused) public onlyOwner {
  	vault.pauseRefund(_paused);
  }
}
