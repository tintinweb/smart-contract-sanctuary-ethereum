...
contract Escrow {
	...
	// taskId => mapping (buyer => DepositType);
	...
	modifier onlyBuyer(uint256 _taskId) {
		require(partnershipOfTaskId[_taskId].buyer == msg.sender, "Only Buyer");
		_;
	}

	modifier onlySeller(uint256 _taskId) {
		require(partnershipOfTaskId[_taskId].seller == msg.sender, "Only Seller");
		_;
	}

	event SetDelivery(uint256 indexed taskId, address indexed buyer);
	event WithdrawByBuyer(uint256 indexed taskId, address indexed buyer);
	event WithdrawBySeller(uint256 indexed taskId, address indexed buyer);
	event Complete(uint256 indexed taskId);

	function startTask(address _seller) external {
		_taskIdTracker.increment();
		Partnership memory partner = Partnership(msg.sender, _seller);
		uint256 newTaskId = _taskIdTracker.current();
		partnershipOfTaskId[newTaskId] = partner;
		stateOfTaskId[newTaskId] = State.STARTED;
	}

	function setDeliveried(uint256 _taskId) external onlyBuyer(_taskId) {
	...
	function withdrawByBuyer(uint256 _taskId) external isFunded(_taskId) onlyBuyer(_taskId) {
		...
		emit WithdrawByBuyer(_taskId, msg.sender);
	}

	function withdrawBySeller(uint256 _taskId) external onlySeller(_taskId) {
		require(stateOfTaskId[_taskId] == State.DELIVERIED, "Only deliveried");
		address buyer = partnershipOfTaskId[_taskId].buyer;
		if (clientWithBNB[_taskId][msg.sender] != 0)
			payable(msg.sender).transfer(clientWithBNB[_taskId][buyer].sub(fee));
		else IERC20(clientWithCurrency[_taskId][buyer].coinAddress)
				.transfer(msg.sender, clientWithCurrency[_taskId][buyer].amount);

		stateOfTaskId[_taskId] = State.COMPLETE;

		emit Complete(_taskId);
		emit WithdrawBySeller(_taskId, msg.sender);
