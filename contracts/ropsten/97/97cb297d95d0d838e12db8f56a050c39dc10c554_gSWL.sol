...
// File: interfaces/IsSWL.sol


pragma solidity >=0.7.5;


interface IsSWL is IERC20 {
    function rebase( uint256 SWLProfit_, uint epoch_) external returns (uint256);
...
contract gSWL is IgSWL, ERC20 {
    ...
    constructor(address _sSWL)
        ERC20("Governance SWL", "gSWL", 18)
    {
        require(_sSWL != address(0), "Zero address: sSWL");
        sSWL = IsSWL(_sSWL);
    }
     ...
     * @param _sSWL address
     */
    function migrate(address _staking, address _sSWL) external override onlyApproved {
        ...
        require(_sSWL != address(0), "Zero address found");
        sSWL = IsSWL(_sSWL);
    }
    ...
    /**
     * @notice pull index from sSWL token
     */
    ...
    /**
        @notice converts gSWL amount to SWL
        @param _amount uint
        @return uint
     */
    ...
    /**
        @notice converts SWL amount to gSWL
        @param _amount uint
        @return uint
     */
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "gSWL::getPriorVotes: not yet determined");
