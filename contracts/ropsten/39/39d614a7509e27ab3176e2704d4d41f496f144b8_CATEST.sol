...
contract CATEST is Context, IERC20, Ownable {
    ...
    // 
    string private _name = "CTEST";                                           // name
    string private _symbol = "$CTEST";                                            // symbol
    uint256 private _tTotal = 1000 * 10**9 * 10**uint256(_decimals);

    // % to holders
    uint256 public ReflectionTaxFee = 1;
    uint256 private _taxFee = ReflectionTaxFee;
    uint256 private _previousTaxFee = _taxFee;

    // % to swap & send to marketing wallet
    uint256 private BuyMarketingFee = 9;
    uint256 private SellMarketingFee = 9;
    uint256 public PerTxMarketFee = BuyMarketingFee;
    uint256 private _previousMarketingFee = PerTxMarketFee;
    ...
    address payable public marketingWallet = payable(0x3e3D0c97e4003255445de37d2DcD1b9687E65D48);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && PerTxMarketFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousMarketingFee = PerTxMarketFee;
    ...
    function restoreAllFee() private {
        ...
        PerTxMarketFee = _previousMarketingFee;
    }
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
     ...
     function AddBot(address[] calldata addresses) external onlyOwner {
    ...
    function calculateMarketingFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(PerTxMarketFee).div(
            10**2
        );
    }
    ...
    function setFees(address recipient) private {
        _taxFee = ReflectionTaxFee;
        PerTxMarketFee = BuyMarketingFee;
        if (recipient == uniswapV2Pair) { // sell
            PerTxMarketFee = SellMarketingFee;
        }
    }
    ...
    function BuyMarketingFees(uint256 MarketingFee) external onlyOwner() {
        BuyMarketingFee = MarketingFee;
    }

    function SellMarketingFees(uint256 _SellMarketingFee) external onlyOwner() {
