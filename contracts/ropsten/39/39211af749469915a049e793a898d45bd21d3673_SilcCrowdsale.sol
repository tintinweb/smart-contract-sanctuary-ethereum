...
contract MyRefundVault is RefundVault, Pausable {
  ...
  function refundWhenNotClosed(address contributor) public whenNotPaused returns(uint256 weiRefunded) {
...
contract SilcCrowdsale is CappedCrowdsale, MyRefundableCrowdsale {
  ...
  function () external payable {
      ...
      if (stage == CrowdsaleStage.phase1) {
        totalWeiRaisedDuringPhase1 = totalWeiRaisedDuringPhase1 + int256(msg.value);
      } else if (stage == CrowdsaleStage.phase2) {
        totalWeiRaisedDuringPhase2 = totalWeiRaisedDuringPhase2 + int256(msg.value);
      } else if (stage == CrowdsaleStage.phase3) {
        totalWeiRaisedDuringPhase3 = totalWeiRaisedDuringPhase3 + int256(msg.value);
      }
  }
  ...
  event RefundRequestCompleted(address contributor, uint256 weiRefunded);
  function refundRequest() public {
    ...
    silcToken.burn(contributor, token.balanceOf(contributor));
    ...
    if (stage == CrowdsaleStage.phase1) {
      totalWeiRaisedDuringPhase1 = totalWeiRaisedDuringPhase1 - int256(weiRefunded);
    } else if (stage == CrowdsaleStage.phase2) {
      totalWeiRaisedDuringPhase2 = totalWeiRaisedDuringPhase2 - int256(weiRefunded);
    } else if (stage == CrowdsaleStage.phase3) {
      totalWeiRaisedDuringPhase3 = totalWeiRaisedDuringPhase3 - int256(weiRefunded);
    }

    RefundRequestCompleted(contributor, weiRefunded);
  }
