...
contract GabrielV2 is Ownable {
    ...
    event PercentsUpdated(uint dev, uint growthFund);
    event ReflectionsClaimed(uint pid, address token, uint amount);
    event ForfeitedRewardsClaimed(uint pid, address token, uint amount);
    ...
    function setTotalRewards(uint _pid, uint[] memory rewards) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        uint NORT = pool.NORT;
        require(rewards.length == NORT, "STR: array length mismatch");
        for (uint i = 0; i < NORT; i++) {
    ...
    function stakerList(uint _pid) external view returns (address[] memory userList) {
        PoolInfo memory pool = poolInfo[_pid];
    ...
    function reusePool(uint _pid, ExtraArgs memory extraArgs, uint _NORT, address[] memory _rewardTokens, uint[] memory _totalRewards) external onlyOwner {
        ...
        address[] memory userList = pool.userList;
        uint len = userList.length;
        for (uint i; i < len; i++) {
            UserInfo storage user = userInfo[_pid][userList[i]];
            user.harvested = false;
        }
        address[] memory resetUserList;
        pool.userList = resetUserList;
    }
        ...
        returns (uint[] memory unclaimedReward)
    {
        ...
        if (block.timestamp > pool.lockTime && block.timestamp < pool.unlockTime && pool.staked != 0) {
            ...
            return array;
        } else if (block.timestamp > pool.unlockTime && user.harvested == false) {
            uint[] memory array = new uint[](NORT);
            for (uint i = 0; i < NORT; i++) {                
    ...
    function harvest(uint _pid) external {
        ...
        pool.userList.push(msg.sender);
        updatePool(_pid);
        uint NORT = pool.NORT;
        for (uint i = 0; i < NORT; i++) {
            uint reward = pool.lockDuration * user.amount * pool.totalRewards[i];
            uint lpSupply = pool.lockDuration * pool.staked;
            uint pending = reward.div(lpSupply);
            if (pending > 0) {
                uint forfeitedRwds = (pool.lockDuration *
                    pool.unstaked *
                    pool.totalRewards[i]) / (pool.lockDuration * pool.staked);
        ...
        pool.tvl = pool.tvl.sub(user.amount);
        user.harvested = true;
    }
    ...
    function unstake(uint _pid) external {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount > 0, "EW: withdraw bad");
     ...
     * @param token address of the ERC20 to to be released
     * @param recipient address that receives the recovered tokens
     * @param amount amount of tokens to be recovered
     */
    function release(address token, address recipient, uint amount) external onlyOwner {
    ...
    function withdraw(uint _pid, bool forfeited, address token, uint amount) external onlyOwner {
        // Calculate LOR.
        uint onePercent = amount.div(100);
        uint devShare = devPercent.mul(onePercent);
        uint gfShare = amount.sub(devShare);
        IERC20(token).safeTransfer(devaddr, devShare);
        IERC20(token).safeTransfer(growthFundAddr, gfShare);
        ...
        uint NORT = pool.NORT;
        for (uint i = 0; i < NORT; i++) {
            // Calculate forfeited reward.
            uint rewardsInPool = forfeited ? pool.rewardsInPool[i] : 0;
            if (rewardsInPool > 0 && block.timestamp > pool.unlockTime && pool.tvl == 0) {
                uint poolBal = IERC20(pool.rewardToken[i]).balanceOf(address(this));
                if (rewardsInPool > poolBal) {
                    ...
                    IERC20(pool.rewardToken[i]).safeTransfer(growthFundAddr, rewardsInPool);
                    emit ForfeitedRewardsClaimed(_pid, pool.rewardToken[i], rewardsInPool);
                }
            }
        }
    }
