...
contract Keungz is ERC721x {
    uint256 public MAX_TOKENS_SUPPLY;
    ...
    // Traits must either has a price or be free. This prevents invalid traits
    mapping(string => uint256) public desiredTraitPrice;
    mapping(string => bool) public isTraitFree;

    // Records what traits of a token the minter requires
    // Should we fetch from calldata instead to save gas?
    // mapping(uint256 => string[]) tokendesiredTraits;

    // Some trait has a limited supply (SSR). Let's check in contract
    // ssrTraitLimit[dt] == 0 is unlimited, vice versa
    mapping(string => uint256) public ssrTraitLimit; // ssrTraitLimit["ssr trait"] => 20
    mapping(string => uint256) public ssrTraitAmount; // ssrTraitAmount["ssr trait"] => 0, 1, 2... etc

    // If a token has this trait, it's genesis
    mapping(string => bool) public isGenesisTrait;
    // And mark the token ID
    mapping(uint256 => bool) public isTokenIDGenesis;
    ...
    function initialize(address _signer, string memory baseURI)
        ...
        initializer
    {
        ERC721x.__ERC721x_init("Keungz", "Keungz");
        MAX_TOKENS_MINTED_PER_ADDRESS = 100; // max mint per address
        MAX_TOKENS_SUPPLY = 10000; // max supply
        baseTokenURI = baseURI;
        signer = _signer;
        mintStartAfter = 0; // disabled as of contract creation
    }
    ...
    function checkValidity(
        ...
        require(
            ECDSA.toEthSignedMessageHash(
                keccak256(
                    abi.encodePacked(msg.sender, "mint", validUntil, allowSSR)
                )
            ) == hash,
            "invalid hash"
        );
    ...
    function setNewSupply(uint256 newMax) external onlyOwner {
        require(newMax >= totalSupply());
        require(newMax < MAX_TOKENS_SUPPLY);
        MAX_TOKENS_SUPPLY = newMax;
    }
    ...
    function getMintPrice(string[] memory desiredTraits, bool allowSSR)
        public
        view
        returns (uint256, bool)
    {
        // No need for desiredTraits.length check
        uint256 price = 0;
        // mapping(string => bool) memory dupCheck;
        string[] memory dupCheck = new string[](desiredTraits.length);
        uint256 idx = 0;
        bool isGenesis = false;
        for (uint256 i = 0; i < desiredTraits.length; i++) {
            string memory dt = desiredTraits[i];
            require(
                allowSSR || ssrTraitLimit[dt] == 0,
                "SSR trait not allowed"
            );
            uint256 p = desiredTraitPrice[dt];
            require(p > 0 || isTraitFree[dt], "Trait not found");
            if (allowSSR && ssrTraitLimit[dt] > 0) {
                // check for duplicate SSR traits because of ssrTraitLimit
                // explicitly not to check non-SSR traits because no incentive + save gas
                for (uint256 j = 0; j < dupCheck.length; j++) {
                    string memory ddt = dupCheck[j];
                    require(!strcmp(ddt, dt), "Duplicate trait");
                }
                dupCheck[idx++] = dt;
                // require(!dupCheck[dt], "Duplicate trait");
                // dupCheck[dt] = true;
            }
            if (!isGenesis && isGenesisTrait[dt]) {
                isGenesis = true;
            }
            price += p;
        }
        return (price, isGenesis);
    }
    ...
    function mint(
        ...
        // whitelist/SSR quota check
        require(
            checkValidity(hash, signature, validUntil, allowSSR),
            "invalid"
        );
        require(
            totalSupply() + 1 <= MAX_TOKENS_SUPPLY,
            "exceed MAX_TOKENS_SUPPLY"
        );
        require(
            tokensMintedPerAddress[msg.sender] + 1 <=
                MAX_TOKENS_MINTED_PER_ADDRESS,
            "tokensMintedPerAddress exceed MAX_TOKENS_MINTED_PER_ADDRESS"
        );
        (uint256 price, bool isGenesis) = getMintPrice(desiredTraits, allowSSR);
        require(price == msg.value, "incorrect ETH");

        if (allowSSR) {
            for (uint256 i = 0; i < desiredTraits.length; i++) {
                string memory dt = desiredTraits[i];
                if (ssrTraitLimit[dt] > 0) {
                    require(
                        ssrTraitAmount[dt] + 1 <= ssrTraitLimit[dt],
                        "exceed SSR trait limit"
                    );
                    ssrTraitAmount[dt] += 1;
                }
            }
        ...
        if (isGenesis) {
            isTokenIDGenesis[tokenId] = true;
        }
        tokensMintedPerAddress[msg.sender] += 1;
        // tokendesiredTraits[tokenId] = desiredTraits;
        _safeMint(msg.sender, 1);
    }

    function airdrop(address recipient, uint256 amount) external onlyOwner {
        require(
            totalSupply() + amount <= MAX_TOKENS_SUPPLY,
            "exceed MAX_TOKENS_SUPPLY"
        );
    ...
    function updateDesiredTraitPrice(
        string[] memory traits,
        uint256[] memory prices
    ) external onlyOwner {
        for (uint256 i = 0; i < traits.length; i++) {
            string memory dt = traits[i];
            uint256 p = prices[i];
            desiredTraitPrice[dt] = p;
        }
    }

    function updateIsTraitFree(string[] memory traits, bool[] memory isFree)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < traits.length; i++) {
    ...
    function isTokenGenesis(uint256 tokenId) external view returns (bool) {
        require(ownerOf(tokenId) != address(0));
        return isTokenIDGenesis[tokenId];
    }
