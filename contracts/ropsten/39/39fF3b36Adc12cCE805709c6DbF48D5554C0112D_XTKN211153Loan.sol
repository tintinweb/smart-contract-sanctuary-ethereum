...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn211153][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn211153][ownable] new owner is the zero address");
...
abstract contract RoleBasedAccessControl is Context, Ownable{
    ...
    // modifiers
        ...
        modifier onlyRole(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()], "[xtkn211153][role based access control] only addresses assigned this role can access this function!");
            _;
        }

        modifier onlyRoles(string[] memory pRoles){
            for(uint256 i=0; i<pRoles.length; i++){
                require(_roleToAddress[pRoles[i]][_msgSender()], "[xtkn211153][role based access control] only addresses assigned this role can access this function!");
            }
            _;
        }
        ...
        modifier onlyRolesOr(string[] memory pRoles){
            ...
            require(rolePresent, "[xtkn211153][role based access control] only addresses assigned this role can access this function!");
            _;
        }

        modifier onlyRoleOrOwner(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()] || owner() == _msgSender(), "[xtkn211153][role based access control] only addresses assigned this role or the owner can access this function!");
            _;
        }
...
*/
contract XTKN211153Loan is Context, Ownable, ReentrancyGuard, RoleBasedAccessControl{
    // lib
    ...
    // storage
        ...
        mapping(uint16 => IXTKN211153LoanProxy) private _proxy;
    ...
    constructor(address pToken){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            _main = IXTKN211153(ADDRESS_TOKEN);
    ...
    function init(address pProxy180D, address pProxy365D) public onlyRole("root"){
        // 180 D
            _proxy[180] = IXTKN211153LoanProxy(pProxy180D);
        ...
        // 365 D
            _proxy[365] = IXTKN211153LoanProxy(pProxy365D);
    ...
    function info(uint16 pPool, address pWallet) public view returns(
        ...
        require(pPool == 180 || pPool == 365, "[xtkn211153][loan] this pool does not exist!");
        require(LOANS[pPool][pWallet].tokens > 0, "[xtkn211153][loan] this wallet has no loans!");
        return(
    ...
    function loan(uint256 pAmount, uint16 pPool) public nonReentrant{
        require(pPool == 180 || pPool == 365, "[xtkn211153][loan] this pool does not exist!");
        require(WALLETS < (MAX_WALLETS - 1), "[xtkn211153][loan] no more wallets allowed to loan!");
        require(pAmount >= 10000 * (10**9), "[xtkn211153][loan] you need to at least loan 10k tokens!");
        require(LOAN_TOKENS > 0, "[xtkn211153][loan] no more tokens to loan!");
        require(_pool(pPool), "[xtkn211153][loan] this staking pool has ende or is full!");
        ...
        require(allowance >= priceForTheLoan, "[xtkn211153][loan] you have to authorize the transfer of your USDC first!");
        require(balance >= priceForTheLoan, "[xtkn211153][loan] you do not have enough USDC!");
        require(_buy(priceForTheLoan), "[xtkn211153][loan] transfer of USDC failed!");
    ...
    function claim(uint16 pPool) public nonReentrant{
        ...
        require(_token.transfer(withdrawWallet, totalLoanInterest), "[xtkn211153][loan] token transfer failed!");    
    }
    ...
    event DebugShare(uint256 cake, uint256 slice, uint256 interest, address wallet, uint256 tokens);
    event DebugShareSlice(uint256 slice, address wallet, uint256 interest);
    function _share(uint16 pPool, address[] memory pWallets) private{
        ...
        for(uint256 i=0; i<pWallets.length; i++){
            ...
            emit DebugShare(cake, slice, POOLS[pPool].interest, pWallets[i], LOANS[pPool][pWallets[i]].tokens);
            if(slice <= POOLS[pPool].interest){
                ...
                emit DebugShareSlice(slice, pWallets[i], POOLS[pPool].interest);
            }else{
                LOANS[pPool][pWallets[i]].interest.add(POOLS[pPool].interest);
                POOLS[pPool].interest = 0;
                emit DebugShareSlice(POOLS[pPool].interest, pWallets[i], POOLS[pPool].interest);
            }
        }
    }
