...
contract PaybLaunchPad is Ownable, ReentrancyGuard {
    ...
    // max stakes seen for each launchpad -- (launchpad) => max stake seen on launchpad
    mapping(uint24 => uint104) public launchpadMaxStakes;
    ...
    // check already set winners-- (launchpad) => bool(is installed)
    mapping (uint24 => bool) isSetWinners;
    ...
    function stake(uint24 launchpadId, uint104 amount) external nonReentrant {
        ...
        // get latest launchpad cp
        LaunchpadCheckpoint memory LaunchpadCp = launchpadCheckpoints[launchpadId][
            launchpadCheckpointCount[launchpadId] - 1
        ];

        // update launchpad max staked
        if (launchpadMaxStakes[launchpadId] < LaunchpadCp.totalStaked) {
            launchpadMaxStakes[launchpadId] = LaunchpadCp.totalStaked;
        }
    ...
    function unstake(uint24 launchpadId) external nonReentrant {
        ...
        //check expired stake
        require(block.timestamp > launchpad.endTime, "staking is not over yet");

        // require not winners
        require(!winners[launchpadId][_msgSender()], 'not winners');
    ...
    function setWinners(uint24 launchpadId, address[] memory _winners) public onlyOwner() nonReentrant{
        ...
        //check only set winners once
        require(!isSetWinners[launchpadId], 'already set');

        for(uint256 i=0; i< _winners.length; ++i){
            winners[launchpadId][_winners[i]] = true;
        }

        isSetWinners[launchpadId]= true;
    }
    ...
    function claim(uint24 launchpadId) external nonReentrant{
        ...
        //check is winners
        require(winners[launchpadId][_msgSender()], 'not losser');
    ...
    function amountRefundToken(uint24 launchpadId) public view returns(uint256) {
        // get launchpad info
        LaunchpadInfo storage launchpad = launchpads[launchpadId];
        ...
        //check amount staked
        require(checkpoint.staked >= launchpad.minTotalStake, 'invalid staked checkpoint');

        uint256 amountRefund = checkpoint.staked - launchpad.minTotalStake;
        if(!winners[launchpadId][_msgSender()]){
            amountRefund = 0;
        }
        return amountRefund;
    }
