...
contract TESecurityToken is MintableToken, Pausable {

	string public constant name = "EUR Reto";
	string public constant symbol = "EURT";
	string public companyURI = "https://www.euroreto.io";
	...
	onlyOwner
	{
		...
		emit LogAddToWhilelist(msg.sender, investor);
	}

	function multipleAddToWhitelist(address[] investors) 
	external 
	onlyOwner
	{
		for (uint256 i = 0; i < investors.length; i++) {
		    this.addToWhitelist(investors[i]);
		}
	}
	...
	onlyOwner
	{
		...
		emit LogRemoveFromWhilelist(msg.sender, investor);
	}
	...
	returns (bool)
	{
		bool ret = super.mint(to, amount);
		return ret;
	}

	function multipleMint(address[] investors, uint256[] attributed) 
	public 
	onlyOwner 
	whenNotPaused
	returns (bool success) 
	{
		require(investors.length == attributed.length, "The size of the attributed array does not correspond to the size of the investors array");

		for (uint256 i = 0; i < investors.length; i++) {
		    mint(investors[i], attributed[i]);
		}
		return true;
	}
