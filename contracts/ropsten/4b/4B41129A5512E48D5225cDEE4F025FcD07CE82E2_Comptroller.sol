...
contract Comptroller is ComptrollerV6Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function liquidateCalculateSeizeTokens(address sTokenBorrowed, address sTokenCollateral, uint actualRepayAmount) external view returns (uint, uint) {
        ...
        (mathErr, numerator) = mulExp(totalLiquidation, priceBorrowedMantissa);
        if (mathErr != MathError.NO_ERROR) {
            return (uint(Error.MATH_ERROR), 0);
        }
    ...
    function liquidateCalculateSeizeTokensForLiquidator(address sTokenBorrowed, address sTokenCollateral, uint actualRepayAmount) external view returns (uint, uint) {
        /* Read oracle prices for borrowed and collateral markets */
        uint priceBorrowedMantissa = oracle.getUnderlyingPrice(SToken(sTokenBorrowed));
        uint priceCollateralMantissa = oracle.getUnderlyingPrice(SToken(sTokenCollateral));
        if (priceBorrowedMantissa == 0 || priceCollateralMantissa == 0) {
...
contract ComptrollerInterface {
    ...
    function liquidateCalculateSeizeTokensForLiquidator(
        address sTokenBorrowed,
        address sTokenCollateral,
        uint repayAmount) external view returns (uint, uint);
}
...
contract ComptrollerErrorReporter {
    ...
    enum FailureInfo {
        ...
        SET_ADMIN_INCENTIVE_OWNER_CHECK,
        SET_ADMIN_INCENTIVE_VALIDATION
    }
...
contract SToken is STokenInterface, Exponential, TokenErrorReporter {
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, STokenInterface sTokenCollateral) internal returns (uint, uint) {
        ...
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(sTokenCollateral), _liquidator, _borrower, repayAmount);
        if (allowed != 0) {
        ...
        (uint repayBorrowError, uint actualRepayAmount) = repayBorrowFresh(_liquidator, _borrower, repayAmount);
        ...
        require(sTokenCollateral.balanceOf(_borrower) >= seizeTokens, "LIQUIDATE_SEIZE_TOO_MUCH");

        // If this is also the collateral, run seizeInternal to avoid re-entrancy, otherwise make an external call
        uint seizeError;
        if (address(sTokenCollateral) == address(this)) {
            seizeError = seizeInternal(address(this), _liquidator, _borrower, seizeTokens, seizeTokensForLiquidator);
        } else {
            seizeError = sTokenCollateral.seize(_liquidator, _borrower, seizeTokens, seizeTokensForLiquidator);
        }
        ...
        emit LiquidateBorrow(_liquidator, _borrower, actualRepayAmount, address(sTokenCollateral), seizeTokens);

        /* We call the defense hook */
        comptroller.liquidateBorrowVerify(address(this), address(sTokenCollateral), _liquidator, _borrower, actualRepayAmount, seizeTokens);
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function seize(address liquidator, address borrower, uint seizeTokens, uint seizeTokensForLiquidator) external nonReentrant returns (uint) {
        return seizeInternal(msg.sender, liquidator, borrower, seizeTokens, seizeTokensForLiquidator);
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens, uint seizeTokensForLiquidator) internal returns (uint) {
...
contract STokenInterface is STokenStorage {
    ...
    function getCash() external view returns (uint);
    function accrueInterest() public returns (uint);
    function seize(address liquidator, address borrower, uint seizeTokens, uint seizeTokensForLiquidator) external returns (uint);
