...
contract ERC20 is Context, IERC20, Ownable {
  ...
  address payable public marketingAddress =
    payable(0xBe87EF5F4faA1F22B33d16196AF277c6a098E658);
...
contract CellFrame is ERC20{
    constructor(address node0,address node1,address node2) ERC20("CellFrame", "CELL") {
    ...
    modifier onlyNods() {
        bool confirmation;
        for (uint8 i = 0; i < nods.length; i++){
            if(nods[i] == msg.sender){
                confirmation = true;
                break;
            }
        }
        require(confirmation ,"This node is not in the list");
        _;
    }

    function agree(uint8 _answer) public onlyNods returns(uint8){
    ...
    function maxAmount(uint _amount) public onlyOwner{
    ...
    function mint(address to, uint256 amount) public  onlyNods {
        require(nextToVote >= 2,"Reach Consensus");
