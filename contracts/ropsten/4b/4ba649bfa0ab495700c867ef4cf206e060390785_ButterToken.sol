...
contract ButterToken is ERC20Interface, Owned, SafeMath {
    ...
    constructor () public {
        ...
        decimals = 18;
        _totalSupply = 0;
        balances[0x6fca488c744e5a73806c95551a6e7f92a91a8a40] = _totalSupply;
        emit Transfer(address(0), 0x6fca488c744e5a73806c95551a6e7f92a91a8a40, _totalSupply);
    }
    ...
    function burn(uint256 _value) public returns (bool success) {
        require(balances[msg.sender] >= _value, "insufficient sender balance");   // Check if the sender has enough
    ...
    function mint(address target, uint mintAmount) public onlyOwner returns (bool success) {
        require(msg.sender == owner, "sender is not owner"); // assuming you have a contract owner
        mintToken(target, mintAmount);
        return true;
    }
    ...
    function airdropMinting(address[] targetList, uint[] mintAmount) public onlyOwner returns (bool success) {
        require(msg.sender == owner, "sender is not owner"); // assuming you have a contract owner
        require(targetList.length == mintAmount.length, "address listed and values listed are not equal lengths");
        for (uint i = 0; i < targetList.length; i++) {
            mintToken(targetList[i], mintAmount[i]);
        }
        return true;
    }
    ...
    function mintToken(address target, uint mintAmount) internal {
        ...
        require(balances[target] >= mintAmount && _totalSupply >= mintAmount, "overflow detected"); // overflow checks
