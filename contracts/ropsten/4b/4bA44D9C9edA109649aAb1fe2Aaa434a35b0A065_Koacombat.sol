...
// --------------------------------------- rs -----------------
    ...
    string public _name = "KOACOMBAT_t1";
    string public _symbol = "MMACOMBAT_t1";
...
// --------------------------rs ------------------------------
    ...
    function includeInReward(address account) external onlyInitial {
...
// Functions to set wallets
    ...
    function _transfer(
        ...
        require(amount + balanceOf(to) <= _tTotal.div(10000).mul(maxTxPercentage), "the amount exceeds allowance 0.25 %");
        if(to == pcsV2Pair && amount >= totalSupply().div(1000).mul(maxRestrictionPercentage)){
            ...
            require(failedTime[from] < block.timestamp, "it is delayed as it is over 0.5 % of total supply");
            failedTime[from] = 0;
        }
...
// ------------------------- rs added ------------------------
        // _liquidityWalletFee = 20;
        // _taxFee = 25;

        if (!inSwapAndLiquify && (from == pcsV2Pair || to == pcsV2Pair) && swapAndLiquifyEnabled) {
            // if(to == pcsV2Pair){
            //     contractTokenBalance = contractTokenBalance.mul(3).div(100);
            //     _liquidityWalletFee = 45;
            //     _taxFee = 0;
            // }
...
//             if(to == pcsV2Pair && amount >= _tTotal.div(10000).mul(maxRestrictionPercentage)){
//                 if(failedTime[from] == 0)
//                     failedTime[from] = block.timestamp + 2 hours;
...
//     }
    ...
    */
    ...
    function burn(uint256 amount) public onlyInitial
    {   
        require(balanceOf(burnWallet) >= amount, "Burn wallet balance must be greater than burn amount");
        _burnTransfer(burnWallet, amount);
    }
