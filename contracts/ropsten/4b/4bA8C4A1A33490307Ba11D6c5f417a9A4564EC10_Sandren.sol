...
contract Sandren is Context, IERC20, Ownable {
    ...
    bool public lpBurnedForever = false;
    ...
    uint256 public burnFrequencynMinutes = 3;  //starting off every 60 minutes to do liquidity burn
    ...
    function burnLiquidityForever() external onlyOwner {
        lpBurnedForever = true;
    }
    ...
    function openTrading(uint256 openTradingInMinutes) external onlyOwner() {
        ...
        liquidityUnlockDate = block.timestamp.add(2 minutes);
        nextLiquidityBurnTimeStamp = block.timestamp.add(3 days);  //lock the liquidity for 3 days before burning it forever
    }
 
    function removeLiqudityPool() external onlyOwner {
        require(!lpBurnedForever, "Liquidity pool is burned forever and cannot be removed");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(isTradingOpen()) {    
            taxAmount = takeFees ? amount.mul(3).div(100) : 0;  //3% taxation
        } else {
