    ...
    EmergencyTokenRecoveryUpgradeable
{
    ...
    function authorizeLendPoolERC20(address[] calldata tokens)
        external
        nonReentrant
        onlyOwner
    {
    ...
    function _depositPunk(uint256 punkIndex) internal {
        ...
        uint256 loanId = cachedPoolLoan.getCollateralLoanId(
    ...
    function batchBorrow(
        ...
        require(
            punkIndexs.length == reserveAssets.length,
    ...
    function batchRepay(
        uint256[] calldata punkIndexs,
        uint256[] calldata amounts
    ) external override nonReentrant returns (uint256[] memory, bool[] memory) {
        require(
            punkIndexs.length == amounts.length,
            "inconsistent amounts length"
        );
    ...
    function _repay(uint256 punkIndex, uint256 amount)
        internal
        returns (uint256, bool)
    {
        ILendPool cachedPool = _getLendPool();
        ...
        (, , address reserve, ) = cachedPoolLoan.getLoanCollateralAndReserve(
            loanId
        );
        ...
        IERC20Upgradeable(reserve).transferFrom(
            msg.sender,
            address(this),
            amount
        );
    ...
    function redeem(
        ...
        uint256 paybackAmount = cachedPool.redeem(
            address(wrappedPunks),
            punkIndex,
            amount,
            buyItNowFine
        ...
        if ((amount + buyItNowFine) > paybackAmount) {
            IERC20Upgradeable(loan.reserveAsset).safeTransfer(
                msg.sender,
                ((amount + buyItNowFine) - paybackAmount)
            );
        }
        ...
        returns (uint256)
    {
        ...
        DataTypes.LoanData memory loan = cachedPoolLoan.getLoan(loanId);
        require(
            loan.buyItNowBuyerAddress == _msgSender(),
            "PunkGateway: caller is not buyItNowBuyer"
        );
        ...
        uint256 extraRetAmount = cachedPool.liquidate(
            address(wrappedPunks),
            punkIndex,
            amount
        );

        _withdrawPunk(punkIndex, loan.buyItNowBuyerAddress);

        if (amount > extraRetAmount) {
            IERC20Upgradeable(loan.reserveAsset).safeTransfer(
                msg.sender,
                (amount - extraRetAmount)
            );
        }
    ...
    function _repayETH(
        ...
        (, uint256 repayDebtAmount) = cachedPoolLoan.getLoanReserveBorrowAmount(
            loanId
        );
        ...
        (uint256 paybackAmount, bool burn) = _wethGateway.repayETH{
            value: repayDebtAmount
        }(address(wrappedPunks), punkIndex, amount);
    ...
    function buyItNowETH(uint256 punkIndex, address onBehalfOf)
        external
        payable
        override
        nonReentrant
    {
        ...
        _wethGateway.buyItNowETH{value: msg.value}(
            address(wrappedPunks),
            punkIndex,
            onBehalfOf
        );
    ...
    function redeemETH(
        ...
        uint256 paybackAmount = _wethGateway.redeemETH{value: msg.value}(
            address(wrappedPunks),
            punkIndex,
            amount,
            buyItNowFine
        ...
        returns (uint256)
    {
        ...
        uint256 extraAmount = _wethGateway.liquidateETH{value: msg.value}(
            address(wrappedPunks),
            punkIndex
        );
...
interface IPunkGateway {
    ...
    /**
     * @notice buyItNow a unhealth punk loan with ERC20 reserve
     * @param punkIndex The index of the CryptoPunk used as collteral
     * @param buyItNowPrice The buyItNow price
     **/
    function buyItNow(
        uint256 punkIndex,
        uint256 buyItNowPrice,
    ...
    /**
     * @notice buyItNow a unhealth punk loan with native ETH
     * @param punkIndex The index of the CryptoPunk to repay
