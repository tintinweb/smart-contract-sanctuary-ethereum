...
contract TestFeature is IFeature, ITestFeature, FixinCommon {
    ...
    function setHello(string calldata name_) external override {
        if (keccak256(abi.encodePacked(("Rick"))) == keccak256(abi.encodePacked((name_))))
            revert RickNotAllowed(block.timestamp);
    ...
    function map(address key) external view returns (MapContents memory) {
        return LibTestStorage.getStorage().testMap[key];
    }

    function setMap(address key, MapContents calldata value) external {
...
library LibTestStorage {
    ...
    struct Storage {
        ...
        string name;
        address lastAddress;
        mapping(address => ITestFeature.MapContents) testMap;
    }
...
interface ITestFeature {
    ...
    error RickNotAllowed(uint256 timestamp);
}
