...
contract V1 is GSNRecipient, Ownable, ERC20, Pausable, Blacklistable {
        ...
        returns (bool)
    {
        _approve(
            _msgSender(),
            _spender,
            allowed[_msgSender()][_spender].sub(_subtractedValue)
        );
        return true;
    }
     ...
     */
    function updateGsnFee(uint256 _newGsnFee) public onlyOwner {
    ...
    function acceptRelayedCall(
        ...
        if (calldataSelector == this.transfer.selector) {
            uint256 valuePlusGsnFee = gsnFee.add(
                uint256(LibBytes.readBytes32(encodedFunction, 36))
            );
            ...
            if (userBalance >= valuePlusGsnFee) {
                ...
                return
                    _rejectRelayedCall(
                        uint256(GSNErrorCodes.INSUFFICIENT_BALANCE)
                    );
            }
        } else if (
            calldataSelector == this.transferFrom.selector ||
            calldataSelector == this.approve.selector
        ) {
