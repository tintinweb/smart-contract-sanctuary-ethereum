...
/**
 * @title Sgame token
 * @author Validity Labs AG <<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="cda4a3aba28dbbaca1a4a9a4b9b4a1acafbee3a2bfaa">[email&#160;protected]</a>>
 */
 ...
 * @author Validity Labs AG <<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="ea83848c85aa9c8b86838e839e93868b8899c485988d">[email&#160;protected]</a>>
 */
contract TokenVesting is Ownable {
    ...
    // key: beneficiary address; value: index in beneficiaries array.
    mapping(address => uint256) private beneficiaryLUT;
    ...
    function setState (address[] accounts, uint256[] balances, address token, uint256 start) public onlyOwner {
        ...
        for (uint256 i = 0; i < length; i = i.add(1)) {
            beneficiaryLUT[accounts[i]] = beneficiaries.length;
            beneficiaries.push(Beneficiary(balances[i], 0));
    ...
    function releaseFor(address beneficiary) public {
        ...
        uint256 beneficiaryIndex = beneficiaryLUT[beneficiary];
        beneficiaries[beneficiaryIndex].released = beneficiaries[beneficiaryIndex].released.add(unreleased);
        _token.safeTransfer(beneficiary, unreleased);
    }

    /**
     * @dev Allows the caller to check his/her balance that has not been released
     */
    function getBalance() public view returns (uint256) {
        uint256 beneficiaryIndex = beneficiaryLUT[msg.sender];
        return beneficiaries[beneficiaryIndex].totalBalance.sub(beneficiaries[beneficiaryIndex].released);
    }
    ...
    function releasableAmount(address beneficiary) private view returns (uint256) {
        uint256 beneficiaryIndex = beneficiaryLUT[beneficiary];
        return vestedAmount(beneficiary).sub(beneficiaries[beneficiaryIndex].released);
    }
    ...
    function vestedAmount(address beneficiary) private view returns (uint256) {
        ...
        if (block.timestamp >= _start) {
            ...
            vested = beneficiaries[beneficiaryIndex].totalBalance.mul(FIRST_RELEASE_PERCENTAGE).div(D_RELEASE);
        }
        if (block.timestamp >= _cliff && block.timestamp < _start.add(DURATION)) {
            // after cliff, continuous vesting of the 70% leftover
            uint256 amountToVest = beneficiaries[beneficiaryIndex].totalBalance.mul(VESTING_PERCENTAGE).div(D_RELEASE);
            vested = vested.add(amountToVest.mul(block.timestamp.sub(_start)).div(DURATION));
        }
        if (block.timestamp >= _start.add(DURATION)) {
            vested = beneficiaries[beneficiaryIndex].totalBalance;
        }
        return vested;
    }
}
