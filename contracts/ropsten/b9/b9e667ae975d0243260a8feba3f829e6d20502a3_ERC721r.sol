...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _mintRandom(uint _numToMint) public virtual{
        ...
        for (uint256 i; i < _numToMint; ++i) { // Do this ++ unchecked?
            uint256 tokenId = getRandomAvailableTokenId(_msgSender(), updatedNumAvailableTokens);
            
            _mintIdWithoutBalanceUpdate(_msgSender(), tokenId);
            
            --updatedNumAvailableTokens;
        }
        
        _numAvailableTokens = updatedNumAvailableTokens;
        _balances[_msgSender()] += _numToMint;
    }
...
interface IRewardPool {
    function addNodeInfo(uint _nftId, uint nodeType, address _owner) external returns (bool);
