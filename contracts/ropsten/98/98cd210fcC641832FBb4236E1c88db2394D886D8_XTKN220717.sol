...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn220717][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn220717][ownable] new owner is the zero address");
...
abstract contract Team is Context, Ownable{
    ...
    modifier onlyTeam(){
        require(_isTeamMember(_msgSender()), "[xtkn220717][team] only team can access this function!");
        _;
    }

    function teamAdd(address pMember) public virtual onlyTeam{
        _teamMembers[pMember] = true;
    }

    function teamRemove(address pMember) public virtual onlyTeam{
...
contract XTKN220717 is Context, IERC20, Ownable, ReentrancyGuard, Team{
    // lib
    ...
    // addresses
        ...
        address public ADDRESS_MARKETING; // marketing wallet addess
        address public ADDRESS_DEVELOPER; // developer wallet address
    ...
    // taxes
        ...
        uint16 public TAX_BUY_DEV = 20;
        uint16 public TAX_BUY_BURN = 30;

        uint16 public TAX_SELL_MARKETING = 50;
        uint16 public TAX_SELL_DEV = 20;
        uint16 public TAX_SELL_BURN = 30;

    // tokenomics
        string private _name = "XTKN220717";
        string private _symbol = "XTKN220717";
    ...
    constructor(){
        ...
        // set interfaces
            _router = IUniswapV2Router02(ADDRESS_ROUTER); // create uniswap router
        ...
        // set ownership
            teamAdd(_msgSender()); // add wallet to team
            renounceOwnership(); // lock contract
    }
    ...
    function setTaxesBuy(uint16 pMarketing, uint16 pDev, uint16 pBurn) public onlyTeam{
        require((pMarketing + pDev + pBurn) <= 100, "[xtkn220717] sum of all taxes can't exceed 10%!");
        TAX_BUY_MARKETING = (
            (pMarketing > 0) ? pMarketing :
            TAX_BUY_MARKETING
        );
        TAX_BUY_DEV = (
            (pDev > 0) ? pDev : 
    ...
    function setTaxesSell(uint16 pMarketing, uint16 pDev, uint16 pBurn) public onlyTeam{
        require((pMarketing + pDev + pBurn) <= 100, "[xtkn220717] sum of all taxes can't exceed 10%!");
        TAX_SELL_MARKETING = (
            (pMarketing > 0) ? pMarketing :
            TAX_SELL_MARKETING
    ...
    function addLiquidity() public onlyTeam{
        _addLiquidity();
    }

    function setMarketingAddress(address pWallet) public onlyTeam{
        ADDRESS_MARKETING = pWallet;
        _addressNoTaxes[ADDRESS_MARKETING] = true; // this needs to be set or we are stuck in a tax loop
    }

    function setDeveloperAddress(address pWallet) public onlyTeam{
        ADDRESS_DEVELOPER = pWallet;
        _addressNoTaxes[ADDRESS_DEVELOPER] = true; // this needs to be set or we are stuck in a tax loop
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[xtkn220717] approve from the zero address");
        require(spender != address(0), "[xtkn220717] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[xtkn220717] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[xtkn220717] transfer amount exceeds allowance"));
        return(true);
    }
    ...
    function _transfer(address pFrom, address pTo, uint256 pAmount) private{
        if(_addressNoTaxes[pFrom] || _addressNoTaxes[pTo]){
            ...
            if(pFrom == ADDRESS_PAIR){
                ...
                // event
                    emit TransferTaxes(Tax.marketing, Tax.dev, Tax.burn, "buy");
            }
            ...
            if(pTo == ADDRESS_PAIR){
                ...
                // event
                    emit TransferTaxes(Tax.marketing, Tax.dev, Tax.burn, "sell");
            }

            if(!Tax.buy && !Tax.sell){
                // transfer, we do not buy or sell, but move tokens from A to B, no Tax!
                Tax.marketing = 0;
                Tax.dev = 0;
                Tax.burn = 0;

                // event
                    emit TransferTaxes(0, 0, 0, "transfer");
            }

            if(Tax.marketing > 0){
                Tax.totalMarketing = _tax(pAmount, Tax.marketing);
                Tax.total = Tax.total.add(Tax.totalMarketing);
                _transactionTokens(pFrom, ADDRESS_MARKETING, Tax.totalMarketing);
            }

            if(Tax.dev > 0){
                Tax.totalDev = _tax(pAmount, Tax.dev);
                Tax.total = Tax.total.add(Tax.totalDev);
                _transactionTokens(pFrom, ADDRESS_DEVELOPER, Tax.totalDev);
            }

            if(Tax.burn > 0){
                Tax.totalBurn = _tax(pAmount, Tax.burn);
    ...
    function _addLiquidity() private{
        ...
        uint256 liquidity = 750000000 * (10**9);

        require(balance > 0 && _balances[address(this)] >= liquidity,  "[xtkn220717] You do not have any coins or tokens to provide liquidity!");
        
        // event
            emit TransferLiquidity(balance, liquidity);
