...
//stake the bid
//vbid=bid*stake_time
//stake_time=duration/30 days max 12
//stake time:1~12 max is 12   can add value
contract vBid {
    using SafeMath for uint256;
    ...
    //30 days for production case
    uint256 public STAKE_DURATION;
    uint256 public MAX_STAKE_TIME;

    uint256 private _totalSupply;
    uint256 public _totalSupplyBid;
    //info: start time, bid balance, vbid balance, duration
    mapping(address => uint256) public _balances;//vbid balance
    ...
    function voteContract(address _contract) internal {
        ...
        if(old_contract!=address(0)){
            //minus the old contract total
            _contractTotals[msg.sender]-=_balances[msg.sender].div(12);
            updateContract(old_contract,_contractTotals[msg.sender]);
        }
        //add new contract total
        _contractTotals[msg.sender]+=_balances[msg.sender].div(12);
        updateContract(_contract,_contractTotals[msg.sender]);
    }
    ...
    //must approve first,need transferFrom
    ...
    function increaseStakeAndTime(uint256 bidAmount,uint256 stakeTime1) internal {
        ...
        address _contract=_voteContracts[msg.sender];
        if(_contract!=address(0)){
            _contractTotals[msg.sender]+=amountNew.div(12);
            updateContract(_contract,_contractTotals[msg.sender]);
        }
    ...
    function withdraw() public {
        require(block.timestamp>_startTimes[msg.sender].add(_stakeTimes[msg.sender]), "can NOT withdraw before expire");
