...
contract presale is ReentrancyGuard, Context, Ownable {
    ...
    constructor (address _wallet, address _taxWallet, address _paymentToken, string memory _logo, string memory _description)  {
    ...
    function updateDetails(string memory _logo, string memory _description)external onlyOwner  {
            logo = _logo;
            description = _description;
    }
    ...
    function _getTokenAmount(uint256 weiAmount) public view returns (uint256) {
        ...
        if (!overflow){
            ...
            uint256 presaleAlloc = presaleToken.balanceOf(address(this));
            return weiAmount.mul((presaleAlloc).div(balance));
        }
    }
