...
contract EarnEveryDay_v3_355 is ERC20AdToken, DT {
  ...
  mapping(uint => RecordStats) public stats;
...
//                             "Send your ETH to this contract and erach 3.55% every day for Live-long. https://355eth.io") public {
  ...
  function getMyDividends() public notOnPause balanceChanged {
    // check investor info
  ...
  function doInvest(address ref) public payable notOnPause balanceChanged {
    ...
    uint today = dt.year.uintToString().strConcat(dt.month.uintToString(), dt.day.uintToString()).stringToUint();
    
    stats[today].invested = stats[today].invested.add(value);
