...
contract MyTotalSupplyToken is BurnableToken {
    ...
    string public constant name = "My Total Supply Token MP AD";
...
contract MTSTMPADCrowdsale is Owned {
...
contract AirDrop is Owned {
  ...
  constructor() public {
      address _tokenAddr; //here pass address of your token
      token = MyTotalSupplyToken(_tokenAddr);
  }
  ...
  // Send same number of tokens to all the recipients
  function sendTokensSingleValue(address[] dests, uint256 value) whenDropIsActive onlyOwner external onlyOwner {
    uint256 i = 0;
  ...
  function sendInternally(address recipient, uint256 tokensToSend, uint256 valueToPresent) internal onlyOwner {
