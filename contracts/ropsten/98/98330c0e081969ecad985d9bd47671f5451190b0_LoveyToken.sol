...
contract Disableable is Owned {
   ...
   */
  function enable() public onlyOwner disabled returns (bool) {
    isEnabled = true;
    emit Enable();
    return true;
  }
   ...
   */
  function disable() public onlyOwner enabled returns (bool) {
    isEnabled = false;
    emit Disable();
    return true;
  }
...
contract PayableToken is Disableable {
   ...
   * @dev called by the owner to pause, triggers stopped state
   */
  function paymentEnable() public onlyOwner paymentDisabled returns (bool) {
   ...
   * @dev called by the owner to unpause, returns to normal state
   */
  function paymentDisable() public onlyOwner paymentEnabled returns (bool) {
...
contract LoveyToken is BaseToken {
    ...
    function () public payable enabled paymentEnabled {
        ...
        if(amount == 0) throw;
        if(amount < (dollarToLoveyRate() * 10**(decimals) * 10)) throw; // don't allow small amounts
