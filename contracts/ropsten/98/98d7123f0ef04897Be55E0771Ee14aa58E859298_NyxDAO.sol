...
abstract contract NFTContract {
    ...
    function balanceOf(address addr) public virtual view returns(uint256);
}
...
contract NyxDAO is ReentrancyGuard, AccessControl, Ownable {
    ...
    event ApprovedForVoteProposal(address indexed proposer, ProposalType proposalType, ProposalConf proposalConf);
        ...
        external onlyOwner
    {
        ...
        emit ApprovedForVoteProposal(proposalConf.proposer, proposalType, proposalConf);
    }
        ...
        returns (ProposalConf memory)
    {
        ...
        uint256 votingPower = nft_contract.balanceOf(voter);
        ...
        private view
    {
        if (proposalConf.votingPassed || proposalConf.livePeriod <= block.timestamp)
        {
            ...
            message = string(abi.encodePacked(message, Strings.toString(proposalConf.livePeriod)));
    ...
    function getVaultContract()
        ...
        returns (address)
    {
        require(nft_contract_address != NULL_ADDRESS, "Have to setup the NFT smart contract address");
        NFTContract nft_contract = NFTContract(nft_contract_address);
        return nft_contract.vault_address();
    }
    ...
    function getProposalVotes(ProposalType proposalType, uint256 proposalId)
        external view
        onlyStakeholder("User is not a stakeholder")
        returns (Vote[] memory)
    {
        uint256 proposalVotesIdx = 0;
        address[] memory holders = getHolders();
        Vote[] memory proposalVotes = new Vote[](holders.length);
        for (uint256 iAddress = 0; iAddress < holders.length; iAddress++)
        {
            address holder = holders[iAddress];
            Vote[] memory holderVotes = stakeholderVotes[proposalType][holder];
            for (uint256 iVote = 0; iVote < holderVotes.length; iVote++)
            {
                Vote memory oneHolderVote  = holderVotes[iVote];
                if (oneHolderVote.proposalId == proposalId)
                {
                    proposalVotes[proposalVotesIdx] = oneHolderVote;
                    proposalVotesIdx += 1;
                }
            }
        }
