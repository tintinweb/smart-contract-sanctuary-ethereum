...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    // The address of the plot protocol to use via this contract
    address public plot;
     ...
     * Constructor
     */
    constructor(address _plot, address _weth) public {
        require(
            IERC165(_plot).supportsInterface(interfaceId),
    ...
    nonReentrant
    {
        ...
        // For plot Protocol, ensure that the bid is valid for the current bidShare configuration
        if(auctions[auctionId].tokenContract == plot) {
            require(
                IMarket(IMediaExtended(plot).marketContract()).isValidBid(
    ...
    /**
     * @notice End an auction, finalizing the bid on Plot if applicable and paying out the respective parties.
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        if(auctions[auctionId].tokenContract == plot) {
            // If the auction is running on plot, settle it on the protocol
            (bool success, uint256 remainingProfit) = _handlePlotAuctionSettlement(auctionId);
    ...
    function _handlePlotAuctionSettlement(uint256 auctionId) internal returns (bool, uint256) {
        address currency = auctions[auctionId].auctionCurrency == address(0) ? wethAddress : auctions[auctionId].auctionCurrency;
        ...
        IERC20(currency).approve(IMediaExtended(plot).marketContract(), bid.amount);
        IMedia(plot).setBid(auctions[auctionId].tokenId, bid);
        uint256 beforeBalance = IERC20(currency).balanceOf(address(this));
        try IMedia(plot).acceptBid(auctions[auctionId].tokenId, bid) {} catch {
            // If the underlying NFT transfer here fails, we should cancel the auction and refund the winner
            IMediaExtended(plot).removeBid(auctions[auctionId].tokenId);
            return (false, 0);
        }
