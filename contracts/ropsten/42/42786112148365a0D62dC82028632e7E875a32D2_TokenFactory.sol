...
contract TokenFactory is ERC721, ERC721Enumerable, ERC721Burnable, AccessControl, Ownable {
  ...
  constructor(
    ...
    string memory newCreator ) ERC721(name, symbol) {
    creator = newCreator;
    fullAdmin = msg.sender;
    _setupRole(DEFAULT_ADMIN_ROLE, fullAdmin);
    _setupRole(MINT_ROLE, fullAdmin);
    _setupRole(REDEEM_ROLE, fullAdmin);
    _setupRole(CONTENT_ROLE, fullAdmin);
  ...
  function setMinter (address newMinter)public{
    _setupRole(MINT_ROLE, newMinter);
  }
  ...
  function Mint(string memory _name, string memory cid, uint256 characterId, uint16 _royalty) public payable {
    ...
    tokenPrice[supply + 1] = msg.value;

    string memory upperName;

    tokenNames[supply + 1] = _name;
    names[upperName] = supply + 1;
    minters[supply + 1] = msg.sender;
    royalties[supply + 1] = _royalty;

  }

  function sendRoyalty(uint256 id) public {
    uint16 royalty = royalties[id];
    address minter = minters[id];
    require(minter != address(0) && royalty != 0, "You are not minter of this token or you didn't set your royalty for this token.") ;
    uint256 royaltyFee = tokenPrice[id].mul(royalty).div(100);
    (bool sent, bytes memory data) = minter.call{value : royaltyFee}("");
    require(sent, "Failed to send royalty.");
    emit TokenMinted(id, data);
  }
  
  function ownerMint(string memory _name, string memory cid, uint256 characterId) public payable onlyOwner{
        uint256 supply = totalSupply();
    ...
    function batchMint(string memory _name, string memory cid, uint256[] calldata characterId, uint16 _royalty) public {
      for (uint idx=0; idx < characterId.length; idx++){
        Mint(_name, cid, characterId[idx], _royalty);
      }
    }

  function presaleMint(string memory _name, string memory cid, uint256 characterId, uint16 _royalty) public payable {
    ...
    require(msg.value >= character_price[characterId], "The price is not correct");
    require(character_minted[characterId] < character_supply[characterId], "The character amount for this archetype has reached its limit");
    require(block.timestamp >= presaleStartDate && block.timestamp < presaleStopDate);
  ...
  function redeem(uint256 _id) public {
    ...
    require(block.timestamp >= redeemStartDate && block.timestamp <= redeemStopDate, "Caller has not the role for presale");
    require(_exists(_id), 'No such token');
    require(minters[_id] == msg.sender, "you are not owner this token.");
  ...
  function setRealTokenURI(string memory cid) public {
      realTokenUri = string(abi.encodePacked(baseuri, '/', cid));
  }
