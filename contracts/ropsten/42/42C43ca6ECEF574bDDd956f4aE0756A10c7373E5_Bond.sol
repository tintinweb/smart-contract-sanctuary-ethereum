...
contract Bond is Ownable, Pausable {  
    ...
    function depositToken(uint _amount, address pairToken) public whenNotPaused {  
    ...
    function calculateDrip(uint256 poolID) internal virtual returns(uint256, uint256){
        ...
        uint256 totalstakedDays = block.timestamp.sub(stakingStartTime[poolID]).div(60);
 
        // calculation of the amount staking time
        if(claimedAt[poolID]> stakingStartTime[poolID]){
            stakingPeriodInDays = block.timestamp.sub(claimedAt[poolID]).div(60);
        }else{
        ...
        if(totalstakedDays > 365 ){
            if(claimedAt[poolID]> stakingStartTime[poolID]){
                uint256 totalAlreadyClaimedDays = claimedAt[poolID].sub(stakingStartTime[poolID]).div(60);
    ...
    function availableBalance(uint256 poolID) public whenNotPaused returns(uint256 balance, address staker, uint256 stakingTime) {
        ...
        return (dripAmount, poolOwner[poolID], stakeTime);
    }
 
 
    function restake(uint256 poolID) public whenNotPaused {
 
        require( isStaking[msg.sender], "Caller has no staking" );
    ...
    function calculateRate(uint256 poolID) internal virtual returns(uint256){
    ...
    function claim(uint256 poolID) public whenNotPaused {
        ...
        uint256 taxRate = calculateRate(poolID);
