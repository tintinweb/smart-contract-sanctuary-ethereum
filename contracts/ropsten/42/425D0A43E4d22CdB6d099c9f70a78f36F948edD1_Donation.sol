...
contract Room {
    ...
    function createRoom(string memory roomName) public payable roomExist(roomName) canCreateRoom(roomName) {
    ...
    function startRoomFundraising(string memory roomName) public roomExist(roomName) onlyRoomOwner(roomName) {
        rooms[roomName].active = true;
        emit RoomUpdated(msg.sender, rooms[roomName]);
    }

    function finishRoomFundraising(string memory roomName) public roomExist(roomName) onlyRoomOwner(roomName) {
    ...
    function transferRoomOwnership(string memory roomName, address newOwner) public roomExist(roomName) onlyRoomOwner(roomName) {
...
contract Donation is Management {
    ...
    function getDonationAfterCommission(string memory roomName) public view roomExist(roomName) returns(uint256) {
        RoomInfo storage room = rooms[roomName];
        return room.donation.sub(room.donation.div(COMMISSION));
    }

    function donate(string memory roomName) public payable roomExist(roomName) canDonate(roomName) {
        rooms[roomName].donation = rooms[roomName].donation.add(msg.value);
        rooms[roomName].participants.push(msg.sender);
        emit Donated(msg.sender, roomName, msg.value);
    }

    function withdraw(string memory roomName, address payable to) public payable roomExist(roomName) onlyRoomOwner(roomName) {
