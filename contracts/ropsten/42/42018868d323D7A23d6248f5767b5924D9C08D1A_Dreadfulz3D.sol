...
contract Dreadfulz3D is ERC721Enumerable, ERC721Burnable, Ownable, ReentrancyGuard, Pausable, IERC721Receiver {
    using Strings for uint256;
    string public baseUri = "https://ipfs.io/ipfs/QmUfeu9YQSXnJXBZRxa6M3kcKauWkTV1zHKBpzCMxmhcdN/";
    string public extension = ".json";  

    IERC721 public dreadfulz2D;
    ERC20PresetMinterPauser public dread;
    ...
    constructor() ERC721("Dreadfulz", "Dreadfulz3D") {  
    ...
    function swap(uint256[] memory tokenIds) external whenNotPaused nonReentrant {
        ...
        for(uint256 i; i < tokenIds.length; i++) {
            ...
            IERC721 nft = correctToken(current);
            if(!hasPaid(current)) {
                requiredBurn += cost;
            }
            _paid[current] = true;
    ...
    function saveNFT(ERC721Enumerable nft, uint256 tokenId, address to) external adminOrOwner {
        nft.safeTransferFrom(address(this), to, tokenId);
    }

    function setDreadfulz2D(IERC721 _dreadfulz) external adminOrOwner {
        dreadfulz2D = _dreadfulz;
    }

    function setDreadz(ERC20PresetMinterPauser _dread) external adminOrOwner {
        dread = _dread;
    }
    ...
    function correctToken(uint256 id) public view returns (IERC721 ) {
        bool _has = (!_exists(id) || ownerOf(id) == address(this));
