...
contract MyNFT is ERC721, Ownable {
    ...
    constructor() ERC721("MyNFT", "NFT")
    {
        for (uint256 iAddress = 0; iAddress < _TEAM_ADDRESSES.length; iAddress++)
        {
            ...
            giveaway(1, teamAddress, temporaryMetadataURI);
        ...
        public onlyOwner
    {
        ...
        for (uint i = 0; i < whitelist.length; i++)
        {
            ...
            uint256[] memory addressTokenIds = getTokenIdsOf(whiteListedAddress);
            // _mintNFT(amount, whiteListedAddress, temporaryMetadataURI);
            for (uint256 j = 0; i < amount; i++)
            {
                uint256 tokenId = addressTokenIds[j];
                _mint(whiteListedAddress, tokenId);
            }
        }
    }

    // Mint Logic
    function _mintNFT(uint256 nMint, address recipient, string memory tokenURI)
        private
        // M1
        // returns (uint256)
        // M2
    ...
    {
        require(_tokenIds.current() + nMint <= _MAX_SUPPLY, "No more NFT to mint");
    ...
    // Normal Mint
    function mintNFT(uint256 nMint, address recipient, string memory tokenURI)
        public payable
        returns (uint256[] memory)
    {
        ...
        return _mintNFT(nMint, recipient, tokenURI);
    }

    // Free Mint
    function giveaway(uint256 nMint, address recipient, string memory tokenURI)
        public onlyOwner
        returns (uint256[] memory)
    {
        return _mintNFT(nMint, recipient, tokenURI);
    ...
    // Presale Mint
    function mintNFTpresale(uint256 nMint, address recipient, string memory tokenURI)
        public payable
        returns (uint256[] memory)
    {
