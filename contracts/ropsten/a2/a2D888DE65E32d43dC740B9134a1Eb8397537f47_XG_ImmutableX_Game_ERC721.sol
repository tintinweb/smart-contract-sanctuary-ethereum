...
/// @title Xternity Games - ERC721A Game Contract (ImmutableX)
/// @author Xternity Games
/// @notice ImmutableX ERC721A Game contract
/// @dev ImmutableX ERC721A Game contract
contract XG_ImmutableX_Game_ERC721 is
    ERC721,
    ERC721Enumerable,
    Mintable,
    Pausable
{
    address private _imx; // IMX contract address

    uint256 public gameId; // Game ID
    string public baseURI; // Game baseURI
    ...
    constructor(
        ...
        address imx_
    ) ERC721(name_, symbol_) Mintable(msg.sender, imx_) {
        gameId = gameId_;
        baseURI = baseURI_;
        _imx = imx_;
    }
    ...
    /// @notice Burn token `id_`
    /// @dev Burn token `id_`
    /// @param id_ (uint256) Token ID
    function burnToken(uint256 id_) public onlyOwnerOrIMX {
        _burn(id_);
    }

    /// @notice Burn tokens `ids_`
    /// @dev Burn token `id_`
    /// @param ids_ (uint256[] memory) Token ID
    function burnBatchToken(uint256[] memory ids_) public onlyOwnerOrIMX {
        unchecked {
