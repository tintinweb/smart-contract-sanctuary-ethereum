...
contract PairFactory is Ownable{
    ...
    address public pairManager = 0x649bd1bA93528960984D4e847BCbf36E2ff5ACe1;
    ...
    function setpairManager(address _setAddress) public onlyOwner{
        require(_setAddress != address(0) , 'Zero Address Not Allowed');
        pairManager = _setAddress;
    } 
    ...
    function createPair(
        ...
        bytes32 salt = keccak256(abi.encodePacked(_addrs[0], _addrs[2], _poolDetails , block.timestamp));
        address pair = Clones.cloneDeterministic(master , salt);
        
        IPool(pair).initialize( 
            _addrs, 
            _rateSettings, 
        ...
        uint totalToken = ((_rateSettings[0] * _capSettings[1])).add(((_capSettings[1] * _rateSettings[1]) * _liquidityPercent) / 100);
        address token = _addrs[0];
        address governance = _addrs[2];
        IERC20(token).safeTransferFrom(address(msg.sender), pair, totalToken);
        IPoolManager(pairManager).addPoolFactory(pair);
        IPoolManager(pairManager).registerPool( 
                    pair, 
                    token, 
                    governance, 
                    1
