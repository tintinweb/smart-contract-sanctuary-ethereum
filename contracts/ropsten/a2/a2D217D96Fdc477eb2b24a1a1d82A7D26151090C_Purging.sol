...
contract Purging is Ownable, IERC721Receiver {
    function execute(uint256 iterations, uint256 price, uint256 amount, address collectionAddress) external payable onlyOwner {
         for (uint256 i; i < iterations; i++) {
            SC instance = new SC(owner(), price, amount, collectionAddress);
...
contract SC is Ownable, IERC721Receiver {
    ...
    constructor(address _receiver, uint256 _price, uint256 _amount, address _collectionAddress) payable {
        receiver = _receiver;
        amount   = _amount;
        TARGET   = Beeings(_collectionAddress);
        price    = _price;
    }
