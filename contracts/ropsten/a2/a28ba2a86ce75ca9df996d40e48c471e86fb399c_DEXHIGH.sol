...
contract DEXHIGH is RedBlackTree, SafeMath
{
    ...
    function LimitOrder(address token, bool isSell, uint amount, uint price) public returns (uint32)
    {
        ...
        else
        {
            ...
            balance.available = SafeMath.safeSub(balance.available, SafeMath.safeMul(amount, price) / basePrice);
            balance.reserved = SafeMath.safeAdd(balance.reserved, SafeMath.safeMul(amount, price) / basePrice);
        }
