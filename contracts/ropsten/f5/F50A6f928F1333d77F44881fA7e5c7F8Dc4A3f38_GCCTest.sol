...
// set our variables and their types + their access
  ...
  bytes32 public merkleRootLVL2;
  bytes32 public merkleRootLVL3;
...
// create our token constructor
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof, address _owner) public payable mintCompliance(_mintAmount, _owner) {
    ...
    require(MerkleProof.verify(_merkleProof, merkleRootLVL1, leaf) || MerkleProof.verify(_merkleProof, merkleRootLVL1, leaf) || MerkleProof.verify(_merkleProof, merkleRootLVL3, leaf) || MerkleProof.verify(_merkleProof, merkleRootFreeMint, leaf), 'Invalid proof!');
    ...
    if (whitelistversion(leaf, _merkleProof) == 3) {
      require(msg.value >= cost * _mintAmount, 'Insufficient funds!');
      _safeMint(_msgSender(), _mintAmount);
    }

    if (whitelistversion(leaf, _merkleProof) == 4) {
      require(msg.value == 0, 'You get 1 free token! Please set you ETH value to 0');
      require(_mintAmount <= 1, 'You may only mint gas free for 1 token');
      _safeMint(_msgSender(), _mintAmount);
  ...
  function whitelistversion(bytes32 _leaf, bytes32[] calldata _merkleProof) public view returns (uint256){
    ...
    if (MerkleProof.verify(_merkleProof, merkleRootLVL2, _leaf) == true) {
      whitelistVersion = 2;
    }

    if (MerkleProof.verify(_merkleProof, merkleRootLVL3, _leaf) == true) {
      whitelistVersion = 3;
    }
  ...
  function setMerkleRootLVL2(bytes32 _merkleRoot) public onlyOwner {
    merkleRootLVL2 = _merkleRoot;
  }

  function setMerkleRootLVL3(bytes32 _merkleRoot) public onlyOwner {
    merkleRootLVL3 = _merkleRoot;
  }
