...
contract Marketplace is IMarketplace, Ownable {
    ...
    // collection => tokenId => trade type => Sell
    mapping(address => mapping(uint256 => mapping( uint8 => Sell))) private onSell; // get function
    // collection => tokenId => Bid
    mapping(address => mapping(uint256 => Bid)) public bids;
    // collection => fee
    mapping(address => uint256) private  collectionFee;
    ...
    function buyAtFixedPrice(address _collection, uint256 _tokenId) public override {
        ...
        require(onSell[_collection][_tokenId][FIXED].startPrice == TOKEN.allowance(msg.sender, address(this)), "insufficient tokens to buy this NFT");
        uint256 price = onSell[_collection][_tokenId][FIXED].startPrice;
        uint256 fee = tradeFee(_collection, price);
    ...
    function placeBid(address _collection, uint256 _tokenId, uint256 _bidAmount) public override {
        ...
        require(onSell[_collection][_tokenId][AUCTION].startTime <= block.timestamp, "auction isn't started yet");
    ...
    function claimAuction(address _collection, uint256 _tokenId) public override {
        ...
        uint256 fee = tradeFee(_collection, amount);
        TOKEN.transfer(FEE_COLLECTOR, fee); // transferring fee
        TOKEN.transfer(seller, amount.sub(fee)); // paying to seller
    ...
    // update/set fee per collection. please note that fee should be in percentage e.g: 2 means 0.002% during fee calculations
    function updateCollectionFee(address _collection, uint256 _feePercentage) onlyOwner public override {
        collectionFee[_collection] = _feePercentage;
        emit CollectionFee(_collection, collectionFee[_collection]);
    }

    // view functions
    function fetchOnSell(address _collection, uint256 _tokenId) public override view returns ( Sell memory){
        Sell memory sell;
        // at fixed
        if (onSell[_collection][_tokenId][FIXED].seller != address(0)) {
            sell = onSell[_collection][_tokenId][FIXED];
        } else if (onSell[_collection][_tokenId][AUCTION].seller != address(0)) {
            sell =  onSell[_collection][_tokenId][AUCTION];
        }
        return sell;
    }
    function tradeFee(address _collection, uint256 _amount) public view override returns ( uint256 fee) {
        uint feePercentage = FEE;
        if (collectionFee[_collection] > 0){
            feePercentage = collectionFee[_collection];
        }
        fee = ((feePercentage.mul(_amount)).div(FLOATPOINT)).div(100); // first div(100) to fix FEE percentage floating points
    ...
    modifier onlyAuthorized(){
        require(msg.sender == owner() || msg.sender == address(FACTORY), "unauthorized to perform this action");
        _;
    }
    modifier onlyValidCollection(address _collection) {
        require(supportedCollections[_collection] != address(0) || FACTORY.ownerOf(_collection) != address(0), "collection isn't registered" );
        _;
    }
}
...
interface IMarketplace {
    ...
    function updateCollectionFee(address _collection, uint256 _feePercentage) external;
    function tradeFee(address _collection, uint256 _amount) external view returns ( uint256 fee);
    function fetchOnSell(address _collection, uint256 _tokenId) external view returns ( Sell memory);
    ...
    event CollectionFee(address collection, uint256 feePecentage);
...
interface IFactory {
    function createCollection(string memory _name, string memory _symbol, string memory _baseURI) external returns (address);
    function rmvCollection(address _collection) external;
    function ownerOf(address _collection) external view returns (address);

    //events
    event NewCollection(address collection, string name, string symbol, address owner);
...
interface INFT {
    ...
    function mint(address to, uint256 nfts) external returns ( uint256[] memory );
}
