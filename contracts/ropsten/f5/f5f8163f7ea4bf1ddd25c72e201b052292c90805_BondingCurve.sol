...
contract StandardToken {
    ...
    function balanceOf(address _owner) public returns (uint256 value);
...
contract BondingCurve is BancorFormula, admined {
  ...
  bool public fromContract = true; //FALSE == From Wallet
  uint256 public baseFactor = 286429711 * (10 ** 18);

  address public holdWallet;

  StandardToken IADOWRCoin;
  ...
  */
  ...
  uint256 MAX_SUPPLY = 2000000000 * (10 ** 18); //2.0B IAD
  uint256 INITIAL_PRICE = 36090000; // 0.3609$
  uint256 public poolBalance = reserveRatio * baseFactor * INITIAL_PRICE / (1000000 * 10 ** 18);

  constructor(StandardToken _address) public {
   ...
   * @dev Buy tokens
   */
  function buy(uint256 _amount, address beneficiary_) public onlyAdmin(1) returns(bool) {
    require(_amount > 0);
    uint256 tokensToSell = calculatePurchaseReturn(baseFactor, poolBalance, reserveRatio, _amount);
    ...
    if(fromContract == true){
      ...
      IADOWRCoin.transfer(beneficiary_,tokensToSell);

    } else {

      IADOWRCoin.transferFrom(holdWallet,beneficiary_,tokensToSell);

    }

    poolBalance = poolBalance.add(_amount);

    emit LogPurchase( tokensToSell, _amount, beneficiary_);
    return true;
  }
  ...
  function sell(uint256 sellAmount, address beneficiary_) public onlyAdmin(1) returns(bool,uint256) {
    ...
    uint256 amountValue = calculateSaleReturn(baseFactor, poolBalance, reserveRatio, sellAmount);

    poolBalance = poolBalance.sub(amountValue);
    baseFactor = baseFactor.sub(sellAmount);

    require(baseFactor > 0);
  ...
  function setTokensSource(address _source) onlyAdmin(2) public{
    ...
    emit LogBondingCurve('Changed Token Source',0);
  }

  function retrieveTokens() onlyAdmin(2) public {
    uint256 tokToRetrieve = IADOWRCoin.balanceOf(this);
    IADOWRCoin.transfer(msg.sender,tokToRetrieve);
    emit LogBondingCurve('Token Retrieve from contract',0);
  }
