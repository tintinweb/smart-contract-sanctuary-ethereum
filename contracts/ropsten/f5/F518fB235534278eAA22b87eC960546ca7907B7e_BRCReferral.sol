...
interface IReferralBound {
    function onReferrerBound(address account, address referrer) external;
}
...
import "../interfaces/IReferralBound.sol";
import "@openzeppelin/contracts/token/ERC20/SafeERC20.sol";

contract BRCReferral is IReferral, SafeOwnable {
    using Address for address;
    address public bindEventHandler;
    mapping(address => address) internal _referrers;
    mapping(address => address[]) internal _recommended;

    constructor(address initReferrer) {
        _referrers[initReferrer] = address(this);
        _recommended[address(this)].push(initReferrer);
    }

    function setBindEventHandler(address _handler) external onlyOwner {
        require(bindEventHandler.isContract(), "Only supports smart contracts");
        bindEventHandler = _handler;
    }
    ...
    function setReferrer(address _referrer) public virtual override {
        ...
        if (bindEventHandler != address(0)) {
            bindEventHandler.functionCall(
                abi.encodeWithSelector(
                    IReferralBound.onReferrerBound.selector,
                    msg.sender,
                    _referrer
                )
            );
