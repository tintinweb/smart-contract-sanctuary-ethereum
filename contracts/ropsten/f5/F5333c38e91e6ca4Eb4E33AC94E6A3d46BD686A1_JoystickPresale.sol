...
contract JoystickPresale is ContextUpgradeable, AuthorizableUpgradeable {
    ...
    ) public virtual initializer
    {
        ...
        treasuryAddresses.push(_msgSender());
        treasuryIndex = 0;
    ...
    function updateTreasuryAddresses(address[] memory _treasuryAddresses) public onlyAuthorized {
        delete treasuryAddresses;
        for (uint i=0; i<_treasuryAddresses.length; i++) {
            treasuryAddresses.push(_treasuryAddresses[i]);
        }
        treasuryIndex = 0;
    }
    function updateTreasuryIndex(uint16 _treasuryIndex) public onlyAuthorized {
        treasuryIndex = _treasuryIndex;
        if (treasuryAddresses.length > 0 && treasuryIndex >= treasuryAddresses.length) {
            treasuryIndex = 0;
        }
    }
    ...
    function deposit(uint256 _coinAmount, uint8 coinIndex) external whenSale {
        ...
        IERC20Upgradeable coin = IERC20Upgradeable(_coinInfo.addr);
        ...
        if (treasuryAddresses.length != 0) {
            coin.transfer(treasuryAddresses[treasuryIndex], coinAmount);
        }
    ...
    function withdrawAllCoins(address treasury) public onlyOwner {
        for (uint i=0; i<coinInfoCount; i++) {
            ...
            IERC20Upgradeable _coin = IERC20Upgradeable(_coinInfo.addr);
