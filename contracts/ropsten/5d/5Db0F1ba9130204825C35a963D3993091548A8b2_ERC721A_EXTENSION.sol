...
contract ERC721A_EXTENSION is ERC721A, Ownable, Pausable {
	...
	function mint(uint256 _quantity) external callerIsUser payable whenNotPaused {
		...
		uint256 _needPayPrice = mintInfo.freeQuantity - _totalSupply >= _quantity ? 0 : (_quantity - (mintInfo.freeQuantity - _totalSupply)) * mintInfo.mintPrice;

		require(_quantity > 0, "Invalid quantity");
		require(_totalSupply + _quantity <= mintInfo.maxSupply, "Exceed supply");
		require(owner() == msg.sender || _currentMintedCount + _quantity <= mintInfo.singleAddressLimitQuantity, "Exceed mint limit");
