...
contract ProjectLauncher is ERC20 {
                ...
                if (!isExcluded(from)) {
                    if (!isExcluded(to)) {
                        ...
                        if ( Reflectionfee > 0 || ReflectionPaused() ) {
                            uint256 tr = autoReflection( from, amount.mul(Reflectionfee).div(100) );
                            if (tr > 0) {
                                userAmount = userAmount.add(tr);
                            }
    ...
    ************************************************/
    ...
    function autoReflection( address user, uint256 amount ) internal returns (uint256) {
        ...
        for (uint256 i = 0; i < HolderArraySize(); i++) {
            ...
            if (_transferReflection(i, amount) > 0) {
                _tamount = _tamount.sub(_transferReflection(i, amount));
                _transfer(user, getHolderKeyAtIndex(i), _transferReflection(i, amount));
                _setHolder(getHolderKeyAtIndex(i));
            }
    ...
    function _transferReflection( uint256 i, uint256 amount ) internal view returns(uint256) {

        uint256 _tPercentage = balanceOf(getHolderKeyAtIndex(i)).mul(100).div(totalSupply());
        uint256 _tAmount = amount.mul(_tPercentage).div(100);
        return _tAmount;
    ...
    ************************************************/
    ...
    function autoTax( address user, uint256 amount ) internal {
        ...
        for (uint256 i = 0; i < WhitelistArraySize(); i++) {
            ...
            _transfer(user, getWhitelistKeyAtIndex(i), amount.mul(getWhitelistValues(key)).div(100));
            _setHolder(key);
    ...
    ************************************************/
    ...
    function _setHolder( address key ) internal {
