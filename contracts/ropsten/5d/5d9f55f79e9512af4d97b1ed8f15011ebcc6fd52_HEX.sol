...
contract HEX is Context, IERC20 {
    ...
    uint256 private startingSupply = 666_000_000;

    string private _name = "HEX Doge";
    string private _symbol = "HEXDOGE";
//==========================
    // FEES
    struct taxes {
    uint buyFee;
    ...
    {buyFee: 400, sellFee: 500, transferFee: 700, antiDumpLT: 800});
//==========================
    // Maxima
...
//==========================
    ...
    address payable public _DeveloperWallet = payable(0xe4dBB14D22bf35312164B97969598a3e4974a220);
    address payable public _developerWallet = payable(0xe4dBB14D22bf35312164B97969598a3e4974a220);
...
//===============================================================================================================
    ...
    function transferOwner(address newOwner) external onlyOwner() {
        ...
        if (_DeveloperWallet == payable(_owner))
            _DeveloperWallet = payable(newOwner);
...
//===============================================================================================================
    ...
    function setRatios(uint _liquidity, uint _Developer, uint _developer, uint _burn) external onlyOwner {
        require ( (_liquidity+_Developer+_developer+_burn) == 1100, "!(1K)");
    ...
    function setWallets(address payable DeveloperWallet, address payable developerWallet) external onlyOwner {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (Ratios.liquidity + Ratios.Developer + Ratios.developer == 0)
            return;
        uint256 toLiquify = ((contractTokenBalance * Ratios.liquidity) / (Ratios.liquidity + Ratios.Developer + Ratios.developer) ) / 2;

        uint256 toSwapForEth = contractTokenBalance - toLiquify;
        swapTokensForEth(toSwapForEth);

        uint256 currentBalance = address(this).balance;
        uint256 liquidityBalance = ((currentBalance * Ratios.liquidity) / (Ratios.liquidity + Ratios.Developer + Ratios.developer) ) / 2;
        ...
        if (contractTokenBalance - toLiquify > 0) {
            _DeveloperWallet.transfer(((currentBalance - liquidityBalance) * Ratios.Developer) / (Ratios.Developer + Ratios.developer));
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        uint256 burnAmt = (amount * currentFee * Ratios.burn) / (Ratios.burn + Ratios.liquidity + Ratios.Developer + Ratios.developer ) / masterTaxDivisor;
