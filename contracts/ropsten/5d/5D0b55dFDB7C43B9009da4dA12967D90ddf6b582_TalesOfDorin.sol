...
contract TalesOfDorin is ERC721A, Ownable, ReentrancyGuard {
    ...
    function _mintPhase1(uint256 amount) internal {
        require(amount <= (phase1FreeMintAllowed[msg.sender] ? 1 : 0) + phase1PaidMintQuota[msg.sender],
            "You have reached your max mint quota, please try minting lesser number of tokens");

        uint256 payableTokensCount = amount - (phase1FreeMintAllowed[msg.sender] ? 1 : 0);
        uint256 amountPayable = payableTokensCount * PUBLIC_PRICE;

        require(msg.value >= amountPayable, "Amount paid is lesser than required amount");

        _safeMint(msg.sender, amount);

        phase1FreeMintAllowed[msg.sender] = false;
        phase1PaidMintQuota[msg.sender] -= payableTokensCount;
    }

    // phase 2 mint
    function _mintPhase2(uint256 amount) internal {
        require(amount <= (phase2FreeMintAllowed[msg.sender] ? 1 : 0) + phase2PaidMintQuota[msg.sender],
            "You have reached your max mint quota, please try minting lesser number of tokens");

        uint256 payableTokensCount = amount - (phase2FreeMintAllowed[msg.sender] ? 1 : 0);
        uint256 amountPayable = payableTokensCount * PUBLIC_PRICE;
        ...
        phase2FreeMintAllowed[msg.sender] = false;
        phase2PaidMintQuota[msg.sender] -= amount;
    }

    // phase 3 mint
    function _mintPhase3(uint256 amount) internal {
        require(phase3TotalMinted[msg.sender] + amount <= MAX_ALLOWED_IN_PHASE_3,
            "You have reached your max mint quota, please try minting lesser number of tokens");

        require(msg.value >= amount * PUBLIC_PRICE, "Amount paid is lesser than required amount");

        _safeMint(msg.sender, amount);

        phase3TotalMinted[msg.sender] += amount;
    }
    ...
    function mint(uint256 amount) external payable callerIsUser {
        ...
        require(_totalMinted() + amount < TOTAL_SUPPLY,
