...
contract SoundMintEdition is ERC721ARoyalty, Ownable, ReentrancyGuard {
  ...
  event teamMinted(address _to, uint256 _qty);
  ...
  ) ERC721ARoyalty ("Kaien Cruz x Somehoodlum", "SM002") {
  ...
  modifier onlyEOA() {
    require(tx.origin == msg.sender, "Contract: Contracts not allowed");
    _;
  }
  ...
  function setPreSaleActive(bool active) external onlyOwner {
    require(preSaleActive != active, "PreSale: Presale Same State");
    preSaleActive = active;
  }

  //  Set public sale active / deactive
  function setPublicSaleActive(bool active) external onlyOwner {
    require(publicSaleActive != active, "PublicSale: Public Sale Same State");
    ...
    onlyEOA
  {
    ...
    for (uint256 i; i < qty; i++) {               
        _setTokenRoyalty(updatedIndex, msg.sender, royaltyFee);            
        updatedIndex++;
    }

    quantityPerWallet[msg.sender] = quantityPerWallet[msg.sender] + qty;
    ...
    onlyEOA
  {
    ...
    require(IERC20(apeAddress).balanceOf(msg.sender) >= salePriceAPE * qty, "PublicSale: Insufficient APE");

    require(_safeTransferAPE(msg.sender, address(this), salePriceAPE * qty), "APE Token: Transfer Error.");
  ...
  function setMintList(bytes32 merkleRootHash) external onlyOwner {
    require(merkleRoot != merkleRootHash, "MintList: Already Being Set");
    merkleRoot = merkleRootHash;
  }

  // Mint on presale / raffle using ETH
  function preSaleMint(uint256 qty, bytes32[] memory _proof)
    external
    payable
    nonReentrant
    onlyEOA
  ...
  function teamMint() external onlyOwner {
    ...
    uint256 numChunks = TEAM_SUPPLY / maxBatchSize;
    ...
    for (uint256 i = 0; i < numChunks; i++){
        ...
        emit teamMinted(msg.sender, maxBatchSize);
        
        for (uint256 j = 0; j < maxBatchSize; j++) {               
            _setTokenRoyalty(updatedIndex, msg.sender, royaltyFee);            
            updatedIndex++;
        }
    }
  ...
  function withdraw() external onlyOwner {
    ...
    require(balance > 0 || balanceAPE > 0, "Withdraw: Insufficient ETH or APE");
    bool success;
    if(balance > 0){
  ...
  function calcRoyaltyFee(
      uint256 tokenId,
      uint256 saleValue
  ) public view returns (uint256) {
      RoyaltyInfo memory royalties = _royalties[tokenId];
      uint256 royaltyAmount = (saleValue * royalties.amount) / 10000;
  ...
  /**
    ...
    */
  function _safeTransferAPE(address from, address to, uint256 amount) internal returns (bool) {
 ...
 * Assumes that an owner cannot have more than the 2**128 - 1 (max value of uint128) of supply
 */
abstract contract ERC721ARoyalty is
    ERC721A,
    IERC2981Royalties
{
    // Royalty
