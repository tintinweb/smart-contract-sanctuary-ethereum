...
contract asset {
    ...
    function registerUser(string memory _name, uint _age, string memory _city,string memory _aadharNumber, string memory _phoneNumber, string memory _document, string memory _email
        ...
        UserMapping[msg.sender] = User(msg.sender, _name, _age, _city,_aadharNumber,_phoneNumber, _document,_email,true);
    ...
    //-----------------------------------------------asset-----------------------------------------------
    function addasset(string memory _fileName, string memory _address, uint _assetPrice,string memory _allLatiLongi, string memory _category,string memory _surveyNum, string memory _document) public {
        require(isUserVerified(msg.sender));
        assetsCount++;
        assets[assetsCount] = assetreg(assetsCount, _fileName, _address, _assetPrice,_allLatiLongi,_category, _surveyNum , _document,false,payable(msg.sender),true);
