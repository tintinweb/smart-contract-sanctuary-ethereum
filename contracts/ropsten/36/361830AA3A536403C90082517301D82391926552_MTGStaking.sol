...
contract MTGStaking is Ownable, ReentrancyGuard {
    ...
    address public ArcadaAddress = 0xa5172D3706f7736B758539d0ff0576495891CB8c;
    ...
    function _stake(
        ...
        uint256 _lockInEndAt = block.timestamp + SEVEN_DAYS;
    ...
    function _restakeAndClaim(
        ...
        uint256 _lockInEndAt = block.timestamp + SEVEN_DAYS;
        stakingRecords[_tokenId].lockInEndAt = _lockInEndAt;
    ...
    function getStakingRecords(address _user)
        public
        view
    ...
    // MIGRATION ONLY.
    function setMTGNFTContract(address _address) public onlyOwner {
        MTG = IMTG(_address);
    }

    function setArcadaContract(address _address) public onlyOwner {
        Arcada = IArcada(_address);
    }
