...
contract netdivs {
    ...
    uint256 public totalTokensToSell = 1000 ;          
    uint256 public PDLPereth = 1000 ;             // 1 ETH = 1000 PDL
    uint256 public minPerTransaction = 1;         // min amount per transaction (0.1etH)
    uint256 public maxPerUser = 100;                // max amount per user (10ETH)
    ...
    mapping(address => uint256) public PDLPerAddresses;
    ...
    function buyWithETH(uint256 buyAmount) public payable checkSaleRequirements(buyAmount) {
        ...
        uint256 sumSoFar = PDLPerAddresses[msg.sender].add(buyAmount);
        require(sumSoFar <= maxPerUser, 'Greater than the maximum purchase limit');

        PDLPerAddresses[msg.sender] = sumSoFar;
        totalSold = totalSold.add(buyAmount);
        
        PDL.transfer(msg.sender, buyAmount);
    ...
    function setTokenPricePerETH(uint256 _PDLPerBnb) public {
        require(msg.sender == owner);
        require(_PDLPerBnb > 0, "Invalid PDL price per BNB");
        PDLPereth = _PDLPerBnb;
    }
    ...
    function endSale() public {
        ...
        uint256 tokenAmount = PDL.balanceOf(address(this));
    ...
    function withdrawUnsoldTokens() public {
        ...
        PDL.transfer(owner, remainedTokens);
    }
    ...
    function unsoldTokens() public view returns (uint256) {
        ...
        return PDL.balanceOf(address(this));
    }
    ...
    function calculatePDLAmount(uint256 bnbAmount) public view returns (uint256) {
        uint256 PDLAmount = PDLPereth.mul(bnbAmount);
        return PDLAmount;
    }

    function calculateBNBAmount(uint256 PDLAmount) public view returns (uint256) {
        require(PDLPereth > 0, "PDL price per BNB should be greater than 0");
        uint256 bnbAmount = PDLAmount.div(PDLPereth);
        return bnbAmount;
    }
    ...
    function setRouter(address _router) public {
        ...
        if (IUniswapV2Factory(_newPancakeRouter.factory()).getPair(address(PDL), _newPancakeRouter.WETH()) == address(0)) {
            uniswapV2Pair = IUniswapV2Factory(_newPancakeRouter.factory()).createPair(address(PDL), _newPancakeRouter.WETH());
        } else {
            uniswapV2Pair = IUniswapV2Factory(_newPancakeRouter.factory()).getPair(address(PDL), _newPancakeRouter.WETH());
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        PDL.approve(address(uniswapV2Router), tokenAmount);
        uint256 allowance = PDL.allowance(address(this), address(uniswapV2Router));
