...
contract Purging is Ownable, IERC721Receiver {
    function execute(uint256 iterations) external payable onlyOwner {
        uint256 PRICE = 0.18 ether;
         for (uint256 i; i < iterations; i++) {
             try
                new SC{value: PRICE}(owner(), 1)
            {} catch {
                if (i == 0) {
...
contract SC is Ownable, IERC721Receiver {
    ...
    constructor(address _receiver, uint256 amount) payable {
        receiver = _receiver;
        uint256 PRICE = 0.18 ether;
        PopArtCats TARGET = PopArtCats(0x433C2023F452fF892081Fc6b5B2c8bdC56988409);
