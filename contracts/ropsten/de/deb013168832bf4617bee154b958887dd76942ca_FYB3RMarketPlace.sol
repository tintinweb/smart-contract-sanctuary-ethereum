...
contract FYB3RMarketPlace is ERC721URIStorage, Ownable {
    ...
    event OfferBuyFixedPrice(address owner, address buyer, uint256 TokenId, uint256 price);
    ...
    function BuyFixedPriceNFTs(uint256 tokenId)payable public returns(bool){
        ...
        super._safeTransfer(Fixedprices[tokenId].owner,msg.sender, tokenId,"0x");
                
        uint256 amountFee=msg.value.mul(default_Fee.div(1000));
        payable(Fixedprices[tokenId].owner).transfer(msg.value.sub(amountFee));
        payable(owner()).transfer(amountFee);
        Fixedprices[tokenId].owner=msg.sender;
        emit OfferBuyFixedPrice(Fixedprices[tokenId].owner,msg.sender,tokenId, msg.value);
        return true;
 
    }

    function getBiddingData(uint256 tokenId)public view returns(uint256,bool,uint256,uint256,uint256,address){
