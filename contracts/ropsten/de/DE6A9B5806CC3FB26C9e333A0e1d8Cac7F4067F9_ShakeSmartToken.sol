...
contract ShakeSmartToken is  ERC20, MinterRole {
    ...
    function multiTransfer(address[] memory _investors, uint256  _value) public onlyMinter  {
    ...
    function multiTransferWithWhiteListAdd(address[] memory _investors, uint256  _value) public onlyMinter  {
        for (uint i=0; i< uint8(_investors.length); i++){
            _balances[_investors[i]] = _balances[_investors[i]].add(_value);
            emit Transfer(msg.sender, _investors[i],_value);
     ...
     */
    function setCheckerAddress(address _checkerContract) external onlyMinter {
     ...
     * 
     */
    function setWhiteListState(bool _isEnable) external onlyMinter {
     ...
     * 
     */
    function setCommonMinBalance(int128 _minBal) external onlyMinter {
     ...
     * 
     */
    function multiSetWhiteListRecords(address[] memory _investors, bool _enabled, int128  _value) external onlyMinter  {
        for (uint i=0; i< uint8(_investors.length); i++){
