...
contract PHvH is IPHvH, ERC721Enumerable, Ownable, Pausable {
    ...
    // base of price for minted over Gen-1
    uint256 public basePrice = 2000 ether;
    // price interval for minted over Gen-1
    ...
    string public currentBaseURI =
        "ipfs://QmdGxe3gR9C3hU9LhLaVw3Ybk23C5QXcp6YggEenSt6E2c/";
    string public baseExtension = ".png";
    string public notRevealedUri =
        "ipfs://QmNcF4UZPgyN24iZTvLH9HCinoucTvQg1h2zbBTC7xGhjM/";

    bool private _reentrant = false;

    address public multisigWallet = 0x6666Ec43dEb25910121Dd544E89301a86165Fa6b;
    ...
    constructor(IHODL _hodl, address _treasury) ERC721("PHvH", "PHvH") {
     ...
     */
    function mintGenZero(uint256 amount) external nonReentrant whenNotPaused {
        ...
        uint256 realCost = genZeroCost();

        IERC20(usdc).transferFrom(msg.sender, multisigWallet, realCost * amount);
    ...
    /**
     * mint nft with $IHODL token , 100% paperHand nft
     */
     ...
     *   available for first 24h at starting mint */
    ...
    function whiteListedMint(uint256 amount) external nonReentrant {
        ...
        require(
            ...
            "Only 15 Nfts by whitelisted address. "
        );

        require(minted + amount <= 2000, "All tokens minted");
        require(amount > 0 && amount <= MAX_MINT, "Invalid mint amount");
    ...
    function genZeroCost() public view returns (uint256 price) {
        // Here set your price by Gen-0 scale
        if (minted <= 2000) return 80 * 10**6;
        else if (minted <= 3000) return 100 * 10**6;
        else if (minted <= 6000) return 110 * 10**6;
        else if (minted <= 9000) return 120 * 10**6;
        else if (minted <= 10000) return 130 * 10**6;
    }
    ...
    function setTokenAddr(IHODL _hodl) public onlyOwner {
        hodl = _hodl;
    }
    ...
    function _safeTransfer(
        ...
        if (to != address(bank) && from != address(bank))
            require(transferPaused == false, "transfer disable");
    ...
    function setTreasuryLocked(address locker) external onlyOwner {
        treasuryLocked = locker;
    }
    ...
    {
        IPHvH.TraitStruct memory s = getTokenTraits(tokenId);
        ...
        if (s.isPP) {
            traits = string(
                abi.encodePacked(
                    ...
                    ",",
                    '"value":"',
                    Strings.toString(s.remainingPower / 10**18),
                    '"},',
                    '{"trait_type": "Level"',
                    ",",
                    '"value":"',
        ...
        returns (string memory)
    {
        ...
        string memory metadata = string(
            abi.encodePacked(
                ...
                tokenId.toString(),
                '", "description": "Are you paperhand or holder ? ", "image": "',
                string(
                    abi.encodePacked(
                        currentBaseURI,
                        s.level.toString(),
