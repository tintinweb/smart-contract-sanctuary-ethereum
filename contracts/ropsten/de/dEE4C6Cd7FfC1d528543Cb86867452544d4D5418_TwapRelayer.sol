...
contract TwapRelayer is ITwapRelayer {
    ...
    uint256 private constant DELAY_EXECUTE_GAS_LIMIT = 500000;
    uint256 private constant GAS_PRICE_MULTIPLIER = 1050;
    uint256 private constant GAS_PRICE_MULTIPLIER_DENOMINATOR = 1000;
    ...
    function sell(Orders.SellParams calldata sellParams) external payable override lock returns (uint256 orderId) {
        ...
        uint256 gasPrepay = msg.value;
        if (gasPrepay == 0) {
            uint256 gasPrice = (getGasPrice() * GAS_PRICE_MULTIPLIER) / GAS_PRICE_MULTIPLIER_DENOMINATOR;
            gasPrepay = gasPrice * DELAY_EXECUTE_GAS_LIMIT;
            gasPrepay = 5000000000000000;
        }
        if (sellParams.tokenIn == weth && sellParams.wrapUnwrap) {
            gasPrepay = gasPrepay.sub(_amountIn, 'TR1E');
        }
        uint256 balanceBefore = address(this).balance;

        orderId = ITwapDelay(delay).sell{ value: gasPrepay }(
            Orders.SellParams(
                sellParams.tokenIn,
                sellParams.tokenOut,
                _amountIn,
    ...
    function buy(Orders.BuyParams calldata buyParams) external payable override lock returns (uint256 orderId) {
        ...
        if (buyParams.tokenIn == weth && buyParams.wrapUnwrap) {
            gasPrepay = gasPrepay.sub(_amountIn, 'TR1E');
        }
        uint256 balanceBefore = address(this).balance;
    ...
    function getGasPrice() internal returns (uint256 gasPrice) {
        gasPrice = tx.gasprice - (tx.gasprice % 1e6);
        if (gasPrice <= 0) {
            gasPrice = ITwapDelay(delay).gasPrice();
        }
    }
    ...
    function calculateAmountIn(
        ...
        (uint8 xDecimals, uint8 yDecimals, uint256 price) = getPrice(pair, inverted);
    ...
    function getPrice(address pair, bool inverted)
        public
        view
        override
        returns (
        ...
        )
    {
        ...
        if (inverted) {
            uint256 _invertedPrice = spotPrice > averagePrice ? spotPrice : averagePrice;
            price = uint256(10**36).div(_invertedPrice);
            invertedPrice = spotPrice < averagePrice ? spotPrice : averagePrice;
        } else {
            price = spotPrice < averagePrice ? spotPrice : averagePrice;
            uint256 _price = spotPrice > averagePrice ? spotPrice : averagePrice;
            invertedPrice = uint256(10**36).div(_price);
        }
    }
