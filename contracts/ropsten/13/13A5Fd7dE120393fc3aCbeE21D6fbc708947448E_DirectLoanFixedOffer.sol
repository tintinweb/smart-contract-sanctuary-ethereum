...
// File: contracts/utils/interfaces/LoanData.sol
...
interface LoanData {
     ...
     * @param nftCollateralWrapper - The Solarr wrapper of the NFT collateral contract.
     ...
     * off-chain Solarr orders. These nonce can be any uint256 value that the user has not previously used to sign an
     * off-chain order. Each nonce can be used at most once per user within Solarr, regardless of whether they are the
     * lender or the borrower in that situation. This serves two purposes:
     * - First, it prevents replay attacks where an attacker would submit a user's off-chain order more than once.
     * - Second, it allows a user to cancel an off-chain order by calling Solarr.cancelLoanCommitmentBeforeLoanHasBegun()
...
// File: contracts/utils/interfaces/IDirectLoanBase.sol
...
// File: contracts/utils/interfaces/IDirectLoanCoordinator.sol
...
// File: contracts/utils/library/ContractKeys.sol
...
library ContractKeys {
    ...
    bytes32 public constant SOLARR_BUNDLER = bytes32("SOLARR_BUNDLER");
...
// File: contracts/utils/interfaces/ISolarrHub.sol
 ...
 * @dev SolarrHub interface
 */
interface ISolarrHub {
    function setContract(string calldata _contractKey, address _contractAddress) external;

    function getContract(bytes32 _contractKey) external view returns (address);
}

// File: contracts/utils/interfaces/IPermittedPartners.sol
...
// File: contracts/utils/interfaces/IPermittedERC20s.sol
...
// File: contracts/utils/library/LoanChecksAndCalculations.sol
...
library LoanChecksAndCalculations {
     ...
     */
    function payBackChecks(uint32 _loanId, ISolarrHub _hub) external view {
    ...
    function checkLoanIdValidity(uint32 _loanId, ISolarrHub _hub) public view {
        require(
     ...
     */
    function getRevenueSharePercent(address _revenueSharePartner, ISolarrHub _hub) external view returns (uint16) {
     ...
     * off-chain Solarr orders. These nonces can be any uint256 value that the user has not previously used to sign an
     * off-chain order. Each nonce can be used at most once per user within Solarr, regardless of whether they are the
     * lender or the borrower in that situation. This serves two purposes:
     * - First, it prevents replay attacks where an attacker would submit a user's off-chain order more than once.
     * - Second, it allows a user to cancel an off-chain order by calling Solarr.cancelLoanCommitmentBeforeLoanHasBegun()
...
// File: contracts/utils/interfaces/IAirdropFlashLoan.sol
...
// File: contracts/utils/interfaces/INftWrapper.sol
...
// File: contracts/utils/interfaces/IAirdropReceiverFactory.sol
...
// File: contracts/utils/library/LoanAirdropUtils.sol
...
// File: contracts/utils/BaseLoan.sol
...
// File: contracts/utils/NftReceiver.sol
...
// File: contracts/utils/interfaces/IBundleBuilder.sol
...
// File: contracts/utils/library/SolarrSigningUtils.sol
...
/**
 * @title  SolarrSigningUtils

 * @notice Helper contract for Solarr. This contract manages verifying signatures from off-chain Solarr orders.
 * Based on the version of this same contract used on Solarr V1
 */
library SolarrSigningUtils {
    /* ********* */
    /* FUNCTIONS */
    /* ********* */
     ...
     * Solarr.cancelLoanCommitmentBeforeLoanHasBegun(), which marks the nonce as used and prevents any future loan from
...
// File: contracts/utils/interfaces/IPermittedNFTs.sol
...
// File: contracts/DirectLoanBaseMinimal.sol
 ...
 * @notice Main contract for Solarr Direct Loans Type. This contract manages the ability to create NFT-backed
 * peer-to-peer loans.
 *
 * There are two ways to commence an NFT-backed loan:
 *
 * a. The borrower accepts a lender's offer by calling `acceptOffer`.
 *   1. the borrower calls nftContract.approveAll(Solarr), approving the Solarr contract to move their NFT's on their
 * be1alf.
 *   2. the lender calls erc20Contract.approve(Solarr), allowing Solarr to move the lender's ERC20 tokens on their
 * behalf.
 ...
 * Solarr promissory note (in ERC721 form) that represents the rights to either the principal-plus-interest, or the
 ...
 * receives an Solarr promissory note (in ERC721 form) that represents the rights to either the principal-plus-interest,
 ...
 * - First, a borrower may call Solarr.payBackLoan() and pay back the loan plus interest at any time, in which case they
 ...
 * Solarr.liquidateOverdueLoan(), in which case they receive the underlying NFT collateral and forfeit the rights to the
...
abstract contract DirectLoanBaseMinimal is IDirectLoanBase, IPermittedERC20s, BaseLoan, NftReceiver, LoanData {
     ...
     * nonces that are used by both the lender and the borrower when they are first signing off-chain Solarr orders.
     *
     * These nonces can be any uint256 value that the user has not previously used to sign an off-chain order. Each
     * nonce can be used at most once per user within Solarr, regardless of whether they are the lender or the borrower
    ...
    ISolarrHub public immutable hub;
    ...
    /**
     * @notice This event is fired whenever a borrower begins a loan by calling Solarr.beginLoan(), which can only occur
     * after both the lender and borrower have approved their ERC721 and ERC20 contracts to use Solarr, and when they
     * both have signed off-chain messages that agree on the terms of the loan.
     *
     * @param  loanId - A unique identifier for this particular loan, sourced from the Loan Coordinator.
     * @param  borrower - The address of the borrower.
     * @param  lender - The address of the lender. The lender can change their address by transferring the Solarr ERC721
     ...
     *
     * @param _admin - Initial admin of this contract.
     * @param  _solarrHub - SolarrHub address
     ...
     * instead to nonces that are used by both the lender and the borrower when they are first signing off-chain Solarr
     * orders. These nonces can be any uint256 value that the user has not previously used to sign an off-chain order.
     * Each nonce can be used at most once per user within Solarr, regardless of whether they are the lender or the
     ...
     * Solarr orders. These nonces can be any uint256 value that the user has not previously used to sign an off-chain
     * order. Each nonce can be used at most once per user within Solarr, regardless of whether they are the lender or
    ...
    function _renegotiateLoan(
        ...
        require(
            SolarrSigningUtils.isValidLenderRenegotiationSignature(
                _loanId,
                _newLoanDuration,
                _newMaximumRepaymentAmount,
                _renegotiationFee,
...
// File: contracts/DirectLoanFixedOffer.sol
 ...
 * @notice Main contract for Solarr Direct Loans Fixed Type. This contract manages the ability to create NFT-backed
...
contract DirectLoanFixedOffer is DirectLoanBaseMinimal {
    ...
    function _acceptOffer(
        ...
        // uint256 value that they have not used yet for an off-chain Solarr
        // signature).
        ...
        require(SolarrSigningUtils.isValidLenderSignature(_offer, _signature), "Lender signature is invalid");

        address bundle = hub.getContract(ContractKeys.SOLARR_BUNDLER);
