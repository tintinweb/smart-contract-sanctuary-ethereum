...
contract BMSport is BMEvents, Ownable {
    ...
    mapping(uint256 => mapping(uint256 => BMDatasets.PlayerComment)) public playerComments_;
        ...
        isWithinLimits(msg.value)
    {
        ...
        //     BMBook.setPlayerLAff(_pID, _affCode);
        //     _affID = _affCode;
        // } else {
        //     _affID = BMBook.getPlayerLAff(_pID);
        // }
        
        // purchase keys for each team
        buysCore(_gameID, _pID, _teamEth, 0);
        ...
        private
    {
        ...
        if (_totalEth >= 1000000000000000) {
            ...
            playerComments_[_gameID][playerCommentsIndex_[_gameID]] = BMDatasets.PlayerComment(_pID, _name, _totalEth, _comment);
        ...
        private
    {
        ...
        for (i = 0; i < _nt; i++) {
            if (_teamEth[i] > 0) {
                ...
                emit BMEvents.onPurchase(_gameID, _pID, msg.sender, _name, i, _teamEth[i], _keys[i], _affID, now);
            }
        }
        ...
        private
    {
        ...
        if (!address(Banker_Address).call.value(_com)(abi.encode("deposit()"))) {
        ...
        returns (uint256)
    {
        ...
        if (_affID == 0 || _affID == _pID) {
            _com = _aff;
        } else if(BMBook.getPlayerHasAff(_affID)) {
            BMBook.depositAffiliate.value(_aff)(_affID);
        } else {
            _com = _aff;
        }
