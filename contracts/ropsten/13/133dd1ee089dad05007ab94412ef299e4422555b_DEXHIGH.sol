...
contract DEXHIGH is RedBlackTree, SafeMath
{
    ...
    function GetTickSize(uint lPrice) public constant returns (uint tickSize)
    {
        uint digit = 0;
        uint firstNum = 0;
        (tickSize, digit, firstNum) = GetTickSize3(lPrice);
    }

    function GetTickSize3(uint lPrice)  public constant returns (uint tickSize, uint digit, uint firstNum)
    {
        digit = 0;
        tickSize = 1;
        while (lPrice >= 10)
    ...
    function GetNbyPrice(uint lPrice) public constant returns (uint)
    {
        ...
        (baseN, lTickSize, digit, firstNum) = GetBaseN(lPrice);
        uint lBasePrice = GetBasePrice(digit, firstNum);
        return (uint)((lPrice - lBasePrice) / lTickSize) + baseN;
    }

    function GetBaseN(uint lPrice) private constant returns (uint N, uint lTickSize, uint digit, uint firstNum)
    {
        (lTickSize, digit, firstNum) = GetTickSize3(lPrice);
    ...
    function GetHoga(address token, uint32 hogaN, bool isExpand) public constant returns (uint[] memory priceB, uint[] memory volumeB, uint[] memory priceA, uint[] memory volumeA)
    {
