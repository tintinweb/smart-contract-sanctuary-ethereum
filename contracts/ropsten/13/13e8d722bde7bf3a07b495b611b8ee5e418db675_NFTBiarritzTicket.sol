...
contract NFTBiarritzTicket is ERC721, Ownable {
    ...
    function updateBasePrice(uint256 price) public onlyOwner {
        basePrice = price * 1000000000000000000;
    }
    ...
    function getDiscount(address buyer) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < ERC721List.length; i++) {
            IERC721 ERC721Contract = IERC721(ERC721List[i]);
            if (ERC721Contract.balanceOf(buyer) >= 1) {
        ...
        for (uint256 i = 0; i < ERC1155List.length; i++) {
            IERC1155 ERC1155Contract = IERC1155(ERC1155List[i]);
            if (
    ...
    function mint(uint256 num) public payable {
        require(quantityMinted() + num <= MAX_SUPPLY, "Exceeds maximum supply");
        require(
