...
contract ERC721 {
    ...
    /**
     * @dev Emitted when the authorised minter is changed
     */
     ...
     * Additionally the contracts authorised token minter address is set. 
     */
    constructor(string memory name_, string memory symbol_, string memory baseTokenURI_, address minter_) {
        _name = name_;
        _symbol = symbol_;
     ...
     */
   modifier onlyMinter {
       require(msg.sender == minter(), "msg.sender must have the minter role");
      _;
   }
     ...
     * Requirements:
     *
     * - caller must be the contract's assigned token minter.
     * - `tokenId` must not exist.
     * - If `to` refers to a smart contract, it must implement {IERC721Receiver-onERC721Received}, which is called upon a safe transfer.
     *
     * Emits a {Transfer} event.
     */
    function mint(address to, uint256 tokenId, bytes calldata data) public onlyMinter {
