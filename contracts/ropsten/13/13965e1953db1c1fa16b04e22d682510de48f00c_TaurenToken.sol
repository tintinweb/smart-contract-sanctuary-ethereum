...
contract TaurenToken is Context, IBEP20, Ownable {
    ...
    mapping (uint256 => address[]) private bullUsers;
    ...
    address public BUSD = 0x4B4c2e58e7E1c45DEAFAEb5f0366B4697900c930;
    ...
    constructor () {
        ...
        TRN = TaurenNFT(0xff3EBda64069f9A8B47E751cA79De41C34096923);
    ...
    function rescueBullNFT(uint256 tokenId) public {

        uint256 _bullPeriodIndex = TRN.tokenIdToPeriod(tokenId);

        require(hammerOwner[_bullPeriodIndex].length < totalSealTimes, "This NFT has been unlocked");

        require(checkBullUsers(_bullPeriodIndex, msg.sender), "You did not participate in this activity");

        address preOwner =  hammerOwner[_bullPeriodIndex][hammerOwner[_bullPeriodIndex].length - 1];
        ...
        uint256 price = hammerBasePrice.mul(105 ** (hammerOwner[_bullPeriodIndex].length - 1)).div(100 ** (hammerOwner[_bullPeriodIndex].length - 1));
        ...
        if(hammerOwner[_bullPeriodIndex].length >= totalSealTimes){
            ...
            TRN.approveToController(preOwner, tokenId);
    ...
    function sellTaurenNFT(uint256 tokenId) public {
        ...
        ERC20(BUSD).transferFrom(castingPoolAddr, msg.sender, TRN.typeToNFTPrice(1));
    ...
    function getBullNFTDetail(uint256 _bullPeriodIndex) public view returns(address currentOwner, uint256 price, uint256 sealTimes) {
        ...
        if(hammerOwner[_bullPeriodIndex].length > 0){
            // price = hammerBasePrice.mul(105 ** (hammerOwner[_bullPeriodIndex].length - 1)).div(100 ** (hammerOwner[_bullPeriodIndex].length - 1));
        }

        sealTimes = totalSealTimes - bullUsers[_bullPeriodIndex].length;

        // currentOwner = bullUsers[_bullPeriodIndex][bullUsers[_bullPeriodIndex].length - 1];

        return (currentOwner, price, sealTimes);
    }

    function getHammerCurrentOwner(uint256 _bullPeriodIndex) public view returns(address user) {

        user = bullUsers[_bullPeriodIndex][bullUsers[_bullPeriodIndex].length - 1];
