...
contract Banger is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "BNG";//
    ...
    address payable private _developmentAddress = payable(0xf575A94ece5E90f58D22E5dac011d11db097C297);//
    address payable private _marketingAddress = payable(0x164641c24b03f412E78344636671a0ceb84B7166);//
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router)) {
                ...
                if(block.number < launchBlock + 1) {
                    _taxFee = 80;
                } else if(block.timestamp <= launchTimestamp + (45 seconds)) {
                    _taxFee = 40;
                } else if (block.timestamp <= launchTimestamp + (2 minutes)) {
                    _taxFee = 20;
                }
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                ...
                uint256 pairBal = balanceOf(to);
                if (amount >= (pairBal * 20) / 100 && _taxFee < 25) {
                    _taxFee = 25;
                }
                if (amount >= (pairBal * 40) / 100) {
                    _taxFee = 50;
                }
            }
