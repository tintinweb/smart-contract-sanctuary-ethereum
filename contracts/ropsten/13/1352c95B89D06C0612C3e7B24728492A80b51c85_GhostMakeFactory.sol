...
interface IBridgeCoinClubMintable {
...
import "../interfaces/IBridgeCoinClubMintable.sol";
import "../core/SafeOwnable.sol";

contract SmartMintableInitializable is ReentrancyGuard {
    using SafeERC20 for IERC20;

    // The address of the smart minner factory
    address public SMART_MINNER_FACTORY;

    IBridgeCoinClubMintable public bccToken;
    ...
    function initialize(
        ...
        isInitialized = true;
        bccToken = IBridgeCoinClubMintable(_bccToken);
        reserve = _reserve;
    ...
    function mint() external payable nonReentrant onlyWhitelist {
        ...
        remaning -= 1;
        bccToken.mint(msg.sender);
    }

    modifier onlyWhitelist() {
        require(
            !hasWhitelistLimit ||
                GhostMakeFactory(SMART_MINNER_FACTORY).whitelist(
                    msg.sender
                ),
...
contract GhostMakeFactory is SafeOwnable {
    ...
    function setAdmin(address admin, bool enable) external onlyOwner {
        require(admin != address(0), "GhostMakeFactory: address is zero");
        isAdmin[admin] = enable;
        emit SetAdmin(admin, enable);
    }

    modifier onlyAdmin() {
        require(isAdmin[msg.sender], "GhostMakeFactory: caller is not the admin");
        _;
    }
