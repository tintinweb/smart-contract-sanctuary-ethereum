...
contract AutoAdjustablePortfolio is IAutoAdjustablePortfolio, BasePortfolio {
    ...
    function initialize(
        ...
        require(
            _minInterestRateUtilizationThreshold <= _optimumUtilization && _optimumUtilization <= _maxInterestRateUtilizationThreshold,
            "AutoAdjustablePortfolio: Min. Util. <= Optimum Util. <= Max. Util. constraint not met"
        );
        __BasePortfolio_init(_duration, _underlyingToken, _borrower);
        borrower = _borrower;
