...
abstract contract NexusShares {
	/**
	 * @notice Register a payment of funds in tokens. May be called directly after a deposit is made.
	 * @dev Calls _updateFundsTokenBalance(), whereby the contract computes the delta of the previous and the new 
	 * funds token balance and increments the total received funds (cumulative) by delta by calling _registerFunds()
	 */
	function updateFundsReceived() external virtual;
}
...
contract TokenSwap {
    ...
    function getEstimatedMaticForNerve(uint256 nerveAmount) public view returns (uint256) {
        return nerveswapRouter.getAmountsOut(nerveAmount, getPathForNerveToMatic())[1];
    }
    ...
    function getPathForNerveToMatic() private view returns (address[] memory) {
        address[] memory path = new address[](2);
        path[0] = nerveToken;               // NERVE
        path[1] = nerveswapRouter.WETH();   // wrapped MATIC
...
contract NerveToken is ERC20, TokenSwap  {
    ...
    uint8 public userRate = 65;
    uint8 public nexusRate = 35;
    ...
    function mintNerve(address _to, uint256 _amount) internal  
    {
        ...
        uint256 userAmount =  nerveForMatic * userRate / 100; 
        uint256 nexusAmount = nerveForMatic * nexusRate / 100; 

        _mint(_to, userAmount);
        _mint(nexusShares, nexusAmount);
        NexusShares(nexusShares).updateFundsReceived();
...
contract NerveGlobal is NerveSocial, NerveToken
{
    ...
    function initialize(address _nexusBurn, address _nexusShares, address _router) public
    {
        require(nexusShares == address(0), "Already initialized.");
        nexusBurn = _nexusBurn;
        nexusShares = _nexusShares;
        nerveswapRouter = IUniswapV2Router02(_router);
    }
