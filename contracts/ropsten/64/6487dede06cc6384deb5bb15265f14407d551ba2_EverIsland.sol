...
contract EverIsland is ERC721A, Ownable {
    ...
    uint256 public PRSALEPRICE = 0.08 ether;
    ...
    */
    ...
    function mintNumberOfTokens(uint numberOfTokens) private{
        ...
        for (uint j = startTokenId; j < endTokenId; j++){
            for (uint i = 0; i < 20; i++) {
                _lands[j]._land[i] = msg.sender;
            }
        }
    }

    function ownerOfLand(uint256 islandId,uint landId) public view returns (address){
        require(_exists(islandId), "operator query for nonexistent token");
        return _lands[islandId]._land[landId];
    }

    function landOfOwnerByIsland(uint256 islandId) public view returns (address[] memory) {
        require(_exists(islandId), "operator query for nonexistent token");
        address[] memory array = new address[](20);
        for (uint i = 0; i < 20; i++) {
            array[i] = _lands[islandId]._land[i];
        }
        return array;
    }
    
    function isLandApprovedOrOwner(address spender, uint256 islandId, uint landId) internal view returns (bool) {
        require(_exists(islandId), "operator query for nonexistent token");
        address owner = ownerOfLand(islandId,landId);
        return (spender == owner || isApprovedForAll(owner, spender));
    }

    function safeTransferLand(address from,address to,uint256 islandId, uint landId) public {
        require(isLandApprovedOrOwner(_msgSender(), islandId,landId), "transfer caller is not owner nor approved");
        require(to != address(0), "transfer to the zero address");
        _lands[islandId]._land[landId] = to;
        emit TransferLand(from, to, islandId,landId);
    }
