    ...
    */
    ...
    function buyLevel(uint8 level, uint256 investAmount) public payable {
        ...
        if (isRootUser) {
            ...
            users[referrer].balance += amountForRootUser;
        ...
        if (rewardAddress != msg.sender) {
            ...
            if (sent) {
                users[rewardAddress].balance += restOfAmount;
    ...
    function getSystemBalances() public view returns (uint256 regFee, uint256 marketing) {
        return (regFeeBalance, marketingBalance);
    }
