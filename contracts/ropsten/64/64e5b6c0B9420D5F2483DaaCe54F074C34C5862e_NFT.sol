...
contract NFT is Initializable, ERC721URIStorageUpgradeable, EIP712Upgradeable, OwnableUpgradeable {
    ...
    event NFTRedeemed(address redeemer, uint256 tokenId);

    function initialize(string memory name_, string memory symbol_, string memory hiddenName, string memory version_, address minter, address payable _paymentReceiver) public initializer {
    ...
    function redeem(address redeemer, NFTVoucher calldata voucher, bytes memory signature) public payable returns (uint256) {
        ...
        emit NFTRedeemed(msg.sender, voucher.tokenId);
