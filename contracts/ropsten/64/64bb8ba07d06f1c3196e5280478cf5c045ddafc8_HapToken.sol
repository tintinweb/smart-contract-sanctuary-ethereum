...
contract HapToken is ERC20, EIP712, Ownable, ReentrancyGuard {
    ...
    constructor(address _voucherSigner, Manager[] memory _managers)
        ERC20("HAPS", "HAPS")
        EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION)
    {
        require(_voucherSigner != address(0), "Zero address");
        voucherSigner = _voucherSigner;

        for (uint256 i = 0; i < _managers.length; i++) {
            _mint(_managers[i].wallet, _managers[i].value);
        }
    }
    ...
    function mint(HaploVoucher calldata userData) external nonReentrant {
        ...
        require(_msgSender() == userData.wallet, "Not your voucher");
