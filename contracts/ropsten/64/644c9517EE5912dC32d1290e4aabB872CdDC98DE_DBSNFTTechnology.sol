...
contract DBSNFTTechnology  is Ownable{
    ...
    function Recast(uint256[] memory _ids) public  returns (uint256, uint256){
        ...
        if(quality == uint256(NFTQuality.NFTCopper)){
            ...
            uint256[] memory tokens = nftAddress.mint(msg.sender, cids[cindex], 1);

            emit newRecast(msg.sender, _ids, tokens[0], cids[cindex]);
            return (tokens[0], cids[cindex]);
            ...
            uint256[] memory tokens = nftAddress.mint(msg.sender, sids[sindex], 1);

            emit newRecast(msg.sender, _ids, tokens[0], sids[sindex]);
            return (tokens[0], sids[sindex]);
        }
    ...
    function RecruitCopper(uint256 _id) public returns (uint256, uint256){
        ...
        uint256[] memory tokens = nftAddress.mint(msg.sender, copperSkill[index], 1);

        emit newRecruitCopper(msg.sender, _id, tokens[0], copperSkill[index]);
        return (tokens[0], copperSkill[index]); 
    }
    ...
    function RecruitDest(uint256[] memory _ids) public returns (uint256, uint256){
        ...
        if(quality == uint256(NFTQuality.NFTCopper)){
            nftAddress.burnBatch(msg.sender, _ids);
            uint256[] memory tokens = nftAddress.mint(msg.sender, copperShip[IDToIndex[tableId]].skill, 1);

            emit newRecruitDest(msg.sender, _ids, tokens[0], copperShip[IDToIndex[tableId]].skill);
            return (tokens[0], copperShip[IDToIndex[tableId]].skill); 
        }else if(quality == uint256(NFTQuality.NFTSilver)){
            nftAddress.burnBatch(msg.sender, _ids);
            uint256[] memory tokens = nftAddress.mint(msg.sender, silverShip[IDToIndex[tableId]].skill, 1);
    ...
    function Decompose(uint256 _id) public  returns (uint256[] memory, uint256[] memory){
        ...
        if(quality == uint256(NFTQuality.NFTSilver)){
            ...
            uint256[] memory tokens = nftAddress.singleBatchMint(msg.sender, destcids);
            emit newDecompose(msg.sender, _id, tokens, destcids);
            return (tokens, destcids); 
        }else{
            nftAddress.burn(msg.sender, _id);
