...
contract MIKAWAINU is Context, IBEP20, LockToken 
{
    ...
    {
        uint256 _totalFees = _liquidityFee.add(_marketingFee).add(_developerFee);
                _totalFees = _totalFees.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleDeveloperFee);
        ...
        uint256 ethForMarketing = newBalance.mul(_marketingFee+_saleMarketingFee).div(_totalFees);
        uint256 ethForDeveloper = newBalance.sub(ethForMarketing).sub(ethForLiquidity);
        marketingAddress.transfer(ethForMarketing);
        marketingAddress.transfer(ethForDeveloper);
