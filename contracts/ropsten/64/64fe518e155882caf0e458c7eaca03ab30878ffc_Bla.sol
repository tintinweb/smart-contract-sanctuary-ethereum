...
contract Bla is ERC20, Ownable {
    ...
    constructor() ERC20("Bla", "Bla") {
		...
		// fees 6.66 %		
        uint256 _LiquidityFee = 222;
        uint256 _TaxFee = 444;
 
        uint256 totalSupply = 1 * 1e12 * 1e18;
 
        maxWallet = 2 * 1e10 * 1e18; // Max Wallet 2%
        swapTokensAtAmount = 1 * 1e9 * 1e18; // 0.1% swap threshold for the CA
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
        require(newAmount >= 1 * 1e8 * 1e18, "Swap amount cannot be lower than 0.01% total supply.");
        require(newAmount <= 5 * 1e9 * 1e18, "Swap amount cannot be higher than 0.5% total supply.");
    ...
    function _transfer(
        ...
        if(takeFee){
			...
			if (TotalFees > 0) {
				...
				tokensForTax += fees * TaxFee / TotalFees;
			}
    ...
    function swapBack() private {
		...
		uint256 ethForMarketing = ethForTax / 2;
