...
contract T94 is IERC20, Auth {
	string constant _name = "Tel94";
	string constant _symbol = "T94";
	...
	mapping (address => uint256) public _lastTxBlock; //TODO MAKE PRIVATE
	...
	event TokenBurn(address indexed burnedByWallet, uint256 tokenAmount);
	...
	constructor () Auth(msg.sender) {      
		...
		//TODO - set balances to be transferred at launch to specific addresses based on final numbers for the relaunch
	...
	function _openTrading() internal {
		maxTxAmount     = 100 * _totalSupply / 1000 + 10**_decimals;  //TODO set final limits
		maxWalletAmount = 100 * _totalSupply / 1000 + 10**_decimals;  //TODO set final limits
		buyTaxRate = 3;
		sellTaxRate = 3;
		tradingOpen = true;
	}
	...
	function _checkLimits(address sender, address recipient, uint256 transferAmount) internal view returns (bool) {
		...
		if ( tradingOpen && !excludedFromLimits[sender] && !excludedFromLimits[recipient] ) {
	...
	function excludeFromAntiBot(address wallet, bool excluded) external onlyOwner {
		if (!excluded) { require(wallet != address(this) && wallet != owner, "This address must be excluded" ); }
		antiBotExcluded[wallet] = excluded;
	}

	function disableFees(address wallet, bool toggle) external onlyOwner {
		if (!toggle) { require(wallet != address(this) && wallet != owner, "Cannot enforce fees for this address"); }
		excludedFromFees[ wallet ] = toggle;
	}

	function disableLimits(address wallet, bool toggle) external onlyOwner {
		if (!toggle) { require(wallet != address(this) && wallet != owner, "Cannot enforce limits for this address"); }
