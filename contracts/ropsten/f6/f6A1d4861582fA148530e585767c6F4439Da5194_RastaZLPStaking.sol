...
contract RastaZLPStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    modifier updateReward(bool withdrawal) {
        ...
        bool penaltyFlag = block.timestamp.sub(user.depositedAt) < rewardCycle && withdrawal ? true : false;
        uint256 pending = penaltyFlag ? 0 : user.amount.mul(accPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function stakeAll() external {  
        ...
        for (uint i = 0; i < balance; i++) {
            uint256 _tokenId = parentNFT.tokenOfOwnerByIndex(msg.sender, i);
            _stake(_tokenId);
        }
    }

    function stake(uint256 _tokenId) external {
        require(parentNFT.ownerOf(_tokenId) == msg.sender, "only owner can stake!");
        _stake(_tokenId);
    }

    function substake(uint256[] memory _tokenIds) external {
        for (uint i = 0; i < _tokenIds.length; i++) {
            if(parentNFT.ownerOf(_tokenIds[i]) == msg.sender){
                _stake(_tokenIds[i]);
            }
        }
    }

    function _stake(uint256 _tokenId) internal nonReentrant whenNotPaused updateReward(false) updateUserList {
        require (block.timestamp < endTime, "expired");
        
        UserInfo storage user = userInfo[msg.sender];
        uint256 amount = parentNFT.price(_tokenId);
    ...
    function isExistTokenId(uint256 _tokenId) internal view returns (bool) {
        UserInfo storage user = userInfo[msg.sender];
        uint256[] memory _tokenIds = user.tokenIds;
        
        for (uint i = 0; i < _tokenIds.length; i++) {
            if(_tokenIds[i] == _tokenId) return true;
        }
        return false;
    }
    ...
    function subunstake(uint256[] memory _tokenIds) external {
        for (uint i = 0; i < _tokenIds.length; i++) {
            if(isExistTokenId(_tokenIds[i]) == true){
                _unstake(_tokenIds[i]);
            }
        }
    }
    ...
    function unstake(uint256 _tokenId) external {
        require(isExistTokenId(_tokenId) == true, "only owner can unstake!");
        _unstake(_tokenId);
    }

    function _unstake(uint256 _tokenId) internal nonReentrant updateReward(true) updateUserList {
        UserInfo storage user = userInfo[msg.sender];

        require(block.timestamp >= user.depositedAt + lockupDuration, "You cannot withdraw yet!");
        uint256 amount = parentNFT.price(_tokenId);
