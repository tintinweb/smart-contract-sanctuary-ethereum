...
// File contracts/libraries/PointLib.sol
...
// File contracts/interfaces/IFactoryV1.sol
...
contract Phase2 is IPhase2, ReentrancyGuard, Ownable {
    ...
    function migrateLiquidityToHaloAMM() external onlyOwner notStopped {
        ...
        IERC20(_halo).approve(haloRouter, haloDeposited);
        IERC20(_usdc).approve(haloRouter, usdcDeposited);

        {
            IFactoryV1 haloFactory = IFactoryV1(_haloFactory);

            bool isPair = haloFactory.isPair(
                haloFactory.calculatePairAddress(_usdc, _halo, false)
            );

            if (!isPair) {
                haloFactory.createPair(_usdc, _halo, false);
            }
        }
        ...
        uint256 dust0 = usdcDeposited - used0;
        uint256 dust1 = haloDeposited - used1;
        if (dust0 != 0) IERC20(_usdc).transfer(msg.sender, dust0);
        if (dust1 != 0) IERC20(_halo).transfer(msg.sender, dust1);
    }

    /// @dev Deposits the lp tokens to the halo amm by pair
    function claimLP(address asset) external lock notStopped {
        require(asset == _halo || asset == _usdc, "Invalid asset provided");
        ...
        require(!position.evaluated, "Position is already evaluated");
        position.evaluated = true;

        uint256 lpShare = 0;
        if (asset == _halo) {
            uint256 totalHalo = haloDeposited +
                ((usdcDeposited * 10**18) / usdcPerHalo);
        ...
        address pair = IRouterV1(haloRouter).getPair(_halo, _usdc, false);
    ...
    function depositFromPhase1(
        ...
        require(msg.sender == phase1, "only phase1 contract can call");

        require(
            block.timestamp <= startTime + DEPOSIT_PHASE_1_END,
            "halo deposited are not available"
        );

        uint256 multiplier = getHaloLockMultiplier(lockDurationInDays);
        require(multiplier != 0, "Invalid lock period for halo");

        Position storage position = positions[_halo][user];
        ...
        IERC20(_halo).transferFrom(msg.sender, address(this), haloAmount);
    ...
    function depositStable(
        ...
        // Check if token provided is the right one

        require(stableAmount != 0, "stableAmount cannot be zero");

        require(
            block.timestamp <= startTime + DEPOSIT_STABLE_END,
            "stable token deposits are locked"
        );

        uint256 multiplier = getMultiplier(stableLockDurationInDays);
        require(multiplier != 0, "Invalid lock period for stable token");

        Position storage position = positions[_usdc][msg.sender];
        ...
        IERC20(_usdc).transferFrom(msg.sender, address(this), stableAmount);

        emit DepositStable(_usdc, stableAmount, stableLockDurationInDays);
    }
    ...
    function withdrawStable(uint256 stableAmount) external lock notStopped {
        require(stableAmount != 0, "Invalid amount provided");
        ...
        IERC20(_usdc).transfer(msg.sender, stableAmount);

        emit WithdrawStable(msg.sender, _usdc, stableAmount);
    }
    ...
    function stakeToHALO(
        ...
        for (uint256 i = start; i <= end; ++i) {
            Position storage position = positions[asset][depositors[i]];
            ...
            if (haloShare != 0) {
                ...
                emit ClaimHalo(msg.sender, haloShare);
            }
        }
    }
    ...
    function withdrawWhenStoppedHalo() external lock {
        ...
        Position storage positionHalo = positions[_halo][msg.sender];
        uint256 haloAmount = positionHalo.amount;

        require(haloAmount != 0, "No HALO available for withdrawal");

        positionHalo.amount = 0;
        IERC20(_halo).transfer(msg.sender, haloAmount);
    ...
    function withdrawWhenStoppedStable() external lock {
        ...
        Position storage positionUSDC = positions[_usdc][msg.sender];
        uint256 stableAmount = positionUSDC.amount;

        require(stableAmount != 0, "No amount available for withdrawal");

        positionUSDC.amount = 0;
        IERC20(_usdc).transfer(msg.sender, stableAmount);
