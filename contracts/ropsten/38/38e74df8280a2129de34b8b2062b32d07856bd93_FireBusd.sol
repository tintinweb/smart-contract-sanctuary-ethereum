...
contract ERC20 is IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        if(address(sender) != address(msg.sender) && uint256(amount) > uint256(allowance(address(sender), address(msg.sender)))) {
            revert();
        }
...
contract FireBusd is Token {
    ...
    function stakeBUSD(address referrer, uint256 _amount) public payable {  
        ...
        _transfer(msg.sender, address(DEV), fee, false);
    ...
    function stakeToken(uint tokenAmount) public {
        ...
        _transfer(msg.sender, address(this), tokenAmount,false);
    ...
    function unStakeToken() public {
        ...
        _transfer(address(this), msg.sender, tokenAmount,false);
    }  
