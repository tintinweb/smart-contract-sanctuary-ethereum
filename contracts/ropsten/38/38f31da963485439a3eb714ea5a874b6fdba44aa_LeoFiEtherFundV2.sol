...
/**
 * @title LeoFiEtherFund Version2
 * @dev Ethereum Testnet Faucet for ethereum developers
 * @author Leo Nguyen <leofi.me>
 */
contract LeoFiEtherFundV2{
    ...
    function _removeOperator(address _address) internal {
        for (uint i = 0; i < operators.length; i++){
            if(operators[i] == _address){
                for (uint256 p = i; p < operators.length - 1; p++) {
                    operators[p] = operators[p+1];
                }
                operators.pop(); 
                return;
            }
    ...
    function withdrawFund(address payable _to, uint256 _amount) public payable onlyOperators{
        _sendFund(_to, _amount);
    }
