...
// https://forum.openzeppelin.com/t/error-contract-is-not-upgrade-safe-use-of-delegatecall-is-not-allowed/16859/4
...
contract SquidSwapExecutable is IAxelarForecallable, UUPSUpgradeable, OwnableUpgradeable {
    error InvalidTrade();
    ...
    function initialize(address _gateway, address _gasReceiver)
        public 
        initializer
    {
        __IAxelarForecallable_init(_gateway);
        __Ownable_init();
        ...
        returns (bool success)
    {
        ...
        IERC20Upgradeable(tokenIn).approve(router, receiveAmount);
        (success, ) = router.call(data);
    }
    ...
    function tradeSend(
        ...
        IERC20Upgradeable(tokenAddress).approve(address(gateway), swapAmount);
        gateway.sendToken(
            destinationChain,
            destinationAddress,
            symbol,
    ...
    function _giveToken(
        ...
        address token = gateway.tokenAddresses(symbol);
        SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(token), destination, amount);
    }
...
abstract contract IAxelarForecallable is Initializable {
    ...
    function __IAxelarForecallable_init(address gatewayAddress) internal initializer {
