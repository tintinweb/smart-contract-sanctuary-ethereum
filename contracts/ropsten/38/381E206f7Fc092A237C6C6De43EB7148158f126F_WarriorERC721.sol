...
abstract contract WeedWarsERC721 is ERC721Burnable, Ownable {
    ...
    uint256 public mergeBaseFee = 1000;
    uint256 public mergeMultiplierFee = 500;
    ...
    mapping(uint256 => string) public tokenIdToNftName;
    ...
    function setAuthorizedToEquip(address _address, bool _isAuthorized) external onlyOwner {
    ...
    function setMergeBaseAndMultiplierFee(
        uint256 _baseFee,
        uint256 _multiplierFee
    ) external onlyOwnerOrAdmin {
        mergeBaseFee = _baseFee;
        mergeMultiplierFee = _multiplierFee;
    }
    ...
    function mintReserve(address _address, uint256 _claimQty) external onlyOwner {
        require(
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal view override {
        require(
            ...
            "WW: can't transfer or burn if locked"
        );
    }
    ...
    function merge(
        ...
        IWeedERC20(weedContractAddress).mint(
            msg.sender,
            mergeBaseFee +
                ((w1.mergeCount + w2.mergeCount) * mergeMultiplierFee)
        );
    ...
    function changeName(uint256 _tokenId, string calldata _name) external onlyTokenOwner(_tokenId) {
        require(locked[_tokenId] == false, "WW: token is locked");
        require(!namesTaken[_name], "HOWL: this name has been taken");

        // if warriors was renamed - unreserve the previous name
        string memory previousName = tokenIdToNftName[_tokenId];
        if (bytes(previousName).length > 0) {
            namesTaken[previousName] = false;
        }

        tokenIdToNftName[_tokenId] = _name;
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        if (
            ...
            !_isDnaRevealed(tokenIdToNft[_tokenId])
        ) {
            return preRevealUrl;
        }
    ...
    function get(uint256 _tokenId) public view returns (WeedWarsNft memory) {
        ...
        w.headGear = getProperty(dna, 1);
        w.clothes = getProperty(dna, 2);
        w.shoulderGuard = getProperty(dna, 3);
        w.armGuards = getProperty(dna, 4);
        w.sideWeapon = getProperty(dna, 5);
        w.backWeapon = getProperty(dna, 6);
