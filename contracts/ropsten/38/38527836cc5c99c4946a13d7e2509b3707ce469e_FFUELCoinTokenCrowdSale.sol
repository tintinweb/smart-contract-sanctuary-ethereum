...
contract FFUELCoinTokenCrowdSale is FinalizableCrowdsale {
    ...
    string public constant version = "v1.6";
...
//    uint256 public reservedAmount;
    ...
    Crowdsale(_startTime, _endTime, _rate, _wallet) public
    {
        ...
        preMint(_vested,_vestedAmount); // make the actual tests failed so can not use it now, need e2e test to be finished first
    }
    ...
    */
    function computeTokenWithBonus(uint256 weiAmount) internal returns(uint256) {
        uint256 tokens_ = 0;
    ...
    **/
    ...
    function buyTokens(address beneficiary) public payable {
        ...
        require(tokenRateGwei!=0);

        // validate KYC here
        // if not part of kyc then throw
        bool cleared;
        uint8 contributor_get;
        address ref;
        uint8 affiliate_get;
        ...
        uint256 contributorGet = 0;

        // compute the amount of bonus, from the cotnribution amount
        ...
        if (refCleared && ref != beneficiary)
        {
            ...
            uint256 affiliateGet = tokens.mul(affiliate_get).div(100);
    ...
    */
    ...
    function preMint(address vestedAddress,uint256 _amount) onlyOwner public{
        runPreMint(vestedAddress,_amount);
//
        runPreMint(0x28587B8dA098f872c3aB74ae5a784E3a509B9682,37500000000000000000000000);  // preSale
        runPreMint(0x4E04B124CECCCb8F540917fE6ff8e674828F9d14,50000000000000000000000000);  // reserved
        runPreMint(0x9c8b1e3648A49950216F895A3878daabB6f2D7E5,5000000000000000000000000);   // bounty
        runPreMint(0x12768c326d3EcBec4b254292Ab9C876E4Ff2e071,5000000000000000000000000);   // team
        runPreMint(0xe7025581964BD3dB95f8A0c52054797F60e516fB,5000000000000000000000000);   // advisors
    ...
    function runPreMint(address _target,uint256 _amount) onlyOwner public{
        if (preSaleMode)
        {
    ...
    function modifyTransferableHash(address _spender,bool value) onlyOwner public
    {
        //address FFUELCoinToken
        _token.modifyTransferableHash(_spender,value);
    }

     // add a way to change the whitelistadmin user
    function setAdmin(address _adminAddress, bool whiteListAdmin) onlyOwner public {
        if (whiteListAdmin)
        {
    ...
    */
    ...
    function setContributor(address _address, bool cleared, uint8 contributor_get, uint8 affiliate_get, address ref) public{

        require(msg.sender == whiteListingAdmin);
        // not possible to give an exorbitant bonus
        require(contributor_get<100);
        require(affiliate_get<100);
    ...
    function getContributor(address _address) view public returns (bool, uint8, address, uint8 ) {
