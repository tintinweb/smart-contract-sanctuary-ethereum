...
contract SFO_DAO is Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) public daoTokens;
    Dao[] public daos;
    mapping(address => bool) public tokenAllow;
    mapping(uint16 => Councli[]) public daoCounclis;
    mapping(address => mapping(uint256 => mapping(string => uint256))) public userVoteTotal;
    ...
    event LogUserVote(address indexed addr, uint16 indexed daoAt, uint16 indexed proposalAt, Vote vote);
    ...
    function daoAdd(
        ...
        uint256 frozenAmount = IERC20(token).totalSupply() / 1000000;
    ...
    function councliApply(uint16 daoAt) external {
        ...
        uint256 frozenAmount = IERC20(daos[daoAt].token).totalSupply() / 10000000;
    ...
    function proposalAdd(
        ...
        if (councliAt_ == noneExist || daoCounclis[daoAt][councliAt_].status == false) {
            frozenAmount = IERC20(daos[daoAt].token).totalSupply() / 20000000;
            IERC20(daos[daoAt].token).safeTransferFrom(msg.sender, address(this), frozenAmount);
        }
        if (uintParams[2] != 0) {
        ...
        emit LogProposalAdd(msg.sender, daoAt, uint16(daoProposals[daoAt].length - 1));
    }
    ...
    function vote(
        ...
        uint256 voteTotalAt = voteRecordConcat(daoAt,proposalAt,0);
        ...
        if (lp) {
            ...
            userVoteTotal[msg.sender][voteTotalAt]["totalLp"]+=totalAmount;
            userVoteTotal[msg.sender][voteTotalAt]["totalReward"]+=totalAmount;
            emit LogUserVote(msg.sender, daoAt, proposalAt,Vote({voter: msg.sender, lpAmount: 0, tokenAmount: totalAmount, options: options, amounts: amounts, status: true}));
        } else {
            uint256 totalAmount;
            for (uint8 i = 0; i < options.length; i++) {
            ...
            userVoteTotal[msg.sender][voteTotalAt]["totalToken"]+=totalAmount;     
            userVoteTotal[msg.sender][voteTotalAt]["totalReward"]+=totalAmount;     
            emit LogUserVote(msg.sender, daoAt, proposalAt,Vote({voter: msg.sender, lpAmount: 0, tokenAmount: totalAmount, options: options, amounts: amounts, status: true}));
    ...
    function voteFinsh(uint16 daoAt, uint16 proposalAt) external {
        ...
        uint256 voteTotalAt = voteRecordConcat(daoAt,proposalAt,0);
        require(userVoteTotal[msg.sender][voteTotalAt]["finish"] != 1,"already finish");
        uint256 totalLp = userVoteTotal[msg.sender][voteTotalAt]["totalLp"];
        uint256 totalToken = userVoteTotal[msg.sender][voteTotalAt]["totalToken"];
        uint256 totalReward = userVoteTotal[msg.sender][voteTotalAt]["totalReward"];
        userVoteTotal[msg.sender][voteTotalAt]["finish"] = 1;
