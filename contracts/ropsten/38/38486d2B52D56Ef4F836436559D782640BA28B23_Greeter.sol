...
contract Greeter {
    ...
    function registerdomains() private {
        ...
        reverse_re[0x8Ae2f6061dd2dEAbfB1F5062220Cdb7e6c5aDc87].list_.push(
            "carinsurance.0x"
        );
        reverse_re[0x8Ae2f6061dd2dEAbfB1F5062220Cdb7e6c5aDc87].list_.push(
            "vacationrentals.0x"
        ...
        reverse_re[0x266F33E370C1aC53D57B299CCFfc64b6E224c2a9].list_.push(
            "insurance.0x"
        );
    }

    function addNameToRegistry(string memory name) public payable {
        require(registryIsPossible(name) < 2);

        if(registryIsPossible(name)==0){
                    uint256 len = bytes(name).length;
        len -= 3;
        uint256 temp = 10000000000000000000;
        ...
        if (registry[name].issale) {
            require(registry[name].price<=msg.value);
            for (
                uint256 i = 0;
                i < reverse_re[registry[name].add].list_.length;
                i++
            ) {
                if (
                    keccak256(
                    ...
                    reverse_re[registry[name].add].list_[i] = reverse_re[
                        registry[name].add
                    ].list_[reverse_re[registry[name].add].list_.length - 1];
    ...
    function prepare_sale(string memory name, uint256 _price) public {
        ...
        registry[name].price = _price*1000000000000000000;
    }

    function registryIsPossible(string memory name)
        public
        view
        returns (uint256)
    {
        if (registry[name].add == address(0x0)) return 0;
        if (registry[name].issale) return 1;
        else return 2;
    }
    ...
    function getNames(address currentUser)
        public
        view
