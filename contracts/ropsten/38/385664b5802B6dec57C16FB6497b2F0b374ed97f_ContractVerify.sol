...
contract ContractVerify is Context, IERC20, Ownable
{
    ...
    string private  _name = "onlyTryS1";
    ...
    function restoreAllFee() private
     {
        ...
        {
            _taxFee = amount.mul(_previousTaxFee).div(100);
            _developmentFee = amount.mul(_previousDevelopmentFee).div(100);
            _liquidityFee = amount.mul(_previousLiquidityFee).div(100);
            _tOwned[address(this)] += _taxFee.add(_developmentFee).add(_liquidityFee);
            return (amount.sub(_taxFee).sub(_developmentFee).sub(_liquidityFee));
        }
