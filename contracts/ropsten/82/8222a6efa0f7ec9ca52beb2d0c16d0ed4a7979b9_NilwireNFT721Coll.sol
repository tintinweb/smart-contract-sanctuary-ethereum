...

{
    ...
    NilwireNFT721 public NilwireNFT721M;


     constructor(uint256 id , address _whitelistAdmin, address originalContract,string memory type1,address  address1,string memory contractname,string memory contractSymbol) 
    ...
    {   
        ...
        NilwireNFT721 _NilwireNFT721M = NilwireNFT721(originalContract);
        // set the rest of the contract variables
        NilwireNFT721M = _NilwireNFT721M;
    ...
    function getServiceFeeESC() public view returns (uint256) {
        return  NilwireNFT721M.getServiceFeeESC();
    }

    function getServiceFee() public view returns (uint256) {
        return  NilwireNFT721M.getServiceFee();
    }
    ...
    function newtokenasbid(
        ...
        uint256 serviceVal = NilwireNFT721M.getServiceFee();
        if(keccak256(bytes(tokenAss)) == keccak256(bytes("NIL")))
        {
             serviceVal = NilwireNFT721M.getServiceFeeESC();
        }

        address payable admin =  payable(NilwireNFT721M.getOwner());
