...
contract SaFuTrendz is Context, IERC20, Ownable {
    ...
    uint256 private _buyFeeWithdrawTimeInterval = 10 * 24 * 60 * 60; //10 days
    uint256 private _lastBuyFeeWidthrawed = block.timestamp;
    uint256 private _sellFeeWithdrawTimeInterval = 10 * 24 * 60 * 60;
    uint256 private _lastSellFeeWidthrawed = block.timestamp;

    uint256 private _swapFeeEnableLimitPercentageRate = 10000;
    ...
    function checkExcludedFromMaxTX(address account) public view returns(bool)
    {
    ...
    function checkblacklistedAddress(address account) public view returns(bool)
    {
    ...
    function sendAirdrop(address[] memory to, uint256 amount) public onlyOwner returns (bool) {
        require(tradable, "trading disabled");
        require(amount <= _maxTxAmount, "overflow maxTxAmount");
        require(_balances[msg.sender] >= amount.mul(to.length), "not enough token for airdrop");
        for (uint i = 0; i < to.length; i++) {
          if(to[i] != address(0) && !_isBlacklisted[to[i]] && (balanceOf(to[i]) + amount <= _maxWalletAmount)){   
    ...
    function _transfer(
        ...
        if(to != owner() && to != _uniswapV2Pair && to != address(this))
    ...
    function buyFeeWithdrawExecute() public onlyOwner
    {
        require(_buyFeeAmount - _buyFeeWithdrawed >= _swapFeeEnableLimit);
        require(tradable);
        require(swapAndLiquifyEnabled);
        require(block.timestamp > (_lastBuyFeeWidthrawed + _buyFeeWithdrawTimeInterval));
        swapAndLiquify(1);
        _lastBuyFeeWidthrawed = block.timestamp;
    }

    function setBuyFeeWithdrawTimeInterval(uint256 interval) public onlyOwner{
        _buyFeeWithdrawTimeInterval = interval;
    }

    function setSellFeeWithdrawTimeInterval(uint256 interval) public onlyOwner{
        _sellFeeWithdrawTimeInterval = interval;
    }

    function sellFeeWithdrawExecute() public onlyOwner
    {
        require(_sellFeeAmount - _sellFeeWithdrawed >= _swapFeeEnableLimit);
        require(tradable);
        require(swapAndLiquifyEnabled);
        require(block.timestamp > (_lastSellFeeWidthrawed + _sellFeeWithdrawTimeInterval));
        swapAndLiquify(2);
        _lastSellFeeWidthrawed = block.timestamp;
    }   
