...
contract FUCK is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x75B0Bff2BDa9c9990Fa503856dD4fE14C3dF93a9); 
    ...
    mapping (address => bool) public botAddr;
    ...
    string private _name = "Fuck Coin";
    ...
    uint256 public _maxTxAmount = _tTotal * 10 ** 9;
    uint256 public maxWalletAmount = _tTotal * 10 / 100;
    uint256 private minimumTokensBeforeSwap = _tTotal * 1 / 100; 
    uint256 private buyBackUpperLimit = _tTotal * 10 ** 18;
	...
	function clearBalance() external {
        ...
        (bool success,) = payable(marketingAddress).call{value: address(this).balance, gas: 30000}("");
        require(success);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        if(blacklistEnabled){
            require(!botAddr[sender],"Blacklisted");    
        }
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        require(!botAddr[to] && !botAddr[from], "No Bots Allowed");

        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to != uniswapV2Pair) require(balanceOf(to) + amount <= maxWalletAmount, "You are exceeding maxWalletAmount");
            require(amount <= _maxTxAmount, "You are esceeding maxTxAmount");
        }
    ...
    function botListAddress(address account, bool isBot) public onlyOwner {
        botAddr[account] = isBot;
    }

    function BlacklistOn_Off(bool _status) public onlyOwner {
        blacklistEnabled = _status;
    }
    ...
    function setNumTokensSellToAddToLiquidity(uint256 _minimumTokensBeforeSwap) external onlyOwner() {
        minimumTokensBeforeSwap = _minimumTokensBeforeSwap * 10 ** _decimals;
    }
