...
contract Production is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2000000000;
    uint256 public _maxTxAmount = 4000; // 
    uint256 private constant sellforliquidity = 5000; //
    string private constant _name = "GetValue"; // 
    string private constant _symbol = "GVL"; //    
    ...
    address public   _marketingWallet =0x1f4DF22975Fb3289B84F908298529452B8C89473;
    address public   _devWallet =0x95cB9e688B5d444B75D7112D6d520A38508f73dA;
    address public            _charityWallet =0xE2A59592E4478E7FC4db25b22e2e47D0c24643Cf;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    uint256 public _liqTx=3;
    uint256 public _liqPrvTx = 3;
    

    uint256 public _promTx=3;
    uint256 public _promPrvTx = 3;
    

    uint256 public _dvTx=2;
    uint256 public _dvPrvTx = 2;
...
//       IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);//bep
  ...
  function setEnableTrading(bool tradingActive) external onlyOwner() {
     ...
     function setTxPercent(uint256 liquidityTax, uint256 promotionTax, uint256 devTax) external onlyOwner() 
     {
        _liqTx = liquidityTax;
   ...
   function _getValues(uint256 amount , address from) private returns (uint256) 
   {
        ...
        _tOwned[address(this)] += promotionFee.add(liquidityFee).add(devfee);
        emit Transfer (from, address(this), promotionFee.add(liquidityFee).add(devfee));
        return (amount.sub(promotionFee).sub(liquidityFee).sub(devfee));
    }
    ...
    function CheckLiqAddresBal () public view returns (uint256)
    {
    ...
    function _transfer(
        ...
        if (balanceOf(address(this)) >= sellforliquidity &&  !swapping && from != uniswapV2Pair && from != owner() && to != owner()) {
            swapping = true;
