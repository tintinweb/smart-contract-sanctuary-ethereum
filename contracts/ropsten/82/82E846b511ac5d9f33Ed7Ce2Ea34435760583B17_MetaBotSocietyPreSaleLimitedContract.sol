...
contract MetaBotSocietyPreSaleLimitedContract is Ownable {
     ...
     * @dev MINT DATA 
     */
    uint256 public startTimePhaseOne = 1646006400; // 28 Feb 2022 00:00 UTC
    uint256 public startTimePhaseTwo = 1646265600; // 3 Mar 2022 00:00 UTC
    uint256 public startTimeOpen = 1646611200; // 7 Mar 2022 00:00 UTC
    ...
    uint256 public maxSupplyOpen = 9999 - maxSupplyPhaseOne - maxSupplyPhaseTwo;
    ...
    event setMaxSupplyOpenEvent(uint256 indexed supply);
    ...
    function buyPhases(uint256 amount, uint256 buyMaxAmount, uint256 phase, bytes32[] calldata proof) 
        ...
        require(address(nft) != address(0), "JUNIOR NFT SMART CONTRACT NOT SET");
    ...
    function nftsLeft() external view returns(uint256) {
        return maxSupplyPhaseOne - mintedPhases[1] + maxSupplyPhaseTwo - mintedPhases[2] + maxSupplyOpen - mintedOpen;
    }
     ...
     * @param newStartTime. The new start time.
     */
    function setStartTimePhaseTwoList(uint256 newStartTime) external onlyOwner {
    ...
    /**
     * @notice Change the maximum supply of the public sale.
     *
     * @param supply. The new max supply.
     */
    function setMaxSupplyOpen(uint256 supply) external onlyOwner {
        maxSupplyOpen = supply;
        emit setMaxSupplyOpenEvent(supply);
    }
    ...
    function withdrawAll() external {
        ...
        for (uint i = 0; i < recipients.length; i++) {
            ...
            uint256 _amount = contractBalance * 1000 / shares[i];
