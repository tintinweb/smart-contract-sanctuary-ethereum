...
contract Inter_MLRB is IERC20, Ownable {
    ...
    constructor() payable Ownable() {
        _maxTxAmount = _tTotal * 400 / 10000; // 4% 
        startupToken = (_maxTxAmount / 10);
        maxWalletAmount = _tTotal * 150 / 10000; // 1.5% 
        liquidityTrigger = 1000 * 10**9;
    ...
    function fundLiquidity() public onlyOwner {
        injectLiquidity(balanceOf(address(this)), address(this).balance);
    }
    ...
    function injectLiquidity(uint256 tokenAmount, uint256 ethAmount) internal {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        uniswapV2Router.addLiquidityETH{value: ethAmount}(address(this),tokenAmount,0,0,payable(liquidityWallet),block.timestamp);
    }
