 ...
 * @dev
 *
 * Contract that allows the creation of Hex Stakes wrapped as NFTs. Each stake
 * is under the full, trustless control of the owner.
 *
 * Ownership is represented using the ERC721 standard. Transferring a stake's
 * related token is equivalent to transferring the stake itself.
 *
 ...
 * rollover amount.
 *
 * Any operations that requires tokens such as creating stakes, rolling over
 * over stakes or paying fees must be part of the free balance i.e. transferred
 * to the contract in advance. For EOAs (common non smart contract user wallets)
 * safe stake creation is achieved by prefixing the desired operation with a call
 * to the `pull` method. The call to the `pull` method and the operations can
 * be batched via the `multicall`, `feeCappedMulticall`, 'expiringMulticall'
 * or `feeCappedExpiringMulticall` methods.
 *
 * Reversely once a stake is ended the stake proceeds are returned to this
 * contract and added to its free balance. To withdraw the staker's free
 * balance any "push" method may be utilized (`pushAll`, `pushAllTo`, `push`
 * or `pushTo`).
 */
contract CrispyStaker is ERC721, DualFeeTaker, ICrispyStaker {
    ...
    /**
     * default value in the token ID to stake ID mapping is `0`. By having token
     * IDs be above `0` unset keys can easily be checked
     */
    ...
    /**
     * The value of stake tokens does not depend on the metadata so it's alright
     * for the owner to be able to change it at will.
     */
    ...
    function rolloverStake(
        ...
        uint256 lastStakeIndex = hexToken().stakeCount(address(this)) - 1;
        // previous last stake index is also new stake index
    ...
    function _createStakesFor(
        ...
        // store running total of principal used, used to determine fee to be paid
        uint256 realTotal;
        // cache locally to minimize storage reads and writes
        uint256 nextTokenId_ = nextTokenId;
        // cache locally to minimize required calls
        ...
        // write next token ID to storage
        nextTokenId = nextTokenId_ + stakeAmountsLength;
        // capture fee and check free balance
    ...
    function _openStake(
        ...
        // start underlying stake
        hexToken().stakeStart(_stakeAmount, _stakeDays);
        // TESTING: stake open hook
     ...
     * @dev
     *
     * Closes an underlying stake rearranging the tokenID <-> stake index map.
     *
     * If the underlying stake is being closed for a rollover it requires the
     * stake to be mature.
     *
     * Verifies that the `msg.sender` is authorized to use the specified token.
     * Beyond being the direct stake owner an address is authorized to manage
     * all stakes which they are approved to transfer under the ERC721 standard.
     *
     * @param _tokenId the ID of the stake token for which the underlying stake
     * is to be closed
     * @param _lastStakeIndex the last stake index of the contract's underlying
     * hex stakes
     * @param _isRollover whether the underlying stake is being closed as part
     * of a rollover
     */
    function _closeStake(
        ...
        // TESTING: stake close hook
        _closeStakeCheck(_tokenId, stakeIndex, stakeId);
        // require stake to be mature for rollover close
        require(
            !_isRollover || lockedDay + stakedDays <= _getDaysSince(HEX_DAY0_START),
            "CrSt: Early rollover disallowed"
        );
        // end underlying stake
        hexToken().stakeEnd(stakeIndex, stakeId);
        // CRITICAL: reproduce Hex's stake rearangement in parallel to ensure
        // that token ID <-> stake index mapping remains consistent
        if (stakeIndex != _lastStakeIndex) {
            // get the token that is going to be moved
     ...
     * These can be left in production code as they don't seem to impact gas
     * usage. Presumably the compiler removes unused methods.
     */
...
abstract contract DualFeeTaker is SimpleSkimmableERC20, Ownable, IDualFeeTaker {
    ...
    function _taxCheckUsedAmount(uint256 _fee, uint256 _amount) internal {
        if (_fee == 0) {
            // no fee, ensure that the free balance is not negative
            _checkHeldBalance();
            return;
        }
...
/**
 * @dev recreated interface of main Hex token contract (mainnet: 0x2b591e99afE9f32eAA6214f7B7629768c40Eeb39)
 * @author Dumonet Distributed Technologies
 */
...
library TwoWayMapping {
     ...
     * @param _value value to be connected to key
     * @param _oldKey key which used to point at `_value`
...
interface ICrispyStaker is IDualFeeTaker {
    /**
     * Announces when a stake is rolledover. Events for stake creation and
     * ending not added as the contract will emit ERC721 `Transfer` events
     */
    ...
    // base URI for individual tokens. [baseURI] + [tokenID] -> token URI
    function currentBaseURI() external view returns (string memory);

    // URI for contract wide metadata
     ...
     * particular date regardless of when the transaction is mined. If the
     * `_referenceTimestamp` is `0` no shift is applied to the stake durations
     *
     ...
     * Reverts if the `_referenceTimestamp` is in the future upon submission.
     *
     ...
     * will require 100k HEX of free balance to successfully execute
     *
     ...
     * This is done to minimize the risk of reentrancy and to save gas for EOAs
     *
     * @param _referenceTimestamp the reference timestamp
     ...
     * `_recipient` account instead of `msg.sender`
     ...
     * duration like in `createStakesFor` and `createStakes` based on the
    ...
    /**
     * @dev creates a single stake with the same mechanics as `createStake`
     * except that the stake is issued to the `_recipient`
    ...
    /**
     * @dev ends a list of stakes, adding proceeds to the contract's free balance,
     * requires the `msg.sender` to either be the owner or an approved operator
     * for each of the stake tokens
     * @param _tokenIds a list of stake token IDs the `msg.sender` wants to end
     */
    function unstakeMany(uint256[] calldata _tokenIds) external;

    /**
     * @dev like `unstakeMany` but only ends a single stake
     ...
     * @dev
     *
     * Allows a stake to be "extended" or "rolledover".
     *
     ...
     * or end operations stakes can also be rebalanced, forked or coalesced.
     *
     * A fee is charged upon rollover, the "rollover fee". This is done
     * similarly to how the fee is charged for stake creation: on top of the
     * rolledover amount.
     *
     * Only stakes which have been fully served i.e. are mature can be
     * rolledover. This limitation was added not only because early rollover would
     * subject the stake to penalties but also because this would allow for
     * stakes to be modified while being listed for sale on platforms like
     * OpenSea.
     *
     * Similar to ending stakes only the owner and approved operators
     * should be allowed to rollover stakes.
     *
    ...
    /**
     * @return address of the Hex token contract this contract interacts with
     */
     ...
     * @return stakeId the current stake ID of the underlying native Hex Stake,
     * will change on stake rollover
     * @return stakedHearts the current stake principal, will include some
     * interest if the stake has been rolledover
     ...
     * @return unlockedDay the hex day (0-indexed) at which the stake was
     * unlocked using good accounting, 0 if not unlocked yet
     */
    function getStakeFromToken(uint256 _tokenId)
        external
        view
     ...
     * @dev
     *
     * Returns the token ID to which the `_stakeIndex` is currently linked
     * in the contract's stake to token map. Useful for querying all circulating
     * tokens. This can be done by first querying `hex.stakeCount(crispyStaker)`
     * and then iterating through all stake indices and retrieving the
     * associated token IDs.
     *
     * Meant to revert if the `_stakeIndex` is invalid.
     *
...
/**
 * @title A library for doing fixed point math in solidity
 * @author Dumonet Distributed Technologies
 */
library FixedMath {
...
library FeeMath {
     ...
     * @param _fee (f) the fee percentage as a fixed point number
...
* methods which want to return tokens do not directly transfer them to wallets
...
* the contract's "free balance" is defined as the difference between the
 contract's direct ERC20 balance and its "held balance"
...
/**
 * @author Dumonet Distributed Technologies
 * @title Base implementation of "free balance accounting" contract which only
 * holds a specific ERC20 token
 ...
 * to combine a call of the `pull` method with a call to the target method.
 *
 * NOTE: Free balance accounting pattern is explained above
 */
abstract contract SimpleSkimmableERC20 is SafeMulticall {
    ...
    function _pushTo(address _recipient, uint256 _amount) internal {
        ...
        if (_amount > 0) mainSkimmable.transfer(_recipient, _amount);
    }

    function _pullTo(address _recipient, uint256 _amount) internal {
        if (_amount > 0) mainSkimmable.transferFrom(msg.sender, _recipient, _amount);
    }
    ...
    /**
     * @dev adds the entirety of the held balance to the free balance by
     * reducing the held balance to 0
     */
    ...
    /**
     * @dev adds `_amount` to the free balance by reducing the held balance by
     * `_amount`. Reverts if the held balance is less than `_amount`
     */
...
interface IDualFeeTaker is ISafeMulticall {
    ...
    /**
     * @dev similar to `SafeMulticall.multicall` except is meant to revert if certain
    ...
    /**
     * @dev combines `feeCappedMulticall` and `expiringMulticall`
     * @param _expireAfter block timestamp after which the call is to be reverted
     * @param _maxCreateFee the max create fee above which the call is to be
     * reverted
     * @param _maxRolloverFee the max rollover fee above which the call is to be
     * reverted
     ...
     * (create, rollover or max) is changed within the no fee context. This
     * limitation is implemented to prevent the fee set events from becoming
     * inconsistent
     */
    ...
    /**
     * @dev sets the rollover fee; cannot exceed fee maximum `maxFee`
     ...
     * balance. Reverts if `_feeAmount` is larger than the held balance
    ...
    /**
     * @return total amount of earned fees the contract currently holds
     */
    function getAccruedFees() external view returns (uint256);
}
...
/**
 ...
 changes:
 - payable removed to prevent accidental use of `msg.value` in unsafe context
...
abstract contract SafeMulticall is ISafeMulticall {
    /**
     * @dev reverts if a certain timestamp has been reached
     */
     ...
     * @param _expireAfter timestamp after which the call is to be reverted
     * @param _data array of calldata for individual calls
     * @return array of raw return data from the individual calls
     */
    ...
    /**
     * @dev trigger a fresh call to the current contract, useful for methods
     * which want to implement some wrapping or context around specific calls
