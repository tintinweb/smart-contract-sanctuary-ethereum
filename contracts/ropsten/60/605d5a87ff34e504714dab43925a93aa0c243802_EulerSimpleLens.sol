...
contract EulerSimpleLens is Constants {
    ...
    // balances and debts
    function getAccountBalancesAndDebts(address underlying, address account) external view returns (uint eTokenBalance, uint dTokenBalance, uint pTokenBalance) {
        (address eTokenAddr, address dTokenAddr, address pTokenAddr) = underlyingToInternalTokens(underlying);
        if (account != address(0)) {
            eTokenBalance = EToken(eTokenAddr).balanceOfUnderlying(account);
            dTokenBalance = IERC20(dTokenAddr).balanceOf(account);
            pTokenBalance = IERC20(pTokenAddr).balanceOf(account);
        } else {
            eTokenBalance = EToken(eTokenAddr).balanceOfUnderlying(msg.sender);
            dTokenBalance = IERC20(dTokenAddr).balanceOf(msg.sender);
            pTokenBalance = IERC20(pTokenAddr).balanceOf(msg.sender);
        }
    }

    // approvals
    function getEulerAccountAllowance(address asset, address account) external view returns (uint256) {
        if (account != address(0)) {
            return IERC20(asset).allowance(account, address(euler));
        } else {
            return IERC20(asset).allowance(msg.sender, address(euler));
        }
    }
