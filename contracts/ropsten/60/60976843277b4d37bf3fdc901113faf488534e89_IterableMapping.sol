...
contract NodeEmpireDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("NodeEmpire_Dividend_Tracker", "NodeEmpire_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "NodeEmpire_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "NodeEmpire_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NodeEmpire contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "NodeEmpire_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "NodeEmpire_Dividend_Tracker: Cannot update claimWait to same value");
...
contract NodeEmpire is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    NodeEmpireDividendTracker public dividendTracker;
    ...
    uint256 public onSelltreasuryFee;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _treasuryFee) public onlyOwner {
        ETHRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        treasuryFee = _treasuryFee;

        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee).add(treasuryFee); // total fee transfer and buy
    }

    function setSellFee(uint256 _onSellbnbRewardFee, uint256 _onSellliuidityFee, uint256 _onSellMarketingFee, uint256 _onSelltreasuryFee) public onlyOwner {
        onSelltotalFees = _onSellbnbRewardFee;
        onSellliquidityFee = _onSellliuidityFee;
        onSellmarketingFee = _onSellMarketingFee;
        onSelltreasuryFee = _onSelltreasuryFee;

        onSelltotalFees = onSellETHRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee).add(onSelltreasuryFee);
    }
    ...
    function settreasuryWallet(address payable _newBuybakWallet) public onlyOwner {
        treasuryWallet = _newBuybakWallet;
    }

/**
Token NAme:Node Empire
Symbol:NE
Total Supply:100.000.000

Taxes and distributions:

Tax 6% to treasury (ETH) - (0x0DdcE65cEC4A6690518baD38683AC4b80790278b)
4% development and marketing (ETH) - (0x5543C25ac1413Eab6e0046542d00ce2a8E13E182)
2% reflections to holders (NE TOKEN)
2% Liquidity pool

Max Transaction limit 1% of total supply
*/
    constructor() ERC20("Node Empire", "NE") {
        ETHRewardsFee = 2;
        liquidityFee = 2;
        marketingFee = 4;
        treasuryFee = 6;

        onSellETHRewardsFee = 2;
        onSellliquidityFee = 2;
        onSellmarketingFee = 4;
        onSelltreasuryFee = 6;

        marketingWallet = payable(0x5543C25ac1413Eab6e0046542d00ce2a8E13E182); // 
        treasuryWallet = payable(0x0DdcE65cEC4A6690518baD38683AC4b80790278b); // 

        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee).add(treasuryFee); // total fee transfer and buy

        onSelltotalFees = onSellETHRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee).add(onSelltreasuryFee);

    	dividendTracker = new NodeEmpireDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "NodeEmpire: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "NodeEmpire: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "NodeEmpire: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NodeEmpire: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "NodeEmpire: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NodeEmpire: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 buybackAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(marketingFee)).mul(treasuryFee);
        treasuryWallet.transfer(buybackAmount);
    ...
    function swapAndSendBNBToBuybak(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, treasuryWallet);
    }
