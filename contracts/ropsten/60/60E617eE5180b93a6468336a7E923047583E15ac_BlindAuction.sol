    ...
    OwnableUpgradeable
{
    ...
    address[] public usersEligibleForAirdrop;
    mapping(address => bool) public isUserEligibleForAirdrop;
    uint256 public refundedUserCount;
    mapping(address => bool) public isUserRefunded;

    uint256 public nonWinnersCount;

    event UserDeposited(address indexed user);
    event UserEligibleForAirdrop(address indexed user);
    ...
    function winnerDepositExtra(bytes calldata signature) external payable {
        ...
        require(
            !isUserEligibleForAirdrop[msg.sender],
            "Already deposited extra"
        );
        require(!isUserRefunded[msg.sender], "Already refunded");
        checkValidity(signature, "winner");
        isUserEligibleForAirdrop[msg.sender] = true;
        usersEligibleForAirdrop.push(msg.sender);
        emit UserEligibleForAirdrop(msg.sender);
    }
    ...
    function refund(bytes calldata signature) external nonReentrant {
        ...
        require(!isUserEligibleForAirdrop[msg.sender], "Is a winner");
        require(!isUserRefunded[msg.sender], "Already refunded");
    ...
    // depositedUserCount - winners count [usually 333]
    function setNonWinnersCount(uint256 _nonWinnersCount) external onlyOwner {
        nonWinnersCount = _nonWinnersCount;
    }

    // 0 = not started
    // 1 = started, bidding
    // 2 = final price announced | winners should pay finalPrice - 0.5e | non-winners can refund
    // 3 = auction end | kill unpaid winner deposits | non-winners can still refund
    function setAuctionState(uint8 state) external onlyOwner {
        auctionState = state;
    }

    function getUsersEligibleForAirdrop()
        external
        view
        returns (address[] memory)
    {
        return usersEligibleForAirdrop;
    }
}
