 ...
 * contract.
 */
abstract contract ReflectionToken is
    Initializable,
    IERC20,
    OwnableUpgradeable,
    Presaleable,
    ...
    Pausable
{
    ...
    function burn(uint256 amount) external {
        ...
        require(
            balance >= amount,
            "ReflectionToken: burn amount exceeds balance"
        );
    ...
    function includeInAntiDumping(address _address, bool value)
        external
        onlyOwner
    {
    ...
    function isIncludedInAntiDumping(address _address)
        public
        view
        returns (bool)
    {
    ...
    function setIsUnlimitedSenderOrRecipient(address account, bool value)
        external
        onlyOwner
    {
        require(
            isInPresale,
            "ReflectionToken: Address can be initialized only during the pre-sale."
        );
        _isUnlimitedSenderOrRecipient[account] = value;
        _isExcludedFromFee[account];
    }

    function isUnlimitedSenderOrRecipient(address account)
        internal
        view
        returns (bool)
    {
    ...
    function _transfer(
        ...
        if (isInPresale) {
            ...
            if (
                ...
                !isUnlimitedSenderOrRecipient(sender) &&
                !isUnlimitedSenderOrRecipient(recipient)
            ) {
                revert(
                    "ReflectionToken: Transfer amount exceeds the maxTxAmount as per anti-whale protocol"
                );
            }
        }
        ...
        if (sumOfFees > 0 && !isInPresale) {
            if (sumOfFees == (getFeeTotal().add(antiDumpingFees))) {
                _takeTransactionFees(
                    amount.mul(sumOfFees).div(FEES_DIVISOR).mul(10),
                    currentRate
                );
            } else {
