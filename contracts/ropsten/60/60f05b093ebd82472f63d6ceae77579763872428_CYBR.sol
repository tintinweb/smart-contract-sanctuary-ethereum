...
contract CYBR is Context, IERC20, IERC20Metadata, Ownable {
    ...
    // Flags
    bool public allowedTrading;
    bool public defenseSmartContractSystem;
    bool public defenseBotSystem;

    uint256 public antiBotTime;
    uint256 public robinHoodProtectionTime = 604800;            // 7 days
    ...
    // Fees
    uint256 private taxPercent = 2;                             // 2%
    bool    private taxStatus;
    ...
    constructor(
        bool _allowedTrading,
        bool _defenseSmartContractSystem,
        bool _defenseBotSystem,
        bool _taxStatus,
        uint256 _antiBotTime,
        address _userDonationWallet,
        address _botDonationWallet) {

        _balances[msg.sender] = _totalSupply;

        allowedTrading = _allowedTrading;
        defenseSmartContractSystem = _defenseSmartContractSystem;
        defenseBotSystem = _defenseBotSystem;
        taxStatus = _taxStatus;
        antiBotTime = _antiBotTime;
        userDonationWallet = _userDonationWallet;
        botDonationWallet = _botDonationWallet;
    ...
    function cyberNationDonors() external view returns (address[] memory) {
        return donors;
    }

    function cyberNationBurners() external view returns (address[] memory) {
        return burners;
    }
    ...
    function donate(uint256 amount) external {
        tokensReceivedFromCommunity = tokensReceivedFromCommunity + amount;
        if (userDonation[msg.sender] == 0) {
            donors.push(msg.sender);
        }
        userDonation[msg.sender] = userDonation[msg.sender] + amount;
        _transfer_(_msgSender(), userDonationWallet, amount);
        emit Donated(msg.sender, amount);
    }

    function burn(uint256 amount) public {
        userBurned[msg.sender] = userBurned[msg.sender] + amount;
        if (userBurned[msg.sender] == 0) {
            burners.push(msg.sender);
        }
        _transfer_(_msgSender(), burnWallet, amount);
    ...
    function addAddressToPool(address _address) external onlyOwner {
        require(!poolAddress[_address], "Already added in pool.");
    ...
    function removeAddressFromPool(address _address) external onlyOwner {
        require(poolAddress[_address], "No exists in pool.");
    ...
    function _addAddressToBlacklist(address _address) private {
        if(!protectedAddress[_address] && _address != userDonationWallet && _address != botDonationWallet && blacklisted[_address] != true) {
    ...
    function punishBot(address botAddress, uint256 amount) external onlyOwner {
        ...
        _transfer_(botAddress, botDonationWallet, amount);
    ...
    function punishSmartContract(address contractAddress, uint256 amount) external onlyOwner {
        ...
        tokensReceivedFromBots = tokensReceivedFromBots.add(amount);
        _transfer_(contractAddress, botDonationWallet, amount);
        ...
        for (uint i = 0; i < _blacklist.length; i++) {
            ...
            if ((block.timestamp - blacklistedAt[blacklistAddress]) > robinHoodProtectionTime) {
                ...
                if (tokenAmount > 10**18) {
                    ...
                    _transfer_(blacklistAddress, botDonationWallet, tokenAmount);
    ...
    function _transfer(
        ...
        if (allowedTrading) {
            ...
            else if (defenseBotSystem && (block.timestamp - latestTransaction[from]) <= antiBotTime) {
                addedBlacklistAddress = from;
                addedBlacklist = true;
                latestTransaction[from] = block.timestamp;
        ...
        if (to == userDonationWallet) {
            if (userDonation[from] == 0) {
                donors.push(from);
            }
            userDonation[from] = userDonation[from].add(amount);
            tokensReceivedFromCommunity = tokensReceivedFromCommunity.add(amount);
            emit Donated(from, amount);
        }

        if (to == botDonationWallet) {
            tokensReceivedFromBots = tokensReceivedFromBots.add(amount);
        }

        if (to == burnWallet) {
            if (userBurned[from] == 0) {
                burners.push(from);
            }
