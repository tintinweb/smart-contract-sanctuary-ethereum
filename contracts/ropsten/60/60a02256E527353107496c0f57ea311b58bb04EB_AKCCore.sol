...
contract AKCCore is Ownable, AccessControl {
     ...
     * @dev TRACKING DATA 
     */
    mapping(address => Tribe[]) public userToTribes;
    mapping(address => uint256) public userToEarnings;
    TribeSpec[] public tribeSpecs;

    uint256 public tribeCount = 0;
    uint256 public allTimeEarnings = 0;

    /**
     * @dev CREATION LOGIC
     */
    uint256 maxBatchTribes = 50;
    uint256 maxIdentifierLength = 10;
     ...
     * @dev EVENTS
     */
    event TribeCreated(address indexed owner, uint256 indexed tribeSpec, string indexed tribeName);
    event ClaimedReward(address indexed owner, uint256 indexed tribeIndex, uint256 indexed reward);
    ...
    event SetMaxIdentifierLengthEvent(uint256 indexed newLength);
    event SetClaimDelayEvent(uint256 indexed delay);
    ...
    function createSingleTribe(address newOwner, uint256 spec, string calldata identifier) 
        ...
        onlyRole(CREATOR_ROLE) {
            ...
            require(bytes(identifier).length < maxIdentifierLength, "IDENTIFIER EXCEEDS LENGTH");

            Tribe memory newTribe = Tribe({
            ...
            userToTribes[newOwner].push(newTribe);
            tribeCount++;

            emit TribeCreated(newOwner, spec, identifier);
    }

    function createManyTribes(address[] calldata newOwners, uint256[] calldata specs, string[] calldata identifiers)
        external 
        onlyRole(CREATOR_ROLE) {
            require(newOwners.length == specs.length, "NEWOWNERS MUST MATCH SPEC");
            require(newOwners.length == identifiers.length, "NEWOWNERS MUST MATCH IDENTIFIERS");
    ...
    function claimRewardOfTribeByIndex(address tribeOwner, uint256 tribeIndex) 
        ...
        returns(uint256) {
            ...
            Tribe storage tribe = userToTribes[tribeOwner][tribeIndex];
            TribeSpec memory tribeSpec = tribeSpecs[tribe.spec];

            uint256 rewardInterval = block.timestamp - tribe.lastClaimedTimeStamp;
            require(rewardInterval > claimDelay, "CLAIM TOO FAST");

            tribe.lastClaimedTimeStamp = block.timestamp;
            uint256 reward = tribeSpec.rps * rewardInterval;

            allTimeEarnings += reward;
            userToEarnings[tribeOwner] += reward;

            emit ClaimedReward(tribeOwner, tribeIndex, reward);
    ...
    function getTribeAmountBySpec(address tribeOwner, uint256 spec) 
        ...
        returns(uint256) {
            ...
            uint256 counter = 0;
            for (uint i = 0; i < userToTribes[tribeOwner].length; i++) {
                Tribe memory tribeInstance = userToTribes[tribeOwner][i];
                if (tribeInstance.spec == spec) {
                    counter++;
                }
            }
    ...
    function getTribeRewardByIndex(address tribeOwner, uint256 tribeIndex)
        ...
        returns (uint256) {
            require(userToTribes[tribeOwner].length > tribeIndex, "TRIBE INDEX OUT OF BOUNDS");
            Tribe memory tribe = userToTribes[tribeOwner][tribeIndex];
            TribeSpec memory tribeSpec = tribeSpecs[tribe.spec];
    ...
    function updateTribeSpec(uint256 index, uint256 newPrice, uint256 newRps)
        ...
        onlyRole(MODIFIER_ROLE) {
            ...
            tribeSpec.price = newPrice * (10**18);
            tribeSpec.rps = newRps * (10**18);
    ...
    function suspendTribesOfUser(address tribeOwner) 
        ...
        onlyRole(MODIFIER_ROLE) {
            ...
            tribeCount -= userToTribes[tribeOwner].length;
    ...
    function setMaxIdentifierLength(uint256 newLength)
        external 
        onlyRole(MODIFIER_ROLE) {
            maxIdentifierLength = newLength;

            emit SetMaxIdentifierLengthEvent(newLength);
        }
    
    function setClaimDelay(uint256 newDelay)
        external 
        onlyRole(MODIFIER_ROLE) {
            claimDelay = newDelay;

            emit SetClaimDelayEvent(newDelay);
        }
}
