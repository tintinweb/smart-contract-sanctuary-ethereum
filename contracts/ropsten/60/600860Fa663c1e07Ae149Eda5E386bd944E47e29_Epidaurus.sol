...
abstract contract Ownable is Context {
 ...
 /** @title Utillity functions for IFO contract */
...
/** @title Epidaurus Main Contract */
contract Epidaurus is ERC20, protectedOfferings {
    uint256 transferFee = 88; 
    uint256 charityFee = 42; 
    uint256 burnFee = 19; 
    uint256 rebaseFee = 19; 
    uint256 airDropsAndGas = 8;
    uint256 PUBLIC_LAUNCH = 1663700400;
    address charityWallet;
    mapping(address => bool) trustedRecipients;
    mapping(address => bool) trustedSenders;

    constructor(address _charityWallet) ERC20("Epidaurus", "EPDR") {
        charityWallet = _charityWallet;
    }

    // @notice Mint as many tokens as an owner can
    // @dev No cap on the tokens.
    // @params Amount of tokens to be minted followed by decimal.
    ...
    // @notice Set the charityWallet. 
    // @params _address the address to be set.
    ...
    // @notice Add the address to the trusted sender mapping.
    // @dev Only these addresses will be able to send tokens before time.
    // these Addresses won't be accounted to fee while transferring tokens.
    ...
    // @notice Add the address to trusted recipients.
    ...
    // @notice Removes the address from trusted senders.
    ...
    // @notice Removes the address from trusted recipients.
    ...
    /// @notice The owner of the smart contract can set transfer fees.
    // @dev This fee will be cut whenever the user is transferring their tokens.
    ...
    // @notice The owner of the smart contract can set charity fees.
    // @dev This fee will be sent to charity address whenever the user is transferring their tokens.
    ...
    // @notice The owner of the smart contract can set rebase fees.
    // @dev This fee will be sent to smart contract address whenever the user is transferring their tokens.
    ...
    // @notice The owner of the smart contract can set air drops and gas fees.
    // @dev This fee will be sent to smart contract address whenever the user is transferring their tokens.
    function setAirDropsAndGas(
        uint256 newAirDropsAndGas) 
        public 
        // onlyOwner 
        {
        airDropsAndGas = newAirDropsAndGas;
    }
    // @notice This function can only be executed by the owner of this smart contract.
    // Upon calling this function the amount given will be transferred to the specified receiver.
    function airDropCrypto(
        address recipient,
        uint256 _amount)
        public
        // onlyOwner
    ...
    // @notice Some preconditions that are to be met before transferring the tokens.
    // Normal users can't send their tokens before the launch date of the token.
    //  Only trusted addresses are enabled to send their tokens. 
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 amount
    ...
    function _afterTokenTransfer(
        ...
        if (!trustedSenders[msg.sender]){
            ...
            uint256 othersAmount = (amount * airDropsAndGas) / 10000;
            address receiver = to;
