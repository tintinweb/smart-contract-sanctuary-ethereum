...
/**
 *Metaelitehungergames vasting code
*/
...
/**
 * MehgTeam tokens vesting contract. 
 *
 * According to the MehgTeam token distribution structure, there are two parties that should
 ...
 *
 * The MehgTeam "Vesting" smart contract should be in place to ensure meeting the token sale commitments.
 *
 ...
 */
contract MehgTokensVesting {

    using SafeMath for uint256;

    /**
     * Address of mehgToken.
     */
    ERC20TokenInterface public mehgToken;
     ...
     *
     * @param token Address of MehgToken that will be locked on contract.
    ...
    function getAvailableTokensToWithdraw () public view returns (uint256 tokensToSend) {
        ...
        if (tokensUnlockedPercentage >= 100) {
            tokensToSend = mehgToken.balanceOf(this);
        } else {
    ...
    function initVestingStages () internal {
        ...
        stages[1].tokensUnlockedPercentage = 5;
        stages[2].tokensUnlockedPercentage = 10;
        stages[3].tokensUnlockedPercentage = 15;
    ...
    function setInitialTokensBalance () private {
        initialTokensBalance = mehgToken.balanceOf(this);
    }
    ...
    function sendTokens (uint256 tokensToSend) private {
        if (tokensToSend > 0) {
            ...
            mehgToken.transfer(withdrawAddress, tokensToSend);
            // Raising event
...
contract TeamAndEarlyInvestorsVesting is MehgTokensVesting {
    constructor(ERC20TokenInterface token, address withdraw) MehgTokensVesting(token, withdraw) public {}
}