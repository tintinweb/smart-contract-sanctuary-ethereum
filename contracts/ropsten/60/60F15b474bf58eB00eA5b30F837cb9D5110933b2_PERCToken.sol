...
///////////////////
    ...
    event TemporaryTransfer(bytes32 _hash, address _from, address _to, uint256 _amount);
    ...
    function doTransfer(address _from, address _to, uint _amount) internal returns(bool) {
        ...
        if (!PaymentProxyInterface(payment_proxy).tx_lock()){
            emit Transfer(_from, _to, _amount);
        }
        else{
            emit TemporaryTransfer(PaymentProxyInterface(payment_proxy).currentTransferRequestHash(), _from, _to, _amount);
        }
...
////////////////
    ...
    function _generateTokens(address _owner, uint _amount) internal returns (bool) {
        ...
        if (!PaymentProxyInterface(payment_proxy).tx_lock()){
            ...
            emit TemporaryTransfer(PaymentProxyInterface(payment_proxy).currentTransferRequestHash(), address(0), _owner, _amount);
        }
    ...
    function _destroyTokens(address _owner, uint _amount) internal returns (bool) {
        ...
        if (!PaymentProxyInterface(payment_proxy).tx_lock()){
            ...
            emit TemporaryTransfer(PaymentProxyInterface(payment_proxy).currentTransferRequestHash(), _owner, address(0), _amount);
        }    
...
contract PERCToken is PERCBase, Ownable, TrustListTools, TokenClaimer{
  ...
  constructor(
    ...
    returns (bool) {
      PaymentProxyInterface(payment_proxy).transferRequest(address(this), _owner, address(0x1), _amount);
