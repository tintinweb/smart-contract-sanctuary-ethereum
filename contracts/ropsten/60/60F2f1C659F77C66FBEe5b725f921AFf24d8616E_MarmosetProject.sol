...
contract MarmosetProject is Ownable {
    ...
    mapping(address => uint) private availableToWithdraw; //address of user -> total won amount;
    ...
    function getWinner(uint[6] memory winNumber) external {
        ...
        address[] memory match2;
        address[] memory match3;
        address[] memory match4;
        address[] memory match5;
        address[] memory match6;
        uint prizeMatch2;
        uint prizeMatch3;
        uint prizeMatch4;
        uint prizeMatch5;
        ...
        for (uint i = 0; i < players[currentLotteryNumber].length; i++) {
            ...
            if (matchNumber == 2) {
                match2[i] = players[currentLotteryNumber][i];
            } else if (matchNumber == 3) {
                match3[i] = players[currentLotteryNumber][i];
            } else if (matchNumber == 4) {
                match4[i] = players[currentLotteryNumber][i];
            } else if (matchNumber == 5) {
                match5[i] = players[currentLotteryNumber][i];
            } else if (matchNumber == 6) {
                match6[i] = players[currentLotteryNumber][i];
            }
        }
    ...
    function calculateMatch(uint[6] memory winNumber, address playerAddr) internal view returns(uint){
        for (uint i = 0; i < winNumber.length; i++) {
    ...
    function getPrice(uint price) public view returns(uint) {
        ...
        path[1] = wbnb;
        path[2] = mmtToken;
        uint[] memory response = router.getAmountsOut(price, path);
        return response[2];
    }
