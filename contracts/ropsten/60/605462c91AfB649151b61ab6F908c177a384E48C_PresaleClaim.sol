...
contract PresaleClaim is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor() {
        ...
        withdrawAddressSigner = owner();
        emit WithdrawAddressSigner(withdrawAddressSigner);

        deployedOnBlock = block.number;
    }
        ...
        view
    {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                DOMAIN_SEPARATOR,
                keccak256(
                    abi.encode(PRESALE_TYPEHASH, _msgSender(), _totalClaim)
                )
            )
        );
        ...
        require(
            recoveredAddress != address(0) &&
                recoveredAddress == address(withdrawAddressSigner),
            "Invalid signature"
        );
    }
