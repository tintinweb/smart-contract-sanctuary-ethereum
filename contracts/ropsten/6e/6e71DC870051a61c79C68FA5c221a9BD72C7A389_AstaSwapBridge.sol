...
contract AstaSwapBridge is Context {
    ...
    /**
     * Returns set minimum swap fee in STRM from BEP20 to other chains
     */
    function setSwapFeePercentageOfSTRM(uint256 _feePerAsta)
        external
        onlyOwner
    {
        require(
    ...
    function swapToken(
        ...
        if (feePercentageInAsta > 0) {
            feeAmountInStrm = amount.mul(feePercentageInAsta);
