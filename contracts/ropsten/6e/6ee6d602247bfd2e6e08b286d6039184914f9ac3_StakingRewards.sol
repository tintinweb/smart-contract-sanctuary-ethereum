...
import "./interfaces/IDreadfulz3D.sol";
...
contract StakingRewards is Initializable, AccessControlEnumerableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, IERC721Receiver {
    using SafeERC20Upgradeable for ERC20PresetMinterPauserUpgradeable;

    ERC20PresetMinterPauserUpgradeable public rewardsToken;
    IERC721EnumerableUpgradeable public stakingToken;
    IDreadfulz3D public staking3DToken;
    ...
    function initialize(
        ...
        rewardsToken = ERC20PresetMinterPauserUpgradeable(_rewardsToken);
    ...
    */
    ...
    function swap(uint256[] memory tokenIds) external nonReentrant whenNotPaused {
        uint256 requiredBurn;
        for(uint256 i = 0; i < tokenIds.length; i++) {
            uint256 current = tokenIds[i];
            require(nfts[current] == msg.sender, "Not authorised");
            IERC721EnumerableUpgradeable stakingTokenAddress = staking3DToken.correctToken(current);

            if(!staking3DToken.hasPaid(current)) {
                requiredBurn += staking3DToken.cost();
            }
            staking3DToken.setPaid(current, true);
            stakingTokenAddress.safeTransferFrom(address(this), address(staking3DToken), current);
        }
        if(requiredBurn > 0) rewardsToken.burnFrom(msg.sender, requiredBurn);
    }
    ...
    function stake(uint256[] memory ids) external nonReentrant whenNotPaused updateReward(msg.sender) {
        ...
        for(uint256 i = 0; i < amount; i++) {
            ...
            require(stakingTokenAddress.ownerOf(current) == msg.sender, "not authorised");
            nfts[current] = msg.sender;
            stakingTokenAddress.transferFrom(msg.sender, address(this), current);
    ...
    function unstake(uint256[] memory ids) external nonReentrant whenNotPaused updateReward(msg.sender) {
        ...
        for(uint256 i = 0; i < amount; i++) {
            ...
            require(nfts[current] == msg.sender, "not authorised");
            delete nfts[current];
            stakingTokenAddress.transferFrom(address(this), msg.sender, current);
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ERC20PresetMinterPauserUpgradeable(tokenAddress).safeTransfer(msg.sender, tokenAmount);
    ...
    function setRewardsToken(ERC20PresetMinterPauserUpgradeable _token) public onlyRole(DEFAULT_ADMIN_ROLE) {
        rewardsToken = _token;
    }
    ...
    function setStaking3DToken(IDreadfulz3D _token) public onlyRole(DEFAULT_ADMIN_ROLE) {
        staking3DToken = _token;
    }
