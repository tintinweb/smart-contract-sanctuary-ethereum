...
contract Pollen {
    ...
    function getBalanceUnder(address sender, uint256 tokenIndex) public returns(uint256) {

        uint tokens =  cToken.balanceOfUnderlying(sender);
        rewards[tokenIndex].underTokens = tokens;

        return tokens;

    }

    function getUnderReturn(uint256 tokenIndex) public view returns(uint256) {

             StakedToken memory staked = rewards[tokenIndex]; 

            uint256 _result = staked.underTokens - staked.amount;
    ...
    function unstake(address _owner, uint256 tokenIndex) public {
      ...
      //  getBalanceUnder(address(this), tokenIndex);       


        uint256 pollenProfit = getUnderReturn(tokenIndex);  
