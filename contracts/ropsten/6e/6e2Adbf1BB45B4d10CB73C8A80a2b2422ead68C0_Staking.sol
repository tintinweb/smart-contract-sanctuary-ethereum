...
contract Staking is IStaking, AccessControl, ReentrancyGuard {
    ...
    function stake(uint256 amount) external override {
        ...
        require(pair.balanceOf(msg.sender) >= amount, "Unsufficient balance");
        require(pair.allowance(msg.sender, address(this)) >= amount, "Unsufficient allowance");
    ...
    function claim() external override nonReentrant {
        // 1. check if block.timestamp > Stake.timestamp + rewardTime
        // solhint-disable-next-line not-rely-on-time
        require(_stakes[msg.sender].timestamp + rewardTime <= block.timestamp, "Rewards are not available");
        
        // 2. Calculate reward amount in token A
        ...
        // 3. mint those tokens to msg.sender
        // TODO: mint instead of transfer, MINTER role in ERC20 is needed
        // ERC20(pair.token0()).mint(msg.sender, rewardInToken0 * rewardPeriods); 
    ...
    function unstake() external override nonReentrant {
        // 1. check if block.timestamp > Stake.timestamp + freezeTime
        // solhint-disable-next-line not-rely-on-time
        require(_stakes[msg.sender].timestamp + freezeTime <= block.timestamp, "Unstake is not available");
        // 3. transfers Stake.amount to msg.sender
    ...
    function _getRewardInTokens(address owner) internal view returns(uint256) {
        ...
        // 2. return total reward in token0
