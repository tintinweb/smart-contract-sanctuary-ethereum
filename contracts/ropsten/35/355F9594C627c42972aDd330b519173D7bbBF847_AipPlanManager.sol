    ...
    IAipExtendCallback
{
        ...
        returns (Plan memory plan, PlanTime memory time)
    {
        ...
        (
            ...
            time.endTick,
            time.endedTime,
            time.lastTriggerTick,
            time.lastTriggerTime
        ) = pool.getPlanTime(planIndex);
    }
...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH =
        0x142b2fb9db45d229e20acc60d97a3bba2bd0087d2de6dff4dc054fbd2f26d9c7;
...
contract AipPool is IAipPool, ReentrancyGuard {
        ...
        )
    {
        ...
        if (endTick > lastTriggerTick) {
            endedTime = lastTriggerTime + period * (endTick - lastTriggerTick);
        } else {
