...
string private _tokenBaseURI = "https://ipfs.io/ipfs/QmTaH2TPvGUVaxFsZh72Ri8iknpycrRriufFhKYyAeDdqP?filename=bug.jpg";
    ...
    function setReceivers(address[] calldata _addresses) external onlyOwner {
       for (uint x = 0; x < _addresses.length; x++) {
          receivers.push(_addresses[x]);
        }
   }

    function removeRecievers() public onlyOwner {
...
//    function _setBaseURI(uint tokenId, string memory _tokenBaseURI) internal virtual {
//           _tokenBaseURI[tokenId] = _tokenBaseURI;
//    }
    

    function Graduation() external onlyOwner {
