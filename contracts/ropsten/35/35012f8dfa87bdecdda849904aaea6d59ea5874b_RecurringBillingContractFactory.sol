 ...
 *   5.3. Ensure that passed `bytes` parameter is exactly 32 bytes in length (two bytes16 aka uint256 values).
 */
contract TokenRecurringBilling {
    ...
    function charge (uint256 billingId) public {
        ...
        if (now > lastChargeAt.add(period.mul(2))) { // If there are 2 periods left, no further charges are possible
            cancelRecurringBillingInternal(billingId);
            return;
        }
    ...
    function allowRecurringBillingInternal (
        ...
        require(billingRegistry[billingId].customer == 0x0, "Recurring billing with this ID is already registered");
