    ...
    PausableUpgradeable
{
    ...
    uint256 public voteFeeRatioBase;
    address public voteFeeTo;

    mapping(uint256 => Order) public orderIDList;

    uint256 public orderExpireTime; // Exipre after 15 days
    ...
    function createOrder(
        ...
        Order memory order = Order({
            ...
            expiresAt: block.timestamp + 365 * 1 days,
            votes: OrderVotes(0, 0, 0, 0)
        });
        ...
        override
    {
        ...
        if (_toStatus == OrderStatus.Submitted) {
            ...
            order.expiresAt = block.timestamp + orderExpireTime * 1 days;
            order.status = OrderStatus.Submitted;
    ...
    function vote(uint256 _orderID, VoteTarget voteTarget) external {
        ...
        require(
            order.votes.buyer <= voteLimit / 2 &&
                order.votes.seller <= voteLimit / 2,
            "Vote limit reached."
        );

        DarenMedal dm = DarenMedal(darenMedal);
        uint256 availablePrice = dm.getStakingSubcategory(msg.sender, 1001); // 1001 is judge
        ERC20 u = ERC20(usdtToken);
        require(
            availablePrice == 999999 ||
                availablePrice * 10**u.decimals() >= order.value,
            "You have no permission to vote."
        );
    ...
    function withdrawOrder(uint256 _orderID) external override {
        ...
        uint256 fee = order.value.mul(feeRatio).div(feeRatioBase);
        uint256 voteFee = order.value.mul(voteFeeRatio).div(voteFeeRatioBase);
        ...
        if (order.status == OrderStatus.Completed) {
            ...
            if (voteFee < 10**u.decimals() * 3) {
                voteFee = 10**u.decimals() * 3;
            }
    ...
    function setFeeRatio(uint256 _feeRatio, uint256 _feeRatioBase)
        ...
        onlyRole(ADMIN_ROLE)
    {
        require(_feeRatioBase > 0, "Fee ratio base must be greater than 0.");
        feeRatio = _feeRatio;
        feeRatioBase = _feeRatioBase;
    }

    function setVoteFeeRatio(uint256 _voteFeeRatio, uint256 _voteFeeRatioBase)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(
            _voteFeeRatioBase > 0,
            "Vote fee ratio base must be greater than 0."
        );
        voteFeeRatio = _voteFeeRatio;
        voteFeeRatioBase = _voteFeeRatioBase;
    }

    function setFeeTo(address payable _feeTo) external onlyRole(ADMIN_ROLE) {
        require(_feeTo != address(0), "Address should not be zero");
        feeTo = _feeTo;
    }
    ...
    PausableUpgradeable
{
    ...
    uint256 public allocRatio; // 1 / 2
    uint256 public allocRatioBase;

    uint256 public usdtExchangeRate; // 1:10
    uint256 public usdtExchangeRateBase;

    mapping(address => uint256) public userCompleteBonus;
    mapping(address => uint256) public userVotingBonus;
    ...
    function initialize(address _darenToken) public initializer {
        ...
        // USDT exchange rate -> 1 USDT : 10 DT
        usdtExchangeRate = 10**2;
        usdtExchangeRateBase = 10**1;
    ...
    function completeOrder(
        ...
        userCompleteBonus[_buyer] = userCompleteBonus[_buyer].add(finalAmount);
        userCompleteBonus[_seller] = userCompleteBonus[_seller].add(
            finalAmount
        );
    ...
    function completeOrderByVoting(
        ...
        if (voterCount > 0) {
            ...
            for (uint256 i = 0; i < voters.length; i++) {
                userVotingBonus[voters[i]] = userVotingBonus[voters[i]].add(
                    finalAmount
                );
            }
        }
    ...
    function getRewardOf(address _account)
        ...
        returns (uint256)
    {
        return userCompleteBonus[_account];
    }

    function getVotingRewardOf(address _account)
        external
        view
        returns (uint256)
    {
        return userVotingBonus[_account];
    }

    function withdrawReward() external override {
        uint256 bonus = userCompleteBonus[msg.sender];
        ...
        userCompleteBonus[msg.sender] = 0;
        dt.transfer(msg.sender, reward);

        emit RewardWithdrawn(msg.sender, reward);
    }

    function withdrawVotingReward() external override {
        uint256 bonus = userVotingBonus[msg.sender];
        ERC20 dt = ERC20(darenToken);
        require(
            dt.balanceOf(address(this)) > bonus,
            "Withdraw is unavailable now"
        ...
        uint256 reward = bonus;
        require(reward > 0, "You have no bonus.");
        userVotingBonus[msg.sender] = 0;
        dt.transfer(msg.sender, reward);

        emit VotingRewardWithdrawn(msg.sender, reward);
    }

    function setAllocRatio(uint256 _allocRatio, uint256 _allocRatioBase)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(
            _allocRatioBase > 0 && _allocRatio < _allocRatioBase,
            "Invalid allocRatioBase."
        );
        allocRatio = _allocRatio;
        allocRatioBase = _allocRatioBase;
    }

    function setUsdtExchangeRate(uint256 rate, uint256 rateBase)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(rate > 0, "Rate must be greater than 0.");
        require(rateBase > 0, "Rate base must be greater than 0.");
        usdtExchangeRate = rate;
        usdtExchangeRateBase = rateBase;
    }
...
interface IDarenOrder {
    enum OrderStatus {
        ...
        Completed, // 3
        Withdrawn, // 4
        AgreeToRefund, // 5 seller want to cancel the order
        Refunded, // 6
        Voting // 7
    }
    ...
    struct Order {
        ...
        // unique order ID
        uint256 orderID;
        // Value of this order in USD in wei
        uint256 value;
        address seller;
        address buyer;
        OrderStatus status;
    ...
    event OrderRefunded(uint256 pk, uint256 orderID, string status);
}
...
interface IDarenBonus {
    ...
    event VotingRewardWithdrawn(address user, uint256 amount);

    function withdrawReward() external;

    function withdrawVotingReward() external;
}
