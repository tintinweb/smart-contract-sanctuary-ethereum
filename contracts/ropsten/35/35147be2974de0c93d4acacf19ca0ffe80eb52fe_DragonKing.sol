...
contract DragonKingTest is DragonKing {
	...
	function DragonKingTest(address tpt, address ndc, address skl, address xper) DragonKing(tpt, ndc, skl, xper, 0, 10, 5, getCosts(), getBalloonCosts()) public {
	...
	function testGetRandomDragon(uint256 nonce) public view returns(uint16) {
		return getRandomDragon(nonce);
	}

	function random(uint256 nonce, uint16 n) public view returns(uint) {
		return uint(keccak256(block.blockhash(block.number - 1), now, n, nonce));
	}

	function dice(uint256 nonce, uint16 n) public view returns(uint) {
		return uint16(random(nonce, n) % 100);
  }
}