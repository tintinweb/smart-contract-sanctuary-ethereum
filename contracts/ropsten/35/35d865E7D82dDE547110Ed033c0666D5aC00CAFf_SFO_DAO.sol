...
contract SFO_DAO is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function daoAdd(
        ...
        uint256 frozenAmount = IERC20Upgradeable(token).totalSupply() / 1000000;
        IERC20Upgradeable(token).safeTransferFrom(msg.sender, address(this), frozenAmount);
    ...
    function councliApply(uint16 daoAt) external {
        ...
        uint256 frozenAmount = IERC20Upgradeable(daos[daoAt].token).totalSupply() / 1000000;
        IERC20Upgradeable(daos[daoAt].token).safeTransferFrom(msg.sender, address(this), frozenAmount);
        if (councliAt_ == noneExist) {
    ...
    function councliQuit(uint16 daoAt) external {
        ...
        IERC20Upgradeable(daos[daoAt].token).safeTransfer(msg.sender, daoCounclis[daoAt][councliAt_].frozen);
        daoCounclis[daoAt][councliAt_].frozen = 0;
    ...
    function lpToTokenPrice(address lp, address token) public view returns (uint256) {
        return (IERC20Upgradeable(token).balanceOf(lp) * 2e18) / IERC20Upgradeable(lp).totalSupply();
    }
    ...
    function proposalAdd(
        ...
        if (councliAt_ == noneExist || daoCounclis[daoAt][councliAt_].status == false) {
            frozenAmount = IERC20Upgradeable(daos[daoAt].token).totalSupply() / 20000000;
            IERC20Upgradeable(daos[daoAt].token).safeTransferFrom(msg.sender, address(this), frozenAmount);
        }
        if (uintParams[2] != 0) {
            IERC20Upgradeable(daos[daoAt].token).safeTransferFrom(msg.sender, address(this), uintParams[2]);
        }
    ...
    function proposalFinsh(uint16 daoAt, uint16 proposalAt) external {
        ...
        if (daoProposals[daoAt][proposalAt].frozenAmount != 0) {
            IERC20Upgradeable(daos[daoAt].token).safeTransfer(daoProposals[daoAt][proposalAt].initiate, daoProposals[daoAt][proposalAt].frozenAmount);
        }

        if (daoProposals[daoAt][proposalAt].totalVote == 0 && daoProposals[daoAt][proposalAt].uintParams[2] != 0) {
            IERC20Upgradeable(daos[daoAt].token).safeTransfer(daoProposals[daoAt][proposalAt].initiate, daoProposals[daoAt][proposalAt].uintParams[2]);
        }
    ...
    mapping(address => EnumerableSetUpgradeable.UintSet) private userVoteRecord;
    ...
    function vote(
        ...
        if (lp) {
            ...
            IERC20Upgradeable(daos[daoAt].lp).safeTransferFrom(msg.sender, address(this), totalAmount);
            ...
            IERC20Upgradeable(daos[daoAt].token).safeTransferFrom(msg.sender, address(this), totalAmount);
    ...
    function voteFinsh(uint16 daoAt, uint16 proposalAt) external {
        ...
        if (daoProposals[daoAt][proposalAt].uintParams[2] != 0) {
            ...
            IERC20Upgradeable(daos[daoAt].token).safeTransfer(msg.sender, totalReward);
        }
        if (totalLp > 0) {
            IERC20Upgradeable(daos[daoAt].lp).safeTransfer(msg.sender, totalLp);
        }
        if (totalToken > 0) {
            IERC20Upgradeable(daos[daoAt].token).safeTransfer(msg.sender, totalToken);
        }
