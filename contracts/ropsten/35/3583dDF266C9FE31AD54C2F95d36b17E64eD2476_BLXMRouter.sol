...
contract BLXMRewardProvider is ReentrancyGuardUpgradeable, BLXMMultiOwnable, BLXMTreasuryManager, IBLXMRewardProvider {
    ...
    function _burn(address to, uint liquidity, uint idx) internal nonReentrant returns (uint amountBlxm, uint amountToken, uint deliveredRewards) {
        ...
        uint rewardAmount;
        {
            ...
            percentOfTotal = liquidity.wdiv(_totalLiquidity);

            extraLiquidity = liquidity == position.liquidity ? position.extraLiquidity : position.extraLiquidity.wmul(liquidity).wdiv(position.liquidity);
        ...
        (amountBlxm, amountToken, deliveredRewards) = _withdraw(getTreasury(position.token), rewardAmount, percentOfTotal, to);
        emit Burn(msg.sender, amountBlxm, amountToken, deliveredRewards, to);
    }
...
contract BLXMTreasuryManager is BLXMMultiOwnable, IBLXMTreasuryManager {
    ...
    function getReserves(address token) public view override returns (uint reserveBlxm, uint reserveToken) {
        address treasury = getTreasury(token);
        (reserveBlxm, reserveToken,,,,) = ITreasury(treasury).get_total_amounts();
    }

    function _withdraw(address treasury, uint rewards, uint percent, address to) internal returns (uint amountBlxm, uint amountToken, uint deliveredRewards) {
        return ITreasury(treasury).get_tokens(rewards, percent, to);
    }
