...
contract Cheek is ERC721A, Pausable, Ownable
{
    ...
    uint16 constant public MAX_USE_PER_TICKET = 2;
    ...
    mapping (uint256 => uint16) public usedTicketCount;

    ParadiseTicket private _pTicketContract;

    constructor(string memory newBaseURI, address paradiseTicketAddress, bytes32 newMerkleRootCheekList) ERC721A("Cheek", "CHEEK")
    {
        baseURI = newBaseURI;

        merkleRootCheekList = newMerkleRootCheekList;

        _pTicketContract = ParadiseTicket(paradiseTicketAddress);
    }
    ...
    function setCheeksListMerkleRoot(bytes32 value) public onlyOwner
    {
        merkleRootCheekList = value;
    }

    function forceMint(uint16 numberOfTokens) public onlyOwner
    {
    ...
    function ticketMint(uint8 numberOfTokens) public payable
    {
        ...
        require(numberOfTokens <= getMaxPurchaseAmount(msg.sender), "Can only mint MAX tokens at a time");

        _setUsedTickets(msg.sender, numberOfTokens);
        _safeMint(msg.sender, numberOfTokens);
    }

    function whitelistMint(bytes32[] memory proof) public payable
    {
        require(saleActive, "Sale is not active");
        require(totalSupply() < MAX_SUPPLY, "Purchase would exceed max supply of Cheeks");
        require(PRICE <= msg.value, "Ether value sent is not correct");
        require(!whitelistMinted[msg.sender], "Address already minted");
        require(isWhitelisted(msg.sender, proof), "addres not in whitelisted");

        whitelistMinted[msg.sender] = true;
        _safeMint(msg.sender, 1);
    }

    function getMaxPurchaseAmount(address addr) public view returns (uint256)
    {
        uint256 amount = 0;

        uint256 ticketBalance = _pTicketContract.balanceOf(addr);
        for (uint256 i = 0; i < ticketBalance; i++)
        {
            uint256 tokenId = _pTicketContract.tokenOfOwnerByIndex(addr, i);
            amount += (MAX_USE_PER_TICKET - usedTicketCount[tokenId]);
        }
    ...
    //   INTERNAL
    function _setUsedTickets(address addr, uint16 numberOfTokens) internal
    {
        uint256 ticketBalance = _pTicketContract.balanceOf(addr);
        for (uint256 i = 0; i < ticketBalance; i++)
        {
            uint256 tokenId = _pTicketContract.tokenOfOwnerByIndex(addr, i);
            uint16 usesLeft = MAX_USE_PER_TICKET - usedTicketCount[tokenId];
            if (usesLeft > 0)
            {
                if (numberOfTokens > usesLeft)
                {
                    numberOfTokens -= usesLeft;
                    usedTicketCount[tokenId] = MAX_USE_PER_TICKET;
                }
                else
                {
                    usedTicketCount[tokenId] = numberOfTokens;
                    break;
                }
            }
        }
