...
contract Appollo_Three is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Appollo Three";
    ...
    uint256 public _sellAppolloFundFee = 7e2;
    uint256 public _buyAppolloFundFee = 7e2;
    uint256 public _liquidityFee = 3e2;
    uint256 public _myBagsFee = 3e2;
    
    uint256 public _taxFeeTotal;
    uint256 public _appolloFundFeeTotal;
    ...
    uint256 private appolloFundFeeTotal_;
    uint256 private liquidityFeeTotal_;
    uint256 private myBagsFeeTotal_;
    
    address public _appolloFundWallet = 0x9F78Fb5Ad49a1c76d18E97585DEe8968367007f4;
    ...
    constructor() {
        ...
        isExcludedFromFee[_appolloFundWallet] = true;
    ...
    function _transfer(
        ...
        if (swapAndLiquifyEnabled && !isPair[sender] && !isPair[recipient] && timeOfSwapAndLiquify >= swapAndLiquifyInterval) {
            ...
            if (appolloFundFeeTotal_ > 0) {
                swapTokensForEther(appolloFundFeeTotal_, _appolloFundWallet);
                appolloFundFeeTotal_ = 0;
            }
    ...
    function collectFeeOnBuy(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev Take burn fee
        if(_buyAppolloFundFee != 0){
            uint256 buyAppolloFundFee = amount.mul(_buyAppolloFundFee).div(1e4);
            transferAmount = transferAmount.sub(buyAppolloFundFee);
            _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(buyAppolloFundFee.mul(rate));
            _appolloFundFeeTotal = _appolloFundFeeTotal.add(buyAppolloFundFee);
            appolloFundFeeTotal_ = appolloFundFeeTotal_.add(buyAppolloFundFee);
            emit Transfer(account, address(this), buyAppolloFundFee);
        }
    ...
    function collectFeeOnSell(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev Take burn fee
        if(_sellAppolloFundFee != 0){
            uint256 sellAppolloFundFee = amount.mul(_sellAppolloFundFee).div(1e4);
            transferAmount = transferAmount.sub(sellAppolloFundFee);
            _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(sellAppolloFundFee.mul(rate));
            _appolloFundFeeTotal = _appolloFundFeeTotal.add(sellAppolloFundFee);
            appolloFundFeeTotal_ = appolloFundFeeTotal_.add(sellAppolloFundFee);
            emit Transfer(account, address(this), sellAppolloFundFee);
        }
    ...
    function updateFees(uint256 tax, uint256 buyAppolloFund, uint256 sellAppolloFund, uint256 liquidity, uint256 myBags) public onlyOwner {
        _taxFee = tax;
        _buyAppolloFundFee = buyAppolloFund;
        _sellAppolloFundFee = sellAppolloFund;
    ...
    function updateApolloFundAddress(address appolloFund) public onlyOwner {
        _appolloFundWallet = appolloFund;
    }
