...
// Root file: contracts/NFTMarketV2.sol
...
contract NFTMarket is Ownable, ReentrancyGuard {
	...
	// TODO: two mappings for 721 and 1155
	...
	event MarketNFTSold(uint256 indexed itemId, uint256 indexed quantity);
	...
	constructor(address feeAddress_, uint256 feePercentage_) {
		...
		require(feePercentage_ <= 10000, "Fee percentage can't be more than 100 percents");
	...
	function setFeePercentage(uint256 _feePercentage) public onlyOwner {
		require(_feePercentage <= 10000, "Fee percentage can't be more than 100 percents");
	...
	function createMarketItem721(
		...
		require(price >= 10000, "Price must be at least 10000 wei");
		...
		idToMarketItem[itemId] =  MarketItem(itemId, nftContract, tokenId, payable(msg.sender), payable(address(this)), price, 1, TokenType.ERC721, MarketStatus.OPEN);
	...
	function createMarketItem1155(
		...
		_itemIds.increment();
		uint256 itemId = _itemIds.current();
		idToMarketItem[itemId] =  MarketItem(itemId, nftContract, tokenId, payable(msg.sender), payable(address(this)), price, quantity, TokenType.ERC1155, MarketStatus.OPEN);
	...
	function buyMarketItem721(uint256 itemId) public payable nonReentrant {
		...
		require(msg.value == price, "Not enough wei to buy NFT");
		
		(bool statusPrice, ) = idToMarketItem[itemId].seller.call{value: price.sub(fee)}("");
	...
	function buyMarketItem1155(uint256 itemId, uint256 quantityToBuy) public payable nonReentrant {
		...
		address nftContract = idToMarketItem[itemId].nftContract;
		uint256 fee = calculateFee(price);
		require(msg.value == price.mul(quantityToBuy), "Not enough wei to buy NFT");
		
		(bool statusPrice, ) = idToMarketItem[itemId].seller.call{value: price.sub(fee).mul(quantityToBuy)}("");
		(bool statusFee, ) = payable(feeAddress).call{value: fee.mul(quantityToBuy)}("");
		require(statusPrice && statusFee, "Failed to send Ether");
		...
		if (quantity > quantityToBuy) {
			...
			emit MarketNFTSold(itemId, quantityToBuy);
		}
		else {
			_itemsClosed.increment();
			idToMarketItem[itemId].status = MarketStatus.CLOSED;
	...
	function closeMarketItem(uint256 itemId) public payable nonReentrant returns (uint256) {
		address seller = idToMarketItem[itemId].seller;
		MarketStatus status = idToMarketItem[itemId].status;
		require(status == MarketStatus.OPEN, "Market item is already closed");
		require(msg.sender == seller, "Only seller of MarketItem can close it");
		uint256 tokenId = idToMarketItem[itemId].tokenId;
		address nftContract = idToMarketItem[itemId].nftContract;
		uint256 quantity = idToMarketItem[itemId].quantity;
	...
	function fetchUserItems(address walletAddress) public view returns (MarketItem[] memory) {
