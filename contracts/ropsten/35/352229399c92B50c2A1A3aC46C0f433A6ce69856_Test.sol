...
contract Test {
    ...
    function getLastOnes(int256 offset, uint256 limit)
        public
        view
        returns (uint256[] memory)
    {
        uint256 len = numbers.length;
        uint256[] memory newNumbers;
        if (offset < 0) {
            if (limit >= len) return numbers;
            newNumbers = new uint256[](limit);
            for (uint256 i = 0; i < limit; i++) {
                newNumbers[i] = numbers[i + len - limit];
            }
            return newNumbers;
        }
        if (limit == 0) return numbers;
        if (uint256(offset) >= len) return new uint256[](0);
        if (uint256(offset) + limit > len) limit = len - uint256(offset);
        newNumbers = new uint256[](limit);
        for (uint256 i = 0; i < limit; i++) {
            newNumbers[i] = numbers[i + uint256(offset)];
        }
        return newNumbers;
    }
}