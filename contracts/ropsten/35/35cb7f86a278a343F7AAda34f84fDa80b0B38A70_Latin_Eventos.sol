...
contract Latin_Eventos  {
    ...
    event evento_creado(uint indexed e_id, bytes24 indexed Codigo_Latin, address indexed autor,uint porcentaje,uint32 Inicio,uint32 Fin,address Verificador);
    event aporte(uint indexed e_id, bytes24 indexed Codigo_Latin, address indexed participante, uint256 monto,uint a_id);
    event devolucion(uint indexed e_id,bytes24 indexed Codigo_Latin, address indexed participante, uint256 monto);
    event ganador_loteria(uint e_id,bytes24 indexed Codigo_Latin , address indexed participante, uint256 monto ,  uint256 randomnumber);
    event cierre_evento(uint indexed e_id, bytes24 indexed Codigo_Latin ,address indexed autor, address op, uint256 recaudado );
    event evento_cancelado(uint e_id, bytes24 indexed Codigo_Latin , address indexed Operador, string motivo);
    ...
    struct Evento {
        ...
        bytes24 codigo_latin; // Codigo LatinSER auntenticador
    }
