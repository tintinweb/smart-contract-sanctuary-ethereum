...
contract Invest is Ownable, Pausable {  
    ...
    function withdraw(uint256 poolID) public whenNotPaused {
        ...
        require(usdtToken.balanceOf(address(this)) > usdtReward, "Not Enough USDT in the smart contract");
        ...
        if(block.timestamp - startTime >= userTerms[poolID]){
            if((block.timestamp - redeemedAt[poolID] >= userTerms[poolID])){
                ...
                usdtToken.transfer(msg.sender, usdtReward);

                IERC20(userToken[poolID]).transfer(msg.sender, stableCoinBalance);

                omegaToken.transfer(msg.sender, stakedOmegaBalance);
    ...
    function restakeByAdmin(uint256 poolID, address Owner) external whenNotPaused onlyOwner returns(bool){
