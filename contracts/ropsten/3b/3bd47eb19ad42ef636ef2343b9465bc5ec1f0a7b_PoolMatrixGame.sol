    ...
    function buyLevel(uint8 level, uint256 investAmount) public payable {
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            while (!users[referrer].levels[level].openedOnce) {
                users[referrer].levels[level].missedProfit = true;
        ...
        if (!users[msg.sender].levels[level].opened) {
            ...
            users[msg.sender].levels[level].missedProfit = false;
    ...
    function getSlots(address userAddr) public view returns(int16[] memory) {
        ...
        for (uint8 level = 0; level < levelsCount; level++) {
            if (block.timestamp < initialTimestamp + levelIntervals[level]) {
                slots[level] = -1; // Not availabled yet (user need to wait some time once it bacome available)
                continue;
            }

            if (level > users[userAddr].maxLevel) {
                if (!users[userAddr].levels[level].missedProfit)
                    slots[level] = -2; // Not allowed yet (previous level is not opened)
                else
                    slots[level] = -3; // Not allowed yet (missed profit)
            ...
            if (!users[userAddr].levels[level].opened) {
                if (!users[userAddr].levels[level].openedOnce)
                    slots[level] = -4; // Available for opening
                else
                    slots[level] = -5; // Available for reopening
