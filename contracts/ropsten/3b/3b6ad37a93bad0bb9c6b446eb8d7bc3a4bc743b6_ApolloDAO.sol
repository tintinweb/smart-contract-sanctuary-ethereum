...
contract ApolloDAO is Context {
    ...
    function _voterIsServiceElgibile(address voter) public view returns (bool) {
        if (_votesCast[_msgSender()].length > 0) {
    ...
    function _candidateIsEligible(address voter, address candidate) public view returns (bool) {
        if(voter == candidate) {
            return false;
        }
