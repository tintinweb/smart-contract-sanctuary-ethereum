...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SRPLC }
    ...
    address public sRPLC;
    uint public sRPLCQueue; // Delays change to sRPLC address
    ...
    /**
        @notice allow approved address to deposit an asset for RPLC
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint RPLC needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( RPLC ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn RPLC for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IRPLCERC20( RPLC ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sRPLC ).balanceOf( msg.sender ); // Can only borrow against sRPLC held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with RPLC
        @param _amount uint
     */
    function repayDebtWithRPLC( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IRPLCERC20( RPLC ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, RPLC, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( RPLC ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( RPLC ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns RPLC valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match RPLC decimals
            value_ = _amount.mul( 10 ** IERC20( RPLC ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SRPLC ) { // 9
            sRPLCQueue = block.number.add( blocksNeededForQueue );
        } else return false;
