...
contract WalletSimple {
   ...
   */
  function createForwarder() onlysigner public returns (address) {
   ...
   */
  function sendMultiSig(address toAddress, uint value, bytes data, uint expireTime, uint sequenceId, bytes signature) onlysigner public {
   ...
   * @param tokenContractAddress the address of the erc20 token contract
   */
  function flushForwarderTokens(address forwarderAddress, address tokenContractAddress) onlysigner public {    
   ...
   */
  function activateSafeMode() onlysigner public {
    safeMode = true;
   ...
   */
  function isSigner(address signer) view public returns (bool) {
   ...
   */
  function recoverAddressFromSignature(bytes32 operationHash, bytes signature) pure private returns (address) {
   ...
   */
  function getNextSequenceId() view public returns (uint) {
