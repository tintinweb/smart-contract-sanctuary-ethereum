...
contract KRLRacers is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    constructor(address signerAddress_) ERC721("KRLRacers", "Racers") {       
    ...
    modifier whenHolderSaleIsOn {
        require(block.timestamp > HOLDER_SALE_START_TIME && block.timestamp < HOLDER_SALE_END_TIME);
        _;
    }
    modifier whenAllowlistSaleIsOn {
        require(block.timestamp > ALLOWLIST_SALE_START_TIME && block.timestamp < ALLOWLIST_SALE_END_TIME);
        _;
    }
    modifier whenPublicaleIsOn {
        require(block.timestamp > PUBLIC_SALE_START_TIME && block.timestamp < PUBLIC_SALE_END_TIME);
        _;
    }    
    ...
    function setCollab(address[] memory wallets, bool status) public onlyOwner{
        for(uint256 i=0;i<wallets.length;i++){
            collabMint[wallets[i]] = status;
        }
    }

    function holderMint(uint256 quantity, bytes calldata signature) public payable whenHolderSaleIsOn {
        require(checkSign(signature)==_signerAddress, "Invalid Signature");
        require(checkHolderMinted().add(quantity)<=HOLDERS_LIMIT, "Mint would exceed limit");
        require(msg.value == HOLDER_MINT_PRICE.mul(quantity));
    ...
    function allowListMint(uint256 quantity, bytes32[] calldata proof) public payable whenAllowlistSaleIsOn {
       ...
       require(checkAllowlistMinted()<=ALLOWLIST_LIMIT.add((HOLDERS_LIMIT.sub(HOLDERS_MINTED))), "Will Exceed Allowlist Limit");
    ...
    function CollabMint() public payable {
       require(msg.value == MINT_PRICE, "Send proper msg value");
       require(totalSupply().add(1)<=MAX_SUPPLY, "Exceeding Max Limit");
       require(isAllowedCollab(msg.sender)==true, "Not Allowed");
       collabMint[msg.sender] =false;
       payable(owner()).transfer(msg.value);
       _tokenIdCounter.increment();
       uint256 tokenId = _tokenIdCounter.current();
       if(tokenId <= MAX_SUPPLY){
    ...
    function checkSign(bytes calldata signature) private view returns (address) {
        return keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                bytes32(uint256(uint160(msg.sender)) + checkHolderWallet(msg.sender))
            )
        ).recover(signature);
    }
