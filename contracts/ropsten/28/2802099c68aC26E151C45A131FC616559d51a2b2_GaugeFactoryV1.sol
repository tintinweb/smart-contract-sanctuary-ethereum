...
contract GaugeFactoryV1 is IGaugeFactoryV1 {
    event GaugeCreated(
        address gauge,
        address pair,
        address bribe,
    ...
    function createGauge(
        ...
        gauge = address(new GaugeV1(_pair, _bribe, _ve, _voter));
        emit GaugeCreated(gauge, _pair, _bribe, _ve, _voter);
    }
}
...
contract GaugeV1 is IGaugeV1, ReentrancyGuard, Ownable {
        ...
        returns (uint256)
    {
        ...
        uint256 veTotal = IVeV1(ve).totalSupply();

        uint256 boost = 0;
        // ownerOf may fail if token doesn't exist - ERC721 spec
        if (
            tokenId > 0 && account == IVeV1(ve).ownerOf(tokenId) && veTotal > 0
        ) {
            uint256 veBal = IVeV1(ve).balanceOfNFT(tokenId);
    ...
    function deposit(uint256 amount, uint256 tokenId) external lock {
        ...
        if (tokenId > 0) {
            require(IVeV1(ve).ownerOf(tokenId) == msg.sender, "not ve owner");
