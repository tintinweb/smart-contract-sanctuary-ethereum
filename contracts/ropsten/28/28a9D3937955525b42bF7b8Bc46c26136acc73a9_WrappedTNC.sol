...
interface IERC20 {
    ...
    /**
     * @dev Emitted when maxSupply updated
     *
     * Note that max supply shouldn't be less then total supply.
     */
    event maxSupplyUpdated(uint256 crrMaxSupply, uint256 updatedMaxSupply);
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        require(maxSupply != _totalSupply, "ERC20: minted maximum supply");
        require(
            maxSupply >= (_totalSupply + amount),
            "ERC20: minting amount exceeding max supply"
        );
     ...
     */
    function _updateMaxSupply(uint256 newMaxSupplyAmount) internal virtual {
        require(
            newMaxSupplyAmount > _totalSupply,
            "ERC20: Max supply amount less then total supply"
        );
        emit maxSupplyUpdated(maxSupply, newMaxSupplyAmount);
        maxSupply = newMaxSupplyAmount;
    }
...
contract WrappedTNC is Pausable, Ownable, Burnable, Lockable, ERC20 {
    ...
    /**
     * @dev only owner can change max supply
     */
    function updateMaxSupply(uint256 _newValue) public onlyOwner whenNotPaused {
        _updateMaxSupply(_newValue);
    }
