...
contract AccessControl is Blackjack_DataSets 
{
    ...
    /// @dev Access modifier for LuGodness-only functionality
    modifier onlyLuGodNess() {
        require(msg.sender == LuGodness);
        _;
    }
...
// ----------------------------------------------------------------------------
contract MoneyMoney_Transection is ERC20_Interface, Blackjack_DataSets, Functionality_Event, AccessControl
{
    ...
    {
        uint Im_CreateLuTokenAmount = (msg.value)/101;
    ...
    ) 
    {
        ...
        bool Success = _GetPayAddress.send(LuTokenAmount * 100);
        
        emit ExchangeLuTokenEvent
        (
            {_ETH_AddressEvent: _GetPayAddress,
            _ETH_ExchangeAmountEvent: LuTokenAmount * 100,
        ...
        return (Success, Im_PayerUserId, _GetPayAddress, LuTokenAmount * 100, Mapping__OwnerUserId_ERC20Amount[Im_PayerUserId]);
    }
...
// ----------------------------------------------------------------------------
contract Blackjack_Functionality is Blackjack_DataSets, Functionality_Event, AccessControl, MoneyMoney_Transection 
{
