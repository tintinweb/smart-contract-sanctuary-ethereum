...
contract Kollab_Share_Factory {
    ...
    function getSplitters (address _sender) public view returns (string[] memory) {
        ...
        for(uint j = 0; j < asoc.length; j++) {
            _address = address(splitters[asoc[j]]);
            result[i] = Strings.toHexString(_address);
            result[i + 1] = Strings.toString(splitter_id[_address]);
            result[i + 2] = splitters[asoc[j]].getName();
            result[i + 3] = splitters[asoc[j]].getDescription();
            result[i + 4] = Strings.toString(splitters[asoc[j]].getUserBalance(_sender));
            result[i + 5] = Strings.toString(address(splitters[asoc[j]]).balance);
            result[i + 6] = Strings.toString(splitters[asoc[j]].getLastWithdrawl(_sender));
            result[i + 7] = Strings.toString(splitters[asoc[j]].getTotalShares());
            result[i + 8] = Strings.toString(splitters[asoc[j]].getPayeeShares(_sender));
            result[i + 9] = Strings.toHexString(splitters[asoc[j]].getCreator());
            i+=10;
        }
        return result;
    }
