...
contract ExchangeNFT is Ownable, TokenRecipient{
     ...
     */
    event logData(uint256 value, address[3] contracts, string strs);
    function exchange(
        address[3] calldata contracts,
        address[8] calldata addrs,
        uint256[3] calldata uints,
        ...
        adminOnly()
    {
        ...
        emit logData(totalValue, contracts, saleType);
        if(keccak256(bytes(saleType)) == keccak256(bytes("BUY"))) {
        ...
        if(contracts[2] != address(0)){
            ...
            if(balanceOfToken == 0 && uintTokens[0] > 0) {
                exchangeNft.mint(addrs[6], uintTokens[0], uintTokens[1], tokenUri);
            }
            if(uintTokens[2] > 0){
                exchangeNft.safeTransferFrom(addrs[6], addrs[7], uintTokens[0],  uintTokens[2], "0x0");
            }
