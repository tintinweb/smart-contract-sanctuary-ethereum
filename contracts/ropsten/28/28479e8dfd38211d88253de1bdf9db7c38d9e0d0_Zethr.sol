...
contract Zethr {
  ...
  */
  ...
  /**
   * TransferZethr tokens from the caller to a new holder.
   * No charge incurred for the TransferZethr. We'd make a terrible bank.
   */
  ...
  returns (bool)
  {
    require(_amountOfTokens >= MIN_TOKEN_TRANSFER || _amountOfTokens == 1
  ...
  /**
   * TransferZethr tokens from the caller to a new holder: the Used By Smart Contracts edition.
   * No charge incurred for the TransferZethr. No seriously, we'd make a terrible bank.
   */
  ...
  returns (bool)
  {
    ...
    bytes memory empty;
    // Make sure we own the tokens we're transferring, are ALLOWED to TransferZethr that many tokens,
  ...
  returns (uint)
  {
    ...
    // TransferZethr to bankroll and div cards
  ...
  internal
  {
    ...
    // Calculate how many back-end dividend tokens to TransferZethr.
    ...
    if (_customerAddress != msg.sender) {
      ...
      // Don't update this if we are transferring our own tokens (via TransferZethr or buyAndTransfer)
    ...
    // Fire logging event.
    emit TransferZethr(_customerAddress, _toAddress, _amountOfTokens);
  }
