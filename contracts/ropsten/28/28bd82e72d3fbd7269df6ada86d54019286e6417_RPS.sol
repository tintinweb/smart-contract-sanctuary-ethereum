...
contract RPS {
    ...
    // struct Encyption {  // keccak으로 두를거
    //     bytes32 r;
    //     bytes32 p;
    //     bytes32 s;
    // }

    enum Hand {  // 가위/바위/보 value에 대한 enum 각각 순서대로 0 , 1 , 2 임
		rock, paper, scissors
	}
	...
	uint roomLen = 0; // rooms의 키 값입니다. 방이 생성될 때마다 1씩 올라감
    ...
    function createRoom (Hand _hand) public payable isValidHand(_hand) returns (uint roomNum) {  // 베팅금액을 설정하기 때문에 payable 키워드를 사용
