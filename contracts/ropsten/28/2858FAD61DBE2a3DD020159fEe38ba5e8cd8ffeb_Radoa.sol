...
contract UpdateMsgVerifier is SignatureVerifier, VDFHashManager {
    ...
    event SignatureResult(address indexed oldAddress, address indexed validAddress, bytes32 indexed msgHash);
    event UpdateMsgBytes(bytes indexed msgBytes);
    ...
    function hasher(UpdateMsg memory _msg) internal returns (bytes32) {
        bytes memory inputBytes = toBytes(_msg);
        emit UpdateMsgBytes(inputBytes);
        bytes32 hashed = sha256(inputBytes);
        return hashed;
    }

    function attestMsgValidity(UpdateMsg memory _oldMsg, UpdateMsg memory _newMsg, Signature memory _updateSignature) internal {
        ...
        emit SignatureResult(oldAddress, validAddress, updateHash);
    }
...
contract Timer is ITimer {
    ...
    uint64 constant UPDATE_PERIOD = 60;
