...
contract MinterRole is Context {
    ...
    modifier onlyMinter() {
        require(isMinter(_msgSender()),"MinterRole: only minter can mint");
        _;
    }
...
contract NFT is ERC721("RentingNFT","RNFT"), Ownable, MinterRole {
    event Register(address user, uint256 _tokenId);
    ...
    function isBusiness() public view returns (bool) {
        ...
        for (uint256 i = 0; i < length; i++) {
            if (businessAddresses[i] == msg.sender){
                _isBusiness = true;
                break;
            }
        }
    ...
    function getBusinessAdresses() public view returns (address[] memory) {
        return businessAddresses;
    }

    function create(address user, address _creator) public onlyManager returns (uint256) {
        _mint(user, ++tokenId);
        metadata[tokenId] = Meta(_creator);
        emit Register(user, tokenId);
        return tokenId;
    }
    ...
    //     require(!userBlocks[msg.sender] && !userBlocks[to]);
    ...
    function transferFrom(
        ...
        require(!userBlocks[msg.sender] && !userBlocks[from] && !userBlocks[to]);
