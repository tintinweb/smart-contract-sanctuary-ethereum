...
contract CustomDex {
    ...
    function swapEthToToken(string memory tokenName) public payable {
        tokenInstanceMap[tokenName].transfer(msg.sender, 10);
    }

    function swapTokenToEth(string memory tokenName) public {
        payable(msg.sender).transfer(1000000000000000);
        tokenInstanceMap[tokenName].transferFrom(msg.sender, address(this), 10);
    }

    function swapTokenToToken(string memory srcTokenName, string memory destTokenName) public {
        tokenInstanceMap[srcTokenName].transferFrom(msg.sender, address(this), 10);
        tokenInstanceMap[destTokenName].transfer(msg.sender, 10);
    }
}