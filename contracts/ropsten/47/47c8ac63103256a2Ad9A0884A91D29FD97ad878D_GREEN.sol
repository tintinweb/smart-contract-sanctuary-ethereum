...
contract GREEN is ERC20 {
    ...
    event TestMinted(address _to, uint256 _amount);
    ...
    constructor(string memory name, string memory symbol, address _treasury) ERC20(name, symbol) {
        _mint(_treasury, 12500000000*1e18);
        minter = msg.sender;
    }
     ...
     * @dev Mint new BEND.
     */
    function mintTest(address _to, uint256 _amount) external onlyMinter {
        _mint(_to, _amount);
        emit TestMinted(_to, _amount);
    }
}