...
contract FaceMask is ERC721, ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _nbTokens, bytes32 hash) external payable whenSaleStarted {
    ...
    function getHashedSecret(string memory code) public view onlyOwner returns(bytes32 hash) {
    ...
    function generateHashSignature(string memory code) public view returns(bytes32 hash) {
        return keccak256(abi.encodePacked(msg.sender, keccak256(abi.encodePacked(code))));
    }
}
