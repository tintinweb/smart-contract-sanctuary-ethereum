...
contract EFCRVVault is Ownable, ReentrancyGuard{
  ...
  function getTotalVolumeInStable() public view returns(uint256){
    return uint256(ChainlinkInterface(oracle).latestAnswer()).safeMul(getTotalVolume()).safeDiv(1e20);
  }
  ...
  function getUserVolumeInStable(address addr) public view returns(uint256){
    return uint256(ChainlinkInterface(oracle).latestAnswer()).safeMul(getUserVolume(addr)).safeDiv(1e20);
  }
