...
contract ZumerLens is Exponential {
    struct LiquidityPoolMetadata {
        address zBond;
        address zNft;
        uint256 totalSupplied;
        uint256 totalBorrowed;
        uint256 totalCash;
        uint256 lendingPoolExchangeRate;
        uint256 provisioningPoolFactorMantissa;
        // fee ratios
        uint256 creditCostRatioMantissa;
        uint256 underwritingFeeRatioMantissa;
        // correspoding provisioning pool data
        address provisioningPool;
        uint256 provisioningPoolCash;
        uint256 provisioningPoolExchangeRate;
    }

    function getLiquidityPoolMetadata(ZBond zBond)
        public
        view
        returns (LiquidityPoolMetadata memory)
    {
        LiquidityPoolMetadata memory lpm = LiquidityPoolMetadata({
            zBond: address(zBond),
            zNft: address(zBond.ZNft()),
            totalSupplied: zBond.totalSupplyPrinciplePlusInterest(),
            totalBorrowed: zBond.totalBorrows(),
            totalCash: zBond.getCashBalance(),
            lendingPoolExchangeRate: zBond.getExchangeRateMantissa(),
            provisioningPoolFactorMantissa: zBond.provisioningPoolMantissa(),
            creditCostRatioMantissa: zBond.creditCostRatioMantissa(),
            underwritingFeeRatioMantissa: zBond.underwritingFeeRatioMantissa(),
            provisioningPool: address(zBond.provisioningPool()),
            provisioningPoolCash: address(zBond.provisioningPool()).balance,
            provisioningPoolExchangeRate: ProvisioningPool(
                zBond.provisioningPool()
            ).getCurrentExchangeRateMantissa()
        });

        return lpm;
    }

    function getLiquidityPoolMetadataMultiple(ZBond[] calldata zBond)
        public
        view
        returns (LiquidityPoolMetadata[] memory)
    {
        uint256 length = zBond.length;
        LiquidityPoolMetadata[] memory lpms = new LiquidityPoolMetadata[](
            length
        );
        for (uint256 i = 0; i < length; i++) {
            lpms[i] = getLiquidityPoolMetadata(zBond[i]);
        }
    ...
    struct ZBondBalance {
        address zBond;
        uint256 balanceOfZBond;
        uint256 balanceOfUnderlyingRedeemable;
        uint256 borrowBalance;
    ...
    struct ProvisioningPoolBalance {
        address provisioningPool;
        // provisioning pool data
        uint256 ppTokenBalance;
        uint256 claimableUnderlyingBalance;
        // ZUMER
        uint256 claimableZumer;
    }

    function getZBondMetaData(
        Comptroller comptroller,
        ZBond zBond,
        address account
    ) public view returns (ZBondBalance memory) {
        ZBondBalance memory zb;
        PriceOracle oracle = comptroller.oracle();

        zb.zBond = address(zBond);
        zb.balanceOfZBond = zBond.balanceOf(account);
        zb.balanceOfUnderlyingRedeemable = divScalarByExpTruncate(
            zb.balanceOfZBond,
            Exp(zBond.getExchangeRateMantissa())
        );
        zb.borrowBalance = zBond.getAccountCurrentBorrowBalance(account);
        zb.priceMantissa = oracle.getUnderlyingPrice(address(zBond));

        (
            zb.deadline,
            zb.loanDuration,
            zb.minimumPaymentDue,
            zb.principalBorrow,
            zb.weightedInteretRate
        ) = zBond.accountBorrows(account);

        return zb;
    }

    function getProvisioningPoolData(ProvisioningPool pp, address account)
        public
        view
        returns (ProvisioningPoolBalance memory)
    {
        ProvisioningPoolBalance memory ppb;
        ProvisioningPool pp = ProvisioningPool(pp);
        //uint test = pp.totalSupply();
        ppb.ppTokenBalance = pp.balanceOf(account);
        ppb.claimableUnderlyingBalance = convertTokenBalance(
            ppb.ppTokenBalance,
            pp
        );
    ...
    function convertTokenBalance(uint256 shareTokenBalance, ProvisioningPool pp)
        public
        view
        returns (uint256)
    {
        pp.balanceOf(address(0));
        return
            getExp(shareTokenBalance, pp.getCurrentExchangeRateMantissa())
                .mantissa;
    }

    function getZBondMetaDataAll(
        Comptroller comptroller,
        ZBond[] memory zBond,
        address account
    ) public view returns (ZBondBalance[] memory) {
        uint256 length = zBond.length;
        ZBondBalance[] memory lpms = new ZBondBalance[](length);
        for (uint256 i = 0; i < length; i++) {
            lpms[i] = getZBondMetaData(comptroller, zBond[i], account);
        }
    ...
    function getLoanToValue(
        ...
        Comptroller comptroller,
        address account
    ) public view returns (uint256 loanToValueMantissa) {
        (uint256 liquidity, uint256 shortfall) = comptroller
            .getAccountLiquidity(account, address(znft));

        (bool isListed, uint256 collateralFactorMantissa) = comptroller.markets(
            address(znft)
        );
        uint256 totalBalance = znft.balanceOf(account);
        uint256 price = NftPriceOracle(comptroller.nftOracle())
            .getUnderlyingPrice(znft);
        ...
        uint256 collateralValue = mul_(
            mul_(totalBalance, Exp(price)),
            Exp(collateralFactorMantissa)
        );
        loanToValueMantissa = getExp(
    ...
    struct ZNFTMetaData {
        address ZNft;
        uint256 collateralFactorMantissa;
        uint256 loanToValueRatioMantissa;
        uint256 priceMantissa;
        uint256 creditCost;
    }

    function getZNFTMetaData(
        ZNft zNft,
        Comptroller comptroller,
        address account
    ) public view returns (ZNFTMetaData memory) {
        (bool isListed, uint256 collateralFactorMantissa) = comptroller.markets(
            address(zNft)
        );
        return
            ZNFTMetaData({
                ...
                loanToValueRatioMantissa: getLoanToValue(
                    zNft,
                    comptroller,
                    account
                ),
                priceMantissa: NftPriceOracle(comptroller.nftOracle())
                    .getUnderlyingPrice(zNft),
                creditCost: 0.05 * 1e18
            });
    }

    function getZNFTMetaDataAll(
        ZNft[] calldata zNft,
        Comptroller comptroller,
        address account
    ) public view returns (ZNFTMetaData[] memory) {
        uint256 length = zNft.length;
        ZNFTMetaData[] memory lpms = new ZNFTMetaData[](length);
        for (uint256 i = 0; i < length; i++) {
            lpms[i] = getZNFTMetaData(zNft[i], comptroller, account);
        }
        return lpms;
    }
    ...
    struct FundingRateMetaData {
        uint256 upperBoundLong;
        uint256 lowerBoundLong;
        uint256 upperTotalLong;
        uint256 lowerTotalLong;
        uint256 upperBoundShort;
        uint256 lowerBoundShort;
        uint256 upperTotalShort;
        uint256 lowerTotalShort;
        uint256 upperLong;
        uint256 lowerLong;
        uint256 upperShort;
        uint256 lowerShort;
        uint256 fundingCostHighEnd;
        uint256 fundingCostLowerEnd;
    }

    function getFundingRateMetaData(FeeSelector feeSelector, address account)
        public
        view
        returns (FundingRateMetaData memory)
    {
        FundingRateMetaData memory frd;
        ...
        (
            ...
            frd.upperShort,
            frd.lowerShort
        ) = feeSelector.userAcounts(account);

        (frd.fundingCostHighEnd, frd.fundingCostLowerEnd) = feeSelector
            .getFundingCostRateFx();
