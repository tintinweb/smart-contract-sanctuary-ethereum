...
contract token {
    ...
    string public standard = "Rajacoin";
    string public name;
...
contract Rajacoin is owned, token {
    ...
    string public tokenName = "Rajacoin";
    string public tokenSymbol = "RJC";
    function Rajacoin() token (initialSupply, tokenName, decimalUnits, tokenSymbol) {}
     /* Send coins */
    function transfer(address _to, uint256 _value) {
        if (balanceOf[msg.sender] < _value) throw;           // Check if the sender has enough
        if (balanceOf[_to] + _value < balanceOf[_to]) throw; // Check for overflows
