...
contract ContractMinter is Default {
    ...
    constructor() Default(10000, 'Contract Minter', 'minter') ERC721Royalty(_msgSenderERC721A(), 1000) {
        ...
        setConfig(0, 0, 3, 0.04 ether, 0);
        setConfig(0, 0, 5, 0.05 ether, 0);
    }
...
abstract contract Default is ERC721, ERC721Burnable, ERC721Metadata, ERC721Pausable, ERC721Royalty, Withdrawable {
    ...
    // Mint Id => Configuration
    mapping(uint256 => Config) public _config;

    // Mint Id => [Address => Minted Counter]
    mapping(uint256 => mapping(address => uint256)) public _minted;


    uint256 public immutable _maxSupply;


    constructor(uint256 maxSupply_, string memory name_, string memory symbol_) ERC721(name_, symbol_) {
    ...
    function deleteAllowlist(uint256 id) external onlyOwner {
        _allowlists[id] = bytes32('');
    }

    function deleteConfig(uint256 id) external onlyOwner {
        _minted[id][owner()] = 0;
    }

    function mint(uint256 id, uint256 quantity) payable public whenNotPaused {
        mint(id, new bytes32[](0), quantity);
    }

    function mint(uint256 id, bytes32[] memory proof, uint256 quantity) payable public whenNotPaused {
        if (_minted[id][owner()] == 0) {
            revert InvalidMintKey();
        }

        address buyer = _msgSenderERC721A();
        Config memory config = _config[id];

        if (_allowlists[id] != 0) {
            MintGate.allowed(buyer, proof, _allowlists[id]);
        }

        MintGate.open(config.endsAt, config.startsAt);
        MintGate.price(buyer, config.price, quantity, msg.value);
        MintGate.supply((_maxSupply - _totalMinted()), config.maxMint, _minted[id][buyer], quantity);

        unchecked {
            _minted[id][buyer] += quantity;
        }

        _safeMint(buyer, quantity);
    }

    function setAllowlist(uint256 id, bytes32 root) public onlyOwner {
        _allowlists[id] = root;
    }

    function setConfig(uint256 id, uint64 endsAt, uint64 maxMint, uint64 price, uint64 startsAt) public onlyOwner {
        _config[id] = Config({
            endsAt: endsAt,
            maxMint: maxMint,
            price: price,
        ...
        // Initialize Mint Id Mapping
        // - Doubles as 'exists' check for mint configuration
        _minted[id][owner()] = 1;
    }
