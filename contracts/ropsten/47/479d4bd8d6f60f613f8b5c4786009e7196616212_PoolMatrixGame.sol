    ...
    */
    ...
    function buyLevel(uint8 level, uint256 investAmount) public payable {
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            users[referrer].userCountInLine[i]++; // todo userCountInLine
            restOfAmount -= value;
    ...
    function getUser(address userAddr) public view returns(bool registered, uint registrationTimestamp, address userAddress, address referrer, uint256 debit, uint256 credit, uint256[] memory userCountInLine, uint256 referralReward, uint256 levelProfit, UserLevelInfo[] memory levels) {
    ...
    function getTotalInfo() public view returns(uint256 totalUsers, uint256 totalTransactions, uint256 totalTurnover) {
        return (userAddresses.length, transactionCounter, turnoverAmount);
    }
