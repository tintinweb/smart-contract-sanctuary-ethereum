...
contract RingOfDICK is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private constant _name = "Ring Of Dick";
    ...
    uint256 private buyDAIUpperLimit = 25 * 1e17; // 0.25 ETH
    uint256 private buyContractXUpperLimit = 25 * 1e17; // 0.25 ETH
    // fees
    uint256 public j_liqBuy = 1; 
    uint256 public j_burnBuy = 1;
    uint256 public j_DAIBuy = 0; //turns on after launch is stable
    ...
    uint256 private j_previousDAIFee = j_DAIFee;
    ...
    address payable private DICK = payable(0xACBcC8FC0c9Bdf80186cBF46d4eD58c19B141E9D); //0xACBcC8FC0c9Bdf80186cBF46d4eD58c19B141E9D
    address payable private mktg = payable(0x4570358dA0c49EdbF7059b9b8724C99E2938791E); //0x4570358dA0c49EdbF7059b9b8724C99E2938791E
    ...
    address DAI = 0x6B175474E89094C44Da98b954EedeAC495271d0F; //0x6B175474E89094C44Da98b954EedeAC495271d0F
    ...
    function removeAllFee() private {
        if (j_contractXTax == 0 && j_burnFee == 0 && j_liqFee == 0 && j_DAIFee == 0 && j_jeetTax == 0) return;
        j_previousBurnFee = j_burnFee;
        j_previousLiqFee = j_liqFee;
        j_previousDAIFee = j_DAIFee;
    ...
    function restoreAllFee() private {
        ...
        j_DAIFee = j_previousDAIFee;
        j_jeetTax = j_previousJeetTax;
        j_contractXTax = j_previousContractXTax;
    }

    function changeBuyTax(uint256 burn, uint256 liq, uint256 ETH, uint256 jeet, uint256 contractX) external {
        require(_msgSender() == DICK);
        j_burnBuy = burn;
        j_liqBuy = liq;
        j_DAIBuy = ETH;
        j_jeetBuy = jeet;
    ...
    function changeSellTax(uint256 burn, uint256 liq, uint256 ETH, uint256 jeet, uint256 contractX) external {
        require(_msgSender() == DICK);
        j_burnSell = burn;
        j_liqSell = liq;
        j_DAISell = ETH;
    ...
    function swapETHforDAITokens(uint256 amount) private {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockSwap {
        uint256 totalFees = (j_burnFee.add(j_DAIFee).add(j_jeetTax).add(j_liqFee).add(j_contractXTax));
        ...
        uint256 tokensForDAI = j_DAIFee.mul(contractTokenBalance).div(totalFees);
        uint256 ethForDAI = ((address(this).balance.sub(initialBalance)).mul(tokensForDAI)).div(tokensForETH);
        if(DAIMode && ethForDAI > 0) {
            buyDAI(ethForDAI);
        }
    ...
    function _transferAgain(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        fees.tDAI = amount.mul(j_DAIFee).div(100);
        fees.tJeet = amount.mul(j_jeetTax).div(100);
        fees.tContractX = amount.mul(j_contractXTax).div(100);
        
        fees.tAmount = amount.sub(fees.tDAI).sub(fees.tJeet).sub(fees.tBurn).sub(fees.tLiq).sub(fees.tContractX);
        ...
        _balances[address(this)] = _balances[address(this)].add(fees.tDAI).add(fees.tJeet).add(fees.tBurn).add(fees.tLiq).add(fees.tContractX);
    ...
    function endJeet() external {
        ...
        j_contractXBuy = 2;
        j_contractXSell = 2;
        contractXaddress = (0x6B175474E89094C44Da98b954EedeAC495271d0F); //0x6B175474E89094C44Da98b954EedeAC495271d0F
        contractXdead = dead;
        contractXMode = true;
        DAIMode = true;
    }
    ...
    function buyDAI(uint256 amount) private {
        if (amount > 0) {
            swapETHforDAITokens(amount);
            _totalDAIBurned = _totalDAIBurned.add(amount);
        }
    }
    ...
    function setBuyDAIRate(uint256 buyDAIToken) external {
        require(_msgSender() == DICK);
        buyDAIUpperLimit = buyDAIToken;
    }
    ...
    function setMktg(address payable _address) external {
        require(_msgSender() == DICK || _msgSender() == mktg);
        mktg = _address;
    }
