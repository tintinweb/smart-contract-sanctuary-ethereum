...
abstract contract UniStakingTokensStorage {
    ...
    constructor(IERC20 rewardsToken_, IERC20 stakingToken_,uint256 claimingFeePercent_) public {
    ...
    function _withdraw(address account, uint256 amount) internal {
       uint256 fee = amount.mul(claimingFeePercent).div(100);
        _feePool = _feePool.add(fee);
       uint256 claimedAmount = amount.sub(fee);
        _rewardsToken.safeTransfer(account, claimedAmount);
        _claimed[account] = _claimed[account].sub(claimedAmount);
        _feesPaid[account] = _feesPaid[account].add(claimedAmount);
    }
...
//
contract Staking is TwoStageOwnable, UniStakingTokensStorage {
    using SafeMath for uint256;
    ...
    ) public TwoStageOwnable(owner_) UniStakingTokensStorage(rewardsToken_, stakingToken_,claimingFeePercent_) {
    }
...
//
contract UniStakingSyntheticToken is Staking {
    ...
    constructor(
        ...
        uint256 claimingFeePercent_,
        address owner_
    ) public Staking(rewardsToken_, stakingToken_, owner_,claimingFeePercent_) {
        name = name_;
        symbol = symbol_;
        decimals = decimals_;
    }
