...
contract myToken is ERC1155 {
    ...
    function le() public view returns (uint256) {
        uint256 len = Propertylist.length;
        return (len);
    }

    function buy(address from, uint256 id) public {
        require(msg.sender != from);
        Propertylist[id].to = msg.sender;
        //  safeTransferFrom(_from, _to, _id);
        _safeTransferFrom(from, msg.sender, id, 1, "");
    }

    function sell(address to, uint256 id) public {
    ...
    // function vb(address user, uint256 id) public view returns (uint256) {
