...
contract ERC20 is Context, IERC20, Ownable {
    ...
    address marketing = 0xdEaE3412a02FA5D67D8bF1be435f180DfeB23D50;
    ...
    uint256 minLpDealTokenCount = 5000 * 10 ** decimals;
    uint256 minRecommend = 2 * 10 ** decimals;

    uint256 public startTime = 1645689491;
    uint256 startLimit = 1000 * 10 ** decimals;
    ...
    constructor() {
        ...
        startNotLimit[liquidityManager];
    }
    ...
    function setMinRecommend(uint256 _minRecommend) public onlyOwner {
        minRecommend = _minRecommend;
    }
    ...
    function _transfer(
        ...
        if (startTime + 3600 > block.timestamp && !startNotLimit[to]) {
            require(amount <= startLimit, "ERC20: Limit 1000");
        }
    ...
    function _saveRecommender(address from, address to) private {
        ...
        recommenderMap[to] = Address.isContract(from) ? market : from;
    }

    bool isFee = false;
    function _fee(address from, address to, uint256 amount) private returns(uint256 finalAmount) {
        if (isFee && (from == address(uniswapV2Pair) || to == address(uniswapV2Pair))) {
            require(startTime <= block.timestamp || startNotLimit[from] || startNotLimit[to], "It hasn't started yet");
    ...
    function _swapForLiquidity() private {
