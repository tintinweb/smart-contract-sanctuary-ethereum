...
contract City is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    IThugCityNFT public citizen = IThugCityNFT(0x542b4a99C54034797167dFf0ff262Ce75716fF57);

    //**HARDCODE**: Initialize bills CHANGE TO PARAM FOR MAINNET
    IBills public bills = IBills(0xEbd410fAE0F454397b8506ae930a424Bd407Af51);

    //**HARDCODE**: Initialize assets CHANGE TO PARAM FOR MAINNET
    IAssets public assets = IAssets(0xCee003D396C7653f3D53a7eC0d794fA3660f0dEF);
    ...
    function addManyToCity(address account, uint256[] calldata tokenIds) external nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(!staked[tokenIds[i]], "Character is already staked!");
    ...
    function withdrawSome(address _addr, uint256 amount) public nonReentrant {
    ...
    function withdrawAll(address _addr) public nonReentrant whenNotPaused {
        require(msg.sender == _addr);
        Stake[] storage thugs = thugStake[_addr];
        Stake[] storage cops = copStake[_addr];
    ...
    function getUnclaimedMedallions(uint256 tokenId) public view virtual returns (uint256 medallions) {
    ...
    function unstakeManyFromCity(uint256[] calldata tokenIds, bool withdrawAllFunds) external whenNotPaused {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (citizen.isCop(tokenId)) {
                ...
                deposited[msg.sender] += ((block.timestamp - copStake[msg.sender][copCollection[tokenId]].value) * copStake[msg.sender][copCollection[tokenId]].currentEarningRate) / 1 days - copStake[msg.sender][copCollection[tokenId]].spent;
                ...
                deposited[msg.sender] += ((block.timestamp - thugStake[msg.sender][thugCollection[tokenId]].value) * thugStake[msg.sender][thugCollection[tokenId]].currentEarningRate) / 1 days - thugStake[msg.sender][thugCollection[tokenId]].spent;
