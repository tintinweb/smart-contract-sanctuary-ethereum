 ...
 */
contract HarbergerTaxed_v11 is Ownable, ReentrancyGuard {
    ...
    function getCurrentPrice() public view returns (uint256) {
        return harbergerInfo.auctionPrice;
    }
    ...
    function TransferOwnershipOfHarbergerAtFirst(uint256 _newAuctionPrice) public payable {
        ...
        harbergerInfo.auctionPrice = _newAuctionPrice;
    ...
    function DelayEndTimeOfOwnership() public notIssuer onlyCurrentOwnerOfHarberger payable {
        ...
        uint256 currentAuctionPrice = harbergerInfo.auctionPrice;
    ...
    function TransferOwnershipOfHarbergerAtSecond(uint256 _newAuctionPrice) public payable {
        uint256 previousAuctionPrice = harbergerInfo.auctionPrice;
        ...
        (bool sent, bytes memory data) = payable(issuer).call{value: _newAuctionPrice * harbergerTax / 100}("");
        require(sent, "Failed to send Ether");

        (sent, data) = payable(currentOwner).call{value: _newAuctionPrice + _newAuctionPrice * harbergerHike / 100}("");
        require(sent, "Failed to send Ether");        
    ...
    function setValueOfSettings(
        ...
        string memory _valueOfString
            ...
            // if (_valueOfString != 'Current owner is not issuer') 
                harbergerInfo.valueOfString = _valueOfString;
