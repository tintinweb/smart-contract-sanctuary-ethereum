...
contract HanfuNFT is ERC721, Ownable {
    ...
    uint256 public constant hanfuPrice = 20000000000000000; //0.020 ETH
    uint256 public constant wlValidPrice = 66000000000000000; //0.066 ETH
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply, address payable _wallet, address payable _wallet1, address payable _wallet2, address payable _wallet3, address payable _wallet4) ERC721(name, symbol) {
    ...
    */
    ...
    function mintHanfuWl(uint numberOfTokens, bytes memory proof) public {
        require(saleIsActive, "Sale must be active to mint Hanfu");
        require(numberOfTokens <= getMaxHanfuPurchase(), "Can only mint 5 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= maxHanfu, "Purchase would exceed max supply of Hanfus");
        //require( getHanfuPrice().mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
    ...
    function mintNFTs(address addr, uint num) internal {
		uint mintIndex = totalSupply();
        for(uint i = 0; i < num; i++) {
            _safeMint(addr, mintIndex + i);
        }
    }

    function ogDrop() public onlyOwner {
        mintNFTs(0x2760B477b95B506dC9d3252B96e9A9eFC2b16a88, 2);
        mintNFTs(0x2760B477b95B506dC9d3252B96e9A9eFC2b16a88, 2);
    }
