...
contract Asstroverse is ERC20, Ownable {
    ...
    uint256 public amountToStart = 200 * (10**(IERC20(busd).decimals()));
    ...
    function _transfer(
        ...
        // ) {
        //     _swap(bal);
        //     // payable(assteroid).call{value: address(this).balance}("");
    ...
    function sendToAssteroid() public {
        payable(assteroid).call{value: address(this).balance}("");
    }

    function _swap(uint256 _amountToSwap) public swapLock {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
    ...
    function forgiveBot(address account) public onlyOwner {
    ...
    function changeMaxBuy(uint256 _newMaxSell) public onlyOwner {
        maxBuy = _newMaxSell;
    }

    function changeMaxSell(uint256 _newMaxSell) public onlyOwner {
        maxSell = _newMaxSell;
    }
    ...
    function setTaxesOff(bool _areOff) public onlyOwner {
        _taxesOff = _areOff;
    }
    ...
    function changeAmountToStart(uint256 amount) public onlyOwner {
        // amount in USD
