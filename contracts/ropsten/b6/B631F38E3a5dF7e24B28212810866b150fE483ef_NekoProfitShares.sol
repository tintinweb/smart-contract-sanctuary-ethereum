...
contract NekoProfitShares is Ownable, ReentrancyGuard {
    ...
    function lock(uint256 _amount) public nonReentrant {
        ...
        //require(userLocks[msg.sender].tokenAmount == 0, 'Already Locked');
        
        userLocks[msg.sender].tokenAmount = userLocks[msg.sender].tokenAmount.add(_amount);
        userLocks[msg.sender].startTime = block.timestamp; 
        userLocks[msg.sender].endTime = block.timestamp.add(lockDuration); 
    ...
    function claimLock(uint256 _amount) public nonReentrant {
        ...
        userLocks[msg.sender].claimedAmount = userLocks[msg.sender].claimedAmount.add(_amount);
