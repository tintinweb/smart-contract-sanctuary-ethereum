...
library CatLibrary {
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        address pair = ICatFactory(factory).getPair(tokenA, tokenB);
        require(pair != address(0), "CatLibrary: ZERO_ADDRESS");
        (uint reserve0, uint reserve1,) = ICatPair(pair).getReserves();
...
contract CatRouter {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? ICatFactory(factory).getPair(output, path[i + 2]) : _to;
            //address to = i < path.length - 2 ? CatLibrary.pairFor(factory, output, path[i + 2]) : _to;
            ICatPair(ICatFactory(factory).getPair(input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ICatFactory(factory).getPair(path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = CatLibrary.sortTokens(input, output);
            ICatPair pair = ICatPair(ICatFactory(factory).getPair(input, output));
            uint amountInput;
            uint amountOutput;
            {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ICatFactory(factory).getPair(path[0], path[1]), amountIn
        );
