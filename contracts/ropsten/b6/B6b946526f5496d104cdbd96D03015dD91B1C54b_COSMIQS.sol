...
contract COSMIQS is ERC721Enumerable, Ownable, PaymentSplitter  {
    ...
    uint256 public constant LegendaryReserved = 10;  //For Charity auction!
...
//        }
     ...
     //Legendary Mint
    function mintLegendary(uint256 _tokenAmount) public onlyOwner {
        uint256 supply = totalSupply();

        require(_tokenAmount <= LegendaryReserved, "More than Legendary mint!");
        for(uint256 i = 0; i < _tokenAmount; i++) {
            _safeMint(msg.sender, supply + i);
        }
    }
