...
contract HitToken is Lock {
    ...
    function HitToken(string _name, string _symbol, uint8 _decimals, uint256 _initialSupply,uint8 fundPrecent,uint8 icoPrecent,address _teamAccount,address _icoAccout) public {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
        icoAccount = _icoAccout;
        totalSupply_ = _initialSupply * 10 ** uint256(_decimals);
        //定义私募总量
        fundingBalance = totalSupply_.mul(fundPrecent).div(100); //20%
        //定义icoAccount 公募
        balances[icoAccount] = totalSupply_.mul(icoPrecent).div(100);
         //定义团队账户余额 
        teamBalance = totalSupply_.mul(100-fundPrecent-icoPrecent).div(100);
        balances[msg.sender] = teamBalance+fundingBalance; // 初始私募数量和团队数量给予消息发送者
        //转出团队账户余额
        mintTeam(_teamAccount,teamBalance);
    }
