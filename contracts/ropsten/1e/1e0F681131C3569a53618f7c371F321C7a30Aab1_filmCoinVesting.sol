...
contract filmCoinVesting{
   ...
   // mapping(uint => wallets) usersWallet;
    ...
    constructor(address _token , address[] memory userWallet){  
        ...
        totalBalance = token.balanceOf(Owner); 
    ...
    function CliffsAndPercentages() public OnlyOwner{
    ...
    function startCliffsAndReleases(address[] memory userWallet) public OnlyOwner{
    ...
    function claimToken(address user , bytes32 userClass) public {
        require(UserDetailsstruct[user].isExist == true,"User does not exist");
        require(user != address(0),"0 address");

        uint totalAmount;

        for(uint i = UserDetailsstruct[user].currIndex; i < cliffAndreleases[userClass].length; i++){
            require(UserDetailsstruct[user].startPeriod + cliffAndreleases[userClass][i] < block.timestamp , "Can't claim the token until the claiming period ends!");
            if(UserDetailsstruct[user].isClaimed[i] == true && UserDetailsstruct[user].startPeriod + cliffAndreleases[userClass][i] < block.timestamp){
                totalAmount += totalAlloc[userClass].mul(cliffAndreleases[userClass][i + 1] * 1e18).div(100e18);
                UserDetailsstruct[user].isClaimed[i] = false;
                UserDetailsstruct[user].isClaimed[i + 2] = true;
            ...
            require(totalAmount <= totalAlloc[userClass],"userWallet's allocated tokens are fully claimed"); 
            if(totalAmount != 0){
                token.transfer(user,totalAlloc[userClass].sub(totalAmount));
            } 
