...
contract testo is Context, IERC20, Ownable, LockToken 
{
    ...
    string private _symbol = "Shibaroo";
	...
	uint256 public totalBuyFee = _taxFee.add(_liquidityFee).add(_marketingFee).add(_devFee).add(_buybackFee); //12% Total Buy Fee
	...
	uint256 public _saledevFee = 2;
    uint256 public _saleBuyBackFee = 1;

    uint256 public totalSaleFee = _saleTaxFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saledevFee).add(_saleBuyBackFee); //13% Total Sale Fee
    

    address payable public marketingWallet =  payable(0xBFaA68C1Ae1ab6318D6DdBdE447D98451DD00101);
	address payable public devWallet =  payable(0x720121a2A1bFB1756647398f65692Ef5d89A5dB7);
    ...
    uint256 public _maxSaleAmount = 1_000_000_000 * 10**9; //1% changed to max supply
    uint256 public _maxBuyAmount = 1_000_000_000 * 10**9; //2% changed toax supply
    uint256 private numTokensSellToAddToLiquidity = 1_000_000 * 10**9;
    uint256 public maxLimit = 1_000_000_000 * 10**9; //max allowed tokens in one wallet. 
    ...
    constructor () 
    {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);  //Prod PCS = 0x2318fc5331AcB73F5CfF6A928c2f909040cfbeDa  Dev PCS =0x720121a2A1bFB1756647398f65692Ef5d89A5dB7
    ...
    function setAllBuyFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 devFee, uint256 buybackFee) public onlyOwner() 
    {
        _taxFee = taxFee;
        ...
        uint256 totalBuyFees = _taxFee.add(_liquidityFee).add(_marketingFee).add(_devFee).add(_buybackFee);
        require(totalBuyFees <= 25, "Total fee is over 25%");
	
    }

    function setAllSaleFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 devFee, uint256 buybackFee) public onlyOwner() 
    {
        _saleTaxFee = taxFee;
        ...
        uint256 totalSaleFees = _saleTaxFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saledevFee).add(_saleBuyBackFee);
    ...
    {  
        return _amount.mul(_liquidityFee.add(_marketingFee).add(_devFee).add(_buybackFee)).div(100);
    }
    ...
    {
        uint256 allFee = _liquidityFee.add(_marketingFee).add(_devFee).add(_buybackFee);
        ...
        marketingWallet.transfer(newBalance.div(allFee).mul(_marketingFee)); //Marketing Wallet to recive BNB
        devWallet.transfer(newBalance.div(allFee).mul(_devFee)); //dev Wallet to recieve BNB
        buybackWallet.transfer(newBalance.div(allFee).mul(_buybackFee)); //BuyBack Wallet to recieve BNB
    }
