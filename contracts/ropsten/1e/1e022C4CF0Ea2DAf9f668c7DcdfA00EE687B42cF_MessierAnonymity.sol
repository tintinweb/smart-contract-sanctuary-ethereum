...
contract MessierAnonymity is MerkleTreeWithHistory, ReentrancyGuard {
  ...
  function calcAccumulateMSS() internal view returns (uint256) {
    uint256 reward = block.number.sub(lastRewardBlock).mul(rewardPerBlock);
    uint256 remaining = cycToken.balanceOf(address(this)).sub(accumulateMSS);
    if (remaining < reward) {
      reward = remaining;
    }
    return accumulateMSS.add(reward);
  }
  ...
  function updateBlockReward() public {
    ...
    if (rewardPerBlock != 0) {
      accumulateMSS = calcAccumulateMSS();
    }
  ...
  function cycDenomination() public view returns (uint256) {
    ...
    uint256 blockNumber = block.number;
    uint256 accCYC = accumulateMSS;
    if (blockNumber > lastRewardBlock && rewardPerBlock > 0) {
      accCYC = calcAccumulateMSS();
    }
  ...
  */
  function withdraw(bytes calldata _proof, bytes32 _root, bytes32 _nullifierHash, address payable _recipient, address payable _relayer, uint256 _relayerFee, uint256 _refund) external payable nonReentrant {
    ...
    uint256 cycDeno = accumulateMSS.div(numOfShares);
    if (cycDeno > 0) {
      accumulateMSS -= cycDeno;
