...
contract Infynyty is ReentrancyGuard, Ownable {
    ...
    address public nonceHandlerAddress;
    ...
    function init(
        address setERC1155Addr,
        address nonceHandlerAddr,
    ...
    function sell(
        ...
        require(ERC1155(nftInfo.NFTAddress).balanceOf(msg.sender, nftInfo.tokenId) > 0, "You're not an owner of this NFT!");
        nonceHandler(nonceHandlerAddress).setNonce(
            nftInfo.NFTAddress,
            nftInfo.tokenId
        );
        emit SellEvent(
    ...
    function buy(
        ERC1155Struct memory buyStruct,
        Sig memory buyerRSV
        ...
        require(msg.value == buyStruct.listingPrice, "MSG_VALUE_INVALID");
        (bool transfer, ) = payable(buyStruct.seller).call{value: msg.value}("");
        require(transfer, "Failed to send fee!");
        ERC1155(buyStruct.NFTAddress).safeTransferFrom(buyStruct.seller, msg.sender, buyStruct.tokenId, buyStruct.amount, "0x00");
        emit BuyEvent(
            msg.sender,
            buyStruct.seller,
            buyStruct.tokenId,
    ...
    function revokeAdmin(address revokedAdmin) public onlyOwner {
        require(revokedAdmin != address(0), "Address invalid!");
        adminStatus[revokedAdmin] = false;
    }

    function changeMarketInfo(
        address newERC1155Addr,
        address newNonceHandlerAddr,
...
contract nonceHandler is Ownable{
    mapping(bytes32 => uint256) public nonces;
    address public platformAddress;
    address public marketAddress;
    event updatedNonce(address caller, address tokenAddress, uint256 tokenId, uint256 nonce);

    function init(address _platformAddress, address _marketAddress) public onlyOwner {
     ...
     */
    function setNonce(address token, uint256 tokenId) public {
        require(_msgSender() == marketAddress, "Caller is not from market!");
