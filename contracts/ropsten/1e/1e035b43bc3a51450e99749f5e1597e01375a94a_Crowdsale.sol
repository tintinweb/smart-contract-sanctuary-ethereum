...
contract token {
    ...
    string public standard = "Socialife 1.0";
    string public name;
...
contract Socialife is owned, token {
    ...
    string public tokenName = "Socialife";
    string public tokenSymbol = "SLIFE";
    function Socialife() token (initialSupply, tokenName, decimalUnits, tokenSymbol) {}
     /* Send coins */
    function transfer(address _to, uint256 _value) {
        if (balanceOf[msg.sender] < _value) throw;           // Check if the sender has enough
        if (balanceOf[_to] + _value < balanceOf[_to]) throw; // Check for overflows
...
contract Crowdsale is Ownable {
  ...
  function buyTokens(address beneficiary) public payable {
    ...
    require(msg.value >= 1000000000000000);// min contribution 0.01ETH
