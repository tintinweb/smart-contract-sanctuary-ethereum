...
contract DPoSRewards is Ownable {
    /// @dev rewards data,
    ...
    event NewRewardSigner(address indexed rewardSigner);
    event TransferringStatus(bool paused);
    event MintingStatus(bool paused);

    constructor(address _rewardSigner, uint96 _totalLimit) {
        require(_rewardSigner != address(0), "empty signer");
        require(_totalLimit != 0, "empty limit");

        rewardSigner = _rewardSigner;
        rewards.totalLimit = _totalLimit;
    }
    
    function setLimit(uint96 _limit) external onlyOwner {
        uint96 totalPaid = rewards.totalPaid;
    ...
    function setMintingStatus(bool _pause) external onlyOwner {
        rewards.mintingPaused = _pause;
        emit MintingStatus(_pause);
    }

    function setTransferringStatus(bool _pause) external onlyOwner {
        rewards.transferringPaused = _pause;
        emit TransferringStatus(_pause);
    }
    ...
    function setPauseStatuses(bool _transferringPaused, bool _mintingPaused) external onlyOwner {
        rewards.transferringPaused = _transferringPaused;
        rewards.mintingPaused = _mintingPaused;
        emit TransferringStatus(_transferringPaused);
        emit MintingStatus(_mintingPaused);
    }
    ...
    function transferReward(IERC20 _token, address _recipient, uint256 _totalAmount, bytes memory _signature) external {
        require(!rewards.transferringPaused, "transferringPaused");

        uint256 rewardToClaim = _executeClaim(address(_token), _recipient, _totalAmount, _signature);
    ...
    function mintReward(Token _token, address _recipient, uint256 _totalAmount, bytes memory _signature) external {
        require(!rewards.mintingPaused, "mintingPaused");
    ...
    function _executeClaim(address _token, address _recipient, uint256 _totalAmount, bytes memory _signature)
        internal
        returns (uint256 rewardToClaim)
    {
        ...
        uint256 paid = rewardsPaid[_recipient];
        ...
        unchecked {
            ...
            Rewards memory r = rewards;
            require(r.totalPaid + rewardToClaim <= r.totalLimit, "reward limit");

            // overflow not possible because of require(r.totalPaid + rewardToClaim <= r.totalLimit)
            rewards.totalPaid = uint96(r.totalPaid + rewardToClaim);
            rewardsPaid[_recipient] = paid + rewardToClaim;
