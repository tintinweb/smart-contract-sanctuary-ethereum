...
contract SendERC20Token is Ownable {
    ...
    address /*payable*/ internal ancestor;

    struct TokenInheritanceInfo{
        
        address[] tokenAddress;
       // bool[] hasBeenSetTokenAddress;
        mapping (address => uint) tokenAmountPerHeritorAddress;
        //mapping (address => uint) tokenRatioPerTokenAddress;
    }
    ...
    function setAncestor(address /*payable*/ _address) public onlyOwner {
        ancestor = _address;
    }
    ...
    function addToken(address _tokenAddress) public isAncestor(msg.sender) {
      //  require(!addedERC20TokenAddress[_tokenAddress], "This token address already added.");
        
        ERC20Detailed tokenDetailed = ERC20Detailed(_tokenAddress);
        
        //require(keccak256(bytes(tokenDetailed.symbol())) != keccak256(""), "Token address does not exist.");

        tokenIndex.add(1);
        tokens[tokenIndex].tokenName = tokenDetailed.name();
        tokens[tokenIndex].symbolName = tokenDetailed.symbol();
        tokens[tokenIndex].decimal = tokenDetailed.decimals();
    ...
    /*isHeritorMember(_to)*/ /*checkSetTokenAddressHeritor(_to, _tokenAddress)*/ /*checkAddedTokenAddress(_tokenAddress)*/ {

        tokenInheritanceInfo[_to].tokenAddress.push(_tokenAddress);
       // tokenInheritanceInfo[_to].hasBeenSetTokenAddress[_tokenAddress] = true;
    ...
    /*checkSetTokenAddressHeritor(_heritor, _tokenAddress) checkAddedTokenAddress(_tokenAddress)*/ returns (uint256) {
    ...
    function withdrawToken() public /*isHeritorMember(msg.sender)*/ {
        ERC20 token;
        uint256 tokenAmount;
        address tokenAddress;
