...
library WethUtils {
    ...
    // test WETH
    IWETH public constant weth = IWETH(0xc778417E063141139Fce010982780140Aa0cD5Ab); // WETH
...
// File contracts/UniswapV2Handler.sol
...
contract UniswapV2Handler is Ownable, ReentrancyGuard {
    ...
    constructor(address _swapRouter) {
        require(
            ...
            "UniswapV2Handler::constructor: _swapRouter invalid address"
        );
    ...
    function transfer(
        ...
        Utils.transfer(_token, _to, _amount);
        emit Transfered(_amount, _to, _token);
    }
    ...
    function execute(
        ...
        address _outputToken = _swapPath[_swapPath.length - 1];
        address _inputToken = _swapPath[0];
        uint256 preOutputAmount = Utils.balanceOf(_outputToken, address(this));
        IERC20(_inputToken).safeApprove(address(swapRouter), 0);
        IERC20(_inputToken).safeApprove(address(swapRouter), _amountIn);
        ...
        require(amountOut > _amountIn, "UniswapV2Handler::execute: amount out not enough");
        emit Swaped(_amountIn, amountOut, _outputToken);
    }

    /// @notice receive ETH
    receive() external payable {}

    event Swaped(uint256 amountIn, uint256 amountOut, address outputToken);

    event Transfered(uint256 amount, address indexed to, address token);
}