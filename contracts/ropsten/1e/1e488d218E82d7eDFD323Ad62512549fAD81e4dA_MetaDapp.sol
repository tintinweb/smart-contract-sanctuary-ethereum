...
contract MetaDapp {
    ...
    function getSecureAddPercent() public view isOwner returns (uint256) {
        return secureAddPercent;
    }

    function __percentValue(uint256 _amount) public view returns (uint256) {
        return (_amount * secureAddPercent) / 100;
    }
    ...
    function transferTokens(address _owner, uint _price, address _buyer) private {
        require(_price <= token.balanceOf(_buyer), "Insufficient tokens to make transfer");
        require(token.allowance(_buyer, address(this)) >= _price, "Insufficient allowance to make reserve");
    ...
    function updateUserContract(string memory contact, string memory name) public {
        require(msg.sender != noOne);
    ...
    function withdraw(address to, uint amount) external isOwner{
