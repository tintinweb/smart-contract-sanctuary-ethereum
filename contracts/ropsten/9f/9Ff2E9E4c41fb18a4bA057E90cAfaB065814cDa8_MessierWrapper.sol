...
contract MessierWrapper {
  ...
  function purchaseCost(IMessierV2dot2 _messier) external view returns (uint256) {
    uint256 cycAmount = _messier.cycDenomination().add(_messier.anonymityFee());
    if (cycAmount == 0) {
        return 0;
    }
    address[] memory paths = new address[](2);
    paths[0] = wrappedCoin;
    paths[1] = address(_messier.cycToken());
    uint256[] memory amounts = router.getAmountsIn(cycAmount, paths);
    return amounts[0];
  }

  function deposit(IMessierV2dot2 _messier, bytes32 _commitment, bool _buyCYC) external payable {
    ...
    if (cycAmount > 0) {
      if (_buyCYC) {
        ...
        path[0] = wrappedCoin;
        path[1] = address(_messier.cycToken());
        ...
        _messier.cycToken().safeTransferFrom(msg.sender, address(this), cycAmount);
      }
    }
    bytes32 commitment;
    ...
    uint256 cycDeno;
    uint256 fee;
    (commitment, insertedIndex, blocktime, cycDeno, fee) = _messier.deposit.value(coinAmount)(_commitment);
  ...
  function whitelist(IMessierV2dot2 _messier) public {
    ...
    _messier.cycToken().safeApprove(MessierAddr, uint256(-1));
