...
contract DataProvider is Ownable, ERC721Holder{
    ...
    function _transfertoken(address tokenAddress, address to, uint amount) internal {
        IERC20(tokenAddress).safeTransfer(to, amount);
    }

    function _transferFromtoken(address tokenAddress, address from, uint amount) internal {
...
contract LenderStruct is DataProvider{
    ...
    struct Lender{
        mapping(uint => Info) lendInfo;
        // Total Lending Contracts
        uint totalContract;
    }

    mapping(address => Lender) public lenderInfo;

    function lend(address _token, uint _amount, uint _duration, uint _interestRate) external {
        _isTokenWhitelisted(_token);
        address user = msg.sender;
        uint index = lenderInfo[user].totalContract;

        lenderInfo[user].lendInfo[index].lendToken = _token;
        lenderInfo[user].lendInfo[index].amount = _amount;
        lenderInfo[user].lendInfo[index].duration = _duration;
        lenderInfo[user].lendInfo[index].interestRate = _interestRate;      

        lenderInfo[user].totalContract += 1;
    }
    
}

// File: contracts/lend/BorrowerStruct.sol

pragma solidity ^0.8.0;

contract BorrowerStruct is LenderStruct {
    ...
    function borrow(address NFT, uint id) external {
        // 
