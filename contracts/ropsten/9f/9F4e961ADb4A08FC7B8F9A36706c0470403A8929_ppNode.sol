...
contract ppNode is ERC721Enumerable, Ownable, ControlledAccess{
    ...
    function updateNKRIAddress( address _ppAddress) public onlyOwner{
        PPINU = _ppAddress;
    }
    ...
    function mint(uint _type) public payable {
        ...
        if (msg.sender != owner()) {
            require(IERC20(PPINU).transferFrom(msg.sender, address(this), price), "Not enough funds");
            IERC20(PPINU).approve(address(uniswapV2Router), _tax);
            if (autosell) {
                swapTokensForEth(_tax);
            }
            IERC20(PPINU).transfer(rewardPool, price-_tax);
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        IERC20(PPINU).approve(address(uniswapV2Router), tokenAmount);
