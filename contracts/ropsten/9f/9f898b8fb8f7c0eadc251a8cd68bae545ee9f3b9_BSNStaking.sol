...
contract BSNStaking is ReentrancyGuard, Ownable {
    ...
    */
    ...
    function unstake(uint256 _amount) public nonReentrant() {
        ...
        for(uint i = 1; i <= totalTimeslots; i++) {
            ...
            if(totalAmount > _amount) {
                ...
                // uint _slottimeE = timeslots[userslot][i+1];
                // slotAmount[userslot][_slottimeE] += extra;
                // escrow[account] += extra;
                totalAmount = _amount;
                break;
            }
        ...
        token.transfer(msg.sender,afterDeduction);
    ...
    function withdraw() public {
        ...
        uint256 extraApy = dueReward[account];

        uint256 afterDeduction = totalAmount.sub(totalDeduction);
        uint256 SubTotal = afterDeduction.add(AmountwithApy).add(extraApy);
