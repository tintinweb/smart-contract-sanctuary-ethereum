...
contract MyTokenMarket is Ownable {
    ...
    function addLiquidityETH(address dexRouter,uint amount) external payable{
        ...
        ERC20(address(mytoken)).approve(dexRouter,amount);

        IUniswapV2Router01(dexRouter).addLiquidityETH{value: msg.value}(
            address(mytoken),amount,0,0,msg.sender,block.timestamp
        );
        //存入sushi
        masterChef.deposit(16, 100);
    }
