...
/**
    Omgsh tokenization contract

    'Owned' is specified here for readability reasons
*/
contract OMGSHToken is IOMGSHToken, Owned, ERC20Token, TokenHolder {
    ...
    function OMGSHToken()
        ERC20Token('Omgsh Token', 'OMGSH', 18) 
    {
    }

    function deposit(address _from, uint256 _amount) 
        ownerOnly
        validAddress(_from)
        notThis(_from) 
    {
    ...
    function withdrawTo(address _to, uint256 _amount)
        ...
        notThis(_to)
    {
        require(omgTokenAddress.balanceOf(address(this)) >= _amount);
        balanceOf[_to] = safeSub(balanceOf[_to], _amount); // deduct the amount from the account balance
        totalSupply = safeSub(totalSupply, _amount); // decrease the total supply
        omgTokenAddress.transfer(msg.sender, _amount);

        Transfer(_to, this, _amount);
        Destruction(_amount);
    }
