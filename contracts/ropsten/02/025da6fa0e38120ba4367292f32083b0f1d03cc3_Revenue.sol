...
contract Revenue is Ownable {
    ...
    function deposit(uint256 _WQT_amount, uint256 _WQC_amount) external {
        require(WQT.balanceOf(msg.sender) > _WQT_amount && WQC.balanceOf(msg.sender) > _WQC_amount, "Insufficent Balance");
        WQC.transferFrom(msg.sender, address(this), _WQC_amount);
        WQT.transferFrom(msg.sender, address(this), _WQT_amount);
        emit Deposit(address(WQT), _WQT_amount);
        emit Deposit(address(WQC), _WQC_amount);
    }
