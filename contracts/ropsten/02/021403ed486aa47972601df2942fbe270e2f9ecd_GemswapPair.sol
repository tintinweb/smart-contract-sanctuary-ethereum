...
contract GemswapPair is GemswapERC20 {
    ...
    function _update(
        ...
        unchecked {
            ...
            //            if (timeElapsed > 0 && (_baseReserves + _quoteReserves != 0) {
    ...
    function burn(address to) external nonReentrant returns (uint256 baseAmount, uint256 quoteAmount) {
        ...
        _update(GemswapERC20(_base).balanceOf(address(this)), GemswapERC20(_quote).balanceOf(address(this)), _baseReserves, _quoteReserves);
    ...
    function swap(
        ...
        address _base = token0;
        address _quote = token1;
        // 1) optimistically transfer "to" token0 tokens
        // 2) optimistically transfer "to" token1 tokens
        // 3) if data length is greater than 0, initiate flashswap
        // 4) store token0 token balance of contract in memory
        // 5) store token1 token balance of contract in memory
