...
contract WorkspaceStorage_v1 {
    ...
    function addSeat(uint256 _spaceID, uint256 _numSeats) external onlyManager(_spaceID) returns(bool) {
        spaces[_spaceID].numSeats += _numSeats;
        return true;
    }

    function removeSeat(uint256 _spaceID, uint256 _numSeats) external onlyManager(_spaceID) returns(bool) {
        Space storage _space = spaces[_spaceID];
        assert(_space.numSeats - _numSeats < _space.numSeats);
        _space.numSeats -= _numSeats;
        return true;
    }


    function addManager(uint256 _spaceID, address _manager) external onlyowner(_spaceID) returns(bool){
        spaces[_spaceID].managers.push(_manager);
        return true;
    }

    function removeManager(uint256 _spaceID, address _manager) external onlyowner(_spaceID) returns(bool){
        Space storage _space = spaces[_spaceID];
        for(uint i = 0; i < _space.managers.length; i++ ){
            if(_space.managers[i] == _manager){
                _space.managers[i]=_space.managers[_space.managers.length-1];
                _space.managers.pop();
            }
        }
        return true;
    ...
    function toggleAvailable(uint256 _spaceID) external onlyManager(_spaceID) returns(bool){
        spaces[_spaceID].available = !spaces[_spaceID].available;
        return true;
    }
    
    //function update() external onlyowner(_spaceID) returns(bool){
        // updates the checkins and checkouts
   //     return true;
   // }

    function setDataURI(uint256 _spaceID, string calldata _uri) external onlyowner(_spaceID) returns(bool){
        spaces[_spaceID].dataURI = _uri;
        return true;
    }
