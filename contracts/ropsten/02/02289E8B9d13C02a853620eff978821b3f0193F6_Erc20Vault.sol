...
contract Owned {
    ...
    modifier onlyPNetwork() {
        require(msg.sender == owner,"Caller must be PNETWORK address!");
        _;
    }

    // function transferOwnership(address _newOwner) public onlyPNetwork {
    //     newOwner = _newOwner;
    // }
...
contract Oracled is Owned {
    ...
    function regOracle(address _newOracle) public onlyPNetwork {
        require(!oracles[_newOracle], "Oracle is already registered");
        oraclesArr.push(_newOracle);
        oracles[_newOracle] = true;
    }

    function unregOracle(address _remOracle) public onlyPNetwork {
...
// Claim
// ----------------------------------------------------------------------------
contract claimToken is Verify, Oracled {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint8 public threshold = 3;
    uint8 public thisChainId = 1;
        ...
        returns (TeleportData memory)
    {
        ...
        assembly {
            ...
            tokenAddress := mload(add(add(sigData, 0x14), 58))
        }
        ...
        td.symbolRaw = symbolRaw - chainId;
        td.chainId = chainId;
        ...
        // require(thisChainId == td.chainId, string(abi.encodePacked("Invalid Chain ID", td.chainId)));
    ...
    function claimAction(bytes memory sigData, bytes[] memory signatures)
        internal
        returns (TeleportData memory)
    {
        ...
        // verify signatures
        require(sigData.length == 78, "Signature data is the wrong size");
        require(
