...
contract Poll {
    ...
    // TODO: convert to string here
    function viewPollDetails(uint pollId, bool isOptDesc) 
        participantCheck()
        pollCheck(pollId)
        external view
    ...
    {   
        bytes memory output;
        if (!isOptDesc) {
            for (uint256 i = 0; i < polls[pollId].choseFrom.length; i++) {
                output = abi.encodePacked(output, polls[pollId].choseFrom[i]);
            }
        } else {
            for (uint256 i = 0; i < polls[pollId].optionDesc.length; i++) {
                output = abi.encodePacked(output, polls[pollId].optionDesc[i]);
            }
        }
