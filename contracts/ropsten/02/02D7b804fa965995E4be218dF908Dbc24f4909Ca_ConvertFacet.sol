...
contract ConvertFacet is ConvertSilo {
    ...
    // из бункера продаются bean токены и вкладываются в ликвидность
    // удаляются все stalks, seeds
    // и начисляются LP токены пользователю в бункер
    ...
    // пользователь из бункера продает lp токен и получает какое-то количество bean обратно в бункер + seeds and stalks
