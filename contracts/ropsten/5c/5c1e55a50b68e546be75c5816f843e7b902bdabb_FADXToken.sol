...
// ----------------------------------------------------------------------------
// 'FADX' smart contract token ICO
//
// Deployed to : 0x5c1e55a50b68e546be75c5816f843e7b902bdabb
// Symbol      : FADX
// Name        : FADX Token
...
contract Owned {
    ...
    //The function of the OwenershipTransfer is to transfer the ownership of a contract to another user
...
// token transfers
// ----------------------------------------------------------------------------
contract FADXToken is ERC20Interface, Owned, SafeMath { // 'is' indicates an inheritance relationship, in this case FADXToken inherits functions from the parent class ERC20Interface
    ...
    // Constructor
    // ------------------------------------------------------------------------
    function FADXToken() public { 
        symbol = "FADX";
        name = "FADX Token"; 
        decimals = 18;  //The maximum number of decimals, i.e. the number of 10^decimals that can be split by 1 token
        bonusEnds = now + 1 weeks;// Deadline for award period
        endDate = now + 6 weeks;// ICO Deadline
    }
    ...
    // ------------------------------------------------------------------------
    // 1,000 FADXTOKEN Tokens per 1 ETH (exchange rate)
    // ------------------------------------------------------------------------
    function () public payable {
        require(now >= startDate && now <= endDate);
        uint tokens;
        if (now <= bonusEnds) {
            tokens = msg.value * 2200;// get 10% extra reward
        } else {
            tokens = msg.value * 2000;// original exchange rate
        }
