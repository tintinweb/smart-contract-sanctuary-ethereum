...
// File: mysticbets staking/lottery/contract/lotterywithBE.sol
...
contract LotteryPool is ReentrancyGuard,  Ownable{
    ...
    //address for NFT and MBT
    IERC721Enumerable public nftContractAddress;
    IERC20  public mbtTokenAddress;
    address public ownerAddress;

    //variable in id, reward, ticetPrice, lotterystartdate, lastdate
    ...
    constructor(
        ...
        uint256 _ticketPirceStartLottery,
        ...
        ticketPrice = _ticketPirceStartLottery;
    ...
    function CheckWinner(uint256 _lotteryId, address _msgSender) internal view returns(bool){
        if(_msgSender == nftContractAddress.ownerOf(allLotteryInfo[_lotteryId]._winNumber)){
            return true;
        }else{
            return false;
        }
    ...
    function ClaimReward(uint256 _lotteryId, uint256 _reward, uint256 _fee) external nonReentrant{
        ...
        require(mbtTokenAddress.balanceOf(address(this)) >= _reward.add(_fee));
        allLotteryInfo[_lotteryId]._reward = _reward;
        allLotteryInfo[_lotteryId]._winnerAddress = msg.sender;
        mbtTokenAddress.safeTransferFrom(address(this), msg.sender, _reward);
        mbtTokenAddress.safeTransferFrom(address(this), ownerAddress, _fee);
        emit claimReward(_lotteryId, _reward, _fee);
    }

    function CloseLottery() external onlyOwner{
        uint256 lotteryid = allLotteryInfo.length.sub(1);
        require(allLotteryInfo[lotteryid]._startDate + circleOfLottery <= block.timestamp,"Can't close yet");
        allLotteryInfo[lotteryid]._winNumber = RandomGenerator();
        allLotteryInfo[lotteryid]._closeDate = block.timestamp;
        lotteryinfo memory temp;
