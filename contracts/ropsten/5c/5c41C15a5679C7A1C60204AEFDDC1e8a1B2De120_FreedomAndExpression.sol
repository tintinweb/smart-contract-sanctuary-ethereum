...
contract FreedomAndExpression is ERC721, Pausable, RaribleRoyaltyV2, Ownable {
  ...
  uint256 private constant UNIQUE_PRICE = 0.0007 ether; //g1.62 ether;
  uint256 private constant EDITION_PRICE = 0.00007 ether; //g0.16 ether;

  uint256 private constant EDITION_SUPPLY = 20;
  uint256 private constant UNIQUE_COUNT = 5;
  uint256 private constant MAX_SUPPLY = 105;

  address private withdrawWallet = 0xFb2bFc75915F20a7493A93098100f027601CAf39; //g0x46AdB53F4DAc76422c618304Eb39E7b5f2d26bae
  address private nftHolder = 0xF7daf144B398dB4C6DB0D5b2922BC501F92f86F4; //g0xfc39a5F7359F036236410B4BAA3824402a1c2c25
  address private royaltyReceiver = 0x6A4c66e953087d8357b6c09FCbAF84b98D6Fe692; //g0x65Ce391F03833434B1D50A2E9bD5401790e66e8D
  address private artist = 0x9Eb3a30117810d5a36568714EB5350480942f644; //g0xeA3f5dC4BFE8C575173F8A5cB04C72c527c17469
  ...
  //  EDITION_HURRICANE = 4;
  mapping(uint256 => uint256) private categorySupply;
  ...
  function _getNextId(uint256 categoryId) internal view returns(uint256) {
    return UNIQUE_COUNT + categoryId * 20 + categorySupply[categoryId] + 1;
  }
  ...
  function mint(uint256 categoryId) public payable whenNotPaused {
    require(categoryId < UNIQUE_COUNT, "That category doesn't exist");
    require(categorySupply[categoryId] < EDITION_SUPPLY, "That category was minted out");
    require(msg.value == EDITION_PRICE, "You must send the right amount");
    uint256 tokenId = _getNextId(categoryId);
    categorySupply[categoryId]++;
  ...
  function remainingSupply(uint256 categoryId) public view returns (uint256) {
    require(categoryId < 5, "That category doesn't exist");
    return EDITION_SUPPLY - categorySupply[categoryId];
  }
