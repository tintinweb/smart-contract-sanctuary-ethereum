...
contract DoubleStaking is Ownable {
    using SafeMath for uint256;

    uint256 BIGNUMBER = 10**18;
    uint256 DECIMAL = 10**3;
    ...
    function stake(uint256 _amount, address _tokenAddr)
        external
        ...
        returns (bool)
    {
        ...
        if (StakeMap[_tokenAddr][msg.sender].amount == 0) {
            ...
            userCummRewardPerStake[_tokenAddr][
                msg.sender
            ] = tokenCummRewardPerStake[_tokenAddr];
        } else {
            claim(_tokenAddr, msg.sender);
            StakeMap[_tokenAddr][msg.sender].amount = StakeMap[_tokenAddr][
                msg.sender
            ].amount.add(_amount);
        }
        tokenTotalStaked[_tokenAddr] = tokenTotalStaked[_tokenAddr].add(
            _amount
        );
        return true;
    }

    function distribute(uint256 _reward, address _tokenAddr)
        external
        isValidToken(_tokenAddr)
        returns (bool)
    {
        require(tokenTotalStaked[_tokenAddr] != 0);
        uint256 reward = _reward.mul(BIGNUMBER); //simulate floating point operations
        uint256 rewardAddedPerToken = reward / tokenTotalStaked[_tokenAddr];
        tokenCummRewardPerStake[_tokenAddr] = tokenCummRewardPerStake[
            _tokenAddr
        ].add(rewardAddedPerToken);
        return true;
    }
    ...
    function claim(address _tokenAddr, address _receiver)
        public
        isValidToken(_tokenAddr)
        returns (uint256)
    {
        uint256 stakedAmount = StakeMap[_tokenAddr][msg.sender].amount;
        uint256 amountOwedPerToken = tokenCummRewardPerStake[_tokenAddr].sub(
            userCummRewardPerStake[_tokenAddr][msg.sender]
        );
    ...
    function initWithdraw(address _tokenAddr)
        external
        isValidToken(_tokenAddr)
        returns (bool)
    {
        ...
        StakeMap[_tokenAddr][msg.sender].releaseDate =
            block.timestamp +
            4 weeks;
        return true;
    }

    function finalizeWithdraw(uint256 _amount, address _tokenAddr)
        external
        isValidToken(_tokenAddr)
        returns (bool)
    {
        ...
        tokenTotalStaked[_tokenAddr] = tokenTotalStaked[_tokenAddr].sub(
            _amount
        );
    ...
    function releaseStake(
        ...
        address _dest
    ) external isMediator(_tokenAddr) isValidToken(_tokenAddr) returns (bool) {
