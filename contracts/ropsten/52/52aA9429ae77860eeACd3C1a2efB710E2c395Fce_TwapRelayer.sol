...
contract TwapRelayer is ITwapRelayer {
    ...
    function sell(Orders.SellParams calldata sellParams) external payable override lock returns (uint256 orderId) {
        ...
        (address tokenIn, address tokenOut) = regulateEthAddress(
            sellParams.tokenIn,
            sellParams.tokenOut,
        ...
        uint256 gasPrepay = calculatePrepay(msg.value, tokenIn, _amountIn, sellParams.wrapUnwrap);
        ...
        emit Swap(orderId, msg.sender, tokenIn, tokenOut, _amountIn, _amountOut, fee, sellParams.to);
    ...
    function buy(Orders.BuyParams calldata buyParams) external payable override lock returns (uint256 orderId) {
        ...
        uint256 gasPrepay = calculatePrepay(msg.value, tokenIn, _amountIn, buyParams.wrapUnwrap);
        uint256 balanceBefore = address(this).balance;
        ...
        emit Swap(orderId, msg.sender, tokenIn, tokenOut, _amountIn, _amountOut, fee, buyParams.to);
    ...
    function calculatePrepay(
        ...
        uint256 amountIn,
        bool wrap
    ) internal returns (uint256 gasPrepay) {
        gasPrepay = _gasPrepay;
        if (tokenIn == weth && wrap) {
            gasPrepay = gasPrepay.sub(amountIn, 'TR1E');
        }
        uint256 gas = ((getGasPrice() * GAS_PRICE_MULTIPLIER) / GAS_PRICE_MULTIPLIER_DENOMINATOR) *
            DELAY_EXECUTE_GAS_LIMIT;
        if (gasPrepay < gas) {
            gasPrepay += gas;
        }
    }
    ...
    function regulateEthAddress(
        ...
        tokenIn = _tokenIn;
        tokenOut = _tokenOut;
        if (wrap && tokenIn == ETH_ADDRESS) {
            tokenIn = weth;
        } else if (wrap && tokenOut == ETH_ADDRESS) {
            tokenOut = weth;
        }
    }
