...
contract DFT is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public buybackWallet = payable(0x13d50FFfd5AB94462135F7A589C539FF7C1BeC59); // buyback wallet Address
    address payable public marketingWallet = payable(0x446F1A2c730Daf2631b9d1d13b18f44308B30142); // marketing Address
    
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public burnAccount = 0x0000000000000000000000000000000000000001;
    ...
    string private _name = "Dragon Fire Token";
    ...
    uint256 public _maxTxAmount = 100000000000000 * 10**9; //0.1%
    uint256 private minimumTokensBeforeSwap = 10000000000000 * 10**9; //0.01%
    ...
    {
        uint256 totalFee = _liquidityFee.add(_marketingFee).add(_buybackFee);
        ...
        transferToAddressETH(buybackWallet, ethForCharity);
    ...
    {
        if(_buybackFee==0 && _marketingFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tCharity = tAmount.div(100).mul(_buybackFee.add(_marketingFee));
    ...
    function setFee(uint256 taxFee, uint256 buybackFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner() {
        _taxFee = taxFee;
    ...
    function setbuybackWallet(address _devAddress) external onlyOwner() 
    {
        buybackWallet = payable(_devAddress);
    }
