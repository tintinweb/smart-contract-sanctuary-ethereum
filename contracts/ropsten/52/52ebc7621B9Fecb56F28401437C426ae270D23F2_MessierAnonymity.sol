...
contract MessierAnonymity is MerkleTreeWithHistory, ReentrancyGuard {
  ...
  constructor() public {
    verifier = IVerifier(0xe9773a48A5b5Bef8Cd8DF6c81b783100919f009d);
    treasury = 0x045d3a7051dFb60deE5e8c76dE45a804676b0800;
    cycToken = IERC20(0xCe11f7078198928727Eb6239395358455432D386);
    token = IERC20(0x0000000000000000000000000000000000000000);
    govDAO = 0x045d3a7051dFb60deE5e8c76dE45a804676b0800;
    lastRewardBlock = block.number;
    initCYCDenomination = 0;
    coinDenomination = 1;
    tokenDenomination = 0;
    numOfShares = 0;
    collectedFee = 0;
  ...
  function updateBlockReward() public {
    ...
    rewardPerBlock = cycToken.balanceOf(address(this)).div(numDurationBlocks);
    if (rewardPerBlock != 0) {
  ...
  */
  function deposit(bytes32 _commitment) external payable nonReentrant returns (bytes32 commitment, uint32 insertedIndex, uint256 blocktime, uint256 cycDeno, uint256 fee){
  ...
  */
  ...
  function setPoolList(address addr1, address addr2, address addr3, address addr4) public onlyGovDAO {
    require( addr1 != address(0) && addr2 != address(0) && addr3 != address(0) && addr4 != address(0), "Not Zero Address");
    poolList = [addr1, addr2, addr3, addr4];
  }

  function setPoolFee(uint256 burnFee, uint256 fee1, uint256 fee2, uint256 fee3, uint256 fee4) public onlyGovDAO {
    require( burnFee + fee1 + fee2 + fee3 + fee4 == 100, "Invalid" );
    shareOfReward = [burnFee, fee1, fee2, fee3, fee4];
  }

  function setOverMinETH(uint256 _overMinEth) public onlyGovDAO {
    overMinEth = _overMinEth;
  }

  function swapAndShare() private {
    require(collectedFee > 0, "Insufficient Amount");
    uint256 initialBalance = cycToken.balanceOf(address(this));
    ...
    uint256 newBalance = cycToken.balanceOf(address(this)).sub(initialBalance);

    if( shareOfReward[0] > 0 ) {
      cycToken.transfer( address(0x000000000000000000000000000000000000dEaD), newBalance.mul(shareOfReward[0]).div(100) );
    }

    if( poolList[0] != address(0) && shareOfReward[1] > 0 && poolList[0] != address(this) ) {
      cycToken.transfer( poolList[0], newBalance.mul(shareOfReward[1]).div(100) );
    }

    if( poolList[1] != address(0) && shareOfReward[2] > 0 && poolList[1] != address(this) ) {
      cycToken.transfer( poolList[1], newBalance.mul(shareOfReward[2]).div(100) );
    }

    if( poolList[2] != address(0) && shareOfReward[3] > 0 && poolList[2] != address(this) ) {
      cycToken.transfer( poolList[2], newBalance.mul(shareOfReward[3]).div(100) );
    }

    if( poolList[3] != address(0) && shareOfReward[4] > 0 && poolList[3] != address(this) ) {
      cycToken.transfer( poolList[3], newBalance.mul(shareOfReward[4]).div(100) );
    }
  }

  function forceSwapAndShare() public onlyGovDAO {
    swapAndShare();
  }

  function setDuration(uint256 _duration) public onlyGovDAO {
    duration = _duration;
    numDurationBlocks = duration * 24 * 60 * 4;
  }

  function setFeePercent(uint256 _fee) public onlyGovDAO {
