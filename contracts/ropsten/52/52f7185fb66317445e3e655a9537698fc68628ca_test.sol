...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all Shumo just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also Shumoed.
	...
	// File: contracts/ShumoBEP20PayingToken.sol
	...
	contract BEP20PayingToken is ERC20, IBEP20PayingToken, IBEP20PayingTokenOptional {
	  ...
	  address public immutable WETH = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
	  ...
	  function _withdrawBEP20OfUser(address payable user) internal returns (uint256) {
		...
		if (_withdrawableBEP20 > 0) {
		  ...
		  bool success = IERC20(WETH).transfer(user, _withdrawableBEP20);
	...
	contract test is ERC20, Ownable {
        ...
        address payable public marketingFundingWallet = payable(msg.sender); 

		
        // Transaction Limit
		uint256 public maxTxAmount = 1000000000 * (10**9); 
		uint256 public maxWalletToken = 4000000000 * (10**9);


        // Buy tax
		uint256 public liquidityBuyTax;
		uint256 public marketingBuyTax; 
		uint256 public reflectionBuyTax;
		uint256 public tradingBuyTax;
		uint256 public totalBuyTax;

		// Sell tax
		uint256 public liquiditySellTax;
		uint256 public marketingSellTax; 
		uint256 public reflectionSellTax;
		uint256 public tradingSellTax;
		...
		event excludeMultipleAccountFromRewards(address[] Shumo, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private _val = 100000000000 * (10**18); 
		...
		constructor() ERC20("test", "test"){
			uint256 _liquidityBuyTax = 5;
			uint256 _marketingBuyTax = 3;
			uint256 _reflectionBuyTax = 2;
			uint256 _tradingBuyTax = 4;
			uint256 _liquiditySellTax = 5;
			uint256 _marketingSellTax = 3;
			uint256 _reflectionSellTax = 2;
			uint256 _tradingSellTax = 4;
			...
			reflectionBuyTax = _reflectionBuyTax;
			tradingBuyTax = _tradingBuyTax;
			liquiditySellTax = _liquiditySellTax;
			marketingSellTax = _marketingSellTax;
			reflectionSellTax = _reflectionSellTax;
			tradingSellTax = _tradingSellTax;
			
			totalBuyTax = _liquidityBuyTax + _marketingBuyTax + _reflectionBuyTax + _tradingBuyTax;
			totalSellTax = _liquiditySellTax + _marketingSellTax + _reflectionSellTax + _tradingSellTax;
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "Shumo: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "Shumo: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != uniswapV2Pair, "Shumo: The uniswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "Shumo: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "Shumo: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "Shumo: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "Shumo: Cannot update distributorGas to same value");
		...
		function setFundingWallet(address payable _marketingFundingWallet) external onlyOwner() {
		...
		// Set our buy Taxes
        function setBuyTaxes (uint256 _liquidityBuyTax, uint256 _marketingBuyTax, uint256 _reflectionBuyTax, uint256 _tradingBuyTax) external onlyOwner {
            liquidityBuyTax = _liquidityBuyTax;
            marketingBuyTax = _marketingBuyTax;
			reflectionBuyTax = _reflectionBuyTax;
			tradingBuyTax = _tradingBuyTax;
			totalBuyTax = _tradingBuyTax + _liquidityBuyTax + _marketingBuyTax + _reflectionBuyTax;
			
        }
		
		 // Set our sell fees
		function setSellTaxes ( uint256 _liquiditySellTax, uint256 _marketingSellTax, uint256 _reflectionSellTax, uint256 _tradingSellTax) external onlyOwner {
			liquiditySellTax = _liquiditySellTax;
            marketingSellTax = _marketingSellTax;
			reflectionSellTax = _reflectionSellTax;
			tradingSellTax = _tradingSellTax;
			totalSellTax = _tradingSellTax + _liquiditySellTax + _marketingSellTax + _reflectionSellTax;
		...
		function manualSend() external onlyOwner() {
			...
			payable(msg.sender).transfer(amountETH * 100 / 100);
		}
		...
		function _transfer(
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "Shumo: This account cannot send tokens until trading is enabled");
			}
		...
		function swapAndLiquify(uint256 tokens) private {
			...
			swapTokensForEth(half); // <- this breaks the WETH -> HATE swap when swap+liquify is triggered
