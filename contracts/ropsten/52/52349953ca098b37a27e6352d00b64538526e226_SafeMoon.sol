...
contract SafeMoon is Context, IERC20, Ownable {
    ...
    uint256 private _reflectionTotal = (~uint256(0) - (~uint256(0) % _totalSupply));
    ...
    uint256 private numTokensSellToAddToLiquidity = 5 * (10**4) * (10**_decimals);
    ...
    event RemoveAllFee();
    event SetLiquidityFeePercent(uint256);
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _reflectionBalances[sender] = _reflectionBalances[sender].sub(rAmount);
        _reflectionBalances[recipient] = _reflectionBalances[recipient].add(rTransferAmount);   
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_reflectionBalances[_excluded[i]] > rSupply || _tokenBalances[_excluded[i]] > tSupply) return (_reflectionTotal, _totalSupply);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _reflectionBalances[address(this)] = _reflectionBalances[address(this)].add(rLiquidity);
    ...
    function removeAllFee() private {
        require(_taxFee > 0 && _liquidityFee >0, "Fees are already removed");
    ...
    function setNumTokensSellToAddToLiquidity(uint256 numTokensSellToAddToLiquidityPercent) external onlyOwner() {
        numTokensSellToAddToLiquidity = _totalSupply.mul(numTokensSellToAddToLiquidityPercent).div(
            10**2
        );
            emit SetNumTokensSellToAddLiquidity(numTokensSellToAddToLiquidityPercent);
    }
