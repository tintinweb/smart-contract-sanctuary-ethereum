...
contract Staking is Ownable, BasePool {
    ...
    modifier allowed2Stake(uint256 duration) {
        require(block.timestamp > start, "staking not started");
        require(
            block.timestamp < end,
            "staking already finished or not started"
        );
        require(end - block.timestamp > duration, "staking duration too long");
        _;
    }
    ...
    constructor(
        ...
        rewardPerSecond = MAX_REWARD / (365 days);
    }

    function startStaking(uint256 _start) public onlyOwner {
        require(
            IERC20(stakingToken).balanceOf(address(this)) >= MAX_REWARD,
    ...
    function stakeWith90Days(uint256 amount) public allowed2Stake(90 days) {
        _stakeWithDuration(msg.sender, amount, 90 days);
    }

    function stakeWith180Days(uint256 amount) public allowed2Stake(180 days) {
        _stakeWithDuration(msg.sender, amount, 180 days);
    }

    function stakeWith270Days(uint256 amount) public allowed2Stake(270 days) {
        _stakeWithDuration(msg.sender, amount, 270 days);
    }

    function stakeWith360Days(uint256 amount) public allowed2Stake(360 days) {
        _stakeWithDuration(msg.sender, amount, 360 days);
    }

    function distributeRewards() public {
        if (rewardReleased >= MAX_REWARD) {
            // staking reaches the end
            return;
        }
        ...
        uint256 reward = rewardPerSecond * (block.timestamp - lastRewardTime);
        if (rewardReleased + reward > MAX_REWARD) {
            reward = rewardReleased + reward - MAX_REWARD;
        }
    ...
    function _stakeWithDuration(
        ...
        uint256 mintAmount = amount * getMultiplier(duration);

        _mint(staker, mintAmount);
        emit Deposited(staker, amount, duration, block.timestamp);
    }

    function getMultiplier(uint256 lockDuration) public view returns (uint256) {
        return lockDuration / MIN_LOCK_DURATION;
    }
    ...
    function getDepositsOf(
        ...
        for (uint256 i = skip; i < (skip + limit).min(depositsOfLength); i++) {
            _depositsOf[i - skip] = depositsOf[account][i];
        }
    ...
    function withdraw(uint256 depositId, address receiver) external {
        ...
        require(depositId < depositsOf[sender].length, "not exist");
        Deposit memory userDeposit = depositsOf[sender][depositId];
        require(block.timestamp >= userDeposit.end, "staking not released");
        ...
        // remove Deposit
        totalDepositOf[sender] -= userDeposit.amount;
        depositsOf[sender][depositId] = depositsOf[sender][
            depositsOf[sender].length - 1
        ];
        depositsOf[sender].pop();

        // update the total staked tokens
        totalStaked -= userDeposit.amount;

        // burn pool shares
        _burn(sender, shareAmount);

        // return tokens
        IERC20(stakingToken).safeTransfer(receiver, userDeposit.amount);

        emit Withdrawn(depositId, receiver, sender, userDeposit.amount);
    }
}
...
abstract contract BasePool is ERC20, AbstractRewards {
    ...
    constructor(
        ...
        require(
            ...
            "BasePool: _stakingToken must be set"
        );
