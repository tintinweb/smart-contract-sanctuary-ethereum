    ...
    function checkForWinner() public view returns(uint16)
{
    ...
    for(uint16 i=1; i<= totalSupply() + MAPtokens; i++)
    {
        ...
        {
            uint16 traitOne = uint16(tokenMetadata[i] % 100);
            uint16 traitTwo =  uint16(tokenMetadata[i] / 100 % 100) + 84;
            uint16 traitThree =  uint16(tokenMetadata[i] / 10000 % 100) + 84;
            uint16 traitFour =  uint16(tokenMetadata[i] / 1000000% 100) + 84;
