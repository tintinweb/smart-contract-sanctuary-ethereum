...
contract Lab is Ownable, IERC721Receiver, Pausable {
    ...
    uint256 public MAX_MINTABLE_TOKENS = 45000; //Max supply of tokens
    uint256 public GENESIS_TOKENS = 10000; //Genesis tokens consist of 1k perfume and 9k flowers
    uint256 public MAX_FLOWERS = 40000;
    uint256 public MAX_PERFUMES = 9000;
    ...
    uint256 public MAX_BUILT_STORES = 500; //Stores you build/claim after burning 5 Perfumes plus 100.000$FLOR
    ...
    uint256 public chemistInsurancePrice = 25000 ether;

    event TokenStaked(address owner, uint256 tokenId, uint256 value);
    event PerfumeClaimed(uint256 tokenId, uint256 earned, bool unstaked);
    ...
    function _mintGenerationTokens(uint256 _amount, bool _stake) internal {
        ...
        for (uint256 i = 0; i < _amount; i++) {
            ...
            if (luckyNumber < 90 && MAX_FLOWERS > MINTED_FLOWERS) {
                if (_stake) {
        ...
        if (totalFlorCost > 0) flor.burn(msg.sender, totalFlorCost);
    }
    ...
    */
    ...
    @Dev 
    *Calculate the mint cost depending on the Generation 

    */
    function mintCost(uint256 tokenId) public view returns (uint256) {
        if (tokenId <= GENESIS_TOKENS) return 0;
    ...
    function stake(
        ...
        if (areFlowers) {
            for (uint256 i = 0; i < tokenIds.length; i++) {
                flower.transferFrom(owner, address(this), tokenIds[i]);
                _stakeFlower(owner, tokenIds[i]);
            }
        } else {
            for (uint256 i = 0; i < tokenIds.length; i++) {
                perfume.transferFrom(owner, address(this), tokenIds[i]);
    ...
    */
    ...
    function makePerfume(uint256[] calldata tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            flowerTraits[i] = flower.getFlowerTrait(tokenIds[i]); //gets the index of the flower trait(ranges from 0-5)
        }
    ...
    *The amount to be minted depends on the numbe rof the perfumes he owns, if he has staked 50% of the perfume supplies he gets 50% of all Flor taxed
    */
    function claimPerfumesFlor() public {
        uint256 userPerfumes = Users[msg.sender].stakedPerfumes;
        uint256 dueAmount = (florDueToPerfumHolders * userPerfumes) / 9000; //9000 is the total perfume supply
    ...
    */
    ...
    function unstakePerfumes(uint256[] calldata tokenIds) public {
        ...
        claimPerfumesFlor(); //claim taxed flowers FLOR if player didn't claim it
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                StakedPerfumes[tokenIds[i]].owner == _msgSender(),
                "NOT OWNER"
            ); // the msg sender is the owner of the staked token
    ...
    function getTraitIndex(uint256[] memory array)
        ...
        returns (uint256)
    {
        uint256[] memory freq = new uint256[](6); //6=number of traits
        uint256 id;
        uint256 maxIndex = 0;
...
contract EvaStore is ERC721Enumerable, Ownable, Pausable {
    ...
    function mint(address _to) public whenNotPaused {
        ...
        require(MINTED_STORES <= MAX_TOKENS, "Mint ended");
        MINTED_STORES++;
        _safeMint(_to, MINTED_STORES);
        generate(MINTED_STORES);
    }
...
*/
    ...
    function getFlowerTrait(uint256 tokenId) external view returns (uint256) {
