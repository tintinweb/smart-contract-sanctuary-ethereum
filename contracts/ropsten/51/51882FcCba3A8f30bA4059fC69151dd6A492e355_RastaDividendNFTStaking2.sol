...
contract RastaDividendNFTStaking2 is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    mapping(uint256 => TokenInfo) public ownTokens;
    ...
    function setOwnToken(uint256[] memory _tokenIds, uint256[] memory prices, bool exist) external onlyOwner {
        require(_tokenIds.length == prices.length, "token length not match with prices length");

        for(uint i = 0; i < _tokenIds.length; i ++) {
            ownTokens[_tokenIds[i]] = TokenInfo(exist, prices[i]);
        }
    }
    ...
    function stake(uint256 _tokenId) external {
        ...
        require(ownTokens[_tokenId].isOwn, "only can stake rasta NFT!");
        _stake(_tokenId);
    }
    ...
    function _stake(uint256 _tokenId) internal nonReentrant whenNotPaused updateReward(false) updateUserList {
        ...
        UserInfo storage user = userInfo[msg.sender];
        uint256 amount = ownTokens[_tokenId].price;
