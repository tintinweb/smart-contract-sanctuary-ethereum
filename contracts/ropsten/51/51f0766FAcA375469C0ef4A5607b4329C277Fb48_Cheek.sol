...
contract Cheek is ERC721A, Pausable, Ownable
{
    ...
    bytes32 public merkleRootTicketList;
    bytes32 public merkleRootCheekList;

    mapping (address => uint256) public mintCount;

    constructor(string memory newBaseURI, bytes32 newMerkleRootTicketList, bytes32 newMerkleRootCheekList) ERC721A("Cheek", "CHEEK")
    {
        baseURI = newBaseURI;

        merkleRootTicketList = newMerkleRootTicketList;
    ...
    function mint(uint16 numberOfTokens, uint256[] memory tickets, bytes32[] memory cheekListProof, bytes32[][] memory ticketListProofs) public payable
    {
        require(saleActive, "Sale is not active");
        require(numberOfTokens > 0, "numberOfTokens needs to be greater than 0");
        require(numberOfTokens <= getMaxPurchaseAmount(msg.sender, tickets, cheekListProof, ticketListProofs), "Can only mint MAX tokens at a time");
        require((totalSupply() + numberOfTokens) <= MAX_SUPPLY, "Purchase would exceed max supply of Cheeks");
    ...
    function getMaxPurchaseAmount(address addr, uint256[] memory tickets, bytes32[] memory cheekListProof, bytes32[][] memory ticketListProofs) public view returns (uint256)
    {
        uint256 amount = 0;

        if (MerkleProof.verify(cheekListProof, merkleRootCheekList, _getCheeksListLeaf(addr)))
        {
            amount++;
        }
        ...
        for (uint256 i = 0; i < tickets.length; i++)
        {
            ...
            {
                if (MerkleProof.verify(ticketListProofs[j], merkleRootTicketList, _getTicketsListLeaf(addr, tickets[i])))
                {
                    amount += 2;
                    break;
                }
    ...
    function _getCheeksListLeaf(address addr) pure private returns (bytes32)
    {
        return keccak256(abi.encode(addr));
    }

    function _getTicketsListLeaf(address addr, uint256 ticketId) pure private returns (bytes32)
    {
