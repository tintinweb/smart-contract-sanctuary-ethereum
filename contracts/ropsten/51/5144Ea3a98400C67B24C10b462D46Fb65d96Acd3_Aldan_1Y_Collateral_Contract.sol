...
abstract contract BaseAldanContract {
    ...
    function checkFund(string memory _key) public returns (uint){
        require(checkDataAccess(_key));
        emit LogWithKey(_key, msg.sender, "Start fund check");
        return getFundByContractIdAndSender(_key, msg.sender).amount;
    }

    function checkVersion() public returns (string memory) {
        emit LogWithKey("none", msg.sender, "Start version check");
        return _version;
    }
    ...
    function getFundByContractIdAndSender(string memory _key, address _sender) internal returns (IAldanTypes.Fund memory) {
        require(notEmptyKey(_key));
        require(_sender != address(0));

        emit LogWithKey(_key, msg.sender, "Start getFund");

        IAldanTypes.Fund[] memory funds = _myMap[_key];
        for (uint i = 0; i < funds.length; i++) {
            if (funds[i].senderPubKey == _sender) {
                emit LogWithKey(_key, msg.sender, "Fund founded");
                return funds[i];
            }
        }

        emit LogWithKey(_key, msg.sender, "No fund founded");
    ...
    function getFundIndexByContractIdAndSender(string memory _key, address _sender) internal returns (uint) {
        require(notEmptyKey(_key));
        require(_sender != address(0));
    ...
    function getFundsByContractId(string memory _key) private returns (IAldanTypes.Fund[] memory) {
        require(notEmptyKey(_key));
        emit LogWithKey(_key, msg.sender, "Start getFunds");
        return _myMap[_key];
    }

    function checkFundDateValidity(string memory _key, address _sender) private returns (bool) {
        emit LogWithKey(_key, msg.sender, "Start checkFundDateValidity");
    ...
    function checkDataAccess(string memory _key) private returns (bool) {
        emit LogWithKey(_key, msg.sender, "Start checkDataAccess");

        IAldanTypes.Fund[] memory funds = getFundsByContractId(_key);
        if (funds.length < 1) {
            emit LogWithKey(_key, msg.sender, "checkDataAccess : Access denied no funds found");
            return false;
        }
        ...
        for (uint i = 0; i < funds.length; i++) {
            if (!funds[i].active) {
                emit LogWithKey(_key, msg.sender, "checkDataAccess : Access denied no active funds found");
                return false;
            }
        }
