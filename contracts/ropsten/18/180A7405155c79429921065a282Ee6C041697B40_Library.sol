...
contract Library is Ownable {
    LibraryToken internal LIBToken;
    uint256 internal BORROW_FEE = 0;
    uint256 internal BORROW_RETURN = 0;

    mapping(bytes32 => Book) public books;
    bytes32[] public bookIds;

    constructor(address tokenAddress) {
        LIBToken = LibraryToken(tokenAddress);
    }
    ...
    modifier haveBorrowFee() {
        require(
            LIBToken.balanceOf(msg.sender) >= BORROW_FEE,
    ...
    function getBook(bytes32 id)
        ...
        bookIsNotTaken(id)
    {
        LIBToken.approve(address(this), BORROW_FEE);
        LIBToken.transferFrom(msg.sender, address(this), BORROW_FEE);
    ...
    function returnBook(bytes32 id) public bookExists(id) bookIsTaken(id) {
        LIBToken.transfer(msg.sender, BORROW_RETURN);
        Book storage book = books[id];
        book.copies += 1;
    ...
    function getCurrentBorrowStatus(bytes32 id)
        public
        view
        bookExists(id)
        returns (BorrowStatus)
    {
        return books[id].borrowStatus[msg.sender];
    }
}
