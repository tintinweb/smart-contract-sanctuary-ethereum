...
contract CryptoAnts is ERC721, ICryptoAnts {
  ...
  modifier expire(address _addr) {
    require(expiryOf[_addr] >= block.timestamp, 'You must wait 10 minutes');
    expiryOf[_addr] = 0;
    _;
  }

  function createEgg() public expire(msg.sender) returns (bool) {
    eggs.mint(msg.sender, 1);
    expiryOf[msg.sender] = block.timestamp + _waitTime;
