...
contract StakingLockable is LockSettings, IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    function getActiveLockIds(address _account, uint256 _offset) external view returns (bool[] memory results) {
        uint256 count = balances[_account].nextLockIndex;
        uint256 batch = 100;

        if (count == 0) return results;
        if (count <= _offset) return results;

        uint256 end = _offset + batch > count ? count : _offset + batch;

        results = new bool[](end);

        for (uint256 i = _offset; i < end; i++) {
            results[i] = locks[_account][i].withdrawnAt == 0;
        }
    }

    /// @notice returns array of max 100 booleans, where index corresponds to lock id. `true` means lock can be withdraw
    function getVestedLockIds(address _account, uint256 _offset) external view returns (bool[] memory results) {
        uint256 count = balances[_account].nextLockIndex;
        uint256 batch = 100;
        ...
        for (uint256 i = _offset; i < end; i++) {
            Lock storage lock = locks[_account][i];
            results[i] = lock.withdrawnAt == 0 && block.timestamp < lock.unlockDate;
        }
    }
    ...
    function _unlockTokenFor(address _user, uint256 _index) internal returns (uint256 amount, uint256 bonus) {
        ...
        lock.withdrawnAt = uint32(block.timestamp);
