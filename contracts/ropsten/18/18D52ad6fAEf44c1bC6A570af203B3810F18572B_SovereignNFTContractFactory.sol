    ...
    ERC2981Upgradeable
{
    ...
    function updateBatchBaseUri(string memory _baseUri, uint256 _batchIndex)
        public
        onlyOwner
        ifNotDisabled
    {
        require(bytes(_baseUri).length != 0, "Cannot be empty");
        require(mintBatches.length > _batchIndex, "Invalid batch index");

        MintBatch storage batch = mintBatches[_batchIndex];

        require(bytes(batch.baseUri).length == 0, "base already set");

        batch.baseUri = _baseUri;

        emit BatchBaseUriUpdated(
            batch.startTokenId,
            batch.endTokenId,
            _baseUri
        );
