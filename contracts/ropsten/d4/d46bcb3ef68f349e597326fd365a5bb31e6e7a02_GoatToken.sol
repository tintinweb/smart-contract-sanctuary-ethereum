...
contract GoatToken is Context, IERC20, Ownable {
    ...
    address payable private _burnPool =
    ...
    function setTax(uint256 _taxType, uint256 _taxSize) external onlyOwner {
        if (_taxType == 1) {
    ...
    function setAntiWhaleSellThreshold(uint256 maxSellThreshold)
        external
        onlyOwner
    {
    ...
    function setSwapAndLiquifyEnabled(bool _enabled, uint256 _numTokensMin)
        public
        onlyOwner
    {
        ...
        )
    {
        ...
        uint256[3] memory rValues = _getRValuesArray(
            tAmount,
            tValues[1],
            tValues[2],
            tValues[3]
    ...
    function _getRValuesArray(
        ...
        uint256 tLiquidity,
        uint256 tOperations
    ) private view returns (uint256[3] memory val) {
        uint256 currentRate = _getRate();
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rTransferAmount = rAmount
            .sub(rFee)
            .sub(tLiquidity.mul(currentRate))
            .sub(tOperations.mul(currentRate));
    ...
    function calculateOperationsFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_operationsFee).div(10**2);
    }
    ...
    function _transfer(
        ...
        if (!isTransferBuy && !isTransferSell) {
            ...
            if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                ...
                uint256[] memory sale_output_estimate = uniswapV2Router
                ...
                if (sale_output_estimate[1] >= _antiWhaleSellThreshold) {
                    ...
                    _firstBuyTime[from] != 0 &&
                    (_firstBuyTime[from] + (24 hours) > block.timestamp)
                ) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (_pendingLiquidityFees < contractTokenBalance)
            liquidityPart = _pendingLiquidityFees;
        ...
        uint256 liquidityBalance = liquidityHalfPart.mul(newBalance).div(
            totalETHSwap
        );

        // add liquidity to uniswap
        if (liquidityHalfTokenPart > 0 && liquidityBalance > 0)
    ...
    function _tokenTransfer(
        ...
        if (!_isExcludedFromFee[recipient] && (recipient != uniswapV2Pair))
            require(
                balanceOf(recipient) < _maxWalletHolding,
                "Max Wallet holding limit exceeded"
            );
    }
