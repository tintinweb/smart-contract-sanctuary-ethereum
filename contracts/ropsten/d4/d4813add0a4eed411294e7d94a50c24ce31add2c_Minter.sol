...
contract Minter is Initializable, UUPSUpgradeable, OwnableUpgradeable {
  Brand private _token;

  // minting management
	bool public mintEnabled;
	uint256 public mintPrice;
	uint256 public invitedMintPrice;
	uint256 public discountedThresholdPrice;
	uint256 public discountThreshold;

  // ref management
  mapping (address => address) public referral;
  uint256 public baseMarket;
  ...
  function initialize(
    ...
    uint256 mintPrice_, 
    uint256 invitedMintPrice_, 
    uint256 discountedThresholdPrice_,
    ...
    invitedMintPrice = invitedMintPrice_;
    discountedThresholdPrice = discountedThresholdPrice_;
  ...
  function mint(uint256 amount_, address inviter_) public payable returns(bool) {
    require(mintEnabled == true, "Mint is closed.");
    require(amount_ > 0, "Amount should be at least 1");
    
    // Check for discounts in case it has been invited
    uint256 inviterTokenBalance = _token.balanceOf(inviter_);
    uint256 amountNeeded;
    if(inviterTokenBalance > 0 && inviter_ != address(0x0)) {
      amountNeeded = invitedMintPrice;
    } else {
      amountNeeded = mintPrice;
    }

    require(msg.value >= amountNeeded, "Please check amount sent for mint.");

    uint256 sentToInviter;
    if(inviterTokenBalance > 2 && inviter_ != address(0x0)) {
      sentToInviter = 0.03 ether * amount_;
      payable(inviter_).transfer(sentToInviter);
    } else {
      sentToInviter = 0.01 ether * amount_ * inviterTokenBalance;
      payable(inviter_).transfer(sentToInviter);
    }
    
    payout(msg.value - sentToInviter);
  ...
  function setInvitedMintPrice(uint256 price_) public onlyOwner {
    invitedMintPrice = price_;
  }

	function setDiscountedPriceAndThreshold(uint256 price_, uint256 threshold_) public onlyOwner {
		discountedThresholdPrice = price_;
