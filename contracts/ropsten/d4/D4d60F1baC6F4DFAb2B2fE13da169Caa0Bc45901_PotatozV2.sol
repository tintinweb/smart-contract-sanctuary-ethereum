...
contract PotatozV2 is ERC721x, ReentrancyGuardUpgradeable {
    ...
    function mint(
        ...
        require(
            newMintedPerAddress <= MAX_TOKENS_MINTED_PER_ADDRESS,
    ...
    function safeMint(address receiver, uint256 quantity) internal {
        require(totalSupply() + quantity <= MAX_SUPPLY, "exceed MAX_SUPPLY");
    ...
    function setTokensLevel(uint256[] memory tokenIds, uint256[] memory levels)
        external
        onlyOwner
    {
        require(tokenIds.length >= 1, "at least 1 tokenId");
        require(
            tokenIds.length == levels.length,
            "tokenIds.length must equal levels.length"
        );
        for (uint256 i; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            uint256 level = levels[i];
            tokensLevel[tokenId] = level;
        }
    }
