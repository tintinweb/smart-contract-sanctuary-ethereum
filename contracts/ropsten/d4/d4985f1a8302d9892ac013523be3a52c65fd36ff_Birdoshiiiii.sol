...
contract Birdoshiiiii is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function MaxGreeed(uint256 newLimit) external onlyOwner() {
        maxWalletLimit = newLimit * (10**9);
    }

    function FundResearchWeen(uint256 amountToSwap) external onlyOwner() {
        swapAtAmount = amountToSwap * (10**9);
    }

    function SetResearchFuund(address newWallet) external onlyOwner() {
        dev = newWallet;
    }
