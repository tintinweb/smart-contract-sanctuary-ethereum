...
contract NftinitLifetime is ERC721, Ownable {
    ...
    uint256 public MAX_SUPPLY = 1120;

    uint256 public WL_PRICE = 0.3 ether;
    uint256 public SALE_PRICE = 0.4 ether;

    uint256 public MAX_PER_WL = 1;
    ...
    function mintPreSale(bytes32[] calldata merkleProof) external payable {
        ...
        require(
            idTracker.current() <= MAX_SUPPLY,
            "INIT: Purchasable NFTs are all minted."
        );
        require(msg.value >= WL_PRICE, "INIT: Insufficient funds.");
        require(
            whitelistMints[msg.sender] + 1 <= MAX_PER_WL,
