        ...
        returns (IProject projectAddress)
    {
        ...
        IProject p = new Project(
            IProject.ProjectOptions(
                ...
                sofinaHub._videos,
                sofinaHub._roi,
                sofinaHub._roiDuration,
                sofinaHub._tokenName,
                sofinaHub._tokenSymbol,
                sofinaHub._tokenDecimal,
                sofinaHub._tokenTotalSupply
            )
        );
...
contract Project is IProject {
        ...
        returns (bool successful)
    {
        ...
        c.amount = (msg.value / (1 ether));

        // Update contributor's balance
        contributors[_contributor] += (msg.value / (1 ether));

        totalFunding += (msg.value / (1 ether));
        contributionsCount++;
        ...
        token.sendReward(
            _contributor,
            ((msg.value / (1 ether)) /
                (properties.goal.div(properties.tokenTotalSupply))) *
        ...
        returns (bool successful)
    {
        ...
        if (properties.creator.send((amount * 1 ether))) {
            return true;
        } else {
            totalFunding = amount;
        ...
        returns (bool successful)
    {
        ...
        if (payable(_contributor).send(amount * 1 ether)) {
        ...
        returns (bool successful)
    {
        ...
        require(
            msg.value >= (expectedFunds * 1 ether),
        ...
        returns (bool successful)
    {
        ...
        // capital and roi
        uint256 capital = properties.goal.div(properties.tokenTotalSupply).mul(
            balance / (10**properties.tokenDecimal)
        );
        uint256 roi = properties.roi.div(100).mul(capital);

        token.transferFrom(_contributor, address(this), balance);

        if (payable(_contributor).send((capital.add(roi)) * 1 ether)) {
            emit LogClaimSent(
                address(this),
                _contributor,
                (capital.add(roi)) * 1 ether
            );
