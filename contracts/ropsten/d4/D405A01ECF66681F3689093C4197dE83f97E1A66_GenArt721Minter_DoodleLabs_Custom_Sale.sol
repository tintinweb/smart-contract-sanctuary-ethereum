...
interface IGenArt721Minter_DoodleLabs_WhiteList {
    ...
    function increaseAmount(uint256 projectId, address to, uint256 quantity) external;
}
...
contract GenArt721Minter_DoodleLabs_Custom_Sale is GenArt721Minter_DoodleLabs_MultiMinter {
    ...
    function purchaseMany(uint256 projectId, uint256 amount)
        public
        payable
    ...
    {
        require(
            amount <= minterState.getPurchaseManyLimit(projectId),
    ...
    function purchase(uint256 _projectId)
        public
        payable
        notRedemptionState(_projectId)
        returns (uint256 _tokenId)
    {
        if (
            uint256(minterState.getState(_projectId)) == uint256(SaleState.FAMILY_COLLECTORS) &&
            msg.value > 0
        ) {
    ...
    //     public
    //     payable
    //     onlyRedemptionState(projectId)
    //     returns (uint256[] memory _tokenIds)
    // {
    //     require(
    //         allottedAmount <= minterState.getPurchaseManyLimit(projectId),
    //         'Max purchase many limit reached'
    //     );
    //     uint256[] memory tokenIds = new uint256[](allottedAmount);
    ...
    //             projectId
    //         );
    //         uint256 refund = msg.value.sub(pricePerTokenInWei.mul(allottedAmount));
    ...
    //     tokenIds = redeemTo(msg.sender, projectId, isDeferredRefund, quantity, allottedAmount, proof);
    ...
    function redeemTo(
        ...
        require(address(activeWhitelist) != address(0), "Active whitelist not set");
        require(
            activeWhitelist.getWhitelisted(projectId, msg.sender).add(quantity) <= allottedAmount,
            'Address has already claimed'
        );

        string memory key = _addressToString(msg.sender);
        ...
        uint256[] memory createdTokens = _purchaseMany(projectId, quantity);
    ...
    function _appendStrings(string memory a, string memory b, string memory c)
        internal
        pure
