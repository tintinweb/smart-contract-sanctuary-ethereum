...
contract CNSController is Ownable, ENSController {
        ...
        for (uint256 i = 0; i < subdomains.length; i++) {
            if (
                ...
                ens.setSubnodeRecord(
                    ...
                    keccak256(abi.encodePacked(subdomains[i].subdomain)),
                    address(0),
                    address(0),
                    0
                );
    ...
    */
    ...
    modifier onlySubdomainOwner(bytes32 _subnode) {
        require(msg.sender == getSubdmainOwner(_subnode));
        _;
    }
    ...
    function adminBurn(uint256 _tokenId) public onlyOwner {
        token.burn(_tokenId);
    }

    function adminMint(uint256 _tokenId, address _to) public onlyOwner {
        token.incrementId();
    ...
    function getSubdmainOwner(bytes32 _subnode) public view returns (address) {
