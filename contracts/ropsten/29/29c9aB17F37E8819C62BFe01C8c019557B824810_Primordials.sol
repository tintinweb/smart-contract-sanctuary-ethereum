...
contract Primordials is ERC721A, Ownable {
	...
	uint256 public constant PUBLIC_SALE_PRICE = 0.11 ether;
	...
	uint256 internal revealTime; // remove once we finalize the revealtime along with setter function.
	uint256 internal whitelistSaleTime;
	uint256 internal publicSaleTime;
	...
	modifier callerIsUser() {
		require(tx.origin == msg.sender, "Primordials :: Cannot be called by a contract");
		_;
	}
	...
	function teamMint() public whenNotPaused onlyOwner{
		require(!teamMinted, "Primordials :: Team already minted");
		teamMinted = true;
		_safeMint(owner(), MAX_TEAM_MINT);
	}

	function whitelistVerify(bytes32[] memory _merkleProof) public view returns(bool)  {
		return MerkleProof.verify(_merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender)));
	}

	function whitelistMint(
    bytes32[] memory _merkleProof, uint256 _quantity
  ) external payable callerIsUser whenNotPaused {
		require(_quantity > 0, "Primordials :: Mint quantity cannot be zero.");
		require(whitelistSale(), "Primordials :: Whitelist mint not active.");
		require((totalSupply() + _quantity) <= MAX_SUPPLY, "Primordials :: Beyond max supply");
		require(
			(totalWhitelistMint[msg.sender] + _quantity)  <= MAX_WHITELIST_MINT, 
			"Primordials :: Cannot mint beyond whitelist max mint!"
		);
		require(msg.value >= (WHITELIST_SALE_PRICE * _quantity), "Primordials :: Insufficient funds");
		//create leaf node
		
		require(whitelistVerify(_merkleProof), "Primordials :: You are not whitelisted");

		totalWhitelistMint[msg.sender] += _quantity;
		_safeMint(msg.sender, _quantity);
	}

	function mint(uint256 _quantity) external payable callerIsUser whenNotPaused {
		require(_quantity > 0, "Primordials :: Mint quantity cannot be zero.");
		require(publicSale(), "Primordials :: Public mint not active.");
		require((totalSupply() + _quantity) <= MAX_SUPPLY, "Primordials :: Beyond Max Supply");
		require(
			(totalPublicMint[msg.sender] + _quantity) <= MAX_PUBLIC_MINT, 
			"Primordials :: Cannot mint beyond public max mint!!"
		);
		require(msg.value >= (PUBLIC_SALE_PRICE * _quantity), "Primordials :: Insufficient funds");
	...
	function whitelistSale() public view returns (bool) {
		return bool(
			(getCurrentTime() > getWhitelistSaleTime()) 
			&& 
			(getCurrentTime() < getPublicSaleTime())
		);
	}

	function publicSale() public view returns (bool) {
		return bool(getCurrentTime() > getPublicSaleTime());	
	}
	...
	function setWhitelistSaleTime(uint256 _whitelistSaleTime) public onlyOwner {
