...
contract LANDAuctionStorage {
    ...
    event BidConversion(
      ...
      uint256 _requiredMegaAmountToBurn,
    ...
    event BidSuccessful(
      ...
      address indexed _token,
      uint256 _pricePerLandInMega,
      uint256 _MegaAmountToBurn,
      int[] _xs,
      int[] _ys
    );
...
contract LANDAuction is Ownable, LANDAuctionStorage {
    ...
    * @param _MegaToken - address of the Mega token
    * @param _landRegistry - address of the LANDRegistry
    * @param _dex - address of the Dex to convert ERC20 tokens allowed to Mega
    */
    constructor(
        uint256[] _xPoints, 
        uint256[] _yPoints, 
    ...
    */
        ...
        external 
    {
        ...
        uint256 bidPriceInMega = _xs.length.mul(getCurrentPrice());
        uint256 MegaAmountToBurn = bidPriceInMega;

        if (address(_fromToken) != address(MegaToken)) {
            require(
                address(dex).isContract(), 
                "Paying with other tokens has been disabled"
            );
            // Convert from the other token to Mega. The amount to be burned might be smaller
            // because 5% will be burned or forwarded without converting it to Mega.
            MegaAmountToBurn = _convertSafe(bidId, _fromToken, bidPriceInMega);
        } else {
            // Transfer Mega to this contract
            require(
                _fromToken.safeTransferFrom(msg.sender, address(this), bidPriceInMega),
        ...
        // Update stats
        _updateStats(_xs.length, MegaAmountToBurn);        
    }
    ...
    */
    ...
    /**
    * @dev Convert allowed token to Mega and transfer the change in the original token
    ...
    * @param _bidPriceInMega - uint256 of the total amount in Mega
    * @return uint256 of the total amount of Mega to burn
    */
    function _convertSafe(
        uint256 _bidId,
        ERC20 _fromToken,
        uint256 _bidPriceInMega
    ) internal returns (uint256 requiredMegaAmountToBurn)
    {
        requiredMegaAmountToBurn = _bidPriceInMega;
        Token memory fromToken = tokensAllowed[address(_fromToken)];

        uint256 bidPriceInMegaPlusSafetyMargin = _bidPriceInMega.mul(conversionFee).div(100);

        // Get rate
        uint256 tokenRate = getRate(MegaToken, _fromToken, bidPriceInMegaPlusSafetyMargin);
        ...
        if (fromToken.shouldBurnTokens || fromToken.shouldForwardTokens) {
            requiredTokenBalance = _calculateRequiredTokenBalance(requiredMegaAmountToBurn, tokenRate);
            requiredMegaAmountToBurn = _calculateRequiredMegaAmount(_bidPriceInMega);
        }

        // Calculate the amount of _fromToken to be converted
        uint256 tokensToConvertPlusSafetyMargin = bidPriceInMegaPlusSafetyMargin
            .mul(tokenRate)
            .div(10 ** 18);
        ...
        // Convert _fromToken to Mega
        uint256 change = dex.convert(
                _fromToken,
                MegaToken,
                finalTokensToConvert,
                requiredMegaAmountToBurn
        );
    ...
    */
    ...
    /** 
    * @dev Calculate the total price in Mega
    ...
    * @return uint256 of the new total price in Mega
    */
    function _calculateRequiredMegaAmount(
        uint256 _totalPrice
    ) 
    internal pure returns (uint256)
    {
    ...
    /**
    * @dev Burn or forward the Mega and other tokens earned
    * Note that as we will transfer or burn tokens from other contracts.
    * We should burn Mega first to avoid a possible re-entrancy
    * @param _bidId - uint256 of the bid Id
    * @param _token - ERC20 token
    */
    function _processFunds(uint256 _bidId, ERC20 _token) internal {
        // Burn Mega
        _burnTokens(_bidId, MegaToken);

        // Burn or forward token if it is not Mega
    ...
    * will convert the total amount of the ERC20 to Mega
    ...
    * @param _MegaAmountBurned - uint256 of the amount of Mega burned
    */
    function _updateStats(uint256 _landsBidded, uint256 _MegaAmountBurned) private {
        totalBids = totalBids.add(1);
        totalLandsBidded = totalLandsBidded.add(_landsBidded);
        totalMegaBurned = totalMegaBurned.add(_MegaAmountBurned);
    }
}