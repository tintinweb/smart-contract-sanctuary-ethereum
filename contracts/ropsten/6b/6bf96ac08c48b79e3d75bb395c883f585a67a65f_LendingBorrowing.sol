...
contract LendingBorrowing is Ownable {
  ...
  */
  ...
  //
  enum lenderStatus {PENDING, OPEN, REPAY, CLOSED}
  ...
  */
  ...
  // mapping for matchinfg count
  mapping (uint256 => uint256) private _borrowerMatchingCount;
  ...
  //
  uint256 public bufferTime = 3456000;
  ...
  function getTokenInterestById(uint256 id) public view returns(uint256){
    if(_borrowerTotalDays[id] == 30) {
        return (_borrowerTotalDays[id]*TOKEN_INTEREST/100);
  ...
  function getNativeTokenInterestById(uint256 id) public view returns(uint256){
    if(_borrowerTotalDays[id] == 30) {
        return (_borrowerTotalDays[id]*NATIVE_INTEREST/100);
    } else if(_borrowerTotalDays[id] == 90) {
               return (_borrowerTotalDays[id]*NATIVE_INTEREST/100);
      } else if(_borrowerTotalDays[id] == 180) { 
                 return (_borrowerTotalDays[id]*NATIVE_INTEREST/100);
  ...
  */
  ...
  function lenderWithdrawinNative(uint256 id) external returns(bool){
    ...
    if(_currentLenderStatus[id] == lenderStatus.PENDING){
       _lenderAddress[id].transfer(_lenderAmount[id]);        
       _lenderTransactionstatus[id] = true;
       _currentLenderStatus[id] = lenderStatus.CLOSED;
    // } else if(_currentLenderStatus[id] == lenderStatus.OPEN){
    //   // if(){

    //   // }
    //     return false;
    } else if(_currentLenderStatus[id] == lenderStatus.REPAY){
       _lenderAddress[id].transfer(_lenderAmount[id]);        
       _lenderTransactionstatus[id] = true;
       _currentLenderStatus[id] = lenderStatus.CLOSED;
        }else {
  ...
  function lenderWithdrawInToken(uint256 id) external returns(bool){
    ...
    require(_lenderTransactionstatus[id] != true,"Either Amount are already withdrawn or blocked by admin");
    if(_currentLenderStatus[id] == lenderStatus.PENDING){
       itoken.transfer(msg.sender,_lenderAmount[id]);        
       _lenderTransactionstatus[id] = true;
       _currentLenderStatus[id] = lenderStatus.CLOSED;
    } else if(_currentLenderStatus[id] == lenderStatus.REPAY){
       itoken.transfer(msg.sender,_lenderAmount[id]);        
  ...
  */ 
  ...
  //
  function loanTakeOffInToken(uint256 tokens, uint256 time, uint256 matchingCount) external payable returns(bool){ 
    _borrowerTime = now + (time);
    ...
    _borrowerMatchingCount[_borrowerCount] = matchingCount;
    _lenderStartTime[matchingCount] = now;
    _lenderEndTime[matchingCount] = _borrowerTime;
    _currentLenderStatus[matchingCount] = lenderStatus.OPEN;
    ...
    _borrowerTransactionstatus[_borrowerCount] = false;
    return true;
  }  

  //
  function loanTakeOffInNativeToken(uint256 tokens, uint256 time, uint256 matchingCount) external payable returns(bool){ 
    _borrowerTime = now + (time);
    _borrowerCount = _borrowerCount + 1 ;
    _borrowerAddress[_borrowerCount] = msg.sender;
    _borrowerId[msg.sender].push(_borrowerCount);
  ...
  //function for withdraw tokens
  function withdrawloanTakeOffInTokens(uint256 id) public returns(bool) {
    require(_borrowerAddress[id] == msg.sender,"No loan amount or token found on this address and ID");
    require(now >= _borrowerEndTime[id], "Unable to Withdraw, Please Try Again Later");
    //require(_TokenTransactionstatus[stakingId] != true,"Either tokens are already withdrawn or blocked by admin");
    //require(_currentBorrowerStatus[id] == OPEN, "Withdraw Mechanism already performed");
    uint256 matchingCounts = 0;
    if(_borrowerTotalDays[id] == 30){
          _borrowerTransactionstatus[id] = true;
          if(now >= _borrowerEndTime[id] || now >= 30 + bufferTime){
                  _borrowerAddress[id].transfer(_borrowerCollatoralAmount[id]);
                  itoken.transfer(address(this),_borrowerAmount[id]);
                  _currentBorrowerStatus[id] = borrowerStatus.CLOSED;
                  _borrowerTransactionstatus[id] = true;
                  matchingCounts = _borrowerMatchingCount[id];
                  _currentLenderStatus[matchingCounts] = lenderStatus.REPAY;
          } else {
                  _borrowerAddress[id].transfer(_borrowerCollatoralAmount[id]);
                  itoken.transfer(address(this),_borrowerAmount[id]);
                  _currentBorrowerStatus[id] = borrowerStatus.CLOSED;
          ...
          _borrowerTransactionstatus[id] = true;
          if(now >= _borrowerEndTime[id] || now >= 90 + bufferTime){
                  _borrowerAddress[id].transfer(_borrowerCollatoralAmount[id]);
                  itoken.transfer(address(this),_borrowerAmount[id]);
                  _currentBorrowerStatus[id] = borrowerStatus.CLOSED;
                  _borrowerTransactionstatus[id] = true;
          ...
          _borrowerTransactionstatus[id] = true;
          if(now >= _borrowerEndTime[id] || now >= 180 + bufferTime){
                  _borrowerAddress[id].transfer(_borrowerCollatoralAmount[id]);
                  itoken.transfer(address(this),_borrowerAmount[id]);
                  _currentBorrowerStatus[id] = borrowerStatus.CLOSED;
                  _borrowerTransactionstatus[id] = true;
          ...
          _borrowerTransactionstatus[id] = true;
          if(now >= _borrowerEndTime[id] || now >= 360 + bufferTime){
                  _borrowerAddress[id].transfer(_borrowerCollatoralAmount[id]);
                  itoken.transfer(address(this),_borrowerAmount[id]);
                  _currentBorrowerStatus[id] = borrowerStatus.CLOSED;
                  _borrowerTransactionstatus[id] = true;
   ...
   //////////////////////////////
   //function for withdraw tokens
  function withdrawloanTakeOffInNativeTokens(uint256 id) public payable returns(bool) {
    require(_borrowerAddress[id] == msg.sender,"No loan amount or token found on this address and ID");
    require(now >= _borrowerEndTime[id], "Unable to Withdraw, Please Try Again Later");
    require(msg.value == _borrowerAmount[id], "unapproriate amount");
    //require(_TokenTransactionstatus[stakingId] != true,"Either tokens are already withdrawn or blocked by admin");
    //require(_currentBorrowerStatus[id] == OPEN, "Withdraw Mechanism already performed");
    uint256 matchingCounts = 0;
    if(_borrowerTotalDays[id] == 30){
          _borrowerTransactionstatus[id] = true;
          if(now >= _borrowerEndTime[id] || now >= 30 + bufferTime){
                  itoken.transfer(msg.sender,_borrowerCollatoralAmount[id]);
                  _currentBorrowerStatus[id] = borrowerStatus.CLOSED;
                  _borrowerTransactionstatus[id] = true;
                  matchingCounts = _borrowerMatchingCount[id];
                  _currentLenderStatus[matchingCounts] = lenderStatus.REPAY;
  ...
  */
  ...
  // function to ge
  function getBorrowerMatchingCount(uint256 id) external view returns(uint256){
    require(id <= _borrowerCount,"Unable to reterive data on specified id, Please try again!!");
    return _borrowerMatchingCount[id];
  }
