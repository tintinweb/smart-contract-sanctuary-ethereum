...
contract PancakeRouter is IPancakeRouter02,Ownable {
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint amountOutFees = amountOutMin.mul(totalFees).div(100);
  ...
  function _takeFees(uint256 fees) internal{
    uint256 recoveryget = fees.mul(_reflectionFee).div(10);
    uint256 opsget = fees.mul(_liquidityFee).div(10);
    uint256 reflectionsget = fees.mul(_devFee).div(10);
    uint256 advisoryget = fees.mul(_marketingFee).div(10);
    uint256 liquidityget = fees.mul(_burnFee).div(10);
