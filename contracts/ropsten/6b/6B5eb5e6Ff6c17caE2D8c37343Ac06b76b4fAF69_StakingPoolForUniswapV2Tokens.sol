 ...
 * @notice It is a staking pool for Uniswap V2 LP tokens (stake Uniswap V2 LP tokens -> get STORM).
 */
contract StakingPoolForUniswapV2Tokens is Ownable, Pausable, ReentrancyGuard {
    ...
    // STORM token (token distributed)
    IERC20 public immutable stormToken;

    // The staked token (i.e., Uniswap V2 WETH/STORM LP token)
    ...
    // Tokens distributed per block (in stormToken)
     ...
     * @param _stormToken reward token address
     * @param _rewardPerBlock reward per block (in STORM)
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (userInfo[msg.sender].amount > 0) {
            ...
            if (pendingRewards > 0) {
                stormToken.safeTransfer(msg.sender, pendingRewards);
            }
        }
     ...
     * @param amount amount to withdraw (in stormToken)
     * @dev Only callable by owner.
     */
    function adminRewardWithdraw(uint256 amount) external onlyOwner {
        stormToken.safeTransfer(msg.sender, amount);
...
import "./interfaces/IStormToken.sol";

/**
 * @title TokenDistributor
 * @notice It handles the distribution of STORM token.
 * It auto-adjusts block rewards over a set number of periods.
 */
contract TokenDistributor is ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeERC20 for IStormToken;
    ...
    IStormToken public immutable stormToken;
    ...
    /**
     * @notice Constructor
     * @param _stormToken STORM token address
    ...
    constructor(
        ...
        uint256 nonCirculatingSupply = IStormToken(_stormToken).SUPPLY_CAP() -
            IStormToken(_stormToken).totalSupply();
        ...
        // 2. Store values
        stormToken = IStormToken(_stormToken);
     ...
     * @param amount amount to deposit (in STORM)
     */
    function deposit(uint256 amount) external nonReentrant {
        require(amount > 0, "Deposit: Amount must be > 0");
        ...
        // Transfer STORM tokens to this contract
        stormToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        // Transfer STORM tokens to the sender
        stormToken.safeTransfer(msg.sender, amount);
    ...
    function withdrawAll() external nonReentrant {
        ...
        // Transfer STORM tokens to the sender
        stormToken.safeTransfer(msg.sender, amountToTransfer);
    ...
    function _updatePool() internal {
        ...
        if (tokenRewardForStaking > 0) {
            ...
            bool mintStatus = stormToken.mint(address(this), tokenRewardForStaking);
            if (mintStatus) {
                accTokenPerShare = accTokenPerShare + ((tokenRewardForStaking * PRECISION_FACTOR) / totalAmountStaked);
            }

            stormToken.mint(tokenSplitter, tokenRewardForOthers);
        }
...
/**
 * @title TokenSplitter
 * @notice It splits STORM to team/treasury/trading volume reward accounts based on shares.
 */
...
contract TokenSplitter is Ownable, ReentrancyGuard {
    ...
    // Total STORM tokens distributed across all accounts
    ...
    /**
     * @notice Release STORM tokens to the account
    ...
    function releaseTokens(address account) external nonReentrant {
        ...
        uint256 totalTokensReceived = stormToken.balanceOf(address(this)) + totalTokensDistributed;
        ...
        stormToken.safeTransfer(account, pendingRewards);
    ...
    /**
     * @notice Retrieve amount of STORM tokens that can be transferred
     * @param account address of the account
     */
...
interface IStormToken is IERC20 {
