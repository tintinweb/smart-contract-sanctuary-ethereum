...
contract SxTCommunity is ISxTCommunity, ERC1155Supply, Ownable, ReentrancyGuard, Pausable {
    ...
    /// @param newTokenEthPrice New price for the NFT token
    ...
    /// @dev Only the owner can call this function
    /// @param id ID of NFT token for which token price needs to be updated
    /// @param newTokenERC20Price New price for the NFT token
    ...
    function mintNFT(uint256 id, address to) external override whenNotPaused {   
        require(keccak256(abi.encodePacked(tokenDetails[id].tokenUri)) != keccak256(abi.encodePacked("")) , "SxTCommunity: URI nonexistent token");     
        require(tokenDetails[id].hasPrice == false && tokenDetails[id].ethPrice == 0 && tokenDetails[id].erc20Price == 0, "SxTCommunity: This token is not free");
    ...
    function mintUsingEth(uint256 id, address to) external override payable whenNotPaused nonReentrant{
        require(keccak256(abi.encodePacked(tokenDetails[id].tokenUri)) != keccak256(abi.encodePacked("")) , "SxTCommunity: URI nonexistent token");     
        require(tokenDetails[id].hasPrice == true && tokenDetails[id].ethPrice > 0, "SxTCommunity: Token Eth Price not yet set");     
        require(!checkMaxSupplyReached(id),"SxTCommunity: Total supply exceeded maximum supply");
        require(!isPreviouslyMintedFrom[id][to],"SxTCommunity: Already minted this NFT once to this account");
    ...
    function mintUsingERC20(uint256 id, address to) external override whenNotPaused nonReentrant{
        require(address(SXT_TOKEN) != ZERO_ADDRESS , "SxTCommunity: ERC20 Token not yet set by owner");
        require(keccak256(abi.encodePacked(tokenDetails[id].tokenUri)) != keccak256(abi.encodePacked("")) , "SxTCommunity: URI nonexistent token");     
        require(tokenDetails[id].hasPrice == true && tokenDetails[id].erc20Price > 0, "SxTCommunity: Token ERC20 Price not yet set");     
        require(!checkMaxSupplyReached(id),"SxTCommunity: Total supply exceeded maximum supply");
        require(!isPreviouslyMintedFrom[id][to],"SxTCommunity: Already minted this NFT once to this account");
        require(SXT_TOKEN.balanceOf(msg.sender) >= tokenDetails[id].erc20Price ,"SxTCommunity: Insufficient ERC20 token balance");
        bool sent = SXT_TOKEN.transferFrom(msg.sender, address(this), tokenDetails[id].erc20Price);
    ...
    function addNewNFTs(string [] memory newTokenURIs, uint256 [] memory maxNewTokenSupplies, bool [] memory hasPrices, uint256 [] memory newEthPrices, uint256 [] memory newERC20Prices) external override whenPaused onlyOwner {
        require(newTokenURIs.length == maxNewTokenSupplies.length && maxNewTokenSupplies.length == hasPrices.length && hasPrices.length == newEthPrices.length && newEthPrices.length == newERC20Prices.length, "SxTCommunity: Array lengths shouls be same");
        for(uint256 index = 0; index < maxNewTokenSupplies.length; index++){
            Token memory newToken;
            require(maxNewTokenSupplies[index] > 0,"SxTCommunity: Maximum supply cannot be 0");
            currentTokenIndex.increment();
...
interface ISxTCommunity {
    ...
    /// @param newTokenEthPrice New price set for the NFT token
    
    event TokenEthPriceSet(uint256 id, uint256 newTokenEthPrice);

    /// @dev This is the event to notify that price of an NFT is set
    /// @param id ID of NFT token for which token price is updated
    /// @param newTokenERC20Price New price set for the NFT token
    ...
    /// @dev This is the function to set the price in ether for an NFT token 
    /// @dev Only the owner can call this function
    /// @param id ID of NFT token for which token price needs to be updated
    /// @param newTokenPrice New price for the NFT token
    
    function setTokenEthPrice(uint256 id, uint256 newTokenPrice) external;

    /// @dev This is the function to set the price in ERC20 token for an NFT token 
    /// @dev Only the owner can call this function
    /// @param id ID of NFT token for which token price needs to be updated
    /// @param newTokenPrice New price for the NFT token
    
    function setTokenERC20Price(uint256 id, uint256 newTokenPrice) external;
    ...
    /// @dev This is the function to buy a new NFT token using ether
    /// @dev If to Token is buyable with Ether and tokenPrice>0, function will accept ETHERS
    /// @param id ID of NFT token to be bought
    /// @param to Address to which NFT token should be minted to

    function mintUsingEth(uint256 id, address to) external payable;
    ...
    function mintUsingERC20(uint256 id, address to) external;
