    ...
    ReentrancyGuard
{
    ...
    uint256 private bridged;

    address private signer;

    // ======== Admin functions ========

    constructor(address wrld, address nftw)
    ...
    function setRewards(
        ...
        require(
            block.timestamp.toUint32() < rewardsPeriod.start ||
                block.timestamp.toUint32() > rewardsPeriod.end,
            "E4"
    ...
    function setWeight(uint256[] calldata tokenIds, uint256[] calldata weights)
        external
        onlyOwner
    {
    ...
    function setRentalContract(INFTWRental _contract) external onlyOwner {
        require(
            _contract.supportsInterface(type(INFTWRental).interfaceId),
            "E0"
        );
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            {
                ...
                uint256 tokenId = tokenIds[i];
                uint256 _weight = worldInfo[tokenId].weight;
            ...
            worldInfo[tokenIds[i]] = WorldInfo(
                weights[i].toUint16(),
                stakeTo,
                _deposit,
                _rentalPerDay,
    ...
    function updateRent(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                NFTW_ERC721.ownerOf(tokenId) == address(this) &&
                    worldInfo_.owner == _msgSender(),
                "E9"
            ); // E9: Not your world
        ...
        nonReentrant
    {
        ...
        require(
            balanceOf(_msgSender()) - userBridged[_msgSender()] >=
                tokenIds.length * 1e18,
            "EP"
        ); // EP: veNFTW bridged to polygon
    ...
    function setRoutingDataIPFSHash(uint256 tokenId, string calldata _ipfsHash)
        external
    {
        require(
            (worldInfo[tokenId].owner == _msgSender() &&
                !NFTWRental.isRentActive(tokenId)) ||
                (worldInfo[tokenId].owner != address(0) &&
                    NFTWRental.getTenant(tokenId) == _msgSender()),
            "EH"
        ); // EH: Not your world or not rented
        NFTWRouter.setRoutingDataIPFSHash(tokenId, _ipfsHash);
    }

    function removeRoutingDataIPFSHash(uint256 tokenId) external {
        require(
            (worldInfo[tokenId].owner == _msgSender() &&
                !NFTWRental.isRentActive(tokenId)) ||
                (worldInfo[tokenId].owner != address(0) &&
        ...
        returns (uint256)
    {
        ...
        return
            ...
            userRewards_.stakedWeight *
            (rewardsPerWeight_.accumulated - userRewards_.checkpoint);
    }
    ...
    {
        return
            interfaceId == type(INFTWEscrow).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    // ======== internal functions ========

    function _verifySignerSignature(bytes32 hash, bytes calldata signature)
        internal
        view
        returns (bool)
    {
    ...
    function _updateRewardsPerWeight(uint32 weight, bool increase)
        internal
        virtual
    {
        RewardsPerWeight memory rewardsPerWeight_ = rewardsPerWeight;
        ...
        if (block.timestamp.toUint32() >= rewardsPeriod_.start) {
            ...
            if (unaccountedTime != 0) {
                ...
                // If the first mint happens mid-program, we don't update the accumulator, no one gets the rewards for that period.
                if (rewardsPerWeight_.totalWeight != 0) {
                    rewardsPerWeight_.accumulated = (rewardsPerWeight_
                        .accumulated +
                        (unaccountedTime * rewardsPerWeight_.rate) /
                        rewardsPerWeight_.totalWeight).toUint96();
                }
