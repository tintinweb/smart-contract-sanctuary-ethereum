...
contract DFreelance is Ownable, ReentrancyGuard { 
    ...
    address private treasury = 0xfC59346F21A002526a328D963ABdddFA7e376046;
    ...
    mapping(address=>uint) private tokenFactory;
    ...
    function getDelayTime(DelayType _type) public view returns(uint){
        uint delay = delayTimes[_type];
        if(delay == 0) {
          delay = 172800;
        }
    ...
    function setDelayTime(DelayType _type, uint _delayTime) external{ 
        requireOwner();
        require(_delayTime > 0, "Invalid delay time");
        delayTimes[_type] = _delayTime;
    }

    /// @notice Get the client fee
    /// @dev Solidity doesnt support the struct return type in public function calls
    /// @return clientFee: client fee (%)
    function getValidToken(address token) public view returns(uint){
        return tokenFactory[token];
    } 

    function setValidToken(address _token, uint _valule) external{ 
        requireOwner();
        tokenFactory[_token] = _valule; // 1: active, 0: inactive
    }
    ...
    function setClientFee(uint _fee) external{ 
        requireOwner();
        require(_fee > 0 && _fee < 100, "Invalid client fee");
    ...
    function setFreelancerFee(uint _level, uint _fee) external{   
        requireOwner();
        require(_fee > 0 && _fee < 200, "Invalid Freelancer fee");
    ...
    function getProject(uint _id)external view returns(uint id,uint state, address client,address freelancer){ 
    ...
    function getMilestone(uint _projectId, uint _milestoneId) external view returns(uint id,uint price,uint state, address token, uint updated){ 
    ...
    function setMilestoneState(uint _projectId, uint _milestoneId, MilestoneState _state) external {
      requireOwner();
        ...
        inProjectProgress(_projectId)
    {
        ...
        if(_token == address(0)) {
          require(msg.value == _amount, "Invalid Amount");
           ...
           require(tokenFactory[_token] == 1, "Invalid token address");
        }
        ...
        onlyClient((projects[_projectId].client))
    {
        ...
        milestones[_projectId][_milestoneId].updated=block.timestamp + getDelayTime(DelayType.Release);
    ...
    function acceptReleaseMilestone(uint _projectId, uint _milestoneId)
        ...
        onlyDeveloper((projects[_projectId].freelancer))
    {
        require(block.timestamp > milestones[_projectId][_milestoneId].updated, "You can receive it 48 hours after the client releases.");
        ...
        if(tokenAddr == address(0)) {
          ...
          payable(getTreasuryInfo()).transfer(freelancerFeeAmt);
        } else {
          IERC20(tokenAddr).safeTransfer(projects[_projectId].freelancer, price);
          IERC20(tokenAddr).safeTransfer(getTreasuryInfo(), freelancerFeeAmt);
        }
    ...
    function getDisputeMilestone(uint _projectId, uint _milestoneId, uint _disputeId)external view returns(uint id, uint milestoneId, address litigator, uint litigatorAmount, uint defendantAmount, uint litigatorUpdatedAt, uint defendantUpdatedAt, uint milestoneState){ 
        ...
        inMilestoneState(_projectId, _milestoneId, MilestoneState.Disputed)
    {
        ...
        if(disputes[_projectId][_disputeId].defendantUpdatedAt == 0) {
          require((block.timestamp - disputes[_projectId][_disputeId].createdAt) < getDelayTime(DelayType.Dispute), "This is a dispute that has already been resolved and cannot be changed.");
        }
    ...
    function settleDisputeMilestone(uint _projectId, uint _milestoneId, uint _disputeId) private
    {
        ...
        paymentInfo.treasury = getTreasuryInfo();
        ...
        if(tokenAddr == address(0)) {
          if(paymentInfo.freelancerAmount > 0) {
            ...
            payable(paymentInfo.treasury).transfer(paymentInfo.freelancerFeeAmt);         
          }
          ...
          if(paymentInfo.freelancerAmount > 0) {
            IERC20(milestones[_projectId][_milestoneId].token).safeTransfer(paymentInfo.freelancer, paymentInfo.freelancerAmount); 
            IERC20(milestones[_projectId][_milestoneId].token).safeTransfer( paymentInfo.treasury, paymentInfo.freelancerFeeAmt);
          }
          
          if(paymentInfo.clientAmount > 0) {
            IERC20(milestones[_projectId][_milestoneId].token).safeTransfer(paymentInfo.client, paymentInfo.clientAmount); 
          }
        }
    }
    ...
    {
        require(msg.sender == projects[_projectId].client || msg.sender == projects[_projectId].freelancer, "Invalid access");     
        require( disputes[_projectId][_disputeId].defendantUpdatedAt == 0 && (block.timestamp - disputes[_projectId][_disputeId].createdAt) > getDelayTime(DelayType.Dispute), "You can accept the dispute payment after 48 hours.");
        ...
        if(tokenAddr == address(0)) {
          if(msg.sender == projects[_projectId].client) {
             if(disputes[_projectId][_disputeId].clientAccepted == 0) {
            ...
            if(disputes[_projectId][_disputeId].freelancerAccepted == 0) {
      ...
      onlyEOA
    {
      if(_token == address(0)) {
        require(msg.value == _amount, "Invalid amount");
        payable(getTreasuryInfo()).transfer(msg.value);
      } else {
        require(tokenFactory[_token] == 1, "Invalid token address");
        require(_amount > 0, "Invalid membership price");

        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount); 
        IERC20(_token).safeTransfer( getTreasuryInfo(),  _amount);
      }
