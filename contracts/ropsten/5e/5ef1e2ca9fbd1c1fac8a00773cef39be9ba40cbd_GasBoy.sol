...
// NICK: TODO:
// 1) Security, maybe add onlyRelayer role?
...
contract GasBoy {
  ...
  function forward(bytes sig, address signer, address destination, uint value, bytes data, address rewardToken, uint rewardAmount) public {
      ...
      if(rewardAmount>0){
          ...
          //REWARD TOKEN - signer pays relayer in rewardToken
          require((StandardToken(rewardToken)).transferFrom(signer, msg.sender, rewardAmount));
      }
