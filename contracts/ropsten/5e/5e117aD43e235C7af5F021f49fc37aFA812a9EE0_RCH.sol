...
contract RCH is IERC20Metadata, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address internal deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    address public marketingWallet = 0xFa9Fa3f52810FD1F6e28beE6e8f7E9cE4DD42153;
    address public adminWallet = 0x2a57D6A953fc9A3B91B0b25Fdbc6E3853f483998;
    
    string constant _name = "Richard Bridle Token";
    string constant _symbol = "RCH";
    ...
    uint256 public rfiFee = 40; // 4%
    uint256 public marketingFee = 40; // 4%
    uint256 public adminFee = 40; // 4%
    uint256 public totalFee = rfiFee.add(marketingFee).add(adminFee);
    
    // Total = 100% (1000)
    uint256 public marketingPortionOfSwap = 500; // 50%
    uint256 public devPortionOfSwap = 350; // 20%
    ...
    event AdminWalletUpdated(address indexed newAdminWallet, address indexed oldDevWallet);
    ...
    constructor () {
        ...
        pair = IPancakeV2Factory(_newPancakeRouter.factory()).getPair(address(this), _newPancakeRouter.WETH());
        if(pair == address(0)) {
    ...
    function calcTotalFees() private {
        totalFee = rfiFee.add(marketingFee).add(adminFee);
    }
    ...
    function updateAdminFee(uint256 newFee) external onlyOwner {
        adminFee = newFee;
        calcTotalFees();
    }
    ...
    function updateAdminWallet(address newAdminWallet) external onlyOwner {
        require(newAdminWallet != adminWallet, "The Admin wallet is already this address");
        emit AdminWalletUpdated(newAdminWallet, adminWallet);
        
        adminWallet = newAdminWallet;
    }
    
    function updatePortionsOfSwap(uint256 marketingPortion, uint256  devPortion, uint256 lpPortion) 
    internal onlyOwner {
        
        uint256 totalPortion = marketingPortion.add(devPortion).add(lpPortion);
    ...
    function setFeesDivisor(uint256 divisor) internal onlyOwner() {
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        sumOfFees = block.timestamp <= _startTimeForSwap ? 990 : sumOfFees;
    ...
    function swapBack() internal {
        ...
        transferToAddress(payable(adminWallet), amountBNBDev);
    ...
    function TransferETH(address payable recipient, uint256 amount) internal onlyOwner {
