...
/**
 * @title NFTurtle contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract NFTurtle is ERC721, Ownable {
    using SafeMath for uint256;

    uint256 public initPrice = 10000000000000000; //0.01 ETH

    uint public constant maxItemsPurchase = 1;

    uint256 public constant MAX_NFTT = 100000;

    bool public saleIsActive = false;

    uint256 public REVEAL_TIMESTAMP;
	
    // Allows retrieving and updating endanger level code based on species name. Retrieve species name from ipfs/json metadata to use this map.
    mapping (string => string) private endangerLevel;
	// Allows current holder to save additional custom metadata stored directly on the blockchain.
    mapping (uint256 => string) private NFTurtleOwnerNote;
	// Link to a real photo that triggered mint of particular NFTurtle.
    mapping (uint256 => string) private NFTurtleOrigPhoto;
	// Allows lookup based on link, prevents duplicates.
    mapping (string => uint256) private tokenIDFromURI;

    // Allows issuer to update mint price.
    function setMintPrice(uint256 priceInWei) external onlyOwner  {
        initPrice=priceInWei;
    }
	
    function getEndangerLevel(string memory speciesName) external view returns (string memory) {
        return endangerLevel[speciesName];
    }
    function setEndangerLevel(string memory speciesName, string memory newLevel) external onlyOwner  {
        endangerLevel[speciesName]=newLevel;
    }

    function getNFTurtleOwnerNote(uint256 id) external view returns (string memory) {
        return NFTurtleOwnerNote[id];
    }
    function setNFTurtleOwnerNote(uint256 id, string memory newNote) external   {
		require(_isApprovedOrOwner(_msgSender(), id), "ERC721: caller is not owner nor approved");
        NFTurtleOwnerNote[id]=newNote;
    }
