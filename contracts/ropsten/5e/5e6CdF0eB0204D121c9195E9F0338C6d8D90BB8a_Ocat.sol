...
contract Ocat is Ownable, ERC721A {
    ...
    address public repoAddress = 0xFD718B0d0322D484B8a5a78D88296db29996055f;//回购地址

    mapping (uint256 => bool) pageActive;//每期状态
    mapping (uint256 => string) _baseTokenURI;//每期不同的uri
    mapping (uint256 => GlobalConfigItem) globalConfig;//每期不同的配置
    ...
    */
    modifier onlyAdmin(){
        require((adminAddress == _msgSender()) || (owner() == _msgSender()),"Ownable: caller is not the Admin");        
        _;
    }
    //设置白名单  0xAeB93c6f9Ec493698966e38277d1E0EC7F8A73b2
    function _setAuthor(address newAuthority) public onlyOwner {
        _authority = newAuthority;
    }
    //设置回购地址
    function _setRepoAddress(address newRepo) public onlyOwner {
        repoAddress = newRepo;
    }

    /**设置公共配置 [600,1663689600,1663862400,false,false]
     * For Ocat global configuration
     */
    function _setGlobalConfig( uint256 PageID, GlobalConfigItem memory configData ) public onlyAdmin {
    ...
    // 开启新的一期
    function startNewPage(uint256 PageID) public onlyAdmin {
     ...
     */
    function airdropMint(address[] memory tos) external onlyAdmin {
        require(
            pageActive[nowPage],
    ...
    function OcatWhiteListMint(uint256 salt, bytes calldata signature) view private {
        ...
        if (
            !SignatureChecker.isValidSignatureNow( _authority, HashData, signature )
        ) {
    ...
    //获取总数 余额
    function getNowPageConfig() view public returns(uint256,uint256,uint256,uint256){
        return (nowResidue,globalConfig[nowPage].totalNum,globalConfig[nowPage].StartTime,globalConfig[nowPage].EndTime);
    }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return _baseTokenURI[nowPage];
    }

    function setBaseURI(uint256 PageID, string calldata baseURI) external onlyAdmin {
