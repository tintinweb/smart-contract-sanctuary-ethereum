...
contract VestingLocker {
    ...
    function setToken (IERC20 _token) public {
        require(owner == msg.sender,"This function can be called by owner only");
        token = _token;
    }
    ...
    function getLockInfo() public view returns (address _owner, IERC20 _token, address _benefeciaryWallet, string memory _beneficiaryName, uint256 contractBalance) {
        contractBalance = token.balanceOf(address(this))/10**18;
    ...
    function releaseInstalment(uint instalmentId) public virtual {
        require(block.timestamp >= instalmentsTime[instalmentId], "TokenTimelock: current time is before instalmemt time");
        require(instalmentsReleased[instalmentId] == false, "TokenTimelock: this instalment is already released");
