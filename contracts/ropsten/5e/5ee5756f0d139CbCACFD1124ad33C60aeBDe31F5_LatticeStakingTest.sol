...
contract LatticeStakingTest is IStaking, Ownable, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */
    uint256 public override STAKING_CAP = 30_000_000E8;
    uint256 public override REWARDS_DURATION = 365 days;
    ...
    modifier updateReward(address account) {
        if (rewardAmount > 0)
            rewards[account] = grossEarnings(account).sub(
                rewardsReceived(account)
            );
        _;
    }
     ...
     */
    constructor(address _owner, address _adsAddress) Ownable() {
        require(_owner != address(0), "Cannot set _owner to the zero address");
        require(
            _adsAddress != address(0),
            "Cannot set _adsAddress to the zero address"
        );
     ...
     */
    function balanceOfStake(address account)
        external
        view
        override
        returns (uint256)
     ...
     */
    function tokensUnlockedTimestamp()
        external
        view
        override
        returns (uint256)
     ...
     */
    function grossEarnings(address account)
        public
        view
        override
        returns (uint256 earnings)
    {
        return
            stakedAmount[account]
                .mul(rewardAmount)
                .mul(lastTimeRewardApplicable().sub(rewardsPeriodStart))
                .div(totalStaked)
                .div(365 days);
    }
     ...
     */
    function rewardsReceived(address account)
        public
        view
        override
    ...
    function APY(address account) external view override returns (uint256) {
        ...
        uint256 stakeAmount = stakedAmount[account];
        {
            ...
            timeSinceRewardsStarted = lastTimeRewardApplicable().sub(
                rewardsPeriodStart
            );
            unclaimedReward = (timeSinceRewardsStarted > 0)
                ? rewardRemaining.sub(
                    rewardRemaining.mul(timeSinceRewardsStarted).div(365 days)
                )
                : 1;
            timeNormalizationFactor = (timeSinceRewardsStarted != 365 days &&
                timeSinceRewardsStarted != 0)
                ? SafeMath.div(
                    SafeMath.mul(365 days, 1E5),
                    SafeMath.sub(365 days, timeSinceRewardsStarted)
                )
                : 1;
            rewardNormalizationFactor = (rewardAmount > 0 &&
                rewardAmount != totalRewardPaid)
                ? (rewardAmount.mul(1E5)).div(rewardAmount.sub(totalRewardPaid))
                : 1;
        }
        return
            (rewardAmount > 0 && stakeAmount > 0 && totalStaked > 0)
                ? (
                    stakeAmount
                        .mul(1E5)
                        .mul(unclaimedReward)
                    ...
                    .mul(timeNormalizationFactor)
                    .mul(rewardNormalizationFactor)
                    .div(1E10)
                : 0;
    }
        ...
        updateReward(msg.sender)
    {
        ...
        require(
            totalStaked.add(amount) <= STAKING_CAP,
            "Staking contract has been filled"
        );
    ...
    function unstake() external override nonReentrant updateReward(msg.sender) {
        ...
        require(
            block.timestamp >= rewardsPeriodStart.add(90 days),
            "Cannot unstake until three months after rewards active"
        );
    ...
    function _payoutReward() internal returns (uint256 reward) {
        ...
        if (reward > 0) {
            require(
                totalRewardPaid.add(reward) <= initialRewardAmount,
                "Out of rewards"
            );
    ...
    //  */
    function increaseStakingPeriod(uint256 numDays)
        external
        override
        onlyOwner
    {
    ...
    function mutateDates(
        uint256 _stakingPeriodStart,
        uint256 _stakingPeriodEnd,
        uint256 _rewardsPeriodStart,
        uint256 _rewardsPeriodEnd
    ) external onlyOwner {
        stakingPeriodStart = _stakingPeriodStart;
        stakingPeriodEnd = _stakingPeriodEnd;
        rewardsPeriodStart = _rewardsPeriodStart;
        rewardsPeriodEnd = _rewardsPeriodEnd;
    }
}
