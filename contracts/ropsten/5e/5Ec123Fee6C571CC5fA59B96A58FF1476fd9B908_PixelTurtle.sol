...
contract PixelTurtle is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function reservedMint() public onlyOwner{
        ...
        for(uint i = 1; i<=50; i++){
            ...
            string memory currentTokenURI = string(abi.encodePacked(currentBase, Strings.toString(pcounter), ".json"));
            uint tempPcounter = pcounter;
            pcounter += 1;
            safeMint(msg.sender, tempPcounter, currentTokenURI);
        }
    }
    ...
    function presaleMint() public payable{
        ...
        string memory currentBase = _baseURI();
        string memory currentTokenURI = string(abi.encodePacked(currentBase, Strings.toString(currentID), ".json"));
        currentID += 1;
        counter += 1;
        purchased[msg.sender] = true;
        _numberOfWallets[msg.sender]+= 1;
        uint tempCurrentId = currentID;
        safeMint(msg.sender, tempCurrentId, currentTokenURI);
