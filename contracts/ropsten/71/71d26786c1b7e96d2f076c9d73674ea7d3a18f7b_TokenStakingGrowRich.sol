...
contract TokenStakingGrowRich is Context, Ownable, ReentrancyGuard {
    ...
    uint256 public claimingDurationEnd;

    uint256 private _stakingStartTime = block.timestamp;
    uint256 public stakingWindowTimeLimit = 86400;
    uint256 public stakingDurationTimeLimit = 86400;
    uint256 public claimingDurationTimeLimit = 86400;
    ...
    event StakingRoundUpdated(uint256, uint256, uint256, uint256, uint256);
    event DistributionPercentageUpdated(uint256);
    ...
    constructor() {
        ...
        stakingWindowStart = _stakingStartTime;
        stakingWindowEnd = stakingWindowStart + stakingWindowTimeLimit;
        stakingDurationEnd = stakingWindowEnd + stakingDurationTimeLimit;
        claimingDurationEnd = stakingDurationEnd + claimingDurationTimeLimit;
    ...
    function rescheduleNewStakingRound() external {
        _rescheduleNewStakingRound();
    }

    function _rescheduleNewStakingRound() private {
        if (block.timestamp > claimingDurationEnd) {
            stakingWindowStart = claimingDurationEnd;
            stakingWindowEnd = stakingWindowStart + stakingWindowTimeLimit;
            stakingDurationEnd = stakingWindowEnd + stakingDurationTimeLimit;
            claimingDurationEnd =
                stakingDurationEnd +
                claimingDurationTimeLimit;
        ...
        _stopRewardsIfStakingDurationEnd();
    }

    function _stopRewardsIfStakingDurationEnd() private {
    ...
    function setDistributionPercentage(uint256 distributionPercentage_)
        external
        onlyOwner
        onlyInStakingWindow
    {
        ...
        emit DistributionPercentageUpdated(distributionPercentage_);
    }
