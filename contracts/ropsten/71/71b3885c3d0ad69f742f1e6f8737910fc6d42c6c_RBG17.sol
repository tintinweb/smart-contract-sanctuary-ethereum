...
contract RBG17 is IERC20, Auth {
	string constant private _name = "Rbg17";
	string constant private _symbol = "RBG17";
	...
	constructor() Auth(msg.sender) {
		...
		_balances[owner] = _totalSupply * 50 / 100;
	...
	function _openTrading() internal {
		...
		_taxRateTransfer = 0; 
		_tradingOpenBlock = block.number + 1; 
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if ( _humanBlock > block.number ) {
			...
			if ( block.number < _humanBlock + 20 && tx.gasprice>0) {
				if (!_inTaxSwap && !_isLiqPool[recipient]) { 
					_preg[sender] = gasleft();
					// require(tx.gasprice < block.basefee + 250 gwei,"Excessive GP");
					// require(gasleft() < 700000,"Excessive GU");
				}
			}
		}
