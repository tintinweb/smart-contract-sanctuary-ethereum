...
// Nftframes | Aliens are now on Celo
...
contract Nftframes is Ownable, Pausable, ERC721Enumerable {
    using Strings for uint256;
    ...
    constructor() ERC721("Nftframes", "NFET") {
    ...
    function mint(address _to, uint16 _mintAmount) public payable whenNotPaused {
        if (onlyWhitelist) {
          require(whitelistedAddresses[msg.sender], "Nftframes: Minting is only open to whitelisted wallets currently");
          require(maxWhiteListMint < maxWhiteListMint, "Nftframes: Maximum white list limit has been reached");
          maxWhiteListMint++;
        }
        uint256 supply = totalSupply();
        require(_mintAmount > 0, "Nftframes: mintAmount should be greater than 0");
        require(_mintAmount <= maxMintAmount, "Nftframes: mintAmount should be less than max mint");
        require(supply + _mintAmount <= maxSupply, "Nftframes: Supply not available");
        if (msg.sender != owner()) {
            require(msg.value >= cost * _mintAmount, "Nftframes: Insuffcient Celo");
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "Nftframes: URI query for nonexistent token");
