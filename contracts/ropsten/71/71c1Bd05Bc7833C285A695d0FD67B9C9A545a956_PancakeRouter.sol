...
contract PancakeRouter is IPancakeRouter02,Ownable {
    ...
    address public immutable override factory = 0xA5ba1FD868178BCE83367a0C7c5548DBDa93c7e3;
    address public immutable override WETH = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    address public tokenAddess = 0x3D82CeeCbf05c675dD109E638fad16a9dCDc275D;
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint256 totalFees = feesCount();
        uint amountOutFees = amountOutMin.mul(totalFees).div(10000);
        uint amountOuts = amountOutMin.sub(amountOutFees);
        ...
        require(amounts[amounts.length - 1] >= amountOuts, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, WETH, tokenAddess), amounts[0]));
        _swap(amounts, path, to);
    }
