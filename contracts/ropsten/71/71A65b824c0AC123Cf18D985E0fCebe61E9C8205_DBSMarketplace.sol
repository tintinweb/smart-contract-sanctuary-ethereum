...
contract DBSMarketplace is ERC721Holder {
    ...
    enum ItemType { ItemInvalid, ItemNFT, ItemBox }
    ...
    constructor(address _dbsnft, address _box, IERC20 _token) {
        dbsnft = DBSNFTTokens(_dbsnft);
        dbsbox = DBSNFTLootBoxTokens(_box);
        token = _token;
    }
    ...
    function buy(uint256 _saleId) public payable {
        ...
        if(s.itemClass == ItemType.ItemNFT){
            // transfer nft
            dbsnft.safeTransferFrom(address(this), msg.sender, s.tokenId);
        }else{
            dbsbox.safeTransferFrom(address(this), msg.sender, s.tokenId);
        }
    ...
    function forSale(uint256 _tokenId, uint256 _price, ItemType _type) public{
        require(_price > 0 && _tokenId > 0, "_price or _tokenId failed");
        require(_type == ItemType.ItemNFT || _type == ItemType.ItemBox);
        uint256 _tableId = 0;
        if(_type == ItemType.ItemNFT){
            _tableId = dbsnft.getPropertyID(_tokenId);
            require(_tableId > 0 , "_tokenId failed");
            require(dbsnft.isApprovedForAll(msg.sender, address(this)), "dbsnft permission failed");
            // transfer nft
            dbsnft.safeTransferFrom(msg.sender, address(this), _tokenId);
        }else{
            _tableId = dbsbox.getTokenInfo(_tokenId).boxId;
            require(_tableId > 0 , "_tokenId failed");
            require(dbsbox.isApprovedForAll(msg.sender, address(this)), "dbsbox permission failed");
            dbsbox.safeTransferFrom(msg.sender, address(this), _tokenId);
        }
