...
// Dependency file: contracts/IWERC721.sol

// pragma solidity ^0.5.0;

interface IWERC721 {
  event SetCustody(address indexed custodyAddress);
...
// Dependency file: contracts/utils/Strings.sol
...
// Dependency file: contracts/WERC721.sol

// pragma solidity ^0.5.0;

// import '/Users/lambda256/git/luniverse-bridge-contract/node_modules/openzeppelin-solidity/contracts/token/ERC721/ERC721Full.sol';
// import 'contracts/access/roles/SignerRole.sol';
// import 'contracts/IWERC721.sol';
// import 'contracts/utils/Strings.sol';

contract WERC721 is ERC721Full, SignerRole, IWERC721 {
	using Strings for uint256;

  string private _baseURI;
  address private _ERC721custody;
  ...
  constructor(
    ...
    _baseURI = _baseUri;
    _ERC721custody = _custodyAddress;
  }

  modifier onlyCustody() {
    require(getCustody() == msg.sender, "WERC721: caller must be a ERC721custody contract address");
    _;
  }

  function mint(address user, uint256 tokenId) external onlyCustody {
      _mint(user, tokenId);
  }

  function burn(uint256 tokenId) external onlyCustody {
      _burn(tokenId);
  } 

  function setCustody(address custodyAddress) public onlySigner{
    _ERC721custody = custodyAddress;
    emit SetCustody(custodyAddress);
  }

  function getCustody() public view returns (address) {
    return _ERC721custody;
  }
...
// Root file: contracts/ERC721Custody.sol
...
// import '/Users/lambda256/git/luniverse-bridge-contract/node_modules/openzeppelin-solidity/contracts/token/ERC721/IERC721Receiver.sol';
// import 'contracts/WERC721.sol';
// import 'contracts/ICustody.sol';
// import 'contracts/IBridge.sol';
// import 'contracts/access/roles/SignerRole.sol';
contract ERC721Custody is ICustody, SignerRole, IERC721Receiver {

  bytes32 private _tokenType; 
  ...
  modifier onlyBridge() {
    require(getBridge() == msg.sender, "ERC721 Custody: caller must be a bridge contract address");
    _;
  }
      ...
      onlyBridge
  {
      ...
      emit LockToken(bytes32(_tokenType), depositor, depositReceiver, originToken, tokenData);
  }
    ...
    {
      uint256 tokenId = abi.decode(bytes(tokenData), (uint256));
      _mintWrappedToken(wrappedToken, depositReceiver, tokenId);
      emit MintWrappedToken(bytes32(_tokenType), depositReceiver, wrappedToken, tokenData);
    }
  ...
  function _mintWrappedToken (
    ...
    private
  {
    WERC721(wrappedToken).mint(depositReceiver, tokenID);
  }
  ...
  {
    uint256 tokenId = abi.decode(bytes(tokenData), (uint256));
    _burnWrappedToken(withdrawer, wrappedToken, tokenId);
    emit BurnWrappedToken(bytes32(_tokenType), withdrawer, wrappedToken, tokenData);
  }

  function _burnWrappedToken (
    address withdrawer,
    address wrappedToken,
    uint256 tokenId
  )
    private
  {
    WERC721(wrappedToken).safeTransferFrom(
      withdrawer, 
      address(this), 
      tokenId
    );
    require(address(this) == WERC721(wrappedToken).ownerOf(tokenId), "ERC721Custody: INVALID_TOKEN_OWNER");
    WERC721(wrappedToken).burn(tokenId);
  }

  function setBridge(address bridgeAddress) public onlySigner{
  ...
  function tokenType() public view returns (bytes32) {
    return _tokenType;
  }
}