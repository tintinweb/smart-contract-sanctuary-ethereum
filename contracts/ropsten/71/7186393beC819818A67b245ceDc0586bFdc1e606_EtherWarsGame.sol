...
contract EtherWarsGame is ERC165, IERC721, IERC721Metadata, Ownable, Pausable {
    ...
    function mint(uint256 _amount) external payable whenNotPaused {
        ...
        if (_circulatingSupply < 2500 && _circulatingSupply + _amount < 2500) {
            require(msg.value >= _amount * _getCurrentPrice(0));
            if (whitelistOnly) {
                require(whitelist[_msgSender()], 'Address is not in whitelist');
            }
            if (msg.value > _amount * _getCurrentPrice(0)) {
                payable(msg.sender).transfer(msg.value - _amount * _getCurrentPrice(0));
            }
        } else if (_circulatingSupply < 2500 && _circulatingSupply + _amount >= 2500) {
            uint256 total = (_amount - _circulatingSupply + _amount - 2500) * _getCurrentPrice(0);
            ...
            Token.safeTransferFrom(
                _msgSender(),
                address(this),
                (_circulatingSupply + _amount - 2500) * _getCurrentPrice(1)
            );
        } else if (_circulatingSupply >= 2500 && _circulatingSupply + _amount < 6000) {
            Token.safeTransferFrom(_msgSender(), address(this), _amount * _getCurrentPrice(1));
        } else if (_circulatingSupply >= 2500 && _circulatingSupply + _amount >= 6000 && _circulatingSupply < 6000) {
            uint256 total = (_circulatingSupply + _amount - 6000) *
                _getCurrentPrice(2) +
                (_amount - _circulatingSupply + _amount - 6000) *
                _getCurrentPrice(1);
            Token.safeTransferFrom(_msgSender(), address(this), total);
        } else if (_circulatingSupply >= 6000) {
            Token.safeTransferFrom(_msgSender(), address(this), _amount * _getCurrentPrice(2));
        }
        for (uint256 i = 0; i < _amount; i++) {
            _circulatingSupply++;
            _safeMint(_msgSender(), _circulatingSupply);
