...
contract ERC721PresetMinterPauserAutoId is Context, AccessControlEnumerable, ERC721A, ERC721ABurnable, ERC721APausable {
     ...
     * See {ERC721-tokenURI}.
     */
    constructor(string memory name, string memory symbol, string memory baseTokenURI, string memory hiddenTokenURI, address owner) ERC721A(name, symbol, owner) {
    ...
    function _hiddenURI() internal view virtual override returns (string memory) {
...
contract MyNFTX is ERC721PresetMinterPauserAutoId, ReentrancyGuard {
    ...
    uint256 public PUBLIC_SALE_PRICE = .099 ether;
    ...
    constructor(
        address owner
    ) ERC721PresetMinterPauserAutoId("MyNFTX", "MNFTX", "Original URI/", "Hidden URI", owner) {}

    modifier onlyOwner() {
        require(_owner == msg.sender, "Only Owner can execute this");
        _;
    }
    ...
    function launch(uint256 whitelistedUserPresaletimePeriod, uint256 _teamMintQuantity) public onlyOwner {
        launched = true;
        launchedAt = block.timestamp;
        presaleStartAt = whitelistedUserPresaletimePeriod.mul(1 days).add(block.timestamp);
        teamMint(_teamMintQuantity);
    }

    function mintRemaining() public onlyOwner {
        require(_revealed, "Public Sale Not Finished Yet");
        _safeMint(msg.sender, MAX_SUPPLY.sub(totalSupply()));
    }

    function teamMint(uint256 _quantity) internal onlyOwner{
        require(!teamMinted, "Team already minted");
        teamMinted = true;
        _safeMint(msg.sender, _quantity);
    }

    function whiteListUsers(address[] calldata userList) public onlyOwner {
    ...
    function removeWhiteListUser(address userAddress) public onlyOwner {
        require(whitelisted[userAddress] == true, "User not whitelisted");
    ...
    function publicMint(uint256 _quantity) public nonReentrant payable {
        ...
        require(!_revealed, "Public Sale Finished");
        require(block.timestamp > presaleStartAt, "Public Sale not started");
    ...
    function whiteListedUsersPresaleMint(uint256 _quantity) public nonReentrant payable {
        require(launched, "Yet to be launched");
        require(!_revealed, "Sale Finished");
        require(whitelisted[msg.sender], "User is not whitelisted");
        require(totalSupply().add(_quantity) <= MAX_SUPPLY, "Exceeds NFT total supply");
        require(msg.value == WHITELIST_SALE_PRICE.mul(_quantity), "Insufficient ETH sent");
        require((totalPublicMint[msg.sender].add(_quantity)) <= MAX_WHITELIST_MINT, "Maximum minting quantity exceeded");
        _safeMint(msg.sender, _quantity);
    }
