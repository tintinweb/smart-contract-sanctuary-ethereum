...
contract GoingNuts is ERC721A, Allowlist, SalesActivation{
    ...
    uint256 public presale_purchased = 0;
    uint256 public presale_phase_max = 1000;
    uint256 public presaleListMax = 2;
    // OG claimed
    mapping(address => bool) private og_list;
    mapping(address => bool) private ogListBought;
    // Presale claimed
    address[] private addressesOfPresaleBought;
    mapping(address => uint256) private presaleListBought;
    // team addresses
    ...
    SalesActivation(_ogSalesStartTime,_ogSalesEndTime,_publicSalesStartTime,_preSalesStartTime,_preSalesEndTime) {
        setBaseURI(tokenURI);
    }
    ...
    */
    function presale(uint256 nutsNumber, bytes memory _signature) public payable isPreSalesActive isSenderAllowlisted(nutsNumber, _signature){
        uint256 supply = totalSupply();
        require( presale_purchased + nutsNumber <= presale_phase_max, 'Exceeds maximum nuts supply');
    ...
    */
    function mint(uint256 nutsNumber) public payable isPublicSalesActive {
        ...
        require(nutsNumber <= 20, "You are not allowed to buy this many nuts at once.");
    ...
    */
    function setOGPurchased(uint256 _max_number) public onlyOwner {
        og_purchased = _max_number;
    }
