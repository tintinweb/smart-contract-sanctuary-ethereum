...
contract BENTOKEN is Ownable, ERC20 {

	IRouter public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name =  "BENNOINU Token";
	string private constant _symbol = "BITOKEN";
	uint8 private constant _decimals = 18;

	BennoInuTokenDividendTracker public dividendTracker;
    
	mapping(address => bool) private _isBlackListedBot;
	address[] private _blackListedBots;

	// initialSupply
	uint256 constant initialSupply = 1000000000000000e18; //5000000000000000 * (10**18);
	...
	// Base taxes
	CustomTaxPeriod private _base = CustomTaxPeriod("base",0,0,1,1,3,3);
	...
	event FeeChange(string indexed identifier, uint8 liquidityFee, uint8 holdersFee);
	...
	event FeesApplied(uint8 liquidityFee, uint8 holdersFee, uint8 totalFee);
	event botAddedToBlacklist(address account);
    event botRemovedFromBlacklist(address account);


	constructor() ERC20(_name, _symbol) {
		dividendTracker = new BennoInuTokenDividendTracker();
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "BennoInuToken: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "BennoInuToken: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "BennoInuToken: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "BennoInuToken: Account is already the value of 'excluded'");
	...
	// Base fees
	function setBaseFeesOnBuy(uint8 _liquidityFeeOnBuy, uint8 _reflectionsFeeOnBuy) external onlyOwner {
		_setCustomBuyTaxPeriod(_base, _liquidityFeeOnBuy, _reflectionsFeeOnBuy);
		emit FeeChange("baseFees-Buy", _liquidityFeeOnBuy, _reflectionsFeeOnBuy);
	}
	function setBaseFeesOnSell(uint8 _liquidityFeeOnSell, uint8 _reflectionsFeeOnSell) external onlyOwner {
		_setCustomSellTaxPeriod(_base, _liquidityFeeOnSell, _reflectionsFeeOnSell);
		emit FeeChange("baseFees-Sell", _liquidityFeeOnSell, _reflectionsFeeOnSell);
	}
	function setUniswapRouter(address newAddress) external onlyOwner {
		require(newAddress != address(uniswapV2Router), "BennoInuToken: The router already has that address");
		emit UniswapV2RouterChange(newAddress, address(uniswapV2Router));
		uniswapV2Router = IRouter(newAddress);
		dividendTracker.setUniswapRouter(newAddress);
	}
	function setMaxTransactionAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxTxAmount, "BennoInuToken: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "BennoInuToken: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "BennoInuToken: Cannot update minimumTokensBeforeSwap to same value");
	...
	function claimETHOverflow(uint256 amount) external onlyOwner {
	    require(amount < address(this).balance, "BennoInuToken: Cannot send more than contract balance");
	...
	function getBaseBuyFees() external view returns (uint8, uint8){
		return (_base.liquidityFeeOnBuy, _base.reflectionsFeeOnBuy);
	}
	function getBaseSellFees() external view returns (uint8, uint8){
		return (_base.liquidityFeeOnSell, _base.reflectionsFeeOnSell);
	}
	...
	function _transfer(
		...
		uint256 amount
			...
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from] || !isSelltoLp) {
					require(amount <= maxTxAmount, "BennoInuToken: Buy amount exceeds the maxTxBuyAmount.");
				}
			if (!_isExcludedFromMaxWalletLimit[to] || !isSelltoLp) {
					require((balanceOf(to) + amount) <= maxWalletAmount, "BennoInuToken: Expected wallet amount exceeds the maxWalletAmount.");
				}
	...
	function _adjustTaxes(bool isBuyFromLp, bool isSelltoLp) private {
		...
		_totalFee = _liquidityFee + _reflectionsFee;
		emit FeesApplied(_liquidityFee, _reflectionsFee, _totalFee);
	}
	...
	function _swapAndLiquify() private {
		...
		uint256 totalETHFee = _totalFee - ((_liquidityFee / 2) + _reflectionsFee);
...
contract BennoInuTokenDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
