...
import './interfaces/IEcoinQuickConverter.sol';
import './interfaces/IEthshToken.sol';
import './interfaces/ISmartToken.sol';
import './interfaces/ITokenConverter.sol';

/*
    The EcoinQuickConverter contract provides allows converting between any token in the 
    ecoin network in a single transaction.

    A note on conversion paths -
    Conversion path is a data structure that's used when converting a token to another token in the ecoin network
...
*/
contract EcoinQuickConverter is IEcoinQuickConverter, TokenHolder {
    mapping (address => bool) public etherTokens;   // list of all supported ether tokens

    /**
        @dev constructor
    */
    function EcoinQuickConverter() {
    }
    ...
    */
    function registerEtherToken(IEthshToken _token, bool _register)
        public
        ownerOnly
        validAddress(_token)
        notThis(_token)
    ...
    /**
        @dev converts the token to any other token in the ecoin network by following
    ...
    */
        ...
        returns (uint256)
    {
        ...
        if (msg.value > 0)
            IEthshToken(fromToken).deposit.value(msg.value)(msg.sender);
        ...
        if (etherTokens[toToken])
            IEthshToken(toToken).withdrawTo(_for, _amount);
        else
    ...
    /**
        @dev claims the caller's tokens, converts them to any other token in the ecoin network
...
contract ITokenConverter {
    function getReturn(ISmartToken _token, uint256 _amount) public constant returns (uint256);
}
