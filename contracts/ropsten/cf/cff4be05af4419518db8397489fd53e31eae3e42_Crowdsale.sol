...
contract Crowdsale is UserPermissions {
    ...
    function finalizeStage(uint256 id) external onlyManyOwners {
        ...
        if (stage.availableTokens > 0) {
            _syndicateTokens = _syndicateTokens.add(stage.availableTokens);
    ...
    function _updatePurchasingState(uint256 weiAmount, uint256 usdAmount, uint256 tokens, bool isEarlyInvestor, address beneficiary) internal {
        if (isEarlyInvestor) _decreaseAllowance(beneficiary, usdAmount);
     ...
     */
    function _addAddressToGrantAccess(address _investor, uint256 _amountUsd) internal {
        uint256 privateSaleUsd = (_privateSale.availableTokens.mul(_privateSale.rate)).div(1 ether);
        require(_amountUsd.add(privateSaleUsd) >= 50000000, "addAddressToGrantAccess: the minimum amount for grant access should be bigger than $500k");
