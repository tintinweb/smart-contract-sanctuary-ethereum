...
contract NFTRewards is ReentrancyGuard, Ownable, Pausable {
  ...
  uint256 private constant STAKE_MULIPLE = 1000 ether;
  ...
  uint256 private constant UNSTAKE_MULIPLE = 100 ether;
  ...
  /**
   * @return time standard time for syncing rewards and stakes => 23:00 pm
   **/
  function standardTime() private view returns (uint256 time) {
    (uint256 year, uint256 month, uint256 day) = getYMD();
    time = BokkyPooBahsDateTimeLibrary.timestampFromDateTime(
      year,
      month,
      day,
      23,
    ...
    returns (uint256 rewards)
  {
    ...
    // ( (Amount * Staked Days) / 1000 ) + previous rewards
    ...
    stakelimits(_amount)
  {
    ...
    staker.rewards = staker.rewards.add(calculateRewards(msg.sender));
    staker.stakeTime = standardTime();
    ...
    unstakelimits(_amount)
  {
    ...
    // on unstaking we will calculate rewards uptill now
    staker.stakeTime = standardTime();
  ...
  modifier stakelimits(uint256 _amount) {
    require(_amount >= MIN_STAKE);
    require(_amount <= MAX_STAKE);
    require(_amount % STAKE_MULIPLE == 0);
    _;
  }

  modifier unstakelimits(uint256 _amount) {
    require(_amount >= MIN_UNSTAKE);
    require(_amount <= MAX_UNSTAKE);
    require(_amount % UNSTAKE_MULIPLE == 0);
    _;
  }
