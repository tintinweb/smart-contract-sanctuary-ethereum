...
contract SkrumbleStaking {
  ...
  function stakeTokensManually (address _staker, uint _level, uint time) public onlyUnlocked {
   ...
   // quick way to set a reward level to spec
  function unitTestSetRewardLevels () public {
    Reward storage r = rewardLevels[1];
    r.stakedAmount = 500000;
    r.lockupPeriod = 2678400;//1 month
    r.rewardAmounts = [50000];//10 percent
    r.rewardEpochStart = [2678400, 8035200, 31622400];//1 month// 3months // 1year
  }
  
  
  // use to double check math on availableReward
  function unitTestGetAvailableReward (uint since, uint last) view public returns (uint) {
    stakerMap[0x00] = Staker(500000, 1, now - since, now - last);
    return getAvailableReward(0x00);
  }
  
  
  // use to modify the stakingSince and LastClaim settings on a staker so you can simulate reward payouts
  function unitTestModifyStaker (uint since, uint last) public {
    stakerMap[msg.sender].stakingSince = since;
    stakerMap[msg.sender].lastClaim = last;
  }
