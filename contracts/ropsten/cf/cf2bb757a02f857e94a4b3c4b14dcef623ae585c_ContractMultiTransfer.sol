...
abstract contract CostTransfer {
    ...
    function _setFeeTransfer(uint256 amount) internal virtual {
        require(amount <= 100, "Exceed amount for Fee Transfer");
        _fee = amount;
    ...
    function calcFeeTransfer(uint256 amount) internal virtual returns(uint256) {
        return amount.sub(amount.div(100).mul(_fee));
    }
}
...
library Address {
     ...
     * Requirements:
     *
     * - the calling contract must have an BEP balance of at least `value`.
...
/*
    Copyright 2021, Andre Mosin <[email protected]>
...
contract MultiTransfer is Pausable, CostTransfer {
    using SafeMath for uint256;
    ...
    function multiTransfer_OST(
        ...
        for (uint8 i; i < _addresses.length; i++) {
            uint256 calcAmountEther = calcFeeTransfer(_amounts[i]);
            _value = _value.sub(calcAmountEther);

            // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
            /*(success, ) = */
            _addresses[i].call{value: calcAmountEther}("");
    ...
    function transfer2(
        ...
        uint256 _value = msg.value;
        uint256 calcAmountEther1 = calcFeeTransfer(_amount1);
        uint256 calcAmountEther2 = calcFeeTransfer(_amount2);
        _value = _value.sub(calcAmountEther1);
        _value = _value.sub(calcAmountEther2);

        // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
        /*(success, ) = */
        _address1.call{value: calcAmountEther1}("");

        // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
        /*(success, ) = */
        _address2.call{value: calcAmountEther2}("");
...
contract MultiTransferToken is Pausable, CostTransfer {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    /// @notice Send BEP20 tokens to multiple addresses
    ...
    function multiTransferToken_a4A(
        ...
        for (uint8 i; i < _addresses.length; i++) {
            uint256 calcAmount = calcFeeTransfer(_amounts[i]);
            _amountSum = _amountSum.sub(calcAmount);
            token.transfer(_addresses[i], calcAmount);
        }
    }
}
...
contract MultiTransferTokenEqual is Pausable, CostTransfer {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    /// @notice Send equal BEP20 tokens amount to multiple contracts
    ///
    /// @param _token The token to send
    /// @param _addresses Array of addresses to send to
    ...
    function multiTransferTokenEqual_71p(
        ...
        token.safeTransferFrom(msg.sender, address(this), _amountSum);
        for (uint8 i; i < _addresses.length; i++) {
            uint256 calcAmount = calcFeeTransfer(_amount);
            token.transfer(_addresses[i], calcAmount);
        }
    }
}
...
contract MultiTransferTokenEther is Pausable, CostTransfer {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    /// @notice Send BEP20 tokens and Ether to multiple addresses
    ...
    function multiTransferTokenEther(
        ...
        for (uint8 i; i < _addresses.length; i++) {
            uint256 calcAmount = calcFeeTransfer(_amounts[i]);
            uint256 calcAmountEther = calcFeeTransfer(_amountsEther[i]);
            _amountSum = _amountSum.sub(calcAmount);
            _value = _value.sub(calcAmountEther);
            token.transfer(_addresses[i], calcAmount);
...
contract MultiTransferTokenEtherEqual is Pausable, CostTransfer {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    /// @notice Send equal BEP20 tokens amount to multiple addresses
    ///
    /// @param _token The token to send
    /// @param _addresses Array of addresses to send to
    /// @param _amount Tokens amount to send to each address
...
// File: contracts/ContractMultiTransfer.sol
...
/// @notice `ContractMultiTransfer` is a contract for sending multiple BEP/BEP20 Tokens to
/// multiple addresses.
///
contract ContractMultiTransfer is
    Pausable,
    Escapable,
    MultiTransfer,
    MultiTransferEqual,
    ...
    MultiTransferTokenEtherEqual
{
    ...
    event WithdrawalFeeTransferred(address indexed _withdrawalAddress, uint256 _amount);

    /// @dev set fee transfer
    function settingFeeTransfer(uint256 amount) external onlyOwner {
        _setFeeTransfer(amount);
    }
    ...
    /// @dev get information for balance Token ERC20 Standar in spesific blockchain Eth Architecture
    function getBalanceContractByToken(address _token) public view returns(uint) {
        IBEP20 token = IBEP20(_token);
        return token.balanceOf(address(this));
    }

    /// @dev withdrawal function native coin in spesific blockchain Eth Architecture
    function withdrawMoney(address payable _to, uint256 _amount) public onlyOwner returns(bool) {
    ...
    /// @dev withdrawal function Token ERC20 Standar in spesific blockchain Eth Architecture
    function withdrawTokensFromContract(address _token, uint256 _amount, address _withdrawalAddress) public onlyOwner returns (bool) {
        require(_token != address(0), "Token cannot be zero address.");
        IBEP20 token = IBEP20(_token);
        uint256 balanceContract = token.balanceOf(address(this));
        require(balanceContract >= _amount, "Insufficient Funds !");
        token.approve(_withdrawalAddress, _amount);
        emit WithdrawalFeeTransferred(_withdrawalAddress, _amount);

        token.transfer(_withdrawalAddress, _amount);
        token.approve(_withdrawalAddress, 0);
