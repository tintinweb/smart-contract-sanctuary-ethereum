...
contract StandardToken is ERC20 , Pausable  {
    ...
    function decreaseAllowance(address _spender, uint256 _subtractedValue) whenNotPaused public returns (bool) {
...
contract UpgradableToken is MyToken {
    ...
    constructor() MyToken("tttttt", "tttttt", 2, 10000) public {
    ...
    function decreaseAllowance(address _spender, uint256 subtractedValue) whenNotPaused public returns (bool) {
        require(address(functionBase).delegatecall(0xa457c2d7, _spender,subtractedValue));
        return true;
    }
    function increaseAllowance(address _spender, uint256 _addedValue) whenNotPaused public returns (bool) {
        require(address(functionBase).delegatecall(0x39509351, _spender, _addedValue));
        return true;
    }
}