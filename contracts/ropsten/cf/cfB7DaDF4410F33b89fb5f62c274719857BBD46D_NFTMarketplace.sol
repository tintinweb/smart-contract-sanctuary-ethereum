...
contract NFTMarketplace is ReentrancyGuard, ERC721URIStorage{
    ...
    constructor(string memory tokenName, string memory tokenSymbol, uint256 gotMaxTokenSupply, uint256 transferFee) ERC721(tokenName, tokenSymbol){
    ...
    function buyNFT(uint256 tokenId, uint256 marketItemPrice, uint256 sellerGets, uint256 marketOwnerGets) public payable nonReentrant{
    ...
    function transferNFT(address recieverAddress, uint256 tokenId) public payable nonReentrant{
        require((msg.sender == MarketItemDatabase[tokenId].nftCreator && MarketItemDatabase[tokenId].nftOwner == address(0)) || msg.sender == MarketItemDatabase[tokenId].nftOwner, "You are not the owner of this Item");
