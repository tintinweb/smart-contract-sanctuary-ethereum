   ...
   //описание номера очереди и номер депозита в очереди
    ...
    function () public payable {
        ...
        if(msg.value > 0){
            ...
            require(getStageStartTime(stage + 1) >= now + MAX_IDLE_TIME); //нельзя инвестировать если время старта позже - MAX_IDLE_TIME
...
//отправка приза
    ...
    function getCurrentStageByTime() public view returns (int) {
        ...
        //int(now - 17 hours) / 1 days - 17837; //Start is 02/11/2018 20:00 GMT+3
    }

    function getStageStartTime(int _stage) public pure returns (uint) {
        return 17841 * 86400 + 7 * 3600 + uint((_stage + 1) * 10 * 60);
        //17 hours + uint(_stage + 17837)*1 days;
    }
