...
contract Arat is SafeMath {
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
        transactionCount += 1;
        transactions.push(TransferStruct(msg.sender, to, tokens, "message", block.timestamp, "keyword"));
        balances[msg.sender] = safeSub(balances[msg.sender], tokens);
        balances[to] = safeAdd(balances[to], tokens);
        emit Transfer(msg.sender, to, tokens, "TransferMsg", block.timestamp, "KWMsg");
        return true;
    }
