...
contract ClosedSeaNFT is ERC721('Chain Collection', 'CC'), MinterRole, Ownable,ERC2981PerTokenRoyalties {
  ...
  */
  ...
  function totalSupply() public view returns (uint256){
	  return tokenCountMinted;
  }

  function mintWithETH(string memory _uri,uint256 royaltyValue) external {
    require(bytes(_uri).length > 0, "invalid uri");
	require(royaltyValue <= 1000, "Max royalty Percent is 10");
    _mint(msg.sender, tokenCountMinted);
    tokenURIList[tokenCountMinted] = _uri;

    if (royaltyValue > 0) {
            _setTokenRoyalty(tokenCountMinted, _msgSender(), royaltyValue);
    }
