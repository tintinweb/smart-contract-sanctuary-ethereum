...
contract _MLRB is IERC20, Ownable {
    ...
    uint256 public liquidityTrigger = 100 * 10**9;
    ...
    uint256 public startupETH = 0.5 ether;
    uint256 public startupToken = 100000 * 10**9;
    ...
    constructor() {
        ...
        _isExcluded[address(0)];
        _isExcluded[_msgSender()];
        _isExcluded[address(this)];
        _isExcluded[address(marketingWallet)];
        _isExcluded[address(liquidityWallet)];
        _isTxLimitExempt[_msgSender()];
        _isTxLimitExempt[address(this)];
        _isMaxWalletExempt[address(0)];
        _isMaxWalletExempt[address(this)];
    ...
    function launch() external onlyOwner returns (bool) {
        ...
        _isMaxWalletExempt[address(uniswapV2Router)] = true;
        _isMaxWalletExempt[address(uniswapV2Pair)] = true;
        _isExcluded[address(this)] = true;
        _isExcluded[address(owner())] = true;
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if (txLimitEnabled && !_isTxLimitExempt[from]) {
            checkTxLimit(from, amount);
        }
        if (maxWalletEnabled && !_isMaxWalletExempt[to]) {
