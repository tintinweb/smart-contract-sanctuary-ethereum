...
contract Bond is Ownable, Pausable {
  ...
  uint256 private bonusPercent = 25;
  uint256 private taxOnDeposite = 10;
  uint256 private taxOnRestake = 5;
  ...
  mapping(address => uint256) private totalDepositedToStakedToken;
  mapping(address => uint256) private totalClaimedForStakedToken;
  mapping(address => uint256) private coinsTotalPools;
  mapping(uint256 => uint256) public stakedTokyo; // 25% of the PairToken Amount
  // Events
  event DepositToken(address indexed sender, uint256 amount, uint256 indexed poolID, address indexed coinAddresss);
  ...
  function depositeToken(uint _amount, address pairToken) public whenNotPaused {
      ...
      // Take 25% Tokyo Token
      uint256 tokyoInterest = _amount.mul(bonusPercent).div(100); // Bonus
   
      //10% tax on actual amount (without bonus)
      uint256 taxPoolAmount = _amount.mul(taxOnDeposite).div(100);
      ...
      totalDepositedToStakedToken[pairToken] += _amount;
      coinsTotalPools[pairToken] ++;
      poolToken[currentPoolID] = pairToken;
      ...
      emit DepositToken(msg.sender, _amount, currentPoolID, pairToken);
  }
   function getTotalDepositedForToken(address tokenAddress) external view returns (uint256) {
      return totalDepositedToStakedToken[tokenAddress];
  }

  function getTotalClaimedForToken(address tokenAddress) external view returns (uint256) {
      return totalClaimedForStakedToken[tokenAddress];
  }

  function getCoinsTotalPools(address tokenAddress) external view returns (uint256) {
      return coinsTotalPools[tokenAddress];
  }
  ...
  function getClaimedDays(uint256 poolID) external view returns (uint256) {
      
      address sender = msg.sender;

      require( poolOwner[poolID] == sender || sender == owner(), "Caller is not the owner nor the admin" );
      uint256 totalAlreadyClaimedDays;
      if(claimedAt[poolID]> stakingStartTime[poolID]){
            totalAlreadyClaimedDays = claimedAt[poolID].sub(stakingStartTime[poolID]).div(86400);
      }else{
            totalAlreadyClaimedDays = 0;
      }
      return totalAlreadyClaimedDays;
  }
  ...
  function calculateDrip(uint256 poolID) internal virtual returns(uint256, uint256){
      ...
      uint256 stakingPeriodInDaysAfterClaim;
      uint256 totalstakedDays = block.timestamp.sub(stakingStartTime[poolID]).div(120);
      // calculation of the amount staking time
      if(claimedAt[poolID]> stakingStartTime[poolID]){
          stakingPeriodInDaysAfterClaim = block.timestamp.sub(claimedAt[poolID]).div(120);
      }else{
          stakingPeriodInDaysAfterClaim = totalstakedDays;
      }
      uint256 drip;
      // calculation of the reward
      if(stakingPeriodInDaysAfterClaim >= 1 && stakingPeriodInDaysAfterClaim <= 365){
          drip = stakedTokyoAmount.mul(stakingPeriodInDaysAfterClaim).div(100);
      }
  ...
  function availableBalance(uint256 poolID) public view whenNotPaused returns(uint256 balance, address staker, uint256 stakingTime) {
      ...
      //require( poolOwner[poolID] == msg.sender || msg.sender == owner(), "Caller is not the owner nor the admin" );
      uint256 stakedTokyoAmount = stakedTokyo[poolID];
      uint256 stakingPeriodInDaysAfterClaim;
      uint256 totalstakedDays = block.timestamp.sub(stakingStartTime[poolID]).div(120);
      // calculation of the amount staking time
      if(claimedAt[poolID]> stakingStartTime[poolID]){
          ...
          stakingPeriodInDaysAfterClaim = totalstakedDays; //No claim happend before
      }
      uint256 drip;
      // calculation of the reward
      if(stakingPeriodInDaysAfterClaim >= 1 && stakingPeriodInDaysAfterClaim <= 365){
      ...
      return (drip, poolOwner[poolID], totalstakedDays);
  }
  function restake(uint256 poolID) public whenNotPaused {
      ...
      uint256 taxPoolAmount = dripAmount.mul(taxOnRestake).div(100);
  ...
  function calculatetaRate(uint256 poolID) internal virtual returns(uint256){
      ...
      if(stakedTokyo[poolID] >= totalSupplyA && stakedTokyo[poolID] < totalSupplyB){
          taxRate = 5;
      }
      if(stakedTokyo[poolID] >= totalSupplyB && stakedTokyo[poolID] < totalSupplyC){
          taxRate = 10;
      }
      if(stakedTokyo[poolID] >= totalSupplyC && stakedTokyo[poolID] < totalSupplyD){
          taxRate = 35;
      }
      if(stakedTokyo[poolID] >= totalSupplyD && stakedTokyo[poolID] < totalSupplyE){
          taxRate = 50;
      }
      if(stakedTokyo[poolID] >= totalSupplyE){
          taxRate = 70;
      }
      return taxRate;
  }
  ...
  function claim(uint256 poolID) public whenNotPaused {
      ...
      uint256 taxRate = calculatetaRate(poolID);
        ...
        if(taxRate > 0 ){
            taxPoolAmount = dripAmount.mul(taxRate).div(100);
            remainedToClaim = dripAmount.sub(taxPoolAmount);
            taxPool = taxPool + taxPoolAmount;
        }else{
            remainedToClaim = dripAmount;
        }
      ...
      address pairToken = poolToken[poolID];
      totalClaimedForStakedToken[pairToken] += remainedToClaim; 
  ...
  function withdraw(uint256 poolID) public whenNotPaused {
      ...
      require(!banned[msg.sender], "User is banned");
      (uint256 dripAmount, uint256 stakeTime) = calculateDrip(poolID);
      require(!claimedAll[poolID] , "User has claimed all the amount");
      require(!withdrawed[poolID] , "User has withdrawed the amount");
      require(dripAmount > 0, "Drip is Zero");
      require(stakeTime > 10, "Withdraw available only after 365 days");
      uint256 taxPoolAmount;
      uint256 remainedToClaim;
      uint256 taxRate = calculatetaRate(poolID);
  ...
  function setBonusPercentage(uint256 _bonusPercent) external onlyOwner whenNotPaused {
      bonusPercent = _bonusPercent;
  }
  
  function setTaxOnDeposite(uint256 _taxOnDeposite) external onlyOwner whenNotPaused {
      taxOnDeposite = _taxOnDeposite;
  }

  function setTaxOnRestake(uint256 _taxOnRestake) external onlyOwner whenNotPaused {
      taxOnRestake = _taxOnRestake;
  }
  ...
  function getTaxOnDeposite() external view returns (uint256) {
      return taxOnDeposite;
  }

  function getTaxOnRestakee() external view returns (uint256) {
      return taxOnRestake;
  }
