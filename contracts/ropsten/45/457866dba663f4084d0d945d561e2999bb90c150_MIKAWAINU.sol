...
contract MIKAWAINU is Context, IBEP20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0xf3DA821a0ED650FF3b94fc9E5470a26f9c65d84B);
    address payable public developerAddress = payable(0xf3DA821a0ED650FF3b94fc9E5470a26f9c65d84B);
    ...
    uint256 public collectedLiquidityTokens = 0;
    uint256 public collectedMarketingTokens = 0;
    uint256 public collectedDeveloperTokens = 0;
    ...
    function _transfer(address from, address to, uint256 amount) private 
    {
        ...
        {          
            if (collectedLiquidityTokens>_minimumTokensBeforeSwap) 
            {
                swapAndLiquify(_minimumTokensBeforeSwap);
                collectedLiquidityTokens -= _minimumTokensBeforeSwap;
            }
        }

        if (!inSwapAndLiquify && swapAndLiquifyEnabled && from != uniswapV2Pair && from != owner()) 
        {         
            if (collectedMarketingTokens>_minimumTokensBeforeSwap) 
            {
                swapTokensForEth(_minimumTokensBeforeSwap, marketingAddress);
                collectedMarketingTokens -= _minimumTokensBeforeSwap;
            }
        }

        if (!inSwapAndLiquify && swapAndLiquifyEnabled && from != uniswapV2Pair && from != owner()) 
        {         
            if (collectedDeveloperTokens>_minimumTokensBeforeSwap) 
            {
                swapTokensForEth(_minimumTokensBeforeSwap, developerAddress);
                collectedDeveloperTokens -= _minimumTokensBeforeSwap;
            }
        }
    ...
    function swapTokensForEth(uint256 tokenAmount, address _accountAddress) private {
    ...
    function sumUpCollectedFees(uint256 amount) private 
    {
        collectedLiquidityTokens += amount.mul(_liquidityFee).div(100);
        collectedMarketingTokens += amount.mul(_marketingFee).div(100);
        collectedDeveloperTokens += amount.mul(_developerFee).div(100);
    }
    ...
    function setAllBuyFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 developerFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _liquidityFee.add(_marketingFee).add(_developerFee);
    ...
    function setAllSaleFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 developerFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _saleLiquidityFee.add(_saleMarketingFee).add(_saleDeveloperFee);
