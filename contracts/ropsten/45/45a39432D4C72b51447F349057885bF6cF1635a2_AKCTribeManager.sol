...
contract AKCTribeManager is Ownable, ReentrancyGuard {
    ...
    bool test_sendToReserve = false;
    bool test_swapLiquidity = false;
    bool test_liquidityUniswapSwap = false;
    ...
    /** DEX HELPER METHODS */
    function addLiquidity(uint256 akcAmount) internal {
        ...
        if (test_liquidityAddLiquidity)
            uniswapV2AddLiquidity(toAKC, ethAdded);
    ...
    function uniswapV2Swap(uint256 akcAmount, address to) internal {
        ...
        router.swapExactTokensForETH(akcAmount, 0, path, to, block.timestamp);
    }
    ...
    function purchaseTribe(uint256 spec, string calldata identifier)
        ...
        nonReentrant { 
            ...
            if (enforceTerms && minimumReached && msg.sender != owner()) {
                ...
                if (test_swapRest)
                    uniswapV2Swap(toDistributor, address(akcDistributor));
            }
