...
contract TwapRelayer is ITwapRelayer {
    ...
    function sell(Orders.SellParams calldata sellParams) external payable override lock returns (uint256 orderId) {
        ...
        uint256 gasPrepay = msg.value > 0 ? (tx.gasprice - (tx.gasprice % 1e6)).mul(sellParams.gasLimit) : msg.value;
    ...
    function buy(Orders.BuyParams calldata buyParams) external payable override lock returns (uint256 orderId) {
        ...
        uint256 gasPrepay = msg.value > 0 ? (tx.gasprice - (tx.gasprice % 1e6)).mul(buyParams.gasLimit) : msg.value;
        ...
        )
    {
        ...
        if (isSellSide) {
            fee = amount.mul(swapFee[pair]).div(PRECISION);
            uint256 amountInMinusFee = amount.sub(fee);
            price = calculateAmountOut(pair, inverted, amountInMinusFee);
        } else {
            uint256 calculatedAmountIn = calculateAmountIn(pair, inverted, amount);
            fee = calculatedAmountIn.mul(swapFee[pair]).div(PRECISION);
            price = calculatedAmountIn.add(fee);
        }
    }
