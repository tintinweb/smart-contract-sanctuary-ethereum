...
contract Token is Context, IBEP20, Ownable {
    ...
    function totalMfee() public view returns (uint256) {
        return _tMTotal;
    }
    ...
    function updateFee(
        ...
        require(_txFee < 100 && _burnFee < 100 && _MFee < 100);
        _TAX_FEE = _txFee * 100;
        _BURN_FEE = _burnFee * 100;
        _M_FEE = _MFee * 100;
        ORIG_TAX_FEE = _TAX_FEE;
    ...
    function _transferStandard(
        ...
        if (takeFee) {
            _sendToFeeAddress(tMfee, sender);
        }
        _reflectFee(rFee, rBurn, tFee, tBurn, tMfee, takeFee);
    ...
    {
        (uint256 tFee, uint256 tBurn, uint256 tMFee) = _getTBasics(
            tAmount,
            _TAX_FEE,
            _BURN_FEE,
            _M_FEE
        ...
        )
    {
        ...
        uint256 tMFee = ((tAmount.mul(mFee)).div(_GRANULARITY)).div(100);
        return (tFee, tBurn, tMFee);
    }
    ...
    function getTTransferAmount(
        ...
        return tAmount.sub(tFee).sub(tBurn).sub(tMFee);
    }
    ...
    function _getRTransferAmount(
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rBurn).sub(rMFee);
        return rTransferAmount;
    }
    ...
    function _sendToFeeAddress(uint256 tMFee, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rMFee = tMFee.mul(currentRate);
    ...
    function removeAllFee() private {
        if (_TAX_FEE == 0 && _BURN_FEE == 0 && _M_FEE == 0) return;
