...
import '@swapify-official/swapify-core/contracts/interfaces/IERC20.sol';
import '@swapify-official/swapify-periphery/contracts/interfaces/ISwapifyRouter02.sol';
...
contract SwapifyOrderbookTradeFacet {
    ...
    function hasPair(address _tokenA, address _tokenB) public view returns (bool) {
        if (ISwapifyFactory(factory).getPair(_tokenA, _tokenB) == address(0)) {
            return false;
        }
        return true;
    }
        ...
        returns (uint256 _unmatchedTokenAmount)
    {
        ...
        if (_matchStatus == OrderBookStorage.MatchStatus.FULL || _matchStatus == OrderBookStorage.MatchStatus.PARTIAL) {
            //Generate Lottery
            ISwapifyLotterySystem(lottery).mintTickets(msg.sender, 1);
        }
    ...
    //     if(_matchStatus == OrderBookStorage.MatchStatus.FULL || _matchStatus == OrderBookStorage.MatchStatus.PARTIAL) {
    //         //Generate Lottery
    //         ISwapifyLotterySystem(lottery).mintTickets(_userAddress, 1);
    //     }
    //     // persist at userOrders
    //     ds.userOrders.push(_order);
    //     raiseTradeEvent(_orderType, _matchStatus, _order, _order.amount);
    ...
    function createUserOrder(
        ...
        if (hasPair(_tradedToken, _txToken)) {
            _pairName = bytes32(abi.encodePacked(ISwapifyFactory(factory).getPair(_tradedToken, _txToken)));
        }
        OrderBookStorage.OrderBook storage ds = OrderBookStorage.orderBookStruct();
        if (ds.buyOrders[_pairName][OrderBookStorage.ORDER_ID_OFFSET] == 0) {
            ds.buyOrders[_pairName][OrderBookStorage.ORDER_ID_OFFSET] = OrderBookStorage.ORDER_ID_OFFSET;
    ...
    function executeMatch(
        ...
        //Generate Lottery
        OrderBookStorage.OrderBook storage ds = OrderBookStorage.orderBookStruct();
        ISwapifyLotterySystem(lottery).mintTickets(ds.userOrders[matchingOrderIndex].user, 1);
    ...
    /**
     * @dev This internal function checks in Swapify Liquidity provider for full match of the Buy or Sell trade.
     */
    function checkLiquidityProviderMatch(
        ...
        if (_orderType == OrderBookStorage.OrderType.BUY) {
            (uint256 reserveA, uint256 reserveB, ) = ISwapifyPair(ISwapifyFactory(factory).getPair(_path[0], _path[1]))
                .getReserves();
            if (ISwapifyRouter02(router).getAmountOut(_txTokenAmount, reserveA, reserveB) >= _tradedTokenAmount) {
                return true;
            }
        }
        if (_orderType == OrderBookStorage.OrderType.SELL) {
            (uint256 reserveA, uint256 reserveB, ) = ISwapifyPair(ISwapifyFactory(factory).getPair(_path[0], _path[1]))
                .getReserves();
            if (ISwapifyRouter02(router).getAmountOut(_tradedTokenAmount, reserveA, reserveB) >= _txTokenAmount) {
                return true;
            }
        }
        return false;
    ...
    /**
     * @dev This internal function performs token swap using Swapify Swap liquidity provider
     */
    ...
    function performExactSwap(
        ...
        if (_path[0] == ISwapifyRouter02(router).WETH()) {
            ISwapifyRouter02(router).swapExactETHForTokens(_tokenOutAmount, _path, address(this), type(uint256).max);
            addToTokenBalance(msg.sender, _path[1], _tokenOutAmount);
        } else if (_path[1] == ISwapifyRouter02(router).WETH()) {
            ISwapifyRouter02(router).swapExactTokensForETH(
                _tokenInAmount,
                _tokenOutAmount,
                _path,
                address(this),
            ...
            ISwapifyRouter02(router).swapExactTokensForTokens(
                _tokenInAmount,
                _tokenOutAmount,
                _path,
                address(this),
...
library LibDiamond {
    bytes32 constant DIAMOND_STORAGE_POSITION = keccak256("diamond.standard.swapifyorderbook.storage");
