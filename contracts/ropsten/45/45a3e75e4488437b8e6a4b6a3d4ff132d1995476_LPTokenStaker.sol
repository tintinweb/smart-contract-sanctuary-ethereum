...
contract LPTokenStaker is Ownable, StakerStructs {
    ...
    address public constant PWRD = address(0xCAdC58879f214a47Eb15B3Ac6eCfBdC29fb17F28);
    // the pid of pwrd
    uint256 public pPid;
    ...
    function getUserPwrd(address user) public view returns (uint256) {
        ...
        uint256 diff = baseAmount * BASE % factor;
        uint256 result = baseAmount * BASE / factor;
        if (diff >= 5E17) {
            result = result + 1;
        }
        return result;
    ...
    function _prepPwrd(uint256 amount) internal view returns (uint256) {
        IPWRD pwrd = IPWRD(PWRD);
        uint256 factor = pwrd.factor();
        uint256 BASE = pwrd.BASE();
    ...
    function _checkPwrd(uint256 withdrawAmount, uint256 userAmount) internal view returns (uint256, uint256) {
        uint256 baseAmount = _prepPwrd(withdrawAmount);
        if (baseAmount - 1 == userAmount || baseAmount == userAmount) return (withdrawAmount, userAmount);
        else if (baseAmount > userAmount) return (_prepPwrd(userAmount), userAmount);
        else if (baseAmount * 1E4 / userAmount >= 9995) return (_prepPwrd(userAmount), userAmount);
