...
contract FINALINU is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("FINAL INU", "FINU") {
        ...
        swapTokensAtAmount = totalSupply * 5 / 100; // 0.5% swap wallet
        ...
        treasuryWallet = address(0x5bc67f156CA7974f2Cf88BE7f6Ad21C0d7Ea8368); // set as treasury wallet
        devWallet = address(0xcB92619fFdb350c0B3a419c2C645Be848399705A); // set as dev wallet
    ...
    function _transfer(
        ...
        // anti bot logic
        if (block.timestamp <= (launchedAt + 15) && 
                to != uniswapV2Pair && 
        ...
        if (!isBuy && enableEarlySellTax) {
            if (_holderFirstBuyTimestamp[from] != 0 &&
                (_holderFirstBuyTimestamp[from] + (2 minutes) >= block.timestamp))  {
