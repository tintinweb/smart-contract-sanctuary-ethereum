...
contract RNFTGeneric is ERC1155{
    ...
    if(msg.sender == RNFTProjectDetails[RNFTId].creator || msg.sender == contract_owner) {
     ...
     if(msg.sender == RNFTProjectDetails[RNFTId].creator && msg.sender != contract_owner && RNFTProjectDetails[RNFTId].status == RNFTStatus.OPEN) {
     valid = 0;
    }
    require(valid == 1, "NTC");
    ...
    function transferFrom(address from, address to, uint256 id, uint256 amount, bytes memory data) public virtual{
        require(RNFTProjectDetails[id].status != RNFTStatus.DENIED, 'osa');
      ...
      function getRNFTTotalRequets() public view returns(uint256 requests){
          return RNFTRequest;
      }
}
