 ...
 * @notice It receives Orbit protocol fees and owns the FeeSharingSystem contract.
...
contract FeeSharingSetter is ReentrancyGuard, AccessControl {
    ...
    constructor(
        ...
        orbitToken = feeSharingSystem.orbitToken();
     ...
     * based on the balance of Orbit in fee-staking addresses that exist in the set.
     */
    function updateRewards() external onlyRole(OPERATOR_ROLE) {
        ...
        if (numberAddressesForFeeStaking > 0) {
            uint256[] memory orbitBalances = new uint256[](numberAddressesForFeeStaking);
            ...
            for (uint256 i = 0; i < numberAddressesForFeeStaking; i++) {
                uint256 orbitBalance = orbitToken.balanceOf(_feeStakingAddresses.at(i));
                totalAmountStaked += orbitBalance;
                orbitBalances[i] = orbitBalance;
            }
            ...
            if (totalAmountStaked > 0) {
                ...
                for (uint256 i = 0; i < numberAddressesForFeeStaking; i++) {
                    uint256 amountToTransfer = (orbitBalances[i] * reward) / totalAmountStaked;
 ...
 * WETH along with the auto-compounding of Orbit.
 */
contract FeeSharingSystem is ReentrancyGuard, Ownable {
    ...
    /**
     * @notice Constructor
     * @param _orbitToken address of the token staked (ORBIT)
     ...
     * @dev There is a limit of 1 ORBIT per deposit to prevent potential manipulation of current shares
     */
    function deposit(uint256 amount, bool claimRewardToken) external nonReentrant {
        require(amount >= PRECISION_FACTOR, "Deposit: Amount must be >= 1 ORBIT");
        ...
        // Transfer ORBIT tokens to this address
        ...
        // Verify ORBIT token allowance and adjust if necessary
        _checkAndAdjustORBITTokenAllowanceIfRequired(amount, address(tokenDistributor));
    ...
    /**
     * @notice Calculate value of ORBIT for a user given a number of shares owned
     * @param user address of the user
     */
    function calculateSharesValueInORBIT(address user) external view returns (uint256) {
    ...
    /**
     * @notice Calculate price of one share (in ORBIT token)
     * Share price is expressed times 1e18
     */
    function calculateSharePriceInORBIT() external view returns (uint256) {
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(address(this));
     ...
     */
    function _checkAndAdjustORBITTokenAllowanceIfRequired(uint256 _amount, address _to) internal {
        if (orbitToken.allowance(address(this), _to) < _amount) {
            orbitToken.approve(_to, type(uint256).max);
        }
    }
    ...
    function _withdraw(uint256 shares, bool claimRewardToken) internal {
        ...
        // Retrieve total amount staked and calculated current amount (in ORBIT)
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(address(this));
        ...
        // Transfer ORBIT tokens to sender
        orbitToken.safeTransfer(msg.sender, currentAmount);
