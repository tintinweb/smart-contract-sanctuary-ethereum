...
library Lib {
    ...
    error Withdrawn();
    error UnsuccessfulHarvest();
    error OrderPaidBefore();
    error WrongData();
...
contract MRT is MrtToken {
    ...
    function regiserUser(string memory username) external {
        ...
        emit Lib.UserRegistred(username, msg.sender);
    }
    ...
    function toggleToken( address tokenAddress) external onlyOwner{
        ...
        emit Lib.TokenUpdate(tokenAddress,enabled);
    }
    ...
    **/
    ...
    function PayWithTokens(
        ...
        assembly {
             function paymentError(s){
                let h
                switch s
                case 1{
                    mstore(0x0, "OrderPaidBefore()")
                    h := keccak256(0x0, 17)
                }
                case 2{
                    mstore(0x0, "UnsuccessfulHarvest()")
                    h := keccak256(0x0, 21)
                }
                default {
                    mstore(0x0, "WrongData()")
                    h := keccak256(0x0, 11)
            ...
            slot := sload(hash)
            if iszero(and(slot, 0xFF)) {
                paymentError(3) // user does not exist
            }
            mstore(0x0, caller())
        ...
        emit Lib.OrderPaid(order_total,order_id,msg.sender,seller,contractAddress);
    }
    ...
    **/
    ...
    function payWithNativeToken(
        ...
        emit Lib.OrderPaid(msg.value,order_id,msg.sender,seller,address(0));
    }
    ...
    function widthrowForBuyers(
        ...
        emit Lib.WBuyer(order_id,msg.sender,seller);
    }
