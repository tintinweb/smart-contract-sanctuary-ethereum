...
contract xJoystickStaking is Authorizable, ReentrancyGuard {
    ...
    /**
        ...
        @param _list address[]
        @param _needle address
        @return bool
     */
    function listContains( address[] storage _list, address _needle ) internal view returns ( bool ) {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (_lastRewardTimestamp == 0) {
            _lastRewardTimestamp = block.timestamp - 60;
        }
    ...
    function claimReward() public {
        ...
        if (userBalance > user.amount) {
            _deposit(msg.sender, userBalance.sub(user.amount), address(0));
            return;
        }
