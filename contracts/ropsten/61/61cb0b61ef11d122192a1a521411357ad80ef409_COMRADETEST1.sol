...
contract COMRADETEST1 is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the COMRADE ERC-20 contract.
     */
    constructor() public ERC20("COMRADETEST1", "COMRADETEST1") {
        // Mints 1 COMRADETEST1 to contract creator for initial pool setup
    ...
    function _getCOMRADETEST1Price() internal view returns (uint256 _COMRADETEST1Price) {
        try IOracle(ComradeOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("COMRADETEST1: failed to fetch COMRADETEST1 price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _COMRADETEST1Price) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_COMRADETEST1Price >= taxTiersTwaps[tierId]) {
    ...
    function setComradeOracle(address _ComradeOracle) public onlyOperatorOrTaxOffice {
        require(_ComradeOracle != address(0), "oracle address cannot be 0 address");
        ComradeOracle = _ComradeOracle;
    }
    ...
    /**
     * @notice Operator mints COMRADETEST1 to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of COMRADETEST1 to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCOMRADETEST1Price = _getCOMRADETEST1Price();
            currentTaxRate = _updateTaxRate(currentCOMRADETEST1Price);
            if (currentCOMRADETEST1Price < burnThreshold) {
                burnTax = true;
            }
        }
