...
contract marketplace is ReentrancyGuard, ERC1155Holder
{
    ...
    uint public immutable feesPercent;
    ...
    function offerItem(IERC1155 _nft, uint _tokenId, uint _price, uint _amount, bytes calldata _bytes) external nonReentrant{
        ...
        items[itemCount]= Item (itemCount,_nft,_tokenId,_amount, _price,payable(msg.sender),false);
        bids[itemCount] = Bids(0, payable(msg.sender));

    }
    function puchaseNft(uint _itemId)  external payable nonReentrant{
    ...
    function bidmechanism(uint _itemId) external payable nonReentrant {
        Bids storage bid = bids[_itemId];
        Item storage item = items[_itemId];
    ...
    function accept_bid(uint _itemId) external payable nonReentrant{
        Bids storage bid = bids[_itemId];
        Item storage item = items[_itemId];
        uint _totalPrice = getTotalPrice(_itemId);
        ...
        item.Seller.transfer(bid.bidPrice-(bid.bidPrice*feesPercent/100));
        feeAccount.transfer(bid.bidPrice*feesPercent/100);

        item.sold = true;

        item.nft.safeTransferFrom(address(this),bid.Bidder, item.tokenId, item.qunatity,"");
