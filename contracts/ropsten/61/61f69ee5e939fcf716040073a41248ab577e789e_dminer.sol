...
contract dminer is Ownable, ReentrancyGuard {
    ...
    uint256 public constant Vortexia = 0.05 ether;
    uint256 public constant Diamond = 0.03 ether;
    uint256 public constant Gold = 0.01 ether;
    uint256 public constant Emerald = 0.005 ether;
    uint256 public constant Bronze = 0.003 ether;
    uint256 public constant Iron = 0.002 ether;
    ...
    */
    ...
    /* mapping to keep user total ticket */
    ...
    mapping(uint256=>uint256) public winningKey;
      ...
      /* Mapping of Lottery round to its winning key */
    mapping(address=>uint256) public usedTicket;
    ...
    function startRound(uint256 startTime, uint256 endTime) public onlyOwner {
        require(block.timestamp > RoundDetails[LotteryRounds].endTime, "Last Round has not ended");
        LotteryRounds++;
        RoundDetails[LotteryRounds] = LotteryRound(LotteryRounds, startTime, endTime, 0);
    }

    function buyTickets(uint256 quantity, uint256 tokenIdHolding) external payable nonReentrant {
        require(msg.value == ticketPrice.mul(quantity), "Send proper ticket fees");
        //require(DIGI_NFT.ownerOf(tokenIdHolding) == msg.sender, "You are not owner of NFT");
        nftUsed[LotteryRounds][tokenIdHolding] += quantity;
        require(nftUsed[LotteryRounds][tokenIdHolding] <= 10, "NFT used enough");
        LotteryRound storage round = RoundDetails[LotteryRounds];
        require(round.endTime>block.timestamp, "Lottery has been ended");
        require(round.ticketsSold < 100000, "All tickets are sold");
        userTicket[msg.sender]+=quantity;
        for(uint256 i= 0;i<quantity; i++){
            round.ticketsSold += 1;
            TicketKey[LotteryRounds][round.ticketsSold] = Ticket(msg.sender, getTicketKey(round.ticketsSold), false);

        }
    }

    function announcerResult() public onlyOwner {
        require(RoundDetails[LotteryRounds].endTime < block.timestamp, "Time not completed");
        require(resultAnnounced[LotteryRounds] == false, "Already Announced");
        resultAnnounced[LotteryRounds] = true;
        uint256 key = random(); /* will use chainlink in real contract */
        winningKey[LotteryRounds] = key;
    }
    ...
    // function getTicketResult(uint256 ticketId) public view returns(uint256){
    //     uint256 TicketKey_ = TicketKey[LotteryRounds][ticketId].ticketKey;
    //     uint256 winningKey_ = winningKey[LotteryRounds];
    //     for(uint256 i =0; i<=usedTicket[msg.sender];i++)
    //     { 
    //        TicketKey_.mul(winningKey_) % 1000;
    //     }
    // }
    

    function getWinningOre(uint256 ticketId) public view returns(uint256){
        uint256 TicketKey_ = TicketKey[LotteryRounds][ticketId].ticketKey;
        uint256 winningKey_ = winningKey[LotteryRounds];
        uint256 value ;
        ...
        for(uint256 i =0; i<usedTicket[msg.sender];i++)
        {
        uint256 result = (TicketKey_.mul(winningKey_).mul(i)) % 1000;
        if(VortexiaOre.contains(result)){
            value= getPrice(Ores.Vortexia);
        } else if(DiamondOre.contains(result)){
            value =getPrice(Ores.Diamond);
        } else if(0 < result && result  >= 480) {
            value = getPrice( Ores.Iron);
        } else if (EmeraldOre.contains(result)){
            value = getPrice( Ores.Emerald);
        } else if (BronzeOre.contains(result)){
            value = getPrice(Ores.Bronze);
        } else if(IronOre.contains(result)){
            value = getPrice( Ores.Iron);
        }
    ...
    function claimWinning(uint256 ticketId) public nonReentrant {
        Ticket storage ticket = TicketKey[LotteryRounds][ticketId];
        require(ticket.claimed == false, "Already claimed");
        require(ticket.holder==msg.sender,"You are not owner");
        usedTicket[msg.sender]+=1;
        uint256 winning = getWinningOre(ticketId);
        TransferHelper.safeTransferETH(msg.sender, winning);
    }
