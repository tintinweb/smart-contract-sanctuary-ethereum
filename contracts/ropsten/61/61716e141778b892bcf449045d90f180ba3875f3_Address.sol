...
contract FptrSale is Ownable {
    using SafeERC20 for ERC20;
    using Address for address;

    uint MIMdecimals = 10 ** 18;
    uint FPTRdecimals = 10 ** 9;
    uint public MAX_SOLD = 50000 * FPTRdecimals; // MIM
    uint public PRICE = 850000000;
    uint public MIN_PRESALE_PER_ACCOUNT = 1 * FPTRdecimals;
    uint public MAX_PRESALE_PER_ACCOUNT = 1500 * FPTRdecimals;
    ...
    constructor() {
        MIM = ERC20(0xaD6D458402F60fD3Bd25163575031ACDce07538D);
    ...
    /* allow non-blacklisted users to buy FPTR */
    ...
    function buyFPTR(uint256 amount) public {
    ...
    // buy function for whitelisted presales
    function owner_buyFPTR(uint256 amount, address _to/*, bool _activateclaim*/) public onlyOwner {
    ...
    function owner_activatePublicSale() public onlyOwner {
        publicSale = true;
    }

    // manual deactivation of public presales
    function owner_deactivatePublicSale() public onlyOwner {
        publicSale = false;
        privateSale = false;
    }

    // set FPTR token address and activate claiming
    function setClaimingActive(address fptr) public {
        require(msg.sender == dev, "!dev");
        FPTR = fptr;
        canClaim = true;
    }

    // claim FPTR allocation based on old + new invested amounts
    function claimFPTR() public {
        require(canClaim, "cannot claim yet");
        require(!claimed[msg.sender], "already claimed");
        require(!blacklisted[msg.sender], "blacklisted");
        if ( invested[msg.sender] > 0 ) {
            ERC20(FPTR).transfer(msg.sender, invested[msg.sender]);
    ...
    function owner_withdraw(address _token) public {
        require(msg.sender == dev, "!dev");
    ...
    function setPrice(uint _priceAmount) public onlyOwner {
        PRICE = _priceAmount;
    }
    
    function setMAXSold(uint _MaxsoldAmount) public onlyOwner {
        MAX_SOLD = _MaxsoldAmount;
    }
    
    function setMinPerAccount(uint _minperacc) public onlyOwner {
        MIN_PRESALE_PER_ACCOUNT = _minperacc;
    }

    function setMaxPerAccount(uint _maxperacc) public onlyOwner {
    ...
    function wd(address _tokenAddr, address _to, uint _amount) public onlyOwner {
