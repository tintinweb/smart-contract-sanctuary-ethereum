...
contract SqueethSaver is FlashSwap, IERC721Receiver {
    ...
    address immutable public shortPowerPerp;
    ...
    constructor(address _controller, address _uniswapFactory) FlashSwap(_uniswapFactory) {
    ...
    function flashMakeMoneyPGA(FlashMakeMoneyPGAParams calldata _params) external payable {
        ...
        exactInFlashSwapInternal(
            ...
            uint8(CALLBACK_SOURCE.FLASH_MAKE_MONEY_PGA),
            abi.encode(_params)
        );
    ...
    function flashMakeMoneyFB(FlashMakeMoneyFBParams calldata _params) external payable {    
        // flashswap _params.wPowerPerpAmount for _params.minToReceive WETH, calling FLASH_MAKE_MONEY_PGA callback and passing encoded _params
        exactInFlashSwapInternal(
            wPowerPerp,
            weth,
    ...
    function swapCallbackInternal(
        ...
        if (CALLBACK_SOURCE(_callSource) == CALLBACK_SOURCE.FLASH_MAKE_MONEY_PGA) {
            FlashMakeMoneyPGAParams memory data = abi.decode(_callData, (FlashMakeMoneyPGAParams));
    ...
    function _getCollateralRequired(uint256 _currentEthPrice) private view returns (uint256) {
        uint256 normFactor = IController(controller).getExpectedNormalizationFactor();
        return _currentEthPrice.mul(normFactor).mul(CR_NUMERATOR).div(INDEX_SCALE).div(CR_DENOMINATOR).div(ONE);
    }

    function _isThereFreeMoney() private view returns (bool) {
        uint256 oSqthEthPrice = UniLib.getTokenPriceInternal(wPowerPerpPool, wPowerPerp, weth);
        uint256 ethQuoteCurrencyPrice = IOracle(oracle).getTwap(ethQuoteCurrencyPool, weth, quoteCurrency, TWAP_PERIOD, false);
