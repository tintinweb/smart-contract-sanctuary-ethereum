...
contract RolaGameUpgradeable is RolaGameInternalUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     * @param baseAddress: Base address for price calculation
     * @param dexAddress: Decentralized exchange pair address
     ...
     */
    function claimable(uint256 roundId, address user) internal view returns (bool) {
    ...
    function recoverToken(address _token, uint256 _amount) public override onlyAdminOrOwner {
        IERC20Upgradeable(_token).safeTransfer(address(_msgSender()), _amount);
