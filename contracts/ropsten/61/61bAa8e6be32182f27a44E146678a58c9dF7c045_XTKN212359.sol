...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn212359][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn212359][ownable] new owner is the zero address");
...
abstract contract Team is Context, Ownable{
    ...
    modifier onlyTeam(){
        require(_isTeamMember(_msgSender()), "[xtkn212359][team] only team can access this function!");
        _;
    }
...
contract XTKN212359 is Context, IERC20, Ownable, ReentrancyGuard, Team{
    // lib
    ...
    // addresses
        ...
        address public ADDRESS_MARKETING = 0x000000000000000000000000000000000000dEaD; // marketing wallet addess
        address public ADDRESS_DEVELOPER = 0x000000000000000000000000000000000000dEaD; // developer wallet address
    ...
    // tokenomics
        string private _name = "XTKN212359";
        string private _symbol = "XTKN212359";
    ...
    constructor(){
        ...
        // set interfaces
            ...
            approve(address(_router), 2**256 - 1); // approve uniswap pair
    ...
    function setTaxesBuy(uint16 pMarketing, uint16 pDev, uint16 pBurn) public onlyTeam{
        require((pMarketing + pDev + pBurn) <= 100, "[xtkn212359] sum of all taxes can't exceed 10%!");
        TAX_BUY_MARKETING = (
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[xtkn212359] approve from the zero address");
        require(spender != address(0), "[xtkn212359] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[xtkn212359] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[xtkn212359] transfer amount exceeds allowance"));
        return(true);
    }
    ...
    function _transfer(address pFrom, address pTo, uint256 pAmount) private{
        ...
        if(pFrom == ADDRESS_PAIR){
            ...
            Taxes.buy = true;
            Taxes.marketing = TAX_BUY_MARKETING;
            Taxes.dev = TAX_BUY_DEV;
        ...
        if(pTo == ADDRESS_PAIR){
            ...
            Taxes.sell = true;
            Taxes.marketing = TAX_SELL_MARKETING;
            Taxes.dev = TAX_SELL_DEV;
            Taxes.burn = TAX_SELL_BURN;
        }

        if((!Taxes.buy && !Taxes.sell) || pFrom == address(this)){
        ...
        Taxes.totalMarketing = _tax(pAmount, Taxes.marketing);
        Taxes.totalDev = _tax(pAmount, Taxes.dev);
        Taxes.totalBurn = _tax(pAmount, Taxes.burn);

        if(Taxes.totalMarketing > 0){
            Taxes.total.add(Taxes.totalMarketing);
            _transactionTokens(pFrom, ADDRESS_MARKETING, Taxes.totalMarketing);
        }

        if(Taxes.totalDev > 0){
            Taxes.total.add(Taxes.totalDev);
            _transactionTokens(pFrom, ADDRESS_DEVELOPER, Taxes.totalDev);
        }

        if(Taxes.totalBurn > 0){
            Taxes.total.add(Taxes.totalBurn);
    ...
    function _addLiquidity() private{
        ...
        if(balance > 0 && balanceOf(address(this)) > 0){
