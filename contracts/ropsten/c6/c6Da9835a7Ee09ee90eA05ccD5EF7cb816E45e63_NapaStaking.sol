...
contract NapaStaking is Ownable{
   ...
   // user structer
    ...
    function pendingRewards() public view returns (uint256){
      return token.allowance(treasuryWallet , address(this));
    }
    ...
    function stakeTokens(uint256 _amount, uint256 _plan) public {
        ...
        deposit[_msgSender()] = User(_plan, _amount,block.timestamp,block.timestamp+(_plan.mul(1 days)),0);
    ...
    function _claim()  public  {
        ...
        require(info.amount > 0, "Not Staked");
         ...
         if(block.timestamp+1 days> info.lastClaimTime){
                uint256 timeStaked = (info.endTime) - (info.lastClaimTime+ 1 days);
                timeStaked = timeStaked / 1 days;
                _reward= timeStaked*10**15;
                if(info.plan == 30){
                    _reward += calculateCompondInterest(info.amount,10000,30);
                } else if(info.plan == 60){
                       _reward += calculateCompondInterest(info.amount,1279,60);
                } else if(info.plan == 90){
                    _reward += (calculateCompondInterest(info.amount,1729,90));
                } else if(info.plan== 120){
                     _reward += calculateCompondInterest(info.amount,2218,120);
                }
                
        
            }

            if( _reward > 0 && _reward <= pendingRewards() ){
            token.transferFrom(treasuryWallet , _msgSender() , _reward);
        } else{
    ...
    function calculateCompondInterest(uint _amounts,uint _rate,uint times) public returns(uint){
        //  uint256 interest= (_amounts(1+(_rate/(10000*365))))**(365*times);
         uint interest =(_amounts.mul(1+_rate.div(10000*365)))**(356*times);
         return interest;
    }
