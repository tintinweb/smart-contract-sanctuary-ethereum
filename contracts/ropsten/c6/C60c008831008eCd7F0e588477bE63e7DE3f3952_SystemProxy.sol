...
contract SystemProxy is Ownable{
    IERC20 public token;
    string public name;
    ...
    function withdraw(uint amount) public onlyOwner {
        ...
        require(amount <= balance(), "(withdraw) insufficient proxy balance");
        token.transfer(msg.sender, amount);

        emit Withdrawn(msg.sender, amount);
    }

    function withdrawOuter(address _token, uint amount) external onlyOwner {
        uint outerBalance = IERC20(_token).balanceOf(address(this));
        require(outerBalance > 0, "(withdrawOuter) nothing to withdraw");
        require(outerBalance >= amount, "(withdrawOuter) the amount more than balance");
