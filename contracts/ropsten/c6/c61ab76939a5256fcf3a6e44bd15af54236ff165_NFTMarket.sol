...
contract NFTMarket is ReentrancyGuard {
    ...
    // create a snippet MarketItem struct > MarketItemCreated event
    ...
    function createMarketItem(
        ...
        require(price > 0, "Please specify value price greater than 0");
        require(
            msg.value == listingPrice,
            "Payment must be equal to listing price 0.025 eth"
        );
        ...
        idToMarketItem[itemId] = MarketItem(
            // zamisli floating labele ovde sa imenima event parametra
            itemId,
            nftContract,
            tokenId,
            payable(msg.sender),
        ...
        // at this point, the nftContract already references this marketplace contract
        // recognizing it as an operator
    ...
    function createMarketSale(address nftContract, uint itemId)
        ...
        nonReentrant
    {
        // TODO: explore reentrancy once again (interception, was it?)
        uint price = idToMarketItem[itemId].price;
        uint tokenId = idToMarketItem[itemId].tokenId;
        require(msg.value == price, "Send exact price of this item.");
        ...
        // the owner of the marketplace gets transfered listingPrice ETH upon item being sold
        payable(owner).transfer(listingPrice);
    }

    /**
    Fetches items which are available for sale (market items)
     */
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        // uint currentIndex = 0;
        for ((uint i, uint currentIndex) = (0, 0); i < itemCount; i++) {
    ...
    function fetchMyNFTs() public view returns (MarketItem[] memory) {
        ...
        for ((uint i, uint currentIndex) = (0, 0); i < totalItemCount; i++) {
            if (idToMarketItem[i + 1].owner == msg.sender) {
                items[currentIndex] = idToMarketItem[i + 1];
                currentIndex += 1;
            }
    ...
    function fetchItemsCreated() public view returns (MarketItem[] memory) {
        ...
        for ((uint i, uint currentIndex) = (0, 0); i < totalItemCount; i++) {
            if (idToMarketItem[i + 1].seller == msg.sender) {
                ...
                // MarketItem storage currentItem = idToMarketItem[i + 1]; -- unnecessary
                // TODO make a bug here and debug with Tenderly debugger
                items[currentIndex] = idToMarketItem[i + 1];
                currentIndex += 1;
            }
        }
