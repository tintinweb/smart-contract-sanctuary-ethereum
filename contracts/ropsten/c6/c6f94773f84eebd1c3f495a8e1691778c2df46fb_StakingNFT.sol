...
contract StakingNFT is Ownable {
    ...
    uint public poolStakers = 0;
    uint public poolShare = 0;
    uint private tokenInfoIndex = 1;
    uint private stakerInfoIndex = 1; 
    struct TokenInfo {
        uint256 tokenId;
        uint rType;
   ...
   mapping(address => userReward[]) public userreward;
    ...
    function getUserNftStakeAvalible(address _userAddress , address _nftAddress) public view returns(uint[] memory){
        uint[] memory totalId;
        uint a = 0;
        for (uint256 i = 1; i <= tokenInfoIndex; i++) {
            if(tokenInfo[i].nftContractAddress ==  _nftAddress){
                address _nftContractAddress = tokenInfo[i].nftContractAddress;
                uint _tokenId = tokenInfo[i].tokenId;
                if(IERC721(_nftContractAddress).ownerOf(_tokenId) == _userAddress){
                   totalId[a] = _tokenId;
                   a += 1;
                } 
            } 
    ...
    function distributeReward(uint _poolId) external onlyOwner {
        ...
        for (uint256 i = 1; i <= stakerInfoIndex; i++) {
            uint index = stakerinfo[i].indexId;
            if(index > 0){
                uint _tokenId = stakerinfo[index].tokenId;
                address _nftContractAddress = stakerinfo[index].nftContractAddress;
                address _userAddress = stakerinfo[index].userAddress;   
    ...
    function unStakeNFT(address _nftContractAddress , uint256 _tokenId) public {
        ...
        uint SindexId = staker[msg.sender][_nftContractAddress][_tokenId].indexId;
        delete staker[msg.sender][_nftContractAddress][_tokenId];
        delete stakerinfo[SindexId];
        poolStakers -= 1;
        poolShare -= rType;
