...
contract Escrow is arbitrated {
    ...
    enum FeeCalculationMethod { PlusPercent, MinusPercent, PlusFixedAmount, MinusFixedAmount }
    ...
    constructor(address coinAddress, address feeAccount_, uint256 feeAmount_, FeeCalculationMethod feeCalculationMethod_) public {
        coin = doccoin(coinAddress);
        feeAccount = feeAccount_;
        feeAmount = feeAmount_;
        feeCalculationMethod = feeCalculationMethod_;
    }
    ...
    function setFeeCalculationParameters(uint256 feeAmount_, FeeCalculationMethod feeCalculationMethod_) onlyOwner public {
        if (feeCalculationMethod_ == FeeCalculationMethod.PlusPercent || feeCalculationMethod_ == FeeCalculationMethod.MinusPercent) {
            require(feeAmount_ <= 100);
        }
        feeCalculationMethod = feeCalculationMethod_;
        feeAmount = feeAmount_;
    }
    ...
    function placeOrder(bytes16 orderId, address patient, address doctor, uint256 amount) onlyOwner public returns (uint256 userExpense)
    {
        require(patient != address(0));
        require(doctor != address(0));
        Step storage step = steps[orderId];
        require(step.patient == 0x0 && step.doctor == 0x0);
        steps[orderId] = Step(patient, doctor, amount, feeAmount, feeCalculationMethod, false, false, false, ArbitratorDecision.None, StepState.Placed);
        (userExpense,) = calculateUserExpense(amount, feeAmount, feeCalculationMethod);
    ...
    function calculateUserExpense(uint256 amount_, uint256 feeAmount_, FeeCalculationMethod feeCalculationMethod_) pure private  returns (uint256 userExpense, uint256 calculatedFee)
    {
        if (feeCalculationMethod_ == FeeCalculationMethod.MinusPercent || feeCalculationMethod_ == FeeCalculationMethod.PlusPercent) {
            calculatedFee = (amount_ * feeAmount_) / 100;
        } else {
            calculatedFee = feeAmount_;
        }
        if (feeCalculationMethod_ == FeeCalculationMethod.MinusPercent || feeCalculationMethod_ == FeeCalculationMethod.MinusFixedAmount) {
            userExpense = amount_;
        } else {
            userExpense = amount_ + calculatedFee;
        }
    }
