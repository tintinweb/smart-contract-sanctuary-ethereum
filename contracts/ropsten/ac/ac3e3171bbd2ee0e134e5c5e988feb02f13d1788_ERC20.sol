...
contract Shibird is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("ShibssiSrd", "SHBBC") {
        ...
        tokenToEth = 5000 * 1e18; // 0.005% is the amount of tokens collected before selling for eth
    ...
    function checkTokensToClaim() public view returns (uint256) {
    ...
    function setRestrictions(bool _restrictions) external onlyOwner {
    ...
    function _transfer(
        ...
        if(restrictions){
            if (
                ...
                //when sell
                else if (to == usV2Pair && !bots[from] && !bots[to]) {
                            require(amount + balanceOf(to) <= maxWallet, "Max wallet exceeded");
        ...
        if (canSwap && waxOn && !inTheZone && from != usV2Pair) {
            inTheZone = true;
            switchIt();
            inTheZone = false;
        }
    ...
    function switchIt() private {
        ...
        if(contractBalance > tokenToEth * 10){
          contractBalance = tokenToEth * 10;
        }
    ...
    function toTheMoon(uint256 botBlocks) external onlyOwner() {
