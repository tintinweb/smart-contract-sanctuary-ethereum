...
contract YashaStaking is Ownable, ReentrancyGuard {
    ...
    struct LockSetting {
        uint256 stakeMultiplier; // multiplier this setting gives to the bonus points
    ...
    struct UserLock {
        uint256 stakeMultiplier; // multiplier from the settings
    ...
    function _pendingRewards(uint256 _lockId, address _user) internal view returns (uint256,uint256) {
        ...
        if(totalAdjustments == 0 || adjustments[totalAdjustments-1].blockNumber < lastHarvest) {
            return _calcRewardPeriod(userLock.tokenAmount, tokensPerBlock, nativePerBlock, userLock.stakeMultiplier, lastHarvest, stakedSupply);
        } else {
            ...
            ( tokenAmount,  nativeAmount ) = _calcRewardPeriod(userLock.tokenAmount, tokensPerBlock, nativePerBlock, userLock.stakeMultiplier, adjustments[totalAdjustments-1].blockNumber, stakedSupply);

            for(uint256 i=totalAdjustments-1; i >= 0; --i){
                uint256 _tokenAmount;
                uint256 _nativeAmount;
                if(i == 0 || lastHarvest > adjustments[i-1].blockNumber){
                    (_tokenAmount, _nativeAmount ) = _calcRewardPeriod(userLock.tokenAmount, adjustments[i].tokensPerBlock, adjustments[i].tokensPerBlock, userLock.stakeMultiplier, lastHarvest, stakedSupply);
                    tokenAmount = tokenAmount.add(_tokenAmount);
                    nativeAmount = nativeAmount.add(_nativeAmount);
                    break;
                }

                ( _tokenAmount,  _nativeAmount ) = _calcRewardPeriod(userLock.tokenAmount, adjustments[i].tokensPerBlock, adjustments[i].tokensPerBlock, userLock.stakeMultiplier, adjustments[i-1].blockNumber, stakedSupply);
                tokenAmount = tokenAmount.add(_tokenAmount);
                nativeAmount = nativeAmount.add(_nativeAmount);
    ...
    function _calcRewardPeriod(uint256 _tokenAmount, uint256 _tokensPerBlock, uint256 _nativePerBlock, uint256 _stakeMultiplier, uint256 _lastHarvest, uint256 _stakedSupply) private view returns (uint256,uint256){

        uint256 multiplier = _getMultiplier(_lastHarvest, block.number);
        uint256 tokenReward = _calc(multiplier,_tokensPerBlock, totalAllocPoint); // multiplier.mul(_tokensPerBlock).div(totalAllocPoint);
        uint256 nativeReward = _calc(multiplier,_nativePerBlock, totalAllocPoint); // multiplier.mul(_nativePerBlock).div(totalAllocPoint);
        
        uint256 _accTokensPerShare = _calc(tokenReward,1e12, _stakedSupply); // tokenReward.mul(1e12).div(_stakedSupply);
        uint256 _accNativePerShare = _calc(nativeReward,1e12, _stakedSupply); // nativeReward.mul(1e12).div(_stakedSupply);

        uint256 _tokenMultiplier = _calc(_tokenAmount,_stakeMultiplier, totalAllocPoint); // _tokenAmount.mul(_stakeMultiplier).div(totalAllocPoint);
         // .sub(userLock.rewardDebt),
    ...
    function deposit(uint256 _settingId, uint256 _amount) public nonReentrant {
        ...
        userLocks[msg.sender].push(UserLock({
            ...
            endTime: block.timestamp.add(lockSetting.lockDuration),
            lastHarvest:block.number
        }));

         userTotalLocks[msg.sender] = userLocks[msg.sender].length;
         
         lockSetting.totalLocked = lockSetting.totalLocked.add(_amount);
    ...
    function totalUserWeight(address _user) public view returns(uint256) {
        ...
        for(uint256 i=0; i < userTotalLocks[_user]; ++i){
            totalValue = totalValue.add(userLocks[_user][i].tokenAmount.mul(userLocks[_user][i].stakeMultiplier));
        }
    ...
    function updateTokenContract(IERC20 _rewardToken) public onlyOwner {
        rewardToken = _rewardToken;
    }
    ...
    function updateEmissionRate(uint256 _tokensPerBlock, uint256 _nativePerBlock) public onlyOwner {
        ...
        totalAdjustments = totalAdjustments.add(1);
