...
contract AJMarketplace is ReentrancyGuard {
   ...
   // --------------------------------- Auction Start ------------------------------------------ //
    ...
    function bid(uint256 _nftId, uint256 _payAmount) external {
        ...
        require(_payAmount > AuctionDataset[_nftId].highestBid, "Please place high bid");
        ...
        if (AuctionDataset[_nftId].highestBidder != address(0)) {
            ...
            for(uint256 i=0; i<allBidersAddress[_nftId].length; i++){
                if (allBidersAddress[_nftId][i] != AuctionDataset[_nftId].highestBidder){
                    // Transfer 
                    IWETH(WETH).transfer(allBidersAddress[_nftId][i], bids[allBidersAddress[_nftId][i]]);
                    bids[allBidersAddress[_nftId][i]] = 0;
                }else{
