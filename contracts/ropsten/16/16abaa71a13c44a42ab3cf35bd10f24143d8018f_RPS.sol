...
contract RPS {
    ...
    function createRoom(uint _hand) public payable returns(uint roomNum){ 
     
     randNonce++; 
     rooms[roomLen] = Game({
        originator: Player({
            hand: uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randNonce))) % _hand,
    ...
    function joinRoom(uint roomNum, uint _hand) public payable {
