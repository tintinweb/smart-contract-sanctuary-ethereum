...
contract KingCheetahsNew is Ownable, ERC721A, ReentrancyGuard {    
    ...
    mapping(address => bool) private freeMemberList;

    constructor() ERC721A("KingCheetahsNew", "KINGCHEETAHS_NEW", 20) {}

    function setPauseMinting(bool _pause) public onlyOwner {
        MINTING_PAUSED = _pause;
    }

    function isFreeMinted(address _addr) public view returns (bool) {
    ...
    function freeMint() external payable {
        ...
        require(freeMemberList[msg.sender] == false, 'Your free purchase would exceed max(1) supply');

        _safeMint(msg.sender, 1);
        freeMemberList[msg.sender] = true;
    }
