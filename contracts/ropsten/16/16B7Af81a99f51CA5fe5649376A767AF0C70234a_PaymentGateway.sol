...
contract PaymentGateway is Ownable, ReentrancyGuard {
    ...
    address payable public paymentCollector;
    ...
    mapping(bytes32 => PaymentInfo) public paymentByOrderId;
    ...
    constructor () {
        paymentCollector = payable(msg.sender);
    }
    ...
    {
        require(isSupportedToken(erc20TokenAddress), "ERC20 token not supported");
        
        PaymentInfo storage p = paymentByOrderId[orderId];
        
        require(p.amount == 0 && p.orderId == bytes32(0), "order id already paid for");
        p.payer = msg.sender;
        p.amount = amount;
        p.orderId = orderId;
        ...
        IERC20(erc20TokenAddress).safeTransferFrom(
            _msgSender(),
            address(this), // can also transfer to paymentCollector here
            amount
        );
    ...
    function getPaymentByOrderId(bytes32 orderId) public view returns (PaymentInfo memory) {
        return paymentByOrderId[orderId];
    }

    function getPaymentByAddress(address account) public view returns (PaymentInfo[] memory) {
        return payments[account];
    }

    function setPaymentCollector(address newPaymentCollector) public onlyOwner {
        require(
            newPaymentCollector != address(0) && !newPaymentCollector.isContract(),
            "invalid payment collector address"
        );
        paymentCollector = payable(newPaymentCollector);
    }

    function withdraw(address tokenAddress) external nonReentrant {
        require(_msgSender() == paymentCollector, "only paymentCollector can withdraw");
        if (tokenAddress == EthAddress) {
            paymentCollector.transfer(address(this).balance);
            emit Withdrawal(tokenAddress, address(this).balance);
        } else {
            IERC20(tokenAddress).safeTransfer(paymentCollector, IERC20(tokenAddress).balanceOf(address(this)));
