...
// File: contracts/Velorex Members Club NFT.sol
...
contract VelorexMembersClubNFT is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public presaleCost = 0.75 ether;
    ...
    constructor(
        ...
        //Pre-mint 5 for the owner wallet for team and giveaways
        mint(msg.sender, 5);
    }
    ...
    // public
    function mint(address _to, uint256 _maxMintPerAddress) public payable {
        uint256 supply = totalSupply();
        require(!paused);
        require(_maxMintPerAddress > 0);
        require(_maxMintPerAddress <= maxMintPerAddress);
        require(supply + _maxMintPerAddress <= maxSupply);

        if (msg.sender != owner()) {
            if (whitelisted[msg.sender] != true) {
                if (presaleWallets[msg.sender] != true) {
                    //general public
                    require(msg.value >= cost * _maxMintPerAddress);
                } else {
                    //presale
                    require(msg.value >= presaleCost * _maxMintPerAddress);
                }
            }
        }

        for (uint256 i = 1; i <= _maxMintPerAddress; i++) {
