...
import "./interfaces/IRythmPair.sol";
...
contract Factory is IFactory {
    ...
    mapping(address => LendingPool) public getLendingPool; // get by RythmPair
    address[] public allLendingPools; // address of the RythmPair
    ...
    function _getTokens(address RythmPair)
        private
        view
        returns (address token0, address token1)
    {
        token0 = IRythmPair(RythmPair).token0();
        token1 = IRythmPair(RythmPair).token1();
    }

    function _createLendingPool(address rythmPair) private {
        if (getLendingPool[rythmPair].lendingPoolId != 0) return;
        allLendingPools.push(rythmPair);
        getLendingPool[rythmPair] = LendingPool(
            false,
    ...
    function createCollateral(address rythmPair)
        external
        returns (address collateral)
    {
        _getTokens(rythmPair);
        require(
            getLendingPool[rythmPair].collateral == address(0),
            "Rythm: ALREADY_EXISTS"
        );
        collateral = cDeployer.deployCollateral(rythmPair);
        ICollateral(collateral)._setFactory();
        _createLendingPool(rythmPair);
        getLendingPool[rythmPair].collateral = collateral;
    }

    function createBorrowable0(address rythmPair)
        external
        returns (address borrowable0)
    {
        _getTokens(rythmPair);
        require(
            getLendingPool[rythmPair].borrowable0 == address(0),
            "Rythm: ALREADY_EXISTS"
        );
        borrowable0 = bDeployer.deployBorrowable(rythmPair, 0);
        IBorrowable(borrowable0)._setFactory();
        _createLendingPool(rythmPair);
        getLendingPool[rythmPair].borrowable0 = borrowable0;
    }

    function createBorrowable1(address rythmPair)
        external
        returns (address borrowable1)
    {
        _getTokens(rythmPair);
        require(
            getLendingPool[rythmPair].borrowable1 == address(0),
            "Rythm: ALREADY_EXISTS"
        );
        borrowable1 = bDeployer.deployBorrowable(rythmPair, 1);
        IBorrowable(borrowable1)._setFactory();
        _createLendingPool(rythmPair);
        getLendingPool[rythmPair].borrowable1 = borrowable1;
    }

    function initializeLendingPool(address rythmPair) external {
        (address token0, address token1) = _getTokens(rythmPair);
        LendingPool memory lPool = getLendingPool[rythmPair];
        ...
        (, , , , , bool oracleInitialized) =
            rythmPriceOracle.getPair(rythmPair);
        if (!oracleInitialized) rythmPriceOracle.initialize(rythmPair);
        ...
        getLendingPool[rythmPair].initialized = true;
...
interface IFactory {
	event LendingPoolInitialized(address indexed rythmPair, address indexed token0, address indexed token1,
	...
	function getLendingPool(address rythmPair) external view returns (
		bool initialized, 
		uint24 lendingPoolId, 
		address collateral, 
		address borrowable0, 
	...
	function allLendingPools(uint) external view returns (address rythmPair);
	...
	function createCollateral(address rythmPair) external returns (address collateral);
	function createBorrowable0(address rythmPair) external returns (address borrowable0);
	function createBorrowable1(address rythmPair) external returns (address borrowable1);
	function initializeLendingPool(address rythmPair) external;
...
interface IBDeployer {
	function deployBorrowable(address rythmPair, uint8 index) external returns (address borrowable);
}
...
interface ICDeployer {
	function deployCollateral(address rythmPair) external returns (address collateral);
}
...
interface IRythmPriceOracle {
    ...
    function getPair(address rythmPair)
        external
        view
        returns (
    ...
    function initialize(address rythmPair) external;

    function getResult(address rythmPair)
        external
