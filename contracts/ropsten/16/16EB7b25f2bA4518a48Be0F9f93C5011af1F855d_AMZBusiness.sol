...
contract AMZBusiness is Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    Counters.Counter cntColletct;
    address  AMZOneContractAddr;
    address tokenContractAddr;  //default Stable Coin contract(USDT);
    // address tokenContractAddr = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);  //default Stable Coin contract(USDT);
    uint256 tokenPrice=1000000000000000000;
    uint256 initCollectAmount=10000;
    struct AMZCollection{ //Amazon Token
        uint256 collectId; //unused =0;
        uint256 leaveAmount; //Rest Amount default =10000
    } 
    
    struct AMZOne{ // Amazon small Token
        uint256 tokenId;
        address holder;
        uint256 amount;
    }

    mapping(uint256=>mapping(uint256=>AMZCollection)) collectionList; //  collection list; latitude=>longtitude=>collection
    mapping(uint256 =>address[]) collectionHolders; //collectonId=>holders
    mapping(address=>uint256[]) NFTsPerHolder; // holder=>nfts; 
    mapping(uint256=>AMZOne) NFTList; //tokenId=>NFT
    ...
    event updatePositionOfCollection(
        uint256 collectionId,
        uint256 longtitude,
        uint256 latitude,
        uint256 leaveAmount
    ...
    constructor(address _amzOneContract,address _tokenContract) {
        AMZOneContractAddr =_amzOneContract;
        tokenContractAddr = _tokenContract;
    }
    function setChangeLongAndLatitude(uint256 _lgtitude,uint256 _latitude) public onlyOwner{
           AMZCollection memory _collection = collectionList[_latitude][_lgtitude];
           collectionList[_latitude][_lgtitude] = AMZCollection(0,0);
           collectionList[_latitude][_lgtitude] = _collection;
           emit updatePositionOfCollection(_collection.collectId, _lgtitude, _latitude, _collection.leaveAmount);
    }
    function createCollection(uint256 _lgtitude,uint256 _latitude) public onlyOwner{
        cntColletct.increment();
        collectionList[_latitude][_lgtitude] = AMZCollection(cntColletct.current(),initCollectAmount);
        emit CreateCollection(cntColletct.current(), _lgtitude, _latitude, initCollectAmount);
    }
 

    function makeNFT(uint256 latitude,uint256 longtitude,address _holder,uint256 amount) private returns(uint256){
            uint256 _cltId = collectionList[latitude][longtitude].collectId;
            require(amount<=collectionList[latitude][longtitude].leaveAmount,"amount must be less than leaveAmount");
            uint256 _tokenId=AMZOneNFT(AMZOneContractAddr).mintToken();
            NFTList[_tokenId] = AMZOne(_tokenId,_holder,amount);
            NFTsPerHolder[_holder].push(_tokenId);
            collectionList[latitude][longtitude].leaveAmount -=amount;  
            emit createNFT(latitude, longtitude,_cltId, _holder,_tokenId, amount);
            return _tokenId;
    }
   
    function Buy (uint256 latitude,uint256 longtitude, uint256 tokenBalance) public payable {
        uint256 amount = tokenBalance/tokenPrice;
        require(amount<=collectionList[latitude][longtitude].leaveAmount,"amount must be less than collection leave Amount");

        uint256 allowance = IERC20(tokenContractAddr).allowance(msg.sender, address(this));
        require(allowance >=tokenBalance,"Check the token allowance");
        require(IERC20(tokenContractAddr).transferFrom(msg.sender, address(this), tokenBalance),"dont send token");

        uint256 _tokenId = makeNFT(latitude, longtitude, msg.sender, amount);
        IERC721(AMZOneContractAddr).transferFrom(address(this), msg.sender, _tokenId);
        uint256 _cltId = collectionList[latitude][longtitude].collectId;
        if(!searchHolder(_cltId,msg.sender))collectionHolders[_cltId].push(msg.sender);
        emit createHolder(longtitude, latitude, _cltId, msg.sender,_tokenId);
    }

    function searchHolder(uint256 collectionId,address holder) public view returns(bool){
        for(uint256 i=0;i<collectionHolders[collectionId].length;i++){
            if(collectionHolders[collectionId][i] == holder){
                break;
            }
            if(i==collectionHolders[collectionId].length-1){
                return false;
            }
        }
        return true;
    }
    // function getUSDTBalance(address addr) public view returns(uint){
    //      IERC20 usdt = IERC20(address(tokenContractAddr));     
    //      return usdt.balanceOf(address(addr));
    // }
    function getTokenBalance(address addr) public view returns(uint256){
        IERC20 token = IERC20(address(tokenContractAddr));
        return token.balanceOf(address(addr));
    }
    function setTokenContractAddr(address to) public onlyOwner {
        tokenContractAddr=to;
    }
