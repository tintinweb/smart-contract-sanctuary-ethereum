...
/**
 * @title FireCatPool contract
 * @notice main: stake, claim, topUP
 * @author FireCat Finance
 */
contract FireCatPool is IFireCatPool, ModifyControl, FireCatTransfer {
    IFireCatNFTStake fireCatNFTStake;
    IFireCatRegistryProxy fireCatRegistryProxy;
    using SafeMathUpgradeable for uint256;

    event SetCycleTime(uint256 cycleTime_);
    event SetRewardRate(uint256 cycleRate_, uint256 cyclePeriod_, uint256 rewardRatePerCycle_, uint256 rewardRatePerSecond_);
    event SetReservesRate(uint256 reservesRewardRate_);
    event SetInviterRewardRate(uint256 inviterRewardRate_);
    ...
    uint256 public reservesRewardRate;
    uint256 public inviterRewardRate;
    
    uint256 public rewardRatePerSecond;
    uint256 public rewardRatePerCycle;

    uint256 private _cycleRate;
    uint256 private _cyclePeriod;
    uint256 private _cycleTime;
   
    mapping(address => userCycle) private _userCycle;
    ...
    */
    ...
    /**
    * @dev the total earned amount which is already generated from this pool.
    */
    uint256 private _totalEarned;

    /**
    * @dev the total reserves amount which is already generated from this pool.
    */
    ...
    */
    ...
    /**
    * @dev the earned amount of user.
    */
    mapping(address => uint256) private _earned;

    mapping(address => uint256) private _inviterReward;

    function initialize(address fireCatNFTStake_, address fireCatRegistryProxy_, address rewardToken_, address stakeToken_) initializer public {
        fireCatNFTStake = IFireCatNFTStake(fireCatNFTStake_);
    ...
    modifier modifyLocked() {
        require(_totalStaked == 0, "POOL:E10");
        _;
    }
    
    /// @inheritdoc IFireCatPool
    ...
    /// @inheritdoc IFireCatPool
    function userCycleOf(address user_) public view returns (userCycle memory) {
        return _userCycle[user_];
    }

    /// @inheritdoc IFireCatPool
    function cycleTime() public view returns (uint256) {
        return _cycleTime;
    }

    /// @inheritdoc IFireCatPool
    function rewardRate() public view returns (uint256, uint256) {
    ...
    /// @inheritdoc IFireCatPool
    function inviteRewardOf(address user_) public view returns (uint256) {
        return _inviterReward[user_];
    }

    /// @inheritdoc IFireCatPool
    function earnedOf(address user_) public view returns (uint256) {
        return _earned[user_];
    }

    /// @inheritdoc IFireCatPool
    function isStaked(address user_) public view returns (bool) {
        userCycle memory cycle = _userCycle[user_];
        return cycle.staked > 0;
    }

    /// @inheritdoc IFireCatPool
    function isDuringCycle(address user_) public view returns (bool) {
        userCycle memory cycle = _userCycle[user_];
        return cycle.startAt < block.timestamp && cycle.expiredAt > block.timestamp;
    }

    /// @inheritdoc IFireCatPool
    function isClaimable(address user_) public view returns (bool) {
        userCycle memory cycle = _userCycle[user_];
        return cycle.totalClaim > cycle.claimed;
    }

    /// @inheritdoc IFireCatPool
    function isStakeable(address user_) public view returns (bool) {
        return IFireCatNFTStake(fireCatNFTStake).isStaked(user_) && !isStaked(user_);
    }

    /// @inheritdoc IFireCatPool
    function stateOf(address user_) public view returns (State) {
        if (isClaimable(user_)) {
            return State.claimable;
        }

        if (isStakeable(user_)) {
    ...
    /// @inheritdoc IFireCatPool
    function calculateEarnings(uint256 amount, uint256 start_, uint256 end_)  public view returns(uint256) {
        require(end_ > start_, "POOL:E11");
        uint256 _baseUnit = 10 ** 18;
        // amount * (10 ** 18 + rewardRatePerSecond * (end_ - start_)) / (10 ** 18);
        return amount.mul(_baseUnit.add(rewardRatePerSecond.mul(end_.sub(start_)))).div(_baseUnit);
    }

    /// @inheritdoc IFireCatPool
    function reviewOf(address user_) public view returns (uint256, uint256, uint256, uint256) {
        userCycle memory prevUserCycle = _userCycle[user_];
        uint256 userStartAt = prevUserCycle.startAt;
        uint256 userExpiredAt = prevUserCycle.expiredAt;
        uint256 userStaked = prevUserCycle.staked;
        uint256 userClaimed = prevUserCycle.claimed;
        uint256 userTotalClaim = prevUserCycle.totalClaim;
        uint256 availableClaim;
        uint256 lockedClaim;

        if (block.timestamp > userExpiredAt) {
            availableClaim = userTotalClaim - userClaimed;
            lockedClaim = 0;
        } else {
            availableClaim = calculateEarnings(userStaked, userStartAt, block.timestamp).sub(userClaimed);
            lockedClaim = calculateEarnings(userStaked, block.timestamp, userExpiredAt);
        }

        return (availableClaim, lockedClaim, userClaimed, userTotalClaim);
    }

    function _shareInviterReward(address user_, uint256 amount_) internal {
        address inviter = fireCatRegistryProxy.getInviter(user_);
        uint256 reward = amount_.mul(inviterRewardRate).div(100);
        _inviterReward[inviter] = _inviterReward[inviter].add(reward);
    }

    function _shareReservesReward(uint256 amount) internal {
        uint256 reward = amount.mul(reservesRewardRate).div(100);
        _totalReserves = _totalReserves.add(reward);
    }
    ...
    function _markUser(address user_, uint256 amount_) internal {
        ...
        // initialize user cycle
        uint256 earned = calculateEarnings(amount_, 0, _cycleTime);
        _totalEarned = _totalEarned.add(earned);
        _earned[user_] = _earned[user_].add(earned);
        _userCycle[user_] = userCycle(
            block.timestamp,
            block.timestamp + _cycleTime,
            amount_,  // staked
            0,  // claimed
            earned   // totalClaim
        );
    ...
    function _updateUserCycleClaimed(address user_, uint256 claimed_) internal {
        userCycle storage cycle = _userCycle[user_];
        cycle.claimed = claimed_;
    }

    function _emptyUserCycle(address user_) internal {
        _userCycle[user_] = userCycle(0, 0, 0, 0, 0);
    }

    /// @inheritdoc IFireCatPool
    function setCycleTime(uint256 cycleTime_) external modifyLocked onlyRole(DATA_ADMIN) {
        require(cycleTime_ != 0, "POOL:E09");
        _cycleTime = cycleTime_;
        emit SetCycleTime(cycleTime_);
    }

    /// @inheritdoc IFireCatPool
    function setRewardRate(uint256 cycleRate_, uint256 cyclePeriod_) external modifyLocked onlyRole(DATA_ADMIN) {
        _cycleRate = cycleRate_;
        _cyclePeriod = cyclePeriod_;

        // cycleRate_ * (10 ** decimals) / 100;
        rewardRatePerCycle = cycleRate_.mul(10 ** 18).div(100);

        // rewardRatePerCycle / (cyclePeriod_ * secondsEachDay);
        rewardRatePerSecond = rewardRatePerCycle.div(cyclePeriod_.mul(86400));
        emit SetRewardRate(cycleRate_, cyclePeriod_, rewardRatePerCycle, rewardRatePerSecond);
    }

    /// @inheritdoc IFireCatPool
    function setReservesRate(uint256 reservesRewardRate_) external modifyLocked onlyRole(DATA_ADMIN) {
        reservesRewardRate = reservesRewardRate_;
        emit SetReservesRate(reservesRewardRate_);
    }

    /// @inheritdoc IFireCatPool
    function setInviterRewardRate(uint256 inviterRewardRate_) external modifyLocked onlyRole(DATA_ADMIN) {
        inviterRewardRate = inviterRewardRate_;
        emit SetInviterRewardRate(inviterRewardRate_);
    }

    /// @inheritdoc IFireCatPool
    function withdrawRemaining(address token, uint256 amount) external nonReentrant onlyRole(DATA_ADMIN) returns (uint256) {
        require(!isClaimOn, "POOL:E08");
        return withdraw(token, amount);
    }

    /// @inheritdoc IFireCatPool
    function claimInviteReward() external beforeClaim nonReentrant returns (uint256)  {
        require(_inviterReward[msg.sender] > 0, "POOL:E07");
        uint256 reward = _inviterReward[msg.sender];
        IERC20(rewardToken).approve(msg.sender, reward);
        uint256 actualClaimedAmount = doTransferOut(rewardToken, msg.sender, reward);

        _claimed[msg.sender] = _claimed[msg.sender].add(actualClaimedAmount);
        _totalClaimed = _totalClaimed.add(actualClaimedAmount);
        _inviterReward[msg.sender] = _inviterReward[msg.sender].sub(actualClaimedAmount);
    ...
    /// @inheritdoc IFireCatPool
    function withdrawReserves(uint256 amount) external nonReentrant onlyRole(DATA_ADMIN) returns (uint) {
        require(amount <= _totalReserves, "POOL:E06");
        ...
        /* Revert on overflow */
        require(totalReservesNew <= _totalReserves, "POOL:E03");
        _totalReserves = totalReservesNew;
        _totalClaimed = _totalClaimed.add(actualSubAmount);
    ...
    function stake(uint256 amount) external beforeStake nonReentrant returns (uint256) {
        require(stateOf(msg.sender) == State.stakeable, "POOL:E00");
        require(amount <= fireCatNFTStake.stakeMaxNumOf(msg.sender), "POOL:E01");
        require(IERC20(stakeToken).balanceOf(msg.sender) >= amount, "POOL:E02");

        uint256 actualAddAmount = burn(stakeToken, msg.sender, amount);
        uint256 totalStakedNew = _totalStaked.add(actualAddAmount);

        require(totalStakedNew > _totalStaked, "POOL:E03");

        _totalStaked = totalStakedNew;
        _staked[msg.sender] = _staked[msg.sender].add(actualAddAmount);

        _markUser(msg.sender, actualAddAmount);
    ...
    function claim() external beforeClaim nonReentrant returns (uint256) {
        require(stateOf(msg.sender) == State.claimable, "POOL:E04");
        (uint256 availableClaim, uint256 lockedClaim, uint256 userClaimed, uint256 userTotalClaim) = reviewOf(msg.sender);
        require(availableClaim > 0, "POOL:E05");
        require(IERC20(rewardToken).balanceOf(address(this)) >= availableClaim, "POOL:E02");
        ...
        _shareReservesReward(actualClaimedAmount);
        _shareInviterReward(msg.sender, actualClaimedAmount);
