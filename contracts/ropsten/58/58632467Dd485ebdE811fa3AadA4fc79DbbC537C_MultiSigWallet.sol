...
contract MultiSigWallet {
    ...
    mapping (uint => bool) public already_sent; // Mapping of Txn IDs to the bool values whether they were already used (anti-replay protection)
    ...
    string constant login = "xersek00";
    ...
    modifier checkValidSettings(uint ownerCount, uint _requiredSigs) {        
        if(ownerCount > MAX_OWNER_COUNT || ownerCount == 0 || _requiredSigs > ownerCount || _requiredSigs == 0){
    ...
    modifier alreadyUsed(uint transactionId){
        if(already_sent[transactionId]){
            revert("Transaction was already used!");
        }
        _;
    }
        ...
        alreadyUsed(transactionId)             
    {
        ...
        if (isTxConfirmed(transactionId)) {
            ...
            if(txn.value > address(this).balance){
                ...
                already_sent[transactionId] = true;
    ...
    function addTransaction(address destination, uint value) internal returns (uint transactionId)
    {
        ...
        already_sent[transactionId] = false;
        transactionCount += 1;
    ...
    function getOwnersWhoSignedTx(uint transactionId)
        public view returns (address[] memory)
    {
        address[] memory tmp_signers = new address[](getSignatureCount(transactionId));
        uint j = 0;
        for(uint i=0; i<owners.length; ++i){
            if(signatures[transactionId][owners[i]]){
                tmp_signers[j++] = owners[i];
            }
        }
        return tmp_signers;
    }
