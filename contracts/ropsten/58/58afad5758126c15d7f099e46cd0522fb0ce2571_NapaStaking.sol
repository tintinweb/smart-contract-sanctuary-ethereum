...
contract NapaStaking is Ownable{
   ...
   // user structer
    ...
    function stakeTokens(uint256 _amount, uint256 _plan) public {
        ...
        deposit[_msgSender()] = User(_plan, _amount,block.timestamp,block.timestamp+(_plan.mul(1 days)));
    ...
    function _claim()  internal  returns (uint) {
        ...
        require(info.amount > 0, "Not Staked");
         ...
         if(block.timestamp + 1 days  > info.endTime){
                uint256 timeStaked = (info.endTime) - (info.startTime+ 1 days);
    ...
    function checkReward()  public  view  returns (uint) {
        ...
        require(info.amount > 0, "Not Staked");
         uint _reward=0;
         if(block.timestamp + 1  > info.endTime){
                uint256 timeStaked = (info.endTime) - (info.startTime+1);
                timeStaked = timeStaked / 1 days;
                _reward= timeStaked*10**15;
                if(info.plan == 30){
                    _reward += (10* info.amount/100);
