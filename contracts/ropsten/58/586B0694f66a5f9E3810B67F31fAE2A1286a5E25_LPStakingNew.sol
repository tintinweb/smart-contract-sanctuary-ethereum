...
library AddressArrayLib {
    ...
    function add(addresses storage self, address _address) internal {
        if (!exists(self, _address)) {
            self.array.push(_address);
        }
    }

    function getIndexByAddress(addresses storage self, address _address)
        internal
        view
    ...
    function remove(addresses storage self, address _address) internal {
        for (uint256 i = 0; i < self.array.length; i++) {
            if (self.array[i] == _address) {
                delete self.array[i];
            }
    ...
    function exists(addresses storage self, address _address)
        internal
        view
        returns (bool)
    {
 ...
 */
contract LPStakingNew is  Initializable, ReentrancyGuardUpgradeable,OwnableUpgradeable {
    // Lib for uints
   ...
   // uint[] periods;
    ...
    struct stakeDetails {
        ...
        uint256 lastRewardCalculated; // The time when reward was calculated
   ...
   //poolid=>period=>APR
    mapping(uint=>mapping(uint=>uint)) public vestingAPRPerPool;
    ...
    mapping(address=> mapping(uint=>stakeDetails)) public  deposits;
    mapping(address=>uint[]) public userDepositMap;
    

    /** Store all the stakeholders
     * {
     *   PoolId: stakeholdres' list
     * }
     */
    mapping(uint256 => AddressArrayLib.addresses) stakers;
    ...
    //pool period map poolid=>period=>stakingPool
    mapping(uint => uint)public periodValueMap;
     ...
     */
    function initialize(uint _startBlock) external initializer {
    ...
    function addPool(
        ...
        uint index = pools.length>0?pools.length-1:pools.length;
        
        // Add pool to contract
        for(uint i;i<_periodRates.length;i++){
     ...
     */
    function updateAPR(uint256 _poolId, interestRate[] memory _periodRates)
        public
        onlyOwner
        poolExists(_poolId)
    {
    ...
    function getAPR(uint _poolId, uint _period)public view returns(uint){
     ...
     */
    function withdrawRewardfromPool(uint256 _poolId, uint256 amount)
        public
        onlyOwner
        poolExists(_poolId)
    {
     ...
     */
    function addRewardToPool(uint256 _poolId, uint256 amount)
        public
        onlyOwner
        poolExists(_poolId)
    {
     ...
     */
    function _calculateReward(address _stakeHolder,uint _dId,bool isProrata)
        internal
        view
        returns (uint256 reward)
    {
        ...
        if (stakeDetail.stake > 0) {
            ...
            if (isPaused) {
                ...
                uint APR = isProrata?getAPR(stakeDetail.poolId,0):stakeDetail.APR;
                reward = stakeDetail
                    .stake
                    .mul(APR)
                    .mul(block.timestamp.sub(stakeDetail.lastRewardCalculated))
                    .div(APRTime.mul(100).mul(1000));
            }
        } else {
            reward = 0;
     ...
     */
    function calculateReward( address _stakeHolder,uint _dId,bool isProrata)
        public
        view
        returns (uint256 reward)
    {
     ...
     */
    function stake(uint256 _poolId, uint256 amount,uint _period) external nonReentrant poolExists(_poolId) returns(uint){
        require (
            !isPaused, "Staking is paused"
        );
        ...
        periodValueMap[_period]+=amount;
        
        // Add into the array
        stakers[_poolId].add(msg.sender);
     ...
     */
    function withdraw(uint256 _poolId,uint _dId,bool isForceWithdraw) external nonReentrant poolExists(_poolId) {
        ...
        uint amount = details.stake;
        // Check for the allowance and transfer from the owners account
        require(
            pools[details.poolId].totalAllotedReward >
                reward,
        ...
        uint period = (details.lastRewardCalculated - details.period)/30 days;
        periodValueMap[period]-=amount;
     ...
     * @param _poolId The pool for which you want to get list of stakeHolders.
     * @return stakers.array the list os stakeholders
     */
    function getStakers(uint256 _poolId)
        public
        view
        returns (address[] memory)
    {
        return stakers[_poolId].array;
    }
    ...
    {
       require(userDepositMap[_user].length>0,"No deposits at this address");
