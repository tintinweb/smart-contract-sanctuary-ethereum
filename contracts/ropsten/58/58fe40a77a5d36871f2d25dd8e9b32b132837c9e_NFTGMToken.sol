...
contract NFTGMToken is BEP20, MinterRole {
    ...
    // uint256 public maxSupply = 380000 * 10**9;

    // Burn address
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD; // INMUTABLE

    address public constant V2ROUTER = 0x60aE616a2155Ee3d9A68541Ba4544862310933d4;
     ...
     */
    constructor() public BEP20("NFTGamingDAO", "PNFTGM") {
        operator = _msgSender();
        ...
        _mint(msg.sender, 10000000 gwei);
        
    }

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    // function mint(address _to, uint256 _amount) public onlyMinter {
    //     require(totalSupply() + _amount <= maxSupply, "Total supply can't over MaxSupply");
    //     _mint(_to, _amount);
    // }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            require(!isBlacked[sender], "sender is blacked to trasner");
    ...
    function setBlackList(address _account, bool _blacked) public onlyOperator {
        isBlacked[_account] = _blacked;
    }
