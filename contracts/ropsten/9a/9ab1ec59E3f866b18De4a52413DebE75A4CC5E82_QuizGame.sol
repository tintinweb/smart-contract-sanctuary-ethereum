...
contract QuizGame {
    ...
    constructor(uint256 _playerReward) {
        manager = msg.sender;
        quizIds = 0;
    ...
    function getPlayerResult(uint256 _quizId)
        public
        view
        returns (bool winner)
    {
        ...
        require(
            quiz.status == Status.Finished,
            "Quiz status should be Started"
        );
        bytes32[] memory quizAnswers = quiz.answers;
        bytes32[] memory senderAnswers = answers[msg.sender][_quizId];
    ...
    function withdraw(uint256 _quizId) public {
        ...
        require(getPlayerResult(_quizId) == true, "Not a winner");
        require(
        ...
        quiz.token.transfer(msg.sender, playerReward);
        emit WinnerWithdraw(_quizId, msg.sender, playerReward);
    }
