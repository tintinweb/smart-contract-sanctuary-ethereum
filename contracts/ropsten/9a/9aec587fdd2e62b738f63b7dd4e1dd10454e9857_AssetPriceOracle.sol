...
contract AssetPriceOracle is IOracle, Ownable {
    ...
    uint256 public roundId = 0; // start 0
    uint256 private constant minUpdateTime = 1200; // 20 minutes

    constructor(){
      // admin = msg.sender;
      adminMap[_msgSender()] = true;
    }

    modifier onlyAdmin() {
        // require(_msgSender() == admin, "NFTOracle: !admin");
        require(adminMap[_msgSender()], "NFTOracle: !admin");
        _;
    }
    ...
    function addAsset(Asset calldata asset) external override onlyOwner returns(bool) {
    ...
    function addAssets(Asset[] calldata _assets) external override onlyOwner returns(bool) {
        ...
        returns (bool)
    {
        ...
        require(assets.length * ASSET_PRICE_DATA_BYTES == data.length,"Oracle: asset and data length disaccord");
        uint256 _blockTimestamp = block.timestamp;
        if(roundId > 0){
          AssetPriceData storage lastPriceData = assetDataList[(roundId-1) % DATA_SIZE];
          require(_blockTimestamp - lastPriceData.updateAt >= minUpdateTime,"Oracle:data update time interval error");
        }
