...
contract UniswapTest is IUniswapV2Callee {
    ...
    // assume 0 is usdc and 1 is wbtc
    // uniswap v2 pair wbtc is cheaper 16k for 1 wbtc we borrow wbtc
    // go to sushipswap and sell for 31k, refund 16k should make 15k usdc
    function testRun(
        address token0,
        address token1,
        uint256 amount0,
    ...
    function uniswapV2Call(
        ...
        // get the addresses of the two tokens from the uniswap liquidity pool because msg.sender is the uniswapv2Pair
        ...
        path2[0] = _amount0 == 0 ? token0 : token1;
        path2[1] = _amount0 == 0 ? token1 : token0;
        ...
        emit Transfer(token0, token1, amountTokenBorrowed, amountRequired);
        ...
        require(amountReceived >= amountRequired, "This is not profitable");
        outputToken.transfer(tx.origin, amountReceived - amountRequired);
    }

    function getAmountsIn(uint256 amountTokenBorrowed, address[] calldata path)
        external
        returns (uint256 amountRequired)
    {
        address addr0 = path[0];
        address addr1 = path[1];
        amountRequired = UniswapV2Library.getAmountsIn(
            factory,
            amountTokenBorrowed,
            path
        )[0];
        emit eventGetAmountsIn(
            factory,
            amountTokenBorrowed,
            addr0,
            addr1,
