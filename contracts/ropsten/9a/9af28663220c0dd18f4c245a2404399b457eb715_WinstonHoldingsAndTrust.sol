...
contract WinstonHoldingsAndTrust is ERC20, ERC20Snapshot, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using Counters for Counters.Counter;
    event Bought(uint256 paid, uint256 received);
    event Sold(uint256 paid, uint256 received);
    event Swap(address from, address to, uint256 amount0, uint256 amount1);
    event Drip(address user, uint256 amount);
    event Reward(address token, uint256 userPaid, uint256 amountPaid);
    event PairCreated(uint256, address, address, uint256, uint256);
    event PairUpdated(uint256, address, address, uint256, uint256);
    event PairWithdraw(uint256, address, address, uint256, uint256);
    struct Account {
        uint256 stake;
    ...
    address[] _users;
    address[] _claiments;
    uint256 private _mTV = 1 * 10 ** decimals();
    ...
    constructor() ERC20("Winston Holdings And Trust", "WAHT") {
        _mint(address(this), 10 * 10 ** decimals());
        _tokenListing[address(this)] = true;
    }
    ...
    function netBalance() public view returns (uint256) {
    ...
    function addPair(address token0, address token1, uint256 amount0, uint256 amount1) external returns (uint256) {
        require(token0 == token1, "Cannot create this pair.");
        require(!_isListed(token0), "Winston Error: token0 Not listed.");
        require(!_isListed(token1), "Winston Error: token1 Not listed.");
        ...
        _new.token0 = token0;
        _new.token1 = token1;
        _new.reserve0 = amount0;
        _new.reserve1 = amount1;
        _new.owner = _msgSender();
        _pairs.push(_new);
        //stake min
        //stake max      
        emit PairCreated(currentId, token0, token1, amount0, amount1);
        return currentId;
    }
    ...
    function updatePair(address token0, address token1, uint256 amount0, uint256 amount1) public returns (bool) {
        ...
        IERC20(token0).safeTransferFrom(_msgSender(), address(this), amount0);
        IERC20(token1).safeTransferFrom(_msgSender(), address(this), amount1);
        emit PairUpdated(pair.id, token0, token1, pair.reserve0, pair.reserve1);
    ...
    function withdrawPair(address token0, address token1, uint256 amount0, uint256 amount1) public returns (bool) {
        ...
        IERC20(token0).safeTransferFrom(_msgSender(), address(this), amount0);
        IERC20(token1).safeTransferFrom(_msgSender(), address(this), amount1);
        emit PairWithdraw(pair.id, token0, token1, pair.reserve0, pair.reserve1);
        return listed && _msgSender() == pair.owner;
    }
    ...
    function setVoterToken(address token) external onlyOwner {
        _voterToken = token;
    ...
    function newProposal(uint256 start, uint256 end, string calldata url ) external {
        require((IERC20(_voterToken).balanceOf(_msgSender()) >= _mTV) == true, "Winston Error: Insufficient Rights.");
    ...
    function buy() payable public nonReentrant {
        ...
        require(amountTobuy > 1000000, "You need to send at least 0.000000000001 eth");
        require(amountTobuy <= dexBalance, "Not enough tokens in the reserve");
        require(dexBalance > 0,  "Insufficient Liquidity.");
        IERC20(address(this)).safeTransfer(msg.sender, msg.value / 10000);
        emit Bought(msg.value, msg.value / 10000);
    }
    
    function sell(address token, uint256 amount) public nonReentrant returns (uint256) {
        require(amount > 0, "You need to sell at least some tokens");
        uint256 allowance = IERC20(token).allowance(msg.sender, address(this));
        require(allowance >= amount, "Check the token allowance");
        require(address(this).balance >= amount * 9999, "Insufficient Liquidity.");
        require(amount > 0,  "Insufficient Liquidity.");
        uint256 amountPaid = amount * 9999;
        IERC20(token).transferFrom(msg.sender, address(this), amount);
        (bool success, ) = payable(msg.sender).call{value: amountPaid}("");
        emit Sold(amount, amountPaid);
        require(success);
        return amountPaid;
    }

    function swap(address from, address to, uint256 amount) public nonReentrant {
        require(amount > 0, "You need to swap at least some tokens");
        require(!_isListed(from), "Winston Error: token0 Not listed.");
        require(!_isListed(to), "Winston Error: token1 Not listed.");
        ...
        if(existingPair.reserve0 > existingPair.reserve1) {
            ...
            IERC20(to).transfer(msg.sender, x);
    ...
    function fund(address token, uint256 amount) public nonReentrant returns (uint256) {
        require(_isListed(token) == true, "Winston Error: Not listed.");
        require(IERC20(token).balanceOf(_msgSender()) >= amount, "Winston Error: Insufficient balance.");
        require((IERC20(token).allowance(_msgSender(), address(this)) < amount) == true, "Winston Error: Insufficient allowance.");
        IERC20(token).transferFrom(msg.sender, address(this), amount);
    ...
    function claimed(address user) public view returns (bool) {
        for (uint i = 0; i < _claiments.length; i++) {
            if (_claiments[i] == user) {
                return true;
            }
        }
        return false;
    ...
    function faucet(address token) external nonReentrant {
        require(_isListed(token), "Winston Error: Not listed.");
    ...
    function unstake(address token, uint256 amount) external nonReentrant {
        ...
        require((_accounts[_msgSender()][token].stake >= amount) == true, "Winston Error: Insufficient balance.");
        _accounts[_msgSender()][token].stake -= amount;
        // base usage fee
        uint256 withdrawAmountFee = amount / 1000;
        rewards[token] += withdrawAmountFee;
        uint256 withdrawAmount = amount -  withdrawAmountFee;
        //early withdraw?
        //vestedPeriod >= 30 days
        //not vested??
        IERC20(token).safeTransfer(_msgSender(), withdrawAmount);
        uint256 withdrawRewardFee = currentRewards / 10000;
    ...
    function sendToken(address token, address recipient, uint256 amount) external onlyOwner nonReentrant {
        require((IERC20(token).balanceOf(address(this)) >= amount) == true, "Winston Error: Contract balance not enough to perform operation");
