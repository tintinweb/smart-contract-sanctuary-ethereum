...
contract Muscle is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 15 * 10**11 * 10**18;
    ...
    string private _name = "Muscle";
    string private _symbol = "$MUSCLE";
    ...
    uint256 private numTokensSellToAddToLiquidity = 225 * 10**7 * 10**18;
    uint256 public _maxTxAmount = 15 * 10**9 * 10**18;
    uint256 public maxWalletHoldingAmount = 15 * 10**9 * 10**18;

    //Marketing tax will be split based on divisor for dev & marketing wallet
    uint256 private _divisor = 50; //Not sales tax, divisor for Marketing & Dev wallets
    ...
    constructor(uint256 _snipeBlockAmt, uint256 _gasPriceLimit, address payable devWallet, address payable marketingWallet) {
    ...
    function setBuyFees(uint256 taxFee,uint256 liquidityFee,uint256 mFee) external onlyOwner() {
         require(taxFee <= 5, "Cannot set BuytaxFee higher than 5%");
         require(liquidityFee <= 5, "Cannot set BuyliquidityFee higher than 5%");
         require(mFee <= 10, "Cannot set BuymFee higher than 10%");
     ...
     function setSellFees(uint256 taxFee,uint256 liquidityFee,uint256 mFee) external onlyOwner() {
         require(taxFee <= 5, "Cannot set SelltaxFee higher than 5%");
         require(liquidityFee <= 5, "Cannot set SellliquidityFee higher than 5%");
         require(mFee <= 10, "Cannot set SellmFee higher than 10%");
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > 1000000000, "Cannot set transaction amount as zero");
        _maxTxAmount = maxTxAmount * 10**_decimals;
    }    

    function setMaxWalletHoldingAmount(uint256 newNum) external onlyOwner() {
        require(maxWalletHoldingAmount > 1000000000, "Cannot set transaction amount as zero");
        maxWalletHoldingAmount = newNum * 10**_decimals;
    }
    ...
    _marketingWallet.transfer(amt0);
    _devWallet.transfer(amount - amt0);
    }
    ...
    function openTrading() internal onlyOwner {
        tradingOpen = true;
    ...
    function manualsend() external {
        ...
        transferToAddressETH(contractETHBalance);
    }
    ...
    function _transfer(
        ...
        if(from != owner() || to != owner()){          
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                require(amount + balanceOf(to) <= maxWalletHoldingAmount, "Max wallet exceeded");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(transferredBalance);
    }
    ...
    function launch() external onlyOwner returns (bool){
        require(!tradingOpen, "Trading is already active, cannot relaunch.");
        openTrading();
