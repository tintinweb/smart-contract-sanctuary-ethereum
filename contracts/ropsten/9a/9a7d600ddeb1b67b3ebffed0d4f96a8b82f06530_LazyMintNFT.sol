    ...
    AccessControl
{
    ...
    mapping(uint256 => mapping(address => RoyaltyInfo)) private royalties;
    ...
    function royaltyInfo(uint256 id, address seller, uint256 value)
        external
        view
        override
        returns (
    ...
    {
        RoyaltyInfo memory royalties_ = royalties[id][seller];
        address _receiver = royalties_.recipient;

        for (uint256 i = 0; i < royaltyAmount.length; i++) {
            receiver[i] = _receiver;
            uint256 _royaltyAmount = (value * royalties_.amount) / 10000;
            royaltyAmount[i] = _royaltyAmount;
            totalAmount += royaltyAmount[i];
        }
    }
    ...
    function setTokenRoyalty(
        ...
        require( balanceOf(msg.sender,id) > 0, "You don't own the token");
        royalties[id][msg.sender] = RoyaltyInfo(recipient, value);
    }
    ...
    function redeem(
        ...
        require(isApprovedForAll(signer, _msgSender()),"redeem: not approved");
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(uint256 tokenId,uint256 amount,uint256 price,uint256 ts,string tokenURI)"
                        ),
                        voucher.tokenId,
                        voucher.amount,
                        voucher.price,
