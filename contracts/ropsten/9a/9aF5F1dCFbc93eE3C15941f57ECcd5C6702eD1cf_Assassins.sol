...
contract Assassins is ERC721Enumerable, Ownable {
    ...
    uint256 public constant max_bills = 15;
    uint256 private _maxRedemption = 10;
    uint256 private _mintLimit = 10;
    uint256 private _price = 700000000000000; //0.5 ETH
    uint256 private _memberPrice = 500000000000000;
    uint256 private _publicSaleTime = 1664616000;
    uint256 private _memberSaleTime = 1664612400;
    uint256 private _earlyAccessTime = 1664583600;
    uint256 private _earlyAccessClaimed = 0;
    uint256 private _totalMaxEarlyAccess = 1200;
    uint256 private _indMaxEarlyAccess = 2;
    ...
    constructor(string memory baseURI)
        ERC721("Board Ape Resistence", "REBELS")
    {   
        setBaseURI(baseURI);
    ...
    function getNormalSaleTime(address user) public view returns (uint256) {
        ...
        if(user !=address(0) ){
            if (walletApprovedForEA(user) && _eAMinted[user] <=_indMaxEarlyAccess && _earlyAccessClaimed <= _totalMaxEarlyAccess && block.timestamp < _memberSaleTime){
    ...
    function earlyAccessMint (uint256 _count) public payable{
        ...
        require(totSupply < max_bills, "All Rebels have been Claimed.");
        require(totSupply + _count <= max_bills,"There are not enough Rebels available");
