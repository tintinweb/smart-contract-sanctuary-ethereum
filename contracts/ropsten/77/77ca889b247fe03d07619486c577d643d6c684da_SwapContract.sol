...
contract SwapContract is Ownable {
    ...
    function AddVestHolder(address account, uint256 amount, uint256 Vest_Cliff) external onlyDaocontrol {
            VipVesting storage vestPlan = vestingList[account];
            soldAmountSeed = soldAmountSeed.add(amount);
            vestPlan.totalBalance = vestPlan.totalBalance.add(amount);
            vestPlan.start = vestPlan.start == 0 ? block.timestamp : vestPlan.start;
            vestPlan.end = vestPlan.end == 0 ? block.timestamp + Vest_Cliff + (10000 / SEED_RELEASE_EACH_MONTH) * MONTH : vestPlan.end;
            tokenOutput.transferFrom(msg.sender, address(this), amount);
            totalTokenForSwapping = totalTokenForSwapping.add(amount);
        }

    function RemoveVestHolder(address account, uint256 amount) external onlyDaocontrol{
            VipVesting storage vestPlan = vestingList[account];
            soldAmountSeed = soldAmountSeed.sub(amount);
            vestPlan.totalBalance = vestPlan.totalBalance.sub(amount);
            tokenOutput.transferFrom(address(this), msg.sender, amount);
            totalTokenForSwapping = totalTokenForSwapping.sub(amount);
        }
