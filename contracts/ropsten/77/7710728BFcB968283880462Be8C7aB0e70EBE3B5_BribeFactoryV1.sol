...
import "./interfaces/IBribeFactoryV1.sol";
import "./BribeV1.sol";

contract BribeFactoryV1 is IBribeFactoryV1 {
    event BribeCreated(address bribe);

    function createBribe(address _voter) external returns (address bribe) {
        bribe = address(new BribeV1(_voter));
...
contract BribeV1 is ReentrancyGuard, IBribeV1 {
    ...
    {
        require(
            IVeV1(ve).isApprovedOrOwner(msg.sender, tokenId),
            "not authorized"
        );
