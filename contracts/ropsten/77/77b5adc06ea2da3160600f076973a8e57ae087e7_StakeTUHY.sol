...
contract StakeTUHY is Initializable {
    ...
    uint256 private APY;                // How much reward single NFT will receive
    uint256 updatedTime;
    ...
    function initialize() public initializer  {
        _token = IERC20Upgradeable(0x8b3dC8D4Edd8944bB8EF11913af9C3b55140805D);
        NFTAddress = 0x0E4245ccB9954eAd73F8Bab13607cD2d86eFC84c;
        owner = msg.sender;
    ...
    function claimAble(address _user) public view returns(uint256, uint256){
        ...
        uint256 perDayReward = stakee.NFT.length.mul(APY);
        uint256 claimableDays;
        if(stakee.stake_time > updatedTime){
            claimableDays = block.timestamp.sub(stakee.stake_time).div(1 minutes);
        }else{
            if(updatedTime > block.timestamp){
                claimableDays = 0;
            }else{
                claimableDays = block.timestamp.sub(updatedTime).div(1 minutes);
            }
        }
    ...
    function redeem() public{
        ...
        require(block.timestamp.sub(stakes[msg.sender].stake_time).div(1 minutes) >= maturityDays,'Rewards not matured');

        (, uint256 claimableReward) = claimAble(msg.sender);
        ...
        _token.safeTransfer(msg.sender, (claimableReward * 1000000000000000000));
    }
   ...
   function _unStake(uint256 __ids) public {
    ...
    function changeStakingAPY(uint256 newAPY) public onlyOwner{
        require(newAPY > 0, '> 0 tokens per day');
        APY = newAPY;
    }
