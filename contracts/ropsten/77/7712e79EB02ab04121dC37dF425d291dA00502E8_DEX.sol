...
contract DEX {
    ...
    uint256 usdc_rate =  1 * 10**uint(decimals());

    constructor(address _token) public {
        ntoken = IERC20(_token);
        _owner = msg.sender;
    }
    ...
    function buy() payable public {
        ...
        uint256 dexBalance = ntoken.balanceOf(address(this));
        require(amountTobuy > 0, "You need to send some ether");
        require(amountTobuy <= dexBalance, "Not enough tokens in the reserve");
        ntoken.transfer(msg.sender, amountTobuy);
        emit Bought(amountTobuy);
    }

    function usdc_buy(uint256 value) payable public {
        uint256 amountTobuy = value*usdc_rate;
        uint256 dexBalance = ntoken.balanceOf(address(this));
        require(amountTobuy > 0, "You need to send some ether");
        require(amountTobuy <= dexBalance, "Not enough tokens in the reserve");
        ntoken.transfer(msg.sender, amountTobuy);
