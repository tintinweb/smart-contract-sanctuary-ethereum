...
contract Treasury is mortal {
    ...
    function withdrawAll(
        ...
        address tempAddress = payable_addresses[0];
        uint256 tempAmount = withdraw_amounts[0];
        for (uint256 i = 1; i < withdraw_amounts.length; i++) {
            if (i < withdraw_amounts.length - 1) {
                if (tempAddress != payable_addresses[i]) {
                    daitoken.transfer(tempAddress, tempAmount);
                    tempAmount = 0;
                    tempAddress = payable_addresses[i];
                } else {
                    tempAmount += withdraw_amounts[i];
                }
            } else {
                if (tempAddress != payable_addresses[i]) {
                    daitoken.transfer(tempAddress, tempAmount);
