...
contract ExchangeNFT is Ownable, TokenRecipient{
    ...
    function validateTransaction(
        ...
        uint256[3] memory uintTokens,
        string memory str)
        internal
        view
        returns (bool)
    {
        if (contracts[0] != address(this)) {
            return false;
        }
        if(contracts[1] != address(0) && !contracts[1].isContract()){
            return false;
        }
        if(contracts[2] != address(0) && !contracts[2].isContract()){
            return false;
        }
        if(keccak256(bytes(str)) == keccak256(bytes("BUY")) && msg.sender != owner && isAdmin[msg.sender] == false) {
            if (addrs[0] == address(0) || addrs[1] == address(0) 
            || addrs[2] == address(0) || addrs[3] == address(0)
            || addrs[5] == address(0) || addrs[6] == address(0)) {
                return false;
            }
            if (addrs[1] != feeWallet || addrs[1] == msg.sender || addrs[3] == msg.sender) {
                return false;
            }
            if(addrs[4] != address(0) && (addrs[5] == address(0) || addrs[5] == msg.sender)){
                return false;
            }
            if(uintTokens[0] == 0 || uintTokens[1] == 0 || uintTokens[2] == 0){
                return false;
            }
        }
        return true;
    ...
    function buy(
        ...
        string[2] calldata strs)
        public
        payable{
        require(keccak256(bytes(strs[0])) == keccak256(bytes("BUY")),"Invalid sale type");
        require(validateTransaction(contracts, addrs, uintTokens, strs[0]));
        ...
        if(contracts[2] != address(0)){
            ...
            uint256 balanceOfToken = exchangeNft.balanceOf(uintTokens[0]);
            if(balanceOfToken == 0) {
                exchangeNft.mint(addrs[6], uintTokens[0], uintTokens[1], strs[1]);
            }
            exchangeNft.safeTransferFrom(addrs[6], addrs[7], uintTokens[0],  uintTokens[2], "0x0");
        }
    }
    ...
    function exchange(
        ...
        adminOnly()
    {
        // require(validateTransaction(contracts, addrs, uintTokens,  ""));
        // require(contracts[1] != address(0));
        uint256 feeValue = uints[0];
        uint256 sellValue = uints[1];
        uint256 royaltyValue = uints[2];
        // uint256 totalValue =  (feeValue + sellValue + royaltyValue);
        ...
        if(keccak256(bytes(strs[0])) == keccak256(bytes("BUY"))) {
            ...
            if(addrs[1] != address(0)){
                require(exchangeToken.transfer(addrs[1], feeValue));
            }
            if(addrs[3] != address(0)){
                require(exchangeToken.transfer(addrs[3], sellValue));
            }
            if(addrs[5] != address(0)){
                require(exchangeToken.transfer(addrs[5], royaltyValue));
            }
        }else{
            // require(contracts[1] != address(0), "can not bid/offer for currency");
            if(addrs[1] != address(0)){
                transferTokens(contracts[1], addrs[0], addrs[1], feeValue);
            }
            if(addrs[3] != address(0)){
                transferTokens(contracts[1], addrs[2], addrs[3], sellValue);
            }
            if(addrs[5]!= address(0)){
                transferTokens(contracts[1], addrs[4], addrs[5], royaltyValue);
            }
        }
        ...
        if(contracts[2] != address(0)){
            exchangeNft = ERC1155(contracts[2]);
            uint256 balanceOfToken = exchangeNft.balanceOf(uintTokens[0]);
            if(balanceOfToken == 0 && uintTokens[0] > 0) {
                exchangeNft.mint(addrs[6], uintTokens[0], uintTokens[1], strs[1]);
            }
            if(uintTokens[2] > 0){
                exchangeNft.safeTransferFrom(addrs[6], addrs[7], uintTokens[0],  uintTokens[2], "0x0");
