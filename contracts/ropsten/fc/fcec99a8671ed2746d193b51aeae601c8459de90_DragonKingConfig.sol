...
contract DragonKingConfig is Ownable {
  ...
  // tokens [tpt, ndc, skl, xper, mag, stg, dex, luck]
  constructor(uint8 characterFee, uint8 eruptionThresholdInHours, uint8 percentageOfCharactersToKill, uint128[] charactersCosts, address[] tokens) public {
    ...
    purchaseRequirements[7].amounts = [5*(10**2)];
    purchaseRequirements[8].tokens = [tokens[5]]; // 10 STG
    purchaseRequirements[8].amounts = [10*(10**2)];
    purchaseRequirements[9].tokens = [tokens[5]]; // 20 STG
    purchaseRequirements[9].amounts = [20*(10**2)];
    purchaseRequirements[10].tokens = [tokens[5]]; // 50 STG
    purchaseRequirements[10].amounts = [50*(10**2)];
    purchaseRequirements[11].tokens = [tokens[5]]; // 100 STG
    purchaseRequirements[11].amounts = [100*(10**2)];
    // wizards
    purchaseRequirements[15].tokens = [tokens[2], tokens[3]]; // 5 SKL % 10 XPER
    purchaseRequirements[15].amounts = [5*(10**18), 10*(10**2)];
    purchaseRequirements[16].tokens = [tokens[2], tokens[3], tokens[4]]; // 10 SKL & 20 XPER & 5 MAG
    purchaseRequirements[16].amounts = [10*(10**18), 20*(10**2), 5*(10**2)];
    purchaseRequirements[17].tokens = [tokens[2], tokens[3], tokens[4]]; // 25 SKL & 50 XP & 10 MAG
    purchaseRequirements[17].amounts = [25*(10**18), 50*(10**2), 10*(10**2)];
    purchaseRequirements[18].tokens = [tokens[2], tokens[3], tokens[4]]; // 50 SKL & 100 XP & 20 MAG
    purchaseRequirements[18].amounts = [50*(10**18), 100*(10**2), 20*(10**2)]; 
    purchaseRequirements[19].tokens = [tokens[2], tokens[3], tokens[4]]; // 100 SKL & 200 XP & 50 MAG 
    purchaseRequirements[19].amounts = [100*(10**18), 200*(10**2), 50*(10**2)];
    purchaseRequirements[20].tokens = [tokens[2], tokens[3], tokens[4]]; // 200 SKL & 400 XP & 100 MAG
    purchaseRequirements[20].amounts = [200*(10**18), 400*(10**2), 100*(10**2)];
    // archers
    purchaseRequirements[21].tokens = [tokens[2], tokens[3]]; // 5 SKL % 10 XPER
    purchaseRequirements[21].amounts = [5*(10**18), 10*(10**2)];
    purchaseRequirements[22].tokens = [tokens[2], tokens[3], tokens[6]]; // 10 SKL & 20 XPER & 5 DEX
    purchaseRequirements[22].amounts = [10*(10**18), 20*(10**2), 5*(10**2)];
    purchaseRequirements[23].tokens = [tokens[2], tokens[3], tokens[6]]; // 25 SKL & 50 XP & 10 DEX
    purchaseRequirements[23].amounts = [25*(10**18), 50*(10**2), 10*(10**2)];
    purchaseRequirements[24].tokens = [tokens[2], tokens[3], tokens[6]]; // 50 SKL & 100 XP & 20 DEX
    purchaseRequirements[24].amounts = [50*(10**18), 100*(10**2), 20*(10**2)]; 
    purchaseRequirements[25].tokens = [tokens[2], tokens[3], tokens[6]]; // 100 SKL & 200 XP & 50 DEX 
    purchaseRequirements[25].amounts = [100*(10**18), 200*(10**2), 50*(10**2)];
    purchaseRequirements[26].tokens = [tokens[2], tokens[3], tokens[6]]; // 200 SKL & 400 XP & 100 DEX
    purchaseRequirements[26].amounts = [200*(10**18), 400*(10**2), 100*(10**2)];
