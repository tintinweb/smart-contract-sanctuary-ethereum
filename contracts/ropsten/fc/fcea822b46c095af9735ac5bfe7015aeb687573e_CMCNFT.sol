...
/**
Token name: Cannabis Miners Club NFT
Token Symbol: CMC
Total Supply: 10,000
How many mint can be done by one wallet? 
For whitelist is 1 and 2 mint max by one wallet address, for public sale its 1,3, and 5 mint per wallet
Price per mint: 0.1 ETH
*/
contract CMCNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  //owner can mint without paying cost
    if (msg.sender != owner()) {
        ...
        require(msg.value >= whitelistcost * _mintAmount, "Ether Amount is not correct");
        }
  ...
  function setmaxMintAmountForWhiteist(uint256 _newmaxMintAmount) public onlyOwner {
