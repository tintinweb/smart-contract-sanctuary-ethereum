...
contract BaseContract is Pausable, Initializable, Ownable, ReentrancyGuard {
    ...
    function callSwapsAndFees(SwapRequest memory request, Call[] calldata calls) private returns (bytes[] memory) {
        ...
        // Get Platform fee
        if (request.feeIn > 0) {
            _sendToken(request.fromToken, request.feeIn, feeContractAddress, isNative, false);
        ...
        if (request.affiliateIn > 0) {
            ...
            _sendToken(request.fromToken, request.affiliateIn, request.affiliatorAddress, isNative, false);
            emit AffiliateReward(request.fromToken, request.affiliatorAddress, request.affiliateIn);
        }

        bytes[] memory returnData = new bytes[](calls.length);
        for (uint256 i = 0; i < calls.length; i++) {
            (bool success, bytes memory ret) = isNative
