...
abstract contract MiddleLayerAdmin is MiddleLayerStorage, MiddleLayerModifiers {
    ...
    function setLoanAgent(address _loanAgent) external onlyOwner() {
    ...
    function addRoute(IRoute _newroute) external onlyOwner() {
        routes.push(_newroute);
        authRoutes[address(_newroute)] = true;
    }

    function removeRoute(IRoute _routeToRemove) external onlyOwner() {
        // slither-disable-next-line uninitialized-local
