...
// File: contracts/StandartNFTPlus.sol
...
contract StandardQDNFTPlus is ERC721QDPlus, Ownable  {
  using Strings for uint256;
  using ECDSA for bytes32;  

  //NFT cost
  uint128 constant public publicCost = 0 ether;
  uint128 constant public presaleCost = 0 ether;

  //provence hash for the art generation
  string constant public provanceHash = "";

  //erc721 metadata
  string constant private _name   = "NFT name";
  string constant private _symbol = "NFT simble";

  //verification address
  address constant private _signer = 0xe05655869a11FEbD52F7686bE1B2B48c40f7E613;

  //project max info
  uint16 constant private _maxSupply  = 5500;
  uint8  constant private _maxOwner   = 100; 
  uint8  constant private _maxPerTnx  = 20; 
  uint8  constant private _maxPresale = 2; 

  //QD payment
  uint256 private QDpayment = 10 ether;
  ...
  modifier inPresale() {
      require(_projectStage == 1, "contract is not on presale");
      _;
  } 
  ...
  function _mintNFT(address _to, uint256 _quantity, uint128 _price) internal {
      ...
      //Ony needs this is the project need max mint per wallet
      _mints[msg.sender] = _quantity;
  ...
  */
  ...
  // @dev mint the _quantity to the message.sender
  // @param _quantity is the quantity that will be minted
  function presaleMint(uint256 _quantity, bytes32 _hash, bytes memory _signature) public payable inPresale {
      require(_quantity + _mints[msg.sender] <= _maxPresale, "Invalid mint amount!");      
      require(_checkHash(_hash, _signature, _signer), "Address is not on Presale List");
      _mintNFT(msg.sender, _quantity, presaleCost);
  }  
  ...
  function ownerMintToAddress(uint256 _quantity, address _to) external onlyOwner  {
    require(_quantity + _mints[msg.sender] <= _maxOwner, "Invalid mint amount!");      
    _mintNFT(_to, _quantity, 0 ether);
  }
  ...
  function ownerMintAirdrop(address[] calldata _addresses) external onlyOwner  {
      uint256 _quantity = _addresses.length;
      require(_quantity + _currentTokenId <= _maxSupply,"Purchase exceeds available supply.");
      for (uint256 i = 0; i < _quantity; i++) {
          _mintNFT(_addresses[i], _quantity, 0 ether);
      }
  }  
  ...
  // @dev set a new _projectStage for the Smart Contract | Paused  = 0, Presale = 1, Public = 2. 
  // @param projectStage_ the new URI
  function setProjectStagePresale() public onlyOwner {
    _projectStage = 1;
  }  
