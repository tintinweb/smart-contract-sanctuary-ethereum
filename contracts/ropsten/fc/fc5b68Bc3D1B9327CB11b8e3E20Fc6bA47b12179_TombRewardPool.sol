  ...
  function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
    for (uint8 epochId = 2; epochId >= 1; --epochId) {
      if (_toTime >= epochEndTimes[epochId - 1]) {
        ...
        uint256 _generatedReward = _toTime.sub(epochEndTimes[epochId - 1]).mul(
          epochTombPerSecond[epochId]
        );
        if (epochId == 1) {
          ...
          if (_fromTime >= epochEndTimes[epochId - 1]) {
            return
              _generatedReward.add(
                epochEndTimes[epochId].sub(_fromTime).mul(epochTombPerSecond[epochId])
              );
          }
