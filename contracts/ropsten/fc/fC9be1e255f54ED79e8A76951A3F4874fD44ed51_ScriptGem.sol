...
error TypeOutOfRange(uint8 _type);

/// @title Common Gems
/// @author @n4beel
/// @notice Contract for Script TV Gems
contract ScriptGem is ERC721ABurnable {
    ...
    // mapping for containing type of a gem for each token ID
    mapping(uint256 => uint8) private gemsType;
     ...
     */
    constructor() ERC721A("ScriptGem", "SGEM") {
        scriptTV = msg.sender;
    }
     ...
     */
    function safeMint(address to, uint8 _type) external onlyScriptTV {
        if (_type < 0 || _type > 2) {
            revert TypeOutOfRange(_type);
        }
        uint256 tokenID = _nextTokenId();
        _safeMint(to, 1);
        gemsType[tokenID] = _type;
    }
