...
contract AKCCoin is ERC20, Ownable {
    ...
    uint256 public reserveAllocation = 25;
    address public tribeCore;

    /**
     * @dev Events
     */
    event SetTribeCoreEvent(address indexed core);
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _wallets.length; i++) {
            _mint(_wallets[i], _amounts[i] * (10**18));
        }
    }
    ...
    function _transfer(
        ...
        if (
            ...
            reserveAllocation < 100) {
                toTransfer = amount - (amount * reserveAllocation / 100);
        }
        super._transfer(from, to, toTransfer);
    }

    /** ONLY OWNER */
    function setTribeCore(address coreAddress)
        external
        onlyOwner {
            tribeCore = coreAddress;
            emit SetTribeCoreEvent(coreAddress);
        }

    function releaseAccidentalEth(uint256 percentage, address _to)
        external
        onlyOwner
    {
    ...
    function releaseAccidentalERC20(
        uint256 percentage,
