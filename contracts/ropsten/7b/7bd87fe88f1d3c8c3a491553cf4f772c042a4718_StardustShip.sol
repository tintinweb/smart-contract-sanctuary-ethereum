...
contract StardustShip is ERC721EnumerableUpgradeable, AccessControlUpgradeable {
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;
    bytes32 public constant STARDUST_CONTRACTS_ROLE = keccak256("STARDUST_CONTRACTS_ROLE");
    address public  STARDUST_CONTRACT;
    address public  BOX_CONTRACT;
    // Whitelist
    ...
    // map tokenId to Stardust struct
    mapping(uint => Ship) public allShips;

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() initializer {}

    event ShipUpgrade(uint shipId, uint lvl);
    ...
    function initialize(
        ...
        __ERC721_init("StardustShip", "SHIP");
        ...
        BOX_CONTRACT = _boxContract;
        _grantRole(STARDUST_CONTRACTS_ROLE, _boxContract);
    }

    function _mint(address to) private {
        uint nextTokenId = totalSupply();
        Ship memory newShip = Ship(nextTokenId, to, 0);
        allShips[nextTokenId] = newShip;
        _safeMint(to, nextTokenId);
    }

    function UpgradeShip(uint _tokenId, uint shiplvl) external onlyRole(STARDUST_CONTRACTS_ROLE) {
        allShips[_tokenId].lvl += 1;
        emit ShipUpgrade(_tokenId, shiplvl);
    }
    ...
    function getShipLevel(uint256 tokenId) public view returns(uint256) {
        return allShips[tokenId].lvl;
    }

    function pickShipsWhitelist(uint numberOfShips, bytes32[] calldata proof) external payable {
        require(openForPresale == true, "Whitelist sale is not open");
        require(_isWhitelist(msg.sender, proof), "Not whitelisted");
        require(quantityMintedByWLs + numberOfShips <= MAX_SUPPLY_PRESALE, "Presale supply is full");
        require(boughtByWL[msg.sender] + numberOfShips <= presaleRestrictedNumber, "Too many ships");

        boughtByWL[msg.sender] += numberOfShips;
        uint price = mintFeeAmountWL * numberOfShips;
        require(msg.value >= price, "Not enough avax");

        for (uint i = 1; i <= numberOfShips; i++) {
    ...
    function pickShips(uint numberOfShips) external payable {
        require(openForPublic == true, "Sale is not open");
        require(numberOfShips <= 10, "Trying to buy too many ships");
        require(totalSupply() + numberOfShips <= MAX_SUPPLY, "Ships supply is full");

        uint price = mintFeeAmount * numberOfShips;
        require(msg.value >= price, "Not enough avax");
    ...
    function openBox(uint shipToGive, address receiver) external onlyRole(STARDUST_CONTRACTS_ROLE) {
        require(totalSupply() + shipToGive <= MAX_SUPPLY, "box is empty, sorry");

        for (uint i = 1; i <= shipToGive; i++) {
            _mint(receiver);
        }
    }
        ...
        returns (string memory)
    {
        require(_exists(tokenId), "URI query for nonexistent token");
                // return string(abi.encodePacked(string(abi.encodePacked(baseURI, allShips[tokenId].lvl.toString(tokenId))), StringsUpgradeable.toString(tokenId), ".json"));
        return string(abi.encodePacked(string(abi.encodePacked(baseURI, StringsUpgradeable.toString(allShips[tokenId].lvl))), StringsUpgradeable.toString(tokenId), ".json"));
    }
    ...
    function setStarDustContractAddress(address _contractAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        STARDUST_CONTRACT = _contractAddress;
        _grantRole(STARDUST_CONTRACTS_ROLE, STARDUST_CONTRACT);
    }
    ...
    {
        // if (address(STARDUST_CONTRACT) != address(0) && address(from) != address(0)) {
        //     Stardust stardust = Stardust(STARDUST_CONTRACT);
        //     stardust.unstakeShi pWithoutClaim(tokenId);
        // }
