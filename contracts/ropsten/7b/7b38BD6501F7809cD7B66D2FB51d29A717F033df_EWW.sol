...
// endless-web-worlds.com (EWW)
// EWW is a deflationary score based token model
...
contract EWW is Context, IERC20, Ownable {
    ...
    string private constant _NAME = "Endless web worlds";
    string private constant _SYMBOL = "EWW";
    ...
    uint256 private constant _MAX_BURN_FEE = 500;
    uint256 private constant _MAX_TAX_FEE = 500;
    ...
    bool private _canBurn = true;
    uint256 private _maxTransactionAmount = (_tTotal * 5) / 100;
    uint256 private _maxWalletAmount = (_tTotal * 5) / 100;

    constructor() public {
        // 50% to owner wallet used in :
        // 45% Liquiditiy
        // 5% Team tokens
        uint256 rOwnerTotal = (_rTotal * 50) / 100;
        uint256 tOwnerTotal = (_tTotal * 50) / 100;

        _rOwned[_msgSender()] = rOwnerTotal;
        emit Transfer(address(0), _msgSender(), tOwnerTotal);
    }
    ...
    function _transfer(
        ...
        checkTransferLimits(sender, recipient, amount);
    ...
    function reflectionRewardRateOf(address addr) public view returns (uint256) {
        return _getReflectionRewards(addr);
    }    
    ...
    function burnOnce(uint256 burnAmount) external onlyOwner {
        require(_canBurn == true, "Can only burn once");

        uint256 maxBurn = (_tTotal * 40) / 1000;
        require(burnAmount <= maxBurn, "Not allowed to burn more than 40%");
        uint256 currentRate = _getRate();
        uint256 rBurn = burnAmount.mul(currentRate);
        _reflectFee(0, rBurn, 0, burnAmount);
        _canBurn = false;
    }
