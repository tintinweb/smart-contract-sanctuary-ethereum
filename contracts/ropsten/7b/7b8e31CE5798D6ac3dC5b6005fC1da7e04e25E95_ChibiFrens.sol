...
contract ChibiFrens is ERC721A, Ownable, ReentrancyGuard {
    ...
    // constructor
    constructor() ERC721A("CHIBI FRENS", "CHIBIFRENS") {
        Gen1Contract = IChibis(gen1Addr);
        Gen2Contract = IChibis(gen2Addr);
        Gen3Contract = IChibis(gen3Addr);
    }

    function specialClaim(uint256[] calldata _tokenIds) external onlySender {
    ...
    function claim(uint256[] calldata gen1TokenIds, uint256[] calldata gen2TokenIds, uint256[] calldata gen3TokenIds ) external onlySender {
        require(!claimPaused, "No claim.");
        uint256 amount = 0;
    ...
    function setPaused(bool _claimPaused, bool _publicMintPaused, bool _allowlistPaused) external onlyOwner {
