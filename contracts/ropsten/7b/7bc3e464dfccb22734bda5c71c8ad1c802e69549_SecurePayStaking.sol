...
contract SecurePayStaking is Ownable, ReEntrancyGuard {
   ...
   function pendingRewards(uint256 _orderId ) external view returns (uint256) {
    ...
    function mapInvestment(address beneficiary, uint256 _amount , uint256 _lockupDuration, uint256 _startTime) external onlyOwner {
        require(address(token) != address(0), "Token Not Set Yet");
        require(_amount > 0, "Amount must be greater than Zero!");
        ...
        orders[++latestOrderId] = OrderInfo(
            ...
            pool.returnPer,
            _startTime,
            _startTime.add(_lockupDuration.mul(86400)),
            0,
            false
        );
        ...
        totalStake = totalStake.add(_amount);
        totalRewardPending = totalRewardPending.add(userReward);
        balanceOf[beneficiary] = balanceOf[beneficiary].add(_amount);
        orderIds[beneficiary].push(latestOrderId);
        emit MappedInvestment(beneficiary, _lockupDuration , _amount ,pool.returnPer);
    }
}