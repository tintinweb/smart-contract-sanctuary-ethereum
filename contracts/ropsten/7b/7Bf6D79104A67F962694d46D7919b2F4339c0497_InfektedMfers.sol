...
contract InfektedMfers is ERC721A, Ownable, Pausable {
    ...
    function imTokenCount(address owner) public view returns (uint256) {
        return balanceOf(owner);
    }

    function vmTokenCount(address owner) public view returns (uint256) {
        ERC721A token = ERC721A(vmAddress);
        uint256 tokenCount = token.balanceOf(owner);
        return tokenCount;
    }

    function mfTokenCount(address owner) public view returns (uint256) {
