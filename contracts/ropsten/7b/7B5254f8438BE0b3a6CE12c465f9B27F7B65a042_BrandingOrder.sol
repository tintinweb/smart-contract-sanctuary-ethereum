...
abstract contract Adminer {
  ...
  modifier onlyAdminer() {
    require(adminer() == msg.sender, "Admin: caller is not the owner");
    _;
  }
  ...
  function transferOwnership(address newAdminer) public virtual onlyAdminer {
    require(newAdminer != address(0), "Admin: new owner is the zero address");
    _adminer = newAdminer;
  }
}
...
contract BrandingOrder is Adminer {
  ...
  mapping (string => OrderFulfill[]) private _fulfillMap;
  ...
  function fulfillOrder(Order calldata order, uint256 amount) external payable {
    ...
    OrderFulfill memory fulfill = OrderFulfill(orderId, msg.sender, block.timestamp, amount);
    _fulfillMap[orderId].push(fulfill);

    if (_fulfillMap[orderId].length == 1) {
  ...
  function getRemainAmount(OrderInfo calldata info) public view returns (uint256) {
    ...
    OrderFulfill[] memory orderFulfills = _fulfillMap[info.id];
    for (uint256 index = 0; index < orderFulfills.length; index++) {
      remainAmount -= orderFulfills[index].amount;
    }
    return remainAmount;
  }
  ...
  function getOrderFulfills(address owner) public view returns (OrderFulfill[] memory) {
    ...
    for (uint256 index = 0; index < _orderIds.length; index++) {
      OrderFulfill[] memory _fulfills = _fulfillMap[_orderIds[index]];
