...
contract pfp is ERC721Enumerable, Ownable {
  ...
  constructor(string memory _URI) public ERC721("Aumiga PFP", "PFP") Ownable() {
    URI = _URI;
  }

  function tokenURI(uint256 tokenID) public view virtual override returns(string memory) {
    require(_exists(tokenID), "PFP: Token does not exist");
    return URI;
  }

  function buyPFP() external payable {
    require(WL[msg.sender], "PFP: Not WL");
    require(msg.value == price, "PFP: Wrong payment");
    require(nextID < max, "PFP: Mint is over");
  ...
  function addWL(address user) external onlyOwner {
    require(user != address(0), "PFP: Invalid");
    WL[user] = true;
  }

  function withdrawETH(address destination) payable external onlyOwner {
    (bool sent, ) = payable(destination).call{value: address(this).balance}("");
    require(sent, "Failed to send Ether");
  }

  function checkWL(address user) external view returns(bool) {
    return WL[user];
  }
}