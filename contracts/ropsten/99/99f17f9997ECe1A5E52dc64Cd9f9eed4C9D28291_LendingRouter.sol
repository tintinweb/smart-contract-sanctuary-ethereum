...
contract LendingRouter is Ownable, ReentrancyGuard, Pausable {
    // Constants
    uint constant MAX_PROTOCOL_FEE = 10000;
    uint constant MAX_SWAP_RATE = 10000;
    ...
    // TO_DISCUSS setter and constructor
    uint public startingBlockNumber;
    uint public chainId;
    uint public minAppId = 100;
    uint public maxAppId = 200; 
    uint public protocolPercentageFee; // A number between 0 to 10000
    uint public minimumSwapRate = 0; // A number between 0 to 10000, value of swapped WBTC to value of TeleBTC rate
    ...
    /// @notice                             Gives default params to initiate cc lending router
    /// @param values                       contains startingBlockNumber, protocolPercentageFee, chainId, minAppId and maxAppId
    /// startingBlockNumber                 Requests that are included in a block older than _startingBlockNumber cannot be executed
    /// protocolPercentageFee               Percentage amount of protocol fee (min: %0.01)
    /// chainId                             Id of the underlying chain
    /// minAppId, maxAppId                  Identify app id range
    /// @param addresses                    contains relay, lockers, teleBTC, factory, treasury, exchange and price pracle address
    /// relay                               The Relay address to validate data from source chain
    /// lockers                             Lockers' contract address
    /// _teleBTC                            TeleportDAO BTC ERC20 token address
    /// _factory                            Address of smart wallets' factory
    /// _treasury                           Address of treasury that collects protocol fees
    /// _exchange                           Address of TeleSwap's exchange contract
    /// _priceOracle                        Address of price oracle
    constructor ( 
        uint[5] memory values, 
        address[7] memory addresses
    ) Ownable() Pausable() {
        startingBlockNumber = values[0];
        protocolPercentageFee = values[1];
        require(
            MAX_PROTOCOL_FEE >= protocolPercentageFee, 
        ...
        exchange = addresses[5];
        priceOracle = addresses[6];
    }
    ...
    /// @notice                             Setter for WBTC
    /// @param _WBTC                       Address of WBTC ERC20 token
    ...
    /// @notice                            Setter for factory
    /// @param _factory                    Address of smart wallets' factory
    ...
    function setProtocolPercentageFee(uint _protocolPercentageFee) external onlyOwner {
        require(
            MAX_PROTOCOL_FEE >= _protocolPercentageFee,
            "CCLendingRouter: protocol fee is out of range"
        );
        protocolPercentageFee = _protocolPercentageFee;
    }


    /// @notice                             Setter for minimum swap rate
    /// @param _swapRate                    new swap rate
    function setMinimumSwapRate(uint _swapRate) external onlyOwner {
        require(
            MAX_SWAP_RATE >= _swapRate,
            "CCLendingRouter: swap rate is out of range"
        );
        minimumSwapRate = _swapRate;
    }

    /// @notice                             Register new lending apps
    /// @param id                           App id
    /// @param name                         App name
    function setAppName(uint id, string memory name) external onlyOwner{
        appName[id] = name;
        emit AppRegistration(id, name);
    }
    ...
    /// @notice                             Validate request's input data
    ...
    /// @notice                             Executes the cross chain lending request
    /// @param _version                     Version of the Bitcoin transaction
    /// @param _vin                         Transaction inputs
    /// @param _vout                        Transaction outputs
    /// @param _locktime                    Bitcoin transaction locktime
    ...
    /// @return                             True if the lending is successful
    function lending(
        // Bitcoin tx
        bytes4 _version,
        bytes memory _vin,
        ...
        bool isSupported;
        uint remainedAmount;
        if (request.mode == Mode.LendWithTeleBTC || request.mode == Mode.LendAndBorrowWithTeleBTC) {
            isSupported = wallet.isValidToken(teleBTC);
            if (!isSupported) {
                (status, ) = _sendTeleBTC(_lockerLockingScript, txId, request.recipientAddress);
                require(
                    status == true, 
                ...
                emit Error ("CCLendingRouter: app doesn't support teleBTC");
                return false;
            }
            ...
            if (!isSupported) {
                // TO_DISCUSS
                (status, ) = _sendWBTC(_lockerLockingScript, txId, request.recipientAddress, request.recipientAddress); 
                if (status == false) {
                    emit Error ("CCLendingRouter: can't send WBTC");
                    return false;
                }

                emit Error ("CCLendingRouter: app doesn't support WBTC");
                return false;
            }
            
            // send wbtc to clone address
            (status, remainedAmount) = _sendWBTC(_lockerLockingScript, txId, cloneAddress, request.recipientAddress); 
    ...
    /// @notice                             Executes the cross chain lending request and send teleBTC to its recipient address
    /// @dev                                Only owner or request's recipient address can call this function
    /// @dev                                Should be used when lending request failed for unpredicted reason
    /// @param _version                     Version of the Bitcoin transaction
    /// @param _vin                         Transaction inputs
    /// @param _vout                        Transaction outputs
    /// @param _locktime                    Bitcoin transaction locktime
    ...
    /// @param _index                       Index of tx in the block
    /// @param _lockerLockingScript         Locking script of locker that user has sent BTC to it
    /// @return                             True if sending teleBTC is successful
    ...
    /// @notice                             Borrow amount must be less than or equal to collateral value
    /// @dev                                Check borrow amount validation with price oracle
    /// @param wallet                       Address of smart wallet 
    /// @param lendToken                    Address of collateral token
    /// @param lendAmount                   The value of collateral token
    /// @param borrowToken                  Address of token that want to borrow
    /// @param borrowAmount                 The value of token that want to borrow
    /// @return                             True if borrowing asset is successful
    ...
    /// @param value                        The value of collateral token
    /// @return                             True if deposit asset as collateral is successful
    ...
    /// @return _lendingRequest             Parsed request 
    ...
    /// @notice                             Sends minted teleBTC to the specified wallet
    /// @param _lockerLockingScript         Locker's locking script
    /// @param _txId                        The transaction ID of the request
    /// @param wallet                       Recipient's wallet address
    /// @return _status                     True if sending teleBTC is successful
    /// @return _remainedAmount             Amount of teleBTC that wallet receives after reducing fees
    ...
    /// @notice                             Sends WBTC to the specified wallet
    /// @dev                                This function mints teleBTC, then swaps minted teleBTC with WBTC
    ///                                     and send them to the specified wallet
    /// @param _lockerLockingScript         Locker's locking script
    /// @param _txId                        The transaction ID of the request
    /// @param wallet                       Recipient's wallet address
    /// @param requestRecipient             Request recipient address
    /// @return _status                     True if sending wbtc is successful
    /// @return _remainedAmount             Amount of WBTC that wallet receives after reducing fees
    function _sendWBTC (bytes memory _lockerLockingScript, bytes32 _txId, address wallet, address requestRecipient) private returns (bool, uint) {
        
        // Gets remained amount after reducing fees
        (bool status, uint _remainedAmount) = _mintAndReduceFees(_lockerLockingScript, _txId);
        require(
            status == true, 
            "CCLendingRouter: mint and reduce fee for TeleBTC goes wrong"
        );
        ...
        if (status == false) {
            // TODO stage test
            status = ITeleBTC(teleBTC).transfer(requestRecipient, _remainedAmount);
            require(
                status == true, 
                "CCLendingRouter: can't transfer teleBTC"
            );
            return (false, 0);
        }
        ...
        require(
            status == true, 
            "CCLendingRouter: can't transfer WBTC"
        );

        return (true, _remainedWBTC);
    }

    /// @notice                             Swap source token with destination token
    /// @param sourceToken                  Source token address
    /// @param destinationToken             Destination token address
    /// @param value                        Source token's value
    /// @return                             True if sending wbtc is successful
    /// @return                             Amount of destination token that is swapped
    function _swap(address sourceToken, address destinationToken, uint256 value) private returns (bool, uint) {
        ...
        try exchangeInstance.getOutputAmount(value, sourceToken, destinationToken) returns (bool status, uint swapAmount) {
            if (status == false) 
                return (false, 0);
            if (
                swapAmount * (10 ** IERC20Metadata(sourceToken).decimals()) * 10000 / 
                (value * (10 ** IERC20Metadata(destinationToken).decimals())) < 
                minimumSwapRate
            ) {
                emit Error("CCLendingRouter: swap rate is below minimum swap rate");
                return (false, 0);
            }
            ...
            try exchangeInstance.swap(value, swapAmount, path, address(this), block.timestamp + INF_TIME, true) returns (bool swapStatus, uint[] memory ignored) {
                if (swapStatus == false) {
                    return (false, 0);
                }
                return (true, swapAmount);
            } catch (bytes memory _err) {
                emit Error("CCLendingRouter: can't swap");
                return (false, 0);
            }  catch Error(string memory _err) {
                emit Error(_err);
                return (false, 0);
            ...
            emit Error("CCLendingRouter: can't get swap amount");
            return (false, 0);
        }  catch Error(string memory _err) {
            emit Error(_err);
            return (false, 0);
    ...
    /// @param _lockerLockingScript   Locker's locking script
    /// @param _txId                  The transaction ID of the request
    /// @return status                True if minting and reducing fees are successful
    ...
    function _mintAndReduceFees(
        ...
        // TODO
        // uint mintedAmount;
        // try ILockers(lockers).mint(
        //     _lockerLockingScript,
        //     address(this),
        //     CCLendingRequests[_txId].inputAmount
        // ) returns (uint ignored) {
        //     mintedAmount = ignored;
