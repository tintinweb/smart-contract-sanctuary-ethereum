...
contract Reserve is IReserve, Ownable {
    ...
    /**
     * @dev Initializess the contract setting up uniswap router address, ELEN address and USDT address.
     */
    constructor(address uniswapRouter, address TOKEN, address USDT, address timelock_) public {
    ...
    /**
     * @dev Buys tokens in the amount of usdt Amount. Then burns them.
     *
     * Emits `BuyAndBurn` event.
     */
    function buyAndBurn(uint256 usdtAmount) external override onlyOwner {
        address[] memory path = new address[](2);
        path[0] = address(usdt);
        path[1] = address(token);
        uint256 tokenBalanceBefore = token.balanceOf(address(this));
        SafeERC20.safeApprove(address(usdt) ,address(uniswapV2Router), usdtAmount);
        ...
        emit BuyAndBurn(tokensSwapped, usdtAmount);
    }

    /**
     * @dev Exchanges tokens to usdt. After that, usdt remains on the contract.
     *
     * Emits `SwapAndCollect` event.
     */
    function swapAndCollect(uint256 tokenAmount) external override onlyToken {
        uint256 usdtReceived = _swapTokensForUSDT(tokenAmount);
        emit SwapAndCollect(tokenAmount, usdtReceived);
    }
    ...
    function swapAndLiquify(uint256 tokenAmount) external override onlyToken {
        ...
        uint256 usdtReceived = _swapTokensForUSDT(otherHalf);
        SafeERC20.safeTransfer(address(token), address(uniswapV2Pair), half);
        SafeERC20.safeTransfer(address(usdt), address(uniswapV2Pair), usdtReceived);
        uniswapV2Pair.mint(timelock);
        emit SwapAndLiquify(otherHalf, usdtReceived, half);
    }

    /**
     * @dev Swaps ELEN tokens for USDT.
     * @param tokenAmount Amount of ELEN tokens to swap.
     * @return uint256 Amount of swapped USDT.
     */
    function _swapTokensForUSDT(uint256 tokenAmount) private returns (uint256) {
        address[] memory path = new address[](2);
        ...
        uint256 usdtSwapped = usdt.balanceOf(address(this)).sub(usdtBalanceBefore);
        return usdtSwapped;
    }
}
