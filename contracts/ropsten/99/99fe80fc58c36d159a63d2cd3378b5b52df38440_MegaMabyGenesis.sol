...
contract MegaMabyGenesis is ERC721A, Ownable, ReentrancyGuard {
    uint256 public MAX_SUPPLY = 1020;
    uint256 public MINT_PRICE = 1 * 10 ** 18; // 1 ETHW
    bool public pausedMint = true;
    string public baseURI = "";

    constructor() ERC721A("MegaMaby Genesis", "MEGY") {}

    function mint(uint _amount) external payable nonReentrant {
        require(_amount > 0, "Amount must not be zero");
        require(pausedMint == false, "Mint is paused");
        require(totalSupply() + _amount <= MAX_SUPPLY, "Max supply exceeded");
        require(msg.value >= MINT_PRICE * _amount, "Invalid ETHW value");
    ...
    function setPausedMint() external onlyOwner {
