...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH =
        0xef13967934e2c303aad9c1e85b087be6d40d82304932f28c3fea3887a72ab22c;
...
contract AipPool is IAipPool, ReentrancyGuard {
    ...
    function price() public view virtual returns (uint256) {
        return
            IAipSwapManager(swapManager).bestLiquidityPoolPrice(token0, token1);
    }
        ...
        )
    {
        ...
        uint256 period = frequencyD * 60; // 24 * 3600;
    ...
    function trigger() external nonReentrant returns (uint256 swapAmount0) {
        ...
        if (tickIndex > 1) {
            require(
                tickTmps[tickIndex - 1] + frequencyD * 60 <= block.timestamp,
                "Not yet"
            ); // 24 * 3600
        }
        tickTmps[tickIndex] = block.timestamp;
        uint256 gasFee = tx.gasprice * PROCESSING_GAS;
        uint256 _price = IAipSwapManager(swapManager).bestLiquidityPoolPrice(
            token0,
            WETH9
        );
...
interface IAipSwapManager {
    function bestLiquidityPoolPrice(address token0, address token1)
        external
        view
...
contract AipUniswapManager is IAipSwapManager, AipPayments {
    ...
    function _bestLiquidityPool(address token0, address token1)
        private
        view
    ...
    function bestLiquidityPoolPrice(address token0, address token1)
        ...
        returns (uint256 price)
    {
        (, price) = _bestLiquidityPool(token0, token1);
    }
    ...
    function swap(
        ...
        (address pool, ) = _bestLiquidityPool(token0, token1);
