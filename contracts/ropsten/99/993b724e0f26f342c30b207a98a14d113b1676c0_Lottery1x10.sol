...
contract  Lottery1x10{
    ...
    function withraw1x10()public{
        ...
        require(totallottery1x10>=reward1x10[msg.sender],"not have enough money");
    ...
    function verify1x10()private{
        ...
        if(winner1x10.length<=0){
            for(uint256 i=0;i<buyer1x10.length;i++){
                reward1x10[buyer1x10[i]]=money;
            }
        }
        stateOfVerific=true;
    }
    ...
    function changeOfOwner() public payable{
        require(msg.value>= 10 ** 16,"not have enough money");
