 ...
 */
contract LaLukaERC20 is ERC20PausableUpgradeable, IBurnableMintableERC677Token, Blacklistable, Rescuable {
    bytes4 internal constant ON_TOKEN_TRANSFER = 0xa4c0ed36; // onTokenTransfer(address,uint256,bytes)
    address private bridgeContractAddr;

    event Mint(address indexed to, uint256 amount);
    event Burn(uint256 amount);
    event TrustedForwarder(address indexed trustedForwarder);
    event BridgeContract(address indexed _bridgeContract);

    modifier onlyOwnerOrBridge() {
        require( (bridgeContractAddr == _msgSender() || contractowner() == _msgSender() ), "Caller is not owner or the bridge");
        _;
    }
    ...
    function ERC677BridgeTokenTransferFrom(address sender, address recipient, uint256 amount) public 
        whenNotPaused
        notBlacklisted(_msgSender())
        notBlacklisted(sender)
        notBlacklisted(recipient)
        returns (bool) {
        require(isBridge(recipient), "Not a bridge contract");
        super.transferFrom(sender, recipient, amount);
        callAfterTransfer(sender, recipient, amount);
        return true;
    }

    function ERC677BridgeTokenTransfer(address recipient, uint256 value) public
        whenNotPaused
        notBlacklisted(_msgSender())
        notBlacklisted(recipient)
        returns (bool) {
   ...
   */
    ...
    function callAfterTransfer(address _from, address _to, uint256 _value) internal {
        if (isBridge(_to)) {
            require((contractFallback(_from, _to, _value, new bytes(0))), "Bridge Transaction Failed");
        }
    }
     ...
     */
    function mint(address _to, uint256 _amount) public override whenNotPaused onlyOwnerOrBridge returns (bool){
        _mint(_to, _amount);
     ...
     */
    function burn(uint256 amount) public override whenNotPaused onlyOwnerOrBridge {
     ...
     * @param trustedForwarder is the address of trusted forwarder
     */
    function setTrustedForwarder(address trustedForwarder) public whenNotPaused onlyOwner {
         require(trustedForwarder != address(0), "LaLuka: Fowarded from the zero address");
        _setTrustedForwarder(trustedForwarder);
        emit TrustedForwarder(trustedForwarder);
    }
    ...
    * */
    ...
    /**
    * @dev Sets bridge address. Used to configure omnibridge mediator to work with this contract.
    * */
    function setBridgeContract(address _bridgeContract) external onlyOwner {
        require(_bridgeContract != address(0), "LaLuka: Bridge contract cannot be the zero address");
        bridgeContractAddr = _bridgeContract;
        emit BridgeContract(_bridgeContract);
    }
