...
interface Iqueue {
    ...
    function processQueue(uint256 gasLimit) external;
    function process(uint256 gasLimit) external returns ( bool) ;
}
...
contract koalas is Initializable,
                    UUPSUpgradeable,
                    ERC20Upgradeable,
                    OwnableUpgradeable,
                    PausableUpgradeable,
  ...
  function initialize() public initializer {
        __Ownable_init();
        __ERC20_init("Koalas Club", "KLUB");
        __Pausable_init_unchained();
        __ReentrancyGuard_init();


    FINAL_SUPPLY = 5_000_000_000 ether;
    MAX_WALLET = 750_000_000 ether; // MAX PER WALLET: 1.5%
    MAX_SELL = 250_000_000 ether; // MAX PER SELL: 0.5%

    gasLimit = 300_000; // GAS FOR REWARDS PROCESSING
    swapThreshold = 5_000_000 ether; // CONTRACT SWAPS TO BSD
    ...
    address ROUTER = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; //0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    teamWallet = payable(0xeC9E1C0Aa18899eEc6df8542F9145e9b72807cf5);
    rewards = payable(0xD206F608068623f720587c327f7B594eA812cbCD);
  ...
  function setRewardsContract(address newContract) external onlyOwner {
    ...
    require(OwnableUpgradeable(newContract).owner() == address(this), "Token must own tracker");

    if (isContract(rewards)) OwnableUpgradeable(rewards).transferOwnership(owner());
  ...
  function _transfer(address from, address to, uint256 amount) internal override {
    ...
    if (!autoMarketMakers[from]) {
        ...
        if (_tokens >= swapThreshold) {
          swapSendTo(taxDIV, rewards);

          swapLiquify(taxLIQ);

          swapSendTo(taxTEAM, teamWallet);
          _setBalances(from, BBBWallet, taxBBB);
          _setBalances(from, StakeContract, taxSTK);
  ...
  function computeTax(address from, address to, uint256 amount) internal returns (uint256) {
    ...
    if (feeExcempt[from] || feeExcempt[to]) return 0;
  ...
  function setStakingContract(address newContract) external onlyOwner {
      ...
      if (isContract(StakeContract)) OwnableUpgradeable(StakeContract).transferOwnership(owner());
  ...
  function vestingPercent(address account) internal view returns (uint32) {
    if (!isStakingOn) return 100;
    uint32 age = ageInDays(openingDate);
    return (age > 40) ? 100 : 20 + (age*2);
  }
