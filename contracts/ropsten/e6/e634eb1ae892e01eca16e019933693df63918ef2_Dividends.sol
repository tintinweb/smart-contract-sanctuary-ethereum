...
contract Dividends  is  Ownable  {
    ...
    address private tokenAaddress = 0x32eecF2Fc053F52A24Bbc29B772CEdc712334953;
    address private tokenBaddress = 0xD3063C5e5C57bb7dc622d39315E18a251558c5D9;

    uint256 public TotalWithdrawal;  
    uint256 public ExtractNftNumber;   
    address[] private  pledgeAddress; 

    uint256 private  profit; 

    uint256 private  marketValue; 
    uint256 public  minimumWithdrawalAmount = 99999999999999;  //小数点精确最低提款额度 Wei

    uint256 public minimumPledgeDays = 7;
    ...
    function setUpminimumPledgeDays(uint256 _number) public onlyOwner {
        minimumPledgeDays = _number;
    }
    ...
    function withdrawChoose(uint256[] memory _tokenId) public {
        ...
        for(uint i = 0;i < tokenIdLength;i++){
            require(getAddresspledgeNumberbool(_tokenId[i]), "No collateral found");
          //  require(block.timestamp > setEndPledgeTime(_tokenId[i]), "time is not up yet");
        }
        ...
        uint256 number = getAddresspledgeNumberlength(msg.sender);
        ...
        for(uint i = 0; i < tokenIdLength; i++){
            (bool respond, uint256 _num)  = getAddresspledgeNumberkey(_tokenId[i]);
            if(respond == true)deleteAddresspledgeNumber(_num);
            deletePledgeTime(_tokenId[i]);
        }
    }

    function OneRelinquishedShare(uint256 _Number) internal view returns(uint256 ,uint256){
        uint256 pleNumber = getAddresspledgeNumberlength(msg.sender);

         (,uint256 ProportionOfOneShareNum) = getIncreaseWei(NumberOfSharesHeld[msg.sender]).tryDiv(getNFTWei(pleNumber));  // 总持股 / 质押数
         uint256 SharesToBeReduced = ProportionOfOneShareNum.mul(getNFTWei(_Number));   // 应减占股
         uint256 ReductionPool = getReduceWei(SharesToBeReduced.mul(getSingleSharePrice()));  // 应减池子量
        return (getReduceWei(SharesToBeReduced),getReduceWei(ReductionPool));
    }
    function stake(uint256[] memory _tokenId) public {
        require(_isStakeSwitch, "Can not be pledged temporarily");
        ...
        for(uint i= 0;i < tokenIdLength;i++){
            ...
            pledgeNumber[msg.sender].push(_tokenId[i]);
            PledgeTime[msg.sender][_tokenId[i]] = block.timestamp;
        }
    ...
    function getTotalPool() public view returns(uint256){
        ...
        return getNFTWei(nftPledgeNum + ExtractNftNumber) + TokenBnumber + TotalWithdrawal - profit;
    }
    ...
    function deletePledgeTime(uint256 _num) internal {
    ...
    function setEndPledgeTime(uint256 _num) public view returns(uint256){
        return PledgeTime[msg.sender][_num] + (minimumPledgeDays * 24 * 3600);
    }
    ...
    function setAllpledgeTime(address _addr,bool _type) public view returns(uint256[] memory){
      uint  NftLength  =   getAddresspledgeNumberlength(_addr);
      uint256[] memory allTime = new uint[](NftLength);
      uint counter = 0;
      for(uint i = 0; i < NftLength; i++){
            if(_type){
                allTime[counter] =  PledgeTime[msg.sender][pledgeNumber[msg.sender][i]] + (minimumPledgeDays * 24 * 3600);
            }else{
                allTime[counter] =  PledgeTime[msg.sender][pledgeNumber[msg.sender][i]];
            }
           counter++;
      }
      return allTime;
