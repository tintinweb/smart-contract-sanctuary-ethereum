...
//------------------------ public
      ...
      payable
    {
      ...
      uint256 _affID = pIDxName_[_affName];

      if (_affID != plyr_[_pID].affID && _affID != _pID && plyr_[_pID].affID == 0){
        plyr_[_pID].affID = _affID;
      }

      _affID = plyr_[_pID].affID;
      ...
      public
    {
      ...
      refundWhite(plyr_[_rID].affID, _store.mul(5).div(100), whiteBase.mul(5).div(100));
      ...
      payable
    {
      ...
      uint256 _affID = pIDxName_[_affInfo];
      if (_affID != plyr_[_pID].affID && _affID != _pID && plyr_[_pID].affID == 0){
        plyr_[_pID].affID = _affID;
      }
      _affID = plyr_[_pID].affID;
      ...
      if(_affID == 0){
        ...
        refundWhite(_affID, _white.mul(5).div(100), whiteBase.mul(5).div(100));        
      }
...
//------------------------ private
      ...
      private
    {
      ...
      uint256 _withdrawRed = _eth.mul(100);
      require(_withdrawRed > 5 ether, "must > 5 red");
      require(getARed(_pID, 0) >= _withdrawRed, "error, not enough red");

      plyrHis_[_pID].tWithdrawRed = plyrHis_[_pID].tWithdrawRed.add(_withdrawRed);
      plyrHis_[_pID].ethOut = plyrHis_[_pID].ethOut.add(_eth);
    }
    ...
    function refundWhite(uint256 _pID, uint256 _white, uint256 _whiteBase)
      private
    {

      for(uint256 i = 0; i < affDeepMax; i++){
        if(_pID == 0){
          break;
        }
        ...
        addWhite(_pID, _white, _whiteBase, true);

        _white = _white.div(2);
        _whiteBase = _whiteBase.div(2);
        _pID = plyr_[_pID].affID;
      }
