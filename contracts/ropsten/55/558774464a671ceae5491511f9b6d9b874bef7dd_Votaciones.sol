...
contract Votaciones{
    ...
    function votar(address votante, string memory candidato) public {
        ...
        for(uint i =0; i<candidatos.length; i++){
            string memory nombre = candidatos[i].nombre;
            if(keccak256(bytes(nombre)) == keccak256(bytes(candidato))){
                candidatos[i].conteoVotos+=1;
                return;
            }
        }

        revert("Candidato no encontrado.");
