...
/* memory : 執行時變數分配在記憶體, 非永久儲存 *//* nonce 防止 replay attacks */
...
/* 驗證的前綴 */
  ...
  /* 將prefix與 _message使用abi.encodepacked(．,．) 重新編碼，並使用    keccak256(．)做雜湊運算 */          
    _message =keccak256(abi.encodePacked(prefix,_message));
    /* 呼叫recoverSigner(．, ．)函數，返回執行簽章的地址  =  owner, 使用            require(．)*/
