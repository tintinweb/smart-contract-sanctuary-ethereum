...
contract OMNINFT is ERC721A, ERC721ABurnable, AccessControl {
     ...
     * saled count
     */
    uint32 public saledCount;

    /**
     * max sale count
     */
    uint32 public saleLimit;
    ...
    constructor(
        ...
        string memory baseURI_, // NFT baseURI
        uint32 maxSupply_, // NFT maxSupply,once set can't be changed
        address paymentReceiver_, // EOA or ContractWallet,recieves payment from mints
        address controller_, // EOA or ContractWallet,contract controller（admin role）
        address saleRole_, // EOA or ContractWallet,can set sale price,sale time
        address mintRole_, // EOA only, can sign for special mint
        address synthRole_ // EOA only, can sign for NFT synthesis
        ...
        _setRole(MINT_ROLE, mintRole_);
        _setRole(SYNTH_ROLE, synthRole_);
    }

    /**
     * @dev Set the base URI for all token IDs, only admin can call
     * @param baseURI_ New base URI (RFC 3986)
     */
    ...
    /**
     * @dev modifier to check if sale if active
     * @param amount_ amount of token to mint
     */
    modifier forSale(uint32 amount_) {
        uint256 tsNow = block.timestamp;
        require(
            saleStartTime > 0 && saleStartTime < tsNow && saleEndTime > tsNow,
            "Sale not enabled"
        );
        uint32 newSaledCount = saledCount + amount_;
        require(newSaledCount <= saleLimit, "Sale limit reached");
     ...
     * @param saleStartTime_ sale start time
     * @param saleEndTime_ sale end time
     * @param salePrice_ sale price
     * @param saleLimit_ sale limit
     */
    function updateSale(
        ...
        require(saleEndTime_ >= saleStartTime_);
     ...
     * @param amount_ Number of NFTs to mint
     */
    function batchMint(uint32 amount_) external payable forSale(amount_) {
     ...
     * @param amount_ Number of NFTs to mint
     * @param signature_ signed message(`msg.sender`+`msg.value`+`amount_`) by mint role
     */
     ...
     * @param ids_ NFT from
     * @param signature_ signed message(`ids_`+`address(this)`) by synth role
     */
