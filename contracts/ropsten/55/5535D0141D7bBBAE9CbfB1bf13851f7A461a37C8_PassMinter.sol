...
contract PassMinter is Ownable, Pausable {
  ...
  constructor(address _passAddress, address _currencyAddress) {
    pass = IPass(_passAddress);
    currency = ICurrency(_currencyAddress);

    // Really they are _amount * 1e18 Currency tokens
    prices[uint256(PassType.Match10)] = 10 ether;
    prices[uint256(PassType.Match20)] = 20 ether;
    prices[uint256(PassType.Match50)] = 50 ether;
    prices[uint256(PassType.Match100)] = 100 ether;
    _pause();
  }
  ...
  function purchase(
    ...
    if (_id > uint256(PassType.Match100)) revert InvalidID();
    if (_to == address(0)) revert InvalidAddress();

    currency.transferFrom(msg.sender, address(this), prices[_id] * _amount);

    pass.mint(_to, _id, _amount);
  }
  ...
  function setPass(address _passAddress) external onlyOwner {
    pass = IPass(_passAddress);
  }
