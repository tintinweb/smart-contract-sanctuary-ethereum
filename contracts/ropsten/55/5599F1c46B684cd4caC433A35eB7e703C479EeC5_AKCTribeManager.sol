...
contract AKCTribeManager is Ownable, ReentrancyGuard {
    ...
    uint256 distributorAllocation;
    bool enforceTerms = true;
    uint256 minimumEnforceBalance;
    ...
    function purchaseTribe(uint256 spec, string calldata identifier)
        ...
        nonReentrant { 
            ...
            IAKCCore.TribeSpec memory tribeSpec = akcCore.tribeSpecs(spec);
            uint256 price = tribeSpec.price;
            require(akcCoin.allowance(msg.sender, address(this)) >= price);
...
contract IAKCCore {
    ...
    function tribeSpecs(uint256 index) external returns(TribeSpec memory) {}
