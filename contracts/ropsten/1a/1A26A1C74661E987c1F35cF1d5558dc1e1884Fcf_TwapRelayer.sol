    ...
    function getPricesVolume(
        ...
        for (uint256 i; i < amountsLength; ++i) {
            if (isSellSide) {
                ...
                uint8 tokenOutDecimals = IERC20(tokenOut).decimals();
                uint256 amount = amounts[i];
                if (tokenOutDecimals > 18) {
                    amount = amounts[i].mul(10**(tokenOutDecimals - 18));
                } else if (tokenOutDecimals < 18) {
                    amount = amounts[i].div(10**(18 - tokenOutDecimals));
                }
                require(amount >= tokenLimitMin[tokenOut], 'TR03');
                require(
                    amount <= IERC20(tokenOut).balanceOf(address(this)).mul(tokenLimitMax[tokenOut]).div(PRECISION),
                    'TR3A'
                );
