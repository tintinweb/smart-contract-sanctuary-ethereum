...
contract DETF is ERC20, Owned, SafeMath {
    ...
    function () public payable {
        ...
        uint totalExpectedCount = safeMul(msg.value, totalExpectedPrice);
        uint kncCount = kyber.trade(ieth, address(this).balance, knc, address(this), totalExpectedCount, kncSlippagePrice, 0);
        uint omgCount = kyber.trade(ieth, address(this).balance, omg, address(this), totalExpectedCount, omgSlippagePrice, 0);
        uint totalCount = 0;
    ...
    function getExpectedRate(uint value) public view returns (uint, uint, uint, uint) {
        ...
        uint totalExpectedPrice = safeDiv(safeMul(kncExpectedPrice, omgExpectedPrice), safeAdd(kncExpectedPrice, omgExpectedPrice));
        uint totalExpectedCount = safeMul(value, totalExpectedPrice);
