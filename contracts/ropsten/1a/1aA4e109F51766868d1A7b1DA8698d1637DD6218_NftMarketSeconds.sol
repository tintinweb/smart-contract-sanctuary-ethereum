...
contract NftMarketSeconds is ERC721URIStorage, Ownable {
        ...
        returns (uint256)
    {
        ...
        require(
            msg.value == AllTokens[tokenType].listingPrice * amount,
            "Price must be equal to listing price"
        );

        uint256 startTokenId =  AllTokens[tokenType].startTokenId;

        require(
            AllTokens[tokenType].startTokenId + amount <= AllTokens[tokenType].endTokenId,
            "Nft minted is max"
            );

        for (uint256 i = 0; i < amount; ++i) {
            _safeMint(msg.sender,startTokenId);
            _createNftItem(startTokenId);
        }
        AllTokens[tokenType].startTokenId += amount;
