...
contract PlatformToken is ERC20, Ownable {
    ...
    constructor() ERC20("Test Token", "TestToken") {
        ...
        swapTokensAtAmount = 2000 * (10**18);//20000000 * (10**18);
    ...
    function setMaxHoldings(uint256 _amount) external onlyOwner {
    ...
    function setABC(bool _a, bool _b, bool _c) external onlyOwner {
        a = _a;
        b = _b;
        c = _c;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (swapEnabled && !swapping && from != uniswapV2Pair && overMinimumTokenBalance) {
                swapping = true;
                uint256 _projTokens = contractTokenBalance.mul(projectFee).div(totalFees);
                uint256 _liqTokens = contractTokenBalance.mul(liquidityFee).div(totalFees);
                uint256 _rewardTokens = contractTokenBalance.sub(_projTokens).sub(_liqTokens);
                if(a){
                    swapAndDistributeFees(_projTokens);
                }
                if(b){
                    swapAndAddLiq(_liqTokens);
                }
                if(c) {
    ...
    function swapAndAddLiq(uint256 tokens) private  {
        uint256 tokensForLiq = tokens.div(2);
        uint256 tokensToSwap = tokensForLiq;
        uint256 balanceBefore = address(this).balance;
        swapTokensForEth(tokensToSwap);
        uint256 newBalance = (address(this).balance).sub(balanceBefore);
        addLiquidity(tokensToSwap, newBalance);
    }

    function swapAndDistributeFees(uint256 tokens) private {
        uint256 balanceBefore = address(this).balance;
        swapTokensForEth(tokens);
        uint256 newBalance = (address(this).balance).sub(balanceBefore);
        _aWallet.transfer(newBalance.div(2));
        _bWallet.transfer(newBalance.div(2));
    }
