...
contract Shop is Ownable {
    event BuyItem(address indexed nft, address indexed buyer, uint num, uint amount);
    ...
    uint public constant MINT_FEE = 50000000000000000;
    ...
    function buy(Cat _type, uint num) external payable {
        ...
        require(_type >= Cat.SnoopDogg && _type <= Cat.ChanKwongYan, "Wrong cat type");
        ...
        if (_type == Cat.ChanKwongYan) {
            uint amount = MINT_FEE * num;
            require(msg.value >= amount, "Insufficient funds");
            bytes memory data = abi.encodeWithSignature("mint(address,uint256)", msg.sender, num);
            (bool success, ) = cat.call{value:msg.value}(data);
            require(success, "ERC721: mint failed");
            catCounts[_type] += num;
            emit BuyItem(cat, msg.sender, num, amount);
        } else {
            uint amount = catPrice[_type] * num;
