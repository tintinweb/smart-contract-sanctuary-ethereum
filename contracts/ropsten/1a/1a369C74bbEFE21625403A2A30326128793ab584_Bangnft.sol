...
contract Bangnft is ERC1155, Ownable, ERC1155Burnable,BlackLkisted {
    bool public hasSaleStarted = true;
    using SafeMath for uint256;
     string public name;
    string public symbol;
    uint256 public creationFess = 5;
    IERC20 token;
    IERC20 [] currencies;
    ...
    constructor() ERC1155("http://nft.borabit.com/metadata/") {
        name = "BANG"; 
        symbol = "BNG"; 
        }

    function checkExitscurrency(IERC20 currency) private view returns (bool){
       bool found=false;
    ...
    function addCurrenicies(IERC20 currency) public onlyOwner{
        require(checkExitscurrency(currency)==false,"Currency Alreday Added");
        currencies.push(currency);
    }
    function setCreationFees(uint256 _fees) public onlyOwner{
        creationFess=_fees;
    }
    
       // Mint function
    function mint(uint256 amountToMint,uint256 _tokenid,bool isToken,uint256 amount,uint256 price,IERC20 paymentCurrency) public payable {
        ...
        if(creationFess > 0) {
                if(isToken) {
                    ...
                    token = IERC20(paymentCurrency);
                    require(token.allowance(msg.sender, address(this)) > amount, "Please allow fund first");
        ...
        _mint(msg.sender, _tokenid, amountToMint, "");
    }
