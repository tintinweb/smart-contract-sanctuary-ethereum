...
contract Loopers is ERC721Enumerable, Ownable {
    ...
    uint public maxMintQuantity = 10;
    bool public isSalesActive = true;
    
    mapping(address => uint) public addressToFreeMinted;

    constructor() ERC721("Loopers", "LOOPERS") {
        // baseUri for the NFT metadata JSON
        _baseUri = "https://ipfs.io/ipfs/QmWcu65qR1PWWjKTCCuoHUCbXvxrmjgyMA4mv1rYV8tWwg/";
    ...
    function mint(uint quantity) external payable {
        require(isSalesActive, "sale is not active");
        require(quantity <= maxMintQuantity, "max mints per transaction exceeded");
        uint totalMinted = totalSupply();
        require(totalMinted < MAX_SUPPLY, "sold out");

        // TODO this msg.value is casuing issue to the transaction process for some reason
