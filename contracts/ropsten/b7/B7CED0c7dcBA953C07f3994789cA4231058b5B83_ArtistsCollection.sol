    ...
    UUPSUpgradeable
{
    ...
    /// @notice the name of the artist
    mapping(uint256 => string) public tokenArtistName;

    /// @notice address of the verification contract
    address public immutable verifierAddress;

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(address verifierAddress_) initializer {
        verifierAddress = verifierAddress_;
    }

    function initialize() public initializer {
        __ERC721_init("Myx Artists", "aMYX");
    ...
    function safeMint(
        ...
        tokenArtistName[tokenId] = artistName_;

        if (isVerified_) {
            // mint the nft to the artist
            _safeMint(to_, tokenId);
        } else {
            // mint the nft to the DAO
            _safeMint(verifierAddress, tokenId);
        }
