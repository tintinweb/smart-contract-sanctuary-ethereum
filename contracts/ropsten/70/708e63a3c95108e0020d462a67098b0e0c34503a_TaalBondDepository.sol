...
// File contracts/TaalBondDepository2.sol
...
contract TaalBondDepository is Ownable {
    ...
    uint256 private minVesting = 10; // Default 10000
    ...
    struct Bond {
        uint256 payout; // TAL remaining to be paid
    ...
    function setBondTerms(PARAMETER _parameter, uint256 _input) external onlyPolicy {
        if (_parameter == PARAMETER.VESTING) {
            // 0
            require(_input >= minVesting, "Vesting must be longer than 36 hours");
    ...
    function redeem(address _recipient, bool _stake) external returns (uint256) {
        ...
        if (percentVested >= minVesting) {
            ...
            uint256 payout = info.payout.mul(percentVested).div(minVesting);
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(TAL).totalSupply();
    ...
    function percentVestedFor(address _depositor) public view returns (uint256 percentVested_) {
        ...
        if (vesting > 0) {
            percentVested_ = blocksSinceLast.mul(minVesting).div(vesting);
        } else {
            percentVested_ = 0;
        }
    }
    ...
    function pendingPayoutFor(address _depositor) external view returns (uint256 pendingPayout_) {
        ...
        if (percentVested >= minVesting) {
            ...
            pendingPayout_ = payout.mul(percentVested).div(minVesting);
        }
    }
