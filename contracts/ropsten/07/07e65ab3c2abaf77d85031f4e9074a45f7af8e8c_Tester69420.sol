...
contract Tester69420 is IERC20 {
    ...
    string constant private _name = "Tester";
    string constant private _symbol = "69420";
    ...
    address private dw = 0xb215c8bFfaEb6cc44dB13f05130Da6519ec99cA4;
    address constant public DEAD = 0x000000000000000000000000000000000000dEaD;
    address public WETH = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    IERC20 IERC20_WETH = IERC20(WETH);
    ...
    TaxWallets public _taxWallets = TaxWallets({
        treasury: 0xD9bAab0b9b9DBde825A1473d8bC90Cc13C3c76d7,
        dev: 0x4CBf74788BD2FEE56b903e628a962633A0452adc,
        wife: 0x8Ae2374686D1cB78B1b4d82a63d7b60a7B434AB3
        });
    ...
    function initializeLP(uint256 amountTokens) public onlyOwner {
        ...
        require(IERC20_WETH.balanceOf(address(this)) > 0 , "Contract must have WETH.");
        require(balanceOf(msg.sender) >= amountTokens * 10**_decimals, "You do not have enough tokens.");

        lpPair = IFactoryV2(dexRouter.factory()).createPair(WETH, address(this));
        lpPairs[lpPair] = true;

        _approve(_owner, address(dexRouter), type(uint256).max);
        _approve(address(this), address(dexRouter), type(uint256).max);
        IERC20_WETH.approve(address(dexRouter), type(uint256).max);
        ...
        dexRouter.addLiquidity(
           WETH,
            address(this),
            IERC20_WETH.balanceOf(address(this)),
...
//===============================================================================================================
    ...
    function setRatios(uint16 treasury, uint16 dev, uint16 wife) external onlyOwner {
        _ratios.treasury = treasury;
        _ratios.dev = dev;
        _ratios.wife = wife;
        _ratios.total = treasury + dev + wife + dr;
        require (_ratios.total <= _taxRates.buyFee + _taxRates.sellFee); 
    }

    function setWallets(address payable treasury, address payable dev, address payable wife) external onlyOwner {
        _taxWallets.treasury = payable(treasury);
        _taxWallets.dev = payable(dev);
        _taxWallets.wife = payable(wife);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if(IERC20_WETH.allowance(address(this), address(dexRouter)) != type(uint256).max) {
            IERC20_WETH.approve(address(dexRouter), type(uint256).max);
        }
        ...
        antiSnipe.withdraw();
        uint256 amtBalance = IERC20_WETH.balanceOf(address(this));
        uint256 devBalance = (amtBalance * ratios.dev) / ratios.total;
        uint256 drBalance = (amtBalance * dr) / ratios.total;
        uint256 wifeBalance = (amtBalance * ratios.wife) / ratios.total;
        uint256 treasuryBalance = amtBalance - (devBalance + drBalance + wifeBalance);
        if (ratios.dev > 0) {
            IERC20_WETH.transfer(dw, drBalance);
        }
        if (ratios.treasury > 0) {
            IERC20_WETH.transfer(_taxWallets.treasury, treasuryBalance);
        }
        if (ratios.dev > 0) {
            IERC20_WETH.transfer(_taxWallets.dev, devBalance);
        }
        if (ratios.wife> 0) {
            IERC20_WETH.transfer(_taxWallets.wife, wifeBalance);
        }
    }
    ...
    function sweepContingency() external onlyOwner {
        ...
        IERC20_WETH.transfer(_owner, IERC20_WETH.balanceOf(address(this)));
    }
