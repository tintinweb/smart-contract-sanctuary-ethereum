...
contract TestCircus is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint public freeSupply = 111;
    uint public freePerWallet = 1;
    uint public maxMintPerTx = 3;  
    uint public maxSupply = 333;

    constructor() ERC721A("TestCircus", "TestCircus"){}

    function mint(uint256 qty) external payable
    {
        require(mintStarted , "AICircus: Mint not live yet!");
        require(qty > 0 && qty <= maxMintPerTx, "AICircus: Invalid mint amount!");
        require(totalSupply() + qty <= maxSupply,"Notice AI: Max Supply Exceeded!");
