...
contract RowdyKids is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    */
    ...
    function privateSaleMint() public payable nonReentrant {
        _internalMint(State.PrivateSale, 1, MAX_MINTS_PER_WALLET_PRIVATE, MAX_MINTS_PER_WALLET_PRIVATE, MAX_MINTS_PRIVATE);

    }

    function presaleMint(uint256 _numberOfTokens) public payable nonReentrant {
        _internalMint(State.PreSale, _numberOfTokens, MAX_MINTS_PER_WALLET_PRESALE, MAX_MINTS_PER_WALLET_PRESALE, MAX_MINTS_PRESALE);
    }

    function publicMint(uint256 _numberOfTokens) public payable nonReentrant {
        _internalMint(State.PublicSale, _numberOfTokens, 0, MAX_MINTS_PER_TX, maxTokenSupply);
    }

    /*
    * Private sale Mint RowdyKids NFTs
    */
    function _internalMint(State _state, uint _numberOfTokens, uint _maxMintWallet, uint _maxMintTX, uint _maxMints) private {

        uint8 idx = uint8(_state);
        require(state == _state, "not in sale");
        require(_numberOfTokens <= _maxMintTX, "maxMints/tx exceeds");
        require(totalSupply() + _numberOfTokens <= _maxMints, "soldout");
        require(mintPrice[idx] * _numberOfTokens == msg.value, "eth value isn't correct");

        require(block.timestamp >= saleStartTime[idx], "not sale time");

        if(state < State.PublicSale) {
            require(block.timestamp <= saleEndTime[idx], "not sale time");
            require(_presaleAllowList[idx][_msgSender()] == true, "not in WL");
            require(_presaleMinted[idx][_msgSender()] + _numberOfTokens <= _maxMintWallet, "maxMints/wallet exceeded");
