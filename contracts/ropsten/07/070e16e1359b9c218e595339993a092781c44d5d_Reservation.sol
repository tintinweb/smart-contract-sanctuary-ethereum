...
contract ReservationCore is SignatureValidator {
    ...
    struct User {
        ...
        mapping(Types.Island => mapping(bytes32 => uint256)) reservedAssestAmount;
    ...
    function _reserve(
        ...
        if (userInfo[_user].reservedAssestAmount[_island][_unitHash] == 0) {
        ...
        userInfo[_user].reservedAssestAmount[_island][_unitHash] += quantity;
...
contract Reservation is ReservationCore {
    ...
    function getReservedAssestAmount(
        address user,
        Types.Island island,
        bytes32 assestHash
    ) external view returns (uint256) {
        _isReserved(user);
        return userInfo[user].reservedAssestAmount[island][assestHash];
    }
