...
contract KARAFURUDAO is Context, IERC20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address payable public teamWalletAddress = payable(0x3f496b376F4C813C2B3d47427652e445f7A97421); 
    ...
    uint256 private _tTotal = 7_000_000_000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "KARAFURU DAO";
    string private _symbol = "FURU";
    ...
    uint256 private minimumTokensBeforeSwap = 700_000 * 10**18;
    ...
    constructor () 
    {
        ...
        _isExemptFromTxLimit[teamWalletAddress] = true;
    }
    ...
    {
        uint256 _totalFees = _liquidityFee.add(_marketingFee);
        ...
        uint256 liquidityShare = newBalance.mul(_liquidityFee).div(_totalFees).div(2);

        addLiquidity(halfLiquidityTokens, liquidityShare);
        emit SwapAndLiquify(halfLiquidityTokens, liquidityShare, halfLiquidityTokens);

        uint256 teamShare = newBalance.sub(liquidityShare);
    ...
    {
        require(_amount>_tTotal.div(1000), "Too low Txn limit"); // Min 0.1%
    ...
    function setTeamAddress(address _teamWalletAddress) external onlyOwner() 
    {
