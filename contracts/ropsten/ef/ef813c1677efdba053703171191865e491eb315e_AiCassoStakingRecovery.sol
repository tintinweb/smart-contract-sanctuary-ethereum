...
contract AiCassoStakingRecovery is Ownable, ERC721Receiver {
    ...
    function recovery(uint256 numberOfTokens) external {
        ...
        uint256 contractBalanceNFT = IERC721( AiCassoNFT ).balanceOf(address(this));
        require(contractBalanceNFT >= numberOfTokens, "check balance fail");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            IERC721( AiCassoNFT ).transferFrom(
                address(this),
                msg.sender,
                IERC721Enumerable( AiCassoNFT ).tokenOfOwnerByIndex(address(this), (contractBalanceNFT - 1))
            );
        }
