...
contract MintController is Controller {
     ...
     */
    function removeMinter(address _minter) public onlyController returns (bool) {
        for (uint i; i < controllers[msg.sender].length; i++){
     ...
     */
    function configureMinter(address _minter, uint256 _newAllowance, uint256 _minterCap, uint256 _maxPerTxn)
        public
        payable
        onlyController
        returns (bool)
    {
        ...
        require(minterController[_minter] == msg.sender, "current contoller must remove minter");
        controllers[msg.sender].push(_minter);
        minterCap[_minter] = _minterCap;
        emit MinterConfigured(msg.sender, _minter, _newAllowance);
        return internal_setMinterAllowance(_minter, _newAllowance, _minterCap, _maxPerTxn);
     ...
     * returns true.
     */
    function incrementMinterAllowance(uint256 _allowanceIncrement, address _minter)
        public
        onlyController
        returns (bool)
    {
        ...
        //require(minterController[_minter] == msg.sender, "this minter has a different controller");
        require(
            minterManager.isMinter(_minter),
        ...
        return internal_setMinterAllowance(_minter, newAllowance, minterCap[_minter], maxPerTxn[_minter]);
    }
     ...
     */
    function decrementMinterAllowance(uint256 _allowanceDecrement, address _minter)
        public
        onlyController
        returns (bool)
    {
     ...
     * @param _newAllowance New allowance to be set for minter.
     */
    function internal_setMinterAllowance(address _minter, uint256 _newAllowance, uint256 _minterCap, uint256 _maxPerTxn)
        internal
        returns (bool)
    {
        return minterManager.configureMinter(_minter, _newAllowance, _minterCap, _maxPerTxn);
    }
}
...
contract Controller is Ownable, ControllerAdmin {
    ...
    /**
    *@notice a minter object has a thresold per txn, a total threshold,
    *a multisig approval rate (nbr sigs needed) and multisig addrs
    */

    /*
    struct Minter {
        address minterAddr;
        uint256 thresholdPerTx;
    ...
    */ 
    ...
    mapping(address => address) internal minterController;
        ...
        onlyControllerAdmin
    {
        ...
        require(_minter != address(0), "Worker must be a non-zero address");
     ...
     */
    function getMinters(address _controller) external view returns (address[] memory) {
        require(controllers[_controller].length != 0, "unconfigured controller");
...
contract ControllerAdmin is Ownable{
    ...
    address internal controllerAdmin;
    ...
    modifier onlyControllerAdmin() {
        require(
            msg.sender == controllerAdmin, 
            "ControllerAdmin: caller not the controller admin"
        );
        _;
    }
