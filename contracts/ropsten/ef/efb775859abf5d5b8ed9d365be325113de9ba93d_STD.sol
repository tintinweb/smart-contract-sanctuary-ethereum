...
pragma solidity >=0.4.22 <0.7.0;
    ...
    contract STD {
        address owner;
        // 宣告 string 變數設定為組員學號。
        string info = "410821314";
        // 記錄學號
        event record(string);
        /* 防止 Replay attacks */
        mapping(uint256 => bool) usedNonces;
        /* 發布合約時, 將 owner 設為發佈地址 */
        constructor() public {
            owner = msg.sender;
    }
    ...
    function verify(bytes32 _message, uint8 nonce,
            bytes memory signture) public{
        require(!usedNonces[nonce]);
        usedNonces[nonce] = true;
        /* 驗證的前綴 */
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        /* 將 prefix 與 _message 使用 abi.encodepacked(．,．)
          重新編碼，並使用 keccak256(．)做雜湊運算 */
