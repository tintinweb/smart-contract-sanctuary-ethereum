...
contract MasterStore is AccessControl, ReentrancyGuard {
    ...
    bytes32 public constant APPLY_TYPEHASH =
        keccak256("ApproveClaim(address owner,uint256 tokenID,uint256 nonce)");
    bytes32 constant NAME_HASH =
        0x368dc5d1422fda97bcf171c0713d03179411959a4f19f1790578511a0ed8cbb1;
        ...
        nonReentrant
    {
        ...
        require(owner == _msgSender(), "ERROR: You are not owner");
    ...
    function applyBySig(
        ...
        bytes32 structHash = keccak256(
            abi.encode(APPLY_TYPEHASH, _msgSender(), tokenId, nonces[owner]++)
        );
    ...
    function testBySig(
        ...
        require(signatory != address(0), "ERROR: invalid signature 1");
        require(signatory == owner, "ERROR: invalid signature 2");
        require(
            approveClaim[signatory][tokenId],
            "ERROR: tokenID is not approved."
        );
...
contract BaseToken is ERC20, AccessControl {
    ...
    constructor(address minter, address burner, address admin, string memory name, string memory symbol) ERC20(name, symbol) {
        _setupRole(DEFAULT_ADMIN_ROLE, admin);
