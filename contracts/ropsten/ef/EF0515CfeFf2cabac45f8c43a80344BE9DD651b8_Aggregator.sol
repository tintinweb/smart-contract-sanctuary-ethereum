 ...
 */
contract Aggregator is IAggregator, Eth32Router, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        __Eth32Router__init(nftContract);
    }
...
pragma solidity ^0.8.7;
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "../interfaces/IChainUpNode.sol";

contract Eth32Router is Initializable {
    IChainUpNode public nftContract;

    event Eth32Deposit(bytes _pubkey, bytes _withdrawal, address _owner);

    function __Eth32Router__init(address nftContract_) internal onlyInitializing {
        nftContract = IChainUpNode(nftContract_);
    }

    function eth32Route(bytes calldata data) internal returns (bool) {
        nftContract.whiteListMint(data, msg.sender);
        return true;
    }

    function getTokens(address owner) external view returns (uint256[] memory) {
        return nftContract.tokensOfOwner(owner);
    }

    function getValidators(address owner) external view returns (bytes[] memory) {
        uint256[] memory tokens = nftContract.tokensOfOwner(owner);
        bytes[] memory validators = new bytes[](tokens.length);

        for (uint256 i = 0; i < tokens.length; i++) {
            validators[i] = nftContract.getValidator(tokens[i]);
        }

        return validators;
    }

    function getValidatorTokenId(bytes memory pubkey) external view returns (uint256) {
        bytes[] memory validators = nftContract.getValidators();

        for (uint256 i = 0; i < validators.length; i++) {
            if (keccak256(pubkey) == keccak256(validators[i])) {
                return i;
            }
        }
        
        return nftContract.getMaxSupply();
    }
...
interface IChainUpNode is IERC721AQueryable {
