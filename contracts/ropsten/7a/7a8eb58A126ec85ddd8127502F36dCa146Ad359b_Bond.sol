...
contract Bond is Ownable, Pausable { 
   ...
   mapping(uint256 => uint256) public stakedAlpha; // 25% of the PairToken Amount
   // Events
   ...
   constructor(IERC20 _alphaToken) {
   ...
   function depositeToken(uint _amount, address pairToken) public whenNotPaused { 
       ...
       // Take 25% Alpha Token
       uint256 alphaInterest = _amount.mul(25).div(100); // Bonus
     
       //10% tax on actual amount (without bonus)
       uint256 taxPoolAmount = _amount.mul(10).div(100);
       uint256 remainedForAlpha = _amount.sub(taxPoolAmount);
       taxPool = taxPool + taxPoolAmount;
       // Total Alpha (cumulative amount + Bonus)
       uint256 alphaToStake = remainedForAlpha + alphaInterest;
       // Updates
       stakers.push(msg.sender);
       poolOwner[currentPoolID] = msg.sender;
       // Stacked Alpha from the interest
       stakedAlpha[currentPoolID] = alphaToStake;
   ...
   function calculateDrip(uint256 poolID) internal virtual returns(uint256, uint256){
       ...
       uint256 stakedAlphAmount = stakedAlpha[poolID];
       ...
       if(stakingPeriodInDays >= 1 && stakingPeriodInDays <= 365){
           drip = stakedAlphAmount.mul(stakingPeriodInDays).div(100);
       }
       if(stakingPeriodInDays >= 365){
           drip = stakedAlphAmount.mul(365).div(100);
       }
       if(totalstakedDays > 365 ){
           if(claimedAt[poolID]> stakingStartTime[poolID]){
               ...
               drip = stakedAlphAmount.mul(toBeClaimedForDays).div(100);
           }
       }
   ...
   function restake(uint256 poolID) public whenNotPaused {
       ...
       taxPool = taxPool + taxPoolAmount;
       // updates
       stakedAlpha[poolID] = stakedAlpha[currentPoolID] + remainedToStaked;
   ...
   function calculatetaRate(uint256 poolID) internal virtual returns(uint256){
       ...
       uint256 taxRate;
       uint256 alphaTotalSyupply = alphaToken.totalSupply();
       uint256 totalSupplyA = alphaTotalSyupply.mul(500).div(DIVISOR).div(100); // 0.5% of total supply
       uint256 totalSupplyB = alphaTotalSyupply.mul(1000).div(DIVISOR).div(100); // 1% of total supply
       uint256 totalSupplyC = alphaTotalSyupply.mul(5000).div(DIVISOR).div(100); // 5% of total supply
       uint256 totalSupplyD = alphaTotalSyupply.mul(10000).div(DIVISOR).div(100); // 10% of total supply
       uint256 totalSupplyE = alphaTotalSyupply.mul(15000).div(DIVISOR).div(100); // 15% of total supply
   ...
   function claim(uint256 poolID) public whenNotPaused {
       ...
       // Alpha Token transfered
       require(alphaToken.balanceOf(address(this)) > remainedToClaim, "Not enough alpha token in the contract");
       alphaToken.safeTransfer(msg.sender, remainedToClaim);
       // updates
       if(stakeTime > 365){
   ...
   function setAlphaToken(IERC20 _alphaToken) external onlyOwner whenNotPaused {
       alphaToken = _alphaToken;
   }
 
   function getAlphaToken() external view whenNotPaused returns(IERC20){
       return alphaToken;
   }
   /**
