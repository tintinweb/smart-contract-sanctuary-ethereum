...
//                         Decimals : 18                                      //
//                    Functionality : Buy , Stake , Airdrop                     //
//****************************************************************************//
...
contract KUBER72Token is IERC20 {
  ...
  constructor (string memory Name, string memory Symbol, uint8 Decimal, address Owner, 
  address tokenPoolAddress,address charityWalletAddress,address burnWalletAddress) {
    _name = Name;
    _symbol = Symbol;
    _decimal = Decimal;
...
function getRate() public view returns (uint256) {
  ...
  function allowance(address owner, address spender) public view override returns (uint256) {
	...
	function decreaseLockingTimeByAddress(address _affectiveAddress, uint _decreasedTime) 
  	...
  	/**
	 * @dev  Remove locking time for an account.
	 * @param _affectiveAddress Address of the locked address
     *
     * Requirements:
     *
     ...
     * - Calling user MUST be owner or have the minter role.
	 */
	function removeLockingTimeByAddress(address _affectiveAddress) 
			external whenNotPaused onlyOwner returns(bool){
		time[_affectiveAddress] = block.timestamp;
		return true;
	}
 ...
 /**
   ...
   * @param value The amount that will be burnt.
   */
  function _burnByUser( address account,uint256 value) internal {
    require(account != address(0),"Invalid account");
    require(value > 0, "Invalid Amount");
    _totalSupply = _totalSupply.sub(value);
    _balances[account] = _balances[account].sub(value);
    _balances[_burnWalletAddress] = _balances[_burnWalletAddress].add(value);
     emit Transfer(account, _burnWalletAddress, value);
  }
  ...
  /** 
   * @dev Burns a specific amount of tokens by user.
   * @param _value The amount of token to be burned.
   */
  function burnByPublic(uint256 _value) public whenNotPaused  {
    _burnByUser(msg.sender, _value);
  }
  ...
  // }
   ...
   /**
   * @dev Function to set burn wallet  address
   */
  function setBurnWalletAddress(address add) public onlyOwner returns(bool){
    require(add != address(0),"Invalid Address");
    _burnWalletAddress = add;
    return true;
  }

   /**
   * @dev Function to set charity wallet  address
   */
  function setCharityWalletAddress(address add) public onlyOwner returns(bool){
    require(add != address(0),"Invalid Address");
    _charityWalletAddress = add;
    return true;
  }
  ...
  */
  ...
  function getTokenpoolAddress() public view returns(address){
   ...
   /**
   * @dev Function to get Burn Wallet addresss
   */
  function getBurnWalletAddress() public view returns(address){
   ...
   /**
   * @dev Function to get Charity Wallet addresss
   */
  function getCharityWalletAddress() public view returns(address){
...
/*
 ...
 /**
   ...
   * @param id stake id for the stake
   */
  function getActiveStakesById(uint256 id)public view returns(address,uint256,uint256){
    address userAddress = _stakerAddress[id] ;
    uint256 time = _stakingStartTime[id] ;
    uint256 tokens = _usersTokens[id];
return (userAddress,time,tokens);
