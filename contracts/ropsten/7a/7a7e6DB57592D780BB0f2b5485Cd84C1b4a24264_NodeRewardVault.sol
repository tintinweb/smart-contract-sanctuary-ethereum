...
contract NodeRewardVault is INodeRewardVault, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256[] private _cumArray;
    uint256[] private _cumHeight;
    ...
    function _rewards(uint256 tokenId) private view returns (uint256) {
        if (_cumArray.length == 0) {
            return 0;
        }
        uint256 gasHeight = _nftContract.gasHeightOf(tokenId);
        uint256 low = 0;
        uint256 high = _cumHeight.length;

        while (low < high) {
            uint256 mid = (low + high) >> 1;

            if (_cumHeight[mid] > gasHeight) {
                high = mid;
            } else {
                low = mid + 1;
            }
        ...
        return _cumArray[_cumArray.length - 1] - _cumArray[low];
    }
    ...
    receive() external payable {
        uint256 a = _cumArray.length;
        if (_nftContract.totalSupply() == 0) { // free money?
            return;
        } else if (a == 0) {
            _cumArray.push(msg.value / _nftContract.totalSupply());
        } else {
            _cumArray.push(_cumArray[a - 1] + msg.value / _nftContract.totalSupply());
        }
        _cumHeight.push(block.number);
    }
}
