...
\__ \| |    |  _  |  | |  |  _  || | | |  |  _| / _ \ | '__| / __| / _ \
(   /| \__/\| | | | _| |_ | | | |\ \_/ /  | |  | (_) || |   | (__ |  __/
 |_|  \____/\_| |_/ \___/ \_| |_/ \___/   |_|   \___/ |_|    \___| \___|

Perhaps the most proper and popular smart contract ever written.

    WEBSITE:
https://www.chiao.io/

    WHITEPAPER:
https://www.chiao.io/whitepaper

    CertiK AUDIT:
Unlike most projects, Chiaotzu Inu will be audited BEFORE LAUNCH!
We do this to ensure your investment is as safe and secure as possible.

    MAIN TELEGRAM (English):
https://t.me/chiaotoken

    DISCORD:
https://discord.io/CHIAO

    TWITTER:
https://twitter.com/chiaotoken

    REDDIT:
https://www.reddit.com/r/chiaotoken/

    MEDIUM:
https://chiaotzutoken.medium.com/

    INSTAGRAM:
https://www.instagram.com/chiaotzuinu/

    FACEBOOK:
https://www.facebook.com/ChiaotzuInuToken

    SLIPPAGE:
Set your slippage to 50% on Uniswap to avoid failed transactions and any other problems.
Chiaotzu Inu uses advanced anti-bot scripting which makes front-running impossible for bots,
even with such high slippage.  

    TOKEN FEE STRUCTURE:
ABSOLUTELY NO FEE OR TAX ON REGULAR SEND / RECEIVE TRANSACTIONS! 
Chiaotzu Inu only charges taxes on Buy and Sell Transactions!
        ON BUY 14%
—10% Marketing | —2% Auto LP | —1% Reflections | —1% Development

        ON SELL 16%
—10% Marketing | —2% Auto LP | —2% Reflections | —2% Development

        ON EARLY SELL 30% (24 HOURS FROM THE FIRST AND ONLY THE FIRST BUY PER WALLET):
—24% Marketing | —2% Auto LP | —2% Reflections | —2% Development
Chiaotzu Inu will charge taxes on send / receive transactions during the early sell period.

    TOKEN DEFAULT THRESHOLDS:
Max Buy & Sell Limit: 0.5%
Max Wallet Size: 2%

    DISTRIBUTION:
—75% Tradable Liquidity | 20% EVM Bridges (BSC, FTM, etc...) | 5% CEX (Binance, OKEx, Bitmart & many many more)

    LOCKED LIQUIDITY:
As you can see, Chiaotzu Inu is very transparent and will honor this
transparency by locking 100% of the tradable liquidity on Unicrypt for
1 year before trading is actually opened up to the public. This will
give $CHIAO investors an undeniable and unmatched trust score. Safely 
invest with total confidence knowing you can believe in your Chiao Force!

Chiao Force may also lock the Project tokens in anticipation of
Bridging and CEX listings before Open trading to ensure a trusted
environment is made suitable for early adoption of the platform.

    ZERO-BOT TOLERANCE:
For the first 5 blocks after Chiaotzu Inu opens trading on mainnet,
all wallets who buy during this time will be blacklisted and unable
to sell their tokens. If a normal user who is not a bot buys during
this time, they may request to be unblacklisted and may be granted the
ability to sell their tokens again if Chiao Force sees them fit.

    AUTOMATED LIQUIDITY TOKEN BURNS:
$CHIAO's floor is literally guaranteed to increase forever. Chiaotzu Inu's
smart contract automatically burns LP tokens at a rate of 0.25% per hour,
this rate may also be increased and never decreased, which will raise the
value of all holders at a much faster rate!
...
contract ChiaotzuInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxTransactionAmount = _tTotal * 5 / 1000; // 0.5% maxTransactionAmountTxn
    uint256 public maxWallet = _tTotal * 2 / 100; // 2% maxWallet
    ...
    uint256 public earlySellMarketingFee = 24;
    uint256 public earlySellLiquidityFee = 2;

    // Penalty Fee Time
    uint256 public _penaltyFeeTime = 86400; // by seconds units
    ...
    constructor() ERC20("Chiaotzu Inu", "CHIAO") {
        ...
        marketingWallet = address(0x42BC308cbf38AB82a4BF8e35cC7c99d145d078c4); // set as marketing wallet
        devWallet = address(0x6421722a7C92dE9deEA6fA88Bb37EC6bc183D725); // set as dev wallet
    ...
    function setPenaltyFee(uint256 _earlySellMarketingFee, uint256 _earlySellLiquidityFee) external onlyOwner {
        ...
        totalFeesForPenalty = earlySellMarketingFee + earlySellLiquidityFee + sellDevFee;
        require(totalFeesForPenalty + sellReflectionFee <= 30, "Must keep fees at 30% or less");
    }
