...
contract BitrngDice {
  ...
  /** @dev Place the game
    * @param betMask bit mask for bet, see detail below
    * @param bet1Amount Bet #1 amount.
    * @param bet2Amount Bet #2 amount.
    * @param bet3Amount Bet #3 amount.
    * @param bet4Amount Bet #4 amount.
    * @param bet5Amount Bet #5 amount.
    * @return commitLastBlock number of the maximum
    *                         block where "commit" is still considered valid.
    * @return commit The calculated perimeter.
    * @return r The calculated perimeter.
    * @return s The calculated perimeter.
  */
  ...
  // <2bit big/small>
  // <6bit same 1/2/3/4/5/6>
  // <4bit number 9/10/11/12>
  // <2bit number 8/13>
  // <2bit number 7/14>
  // <2bit number 6/15>
  // <2bit number 5/16>
  ...
  {
    // Check actul amount and sum of place amount are equal.
  ...
  {
    bytes32 signatureHash = keccak256(abi.encodePacked(uint40(commitLastBlock), commit));
    if(secretSigner == ecrecover(signatureHash, 27, r, s)){
      return 1;
    }else{
      return 0;
    }
