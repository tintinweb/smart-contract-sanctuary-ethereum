...
contract AgencyTreasury is Ownable {
    // Underlying AGENCY token
    AgencyERC20Token private AGENCY;

    // underlying stable token we are holding and valuing treasury with
    IERC20 private STABLEC;

    // Minted agency allocated to various investment contracts
    MintAllowance public MINT_ALLOWANCE;

    // Ratio of treasury value in stablec to open supply of agency.
    ...
    // Agency rewards harvested, and (yet) to be allocated to a pool
    uint256 public harvestedRewardsAgency;
    ...
    constructor(AgencyERC20Token _AGENCY, IERC20 _STABLEC) {
      AGENCY = _AGENCY;
      STABLEC = _STABLEC;
      MINT_ALLOWANCE = new MintAllowance(_AGENCY);
    }
    ...
    /**
     * Seed treasury with STABLEC and Agency to bootstrap
     */
    function seedMint(uint256 amountStablec, uint256 amountAgency) external onlyOwner {
      require(!seeded, "Owner has already seeded treasury");
      seeded = true;

      // can this go in the constructor?
      intrinsicValueRatio.stablec = amountStablec;
      intrinsicValueRatio.agency = amountAgency;

      SafeERC20.safeTransferFrom(STABLEC, msg.sender, address(this), amountStablec);
      // treasury should be whitelisted in agency contract
      AGENCY.mint(msg.sender, amountAgency);
    }
    ...
    function harvest(uint256 distributionPercent) external onlyOwner {
      ...
      // // Burn any excess agency, that is Any agency over and beyond harvestedRewardsAgency.
      // // NOTE: If we don't do this, IV could drop...
      if (AGENCY.balanceOf(address(this)) > harvestedRewardsAgency) {
        // NOTE: there isn't a Reentrancy issue as we control the AGENCY ERC20 contract, and configure
        //       treasury with an address on contract creation
        AGENCY.burn(AGENCY.balanceOf(address(this)) - harvestedRewardsAgency);
      }

      uint256 totalSupplyAgency = AGENCY.totalSupply() - AGENCY.balanceOf(address(MINT_ALLOWANCE));
      uint256 impliedSupplyAtCurrentIVAgency = reserveStablec * intrinsicValueRatio.agency / intrinsicValueRatio.stablec;

      require(impliedSupplyAtCurrentIVAgency >= totalSupplyAgency, "Cannot run harvest when IV drops");

      uint256 newHarvestAgency = (impliedSupplyAtCurrentIVAgency - totalSupplyAgency) * distributionPercent / 100;
      harvestedRewardsAgency += newHarvestAgency;

      intrinsicValueRatio.stablec = reserveStablec;
      intrinsicValueRatio.agency = totalSupplyAgency + newHarvestAgency;

      AGENCY.mint(address(this), newHarvestAgency);
      emit RewardsHarvested(newHarvestAgency);
    }
    ...
    function resetIV() external onlyOwner {
      ...
      uint256 totalSupplyAgency = AGENCY.totalSupply() - AGENCY.balanceOf(address(MINT_ALLOWANCE));
      intrinsicValueRatio.stablec = reserveStablec;
      intrinsicValueRatio.agency = totalSupplyAgency;
    }
    ...
    function distributeHarvest() external onlyOwner {
      ...
      for (uint256 i = 0; i < pools.length; i++) {
        uint256 allocatedRewards = harvestedRewardsAgency * poolHarvestShare[pools[i]] / totalHarvestShares;

        // integer rounding may cause the last allocation to exceed harvested
        // rewards. Handle gracefully
        if ((totalAllocated + allocatedRewards) > harvestedRewardsAgency) {
          allocatedRewards = harvestedRewardsAgency - totalAllocated;
        }
        totalAllocated += allocatedRewards;
        SafeERC20.safeTransfer(AGENCY, pools[i], allocatedRewards);
        emit HarvestDistributed(pools[i], allocatedRewards);
      }
      harvestedRewardsAgency -= totalAllocated;
    }

    /**
     * Mint and Allocate treasury AGENCY.
     */
    function mintAndAllocateAgency(address _contract, uint256 amountAgency) external onlyOwner {
      require(amountAgency > 0, "AGENCY to mint and allocate must be > 0");

      // Mint and Allocate AGENCY via MINT_ALLOWANCE helper
      AGENCY.mint(address(this), amountAgency);
      SafeERC20.safeIncreaseAllowance(AGENCY, address(MINT_ALLOWANCE), amountAgency);
      MINT_ALLOWANCE.increaseMintAllowance(_contract, amountAgency);
    }

    /**
     * Burn minted agency associated with a specific contract
     */
    function unallocateAndBurnUnusedMintedAgency(address _contract) external onlyOwner {
...
contract AgencyERC20Token is ERC20, ERC20Burnable, Ownable, AccessControl {
    bytes32 public constant CAN_MINT = keccak256("CAN_MINT");

    constructor() ERC20("Agency", "AGENCY") {
...
/**
 * Newly minted agency allocated to various agency strategies
 *
 * Any agency held by this contract is assumed to be unused,
...
contract MintAllowance is Ownable {
    // Agency contract address
    AgencyERC20Token AGENCY;

    constructor(AgencyERC20Token _AGENCY) {
      AGENCY = _AGENCY;
    }
    ...
    function increaseMintAllowance(address _pool, uint256 _amount) external onlyOwner {
      SafeERC20.safeTransferFrom(AGENCY, msg.sender, address(this), _amount);
      SafeERC20.safeIncreaseAllowance(AGENCY, _pool, _amount);
    }
    ...
    function burnUnusedMintAllowance(address _pool) external onlyOwner {
      uint256 unusedMintAllowance = AGENCY.allowance(address(this), _pool);
      SafeERC20.safeDecreaseAllowance(AGENCY, _pool, unusedMintAllowance);
      AGENCY.burn(unusedMintAllowance);
    }
}
