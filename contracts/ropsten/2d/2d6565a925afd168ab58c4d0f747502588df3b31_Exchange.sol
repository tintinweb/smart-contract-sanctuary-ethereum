...
contract Exchange is Initializable, OwnableUpgradeable, UUPSUpgradeable{
    ...
    uint256 public bnbDecimal;
    uint256 public realtyumDecimal;
...
/// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function initialize() initializer public {
        ...
        admin=0x262cf988153e4eD8171d457BB8C33DdfA6d64bD3;
    }
    ...
    function setBNBDecimal(uint256 _bnbDecimal) public onlyAdmin(msg.sender){
        bnbDecimal=_bnbDecimal;        
    }

    function setRealtyumDecimal(uint256 _realtyumDecimal) public onlyAdmin(msg.sender){
        realtyumDecimal=_realtyumDecimal;        
    }
    ...
    function setExchangeRate( uint256 _exchangeRate, string memory _tokenHash) 
    public validateHash(_tokenHash) {
        
        exchangeRate=_exchangeRate;
    }

    function exchangeBNB(uint256 _bnbAmount, uint256 _realtyumAmount, string memory _tokenHash) 
    public payable validateHash(_tokenHash) validAmount(_bnbAmount){

        // uint256 bnbAmount = (_bnbAmount / (10**bnbDecimal) );
        // uint256 _amount=(bnbAmount*exchangeRate)* (10**realtyumDecimal);
        // require(_amount>0, "Package Data Amount is not valid");
        
        // check allowance
        uint256 allowance=realtyumToken.allowance(admin,address(this));
        require(allowance>=_realtyumAmount,"check token allowance");
        ...
        realtyumToken.transferFrom(admin,msg.sender,_realtyumAmount); 
            
        emit ExchangeCompleted(_bnbAmount, msg.sender,block.timestamp);
