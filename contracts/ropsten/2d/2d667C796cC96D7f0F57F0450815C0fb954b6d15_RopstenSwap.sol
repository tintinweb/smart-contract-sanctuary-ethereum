...
import "./RopstenSwapErrors.sol";

/**
 * Ropsten-to-Ropsten swap via 0x API
 */
contract RopstenSwap is ZetaInteractor, ZetaReceiver, RopstenSwapErrors {
    bytes32 public constant SWAP_MESSAGE = keccak256("ROPSTEN_SWAP");
        ...
        payable
    {
        ...
        if (!sellToken.approve(args.allowanceTarget, args.sellAmount))
        ...
        isValidMessageCall(zetaMessage)
    {
        ...
        ) = abi.decode(zetaMessage.message, (bytes32, address, address, address, bytes));
        ...
        if (!zetaToken.approve(secondLegSwapTarget, zetaMessage.zetaAmount))
            revert ErrorApprovingTokens(_zetaToken);
        ...
        bool success2 = buyToken.transfer(receiverAddress, buyAmount);
