...
contract ESkillzStraightBet is Ownable {
    ...
    constructor () public { sport = IERC20(0x1ff9C508F4Ba854cC5eEb50E0CBd6cAF9cc88006);  eskillz_fee = 5; feeReceiver = 0xa300915690Ac000E05bd5fD91f6C14EF0838727c;}
    ...
    function finishGame(uint256 game) external returns(bool) {
        uint256 rand = genRand(2);
        bool retval = false;
        (uint256 amountToWinner,uint256 amountToESkillz) = getAmountsToDistribute(game);
        if(rand == 1) {
            retval = true;
            sport.transferFrom(address(this), gamebetting[game][0].player, amountToWinner);
        } else {
            sport.transferFrom(address(this), gamebetting[game][1].player, amountToESkillz);
        }
