...
2. Receive your Rand Labs Super Coder NFT
3. Discover the application email address and fire your CV over (make sure you include your id and NFT id)
*/

pragma solidity >0.8.0;

import "@openzeppelin/contracts/access/Ownable.sol";
import "./IRandLabsSuperCoderNFT.sol";
...
contract Puzzle is Ownable {
  ...
  function addCandidateIdToWhitelist(bytes32 _key, string memory _candidateId)
    public
  {
    require(!candidateIdWhitelist[_candidateId], "Candidate id exists");
    ...
    require(_key == secretKey, "Invalid key");

    candidateIdWhitelist[_candidateId] = true;

    if (nftAddress != address(0)) {
      IRandLabsSuperCoderNFT nftContract = IRandLabsSuperCoderNFT(nftAddress);
      require(nftContract.balanceOf(msg.sender) == 0, "NFT already awarded");
      nftContract.award(msg.sender);
    }
  }

  function candidateIdIsWhitelisted(string memory _candidateId)
    public
    view
    returns (bool)
  {
    return candidateIdWhitelist[_candidateId];
  }
...
interface IRandLabsSuperCoderNFT is IERC721 {
