...
contract Reveal is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function createDrop(uint256 _redeemTime, uint256 _price , uint256 _dropsupply) external onlyOwner {
        require(_redeemTime >= block.timestamp , "Invalid Redeem Time");
        require(_price > 0, "Invalid price");
        require(_dropsupply > 0, "Invalid Supply");
        require(supply + _dropsupply <= totalTokenCount, "Total Supply Reached");
        drop += 1;
        redeemTime[drop] = _redeemTime;
        supply += _dropsupply;
        dropSupply[drop] = _dropsupply;
        mintPrice = _price;
    }
