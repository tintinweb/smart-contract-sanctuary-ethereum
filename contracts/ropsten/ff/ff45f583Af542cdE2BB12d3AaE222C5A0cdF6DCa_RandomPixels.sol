...
contract RandomPixels is ERC721, ERC721Enumerable, Ownable {
    ...
    function drawSvg(uint256 tokenId) private pure returns (string memory) {
        ...
        string memory color;
        uint8 SIZE = 16;
        // generate 16 x 16 px SVG
        for (uint8 i = 0; i < SIZE; ++i) {
            for (uint8 j = 0; j < SIZE; ++j) {
                // ensure below max shift of 256
                color = generateRandomColor(tokenId >> 1);
                descriptor = string(
                    abi.encodePacked(
                        descriptor,
                        "<rect fill='",
        ...
        return
            string(
                abi.encodePacked(
                    "<svg id='random-image' xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMinYMin meet' viewBox='0 0 16 16'>",
                    descriptor,
