...
contract Infynyty is ReentrancyGuard, Ownable{
    ...
    function buy(ERC1155Struct memory sellerInfo, ERC1155Struct memory buyerInfo, Sig memory sellerRSV, Sig memory buyerRSV) public payable nonReentrant {
        require(verifySigner(platformAddress, messageHash(abi.encodePacked(sellerInfo.seller, sellerInfo.tokenId, sellerInfo.NFTAddress, sellerInfo.nonce, msg.value, sellerInfo.amount)), sellerRSV), "Seller RSV Invalid");
        require(verifySigner(platformAddress, messageHash(abi.encodePacked(msg.sender, buyerInfo.seller, buyerInfo.tokenId, buyerInfo.NFTAddress, buyerInfo.nonce, msg.value, buyerInfo.amount)), buyerRSV), "Buyer RSV Invalid");
        (bool sent, bytes memory data) = payable(sellerInfo.seller).call{value: msg.value}("");
        require(sent, "Failed to send Ether");
        TransferNFT(nftTransferAddress).transferERC1155(sellerInfo.NFTAddress, sellerInfo.seller, msg.sender, sellerInfo.tokenId, sellerInfo.amount);
    }
