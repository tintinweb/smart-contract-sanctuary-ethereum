...
interface IDDLERC721 {
    ...
    function setBakOwner(address bakOwner) external;
}
...
interface IDDLPool is IERC721Enumerable{
    ...
    function depositDDL(uint256 amount) external;
    function retrieveDDL(uint256 lpTokenId) external;
    function extractDDLInterest(uint256 lpTokenId) external;
    function findDepositDDLInterest(uint256 lpTokenId)
        external
		view
        returns (uint256 amount);
    function setNFTAddress(address _nftAddress) external;
    function setDDLAddress(address _ddlAddress) external;
}

contract DDLPool is IDDLPool, ERC721Enumerable, Ownable{
    using EnumerableMap for EnumerableMap.UintToUintMap;

    address private nftAddress;
    address private ddlAddress;
    ...
    EnumerableMap.UintToUintMap private _lpDDLTokens;

    constructor(address _nftAddress, address _ddlAddress) ERC721("DDL LP", "DDLLP")  {
        nftAddress = _nftAddress;
        ddlAddress = _ddlAddress;
    ...
    function depositNFT(uint256 tokenId) external lock override {
        require(msg.sender == IDDLERC721(nftAddress).ownerOf(tokenId), "ERC721: transfer of token that is not owner");

        uint256 lpTokenId = block.timestamp;
         _safeMint(msg.sender, lpTokenId);
         
        IDDLERC721 erc721 = IDDLERC721(nftAddress);
    ...
    function retrieveNFT(uint256 lpTokenId) external lock override {
        ...
        uint256 amount = CalProfitMath.calStepProfitAmount(blockStartTime,uint32(lpTokenId % 2 ** 32),endTime,DAY_PROFIT);

        if(amount > 0 ){
            IDDLERC20 erc20 = IDDLERC20(ddlAddress);
            erc20.safetransfer(ddlAddress,msg.sender,amount);
        }
    ...
    function extractNFTInterest(uint256 lpTokenId) external lock override {
        require(lpTokenId > 0, "ERC721: deposit query for nonexistent token");
        require(ownerOf(lpTokenId) == msg.sender, "ERC721: extractNFTDai of token that is not owner");
        uint256 _tokenId = _lpNFTTokens.get(lpTokenId);
        require(_tokenId > 0, "ERC721: deposit query for existent token");
        ...
        if(amount > 0 ) {
            ...
            _safeMint(msg.sender, endTime);
            _lpNFTTokens.set(endTime, _tokenId);
        }
    }
    ...
    function findDepositNFTInterest(uint256 lpTokenId)
       ...
       uint256 _tokenId = _lpNFTTokens.get(lpTokenId);
        ...
        uint32 endTime = uint32(block.timestamp % 2 ** 32);
        amount = CalProfitMath.calStepProfitAmount(blockStartTime,uint32(lpTokenId % 2 ** 32),endTime,DAY_PROFIT);
    }

    function depositDDL(uint256 amount) external lock override {
        IDDLERC20 erc20 = IDDLERC20(ddlAddress);
        ...
        erc20.safetransfer(msg.sender,address(this),amount);
         ...
         uint256 lpTokenId = block.timestamp;
         _safeMint(msg.sender, lpTokenId);
         _lpDDLTokens.set(lpTokenId,amount);
    }

    function retrieveDDL(uint256 lpTokenId) external lock override  {
        ...
        uint256 _amount = _lpDDLTokens.get(lpTokenId);
        require(_amount > 0 , "deposit DDL amount not enough"); 

        IDDLERC20 erc20 = IDDLERC20(ddlAddress);
        uint256 pgamount = CalProfitMath.colProfitAmount(uint32(lpTokenId % 2 ** 32),uint32(block.timestamp % 2 ** 32),_amount,PDOG_PRV,PDOG_PRV_M);
        
        if(pgamount > 0){
            erc20.safetransfer(ddlAddress,msg.sender,pgamount);
        }
        erc20.transfer(msg.sender,_amount);
        
        _lpDDLTokens.remove(lpTokenId);
        _burn(lpTokenId);
    }

    function extractDDLInterest(uint256 lpTokenId) external lock override  {
        require(lpTokenId > 0, "DDLLP: deposit query for nonexistent token");
        require(ownerOf(lpTokenId) == msg.sender, "ERC721: retrieveDai of token that is not owner");
        ...
        uint32 endtime = uint32(block.timestamp % 2 ** 32);
        uint256 pgamount = CalProfitMath.colProfitAmount(uint32(lpTokenId % 2 ** 32),endtime,_amount,PDOG_PRV,PDOG_PRV_M);
        ...
        _safeMint(msg.sender, endtime);
        _lpDDLTokens.set(endtime,_amount);
    }
    ...
    function findDepositDDLInterest(uint256 lpTokenId)
        ...
        returns (uint256 amount) {
        uint256 _amount =  _lpDDLTokens.get(lpTokenId);
        amount = CalProfitMath.colProfitAmount(uint32(lpTokenId % 2 ** 32),uint32(block.timestamp % 2 ** 32),_amount,PDOG_PRV,PDOG_PRV_M);
    }
    ...
    function findUserLPTokens(address account) 
        ...
        for(uint32 i=0;i<len;i++){
            ...
            _amount = _lpDDLTokens.get(_lpTokenId);
            if(_tokenId > 0) {
                _lpTokens[i] = _lpTokenId;
                _tokens[i] = _tokenId;
                _URIs[i] = erc721.tokenURI(_tokenId);
                _amounts[i] = CalProfitMath.calStepProfitAmount(blockStartTime,uint32(_lpTokenId % 2 ** 32),endTime,DAY_PROFIT); 
            }else if(_amount > 0){
                _lpTokens[i] = _lpTokenId;
                _tokens[i] = _amount;
                _amounts[i] = CalProfitMath.colProfitAmount(uint32(_lpTokenId % 2 ** 32),endTime,_amount,PDOG_PRV,PDOG_PRV_M);
            }
        }
        return (_lpTokens,_tokens,_URIs,_amounts);
    }
    function findLpToTokenId(uint256 _lpTokenId) internal view virtual returns (uint256) {

    }

    function findUserNFTTokens(address amount) public view virtual override returns (uint256[] memory,string[] memory) {
        require(amount != address(0), "ERC721: balance query for the zero address");

        IDDLERC721 erc721 = IDDLERC721(nftAddress);
        uint256 balances = erc721.balanceOf(amount);

        uint256[] memory tokens = new uint256[](balances); 
        string[] memory tokenURIs  = new string[](balances);
        if(balances>0){
            for(uint i=0;i<balances;i++){
                tokens[i] = erc721.tokenOfOwnerByIndex(amount,i);
    ...
    function setDDLAddress(address _ddlAddress) external onlyOwner lock override {
        ddlAddress = _ddlAddress;
    }

    function updateTokenURI(string memory _tokenURI) external virtual onlyOwner lock {
        baseURI = _tokenURI;
    }
