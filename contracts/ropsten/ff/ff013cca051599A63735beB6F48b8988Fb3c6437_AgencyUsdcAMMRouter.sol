...
contract AgencyUsdcAMMRouter is Ownable, AccessControl {
    ...
    AgencyERC20Token public immutable agencyToken;
    IERC20 public immutable usdcToken;
    IAgencyTreasury public immutable agencyTreasury;
    ...
    constructor(
            ...
            AgencyERC20Token _agencyToken,
            IERC20 _usdcToken,
            IAgencyTreasury _agencyTreasury,
        ...
        agencyTreasury = _agencyTreasury;
        ...
        // experimental
        _setupRole(CAN_ADD_ALLOWED_USER, owner());
    }
    ...
    //   require(hasRole(CAN_ADD_ALLOWED_USER, owner()),"you don't have access to this");
    //   allowed[userAddress] = true;
    // }
    function addAllowedUser(address userAddress) external {
      require(hasRole(CAN_ADD_ALLOWED_USER, owner()),"you don't have access to this");
      allowed[userAddress] = true;
    }
    ...
    function addLiquidity(
        ...
        SafeERC20.safeTransferFrom(agencyToken, msg.sender, address(pair), amountA);
        SafeERC20.safeTransferFrom(usdcToken, msg.sender, address(pair), amountB);
    ...
    function swapExactFraxForTemple(
        ...
        // Swap on AMM
        if (amountInAMM > 0) {
            SafeERC20.safeTransferFrom(usdcToken, msg.sender, address(pair), amountInAMM);
            pair.swap(amountOutAMM, 0, to, new bytes(0));
        }

        // Mint on protocol
        if (amountInProtocol > 0) {
            SafeERC20.safeTransferFrom(usdcToken, msg.sender, protocolMintEarningsAccount, amountInProtocol);
            agencyToken.mint(to, amountOutProtocol);
    ...
    //     SafeERC20.safeTransferFrom(usdcToken, msg.sender, pair, amountIn);
    ...
    function swapExactTempleForFrax(
        ...
        if (priceBelowIV) {
            // if price is bellow IV, agency tokens that are sold are burned and stablec is sent from pair? 
            require(amountOut >= amountOutMin, 'AgencyUsdcAMMRouter: INSUFFICIENT_OUTPUT_AMOUNT');
            agencyToken.burnFrom(msg.sender, amountIn);
            SafeERC20.safeTransfer(usdcToken, to, amountOut);
        } else {
            ...
            require(amountOut >= amountOutMin, 'AgencyUsdcAMMRouter: INSUFFICIENT_OUTPUT_AMOUNT');
            SafeERC20.safeTransferFrom(agencyToken, msg.sender, address(pair), amountIn);
    ...
    // function swapAgencyForExactUsdc(
    //     uint amountOut,
    //     uint amountInMax,
    //     address to,
    //     uint deadline
    ...
    //     require(amountIn <= amountInMax, 'TempleFraxAMMRouter: EXCESSIVE_INPUT_AMOUNT');
    //     SafeERC20.safeTransferFrom(agencyToken, msg.sender, pair, amountIn);
    ...
    function swapExactTempleForFraxQuote(uint amountIn) public view returns (bool priceBelowIV, bool willCrossDynamicThreshold, uint amountOut) {
        ...
        // if AMM is currently trading above target, route some portion to mint on protocol
        (uint256 ivFrax, uint256 ivTemple) = agencyTreasury.intrinsicValueRatio();
