...
contract Synchron is ISynchron,ERC1155Holder{
    ...
    function createOrder(TradingType tType,address token,address creator,uint256 id,uint256 amount,uint256 single,uint256 startTime,
        ...
        if(tType == TradingType.trading){
            ...
            FixedOption memory option = FixedOption(token,creator,id,amount,single,startTime,endTime,block.timestamp,address(0),0,State.solding);
    ...
    function getActiveBid(uint256 orderId) public override view returns(uint256){
        ...
        if(offs.length > 1){
            for(uint i = offs.length.sub(1); i >= 0; i--){
                if(offs[i] > 0){
                    ...
                    uint256 amount = IERC20(weth).balanceOf(offer.bidder);
            ...
            if(offs.length == 1){
                Offer storage offer = offers[offs[0]];
                uint256 amount = IERC20(weth).balanceOf(offer.bidder);
                if(amount >= offer.price){
                    offId = offs[0];
                }
...
contract Trading is IEnum,ERC1155Holder{
    ...
    function getCancelStatus(TradingType tradingType,uint256 orderId) public view returns(bool whether){
        if(tradingType == TradingType.trading){
    ...
    function cancel(TradingType tradingType,uint256 orderId) public{
        require(getCancelStatus(tradingType,orderId) == true,"Trading:State wrong");
    ...
    function getCancelOfferStatus(uint256 orderId,uint256 offerId) public view returns(bool isSupp){
    ...
    function cancelOffer(uint256 orderId,uint256 offerId) public {
        require(getCancelOfferStatus(orderId,offerId),"Trading:State wrong");
    ...
    function getActiveOffers(uint256 orderId) public view returns(uint256){
        return ISynchron(sync).getActiveBid(orderId);
    }
    ...
    function getModifyStatus(TradingType tradingType,uint256 orderId,uint256 price) public view returns(bool isSupport){
        if(tradingType == TradingType.trading){
    ...
    function modify(TradingType tradingType,uint256 orderId,uint256 price) public{
        require(getModifyStatus(tradingType,orderId, price) == true,"Trading:State wrong");
        if(tradingType == TradingType.trading){
    ...
    function getRefuseStatus(uint256 orderId) public view returns(bool isSupport){
    ...
    function refuseDelivery(uint256 orderId) public {
        require(getRefuseStatus(orderId) == true,"Trading:State wrong");
        ISynchron(sync).cancelOrder(TradingType.auction, orderId);
        emit Cancel(TradingType.auction, msg.sender, orderId, block.timestamp);
    }

    function getResetStatus(TradingType tradingType,uint256 orderId) public view returns(bool isSupport){
        if(tradingType == TradingType.trading){
            (,address creator,,,,,uint256 endTime,,,,State state) = ISynchron(sync).fixedOptions(orderId);
            if(block.timestamp >= endTime && state == State.solding && creator == msg.sender){
                isSupport = true;
    ...
    function reSet(TradingType tradingType,uint256 orderId,uint256 sin,uint256 exp,uint256 start,uint256 end) public{
        require(getResetStatus(tradingType, orderId) == true,"Trading:State wrong");
...
//token721:0x9b925E4e94C747314002585A7cDA3daF08248178
//token155:0x1945bdA19047fd91578A24267b4BB360d6956520
//data:0x61724b559A1E86E891d1d59Ade9b13184fE40779
//trading:0xff52605D12957DBCe7D27c930E655913C1fE0835
