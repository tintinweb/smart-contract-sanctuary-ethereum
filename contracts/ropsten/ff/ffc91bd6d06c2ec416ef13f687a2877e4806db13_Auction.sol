...
contract Auction is Ownable {
    ...
    // Auction Resets for 90 seconds
    uint256 public auctionTime = 90;
    ...
    function bid(uint256 _auctionId, uint256 _amount) public {      
        ...
        require((auction[_auctionId].endTime - block.timestamp) <= auctionTime, "You can bid in only 90 seconds");
        ...
        emit Bid(msg.sender, _auctionId, auction[_auctionId].endTime, _amount, block.timestamp);
    }
    ...
    function cancelAuction(uint256 _auctionId, uint256 _type) public {
        ...
        require(auction[_auctionId].highestBidder == address(0),"You can't this auction");
        if(_type == 0){
    ...
    function changeAuctionTime(uint256 _time) public onlyOwner {
        auctionTime = _time;
    }
}