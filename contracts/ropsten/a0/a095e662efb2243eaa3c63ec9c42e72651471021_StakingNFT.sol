...
contract StakingNFT is Ownable {
   ...
   function addPool(uint _starttime , uint256 _endtime , uint _totalReward ) external onlyOwner {
    ...
    function addBatchTokenId( address _nftContractAddress , uint256[] memory _batchTokenIds) external onlyOwner{
        ...
        for (uint256 i = 0; i < _batchTokenIds.length; i++) {
            tokeninfo[_nftContractAddress][_batchTokenIds[i]]
            .tokenId = _batchTokenIds[i];
            tokeninfo[_nftContractAddress][_batchTokenIds[i]]
    ...
    function stakeNFT(uint256 _poolId , address _nftContractAddress  ,uint256 _tokenId, uint _rType ) public {
        ...
        staker[msg.sender][_nftContractAddress][_tokenId].push(
            Staker
            (
            _poolId,
            _nftContractAddress,
    ...
    function _checkIsUserStaked(address _nftContractAddress  ,uint256 _tokenId)  internal view returns (bool){
        Staker[] storage stakedata = staker[msg.sender][_nftContractAddress][_tokenId];
        require(stakedata.length > 0 , "Token Doesn't Exist");
        return true;
    } 
