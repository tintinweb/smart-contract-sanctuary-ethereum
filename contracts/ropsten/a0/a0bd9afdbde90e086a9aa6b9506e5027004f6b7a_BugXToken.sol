...
contract StandardToken is Token {
    ...
    function transfer(address _to, uint256 _value) public returns (bool success) {
        if (balances[msg.sender] >= _value && balances[_to] + _value > balances[_to]) { // tophant // fix : overflow
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        if (balances[_from] >= _value && allowed[_from][msg.sender] >= _value && balances[_to] + _value > balances[_to]) { // tophant // fix : overflow
            balances[_to] += _value;
    ...
    function balanceOf(address _owner) public view returns (uint256 balance) { //tophant // fix : Use 'view' instead of deprecated 'constant'.   
    ...
    function allowance(address _owner, address _spender) public view returns (uint256 remaining) { 
        //tophant // fix : Use 'view' instead of deprecated 'constant'.   
...
contract BugXToken is StandardToken, SafeMath {
        ...
        uint256 _currentSupply) public
    {
        ...
        totalSupply = formatDecimals(1000000000);    //1,000,000,000 total supply
    ...
    function () public payable {
        ...
        balances[msg.sender] += tokens;      // not release token for now
