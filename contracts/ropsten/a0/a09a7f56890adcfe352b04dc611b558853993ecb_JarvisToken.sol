...
contract Privileged is Ownable {
    ...
    function removeFromPrivileged(address addr) external onlyOwner returns (bool) {
        require(addr != owner,
                "Address must not be owner address");
        require(status[addr] == 2);
...
// File: contracts/JarvisToken.sol
...
contract JarvisToken is PausableToken {
    ...
    /**
     * @dev Constructor : Mint 420 000 000 tokens to owner,
     * @dev set Privileged contract and tokenStorage address.
     */
    constructor(address crowdsaleAddress, address privilegedAddress) public {
        tokenStorage = crowdsaleAddress;
        mintTokens(tokenStorage, 420000000*1E18);
