...
event Output(int qn_id,string question,int option_id,string option,int point);
// Event to emit when quiz is completed
event FinalOutput(string user,int total);
...
function addchoices(
  ...
  Choice memory e
    =Choice(qn_id,
        ...
        user_address);
  choices.push(e);
  emit Output(qn_id,question,option_id,option,point);
}

// Function to add all the results to the blockchain. Arguments based on Result structure
function addresult(string memory user,int point) public{
    Result[user]=point;
    emit FinalOutput(user,point);
}
