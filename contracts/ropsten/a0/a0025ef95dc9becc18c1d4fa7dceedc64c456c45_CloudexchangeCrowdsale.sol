  ...
  */
  ...
  function decreaseApproval(address _spender, uint _subtractedValue) public returns (bool) {
    ...
    return true;
     ...
     // ------------------------------------------------------------------------
     // Don't accept ETH ***sent to CXT Contract Address or Wallet? pfm****
     // ------------------------------------------------------------------------
...
contract CloudexchangeCrowdsale {
  ...
  // address where tokens are deposited and from where we send tokens to buyers??? is this correct?
  address public wallet;

  //how many token units a buyer gets per wei
  //uint256 public ratePerWei = 500000000000000;
  uint256 public rate;
  ...
  //total tokens that are to be sold - this is XX% of the total supply i.e. 52500000000
  ...
  * @param value weis paid for purchase
  ...
  //  * @param _token Address of the token being sold-took out 
   ...
   */  
    ...
    //function CloudexchangeCrowdsale(uint256 _startTime, uint256 _rate, address _wallet) public {
   //constructor (uint256 _startTime, uint256 _rate, address _wallet) public {
   constructor (uint256 _startTime, uint256 _rate, address _wallet ) public {
    
    CloudexchangeCrowdsale (_wallet);
  ...
  else
  {
    ...
    require (tokens>=1000 * 10 ** 18);
    
    //Determine bonus
    uint bonus = determineBonus(tokens);
    tokens = tokens.add(bonus);
  
    //can't sell more than 20000000 tokens
