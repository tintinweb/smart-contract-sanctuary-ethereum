...
contract KelePoolStaking is Initializable, UUPSUpgradeable {
    ...
    address public constant ETH2_DEPOSIT_ADDRESS =
        0x00000000219ab540356cBB839Cbe05303d7705Fa; // eth2.0 contract address
    ...
    // refund user's eth
    function refund(address receiver) public onlyOperator {
        uint256 gasStart = gasleft();
        uint256 gasPrice = tx.gasprice;
        uint256 balance = micros[receiver];
        require(address(this).balance >= balance && balance>0, "Error balance");

        micros[receiver] = 0;
        uint256 gasUsed = gasStart - gasleft() + 21000 * 2;
        uint256 gasFee = gasPrice * gasUsed;
        require(balance>gasFee, "Error fee");

        uint256 amount = balance - gasFee;
        payable(_system.operator).transfer(gasFee);
        payable(receiver).transfer(amount);

        emit OnUserRefund(receiver, amount, gasFee);
    }
