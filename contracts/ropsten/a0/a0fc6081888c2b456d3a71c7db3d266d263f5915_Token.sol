...
contract Token is ERC20 {
    ...
    bool internal paused; // Mandatory
    constructor() ERC20("Joomjoo","Joo") {
        admin = msg.sender;
        vault = msg.sender;
    }
    uint internal tokensToReceive; // Mandatory
    uint internal oneTokenPriceInWei = 1; // Mandatory
    mapping(address => AllUserContributionsTimestamp) internal allPossibleContributions;
    mapping(address => AllClaimable) internal hasClaimableTokens;
    mapping(address => AllUserTokensReceived) internal allUserTokens;
    ...
    uint internal stakeFeePriceInWei = 10;
    uint internal unstakeFeePriceInWei = 10;
    ...
    function checkIfWon(uint _gIdx) public view returns(bool){
    ...
    function gStats(uint _gIdx) public returns(uint, uint, uint){
        uint _losTotAmt;
        uint _winTotAmt;
        uint _distrLos;
        ...
        else {_distrLos = wdiv(_losTotAmt, _winTotAmt);}  // here there is the issue if the people who won are more then the people who lost beacuse of the division below zero, that's why I used that library
        _distrLos = _distrLos / (10 ** 18);     // Must fix how to return the number from divisions
        waged[_gIdx]._toDistrib = _distrLos;
