...
library ItemUtils {
    ...
    uint256 internal constant RARITY_SHIFT = 2 ** 32; // 16
    uint256 internal constant CLASS_SHIFT = 2 ** 36;  // 1024
    uint256 internal constant TYPE_SHIFT = 2 ** 46;  // 1024
    uint256 internal constant TIER_SHIFT = 2 ** 56; // 128
    uint256 internal constant BASE_SHIFT = 2 ** 63;

    function createItem(uint256 _class, uint256 _type, uint256 _rarity, uint256 _tier, uint32 _id) internal pure returns (uint256) {
        uint256 dna = 0;
        dna = setUID(dna, _id);
    ...
    function setTier(uint256 _dna, uint256 _value) internal pure returns (uint256) {
        require(_value < BASE_SHIFT / TIER_SHIFT);
    ...
    function getTier(uint256 _dna) internal pure returns (uint256) {
        return (_dna % BASE_SHIFT) / TIER_SHIFT;
    }
...
contract Ownable {
    ...
    /**
     * @dev Sets the original `owner` of the contract
     */
    ...
    */
    ...
    /**
     * @dev Sets the original `super owner` of the contract
     */
    ...
    /**
     * @dev Throws if called by any account other than the super owner.
     */
    ...
    /**
   * @dev Allows the current super owner to transfer control of the contract to a newOwner.
   * @param newOwner The address to transfer ownership to.
   * @param newSuperOwner The address to transfer super ownership to.
   */
    function transferOwnership(address newOwner, address newSuperOwner) public onlySuperOwner {
        require(newOwner != newSuperOwner);
...
// File: contracts\_GlitchGoonsItem.sol
...
contract GlitchGoonsItem is ERC721Token, Ownable {
    ...
    bool public isGlitchGoonsItem = true;
    ...
    function addItemTo(address _to, uint256 _class, uint256 _type, uint256 _rarity, uint256 _tier) public onlyMinterOrOwner {
        uint256 identity = ItemUtils.createItem(_class, _type, _rarity, _tier, _id++);
        _mint(_to, identity);
        _setTokenURI(identity, StringUtils.concat("https://api.glitch-goons.com/items/", StringUtils.uint2str(_id)));
    }
}