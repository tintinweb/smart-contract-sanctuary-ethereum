...
contract PoolToken is IPoolToken, TarotERC20 {
    ...
    function _update() internal {
        totalBalance = IERC20(underlying).balanceOf(address(this)); //이 어드래스의 토탈벨런스를 업데이트 한다고?
    ...
    function exchangeRate() public returns (uint256) {
        ...
        return _totalBalance.mul(1e18).div(_totalSupply); // 여기에 있는 양을 토탈 서플라이로 나눈 값
    }
    ...
    {
        //balance underlying 
        uint256 balance = IERC20(underlying).balanceOf(address(this)); //이 스마트 컨트랙트가 가지고 있는 기존(Eth) 토큰의 양
        uint256 mintAmount = balance.sub(totalBalance); //현재 가지고 있는 것에서 - 기존 토큰의 양
