...
contract UrbitexExchangeV7 is Context, Ownable {
        ...
        payable
    {
        ...
        require((uint96(msg.value) == price && token == address(0)) || (_amount == price && _token == token && token != address(0)));
    ...
    function addListing(uint32 _point, uint96 _price, address _token) external
    {
        ...
        // add the price of the point, the seller address to the registry
        // optional ERC-20 token can be set, if a zero address then it's Ether
        //         
