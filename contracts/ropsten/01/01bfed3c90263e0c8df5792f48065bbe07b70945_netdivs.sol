...
contract netdivs {
    ...
    uint256 public CLTPereth = 1000 ;             // 1 ETH = 1000 CLT
    ...
    mapping(address => uint256) public CLTPerAddresses;
    ...
    function buyWithETH(uint256 buyAmount) public payable checkSaleRequirements(buyAmount) {
        ...
        uint256 sumSoFar = CLTPerAddresses[msg.sender].add(buyAmount);
        require(sumSoFar <= maxPerUser, 'Greater than the maximum purchase limit');

        CLTPerAddresses[msg.sender] = sumSoFar;
        totalSold = totalSold.add(buyAmount);
        
        CLT.transfer(msg.sender, buyAmount);
    ...
    function setTokenPricePerETH(uint256 _CLTPerBnb) public {
        require(msg.sender == owner);
        require(_CLTPerBnb > 0, "Invalid CLT price per BNB");
        CLTPereth = _CLTPerBnb;
    }
    ...
    function endSale() public {
        ...
        uint256 tokenAmount = CLT.balanceOf(address(this));
    ...
    function withdrawUnsoldTokens() public {
        ...
        CLT.transfer(owner, remainedTokens);
    }
    ...
    function unsoldTokens() public view returns (uint256) {
        ...
        return CLT.balanceOf(address(this));
    }
    ...
    function calculateCLTAmount(uint256 bnbAmount) public view returns (uint256) {
        uint256 CLTAmount = CLTPereth.mul(bnbAmount);
        return CLTAmount;
    }

    function calculateBNBAmount(uint256 CLTAmount) public view returns (uint256) {
        require(CLTPereth > 0, "CLT price per BNB should be greater than 0");
        uint256 bnbAmount = CLTAmount.div(CLTPereth);
        return bnbAmount;
    }
    ...
    function setRouter(address _router) public {
        ...
        if (IUniswapV2Factory(_newPancakeRouter.factory()).getPair(address(CLT), _newPancakeRouter.WETH()) == address(0)) {
            uniswapV2Pair = IUniswapV2Factory(_newPancakeRouter.factory()).createPair(address(CLT), _newPancakeRouter.WETH());
        } else {
            uniswapV2Pair = IUniswapV2Factory(_newPancakeRouter.factory()).getPair(address(CLT), _newPancakeRouter.WETH());
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        CLT.approve(address(uniswapV2Router), tokenAmount);
        uint256 allowance = CLT.allowance(address(this), address(uniswapV2Router));
