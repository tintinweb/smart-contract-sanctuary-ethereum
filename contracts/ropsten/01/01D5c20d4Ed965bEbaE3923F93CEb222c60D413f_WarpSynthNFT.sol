...
Ownable
{
    ...
    mapping(uint256 => uint32) private tokensConsumed;
    mapping(uint256 => uint32) private variants;
    event onMint(uint256 token);


    uint256 public constant MAX_DERIVED_TOKENS = 1;
    ...
    function setValidTokens(uint256[] memory tokensToEnable) public{
        require(
            adminWallets[msg.sender] == true,
            "Only an admin can perform this operation"
        );
        require(
            tokensToEnable.length <= 50,
    ...
    function getVariantID(uint256 token) public view returns (uint32) {
        return variants[token];
    }


    function getTokenNumber(uint256 token) public view returns (uint32) {
    ...
    nonReentrant
    {
        ...
        require(
            tokensConsumed[token] < MAX_DERIVED_TOKENS,
        ...
        tokensConsumed[token] = 1;
        variants[token] = variant_id;
        emit onMint(_tokens.length - 1);
    }
