...
abstract contract Swap is Storage, UserWallet, EIP712Upgradeable {
  ...
  bytes32 public constant _SWAP_TYPEHASH =
   keccak256("Swap(address user, bytes32 swapGateway, address tokenFrom, uint256 amountFrom, address tokenTo, uint256 minAmountTo, uint256 nonce, uint256 deadline)");
  ...
  event SwapPerformed(string swapId, string swapGateway, address indexed user,
  ...
  function executeSwapWithSignature(
    ...
    { // avoid stack too deep for postSwap
    SwapConstraints memory swapConstraints = buildSwapConstraints(
      user,
      tokenFrom,
      amountFrom,
      tokenTo,
    ...
    verifySwapSignature(swapConstraints, deadline, signature);
    }

    uint256 receivedTokens = performSwap(user, tokenFrom, tokenTo, amountFrom, minAmountTo, data);
    ...
    postSwap(swapGateway, receivedTokens, tokenTo, user);
  }
  ...
  function executeSwapWithSelfLiquidity(
    ...
    performSwap(user, tokenFrom, tokenTo, amountFrom, minAmountTo, data);
  }
  ...
  function performSwap(
    ...
    uint256 tokenFromBalance = IERC20Upgradeable(tokenFrom).balanceOf(address(this));
    uint256 tokenToBalance = IERC20Upgradeable(tokenTo).balanceOf(address(this));
    ...
    tokenFromBalance = tokenFromBalance - IERC20Upgradeable(tokenFrom).balanceOf(address(this));
    tokenToBalance = IERC20Upgradeable(tokenTo).balanceOf(address(this)) - tokenToBalance;

    require(tokenFromBalance <= amountFrom, "HIGHER_THAN_AMOUNT_FROM");
    require(tokenToBalance >= minAmountTo, "LOWER_THAN_MIN_AMOUNT_TO");

    _decreaseBalance(tokenFrom, user, tokenFromBalance);
    _increaseBalance(tokenTo, user, tokenToBalance);

    // Ensure any amount not spent is disallowed
    IERC20Upgradeable(tokenFrom).safeApprove(paraswapTransferProxy, 0);

    return tokenToBalance;
  }
  ...
  function verifySwapSignature(
    ...
    structHash = _hashTypedDataV4(structHash);

    address signer = ECDSAUpgradeable.recover(structHash, signature);
    require(signer == swapConstraints.user, "INVALID_SIGNAURE");
    userNonces[swapConstraints.user] = swapConstraints.nonce;
  }

  function buildSwapConstraints(
    address user,
    address tokenFrom,
    uint256 amountFrom,
    address tokenTo,
    ...
    uint256 nonce,
    bytes32 swapGateway
  ) private pure returns (SwapConstraints memory swapConstraints) {
...
abstract contract UserWallet is Storage, DvfAccessControl {
  ...
  /**
   * @dev Withdraw funds directly from this contract
   */
  function withdraw(address _token, uint256 amount) external {
    _ensureUserBalance(msg.sender, _token, amount);

    IERC20Upgradeable token = IERC20Upgradeable(_token);

    token.safeTransfer(msg.sender, amount);

    _decreaseBalance(_token, msg.sender, amount);
  }
