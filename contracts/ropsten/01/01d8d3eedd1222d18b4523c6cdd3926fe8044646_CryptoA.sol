...
contract CryptoA is ERC721A, Ownable, RoyaltiesV2Impl{
    using Strings for uint256;

    uint256 public constant MINT_PRICE = .0005 ether;
    string private  baseTokenUri;
    bool public pause;

    constructor() ERC721A("CryptoA", "CAA"){

    }

    modifier denyContract() {
        require(tx.origin == msg.sender, "CryptoA :: Call from contract is not allowed.");
        _;
    }

    function mint(uint256 _quantity) external payable denyContract{
        require(msg.value >= (MINT_PRICE * _quantity), "CryptoA :: Not enough funds sent.");
        require(!pause, "CryptoA :: Minting not allowed at the moment.");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 storageFolderId = tokenId + 1;

        return bytes(baseTokenUri).length > 0 ? string(abi.encodePacked(baseTokenUri, storageFolderId.toString(), ".json")) : "";
    }
