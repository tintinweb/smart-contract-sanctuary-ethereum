...
contract AllowlistPolicy is CNSAccessControl {
    ...
    function addAllowlistWithOutUseThis(string memory _domain, address _account)
        public
        onlyDomainOwner(_domain, msg.sender)
    {
        require(_account != address(0), "Account isn't valid");
    ...
    function addAllowlistWithOutOnlyDomainOwner(
        string memory _domain,
        address _account
    ) public isUseThisPolicy(_domain, address(this)) {
        require(_account != address(0), "Account isn't valid");
        _safeAddAllowlist(_domain, _account);
    }

    function addAllowlistWithOutAll(string memory _domain, address _account)
        public
    {
        require(_account != address(0), "Account isn't valid");
        _safeAddAllowlist(_domain, _account);
