...
contract EcosystemProxy is Initializable, OwnableUpgradeable {
    ...
    function setDistributionSystem(uint[] calldata _distributionDates, uint[] calldata _distributionAmounts) public onlyOwner {
        require(_distributionDates.length != 0, "(initialize) zero number of distribution dates");
        require(_distributionDates.length == _distributionAmounts.length, 
            "(initialize) number of distribution dates and amounts is not the same");
        require(_sumArray(_distributionAmounts) <= token.balanceOf(address(this)), 
            "(initialize) insufficient contract balance");
    ...
    function withdrawAdditionally(uint fromPortionId, uint amount) public onlyOwner {
        ...
        withdrawnOutsideSchedule.add(amount);
    ...
    function getWithdrawingProcess() public view returns(bool[] memory) {
