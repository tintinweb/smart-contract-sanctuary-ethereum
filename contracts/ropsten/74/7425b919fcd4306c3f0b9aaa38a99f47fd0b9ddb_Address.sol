...
contract CharactorByERC721 is ERC721{
    struct Charactor {
        ...
        string ImageHash;   // 이미지 파일의 해시
        string ImageUrl;    // 이미지 파일의 주소        
    }
    ...
    // 캐릭터 생성
    function mint(string memory Name, uint256 Level, string memory ImageHash, string memory ImageUrl, address account) public isOwner {
        uint256 cardId = charactors.length; // 유일한 캐릭터 ID
        charactors.push(Charactor(Name, Level, ImageHash, ImageUrl));
