...
import './FixedPriceMarketPlace.sol';
import './AuctionMarketPlace.sol';
contract NFTation_Storage {
    using CountersUpgradeable for CountersUpgradeable.Counter;

    CountersUpgradeable.Counter internal _tokenIdCounter;
    mapping(string => bool) internal tokenURIsMapping;
...
contract AuctionMarketPlace is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function acceptBid(uint256 _tokenId, address _winner) external {
        ...
        if(royaltyAmount > 0){
            require(_ERC20.transferFrom(_winner, tokenCreator, royaltyAmount));
        }
        if(NFTationContract.checkFirstSale(_tokenId)){
            NFTationContract.disableFirstSale(_tokenId);
        }
        uint256 remaining = (price.sub(marketPlaceShareAmount)).sub(royaltyAmount);
        require(_ERC20.transferFrom(_winner, _auction.seller, remaining));
