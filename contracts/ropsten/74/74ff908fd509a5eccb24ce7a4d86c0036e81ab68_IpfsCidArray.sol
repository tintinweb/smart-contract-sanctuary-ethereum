...
contract IpfsCidArray is Ownable {
    string[] cidArray;

    function add(string memory x) public onlyOwner {
        cidArray.push(x);
    }

    function show() public view returns (uint256, string memory) {
        string memory str;
        for (uint256 j = 0; j < cidArray.length; j++) {
            str = string(abi.encodePacked(str, cidArray[j]));
        }

        return (cidArray.length, str);
    }
}