...
contract LEARNtoken is IERC20, owned {
   ...
   constructor(uint256 total) {  
    ...
    function buylearn(uint buyqty) external {
        uint usdcamount = ((buyqty * (10 ** 18)) / 100);
    ...
    function redeemlearn(uint256 redeemqty) external {
        uint dairebate = ((redeemqty * (10 ** 18)) * 100);
        ...
        fakeusdctoken.transfer(msg.sender, dairebate);
        transfer(address(this), redeemqty);
        emit Redemption(redeemqty);
    }

    function giverebate(uint256 giveusdc) public {
        require(giveusdc <= fakeusdctoken.balanceOf(address(msg.sender)),
        "not enough USDC");
        uint fakeusdcBalance = fakeusdctoken.balanceOf(address(msg.sender));
