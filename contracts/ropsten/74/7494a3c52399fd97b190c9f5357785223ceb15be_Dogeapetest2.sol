...
pragma solidity ^0.8.0;
    ...
    contract Dogeapetest2 is Context, IERC20, Ownable {
    ...
    string private _name = "Dogeapetest2";

    string private _symbol = "DApetest2";
    ...
    uint256 public marketingFeePercent = 240;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(!takeFee)
         ...
         uint256 marAmt = amount.mul(marketingFeePercent).div(100);



        if (_isExcluded[sender] && !_isExcluded[recipient]) {

            _transferFromExcluded(sender, recipient, (amount.sub(marAmt)));

        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {

            _transferToExcluded(sender, recipient, (amount.sub(marAmt)));

        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {

            _transferStandard(sender, recipient, (amount.sub(marAmt)));

        } else if (_isExcluded[sender] && _isExcluded[recipient]) {

            _transferBothExcluded(sender, recipient, (amount.sub(marAmt)));
        ...
        _transferStandard(sender, marketingAddress, marAmt);
