    ...
    Whitelist
{
    ...
    /**
     * @dev - Sets new vault address
     * @param _newVaultAddress - Address of the new vault contract
     */
    ...
    function setBloomNodes(address _newBloomNodes) external onlyOwner {
        require(_newBloomNodes != address(0), "Address 0");
    ...
    function setBloomReferral(address _newBloomReferral) external onlyOwner {
        require(_newBloomReferral != address(0), "Address 0");
    ...
    function transfer(address _to, uint256 _amount)
        ...
        returns (bool)
    {
        require(_to != address(0), "Nectar: transfer to the zero address");
        if (_to != bloomNodes) {
        ...
        super._burn(_msgSender(), taxAmount);
    ...
    function swapUsdcForToken(uint256 amountIn, uint256 amountOutMin) external {
        ...
        liquidityManager.swapUsdcForToken(_msgSender(), amountIn, amountOutMin);
        _setSwapAllowed(_msgSender(), false);
    }

    function swapTokenForUsdc(uint256 amountIn, uint256 amountOutMin) external {
        _setSwapAllowed(_msgSender(), true);
        liquidityManager.swapTokenForUsdc(_msgSender(), amountIn, amountOutMin);
        _setSwapAllowed(_msgSender(), false);
    }
...
contract Whitelist is IWhitelist, Initializable, OwnableUpgradeable {
    ...
    function __Whitelist_init() external initializer {
        __Ownable_init();
    }
...
abstract contract LiquidityPoolManagerImplementationPointerUpgradeable is
    OwnableUpgradeable
{
