...
contract Alimatok is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public matokFreeMaxSupply = 6;
    uint256 public freeMaxMintPerTx = 2;
    uint256 public matokMaxSupply = 20; //<-- Overall supply
    uint256 public maxMintPerTx = 4;
    uint256 public constant reservedForTeam = 4;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(_mintAmount > 0,  "Invalid, must mint atleast one token.");

        if (matokFreeMaxSupply + reservedForTeam > totalSupply()){
            require(_mintAmount <= freeMaxMintPerTx && freeMinted[msg.sender] + _mintAmount <= freeMaxMintPerTx, "Transaction limit reached.");
            require(totalSupply() + _mintAmount <= matokFreeMaxSupply + reservedForTeam, "Not enough free mint supply.");
            freeMinted[msg.sender] += _mintAmount;
        } else {
            require(_mintAmount <= maxMintPerTx && minted[msg.sender] + _mintAmount <= maxMintPerTx, "Transaction limit reached.");
