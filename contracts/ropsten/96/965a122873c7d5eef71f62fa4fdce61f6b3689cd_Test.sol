...
contract Test is ERC721Enumerable, Ownable {
  ...
  function mint(uint256 _mintQuantity) public payable {
    ...
    for (uint256 i = 1; i <= _mintQuantity; i++) {
      uint256 RandomIndex  = _generateRandom(preID);
      uint256 ID = UnMinted[RandomIndex];
      _safeMint(msg.sender, ID);
      UnMinted[RandomIndex] = UnMinted[(UnMinted.length - 1)];
      UnMinted.pop();
      preID = ID;
    }
  }
  ...
  function CreateUnMintedArray() public onlyOwner returns(uint16){
    require(ArrayCreated == false);
    uint16 index = 0;
    uint16 idincrement = 1;
    while(index < maxSupply){
