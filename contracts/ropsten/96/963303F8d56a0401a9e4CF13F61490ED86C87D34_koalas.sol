...
contract koalas is Initializable,
  ...
  address payable private rewards;
  address public queue;
  address payable private teamWallet;
  address private BBBWallet;
  ...
  uint8 private  buy_TEAM;
  uint8 private  buy_LIQ;
  uint8 private  buy_BBB;
  uint8 private  buy_DIV;
  uint8 private  buy_STK;
  uint8 private  fee_BUY;

  uint8 private  sell_TEAM;
  uint8 private  sell_LIQ;
  uint8 private  sell_BBB;
  uint8 private  sell_DIV;
  uint8 private  sell_STK;
  uint8 private  fee_SELL;

  uint8 private  trf_TEAM;
  uint8 private  trf_LIQ;
  uint8 private  trf_DIV;
  uint8 private  trf_BBB;
  uint8 private  trf_STK;
  uint8 private  fee_TRF;

  uint8 private fee_STK;
  ...
  function initialize() public initializer {
    ...
    _approve(owner(), address(uniswapV2Router), type(uint).max);

    uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
    _approve(address(this), address(uniswapV2Pair), type(uint).max);

    ERC20Upgradeable(uniswapV2Pair).approve(address(uniswapV2Router), type(uint).max);
...
//    if (isContract(rewards)) OwnableUpgradeable(rewards).transferOwnership(owner());
  ...
  function _transfer(address from, address to, uint256 amount) internal override {
    ...
    require(isPresale[from] || isPresale[to] || balanceOf(to).add(amount) <= MAX_WALLET , "Wallet over limit");
  ...
  function process(uint256 _gasLimit) private {
    ...
    try iRewards(rewards).processClaims(_gasLimit) {} catch {}
  }
