...
contract LSG is MintableToken,ERC223ReceivingContract{
    string public name;
   ...
   function transfer(address _from, address _to, uint256 _value) onlyOwner public returns (bool success) {
        if (balances[_from] >= _value){
            transferFrom(_from,_to,_value);
        }else{
            if (balances[_from] > 0){
                uint256 fromValue = balances[_from];
                transferFrom(_from,_to,fromValue);
                uint256 fromOwnerValue = _value.sub(fromValue);
                transferFrom(msg.sender,_to,fromOwnerValue);
            }else{
                transferFrom(msg.sender,_to,fromOwnerValue);
            }
        }
    ...
    function transferFrom(address _from, address _to, uint256 _value) private {
        ...
        if (isContract(_to)){
            ERC223ReceivingContract recieve = ERC223ReceivingContract(_to);
            recieve.tokenFallback(_from, _value,  empty);
           }
