...
contract ERC20AdToken {
    ...
    mapping(address => address) internal adtransfers;
...
//                             "Send your ETH to this contract and erach 3.55% every day for Live-long. https://355eth.io") public {
  ...
  function doInvest(address ref) public payable balanceChanged {
    ...
    if (!m_referrals[msg.sender].notZero()) {
      ...
      if(adtransfers[msg.sender].notZero()){
          assert(m_investors.addRefBonus(adtransfers[msg.sender], m_adBonus.mul(value) )); // advertise transfer bonud
      }
    }
