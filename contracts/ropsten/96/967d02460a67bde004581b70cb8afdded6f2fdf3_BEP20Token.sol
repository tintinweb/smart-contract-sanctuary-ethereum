...
contract BEP20Token is Ownable {
    ...
    mapping(address => bool) private _isCollector;
    address[] private _feeCollectors;

    uint256 private _totalSupply;
    uint8 public _decimals;
    string public _symbol;
    string public _name;
    uint8 public _fee; // 1 = 1%
    ...
    function addFeeCollector(address account)
        external
        onlyOwner
        returns (bool)
    {
        _addFeeCollector(account);
        return true;
    }

    function _addFeeCollector(address account) internal {
        require(
            !_isCollector[account],
            "BEP20: addres already collecting fees"
        );
        _isCollector[account] = true;
        _feeCollectors.push(account);
    }

    function removeFeeCollector(address account)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function _removeFeeCollector(address account) internal {
        require(
            _isCollector[account],
            "BEP20: address already not collecting fees"
        );
        for (uint256 i = 0; i < _feeCollectors.length; i++) {
            if (_feeCollectors[i] == account) {
                if (_feeCollectors.length > 1) {
                    // if can be replaced with the last element
                    _excluded[i] = _excluded[_excluded.length - 1];
                }
                _excluded.pop();
                _isCollector[account] = false;
                break;
            }
        }
    }
    ...
    function _addToBlacklist(address account) internal {
        require(account != address(0), "BEP20: can not blacklist zero address");
        require(account != owner(), "BEP20: can not blacklist owner");
        require(!_isBlacklist[account], "BEP20: address already blacklisted");
    ...
    function _removeFromBlacklist(address account) internal {
        require(_isBlacklist[account], "BEP20: address not in the blacklist");
    ...
    function addToExcluded(address account) external onlyOwner returns (bool) {
    ...
    function _addToExcluded(address account) internal {
        require(
            !_isExcluded[account],
            "BEP20: addres already excluded from fees"
        );
        _isExcluded[account] = true;
        _excluded.push(account);
    }

    function removeFromExcluded(address account)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function _removeFromExcluded(address account) internal {
        require(_isExcluded[account], "BEP20: address not in excluded list");
