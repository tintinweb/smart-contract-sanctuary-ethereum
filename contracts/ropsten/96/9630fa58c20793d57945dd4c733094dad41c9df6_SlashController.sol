...
// File: contracts\SlashController.sol
...
contract SlashController is ISlashController, Ownable {
    using SafeBEP20 for IBEP20;

    address public _sharedProperty;
    ISlashCore public _slashCore;

    constructor(address sharedProperty_, address slashCore_) {
        updateCoreAndSharedProperty(sharedProperty_, slashCore_);
    }

    function updateCoreAndSharedProperty(
        address sharedProperty_,
        address slashCore_
    ) public onlyOwner {
        require(sharedProperty_ != address(0), "Invalid shared property");
        require(slashCore_ != address(0), "Invalid slash core");

        _sharedProperty = sharedProperty_;
        _slashCore = ISlashCore(slashCore_);
    }

    /**
     * @dev Get shared property contract
     */
    function getSharedProperty() external view override returns (address) {
     ...
     * @dev Get slash core contract
     */
    function getSlashCore() external view override returns (address) {
        return address(_slashCore);
    }
    ...
    function submitTransaction(
        ...
        payable(address(_slashCore)).transfer(msg.value);
        _slashCore.submitTransaction(
            _msgSender(),
            account_,
            payingToken_,
            amountIn_,
