...
contract StockOptions is ReentrancyGuardUpgradeable {
  ...
  mapping(uint256 => mapping(address => Participants.Vesting[]))
    private vestings;
  ...
  using Participants for Participants.Vesting;
   ...
   * @param intervals Vesting intervals
   * @param uri Certificate file IDs
   */
  ...
  function terminateParticipant(
    ...
    for (uint256 i; i < vestings[tokenPoolId][_address].length; i++) {
      uint256 vestingTokensToClaim;
      uint256 vestingTokensReverted;

      (vestingTokensToClaim, vestingTokensReverted) = _terminateVesting(
        tokenPoolId,
        i,
        _address,
        terminationDate
      ...
      tokensToClaim = tokensToClaim.add(vestingTokensToClaim);
      tokensReverted = tokensReverted.add(vestingTokensReverted);
    }
  ...
  function terminateVesting(
    ...
    _terminateVesting(tokenPoolId, vestingId, _address, terminationDate);
  }
  ...
  function claimTokens(
    ...
    _claimTokens(tokenPoolId, vestingId, msg.sender, block.timestamp, amount);
    setParticipantCertificate(tokenPoolId, vestingId, msg.sender, _uri);
  }
...
library Participants {
  ...
  function getClaimableTokens(
    Participants.Vesting storage self,
    uint256 claimDate
  ...
  function getIntervals(Participants.Vesting storage self)
    internal
    view
  ...
  function canClaim(Participants.Vesting storage self, uint256 atDate)
    internal
    view
    returns (bool)
  {
  ...
  function pause(Participants.Vesting storage self, uint256 pauseDate)
    internal
  {
  ...
  function unPause(Participants.Vesting storage self, uint256 unpauseDate)
    internal
  {
