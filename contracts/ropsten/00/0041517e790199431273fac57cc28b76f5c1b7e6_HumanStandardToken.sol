...
contract HumanStandardToken is StandardToken { 
    ...
    uint public currentAirDropTotalSupply = 0;    // 已经空投数量
    uint public totalAirDropSupply = 2;   //总的空投数量 
    uint public airDropNum = 1;      // 单个账户空投数量
    
    // 存储是否空投过
    mapping(address => bool) touched;
    mapping (string => uint) currentAirDropTotalSupplys;

    function HumanStandardToken(uint256 _initialAmount, string _tokenName, uint8 _decimalUnits, string _tokenSymbol) {
        
        totalSupply = _initialAmount * 10 ** uint256(_decimalUnits);         // 设置初始总量  
        balances[msg.sender] = totalSupply - totalAirDropSupply * 10 ** uint256(_decimalUnits); // 初始token数量给予消息发送者，因为是构造函数，所以这里也是合约的创建者  
