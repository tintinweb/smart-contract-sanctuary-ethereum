...
contract Asstroverse is ERC20, Ownable {
    ...
    address public busd = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F; // testnet usdc ropsten

    address public assteroid;

    uint256 public amountToStart = 200 * (10**6);
    ...
    modifier ownerOrAssteroid() {
        require(msg.sender == owner() || msg.sender == assteroid, 'cannot do that');
        _;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            // gets set too many times
            // if (IERC20(busd).balanceOf(assteroid) >= amountToStart) {
            //     IAssteroid(assteroid).setStartTime(block.timestamp);
            // }
        }
    ...
    function changeAmountToStart(uint256 amount) external onlyOwner {
        // amount in USD
        amountToStart = amount * (10**(IERC20(busd).decimals()));
    }
