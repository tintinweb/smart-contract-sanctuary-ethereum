...
contract SpaceCash is ERC20Capped, Ownable {
    ...
    function setFee(bool f) public onlyOwner{
        isFee = f;
    }
    ...
    function applyFee(address sender, address recipient, uint256 amount) internal returns(uint256) {
        ...
        if(sender != address(this) && sender != owner())
        {
            if(isLpAddress[recipient]) {    // Sell
                uint256 burnAmount = amount * sellFee / div;
                amount = amount - burnAmount;
                _burn(sender, burnAmount);
            } else if(isLpAddress[sender]) {    //Buy
                if(recipient != owner()) {
                    uint256 manageAmount = amount * manageFee / div;
                    uint256 marketAmount;
                    uint256 referrerAmount;
                    uint256 devAmount = amount * devFee / div;
                ...
                uint256 burnAmount = amount * taxFee / div;
                amount = amount - burnAmount;
                _burn(sender, burnAmount);
            }
        }
    ...
    function setDividor(uint16 d) public onlyOwner {
        div = d;
    }
}
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     *
     * The default value of {decimals} is 2. To select a different value for
     ...
     *
     * Tokens usually opt for a value of 2, imitating the relationship between
