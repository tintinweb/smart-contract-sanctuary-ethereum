...
contract PokerGene is ERC721, Ownable {
    ...
    // We substract 1 from it before comparing
    ...
    function publicMint(uint256 _count) external payable {
        ...
        require(_totalSupply.add(_count) - 1 < MAX_SUPPLY, "not enough supply");
        require(msg.sender == tx.origin, "no bots");
        require(
            balanceOf(msg.sender).add(_count) < MAX_PER_WALLET,
            "too many per wallet"
        );
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        uint256 _totalSupply = totalSupply;
        require(
            _totalSupply.add(receivers.length) - 1 < MAX_SUPPLY,
            "not enough supply"
        );
