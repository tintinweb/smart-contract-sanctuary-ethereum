...
contract Library is Ownable {
    ...
    function borrowBook(uint id) external payable {
        ...
        // Approve that the Library can spend 
        // 0.1 LIB of the sender's LIB balance
        LibToken.approve(address(this), borrowCost);
        // Transfer the amount from the sender to the Library
    ...
    function convertAndWithdraw() public payable onlyOwner {
     // Approve that the owner can spend 
        // the whole LIB balance of the Library
        LibToken.approve(msg.sender, LibToken.balanceOf(address(this)));
        // Transfer the whole balance of Library
        // from Library to msg.sender (the owner)
        LibToken.transferFrom(address(this), msg.sender, LibToken.balanceOf(address(this)));
    }
