...
interface ISynchron{
    ...
    function addOption(bytes memory info) external  returns(uint256 optionId);

    function updateOption(uint256 optionId,bytes memory info) external;

    function updateOptionWithProperty(uint256 optionId,bytes memory info) external;

    function addBidding(uint256 optionId,bytes memory offer) external returns(uint256 offerId);

    function cancelBiding(uint256 offerId,bytes memory offer) external;

    function submitUserInfo(uint256 iden,address customer,uint256 id) external;
    
    function getBiddingForOption(uint256 optionId) external  view returns(uint256[] memory offers);
    ...
    function safeTransferERC20(address token,address sender,address creator,address feeTo,address royal,uint256 fee,uint256 toReward,uint256 toRoyal) external;
}
...
contract Synchron is ISynchron,ERC1155Holder,ERC721Holder{
    ...
    function addOption(bytes memory info) external override onlyOperator returns(uint256 optionId){
    ...
    function updateOption(uint256 optionId,bytes memory info) external override onlyOperator{
        optionInfo[optionId] = info;
    }

    function updateOptionWithProperty(uint256 optionId,bytes memory info) external override onlyOperator{   
    ...
    function addBidding(uint256 optionId,bytes memory offer) external override onlyOperator returns(uint256 offerId){
    ...
    function submitUserInfo(uint256 iden,address customer,uint256 id) external override onlyOperator{
        User storage user = userInfo[customer];
        //0-sale 1-purchase 2-offer
        if(iden == 0) user.saleOptionIds.push(id);
        if(iden == 1) user.purchaseOptionIds.push(id);
        if(iden == 2) user.offerIds.push(id);
    }

    function getBiddingForOption(uint256 optionId) external override view returns(uint256[] memory offers){
    ...
    function safeTransferERC20(address token,address sender,address creator,address feeTo,address royal,uint256 fee,uint256 toReward,uint256 toRoyal) external override onlyOperator{
        TransferHelper.safeTransferFrom(token, sender, creator, toReward);
        TransferHelper.safeTransferFrom(token, sender, feeTo, fee);
        if(royal != address(0) && toRoyal > 0){
            TransferHelper.safeTransferFrom(token, sender, royal, toRoyal);
        }
    }
...
interface IStructure{
    ...
    //[]
    //"0","0xA47afC854e418cB42f5A0ee5150868A0FCf7F256","0x3024a5c0870dde2b65ddDd1BFC139f94941EDCAC","3","1","1000000","7","0"
...
interface ITrading is IStructure{
    ...
    function createOption(Option calldata option) external returns(uint256 optionId);
...
contract Trading is ITrading{
    ...
    function getPayment(uint256 optionId) public override view returns(uint256 payment,uint256 expect){
        ...
        if(option.tradingType == TradingType.trading){
            ...
            uint256 offerId = getEffectiveOffer(optionId);
            if(offerId == 0){
                payment = option.amount.mul(option.single);
            }
            if(offerId > 0){
                ...
                payment = offer.price.add(offer.price.mul(5).div(100));
            }
    ...
    function getBiddingStatus(uint256 optionId,address customer,uint256 price) public override view returns(bool state){
        ...
        if(option.expiration > block.timestamp && customer != option.creator && option.tradingType != TradingType.trading && option.state == State.solding){
            (uint256 currentPrice,) = getPayment(optionId);
    ...
    function createOption(Option calldata option) public override returns(uint256 optionId){
        bool isBatch = IERC165(option.token).supportsInterface(0xd9b67a26);
        if(option.tradingType == TradingType.auction && isBatch == true && isSupport1155 == false) revert("Trading:Erc1155 protocol is not supported to publish auction orders");
        ...
        optionId = ISynchron(synchron).addOption(abi.encode(info));
        ISynchron(synchron).submitUserInfo(0, msg.sender, optionId);

        emit Created(option.tradingType, option.creator, optionId, block.timestamp);
    }
    //function permit(address sender,address operator,bool approved,uint deadline, uint8 v, bytes32 r, bytes32 s) external

    function createOptionPermit(Option calldata option,uint deadline,uint8 v, bytes32 r, bytes32 s) external returns(uint256 optionId){
        IERC1155(option.token).permit(msg.sender, address(this), true, deadline, v, r, s);
    ...
    function modify(uint256 optionId,uint256 price) external override{
        ...
        option.single = price;
        ISynchron(synchron).updateOption(optionId, abi.encode(option));
    ...
    function purchase(uint256 optionId) external override payable{
        ...
        ISynchron(synchron).updateOptionWithProperty(optionId, abi.encode(option));
        ISynchron(synchron).submitUserInfo(1, msg.sender, optionId);
    ...
    function bidding(uint256 optionId,uint256 price) external override{
        ...
        uint256 offerId = ISynchron(synchron).addBidding(optionId, abi.encode(offer));
        ISynchron(synchron).submitUserInfo(2, msg.sender, offerId);
        bytes memory optionInfo = ISynchron(synchron).optionInfo(optionId);
        StandardOption memory option = IGlibrary(glibrary).analyseOption(optionInfo);
    ...
    function delivery(uint256 optionId) external override{
        ...
        option.state = State.saled;
        ISynchron(synchron).updateOptionWithProperty(optionId, abi.encode(option));
        ISynchron(synchron).submitUserInfo(1, offer.bidder, optionId);
