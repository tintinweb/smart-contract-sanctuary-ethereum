...
contract Bloodles is ERC721Enumerable, Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    uint256 public constant MAX_ELEMENTS = 4444;
    uint256 public constant WLPRICE = 0.05 ether;
    ...
    event CreateBloodles(uint256 indexed id);

    constructor(string memory baseURI, bytes32 _whitelist)
        ERC721("Bloodles", "BLD")
    {
        setBaseURI(baseURI);
     ...
     */
    function publicSale(uint256 _amount) public payable {
        ...
        for (uint256 i = 0; i < _amount; i++) {
            ...
            emit CreateBloodles(id);
        }
    }

    /**
     * @dev Presale for whitelist
     * @param proof is the proof bytes
     */
    function preSale(bytes32[] calldata proof) public payable nonReentrant {
        require(preSaleIsActive(), "Pre-sale is not active");
        require(!presalelist[msg.sender], "Already claimed.");
        require(
            _whitelistVerify(
                _whitelistLeaf(
                    msg.sender /*, tokenId*/
        ...
        require(msg.value >= WLPRICE, "Value below price");

        _safeMint(msg.sender, total);
        presalelist[msg.sender] = true;

        emit CreateBloodles(total);
    }
