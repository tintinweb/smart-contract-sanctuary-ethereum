...
contract DiveWalletNft is ERC721Enumerable, ReentrancyGuard, Ownable, Pausable {
    using Strings for uint256;

    string[5] public baseURI;

    uint256 public constant MAX_NFT = 5000;
    uint256 public walletLimit = 5;
    uint256 public nftPrice = 0.5 ether;
    uint256 public released;

    bool public isWhitelistActive;
    bool public isSaleActive;

    mapping(address => uint256) public psBought;
    mapping(address => uint256) public claimed;
    mapping(address => bool) public whitelist;

    constructor() ERC721("Dive Wallet", "DWTNFT") {}
    ...
    function release(uint256 _upto, uint256 _price) external onlyOwner {
        released = _upto;
        nftPrice = _price;
    }
    ...
    function setPrivateSaleUsers(address[] memory _to, uint256[] memory _amount) external onlyOwner {
        require(_to.length == _amount.length, "array size mismatch");
        
        for(uint256 i = 0; i < _to.length; i++) {
            psBought[_to[i]] = _amount[i];
        }
    }
    ...
    function setURIs(string[5] memory _URI) external onlyOwner {
        baseURI = _URI;
    }
    ...
    function mint(uint256 _numOfTokens) public payable whenNotPaused nonReentrant {
        require((totalSupply() + _numOfTokens) <= released, "Purchase would exceed max NFTs");
        require((nftPrice * _numOfTokens) == msg.value, "Invalid Amount");
    ...
    function claim() external whenNotPaused nonReentrant {
        require(psBought[msg.sender] > 0, "Nothing to Claim");
        
        for(uint256 i=0; i < psBought[msg.sender]; i++) {
            _mint(msg.sender, totalSupply());
        }

        psBought[msg.sender] = 0;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory uri) {
        ...
        if (tokenId < 1000) {
            uri = string(abi.encodePacked(baseURI[0], tokenId.toString()));
        } else if (tokenId >= 1000 && tokenId < 2000) {
            uri = string(abi.encodePacked(baseURI[1], tokenId.toString()));
        } else if (tokenId >= 2000 && tokenId < 3000) {
            uri = string(abi.encodePacked(baseURI[2], tokenId.toString()));
        } else if (tokenId >= 3000 && tokenId < 4000) {
            uri = string(abi.encodePacked(baseURI[3], tokenId.toString()));
        } else if (tokenId >= 4000 && tokenId < 5000) {
            uri = string(abi.encodePacked(baseURI[4], tokenId.toString()));
        }
    }

    function airdrop(address _to, uint256 _amount) external onlyOwner {
        require(totalSupply() + _amount <= MAX_NFT, "Reached Max NFT");
