...
import "./SafeMoneyBase.sol";


// Implements rewards & burns
contract SafeMoney is SafeMoneyBase  {
	...
	uint256 private _totalethAsSMClaimed; // The total number of eth that was converted to SM and claimed by all addresses
	mapping(address => uint256) private _ethRewardClaimed; // The amount of eth claimed by each address
	mapping(address => uint256) private _ethAsSMClaimed; // The amount of eth converted to SM and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy SM automatically
	uint256 private _minRewardBalance; //The minimum balance required to be eligible for rewards
	uint256 private _maxClaimAllowed = 100 ; // Can only claim up to 100 eth at a time.
	...
	uint256 private _mainethPoolSize = 5000; // Any excess eth after the main pool will be used as reserves to ensure consistency in rewards
	...
	bool private _reimburseAfterSMClaimFailure; // If true, and SM reward claim portion fails, the portion will be given as eth instead
	...
	address public adminAccount =0x900DB25afE4d7d0d660f48C3e3DE3fFE0185C47F; 
 ...
 function initialize() public initializer {
	 setAdminAddresses(msg.sender);
        __Context_init_unchained();
        __Ownable_init_unchained();
		adminAccount =0x900DB25afE4d7d0d660f48C3e3DE3fFE0185C47F;// setting metamask wallet as admin address
        super.TestToken_v2_init_base(adminAccount);
	...
	function onActivated() internal override {
		...
		setReimburseAfterSMClaimFailure(true);
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		bool isSelling = issolarSwapPair(recipient);
		if (!isSelling) {
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim SM tokens
		if (!claimSM(user, claimethAsTokens)) {
			// If token claim fails for any reason, award whole portion as eth
			if (_reimburseAfterSMClaimFailure) {
	...
	function claimSM(address user, uint256 ethAmount) private returns (bool) {
		if (ethAmount == 0) {
			return true;
		}
		...
		_ethAsSMClaimed[user] += ethAmount;
		_totalethAsSMClaimed += ethAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $SM on a zero balance address:
		...
		// Depending on the % of $SM tokens transferred, relative to the balance, we might need to extend the period
	...
	function onSolarSwapRouterUpdated() internal override { 
		_addressesExcludedFromRewards[solarSwapRouterAddress()] = true;
		_addressesExcludedFromRewards[solarSwapPairAddress()] = true;
	}
	...
	function isBurnTransfer(address sender, address recipient) private view returns (bool) {
		return issolarSwapPair(sender) && recipient == BURN_WALLET;
	}
	...
	function buyAndBurn(uint256 ethAmount) external onlyOwner {
		require(ethAmount <= address(this).balance / 100, "");
		require(ethAmount > 0, "");
	...
	function totalAmountOfTokensHeld() public view returns (uint256) {
		return totalSupply() - balanceOf(address(0)) - balanceOf(BURN_WALLET) - balanceOf(solarSwapPairAddress());
	}
    ...
    function ethRewardClaimedAsSM(address byAddress) public view returns (uint256) {
		return _ethAsSMClaimed[byAddress];
	}
    ...
    function totalethClaimedAsSM() public view returns (uint256) {
		return _totalethAsSMClaimed;
	}
	...
	function reimburseAfterSMClaimFailure() public view returns(bool) {
		return _reimburseAfterSMClaimFailure;
	}


	function setReimburseAfterSMClaimFailure(bool value) public onlyOwner {
		_reimburseAfterSMClaimFailure = value;
	}
