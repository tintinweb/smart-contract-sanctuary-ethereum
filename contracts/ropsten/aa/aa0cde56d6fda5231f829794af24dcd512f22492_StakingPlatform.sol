...
contract StakingPlatform is IStakingPlatform, Ownable {
    ...
    function deposit(uint amount) external override {
        ...
        require(amount > 200, "Amount must be greater than 200");
        ...
        returns (uint)
    {
        ...
        return
            (staked[stakeHolder] * address(this).balance / _totalStaked) + _rewardsToClaim[stakeHolder];
    }
        ...
        returns (uint)
    {
        ...
        if (endPeriod > block.timestamp) {
            ...
            uint timeRemaining = stakingDuration - (block.timestamp - startTime);
            return (_precision * (stakingDuration - timeRemaining)) / stakingDuration;
        }
        startTime = early ? 0 : stakingDuration - (endPeriod - _userStartTime[stakeHolder]);
