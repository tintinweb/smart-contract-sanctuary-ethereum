    ...
    Signing
{
    ...
    uint256 private currentLunchboxes = 0;
    string private name_ = "VoxSoulsLunchboxes";
    string private symbol_ = unicode"🍱";
    ...
    /**
     * @notice Get the current cost of a soul.
     * @param count the number of souls to be purchased.
     * @dev The cost of a soul is determined by the number of souls purchased.
     * @dev the first 5000 mints are free.  After the mint is .1 ether.
     */
    function getCurrCost(uint256 count) public view returns (uint256) {
        uint256 total;

        for (uint256 i = 0; i < count; i++) {
            if (currentLunchboxes + i > 5000) {
                total += .1 ether;
            }
        }
    ...
    function _mintSoul(
        ...
        require(msg.value >= getCurrCost(1), "Not enough ether!");
     ...
     * @param account The account that will own the lunchbox.
     * @param _contract The Vox contract address.
     * @param id The id of the lunchbox.
     * @param mintTime The mintTime seconds the Vox was minted.
     * @param sig The transaction signature.
     */
