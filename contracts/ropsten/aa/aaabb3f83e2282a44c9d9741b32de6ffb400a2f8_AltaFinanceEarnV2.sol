...
contract AltaFinanceEarnV2 is EarnBase {
    ...
    event BidMade(address indexed bidder, uint256 indexed bidId);

    /// @param earnContractId Index of earn contract in earnContracts
    event ContractForSale(uint256 indexed earnContractId);

    /// @param earnContractId Index of earn contract in earnContracts
    event ContractOffMarket(uint256 indexed earnContractId);
        ...
        returns (uint256 baseTokenAmount, uint256 altaAmount)
    {
        ...
        altaAmount = altaAmount - earnContract.altaPaid + earnContract.altaStaked;
    ...
    function acceptBid(uint256 _bidId) external whenNotPaused {
        ...
        emit ContractOwnershipTransferred(bid.to, bid.bidder, earnContractId);
    ...
    function _closeContract(uint256 _earnContractId) internal {
        ...
        earnContracts[_earnContractId].status = ContractStatus.CLOSED;
        ALTA.transfer(owner, earnContract.altaStaked);
    }
