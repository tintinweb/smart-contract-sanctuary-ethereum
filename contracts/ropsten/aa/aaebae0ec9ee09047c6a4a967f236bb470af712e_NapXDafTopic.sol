...
contract NapXDafTopic{
  ...
  event ProposalAdded(uint proposalID, uint weight1, uint weight2, bytes32 description);
  event Voted(uint proposalID, bool position, uint voteWeight, address voter);
  event TopicTallied(uint chosenProposalID, int voteWeight);
  event TopicCreated(address dafTokenAddress, uint debatingPeriodInMinutes, bytes15 assetClass, bytes32 longDescription);
   ...
   * First time setup
   */
  constructor(NapXDafVotingToken _dafTokenAddress, uint _debatingPeriodInMinutes, bytes15 _assetClass, bytes32 _longDescription) public {
      ...
      returns (uint voteID)
  {
      ...
      int castedVoteWeight = int(voteWeight);
      if (supportsProposal){
        p.proposalWeightedVote += castedVoteWeight;
      } else {
        p.proposalWeightedVote -= castedVoteWeight;        
      }
  ...
  returns(uint winningProposal)
  {
      ...
      int winningVoteCount = -int(dafTokenAddress.INITIAL_SUPPLY());
      for (uint i=0; i < proposals.length; i++){
        int proposalWeight = proposals[i].proposalWeightedVote;
