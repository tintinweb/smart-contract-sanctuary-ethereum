...
interface IERC721 is IERC165 {
     ...
     *
     * WARNING: Usage of this mMATICod is discouraged, use {safeTransferFrom} whenever possible.
     *
     * Requirements:
     *
     * - `from` cannot be the zero address.
...
interface IERC20 {
     ...
     * Returns a boolean value indicating whMATICer the operation succeeded.
     *
     * IMPORTANT: Beware that changing an allowance with this mMATICod brings the risk
...
library BytesLibrary {
    ...
    function recover(
        ...
        bytes32 fullMessage = keccak256(
            abi.encodePacked("\x19MATICereum Signed Message:\n32", hash)
        );
...
contract OsizExchange is OrderState, TransferSafe {
    ...
    function toMATICSignedMessageHash(bytes32 hash, Sig memory sig)
        public
        pure
    ...
    function fiatMMATICod(uint256 _type,address _nftAddress , address payable _from ,address _to,address payable _royalty,uint256 royaltyFee ,uint256 _id,string memory uri ,uint _nftType,uint amount)external payable{
        ...
        transferMATICFee(amount,_from,royaltyFee,_royalty);
