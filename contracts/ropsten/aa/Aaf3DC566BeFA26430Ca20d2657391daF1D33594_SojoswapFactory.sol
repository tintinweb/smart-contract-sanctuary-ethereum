...
import "./interfaces/ISojoswapPair.sol";
import "./SojoswapERC20.sol";
import "./libraries/Math.sol";
import "./libraries/UQ112x112.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/ISojoswapFactory.sol";
import "./interfaces/ISojoswapCallee.sol";

contract SojoswapPair is ISojoswapPair, SojoswapERC20 {
    using SafeMath for uint256;
    using UQ112x112 for uint224;

    uint256 override public constant MINIMUM_LIQUIDITY = 10**3;
    ...
    uint256 override public price0CumulativeLast;
    uint256 override public price1CumulativeLast;
    uint256 override public kLast; // reserve0 * reserve1, as of immediately after the most recent liquidity event

    uint256 private unlocked = 1;
    modifier lock() {
        require(unlocked == 1, "Sojoswap: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function initialize(address _token0, address _token1) override external {
        require(msg.sender == factory, "Sojoswap: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function mint(address to) override external lock returns (uint256 liquidity) {
        ...
        require(liquidity > 0, "Sojoswap: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "Sojoswap: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        address to,
        bytes calldata data
    ) override external lock {
        require(
            amount0Out > 0 || amount1Out > 0,
            "Sojoswap: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "Sojoswap: INSUFFICIENT_LIQUIDITY"
        );
        ...
        uint256 balance1;
        {
            ...
            require(to != _token0 && to != _token1, "Sojoswap: INVALID_TO");
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0)
                ISojoswapCallee(to).SojoswapCall(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
        ...
        require(
            ...
            "Sojoswap: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
...
interface ISojoswapCallee {
    function SojoswapCall(
        address sender,
        uint256 amount0,
        uint256 amount1,
        bytes calldata data
