...
contract AltaFinanceEarnV2 is EarnBase {
    ...
    constructor() {
        _transferOwnership(address(0x087183a411770a645A96cf2e31fA69Ab89e22F5E));
    }
        ...
        returns (uint256 baseTokenAmount, uint256 altaAmount)
    {
        ...
        if (timeOpen <= earnContract.contractLength) {
            ...
            altaAmount = (((((earnContract.lentAmount * (10**ALTA.decimals())) /
                (10**Token.decimals())) * earnTerm.altaRatio) / 10000) *
    ...
    function acceptBid(uint256 _bidId) external whenNotPaused {
        ...
        if (fee > 0) {
            IERC20Metadata(earnContract.token).transfer(loanAddress, fee);
            bid.amount = bid.amount - fee;
        }
        IERC20Metadata(earnContract.token).transfer(bid.to, bid.amount);
        ...
        if (length > 0) {
            for (uint256 i = length; i > 0; i--) {
                ...
                if (_bids[bidId].earnContractId == _earnContractId) {
                    if (!_bids[bidId].accepted) {
                        IERC20Metadata(
                            earnContracts[_bids[bidId].earnContractId].token
                        ).transfer(_bids[bidId].bidder, _bids[bidId].amount);
                    }
                    _removeBid(bidId);
                }
            }
    ...
    function removeBid(uint256 _bidId) external {
        ...
        EarnContract memory earnContract = earnContracts[bid.earnContractId];
        IERC20Metadata(earnContract.token).transfer(bid.bidder, bid.amount);
    ...
    /// @param _transferFee Percent of accepted earn contract bid to be sent to Alta wallet
    ...
    function init(IERC20Metadata _ALTA, address _loanAddress)
        external
        onlyOwner
    {
