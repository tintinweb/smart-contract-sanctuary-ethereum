...
contract IDO {
    ...
    constructor(address _usdt, address _plexus) public {
        ...
        usdt = _usdt;
        plexus = _plexus;
        // claim During 3month ( 3 * 30 * 24 * 60 * 4)
    ...
    function deposit(uint256 _userDepositAmount) public {
        ...
        require(
            usdtHardCap.sub(usdtTotalReciveAmount) >= _userDepositAmount,
    ...
    function pendingClaim() public view returns (uint256 pendingAmount) {
        UserInfo memory user = userInfo[msg.sender];
    ...
    function claim() public {
        ...
        if (userInfo[msg.sender].lastRewardBlock < lockupBlock) {
            userInfo[msg.sender].lastRewardBlock = lockupBlock;
        }
        UserInfo memory user = userInfo[msg.sender];
        ...
        TransferHelper.safeTransfer(plexus, msg.sender, claimAmount);
        user.lastRewardBlock = block.number;
        user.recivePLX += claimAmount;
        userInfo[msg.sender] = user;

        emit Claim(msg.sender, claimAmount, user.recivePLX);
    }
