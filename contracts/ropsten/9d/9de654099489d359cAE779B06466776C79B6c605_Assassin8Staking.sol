...
contract Assassin8Staking is Ownable {

    address public treasuryAddress = 0x75f5B78015D79B2f96BD6f24F77EF22ec829D7D0; 
    ...
    uint256 public immutable stakingNFTType;
    address public immutable stakingNFTContract;
    uint256 public totalReward;
    uint256 public rewardPerBlock; // Tokens distributed per block. Use getTokenPerBlock() to get the updated reward.

    mapping(address => uint256) public UserRewardDebt; // Info of each user that stakes Staking Tokens.
    ...
    constructor(address _stakingNFTContract, uint256 _stakingNFTType, uint256 _startBlock) {
        stakingNFTContract = _stakingNFTContract;
        stakingNFTType = _stakingNFTType;
    ...
    receive() external payable {
        rewardPerBlock = address(this).balance * blockRewardPercentage / 100 / blocksPerDay;
    ...
    function getRewardPerBlock() public view returns (uint256, bool) {
        ...
        uint256 poolReward = address(this).balance;
        if (poolReward == 0) {
            return (0, rewardPerBlock != 0);
        }
        return (poolReward * blockRewardPercentage / 100 / blocksPerDay, true);
    }
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && totalStaked != 0) {
            ...
            uint256 tokenReward = multiplier * blockReward * allocPoint / totalAllocPoint;
            thisAccTokensPerShare = thisAccTokensPerShare + (tokenReward * 1e12 / totalStaked);
        }
        return UserNFTs[_msgSender()].length * thisAccTokensPerShare / 1e12 - UserRewardDebt[_user];
    }

    // Update reward variables to be up-to-date when NFT deposit changes
    ...
    function updateRewards() public updateRewardPerBlock {
        ...
        uint256 multiplier = block.number - (lastRewardBlock);
        uint256 tokenReward = multiplier * rewardPerBlock * allocPoint / totalAllocPoint;
    ...
    // Deposit NFTs to farm ETH
    function deposit(uint256 _nftID) external {
        require(stakingEnabled, "STAKING_DISABLED");
        require(NFTSales(stakingNFTContract).NftIdToNftType(_nftID) == stakingNFTType, "WRONG_NFT_TYPE");
        updateRewards();
        if (UserNFTs[_msgSender()].length > 0) {
            uint256 pending = UserNFTs[_msgSender()].length * accTokensPerShare / 1e12 - UserRewardDebt[_msgSender()];
            (bool sent, ) = _msgSender().call{value: pending}("");
            require(sent, "FAILED_SENDING_FUNDS");
        }
        ERC721Min(stakingNFTContract).transferFrom(address(_msgSender()), address(this), _nftID);
        UserNFTs[_msgSender()].push(_nftID);
        totalStaked++;
        UserRewardDebt[_msgSender()] = UserNFTs[_msgSender()].length * accTokensPerShare / 1e12;
        emit Deposit(_msgSender(), _nftID);
    }

    // Withdraw all NFTs from Farming
    function withdraw() external {
        require(UserNFTs[_msgSender()].length >= UserNFTs[_msgSender()].length, "AMOUNT_TOO_BIG");
        updateRewards();
        ...
        if (UserNFTs[_msgSender()].length > 0) {
            ERC721Min(stakingNFTContract).batchSafeTransferFrom(address(this), _msgSender(), UserNFTs[_msgSender()],"");
            delete UserNFTs[_msgSender()];
            totalStaked -= UserNFTs[_msgSender()].length;
        }
    ...
    function emergencyWithdraw() public {
        ...
        delete UserNFTs[_msgSender()];
        totalStaked -= UserNFTs[_msgSender()].length;
        UserRewardDebt[_msgSender()] = 0;

        emit EmergencyWithdraw(_msgSender(), UserNFTs[_msgSender()].length);
    }
...
contract NFTSales is Ownable, ERC721Min, ReentrancyGuard {
    ...
    address private treasuryAddress = 0x75f5B78015D79B2f96BD6f24F77EF22ec829D7D0; 
        ...
        uint16 _maxPerAddress, bool _saleActive, string calldata _uri) external onlyOwner
    {
        ...
        NftTypes[nftTypeCount].maxMintForThree = _maxMint-1;
