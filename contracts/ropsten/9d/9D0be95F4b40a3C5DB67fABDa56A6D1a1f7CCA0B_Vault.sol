...
contract Vault is ERC20 {
    ...
    // function extractOneInch(bytes calldata swapCalldata) external returns (address caller, SwapDescription memory desc, bytes memory data) {
        // (caller, desc, data) = abi.decode(swapCalldata, (address, SwapDescription, bytes));
        caller = pancakeRouter;
    }
}
