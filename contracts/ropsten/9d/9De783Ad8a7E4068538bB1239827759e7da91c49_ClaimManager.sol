...
contract ClaimManager {
  ...
  */
  ...
  uint8 public immutable claimTokenDecimals;
  bool public opened;
  ...
  function spendToClaim(uint256 spent) public view returns (uint256) {
    // convert an integer value of tokens spent to an integer value of tokens to claim
    return (spent * 10 ** claimTokenDecimals) / saleManager.getPrice(saleId);
  }
  ...
  function getRemainingClaim(address claimant) public view returns (uint256) {
    ...
    return spendToClaim(saleManager.getSpent(saleId, claimant));
  }
  ...
  function open() public isSeller saleOver {
    ...
    uint256 _remainingClaims = spendToClaim(saleManager.getTotalSpent(saleId)) - voidClaims;
...
contract SaleManager is ReentrancyGuard {
  ...
  */
  function newSale(
    ...
    saleCount++;
    emit NewSale(saleId, merkleRoot, msg.sender, saleBuyLimit, userBuyLimit, startTime, endTime, name, symbol, price);
  ...
  function buy(
    ...
    uint256 tokenQuantity = (msg.value * getLatestPrice() * 10 ** paymentTokenDecimals) / (10 ** (priceOracle.decimals() + 18));
