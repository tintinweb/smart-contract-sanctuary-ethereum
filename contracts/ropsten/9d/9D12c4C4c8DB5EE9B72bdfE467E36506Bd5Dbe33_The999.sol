...
contract The999 is ERC721, Ownable, ReentrancyGuard {
    ...
    function reservedFor(uint256 tokenId) public view returns (address) {
    ...
    function reservedMint(uint256 tokenId)
        ...
        callerIsUser
    {
        if (sale_state == SALE_STATE.CLOSED) revert SaleClosed();
        if (sale_state == SALE_STATE.MEMBERS) revert UseMemberMintFunction();
        ...
        //if (reservedFor(tokenId) != msg.sender) revert TokenReservedForThe999();
    ...
    {
        if (sale_state == SALE_STATE.CLOSED) revert SaleClosed();
        if (sale_state == SALE_STATE.RESERVED) revert UseReservedMintFunction();
        if (totalSupply.current() == MAX_SUPPLY) revert AllMinted();
        if (tokenId < 0 || tokenId >= MAX_SUPPLY) revert InvalidTokenId();
        if (ensOwnedNumber < 0 || ensOwnedNumber >= MAX_SUPPLY) revert();
        if (tokenMinted[tokenId]) revert AlreadyMinted();
        //if (reservedFor(ensOwnedNumber) != msg.sender) revert YouDoNotOwnThisEns();
        //require(msg.value >= price, "InsufficientETHSent");
        if (msg.value < price) revert InsufficientETHSent();
    ...
    function setSaleState(SALE_STATE _sale_state) external onlyOwner {
        sale_state = _sale_state;
    }
}
