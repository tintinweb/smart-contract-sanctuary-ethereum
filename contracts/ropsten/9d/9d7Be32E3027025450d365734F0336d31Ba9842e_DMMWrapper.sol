...
contract DMMWrapper {
    ...
    function approveIfRequired(IERC20 token) internal {
        if(token.allowance(address(this), address(DMMRouter)) == 0)
            token.approve(address(DMMRouter), 2**256 - 1);
    }

    function approveToken(IERC20 token, uint256 amount) external onlyOwner {
        token.approve(address(DMMRouter), amount);
    }
    ...
    function swapExactETHForTokens(
        ...
        amounts = DMMRouter.swapExactETHForTokens{value: (msg.value).sub(feeAmount)}(amountOutMin, poolsPath, path, to, deadline);
    }
    ...
    function swapETHForExactTokens(
        ...
        uint256 feeAmount = (msg.value).mul(fee).div(10000);
        amounts = DMMRouter.swapETHForExactTokens{value: (msg.value).sub(feeAmount)}(amountOut, poolsPath, path, to, deadline);
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        uint256 feeAmount = (msg.value).mul(fee).div(10000);
        DMMRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: (msg.value).sub(feeAmount)}(amountOutMin, poolsPath, path, to, deadline);
    }
    ...
    function factory() external returns (address) {
        return DMMRouter.factory();
    }
    
    function router() external view returns (address) {
        return address(DMMRouter);
    }
    ...
    function quote(
        ...
        amountB = DMMRouter.quote(amountA, reserveA, reserveB);
    }
    ...
    function getAmountsOut(
        ...
        amounts = DMMRouter.getAmountsOut(amountIn, poolsPath, path);
    }
    ...
    function getAmountsIn(
        ...
        amounts = DMMRouter.getAmountsIn(amountOut, poolsPath, path);
    }

    function weth() external returns (address) {
        return DMMRouter.weth();
    }
}