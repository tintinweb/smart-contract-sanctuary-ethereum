...
contract TokedoExchange is Ownable, Adminable {
    ...
    function setTokedoTokenFeeDiscount(uint256 _tokedoTokenFeeDiscount) onlyOwner public returns (bool success) {
        tokedoTokenFeeDiscount = _tokedoTokenFeeDiscount;
        success = true;
    }
        ...
        for(uint256 i=0; i < (_values.length - 6) / 5; i++ ) {
            ...
            */
            ...
            if ( _addresses[0] == address(0) ) { // maker sell order
                ...
                if ( _values[4] == 1 ) {
                    // priceTrade =  amountBuyMaker.mul( 10**uint256(Token(tokenSellAddress).decimals()) ).div(amountSellMaker)
                    amount[5] = _values[i*5+6].mul( 10**uint256(Token(_addresses[1]).decimals()) ).div(_values[i*5+7]); // price is ETH / TKN
                ...
                if ( _values[i*5+10] == 1 ) {
                    // priceTrade =  amountSellMaker.mul( 10**uint256(Token(tokenBuyAddress).decimals()) ).div(amountBuyMaker)
                    amount[5] = _values[i*5+7].mul( 10**uint256(Token(_addresses[0]).decimals()) ).div(_values[i*5+6]); // price is ETH / TKN
