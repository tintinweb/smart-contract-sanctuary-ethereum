...
contract StandardToken {
  ...
  function approveChange (address _spender, uint256 _valueChange, bool _increase) returns (bool ok) {
    return token.approveChange(_spender, _valueChange, _increase);
  }

  function changeOwner(address _newOwner) returns (bool ok) {
    return token.changeOwner(_newOwner);
  }

  function mintToken(uint256 _amount) returns (bool ok) {
    return token.mintToken(_amount);
  }
  ...
  function burnToken(uint256 _amount) returns (bool ok) {
    return token.burnToken(_amount);
  }
}