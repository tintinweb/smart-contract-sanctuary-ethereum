...
contract Pollen  {
    ...
    function stake(uint256 amount, uint256 period ) public returns(uint256)  {
        ...
        rewards[newItemId] = StakedToken(newItemId, amount, block.timestamp ,block.timestamp, period);
     ...
     function calculateReward(uint256 tokenIndex, AggregatorV3Interface _tokenPriceFeed) public  view returns (uint256) {
       ...
       uint256 rewardPriceUsd = (staked.amount * 100256892 / 1e8 * rewardFactor * (block.timestamp - staked.lastHarvestTimestamp)) / 100 / rewardInterval; // ropsten 
    ...
    function getReward(address _owner, uint256 tokenIndex) public   {
        ...
        require( PollenNFT.balanceOf(_owner) >= 1 , "You don't have any Pollen NFT amount");
    ...
    function reedemCompound(uint256 _amount , uint256 tokenIndex) public returns(uint256) {
    ...
    function getBalanceCToken(address sender, uint256 tokenIndex) public  returns(uint) {

        //StakedToken memory staked = rewards[tokenIndex];

        _ctokenBalance =  cToken.redeem(cToken.balanceOf(address(this)));

//        uint256 _total =  calc - staked.amount;
   ...
   function unstake(address _owner, uint256 tokenIndex) public  {
       ...
       uint256 daiProfit =  cToken.redeem(cToken.balanceOf(address(this))) - rewards[tokenIndex].amount;


       //  cToken.redeem(getBalanceCToken(address(this)));
