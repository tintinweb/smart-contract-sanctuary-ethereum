...
contract legendsofasians is ERC721A,Ownable {
    ...
    uint256 public preSaleNftMinted;

    uint256 public  PublicSale_PRICE_PER_NFT = 100000000000000000 wei;  //0.10 ETH

    uint256 public  PreSale_PRICE_PER_NFT = 80000000000000000 wei;  //0.08 ETH
   ...
   //Token Name and Token Symbol inside constructor
  constructor(string memory MetaUri) ERC721A("legendsofasians", "LOFA") {
             _preSaleIsActive=true;
             _publicSaleIsActive=true;
    ...
    //Start Public sale(Pass true to start and false to stop) 
    function setPublicSaleOn(bool value) public onlyOwner{ 
       require(paused == false, "Contract is Paused");
        _publicSaleIsActive=value;
    } 
     //Check the Public Sale is Active or Not
       function publicSaleIsActive() public view returns(bool) {
        return _publicSaleIsActive;
    }

    //Start Pre sale(Pass true to start and false to stop) 
    function setPreSaleOn(bool value) public{
        require(paused == false, "Contract is Paused");
        _preSaleIsActive=value;
    } 

    //Sets PreSale NFT cost(Must be in wei)
    function setPreSalePrice(uint256 price) public onlyOwner{
        require(paused == false, "Contract is Paused");
        PreSale_PRICE_PER_NFT=price;
    }

      //Sets PublicSale  NFT cost(Must be in wei)
    function setPublicSalePrice(uint256 price) public onlyOwner{
         require(paused == false, "Contract is Paused");
         PublicSale_PRICE_PER_NFT=price;
    }

    //Check the Pre Sale is Active or Not
    ...
    //Set Max Mint Amount per transaction
     function setMaxPurchase(uint256 tokens) public onlyOwner{
         require(paused == false, "Contract is Paused");
         MAX_PURCHASE=tokens;
    }
    ...
    function mint(uint16 numberOfTokens) public payable {
        ...
        require(PublicSale_PRICE_PER_NFT.mul(numberOfTokens) <= msg.value, "Ether value sent is incorrect");
        require(numberOfTokens <= MAX_PURCHASE, "Can only mint specific number of tokens per transaction");
    ...
    function preSalemint(uint16 numberOfTokens) public payable {
        ...
        require(PreSale_PRICE_PER_NFT.mul(numberOfTokens) <= msg.value, "Ether value sent is incorrect.");
        require(numberOfTokens <= MAX_PURCHASE, "Can only mint specific number of tokens per transaction");
        require(totalSupply().add(numberOfTokens) < Max_Supply, "Insufficient supply");
        preSaleNftMinted += numberOfTokens;
