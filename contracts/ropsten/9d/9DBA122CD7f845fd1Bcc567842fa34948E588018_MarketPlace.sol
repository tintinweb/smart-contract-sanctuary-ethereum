...
contract MarketPlace is BasePlace, CrossChain {
    ...
    function getMessageHashBid(uint256 id, address nftContract, uint256 tokenId, address seller, address buyer, address token, uint256 price, address creatorAddress, uint256 creatorRoyalty) public pure returns (bytes32 mh, bytes32 smh) {
        mh = keccak256(abi.encodePacked(id, nftContract, tokenId, seller, buyer, token, price, creatorAddress, creatorRoyalty));
        smh = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", mh));
    }

    function getMessageHashPost(address nftContract, uint256 tokenId, address seller, uint256 price, uint256 expiry, address creatorAddress, uint256 creatorRoyalty) public pure returns (bytes32 mh, bytes32 smh) {
        mh = keccak256(abi.encodePacked(nftContract, tokenId, seller, price, expiry, creatorAddress, creatorRoyalty));
        smh = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", mh));
    }
