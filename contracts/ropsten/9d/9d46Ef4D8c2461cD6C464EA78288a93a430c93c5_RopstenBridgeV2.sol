...
interface ICrossBridgeV2 {
    ...
    function setEpoch(uint64 epoch, bool epochInit) external;

    function setScanRange(uint64[2] memory scanRange, bool scanRangeInit)
        external;
}
    ...
    ICrossBridgeV2
{
    ...
    bool private epochInit;
    bool private scanRangeInit;
    ...
    function setEpoch(uint64 epoch_, bool epochInit_)
        external
        virtual
        override
        onlyOwner
    {
        require(!epochInit, "Epoch already init");
        require(epoch_ > 0, "IE");
        bs.setEpoch(epoch_);
        epochInit = epochInit_;
    }

    function setScanRange(uint64[2] memory scanRange, bool scanRangeInit_)
        external
        virtual
        override
        onlyOwner
    {
        require(!scanRangeInit, "ScanRange already init");
        bts.setScanRange(scanRange);
        scanRangeInit = scanRangeInit_;
    }
}

// File: contracts/Net-Ropsten/BridgeV2/RopstenBridgeV2.sol

pragma solidity ^0.8.7;

contract RopstenBridgeV2 is CrossBridgeUpgradeableV2 {
    function initialize(
        address tokenFactory,
        address bridgeStorage,
        address bridgeTokenStorage,
