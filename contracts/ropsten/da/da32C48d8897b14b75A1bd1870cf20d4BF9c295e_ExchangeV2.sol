...
contract ExchangeV2 is IExchange, Ownable {
    ...
    function setRouter(address _router) external onlyOwner {
        router = ISwapV2Router(_router);  // switching router does not affect end-users, it affects only project funds itself
        emit RouterSet(_router);
    }

    /// @inheritdoc IExchange
    function getEstimatedTokensForETH(IERC20 _token, uint256 _ethAmount) external returns (uint256) {
        return router.getAmountsIn(_ethAmount, _getSwapPair(_token))[0];
    }

    /// @inheritdoc IExchange
    function swapTokensToETH(
        IERC20 _token,
        uint256 _receiveEthAmount,
        uint256 _tokensMaxSpendAmount,
        address _ethReceiver,
        address _tokensReceiver
    ) external returns (uint256) {
        // Approve tokens for router V2
        _token.safeApprove(address(router), _tokensMaxSpendAmount);

        // Make exchange
        uint256 spentTokens = router.swapTokensForExactETH(_receiveEthAmount, _tokensMaxSpendAmount, _getSwapPair(_token), _ethReceiver, block.timestamp)[0];
    ...
    function _getSwapPair(IERC20 _token) internal returns (address[] memory) {
        address[] memory swapPair = new address[](2);

        swapPair[0] = address(_token);
