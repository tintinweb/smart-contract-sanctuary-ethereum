...
contract PlexusFactory {
    ...
    event PairCreated(
        ...
        address pair,
        uint256 fee,
        uint256 pairOwnerFee,
        address pairOwner
    );
    event updatePairOwnerFee(address pair, uint256 newOwnerFee);
    event updatePairOwner(address pair, address newOwner);
    ...
    function createPool(
        ...
        if (msg.sender != owner || msg.sender != feeToSetter) {
            IERC20(plexus).transferFrom(msg.sender, deadAddress, pairCreateFee);
        ...
        emit PairCreated(token0, token1, pair, _fee, _pairOwnerFee, msg.sender);
    }
...
interface IFeeDistributor {
    function updateFeeToken(address feeToken, uint256 amount) external;
}

interface IAirdrop {
    function harvest(address pair, address to) external;

    function lpTokenUpdatePool(address pair) external;

    function lpTokenId(address lpaddress) external view;

    function poolCheck(address pair) external view;
}

interface IPlexusFactory {
    ...
    function getPairOwner(address pair) external view returns (address);

    function pairOwnerFee(address pair) external view returns (uint256 fee);
...
contract PlexusPair {
    ...
    function increaseTotalSupply(uint256 amount) private {
        IAirdrop(IPlexusFactory(factory).airdrop()).lpTokenUpdatePool(address(this));
    ...
    function _mint(address to, uint256 value) internal {
        increaseTotalSupply(value);
        increaseBalance(to, value);
        emit Transfer(address(0), to, value);
    }

    function _burn(address from, uint256 value) internal {
        decreaseTotalSupply(value);
    ...
    function initialize(address _token0, address _token1) external {
        ...
        name = string(abi.encodePacked("PlexusSwap LP ", IERC20(_token0).symbol(), "-", IERC20(_token1).symbol()));
        uint256 chainId = 1;
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                keccak256("EIP712Domain(string name, string version, uint256 chainId, address verifyngContract)"),
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            liquidity = Math.sqrt(amount0.mul(amount1)); // permanently lock the first MINIMUM_LIQUIDITY tokens
        } else {
    ...
    function swap(
        ...
        uint256 fee = IPlexusFactory(factory).getPairFee(address(this));
        // fee 수수료 설정 부분
        {
            // scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint256 balance0Adjusted = balance0.mul(10000).sub(amount0In.mul(fee));
            uint256 balance1Adjusted = balance1.mul(10000).sub(amount1In.mul(fee));
            require(balance0Adjusted.mul(balance1Adjusted) >= uint256(_reserve0).mul(_reserve1).mul(10000**2), " K");
        }

        //amountIn transfer fee
        uint256 buybackFee;
        uint256 gnrFee;
        uint256 feeAmount = amount0In == 0 ? amount1In.mul(fee) / 10000 : amount0In.mul(fee) / 10000;
        address feeToken = amount0In == 0 ? token1 : token0;

        if (IPlexusFactory(factory).getPairOwner(address(this)) != IPlexusFactory(factory).feeToSetter()) {
            uint256 plexusFee = feeAmount / 3;
            buybackFee = (plexusFee * 7) / 10;
            gnrFee = (plexusFee * 3) / 10;
            if (IPlexusFactory(factory).pairOwnerFee(address(this)) != 0) {
                uint256 pairOwnerFee = ((feeAmount - plexusFee) * IPlexusFactory(factory).pairOwnerFee(address(this))) /
                    100;
                _safeTransfer(feeToken, address(IPlexusFactory(factory).getPairOwner(address(this))), pairOwnerFee);
            }
            _safeTransfer(feeToken, IPlexusFactory(factory).buyback(), buybackFee);
            if (IPlexusFactory(factory).feeDistributor() != address(0)) {
                IFeeDistributor(IPlexusFactory(factory).feeDistributor()).updateFeeToken(feeToken, gnrFee);
            } else {
                _safeTransfer(feeToken, IPlexusFactory(factory).buyback(), gnrFee);
            }
        } else {
            buybackFee = (feeAmount * 6) / 10;
            gnrFee = (feeAmount * 2) / 10;
            _safeTransfer(feeToken, IPlexusFactory(factory).buyback(), buybackFee);
    ...
    function giveReward(address user) private {
        IAirdrop(IPlexusFactory(factory).airdrop()).harvest(address(this), user);
    }
    ...
    function claimReward(address user) public lock {
        giveReward(user);
    }
}
