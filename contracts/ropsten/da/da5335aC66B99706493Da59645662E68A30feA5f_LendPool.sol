...
library GenericLogic {
    ...
    )
  {
    ...
    vars.liquidatePrice = vars.nftPriceInReserve.percentMul(PercentageMath.PERCENTAGE_FACTOR);
  ...
  function calculateLoanBuyItNowFine(
    ...
    vars.reserveDecimals = reserveData.configuration.getDecimals();
    vars.reservePriceInETH = IReserveOracleGetter(reserveOracle).getAssetPrice(reserveAsset);
    vars.baseBuyItNowFineInReserve = (1 ether * 10**vars.reserveDecimals) / vars.reservePriceInETH;

    vars.minBuyItNowFinePct = nftData.configuration.getMinBuyItNowFine();
    vars.minBuyItNowFineInReserve = vars.baseBuyItNowFineInReserve.percentMul(vars.minBuyItNowFinePct);

    (, vars.debtAmount) = ILendPoolLoan(poolLoan).getLoanReserveBorrowAmount(loanData.loanId);

    vars.buyItNowFineInReserve = vars.debtAmount.percentMul(nftData.configuration.getRedeemFine());
...
library LiquidateLogic {
    ...
    function executeBuyItNow(
        ...
        // first time buy-it-now need to burn debt tokens and transfer reserve to bTokens
