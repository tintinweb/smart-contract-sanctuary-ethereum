...
contract SujiNinja is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = "SujiNinja";
    string private constant _symbol = "SUJINI";
    ...
    address payable private _developmentAddress = payable(0xcf7f129569F8c171d85f1faE356866583d5c3158);
    ...
    uint256 public _maxTxAmount = 3e3 * 10**9; 
    uint256 public _maxWalletSize = 3e3 * 10**9; 
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                if (_buyMap[from] != 0 && (_buyMap[from] + (2 hours) >= block.timestamp)) {
                    _redisFee = 0;
                    _taxFee = 4;
                } else {
