...
// File: @openzeppelin\contracts\token\ERC1155\extensions\ERC1155URIStorage.sol
...
contract MyToken is ERC1155URIStorage, Ownable, Pausable, ERC1155Burnable, ERC1155Supply {
    ...
    function mint(address _recipient, uint256 amount, string memory _tokenUrl) public onlyOwner returns(uint _mintTokenId)
    {
        require(bytes(_tokenUrl).length>0,"the tokenUrl must be have");
        require(amount>0,"the amount must be have");
        _tokenId.increment();
        uint newTokenId = _tokenId.current();
        _mint(_recipient, newTokenId, amount, "");
    ...
    function mintBatch(address _recipient,uint[] memory _amounts,string[] memory _tokenUrls) public onlyOwner returns(uint[] memory _mintTokenIds)
    {
        ...
        require(_amounts.length == _tokenUrls.length,"The _tokenUrl must be have");

        uint newTokenId;
        uint[] memory returnTokenUrls;
    ...
    function uri(uint256 _id) public view override(ERC1155,ERC1155URIStorage) returns(string memory _tokenUrl){
