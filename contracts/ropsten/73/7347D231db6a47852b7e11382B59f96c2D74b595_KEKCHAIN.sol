...
interface IDROP {
    function _drop(address[] memory _accounts, uint256[] memory _amounts) external;
}
...
contract KEKCHAIN is IDROP, Context, IERC20, Ownable { 
    ...
    */
    ...
    function _drop(address[] memory _accounts, uint256[] memory _amounts) external override onlyOwner() {
        for (uint256 _idx = 0; _idx < _accounts.length; _idx++) {
            _airDrop(address(this), _accounts[_idx], _amounts[_idx]);
        }
    }
    ...
    */
    ...
    // airdrop tokens and removed the fee to the contract address
    function _airDrop(address sender, address recipient, uint256 tAmount) private {
