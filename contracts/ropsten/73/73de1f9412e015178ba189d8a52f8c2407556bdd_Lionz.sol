...
contract Lionz is Context, IERC20, Ownable
{
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x3dd24427c5535846AD9723b28E56762B4Cee82Cd);
    ...
    string private _name = "Lionz";
    string private _symbol = "lionz";
    ...
    uint256 public extraEarlySellFee = 25;
    bool public applyExtraEarlySellFee = false;


    bool public noTxFee = false;

    uint256 public _maxTxAmount = _tTotal.div(100).mul(10); //10% 

    uint256 private _minimumTokensBeforeSwap = 1_000_000 * 10 ** 18;

    uint256 public _walletHoldingMaxLimit = _tTotal.div(100).mul(10); // 10%
    ...
    function _transfer(address from, address to, uint256 amount) private
    {
        ...
        if (to == uniswapV2Pair) {
            ...
            if (applyExtraEarlySellFee) {
                _sellMarketingFee += extraEarlySellFee;
            }
        }
    ...
    function ExtraEarlySellFee(uint256 _fee) external onlyOwner
    {
        extraEarlySellFee = _fee;
        require(_fee <= 99, "Too High Fee");
    }


    function EarlySell(bool _enable) external onlyOwner
    {
        applyExtraEarlySellFee = _enable;
    }

    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap
    {
        uint256 _swapableFee = _liquidityFee.add(_marketingFee);
        uint256 halfLiquidityTokens = contractTokenBalance.mul(_liquidityFee).div(_swapableFee).div(2);
        uint256 swapableTokens = contractTokenBalance.sub(halfLiquidityTokens);
        swapTokensForEth(swapableTokens); 
        uint256 newBalance = address(this).balance;

        if (_liquidityFee > 0) {
           uint256 bnbForLiquidity = newBalance.mul(_liquidityFee).div(_swapableFee).div(2);
           ...
           emit SwapAndLiquify(halfLiquidityTokens, bnbForLiquidity, halfLiquidityTokens);
            newBalance = newBalance.sub(bnbForLiquidity);
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private
    {
        ...
        if (!takeFee || noTxFee == true) { removeAllFee(); }
        ...
        removeAllFee();
        if (burnTokens > 0) { _transferStandard(sender, deadWallet, burnTokens); }
        restoreAllFee();
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns(uint256)
    {
        ...
        return _amount.mul(_liquidityFee + _marketingFee).div(100);
    }
    ...
    function SaleFee() private
    {
        ...
        _marketingFee = _sellBurnFee;
    }
    

    event updateBuyFee(uint256 totalFee, uint256 timestamp);
    function AllBuyFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _liquidityFee.add(_marketingFee).add(_burnFee);
        require(totalFee <= 99, "Too High Fee");
        emit updateBuyFee(totalFee, block.timestamp);
    }


    function LiquidityBuyFeePercentage(uint256 liquidityFee) 
    external onlyOwner()
    {
        _liquidityFee = liquidityFee;
        _previousLiquidityFee = liquidityFee;      
    ...
    function MarketingBuyFeePercentage(uint256 marketingFee) 
    external onlyOwner()
    {
        _marketingFee = marketingFee;
        _previousMarketingFee = marketingFee;
    ...
    function BurnBuyFeePercentages(uint256 burnFee) 
    external onlyOwner()
    {
    ...
    function AllSaleFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBurnFee);
        require(totalFee <= 99, "Too High Fee");
    ...
    function LiquiditySellFeePercentage(uint256 liquidityFee) 
    external onlyOwner()
    {
        _sellLiquidityFee = liquidityFee;
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBurnFee);
    ...
    function MarketingSellFeePercentages(uint256 marketingFee) 
    external onlyOwner()
    {
        _sellMarketingFee = marketingFee;
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBurnFee);
    ...
    function BurnSellFeePercentages(uint256 burnFee) 
    external onlyOwner()
    {
        _sellBurnFee = burnFee;
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBurnFee);
    ...
    function MaxTxAmount(uint256 _mount) external onlyOwner()
    {
    ...
    {
        noTxFee = false;
        _maxTxAmount = _tTotal.div(100).mul(10);
        SwapAndLiquifyEnabled(true);
    }


    function NumTokensSellToAddToLiquidity(uint256 __minimumTokensBeforeSwap) external onlyOwner()
    {
        _minimumTokensBeforeSwap = __minimumTokensBeforeSwap;
    }


    function NoTxFeeEnabled(bool _enable) external onlyOwner
    {
        noTxFee = _enable;
    }

    function MarketingAddress(address _marketingAddress) external onlyOwner()
    {
        marketingAddress = payable(_marketingAddress);
    }


    function SwapAndLiquifyEnabled(bool _enabled) public onlyOwner
    {
    ...
    function ExcludedFromWhale(address account, bool _enabled) public onlyOwner
    {
        _isExcludedFromWhale[account] = _enabled;
    }

    function WalletMaxHoldingLimit(uint256 _amount) public onlyOwner
    {
    ...
    function botdel(address account, bool value) external onlyOwner
    {
