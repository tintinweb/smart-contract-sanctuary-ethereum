...
contract AEXCR is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    AEXCRDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable USDT = address(0x110a13FC3efE6A245B50102D2d79B3E76125Ae83); //USDT

    uint256 public _totalSupply = 600_000_000 * (10**18);
    ...
    address payable public _marketingWalletAddress = payable(0xF9CA766a3d37829aF18bab77b8bECae72B06553C);
    address payable public _developerWalletAddress = payable(0x972DB77D2c103958AC1326c769d68FF8CaCeb97A);
    ...
    constructor() BEP20("AEXC Rewards", "$AEXCR") 
    {
        dividendTracker = new AEXCRDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "AEXCR: The dividend tracker already has that address");
        AEXCRDividendTracker newDividendTracker = AEXCRDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "AEXCR: The new dividend tracker must be owned by the AEXCR token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AEXCR: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AEXCR: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AEXCR: The PanUSDTSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AEXCR: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AEXCR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AEXCR: Cannot update gasForProcessing to same value");
    ...
    function swapAndSendToFee(uint256 tokens) private  
    {
        ...
        IERC20(USDT).transfer(_marketingWalletAddress, marketingShare);
        IERC20(USDT).transfer(_developerWalletAddress, newBalance.sub(marketingShare));
    }
    ...
    function swapAndLiquify(uint256 tokens) private 
    {
       uint256 swapableFee = totalFees.sub(USDTRewardsFee);
        ...
        uint256 ethForDeveloper = newBalance.mul(developerFee).div(swapableFee);

        _marketingWalletAddress.transfer(ethForMarketing);
        _developerWalletAddress.transfer(ethForDeveloper);
    }
...
contract AEXCRDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AEXCR_Dividen_Tracker", "AEXCR_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 1000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "AEXCR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "AEXCR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AEXCR contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "AEXCR_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AEXCR_Dividend_Tracker: Cannot update claimWait to same value");
