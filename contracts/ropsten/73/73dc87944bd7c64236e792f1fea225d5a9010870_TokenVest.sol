...
contract TokenVest{
    ...
    uint256 public totalAddedToken; // total no of token invested in contract
    uint256 public totalReleasedToken; // total released token 
    uint256 public RemainToken;
    uint256 public totalInvestor; // total no. of investor
    ...
    constructor(address payable _owner, address token1) {
        owner = _owner;
        investedToken = IERC20(token1); // address of token that we will stake or vest
        lockingPeriod = 240; // locking period in days or no of seconds like 3600 for 1 hour
        releaseInterval = 120; // release interval in days or seconds
        releasePer = 50; // release percentage per interval valuse will be multiplied with 10
        percentDivider = 1000;
        minimumLimit = 1e20; // minimum limit for staking in wei
    }
   ...
   /** this method will used to withdraw invested token 
	* before release token will locked for locking period and after that it will release setted percentage for setted period */
    function releaseToken(uint256 index) public {
        ...
        Investors[msg.sender].totalWithdrawedTokenUser = Investors[msg.sender].totalWithdrawedTokenUser.add(releaseLimitTillNow);

        if(investorRecord[msg.sender][index].totalWithdrawal == investorRecord[msg.sender][index].amount){
    ...
    /** this method is only callable by owner address and it is used to change the Release Interval, argument will be in second */
