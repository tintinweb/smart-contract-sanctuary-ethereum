...
contract AccrualBondsV1 is AccrualBondStorageV1, Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function getSpotPrice(
        ...
        // decay virtual reserves
        uint256 virtualReserves = BondPriceLib.expToLevel(
            quote.virtualReserves, 
            block.timestamp - quote.lastUpdate, 
            quote.halfLife, 
            quote.levelBips
...
library BondPriceLib {
    ...
    function getAmountOut(
        ...
        uint256 decayedInputReserves = expToLevel(virtualInputReserves, elapsed, halfLife, levelBips);

        outputReserves += virtualOutputReserves;

        output = FixedPointMathLib.fmul(input, outputReserves, (decayedInputReserves + input));

        virtualInputReservesAdjusted = virtualInputReserves + input;

        // require(virtualInputReservesAdjusted * (outputReserves - output) >= virtualInputReserves * outputReserves, "getAmountOut: K");
    }
