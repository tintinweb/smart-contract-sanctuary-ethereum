...
// File contracts/UnityNFTGame.sol
...
contract UnityNFTGame is IMintable, ERC721, Ownable {

    string public baseURI;
    using SafeMath for uint;
    address public imx;
    uint public MAX_COUNT_PER_WALLET = 5;

    event tokenMinted(address to, uint tokenID, bytes blueprint);
    mapping (address=> uint) public maxCountPerWallet;
    mapping (uint => bytes) public minting;

    constructor(address _imx) ERC721('UnityNFTGame', 'UNG') {
        imx = _imx;
    }
    ...
    function tokenMint(address _to, uint tokenId) internal {
        require(maxCountPerWallet[_to] + 1 <= MAX_COUNT_PER_WALLET, "Purchage Limit Fullfilled");
        /* checking will be done in the backend */
        maxCountPerWallet[_to] = maxCountPerWallet[_to] + 1;
    ...
    */
    ...
    /*
    * @dev transferring _amount Ether to the _receipient address from the contract.
    * 
    ...
    */
    ...
    function transferEther(address payable _recipient, uint _amount) external onlyOwner returns (bool) {
        require(address(this).balance >= _amount, "Not enough Ether in contrat!");
