 ...
 * @author Strike
 */
contract SErc20Delegator is STokenInterface, SDelegatorInterface {
    /**
   ...
   /**
    ...
    function seize(address liquidator, address borrower, uint seizeTokens, uint seizeAndGiveLiquidator) external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("seize(address,address,uint256,uint256)", liquidator, borrower, seizeTokens,seizeAndGiveLiquidator));
        return abi.decode(data, (uint));
    }
