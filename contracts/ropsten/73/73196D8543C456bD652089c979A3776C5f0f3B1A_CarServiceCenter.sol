...
contract CarServiceCenter {
    ...
    mapping(uint256 => mapping(address => bool)) public acknoledgement;
    mapping(uint256 => uint256) public setIndexOfServiceId;
    mapping(uint256 => address) public setServiceIdToAddress;
    ...
    */
    function createService(
        uint256 _service_id,
        uint256 _service_start_date,
        address _customer_address,
        uint256 _service_cost
    ) public {
        require(
            setServiceIdToAddress[_service_id] ==
                0x0000000000000000000000000000000000000000,
            " service id already exist"
        );
    ...
    */
    function updateServiceCost(uint256 _service_id, uint256 _service_cost)
        public
    {
        address customer_address = setServiceIdToAddress[_service_id];
        require(
            customer_address != 0x0000000000000000000000000000000000000000,
            " service id does'nt exist"
        );
        require(
            services[setIndexOfServiceId[_service_id]].service_creator ==
                msg.sender,
            "only service creator can update the status"
        );
    ...
    */
    function updateAcknowledgmentStatusForServiceCretor(
        uint256 _service_id,
        address _address,
        bool _acknoledgement_status
    ) public {
        require(
            services[setIndexOfServiceId[_service_id]].service_creator ==
                msg.sender,
            "only service creator can update the acknowledgement"
        );
        require(
            services[setIndexOfServiceId[_service_id]].service_creator ==
                _address,
        ...
        if (
            acknoledgement[_service_id][_address] &&
            acknoledgement[_service_id][
                services[setIndexOfServiceId[_service_id]].customer_address
            ] ==
            true
        ) {
            services[setIndexOfServiceId[_service_id]].status = "completed";
        }
    }
    ...
    */
    function updateAcknowledgmentStatusForCustomer(
        uint256 _service_id,
        address _address,
        bool _acknoledgement_status
    ) public {
        ...
        require(
            services[setIndexOfServiceId[_service_id]].customer_address ==
                msg.sender,
            "you cannot call this method through this address"
        );
        require(
            services[setIndexOfServiceId[_service_id]].customer_address ==
                _address,
    ...
    @return  CustomerAcknowledgment 'Customer Acknowledgment'
    @return  ServiceCreatorAcknowledgment 'Customer Acknowledgment'
    @return  withdrawStatment 'service creator have withdraw the amount of service cost or not'
    */

    function getServiceDetails(uint256 _service_id)
        public
        view
        returns (
            uint256 service_id,
            ...
            bool CustomerAcknowledgment,
            bool ServiceCreatorAcknowledgment,
            bool withdrawStatment,
            bool depositFundByCustomer
        )
    {
        Service memory myService = services[setIndexOfServiceId[_service_id]];
        uint256 _service_id_ = _service_id;
        address _customer_address = setServiceIdToAddress[_service_id_];
        require(
            _customer_address != 0x0000000000000000000000000000000000000000,
            "service id does'nt exist"
        );
        return (
            ...
            myService.status,
            acknoledgement[_service_id_][
                services[setIndexOfServiceId[_service_id_]].customer_address
            ],
            acknoledgement[_service_id_][
                services[setIndexOfServiceId[_service_id_]].service_creator
            ],
    ...
    */
    function depositFund(uint256 _service_id) public payable {
        require(
            services[setIndexOfServiceId[_service_id]].customer_address ==
                msg.sender,
            "only this service id customer can call this methode"
        );
        require(
            msg.value ==
                services[setIndexOfServiceId[_service_id]].service_cost,
            "incorrect amount for service cost"
        );
    ...
    */
    function withdrawFund(
        uint256 _service_id,
        address _service_creater_address,
        uint256 _service_end_date
    ) public {
        address customer_address = services[setIndexOfServiceId[_service_id]]
            .customer_address;
        require(
            services[setIndexOfServiceId[_service_id]].service_creator ==
                msg.sender,
            "only service creator of this service id can call this methode"
        );
        require(
            acknoledgement[_service_id][customer_address] == true,
            "customer acknoledgement is not true"
        );
        require(
            acknoledgement[_service_id][_service_creater_address] == true,
            "service creater acknoledgement is not true"
        );
        require(
            services[setIndexOfServiceId[_service_id]].withdrawStatment != true,
            "service amount already withdraw"
        );
        services[setIndexOfServiceId[_service_id]]
            .service_end_date = _service_end_date;
        payable(_service_creater_address).transfer(
            services[setIndexOfServiceId[_service_id]].service_cost
        );
        services[setIndexOfServiceId[_service_id]].withdrawStatment = true;
    }
