...
contract NFT is Ownable, ERC721A, ReentrancyGuard {
  ...
  string private _defaultFileName;
  ...
  function setDefaultFileName(string memory fn) external onlyOwner {
    _defaultFileName = fn;
  }
    ...
    returns (string memory)
  {
    ...
    if (_enableShowTokenName(tokenId)) {
      ...
      return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _defaultFileName)) : "";
    }
  }

  function _enableShowTokenName(uint256 tokenId) private view returns(bool){
    if (tokenId < issueStartTokenId && issueNo > 1) {
      return true;    
    } else {
      return _blindBoxOpened[issueNo];
    }
  }
}
