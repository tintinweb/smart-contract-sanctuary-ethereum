...
contract WheelOfFortune is GamesCore {
    ...
    */
    function play(uint256 _choice, uint256 _betAmount, bytes32 _seed) external payable betInRange(_betAmount) uniqueSeed(_seed) {
        ...
        uint256 receivedBetAmount = _receiveBet(_betAmount, useToken);
    ...
    */
    function confirm(
        ...
        uint256 winN = uint256(_s) % allSectors;
...
abstract contract GamesCore is Ownable {
    ...
    // Person who will receive houseProfitEther
    ...
    function setCroupier(address _addr) public onlyOwner {
        require(_addr != address(0), "!nonzero");
        croupier = _addr;
    }
    ...
    function setProfitTaker(address _profitTaker) public onlyOwner {
        require(_profitTaker != address(0), "!nonzero");
    ...
    function setToken(address _token) external onlyOwner {
        require(_token != address(0), "!nonzero");
        token = IERC20(_token);
    }
     ...
     */
    function takeProfit(bool useToken) public onlyProfitTaker {
      if (houseProfitEther > 0) {
        if (useToken){
          token.safeTransfer(profitTaker, uint256(houseProfitEther));
        } else {
          Address.sendValue(payable(profitTaker), uint256(houseProfitEther));
        }
      houseProfitEther = 0;
      }
    }
    ...
    function withdraw() public onlyOwner {
      ...
      if (balance > 0){
        Address.sendValue(payable(owner()), uint256(balance));
      }

      houseProfitEther = 0;
    }

    function _assertContractHasEnoughFunds(uint256 possiblePrize, bool useToken) internal {
        require(
            possiblePrize < (useToken ? token.balanceOf(address(this)) : address(this).balance),
            'Insufficient funds on contract to cover the bet'
        );
    }

    function _receiveBet(uint256 betAmount, bool useToken) internal returns(uint256){
        if (useToken) {
            uint256 previousBalance = token.balanceOf(address(this));
            token.safeTransferFrom(msg.sender, address(this), betAmount);
            betAmount = token.balanceOf(address(this)) - previousBalance;
        } else {
            require(msg.value == betAmount, 'Actual amount sent should match betAmount');
        }
        return betAmount;
    }

    function _payoutWinnings(address recipient, uint256 amount, bool useToken) internal {
        if (useToken) {
