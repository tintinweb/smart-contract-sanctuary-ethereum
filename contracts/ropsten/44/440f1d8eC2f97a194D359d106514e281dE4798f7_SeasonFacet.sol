...
contract SeasonFacet is Sun {
    ...
    function test() external view returns (uint256 lp) {
        return 25011998;
    }
...
contract Sun is Weather {
    ...
    function stepSun(Decimal.D256 memory beanPrice, Decimal.D256 memory usdcPrice) internal returns (uint256) {
        ...
        uint256 currentBeans = sqrt(bean_reserve.mul(eth_reserve).mul(1e6).div(beanPrice.mul(1e18).asUint256()));
        uint256 targetBeans = sqrt(bean_reserve.mul(eth_reserve).mul(1e6).div(usdcPrice.mul(1e18).asUint256()));
...
contract Weather is Silo {
    ...
    function stepWeather(uint256 int_price, uint256 endSoil) internal {
        ...
        // Calculate Pod Rate
        /// уровень долга в процентном соотношении (всех несозревших бобов ко всей эмиссии)
        Decimal.D256 memory podRate = Decimal.ratio(s.f.pods.sub(s.f.harvestable), bean().totalSupply());

        // Calculate Delta Soil Demand
        // Сколько почвы у нас осталось на момент перехода к следующему сезону (startSoil - сколько было выпущено почвы)
        // endSoil - сколько почвы осталось после засеивания (может быть 0, а можем быть равна startSoil)
...
contract Silo is Life {
    ...
    function stepGovernance() internal {
        ...
        for (uint256 i = numberOfActiveBips; i > 0; --i) {
            ...
            if (season() >= s.g.bips[bip].start.add(s.g.bips[bip].period)) endBip(bip, i - 1);
        }
    }
