...
contract Marketplace is Ownable, ReentrancyGuard{
    ...
    function init(address _signerAddress, address _NFTAddress, address _platformAddress) public onlyOwner {
        require(!Initialized, "Contract already initialized!");
        require(_signerAddress != address(0), "ADDRESS_SIGNER_INVALID");
        ...
        Initialized = true;
        statusAdmin[_signerAddress] = true;
    ...
    function buyPack(uint256 packID, uint256 amount, uint256 pricePack, string memory transactionID, Sig memory buyPackRSV) external payable initializer {
        require(verifySigner(signerAddress, messageHash(abi.encodePacked(msg.sender, packID, amount, pricePack, transactionID)), buyPackRSV), "BuyPack rsv invalid");
    ...
    function mint(string memory pointerMoment, string memory transactionID, Sig memory mintRSV) public initializer{
        require(verifySigner(signerAddress, messageHash(abi.encodePacked(msg.sender, pointerMoment, transactionID)), mintRSV), "Sell rsv invalid");
        HDY721(NFTAddress).mint(msg.sender, pointerMoment);
        emit MintEvent(msg.sender, pointerMoment, transactionID, block.timestamp);
    }
    ...
    function sell(address nftAddress, uint256 tokenID, uint256 price, string memory transactionID, Sig memory sellRSV) public initializer onlyNFTOwner(nftAddress, tokenID){
        ...
        require(verifySigner(signerAddress, messageHash(abi.encodePacked(msg.sender, nftAddress, tokenID, price, transactionID)), sellRSV), "Sell rsv invalid");
    ...
    function buy(Buy memory buyStruct, Sig memory buyRSV) public payable initializer {
        ...
        require(verifySigner(signerAddress, messageHash(abi.encodePacked(msg.sender, buyStruct.nftAddress, buyStruct.tokenID, msg.value, buyStruct.transactionID, buyStruct.percIpHolder, buyStruct.ipHolderAddr, buyStruct.percProjOwn, buyStruct.projOwnAddr)), buyRSV), "Buy rsv invalid");
        require(priceNFT[buyStruct.nftAddress][buyStruct.tokenID] == msg.value, "MSG_VALUE is not match with listing price!");
        require(ERC721(buyStruct.nftAddress).ownerOf(buyStruct.tokenID) != msg.sender, "You can't buy your own NFT!");
    ...
    function cancelSell(address nftAddress, uint256 tokenID, string memory transactionID, Sig memory cancelRSV) public initializer onlyNFTOwner(nftAddress, tokenID) {
        require(verifySigner(signerAddress, messageHash(abi.encodePacked(msg.sender, nftAddress, tokenID)), cancelRSV), "Cancel rsv invalid");
    ...
    function changePrice(address nftAddress, uint256 tokenID, uint256 newPrice, string memory transactionID, Sig memory changePriceRSV) public initializer onlyNFTOwner(nftAddress, tokenID) {
        require(verifySigner(signerAddress, messageHash(abi.encodePacked(msg.sender,nftAddress, tokenID, newPrice)), changePriceRSV), "ChangePrice rsv invalid");
