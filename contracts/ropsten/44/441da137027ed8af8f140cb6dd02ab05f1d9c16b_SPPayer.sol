...
// File: contracts/structs/SPProfitDistribution.sol

pragma solidity 0.8.6;

struct SPProfitDistribution {
  uint duration;
}
...
interface ISPProjects {
  ...
  function createdAtOf(uint256 _projectId) external view returns (uint);

  function createFor(address _owner, SPProjectMetadata calldata _metadata, SPProfitDistribution calldata _profitDistribution)
    external
    returns (uint256 projectId);

  function setMetadataOf(uint256 _projectId, SPProjectMetadata calldata _metadata) external;

  function getProfitDistributionOf(uint256 _projectId) external view returns(SPProfitDistribution memory);
}
...
interface ISPPayer {
  ...
  function claim(uint256 _projectId, address _token) external;
}
...
contract SPPayer is ISPPayer {
  ...
  mapping(address => mapping(uint256 => uint)) public lastClaimedDateBy;

  mapping(address => mapping(uint256 => uint256)) public investedRate;
  ...
  function pay(uint256 _projectId, address _token, uint256 _tokenPrice, uint256 _amount) external override {
    ...
    //Add invested rate
    investedRate[msg.sender][_projectId] = paymentsOfHolders[msg.sender][_projectId] / ISPProjects(projects).crowdfundingAmount(_projectId);
    
    // Set the last claimed date as last paid date
    lastClaimedDateBy[msg.sender][_projectId] = block.timestamp;
  ...
  function claim(uint256 _projectId, address _token) external override {
    require(block.timestamp - lastClaimedDateBy[msg.sender][_projectId] >= ISPProjects(projects).getProfitDistributionOf(_projectId).duration);
    require(paymentsOfHolders[msg.sender][_projectId] > 0);
    IERC20(address(_token)).transfer(msg.sender, investedRate[msg.sender][_projectId] * ISPProjects(projects).crowdfundingAmount(_projectId) * (10 ** decimals));
    lastClaimedDateBy[msg.sender][_projectId] = block.timestamp;
  }
}