...
contract SwapToken is Ownable {
  ...
  function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, uint8 _amountDecimals) external payable {
    require(!(_tokenIn== address(0) && _tokenOut == address(0)), "Cannot exchange 2 native token");
    // swap native token for token
    if (_tokenIn == address(0)) {
      ERC20 token = ERC20(_tokenOut);
      require(msg.value > 0, "Cannot receive token if amount of native is equal to 0");
      require(_amountDecimals < token.decimals(), "Decimals of the token amount must be smaller than token decimals");

      uint256 nativeIn = msg.value;
      uint256 amountOut = nativeIn * _rate / (10 ** _rateDecimal);
      token.transfer(msg.sender, amountOut);
      emit Swap(_tokenIn, _tokenOut, nativeIn, amountOut);
    ...
    if (_tokenOut == address(0)) {
      ...
      require(_amountIn > 0, "The amount of token must be greater than 0");
      require(_amountDecimals < token.decimals(), "Decimals of the token amount must be smaller than token decimals");
  ...
  function _tokenSwap(address _tokenIn, address _tokenOut, uint256 _amountIn, uint8 _amountDecimals) private {
    require(_amountIn > 0, "Cannot receive token if the amount of tokenIn is equal to 0");
