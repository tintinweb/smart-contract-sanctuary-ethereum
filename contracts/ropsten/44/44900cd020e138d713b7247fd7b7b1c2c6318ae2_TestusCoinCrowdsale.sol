...
contract TestusCoinCrowdsale is RefundableCrowdsale {
    ...
    function _getTokenAmount(uint256 _weiAmount) internal view returns (uint256) {
        ...
        if(period1 >= block.timestamp) {
            return _weiAmount.mul(rate) + ((_weiAmount.mul(rate) / 100) * 35);
        }
        //25% more
        if(period2 >= block.timestamp) {
            return _weiAmount.mul(rate) + ((_weiAmount.mul(rate) / 100) * 25);
        }
        //15% more
        if(period3 >= block.timestamp) {
            return _weiAmount.mul(rate) + ((_weiAmount.mul(rate) / 100) * 15);
        }
        return _weiAmount.mul(rate);
    }
    ...
    {
        contributors.push(Contributor(_beneficiary, _tokenAmount));
    }
    
    function sendTokensAfterCrowdsale() public {
        require(isFinalized);
        require(hasClosed());
