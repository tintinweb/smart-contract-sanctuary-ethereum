...
error ExceedMaxMintPerTime();

/**
 * @title IEC Wolves Minter
 * @dev Contract for minting IEC Wolves ERC721
 * @custom:version v1.0
 * @custom:date 31 May 2022
 */
contract IECWolvesMinter is AccessControl, PaymentSplitter {
    ...
    uint256 public teamReserve = 125;

    // Public mint configuration
    bool public publicMintActive = false;
    uint256 public publicFee = 0.05 * 10**18; // 0.05 Ether (with 18 decimals)
    uint16 public maxPublicMintAtOnce = 20;

    // Allowlist mint configuration
    bool public allowlistMintActive = false;
    uint256 public allowlistFee = 0.05 * 10**18; // 0.05 Ether (with 18 decimals)
    uint16 public maxAllowlistMintAtOnce = 10;

    // Mapping of allowlist
    ...
    function publicMint(uint256 amount) external payable {
        ...
        if (msg.value < publicFee * amount) revert NotEnoughEtherSent();
        if (_iecwolves.totalSupply() + teamReserve + amount > maxTotalSupply) revert WouldExceedMaxSupply();
        if (amount > maxPublicMintAtOnce) revert ExceedMaxMintPerTime();

        // Collect payment
        payable(msg.sender).transfer(amount * publicFee);
    ...
    function allowlistMint(uint256 amount) external payable {
        ...
        if (msg.value < allowlistFee * amount) revert NotEnoughEtherSent();
        if (_iecwolves.totalSupply() + teamReserve + amount > maxTotalSupply) revert WouldExceedMaxSupply();
        if (amount > maxAllowlistMintAtOnce) revert ExceedMaxMintPerTime();

        // Collect payment
        payable(msg.sender).transfer(amount * allowlistFee);
    ...
    function _safeMint() internal returns (uint256 tokenId) {
        return _iecwolves.safeMint(msg.sender);
    }
    ...
    function flipPublicMintActive() external onlyRole(STAFF_ROLE) {
        publicMintActive = !publicMintActive;
    }

    function flipAllowlistMintActive() external onlyRole(STAFF_ROLE) {
    ...
    function setPublicMintFee(uint256 _publicFee) external onlyRole(STAFF_ROLE) {
        publicFee = _publicFee;
    }

    function setAllowListMintFee(uint256 _allowlistFee) external onlyRole(STAFF_ROLE) {
        allowlistFee = _allowlistFee;
        allowlistFee = _allowlistFee;
    }

    function setTeamReserve(uint256 _teamReserve) external onlyRole(STAFF_ROLE) {
        teamReserve = _teamReserve;
    }

    function setMaxPublicMintAtOnce(uint16 _maxPublicMintAtOnce) external onlyRole(STAFF_ROLE) {
        maxPublicMintAtOnce = _maxPublicMintAtOnce;
    }

    function setMaxAllowlistMintAtOnce(uint16 _maxAllowlistMintAtOnce) external onlyRole(STAFF_ROLE) {
        maxAllowlistMintAtOnce = _maxAllowlistMintAtOnce;
    }

    function withdrawToken(address tokenContractAddress, uint256 amount) external onlyRole(STAFF_ROLE) {
