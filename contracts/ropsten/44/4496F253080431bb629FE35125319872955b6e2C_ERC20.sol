...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable{
    ...
    uint256 openingTime = 1649350558; //TEMP 1650427200; //epoch time april 20 12AM
    uint256 closingTime = openingTime + 86400*5; //TEMP 1650427200; //epoch time april 20 12AM
    ...
    constructor(address payable mktDevAddress_, address payable treasuryAddress_) payable {
        ...
        _transfer(owner(),address(this),_contractStartingLiquidity);
    }
    ...
    function _transfer(
        ...
        if ((postPresaleComplete == false) && (block.timestamp > closingTime)){
    ...
    function postPresale() public onlyOwner {
        ...
        addLiquidity(_presaleRate*half, half);
        drainEther(address(this).balance);
        drainTokens(balanceOf(address(this)));
    }
    ...
    function buyBackAndBurn() external payable onlyOwner {
        ...
        uint256 diffBalance = balanceOf(owner())-initialBalance;

        _burn(owner(),diffBalance);
    }
    ...
    function getPresaleValues() public view returns(uint256,uint256,uint256,uint256,uint256,uint256) {
        return (_presaleTokenLimit,_presaleRate,_presaleTokensSold,openingTime,closingTime,minInvestment);
    }
