...
contract ScriptTV {
     ...
     */
    event GemMinted(address indexed user, uint8 indexed gemType);
    ...
    function mintGlasses(uint8 _type) external {
        ...
        scriptGlasses.safeMint(msg.sender, _type);
        glasses[_type].mintedSupply++;
        spay.burnFrom(msg.sender, cost);
        _lockTokens(collateral);

        emit GlassMinted(msg.sender, _type, glasses[_type].mintedSupply);
    }
    ...
    function mintGem(uint8 _type) external {
        ...
        gem.safeMint(msg.sender, _type);
        spay.burnFrom(msg.sender, cost);

        emit GemMinted(msg.sender, _type);
    }
...
interface IScriptGlasses is IERC721A {
     ...
     * @dev Only callable by Owner, will be called by low level call function
     */
    function safeMint(address to, uint8 _type) external;
}
