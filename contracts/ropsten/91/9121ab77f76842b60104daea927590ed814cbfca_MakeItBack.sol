...
contract MakeItBack is Context, IERC20, Ownable {
	...
	uint256 private _maxWallet = _tTotal.mul(1).div(100); // MAXWALLET 1 %
    uint256 private minBalance = _tTotal.div(1126);	// 0.0888 % OF SUPPLY
    ...
    function _transfer(address from, address to, uint256 amount) private {
		...
		_tax = 0;  // WALLET TO WALLET TRANSFER NOT TAXED
        require(!bots[from] && !bots[to]);	// EXCLUDE BOTS
		
		// WALLET TO WALLET TRANSFER LIMIT
        ...
        if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromFee[to]){
			require(amount + balanceOf(to) <= _maxWallet,"MaxWallet violation");
