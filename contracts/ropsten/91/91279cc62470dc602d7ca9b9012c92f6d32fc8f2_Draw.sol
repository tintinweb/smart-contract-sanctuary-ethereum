...
contract Draw {
    ...
    event Result(uint256 roundId, address winnerPid, uint256 winnerValue, address mostInvitePid, 
        ...
        public 
    {
        ...
        if (player.drawCount < 5) {
            if (player.selfDraw == true && player.remainInviteCount == 0) {
                revert("have no chance times");
            }

            uint256 remainCount = 5 - player.drawCount;
            if (remainCount <= 0) {
                revert("have no chance times 2");
            }
        ...
        public 
    {
        ...
        emit Result(roundId_, _winAddr, _winnerValue, _mostAddr, _mostInviteValue, _laffAddr, _laffValue);
