...
import "./extensions/ERC1155MintPausable.sol";
import "./extensions/ERC1155OpenSeaGassLess.sol";
import "./extensions/ERC1155Royalty.sol";
import "./extensions/ERC1155Whitelist.sol";

contract CryptoTravelClub is
  ERC1155,
  Ownable,
  Pausable,
  ERC1155Supply,
  ERC1155MintPausable,
  ERC1155OpenSeaGassLess,
  ERC1155Royalty,
  ERC1155Whitelist,
  ReentrancyGuard
{
  ...
  uint256 public constant MAX_SUPPLY_SAPPHIRE = 5000;
  uint256 public constant MAX_SUPPLY_PLATINUM = 2000;
  uint256 public constant MAX_SUPPLY_DIAMOND = 500;

  uint256 public constant PRICE_IN_WEI_SAPPHIRE = 0.05 ether;
  uint256 public constant PRICE_IN_WEI_PLATINUM = 0.1 ether;
  uint256 public constant PRICE_IN_WEI_DIAMOND = 0.15 ether;

  uint256 public constant MAX_MINT_PER_WALLET = 3;

  uint96 public constant ROYALTIES_IN_BASIS_POINTS = 500;

  bool public _whiteListMintOpen = false;
  ...
  constructor(
  ...
  ) ERC1155(uri_) ERC1155OpenSeaGassLess(openSeaProxyRegistryAddress_) {
    ...
    _mint(wallet_, 0, 50, ""); // Sapphire
    _mint(wallet_, 1, 50, ""); // Platinum
    _mint(wallet_, 2, 50, ""); // Diamond
  }
  ...
  function mintAllowance(address account) external view returns (uint256) {
    return MAX_MINT_PER_WALLET - _addressToMinted[account];
  }
  ...
  function mint(
    ...
    uint256 amount,
    bytes memory data
  ) private whenMintNotPaused {
    if (amount == 0) {
      return;
    }
    require(id <= 2, "Invalid id");
    require(_addressToMinted[_msgSender()] + amount <= MAX_MINT_PER_WALLET, "Exceeds max per wallet");
    require(
      totalSupply(id) + amount <= (id == 0 ? MAX_SUPPLY_SAPPHIRE : id == 1 ? MAX_SUPPLY_PLATINUM : MAX_SUPPLY_DIAMOND),
      "Exceeds max supply"
    );
    _addressToMinted[_msgSender()] += amount;
  ...
  function whiteListMint(
    ...
    require(_whiteListMintOpen, "Whitelist not active");
    require(_validateWhitelistMerkleProof(MAX_MINT_PER_WALLET, proof), "Invalid proof");
    require(
      (sapphire * PRICE_IN_WEI_SAPPHIRE) + (platinum * PRICE_IN_WEI_PLATINUM) + (diamond * PRICE_IN_WEI_DIAMOND) ==
        msg.value,
      "Invalid funds provided"
    );
    mint(_msgSender(), 0, sapphire, "");
    mint(_msgSender(), 1, platinum, "");
    mint(_msgSender(), 2, diamond, "");
  }
  ...
  function publicMint(
    ...
    require(_publicMintOpen, "Public not active");
    require(
      (sapphire * PRICE_IN_WEI_SAPPHIRE) + (platinum * PRICE_IN_WEI_PLATINUM) + (diamond * PRICE_IN_WEI_DIAMOND) ==
        msg.value,
      "Invalid funds provided"
  ...
  function toggleWhiteListMint() external onlyOwner {
    _whiteListMintOpen = !_whiteListMintOpen;
  }

  function togglePublicMint() external onlyOwner {
    _publicMintOpen = !_publicMintOpen;
  }
  ...
  function _beforeTokenTransfer(
    ...
    uint256[] memory amounts,
    bytes memory data
  ) internal override(ERC1155, ERC1155Supply, ERC1155MintPausable) whenNotPaused {
    super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
  }
  ...
  function pauseMint() external whenMintNotPaused onlyOwner {
    _pauseMint();
  }

  function unpauseMint() external whenMintPaused onlyOwner {
    _unpauseMint();
  }
  ...
  function setOpenSeaProxyRegistryAddress(address openSeaProxyRegistryAddress_) external onlyOwner {
    _setOpenSeaProxyRegistryAddress(openSeaProxyRegistryAddress_);
  }

  function isApprovedForAll(address _owner, address operator)
    public
    view
    override(ERC1155, ERC1155OpenSeaGassLess)
    returns (bool)
  {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155, ERC1155Royalty) returns (bool) {
    return
      ...
      interfaceId == type(ERC1155MintPausable).interfaceId ||
      interfaceId == type(ERC1155OpenSeaGassLess).interfaceId ||
      interfaceId == type(ERC1155Royalty).interfaceId ||
      interfaceId == type(ERC1155Whitelist).interfaceId ||
      super.supportsInterface(interfaceId);
  }
}
...
abstract contract ERC1155MintPausable is ERC1155 {
  ...
  modifier whenMintPaused() {
    require(mintPaused(), "Mint not paused");
    _;
  }
  ...
  function _beforeTokenTransfer(
    ...
    super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
    require((!mintPaused() || from != address(0)), "Minting is disabled");
  }
}
...
abstract contract ERC1155Whitelist is Context {
  ...
  function _verify(bytes32 leaf, bytes32[] memory proof) internal view returns (bool) {
    require(_whitelistMerkleRoot != 0, "No merkle root");
    return MerkleProof.verify(proof, _whitelistMerkleRoot, leaf);
  }

  function getAllowance(string memory allowance, bytes32[] calldata proof) external view returns (string memory) {
    string memory payload = string(abi.encodePacked(_msgSender()));
    require(_verify(_leaf(allowance, payload), proof), "Invalid proof");
    return allowance;
  }
