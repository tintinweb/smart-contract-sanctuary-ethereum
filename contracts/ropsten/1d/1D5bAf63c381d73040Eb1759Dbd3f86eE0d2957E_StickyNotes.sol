...
contract StickyNotes is ERC721, ERC721Enumerable, AccessControl, Ownable {
  ...
  function getSvg(uint256 tokenId) public view returns (string memory) {
    ...
    parts[
      4
    ] = "</style><foreignObject x='76' y='130' width='490' height='420'><div xmlns= 'http://www.w3.org/1999/xhtml%22%3E'><p id='text' class='bodyText'></p><script>var input = '";
    parts[5] = Base64.encode(abi.encode(message));
    parts[
      6
    ] = "' let test = atob(input) document.querySelector('.bodyText').innerText = test;</script></div></foreignObject></svg>";
    return
      Base64.encode(
        abi.encodePacked(
          parts[0],
