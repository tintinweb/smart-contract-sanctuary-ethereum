...
contract AccessoryLayer is BaseIlluvitar {
    ...
    function _parseBlueprint(bytes memory blueprint) private pure returns (uint8 tier, AccessoryType accessoryType) {
        uint8 accessoryTypeUint8;
        uint8 p;

        p = _skipNonDecimal(blueprint, p);
        (tier, p) = _atoi(blueprint, p, 2);
        (accessoryTypeUint8, p) = _atoi(blueprint, p, 1);
        accessoryType = AccessoryType(accessoryTypeUint8);
        p = _skipNonDecimal(blueprint, p);
        require(p == blueprint.length, "Wrong blueprint format");
    }

    /**
     * @dev Returns the decimal representation of byte char at a given index and the incremented index
     *
     ...
     * @param increment the amount to increment the provided index
     * @return i converted decimal number char to uint8
     * @return p incremented index
     */
    function _atoi(
        bytes memory a,
        uint8 index,
        ...
        return (uint8(a[index]) - 0x30, index + increment);
    }
