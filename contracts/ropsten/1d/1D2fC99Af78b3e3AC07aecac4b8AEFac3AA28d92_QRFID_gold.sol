...
contract QRFID_gold is IERC1400, Ownable {
    ...
    function setDocument(
        ...
        require(
            _docName != bytes32(0),
            "Document: Zero address is not allowed"
        );
        require(bytes(_uri).length > 0, "Document: Should not be a empty uri");
        require(_documentHash != bytes32(0), "Document: Zero hash provided");
