...
contract Minter is AccessControlEnumerableUpgradeable, Provenance, PaymentSplitterUpgradeable, IMintable {
    ...
    uint256 public marketTax;          // tax when buying from game market
        ...
        payable
    {
        ...
        if (tokenPayment == address(0)) {
            ...
            payable(receiver).call{value: (msg.value)*(100 - marketTax)/(100)}("");
        } else {
            IERC20Upgradeable(tokenPayment).safeTransferFrom(msg.sender, address(this), price*marketTax/100);
            IERC20Upgradeable(tokenPayment).safeTransferFrom(msg.sender, receiver, price*(100 - marketTax)/100);
        }
