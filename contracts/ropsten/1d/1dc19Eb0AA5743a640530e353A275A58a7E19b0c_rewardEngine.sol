...
contract rewardEngine {
    ...
    function walletToBOB(uint _walletType, uint _numOfWalletsSold, address _BOBaddress) public {
        ...
        if (_walletType == REALid) {
            uint _amountOfBOB = (REALrate * _numOfWalletsSold * (10 ** 18));
            IBOBtoken(_BOBaddress).mintBOB(msg.sender, _amountOfBOB);
        } else if (_walletType == GOLDid) {
            uint _amountOfBOB = (GOLDrate * _numOfWalletsSold * (10 ** 18));
            IBOBtoken(_BOBaddress).mintBOB(msg.sender, _amountOfBOB);
        } else if (_walletType == PROid) {
            uint _amountOfBOB = (PROrate * _numOfWalletsSold * (10 ** 18));
            IBOBtoken(_BOBaddress).mintBOB(msg.sender, _amountOfBOB);
        } else {
            uint _amountOfBOB = (PURErate * _numOfWalletsSold * (10 ** 18));
            IBOBtoken(_BOBaddress).mintBOB(msg.sender, _amountOfBOB);
        }
    ...
    function BOBtoJUL(uint _amountInBOB, address _BOBaddress, address _JULaddress) public {
        ...
        uint _amountInBOBAdjusted = _amountInBOB * (10 ** 18);
        uint amountInJUL = _amountInBOBAdjusted / 2;

        IBOBtoken(_BOBaddress).burnDRNK(msg.sender, _amountInBOBAdjusted);
