...
contract LaunchpadFactoryFairLaunch is ReentrancyGuard, Ownable, Pausable {
    ...
    function createLaunch(
        ...
        if (par.saleType == 3) {
            ...
            require(IERC20(token).transferFrom(msg.sender, _burnAddress, extraToken), "Transfer Failed");
            if (raiseIn != address(0)) {
                IApolloSafePlay(token).setRaiseInAddress(raiseIn);
            } else {
                IApolloSafePlay(token).setRaiseInAddress(IUniswapV2Router02(router).WETH());
            }
        }
