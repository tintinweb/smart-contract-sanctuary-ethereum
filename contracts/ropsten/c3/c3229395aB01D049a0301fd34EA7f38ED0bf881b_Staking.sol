...
contract Staking is Ownable, ReentrancyGuard {
    ...
    bool public timeSet = false; //BOOLEAN FOR SET THE TIME
    ...
    mapping(address => uint256) private stakedTime; // stake = {};
    ...
    function showReward(address sender) public view returns (uint256 shares) {
        uint256 percentShares;
        uint256 profit = stakeProfit[sender];
        for (uint256 i = 0; i < endWitdrawTimestamps.length; i++) {
            if (endWitdrawTimestamps[i] > stakedTime[sender]) {
                percentShares = calculate(profit, allStakedBalances[i]);
            ...
            shares += Math.ceilDiv(
                (calculate(profit, allStakedBalance) * StakedReward),
                100 * 10**18
            );
        }
    ...
    function _getReward(address sender) internal view returns (uint256) {
        uint256 shares;
        uint256 percentShares;
        for (uint256 i = 0; i < endWitdrawTimestamps.length; i++) {
            if (endWitdrawTimestamps[i] > stakedTime[sender]) {
    ...
    {
        valueLpPerBusd = IStaking(0x5af257437EeF4df9Cf4967e1c15e49adBC37CCF6)
            .calculateValue(_LpTokenAmount);
    }
        ...
        )
    {
        ...
        value = calculateValue(shares);
        reward = showReward(msg.sender);
    }
...
interface IStaking {
    ...
    function AllPositions(address _account)
        external
        view
        returns (getPositions[] memory);

    // mapping(address => getPositions[]) public AllPositions;
