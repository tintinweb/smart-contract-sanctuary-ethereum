...
// imported contracts/proposals/OCP-IP-10/DataStreamRegistrySealer.sol
...
// imported contracts/proposals/OCP-IP-7/LicenseNFTSealer.sol
...
// imported contracts/proposals/OCP-IP-10/DataStreamRegistry.sol
...
// imported contracts/proposals/OCP-IP-10/IDataStreamRegistry.sol
contract IDataStreamRegistry {
  event DataStreamAdded(bytes32 hash, uint256 updatedAt);
  event DataStreamLicenseTermsAdded(bytes32 hash, bytes32 licenseTerms, uint256 updatedAt);
  event DataStreamLicenseTermsChanged(bytes32 hash, bytes32 licenseTerms, uint256 updatedAt);
  event DataStreamLicenseTermsRemoved(bytes32 hash, bytes32 licenseTerms, uint256 updatedAt);
  event DataStreamHeartBeat(bytes32 hash, int longitude1e9, int latitude1e9, uint256 updatedAt);
...
contract DataStreamRegistry is Secondary, IDataStreamRegistry {
  ...
  function hashDataStream(
    ...
    return keccak256(abi.encodePacked(creator, name, schema));
  }
  function addDataStream(address creator, string name, bytes32 schema) public {
    bytes32 hash = hashDataStream(creator, name, schema);
    ...
    emit DataStreamAdded(hash, now); // solhint-disable-line not-rely-on-time
  }
  function addLicenseTerms(
    bytes32 hash,
    bytes32 licenseTerms,
  ...
  function _addLicenseTerms(
    ...
    emit DataStreamLicenseTermsAdded(hash, licenseTerms, now); // solhint-disable-line not-rely-on-time
  }
  function _changeLicenseTerms(
    ...
    emit DataStreamLicenseTermsChanged(hash, licenseTerms, now); // solhint-disable-line not-rely-on-time
  }
  function _removeLicenseTerms(bytes32 hash, bytes32 licenseTerms) internal {
    ...
    emit DataStreamLicenseTermsRemoved(hash, licenseTerms, now); // solhint-disable-line not-rely-on-time
  }
  function _emitHeartBeat(bytes32 hash, int longitude1e9, int latitude1e9) internal {
    require(_hasDataStream[hash]);
    emit DataStreamHeartBeat(hash, longitude1e9, latitude1e9, now); // solhint-disable-line not-rely-on-time
  }
  function _emitRecord(
    ...
    require(_hasDataStream[hash]);
    emit DataStreamRecord(hash, startTimeUtcSec, endTimeUtcSec, numAtoms, now); // solhint-disable-line not-rely-on-time
  }
  function _emitFuture(
    ...
    require(_hasDataStream[hash]);
    emit DataStreamFuture(hash, startTimeUtcSec, endTimeUtcSec, estimatedNumAtoms, now); // solhint-disable-line not-rely-on-time
  }
}

// imported contracts/proposals/OCP-IP-10/IDataStreamRegistrySealer.sol
contract IDataStreamRegistrySealer is ILicenseNFTSealer {
  ...
  // keccak256(abi.encodePacked("dataStreamLicenseTermsHash"))
  bytes32 public constant DATA_STREAM_LICENSE_TERMS_HASH_KEY = 0x182d56aafb65a8251df9c8416d0de673dc1bee09fb3d9f84739e1bd95d304857;
  ...
  event DataStreamLicenseSealed(bytes32 hash, uint256 tokenId, uint256 updatedAt);
...
contract DataStreamRegistrySealer is DataStreamRegistry, IDataStreamRegistrySealer, LicenseNFTSealer {
  ...
  function validateTokenProperties(uint256 tokenId) public view returns(bool) {
    bytes32 mintedSchema = _licenseNFT.mintableProperties(tokenId, SCHEMA_KEY);
    bytes32 mintedLicenseTerms = _licenseNFT.mintableProperties(tokenId, LICENSE_TERMS_KEY);
    uint256 duration = uint256(_licenseNFT.mintableProperties(tokenId, _licenseNFT.DURATION_SEC_KEY()));
    bytes32 hash = _licenseNFT.sealableProperties(tokenId, DATA_STREAM_HASH_KEY);
    uint256 startTimeUtcSec = uint256(_licenseNFT.sealableProperties(tokenId, START_TIME_UTC_SEC_KEY));
    bytes32 dataStreamLicenseTermsHash = _licenseNFT.sealableProperties(tokenId, DATA_STREAM_LICENSE_TERMS_HASH_KEY);
    // todo: valdiate license min and max costs
    // deal with refunding and balance transfers in a different function...
    return hasDataStream(hash) &&
      startTimeUtcSec > 0 &&
      mintedSchema == schema(hash) &&
      hasLicenseTerms(hash, mintedLicenseTerms) &&
      hashLicenseTerms(hash, mintedLicenseTerms) == dataStreamLicenseTermsHash &&
      duration > 0;
  }
  function onSealed(
    ...
    bytes32 hash = _licenseNFT.sealableProperties(tokenId, DATA_STREAM_HASH_KEY);
    emit DataStreamLicenseSealed(hash, tokenId, now);  // solhint-disable-line not-rely-on-time
