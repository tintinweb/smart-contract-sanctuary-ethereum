...
contract ERC20 is Context, IERC20, Ownable {
    ...
    uint256 startLimit = 1500 * 10 ** decimals;
    ...
    event Recommender(address indexed addr, address indexed recommender);
    event Trans(address indexed from, address indexed to, uint256 amount);
    event Level(uint256 level, uint256 totalRate);
    event Fee(uint256 marketFee, uint256 marketingFee, uint256 lpFee, uint256 lpShareFee, uint256 burnFee, uint256 bonusFee);
    event LP(uint256 lpToken, uint256 lpEth, uint256 lpShareToken, uint256 lpShareEth);
    ...
    function setStep(bool _step1, bool _step2) public onlyOwner {
        step1 = _step1;
        step2 = _step2;
    }
    ...
    function _transfer(
        ...
        if (startTime + 3600 < block.timestamp) {
            require(_balances[to] + finalAmount <= startLimit, "ERC20: Limit 1500");
        }
    ...
    function _countFee(address from, address addr, uint256 amount) private returns(uint256 finalAmount) {
        ...
        if (debug) {
            emit Fee(marketFee, marketingFee, lpFee, lpShareFee, burnFee, bonusFee);
        }
    ...
    function _swapForLiquidity() private swapLock {
        ...
        uint256 lpShareEth = newEth * lpShareRate / (lpRate + lpShareRate);
        uint256 lpEth = newEth - lpShareEth;

        if (step1) {
            _shareLiquidity(lpShareToken, lpShareEth);
        }
        ...
        if (debug) {
            emit LP(lpToken, lpEth, lpShareToken, lpShareEth);
        }
    }
    ...
    function _shareLiquidity(uint256 tokenAmount, uint256 ethAmount) public {
        if (tokenAmount == 0) {
            return;
        }
