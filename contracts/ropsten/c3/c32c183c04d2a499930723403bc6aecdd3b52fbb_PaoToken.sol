...
contract PaoToken is Lockable {
    ...
    ConsumeToken public consumeToken;  //consume token
    ...
    function _buyToken(address _to, uint256 _value) internal {
        ...
        uint256 _bonusConsumeToken = 1000 * 10 ** uint256(decimals);
        consumeToken.giveBonus(_to, _bonusConsumeToken);
    ...
    function setConsumeToken(address _tokenAddress) external onlyOwner {
...
contract ConsumeToken is Lockable {
    ...
    function _buyToken(address _to, uint256 _value) internal {
        ...
        require (balances[_to].add(_value) >= balances[_to]);       // Check for overflows
        balances[fundWallet] = balances[fundWallet].sub(_value);    // Subtract from the sender
        balances[_to] = balances[_to].add(_value);                  // Add the same to the recipient
