...
contract NoseStaking {
    ...
    function withdraw(uint40 index) external {
        ...
        emit data(time_end, from, to);
        
        if(from < to) {
            withdrawAmount = dep.staked_val.div(time_end-dep.time).mul(to - from).mul(1000);
        }
        ...
        //save staked_value in coinlist
        uint8 _index=coins_length;
        for (uint8 i=0;i<coins_length;i++) {
            if (strcmp(coins[i].symbol,dep.coin_symbol)==true) {
                _index=i;
                break;
            }
        }
        coins[_index].staked_value += withdrawAmount;
        //save withdraw
