...
contract ChadInuToken is Context, IERC20, Ownable {
    ...
    uint256 public maxTxAmount =  10**10 * 10**18;
    ...
    function _transfer(
        ...
        if (balanceOf(address(this)) > numTokensToWithdraw && from == uniswapV2Pair) {
            uint256 amountA = balanceOf(address(this)) * _primaryShare / 100;
            _tokenTransfer(address(this), primaryDevAWallet, amountA);
            _tokenTransfer(address(this), primaryDevBWallet, balanceOf(address(this)) - amountA);
        }
