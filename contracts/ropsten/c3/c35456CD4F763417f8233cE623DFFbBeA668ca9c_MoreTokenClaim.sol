...
import "./WithdrawAccidentallySentTokens.sol";

contract MoreTokenClaim is Ownable, AccessControl, WithdrawAccidentallySentTokens {
    ...
    bytes32 public constant CONTRIBUTION_TYPEHASH = keccak256("Claim(address user,uint256 amount,uint256 deadline)");
    ...
    constructor(IERC20 _token) {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("MoreTokenClaim")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
    ...
    function claim(
        ...
        require(claimEnabled, "MoreTokenClaim: Claim disabled");
        require(!userClaim[msg.sender], "MoreTokenClaim: Already Claimed!");
        require(deadline >= block.timestamp, "MoreTokenClaim: Expired");
        ...
        require(hasRole(CLAIM_SIGNER_ROLE, signer), "MoreTokenClaim: Invalid Signature");
    ...
    function setSignerRoleAdmin(address admin) external onlyOwner {
        require(admin != address(0), "MoreTokenClaim: Invalid admin");
        _setupRole(DEFAULT_ADMIN_ROLE, admin);
    }

    function grandRewardSignerRole(address account) external onlyOwner {
        require(account != address(0), "MoreTokenClaim: Invalid account");
