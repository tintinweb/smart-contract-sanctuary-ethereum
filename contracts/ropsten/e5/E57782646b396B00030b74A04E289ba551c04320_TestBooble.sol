...
contract TestBooble is IERC721, Ownable, Functional {
    ...
    constructor() {
        _name = "Test Booble";
        _symbol = "TBNFT";
        _baseURI = "https://chameleoncollective.io/metadata/";
        _hideTokens = true;
        
        totalTokens = 8008; // 0-9999
        price = 10 * (10 ** 15); // Replace leading value with price in finney 
        pricePublic = 20 *(10 ** 15);      
        discountPrice = 15 * (10 ** 15);
        reservedTokens = 200; // reserved for giveaways and such
        maxPerTxn = 50;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == TestBooble.onERC721Received.selector;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        address cmanager        = payable(0x7a84CDa5F9EB2b50eD2d64a9D8feE0E2e332B5F8);
        address founder         = payable(0x7a84CDa5F9EB2b50eD2d64a9D8feE0E2e332B5F8);
        address coo             = payable(0x7a84CDa5F9EB2b50eD2d64a9D8feE0E2e332B5F8);
        address marketing       = payable(0x7a84CDa5F9EB2b50eD2d64a9D8feE0E2e332B5F8);
        address dev             = payable(0x7a84CDa5F9EB2b50eD2d64a9D8feE0E2e332B5F8);
        address community       = payable(0x7a84CDa5F9EB2b50eD2d64a9D8feE0E2e332B5F8);
    ...
    function FreeMint() external reentryLock {
        require(totalTokens >= numberMinted, "sold out");
        require(mintActive,"Mint is not Active");
        require(numberMinted <  500,"Cannot Mint free token after 10 Token ID");
        uint256 mintSeedValue = numberMinted;
        numberMinted += 1;
        _safeMint(msg.sender, mintSeedValue );
        _tokensMintedby[_msgSender()] ++;
    }
    
    /*Function to mint token with 0.01 ETH price utpo 4003 
      And apply mint for 1 token after 3083 token ID*/

    function FirstPublicMint(uint256 qty) external payable reentryLock {
        require(totalTokens >= qty + numberMinted, "sold out");
        require(numberMinted + qty  >= 500 && numberMinted + qty < 4004,"Cannot Mint token id greater than 20");
        require(qty <= maxPerTxn, "max 50 per txn");
        require(mintActive, "Mint is not active");
        require(msg.value == qty * price, "Wrong Eth Amount");
        if(numberMinted + qty >= 3083)
        {
            require(qty == 1, "Cannot mint more than 1 qty after token ID 17");
    ...
    /*Function to mint token with 0.02 ETH price utpo 8008 
      And apply discount  token price to .015 if qty is more than 10*/
    
    function SecondMint(uint256 qty) external payable reentryLock {
        require(totalTokens >= qty + numberMinted, "sold out");
        require(qty <= maxPerTxn, "max 50 per txn");
        require(numberMinted + qty >= 4004 && numberMinted + qty <= 8008, "Cannot call this mint with ID greater than 25");
        require(mintActive,"Mint is not Active");
        uint256 refundPrice;
        if(qty > 10 )
        {
        require(msg.value == qty * discountPrice, "Wrong Eth Amount");
        refundPrice = discountPrice;
        }
        else
        {
        require(msg.value == qty * pricePublic, "Wrong Eth Amount");
        refundPrice = pricePublic;
        }
