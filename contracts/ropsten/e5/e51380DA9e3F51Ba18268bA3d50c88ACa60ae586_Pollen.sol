    ...
    PausableUpgradeable
{
    ...
    mapping(uint => Months) public months;
    ...
    function stake(
        ...
        months[currentMonth] = Months(currentMonth, amount);
    ...
    for (uint y = 0; y < totalNFT.length; y++) { 

        for (uint i = 0; i < totalTVL.length; i++) {
          
          if(month == totalTVL[i]) {
            _totalAmountbyMonth += months[month].amount;
            }
         }
        }
