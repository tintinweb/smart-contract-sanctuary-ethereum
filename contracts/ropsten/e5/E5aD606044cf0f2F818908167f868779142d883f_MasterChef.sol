...
// MasterChef เป็นปรมาจารย์แห่ง Glide เขาสามารถสร้าง Glide และเขาเป็นคนยุติธรรม
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// สังเกตว่ามันเป็นเจ้าของได้และเจ้าของใช้พลังมหาศาล ความเป็นเจ้าของ

// will be transferred to a governance smart contract once GLIDE is sufficiently
// จะถูกโอนไปยังสัญญาอัจฉริยะในการกำกับดูแลเมื่อ GLIDE เพียงพอ

// distributed and the community can show to govern itself.
// แจกจ่ายและชุมชนสามารถแสดงเพื่อปกครองตนเองได้

//
// Have fun reading it. Hopefully it's bug-free. God bless.
//ขอให้สนุกกับการอ่านนะครับ หวังว่ามันจะปราศจากข้อผิดพลาด พระเจ้าอวยพร.
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount; // How many LP tokens the user has provided. จำนวนโทเค็น LP ที่ผู้ใช้ระบุ
        uint256 rewardDebt; // Reward debt. See explanation below. หนี้ตอบแทน. ดูคำอธิบายด้านล่าง
        //
    ...
    uint256 public constant MAX_EMISSION_RATE = 5 ether; // Safety check ตรวจสอบความปลอดภัย 

    // Bounus reduction period ระยะเวลาในการลดโบนัส
    uint256 public constant bonusPeriod = 1572480; //12 * 60 * 24 * 91 (91 is 3 months in days)
    // Reduction period ระยะเวลาในการลด 
    ...
    // คะแนนการจัดสรรทั้งหมด จะต้องเป็นผลรวมของคะแนนการจัดสรรทั้งหมดในกลุ่มทั้งหมด
    uint256 public totalAllocPoint = 0;
    // Max allocation points safety check
    // ตรวจสอบความปลอดภัยคะแนนการจัดสรรสูงสุด
    uint256 public constant MAX_ALLOC_POINT = 100000;
    // The block number when GLIDE mining starts.
    // หมายเลขบล็อกเมื่อเริ่มการขุด GLIDE
    ...
    function phase(uint256 blockNumber) public view returns (uint256) {
        ...
        if (blockNumber >= startBlock) {
            ...
            // คำนวณหมายเลขบล็อกจากบล็อกเริ่มต้น
            ...
            // ถ้า blockNumberFromStart น้อยกว่า บล็อคช่วงลดระยะเวลา แสดงว่าเราอยู่ในเฟสเดียว

            if (blockNumberFromStart <= bonusPeriod) {
                return 1;
            } else {
                // calculate phase and add 1, because, will be start from one, and one phase is bonus phase
                // คำนวณเฟสและเพิ่ม 1 เพราะจะเริ่มต้นจากหนึ่งและหนึ่งเฟสเป็นโบนัสเฟส
                return
    ...
    function rewardPerPhase(uint256 phaseNumber) public view returns (uint256) {
        ...
        // หากมากกว่า 25 จะเป็นข้อผิดพลาดล้น (เช่นใน 25 เฟสแรกเราจะแจกจ่ายโทเค็นทั้งหมด)
    ...
    // อัปเดตจุดการจัดสรร GLIDE ของพูลที่ระบุ เจ้าของสามารถเรียกได้เท่านั้น
