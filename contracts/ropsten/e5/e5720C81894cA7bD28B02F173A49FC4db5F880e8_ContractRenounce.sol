...
contract ContractRenounce {
    ...
    mapping (address => string) private contractLockState;
    mapping (address => uint256) private contractLockTime;
    ...
    function lockContract( address _contractAddress, uint _timeLock ) payable public {
        ...
        contractLockState[_contractAddress] = "Locked";
        contractLockTime[_contractAddress] = block.timestamp + _timeLock;
        payFee(_lockFee, _feeWallet);
    }

    function extendLock( address _contractAddress, uint _timeLock ) payable public {
        require( msg.value == _lockFee, "ERROR: Fee not paid!" );
        for ( uint256 i = 0 ; i < contractLocks[msg.sender].lock.length; i++ ){
            if ( contractLocks[msg.sender].lock[i].contractAddress == _contractAddress && contractLocks[msg.sender].lock[i].currentOwner == address(this) ){
                address _contractOwner = getOwner( _contractAddress );
                require( _contractOwner == address(this), "ERROR: Contract Lock dApp cannot extend a lock for a contract it does not own!" );
                contractLocks[msg.sender].lock[i].unlockTime += _timeLock;
                contractLockState[_contractAddress] = "Locked";
                contractLockTime[_contractAddress] = contractLocks[msg.sender].lock[i].unlockTime;
                payFee(_lockFee, _feeWallet);
                break;
            }
        }
    ...
    function returnContract( address _contractAddress ) payable public returns (bool _success){
        ...
        for ( uint256 i = 0 ; i < contractLocks[msg.sender].lock.length; i++ ){
            if ( contractLocks[msg.sender].lock[i].contractAddress == _contractAddress && contractLocks[msg.sender].lock[i].currentOwner == address(this) && contractLocks[msg.sender].lock[i].originalOwner == msg.sender ){
                ...
                contractLockState[_contractAddress] = "Owned";
                contractLockTime[_contractAddress] = 0;
                payFee(_lockFee, _feeWallet);
                _success = true;
                break;
            }
    ...
    function getContractLockState(address _contractAddress) public returns(string memory){
        address _contractOwner = getOwner( _contractAddress );
        if ( _contractOwner == 0x0000000000000000000000000000000000000000 ){ 
            contractLockState[_contractAddress] = "Renounced";
        }
        if ( _contractOwner == address(this) ){ 
            contractLockState[_contractAddress] = "Locked";
        }
        if ( _contractOwner != 0x0000000000000000000000000000000000000000 &&  _contractOwner != address(this) ){ 
            contractLockState[_contractAddress] = "Owned";
        }
        return contractLockState[_contractAddress];
    }

    function getContractLockTime(address _contractAddress) public returns(uint256){
        address _contractOwner = getOwner( _contractAddress );
        if ( _contractOwner == 0x0000000000000000000000000000000000000000 ){ 
            contractLockTime[_contractAddress] = 0;
        }
        return contractLockTime[_contractAddress];
    }
