...
contract StakingV1 is StakingStorageV1, Initializable, OwnableUpgradeable, PausableUpgradeable, ERC721Upgradeable {
    ...
    /// @return vebase      whether a rebase took place
    ...
    /// @return cap        number of CNV that can be locked in pool
    /// @dev
    ...
    function viewStakingCap(uint256 poolNum) public view virtual returns(uint256) {
        ...
        return (lhs * (circulatingSupply() - IBonding(BONDS).cnvEmitted()) / 1e18 - lgm) * 1e18/lp.g - lp.balance;
    }
    ...
    /// @return coopRate    effective rate of amount distributed to COOP
    ...
    function circulatingSupply() public view virtual returns(uint256) {
        return ICNV(CNV).totalSupply() - IBonding(BONDS).getAvailableSupply() - lockedExcessRewards;
    }
    ...
    /// @return memory      returns lock position NFT image
