...
*/
    ...
    **/
	...
	amount_to_send = amount2.sub(Personal[id][addr].payed).add(Personal[id][addr].staked);
	...
	for(uint8 j=1;j <= GroupLen[i];j++)
	{
	    ...
	    r = RewardCalc(i,j,0);
	    Personal[i][addr].staked = Personal[i][addr].staked.add(r.amount).div(Personal[i][addr].payed);
