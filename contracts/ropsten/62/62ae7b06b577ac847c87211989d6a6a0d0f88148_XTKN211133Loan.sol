...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn211133][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn211133][ownable] new owner is the zero address");
...
abstract contract RoleBasedAccessControl is Context, Ownable{
    ...
    // modifiers
        ...
        modifier onlyRole(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()], "[xtkn211133][role based access control] only addresses assigned this role can access this function!");
            _;
        }

        modifier onlyRoles(string[] memory pRoles){
            for(uint256 i=0; i<pRoles.length; i++){
                require(_roleToAddress[pRoles[i]][_msgSender()], "[xtkn211133][role based access control] only addresses assigned this role can access this function!");
            }
            _;
        }
        ...
        modifier onlyRolesOr(string[] memory pRoles){
            ...
            require(rolePresent, "[xtkn211133][role based access control] only addresses assigned this role can access this function!");
            _;
        }

        modifier onlyRoleOrOwner(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()] || owner() == _msgSender(), "[xtkn211133][role based access control] only addresses assigned this role or the owner can access this function!");
            _;
        }
...
*/
contract XTKN211133Loan is Context, Ownable, ReentrancyGuard, RoleBasedAccessControl{
    // lib
    ...
    // storage
        ...
        mapping(uint16 => IXTKN211133LoanProxy) private _proxy;
        mapping(uint16 => address) private _address;
        uint16[] private _pools = [180, 365];
        uint256 public LOAN_TOKENS = 200000000 * (10**9);
        uint16 public MAX_WALLETS = 150;
    ...
    constructor(address pToken){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            _main = IXTKN211133(ADDRESS_TOKEN);
    ...
    function init(address pProxy180D, address pProxy365D) public onlyRole("root"){
        // 180 D
            _proxy[180] = IXTKN211133LoanProxy(pProxy180D);
        ...
        // 365 D
            _proxy[365] = IXTKN211133LoanProxy(pProxy365D);
    ...
    event LoanDeposit(uint16 pool, uint256 interest, uint256 totalInterest);
    function deposit(uint16 pPool, uint256 pInterest) public onlyRole("proxy"){
        ...
        // event
            emit LoanDeposit(pPool, pInterest, POOLS[pPool].interest);
    }
    ...
    function info(uint16 pPool, address pWallet) public view returns(
        ...
        require(pPool == 180 || pPool == 365, "[xtkn211133][loan] this pool does not exist!");
        require(LOANS[pPool][pWallet].tokens > 0, "[xtkn211133][loan] this wallet has no loans!");
        return(
            LOANS[pPool][pWallet].tokens,
            LOANS[pPool][pWallet].interest,
            LOANS[pPool][pWallet].total,
            LOANS[pPool][pWallet].paid,
            LOANS[pPool][pWallet].profit
        );
    }
    ...
    function priceLoan(uint256 pAmount) public view returns(uint256){
        return(_percentage(price().mul(pAmount.div(10**_main.decimals())), 100));
    }
    ...
    function loan(uint256 pAmount, uint16 pPool) public nonReentrant{
        require(pPool == 180 || pPool == 365, "[xtkn211133][loan] this pool does not exist!");
        require(WALLETS < (MAX_WALLETS - 1), "[xtkn211133][loan] no more wallets allowed to loan!");
        require(pAmount >= 10000 * (10**9), "[xtkn211133][loan] you need to at least loan 10k tokens!");
        require(LOAN_TOKENS > 0, "[xtkn211133][loan] no more tokens to loan!");
        require(_pool(pPool), "[xtkn211133][loan] this staking pool has ende or is full!");
        ...
        uint256 priceForTheLoan = priceLoan(pAmount);
        require(allowance >= priceForTheLoan, "[xtkn211133][loan] you have to authorize the transfer of your USDC first!");
        require(balance >= priceForTheLoan, "[xtkn211133][loan] you do not have enough USDC!");
        require(_buy(priceForTheLoan), "[xtkn211133][loan] transfer of USDC failed!");

        _create(_msgSender(), pPool, pAmount, priceForTheLoan);
    ...
    function claim(uint16 pPool) public nonReentrant{
        require(pPool == 180 || pPool == 365, "[xtkn211133][loan] this pool does not exist!");
        _proxy[pPool].updateAndClaim();  
        uint256 totalLoanInterest;
        address withdrawWallet = _msgSender();
        uint256 loanInterest180 = LOANS[180][withdrawWallet].interest;
        uint256 loanInterest365 = LOANS[365][withdrawWallet].interest;
        if(loanInterest180 > 0){
            totalLoanInterest = totalLoanInterest.add(loanInterest180);
            LOANS[180][withdrawWallet].profit = LOANS[180][withdrawWallet].profit.add(loanInterest180.mul(price()));
            LOANS[180][withdrawWallet].total = LOANS[180][withdrawWallet].total.add(loanInterest180);
            LOANS[180][withdrawWallet].interest = 0;
        }
        if(loanInterest365 > 0){
            totalLoanInterest = totalLoanInterest.add(loanInterest365);
            LOANS[365][withdrawWallet].profit = LOANS[365][withdrawWallet].profit.add(loanInterest365.mul(price()));
            LOANS[365][withdrawWallet].total = LOANS[365][withdrawWallet].total.add(loanInterest365);
            LOANS[365][withdrawWallet].interest = 0;
        }

        require(_token.transfer(withdrawWallet, totalLoanInterest), "[xtkn211133][loan] token transfer failed!");    
    }
    ...
    function _create(address pWallet, uint16 pPool, uint256 pAmount, uint256 pPaid) private{
        if(LOANS[pPool][pWallet].tokens <= 0){
            ...
            LOANS[pPool][pWallet].paid = LOANS[pPool][pWallet].paid.add(pPaid);
            _token.transfer(_address[pPool], pAmount);
            _proxy[pPool].stake(pAmount);
            POOLS[pPool].loan = POOLS[pPool].loan.add(pAmount);
            if(LOAN_TOKENS >= pAmount){
    ...
    event DebugShareCall(uint16 pool, address[] wallets, uint256 length);
    event DebugShare(uint256 cake, uint256 slice, address wallet, uint256 tokens);
    function _share(uint16 pPool, address[] memory pWallets) private{
        emit DebugShareCall(pPool, pWallets, pWallets.length);
        ...
        for(uint256 i=0; i<pWallets.length; i++){
            ...
            emit DebugShare(cake, slice, pWallets[i], LOANS[pPool][pWallets[i]].tokens);
            if(POOLS[pPool].interest >= slice){
                POOLS[pPool].interest = POOLS[pPool].interest.sub(slice);
                LOANS[pPool][pWallets[i]].interest.add(slice);
            }else{
                LOANS[pPool][pWallets[i]].interest.add(POOLS[pPool].interest);
