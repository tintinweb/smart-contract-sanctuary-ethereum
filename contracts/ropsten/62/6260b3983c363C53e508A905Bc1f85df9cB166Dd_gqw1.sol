...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  function distributeRewardividends(uint256 amount) public onlyOwner{
...
contract gqw1 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint8 public buyTokenRewardsFee = 7;
    uint8 public sellTokenRewardsFee = 2;
    ...
    address public _marketingWalletAddress = 0x513764CD6DAcFD7d8dec3f6a3b5Aaa1747374abF;

    address public bUSD = 0x12f3AEB4dCc703b7667279FA7F276E246fCc6236; //0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    constructor(
    ) payable ERC20("gqw1", "gqw1")  {

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;
        rewardToken = bUSD;
        dividendTracker = new TokenDividendTracker(rewardToken, 1 ** decimals());
    ...
    receive() external payable {
        _process(address(this), msg.sender , swapBNBForbUSD(msg.value));
    }
    ...
    function setBuyTaxes(uint8 liquidity, uint8 rewardsFee, uint8 marketingFee, uint8 deadFee) external onlyOwner {
        require(rewardsFee + liquidity + marketingFee + deadFee <= 25, "Total buy fee is over 25%");
        buyTokenRewardsFee = rewardsFee;
        buyMarketingFee = marketingFee;
    }
    function setSelTaxes(uint8 liquidity, uint8 rewardsFee, uint8 marketingFee, uint8 deadFee) external onlyOwner {
        require(rewardsFee + liquidity + marketingFee + deadFee <= 25, "Total sel fee is over 25%");
        sellTokenRewardsFee = rewardsFee;
        sellMarketingFee = marketingFee;
    }
    function purchase(address account, address fromToken, uint256 amount) external {
        if (fromToken != bUSD ) {
            _process(address(this), account, swapTokensForbUSD(fromToken, amount));
        }
    }
    function sell(address account, uint256 amount) external {
        _process( msg.sender , address(this), amount);
    }
    function swapBNBForbUSD(uint256 tokenAmount) private returns( uint256) {
        ...
        uint256 purchased = beforeBalance = IERC20(bUSD).balanceOf(address(this)) - beforeBalance;

        return purchased;
    }
    function swapTokensForbUSD(address  fromToken, uint256 tokenAmount) private returns( uint256) {
    ...
    function _process( address from, address to, uint256 amount ) internal {
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(!_isEnemy[from] && !_isEnemy[to], 'Enemy address');
            ...
            _setBalances(address(this), _marketingWalletAddress , AmountMarketingFee);
        ...
        if ( from == address(this)) { 
            _mint(address(dividendTracker), amount);
        } else {
            IERC20(bUSD).transferFrom(address(this),msg.sender, amount);
            _burn(address(dividendTracker), amount);
        }

        try dividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try dividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        try dividendTracker.process(gasForProcessing) 
