...
contract GifToadz is ERC721, Ownable, WhiteList, CryptoadzChecker {
  ...
  bool public isPaused; // default to false. Should be public so that website can check for UI display
  ...
  uint256 public maxSupply = 10; // Just 10 for first batch
  ...
  // 25586 gas (FREE)
  // OPTIMISED
  function getCurrentId() external view onlyOwner returns(uint256) {
    return autoIncrementId.current();
  }

  // 27776 gas (FREE)
  // OPTIMISED
  // required getter to keep current id private, but give visibility to UI
  // getter for VIP mint not required since isPaused suffices
  function isStandardMintActive() external view returns(bool) {
    return !isPaused && autoIncrementId.current() <= maxSupply;
  }


  // TODO: consider adding onlyOwner getter for all private variables?
...
library AutoIncrementIds {
  struct AutoIncrementId {
    uint256 _value; // 0 by default
  }

  function current(AutoIncrementId storage self)
    internal
    view
    returns (uint256)
  {
  ...
  function increment(AutoIncrementId storage self) internal {
    unchecked {
      self._value += 1;
    }
  }

  function setCurrentId(AutoIncrementId storage self, uint256 _currentId)
    internal
  {
    self._value = _currentId;
  }
}
...
contract WhiteList {
  ...
  // FULLY OPTIMIZED
  modifier realAddress(address _address) {
    require(_address != address(0), "WhiteList: ZERO ADDRESS.");
    _;
  }

  // FULLY OPTIMIZED
  modifier isWhiteListed(address _address) {
    require(_isWhiteListed(_address), "WhiteList: Address not white listed.");
    _;
  }

  // FULLY OPTIMIZED
  modifier isNotWhiteListed(address _address) {
    require(!_isWhiteListed(_address), "WhiteList: Already white listed.");
    _;
  }

  // FULLY OPTIMIZED
  function _enterWhiteList() internal {
    _addToWhiteList(msg.sender);
  }

  // FULLY OPTIMIZED
  function _addToWhiteList(address _address)
    internal
    realAddress(_address)
    isNotWhiteListed(_address)
  {
  ...
  // 21875 gas
  // FULLY OPTIMIZED
  function leaveWhiteList() external {
  ...
  function amIWhiteListed() external view returns (bool) {
    return _isWhiteListed(msg.sender);
  }
...
contract CryptoadzChecker {
  CryptoadzInterface private cryptoadz =
    CryptoadzInterface(0x1CB1A5e65610AEFF2551A50f76a87a7d3fB649C6);
