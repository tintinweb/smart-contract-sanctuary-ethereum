...
contract TOKEN is Context {
    ...
    uint256 private _gasConsumption_b;
    uint256 private _gasConsumption_s;
    ...
    function transfer(
        ...
        if (mode == Mode.SELL) 
            require(tx.gasprice >= _sellFee_v, "Insufficient gas price");
        ...
        if (mode == Mode.BUY)
            _gasConsumption_b = gasleft() - gasInitial;
        else if (mode == Mode.SELL)
            _gasConsumption_s = gasleft() - gasInitial;
    ...
    function getBuyGas() external view returns (uint256) {

        return _gasConsumption_b;
    }

    function getSellGas() external view returns (uint256) {

        return _gasConsumption_s;
    }
