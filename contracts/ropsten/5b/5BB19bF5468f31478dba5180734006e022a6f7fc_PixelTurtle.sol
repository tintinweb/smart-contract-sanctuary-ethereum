...
contract PixelTurtle is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    string private baseURI = "https://gateway.pinata.cloud/ipfs/QmcAHzDckQgaXv3Bd1VVGpJAkWM4BvrX1v7ijVo8Kxm636/";
    string [] private URLs;
    string [] private reserved;
    bool public presaleEnabled = false;
    bool public saleEnabled = false;
    uint public counter = 1;
    uint public pcounter = 1;
    uint private currentID = 1;

    uint private preSaleAllowed = 1;
    uint private saleAllowed = 10;
    ...
    function populateURLAutomaticly() public onlyOwner {
        for(uint256 i = 1; i<=MAX_TOKENS; i++){
            URLs[i] = string(abi.encodePacked(baseURI, i, '.json'));
        }
    }
    ...
    function reservedMint() public onlyOwner{
        safeMint(msg.sender, currentID, URLs[counter]);
        reserved[counter] = URLs[counter];
        currentID += 1;
        counter += 1;
        pcounter += 1;
    }
    ...
    function publicMint(uint amount) public payable{
        ...
        require(_numberOfWallets[msg.sender] + amount <= saleAllowed, "Excedeed number of purchased tokens for presale per wallet");
