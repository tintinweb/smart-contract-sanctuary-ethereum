...
// File: contracts/NftaniaAirdropV5.sol
...
contract NftaniaAirdrop is Pausable, Ownable, ReentrancyGuard{
   ...
   //////////////////////// Events ///////////////////////////////////////////
        ...
        // New Airdrop
            event NewAidropSupply (uint256 _currentAirdropSupply);
        // Withdraw
            event WithdrawTransfer( address indexed receivingWallet, uint amount, uint balance,uint256 indexed date);

   //////////////////////// Constructor //////////////////////////////////////            
    constructor(uint _startDate, uint _endDate) {
        contractSigner = msg.sender;
        require (_startDate != 0, "Start date can not be zero");
        require (_endDate != 0, "End date can not be zero");
   ...
   //////////////////////// Set Token Address //////////////////////////////// 
    function setTokenAddress (address _tokenAddress)  public onlyOwner { 
        require (_tokenAddress != address(0), "not allowed address");
        tokenAddress = _tokenAddress;  
    }

   //////////////////////// Claim Airdrop //////////////////////////////////// 
    function getAirdrop ( bytes32 msgh, uint8 v, bytes32 r, bytes32 s, uint amount, address to, uint signedNonce)
     public whenNotPaused() nonReentrant() returns (bool) { 
        availableTokens= getAvailableTokens();
        // Prequalify airdrop
        require(amount + currentAirdropSupply<=availableTokens, "No more Airdrop tokens are available");  
        require(block.timestamp <= startDate,"Airdrop has not startd yet");                                                 // Checks if date started          
        require(block.timestamp >= endDate,"Airdrop has already finished");                                                  // Checks if date Ended    
        // Verify message
        ...
        require(ecrecover(msgh, v, r, s) == contractSigner, "Nftania: Invalid Signer");
        require(msgh == SignedHashedMessage, "Nftania: Invalid Message");
        require(nonce == true,"This airdrop is already minted");
        // Update status
        currentAirdropSupply+=amount; 
        airdropID+=1;
        // Deliver tokens
        deliverTokens (to, amount);
        // Events
        emit AirdropClaim (to, amount, airdropID,block.timestamp);
        emit NewAidropSupply (currentAirdropSupply);
        return true;        
    }
   ...
   //////////////////////// function Minimum Value /////////////  
    function min (uint256 a ,uint256 b) public pure returns (uint minimum) {
        if (a>b) {
            return b;
        }
        else{
   ...
   //////////////////////// Claim Aidrop Supporting Functions ////////////////
    function messageHash(address to, uint amount, uint signedNonce ) public pure returns (bytes32 base_msg) {
    ...
    function nonceCheck(uint signedNonce) public  returns (bool nonce){
        ...
        if (signedNonce == nextNonce[msg.sender]){
            ...
            //setting the NextNonce value, as the current value is just claimed.
            nextNonce[msg.sender]+=1;
            nonce = true;
        }
        // in this case nonce value is already claimed and nonce is rejected(false)
        // the new request is trying to do "replay attack" to the arealdy 
        // calimed airdrop Message.
        else {
            nonce = false;
        }
    }

   //////////////////////// Increase Drop Tokens Supply //////////////////  
    function increaseTokenSupply(uint256 amount) public returns(bool status){ 
   ...
   //////////////////////// Set Dates //////////////////////////////////////// 
    function setDates(uint _startDate, uint _endDate) public onlyOwner returns (bool status){
        require(_endDate > startDate,"End Date should be after Start Date."); 
        startDate=_startDate;
        endDate = _endDate;
        return status = true;        
    } 
   ...
   //////////////////////// Get Dates //////////////////////////////////////// 
    function getDates() public view onlyOwner returns (uint _startDate, uint _endDate){ 
