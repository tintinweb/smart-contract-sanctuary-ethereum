...
contract MMACombat is Context, IERC20, Ownable {
    ...
    address public marketingAddress = 0x1eC3a19DD007e5F36bac37b1Bf2225b16b58DedA;
    address public charityAddress = 0x1eC3a19DD007e5F36bac37b1Bf2225b16b58DedA;
    address public investmentAddress = 0x1eC3a19DD007e5F36bac37b1Bf2225b16b58DedA;
    address public lpRecipient = 0x1eC3a19DD007e5F36bac37b1Bf2225b16b58DedA;


    string private constant _name = "KoaCombat";
    string private constant _symbol = "MMACombat";
    ...
    Taxes public taxes = Taxes(25,35,20,20,20);
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _charity, uint256 _investment, uint256 _liquidity) public onlyOwner {
        taxes.rfi = _rfi;
        taxes.marketing = _marketing;
        taxes.charity = _charity;
        taxes.investment = _investment;
    ...
    function _takeInvestment(uint256 rInvestment, uint256 tInvestment) private{
        totFeesPaid.investment +=tInvestment;
        if(_isExcluded[charityAddress])_tOwned[charityAddress]+=tInvestment;
        _rOwned[charityAddress] +=rInvestment;
    }

    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        to_return = _getTValues(tAmount, takeFee);
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rMarketing,to_return.rCharity, to_return.rInvestment, to_return.rLiquidity) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tInvestment = tAmount*taxes.investment/100;
        s.tLiquidity = tAmount*taxes.liquidity/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tCharity-s.tInvestment;
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing,uint256 rCharity,uint256 rInvestment,uint256 rLiquidity) {
        ...
        rCharity = s.rCharity*currentRate;
        rInvestment = s.rInvestment*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rCharity-rInvestment;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rCharity,rInvestment,rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if(s.rMarketing > 0 || s.tMarketing > 0){
            ...
            emit Transfer(sender, marketingAddress, s.tCharity);
        }
        if(s.rCharity > 0 || s.tCharity > 0){
            _takeCharity(s.rCharity, s.tCharity);
            emit Transfer(sender, charityAddress, s.tCharity);
        }
        if(s.rInvestment > 0 || s.tInvestment > 0){
            _takeInvestment(s.rInvestment, s.tInvestment);
            emit Transfer(sender, investmentAddress, s.tInvestment);
        }
    ...
    function updateInvestmentWallet(address newWallet) external onlyOwner{
        require(investmentAddress != newWallet ,'Wallet already set');
        investmentAddress = newWallet;
        _isExcludedFromFee[investmentAddress];
    }
