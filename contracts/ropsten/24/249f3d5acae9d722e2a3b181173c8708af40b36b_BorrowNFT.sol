...
contract Staking{
    ...
    function _claimInterst(address user, uint256 blockTimeStamp, uint lastInterestClaimed) internal {
        uint intrs;
        if( blockTimeStamp >= (lastInterestClaimed + interestPeriod)){ 

            intrs = (blockTimeStamp - lastInterestClaimed) / interestPeriod;
    ...
    function _transfertoken(address to, uint amount) internal {
        STAKE.safeTransfer(to, amount);
    }

    function _transferFromtoken(address from, uint amount) internal {
        STAKE.safeTransferFrom(from, address(this),  amount);
    }

    function getInterst(address user) external view returns(uint256 intrs) {

        if(block.timestamp >= (stakerInfo[user].lastInterestClaimed + interestPeriod )  ){ 
            intrs = (block.timestamp - stakerInfo[user].lastInterestClaimed) / interestPeriod;
            intrs = ((intrs * interestRate)  * stakerInfo[user].amount) / 100;        
        }
        return intrs;
    }
...
contract BorrowNFT is Ownable, ERC721Holder, Staking{
    using SafeERC20 for IERC20;
