...
//            _rOwned[account] = tOwned.mul(_getRate());
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(address(this).balance >= minSpendAmount.mul(2) || buybackTriggered){
...
abstract contract LPSwapSupport is OwnableUpgradeable {
    ...
    uint256 private buybackTokensPending;
    ...
    function buybackAndLiquify(uint256 amount) internal { //returns(uint256 remainder){
        ...
        if(buybackTriggered){
            ...
            // swap tokens for
            swapCurrencyForTokensUnchecked(address(this), amount.div(2), address(this));

            // how much did we just swap into?
            buybackTokensPending = _balanceOf(address(this)).sub(initialTokenBalance);
        } else {
            // add liquidity to uniswap

            addLiquidity(buybackTokensPending, amount);

            emit BuybackAndLiquify(buybackTokensPending, amount);
        }
