...
contract Zenogakki is Ownable, ERC721A, ReentrancyGuard {
  ...
  uint16 public constant MAX_COLLECTION_SIZE = 10000;
  uint16 public constant MAX_PUBLIC_SIZE = 7500;
  uint8 public constant MAX_PER_WALLET = 5;
  uint256 public constant MAX_CUT = 6.75 ether;

  uint8 public constant OG_MAX_MINT = 4; // includes 1 free NFT offered on first mint
  uint8 public constant WL_MAX_MINT = 2;
  ...
  mapping(address => bool) public ogAirdrop;

  uint256 revenueCut;
  
  constructor(uint16 toTreasury) ERC721A("Zenogakki", "ZENO") {
    maxCollectionSize = MAX_COLLECTION_SIZE;
    maxPublicSize = MAX_PUBLIC_SIZE;
    maxPerWallet = MAX_PER_WALLET;
    _mint(msg.sender, toTreasury);
  }
  ...
  function mint(uint8 quantity) external payable callerIsUser {
    ...
    uint8 maxMint = allowanceForCurrentPhase(msg.sender);

    // any OG minting gets a free NFT.
    uint8 nbToMint = quantity;
    if (og[msg.sender] && !ogAirdrop[msg.sender]) {
      require(quantity > 0, "Buy at least 1 NFT to get your airdrop");
      nbToMint++;
      ogAirdrop[msg.sender] = true;
    }

    require(totalSupply() + nbToMint <= maxPublicSize, "Reached max public supply");
    require(numberMinted(msg.sender) + nbToMint <= maxMint, "cannot mint this quantity");
    ...
    _mint(msg.sender, nbToMint);
    refundIfOver(price * quantity);
  }

  function allowanceForCurrentPhase(address minter) internal view returns (uint8) {
    if (block.timestamp > publicStart) {
      return maxPerWallet;
    } else if (og[minter]) {
      return OG_MAX_MINT;
  ...
  function seedOg(address[] memory addresses, bool allow)
    external
    onlyOwner
  {
  ...
  function seedWl1(address[] memory addresses, bool allow)
    external
    onlyOwner
  {
    for (uint256 i = 0; i < addresses.length; i++) {
  ...
  function seedWl2(address[] memory addresses, bool allow)
    external
    onlyOwner
  {
    for (uint256 i = 0; i < addresses.length; i++) {
  ...
  function setMaxPublicSize(uint16 maxPublicSize_) external onlyOwner {
    // Can only be decreased
    require(maxPublicSize_ < maxPublicSize);
    maxPublicSize = maxPublicSize_;
  }

  function setMaxPerWallet(uint8 maxPerWallet_) external onlyOwner {
  ...
  function withdrawMoney() external onlyOwner nonReentrant {
    if (revenueCut < MAX_CUT) {
        ...
        revenueCut += cut;
        (bool cutSuccess, ) = address(0xAc047cF33CBAcEd70E77Efb41Cff705A31031d26).call{value: cut}("");
