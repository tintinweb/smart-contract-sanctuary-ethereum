...
contract CryptoAnts is ERC721, ICryptoAnts, Ownable {
  ...
  mapping(address => uint256) public ownerAntCount;
  IEgg public immutable eggs;

  //What is the use of Locked?
  ...
  modifier isYourAnt(uint256 _antId) {
    require (antToOwner[_antId] == msg.sender);
    _;
  }
  ...
  function buyEggs(uint256 _amount) external payable override lock {
    // The value divided by the amount to buy should always return the eggPrice (eg 0.12/12 = 0.01)
    require(msg.value / _amount == eggPrice, "Value or amount not valid");
  ...
  function changeEggPrice(uint256 _newPrice) external onlyOwner{
    // Price can't be zero
    require(_newPrice != 0);
    eggPrice = _newPrice;
  }
  ...
  function sellAnt(uint256 _antId) external isYourAnt(_antId) {
    // solhint-disable-next-line
  ...
  function ownerOf(uint256 _antId) view public override(ERC721, IERC721) returns(address) {
    return antToOwner[_antId];
  }
  
  function balanceOf(address _owner) view public override(ERC721, IERC721) returns(uint256) {
