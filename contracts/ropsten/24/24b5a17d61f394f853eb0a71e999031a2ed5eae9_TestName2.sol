...
contract TestName2 is ERC20, Ownable {
    // DEX
    ...
    constructor() ERC20("Test Name 2", "TEST2") {
    ...
    function letsLunch(uint256 botBlocks_, address projectWallet_, uint256 maxTxAmount_, uint256 maxWalletAmount_) external onlyOwner {
        ...
        require(botBlocks_ >= 0 && botBlocks_ <= 8, "Token: bot blocks must range between 0 and 8");
    ...
    function setProjectWallet(address projectWallet_) public onlyAuth {
    ...
    function setSwapEnabled(bool swapEnabled_) public onlyAuth {
    ...
    function setAntiBotEnabled(bool antiBotEnabled_) external onlyAuth {
        require(antiBotEnabled != antiBotEnabled_, "Token: anti-bot enabled already set to value");
    ...
    function setSellFees(uint256 sellProjectFee_, uint256 sellLiquidityFee_) external onlyAuth {
        require(sellProjectFee_ >= 0 && sellProjectFee_ <= 10, "Token: sell project fee must range between 0% and 12%");
        require(sellLiquidityFee_ >= 0 && sellLiquidityFee_ <= 6, "Token: sell liquidity fee must range between 0% and 6%");
        require((sellProjectFee_ + sellLiquidityFee_) <= 12, "Token: total sell fee must range between 0% and 12%");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (limitsEnabled && !_isSwapLocked && from != owner() && to != owner() && to != address(0) && to != address(0xdead)) {
            ...
            if (antiBotEnabled && block.number <= (_launchBlock + _botBlocks) && to != address(this) && to != dexPair && to != address(dexRouter)) {
                _bots[to] = true;
        ...
        if (isExcludedFromFees(from) || isExcludedFromFees(to) || from == address(dexRouter)) {
            deductFees = false;
        }
