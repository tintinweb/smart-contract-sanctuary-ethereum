...
contract Staking is Ownable, Pausable {
    ...
    function deposit(
        ...
        require(
            ISignature(verifySignature).isDepositERC20Valid(
            ...
            "Staking: signature is not verify"
        );
        require(_amount > 0, "Staking: amount greater than zero");
        require(
            package.token.balanceOf(msg.sender) >= _amount,
            "Staking: your balance not enough to staking"
        );
    ...
    function depositBNB(
        ...
        require(
            address(package.token) == address(0),
            "Staking: can not deposit this package"
        );
    ...
    function add(IERC20 _token) public onlyOwner {
        packageInfo.push(Package(_token, true, 0));
    }
    ...
    function activePool(uint256 _pid, bool _enable) public onlyOwner {
        Package storage p = packageInfo[_pid];
        p.stopTime = !_enable ? block.timestamp : 0;
        p.enable = _enable;
    }
