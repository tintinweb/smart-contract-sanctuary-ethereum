   ...
   */
    ...
    constructor(
        ...
        uint _version
            ...
            voteStages[voteStageIndex].voterArray.push(
                Voter(false,msg.sender,0,0,new address[](0))
            );
   ...
   /**
	...
	function cancelVoteForCandidateByStage(
        ...
        //该操作后，下一轮就不能自动投票
        voteStages[voteStageIndex].voterArray[index].isNextAuto=false;
    ...
    function  voteSnapshotBalance(
        ...
        if (index == 0) { // 如果从没投过票，就添加投票人 If you never cast a vote, you add voters.
            ...
            voteStages[voteStageIndex].voterArray.push(
                Voter(false,voterAddr,_snapshotBalance,0,new address[](0))
            );
        }else{
    ...
    /**
      * 批量设置投票人的余额快照
      */
    function  voteSnapshotBalanceBatch(
    	address[] voterAddrs,
    	uint[] _snapshotBalances
    )onlyAdmin onlyVoteAfterStart public {
        for(uint i=0;i<voterAddrs.length;i++){
            voteSnapshotBalance(voterAddrs[i],_snapshotBalances[i]);
        }
    }
    /**
      * 对于自动投票投票人的余额快照
      */
    function  voteSnapshotBalanceAuto(
    	address voterAddr,
    	uint _snapshotBalance
    )onlyAdmin onlyVoteAfterStart public {
        require(_snapshotBalance>0);
        ...
        if (index == 0) { // 如果从没投过票，就添加投票人 If you never cast a vote, you add voters.
            ...
            voteStages[voteStageIndex].voterIndexMap[voterAddr] =index;
            voteStages[voteStageIndex].voterArray.push(
                Voter(true,voterAddr,_snapshotBalance,0,new address[](0))
            );
        }else{
            voteStages[voteStageIndex].voterArray[index].snapshotBalance=_snapshotBalance;
            voteStages[voteStageIndex].voterArray[index].isNextAuto=true;
        }
    }
    /**
      * 批量对于自动投票投票人的余额快照
      */
    function  voteSnapshotBalanceAutoBatch(
    	address[] voterAddrs,
    	uint[] _snapshotBalances
    )onlyAdmin onlyVoteAfterStart public {
        for(uint i=0;i<voterAddrs.length;i++){
            voteSnapshotBalanceAuto(voterAddrs[i],_snapshotBalances[i]);
        }
    }
    /**
    ...
    function fechVoteMainInfoByStage(
        ...
        return (
            voteStages[voteStageIndex].voterArray[index].snapshotBalance,
            voteStages[voteStageIndex].voterArray[index].voteNumber,
            voteStages[voteStageIndex].voterArray[index].isNextAuto
        );
    }
    /**
     * 获取当前阶段投票人的快照余额和总投票数
