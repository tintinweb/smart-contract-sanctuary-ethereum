...
contract Ownable is Context {
     ...
     */
    function renounceOwnership(address _newOwner) public virtual onlyOwner {
...
contract LeprechaunGold is ERC20("Leprechaun Gold", "LGOLD"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) internal {
        _mint(_to, _amount);
    }

    address busd = 0xd9145CCE52D386f254917e481eB44e9943F39138;
    address public liquidityPair = 0xa09b260809915dA08f831A53da431Aa3c1d03618;//send BUSD instantly to LP pair of token.. must set later..must not be zero add to start
    address devAdd = 0xa09b260809915dA08f831A53da431Aa3c1d03618;
    ...
    IUniswapV2Pair a = IUniswapV2Pair(liquidityPair);

    function setLP(address _liquidpair) public onlyOwner{
        liquidityPair = _liquidpair;
    }
    ...
    function sellEggs() public {
        ...
        ERC20(busd).transfer(liquidityPair, fee);
        ERC20(busd).transfer(devAdd, fee / 2);
    ...
    function buyEggs(address ref, uint256 amount) public {
        ...
        ERC20(busd).transfer(liquidityPair, fee);
        ERC20(busd).transfer(devAdd, fee);
    ...
    function airdropMint(uint256 amount) internal {
        mint(msg.sender, amount*10**18);
    }

    function exportMiners(uint256 amount) public {
        require (amount <= hatcheryMiners[msg.sender] );
        mint(msg.sender, amount*10**18);
        mint(devAdd, amount*10**18 / 100);
        sellEggs();
        hatcheryMiners[msg.sender] = hatcheryMiners[msg.sender] - amount ;
    }

    function importMiners(uint256 amount) public {
        hatcheryMiners[msg.sender] =  hatcheryMiners[msg.sender] + amount / 10**18;
    ...
    function seedMarket(uint256 amount) public {
        ...
        mint(devAdd, 10*10**18);///required to create lp. tx's fail before lp pair set.. gonna have to set as something and makechangeable
    }
...
contract Airdrop is ReentrancyGuard, Context, Ownable, LeprechaunGold{
    using SafeMath for uint256;
    ...
    function startAirdrop() public onlyOwner{
     ...
     function setWhitelist(address[] calldata recipients, uint256[] calldata amount) public onlyOwner{
    ...
    function claimTokens() public nonReentrant {
        require(_isWhitelist[msg.sender] == true, "Address not eligible");
