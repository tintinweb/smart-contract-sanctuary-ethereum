 ...
 * 
 * Agent verifies off-chain the recipient in accordance with verification 
...
contract Escrow is Pausable {
  ...
  // fee that accrues to agent with each transfer
  ...
  event LogChangeCommissionFeeRate( uint oldFeeRate, uint newFeeRate );
  
  event LogChangeAgent( address oldAgent, address newAgent );  

  event LogChangeCusd( address oldCUSD, address newCUSD );
  ...
  mapping (address => Transfer) pendingTransfers;
  ...
  modifier onlyAgent() {
    require(msg.sender == agent, "msg.sender not an agent");
    _;
  }
   ...
   * @notice The caller of deposit must transfer CUSD to this contract to call deposit
   * 
   * @param _transitAddress transit address assigned to transfer.
   * @param _amount amount of CUSD that is deposited to this contract
   */
  function deposit(address _transitAddress, uint _amount)
                            public
                            whenNotPaused
  {
    // check if user has deposited CUSD
    // TODO:
    // this is not rigorous right now, it only checks if contract has enough CUSD, presumably
    // the contract would only own CUSD that users have deposited minus fees, but there must be a better way
    // perhaps check transaction logs? require user to pass in txn hash?
    // To make sure that this contract only owns deposited CUSD minus fees, immediately transfer commissions to agent
    
    require(_amount <= CarbonDollar(cusd).balanceOf(address(this)),"check if user has sent CUSD to this contract");
    // can not override existing transfer
    require(pendingTransfers[_transitAddress].amount == 0, "pending transfer already exists");

    uint fee = computeFee(_amount, commissionFeeRate);
    uint depositAfterFee = (_amount).sub(fee);

    // saving transfer details
    pendingTransfers[_transitAddress] = Transfer(
					    msg.sender,
					    depositAfterFee
					    );
    ...
    // log deposit event
    emit LogDeposit(msg.sender, _transitAddress, _amount, fee);
  }
  ...
  function changeCommissionFeeRate(uint _newCommissionFeeRate)
                          ...
                          onlyOwner
  {
    require(_newCommissionFeeRate != commissionFeeRate, "Must be a new rate");
    uint oldCommissionFeeRate = commissionFeeRate;
    commissionFeeRate = _newCommissionFeeRate;
    emit LogChangeCommissionFeeRate(oldCommissionFeeRate, commissionFeeRate);
  }
  ...
  function changeAgent(address _newAgent)
                          ...
                          onlyOwner
  {
    require(_newAgent != agent, "Must be a new agent");
    address oldAgent = agent;
    agent = _newAgent;
    emit LogChangeAgent(oldAgent, agent);
  }

  /**
   * @dev Change cusd address.
   * Only owner can change cusd address.
   * 
   * @param _newCUSD address new carbon dollar address
   */
  function changeCUSD(address _newCUSD)
                          public
                          whenNotPaused
                          onlyOwner
  {
    require(_newCUSD != cusd, "Must be a new cusd");
    address oldCUSD = cusd;
    cusd = _newCUSD;
    emit LogChangeCusd(oldCUSD, cusd);
  }
   ...
   * @return True if success.
   */
  function cancelTransfer(address _transitAddress) public {
    // pending transfer must exist
    require(pendingTransfers[_transitAddress].amount > 0, "pending transfer does not exist");
    Transfer memory transferOrder = pendingTransfers[_transitAddress];

    // only sender can cancel transfer;
    require(msg.sender == transferOrder.from, "only sender of deposit can cancel transfer");
  ...
  function withdraw(
    ...
    public
    onlyAgent // only through agent can withdraw transfer;
    whenNotPaused
  {
    // pending transfer must exist
    require(pendingTransfers[_transitAddress].amount > 0, "pending transfer does not exist");
    ...
    require(verifySignature(_transitAddress,
    		     _recipient, _v, _r, _s ), "incorrect signature");
   ...
   */
  function getTransfer(address _transitAddress) public view
    returns (
	     address id,
	     address from, // transfer sender
	     uint amount) // in wei
  {
    Transfer memory transfer = pendingTransfers[_transitAddress];
    return (
	    _transitAddress,
	    transfer.from,
	    transfer.amount
  ...
  /**
  * @notice Compute the fee that will be charged on a "deposit" operation.
  * @param _amount The amount that was deposited.
  * @param _fee The fee that will be charged, in tenths of a percent.
  */
  function computeFee(uint _amount, uint _fee) public pure returns (uint) {
      return _amount.mul(_fee).div(1000);
  }
    ...
    internal pure returns(bool success)
  {
    ...
    // ECDSA signature parameters v, r, and s comprise the private key of the _transitAddress public key
    // For more information: https://github.com/ethereum/wiki/wiki/JavaScript-API#web3ethsign
