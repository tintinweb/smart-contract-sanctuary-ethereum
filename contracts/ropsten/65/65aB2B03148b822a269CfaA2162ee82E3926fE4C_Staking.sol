...
contract Staking is Ownable, ReentrancyGuard, Deadline {
        ...
        nonReentrant
    {
        ...
        AllPositions[msg.sender].push(
            getPositions(endTime, lock, _Choise, _amount)
        );
        ...
        returns (uint256)
    {
        ...
        if (choice == staking(1)) {
            stakeEndTime = luckTime[sender][position] = uint128(
                block.timestamp + 120 //1 month lock
            );
        } else if (choice == staking(2)) {
            stakeEndTime = luckTime[sender][position] = uint128(
                block.timestamp + 180 //3 month lock
            );
        } else if (choice == staking(3)) {
            stakeEndTime = luckTime[sender][position] = uint128(
                block.timestamp + 240 + 12 hours // half year lock
            );
        } else if (choice == staking(4)) {
            stakeEndTime = luckTime[sender][position] = uint128(
                block.timestamp + 300 //1 year lock
            );
        }
        positions[sender]++;
        return stakeEndTime;
