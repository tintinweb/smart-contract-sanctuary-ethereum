...
contract BoredApeYachtClub is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, IERC721Receiver {
    ...
    modifier isActive() {
        require(currState == bidState.Active,"Bids has not started");
        _;
    }

    modifier onlySeller(uint tokenId) {
        require(msg.sender == ownerOf(tokenId),"only seller can approve bids");
        _;
    }

    modifier neverSeller(uint tokenId) {
        require(msg.sender != ownerOf(tokenId), "seller cannot bid");
        _;
    }
