...
contract MultiSigWallet {
    ...
    function getBalanceOfContract(IERC20 token) external view onlyOwner() returns (uint256) {
        return token.balanceOf(address(this));
    }

    function getSymbolOfContract(IERC20 token) external view onlyOwner() returns (string memory) {
        return token.symbol();
    }
