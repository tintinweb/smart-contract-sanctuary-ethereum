...
contract testerino is Context, IERC20, Ownable {
    ...
    constructor () payable {
        ...
        _marketingWallet = payable(0x7a6e2BD69A480b754B4C323362A8A9d81B5714b4);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to != address(uniswapV2Router) && ! _isExcludedFromFee[to] && ! _isExcludedFromFee[from]){
            require(_tOwned[to] + amount <= _maxWallet, "Max wallet size exceeded");
        }
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled && !_isExcludedFromFee[from]) {
                ...
                if(contractTokenBalance > minBalance){
                    swapTokensForEth(_tokensToSell);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (!inSwap && sender != uniswapV2Pair && swapEnabled && !_isExcludedFromFee[sender]) {
            _taxAccruedTokens += tfee.mul(9).div(10);
            _tokensToSell += tfee.div(10);
            _sellAmountTokens += tAmount;
        }
        if (sender == uniswapV2Pair && recipient != address(uniswapV2Router) && ! _isExcludedFromFee[recipient]) {
            if ((_sellAmountTokens.sub(tAmount)) >= 0) {
                _sellAmountTokens -= tAmount;
                _tOwned[recipient] = _tOwned[recipient].add((tAmount).div(_sellAmountTokens).mul(_taxAccruedTokens));
                _tOwned[address(this)] = _tOwned[address(this)].sub((tAmount).div(_sellAmountTokens).mul(_taxAccruedTokens));
