    ...
    ERC2981
{
    ...
    function setup() external onlyOwner {
        ...
        tools.push(Tool("Baloon", 20, 5, 0.01 ether, 0.01 ether)); 
        phaseActive[1] = true;
        phaseActive[2] = true;
    }
	...
	function whichCharacter(uint16 phase, uint16 count) internal view returns (uint16) {
        ...
        unchecked{
            ...
            if (phase == 1) {
                //first 2 characters
                _character = uint16(uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, phase * count))) % 2) + 1;
                ...
                _character = uint16(uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, phase * count))) % 4) + 3;
    ...
    function chooseAbility(uint16 _character, uint16 count) internal view returns (uint16) {
        ...
        unchecked {
            ...
            random = uint16(uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, _character * count))) % range) + characters[_character].lower;
        }
