...
contract PoolFactory is OwnableUpgradeable{
    ...
    // using IERC20Upgradeable for IERC20token;
    // IERC20Upgradeable public TokenV2;
    ...
    function initialize(address _master , address _privatemaster , address _poolmanager , address _fairmaster , uint8 _version , uint256 _kycPrice , uint256 _auditPrice , uint256 _poolPrice , uint256 _contributeWithdrawFee)external initializer {
        __Ownable_init();
        master = _master;
    ...
    function initalizeClone(
        ...
        address getPair = IUniswapV2Factory(factoryAddress).getPair(ethAddress,_addrs[0]);
    ...
    function createSale(
        address[3] memory _addrs, // [0] = token, [1] = router, [2] = governance
         uint256[16] memory _saleInfo, // [0] = rate, [1] = uniswap rate  [2] = min, [3] = max [4] = soft cap, [5] = hard cap [6] = start, [7] = end, [8] = unlock seconds [9] = whitelist ,[10] = audit , [11] = kyc [12] = token fee percent, [13] = eth fee percent[14] = liquidityPercent , [15] = refundType 
        string memory _poolDetails,
    ...
    function checkfees(uint256 _audit , uint256 _kyc , address[] memory _address) internal {
        uint256 totalFees = 0;
        totalFees += poolPrice; 
    ...
    function checkPrivateSalefees(uint256 _audit , uint256 _kyc , address[] memory _address) internal {
        uint256 totalFees = 0;
        totalFees += poolPrice; 
        totalFees +=  IPoolManager(poolManager).countTotalPay(_address);
        if(_audit == 1){
