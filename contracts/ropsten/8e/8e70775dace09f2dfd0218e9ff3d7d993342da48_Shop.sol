...
contract Shop is Ownable {
    ...
    mapping(address => mapping(address => Record[])) buyRecords;
    ...
    function setCatPrice(Cat _type, uint _price) external onlyOwner {
    ...
    function buy(Cat _type, uint num) external {
        ...
        buyRecords[msg.sender][cat].push(Record({
            nft: cat,
            buyer: msg.sender,
            price: price,
            num: num,
    ...
    function totalUsdt() external view onlyOwner returns(uint) {
        return usdtAmount;
    }
