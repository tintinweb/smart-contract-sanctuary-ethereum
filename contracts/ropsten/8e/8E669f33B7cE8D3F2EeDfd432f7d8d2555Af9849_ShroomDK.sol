...
contract ShroomDK is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    function mint(bytes calldata signature) external payable {
        ...
        // TODO race conditions because of drops
        // we could keep count of nfts a user has minted on chain and limit it to like 3
        // then we could use address as nonce

        uint256 tokenId = getTokenId();
        require(verify(signature, tokenId), "Signature is not valid, can only mint through flipside website!");
     ...
     */
    function verify(bytes memory signature, uint256 tokenId) public view returns (bool) {
        // address holder = ownerOf(tokenId);
        bytes32 hashed = hashPayload(tokenId);
     ...
     */
    function hashPayload(uint256 tokenId) public pure returns (bytes32) {
        return keccak256(abi.encode(tokenId)).toEthSignedMessageHash();
    }
    ...
    function setVerifiedSigner(address signer) public onlyOwner {
