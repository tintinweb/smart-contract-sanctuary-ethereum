...
contract Coomer is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        taxFees = TaxFees(1,8,1,8,10);
        distribution = Distribution(50, 20, 20, 10);
...
contract DividendTracker is IERC20, Context, Ownable {
    ...
    string private _name = "COOMER TRACKER";
    string private _symbol = "COOMT";
    uint8 private _decimals = 9;
    uint256 public totalDividendsDistributed;
    IterableMapping private tokenHoldersMap = new IterableMapping();
    uint256 public minimumTokenBalanceForDividends = 100000 * 10 **  _decimals;
    Coomer private coomer;
    ...
    function setTokenBalance(address account) external {
        uint256 balance = coomer.balanceOf(account);
        if(!coomer.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        coomer = Coomer(payable(contractAddr));
    }
