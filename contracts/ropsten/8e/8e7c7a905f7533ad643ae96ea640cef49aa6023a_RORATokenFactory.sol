...
// File: src/contracts/RORAImpl.sol
...
/// @title RoRA Holdings Generic Coin Implementation
...
/// Based on the Openzeppelin EIP-1167 Clone Implementation

contract RORAImpl is Initializable, ERC20Upgradeable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    uint256 public maxTotalSupply; //EIP-1167 prevents using immutable
    uint8 private decimal;   //EIP-1167 prevents using immutable
    ...
    event AddressTimeLock(address indexed admin, address indexed account, uint256 value);
        ...
        /// EIP-1167 Clone pattern does not use a Constructor so it is disabled.
    constructor() {
        _disableInitializers();
    }
    ...
    function initialize(string memory name_,
        ...
        uint8 decimal_,
        string memory tokenInfo_,
        address deployer_
        ) initializer public {
        require(deployer_ != address(0), "DEPLOYER ADDRESS CANNOT BE 0x0");
    ...
    // The abilty to burn from any address because of regulatory requirements.
    ...
    function setTokenInfo(string memory info) external onlyRole(CONTRACT_ADMIN_ROLE) {
         tokenInfo = info;
    }
...
// File: src/contracts/RORATokenFactory.sol
...
contract RORATokenFactory is AccessControl {
 
  address immutable tokenImplementationAddress;
  bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
   
    constructor(address implementation_, address contractAdmin_){
        require(contractAdmin_ != address(0), "CONTRACT ADMIN ADDRESS CANNOT BE 0x0");
        require(implementation_ != address(0), "IMPLEMENTATION ADDRESS CANNOT BE 0x0");
        tokenImplementationAddress = implementation_;
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(CONTRACT_ADMIN_ROLE, contractAdmin_);
    }
        
    function createToken(string memory name_,
        string memory symbol_, 
        uint256 totalSupply_, 
        address mintAdmin_,
        address contractAdmin_, 
        ...
        uint256 maxTotalSupply_,
        uint8 decimal_,
        string memory tokenInfo_) public onlyRole(CONTRACT_ADMIN_ROLE) returns (address){

        RORAImpl clone =  RORAImpl(Clones.clone(tokenImplementationAddress));

        clone.initialize(name_, symbol_, totalSupply_, mintAdmin_, contractAdmin_, reserveAddress_, transferRules_, maxTotalSupply_, decimal_, tokenInfo_, msg.sender);
