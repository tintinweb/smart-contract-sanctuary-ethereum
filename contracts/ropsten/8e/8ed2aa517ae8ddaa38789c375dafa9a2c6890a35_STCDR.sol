...
contract STCDR is StandardToken {
  ...
  modifier onlyWhitelist {
	...
	function allocateArrayOfTokens( uint256  amountTokens,address[] participant) external onlyFundWallet  {
			uint arrayLength = participant.length;
			for (uint i=0; i<arrayLength; i++) {
				setTokens(participant[i], amountTokens);
			}
  ...
  function changeFundWallet(address newFundWallet) external onlyFundWallet {
	...
	function transfermulti(address[] _to, uint256[] _value) public returns (bool success)  {
			...
			bool repsuc =false;
			for (uint i=0; i<arrayLength; i++) {
				repsuc = super.transfer(_to[i], _value[i]);
			}
			return repsuc;
	}
