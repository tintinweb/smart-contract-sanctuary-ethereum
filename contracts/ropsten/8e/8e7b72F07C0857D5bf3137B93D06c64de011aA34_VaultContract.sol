...
contract VaultContract is IERC721Receiver, Initializable {
    ...
    function swapExactOutputSingle(
        ...
        TransferHelper.safeApprove(tokenIn, swapRouterAddress, amountInMaximum);
    ...
    function investorWithdraw(uint _amount) external {
        require(freezeWithdrawal == false, "Owner Freezed Withdrawal");
        INonfungiblePositionManager nonfungiblePositionManager = INonfungiblePositionManager(
                nonfungiblePositionManagerAddress
            );
        uint totalPositions = nonfungiblePositionManager.balanceOf(
            address(this)
        );

        for (uint index = 0; index < totalPositions; index++) {
            // check if there is any open position
            uint tokenId = nonfungiblePositionManager.tokenOfOwnerByIndex(
                address(this),
                index
            );
            uint128 liquidity;
            (, , , , , , , liquidity, , , , ) = nonfungiblePositionManager
                .positions(tokenId);
            if (liquidity > 0) freezeWithdrawal = true;
        }
    ...
    function investorGetBalance() public returns (uint) {
    ...
    function toggleFreezeWithdraw() public {
        require(msg.sender == Owner, "Invalid Owner");
        if (freezeWithdrawal == true) freezeWithdrawal = false;
        else freezeWithdrawal = true;
    }
