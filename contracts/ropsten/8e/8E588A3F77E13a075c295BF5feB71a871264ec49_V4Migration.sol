...
contract V4Migration is Ownable, ReentrancyGuard {
  ...
  uint256 public blocksWindow = 100; // used for quick test, will be 1 month, number of blocks

  constructor(address _oceanAddress, address _OPFWallet) {
    ...
    nonReentrant
  {
    ...
    newPool.deadline = block.number.add(blocksWindow);
    ...
    nonReentrant
  {
    ...
    require(
      uint256(pool[_poolAddress].status) == 1,
  ...
  function canAddShares(address _poolAddress) public view returns (bool) {
    if (uint256(pool[_poolAddress].status) == 1) return true;
    return false;
  }
    ...
    nonReentrant
  {
    ...
    // uint256 totalLPSupply = IERC20(poolAddress).totalSupply();
    require(
      thresholdMet(poolAddress) || pool[poolAddress].deadline < block.number,
