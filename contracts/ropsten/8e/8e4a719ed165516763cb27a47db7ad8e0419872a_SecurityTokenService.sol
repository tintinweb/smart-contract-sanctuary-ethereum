 ...
 * @author Remco Bloemen <<a href=/cdn-cgi/l/email-protection class=__cf_email__ data-cfemail=c2b0a7afa1ad82f0>[email&#160;protected]</a>π.com>
...
contract SecurityToken is ISecurityToken, ReentrancyGuard, ServiceHelper {
    ...
    function getSig(bytes memory _data) internal pure returns (bytes4 sig) {
        uint len = _data.length < 4 ? _data.length : 4;
        for (uint i = 0; i < len; i++) {
            // sig = bytes4(uint(sig) + uint256(_data[i]) * (2 ** (8 * (len - 1 - i))));
