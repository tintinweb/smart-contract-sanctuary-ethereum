...
contract TEST is ERC20, Ownable {
    ...
    function _transfer(
        ...
        uint256 maxTxAmount = totalSupply().mul(_maxTransfer).div(10000);
    ...
    function getUserTaxRelief(address addr) public view returns (uint8) {
        ...
        if(userTaxReliefStartDate > 0) {
            ...
            uint256 taxReliefPeriod = (block.timestamp - userTaxReliefStartDate).div(120);
    ...
    function _swap(uint256 amount) internal lockSwap {
        ...
        uint256 totalTaxes = totalTreasury.add(totalReflected);
        uint256 treasuryAmount = amount.mul(totalTreasury).div(totalTaxes);
        uint256 reflectedAmount = amount.sub(treasuryAmount);

        uint256 treasuryEth = tradeValue.mul(totalTreasury).div(totalTaxes);
        uint256 reflectedEth = tradeValue.sub(treasuryEth);
    ...
    function _takeTaxes(
        ...
        uint256 totalAmount = _treasuryAmount.add(_reflectAmount);
    ...
    )
    {
        ...
        reflect = amount.mul(reflectRate).div(100).mul(taxRelief).div(1000);
        treasury = amount.mul(treasuryRate).div(100).mul(taxRelief).div(1000);
        send = amount.sub(reflect).sub(treasury);
    }
