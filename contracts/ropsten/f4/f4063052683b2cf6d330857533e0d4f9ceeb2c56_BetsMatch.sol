...
contract BalanceToken is Ownable, Math {
	...
	address public _token = 0x37A4b8412CD5bD15F2473f172ad93F5a40376E67;
...
contract BetsMatch is Admins {
	...
	*/
	...
	// 	payable
	// {
	// 	createBet(hashEvent, hashMarketItem, hashOutcome, hashBet, msg.value, coef);
	// }
	...
	*/
		...
		public
	{
		...
		// emit AcceptBet(hashEvent, hashMarketItem, hashBet, status); // возможно отрубить тут евент для экономии газа
	}
	...
	function 	cancelEvent(
		...
		onlyModerator()
	{
		// если евент уже закрыт и есть победители может ли произойти отмена события?
		require(hashEvent != bytes16(0x0));
	...
	*/
	function 	createBet(
		...
		internal
	{
		require(_eventsMapping[hashEvent].hashMarketItem.length != 0); // если весов нету значет евента нету
		require(_marketItemMapping[hashMarketItem].outcomes.length != 0); // существует ли весы
		...
		require(hashBet != bytes16(0x0)); // отдельно чтобы был не пустой хеш
