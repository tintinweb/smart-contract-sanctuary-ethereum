...
import "../DummyLeaderToken.sol";

import "./ICopygameBridge.sol";

import { IAxelarGateway } from "@axelar-network/axelar-cgp-solidity/src/interfaces/IAxelarGateway.sol";

contract CopygameBridgePlugin is Ownable {
  using SafeERC20 for IERC20;

  uint256 public immutable thisChainId;
  CopycatLeaderFactory public immutable factory;
  DummyLeaderToken public immutable dummyLeaderToken;
  CopycatLeaderStorage public immutable S;
  IAxelarGateway public immutable gateway;

  mapping(uint256 => mapping(address => bool)) public whitelistedBridge;

  struct OriginalLeader {
    uint256 chainId;
    address leaderAddress;
  }

  mapping(uint256 => mapping(address => CopygameLeader)) public mirroredLeaders;
  mapping(address => OriginalLeader) public originalLeaders;

  modifier onlyWhitelistedBridge(uint256 chainId, address _bridge) {
    require(whitelistedBridge[chainId][_bridge], "Bridge Forbidden");
    _;
  }

  constructor(
    uint256 _thisChainId,
    CopycatLeaderFactory _factory,
    ...
    factory = _factory;
    dummyLeaderToken = _dummyLeaderToken;
    dummyLeaderToken.approve(address(_factory), type(uint256).max);
    S = _factory.S();
    gateway = _gateway;
  }

  event WhitelistBridge(address indexed caller, uint256 indexed chainId, address indexed bridge);
  function whitelistBridge(uint256 chainId, address _bridge) public onlyOwner {
    whitelistedBridge[chainId][_bridge] = true;
    emit WhitelistBridge(msg.sender, chainId, _bridge);
  }

  event CreditMaster(uint256 indexed chainId, address indexed originalLeader, address indexed tokenAddress, uint256 amount);
  function creditMaster(
    uint256 chainId, 
    address originalLeader,
    IERC20 tokenAddress,
    uint256 amount
  ) public onlyWhitelistedBridge(chainId, msg.sender) {
    CopygameLeader mirroredLeader = mirroredLeaders[chainId][originalLeader];
    tokenAddress.safeTransfer(address(mirroredLeader), amount);
    emit CreditMaster(chainId, originalLeader, address(tokenAddress), amount);
  }

  event NewDeployment(uint256 indexed chainId, address indexed originalLeader, address indexed mirroredLeader);
  function deploy(
    uint256 chainId, 
    address originalLeader,
    ...
    if (chainId != thisChainId && address(mirroredLeaders[chainId][originalLeader]) == address(0)) {
      dummyLeaderToken.mint(address(this), 0.0001 ether);

      address payable mirroredLeader = factory.deployLeaderGameMirror(
        _tokenName,
        _tokenSymbol,
        _description,
        _avatar
      ...
      CopygameLeader(mirroredLeader).transferOwnership(_leaderOwner);

      mirroredLeaders[chainId][originalLeader] = CopygameLeader(mirroredLeader);
      originalLeaders[mirroredLeader] = OriginalLeader({
        chainId: chainId,
        leaderAddress: originalLeader
      });

      emit NewDeployment(chainId, originalLeader, mirroredLeader);
    }
  }

  event Enable(address indexed caller, address indexed leader, uint256 indexed chainId, address bridge);
  function enable(address bridge, uint256 chainId, CopygameLeader leader) public payable onlyWhitelistedBridge(chainId, bridge) {
    require(msg.sender == leader.owner(), "Not leader");

    CopycatLeaderStorage.LeaderInfo memory info = S.getLeaderInfo(address(leader));

    uint256 originalChainId = thisChainId;
    address originalLeader = address(leader);

    OriginalLeader storage original = originalLeaders[address(leader)];

    if (address(original.leaderAddress) != address(0)) {
      originalChainId = original.chainId;
      originalLeader = original.leaderAddress;
    }

    mirroredLeaders[originalChainId][originalLeader] = leader;
    originalLeaders[address(leader)] = OriginalLeader({
      chainId: originalChainId,
      leaderAddress: originalLeader
    });

    ICopygameBridge(bridge).bridge{value: msg.value}(chainId, abi.encodeWithSelector(
      CopygameBridgePlugin(address(this)).deploy.selector,
      originalChainId,
      originalLeader,
      leader.owner(),
      info.tokenName,
    ...
    emit Enable(msg.sender, address(leader), chainId, bridge);
  }

  event TokenBridge(address indexed caller, address indexed leader, uint256 indexed chainId, address bridge, string symbol, uint256 amount);
  function tokenBridge(address bridge, uint256 chainId, CopygameLeader leader, string calldata symbol, uint256 amount) public payable onlyWhitelistedBridge(chainId, bridge) {
    require(msg.sender == leader.owner(), "Not leader");

    IERC20 token = IERC20(gateway.tokenAddresses(symbol));

    // require(token.balanceOf(address(leader)) > 0, Strings.toHexString(uint160(address(leader)), 20));

    token.safeTransferFrom(address(leader), address(this), amount);
    token.safeApproveNew(bridge, amount);
    ...
    ICopygameBridge(bridge).bridgeWithToken{value: msg.value}(chainId, abi.encode(originalChainId, originalLeader), symbol, amount);

    emit TokenBridge(msg.sender, address(leader), chainId, bridge, symbol, amount);
  }
}
...
import "./CopygameBridgePlugin.sol";

abstract contract ICopygameBridge {
  CopygameBridgePlugin public immutable controller;

  constructor(address _controller) {
    controller = CopygameBridgePlugin(_controller);
  }
  ...
  function bridge(uint256 chainId, bytes calldata payload) public payable onlyController {
    _bridge(chainId, payload);
  }

  function _bridgeWithToken(uint256 chainId, bytes calldata payload, string calldata symbol, uint256 amount) virtual internal;
  function bridgeWithToken(uint256 chainId, bytes calldata payload, string calldata symbol, uint256 amount) public payable onlyController {
    _bridgeWithToken(chainId, payload, symbol, amount);
  }
}
