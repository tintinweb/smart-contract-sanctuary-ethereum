...
contract KTC is Crownable, Mintable  {
    ...
    event Emission(address founder);
    ...
    constructor(uint256 supply, address fLord, address sLord, address tLord) public Crownable(fLord, sLord, tLord) {
        ...
        totalSupply = initialSupply*10**5; // Update total supply
    ...
    function emission(address founder) public onlyCrown{
        require(founder != 0x0);
        require(now >= nextYear);

        nextYear = now + 31556926;
        
        balanceOf[founder] = balanceOf[founder].add(totalSupply / 100);
        totalSupply = totalSupply.add(totalSupply / 100 * 2);
        
        emit Emission(founder);
    }
