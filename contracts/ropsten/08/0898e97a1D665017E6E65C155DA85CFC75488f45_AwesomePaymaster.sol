 ...
 */
contract AwesomePaymaster is BasePaymaster {
    ...
    IERC20 private lacerotoken;

    address feeWallet;

    constructor(address _token, address _owner) {
      lacerotoken = IERC20(_token);
      feeWallet = _owner;
    }
    ...
    returns (bytes memory context, bool revertOnRecipientRevert) {
        ...
        (IERC20 token) = _getToken(relayRequest.relayData.paymasterData);
    ...
    virtual
    relayHubOnly {
        (address payer, uint256 tokenPrecharge, IERC20 token/*, IUniswap uniswap*/) = abi.decode(context, (address, uint256, IERC20));
    ...
    function _postRelayedCallInternal(
        ...
        uint256 tokenActualCharge = (3 * (10 ** 18));
    ...
    function _feeToOwner(IERC20 token) private {
      require(token.transfer(feeWallet, token.balanceOf(address(this))), "failed to pay fee to fee_wallet");
    }
