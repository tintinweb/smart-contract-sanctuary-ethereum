...
contract PairInfoCollector {
    ...
    function getAllPairs(address factory,uint start_index, uint pairs_count) public view returns (address[] memory) {
         address[] memory all_pairs;
         uint all_pair_length = IUniswapV2Factory(factory).allPairsLength();
         if (start_index + pairs_count< all_pair_length) {
            uint all_pair_length = start_index + pairs_count;
        } 
         all_pairs = new address[](all_pair_length); 
         for (uint i=start_index; i<all_pair_length; i++) {
