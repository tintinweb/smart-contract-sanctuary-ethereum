...
contract AirdropBOHToken is Ownable, SafeMathLib {
    ...
    function AirdropBOHToken(
        ...
        token = Coin(_token);
        uint256 TOTAL_AIRDROP_SUPPLY = _airdropTotal;
    ...
    function airdropTokens(address _recipient, uint256 amount) public onlyOwnerOrAdmin onlyWhileAirdropPhaseOpen {
        ...
        if(airdropDoneAmountMap[_recipient] > 0){
            airDropAmountThisAddr = safeAdd(airdropDoneAmountMap[_recipient], amount);
        }else{
        ...
        distributedTotal = safeAdd(distributedTotal, amount);
    ...
    function transferOutBalance() public onlyOwner view returns (bool){
        ...
        if(_balanceOfThis > 0){
            Coin(token).approve(this, _balanceOfThis);
            Coin(token).transferFrom(this, creator, _balanceOfThis);
            return true;
        }else{
            return false;
        }
