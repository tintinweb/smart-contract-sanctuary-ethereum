...
contract FIEFAvatarStoreTransactor is AccessControlEnumerable, Signable, Pausable {
    ...
    */
    ...
    // Avatar address => Type Id ( > 0) => SKU Data
    mapping(address => mapping(uint8 => SKU)) internal skuConfig;
    ...
    function configNft(
        ...
        emit NftConfigured(nft, maxPerWallet, maxTokensToBuy, exist);
    }
    ...
    function configSku(
        ...
        // All IDs are based on index+1. See NFT.`_startTokenId` function.
        skuConfig[avatarAddress][typeId] = SKU({
    ...
    function getSkuConfig(address avatarAddress, uint8 typeId)
        external
        view
        returns (
            uint256 startTokenId,
    ...
    {
        SKU memory sku = skuConfig[avatarAddress][typeId];
        ...
        // If current > end, it means the type id is sold out.
        if (sku.exist) {
    ...
    function _trackSells(
        ...
        SKU storage skuInfo = skuConfig[nft][typeId];
