...
contract gameContract is usingOraclize, Mortal {
	...
	uint256 public callbackGas = 1000000; /* The gas amount required to call oraclize function */
...
contract bucketContract is Mortal {
  ...
  constructor(address gcAddress) public {
	...
	function setWinner(uint256 _gameId) public onlyGameContractOrOwner {
		...
		assert(uint(gameContractObject.state())== 0 || uint(gameContractObject.state()) == 6);
		assert(!isWinner);
		isWinner = true;
