 ...
 * @notice It vests the STORM tokens to an owner over a linear schedule.
...
contract VestingContractWithFeeSharing is Ownable, ReentrancyGuard {
     ...
     * @param _numberUnlockPeriods number of unlock periods (e.g., 5)
     * @param _maxAmountToWithdraw maximum amount in STORM to withdraw per period
     * @param _stormToken address of the STORM token
     */
    constructor(
    ...
    /**
     * @notice Unlock STORM tokens
     * @dev It includes protection for overstaking
     */
    function unlockStormToken() external nonReentrant onlyOwner {
        require(
            (numberPastUnlocks == NUMBER_UNLOCK_PERIODS) || (block.number >= nextBlockForUnlock),
            "Unlock: Too early"
        );

        uint256 balanceToWithdraw = stormToken.balanceOf(address(this));
        ...
        // Transfer STORM to owner
        stormToken.safeTransfer(msg.sender, balanceToWithdraw);
    ...
    function withdrawOtherCurrency(address _currency) external nonReentrant onlyOwner {
        require(_currency != address(stormToken), "Owner: Cannot withdraw STORM");
