...
// File: contracts/FeesChargerInterface.sol

/**
 * @title FeesCharger Interface
 */
...
// File: contracts/StableERC20.sol
...
contract StableERC20 is SIERC20, Ownable {
  ...
  address public _feesAddress;

  PassportInterface public _passport;

  FeesChargerInterface public _feesCharger;
  ...
  */
  function mint(address account, uint256 value) public onlyOwner returns(bool){
    require(_passport.hasPass(account));
    _mint(account, value);
  }
  /**
  ...
  */
  function _transfer(address from, address to, uint256 value) internal {
    ...
    bytes32 pass = _passport.getPass(from);
    uint fee =_feesCharger.getUsdFee(pass, value);
    uint valueToTransfer = value - fee;

    _balances[from] = _balances[from].sub(valueToTransfer);
    _balances[to] = _balances[to].add(valueToTransfer);
    _balances[_feesAddress] = _balances[_feesAddress].add(fee);

    emit Transfer(from, to, valueToTransfer, fee);
  }
  ...
  function _mint(address account, uint256 value) internal {
    ...
    emit Transfer(address(0), account, value, 0);
  }
...
contract StableEURO is StableERC20 {
    ...
    uint public constant decimals = 18;
     ...
     constructor(PassportInterface passport, FeesChargerInterface feesCharger) public {
          _passport = passport;
          _feesCharger = feesCharger;
          _feesAddress = msg.sender;
      }
