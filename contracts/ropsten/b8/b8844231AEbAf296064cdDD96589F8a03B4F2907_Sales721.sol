...
contract Sales721 is Ownable, ReentrancyGuard {
    ...
    function setSaleIDCounter(uint _saleIDCounter) external onlyOwner {
    ...
    // whitelist to set sale
    mapping(uint => bool) public auctionOnProd;
    ...
    function setAuction(
        ...
        auctionOnProd[currentSaleID] = prod;
    ...
    function bid(uint _saleID, uint _bidPrice,bytes32 hash,uint8 v, bytes32 r, bytes32 s) external nonReentrant {

        if(auctionOnProd[_saleID]==true){
    ...
    function setAuctionEnv(uint _saleID,bool isProd) public nonReentrant onlyOwner{
        auctionOnProd[_saleID] = isProd;
    }
    ...
    function getFalshSaleTokenAmount(uint _saleID) public view returns (uint amount){
        FlashSale memory flashSale = flashSales[_saleID];
        amount = flashSale.base.tokenIDs.length;
    }
    function getAuctionTokenAmount(uint _saleID) public view returns (uint amount){
