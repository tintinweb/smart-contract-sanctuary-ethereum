...
contract BOABridge {
    ...
    event Open(bytes32 _swapID, bytes32 _withdrawTrader, bytes32 _secretLock, uint _amount);
    ...
    function open(bytes32 _swapID, bytes32 _recipient, bytes32 _secretLock, uint256 _amount)
        notPaused() public onlyInvalidSwaps(_swapID)
    {

        require(_amount >= MINIMUM_AMOUNT, "Require transfer greater than minimum");

        require(token.transferFrom(msg.sender, address(this), _amount), "Failed to transfer tokens. Check Allowance?");
        ...
        // Trigger open event.
        emit Open(_swapID, _recipient, _secretLock, _amount);
    }
    ...
    function expire(bytes32 _swapID) public onlyOpenSwaps(_swapID) onlyExpirableSwaps(_swapID) {
        ...
        require(token.transfer(swap.BOASwapper, swap.value), "Failed to return tokens");
        // Expire the swap.
    ...
    function check(bytes32 _swapID) public view returns (uint256 timelock, uint256 value,
        bytes32 withdrawTrader, bytes32 secretLock, States state) {

        Swap memory swap = swaps[_swapID];
        return (swap.timelock, swap.value, swap.recipient, swap.secretLock, swapStates[_swapID]);
    }

    constructor (address _BOATokenAddressArg) {

        BOATokenAddress = _BOATokenAddressArg;
        token = IERC20(BOATokenAddress);
        owner = msg.sender;
    }
}
