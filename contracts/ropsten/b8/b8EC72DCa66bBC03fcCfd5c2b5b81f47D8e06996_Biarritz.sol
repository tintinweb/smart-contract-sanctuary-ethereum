...
contract Biarritz is ERC721, ERC2981ContractWideRoyalties, Ownable, RoyaltiesV2Impl {
    // TODO: ASQII OpenGem + desc + vision
    // TODO: comment readable on etherscan for each function
    // TODO : check si c'est sur opensea
    ...
    constructor(address _paidWallet, uint256 _maxSupply, bytes32 _merkleRoot) ERC721("Biarritz", "NFTB22") {
    ...
    function setImgData(string calldata _imagedata) public onlyOwner {
        emit Data(_imagedata);
    }

    function setTxHashImgData(string memory _txHash) public onlyOwner {
    ...
    function getBalanceOf721(address _nft, address _wallet)
        public
        view
        returns (uint256)
    {
        IERC721 ERC721Contract = IERC721(_nft);
        return ERC721Contract.balanceOf(_wallet);
    }
    ...
    function getBalanceOf1155(
        ...
        IERC1155 ERC1155Contract = IERC1155(_nft);
        return ERC1155Contract.balanceOf(_wallet, _id);
    }

    function append721Coupon(address _nft, uint256 _discount) public onlyOwner {
        getBalanceOf721(_nft, 0xdb8FFd3c97C1263ccf6AD75e43d46ecc65ef702a);
        ERC721List.push(_nft);
        couponStructs[_nft].discount = _discount;
    }
    ...
    function append1155Coupon(
        ...
        getBalanceOf1155(_nft, _id, 0xdb8FFd3c97C1263ccf6AD75e43d46ecc65ef702a);
        ERC1155List.push(_nft);
        couponStructs[_nft].id = _id;
        couponStructs[_nft].discount = _discount;
    }

    function getDiscount(address _buyer) public view returns (uint256) {
        uint256 discount = 0;
        for (uint256 i = 0; i < ERC721List.length; i++) {
            uint256 balance = getBalanceOf721(ERC721List[i], _buyer);
            if (balance >= 1) {
    ...
    function getFinalPrice(address _buyer) public view returns (uint256) {
        return basePrice - (getDiscount(_buyer) * basePrice) / 100;
    }
    ...
    function mint(uint256 _num) public payable {
        require(
            (msg.value >= getFinalPrice(msg.sender) * _num),
            "Ether value sent is not correct"
        );
        _mainMint(msg.sender, _num);

        payable(paidWallet).transfer(msg.value);
    }

    function emergencyMint(uint256 _num) public payable {
        require(
            (msg.value >= basePrice * _num),
            "Ether value sent is not correct"
        );
        _mainMint(msg.sender, _num);
    ...
    function _mainMint(address _to, uint256 _num) internal {
        require(tx.origin == _to, "The caller is another contract"); 
        require(sales, "Sales are closed"); 
        require(quantityMinted() + _num <= maxSupply, "Exceeds maximum supply");
