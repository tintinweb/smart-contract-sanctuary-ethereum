...
// Getters ====================
    ...
    function updateGuReferral(uint256 _pID, uint256 _affID, uint256 _eth) private {
        ...
        if (plyrPhas_[_affID][_newPhID].eth >= phrase_[_newPhID].minEthRequired && _remainGuReward >= 1e18) {
            ...
            uint256 _totalReward = (plyrPhas_[_affID][_newPhID].eth.mul(1e18)) / phrase_[_newPhID].minEthRequired;
        ...
        returns(uint256)
    {
        ...
        updateGenVault(_pID, plyr_[_pID].lrnd);
        plyrRnds_[_pID][plyr_[_pID].lrnd].genWithdraw = plyr_[_pID].gen.add(plyrRnds_[_pID][plyr_[_pID].lrnd].genWithdraw); // for doubled profit
