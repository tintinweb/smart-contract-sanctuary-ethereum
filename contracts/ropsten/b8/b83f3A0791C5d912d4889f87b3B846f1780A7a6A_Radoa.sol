...
contract VDFHashManager is VerifyVDFForRoot, VDFNodeManager {
    ...
    function getPreviousIndexOfNodeId(uint64 _nodeId) public view returns (uint64) {
        uint32 preIndex = uint32(lastIndexOfNodeId[_nodeId]);
        if(preIndex==0){
            preIndex = getJoinedIndexOfNodeId(_nodeId);
        }
        return preIndex;
    }
    ...
    function attestIndexAndTime(uint64 _nodeId, uint32 _newIndex, uint256 _numProof) private {
        uint preIndex = uint256(getPreviousIndexOfNodeId(_nodeId));
        require(uint256(_newIndex).sub(preIndex)==_numProof, "invalid numProof");
        uint32 limitIndex = uint32(uint256(uint32(preIndex)).add(_numProof).add(2));
...
interface ITimer {
    ...
    function computeOpenedIndex(uint64 _timestamp) external view returns (uint32);
...
contract Timer is ITimer {
    ...
    function computeOpenedIndex(uint64 _timestamp) public view override returns (uint32) {
        uint subTime = _timestamp.sub(deployedTime);
        uint index = uint64(subTime).div(UPDATE_PERIOD);
        return uint32(index);
    }

    function computetLastOpenedIndex() public view override returns (uint32) {
        return computeOpenedIndex(uint64(getBlockTimestamp()));
    }
