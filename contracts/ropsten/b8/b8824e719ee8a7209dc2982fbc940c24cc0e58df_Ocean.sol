    ...
    ReentrancyGuardUpgradeable
{
    ...
    // base reward rate
    uint256 public DAILY_WHALES_BASE_RATE;
    uint256 public DAILY_WHALES_RATE_TIER_1;
    uint256 public DAILY_WHALES_RATE_TIER_2;
    uint256 public DAILY_WHALES_RATE_TIER_3;
    uint256 public DAILY_WHALES_RATE_TIER_4;
    uint256 public DAILY_WHALES_RATE_TIER_5;
    uint256 public DAILY_WHALES_RATE_LEGENDRY;

    uint256 public INITIAL_MINT_REWARD_TIER_1;
    uint256 public INITIAL_MINT_REWARD_TIER_2;
    uint256 public INITIAL_MINT_REWARD_TIER_3;
    uint256 public INITIAL_MINT_REWARD_TIER_4;
    uint256 public INITIAL_MINT_REWARD_LEGENDRY_TIER;

    // amount of $WHALES earned so far

    uint256 public totalWhalesEarned;
    // number of Blues in the ocean
    uint256 public totalBluesStaked;
    // the last time $WHALES was claimed
    //uint16 public lastInteractionTimeStamp;
    // max $WHALES supply

    IERC721Upgradeable public Blues;
    IWhales public Whales;
    ...
    mapping(address => uint256) public totalBluesStakedPerAddress;
    mapping(address => bool) public userFirstInteracted;
    mapping(uint256 => bool) public initialMintClaimLedger;
    mapping(address => uint256) public legendryHoldingsPerUser;
        ...
        updateRewardsForUser(msg.sender)
    {
        ...
        require(tokenIds.length >= 1, "need at least 1 Ble");
    ...
    function _addBlueToOcean(address account, uint256 tokenId)
        internal
        whenNotPaused
    {
        ocean[tokenId] = Stake({owner: account, stakedAt: block.timestamp});
        ...
        internal
    {
        ...
        if (staking) {
            if (tokenId >= 15) legendryHoldingsPerUser[msg.sender]++;
        } else {
            if (tokenId >= 15) legendryHoldingsPerUser[msg.sender]--;
        }
    }
        ...
        updateRewardsForUser(msg.sender)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                ocean[tokenIds[i]].owner == msg.sender,
                "You are not the owner of this blue"
            );
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            else if (tokenIds[i] > 5555) // todo add the tokenIDs for the other 1/1
    ...
    function getDailyYield(address account) external view returns (uint256) {
        ...
        yeild += 
            (legendryHoldingsPerUser[account] * DAILY_WHALES_RATE_LEGENDRY);
