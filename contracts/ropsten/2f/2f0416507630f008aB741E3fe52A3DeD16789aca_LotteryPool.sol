...
contract LotteryPool is ReentrancyGuard,  Ownable{
    ...
    function CheckNFTholder(address _msgSender) internal view returns(bool){
        ...
        console.log("check NFTholder", balanceNFT);
    ...
    function CheckWinner(uint256 _lotteryId, address _msgSender) internal view returns(bool){
        for(uint256 i = 0; i < allLotteryInfo[_lotteryId]._playerList.length; i++){
            if(allLotteryInfo[_lotteryId]._playerList[i] == _msgSender){
                console.log("check playerlist",true);
                return true;
            }
        }
        console.log("check playerlist",false);
        return false;
    }
    ...
    function CloseLottery(address[] memory _playerlist) external onlyOwner{
        ...
        address winnerAddress = nftContractAddress.ownerOf(allLotteryInfo[lotteryid]._winNumber);
        uint256 remainPrice;
        allLotteryInfo[lotteryid]._closeDate = block.timestamp;
        bool isWinner = false;
        for(uint256 i = 0; i < _playerlist.length; i++){
            allLotteryInfo[lotteryid]._playerList.push(_playerlist[i]);
            if(_playerlist[i]== winnerAddress){
                if(lotteryid == 0){
                    remainPrice = 0;
                }else{
                    lotteryinfo memory temp2 = allLotteryInfo[lotteryid-1];
                    lotteryinfo memory temp1 = allLotteryInfo[lotteryid];
                    remainPrice = temp2._remainPrice + temp1._ticketPrice * _playerlist.length;
                }
                allLotteryInfo[lotteryid]._winnerReward = allLotteryInfo[lotteryid]._reward + remainPrice * 3 /4;
                allLotteryInfo[lotteryid]._fee = remainPrice * 1 / 4;
                allLotteryInfo[lotteryid]._remainPrice = 0;
                isWinner = true;                
            }
        }
        if(!isWinner){
            allLotteryInfo[lotteryid]._remainPrice = allLotteryInfo[lotteryid-1]._remainPrice + remainPrice;
            allLotteryInfo[lotteryid]._winnerReward = 0;
            allLotteryInfo[lotteryid]._fee = 0;
        }
