...
contract RolaGameUpgradeable is RolaGameInternalUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function claim(uint256[] calldata epochs) external override nonReentrant notContract {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            if (roundData[epochs[i]].DexCalled) {
                require(claimable(epochs[i], msg.sender), "RolaGame: Not eligible for claim");
     ...
     */
    function executeRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime, uint256 preroundId, uint256 endroundId) external override whenNotPaused onlyOperator {
        require(
        ...
        (int currentPrice) = _getPriceFromDex(round.dexAddress);
        _safeLockRound(preroundId, currentPrice);

        Round memory roundEnd = roundData[endroundId];
        (int currentPriceEndRound) = _getPriceFromDex(roundEnd.dexAddress);
        _safeEndRound(endroundId, currentPriceEndRound);
        _calculateRewards(endroundId);
        
        _safeStartRound(dexAddress, roundId, startRoundTime, roundExecutionTime, endroundId);
     ...
     * @dev Callable by operator
     */
    function genesisLockRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime, uint256 preRoundId) external override whenNotPaused onlyOperator {
        ...
        (int currentPrice) = _getPriceFromDex(round.dexAddress);
        _safeLockRound(preRoundId, currentPrice);
        _startRound(dexAddress, roundId, startRoundTime, roundExecutionTime);
        genesisRound[preRoundId].genesisLockOnce = true;
        genesisRound[roundId].genesisStartOnce = true;
    }
     ...
     */
    function genesisStartRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime) external override whenNotPaused onlyOperator {
...
abstract contract RolaGameInternalUpgradeable is DexPriceCalculationUpgradeable, IRolaGameUpgradeable {
    ...
    */
    ...
    function _safeStartRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime, uint256 endroundId) internal {
    ...
    */
    ...
    function _startRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime) internal {
        Round storage round = roundData[roundId];
        require(
            round.roundExecutionTime >= 0,
            "RolaGame: RolaGame: Round ID already taken"
        );
    ...
    */
    ...
    function _getPriceFromDex(address dexAddress) internal view returns (int) {
        (int currentPrice) = getTokenPriceInTokenZero(dexAddress);
        return (currentPrice);
    }
...
interface IRolaGameUpgradeable {
    ...
    function genesisStartRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime) external;

    function betBear(uint256 roundId, uint256 amount) external;

    function betBull(uint256 roundId, uint256 amount) external;

    function genesisLockRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime, uint256 preRoundId) external;

    function executeRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime, uint256 preroundId, uint256 endroundId) external;
...
contract DexPriceCalculationUpgradeable is Initializable {
     ...
     * @param dexAddress: Dex Address
    */
    function getTokenPriceInTokenZero(address dexAddress) public view returns(int) {
