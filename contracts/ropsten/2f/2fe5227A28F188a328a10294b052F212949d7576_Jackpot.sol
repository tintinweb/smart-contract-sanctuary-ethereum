...
contract Jackpot is Ownable, ReentrancyGuard {
    ...
    uint256 public playFee = 1000; //10000 is 100, so 10% is 1000 out of 10000
    uint64 public prepareTime = 1 days;
    uint64 public playTime = 1 weeks;
    uint256 minRequiredPlayer = 10;
    ...
    // mapping(uint256 => Status) public gameStatus_;
    // IERC20 public immutable token;

    // Game ID's to info
    mapping(uint256 => JackpotInfo) internal allGames_;

    // JackpotInfo[] public jackpotInfos;

    // address _currentWinner;
    // uint _currentMaxBalance;
    // uint private dayPassed;
    // uint public balance;
    ...
    function setGame(uint8 _plan, uint256 _dailyRange) public onlyOwner {
    ...
    function openGame(uint256 _gameID) public onlyOwner {
        require(
            allGames_[_gameID].gameStatus == Status.Prepairing,
    ...
    function claimReward(uint256 _gameID) public {
        ...
        JackpotInfo memory _currentGameInfo = allGames_[_gameID];
        require(
            _currentGameInfo.winner == msg.sender,
    ...
    function getPrize(uint256 _gameID) public {
        require(
            allGames_[_gameID].gameStatus == Status.Completed,
            "Yon cannot claim till finish."
        );
        ...
        payable(owner()).transfer(getBalance()*playFee/10000);
        payable(allGames_[_gameID].winner).transfer(getBalance()*(10000-playFee)/10000);         
        emit ClaimedPrize(allGames_[_gameID].winner, getBalance()*(10000-playFee)/10000 );
    ...
    function winnerAddress(uint256 _gameID) public view returns (address) {
        _gameID;
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < _playersOnGame; i++) {
            uint256 randNumber = (uint256(
                keccak256(abi.encodePacked(block.timestamp, _Random_Array[i]))
            ) % _playersOnGame) + 1;
