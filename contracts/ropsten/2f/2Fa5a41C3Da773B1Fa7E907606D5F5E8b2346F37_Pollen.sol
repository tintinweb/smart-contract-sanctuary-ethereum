...
contract Pollen is ReentrancyGuard, Pausable, Ownable {
    ...
    function claimAll(address _owner) public  {
        ...
        IPollenNft.OwnedNFT[] memory ownedNFTs = PollenNFT.getNFTsByOwner(_owner);
    ...
    function getAllRewards(address _owner) public view returns (uint256) {
        ...
        IPollenNft.OwnedNFT[] memory ownedNFTs = PollenNFT.getNFTsByOwner(_owner);
             ...
             for (uint i=0; i < ownedNFTs.length; i++) {
                 _totalRewards += calculateReward(ownedNFTs[i].tokenId, DAIPriceFeed);
            }    
    ...
    function getAllNfts(address _owner) public view returns(IPollenNft.OwnedNFT[] memory) {

        return  PollenNFT.getNFTsByOwner(_owner);
