...
contract MyERC20 is ERC20Interface {
    ...
    function transferFrom(address from, address to, uint tokens) override public returns (bool success) {
        ...
        // 检验地址是否合法
        require(to != address(0) && from != address(0),'sender not right');
        // 检验发送者账户余额是否足够
        require(balanceOf[from] >= tokens,'not enought fonds');
        // 检验操作的金额是否是被允许的
        uint alloweds = allowed[from][msg.sender];
        require(alloweds >= tokens,'allowed too bigger');
        // 检验是否会发生溢出
        require(balanceOf[to] + tokens >= balanceOf[to],'yichu yichu yichu');
    ...
    */
    ...
    function transferCollect(address[] calldata _from,  uint256 _value) payable public {
