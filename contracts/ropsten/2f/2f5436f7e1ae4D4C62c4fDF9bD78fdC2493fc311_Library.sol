...
contract Library is Ownable {
    ...
    mapping(address => mapping(uint256 => bool)) private userToBorrowedBookId;
    mapping(uint256 => address[]) private bookIdToBorrowerHistory;
    ...
    function borrowBooks(uint256[] calldata bookIds) external {
        ...
        for (uint256 i = 0; i < bookIds.length; i++) {
            ...
            userToBorrowedBookId[msg.sender][bookIds[i]] = true;
            book.copies--;
    ...
    function returnBook(uint256[] calldata bookIds) external {
        ...
        for (uint256 i = 0; i < bookIds.length; i++) {
            ...
            userToBorrowedBookId[msg.sender][bookIds[i]] = false;
