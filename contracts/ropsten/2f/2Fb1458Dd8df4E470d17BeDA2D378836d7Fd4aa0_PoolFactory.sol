...
contract PoolViews is PoolHelper {
        ...
        returns (uint256)
    {
        ...
        return multiplicativeFactor
            * newUtilisation
            * PRECISION_FACTOR_E18
            / ((resonanceFactor - newUtilisation) * resonanceFactor);
    }

    function getLoanPaymentDetails(
        address _nftAddress,
        uint256 _nftTokenId,
        uint256 _deadline
    )
        ...
        returns (uint256)
    {
        ...
        uint256 marginInterest = borrowRate
            * totalTokensDue
            * timeChange
     ...
     * calculates approriate shares to withdraw (in total including tokenised)
     */
...
contract PoolHelper is PoolBase, PoolShareToken, LiquidTransfer {
    ...
    function _newBorrowRate()
        internal
    {
        uint256 baseMultipicator = resonanceFactor
            * (resonanceFactor - utilisationRate);
        ...
        returns (bool)
    {
        return _shareValue < maxPoolShares
            * THRESHOLD_RESET_RESONANCE_FACTOR
            / ONE_HUNDRED;
    }
    ...
    /**
     * @dev Stepping function increasing the resonance
     * factor depending on the time past in the last time interval.
    ...
    */
    ...
    function merklePriceInPoolToken(
        ...
        returns (uint256)
    {
        if (chainLinkPoolToken == chainLinkETH) {
        ...
        if (chainLinkIsDead(chainLinkPoolToken) == true) {
        ...
        return valueETHinUSD
            * IChainLink(chainLinkPoolToken).decimals()
            / IChainLink(chainLinkPoolToken).latestAnswer()
...
contract PoolBase {
    ...
    address public chainLinkPoolToken;
...
contract LiquidRouter is LiquidTransfer, AccessControl, RouterEvents {
    ...
    function liquidateNFT(
        ...
        external
    {
        uint256 currentAuctionPrice = ILiquidPool(_pool).liquidateNFT(
            msg.sender,
            _nftAddress,
            _nftTokenId,
            _merkleIndex,
    ...
    /**
     * @dev
     * If user has not borrowed full amount
     * this allows to take out more funds
     */
...
contract LiquidPool is PoolViews, LiquidEvents {
    ...
    */
        ...
        external
    {
        ...
        chainLinkPoolToken = _chainLinkPoolToken;
        ...
        minResonaceFactor = PRECISION_FACTOR_E18 / 2
        ...
        maxResonaceFactor = PRECISION_FACTOR_E18 / 2
            + Babylonian.sqrt(PRECISION_FACTOR_E36 / 4
                + _multiplicationFactor
                    * PRECISION_FACTOR_E36
    ...
    /**
     * @dev Withdraw funds from pool but only from internal shares
     */
    ...
    /**
     * @dev Withdraw funds from pool but only from token shares.
...
interface ILiquidRouter {
    ...
    */
    ...
    /**
     * @dev Router interface to get chainlin ETH address for pool creation
    */
