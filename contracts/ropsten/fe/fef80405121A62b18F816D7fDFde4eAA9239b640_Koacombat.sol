...
// --------------------------rs ------------------------------
    ...
    constructor(
        ...
        _tTotal = __metacrypt_initial.mul(10**_decimals);
    ...
    function balanceOf(address account) public view override returns (uint256) {
        return _balanceOf(account).div(10**_decimals);
    }
...
// Functions to set wallets
    ...
    function _transfer(
        ...
        uint256 _amount = amount.mul(10**_decimals);
        require(from == pcsV2Pair || cooldown[from] < block.timestamp, "Cooldown is not ready yet!!");
        require(_amount + _balanceOf(to) <= _tTotal.div(10000).mul(maxTxPercentage), "the amount exceeds allowance 0.25 %");

        if(from != owner() && to != owner()) {
            if(to == pcsV2Pair && _amount >= _tTotal.div(10000).mul(maxRestrictionPercentage)){
...
// ------------------------- rs added ------------------------
    ...
    */
    function _burnTransfer(address account, uint256 amount) private {
        ...
        */
        _tTotal = _tTotal.sub(amount.mul(10**_decimals));
