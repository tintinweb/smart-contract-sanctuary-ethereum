...
contract ETHPool is Context,Ownable {
	...
	EnumerableMap.AddressToUintMap depositAccount;
	EnumerableMap.AddressToUintMap rewardAccount;
	...
	function deposit() public payable {
		...
		require(amount>0);
		(,uint256 depositBalance)=depositAccount.tryGet(user);
		depositBalance=depositBalance.add(amount);
		depositAccount.set(user,depositBalance);
		total=total.add(amount);
		emit Deposit(user, depositBalance);
	}

	function withdraw(uint256 amount) public {
		address payable user=payable(_msgSender());
		(,uint256 depositBalance)=depositAccount.tryGet(user);
		(,uint256 rewardBalance)=rewardAccount.tryGet(user);
		uint256 balance=depositBalance.add(rewardBalance);
		require(balance>=amount);
		balance=balance.sub(amount);

		user.transfer(amount);
		//withdraw from reward first
		if(amount>rewardBalance) {
			amount=amount.sub(rewardBalance);
			depositAccount.set(user,depositBalance.sub(amount));
			total=total.sub(amount);
			amount=rewardBalance;
		}
		rewardAccount.set(user,rewardBalance.sub(amount));
		emit Withdraw(user, balance);
	}

	function depositReward() public payable onlyOwner {
		uint256 amount=msg.value;
		uint256 size=depositAccount.length();
		for(uint256 i=0;i<size;i++) {
			(address user,uint256 depositBalance)=depositAccount.at(i);
			(,uint256 rewardBalance)=rewardAccount.tryGet(user);
			rewardAccount.set(user,rewardBalance.add(amount.mul(depositBalance).div(total)));
		}
	}
	...
	function balanceOf(address user) external view returns(uint256) {
		(,uint256 depositBalance)=depositAccount.tryGet(user);
		(,uint256 rewardBalance)=rewardAccount.tryGet(user);
		return depositBalance.add(rewardBalance);
	}
}
