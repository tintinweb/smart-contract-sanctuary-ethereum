...
contract Assassins is ERC721Enumerable, Ownable {
    ...
    uint256 public constant max_bills = 3333;
    ...
    constructor(string memory baseURI, string memory provenanceHash)
        ERC721("Boardwalk Assassins Guild", "BAG")
    {
        setBaseURI(baseURI);
    ...
    function approveFreeMints() public onlyOwner {
        uint256 functionIndex = _wlIndex;
        ...
        _wlIndex = functionIndex;
    }

    function checkWhitelistStatus() public view returns(bool) {
        return _wlIndex == (_bacOwners.length);
    }
    function checkWhiteListIndex() public view returns (uint256){
        return _wlIndex;
    }

    function walletExistsinWL(address _checkWallet) public view returns (bool){
    ...
    function whitelistMint (uint256 _count) public payable{
        ...
        require(walletExistsinWL(msg.sender),"Wallet Not Approved for Whitelist");
        require(tx.origin == msg.sender, "What are you doing, stop that.");
        require(totSupply < max_bills, "All Cases have been Claimed.");
        require(totSupply + _count <= max_bills,"There are not enough Cases available");
        require(_wlMinted[msg.sender] + _count <= 5, "Wallet would exceed wl limit");       
...
abstract contract BoardApeCollective is ERC721Enumerable, Ownable {
