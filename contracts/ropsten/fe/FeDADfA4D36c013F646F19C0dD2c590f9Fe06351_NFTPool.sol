...
contract NFTPool is Ownable {
    ...
    uint256 public aprPercent1 = 15;
    uint256 public aprPercent2 = 20;
    uint256 public aprPercent3 = 25;
    ...
    function getReward(address account) public view returns (uint256) {
        uint256 reward = 0;
        if(DURATION == 90 days){
            reward = deposits[account] * aprPercent1 * (block.timestamp - lastUpdate[account]) / 100 / 30 days;
        }
        if(DURATION == 180 days){
            reward =  deposits[account] * aprPercent2 * (block.timestamp - lastUpdate[account]) / 100 / 30 days;
        }
        if(DURATION==30 days){
           reward =  deposits[account] * aprPercent3 * (block.timestamp - lastUpdate[account]) / 100 / 30 days;
        }
        return reward;
