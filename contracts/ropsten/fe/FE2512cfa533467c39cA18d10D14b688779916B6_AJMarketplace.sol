...
contract AJMarketplace is ReentrancyGuard {
    ...
    // // Put item on Auciton
    // function putOnAuction(uint256 _tokenId, uint256 _minAmount) external nonReentrant {
    //     Item storage item = items[_tokenId];
    //     require(IERC721(nftContractAddr).ownerOf(_tokenId) == msg.sender, "Only Nft Owner can Put NFT for Auction");
    //     require(!item.forAuction, "item already exist on Auction");
    //     require(!item.forSale, "item already exist on Fixed Price");
    //     items[_tokenId] = Item (_tokenId, nftContractAddr, _minAmount, payable(msg.sender), false, true);
    // }
   ...
   // --------------------------------- Auction Start ------------------------------------------ //
    ...
    function transferToHigherBidder(uint256 _nftId) external payable {
        ...
        if (AuctionDataset[_nftId].highestBidder != address(0)) {
            ...
            // IERC721(nftContractAddr).transfer(AuctionDataset[_nftId].seller, _nftId);
