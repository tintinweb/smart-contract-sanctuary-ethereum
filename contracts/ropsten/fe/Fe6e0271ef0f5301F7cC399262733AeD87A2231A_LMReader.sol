...
import {IKyberSwapElasticLMV2} from './IKyberSwapElasticLMV2.sol';
import {IPoolStorage} from './IPoolStorage.sol';
import {QtyDeltaMath} from './QtyDeltaMath.sol';
import {TickMath} from './TickMath.sol';

contract LMReader {
  IKyberSwapElasticLMV2 public immutable lm;

  constructor(IKyberSwapElasticLMV2 _lm) {
    lm = _lm;
  }

  function getPositionTokenAmounts(uint256 pId, uint256[] calldata tokenIds)
    external
    view
    returns (int256 token0Amount, int256 token1Amount)
  {
    (address poolAddress, , , , , , , , ) = lm.getPoolInfo(pId);
    (uint160 sqrtP, int24 currentTick, , ) = IPoolStorage(poolAddress).getPoolState();

    for (uint256 i = 0; i < tokenIds.length; i++) {
      (IBasePositionManager.Position memory posData, ) = IBasePositionManager(lm.nft()).positions(
        tokenIds[i]
      );
...
interface IKyberSwapElasticLMV2 is IKyberSwapElasticLMEvents {
  struct RewardData {
    address rewardToken;
