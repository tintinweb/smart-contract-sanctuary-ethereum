...
contract Arb is Ownable {
  ...
  function estimateDexTrade(address _router1, address _token1, address _token2, uint256 _amount) external view returns (uint256) {
		uint256 amtBack1 = getAmountOutMin(_router1, _token1, _token2, _amount);
		return amtBack1;
	}
  function dexTrade(address _router1, address _token1, address _token2, uint256 _amount) external onlyOwner {
  ...
  function dualDexTrade(address _router1, address _router2, address _token1, address _token2, uint256 _amount) external onlyOwner {
    ...
    require(endBalance >0, "Trade Reverted, No Profit Made");
    require(startBalance >0, "Trade Reverted, No Profit Made1");
  }
  ...
  function quadDexTrade(address _router1, address _router2, address _router3, address _router4, address _token1, address _token2, address _token3, address _token4, uint256 _amount) external onlyOwner {
    uint startBalance = IERC20(_token1).balanceOf(address(this));
    uint token2InitialBalance = IERC20(_token2).balanceOf(address(this));//These could be assumed to be zero.
    uint token3InitialBalance = IERC20(_token3).balanceOf(address(this));
  ...
  function quadDexTrade1(address _router1, address _router2, address _router3, address _router4, address _token1, address _token2, address _token3, address _token4, uint256 _amount) external onlyOwner {
    uint startBalance = IERC20(_token1).balanceOf(address(this));
    ...
    swap(_router2,_token2, _token3,tradeableAmount);
    uint token3Balance = IERC20(_token3).balanceOf(address(this));
    tradeableAmount = token3Balance;

    swap(_router3,_token3, _token4,tradeableAmount);
    uint token4Balance = IERC20(_token4).balanceOf(address(this));
    tradeableAmount = token4Balance;
  ...
  function multiDexTrade(address[] memory _routers, address[] memory _tokens, uint256 _amount, uint256 _fees) external onlyOwner {
    ...
    require(_fees<0, "Trade Reverted, No Profit Made");
  }
