...
contract ERCX is ERC721BasicToken{
    ...
    function createNewAsset(string _name, string _symbol, uint _supply) public {
        ...
        assets[newAssetToken] = Asset(_name,_symbol ,newAssetToken, msg.sender, _supply, nonce);
        tokens.push(newAssetToken);
        _mint(msg.sender, nonce); 
        nonce++;
        emit AssetAdded(_name, _supply, newAssetToken);
    }
    ...
    function getUserAssetBalance(address _tokenAddr) view public returns (uint amount, uint supply, string name, string symbol){
        uint _balance = ERC20(_tokenAddr).balanceOf(msg.sender) / (10 **18);
        return (_balance, assets[_tokenAddr].supply, assets[_tokenAddr].name, assets[_tokenAddr].symbol);
    }
    
    function getAssetData(address _tokenAddr) 
    view 
    public 
    ...
    function checkAssetListLength() view public returns(uint) {
