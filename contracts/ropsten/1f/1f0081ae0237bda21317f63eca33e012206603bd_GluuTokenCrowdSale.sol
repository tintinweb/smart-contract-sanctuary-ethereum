...
contract Crowdsale {
  ...
  function createTokenContract() internal returns (GluuToken) {
  ...
  function calculateBonus(uint256 weiAmount) internal returns (uint256) {
    ...
    if (bonus != 0) {
      ...
      if (weiAmount > bonusMaxWei){
        ...
        uint256 bonusEligibleWei = bonusMaxWei.mul(bonus).div(100);
        uint256 totalEligibleWei = bonusEligibleWei.add(bonusMaxWei);
        uint256 bonusEligibleTokens = totalEligibleWei.mul(rate);

        uint256 remainingTokens = remainingWei.mul(rate);

        tokens = bonusEligibleTokens.add(remainingTokens);

      } else {
        eligibleWei = weiAmount;
        uint256 bonusWei = eligibleWei.mul(bonus).div(100);
        uint256 totalWei = bonusWei.add(weiAmount);
        tokens = totalWei.mul(rate);
      }
...
contract GluuTokenCrowdSale is Crowdsale, RefundableCrowdsaleWithCommission {
    ...
    **/
    ...
    function validPurchase() internal view returns (bool) {
        ...
        require((token.totalSupply()) + (msg.value.mul(rate)) <= maxTokenSupply);
