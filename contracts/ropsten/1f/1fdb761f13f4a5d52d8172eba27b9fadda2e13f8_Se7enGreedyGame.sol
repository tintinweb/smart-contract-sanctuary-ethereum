...
contract Se7enGreedyGame is modularLong {
    ...
    uint256 private roundExtra = 1000000; // extSettings.getLongExtra();     // length of the very first ICO
    uint256 private roundGap = 1000000; // extSettings.getLongGap();         // length of ICO phase, set to 1 year for EOS.
        ...
        payable
   {
          ...
          emit GreedyGameEvent.Deposit(msg.sender, msg.value);
    ...
    private
    {
      ...
      {
          emit GreedyGameEvent.Log("in round is activie....");
          // call core
          core(_currendRoundId, _playerId, msg.value);
      // if round is not actives
      } else {
          emit GreedyGameEvent.Log("in round is over....");

          // check to see if end round needs to be ran
          if (_now > rounds[_currendRoundId].end && rounds[_currendRoundId].gameover == false)
          {

              emit GreedyGameEvent.Log("we start to end the game previous round....");
              ...
              (
                  msg.sender,
                  msg.value,
                  rounds[_currendRoundId].winnerAddr,
                  rounds[_currendRoundId].amountWon,
                  rounds[_currendRoundId].newPot,
                  rounds[_currendRoundId].genAmount
              ); */
          }
        ...
        private
    {
      ...
      if (_eth > 1000000000)
      {
          ...
          // distribute eth
          distributeInternal(_currentRoundId, _playerId, _eth, _keys);

          // call end tx function to fire end tx event.
          endTx(_playerId, _eth, _keys);
      }
    }
     ...
     */
    function managePlayer(uint256 _playerId)
        private
    {
     ...
     */
    function distributeInternal(uint256 _currentRoundId, uint256 _playerId, uint256 _eth, uint256 _keys)
        private
    {
        // calculate gen share
     ...
     */
    function endTx(uint256 _playerId, uint256 _eth, uint256 _keys)
        private
    {
        // TODO send.... event
