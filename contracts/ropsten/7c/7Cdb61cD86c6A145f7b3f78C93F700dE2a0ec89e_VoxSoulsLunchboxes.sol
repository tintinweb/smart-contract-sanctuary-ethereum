    ...
    Signing
{
    ...
    function batchMintLunchboxes(
        ...
        require(isValid, "Invalid signature");
        require(
            msg.value >= getCurrCost(id.length),
            "Not enough either to mint"
        );

        for (uint256 i = 0; i < id.length; i++) {
            require(
                !_isMinted[_contract[i]][id[i]],
                "Lunchbox already minted."
            );
            ...
            uint256 voxSign = (mintTime[i] % 60) % 12;
            _isMinted[_contract[i]][id[i]] = true;
            currentLunchboxes++;
            _mint(account, voxSign, 1, "0x0");
            emit SoulMinted(account, _contract[i], id[i], voxSign);
        }
    }
