...
contract VoxToken is ERC20Burnable, Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    ...
    IERC721Enumerable public immutable residentContract;
    ...
    constructor(        
        ...
        address _residentContract
    ) ERC20(name, symbol)  {

        residentContract = IERC721Enumerable(_residentContract);
    ...
    function stakeResidents(uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        require(tokenIds.length <= 40 && tokenIds.length > 0, "Stake: amount prohibited");

        for (uint256 i; i < tokenIds.length; i++) {
            require(residentContract.ownerOf(tokenIds[i]) == msg.sender, "Stake: sender not owner");

            residentContract.safeTransferFrom(msg.sender, address(this), tokenIds[i]);
    ...
    function unstakeResidents(uint256[] calldata tokens) external whenNotPaused nonReentrant {
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            residentContract.safeTransferFrom(address(this), msg.sender, tokens[i]);
        }
    ...
    function claimHolderRewards() external whenNotPaused {
        ...
        for(uint256 i; i < residentContract.balanceOf(msg.sender); i++) {

            uint256 tokenId = residentContract.tokenOfOwnerByIndex(msg.sender, i);
    ...
    function stakedResidentsOf(address account) external view returns (uint256[] memory) {
