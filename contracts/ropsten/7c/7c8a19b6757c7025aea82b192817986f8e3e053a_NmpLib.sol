...
contract NftMarketplace is Configurable,ContextUpgradeable,IERC721ReceiverUpgradeable {
   ...
   function getFee(address nft) public view  returns(uint fee) {
    ...
    function claimAllMarket() external {
        IDOTC(address2(config[_DOTC_])).claimFor(msg.sender);
        claimFor(msg.sender);
    }
    ...
    function anyF() public returns (uint) {
        x = 3;
        return x;
    }
 
    function anyFGov() public governance returns (uint) {
        x = 5;
        return x;
    }

    function anyFGov2() external governance returns (uint) {
        x = 5;
        return x;
    }

    function anyFGov3() external returns (bool) {
...
interface IDOTC {
    ...
    function claimFor(address acct) external ;
}
