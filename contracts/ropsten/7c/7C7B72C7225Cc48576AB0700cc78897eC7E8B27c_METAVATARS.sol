...
contract METAVATARS is ERC721Enumerable, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    function privateSaleMint (bytes32[] calldata proofPrivateMerkle, uint32 amount) external payable nonReentrant {
        ...
        require(amount <= MAX_MINT_PER_RAFFLE_SALE_WALLET, "METAVATARS: Max 2 Tokens mint at once !");
        require(totalSupply + amount <= MAX_TOTAL_SUPPLY, "METAVATARS: You're mint amount is too large for the remaining tokens !");
        require(tokensPerWallet[msg.sender] + amount <= MAX_MINT_PER_RAFFLE_SALE_WALLET, "METAVATARS: Max 2 Tokens Mintable per Wallet !");
    ...
    function publicSaleMint (uint32 amount) public payable nonReentrant {
        ...
        if (amount <= 3) {
            ...
            require(msg.value >= MAX_MINT_PER_PUBLIC_SALE_WALLET, "METAVATARS: Insufficient Funds !");
        }
        require(amount <= MAX_MINT_PER_PUBLIC_SALE_WALLET, "METAVATARS: Max 5 Tokens mint at once !");
        require(totalSupply + amount <= MAX_TOTAL_SUPPLY, "METAVATARS: You're mint amount is too large for the remaining tokens !");
        require(tokensPerWallet[msg.sender] + amount <= MAX_MINT_PER_PUBLIC_SALE_WALLET, "METAVATARS: Max 5 Tokens Mintable per Wallet !");
