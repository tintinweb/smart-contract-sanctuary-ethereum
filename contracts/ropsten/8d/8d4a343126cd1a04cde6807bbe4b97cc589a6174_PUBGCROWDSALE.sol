...
contract PUBGCROWDSALE is owned, Killable {
    ...
    ///Softcap changed
    event Soft_CapChanged(uint256 oldValue, uint newValue);
    ///hardcap changed
    event Hard_CapChanged(uint256 oldValue, uint newValue);
    ...
    function setSoft_Cap(uint256 value) onlyOwner public {
        require(!finalized);
        require(value > 0);
        emit Soft_CapChanged(Soft_Cap, value);
        Soft_Cap = value;
    }
        function setHard_Cap(uint256 value) onlyOwner public {
        require(!finalized);
        require(value > 0);
        emit Hard_CapChanged(Hard_Cap, value);
        Hard_Cap = value;
        }

    function finalize() public onlyOwner {
        // Finalized  ICO crowdsele.
        finalized = true;
