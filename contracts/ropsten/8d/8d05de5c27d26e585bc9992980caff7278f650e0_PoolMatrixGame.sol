...
contract PoolMatrixGame {
    ...
    */
    ...
    // struct UserIncrements24h {
    //     uint32 date;
    //     uint256 referrerRewards;
    // }

    // struct GlobalIncrements24h {
    //     uint32 date;
    //     uint32 totalUsers;
    ...
    // mapping (address => UserIncrements24h[]) userIncrements24h;
    // GlobalIncrements24h[] globalIncrements24h;
    ...
    constructor(address regFee, address marketing, address[] memory owners, address[] memory roots) {
        ...
        for (i = 0; i < ROOT_WALLETS_COUNT; i++) {
            ...
            // for (j = 0; j < 4; j++) {
            //     userIncrements24h[addr].push(UserIncrements24h({
        ...
        // for (i = 0; i < 4; i++) {
        //     globalIncrements24h.push(GlobalIncrements24h({
        //         date: uint32(block.timestamp / SECONDS_IN_DAY),
        //         totalUsers: 0,
        //         totalTransactions: 0,
        //         totalTurnover: 0
    ...
    function register(address referrer, uint256 investAmount, uint256 initialBalance) public payable {
        ...
        // // Allocating increments
        // for (uint i = 0; i < 4; i++) {
        //     userIncrements24h[msg.sender].push(UserIncrements24h({
        //         date: uint32(block.timestamp / SECONDS_IN_DAY),
        //         referrerRewards: 0
        //     }));
        // }
        ...
        // globalIncrements24hNormalize(quarter);
        // globalIncrements24h[quarter].totalUsers++;
        // globalIncrements24h[quarter].totalTurnover += REGISTRATION_PRICE;
    }
    ...
    function buyLevel(uint8 level, uint256 investAmount) public payable {
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            // // Updating referrel rewards increment
            // userIncrements24hNormalize(quarter, referrer); todo
            // userIncrements24h[referrer][quarter].referrerRewards += value;
        ...
        if (rewardAddress != msg.sender) {
            ...
            bool sent = payable(rewardAddress).send(reward);// todo
            if (sent) {
                investAmount -= reward;
        ...
        // // Updating increments
        // globalIncrements24h[quarter].totalTransactions++;
        // globalIncrements24h[quarter].totalTurnover += investAmount;
    }

    // function globalIncrements24hNormalize(uint32 quarter) private {
    //     uint32 date = uint32(block.timestamp / SECONDS_IN_DAY);
    //     if (globalIncrements24h[quarter].date != date) {
    //         globalIncrements24h[quarter].date = date;
    //         globalIncrements24h[quarter].totalUsers = 0;
    //         globalIncrements24h[quarter].totalTransactions = 0;
    //         globalIncrements24h[quarter].totalTurnover = 0;
    //     }
    // }

    // function userIncrements24hNormalize(uint32 quarter, address userAddr) private {
    //     uint32 date = uint32(block.timestamp / SECONDS_IN_DAY);
    //     if (userIncrements24h[userAddr][quarter].date != date) {
    //         userIncrements24h[userAddr][quarter].date = date;
    //         userIncrements24h[userAddr][quarter].referrerRewards = 0;
    //     }
    // }
    ...
    function getTotalInfo() public view returns(uint256 totalUsers, uint256 totalTransactions, uint256 totalTurnover, uint32 totalUsersIncrement, uint32 totalTransactionsIncrement, uint256 totalTurnoverIncrement) {
        uint32 usersIncrement = 0;//globalIncrements24h[0].totalUsers + globalIncrements24h[1].totalUsers + globalIncrements24h[2].totalUsers + globalIncrements24h[3].totalUsers;
        uint32 transactionsIncrement = 0;//globalIncrements24h[0].totalTransactions + globalIncrements24h[1].totalTransactions + globalIncrements24h[2].totalTransactions + globalIncrements24h[3].totalTransactions;
        uint256 turnoverIncrement = 0;//globalIncrements24h[0].totalTurnover + globalIncrements24h[1].totalTurnover + globalIncrements24h[2].totalTurnover + globalIncrements24h[3].totalTurnover;
    ...
    // function getUserReferrerRewardsIncrement(address userAddr) public view returns(uint256) {
    //     return userIncrements24h[userAddr][0].referrerRewards + userIncrements24h[userAddr][1].referrerRewards + userIncrements24h[userAddr][2].referrerRewards + userIncrements24h[userAddr][3].referrerRewards;
    // }
