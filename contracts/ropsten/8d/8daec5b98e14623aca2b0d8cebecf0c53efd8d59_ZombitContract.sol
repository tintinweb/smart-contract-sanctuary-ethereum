...
contract ZombitContract is ERC721Enumerable, Ownable {
    ...
    * Mint Zombits
    */
    function mintZombits(uint numberOfTokens) public payable {
		require(numberOfTokens <= maxZombitPurchase, "Exceeds max number of Zombits in one transaction");
        require(saleStatus == 1 || saleStatus == 2, "Purchase not open");
        require(totalSupply().add(numberOfTokens) <= maxZombits, "Purchase would exceed max supply of Zombits");
        require(zombitPrice.mul(numberOfTokens) == msg.value, "Ether value sent is not correct");
        // Pre-sale
        if (saleStatus == 1) {
            require(whitelist[_msgSender()], "No permission to participate in the pre-sale");
            require(totalAmount[_msgSender()] + numberOfTokens <= presaleMaxZombitPurchase, "Exceeded quantity limit");
        }    
