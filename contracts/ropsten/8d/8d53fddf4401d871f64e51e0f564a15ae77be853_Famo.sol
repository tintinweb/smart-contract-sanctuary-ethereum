...
contract Famo{
    ...
    function getPlayerInfoByAddress(address myAddr)
        public 
        view 
        returns( uint256 myKeyNum, uint256 myValut,uint256 affGen,uint256 lockGen,uint256 affCodeSelf )
    {
        // setup local rID
	...
	function MakeWinner(address _pID, uint256 _keyNum, uint256 iCurRdIdx) private {
		for (uint256 i = 0; i < 30; i++) {
			if (roundList[iCurRdIdx].winerList[i].plyr == _pID) {
				roundList[iCurRdIdx].winerList[i].plyr = 0x0;
				break;
			}
		}
		WinPerson[30] memory winerList;
		uint256 j = 0;
		for (i = 0; i < 30; i++) {
			if (roundList[iCurRdIdx].winerList[i].plyr != 0x0) {
				winerList[j] = roundList[iCurRdIdx].winerList[i];
				j++;
			}
		}
		if (j < 30) {
			winerList[j].plyr = _pID;
			winerList[j].iLastKeyNum = _keyNum;
		}
		else {
			for (i = 0; i < 29; i++) {
				winerList[i] = winerList[i + 1];
			}
			winerList[29].plyr = _pID;
			winerList[29].iLastKeyNum = _keyNum;
		}
		for (i = 0; i < 30; i++) {
			roundList[iCurRdIdx].winerList[i] = winerList[i];
		}
	}
