    ...
    ReentrancyGuardUpgradeable
{
    ...
    function createMarketItem(uint256 tokenId, string memory name) public {
    ...
    function toggleForSale(uint256 itemId) public {
        bool currentStatus = idMarketItems[itemId].forSale;
        idMarketItems[itemId].forSale = !currentStatus;
    }

    /// @notice fetch list of all NFTs including sold and unsold items
    function fetchItemsAll() public view returns (MarketItem[] memory) {
        ...
        returns (MarketItem[] memory)
    {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            ...
            //by checking if the owner filed is empty
            if (idMarketItems[i + 1].owner == address(0)) {
