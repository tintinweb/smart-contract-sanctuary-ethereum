...
contract VaultContractV2 is IERC721Receiver, Initializable {
    ...
    uint256 public investorTotalSupply;
    ...
    function investorMint(address _to, uint _shares) private {
        investorTotalSupply += _shares;
        investorBalance[_to] += _shares;
    }

    function investorBurn(address _from, uint _shares) private {
        investorTotalSupply -= _shares;
        investorBalance[_from] -= _shares;
    }

    function investorDeposit(uint _amount) external {
        IERC20 investorToken = IERC20(usdc);
        uint shares;
        if (investorTotalSupply == 0) {
            shares = _amount;
        } else {
            shares = (_amount * investorTotalSupply) / vaultBalance;
        }
    ...
    function investorWithdraw(uint _amount) external {
        ...
        IERC20 investorToken = IERC20(usdc);
        uint shares = (_amount * investorTotalSupply) / vaultBalance;
        require(investorBalance[msg.sender] >= shares, "Not enough shares");
        investorBurn(msg.sender, shares);
        investorToken.transfer(msg.sender, _amount);
        vaultBalance = vaultBalance - _amount;
    }

    function investorGetBalance() public view returns (uint) {
        require(vaultBalance != 0, "No Balance in the Contract, Deposit First");
        uint share = investorBalance[msg.sender];
        uint amount = (share * vaultBalance) / investorTotalSupply;
        return amount;
    }
    ...
    function getFreezeState() public view returns (bool) {
        bool state = freezeWithdrawal;
        return state;
    }
