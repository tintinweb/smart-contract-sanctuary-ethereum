...
// File contracts/interface/Mintable.sol
...
// File contracts/interface/ERC20BurnableMintable.sol
...
// File contracts/ACDMPlatform.sol
...
contract ACDMPlatform is Ownable, ReentrancyGuard {
    ...
    //todo: temporary
    event ErrorDuringSwap(bytes message);

    event ErrorDuringSwap(string message);
    ...
    function spendFees(bool sendToOwner, uint256 deadline) public onlyDAO initialized nonReentrant {
        ...
        if (sendToOwner) {
            ...
            //todo arefev: check
            try uniswapRouter.swapExactETHForTokens{value : value}(0, path, address(this), deadline) returns (uint[] memory amounts) {
                xxxToken.burn(amounts[2]);
            } catch Error(string memory reason) {
                emit ErrorDuringSwap(reason);
