...
contract Phrases is Ownable {
  ...
  function mint(string memory phrase, uint256 charsInPhrase) public payable returns (uint256) {
    require(currentPriceToMint(charsInPhrase) <= msg.value, "Eth value sent is not sufficient");
  ...
  function currentPriceToMint(uint256 charsInPhrase) public view returns (uint256) {
    return basePrice * charsInPhrase;
  }
