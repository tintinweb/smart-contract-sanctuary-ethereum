...
interface IERC721Enumerable is IERC721 {
    ...
    function tokensGoldlist() external view returns(uint);
...
contract StakingPool is ERC20, Ownable {
    ...
    constructor() ERC20("Wing Wars", "WW") {
    ...
    function depositAll() external {
        ...
        for (uint i = 0; i < balance; i++) {
            _deposit(NFTCollection.tokenOfOwnerByIndex(msg.sender, i));
        }
    ...
    function calculateReward(uint tokenId) public view returns(uint256) {
        ...
        if (tokenId <= NFTCollection.tokensGoldlist()) {
            uint256 publicSaleLaunchTime_ = NFTCollection.publicSaleLaunchTime();
            if (publicSaleLaunchTime_ == 0) {
                return ((block.timestamp - checkpoints[tokenId]) / 300) * 15000000000;
            }
            else if (checkpoints[tokenId] < NFTCollection.publicSaleLaunchTime()) {
                return (((publicSaleLaunchTime_ - checkpoints[tokenId]) / 300) * 15000000000) + (((block.timestamp - publicSaleLaunchTime_) / 300) * 8000000000);
            }
            else {
                return ((block.timestamp - checkpoints[tokenId]) / 300) * 8000000000;
            }
        }
        else if (tokenId <= NFTCollection.tokensWhitelist()) {
            return ((block.timestamp - checkpoints[tokenId]) / 300) * 7000000000;
        }
        else {
            return ((block.timestamp - checkpoints[tokenId]) / 300) * 6000000000;
        }
