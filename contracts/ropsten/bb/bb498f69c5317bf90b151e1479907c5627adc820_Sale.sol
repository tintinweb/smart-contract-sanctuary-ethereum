...
contract Sale {
    ...
    Player[] public arrPlayers; // array of players to tract and distribute them tokens when the game ends
    ...
    function participateEvent(address referredAddress) external payable {
        ...
        if (timeLeft >= maxTimeInHours || now >= eventSaleEnd)
        {
            gameOver();
        }
        else {
    ...
    function distribute(address sender, uint256 value, address referredAddress) private {
        ...
        if (now.sub(checkpoint) >= maxTimeInHours)
            checkpoint = now;
