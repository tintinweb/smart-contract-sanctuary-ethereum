...
contract MOJICLUB is RewardableERC721, Whitelist {
    ...
    // Traits scarcity variables
    string private _traitsScarcitySHA;
    // Quantity of each trait that has been minted already. _traitsMintedQty[12]=23 if trait 12 is in 23 minted NFTs.
    mapping(uint8 => uint256) private _traitsMintedQty;
    ...
    function TraitsQty(uint8 _nbTraits) public view returns (uint256[] memory){
        uint256[] memory traitsQty = new uint256[](_nbTraits+1);
        for (uint8 i = 0; i <= _nbTraits; i++) {
            traitsQty[i] = _traitsMintedQty[i];
        }
        return traitsQty;
    }
    ...
    * Mint one token
    function mint(
        string memory _base36_msg, bytes32 _base36_hash, uint8 _base36_v, bytes32 _base36_r, bytes32 _base36_s,
        string memory _url_msg, bytes32 _url_hash, uint8 _url_v, bytes32 _url_r, bytes32 _url_s,
        string memory _ids_msg, bytes32 _ids_hash, uint8 _ids_v, bytes32 _ids_r, bytes32 _ids_s
    ) public payable {
    */
    function mint(string[3] memory _msgs, bytes32[9] memory _hashs_r_s, uint8[3] memory _v) public payable {
        /*
        require(ecrecover(_base36_hash, _base36_v, _base36_r, _base36_s) == _hash_signer && _base36_hash==sha256(abi.encodePacked(_base36_msg)),"Signature invalid");
        require(ecrecover(_url_hash, _url_v, _url_r, _url_s) == _hash_signer && _url_hash==sha256(abi.encodePacked(_url_msg)),"IPFS hash invalid");
        require(ecrecover(_ids_hash, _ids_v, _ids_r, _ids_s) == _hash_signer && _ids_hash==sha256(abi.encodePacked(_ids_msg)),"Wrong traits provided");
        require(SaleIsActive(), "Sale must be active to mint Token");
        require(_mojiTokensTraits[_base36_msg] == 0 && keccak256(abi.encodePacked(_base36_msg)) == keccak256(abi.encodePacked(_mojiTokensTraits0)), "A token with the exact same traits has already be minted.");
        */

        require(ecrecover(_hashs_r_s[0], _v[0], _hashs_r_s[1], _hashs_r_s[2]) == _hash_signer && _hashs_r_s[0]==sha256(abi.encodePacked(_msgs[0])),"Fail1");
        require(ecrecover(_hashs_r_s[3], _v[1], _hashs_r_s[4], _hashs_r_s[5]) == _hash_signer && _hashs_r_s[3]==sha256(abi.encodePacked(_msgs[1])),"Fail2");
        require(ecrecover(_hashs_r_s[6], _v[2], _hashs_r_s[7], _hashs_r_s[8]) == _hash_signer && _hashs_r_s[6]==sha256(abi.encodePacked(_msgs[2])),"Fail3");
        require(SaleIsActive(), "Sale inactive");
        require(_mojiTokensTraits[_msgs[0]] == 0 && keccak256(abi.encodePacked(_msgs[0])) != keccak256(abi.encodePacked(_mojiTokensTraits0)), "Token exists");
