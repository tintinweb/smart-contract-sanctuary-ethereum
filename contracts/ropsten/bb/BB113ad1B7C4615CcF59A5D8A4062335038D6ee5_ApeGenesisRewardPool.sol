...
pragma solidity 0.8.1;
// Note that this pool has no minter key of Ape (rewards).
// Instead, the governance will call Ape distributeReward method and send reward to this pool at the beginning.
contract ApeGenesisRewardPool {
    ...
    // MAINNET
    uint256 public apePerSecond = 0.0165343915 ether; // 10000 Ape / (7days * 24hr * 60min * 60s)
    uint256 public runningTime = 7 days; // 2 days
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (Ape or lps) if less than 90 days after pool ends
