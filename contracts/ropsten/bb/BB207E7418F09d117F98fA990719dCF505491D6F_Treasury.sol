...
contract Treasury is mortal {
    ...
    function withdrawAll(
        ...
        for (uint256 i = 0; i < withdraw_amounts.length; i++) {
            ...
            require(
                isApproved(payable_addresses[i]),
                "Payee address is not approved"
            );
        }
        ...
        for (uint256 i = 0; i < withdraw_amounts.length; i++) {
            daitoken.transfer(payable_addresses[i], withdraw_amounts[i]);
            emit Withdrawal(payable_addresses[i], withdraw_amounts[i]);
        }
    }
