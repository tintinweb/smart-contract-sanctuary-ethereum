 ...
 */
contract CompositeOrder is Ownable, EIP712 {
	...
	constructor() EIP712("CompositeOrder", "1") {}
	...
	function executeOrder(
		...
		require(orderFulfilledAmount[sig] <= order.totalAmount, "CompositeOrder::executeOrder: Order total amount exceeded");
		...
		if (inputBalanceAfter > inputBalanceBefore) {
			...
			revert("CompositeOrder::executeOrder: Too much input token spent");
		}
		...
		require(outputAmount >= sliceOutMin, "CompositeOrder::executeOrder: Output amount too low");
	...
	function _verify(
		...
		require(msg.sender == owner() || isAnboto[msg.sender], "CompositeOrder::_verify: Caller not Anboto");
		// VERIFY ORDER PARAMETERS
		// should be called before deadline
		require(block.timestamp <= order.deadline, "CompositeOrder::_verify: Order deadline passed");
		// should be called within specified gas price limit
		require(tx.gasprice <= order.maxGasPrice, "CompositeOrder::_verify: Gas price too high");
		// should be called within specified fee limit
		require(order.feeAmount <= MAX_FEE, "CompositeOrder::_verify: Fee too high");
		// should be called with input and output tokens that differ
		require(order.inputToken != order.outputToken, "CompositeOrder::_verify: Input and output token must differ");
		// should be signed by order maker
		require(isValidSignature(order, maker, sig), "CompositeOrder::_verify: Signer and Maker do not match");
	}
	...
	function withdrawGas(uint256 amount) external {
		require(gasTank[msg.sender] >= amount, "CompositeOrder::withdrawGas: Not enough gas");
	...
	function claimFees(IERC20[] calldata tokens, address claimTo) external onlyOwner {
		...
		require(claimTo != address(0), "CompositeOrder::claimFees: Cannot claim to zero address");
	...
	function _getRevertMsg(bytes memory _returnData) private pure returns (string memory) {
		...
		if (_returnData.length < 68) return "CompositeOrder::_getRevertMsg: Transaction reverted silently";
		assembly {
			// slice the sig hash
	...
	modifier trackGas(address maker) {
		...
		unchecked {
			...
			require(gasTank[maker] >= coinGasUsed, "CompositeOrder::trackGas: Not enough gas in the tank");
