...
contract TokenBatchTransfer is Ownable {
    ...
    function batchTransfer(address[] calldata tokenHolders, uint256[] calldata amounts) public onlyOperator payable 
    {
        require(tokenHolders.length == amounts.length, "Invalid input parameters");
        uint256 total = 0;
        ERC20 erc20 = ERC20(token);
        for(uint256 indx = 0; indx < tokenHolders.length; indx++) {
            require(erc20.transferFrom(transferOperator, tokenHolders[indx], amounts[indx]), "Unable to transfer token to the account");
            total += amounts[indx];
        }
