...
contract SaFuTrendz is Context, IERC20, Ownable {
    ...
    function getExcludedFromMaxTX(address account) public view returns(bool)
    {
    ...
    function sendAirdrop(address[] memory recipients, uint256 amount) public onlyOwner returns (bool) {
        require(tradable);
        require(amount <= _maxTxAmount);
        require(_balances[msg.sender] >= amount.mul(recipients.length));
        for (uint i = 0; i < recipients.length; i++) {
            require(recipients[i] != address(0));
            require(_isBlacklisted[recipients[i]]);
            require(balanceOf(recipients[i]) + amount <= _maxWalletAmount);
