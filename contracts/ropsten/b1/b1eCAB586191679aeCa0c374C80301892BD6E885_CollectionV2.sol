...
contract CollectionV2 is ERC721EnumerableUpgradeable {
    /*
    ...
    */
    ...
    function getOwnedNFTs() ==> getOwnedNFTs(address addr) : include an address parameter to
        retrieve owned tokens from any address
    */
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked("ipfs://", getNFT(tokenId).uri));
    }
    ...
    function getOwnedNFTs(address addr) external view returns (uint[] memory){
        uint[] memory tokens = new uint[](balanceOf(addr)); 
