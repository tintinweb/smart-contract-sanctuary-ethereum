...
contract Pr_TestV2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Pr Test 1"; 
    string private constant _symbol = "Pr_Test";
    ...
    uint256 public _Pr_TestBurned;

    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isExcludedFromFee;
    
    address payable private _presa;
    address payable private _TrogeTreasury;
    
    address public Troge = 0xaD6D458402F60fD3Bd25163575031ACDce07538D; 
    ...
    uint256 private _Pr_TestBurn = 1;
    ...
    constructor(address payable addr1, address payable addr2, address addr3) {
        _presa = addr1;
        _TrogeTreasury = addr2;
        _rOwned[_msgSender()] = _rTotal;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_presa] = true;
        _isExcludedFromFee[_TrogeTreasury] = true;
    ...
    function manualTroge(uint amount) external onlyOwner() {
        swapETHforTroge(amount);
    }
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if (contractETHBalance > 0) {
                    swapETHforTroge(address(this).balance);
                }
            }
    ...
    function removeAllFee() private {
        if (_reflection == 0 && _contractFee == 0 && _Pr_TestBurn == 0) return;
        _reflection = 0;
        _contractFee = 0;
        _Pr_TestBurn = 0;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 tAmount, uint256 tBurn) = _Pr_TestEthBurn(amount);
    ...
    function _Pr_TestEthBurn(uint amount) private returns (uint, uint) {  
        uint orgAmount = amount;
        uint256 currentRate = _getRate();
        uint256 tBurn = amount.mul(_Pr_TestBurn).div(100);
        uint256 rBurn = tBurn.mul(currentRate);
        _tTotal = _tTotal.sub(tBurn);
        _rTotal = _rTotal.sub(rBurn);
        _Pr_TestBurned = _Pr_TestBurned.add(tBurn);
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
     ...
     function swapETHforTroge(uint ethAmount) private {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(Troge);
    ...
    function distributeETH(uint256 amount) private {
        ...
        _TrogeTreasury.transfer(amount.div(3));
    }
