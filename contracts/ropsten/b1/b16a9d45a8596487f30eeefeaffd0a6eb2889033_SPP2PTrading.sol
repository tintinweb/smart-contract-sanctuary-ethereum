...
// File: contracts/p2p/SPP2PTrading.sol


pragma solidity ^0.8.6;


contract SPP2PTrading {
  ...
  /** 
    ...
    @param _tokenCount Num of tokens for selling
    @param _tokenPrice Token price for selling
    @param _seller Token seller address
    @param _openToken Token address for selling
  */
  function sellToken(uint256 _tokenCount, uint256 _tokenPrice, address _seller, address _openToken) public {
    // Transfer value from the opening trader to this contract.
    IERC20 openERC20Contract = IERC20(_openToken);
    require(_tokenCount <= openERC20Contract.allowance(_seller, address(this)));
    require(openERC20Contract.transferFrom(_seller, address(this), _tokenCount));

    // Store the details of the swap.
    Swap memory swap = Swap({
      openValue: _tokenCount,
      openCost: _tokenPrice,
      openTrader: _seller,
      openContractAddress: _openToken
    });
    swaps[nSwaps] = swap;
  ...
  function getList (address _openContractAddress) public view returns (uint256[] memory openValues, uint256[] memory openCosts, uint256[] memory closeValues, uint256[] memory closeCosts) {
    ...
    uint256 bufSize = nSwaps > nFailedBuyOrders ? nSwaps : nFailedBuyOrders;
    uint256[] memory keyBuf = new uint256[] (bufSize);
    ...
    openValues = new uint256[](count);
     openCosts = new uint256[](count);

    for (uint256 i = 0; i < count; i++) {
        openValues[i] = swaps[keyBuf[i]].openValue;
        openCosts[i] = swaps[keyBuf[i]].openCost;
    }

    delete keyBuf;

    keyBuf = new uint256[] (nFailedBuyOrders);

    for (uint256 i = 0; i < nFailedBuyOrders; i++) {
      if (failedBuyOrders[i].openContractAddress == _openContractAddress) {
        keyBuf[failedCount] = i;
        failedCount++;
      }
    }

    closeValues = new uint256[](failedCount);
     closeCosts = new uint256[](failedCount);

    for (uint256 i = 0; i < failedCount; i++) {
        closeValues[i] = failedBuyOrders[keyBuf[i]].closeValue;
        closeCosts[i] = failedBuyOrders[keyBuf[i]].closeCost;
    }
  ...
  /** 
    ...
    @param _openToken Num of tokens for selling
    @param _buyer Token buyer
    @param _closeToken Token address to exchange for openToken
    @param _closeTokenCount Count of close tokens
    @param _closeTokenPrice Price of a close token
  */
  function buyToken(address _openToken, address _buyer, address _closeToken, uint256 _closeTokenCount, uint256 _closeTokenPrice) public returns (bool success) {
      
      for (uint256 i = 0; i < nSwaps; i++) {
        if (swaps[i].openContractAddress == _openToken && 
            swaps[i].openValue >= _closeTokenCount && 
            swaps[i].openCost <= _closeTokenPrice) {
          IERC20 closeERC20Contract = IERC20(_closeToken);
          require(_closeTokenCount * _closeTokenPrice <= closeERC20Contract.allowance(_buyer, address(this)));
          require(closeERC20Contract.transferFrom(_buyer, swaps[i].openTrader, _closeTokenPrice * _closeTokenCount));

          IERC20 openERC20Contract = IERC20(swaps[i].openContractAddress);
          require(openERC20Contract.transfer(_buyer, swaps[i].openValue));
      ...
      FailedBuyOrder memory failedOrder = FailedBuyOrder({
        openContractAddress: _openToken,
        closeValue: _closeTokenCount,
        closeCost: _closeTokenPrice,
        closeTrader: _buyer,
        closeContractAddress: _closeToken
      });
