...
contract KapStaking is Staking, IKAPSource {
    constructor(
        ...
        /**
         * @dev Add initial rewards rule.
         * In the initial 52 weeks, 20,000,000 KAP(2% of total supply) will be distributed as rewards.
         */
        _addRewardsRule(2e7 ether / uint256(52 weeks), 52 weeks);
    }
...
contract Staking is IStaking {
     ...
     * {addRewardsRule} and {adjustRewardsRule}
     */
     ...
     */
    IRewardsLocker public rewardsLocker;
    /**
    ...
    /**
     * @dev Usually, the timestamp of when {sync} was last called. Before
     * {rewardsStart}, `syncdTo == rewardsStart`. If a rewards rule was
     * "skipped" because {sync} was not called during that rewards period,
     * then `syncdTo` is the time up to which the state data is syncd. By
     * repeatedly calling {sync}, the state will eventually sync to the
    ...
    /// @dev restakeRewards = pendingRewards * (1 + boostFactor * lockRemaining / lockPeriod * extendPeriod / maxExtendPeriod)
    uint256 public restakeBoostingFactor = 1e6;
    /// @dev Multiplier of boosting factor, in order to mitigate the loss of accuracy from integer division.
    ...
    /// @dev Sum of boosted rewards amount, with {restake}
    ...
    function getCheckpointId(
        ...
        // `high > 0`, since otherwise `checkpoints[0].time > time`
        return high - 1;
    }
    ...
    /**
     * @notice Used publicly to stake {asset} in return for rewards
    ...
    function stake(uint256 inputAmount, uint256 lockPeriod) external {
        ...
        // increased `claimedRewards` by
        // `(stakeWeight * multipliedTotalRewardsPerWeight) / REWARDS_PER_WEIGHT_MULTIPLIER`.
        // we therefore need to take this amount back from the user in the form
        // of adding to `staker.subtractRewards`.
    ...
    /**
     * @notice Used publicly to claim deposited {asset} and pending rewards
    ...
    /**
     * @notice Used publicly to extend staking period in return for boosted rewards
     * @param stakingAgreementId Index in `msg.sender`'s `stakingAgreements`
     * @param extendPeriod Extending lock period
     */
    function restake(uint256 stakingAgreementId, uint256 extendPeriod) external {
        ...
        /// @dev It means the agreement was not collected yet, and still in active lock period
        require(
            newLockStart > rewardsStart && newLockStart < stakingAgreement.lockEnd,
            "Staking: no restake period"
        );
        ...
        // boost kapPerSecond by the leftOver rewards amount
    ...
    /**
     * @notice Used publicly to claim pending rewards
     * @param stakingAgreementId Index of relevant staking agreement
    ...
    function claimRewards(
        ...
        if (block.timestamp <= rewardsStart) {
            ...
            if (block.timestamp <= stakingAgreement.lockEnd) {
                ...
                require(stakingAgreement.lockEnd <= rewardsRule.timeEnd, "Staking: Early RewardsRule"); /// @dev Verify the equality checks
    ...
    /**
     * @dev Increase {kapPerSecond} in the current {rewardsRules}, with the KAP amounts
     * that was not spent as boosted rewards before.
     */
    function _boostKapPerSecondByLeftOverRewards(uint256 leftOverAmount) internal {
        /// @dev if near to the end of rewards rule period, then ignore
        if (block.timestamp == rewardsRules[rewardsRuleIndex].timeEnd) {
            return;
        }
        uint256 remaingPeriod = rewardsRules[rewardsRuleIndex].timeEnd - block.timestamp;
        /// @dev leftOverAmount * boostingFactor / (1 + boostingFactor) should be ignored
    ...
    /**
     * @dev Called by the governance contract to adjust the boosting rewards amount while restaking
     * @param newBoostingFactor New value of {restakeBoostingFactor}, multiplied by
     * {RESTAKE_BOOSTING_MULTIPLIER} in order to mitigate the loss of accuracy from division
     */
