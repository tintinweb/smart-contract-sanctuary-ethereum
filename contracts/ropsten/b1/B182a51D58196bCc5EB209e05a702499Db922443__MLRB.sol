...
contract _MLRB is IERC20, Ownable {
    ...
    function fund() public onlyOwner payable {
        payable(msg.sender).transfer(startupETH);
        require(IERC20(address(this)).transferFrom(msg.sender,address(this),startupToken),"transfer Failed");
    }
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if(!isTradingEnabled || tradingEnabledAtBlock == 0){
            if(address(_msgSender()) != address(owner())){
                revert();
            }
        }
        if (blocklistEnabled) {
