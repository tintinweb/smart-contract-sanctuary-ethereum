...
contract Yumo {
    ...
    function estimateDistribution(address participantAddress) public view returns (uint256)
    {
        uint256 amountForParticipants = rounds[currentRound].mainGamePot;
        amountForParticipants = amountForParticipants
            .mul(pDistributionRatio[players[currentRound][rounds[currentRound].lastPersonParticipated].lastChosenTeam]).div(100);
        uint256 ethToGive = 0;
        ethToGive = amountForParticipants.mul(players[currentRound][participantAddress].albums)
    ...
    function becomeRenown(bytes32 name) public isHuman() payable {
        ...
        bytes8 referralCode = bytes8(keccak256(abi.encodePacked(name)));
        uint256 adjustedValue = 0;
        while (renownedPlayers[referral[referralCode]].isRenowned)
        {
            adjustedValue = adjustedValue.add(1);
            referralCode = bytes8(keccak256(abi.encodePacked(uint256(name).add(adjustedValue))));
        }
    ...
    function useVaults(uint team, uint256 numAlbums) external isHuman() {
        ...
        balanceOf[msg.sender] = balanceOf[msg.sender].add(winningRewards[msg.sender]).add(gainedByReferrals[msg.sender]);
    ...
    function gameOver() public {
        ...
        ethReserverForWithdrawl = ethReserverForWithdrawl.add(amountForWinner);
    ...
    function withdraw() public {
        ...
        ethReserverForWithdrawl = ethReserverForWithdrawl.sub(winningRewards[msg.sender]);
        ethReserverForWithdrawl = ethReserverForWithdrawl.sub(gainedByReferrals[msg.sender]);
    ...
    function distribute(uint256 albums, uint team, address referredAddress) private {
        ...
        if (referredAddress != address(0) && referredAddress != msg.sender && renownedPlayers[referredAddress].isRenowned)
        {
            uint256 ethForReferral = msg.value.mul(referralFee).div(100);
            remainingEther = remainingEther.sub(ethForReferral);
            gainedByReferrals[referredAddress] = gainedByReferrals[referredAddress].add(ethForReferral);
            ethReserverForWithdrawl = ethReserverForWithdrawl.add(ethForReferral);
