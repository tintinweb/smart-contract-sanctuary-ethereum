...
contract MultiSigWallet {
    ...
    // bool changed to uint. This value is the timestamp when the confirmation becomes valid.
...
// as of now this could be refactored out - there is verification in code
  ...
  //    require(!confirmations[transactionId][owner] && (owner == delayedOwner && (delayedConfirmations[transactionId] == 0 || delayedConfirmations[transactionId] > now)));
  //    _;
  //  }
   ...
   //     notConfirmed(transactionId, msg.sender) // removed because we do not need it, we'd like to update confirmation status for delayed owner
    {
        ...
        {
            delayedConfirmations[transactionId] = now + 5 minutes; // + 2 weeks in prod
    ...
    function executeTransaction(uint transactionId)
        public
        ownerExists(msg.sender)
        confirmed(transactionId, msg.sender) // do we need it? (there is a verification below)
