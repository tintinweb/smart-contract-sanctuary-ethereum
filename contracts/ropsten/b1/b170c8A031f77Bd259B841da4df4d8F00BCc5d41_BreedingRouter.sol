...
contract BreedingRouter is Ownable, Pausable, ERC721Holder, ReentrancyGuard {
    ...
    event UserGatchaBunny(address _user, uint256 _bunnyId1);
    ...
    mapping(uint256 => uint16) public idToBreedCounts;
    ...
    function rent(uint256 tokenId, uint256 durationInDay) external {
        require(!paused(), "contract paused");
        LeaseMarket storage leaseMarket = tokenIdLeaseMarket[tokenId];
    ...
    function rentExtension(uint256 tokenId, uint256 durationInDay) external {
        require(!paused(), "contract paused");
        LeaseMarket storage leaseMarket = tokenIdLeaseMarket[tokenId];
        require(
            ERC20(address(rewardToken)).balanceOf(msg.sender) >= leaseMarket.price.mul(durationInDay),
        ...
        leaseMarket.duration = leaseMarket.duration + durationInDay.mul(timestampPerDay);
        emit UserRentExtensionBunny(leaseMarket.owner, msg.sender, tokenId, leaseMarket.price, leaseMarket.duration);
    }
    ...
    function _getSuccessRate(uint256 bunnyId1, uint256 bunnyId2) internal view returns (uint256) {
        /// calculate in zoom 10000, smart contract don't have decimal;
    ...
    function getSuccessRate(uint256 bunnyId1, uint256 bunnyId2) external view returns (uint256) {
        /// calculate in zoom 10000, smart contract don't have decimal;
        return _getSuccessRate(bunnyId1, bunnyId2);
    }
    ...
    function _breedingWithRentedBunny(
        ...
        uint256 successRate = _getSuccessRate(bunnyId1, rentedBunnyId);
    ...
    function _breedingWithBunnies(
        ...
        uint256 successRate = _getSuccessRate(bunnyId1, bunnyId2);
        ...
        breed.cooldownEndBlock = block.number + breedingTime.div(secondsPerBlock);
        tokenIdBreedings[bunnyId1] = breed;
        emit UserBreedBunny(msg.sender, msg.sender, bunnyId1, bunnyId2, true, breedingTime);
    ...
    function gatchaBunny(uint256 tokenId) external {
        ...
        emit UserGatchaBunny(msg.sender, tokenId);
    }
    ...
    function deposit(uint256 _nftId, uint256 _itemId) public {
        ...
        emit Deposit(msg.sender, _nftId, _itemId);
    }
    ...
    function _depositFromBreeding(address userAddress, uint256 _nftId) internal {
        ...
        user.lastRewardBlock[_nftId] = block.number;
        user.nftIds.add(_nftId);
        emit Deposit(userAddress, _nftId, 0);
    }
    ...
    function viewReward(address _user, uint256 _nftId) public view returns (uint256) {
        ...
        if (user.item[_nftId].itemId != 0 && user.lastRewardBlock[_nftId] <= user.item[_nftId].endBlock) {
            if (maxBlock <= user.item[_nftId].endBlock) {
                return
                    rewardPerDay
                        .mul(maxBlock - user.lastRewardBlock[_nftId])
                        .mul(itemRate[user.item[_nftId].itemId])
                        .mul(nftRate)
                        .div(1e8)
                        .div(blockPerDay);
            } else {
