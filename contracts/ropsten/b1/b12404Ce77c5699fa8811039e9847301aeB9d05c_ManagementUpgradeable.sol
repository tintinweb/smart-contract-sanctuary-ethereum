...
contract ManagementUpgradeable is IManagement, EIP712Upgradeable, OwnableUpgradeable {
    ...
    /// @param address_ the addresses that need to whitelist
    /// @param value_ the true or false for kyc whitelist
    function setKycWhitelists(address[] calldata address_, bool value_) external virtual override {
        require(_permissions[_msgSender()][MANAGEMENT_CAN_SET_KYC_WHITELISTED], ERROR_ACCESS_DENIED);
        for (uint256 i = 0; i < address_.length; i++) {
            _permissions[address_[i]][MANAGEMENT_WHITELISTED_KYC] = value_;
        }
        emit UsersPermissionsSet(address_, MANAGEMENT_WHITELISTED_KYC, value_);
    }
