...
contract BloodVesselsFangersV1 is MintableUpgradeable, PausableUpgradeable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, ERC721RoyaltyUpgradeable {
    ...
    uint private _maxPurchaseLimit;
    EnumerableMapUpgradeable.AddressToUintMap private _numPurchased;
    ...
    function initialize (
        ...
        uint maxPurchaseLimit) initializer public {

        __Mintable_init(owner, imx);
        __Pausable_init();
        ...
        setMaxPurchaseLimit(maxPurchaseLimit);
    }
    ...
    function _mintFor(
        ...
        (bool contains, uint256 purchaseCount) = EnumerableMapUpgradeable.tryGet(_numPurchased, user);
        require(!contains || purchaseCount < _maxPurchaseLimit,  "Max Purchase Limit Reached");
        
        string memory tokenUri = string(blueprint);

        _safeMint(user, id);
        _setTokenURI(id, tokenUri);
        EnumerableMapUpgradeable.set(_numPurchased, user, purchaseCount + 1);
    }
    ...
    function getNumPurchased(address user) public view returns(uint) {
        (bool contains, uint256 purchaseCount) = EnumerableMapUpgradeable.tryGet(_numPurchased, user);
        if(!contains) {
            return 0;
        } else {
    ...
    function resetPurchaseLimit(address user) public onlyOwner {
        EnumerableMapUpgradeable.set(_numPurchased, user, 0);
    }

    function resetAllPurchaseLimits() public onlyOwner {
        for(uint i = 0; i < EnumerableMapUpgradeable.length(_numPurchased); i++) {
            (address user, ) = EnumerableMapUpgradeable.at(_numPurchased, i);
            resetPurchaseLimit(user);
        }
    }

    function getMaxPurchaseLimit() public view returns (uint) {
        return _maxPurchaseLimit;
    }

    function setMaxPurchaseLimit(uint maxPurchaseLimit) public onlyOwner {
        _maxPurchaseLimit = maxPurchaseLimit;
    }
...
abstract contract MintableUpgradeable is OwnableUpgradeable, IMintable {
    ...
    function __Mintable_init(address _owner, address _imx) internal onlyInitializing {
        __Ownable_init();
        imx = _imx;
