...
/**
 * @notice The contract helps to sell NFTs.
 * @dev A seller signs trade data to show he approved the deal.
 * A user passes signed trade data and seller to swap function. After some validation steps a buyer exchange ERC20 tokens on NFT.
 */
contract NftMarketplace is RoyaltiesInfo {
    ...
    /**
     * @notice Percentage from transaction amount sent to `feeReceiver`.
     */
    uint256 public feePercentage = 2_50; // 2.5%

    /**
     * @notice Address to sent fees.
     */
    ...
    /**
     * @notice Amount of gas supplied to external function calls.
     */
    ...
    /**
     * @notice Set function for `feePercentage`.
     * @dev Can be called only by the contract admin.
     * @param newValue New fee
     */
    ...
    /**
     * @notice Set function for `feeReceiver`.
     * @dev Can be called only by the contract admin.
     * @param newValue New address to receive fees
     */
    ...
    /**
     * @notice Function pauses/unpauses trades.
     * @dev Can be called only by the contract admin.
     */
    ...
    /**
     * @notice Main NFT trade function.
     * @param signatureInfoSeller Trade data
     * @param sellerSignature Signed by `sellerAddress` trade data
    ...
    /**
     * @notice The function checks the `signatureInfoSeller` was signed by `sellerAddress`.
     * @param signatureInfoSeller Trade data
     * @param sellerSignature Signed by `sellerAddress` trade data
     * @param sellerAddress NFT seller address
     */
    ...
    /**
     * @notice The functions verifies the token type.
     * @param tokenInfo Trade data
     */
    ...
    /**
     * @notice The function checks the `signatureInfoSeller` was signed by `sellerAddress`.
     * @dev Should be called only after important checks in `_verifySignature()` and `_verifyToken()`.
     * @param tokenInfo Token to transfer
     * @param from Token sender address
     * @param to Token recipient address
     * @param royaltyReceiver NFT seller address
     * @param royaltyAmount Calculated royalties amount
     */
...
/**
 * @notice The contract manages traded tokens royalties.
 * @dev For each token can be manually set `royaltyReceiver` and `royaltyPercentage`.
 * Royalties also can be disabled for the token admin. Then, royalties are directed to the token owner.
 */
contract RoyaltiesInfo is AccessControlEnumerable {
    ...
    /**
     * @notice Set admin royalties for the token.
     * @dev Can only be called by the contract admin.
     * @param token Token to set admin royalties
     * @param royaltyReceiver Address to receive royalties
     * @param royaltyPercentage Token to set admin royalties
     */
    function setRoyalty(
        address token,
        address royaltyReceiver,
    ...
    /**
     * @notice Sets default fee.
     * @dev Can only be called by the contract admin.
     * @param newValue New `defaultFeeForOwner`
     */
    ...
    /**
     * @notice Get method for token's royalties info.
     * @dev If the token admin royalty is disabled returns the token owner and `defaultFeeForOwner * salePrice) / 100_00`.
     * @param token NFT address to get info about
