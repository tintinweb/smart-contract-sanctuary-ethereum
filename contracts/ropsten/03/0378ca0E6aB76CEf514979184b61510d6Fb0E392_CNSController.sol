...
contract CNSController is CNSToken {
    ENS internal ens;
    ...
    mapping(string => address[]) public domainMembers;
    ...
    function domainRegister(
        ...
        require(domains[_domain].tokenId == 0, "Domain already registered");
    ...
    function _safeRegisterPolicy(string memory _domain, address _policy)
        private
    {
        domainPolicy[_domain] = policy(_domain, _policy);
        IPolicy(_policy).registerPolicy(_domain, domains[_domain].owner);
    }
    ...
    function registerSubdomain(
        ...
        require(
            resolver.addr(_subnode) == address(0),
            "Subdomain already exists from ENS"
        );

        require(
            IPolicy(_policy).permissionCheck(_domain, msg.sender),
            "Permission denied"
        );

        require(
            subdomains[_subdomain].owner == address(0),
            "Subdomain already exists"
        );

        require(
            _isDomainMember(_domain, msg.sender),
            "You already have subdomain on this domain"
        );
    ...
    function _setSubdomain(
        ...
        domainMembers[_domain].push(msg.sender);
    ...
    function _minter() private {
        ...
        _setTokenURI(
            ...
            string(
                bytes.concat(
                    bytes("https://cns-domains.vercel.app/api/v1/metadata/"),
    ...
    function _isDomainMember(string memory _domain, address _member)
        internal
        view
        returns (bool)
    {
        for (uint256 i = 0; i < domainMembers[_domain].length; i++) {
            if (domainMembers[_domain][i] == _member) {
                return false;
            }
        }
        return true;
    ...
    function checkDomainPolicy(string memory _domain)
        public
        view
        returns (address)
    {
    ...
    {
        address _policy = domainPolicy[_domain].policy;
        return IPolicy(_policy).permissionCheck(_domain, _account);
    }
    ...
    function addPolicy(address _policy) external onlyOwner {
        policies[_policy] = true;
    }

    function deletePolicy(address _policy) external onlyOwner {
        delete policies[_policy];
    }

    function changePolicy(string memory _domain, address _newPolicy)
        external
        onlyDomainOwner(_domain)
    {
        unRegisterPolicy(_domain);
        _safeRegisterPolicy(_domain, _newPolicy);
    }
    ...
    function unRegisterPolicy(string memory _domain)
        ...
        onlyDomainOwner(_domain)
    {
        IPolicy(domainPolicy[_domain].policy).unRegisterPolicy(_domain);
        delete domainPolicy[_domain];
    }
...
interface IPolicy {
    ...
    function registerPolicy(string memory _domain, address _owner) external;

    function unRegisterPolicy(string memory _domain) external;
}
