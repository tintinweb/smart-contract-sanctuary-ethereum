...
abstract contract Default is ERC721, ERC721Burnable, ERC721Metadata, ERC721Pausable, ERC721Royalty, Withdrawable {
    ...
    // Mint Key => Configuration
    mapping(uint256 => Config) public _config;

    // Mint Key => [Address => Minted Counter]
    ...
    function deleteConfig(uint256 key) external onlyOwner {
        delete _minted[key][owner()];
    }

    function mint(uint256 key, uint256 quantity) payable public whenNotPaused {
        mint(key, new bytes32[](0), quantity);
    }

    function mint(uint256 key, bytes32[] memory proof, uint256 quantity) payable public whenNotPaused {
        if (_minted[key][owner()] == 0) {
            revert InvalidMintKey();
        }

        address buyer = _msgSenderERC721A();
        Config memory config = _config[key];
        uint256 minted = _minted[key][buyer];
        ...
        // Max supply within mint config should never exceed max supply of collection
        if (available > config.maxSupply) {
            available = uint256(config.maxSupply);
        }

        MintGate.open(uint256(config.endsAt), uint256(config.startsAt));
        MintGate.price(buyer, uint256(config.price), quantity, msg.value);
        MintGate.supply(available, uint256(config.maxMint), minted, quantity);

        unchecked {
            _config[key].maxSupply -= uint32(quantity);
            _minted[key][buyer] += quantity;
        }
