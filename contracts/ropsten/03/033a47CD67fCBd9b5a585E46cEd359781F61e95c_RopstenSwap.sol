 ...
 */
contract RopstenSwap is ZetaInteractor, ZetaReceiver, RopstenSwapErrors, Pausable {
        ...
        whenNotPaused()
    {
        ...
        (
            ...
            bytes memory revertLegSwapCallData
        ) = abi.decode(zetaMessage.message, (bytes32, address, address, bytes, bytes));

        // Validate message
        if (messageType != SWAP_MESSAGE) revert InvalidMessageType();
        if (secondLegSwapCallData.length == 0) revert InvalidCallData();
        if (revertLegSwapCallData.length == 0) revert InvalidCallData();
    ...
    function onZetaRevert(ZetaInterfaces.ZetaRevert calldata zetaRevert)
        ...
        whenNotPaused()
    {
        // Decode incoming message. Here we buy back the original sold token
        // a few possible naming schemes here. Have gone with sender==leg1Sender, soldAmount==leg1SellAmount, 
        // soldTokenAddress==leg1sellTokenAddress
        (
            bytes32 messageType,
            uint256 zetaAmount,
            address senderAddress,
            address payable soldTokenAddress,
            bytes memory revertLegSwapCallData
        ) = abi.decode(zetaRevert.message, (bytes32, uint256,  address, address, bytes));

        // Validate message
        if (messageType != SWAP_MESSAGE) revert InvalidMessageType();
        if (revertLegSwapCallData.length == 0) revert InvalidCallData();

        // if something has gone wrong then swap zeta back to sell token

        // Give `0xExchangeProxy` an allowance to spend reverted ZETA. May not be necessary
        IERC20 zetaToken = IERC20(_zetaToken);
        if (!zetaToken.approve(_zeroxExchangeProxy, zetaAmount))
            revert ErrorApprovingTokens(_zetaToken);

        // Call revert leg 0x Swap - should sell ZETA
        // Also need to think about min swap qty so it doesnt get a really bad deal. 
        // But this would mean the amount is stuck in the contract, so need a way to clear 'stuck' amounts
        (bool success, ) = _zeroxExchangeProxy.call{value: 0}(revertLegSwapCallData);
        if (!success) revert SellZetaFailed();

        // Transfer re-purchased ETH/tokens to original sender

        // quite a few bugs possible here. 
        // for example if something has gone wrong with a previous revert and the coins are left
        // in this contract. We shouldnt naively give all those coins to the original
        // customer.
        // Be naive for now because it means I dont have to fully think about
        // the zeta that was used in leg 1 and subtract it from the amount here

        uint256 repurchasedAmount = 0;
        if (soldTokenAddress == _ethAddress) {
            repurchasedAmount = address(this).balance;
            bool ethTransfer = payable(senderAddress).send(repurchasedAmount);
            if (!ethTransfer) revert ErrorTransferringEther();
        } else {
            IERC20 repurchasedToken = IERC20(soldTokenAddress);
            repurchasedAmount = repurchasedToken.balanceOf(address(this));
            bool tokenTransfer = repurchasedToken.transfer(senderAddress, repurchasedAmount);
            if (!tokenTransfer) revert ErrorTransferringTokens(soldTokenAddress);
        }

        // Emit revert event
        emit FirstLegReverted(senderAddress, soldTokenAddress, zetaRevert.zetaAmount);
    }
}
