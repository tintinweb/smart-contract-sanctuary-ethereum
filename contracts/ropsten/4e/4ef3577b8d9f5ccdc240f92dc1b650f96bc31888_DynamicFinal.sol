...
contract DynamicFinal is Context, IERC20, Ownable {
    ...
    uint256 private minContractTokensToSwap = 2500000 * 10**9;
    uint256 private _taxFee = 0;
    uint256 private _teamFee = 10;
    uint256 private _liquidityFeePercentage = 0;
    uint256 private _maxWalletPercentage = 3;
    uint256 private _launchBlock = 0;
    uint256 private _maxBuyAmount = 50000000 * 10**9; // 5%
    uint256 private _maxSellAmount = 10000000 * 10**9; // 1%

    uint256 private _baseFee = 10;

    uint256 private _moderateApeImpact = 1;
    uint256 private _highApeImpact = 3;
    uint256 private _extremeApeImpact = 5;

    uint256 private _moderateApeFee = 25; // 2.5% fee discount
    uint256 private _highApeFee = 50; // 5% fee discount
    uint256 private _extremeApeFee = 75; // 2.5% fee discount

    uint256 private _moderateDumpImpact = 1;
    uint256 private _highDumpImpact = 3;
    uint256 private _extremeDumpImpact = 5;

    uint256 private _moderateDumpFee = 50; // 5% fee increase
    uint256 private _highDumpFee = 100; // 10% fee increase
    uint256 private _extremeDumpFee = 150; // 25% fee increase
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if (amount >= balanceOf(uniswapV2Pair).mul(_extremeApeImpact).div(100)) {
                    _teamFee = _baseFee - _baseFee.mul(_extremeApeFee).div(100);
                } else if (amount >= balanceOf(uniswapV2Pair).mul(_highApeImpact).div(100)) {
                    _teamFee = _baseFee - _baseFee.mul(_highApeFee).div(100);
                } else if (amount >= balanceOf(uniswapV2Pair).mul(_moderateApeImpact).div(100)) {
                    _teamFee = _baseFee - _baseFee.mul(_moderateApeFee).div(100);
                } else {
                    _teamFee = _baseFee;
                }
            ...
            if(!_inSwap && from != uniswapV2Pair && _tradingOpen) {
                ...
                if (amount >= balanceOf(uniswapV2Pair).mul(_extremeDumpImpact).div(100)) {
                    _teamFee = _baseFee + _baseFee.mul(_extremeDumpFee).div(100);
                } else if (amount >= balanceOf(uniswapV2Pair).mul(_highDumpImpact).div(100)) {
                    _teamFee = _baseFee + _baseFee.mul(_highDumpFee).div(100);
                } else if (amount >= balanceOf(uniswapV2Pair).mul(_moderateDumpImpact).div(100)) {
                    _teamFee = _baseFee + _baseFee.mul(_moderateDumpFee).div(100);
                } else {
                    _teamFee = _baseFee;
                }
    ...
    function updateDdynamicApe(uint256 moderateApeImpact, uint256 highApeImpact, uint256 extremeApeImpact) public onlyOwner {
        _moderateApeImpact = moderateApeImpact;
        _highApeImpact = highApeImpact;
        _extremeApeImpact = extremeApeImpact;
    }

    function updateApeFees(uint256 moderateApeFee, uint256 highApeFee, uint256 extremeApeFee) public onlyOwner {
        _moderateApeFee = moderateApeFee;
        _highApeFee = highApeFee;
        _extremeApeFee = extremeApeFee;
    }

    function updateDdynamicDump(uint256 moderateDumpImpact, uint256 highDumpImpact, uint256 extremeDumpImpact) public onlyOwner {
        _moderateDumpImpact = moderateDumpImpact;
        _highDumpImpact = highDumpImpact;
        _extremeDumpImpact = extremeDumpImpact;
    }

    function updateDumpFees(uint256 moderateDumpFee, uint256 highDumpFee, uint256 extremeDumpFee) public onlyOwner {
        _moderateDumpFee = moderateDumpFee;
        _highDumpFee = highDumpFee;
        _extremeDumpFee = extremeDumpFee;
    }
