...
contract PandolfiniEscrow is RoleControl {
  error CannotReceive();
  ...
  */
  ...
  function transferItem(address _nftAddress, uint256 _tokenId, address _toAddress) public onlyOperator  {
    require(IERC721(_nftAddress).getApproved(_tokenId) == address(this), "NFT must be approved to Pandolfini Escrow");
  ...
  function releaseApproved(address _nftAddress, uint256 _tokenId) public onlyOperator  {
    require(IERC721(_nftAddress).getApproved(_tokenId) == address(this), "NFT must be approved to Pandolfini Escrow");
