...
contract CryptoDudes is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_PUBLIC = 2000;
    uint private constant MAX_GIFT = 22;

    uint private constant MAX_NFT_PER_WALLET_WHITELIST = 1;

    //white list duration
    uint private constant WHITELIST_DURATION = 2880 minutes;
    ...
    constructor(address[] memory _team, uint[] memory _teamShares, bytes32 _merkleRoot, string memory _baseURI, uint _startTime) ERC721A("Nom de la collection", "SYMBOL")
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(amountNFTsperWalletWhitelistSale[msg.sender] + _quantity <= MAX_NFT_PER_WALLET_WHITELIST, "You can only get 1 NFT on the Whitelist Sale");
