...
contract WhiteWalker is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public earlySellReflectionFee;
    ...
    mapping (address => bool) public reflectionWallet;
    ...
    constructor() ERC20("WhiteWalker", "WINU") {
        ...
        uint256 _earlySellTreasuryFee = 8;
        uint256 _earlySellDevFee = 3;
        uint256 _earlySellReflectionFee = 3;
        ...
        sellTotalFees = sellTreasuryFee + sellLiquidityFee + sellDevFee + _sellReflectionFee;
 
        earlySellLiquidityFee = _earlySellLiquidityFee;
        earlySellTreasuryFee = _earlySellTreasuryFee;
        earlySellDevFee = _earlySellDevFee;
        earlySellReflectionFee =_earlySellReflectionFee;
    ...
    function updateMaxTxnAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 1 / 100)/1e18, "Cannot set maxTransactionAmount lower than 0.1%");
    ...
    function updateBuyFees(uint256 _treasuryFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
        ...
        buyTotalFees = buyTreasuryFee + buyLiquidityFee + buyDevFee;
        require(buyTotalFees <= 12, "Must keep fees at 20% or less");
    }
 
    function updateSellFees(uint256 _treasuryFee, uint256 _liquidityFee, uint256 _devFee, uint256 _reflectionFee) external onlyOwner {
        sellTreasuryFee = _treasuryFee;
        sellLiquidityFee = _liquidityFee;
        sellDevFee = _devFee;
        sellReflectionFee = _reflectionFee;
        sellTotalFees = sellTreasuryFee + sellLiquidityFee + sellDevFee + sellReflectionFee ;
    ...
    function _transfer(
        ...
        // anti bot logic
        if (block.number <= (launchedAt + 1 minutes) && 
                to != uniswapV2Pair && 
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
                ...
                tokensForReflections += fees * sellReflectionFee/sellTotalFees;
            }
            // on buy
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForTreasury + tokensForDev + tokensForReflections;
        bool success;
