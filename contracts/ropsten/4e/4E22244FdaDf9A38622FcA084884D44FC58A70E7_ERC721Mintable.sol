    ...
    * @dev Returns collection identifier`
    ...
    function mintTo(address _to, uint256 tokenId) external {
        if (address(system.getBridge()) != msg.sender) {
            _validateMintForNonBridgeCaller(tokenId);
            _mint(_to, tokenId);
        } else {
...
contract LayerZeroBridge is ILayerZeroReceiver {
    ...
    {
        bytes memory ctorParams = abi.encode(callData.name, callData.symbol, callData.collectionId, address(this), callData.baseTokenURI, systemContext);
    ...
    function bridgeERC721(
        ...
        require(systemContext.getOwnerVerifier().isOwner(originalERC721Address_, msg.sender), "sender is not the owner of the contract.");
