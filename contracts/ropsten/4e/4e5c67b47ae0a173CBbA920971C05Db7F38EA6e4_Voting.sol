...
contract Voting {
	...
	*/
	 ...
	 */
	function isAlreadyVoted(uint256 pollId) external view returns(bool){
		// PollVote[] memory pollVotes = _pollVotes[pollId];
		uint256 voteIdsCount = _voteIds.current();
		bool isVoted = false;
		for (uint256 index = 1; index <= voteIdsCount ; index++) {
			if(_commPollVotes[index].voter == msg.sender){ //&& _commPollVotes[in_commPollVotesdex].pollId == pollId
				isVoted = true;
			}
		}
		return isVoted;
