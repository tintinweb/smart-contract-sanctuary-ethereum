...
contract NFTCollectible is ERC721Enumerable, Ownable {
    ...
    uint public constant PRICE = 0.5 ether;
    uint public constant MAX_PER_MINT = 1;
    
    string public baseTokenURI;
    
    constructor(string memory baseURI) ERC721("MetaDoc", "MTDC") {
        setBaseURI(baseURI);
    }
    ...
    function mintNFTs(uint _count) private onlyOwner {
        uint totalMinted = _tokenIds.current();

        require(totalMinted.add(_count) <= MAX_SUPPLY, "Not enough Metadocs left!");
        require(_count >0 && _count <= MAX_PER_MINT, "Cannot mint specified number of Metadocs.");
        require(msg.value >= PRICE.mul(_count), "Not enough ether to purchase Metadocs.");
