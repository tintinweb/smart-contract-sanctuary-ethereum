...
contract Moonshot is Context, IERC20, Ownable {
    ...
    uint256 public _buyBackMinTokenAmount = MIN_BUY_AMOUNT;
    uint256 public _buyBackMaxTokenAmount = MAX_BUY_AMOUNT;
    ...
    event SetBuyBackCooldownInterval(uint256 value, uint256 timestamp);
    event SetBuyBackSize(uint256 percentage);*/
   ...
   function setFees(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newProjectFee, uint256 newBuyBackFee) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forBuyBack >0 && _buyBackFee > 0) {
            ...
            if( buyBackAmount > _buyBackMaxTokenAmount ) {
                buyBackAmount = _buyBackMaxTokenAmount;
            }

            // buy if more than minimum amount of bnb to spent
            if( buyBackAmount > _buyBackMinTokenAmount && _buyBackCooldownTimestamp < block.timestamp) {
