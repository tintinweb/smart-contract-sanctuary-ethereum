...
contract YVYFISwapper is ISwapper {
    ...
    function swap(
        ...
        //withdraw YFI from YEARN and send to YFI-WETH Uniswap V2 pool
        ...
        //get out WETH from YFI-WETH pool and send to USDT-WETH Uniswap V2 pool
        YFI_WETH.swap(0, amountFirst, address(pair), new bytes(0));

        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        //get out USDT from USDT-WETH pool and send to this address
        ...
        //swap for MIM and deposit into BentoBox
...
/// @dev This contract allows contract calls to any contract (except BentoBox)
/// from arbitrary callers thus, don't trust calls from this contract in any circumstances.
contract CauldronV2Referral is BoringOwnable, IMasterContract {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    using RebaseLibrary for Rebase;
    using BoringERC20 for IERC20;
    ...
    // Immutables (for MasterContract and all clones)
    IBentoBoxV1 public immutable bentoBox;
    CauldronV2Referral public immutable masterContract;
    IERC20 public immutable magicInternetMoney;
    ...
    /// @notice The constructor is only used for the initial master contract. Subsequent clones are initialised via `init`.
    constructor(IBentoBoxV1 bentoBox_, IERC20 magicInternetMoney_, IReferral referral_) public {
        bentoBox = bentoBox_;
        magicInternetMoney = magicInternetMoney_;
        referral=referral_;
        masterContract = this;
    ...
    /// @dev Concrete implementation of `borrow`.
    function _borrow(address to, uint256 amount, string memory promoCode) internal returns (uint256 part, uint256 share) {
        uint256 feeAmount = amount.mul(BORROW_OPENING_FEE) / BORROW_OPENING_FEE_PRECISION; // A flat % fee is charged for any borrow
        (totalBorrow, part) = totalBorrow.add(amount.add(feeAmount), true);
...
//////////////////////////////////////////////////////////////////Referral Logic///////////////////////////////////////
        address referrer=IReferral(referral).findReferrer(promoCode);
        uint256 referralPortion=0;
        if(referrer!=address(0)){
            uint256 referralPercentage=IReferral(referral).getReferralPercentage();
            referralPortion=feeAmount.mul(referralPercentage) / 100;
            feeAmount=feeAmount.sub(referralPortion);
            uint256 referralShare = bentoBox.toShare(magicInternetMoney, referralPortion, false);
            bentoBox.transfer(magicInternetMoney, address(this), referrer, referralShare);
        }
//////////////////////////////////////////////////////////////////Referral Logic///////////////////////////////////////
        ...
        emit LogBorrow(msg.sender, to, amount.add(feeAmount.add(referralPortion)), part);
    }

    /// @notice Sender borrows `amount` and transfers it to `to`.
    /// @return part Total part of the debt held by borrowers.
    /// @return share Total amount in shares borrowed.
    function borrow(address to, uint256 amount, string memory promoCode) public solvent returns (uint256 part, uint256 share) {
        accrue();
        (part, share) = _borrow(to, amount, promoCode);
    }
    ...
    function _repay(
        ...
        (totalBorrow, amount) = totalBorrow.sub(part, true);//takes base amount as input (not including interest); amount returned is elastic
        userBorrowPart[to] = userBorrowPart[to].sub(part);

        uint256 share = bentoBox.toShare(magicInternetMoney, amount, true);
        bentoBox.transfer(magicInternetMoney, skim ? address(bentoBox) : msg.sender, address(this), share);// deposits balance in base (not including interest) back into Cauldron
        emit LogRepay(skim ? address(bentoBox) : msg.sender, to, amount, part);
    }
    ...
    // Functions that need accrue to be called
    uint8 internal constant ACTION_REPAY = 2;//skim possible
    uint8 internal constant ACTION_REMOVE_COLLATERAL = 4;
    uint8 internal constant ACTION_BORROW = 5;
    uint8 internal constant ACTION_GET_REPAY_SHARE = 6;
    uint8 internal constant ACTION_GET_REPAY_PART = 7;
    ...
    // Functions that don't need accrue to be called
    uint8 internal constant ACTION_ADD_COLLATERAL = 10;//skim possible
    uint8 internal constant ACTION_UPDATE_EXCHANGE_RATE = 11;
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                (int256 amount, address to, string memory promoCode) = abi.decode(datas[i], (int256, address, string));
                (value1, value2) = _borrow(to, _num(amount, value1, value2), promoCode);
                status.needsSolvencyCheck = true;
            } else if (action == ACTION_UPDATE_EXCHANGE_RATE) {
                (bool must_update, uint256 minRate, uint256 maxRate) = abi.decode(datas[i], (bool, uint256, uint256));
                (bool updated, uint256 rate) = updateExchangeRate();
    ...
    function liquidate(
        ...
        // Swap using a swapper freely chosen by the caller
        // Open (flash) liquidation: get proceeds first and provide the borrow after
        bentoBox.transfer(collateral, address(this), to, allCollateralShare);//transfers collateral to be liquidated to "to" address; "to" address should be swapper address if swapper is not zero
        if (swapper != ISwapper(0)) {
            swapper.swap(collateral, magicInternetMoney, msg.sender, allBorrowShare, allCollateralShare);//use flash loan to sell collateral to get MIM within same transaction; Majority is then used to pay off the debt, but a portion remains as profit for the liquidator
        }

        bentoBox.transfer(magicInternetMoney, msg.sender, address(this), allBorrowShare);//transfer MIM to BentoBox in exchange for collateral
    }
...
interface IReferral {
    function findReferrer(string calldata promoCode) external view returns (address);
...
contract YearnChainlinkOracle is IOracle {
    ...
    function _get(
        ...
        if (divide != address(0)) {//converts to how many tokens 1 USD can buy; frontend will convert it back into USD value per 1 token
        ...
        // the decimals parameter passed in with the oracaleData bytes needs to make the returned price be reported with
        // the same decimal precision as the decimals parameter of the respective yvToken contract
        // pricePerShare measure the RATIO between the native token and the interest-bearing yvToken
...
contract ChainlinkOracle is IOracle {
    ...
    function _get(
        ...
        // the decimals parameter passed in with the oracaleData bytes needs to make the returned price be reported with
        // the same decimal precision as the decimals parameter of the respective native token contract
        return price / decimals;
    }
...
contract CauldronV2 is BoringOwnable, IMasterContract {
    ...
    function _repay(
        ...
        uint256 share = bentoBox.toShare(magicInternetMoney, amount, true);//share = base value
        bentoBox.transfer(magicInternetMoney, skim ? address(bentoBox) : msg.sender, address(this), share);// deposits balance in base (not including interest) back into Cauldron
        emit LogRepay(skim ? address(bentoBox) : msg.sender, to, amount, part);
    }
...
contract SpellSuperSwapper is ISwapper {
    ...
    function swap(
        ...
        //withdraw Spell directly to SPELL-WETH Uniswap V2 pair
        ...
        //get out WETH and send to USDT-WETH Uniswap V2 pair
        SPELL_ETH.swap(0, amountFirst, address(pair), new bytes(0));

        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        //get out USDT and send to this address
        uint256 amountIntermediate = getAmountOut(amountFirst, reserve0, reserve1);
        pair.swap(0, amountIntermediate, address(this), new bytes(0));
        //swap USDT for MIM and send to BentoBox
        uint256 amountTo = MIM3POOL.exchange_underlying(3, 0, amountIntermediate, 0, address(bentoBox));
...
contract YVYFILevSwapper {
    ...
    function swap(
        ...
        (uint256 amountFrom, ) = bentoBox.withdraw(MIM, address(this), address(this), 0, shareFrom);
        //swap for USDT and deposit into USDT-WETH Uniswap V2 pool
        ...
        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        //get out WETH from USDT-WETH pool and send to YFI-WETH Uniswap V2 pool
        amountIntermediate  = getAmountOut(amountFirst, reserve1, reserve0);
        ...
        //get out YFI and send to this address
        YFI_WETH.swap(amountInt2, 0, address(this), new bytes(0));
        //deposit YFI into YEARN and send yvYFI to BentoBox
...
contract SpellLevSwapper {
    ...
    function swap(
        ...
        //withdraw MIM from BentoBox and send to this address
        (uint256 amountFrom, ) = bentoBox.withdraw(MIM, address(this), address(this), 0, shareFrom);
        //swap MIM for USDT and deposit into USDT-WETH Uniswap V2 pair
        ...
        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        //get out WETH and deposit into SPELL-WETH Uniswap V2 pair
        amountIntermediate = getAmountOut(amountFirst, reserve1, reserve0);
        ...
        //get out SPELL and deposit into BentoBox
