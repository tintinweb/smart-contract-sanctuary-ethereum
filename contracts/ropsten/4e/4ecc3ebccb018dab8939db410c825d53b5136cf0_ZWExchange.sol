...
contract ZWExchange is ExchangableToken, Ownable {
    ...
    event EventExchangeZWCToTokenGetZWC(address indexed src_address,
                                address indexed dst_address,
                                address indexed token_address,
                                uint256 zwc_amount_wei);

    event EventExchangeZWCToToken(address indexed src_address,
                                address indexed dst_address,
                                address indexed token_address,
                                uint256 token_amount_wei,
                                uint256 zwc_amount_wei);
    ...
    {
        uint256 amount_wei = toWei(token_address, daily_exchange_amount);

        tokenMap[token_address] = TokenInfo({
            rate:                       token_rate,
            enable:                     token_enable,
            daily_exchange_amount_wei:  amount_wei,
    ...
    //更新token每日兑换上限
    function UpdateTokenDailyExchangeAmount(address token_address, uint256 daily_exchange_amount)
        public
        onlyOwner()
        notNullAddress(token_address)
        tokenExists(token_address)
    {
        uint256 amount_wei = toWei(token_address, daily_exchange_amount);
        tokenMap[token_address].daily_exchange_amount_wei = amount_wei;
        emit EventUpdateTokenDailyExchangeAmount(token_address, daily_exchange_amount);
    }
        ...
        isExchangeEnable()
    {
        ...
        //检查owner剩余的zwc数量是否足够
        require(zwcToken.balanceOf(owner) >= zwc_amount_wei);

        //检查用户剩余的token数量是否足够
        // ExchangableToken otherToken = createTokenContract();
        ...
        //把zwc转移到用户账户中
        require(zwcToken.TransferToken(owner, msg.sender, zwc_amount_wei));
        emit EventExchangeTokenToZWC(msg.sender, owner, token_address, token_amount_wei, zwc_amount_wei);
    ...
    //ZWC兑换成token
    function ExchangeZWCToToken(address token_address, uint256 zwc_amount_wei)
        public
        notNullAddress(token_address)
        tokenExists(token_address)
        isExchangeEnable()
    {
        //根据汇率计算zwc可以兑换的token数量
        uint256 token_amount_wei = calcTokenAmountByZWC(token_address, zwc_amount_wei);
        if (token_amount_wei == 0)
        {
            emit EventExchangeZWCToToken(msg.sender, owner, token_address, token_amount_wei, zwc_amount_wei);
            return;
        }

        //检查owner剩余的Token数量是否足够
        ExchangableToken otherToken = ExchangableToken(token_address);
        require(otherToken.balanceOf(owner) >= token_amount_wei);

        //检查用户剩余的zwc数量是否足够
        require(zwcToken.balanceOf(msg.sender) >= zwc_amount_wei);

        //用户把自己的ZWC转移到owner账户中, 之前需要用户先调用approve
        require(zwcToken.transferFrom(msg.sender, owner, zwc_amount_wei));
        emit EventExchangeZWCToTokenGetZWC(msg.sender, owner, token_address, zwc_amount_wei);

        //把token转移到用户账户中
        // ExchangableToken otherToken = createTokenContract();
        // otherToken = token_address;
        require(otherToken.TransferToken(owner, msg.sender, token_amount_wei));
        emit EventExchangeZWCToToken(msg.sender, owner, token_address, token_amount_wei, zwc_amount_wei);
    }
     ...
     */
    //计算ZWC能换成多少token
    function calcTokenAmountByZWC(address token_address, uint256 zwc_amount_wei)
        internal
        view
        notNullAddress(token_address)
        tokenExists(token_address)
        returns (uint256)
    {
        ...
        uint256 zwc_amount = fromWei(address(zwcToken), zwc_amount_wei);
        uint256 token_amount = zwc_amount.mul(10 ** RATE_DECIMAL).div(rate);

        uint256 token_amount_wei = toWei(token_address, token_amount);
