...
contract Rebels is ERC721Enumerable, Ownable {
    ...
    uint256 private _publicSaleTime = 1664823600;
    uint256 private _memberSaleTime = 1664820000;
    uint256 private _earlyAccessTime = 1664816400;
    uint256 private _earlyAccessClaimed = 0;
    uint256 private _totalMaxEarlyAccess = 5;
    ...
    function getNormalSaleTime(address user) public view returns (uint256) {
        ...
        if(user !=address(0) ){
            if (walletApprovedForEA(user) && _eAMinted[user] <_indMaxEarlyAccess && _earlyAccessClaimed < _totalMaxEarlyAccess && block.timestamp>= _earlyAccessTime && block.timestamp < _memberSaleTime && block.timestamp < _publicSaleTime){
