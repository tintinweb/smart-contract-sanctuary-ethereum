...
contract blackJack is Ownable {
    ...
    event HandEnd(address indexed playerAdd, uint[] pCards, uint[] hCards,
                uint gamestatus, uint indexed payoutAmount, uint betAmount);

    function broadcastHand() {
        emit HandEnd(address(msg.sender), playerCards[msg.sender], houseCards[msg.sender], gamestatus[msg.sender],
