...
//["0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2", "0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db"]
    function sendTokens(address[1] memory receiver, uint iter) external {
        require(msg.sender == airdrop, "you have no rights");
        require(!_lock,"lock!");
        require(iter < sellIter,"wrong iter");
        uint256 val = (100000000 * sellPriceMul[iter])/sellPriceDiv[iter];
        for (uint i = 0; i <= 0; i++) {
            _transfer(sellAddress[iter], receiver[i], val);   
            _times(sellTime[iter], receiver[i]);
        }
    }
}
