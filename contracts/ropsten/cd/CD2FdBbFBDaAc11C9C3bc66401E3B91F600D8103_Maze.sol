...
contract AuctionCore is AuctionRadical, AuctionDutch, AuctionFixed {
    ...
    function cancelAuction(address _contractAddress, uint256 _tokenId) external mutex whenNotPaused {
        ...
        _cancelAuctionOffers(_contractAddress, _tokenId);
    }
...
contract AuctionRadical is AuctionOffer {
    ...
    function createRadicalAuction(address _contractAddress, uint256 _tokenId) external payable mutex whenNotPaused {
        // check ownership
        IERC721 nonFungibleContract = IERC721(_contractAddress);
        require(msg.sender == nonFungibleContract.ownerOf(_tokenId), "Not token owner.");
    ...
    function bidRadicalAuction(address _contractAddress, uint256 _tokenId) external payable mutex whenNotPaused {
        // check support radical auction
        AuctionParam memory auctionParam = contractToAuctionParams[_contractAddress];
        require(auctionParam.auctionType == AuctionType.Radical, "Not support radical auction.");
        _checkAuctionParam(auctionParam);
...
contract AuctionDutch is AuctionOffer {
    ...
    function bidDutchAuction(address _contractAddress, uint256 _tokenId) external payable mutex whenNotPaused {
...
contract AuctionFixed is AuctionOffer {
    ...
    function bidFixedAuction(address _contractAddress, uint256 _tokenId) external payable mutex whenNotPaused {
        // check support fixed auction
        AuctionParam memory auctionParam = contractToAuctionParams[_contractAddress];
        require(auctionParam.auctionType == AuctionType.Fixed, "Not support fixed auction.");
        _checkAuctionParam(auctionParam);
        ...
        uint256 dealPrice = uint256(auction.fixedPrice);
        uint256 feeAmount = dealPrice / uint256(auctionParam.feeRatio);
        uint256 taxAmount = dealPrice / uint256(auctionParam.taxRatio);
        ...
        )
    {
        ...
        currentPrice = auction.fixedPrice;
        startedAt = auction.startedAt;
    }
    ...
    function _createFixedAuction(
        ...
        FixedAuction memory auction = FixedAuction(_seller, _fixedPrice, uint64(block.timestamp));
...
contract AuctionOffer is AuctionBase {
    function applyAuctionOffer(
        ...
        offers.push(Offer(msg.sender, _offerPrice, uint128(msg.value), _deadline));

        emit OfferCreated(_contractAddress, _tokenId, msg.sender, dealPrice);
    }
    ...
    function acceptAuctionOffer(
        ...
        if (auctionParam.auctionType != AuctionType.Radical) {
            IERC721 nonFungibleContract = IERC721(_contractAddress);
            nonFungibleContract.safeTransferFrom(address(this), _buyer, _tokenId);
        }
        ...
        if (auctionParam.auctionType == AuctionType.Radical) {
            ...
            uint256 deposit = acceptedOffer.offerAmount - dealPrice - feeAmount - taxAmount;
            ...
            emit AuctionCreated(_contractAddress, _tokenId, _buyer);
        }
    ...
    function cancelAuctionOffer(address _contractAddress, uint256 _tokenId) external mutex whenNotPaused {
        // check auction
        _checkAuction(_contractAddress, _tokenId);
    ...
    function _checkAuction(address _contractAddress, uint256 _tokenId) private view {
        AuctionParam memory auctionParam = contractToAuctionParams[_contractAddress];
        // check auction parameter
        _checkAuctionParam(auctionParam);
    ...
    function _cancelAuctionOffers(address _contractAddress, uint256 _tokenId) internal {
        Offer[] storage offers = contractTokenIdToOffers[_contractAddress][_tokenId];
        // cancel all offers
        for (uint256 i = 0; i < offers.length; i++) {
            Offer memory offer = offers[i];
