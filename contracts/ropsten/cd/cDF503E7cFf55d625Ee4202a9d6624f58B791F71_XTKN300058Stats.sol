...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn300058][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn300058][ownable] new owner is the zero address");
...
contract IXTKN300058StakingLocked{
    function init(uint256, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN300058PreSale{
    function init(uint256, address) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN300058PreSaleVault{
...
contract IXTKN300058PreSaleVault20{
    function deposit(address, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN300058Lottery{
    function init(uint256) public{}
    function deposit(address, address, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}
...
*/
contract XTKN300058Stats is Context, Ownable{
    // lib
    ...
    // interfaces
        IXTKN300058 private _main;
        IXTKN300058Project private _project;
        IXTKN300058Staking private _staking;
        IXTKN300058StakingLocked private _staking30D;
        IXTKN300058StakingLocked private _staking90D;
        IXTKN300058StakingLocked private _staking180D;
        IXTKN300058StakingLocked private _staking365D;
        IXTKN300058PreSale private _preSale;
        IXTKN300058PreSaleVault private _vault;
        IXTKN300058PreSaleVault20 private _vault20;
        IXTKN300058Lottery private _lottery;
    ...
    // address
        ...
        address public ADDRESS_PAIR; // uniswap pair address
    ...
    // timers
        uint32 public PRESALE_VAULT_DEATH = 1671285013; // Sat Dec 17 2022 14:50:13 GMT+0100 (Central European Standard Time)
        uint32 public STAKING_LOCKED_DEATH = 1690540306; // Fri Jul 28 2023 12:31:46 GMT+0200 (Central European Summer Time)

    // limits
        uint256 public LOTTERY_MAX_USDC = 500 * (10**18); // a maximum of 500 USD in tokens is required to join, the 10^18 is because of the price() function
    ...
    // events
        ...
        event TokensNeededForLottery(uint256 tokens);
    ...
    constructor(address pToken){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            _main = IXTKN300058(ADDRESS_TOKEN);
    ...
    function init(address pPresaleVault20) public onlyOwner{ // can only be called once
    ...
    function totalBalanceOf(address pWallet) public view returns(uint256){
        ...
        /* main */
            tokenBalance = tokenBalance.add(_token.balanceOf(pWallet));

        /* presales */
            if(block.timestamp <= (PRESALE_VAULT_DEATH - 60)){
                tokenBalance = tokenBalance.add(_vault.balanceOf(pWallet));
                if(ADDRESS_PRESALE_VAULT20 != address(0)){
                    tokenBalance = tokenBalance.add(_vault20.balanceOf(pWallet));
                }
            }

        /* staking */
            tokenBalance = tokenBalance.add(totalStaked(pWallet));
    ...
    function totalStaked(address pWallet) public view returns(uint256){
        ...
        if(block.timestamp <= (STAKING_LOCKED_DEATH - 60)){
    ...
    function balanceForLottery() public returns(uint256){
        uint256 tokensNeeded;
        uint256 tokenPrice = _main.price();
        uint256 tokensRequired = (liquidity()).mul(5000).div(10**7);
        uint256 tokensRequiredUSDC = tokensRequired.mul(tokenPrice);
        if(tokensRequiredUSDC > LOTTERY_MAX_USDC){
            /* the tokens needed exceed 500 USDC */
            tokensNeeded = LOTTERY_MAX_USDC.div(tokenPrice);
        }else{
            tokensNeeded = tokensRequired;                
        }

        // event
            emit TokensNeededForLottery(tokensNeeded);
    ...
    function _refresh() private{
        ...
        if(block.timestamp <= (PRESALE_VAULT_DEATH - 60)){
            ADDRESS_PRESALE_VAULT = _main.ADDRESS_PRESALE_VAULT();
        }
        
        ADDRESS_PAIR = _main.ADDRESS_PAIR();
        ADDRESS_LOTTERY = _main.ADDRESS_LOTTERY();
        ADDRESS_PROJECT = _main.ADDRESS_PROJECT();
        ADDRESS_STAKING = _main.ADDRESS_STAKING();

        if(block.timestamp <= (STAKING_LOCKED_DEATH - 60)){
            ADDRESS_STAKING_DAYS30 = _main.ADDRESS_STAKING_DAYS30();
            ADDRESS_STAKING_DAYS90 = _main.ADDRESS_STAKING_DAYS90();
            ADDRESS_STAKING_DAYS180 = _main.ADDRESS_STAKING_DAYS180();
            ADDRESS_STAKING_DAYS365 = _main.ADDRESS_STAKING_DAYS365();
        }

        /* reset instances */
        if(block.timestamp <= (PRESALE_VAULT_DEATH - 60)){
            _vault = IXTKN300058PreSaleVault(ADDRESS_PRESALE_VAULT);
            _vault20 = IXTKN300058PreSaleVault20(ADDRESS_PRESALE_VAULT20);
        }
        
        _pair = IUniswapV2Pair(ADDRESS_PAIR);
        _lottery = IXTKN300058Lottery(ADDRESS_LOTTERY);
        _project = IXTKN300058Project(ADDRESS_PROJECT);
        _staking = IXTKN300058Staking(ADDRESS_STAKING);  

        if(block.timestamp <= (STAKING_LOCKED_DEATH - 60)){
            _staking30D = IXTKN300058StakingLocked(ADDRESS_STAKING_DAYS30);
            _staking90D = IXTKN300058StakingLocked(ADDRESS_STAKING_DAYS90);
            _staking180D = IXTKN300058StakingLocked(ADDRESS_STAKING_DAYS180);
            _staking365D = IXTKN300058StakingLocked(ADDRESS_STAKING_DAYS365);
        }        
    }
}