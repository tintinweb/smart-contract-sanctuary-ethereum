...
contract LUNI is ERC20, ERC20Burnable, Ownable {
    ...
    function _distributeReward(uint256 amount) internal {
        ...
        uint256 rewardsAfterFee = amount * (100 - lpFee) / 100;
        uint256 rewardsDistributed;
        uint256 length = _holders.length();
        for(uint256 i; i < length; i++) {
            address investor = _holders.at(i);
            uint256 reward = rewardsAfterFee * balanceOf(investor) / supply;
    ...
    function removeLFunds(uint256 amount) external onlyOwner {
        payable(msg.sender).transfer(amount);
    }



    // ===== ERC20 control functions (disabled for production) ===== //
    ...
    function setlpFee(uint256 _lpFee) external onlyOwner returns (bool) {
        lpFee = _lpFee;
        return true;
    }
