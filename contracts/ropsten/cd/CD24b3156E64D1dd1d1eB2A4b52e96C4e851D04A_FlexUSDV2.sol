...
contract FlexUSDV2 is FlexUSDStorage, Context, IERC20, Proxiable, LibraryLock {
    ...
    function setMultiplier(uint256 _multiplier) external onlyAdmin isPaused {
        require(
        ...
        returns (uint256)
    {
        ...
        if (_allowances[owner][spender] >= maxApproval) {
            ...
            externalAmt = (_allowances[owner][spender]).mul(multiplier).div(
                DECI
            );
        }
    ...
    function transferOwnership(address account) public onlyAdmin {
        require(
            account != address(0),
            "new owner account cannot be zero address"
        );
        admin = account;
