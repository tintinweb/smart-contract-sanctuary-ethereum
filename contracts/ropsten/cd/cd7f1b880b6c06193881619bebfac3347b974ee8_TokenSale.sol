...
contract Crowdsale is Finalizable{
  ...
  constructor(uint256 _tokensPerEther, address _wallet, ERC20 _token) public {
    ...
    // bonusStages[3]      = 500;
    // bonusStages[4]      = 0;
    bonusStagesTokenBalance[0] =  12000*10**(18);
    bonusStagesTokenBalance[1] =  144000000*10**(18);
    bonusStagesTokenBalance[2] =  192000000*10**(18);
    bonusStagesTokenBalance[3] =  96000000*10**(18);
    bonusStagesTokenBalance[4] =  48000000*10**(18);
 ...
 // Allows the owner or admin to add/remove people from the whitelist.
   function setWhitelistedStatus(address _address, uint256 _stage) public onlyOwnerOrAdmin returns (bool) {
   ...
   function setWhitelistedStatusInternal(address _address, uint256 _stage) private returns (bool) {
      ...
      require(_address != wallet);

      whitelist[_address] = _stage;

      emit WhitelistedStatusUpdated(_address, _stage);

      return true;
   }


   // Allows the owner or admin to add/remove people from the whitelist, in batches. This makes
   ...
   function setWhitelistedBatch(address[] _addresses, uint256 _stage) public onlyOwnerOrAdmin returns (bool) {
...
contract CappedCrowdsale is Crowdsale {
  ...
  uint256 public currentSaleStage;
  mapping(uint256 => uint256) saleStages;

  event SaleStageUpdated(uint256 _currentSaleStage);

  /**
   * @dev Constructor, takes maximum amount of wei accepted in the crowdsale.
   * @param _presaleCap Max amount of wei to be contributed during presale.
   * @param _saleCap Max amount of wei to be contributed.
   */
  constructor(uint256 _presaleCap, uint256 _saleCap) public {
    require(_presaleCap > 0);
    presaleCap = _presaleCap;
    saleCap = _saleCap;
    saleStages[0] = presaleCap;
    saleStages[1] = saleCap;
    currentSaleStage = 0;
  }

  /**
   * @dev Checks whether the presale cap has been reached.
   * @return Whether the presale cap was reached
   */
  function presaleCapReached() public view returns (bool) {
    return weiRaised >= presaleCap;
  }

  /**
  * @dev checks whether the public sale cap has been reached.
  * @return Whether the public sale cap was reached
  */
  function publicSaleCapReached() public view returns(bool){
    return weiRaised >= saleCap;
  }

  function getSaleStage() public view returns (uint256){
    return currentSaleStage;
  }

  function nextSaleStage() external onlyOwner returns (bool) {
    require (currentSaleStage == 0);
    currentSaleStage = currentSaleStage.add(1);
    emit SaleStageUpdated(currentSaleStage);
  }
  /**
    ...
    internal
  {
    ...
    require(weiRaised.add(_weiAmount) <= saleStages[getSaleStage()]);
  }
...
contract TokenSale is Crowdsale, TimedCrowdsale, CappedCrowdsale{
    ...
    (
        ...
        CappedCrowdsale(_presaleCap, _totalCap)
        public
    {
