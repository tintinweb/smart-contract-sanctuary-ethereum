...
//	IERC20 public token;
	...
	event debug2(uint48);
	event debug3(address addr,uint256 stale);
...
//	    uint256 v1 = amount / IToken(TokenAddr).decimals();
//	    v1 = v1 * IToken(TokenAddr).decimals();
	    ...
	    {
		Stakers[addr].time 	= uint48(block.timestamp);
		uint256 stale = StakeCalculate(addr,Stakers[addr].time);
		Stakers[addr].amount 	= amount - stale + Stakers[addr].amount;
		Stakers[addr].stale 	= stale;
		emit debug2(Stakers[addr].time);
		emit debug3(addr,StakeCalculate(addr,Stakers[addr].time));
		emit debug3(addr,StakeCalculate(addr,0));
	    }
