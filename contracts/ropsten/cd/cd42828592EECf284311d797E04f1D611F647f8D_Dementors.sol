...
contract Dementors is ERC721Tradable {
    using Strings for uint256;

    IERC20 public WETH = IERC20(0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619);
    enum SalePhase { LOCKED, WHITELIST, PUBLICSALE }
    ...
    mapping(address => uint64) public whitelistCounter;
    mapping(address => uint64) public publicCounter;
    

    // tracks total mints per phase
    uint64 public totalTeamCounter;
    uint64 public totalWhitelistCounter;
    uint64 public totalPublicCounter;
    

    // total reserve limits
    uint64 public TOTAL_TEAM_RESERVE = 2223;
    uint64 public TOTAL_WHITELIST_RESERVE = 2223;
    uint64 public TOTAL_PUBLIC_RESERVE = 5554;

    // per wallet limits
    uint16 public MAX_PER_WHITELIST_ADDRESS = 20;
    uint16 public MAX_PER_PUBLIC_ADDRESS = 100;

    uint64 public MAX_SUPPLY = 10000;

	uint256 public mintPrice = 0.5 ether; // change as phase is updated
    ...
    constructor(address _proxyRegistryAddress, uint64 maxSupply, address _WETH)
        ERC721Tradable("Dementors", "DEMONS", _proxyRegistryAddress)
    {
    ...
    function setLimits(uint16 whitelistLimit, uint16 publicLimit) public onlyOwner {
        require(!settingsIsFrozen, "Settings are permanently frozen");
        MAX_PER_WHITELIST_ADDRESS = whitelistLimit;
        MAX_PER_PUBLIC_ADDRESS = publicLimit;
    }

    function setReserve(uint64 teamReserve, uint64 whitelistReserve, uint64 publicReserve) public onlyOwner {
        require(!settingsIsFrozen, "Settings are permanently frozen");
        TOTAL_TEAM_RESERVE = teamReserve;
        TOTAL_WHITELIST_RESERVE = whitelistReserve;
        TOTAL_PUBLIC_RESERVE = publicReserve;
    }

    function setMerkleRoots(bytes32 _whitelistRoot) public onlyOwner {
    ...
    function mintWhitelist(bytes32[] memory _merkleProof, uint64 count) public {
        require(whitelistCounter[_msgSender()] + count <= MAX_PER_WHITELIST_ADDRESS, "exceeding limit per wallet");
        require(totalWhitelistCounter + count <= TOTAL_WHITELIST_RESERVE, "exceeding total airdrop limit");
        whitelistCounter[_msgSender()] += count;
        totalWhitelistCounter += count;
        _mintWhitelist(whitelistMerkleRoot, _merkleProof, count, true);
    }
    ...
    function mintTo(address _to, uint64 count) public
        ...
        require(publicCounter[_msgSender()] + count <= MAX_PER_PUBLIC_ADDRESS, "exceeding max limit per wallet");
        publicCounter[_msgSender()] += count;
        transferTokens(_msgSender(), mintPrice * count);
        _mint(_to, count);
    }
