...
contract SupplierCertification {
    ...
    constructor(
        ...
        _setParty(msg.sender, "cert_receiver");
        _pushReqs(msg.sender);

        // set the suppliers
        _setParty(addr_suppl_1, name_suppl_1);
        _pushReqs(addr_suppl_1);
        _setParty(addr_suppl_2, name_suppl_2);
        _pushReqs(addr_suppl_2);
        _setParty(addr_suppl_3, name_suppl_3);
        _pushReqs(addr_suppl_3);
    }

    // assigns name to given address
    function _setParty(address _wallet, string memory party_name) internal {
