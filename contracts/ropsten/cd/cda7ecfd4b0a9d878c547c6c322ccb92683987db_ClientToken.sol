...
contract ClientToken is ERC20, ERC20Burnable, Pausable, Ownable {
    ...
    // add multiple accounts at a time to the exempt list by separating all of them with ',' (comma)
    function addExempt(address[] calldata accounts) external onlyOwner {
        for(uint256 i; i<accounts.length; ++i){
            _isExempted[accounts[i]] = true;
        } 
    }

    // remove single account at a time from the exempt list
