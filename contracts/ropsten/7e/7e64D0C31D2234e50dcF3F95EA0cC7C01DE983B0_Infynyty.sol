...
contract Infynyty is ReentrancyGuard, Ownable{
    ...
    event SellEvent(address caller, uint256 tokenId, address NFTAddress, uint256 nonce, uint256 listingPrice, string desc);
    event BuyEvent(address seller, address buyer, uint256 tokenId, address NFTAddress, uint256 nonce, uint256 listingPrice, string desc);
    event CancelSellEvent(address caller, uint256 tokenId, address NFTAddress, uint256 nonce, uint256 listingPrice, string desc);
    ...
    function sell(ERC1155Struct memory nftInfo, nonceHandler.Sig memory sellNonceRSV, Sig memory sellRSV) public nonReentrant {
        ...
        emit SellEvent(nftInfo.seller, nftInfo.tokenId, nftInfo.NFTAddress, nftInfo.nonce, nftInfo.listingPrice, "NFT Set to Sell");
    }
    function cancelSell(ERC1155Struct memory nftInfo, nonceHandler.Sig memory cancelSellNonceRSV, Sig memory cancelRSV) public nonReentrant {
        ...
        emit CancelSellEvent(nftInfo.seller, nftInfo.tokenId, nftInfo.NFTAddress, nftInfo.nonce, nftInfo.listingPrice, "NFT sell cancelled");
    }
    function cancelSellPlatform(ERC1155Struct memory nftInfo, nonceHandler.Sig memory cancelSellNonceRSV, Sig memory cancelRSV) public nonReentrant onlyAdmin{
        require(verifySigner(platformAddress, messageHash(abi.encodePacked(nftInfo.seller, nftInfo.tokenId, nftInfo.NFTAddress, nftInfo.nonce)), cancelRSV), "Cancel rsv invalid");
        nonceHandler(nonceHandlerAddress).setNonce(nftInfo.NFTAddress, nftInfo.tokenId, cancelSellNonceRSV);
        emit CancelSellEvent(msg.sender, nftInfo.tokenId, nftInfo.NFTAddress, nftInfo.nonce, nftInfo.listingPrice, "NFT sell cancelled");
    }
    function buy(ERC1155Struct memory sellerInfo, ERC1155Struct memory buyerInfo, Sig memory sellerRSV, Sig memory buyerRSV) public payable nonReentrant {
        ...
        emit BuyEvent(sellerInfo.seller, msg.sender, sellerInfo.tokenId, sellerInfo.NFTAddress, sellerInfo.nonce, msg.value, "NFT have been bought!");
    }
