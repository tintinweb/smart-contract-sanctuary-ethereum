...
library SafeMath {
 ...
 /////////////////////Token////////////////////////
  contract MemeCoin is IERC20Metadata {
        ...
        address internal AirdropAddress = address(this);
        address internal devAddress = 0xC26fc4443145a582A95bA82b4c12EE8FA832D1C2;
        address internal presaleAddress = 0xC26fc4443145a582A95bA82b4c12EE8FA832D1C2; 

        uint256 internal airdropFee = 10;
        uint256 internal developFee = 10;
        uint256 internal burnFee = 10;
        uint256 public holderFee = 96;
        uint256 internal totalFee = airdropFee.add(burnFee).add(developFee).add(holderFee);
        uint256 internal divider = 1000;
        ...
        constructor ()  {
            ...
            isExcludFee[AirdropAddress] =true;
            isExcludFee[presaleAddress] =true;
            isExcludFee[address(this)] =true;

            isLimitExempt[Owner] =true;
            isLimitExempt[address(this)] = true;
            isLimitExempt[devAddress] = true;
            isLimitExempt[AirdropAddress] = true;
            isLimitExempt[presaleAddress] = true;
    ...
    function setAir(address _newAirAddr) external onlyOwner {
        AirdropAddress = _newAirAddr;
    }
    function setPre(address _newPreAddr) external onlyOwner {
        presaleAddress = _newPreAddr;
    }
        ...
        if(lauchBlock.add(3) > block.number && !isExcludFee[from]){ 
           ...
           if(pair != to){
            ...
            emit Transfer(from , address(0), burns);
                ...
                if(airdropFee > 0){
                          airs = amount.mul(airdropFee).div(divider);
                          unchecked{
                              _balances[AirdropAddress] = _balances[AirdropAddress].add(airs);
                              }
                          emit Transfer(from , address(AirdropAddress), airs);
                      }
                if(holderFee > 0){
             ...
             return amount.sub(burns).sub(marks).sub(airs).sub(taxs);
    }
