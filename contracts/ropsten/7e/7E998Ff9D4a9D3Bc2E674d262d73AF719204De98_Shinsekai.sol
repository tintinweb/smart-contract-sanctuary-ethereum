...
contract Shinsekai is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 private constant MAX_BATCH = 5;
    ...
    constructor(uint256 amountForTeam_) ERC721A("Shinsekai", "SSK") {
        amountForTeam = amountForTeam_;
        require(amountForTeam_ <= MAX_SUPPLY, "larger collection size needed");
    }
    ...
    function portalAccess(bytes32[] calldata _merkleProof, uint256 quantity)
        external
        payable
        callerIsUser
    {
        ...
        require(
            MerkleProof.verify(_merkleProof, portalAccessRoot, leaf),
        ...
        require(
            portalAccessClaimed[msg.sender] - quantity >= 0,
            "Address already minted num of tokens allowed"
        );
        portalAccessClaimed[msg.sender] =
            portalAccessClaimed[msg.sender] -
            quantity;
    ...
    function reserveCorp(bytes32[] calldata _merkleProof, uint256 quantity)
        external
        payable
        callerIsUser
    {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, reserveCorpRoot, leaf),
            "Invalid Merkle Proof."
        );
        ...
        require(
            reserveCorpClaimed[msg.sender] - quantity >= 0,
            "Address already minted num of tokens allowed"
        );
        reserveCorpClaimed[msg.sender] =
            reserveCorpClaimed[msg.sender] -
            quantity;
        _safeMint(msg.sender, quantity);
        refundIfOver(price * quantity);
    ...
    function publicRegistration(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, publicRegistrationRoot, leaf),
            "Invalid Merkle Proof."
        );
        ...
        require(
            publicRegistrationClaimed[msg.sender] - quantity >= 0,
            "Address already minted num of tokens allowed"
        );
        publicRegistrationClaimed[msg.sender] =
            publicRegistrationClaimed[msg.sender] -
            quantity;
        _safeMint(msg.sender, quantity);
        refundIfOver(price * quantity);
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        payable(0xa6Fc099Cc057b647B1E01562305D3e8150ADf847).transfer(balance);
    }
