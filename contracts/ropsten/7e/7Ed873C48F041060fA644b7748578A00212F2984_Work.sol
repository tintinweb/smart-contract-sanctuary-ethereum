...
contract Work is AccessControlEnumerable{
    ...
    function swapTOKENForBUSD (uint256 bnbAmount) private {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = BUSD;
    ...
    function swapBUSDforTOKEN(address _token, uint256 busdAmount) private {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = _token;
    ...
    function addToMods(address _toAdd) public {
        ...
        _setupRole(keccak256("Mod"), _toAdd);
    }
    ...
    // allow a user to ask a Freelancers for Services
    function askForServices(address _worker, bytes16 orderID) payable public{
        ...
        workAgreement memory newWork = workAgreement(workCount,false,_worker,msg.sender,msg.value,block.timestamp,false,false,true);

        // ERC20 paymentToken = ERC20(_paymentToken);
        // paymentToken.transferFrom(msg.sender,_worker,_price*ERC20(_paymentToken).decimals())
        ...
        emit JobProposal(msg.sender, _worker ,msg.value ,newWork.id, orderID);
    }
    function askForServicesBUSD(address _worker,uint256 _price, bytes16 orderID) public{
        require(Users[msg.sender] != 0, "You're not registered");
        require(isRegisteredAsFreelancers[_worker], "Worker is not a Freelancer");
        require(isVerified[_worker], "Worker has not been verified");
        // require(_price == msg.value, "Price and Value send are different");

        workAgreement memory newWork = workAgreement(workCount,false,_worker,msg.sender,_price,block.timestamp,false,false,true);

        ERC20 paymentToken = ERC20(BUSD);
        paymentToken.transferFrom(msg.sender,_worker,_price*ERC20(paymentToken).decimals());
        ...
        emit JobProposal(msg.sender, _worker ,_price ,newWork.id, orderID);
    }
    ...
    function handleDispute(uint256 _id, address _winner) external {
        require(hasRole("Mod", msg.sender), "Not a Mod");
    ...
    function ban(address _user) external {
        require(hasRole("Mod", msg.sender) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not a Mod");
...
library IterableMapping {
    ...
    // Iterable mapping from address to workAgreement;
    
    struct Map {
        address[] keys;
        mapping(address => workAgreement) values;
        mapping(address => uint) indexOf;
        mapping(address => bool) inserted;
    }

    function get(Map storage map, address key) public view returns (uint) {
        return map.values[key].id;
    }
    ...
    function set(Map storage map, address key, workAgreement memory val) public {
        if (map.inserted[key]) {
            map.values[key] = val;
        } else {
