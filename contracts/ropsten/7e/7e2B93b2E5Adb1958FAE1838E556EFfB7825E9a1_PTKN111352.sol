...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[ptkn111352][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[ptkn111352][ownable] new owner is the zero address");
...
contract PTKN111352 is Context, IERC20, Ownable{
    // lib
    ...
    // tokenomics
        string private _name = "PTKN111352";
        string private _symbol = "PTKN111352";
    ...
    // modifiers
        modifier onlyAuthorized(){
            require(_addressAuthorized[_msgSender()], "[ptkn111352] only authorized wallets can call this function!");
            _;
        }
    ...
    function addLiquidity() public onlyAuthorized{
        require(!_liquidityProvided, "[ptkn111352] liquidity can only be enabled once!");
        _addLiquidity();
    }

    function enableTrading() public onlyAuthorized{
        require(!_tradingEnabled, "[ptkn111352] trading can only be enabled and not disabled!");
    ...
    function setTaxes(uint16 pMarketing, uint16 pDeveloper, uint16 pBuyback, bool pBuy) public onlyAuthorized{
        require((pMarketing + pDeveloper + pBuyback) <= 200, "[ptkn111352] sum of all taxes cant exceed 20%!");
    ...
    function send(address pTo, uint256 pAmount) public{
        require(pAmount <= (balanceOf(address(this)).sub(TAXES_MARKETING.add(TAXES_DEVELOPER).add(TAXES_BUYBACK))), "[ptkn111352] you cant send more tokens than the contract has!");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[ptkn111352] approve from the zero address");
        require(spender != address(0), "[ptkn111352] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[ptkn111352] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[ptkn111352] transfer amount exceeds allowance"));
        return(true);
    }
    ...
    function _transfer(address pFrom, address pTo, uint256 pAmount) private{
        ...
        if(_addressNoTaxes[pFrom] || _addressNoTaxes[pTo]){
            ...
            if(pFrom == ADDRESS_PAIR){
                require(_tradingEnabled, "[ptkn111352] trading not enabled yet, stay tuned!");
                if(LIMIT_ACTIVE && !_addressNoLimits[pTo]){
                    require(pAmount <= limitWalletTokens && _balances[pTo].add(pAmount) <= limitWalletTokens, "[ptkn111352] wallet limit reached, cant buy more tokens!");
                    require(pAmount <= limitTransactionTokens, "[ptkn111352] transaction limit reached, cant buy more tokens!");
                }
            ...
            if(!Tax.buy && !Tax.sell){
                if(LIMIT_ACTIVE && !_addressNoLimits[pTo]){
                    require(pAmount <= limitWalletTokens && _balances[pTo].add(pAmount) <= limitWalletTokens, "[ptkn111352] wallet limit reached, cant transfer more tokens!");
                }
    ...
    function _addLiquidity() private{
        ...
        require(balance > 0 && _balances[address(this)] >= liquidityForUniswap, "[ptkn111352] you do not have any coins or tokens to provide liquidity!");
