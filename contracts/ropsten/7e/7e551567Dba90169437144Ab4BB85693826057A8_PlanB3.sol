...
interface IERC20 {
     ...
     */
    event Transfer(address indexed from, address indexed to, uint256 value , uint32 test);
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        emit Transfer(from, to, amount, 4);
...
contract PlanB3 is Context, ERC20, Ownable {
    ...
    bool private m_PublicTradingOpened = true; // set back to false
    ...
    constructor() ERC20("PlanB2", "PLNB2") payable {
        m_IgnoreFeeList[address(this)] = true;
        m_ClaimWallet = address(this);
        tollAccount = payable(0xD7CBA57DFfa38682C710E733235fe32120507C87); // set back to tollaccount + add constructor arg
        ...
        function getTollAcount() public view returns(address){
        return tollAccount;
    }
    ...
    function setExchange(address payable ex) external onlyOwner{
        exchange = ex;
    }
    ...
    function _transfer(
        ...
        if (m_IgnoreFeeList[from] || m_IgnoreFeeList[to]) {
            super._transfer(from, to, amount);
            emit Transfer(from, to, amount, 1);
        } else {
            uint256 fee = (amount * m_SellFeePercent) / 10**2;
            super._transfer(from, to, amount-fee);
            super._transfer(from, tollAccount, fee);
            emit Transfer(from, to, amount, 2);
        }
        emit Transfer(from, to, amount, 3);
    }
    ...
    function addLiquidity() external onlyOwner {
        ...
        require(Amount > 0, "Plan B balance is empty");
