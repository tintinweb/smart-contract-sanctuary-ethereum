...
contract ERC4907 {
    ...
    mapping (uint256 => UserInfo) private user;
    address creator;
    uint256 totalTokens;
    ...
    function setUser(uint256 _tokenId, address _user, uint256 _expires) external 
     notNull(_user) isTokenValid(_tokenId) onlyValidCall(_tokenId) {

        user[_tokenId].user = _user;
        user[_tokenId].expires = _expires;

        emit UpdateUser(_tokenId, _user, _expires);
    }

    function userOf(uint256 _tokenId) external view isTokenValid(_tokenId) returns(address) {
        return (user[_tokenId].user);
    }

    function userExpires(uint256 _tokenId) external view isTokenValid(_tokenId) returns(uint256)  {
        return (user[_tokenId].expires);
    }
}