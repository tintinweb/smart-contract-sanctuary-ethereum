 ...
 * Github: madjarevicn
 */
contract HordTicketManager is HordUpgradable, ERC1155HolderUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for *;
    ...
    // Represent hordConfiguration contract
    IHordConfiguration public hordConfiguration;
    ...
    external
    {
        ...
        emit HordTicketFactorySet(_hordTicketFactory);
    }
    ...
    */
    ...
    nonReentrant
    {
        ...
        bool check = numberOfTicketsReserved.add(numberOfTickets)<= hordTicketFactory.tokenIdToMintedSupply(tokenId);
        require(check, "Not enough tickets to sell.");

        // Fixed stake per ticket
        uint amountOfTokensToStake = hordConfiguration.minAmountToStake().mul(numberOfTickets);
        ...
        UserStake memory userStake = UserStake({
            ...
            unlockingTime: hordConfiguration.minTimeToStake().add(block.timestamp),
            isWithdrawn: false
        });
    ...
    nonReentrant
    {
        ...
        if(totalStakeToWithdraw > 0) {
            stakingToken.safeTransfer(msg.sender, totalStakeToWithdraw);
        }
    ...
    returns (uint256)
    {
        uint mintedSupply = hordTicketFactory.tokenIdToMintedSupply(tokenId);
