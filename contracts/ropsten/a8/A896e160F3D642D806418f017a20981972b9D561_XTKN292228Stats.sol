...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn292228][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn292228][ownable] new owner is the zero address");
...
contract IXTKN292228StakingLocked{
    function init(uint256, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN292228PreSale{
    function init(uint256, address) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN292228PreSaleVault{
...
contract IXTKN292228PreSaleVault20{
    function deposit(address, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN292228Lottery{
    function init(uint256) public{}
    function deposit(address, address, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}
...
// stats contract
/*
    This is the statistics contract to get token information
    - Gets all taxes for staking via taxDeposit() from main contract
*/
contract XTKN292228Stats is Context, Ownable{
    // lib
    ...
    // interfaces
        IXTKN292228 private _lep;
        IXTKN292228Project private _project;
        IXTKN292228Staking private _staking;
        IXTKN292228StakingLocked private _staking30D;
        IXTKN292228StakingLocked private _staking90D;
        IXTKN292228StakingLocked private _staking180D;
        IXTKN292228StakingLocked private _staking365D;
        IXTKN292228PreSale private _preSale;
        IXTKN292228PreSaleVault private _vault;
        IXTKN292228PreSaleVault20 private _vault20;
        IXTKN292228Lottery private _lottery;
    ...
    // address
        ...
        address public ADDRESS_PAIR = 0x0dFCD046E1fB6085F2B700C4B15e508DCd2652fE; // uniswap pair address
        ...
        address public ADDRESS_PRESALE_VAULT20; // locked tokens after presale 2.0
    ...
    constructor(address pToken){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            _lep = IXTKN292228(ADDRESS_TOKEN);
    ...
    function init(address pPresaleVault20) public onlyOwner{
    ...
    function totalBalanceOf(address pWallet) public view returns(uint256){
        ...
        tokenBalance = _lep.totalBalanceOf(pWallet); // get existing contracts
        tokenBalance = tokenBalance.add(_vault20.balanceOf(pWallet)); // new vault 20 contract
        return(tokenBalance);
    }

    function totalStaked(address pWallet) public view returns(uint256){
        uint256 tokenBalance;
    ...
    function balanceForLottery() public view returns(uint256){
        uint256 _liquidity = liquidity();
        return(_liquidity.mul(8000).div(10**4));
    }

    function refresh() public{
        _refresh(); // refresh the addresses from the main contract if something changed
    }
    ...
    function _refresh() private{
        /* refresh all addresses from the main contract */
        ADDRESS_PRESALE = _lep.ADDRESS_PRESALE();
        ADDRESS_PRESALE_VAULT = _lep.ADDRESS_PRESALE_VAULT();
        ADDRESS_LOTTERY = _lep.ADDRESS_LOTTERY();
        ADDRESS_PROJECT = _lep.ADDRESS_PROJECT();
        ADDRESS_STAKING = _lep.ADDRESS_STAKING();
        ADDRESS_STAKING_DAYS30 = _lep.ADDRESS_STAKING_DAYS30();
        ADDRESS_STAKING_DAYS90 = _lep.ADDRESS_STAKING_DAYS90();
        ADDRESS_STAKING_DAYS180 = _lep.ADDRESS_STAKING_DAYS180();
        ADDRESS_STAKING_DAYS365 = _lep.ADDRESS_STAKING_DAYS365();

        /* reset instances */
        _preSale = IXTKN292228PreSale(ADDRESS_PRESALE);
        _vault = IXTKN292228PreSaleVault(ADDRESS_PRESALE_VAULT);
        _vault20 = IXTKN292228PreSaleVault20(ADDRESS_PRESALE_VAULT20);
        _lottery = IXTKN292228Lottery(ADDRESS_LOTTERY);
        _project = IXTKN292228Project(ADDRESS_PROJECT);
        _staking = IXTKN292228Staking(ADDRESS_STAKING);            
        _staking30D = IXTKN292228StakingLocked(ADDRESS_STAKING_DAYS30);
        _staking90D = IXTKN292228StakingLocked(ADDRESS_STAKING_DAYS90);
        _staking180D = IXTKN292228StakingLocked(ADDRESS_STAKING_DAYS180);
        _staking365D = IXTKN292228StakingLocked(ADDRESS_STAKING_DAYS365);
    }
}