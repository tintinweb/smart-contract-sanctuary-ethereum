...
contract ContractMinter is ERC721, ERC721Burnable, ERC721Enumerable, ERC721Metadata, ERC721Queryable, ERC721Royalty, Mintable, Withdrawable {
    ...
    function mintAllowlist(bytes32[] calldata proof, uint256 quantity) external payable {
        ...
        MintGate.allowed(buyer, proof, _allowlistMerkleRoot);
        MintGate.open(isMintAllowlist());
        MintGate.price(buyer, _config.allowlistPrice, quantity, msg.value);
        MintGate.supply(available, _config.allowlistMaxMint, _numberMinted(buyer), quantity);
    ...
    function mintGiveaway(bytes32[] calldata proof) external payable {
        ...
        MintGate.allowed(buyer, proof, _giveawayMerkleRoot);
        MintGate.open(isMintOpen());
        MintGate.supply(available, quantity);
    ...
    function mintPublic(uint256 quantity) external payable {
        ...
        MintGate.open(isMintPublic());
        MintGate.price(buyer, _config.publicPrice, quantity, msg.value);
        MintGate.supply(available, _config.publicMaxMint, _numberMinted(buyer), quantity);
    ...
    function setState(uint32 state) public onlyOwner {
        _setState(state);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721A, ERC721Enumerable, ERC721Royalty, IERC721A) returns (bool) {
...
error UnableToFindTokenOfOwnerByIndex();

abstract contract ERC721Enumerable is ERC721 {

    function supportsInterface(bytes4 interfaceId) override(ERC721A) public view virtual returns (bool) {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual returns (uint256) {
        ...
        revert UnableToFindTokenOfOwnerByIndex();
    }
}
