...
contract UEPlots is ERC721Enumerable, MultiOwnable, ReentrancyGuard {
	...
	uint256 public constant STARTING_TOKEN_ID = 100000;
...
abstract contract MultiOwnable is Context {
  ...
  function _addOwner(address add) internal virtual {
	...
	function renounceOwnership() public virtual onlyOwner {
		delete ownerMapping[_msgSender()];
    removeAddressFromOwners(_msgSender());
	}

	function removeAddressFromOwners(address _valueToFindAndRemove)
		internal
	{
    ...
    // address[] memory newOwners = _owners;
		...
		for (uint256 i = 0; i < oldOwners.length; i++) {
			if (oldOwners[i] != _valueToFindAndRemove) {
