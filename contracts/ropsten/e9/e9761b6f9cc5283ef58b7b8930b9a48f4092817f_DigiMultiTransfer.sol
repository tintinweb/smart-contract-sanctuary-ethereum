...
abstract contract CostTransfer {
    ...
    event CommissionPaid(uint256 _amount, address _referAddress, uint256 profitSplit, address _owner);
    ...
    function calcSubstractAmountWithFeeTransfer(uint256 amount) internal virtual returns(uint256) {
    ...
    function distributeCommission(uint256 _amount, address _referAddress, address _owner) internal virtual {
        ...
        uint256 feeOwner = calcFeeTransfer(_amount);
        uint256 feeRefer = calcSubstractAmountWithFeeTransfer(_amount);

        (bool sentOwner, bytes memory dataOwner) =  payable(_owner).call{value: feeOwner}("");
        require(sentOwner, "Failed to send commission owner");
        (bool sentRefer, bytes memory dataRefer) =  payable(_referAddress).call{value: feeRefer}("");
        require(sentOwner, "Failed to send commission refer address");

        emit CommissionPaid(_amount, _referAddress, feeRefer, _owner);
    }
}
