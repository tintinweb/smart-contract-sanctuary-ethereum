...
interface ICryptoAnts is IERC721Upgradeable {
  event EggsBought(address, uint256);
  event AntCreated(uint256);
  event AntSold();
...
contract CryptoAnts is Initializable, ERC721VotesUpgradeable, ReentrancyGuardUpgradeable, ICryptoAnts, OwnableUpgradeable, UUPSUpgradeable {
  mapping(uint256 => address) public antToOwner;
  //TODO research workaround to use immutable in upgradeable contracts
  IEgg public eggs;
  uint256 public eggPrice;
  uint256 public antsCreated;
  mapping(uint256 => uint256) public antEggLaying;

  function initialize(address _eggs) public initializer {
    __ERC721_init('Crypto Ants', 'ANTS');
  ...
  //TODO what should happend when the value sent is grather thant the egg price but less thant double example 0.015
  ...
  function layEgg(uint256 _antId) external nonReentrant {
    if (antToOwner[_antId] != msg.sender) revert Unauthorized();
    if (block.timestamp - antEggLaying[_antId] <= 10 minutes) revert NeedToWait10Minutes();
    antEggLaying[_antId] = block.timestamp;
    eggs.mint(msg.sender, _getPseudoRandomNumber(20));
    if (_getProbabilityOfDying()) {
      _burn(_antId);
    }
  }
  ...
  /*
  EVM is deterministic so there is no way to achieve real randomness 
  because different nodes will produce different result and the transaction will not be verified correctly
  there are of-chain available solutions like chainlink vrf
  */
  function _getPseudoRandomNumber(uint256 percentage) private view returns (uint256 randomKeccak) {
    randomKeccak = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender))) % percentage;
  }

  function _getProbabilityOfDying() private view returns (bool) {
    return _getPseudoRandomNumber(10) < 1;
  }
}
