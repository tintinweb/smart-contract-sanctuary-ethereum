...
contract ERC721A_EXTENSION is ERC721A, Ownable, Pausable {
	...
	function mint(uint256 _quantity) external callerIsUser payable whenNotPaused {
		...
		uint256 _currentMintedCount = _mintedCount[msg.sender];
		uint256 _needPayPrice = mintInfo.freeQuantity - _hasMinted >= _quantity
			? 0
			: (_quantity - (mintInfo.freeQuantity - _hasMinted < 0 ? 0 : mintInfo.freeQuantity - _hasMinted)) * mintInfo.mintPrice;

		require(_quantity > 0, "Invalid quantity");
		require(_hasMinted + _quantity <= mintInfo.maxSupply, "Exceed supply");
		require(
			_currentMintedCount + _quantity <= mintInfo.singleAddressLimitQuantity ||
				(mintInfo.ownerMintUnrestricted && msg.sender == owner()),
			"Exceed mint limit"
		);
		...
		_mintedCount[msg.sender] = _currentMintedCount + _quantity;
	}
