...
// File contracts/bonds/libraries/SelfPermitLib.sol
...
// File contracts/bonds/libraries/BondPriceLib.sol
...
library BondPriceLib {
    ...
    /// @param vyReserves virtual reserves for base asset
...
// File contracts/bonds/libraries/AccrualBondLib.sol
...
// File contracts/bonds/AccrualBondStorageV1.sol
...
// File contracts/bonds/AccrualBondsV1.sol
...
contract AccrualBondsV1 is AccrualBondStorageV1, Initializable, OwnableUpgradeable {
    ...
    /// @notice emitted when a bond is sold
    event BondSold(
        // bond creditor
        address indexed bonder,     
        // bond principal
        address indexed token,
        // amount of tokens used to purchase bond
        uint256 input, 
        // amount of output tokens obligated to user
        uint256 output
    );
    
    /// @notice emmitted when a bond is redeemed
    event BondRedeemed(
        // bond creditor
        address indexed bonder,
        // ERC721 token id
        uint256 indexed bondId,
        // amount of output tkens redeemed
        uint256 output
    );
    
    /// @notice emitted when revenue beneficiary is set
    event BeneficiarySet(
        // caller, presumably policy MS
        address indexed caller, 
        // newly recipeint of revenue
        address newBeneficiary
    );

    /// @notice emitted when control weight param is set
    event VirtualOutputReservesSet(
        // caller, presumably policy MS
        address indexed caller, 
        // newly set control weight
        uint256 newControlWeight
    );
    
    /// @notice emitted when quote pricing params are set
    event QuotePricingSet(
        // caller, presumably policy MS
        address indexed caller, 
        // bond principal
        address indexed token, 
        // virtual reserves paired against CNV
        uint256 virtualReserves, 
        // grow/decay halfLife
        uint256 halfLife, 
        // grow/decay target percent bips
        uint256 levelBips
    );
    ...
    function _purchaseBond(
        ...
        uint256 input,
        uint256 minOutput
    ) internal virtual returns (uint256 output) {
        
        // F6: CHECKS
        // Fetch quote price info from storage 
        BondPriceLib.QuotePriceInfo storage quote = quoteInfo[token]; 
        // If quote reserves are equal to zero, revert because of lack of liquidity
        ...
        if (output < minOutput || availableDebt < output) revert BAD_OUTPUT();

        // F6: EFFECTS
        // Transfer principal from sender -> beneficiary
        TransferHelper.safeTransferFrom(token, sender, beneficiary, input);
        // Increase cnvEmitted by amount output for rebasing
        unchecked { cnvEmitted += output; }
        // Sync virtual reserves and lastUpdate 
        quote.virtualReserves = adjustedReserves;
        // Increase totalDebt, cannot be greater than 
        // totalSupply which is checked for overflow
        unchecked { totalDebt += output; }
    ...
    function redeemBond(
        ...
        // T2 - Are events emitted for every storage mutating function?
        emit BondRedeemed(msg.sender, bid, output);
    }
    ...
    function setBeneficiary(
        ...
        // T2 - Are events emitted for every storage mutating function?
        emit BeneficiarySet(msg.sender, _newBeneficiary);
    }

    function modifyVirtualOutputReserves(
        uint256 cw
    ) external virtual onlyOwner {
        ...
        // T2 - Are events emitted for every storage mutating function?
        emit VirtualOutputReservesSet(msg.sender, cw);
    }

    function modifyQuotePricingBatch(
        ...
        for (uint256 i; i < length; ) {
            ...
            // T2 - Are events emitted for every storage mutating function?
            emit QuotePricingSet(msg.sender, tokens[i], virtualReserves[i], halfLives[i], levelBips[i]);
    ...
    function modifyQuotePricing(
        ...
        // T2 - Are events emitted for every storage mutating function?
        emit QuotePricingSet(msg.sender, token, virtualReserves, halfLife, levelBips);
    }
    ...
    function addQuoteAsset(
        ...
        require(quoteInfo[token].lastUpdate == 0, "!exists");  
        ...
        // T2 - Are events emitted for every storage mutating function?
        emit QuoteAssetAdded(msg.sender, token, virtualReserves, halfLife, levelBips);
    }
    ...
    function removeQuoteAsset(
        ...
        // T2 - Are events emitted for every storage mutating function?
        emit QuoteAssetRemoved(msg.sender, token);
    }
    ...
    function getSpotPrice(
        ...
        return FixedPointMathLib.fmul(
            1e18, 
            quoteInfo[token].virtualReserves, 
            IERC20Upgradeable(outputToken).balanceOf(address(this)) - totalDebt + vyReserves
        );
    }
