...
abstract contract UniStakingTokensStorage {
    ...
    constructor(IERC20 rewardsToken_, IERC20 stakingToken_,uint256 claimingFeePercent_,address feeReceiver_) public {
    ...
    function _unstake(address account, uint256 amount) internal {
      ...
      _stakingToken.safeTransfer(account, NetAmount);
      _stakingToken.safeTransfer(_feeReceiver, fee);
      _feesPaid[account] = _feesPaid[account].add(fee);
      _balances[account] = _balances[account].sub(NetAmount);
...
contract Staking is TwoStageOwnable, UniStakingTokensStorage {
    ...
    event Unstaked(address indexed account, uint256 amount,uint256 rewardUnlockingTime,uint256 claimedRewards);
    ...
    ) public TwoStageOwnable(owner_) UniStakingTokensStorage(rewardsToken_, stakingToken_,claimingFeePercent_,feeReceiver_) {
    }
    ...
    function unstake(uint256 amount) public onlyPositiveAmount(amount) {
        ...
        _claim(sender, rewardOf(sender));
        emit Unstaked(sender, amount,rewardUnlockingTime_,rewardOf(sender));
    }
...
contract UniStakingSyntheticToken is Staking {
    ...
    constructor(
        ...
        address feeReceiver_,
        address owner_
    ) public Staking(rewardsToken_, stakingToken_, owner_,claimingFeePercent_,feeReceiver_) {
        name = name_;
        symbol = symbol_;
        decimals = decimals_;
    }
