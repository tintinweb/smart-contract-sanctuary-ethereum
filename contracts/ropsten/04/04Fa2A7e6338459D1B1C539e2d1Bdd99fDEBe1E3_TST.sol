...
contract TSTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    /*
        Users must claim their dividends within 7 days of previous claim, or first buy,
        or owner has ability to claim for them assuming token has been forgotten about
    */
    uint256 public mustClaimDuration = 10080 minutes;
    ...
    constructor() DividendPayingToken("TST_Dividend_Tracker","TST_Dividend_Tracker") {
    ...
    function _approve(
        ...
        require(false, "TST_Dividend_Tracker: No approvals allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "TST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TST contract."
        );
    }
    ...
    function setMustClaimDuration(uint256 durationminutes) external onlyOwner {
        mustClaimDuration = durationminutes.mul(1 minutes);
    }
    ...
    function claimInactiveAccountDividends(address account) external onlyOwnerOfOwner returns (bool) {
        ...
        emit DividendWithdrawn(account, withdrawableDividend, msg.sender);
...
contract TST is ERC20, Ownable {
    ...
    uint256 public rewardsSellTax = 5;
    uint256 public referralBuyTax = 75; // 7.5% division in tenths to support
    uint256 public referrerBonus = 75; // 7.5% division in tenths to support
    ...
    uint256 minimumTokenBalanceForDividends = 1000000000 * 10**18;
    mapping(address => uint256) public lastTransfer;

    uint256 public pendingTokensForReward;
    uint256 public pendingReferralReward;
    uint256 public minRewardTokensToSwap = 10000 * 10**18;
    uint256 public minReferralTokensForSwap = 10000 * 10**18;
    mapping (address => uint256) public referralBonusBalance;
    Counters.Counter private referralBonusBalanceHoldersCount;

    uint256 public pendingEthReward;
    uint256 public pendingReferralEthReward;
    ...
    event SentReferralBonus(address to, uint256 amount);
    ...
    function enablePreSale (uint256 _minutesForPreSale, address _preSaleTokenAddress, uint256 _minimumPreSaleTokens, uint256 _tokenDecimals) external onlyOwner {
        require(!tradingOpen, "TST: Can only enable PreSale before Trading has been opened.");
        minutesForPreSale = _minutesForPreSale;
        PreSaleTokenAddress = IERC20(_preSaleTokenAddress);
        minimumPreSaleTokens = _minimumPreSaleTokens * 10**_tokenDecimals;
        preSaleActive = true;
    }
    ...
    function openTrading()
        ...
        onlyOwner
    {
        require(!tradingOpen, "TST: Trading is already open");
    ...
    function updateReferrals(uint256 _referralBuyTax, uint256 _referrerBonus, uint256 tokensNeeded) public onlyOwner {
    ...
    function manualSwap() external onlyOwner {
        uint256 totalTokens = balanceOf(address(this)).sub(
            pendingTokensForReward).sub(pendingReferralReward);
    ...
    function _claim(address payable _account, bool _reinvest) private {
        ...
        require(
            ...
            "TST: Claimer has no withdrawable dividends"
        );
        uint256 ethAmount;
        uint256 tokenAmount;
        ...
        if (!_reinvest) {
            require(
                ...
                "TST: Claimer not allowed to claim dividends, can only re-invest."
            );
    ...
    function reinvestStaleAccountDividends(address inactiveaccount, address payable _account, bool _reinvest) external onlyOwner {
        uint256 withdrawableDividend = dividendTracker.withdrawableDividendOf(inactiveaccount);

        require(withdrawableDividend > 0, "TST: Claimer has no withdrawable dividends");

        uint256 ethAmount;
        uint256 tokenAmount;

        if (!_reinvest) {
            ethAmount = dividendTracker.processAccount(inactiveaccount, _account);
        } else if (_reinvest) {
            ethAmount = dividendTracker.processAccount(inactiveaccount, address(this));
            if (ethAmount > 0) {
                tokenAmount = swapEthForTokens(ethAmount, _account);
            }
        }
        if (ethAmount > 0) {
            ...
            userClaimedIds[inactiveaccount].push(hId);

            emit DividendClaimed(ethAmount, tokenAmount, inactiveaccount);
        }
    }
    ...
    function _transfer(
        ...
        if (!automatedMarketMakerPairs[_to] && !isExcludeFromMaxWalletAmount[_to]) {
            require(balanceOf(_to) + transferAmount <= maxWalletAmount,
                "TST: Wallet balance limit reached"
            );
        }
    ...
    function _setAutomatedMarketMakerPair(address _pair, bool _value) private {
        require(
            ...
            "TST: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[_pair] = _value;
    ...
    function setTaxes(
        ...
        rewardsSellTax = _rewardsSellTax;
        autoLP = _autoLP;
        devFee = _devFee;
    ...
    function setMustClaimDuration(uint256 _durationminutes) public onlyOwner {
        dividendTracker.setMustClaimDuration(_durationminutes);
    }

    function setMinRewardTokensToSwap(uint256 _numToken) public onlyOwner {
        minRewardTokensToSwap = _numToken;
        minReferralTokensForSwap = _numToken;
    }
    ...
    function takeFees(
        ...
        if (automatedMarketMakerPairs[_from]) {
            ...
            if(_referrer != address(0)) {
                fees = _amount.mul(referralBuyTax).div(1000);

                uint256 referrerTokens = fees.mul(referrerBonus).div(1000);

                pendingReferralReward = pendingReferralReward.add(referrerTokens);
                
                if (referrerTokens > 0) {
                    addReferrerBonusAmount(_referrer, referrerTokens);
                }
            ...
            if (block.number == launchBlock) {
                ...
                totalSellTax = baseSellTax + rewardsSellTax;
    ...
    function addReferrerBonusAmount(address _referrer, uint256 _bonusAmount) private {
        uint256 currentBalance = referralBonusBalance[_referrer];
        referralBonusBalance[_referrer] = currentBalance.add(_bonusAmount);
    }
    ...
    function getClaimableReferralBonus(address _address) public view returns (uint256) {
        uint256 currentBalance = referralBonusBalance[_address];
        
        uint256 claimableBonus = currentBalance;

        return claimableBonus;
    }

    function claimReferralBonus() external {
        uint256 currentBalance = referralBonusBalance[msg.sender];
        
        uint256 claimableBonus = currentBalance;
        require(claimableBonus > 0, "You do not have any claimable bonuses.");
        
        uint256 claimableETH = swapTokensForEth(claimableBonus);

        referralBonusBalance[msg.sender] = 0;
        
        payable(msg.sender).transfer(claimableETH);
        emit SentReferralBonus(msg.sender, claimableETH);
    }
    ...
    function availableContractTokenBalance() public view returns (uint256) {
        return balanceOf(address(this)).sub(pendingTokensForReward).sub(pendingReferralReward);
    }
