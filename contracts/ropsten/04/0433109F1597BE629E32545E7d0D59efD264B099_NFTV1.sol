...
contract NFTV1 is ERC2981, ERC721Tradable, Mintable {
    ...
    Counters.Counter public _numNFTMinted;
    // totalSupply
    ...
    function _createCard(
        ...
        _numNFTMinted.increment();
        _totalNFT.increment();

        //mint a nfting
        uint256 tokenId = _numNFTMinted.current();

        //mint token
        _mint(to_address, tokenId);

        //do events
        emit evtNFTCreated(to_address, tokenId);
    }
    ...
    function createCards(
        ...
        _createCard(to_address, name, uri, cost, grade, atk, hp, speed);
    }
    ...
    function unpack(
        ...
        require(
            names.length == amount &&
                ...
                atks.length == amount &&
                hps.length == amount &&
                speeds.length == amount,
            "ERC721: values length mismatch"
        );
