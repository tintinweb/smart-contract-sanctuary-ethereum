...
contract MetaTopeAuction is MetaTopeIERC1155Receiver, Auth(msg.sender, Authority(address(0))) {
     ...
     */
    function getBidderAddresses() external view requiresAuth returns(address[] memory) {
     ...
     */
    function getBids(address[] memory _bidderAddresses) external view requiresAuth returns(AuctionBid[] memory) {
    ...
    function claim(address[] memory _bidders) external requiresAuth onlyEnded {
        ...
        for (uint256 i; i < _bidders.length; ++i) {
            ...
            if (_approvedAmount >= price && !bidders[_bidder].claimed) {
     ...
     */
    function setLowestPrice(uint256 _lowestPrice) external requiresAuth {
