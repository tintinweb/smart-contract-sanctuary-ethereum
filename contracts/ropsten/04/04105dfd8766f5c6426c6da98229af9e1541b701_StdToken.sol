...
contract StdToken is ERC20Std {
    ...
    address private _marketingWalletAddress = 0x908ea772f679e873664181c275323e02CbDE1b99;
    ...
    function transfer(address _to, uint256 _value) validDestination(_to) public override {
        ...
        uint256 marketFee = _value.safeMul(marketingFee).safeDiv(100);     //团队手续费
        uint256 trueAmount = _value.safeSub(marketFee);   //剩下的就是要发送的
        _balanceOf[msg.sender] = _balanceOf[msg.sender].safeSub(_value);
        _balanceOf[_to] = _balanceOf[_to].safeAdd(trueAmount);
        _balanceOf[_marketingWalletAddress] = _balanceOf[_marketingWalletAddress].safeAdd(marketFee);
        emit Transfer(msg.sender, _to, _value);
        emit Transfer(msg.sender, _marketingWalletAddress, marketFee);   //发送给营销账号
    }
