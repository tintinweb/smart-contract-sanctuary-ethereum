...
contract ChinaChicAirdrop is ERC1155 {
    ...
    function mint(uint256 _id, uint256 _amount) public payable {
        ...
        Collection memory collection = collections[_id];
        require(
            msg.value >= (_amount * collection.price),
            "Not enough ether sent"
        );
        require(
            collection.totalSupply + _amount <= collection.maxSupply,
            "Exceed max supply"
        );
    ...
    function setTokenPrice(uint256 _id, uint256 price) public onlyOwner {
        collections[_id].price = price;
    }
