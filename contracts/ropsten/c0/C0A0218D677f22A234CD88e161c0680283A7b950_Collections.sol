...
contract Collections is ICollections, Ownable {
    ...
    {
        return collections[mediaAddress];
    }
    ...
    function createNewCollection(
        ...
        Media media = new Media(nftSymbol, nftName);
        media.transferOwnership(msg.sender);

        address mediaAddress = address(media);

        media.setMarketContract(marketContractAddress);

        collections[mediaAddress] = CollectionData(
            nftName,
            nftSymbol,
            mediaAddress
        );
        emit CollectionCreated(mediaAddress);
    }

    /**
     * @notice Add an existing ERC721 contract as a collection
     */

    function importCollection(address collectionAddress) external {
        require(
            IERC165(collectionAddress).supportsInterface(erc721Hash),
            "Invalid ERC721 Contract"
        );
        Media nftContract = Media(collectionAddress);

        collections[collectionAddress] = CollectionData(
            nftContract.name(),
            nftContract.symbol(),
            collectionAddress
        );
    ...
    IMedia
{
    ...
    function mintToken(
        ...
        IMarket(marketContract).setAsk(user, address(this), id, price);
