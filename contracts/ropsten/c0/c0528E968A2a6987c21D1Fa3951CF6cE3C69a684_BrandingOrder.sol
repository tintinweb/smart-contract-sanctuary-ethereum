...
contract BrandingOrder is Adminer {
  ...
  function verifyOrder(Order calldata order) public pure {
    ...
    require(signatureOwner == info.owner, "Order verification failed");
  }
  ...
  function fulfillOrder(Order calldata order, uint256 amount) external payable {
    ...
    if (info.orderType == 0) {
      ERC1155(info.contractAddress).safeTransferFrom(owner, msg.sender, info.tokenId, amount, "Branding NFT trade");
