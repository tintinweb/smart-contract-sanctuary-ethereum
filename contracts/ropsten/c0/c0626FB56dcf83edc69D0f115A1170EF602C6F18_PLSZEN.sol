...
contract PLSZEN is Context, IERC20, Ownable {
    ...
    address private _charityWalletAddress = 0xE69Ac38Cd6DA0EA9a540B47399C430131216Ced7;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 880000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 private _tFeeTotal;

    string private _name = "Pulse Zen";
    string private _symbol = "PLSZEN";
    uint8 private _decimals = 9;

    uint256 public _taxFee = 20;                              // 2% Reflection
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _charityFee = 2;                           // 0.2% charity
    uint256 private _previousCharityFee = _charityFee;

    uint256 public _burnFee = 100000;                         // 0.0001 token burn  Amount
    uint256 private _previousBurnFee = _burnFee;

    uint256 public _walletLimit = (_tTotal * 30) / 100;      // 30% Wallet Limit 
    ...
    function setRefFeePercent(uint256 taxFee) external onlyOwner() {
        _taxFee = taxFee;
    }
    ...
    function setBurnAmount(uint256 burnAmount) external onlyOwner() {
        _burnFee = burnAmount;
    }
    ...
    function setWalletLimitExempt(address _adr, bool _status) external onlyOwner() {
    ...
    function setCharityWallet(address _adr) external onlyOwner() {
        _charityWalletAddress = _adr;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tBurn = calculateBurnFee();
    ...
    function calculateBurnFee() private view returns (uint256) {
        return _burnFee;
    }
    ...
    function _transfer(
        ...
        if(checkWalletLimit && !isWalletLimitExempt[to]) {
            ...
            require(recipientBalance.add(amount) <= _walletLimit,"Error: MAX WALLET LIMIT EXCEEDED!!");
        }
