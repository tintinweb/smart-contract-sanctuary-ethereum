    ...
    function _claim(address owner, uint[] calldata tokenIds, bool _unstake) internal{
        ...
        uint totalEarned;
         for(uint i = 0; i < tokenIds.length; i++){
             ...
             require(thisStake.owner == owner, " not the owner, you cannot claim the award");
         ...
         function totalStakedByOwner(address owner) external view returns(uint[] memory){
             uint totalSuplly = nft.totalSupply();
             uint[] memory tmp = new uint[](totalSuplly);
             uint index= 0;

             for(uint i = 0 ; i < totalSuplly ; i++){
