...
contract GossamerSerum is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    constructor(uint256 MaxCollection) ERC721("Gossamer Serum", "GSER")  {
    ...
    @params adds [] - array of Addresses to receive NFT
    Can be called only by owner
     */
    function airdropMint(uint numberOfTokens, address[] memory adds) external onlyOwner {
        require(totalSupply().add(numberOfTokens) <= MAX_COLLECTION, "Mint would exceed max supply");
        require(numberOfTokens!=0 && adds.length == numberOfTokens, "Send correct address array");
        for(uint256 i=0; i<numberOfTokens; i++){
            require(checkAirdrop(adds[i]) == false, "One of the address already processed");
            airdropDone[adds[i]] = true;
            uint mintIndex = totalSupply().add(1);
            if (totalSupply() < MAX_COLLECTION) {
                _safeMint(adds[i], mintIndex);
            }
        }
    }
