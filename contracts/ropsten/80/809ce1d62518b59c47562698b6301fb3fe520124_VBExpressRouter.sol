...
interface IVBExpressFactory {
    event ExpressPairCreated(address indexed token0, address indexed token1, uint24 k0, uint24 k1, address indexed pair, uint);
...
interface IVBExpressPair{
    ...
    function totalPriorities() external view returns (uint256);
...
contract VBExpressRouter is Context, IVBExpressRouter {
    ...
    function withdrawToken(
        ...
        uint256 totalPriorities = IVBExpressPair(pair).totalPriorities();
        uint256 lockedLiquidity = assiged*kA/kB + liquidity*(_reserve1- totalPriorities)*kA/(kB*totalSupply);
...
library VBExpressLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB, uint24 kA, uint24 kB) public pure returns (address pair) {
        ...
        pair = address(uint160(uint256(keccak256(abi.encodePacked(
                ...
                hex"38a772e70924c7ba10b477d18113d4105e1e76eac0d98657849fc205d4f9f2b7" // init code hash
            )))));
    }
