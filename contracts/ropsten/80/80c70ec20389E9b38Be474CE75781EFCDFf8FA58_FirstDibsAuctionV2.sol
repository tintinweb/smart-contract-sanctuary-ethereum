    ...
    FirstDibsERC2771Context
{
    ...
    function endAuction(uint256 _auctionId) external nonReentrant {
        ...
        if (nftCreator == address(0)) {
            try IOwnable(auctions[_auctionId].nftAddress).owner() returns (address owner) {
                nftCreator = owner;
            } catch {}
        }

        uint256 royaltyAmount = 0;
        if (nftCreator != auctions[_auctionId].tokenOwner && nftCreator != address(0)) {
        ...
        _sendFunds(auctions[_auctionId].fundsRecipient, sellerFee);
