...
contract Minter is Initializable, UUPSUpgradeable, OwnableUpgradeable, PullPaymentUpgradeable {
  ...
  function mint(uint256 amount_, address inviter_, bytes memory inviterSignature_, bytes32 inviterSignatureHash_) public payable returns(bool) {
    ...
    uint256 amountNeeded = mintPrice;
    if(inviter_ != address(0)) {
      inviterTokenBalance = _token.balanceOf(inviter_);
      require(SignatureCheckerUpgradeable.isValidSignatureNow(inviter_, inviterSignatureHash_, inviterSignature_), "The inviter signature is invalid.");
    ...
    if(inviterTokenBalance > 2 && inviter_ != address(0)) {
      sentToInviter = 0.03 ether * amount_;
      payable(inviter_).transfer(sentToInviter);
    } else if(inviterTokenBalance <= 2 && inviter_ != address(0)) {
