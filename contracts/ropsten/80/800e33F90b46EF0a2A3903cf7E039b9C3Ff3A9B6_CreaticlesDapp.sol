...
contract CreaticlesDapp is ContextUpgradeable {
    ...
    */
    ...
    function addToken(address _token, string memory _symbol) external isAdmin() {
        uint256 i;
        ...
        if (i == tokenCount) {
            tokenWhitelist[tokenCount] = WhitelistEntry(_token, _symbol);
            tokenCount += 1;
        }
    }
    ...
    function addCreaticles(address _token, string memory _symbol) external isAdmin() {
        string memory empty = "";
        if (keccak256(bytes(_symbol)) != keccak256(bytes(empty)))
            tokenWhitelist[0] = WhitelistEntry(_token, _symbol);
        else
            tokenWhitelist[0] = WhitelistEntry(_token, "CRE8");
    }
