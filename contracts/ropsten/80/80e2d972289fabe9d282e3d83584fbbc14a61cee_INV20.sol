...
pragma solidity ^0.8.12;
    ...
    contract Initializable { 
        ...
        require(!inited, "already inited"); _; inited = true; 
    } 
} 
...
contract NativeMetaTransaction is EIP712Base { 
            using SafeMath for uint256; bytes32 private constant META_TRANSACTION_TYPEHASH = keccak256( bytes( "MetaTransaction(uint256 nonce,address from,bytes functionSignature)" ) ); 
...
function msgSender() internal view 
    returns (address payable sender) { 
        if (msg.sender == address(this)) { bytes memory array = msg.data; uint256 index = msg.data.length; assembly { 
...
function _setupContractId(string memory contractId) internal { 
    ...
    require( 
            hasRole(role, _msgSender()), _revertMsg ); _; 
    } 
} 
