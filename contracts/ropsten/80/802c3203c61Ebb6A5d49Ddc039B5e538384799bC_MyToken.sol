...
contract MyToken is Stakeable, ERC20 {
    ...
    * @notice withdrawStake is used to withdraw stakes from the account holder
    ...
    function withdrawStake() public 
    {
        ...
        LPOXLP.approve(address(this), original_LP_staked);
        LPOXLP.transferFrom(address(this), msg.sender, original_LP_staked);
...
contract Stakeable {
    ...
    uint256 internal exitDaysPassedTreshold = 5 days;
  ...
  /**
    ...
    */
     function _withdrawStake() internal returns(uint256){
        ...
        uint256 reward = calculateStakeReward(current_stake);
         ...
         if(current_stake.amount == 0){
             ...
             stakeholders[user_index].stake.amount = current_stake.amount;
     ...
     function _getOriginalLPStaked() internal returns(uint256){
         // Grab user_index which is the index to use to grab the Stake[]
        uint256 user_index = stakes[msg.sender];
        Stake memory current_stake = stakeholders[user_index].stake;
        ...
        {
            uint256 calculatedValue =  ((current_stake.LPAmount ) * ((100-earlyExitFee)/100));
            emit IsEarlyWithDrawal(isEarlyWithdrawal, calculatedValue);
            return calculatedValue;
        }
        else
        {
            return stakeholders[user_index].stake.LPAmount;
        }
     }
    ...
    * @notice Staked event is triggered whenever a user stakes tokens, address is indexed to make it filterable
     */
     ...
     event IsEarlyWithDrawal(bool isEarly, uint256 LPAmount);
}
