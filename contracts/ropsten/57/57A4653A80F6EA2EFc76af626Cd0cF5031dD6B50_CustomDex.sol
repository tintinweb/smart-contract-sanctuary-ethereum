...
contract CustomDex {
    ...
    function swapEthToToken(string memory tokenName) public payable returns (uint256) {
        ...
        uint256 outputValue = (inputValue / 100000000000000) * 10 ** 18; // 0.0001 eth
    ...
    function swapTokenToEth(string memory tokenName, uint256 _amount) public returns (uint256) {
        uint256 outputValue = (_amount * 100000000000000) * 10 ** 18; // 0.0001 eth;
        require(address(this).balance >= outputValue, "Dex is running low on balance.");
    ...
    function swapTokenToToken(string memory srcTokenName, string memory destTokenName, uint256 _amount) public {
        require(tokenInstanceMap[srcTokenName].transferFrom(msg.sender, address(this), _amount * 10 ** 18));
        require(tokenInstanceMap[destTokenName].transfer(msg.sender, _amount * 10 ** 18));
    }
}