...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable{
    ...
    uint256 private _contractStartingLiquidity = _presaleTokenLimit * 3 / 2; //TEMP 
    uint256 private _presaleTokensSold = 0;
    uint256 private minInvestment = 100 * 10**_decimals; //TEMP 
    
    address private _mktDevAddress;
    address private _treasuryAddress;

    bool private postPresaleComplete = false;
    ...
    uint256 lockPeriod = 1654041600; //TEMP 1654041600; //epoch time June 1 12AM

    uint256 openingTime = 1649809079; //TEMP 1650427200; //epoch time april 20 12AM
    uint256 closingTime0 = openingTime + 86400*1/2; //TEMP 1650427200; //epoch time april 20 12AM
    uint256 closingTime1 = closingTime0 + 86400*1/2; //TEMP 1650427200; //epoch time april 20 12AM
    ...
    modifier onlyWhileOpen {
        ...
        require(block.timestamp <= closingTime1, "Token purchase must be within presale period");
        _;
    }
    ...
    constructor(address payable mktDevAddress_, address payable treasuryAddress_) payable {
        ...
        _transferToken(owner(),address(this),_contractStartingLiquidity,false);
    }
    ...
    function _transfer(
        ...
        require((block.timestamp > closingTime1) || ((to == owner()) || (from == address(this)) || (to == address(this))),"Metaland tokens may not be transacted until the presale period ends");
        if (from == owner()) require(block.timestamp > lockPeriod, "Owner wallet MLAND is locked");

        if ((postPresaleComplete == false) && (block.timestamp > closingTime1)){
            postPresaleComplete = true;
            postPresale();
        } else if (block.timestamp > closingTime1) {
    ...
    function postPresale() public onlyOwner {
        uint256 half = address(this).balance / 2;
        uint256 half2 = address(this).balance - half;

        uint256 initBalance = balanceOf(address(this));
        uint256 swapBalance = _presaleRate*half;
        uint256 otherBalance = initBalance - swapBalance;

        addLiquidity(swapBalance, half);
        drainEther(half2);
        drainTokens(otherBalance);
    }
    ...
    function drainTokens(uint256 tokenAmount) private {
        _transferToken(address(this), owner(), tokenAmount, false);
    }

    function drainEther(uint256 etherAmount) private {
        (bool sent, bytes memory data) = owner().call{value: etherAmount}("");
        require(sent, "Failed to send Ether");
    }

    function getPresaleValues() public view returns(uint256,uint256,uint256,uint256,uint256,uint256,uint256) {
        return (_presaleTokenLimit,_presaleRate,_presaleTokensSold,openingTime,minInvestment,closingTime0,closingTime1);
    }

    function buyTokens() public payable onlyWhileOpen {
        if (block.timestamp < closingTime0) require(_isWhitelisted[_msgSender()],"Buyer address must be whitelisted during this stage pf presale.");
