...
//	event StakeLog(string name,address addr,uint256 time,uint256 amount, uint256 frozen,uint256 unlock);
	event TxLog(uint256 sale,address addr,uint256 mount,address vault,uint256 blk);
...
*/
    ...
    {                                                                                                                                                                                   
        ...
        Sale[i].minimal = minimal * 10**decimal;                                                                                                                                                      
    }                           
    ...
    mapping(uint256 => mapping(address => mapping(uint256 => txs)))public Txs;
	...
	Txs[sale][addr][TxsCount[sale][addr]].blk = block.number;
	Txs[sale][addr][TxsCount[sale][addr]].sale = sale;
	Txs[sale][addr][TxsCount[sale][addr]].amount = amount;
	Txs[sale][addr][TxsCount[sale][addr]].addr = addr;

	TxsCountGlob[sale] = TxsCountGlob[sale].add(1);
	Txs[sale][addr][TxsCountGlob[sale]].blk = block.number;
	Txs[sale][addr][TxsCountGlob[sale]].sale = sale;
	Txs[sale][addr][TxsCountGlob[sale]].amount = amount;
	Txs[sale][addr][TxsCountGlob[sale]].addr = addr;

	payable(Sale[sale].vault).transfer(amount);
	emit TxLog(sale,addr,amount,Sale[sale].vault,block.number);
        }
