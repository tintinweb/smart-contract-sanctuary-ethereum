...
contract CNSController {
    ...
    mapping(address => subdomain[]) public subdomains;
     ...
     * Get Domain Owner.
     */
    function getDomainOwner(bytes32 _domain) public view returns (address) {
     ...
     * Get Domain Token Id.
     */
    function getDomainTokenId(bytes32 _domain) public view returns (uint256) {
    ...
    function setSubdomain(
        ...
        _setSubdomain(_domain, _node, label, _subnode);
    }
    ...
    function _setSubdomain(
        ...
        ens.setSubnodeRecord(_node, label, address(this), resolverAddress, 0);
        resolver.setAddr(_subnode, msg.sender);
        subdomain memory _subdomain = subdomain({
            domain: _domain,
            subdomain: label
        });
        subdomains[msg.sender].push(_subdomain);
    }
}
