...
contract  FeeRateControlled is Controlled{
    ...
    function setTransferFeeMin(uint256 _transferFeeMin) public onlyOwner returns (bool success){
        require(_transferFeeMin>=0 && _transferFeeMin<transferFeeMax);
        transferFeeMin = _transferFeeMin;
        return true;
    }

    function setTransferFeeMax(uint256 _transferFeeMax) public onlyOwner returns (bool success){
        require(_transferFeeMax>transferFeeMin);
...
contract HanYinToken is StableToken{
    ...
    function HanYinToken() public {
        ...
        setDefaultTransferRate(200); // default transfer rate is 2%
        setTransferFeeMin(0);
        setTransferFeeMax(10 ** 8);
        //enableTransfer(true);
    }
