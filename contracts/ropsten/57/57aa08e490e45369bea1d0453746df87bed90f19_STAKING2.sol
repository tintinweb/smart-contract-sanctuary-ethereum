...
interface UserStorageInterface {
    function getUserType(address userAddress)
        external
        view
...
contract GETS is ERC20, BaseContract {
    ...
    uint256 private tokensPerEther = 4000;
    ...
    function buyGetsToken() external payable {
        ...
        uint256 tokensToBuy = msg.value * tokensPerEther; //no.of token need to buy is depend on calculation
...
contract STAKING2 is Ownable {
    ...
    function stake(uint256 _amount) public {
        //check user balance
        require(
            _amount <= GETS(stakingToken).balanceOf(msg.sender),
            "no amount to stake "
        );
    ...
    function AmountUstaked() public view returns (uint256) {
        return AvailableForClaim[msg.sender];
    }
    ...
    function Claim() public {
        ...
        GETS(stakingToken).transferPrice(
            primaryWallet,
            msg.sender,
            AvailableForClaim[msg.sender]
        );
    ...
    function UserTotallockTime(address addr ) public view returns (uint256) {
