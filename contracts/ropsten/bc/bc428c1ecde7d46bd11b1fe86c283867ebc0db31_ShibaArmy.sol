...
contract ShibaArmy is ERC721, Ownable {
    ...
    // TEST FUNCTION REMOVE BEFORE DEPLOY
    function accelerateLaunchTime(uint256 _days) public {
        launchtime -= (_days) * 1 days;
    }

    // TEST FUNCTION REMOVE BEFORE DEPLOY
    function decelerateLaunchTime(uint256 _days) public {
        launchtime += (_days) * 1 days;
    ...
    function testDaysSinceStarted() public view returns (uint256) {
        uint256 daysSinceSaleStarted = (block.timestamp - launchtime)  / 60 / 60 / 24;
        return daysSinceSaleStarted;
    }
    ...
    // TESTING VERSION, ADD ONLY OWNER
