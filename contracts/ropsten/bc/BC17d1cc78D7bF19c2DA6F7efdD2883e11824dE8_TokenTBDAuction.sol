...
contract TokenTBDAuction is Ownable {
    uint8 public constant PLATINUM_SUPPLY = 100;
    uint16 private constant SECONDS_IN_HOUR = 3600;
    uint8 private constant DEFAULT_INTERVAL_HOURS = 12;
    ...
    function placeBid(uint8 tokenID, uint256 bidAmount)
        external
        payable
        notLocked
    {
        ...
        if (currentTopBid.bidAmount == 0) {
            require(latestBid.bidAmount >= minimumBid, "INSUFFICIENT_ETH");
        } else {
            require(
    ...
    function endAuction(uint8 tokenId) external {
        ...
        require(
            currentTopBids[tokenId].wallet != address(0),
            "NULL_WALLET_ADDRESS"
        );
    ...
    function initializeTimestamps(
        ...
        for (uint8 i = 0; i < PLATINUM_SUPPLY; i++) {
...
// TODO: change contract and token name
contract TokenTBD is ERC721Enumerable, Ownable {
    ...
    function mint(
        ...
        bytes32 nonce,
        uint16 silverQuantity,
        uint16 goldQuantity) external payable {
        require(publicSaleLive && !presaleLive && !earlyBirdSaleLive, "SALE_CLOSED");
        require(mintedSilver + silverQuantity <= silverSupply, "OUT_OF_STOCK_SILVER");
        require(mintedGold + goldQuantity <= goldSupply, "OUT_OF_STOCK_GOLD");
        require(silverQuantity + goldQuantity <= maxMintsPerTx, "EXCEED_QUANTITY_LIMIT");
        require(msg.value >= (SILVER_PRICE * silverQuantity + GOLD_PRICE * goldQuantity),
            "INSUFFICIENT_ETH");
        require(matchAddresSigner(hash, signature), "UNAUTHORIZED_MINT");
        require(!nonces[nonce], "HASH_USED");
        require(hashTransaction(msg.sender, silverQuantity + goldQuantity, nonce) == hash, "HASH_FAIL");

        if (silverQuantity > 0) {
            mintSilverQuantity(silverQuantity);
        }
        if (goldQuantity > 0) {
            mintGoldQuantity(goldQuantity);
        }
    ...
    function mintEarlyBird(
        uint16 silverQuantity, uint16 goldQuantity) external payable {
        require(earlyBirdSaleLive && !publicSaleLive && !presaleLive, "EARLY_BIRD_SALE_CLOSED");
        require(earlyBirdSaleList[msg.sender], "UNAUTHORIZED");
        require(mintedSilver + silverQuantity <= silverSupply, "OUT_OF_STOCK_SILVER");
        require(mintedGold + goldQuantity <= goldSupply, "OUT_OF_STOCK_GOLD");
        require(
            earlyBirdSalePurchasesSilver[msg.sender] + silverQuantity +
            earlyBirdSalePurchasesGold[msg.sender] + goldQuantity
            <= maxMintsPerTx,
    ...
    function mintPresale(
        ...
        require(presaleLive && !publicSaleLive && !earlyBirdSaleLive, "PRESALE_CLOSED");
        require(presaleList[msg.sender], "UNAUTHORIZED");
        require(mintedSilver + silverQuantity <= silverSupply, "OUT_OF_STOCK_SILVER");
        require(mintedGold + goldQuantity <= goldSupply, "OUT_OF_STOCK_GOLD");
        require(
            presaleReservedSilver[msg.sender] == silverQuantity &&
            presaleReservedGold[msg.sender] == goldQuantity,
            "QUANTITY_MUST_BE_EQUAL_TO_RESERVED");
        require(
            presalePurchasesSilver[msg.sender] + silverQuantity +
            presalePurchasesGold[msg.sender] + goldQuantity <= maxMintsPerTx,
            "EXCEED_PRESALE_PERSONAL_LIMIT");
        require(msg.value >= (SILVER_PRICE * silverQuantity + GOLD_PRICE * goldQuantity),
            "INSUFFICIENT_ETH");

        presalePurchasesSilver[msg.sender] += silverQuantity;
        presalePurchasesGold[msg.sender] += goldQuantity;
    ...
    function mintSilverQuantity(uint16 quantity) private {
        for (uint16 i = 0; i < quantity; i++) {
            _safeMint(msg.sender, mintedSilver + goldSupply + PLATINUM_SUPPLY + 1 + i);
        }
        mintedSilver += quantity;
    }

    function mintGoldQuantity(uint16 quantity) private {
        for (uint16 i = 0; i < quantity; i++) {
            _safeMint(msg.sender, mintedGold + PLATINUM_SUPPLY + 1 + i);
        }
        mintedGold += quantity;
    }
    ...
    function giftSilver(address[] calldata receivers) external onlyOwner {
        require(mintedSilver + receivers.length <= silverSupply, "OUT_OF_STOCK");
        ...
        for (uint16 i = 0; i < receivers.length; i++) {
            _safeMint(receivers[i], mintedSilver + goldSupply + PLATINUM_SUPPLY + 1 + i);
        }
    ...
    function giftGold(address[] calldata receivers) external onlyOwner {
        ...
        for (uint16 i = 0; i < receivers.length; i++) {
            _safeMint(receivers[i], mintedGold + PLATINUM_SUPPLY + 1 + i);
        }
    ...
    function addToPresale(
        ...
        uint256[] calldata reservedSilver,
        uint256[] calldata reservedGold) external onlyOwner {
        require(entries.length == reservedSilver.length && 
            reservedSilver.length == reservedGold.length, "UNEQUAL_LENGTH");
        for (uint256 i = 0; i < entries.length; i++) {
            address entry = entries[i];
            uint256 silverReserve = reservedSilver[i];
            uint256 goldReserve = reservedGold[i];
            require(silverReserve > 0 || goldReserve > 0, "RESERVE_MUST_BE_GREATER_THAN_ZERO");
            require(silverReserve + goldReserve <= maxMintsPerTx, "RESERVE_OVER_LIMIT");
            require(entry != address(0), "NULL_ADDRESS");
            require(!presaleList[entry], "DUPLICATE_ENTRY");

            presaleList[entry] = true;
            if (silverReserve > 0) {
                presaleReservedSilver[entry] = silverReserve;
            }
            if (goldReserve > 0) {
                presaleReservedGold[entry] = goldReserve;
            }
        }
    }

    function removeFromEarlyBirdSaleList(address[] calldata entries) external onlyOwner {
        for (uint256 i = 0; i < entries.length; i++) {
            address entry = entries[i];
            require(entry != address(0), "NULL_ADDRESS");
    ...
    function assignAuctionContractAddress(address _auctionAddress) external onlyOwner {
    ...
    // NFT meta functions
    // ------------------------------------------------------
    function setBaseTokenURI(string calldata uri) external onlyOwner metadataNotLocked {
        baseTokenURI = uri;
    }

    function setProvenanceHash(string calldata hash) external onlyOwner metadataNotLocked {
        proof = hash;
    }
     ...
     */
    function emergencySetStartingIndexBlock() external onlyOwner metadataNotLocked {
