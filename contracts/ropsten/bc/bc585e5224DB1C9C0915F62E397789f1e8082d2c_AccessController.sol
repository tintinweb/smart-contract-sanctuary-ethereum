...
contract AccessController is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable, EIP712Upgradeable, ERC721VotesUpgradeable, AccessControlUpgradeable {
    ...
    modifier isValidMerkleProof(address to, bytes32[] calldata merkleProof) 
    {
        require(MerkleProofUpgradeable.verify(
                    merkleProof,
                    merkleRoot,
                    keccak256(abi.encodePacked(to))), "AccessController: Address not in whitelist");
        _;
    }

    function initialize(string memory name, string memory symbol, bytes32 _merkleRoot) 
    public 
    initializer 
    {
        ...
        super._transferOwnership(tx.origin);
    }
    ...
    function whiteListMint(address to, bytes32[] calldata _merkleProof)
        public
        isValidMerkleProof(to, _merkleProof)
    {
        require(balanceOf(to) == 0, "AccesControl: Address has already claimed");
        _safeMint(to);
    }
}
