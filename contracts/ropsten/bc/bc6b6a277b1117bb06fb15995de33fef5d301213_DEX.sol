...
contract DEX {
    ...
    event Vendido(uint256 quantidadeParaVender);

    constructor() {
        token = new StakingToken_Exemplo();
    }
   
   event Evento_Comprar( // gera evento de venda
       address user,
       uint etherAmount,
       uint time
   );
      event Evento_vender( //gera evento de venda
       address user,
       uint etherAmount,
       uint time
   );
    ...
    function comprar() payable public {
        ...
        emit Evento_Comprar(msg.sender, msg.value, block.timestamp); //chamamos a funcao Evento comprar
    }
    ...
    function vender(uint256 quantidadeParaVender) public {
        ...
        emit Evento_vender(msg.sender, quantidadeParaVender, block.timestamp); //chamamos a funcao Evento comprar
    }
...
contract StakingToken_Exemplo is ERC20, Ownable {    
   /**
     ...
     */
    uint256 totallSupply = 10000 * (10**uint256(decimals()));
    uint256 initialSupply = 1000 * (10**uint256(decimals()));

    /**
     * criacao do asser onde o supply sempre deve ser 10000 e o suppl inicial 1000      
     */
    function Teste_Assert_Supply() public view{
        assert(totallSupply == 10000);
    }

     function Teste_Assert_SupplyInicial() public view{
        assert(initialSupply == 1000);
    }


    constructor() ERC20("SmartCoinToken", "SMART") {
