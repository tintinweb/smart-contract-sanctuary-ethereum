...
contract LeagueOfTitans is Context, IERC20Upgradeable {
    ...
    uint256 public _liquidityRatio = 500;
    ...
    address public LIQUIDITY = 0x000000000000000000000000000000000000dEaD;

    address payable private _marketingWallet = payable(0xB22eF2918F98aF97eA04e7b78B1E10d8fE7FF555);//ETH Mainnet
    ...
    uint256 private _liqAddStatus = 50000000;
    ...
    function intializeContract() external onlyOwner {
        ...
        _maxWalletSize = (_tTotal * 1) / 10000;
        maxWalletSizeUI = (startingSupply * 1) / 10000;
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint256 liquidity, uint256 marketing) external onlyOwner {
        require(liquidity <= maxLiquidityFee
                && marketing <= maxMarketingFee
                );
        require(liquidity + marketing <= maxLiquidityFee + maxMarketingFee);
        _buyLiquidityFee = liquidity;
        _buyMarketingFee = marketing;
    }

    function setTaxesSell(uint256 liquidity, uint256 marketing) external onlyOwner {
        require(liquidity <= maxLiquidityFee
                && marketing <= maxMarketingFee
                );
        require(liquidity + marketing <= maxLiquidityFee + maxMarketingFee);
    ...
    function setTaxesTransfer(uint256 liquidity, uint256 marketing) external onlyOwner {
        require(liquidity <= maxLiquidityFee
                && marketing <= maxMarketingFee
                );
        require(liquidity + marketing <= maxLiquidityFee + maxMarketingFee);
