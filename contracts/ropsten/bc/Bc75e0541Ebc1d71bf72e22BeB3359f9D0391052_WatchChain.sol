...
contract WatchChain is ERC1155Holder,
    ...
    uint256 constant private _MAX_SHARE_PRICE=10000000;
    uint256 constant private _SHARES_AMOUNT_STEP=100;
    ...
    event ProposalCreated(uint256 watchId,uint256 proposalId,address proposer,uint256 price);
    ...
    event ProposalAccepted(uint256 watchId,uint256 proposalId,uint256 price,address proposer);
    ...
    event EditWatch(uint256 watchId,uint256 price,bytes32 ref,uint256 startTime);
    ...
    uint256 public proposalFeeBP=35;//0.35%
    ...
    mapping(address=>KYC) public kycInfo;
    uint256 public quoteTokenDelimiter;

    modifier status(uint256 watchId,Status st) {
        require(watchStatus(watchId)==st,
        "disallowed with the current status");
        _;
    }
    ...
    modifier onlyKnownYourCustomer() {
        if(kycSigner!=address(0)){
            require(kycInfo[msg.sender].verification,
    ...
    constructor(address _quoteToken,address _feeReceiverAddress,address _kycSigner) {
    ...
    function checkOrderError(
        ...
        uint256 shares) checkMultiplicity(shares) external view returns(OrderError){

        bool kyc=(kycInfo[order.maker].verification==true || kycSigner==address(0));
        ...
        if (remainingOrderAmount==_ORDER_NEVER_FILLED) {
            ...
            uint256 maxShares=watches[order.watchId].price/quoteTokenDelimiter;
            if(
            ...
            unchecked { remainingOrderAmount -= 1; }
        }
        ...
        bool allowance=(order.orderType==0 ? 
            quoteToken.allowance(order.maker, address(this)) >= (remainingOrderAmount * order.price)
            : 
        ...
        if(order.orderType==0){
            ...
            if(balanceQuote<(shares>0 ? (shares*order.price):(remainingOrderAmount*order.price))){
                return OrderError.AmountExceedsBalance;
            }
        }else{
            uint256 balanceShares=erc1155.balanceOf(order.maker, order.watchId);
     ...
     * @param shares NFT-share desired selling amount(for SellLimit order) or buying amount(for BuyLimit order)
    ...
    function fillOrder(
        ...
        "filling own order disallowed");
 
        require(
            kycInfo[order.maker].verification==true 
            || 
            kycSigner==address(0),
        ...
        require(remainingOrderAmount != _ORDER_CLOSED, 
        "order closed");
        ...
        uint256 fee = quoteTokenAmount*feeBP/10000;
        ...
        if(order.orderType==BuyLimit){
            erc1155.safeTransferFrom(msg.sender, order.maker, order.watchId, shares, "");
            if(fee>0){
                quoteToken.safeTransferFrom(order.maker,feeReceiverAddress,fee);
                unchecked { quoteTokenAmount -= fee; }
            }
            quoteToken.safeTransferFrom(order.maker,msg.sender,quoteTokenAmount);

        }else if(order.orderType==SellLimit){
            erc1155.safeTransferFrom(order.maker, msg.sender, order.watchId, shares, "");
            if(fee>0){
                quoteToken.safeTransferFrom(msg.sender,feeReceiverAddress,fee);
            }
            quoteToken.safeTransferFrom(msg.sender,order.maker,quoteTokenAmount);
        }else{
    ...
    /**
     * @dev set trading fee
     * @param  _feeBP - 100(1%) maximum
     */
    function setFee(uint256 _feeBP) external onlyOwner{
        require(_feeBP<=100,"fee is too high");
        feeBP=_feeBP;
        emit NewFee(feeBP);
    }
    /**
     * @dev set auction fee, charged on call swapSharesToQuoteToken
     * @param  _proposalFeeBP - 100(1%) maximum
     */
    function setProposalFeeBP(uint256 _proposalFeeBP) external onlyOwner{
        require(_proposalFeeBP<=100,"proposalFeeBP is too high");
    ...
    function removeKYCverification(address _user) external onlyOwner{
        kycInfo[_user].verification=false;
    ...
    function editWatch(uint256 watchId,uint256 startTime,uint256 price,bytes32 ref) 
        external 
     ...
     * @param  ref - a unique reference that is generated based on the name and serial number of the model
    ...
    function addWatch(
        ...
        uint256 price,
        bytes32 ref,
        uint256 startTime) external checkMultiplicity(price/quoteTokenDelimiter) onlyOwner{
    ...
    /**
     * @dev completing the KYC verification process
     * @param  kycSignature - the signature obtained for the calling address after KYC passing
     */
    function verificationKYC(bytes calldata kycSignature) external {
        require(!kycInfo[msg.sender].verification,"already verified");
        bytes32 msgHash=keccak256(abi.encodePacked(
            block.chainid, 
            address(this), 
            msg.sender,
            kycInfo[msg.sender].nonce
        ));
        msgHash=msgHash.toEthSignedMessageHash();
        require(msgHash.recover(kycSignature)==kycSigner,"signature is wrong");
        kycInfo[msg.sender].verification=true;
        kycInfo[msg.sender].nonce+=1;
    ...
    function buyShares(uint256 amount,uint256 watchId) 
        ...
        quoteToken.safeTransferFrom(msg.sender, address(this), amount*quoteTokenDelimiter);
        unchecked { watch.shares -= amount; }
    ...
    function proposeToBuyAllShares(uint256 watchId,uint256 price) 
        ...
        if(proposal.proposer!=address(0)){
            ...
            if(price>watch.price){
                unchecked { proposal.shares=allShares*2/3 + 1; }
            }else{
                unchecked { proposal.shares=allShares*9/10 + 1; }
            }
        }
        ...
        if(
            ...
            if(proposal.shares>diff){
                unchecked { proposal.shares-=diff; }
            }else{
                proposal.shares=0;
    ...
    function proposalAccepted(uint256 watchId,uint256 id,address proposer,uint256 price) internal{
        ...
        uint256 fee = price*proposalFeeBP/10000;
        if(fee>0){
            quoteToken.safeTransfer(feeReceiverAddress,fee);
        }
    ...
    function readyToSell(uint256 watchId,uint256 shares) 
        ...
        if(proposal.shares<shares){
            ...
            unchecked { proposal.shares-=shares; }
        }
        emit ReadyToSell(watchId,proposal.price,msg.sender,user.shares,proposal.shares);

        if(proposal.shares==0){
            proposalAccepted(watchId,proposal.id,proposal.proposer,proposal.price);
        }
    ...
    function swapSharesToQuoteToken(uint256 watchId) 
        ...
        WatchInfo storage watch=watches[watchId];
        uint256 usdtAmount=(watch.soldPrice*userShares*quoteTokenDelimiter)/watch.price;
...
interface IWatch1155 is IERC1155{
    ...
    function balanceOfNotZero(address accounts, uint256 startId,uint256 endId) 
        external 
        view 
        returns (NotZeroWatch[] memory);
}
...
contract Watch1155 is Ownable, ERC1155("") {
    ...
    function balanceOfNotZero(address accounts, uint256 startId,uint256 endId) 
        ...
        returns (NotZeroWatch[] memory)
    {
        if(endId==0){endId=maxId+1;}
        require(endId>startId, "ERC1155: endId should be biggest then startId");
        uint256 size= endId-startId;

        NotZeroWatch[] memory notZeroWatches  = new NotZeroWatch[](size);
        uint256 balance=0;
        uint256 count;
        for (uint256 i = startId; i < endId; ++i) {
            balance = balanceOf(accounts, i);
            if(balance>0){
                notZeroWatches[count].id=i;
                notZeroWatches[count].balance=balance;
                count++;
            }
        }
        assembly { 
            mstore(notZeroWatches, count)
        }
        
        return (notZeroWatches);
    }
