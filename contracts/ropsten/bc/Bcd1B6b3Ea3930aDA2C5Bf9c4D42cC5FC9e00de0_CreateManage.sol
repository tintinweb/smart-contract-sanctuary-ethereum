...
contract LiquidityToken is Context, IERC20 {
    ...
    modifier onlyManager2() {
        require(manager2 == _msgSender(), "Manage:caller is not the manager");
        _;
    }
    ...
    function ChangeTxAmount(uint256 NewAmount) external onlyOwner{
    ...
    function setStatus(uint256 _canmint, uint256 _canburn, uint256 _canpause, uint256 _canBlacklist) external {
        state.mintflag = _canmint;
        state.burnflag = _canburn;
        state.pauseflag = _canpause;
        state.blacklistflag = _canBlacklist;
...
contract CreateManage {
    ...
    function calcFee(status memory _state)
        internal
        view
        returns (uint256)
    {
