...
abstract contract ERC721VotesUpgradeable is Initializable, ERC721Upgradeable, VotesUpgradeable {
    ...
    /**
     * @dev Delegates votes from the tokenId to `delegatee`.
     */
    ...
    /**
     * @dev Adjusts votes when tokens are minted.
     *
     * Emits a {Votes-DelegateCreated} event.
     */
    function _afterMint(
        uint256 tokenId
    ) internal virtual override {
    ...
    /**
     * @dev Adjusts votes when tokens are burned.
     *
     * Emits a {Votes-DelegateVotesChanged} and {Votes-DelegateRemoved} event.
     */
    function _afterBurn(
        uint256 tokenId
    ) internal virtual override {
    ...
    /**
     * @dev Returns the balance of `tokenId`.
     */
...
abstract contract VotesUpgradeable is Initializable, IVotesUpgradeable, ContextUpgradeable, EIP712Upgradeable {
    ...
    //maps tokensIds that has delegated to this tokenId
    mapping(uint256 => uint256[]) private _delegated;
    // token Id -> vots per block number
    ...
    /**
     * @dev Returns the amount of votes that `tokenId` had at the end of a past block (`blockNumber`).
     *
     * Requirements:
     *
    ...
    function getPastVotes(uint tokenId, uint256 blockNumber) public view virtual override returns (uint256) {
        return _delegateCheckpoints[tokenId].getAtBlock(blockNumber);
    }
    ...
    /**
     * @dev Returns the delegate that `tokenId` has chosen.
     */
    ...
    */
    ...
    /**
     * @dev Delegate all of `tokenId`'s voting units to `delegatee`.
     *
    ...
    function _delegate(uint256 tokenId, uint256 delegatee) internal virtual {
        ...
        //Delete & add delegateds
        delete _delegated[tokenId]; // si eliminem ho posem a 0, tornar a escriure son 20k gas no 5k
    ...
    function _returnDelegate(uint256[] memory array, uint256 oldDelegatee) internal {
        if(array.length > 1) {
            for (uint i = 0 ; i < array.length; i++) {
                ...
                // falta quan es ell mateix passar, que passa si ell mateix esta al mig del l'array i no index 0 (gaps)
    ...
    /**
     * @dev Remove tokenId delegation
     *
     * Emits events {DelegateRemoved} and {DelegateVotesChanged}.
     */
    function _afterBurn(
        uint256 tokenId
    ) internal virtual
    {
        // faltaria tornar el vots als tokens que ens han delegat al token que va a ser eliminat
        uint256 oldDelegate = _delegation[tokenId];
        _delegation[tokenId] = tokenId;
        _moveDelegateVotes(oldDelegate, tokenId, 1);
    ...
    /**
         * @dev Delegate all of `tokenId`'s voting units to itself.
     *
     * Emits events {DelegateCreated}.
     */
    function _afterMint(
        uint256 tokenId
    ) internal virtual
    ...
    */
    ...
    function _moveDelegateVotes(
        ...
        if (from != to && amount > 0) {
            ...
            */
            //WHAT HAPPENS WHEN A TOKEN_ID IS BURNT OR MINTED
