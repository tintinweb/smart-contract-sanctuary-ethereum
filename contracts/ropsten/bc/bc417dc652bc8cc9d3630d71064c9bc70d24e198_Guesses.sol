...
contract Guesses is Ownable{
   ...
   // uint internal maxId = 4*(10**9);//最大id 4亿
    ...
    uint internal fee=5;//手续费,用于返奖打款消耗 
    ...
    struct Guess {
        ...
        mapping(address=>uint) rewardLog;//获奖记录
    }
...
*/
    ...
    function modifyGuess(uint _id,uint _startTimestamp,uint _endTimestamp) public returns(bool){
        require(allGuesses[_id].startTimestamp>now && (allGuesses[_id].drawer==msg.sender||msg.sender==owner));
        allGuesses[_id].startTimestamp=_startTimestamp;
        allGuesses[_id].endTimestamp=_endTimestamp;
        return true;
    }
    ...
    function stopGuess(uint _id,bool stoped) public returns(bool){
        require(allGuesses[_id].endTimestamp>now && (allGuesses[_id].drawer==msg.sender||msg.sender==owner));
     ...
     * @return 已设置的手续费
     */ 
	function setFee(uint _fee) public onlyOwner returns(uint){
	    winPercent=_fee;
	    return fee;
	}
    ...
    function guess(string bet) public payable{
        ...
        for(uint i=0;i<splits.length;i++){
            ...
            totPrice = totPrice.add(allGuesses[singleBet[0]].price.mul(singleBet[2]));
            intInfo[i*3]=singleBet[0];//竞猜id
            intInfo[i*3+1]=singleBet[1];//选项index
            intInfo[i*3+2]=singleBet[2];//数量
        }
    ...
    function setGuessAnswer(string answers) public noReentrancy returns(bool){
        ...
        for(uint i=0;i<splits.length;i++){
            ...
            for(uint j=1;j<=guess.totOptionNum;j++){
                if(j==winOptionIndex){
                    totWin =guess.options[j].totNum;
                }else{
                    totLost += guess.options[j].totNum;
                }
            }
            var option=guess.options[winOptionIndex];
            for(uint x=0;x<option.players.length;x++){
                var player=option.players[x];
                //奖金 = 投入 *(1+ 输总数/赢总数) * 返奖比例 /100
                var cost=option.bets[player]*guess.price;
                var winMoney=(cost+cost*totLost/totWin)*(winPercent-fee)/100;
                guess.rewardLog[player]=winMoney;
    ...
    function getSingleGuessInfo(uint id) internal view returns(string){
        if(allGuesses[id].id!=id){   
         ...
         var result=new strings.slice[](num+2);
         
         result[0]=Integers.toString(id).toSlice();
         result[num+1]=Integers.toString(allGuesses[id].price).toSlice();
