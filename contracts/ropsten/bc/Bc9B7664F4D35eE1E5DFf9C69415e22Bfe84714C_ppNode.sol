...
contract ppNode is ERC721A, Ownable, ControlledAccess {
    ...
    function updateMintPrices(uint256 _price) public onlyOwner{
        mintPrice = _price;
    }
        ...
        super._beforeTokenTransfers(from, to, startTokenId, quantity);
            if(from == address(0)) {
                for(uint256 i; i < quantity; i++){
                    IRewardPool(rewardPool).addNodeInfo(startTokenId, nodeType[startTokenId], to);
                }    
            } else if (from != to) {
                IRewardPool(rewardPool).claimReward(startTokenId);        
                IRewardPool(rewardPool).updateNodeOwner(startTokenId, to);
            }
    }
