    ...
    ERC2981
{
    ...
    function setup() external onlyOwner {
        ...
        tools.push(Tool("Bat", 10, 10, 0.05 ether, 0.05 ether));
        tools.push(Tool("Balloon", 20, 5, 0.01 ether, 0.01 ether)); 
        phaseActive[1] = true;
        phaseActive[2] = true;
    }
    ...
    function ownedList(address _owner) external view returns (uint16[7][200] memory list) {
	...
	function whichCharacter(uint16 phase, uint16 count) public view returns (uint16) {
        uint16 _character;
