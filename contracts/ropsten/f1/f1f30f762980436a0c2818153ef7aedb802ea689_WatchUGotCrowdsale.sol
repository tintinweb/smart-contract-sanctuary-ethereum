...
// File: contracts\WatchUGotToken.sol

/**
 * @title WatchUGot Token
 * @dev WatchUGot Token - Token code for the WatchUGot Project
 * This is a standard ERC20 token with:
 * - a cap
 * - ability to pause transfers
 */
contract WatchUGotToken is CappedToken, PausableToken {

    string public constant name                 = "WatchUGot Token";
...
// File: contracts\WatchUGotCrowdsale.sol

/**
 * @title WatchUGot Crowdsale Contract
 * @dev WatchUGot Crowdsale Contract
 * The contract is for the crowdsale of the WatchUGot token. It is:
 ...
 */
contract WatchUGotCrowdsale is 
    MintedCrowdsale,
    CappedCrowdsale,
    TimedCrowdsale,
    ...
    uint256 public constant SALE_TOKENS    = 10; // 10% from totalSupply
    uint256 public constant TEAM_TOKENS    = 15; // 15% from totalSupply
    uint256 public constant COMMUNITY_TOKENS   = 30; // 30% from totalSupply
    uint256 public constant ADVISOR_TOKENS = 5;  // 5% from totalSupply
    uint256 public constant AIRDROP_TOKENS = 5;  // 5% from totalSupply
    uint256 public constant RESERVE_TOKENS = 35;  // 35% from totalSupply

    uint256 public constant TEAM_LOCK_TIME    = 7885000; // 3 months in seconds
    uint256 public constant RESERVE_LOCK_TIME = 7885000; // 3 months in seconds

    // Rate bonuses
    uint256 public initialRate;
    uint256[6] public bonuses = [75,65,50,40,30,20];
    uint256[6] public stages = [
        1546684200, // 5th  of Jan - 18th of Sep -> 75% Bonus
        1547812800, // 18th of Jan - 1st  of Oct -> 65% Bonus
        1549022400, // 1st  of Feb - 15th of Oct -> 50% Bonus
        1550232000, // 15th of Feb - 1st  of Dec -> 40% Bonus
        1551441600, // 1st  of Mar - 15th of Mar -> 30% Bonus
        1552651200  // 15th of Mar - 31st of Mar -> 20% Bonus
    ];
     ...
     * @param _rate uint256 initial rate WUG for 1 ETH
     ...
     * @param _token WatchUGotToken our token
     */
    constructor(
        uint256 _cap, 
        uint256 _goal, 
    ...
    /**
     * @dev Perform the initial token distribution according to the WatchUGot crowdsale rules
     * @param _teamAddress address address for the team tokens
     * @param _communityPoolAddress address address for the community pool
    ...
    function doInitialDistribution(
        ...
        MintableToken(token).mint(_communityPoolAddress, tokenCap.mul(COMMUNITY_TOKENS).div(100));
    ...
    * 5th  of Jan - 18th of Sep -> 75% Bonus
    * 18th of Jan - 1st  of Oct -> 65% Bonus
    * 1st  of Feb - 15th of Oct -> 50% Bonus
    * 15th of Feb - 1st  of Dec -> 40% Bonus
    * 1st  of Mar - 15th of Mar -> 30% Bonus
    * 15th of Mar - 31st of Mar -> 20% Bonus
    */
    ...
    */
    function finalization() internal {
        WatchUGotToken dmToken = WatchUGotToken(token);
...
// File: contracts\WatchUGotPresale.sol

/**
 * @title WatchUGot Presale Contract
 * @dev WatchUGot Presale Contract
 * The contract is for the private sale of the WatchUGot token. It is:
 * - With a hard cap in ETH
 * - Limited in time (start/end date)
 * - Only for whitelisted participants to purchase tokens
 * - Tokens are minted on each purchase
 */
contract WatchUGotPresale is 
    MintedCrowdsale,
    CappedCrowdsale,
    TimedCrowdsale,
    WhitelistedCrowdsale {
