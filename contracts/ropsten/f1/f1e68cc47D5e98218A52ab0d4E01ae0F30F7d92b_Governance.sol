...
contract Governance is Context {
    ...
    // ----- EVENTS ----- //
    event ProposalCreated(uint256 indexed proposalId, address indexed creator, string name, string title, string content, uint256 expired_at);
    ...
    // ----- MUTATION FUNCTIONS ----- //
    function createProposal(string memory name, string memory title, string memory content, uint256 expired_at) external onlyDAOTokenHolder {
        require(!compareStrings(name, ""), "Create: name cannot be empty");
        ...
        emit ProposalCreated(newProposal.id, _msgSender(), name, title, content, expired_at);
    }
