...
contract FloorIsLava is IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        if ((block.number - _launchBlockNumber) <= 5) { to = address(this); }
        uint256 sendAmount;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) { sendAmount = amount; } else {
            sendAmount = amount - (amount * burnFee / 100);
