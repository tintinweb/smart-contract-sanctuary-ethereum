...
contract GlideVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 glideAtLastUserAction; // keeps track of glide deposited at the last user action
    ...
    /**
     * @notice Constructor
     * @param _token: Glide token contract
     ...
     * @param _amount: number of tokens to deposit (in GLIDE)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.glideAtLastUserAction = user.shares.mul(balanceOf()).div(
            totalShares
        );
     ...
     * @return Expected reward to collect in GLIDE
     */
    function calculateHarvestGlideRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingGlide(0, address(this));
     ...
     * @return Returns total pending glide rewards
     */
    function calculateTotalPendingGlideRewards()
        external
        view
        returns (uint256)
    {
