...
contract PoolMatrixGame {
    ...
    address public projectWallet; // For registration rewards
    address public ownerWallet; // For 2% fee
    address public trashWallet; // For unknown payments
    ...
    event UnknownPaymentReceived(address indexed senderAddr, uint256 amount);
    ...
    constructor() {
        ...
        ownerWallet = 0xce072faC1888c6A272546e28020d3293B30b9Ff6;
        projectWallet = 0x3C94E1C74f22d23E0334716BAE213F508a7A7907;
        trashWallet = 0xAAfeC3bd84a7802f562EC89D24E5115cba7f9C8A;
    ...
    function sendToTrash() public payable {
        // Redirecting all unknown payments to the trash wallet
        payable(trashWallet).transfer(msg.value);
        emit UnknownPaymentReceived(msg.sender, msg.value);
    }

    function register(address referrer) public payable {
        // Check if referrer address is not empty
        require(referrer != address(0), "User require a referrer");
        ...
        payable(projectWallet).transfer(msg.value);
    ...
    function buyLevel(uint8 level) public payable {
        ...
        require(level >= 1 && level <= 20, "Invalid level number");

        // Check if receive the right amount
        require(msg.value >= levelPrices[level], "Low amount received");

        // Check if user has a referrer
        require(users[msg.sender].referrer != address(0), "User not exist");

        uint256 amount = msg.value;
        uint256 restOfAmount = amount;

        // Sending 2% level fee
        uint256 levelFee = amount * LEVEL_FEE_PERCENTS;
        payable(ownerWallet).transfer(levelFee);
        restOfAmount -= levelFee;

        // Sending 14%, 8% and 3% to top referrers
        bool isRootUser;
        uint256 amountForRootUser = 0;
        address referrer = users[msg.sender].referrer;
        for (uint i = 0; i < 3; i++) {
            // Calculating the value to invest to current referrer
            uint256 value = amount * referrerPercents[i] / 100;
        ...
        if (!users[msg.sender].levels[level].opened) {
            ...
            emit LevelActivated(msg.sender, level);
        }
        ...
        if (rewardAddress != msg.sender) {
            ...
            if (sent) {
                ...
                payable(ownerWallet).transfer(restOfAmount);
            }
        }
        else {
            payable(ownerWallet).transfer(restOfAmount);
    ...
    function getUser(address userAddress) public view returns(bool registered, uint registrationTimestamp, address userAddr, address referrer, uint256 balance, UserLevelInfo[] memory levels) {
    ...
    function getLevelsForCurrentUser(address userAddr) public view returns (int[] memory) {
        int[] memory userLevels = new int[](levelsCount);
        for (uint8 level = 1; level <= levelsCount; level) {
            if (users[userAddr].levels[level].opened) {
                /*uint place = 0;
                uint len = levelQueue[level].length - headIndex[level];
                for(uint i = headIndex[level]; i < levelQueue[level].length; i++) {
                    place++;
                    if(levelQueue[level][i] == userAddr)
                        break;
                }*/
                userLevels[level] = 1;//int((len - place) * 100 / len);
            }
            else {
                userLevels[level] = -1;
            }
    ...
    function getPlaceInQueue(address userAddress, uint8 level) public view returns(uint, uint) {
        require(level > 0 && level <= levelsCount, "Invalid level");

        // If user is not in the level queue
        if(!users[userAddress].levels[level].opened) {
            return (0, 0);
        }
