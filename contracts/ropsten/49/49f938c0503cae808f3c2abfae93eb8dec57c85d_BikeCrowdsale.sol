...
contract BikeCrowdsale is Ownable, StandardToken {
  ...
  function sellTokens(address _recepient, uint256 _value) private
  {
    ...
    if (hardCap <= newTotalCollected) {
        uint256 _refund = newTotalCollected - hardCap; // unit: wei
        uint256 _diff =  _value - _refund; // unit: wei
        _recepient.transfer(_refund); // send given amount of Wei to Address,
        _value = _diff; // unit: wei
