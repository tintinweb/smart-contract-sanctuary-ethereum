...
contract FilesFMToken is TokenRecoverable, ERC777ERC20BaseToken {
    ...
    string private constant name_ = "Files.fm Library Token";
    string private constant symbol_ = "FFL";
    uint256 private constant granularity_ = 1;
    uint256 private constant TOKEN_CAP = 10000000000e18; // 10 000 000 000 (ten billion) tokens
    ...
    function send(address _to, uint256 _amount, bytes _userData) public canTransfer {
    ...
    function operatorSend(
        ...
        uint256 _amount, 
        bytes _userData, 
        bytes _operatorData) public canTransfer {
        super.operatorSend(_from, _to, _amount, _userData, _operatorData);
    }

    function transfer(address _to, uint256 _amount) public erc20 canTransfer returns (bool success) {
        return super.transfer(_to, _amount);
    }

    function transferFrom(address _from, address _to, uint256 _amount) public erc20 canTransfer returns (bool success) {
    ...
    function mint(address _tokenHolder, uint256 _amount, bytes _operatorData) public onlyOwner canMint {
        requireMultiple(_amount);
        require(mTotalSupply.add(_amount) <= TOKEN_CAP, "Token cap reached");
