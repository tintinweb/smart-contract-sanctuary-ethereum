...
contract ERC20 {
     ...
     * The hold is currently under the control of `notary`. The hold contains `amount` of tokens and can be executed or renewed until `expiration`. 
     */
    event HoldCreated(address holdCreator, string indexed operationId, address indexed payerId, address payeeId, address indexed notary, uint256 amount, uint256 expiration);
    /**
     * @dev Emitted when the hold with id `operationId' has been executed by it's assigned notary.
     */
    event HoldExecuted(string indexed operationId);
    /**
     * @dev Emitted when the hold with id `operationId' has been released (without payment) by it's assigned notary.
    ...
    /**
     * @dev Emitted when an authorised operator `operator` has been assigned to be able to create holds for `account`.
     */
    ...
    /**
     * @dev Functions using this modifier restrict the caller to only be the minter address (or the operator of the minter address)
     */
   modifier onlyMinter {
        require(isOperatorFor(msg.sender, minter()), "Caller is not an operator for Minter");
      _;
   }
   ...
   /**
    ...
    /**
     * @dev Returns true if `operatorId` is an operator of `accountId`.
     * Operators can send and burn tokens for an authorised account. All
     * accounts are their own operator.
     */
    function isOperatorFor(address operatorId, address accountId) public view returns (bool) {
        return
            operatorId == accountId ||
            _operators[accountId][operatorId];
    }
     ...
     * -  the caller must be an operator for `payerId`.
     * - `payeeId` cannot be the zero address.
     */
    function operatorTransfer(
        ...
        require(isOperatorFor(msg.sender, payerId), "Caller is not an operator for payerId");
     ...
     * -  the caller must be an operator for `payeeId`.
     * - `payeeId` cannot be the zero address.
     */
    function operatorTransferFrom(
        ...
        require(isOperatorFor(msg.sender, payeeId), "Caller is not an operator for payeeId");
        _transferFrom(payerId, payeeId, amount);
    }
     ...
     * The caller must be an operator of `accountId`.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     * Emits a {Transfer} event with `to` set to the zero address.
     ...
     * - the caller must be an operator for `accountId`.
     */
    ...
    /**
     * @dev Make `operatorId` an operator of `accountId`.
     *
     * Emits an {AuthorizedOperator} event.
     *
     * Requirements:
     *
     * -  caller must be an operator of `accountId`.
     * - `operatorId` must not equal the caller.
     */
    function authorizeOperator(address accountId, address operatorId) public {
        require(isOperatorFor(msg.sender, accountId), "Caller is not an operator for accountId");
        require(msg.sender != operatorId, "Authorizing self as operator");

        _operators[accountId][operatorId] = true;

        emit AuthorizedOperator(operatorId, accountId);
    }

    /**
     * @dev Revoke `operatorId`'s operator status for `accountId`.
     *
     ...
     * - `operatorId` must not equal the caller.
     */
    function revokeOperator(address accountId, address operatorId) public {
        require(isOperatorFor(msg.sender, accountId), "Caller is not an operator for accountId");

        delete _operators[accountId][operatorId];

        emit RevokedOperator(operatorId, accountId);
    }
     ...
     * -  the caller must be `payerId` or an operator for `payerId`.
     ...
     * -  the caller must be `payeeId` or an operator for `payeeId`.
     * -  `payeeId` cannot be the zero address.
     */
     ...
     * It specifies a `notaryId` who is responsible to either execute or release the hold.
     * This hold can be executed by `notaryId` until `secondsToExpiration` have passed from the current block timestamp.
     ...
     * - `payeeId` cannot be the zero address.
     * - `notaryId` cannot be the zero address.
     * - `payeeId` must have at least `amount` tokens (where `amount` is greater than zero).
     * -  caller must be `payerId` or an authorised operator for `payerId`.
     * - `notaryId` must be an authorised hold operator of `payerId`.
     */
    function hold(
     ...
     * - `notaryId` cannot be the zero address.
     * - `payeeId` must have at least `amount` tokens (where `amount` is greater than zero).
     * -  caller must be `payeeId` or an authorised operator for `payerId`.
     * - `notaryId` must be an authorised hold operator of `payerId`.
     */
    function holdFrom(
        string memory operationId,
     ...
     * This hold can be only be released by `notaryId` until `secondsToExpiration` have passed from the current block timestamp.
     ...
     * - if the hold has not expired, only the notary (or the notary's operator) can release the hold
     */
    function releaseHold(string memory operationId) public returns (bool) {
        ...
        require(
            ...
            (isOperatorFor(msg.sender, releasableHold.notary)),
            "A not expired hold can only be released by the notary"
        );
     ...
     * - the caller must be the hold's notary.
     ...
     * - the caller must be the hold's creator or payer (or their corresponding operators)
     */
    ...
    /**
     * @dev authorises `operator' to receive holds from `account`.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     ...
     * Requirements
     *
     * - both `account` and `operator` must be different
     * - the caller must be `account` or an authorised operator of `account`.
     */
    function authorizeHoldOperator(address account, address operator) public returns (bool){
        require(isOperatorFor(msg.sender, account), "Caller is not an operator for account");
        require(account != operator, "Authorizing self as operator");

        _holdOperators[account][operator] = true;

        emit AuthorizedHoldOperator(operator, account);

        return true;
    }

    /**
     * @dev revokes `operator' from receiving holds from `account`.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     * Emits a {AuthorizedHoldOperator} event
     ...
     * - the caller must be `account` or an authorised operator of `account`.
     */
    function revokeHoldOperator(address account, address operator) public returns (bool){
        require(isOperatorFor(msg.sender, account), "Caller is not an operator for account");
        require(account != operator, "Revoking self as operator");

        delete _holdOperators[account][operator];

        emit RevokedHoldOperator(operator, account);

        return true;
    }

     /**
     * @dev checks to see if `operator` is an authorised hold operator of `account`
     */   
    function isHoldOperatorFor(address operator, address account) public view returns (bool){
        return
            operator == account ||
            _holdOperators[account][operator];
    }
   ...
   /**
    ...
    ) private returns (bool)
    {
        ...
        require(notaryId != address(0), "Notary address must not be zero address");
        ...
        require(isHoldOperatorFor(notaryId, payerId), "This notary is not authorized for payerId");
        

        newHold.creator = creatorId;
        newHold.payer = payerId;
        newHold.payee = payeeId;
        newHold.notary = notaryId;
    ...
    /**
     * @dev executes the hold corresponding to `operationId`.
     * Via `doTransfer` the notary says whether to send the held funds to the payee (true) or to return the funds to the payer (false)
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     * Emits a {HoldCreated} event
