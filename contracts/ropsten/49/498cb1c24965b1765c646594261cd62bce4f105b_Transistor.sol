...
contract Transistor is Context, ERC20, Ownable {
    ...
    string private _name = "Transistor";
    string private _symbol = "Transistor";
    uint8 private _decimals = 9;
    uint256 private _tTotal = 100000000000 * 1e9;
    ...
    uint256 public _buyMarketingTax = 50; // 5%
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "Transistor: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "Transistor: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function setAmountLimits(uint256 _maxSellTxAmount, uint256 _maxBuyTxAmount)
        ...
        onlyOwner
    {
        require(_maxSellTxAmount >= 500000000 * 1e9);  // must be greater than 0.5%
    ...
    function _approve(
        ...
        require(owner != address(0), "Transistor: approve from the zero address");
        require(spender != address(0), "Transistor: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "Transistor: transfer from the zero address");
        require(to != address(0), "Transistor: transfer to the zero address");
        require(
            amount > 0,
            "Transistor: Transfer amount must be greater than zero"
        );
        ...
        if (
            ...
            if (_taxableExchange[to]) {
                // sell
                require(
                    amount <= maxSellTransaction.add(5*1e9),
                    "Transistor: max sell transaction exceeded"
                );
            }
            if (_taxableExchange[from]) {
                // buy
                require(
                    amount <= maxBuyTransaction.add(5*1e9),
                    "Transistor: max buy transaction exceeded"
                );
            }
        }
    ...
    function manualSwap() external onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this)).sub(1000 * 1e9); // maintain tokens in the contract
        require(
            contractTokenBalance > 0,
            "Transistor: contract balance must be greater than zero"
        );
    ...
    function manualSend() external onlyOwner {
        ...
        require(deltaBalance > 0, "Transistor: Insufficient contract balance");
        ...
        uint256 buyBackPercent = takeBuyBackTax().mul(devisor).div(totalPercent);
...
// 0x25664f4Ff9EE92162675d5204CCc306c3cd01382
// 0x870242a85fB38B9F4A81c1684e41C9F8A3E3C651
// 0x4e5De75838DC1C6760C8761A756e6759f1f0691f
// 0xE7a1454d8b3DD68612D7C32d955e32ec764E7521