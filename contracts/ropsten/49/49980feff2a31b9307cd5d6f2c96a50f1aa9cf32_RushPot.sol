...
contract RushPot is modularShort {
    using SafeMath for *;
    using NameFilter for string;

    PlayerBookInterface constant private PlayerBook = PlayerBookInterface(0xB00Ee59dF7a2A792254f7830676089026964D174);
    DiviesInterface constant private Divies = DiviesInterface(0x261CC466AA6FED95323BDcd69017016ac2070D5f);
...
****************************************************************************************************************************/
    ...
    string constant public name = "Rush Pot";
...
//****************
     ...
     uint256 public incrementPercentage_;
...
****************************************************************************************************************************/
        ...
        public
    {
        ...
        incrementPercentage_ = 5000;

        keyPrice_ = 2200000000000000;
...
****************************************************************************************************************************/
        ...
        public
    {
        ...
        if (_now > round_[_rID].end && round_[_rID].ended == false && round_[_rID].plyr != 0)
        {
            ...
            emit RPevents.onWithdrawAndDistribute
            (
                msg.sender, 
                plyr_[_pID].name, 
                _eth, 
        ...
        // in any other situation
            ...
            // fire withdraw event
            emit RPevents.onWithdraw(_pID, msg.sender, plyr_[_pID].name, _eth, _now);
        }
    ...
    /**
     * @dev Set increment percentage
     * @param _percentage price increment
     */
    function setIncrementPercentage(uint256 _percentage) public{

        require(msg.sender == admin, "only admin can activate");

        incrementPercentage_ = _percentage;
        ...
        public
    {
        ...
        uint256 _newTime;
        if (_now > round_[rID_].end && round_[rID_].plyr == 0)
            _newTime = (((_keys) / (1000000000000000000)).mul(rndInc_)).add(_now);
        else
            _newTime = (((_keys) / (1000000000000000000)).mul(rndInc_)).add(round_[rID_].end);

        // compare to max and set new end time
        if (_newTime < (rndMax_).add(_now))
            round_[rID_].end = _newTime;
        else
            round_[rID_].end = rndMax_.add(_now);
...
****************************************************************************************************************************/
        ...
        returns(uint256)
    {
        ...
        //if (_now > round_[_rID].strt + rndGap_ && (_now <= round_[_rID].end || (_now > round_[_rID].end && round_[_rID].plyr == 0)))
            ...
            //return  eth(round_[_rID].keys);
        //else    
            //return ( keyPrice_ ); // init
            
        uint256 increment = round_[rID_].pot.mul(1e13).mul(incrementPercentage_) / 1e18;

        uint keyPrice = keyPrice_.add(increment);

        return ((1000000000000000000 * keyPrice) / 1e18).ceil(1000);  
...
****************************************************************************************************************************/
        ...
        returns (uint256)
    {
        ...
        keyPrice_ = keyPrice_.add(increment);

        return _eth.mul(1e18) / keyPrice_;
        ...
        returns (uint256)
    {
        ...
        return ((_keys * keyPrice_) / 1e18).ceil(1000);  
