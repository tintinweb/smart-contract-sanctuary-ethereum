...
contract Elfheim is ERC721, Ownable{
    ...
    mapping(address => uint8) _whitelistClaimed;

    mapping(address => uint8) _nbClaimedByAddress;

    string private _uriSuffix=".json";
    string private _uriPrefixUnrevealed="";
    ...
    constructor() ERC721("Elf Heim","ELF"){
        ...
        _whouhouElfsAreComing(msg.sender, NB_ELF_FOR_TEAM);
    ...
    function drinkWithElfForFree(uint8 _howMuchBuddy,bytes32[] calldata _merkleProof) public{
        ...
        require(_whitelistClaimed[msg.sender] + _howMuchBuddy <=2,"2 for free, isn't it enough ?");
        ...
        _whitelistClaimed[msg.sender] += _howMuchBuddy;
        _whouhouElfsAreComing(msg.sender,_howMuchBuddy);
    }
    ...
    function drinkWithElf(uint8 _howMuchBuddy) payable public{
        ...
        require(_nbClaimedByAddress[msg.sender] + _howMuchBuddy <=50,"You can only mint 50 Elfs");
    ...
    function _whouhouElfsAreComing(address _receiver, uint256 _mintAmount) internal {
    ...
    function airdropElfAreFlying(address[] memory _addresses) public onlyOwner{
        ...
        for (uint i=0; i<_addresses.length; i++) {
            _whouhouElfsAreComing(_addresses[i],1);
        }
    }
