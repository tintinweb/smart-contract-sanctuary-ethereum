...
contract HawkToken is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable, IAccountManagement {
    ...
    function initialize(string memory tokenName, string memory symbol, uint256 initialSupply, address initialSupplyOwner) initializer public {
        __ERC20_init(tokenName, symbol);
        __Ownable_init();
        __UUPSUpgradeable_init();
        super._mint(initialSupplyOwner, initialSupply);
    }    

    function _authorizeUpgrade(address newImplementation) internal onlyOwner override {
        // Intentionally empty function. 
        // This is called by the UUPSUpgradeable base contract and the onlyOwner modifier ensures that only the owner can upgrade. 
    }

    function approve(address payeeId, uint256 amount) public override returns (bool) {
        if (allowance(msg.sender, payeeId) > 0){
    ...
    function mint(address beneficiaryAccountId, uint256 amount, bytes calldata data) external onlyOwner() returns (bool) {
        super._mint(beneficiaryAccountId, amount);
