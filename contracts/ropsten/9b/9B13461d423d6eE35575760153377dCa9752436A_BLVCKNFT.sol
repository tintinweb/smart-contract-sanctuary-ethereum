...
contract BLVCKNFT is ERC721, Ownable, PaymentSplitter {
     ...
     * @notice setup event
     */
    event Setup(string newBaseURI, uint256 newPrice, bool setPublicSale, address setMinterAddress);
    ...
    constructor(address[] memory payeesList, uint256[] memory sharesList)
        ERC721("BLVCK NFT", "BLVCK")
        PaymentSplitter(payeesList, sharesList) {
    }

    /**
     * @notice This must be called immediately after contract deployment, and later can be called at any time to configure contract settings
     * @notice WARNING: If some parameters should stay unchanged, be sure to call this method with EXACT previous parameter value!
     * @notice Cannot be executed by anyone except Owner
     * @param newBaseURI is to set baseURI
     * @param newPrice is to set currentPrice WARNING: the price is in wei!
     * @param setPublicSale is to enable or disable public sale
     * @param setMinterAddress is sto assign a minter account
     */
    function setup(string memory newBaseURI, uint256 newPrice, bool setPublicSale, address setMinterAddress) external onlyOwner() {
        require(setMinterAddress != address (0), "failed to set minter addr(0)");
        baseURI = newBaseURI;
        currentPrice = newPrice;
        publicSaleIsActive = setPublicSale;
        minterAddress = setMinterAddress;
        emit Setup(newBaseURI, newPrice, setPublicSale, setMinterAddress);
    }
    ...
    /**
     * @notice custom _baseURI() provider allows migration of NFT urls later
     */
    ...
    function reserveTokens(uint256 amount) external onlyOwner() {
        uint256 tokenId = checkTotalSupply(amount);
        safeMintMany(_msgSender(), tokenId, amount);
    }

    /**
     * @notice this allows minter account to mint amount of tokens for address to
     * @param amount is number of tokens to mint
     * @param to is account receiving the tokens
     */
    function minterMintBatch(uint256 amount, address to) external onlyMinter {
        uint256 tokenId = checkTotalSupply(amount);
     ...
     * @param to is account receiving the tokens
     */
    function minterMint(address to) external onlyMinter {
     ...
     * @param to is the account to receive NFTs (if deposited enough funds)
     * @param maxAmount is a limit to how many tokens will be minted to user. if there is an excess funds deposited, it will be cleared to zero.
     */
    function minterMintWithPendingBalance(address to, uint256 maxAmount) external onlyMinter {
        uint256 tokenId = checkTotalSupply(maxAmount);
        while (pendingMintBalances[to] >= currentPrice && maxAmount > 0) {
            _tokenIdCounter.increment();
