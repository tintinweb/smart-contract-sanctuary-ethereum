...
contract nft is ERC721, Ownable {
    ...
    constructor() ERC721('Digital Ape Club', 'DAC') {
        maxTotalTokens = 7777;
    ...
    function tokenId() internal view returns(uint256) {
        uint currentId = totalSupply() + 1 + maxReservedMints - _reservedMints;
        bool exists = true;
        while (exists) {
            currentId += 1;
    ...
    */
    ...
    function publicSaleMint(uint256 number) public payable {
        ...
        require(numberOfTotalTokens + number <= maxTotalTokens - (maxReservedMints - _reservedMints), "Not enough NFTs left to mint..");
        //require(mintsPerAddress[msg.sender] + number <= maxMintPublicSale, "Maximum 10 Mints per Address allowed!");
