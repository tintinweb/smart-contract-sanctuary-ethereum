 ...
 */
contract CPToken is ERC721Full, MinterRole, Ownable {

  // structure that defines a model
  struct CPModelStructure {
    string modelNumber;
  ...
  mapping(uint256 => CPModelStructure) private _structureIndex;
  ...
  function newCPModel(
    ...
    string calldata nfcSerialNumber,
    ...
    returns (uint256)
  {
    ...
    _structureIndex[tokenId] = CPModelStructure(
      modelNumber,
      nfcSerialNumber,
      ownerAddress,
      ownerName
  ...
  function isVisible (
    ...
    external
    view
    returns (bool visible, address date)
  {
    if (_exists(tokenId)) {
      CPModelStructure storage mjItem = _structureIndex[tokenId];
      date = mjItem.ownerAddress;
    } else {
      visible = false;
      date = address(0x0);
    }
  ...
  /**
   * @dev Returns the mjItem structure.
   */
  function getCPModel (uint256 tokenId)
    external
    view
    returns (
    ...
    )
  {
    ...
    CPModelStructure storage mjItem = _structureIndex[tokenId];
    modelNumber = mjItem.modelNumber;
    ownerAddress = mjItem.ownerAddress;
    nfcSerialNumber = mjItem.nfcSerialNumber;
    ownerName = mjItem.ownerName;
  }
  ...
  function transferOwnershipToken(
      address newOwner, 
    ...
    returns (uint256)
    {
        ...
        CPModelStructure storage mjItem = _structureIndex[tokenId];
        require (mjItem.ownerAddress == msg.sender);
        mjItem.ownerAddress = newOwner;
        mjItem.ownerName = newOwnerName;
    }
}