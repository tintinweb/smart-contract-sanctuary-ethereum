...
contract Yumo {
    ...
    function gameOver() public {
        ...
        ethReserverForWithdraw = ethReserverForWithdraw.add(rounds[currentRound].participantsPot).add(amountForWinner);
        // start distribution
    ...
    function withdraw(uint256 ethValue, uint256 tokenValue) public {
        ...
        ethReserverForToken = ethReserverForToken.sub(ethValue);
        tokenBalanceOf[msg.sender] = tokenBalanceOf[msg.sender].sub(tokenValue);
        ethReserverForToken = ethReserverForToken.sub(tokenValue);
    }
