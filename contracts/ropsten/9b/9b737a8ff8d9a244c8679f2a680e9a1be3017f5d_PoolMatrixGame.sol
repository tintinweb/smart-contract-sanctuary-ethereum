...
contract PoolMatrixGame {
    ...
    */
    ...
    function getUserQueueForLevel(address userAddr, uint8 level) public view returns (address[] memory addesses, uint8[] memory payouts) {
        require (users[userAddr].registered, "Invalid user");
        require (level >= 1 && level <= levelsCount, "Invalid level");
        level--;
        ...
        uint n = levelQueue[level].length;
        for (uint i = headIndex[level]; i < n; i++) {
            addressQueue[index] = levelQueue[level][i];
            payoutsQueue[index] = users[userAddr].levels[level].payouts;
            index++;
        }
