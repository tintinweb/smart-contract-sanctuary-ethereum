...
/**
 * @dev GIMA BLOCKCHAIN
 * @author Victor Tin <[email protected]>
 */
...
contract GIMABLOCKCHAIN is Context, IERC20, Ownable {
 ...
 address _feeAddress = 0x998CA59652bC95e21Ad54Ef1a3C19e03883F0089;

  string private constant _name = 'GIMA';
  string private constant _symbol = 'GIMA';
  uint8 private constant _decimals = 18;

  uint256 private _totalSupply = 1500000000 * 10 ** uint256(_decimals);
  ...
  function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!_isBlacklisted[sender], 'Blacklisted address');
             ...
             if(feeEnabled){
                    ...
                    uint256 tFee = findPercent(amount,_fee);
                    uint256 tokensToTransfer = amount.sub(tFee);              
                    

                    _balances[sender] = _balances[sender].sub(amount);
                    _balances[recipient] = _balances[recipient].add(tokensToTransfer);
                    _balances[_feeAddress] = _balances[_feeAddress].add(tFee);
                    
                        
                            _reflectFee(tFee);
                            emit Transfer(sender, recipient, tokensToTransfer);
                            emit Transfer(_msgSender(), _feeAddress, tFee);
                    }else{
                    _balances[sender] = _balances[sender].sub(amount);
...
function _mint(address account, uint256 amount) internal {
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    function _getFee() public view returns(uint256) {
        return _fee;
    }  

    
    function _setFee(uint256 fee) external onlyOwner(){
          require(fee <= 2000, "Fee must be less than  or equal to 20%");
        _fee = fee;
    }
    ...
    function EnabledFee(bool _enabled) external onlyOwner {
        feeEnabled = _enabled;
    }
