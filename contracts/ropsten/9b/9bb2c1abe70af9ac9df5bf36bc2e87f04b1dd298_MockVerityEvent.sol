...
// File: contracts/MockVerityEvent.sol

contract MockVerityEvent is VerityEvent {
    constructor(
        string _eventName,
        uint _applicationStartTime,
        uint _applicationEndTime,
        ...
        uint[3] _disputeRules, // [dispute amount, dispute timeout, dispute multiplier]
        string _ipfsEventHash
    ) public payable VerityEvent(
        _eventName,
        _applicationStartTime,
        _applicationEndTime,
        _eventStartTime,
        _eventRunTime, // in seconds
        _tokenAddress,
        _registry,
        _eventResolvers,
        _leftoversRecoverableAfter, // with timestamp (in seconds)
        _consensusRules, // [minTotalVotes, minConsensusVotes, minConsensusRatio, minParticipantRatio, maxParticipants, distribution]
        _stakingAmount,
        _disputeRules,
        _ipfsEventHash
    ) {}

    function rewind(uint secs) public {
        applicationStartTime -= secs;
        applicationEndTime -= secs;
        eventStartTime -= secs;
        eventEndTime -= secs;
        dispute.expiresAt -= secs;
    }

    function mockSetResult(string _result) public {
        result = _result;
    }
}