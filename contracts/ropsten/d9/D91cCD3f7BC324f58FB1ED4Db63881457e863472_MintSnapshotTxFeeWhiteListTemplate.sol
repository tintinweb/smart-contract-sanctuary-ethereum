...
// File contracts/templates/MintSnapshotTxFeeWhiteListTemplate.sol


pragma solidity ^0.8.0;

contract MintSnapshotTxFeeWhiteListTemplate is ERC20Base {
...
// File contracts/test/LibrariesCheck.sol
...
contract LibrariesCheck {
    uint[] array = [9, 5];
    mapping(uint => uint) map;
    mapping(uint => uint[]) mapWithArr;

    bytes32 ARRAY_POSITION = 0;
    bytes32 MAP_POSITION = bytes32(uint256(1));
    bytes32 MAP_WITH_ARRAY_POSITION = bytes32(uint256(2));

    /**
     * @notice Get the value from array at `index` using the EVM format and the ArrayStorageSlot Library
     */
    function getArrayValue(uint index) public view returns(uint, uint) {
        uint normalEVMValue = array[index];
        uint libraryValue = ArrayStorageSlot.getUint256Slot(ARRAY_POSITION, index).value;
        return(normalEVMValue, libraryValue);
    } 
    /**
     * @notice Set the value in array at `index` using the EVM format and the ArrayStorageSlot Library
     */
    function setArrayValue(uint index, uint value) public {
        array[index] = value;
        ArrayStorageSlot.getUint256Slot(ARRAY_POSITION, index).value = value;
    }
    /**
     * @notice Push the value in the end of array using the EVM format and the ArrayStorageSlot Library
     */
    function pushArrayValue(uint value) public {
        array.push(value);
        ArrayStorageSlot.push(ARRAY_POSITION, value);
    }
    /**
     * @notice Get the length from array using the EVM format and the ArrayStorageSlot Library
     */
    function getArrayLength() public view returns(uint, uint) {
        uint normalEVMValue = array.length;
        uint libraryValue = ArrayStorageSlot.length(ARRAY_POSITION).value;
        return(normalEVMValue, libraryValue);
    }

    /**
     * @notice Get the value from map at `key` using the EVM format and the MappingStorageSlot Library
     */
    function getMapValue(uint key) public view returns(uint, uint) {
        uint normalEVMValue = map[key];
        uint libraryValue = MappingStorageSlot.getUint256Slot(MAP_POSITION, key).value;
        return(normalEVMValue, libraryValue);
    }

    /**
     * @notice Set the value in array at `index` using the EVM format and the MappingStorageSlot Library
     */
    function setMapValue(uint key, uint value) public {
        map[key] = value;
        MappingStorageSlot.getUint256Slot(MAP_POSITION, key).value = value;
    }

    /**
     * @notice Get the value from map at `key` using the EVM format and the Library
     */
    function getMapWithArr(uint key, uint index) public view returns(uint, uint) {
        uint normalEVMValue = mapWithArr[key][index];
        bytes32 keySlot = keccak256(abi.encode(key, MAP_WITH_ARRAY_POSITION));
        uint libraryValue = ArrayStorageSlot.getUint256Slot(keySlot, index).value;
        return(normalEVMValue, libraryValue);
    }

    /**
     * @notice Set the value in array at `index` using the EVM format and the Library
     */
    function setMapWithArr(uint key, uint index, uint value) public {
        mapWithArr[key][index] = value;
        bytes32 keySlot = keccak256(abi.encode(key, MAP_WITH_ARRAY_POSITION));
        ArrayStorageSlot.getUint256Slot(keySlot, index).value = value;
    }
    /**
     * @notice Push the value in array using the EVM format and the Library
     */
    function pushMapWithArr(uint key, uint value) public {
        mapWithArr[key].push(value);
        bytes32 keySlot = keccak256(abi.encode(key, MAP_WITH_ARRAY_POSITION));
        ArrayStorageSlot.push(keySlot, value);
    }

    /**
     * @notice Get the length from mapWithArr using the EVM format and the ArrayStorageSlot Library
     */
    function getMapWithArrayLength(uint key) public view returns(uint, uint) {
        uint normalEVMValue = mapWithArr[key].length;
        bytes32 keySlot = keccak256(abi.encode(key, MAP_WITH_ARRAY_POSITION));
        uint libraryValue = ArrayStorageSlot.length(keySlot).value;
        return(normalEVMValue, libraryValue);
    }
}
...
// File contracts/test/PolkalokrTestToken.sol
...
contract PolkalokrTestToken is ERC20Upgradeable, UUPSUpgradeable, OwnableUpgradeable, ERC20PausableRewrited, ERC20SnapshotRewrited {
    ...
    function _authorizeUpgrade(address) internal override(BaseGovernanceWithUserUpgradable, UUPSUpgradeable) onlyOwner {}
