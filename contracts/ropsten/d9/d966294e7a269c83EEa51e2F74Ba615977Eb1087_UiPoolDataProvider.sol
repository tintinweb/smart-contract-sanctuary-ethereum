...
contract UiPoolDataProvider is IUiPoolDataProvider {
  ...
  function _fillNftData(
    ...
    (nftData.redeemDuration, nftData.buyItNowDuration, nftData.redeemFine, nftData.redeemThreshold) = baseData
      .configuration
      .getBuyItNowParamsMemory();
    (nftData.isActive, nftData.isFrozen) = baseData.configuration.getFlagsMemory();
    nftData.minBuyItNowFine = baseData.configuration.getMinBuyItNowFineMemory();
  }
  ...
  function getSimpleLoansData(
    ...
    for (uint256 i = 0; i < nftAssets.length; i++) {
      ...
      (, loanData.buyItNowBuyerAddress, loanData.buyItNowPrice, loanData.buyItNowBorrowAmount, loanData.buyItNowFine) = lendPool
        .getNftBuyItNowData(nftAssets[i], nftTokenIds[i]);
    }
