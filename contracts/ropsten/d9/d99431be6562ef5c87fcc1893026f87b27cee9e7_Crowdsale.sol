...
contract token {
    ...
    string public standard = "LoveCoin 1.0";
    string public name;
...
contract LoveCoin is owned, token {
    ...
    string public tokenName = "LoveCoin";
    string public tokenSymbol = "love";
    function LoveCoin() token (initialSupply, tokenName, decimalUnits, tokenSymbol) {}
     /* Send coins */
    function transfer(address _to, uint256 _value) {
        if (balanceOf[msg.sender] < _value) throw;           // Check if the sender has enough
        if (balanceOf[_to] + _value < balanceOf[_to]) throw; // Check for overflows
