...
contract asset {
    ...
    //-----------------------------------------------asset-----------------------------------------------
    function addasset(string memory _fileName, string memory _fileType, uint _assetPrice,string memory _category, string memory _description, string memory _document) public {
        require(isUserVerified(msg.sender));
        assetsCount++;
        assets[assetsCount] = assetreg(assetsCount, _fileName, _fileType, _category, _description, _assetPrice, _document,false,payable(msg.sender),false);
