...
contract SSSNFTMarket is BEP721 {
    ...
    uint256  constant mintPricepernft = 0.025 ether ; 
    uint256  constant maxMint = 25;
    uint256  MAX_TOKENS = 25 ;
    uint256 mintfess = mintPricepernft * maxMint;
    address payable nftminter ;
    // Name token using inherited BEP721 constructor.
    constructor(string memory  name ,string memory symbol) BEP721 (name , symbol){
         collectionName = name ;
    ...
    // mint a new nft
    function mintNFT(uint256 numberOfTokens,string memory _tokenURI,uint256 _price) external payable {       
         require (payable(msg.sender) == nftminter);      
        nftCounter++;
        ...
        require(numberOfTokens == maxMint, "You can only mint 25 tokens at a time");
        ...
        require(mintfess == msg.value, "Not enough Ether sent.");        
        
             require(_price > 0, "Price must be at least 1 wei");
        require(msg.value == mintfess,"Price must be equal to listing+minting" );
        ...
        payable(owner).transfer(mintfess);
    }
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        require(msg.sender != nftminter,"nft owner cannot buy");
