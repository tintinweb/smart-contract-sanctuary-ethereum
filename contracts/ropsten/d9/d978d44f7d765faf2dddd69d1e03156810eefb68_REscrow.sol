...
contract REscrow is EscrowProducer, EscrowProvider {
    ...
    function addProvider(uint[] memory _collectedAssets) public returns(string memory){
        ...
        {
            (ProducerInfoStruct memory producerInfo, uint[] memory matchedAssets, uint redeemedAmount, uint rebateReceivedAmount) = EscrowProducer.findMatchedAssets(i, _collectedAssets);
            ProviderStruct memory newProvider;
