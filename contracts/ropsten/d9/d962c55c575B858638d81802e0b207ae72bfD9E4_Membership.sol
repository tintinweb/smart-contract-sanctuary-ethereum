...
contract Membership is ERC1155, Ownable, Pausable {
    ...
    event TierUpdated(uint256 id, string name, uint256 price, uint256 max);

    constructor(string memory uri_, address payable beneficiary_, address signer_) ERC1155(uri_) {
        beneficiary = beneficiary_;
        signer = signer_;
        setTier(0, "Royal", 1.0, 100);
        setTier(1, "Black", 1.0, 100);
        setTier(2, "Diamond", 1.0, 100);
        setTier(3, "Gold", 1.0, 100);
        setTier(4, "Silver", 1.0, 100);
    }
    ...
    function setTier(uint256 id, string memory name, uint256 price_, uint256 max) public onlyOwner {
        tiers[id] = Tier(id, name, price_, max, 0);
        emit TierUpdated(id, name, price_, max);
    }
    ...
    function mint(uint256 amount, uint256 slotId, uint256 tokenId, uint256 validUntil, bytes memory signature) external payable whenNotPaused {
        ...
        require(tier.supply + amount <= tier.max, "Cannot mint over collection size");
    ...
    function rescueERC721(IERC721 tokenToRescue, uint256 tokenId) external onlyOwner {
        tokenToRescue.safeTransferFrom(address(this), _msgSender(), tokenId);
    }

    // @notice Rescue other tokens sent accidentally to this contract.
    function rescueERC20(IERC20 tokenToRescue) external onlyOwner {
