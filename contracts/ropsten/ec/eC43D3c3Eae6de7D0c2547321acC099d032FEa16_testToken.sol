...
contract testToken is  IERC20, Ownable {
    ...
    string private _name = "testTheToken";
    ...
    address public teamWallet = address(0x789);
    address public treasuryWallet = address(0x456);
    address public potWallet = address(0x123);
    ...
    uint8 public constant BLOCKCOUNT = 5; //Blacklist first 5 block buys 
    ...
    constructor() {
        ...
        transferFee.treasuryFee = 10;
        transferFee.teamFee = 10;
        transferFee.taxFee = 10;
        transferFee.appPot = 20;
        ...
        maxSellAmount = totalSupply().mul(125).div(100000); // 0.125% of the supply per transaction
        maxBuyAmount = totalSupply().mul(125).div(100000); //  0.125% of the supply per transaction
        maxWalletAmount = totalSupply().mul(5).div(1000);  // 0.5% of the supply per wallet limit
    ...
    function setCooldown(uint256 _timeInSeconds) external onlyOwner {
    ...
    function setBuyFee(
        ...
        buyFee.treasuryFee = market;
        buyFee.teamFee = liq;
        buyFee.taxFee = tax;
        buyFee.appPot = apot;

        uint256 totalBuyFee = market + liq + tax + apot;
        require(totalBuyFee <= 200, "max buy fees limit is 20%");
    }
    ...
    function setSellFee(
        ...
        sellFee.teamFee = liq;
        sellFee.treasuryFee = market;
        sellFee.taxFee = tax;
        sellFee.appPot = apot;
        uint256 totalSellFee = market + liq + tax + apot;
        require(totalSellFee <= 200, "max buy fees limit is 20%");
    }
    ...
    function setTransferFee(
        ...
        transferFee.teamFee = liq;
        transferFee.treasuryFee = market;
        transferFee.taxFee = tax;
        transferFee.appPot = apot;
        uint256 totalTransferFee = market + liq + tax + apot;
        require(totalTransferFee <= 200, "max buy fees limit is 20%");
    }
    ...
    function setMaxWallet(uint256 value) external onlyOwner {
        require (value > 1000000, "can't set it lower.no rug pull");
        maxWalletAmount = value * 10**9;
    }

    function setMaxBuyAmount(uint256 value) external onlyOwner {
        require (value > 100000, "can't set it lower. no rug pull");
    ...
    function setTreasuryWallet(address payable wallet) external onlyOwner {
        require(wallet != address(0), "treasuryWallet address can not be zero!");
        treasuryWallet = wallet;
    }

    function setTeamWallet(address payable wallet) external onlyOwner {
        require(wallet != address(0), "teamWallet address can not be zero!");
        teamWallet = wallet;
    }

    function setPotWallet(address payable wallet) external onlyOwner {
        require(wallet != address(0), "potWallet address can not be zero!");
        potWallet = wallet;
    }
        ...
        )
    {
        ...
        tTransferAmount = tTransferAmount.sub(tTreasury).sub(tPot);
    ...
    function _takeTeam(uint256 tTeam) private {
        if (tTeam > 0) {
            ...
            emit Transfer(msg.sender, teamWallet, tTeam);
        }
    }
    ...
    function _takeTreasuryAndPot(uint256 tTreasury, uint256 tPot) private {
        if (tTreasury > 0 || tPot > 0) {
            ...
            _rOwned[potWallet] = _rOwned[potWallet].add(rPot);
            if (_isExcluded[treasuryWallet]) {
                _tOwned[treasuryWallet] = _tOwned[treasuryWallet].add(tTreasury);
            }
            if (_isExcluded[potWallet]) {
                _tOwned[potWallet] = _tOwned[potWallet].add(tPot);
            }

            emit Transfer(msg.sender, treasuryWallet, tTreasury);
            emit Transfer(msg.sender, potWallet, tPot);
        }
    }
    ...
    function calculateTeamFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_teamFee).div(10**3);
    }
    ...
    function calculateTreasuryFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_treasuryFee).div(10**3);
    }

    function calculatePotFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_potFee).div(10**3);
    }

    function burn(uint256 amount) external {
        _transfer(msg.sender, address(0xdead), amount * 10**9);
    }
