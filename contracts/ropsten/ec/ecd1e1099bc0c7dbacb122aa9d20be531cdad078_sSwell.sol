...
contract sSwell is IsSWL, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgSWL public gSWL; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked SWL", "sSWL", 9) ERC20Permit("Staked SWL") {
    ...
    function setgSWL(address _gSWL) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gSWL) == address(0), "gSWL:  gSWL already set");
        require(_gSWL != address(0), "gSWL:  gSWL is not a valid contract");
        gSWL = IgSWL(_gSWL);
    }
    ...
    /**
        @notice increases rSWL supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sSWL of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sSWL
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sSWL: insufficient balance");
    }
    ...
    // toG converts an sSWL balance to gSWL terms. gSWL is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gSWL.balanceTo(amount);
    }

    // fromG converts a gSWL balance to sSWL terms. sSWL is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gSWL.balanceFrom(amount);
    }

    // Staking contract holds excess sSWL
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gSWL.balanceFrom(IERC20(address(gSWL)).totalSupply())).add(
