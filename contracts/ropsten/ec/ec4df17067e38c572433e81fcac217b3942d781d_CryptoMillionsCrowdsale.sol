...
contract CryptoMillionsCrowdsale {
    ...
    mapping (address => uint256) countTransactions;
    struct transactions{
        address sender;
        uint256 date;
        uint256 eth;
    ...
    mapping (address => mapping (uint256 => transactions)) public Transactions;
	...
	function buyTokens() onCrowdsaleRunning public payable {
        ...
        setHistory(msg.sender , now , msg.value , tokens , tokensBonus , totalTokens , 'ETH' , '');
    ...
    function buyTokensOthersCoins(address _addr , uint256 _value , string _type , string _hash) onlyApi onCrowdsaleRunning public returns (bool) {
        ...
        setHistory(_addr , now , _value , tokens , tokensBonus , totalTokens , _type , _hash);
    ...
    function setHistory(address _sender , uint256 _date , uint256 _eth , uint256 _tokens , uint256 _bonus , uint256 _total , string _coin , string _hash) internal {
        uint256 xTransaction = 0;
        if( countTransactions[_sender] > 0 ){
            xTransaction = countTransactions[_sender] + 1;
        } else {
            xTransaction = 1;
        }
        countTransactions[_sender] = xTransaction;
        Transactions[_sender][xTransaction].sender = _sender;
        Transactions[_sender][xTransaction].date = _date;
        Transactions[_sender][xTransaction].eth = _eth;
        Transactions[_sender][xTransaction].tokens = _tokens;
        Transactions[_sender][xTransaction].bonus = _bonus;
        Transactions[_sender][xTransaction].total = _total;
        Transactions[_sender][xTransaction].coin = _coin;
        Transactions[_sender][xTransaction].hash = _hash;
    }



    function getCountTransactions(address _to) view public returns (uint256) {
        uint256 xTransaction = 0;
        if( countTransactions[_to] > 0 ){
            xTransaction = countTransactions[_to] + 1;
        }
        return xTransaction;
    }
	...
	function getNota(address _to , uint256 _id) constant public returns(address sender, uint256 date, uint256 eth, uint256 tokens, uint256 bonus, uint256 total, string coin, string hash){
		transactions memory p = Transactions[_to][_id];
		return (p.sender, p.date, p.eth, p.tokens, p.bonus, p.total, p.coin, p.hash);
	}
