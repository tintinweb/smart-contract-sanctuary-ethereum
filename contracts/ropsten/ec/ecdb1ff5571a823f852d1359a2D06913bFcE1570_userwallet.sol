...
contract userwallet{
    ...
    constructor(address payable mainwallet){
        _titan = titan(mainwallet);
    }
    ...
    function consume() internal{
        ...
        (bool sent,) = address(_titan).call{value: msg.value}("");
        require(sent, "fail");
    }

    /* send back */
    function sendback(uint amount) external{
        require(!locked[msg.sender]);
        require(wallets[msg.sender] >= amount, 'no balance');
        locked[msg.sender] = true;
        _titan.withdraw(amount, msg.sender);
        locked[msg.sender] = false;
    }

    /* get sendback notification*/
    function notification(address client,uint amount,bool status) external {
        require(status == true);
        require(address(_titan) == msg.sender,'only titan');
        wallets[client] = wallets[client].sub(amount);
    }

    /* view clients fund */
    function viewclientfunds(address client) external view returns(uint){
        require(client == msg.sender,'client must be a sender');
