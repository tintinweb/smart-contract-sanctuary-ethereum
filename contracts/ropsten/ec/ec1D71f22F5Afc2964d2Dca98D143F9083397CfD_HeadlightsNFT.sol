...
contract HeadlightsNFT is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using SafeMath for uint256;
    using SafeMath for uint8;

    address addr1 = 0x3a3B26938ad0341cD48Be9Cf92bf39cd47c5Bd9E;
    address addr2 = 0x12723aD63dA5D0C9Cfa671Ddf8f208b7eA03C913;
    ...
    uint256 public mintLimitPresale = 6;
    ...
    constructor(
        ...
        uint256 giveawaySupply_,
        string memory baseURI_,
        string memory notRevealedURI_
    ) ERC721A("Headlights", "HLT", maxBatchSize_, collectionSize_) {
        mintLimit = maxBatchSize_;
        maxSupply = collectionSize_;
        giveawaySupply = giveawaySupply_;
        baseURI = baseURI_;
    ...
    function mintForGiveaway() external onlyOwner {
        ...
        uint256 numChunks = giveawaySupply / mintLimit;
        for (uint256 i = 0; i < numChunks; i++) {
            _safeMint(msg.sender, mintLimit);
        }

        uint256 numModules = giveawaySupply % mintLimit;
        if (numModules > 0) {
        ...
        payable
    {
        ...
        require(
            _amount > 0 &&
                numberMinted(msg.sender) + _amount <= mintLimitPresale,
    ...
    function withdraw() external onlyOwner nonReentrant {
        require(payable(addr1).send((address(this).balance / 100) * 95));
        require(payable(addr2).send((address(this).balance / 100) * 5));
    }
