...
contract PaybLaunchPad is Ownable, ReentrancyGuard {
    ...
    // ) external onlyOwner {
        ...
        // emit
        emit AddLaunchpad(uint24(launchpads.length),name, address(stakeToken), _rootId);
    }
    ...
    //set Winers
    function setWinners(uint24 launchpadId, address[] memory _winners) public onlyOwner(){
        // require launchpad is disabled
        require(launchpadDisabled[launchpadId], 'launchpad !disabled');
    ...
    function claim(uint24 launchpadId) external nonReentrant{
        ...
        // check staked
        require(amountRefund >= 0, 'staked < 0');
