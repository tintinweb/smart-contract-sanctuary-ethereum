    ...
    UUPSUpgradeable
{
    ...
    function setStakeLimits() internal onlyOwner {

         stakesLimit[0] = 250000;
         stakesLimit[1] = 400000;
    ...
    function stakeOf(address _stakeholder) public view returns (uint256) {
        return stakeOfByTier(_stakeholder, 0).add(stakeOfByTier(_stakeholder, 1)).add(stakeOfByTier(_stakeholder, 2));
    }
    ...
    function totalStakes() public view returns (uint256) {
        ...
        for (uint256 s = 0; s < stakeholders.length; s += 1) {
            for (uint8 t = 0; t < stakes[stakeholders[s]].length; t += 1) {
                _totalStakes = _totalStakes.add(stakes[stakeholders[s]][t]);
            }
        }
        return _totalStakes;
    }
    ...
    * @param _tier The tier to stake (tier 0 to tier 2).
    ...
    function createStake(uint256 _stake, uint8 _tier) public {
        ...
        // Validation: New staked amount > tier threshold
        uint256 newStakedAmount = stakes[msg.sender][_tier].add(_stake);
        require(stakesLimit[_tier] == 0 || newStakedAmount <= stakesLimit[_tier], "Total staking amount above Tier Threshold");

        // Add the stakeholder if needed
        addStakeholder(msg.sender);

        // Stake !
        stakes[msg.sender][_tier] = newStakedAmount;
    ...
    function removeStake(uint256 _stake, uint8 _tier) public { 
        ...
        require(stakes[msg.sender][_tier] >= _stake, "Insufficient staked amount to be unstaked");

        // Unstake !
        stakes[msg.sender][_tier] = stakes[msg.sender][_tier].sub(_stake);
