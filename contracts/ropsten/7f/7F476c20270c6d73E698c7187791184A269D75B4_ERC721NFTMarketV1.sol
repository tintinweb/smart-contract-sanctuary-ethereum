...
// import "contracts/interfaces/ICollectionWhitelistChecker.sol";
...
contract ERC721NFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    address public immutable WBNB; // solhint-disable-line
    ...
    constructor(
        ...
        address _WBNBAddress, // solhint-disable-line
        ...
        require(_minimumAskPrice > 0, "_minimumAskPrice must be > 0");
        require(_minimumAskPrice < _maximumAskPrice, "_minAskPrice < _maxAskPrice");
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        require(revenueToClaim != 0, "Nothing to claim");
    ...
    function createAskOrder(
        ...
        require(_askPrice >= minimumAskPrice && _askPrice <= maximumAskPrice, "Price not within range");

        // Verify collection is accepted
        require(_collections[_collection].status == CollectionStatus.Open, "Not for listing");

        // Verify token has restriction
        require(_canTokenBeListed(_collection, _tokenId), "tokenId not eligible");
    ...
    function modifyAskOrder(
        ...
        require(_newPrice >= minimumAskPrice && _newPrice <= maximumAskPrice, "Price not within range");
    ...
    function addCollection(
        ...
        require(IERC721(_collection).supportsInterface(0x80ac58cd), "Not ERC721");

        require(
            (_creatorFee == 0 && _creator == address(0)) || (_creatorFee != 0 && _creator != address(0)),
            "Creator parameters incorrect"
        );

        require(_tradingFee + _creatorFee <= TOTAL_MAX_FEE, "Sum of fee > TOTAL_MAX_FEE");
    ...
    function closeCollectionForTradingAndListing(address _collection) external onlyAdmin {
        require(_collectionAddressSet.contains(_collection), "Collection not listed");
    ...
    function modifyCollection(
        ...
        require(_tradingFee + _creatorFee <= TOTAL_MAX_FEE, "Sum of fee > to TOTAL_MAX_FEE");
    ...
    function recoverFungibleTokens(address _token) external onlyOwner {
        require(_token != WBNB, "Cannot recover WBNB");
    ...
    function recoverNonFungibleToken(address _token, uint256 _tokenId) external onlyOwner nonReentrant {
        require(!_askTokenIds[_token].contains(_tokenId), "NFT not recoverable");
    ...
    function _buyToken(
        ...
        require(_collections[_collection].status == CollectionStatus.Open, "Not for trading");
        require(_askTokenIds[_collection].contains(_tokenId), "Not for sale");
