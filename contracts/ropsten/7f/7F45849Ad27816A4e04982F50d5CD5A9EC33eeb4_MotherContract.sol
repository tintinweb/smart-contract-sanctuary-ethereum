...
contract MotherContract {
  ...
  function createPool(uint256 amount, string memory name, address base) external payable{
    LiquidityPool pool;
    ...
    if(base==address(0x0)){
      pool.init{value:amount}(msg.sender,name,base,lpTokenTemplate, lpErc1155Template);
      emit PoolCreated(address(pool),msg.value);
    }
    else {
      erc20Token = IERC20(base);
      erc20Token.approve(address(pool),amount);
      pool.init(msg.sender,amount,name,base,lpTokenTemplate, lpErc1155Template);
      emit PoolCreated(address(pool),amount);
    }
...
contract LiquidityPool is Initializable, OwnableUpgradeable {
    ...
    address public ROUTER;//Uniswap-like router address
    ...
    mapping (address => uint256) public tokenInvestedAmount;//base amount of every token at invest time
    ...
    function init(address owner,string memory _name, address _base, address _lpTokenTemplate, address _lpErc1155Template) external payable initializer {
        _init(owner,msg.value,_name,_base,_lpTokenTemplate,_lpErc1155Template);
    }

    function init(address owner,uint256 amount,string memory _name, address _base, address _lpTokenTemplate, address _lpErc1155Template) external initializer {
    ...
    function tokenize(string memory symbol) external onlyOwner{
  ...
  function _deposit(address investor, uint256 amount) internal {
    ...
    if(base!=address(0x0)){
        tokenBalances[base] = IERC20(base).balanceOf(address(this));
    }
    else {
        tokenBalances[base] = address(this).balance;
    }
    tokenInvestedAmount[base]=tokenBalances[base];
  ...
  function estimateWithdraw() external view returns (Withdraw memory){
   ...
   function approveSwap(address from, address to, uint256 amount) public onlyOwner {
       ...
       swaps.push(Swap(from,to,amount,swaps.length,false));
       if(!tokenExist[to]) {
           tokens.push(to);
           tokenExist[to] = true;
       }
   ...
   function sync(uint256 swap_id) internal onlyOwner {
       uint256 delta;
       Swap storage _swap;
       _swap = swaps[swap_id];
       ...
       if (_swap.from==base) {
           ...
           delta = IPriceFeed(oracle).getRate(IERC20(_swap.from),IERC20(base),false)*_swap.amount/1000000000000000000;
       }

        tokenInvestedAmount[_swap.from] = tokenInvestedAmount[_swap.from] - delta;
        tokenInvestedAmount[_swap.to] = tokenInvestedAmount[_swap.to] + delta;

        tokenBalances[_swap.from]=IERC20(_swap.from).balanceOf(address(this));
        tokenBalances[_swap.to]=IERC20(_swap.to).balanceOf(address(this));
        _swap.synced = true;
   }
