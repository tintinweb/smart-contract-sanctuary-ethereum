...
contract SecretVerseNFTDistributor is AccessControl {
    ...
    uint256 internal usersMintCount; // 9650 for users
    uint256 internal adminMintCount; // 350 for admin
    ...
    function mint(uint256 _num) public payable returns (bool) {
        ...
        require(_num > 0, "Should mint atleast 1 Nft");
        require(
        ...
        require(
            ...
            "ONLY WHITELIST or SHOULD HOLD PRE-REQ NFTS"
        ); //either on whitelist or holds PRE-REQ NFTS or whitelist is false

        require(
            whitelistOnly ||
                (!whitelistOnly &&
                    totalMintsPerAddress[msg.sender] + _num <= 20),
            "You can mint a maximum of 20"
        ); //whitelist is false, and you can max mint 20
        require(
            !whitelistOnly ||
                (whitelistOnly &&
                    totalMintsPerAddress[msg.sender] + _num <= 10),
            "You can mint a maximum of 10 when whitelist enabled"
        ...
        require(
            ...
            "Maximum cap of 10k mints reached"
        );
    ...
    function adminMint(address _to, uint256 _quantity) public returns (bool) {
        ...
        require(_quantity > 0, "Should mint atleast 1 NFT");
    ...
    // checks whether user holds one of the NFTS of "holderNfts" contract address
    function doesHoldNfts() public view returns (bool) {
    ...
    function giveAway(address _to, uint256 _amount) external {
        ...
        require(
            cbToken.totalSupply() + _amount <= cbToken.cap(),
            "Cap reached, maximum 10000 mints possible"
        );
