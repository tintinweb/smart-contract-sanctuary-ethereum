...
contract Settlement is EIP712 {
    ...
    bytes32 private constant _SIGN_TYPEHASH = keccak256("RFQI(uint256 offerId, address seller, uint256 nonce)");
    struct signData {
        uint256 offerId;
        address seller;
        uint8 v; // v
    ...
    function getSignerr(signData calldata _bidData) external view returns (address) {
        bytes32 structHash = keccak256(
            abi.encode(
                _SIGN_TYPEHASH,
                _bidData.offerId,
                _bidData.seller,
                _nonces[_bidData.seller]
            )
        );
        bytes32 hash = _hashTypedDataV4(structHash);
        return ECDSA.recover(
