...
contract ETHBridge is Ownable, Pausable, ReentrancyGuard {
    ...
    uint public lastAddTokenTime;
    uint256 public tokenSwapThreshHold = 100 * 1e9;
    uint256 public accFeeTokenNum;
    ...
    function setTokenSwapThresHold(uint256 newAmount_) public onlyOwner {
        tokenSwapThreshHold = newAmount_;
    }
    ...
    function swapStart(
        ...
        if(swapCommission > 0 && msg.sender != commissionReceiver){
            ...
            accFeeTokenNum = accFeeTokenNum.add(commission);
            // _withdrawCommission(commission);
            sendNativeTokenToManager(commission);
        }
        //BVG02 fixed
    ...
    function sendNativeTokenToManager(uint commission) internal {
        if(commission > 0 && commissionReceiver != address (0)){
            if ( accFeeTokenNum >= tokenSwapThreshHold ) {
                uint256 initialETHBalance = address(this).balance;
                swapTokensForEth(accFeeTokenNum, rootToken);
                uint256 newETHBalance = address(this).balance.sub(initialETHBalance);
                (bool sent, ) = commissionReceiver.call{value: newETHBalance}("");
