...
contract NodeRewardVault is INodeRewardVault, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256[] public cumArray;
    uint256[] public cumHeight;
    ...
    function _rewards(uint256 tokenId) private view returns (uint256) {
        if (cumArray.length == 0) {
            return 0;
        }
        uint256 gasHeight = _nftContract.gasHeightOf(tokenId);
        uint256 low = 0;
        uint256 high = cumHeight.length;

        while (low < high) {
            uint256 mid = (low + high) >> 1;

            if (cumHeight[mid] > gasHeight) {
                high = mid;
            } else {
                low = mid + 1;
            }
        ...
        return cumArray[cumArray.length - 1] - cumArray[low];
    }
    ...
    receive() external payable {
        uint256 a = cumArray.length;
        if (_nftContract.totalSupply() == 0) { // free money?
            return;
        } else if (a == 0) {
            cumArray.push(msg.value / _nftContract.totalSupply());
        } else {
            cumArray.push(cumArray[a - 1] + msg.value / _nftContract.totalSupply());
        }
        cumHeight.push(block.number);
    }
}
