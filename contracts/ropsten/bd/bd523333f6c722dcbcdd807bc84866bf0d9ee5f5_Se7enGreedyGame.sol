...
library GreedyKeysCalcLong {
    ...
    function keys(uint256 _eth)
        ...
        returns(uint256)
    {
        // 由于fomo3d 的价格曲线，走势缓慢，我们对 key 放大了400倍处理，用以拉动kkey的价格快速增长，结束本轮游戏.
        return (((  (   ((_eth).mul(1000000000000000000)).mul(312500000000000000000000000)) .add(5624988281256103515625000000000000000000000000000000000000000000)).sqrt()).sub(74999921875000000000000000000000)) / (156250000) / 400;
    }
    ...
    function eth(uint256 _keys)
        ...
        returns(uint256)
    {
        return ((78125000).mul(_keys.sq().mul(160000)).add(((149999843750000).mul(_keys.mul(1000000000000000000).mul(400))) / (2))) / ((1000000000000000000).sq());
    }
}
