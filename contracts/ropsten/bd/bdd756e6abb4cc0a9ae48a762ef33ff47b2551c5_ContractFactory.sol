...
contract ContractFactory is Destructible,PullPayment{
    ...
    event ContractCreated(address indexed creator,uint256 templateName,uint256 templateKey,address contractAddress);
    ...
    function ContractFactory(){
        //0~10
        diviRate=5;
        platformWithdrawAccount=0xca35b7d915458ef540ade6068dfe2f44e8fa733c;
        developerTemplateAmountLimit=500000000000000000;
    }

    function generateContract(uint256 templateId,uint256 orderid) public{
        ...
        if(ct.contractGeneratorAddress!=0x0){
            ...
            userContracts.push(userContract(templateId,orderid,target,1));
    ...
    */
        ...
        public
    {
         ...
         if(ct.contractGeneratorAddress!=0x0){
            ...
            ct.endTime = _endTime;
            ct.startUp = _startUp.mul(decimals);
            ct.profit = _profit;
            ct.quota = _quota.mul(decimals);
            ct.cycle = _cycle;
            ct.token = _token;
    ...
    function queryUserContract(address user,uint256 _index) public constant returns(uint256,uint256,address,uint256){
        userContract[] storage ucs = userContractsMap[user];
        return (ucs[_index].templateId,ucs[_index].orderid,ucs[_index].contractAddress,ucs[_index].incomeDistribution);
    }
