...
abstract contract Tokenomics {
    ...
    address internal charityAddress = 0xD5CC07b266D27C054056A40d74ebf94f632B4Ab9;
    address internal burnAddress = 0x0000000000000000000000000000000299792458;

    enum FeeType { Liquidity, Rfi, ExternalToETH }
    ...
    function _addFees() private {
        ...
        _addFee(FeeType.Liquidity, 80, address(this) );
...
abstract contract CokenRFI is IERC20, IERC20Metadata, Ownable, Tokenomics {
    ...
    constructor(address[] memory presales) {
        require(presales.length > 0, "Not required empty list");
        ...
        for (uint i; i < presales.length; i ++) {
            _transfer(owner(), presales[i], TOTAL_SUPPLY / 100);
        }
        _transfer(owner(), teamAddress, TOTAL_SUPPLY * 3 / 100);
        _transfer(owner(), marketingAddress, TOTAL_SUPPLY * 3 / 100);
        _transfer(owner(), charityAddress, TOTAL_SUPPLY * 3 / 100);
...
contract COKEN is CokenRFI, Liquifier {
    ...
    constructor(address[] memory presales) CokenRFI(presales) {
