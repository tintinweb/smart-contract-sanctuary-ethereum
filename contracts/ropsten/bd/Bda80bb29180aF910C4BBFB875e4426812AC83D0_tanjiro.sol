...
contract tanjiro is Context, IERC20, Ownable {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 percentMar = (_totMar.div(_totSwap));
        uint256 tokensForMar = (contractTokenBalance.mul(percentMar));
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(antiBotLaunch){
            if(block.number <= (firstLiveBlock.add(6)) && sender == uniswapV2Pair && recipient != address(uniswapV2Router) && recipient != address(this)){
