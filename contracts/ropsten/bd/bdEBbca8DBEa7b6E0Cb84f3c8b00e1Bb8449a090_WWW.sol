...
contract WWW is ERC721, AccessControl  {
  ...
  constructor(address _privateMinterRoleAddress) ERC721("Tests W W W", "TWW") {
      _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
      _setupRole(PRIVATE_MINTER_ROLE, msg.sender);
      _setupRole(PRIVATE_MINTER_ROLE, _privateMinterRoleAddress);
  ...
  function mintForAddress(uint256 _mintAmount, address _receiver) public mintCompliance(_mintAmount) onlyRole(PRIVATE_MINTER_ROLE) {
  ...
  function setOnlyWhitelisted(bool _state) public onlyRole(DEFAULT_ADMIN_ROLE) {
    onlyWhitelisted = _state;
  }

  function setMerkleRoot(bytes32 _merkle) public onlyRole(DEFAULT_ADMIN_ROLE) {
    merkleRoot = _merkle;
  }
