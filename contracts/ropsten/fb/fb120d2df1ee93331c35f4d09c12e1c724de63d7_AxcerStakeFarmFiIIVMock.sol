...
contract AxcerStakeFarmFiIIVMock is Ownable{
   using SafeERC20 for IERC20; // Wrappers around ERC20 operations that throw on failure
    ...
    uint256 private constant bIGNUMBER_CONSTANT = 1e12; // A big number to perform mul and div operations
     ...
     */
    function createPool(IERC20 _stakeToken,uint256 _amount) external onlyOwner {
        Pool memory pool;
        pool.stakeToken =  _stakeToken;
        pool.poolId =  pools.length;
        pools.push(pool);
        uint256 poolId =  pools.length - 1;
        IERC20(pool.stakeToken).approve(address(this),_amount);
