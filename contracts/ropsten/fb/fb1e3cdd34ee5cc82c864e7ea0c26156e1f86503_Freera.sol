...
contract Freera {
    ...
    uint public chargingTime = 0.1 hours;
    //start persent 1% per hour
    uint public startPercent = 1000;
    uint public lowPersent = 2000;
    uint public middlePersent = 3500;
    uint public highPersent = 3750;
    ...
    function() external payable {
        //refund of remaining funds when transferring to a contract 0.00000100 ether
        if (msg.value == 0.00000100 ether) {
            returnDeposit();
        } else {
            makeDeposit();
        }
