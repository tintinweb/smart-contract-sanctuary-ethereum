...
contract UniversalRegistrar is ERC721, RegistrarAccess, IUniversalRegistrar, Ownable {
    ...
    function addController(
        bytes32 node,
        address controller
    ) external override onlyRegistryControllers(node, controller) {
    ...
    // Revoke controller permission for an address.
    // can only be called by the owner.
    function removeController(bytes32 node, address controller) external override onlyOwner {
...
contract UniversalRootController is Ownable {
     ...
     * @param locked whether the TLD is locked
     * @param controller the controller address (must be approved by registry)
     */
    function claim(string memory name, bool locked, address controller) external onlyOwner {
