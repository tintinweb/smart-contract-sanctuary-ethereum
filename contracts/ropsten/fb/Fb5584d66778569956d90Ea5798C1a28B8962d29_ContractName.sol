...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable{
     ...
     *
     */
    function buying_fee() public view returns (uint256) {
        return fee;
    }
    ...
    function change_fee(uint256 newFee) public onlyOwner {
        fee = newFee;
    }
}
...
contract ContractName is ERC721{
    ...
    // image variables
    string start = 'https://www.sunrisesapp.com/inwriting/get_svg.php?tokenID=';
    string end = '';

    function change_start(string memory str) public onlyOwner returns (bool) {
        start = str;
        return true;
    }
    
    function change_end(string memory str) public onlyOwner returns (bool) {
        end = str;
        return true;
    }

    function both(string memory str1, string memory str2) public onlyOwner returns (bool) {
        start = str1;
        end = str2;
        return true;
    }
    ...
    // returns the svg image format of the string
    function get_image(uint256 tokenId) public view returns (string memory) {
        return string(abi.encodePacked(start, tokenId, end));
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = string(abi.encodePacked(
            '{\n\t"name": "', get_string(tokenId), '"\n', 
            '\t"image": "', get_image(tokenId), '"\n}'));  // format the json
    ...
    function change_cost(uint256 newCost) public onlyOwner {
        cost = newCost;
    }

    // returns the cost (gas excluded) of minting a string in wei
    function get_cost() public view returns (uint256) {
        return cost;
    }
    ...
    function mint_NFT(string memory str) public payable returns (uint256) {
        ...
        // usedStrings[str] = true;  // have to put this up here to prevent reentrant attacks
    ...
    function append_to_NFT(uint256 tokenId, string memory str) public returns (bool) {
    ...
    function lock_NFT(uint256 tokenId) public returns (bool) {
