...
contract PayslicEscrow is Ownable {
    enum State { AWAITING_DELIVERY, AWAITING_CONFIRMATION, COMPLETE }
  
    uint256 public FEE_PERCENT = 3;
    address private _vaultAddress = 0x80eeb0a5AE88cdE526810951F4D7C876B0D6f5F5;
    ...
    function setContractFee(uint256 fee) external onlyOwner {
        FEE_PERCENT = fee;
    }
    ...
    function buyConfirm(string memory id) onlyBuyer(id) external {
        ...
        payable(_vaultAddress).transfer(calcContractFee(txns[id].amount)); //Send fee to treasury
    }
    ...
    function refundBuyer(string memory id) external onlyOwner {
        txns[id].buyer.transfer(txns[id].amount);
        txns[id].currState = State.COMPLETE;
        txns[id].isActive = false;
    }

    function refundSeller(string memory id) external onlyOwner {
