    ...
    UUPSUpgradeable
{
    ...
    function setStakeLimit(uint8 _tier, uint256 _newLimit) internal onlyOwner {

        stakesLimit[_tier] = _newLimit * 10**decimals();
    }
    ...
    * @param _tier The tier to stake (tier 0 to tier 2).
    ...
    function createStakeInternal(address _stakeholderAddress, uint256 _stake, uint8 _tier, bool _isOverride) internal {
        ...
        // Validation: Sufficient token for staking
        require(stakes[_stakeholderAddress][_tier] < _stake, "Insufficient token for staking");
        ...
        // require(!_isOverride && (stakesLimit[_tier] == 0 || newStakedAmount <= stakesLimit[_tier]), "Total staking amount above tier Threshold");
    ...
    function removeStake(uint256 _stake, uint8 _tier) public { 
        ...
        removeStakeInternal(msg.sender, _stake, _tier, false);
    }
     ...
     * @param _tier The tier to reomve the stake from (tier 0 to tier 2).
     */
    function removeStakeByAdmin(address _stakeholderAddress, uint256 _stake, uint8 _tier) public onlyOwner { 

        removeStakeInternal(_stakeholderAddress, _stake, _tier, true);
    }
     ...
     * @param _isOverride whether the call is an overriding one for tier threshold and staking rules.
     */
    function removeStakeInternal(address _stakeholderAddress, uint256 _stake, uint8 _tier, bool _isOverride) public { 
        ...
        // TODO calcuate penalty amount (if applicable)
        ...
        // Finally, mint the stake amount to the client's wallet
        _mint(_stakeholderAddress, _stake);
    }
}
