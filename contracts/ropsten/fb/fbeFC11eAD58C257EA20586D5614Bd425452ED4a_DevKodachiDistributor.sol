...
// KodachiToken locker contract.
contract KodachiLocker is Ownable {
    ...
    constructor(address _kodachiToken, address _emergencyAddress, uint256 _lockingPeriodInDays, uint256 _devLockingPeriodInDays) public {
        require(address(_kodachiToken) != address(0), "_kodachi token is a zero address");
        require(address(_emergencyAddress) != address(0), "_emergencyAddress is a zero address");
        kodachiToken = IERC20(_kodachiToken);
    ...
    // function to lock user reward kodachi tokens in token contract, called by onlyOwner that would be TopDog.sol
    ...
    function claimAllForUser(uint256 r, address user) public {
        ...
        kodachiToken.transfer(user, totalTransferableAmount);
    }
    ...
    function emergencyWithdrawOwner(address _to) external onlyOwner{
        uint256 amount = kodachiToken.balanceOf(address(this));
        require(kodachiToken.transfer(_to, amount), 'MerkleDistributor: Transfer failed.');
    }
...
contract DevKodachiDistributor is Ownable {
    using SafeMath for uint256;

    IERC20 public kodachi;
    KodachiLocker public kodachiLocker;
    ...
    constructor (
        IERC20 _kodachi,
        KodachiLocker _kodachiLocker,
        address _devWallet,
        address _marketingAndGrowthWallet
    ) public {
        require(address(_kodachi) != address(0), "_kodachi is a zero address");
        require(address(_kodachiLocker) != address(0), "_kodachiLocker is a zero address");
        kodachi = _kodachi;
        kodachiLocker = _kodachiLocker;
    ...
    function kodachiBalance() external view returns(uint) {
        return kodachi.balanceOf(address(this));
    }
    ...
    function distribute(uint256 _total) external onlyOwner {
        ...
        require(kodachi.transfer(devWallet, devWalletShare), "transfer: devWallet failed");
        require(kodachi.transfer(marketingAndGrowthWallet, marketingAndGrowthWalletShare), "transfer: marketingAndGrowthWallet failed");
    }

    // funtion to claim the locked tokens for devKodachiDistributor, which will transfer the locked tokens for dev to devAddr after the devLockingPeriod
    function claimLockedTokens(uint256 r) external onlyOwner {

        kodachiLocker.claimAll(r);
    }
    // Update kodachiLocker address by the owner.
    function kodachiLockerUpdate(address _kodachiLocker) public onlyOwner {
        kodachiLocker = KodachiLocker(_kodachiLocker);
    }
}