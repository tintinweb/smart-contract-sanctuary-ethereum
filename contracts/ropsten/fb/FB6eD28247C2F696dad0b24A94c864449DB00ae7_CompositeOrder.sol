 ...
 * @member totalSlices number of slices to split order into
...
contract CompositeOrder is Ownable, EIP712 {
	 ...
	 */
	event SetAnboto(
		address indexed anboto,
		bool set
	);

	bool public constant TEST = true;

	/// @dev 21000 is base tx gas, 15000 is an added estimation gas considering the cost of calldata, gas refund and storage changes after gas calculation
	uint256 private constant ADDITIONAL_TX_GAS = 21_000 + 15_000;
	...
	/// @notice Holds reimbursed gas
	uint256 public anbotoReimbursedGas;

	/// @notice Holds execution count by order signature to prevent replay attacks executing order more then number of slices
	mapping(bytes => uint256) public orderExecutionCount;
	...
	function executeOrder(
		...
		uint256 sliceInputAmount = order.totalAmount / order.totalSlices;
		...
		// if we have more input tokens then before, return the supplementary tokens to the maker
		if (inputBalanceAfter > inputBalanceBefore) {
			inToken.safeTransfer(maker, inputBalanceAfter - inputBalanceBefore);
			...
			revert("Too much input token spent");
		}
		...
		uint256 sliceOutMin = order.outMin / order.totalSlices;

		// if we have less output tokens then required by contract, revert the transaction
		require(outputAmount >= sliceOutMin, "Output amount bought too low");
	...
	function _verify(
		...
		require(msg.sender == owner() || isAnboto[msg.sender], "Caller not Anboto");
		// VERIFY ORDER PARAMETERS
		// should be called before deadline
		require(block.timestamp <= order.deadline, "Order deadline passed");
		// should be called within specified gas price limit
		require(tx.gasprice <= order.maxGasPrice, "Gas price too high");
		// should be called within specified fee limit
		require(order.feeAmount <= MAX_FEE, "Fee too high");
		// should be called with input and output tokens that differ
		require(order.inputToken != order.outputToken, "Input and output token must differ");
		// should be signed by order maker
		require(isValidSignature(order, maker, sig), "Signer and Maker do not match");
		// should be called at most total slices times
		require(orderExecutionCount[sig] < order.totalSlices, "Number of calls exceed number of slices");
	}
	...
	bytes32 private constant ORDER_TYPEHASH =
		keccak256(
			"Order(address inputToken,address outputToken,uint256 totalAmount,uint256 totalSlices,uint256 outMin,uint256 maxGasPrice,uint256 feeAmount,uint256 deadline,uint256 salt)"
		);
	...
	*/
	...
	function withdrawGas(uint256 amount) external {
		require(gasTank[msg.sender] >= amount, "Not enough gas");
	...
	/**
	 * @notice Claim Reimbursed Gas
	 * @dev This function transfers all of reimbursed gas amount to specified address
	 * Requirements:
	 * - should be called by owner
	 * - should be called with non-zero destination address
	 * - should be called when there is gas to reimburse
	 * @param claimTo destination address
	 */
	function claimReimbursedGas(address payable claimTo) external onlyOwner {
		// guard against zero addresses
		require(claimTo != address(0), "Cannot claim to zero address");
		uint256 _anbotoReimbursedGas = anbotoReimbursedGas;

		// guard against empty transfers
		require(_anbotoReimbursedGas > 0, "No gas to reimburse");
		anbotoReimbursedGas = 0;
		claimTo.transfer(_anbotoReimbursedGas);
	}
	...
	modifier trackGas(address maker) {
		...
		_;
		unchecked {
			uint256 gasUsed = gas + ADDITIONAL_TX_GAS - gasleft();
			uint256 coinGasUsed = gasUsed * tx.gasprice;
			
			// guard against lack of gas
			require(gasTank[maker] >= coinGasUsed, "Not enough gas in the tank.");

			gasTank[maker] -= coinGasUsed;
			anbotoReimbursedGas += coinGasUsed;
		}
	}
}