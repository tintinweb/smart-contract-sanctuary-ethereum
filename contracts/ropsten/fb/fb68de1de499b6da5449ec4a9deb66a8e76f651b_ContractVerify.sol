...
// File: contracts/ContractVerify.sol
...
contract ContractVerify is Context, IERC20, Ownable
{
    ...
    string private  _name = "satrata";
    string private  _symbol = "sat";
    
    uint8  private  _decimals = 0;
            uint256 public  _buytaxFee = 1;
            uint256 public  _buypreviousTaxFee = _buytaxFee;
            uint256 public  _buydevelopmentFee = 2;
            uint256 public  _buypreviousDevelopmentFee = _buydevelopmentFee;
            uint256 public  _buyliquidityFee = 1;
            uint256 public  _buypreviousLiquidityFee = _buyliquidityFee;
        ...
        bool takeFee = true;
         uint256 contractTokenBalance = balanceOf(address(this));
           ...
           bool  isSwapstarted=contractTokenBalance >=numTokensSellToAddToLiquidity;
            if (isSwapstarted && from != uniswapV2Pair && inSwapAndLiquify &&  swapAndLiquifyEnabled  && from != owner() && to != owner() )
            {
 ...
 function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
  ...
  (
        ...
        require(to != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        _tOwned[from] =_tOwned[from].sub(amount, "BEP20: transfer amount exceeds balance");
   ...
   function removeAllFee() private  {
        ...
        if(_buytaxFee == 0 && _buyliquidityFee == 0 && _buydevelopmentFee==0) return;
        _buypreviousTaxFee = _buytaxFee;
    ...
    function restoreAllFee() private
     {
        ...
        {
            _buytaxFee = amount.mul(_buypreviousTaxFee).div(100);
            _buydevelopmentFee = amount.mul(_buypreviousDevelopmentFee).div(100);
            _buyliquidityFee = amount.mul(_buypreviousLiquidityFee).div(100);
            _tOwned[address(this)] += _buytaxFee.add(_buydevelopmentFee).add(_buyliquidityFee);
            return (amount.sub(_buytaxFee).sub(_buydevelopmentFee).sub(_buyliquidityFee));
        }
