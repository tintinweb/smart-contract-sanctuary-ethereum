...
interface IAccountManagement {
    ...
    struct AccountInformationData {
        address accountId;
        AccountStatus accountStatus;
        string accountType;
        address parentAccountId;
        string owningInstitutionId;
    }

    function accountInformation(address accountId) external view returns(AccountInformationData calldata accountInformation);
    function addOwner(address accountId) external;
    function removeOwner(address accountId) external;
    function openAccount(address accountId, string calldata accountType, address parentAccountId, string calldata owningInstitutionId) external;
    function closeAccount(address accountId) external;
    function enableAccount(address accountId) external;
    function disableAccount(address accountId) external;
    function freezeAccount(address accountId) external;

    event AccountInformation(address indexed accountId, AccountStatus oldStatus, AccountStatus currentStatus);
    event OwnersUpdated(address indexed ownerAddress, string action);
}
...
contract ERC20 is IAccountManagement {
    ...
    function accountInformation2(address accountId) external view returns (address, AccountStatus, string memory, bool, address) {

        return (accountId, AccountStatus.open, "Business", true, address(0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2));
