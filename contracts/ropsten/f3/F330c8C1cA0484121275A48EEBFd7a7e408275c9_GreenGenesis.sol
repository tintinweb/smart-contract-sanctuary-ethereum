...
contract GreenGenesis {
    ...
    function swapIn(uint256 amount) external 
    {
        ...
        if (allocations[msg.sender].lastWithdrawalBlock == 0) {
            ...
            internalWithdrawShare(msg.sender);
        }
     ...
     */
    function internalWithdrawShare(address user) internal
    {
        require(allocations[user].lastWithdrawalBlock < endBlock, "All shares have already been claimed.");
        uint256 unlockedBlock;
        if (block.number > endBlock) {
            unlockedBlock = endBlock;
        } else {
