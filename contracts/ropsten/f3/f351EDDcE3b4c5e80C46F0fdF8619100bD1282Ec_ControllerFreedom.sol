...
import {CommunityMember, Community, CommunityStatus, MemberRole} from "../Community/Community.sol";
import {CommunityPoll, PollType} from "../Community/Poll.sol";
...
interface VotingPoll {
    function communityPollCreate(uint256 daoId, PollType pollType, string memory name, string memory purpose, string memory links,
		string memory logoImage, string memory coverImage, string memory legalStatus, string memory legalDocs, string memory votingConsensus, 
		address memberToAdd, address memberToRemove, string memory gasFee, string memory bond, string memory functionCode, 
		address bountyReceiver, string memory bounty) external returns (bool);
...
contract ControllerFreedom is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    *   @notice constructor 
    */
    constructor(address votingContractAddr ) {
	...
	function communityCreate(string memory name) external onlyProxy returns (bool) {
        ...
        _communitiesMembers[memberId] = communityMemberDetails;

		emit EvtCommunityCreate(commId, name);
        return true;
	}
