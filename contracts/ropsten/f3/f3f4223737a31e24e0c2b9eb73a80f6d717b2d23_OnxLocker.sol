...
contract OnxLocker is Initializable, ReentrancyGuardUpgradeable {
    ...
    function getBoostedAmount(
        ...
        if (staked.slpAddress != address(0)) {
            IERC20Upgradeable vault = IERC20Upgradeable(staked.token);
            IUniswapV2Pair slpUniV2 = IUniswapV2Pair(staked.slpAddress);

            uint256 ts = slpUniV2.totalSupply();
            uint256 b = vault.balanceOf(_staker);
    ...
    function _lock(
        ...
        uint256 boostedAmount = getBoostedAmount(_account, _token, _amount);
        ...
        IERC20Upgradeable(governanceToken).safeApprove(_account, 0);
