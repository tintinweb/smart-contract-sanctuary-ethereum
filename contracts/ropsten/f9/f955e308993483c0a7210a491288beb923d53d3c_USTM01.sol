...
contract USTM01 is ERC20Interface, Owned, SafeMath {
    ...
    function USTM01() public {
        ...
        _VaultToken  = ERC20Interface(0xD4981e26722a1Ba2280A37b5B741622709AE3278);
    }
    ...
    function IsControledAddress(address targetAddress) public constant returns (bool isControlled)
    {
    ...
    function IsDividendAvailabe() public constant returns (bool bAvailable)
    {
        return _isDividendAvailable;
    }
    
    function GetDivPerCoinLarge() public constant returns (uint DivPerCoin)
    {
        if (_isDividendAvailable== false)
        return 0;
        
        return _divdendSnapshots[_currentDivIteration]._divAmount;
    }
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
         ...
         if (tokens == 0) 
        {
            ...
            {
                uint div = LookUpAvailableDivLarge(msg.sender);
                {
                    if (div > 1 ether)
                    {
                        _VaultToken.transferFrom(address(this), msg.sender,div);
                        return true;
                    }
                }
            }
    ...
    function LookUpAvailableDivLarge(address to) private constant returns(uint amountLarge)
    {
        ...
        amountLargeInt = safeDiv(amountLargeInt,1 ether);
        
        
        return amountLargeInt;
    ...
    {
        uint AmountLarge = LookUpAvailableDivLarge(to);
        amountInt = safeDiv(AmountLarge,1 ether);
        return amountInt;
    }
    function LockDiv(address to, uint tokens) private
    {
        ...
        {
            _divdendSnapshots[_currentDivIteration]._divState[to] == 1;
