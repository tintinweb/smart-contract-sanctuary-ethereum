...
contract SaleNFT {
    ...
    IERC721 public mintNftTokenAddress;  
    IERC20 public currencyTokenAddress;
    uint256 feePercent;
    address payable feeAddress;
    address owner;
    // constructor    
    constructor(IERC721 _mintNftTokenAddress, IERC20 _currencyTokenAddress, address payable _feeAddress, uint256 _feePercent) {
        mintNftTokenAddress = _mintNftTokenAddress;
    ...
    function purchaseToken(uint256 _tokenId) external {
        ...
        for(uint256 i = 0; i < onSaleTokenArray.length; i++) {
            if (trades[onSaleTokenArray[i]].price == 0) {
                if ((onSaleTokenArray.length - 1) > 0 && i != (onSaleTokenArray.length - 1)) {
