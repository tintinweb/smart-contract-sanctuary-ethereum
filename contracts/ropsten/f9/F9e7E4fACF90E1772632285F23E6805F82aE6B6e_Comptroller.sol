...
contract Comptroller is ComptrollerV6Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function _setAdminSwappingIncentive(uint newAdminSwappingIncentiveMantissa) external returns (uint) {
        ...
        Exp memory minAdminSwappingIncentive = Exp({mantissa: liquidationIncentiveMinMantissa});
...
contract SToken is STokenInterface, Exponential, TokenErrorReporter {
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, STokenInterface sTokenCollateral) internal returns (uint, uint) {
        ...
        if (address(sTokenCollateral) == address(this)) {
            seizeError = seizeInternal(address(this), liquidator, borrower, seizeTokens, seizeTokensForLiquidator);
        } else {
            seizeError = sTokenCollateral.seize(liquidator, borrower, seizeTokens, seizeTokensForLiquidator);
        }
