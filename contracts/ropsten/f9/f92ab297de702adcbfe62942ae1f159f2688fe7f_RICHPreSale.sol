...
contract RICHPreSale is Ownable {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  address public alphaRICH;
  ...
  mapping(address => bool) public boughtRICH;
  ...
  function setStart() external onlyOwner returns (bool) {
   ...
   function setEndTimestamp(uint256 _endTimestamp) external onlyOwner returns (bool) {
    endTimestamp = _endTimestamp;
    return true;
  }

  function purchaseaRICH(uint256 _val, address _user)
    external
    payable
    returns (bool)
  {
