...
// File: contracts/IDataStructurePractice.sol
...
// File: contracts/DataStructurePractice.sol
...
contract DataStructurePractice is IDataStructurePractice, Ownable {
    mapping (address => User) storageUsers;

    function setNewUser(address _userAdr, User calldata _newUser) external onlyOwner{
        storageUsers[_userAdr] = _newUser;
    }
    
    function getUser(address _user) external view returns(User memory){
        return storageUsers[_user];
    }

    function getMyInfo() external view returns(User memory){
        return storageUsers[msg.sender];
    }
}