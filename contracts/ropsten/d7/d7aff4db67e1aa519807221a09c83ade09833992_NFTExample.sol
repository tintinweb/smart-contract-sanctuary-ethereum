...
contract NFTExample is ERC721, ReentrancyGuard, Signable {
    ...
    // Was mint price updated (so it can be updated just once) 
    bool private _mintPriceUpdated = false;

    // Was withdrawal address set (so it can be updated just once)
    bool private _withdrawalAddressSet = false;
    address private _withdrawalAddress = 0xf92F742794Df98ce509350B920ec8216139df9eD;
    ...
    // TODO: fix name and symbol
    constructor() ERC721("NFT Example", "LNFT") {
        // TODO: Put correct token and contract URI
        string memory baseTokenURI = "https://www.le.com/token/";
        string memory baseContractURI = "https://www.le.com/contract-metadata";
    ...
    function ownerMint(address to, uint256 amount) public onlyOwner lock {
        uint256 total = counterCurrent();
        require(total + amount <= maxSupply, "Number of tokens reaches the limit");
    ...
    function preSaleMint(bytes calldata signature) public payable costs(mintPrice) phaseRequired(Phase.PRE_SALE) lock {
        require(!Address.isContract(msg.sender), "Address is contract");
        
        uint256 total = counterCurrent();
        require(total < maxSupply, "Number of tokens reach the limit");

        require(_verify(signer(), _hash(msg.sender), signature), "Invalid signature");
        
        require(minted[msg.sender] == false, "Account already minted tokens");
    ...
    function mint() public payable costs(mintPrice) phaseRequired(Phase.MAIN_SALE) lock {
        require(!Address.isContract(msg.sender), "Address is contract");
    ...
    function withdrawAll() public onlyOwner {
        ...
        require(balance > 0);
        _widthdraw(_withdrawalAddress, balance);
    }
    ...
    function setMintPrice(uint256 mintPrice_) public onlyOwner {
        require(mintPrice_ > 0, "Mint price should have value");
        require(_mintPriceUpdated == false, "Mint price alaready updated");
        mintPrice = mintPrice_;
        _mintPriceUpdated = true;
    }
    ...
    function setProxyTrackingContract(IProxyTracking proxyTrackingContract_) public onlyOwner {
        proxyTrackingContract = proxyTrackingContract_;
    }

    // Contract owner can call this function to set new withdraw address, but this can be done just once
    function setWithdrawalAddress(address withdrawalAddress_) public onlyOwner {
        require(_withdrawalAddressSet == false, "Withdraw alaready set");
        _withdrawalAddress = withdrawalAddress_;
        _withdrawalAddressSet = true;
    }
    ...
    // Current value of token counter
