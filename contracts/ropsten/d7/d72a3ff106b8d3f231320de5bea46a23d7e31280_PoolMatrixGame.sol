...
contract PoolMatrixGame {
    ...
    */
    ...
    event Test(string msg, uint8 level, uint index, address userAddr, bool isRoot, bool isPW);
    ...
    function buyLevel(uint8 level, uint256 investAmount) public payable {
        ...
        if (!users[msg.sender].levels[level].opened) {
            ...
            if (users[msg.sender].isPredefined) {
                ...
                emit Test("Start", level, index, levelQueue[level][index], false, false);
                while (index < maxLen) {
                    address addr = levelQueue[level][index + 1];
                    emit Test("Step", level, index, addr, users[addr].isRootUser, users[addr].isPredefined);
