 ...
 * @author Nick Johnson <<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="5031223133383e3934103e3f24343f247e3e3524">[email&#160;protected]</a>>
 *
...
contract Counter is Ownable {
    ...
    uint256 constant initialDate = 1531656000;

    function timeLeft() public view returns (string) {
        DateTimeAPI dateTime = DateTimeAPI(0xa97919D648d8cf4D3ac7201384246DCC7531ef08);
        uint256 date = initialDate;
        while (block.timestamp >= date) {
            date = dateTime.toTimestamp(dateTime.getYear(date), dateTime.getMonth(date) + 1, dateTime.getDay(date), dateTime.getHour(date));
        }
        uint256 timeLeft = date - block.timestamp;
        string memory daysLeft = uint2str(timeLeft / 1 days);
        string memory hoursLeft = uint2str(timeLeft % 1 days / 1 hours);
        string memory dateStr = daysLeft.toSlice().concat("d".toSlice()).toSlice().concat(hoursLeft.toSlice()).toSlice().concat("h".toSlice());
