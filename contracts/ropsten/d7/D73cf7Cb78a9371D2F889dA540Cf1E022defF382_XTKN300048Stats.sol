...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn300048][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn300048][ownable] new owner is the zero address");
...
contract IXTKN300048StakingLocked{
    function init(uint256, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN300048PreSale{
    function init(uint256, address) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN300048PreSaleVault{
...
contract IXTKN300048PreSaleVault20{
    function deposit(address, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN300048Lottery{
    function init(uint256) public{}
    function deposit(address, address, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}
...
*/
contract XTKN300048Stats is Context, Ownable{
    // lib
    ...
    // interfaces
        IXTKN300048 private _main;
        IXTKN300048Project private _project;
        IXTKN300048Staking private _staking;
        IXTKN300048StakingLocked private _staking30D;
        IXTKN300048StakingLocked private _staking90D;
        IXTKN300048StakingLocked private _staking180D;
        IXTKN300048StakingLocked private _staking365D;
        IXTKN300048PreSale private _preSale;
        IXTKN300048PreSaleVault private _vault;
        IXTKN300048PreSaleVault20 private _vault20;
        IXTKN300048Lottery private _lottery;
    ...
    // timers
        ...
        uint256 private _lastLotteryUpdate; // stores the timestamp of the last lottery update
        uint256 private _lastLotteryTokens; // stores the last tokens calculated for the lottery
    ...
    constructor(address pToken){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            _main = IXTKN300048(ADDRESS_TOKEN);
    ...
    function balanceForLottery() public returns(uint256){
        if((block.timestamp - _lastLotteryUpdate) > 3600){
            ...
            if(tokensRequiredUSDC > LOTTERY_MAX_USDC){
                ...
                _lastLotteryTokens = LOTTERY_MAX_USDC.div(tokenPrice);
            }else{
                
                _lastLotteryTokens = tokensRequired;                
            }

            _lastLotteryUpdate = block.timestamp;
        }

        return(_lastLotteryTokens);
    }
    ...
    function _refresh() private{
        ...
        /* reset instances */
        if(block.timestamp <= (PRESALE_VAULT_DEATH - 60)){
            _vault = IXTKN300048PreSaleVault(ADDRESS_PRESALE_VAULT);
            _vault20 = IXTKN300048PreSaleVault20(ADDRESS_PRESALE_VAULT20);
        }
        
        _pair = IUniswapV2Pair(ADDRESS_PAIR);
        _lottery = IXTKN300048Lottery(ADDRESS_LOTTERY);
        _project = IXTKN300048Project(ADDRESS_PROJECT);
        _staking = IXTKN300048Staking(ADDRESS_STAKING);  

        if(block.timestamp <= (STAKING_LOCKED_DEATH - 60)){
            _staking30D = IXTKN300048StakingLocked(ADDRESS_STAKING_DAYS30);
            _staking90D = IXTKN300048StakingLocked(ADDRESS_STAKING_DAYS90);
            _staking180D = IXTKN300048StakingLocked(ADDRESS_STAKING_DAYS180);
            _staking365D = IXTKN300048StakingLocked(ADDRESS_STAKING_DAYS365);
        }        
    }
}