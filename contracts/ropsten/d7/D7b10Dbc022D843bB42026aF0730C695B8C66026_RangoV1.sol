...
contract RangoV1 is BaseContract, RangoCBridgeProxy, RangoThorchainProxy, RangoMultichainProxy {
    ...
    function initialize() public initializer {
        ...
        rangoMultichainAddress = NULL_ADDRESS;
    }
...
contract RangoThorchainProxy is BaseContract {
    ...
    function swapInToThorchain(
        ...
        IRangoThorchain(rangoThorchainAddress).swapInToThorchain{value : value}(
            request.toToken,
            out,
            tcRouter,
            tcVault,
...
contract RangoMultichainProxy is BaseContract {
    ...
    function multichainBridge(
        ...
        require(rangoMultichainAddress != NULL_ADDRESS, 'Multichain address in Rango contract not set');
        ...
        (, uint out) = onChainSwapsInternal(request, calls);
        if (request.toToken != NULL_ADDRESS)
            approve(request.toToken, rangoMultichainAddress, out);

        uint value = request.toToken == NULL_ADDRESS ? (out > 0 ? out : request.amountIn) : 0;

        IRangoMultichain(rangoMultichainAddress).multichainBridge{value: value}(
            _actionType,
            request.toToken,
            _underlyingToken,
            out,
