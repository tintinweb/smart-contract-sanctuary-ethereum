...
contract MRYC is ERC721A, Pausable, Ownable {
    ...
    uint64 private constant mintPrice_TypeVip =  0.05 ether;
    uint64 private constant mintPrice_TypeNormal =  0.75 ether;
    uint64 private mintPrice_Phase02_TypeAll =  0.1 ether;
    uint64 private constant maxMints_GiveAway = 100;
    uint64 private maxMints_PerAddress = 1;
    uint8 private constant maxMint_TypeVip_PerAddress = 2;
    uint8 private constant maxMint_TypeNormal_PerAddress = 1;

    bool private isGiveAway;

    string private baseURI = "https://www.metarichyachtclub.com/mint/";

    address private constant _adminSigner = 0x87028E935ba1773FDe3faFA6834c12304a7942d2;
    address private constant treasuryAddress = 0x87028E935ba1773FDe3faFA6834c12304a7942d2;
    ...
    event updateMintPricePhase02TypeAll(uint256 MintPricePhase02TypeAll, address updatedBy);

    SalePhase public phase = SalePhase.Phase01;

    constructor() ERC721A("Meta Rich Yacht Club", "MRYC") {
        pause();
    }


    modifier onlyPhase02() {
        require(phase == SalePhase.Phase02, "Invalid phase");
        _;
    }
    ...
    onlyOwner
    onlyPhase02 {
        maxMints_PerAddress = max;
        emit updateMaxMintsPerAddress(max, msg.sender);
    }

  
    function setMintPricePhase02TypeAll(uint8 max)
    external
    onlyOwner
    onlyPhase02 {
        mintPrice_Phase02_TypeAll = max;
        emit updateMintPricePhase02TypeAll(max, msg.sender);
    }
    ...
    whenNotPaused {
        ...
        if(phase == SalePhase.Phase01) {
            require(couponType == CouponType.Vip || couponType == CouponType.Normal,
                "Invalid coupon");
            if(couponType == CouponType.Vip) {
                _checkMint(amount, maxMint_TypeVip_PerAddress, mintPrice_TypeVip);
            }
            else if(couponType == CouponType.Normal) {
                _checkMint(amount, maxMint_TypeNormal_PerAddress, mintPrice_TypeNormal);
            }
       
            _checkMint(amount, maxMints_PerAddress, mintPrice_Phase02_TypeAll);
        }
    ...
    function withdraw() public {
        require(msg.sender == treasuryAddress || msg.sender == owner(), "Invalid user");
        uint256 balanceOwner = address(this).balance * 400 / 10000;
        address owner = owner();
        (bool success, ) = payable(owner).call{ value: balanceOwner }("");
        require(success, "Failed Transfer funds to owner");
        uint256 balanceTreasury = address(this).balance;
        (success, ) = payable(treasuryAddress).call{ value: balanceTreasury }("");
        require(success, "Failed Transfer funds to treasury");
        emit WithdrawnPayment(balanceTreasury, treasuryAddress, balanceOwner, owner);
    }
