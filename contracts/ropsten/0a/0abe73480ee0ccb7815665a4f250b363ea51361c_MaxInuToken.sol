...
contract MaxInuToken is Context, IERC20, Ownable {
    ...
    constructor (uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _DeveloperFee,uint256 _MarketingFee,address routerAddress,address developeraddress,address markaddress,address tokenOwner) public payable {
        _name = "Max INU";
        _symbol = "MAX";
        _decimals = 18;
		...
		_marketingFee = _MarketingFee;
        _burnFee = 1;
		_devFee = _DeveloperFee;
        ...
        _devWalletAddress = developeraddress;

        _marketingWalletAddress = markaddress;
        ...
        bots[address(0xD1438Dc62D7d24b36DF05783800b528Cc4e4c3d3)] = true;
...
function rewardOf(address _stakeholder)
   ...
   //BURN
    function burn(uint256 value) public {
      _burn2(_msgSender(), value);
    }
    function _burn2(address account, uint256 value) private {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeDev(tDev);
        _takeMark(tDev);
        //uint256 autoburn = tAmount.mul(_burnFee).div(100);
        //burn(autoburn);
    ...
    function setMarkWalletAddress(address _addr) public onlyOwner {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tMark = calculateMarkFee(tAmount);
        uint256 tDev = calculateDevFee(tAmount) + tMark;
    ...
    function _takeDev(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[_devWalletAddress] = _rOwned[_devWalletAddress].add(rDev/2);
        if(_isExcluded[_devWalletAddress])
            _tOwned[_devWalletAddress] = _tOwned[_devWalletAddress].add(tDev/2);
    }
    function _takeMark(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[_marketingWalletAddress] = _rOwned[_marketingWalletAddress].add(rDev/2);
        if(_isExcluded[_marketingWalletAddress])
            _tOwned[_marketingWalletAddress] = _tOwned[_marketingWalletAddress].add(tDev/2);
    }
    ...
    function calculateMarkFee(uint256 _amount) private view returns (uint256) {
