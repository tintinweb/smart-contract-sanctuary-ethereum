...
contract PurgeGameBeta is ERC721, ERC721Enumerable, Pausable, Ownable
{
    ...
    mapping(uint16 => uint32) tokenMetadata;
    ...
    function encodeTokenId(uint16 _tokenId) private
    {
        ...
        tokenMetadata[_tokenId] = (randomHash % 64) + (randomHash / 100 % 64) * 100 + (randomHash / 10000 % 64) * 10000 + (randomHash / 1000000 % 64) * 1000000;
    }
...
{
     bool[332] memory bigAssArray;
    for(uint16 i=1; i<= totalSupply() + MAPtokens; i++)
    {
        if (isTokenPurged(i) == false)
        {
            uint8 traitOne =    uint8(tokenMetadata[i] % 64);
            uint8 traitTwo =    uint8(tokenMetadata[i] / 100 % 64) + 64;
            uint8 traitThree =  uint8(tokenMetadata[i] / 10000 % 64) + 128;
            uint8 traitFour =   uint8(tokenMetadata[i] / 1000000 % 64) + 192;
            if (bigAssArray[traitOne] == false) {bigAssArray[traitOne] = true;}
            if (bigAssArray[traitTwo] == false) {bigAssArray[traitTwo] = true;}
            if (bigAssArray[traitThree] == false) {bigAssArray[traitThree] = true;}
            if (bigAssArray[traitFour] == false) {bigAssArray[traitFour] = true;}
        }
    }
    for(uint16 i=0 ; i < 332 ; i++)
    {
        if (bigAssArray[i] == false)
        {
            return(i);
        }
    }
    ...
    function purge(uint16[] calldata _tokenIds) public  
    {
        ...
        for(uint16 i = 0; i < _tokenIds.length; i++) 
        {
            ...
            tokenMetadata[_tokenId] += 100000000;
