...
contract TTTest is Ownable, ERC721A, ReentrancyGuard {
    uint256 public constant PRICE = 0.03 ether;
    uint256 public constant MAX_TOKEN_PURCHASE = 3;
    uint256 public constant MAX_TOKENS = 10;
    uint256 public constant MAX_TOKENS_PUBLIC = 8;
    ...
    constructor() ERC721A("Ti Tow Test", "TTTest", MAX_BATCH_SIZE, MAX_TOKENS) {
    }

    modifier mintGuard(uint256 tokenCount) {
        require(tokenCount > 0, "Min mint is 1 NFT");
        require(PRICE * tokenCount <= msg.value, "Wrong ether value");
        require(totalSupply() + tokenCount <= MAX_TOKENS_PUBLIC, "max NFT limit exceeded");
        require(tokenCount <= MAX_TOKEN_PURCHASE, "Exceeds token txn limit ");
        _;
    }
    ...
    function mintPresale(bytes32[] calldata proof, uint256 amount) external payable mintGuard(amount) nonReentrant {
        ...
        require(presaleAddressMintCount[msg.sender] + amount <= MAX_TOKEN_PURCHASE, "Address already minted allocation");
