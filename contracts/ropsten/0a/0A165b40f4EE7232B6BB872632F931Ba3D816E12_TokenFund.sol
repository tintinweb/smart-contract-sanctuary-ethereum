...
_contractUSDT.approve(sushiRouter, type(uint256).max);
        console.log("Deploying a Tokenfund1");

_contractUSDC.approve(sushiRouter, type(uint256).max);
_contractLINK.approve(sushiRouter, type(uint256).max);
_contractWETH.approve(sushiRouter, type(uint256).max);

_contractUSDT.approve(uniswapRouter, type(uint256).max);
_contractUSDC.approve(uniswapRouter, type(uint256).max);
_contractLINK.approve(uniswapRouter, type(uint256).max);
_contractWETH.approve(uniswapRouter, type(uint256).max);

_contractUSDT.approve(bankorRouter, type(uint256).max);
_contractUSDC.approve(bankorRouter, type(uint256).max);
_contractLINK.approve(bankorRouter, type(uint256).max);
_contractWETH.approve(bankorRouter, type(uint256).max);
