...
library NameFilter {
	...
	function byte32ToString(bytes32 b) 
	    internal
        pure
	    returns (string) 
	{
...
contract Famo{
	...
	event test(uint256 value, uint256 d1);
	...
	function GetCurRoundInfo()constant public returns ( 
        ...
        return ( 
            ...
            NameFilter.byte32ToString(plyMap[roundList[idx].plyr].name),// 10
    ...
    function getPlayerInfoByAddress(address myAddr)
        public 
        view 
        returns( string myName, uint256 myKeyNum, uint256 myValut,uint256 affGen,uint256 lockGen )
    {
        // setup local rID
        ...
        if( plyMap[_addr].iLastRoundId == 0 || _rID <= 0 ){
                    return
            (
                NameFilter.byte32ToString(plyMap[_addr].name),
                0,         //2
                0,      //4
