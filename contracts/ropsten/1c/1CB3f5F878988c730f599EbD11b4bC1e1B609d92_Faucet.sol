...
contract Faucet is InitializableOwnable {
    ...
    function getFaucetToken(ERC20 token) external {
        require(block.timestamp - lastTimestampList[msg.sender] > interval, "You used faucet less then 24 hours");
    
        uint amount = 200 * token.decimals();
        lastTimestampList[msg.sender] = block.timestamp;
        require(token.transfer(msg.sender, amount), "Faucet: transfer failed");
