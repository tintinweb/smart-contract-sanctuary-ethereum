...
contract NFT is Ownable, ERC721A, ReentrancyGuard {
  ...
  mapping(uint256 => string) public tokenToFileNames;
  ...
  function batchSetIssueTokensPayload(uint32 startTokenId, uint32[] memory tokenIds, bytes[] memory payloads) external onlyOwner {
    require(tokenIds.length == payloads.length, "tokenIds length must equals to payloads length");
    _randomArray(tokenIds);
    for (uint256 i = 0; i < tokenIds.length; i++) {
      _tokenPayloads[startTokenId + i] = payloads[tokenIds[i] - startTokenId];
      tokenToFileNames[startTokenId + i] = generateFNByToken(tokenIds[i]);
    }
  }
  ...
  function generateFNByToken(uint256 tokenId) pure public returns (string memory) {
    return string(abi.encodePacked(bytes("RND"), Strings.toString(tokenId)));    
  }
  ...
  function setBlindBoxOpened(bool open) external onlyOwner {
    ...
    returns (string memory)
  {
    ...
    if (_enableShowTokenName(tokenId)) {
      return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenToFileNames[tokenId])) : "";
    } else {
