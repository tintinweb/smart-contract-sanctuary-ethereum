...
contract SahayogiAgency is AccessControl, IMerkleDistributor {
    ...
    event Create(uint256 projectId, string projectName);
    ...
    //FUNCTIONS
    function createProject(string calldata _projectName)
        public
        OnlyAgency
        returns (uint256)
    {
        count += 1;
        projects[count] = Project({projectName: _projectName, running: true});
        emit Create(count, _projectName);
        return count;
    }

    function getProjectInfo(uint256 _projectId) public view returns(string memory PROJECTNAME_){
    return(projects[_projectId].projectName);
    }
    ...
    function _setClaimed(uint256 projectId, uint256 index) internal {
        ...
        claimedBitMap[projectId][claimedWordIndex] =
            claimedBitMap[projectId][claimedWordIndex] |
            (1 << claimedBitIndex);
    }
    ...
    function claim(
        ...
        require(
            MerkleProof.verify(merkleProof, merkleRoot[projectId], node),
    ...
    function isClaimed(uint256 projectId, uint256 index)
        public
        view
        override
        returns (bool)
    ...
    function updateMerkleRoot(uint256 _projectId, bytes32 _newMerkleRoot)
        external
        OnlyAgency
    {
