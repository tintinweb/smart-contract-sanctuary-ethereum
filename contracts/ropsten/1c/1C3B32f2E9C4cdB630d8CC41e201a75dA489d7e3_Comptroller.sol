...
contract SToken is STokenInterface, Exponential, TokenErrorReporter {
    ...
    uint private tempMoreForLiquidatorAndStrike;
    uint private seizeAndGiveLiquidator;
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, STokenInterface sTokenCollateral) internal returns (uint, uint) {
       //avoind stack too deep
        ...
        MathError mathErr;
        (mathErr, moreForLiquidatorAndStrike) = mulUInt(seizeTokens, 1500);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED_CANOT_ADD15%");
        moreForLiquidatorAndStrike = moreForLiquidatorAndStrike / 10000;

        // check if out of _borrower owe 
        uint tempNewSeize;
        (mathErr, tempNewSeize) = addUInt(moreForLiquidatorAndStrike, seizeTokens);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED_CANOT_ADDNEW_SEIZE");

        if (tempNewSeize > _sTokenCollateral.balanceOf(borrower)){
            (mathErr,moreForLiquidatorAndStrike) = subUInt(_sTokenCollateral.balanceOf(borrower),seizeTokens);
            require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED_CANNOT_CALCULATE15%");

        }

        (mathErr, tempMoreForLiquidatorAndStrike) = mulUInt(moreForLiquidatorAndStrike,2);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED_WHENGET10%");

        (mathErr,tempMoreForLiquidatorAndStrike) = divUInt(tempMoreForLiquidatorAndStrike,3);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED_WHENGET10%");

        (mathErr, seizeAndGiveLiquidator) = addUInt(seizeTokens, tempMoreForLiquidatorAndStrike); // 10% liquidator -> seize and give liquidator
        
        //get 15% more for seize token
        (mathErr,seizeTokens) = addUInt(seizeTokens, moreForLiquidatorAndStrike); // -> 15% for liquidator and swap -> seize from borrower
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");
