        ...
        for(uint i=0;i<ContractAddress.length;i++)
        {
            for(uint j=x;j<x+cardHoldingsPerPlayer;j++)
            {
                ...
                if(c[i].Power > c[i+1].Power)
                {
                    ...
                    winpoint[ContractAddress[i]] = winningPoints;
                    max2 = c[i].Power;
                }
...
//This is done without the nested mapping
    function startgamenested() public {
        ...
        {
            for(uint i=0;i<ContractAddress.length-1;i++)
            {
                Cards memory c1 = ownerOfCards[ContractAddress[i]][j];
                Cards memory c2 = ownerOfCards[ContractAddress[i+1]][j];

                if(c1.Power > c2.Power)
                {
                    winningPoints = winningPoints +1;
                    winpoints[ContractAddress[i]] = winningPoints;
                    max2 = c1.Power;
                }
                else if(c1.Power > max2)
                {
                    winningPoints = winningPoints +1;
                    winpoints[ContractAddress[i]] = winningPoints;
                }
