...
contract Token is owned, TokenERC20  {
	...
	function checkLockedBalance(address wallet, uint256 _value) internal returns (bool){
		if(wallet==wallet1){
			if(now<startTime + 183 * 1 seconds){
				return balanceOf[wallet1].sub(_value)>=totalSupply.mul(100).div(100)? true : false;
			}else{
				return true;
			}
		}else if(wallet==wallet3){
			if(now<startTime + 183 * 1 seconds){
				return balanceOf[wallet3].sub(_value)>=totalSupply.mul(20).div(100)? true : false;
			}else if(now>=startTime + 183 * 1 seconds && now<startTime + 365 * 1 seconds){
				return balanceOf[wallet3].sub(_value)>=totalSupply.mul(10).div(100)? true : false;
			}else{
				return true;
			}
		}else{
