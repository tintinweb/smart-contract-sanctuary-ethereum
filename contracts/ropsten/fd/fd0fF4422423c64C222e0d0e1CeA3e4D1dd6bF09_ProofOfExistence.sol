...
hashToProof[hash] = proof(msg.sender, hash, block.timestamp); // set
//owner to msg.sender and time to now
}
// 6.
// Check whether a hash exists or not
function checkExists(string memory hash) public view returns (bool){
return hashToProof[hash].timestamp > 0; //seeing if it has a time of
//existence
}
// 7.
