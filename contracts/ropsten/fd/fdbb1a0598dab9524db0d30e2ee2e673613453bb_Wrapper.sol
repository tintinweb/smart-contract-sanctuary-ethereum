...
contract Wrapper is Utils {
    ...
    function getSrcReserveAddress(KyberNetwork network, ERC20 token, uint id) public view returns(address) {
        return network.reservesPerTokenSrc(token, id);
    }

    function getDestReserveAddress(KyberNetwork network, ERC20 token, uint id) public view returns(address) {
        return network.reservesPerTokenDest(token, id);
    }
