...
contract PoolMatrixGame {
    ...
    address internal regFeeWallet;
    address internal marketingWallet;
    uint internal initialDate;
    mapping (address => User) internal users;
    address[] internal userAddresses;
    uint internal userCount;
    address internal rootWallet1;
    address internal rootWallet2;
    address internal rootWallet3;
    address internal rootWallet4;
    mapping(uint => address[]) internal levelQueue;
    mapping(uint => uint) internal headIndex;
    uint internal marketingBalance;
    uint internal transactionCounter;
    uint internal turnoverAmount;
    uint internal stats1date;
    uint internal stats2date;
    uint internal stats1totalUsers;
    uint internal stats2totalUsers;
    uint internal stats1totalTransactions;
    uint internal stats2totalTransactions;
    uint internal stats1totalTurnover;
    uint internal stats2totalTurnover;
    uint internal mode; // 0 - default, 1 - disable complex algorithms, 2 - suspended
    ...
    function buyLevel(uint level, uint investAmount) public payable {
        ...
        require(block.timestamp >= initialDate + levelIntervals[level]); // Check if level is avalilable
