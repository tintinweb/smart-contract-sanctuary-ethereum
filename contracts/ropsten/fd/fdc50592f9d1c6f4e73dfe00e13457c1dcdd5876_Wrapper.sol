...
contract Wrapper is Utils {
    ...
    function getListReserves(KyberNetwork network, ERC20 token) public view returns(address[] memory src, address[] memory dest) {
        uint numReserves = network.getNumReserves();
        uint i;
        // count number src reserves
        uint count = 0;
        for(i = 0; i < numReserves; i++) {
            if (network.reservesPerTokenSrc(token, i) == 0x0) { break; }
            else { count++; }
        }
        src = new address[](count);
        for(i = 0; i < count; i++) {
        ...
        count = 0;
        for(i = 0; i < numReserves; i++) {
            if (network.reservesPerTokenDest(token, i) == 0x0) { break; }
            else { count++; }
        }
        dest = new address[](count);
        for(i = 0; i < count; i++) {
