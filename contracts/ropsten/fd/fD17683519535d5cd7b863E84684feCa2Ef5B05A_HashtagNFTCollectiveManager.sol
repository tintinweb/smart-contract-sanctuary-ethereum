...
import "./Interface/ITokenRecipient.sol";
    ...
    IHashtagNFTCollectiveManager
{
    ...
    function approveAndCall(address _spender, uint256 _value, bytes calldata _extraData) nonReentrant public returns (bool success) {
...
contract Settings is ISettings, Ownable, Initializable {
    ...
    modifier onlyFactory() {
        require(msg.sender == itemMgrProxyFactory, "Settings: not factory");
        _;
    }

    function setGovernorLogic(address _govLogic) external override {
        require(msg.sender == itemMgrProxyFactory || msg.sender == livetree, "Settings: not factory or livetree");
    ...
    IHashtagNFTCollectiveGovernor
{
    ...
    function initialize(
        ...
        address settings = IHashtagNFTCollectiveManager(_tokenAddr).settings();
        address factory = ISettings(settings).getItemMgrProxyFactory();
        uint256 vaultCount = IHashtagNFTCollectiveFactoryProxy(factory).vaultCount();
        __GovernorVotesQuorumFraction_init(vaultCount == 0 ? 30 : 51);
    ...
    function requestUpdateQuorumNumerator(uint256 newQuorumNumerator, string calldata description)
        external
        override
        returns (uint256)
    {
    ...
    function requestNFTSale(
        address nftAddress,
        uint256 itemId,
        string calldata description
    ) external override returns (uint256) {
        address nftVault = IHashtagNFTCollectiveERC721(nftAddress).nftVault();
        require(IHashtagNFTCollectiveManager(tokenAddr).nftVault() == nftVault, "Governor: invalid governor contract");

        require(!IHashtagNFTCollectiveNFTVault(nftVault).isItemOnAuctionOrProposal(itemId), "Governor: item already on auction");

        address[] memory targets = new address[](1);
        targets[0] = address(nftVault);
        ...
        bytes[] memory calldatas = new bytes[](1);
        calldatas[0] = abi.encodeWithSignature(
            "setItemForSale(address,uint256)",
            nftAddress,
            itemId
        );
        ...
        IHashtagNFTCollectiveNFTVault(nftVault).setItemOnAuctionOrProposal(itemId, 0, proposalId);
