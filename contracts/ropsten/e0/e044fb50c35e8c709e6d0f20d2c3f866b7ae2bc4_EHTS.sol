...
contract EHTS is ERC20,Ownable {
    ...
    function increaseAllowance(address spender, uint256 addedValue) on public virtual override returns (bool) {
        require(!paused, "the contract is paused");
        _approve(_msgSender(), spender, allowance(_msgSender(),spender).add(addedValue));
        return true;
    }
    function decreaseAllowance(address spender, uint256 subtractedValue) on public virtual override returns (bool) {
        require(!paused, "the contract is paused");
