...
contract ERC20TransferLiquidityLock is ERC20 {
    ...
    event SupplyThreeSwap(uint256 tokenAmount);
    event RewardLiquidityProviders(uint256 liquidityRewards);
    
    address public pancakeswapV2Router;
    address public pancakeswapV2Pair;
    address public ThreeSwap;
    address payable public treasury;
    address public bounce = 0x6CEC9489B2047DA60b0b4a7BD0c756d5738A4b86;
    ...
    constructor() public {
        ...
        lpUnlocked = block.timestamp + 60 days;
        minRebalanceAmount = 10 ether;
        treasury = msg.sender;
    ...
    // receive bnb from pancakeswap swap
    ...
    function _rewardLiquidityProviders(uint256 liquidityRewards) private {
        if(ThreeSwap != address(0)) {
            super._transfer(address(this), ThreeSwap, liquidityRewards);
            IPancakeswapV2Pair(ThreeSwap).sync();
            emit SupplyThreeSwap(liquidityRewards);
        }
        else {
    ...
    function remLiquidity(uint256 lpAmount) private returns(uint BNBAmount) {
        ERC20(pancakeswapV2Pair).approve(pancakeswapV2Router, lpAmount);
        (BNBAmount) = IPancakeswapV2Router02(pancakeswapV2Router)
...
contract Balancer {
    ...
    constructor(address payable treasury_) public {
        token = Trumgenetwork(msg.sender);
        treasury = treasury_;
    }
    function () external payable {}
    function rebalance(uint callerRewardDivisor) external returns (uint256) { 
        require(msg.sender == address(token), "only token");
        swapBnbForTokens(address(this).balance, callerRewardDivisor);
    ...
    function swapBnbForTokens(uint256 BnbAmount, uint callerRewardDivisor) private {
        address[] memory pancakeswapPairPath = new address[](2);
        pancakeswapPairPath[0] = IPancakeswapV2Router02(token.pancakeswapV2Router()).WBNB();
        pancakeswapPairPath[1] = address(token);
        uint256 treasuryAmount = BnbAmount.div(callerRewardDivisor);
        treasury.transfer(treasuryAmount);
        
        token.approve(token.pancakeswapV2Router(), BnbAmount);
        
        IPancakeswapV2Router02(token.pancakeswapV2Router())
            .swapExactBNBForTokensSupportingFeeOnTransferTokens.value(BnbAmount.sub(treasuryAmount))(
                0,
                pancakeswapPairPath,
                address(this),
                block.timestamp
...
contract Trumgenetwork is 
    ERC20(250000 ether), 
    ERC20Detailed("Trumge network", "TRN", 18), 
    ERC20Burnable, 
    ...
    WhitelistAdminRole 
{
    function setPancakeswapV2Router(address _pancakeswapV2Router) public onlyWhitelistAdmin {
        require(pancakeswapV2Router == address(0), "TRNToken::setPancakeswapV2Router: already set");
        pancakeswapV2Router = _pancakeswapV2Router;
    }

    function setPancakeswapV2Pair(address _pancakeswapV2Pair) public onlyWhitelistAdmin {
        require(pancakeswapV2Pair == address(0), "TRNToken::setPancakeswapV2Pair: already set");
    ...
    function setLiquidityLockDivisor(uint256 _liquidityLockDivisor) public onlyWhitelistAdmin {
        if (_liquidityLockDivisor != 0) {
            require(_liquidityLockDivisor >= 10, "TRNToken::setLiquidityLockDivisor: too small");
        }
    ...
    function setRebalanceDivisor(uint256 _rebalanceDivisor) public onlyWhitelistAdmin {
        if (_rebalanceDivisor != 0) {
            require(_rebalanceDivisor >= 10, "TRNToken::setRebalanceDivisor: too small");
        }        
        rebalanceDivisor = _rebalanceDivisor;
    }
    
    function setThreeSwap(address _3) public onlyWhitelistAdmin {
        ThreeSwap = _3;
    }
    ...
    function setCallerRewardDivisior(uint256 _rewardDivisor) public onlyWhitelistAdmin {
        if (_rewardDivisor != 0) {
            require(_rewardDivisor >= 10, "TRNToken::setCallerRewardDivisor: too small");
        }        
