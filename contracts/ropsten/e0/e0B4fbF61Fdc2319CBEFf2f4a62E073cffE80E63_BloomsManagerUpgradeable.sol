    ...
    NectarImplementationPointerUpgradeable
{
    ...
    function autoCompound(uint256 _numNodes) external onlyOwner {
        ...
        while (_numNodes > 0) {
            ...
            emit Autocompound(_msgSender(), bloomId, amountToCompound);
        }
    ...
    function _swapAndBurn(uint256 _value) private {
        ...
        uint256 nctrAmountOut = router.getAmountOut(half, reserve0, reserve1);
        ...
        returns (uint256 nctrValue)
    {
        if (nectar.transferFrom(_msgSender(), address(this), _value)) {
            ...
            usdc.transfer(devWallet, (_value * 10) / 100); ///////////////////////// CHECK IF THIS IS HOW IT SHOULD BE //////////////////////////

            _swapAndBurn((_value * 90) / 100);

            nctrValue =
                _value *
                (nectar.balanceOf(address(pair)) /
        ...
        returns (uint256 nctrValue)
    {
        ...
        usdc.transfer(devWallet, (_value * 10) / 100);
    ...
    function _updateEmergencyStatus(address _sender) private returns (uint256) {
        EmergencyStats storage emergencyStats = emergencyStats[_sender];
