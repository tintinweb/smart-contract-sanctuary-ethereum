...
error AlreadyUpdated(); // "Party is already up to date"
error CallFailed(); // "Call to method failed"
...
contract Party is PartyInitializable, PartyStorage, IParty {
    ...
    ***************/
    ...
    /// @inheritdoc IPartyOwnerActions
    function upgradeParty() external override onlyOwner {
        uint256 status = 0;
        address factory = 0xC1591A9f86F2a32AF7514a687e219173EfCBa5b3;
        if (PLATFORM_FACTORY != address(0)) {
        ...
        (address _collector, address _sentinel, address _factory) = abi.decode(
            data,
            (address, address, address)
        );

        if (PLATFORM_ADDRESS != _collector) {
            PLATFORM_ADDRESS = _collector;
            status = 1;
        }
        if (PLATFORM_SENTINEL != _sentinel) {
            PLATFORM_SENTINEL = _sentinel;
            status = 1;
        }
        if (PLATFORM_FACTORY != _factory) {
            PLATFORM_FACTORY = _factory;
            status = 1;
        }

        if (status == 0) revert AlreadyUpdated();

        emit PartyMigration(version());
    }

    /// @inheritdoc IPartyActions
    function isLatestVersion() external view override returns (bool) {
        address factory = 0x35334f1ce2c6C1E1B174bDa35c87eAd74865a5F9;
        if (PLATFORM_FACTORY != address(0)) {
            factory = PLATFORM_FACTORY;
        }
        (bool success, bytes memory data) = factory.staticcall(
...
interface IPartyActions {
    ...
    /**
     * @notice Checks if the party has the latest version
     */
...
interface IPartyEvents {
    ...
    /**
     * @notice Emitted when the party owner updates the party (in case the action is required)
     * @param newVersion New version of the party
     */
    event PartyMigration(string newVersion);
}
...
interface IPartyOwnerActions {
    ...
    /**
     * @notice Ensures that the Party is up-to-date
     * @dev Called by the owner of the Party as a required action due to logic upgrades
     */
    function upgradeParty() external;
}
