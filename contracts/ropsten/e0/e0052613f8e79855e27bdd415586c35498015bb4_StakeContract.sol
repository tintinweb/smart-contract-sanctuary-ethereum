...
function stakeWithAmd(uint256 amount, uint256 plan) public {
    ...
    function withdrawInWolve(uint256 count) public {
        ...
        require(user.stakerecord[count].amount > 0,"value must be greater then Zero");
        require(owner != address(0)," withdraw completed ");
        require(msg.sender != address(0)," withdraw completed ");
        require(user.stakerecord[count].bonus > 0,"value must be greater then Zero");
