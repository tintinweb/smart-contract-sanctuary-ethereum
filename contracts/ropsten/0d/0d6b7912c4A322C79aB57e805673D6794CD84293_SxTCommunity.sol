...
contract SxTCommunity is ISxTCommunity, SxTCommunityStorage, ERC1155, Ownable, ReentrancyGuard, Pausable {
    ...
    function retrieveNFTs(uint256 startIndex, uint256 count, bool isAvailable) public view returns(string memory) {
        require(startIndex <= currentTokenIndex.current() && startIndex > 0, "SxTCommunity: Invalid startIndex");
        require(count <= currentTokenIndex.current() && count <= startIndex && count > 0, "SxTCommunity: Invalid count");
        // string [] memory resultTokensJSON = new string[](count);
        Token [] memory resultTokens = new Token[](count);
        uint256 currentCount = 0;
        string memory nftsResponseJSON;
        uint256 index = startIndex;
        while(index > 0 && currentCount < count){
            if(isAvailable != checkMaxSupplyReached(index)){
                Token storage resultTemp = tokenDetails[index];
                // resultTokensJSON[currentCount] = getJSONfromStruct(resultTemp);
                resultTokens[currentCount] = resultTemp;
                currentCount++;
            }
            index--;
        }
        nftsResponseJSON = getJSONResponse(resultTokens);
        return nftsResponseJSON;
    }
