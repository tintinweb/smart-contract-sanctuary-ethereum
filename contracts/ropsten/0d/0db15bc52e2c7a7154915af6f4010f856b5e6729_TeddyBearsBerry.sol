...
//An Augminted Labs Project - TBerry IS A UTILITY TOKEN FOR THE TEDDY BEARS ECOSYSTEM.
//$TBERRY is NOT an investment and has NO economic value. 
//It will be earned by active holding within the Teddy Bears ecosystem. Each Genesis Teddy will be eligible to claim tokens at a rate of 5 $TBERRY per day.
...
contract TeddyBearsBerry is ERC20, Ownable {
    ...
    iTeddyBears public TeddyBears;

    uint256 constant public BASE_RATE = 5000 ether;
    ...
    constructor(address teddyAddress) ERC20("TeddyBearsBerry", "TBERRY") {
        TeddyBears = iTeddyBears(teddyAddress);
        START = block.timestamp;
    }

    function updateReward(address from, address to) external {
        require(msg.sender == address(TeddyBears));
        if(from != address(0)){
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(TeddyBears), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return TeddyBears.balanceGenesis(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
    }
