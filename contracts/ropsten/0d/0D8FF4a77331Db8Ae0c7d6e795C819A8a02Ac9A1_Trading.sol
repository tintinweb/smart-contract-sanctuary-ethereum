...
contract Trading {
	...
	struct Product {
		...
		uint64 liquidationThreshold; // in bps. 8000 = 80% // Positions only liquidated at -95% margin (5% buffer) this value = 80% is the rewards set to go to pool on liquidations
	...
	mapping(bytes32 => Position) public positions; // key = currency,user,product,direction
	mapping(bytes32 => Order) public orders; // position key => Order
	...
	function settleOrder(
		...
		if (order.isClose) {
			
			{
				...
				//extra check to emit correct event
