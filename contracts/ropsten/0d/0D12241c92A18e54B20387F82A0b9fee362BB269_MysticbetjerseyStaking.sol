...
// File: mysticbets staking/NFT staking.sol
...
contract MysticbetjerseyStaking is IERC721Receiver, ReentrancyGuard,  Ownable{
    ...
    struct UserInfo {
        uint256[] amount; // How many staked NFTs the user has provided
    ...
    event Initialize(IERC721 _stakedToken, IERC20 _rewardToken, uint256 _rewardPerBlock, 
    ...
    function deposit(uint256[] calldata _amount) external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];

        if (hasUserLimit) {
            require(_amount.length.add(user.amount.length) <= poolLimitPerUser, 'User amount above limit');
        }
        
        if (lockStakeDate != 0 && _amount.length > 0) {
        ...
        if (user.amount.length > 0) {
            uint256 pending = user.amount.length.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        if (_amount.length > 0) {
            for(uint256 i = 0; i < _amount.length; i++){
                user.amount.push(_amount[i]);
            }
            
            for(uint256 i = 0; i < _amount.length; i++){
                stakedNFT.safeTransferFrom(msg.sender, address(this), _amount[i]);
            }                      
            user.withdrawDate = block.timestamp;
        }

        user.rewardDebt = user.amount.length.mul(accTokenPerShare).div(PRECISION_FACTOR);
     ...
     */
    function withdraw(uint256[] calldata _amount) external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount.length >= _amount.length, 'Amount to withdraw too high');
        ...
        if (_amount.length > 0) {
            ...
            for(uint256 i = 0; i < _amount.length; i++){
                stakedNFT.safeTransferFrom(address(this), msg.sender, _amount[i]);
            }
            for(uint256 i = 0; i < _amount.length; i++){
                user.amount.pop();
            }
    ...
    /*
     * @notice Withdraw staked NFTs without caring about rewards rewards
     * @dev Needs to be for emergency.
     */
    function emergencyWithdraw() external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];        
        for(uint256 i = 0; i < user.amount.length; i++){
            user.amount.pop();
        } 
        user.rewardDebt = 0;
        ...
        if (user.amount.length > 0) {
            for(uint256 i = 0; i < user.amount.length; i++){
                stakedNFT.safeTransferFrom(address(this), msg.sender, user.amount[i]);
            }
        }
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(stakedNFT), 'Cannot be staked NFT');
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedTokenSupply = stakedNFT.balanceOf(address(this));
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            return user.amount.length.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
        } else {
            return user.amount.length.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
        }
    }
    /*
