...
contract USDT is LibNote {
    ...
    function transferFrom(
        ...
        if (src != msg.sender && _allowance(src, msg.sender) != 0) {
            require(_allowance(src, msg.sender) >= wad, "Usdt/insufficient-allowance");
    ...
    function adjustUsdtlyUSDTLimit(uint256 _limit) external auth {
    ...
    function mint(address usr, uint256 wad) external {
        if (wards[msg.sender] == 0) {
            require(
                add(wad, daiMintedToday[msg.sender]) <= dailyUSDTLimit ||
                    (sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= dailyUSDTLimit),
    ...
    function burn(address usr, uint256 wad) external {
        ...
        if (usr != msg.sender && _allowance(usr, msg.sender) != 0) {
            require(_allowance(usr, msg.sender) >= wad, "Usdt/insufficient-allowance");
    ...
    function permit(
        ...
        require(expiry == 0 || block.timestamp <= expiry, "Usdt/permit-expired");
