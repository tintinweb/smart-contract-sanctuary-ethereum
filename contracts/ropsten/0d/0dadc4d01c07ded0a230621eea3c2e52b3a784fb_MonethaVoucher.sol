...
contract MonethaVoucher is IMonethaVoucher, Restricted, Pausable, IERC20, CanReclaimEther, CanReclaimTokens {
    ...
    event PurchasedVouchersReleased(address indexed _from, address indexed _to, uint256 vouchers);
    ...
    uint256 constant public RATE_COEFFICIENT = 100000000000; // 10^11
    uint256 constant private RATE_COEFFICIENT2 = RATE_COEFFICIENT * RATE_COEFFICIENT; // RATE_COEFFICIENT^2
    
    uint256 public voucherMthRate; // number of voucher units in 10^11 MTH units
    uint256 public mthEthRate; // number of mth units in 10^11 wei
    uint256 internal voucherMthEthRate; // number of vouchers units (= voucherMthRate * mthEthRate) in 10^22 wei
