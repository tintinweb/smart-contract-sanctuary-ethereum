...
contract TokenERC20 is Ownable {
    ...
    constructor() public {
        name = "LQUID";                               // Set the name for display purposes
        symbol = "WFX";                               // Set the symbol for display purposes
    }

    modifier canTransfer() {
        require(lockStatus == false || msg.sender == owner);
        _;
    }
    ...
    function _directTransfer(address _to, uint256 _tokenAmount) internal checkAmount(_tokenAmount) returns (bool) {
        // SafeMath.sub will throw if owner doesn't have enough tokens.
        balanceOf[this] = balanceOf[this].sub(_tokenAmount);
        // SafeMath.add checks for overflows
        balanceOf[_to] = balanceOf[_to].add(_tokenAmount);

        emit Transfer(this, _to, _tokenAmount);
        return true;
    }

    function _transfer(address _to, uint256 _value) internal returns (bool) {
        // SafeMath.sub will throw if msg.sender doesn't have enough tokens.
    ...
    function _transferFrom(address _from, address _to, uint256 _value) internal returns (bool) {
        ...
        // SafeMath.add checks for overflows
        balanceOf[_to] = balanceOf[_to].add(_value);
        // will throw if not enough allowance has been given
    ...
    function setEthPrice(uint256 _value) public onlyOwner returns (bool) {
        ethPrice = _value;
        return true;
    }

    function _send(address _user, uint256 amtEth) internal canSend() returns(bool) {

        uint256 tokenValue = tokenUSD.mul(10 ** 20).div(ethPrice);
        uint256 amtTokens = (amtEth.div(tokenValue)).mul(10 ** uint256(decimals));

        require(_directTransfer(_user, amtTokens));
        totalSupply_ = totalSupply_.add(amtTokens);
        return true;
    }

    function setLockStatus(bool status) public onlyOwner returns(bool) {
        lockStatus = status;
        return true;
    }
}
...
contract BancorFormula is Power {
  ...
  bool public bondingStatus = true;
  string public version = "0.3";
  uint32 private constant MAX_WEIGHT = 1000000;
  
  modifier bonded() {
    require(bondingStatus == true);
    _;
  }
  ...
  */
  function calculatePurchaseReturn(uint256 _supply, uint256 _connectorBalance, uint32 _connectorWeight, uint256 _depositAmount) public bonded constant returns (uint256) {
    // validate input
  ...
  */
  function calculateSaleReturn(uint256 _supply, uint256 _connectorBalance, uint32 _connectorWeight, uint256 _sellAmount) public bonded constant returns (uint256) {
    // validate input
 ...
 */
contract BondingCurve is TokenERC20, BancorFormula {

  uint256 public poolBalance;
  uint256 maxBonded;

  event LogMint(uint256 amountMinted, uint256 totalCost);
  event LogSale(uint256 amountSold, uint256 reward);
  ...
  */
  ...
  modifier canSell(uint256 _sellAmount) {
    require(_sellAmount > 0 && balanceOf[msg.sender] >= _sellAmount, "Not enough tokens");
    _;
  }
  ...
  function() public payable {
    if (bondingStatus == true) {
      buy();
    } else {
      require(_send(msg.sender, msg.value));
    }
  }

  function disableBondingCurve() internal returns(bool) {
    bondingStatus = false;
    totalSupply_ = totalSupply_.sub(100000 * 10 ** uint256(decimals));
    balanceOf[this] = (1000000 * 10 ** uint256(decimals)).sub(totalSupply_);
    poolBalance = poolBalance.sub(44923099*10**3);
    return true;
  }
   ...
   * @dev Buy tokens
   */
  function buy() validGasPrice bonded canSend public payable returns(bool) {
    ...
    require(totalSupply_ <= maxBonded);

    balanceOf[msg.sender] = balanceOf[msg.sender].add(tokensToMint);
    poolBalance = poolBalance.add(msg.value);

    // trigger to disable bonding curve & enable auto-ICO mode
    if (totalSupply_ >= 199999999000000000000000) {
      disableBondingCurve();
    }
   ...
   */
  function sell(uint256 sellAmount) validGasPrice bonded canSell(sellAmount) public returns(bool) {
    ...
    balanceOf[msg.sender] = balanceOf[msg.sender].sub(sellAmount);
    totalSupply_ = totalSupply_.sub(sellAmount);
    emit LogSale(sellAmount, ethAmount);
    return true;
  }
...
contract WFX is BondingCurve {
  constructor() public {

    reserveRatio = 24989; // WFX curve
    totalSupply_ = 100000*10**uint256(decimals); // 100,000 bonded WFX
    poolBalance = 44923099*10**3; // starting purchase ~0.000000000018 ETH
    gasPrice = 19 * (10 ** 9); // default max 19 gwei
    maxBonded = totalSupply_*2; // max bonded WFX
  }
}