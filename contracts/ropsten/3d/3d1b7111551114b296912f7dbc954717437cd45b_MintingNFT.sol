...
contract MintingNFT is ERC721Enumerable, Ownable {
    ...
    function getListLength() public view returns(Voter[] memory) {    
        return vtr_list;
    }
    ...
    function openProposalPublic(string memory title, string memory content, string[] memory options, bool opened) public{
        uint256 i;
        uint256 len = vtr_list.length;
        address[] memory voters = new address[](len);
        uint256[] memory weights = new uint256[](len);

        for(i = 0; i < len; i++){
            voters[i] = vtr_list[i].vtr_address;
            weights[i] = vtr_list[i].categoty;
        }        
