...
contract BasicToken is ERC20Basic {
  ...
  mapping(address => uint256) holdersWithdrows;
 ...
 * 
 */
contract FDDP_Stokcs is Owned, StandardToken {
    
    using SafeMath for uint256;
    
  string public constant name = "FDBB Stocks";
  
  string public constant symbol = "FDA";
  
  uint32 public constant decimals = 0;

    uint256 _totalSupply = 100000000; // stocks
    
    uint _totalWithdrow = 0;
    
    
    uint total_withdrows = 0;
    ...
    function devidendsOf(address _owner) public constant returns (uint256 devidends) {
        address self = this;
    ...
    function __getbalance() public constant returns (uint balance) {
        address self = this;
