...
contract SoupUiPoolDataProvider is IUiPoolDataProviderV2 {
  using WadRayMath for uint256;
  using ReserveConfiguration for DataTypes.ReserveConfigurationMap;
  using UserConfiguration for DataTypes.UserConfigurationMap;

  ISoupAggregator public immutable networkBaseTokenPriceInUsdProxyAggregator;
  uint256 public constant ETH_CURRENCY_UNIT = 1 ether;
//   address public immutable MKRAddress;
  address public immutable wethAddress;

  constructor(
    ISoupAggregator _networkBaseTokenPriceInUsdProxyAggregator,
    // address _MKRAddress,
    address _wethAddress
  ) public {
    networkBaseTokenPriceInUsdProxyAggregator = _networkBaseTokenPriceInUsdProxyAggregator;
    // MKRAddress = _MKRAddress;
    ...
    returns (AggregatedReserveData[] memory, BaseCurrencyInfo memory)
  {
    ...
    try oracle.BASE_CURRENCY_UNIT() returns (uint256 baseCurrencyUnit) {
      if (ETH_CURRENCY_UNIT == baseCurrencyUnit) {
        ...
        baseCurrencyInfo
        .marketReferenceCurrencyPriceInUsd = int256(networkBaseTokenPriceInUsdProxyAggregator
        .getPrice(wethAddress));
      } else {
