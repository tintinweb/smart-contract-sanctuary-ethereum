...
contract Auctionv1 is Initializable, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    event NewAuctionCreated(
        uint256[] tokenIds,
        string metaDataUrl,
        address imxANftAddress,
        uint256 endTime,
    ...
    event AuctionEnded(uint256[] tokenIds, address[] winnerAddresses);
    ...
    */
    ...
    modifier onlyIfAuctionIsActive(uint256 auctionId) {
        require(
            auctionsDetails.length > auctionId &&
                auctionsDetails[auctionId].endTime > block.timestamp
        );
        _;
    }
    ...
    function startNewAuctions(
        ...
        _auc.metaDataUrl = metaDataUrl;
        _auc.imxNftAddress = imxANftAddress;
    ...
    ) external onlyIfAuctionIsActive(autionId) {
        Auctions storage _auc = auctionsDetails[autionId];
        require(_auc.price == 0, "Can't place bid.");
    ...
    function _endAuctions(uint256 auctionId) internal {
        ...
        _auc.isEnded = true;
        _auc.paymentToken.safeTransfer(withdrawAddress, totalTokenToSend);
        emit AuctionEnded(tokenIds, winnerAddresses);
    }

    function endAuctions() external onlyOwner {
        for (uint256 i = 0; i < auctionsDetails.length; i++) {
            if (
                !auctionsDetails[i].isEnded &&
                auctionsDetails[i].endTime < block.timestamp
            ) {
                _endAuctions(i);
            }
        }
    ...
    function getTokenBid(uint256 auctionId, uint256 tokenId)
        external
        view
    ...
    function getActiveAuctionsId() external view returns (uint256[] memory) {
        uint256 size = 0;
        for (uint256 i = 0; i < auctionsDetails.length; i++) {
            if (!auctionsDetails[i].isEnded) {
                size++;
            }
        }
