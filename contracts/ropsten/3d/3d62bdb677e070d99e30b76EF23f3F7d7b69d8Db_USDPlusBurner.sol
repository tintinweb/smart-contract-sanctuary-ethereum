...
/// @notice 
/// @dev 
contract USDPlusBurner is Pausable, AccessControl, ReentrancyGuard {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");// assigned to vault
    bytes32 public constant REQUESTER_BURNER_ROLE = keccak256("REQUESTER_BURNER_ROLE");// the redeemers needs to have a request burner role
    bytes32 public constant PROTOCOL_MANAGER = keccak256("PROTOCOL_MANAGER");

    //burn events
    event BurnRequestUSDPlus(
        ...
        returns (bool isRequestPlaced)
    {
        ...
        require(IERC20(usdPlusAddr).balanceOf(msg.sender) >= amount, "NOT_ENOUGH_BALANCE");

        require(
            _isWhiteListed(fedMemberID),
            "NOT_WHITELISTED"
        );
        ...
        emit BurnRequestUSDPlus(msg.sender, refId, redeemerContractAddress, redeemerPerson, fedMemberID, amount);
    ...
    function executeBurn(
        ...
        uint256 amount,
        address vault
    ) external onlyRole(BURNER_ROLE) whenNotPaused returns (bool isAmountBurned) {
        ...
        require(
            ticket.redeemerContractAddress == redeemerContractAddress,
            "WRONG_REDEEMER_CONTRACT"
        );
        require(ticket.amount == amount, "WRONG_AMOUNT");

        emit BurnExecution(vault, refId, redeemerContractAddress, fedMemberId, amount);

        ticket.executed = true;
        ticket.confirmedBlock = block.number;

        
        IFluentUSDPlus(usdPlusAddr).burnFrom(redeemerContractAddress, ticket.amount);
    ...
    {
        require(newComplianceManagerAddr != address(0x0), 'ZERO Addr is not allowed');
        complianceManagerAddr = newComplianceManagerAddr;
    }

    /// @notice Links the current contract to original USDPlus contract
    /// @dev
    /// @param newUsdPlusAddr The address where the USDPlus was deployed
    function setUsdPlusAddr(address newUsdPlusAddr)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(newUsdPlusAddr != address(0x0), 'ZERO Addr is not allowed');
    ...
    /// @notice Checks if the address is blacklisted or not
    /// @dev use to check before business critical function calls
    /// @param _addr address to be checked
    /// @return booleanValue confirmation if the address is blacklisted or not    
