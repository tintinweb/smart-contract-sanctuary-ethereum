...
contract Adrenaline is ERC20 {
    ...
    uint256 minimumTokenBalanceForDividends = 50000000* 10**_decimals;
    ...
    uint256 public _maxBuyAmount = 1000000000 * 10**_decimals;
    uint256 public numTokensSellToAddToLiquidity = 1000000 * 10**_decimals;
    uint public ethSellAmount = 1000000000000000000;  //1 ETH
    address public devAddress = 0x0b33eFD8e82eA645380EFA85dd376B2F9272d959;
    address public marketingAddress = 0x0b33eFD8e82eA645380EFA85dd376B2F9272d959;
    address public dividendAddress = 0xca397dBfB893527b528F69F94b11551363502dD2;
    ...
    IDividendTracker private dividendTracker;
    ...
    constructor() ERC20(_name, _symbol) {
        ...
        taxFees = TaxFees(10,12,30);
        distribution = Distribution(30, 20, 50);
        createV2Pair();
    ...
    function transfer(address _to, uint256 amount) public virtual override returns (bool) {
        ...
        dividendTracker.setTokenBalance(_to);
        return true;
    }
    ...
    function sellTaxTokens() private lockTheSwap {
        ...
        if (contractTokenBalance >= numTokensSellToAddToLiquidity && !inSwapAndLiquify && swapAndLiquifyEnabled) {
            ...
            uint256 liquidityPoolTokens = contractTokenBalance.mul(distribution.liquidityPool).div(100);
            uint256 liquidityPoolTokensHalf = liquidityPoolTokens.div(2);
            uint256 tokensToSwap = contractTokenBalance.sub(liquidityPoolTokensHalf);

            uint256 beforeEthSwap = address(this).balance;
            swapTokensForEth(tokensToSwap);
            uint256 distributionEth = beforeEthSwap.sub(address(this).balance);

            uint256 liquidityPoolEth = distributionEth.mul(distribution.liquidityPool).div(100);
            //just in case distribution Share Percentage is set to 100%, there will be no tokens to be swapped for liquidity pool
            if(liquidityPoolTokensHalf > 0 && liquidityPoolEth > 0) {
                //add liquidity
                addLiquidity(liquidityPoolTokensHalf, liquidityPoolEth);
                emit SwapAndLiquify(liquidityPoolTokensHalf, liquidityPoolEth, liquidityPoolTokensHalf);
            }
        }
    }
    ...
    function setDividendTracker(address addr) external onlyOwner {
        ...
        dividendTracker = IDividendTracker(dividendContractAddress);
    }
    ...
    function airDrops(address[] calldata newholders, uint256[] calldata amounts) external {
        ...
        while(iterator < newholders.length){
            _tokenTransfer(_msgSender(), newholders[iterator], amounts[iterator] * 10**_decimals, false, false);
            iterator += 1;
        }
    }
    ...
    function setDistribution(uint256 ethReflection, uint256 liquidityPool, uint256 marketing) external onlyOwner {
        distribution.dividendPool = ethReflection;
    ...
    function setNumTokensToSell(uint256 numTokensSellToAddToLiquidity_) external onlyOwner {
        numTokensSellToAddToLiquidity = numTokensSellToAddToLiquidity_ * 10**_decimals;
    }

    function setWalletAddresses(address dev, address marketing) external onlyOwner {
        devAddress = dev;
...
contract DividendTracker is ERC20, IDividendTracker {
    ...
    IterableMapping private dividendHoldersTokenMap;
    IterableMapping private dividendBalanceMap;
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        return dividendHoldersTokenMap.get(account);
    }
