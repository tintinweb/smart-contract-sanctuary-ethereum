...
contract HpbBallot {
    ...
    function setNextSnapshotBlock(
        ...
        for(uint i = 1;i<cl;i++){
           autoVote.nextVoterBalance[
               voteStages[voteStageIndex].voterArray[i].voterAddr
    ...
    function addCandidate(
        ...
        voteStages[0].candidateArray.push(Candidate(_candidateAddr,0,new address[](0)));
    ...
    function  getNextSnapshotBalance(
        ...
        require(autoVote.nextSnapshotBlock!=0);
        return autoVote.nextVoterBalance[voterAddr];
    }
    ...
    function  voteNoLockByAdmin(
        ...
        for(uint i = 1;i<cl;i++){
            uint bl = autoVote.nextVoterBalance[
                voteStages[stageIndex].voterArray[i].voterAddr
            ];
    ...
    function calVoteResult(
        ...
        for(uint n = 0;n<_addrs.length;n++){
            candidateIndexArray.push(
                voteStages[round].candidateIndexMap[_addrs[n]]
            );
        }
        return (_addrs,_facilitys,_nums);
    }
    /**
     * 获取指定阶段投票人的快照余额和总投票数
     * fetch the snapshot balances and total votes of voters.
     */
    function fetchVoteMainInfoBySnapshotBlock(
        uint _snapshotBlock
    ) onlyVoteAfterOpen public constant returns (
        uint snapshotBalance,
        uint voteNumber
    ){
        return fetchVoteMainInfoForVoterBySnapshotBlock(msg.sender,_snapshotBlock);
    }
     ...
     * fetch the snapshot balances and total votes of voters.
     */
    function fetchVoteMainInfo(
    ) onlyVoteAfterOpen public constant returns (
        uint snapshotBalance,
        uint voteNumber
    ){
        return fetchVoteMainInfoBySnapshotBlock(currentSnapshotBlock);
    }
    function fetchVoteMainInfoForVoter(
        address voterAddr
    ) onlyVoteAfterOpen public constant returns (
        uint snapshotBalance,
        uint voteNumber
    ){
        return fetchVoteMainInfoForVoterBySnapshotBlock(voterAddr,currentSnapshotBlock);
    }

    /**
     * 获取指定阶段投票人的所有投票情况 fetch all the votes of voters.
     */
    ...
    /**
     * 获取当前阶段投票人的所有投票情况 fetch all the votes of voters.
     */
    function fetchVoteInfoForVoter(
        ...
        return fetchVoteInfoForVoterBySnapshotBlock(voterAddr,currentSnapshotBlock);
    }
    /**
     ...
     */
    function fetchStageIndexBySnapshotBlock(
        uint _snapshotBlock
    ) onlyVoteAfterOpen public constant returns (
        uint _index
    ){
    ...
    function fetchBalanceByAddress(
        address[] paddrs
    ) onlyVoteAfterOpen public constant returns (
        address[] addrs,
        uint[] nums
