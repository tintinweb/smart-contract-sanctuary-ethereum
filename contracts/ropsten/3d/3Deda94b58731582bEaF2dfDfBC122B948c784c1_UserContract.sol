...
contract AuctionContract is ReentrancyGuard {
    ...
    function createAuction(string memory _url, string memory _name, uint _curPrice, uint _dataNum, DataType _dataType, DataField _dataField, uint _deadline) public payable nonReentrant returns (uint){
        require(userContract.isBuyer(msg.sender), "Only buyers can create auction!");
    ...
    function createBidder(uint _auctionId) public payable nonReentrant validId(_auctionId) {
        require(userContract.isSeller(msg.sender), "Only sellers can become bidder!");
...
contract Market is ReentrancyGuard {
    ...
    function createSale(uint _price, 
        ...
        require(userContract.isBuyer(buyer), "Buyer is not valid buyer!");
        require(userContract.isSeller(seller), "Seller is not valid seller!");
    ...
    function putDigestAndDeposit(uint _saleId, string memory _digest) public payable nonReentrant {
    ...
    function payPriceAndDeposit(uint _saleId) public payable nonReentrant{
        require(_saleId > 0 && _saleId <= saleId, "Sale doesn't exist!");
        Sale storage sale = idToSale[_saleId];
    ...
    function confirm(uint _saleId, bool succeeded) public {
        require(_saleId > 0 && _saleId <= saleId, "Sale doesn't exist!");
        Sale storage sale = idToSale[_saleId];
...
contract DataContract is ReentrancyGuard {
    address payable owner;
    AuctionContract auctionContract;
    UserContract userContract;
    ...
    function createData(string memory _url, string memory _name, uint _price, AuctionContract.DataType _dataType, AuctionContract.DataField _dataField, uint _createdTime, string memory _uuid) public returns (uint){
        require(userContract.isSeller(msg.sender), "Only sellers can create data!");
        require(bytes(_url).length != 0 && bytes(_name).length != 0, "Parameters can't be empty!");
        dataId++;
