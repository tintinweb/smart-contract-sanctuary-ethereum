...
contract 	Admins is Ownable {
	...
	*/
	...
	function 	setNextSmartContract(
		...
		onlyOwner()
	{
		require(_nextSmartContract == address(0x0));
...
contract BalanceToken is Admins, Math {
	...
	function 	getAddressToken() view public returns(address) {
		return (_token);
	}

	function 	getAddressLiquidatePool() view public returns(address) {
		return (_liquidatePool);
	}
}
...
contract BetsMatch is BalanceToken {
	...
	*/
		...
		public
	{
		...
		require(arrayHashMarketItem.length != 0 && arrayHashOutcome.length != 0);

		uint elem = 0;
		for (uint j = 0; j < arrayHashMarketItem.length; j++) {
			require(_marketItemMapping[arrayHashMarketItem[j]].hashEventParent == bytes16(0x0)); // Protection against reuse
	...
	*/
	function 	acceptBet(
		...
		public
	{
		require(_eventsMapping[hashEvent].hashMarketItem.length != 0); // проверка что евент существует, важно защита от 0x0
	...
	*/
	...
	// function 	getStatusEventClosed(bytes16 hashEvent) view public returns (bool) {
	// 	return (_eventsMapping[hashEvent].closed);
	// }

	// function 	getStatusEventCloseSuccess(bytes16 hashEvent) view public returns (bool) {
	// 	return (_eventsMapping[hashEvent].closeSuccess);
	// }

	// function getInfoOutcome(
	// 	bytes16 hashOutcome
	// )
	// 	view
	// 	public
	// 	returns (bytes16, bytes16[], uint)
	// {
	// 	return (
	// 		_outcomeMapping[hashOutcome].hashMarketItem,
	// 		_outcomeMapping[hashOutcome].bet,
	// 		_outcomeMapping[hashOutcome].amountCloseBet
	// 	);
	// }
	/* 
