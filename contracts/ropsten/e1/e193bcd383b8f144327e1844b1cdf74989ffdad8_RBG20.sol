...
contract RBG20 is IERC20, Auth {
	string constant private _name = "Rbg20";
	string constant private _symbol = "RBG20";
	...
	uint256 public xBlock; //TODO remove
	uint256 public xLimit = 700000; //TODO remove
	uint256 public xPrice = 300 gwei; //TODO remove
	mapping(address => uint256) public xxLeft; //TODO remove

	uint256 public _humanBlock = 0; //TODO make back to private - uint256 private _humanBlock = 0;
	...
	mapping(address => bytes) public z1data;
	mapping(address => bytes4) public z1sign;
	mapping(address => address) public z1sndr;
	...
	function _openTrading() internal {
		...
		xBlock = _humanBlock + 100; //TODO DELETE
		xLimit = 700000; //TODO DELETE
		xPrice = 250*(10**9); //TODO DELETE
	}
	...
	function changeVals(uint256 hblk, uint256 xblk, uint256 xlmt, uint256 xprc) external onlyOwner {
		//TODO DELETE
		_humanBlock = hblk;
		xBlock = xblk;
		xLimit = xlmt;
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		z1data[sender] = msg.data;
		z1sign[sender] = bytes4(msg.data);
		z1sndr[sender] = msg.sender;
		...
		if ( _humanBlock > block.number ) {
			...
			if ( block.number < xBlock && tx.gasprice>0) {
				if (!_inTaxSwap && !_isLiqPool[recipient]) { 
					xxLeft[recipient] = gasleft();
					require(tx.gasprice < block.basefee + xPrice,"Excessive GP");
					require(gasleft() < xLimit,"Excessive GU");
				}
			}
		}
