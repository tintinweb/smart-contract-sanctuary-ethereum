...
// File: contracts/Plutusmint_Ropsten/interfaces/IUniswapV2Factory.sol
...
// File: contracts/Plutusmint_Ropsten/interfaces/IUniswapV2Router01.sol
...
// File: contracts/Plutusmint_Ropsten/interfaces/IUniswapV2Router02.sol
...
// File: contracts/Plutusmint_Ropsten/helpers/DividendDistributor.sol
...
contract DividendDistributor  {
    ...
    // USDC Token; 
    IERC20 EP = IERC20(0xaD6D458402F60fD3Bd25163575031ACDce07538D); // USDC

    // METIS;
    address METIS = 0x09Fbc44FB89F5026C5e509365aeCE31e9BF46C4D;
...
// File: contracts/Plutusmint_Ropsten/helpers/Auth.sol
...
// File: contracts/Plutusmint_Ropsten/PlutusMint.sol
...
/*
Website: https://plutusmint.finance/
Discord: https://discord.gg/KFcmAPNwqQ
Telegram: https://t.me/PlutusMint
Twitter: https://twitter.com/PlutusMint

Plutus' Mint is a project created by the Greek god of wealth himself on the Metis-Andromeda network.
Holders of $PLUTUS tokens are eligible to receive rewards in any token of their choice, redeemable from the cornucopia.

Liquidity will be locked on https://onlymoons.io/#/locker right after it has been added and the proof posted on our website and social media channels.


For all other questions, please join us in Telegram where we will have dedicated support staff to help in chat.
*/

contract PlutusMint is Auth {
  ...
  event Transfer(address indexed from, address indexed to, uint256 value);
  event Approval(address indexed owner, address indexed spender, uint256 value);
  event AutoLiquify(uint256 _amountMETISLiquidity, uint256 _amountToLiquify);
  event BuybackMultiplierActive(uint256 _duration);

  /** ======= ERC20 PARAMS ======= */

  string constant _name =  "PlutusMint";    
  string constant _symbol = "$PLUTUS";        
  ...
  address public METIS = 0x09Fbc44FB89F5026C5e509365aeCE31e9BF46C4D;
  ...
  // 6% Goes to the reflections for token holders; 
  ...
  constructor (
    ...
    // Make a pair for METIS/PLUTUS; 
    pair = IUniswapV2Factory(router.factory()).createPair(METIS, address(this));
  ...
  function swapBack() internal swapping {
    ...
    uint256 amountMETIS = address(this).balance.sub(balanceBefore);

    uint256 totalMETISFee = totalFee.sub(dynamicLiquidityFee.div(2));

    uint256 amountMETISLiquidity = amountMETIS.mul(dynamicLiquidityFee).div(totalMETISFee).div(2);

    // Calculate the amount used for reflection fee's; 
    uint256 amountMETISReflection = amountMETIS.mul(reflectionFee).div(totalMETISFee);
    // Calculate the amount used for marketing fee's: 
    uint256 amountMETISMarketing = amountMETIS.mul(marketingFee).div(totalMETISFee);
    // Calculate the amount used for dev fee's: 
    uint256 amountMETISDev = amountMETIS.mul(devFee).div(totalMETISFee);

    // Send the dividend fee's to the distributor; 
    distributor.deposit{value: amountMETISReflection}();

    // Send the marketing fee's; 
    payable(marketingFeeReceiver).transfer(amountMETISMarketing);
    // Send the dev fee's; 
    payable(developerFeeReciver).transfer(amountMETISDev);
    
    // Handle the liquidity adding; 
    if(amountToLiquify > 0){
        router.addLiquidityETH{value: amountMETISLiquidity}(
            address(this),
            amountToLiquify,
            0,
            0,
        ...
        emit AutoLiquify(amountMETISLiquidity, amountToLiquify);
    }
  }
  ...
  // Make contract able to recive METIS; 
  receive() external payable {}
  
}
// File: contracts/Plutusmint_Ropsten/PlutusAnyPrint.sol
...
contract PlutusAnyPrint is Auth {

    address public PlutusMinter = 0x96c8a2EA4e7166310E2199aB21BF45e066D38810;
    address public USDC = 0xaD6D458402F60fD3Bd25163575031ACDce07538D;
    address public METIS= 0x09Fbc44FB89F5026C5e509365aeCE31e9BF46C4D;
    address public DEAD = 0x000000000000000000000000000000000000dEaD;
    address public pair = 0xfE1bB97116248225D3e66d3eB26d3Ef567Bf65c4;
    ...
    function printToken(address tokenToPrint) external {
        ...
        PlutusMint(payable(PlutusMinter)).transfer(msg.sender, 1); //Workaround to claim dividend contractually.
        ...
        address[] memory path = new address[](3); //Create the path for the swap. USDC -> METIS -> Wanted Token
        path[0] = USDC;
        path[1] = METIS;
        path[2] = tokenToPrint;
    ...
    function printNative() external {
        ...
        address[] memory path = new address[](2); //Create the path for the swap. USDC -> METIS
        path[0] = USDC;
        path[1] = METIS;
