...
contract MBTC is ERC20 {
    ...
    constructor(string _name, string _symbol, uint8 _decimals, uint256 _nav) ERC20(_name, _symbol, _decimals) public {
         owner = msg.sender;
         nav = _nav;
      }
    ...
    function updateUnderlying(string _underlyingName, uint256 _underlyingNameWeight) public {
        require(msg.sender == owner);
        underlyings[_underlyingName] = _underlyingNameWeight;
    }
    
    function weightOf(string _underlyingName) public view returns (uint256) {
        return underlyings[_underlyingName];
    }    
    
    function buy() external payable {
        emit Debug("buy() msg.value", msg.value);
        buyTo(msg.sender);
    }
    
    function buyTo(address beneficiary)  public payable {
        require(beneficiary != address(0));
        uint256 nbTokens = (msg.value / (nav / 10**6))*10**12;
        emit Debug("buytTo() nbTokens", nbTokens);
        
        totalSupply_ = totalSupply_.add(nbTokens);
        balances[beneficiary] = balances[beneficiary].add(nbTokens);
