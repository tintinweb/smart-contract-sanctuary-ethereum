...
contract ScribePriceOracle is IScribePriceOracle, Ownable {
    ...
    function update() external override {
        require(tryUpdate(), 'ScribePriceOracle: PERIOD_NOT_ELAPSED');
    }
    ...
    function consult(address token, uint amountIn) external view override returns (uint amountOut) {
        if (token == token0) {
            ...
            require(token == token1, 'ScribePriceOracle: INVALID_TOKEN');
    ...
    function changePeriod(uint256 _period) public onlyOwner {
        require(_period > 0, 'ScribePriceOracle: INVALID_PERIOD');
        period = _period;
        ...
        require(reserve0 != 0 && reserve1 != 0, 'ScribePriceOracle: NO_RESERVES'); // ensure that there's liquidity in the pair
