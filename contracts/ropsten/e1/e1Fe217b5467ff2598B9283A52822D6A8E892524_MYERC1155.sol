...
contract MYERC1155 is AccessControl, ERC1155, ERC1155Burnable {
    ...
    constructor (address _addPriceRole) ERC1155("") {
        for(uint i; i < 5; ++i){
            uint tokenMint = _tokenIDs.current();
            _tokenIDs.increment();
            _mint(msg.sender, tokenMint, 10, "");
            _tokensCount[tokenMint] = 10;
    
        }
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(TOKEN_COST_ROLE, _addPriceRole);
        owner = _msgSender();// No need 
    }
    ...
    function tokenPrice(uint _price) external onlyRole(TOKEN_COST_ROLE){
        require(_price > 0, "The token price should be not zero");
        _tokenPrice = _price;
    }

    function availableTokens(uint _tokenId) external view returns(uint){
        require(_tokenId >= 0 && _tokenId < 5, "Not a valid token id");
        return 100 - _tokensCount[_tokenId];
    }

    function tokensMint(uint _tokenId, uint _tokenQuantity) external payable {
        require(_tokenQuantity > 0 && _tokenPrice > 0, "The Quantity / Price should not be zero");
        require(_tokenId >= 0 && _tokenId <= 4, "Not a valid token id");
        require(_tokensCount[_tokenId] + _tokenQuantity <= 100, "Tokens already minted");//
    ...
    function tokensBurn(address _from, uint _tokenId, uint _amount) external {// msg.sender ...
...
// ERC721, whitle list, 3 phaes of sales, TS 10,000, (2k,3k,5k)...diffrenet prices,
