...
contract StakeAirdropFactory is IStakeAirdropFactory, AirdropAccessControl{
    ...
    function createCycle(
        ...
        proxy = new TransparentUpgradeableProxy(address(stakeAirdrop), proxyAdmin_, '');
...
contract StakeAirdrop is IStakeAirdrop, FireCatTransfer, AirdropAccessControl {
    ...
    event SetClaimOn(bool claimOn_);
    ...
    */
    ...
    modifier modifyLocked() {
        require(_totalClaimed == 0, "user already claimed, params can not be modified");
        _;
    }

    modifier beforeStake() {
        require(isStakeOn, "stake is off");
        _;
    }

    modifier beforeClaim() {
        require(isClaimOn, "claim is off");
        _;
    }
    ...
    function reviewOf(address user_) public view returns (uint256, uint256, uint256) {
        ...
        require(totalClaim <= _totalSupply, "Total claim is greater than total supply");
        require(availableClaim + _claimed[user_] <= _maxFakeClaim, "Available claim is greater than total claim");
        require(lockedClaim + _maxFakeClaim <= totalClaim, "Locked claim is greater than total claim");
    ...
    */
    ...
    function setCycleTime(uint256 startAt_, uint256 expiredAt_) external modifyLocked onlyRole(DATA_ADMIN) {
        require(startAt_ > block.timestamp && expiredAt_ > startAt_, "Start time is less than current time");
        _startAt = startAt_;
    ...
    function setReleaseConfig(uint256[] memory releaseTimePeriod_, uint256[] memory releaseNumFactor_) external modifyLocked onlyRole(DATA_ADMIN) {
        require(releaseTimePeriod_.length > 0, "releaseTimePeriod_ is empty");
        require(releaseTimePeriod_.length == releaseNumFactor_.length, "Length of timePeriod and issueFactor must be equal");
        require(releaseTimePeriod_[0] > _expiredAt, "First release time period is less than expired time");

        uint256 arrayLen = releaseTimePeriod_.length;
        uint256 releaseNumTotal = 0;
        for (uint256 i = 0; i < arrayLen; i++) {
            if (i + 1 < arrayLen) {
                require(releaseTimePeriod_[i] < releaseTimePeriod_[i + 1], "Time period must be in ascending order");
            }

            releaseNumTotal += releaseNumFactor_[i];
        }
        require(releaseNumTotal == 100, "Total release factor must be equal 100");
    ...
    /// @inheritdoc IStakeAirdrop
    function importStaked(address[] memory userArray_, uint256[] memory userStaked_) external modifyLocked onlyRole(DATA_ADMIN) returns(bool) {
        require(userArray_.length == userStaked_.length, "Length of timePeriod and issueFactor must be equal");

        uint256 totalStakedNew;
        for (uint256 i = 0; i < userArray_.length; i++) {
            address uesr = userArray_[i];
            uint256 userStaked = userStaked_[i];

            totalStakedNew = _totalStaked.add(userStaked);
            require(totalStakedNew > _totalStaked, "Total staked is less than before");

            _totalStaked = totalStakedNew;
            _staked[uesr] = _staked[uesr].add(userStaked);
    ...
    /// @inheritdoc IStakeAirdrop
    function withdrawRemaining(address token, uint256 amount) external nonReentrant onlyRole(DATA_ADMIN) returns (uint) {
        require(!isClaimOn, "Claim is not over");
        return withdraw(token, amount);
    }

    /// @inheritdoc IStakeAirdrop
    function topUp(uint256 addAmount) external modifyLocked onlyRole(DATA_ADMIN) returns (uint256) {
        require(IERC20(airdropToken).balanceOf(msg.sender) >= addAmount, "Not enough balance");
        require(addAmount == _topUpAmount, "Top up amount is not equal to top up amount");
        ...
        /* Revert on overflow */
        require(totalSupplyNew > _totalSupply, "Total supply is less than before");
    ...
    function stake(uint256 amount) external beforeStake nonReentrant returns (uint256) {
        require(block.timestamp > _startAt && block.timestamp < _expiredAt, "Stake is expired or not started");
        require(IERC20(stakeToken).balanceOf(msg.sender) >= amount, "Insufficient balance");
    ...
    function claim() external beforeClaim nonReentrant returns (uint256) {
        ...
        require(availableClaim > 0, "You have no available claim");
...
interface IStakeAirdropFactory {
    ...
    */
    function createCycle(address proxyAdmin_, address airdropToken_, address stakeToken_ ) external returns(address);
...
interface IStakeAirdrop {
    ...
    * @return result
    */
    function importStaked(address[] memory userArray_, uint256[] memory userStaked_) external returns(bool);
