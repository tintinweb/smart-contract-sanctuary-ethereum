...
contract BoosterV4 is Initializable, Ownable, ReentrancyGuard {
    ...
    function getRewardBurnDebt(UserInfo storage user) internal view returns (uint256) {        

        uint256 stakedTokenSupply = stakedToken.balanceOf(address(this));
        uint256 blockNumber = block.number;
        uint256 blocks = blockNumber - user.lastUserBurnBlock;
        ...
        uint256 rewardByBlock = gpReward.mul(PRECISION_FACTOR).div(blockNumber.sub(startBlock));
        ...
        if (blocks > burnAfterBlocks) {
            ...
            rewardByBlock = gpReward.mul(PRECISION_FACTOR).div(blockNumber.sub(startBlock));

            uint256 rewards = (blockNumber - (user.lastUserBurnBlock + burnAfterBlocks)).mul(rewardByBlock);
            return rewards.div(stakedTokenSupply).mul(user.amount).div(PRECISION_FACTOR);
        }

        uint256 multiplier = rewardByBlock.add(user.amount.mul(PRECISION_FACTOR).div(burnAfterBlocks));

        return blocks.mul(multiplier).div(stakedTokenSupply).div(PRECISION_FACTOR);
    }


    function getBurnDebt(UserInfo storage user) internal view returns (uint256) {
        uint256 blockNumber = block.number;
        uint256 blocks = blockNumber - user.lastUserBurnBlock;
        if (blocks > burnAfterBlocks) {
            return user.amount.mul(PRECISION_FACTOR);
        }
        uint256 burnMultiplier = user.amount.mul(PRECISION_FACTOR).div(burnAfterBlocks);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            // calc burnDebt
            uint256 rewardBurnDebt = getRewardBurnDebt(user);

            // calc max reward
            uint256 pending = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt).sub(rewardBurnDebt);
        ...
        if (_amount > 0) {
            ...
            if (user.amount > 0) {
                ...
                uint256 burnDebt = getBurnDebt(user);
                uint256 _oldAmount = user.amount;
                console.log("----> Deposit, existing burnDebt %s", burnDebt);
                // set user values
                user.amount = (user.amount.mul(PRECISION_FACTOR).sub(burnDebt)).div(PRECISION_FACTOR);
                
                // Actual burn pending tokens 
                _burnTokens(_oldAmount.sub(user.amount));
                user.lastUserBurnBlock = block.number;
            }
            console.log("----> Deposit, existing user.amount %s", user.amount);
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        uint256 amountAfterBurn = (user.amount.mul(PRECISION_FACTOR).sub(burnDebt)).div(PRECISION_FACTOR);
            
        console.log("WITHDRAW amountAfterBurn",  amountAfterBurn);
        require(amountAfterBurn >= _amount, "Amount to withdraw too high");
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        uint256 amountToTransfer = user.amount; // .sub(burnDebt);
        user.amount = 0;
        user.rewardDebt = 0;
    ...
    function getUserInfo(address _user) external view returns (uint256 amount, uint256 rewardDebt, 
                        uint256 lockedAt, uint256 lastUserBurnBlock, uint256 burnDebt) {
         UserInfo storage user = userInfo[_user];

        burnDebt = getBurnDebt(user);

        console.log("BURN DEBT %s", burnDebt);
        console.log("AMOUNT - BURN DEBT %s", (user.amount.mul(PRECISION_FACTOR).sub(burnDebt)).div(PRECISION_FACTOR));

        return ( user.amount, // (1e18 ** burnEvents), 
        user.rewardDebt, user.lockedAt, user.lastUserBurnBlock, burnDebt);
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            // console.log("GP REWARD %s", gpReward);
            ...
            console.log("REWARD BURN DEBT %s", rewardBurnDebt);
            console.log("USER REWARD %s", user.amount.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt));
            return user.amount.mul(adjustedTokenPerShare)
    ...
    function getMonthlyReward(uint256 blockNumber) public view returns (MonthlyReward memory) {
        for (uint i = 0; i < slices.length; i++) {
            ...
            if (blockNumber >= slice.startBlock && blockNumber <= slice.endBlock) {
               // console.log("Searching blockNumber %s from slice %s to %s", blockNumber, slice.startBlock, slice.endBlock);
                return slice;
            }
        } 
    ...
    function _rewardForSlices(uint256 blockNumber, MonthlyReward memory fromSlice, MonthlyReward memory toSlice) internal view returns (uint256 reward) {
