...
// File: verified-sources/0x4Cfa346DBf785317cb11895b88f1E14DAc304c68/sources/contracts/Market/Interfaces/IParams.sol
...
// File: verified-sources/0x4Cfa346DBf785317cb11895b88f1E14DAc304c68/sources/contracts/Market/Interfaces/IGetters.sol
...
// File: verified-sources/0x4Cfa346DBf785317cb11895b88f1E14DAc304c68/sources/contracts/Market/Contracts/Getters.sol
...
contract Getters is IGetters, Ownable {
    address paramsAddress;

    function setParams(address _paramsAddress) external onlyOwner {
        paramsAddress = _paramsAddress;
    }

    function getTokenDetails(address _token_address, uint256 _tokenId)
        external
        view
        returns (string memory, string memory)
    {
        return (
            ERC721(_token_address).tokenURI(_tokenId),
            ERC721(_token_address).name()
        );
    }

    function getItemWithId(uint256 id) external returns (IParams.Item memory) {
        IParams.Item memory item = IParams(paramsAddress).getItemById(id);
        return item;
    }
    ...
    function getItem(address _token_address, uint256 _tokenId)
        ...
        returns (IParams.Item memory)
    {
        IParams params = IParams(paramsAddress);
        IParams.Item memory item = params.getItemByAddress(
            _token_address,
            _tokenId
        );
        _itemExist(item);
    ...
    function getListedItems() external returns (IParams.Item[] memory) {
        IParams params = IParams(paramsAddress);
        uint256 size = 0;
        uint256 itemId = params.getItemId();
        for (uint256 i = 1; i < itemId; i++) {
            IParams.Item memory item = params.getItemById(i);
            if (
                item.status == IParams.Status.LISTED &&
                item.time_period >= block.timestamp
            ) {
                size++;
            }
        }
        IParams.Item[] memory _items = new IParams.Item[](size);
        size = 0;
        for (uint256 i = 1; i < itemId; i++) {
            IParams.Item memory item = params.getItemById(i);
            if (
    ...
    function getRejectedBuyNowPayLaterOffer(address caller)
        ...
        returns (IParams.BuyNowPayLaterOffer[] memory)
    {
        return IParams(paramsAddress).getRejectedBuyNowPayLaterOffers(caller);
    }
    ...
    function getRejectedSwapOffer(address caller)
        ...
        returns (IParams.SwapOffer[] memory)
    {
        return IParams(paramsAddress).getRejectedSwapOffers(caller);
    }

    // internal
    function _itemExist(IParams.Item memory item) internal pure {
