...
contract Knjiznica {
    ...
    function checkAvailability(string memory title) public view returns(uint)
		{
            uint result = 0;
            for (uint i=0; i<BookArray.length; i++) {
				if(BookArray[i].borrowed == false && keccak256(abi.encodePacked(BookArray[i].title)) == keccak256(abi.encodePacked(title))) {
					result++;
				}
			}
            return result;
