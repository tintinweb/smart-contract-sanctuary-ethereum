...
contract AlbaritTokenV4 is BurnableToken, UpgradeableToken {
  ...
  function AlbaritTokenV4(address _owner, string _name, string _symbol, uint _totalSupply, uint _decimals)  UpgradeableToken(_owner) {
    name = _name;
    symbol = _symbol;
...
modifier onlyOwner() {
  ...
  // privileged transfer
  function transferPrivileged(address _to, uint _value) onlyOwner onlyPayloadSize(2 * 32) returns (bool success) {
