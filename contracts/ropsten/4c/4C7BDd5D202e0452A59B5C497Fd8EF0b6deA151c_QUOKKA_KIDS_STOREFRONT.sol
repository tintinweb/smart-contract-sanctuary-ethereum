...
contract QUOKKA_KIDS_STOREFRONT is Ownable {
  ...
  function preorderMint(uint doublePreorders, uint triplePreorders) payable public {
    ...
    // check holdings
    console.log("POContract.balanceOf(_msgSender(), DOUBLE_TICKET)", POContract.balanceOf(_msgSender(), DOUBLE_TICKET));
    console.log("doublePreorders", doublePreorders);
  ...
  function parent(bool[] memory tokentypes, uint256[] memory parents, address[] memory owners, bool[] memory parentsfur, bytes memory signature) public payable {
    ...
    console.log('whitelistSigner', whitelistSigner);
