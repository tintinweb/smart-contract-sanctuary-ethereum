...
contract JoydomCreatorPass is Ownable, ERC721A, ReentrancyGuard {
    ...
    function isInAllowList2() public view returns (address) {
        return msg.sender;
    }

    function isInAllowList3(bytes32[] memory proof) public view returns (bytes32) {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        uint256 count = proof.length;
        if (count == 0) {
          return 0x0123456789012345678901234567890123456789012345678901234567891234;
        }
        return leaf;
    }

    function isInAllowList4(bytes memory a) public pure returns (bytes32) {
        bytes32 leaf = keccak256(abi.encodePacked(a));
        return leaf;
    }

    function isInAllowList5(bytes32[] memory proof) public pure returns (uint256) {
        return proof.length;
    }

    function isInAllowList6(bytes32[] memory proof) public pure returns (bytes32) {
        bytes32 a = proof[1];
        return a;
    }

    function isInAllowList7(bytes memory addr, bytes32[] memory proof) public view returns (bool) {
