...
contract BasicTime {
    uint constant DAY_IN_SECONDS = 60 * 60 * 24;
...
contract MainBonus is BasicTime,BasicAuth,MainBase,MainCard
{
    ...
    function GenDayRatio(uint dayNo) internal view returns(uint dr)
    {
        DayRatio storage DR = g_Bonus.m_DayRatio[dayNo];
        dr += DR.m_Permanent;
        dr += DR.m_Static;
        dr += GetDynamicRatio(dayNo);
    }
...
contract MainBag is BasicTime,BasicAuth,MainChip,MainCard
{
    ...
    function GainTempStuff(address acc, uint32 iStuff, uint dayCnt) external AuthAble OwnerAble(acc)
    {
        ...
        obj.m_TempStuff.set(iStuff,now+dayCnt*DAY_IN_SECONDS);
    }
...
contract Child is Base {
    ...
    {
        address(main).transfer(msg.value);
        main.AddBonus(msg.value * percent / 100);
    }
...
{
    uint constant BONUS_PERCENT_PURCHASE = 60;
    ...
    constructor(Main _main, StoreGoods goods, StoreGifts gifts) public Child(_main)
    {
        g_Goods = goods;
        g_Gifts = gifts;
    ...
    function GiveChipGitf() internal
    {
        for (uint8 level=HEROLEVEL_MIN; level<=HEROLEVEL_MAX; level++)
        {
            ...
            uint32 iChip = main.GenChipByRandomWeight(random, level, GenExtWeightList(level));
            main.GainChip(msg.sender, iChip, true);
        }
    }
    ...
    function BuyGoods(uint32 iGoods) external payable
    {
        ...
        if (iCostItem == 0) {
            ...
            // cost other stuff
            main.CostStuff(msg.sender,iCostItem,iCostNum);
        }
        ...
        if (iItemRef == CHIPGIFT_ITEMINDEX) {
            ...
            if (iDuration == 0) {
                main.GainStuff(msg.sender, iItemRef, iAmount);
            }
            else {
                main.GainTempStuff(msg.sender, iItemRef, iDuration);
            }
        }
    }
    ...
    function ExchangeGift(string key) external
    {
        ...
        {
            main.GainStuff(msg.sender, idxList[i], numList[i]);
        }
    }
