...
contract PublicSale is Ownable {
    ...
    constructor(address payable _paymentReceiver, IERC20 _mega, IERC20 _USDT, IERC20 _USDC, IERC20 _dai) public {
       require(_paymentReceiver != address(0x00), "Zero address");
    ...
    function calculateReturns(uint256 _value, address _token) public ifNotPaused view returns(uint){
        require(_value.div((uint256(IERC20(_token).decimals()))) > 0, "Less than min payment" );
    ...
    function withdrawUnsoldTokens() external onlyOwner {
        uint256 unsold = MEGA.balanceOf(address(this));
        MEGA.transfer(paymentReceiver, unsold);
    }

    function withdrawstuckTokens(address _token) external onlyOwner {
        if(_token != address(0x000)){
            IERC20(_token).transfer(paymentReceiver, IERC20(_token).balanceOf(address(this)));
        }else {
