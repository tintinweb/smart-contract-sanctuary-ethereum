...
contract NFTMarketplace is ERC721Enumerable, Ownable {
    ...
    uint256 mintingPrice = 0.01 ether;
    ...
    constructor() ERC721("WalterTest4", "WalterTest4") {
    }
    ...
    function updateListingPrice(uint _listingPrice) public onlyOwner payable {
      listingPrice = _listingPrice;
    }

    function updateBaseURI(string memory uri) public onlyOwner payable {
      _baseTokenURI = uri;
    }

    function updateMintingPrice(uint _mintingPrice) public onlyOwner payable {
      mintingPrice = _mintingPrice;
    }

    function updateMaxSupply(uint _maxSupply) public onlyOwner payable {
    ...
    function mintToken() public payable returns (uint) {
      require(_tokenIds.current() <= maxSupply, "enough minting");
      require(msg.value == mintingPrice, "Price must be equal to listing price");
    ...
    function createMarketListing(uint256 price, uint256 tokenId) public payable {
      ...
      require(ownerOf(tokenId) == payable(msg.sender), "Only item owner can perform this operation");
