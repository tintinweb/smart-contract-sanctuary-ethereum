...
contract ApolloThree is Context, IERC20, Ownable {
    ...
    string constant private _name = "Apollo Three";
    string constant private _symbol = "$APT";
    ...
    address payable public APPLOFUNDAddress = payable(0xF6a88B388D253dF0e3C1223A055a7682F8c680aA);
    address payable public MYBAGSAddress = payable( 0x83c6f78DcF957aAddEFD5F8F4EA938dF7038C209);
    address public  deadAddress = 0x000000000000000000000000000000000000dEaD;
    address private wallet1 = 0xF6a88B388D253dF0e3C1223A055a7682F8c680aA;
    address private wallet2 = 0xF6a88B388D253dF0e3C1223A055a7682F8c680aA;
    address private wallet3 = 0xF6a88B388D253dF0e3C1223A055a7682F8c680aA;
    ...
    uint256 public _APPLOFUNDFee= 7;
    uint256 private _previousAPPLOFUNDFee = _APPLOFUNDFee;
    uint256 public _MYBAGS= 0;
    uint256 private _previousMYBAGSFee = _MYBAGS;
	
	
	    uint256 public _SELLtaxFee =5 ;
    uint256 private _previousSELLTaxFee = _SELLtaxFee;
	
	 uint256 public _SELLMYBAGS= 1;
    uint256 private _previousSELLMYBAGSFee = _SELLMYBAGS;
    ...
    constructor() {
        ...
        _isExcludedFromFee[APPLOFUNDAddress] = true;
        _isExcludedFromFee[MYBAGSAddress] = true;
        ...
        _isExcludedFromMaxTxLimit[APPLOFUNDAddress] = true;
        _isExcludedFromMaxTxLimit[MYBAGSAddress] = true;
    ...
    function totalFees() internal view returns (uint256) {
        return _taxFee.add(_liquidityFee).add(_burnFee).add(_APPLOFUNDFee).add(_MYBAGS);
    }
    ...
    function setAPPLOFUNDPercent(uint256 APPLOFUNDFee) external onlyOwner() {
        _APPLOFUNDFee = APPLOFUNDFee;
    }

    function setMYBAGSPercent(uint256 MYBAGS) external onlyOwner() {
        _MYBAGS = MYBAGS;
    }
    ...
    function setAPPLOFUNDWallet(address wallet) external onlyOwner()
    {
        APPLOFUNDAddress = payable(wallet);
    }

    function setMYBAGSWallet(address wallet) external onlyOwner()
    {
        MYBAGSAddress = payable(wallet);
    }
    ...
    function calculateAPPLOFUNDFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_APPLOFUNDFee).div(
            10**2
        );
    }

     function calculateMYBAGS(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_MYBAGS).div(
            10**2
        );
    }
    ...
    function _takeAPPLOFUND(address sender, uint256 tAPPLOFUND) private returns(uint256){
      uint256 rAPPLOFUND = calculateAPPLOFUNDFee(tAPPLOFUND);
      _rOwned[address(this)] = _rOwned[address(this)].add(rAPPLOFUND);
      _rOwned[sender] = _rOwned[sender].sub(rAPPLOFUND);
      emit Transfer(sender, address(this), rAPPLOFUND);
         if(_isExcludedFromReward[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tAPPLOFUND);
      return rAPPLOFUND;
    }

    function _takeMYBAGS(address sender, uint256 tMYBAGS) private returns(uint256){
      uint256 rMYBAGS = calculateMYBAGS(tMYBAGS);
      _rOwned[address(this)] = _rOwned[address(this)].add(rMYBAGS);
      _rOwned[sender] = _rOwned[sender].sub(rMYBAGS);
      emit Transfer(sender, address(this), rMYBAGS);
         if(_isExcludedFromReward[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tMYBAGS);
      return rMYBAGS;
    }
    ...
    function _transfer(
     ...
     //indicates if fee should be deducted from transfer
        ...
        if(takeFee == 1)
        {
        uint256 APPLOFUNDAmount = _takeAPPLOFUND(from, amount);
        uint256 MYBAGSAmount = _takeMYBAGS(from, amount);
        amount = amount - (APPLOFUNDAmount+MYBAGSAmount);}
    ...
    function swapAndLiquify(uint256 tokensToLiquify) private lockTheSwap {
        ...
        uint256 ethForAPPLOFUND = ethBalance.mul(_APPLOFUNDFee).div(ethFeeFactor);
        uint256 ethForFounders = ethBalance.mul(_MYBAGS).div(ethFeeFactor);
     
        addLiquidity(tokensToLP, ethForLiquidity);

        payable(APPLOFUNDAddress).transfer(ethForAPPLOFUND);
        payable(MYBAGSAddress).transfer(ethForFounders);
