...
// Contract
contract Shape is ERC721, ERC721URIStorage, Ownable { //0xf74281336E6F756e20084Ba960329ECDDcC362dD
    using Counters for Counters.Counter;
    using SafeMath for uint256;

    mapping (uint => Attributes) public _attributes;
    ...
    function mint(address to, string memory _shape, string memory _color, string memory _size ) public onlyOwner returns (uint) {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _attributes[tokenId] = Attributes(_shape, _color, _size);
        return tokenId;
    }
    ...
    {
        // Code
        string memory _name = string(abi.encodePacked("NFT_Shape_", Strings.toString(tokenId)));
        string memory metadata = Base64.encode(
            bytes(string(
                abi.encodePacked(

                    '{"name": "', _name , '" ,',
                    '"image_data": "',drawSVG(_attributes[tokenId].shape,_attributes[tokenId].color, _attributes[tokenId].size), '" ,',
                    '}'
                )
            ))
        );
