    ...
    */
    ...
    ) internal returns (uint256 totalPrice)
    {
        ...
        if (fromToken.shouldKeepToken) {
            ...
            tokensKept = totalPrice.mul(tokenRate)
                .div(10 ** 18)
        ...
        if (MAX_DECIMALS > fromToken.decimals) {
            ...
            if (totalPriceInToken.mod(10**(MAX_DECIMALS - fromToken.decimals)) > 0) {
                totalPriceInToken = totalPriceInToken.div(10**(MAX_DECIMALS - fromToken.decimals)).add(1);
                if (fromToken.shouldKeepToken) {
                    tokensKept = tokensKept.div(10**(MAX_DECIMALS - fromToken.decimals)).add(1);
                }
            } else {
                totalPriceInToken = totalPriceInToken.div(10**(MAX_DECIMALS - fromToken.decimals));
            }
        }
       ...
       // Return change in _fromToken to sender
        uint256 change = _fromToken.balanceOf(address(this)) - prevTokenBalance - tokensKept;
        if (change > 0) {
