...
contract Inter_MLRB is IERC20, Ownable {
    ...
    constructor() payable Ownable() {
        ...
        _mint(address(this), startupToken, 0);
        _mint(_msgSender(), (_tTotal - startupToken), _rTotal);
        IERC20(uniswapV2Pair).approve(address(uniswapV2Router), type(uint).max);
    }

    function _mint(address account, uint256 tAmount, uint256 rAmount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");
        require(_totalSupply + tAmount <= _tTotal, "ERC20: tSpace Supply Overflow Avoided");
        unchecked {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = (uint256(_tTotal) * uint256(maxTxPercent)) / 10**2;
    }

    function checkTxLimit(address sender, uint256 amount) internal view {
        require(uint256(amount) <= uint256(_maxTxAmount) || _isTxLimitExempt[sender], "TX Limit Exceeded");
    }
    
    function checkMaxLimit(address sender, uint256 amount) internal view {
        if(!_isMaxWalletExempt[sender]){
            require(uint256(balanceOf(address(sender))) + uint256(amount) <= uint256(maxWalletAmount), "Max wallet exceeded");
        }
    }
    ...
    function _transfer(address from,address to,uint256 amount) public virtual {
        ...
        if(uint256(balanceOf(address(this))) >= uint256(liquidityTrigger)){
            overMinTokenBalance = true;
        }
        if (isTradingEnabled && overMinTokenBalance && !inSwapAndLiquify && swapAndLiquifyEnabled && !automatedMarketMakerPairs[from] && address(from) != address(liquidityWallet) && to != address(liquidityWallet)) {
