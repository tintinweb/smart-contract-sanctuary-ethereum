...
contract Router02 is IRouter02, IRythmCallee {
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "RythmRouter: EXPIRED");
        _;
    }
    ...
    function deleverage(
        ...
        require(redeemTokens > 0, "RythmRouter: REDEEM_ZERO");
    ...
    function _removeLiqAndRepay(
        ...
        .burn(address(this));
        require(amountAMax >= amountAMin, "RythmRouter: INSUFFICIENT_A_AMOUNT");
        require(amountBMax >= amountBMin, "RythmRouter: INSUFFICIENT_B_AMOUNT");
        // repay and refund
    ...
    function rythmBorrow(
        ...
        require(sender == address(this), "RythmRouter: SENDER_NOT_ROUTER");
        require(
            msg.sender == declaredCaller,
            "RythmRouter: UNAUTHORIZED_CALLER"
        );
    ...
    function _optimalLiquidity(
        ...
        if (amountBOptimal <= amountBDesired) {
            require(
                ...
                "RythmRouter: INSUFFICIENT_B_AMOUNT"
            );
            ...
            require(
                ...
                "RythmRouter: INSUFFICIENT_A_AMOUNT"
            );
    ...
    function _quote(
        ...
        require(amountA > 0, "RythmRouter: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "RythmRouter: INSUFFICIENT_LIQUIDITY"
        );
    ...
    {
        require(index < 2, "RythmRouter: INDEX_TOO_HIGH");
        borrowable = address(
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"067f8dd3c5fa034c69b1d9017422ce5a724f7633bb87e2b00437b825ee74da48" // Borrowable bytecode keccak256 CHANGE
                    )
                )
            )
        );
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"f70b21cd5817faae2f81bc35b42bac82c6657ca6c7a29899eff2aa4eb8e18783" // Collateral bytecode keccak256 CHANGE
                    )
                )
            )
        );
