...
contract SecretVerseNFTDistributor is AccessControl {
    ...
    function mint(uint256 _num) public payable returns (bool) {
        ...
        require(
            !whitelistOnly,
            "SecretVerseNftDistributor: Use whitelistMint function"
        );
        require(!_mintingPaused, "Minting paused");
        isCapReached(_num);
        require(
        ...
        isAmountSufficient(msg.value, _num);
        _mint(_num);
        return true;
    }

    function whitelistMint(
        // when whitelist in ON
        address wallet,
        uint256 _mintCount,
        ...
        require(whitelistOnly, "WhiteList is closed now");
        require(!_mintingPaused, "Minting paused");
        isCapReached(_mintCount);
        ...
        // should be whitelisted or should HOLD one of the PRE-REQ NFTS
        require(
            hasRole(DEFAULT_ADMIN_ROLE, signerOwner) || doesHoldNfts(),
            "SecretVerseNftDistributor: Not authorized to mint"
        );
        require(
            totalMintsPerAddress[msg.sender] + _mintCount <= 10,
            "SecretVerseNftDistributor: You can mint a maximum of 10 when whitelist enabled"
        );

        isAmountSufficient(msg.value, _mintCount);
        _mint(_mintCount);
        return true;
    }

    function _mint(uint256 _mintCount) internal {
        totalMintsPerAddress[msg.sender] += _mintCount; //log the total mints per address
        usersMintCount += _mintCount;
        for (uint256 i; i < _mintCount; i++) {
            cbToken.mintNextToken(msg.sender); //, tokenId + i + 1
        }
    }

    function isAmountSufficient(uint256 _amount, uint256 _tokensCount)
        internal
        view
    {
        require(
            _amount >= (tokenPrice * _tokensCount),
            "SecretVerseNftDistributor: Insufficient amount provided"
        );
    }

    function isCapReached(uint256 _tokensCount) internal view {
        require(
            usersMintCount + _tokensCount <= 9650,
            "SecretVerseNftDistributor: Total Cap reached"
        );
    }
