...
contract CrowdFunding {
    ...
    function launch(uint _goal, uint32 _startAt, uint32 _endAt) external {
        require(_startAt >= block.timestamp, "start cannot be in past");
        require(_endAt > _startAt, "end cannot be before start");
        require(_endAt <= block.timestamp + 90 days, "end cannot be greater than 90 days");
