...
interface DSValue {
    function price(address baseimple) external view returns (uint64);
}
...
// StablePrice sets a price in USD, based on an oracle.
contract StablePrice is Ownable, PriceOracle {
    using SafeMath for *;
    using StringUtils for *;

    DSValue pricecontrol;
    ...
    address public baseimple;

    constructor(uint[] memory _rentPrices,DSValue _pricecontrol,address _baseimple) public {
        setPrices(_rentPrices);
        pricecontrol = _pricecontrol;
        baseimple = _baseimple;
    }
