...
contract DDAOTeamClaim is AccessControl
{
    ...
    uint256 constant public MAX_AMOUNT = 1680000;
    uint48  constant public FINAL_DATE = START_DATE + 24 * 86400*30;
...
//	    if(block.chainid == 3 || block.chainid == 80001)
	    {
...
//	uint256 z;
    ...
    function RewardCalc(uint8 id,uint8 nn)public view returns(ret memory b)
    {
//	ret b;
	uint256 x;
	uint256 y;
...
//	    b.x3 = x;
	    b.amount = r * x / 10**36;
//	    b.a = a;
	    //emit eRecalc(block.timestamp,block.number,addr,i,x,y,a);
	}
	}
    }
    event eRecalc(uint256 time,uint256 block,address addr,uint8 group,uint256 part_group,uint256 part_member,uint256 amount);
    function RewardAllNow()public view returns(uint256 s)
    {
	uint48 t;
	uint48 d;

	t = uint48(block.timestamp);
	if(t > FINAL_DATE) t = FINAL_DATE;

	d = t-UpdateTime;
	s = MAX_AMOUNT * 10**18;
	s = s * d / (FINAL_DATE - START_DATE);

    }
/*
    function RewardNowGroup(uint8 id)public view returns(uint256)
    {
	uint8 x;
	s = RewardNow();
	x = GroupMaxVal(1) + GroupMaxVal(2) + GroupMaxVal(3) + GroupMaxVal(4);
...
*/
	...
	Personal[id][addr].payed = Personal[id][addr].payed.sub(amount_to_send);
//	IToken(AddrDDAO).safeTransferFrom(address(this),addr,amount_to_send);
//	IToken(AddrDDAO).transfer(addr,amount_to_send);
	...
	//IToken(AddrDDAO).safeTransferFrom(_msgSender(),address(this),balance);
    }
