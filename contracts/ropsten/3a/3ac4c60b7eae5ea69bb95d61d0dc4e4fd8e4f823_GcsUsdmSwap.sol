...
contract GcsUsdmSwap is ReentrancyGuard{
    address payable admin;
    //ratioAX is the percentage of how much TokenA is worth of TokenX
    uint256 ratioUSDMGCS; //ration AX
    bool USDMcheaperthanGCS = true; //A Cheaper than X
    uint256 fees;
    IERC20 public USDM; //tokenABC
    IERC20 public GCS; //tokenXYZ

    constructor(address _USDM, address _GCS) {
    ...
    function setUSDM(IERC20 _USDM) public onlyAdmin {
        USDM = IERC20(_USDM);
    }

    function setGCS(IERC20 _GCS) public onlyAdmin {
        GCS = IERC20(_GCS);
    }
    ...
    function swapUSDMTOGCS(uint256 amountUSDM, uint256 _ratioUSDMGCS) public nonReentrant returns (uint256) {
        ratioUSDMGCS = _ratioUSDMGCS;
        //check if amount given is not 0
        // check if current contract has the necessary amout of Tokens to exchange
        require(amountUSDM > 0, "amountUSDM must be greater then zero");
        require(
            USDM.balanceOf(msg.sender) >= amountUSDM,
            "sender doesn't have enough Tokens"
        );

        uint256 exchangeA = uint256(mul(amountUSDM, ratioUSDMGCS));
        ...
        USDM.transferFrom(msg.sender, address(this), amountUSDM);
    ...
    function swapGCSTOUSDM(uint256 amountGCS, uint256 _ratioUSDMGCS) public nonReentrant returns (uint256) {
        ratioUSDMGCS = _ratioUSDMGCS;
        //check if amount given is not 0
        // check if current contract has the necessary amout of Tokens to exchange and the sender
        require(amountGCS >= ratioUSDMGCS, "amountGCS must be greater then ratio");
        require(
            GCS.balanceOf(msg.sender) >= amountGCS,
            "sender doesn't have enough Tokens"
        );

        uint256 exchangeA = amountGCS/ ratioUSDMGCS;
        ...
        GCS.transferFrom(msg.sender, address(this), amountGCS);
    ...
    // buy automatically tokens since contracts are immutable and in case the value of some tokens becomes
    // worthless its better to not to do any exchange at all
    // function buyTokensUSDM(uint256 amount) public payable onlyAdmin {
    //     USDM.buyTokens{value: msg.value}(amount);
    // }

    // function buyTokensGCS(uint256 amount) public payable onlyAdmin {
    //     GCS.buyTokens{value: msg.value}(amount);
    // }
    ...
    function recoverLostETH() public onlyAdmin {
        payable(admin).transfer (address(this).balance);
    }

    function WithdrawOtherTokens(address _token, uint256 amount) public onlyAdmin {
