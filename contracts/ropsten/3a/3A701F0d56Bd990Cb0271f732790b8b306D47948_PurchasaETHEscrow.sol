...
contract PurchasaETHEscrow is Ownable {
    ...
    function lockETHToken(string memory orderId,Orders[] memory orders, address reciever,string memory currency,string memory userId,uint256 damount,uint256 paymentfee) payable public {
        ...
        uint totalc = ((((orders[i].amount * orders[i].quantity) / 100) * (categoryList[orders[i].categoryName].ethcommission + categoryList[orders[i].categoryName].commission)));
        newOrder.items.push(Items(currentTime,currentTime + categoryList[orders[i].categoryName].escrowtime,totalc,totalc,false,orders[i].amount * orders[i].quantity,orders[i].serveId,orders[i].quantity,orders[i].categoryName,(orders[i].amount * orders[i].quantity),categoryList[orders[i].categoryName]));
        }
    }
    ...
    function lockERCToken(uint256 paymentfee,uint256 damount,string memory orderId,Orders[] memory orders, address tokenAddress,uint256 amount, address reciever,string memory currency,string memory userId) public {
        ...
        for(uint256 i = 0; i < lengthO; i++) {
        require(keccak256(abi.encodePacked(categoryList[orders[i].categoryName].name)) == keccak256(abi.encodePacked(orders[i].categoryName)), "Category not exists");
        uint totalc = ((((orders[i].amount * orders[i].quantity) / 100) * (tokens[tokenAddress].commission + categoryList[orders[i].categoryName].commission)));
        newOrder.items.push(Items(currentTime,currentTime + categoryList[orders[i].categoryName].escrowtime,totalc,totalc,false,orders[i].amount * orders[i].quantity,orders[i].serveId,orders[i].quantity,orders[i].categoryName,orders[i].amount,categoryList[orders[i].categoryName]));
        }
    }
    ...
    function withdrawalERCCommission(address tokenAddress,string memory currency) public onlyOwner {
        ...
        for(uint i=0; i < userOrders.length; i++){
            if(keccak256(abi.encodePacked(userOrders[i].currency)) == keccak256(abi.encodePacked(currency)) && userOrders[i].isDispute == false) {
               totalAmount = totalAmount.add(userOrders[i].damount);
               totalAmount = totalAmount.add(userOrders[i].paymentfee);
               for(uint j=0; j < userOrders[i].items.length; j++) {
                   if(currentTime >= userOrders[i].items[j].end && userOrders[i].items[j].isDispute == false) {
        ...
        tokens[tokenAddress].tokenAddress.transfer(msg.sender,  totalAmount);
         for(uint i=0; i < userOrders.length; i++){
            if(keccak256(abi.encodePacked(userOrders[i].currency)) == keccak256(abi.encodePacked(currency)) && userOrders[i].isDispute == false) {
                userOrders[i].damount = 0;
                userOrders[i].paymentfee = 0;
               for(uint j=0; j < userOrders[i].items.length; j++) {
                   if(currentTime >= userOrders[i].items[j].end && userOrders[i].items[j].isDispute == false) {
    ...
    function getUserBalance(address merchantAddress,string memory currency) public view returns (uint256) {
        uint256 totalAmount = 0;
        for(uint i=0; i < userOrders.length; i++){
    ...
    function getCommissionBalance(string memory currency) public view onlyOwner returns (uint256) {
        uint256 totalAmount = 0;
        uint256 currentTime = block.timestamp;
        for(uint i=0; i < userOrders.length; i++){
            if(keccak256(abi.encodePacked(userOrders[i].currency)) == keccak256(abi.encodePacked(currency)) && userOrders[i].isDispute == false) {
