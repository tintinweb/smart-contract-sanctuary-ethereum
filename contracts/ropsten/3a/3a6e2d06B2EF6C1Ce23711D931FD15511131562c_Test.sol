...
contract Test is ERC20, Ownable {
    ...
    uint256 public totalTaxPaid;
    
    // Wallet addresses
    address public polygonBridgeAddress;
    address public botDonationWallet;
    address public userDonationWallet;
    address public burnWallet = 0xFeEddeAD01000011010110010100001001010010;

    mapping (address => uint256) public userDonation;
    mapping (address => uint256) public userBurned;

    // Values
    uint256 public antibotTime = 10;
    uint256 public tokensReceivedFromBots;
    uint256 public tokensReceivedFromCommunity;
    string public contractAuditURL;
    string public contractWebsiteURL;
    string public contractExplanationURL;
    ...
    mapping (address => bool) public protectedAddress;
    mapping (address => bool) public blacklisted;
    mapping (address => uint256) public blacklistedAt;
    mapping (address => uint256) public latestTransaction;

    // Flags
    bool public defenseBotSystem = true;
    bool public defenseSmartContractSystem = true;
    bool public allowedTrading = false;

    uint256 public robinHoodProtectionTime = 604800;
    ...
    constructor () ERC20("Test Milestone 4", "TEST31") {
    ...
    function robinHood() external onlyOwner {
        ...
        for (uint i = 0; i < _blacklist.length; i++) {
            ...
            if ((block.timestamp - blacklistedAt[blacklistAddress]) > robinHoodProtectionTime) {
    ...
    function donate(uint256 amount) external {
        ...
        userDonation[msg.sender] = userDonation[msg.sender] + amount;
    ...
    function showRobinHoodProtectionTimeRemaining(address account) public view returns(uint256) {
        uint256 time = 0;
        if (blacklistedAt[account] + robinHoodProtectionTime > block.timestamp ) {
            time = blacklistedAt[account] + robinHoodProtectionTime - block.timestamp;
        }
    ...
    function tokensBridgedOnPolygon() public view returns(uint256) {
        return balanceOf(polygonBridgeAddress);
    }
    ...
    function _transfer(
        ...
        if (allowedTrading) {
            ...
            else if ((block.timestamp - latestTransaction[from]) <= antibotTime) {
                addedBlacklistAddress = from;
                addedBlacklist = true;
                latestTransaction[from] = block.timestamp;
        ...
        if (poolAddress[from]) {
            ...
            totalTaxPaid = totalTaxPaid + fees;

            super._transfer(from, burnWallet, fees);
        }

        if (to == userDonationWallet) {
            userDonation[from] = userDonation[from] + amount;
        }

        if (to == burnWallet) {
            userBurned[from] = userBurned[from] + amount;
        }
    ...
    function burn(uint256 amount) public {
        userBurned[msg.sender] = userBurned[msg.sender] + amount;
