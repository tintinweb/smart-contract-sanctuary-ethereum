...
// We will set the baseTokenURI in our constructor call.
...
// we need to tell our contract that the baseTokenURI variable that we defined is the base
// URI that the contract must use. To do this, we override an empty function called _baseURI()
...
// We also write an only owner function that allows us to change the
// baseTokenURI even after the contract has been deployed.
function setBaseURI(string memory _baseTokenURI) public onlyOwner {
     baseTokenURI = _baseTokenURI;
}
// Our users and customers will call this function(mintNFTs) when they want to purchase
// and mint NFTs from our collection.
...
// mintSingleNFT() function that’s being called whenever we (or a third party) want to mint an NFT.
// --> We get the current ID that hasn’t been minted yet.
// --> We use the _safeMint() function already defined by OpenZeppelin to assign the NFT ID to the account
