 ...
 */
contract CamelCoinV4 is Context, IERC20, IERC20Metadata, Pausable {
    ...
    uint256 constant masterTaxDivisor = 10_000;
    ...
    function _transfer(
        ...
        if (to == liquidityProcessor.uniswapPair() && !inSwapAndLiquify) {
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        if (_isExcludedFee[from] || _isExcludedFee[to]) {
            return amount;
        } else if (from == liquidityProcessor.uniswapPair() && !inSwapAndLiquify) {
            currentFee = _taxRates.buyFee;
        } else if (to == liquidityProcessor.uniswapPair() && !inSwapAndLiquify) {
