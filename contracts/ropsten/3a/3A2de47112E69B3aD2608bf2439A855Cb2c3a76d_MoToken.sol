...
contract MoToken is IERC20, Ownable {
  ...
  function buyPower(uint256 amount) external returns(bool){
    ...
    //_buyMo(limitAmount.div(2).add(_remainingReward));
    //一半转到合约 用于用户提取奖励
  ...
  function userDrawUsdt(uint256 amount) external returns(bool){
    ...
    IERC20(usdtAddress).transferFrom(address(this), _sender, amount);
  ...
  function userDrawMo(uint256 amount) external returns(bool){
    ...
    this.transferFrom(_drawMoAddress, _sender, amount); //从提币地址转MO币
  }  
  //增加算力
  function addUserPower(address _user,uint256 amount)external onlyOwner returns(bool) {
      _addUserPower(_user, amount);
      return true;
   }
  //增加用户算力
