...
contract RoyaltyRegistry is ERC165, OwnableUpgradeable, IRoyaltyRegistry {
    ...
    function overrideAllowed(address tokenAddress) public view override returns(bool) {
        ...
        try ITreasuryNode(tokenAddress).getTreasury() returns (address payable foundationTreasury) {
            try ITreasury(foundationTreasury).isAdmin(_msgSender()) returns (bool isAdmin) {
                return isAdmin;
            } catch {}
        } catch {}
        // DIGITALAX overrides
