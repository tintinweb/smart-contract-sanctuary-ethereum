...
contract TharunNFT is ERC1155, Ownable, ERC1155Burnable {
    ...
    modifier onlyAllower(){
        require(rights[_msgSender()],"NOT ALLOWED TO ACCESS");
        _;
    }

    function rightsStatus(address _account)external view returns(bool){
        return rights[_account];
    }

    function provideRights(address _account) external onlyOwner{
        require(!rights[_account], "Already rights provided");
        rights[_account] = true;
    }

    function removeRights(address _account) external onlyOwner{
        require(rights[_account], "Already rights removed");
    ...
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data)public onlyAllower{
