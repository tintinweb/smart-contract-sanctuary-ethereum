...
contract Referral is IReferral {
     ...
     */
    function hasReferrer(address addr) external override view returns (bool){
        return accounts[addr].referrer != address(0);
    }

    function getAccount(address addr) public view returns (Account memory) {
        return accounts[addr];
    }

    function getReferrer(address addr) external override view returns (address) {
        return accounts[addr].referrer;
    }

    function getTotalRefferedCount(address referrer) public view returns (uint256) {
     ...
     */
    function addReferrer(address payable referrer) external override returns (bool) {
