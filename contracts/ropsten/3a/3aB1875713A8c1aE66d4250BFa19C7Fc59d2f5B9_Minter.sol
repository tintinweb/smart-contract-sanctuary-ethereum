...
contract Minter is AccessControlEnumerableUpgradeable, Provenance, PaymentSplitterUpgradeable, IMintable {
    ...
    function setTokenMintPassType(string memory tokenMintPassTypeStr) public onlyAdmin {
        tokenMintPassType = tokenMintPassTypeStr;
    }

    function setTokenMintPassActive(bool status) public onlyAdmin {
        tokenMintPassActive = status;
    }
    ...
    {
        require(minterActive, "minter is not active");
        require(tokenMintPassActive, "mint pass via prior tokens not active");
    ...
    function hashMarketTransaction(
        address sender,
        uint256 numberOfTokens,
        bytes32 nonce,
        address receiver,
        ...
        returns(bytes32)
    {
        ...
        return ECDSAUpgradeable.toEthSignedMessageHash(
            keccak256(abi.encode(chainId, address(this), sender, numberOfTokens, nonce, receiver, price, tokenPayment))
        );
    }

    /* ------------------------------ Market Place ----------------------------- */

    function signedMarketPurchase(
        uint256 numberOfTokens,
        uint256 maxPermitted,
        bytes memory signature,
        bytes32 nonce,
        ...
        payable
    {
        require(marketTaxTimesTen > 0, "Marketplace is not live yet");
        require(numberOfTokens <= maxPermitted, "Minter: numberOfTokens exceeds maxPermitted");

        bool signatureIsValid = SignatureCheckerUpgradeable.isValidSignatureNow(
            mintSigner,
            hashMarketTransaction(msg.sender, maxPermitted, nonce, receiver, price, tokenPayment),
            signature
        );
        require(signatureIsValid, "Minter: invalid signature");
        require(!nonces[nonce], "Minter: nonce already used");
        ...
        if (tokenPayment == address(0)) {
            require(msg.value == price, "Minter: Sent ether value is incorrect");
            payable(receiver).call{value: (msg.value)*(100 - (marketTaxTimesTen/10))/(100)}("");
        } else {
            IERC20Upgradeable(tokenPayment).safeTransferFrom(msg.sender, address(this), price*(marketTaxTimesTen/10)/100);
            IERC20Upgradeable(tokenPayment).safeTransferFrom(msg.sender, receiver, price*(100 - (marketTaxTimesTen/10))/100);
        }
