...
contract userwallet{
    ...
    /* userwallet events */
    event new_allocation(address mainwallet);
    event get_transaction(address client, uint amount, bool status);
    event ongoing_transaction(address client, uint amount, bool status);

    constructor(address payable mainwallet){
        _titan = titan(mainwallet);
        emit new_allocation(mainwallet);
    }
    ...
    function consume() internal{
        ...
        require(sent, "fail");
        emit get_transaction(msg.sender, msg.value, sent);
    }
    ...
    function sendback(uint amount) external{
        ...
        emit ongoing_transaction(msg.sender, amount, true);
