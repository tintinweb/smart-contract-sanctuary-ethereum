...
contract ERC721Store is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, Royalties, UUPSUpgradeable {
  ...
  function __ERC721Store_init(address defaultOperator_) internal onlyInitializing {
    __ERC721Store_init_unchained(defaultOperator_);
  }

  function __ERC721Store_init_unchained(address defaultOperator_) internal onlyInitializing {
  ...
  function initialize() initializer public {
    ...
    __Ownable_init();
    __Royalties_init(1000);
    __ERC721Store_init(0xFc91bd78227E756c9BfB6586F2A58F2011CC6f83);
  ...
  // * 기본 오퍼레이터 지정
  function setDefaultOperator(address operator) virtual public onlyOwner {
  ...
  // * create
  // 로열티와 오퍼레이터 등록
  function create(address to, string memory uri, Royalty[] memory creators) virtual public {
    require(defaultOperator != address(0), "ERC721Store: default operator is zero address");
    uint256 tokenId = _tokenIdCounter.current();
    _tokenIdCounter.increment();
    _setRoyalties(tokenId, creators);
    _safeMint(to, tokenId);
    _setTokenURI(tokenId, uri);
    if (!isApprovedForAll(_msgSender(), defaultOperator)) {
      setApprovalForAll(defaultOperator, true);
    }
  }

  // * safeMint
  // 로열티와 오퍼레이터 등록 없이 민팅
  function safeMint(address to, string memory uri) virtual public {
    uint256 tokenId = _tokenIdCounter.current();
    _tokenIdCounter.increment();
    _safeMint(to, tokenId);
    _setTokenURI(tokenId, uri);
