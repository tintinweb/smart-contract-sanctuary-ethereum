...
contract MegafiAggregator is Ownable{
    ...
    constructor( address _router, address _megaFi) public {
        router = IMegafiswapV2Router02(_router);
        megaRouter = IMegafiswapV2Router02(_megaFi);

        factory = IMegafiswapV2Factory(IMegafiswapV2Router02(_router).factory());
        megaFactory = IMegafiswapV2Factory(IMegafiswapV2Router02(_megaFi).factory());

        weth = IMegafiswapV2Router02(_megaFi).WETH() ;
    }
    ...
    function _approve(address _token, address _receiver, uint amount) private {
        TransferHelper.safeApprove(_token, _receiver, amount);
    }
    ...
    function getPair(address token0, address token1) external view returns(address pair){
        if(IMegafiswapV2Factory(factory).getPair(token0, token1) != address(0x000)){
