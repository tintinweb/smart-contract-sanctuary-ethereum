...
contract asset {
    ...
    function addassetInspector(address _addr,string memory _name, uint _age, string memory _designation,string memory _location) public returns(bool){
        if(contractOwner!=msg.sender)
            return false;
        require(contractOwner==msg.sender);
        RegisteredInspectorMapping[_addr]=true;
        allassetInspectorList[1].push(_addr);
        InspectorMapping[_addr] = assetInspector(inspectorsCount,_addr,_name, _age, _designation,_location);
        return true;
    }
