...
contract MessierWrapper {
  ...
  function deposit(IMessierV2dot2 _messier, bytes32 _commitment, bool _buyM87) external payable {
    require(whitelisted[address(_messier)], "not whitelisted");
  ...
  function whitelist(IMessierV2dot2 _messier) public {
    address MessierAddr = address(_messier);
    require(!whitelisted[MessierAddr], "already whitelisted");
