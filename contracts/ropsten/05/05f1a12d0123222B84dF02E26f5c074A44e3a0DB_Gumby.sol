...
contract Gumby is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private _totalSupply = 777 * 1e11 * 1e18; 
    IUSV2Router02 private usV2Router;
    address public usV2Pair;

    bool private inTheParty; //swap
    bool private misa = true; 

    address public gumbyMktg;
    ...
    uint256 public buyGumbyMktgFee;
    uint256 public buyLiqFee;
    
    uint256 public sellTotalFees;
    uint256 public sellGumbyMktgFee;
    uint256 public sellLiqFee;
    
    uint256 private tokensForBank;
    uint256 private tokensForLiq;

    uint256 _buyGumbyMktgFee = 6;
    uint256 _buyLiqFee = 6;

    uint256 _sellGumbyMktgFee = 10;
    uint256 _sellLiqFee = 6;
    ...
    event gumbyMktgUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("Gumby", "GUMBY") {
        ...
        maxTx = 777 * 1e9 * 1e18; // 1% of supply max transaction allowed at launch
        maxWallet = 777 * 1e9 * 1e18; // 1% of supply max wallet
        tokenToEth = 777 * 1e8 * 1e18; // .1

        buyGumbyMktgFee = _buyGumbyMktgFee;
        buyLiqFee = _buyLiqFee;
        buyTotalFees = buyGumbyMktgFee + buyLiqFee;
        
        sellGumbyMktgFee = _sellGumbyMktgFee;
        sellLiqFee = _sellLiqFee;
        sellTotalFees = sellGumbyMktgFee + sellLiqFee;
        
        //update this or you are going to donate all your fees to 0xDead
        gumbyMktg = payable(0xb086295639178487623484E9FB8A8Ca888cfEE0D); 
        liqOwner = payable(0xb086295639178487623484E9FB8A8Ca888cfEE0D);
        
        // exclude from paying fees or having max transaction amount
        excludeFromFees(owner(), true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(gumbyMktg), true);
        excludeFromFees(address(liqOwner), true);
        
        excludeFromMaxTx(owner(), true);
        excludeFromMaxTx(address(this), true);
        excludeFromMaxTx(address(gumbyMktg), true);
    ...
    function updateBuyFees(uint256 _gumbyMktgFee, uint256 _liqFee) external onlyOwner {
        buyGumbyMktgFee = _gumbyMktgFee;
        buyLiqFee = _liqFee;
        buyTotalFees = buyGumbyMktgFee + buyLiqFee;
        require(buyTotalFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _gumbyMktgFee, uint256 _liqFee) external onlyOwner {
        sellGumbyMktgFee = _gumbyMktgFee;
        sellLiqFee = _liqFee;
        sellTotalFees = sellGumbyMktgFee + sellLiqFee;
    ...
    function updateGumbyMktgWallet(address newGumbyMktgWallet) external onlyOwner {
        emit gumbyMktgUpdated(newGumbyMktgWallet, gumbyMktg);
        gumbyMktg = newGumbyMktgWallet;
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (to == usV2Pair && sellTotalFees > 0){
                ...
                tokensForBank += fees * sellGumbyMktgFee / sellTotalFees;
            }
            
            // on buy
            if(from == usV2Pair && snipe[to] && buyTotalFees > 0) {
                buyGumbyMktgFee = 90;
                buyLiqFee = 0;
                buyTotalFees = buyGumbyMktgFee + buyLiqFee;
        	    fees = amount.mul(buyTotalFees).div(100);
        	    tokensForLiq += fees * buyLiqFee / buyTotalFees;
                tokensForBank += fees * buyGumbyMktgFee / buyTotalFees;
            }
            
            else if(from == usV2Pair && !snipe[to] && buyTotalFees > 0) {
                buyGumbyMktgFee = _buyGumbyMktgFee;
                buyLiqFee = _buyLiqFee;
                buyTotalFees = buyGumbyMktgFee + buyLiqFee;
        	    fees = amount.mul(buyTotalFees).div(100);
    ...
    function switchIt() private {
        ...
        uint256 ethForGumbyMktg = ethBalance.mul(tokensForBank).div(totalTokensToSwap);
        uint256 ethForLiq = ethBalance - ethForGumbyMktg;
        ...
        (success,) = address(gumbyMktg).call{value: address(this).balance}("");
    }
    ...
    function startGumby(uint256 botBlocks) external onlyOwner() {
