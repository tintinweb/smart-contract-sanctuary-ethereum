...
contract Collection is ERC721Enumerable, Ownable, RoyaltiesV2Impl {
    ...
    uint256 private constant MAX_NFT = 12;  
    ...
    constructor(
        ...
        // Init epochs 
        // TODO check epochs 
        epochs[1] = EpochDetails(2,10,0,1655055000,1655056500);
        epochs[2] = EpochDetails(1,2,0,1655056800,1655058300); 
        epochs[3] = EpochDetails(3,12,0,1655058600,0); 
        epochs[4] = EpochDetails(1,2,0,1655056800,0); 
    }
    ...
    */ 
        ...
        onlyWhenNotFreeze
    {
        ...
        // Epoch 2
            ...
            if (epochs[1].supply < epochs[1].maxTotal) epochs[2].maxTotal = epochs[1].maxTotal - epochs[1].supply;
            _mintNFT(_numOfTokens, _proof, 2, msg.sender);
        }
        // Epoch 3
        else if (block.timestamp >= epochs[3].startDate) { 
            if (epochs[2].supply < epochs[2].maxTotal) epochs[3].maxTotal = MAX_NFT - epochs[1].supply - epochs[2].supply - epochs[4].maxTotal;
            _mintNFT(_numOfTokens, _proof, 3, msg.sender);
        }
        else {
            require(false,"not available");
        }
    }
