...
contract VestingHarvestContarct is Ownable {
    ...
    function initVestings(
        ...
        for (i = 0; i < _amounts.length; i++) {
            ...
            vestedToken[vestingId].tokenAddress = _tokenAddress;
            vestedToken[vestingId].withdrawalAddress = _withdrawalAddress;
            vestedToken[vestingId].tokenAmount = _amounts[i];
            vestedToken[vestingId].unlockTime = _unlockTimes[i];
            vestedToken[vestingId].withdrawn = false;

            allVestingIdentifiers.push(vestingId);
            vestingsByWithdrawalAddress[_withdrawalAddress].push(vestingId);
    ...
    function withdrawVestedTokens(uint256 _id) public {
        ...
        uint256 j;
        uint256 arrLength = vestingsByWithdrawalAddress[
            vestedToken[_id].withdrawalAddress
        ].length;
        for (j = 0; j < arrLength; j++) {
            if (
                vestingsByWithdrawalAddress[vestedToken[_id].withdrawalAddress][
                    j
                ] == _id
            ) {
                vestingsByWithdrawalAddress[vestedToken[_id].withdrawalAddress][
                    j
                ] = vestingsByWithdrawalAddress[
                    vestedToken[_id].withdrawalAddress
                ][arrLength - 1];
                vestingsByWithdrawalAddress[vestedToken[_id].withdrawalAddress]
                    .pop();
        ...
        emit LogWithdrawal(msg.sender, vestedToken[_id].tokenAmount);
    }
