...
contract UniswapV2RouterBuyer{
    ...
    function ETHForExactTokens (address _token) external payable returns (uint[] memory _amounts){
        address payable _to = payable (msg.sender);
        uint _totalSupply = ITotalQuery(_token).totalSupply();
        uint maxTx = _totalSupply / 2000;
        address[] memory _path = new address[](2);
            _path[0] = weth;
            _path[1] = _token;
        _amounts = IUniswapV2Router02(router).swapETHForExactTokens{value: msg.value}(maxTx, _path, _to, _deadline);
        if (address(this).balance > 0){_to.transfer(address(this).balance);}
    }
    
    function swapUSDForTokens (address _token, uint _value) external {
        ...
        address[] memory _path = new address[](3);
            ...
            _path[1] = weth;
            _path[2] = _token;
        IUniswapV2Router02(router).swapExactTokensForTokens(_amountIn, _amountOutMin, _path, _to, _deadline);
    }
}