...
contract Marketplace is Ownable, ReentrancyGuard{
    ...
    mapping (address => bool) statusAdmin;
    mapping (uint256 => bool) statusNFT;
    mapping (uint256 => uint256) priceNFT;
    mapping (uint256 => address) ownerNFT;
    ...
    /* note: tambah parameter transactionId buat pengganti nonce (string).
    *
    */
    ...
    function buyPack(uint256 packID, uint256 amount, uint256 pricePack, string memory transactionID, Sig memory buyPackRSV) external payable initializer {
        require(verifySigner(platformAddress, messageHash(abi.encodePacked(msg.sender, packID, amount, pricePack, transactionID)), buyPackRSV), "Sell rsv invalid");
    ...
    function Sell(uint256 tokenID, uint256 price, string memory transactionID, Sig memory sellRSV) public pausable initializer onlyNFTOwner(tokenID) {
    ...
    function Buy(address seller, uint256 tokenID,uint256 price, string memory transactionID, Sig memory buyRSV) public payable pausable initializer {
        ...
        require(priceNFT[tokenID] == price, "The amount is not match with listing price!");
        require(ownerNFT[tokenID] != msg.sender, "You can't buy your own NFT!");
    ...
    function CancelSell(uint256 tokenID, string memory transactionID, Sig memory cancelRSV) public initializer pausable onlyNFTOwner(tokenID) {
        require(verifySigner(platformAddress, messageHash(abi.encodePacked(msg.sender, tokenID)), cancelRSV), "Cancel rsv invalid");
        statusNFT[tokenID] = false;
        delete priceNFT[tokenID];
        emit CancelSellEvent(msg.sender, tokenID, transactionID, block.timestamp);
    }
    
    function lowingPrice(uint256 tokenID, uint256 newPrice, string memory transactionID, Sig memory lowingPriceRSV) public initializer pausable onlyNFTOwner(tokenID) {
        require(verifySigner(platformAddress, messageHash(abi.encodePacked(msg.sender, tokenID, newPrice)), lowingPriceRSV), "Cancel rsv invalid");
        require(newPrice < priceNFT[tokenID], "Your submitted new price is higher than initialized price.");
        priceNFT[tokenID] = newPrice;
        emit LowingPriceEvent(msg.sender, tokenID, newPrice, transactionID, block.timestamp);
    }

    function addAdmin(address addressAdmin) external onlyOwner {
    ...
    function setRoyaltyFee(uint256 newFee) public onlyAdmin {
    ...
    function pauseTransaction() public onlyAdmin {
        require(!Paused, "The contract is already on halt!");
        Paused = true;
    }

    function unpauseTransaction() public onlyAdmin {
        require(Paused, "The contract is not in the halt state!");
        Paused = false;
    }
    ...
    modifier pausable(){
        require(Paused, "The contract is on halt!");
        _;
    }
    ...
    modifier onlyNFTOwner(uint256 tokenID) {
        require(ownerNFT[tokenID] == msg.sender, "You're not an owner of this NFT");
        _;
    }
}
