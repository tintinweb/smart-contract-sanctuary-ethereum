...
contract WorkV2 is AccessControlEnumerable{
    ...
    address public TimerrToken = 0xb5c9Aa0d54091b32c6496b1F3F745e2492daa2fF;
    ...
    function askForServices(address _worker,uint256 _total, bytes16 _orderID,address _currency) public{
        require(isRegistered[msg.sender], "You're not registered");
        ...
        require(!isBanned[_worker] && !isBanned[msg.sender],"You or the worker has been banned");
        // require(_price == msg.value, "Price and Value send are different");
        uint256 fee;
        if(_currency == TimerrToken)
            fee = 0;
        else
            fee = (_total*(baseFee))/1000;
        uint256 price = _total.sub(fee);

        workAgreement memory newWork = workAgreement(workCount,false,_worker,msg.sender,_currency,price,fee,_total,block.timestamp,false,false);

        ERC20 paymentToken = ERC20(_currency);
        paymentToken.transferFrom(msg.sender,address(this),_total*(10**paymentToken.decimals()));
        ...
        emit JobProposal(msg.sender, _worker ,_total ,newWork.id, _orderID,_currency);
    }

    // function askForServicesTimerr(address _worker,uint256 _total, bytes16 orderID) public{
    //     require(isRegistered[msg.sender], "You're not registered");
    //     require(isRegisteredAsFreelancers[_worker], "Worker is not a Freelancer");
    //     require(isVerified[_worker], "Worker has not been verified");
    //     require(!isBanned[_worker] && !isBanned[msg.sender],"You or the worker has been banned");
    ...
    //     workAgreement memory newWork = workAgreement(workCount,false,_worker,msg.sender,TimerrToken,_total,0,_total,block.timestamp,false,false);

    //     ERC20 paymentToken = ERC20(TimerrToken);
    //     paymentToken.transferFrom(msg.sender,address(this),_total*(10**paymentToken.decimals()));
    //     // paymentToken.transfer(feeAddress,fee*(10**paymentToken.decimals())); fee are taken after
    ...
    function refuseJob(uint256 _id) external{
        ...
        paymentToken.transfer(contractByID[_id].user,contractByID[_id].price*(10**paymentToken.decimals()));
    ...
    function cancelJob(uint256 _id) external {
        ...
        uint256 indexU = getPendingContractIndexByID(msg.sender, _id);
        uint256 indexW = getPendingContractIndexByID(current.worker, _id);

        ERC20 paymentToken = ERC20(BUSD);
        paymentToken.transfer(current.user,contractByID[_id].price*(10**paymentToken.decimals()));

        remove(pendingContract[msg.sender],indexU);
    ...
    function deliveryAccepted(uint256 _id) public{
        ...
        paymentToken.transfer(contractByID[_id].worker,contractByID[_id].price*(10**paymentToken.decimals()));

        paymentToken.transfer(feeAddress,contractByID[_id].fee*(10**paymentToken.decimals()));
