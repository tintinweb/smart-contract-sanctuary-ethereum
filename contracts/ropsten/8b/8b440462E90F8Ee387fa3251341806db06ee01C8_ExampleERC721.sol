...
contract ExampleERC721 is ERC721Full, Ownable {
  ...
  {
    require(address(msg.sender) == ownerOf(tokenId), "ERC721: INVALID_TOKEN_OWNER");
    _burn(tokenId);
  }

  function getTokenURI(uint256 tokenId) internal view returns (string memory) {
		require(_exists(tokenId), "ExampleERC721: URI query for nonexistent token");
