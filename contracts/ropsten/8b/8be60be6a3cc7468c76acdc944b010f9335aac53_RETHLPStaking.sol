...
contract RETHLPStaking is SMAuth, ReentrancyGuard  {
  ...
  mapping (uint256 => mapping(address=> uint256)) public userstakes;
    ...
    constructor(IERC20 _rewardtoken,IERC20 _depositToken , address token ) {
        auth = msg.sender;
    ...
    function getApproxRETH() internal view returns(uint256) {
        uint256 lp_supply = IERC20(depositToken).totalSupply();
        uint256 currentReth = IERC20(rewardToken).balanceOf(_token);
    ...
    function withdrawToken(IERC20 token) external nonReentrant onlyAuth{
