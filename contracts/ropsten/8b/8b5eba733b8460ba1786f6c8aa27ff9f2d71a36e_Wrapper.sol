...
//    }
    ...
    function getListTokens(KyberNetwork network) public view returns (OrderbookReserve[] memory oReserves, KyberReserve[] memory kReserves) {
        uint numOrderReserves = 0; 
        uint numKyberReserves = 0;
        uint rID;
        KyberReserveInterface reserve;
        ...
        oReserves = new OrderbookReserve[](numOrderReserves);
        kReserves = new KyberReserve[](numKyberReserves);
        numOrderReserves = 0;
        numKyberReserves = 0;
        for(rID = 0; rID < numReserves; rID++) {
            reserve = network.reserves(rID);
            if (network.reserveType(reserve) == KyberNetwork.ReserveType.PERMISSIONLESS) {
                // permissionless reserve
                oReserves[numOrderReserves] = OrderbookReserve(reserve);
                numOrderReserves += 1;
            } else {
                kReserves[numKyberReserves] = KyberReserve(reserve);
                numKyberReserves += 1;
            }
        }
        return (oReserves, kReserves);
    }
}