...
contract MOGE is Context, Ownable, IERC20, ERC20Detailed {
  ...
  using SafeMath for uint256;
    ...
    mapping (address => uint256) public coolDownTimerForAddress;
    ...
    bool public cooldownEnbaled = true;
    uint256 buyerCount = 0;
    uint256 jackpotReward = 0;
  

    address payable public marketingAddress = payable(0xa8dB671d726678d9a65CDa0FF8611c208E71dE10);
    ...
    constructor () ERC20Detailed("Mozzle Doge", "MOGE", 9) {
	...
	//uniswapv3 router = 0xE592427A0AEce92De3Edee1F18E0157C05861564
	 IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        ...
        _maxWalletAmount = _totalSupply.mul(2).div(100);
    ...
    function setFeePercent(uint256 updatedMarketingFee, uint256 updatedJackpotFee, uint256 updatedLiquidityFee) external onlyOwner {
        marketingFee = updatedMarketingFee;
        jackpotFee =  updatedJackpotFee;
        liquidityFee = updatedLiquidityFee;
        require(marketingFee.add(jackpotFee).add(liquidityFee) <= 15, "Fee is crossing the boundaries");
    }
    ...
    function addInBlacklist(address[] memory wallets) external onlyOwner
    {
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        {
        if(AMMs[sender]){
            coolDownTimer = block.timestamp.add(coolDownTimer);
            coolDownTimerForAddress[sender] = coolDownTimer;
            buyerCount++;
          
            if(buyerCount == 3)
            {
                swapAndLiquify(jackpotReward);
        ...
        if(AMMs[recipient])
        {
            require(block.timestamp >= coolDownTimerForAddress[sender], "Your cool down timer has not yet expired!!");
        }
        ...
        if(takeFee)
        { 
            uint256 taxAmount = (amount.mul(marketingFee + jackpotFee + liquidityFee )).div(100);
            ...
            uint256 jackpotAmount = taxAmount.mul(jackpotFee).div(totalFee());
            jackpotReward = jackpotReward.add(jackpotAmount);
        }
        else
        {
            _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
    ...
    function enableDisableCooldown(bool _cooldownEnabled) external onlyOwner
    {
        cooldownEnbaled = _cooldownEnabled;
    }

    function totalFee() internal view returns(uint256)
    {
        return marketingFee.add(jackpotFee).add(liquidityFee);
    }
    ...
    function swapAndLiquify(uint256 tokensToLiquify) private {
       ...
       uint256 tokensToLP = tokensToLiquify.mul(liquidityFee).div(totalFee()).div(2);
        ...
        uint256 ethForMarketing = ethBalance.mul(marketingFee).div(ethFeeFactor);
