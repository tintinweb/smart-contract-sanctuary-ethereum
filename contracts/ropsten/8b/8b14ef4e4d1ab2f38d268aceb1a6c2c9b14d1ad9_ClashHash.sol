...
contract ClashHash is Ownable {
    ...
    function claimRewardForUser(uint256 blockNumber, address user) public {
        ...
        (uint256 userReward, uint256 referrerReward, uint256 adminReward) = round.records.rewards(
            user,
            referrers[user],
            referrerFee,
            ADMIN_FEE
        );
        round.records.claimReward(user, _admin, ADMIN_FEE, referrers[user], referrerFee);
