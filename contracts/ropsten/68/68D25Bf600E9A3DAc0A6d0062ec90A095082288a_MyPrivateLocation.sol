...
contract MyPrivateLocation is Context, Ownable {
    ...
    event AnsweredInvitation(uint256 id);
    ...
    function accept(uint256 _invitationId, bool acceptation) public {
        ...
        for (
            uint256 i = 0;
            i < _allowances[_invitations[_invitationId].from].length;
            i++
        ) {
            require(
                _allowances[_invitations[_invitationId].from][i] !=
                    _msgSender(),
        ...
        require(
            _invitations[_invitationId].to == _msgSender(),
            "You're not granted to view this user location"
        );

        if (!acceptation) {
            _invitations[_invitationId].state = InvitationState.Declined;
            return;
        }

        _invitations[_invitationId].state = InvitationState.Accepted;
        _allowances[_invitations[_invitationId].from].push(_msgSender());

        emit AnsweredInvitation(_invitationId);
