...
// File: contracts/InEthWeTrust.sol
...
contract InEthWeTrust is ERC721A, Ownable {
  
  uint256 public mintPrice = 0.004878 ether;

  string _baseTokenURI;

  bool public isActive = false;

  uint256 public MAX_SUPPLY = 4878;
  ...
  constructor(string memory baseURI) ERC721A("In Eth We Trust", "IEWT") {
    setBaseURI(baseURI);
  }
  ...
  function mint(uint256 _count) public payable saleIsOpen {
    ...
    if(balanceOf(msg.sender) == 0 && _count > 1) {
        uint256 discountPrice = _count - 1;
