...
contract IDO is Ownable {
    ...
    function withdraw(address to) external onlyOwner {
        if (remaining > 0) {
            icoToken.safeTransfer(to, remaining.add(remaining.mul(8).div(100)));
            remaining = 0;
        }
    }
    ...
    function subscribe(uint256 value) external {
        ...
        uint usdtAmount = value.mul(price).div(BASE_RATIO);
        require(usdtAmount >= minLimit, "cannot be lower than the minimum value");
        require(block.timestamp < endTime, "ico has ended");
        UserInfo storage user = userInfos[msg.sender];
        require(
            user.collectState == 0,
            "can no longer participate after withdrawal"
        );
        require(
            icoLimitPerUser == 0 || user.amount.add(value).mul(price).div(BASE_RATIO) <= icoLimitPerUser,
        ...
        if (!isValidUser[msg.sender]) {
            ...
            if (referrer != address(0)) {
                referralNumber[referrer] = referralNumber[referrer].add(1);
                if (referralNumber[referrer] % 10 == 0) {
                    nftToken.mint(referrer);
                    if (referralNumber[referrer] % 100 == 0) {
                        nftToken.mintAndSetTokenUri(referrer, "chili");
                    }
                }
            }
        }
    ...
    function collect() external {
        ...
        if (
            block.timestamp > endTime && block.timestamp.sub(endTime) > 14 days
        ) {
            user.collectState = 3;
        } else if (
            block.timestamp > endTime && block.timestamp.sub(endTime) > 7 days
        ) {
            user.collectState = 2;
        } else {
            user.collectState = 1;
        ...
        address referral = referralHandler.referrers(msg.sender);
        if (referral != address(0)) {
            icoToken.safeTransfer(referral, amount.mul(5).div(100));
            referral = referralHandler.referrers(referral);
            if (referral != address(0)) {
                icoToken.safeTransfer(referral, amount.mul(3).div(100));
            } else {
                icoToken.safeTransfer(vault, amount.mul(3).div(100));
            }
        } else {
            icoToken.safeTransfer(vault, amount.mul(8).div(100));
        }
    ...
    function harvest(address account) public view returns (uint256) {
        ...
        if (
            ...
            if (user.collectState == 0) {
                ...
                amount = user.amount.mul(70).div(100);
            } else if (user.collectState == 2) {
                amount = user.amount.mul(40).div(100);
            }
        } else if (
            block.timestamp > endTime && block.timestamp.sub(endTime) > 7 days
        ) {
            if (user.collectState == 0) {
                amount = user.amount.mul(60).div(100);
            } else if (user.collectState == 1) {
