...
contract BLVCKNFT is ERC721, Ownable, PaymentSplitter {
    ...
    function checkTotalSupply(uint256 amount) private view returns (uint256) {
        ...
        require(tokenId + amount < MAX_SUPPLY - RESERVED_AMOUNT, "total supply reached");
        return tokenId;
    }
     ...
     */
    function minterMintWithPendingBalance(address[] memory addresses) external onlyMinter {
    ...
    */
    ...
    /*
        ...
        in round 1 amount is max 1, round 2 up to 2, round 3 is currently unlimited - we need to check this
    */
    ...
    function getClaimedAmounts(address addr) external view returns (uint256) {
    ...
    */
    function whitelistMint(bytes32[] calldata merkleProof) external payable {
        ...
        if (currentRound == 1 || currentRound == 2) {
            require(claimedAmounts[_msgSender()] < currentRound, "maximum is 1 mint per round");
        }
