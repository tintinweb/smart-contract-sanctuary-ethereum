...
// MasterDiv is the master of Div and sDiv.
...
contract MasterDiv is Ownable, IMasterBonus, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Divs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDivPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDivPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lpSupply;
        uint256 allocPoint;       // How many allocation points assigned to this pool. Divs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Divs distribution occurs.
        uint256 accDivPerShare; // Accumulated Divs per share, times 1e12. See below.
    ...
    // The Div TOKEN!
    ShibaBEP20 public Div;
    // The SDiv TOKEN!
    ShibaBEP20 public sDiv;
    // Dev address.
    address public devaddr;
    // Div tokens created per block.
    uint256 public DivPerBlock;
    // Deposit Fee address
    ...
    // The block number when Div mining starts.
    uint256 public immutable startBlock;

    // Initial emission rate: 1 Div per block.
    uint256 public immutable initialEmissionRate;
    // Minimum emission rate: 0.5 Div per block.
    ...
    constructor(
        ...
        DivPerBlock = _DivPerBlock;
        startBlock = _startBlock;
        initialEmissionRate = _DivPerBlock;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint256 _depositFeeBP, bool _isSDivRewards) external onlyOwner {
        ...
        poolInfo.push(PoolInfo({
            ...
            isSDivRewards: _isSDivRewards
        }));
    }

    // Update the given pool's Div allocation point. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint256 _depositFeeBP, bool _isSDivRewards) external onlyOwner {
        ...
        poolInfo[_pid].isSDivRewards = _isSDivRewards;
        if (prevAllocPoint != _allocPoint) {
            totalAllocPoint = totalAllocPoint.sub(prevAllocPoint).add(_allocPoint);
        }
    }

    // View function to see pending Divs on frontend.
    function pendingDiv(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accDivPerShare = pool.accDivPerShare;
        uint256 lpSupply = pool.lpSupply;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 DivReward = multiplier.mul(DivPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accDivPerShare = accDivPerShare.add(DivReward.mul(1e12).div(lpSupply));
        }
        uint256 userRewards = user.amountWithBonus.mul(accDivPerShare).div(1e12).sub(user.rewardDebt);
        if(!pool.isSDivRewards){
            // taking account of the 2% auto-burn
            userRewards = userRewards.mul(98).div(100);
        }
        return userRewards; // taking account of the 2% auto burn on Div
    }
    ...
    function updateEmissionRate() internal {
        ...
        if(DivPerBlock <= minimumEmissionRate){
            return;
        }
        ...
        uint256 newEmissionRate = DivPerBlock;
        ...
        if (newEmissionRate >= DivPerBlock) {
            return;
        }
        
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = DivPerBlock;
        DivPerBlock = newEmissionRate;
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 DivReward = multiplier.mul(DivPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 devMintAmount = DivReward.div(10);
        Div.mint(devaddr, devMintAmount);
        if (pool.isSDivRewards){
            sDiv.mint(address(this), DivReward);
        }
        else{
            Div.mint(address(this), DivReward);
        }
        pool.accDivPerShare = pool.accDivPerShare.add(DivReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function updateUserBonus(address _user, uint256 _pid, uint256 bonus) external virtual override validatePool(_pid) onlyAggregator{
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amountWithBonus.mul(pool.accDivPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                if(pool.isSDivRewards){
                    safeSDivTransfer(_user, pending);
                }
                else{
                    safeDivTransfer(_user, pending);
                }
            }
        }
        ...
        user.rewardDebt = user.amountWithBonus.mul(pool.accDivPerShare).div(1e12);
    }

    // Deposit LP tokens to MasterShiba for Div allocation.
    function deposit(uint256 _pid, uint256 _amount) external validatePool(_pid) nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(Div)) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) external validatePool(_pid) nonReentrant {
        ...
        if(pending > 0) {
            if(pool.isSDivRewards){
                safeSDivTransfer(msg.sender, pending);
            }
            else{
                safeDivTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
    ...
    returns(address lpToken, uint256 allocPoint, uint256 lastRewardBlock,
            uint256 accDivPerShare, uint256 depositFeeBP, bool isSDivRewards) {
        return (
            address(poolInfo[_pid].lpToken),
            poolInfo[_pid].allocPoint,
            poolInfo[_pid].lastRewardBlock,
            poolInfo[_pid].accDivPerShare,
            poolInfo[_pid].depositFeeBP,
            poolInfo[_pid].isSDivRewards
        );
    }

    // Safe Div transfer function, just in case if rounding error causes pool to not have enough Divs.
    function safeDivTransfer(address _to, uint256 _amount) internal {
        uint256 DivBal = Div.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > DivBal) {
            transferSuccess = Div.transfer(_to, DivBal);
        } else {
            transferSuccess = Div.transfer(_to, _amount);
        }
        require(transferSuccess, "safeDivTransfer: Transfer failed");
    }

    // Safe sDiv transfer function, just in case if rounding error causes pool to not have enough SDivs.
    function safeSDivTransfer(address _to, uint256 _amount) internal {
        uint256 sDivBal = sDiv.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > sDivBal) {
            transferSuccess = sDiv.transfer(_to, sDivBal);
        } else {
            transferSuccess = sDiv.transfer(_to, _amount);
        }
