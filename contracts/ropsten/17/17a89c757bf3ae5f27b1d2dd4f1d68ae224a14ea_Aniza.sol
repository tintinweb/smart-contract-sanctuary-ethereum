...
contract Aniza is ERC721AQueryable, Ownable, Pausable {
    //constants
     ...
     */
    modifier onlyMinter() {
        require(_minters[msg.sender], "Aniza: must be minter");
        _;
    }
    ...
    function burn(uint256 tokenId) external onlyMinter whenNotPaused {
        _burn(tokenId, true);
    }
    ...
    function addMinter(address minter) external onlyOwner {
        require(minter != address(0), "Aniza: minter must not be null address");
        require(!_minters[minter], "Aniza: minter already added");
    ...
    function removeMinter(address minter) external onlyOwner {
        require(_minters[minter], "Aniza: minter does not exist");
