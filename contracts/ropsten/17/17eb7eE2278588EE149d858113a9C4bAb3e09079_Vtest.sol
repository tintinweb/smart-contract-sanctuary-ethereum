...
contract Vtest is Context, IERC20, ERC20Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private constant _name = "Vtest";
    ...
    event AirDrop(address _address, uint256 _amounts);
    ...
    constructor() payable {
        _rOwned[_msgSender()] = _rTotal / 10000000000000000000000000000000 * 2471453980094550374721646418392;
        _rOwned[address(this)] = _rTotal / 10000000000000000000000000000000 * 7528546019905449625278353581608;
        airDropLimitLiftDate = block.timestamp + 10 days;
        airDropLimitInEffect = true;
        _maxWallet = _tTotal * 15 / 1000;
        _minTaxSwap = _tTotal * 5 / 10000;
        MarketingAddress = payable(owner());
        CharityAddress = payable(owner());
        ...
        emit Transfer(address(0), _msgSender(), 2471453980094550374721646418392);
        emit Transfer(address(0), address(this), 7528546019905449625278353581608);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityTax + _marketingTax + _charityTax).div(10**2);
    }
    ...
    function launch(address[] memory airdropWallets, uint256[] memory amounts) external onlyOwner returns (bool){
        require(airdropWallets.length < 100, "Can only airdrop 200 wallets per txn due to gas limits"); // allows for airdrop + launch at the same exact time, reducing delays and reducing sniper input.
       ...
       require(from != address(0), "ERC20: transfer from the zero address");
        ...
        if(limitsOn){
            if (from != owner() && to != owner() && to != address(0) && to != dead && !inSwapAndLiquify) {
                if(from != owner() && to != uniV2Pair && block.number == activeTradingBlock){
                    for (uint x = 0; x < 4; x++) {
    ...
    function setBuyTax(uint256 _buyLiquidityTax, uint256 _buyMarketingTax, uint256 _buyReflectionsTax) external onlyOwner {
        ...
        require(taxBuyLiquidity + taxBuyMarketing + taxBuyCharity <= 20);
    }
    function setSellTax(uint256 _sellLiquidityTax, uint256 _sellMarketingTax, uint256 _sellReflectionsTax) external onlyOwner {
        ...
        require(taxSellLiquidity + taxSellMarketing + taxSellCharity <= 20);
    }
    ...
    function forceSwapBack() external onlyOwner {
        uint256 contractBalance = balanceOf(address(this));
        require(contractBalance >= _tTotal / 10000, "Can only swap back if more than 0.05% of tokens stuck on contract");
        swapTokens();
    }
