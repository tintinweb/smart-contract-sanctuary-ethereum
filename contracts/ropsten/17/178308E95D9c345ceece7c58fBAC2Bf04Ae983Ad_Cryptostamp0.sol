...
// File: contracts/Cryptostamp0.sol
...
contract Cryptostamp0 is ERC721Token("Cryptostamp Edition 0", "CS0") {
    ...
    string constant uribase = "https://test.cryptostamp.at/CS0/";
    ...
    // Issue a new cryptostamp asset, giving it to a specific owner address.
    ...
    requireMinting
    {
        ...
        require(_tokenId == 0 || exists(_tokenId.sub(1)));
        // _mint already ends up checking if owner != 0 and that tokenId doesn't exist yet.
        _mint(_owner, _tokenId);
    }

    // Batch-issue multiple cryptostamp with adjacent IDs.
    function createMulti(uint256 _tokenIdStart, address[] _owners)
    public
    onlyCreateControl
    requireMinting
    {
        // Make sure we do not get any holes in Ids so we can do more optimizations.
        require(_tokenIdStart == 0 || exists(_tokenIdStart.sub(1)));
    ...
    // Override ERC721Token to create the URI from the base and ID.
    ...
    /*** Override ERC721Token to make creation cheaper ***/

    /**  This eliminates use of the the allTokens array and allTokensIndex mapping.
     *   Note: _burn cannot be used any more with this, and creationg needs to be
     *   in ascending order, starting with 0, and have no holes in the row of IDs
     */
    ...
    function _mint(address _to, uint256 _tokenId) internal {
        // We can't use ERC721BasicToken._mint() directly, so copy it's contents.
        require(_to != address(0));
        addTokenTo(_to, _tokenId);
        emit Transfer(address(0), _to, _tokenId);
        // Instead of allTokens*, we only need to bump total supply.
