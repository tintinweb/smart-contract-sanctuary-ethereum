...
contract GamesVaultDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("GamesVault_Dividend_Tracker", "GamesVault_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GamesVault_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GamesVault_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GamesVault contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GamesVault_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "GamesVault_Dividend_Tracker: Cannot update claimWait to same value");
...
contract GamesVault is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    GamesVaultDividendTracker public dividendTracker;
...
/**
Token NAme:Games Vault
Symbol:GV
Total Supply:100.000.000

Taxes and distributions:

Tax 6% to treasury (ETH) - (0xf7bC19E474d0628353ca67Db6185013922Ab2785)
4% development and marketing (ETH) - (0x790ecC1a3d9a2ec18a45e352aDD1AeFEf5241eec)
2% reflections to holders (GV TOKEN)
2% Liquidity pool

Max Transaction limit 1% of total supply
*/
    constructor() ERC20("Games Vault", "GV") {
        ETHRewardsFee = 2;
        liquidityFee = 2;
        marketingFee = 4;
        treasuryFee = 6;
        ...
        marketingWallet = payable(0x790ecC1a3d9a2ec18a45e352aDD1AeFEf5241eec); // 
        treasuryWallet = payable(0xf7bC19E474d0628353ca67Db6185013922Ab2785); // 

        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee).add(treasuryFee); // total fee transfer and buy

        onSelltotalFees = onSellETHRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee).add(onSelltreasuryFee);

    	dividendTracker = new GamesVaultDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GamesVault: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GamesVault: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GamesVault: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GamesVault: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GamesVault: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GamesVault: Cannot update gasForProcessing to same value");
