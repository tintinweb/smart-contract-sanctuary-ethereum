...
contract Voting {
	...
	mapping(uint256 => PollVote[]) public _pollVotes; // _PollIds -> PollVotes 

	event EvtCommunityPollCreate( uint256 pollId, CommunityPoll[] poll);
	event EvtCommunityPollDelete( uint256 pollId);
	event EvtCommunityPollVotes( PollVote[] pollArr );
	...
	*/
	function communityVoteCast(uint256 commId, uint256 pollId, bool vote) external {
		CommunityPoll storage poll = pollFetch(commId, pollId);
		...
		if(poll.endTimeStamps < block.timestamp){
			...
			PollVote[] storage pollVotes = _pollVotes[pollId];
			pollVotes.push(pollvote);
		}else {
	...
	function isAlreadyVoted(uint256 pollId) internal view returns(bool){
		...
		for (uint256 index = 0; index < pollVotes.length ; index++) {
			if(pollVotes[index].voter == msg.sender){
				isVoted = true;
			}
		}
		return isVoted;
	...
	function VotesInfo(uint256 pollId) external {
		PollVote[] memory pollVotes = _pollVotes[pollId];
		emit EvtCommunityPollVotes(pollVotes);
	}

	/**
	* @notice get the details for the community poll
	*/
	function pollFetch(uint256 commId, uint256 id) internal returns ( CommunityPoll storage ){
		CommunityPoll storage poll = _polls[commId][id];
		// require(id > 0, "No poll found with provided pollId");
		emit EvtCommunityPoll(poll);
		return poll;
	}
	...
	*/
	function communityPollCreate(uint256 daoId, uint256 pollType, ProposeChanges memory proposedChanges, uint256 startTime, uint256 endTime
		...
		CommunityPoll[] storage communityPoll = _polls[daoId]; 
		communityPoll.push(poll);
		emit EvtCommunityPollCreate(pollId, communityPoll);
		return true;
	}
	...
	*/
	function _computePollResult(uint256 communityId, uint256 pollId) internal returns(bool){
		...
		CommunityPoll storage commPoll = pollFetch(communityId, pollId);
	...
	*/
	function _doProposedChanges(uint256 commId, uint256 pollId) external returns (bool) { 
		if(_computePollResult(commId, pollId)){
			...
			CommunityPoll storage commPoll = pollFetch(commId, pollId);
