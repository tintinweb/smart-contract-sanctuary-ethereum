...
contract ExtraContract is ERC721Enumerable, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    uint256[] private _teamShares = [25];
    address[] private _team = [
        0x8b87AcB95B0bd39896e364CcB6facfFA8795509A
    ];

    constructor()
    ERC721("ExtraContract", "EC")
    ...
    function giftMint(address[] calldata _addresses) external onlyOwner {
        require(
            ...
            "Contract: max total supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Contract: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function presaleMint(uint256 _amount) external payable nonReentrant {
        ...
        require(
            ...
            "Contract: You can't mint so much tokens"
        );
        require(
        ...
        require(
            ...
            "Contract: max supply exceeded"
        );
        require(
            _price * _amount <= msg.value,
            "Contract: Ether value sent is not correct"
        );
        uint256 _newItemId;
    ...
    function publicMint(uint256 _amount) external payable nonReentrant {
        ...
        require(_amount > 0, "Contract: zero amount");
        require(
            totalMint + _amount <= maxSupply - giftSupply,
            "Contract: max supply exceeded"
        );
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Contract: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Contract: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
