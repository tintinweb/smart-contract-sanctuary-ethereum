...
import "@openzeppelin/contractsv4/token/ERC721/IERC721.sol";
import "@openzeppelin/contractsv4/token/ERC721/IERC721Receiver.sol";
...
contract NFTAuction is Ownable, Pausable {
    ...
    //fee
    uint public constant PERCENTAGE = 1e6;
    uint public FEE;
    uint public MAX_FEE = 1e5;
    ...
    event FeeChanged(uint indexed oldFee, uint indexed newFee);
    event WhiteListNFT(address indexed nft, bool indexed status);
    event WhiteListERC20(address indexed erc20, bool indexed status);
    event FeeReceiver(address indexed _feeReceiver);
    /**********************************/
    /*╔═════════════════════════════╗
      ║             END             ║
      ║            EVENTS           ║
    ...
    modifier inWhiteListNFT(address _nft) {
        require(whiteListNFT[_nft], "NFT not allow");
        _;
    }

    modifier inWhiteListERC20(address _erc20) {
        require(whiteListNFT[_erc20], "ERC20 not allow");
        _;
    }
   ...
   ║    ADMIN FUNCTIONS   ║
   ╚══════════════════════════════╝*/
    function changeFee(uint newFee) external onlyOwner whenPaused {
        require(newFee <= MAX_FEE, "FEE invalid");
        uint oldFee = FEE;
        FEE = newFee;
    ...
    function toggleNFT(address _nft) external onlyOwner whenPaused {
        whiteListNFT[_nft] = !whiteListNFT[_nft];
        emit WhiteListNFT(_nft, whiteListNFT[_nft]);
    }

    function toggleERC20(address _erc20) external onlyOwner whenPaused {
        whiteListERC20[_erc20] = !whiteListERC20[_erc20];
        emit WhiteListERC20(_erc20, whiteListERC20[_erc20]);
    }

    function changeFeeReceiver(address _feeReceiver) external onlyOwner whenPaused{
        require(_feeReceiver != address(0), "!zero address");
        feeReceiver = _feeReceiver;
        emit FeeReceiver(_feeReceiver);
    }
    ...
    returns (uint256)
    {
        return (_totalBid * (_percentage)) / PERCENTAGE;
    }
    ...
    whenNotPaused
    inWhiteListERC20(_erc20Token)
    inWhiteListNFT(_nftContractAddress)
    {
    ...
    function _payFeesAndSeller(
        ...
        uint256 feesPaid = _getPortionOfBid(_highestBid, FEE);
//        for (
//            uint256 i = 0;
//            i <
//            nftContractAuctions[_nftContractAddress][_tokenId]
...
//        }
    ...
    ) external onlyNftSeller(_nftContractAddress, _tokenId) whenNotPaused{
