...
contract SlashCore is ISlashCore, Ownable {
    ...
    mapping (address => bool) public merchantListbool; 
    ...
    function handleFee(
        ...
        if(merchantListbool[merchant_] == false){
            merchantListbool[merchant_] = true;
            merchantList.push(merchant_);
        }
    ...
    function handlingEmergencyFees() public onlyOwner{

        for(uint256 i=0; i< merchantList.length; i++ ){
            IMerchantProperty merchant = IMerchantProperty(address(merchantList[i]));
            
            uint256 donationFeeAmount = _donationFeePerMerchant[address(merchantList[i])];
            if(donationFeeAmount>0)
            {
                merchant.viewDonationWallet().transfer(donationFeeAmount);
                _donationFeePerMerchant[merchantList[i]] = 0;
            }
            uint256 taxFeeAmount = _taxFeePerMerchant[address(merchantList[i])];
            if(taxFeeAmount>0)
            {
                address payable marketingWallet = merchant.viewMarketingWallet();
                FeeMethod feeProcessingMethod = FeeMethod(merchant.viewFeeProcessingMethod());
                if (feeProcessingMethod == FeeMethod.SIMPLE) {
                    marketingWallet.transfer(taxFeeAmount);
                } else if (feeProcessingMethod == FeeMethod.LIQU) {
                    handleFeeLiq(merchant, taxFeeAmount);
                ...
                _taxFeePerMerchant[merchantList[i]] = 0;
            }
