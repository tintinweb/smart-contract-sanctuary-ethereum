...
contract IFOInitializable is IIFOV2, ReentrancyGuard, Ownable {
    ...
    //List of cliffs for overflow 
    uint256[6] private overflowCliff;
    ...
    // IFODeployer
    IIFODeployer public ifoDeployer;
    ...
    function initialize(
        ...
        ifoDeployer = IIFODeployer(IFO_FACTORY);
        ifoPool = IIFOPool(_ifoPoolAddress);
        startBlock = _startBlock;
        endBlock = _endBlock;
        MAX_BUFFER_BLOCKS = _maxBufferBlocks;
        overflowCliff = _overflowCliff;
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                ifoDeployer.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (ratioOverflow >= overflowCliff[5]) {
            return 500000000; // 0.05%
        } else if (ratioOverflow >= overflowCliff[4]) {
            return 1000000000; // 0.1%
        } else if (ratioOverflow >= overflowCliff[3]) {
            return 2000000000; // 0.2%
        } else if (ratioOverflow >= overflowCliff[2]) {
            return 2500000000; // 0.25%
        } else if (ratioOverflow >= overflowCliff[1]) {
            return 3000000000; // 0.3%
        } else if (ratioOverflow >= overflowCliff[0]) {
    ...
    function viewCliffOverflow(uint8 _index) public view returns(uint256){
        return overflowCliff[_index];
    }
