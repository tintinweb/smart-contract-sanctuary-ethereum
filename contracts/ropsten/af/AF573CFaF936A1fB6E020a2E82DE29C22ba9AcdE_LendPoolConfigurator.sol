...
contract LendPoolConfigurator is Initializable, ILendPoolConfigurator {
  ...
  function configureNftAsBuyItNow(
    ...
    for (uint256 i = 0; i < assets.length; i++) {
      ...
      require(redeemDuration <= buyItNowDuration, Errors.LPC_INVALID_CONFIGURATION);
      ...
      emit NftBuyItNowChanged(assets[i], redeemDuration, buyItNowDuration, redeemFine);
    }
  }
  ...
  function setNftMinBuyItNowFine(address[] calldata assets, uint256 minBuyItNowFine) external onlyPoolAdmin {
    ILendPool cachedPool = _getLendPool();
    for (uint256 i = 0; i < assets.length; i++) {
      DataTypes.NftConfigurationMap memory currentConfig = cachedPool.getNftConfiguration(assets[i]);
  ...
  function batchConfigNft(ConfigNftInput[] calldata inputs) external onlyPoolAdmin {
    ...
    for (uint256 i = 0; i < inputs.length; i++) {
      ...
      emit NftMinBuyItNowFineChanged(inputs[i].asset, inputs[i].minBuyItNowFine);
    }
  }
