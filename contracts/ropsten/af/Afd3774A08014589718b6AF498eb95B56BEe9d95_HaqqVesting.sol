...
contract HaqqVesting {
    ...
    function portionsAvailableToWithdraw(address _beneficiaryAddress, uint256 _depositId) public view returns (uint256){
        ...
        /// One payment is unlocked immediately
        uint256 totalPayoutsUnblocked = (depositAge / TIME_BETWEEN_PAYMENTS) + 1;

        if (totalPayoutsUnblocked > NUMBER_OF_PAYMENTS) {
            totalPayoutsUnblocked = NUMBER_OF_PAYMENTS;
        }

        /// Number of unblocked but not yet made withdrawals
        return totalPayoutsUnblocked - deposits[_beneficiaryAddress][_depositId].portionsPaidAlready;
    }
    ...
    function amountToWithdrawNow(address _beneficiaryAddress, uint256 _depositId) public view returns (uint256){
        return portionsAvailableToWithdraw(_beneficiaryAddress, _depositId) * amountForOneWithdrawal(_beneficiaryAddress, _depositId);
    }
    ...
    function withdraw(address _beneficiaryAddress) external returns (bool success) {
        ...
        if (depositsCounter[_beneficiaryAddress] > 0) {
            for (uint256 depositId = 1; depositId <= depositsCounter[_beneficiaryAddress]; depositId ++) {
                ...
                console.log(amountToWithdrawNow(_beneficiaryAddress, depositId));

                sumToWithdraw = sumToWithdraw + amountToWithdrawNow(_beneficiaryAddress, depositId);
                deposits[_beneficiaryAddress][depositId].portionsPaidAlready = deposits[_beneficiaryAddress][depositId].portionsPaidAlready + 1;
            }
        }
