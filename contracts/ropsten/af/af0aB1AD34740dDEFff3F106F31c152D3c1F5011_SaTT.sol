...
contract SaTT is ERC20,Ownable {
    ...
    IERC20 private prevSatt;

    mapping(address => bool) private migratedBalances;

    constructor(IERC20 _prevSatt) ERC20("Smart Advertising Transaction Token", "SATT") {
       prevSatt = _prevSatt;
    }
    ...
    function migrate(address who) internal {
         if (!migratedBalances[who]) {
            _mint(who,prevSatt.balanceOf(who) );
            migratedBalances[who] = true;
        }
    }

    function balanceOf(address who) public override  view  returns (uint256) {
        if (!migratedBalances[who]) {
            return prevSatt.balanceOf(who);
        }
        else {
