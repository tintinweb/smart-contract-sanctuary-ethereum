...
contract Forwarder {
   ...
   */
    ...
    function balance(address tokenContractAddress) public view returns(uint){
        uint forwarderBalance = ERC20Interface(tokenContractAddress).balanceOf(address(this));
...
contract WalletSimple {
    ...
    */
    function sendMultiSigToken(address toAddress,uint value,address tokenContractAddress,uint expireTime,uint sequenceId,bytes signature) public onlySigner {
        ...
        ERC20Interface instance = ERC20Interface(tokenContractAddress);
        if (instance.balanceOf(address(this)) == 0) {
            return;
        }
