...
contract UniswapV2Handler is Ownable, ReentrancyGuard {
    ...
    function run(bytes memory _data) external payable nonReentrant {
        (
            ...
            uint256 _fee
        ) = abi.decode(_data, (address, uint256, uint256, address, address, address[], uint256));
        require(_swapPaths.length >= 2, "SwapStrategyV1::constructor: Invalid swapPaths");
        swapRouter = IUniswapV2Router02(_swapRouter);
        address inputToken = _swapPaths[0];
        address outputToken = _swapPaths[_swapPaths.length - 1];
        uint256 preOutputAmount = Utils.balanceOf(outputToken, address(this));
        if (outputToken == Utils.ETH_ADDRESS) {
        ...
        uint256 postOutputAmount = Utils.balanceOf(outputToken, address(this));
        ...
        Utils.transfer(outputToken, _to, amountOut - _fee);
        if (_fee > 0) {
            Utils.transfer(outputToken, _relayer, _fee);
        }
    }
