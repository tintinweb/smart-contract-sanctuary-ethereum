...
contract NFT is Ownable, ERC721A, ReentrancyGuard {
  ...
  )  ERC721A("NFT", "NFT", maxBatchSize_, collectionSize_, maxNormalNftIndex_, maxAmountForDevs_) {
  ...
  function setIssueParam(uint64 mintlistPriceWei, uint32 publicSaleStartTime,uint256 mintMaxLimit,uint256 issueAmount_, uint32 issueNo_) 
    external 
    onlyOwner {
  ...
  function mint(uint256 quantity) public payable isOpen callerIsUser {
    ...
    emit MintEvent(msg.sender, "mint token", quantity);
  }

  function mint(bytes32[] memory _proof, uint256 quantity) public payable isOpen callerIsUser {
    require(quantity > 0, "mint quantity must be nonzero");
    uint256 _saleStartTime = uint256(saleConfig.publicSaleStartTime);
    require(
      _saleStartTime != 0 && block.timestamp >= _saleStartTime,
    ...
    if (_whiteListCheck) {
      ...
      require(whitelistMintCounter[msg.sender] + quantity > mintLimitPerAddress, "reached maxnot eligible for whitelist mint");
      allowlist[msg.sender] += quantity;
    }
  ...
  function seedAllowlist(address[] memory addresses, uint256[] memory numSlots, bool check)
    external
    onlyOwner
  {
    require(
  ...
  function _random(uint32 length) private view returns(uint32) {
    uint256 random = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
    return uint32(random % length);
  }

  function _randomArray(uint32[] memory data) private view returns (uint32[] memory) {
    require(data.length > 0, "data must be above zero");
    uint32 seed = _random(uint32(data.length));
    uint32 n1 = seed;
    uint32 ch = 0;
    for (uint32 i = 0; i < data.length; i++) {
      n1 = uint32((data[n1] + data[i] + i) % data.length);
      ch = data[n1];
      data[n1] = data[i];
      data[i] = ch;
    }
  ...
  IERC721Enumerable
{
  ...
  uint256 private _vipCurrentIndex = 0;

  uint256 internal immutable maxNormalNftIndex;

  uint256 private maxAmountForDevs;
  ...
  constructor(
    ...
    require(
      maxNormalNftIndex_ > 0 && maxNormalNftIndex_ <= collectionSize_,
      "ERC721A: maxNormalNftIndex must be nonzero and must less then collectionSize");

    _name = name_;
    _symbol = symbol_;
    maxBatchSize = maxBatchSize_;
    collectionSize = collectionSize_;
    maxNormalNftIndex = maxNormalNftIndex_;
    _vipCurrentIndex = 0;
    maxAmountForDevs = maxAmountForDevs_;
    currentIndex = maxAmountForDevs;
  }
  ...
  function _exists(uint256 tokenId) internal view returns (bool) {
    if (tokenId >= maxAmountForDevs) {
      return tokenId < currentIndex;
    } else {
      return tokenId < _vipCurrentIndex;
    }
  }

  function _safeMint(address to, uint256 quantity, bool vip) internal {
    _safeMint(to, quantity, "", vip);
  }
  ...
  function _safeMint(
    ...
    uint256 startTokenId = (vip ? _vipCurrentIndex : currentIndex);
  ...
  //
  function _safeUpdateTokenPayload(uint256 tokenId) 
    internal
  {
    TokenOwnership memory tos = ownershipOf(tokenId);
    if (tos.addr != address(0)) {
      _ownerships[tokenId] = TokenOwnership(tos.addr, tos.startTimestamp);
    }
  }

  function leftAmountForDevs() public view returns(uint256) {
    return maxAmountForDevs - _vipCurrentIndex;
  }
