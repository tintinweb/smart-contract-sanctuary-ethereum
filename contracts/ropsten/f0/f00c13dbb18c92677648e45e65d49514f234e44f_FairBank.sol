...
contract FairBank is ERC721{
     ...
     */ 
    FairBankCompute constant private bankCompute = FairBankCompute(0xf13015D13b24D9c31B4536215BC3b86dF198D6b5);
        ...
        public
    {
        ...
        uint256 _purchasePrice = cardList[_cId].amount.mul(105) / 100;
        ...
        if(cardIndexToApproved[_cId] != address(0)){
            cardIndexToApproved[_cId].send(
                cardList[_cId].amount.mul(1025) / 1000
                );
        ...
        developerAddr.send(cardList[_cId].amount.mul(25) / 1000);
        ...
        returns(uint256)
    {
        buyAnalysis(
            ...
            _protectRatio <= 95 ? _protectRatio : standardProtectRatio, 
            _recommendAddr
            );
    }
        ...
        private
    {
        ...
        bankerFeeDataRecord(_recommendAddr, _amount, _protectRatio);    
    }
        ...
        private
    {
        ...
        uint256 _protectAmount = _amount.mul(_protectRatio) / 100;
    ...
    function bankerFeeDataRecord(address _recommendAddr, uint256 _amount, uint256 _protectRatio)
        private
    {
        uint256 _jackpotProportion = 15;
        if(_recommendAddr != address(0) 
            && _recommendAddr != msg.sender 
            && (register[_recommendAddr] != bytes32(0))
        ){
            _recommendAddr.send(_amount.mul(15) / 1000);
            msg.sender.send(_amount / 100);
        }
        else
            _jackpotProportion = 40;
            
        round[rId].jackpotAmount = round[rId].jackpotAmount.add(_amount.mul(_jackpotProportion).div(1000));

        if(_protectRatio == 0)
            cardList[0].playerAddress.send(_amount.mul(5) / 1000);
        else if(_protectRatio > 0 && _protectRatio < 50)
            cardList[1].playerAddress.send(_amount.mul(5) / 1000);   
        else if(_protectRatio == 50)
            cardList[2].playerAddress.send(_amount.mul(5) / 1000);   
        else if(_protectRatio > 50 && _protectRatio < 95)
            cardList[3].playerAddress.send(_amount.mul(5) / 1000);   
        else if(_protectRatio == 95)
            cardList[4].playerAddress.send(_amount.mul(5) / 1000);   
        
        developerAddr.send(_amount.mul(5) / 1000);
    }
        ...
        private
    {
        ...
        round[_rId + 1].jackpotAmount = (
            round[_rId].jackpotAmount.add(
                (round[_rId].amount.mul(95) / 100)
        ...
        if(_goods.stepSize == 0){
            if(round[_goods.rId].ended == true){
                if(round[_goods.rId].endSid > _goods.endSid){
                    ...
                    _amount = _goods.amount.mul(_goods.protectRatio) / 100;
        ...
        returns(uint256, uint256, uint256, uint256, uint256, uint256)
    {
        ...
        return
            (
                ...
                stage[rId][sId].dividendAmount <= _targetExpectedAmount ? _targetExpectedAmount : stage[rId][sId].dividendAmount,
                round[rId].jackpotAmount.add(
                    (round[rId].amount.mul(95) / 100)
        ...
        public
    {
        ...
        require (_size == 0, "Error type address");
