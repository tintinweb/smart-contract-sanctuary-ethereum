...
contract FintendToken is PausableToken {
  ...
  modifier onlyWhenTransferEnabled() {
    if (!transferable) {
      require(msg.sender == owner || msg.sender == fintendWallet);
    }
    _;
  }
  ...
  constructor(address _fintendWallet) public StandardToken("Fintend Token", "FT", 18) {

    require(_fintendWallet != address(0));
    fintendWallet = _fintendWallet;
    totalSupply_ = INITIAL_SUPPLY;
    balances[_fintendWallet] = totalSupply_;
    emit Transfer(address(0), _fintendWallet, totalSupply_);
  }
