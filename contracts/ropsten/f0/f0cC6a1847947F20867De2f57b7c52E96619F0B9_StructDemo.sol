...
event Output(int qn_id,string question,int option_id,string option,int point,string useraddress);
...
function addchoices(
  ...
  Choice memory e
    =Choice(qn_id,
        ...
        user_address);
  choices.push(e);
  emit Output(qn_id,question,option_id,option,point,user_address);
}
