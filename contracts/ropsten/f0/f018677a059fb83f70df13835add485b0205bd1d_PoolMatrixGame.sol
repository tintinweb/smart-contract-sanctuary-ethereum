...
contract PoolMatrixGame {
    ...
    */
    ...
    address[] internal predefinedWallets;
    ...
    constructor() {
        ...
        predefinedWallets.push(0xB792A69f270049D8A4c42E6cebCCbdf78CbDfd15); // Test5
        predefinedWallets.push(0xC18C40bA0aE6b0f692c42169E0580b4147E33D49); // Test6
        predefinedWallets.push(0xe1727079Dee0197a38251533E8B5E218df557182); // Alex
        ...
        // Adding predefined wallets to the users table
        for (i = 0; i < predefinedWallets.length; i++) {
            address addr = predefinedWallets[i];
            
            users[addr].registered = true;
            users[addr].registrationTimestamp = block.timestamp;
            users[addr].userAddr = addr;
            users[addr].isRootUser = true;
            users[addr].referrer = rootWallets[0];
            users[addr].initialBalance = 0;
            users[addr].debit = 0;
            users[addr].credit = 0;
            users[addr].maxLevel = 1;
            users[addr].isPredefined = true;
            userAddresses.push(addr);
        ...
        for (level = 0; level < levelsCount; level++) {
            ...
            headIndex[level] = predefinedWallets.length;
    ...
    function register(address referrer, uint256 investAmount, uint256 initialBalance) public payable {
        ...
        users[msg.sender].isPredefined = false;
    ...
    function buyLevel(uint8 level, uint256 investAmount) public payable {
        ...
        if (!users[msg.sender].levels[level].opened) {
            ...
            if (users[msg.sender].isPredefined) {
                headIndex[level]--;
                uint index = headIndex[level];
                uint maxLen = levelQueue[level].length - 1;
                while (index < maxLen) {
                    address addr = levelQueue[level][index];
