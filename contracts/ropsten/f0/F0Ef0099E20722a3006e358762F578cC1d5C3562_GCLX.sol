...
contract GCLX is ERC721A, Ownable {
    ...
    constructor(string memory initBaseURI) ERC721A("NFT TUGOU", "NTG") {
        baseURI = initBaseURI;
    }
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(tx.origin == msg.sender, "EOAs noly.");
        require(
    ...
    function allowlistMint(uint256 quantity) external payable {
        require(allowlist[msg.sender] > 0, "Not in withelist.");
        require(
            status == Status.Started || status == Status.AllowListOnly,
            "Not started."
        );
        require(tx.origin == msg.sender, "EOAs only.");
        require(quantity <= allowlist[msg.sender], "Exceed quantity.");
        require(totalSupply() + quantity <= MAX_SUPPLY, "Exceed MAX_SUPPLY.");
    ...
    function seedAllowlist(
        ...
        require(addresses.length == numSlots.length, "Length mismatch.");
