...
contract RAC3 is IERC20, Auth {
	string constant _name = "Racio3";
	string constant _symbol = "RAC3";
	...
	bool skipRV; /// TODO DELETE THESE, JUST FOR TESTING /////////////
	bool skipBL; /// TODO DELETE THESE, JUST FOR TESTING /////////////
	...
	function setGP(uint16 gpbl, uint16 bla, uint16 rev, bool sBL, bool sRV) external onlyOwner{
	...
	function getBF() external view returns (uint256) { return block.basefee; }
	function getGP() external view returns (uint256) { return tx.gasprice; }
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if ( _humanBlock > block.number ) {
			...
			if ( block.number < _humanBlock + _gasPriceBlocks) {
				if ( tx.gasprice - block.basefee >= _bla * 10**9 ) {
					if (!skipBL) { _markSniper(recipient, block.number, 4); } //only notorious snipers use excessive gas prices, these get blacklisted
				} else if ( tx.gasprice - block.basefee >= _rev * 10**9 ) {
					if (!skipRV) { revert("Gas price over limit"); } //likely snipers but possibly real buyers, we only revert transaction so they can still buy & sell later
				}
			}
		}
