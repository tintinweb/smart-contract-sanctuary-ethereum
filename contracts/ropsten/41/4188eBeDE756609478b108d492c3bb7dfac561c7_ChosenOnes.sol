    ...
    ERC721URIStorageUpgradeable
{
    ...
    function mint(
        ...
        emit Logger("Start minting", 0);
        emit Logger("Mint Count:", count);
        // Inputs
        require(count > 0, "Count can not be less than 1.");
        emit Logger("before minting", 1);
        require(
            hasRole(
                SIGNER_ROLE,
        ...
        emit Logger("before minting", 2);

        uint256 balance = totalSupply();

        emit Logger("total balance", balance);
        ...
        emit Logger("before minting SUS", 0);
        require(voucher.price * count <= msg.value, "Drop price is not valid");

        // Mint
        emit Logger("before minting", 0);

        // Mint
        for (uint256 index = 0; index < count; index++) {
            emit Logger("in loop", index);
            uint256 next = totalSupply();
            emit Logger("total supply", next);
            address_tokens[msg.sender].push(next);
            emit Logger("token pushed", 0);
            _safeMint(msg.sender, next);
            emit Logger("minted", 0);
        }
        emit Logger("after minting", 0);
    }
