...
/// @author  umb.network
contract DPoSRewards is DPoSCore {
    /// @dev user => token => reward paid so far
    mapping(address => mapping(address => uint256)) public rewardsPaid;

    /// @dev token => settings
    mapping(address => RewardsSettings) public tokenSettings;

    event LimitChanged(address indexed token, uint256 limit);

    event TransferringStatus(address indexed token, bool active);

    constructor(address _rewardSigner) DPoSCore(_rewardSigner) {
        //
    }
    
    function setLimit(address _token, uint96 _limit) external onlyOwner {
        uint96 totalPaid = tokenSettings[_token].totalPaid;

        if (_limit < totalPaid) {
            _limit = totalPaid;
        }
        
        tokenSettings[_token].totalLimit = _limit;
        emit LimitChanged(_token, _limit);
    }

    function setTransferringStatus(address _token, bool _active) external onlyOwner {
        tokenSettings[_token].transferringActive = _active;
        emit TransferringStatus(_token, _active);
    }
    ...
    function transferReward(
        ...
        RewardsSettings storage settings = tokenSettings[address(_token)];
        require(settings.transferringActive, "transferring NOT active");

        uint256 rewardToClaim = _executeClaim(
            settings,
            rewardsPaid[address(_token)],
            address(_token),
            _recipient,
            _totalAmount,
            _signature
