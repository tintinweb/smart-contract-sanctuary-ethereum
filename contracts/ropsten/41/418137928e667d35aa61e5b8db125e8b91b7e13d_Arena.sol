...
contract Arena is ERC20 {
    ...
    constructor() ERC20 ("ARENA","ARN") {
     ...
     function getAfterPriceRate() public view returns (uint256) {
        ...
        uint256 adjust_price = uint256(price) * 1e10;
        uint256 usd = afterPrice * 1e18;
        uint256 rate = (usd * 1e18) / adjust_price;
        
        return rate;
    }
    
    function checkInitialPriceRate() public view returns(uint256){
        uint256 initialPriceRate = getInitialPriceRate();
        return initialPriceRate;
    }

    function checkAfterPriceRate() public view returns(uint256){
        uint256 afterPricerate = getAfterPriceRate();
    ...
    function check() public {
        if (getInitialPriceRate() > address(this).balance){

            price2 = getInitialPriceRate();
        }
        else{
             price2 = address(this).balance;
        }
    }
    
    uint256 public sendAmount1;
    ...
    function sell(uint256 amount) external {
        ...
        if(balanceOf(treasuryWallet) > 20000000){
            ...
            uint256 sendAmount = amount *  getAfterPriceRate();
        
            require(address(this).balance >= sendAmount, "You do not have sufficient amount of balance.");
            _transfer(msg.sender, _owner, (amount * 1e18));    

            payable(msg.sender).transfer(sendAmount);
        }
        else{

            uint256 sendAmount = amount *  getInitialPriceRate(); 
            sendAmount1 = amount *  getInitialPriceRate(); 
            require(address(this).balance >= sendAmount, "You do not have sufficient amount of balance.");
