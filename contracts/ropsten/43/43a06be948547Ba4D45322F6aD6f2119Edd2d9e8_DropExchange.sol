...
contract DropExchange is AccessControl, ReentrancyGuard, EIP712, Pausable {
    ...
    bytes32 constant SELLORDER_TYPE_HASH = keccak256(
        "SellOrder(address collection,uint256 token,uint256 quantity,uint256 amount,address seller,address buyer,uint256 deadline,address feesRecipient,uint256 feesAmount)"
    );

    // fired every time a drop is redeemed
    event DropRedeemed(address collection, uint256 token, uint256 quantity, address buyer);
    ...
    // executes a drop buy authorized by a signing authority
    // re-entry guard is needed because we transfer ETH
    function executeOrder(
        // needs to be a valid order
        SellOrder memory order, 
        // needs to be a signature from an authority
        DropExchange.Signature memory signature
        ...
        // make sure the amount of eth sent is right
        require(msg.value == order.amount + order.feesAmount, "CB: Eth sent does not match order");
        ...
        if (order.quantity == 0) {
            ...
            erc1155Collection.safeTransferFrom(
                order.seller,
                order.buyer,
                order.token,
                order.quantity,
    ...
    // invalidates a sell order before it gets used
    function invalidate(SellOrder memory order) external onlyRole(MINTER_ROLE) returns(bool) {
        ...
        executedOrders[hashOrder(order)] = true;
