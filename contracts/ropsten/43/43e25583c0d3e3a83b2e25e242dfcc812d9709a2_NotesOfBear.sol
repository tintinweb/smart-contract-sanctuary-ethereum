...
contract ERC1155Upgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, IERC1155Upgradeable, IERC1155MetadataURIUpgradeable {
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(_uri).length > 0 ? string(abi.encodePacked(_uri, tokenId.toString(), postfix)) : "";
    }
    ...
    function _increaseSupply( uint256 tokenId, uint256 amount ) internal virtual {
        ...
        emit TransferSingle(msg.sender, address(0), address(this), tokenId, amount);
    }
...
contract NotesOfBear is ERC1155Upgradeable {
    ...
    mapping(uint => mapping(uint => bool)) public isClaimed;
    ...
    function initialize() public initializer {
      ...
      BearXContract  = BearX_interface(0xE22e1e620dffb03065CD77dB0162249c0c91bf01);
    ...
    function Claim(uint[] memory c_ids, uint[] memory h_ids) public checkSale() {
        require(dependent == 0 || balanceOf(msg.sender, dependent) > 0, errMsg );
        for(uint i; i<c_ids.length; i++){
            for(uint j; j<h_ids.length; j++){
                _claim(c_ids[i], h_ids[j]);
            }
        }
    }
    function _claim(uint CID, uint HID) internal checkSale() {
        require(exists[CID] == true, "ERC1155 query for nonexistent token");
        unchecked { require(HID <= range || (HID >= 1000000000000 && HID < 1000000000006), "You can't claim"); }
        require(isClaimed[CID][HID] == false, "This item is already claimed");
        require( BearXContract.ownerOf(HID) == msg.sender, "You Don't Own This Item");
        unchecked { require(1 <= balanceOf(address(this), CID), "MAX_REACHED"); }
    ...
    function StakeClaim (uint CID, address _address) public {
    ...
    function _stakeClaim(uint CID, uint HID) internal checkSale() {
        require(exists[CID] == true, "ERC1155 query for nonexistent token");
        unchecked { require(HID <= range || (HID >= 1000000000000 && HID < 1000000000006), "You can't claim"); }
        require(isClaimed[CID][HID] == false, "This item is already claimed");
        unchecked { require(1 <= balanceOf(address(this), CID), "MAX_REACHED"); }
    ...
    function walletOfOwner(uint256[] memory ids) public view returns(uint256[] memory) {
