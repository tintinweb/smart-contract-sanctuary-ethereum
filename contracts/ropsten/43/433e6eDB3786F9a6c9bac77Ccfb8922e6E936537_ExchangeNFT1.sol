...
contract ExchangeNFT1 is Ownable, TokenRecipient{
    ...
    function exchange(
        ...
        uint256 feeValue = uint_0;
        uint256 sellValue = uint_1;
        uint256 royaltyValue = uint_2;
        // uint256 totalValue =  (feeValue + sellValue + royaltyValue);

        if (keccak256(bytes(str0)) == keccak256(bytes("BUY"))) {
            exchangeToken = ERC20(contract1);
            // require(exchangeToken.balanceOf(payable(address(this))) >= totalValue, "not enough token balance");

            if (addr1 != address(0)) {
                require(exchangeToken.transfer(addr1, feeValue));
            }
            if (addr3 != address(0)) {
                require(exchangeToken.transfer(addr3, sellValue));
            }
            if (addr5 != address(0)) {
                require(exchangeToken.transfer(addr5, royaltyValue));
            }
        } else {
            // require(contracts[1] != address(0), "can not bid/offer for currency");
            if (addr1 != address(0)) {
                transferTokens(contract1, addr0, addr1, feeValue);
            }
            if (addr3 != address(0)) {
                transferTokens(contract1, addr2, addr3, sellValue);
            }
            if (addr5 != address(0)) {
                transferTokens(contract1, addr4, addr5, royaltyValue);
            }
        }
    }
}