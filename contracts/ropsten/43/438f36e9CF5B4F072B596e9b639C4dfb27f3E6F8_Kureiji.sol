...
contract Kureiji is ERC721Enum, Ownable, PaymentSplitter {
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 5555;
    uint256 public constant MAX_PRESALE_MINT = 2;
    uint256 public constant MAX_PUBLIC_SALE_PER_TX = 5;
    ...
    bytes32 private allowlist_merkleRoot;
    ...
    mapping(address => uint256) public allowAddressMintedBalance;
    ...
    constructor(string memory _hiddenURI, bytes32 _whitelistMerkleRoot, bytes32 _allowlistMerkleRoot) ERC721P("Kureiji", "KJ") PaymentSplitter(_splitterAddressList, _shareList) {
        setHiddenURI(_hiddenURI);
        whitelist_merkleRoot = _whitelistMerkleRoot;
        allowlist_merkleRoot= _allowlistMerkleRoot;
    }
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _proof) external payable mintCheck(_mintAmount) {
        ...
        require(senderMintedCount + _mintAmount <= MAX_PRESALE_MINT, "Total mints after tx exceeds max mints for this address");
        whitelistAddressMintedBalance[msg.sender] += _mintAmount;
        
        _doMint(msg.sender, _mintAmount);
    }

    function allowlistMint(uint256 _mintAmount, bytes32[] calldata _proof) external payable mintCheck(_mintAmount) {
        require(!presaleOnlyActive && !paused && !publicSaleOnlyActive, "Allowlist sale is not active");
        require(_verifyLeaf(_leaf(msg.sender), allowlist_merkleRoot, _proof), "Invalid allowlist proof");
        require(_mintAmount <= MAX_PUBLIC_SALE_PER_TX, "Mint amt greater than max per tx");

        uint256 senderMintedCount = allowAddressMintedBalance[msg.sender];
        require(senderMintedCount + _mintAmount <= MAX_PUBLIC_SALE_PER_WALLET, "Total mints after tx exceeds max mints for this address");
        allowAddressMintedBalance[msg.sender] += _mintAmount;
    ...
    function publicSaleOnly(bool _state) public onlyOwner {
