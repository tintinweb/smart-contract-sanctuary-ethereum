...
contract Countdown3D {
        ...
        returns(uint256 blockCap, uint256 claimed, uint256 pot, uint256 random, uint256 startTime, uint256 tickets)
    {
        ...
        return(round.blockCap, round.claimed, round.pot, round.random, round.startTime, round.tickets);
    }
...
/* === vvv === FOR TESTING - REMOVE BEFORE GOING LIVE vvv === */

    function resetRoundStartTime(uint256 roundId)
        public
    {
        rounds[roundId].startTime = (now - COOLDOWN);
    }

    // function setRandom(uint256 roundId, uint256 random)
    //     public
    // {
    //     rounds[roundId].random = random;
    // }
