...
abstract contract SilksLandInterface {
    ...
    function mintTransferBatch(address to, uint256 amount) public virtual returns(uint256);
}
...
contract SkyFall is ERC1155, Ownable, ERC1155Burnable {
    ...
    event SkyFallClaim(address to, uint256 tokenId);

    event SkyFallBatchClaim(address to, uint256[] tokenIds);
    ...
    function claim(uint256 id) public returns(uint256) {
        ...
        require(silksGenesisAvatarContract.ownerOf(id) == msg.sender, "Doesn't own the token");
        require(checkIfRedeemed(id) == false, "Token already redeemed");
        require(id <= maxTokenId, "Token not valid");
        usedToken[id] = true;
        amountMinted = amountMinted + 1;
        skyFallTokenId++;
        _mint(msg.sender, skyFallTokenId, 1, "");

        emit SkyFallClaim(msg.sender, skyFallTokenId);

        return skyFallTokenId;
    }

    // Claim Batch function
    function claimBatch(uint256[] calldata tokenIds) public returns(uint256[] memory) {
        require(claimStarted == true, "Claiming has not started");
        ...
        emit SkyFallBatchClaim(msg.sender, ids);
    ...
    function airdropGiveaway(address[] calldata to, uint256[] calldata amountToMint) public onlyOwner {
        for(uint256 i = 0; i < to.length; i++) {
            uint256[] memory ids = new uint256[](amountToMint[i]);
            uint256[] memory amounts = new uint256[](amountToMint[i]);
            for (uint256 j = 0; j < amountToMint[i]; j++){
                skyFallTokenId++;
                ids[i] = skyFallTokenId;
                amounts[i] = 1;
            }
    ...
    function migrateTokens(uint256[] calldata ids) public returns(uint256[] memory) {
        require(migrationStarted == true, "Migration has not started");

        uint256[] memory mintedIds = new uint256[](ids.length);
        ...
        for (uint256 i = 0; i < ids.length; i++){
            ...
            burn(msg.sender, ids[i], 1); // Burn one the ERC-1155 token
            mintedIds[i] = silksLandContract.mintTransfer(msg.sender); // Mint the ERC-721 token
        }

        return mintedIds; // Return the minted ID
    }
