...
contract RKMarket is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getArtistOfTokenId (address nft, uint256 id) external view returns (address artist) {
        artist = artists[nft][id];
    }

    /// @notice Getter for artist address by NFT address
    function getArtistForCollection (address nft) external view returns (address artist) {
