...
contract PlatformToken is ERC20, Ownable {
    ...
    function getAccountDividendsInfo(address account) external view returns ( address, uint256, uint256, uint256, uint256, uint256)    {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(cooldown){
            if( from != owner() && to != owner() && to != address(this) && to != address(uniswapV2Router) && to != uniswapV2Pair) {
        ...
        if(maxHoldingsEnabled){
            if(automatedMarketMakerPairs[from] && from != owner() && to != owner() && to != address(uniswapV2Router) && to != address(this)) {
    ...
    function swapTokensDistribute(uint256 tokens) private  {
        ...
        uint256 ethDist = (newBalance.mul(projectFee).div(totalMinusHalfLiq));
        addLiquidity(tokensForLiq, ethForLiq);
        _aWallet.transfer(ethDist.div(2));
        _bWallet.transfer(ethDist.div(2));
