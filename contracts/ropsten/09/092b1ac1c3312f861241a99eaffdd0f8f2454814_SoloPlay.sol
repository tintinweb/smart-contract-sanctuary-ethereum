...
// File: contracts\SoloPlay.sol

contract SoloPlay is IERC20, Ownable {
    ...
    uint256 private _swapThreshold = 5_000_000 * 10**9;
    ...
    event SwapThresholdModified(uint256 oldValue, uint256 newValue);
    ...
    constructor() {
        ...
        _liquidityWallet = payable(owner());
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                sender == address(this) &&
                    ...
                    ? 0 // ensure router will always be able to execute swaps (swapBack)
                    : amount,
    ...
    function swapBack() private lockTheSwap {
        uint256 swapThreshold = _swapThreshold;
    ...
    function _takeTaxes(uint256 tTaxes) private {
        ...
        if (tTaxes > 0) emit Transfer(_msgSender(), address(this), tTaxes);
    }
    ...
    function setReflectionFee(uint256 feeReflection) external onlyOwner {
        require(
            feeReflection.add(_feeTeam.add(_feeLiquidity)) < _maxFees,
            "fees to high"
        );
        emit ReflectionFeeModified(_feeReflection, feeReflection);
        _feeReflection = feeReflection;
    }
    function setFeeSettings(uint256 feeTeam, uint256 feeLiquidity)
        external
        onlyOwner
    {
        require(
            _feeReflection.add(feeTeam.add(feeLiquidity)) < _maxFees,
            "fees to high"
        );
        _feeTeam = feeTeam;
    ...
    function setWalletSettings(
        ...
        require(liquidityWallet != address(0), "liquidityWallet: zero address");
        // ensure to remove from excluded list
        if (_marketingWallet != owner())
            _isExcludedFromFee[_marketingWallet] = false;
        if (_devFundWallet != owner())
            _isExcludedFromFee[_devFundWallet] = false;
        if (_liquidityWallet != owner())
        ...
        _isExcludedFromFee[devFundWallet] = true;
        _isExcludedFromFee[liquidityWallet] = true;
    }
    function setSwapThreshold(uint256 swapThreshold) external onlyOwner {
        emit SwapThresholdModified(_swapThreshold, swapThreshold);
        _swapThreshold = swapThreshold;
    }
    function manualSwap() external onlyOwner {
        uint256 tmp = _swapThreshold;
        _swapThreshold = balanceOf(address(this)) >= tmp
            ? tmp
            : balanceOf(address(this));
        swapBack();
        _swapThreshold = tmp;
