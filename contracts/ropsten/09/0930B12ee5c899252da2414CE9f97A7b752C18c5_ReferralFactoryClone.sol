...
/**
 * @title Elixir Referral programa
 * @author Satoshis.games
 * @notice this contract manages the referral program
 * @dev this version only works whit ERC20 rewards tokens
 */
contract ElixirReferralMaster is
    Initializable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    // TODO: use to be immutable, make private with view accesors
    ...
    // Rewards Tiers example
    //  id     inicial 	    final 		rewardERC20     rewardERC721    rewardERC721Step
    //  1        1		    100		     2000               0               0
    //  2        101		200 		 1500	            1               30
    //  3        201		 0 (max)  	 1000	            1               50
    ...
    event rewardsCclaimed(address _user);

     /**
     * @notice Clone initializer function
     * @param _publisherAddress Publisher address (reward token owner)
    ...
    function initialize(
        ...
        gameId = _gameId;
        require(_publisherAddress != address(0), "Zero token address");
   ...
   /**
     ...
     */
    function updateProgramEndTime(uint256 _newEndTime) external  {
     ...
     */
    function updateMxRewardAmount(uint256 _newMaxReward) external  {
     ...
     */
    function updateMinClaimAmount(uint256 _newMinClaim) external  {
     ...
     */
    function setNFTRewards(uint256[] memory _nftRewards) public  {
        ...
        returns (bool)
    {
        ...
        require(
            _referral != publisherAddress && _referee != publisherAddress,
        ...
        uint256 totalRewardAsigned = totalRewardAmount +
            rewardERC20 +
            refereeRewAmountERC20;
        require(
            totalRewardAsigned <= maxRewardAmount,
            "addReferral: The maximum number of rewards to assign has been reached"
        );
        totalRewardAmount = totalRewardAsigned;
     ...
     * @dev for now only for ERC20 reward tokens
     */
    function claimRewards() external nonReentrant whenNotPaused returns (bool) {
        ...
        emit rewardsCclaimed(msg.sender);
        return true;
    }
    /**
     * @dev Triggers stopped state.
...
import "./ElixirReferralMaster.sol";
...
contract ReferralFactoryClone is Ownable {
     ...
     */
    constructor() {
        referralImplementation = address(new ElixirReferralMaster());
    }
    ...
    function createReferral(
        ...
        ElixirReferralMaster(clone).initialize(
            _publisherAddress,
            _gameId,
            _rewardTokenERC20,
            _rewardTokenERC721,
