...
contract PurchasaETHEscrow is Ownable {
    ...
    function lockETHToken(address reciever,string memory orderId,string memory currency,string memory categoryName) payable public {
        require(keccak256(abi.encodePacked(categoryList[categoryName].name)) == keccak256(abi.encodePacked(categoryName)), "Category not exists");
        require(msg.sender.balance >= msg.value, "Insufficient Balance");
        uint256 currentTime = block.timestamp;
        Escrow storage escrowOrderList = escrowList[orderId];
        escrowOrderList.totalLocked = msg.value;
        escrowOrderList.reciever = reciever;
        escrowOrderList.sender = msg.sender;
        escrowOrderList.start = currentTime;
        escrowOrderList.end = currentTime + categoryList[categoryName].escrowtime;
        escrowOrderList.orderId = orderId;
        escrowOrderList.totalCommission = ((msg.value / 100) * categoryList[categoryName].commission);
        escrowOrderList.isDispute = false;
        escrowOrderList.currency = currency;
        escrowOrderList.itemCategory = categoryList[categoryName];
        Escrow memory orderData = Escrow(msg.value,reciever,msg.sender,currentTime,currentTime + categoryList[categoryName].escrowtime,orderId,((msg.value / 100) * categoryList[categoryName].commission),false,currency,categoryList[categoryName]);
    ...
    function lockERCToken(address tokenAddress,uint256 amount, address reciever,string memory orderId,string memory currency,string memory categoryName) public {
        require(keccak256(abi.encodePacked(categoryList[categoryName].name)) == keccak256(abi.encodePacked(categoryName)), "Category not exists");
        ...
        uint256 currentTime = block.timestamp;
        Escrow storage escrowOrderList = escrowList[orderId];
        escrowOrderList.totalLocked = amount;
        escrowOrderList.reciever = reciever;
        escrowOrderList.sender = msg.sender;
        escrowOrderList.start = currentTime;
        escrowOrderList.end = currentTime + categoryList[categoryName].escrowtime;
        escrowOrderList.orderId = orderId;
        escrowOrderList.totalCommission = ((amount / 100) * tokens[tokenAddress].commission);
        escrowOrderList.isDispute = false;
        escrowOrderList.currency = currency;
        escrowOrderList.itemCategory = categoryList[categoryName];
        Escrow memory orderData = Escrow(amount,reciever,msg.sender,currentTime,currentTime + categoryList[categoryName].escrowtime,orderId,((amount / 100) * tokens[tokenAddress].commission),false,currency,categoryList[categoryName]);
        userOrders.push(orderData);
    }
