...
contract FeesDistribution {
    ...
    function _getFeeAmount(address user, uint256 currentAmount) public view returns(uint256) {
        return (currentAmount - _lastKnownAmount[user]) * _comissionPercentage[user] / totalPercentage;
    }
...
contract ContractRoles is AccessControlUpgradeable {
    ...
    function grantRole(address user, bytes32 role) external _onlyOwner {
...
contract ERC721Special is Initializable, ERC721, ContractRoles, Whitelist, FeesDistribution {
    ...
    uint256 internal whitelistMinted;
    uint256 internal whitelistNFTLimit;
    ...
    /*************************************************************************** */
    //                                  Mint                                     //
    function mint() external payable reentrancyGuard mintingEnabled whitelistDisabled returns (uint256) {
        require(
            MINTING_LIMIT + whitelistMinted - whitelistNFTLimit - totalMinted > 0,
            "All available NFTs minted"
        );
        return _mint(msg.sender, _defaultMintingPrice);
    }

    function mintWhitelist(bytes32[] calldata proof) external payable reentrancyGuard mintingEnabled returns (uint256) {
        require(
        ...
        whitelistMintedByUsers[msg.sender] += 1;
        whitelistMinted += 1;

        return _mint(msg.sender, _whitelistMintingPrice);
    }
    ...
    function airdropMintTo(
        ...
        returns(uint256[AIRDROP_LIMIT] memory) 
    {
        require(
            MINTING_LIMIT + whitelistMinted - whitelistNFTLimit - totalMinted > 0,
        ...
        uint256[AIRDROP_LIMIT] memory mintedIndexes;

        for (uint itemsIndex = 0; itemsIndex < amountToMint; itemsIndex++) {
            mintedIndexes[itemsIndex] = _mint(_tokenReceiver, 0);
        }

        return mintedIndexes;
    }

    function _mint(address _to, uint256 _mintCost) internal notZeroAddress(_to) returns (uint256 _mintedTokenId) {
        if (_mintCost > 0) {
            require(
                msg.value >= _mintCost, 
            ...
            if (msg.value > _mintCost) {
                payable(_to).transfer(msg.value - _mintCost);
            }
            totalFees += _mintCost;
        }
    ...
    function setWhitelistParams(bytes32 whitelistRoot, uint256 reservedTokens) external _hasRole(ADMIN_ROLE) {
