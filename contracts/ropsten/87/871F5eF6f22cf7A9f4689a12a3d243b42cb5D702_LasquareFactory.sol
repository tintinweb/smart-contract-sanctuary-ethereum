...
pragma abicoder v2;
import "./LasquareOpenseaStores.sol";
import "@openzeppelin/contracts/math/SafeMath.sol";

contract LasquareFactory is Ownable {
    ...
    constructor( address _registry, address _exchange) {
    ...
    function setProxyRegistry(address _registry) external onlyOwner {
        require(_registry != _proxyRegistryAddress, "LasquareFactory::SAME REGISTRY ADDRESS");
    ...
    function setNewExchange(address _exchange) external onlyOwner {
        require(_exchange != _exchangeAddress, "LasquareFactory::SAME EXCHANGE ADDRESS");
    ...
    function newCollection(
        ...
        LasquareOpenseaStore NewCollection = new LasquareOpenseaStore(_name, _symbol,_proxyRegistryAddress,_tokenURI, _exchangeAddress);
        
        collections[sender].push(address(NewCollection));
        totalCollections = totalCollections.add(1);

        emit CollectionDeployed(address(NewCollection), sender);
...
// File: contracts/LasquareOpenseaStore.sol
...
/**
 * @title LasquareOpenseaStore
 * OpenSea shared asset contract - A contract for easily creating custom assets on OpenSea
 */
contract LasquareOpenseaStore is AssetContract, ReentrancyGuard {
    // Migration contract address
    LasquareOpenseaStore public migrationTarget;
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            ...
            "LasquareOpenseaStore#creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
    ...
    modifier onlyFullTokenOwner(uint256 _id) {
        require(
            ...
            "LasquareOpenseaStore#onlyFullTokenOwner: ONLY_FULL_TOKEN_OWNER_ALLOWED"
        );
        _;
    }
    ...
    constructor(
        ...
        migrationTarget = LasquareOpenseaStore(_migrationAddress);
    }
    ...
    function disableMigrate() public onlyOwnerOrProxy {
        migrationTarget = LasquareOpenseaStore(address(0));
    }
    ...
    function migrate(Ownership[] memory _ownerships) public onlyOwnerOrProxy {
        LasquareOpenseaStore _migrationTarget = migrationTarget;
        require(
            _migrationTarget != LasquareOpenseaStore(address(0)),
            "LasquareOpenseaStore#migrate: MIGRATE_DISABLED"
        );
        ...
        for (uint256 i = 0; i < _ownerships.length; ++i) {
            ...
            require(
                owner != address(0),
                "LasquareOpenseaStore#migrate: ZERO_ADDRESS_NOT_ALLOWED"
            );
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                ...
                "LasquareOpenseaStore#_batchMint: ONLY_CREATOR_ALLOWED"
            );
        }
        _batchMint(_to, _ids, _quantities, _data);
    }
    ...
    function setCreator(uint256 _id, address _to) public creatorOnly(_id) {
        require(
            _to != address(0),
            "LasquareOpenseaStore#setCreator: INVALID_ADDRESS."
        );
    ...
    function _requireMintable(address _address, uint256 _id) internal view {
        require(
            ...
            "LasquareOpenseaStore#_requireMintable: ONLY_CREATOR_ALLOWED"
        );
    }
