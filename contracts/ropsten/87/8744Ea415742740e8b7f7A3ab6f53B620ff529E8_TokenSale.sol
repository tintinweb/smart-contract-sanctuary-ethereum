...
contract ERC20 is IERC20, Ownable{
    ...
    constructor() {
        ...
        _symbol = "PIA";
        decimals = 18;
        lockTime = 20 + block.timestamp; //2629743 = one month ----- for practice purpose 2 minutes are keeping lockTime
    ...
    modifier isTimeLock(){
        ...
        require(block.timestamp < (lockTime + 365 days), "ERC20: Sales Ended");
        _;
    } 
    ...
    function buyTokens(address recipient, uint tokens)internal virtual isTimeLock returns(bool){
    ...
    function mint(address account, uint tokens)internal virtual isTimeLock returns(bool){
        require(account != address(0) && tokens > 0,"unauthorized call");
        _totalSupply += tokens;
...
contract TokenSale is ERC20{
    ...
    constructor(){
        maxSupply = 50000000 * 10**decimals;
        rateBuy = 200000 ;                     // 1 ether = 100000 tokens
    }
    
    modifier isAuthorizer () {
       require(msg.sender == owner() ,"caller is not authorizer"); 
        _;
    }
    ...
    function minting(address account, uint _token)public  returns(bool){
        require(account != address(0),"account should not be zeor address");
        require(totalSupply() + _token <= maxSupplyCap(),"TokenCaped: Cap exceeded");
        mint(account,_token);
        return true;
    }
    ...
    function getBuyingRate() public view returns(uint){
        return rateBuy;
    }
    ...
    function buyToken(address account) public payable returns(uint FundsRais){
        ...
        require(msg.value > 0,"minimum purchase must not be Zero ether");
         require(totalSupply() + tokensAllocation <= maxSupplyCap(),"TokenCaped: Cap exceeded");
