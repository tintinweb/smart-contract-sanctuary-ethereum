...
contract AngelTokenCrowdsale is Pausable, Crowdsale, Ownable {
    ...
    uint256 public _rate; //the rate for which 1 ether = rate
        ...
        returns (uint256)
    {
        ...
        if (discountRates.length > 0) {
            ...
            tokens =
                _rate.mul(weiAmount).div(10**18) +
                (_rate.mul(weiAmount)).mul(discount).div(10000).div(10**18);
        } else {
            tokens = _rate.mul(weiAmount).div(10**18);
        }
