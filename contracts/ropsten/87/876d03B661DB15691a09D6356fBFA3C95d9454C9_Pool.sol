 ...
 * @title Pool contract
 * @author None
 * @notice Main point of interaction with an protocol's market
 * - Users can:
 *   # Supply
 *   # Withdraw
 *   # Borrow
...
contract Pool is VersionedInitializable, PoolStorage, IPool {
   ...
   **/
    ...
    function _onlyM1Pool() internal view virtual {
        require(
            ADDRESSES_PROVIDER.getM1Pool() == msg.sender,
            Errors.CALLER_NOT_M1POOL
        );
    }
   ...
   **/
    ...
    function withdraw(
        ...
        SupplyLogic.executeWithdraw(
            ...
            DataTypes.ExecuteWithdrawParams({
                ...
                amount : amount,
                to : to,
                userParams: _getM0UserAccountDataParamCache(msg.sender)
            })
        );
    }
    ...
    function _borrow(
        ...
        address onBehalfOf,
        address user,
        DataTypes.BorrowType borrowType,
        ...
        BorrowLogic.executeBorrow(
            ...
            DataTypes.ExecuteBorrowParams({
                ...
                m1BorrowParamsCache : m1BorrowParamsCache,
                m1Pool: ADDRESSES_PROVIDER.getM1Pool(),
                orderId: m1BorrowParamsCache.orderId
            })
        );
    }
    /// @inheritdoc IPool
    function borrow(
        ...
        DataTypes.M1BorrowParams memory m1BorrowParamsCache;
        m1BorrowParamsCache.orderId = uint256(DataTypes.M0OrderId.ZERO);

        _borrow(asset,amount,interestRateMode,referralCode,onBehalfOf,msg.sender,DataTypes.BorrowType.M0,m1BorrowParamsCache);
    }
    ...
    function repay(
        ...
        return _repay(asset,amount,interestRateMode,onBehalfOf,false,DataTypes.BorrowType.M0,msg.sender,uint256(DataTypes.M0OrderId.ZERO));
    ...
    function repayWithATokens(
        ...
        return _repay(asset,amount,interestRateMode,msg.sender,true,DataTypes.BorrowType.M0,msg.sender,uint256(DataTypes.M0OrderId.ZERO));
    }
    ...
    function swapBorrowRateMode(address asset, uint256 interestRateMode) public virtual override {
        BorrowLogic.executeSwapBorrowRateMode(
            ...
            uint256(DataTypes.BorrowType.M0),
            uint256(DataTypes.M0OrderId.ZERO)
        );
    }

    /// @inheritdoc IPool
    function rebalanceStableBorrowRate(address asset, address user) public virtual override {
        BorrowLogic.executeRebalanceStableBorrowRate(_reserves[asset], asset, user,uint256(DataTypes.BorrowType.M0),uint256(DataTypes.M0OrderId.ZERO));
    }
    ...
    override
    {
        SupplyLogic.executeUseReserveAsCollateral(
            ...
            asset,
            useAsCollateral,
            _getM0UserAccountDataParamCache(msg.sender)
        );
    }
    ...
    function liquidationCall(
        ...
        LiquidationLogic.executeLiquidationCall(
            ...
            DataTypes.ExecuteLiquidationCallParams({
                ...
                priceOracleSentinel : ADDRESSES_PROVIDER.getPriceOracleSentinel(),
                m1Pool: ADDRESSES_PROVIDER.getM1Pool(),
                borrowType:uint256(DataTypes.BorrowType.M0),
                orderId:uint256(DataTypes.M0OrderId.ZERO)
            })
        );
    }
    ...
    )
    {
        ...
        PoolLogic.executeGetUserAccountData(
            ...
            DataTypes.CalculateUserAccountDataParams({
                ...
                userParams: _getM0UserAccountDataParamCache(user)
            })
        );
    }

    function _getM0UserAccountDataParamCache(address user) internal view returns(DataTypes.UserAccountDataParam memory){
        return DataTypes.UserAccountDataParam({
            user: user,
            userEModeCategory:_usersEModeCategory[user],
            reservesCount: _reservesCount,
            oracle:ADDRESSES_PROVIDER.getPriceOracle(),
    ...
    function borrowM1(
        ...
        address user,
        DataTypes.M1BorrowParams memory m1BorrowParamsCache
    ) public virtual override onlyM1Pool{
        _borrow(asset,amount,interestRateMode,referralCode,onBehalfOf,user,DataTypes.BorrowType.M1,m1BorrowParamsCache);
    }


//    function repayM1Test(
//        address asset,
//        uint256 amount,
//        uint256 interestRateMode,
//        address onBehalfOf,
...
//        return _repay(asset,amount,interestRateMode,onBehalfOf,false,DataTypes.BorrowType.M1,user,orderId);
//    }
    function repayM1(
        address asset,
        uint256 amount,
        ...
        address user,
        uint256 orderId
    ) public virtual override  onlyM1Pool returns (uint256){
        return _repay(asset,amount,interestRateMode,onBehalfOf,false,DataTypes.BorrowType.M1,user,orderId);
    }
    ...
    function liquidationCallM1(
        ...
        LiquidationLogic.executeLiquidationCallM1(
            _reserves,
            debtAsset,
            debtUser,
            liquidator,
      ...
      if (
        ...
        BorrowLogic.executeBorrow(
          ...
          DataTypes.ExecuteBorrowParams({
            ...
            borrowType: DataTypes.BorrowType.M0,
            m1BorrowParamsCache: m1BorrowParamsCache,
            m1Pool: params.m1Pool,
            orderId: uint256(DataTypes.M0OrderId.ZERO)
          })
...
library BorrowLogic {
  ...
  event Borrow(
    ...
    uint256 borrowRate,
    uint16 indexed referralCode,
    DataTypes.BorrowType borrowTye
  );
  event Repay(
    address indexed reserve,
  ...
  event RebalanceStableBorrowRate(address indexed reserve, address indexed user,uint256 borrowType,uint256 orderId);
  event SwapBorrowRateMode(
    address indexed reserve,
    address indexed user,
    DataTypes.InterestRateMode interestRateMode,
    uint256 borrowType,uint256 orderId
  );
  event IsolationModeTotalDebtUpdated(address indexed asset, uint256 totalDebt);

  /**
   * @notice Implements the borrow feature. Borrowing allows users that provided collateral to draw liquidity from the
   * protocol proportionally to their collateralization power. For isolated positions, it also increases the
   * isolated debt.
  ...
  function executeBorrow(
    ...
    ValidationLogic.validateBorrow(
      ...
      DataTypes.ValidateBorrowParams({
        ...
        borrowType:params.borrowType,
        m1BorrowParamsCache: params.m1BorrowParamsCache,
        m1Pool: params.m1Pool
      })
    );
    ...
    if (params.interestRateMode == DataTypes.InterestRateMode.STABLE) {
      ...
      (
        ...
        params.amount,
        currentStableRate,
        uint256(params.borrowType),
        params.orderId
      );
    } else {
      (isFirstBorrowing, reserveCache.nextScaledVariableDebt) = IVariableDebtToken(
        reserveCache.variableDebtTokenAddress
      ).mint(params.user, params.onBehalfOf, params.amount, reserveCache.nextVariableBorrowIndex,uint256(params.borrowType),params.orderId);
    }
  ...
  function executeRepay(
    ...
    // Allows a user to repay with aTokens without leaving dust from interest. (only M0)
    if (params.useATokens && params.amount == type(uint256).max
        && params.borrowType== DataTypes.BorrowType.M0) {
      params.amount = IAToken(reserveCache.aTokenAddress).balanceOf(params.user);
    }
    ...
    if (params.interestRateMode == DataTypes.InterestRateMode.STABLE) {
      ...
      ).burn(params.onBehalfOf, paybackAmount,uint256(params.borrowType),params.orderId);
    } else {
      reserveCache.nextScaledVariableDebt = IVariableDebtToken(
        reserveCache.variableDebtTokenAddress
      ).burn(params.onBehalfOf, paybackAmount, reserveCache.nextVariableBorrowIndex,uint256(params.borrowType),params.orderId);
    }
    ...
    if (params.useATokens) {
      ...
      IAToken(reserveCache.aTokenAddress).handleRepayment(params.user, paybackAmount);
    }

    emit Repay(params.asset, params.onBehalfOf, params.user, paybackAmount, params.useATokens,params.borrowType);
  ...
  function executeRebalanceStableBorrowRate(
    ...
    uint256 stableDebt = IDebtERC20(address(stableDebtToken)).balanceOf(user,borrowType,orderId);

    stableDebtToken.burn(user, stableDebt,borrowType,orderId);

    (, reserveCache.nextTotalStableDebt, reserveCache.nextAvgStableBorrowRate) = stableDebtToken
      .mint(user, user, stableDebt, reserve.currentStableBorrowRate,borrowType,orderId);

    reserve.updateInterestRates(reserveCache, asset, 0, 0);

    emit RebalanceStableBorrowRate(asset, user,borrowType,orderId);
  }
  ...
  function executeSwapBorrowRateMode(
    ...
    if (interestRateMode == DataTypes.InterestRateMode.STABLE) {
      (reserveCache.nextTotalStableDebt, reserveCache.nextAvgStableBorrowRate) = IStableDebtToken(
        reserveCache.stableDebtTokenAddress
      ).burn(msg.sender, stableDebt,borrowType,orderId);

      (, reserveCache.nextScaledVariableDebt) = IVariableDebtToken(
        reserveCache.variableDebtTokenAddress
      ).mint(msg.sender, msg.sender, stableDebt, reserveCache.nextVariableBorrowIndex,borrowType,orderId);
    } else {
      reserveCache.nextScaledVariableDebt = IVariableDebtToken(
        reserveCache.variableDebtTokenAddress
      ).burn(msg.sender, variableDebt, reserveCache.nextVariableBorrowIndex,borrowType,orderId);

      (, reserveCache.nextTotalStableDebt, reserveCache.nextAvgStableBorrowRate) = IStableDebtToken(
        reserveCache.stableDebtTokenAddress
      ).mint(msg.sender, msg.sender, variableDebt, reserve.currentStableBorrowRate,borrowType,orderId);
    }

    reserve.updateInterestRates(reserveCache, asset, 0, 0);

    emit SwapBorrowRateMode(asset, msg.sender, interestRateMode,borrowType,orderId);
  }
}
...
library LiquidationLogic {
  ...
  function executeLiquidationCallM1(
    mapping(address => DataTypes.ReserveData) storage reservesData,
    address debtAsset,
    address debtUser,
    address liquidator,
    ...
    if(DataTypes.InterestRateMode.STABLE == DataTypes.InterestRateMode(interestRateMode)){
      IStableDebtToken(debtReserveCache.stableDebtTokenAddress).burn(
        debtUser,debtToCover,uint256(DataTypes.BorrowType.M1),orderId
      );
    }else{
      IVariableDebtToken(
        debtReserveCache.variableDebtTokenAddress
      ).burn(debtUser, debtToCover, debtReserveCache.nextVariableBorrowIndex,uint256(DataTypes.BorrowType.M1),orderId);
    }
    ...
    emit LiquidationCallM1(debtAsset,debtUser,liquidator,debtToCover,orderId,interestRateMode);
    ...
    if (vars.userVariableDebt >= vars.actualDebtToLiquidate) {
      vars.debtReserveCache.nextScaledVariableDebt = IVariableDebtToken(
        vars.debtReserveCache.variableDebtTokenAddress
          ...
          params.user,
          vars.actualDebtToLiquidate,
          vars.debtReserveCache.nextVariableBorrowIndex,params.borrowType,params.orderId
        );
    } else {
      ...
      if (vars.userVariableDebt != 0) {
        vars.debtReserveCache.nextScaledVariableDebt = IVariableDebtToken(
          vars.debtReserveCache.variableDebtTokenAddress
        ).burn(params.user, vars.userVariableDebt, vars.debtReserveCache.nextVariableBorrowIndex,params.borrowType,params.orderId);
      }
      (
        ...
        params.user,
        vars.actualDebtToLiquidate - vars.userVariableDebt,params.borrowType,params.orderId
      );
    }
  }
...
library Helpers {
   ...
   **/
  function getUserCurrentDebt(address user, DataTypes.ReserveCache memory reserveCache,uint256 borrowType,uint256 orderId)
    internal
    view
    returns (uint256, uint256)
  {
    return (
      IDebtERC20(reserveCache.stableDebtTokenAddress).balanceOf(user,borrowType,orderId),
      IDebtERC20(reserveCache.variableDebtTokenAddress).balanceOf(user,borrowType,orderId)
    );
  }
}
