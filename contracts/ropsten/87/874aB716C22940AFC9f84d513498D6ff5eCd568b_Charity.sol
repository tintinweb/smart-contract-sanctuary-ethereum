...
contract Charity is ERC1155, Ownable, ERC1155Receiver, ICharity,
    ...
    */
    ...
    function getSplitterTokenBalances(uint256[] memory ids, address _token)
        public
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            batchBalances[i] = tokenContract.balanceOf(splitters[creators[ids[i]]]);
        }
  ...
  /**
    ...
    */
    function create(
        ...
        emit URI(_uri, _id);
        _mint(address(this), _id, _initialSupply, "0x0");
