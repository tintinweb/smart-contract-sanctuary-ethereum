...
contract CarbonCreditBundleTokenFactory is AbstractTokenFactory {

  using ClonesUpgradeable for address;

  /// @param implementationContract_ the contract that is used a implementation base for new tokens
  constructor (CarbonCreditBundleToken implementationContract_, address owner_) {
  ...
  /// @param owner_ the owner of the bundle token, eligible for fees and finalize retirement
  /// @param feeDivisor_ the fee divisor that should be taken upon deconstructing (withdraw)
  /// @return the address of the newly created token
  function createCarbonCreditBundleToken(
      string memory name_, string memory symbol_, uint16 vintage_,
      CarbonCreditToken[] memory tokens_, address owner_, uint256 feeDivisor_
  ) onlyOwner external returns (address) {
    CarbonCreditBundleToken token = CarbonCreditBundleToken(implementationContract.clone());
    token.initialize(name_, symbol_, vintage_, tokens_, owner_, feeDivisor_);
