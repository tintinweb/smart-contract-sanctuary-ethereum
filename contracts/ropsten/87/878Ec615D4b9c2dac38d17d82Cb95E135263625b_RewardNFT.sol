...
contract RewardNFT is ERC721{
    ...
    mapping(address => mapping(uint => bool)) internal _pass;
    ...
    function claimPass(address user) internal{
        _pass[user][checkPass(user)] = true;
    }
    ...
    function mint() public {
        require(checkPass(msg.sender) > 1 && _pass[msg.sender][checkPass(msg.sender)] != true, "you don't have Pass");
        _safeMint(msg.sender, (tokenID++));
        tokenID++;
        remaining - tokenID;
        _pass[msg.sender][Ticket.checkPass(msg.sender)] = true;
    }
    function checkPass(address user) view public returns (uint) {
         return Ticket.checkPass(user);
    }
}
