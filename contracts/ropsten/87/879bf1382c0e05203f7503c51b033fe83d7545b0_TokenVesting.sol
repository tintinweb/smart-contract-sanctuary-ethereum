...
contract TokenVesting is Ownable {
  ...
  function vestedAmount(ERC20Basic token) public view returns (uint256) {
    ...
    if( _now < listed_exchange.add(duration) ){
      ...
      //require(_now >= listed_exchange.add(duration));//当前时间必须大于上交易所时间+一个周期时间
      uint256 need_release_count = (_now.sub(listed_exchange)).div(duration);//当前时间减去上 交易所时间 再除以周期 得到当前可以释放的次数
      uint256 rate_count = (need_release_count.mul(duration_rate)).add(first_release_rate);//全部可以释放的比例
      if(rate_count>100){
        rate_count = 100;
      }
