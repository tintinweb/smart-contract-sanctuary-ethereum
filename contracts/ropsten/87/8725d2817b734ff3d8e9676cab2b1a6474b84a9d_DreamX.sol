...
contract DreamX {
    ...
    function _calculate_rewards(uint id, address addy) public view returns (uint) {
        ...
        uint updated_reward_rate = reward_rate + token_pool[farming_unit[addy][id].token].lock_multiplier[farming_unit[addy][id].locked_time];
     ...
     function get_reward_rate_timed(uint time) public view returns (uint) {
         uint reward_timed = ((reward_rate*1000) + ((reward_rate*1000)*(lock_multiplier[Dream][time])))/1000;
         return reward_timed;
     }
