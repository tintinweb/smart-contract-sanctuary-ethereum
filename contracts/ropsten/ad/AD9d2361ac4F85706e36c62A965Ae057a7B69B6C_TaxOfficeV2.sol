...
contract TaxOfficeV2 is Operator {
    ...
    address public chicken = address(0x6c021Ae822BEa943b2E66552bDe1D2696a53fbB7);
    ...
    function _excludeAddressFromTax(address _address) private returns (bool) {
        if (!ITaxable(chicken).isAddressExcluded(_address)) {
    ...
    function taxRate() external view returns (uint256) {
        return ITaxable(chicken).taxRate();
    }
    ...
    {
        require(amtChicken != 0 && amtToken != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);

        IERC20(chicken).transferFrom(msg.sender, address(this), amtChicken);
        IERC20(token).transferFrom(msg.sender, address(this), amtToken);
        _approveTokenIfNeeded(chicken, uniRouter);
        _approveTokenIfNeeded(token, uniRouter);

        _includeAddressInTax(msg.sender);

        uint256 resultAmtChicken;
        uint256 resultAmtToken;
        uint256 liquidity;
        (resultAmtChicken, resultAmtToken, liquidity) = IUniswapV2Router(uniRouter).addLiquidity(
            chicken,
            token,
            amtChicken,
            amtToken,
        ...
        if(amtChicken.sub(resultAmtChicken) > 0) {
            IERC20(chicken).transfer(msg.sender, amtChicken.sub(resultAmtChicken));
        }
        if(amtToken.sub(resultAmtToken) > 0) {
            IERC20(token).transfer(msg.sender, amtToken.sub(resultAmtToken));
        }
        return (resultAmtChicken, resultAmtToken, liquidity);
    }
    ...
    {
        require(amtChicken != 0 && msg.value != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);
        ...
        uint256 liquidity;
        (resultAmtChicken, resultAmtFtm, liquidity) = IUniswapV2Router(uniRouter).addLiquidityETH{value: msg.value}(
            chicken,
            amtChicken,
            amtChickenMin,
            amtFtmMin,
        ...
        return (resultAmtChicken, resultAmtFtm, liquidity);
    }
    ...
    function taxFreeTransferFrom(
        ...
        IERC20(chicken).transferFrom(_sender, _recipient, _amt);
