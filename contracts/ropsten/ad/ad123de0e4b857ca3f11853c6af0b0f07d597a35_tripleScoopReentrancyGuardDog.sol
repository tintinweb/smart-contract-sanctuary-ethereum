 ...
 */
contract tripleScoopReentrancyGuardDog is IBonusRewards, Ownable, ReentrancyGuard {
    ...
    modifier notPaused() {
        require(!paused, "tripleScoopReentrancyGuardDog: paused");
        _;
    }
    ...
    function addBonus(
        ...
        require(
            ...
            "tripleScoopReentrancyGuardDog: not authorized caller"
        );
        require(
            _startTime >= block.timestamp,
            "tripleScoopReentrancyGuardDog: startTime in the past"
        );

        // make sure the pool is in the right state (exist with no active bonus at the moment) to add new bonus tokens
        Pool memory pool = pools[_lpToken];
        require(pool.lastUpdatedAt > 0, "tripleScoopReentrancyGuardDog: pool does not exist");
        ...
        for (uint256 i = 0; i < bonuses.length; i++) {
            if (bonuses[i].bonusTokenAddr == _bonusTokenAddr) {
                ...
                require(
                    ...
                    "tripleScoopReentrancyGuardDog: last bonus period hasn't ended"
                );
                require(
                    bonuses[i].remBonus == 0,
                    "tripleScoopReentrancyGuardDog: last bonus not all claimed"
                );
            }
        }
    ...
    function extendBonus(
        ...
        require(
            ...
            "tripleScoopReentrancyGuardDog: bonus and id dont match"
        );
        require(
            bonus.endTime > block.timestamp,
            "tripleScoopReentrancyGuardDog: bonus program ended, please start a new one"
        );
    ...
    function addPoolsAndAllowBonus(
        ...
        for (uint256 i = 0; i < _lpTokens.length; i++) {
            ...
            require(
                ...
                "tripleScoopReentrancyGuardDog: lptoken decimals > 18"
            );
    ...
    function collectDust(
        ...
        require(
            ...
            "tripleScoopReentrancyGuardDog: lpToken, not allowed"
        );
        ...
        if (_token == address(0)) {
            ...
            if (bonusTokenAddrMap[_token] == 1) {
                ...
                require(
                    ...
                    "tripleScoopReentrancyGuardDog: wrong pool"
                );
                require(
                    bonus.endTime + WEEK < block.timestamp,
                    "tripleScoopReentrancyGuardDog: not ready"
                );
    ...
    function setPaused(bool _paused) external override {
        require(
            ...
            "tripleScoopReentrancyGuardDog: caller not responder"
        );
        paused = _paused;
    }
