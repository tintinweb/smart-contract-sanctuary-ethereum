...
contract ZLP is ERC721, Ownable {
    ...
    function setArtist(address _artist, bool _exist) external onlyOwner {
        if (_exist == true && artist[_artist] != true) {
            artist[_artist] = _exist;
            artists.push(_artist);
        }
        
        if (_exist == false && artist[_artist] == true) {
            delete artist[_artist];
