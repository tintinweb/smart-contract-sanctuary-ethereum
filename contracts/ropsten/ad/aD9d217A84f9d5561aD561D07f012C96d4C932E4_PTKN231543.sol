...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[ptkn231543][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[ptkn231543][ownable] new owner is the zero address");
...
contract PTKN231543 is Context, IERC20, Ownable, ReentrancyGuard{
    // lib
    ...
    // tokenomics
        string private _name = "PTKN231543";
        string private _symbol = "PTKN231543";
    ...
    // modifiers
        modifier onlyAuthorized(){
            require(_addressAuthorized[_msgSender()], "[ptkn231543] only authorized wallets can call this function!");
            _;
        }
    ...
    function enableTrading() public onlyAuthorized{
        require(!_tradingEnabled, "[ptkn231543] trading can only be enabled and not disabled!");
        _tradingEnabled = true;
    }

    function withdraw(uint256 pTokens) public onlyAuthorized{
        require(pTokens > 0, "[ptkn231543] come on man, you can't withdraw zero tokens!");
        require(_developerAllowance > 0, "[ptkn231543] sorry no more development tokens available!");
        require(pTokens <= _developerAllowance, "[ptkn231543] greed is bad, you can't withdraw more than you are allowed to!");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[ptkn231543] approve from the zero address");
        require(spender != address(0), "[ptkn231543] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[ptkn231543] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[ptkn231543] transfer amount exceeds allowance"));
        return(true);
    }
    ...
    function _transfer(address pFrom, address pTo, uint256 pAmount) private{
        if(_addressNoTaxes[pFrom] || _addressNoTaxes[pTo]){
            ...
            if(pFrom == ADDRESS_PAIR){
                require(_tradingEnabled, "[ptkn231543] trading not enabled yet, stay tuned!");
                if(!_addressNoLimits[pTo]){
                    require(pAmount <= limitWalletTokens && _balances[pTo].add(pAmount) <= limitWalletTokens, "[ptkn231543] wallet limit reached, can't buy more tokens!");
                }
            ...
            if(!Tax.buy && !Tax.sell){
                if(!_addressNoLimits[pTo]){
                    require(pAmount <= limitWalletTokens && _balances[pTo].add(pAmount) <= limitWalletTokens, "[ptkn231543] wallet limit reached, can't transfer more tokens!");
                }
    ...
    function _addLiquidity() private{
        require(!_liquidityProvided, "[ptkn231543] liquidity can only be enabled once!");
        uint256 balance = address(this).balance;
        uint256 liquidityForUniswap = 750000000 * (10**9);

        require(balance > 0 && _balances[address(this)] >= liquidityForUniswap, "[ptkn231543] you do not have any coins or tokens to provide liquidity!");
