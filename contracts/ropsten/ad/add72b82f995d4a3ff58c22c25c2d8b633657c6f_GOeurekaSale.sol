...
contract gotTokenSaleConfig is GoConfig {
    ...
    uint public constant PRESALE_START = 1528818684; // Friday, June 15, 2018 12:00:46 PM GMT+08:00
    uint public constant SALE_START = PRESALE_START + 8 minutes;
...
contract GOeurekaSale is Claimable, gotTokenSaleConfig, Pausable, Salvageable {
    ...
    function calcDates(uint presaleStart_, uint saleStart) internal {
        ...
        week1End = week1Start + 5 minutes;
        week2End = week1Start + 10 minutes;
        week3End = week1Start + 15 minutes;
    }
    ...
    function calculateRates() internal {
        presaleCap =              uint(10**17).div(basicRate);
        week1Cap = presaleCap.add(uint(10**17).div(basicRate));
        week2Cap = week1Cap.add(uint(10**17).div(basicRate));
        week3Cap = week2Cap.add(uint(10**17).div(basicRate));
        minContribution = uint(1 * DECIMALSFACTOR).div(basicRate);
