...
contract OxsSafe is Ownable {
    ...
    * @param safeWalletAddress  Created safe wallet contract address
    ...
    */
    event NewSafeWallet(address safeWalletAddress, address creator, address[] owners, uint threshold);
    ...
    */
    ...
    /// @return address of created new safe wallet
    function createWallet(address[] calldata _owners, uint _threshold) external returns (address) {
        address cloneAddress = implementation.clone();

        IOxsWallet wallet = IOxsWallet(cloneAddress);
        wallet.initialize(_owners, _threshold);
        wallets.push(wallet);

        emit NewSafeWallet(cloneAddress, _msgSender(), _owners, _threshold);
...
import "../base/IOwnerManager.sol";
import "../base/ITransactionManager.sol";
