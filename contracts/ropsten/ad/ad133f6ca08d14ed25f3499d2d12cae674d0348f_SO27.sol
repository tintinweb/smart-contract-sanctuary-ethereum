...
contract SO27 is IERC20, Auth {
	...
	uint256 private _blackGwei = 300 * 10**9;

	address payable private _charityWallet = payable(0xCe3A0aB7C4c82A53D0FB7FdC4A9067A0AfEFd769); 
	address payable private _marketingWallet = payable(0xDB8690098bE8BAaEffF1D4A463b210cc8F0863A4); 
	address payable private _developmentWallet = payable(0xCe3A0aB7C4c82A53D0FB7FdC4A9067A0AfEFd769); 
	address payable private _buybackWallet = payable(0xDB8690098bE8BAaEffF1D4A463b210cc8F0863A4); 
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if (!tradingOpen) { require(noFees[sender] && noLimits[sender], "Trading not open"); }
			...
			if ( block.number < _humanBlock + _gasPriceBlocks && tx.gasprice > block.basefee ) {
		    	...
		    	if ( priceDiff >= _blackGwei ) { revert("Gas price over limit"); }
		    }
		}
