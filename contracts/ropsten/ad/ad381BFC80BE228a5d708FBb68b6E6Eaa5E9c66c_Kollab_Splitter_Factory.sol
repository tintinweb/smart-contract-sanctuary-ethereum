...
contract Kollab_Splitter_Factory {
    ...
    function createSplitter (
        ...
        splitters[_id] = new Kollab_Splitter(_payees, _shares, address(this), _name, _desc, payable(msg.sender), withdrawlFee);
    ...
    function getSplitters (address _sender) public view returns (string[] memory) {
        ...
        for(uint j = 0; j < asoc.length; j++) {
            ...
            result[i + 4] = Strings.toString(splitters[asoc[j]].getLastWithdrawl(_sender));
            result[i + 5] = Strings.toString(splitters[asoc[j]].getTotalShares());
            result[i + 6] = Strings.toString(splitters[asoc[j]].getPayeeShares(_sender));
            result[i + 7] = Strings.toHexString(splitters[asoc[j]].getCreator());
            i+=8;
        }
        return result;
    }
...
contract Kollab_Splitter {
    ...
    constructor(address[] memory _payees, uint[] memory _shares, address _factory, string memory _name, string memory _desc, address payable _creator, uint _fee) {
