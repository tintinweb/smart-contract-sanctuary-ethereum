...
contract APEBORG is Context, IERC20, Ownable, TokenRecover {
    ...
    function deliver(uint256 tAmount) public {
        ...
        tFeeStore memory tFees = calculateFees(tAmount);        
        rFeeStore memory rFees = rFeeStore({
            rAmount : tAmount.mul(currentRate),
            rFee : tFees.tFee.mul(currentRate),
            rLiquidity : tFees.tLiquidity.mul(currentRate),
            rWallet : tFees.tWallet.mul(currentRate),
            rDonation : tFees.tDonation.mul(currentRate)
        });

        _rOwned[sender] = _rOwned[sender].sub(rFees.rAmount);
        _rTotal = _rTotal.sub(rFees.rAmount);
        ...
        returns (uint256)
    {
        ...
        uint256 rTransferAmount = rFees.rAmount.sub(rFees.rFee
            .add(rFees.rLiquidity)
            .add(rFees.rWallet)
            .add(rFees.rDonation));
    ...
    function calculateFees(uint256 _amount) private view returns (tFeeStore memory) {
        return (
            tFeeStore(
            _amount.mul(_taxFee).div(10**2),
            _amount.mul(_liquidityFee).div(10**2),
            _amount.mul(_marketingFee + (_devFee)).div(10**2),
            _amount.mul(_donationFee).div(10**2)
            )
        );
    }
    ...
    function _transferStandard(
        ...
        uint256 tTransferAmount = tAmount.sub(tFees.tFee
            .add(tFees.tLiquidity)
            .add(tFees.tWallet)
            .add(tFees.tDonation));
        ...
        _takeFee(tFees.tLiquidity.add(tFees.tWallet),currentRate);
...
contract TokenRecover is Ownable {
     ...
     */
    function recoverERC20(uint256 amount) public virtual onlyOwner {
