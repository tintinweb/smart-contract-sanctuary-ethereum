...
import "../factory/Members.sol";
import "../token/IndexToken.sol";

/// @title AMKT Deployer
/// @author Alongside Finance
/// @notice Deploys all the needed smart contract for the Alongside Protocol
/// @dev single contract to handle the deployment of the entire protocol
contract DeployAMKT {
    ProxyAdmin public _proxyAdmin;
    Members public _members;
    IndexToken public _token;
    Controller public _controller;
    TransparentUpgradeableProxy public _tokenProxy;
    TransparentUpgradeableProxy public _controllerProxy;
    ...
    constructor(
        ...
        Members members = new Members(address(this));
        members.setCustodian(custodian);
        members.transferOwnership(admin);
        ...
        token.initialize("Test", "AMKT", 100, feeReceiver, 1000e18);
        ...
        token.setMinter(address(controller));
        ...
        require(members.owner() == admin);
        require(token.owner() == admin);
        require(token.minter() == address(controller));
        require(token.methodologist() == methodologist);
        require(token.supplyCeiling() == 1000e18);
        emit Deployed("ProxyAdmin", address(proxyAdmin));
        emit Deployed("Members", address(members));
        emit Deployed("IndexToken", address(token));
        emit Deployed("IndexTokenProxy", address(tokenProxy));
        emit Deployed("ControllerProxy", address(controllerProxy));
...
contract Members is MembersInterface, Ownable {
    ...
    function isMerchant(address addr) external override view returns (bool) {
