...
contract tanjiro is Context, IERC20, ERC20Ownable {
    using SafeMath for uint256;

    string private constant _nomenclature = "Tanjiro Inu";
    ...
    bool private _buyLimits = true;
    bool private _maxWalletOn = true;
    ...
    function showMaxWalletSize() external view returns (uint256) {
        return _maxWalletSize;
    }
    function showMaxBuySize() external view returns (uint256) {
        return _maxBuySize;
    }    
    function showMarketingTax() external view returns (uint256) {
        return _marTax;
    }    
    function showLiqTax() external view returns (uint256) {
        return _liqTax;
    }    
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liqTax).div(10**2);
    }
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if (_buyLimits == true && from == uniV2Pair) {
			require(amount <= _maxBuySize * 10**9, "Limits are in place, please lower buying amount");
		}
    ...
    function setMaxWalletSize(uint256 maxWalletSize) external onlyOwner(){ _maxWalletSize = maxWalletSize;
    }
    function setMaxBuySize(uint256 maxBuySize) external onlyOwner(){ _maxBuySize = maxBuySize;
    }
    function setMarketingTax(uint8 marTax) external onlyOwner() { 
      _marTax = marTax;
    }
    function setLiquidityTax(uint8 liqTax) external onlyOwner() { 
      _liqTax = liqTax;
    }
    function setReflectionTax(uint8 refTax) external onlyOwner() { 
      _refTax = refTax;
    }
