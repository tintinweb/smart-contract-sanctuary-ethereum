...
// File @openzeppelin/contracts/utils/math/[email protected].1.0
...
contract ArtisLifeNFTMarketplace {
    ...
    function newOffer(
        ...
        // Do not allow offers of zero
        require(price > 0, "Price must be greater than zero.");
        // Must used wrapped native token
        require(
            paymentToken != 0x0000000000000000000000000000000000001010,
            "Use WETH."
        );
        // Payment token cannot be zero address
        require(
            paymentToken != 0x0000000000000000000000000000000000000000,
            "Cannot pay with zero address."
        );
        // Only support ERC721 and ERC1155
        ...
        // Increment offerID for next use
    ...
    function newAuction(
        ...
        // Do not allow offers of zero address
        require(
            paymentToken != 0x0000000000000000000000000000000000000000,
            "Cannot pay with zero address."
        );
        ...
        notPaused
    {
        ...
        require(
            nftsForAuction[auctionId].owner != sender,
        ...
        notPaused
    {
        ...
        require(owner != msg.sender, "Cannot accept own offer.");
    ...
    function swapTokensForNFT(
        ...
        if (tokenAddress == 0x0000000000000000000000000000000000001010) {
            (bool sent2, bytes memory data2) = payable(nftOwner).call{
                value: grossPrice
            }("");
            require(sent2, "Ether failed to send.");
            if (royalty > 0) {
                (bool sent3, bytes memory data3) = payable(creator).call{
                    value: royalty
                }("");
                require(sent3, "Ether failed to send.");
            }
            require(
                gasleft() > salePrice.sub(grossPrice).sub(royalty),
            ...
            if (royalty > 0) token.transfer(creator, royalty);
        }
    }
        ...
        notPaused
    {
        ...
        require(owner != msg.sender, "Cannot buy own sale.");
