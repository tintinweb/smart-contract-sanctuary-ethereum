...
//╚█████╔╝╚█████╔╝╚█████╔╝██║░╚███║███████╗██║░╚██╗╚█████╔╝╚█████╔╝███████╗╚██████╔╝██████╦╝//
//░╚════╝░░╚════╝░░╚════╝░╚═╝░░╚══╝╚══════╝╚═╝░░╚═╝░╚════╝░░╚════╝░╚══════╝░╚═════╝░╚═════╝░//
contract NEKO is ERC721, Ownable {
    using Strings for uint256;

    uint256 public itemPrice = 0.088 ether;

    bool public isSaleActive = false;
    uint256 public constant totalSupply = 8888;
    uint256 public purchasedNEKOS = 0;
    ...
    constructor() ERC721("888NEKO CLUB", "NEKO") {}

    function purchaseNeko(uint256 _howMany) external payable {
        require(_howMany > 0, "Minimum 1 to be minted");
        require(
            _howMany <= NekosRemainingToBeMinted(),
            "Quantity is greater than remaining"
        );
        require(isSaleActive, "Sale is not active");
        require(_howMany <= 10, "Max 10 Units");
        require(itemPrice * _howMany == msg.value, "Not enough Eth to mint");
        for (uint256 i = 0; i < _howMany; i++) {
            _mintNeko(_msgSender());
        }
    }
    ...
    function _mintNeko(address _to) private {
        if (purchasedNEKOS == 0) {
            _lastIpfsId = random(
                1,
                totalSupply,
        ...
        purchasedNEKOS++;
        require(!_exists(purchasedNEKOS), "Mint: Token already exist.");
        _mint(_to, purchasedNEKOS);
        _tokenURIs[purchasedNEKOS] = _lastIpfsId;
    }
    ...
    function NekosRemainingToBeMinted() public view returns (uint256) {
        return totalSupply - purchasedNEKOS;
    }

    function isAllTokenMinted() public view returns (bool) {
        return purchasedNEKOS == totalSupply;
    }

    function getIpfsIdToMint() private view returns (uint256 _nextIpfsId) {
        require(!isAllTokenMinted(), "All tokens have been minted");
        if (
            _lastIpfsId == totalSupply &&
            purchasedNEKOS < totalSupply
        ) {
            _nextIpfsId = 1;
        } else if (purchasedNEKOS < totalSupply) {
