...
function mint(uint256 _mintAmount) public payable enabled mintable(_mintAmount) funded(_mintAmount) {
...
//_______________________________________________________________________________
    modifier mintable(uint256 _mintAmount) {
      require(_mintAmount > 0 && (_mintAmount <= maxMintAmountPerTx || msg.sender == owner()),  'Invalid mint amount!');
      require(totalSupply() + _mintAmount <= maxSupply, 'Max supply exceeded!');
      _;
    }

    modifier funded(uint256 _mintAmount) {
      require(((totalSupply() + _mintAmount) <= (freeSupply + reservedSupply)) || (msg.value >= cost * _mintAmount), 'Insufficient funds!');
      _;
    }
