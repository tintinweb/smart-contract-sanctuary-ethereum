...
contract Defi is Ownable {
    ...
    mapping(uint256 => uint256) private due;
    ...
    constructor(
        ...
        loanLendDate = block.timestamp;
        uint256 duerepaydate = loanLendDate;
    ...
    function numberOfRepaymentRemainning() public view returns(uint256){
    ...
    function lendDate() public view returns(uint256){
        return loanLendDate;
    }

    function setDeutime(uint256 i, uint256 duedate) private {
        due[i] = duedate;
    }

    function getDueDate(uint256 numberOfDue) public view returns(uint256){
    ...
    function repay(address to, uint256 amount, uint numberofdue) external payable returns (bool) {
