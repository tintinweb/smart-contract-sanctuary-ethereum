...
contract MoToken is IERC20, Ownable {
  ...
  function userDrawMo(uint256 amount) external returns(bool){
    ...
    _totalPower = _totalPower.sub(_value.div(2), "MO: power amount exceeds balance"); //减少全网算力
    _userMaxIncome[_sender] = _userMaxIncome[_sender].sub(_value, "MO: userIncome amount exceeds balance"); //减少用户最大收益  
    _balances[_drawMoAddress] = _balances[_drawMoAddress].sub(amount,"MO: userMaxIncome amount exceeds balance"); //从提币地址转MO币
