...
contract TATASTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    function _approve(
        ...
        require(false, "TATAST_Dividend_Tracker: No approvals allowed");
    }
    ...
    function _transfer(
        ...
        require(false, "TATAST_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "TATAST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TATAST contract."
        );
    }
...
contract TATAST is ERC20, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    string private constant _name = "TATAST";
    string private constant _symbol = "TATAST";
    ...
    uint256 private baseBuyTax = 6;
    uint256 private baseBuyEthTax = 9;
    uint256 public baseSellTax = 11;
    ...
    uint256 private devFee = 28;
    uint256 private marketingFee = 30;
    uint256 private charityFee = 15;
    ...
    TATASTDividendTracker public dividendTracker;
    ...
    event DistributeFees(uint256 devEth, uint256 remarketingEth, uint256 recharityEth);
    ...
    constructor(
        ...
        dividendTracker = new TATASTDividendTracker();
        dividendTracker.excludeFromDividends(address(dividendTracker));
        dividendTracker.excludeFromDividends(address(this));
        dividendTracker.excludeFromDividends(owner());
        dividendTracker.excludeFromDividends(address(uniswapV2Router));
        minutesRangeTaxes[1].from = 0 minutes;
        minutesRangeTaxes[1].to = 4320 minutes;
        minutesRangeTaxes[1].tax = 25;
        minutesRangeTaxes[2].from = 4320 minutes;
        minutesRangeTaxes[2].to = 10080 minutes;
        minutesRangeTaxes[2].tax = 22;
        minutesRangeTaxes[3].from = 10080 minutes;
        minutesRangeTaxes[3].to = 14400 minutes;
        minutesRangeTaxes[3].tax = 18;
        minutesRangeTaxes[4].from = 14400 minutes;
        minutesRangeTaxes[4].to = 15840 minutes;
        minutesRangeTaxes[4].tax = 16;
    ...
    function openTrading(uint256 _launchTime, uint256 _minutesForPrivateSale)
        ...
        onlyOwner
    {
        require(!tradingOpen, "TATAST: Trading is already open");
        require(_launchTime > block.timestamp, "TATAST: Invalid timestamp");
    ...
    function manualSend() external onlyOwner {
        ...
        uint256 charityFeesToSend = totalEth.mul(charityFee).div(
            uint256(100).sub(autoLP)
        );
        uint256 remainingEthForFees = totalEth.sub(devFeesToSend).sub(
            marketingFeesToSend).sub(charityFeesToSend);
        devFeesToSend = devFeesToSend.add(remainingEthForFees);

        sendEthToWallets(devFeesToSend, marketingFeesToSend, charityFeesToSend);
    }
    ...
    function _claim(address payable _account, bool _reinvest) private {
        ...
        require(
            ...
            "TATAST: Claimer has no withdrawable dividend"
        );
        uint256 ethAmount;
        uint256 tokenAmount;
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                ...
                "TATAST: Private and public sale is not open"
            );
            if (
                privateSaleTimestamp <= block.timestamp &&
        ...
        if (!automatedMarketMakerPairs[_to] && !isExludeFromWalletLimit[_to]) {
            ...
            require(
                ...
                "TATAST: Wallet balance limit reached"
            );
        }
    ...
    function _setAutomatedMarketMakerPair(address _pair, bool _value) private {
        require(
            ...
            "TATAST: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[_pair] = _value;
    ...
    function setTaxes(
        ...
        require(_baseBuyTax <= 6 && _baseBuyEthTax <= 9 && baseSellTax <= 12);

        baseBuyTax = _baseBuyTax;
        baseBuyEthTax = _baseBuyEthTax;
    ...
    function setWalletAddress(address _devWallet, address _marketingWallet, address _charityWallet)
        external
        onlyOwner
    {
        devWalletAddress = _devWallet;
        marketingWalletAddress = _marketingWallet;
        charityWalletAddress = _charityWallet;
    }
    ...
    function distributeTokensEth(uint256 _tokenAmount) private {
        ...
        uint256 charityFeesToSend = totalEth.mul(charityFee).div(100);
        uint256 remainingEthForFees = totalEth
            .sub(ethForAddLP)
            .sub(devFeesToSend)
            .sub(marketingFeesToSend)
            .sub(charityFeesToSend);
        devFeesToSend = devFeesToSend.add(remainingEthForFees);
    ...
    function sendEthToWallets(uint256 _devFees, uint256 _marketingFees, uint256 _charityFees)
        private
    {
        if (_devFees > 0) {
        ...
        if (_charityFees > 0) {
            payable(charityWalletAddress).transfer(_charityFees);
        }
        emit DistributeFees(_devFees, _marketingFees, _charityFees);
    }
