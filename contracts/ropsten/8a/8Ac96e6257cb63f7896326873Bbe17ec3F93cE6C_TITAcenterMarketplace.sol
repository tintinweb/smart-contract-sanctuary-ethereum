...
contract TITAcenterMarketplace is IERC721Receiver {
    uint256 gasFee = 0.025 ether;
    ...
    function buy(address nftContract, uint256 tokenId) public payable {
        ...
        IERC721(nftContract).transferFrom(tokenOwner, msg.sender, tokenId);
        payable(seller).transfer(item.price);
        payable(owner).transfer(gasFee);
        item.seller = payable(msg.sender);
        item.sale = false;
        emit NftBought(nftContract, seller, msg.sender, msg.value, tokenId);
    }
    ...
    function transferToken(
        ...
        address _owner = IERC721(nftContract).ownerOf(tokenId);
        require(msg.sender == _owner, "Not a owner of thiis token");
        require(msg.value >= gasFee, "Please sumbit GasFee to transfer token");
        IERC721(nftContract).transferFrom(msg.sender, to, tokenId);
        payable(owner).transfer(gasFee);
    }

    /* Get Market Item Details */
    function getTokenMarketItemDetails(address nftContract, uint256 tokenId)
        public
        view
    ...
    function startAuction(
        ...
        ERC721(nftContract).transferFrom(_owner, address(this), tokenId);
    ...
    /* Get Auction Details */
    function getTokenAuctionDetails(address nftContract, uint256 tokenId)
        public
        view
