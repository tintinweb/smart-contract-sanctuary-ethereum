...
contract FakeNewsApp{
   ...
   //mapping(address=>uint256) public voteStore;
    ...
    function endSession(uint256 id)public{
        articleList[id].status = ArticleStatus.VotingCompleted;
         DisplayArticleList[id].status = ArticleStatus.VotingCompleted;

    }
    function calculateResult(uint256 id)public {
    ...
    function distribute(uint256 _id)public payable{
...
//VIEW FUNCTIONS
    ...
    function getParticipantsStake(uint256 id,address _add) external view returns(uint256){
        ...
        function getCurrentStatus(uint256 id) external view returns (uint8 stat){
           ...
           if(articleList[id].status == ArticleStatus.VotingCompleted){
               return 1;
           }
           else{
               return 2;
