...
/// @dev A contract for deploying and configuring a minimal KSwap contract.
contract InitialMigration {
    ...
    /// @dev The allowed caller of `initializeKSwap()`. In production, this would be
    ///      the governor.
    address public immutable initializeCaller;
    /// @dev The real address of this contract.
    address private immutable _implementation;

    /// @dev Instantiate this contract and set the allowed caller of `initializeKSwap()`
    ///      to `initializeCaller_`.
    /// @param initializeCaller_ The allowed caller of `initializeKSwap()`.
    constructor(address initializeCaller_) public {
        initializeCaller = initializeCaller_;
        _implementation = address(this);
    }

    /// @dev Initialize the `KSwap` contract with the minimum feature set,
    ///      transfers ownership to `owner`, then self-destructs.
    ///      Only callable by `initializeCaller` set in the contstructor.
    /// @param owner The owner of the contract.
    /// @param kSwap The instance of the KSwap contract. KSwap should
    ///        been constructed with this contract as the bootstrapper.
    /// @param features Features to bootstrap into the proxy.
    /// @return _kSwap The configured KSwap contract. Same as the `kSwap` parameter.
    function initializeKSwap(
    ...
    /// @dev Sets up the initial state of the `KSwap` contract.
    ///      The `KSwap` contract will delegatecall into this function.
    /// @param owner The new owner of the KSwap contract.
    /// @param features Features to bootstrap into the proxy.
