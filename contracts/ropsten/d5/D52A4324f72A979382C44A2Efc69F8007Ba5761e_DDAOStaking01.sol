...
//	    require(amount == v1,"Amount must be integer only.");
	...
	function Unstake(uint256 amount)public
	{
	    ...
	    Stakers[addr].time = uint48(block.timestamp);
	    IERC20(TokenAddr).safeTransferFrom(address(this),_msgSender(), amount.mul(10**IToken(TokenAddr).decimals()));
	...
	{
	    require(Stakers[addr].amount >= 0,'Stake for this address is zero');
	    uint256 temp;
