...
contract Distributor is Ownable, IERC721Receiver {
   ...
   * @param _fromId starting token id; must not be zero
  ...
  function withdrawBatch(
    ...
    if (_fromId == 0) revert InvalidNFTId();

    unchecked {
      // we are not able to accept enough data to overflow `i` and `_fromId`
      for (uint256 i = 0; i < _accounts.length; i++) {
        _token.safeTransferFrom(address(this), _accounts[i], _fromId + i);
      }
    }

    emit LogERC721WithdrawnBatch(_token, _fromId, _accounts);
  }
}
