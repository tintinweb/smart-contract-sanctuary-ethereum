...
contract PUNK is ERC721Enumerable, Ownable {
	...
	address private wallet1 = 0x431840A475E0aD62F9a67D752C9110F09CdB97c7;
    address private wallet2 = 0x4e209Fc427BA76f073a91e837a1E3377EcFb9B6a;
    address private wallet3 = 0xf2bAd1685b3B77ae95d165A1417273CBE121bddc;
	address private wallet4 = 0x40B15D228838836b62Fb372DBBabcdB7c8f89B49;
    address private wallet5 = 0xb3a696bd8D35C2908E2fD1805C75718088Acbd57;
    address private Authorized = 0x431840A475E0aD62F9a67D752C9110F09CdB97c7;
	...
	uint public maxMCPPurchase = 40;
    uint public mintWhitelistWL = 10;
    uint public maxMCPPurchaseWL = 10;

    uint256 public constant MAX_MCP = 6969;
    uint public MCPReserve = 0;
	uint public MCPWhitelistReserve = 3485;
    ...
    constructor() ERC721("Test Punks NFT", "PUNK") { }
    ...
    function flipPublicSaleState() public onlyAuthorized {
     ...
     function mint(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= maxMCPTx, "20 pTX allowed");
        require(msg.value == MCPPrice_public.mul(numberOfTokens), "Check ETH");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= maxMCPPurchase,"Max pW reached"); // tdl remove
   ...
   // to set the merkle root
	...
	function mintWhitelist(uint numberOfTokens, bytes32[] calldata merkleProof ) payable external  {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= mintWhitelistWL, "10 pTX allowed");
        require(msg.value == MCPPrice_whitelist.mul(numberOfTokens), "Check ETH");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= maxMCPPurchaseWL,"Max whitelist mints reached");

        // Verify the merkle proof
        require(MerkleProof.verify(merkleProof, merkleRoot,  keccak256(abi.encodePacked(user_))  ), "Check proof");

		for(uint i = 0; i < numberOfTokens; i++) {
            if (totalSupply() <= MCPWhitelistReserve) {
                _safeMint(msg.sender, totalSupply()+1);
                max_mints_per_address[msg.sender] = max_mints_per_address[msg.sender].add(1);
            }
			else {
    ...
    function withdrawAll() external onlyOwner {
        ...
        (bool wallet1Success, ) = wallet1.call{value: _amount.mul(20).div(100)}("");
        (bool wallet2Success, ) = wallet2.call{value: _amount.mul(20).div(100)}("");
        (bool wallet3Success, ) = wallet3.call{value: _amount.mul(20).div(100)}("");
		(bool wallet4Success, ) = wallet4.call{value: _amount.mul(20).div(100)}("");
		(bool wallet5Success, ) = wallet5.call{value: _amount.mul(20).div(100)}("");
  ...
  function setmintWhitelistWL(uint256 newmintWhitelistWL) public onlyAuthorized {
            mintWhitelistWL = newmintWhitelistWL;
        }
