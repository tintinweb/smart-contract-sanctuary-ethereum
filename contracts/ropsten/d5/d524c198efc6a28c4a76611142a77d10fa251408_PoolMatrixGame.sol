...
contract PoolMatrixGame {
    ...
    */
    ...
    struct User {
        ...
        uint256 levelProfit;
        UserIncrements24h[] increments24h;
    }
    ...
    constructor(address regFee, address marketing, address[] memory owners, address[] memory roots) {
        ...
        for (i = 0; i < rootWallets.length; i++) {
            ...
            for (j = 0; j < 4; j++) {
                users[addr].increments24h.push(UserIncrements24h({
    ...
    function register(address referrer, uint256 investAmount, uint256 initialBalance) public payable {
        ...
        for (uint i = 0; i < 4; i++) {
            users[msg.sender].increments24h.push(UserIncrements24h({
                date: uint32(block.timestamp / SECONDS_IN_DAY),
                referrerRewards: 0
            }));
        }
    ...
    function buyLevel(uint8 level, uint256 investAmount) public payable {
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            userIncrements24hNormalize(quarter, referrer);
            users[referrer].increments24h[quarter].referrerRewards += value;
    ...
    function userIncrements24hNormalize(uint32 quarter, address userAddr) private {
        uint32 date = uint32(block.timestamp / SECONDS_IN_DAY);
        if (users[userAddr].increments24h[quarter].date != date) {
            users[userAddr].increments24h[quarter].date = date;
            users[userAddr].increments24h[quarter].referrerRewards = 0;
        }
    }
    ...
    function getTotalInfo() public view returns(uint256 totalUsers, uint256 totalTransactions, uint256 totalTurnover, uint32 totalUsersIncrement, uint32 totalTransactionsIncrement, uint256 totalTurnoverIncrement) {
        uint32 usersIncrement = globalIncrements24h[0].totalUsers + globalIncrements24h[1].totalUsers + globalIncrements24h[2].totalUsers + globalIncrements24h[3].totalUsers;
        uint32 transactionsIncrement = globalIncrements24h[0].totalTransactions + globalIncrements24h[1].totalTransactions + globalIncrements24h[2].totalTransactions + globalIncrements24h[3].totalTransactions;
        uint256 turnoverIncrement = globalIncrements24h[0].totalTurnover + globalIncrements24h[1].totalTurnover + globalIncrements24h[2].totalTurnover + globalIncrements24h[3].totalTurnover;
    ...
    function appendUsers(User[] memory newUsers) public {
