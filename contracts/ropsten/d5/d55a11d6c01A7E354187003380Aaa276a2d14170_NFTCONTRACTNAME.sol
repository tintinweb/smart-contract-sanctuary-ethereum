...
contract NFTCONTRACTNAME is ERC721Enumerable, Ownable, ERC721URIStorage {
    using Strings for uint256;
    ...
    // USAGE: holderReservedCount[address] = number of reserved items for address
    mapping(address => uint16 ) public holderReservedCount;
    // Optional mapping for token URIs
    mapping(uint256 => string) private _tokenURIs;
    ...
    function mint(uint8 mintQty) public payable {
        ...
        require(supply <= curSupply);
        //Possible deletes Autofloor

        //Possible deletes Autofloor
        if (msg.sender != owner()) {
            require(msg.value >= fCost);
        }
    ...
    function mintReserved(uint8 mintQty) public payable {
        uint256 supply = totalSupply() + mintQty;
        uint256 fCost = cost * mintQty;
        ...
        // End of deletes Autofloor
        // CHECK IF IT IS RESERVED
        checkSupplyAndReserved(mintQty);
    ...
    // Possible deletes Giveaway Allowance

    function batchAddGiveAwayAllowance(
        address[] memory addr,
        uint8[] memory count
    ) external onlyOwner {
        _iterateBatchAddToGiveAway(addr,count);
    }
    ...
    // Possible deletes reserved holder
    function batchAddHolderReservedCount(
        address[] memory addr,
        uint8[] memory count
    ) external onlyOwner {
        _iterateBatchAddHolderReservedCount(addr,count);
    }
    ...
    function _iterateBatchAddHolderReservedCount(
        ...
        for (uint8 i = 0; i < addr.length; i++) {
            holderReservedCount[addr[i]] = count[i];
            remainingReserved = remainingReserved + count[i];
            curSupply = curSupply - count[i];
        }
    }

    function checkSupplyAndReserved(uint16 mintQty) internal {
        address sender = msg.sender;
        uint16 holderReserved_ = holderReservedCount[sender];
        uint16 maxMints_ = maxSupply;
        if(holderReserved_ == 0) {
            require(
                curSupply + mintQty <= maxMints_ - remainingReserved,
                "ER36"
            );
        }
        if(mintQty >= holderReserved_) {
            require(
                curSupply + mintQty <= maxMints_,
                "ER37"
            );
        }
        else {
            remainingReserved = remainingReserved - mintQty;
            holderReservedCount[sender] = holderReserved_ - mintQty;
    ...
    // overrides    // The following functions are overrides required by Solidity.
