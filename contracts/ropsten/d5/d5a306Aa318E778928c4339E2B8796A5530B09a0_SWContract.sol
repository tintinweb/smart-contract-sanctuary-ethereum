...
contract SWContract is OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    event ReporterTypeCreated(
        uint32 indexed reporterTypeId,
        string indexed name,
        uint32 indexed rewardFactor,
        bool privileged
    );
    /**
     * @dev Emitted when reporter type updated.
     */
    event ReporterTypeUpdated(
        uint32 indexed reporterTypeId,
        string indexed name,
        uint32 indexed rewardFactor,
        bool privileged
     ...
     */
    event CaseReported(uint32 indexed caseId, uint32 indexed category, uint32 indexed reporterId);
    /**
     * @dev Emitted when case updated.
     */
    event CaseUpdated(uint32 indexed caseId, uint32 indexed category);

    /**
     * @dev Emitted when address reported.
     */
    event AddressReported(
        address indexed account,
        uint8 rank,
        uint32 category,
        uint32 indexed caseId,
        uint32 indexed reporterId
    );
    /**
     ...
     */
    function initialize(
        address rewardERC20TokenAddress_,
        uint256 gasPrice_,
        uint256 averageReportGas_
    ) public initializer {
        __Ownable_init();

        require(
            rewardERC20TokenAddress_ != address(0),
            "SWContract: Invalid reward erc20 token address"
        );
    ...
    modifier onlyReporter() {
        require(
            _reporters[_reporterIds[_msgSender()]].reporterTypeId != 0,
            "SWContract: caller is not active reporter"
        );
        _;
    }
        ...
        )
    {
        return (
            ...
            IERC20Upgradeable(_rewardERC20TokenAddress).balanceOf(address(this))
        );
    }
        ...
        )
    {
        ...
        return (reporterType.name, reporterType.rewardFactor, reporterType.privileged);
    }
        ...
        )
    {
        ...
        return (_case.name, _case.category, _case.timestamp);
    }
        ...
        )
    {
        ...
        return (account.rank, account.category, account.caseId);
    }
    ...
    function updateGasPrice(uint256 gasPrice_) public onlyReporter {
        ...
        require(
            _reporterTypes[reporterTypeId].privileged,
            "SWContract: Reporter has not permissions"
        );
        require(_gasPrice != gasPrice_, "SWContract: Invalid gasPrice");
    ...
    function updateAverageReportGas(uint256 averageReportGas_) public onlyOwner {
        require(_averageReportGas != averageReportGas_, "SWContract: Invalid averageReportGas");
     ...
     */
    function addReporterType(
        string memory name_,
        uint32 rewardFactor_,
        bool privileged_
    ) external onlyOwner {
        require(bytes(name_).length != 0, "SWContract: Invalid name");
     ...
     * - `name` must not be an empty string.
     */
    function updateReporterType(
        uint32 reporterTypeId_,
        string memory name_,
        uint32 rewardFactor_,
        bool privileged_
    ) external onlyOwner {
        require(
            reporterTypeId_ > 0 && reporterTypeId_ <= _reporterTypeIdTracker,
            "SWContract: ReporterTypeId not exist"
        );
        require(bytes(name_).length != 0, "SWContract: Invalid name");

        ReporterType storage reporterType = _reporterTypes[reporterTypeId_];
        require(
            keccak256(bytes(reporterType.name)) != keccak256(bytes(name_)) ||
                reporterType.rewardFactor != rewardFactor_ ||
                reporterType.privileged != privileged_,
            "SWContract: Invalid params"
        );
        reporterType.name = name_;
        reporterType.rewardFactor = rewardFactor_;
        reporterType.privileged = privileged_;
    ...
    function addReporter(
        ...
        require(bytes(name_).length != 0, "SWContract: Invalid name");
        require(account_ != address(0), "SWContract: Invalid account");
        require(
            _reporterIds[account_] == 0,
            "SWContract: Reporter with current account already exists"
        );
        require(reporterTypeId_ <= _reporterTypeIdTracker, "SWContract: ReporterTypeId not exist");
    ...
    function updateReporter(
        ...
        require(
            reporterId_ > 0 && reporterId_ <= _reporterIdTracker,
            "SWContract: ReporterId not exist"
        );
        require(bytes(name_).length != 0, "SWContract: Invalid name");
        require(reporterTypeId_ <= _reporterTypeIdTracker, "SWContract: ReporterTypeId not exist");
        ...
        Reporter storage reporter = _reporters[reporterId_];
        require(
            keccak256(bytes(reporter.name)) != keccak256(bytes(name_)) ||
                reporter.rate != rate_ ||
                reporter.reporterTypeId != reporterTypeId_,
            "SWContract: Invalid params"
        );
        ...
        emit ReporterUpdated(
            reporterId_,
            name_,
            _reporters[reporterId_].account,
            rate_,
            reporterTypeId_
        );
    }
    ...
    function updateCase(
        ...
        require(caseId_ > 0 && caseId_ <= _caseIdTracker, "SWContract: CaseId not exist");
        require(bytes(name_).length != 0, "SWContract: Invalid name");

        Case storage _case = _cases[caseId_];
        require(
            keccak256(bytes(_case.name)) != keccak256(bytes(name_)) || _case.category != category_,
            "SWContract: Invalid params"
        );
        _case.name = name_;
        _case.category = category_;
    ...
    function reportAddress(
        ...
        uint32 category_,
        uint32 caseId_
    ) external onlyReporter nonReentrant {
        require(account_ != address(0), "SWContract: Invalid account");
        require(rank_ > 0 && rank_ <= _MAX_RANK, "SWContract: Invalid rank");
        require(caseId_ > 0 && caseId_ <= _caseIdTracker, "SWContract: CaseId not exist");

        AddressInfo storage account = _addresses[account_];
        require(rank_ > account.rank, "SWContract: Rank must be greater than the current value");
    ...
    function updateAddress(
        ...
        require(account_ != address(0), "SWContract: Invalid account");
        require(rank_ <= _MAX_RANK, "SWContract: Invalid rank");

        AddressInfo storage account = _addresses[account_];
        require(account.caseId != 0, "SWContract: Account not exist");
        require(
            account.rank != rank_ || account.category != category_,
            "SWContract: Invalid params"
        );

        uint32 reporterTypeId = _reporters[_reporterIds[_msgSender()]].reporterTypeId;
        require(
            _reporterTypes[reporterTypeId].privileged || rank_ > account.rank,
            "SWContract: Rank must be greater than the current value"
        );
     ...
     */
    function withdrawRewardERC20Token(address account_, uint256 amount_)
        external
        onlyReporter
        nonReentrant
    {
        ...
        require(
            ...
            "SWContract: Not enough reporter balance"
        );

        _reporters[_reporterIds[_msgSender()]].balance -= amount_;
        IERC20Upgradeable(_rewardERC20TokenAddress).safeTransfer(account_, amount_);
    }
