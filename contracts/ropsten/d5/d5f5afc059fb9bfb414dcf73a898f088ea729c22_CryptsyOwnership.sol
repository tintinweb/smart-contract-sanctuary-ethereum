...
contract CryptsyBase {
        ...
        returns (uint)
    {
        ...
        Cryptsy memory _cryptsy = Cryptsy({
            ...
            variableX: uint32(_variableX),
            variableY: uint32(_variableY),
            p_constant: uint32(_p_constant),
            sum: uint32(_sum),
            product: uint32(_product),
            mod: uint32(_mod),
            well: uint32(_well),
            tent: uint32(_tent),
            sin: uint32(_sin),
        ...
        require(newCryptsyId == uint256(uint32(newCryptsyId)));
...
contract CryptsyOwnership is CryptsyBase, ERC721 {
