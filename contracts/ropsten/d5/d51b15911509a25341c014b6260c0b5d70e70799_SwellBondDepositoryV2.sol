...
interface ISwellAuthority {
    /* ========== EVENTS ========== */
    
    event GovernorPushed(address indexed from, address indexed to, bool _effectiveImmediately);
    event GuardianPushed
(address indexed from, address indexed to, bool _effectiveImmediately);    
...
abstract contract SwellAccessControlled {

    /* ========== EVENTS ========== */

    event AuthorityUpdated(ISwellAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    ISwellAuthority public authority;


    /* ========== Constructor ========== */

    constructor(ISwellAuthority _authority) {
    ...
    function setAuthority(ISwellAuthority _newAuthority) external onlyGovernor {
...
interface INoteKeeper {
  ...
  struct Note {
    uint256 payout; // gSWL remaining to be paid
  ...
  function redeem(address _user, uint256[] memory _indexes, bool _sendgSWL) external returns (uint256);
  function redeemAll(address _user, bool _sendgSWL) external returns (uint256);
...
interface IgSWL is IERC20 {
  ...
  function migrate( address _staking, address _sSWL ) external;
}


pragma solidity ^0.8.10;

abstract contract FrontEndRewarder is SwellAccessControlled {
  ...
  IERC20 internal immutable SWL; // reward token

  constructor(
    ISwellAuthority _authority, 
    IERC20 _SWL
  ) SwellAccessControlled(_authority) {
    SWL = _SWL;
  }
  ...
  function getReward() external {
    ...
    SWL.transfer(msg.sender, reward);
  }
...
interface IBondDepository {
  ...
  struct Market {
    ...
    bool capacityInQuote; // capacity limit is in payment token (true) or in SWL (false, default)
  ...
  struct Terms {
    ...
    uint64 maxDebt; // 9 decimal debt maximum in SWL
  }
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
  ...
  constructor (
    ...
    IStaking _staking,
    ITreasury _treasury
  ) FrontEndRewarder(_authority, _SWL) {
    gSWL = _gSWL;
    staking = _staking;
    treasury = _treasury;
  }
...
/* ========== ADD ========== */
   ...
   * @param _payout      the amount of SWL due to the user
  ...
  function addNote(
    ...
    // note that only the payout gets staked (front end rewards are in SWL)
...
/* ========== REDEEM ========== */
   ...
   * @param _sendgSWL    send payout as gSWL or sSWL
   * @return payout_     sum of payout sent, in gSWL
   */
  function redeem(address _user, uint256[] memory _indexes, bool _sendgSWL) public override returns (uint256 payout_) {
    ...
    if (_sendgSWL) {
      gSWL.transfer(_user, payout_); // send payout as gSWL
    } else {
      staking.unwrap(_user, payout_); // unwrap and send payout as sSWL
    }
  }
   ...
   * @param _sendgSWL    send payout as gSWL or sSWL
   * @return             sum of payout sent, in gSWL
   */ 
  function redeemAll(address _user, bool _sendgSWL) external override returns (uint256) {
    return redeem(_user, indexesFor(_user), _sendgSWL);
  }
...
/* ========== VIEW ========== */
   ...
   * @return payout_     the payout due, in gSWL
...
contract SwellBondDepositoryV2 is IBondDepository, NoteKeeper {
/* ======== DEPENDENCIES ======== */
...
/* ======== CONSTRUCTOR ======== */
  ...
  constructor(
    ...
    IStaking _staking,
    ITreasury _treasury
  ) NoteKeeper(_authority, _SWL, _gSWL, _staking, _treasury) {
    // save gas for users by bulk approving stake() transactions
    _SWL.approve(address(_staking), 1e45);
  }
...
/* ======== DEPOSIT ======== */
   ...
   * @return payout_     the amount of gSWL due
  ...
  function deposit(
     ...
     * price = quote tokens : SWL (i.e. 42069 DAI : SWL)
     *
     * 1e18 = SWL decimals (9) + price decimals (9)
     */
     ...
     *
     * this is either the number of SWL that the market can sell
    ...
    // purchased, and how much SWL has been sold
  ...
  function _tune(uint256 _id, uint48 _time) internal {
    ...
    if (_time >= meta.lastTune + meta.tuneInterval) {
      ...
      // SWL decimals (9) + price decimals (9)
       ...
       * is 10,000 SWL. max payout would be 1,000 SWL (10,000 * 1 / 10).
       */  
...
/* ======== CREATE ======== */
   ...
   * @param _market      [capacity (in SWL or quote), initial price / SWL (9 decimals), debt buffer (3 decimals)]
  ...
  function create(
     ...
     *
     * 1e18 = SWL decimals (9) + initial price decimals (9)
     */
     ...
     * interval. for example, if capacity is 1,000 SWL, there are 10 days to conclusion, 
     * and the preferred deposit interval is 1 day, max payout would be 100 SWL.
     */
    ...
    emit CreateMarket(id_, address(SWL), address(_quoteToken), _market[1]);
  }
...
/* ======== EXTERNAL VIEW ======== */
   ...
   * @param _id          ID of market
   * @return             price for market in SWL decimals
   *
   ...
   * @param _id          ID of market
   * @return             amount of SWL to be paid in SWL decimals
   *
   * @dev 1e18 = SWL decimals (9) + market price decimals (9)
   */
   ...
   * @param _id          ID of market
   * @return             current debt for market in SWL decimals
   */
   ...
   * @param _id          ID of market
   * @return             control variable for market in SWL decimals
   */
