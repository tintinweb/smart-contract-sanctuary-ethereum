...
contract FXTOKEN is WhiteL {
	...
	function updateminAmount(uint256 newEndTime) external onlyManagingWallets {
		require(newEndTime > 0);
		minAmount =newEndTime; //eg 1 ether at start
	}
	function updatefundendtime(uint256 newminAmount) external onlyManagingWallets {
		require(newminAmount > 0);
		fundendtime =newminAmount; //eg 1 ether at start
	}
