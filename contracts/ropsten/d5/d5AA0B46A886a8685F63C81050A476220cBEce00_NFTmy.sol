...
//set MetaFreez
	function freezeMetadata() external onlyOwner {               //Testcase done
...
//swt Mint Enabled
    ...
    // for minting NFT````````                                                              //, uint256 boxID, uint256 right, uint256 left, uint256 top, uint256 bottom 
        ...
        for (uint256 i = 0; i <count; i++) {
            ...
            assert(id > max_box_Small + max_box_Medium + max_box_Large + max_box_Ultra + max_box_Mega + number_of_Reserved &&
              ...
              _setTokenURI(id,ipfsURLs[id]);
               //attributes[id] = Attr(_name, _desceription, _boxID, _externallink, _uri updated); pass these arrugments on calling mint
        //      boxSizes[id] = box(boxID, right, left, top, bottom);
  ...
  //set UPDATE URI With Image and other name etc
     function UpdateNewURI(string memory newURIupdate, uint256 id, string memory name, string memory description, string memory link) public {
  ...
  // string memory imagedata = link;
        attributes[id] = Attr(name, description, link, uri);
        ...
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    ...
                    '{"trait_type": "external link", "value": ', (attributes[tokenId].link), '},',
