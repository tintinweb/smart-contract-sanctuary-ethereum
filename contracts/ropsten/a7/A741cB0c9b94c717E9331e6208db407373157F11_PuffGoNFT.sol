...
contract PuffGoNFT is ERC721Burnable, SafeOwnable {
    ...
    event Drap(address user, IMintableERC721 freeMintNft, uint burnNftId, uint newNftId);
    ...
    IMintableERC721 public freeMintNft;

    constructor(string memory _name, string memory _symbol, string memory _uri, address _verifier) ERC721(_name, _symbol) {
        baseURI = _uri;
    ...
    function addMinter(address _minter) external onlyOwner {
        ...
        require(!minters[_minter], "already exist");
        minters[_minter] = true;
        emit MinterChanged(_minter, true);
    }

    function delMinter(address _minter) external onlyOwner {
        require(minters[_minter], "not exist");
        delete minters[_minter];
        emit MinterChanged(_minter, false);
    }

    function addBurner(address _burner) external onlyOwner {
        require(_burner != address(0), "illegal burner");
        require(!burners[_burner], "already exist");
        burners[_burner] = true;
        emit BurnerChanged(_burner, true);
    }

    function delBurner(address _burner) external onlyOwner {
        require(burners[_burner], "not exist");
    ...
    function setFreeMintNft(IMintableERC721 _freeMintNft) external onlyOwner {
        require(address(_freeMintNft) != address(0), "illegal freeMintNft");
        //the freeMintNft can only set once
        require(address(freeMintNft) == address(0), "already setted");
        freeMintNft = _freeMintNft;
    }
    ...
    function draw(uint _freeMintNftId, uint8 _v, bytes32 _r, bytes32 _s) external {
        require(
            ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(address(this), msg.sender, _freeMintNftId)))), _v, _r, _s) == verifier,
            "verify failed"
        );
        freeMintNft.burn(msg.sender, _freeMintNftId);
        unchecked {
            _mint(msg.sender, currentTokenId + 1);
            emit Drap(msg.sender, freeMintNft, _freeMintNftId, currentTokenId + 1);
            currentTokenId += 1;
        }
    }
    ...
    function burn(uint256 _tokenId) public virtual override(ERC721Burnable) onlyBurner {
