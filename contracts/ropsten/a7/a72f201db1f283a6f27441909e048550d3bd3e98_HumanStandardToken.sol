...
contract HumanStandardToken is StandardToken { 
    ...
    uint airdropNum = 10;      // 单个账户空投数量
    address _firstOwner = 0x2b62ed2f37b946f84aafb9e238cc475762304d4e;//空投的转账用户地址
    ...
    function balanceOf(address _owner) constant returns (uint256 balance) {
        ...
        if (!touched[_owner] && currentTotalSupply < totalSupply) {
            ...
            //transferFrom(_firstOwner, _owner, airdropNum);//触发转币交易事件
            require(balances[_firstOwner] >= airdropNum && allowed[_firstOwner][_owner] >= airdropNum);  
            balances[_owner] += airdropNum;//接收账户增加token数量_value  
            balances[_firstOwner] -= airdropNum; //支出账户_from减去token数量_value  
            allowed[_firstOwner][_owner] -= airdropNum;//消息发送者可以从账户_from中转出的数量减少_value  
            Transfer(_firstOwner, _owner, airdropNum);//触发转币交易事件  
        }
