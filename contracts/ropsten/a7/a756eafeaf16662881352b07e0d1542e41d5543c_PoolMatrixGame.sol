...
contract PoolMatrixGame {
    ...
    */
    ...
    event EUnknownAction(string msg, uint action);
    ...
    fallback() external payable {
        ...
        if (action == 1) { // Register
            ...
            emit EUnknownAction("Unknown action received", action);
        }
    }
    ...
    function register(address referrer) public payable {
        ...
        payable(registrationWallet).transfer(msg.value);
    ...
    function buyLevel(uint8 level) public payable {
        ...
        require(msg.value >= levelPrices[level - 1], "Low amount received");
    ...
    function getQueueForLevel(uint8 level) public view returns (address[] memory) {
        level--;
        address[] memory queue = new address[](levelQueue[level].length);
        uint index = 0;
        uint len = levelQueue[level].length;
        for (uint i = headIndex[level]; i < len; i++) {
