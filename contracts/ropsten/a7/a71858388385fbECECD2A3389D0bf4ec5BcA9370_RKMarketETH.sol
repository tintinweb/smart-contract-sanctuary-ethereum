...
contract RKMarketETH is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice Getter for artist address by NFT address and token ID
    function getArtistOfTokenId(address nft, uint256 id)
        external
        view
        returns (address artist)
    {
        artist = artists[nft][id];
    }

    /// @notice Getter for artist address by NFT address
    function getArtistForCollection(address nft)
        external
        view
        returns (address artist)
    {
        artist = artistOfCollection[nft];
    }
    ...
    /**
     * @notice Admin function for primary sale collection listing
     */
    function primarySaleCollection(
        address nft,
        uint256[] memory tokenIds,
        uint256 price,
        address artist
        ...
        require(tokenIds.length > 0, "nothing to sale");
        require(artist != address(0), "bad artist address");
        artistOfCollection[nft] = artist;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _createSale(nft, tokenIds[i], price, true);
        }
    }
    ...
    function buy(uint256 lotId) external splitterExist nonReentrant {
        Lot storage _lot = lots[lotId];
        require(
            IERC20Upgradeable(weth).balanceOf(msg.sender) >= _lot.price,
            "insufficient weth"
        );
    ...
    /**
     * @notice Buy NFT using specified lot ID with native Ethereum currency
     * @param lotId ID of lot (possition in lots array)
     */
    function buyETH(uint256 lotId) external payable splitterExist nonReentrant {
        Lot storage _lot = lots[lotId];
        require(msg.value == _lot.price, "wrorg eth amount");
        require(!_lot.isSold, "already sold");
        require(!_lot.isDelisted, "lot delisted");
        require(
            artists[_lot.nft][_lot.tokenId] != address(0) ||
        ...
        if (_lot.isPrimary) {
            ...
            ISplitterContract(splitter).secondaryDistributionETH{
                value: _lot.price
            }(_artist, _lot.seller, _lot.price);
        }
        _lot.isSold = true;
        IERC721Upgradeable(_lot.nft).transferFrom(
            address(this),
    ...
    function cancelOffer(uint256 offerId) external {
        ...
        require(msg.sender == _offer.buyer, "not your offer");
