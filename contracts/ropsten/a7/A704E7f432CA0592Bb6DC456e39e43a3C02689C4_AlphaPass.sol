...
contract AlphaPass is ERC721, Ownable, Pausable, RoyaltiesV2Impl {
    ...
    event MintPass(address indexed _address, uint256 indexed _amount);

    modifier onlyWhitelist() {
        if (whitelisted) {
            require(_whitelisst[msg.sender], "Not whitelisted");
        }
        _;
    }
    ...
    function totalSupply() public view returns (uint256) {
        uint256 lasId = _tokenIdCounter.current();
        ...
        onlyWhitelist
    {
        ...
        emit MintPass(msg.sender, _mintAmount);
    }
    ...
    function mintFor(address _address, uint256 _mintAmount)
        public
        payable
        whenNotPaused
        onlyOwner
    ...
    function setWhitelisted(bool _whitelisted) public onlyOwner {
        whitelisted = _whitelisted;
    }
