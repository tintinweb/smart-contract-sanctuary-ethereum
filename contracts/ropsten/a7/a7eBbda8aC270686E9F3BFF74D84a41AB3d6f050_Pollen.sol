    ...
    PausableUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        PollenNFT = IPollenNft(0x387102a28359e021197F1f91AD34eBAefE740A3B); // atualizado
    ...
    function getTotalStakedFromUser(address _owner) public view returns (uint256) {
         ...
         IPollenNft.OwnedNFT[] memory nftsFromUser =  getAllNfts(_owner);
        ...
        uint256 _totalStaked;
         ...
         for (uint i = 0; i < totalNFT.length; i++) {
                ...
                for (uint y = 0; y < nftsFromUser.length; y++) {

                    if(totalNFT[i] == nftsFromUser[i].tokenId) {

                        _totalStaked += rewards[i].amount;
