...
contract Voting {
	...
	*/
	function communityVoteCast(uint256 commId, uint256 pollId, bool vote) external {
		...
		require(isAlreadyVoted(pollId) == false, "You already voted.");
		
		CommunityPoll[] storage poll = _polls[commId];
		// CommunityPoll storage poll = pollFetch(commId, pollId);
		require(community.votingRight(poll[pollId]._type, commId, msg.sender) != false, "You dont have voting right.");
		if(poll[pollId].endTimeStamps < block.timestamp){
			...
			poll[pollId].status = PollStatus.ENDED;
		}
	}
	...
	*/
	function pollFetch(uint256 commId, uint256 id) internal returns ( CommunityPoll storage ){
		...
		emit EvtCommunityPoll(pollArr[id]);
		return pollArr[id];
	}
