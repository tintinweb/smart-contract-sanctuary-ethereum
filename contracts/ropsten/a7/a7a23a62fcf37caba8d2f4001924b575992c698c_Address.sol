...
// contract returnCodeHash{
//     function returnsCodeHash(address account)public view returns(bytes32){
//         bytes32 codehash;
...
// function findingMax()public pure returns(uint){
// uint MAX = ~uint(0);
...
contract Shikage is IERC20, Ownable {
    ...
    address public constant liquidityReceiver =
        0xd97fC9Ba0296Afbc8cDf95Bd36678C80827fE2F0;
    ...
    function setProtection(bool _protect) external onlyOwner {
        if (_protect) require(!protectionDisabled);
    ...
    function setTxLimit(uint256 numerator, uint256 divisor) external onlyOwner {
        require(
            numerator > 0 && divisor > 0 && (numerator * 1000) / divisor >= 5
        );
