...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[ptkn081139][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[ptkn081139][ownable] new owner is the zero address");
...
abstract contract RoleBasedAccessControl is Context, Ownable{
    ...
    // modifiers
        modifier onlyRole(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()], "[ptkn081139][role based access control] only addresses assigned this role can access this function!");
            _;
        }

        modifier onlyRoles(string[] memory pRoles){
            for(uint256 i=0; i<pRoles.length; i++){
                require(_roleToAddress[pRoles[i]][_msgSender()], "[ptkn081139][role based access control] only addresses assigned this role can access this function!");
            }
            _;
        }
        ...
        modifier onlyRolesOr(string[] memory pRoles){
            ...
            require(rolePresent, "[ptkn081139][role based access control] only addresses assigned this role can access this function!");
            _;
        }

        modifier onlyRoleOrOwner(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()] || owner() == _msgSender(), "[ptkn081139][role based access control] only addresses assigned this role or the owner can access this function!");
            _;
        }
...
contract IPTKN081139Vote{
    function addVote(address) public {}
    function removeVote(address) public {}
}

contract PTKN081139Pools is Context, Ownable, ReentrancyGuard, RoleBasedAccessControl{
    // lib
    ...
    // interfaces
        IPTKN081139 private _main;
        IPTKN081139Vote private _vote;
    ...
    constructor(address pToken, address pVote){
        // token 0x7Ee1e82cFbEb0631387AbaCAf2484b58A3605133
        // vote 0xe63FFa4337B689e930133AE921f14112565643e3

        // all things wrong
            ERROR['unknownToken'] = '[ptkn081139][pools] this token is not allowed !';
            ERROR['poolExist'] = '[ptkn081139][pools] this pool exists already!';
            ERROR['poolDaysToLow'] = '[ptkn081139][pools] cant create a zero days pool!';
            ERROR['poolNotExist'] = '[ptkn081139][pools] this pool does not exist!';
            ERROR['stakingZero'] = '[ptkn081139][pools] cant stake zero tokens!';
            ERROR['stakingNotEnded'] = '[ptkn081139][pools] your staking pool has not ended yet!';
            ERROR['stakingNoBrackets'] = '[ptkn081139][pools] pool has no tack brackets!';
            ERROR['balanceLessThan'] = '[ptkn081139][pools] you do not have enough tokens!';
            ERROR['allowanceLessThan'] = '[ptkn081139][pools] your set allowance on this token is not enough!';
            ERROR['tokenTransferFailed'] = '[ptkn081139][pools] transfer of tokens failed!';
            ERROR['callVoteError'] = '[ptkn081139][pools] could not interact with voting contract!';
        ...
        // all things tokens
            ...
            _main = IPTKN081139(ADDRESS_TOKEN);
            _vote = IPTKN081139Vote(ADDRESS_VOTE);
    ...
    function setIndividualTaxes(address pWallet, uint16 pMarketing, uint16 pDeveloper, uint16 pBuyback, bool pBuy) public{
        try _main.enableIndividualTaxes(
                ...
                // event
                    emit PoolSetTaxesError(_error, pWallet, ADDRESS_TOKEN, 30, pMarketing, pDeveloper, pBuyback, pBuy);
            }catch (bytes memory _error){
                // event
                    emit PoolSetTaxesErrorBytes(_error, pWallet, ADDRESS_TOKEN, 30, pMarketing, pDeveloper, pBuyback, pBuy);
            }
    }
    ...
    event PoolStakeError(address indexed wallet, string message);
    function stake(address pToken, uint256 pTokens, uint256 pPool) public nonReentrant{
        ...
        if(STAKES[pToken][pPool][_msgSender()].tokens <= 0){
            ...
            try _vote.addVote(_msgSender()){
                ...
                // event
                    emit PoolStakeError(_msgSender(), ERROR['callVoteError']);
            }
        }
    ...
    function unstake(address pToken, uint256 pPool) public nonReentrant{
        ...
        POOLS[pToken][pPool].tokens = POOLS[pToken][pPool].tokens.sub(STAKES[pToken][pPool][_msgSender()].tokens);
        STAKES[pToken][pPool][_msgSender()].tokens = 0;
        _main.disableIndividualTaxes(_msgSender());
        _vote.removeVote(_msgSender());
    }
    ...
    event PoolSetTaxesError(string error, address indexed wallet, address indexed token, uint256 pool, uint16 marketing, uint16 developer, uint16 buyback, bool buy);
    event PoolSetTaxesErrorBytes(bytes error, address indexed wallet, address indexed token, uint256 pool, uint16 marketing, uint16 developer, uint16 buyback, bool buy);
    function _setTaxes(address pToken, uint256 pPool, uint256 pTokens) private{
        ...
        if(_bracket > 0){
            try _main.enableIndividualTaxes(
                ...
                // event
                    emit PoolSetTaxesError(_error, _msgSender(), pToken, pPool, TAXES[pToken][pPool][_bracket].marketing, TAXES[pToken][pPool][_bracket].developer, TAXES[pToken][pPool][_bracket].buyback, false);
            }catch (bytes memory _error){
                // event
                    emit PoolSetTaxesErrorBytes(_error, _msgSender(), pToken, pPool, TAXES[pToken][pPool][_bracket].marketing, TAXES[pToken][pPool][_bracket].developer, TAXES[pToken][pPool][_bracket].buyback, false);
            }
