...
contract PriceFeedV1 is Initializable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function getAveragePrice(
        ...
        uint256 startMonth,
        uint256 endMonth, 
        uint256 startMonthYear,
        uint256 endMonthYear
        ...
        return (Storage.filter(
            ...
            DateTimeLibrary.timestampFromDate(startMonthYear,endMonth,1)
            ));
    }
    ...
    function submitPrices(
        ...
        uint256 month,
        uint256 day,
        uint256 year) external whenNotPaused yearSlot {
...
library Storage {
    ...
    function filter(bytes32 startSlot,bytes32 endSlot,uint256 startTime,uint256 endTime) internal view returns (uint256 average) {
        AssetPrice storage set = diamondStorage();
        uint256 abut1 = set._price[startSlot]._inner.length;
        uint256 abut2 = set._price[endSlot]._inner.length;
        uint256 totalDate;
        for(uint256 i; i<abut1; i++) {
            if(set._price[startSlot]._inner[i]._timestamp >= startTime){
                average = average + set._price[startSlot]._inner[i]._price;
                totalDate++;
            }
        }

        for(uint256 i; i<abut2; i++) {
            if(set._price[endSlot]._inner[i]._timestamp <= endTime){
                average = average + set._price[endSlot]._inner[i]._price;
                totalDate++;
            }
        }
