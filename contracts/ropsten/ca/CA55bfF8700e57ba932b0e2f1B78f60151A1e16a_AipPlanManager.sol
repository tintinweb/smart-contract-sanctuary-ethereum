    ...
    IAipExtendCallback
{
    ...
    function unsubscribe(uint256 id) external returns (IAipPool pool) {
        Plan memory plan = _plans[id];
        require(plan.index > 0, "Invalid plan");
        PoolAddress.PoolInfo memory poolInfo = PoolAddress.PoolInfo({
            token0: plan.token0,
        ...
        pool = IAipPool(PoolAddress.computeAddress(factory, poolInfo));
        pool.unsubscribe(msg.sender, plan.index);
    }
...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH =
        0xd384cafdfb0bd3dfa92e9b9316182184fd2a5077b3ebe2e635fdc1e90a50085f;
