...
contract WeedPunksGenesis is ERC721A, Ownable, ReentrancyGuard, Pausable {
    ...
    function greenlistMint(uint256 count, bytes32[] calldata proof) external payable nonReentrant notBots {
        require(greenlistLive, "Not live");
        require(count <= config.maxgreenlistPerTx, "Exceeds max");
         require(greenlist.isPermitted(msg.sender, proof), "not whitelisted");
    ...
    function setGreenlistMerkleRoot(IMerkle _greenlist) external onlyOwner {
