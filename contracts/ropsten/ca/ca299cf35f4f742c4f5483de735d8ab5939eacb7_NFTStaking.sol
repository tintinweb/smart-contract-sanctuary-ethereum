...
contract NFTStaking is Ownable, IERC721Receiver {
    ...
    constructor(IERC721 _nft, IERC20 _token, uint256 duration, uint256 rate_, uint256 tokenDecimals_) { 
        nft = _nft;
        token = _token;
    ...
    function editRarityTierMultiplier(uint24 t, uint24 multiplier) external onlyOwner {
        rarityTiers[t] = multiplier;
    }

    function addLock(uint256 lockTime_, uint48 rewardMultiplier_) public onlyOwner {
    ...
    function editLock(uint24 lockNumber, uint256 lockTime_, uint48 rewardMultiplier_) external onlyOwner {
    ...
    function _unstakeMany(address account, uint256[] calldata tokenIds) internal {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            require(staked.lockEndTimestamp <= block.timestamp, "Nft is still locked");
            delete vault[tokenId];
            start = false;
            for (uint ii = 0; ii < stakedIds[account].length - 1; ii++){
                if (stakedIds[account][ii] == tokenId) start = true;
                if (start) stakedIds[account][ii] = stakedIds[account][ii+1];
            }
            if (start || stakedIds[account].length == 1 && stakedIds[account][0] == tokenId) stakedIds[account].pop();
    ...
    function updateRarityTier(uint256[] calldata tokenIds_, uint24[] calldata tiers) external onlyOwner {
    ...
    function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            earned += ratePerDay * 10 ** tokenDecimals * (block.timestamp - staked.timestamp) * (vault[tokenId].rewardMultiplier + rewardDenominator) / rewardDenominator * rarityMultiplier / rarityDenominator / 86400 ;
    ...
    function withdrawTokens(address token_, uint256 amount) external onlyOwner {
        if (amount > IERC20(token_).balanceOf(address(this))) amount = IERC20(token_).balanceOf(address(this));
    ...
    function earningInfo(address account, uint256[] calldata tokenIds) external view returns (uint256 earned) {
        uint256 tokenId;
    ...
    function getTimestamp() private view returns (uint48) {
        return uint48(block.timestamp) > uint48(endTimestamp) ? uint48(endTimestamp) : uint48(block.timestamp);
    }
