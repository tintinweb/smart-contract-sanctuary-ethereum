...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn292316][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn292316][ownable] new owner is the zero address");
...
contract IXTKN292316StakingLocked{
    function init(uint256, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN292316PreSale{
    function init(uint256, address) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN292316PreSaleVault{
...
contract IXTKN292316PreSaleVault20{
    function deposit(address, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}


contract IXTKN292316Lottery{
    function init(uint256) public{}
    function deposit(address, address, uint256) public{}
    function balanceOf(address) public view returns(uint256){}
}
...
*/
contract XTKN292316Stats is Context, Ownable{
    // lib
    ...
    // interfaces
        IXTKN292316 private _lep;
        IXTKN292316Project private _project;
        IXTKN292316Staking private _staking;
        IXTKN292316StakingLocked private _staking30D;
        IXTKN292316StakingLocked private _staking90D;
        IXTKN292316StakingLocked private _staking180D;
        IXTKN292316StakingLocked private _staking365D;
        IXTKN292316PreSale private _preSale;
        IXTKN292316PreSaleVault private _vault;
        IXTKN292316PreSaleVault20 private _vault20;
        IXTKN292316Lottery private _lottery;
    ...
    constructor(address pToken){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            _lep = IXTKN292316(ADDRESS_TOKEN);
    ...
    function _refresh() private{
        ...
        ADDRESS_PAIR = _lep.ADDRESS_PAIR();
        ...
        /* reset instances */
        if(block.timestamp <= (PRESALE_VAULT_DEATH - 60)){
            _vault = IXTKN292316PreSaleVault(ADDRESS_PRESALE_VAULT);
            _vault20 = IXTKN292316PreSaleVault20(ADDRESS_PRESALE_VAULT20);
        }
        
        _lottery = IXTKN292316Lottery(ADDRESS_LOTTERY);
        _project = IXTKN292316Project(ADDRESS_PROJECT);
        _staking = IXTKN292316Staking(ADDRESS_STAKING);    

        if(block.timestamp <= (STAKING_LOCKED_DEATH - 60)){
            _staking30D = IXTKN292316StakingLocked(ADDRESS_STAKING_DAYS30);
            _staking90D = IXTKN292316StakingLocked(ADDRESS_STAKING_DAYS90);
            _staking180D = IXTKN292316StakingLocked(ADDRESS_STAKING_DAYS180);
            _staking365D = IXTKN292316StakingLocked(ADDRESS_STAKING_DAYS365);
        }        
    }
}