...
contract TaalTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STAL }
    ...
    address public sTAL;
    uint public sTALQueue; // Delays change to sTAL address
    ...
    constructor (
        ...
        isLiquidityToken[ _TALDAI ] = true;
        liquidityTokens.push( _TALDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for TAL
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint TAL needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( TAL ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TAL for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITALERC20( TAL ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sTAL ).balanceOf( msg.sender ); // Can only borrow against sTAL held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TAL
        @param _amount uint
     */
    function repayDebtWithTAL( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITALERC20( TAL ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TAL, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TAL ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TAL ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TAL valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TAL decimals
            value_ = _amount.mul( 10 ** IERC20( TAL ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.STAL ) { // 9
            sTALQueue = block.number.add( blocksNeededForQueue );
        } else return false;
