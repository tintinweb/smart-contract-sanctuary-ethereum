...
contract ERC20 {
   ...
   modifier onlyOwner {
     ...
     */
    function mint(address accountId, uint256 amount, bytes calldata message) external onlyOwner() returns (bool) {
     ...
     */
    function burn(uint256 amount, bytes calldata message) external returns (bool) {
     ...
     */
    function operatorBurn(address accountId, uint256 amount, bytes calldata message) external returns (bool) {
        require(isOperatorFor(msg.sender, accountId), "Caller not an operator for accountId");
    ...
    function changeOwner(address newOwner) external onlyOwner() returns (bool) {
        require(newOwner != address(0x0), "Zero address used");
     ...
     */
    function authorizeOperator(address accountId, address accountOperatorId) external {
        require(isOperatorFor(msg.sender, accountId), "Caller not an operator for accountId");
     ...
     * - `accountOperatorId` must not equal the caller.
     */
    function revokeOperator(address accountId, address accountOperatorId) external {
        require(isOperatorFor(msg.sender, accountId), "Caller not an operator for accountId");
     ...
     * -  `payeeId` cannot be the zero address.
     */
    function transferFromBatch(address payeeId, address[] calldata payerIds, uint256[] calldata amounts) external returns (bool, address[] memory) {
        require(isOperatorFor(msg.sender, payeeId), "Caller not an operator for payeeId");
        uint count;
        uint size = payerIds.length;
        address[] memory failedAddresses;
        uint failedArrayCount;
        bool success = true;
        while (count < size){
            //
