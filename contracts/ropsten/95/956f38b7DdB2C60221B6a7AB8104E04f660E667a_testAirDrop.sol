...
contract testAirDrop {
    ...
    function claim(uint256 index, address account, uint256 _amount, bytes32[] calldata merkleProof) external {
        require(!isClaimed(index), 'testAirDrop: Drop already claimed.');

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index, account, _amount));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), 'testAirDrop: Invalid proof.');

        // Mark it claimed and send the token.
        uint256 amount = _amount * 1e18;
        _setClaimed(index);
        require(IERC20(token).transfer(account, amount), 'testAirDrop: Transfer failed.');
