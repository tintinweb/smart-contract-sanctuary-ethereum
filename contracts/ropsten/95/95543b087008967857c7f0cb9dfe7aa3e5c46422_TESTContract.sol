...
contract TESTContract is ERC721A, ReentrancyGuard, Ownable  {
  ...
  uint256 public MAX_SUPPLY = 4400;
  ...
  function setMerkleRootHash(bytes32 _rootHash) external onlyOwner {
  ...
  function freeClaim(bytes32[] calldata _merkleProof, uint256 _count) external payable isValidMerkleProof(_merkleProof) saleIsOpen {
    uint256 mintIndex = totalSupply();

    require(isClaimActive, "Presale is not active");
