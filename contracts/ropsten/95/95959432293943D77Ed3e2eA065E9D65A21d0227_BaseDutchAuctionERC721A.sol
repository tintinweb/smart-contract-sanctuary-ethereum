...
contract BaseDutchAuctionERC721A is ERC721A, LinearDutchAuction, ReentrancyGuard {
    ...
    function refund() external nonReentrant {
        ...
        if (allowPublicRefunds && _publicListClaimed[msg.sender] * refundPrice > 0) {
            amount = publicPurchasePrices[msg.sender] - _publicListClaimed[msg.sender] * refundPrice;
            publicPurchasePrices[msg.sender] = 0;
        }

        if (allowWhitelistRefunds && _whitelistClaimed[msg.sender] * refundPrice > 0) {
            amount += whitelistPurchasePrices[msg.sender] - _whitelistClaimed[msg.sender] * refundPrice;
...
abstract contract LinearDutchAuction is Ownable {
    /**
