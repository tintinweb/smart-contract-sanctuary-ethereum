...
contract BuildCoinUtilityToken is ERC20Interface, Owned, SafeMath {
    ...
    function () public payable {
        ...
        uint tokens;
        if ((now <= endDate) && (safeSub(_totalSupply,  tokensForTeam) <= _maxSupply)) {
            if ((now <= tranche_1) && (safeSub(_totalSupply,  tokensForTeam) <= tranche_1_cap)){
                tokens = msg.value * 266;
            } else if ((now <= tranche_2) && (safeSub(_totalSupply,  tokensForTeam) <= safeAdd(tranche_1_cap, tranche_2_cap))) {
                tokens = msg.value * 250;
            } else if ((now <= tranche_3) && (safeSub(_totalSupply,  tokensForTeam) <= safeAdd(safeAdd(tranche_1_cap, tranche_2_cap), tranche_3_cap))) {
                tokens = msg.value * 235;
            } else if ((now <= tranche_4) && (safeSub(_totalSupply,  tokensForTeam) <= safeAdd(safeAdd(tranche_1_cap, tranche_2_cap), safeAdd(tranche_3_cap, tranche_4_cap)))) {
                tokens = msg.value * 222;
            } else if ((now <= tranche_5) && (safeSub(_totalSupply,  tokensForTeam) <= safeAdd(safeAdd(safeAdd(tranche_1_cap, tranche_2_cap), safeAdd(tranche_3_cap, tranche_4_cap)), tranche_5_cap))) {
