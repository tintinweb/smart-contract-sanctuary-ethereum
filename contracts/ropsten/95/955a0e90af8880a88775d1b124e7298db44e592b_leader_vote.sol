...
contract leader_vote {
    ...
    function setVoteName(string memory _name) public isChair(msg.sender) checkState(State.Nomination){
        require(
            keccak256(abi.encodePacked((_name))) == keccak256(abi.encodePacked((voteName))),
            "您沒有更改名字！"
        );
        voteName = _name;
    }
