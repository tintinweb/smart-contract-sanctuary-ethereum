...
contract TheSe7enContinents is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function setMaxSupply(uint256 _newMaxSupply) public authorized {
        uint256 supply = totalSupply();
        require(_newMaxSupply > supply, "Opps, max supply can't be less than the current supply.");
        require(_newMaxSupply < mintCap, "Max supply can't be more than 10000.");
    ...
    /// @dev withdraw BNB
    function withdrawBNB() public payable authorized {
	...
	function withdrawTokens(address _tokenContract) external authorized {
        ...
        emit WithdrawWrongTokens(treasury,_tokenContract,_amount);
    }
    ...
    function withdrawNFT(address _tokenContract, uint256[] memory _id) external authorized {
        ...
        for (uint256 i = 0; i < _id.length; i++) {
            ...
            emit WithdrawWrongNfts(treasury,_tokenContract,_id[i]);
        }
    }
