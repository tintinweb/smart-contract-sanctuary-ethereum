...
// File: contracts/token/ExchangeableToken.sol
...
contract ExchangeableToken is BurnableToken {
    ...
    event Exchange(address indexed exchanger, uint256 amount);

    /**
    * @dev The ExchangeableToken constructor sets the target token for future exchanges.
    * @param _targetToken The address of token that can be exchanged for this one.
    */
    function ExchangeableToken(address _targetToken) public {
    ...
    */
    function exchange() public returns (bool) {
        ...
        burn(amount); // burn all source (this) tokens
        // mint same amount of target tokens, revert all if not minted
        assert(targetToken.mint(msg.sender, amount));
        
        Exchange(msg.sender, amount);
        return true;
    }
...
contract IcoTokenMock is ExchangeableToken, StandardToken, Ownable {
    ...
    function IcoTokenMock(address _targetToken) public ExchangeableToken(_targetToken) {
