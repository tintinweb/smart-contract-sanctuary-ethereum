...
contract TOKENNAME is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x252C0fDa7d557a5cB80FAD0A31218CAa8f805C34);
    address payable public developmentAddress = payable(0x252C0fDa7d557a5cB80FAD0A31218CAa8f805C34);
    address payable public charityAddress = payable(0x252C0fDa7d557a5cB80FAD0A31218CAa8f805C34);
    address payable public vaultRewardAddress = payable(0x252C0fDa7d557a5cB80FAD0A31218CAa8f805C34);
    ...
    function setLiquidityAddress(address payable newLiquidityWallet)
        public
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (
            ...
            if (_abmode == true) {
                require(
                    bots[from] != true,
                    "No bot transactions during antibotmode."
                );
            }
                    ...
                    if (cooldown[to] > block.timestamp) {
                        if (cooldown[to] > block.timestamp + 28 minutes) {
                            ...
                            if (cooldown[to] > block.timestamp + 22 hours) {
                                require(
                                    amount <= _bMaxTxAmount / 4,
                                    "Antibot/whale mode for first few hours: Transfer amount exceeds max tx amount."
                                );
                            }
                        } else {
                ...
                if (_cooldownMode == true) {
                    if (cooldown[from] > 10 minutes) {
                        require(
                            ...
                            "Antibotmode active for first few hours: sell cooldown for your address is not elapsed. Sell cooldown is caused by repeat purchases, if you did not repeat purchase just wait 10 minutes."
                        );
                    }
                }
                require(
        ...
        if (
            ...
            if (to == uniswapV2Pair) {
                ...
                if (buyBackEnabled && balance > 0) {
                    ...
                    // Min = 80% of max
                    uint256 _bBSLimitMin = buyBackSellLimit
                    ...
                    uint256 _bBSLimit = _bBSLimitMin +
                        ...
                        ) % (buyBackSellLimit - _bBSLimitMin + 1));
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(
            charityAddress,
            convertedBalance.div(_liquidityFee).mul(charityDivisor)
        );
        // Send to Development address
        transferToAddressETH(
            developmentAddress,
            convertedBalance.div(_liquidityFee).mul(developmentDivisor)
        );
        // Send to Marketing address
        transferToAddressETH(
            marketingAddress,
            convertedBalance.div(_liquidityFee).mul(marketingDivisor)
        );
    }
    ...
    function checkIfOnBotList(address _address)
        public
        view
        onlyOwner
        returns (bool)
    ...
    function manualSwapAndLiquifyBNB(uint256 bnbLiquifyAmount)
        public
        lockTheSwap
        onlyOwner
    {
        ...
        uint256 newTokenBalance = balanceOf(address(this)).sub(
            initialTokenBalance
        );
