...
contract TrovesBase is Ownable {
    using SafeMath for uint;
    ...
    uint public constant DAY_TO_SECENDS = 86400;
    ...
    // ---- vault variables with init ----
    uint public MCR = DECIMAL_PRECISION / 10 * 9;   // 90%
    uint public MaxVCR = DECIMAL_PRECISION / 10 * 98;   // 98%
    uint public APY = DECIMAL_PRECISION / 100 * 2;  // 2%
    uint public redLiquidationFee = DECIMAL_PRECISION / 100 * 3;   // 3%
    uint public orangeLiquidationFee = DECIMAL_PRECISION / 100 * 5;   // 3%

    uint8 public constant BORROW_OPTYPEID = 1;
    uint8 public constant LEVERAGE_OPTYPEID = 2;
...
import "./TrovesBase.sol";

import "hardhat/console.sol";


contract TrovesManager is TrovesBase, ITrovesManager {
    using SafeMath for uint;
    ...
    // ---- contract variables ----
    // User => borrow/leverage-code => Trove
    // mapping(address => mapping(uint8 => Trove)) public troves;
    mapping(address => Trove) public borrowTroves;
    address[] public borrowTroveOwners;

    mapping(address => Trove) public leverageTroves;
    address[] public leverageTroveOwners;

    // Valut total collaterals and debts
    ...
    function setAddresses(
        ...
        address _leverageOperationsAddress,
        address _redeemOperationsAddress,
        address _liquidationOperationAddress
    ) external onlyOwner {
        oracle = IOracle(_oracleAddr);
        borrowOperationAddr = _borrowerOperationsAddress;
        leverageOperationAddr = _leverageOperationsAddress;
        redeemOperationAddr = _redeemOperationsAddress;
        liquidationOperationAddr = _liquidationOperationAddress;
    }
    ...
    function closeBorrowTrove(address _borrower, uint _num) external override {
        _requireCallerIsAllowed();
        return _closeBorrowTrove(_borrower, Status(_num));
    }

    function _closeBorrowTrove(address _borrower, Status status) internal {
        borrowTroves[_borrower].status = status;
        borrowTroves[_borrower].coll = 0;
        borrowTroves[_borrower].debt = 0;
        borrowTroves[_borrower].startTime = 0;
        borrowTroves[_borrower].margin = 0;
    }

    // -- Trove about leverage
    function setLeverageTroveStatus(address _borrower, uint _num)
        external
        override
    {
        _requireCallerIsAllowed();
        leverageTroves[_borrower].status = Status(_num);
    }

    function increaseLeverageTroveColl(address _borrower, uint _collIncrease)
        external
        override
        returns (uint)
    {
        _requireCallerIsAllowed();
        uint newColl = leverageTroves[_borrower].coll.add(_collIncrease);
        leverageTroves[_borrower].coll = newColl;

        totalColls = totalColls.add(_collIncrease);
        return newColl;
    }

    function decreaseLeverageTroveColl(address _borrower, uint _collDecrease)
        external
        override
        returns (uint)
    {
        _requireCallerIsAllowed();
        uint newColl = leverageTroves[_borrower].coll.sub(_collDecrease);
        leverageTroves[_borrower].coll = newColl;
    ...
    function increaseLeverageTroveDebt(address _borrower, uint _debtIncrease)
        external
        override
        returns (uint)
    {
        _requireCallerIsAllowed();
        uint newDebt = leverageTroves[_borrower].debt.add(_debtIncrease);
        leverageTroves[_borrower].debt = newDebt;

        totalDebts = totalDebts.add(_debtIncrease);
        return newDebt;
    }

    function decreaseLeverageTroveDebt(address _borrower, uint _debtDecrease)
        external
        override
        returns (uint)
    {
        _requireCallerIsAllowed();
        uint newDebt = leverageTroves[_borrower].debt.sub(_debtDecrease);
        leverageTroves[_borrower].debt = newDebt;
    ...
    function setLeverageTroveStartTime(address _borrower, uint _timestamp)
        external
        override
    {
        _requireCallerIsAllowed();
        leverageTroves[_borrower].startTime = _timestamp;
    }

    function increaseLeverageTroveMargin(
        address _borrower,
        uint _marginIncrease
    ) external override returns (uint) {
        _requireCallerIsAllowed();
        uint newMargin = leverageTroves[_borrower].margin.add(_marginIncrease);
        leverageTroves[_borrower].margin = newMargin;
        return newMargin;
    }

    function decreaseLeverageTroveMargin(
        address _borrower,
        uint _marginDecrease
    ) external override returns (uint) {
        _requireCallerIsAllowed();
        uint newMargin = leverageTroves[_borrower].margin.sub(_marginDecrease);
        leverageTroves[_borrower].margin = newMargin;
        return newMargin;
    }

    function closeLeverageTrove(address _borrower, uint _num)
        external
        override
    {
        _requireCallerIsAllowed();
        return _closeLeverageTrove(_borrower, Status(_num));
    }

    function _closeLeverageTrove(address _borrower, Status status) internal {
        leverageTroves[_borrower].status = status;
        leverageTroves[_borrower].coll = 0;
        leverageTroves[_borrower].debt = 0;
        leverageTroves[_borrower].startTime = 0;
        leverageTroves[_borrower].margin = 0;
    }

    // ---- Borrow Trove property getters ----
        ...
        returns (uint)
    {
        ...
        return _baseDebt.div(APY).div(interestDays);
    }
    ...
    // ---- Leverage Trove property getters ----
    function getLeverageTroveMargin(address _borrower)
        external
        view
        override
        returns (uint)
    {
        return leverageTroves[_borrower].margin;
    }

    function getLeverageTroveDebt(address _borrower)
        external
        view
        override
        returns (uint)
    {
        return leverageTroves[_borrower].debt;
    }

    function getLeverageTroveInterest(address _borrower)
        external
        view
        override
        returns (uint)
    {
        require(
            block.timestamp >= leverageTroves[_borrower].startTime, 
            "TrovesManager: trove start timestamp lt block now"
        );
        return _calcInterest(leverageTroves[_borrower].debt, leverageTroves[_borrower].startTime);
    }

    function getLeverageTroveColl(address _borrower)
        external
        view
        override
        returns (uint)
    {
        return leverageTroves[_borrower].coll;
    }

    function getLeverageTroveStatus(address _borrower)
        external
        view
        override
        returns (uint)
    {
        return uint(leverageTroves[_borrower].status);
    }

    function getLeverageTroveStartTime(address _borrower)
        external
        view
        override
        returns (uint)
    {
        return leverageTroves[_borrower].startTime;
    }

    function getVCR(uint _price) external view override returns (uint) {
        return _getVCR(_price);
    }

    function _getVCR(uint _price) internal view returns (uint) {
        if (totalColls == 0 || _price == 0) {
            return 0;
        }
        return ArchimedesMath._computeCR(totalColls, totalDebts, _price);
    }

    function checkRecoveryMode(uint _price)
        external
        view
        override
        returns (bool)
    ...
    function _checkRecoveryMode(uint _price) internal view returns (bool) {
        uint VCR = _getVCR(_price);

        return VCR > MaxVCR;
    }

    function getOraclePrice() external view override returns (uint _price) {
        (, _price) = oracle.get();
    }

    function getTotalDebts() external view override returns (uint) {
        return totalDebts;
    }

    function getTotalColls() external view override returns (uint) {
        return totalColls;
    }

    // ---- adjust system vault params ----
    function adjustVaultMCR(uint _value) external onlyOwner {
        MCR = _value;
    }

    function adjustVaultMaxVCR(uint _value) external onlyOwner {
        MaxVCR = _value;
    }

    function adjustVaultAPY(uint _value) external onlyOwner {
        APY = _value;
    }
    ...
    function updateBaseRateFromRedemption(
        ...
        uint redeemedUSDAFraction = _CollDrawn.mul(_price).div(
            _totalUSDASupply
        );
    ...
    function computeNominalCR(uint _coll, uint _debt)
        ...
        returns (uint)
    {
        return ArchimedesMath._computeNominalCR(_coll, _debt);
    }

    // ---- require function to reuse ----
    ...
    function _requireCallerIsLeverageOperations() internal view {
        require(
    ...
    function _requireCallerIsBorrowOrLeverage() internal view {
        require(
            msg.sender == borrowOperationAddr ||
                msg.sender == leverageOperationAddr,
    ...
    function _requireValidMaxFeePercentage(uint _maxFeePercentage)
        internal
        pure
    {
        require(
    ...
    function _requireBorrowTroveIsActive(address _borrower) internal view {
        require(
            borrowTroves[_borrower].status == Status.active,
