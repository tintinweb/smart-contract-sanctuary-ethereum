...
contract VoxToken is ERC20Burnable, Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.UintSet; 

    IERC721Enumerable public immutable voxelVilleContract;
    ...
    constructor(        
        ...
        voxelVilleContract = IERC721Enumerable(_voxelVilleContract);

        _mint(0xDB9cCBE822cA5Dd2605E60610A9174DaB5dF9Ce5, 300000000000000000000000);

        _pause();        
    }     

    function stakePlots(uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        require(tokenIds.length <= 40 && tokenIds.length > 0, "Stake: amount prohibited");

        for (uint256 i; i < tokenIds.length; i++) {
            require(voxelVilleContract.ownerOf(tokenIds[i]) == msg.sender, "Stake: sender not owner");

            voxelVilleContract.safeTransferFrom(msg.sender, address(this), tokenIds[i]);
    ...
    function unstakePlots(uint256[] calldata tokens) external whenNotPaused nonReentrant {
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            voxelVilleContract.safeTransferFrom(address(this), msg.sender, tokens[i]);
        }
    ...
    function claimStakingRewards(uint256[] calldata tokens) external whenNotPaused {
        require(tokens.length > 0, "no resident id given");
    ...
    function claimHolderRewards() external whenNotPaused {
        ...
        for(uint256 i; i < voxelVilleContract.balanceOf(msg.sender); i++) {

            uint256 tokenId = voxelVilleContract.tokenOfOwnerByIndex(msg.sender, i);
    ...
    function stakedPlotsOf(address account) external view returns (uint256[] memory) {
