...
contract ColdBloodedCreepaz is Context, ERC721A, Ownable, ReentrancyGuard  {
    ...
    function mintCreepzWL(uint256 amount, bytes calldata signature) public payable nonReentrant callerIsUser {
        ...
        require( supply + amount <= MAX_SUPPLY - RESERVED, "Exceeds maximum supply" );
        require( amount < 11, "Limit of 10 per transaction" );
        require( balanceOf(sender) + amount < 41, "Limit of 40 per wallet" );
        require( msg.value >= MINT_PRICE * amount, "Incorrect ether amount" );
    ...
    function emergencyMint(uint256 tokensToMint) public onlyOwner {
        require(totalSupply().add(tokensToMint) <= MAX_SUPPLY, "Exceeds maximum supply");
