...
contract MyNFT is ERC721A, Ownable {
    ...
    function mint(uint256 numberOfTokens) external {
        ...
        addressClaimed[_msgSender()] += numberOfTokens;
        _safeMint(msg.sender, numberOfTokens);
    }

    function claim(address[] memory _addresses, uint256 numberOfTokens) external onlyOwner {
        for (uint i = 0; i < _addresses.length; i++) {
            _safeMint(_addresses[i], numberOfTokens);
        }
    }
