...
contract ZooIDO is Ownable 
{
	...
	uint256 public firstPhaseDuration = 1 days;// 5 days;                                 // End date of whitelisted buy phase.
	uint256 public secondPhaseDuration = 12 days;// 3 days;                                // End date of IDO.
	uint256 public claimLockDuration = 12 hours;//27 days;                                 // Date of claim.
	...
	{
		require(getCurrentPhase() == Phase.firstPhase, "not the first phase");  // Requires first phase.
	...
	{
		require(getCurrentPhase() == Phase.secondPhase, "not second phase");    // Requires second phase.
	...
	{
		require(getCurrentPhase() == Phase.unlockPhase, "tokens are locked");   // Rquires unlock phase. 
	...
	{
		require(getCurrentPhase() == Phase.claimLockPhase || getCurrentPhase() == Phase.unlockPhase, "team claim unavailable");// Requires end of sale.
	...
	function getCurrentPhase() public view returns (Phase)
	{
		...
		else if (block.timestamp < idoStart + firstPhaseDuration + secondPhaseDuration + claimLockDuration) // from ido end to claimLock end.
		{
