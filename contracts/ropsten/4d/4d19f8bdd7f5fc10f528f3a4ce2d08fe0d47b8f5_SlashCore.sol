...
contract SlashCore is ISlashCore, Ownable {
    ...
    function doMerchantDeposit(
        PaymentInfo memory info
    ) private returns (uint256 redundantAmount, uint256 cashBackAmount) {
        // Blacklisted token can not be used as paying token
        ...
        cashBackAmount = info.requiredAmount.mul(merchant.viewCashBackPercent()).div(10000);
        ...
        if (
            ...
            require(info.amountIn >= info.requiredAmount, "Insufficient amountIn_");
            ...
            IERC20(receiveToken).universalTransfer(
                info.account,
                info.amountIn.sub(info.requiredAmount).add(cashBackAmount)
            );
        } else {
            (address[] memory exchanges, uint256[] memory flags) = merchant
                .viewExchanges();
    ...
    function submitTransaction(
        PaymentInfo memory info
    ) external payable override returns (uint256 refTokBal, uint256 refFeeBal, uint256 cashBack) {
        ...
        if (true) {
            (refTokBal, cashBack) = doMerchantDeposit(
                info
            );
        } else {
            (refTokBal, cashBack) = doMerchantDepositContractCall(
                info
            );
        }
    }
