...
contract RopstenSwap is ZetaInteractor, ZetaReceiver, RopstenSwapErrors {
        ...
        payable
    {
        ...
        if (args.sellTokenAddress == address(0))
        ...
        isValidMessageCall(zetaMessage)
    {
        ...
        if (buyTokenAddress == address(0)) revert InvalidTokenAddress();
        ...
        (bool success, ) = _zeroxExchangeProxy.call(secondLegSwapCallData);
        ...
        if (buyTokenAddress == _ethAddress) {
            ...
            successTransfer = buyToken.transfer(receiverAddress, buyAmount);
            if (!successTransfer) revert ErrorTransferringTokens(buyTokenAddress);
        } else {
            successTransfer = receiverAddress.send(buyAmount);
            if (!successTransfer) revert ErrorTransferringEther();
        }
