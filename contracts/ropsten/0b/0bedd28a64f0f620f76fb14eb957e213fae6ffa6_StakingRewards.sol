...
abstract contract RewardsDistributionRecipient is Owned {
    ...
    function notifyRewardAmount(uint256 reward, uint256 rate) virtual external;
...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    IERC20 override public rewardsToken;
    IERC20 public  stakingToken;
    uint256 public periodFinish = 0;
    uint256 public rewardRate = 70;
    ...
    function rewardPerToken() public override view returns (uint256) {
        ...
        return
            rewardPerTokenStored.add(
                lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate.div(100)).mul(1e18) //.div(_totalSupply)
            );
    }
    ...
    function notifyRewardAmount(uint256 reward,uint256 rate) override external onlyRewardsDistribution updateReward(address(0)) {
    ...
    function updateRewardRate(uint256 rate) external onlyRewardsDistribution updateReward(address(0)) {
        require(rewardRate<=0,"Reward rate must be more than 0");
        uint256 oldRate = rewardRate;
        uint balance = rewardsToken.balanceOf(address(this));
