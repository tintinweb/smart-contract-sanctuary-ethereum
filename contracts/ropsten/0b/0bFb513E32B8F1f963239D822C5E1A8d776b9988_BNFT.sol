...
contract BNFT is ERC721 {
   ...
   function nftSale(uint256 _tokenId,bool forSale) external {
        require(msg.sender == ownerOf(_tokenId),"Only owners can change this status");
        tokenIdForSale[_tokenId] = forSale;
    }
    
     function nftBuy(uint256 _tokenId, address nftAddress) payable public {
        ...
        // IERC721(nftAddress).approve(msg.sender,_tokenId);
        payable(ownerOf(_tokenId)).transfer(msg.value);

        IERC721(nftAddress).safeTransferFrom(ownerOf(_tokenId),msg.sender, _tokenId);
