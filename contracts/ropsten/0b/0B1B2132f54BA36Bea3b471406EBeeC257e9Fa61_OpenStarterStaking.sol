...
contract OpenStarterLibrary is Ownable {
    ...
    bool private isStakingAllowed = true;
    bool private isUnstakingAllowed = true;
    ...
    function getStakingAllowed() external view returns (bool) {
        return isStakingAllowed;
    }

    function setStakingAllowed(bool _allowed) external onlyStarterDev {
        isStakingAllowed = _allowed;
    }

    function getUnstakingAllowed() external view returns (bool) {
        return isUnstakingAllowed;
    }

    function setUnstakingAllowed(bool _allowed) external onlyStarterDev {
        isUnstakingAllowed = _allowed;
    }
...
contract OpenStarterStaking is ReentrancyGuard {
    ...
    struct AccountInfo {
        ...
        uint256 tier; // 0: Lottery 1: Navigator 2: Researcher 3: Conservator 4: Archeologist
    }

    mapping(address => AccountInfo) public accounts;

    uint256[5] public stakersCount; // Staker Numbers Array
    address[5] public stakingTokens; // Staking Tokens
    ...
    constructor(
        ...
        address _sosLpToken,
        address _startLpToken
    ) public {
        starterLibrary = OpenStarterLibrary(_starterLibrary);
        stakingTokens[0] = _sosLpToken;
        stakingTokens[1] = _startLpToken;
    }

    function stake(address _stakedToken, uint256 _amount) public nonReentrant {
        require(_amount > 0, "Invalid amount");
        require(starterLibrary.getStakingAllowed(), "staking disabled");
        require(
            ERC20(_stakedToken).balanceOf(msg.sender) >= _amount,
            "Invalid balance"
        );

        uint256 stakeTokenIndex = 5;
        for (uint256 i = 0; i < 5; i++) {
            if (stakingTokens[i] == _stakedToken) {
                stakeTokenIndex = i;
            }
        }
        require(stakeTokenIndex < 5, "invalid token");
        ...
        emit Staked(msg.sender, _stakedToken, _amount);
    }
    ...
    function unstake(address _stakedToken, uint256 _amount)
        ...
        nonReentrant
    {
        require(starterLibrary.getUnstakingAllowed(), "unstaking disabled");
        uint256 stakeTokenIndex = 5;
        for (uint256 i = 0; i < 5; i++) {
            if (stakingTokens[i] == _stakedToken) {
                stakeTokenIndex = i;
        ...
        uint256 burnAmount = _amount.mul(getLpBurnFee(msg.sender)).div(10000);
        if (burnAmount > 0) {
        ...
        updateUserTier(msg.sender);
        emit Unstaked(msg.sender, _stakedToken, _amount);
    }

    function getLpBurnFee(address _staker) public view returns (uint256) {
        AccountInfo memory account = accounts[_staker];
    ...
    function updateUserTier(address _staker) public returns (uint256) {
        AccountInfo storage account = accounts[_staker];
        uint256 userTier = 0;
        for (uint256 i = 0; i < 5; i++) {
        ...
        if (account.tier != userTier) {
            decreaseStakersCount(account.tier);
            increaseTiersCount(userTier);
            account.tier = userTier;
        }
    }

    function decreaseStakersCount(uint256 _tier) private {
        if (_tier > 0) {
            stakersCount[_tier] = stakersCount[_tier].sub(1);
        }
    }

    function increaseTiersCount(uint256 _tier) private {
        if (_tier > 0) {
            stakersCount[_tier] = stakersCount[_tier].add(1);
        }
    }

    function setBurnFee(uint256 _index, uint256 fee) external onlyStarterDev {
        burnFees[_index] = fee;
    }

    function setBurnCycle(uint256 _index, uint256 _cycle)
        external
        onlyStarterDev
    {
