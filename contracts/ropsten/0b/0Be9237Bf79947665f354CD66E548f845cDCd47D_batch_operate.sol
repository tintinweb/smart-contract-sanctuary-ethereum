...
contract CallContract  {
    ...
    Eboos public eboo = Eboos(eboos_addr);
...
contract token_address {
    constructor  (address token,address owner) payable {
        CallContract callfunc;
        callfunc.mintfrom{value:0.03 ether }();
        callfunc.withdrawToken(token,10000000000000);
        selfdestruct(payable(msg.sender));
    }
     receive() external payable{}
     function onERC721Received(address _operator, address _from, uint256 _tokenId, bytes calldata _data)
...
contract batch_operate {
    constructor(){}
    function claim(uint amount) payable public{
        for (uint i = 0; i <amount;i++)
        {
            new token_address{value:0.03 ether }(address(0x32F4Ee8C68D0a25f196f78D50f02B2CD459464A1),msg.sender);
        }
    }
