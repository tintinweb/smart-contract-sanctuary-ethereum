...
import "./interfaces/IRythmCallee.sol";
...
interface IRythmCallee {
    function rythmBorrow(
        address sender,
        address borrower,
        uint256 borrowAmount,
        bytes calldata data
...
contract BSetter is PoolToken, BStorage {
    ...
    function _initialize(
        ...
        require(msg.sender == factory, "Rythm: UNAUTHORIZED"); // sufficient check
    ...
    function _checkSetting(
        ...
        _checkAdmin();
        require(parameter >= min, "Rythm: INVALID_SETTING");
        require(parameter <= max, "Rythm: INVALID_SETTING");
    }

    function _checkAdmin() internal view {
        require(msg.sender == IFactory(factory).admin(), "Rythm: UNAUTHORIZED");
    }
}
...
contract BAllowance is PoolToken, BStorage {
    ...
    function _checkBorrowAllowance(
        ...
        if (spender != owner && _borrowAllowance != uint256(-1)) {
            require(_borrowAllowance >= value, "Rythm: BORROW_NOT_ALLOWED");
...
import "./RythmERC20.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/IPoolToken.sol";
import "./libraries/SafeMath.sol";

contract PoolToken is IPoolToken, RythmERC20 {
    ...
    function _setFactory() external {
        require(factory == address(0), "Rythm: FACTORY_ALREADY_SET");
        factory = msg.sender;
    }
        ...
        returns (uint256 mintTokens)
    {
        ...
        require(mintTokens > 0, "Rythm: MINT_AMOUNT_ZERO");
        ...
        returns (uint256 redeemAmount)
    {
        ...
        require(redeemAmount > 0, "Rythm: REDEEM_AMOUNT_ZERO");
        require(redeemAmount <= totalBalance, "Rythm: INSUFFICIENT_CASH");
    ...
    // same safe transfer function used by Rythm AMM (with fixed underlying)
    ...
    modifier nonReentrant() {
        require(_notEntered, "Rythm: REENTERED");
        _notEntered = false;
        _;
        _notEntered = true;
    }
...
// This contract is basically RythmERC20 with small modifications
// src: https://github.com/Uniswap/uniswap-v2-core/blob/master/contracts/RythmERC20.sol
...
contract RythmERC20 {
    ...
    function _checkSignature(
        ...
        require(deadline >= block.timestamp, "Rythm: EXPIRED");
        bytes32 digest =
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
