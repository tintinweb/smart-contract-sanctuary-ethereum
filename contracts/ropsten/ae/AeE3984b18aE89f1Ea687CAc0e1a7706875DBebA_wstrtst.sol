...
contract wstrtst is Context, Ownable {
    ...
    //
    constructor(IAzimuth _azimuth, uint32 _fee, IERC20Partial _wstr) 
        payable 
    {     
        azimuth = _azimuth;
        fee = _fee;
    ...
    //
    function purchase(uint32 _point, uint96 _amount)
        external
    {
        ...
        wstr.transferFrom(_msgSender(), seller, _amount);
        // DEDUCT EXCHANGE FEE
    ...
    //
    function safePurchase(uint32 _point, bool _unbooted, uint32 _spawnCount, bool _isProxyL2, uint96 _amount)
        external
    {
    ...
    {
        return wstr.balanceOf(_msgSender());
    }
    ...
    // 
    function getAssetInfo(uint32 _point) external view returns (address, uint96) {
