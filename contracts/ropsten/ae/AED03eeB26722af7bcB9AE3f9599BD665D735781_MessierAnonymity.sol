...
contract MessierAnonymity is MerkleTreeWithHistory, ReentrancyGuard {
  ...
  constructor() public {
    verifier = IVerifier(0x23254cFEF3481cF315cf4DbA71244b096d034C58);
    treasury = 0x045d3a7051dFb60deE5e8c76dE45a804676b0800;
    M87Token = IERC20(0xCe11f7078198928727Eb6239395358455432D386);
    token = IERC20(0x0000000000000000000000000000000000000000);
    messier_owner = 0x045d3a7051dFb60deE5e8c76dE45a804676b0800;
    lastRewardBlock = block.number;
    initM87Denomination = 0;
    coinDenomination = 10000000000000000;
    tokenDenomination = 0;
    numOfShares = 0;
    collectedFee = 0;
  ...
  function calcAccumulateM87() internal view returns (uint256) {
    ...
    uint256 remaining = M87Token.balanceOf(address(this)).sub(accumulateM87);
    if (remaining < reward) {
      reward = remaining;
    }
    return accumulateM87.add(reward);
  }
  ...
  */
  ...
  function withdraw(bytes calldata _proof, bytes32 _root, bytes32 _nullifierHash, address payable _recipient, address payable _relayer, uint256 _relayerFee, uint256 _refund) external payable nonReentrant {
    ...
    uint256 M87Deno = accumulateM87.div(numOfShares);
    if (M87Deno > 0) {
      accumulateM87 -= M87Deno;
