...
contract CGNToken is ERC20BasicToken {
	...
	string public constant name      = "CGN Platform"; //token name
	...
	/**
		* Set CGN Contract for this token to make sure called by our ICO contract
		* @param _cgnContract - ICO Contract address
	*/
	function setIcoContract(address _cgnContract) public onlyOwner {
		if (_cgnContract != address(0)) {
			cgnContract = _cgnContract;
		}
	}
	...
	*/
	function sell(address _recipient, uint256 _value) public  returns (bool success) {
		assert(_value > 0);
		require(msg.sender == cgnContract);
...
contract CGNContract is Ownable{
	...
	//constructor
	constructor(address _tokenAddress,address _phaseBonusAddress) public {
	    //address _tokenAddress
		tokenAddress = _tokenAddress;
		cgnToken = CGNToken(tokenAddress);
		phaseBonusAddress = _phaseBonusAddress;
		phaseBonus = PhaseBonus(_phaseBonusAddress);
		isFinalized=false;
	...
	*/
	...
	function createTokens(address _sender, uint256 _value) internal{
		...
		uint256 isPhaseBonus = phaseBonus.getCurrentPhase();
		require (isPhaseBonus!=0);
		
		uint256 tokens = SafeMath.mul(_value, tokenRate[isPhaseBonus]);
		uint256 tokenRemain = 0;
		if(isPhaseBonus==1){
			require (_value>=minBuy[isPhaseBonus][1] && _value<=maxBuy[isPhaseBonus]);
			require ((phaseSupply[isPhaseBonus]-phaseSold[isPhaseBonus])>=_value);
			cgnToken.sell(msg.sender, tokens);
			phaseSold[isPhaseBonus] = phaseSold[isPhaseBonus].add(tokens);
		}else if(isPhaseBonus==2){
		    require (_value>=minBuy[isPhaseBonus][0] && _value<=maxBuy[isPhaseBonus]);
			require ((phaseSupply[isPhaseBonus]-phaseSold[isPhaseBonus])>=_value);
			cgnToken.sell(msg.sender, tokens);
			phaseSold[isPhaseBonus] = phaseSold[isPhaseBonus].add(tokens);
		}
		ethFundDeposit.transfer(this.balance);
		accounts[msg.sender]=accounts[msg.sender].add(tokens);
	}
	
	function setPhaseSold(uint256 _phase,uint256 _value) public onlyOwner returns(bool){
	...
	function setMinBuy(uint256 _phase,uint256 _index,uint256 _value) public onlyOwner returns (bool){
	...
	function setMaxBuy(uint256 _phase,uint256 _value) public onlyOwner returns (bool){
	    require(_phase>0 && _value>0);
	...
	function setTokenRate(uint256 _phase,uint256 _value) public onlyOwner returns (bool){
	    require(_phase>0 && _value>0);
	...
	function setPhaseSupply(uint256 _phase,uint256 _value) public onlyOwner returns (bool){
	    require(_phase>0 && _value>0);
