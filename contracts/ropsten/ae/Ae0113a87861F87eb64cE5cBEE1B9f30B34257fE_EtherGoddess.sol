...
contract EtherGoddess is ERC721A, Ownable, ReentrancyGuard {
    ...
    bool isRarityMultiplierEnabled = false;

    mapping(uint256 => bool) public tokenStakedStatus;
    mapping(uint256 => uint256) public tokenRarityMultiplier;

    using EnumerableSet for EnumerableSet.UintSet;
    mapping(address => EnumerableSet.UintSet) private stakedTokensSet;
    ...
    function _stake(uint256 _tokenId) internal {
        ...
        stakedTokensSet[msg.sender].add(_tokenId);
    }
    ...
    function _unstake(uint256 _tokenId) internal {
        ...
        stakedTokensSet[msg.sender].remove(_tokenId);
    }
    ...
    function calculateAccumulatedReward(address _user)
        ...
        returns (uint256)
    {
        uint256 stakedBalance = stakedTokensSet[_user].length();
        ...
        if (isRarityMultiplierEnabled) {
            uint256 totalReward = 0;
            for (uint256 i = 0; i < stakedBalance; i++) {
                uint256 tokenId = stakedTokensSet[_user].at(i);
                uint256 rarity = tokenRarityMultiplier[tokenId];
                totalReward += (rewardRate * timeDelta * tokenRarityMultiplier[rarity]);
            }
            return totalReward;
        } else {
    ...
    function toggleRarityMultiplierStatus() public onlyOwner {
        isRarityMultiplierEnabled = !isRarityMultiplierEnabled;
    }

    // Batch set token of the same rarities
    function setTokenRarityMultiplier(
        uint256[] calldata _tokenIDs,
        uint256 _rarityMultiplier
    ) public onlyOwner {
        for (uint256 i = 0; i < _tokenIDs.length; i++) {
            tokenRarityMultiplier[_tokenIDs[i]] = _rarityMultiplier;
        }
    }
