...
contract Settlement is EIP712 {
    ...
    bytes32 private constant _OPYN_RFQ_TYPEHASH =
        keccak256(
            "RFQ(uint256 offerId, uint256 bidId, address bidderAddress, address bidToken, address offerToken, uint256 bidAmount, uint256 sellAmount, uint256 nonce)"
        );
    ...
    function settleOffer(uint256 _offerId, BidData calldata _bidData) external {
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                ...
                _bidData.bidAmount,
                _bidData.sellAmount,
                _useNonce(_bidData.bidderAddress)
            )
        );
        ...
        require(bidSigner == _bidData.bidderAddress, "Invalid bid signature");
    ...
    function checkBid(BidData calldata _bidData) external view returns (uint256, bytes32[] memory) {
        ...
        if (signerAddress != _bidData.bidderAddress) {
    ...
    function _getSigner(BidData calldata _bidData) internal view returns (address) {
        bytes32 structHash = keccak256(
            abi.encode(
                ...
                _bidData.bidAmount,
                _bidData.sellAmount,
                _nonces[_bidData.bidderAddress].current()
            )
        );
        bytes32 hash = _hashTypedDataV4(structHash);
