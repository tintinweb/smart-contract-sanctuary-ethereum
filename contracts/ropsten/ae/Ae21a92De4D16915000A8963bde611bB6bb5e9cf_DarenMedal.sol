    ...
    AccessControlEnumerableUpgradeable
{
    ...
    mapping(address => uint256) public staking; // tokenId => address1
    ...
    function getStakingSubCategory(address _owner, uint256 _expectedCategory)
        external
        view
        returns (uint256)
    {
        uint256 stakingSubCategory = 0;

        uint256 tokenId = staking[_owner];
        require(tokenId > 0, "No staking token.");

        uint256 category = _getCategory(tokenId);
        require(category == _expectedCategory, "Wrong category.");
        stakingSubCategory = _getSubCategory(tokenId);

        return stakingSubCategory;
    }
    ...
    {
        require(
            _darenTokenAddress != address(0),
            "Daren token address should not be 0."
        );
    ...
    function createOrder(uint256 _tokenId, uint256 _price) public {
        ...
        require(
            staking[msg.sender] != _tokenId,
            "Can not sell NFTs in staking."
        );
        address sender = _msgSender();
        require(
            ownerOf(_tokenId) == sender,
            "Only the owner of the NFT can create orders."
        );

        Order memory order = orders[_tokenId];
        if (order.price == _price && order.seller == sender) {
            require(false, "Can not create duplicated orders.");
        }
    ...
    function cancelOrder(uint256 _tokenId) public {
        address sender = _msgSender();
        Order memory order = orders[_tokenId];
        require(order.pk > 0, "Order was not created.");
        require(
    ...
    function executeOrder(uint256 _tokenId) public {
        ...
        require(goods.pk > 0, "Order was not published.");
    ...
    function setFeeRatio(uint256 _feeRatio, uint256 _feeRatioBase)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(
            _feeRatioBase > 0 && _feeRatio < feeRatioBase,
            "Invalid _feeRatioBase."
        );
        feeRatio = _feeRatio;
        feeRatioBase = _feeRatioBase;
    }
...
contract DarenToken is ERC20Upgradeable, AccessControlEnumerableUpgradeable {
        ...
        returns (bool)
    {
        ...
        if (feeRatio > 0) {
            uint256 fee = value.mul(feeRatio).div(feeRatioBase);
            value = value.sub(fee);
            _transfer(_msgSender(), feeTo, fee);
        }
...
interface IDarenMedal {
  ...
  event OrderCreated(uint256 tokenId, uint256 price, address seller);
