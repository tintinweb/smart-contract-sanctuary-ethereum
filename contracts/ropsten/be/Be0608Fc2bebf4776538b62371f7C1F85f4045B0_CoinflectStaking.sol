...
contract CoinflectStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    modifier updateReward(bool withdrawal) {
        ...
        if (totalSupply > 0) {
            ...
            accPerShare = accPerShare.add(reward.mul(1e12*(10**decimalsDiff)).div(totalSupply));
        }
        ...
        uint256 pending = penaltyFlag ? 0 : user.amount.mul(accPerShare).div(1e12*(10**decimalsDiff)).sub(user.rewardDebt);
        user.pendingRewards = user.pendingRewards.add(pending);
        
        _;
        
        user.rewardDebt = user.amount.mul(accPerShare).div(1e12*(10**decimalsDiff));
    ...
    function claimable(address _user) external view returns (uint256) {
        ...
        if (totalSupply > 0) {
            uint256 multiplier = Math.min(block.timestamp, endTime).sub(lastUpdateTime);
            uint256 reward = multiplier.mul(rewardRate);
            curAccPerShare = accPerShare.add(reward.mul(1e12*(10**decimalsDiff)).div(totalSupply));
        }
        uint amount = user.amount;
        return amount.mul(curAccPerShare).div(1e12*(10**decimalsDiff)).sub(user.rewardDebt).add(user.pendingRewards);
    }
