...
contract STAKING2 is Ownable {
    ...
    function isStaker() public view returns (bool) {
        for (uint256 i; i < details.length; i++) {
            if (details[i] == msg.sender) return exist;
        }
    }

    // total amount staked by the user
    function UserTotallockTime() public view returns (uint256) {
        return totallock[msg.sender];
    }
    ...
    function myBalance() public view returns (uint256) {
        return GETS(stakingToken).balanceOf(msg.sender);
    }
    ...
    function DetailOfStake()
        public
        view
        returns (uint256[] memory, uint256[] memory)
    {
        return (
            _stakeDetail[msg.sender].stakedTokens[msg.sender],
            _stakeDetail[msg.sender].lockedTime[msg.sender]
        );
    }
}