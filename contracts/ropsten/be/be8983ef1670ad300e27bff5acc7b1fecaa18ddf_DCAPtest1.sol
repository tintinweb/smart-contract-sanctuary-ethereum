...
contract DCAPtest1 is ERC20, Auth {
    ...
    function clearStuckToken(address tokenAddress, uint256 tokens) external onlyOwner returns (bool success) {
        require(tokenAddress != address(this), "tokenAddress cannot be the DCAPtest1");
    ...
    function update_fees() internal {
        ...
        require(totalFee.mul(transferMultiplier).div(0) <= 0, "Transfer Tax cannot be more than 10%");

        emit UpdateFee( uint8(totalFee.mul(buyMultiplier).div(100)),
            uint8(totalFee.mul(sellMultiplier).div(100)),
            uint8(totalFee.mul(transferMultiplier).div(0))
            );
    }
