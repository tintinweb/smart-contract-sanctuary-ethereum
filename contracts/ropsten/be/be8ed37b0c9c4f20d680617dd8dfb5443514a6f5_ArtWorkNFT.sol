...
//Nft collection contract for hungama assignment
contract ArtWorkNFT is ERC721Enumerable, Ownable {
    ...
    constructor(string memory _customURI) ERC721("ArtWorkNFT", "ART"){
    ...
    {   
        ...
        require(_maxTokensAvailable <= TEN_THOUSAND, "The maximum number of tokens available per collection must be less than or equal to 10 thousand");
    ...
    function changeAtype(uint256 _collectionNumber, string memory _Atype) public onlyOwner ifCollectionExists(_collectionNumber) ifCollectionNotLocked(_collectionNumber) {
        collections[_collectionNumber].Atype = _Atype;
    }
...
// To purchase nft
    ...
    function _mintNft(address _to, uint256 _collectionNumber) internal{
        uint256 tokenIdToMint = (_collectionNumber * TEN_THOUSAND) + (++collections[_collectionNumber].tokensMinted);
        _safeMint(_to, tokenIdToMint);
    }

    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        require(_exists(_tokenId), "ERC721Metadata: URI query for nonexistent token");
        string memory baseURI = collections[_tokenId/TEN_THOUSAND].baseURI;
        return bytes(baseURI).length > 0
...
//To get all token ids owned by owner.
...
//To get all the token ids present in collection. 
    function getTokenIdsInCollection(uint256 _collectionNumber) public view returns(uint256[] memory tokenIds) {
        ...
        for(uint256 index=0; index < totalMinted; index++){
            tokenIds[index] = (_collectionNumber * TEN_THOUSAND) + index + 1;
        }
    }
}