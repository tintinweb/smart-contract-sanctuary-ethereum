...
{   
    ...
    uint public _QRIndex = 333;                                                                         // QR Code Index
    ...
    mapping(uint => mapping(uint => uint)) public BookingsAvailable;                                // Returns Amount Of Bookings Available For Input TimeSlot | Golden Tokens
    mapping(uint => mapping(uint => mapping(uint => uint))) public BookingsAvailableArtists;        // Returns Amount Of Bookings Available For Input TimeSlot | Artists
    ...
    { 
        ...
        ArtistMintPasses[_CurrentCityIndex][7] = address(0);
        ArtistMintPasses[_CurrentCityIndex][8] = address(0);
        ArtistMintPasses[_CurrentCityIndex][9] = address(0);
        ArtistMintPasses[_CurrentCityIndex][10] = address(0);
        ArtistMintPasses[_CurrentCityIndex][11] = address(0);
        ArtistMintPasses[_CurrentCityIndex][12] = address(0);
     ...
     **/
    function RedeemGT(uint TicketID, uint TimeSlot, bool Delegate) public nonReentrant 
    {
        ...
        ICTZN(CTZN).IncrementRewards(msg.sender, _RewardRates[5]); 
     ...
     **/
    function RedeemArtistPass(uint ArtistID, uint TicketID, uint TimeSlot, bool Delegate) public nonReentrant 
    {
        require(BookingsAvailableArtists[_CurrentCityIndex][ArtistID][TimeSlot] > 0, "LiveMint: TimeSlot Fully Booked");
        address ArtistMintPass = ArtistMintPasses[_CurrentCityIndex][ArtistID];
        ...
        BookingsAvailableArtists[_CurrentCityIndex][ArtistID][TimeSlot]--;
        ...
        ICTZN(CTZN).IncrementRewards(msg.sender, _RewardRates[6]);
     ...
     */
    function RedeemQR(uint TimeSlot, bytes32[] calldata Proof) public nonReentrant 
    {
        require(BookingsAvailable[_CurrentCityIndex][TimeSlot] > 0, "LiveMint: TimeSlot Fully Booked");
        require(readQREligibility(msg.sender, Proof), "LiveMint: User Is Not Eligible To Redeem QR"); 
        if(QRAllocation[_CurrentCityIndex][msg.sender] == 0) { require(!RedeemedQR[_CurrentCityIndex][msg.sender], "LiveMint: User Has Already Redeemed"); }
        else { QRAllocation[_CurrentCityIndex][msg.sender]--; }
        ...
        ICTZN(CTZN).IncrementRewards(msg.sender, _RewardRates[7]);
     ...
     **/
    function _LiveMint(uint TicketID) public onlyMinter  
    {
        ...
        if(DelegateStatusCitizen[_CurrentCityIndex][TicketID]) { ICTZN(CTZN).IncrementRewards(Recipient, _RewardRates[0]); }
        else { ICTZN(CTZN).IncrementRewards(Recipient, _RewardRates[1]); }
     ...
     * @dev IRL Minting Function Available Only At Bright Moments NFT Art Gallery 
     **/
    function _LiveMintArtist(uint ArtistID, uint TicketID) public onlyMinter  
    {
        ...
        if(DelegateStatusArtist[_CurrentCityIndex][ArtistID][TicketID]) { ICTZN(CTZN).IncrementRewards(Recipient, _RewardRates[3]); }
        else { ICTZN(CTZN).IncrementRewards(Recipient, _RewardRates[4]); }
    }
     ...
     * note: Slot is denoted in YYYY-MM-DD with no -'s (20220102) for Jan 2nd
     */
    function __ChangeTimeSlotsGoldenTokens(uint[] calldata Slots, uint[] calldata Amounts) external onlyOwner
    {
     ...
     /**
     * @dev Changes Multiple TimeSlots For Artists
     * note: Slot is denoted in YYYY-MM-DD with no -'s (20220102) for Jan 2nd
     */
    function __ChangeTimeSlotsArtists(uint[] calldata ArtistIDs, uint[] calldata Slots, uint[] calldata Amounts) external onlyOwner
    {
        require(Slots.length == Amounts.length && Amounts.length == ArtistIDs.length, "Arrays Must Match Length");
        for(uint i; i < ArtistIDs.length; i++)
        {
            BookingsAvailableArtists[_CurrentCityIndex][ArtistIDs[i]][Slots[i]] = Amounts[i];
        }
        emit TimeSlotsChangedArtists(ArtistIDs, Slots, Amounts);
    }
    ...
    /**
     * @dev Returns # At Available TimeSlots
     */
    function readTimeSlotsAvailable(uint[] calldata TimeSlots) public view returns(uint[] memory)
    {
        uint[] memory TimeSlotsAvailable = new uint[](TimeSlots.length);
        for(uint x; x < TimeSlots.length; x++)
        {
            TimeSlotsAvailable[x] = BookingsAvailable[_CurrentCityIndex][TimeSlots[x]];
        }
     ...
     * @dev Returns # At Available TimeSlots
     */
    function readTimeSlotsAvailableArtists(
        uint[] calldata ArtistIDs,
        ...
        for(uint x; x < ArtistIDs.length; x++)
        {
            require(ArtistIDs.length == TimeSlots[x].length, "Arrays First Indicies Must Be Equal Length");
            for(uint y; y < TimeSlots.length; y++)
            {
                TimeSlotsAvailable[x][y] = BookingsAvailableArtists[_CurrentCityIndex][ArtistIDs[x]][TimeSlots[x][y]];
            }
        }
        return TimeSlotsAvailable;
    }
    ...
    /**
     * @dev Returns Which Artist Mint Passes Have Been Redeemed
     */
    function readArtistPassesRedeemed(uint[] calldata ArtistIDs, uint[] calldata Ranges) public view returns(uint[][] memory)
    {
        uint[][] memory ArtistMintPassesRedeemed = new uint[][](ArtistIDs.length);
        for(uint x; x < ArtistIDs.length; x++)
        {
            uint[] memory PreFormattedArtistMintPassesRedeemed = new uint[](Ranges[x]);
            uint Counter;
            for(uint y; y <= Ranges[x]; y++) // Finds Artist Mint Passes That Have Been Redeemed
            {
                if(BrightListArtist[_CurrentCityIndex][ArtistIDs[x]][y] != address(0))
                {
                    PreFormattedArtistMintPassesRedeemed[Counter] = y;
                    Counter++;
                }
            }
            uint[] memory FormattedArtistMintPassesRedeemed = new uint[](Counter);
            for(uint z; z < Counter; z++) // Formats Array
            {
                FormattedArtistMintPassesRedeemed[z] = PreFormattedArtistMintPassesRedeemed[z];
            }
            ArtistMintPassesRedeemed[x] = FormattedArtistMintPassesRedeemed; // Assigns Formatted Array To ArtistID
        }
        return ArtistMintPassesRedeemed;
    }

    /**
     * @dev Returns Which Golden Token Indexes Have Been Redeemed
     */
    function readGoldenTokensRedeemed(uint Range) public view returns(uint[] memory)
    {
        uint[] memory PreFormattedGoldenTokensRedeemed = new uint[](Range);
        uint Counter;
        for(uint x; x <= Range; x++) // Finds Golden Tokens That Have Been Redeemed
        {
            if(readBrightListCitizen(x) != address(0))
            {
                PreFormattedGoldenTokensRedeemed[Counter] = x;
                Counter++;
            }
        }
        uint[] memory GoldenTokensRedeemed = new uint[](Counter);
        for(uint y; y < Counter; y++) // Formats Array
        {
            GoldenTokensRedeemed[y] = PreFormattedGoldenTokensRedeemed[y];
        }
        return GoldenTokensRedeemed;
    }

    /**
     * @dev Returns Indexes Of Minted Golden Tokens
     */
    function readGoldenTokensMinted(uint Range) public view returns(uint[] memory)
    {
        uint[] memory PreFormattedGoldenTokensMinted = new uint[](Range);
        uint Counter;
        for(uint x; x <= Range; x++) // Finds Golden Tokens That Have Been Redeemed
        {
            if(MintedCitizen[_CurrentCityIndex][x])
            {
                PreFormattedGoldenTokensMinted[Counter] = x;
                Counter++;
            }
        }
        uint[] memory GoldenTokensMinted = new uint[](Counter);
        for(uint y; y < Counter; y++) // Formats Array
        {
            GoldenTokensMinted[y] = PreFormattedGoldenTokensMinted[y];
        }
        return GoldenTokensMinted; 
    }

        /**
     * @dev Returns Which Artist Mint Passes Have Been Redeemed
     */
    function readArtistPassesMinted(uint[] calldata ArtistIDs, uint[] calldata Ranges) public view returns(uint[][] memory)
    {
        uint[][] memory ArtistMintPassesMinted = new uint[][](ArtistIDs.length);
        for(uint x; x < ArtistIDs.length; x++)
        {
            uint[] memory PreFormattedArtistMintPassesMinted = new uint[](Ranges[x]);
            uint Counter;
            for(uint y; y <= Ranges[x]; y++) // Finds Artist Mint Passes That Have Been Redeemed
            {
                if(MintedArtist[_CurrentCityIndex][ArtistIDs[x]][y])
                {
                    PreFormattedArtistMintPassesMinted[Counter] = y;
                    Counter++;
                }
            }
            uint[] memory FormattedArtistMintPassesRedeemed = new uint[](Counter);
            for(uint z; z < Counter; z++) // Formats Array
            {
                FormattedArtistMintPassesRedeemed[z] = PreFormattedArtistMintPassesMinted[z];
            }
            ArtistMintPassesMinted[x] = FormattedArtistMintPassesRedeemed; // Assigns Formatted Array To ArtistID
        }
        return ArtistMintPassesMinted;
    }
...
{
    address public immutable CTZN = 0x5FDcDAAf45dBe6AB9c21Be696357e0b236fE876A;
    uint[] public _RewardRates;

    /**
     * @dev Modifies The Reward Rate
     */
    function ModifyRewardRates(uint[] calldata RewardRates) external onlyCTZN
    {   
        for(uint x; x < RewardRates.length; x++)
        {
            _RewardRates[x] = RewardRates[x];
        }
    }
...
interface ICTZN 
{ 
    function IncrementRewards(address Recipient, uint Amount) external;
}
