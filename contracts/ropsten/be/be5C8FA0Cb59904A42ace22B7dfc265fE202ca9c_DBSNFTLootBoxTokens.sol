...
contract DBSNFTLootBoxTokens is ERC721Burnable, Ownable{
    ...
    */
     ...
     */
    function addSupply(uint256 _id, uint256 _amount) public onlyOwner returns(uint256[] memory){
        require(_id > 0 && _amount > 0, "DBSNFTLootBox: listbox param failed");

        DBSNFTLootBox box = DBSNFTLootBox(lootBox);
        box.addSupply(_id, _amount);
        return mint(_msgSender(), _id, _amount);
    }

    function getBox(uint256 _tokenid) public view returns(uint256){
        require(_tokenid > 0);
    ...
    function unpack(
        ...
        DBSNFTLootBox box = DBSNFTLootBox(lootBox);
        uint256 nftTokenId = box.unpack(tokens[_tokenId], _toAddress);
        burn(_tokenId);
        return nftTokenId;
    }
}
