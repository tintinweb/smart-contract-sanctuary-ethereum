...
contract NFT{
   ...
   string[] now_morse_text_list;
    ...
    function create(string memory text) public returns(string[] memory){
        ...
        string memory now_morse_text = '';

        string memory remain_morse_text = morse_code_text;
        uint range_number = bytes(morse_code_text).length/number_of_newline;

        uint line = 0;

        
        while (bytes(remain_morse_text).length >= number_of_newline){
            line += 1;
            now_morse_text = slice(morse_code_text,first,end);
            remain_morse_text = slice(morse_code_text,end,bytes(morse_code_text).length-end-1);
            first = end;
            end = first + number_of_newline;
            now_morse_text_list.push(now_morse_text);
        }

        return now_morse_text_list;
