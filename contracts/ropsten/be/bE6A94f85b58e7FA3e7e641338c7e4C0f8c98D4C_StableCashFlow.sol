...
contract RedirectTokens is SuperAppBase {
    ...
    function getRatio(uint numerator, uint denominator) pure internal returns (uint) {
        uint _numerator = numerator * 10 ** (4 + 1);
    ...
    function calculateFlowToken1(int96 _flowRate) internal returns (int96) {

        // token1 USDC
        // token2 DAI
        // 99000000 DAI , 100000000 USDC
        // ratio : 9.9e18
        // flowRate : 1000000 USDC / s
        // out : 9900000 DAI / s
        // 9900 / 100 = 99
        if (_flowRate < 0) {
            return int96(0);
        ...
        if (token1.balanceOf(address(this)) > token2.balanceOf(address(this))) {
            uint256 ratio = getRatio(token2.balanceOf(address(this)), token1.balanceOf(address(this)));
            return safeCastToInt96((ratio * (flowRate / 10 ** 4)) + flowRate);
        }

        uint256 ratio = getRatio(token1.balanceOf(address(this)), token2.balanceOf(address(this)));

        return safeCastToInt96(flowRate - (ratio * (flowRate / 10 ** 4)));

    }

    function calculateFlowToken2(int96 _flowRate) internal returns (int96) {
        if (_flowRate < 0) {
            return int96(0);
        }
        uint256 flowRate = uint256(int(_flowRate));
