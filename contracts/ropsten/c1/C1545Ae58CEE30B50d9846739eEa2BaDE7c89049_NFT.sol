...
contract NFT is ERC721URIStorage {
    ...
    constructor(address _marketplaceAddress) ERC721("Mohammad", "MHMD") {
    ...
    function mintToken(string memory tokenURI) public returns (uint256) {
        ...
        /*  marketplace should approve transact NFTs between users */
    ...
    function getMyTokens() public view returns (uint256[] memory) {
        ...
        uint256 numberOfMyTokens = balanceOf(msg.sender);
        uint256[] memory myTokensIds = new uint256[](numberOfMyTokens);

        uint256 currentIndex = 0;
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = i + 1;
            if (ownerOf(tokenId) != msg.sender) continue;
            myTokensIds[currentIndex] = tokenId;
            currentIndex += 1;
        }
