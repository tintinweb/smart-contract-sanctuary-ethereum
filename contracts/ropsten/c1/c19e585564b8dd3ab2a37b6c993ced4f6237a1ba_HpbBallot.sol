...
contract HpbBallot {
    ...
    bool public hasAutoVote=true;
   
    modifier onlyAutoVoteAfter{
        require(hasAutoVote == true);
        _;
    }
    // 增加候选者
    // add candidate
   ...
   */
   ...
   /**
    ...
    function  voteNoLock(
    	address candidateAddr,
    	uint num
    ) onlyVoteAfterStart onlyAutoVoteAfter public {
    ...
    function fechAllVoteResultPreStageByBlock(
       ...
       uint fromBlock,
        ...
        if(_block==safeAdd(voteStages[voteStageIndex].createSnapshotBlock,1000)){
            require(voteStages[voteStageIndex-1].candidateArray.length>1);
