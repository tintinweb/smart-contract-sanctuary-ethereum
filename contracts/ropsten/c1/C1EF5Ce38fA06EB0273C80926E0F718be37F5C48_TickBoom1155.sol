...
contract TickBoom1155 is ERC1155, EIP712, Ownable, Pausable {
    ...
    modifier ownersOnly(uint256 _id) {
        require(
            ...
            "TickBoom1155#ownersOnly: ONLY_OWNERS_ALLOWED"
        );
        _;
    }
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            ...
            "TickBoom1155#creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }

    function _isCreator(uint256 tokenId, address _address)
        internal
        view
        returns (bool)
    {
     ...
     */
    function maxSupply(uint256 tokenId) public pure returns (uint256) {
        return tokenId & SUPPLY_MASK;
    }
     ...
     * @param tokenId  Token IDs to change creator of
     */
    function setCreator(uint256 tokenId, address _to)
        public
        creatorOnly(tokenId)
    {
        require(_to != address(0), "TickBoom1155#setCreator: INVALID_ADDRESS.");
        _creatorOverride[tokenId] = _to;
        emit CreatorChanged(tokenId, _to);
    }
    ...
    function createBuyItem(
        ...
        require(
            price >= voucher.minPrice * quantity,
            "Insufficient funds to redeem"
        );

        if (!exists(voucher.tokenId)) {
            // first assign the token to the signer, to establish provenance on-chain
            mint(signer, voucher.tokenId, voucher.quantity, voucher.uri);
        } else {
            uint256 balance = super.balanceOf(signer, voucher.tokenId);
            require(balance >= quantity, "TickBoom1155: Not enough quantity");
        }
    ...
    function mint(
        ...
        require(checkTokenIdOwner(to, tokenId), "TokenId is Wrong");
        // check tokenId is exist
        require(!exists(tokenId), "TickBoom1155#mint: id exist");

        _beforeMint(tokenId, quantity);

        _mint(to, tokenId, quantity, bytes(data));
        tokenSupply[tokenId] = quantity;
        if (bytes(data).length > 1) {
            _setURI(tokenId, data);
        }
    }
        ...
        returns (uint256)
    {
        ...
        return
            _isCreator(_id, _owner) ? balance + _remainingSupply(_id) : balance;
    }
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(uint256 tokenId,uint256 minPrice,uint256 quantity,string uri)"
                        ),
                        voucher.tokenId,
                        voucher.minPrice,
                        voucher.quantity,
    ...
    function checkTokenIdOwner(address owner, uint256 tokenId)
        internal
        pure
        returns (bool)
    {
        return tokenCreator(tokenId) == owner;
    }

    function _beforeMint(uint256 _id, uint256 _quantity) internal view {
        require(
            _quantity <= _remainingSupply(_id),
            "TickBoom1155#_beforeMint: QUANTITY_EXCEEDS_TOKEN_SUPPLY_CAP"
        );
    }
