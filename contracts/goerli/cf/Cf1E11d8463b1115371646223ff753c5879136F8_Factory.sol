...
import "./Buffer2.sol";
contract Factory {
  event ContractDeployed(address indexed owner, address indexed group, bytes32 cid, uint index);
  address public immutable implementation;
  constructor() {
    implementation = address(new Buffer2 { salt: bytes32(uint(1)) }());
  }
  function genesis(bytes32 _root, bytes32 _cid, bool _encoding, uint _index) external returns (address) {
    ...
    buffer.initialize(_root, _cid, _encoding);
    emit ContractDeployed(msg.sender, clone, _cid, _index);
    return clone;
  }
}
