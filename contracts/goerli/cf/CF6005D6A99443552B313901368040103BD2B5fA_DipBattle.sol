...
contract DipBattle is Ownable {
    ...
    address private _chips; // = 0xc36cb218848F173148ff55f4dfC18f1540FB7475;
    ...
    constructor(address chipPowers, address chips) {
        _chipPowers = chipPowers;
        _chips = chips;
    }

    function setChipPowers(address chipPowers) external onlyOwner {
        _chipPowers = chipPowers;
    }
    ...
    {
        uint256[] memory allChips = IBlueChips(_chips).tokensOfOwner(owner); // new uint256[](tokenCount);
        uint256 unusedCount;
