...
contract TestLoanController is Ownable {
    function lock(
        ...
        DelegationOwner(_safeOwner).lockAsset(_nft, _id);
    }
...
contract DelegationOwner is ISignatureValidator, Initializable {
    ...
    modifier onlyDelegationController() {
        if (delegationController != msg.sender) revert DelegationOwner__onlyDelegationController();
        _;
    }
    ...
    function claimAsset(
        ...
        // safe should be owner
        if (IERC721(_asset).ownerOf(_assetId) != safe) revert DelegationOwner__claimAsset_assetNotOwned();
