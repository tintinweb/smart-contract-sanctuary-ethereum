...
contract MetaBrewSocietyMBSG is ERC721, Ownable, AccessControl, ERC721Burnable, ERC721Pausable
{
    ...
    uint public perWalletMaxTokens = 0;
    ...
    constructor() ERC721("MetaBrewSociety Genesis Gold Pass", "MBSG") {  //Name of Project and Token "Ticker"
    ...
    function mintW(uint256 amount, address to) external payable //This function is for usewwinter.com Mint
    {
        ...
        require(perWalletMaxTokens == 0 || balanceOf(to) + amount <= perWalletMaxTokens || to == winterWallets[0] || to == winterWallets[1], "reached per wallet limit!");
    ...
    function mint(uint256 amount, address to, bytes32[] calldata proof, uint256 discountTokenId) external payable
    {
        ...
        require(saleIsActive, "Sale not active");
        require(_tokenSupply.current() + amount <= currentSaleMaxTokens, "Current Supply reached");
        require(perWalletMaxTokens == 0 || balanceOf(to) + amount <= perWalletMaxTokens, "reached per wallet limit!");
    ...
    function mintReservedTokens(uint256 amount, address to) external onlyRole(RESERVED_MINT_ROLE){
