...
contract Marketplace is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => bool) public allowedNFTs;                                // allowed NFTs to be sold in this marketplace
    ...
    modifier onlyUnListed(address _nftAddress, uint256 _tokenId){
        if(listed[_nftAddress][_tokenId]){
    ...
    function listItem(
        ...
        nonReentrant
    {
        _listItem(_nftAddress, _tokenId, _price, _numberOfSeconds);
    }
        ...
        nonReentrant
    {
        ...
        for(uint256 i = 0; i < _tokensIds.length; ++i)
            _listItem(_nftAddress, _tokensIds[i], _prices[i], _numberOfSeconds[i]);
    }
    ...
    function _listItem(
        ...
        onlyUnListed(_nftAddress, _tokenId)
        onlyOwnerOfNFT(_nftAddress, _tokenId)
        checkPrice(_price)
        whenNotPaused 
    {
        if (_numberOfSeconds > listingMaxDuration){
    ...
    function editListing(address _nftAddress, uint256 _tokenId, uint256 _newPrice, uint256 _newDuration) external  
        ...
        whenNotPaused 
    {
        if (_newDuration > listingMaxDuration){
            revert DurationMaximumExceeded();
        }
    ...
    function batchCancelListing(address _nftAddress, uint256[] calldata _tokensIds) external {
        for(uint256 i = 0; i < _tokensIds.length; ++i)
            _cancelListing(_nftAddress, _tokensIds[i]);
    }
     ...
     */
    function buyListedItem(address _nftAddress, uint256 _tokenId, address _token) external nonReentrant {
     ...
     * @dev The owner of an NFT could unapprove the marketplace, which would cause this function to fail
     */
    function batchBuyListedItem(address _nftAddress, uint256[] calldata _tokensIds, address _token) external nonReentrant {
        for(uint256 i = 0; i < _tokensIds.length; ++i)
     ...
     * @dev The owner of an NFT could unapprove the marketplace, which would cause this function to fail
     */
    function buyListedItemWithLowerPrice(address _nftAddress, uint256 _tokenId, uint256 _price, uint256 _creationTime, bytes memory _signature, address _token) external nonReentrant {
    ...
    function batchBuyListedItemWithLowerPrice(address _nftAddress, uint256[] calldata _tokensIds, 
        uint256[] calldata _prices, uint256[] calldata _creationTime, bytes[] calldata _signature, address _token) external nonReentrant 
    {
        ...
        whenNotPaused  
    {
        ...
        emit NewOffer(msg.sender, _nftAddress, _tokenId, offers[_nftAddress][_tokenId].length - 1, _price, block.timestamp + _duration);
    }
        ...
        whenNotPaused 
    {
        ...
        if(auction.highestOffer.buyer != address(0)){
            if(_price <= auction.highestOffer.price || _price < auction.minPrice){
                revert PriceMinimumNotMet();
            }
        ...
        whenNotPaused nonReentrant 
    {
        ...
        if(auction.highestOffer.buyer == address(0)){
            IERC721(_nftAddress).safeTransferFrom(auction.seller, auction.seller, _tokenId);
    ...
    function getOffers(address _nftAddress, uint256 _tokenId) external view returns (Offer[] memory) {
        ...
        for(uint256 i = 0; i < offersActive.length; i++){
