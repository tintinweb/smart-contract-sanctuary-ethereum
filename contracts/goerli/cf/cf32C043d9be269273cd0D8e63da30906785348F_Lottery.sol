...
//Lottery by @itsalexey
//Enter the lottery with set ticket price (multiple entries allowed)
//If >s_target tickets sold -> raffle the nft
//If <s_target tickets sold -> raffle the usual every 24 hrs
//chainlink oracle -> random, auto exec
...
contract Lottery is Ownable, VRFConsumerBaseV2, AutomationCompatibleInterface, ERC721Holder {
    ...
    function enterRaffle() public payable {
        ...
        s_raffleBalance += (msg.value * (1 - ((s_feePercent) / 100)));
        s_collectedFunds += msg.value * (s_feePercent / 100);
    ...
    function fulfillRandomWords(
        ...
        if (s_raffleBalance >= s_target && s_nftRaffled.balanceOf(address(this)) >= 1) {
            ...
            s_nftRaffled.safeTransferFrom(address(this), recentWinner, s_tokenId);
            s_NFTState = NFTState.OPEN;
            s_lotteryState = LotteryState.OPEN;
            s_lastTimeStamp = block.timestamp;
            ...
            uint256 indexOfWinner = randomWords[0] % s_players.length;
            address payable recentWinner = s_players[indexOfWinner];
            uint256 payout = s_raffleBalance;
            s_players = new address payable[](0);
            s_recentWinner = recentWinner;
            s_raffleBalance = 0;
            (bool success, ) = recentWinner.call{value: payout}("");
            if (!success) {
    ...
    /* Used manually in case of the failure of the previous, automatic safeTransferFrom */
    function sendNFT(address nftAddress, address to, uint256 tokenId) external onlyOwner {
    ...
    function setNFTwithTarget(address nftContract, uint256 tokenId, uint256 target) external {
        ...
        if (s_NFTState != NFTState.OPEN) {
            revert Lottery__NFTReceived();
        }
        s_nftRaffled = IERC721(nftContract);
        s_tokenId = tokenId;
        s_target = target;
        s_NFTState = NFTState.RECEIVED;
    }
    ...
    function getNFT() public view returns (address, uint256) {
        return (address(s_nftRaffled), s_tokenId);
    }
