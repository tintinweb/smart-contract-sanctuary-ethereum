...
contract Gamble is IERC20, IERC20Metadata, AccessControl {
    ...
    event PotTransferredToSenderOnBuy(address indexed sender, uint256 amount);
    event PotTransferredToSenderOnSell(address indexed sender, uint256 amount);
    ...
    function setPair(address pairAddress) public onlyRole(ADMIN_ROLE) {
        pair = pairAddress;
    }
    ...
    constructor() {
        ...
        _symbol = "kek";
        _decimals = 18;
        _totalSupply = 123456789 * 10**uint256(_decimals);
        ...
        _house = address(0x55Ca5f7bB8Ceb2eea6a1CE4969069e4ca0BaAEA1);
        _fibonacciSequence = 1;
        _fibonacciIndex = 1;
    }

    // todo only for test
    function setLuckyNumber(uint256 number) public onlyRole(ADMIN_ROLE) {
        luckyNumber = number;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual nonReentrant {
        ...
        // Deduct the tax from the transferred amount
        ...
        // Add the pot amount to the internal pot balance
        _pot += potAmount;

        // Burn the remaining tax amount
        ...
        // Spin the cosmic wheel and see where the ball lands
        if (_pot > 0 && pair != address(0)) {
            _calculateNextFibonacciNumber();
            // Calculate the sum of timestamp, pot amount, and the current Fibonacci number
            uint256 num = block.timestamp + potAmount + _fibonacciSequence;

            // Keep only the last 9 decimals if the number has more
            luckyNumber = num % 10**9;
            uint256 test = luckyNumber % 10**9;

            emit Number(test);
            // if (num % _threshold1 == 0) {
            if (test % _threshold1 == 0) {
                // Transfer the pot balance to the house address
                _transferPotToHouse();
            // } else if (num % _threshold2 == 0) {
            } else if (test % _threshold2 == 0) {
              /**
                * when somebody buys msg.sender is the pair and receiver is the buyer
               **/
                // Transfer the pot balance to the wallet that initiated the transfer
                _transferPotToSender(sender, recipient);
            // } else if (num % _threshold3 == 0) {
            } else if (test % _threshold3 == 0) {
    ...
    function setHouse(address account) public onlyRole(ADMIN_ROLE) {
        require(account != address(0), "GambleToken: Invalid house address");

        // Check if the new house address already has the ADMIN_ROLE
        require(!hasRole(ADMIN_ROLE, account), "GambleToken: House address already has ADMIN_ROLE");

        // Grant the ADMIN_ROLE to the new house address
        grantRole(ADMIN_ROLE, account);

        // Revoke the ADMIN_ROLE from the previous house address, if set
        if (_house != address(0)) {
    ...
    function _transferPotToHouse() internal {
        require(_house != address(0), "GambleToken: House address not set");
        require(_pot > 0, "GambleToken: Pot balance is zero");
    ...
    function _transferPotToSender(address sender, address receiver) internal {
        require(sender != address(0), "GambleToken: Sender address is zero");
        require(_pot > 0, "GambleToken: Pot balance is zero");

        uint256 potAmount = _pot;
        _pot = 0;

        // happens if it's a swap
        if (sender == pair) {
            // Transfer the pot balance to the wallet that initiated the transfer
            _balances[receiver] += potAmount;
            emit PotTransferredToSenderOnBuy(receiver, potAmount);
        } else if (receiver == pair) {
            _balances[sender] += potAmount;
            emit PotTransferredToSenderOnSell(sender, potAmount);
        }
