...
// File: client/src/contracts/ProductToken/TokenFactory.sol
...
contract TokenFactory is Initializable, UUPSUpgradeable, OwnableUpgradeable {
  ...
  */
   ...
   *
  */
  function _UpdateBeacon(address _beacon) internal onlyOwner {
   ...
   *
  */
	function UpdateBeacon(address _beacon) public onlyOwner withSeraph {
		require( Address.isContract(_beacon), "Invalid Beacon address");
		beacon = IBeacon(_beacon);
		emit UpdatedBeacon(_beacon);
	}
  ...
  */
	function createToken(string memory _productName, bytes memory _data) external onlyOwner withSeraph{
