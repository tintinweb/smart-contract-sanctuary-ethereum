...
contract GoldenTokenSale is Ownable, ReentrancyGuard
{
    ...
    bytes32 public _RootFullSet = 0x0217cf7137ac9bcacc86e4b7228420d6d14b0c4d403e6a41bd7708bb29758231;
    bytes32 public _RootCitizen = 0x2e0d039308d61d7344c45c4fb5b9beb2046c4b212aad393a78a8ee30e3d9b880;
    bytes32 public _RootAmountFullSet = 0x4c4a37753662bbe9318d59966fe567313e31c65f71bddde35cdfab092cea8d54;
    ...
    uint public _CurrentTokenIndex = 111;
    uint public _TimestampSaleStart = 1675736400;
    uint public _TimestampEndFullSet = 1675737000;
    uint public _TimestampEndCitizen = 1675737600;
    ...
    address private constant _GOLDEN_TOKEN = 0xd2d840F1672D9423606d9cC4dbfd0C0334B5b5Dc;
    ...
    function Purchase (
        ...
        if(block.timestamp <= _TimestampEndCitizen)
        {
            ...
            { 
                ...
                require(AmountPurchasedFullSetWindow[msg.sender] + DesiredAmount <= MaxAmount, "Invalid Full Set Desired Amount");
            ...
            { 
                ...
                require(VerifyBrightList(msg.sender, _RootCitizen, Proof), "Not Eligible For CryptoCitizen Presale Window"); 
                require(AmountPurchasedCitizenWindow[msg.sender] + DesiredAmount <= MaxAmount, "Invalid Citizen Desired Amount");
        ...
        { 
            require(DesiredAmount <= 3, "Invalid Public Desired Amount. Must Be <= 3");
            _Price = _PricePublic * DesiredAmount;
            PresaleEnded = true; 
        }
        require(_AmountSold + DesiredAmount <= _MaxForSale, "Sale Ended");
        require(msg.value >= _Price && DesiredAmount > 0 && _Price > 0, "Invalid ETH Amount"); // Ensures ETH Amount Sent Is Correct
        if(msg.value > _Price) { __Refund(msg.sender, msg.value - _Price); } // Refunds The Difference
        for(uint y = _CurrentTokenIndex; y < (_CurrentTokenIndex + DesiredAmount); y++) 
        { 
            IERC721(_GOLDEN_TOKEN).transferFrom(_OPERATOR, msg.sender, _CurrentTokenIndex); 
        }
    ...
    function __NewRoots(
        bytes32 FullsetRoot,
        bytes32 CitizenRoot,
        bytes32 CitizenAmount,
        bytes32 FullsetAmount
    ) external onlyOwner {
        (_RootFullSet, _RootCitizen, _RootAmountFullSet, _RootAmountCitizen)
        =
    ...
    function WalletSaleInformation(
        ...
        return WalletSaleInfo(
            ...
            VerifyBrightList(Wallet, _RootFullSet, FullsetProof),
            VerifyBrightList(Wallet, _RootCitizen, CitizenProof),
