    ...
    Ownable
{
    ...
    //Address for the royalty wallet
     ...
     * @param   _uri  .
     * @param   _admin  .
     * @param   _royaltyAmount  .
     * @param   _royaltyWallet  .
     * @param   _usdc  .
     * @param   _marketPlace  .
     */
    function initialize(
    ...
    function upgradeOffer(
        ...
        emit offerUpgrade(_tokenId, _offerNumber, _newOfferedPrice, _isGlobal);
    }
    ...
    function transferShares(voucher.offer memory offerDetails, bool _isGlobal)
        external
    {
        require(balanceOf(msg.sender, offerDetails.tokenId) > 0, "OI"); //Offeree Invalid
        ...
        IERC20(usdc).transfer(msg.sender, (offerDetails.offeredPrice - fee));
        _safeTransferFrom(
            msg.sender,
            offerDetails.offerer,
            offerDetails.tokenId,
    ...
    function refundFunds(
        ...
        if (_isGlobal) {
            ...
            refundPrice = paidPrice - acquiredSharesprice;
            ...
            refundPrice = offers[msg.sender].singleOffers[_offerNumber];
            offers[msg.sender].singleOffers[_offerNumber] = 0;
            IERC20(usdc).transfer(msg.sender, refundPrice);
        }
        emit refundClaimed(_tokenId, _offerNumber, refundPrice, _isGlobal);
    }
    ...
    function lazyMintNFT(
        ...
        _mint(_buyer, _tokenId, _amount, "");
        _setURI(_tokenId, _uri);
        _setTokenRoyalty(_tokenId, royaltyWallet, royaltyAmount);
        tokenSupply[_tokenId] += _amount;
    }

    /**
     * @notice Function to add new operator for the platform
     * @param _newOperator is the new operators address
     */
    /**
     * @notice  .
     * @dev     .
    ...
    function addOperator(address _newOperator) external onlyOwner {
        require(_newOperator != address(0), "ZA"); //Zero Address
    ...
    function setMarketPlace(address _marketPlace) external onlyOperator {
        require(_marketPlace != address(0), "ZA"); // Zero Address
        marketPlace = _marketPlace;
    }
...
interface events {
    ...
    event offerAccepted(uint256 offerNumber, address offeree, uint256 quantity);
    event refundClaimed(
        uint256 _tokenId,
        uint256 _offerNumber,
    ...
    event redeemedToken(uint256 _tokenId);
}
