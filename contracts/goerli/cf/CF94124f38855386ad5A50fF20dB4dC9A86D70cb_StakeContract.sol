...
contract StakeContract is Ownable, ReentrancyGuard {
    ...
    IMai public Mai = IMai(0x65c177573B68a5c0FF9f9d73230D5b39BB80e939);

    event Stake(uint256 id);
    event Unstake(uint256 id);

    modifier preCheck(uint256[] memory _tokenIdList) {
        bool isC = isContract(msg.sender);
        require(!isC, "anti contract");
    ...
    function stake(uint256[] memory _tokenIdList) external preCheck(_tokenIdList) nonReentrant {
    ...
    function unstake(uint256 _obId) external nonReentrant {
