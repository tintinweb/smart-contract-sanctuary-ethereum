...
abstract contract CollectionContract {
  ...
  function transferFrom(address from, address to, uint256 nftId) public virtual;
  function totalSupply() public virtual returns(uint256);}  

contract RandomsStake is Ownable, IERC721Receiver {
  ...
  // struct to store the collection that are allowed to stake in the contract
  struct StakeAllowed {
    address owner;
  }
  ...
  function stake(uint256[] calldata nftIds, address collection) external {
    uint256 nftId;
    totalStaked += nftIds.length;
    CollectionContract c = CollectionContract(collection);
    for (uint i = 0; i < nftIds.length; i++) {
      nftId = nftIds[i];      
      require(stakeAllowedCollections[collection], "Collection not allowed to stake in this contract");
      require(c.ownerOf(nftId) == msg.sender, 'not an allowed collection owner');
      require(randomsStaked[nftId][collection].nftId == 0, 'this nft is already staked');
      c.transferFrom(msg.sender, address(this), nftId);
      emit randomStaked(msg.sender, nftId, block.timestamp);
  ...
  function _unstakeMany(address account, uint256[] calldata nftIds, address collection) internal {
    uint256 nftId;
    totalStaked -= nftIds.length;
    CollectionContract c = CollectionContract(collection);
    for (uint i = 0; i < nftIds.length; i++) {
      ...
      require(staked.owner == msg.sender, "not an nft staker");

      delete randomsStaked[nftId][collection];
      emit nftRemoved(account, nftId, block.timestamp);
      c.transferFrom(address(this), account, nftId);
    }
  }

  function claim(uint256[] calldata nftIds, address collection) external {
      _claim(msg.sender, nftIds, false, collection);
  }

  function claimForAddress(address account, address collection,uint256[] calldata nftIds) external {
      _claim(account, nftIds, false, collection);
  }

  function unstake(uint256[] calldata nftIds, address collection) external {
      _claim(msg.sender, nftIds, true, collection);
  }

  function _claim(address account, uint256[] calldata nftIds, bool _unstake, address collection) internal {
    uint256 nftId;
    uint256 earned = 0;
    ...
    for (uint i = 0; i < nftIds.length; i++) {
      nftId = nftIds[i];
      Stake memory staked = randomsStaked[nftId][collection];
      require(staked.owner == account, "not the nft owner");
    ...
    if (_unstake) {
      _unstakeMany(account, nftIds, collection);
    }
    emit retreiveTokens(account, earned);
  }

  function earningInfo(uint256[] calldata nftIds, address collection) external view returns (uint256[2] memory info) {
  ...
  function balanceOf(address account, address collection) public returns (uint256) {
    uint256 balance = 0;
    CollectionContract c = CollectionContract(collection);
  ...
  // should never be used inside of transaction because of gas fee
  function tokensOfOwner(address account, address collection) public returns (uint256[] memory ownerTokens) {
    CollectionContract c = CollectionContract(collection);
    uint256 supply = c.totalSupply();
...
contract RandomToken is ERC20, ERC20Burnable, Ownable {
  ...
  constructor() ERC20("Random", "RDM") { }

  function mint(address to, uint256 amount) external {
    require(controllers[msg.sender], "You are not random");
    _mint(to, amount);
  }
