...
contract ElkenToken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    bool private inSwap;
    uint256 internal _treasureFeeCollected;
    ...
    event Swap(uint swaped, uint sentToTreasure, uint sentToExecutive);
     ...
     * construction.
     */
    constructor(address _treasure_wallet, address  _executive_wallet) {
        // Editable
        ...
        treasure_wallet = _treasure_wallet;
        ...
        _treasureFee.push(100); // Buy  fee is  1.00%
        _treasureFee.push(100); // Sell fee is  1.00%
        _treasureFee.push(200); // P2P  fee is  2.00%
 ...
 modifier lockTheSwap() {
    ...
    function swap() private  lockTheSwap {
        ...
        uint totalCollected = _treasureFeeCollected + _executiveFeeCollected;
        uint amountToSwap = _treasureFeeCollected + _executiveFeeCollected;
        ...
        // Send to treasure DAO wallet
        uint amountTreasure = (amountFee * _treasureFeeCollected) / totalCollected;
        if(amountTreasure > 0) sendViaCall(payable(treasure_wallet), amountTreasure);
        ...
        _treasureFeeCollected = 0;
        _executiveFeeCollected = 0;

        emit Swap(totalCollected, amountTreasure, amountExecutive);
    ...
    function calculateFee(uint256 feeIndex, uint256 amount, bool extra_fee) internal returns(uint256) {
        if(extra_fee)
        {
            ...
            _treasureFeeCollected += extraFee;
            return extraFee;
        }
        uint256 treasureFee = (amount * _treasureFee[feeIndex]) / (10**(_feeDecimal + 2));
        uint256 executiveFee = (amount * _executiveFee[feeIndex]) / (10**(_feeDecimal + 2));
        
        
        
        _treasureFeeCollected += treasureFee;
        _executiveFeeCollected += executiveFee;

       

        return treasureFee + executiveFee;
    }
    ...
    function setTreasureWallet(address wallet)  external onlyOwner {
        treasure_wallet = wallet;
    }
    ...
    function setTreasureFees(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
        _treasureFee[0] = buy;
        _treasureFee[1] = sell;
        _treasureFee[2] = p2p;
    }
