...
contract SmartYield is ISmartYield, ReentrancyGuardUpgradeable {
    ...
    event BondRedeemed(address indexed owner, address indexed bond, uint256 amount, uint256 fee);
    ...
    function redeemBond(address _bond, uint256 _tokenAmount) external override nonReentrant defaultCheck {
        ...
        (uint256 _totalRedeem, uint256 _fee) = _redeem(_bond, _tokenAmount);
        ...
        emit BondRedeemed(msg.sender, _bond, _totalRedeem, _fee);
    }
    ...
    function rolloverBond(address _bond, uint256 _tokenAmount) external override nonReentrant defaultCheck {
        ...
        (uint256 claimable, uint256 _fee) = _redeem(_bond, _tokenAmount);
        IBond(_bond).burn(msg.sender, _tokenAmount);
        IBond(nextTerm).mintLocked(msg.sender, claimable);
        emit BondRolledOver(msg.sender, _bond, nextTerm, _tokenAmount, claimable, _fee);
    }
    ...
    function liquidateTerm(address _bond) external nonReentrant defaultCheck {
        ...
        if (nextTerm != address(0)) {
            bondData[nextTerm].realizedYield = _realizedYield;
            activeTerm = nextTerm;
        } else {
     ...
     */
    function _redeem(address _bond, uint256 _tokenAmount) internal returns (uint256, uint256) {
        TermInfo memory termInfo = bondData[_bond];
