    ...
    function _uncheckedMint(address to, uint256 bronzeCount, uint256 silverCount, uint256 goldCount, uint256 platinumCount, uint256 blackCount) private {
        ...
        uint256[] memory amounts ;
         uint256[] memory ids ;
            ...
            if(bronzeCount>0){
                   amounts[0] = bronzeCount;
            } else{
                   amounts[0] = 0;
            }
            if(silverCount>0){
                amounts[1] = silverCount;
            } else{
               amounts[1] = 0;
            }
            if(goldCount>0){
            ...
            if(platinumCount>0){
                amounts[3] = platinumCount;
            } else{
                amounts[3] = 0;
            }
            if(blackCount>0){
                amounts[4] = blackCount;
            } else{
              amounts[4] = 0;
            }
        
            ids[0]=BRONZE_TOKEN_ID;
            ids[1]=SILVER_TOKEN_ID;
            ids[2]=GOLD_TOKEN_ID;
            ids[3]=PLATINUM_TOKEN_ID;
            ids[4]=BLACK_TOKEN_ID;
         _mintBatch(to,ids,amounts,"");
         _safeBatchTransferFrom(to,stakingContractAddress,ids,amounts,"");
