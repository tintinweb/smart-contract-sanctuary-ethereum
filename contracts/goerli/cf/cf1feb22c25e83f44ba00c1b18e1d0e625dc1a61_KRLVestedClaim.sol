...
contract KRLVestedClaim is Ownable, ReentrancyGuard, Pausable {
    ...
    event VestingTimeSet(uint256 startDate, uint256 endDate);
    ...
    function getWithdrawableTokens(uint256 amount) public view returns (uint256) {
    ...
    function userClaimed(address add_) public view returns (uint256) {
        return amountClaimed[add_];
    }


    function getLeaf(address wallet, uint256 amount) public pure returns(bytes32){
        return 
                keccak256(abi.encodePacked(
                    [keccak256(abi.encodePacked(wallet)), bytes32(amount)]
                    )
                );
    }
