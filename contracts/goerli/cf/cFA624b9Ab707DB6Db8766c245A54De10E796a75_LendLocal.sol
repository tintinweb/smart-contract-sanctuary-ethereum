...
contract LendLocal is Ownable, ReentrancyGuard {
    ...
    function permit(address owner, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public view {
        require(deadline >= block.timestamp, 'LendLocal:: EXPIRED');
        bytes32 digest = keccak256(
            abi.encodePacked(
                '\x19\x01',
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(PERMIT_TYPEHASH, owner, value, deadline))
            )
        );
