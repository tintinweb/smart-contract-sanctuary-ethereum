...
// @title:  Playground/RECESS Frens
// @desc:   Frens, ready to play.
...
contract Frens is ERC721BaseTokenURI {
    ...
    uint256 public currentScholarshipMint = 0;
    uint256 public currentReferredMint = 0;
    uint256 public currentTokenId = 0;

    uint256 public maxScholarshipMint;
    uint256 public maxReferredMint;

    bytes32 public recessMerkleRoot;
    bytes32 public scholarshipMerkleRoot;
    bytes32 public referredMerkleRoot;
    ...
    modifier hasntClaimed(address addr) {
        require(!hasMintedRecess[addr], 'Already claimed your Fren!');
        _;
    }
    ...
    modifier validReferralMerkleProof(
        ...
        require(
            MerkleProof.verify(merkleProof, referredMerkleRoot, leafNode),
            'Invalid merkleProof.'
        );
        _;
    }

    modifier checkBelowMax(uint256 currentTokens, uint256 maxTokens) {
        require(currentTokens < maxTokens, 'No more Frens left for your mint type right now.');
        require(currentTokenId < MAX_SUPPLY, 'No more Frens left!');
        _;
    }
        ...
        validMerkleProof(merkleProof, recessMerkleRoot)
    {
        ...
        _mintFren(_msgSender());
    }

    function scholarshipMint(bytes32[] calldata merkleProof)
        external
        isRightState(State.RecessMint)
        hasntClaimed(_msgSender())
        checkBelowMax(currentScholarshipMint, maxScholarshipMint)
        validMerkleProof(merkleProof, scholarshipMerkleRoot)
    {
        hasMintedRecess[_msgSender()] = true;
        _mintFren(_msgSender());
        currentScholarshipMint++;
    }
    ...
    function referredMint(
        ...
        checkBelowMax(currentReferredMint, maxReferredMint)
    ...
    function _mintFren(address recipient) private {
        currentTokenId++;
    ...
    function setScholarshipMerkleRootAndAddToMax(bytes32 _merkleRoot, uint256 addToMax)
        external
        onlyOwner
    {
        scholarshipMerkleRoot = _merkleRoot;
        maxScholarshipMint += addToMax;
    }

    function setReferredMerkleRootAndAddToMax(bytes32 _merkleRoot, uint256 addToMax)
        external
        onlyOwner
    {
        referredMerkleRoot = _merkleRoot;
        maxReferredMint += addToMax;
    }
