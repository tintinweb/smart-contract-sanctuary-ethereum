...
contract NFTMarketSimple is ReentrancyGuard, MarketTools {
  ...
  constructor() MarketTools(getit()) {}

  // TODO: make nicer, somehow
  function getit() internal pure returns (address[] memory) {
...
abstract contract MarketTools is Ownable, Pausable {
  ...
  function transferNFT(
    ...
    if (nftContract.supportsInterface(InterfaceId_ERC721)) {
      IERC721(nftContract).safeTransferFrom(sender, receiver, nftTokenId);
    } else if (nftContract.supportsInterface(InterfaceId_ERC1155)) {
