...
contract OnChainBears is IOnChainBears, OwnableRoles, ERC721AQueryable {
    using Base64 for bytes;

    /// @dev Define `Trait` struct.
    struct Trait {
        string name;
        string value;
        string pixels;
    ...
    /// @dev Mints are free, tips are appreciated! (づ｡◕w◕｡)づ
    function mint(uint256 amount) external payable {
        ...
        /// Assign `_dna` values for each token to be minted.
    ...
    /// @notice Function used to add traits.
    /// @param id A trait identifier.
    /// @param traitData An array of trait data.
    ...
    /// @notice Function used to withdraw tips, if any, probably none.
    function withdraw() external onlyOwner {
        (bool ok, ) = msg.sender.call{value: address(this).balance}("");
        if (!ok) revert TransferFailed();
    }

    /// @notice Function used to return a URI for a specified `id`.
    ...
    /// @notice Function used to define trait weightings.
    ///
    ///  -------------------
    /// |  i  | Trait       |
    /// |-------------------
    ...
    function _defineWeights() internal {
        ...
        _weights[0] = [400, 700, 600, 1000, 300, 700, 600, 700, 700, 500, 600, 700, 600, 700, 500, 700];
        _weights[1] = [500, 200, 400, 825, 825, 825, 450, 825, 825, 825, 825, 825, 825, 825, 200];
        _weights[2] = [4750, 4750, 500];
        _weights[3] = [500, 500, 1500, 1000, 1500, 1500, 500, 500, 500, 1000, 500, 500];
        _weights[4] = [1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000];
        _weights[5] = [500, 750, 1125, 1125, 1125, 1125, 1000, 1000, 750, 500, 1000];

        // Sanity check to ensure weights are distributed correctly.
        unchecked {
    ...
    function _parseTraitsFromDna(uint256 dna) internal view returns (Trait[] memory _traits) {
        ...
        // Iterate over `dna` to acquire the roll for each trait.
        for (uint256 i = 0; i < _weights.length; i++) {
            
            // Calculate the roll for trait `i`.
            roll = (dna >> i * 40 & _BIT_MASK) % _TRAIT_WEIGHT;
            
            weight = 0;
            traitWeight = 0;

            // Using weighted numbers, determine the rolled trait.
            for (uint256 j = 0; j < _weights[i].length; j++) {
    ...
    function _getAttributes(Trait[] memory data) internal pure returns (string memory attrs) {
        return string(abi.encodePacked(
            '{"trait_type":"Hat","value":"', data[0].value, '"},',
            '{"trait_type":"Eyes","value":"', data[1].value, '"},',
            '{"trait_type":"Nose","value":"', data[2].value, '"},',
            '{"trait_type":"Mouth","value":"', data[3].value, '"},',
            '{"trait_type":"Fur","value":"', data[4].value, '"},',
            '{"trait_type":"Background","value":"', data[5].value, '"}'
        ));
    }

    function _getMetadata(uint256 id, Trait[] memory data) internal pure returns (string memory metadata) {
        return string(abi.encodePacked(
            "data:application/json;base64,",
            abi.encodePacked(
                '{"name":"On Chain Bears #', _toString(id),
                '","description":"On Chain Bears is a 100% on-chain NFT collection that aims to bring playfulness to the blockchain."',
                '","image": "data:image/svg+xml;base64,', _getSVG(data),
                '","attributes":[', _getAttributes(data), ']}'
            ).encode())
        );
    }

    function _getSVG(Trait[] memory data) internal pure returns (string memory) {
        ...
        for (uint256 i = 0; i < data.length; i++) {
            ...
            b = bytes(data[i].pixels);

            for (uint256 j = 0; j < b.length / 4; j++) {
                
                uint256 x = uint8(b[j*4]) - _CHAR_OFFSET;
                uint256 y = uint8(b[j*4+1]) - _CHAR_OFFSET;
        ...
        return abi.encodePacked(
            ...
            ".c00{fill:#9ccc65}.c01{fill:#689f38}.c02{fill:#01579b}.c03{fill:#546e7a}.c04{fill:#80deea}.c05{fill:#b71c1c}.c06{fill:#ffeb3b}.c07{fill:#f44336}.c08{fill:#c5e1a5}.c09{fill:#00bcd4}.c10{fill:#42a5f5}.c11{fill:#ffff00}.c12{fill:#00c853}.c13{fill:#9fa8da}.c14{fill:#a1887f}.c15{fill:#9e9e9e}.c16{fill:#7cb342}.c17{fill:#03a9f4}.c18{fill:#ff5252}.c19{fill:#4dd0e1}.c20{fill:#ffff8d}.c21{fill:#c62828}.c22{fill:#673ab7}.c23{fill:#00897b}.c24{fill:#fbc02d}.c25{fill:#9c27b0}.c26{fill:#ff9800}.c27{fill:#7c4dff}.c28{fill:#5d4037}.c29{fill:#00acc1}.c30{fill:#26c6da}.c31{fill:#ef9a9a}.c32{fill:#d32f2f}.c33{fill:#33691e}.c34{fill:#8d6e63}.c35{fill:#ff5722}.c36{fill:#ff6e40}.c37{fill:#ce93d8}.c38{fill:#bcaaa4}.c39{fill:#fff59d}.c40{fill:#b388ff}.c41{fill:#000000}.c42{fill:#ffee58}.c43{fill:#fff176}.c44{fill:#fafafa}.c45{fill:#fdd835}.c46{fill:#795548}.c47{fill:#ffc107}.c48{fill:#bdbdbd}.c49{fill:#76ff03}.c50{fill:#212121}.c51{fill:#ffd600}.c52{fill:#6d4c41}.c53{fill:#2196f3}.c54{fill:#ea80fc}.c55{fill:#424242}.c56{fill:#0097a7}.c57{fill:#f06292}.c58{fill:#ffffff}.c59{fill:#ef5350}.c60{fill:#aed581}.c61{fill:#f48fb1}.c62{fill:#eeeeee}.c63{fill:#d50000}.c64{fill:#8bc34a}.c65{fill:#0288d1}",
            "</style>",
            "</svg>"
        ).encode();
