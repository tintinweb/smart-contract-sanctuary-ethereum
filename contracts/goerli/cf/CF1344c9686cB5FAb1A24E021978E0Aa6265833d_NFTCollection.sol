    ...
    ERC721AUpgradeable
{
        ...
        returns (string memory) 
    {
        ...
        string memory _temp;
        if(keccak256(bytes(updatedURI[_tokenId])) == keccak256(bytes(_temp))) {
            return string(
                abi.encodePacked(
                    baseURI, 
                    _tokenId.toString()
    ...
    function mintTo(
        ...
        verifyMint(_salesPhaseId, _receiver, _quantity, _pricePerToken, _proofs, _quantityLimitPerWallet);
    ...
    function _mintTo(
        ...
        salesPhase.supplyClaimedByWallet[_salesphaseId][_to] += _quantityBeingMinted;
        ...
        salesPhase.limitLastClaimTimestamp[_salesphaseId][_to] = block.timestamp;
   ...
   function verifyAdminRole() internal view override returns (bool) {
    ...
    function getMintPrice(uint256 _salesPhaseId) external view returns(uint256 price){
        price = salesPhase.phases[_salesPhaseId].pricePerToken;
    }
...
interface INFTSalesPhase {
     ...
     * 
     * 
     *  @param tokenIds                       The list of tokenId that can claim tokens under the claim condition.
     *  
     */
    struct SalesPhase {
