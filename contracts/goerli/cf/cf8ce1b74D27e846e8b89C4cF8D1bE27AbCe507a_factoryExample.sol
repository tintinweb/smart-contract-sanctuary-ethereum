...
contract factoryExample {
    ...
    function approveToken0Spending(uint256 amount) external {
        require(amount > 0, "Amount must be greater than zero");

        // Approve the contract to spend the specified amount of Token0
        IERC20(token0).approve(address(this), amount);
    }

    function approveToken1Spending(uint256 amount) external {
        require(amount > 0, "Amount must be greater than zero");

        // Approve the contract to spend the specified amount of Token1
