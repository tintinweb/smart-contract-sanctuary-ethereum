...
error FreeOwlsLimitReached();
error PaidOwlsLimitReached();
...
contract NOCO is ERC721A, OperatorFilterer, Ownable {
    ...
    /*function FreeOwls() external nonContract {
        if (!NestOpen) revert NestNotOpen();
        if (_totalMinted() >= MAX_SUPPLY) revert ExceedsMaxSupply();
        if (_getAux(msg.sender) != 0) revert FreeOwlsLimitReached();
    ...
    function PaidOwls(uint256 qty) external payable nonContract {
        if (!NestOpen) revert NestNotOpen();
        if (_totalMinted() + qty > MAX_SUPPLY) revert ExceedsMaxSupply();
        if (qty > PAID_OWLS_LIMIT) revert PaidOwlsLimitReached();
        if (msg.value < qty * PAID_OWLS) revert InsufficientPayment();
