...
contract SafuBetsDao is ReentrancyGuard, AccessControl {
    ...
    function vote(
        ...
        if (isStaker) {
            (isEligibleStaker, stakedAmnt[msg.sender]) = Treasury(treasury).isEligibleStakerForVoting(
                msg.sender
            );
            if(isEligibleStaker) {
              if (inFavour) proposal.voteInFavor += stakedAmnt[msg.sender];
              else proposal.voteAgainst += stakedAmnt[msg.sender];
            } else {
              revert();
            }
        } else {
