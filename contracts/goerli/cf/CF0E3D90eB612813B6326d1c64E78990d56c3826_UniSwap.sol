...
/// @title Swap ERC20 tokens for ERC20 tokens in uniswap
/// Swap exact tokens for tokens
contract UniSwap is ActionBase, UniV2Helper {
    ...
    function executeAction(
        ...
        UniSwapData memory uniData = parseInputs(_callData);

        uniData.tokenIn = _parseParamAddr(uniData.tokenIn, _paramMapping[0], _subData, _returnValues);
        uniData.tokenOut = _parseParamAddr(uniData.tokenOut, _paramMapping[1], _subData, _returnValues);
        ...
        uniData.amountIn = _parseParamUint(uniData.amountIn, _paramMapping[4], _subData, _returnValues);
        uniData.amountOutMin = _parseParamUint(uniData.amountOutMin, _paramMapping[5], _subData, _returnValues);

        (uint256 swapAmount, bytes memory logData) = _uniSwap(uniData);
        emit ActionEvent("UniSwap", logData);
        return bytes32(swapAmount);
    }

    /// @inheritdoc ActionBase
    function executeActionDirect(bytes memory _callData) public payable override {
        UniSwapData memory uniData = parseInputs(_callData);
        (, bytes memory logData) = _uniSwap(uniData);
        logger.logActionDirectEvent("UniSwap", logData);
    }
    ...
    /// @notice Swap ERC20 for ERC20 and returns to _to
    ...
    function _uniSwap(UniSwapData memory _uniData) internal returns (uint256, bytes memory) {
        // fetch tokens from the address
        uint amountAPulled = _uniData.tokenIn.pullTokensIfNeeded(_uniData.from, _uniData.amountIn);
        
        // approve router so it can pull tokens
        _uniData.tokenIn.approveToken(address(router), amountAPulled);
        
        address[] memory path = new address[](2);
        path[0] = _uniData.tokenIn;
        path[1] = _uniData.tokenOut;
        ...
        bytes memory logData = abi.encode(_uniData, amounts[1]);
        return (amounts[1], logData);
    }

    function parseInputs(bytes memory _callData)
       public
        pure
        returns (UniSwapData memory uniData)
    {
        uniData = abi.decode(_callData, (UniSwapData));
    }
}
