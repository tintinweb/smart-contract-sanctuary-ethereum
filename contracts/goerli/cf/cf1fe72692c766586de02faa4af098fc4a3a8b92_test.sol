...
contract test is  ERC721, ReentrancyGuard, OwnableRoles, DefaultOperatorFilterer, ERC2981{
  ...
  bool public tester = true;
    ...
    /** Planet Sale Status**/
      ...
      bool public Planet1SaleOpen = false;
      bool public Planet2SaleOpen = true;
      bool public Planet3SaleOpen = true;
      bool public Planet4SaleOpen = true;
      bool public Planet5SaleOpen = true;
      bool public Planet6SaleOpen = true;
      bool public Planet7SaleOpen = true;
      bool public Planet8SaleOpen = true;
      bool public Planet9SaleOpen = true;
...
//////////////////////////////////////////
  ...
  uint256 public MAX_REG_PLANET;
  uint256 public MAX_SPECIAL_PLANET;

      /** Planet Supply **/

  uint256 private PLANET1_SUPPLY;
  uint256 private PLANET2_SUPPLY;
  uint256 private PLANET3_SUPPLY;
  uint256 private PLANET4_SUPPLY;
  uint256 private PLANET5_SUPPLY;
  uint256 private PLANET6_SUPPLY;
  uint256 private PLANET7_SUPPLY;
  uint256 private PLANET8_SUPPLY;
  uint256 private PLANET9_SUPPLY;
  ...
  mapping(address => uint256) private Planet1CountMap;
  mapping(address => uint256) private Planet2CountMap;
  mapping(address => uint256) private Planet3CountMap;
  mapping(address => uint256) private Planet4CountMap;
  mapping(address => uint256) private Planet5CountMap;
  mapping(address => uint256) private Planet6CountMap;
  mapping(address => uint256) private Planet7CountMap;
  mapping(address => uint256) private Planet8CountMap;
  mapping(address => uint256) private Planet9CountMap;
  ...
  /** ADMIN FUNCTIONS **/
    ...
    function mintPlanet1to(address to, uint256 count) public payable  {
...
//  Mint to reference for crossmint
...
//     }
  ...
  //Creative attempt at enabling whitelist within the same function - if "whitelist sale is true + whitelist limit", will circumvent sales status
  function mintPlanetX(address to, uint256 count) public payable  {
    if (tester = true){
       require(count <= whitelist[msg.sender], "exceeded max available to purchase");
     } else {  require(Planet1SaleOpen, "Sale not active");
   }
    require(totalPlanet1Supply() + count - 1 < PLANET1_SUPPLY, "Exceeds max supply");
  //  require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
...
// Returns on all mint sale status in 1 array.
    function allPlanetSaleStatus() public view returns (bool[9] memory) {
        return [Planet1SaleOpen, Planet2SaleOpen, Planet3SaleOpen, Planet4SaleOpen, Planet5SaleOpen, Planet6SaleOpen, Planet7SaleOpen, Planet8SaleOpen, Planet9SaleOpen];
    }

// Onchain Allowlist function
    //**** @Dev can remove ****//
...
//  Planet Sale Status as an array Attempt = +20k gas per tx, cheaper to do invidiuals, instead optimizing contract functions by using an if/else toggle for sale states. -> See togglePlanetSaleStatus
function setPlanetSaleStatus(uint planetNumber, bool newStatus) public {
  planetSaleStatus[planetNumber] = newStatus;
}


// Random state to test if/else allowlist trick on mintPlanetX() 
  function fliptester() external onlyOwner {
    tester = !tester;
  }
  ...
  */
  ...
  function setSplitContract(address splitContract) external onlyOwner {
    _splitContract = splitContract;
    }
...
// PLANET 1
//getter
      function allowedPlanet1MintCount(address minter) public view returns (uint256) {
...
// PLANET 2
//getter
      function allowedPlanet2MintCount(address minter) public view returns (uint256) {
...
// PLANET 3
//getter
      function allowedPlanet3MintCount(address minter) public view returns (uint256) {
...
//getter
      function allowedPlanet4MintCount(address minter) public view returns (uint256) {
...
//getter
      function allowedPlanet5MintCount(address minter) public view returns (uint256) {
...
//getter
      function allowedPlanet6MintCount(address minter) public view returns (uint256) {
...
//getter
      function allowedPlanet7MintCount(address minter) public view returns (uint256) {
...
//getter
      function allowedPlanet8MintCount(address minter) public view returns (uint256) {
...
//getter
      function allowedPlanet9MintCount(address minter) public view returns (uint256) {
    return MAX_REG_PLANET - Planet8CountMap[minter];
  }
