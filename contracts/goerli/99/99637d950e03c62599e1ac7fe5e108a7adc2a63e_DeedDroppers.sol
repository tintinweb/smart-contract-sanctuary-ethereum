...
// File: deeddroppers/deeddroppers.sol
...
contract DeedDroppers is Delegated, ERC721Holder, ERC721EnumerableLite, PaymentSplitterMod {
    using Strings for uint;

    // General NFT Variables
    uint public maxTokens = 200;
    uint public maxOrder = 20;
    uint public priceOne = 0.03 ether;
    uint public priceTwo = 0.0275 ether;
    uint public priceThreeOrMore = 0.025 ether;
    bool public isMintActive;

    string private _tokenURIPrefix = '';
    string private _tokenURISuffix = '';

    // address public deedAddress = 0x34d85c9CDeB23FA97cb08333b511ac86E1C4E258;
    address public deedAddress = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;
    ...
    event TokenDropped(uint seed, uint randomNumber, uint winningToken, address indexed winner, uint tokenId);
    event DeedDropped(uint tokenId);
    ...
    function mint(uint amount_) external payable onlySender {
        ...
        if (amount_ == 1) {
            require(msg.value >= priceOne * amount_, "Ether sent is not correct!");
        } else if (amount_ == 2) {
            require(msg.value >= priceTwo * amount_, "Ether sent is not correct!");
        } else {
            require(msg.value >= priceThreeOrMore * amount_, "Ether sent is not correct!");
        }
    ...
    /**
     * @dev Chooses a random winner between startToken_ and endToken_, inclusive, and drops an 
     * specified ERC-721 held by this contract. Can only be called by delegates.
     */
    function drop(uint startToken_, uint endToken_, address droppedTokenAddress_, uint tokenId_) public onlyDelegates {
        uint _lastToken = totalSupply() - 1;
        require(startToken_ <= _lastToken, "Start token does not exist!");
        require(endToken_ <= _lastToken, "End token does not exist!");
        uint _tokenID = tokenId_;
        require(IERC721(droppedTokenAddress_).ownerOf(_tokenID) == address(this), "Token not held by this contract!");
        uint _seed = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
        uint _startOffset = startToken_;
        uint _endOffset = _lastToken - endToken_;
        uint _randNum = uint(keccak256(abi.encodePacked(block.difficulty, ownerOf(_seed % _lastToken), block.timestamp)));
        uint _winningToken = _randNum % (_lastToken - (_startOffset + _endOffset) + 1) + _startOffset;
        address _winner = ownerOf(_winningToken);
        IERC721(droppedTokenAddress_).transferFrom(address(this), _winner, _tokenID);
        emit TokenDropped(_seed, _randNum, _winningToken, _winner, _tokenID);
    }

    /**
     * @dev Drops the next deed held in contract. Can only be called by delegates.
     */
    function dropDeed(uint startToken_, uint endToken_) external onlyDelegates {
        uint _tokenId = IDeed(deedAddress).tokenOfOwnerByIndex(address(this), 0);
        drop(startToken_, endToken_, deedAddress, _tokenId);
        emit DeedDropped(_tokenId);
    }
     ...
     */
    function setPrice(uint priceOne_, uint priceTwo_, uint priceThreeOrMore_) external onlyDelegates {
        require(priceOne != priceOne_ || priceTwo != priceTwo_ || priceThreeOrMore != priceThreeOrMore_, "New value matches old");
        priceOne = priceOne_;
        priceTwo = priceTwo_;
        priceThreeOrMore = priceThreeOrMore_;
    }
