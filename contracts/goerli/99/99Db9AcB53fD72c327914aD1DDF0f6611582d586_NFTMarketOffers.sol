...
contract NFTMarketOffers is MarketTools {
  ...
  function offerOnNft(
    ...
    // TODO: should we check whether the address+tokenId has an NFT? Maybe not?
  ...
  function acceptOffer(uint256 offerId) external whenNotPaused {
    ...
    if (offer.nftContract.supportsInterface(InterfaceId_ERC1155)) {
    ...
    } else if (offer.nftContract.supportsInterface(InterfaceId_ERC721)) {
      address nftOwner = IERC721(offer.nftContract).ownerOf(offer.tokenId);
      require(nftOwner == msg.sender, 'Only owner can accept offer');

      // transfer the nft from owner to offerer
      transferNFT(offer.nftContract, nftOwner, offer.offerer, offer.tokenId, 1);
    } else {
      revert('Unsupported');
    }
    ...
    // TODO add commission and royalty
