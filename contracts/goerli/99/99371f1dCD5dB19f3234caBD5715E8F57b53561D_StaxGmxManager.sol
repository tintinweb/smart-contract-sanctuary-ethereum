...
contract StaxGmxManager is IStaxGmxManager, Ownable, Operators {
    ...
    address public gmxInvestmentManager;
    address public glpInvestmentManager;
    ...
    event InvestmentManagersSet(address gmxInvestmentManager, address glpInvestmentManager);

    // event StaxStakingAndDistributorSet(address indexed staxStaking, address indexed rewardsDistributor);
    // event RewardsHarvested(
    //     uint256 wrappedNativeUserRewards,
    //     uint256 stxGmxUserRewards,
    //     uint256 wrappedNativeFees,
    //     uint256 stxGmxFees,
    //     uint256 esGmxStaked,
    //     uint256 esGmxVesting,
    //     uint256 bnGmxStaked,
    //     uint256 vestedGmxStaked
    // );
    event DepositorSet(address indexed depositor);

    error OnlyInvestmentManager(address caller);
    ...
    function setInvestmentManagers(address _gmxInvestmentManager, address _glpInvestmentManager) external onlyOwner {
        gmxInvestmentManager = _gmxInvestmentManager;
        glpInvestmentManager = _glpInvestmentManager;
        emit InvestmentManagersSet(_gmxInvestmentManager, _glpInvestmentManager);
    }
    
    /// @notice Set the STAX staking and rewards distributor contracts.
    // function setStaxStakingAndDistributor(address _staxStaking) external onlyOwner {
    //     if (_staxStaking == address(0)) revert CommonEventsAndErrors.InvalidAddress(address(0));
    //     // staxStaking = IStaxStaking(_staxStaking);
    //     rewardsDistributor = address(staxStaking.distributor());
    //     emit StaxStakingAndDistributorSet(_staxStaking, rewardsDistributor);
    // }
    ...
    // The amount of stxGMX/stxGLP which users use to sell, after STAXs fees are deducted.
    function _sellAmountAfterFees(uint256 _sellAmount) internal view returns (uint256) {
        if (sellFeeRate.numerator == 0) return _sellAmount;

        return _sellAmount - (
            (_sellAmount * sellFeeRate.numerator) /
            sellFeeRate.denominator
        );
    }
    ...
    function harvestableRewards(bool glpTrackerRewards) external view returns (uint256[] memory amounts) {
        ...
        (uint256 depositorNativeAmount, uint256 depositorEsGmxAmount) = depositor.harvestableRewards(glpTrackerRewards);
    ...
    function projectedRewardRates(bool glpTrackerRewards) external view returns (uint256[] memory amounts) {
        ...
        (uint256 depositorNativeRewardRate, uint256 depositorEsGmxRewardRate) = depositor.rewardRates(glpTrackerRewards);
    ...
    // function _mintStxGmxForEsGmx(bool sourcedFromGmx, uint256 esGmxAmount) internal returns (uint256 feeAmount, uint256 rewardAmount){
    //     (feeAmount, rewardAmount) = stxGmxRewardsFeeRate.split(esGmxAmount);
    //     if (sourcedFromGmx && rewardAmount > 0) {
    //         stxGmxToken.mint(investmentManager, rewardAmount);
    //     }
    // }
    ...
    function harvestRewards() external override {
        if (msg.sender != gmxInvestmentManager && msg.sender != glpInvestmentManager) revert OnlyInvestmentManager(msg.sender);

        // Do the rewards harvest - any vestedGMX is collected back here.
        (
        ...
        uint256 _rewards;
        {
            ...
            if (esGmxClaimedFromGmx > 0) {
                ...
                totalFees += _fees;
                if (_rewards > 0) stxGmxToken.mint(gmxInvestmentManager, _rewards);
            }
            ...
            if (esGmxClaimedFromGlp > 0) {
                ...
                totalFees += _fees;
                if (_rewards > 0) stxGmxToken.mint(glpInvestmentManager, _rewards);
            }
        ...
        // Handle ETH/AVAX rewards
        {
            ...
            if (wrappedNativeClaimedFromGmx > 0) {
                ...
                totalFees += _fees;
                if (_rewards > 0) IERC20(wrappedNativeToken).safeTransfer(gmxInvestmentManager, _rewards);
            }
            ...
            if (wrappedNativeClaimedFromGlp > 0) {
                ...
                totalFees += _fees;
                if (_rewards > 0) IERC20(wrappedNativeToken).safeTransfer(glpInvestmentManager, _rewards);
            }
    ...
    /// @notice Get a quote to buy stxGLP, with a GMX whitelisted token
    ...
    function buyStxGlpQuote(
        uint256 _amount,
        address _token
    ) external view override returns (uint256 feeBasisPoints, uint256 usdgAmountOut, uint256 glpAmountOut) {
        if (_amount == 0) return (feeBasisPoints, usdgAmountOut, glpAmountOut);
        ...
        (feeBasisPoints, usdgAmountOut) = buyUsdgQuote(_amount, _token);
    ...
    function sellStxGlpQuote(
        uint256 _stxGmxAmount,
        address _toToken
    ) external view returns (uint256 feeBasisPoints, uint256 tokenAmountOut) {
        uint256 glpAmount = _sellAmountAfterFees(_stxGmxAmount);
        if (glpAmount == 0) return (feeBasisPoints, tokenAmountOut);
        ...
        (feeBasisPoints, tokenAmountOut) = sellUsdgQuote(usdgAmount, _toToken);
    }

    // /// @notice The GMX Locker can mint stxGMX for users, and optionally immediately stake it on their behalf
    // function mintStxGmx(address _for, uint256 _amount, bool _stake) external onlyOperators {
    //     if (_stake) {
    ...
    //     } else {
    //         stxGmxToken.mint(_for, _amount);
    //     }
    // }

    /// @notice Sell stxGMX to GMX. STAX optionally receives a portion of the GMX as a fee and returns the rest to the user.
    ...
    function sellStxGmx(
        ...
        stxGmxToken.burn(_seller, _sellAmount);

        // The amount of GMX to liquidate - the requested amount minus a protocol fee percentage.
        // Left in expanded form to avoid truncation from uint256 division.
        amountOut = _sellAmountAfterFees(_sellAmount);
        ...
        if (amountOut > unstakedBal) {
            depositor.unstakeGmx(amountOut - unstakedBal);
        }

        gmxToken.safeTransfer(_recipient, amountOut);
    }

    /// @notice The GLP Locker can mint stxGLP for users, and optionally immediately stake it on their behalf
    // function mintStxGlp(address _for, uint256 _amount, bool _stake) external onlyOperators {
    //     if (_stake) {
    ...
    /// @notice Sell stxGLP to a whitelisted token. STAX optionally receives a portion of the GLP as a fee and returns the rest to the user.
    ...
    /// @notice Sell stxGLP to Staked GLP to the recipient. STAX optionally keeps a portion of the staked GLP as a fee and returns the rest to the user.
    /// @dev This burns the full amount of stxGLP, and then unstakes and sends the `_toToken` amount to the recipient.
    function sellStxGlpToStakedGlp(
        ...
        // Burn the users stxGMX
        // No approval required? Expected, or do we want to pull the tokens here first (extra gas for user)
        stxGlpToken.burn(_seller, _sellAmount);

        // Transfer only the portion after the fees to the recipient
        amountOut = _sellAmountAfterFees(_sellAmount);
    ...
    function buyUsdgQuote(uint256 fromAmount, address fromToken) internal view returns (
        ...
        uint256 basisPointsDivisor = gmxVault.BASIS_POINTS_DIVISOR();
