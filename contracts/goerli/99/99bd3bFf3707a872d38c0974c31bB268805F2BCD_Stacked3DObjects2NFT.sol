...
contract Stacked3DObjects2NFT is ERC721 {
    address public scriptyStorageAddress;
    address public scriptyBuilderAddress;

    constructor(address _scriptyStorageAddress, address _scriptyBuilderAddress)
        ERC721("example", "EXP")
    {
    ...
    function tokenURI(
        ...
        requests[4].name = "stacked3DObjects2";
        requests[4].wrapType = 0; // <script>[script]</script>
        requests[4].contractAddress = scriptyStorageAddress;

        // Calculated off-chain.
        // Check examples/stacked3DObjects/deployStacked3DObjects.js
        uint256 bufferSize = 212579;

        bytes memory encodedHTML = IScriptyBuilder(scriptyBuilderAddress)
            .getEncodedHTMLWrapped(requests, bufferSize);

        bytes memory metadata = abi.encodePacked(
            '{"name":"scripty.sol ART 2", "description":"An on-chain art that uses another on-chain art as base.","animation_url":"',
            encodedHTML,
            '"}'
        );
