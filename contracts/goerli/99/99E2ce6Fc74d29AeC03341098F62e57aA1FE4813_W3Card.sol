...
import "../token/ERC721A/ERC721AURIStorage.sol";
...
contract W3Card is ERC721AURIStorage, Ownable, ReentrancyGuard {    
    ...
    uint private _cardPrice = 0.005 ether;
    uint private _stampPrice = 0.001 ether;
     ...
     */
    function setMemberships(address memberships_) public onlyOwner nonReentrant {
     ...
     */
    function cardPrice() public view returns (uint) {
        return _cardPrice;
    }
     ...
     */
    function setCardPrice(uint price) public onlyOwner nonReentrant {
        _cardPrice = price;
    }
     ...
     */
    function stampPrice() public view returns (uint) {
        return _stampPrice;
    }
     ...
     */
    function setStampPrice(uint price) public onlyOwner nonReentrant {
        _stampPrice = price;
    }
     ...
     * @param giftAmount The amount of ether to send to recipient/charity
     * @param giftDate Unix timestamp (seconds) to send the giftAmount
     * @param isDonation Whether this is a donation or not
     */
    function mint(address to, uint quantity, uint giftAmount, uint giftDate, bool isDonation) public virtual payable {
        require(!_blacklist[msg.sender], "W3Card: you are blacklisted and cannot mint");
        require(msg.value >= (_cardPrice + _stampPrice + giftAmount) * quantity, "W3Card: insufficient funds");
        require(quantity >= 1, "W3Card: quantity must be at least 1");
        require(giftDate == 0 || giftDate > block.timestamp, "W3Card: we haven't quite figured out time travel yet");

        // Ensure the gift isn't sent twice
        bool redeemed = false;

        // Transfer giftAmount to receiver
        if (giftAmount > 0 && giftDate == 0 && !isDonation) {
            (bool transferToRecipientSuccessful,) = payable(to).call{value: giftAmount * quantity}("");
        ...
        deliveries[msg.sender].push(Delivery(to, block.timestamp, giftDate, giftAmount, quantity, isDonation, redeemed));

        _safeMint(to, quantity);
    }

    /**
     * @dev Mint an asset to this contract if you have a membership.
     * @param to The address of the receiver
     * @param membershipId The membership token ID
     * @param quantity The number of cards to mint
     * @param giftAmount The amount of ether to send to recipient/charity
     * @param giftDate Unix timestamp (seconds) to send the giftAmount
     * @param isDonation Whether this is a donation or not
     */
    function mintWithMembership(address to, uint membershipId, uint quantity, uint giftAmount, uint giftDate, bool isDonation) public virtual payable {
        require(!_blacklist[msg.sender], "W3Card: you are blacklisted and cannot mint");
        require(quantity >= 1, "W3Card: quantity must be at least 1");
        require(giftDate == 0 || giftDate > block.timestamp, "W3Card: we haven't quite figured out time travel yet");

        // Check membership
        (bool successful, bytes memory data) = memberships.delegatecall(abi.encodeWithSignature("ownerOf(uint256)", membershipId));
        require(successful, "W3Card: something went wrong while verifying your membership");

        address memberAddress = abi.decode(data, (address));
        require(memberAddress == msg.sender, "W3Card: you are not a member");
    ...
    function sendEthGift(uint index, address to) public onlyOwner nonReentrant {
        ...
        require(senderDelivery.giftDate >= block.timestamp, "W3Card: deliveries cannot be sent before their set dates");
        require(!senderDelivery.redeemed, "W3Card: this delivery was already sent");

        // Transfer gift amount
        (bool success,) = payable(to).call{value: senderDelivery.giftAmount * senderDelivery.quantity}("");
