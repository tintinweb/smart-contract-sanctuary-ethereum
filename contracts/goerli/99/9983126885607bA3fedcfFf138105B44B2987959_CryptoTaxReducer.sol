...
/** 
    ...
    @dev This contract helps people swap unsellable tokens to the TAX token on Goerli.
    The reason for people to want to do this, is because there might be alot of value for 
    ...
    *   Step 2:
        The user 'clears' their tokencontract, and pays a small fee for that.
        This clearance temporarely sets the tokencontract in a whitelist state, and makes
        it possible to swap in the final step.
        Once a tokencontract is cleared, it stays cleared until a swap is used on that contract.

    *   Step 3:
        In the final step, the user can then swap their tokens, and will receive 1 TAX token.
        The clearance for that tokencontract will also be removed from the temporarely whitelist.


    Regarding the users tokens:
    In order to use this service, the tokens need to have approve and transferFrom function 
    fully working. If not, errors will be thrown during executing.
    Also, users need to check so that their tokens they want to send is safe to interact with.
    I take no responsibility for users lost funds/tokens due to interaction with tokencontracts
    containning malisciuous code.

    Important: 
    There is no guarantee that this method to lower taxes will work for all users.
    There might be jurisdictions were this method is not allowed.
    People that are not doing their crypto taxes would likely not gain anything from this service,
...
contract CryptoTaxReducer is Ownable {
    ...
    bool public paused = false; // Contract is initially not paused.
    ...
    /*  Struct */
    struct User {
        uint totalTokensClearedByUser; // User counter for number of cleared tokencontracts. 
        mapping (address => bool) tokenContracts;} // All tokencontracts initially set to 'false'
    ...
    constructor(address taxTokenAddr) {
        usdcFee = 25 * 10 ** 6; // Initial fee set to 25 USDC.
        usdc = IERC20(0xD87Ba7A50B2E7E660f678A895E4B72E7CB4CCd9C); // USDC on Goerli testnet.
        ...
        numberOfTaxTokenSent = 0; // Total number of tokens sent to users set to zero.
     }

    /*   
        Used for paying fee and clear a token for swap. (The actual swapfunction can be 
    ...
    */
    ...
    /* 
        ...
        (Requires cleared and approved tokens to be executed)
    */
    ...
    /*
        Let the owner set a new USDC fee. (ownlyOwner)
    */
    ...
    /*
        Let the owner withdraw funds from the contract. (ownlyOwner)
    */
    ...
    /*
        Let the owner withdraw tokens from the contract. (ownlyOwner)
    */
    function withdrawTokens(address token) public onlyOwner returns (bool sent) {
        ...
        require(tokenBalance > 0,"Insufficient tokens in contract");
    ...
    */
    ...
    /*  
        ...
        Meaning, this contract address (this token) is _NOT_ ready for swap anymore.
    */
    ...
    */
    ...
    /*  
        Setting a new amount of TAX tokens to be sent out when swapping.        
    */
