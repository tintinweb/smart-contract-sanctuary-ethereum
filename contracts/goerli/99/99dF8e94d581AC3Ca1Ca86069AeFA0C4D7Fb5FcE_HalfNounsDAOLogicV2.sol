...
/// @title The HalfNouns DAO logic version 2
...
// LICENSE
// HalfNounsDAOLogicV2.sol is a modified version of Compound Lab's GovernorBravoDelegate.sol:
...
//
// MODIFICATIONS
// See HalfNounsDAOLogicV1 for initial GovernorBravoDelegate modifications.

// HalfNounsDAOLogicV2 adds:
...
// max quorum votes using the current HalfNoun supply.
...
//
// HalfNounsDAOLogicV2 removes:
// - `quorumVotes()` has been replaced by `quorumVotes(uint256 proposalId)`.

pragma solidity ^0.8.9;

import './HalfNounsDAOInterfaces.sol';

contract HalfNounsDAOLogicV2 is HalfNounsDAOStorageV2, HalfNounsDAOEventsV2 {
    /// @notice The name of this contract
    string public constant name = 'Half Nouns DAO';

    /// @notice The minimum setable proposal threshold
    uint256 public constant MIN_PROPOSAL_THRESHOLD = 1; // 1 NFT

    /// @notice The maximum setable proposal threshold
    uint256 public constant MAX_PROPOSAL_THRESHOLD = 10; // 10 NFTs

    /// @notice The minimum setable voting period
    uint256 public constant MIN_VOTING_PERIOD = 14; // About 24 hours
     ...
     * @param timelock_ The address of the HalfNounsDAOExecutor
     * @param halfNouns_ The address of the NOUN tokens
    ...
    function initialize(
        ...
        require(address(timelock) == address(0), 'HalfNounsDAO::initialize: can only initialize once');
        if (msg.sender != admin) {
            revert AdminOnly();
        }
        require(timelock_ != address(0), 'HalfNounsDAO::initialize: invalid timelock address');
        require(halfNouns_ != address(0), 'HalfNounsDAO::initialize: invalid halfNouns address');
        require(
            votingPeriod_ >= MIN_VOTING_PERIOD && votingPeriod_ <= MAX_VOTING_PERIOD,
            'HalfNounsDAO::initialize: invalid voting period'
        );
        require(
            votingDelay_ >= MIN_VOTING_DELAY && votingDelay_ <= MAX_VOTING_DELAY,
            'HalfNounsDAO::initialize: invalid voting delay'
        );
        require(
            proposalThreshold_ >= MIN_PROPOSAL_THRESHOLD && proposalThreshold_ <= MAX_PROPOSAL_THRESHOLD,
            'HalfNounsDAO::initialize: invalid proposal threshold bps'
        );

        emit VotingPeriodSet(votingPeriod, votingPeriod_);
        emit VotingDelaySet(votingDelay, votingDelay_);
        emit ProposalThresholdBPSSet(proposalThreshold, proposalThreshold_);

        timelock = IHalfNounsDAOExecutor(timelock_);
        halfNouns = HalfNounsTokenLike(halfNouns_);
        vetoer = vetoer_;
    ...
    function propose(
        ...
        temp.totalSupply = halfNouns.totalSupply();

        temp.proposalThreshold = proposalThreshold;

        require(
            halfNouns.getPriorVotes(msg.sender, block.number - 1) >= temp.proposalThreshold,
            'HalfNounsDAO::propose: proposer votes below proposal threshold'
        );
        require(
            targets.length == values.length &&
                targets.length == signatures.length &&
                targets.length == calldatas.length,
            'HalfNounsDAO::propose: proposal function information arity mismatch'
        );
        require(targets.length != 0, 'HalfNounsDAO::propose: must provide actions');
        require(targets.length <= proposalMaxOperations, 'HalfNounsDAO::propose: too many actions');
        ...
        if (temp.latestProposalId != 0) {
            ...
            require(
                ...
                'HalfNounsDAO::propose: one live proposal per proposer, found an already active proposal'
            );
            require(
                proposersLatestProposalState != ProposalState.Pending,
                'HalfNounsDAO::propose: one live proposal per proposer, found an already pending proposal'
            );
        }
    ...
    function queue(uint256 proposalId) external {
        require(
            ...
            'HalfNounsDAO::queue: proposal can only be queued if it is succeeded'
        );
    ...
    function queueOrRevertInternal(
        ...
        require(
            ...
            'HalfNounsDAO::queueOrRevertInternal: identical proposal action already queued at eta'
        );
    ...
    function execute(uint256 proposalId) external {
        require(
            ...
            'HalfNounsDAO::execute: proposal can only be executed if it is queued'
        );
        Proposal storage proposal = _proposals[proposalId];
    ...
    function cancel(uint256 proposalId) external {
        ...
        require(
            msg.sender == proposal.proposer ||
                halfNouns.getPriorVotes(proposal.proposer, block.number - 1) < proposal.proposalThreshold,
            'HalfNounsDAO::cancel: proposer above threshold'
        );
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        require(proposalCount >= proposalId, 'HalfNounsDAO::state: invalid proposal id');
        Proposal storage proposal = _proposals[proposalId];
        if (proposal.vetoed) {
    ...
    function castVoteBySig(
        ...
        require(signatory != address(0), 'HalfNounsDAO::castVoteBySig: invalid signature');
    ...
    function castVoteInternal(
        ...
        require(state(proposalId) == ProposalState.Active, 'HalfNounsDAO::castVoteInternal: voting is closed');
        require(support <= 2, 'HalfNounsDAO::castVoteInternal: invalid vote type');
        Proposal storage proposal = _proposals[proposalId];
        Receipt storage receipt = proposal.receipts[voter];
        require(receipt.hasVoted == false, 'HalfNounsDAO::castVoteInternal: voter already voted');

        /// @notice: Unlike GovernerBravo, votes are considered from the block the proposal was created in order to normalize quorumVotes and proposalThreshold metrics
        uint96 votes = halfNouns.getPriorVotes(voter, proposalCreationBlock(proposal));
    ...
    function _setVotingDelay(uint256 newVotingDelay) external {
        ...
        require(
            ...
            'HalfNounsDAO::_setVotingDelay: invalid voting delay'
        );
    ...
    function _setVotingPeriod(uint256 newVotingPeriod) external {
        ...
        require(
            ...
            'HalfNounsDAO::_setVotingPeriod: invalid voting period'
        );
    ...
    function _setProposalThreshold(uint256 newProposalThreshold) external {
        ...
        require(
            ...
            'HalfNounsDAO::_setProposalThreshold: invalid proposal threshold bps'
        );
        uint256 oldProposalThreshold = proposalThreshold;
        proposalThreshold = newProposalThreshold;

        emit ProposalThresholdBPSSet(oldProposalThreshold, proposalThreshold);
    }
    ...
    function _setMinQuorumVotesBPS(uint16 newMinQuorumVotesBPS) external {
        ...
        require(
            ...
            'HalfNounsDAO::_setMinQuorumVotesBPS: invalid min quorum votes bps'
        );
        require(
            newMinQuorumVotesBPS <= params.maxQuorumVotesBPS,
            'HalfNounsDAO::_setMinQuorumVotesBPS: min quorum votes bps greater than max'
        );
    ...
    function _setMaxQuorumVotesBPS(uint16 newMaxQuorumVotesBPS) external {
        ...
        require(
            ...
            'HalfNounsDAO::_setMaxQuorumVotesBPS: invalid max quorum votes bps'
        );
        require(
            params.minQuorumVotesBPS <= newMaxQuorumVotesBPS,
            'HalfNounsDAO::_setMaxQuorumVotesBPS: min quorum votes bps greater than max'
        );
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        // Check caller = admin
        require(msg.sender == admin, 'HalfNounsDAO::_setPendingAdmin: admin only');
    ...
    function _acceptAdmin() external {
        ...
        require(msg.sender == pendingAdmin && msg.sender != address(0), 'HalfNounsDAO::_acceptAdmin: pending admin only');
    ...
    function _burnVetoPower() public {
        ...
        require(msg.sender == vetoer, 'HalfNounsDAO::_burnVetoPower: vetoer only');
    ...
    // /**
    //  * @notice Current proposal threshold using HalfNoun Total Supply
    //  * Differs from `GovernerBravo` which uses fixed amount
    //  */
    // function proposalThreshold() public view returns (uint256) {
    //     return bps2Uint(proposalThresholdBPS, halfNouns.totalSupply());
    // }
     ...
     * @param totalSupply The total supply of HalfNouns at the time of proposal creation
    ...
    function getDynamicQuorumParamsAt(uint256 blockNumber_) public view returns (DynamicQuorumParams memory) {
        uint32 blockNumber = safe32(blockNumber_, 'HalfNounsDAO::getDynamicQuorumParamsAt: block number exceeds 32 bits');
    ...
    /**
     * @notice Current min quorum votes using HalfNoun total supply
     */
    function minQuorumVotes() public view returns (uint256) {
        return bps2Uint(getDynamicQuorumParamsAt(block.number).minQuorumVotesBPS, halfNouns.totalSupply());
    }

    /**
     * @notice Current max quorum votes using HalfNoun total supply
     */
    function maxQuorumVotes() public view returns (uint256) {
        return bps2Uint(getDynamicQuorumParamsAt(block.number).maxQuorumVotesBPS, halfNouns.totalSupply());
    }
...
/// @title HalfNouns DAO Logic interfaces and events
...
// LICENSE
// HalfNounsDAOInterfaces.sol is a modified version of Compound Lab's GovernorBravoInterfaces.sol:
...
//
// MODIFICATIONS
// HalfNounsDAOEvents, HalfNounsDAOProxyStorage, HalfNounsDAOStorageV1 add support for changes made by HalfNouns DAO to GovernorBravo.sol
// See HalfNounsDAOLogicV1.sol for more details.
// HalfNounsDAOStorageV1Adjusted and HalfNounsDAOStorageV2 add support for a dynamic vote quorum.
// See HalfNounsDAOLogicV2.sol for more details.

pragma solidity ^0.8.6;

contract HalfNounsDAOEvents {
    ...
    /// @notice An event emitted when a proposal has been queued in the HalfNounsDAOExecutor
    event ProposalQueued(uint256 id, uint256 eta);

    /// @notice An event emitted when a proposal has been executed in the HalfNounsDAOExecutor
...
contract HalfNounsDAOEventsV2 is HalfNounsDAOEvents {
...
contract HalfNounsDAOProxyStorage {
 ...
 * @notice For future upgrades, do not change HalfNounsDAOStorageV1. Create a new
 * contract which implements HalfNounsDAOStorageV1 and following the naming convention
 * HalfNounsDAOStorageVX.
 */
// contract HalfNounsDAOStorageV1 is HalfNounsDAOProxyStorage {
...
//     /// @notice The address of the HalfNouns DAO Executor HalfNounsDAOExecutor
//     IHalfNounsDAOExecutor public timelock;

//     /// @notice The address of the HalfNouns tokens
//     HalfNounsTokenLike public halfNouns;
...
/**
 * @title Extra fields added to the `Proposal` struct from HalfNounsDAOStorageV1
 * @notice The following fields were added to the `Proposal` struct:
 * - `Proposal.totalSupply`
 * - `Proposal.creationBlock`
 */
contract HalfNounsDAOStorageV1Adjusted is HalfNounsDAOProxyStorage {
    /// @notice Vetoer who has the ability to veto any proposal
    address public vetoer;
...
/**
 * @title Storage for Governor Bravo Delegate
 * @notice For future upgrades, do not change HalfNounsDAOStorageV2. Create a new
 * contract which implements HalfNounsDAOStorageV2 and following the naming convention
 * HalfNounsDAOStorageVX.
 */
contract HalfNounsDAOStorageV2 is HalfNounsDAOStorageV1Adjusted {
...
interface IHalfNounsDAOExecutor {
