    ...
    AccessControlUpgradeable
{
    ...
    //(withdrawal,depositor) pair
    mapping(address => address) public depositorAddresses;
    //(depositor,counter) pair. Note: depositor could be a depositor address for more than one withdrawal addresses, hence a uint is returned
    mapping(address => uint256) public isDepositorAddress;
    ...
    function addToDepositorAddresses(
        ...
        require(
            ...
            "Account is already added into depositorAddresses"
        );
        depositorAddresses[receiverAddress] = depositorAddress;
        isDepositorAddress[depositorAddress] += 1;
        ...
        onlyRole(DEPOSITOR_ADDRESSES_ADD_REMOVE_ROLE)
    {
        require(
            depositorAddresses[receiverAddress] != address(0),
            "Account is not yet added into depositorAddresses"
        );
        isDepositorAddress[depositorAddresses[receiverAddress]] -= 1;
        depositorAddresses[receiverAddress] = address(0);
    ...
    modifier whenNotPausedWithException(address caller, address recipient) {
        if (hasRole(FULLTIME_TRANSFER_ROLE, caller)) {
            ...
            (isDepositorAddress[caller] > 0) &&
            (hasRole(FULLTIME_TRANSFER_ROLE, recipient))
        ) {
            _;
        } else {
            _requireNotPaused();
