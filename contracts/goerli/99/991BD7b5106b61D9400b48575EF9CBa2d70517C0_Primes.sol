...
contract Primes is ERC721r, Ownable {
    ...
    uint256 private constant PRESALE_MAX_SUPPLY = 750;
    ...
    function privateMint(uint256 _quantity) external payable isUser {
        require(!paused, "Contract paused");
        uint price = presalePrice;
        require(price > 0, "Price is 0");
        require(_quantity <= maxPerAddress, "Max per address exceeded");
        require(
        ...
        require(currentTime() >= timestampStart, "Sale has not begun yet");
        require(currentTime() < timestampStart + 1 days, "Sale is finished");
    ...
    function publicMint(uint256 _quantity) external payable isUser {
        require(!paused, "Contract paused");
    ...
    function setTimestampStart(uint256 _timestampStart) external onlyOwner {
