...
contract Match is Initializable, IMatch { 
        ...
        onlyDataManager
    {
        ...
        matches[_matchCounter.current()]=MatchData(_isPlayOff,_teamA,_teamB,1,0,_startDate);
        emit MatchAdded(_matchCounter.current(), _teamA, _teamB, _isPlayOff, _startDate);
        _matchCounter.increment();
    }
    /// @inheritdoc IMatch
...
interface IMatch {
    ...
    function matches(uint256 _matchId)
        external
        view
        returns ( bool,uint32,uint32,uint32,uint48,uint64 );
