...
contract UChicagoFinMathInherited is ERC20 {
    ...
    // Modifier to ensure that someone is one of the owners (original or not)
    modifier oneOfOwners() {
        ...
        if (isOwner == false) {emit MessageLog("Only an owner can do this!");}
        require(isOwner);
        _;
    }

    function addOwner(address payable wallet) public oneOfOwners {

        // Make sure they aren't already an owner
        for (uint256 i = 0; i < ownerCount; i++){
    ...
    function deleteOwner(address payable wallet) public onlyOwner {
        ...
        for (uint256 i = 0; i < ownerCount; i++) {
            ...
            // If it is the wallet we want to delete, delete it
            if (owners[i] == wallet) {
                delete owners[i];
                ownerCount--;
                }
