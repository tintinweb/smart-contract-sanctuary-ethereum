...
contract Transfer is Ownable, ReentrancyGuard {
    ...
    event TransferSent(
        ...
        uint256 srcChainTokenQty,
        address srcChainBridgeToken,
        uint256 srcChainBridgeTokenQty,
        address srcChainAggregator
    );
    ...
    event TransferComplete(
        ...
        uint256 dstChainTokenQty,
        address dstChainBridgeToken,
        uint256 dstChainBridgeTokenQty,
        address dstChainAggregator,
        bool dstChainSwapSuccess,
        address to
    );
    ...
    function completeTransfer(address _tokenReceived, uint256 _bridgeTokenAmount, bytes calldata _payload) internal {
...
contract Connext is Transfer, IXReceiver {
    ...
    /// _transferId The unique id of the xchain transaction
    ...
    /// _originSender The address of the contract or EOA that called xcall on the origin chain
    /// _origin The domain ID of the chain that the transaction is coming from
    ...
    function xReceive(
        ...
        completeTransfer(_asset, _amount, _calldata);
    }
}