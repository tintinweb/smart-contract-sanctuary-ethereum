...
contract WNFT is ERC5496Cloneable, IERC721Receiver {
    ...
    constructor() ERC5496("UniNFT V1", "WNFT") {
        factory = msg.sender;
    }
    ...
    function flashExecute(uint tokenId, address target, bytes4 selector, bytes memory tailParams) external {
        require(ownerOf(tokenId) == msg.sender, "not owner");
        require(target != nft, "cannot call nft contract");
        (bool success, bytes memory returndata) = target.call(abi.encodeWithSelector(selector, tokenId, tailParams));
