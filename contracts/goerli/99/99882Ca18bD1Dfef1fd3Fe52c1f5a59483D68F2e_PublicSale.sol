...
contract PublicSale is Validatable {
     ...
     */
    function initialize(IAdmin _admin, IGenesis _genesis) public initializer {
    ...
    function createPublicSale(
        ...
        uint256 endTime,
        uint256 price
    ) external onlyAdmin notZero(price) notZero(maxPerUser) {
    ...
    function updatePublicSale(
        ...
        uint256 endTime,
        uint256 price
    ) external onlyAdmin validPublicSaleId(id) notZero(price) notZero(maxPerUser) {
        require(admin.isPermittedPaymentToken(paymentToken), "Invalid payment token");
        require(startTime > 0, "Invalid time");
        if (endTime > 0) {
            require(startTime < endTime, "Invalid end time");
     ...
     * emit {Bought} events
     */
    function buy(uint256 id, uint256 times) external payable notZero(times) validPublicSaleId(id) {
        PublicSaleInfo storage publicSaleInfo = publicSales[id];
        require(publicSaleInfo.status, "Public sale was cancel");
        ...
        TransferHelper._transferToken(
            publicSaleInfo.paymentToken,
            times * publicSaleInfo.price,
            _msgSender(),
            treasury
        );
        ...
        genesis.mintBatch(_msgSender(), times);
