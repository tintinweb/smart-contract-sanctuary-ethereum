...
contract ERC20NFTBond is NFTBond {
    ...
    external initializer {
        ...
        __LockedBondGranter_init(_cooldownSeconds, _unstakeWindow);
    }
...
abstract contract NFTBond is NFT, CollateralizedBondGranter, LockedBondGranter, LiquidityRequester, PausableUpgradeable {
     ...
     * @dev Issue the bond
     */
    function _issueBond(uint256 tokenId, uint256 maturity, uint256 principal) internal virtual override (BondGranter, CollateralizedBondGranter) {}
 ...
 * @dev This contract contains functions related cooldown and unstake window of the bonds
 * @author Ethichub
 */
abstract contract LockedBondGranter is BondGranter {
    ...
    event CooldownStarted(uint256 tokenId, uint256 startCooldown);
    ...
    function activateCooldown(uint256 tokenId) external {
        require(cooldowns[tokenId] == 0 || !_isUnlock(tokenId), "LockedBondGranter::Cooldown cannot be activated");
        Bond memory bond = bonds[tokenId];
        require((bond.maturity + bond.mintingDate) < block.timestamp, "LockedBondGranter::Cooldown cannot be activated");
        cooldowns[tokenId] = block.timestamp;

        emit CooldownStarted(tokenId, cooldowns[tokenId]);
    }
     ...
     */
    function setUnstakeWindow(uint256 unstakeWindow) external onlyRole(COOLDOWN_SETTER) {
        UNSTAKE_WINDOW = unstakeWindow;
    }

    function isUnlock(uint256 tokenId) external view returns (bool) {
        return _isUnlock(tokenId);
    }
    ...
    function _redeemBond(uint256 tokenId) internal virtual override returns (uint256) {
        uint256 bondValue = super._redeemBond(tokenId);
        require(_isUnlock(tokenId), "LockedBondGranter::Bond is locked");
        return bondValue;
    }

    /**
     * @dev Checks if cooldown and unstake window is active or not
     * @param tokenId uint256
     */
    function _isUnlock(uint256 tokenId) internal view returns (bool) {
        if (block.timestamp > cooldowns[tokenId] + COOLDOWN && block.timestamp - (cooldowns[tokenId] + COOLDOWN) <= UNSTAKE_WINDOW) {
            return true;
        }
        return false;
    }
