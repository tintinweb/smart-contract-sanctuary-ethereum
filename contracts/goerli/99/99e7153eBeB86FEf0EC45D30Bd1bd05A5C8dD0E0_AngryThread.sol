...
contract AngryThread is ERC721G, Ownable, DefaultOperatorFilterer {
    ...
    ) ERC721G("Angry Thread", "ANGRY", 1, 20) {
    ...
    uint256 public wtfMintPriceInWei = 1 ether; //Mint with $WTF
    ...
    uint256 public firstStartingIndexBlock;
    uint256 public firstStartingIndex;

    string public secondProvenance; //>10K (10K-20K)
    uint256 public secondStartingIndexBlock;
    ...
    function setFirstStartingIndex() public {
        require(firstStartingIndex == 0, "First Starting index is already set");
        // require(
        //     firstStartingIndexBlock != 0,
        ...
        firstStartingIndex =
            uint(blockhash(firstStartingIndexBlock)) %
            maxSupply;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(firstStartingIndexBlock) > 255) {
            firstStartingIndex = uint(blockhash(block.number - 1)) % maxSupply;
        }
        // Prevent default sequence
        if (firstStartingIndex == 0) {
            firstStartingIndex = firstStartingIndex.add(1);
        }
    }

    function setSecondStartingIndex() public {
        require(secondStartingIndex == 0, "Starting index is already set");
        // require(
        //     secondStartingIndexBlock != 0,
        //     "Starting index block must be set"
        // );
        ...
        secondStartingIndex =
            uint(blockhash(secondStartingIndexBlock)) %
            maxSupply;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(secondStartingIndexBlock) > 255) {
            secondStartingIndex = uint(blockhash(block.number - 1)) % maxSupply;
        }
        // Prevent default sequence
        if (secondStartingIndex == 0) {
            secondStartingIndex = secondStartingIndex.add(1);
        }
    }
