...
contract TreasuryAdaptor {
    ...
    /// @notice nonce for operator message ordering
    ...
    bytes32 internal constant AUTHORIZATION_TYPEHASH =
        keccak256(
            "Authorization(uint256 amount,address destination,uint256 nonce)"
        );
    ...
    function addOperator(address new_operator) public {
        if (msg.sender != admin) revert Unauthorized();
        operators[new_operator] = block.timestamp + delay;
    }

    function removeOperator(address old_operator) public {
        if (msg.sender != admin) revert Unauthorized();
        operatorsBlocklist[old_operator] = block.timestamp + delay;
    }
    ...
    function withdraw(
        ...
        bytes32 digest = createDigestMessage(amount, destination, currentNonce);
    ...
    /// @dev Generage digest message with EIP-712 typehash info
    ...
    function isActiveOperator(address addr) public view returns (bool) {
        return
            isTimelockOver(operators[addr]) &&
            !isTimelockOver(operatorsBlocklist[addr]);
    }
    ...
    {
        return
            isTimelockOver(withdrawalAddresses[addr]) &&
            !isTimelockOver(withdrawalAddressesBlocklist[addr]);
    }

    function isTimelockOver(uint256 timestamp) private view returns (bool) {
