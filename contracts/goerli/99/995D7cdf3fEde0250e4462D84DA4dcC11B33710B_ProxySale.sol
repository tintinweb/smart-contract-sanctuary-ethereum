...
contract ProxySale is Ownable, ReentrancyGuard, IProxySale {
    ...
    mapping(bytes => uint256) public unStakeList;
    ...
    function stake(uint256 _tokenId, uint256 _stakeType) public {
        require(!(stakeList[_tokenId].tokenId > 0), "Token already staked");
    ...
    function unStake(string calldata salt, bytes calldata _signature,  uint256 _tokenId) external {
        require(stakeList[_tokenId].tokenId > 0, "Token not staked");
        ...
        require(unStakeList[_signature] > 0, "Already unStaked");

        delete stakeList[_tokenId];

        unStakeList[_signature] = _tokenId;
