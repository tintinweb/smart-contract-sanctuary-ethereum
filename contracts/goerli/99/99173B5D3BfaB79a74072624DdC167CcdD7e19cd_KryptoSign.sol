...
contract KryptoSign is IKryptoSign {
    ...
    function signDocument(
        ...
        require(
            keccak256(abi.encodePacked(documentId, ipfs, msg.sender))
                .toEthSignedMessageHash()
                .recover(signature) == msg.sender,
            "Invalid Signature"
        );
        require(!_signed[documentId][msg.sender], "Already Signed");

        IKryptoSign.Document storage document = _documents[documentId];
    ...
    function getDocumentId(string calldata ipfs)
        ...
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(ipfs, msg.sender, block.number));
    }
}
