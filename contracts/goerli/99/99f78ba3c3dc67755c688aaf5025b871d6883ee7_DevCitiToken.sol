...
import "./base/DevToken.sol";

contract DevCitiToken is DevToken {
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() initializer {}

    function initialize(address admin) initializer public {
        __DevToken_init("CitiToken", "CITI", admin);
    }
}
...
/**
 * @dev Extension of BaseToken with dev features only available on testnets.
 */
contract DevToken is BaseToken {
    /**
     * @dev Initializes the `name` and `symbol` of the contract.
     */
    function __DevToken_init(string memory _name, string memory _symbol, address admin) internal onlyInitializing {
        require(!Utils.isMainnet(), "mainnet not allowed");
        __BaseToken_init(_name, _symbol, admin);
    }

    /**
     * @dev Mints `amount` tokens for `to`.
     */
    function devMint(address to, uint256 amount) public {
        _mint(to, amount);
    }
}
