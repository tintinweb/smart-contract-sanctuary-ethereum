...
contract ScheduleTheRandomness is
  ...
  Ownable
{
  uint256 intervalComponents = 120;
  ...
  mapping(uint8 => COMPONENT) public components;

  uint8[] toCheckComponents;

  mapping(bytes32 => bool) public expectingRequestWithIdToBeFulfilled;
  bytes32 qualityPlanTaskId;
  bool planIsActive = false;

 event qualityControl(COMPONENT[20] components);
 ...
 event randomComponent(uint8 id );
  ...
  function startQualityPlan() public {
      require(planIsActive == false && qualityPlanTaskId == bytes32(0), 'PLAN_RUNNING');
      planIsActive = true;
  ...
  function doQualityControl() public {
    ...
    COMPONENT[20] memory eventComponents;
    ...
    for (uint256 i = 0; i < 2; i++) {
      ...
      compo.status = COMPONENT_STATUS.CHECKED;
  ...
  for (uint8 i = 0; i < 20; i++) {
    ...
    eventComponents[i] = _compo;
  }


    emit qualityControl(eventComponents);
  ...
  function getRandomComponents() public returns (uint8[2] memory retArray) {
    for (uint8 i = 0; i < 20; i++) {
      ...
      if (
        ...
        _compo.status == COMPONENT_STATUS.CHECKED
      ) {
        _compo.status = COMPONENT_STATUS.STILL;
      } else if (_compo.status == COMPONENT_STATUS.STILL) {
    ...
    for (uint256 i = 0; i < 2; i++) {
      ...
      retArray[i] = id;
      toCheckComponents[rand] = toCheckComponents[toCheckComponents.length - 1];
      toCheckComponents.pop();
    }
  }

  function getRandomControlType() public returns (uint8 _controlType) {
    uint8 _controlType = uint8(randMod(3));
  }
  ...
  function checkQualityPlanIsActive()     public
    view
    returns (bool canExec, bytes memory execPayload)
  {
    canExec = planIsActive == true;
