...
contract DuoPowerStation is Initializable, UUPSUpgradeable, OwnableUpgradeable,IPowerStation{
  ...
  Pool[] public pools; // Staking pools
    ...
    function stakerJoinPool(uint256 _pid, uint256 shares, string memory payby) public override {
        ...
        require(renFil.transferFrom(msg.sender,devAddr, poolUSDT.poolInfoVars[5] * shares),"transfer failed");
        ...
        //Data+Gas
        require(renFil.transferFrom(msg.sender, devAddr, 
                    ((poolUSDT.timing[2]-poolUSDT.timing[1])/30 days * ( poolUSDT.poolInfoVars[0]+poolUSDT.poolInfoVars[1]) * shares)),"transfer failed");          // 15/10 is ultility/T/month

        emit PayDataGas(msg.sender, (poolUSDT.timing[2]-poolUSDT.timing[1])/30 days
         * ( poolUSDT.poolInfoVars[0]+poolUSDT.poolInfoVars[1]) * shares);


        ///@dev pay for the contract by usdt or RGP
        if (keccak256(abi.encodePacked(payby)) == keccak256(abi.encodePacked("USDT"))){

            require(usdt.transferFrom(msg.sender,devAddr, (poolUSDT.poolInfoVars[4] + poolUSDT.poolInfoVars[2]) * shares),"transfer failed");
            emit PayPower(msg.sender, poolUSDT.poolInfoVars[4] * shares);
            emit PayIDC(msg.sender, poolUSDT.poolInfoVars[2] * shares);
            ...
            //RGP
            require(rgp.transferFrom(msg.sender,devAddr, poolRGP.poolInfoVars[3] * shares),"transfer failed");
            emit PayPower(msg.sender, poolRGP.poolInfoVars[3] * shares);

            //IDC
            require(usdt.transferFrom(msg.sender, devAddr, poolRGP.poolInfoVars[2]*shares),"transfer failed");
            emit PayIDC(msg.sender, poolRGP.poolInfoVars[2] * shares);
    ...
    function _poolIsOpen(uint256 _pid) internal{
        ...
        if (block.timestamp > pool.timing[0]){      // buy end time
            pool.isOpen = false;
        }
        require(pool.isOpen, "pool closed");
    }
    ...
    function withdraw(uint256 _pid) public override{
        ...
        uint256 remainingRewardsPerShareUSDT = poolUSDT.totalDailyRewardPershare -stakerUSDT.redeemedRewards / (stakerUSDT.shares);
        uint256 remainingRewardsPerShareRGP = poolRGP.totalDailyRewardPershare -stakerRGP.redeemedRewards / (stakerRGP.shares);
    ...
    function topUpPool(uint256 _pid, uint256 _allocPoint) public override{
        ...
        renFil.transferFrom(msg.sender, devAddr, _allocPoint - allocatedRewardsRGP - allocatedRewardsUSDT);
        renFil.transferFrom(msg.sender, address(this), allocatedRewardsRGP + allocatedRewardsUSDT);
    ...
    function emergencyWithdraw()public override onlyOwner{
        ...
        renFil.transfer(devAddr, renFil.balanceOf(address(this)));
    ...
    function getStakerHoldShares(uint256 _pid, address stakerAddr) public override view returns(uint256){
        _stakerExist(_pid,stakerAddr);
        Staker storage stakerUSDT = poolStaker[_pid*2][stakerAddr];
        Staker storage stakerRGP =  poolStaker[_pid*2+1][stakerAddr];

        return(stakerUSDT.shares + stakerRGP.shares);
    }
...
interface IPowerStation {
    ...
    function getPoolVars(uint256 _pid) external returns(uint256[7] memory);

    function getPoolDailyPricePerShare(uint256 _pid) external returns(uint256[] memory);
    
    function getTiming(uint256 _pid) external view returns(uint256[3] memory);

    function getStakerHoldShares(uint256 _pid, address stakerAddr) external view returns(uint256);
