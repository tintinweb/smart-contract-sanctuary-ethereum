...
/**
 * @dev DXTAYieldDistribution contract which enables bulk sending
 * of DXTA tokens or UDSC to users.
 */
contract DXTAYieldDistribution is Administrable {
    using SafeERC20 for IERC20;
    uint256 constant ARRAY_LIMIT = 255;
    ...
    event SentTotal(address token, address sourceAccount, uint256 total);

    function yieldDistribute(
        address token,
        address[] memory accounts,
        uint256[] memory amounts
    ) public payable onlyAdmin returns (bool success) {
        multiTransferToken(token, _msgSender(), accounts, amounts);
        return true;
    }
    ...
    function multiTransferToken(
        ...
        require(
            accounts.length <= ARRAY_LIMIT,
            "Array size exceed the array limit"
        );
        require(
            IERC20(token).allowance(sourceAccount, address(this)) > 0,
            "Not enough funds"
        );
        uint256 total = 0;
        for (uint256 index = 0; index < accounts.length; index++) {
            require(amounts[index] > 0, "Amount must be bigger than 0");
            total += amounts[index];
            emit Sent(token, sourceAccount, accounts[index], amounts[index]);
        ...
        emit SentTotal(token, _msgSender(), total);
    }
}
