...
contract JardinXBatchSupport is
    Initializable,
    Administration,
    ERC1155Upgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    event TokenDeposited(address indexed to, uint256 amount, uint256 timestamp);
    ...
    event RequestApproval(address indexed requester, uint256[] cid, uint256[] amount, string category);
        ...
        whenPauseSA
    {
        ...
        require(sentWei >= amount.add(_txFee), "amount is too small");
        mint(to, 0, amount, "0x");
        emit TokenDeposited(to, amount, block.timestamp);
    }

    function requestApproval(uint256[] calldata cid, uint256[] calldata amount, string memory category) public payable nonReentrant whenPauseSA{
        require(keccak256(abi.encodePacked(category)) !=
        ...
        for(uint256 i = 0; i < cid.length; i++){
            require(balanceOf(msg.sender, cid[i]) >= amount[i], "not enough balance");
            if(keccak256(abi.encodePacked(category)) != keccak256(abi.encodePacked("withdraw"))){
                require(cid[i] != 0, "not valid collection");
            }
        }
        emit RequestApproval(msg.sender, cid, amount, category);
    }

    function approveRequest(address requester, uint256[] calldata cid, uint256[] calldata amount, uint256 sumAmount, uint256 status, string memory category) public onlySAnA {
        ...
        whenPauseSA
    {
        ...
        require(requesters[msg.sender].status == 1, "withdrawal not approved");
        require(requesters[msg.sender].cid[0] == 0, "not valid category");
        require(balanceOf(msg.sender, 0) >= requesters[msg.sender].sumAmount,"not enough balance");
        for(uint256 i = 0; i < requesters[msg.sender].cid.length; i++){
            bool sent = payable(msg.sender).send(requesters[msg.sender].amount[i]);
            require(sent, "Failed to send Ether");
            burn(msg.sender, 0, requesters[msg.sender].amount[i]);
        }
        emit TokenWithdrawn(msg.sender, requesters[msg.sender].amount, block.timestamp);
        delete requesters[msg.sender];
    }
    ...
    function trade(
        ...
        uint256 escrowCollateralBidder;
        uint256 escrowCollateralAsker;
        uint256 tokenCollateralBidder;

        if(requesters[asker].cid[0] == 0){
        escrowCollateralAsker = requesters[asker].sumAmount;
        }
        for(uint256 i = 0; i < requesters[bidder].cid.length; i++){
            if(requesters[bidder].cid[i] == cid){
                tokenCollateralBidder = requesters[bidder].amount[i];
            }
        }
        if(requesters[bidder].cid[0] == 0){
            escrowCollateralBidder = requesters[bidder].sumAmount;
        }
        
        require(balanceOf(asker, 0) >= amountAsk + uint256((amountAsk * _txRate) / 100).add(_txFee) + escrowCollateralAsker, "insufficient bid balance");
        require(balanceOf(bidder, cid) >= amountBid + tokenCollateralBidder, "insufficient ask balance");
        require(balanceOf(bidder, 0) >= _txTrade + escrowCollateralBidder, "insufficient tax balance");

        transferTrade(bidder, asker, cid, amountAsk, "0x");
        transferTrade(asker, bidder, 0, amountBid, "0x");
        transferTrade(bidder, owner(), 0, _txTrade, "0x");
        transferTrade(asker, owner(), 0, _txTrade, "0x");

        emit TradeSucceed(bidder, asker, cid, amountBid, amountAsk, block.timestamp);
    }
    ...
    function mintSuperAdmin(
        ...
        uint256 amount,
        bytes memory data
    ) public whenPauseSA onlySuperAdmin {
        _mint(account, id, amount, data);
    }
    ...
    function transfer(address to) public nonReentrant whenPauseSA {
        require(requesters[msg.sender].status == 1, "withdrawal not approved");
        for(uint256 i = 0; i < requesters[msg.sender].cid.length; i++){
            require(requesters[msg.sender].cid[i] != 0, "not valid category");
            require(
                balanceOf(msg.sender, requesters[msg.sender].cid[i]) >= requesters[msg.sender].amount[i],
                "not enough balance"
            );
        }
        super.safeBatchTransferFrom(msg.sender, to, requesters[msg.sender].cid, requesters[msg.sender].amount, "0x");
        delete requesters[msg.sender];
    }
        ...
        whenPauseSA
    {
        ...
        collections[nft].totalStaked = collections[nft].totalStaked.add(
            uint256(tokenIds.length)
        );
        collections[nft].totalToken = collections[nft].totalToken.add(
            10**18 * tokenIds.length
        );
    }
        ...
        whenPauseSA
    {
        ...
        collections[nft].totalStaked = collections[nft].totalStaked.sub(
            uint256(tokenIds.length)
        );
        collections[nft].totalToken = collections[nft].totalToken.sub(
            10**18 * tokenIds.length
        );
        delete requesters[msg.sender];
    }
