...
// todo double check the unchecked{} logic searching for underflow/overflow
...
contract KarmaHeadNFT is Ownable, ERC721Royalty, ReentrancyGuard, TokenHandler {
    ...
    // todo: update with real wallet address
    address public constant ARTIST_ADDRESS = 0xc2d407E58f1ef489a3eEd45Ff04aBfA6405F3f6f;

    // todo: final mint price
    uint256 public constant MINT_PRICE = 10 ether;
    // fees in basis points (100% = 10000)
    uint256 public constant VINCI_FEE_POINTS = 1500; // 15%
    uint256 public constant BASIS_POINTS = 10000; // 100%
    // as mint price is fixed, all fees are fixed, so set them as constants to save gas
    uint256 public constant VINCI_FEE = (MINT_PRICE * VINCI_FEE_POINTS) / BASIS_POINTS;
    uint256 public constant ARTIST_PAYOUT_PER_MINT = MINT_PRICE - VINCI_FEE;

    // fees for secondary sales (handled by decentralized marketplaces)
    uint96 public constant ARTIST_ROYALTY_FEE = 1000; // 10%
    //
    //    // ERC20 token in which dividends are paid.
    ...
    // how much have been claimed already per tokenId
    // tokenId => (Erc20tokenAddress => amount)
    mapping(uint256 => mapping(address => uint256)) public claimed;

    // buffer of addresses that didnt claim their tokens while holding the NFTs and receiving dividends
    ...
    event EtherReceived(address indexed depositor, uint256 amount);
    ...
    // @notice Mints a specific tokenId NFT to msg.sender.
    // @dev Requires: msg.value must be == MINT_PRICE
    //      Requires: total supply must be lower than maxSupply to avoid exceeding maxSupply
    //      requries: tokenId must be within the boundaries (1,12) both inclusive
    //      TokenId to be minted must not exist yet (ERC721 will revert otherwise)
    //      We let the minter choose the tokenId to mint and they are pre-revealed
    ...
    // @notice  Deposits an amount of USDC from the msg.sender into the contract and splits into
    //          the corresponding share for all NFT holders
    // @dev     Requires: usdc is approved by sender (for this contract as spender)
    //          Requires: amount must be higher than totalSupply to avoid 0 divisions
    //          Requires: current totalSupply must be non-zero
    // @param   Amount of USDC to be deposited (with all the decimals as uint256)
    ...
    // @notice  Transfers usdc dividends to msg.sender
    // @dev     Requires: tokenId must exist and msg.sender must be the ownerOf(tokenId)
    // @tokenid The tokenId from which balances are intended to be claimed
    ...
    // @param   The tokenId to check for dividends
    // @dev     Requires: tokenId must exist
    // @return  The tokenURI pointing to tokenId metadata
    ...
    // @param numerator Fees in basis points, max = 100% == 10_000 basis points
    ...
    // allows owner to withdraw all ether collected from the public sale mint
    // reentrancy might be redundant as state changes only happen at the end. Better safe than sorry
    ...
    // in case USDC or WETH are sent by mistake to the contract, this function allows to split
    ...
    function _clearPendingClaims(uint256 tokenId, address from, address erc20TokenAddress) internal {
        // if its a new mint, the new tokenId is not entitled to the existing claimablePerNFT
        if ((from == address(0)) && (claimablePerNFT[erc20TokenAddress] > 0)) {
            // hardcode these ones to avoid reading the token addresses from storage
            // constant variables are not actualy written in storage
            ...
            if (pendingAmount > 0) {
                // there is not enough USDC in circulation to overflow these balances
                ...
                unchecked {
                    ...
                    bufferByAddress[from][erc20TokenAddress] += pendingAmount;
                }
            }
        }
    }
    ...
    function _depositErc20(address tokenAddress, uint256 amount) internal {
        ...
        require(IERC20(tokenAddress).balanceOf(sender) >= amount, "KH: Not enough tokens");
    ...
    function _claimErc20(uint256 tokenId, address tokenAddress) internal {
        ...
        if (amount > 0) {
            unchecked {
                // amount is capped by _claimableTokensPerTokenId so this is safe
    ...
    {
        if (!_exists(tokenId)) revert NonExistingToken();
        // claimed is bound by the difference between claimed and claimable every time
        unchecked {
...
contract TokenHandler {
    ...
    function _recoverTokens(address token, address to) internal {
        ...
        // No need to protect the function with onlyOwner
