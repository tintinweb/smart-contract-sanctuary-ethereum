    ...
    Ownable
{
    ...
    function cloneLeagueLedger(
        ...
        ownershipLedger[clone] = msg.sender;
        emit OwnerShipChanged(msg.sender, clone);
        ICatanLeagueLedger(clone).inializeLedger(address(this), leagueName);
        return clone;
    }
    /* 
...
interface ICatanLeagueLedger {
    function inializeLedger(address leagueFactory_, string calldata leagueNickName_) external;
