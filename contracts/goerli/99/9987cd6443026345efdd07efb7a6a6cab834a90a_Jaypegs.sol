...
contract Jaypegs is ERC721Pausable, Ownable {
    ...
    constructor() ERC721("Jay Pegs", "PEGS") {
        _pause();
    }

    modifier isCorrectPayment(uint256 _price, uint256 amount) {
        if(_tokenIds.current()>NUM_FREE_MINTS){
        require(
            (_price * amount) == msg.value,
            "Incorrect ETH value set."
        );
        }
        _;
    }
    ...
    function mintItems(uint256 amount) public isCorrectPayment(_price, amount) payable {
        require(amount <= _maxMint);
        require(_saleActive);
