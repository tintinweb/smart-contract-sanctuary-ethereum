...
contract AAWallet {
    ...
    function getSender(UserOperation memory userOp) internal pure returns (address ret) {
    ...
    function executionFromEntryPoint(UserOperation calldata userOp) external view onlyEntryPoint returns(string memory destination, uint amount, address test) {
        // unpacking callData.
        bytes memory callData = userOp.callData;
        (destination, amount) = abi.decode(callData, (string, uint));

        test = getSender(userOp);
        // main execution.
        // _call(test, amount);
