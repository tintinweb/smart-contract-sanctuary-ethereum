...
contract ChookyRoyaltySharing is ReentrancyGuard, Ownable {
    ...
    function _stakeChookyForRoyalty(uint256 _amount) external nonReentrant {
        ...
        if(holder.userStakedBalance >= ChookySupply * 100 / 80) {
            holder.tier = 0;
            tierIHolders ++;
        }
        else if(holder.userStakedBalance < ChookySupply * 100 / 80 && holder.userStakedBalance >= ChookySupply * 100 / 125) {
            holder.tier = 1;
            tierIIHolders ++;
        }
        else if(holder.userStakedBalance < ChookySupply * 100 / 125 && holder.userStakedBalance >= ChookySupply * 100 / 200) {
            holder.tier = 2;
            tierIIIHolders ++;
        }
        else if(holder.userStakedBalance < ChookySupply * 100 / 200 && holder.userStakedBalance >= ChookySupply * 100 / 400) {
            holder.tier = 3;
            tierIVHolders ++;
        }
    ...
    function _unstakeChooky() external nonReentrant {
        Holder storage holder = chookyHolders[msg.sender];
        require(ChookyInu.balanceOf(address(this)) >= holder.userStakedBalance, "Not enough Balance");
        require(block.timestamp >= holder.lastClaimTime + 14 days, "");
        ...
        uint256 _amount = holder.userStakedBalance;
    ...
    function _claimRoyalty() external nonReentrant{
        ...
        require(block.timestamp >= holder.lastClaimTime + 14 days, "Can't claim before two weeks of last claim!");
