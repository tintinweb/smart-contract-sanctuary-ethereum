...
abstract contract StakingUpgradeable is Initializable, UUPSUpgradeable {
    ...
    event Update(uint256 balance, uint256 accRewardsPerShare, uint256 lastUpdate);

    /// @notice Event emitted when the staking account data is updated
    event UpdateData(address account, uint256 amount, uint256 rewardDebt, uint256 notClaimed);
    ...
    function data(address account) public view returns (Data memory) {
    ...
    function pendingRewards(address account) public view returns (uint256) {
        Data memory data_ = _data[account];
        uint256 rewards = data_.notClaimed;
        if (_balance > 0) {
            IDistributionVaultAllocation dvInterface = IDistributionVaultAllocation(_vault);
            uint256 available = dvInterface.available(address(this));
            uint256 total = (available * 1 ether / _balance);
            rewards += (_accRewardsPerShare - data_.rewardDebt + total) * data_.balance / 1 ether;
        }
        return rewards;
    }

    function deposit(address account, uint256 amount) public virtual {
        require(account == msg.sender || _approvals[account][msg.sender], "Staking: unauthorized account");
        require(amount > 0, "Staking: amount must be over zero");

        _update();
        _updateData(account);

        _data[account].balance += amount;
        _balance += amount;
    ...
    function withdraw(address account, uint256 amount) public virtual {
        require(account == msg.sender || _approvals[account][msg.sender], "Staking: unauthorized account");
        require(amount > 0, "Staking: amount must be over zero");
        require(amount <= _data[account].balance, "Staking: user has not enough staking balance");

        _update();
        _updateData(account);

        _data[account].rewardDebt = _accRewardsPerShare;
        _data[account].balance -= amount;
        _balance -= amount;
    ...
    function claim(address account) public virtual {
        ...
        uint256 pending = _data[account].notClaimed;
    ...
    function _update() internal {
        ...
        uint256 available = dvInterface.available(address(this));
        if (_balance > 0) {
            _accRewardsPerShare += (available * 1 ether / _balance);
        }
        // solhint-disable-next-line
        _lastUpdate = block.timestamp;
        emit Update(_balance, _accRewardsPerShare, _lastUpdate);
    }

    function _updateData(address account) internal {
        uint256 diff = _accRewardsPerShare - _data[account].rewardDebt;
        _data[account].notClaimed += diff * _data[account].balance / 1 ether;
        _data[account].rewardDebt = _accRewardsPerShare;
        emit UpdateData(account, _data[account].balance, _data[account].rewardDebt, _data[account].notClaimed);
    }
...
contract TurinFarming is StakingUpgradeable {
    constructor() {
        _disableInitializers();
    }

    function initialize(address vault_, IERC20Upgradeable token_) public initializer {
