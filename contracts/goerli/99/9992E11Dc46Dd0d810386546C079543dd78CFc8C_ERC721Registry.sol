...
contract ERC721Registry is AccessControlEnumerable, IERC721Registry {
        ...
        onlyRole(CONFIGURATOR_ROLE)
    {
        ...
        for (uint256 index = 0; index < newSources.length; index++) {
            if (add) {
                sources[newSources[index]] = true;
            } else {
                sources[newSources[index]] = false;
            }
        }
        emit SourcesChanged(newSources, add);
    }
...
interface IERC721Registry {
    ...
    event SourcesChanged(address[] indexed newSources, bool added);
