...
contract NormalPoolNFT is PoolBaseDerivativeNFT {
    ...
    function upgrade(uint256 tokenId) public payable {
        ...
        require(INormalPoolFactory(factory).whiteListMap(getLegacyFromAddress(ownerOfSpanning(tokenId))), "Not Selected WhiteList Builder.");
    ...
    {
        require(!ownerToTokenIdMap[msg.sender].hasLicense, "Tx Revert: You already own a license.");
    ...
    returns (uint256 offerId) {
        ...
        require(ownerToTokenIdMap[msg.sender].hasLicense, "You don't own a license");
    ...
    function taxOwed(uint256 tokenId) public view returns (uint256) {
        if(!_exists(tokenId) || getLegacyFromAddress(ownerOfSpanning(tokenId)) == address(this) || isUpgraded[tokenId] == true) {
            return 0;
        }
