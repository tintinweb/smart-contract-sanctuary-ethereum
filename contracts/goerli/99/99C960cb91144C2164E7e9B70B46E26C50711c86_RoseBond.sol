...
contract RoseBond is Ownable {
    ...
    function bond(uint256 _pid, uint256 _amount) public payable {
        ...
        uint256 _payout = msg.value;
        UserInfo[] storage user = userInfo[msg.sender];
        if ( _payout > 0 ){ // Bond ETH
            require( _amount == 0 && _pid == 0, "Only ETH bond is possible" );
            user[_pid].payout += _payout;
            totalBondAmount[_pid] += _payout;

            roseLp.sync();

            uint256 debt = _payout;//getBondAmount(payout);
            ...
            user.push( UserInfo({
                ...
                currentDebt: _payout,
                depositTime: _amount,
                lastRedeemTime:block.timestamp
            }));
        }
