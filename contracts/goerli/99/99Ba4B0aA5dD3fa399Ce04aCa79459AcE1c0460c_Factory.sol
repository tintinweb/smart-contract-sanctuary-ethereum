...
contract Factory is OwnableUpgradeable {
  ...
  // genesis: deploy with the deployer as the signer
  ...
  // genesis2: deploy with a custom signer
  function genesis2(uint _index, string calldata name, string calldata symbol, address signer) external payable returns (address) {
    bytes32 salt = keccak256(abi.encodePacked(_msgSender(), _index));
    address payable clone = payable(ClonesUpgradeable.cloneDeterministic(implementation, salt));
    C0 token = C0(clone);
    token.initialize(name, symbol, signer);
    token.transferOwnership(_msgSender());
    return clone;
  }
  // genesis3: deploy with a custom signer and mint tokens in one transaction
  function genesis3(uint _index, string calldata name, string calldata symbol, address signer, C0.Body[] calldata bodies, C0.Input[] calldata inputs) external payable returns (address) {
    bytes32 salt = keccak256(abi.encodePacked(_msgSender(), _index));
    address payable clone = payable(ClonesUpgradeable.cloneDeterministic(implementation, salt));
    C0 token = C0(clone);
    token.initialize(name, symbol, signer);
    token.transferOwnership(_msgSender());
    if (bodies.length > 0) token.token(bodies, inputs);
    return clone;
  }
