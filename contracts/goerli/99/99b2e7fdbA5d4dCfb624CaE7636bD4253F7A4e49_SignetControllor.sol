...
contract SignetControllor is ReentrancyGuard, Ownable {
    ...
    {
        (, address b, uint256 c) = STCrator.getresponse(0, contractOwner);
        return (b, c);
    }
    ...
    function sendmessage(address addr, string memory tokenURI_) public returns (bool success) {
        (bool success, ) = addr.delegatecall(
            abi.encodeWithSignature("sendmessage(string memory tokenURI_)", tokenURI_)
        );
        require(success);
        return true;
    }
...
contract Signetor is ERC721, Ownable {
    ...
    function sendmessage(string memory tokenURI_) public onlyOwner {
