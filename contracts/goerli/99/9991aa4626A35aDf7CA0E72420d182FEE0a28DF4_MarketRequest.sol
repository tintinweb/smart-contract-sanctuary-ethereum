...
contract Offer is EscowVault, Fee, ReentrancyGuard{
    ...
    event ChangePrice(uint indexed itemId, uint indexed tokenId, address token_address, uint old_price, uint new_price);
    event ChangeOwner(uint indexed itemId, uint indexed tokenId, address token_address, address new_owner, address old_owner);
    ...
    function changePrice(uint _tokenId, address _tokenAddress, uint _nprice) public virtual{
        ...
        emit ChangePrice(nft2Item[_tokenAddress][_tokenId].itemId, nft2Item[_tokenAddress][_tokenId].tokenId,_tokenAddress, old_price, nft2Item[_tokenAddress][_tokenId].price);    
    }
    ...
    //     Items memory Items = tokenIdToItems[_tokenAddress][_tokenId];
        ...
        emit ChangeOwner(nft2Item[_tokenAddress][_tokenId].itemId, nft2Item[_tokenAddress][_tokenId].tokenId, _tokenAddress, _nowner, nft2Item[_tokenAddress][_tokenId].owners);    
    }
    function cancel(uint _tokenId, address _tokenAddress) public virtual{
        ...
        emit ChangeOwner(nft2Item[_tokenAddress][_tokenId].itemId, nft2Item[_tokenAddress][_tokenId].tokenId,_tokenAddress, msg.sender, address(0x0));    
    }
...
contract MarketAuction is ReentrancyGuard, Offer{
    ...
    function cancel(address _tokenAddress, uint _tokenId) public{
        require(activeAuction[_tokenAddress][_tokenId] == true,"!avai");
        require(AuctionV1(nft2Auction[_tokenAddress][_tokenId]).isEnd()==false, "!end");
        AuctionV1 auctionContract = AuctionV1(nft2Auction[_tokenAddress][_tokenId]);
        _sendToken(_tokenAddress,_tokenId,auctionContract.getTokenOwner());
        AuctionV1(nft2Auction[_tokenAddress][_tokenId]).auctionEnd();
        withdrawalAuction(_tokenAddress,_tokenId);
        delete(nft2Auction[_tokenAddress][_tokenId]);
        emit AuctionTransaction(3, _tokenId, _tokenAddress, nft2Auction[_tokenAddress][_tokenId], auctionContract.getHighestBid(),auctionContract.getHighestBidder(),auctionContract.getEndTime(), true);
...
contract WrapperArtscape is ArtscapeMarket, MarketAuction{
    ...
    event CreateMarketRequest(address market_address);
    constructor(
        ...
        emit CreateMarketRequest(address(market_request));
    }
