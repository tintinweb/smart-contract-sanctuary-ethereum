...
contract TokenTimelockController is Ownable {
    ...
    event TokenTimelockCreated(TokenTimelock tokenTimelock, address beneficiary, uint256 releaseTime, uint256 amount);
    event TokenTimelockClaimed(TokenTimelock tokenTimelock, address beneficiary, uint256 claimTime);
    event TokenTimelockDestroyed(TokenTimelock tokenTimelock, address beneficiary, uint256 destroyTime);
    ...
    function createTokenTimelock(address beneficiary_, uint256 releaseTime_, uint256 amount_) public onlyOwner {
        ...
        emit TokenTimelockCreated(tokenTimelock, beneficiary_, releaseTime_, amount_);
    }
    ...
    function _claim(address tokenTimelock_) private {
        ...
        emit TokenTimelockClaimed(tokenTimelock, beneficary, block.timestamp);
    }
    ...
    function claimBeneficiaryAllTokenTimelock(address beneficiary_) public {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (block.timestamp >= tokenTimelock.releaseTime()) {
                ...
                emit TokenTimelockClaimed(tokenTimelock, beneficiary_, block.timestamp);
            }
        }
    }
    ...
    function getBeneficiaryAllTokenTimelock(address beneficiary_) public view returns (
        uint256[] memory,
        address[] memory,
        bool[] memory,
        uint256[] memory) 
    {
        ...
        return (indexes, addresses, claimables, releaseTimes);
    }
