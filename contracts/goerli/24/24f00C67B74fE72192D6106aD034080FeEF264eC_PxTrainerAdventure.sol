...
contract PxTrainerAdventure is Utils, ReentrancyGuard {
    ...
    uint256 public randomNumber1 =0;
    uint256 public selectedToken1 =0;
    ...
    struct WeekInfo {
        ...
        mapping(address => WinnerInfo) winner;
        
        uint256 remainingSupply;
        uint256 totalTreasure;
        mapping(uint256 => TreasureInfo) treasureInfos;
    }
    ...
    function claimTreasure(uint256 weekNumber) external{
        WeekInfo storage week = weekInfos[weekNumber];
        if(week.winner[msg.sender].claimed ==  week.winner[msg.sender].totalPrize){
            revert NotAWinner();
        }
        ...
        uint256 selectedToken = 0;
        uint256 indexCount =0;
        for(uint256 index = 1;index <=week.totalTreasure;index++){
            //uint256 treasureType = treasures[week.treasureInfos[index].index].treasureType;
            // if(week.treasureInfos[index].totalSUpply == 0 || week.winner[msg.sender].claimedTreasureType[treasureType]){
            //     continue;
            // }
            indexCount+=week.treasureInfos[index].totalSUpply;
            if(random<=indexCount){
                selectedToken = week.treasureInfos[index].index;
                selectedToken1 = selectedToken;
                week.treasureInfos[index].totalSUpply--;
                //week.winner[msg.sender].claimedTreasureType[treasureType] = true;
                week.winner[msg.sender].claimed++;
        ...
        if(selectedToken != 0){
            transferToken(treasures[selectedToken].collectionAddress,treasures[selectedToken].contractType,msg.sender,treasures[selectedToken].tokenId);
    ...
    function updateWeeklyWinner(address[] calldata winners, uint256[] calldata count, uint256 weekNumber) external {
        for(uint256 index = 0;index < winners.length ; index++ ){
            weekInfos[weekNumber].winner[winners[index]].totalPrize  = count[index];
        }
    }
    ...
    function updateWeeklyTreasureDistribution(uint256 weekNumber, uint256[] calldata treasureindexes, uint256[] calldata counts) external{
        ...
        for(uint256 index = 0;index<treasureindexes.length;index++){
            week.totalTreasure++;
            week.treasureInfos[ week.totalTreasure].index = treasureindexes[index];
            week.treasureInfos[ week.totalTreasure].maxSupply = counts[index];
            week.treasureInfos[ week.totalTreasure].totalSUpply = counts[index];
