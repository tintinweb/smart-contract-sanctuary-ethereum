...
contract Unbounded1155V4 is ERC1155, IERC1155Receiver,IERC2981 {
  ...
  */
  ...
  function setAuthorizedContract(address _contractAddress) external onlyAdmin {
    authorizedContracts[_contractAddress] = true;
  }

  function removeAuthorizedContract(address _contractAddress) external onlyAdmin {
  ...
  function mint(address _to, uint256 _baseId) external onlyAuthorizedContract {
  ...
  function mintBatch(address _to, uint256[] calldata _baseIds, uint256[] calldata _amounts) public onlyAuthorizedContract {
    for (uint256 i; i < _baseIds.length; i++) {
      ...
      unchecked {
        require(
          (_item.mintCount + _amounts[i]) <= (_item.maxTokenID - _item.minTokenID + 1),
          "Can't mint this amount, please contact admins!"
        );
      }
      ...
      unchecked {
        templateInfo[_baseIds[i]].mintCount += _amounts[i];
      }
    }
  }
    ...
    for (uint256 i; i < _baseIds.length; i++) {
      ...
      for (uint256 j; j < _amounts[i]; j++) {
        unchecked {
          uint256 mintId = _item.minTokenID + _item.mintCount;
          _mint(_to, mintId, 1, "");
          tokenMetaverseURI[mintId][_metaverseId] = _metaverseMetadataTemplate;
          _item.mintCount++;
          emit ItemMinted(_baseIds[i], mintId);
          emit MetaverseURIAdded(mintId, _metaverseId, _metaverseMetadataTemplate);
        }
      }
      unchecked {
        templateInfo[_baseIds[i]].mintCount += _amounts[i];
  ...
  function mintWithMetadaSlots(
    ...
    unchecked {
      ...
      addBatchMetaverseSlotsToTemplate(_to, mintId, _metaverseIds, _metaverseMetadataTemplate);
      emit ItemMinted(_baseId, mintId);
    }
  }
  ...
  //*@notice to keep mintCount balance, changing min token id will create a dead zone!
  //*(there might not be happening any mint event for some token ids)
  ...
  function addMetadataSlotToTemplate(
    ...
    require(balanceOf(_owner, _tokenId) == 1, "You don't have the token!");
    require(_metaverseId > 0, "ID can't be zero!");
    tokenMetaverseURI[_tokenId][_metaverseId] = _metaverseMetadataTemplate;

    emit MetaverseURIAdded(_tokenId, _metaverseId, _metaverseMetadataTemplate);
  }
  ...
  function addBatchMetaverseSlotsToTemplate(
    ...
    for (uint256 i = 0; i < _metaverseIds.length; i++) {
      addMetadataSlotToTemplate(_owner, _tokenId, _metaverseIds[i], _metaverseMetadataTemplate[i]);
    }
  }
  ...
  function addBatchMetaverseSlotToTemplates(
    ...
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      addMetadataSlotToTemplate(_owner,_tokenIds[i], _metaverseId, _metaverseMetadataTemplate);
    }
  }
  ...
  function resetMetaverseURI(
    ...
    tokenMetaverseURI[_tokenId][_metaverseId] = _metaverseTemplate;
  ...
  function uri(uint256 _tokenId) public view override returns (string memory) {
    uint256 templateId = this.checkItemBaseId(_tokenId);
  ...
  function supportsInterface(bytes4 interfaceId) public pure override(ERC1155, IERC165) returns (bool) {
    return (interfaceId == type(IERC1155Receiver).interfaceId) || (interfaceId == type(IERC2981).interfaceId);
  }
