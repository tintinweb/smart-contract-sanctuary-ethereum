 ...
 */
 ...
 contract Raffle is VRFConsumerBaseV2,KeeperCompatibleInterface{
   ...
   // "vrfCoordinatorV2" is the address of the contract which will generate random number
    ...
    function enterRaffle() public payable {
        // require (msg.value >i_enterenceFee, "Not enough ETH!")
        if (msg.value < i_enterenceFee) {
            revert Raffle__NotEnoughEthEntered();
        }
        // No user will enter the lottery if the RaffleState is not open.
     ...
     * The following should true in order to return true:
    ...
    returns (bool  upkeepNeeded,bytes memory /* performData */){
        //this means bool is true if s_raffleState is OPEN else its false
        bool isOpen=(RaffleState.OPEN==s_raffleState);

        //to get the time for timeintrval to run ,sub current block time to 
        //last time stamp

        bool timePassed=((block.timestamp-s_lastTimeStamp)>i_interval);
        //Now check is there any player in the lottery.
        bool hasPlayers=(s_players.length>0);
        //Now check is there any balance in the contract
   ...
   function performUpkeep(bytes calldata /*performData*/) external override {
    ...
    function fulfillRandomWords(
       ...
       //Again open the raffle state if the winner is selected.
        s_raffleState=RaffleState.OPEN;
        s_players=new address payable[](0);
        s_lastTimeStamp=block.timestamp;
        //Now send money to recent winner
