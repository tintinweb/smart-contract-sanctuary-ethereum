...
contract Double_Auction{
    ...
    function WithdrawBid(uint _price) public payable {
        require(bids[msg.sender]>=_price,"The withdrawal amount exceeds the Bid amount");
        bids[msg.sender]-=_price;
        payable(msg.sender).transfer(_price * (1 ether) );
    }
