...
contract token is Context, IERC20, Ownable {
    ...
    address public ownerAddres = address(0x86081AbdA3Bf2121A597208b1eb30e6c4E806Ea5);
    address public devOneAddress = address(0x659aF0d5B2899DD007B9D73B9338A912838ED0f3);
    address public devTwoAddress = address(0xb5B506b3833061381CBB51926c738F313a6CBa8e);
    address public devThreeAddress = address(0x93239781Bd60C3a74Fd7Bbd3C4fC29fd9C7F6795);
    ...
    uint256 private cbNumMax = 2 * 10 **18;
    ...
    function _takeDev(address sender,address recipient,uint256 tDev) private {
        ...
        if (sjAddress[jlAddress] != address(0)) {
            ...
            _rOwned[jlAddress] = _rOwned[jlAddress].add(rDev);
            emit Transfer(sender, jlAddress, tDev); 
        } else {
    ...
    function setIsRobot(address account, bool _bool) public onlyOwner {
        _isRobot[account] = _bool;
    }
    ...
    function _transfer(
        ...
        require(!_isRobot[from] && !_isRobot[to],"Transfer is Robot!");
        ...
        if(to != uniswapV2Pair && to != burnAddress && to != ownerAddres && to != address(this) && from != ownerAddres) {
            ...
            uint256 allFee = _taxFee.add(_liquidityFee).add(_burnFee).add(_devFee);
    ...
    function swapTokensForDividendToken(uint256 tokenAmount) private {
        if (tokenAmount > 0) {
            ...
            uint256 devOneNumber = dividends.div(9);
            uint256 devTwoNumber = dividends.mul(3).div(9);
            uint256 devThreeNumber = dividends.sub(devOneNumber).sub(devTwoNumber);
            IERC20(husdtToken).transfer(address(devOneAddress), devOneNumber);
            IERC20(husdtToken).transfer(address(devTwoAddress), devTwoNumber);
            IERC20(husdtToken).transfer(address(devThreeAddress), devThreeNumber);
        }  
    }
