...
contract CreatorNFTContract is ERC721Enumerable, ReentrancyGuard {
    ...
    function adopt(
        ...
        modelInfo[supply + 1] = ModelInfo(_title, _description, _modelId);
    ...
    function walletOfOwnerInDetail(address _owner)
        public
        view
        returns (ModelInfoDetail[] memory)
    {
        uint256 tokenCount = balanceOf(_owner);

        ModelInfoDetail[] memory tokenDetails = new ModelInfoDetail[](tokenCount);
        for (uint256 i; i < tokenCount; i++) {
            uint256 _tokenId = tokenOfOwnerByIndex(_owner, i);
            tokenDetails[i] = ModelInfoDetail(
                _tokenId,
                modelInfo[_tokenId].title,
                modelInfo[_tokenId].description,
                modelInfo[_tokenId].modelId
            );
        }
        return tokenDetails;
    }
