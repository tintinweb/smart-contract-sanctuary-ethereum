...
contract ReentalReferrals is UUPSUpgradeableByRole, RefundableUpgradeable {
  ...
  event Distribute(address account, address to, uint256 amount);

  function initialize () public initializer {
    __AccessControlProxyPausable_init(msg.sender);
    _setFactor(IReentalManager(config).get(keccak256("RNT")), 5 ether);
    _setFactor(IReentalManager(config).get(keccak256("USDT")), 2 ether);
  }

  function updateMerkleRoot(bytes32 merkleRoot_, string memory uri_) public onlyRole(keccak256("REFERRALS_PAYER_ROLE")){
  ...
  function claimable(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) public view returns(uint256){
    ...
    require(MerkleProofUpgradeable.verify(merkleProof, merkleRoot, node), "ReentalReferrals: invalid proof");
  ...
  function distribute(
    ...
    require(claimable_ > 0, "ReentalReferrals: nothing to distribute");
    require(distribution <= claimable_, "ReentalReferrals: amount to distribute exceeds referrals balance claimable");

    distributed[account] += distribution;

    uint256 distributable = distribution * factor[token] / 100 ether; // RNT / USDT?

    address vault = IReentalManager(config).get(keccak256("REENTAL_VAULT"));
    IERC20(token).transferFrom(vault, to, distributable);
    emit Distribute(account, to, distributable);
  }

  function _setFactor(address token, uint256 factor_) internal {
    factor[token] = factor_;
  }

  function setFactor(address token, uint256 factor_) public onlyRole(keccak256("REFERRALS_ADMIN_ROLE")) {
    _setFactor(token, factor_);
  }
}
