...
contract RefineryCOAV1 is OwnableUpgradeable {
    ...
    function deposit(address _upline, uint256 _amount) external {
        ...
        (uint256 realizedDeposit, uint256 taxAmount) = hfuelToken
        ...
        */
        ...
        if (
            _up != address(0) && isNetPositive(_up) && isBalanceCovered(_up, 1)
        ) {
    ...
    function _refPayout(address _addr, uint256 _amount) internal {
        ...
        for (uint8 i = 0; i < ref_depth; i++) {
            ...
            if (isNetPositive(up) && isBalanceCovered(up, i + 1)) {
    ...
    function _claim_out(address _addr) internal {
        ...
        uint256 vaultBalance = hfuelToken.balanceOf(refineryVaultAddress);
        ...
        require(hfuelToken.transfer(address(msg.sender), realizedPayout));
    ...
    function balanceLevel(address _addr) public view returns (uint8) {
        uint8 _level = 0;
        for (uint8 i = 0; i < ref_depth; i++) {
            if (xHNWToken.balanceOf(_addr) < ref_balances[i]) break;
            _level += 1;
        }
    ...
    function maxPayoutOf(address _addr, uint256 _amount)
        public
        view
        returns (uint256)
    {
    ...
    function airdrop(address _to, uint256 _amount) external {
        ...
        (uint256 _realizedAmount, uint256 taxAmount) = hfuelToken
            .calculateTransferTaxes(_addr, _amount);
