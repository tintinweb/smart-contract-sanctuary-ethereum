...
/// @title HEU - Himalaya Euro
contract HEU is HHHmainnet, ReentrancyGuard {

    /// @dev Upper bound limit of the {nonWhitelistedDustThreshold} can be set
    uint256 public nonWhitelistedDustThresholdUpperBound;

    /**
     * @dev Init the value of {nonWhitelistedDustThresholdUpperBound}
     * (recommend  10**17, 10**17 0.1 of the coin)
     * If the input value > {nonWhitelistedDustThresholdUpperBound}, it will set as {nonWhitelistedDustThresholdUpperBound}
     *
     * Requirements:
     *
    ...
    /**
     * @dev Updates the {nonWhitelistedDustThresholdUpperBound}
     * Update {nonWhitelistedDustThresholdUpperBound}
     *
     * Requirements:
     *
     * - the caller must be admin - {onlyAdmin} modifier is applied.
     */
    function updateNonWhitelistedDustThresholdUpperBound(uint256 _upperBound) external virtual onlyAdmin {
        nonWhitelistedDustThresholdUpperBound = _upperBound;
    }
     ...
     * - the caller must be admin - {onlyAdmin} modifier is applied.
     */
    function setNonWhitelistedDustThreshold(uint256 _nonWhitelistedDustThreshold) external virtual override onlyAdmin {
        if (_nonWhitelistedDustThreshold > nonWhitelistedDustThresholdUpperBound) {
            nonWhitelistedDustThreshold = nonWhitelistedDustThresholdUpperBound;
        }
     ...
     */
    function recoverFrozenFunds(address from, address to, uint256 amount) external virtual override onlyAdmin noReentrant {
        ...
        if (!managementContract.isWhitelisted(from)) {
            super.removeAllPendingDeposits(from);
        }
...
abstract contract ReentrancyGuard {
    bool internal rlocked;

    modifier noReentrant() {
        require(!rlocked, "No re-entrancy");
        rlocked = true;
        _;
        rlocked = false;
    }
