...
interface IMaster {
    ...
    function becomeOracle(string calldata _metricName, string calldata _tag, string calldata _description) external;
    ...
    function getId(address _oracle) external view returns(bytes32);

    function getTag(address _oracle) external view returns(string memory);
}
...
contract Master is ReentrancyGuard, AccessControl, IMaster {
    string public constant kycMetricName = 'KYC_ORACLE';
    bytes32 public constant KYC_ORACLE = keccak256(abi.encodePacked(kycMetricName));
    bytes32 public constant ADMIN_ROLE = keccak256('ADMIN_ROLE');

    uint public proposalDuration;
    uint public memberQuantity;

    mapping(address => bytes32) public oracleToId;
    ...
    constructor(address[] memory _initMemebers, uint _proposalDuration) {
        for(uint i = 0; i < _initMemebers.length; i++) {
            community[_initMemebers[i]] = true;
            memberQuantity += 1;
        }
    ...
    function becomeOracle(string calldata _metricName, string calldata _tag, string calldata _description) external nonReentrant {
        address sender = _msgSender();
        uint endTime = block.timestamp + proposalDuration;
        OracleProposal memory currentProposal;
        currentProposal.id = keccak256(abi.encodePacked(sender, endTime));
        currentProposal.description = _description;
        currentProposal.oracle = sender;
        currentProposal.endTime = endTime;
        if(_isEmptyString(_metricName) && !_isEmptyString(_tag)) { 
            require(_isEmptyString(tags[sender]), 'Address already in use');
            currentProposal.tag = _tag;
            OracleProposals.push(currentProposal);
        } else if(!_isEmptyString(_metricName) && _isEmptyString(_tag)) {
            require(oracleToId[sender] == bytes32(0), 'Address already in use');
            currentProposal.metricName = _metricName;
            OracleProposals.push(currentProposal);
        } else {
    ...
    function voteForOracle(uint _id) external {
        ...
        require(!votes[proposal.id][sender], 'You already voted');
        proposal.support += 1;
        votes[proposal.id][sender] = true;
    }
    
    function finishOracleVoiting(uint _id) external {
        OracleProposal memory proposal = OracleProposals[_id];
        require(proposal.endTime <= block.timestamp, 'Time is not over yet');
        _removeOracleProposal(_id, OracleProposals);
        if (_isMoreThanHalf(memberQuantity, proposal.support)) {
            if (_isEmptyString(proposal.metricName)) {
                tags[proposal.oracle] = proposal.tag;
                oracleToId[proposal.oracle] = KYC_ORACLE;
            } else if (_isEmptyString(proposal.tag)) {
                oracleToId[proposal.oracle] = _stringToHash(proposal.metricName);
            }
        }
    }
    ...
    function removeOracle(address _oracle) external onlyRole(ADMIN_ROLE) {
        require(oracleToId[_oracle] != bytes32(0), "Slot empty");
        if (!_isEmptyString(tags[_oracle])) {
            tags[_oracle] = '';
        }
        oracleToId[_oracle] = bytes32(0);
    }
