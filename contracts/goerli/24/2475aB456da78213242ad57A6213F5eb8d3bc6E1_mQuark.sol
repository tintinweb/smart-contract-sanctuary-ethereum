...
contract mQuark is ERC721, IERC2981 {
  ...
  event NFTMinted(address project, uint256 templateId, uint256 collectionId, uint256 tokenId, address to);
  ...
  **/
  mapping(address => bool) public authorizedContracts;
   ...
   /**
   *  @dev Mapping from 'Wallet Address' -> 'Template ID' to collection ID
   *  This accesses the last ID of selected collection
  **/
  mapping(address => mapping(uint256 => uint256)) projectCollectionIds;

  /**
   *  @dev Mapping from 'Template ID' -> 'Wallet Address' -> 'Collection ID ' to Collection Data
   *  This accesses the info of the created collections
  **/
  mapping(uint256 => mapping(address => mapping(uint256 => CollectionData))) public projectCollections;
  ...
  */
  function mint(
    ...
    CollectionData memory _tempData = projectCollections[templateId][project][collectionId];
    ...
    _mint(to, _tokenId);
    if (_tempData.isCollectionMetadataSet) tokenIdURIs[_tokenId] = _tempData.collectionURI;
    else tokenIdURIs[_tokenId] = templateURIs[templateId];
    ++projectCollections[templateId][project][collectionId].mintCount;

    emit NFTMinted(project, templateId, collectionId, _tokenId, to);
  }
  ...
  function mintBatch(
    ...
    require(collectionParams.templateIds.length == collectionParams.collectionIds.length, "ids mismatch");//*** */
    require(collectionParams.templateIds.length == collectionParams.amounts.length, "amount mismatch");//*** */

    CollectionData memory _tempData;
    uint256 _tokenId;

    uint8 _templateIdsLength = uint8(collectionParams.templateIds.length);
    for (uint8 i; i < _templateIdsLength;) {
      require(collectionParams.amounts[i] <= 10, "exceeds mint amount");
      uint8 _mintCount = collectionParams.amounts[i];
      require(collectionParams.templateIds[i] != 0 && _mintCount != 0, "invalid id/amount");
      
      _tempData = projectCollections[collectionParams.templateIds[i]][collectionParams.project][collectionParams.collectionIds[i]];
      require(_tempData.minTokenId != 0, "unexisting token mint");
      require(
        (_tempData.mintCount + _mintCount) <= (_tempData.amount),
        "no enough supply"
      );
      ...
      for (uint8 ii; ii < _mintCount;) {
        ...
        ++_tempData.mintCount;
        if (_tempData.isCollectionMetadataSet) tokenIdURIs[_tokenId] = _tempData.collectionURI;
        else tokenIdURIs[_tokenId] = templateURIs[collectionParams.templateIds[i]];
        emit NFTMinted(collectionParams.project, collectionParams.templateIds[i], collectionParams.collectionIds[i], _tokenId, to);
        unchecked { ++ii; }
      }

      projectCollections[collectionParams.templateIds[i]][collectionParams.project][collectionParams.collectionIds[i]]
  ...
  function mintBatchWithMetadataSlot(
    ...
    require(collectionParams.templateIds.length < 256, "mint items are too many");
    require(collectionParams.templateIds.length == collectionParams.collectionIds.length, "template id and index mismatch");
    require(collectionParams.amounts.length == collectionParams.collectionIds.length, "amount and index mismatch");
    ...
    for (uint8 i; i < _templateIdsLength;) {
      require(
        collectionParams.templateIds[i] != 0 && collectionParams.collectionIds[i] != 0,
        "invalid id/index"
      );
      _tempData = projectCollections[collectionParams.templateIds[i]][collectionParams.project][collectionParams.collectionIds[i]];
      require(_tempData.minTokenId != 0, "unexisting token mint");
      require((_tempData.mintCount) <= (_tempData.amount), "no enough supply");
      uint8 _metadataSlotCount = collectionParams.amounts[i];
      for (uint8 ii; ii < _metadataSlotCount;) {
        _tokenId = _tempData.minTokenId + _tempData.mintCount;
        _mint(to, _tokenId);
        _tokenProjectURIs[_tokenId][projectId] = projectMetadataTemplate;
        if (_tempData.isCollectionMetadataSet) tokenIdURIs[_tokenId] = _tempData.collectionURI;
        else tokenIdURIs[_tokenId] = templateURIs[collectionParams.templateIds[i]];

        ++_tempData.mintCount;
        emit NFTMinted(collectionParams.project, collectionParams.templateIds[i], collectionParams.collectionIds[i], _tokenId, to);
        emit ProjectSlotAdded(_tokenId, projectId, projectMetadataTemplate);
      
        unchecked { ++ii; }
      }

      projectCollections[collectionParams.templateIds[i]][collectionParams.project][collectionParams.collectionIds[i]]
        .mintCount += _metadataSlotCount;
  ...
  function mintWithMetadaSlots(
    ...
    string[] calldata projectMetadataTemplates
  ) external onlyAuthorizedContract {
    require(templateId != 0, "invalid template id");

    CollectionData memory _tempData = projectCollections[templateId][projects[0]][collectionId];
    require(_tempData.mintCount <= _tempData.amount, "no enough supply");
    uint256 _tokenId = _tempData.minTokenId + _tempData.mintCount;
    _mint(to, _tokenId);
    if (_tempData.isCollectionMetadataSet) tokenIdURIs[_tokenId] = _tempData.collectionURI;
    else tokenIdURIs[_tokenId] = tokenIdURIs[_tokenId] = templateURIs[templateId];
    ++projectCollections[templateId][projects[0]][collectionId].mintCount;
    addBatchMetadataSlotsToTemplate(to, _tokenId, projectIds, projectMetadataTemplates);
    emit NFTMinted(projects[0], templateId, collectionId, _tokenId, to);
  }
  ...
  function addMetadataSlotToTemplate(
    ...
    _tokenProjectURIs[tokenId][projectId] = projectMetadataTemplate;

    emit ProjectSlotAdded(tokenId, projectId, projectMetadataTemplate);
  }

  ///@dev Adds different multiple metadata slots to a single NFT
  //Different Multiple Metadata Slots => Single NFT
  function addBatchMetadataSlotsToTemplate(
    address owner,
    uint256 tokenId,
    uint256[] calldata projectIds,
    string[] calldata projectMetadataTemplate
    ...
    for (uint256 i = 0; i < projectCount;) {
      addMetadataSlotToTemplate(owner, tokenId, projectIds[i], projectMetadataTemplate[i]);
      unchecked { ++i; }
    }
  }
  ...
  function addBatchMetadataSlotToTemplates(
    address owner,
    ...
    for (uint8 i; i < mintingLength;) {
      addMetadataSlotToTemplate(owner, tokenIds[i], projectId, projectMetadataTemplate);
      unchecked { ++i; }
    }
  }
  ...
  function createCollections(
    ...
    for (uint256 i; i < templateCount;) {
      _collectionId = ++projectCollectionIds[project][_templateIds[i]];
      CollectionData memory _tempData = projectCollections[_templateIds[i]][project][_collectionId];
   ...
   */
  function setTemplateCategory(string memory category, uint256[] calldata templates) external onlyAdmin {
  ...
  function updateURI(
    ...
    require(ownerOf(tokenId) == owner,"you are not the owner");
    bool isVerified = _verifyUpdateURISignature(signature, project, projectId, tokenId, newURI);
    if (isVerified) {
  ...
  */
  ...
  function _verifyNameSignature(
    ...
    bytes32 messageHash = keccak256(abi.encode(_admin, project, templateId, collectionId, uri));
  ...
  ///@dev Checks the validity of given signature
  function _verifyUpdateURISignature(
    bytes memory signature,
    address project,
    uint256 projectId,
    uint256 tokenId,
