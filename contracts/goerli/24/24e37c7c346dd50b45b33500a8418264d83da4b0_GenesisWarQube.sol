...
contract GenesisWarQube is ERC721A, Ownable, ReentrancyGuard {
    ...
    string public prerevealURL = 'IPFS LINK';
    bool private _addJson = true;

    uint16 constant MAX_GENESIS = 2500;
    uint16 constant TEAM_NFT = 250;

    bool private _publicSaleActive = false;

    mapping(address => uint) private _mintedPerAddress;

    constructor() ERC721A("Genesis WarQube", "GENWQ") Ownable() ReentrancyGuard() { }
    
    function mint(uint16 quantity) public payable nonReentrant {
        
        require(_totalMinted() + quantity <= MAX_GENESIS, "Maximum amount of avatars reached");
    
        if (msg.sender != owner()) {

            require(quantity > 0 && quantity <= 5 && balanceOf(msg.sender) + quantity <= 5, "Minting is limited to max. 5 per wallet");
