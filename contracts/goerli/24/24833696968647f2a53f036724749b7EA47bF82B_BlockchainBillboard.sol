...
contract BlockchainBillboard is ERC721, ReentrancyGuard, Ownable, Pausable {
  ...
  function mint(
    ...
    require(verifySignature(tokenId, cid, fee, signature), "Invalid signature");
  ...
  function getTokenRange(
    ...
    require(to <= _highestTokenMinted, "Invalid range");

    TokenData[] memory tokens = new TokenData[](to - from + 1);
    for (uint256 i = from; i <= to; i++) {
      if (_exists(i))
        tokens[i - from] = TokenData({tokenId: i, cid: _tokenCid[i]});
    }
    return tokens;
  }
  ...
  function verifySignature(
    ...
    bytes32 payloadHash = keccak256(abi.encode(tokenId, cid, fee));
