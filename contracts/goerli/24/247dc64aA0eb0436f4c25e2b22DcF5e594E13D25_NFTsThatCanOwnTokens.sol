...
contract NFTsThatCanOwnTokens is ERC721A("", ""), ReentrancyGuard {
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
      ...
      if(isSellable) {
        parts = string(abi.encodePacked(parts, "This token is buyable!", '</text><text x="10" y="40" class="base">'));
      } else {
        parts = string(abi.encodePacked(parts, "This token is locked and unbuyable!", '</text><text x="10" y="40" class="base">'));
      }
