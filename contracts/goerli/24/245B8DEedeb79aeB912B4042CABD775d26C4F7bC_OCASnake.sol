...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    //mapping for attrivutes

    function metaDataCreator(
        string[3] memory vars,
        ...
        return
            abi.encodePacked(
                metaBytes,
                attributeCreator(attributes),
                metadata[3]
            );
    }

    function attributeCreator(Attributes[10] memory attributes)
        private
        pure
        returns (bytes memory)
    {
        ...
        for (uint8 i = 0; i < attributes.length; i++) {
            attributeBytes = abi.encodePacked(
                i == 0 ? bytes('"attributes":[') : attributeBytes,
                '{"trait_type":"',
    ...
    function getTokenSaleInfo(uint256 tokenId) public view returns (uint256) {}
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < 10; i++) {
            attributes[i] = Attributes("test", "terd");
        }

        string memory json = Base64.encode(
            metaDataCreator(metadataVars, attributes)
        );
...
contract OCASnakeArt is Ownable, ReentrancyGuard {
    ...
    function allYourBaseAreBelongToUs(uint256 tokenId)
        ...
        returns (string memory)
    {
        string[30] memory selectedColors = retrofy(tokenId);
    ...
    function retrofy(uint256 tokenId) public view returns (string[30] memory) {
        string[30] memory parts;
        string[4] memory colors;

        for (uint256 i = 0; i < 20; i++) {
            for (uint256 index = 0; colors.length - 1 < 3; index++) {
    ...
    function cloneSnake(uint256 tokenId) private view returns (bytes memory) {
        ...
        uint256 randomColorArrayLength = (generateAppleSeed(tokenId, 69) %
            uint256(10)) + uint256(16);
        for (uint256 i = 0; i < randomColorArrayLength; i++) {
