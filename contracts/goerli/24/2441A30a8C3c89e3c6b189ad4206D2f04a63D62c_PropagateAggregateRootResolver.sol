...
contract PropagateAggregateRootResolver is IResolver, ProposedOwnable {
  IRootManager public immutable ROOT_MANAGER;
  uint256 public EXECUTION_INTERVAL;
  uint256 public lastExecuted;

  constructor(address _rootManager, uint256 _executionInterval) ProposedOwnable() {
    require(_executionInterval > 0, "SendOutboundRootResolver: execution interval must be > 0");
    _setOwner(msg.sender);
    ROOT_MANAGER = IRootManager(_rootManager);
  ...
  function checker() external view override returns (bool canExec, bytes memory execPayload) {
    if ((lastExecuted + EXECUTION_INTERVAL) > block.timestamp) {
      ...
      execPayload = abi.encodeWithSelector(this.propagate.selector);
