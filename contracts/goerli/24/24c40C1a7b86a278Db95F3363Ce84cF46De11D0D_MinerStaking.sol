...
contract MinerStaking is MinerAccessControlled {
     ...
     * @param _rebasing  Whether to rebase the stake.
     * @return uint
     */
    function stake(
        address _to,
    ...
    function claim(address _to, bool _rebasing) public returns (uint256) {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            return _send(_to, vMINER.balanceForGons(info.gons), _rebasing);
        }
        return 0;
    }
    ...
    function unstake(
        ...
        if (_rebasing) {
            ...
            gMINER.burn(msg.sender, _amount); // amount was given in gMINER terms
            amount_ = gMINER.balanceFrom(amount_).add(bounty); // convert amount to MINER terms & add bounty
        }
    ...
    function _send(
        ...
        if (_rebasing) {
            ...
            gMINER.mint(_to, gMINER.balanceTo(_amount)); // send as gMINER (convert units from MINER)
            return gMINER.balanceTo(_amount);
        }
    }
