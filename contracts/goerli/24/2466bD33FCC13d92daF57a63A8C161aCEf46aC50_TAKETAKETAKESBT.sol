...
contract TAKETAKETAKEMAIN is ERC721Enumerable, Ownable {
    TAKETAKETAKESBT public cntAddr;

    constructor(address _cntAddr) ERC721("TAKETAKETAKEMAIN", "TAKEM") {
        require(_cntAddr != address(0x0));
        cntAddr = TAKETAKETAKESBT(_cntAddr);
    }

    function getClaimCount() public view returns (uint256) {
        return cntAddr.getTotalMintedCount(msg.sender);
    }
}
...
contract TAKETAKETAKESBT is ERC721Enumerable, Ownable {
    using Strings for *;

    string baseURI;
    string public baseExtension = ".json";
    uint256 private _baseCost = 0.0001 ether;
    ...
    constructor() ERC721("TAKETAKETAKE", "TAKE") {}

    function _baseURI() internal view virtual override returns (string memory) {
        return "ipfs://QmZnxW9dpvPzp6BLDVDrb4LzHUm7ExN7ZYoHtcwTsZkycw/";
    }
    ...
    function totalMintPrice(uint256 _mintAmount)
        private
        view
        returns (uint256)
    {
        return _baseCost * _mintAmount;
    }

    function _publicMint(uint256 _mintAmount) private {
        uint256 mintableCount = 5;
        uint256 mintedCount = getMintedCount(msg.sender);
        require(mintedCount + _mintAmount <= mintableCount);
        ...
        payable
    {
        ...
        require(totalMintCount + _mintAmount <= maxSupply);
        require(msg.value == totalMintPrice(_mintAmount));
    ...
    function setSilverMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        silverMerkleRoot = _merkleRoot;
    }

    function setBronzeMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
