...
contract ItemNFT is ERC721Enumerable {
    ...
    constructor(
        ...
        unit = _unit;
        category = _category;
        abnormalWeight = _abnormalWeight;
    ...
    function mint(
        uint256 tokenId_,
        string memory itemName_,
        string memory code_,
        uint256 quantity_,
        string memory unit_,
        string memory category_,
        bool abnormalWeight_,
        address toAddress_
    ) public {
        Item memory newItem = Item({
            ...
            unit: unit_,
            category: category_,
            abnormalWeight: abnormalWeight_,
            toAddress: toAddress_
        });
        _items[_totalSupply] = newItem;
        _totalSupply++;
    ...
    function transferItemNFT(address recipient, uint256 tokenId) public {
...
contract Machine is ERC721Enumerable {
    ...
    function process(
        uint256 itemTokenIdCounter_,
        ItemNFT[] memory items,
        ItemNFT outputItem,
        address nextMachine
    ) public {
        uint256 tokenId = 0;
        uint256 numberOfTokensInNextMachine = 0;
        uint256 _itemTokenIdCounter = itemTokenIdCounter_;

        // Loop through the items array and process each item
        for (uint256 i = 0; i < items.length; i++) {
            // Destroy the original token
            tokenId = items[i].tokenByIndex(0);
            items[i].destroy(tokenId);

            (
                uint256 itemTokenIdCounter,
                ...
                bool abnormalWeight,
                address toAddress
            ) = outputItem.getItem(0);

            // Mint a new NFT with updated item details
            if (i == 0) {
                outputItem.mint(
                    _itemTokenIdCounter + 1,
                    itemName,
                    code,
                    quantity,
                    unit,
                ...
                numberOfTokensInNextMachine = outputItem.balanceOf(nextMachine);

                outputItem.transferItemNFT(
                    nextMachine,
                    numberOfTokensInNextMachine + 1
                );
            }
        }
    }

    function destroyFromMachine(ItemNFT itemNFT_, uint256 tokenId) public {
        itemNFT_.destroy(tokenId);
    }

    function transferItemNFT(address recipient, uint256 _tokenId) public {
        transferFrom(address(this), recipient, _tokenId);
    }
}