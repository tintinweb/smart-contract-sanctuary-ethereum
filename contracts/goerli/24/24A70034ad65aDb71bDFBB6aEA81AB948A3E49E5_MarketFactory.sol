...
contract MarketFactory is IMarketFactory {
        ...
        returns (address)
    {
        ...
        /*bNFTMaker bMaker = new bNFTMaker(bName, bSymbol, token, address(config) );
        bNFTTaker bTaker = new bNFTTaker(bName, bSymbol, token, address(config) );

        // add b-Token to the protocol config
        config.setNFT( token, address(bTaker), address(bMaker) );*/
...
contract ProtocolConfig is IProtocolConfig, Initializable, BACUpgradable {
    ...
    function getPremiumMultiplier(
        ...
        uint16 risk,
        uint16 term
    ) external view override returns (int128 multiplier) {
        uint16 periodIndex = getPeriodIndex(term);
        uint16 riskIndex = getRiskIndex(risk);
        multiplier = marketConfigs[token].Premium_Mul[riskIndex][periodIndex];
    }
    ...
    function getYieldMultiplier(
        ...
        uint16 periodIndex = getPeriodIndex(term);
        multiplier = marketConfigs[token].Yield_Mul[riskIndex - 1][periodIndex];
    }
...
contract bNFTMaker is bNFTBase {
    ...
    constructor (    
        ...
        address _token, 
        address _config        
    ) bNFTBase(name, symbol) {
        config = IProtocolConfig(_config);
        market = config.getMarket(_token);
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal override {
        ...
        IMarketPositionTransfer(market).safeTransferMakerPosition(from,to, tokenId);
    }   
}
...
contract bNFTTaker is bNFTBase {
    ...
    function _transfer(address from, address to, uint256 tokenId) internal override { 
        super._transfer(from,to, tokenId);               
        IMarketPositionTransfer(market).safeTransferTakerPosition(from,to, tokenId);
    }
...
/// @notice NFT token for taker position
abstract contract bNFTBase is Context, ERC165, IERC721, IERC721Metadata, IPositionNFT {
    using Address for address;
    using Strings for uint256;
