...
contract TAVesting is KeeperCompatibleInterface, ReentrancyGuard {
  /*==================================================== State Variables ====================================================*/
  Vesting vesting;
  ...
  function checkUpkeep(
    ...
    for (uint256 i = 0; i < _users.length; i++) {
      ...
      for (uint256 k = 0; k < _indexes.length; k++) {
        uint256 _last = vesting.getInvestorInfo(_users[i], 2, _indexes[k]).lastReceivedTime;
        if (upkeepNeeded = (block.timestamp > startTime) && ((block.timestamp - _last) > 180 days)) {
          performData = abi.encode(_users[i], _last, 2, _indexes[k]);
          break;
        }
      }
      if (outBreak) break;
  ...
  function performUpkeep(bytes calldata performData) external override nonReentrant {
    ...
    require((block.timestamp - _last) > 180 days, "You Shall Not Pass!");
