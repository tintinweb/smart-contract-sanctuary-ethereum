...
/// @notice An adapter contract enabling raise mints via paper.xyz
contract PaperAdapter is IPaperAdapter, Controllable, ReentrancyGuard {
    ...
    address internal immutable mintETHImpl;
    address internal immutable mintERC20Impl;
    ...
    function mint(
        ...
        if (currency == ETH) {
            ...
            IERC20(currency).safeTransfer(address(minter), mintPrice);
            minter.mint(
                MintERC20Params({
                    raises: IRaises(raises),
                    tokens: ITokens(tokens),
