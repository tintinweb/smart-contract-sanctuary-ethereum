...
contract ZivoeGlobals is ZivoeOwnableLocked {
    ...
    function initializeGlobals(address[] calldata globals) external onlyOwner {
        ...
        stablecoinWhitelist[0x419E71ec137232E15E7188A162fD34588fDFc35a] = true; // DAI
        stablecoinWhitelist[0xa75E25235dEe5deb25d2Bc4D522BBAeCBDe62856] = true; // USDC
        stablecoinWhitelist[0xEa4Fe751f00dA9Abb7a8c4823A1b554f9eE9BBeF] = true; // USDT
    }
...
contract ZivoeITO is Context {
    ...
    constructor(
        ...
        stablecoinWhitelist[0x419E71ec137232E15E7188A162fD34588fDFc35a] = true; // DAI
        stablecoinWhitelist[0x6c7220C502bBaeeF9f64C9390F625d209AB07bc7] = true; // FRAX
        stablecoinWhitelist[0xa75E25235dEe5deb25d2Bc4D522BBAeCBDe62856] = true; // USDC
        stablecoinWhitelist[0xEa4Fe751f00dA9Abb7a8c4823A1b554f9eE9BBeF] = true; // USDT
    }
    ...
    {
        require(
            block.timestamp > end || migrated,
            "ZivoeITO::claim() block.timestamp <= end && !migrated"
        );

        address caller = _msgSender();

        require(
            !airdropClaimed[caller],
            "ZivoeITO::claim() airdropClaimeded[caller]"
        );
        require(
            seniorCredits[caller] > 0 || juniorCredits[caller] > 0,
            "ZivoeITO::claim() seniorCredits[caller] == 0 && juniorCredits[caller] == 0"
        );
        ...
        uint256 middle = IERC20(ITO_IZivoeGlobals(GBL).ZVE()).totalSupply() /
            10;
        uint256 lower = IERC20(ITO_IZivoeGlobals(GBL).zSTT()).totalSupply() *
            3 +
        ...
        IERC20(ITO_IZivoeGlobals(GBL).zJTT()).safeTransfer(
            caller,
            juniorCreditsOwned
        );
        IERC20(ITO_IZivoeGlobals(GBL).zSTT()).safeTransfer(
            caller,
            seniorCreditsOwned / 3
        );
        IERC20(ITO_IZivoeGlobals(GBL).ZVE()).safeTransfer(
            caller,
            (upper * middle) / lower
        );
    ...
    function depositJunior(uint256 amount, address asset) external {
        require(
            ...
            "ZivoeITO::depositJunior() block.timestamp < start"
        );
        require(
            block.timestamp < end,
            "ZivoeITO::depositJunior() block.timestamp >= end"
        );
        require(!migrated, "ZivoeITO::depositJunior() migrated");
        require(
            stablecoinWhitelist[asset],
            "ZivoeITO::depositJunior() !stablecoinWhitelist[asset]"
        );

        address caller = _msgSender();

        uint256 standardizedAmount = ITO_IZivoeGlobals(GBL).standardize(
            amount,
            asset
        );
        ...
        ITO_IERC20Mintable(ITO_IZivoeGlobals(GBL).zJTT()).mint(
            address(this),
            standardizedAmount
        );
    }
    ...
    function depositSenior(uint256 amount, address asset) external {
        require(
            block.timestamp >= start,
            "ZivoeITO::depositSenior() block.timestamp < start"
        );
        require(
            block.timestamp < end,
            "ZivoeITO::depositSenior() block.timestamp >= end"
        );
        require(!migrated, "ZivoeITO::depositSenior() migrated");
        require(
            stablecoinWhitelist[asset],
            "ZivoeITO::depositSenior() !stablecoinWhitelist[asset]"
        );
        ...
        emit SeniorDeposit(
            ...
            asset,
            amount,
            standardizedAmount * 3,
            standardizedAmount
        );

        IERC20(asset).safeTransferFrom(caller, address(this), amount);
        ITO_IERC20Mintable(ITO_IZivoeGlobals(GBL).zSTT()).mint(
            address(this),
            standardizedAmount
        );
    }
    ...
    function migrateDeposits() external {
        ...
        emit DepositsMigrated(
            IERC20(0x419E71ec137232E15E7188A162fD34588fDFc35a).balanceOf(
                address(this)
            ),
            IERC20(0x6c7220C502bBaeeF9f64C9390F625d209AB07bc7).balanceOf(
                address(this)
            ),
            IERC20(0xa75E25235dEe5deb25d2Bc4D522BBAeCBDe62856).balanceOf(
                address(this)
            ),
            IERC20(0xEa4Fe751f00dA9Abb7a8c4823A1b554f9eE9BBeF).balanceOf(
                address(this)
            )
        );

        IERC20(0x419E71ec137232E15E7188A162fD34588fDFc35a).safeTransfer(
            ITO_IZivoeGlobals(GBL).ZVL(),
            (IERC20(0x419E71ec137232E15E7188A162fD34588fDFc35a).balanceOf(
                address(this)
            ) * operationAllocation) / BIPS // DAI
        );
        IERC20(0x6c7220C502bBaeeF9f64C9390F625d209AB07bc7).safeTransfer(
            ITO_IZivoeGlobals(GBL).ZVL(),
            (IERC20(0x6c7220C502bBaeeF9f64C9390F625d209AB07bc7).balanceOf(
                address(this)
            ) * operationAllocation) / BIPS // FRAX
        );
        IERC20(0xa75E25235dEe5deb25d2Bc4D522BBAeCBDe62856).safeTransfer(
            ITO_IZivoeGlobals(GBL).ZVL(),
            (IERC20(0xa75E25235dEe5deb25d2Bc4D522BBAeCBDe62856).balanceOf(
                address(this)
            ) * operationAllocation) / BIPS // USDC
        );
        IERC20(0xEa4Fe751f00dA9Abb7a8c4823A1b554f9eE9BBeF).safeTransfer(
            ITO_IZivoeGlobals(GBL).ZVL(),
            (IERC20(0xEa4Fe751f00dA9Abb7a8c4823A1b554f9eE9BBeF).balanceOf(
                address(this)
            ) * operationAllocation) / BIPS // USDT
        );
