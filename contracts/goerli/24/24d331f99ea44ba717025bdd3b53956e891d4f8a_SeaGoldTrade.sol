...
contract SeaGoldTrade is Initializable, OwnableUpgradeable {
    ...
    function acceptBId(
        ...
        require(
            netamount + covertToken <= salestruct.amount,
            "Amount is not Equl"
        );
        IWETH convertToken = IWETH(tokenAddress);
        convertToken.transferFrom(salestruct.user, address(this), covertToken);
        convertToken.withdraw(covertToken);
        (bool sent, ) = buyBackAddress.call{value: covertToken}("");
        require(sent, "Failed to send Ether");
        t.transferFrom(salestruct.user, msg.sender, netamount);
        transferNft(
            salestruct.conAddr,
            salestruct.tokenId,
            salestruct.seller,
    ...
    function calculaterewardsAndRoyaltyBid(
        ...
        uint256 buyBackPercent = betaLaunched == true ? buyBackFee : 0;
