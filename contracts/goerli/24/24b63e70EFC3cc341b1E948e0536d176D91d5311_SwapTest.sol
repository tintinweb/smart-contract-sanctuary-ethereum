...
contract Swap is SwapGovernance {
    ...
    function _swapToken0x(
        ...
        if (srcAddr != address(0)) {
            swapDescriptionObj.inputToken = srcAddr;
        }

        if (dstAddr != address(0)) {
            swapDescriptionObj.outputToken = dstAddr;
        }
        ...
        if (address(swapDescriptionObj.outputToken) == NATIVE_ADDRESS) {
            ...
            payable(msg.sender).transfer(swapDescriptionObj.inputTokenAmount);
        } else {
            IERC20 swapOutputToken = IERC20(swapDescriptionObj.outputToken);
...
contract SwapTest is Swap, SwapGovernanceTest {
    ...
    constructor(address _feeCollector, address _1inchRouter, address _0xRouter) Swap(_feeCollector, _1inchRouter, _0xRouter) {
    }
     ...
     * @param _1inchData a 1inch data to call aggregate router to swap assets.
     */
    function swapTokens(bytes calldata _1inchData, bytes calldata _0xData, uint256 amount) external payable override returns (uint) {
        if (_1inchData.length > 1) {
            ...
            if (swapDescriptionObj.inputToken != NATIVE_ADDRESS) {
                require(tokensMap[IERC20(swapDescriptionObj.inputToken)] != address(0), "Key Pair: mock does not exist for src token");
            }

            if (swapDescriptionObj.outputToken != NATIVE_ADDRESS) {
                require(tokensMap[IERC20(swapDescriptionObj.outputToken)] != address(0), "Key Pair: mock does not exist for dst token");
            }
             return _swapToken0x(_0xData, amount, tokensMap[IERC20(swapDescriptionObj.inputToken)], tokensMap[IERC20(swapDescriptionObj.outputToken)]);
        }
