...
*/
contract IssuerRegistry is CertificateHolder {
    struct Issuer {
    ...
    function addIssuerToRegistry(address payable _issuerAddress, bool _isKycComplete, bytes32 _kycCert, string calldata _name) external onlyOwner() {
        ...
        createCert(_kycCert, _issuerAddress);
    ...
    function deleteIssuerFromRegistry(address _issuerAddress, bytes32 _existingCert) external onlyOwner onlyIfIssuerExistsInRegistry(_issuerAddress) {
        removeCertificate(_existingCert);
        delete issuers[_issuerAddress];
        emit IssuerRemovedFromRegistry(_issuerAddress);
    }

    function updateCertificate(address _issuerAddress, bytes32 _existingCert, bytes32 _newCert) public onlyOwner onlyIfIssuerExistsInRegistry(_issuerAddress) {
        updateCertificate(_newCert, _existingCert);
    }
    ...
    function isIssuerCertified(address _issuerAddress, bytes32 _cert) view  external returns (bool){
        return isUserCertified(_cert, _issuerAddress);
    }
