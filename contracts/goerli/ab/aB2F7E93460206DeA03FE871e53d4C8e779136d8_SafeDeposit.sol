...
contract SafeDeposit is Token{
    ...
    function deposit(address token, uint256 amount) public {
        ...
        // Update balance
        balances2[token][msg.sender] += amount;
    }

    function withdraw(address token, uint256 amount) public {
        require(amount > 0, "Withdrawal amount must be greater than zero.");
        require(token != address(0), "Token address must be valid.");
        require(balances2[token][msg.sender] >= amount, "Insufficient balance.");
    ...
    function balanceOf_erc20(address account, address token) public view returns (uint256) {
        return balances2[token][account];
    }
}
