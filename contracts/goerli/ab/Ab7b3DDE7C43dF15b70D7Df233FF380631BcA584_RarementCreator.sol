    ...
    OwnableUpgradeable
{
    ...
    function initialize(
        ...
        rarementId = id;
        _numAvailableTokens = rarementData.maxSupply;
        info = rarementData;
    }
    ...
    function totalSupply() public view virtual returns (uint256) {
        return info.maxSupply - _numAvailableTokens;
    }
        ...
        onlyOwner
    {
        ...
        require(
            to.length * quantity <= _numAvailableTokens,
        ...
        unchecked {
            ...
            for (uint256 i; i != toLength; ++i) {
                _mintRarement(to[i], quantity);
            }
        }

        emit Airdropped(rarementId, to, quantity, _numAvailableTokens);
    }
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(info.endTime > block.timestamp, "Auction has ended");

        // Mint a new token for the sender, using the `tokenId`.
        uint256[] memory tokenIds = _mintRarement(msg.sender, quantity);
    ...
    function _mintRarement(address to, uint256 numToMint)
        internal
        virtual
        returns (uint256[] memory)
    {
        require(to != address(0), "ERC721: mint to the zero address");
        require(numToMint > 0, "ERC721: need to mint at least one token");

        // TODO: Probably don't need this as it will underflow and revert automatically in this case
        require(
            _numAvailableTokens >= numToMint,
            "ERC721: minting more tokens than available"
        );
...
import "./openzeppelin/BeaconProxy.sol";
import "./openzeppelin/UpgradeableBeacon.sol";
