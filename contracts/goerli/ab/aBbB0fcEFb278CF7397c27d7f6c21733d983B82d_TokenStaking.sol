...
contract TokenStaking is Ownable, ReentrancyGuard {
    ...
    function updateYield(address _address, uint256 _yield) external onlyOwner {
        ...
        require(stakingMetaList[_address].depositAmount != 0, "TokenStaking: updateYield: Not deposited.");
        uint256 elapsedPeriod = (block.timestamp.sub(meta.startDepositedAt)).mul(1000).div(ONE_DAY_UNIXTIME);
        uint256 _totalReward = meta.totalReward.add(meta.depositAmount.mul(meta.yield).mul(elapsedPeriod).div(depositPeriod.div(ONE_DAY_UNIXTIME)).div(DEMOMINATOR).div(1000)).div(2);
    ...
    function getTotalReward() public view returns(uint256) {
        ...
        return meta.totalReward.add(
            meta.depositAmount.mul(meta.yield).mul(elapsedPeriod).div(depositPeriod.div(ONE_DAY_UNIXTIME)).div(DEMOMINATOR).div(1000)
        ).div(2);
    }
