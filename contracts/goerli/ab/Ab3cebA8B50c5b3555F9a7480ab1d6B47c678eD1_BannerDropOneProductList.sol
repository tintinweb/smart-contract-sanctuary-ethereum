...
import './IFlavorInfoV3.sol';

/**
 * @title IFlavorInfoProviderV3
 * @author @NFTCulture
 * @dev Interface for Providing a product list definition.
 *
 * Note: This definition is compatible with the V3 version of Flavor Infos.
 */
interface IFlavorInfoProviderV3 is IFlavorInfoV3 {
    function provideFlavorInfos() external view returns (FlavorInfoV3[] memory);

    function tokenURI(uint256 flavorId, uint256 tokenId) external view returns (string memory);
}
 ...
 * @title IFlavorInfoV3
 * @author NFT Culture
 * @dev Interface for FlavorInfoV3 objects.
 *
 *  Bits Layout:
 *    256 bit slot #1
 *    - [0..63]    `flavorId`
...
/**
 * @title IFlavorInfoValidatorV3
 * @author @NFTCulture
 * @dev Interface for validating attempts to mint a Flavor token.
 *
 * Note: This definition is compatible with the V3 version of Flavor Infos.
 */
interface IFlavorInfoValidatorV3 is IFlavorInfoV3 {
    /**
    ...
    function validateMint(
        ...
        address holder,
        uint256 count,
        FlavorInfoV3 memory flavorInfo,
...
// NFTC Prerelease Contracts
import '@nftculture/nftc-contracts-private/contracts/token/v3/IFlavorInfoProviderV3.sol';
import '@nftculture/nftc-contracts-private/contracts/token/v3/IFlavorInfoValidatorV3.sol';
 ...
 */
contract BannerDropOneProductList is IFlavorInfoProviderV3, IFlavorInfoValidatorV3, NFTCDelegateEnforcer, Ownable {
    address private constant ALPHA_ADDRESS = 0xd4BEeE7c22F935D30AcA980b94468B0B3754e639;
    address private constant BETA_ADDRESS = 0x225Bb07563eCEb14976fB18166739CCcd0A731f1;
    ...
    constructor() {
        exclusiveNft200001 = IERC721(ALPHA_ADDRESS);
        exclusiveNft200002 = IERC721(BETA_ADDRESS);
     ...
     */
    function provideFlavorInfos() external view override returns (FlavorInfoV3[] memory) {
        FlavorInfoV3[] memory initialFlavors = new FlavorInfoV3[](5);

        // General Banners.
        initialFlavors[0] = FlavorInfoV3(100001, 2000, 0, 0, address(0), .1 ether);
        initialFlavors[1] = FlavorInfoV3(100002, 2000, 0, 0, address(0), .1 ether);

        // Exclusive Banners for holders of Alpha
        initialFlavors[2] = FlavorInfoV3(200001, 1000, 0, 0, address(this), .2 ether);

        // Exclusive Banner for holders of Beta
        initialFlavors[3] = FlavorInfoV3(200002, 1000, 0, 0, address(this), .2 ether);

        // Exclusive Banner for holders of Alpha and Beta
        initialFlavors[4] = FlavorInfoV3(200003, 100, 0, 0, address(this), .5 ether);
    ...
    function validateMint(
        ...
        address holder,
        uint256 count,
        FlavorInfoV3 memory tokenFlavor,
        uint256 validationData
    ...
    function _approvedToMint(
        ...
        if (tokenValidatorsInitialized) {
            if (flavorId == 200001) {
                left = _getOperatorFromDelegation(caller, holder, ALPHA_ADDRESS, tokenId);
                right = exclusiveNft200001.ownerOf(tokenId);
                return (left == right, 'Operator not owner');
            }
            if (flavorId == 200002) {
                left = _getOperatorFromDelegation(caller, holder, BETA_ADDRESS, tokenId);
            ...
            if (flavorId == 200003) {
                ...
                left = _getOperatorFromDelegation(caller, holder, ALPHA_ADDRESS, tokenId);
                right = _getOperatorFromDelegation(caller, holder, BETA_ADDRESS, tokenId);
    ...
    function tokenURI(uint256, uint256) external pure returns (string memory) {
        // Not needed for standard ERC1155 implementation.
        return '';
    }
}