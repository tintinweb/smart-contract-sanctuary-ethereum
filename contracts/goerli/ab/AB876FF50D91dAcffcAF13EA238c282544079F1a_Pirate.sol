    ...
    UUPSUpgradeable
{
    ...
    function mint(uint256 amount, bool staked) external payable noContract {
    ...
    function mint(
        ...
        if (!MerkleProofUpgradeable.verify(proof, _merkleRoots[amount - 1], keccak256(abi.encodePacked(msg.sender)))) {
            revert InvalidProof();
        }

        if (_mintCounts[msg.sender] >= amount) {
            revert MintLimitExceeded();
        }
