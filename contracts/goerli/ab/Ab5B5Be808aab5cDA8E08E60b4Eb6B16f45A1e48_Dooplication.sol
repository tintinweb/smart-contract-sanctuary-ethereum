...
/*
  ...
  - fully remove internal approval mechanism
  - make dooplicator and token contracts storage consistent
  - updateable proxy?
  - fix canDooplicate view function, or remove it
...
contract Dooplication is Ownable, AccessControl {
     ...
     */
    function dooplicate(uint256 dooplicatorId,
                        uint256 tokenId,
        ...
        contractIsApproved(tokenContract)
    {
        ...
        // check token delegation and ownership
        
        if (tokenVault != address(0)) { 
            bool isDelegateValid = IDelegationRegistry(DELEGATION_REGISTRY).checkDelegateForToken(msg.sender, tokenVault, tokenContract, tokenId);
            require(isDelegateValid, "Invalid delegate-vault pairing for token.");
            requester = tokenVault;
        }

        if (IERC721(tokenContract).ownerOf(tokenId) != requester) {
            revert NotOwnerOfToken();
        }

        // check dooplicator delegation and ownership

        requester = msg.sender;

        if (dooplicatorVault != address(0)) { 
            bool isDelegateValid = IDelegationRegistry(DELEGATION_REGISTRY).checkDelegateForToken(msg.sender, dooplicatorVault, address(dooplicatorContract), dooplicatorId);
            require(isDelegateValid, "Invalid delegate-vault pairing for dooplicator.");
            requester = dooplicatorVault;
        }

        if (dooplicatorContract.ownerOf(dooplicatorId) != requester) {
     ...
     *  specific dooplicator
     ...
     */
    function canDooplicate(address operator,
                           uint256 dooplicatorId,
                           uint256 tokenId,
                           address tokenContract,
                           address tokenVault,
                           address dooplicatorVault) external view returns (bool) {
        // FIXME: run the checks
        return true;
    }
