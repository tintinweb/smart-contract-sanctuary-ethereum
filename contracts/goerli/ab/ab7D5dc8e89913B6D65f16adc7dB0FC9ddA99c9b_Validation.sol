...
contract Validation is Ownable {
    ...
    function setAllowance(
        ...
        if (_nftHolder == address(0) || _nftContractAddress == address(0)) {
            revert InvalidAddress();
        }

        LuckyBears nft = LuckyBears(_nftContractAddress);
        address NFTHolder = nft.ownerOf(_tokenId);
        if (NFTHolder != _nftHolder) {
            revert NotTheOwnerOfNFT();
        }
        (,uint identifier) = nft.getNFTIdentity(_tokenId);
        if (identifier == 0) {
            validationRecord[_nftContractAddress][_tokenId] = ValidationData(
                _nftHolder,
                nft.personalNFTLimit(),
                _permission
            );
    ...
    function validateBuyerAddress(address _nftContractAddress, uint256 _tokenId)
        public
        view
        returns (bool)
    {
    ...
    function validateNFTUsage(address _nftContractAddress, uint256 _tokenId)
        public
        view
        returns (bool)
    {
        return validationRecord[_nftContractAddress][_tokenId].used;
    }
    ...
    function setNftUsed(
        ...
        if (!validateBuyerAddress(_nftContractAddress, _tokenId)) {
            revert NotTheOwnerOfNFT();
        }

        if (validateNFTUsage(_nftContractAddress, _tokenId)) {
            revert NFTHasBeenUsed();
        }
        (, uint256 identifier ) = nft.getNFTIdentity(_tokenId);
        if (identifier == 0 && _productQuantity > nft.personalNFTLimit()) {
            revert ExceedingPurchaseLimitOfPersonalNFT();
        }

        if (identifier == 1 && _productQuantity > nft.companyNFTLimit()) {
            revert ExceedingPurchaseLimitOfCompanyNFT();
        }

        // add another internal function to check allowance of products
        validationRecord[_nftContractAddress][_tokenId].user = msg.sender;
        validationRecord[_nftContractAddress][_tokenId]
            .purchaseLimit = _productQuantity;
        validationRecord[_nftContractAddress][_tokenId].used = true;
