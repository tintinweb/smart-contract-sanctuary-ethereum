...
contract xGorilla is ERC20, ERC20Burnable, ReentrancyGuard {
    ...
    function isTimeUnlock(address _account)public view returns(bool){
        require(stakers[_account].deposited>0,"Do Not Deposit");
        if(stakers[_account].timeOfFristUpdate + 30 days  < block.timestamp   ){
            return true;
        }
        return false;
    }
    function getFirstTime(address _account)public view returns(uint256){
       return stakers[_account].timeOfFristUpdate;
    }
