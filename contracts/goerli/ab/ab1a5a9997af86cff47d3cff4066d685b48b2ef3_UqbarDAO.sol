...
contract UqbarDAO is Ownable {
  ...
  */
  ...
  mapping(uint256 => mapping(address => bool)) public votes;  // map atBlock to votes

  InflationProposal public proposal;

  uint256 public minQuorumPercent = 15000000000;
  ...
  event Inflation(uint quorumVotesNeeded, uint256 proposedAtBlock, uint256 newSupply);
  ...
  function setQuorum(uint256 _percent) external onlyOwner {
    require(!proposal.open, "Cannot change votingPeriod while a proposal is open");
    require(_percent >= minQuorumPercent, "Quorum must be at least 15%");
    require(_percent <= quorumDenominator, "Quorum cannot be > 100%");
  ...
  function executeProposal() external onlyOwner {
    ...
    emit Inflation(quorum, proposal.atBlock, totalSupply);
  }
}
