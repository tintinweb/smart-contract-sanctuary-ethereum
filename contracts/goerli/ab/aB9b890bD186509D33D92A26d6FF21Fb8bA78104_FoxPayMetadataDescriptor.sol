...
contract FoxPayMetadataDescriptor is IERC3525MetadataDescriptor {
    ...
    function constructTokenURI(uint256 tokenId_) external view override returns (string memory) {
        ...
        string memory image = _generateSVG(tokenId_, balance, foxPay.valueDecimals(), startDate, expireDate);
        return
            string(
                abi.encodePacked(
                    "data:application/json;base64,",
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    "</defs>",
                    _generateSVGGroups(),
                    _generateSVGTexts(tokenId_, value_, decimals_, startDate_, expireDate_),
                    "</svg>"
                    /* solhint-enable */
                )
            );
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    value_.uint2decimal(decimals_).addThousandsSeparator(),
                    "</text>",
