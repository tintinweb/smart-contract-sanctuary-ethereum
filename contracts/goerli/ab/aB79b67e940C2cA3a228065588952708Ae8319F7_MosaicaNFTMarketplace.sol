...
interface IMosaicaNFTFactory {
    ...
    function isMosaicaNFT(address _nft) external view returns (bool);
}

interface IMosaicaNFT {
...
contract MosaicaNFTMarketplace is Ownable, ReentrancyGuard {
    IMosaicaNFTFactory private immutable mosaicaNFTFactory;
    ...
    constructor(
        ...
        IMosaicaNFTFactory _mosaicaNFTFactory
    ) {
        require(_platformFee <= 10000, "can't more than 10 percent");
        platformFee = _platformFee;
        feeRecipient = _feeRecipient;
        mosaicaNFTFactory = _mosaicaNFTFactory;
    }

    modifier isMosaicaNFT(address _nft) {
        require(mosaicaNFTFactory.isMosaicaNFT(_nft), "not a Mosaica NFT");
        _;
    }
    ...
    function listNft(
        ...
        address _payToken,
        uint256 _price
    ) external isMosaicaNFT(_nft) isPayableToken(_payToken) {
        ...
        external isListedNFT(_nft, _tokenId) 
    {
        ...
        IMosaicaNFT nft = IMosaicaNFT(listedNft.nft);
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        IMosaicaNFT nft = IMosaicaNFT(offer.nft);
        address royaltyRecipient = nft.getRoyaltyRecipient();
        uint256 royaltyFee = nft.getRoyaltyFee();
    ...
    function resultAuction(address _nft, uint256 _tokenId) external {
        ...
        IMosaicaNFT mosaicaNft = IMosaicaNFT(_nft);
        address royaltyRecipient = mosaicaNft.getRoyaltyRecipient();
        uint256 royaltyFee = mosaicaNft.getRoyaltyFee();
