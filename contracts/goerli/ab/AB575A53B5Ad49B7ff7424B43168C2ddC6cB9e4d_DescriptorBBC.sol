...
contract DescriptorBBC is IDescriptor, Ownable {
    ...
    constructor(address _addr, string memory _baseURL){
        setDescriptedContract(_addr);
        baseURL = _baseURL;
        name = "BabyBunta 2nd collection";
        description = "";
    }
    ...
    function setBaseExtension(string memory _newExt) external onlyOwner {
        baseExtension = _newExt;
    }

    function tokenURI(uint256 tokenId) external view override returns(string memory){
        if (msg.sender != descriptedContract) revert InvalidCaller(msg.sender);
        string memory strId = (tokenId + 1).toString();
