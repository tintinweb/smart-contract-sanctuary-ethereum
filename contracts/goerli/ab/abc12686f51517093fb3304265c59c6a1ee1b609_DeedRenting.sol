...
/**
 * @title Deed Tenant Renting Contract
 */
contract DeedRenting is UUPSUpgradeable, Initializable, ProvisioningManager, Context, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint public MONTH_IN_DAYS;

    struct DeedLease {
        uint256 id; // Unique identifier for a Lease
        uint256 deedId;
    ...
    struct DeedOffer {
        ...
        uint256 overAllPrice; // Overall Rental Period Price
        ...
        address authorizedTenant; // Tenant Address who acquired the offer
    }
    ...
    modifier onlyDeedOwner(uint256 _deedId) {
        require(deed.balanceOf(_msgSender(), _deedId) > 0, "DeedRenting#onlyDeedOwner - Not Owner Of NFT");
        _;
    }
     ...
     * @dev Throws if called by any account other than the owner of the NFT.
     */
    modifier isDeedOwner(uint256 _leaseId, address _deedOwner) {
        require(deed.balanceOf(_deedOwner, deedOffers[_leaseId].deedId) > 0, "DeedRenting#isDeedOwner - Receiver isn't the Owner Of NFT");
        _;
    }
     ...
     * @dev Throws if called by any account other than the owner of the NFT.
     */
    modifier notDeedOwner(uint256 _deedId) {
        require(deed.balanceOf(_msgSender(), _deedId) == 0, "DeedRenting#notDeedOwner - NFT Owner can't acquire his own offer");
        _;
    }
     ...
     */
    modifier onlyDeedOwnerByLeaseId(uint256 _leaseId) {
        require(deed.balanceOf(_msgSender(), deedOffers[_leaseId].deedId) > 0, "DeedRenting#onlyDeedOwnerByLeaseId - Not Owner Of NFT");
        _;
    }

    /**
     * @dev Throws when leaseId offer doesn't have same deed Id
     */
    modifier isDeedOffer(uint256 _leaseId, uint256 _deedId) {
        require(_deedId == deedOffers[_leaseId].deedId, "DeedRenting#onlyDeedOwnerByLeaseId - Not Owner Of NFT");
        _;
    }
     ...
     */
    modifier isOfferCreatorDeedOwner(uint256 _leaseId) {
        require(deed.balanceOf(deedOffers[_leaseId].creator, deedOffers[_leaseId].deedId) > 0, "DeedRenting#isOfferCreatorDeedOwner - Offer creator isn't owner of NFT anymore");
        _;
    }
     ...
     * identified using Lease ID.
     */
    modifier onlyOfferCreator(uint256 _leaseId) {
        require(deedOffers[_leaseId].creator == _msgSender(), "DeedRenting#onlyOfferCreator - Not Owner Of Offer");
        _;
    }
     ...
     */
    modifier notAcquiredOffer(uint256 _leaseId) {
        require(deedLeases[_leaseId].leaseStartDate == 0, "DeedRenting#notAcquiredOffer - Offer has been already acquired by a Tenant");
        _;
    }
    ...
    modifier hasOfferValidStartDate(uint256 _deedId, uint256 _offerStartDate) {
        for (uint i = 0; i < leases[_deedId].length; i++) {
            require(_offerStartDate >= deedLeases[leases[_deedId][i]].leaseEndDate, "DeedRenting#hasOfferValidStartDate - Offer Start Date must be greater than acquired leases end dates");
        }
        _;
    }
    ...
    modifier isValidOfferPeiod(uint16 _months, uint8 _noticePeriod) {
        require(_months > 0, "DeedRenting#updateOffer - Rental Duration Must be positive");
        require(_noticePeriod > _months, "DeedRenting#updateOffer - Notice Period Must be less than Rental Duration");
        _;
    }
     ...
     */
    modifier isAuthorizedTenant(uint256 _leaseId) {
        DeedOffer storage offer = deedOffers[_leaseId];
        if (offer.authorizedTenant != address(0)) {
          require(offer.authorizedTenant == _msgSender(), "DeedRenting#isAuthorizedTenant - Offer is not authorized to be acquired using this address");
        }
        _;
    }
     ...
     */
    modifier isOfferNotExpired(uint256 _leaseId) {
        DeedOffer storage offer = deedOffers[_leaseId];
        if (offer.offerExpirationDate > 0) {
          require(offer.offerExpirationDate < block.timestamp, "DeedRenting#isOfferNotExpired - Offer has expired");
        }
        for (uint i = 0; i < leases[offer.deedId].length; i++) {
          uint256 leaseId = leases[offer.deedId][i];
          require(deedOffers[leaseId].offerStartDate >= deedLeases[leaseId].leaseEndDate, "DeedRenting#isOfferNotExpired - Offer Start Date must be greater than acquired leases end dates");
        }
        _;
    }
    ...
    modifier onlyDeedTenant(uint256 _leaseId) {
        ...
        require(lease.tenant == _msgSender(), "DeedRenting#isDeedTenant - Not Deed Tenant");
        _;
    }
     ...
     * @dev Throws if Deed tenant isn't the current address
     */
    modifier isRentNonPayment(uint256 _leaseId) {
        DeedLease storage lease = deedLeases[_leaseId];
        require(lease.paidRentsDate < block.timestamp, "DeedRenting#isRentNotPaid - Tenant had Paid his due Rents");
        _;
    }
    ...
    function initialize(
        ...
        uint256 monthInSeconds
    )
        virtual
        public
        initializer {

        MONTH_IN_DAYS = monthInSeconds;
        meed = _meed;
        deed = _deed;
    ...
    /**
     * @dev This method allows to an offer by a Tenant
     */
    function acquireRent(uint256 _id, uint8 _monthsToPay)
        ...
        require(overallMonthsToPay <= offer.months, "DeedRenting#payRent - Shouldn't pay more than expected!");

        uint256 amount;
        if (offer.overAllPrice > 0 && overallMonthsToPay == offer.months) {
          amount = offer.overAllPrice;
        } else {
          amount = offer.price.mul(overallMonthsToPay);
        }
        require(meed.transferFrom(_msgSender(), offer.creator, amount), "DeedRenting#acquireOffer - Deed Renting Payment failed");
        ...
        DeedLease memory lease = DeedLease({
            ...
            leaseEndDate: leaseStartDate + MONTH_IN_DAYS * offer.months,
            paidRentsDate: leaseStartDate + MONTH_IN_DAYS * _monthsToPay,
            noticePeriodDate: leaseStartDate + MONTH_IN_DAYS * overallMonthsToPay,
    ...
    /**
     * @dev This method allows to pay a Rent by Deed Tenant
     */
    function payRent(uint256 _id, address _deedOwner, uint8 _monthsToPay)
        public
        isDeedOwner(_id, _deedOwner)
        onlyDeedTenant(_id)
        isOngoingLease(_id) {
        ...
        require((offer.months - lease.paidMonths) <= _monthsToPay, "DeedRenting#payRent - Shouldn't pay more than expected!");

        uint256 amount = offer.price.mul(_monthsToPay);
        require(meed.transferFrom(_msgSender(), _deedOwner, amount), "DeedRenting#payRent - Renting Payment failed");

        lease.paidMonths += _monthsToPay;
        lease.paidRentsDate += MONTH_IN_DAYS * _monthsToPay;
        lease.noticePeriodDate += MONTH_IN_DAYS * _monthsToPay;

        emit RentPaid(_id, lease.deedId, _msgSender(), _deedOwner, _monthsToPay, false);
    }

    /**
     * @dev This method allows to end Lease by Deed Tenant before the Date
     */
    ...
    /**
     * @dev This method allows to evict a Deed Tenant who hasn't paid rents
     * at time even after considering already paid Notice Period
     */
    function evictTenant(uint256 _id)
        public
        onlyDeedOwnerByLeaseId(_id)
        isOngoingLease(_id)
        isRentNonPayment(_id) {
    ...
    /**
     * @dev returns true if the address can manage Deed Tenant provisioning (Start & Stop)
     */
    ...
    function _setTenantProvisioningDelegatee(uint256 _deedId) internal {
        if (tenantProvisioning.getDelegatee(_deedId) != address(this)) {
          require(tenantProvisioning.setDelegatee(address(this), _deedId), "DeedRenting#_setTenantProvisioningDelegatee - Can't set Current Contract as Tenant Provisioning Manager");
        }
    }
    ...
    function _setOffer(DeedOffer memory _offer) internal {
        ...
        if (_offer.offerStartDate == 0) {
            ...
            _offer.offerExpirationDate = _offer.offerStartDate + _offer.offerExpirationDays * 1 days;
        }
