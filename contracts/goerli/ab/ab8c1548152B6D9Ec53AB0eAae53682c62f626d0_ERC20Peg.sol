...
interface IBridge {
    ...
    event SendMessage(uint messageId, address source, address destination, bytes message, uint256 fee, address feeRecipient);
    ...
    function sendMessage(address destination, bytes calldata message, address payable feeRecipient) external payable;
}
...
contract ERC20Peg is Ownable, IBridgeReceiver, ReentrancyGuard, ERC165 {
    ...
    // Fee for deposit (~XRP required to make the deposit on ROOT network)
    uint public depositFee;
    // Address to forward deposit fees to
    ...
    event DepositFeeUpdated(uint indexed depositFee, address indexed depositFeeAddress);

    constructor(IBridge _bridge) {
        bridge = _bridge;
        depositFeeAddress = address(_bridge); // default fee recipient to bridge address
    }
    ...
    function deposit(address _tokenAddress, uint128 _amount, address _destination) payable external {
        ...
        if (_tokenAddress == ETH_RESERVED_TOKEN_ADDRESS) {
            require(msg.value == (_amount + depositFee), "ERC20Peg: incorrect deposit amount (requires deposit fee)");
        } else {
            require(msg.value == depositFee, "ERC20Peg: incorrect token address (requires deposit fee)");
        ...
        bridge.sendMessage{value: depositFee }(palletAddress, message, payable(depositFeeAddress));
    }
    ...
    /// @notice Update the deposit fee and fee recipient
    function setDepositFee(uint256 _depositFee, address _depositFeeAddress) external onlyOwner {
        depositFee = _depositFee;
        depositFeeAddress = _depositFeeAddress;
        emit DepositFeeUpdated(_depositFee, _depositFeeAddress);
    }
}