...
contract PairWithL2Overlay is UniswapV2Pair, EmergencyWithdrawal, StarkWareAssetData, Initializable {
  ...
  uint private constant defaultVaultId = 0;
  ...
  */
  ...
  function _swapStarkWare( 
    ...
    getStarkExRegistry(starkEx).registerLimitOrder(exchangeAddress, tokenAmmSell, tokenAmmBuy,
      tokenAmmSell, amountAmmSell, amountAmmBuy, 0, 0, 0, 0, nonce, unsignedInt22);
  ...
  function verifyNonceAndLocked(uint nonceToUse) private view {
    ...
    bool isNonceUsed = nonce >= nonceToUse;
  ...
  */
  ...
  function toggleLayer2(bool _isLayer2Live) external operatorOnly {
    ...
    if (_isLayer2Live) {
      ...
      depositStarkWare(starkEx, token0, tokenAQuantum, tokenAAssetId, defaultVaultId, tokenAQuantum.toQuantizedUnsafe(balance0));
      depositStarkWare(starkEx, token1, tokenBQuantum, tokenBAssetId, defaultVaultId, tokenBQuantum.toQuantizedUnsafe(balance1));
  ...
  function balances() internal view override returns (uint balance0, uint balance1) {
    if (isLayer2Live) {
      IStarkEx starkEx = getStarkEx();
      balance0 = starkEx.getVaultBalance(address(this), tokenAAssetId, defaultVaultId);
      balance1 = starkEx.getVaultBalance(address(this), tokenBAssetId, defaultVaultId);
    } else {
  ...
  */
    ...
    returns (uint token0Amount, uint token1Amount) 
  {
    ...
    if (isLayer2Live) {
     ...
     IStarkEx starkEx = getStarkEx();
     withdrawStarkWare(starkEx, token0, tokenAQuantum, tokenAAssetId, defaultVaultId, tokenAQuantum.toQuantizedUnsafe(token0Amount));
     withdrawStarkWare(starkEx, token1, tokenBQuantum, tokenBAssetId, defaultVaultId, tokenBQuantum.toQuantizedUnsafe(token1Amount));
  ...
  function authorizeWithdrawals(uint blockNumberTo, uint lpAmount) external override operatorOnly {
    _authorizeWithdrawals(blockNumberTo, lpAmount);
  }
...
abstract contract EmergencyWithdrawal is UniswapV2ERC20, PairStorage {
  using SafeERC20 for IERC20;
  uint public constant MAX_WITHDRAWAL_DELAY = 28 days;
  ...
  event WithdrawalRequested(address user, uint amount);
  ...
  function moveWithdrawalBlockPointer(uint blockNumber) internal {
    withdrawalBlock = blockNumber;
  }

  function verifyWithdrawAtCurrentBlock(uint withdrawalBlockNumber) internal view {
    require(block.number > withdrawalBlock, 'CURRENT_BLOCK_TOO_LOW');
    require(withdrawalBlockNumber <= withdrawalBlock, 'WITHDRAWAL_BLOCK_TOO_HIGH');
  }
  ...
  function requestWithdrawal() external returns (uint) {
    ...
    userWithdrawals[msg.sender] = Withdrawal(balance, block.number, block.timestamp + withdrawalDelay);
    totalRequested += balance;

    if (block.number > highestBlock) {
      highestBlock = block.number;
    }
  ...
  function withdrawUserFunds() external returns (uint lpAmount, uint token0Amount, uint token1Amount) {
    ...
    verifyWithdrawAtCurrentBlock(userWithdrawals[user].blockNumber);
  ...
  */
  function authorizeWithdrawals(uint blockNumberTo, uint lpAmount) external virtual;

  /**
   * @dev Move the block to an authorized point
  */
  function _authorizeWithdrawals(uint blockNumberTo, uint amount) internal {
    require(blockNumberTo > withdrawalBlock, 'BLOCK_NUMBER_TOO_LOW');
    ...
    IERC20(token0).safeTransfer(withdrawalWalletAddress, token0Amount);
    IERC20(token1).safeTransfer(withdrawalWalletAddress, token1Amount);
    ...
    moveWithdrawalBlockPointer(blockNumberTo);
  }
  ...
  function forceWithdrawalTimelimitReached(address user) external {
    ...
    _authorizeWithdrawals(highestBlock, totalRequested);
  }
}