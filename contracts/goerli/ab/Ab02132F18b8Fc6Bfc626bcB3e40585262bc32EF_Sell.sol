...
contract Sell is Permission, Ownable, ReentrancyGuard {
    ...
    function getWhitelist(uint8 _table, address add)public view returns(bool) {
        return fffAddress.whitelist(_table, add);
    }

    function getInvestor(address add)public view returns(uint256) {
        return fffAddress.investor(add);
    }
    ...
    function buyVIP(bytes memory _signature) external payable nonReentrant{
        ...
        uint256 _tokenId = fffAddress.tokenOfOwnerByIndex(msg.sender, 0);
    ...
    function buyfff(bytes memory _signature) external payable nonReentrant{
        require(!isContract(msg.sender), "It is contracts");
        require(verifySigner(_signature)==msg.sender,"The signer is error");
        require(msg.value >= ticketPrice,"Your eth is not enough!");
        require(sellAmount + 1 <= 999,"It is solt out");
