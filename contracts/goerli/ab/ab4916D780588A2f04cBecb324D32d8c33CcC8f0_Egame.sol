...
contract Egame is ReentrancyGuard {
  ...
  function claimScore(uint256 _score) public nonReentrant returns (bool)  {
    ...
    if ( !(validating(newPointer, getPointer(msg.sender)))) return false;
  ...
  function getScore() public view returns (uint256) {
    return mappingPlayer[msg.sender].score;
  }

  function getToken() public view returns (uint256) {
    return mappingPlayer[msg.sender].token;
  }
