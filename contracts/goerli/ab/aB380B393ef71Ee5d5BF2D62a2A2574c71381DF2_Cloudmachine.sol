 ...
 * generate an index into the previously-ordered metadata
 * (see getInitialMetadataSequenceIndex).
 * @author backuardo.eth
 */
contract Cloudmachine is ERC721A, Ownable, ReentrancyGuard {
    enum MintPhase {
        NONE,
        PAUSED,
        ALLOWLIST_SALE,
    ...
    uint256 public constant COLLECTION_SIZE = 4444;
    uint256 public constant MINT_PRICE_PUBLIC = .16 ether;
    uint256 public constant MINT_PRICE_ALLOWLIST = .12 ether;
     ...
     */
    constructor(string memory _provenanceHash)
        ERC721A("Cloudmachine", "CLOUD")
    {
     ...
     * @param _maxAllowlistAllocation Maximum number of tokens the message sender can mint
     * during the allowlist mint
     * @param _proof Merkle proof to verify msg.sender is part of the allowlist
     */
    function allowlistMint(
        uint256 _quantity,
        uint256 _maxAllowlistAllocation,
        ...
        inMintPhase(MintPhase.ALLOWLIST_SALE)
    {
        if (
            !MerkleProof.verify(
                _proof,
                merkleRoot,
                keccak256(abi.encodePacked(msg.sender, _maxAllowlistAllocation))
            )
        ) {
            revert InvalidMerkleProof();
        }
        if (msg.value != MINT_PRICE_ALLOWLIST * _quantity) {
            revert IncorrectPayment();
        }
        if (totalSupply() + _quantity > COLLECTION_SIZE) {
            revert InsufficientSupply();
        }
        if (getRedemptionsAllowlist() + _quantity > _maxAllowlistAllocation) {
        ...
        inMintPhase(MintPhase.PUBLIC_SALE)
    {
        ...
        if (getRedemptionsPublic() + _quantity > MAX_PER_WALLET_PUBLIC) {
