...
contract SahabaMarketplace is ERC721URIStorage {
    ...
    // Declaring an event to print the buy platform fees and owner returned
    event BuyEventFees(uint256 platformFees, uint256 ownerFees);
    ...
    function buyToken(uint256 tokenId) public payable {
        ...
        // calc fees
        uint256 platformFees = marketItem.price.mul(_service_fees);
        uint256 ownerFees = marketItem.price.sub(platformFees);

        emit BuyEventFees(platformFees, ownerFees);

        // send token's worth of ethers to the owner
        // marketItem.currentOwner.transfer(ownerFees);
