...
contract ERC1155Lockable is
    ...
    // mapping(uint256 => _PrivateFiles) private PrivateFile;
    ...
    function uri(uint _id) public override view returns (string memory) {
        return string(_tokenInfo[_id].uri);
    }
    ...
    **/
    ...
    function mint(address to, NftType N_type, uint256 amount, bytes calldata uri_, bytes calldata key, 
        ...
        // PrivateFile[id].files = _PrivateFiles;
    ...
    function SetPrivateFiles(uint256 id, string[] calldata _PrivateFiles) public returns (bool){
        require(balanceOf(address(msg.sender), id) > 0, "This NFT is not present in ur Account");
        require(_PrivateFiles.length > 0, "Please enter the valid length");

         if(_PrivateFiles.length>0){
            delete _tokenInfo[id].files;
            string[] storage _file = _tokenInfo[id].files;
            for (uint256 i=0; i < _PrivateFiles.length;){
                _file.push(_PrivateFiles[i]);
                unchecked {i++;}
            }
         }
