...
contract GetterFacet is IGetter {
    ...
    function getGovernanceParameters()
        ...
        returns (
            ...
            uint8 maxUpdateIdToFees,
            uint8 maxUpdateIdToSettlementPeriods
        )
    {
        LibDIVAStorage.GovernanceStorage storage gs = LibDIVAStorage
            ._governanceStorage();

        (currentFees, maxUpdateIdToFees) = _getCurrentFees(gs);
        (
            currentSettlementPeriods,
            maxUpdateIdToSettlementPeriods
        ) = _getCurrentSettlementPeriods(gs);
        treasury = gs.treasury;
        fallbackDataProvider = gs.fallbackDataProvider;
    ...
    function getFees(uint256 _poolId)
        external
        view
        override
    ...
    function getCurrentSettlementPeriods()
        external
        view
        override
        returns (
    ...
    function getSettlementPeriods(uint256 _poolId)
        external
        view
        override
        returns (LibDIVAStorage.SettlementPeriods memory)
    {
        LibDIVAStorage.SettlementPeriods memory _settlementPeriods = LibDIVA
            ._getSettlementPeriods(_poolId);
    ...
    function getClaim(address _collateralToken, address _recipient)
        ...
        returns (uint256)
    {
        return LibDIVA._claim(_collateralToken, _recipient);
    }
    ...
    {
        maxUpdateIdToFees = _gs.maxUpdateIdToFees;
        currentFees = _gs.updateIdToFees[maxUpdateIdToFees];
    }
    ...
    {
        maxUpdateIdToSettlementPeriods = _gs.maxUpdateIdToSettlementPeriods;
        currentSettlementPeriods = _gs.updateIdToSettlementPeriods[
            maxUpdateIdToSettlementPeriods
        ];
    }
}
...
interface IGetter {
    ...
    /**
     * @dev Same as `getPoolParameters` but using the position token
     * address as input.
    ...
    /**
     * @dev Returns the most recent governance parameters.
     ...
     * @return maxUpdateIdToFees Latest update id for fees.
     * @return maxUpdateIdToSettlementPeriods Latest update id for settlement periods.
     */
    function getGovernanceParameters()
        external
        view
    ...
    /**
     * @dev Returns the current protocol and settlement fees.
     * @return currentFees The current applicable fees.
     * @return maxUpdateIdToFees Latest update id for fees.
     */
    function getCurrentFees()
    ...
    /**
     * @dev Returns the protocol and settlement fees applicable to
     * a given `poolId`.
     * @return Fees struct.
     */
    function getFees(uint256 _poolId)
    ...
    /**
     * @dev Returns the current settlement related periods.
     * @return currentSettlementPeriods The current applicable settlement periods.
     * @return maxUpdateIdToSettlementPeriods Latest update id for settlement periods.
     */
    function getCurrentSettlementPeriods()
    ...
    /**
     * @dev Returns the settlement related periods applicable to
     * a given `poolId`.
    ...
    /**
     * @dev Returns the poolId for a given `_typedOfferHash` derived from a
    ...
    /**
     * @dev Returns the filled amount for a given `_typedOfferHash` (EIP712 specific).
     * @param _typedOfferHash Typed offer hash.
