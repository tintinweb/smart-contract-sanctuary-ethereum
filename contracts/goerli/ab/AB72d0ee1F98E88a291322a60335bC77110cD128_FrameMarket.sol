...
interface IFrameNFT {
	...
	function addItem(uint256 tokenId,address sender,string memory _uri,uint256 supply,uint256 royalty,address marketAddress,uint256 expireTime,bytes memory _sig) external;
...
contract FrameMarket is Ownable, ERC1155Holder {
    ...
    function onBuyOrder(
		...
		if (isMulti){
			...
			require(nft.ownerOf(_transaction.tokenId) == msg.sender, "You are not admin.");
		}
    ...
    function onAcceptOffer(
		...
		if (isMulti){
			uint256 nft_token_balance = nft.balanceOf(msg.sender, _transaction.tokenId);
			require(nft_token_balance >= _transaction._amount, "Your owned amount is not enough");
		}else{
			require(nft.ownerOf(_transaction.tokenId) == msg.sender, "You are not owner" );
		}
        ...
        IERC20 governanceToken = IERC20(_transaction.tokenAdr);
        require(governanceToken.balanceOf(_transaction.owner) >= offerAmount, "The buyer's balance is insufficient.");
        // transfer governance token to feeAddress
        if (feeAmount > 0)require(governanceToken.transferFrom(_transaction.owner, address(this), feeAmount));			
        // transfer governance token to creator
        if (createrAmount > 0)require(governanceToken.transferFrom(_transaction.owner, _transaction.itemCreator, createrAmount));			
        // transfer governance token to owner
        require(governanceToken.transferFrom(_transaction.owner, msg.sender, ownerAmount));

		if (isMulti){
			nft.safeTransferFrom(msg.sender, _transaction.owner, _transaction.tokenId, _transaction._amount, "buy from Marketplace");
		}else{
			nft.safeTransferFrom(msg.sender, _transaction.owner, _transaction.tokenId, "buy from Marketplace");
		}
