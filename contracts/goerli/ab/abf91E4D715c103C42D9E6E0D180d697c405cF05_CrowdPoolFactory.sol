...
library SharedStructs {
    struct CrowdPoolInfo {
        ...
        address sale_token; // the token contract address
        uint256 token_rate; // 1 ETH = token_rate of sale token
        uint256 raise_min; // min ETH pool contribution per wallet
        uint256 raise_max; // max ETH pool contribution per wallet
