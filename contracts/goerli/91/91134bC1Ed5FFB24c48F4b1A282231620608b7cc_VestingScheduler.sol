...
interface IVestingScheduler {
     ...
     * @param cliffAndFlowDate Date that will be used to start vesting.
     * @param endDate The superToken to be streamed.
     * @param flowRate For the stream (or 0 if starting not required)
     * @param cliffAmount Amount you would like to transfer when you start streaming.
     */
     ...
     * @param flowRate The flowRate for the stream.
     * @param cliffAmount The amount you would like to transfer at the cliffDate when you start streaming
    ...
    /**
     * @dev Vesting order event executed by bot.
     * @notice Updating a running vesting, the interval can be < 7 days
     * @param superToken SuperToken to be streamed
     * @param sender Account who will be send the stream
     * @param receiver Account who will be receiving the stream
    ...
    /**
     * @dev Update a vesting schedule that is already running
     * @param superToken SuperToken to be streamed
     * @param receiver Accounts who will be receiving the stream.
     * @param endDate The timestamp when the stream should stop (or 0 if closing not required)
     * @param ctx Superfluid context used when batching operations. (or bytes(0) if not SF batching)
     */
     ...
     * @param sender The account who will be send the stream
     * @param receiver The account who will be receiving the stream
     * @param cliffAndFlowDate The timestamp when the stream should start (or 0 if starting not required)
...
contract VestingScheduler is IVestingScheduler, SuperAppBase {
    ...
    mapping(bytes32 => VestingSchedule) public vestingSchedules; // id = keccak(sender, receiver, superToken)
    ...
    function updateVestingSchedule(
        ...
        // Only update if vesting is already flowing.
    ...
    function executeEndVesting(
        ...
        if (earlyEndCompensation > 0) {
            // try-catch this because if the account does not have tokens for earlyEndCompensation we should close the stream anyway.
