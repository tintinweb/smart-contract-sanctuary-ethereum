...
contract TradingInsults is ERC721A, IERC2981, Ownable {
    ...
    struct TimerConfig {
        ...
        uint64 boostReductionBps;
        uint64 boosterReductionBps;
        uint64 boostestReductionBps;
    }

    struct Shares {
        uint64 kingShares;
        uint64 lastMinterShares;
        uint64 deployerShares;
    }
    ...
    function setSaleDuration(uint64 saleDuration) external onlyOwner {
    ...
    function setSaleState(bool isSaleStateOpen) external onlyOwner {
        ...
        if (_config.saleExpiry == 0)
            _config.saleExpiry = uint64(block.timestamp) + _config.saleDuration;
    ...
    function mint(
        ...
        if (SaleType.standardInsult == saleType) {
            ...
            _mintHelper(insultee, 1, 1);
            _config.saleExpiry -=
                ((_config.saleExpiry - uint64(block.timestamp)) *
    ...
    function setRoyaltyBps(uint64 royaltyBps) external onlyOwner {
        _config.royaltyBps = royaltyBps;
    }
