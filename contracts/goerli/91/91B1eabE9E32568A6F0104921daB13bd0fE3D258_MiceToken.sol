...
import "./lib/FxERC20Root.sol";

contract MiceToken is FxERC20Root, AccessControl {
    ...
    function burnFrom(address user, uint256 amount) external payable {
        if (!hasRole(AUTHORITY, msg.sender)) {
    ...
    function earn(address user, uint256 amount) external payable onlyRole(AUTHORITY) {
...
abstract contract FxERC20Root is FxBaseRootTunnel, ERC20 {
