    ...
    CommonChecks
{
    ...
    function castVote(
        ...
        return _castVote(_proposalId, _msgSender(), _vote, _reason);
    }
    ...
    function state(uint _proposalId) public view returns (ProposalState) {
        ...
        if (proposal.rejected) {
            ...
            quorumReached(proposal, autonomousQuorumNumerator) &&
            proposal.forVotes > proposal.againstVotes
        ) {
            return ProposalState.Succeeded;
        } else if (
            quorumReached(proposal, quorumNumerator) &&
            proposal.forVotes > proposal.againstVotes
        ) {
            return ProposalState.Succeeded;
        } else {
     ...
     */
    function quorumReached(Proposal memory _proposal, uint _numerator)
        public
        view
        returns (bool)
    {
        uint _numVotes = _proposal.forVotes + _proposal.againstVotes;
        uint _effectiveNumerator = token.getPastTotalSupply(
            _proposal.voteStart
        ) * _numerator;
        return _effectiveNumerator / denominator <= _numVotes;
    }
    ...
    function _castVote(
        ...
        _countVote(_proposalId, _account, _vote, weight.toUint128());
    ...
    function _countVote(
        ...
        if (_vote == uint8(VoteType.Against)) {
            ...
            revert("Governor: INVALID_VOTE_TYPE");
        }
    }
