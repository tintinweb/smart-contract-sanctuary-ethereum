...
contract TestInu is ERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        // check for blacklisted account before transfer
        require(blacklistedAccounts[to], "You have been blacklisted from transferring tokens");
        ...
        if (limitsInEffect) {
            if (from != owner() && to != owner() && to != address(0) && to != address(0xdead)) {
                ...
                if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                    require(amount <= maxBuyAmt, "Buy transfer amount exceeds the max buy.");
                    require(amount + balanceOf(to) <= maxWalletAmt, "Cannot Exceed max wallet");
                } else if (automatedMarketMakerPairs[to] && !_isExcludedMaxTransactionAmount[from]) {
                    require(amount <= maxSellAmt, "Sell transfer amount exceeds the max sell.");
