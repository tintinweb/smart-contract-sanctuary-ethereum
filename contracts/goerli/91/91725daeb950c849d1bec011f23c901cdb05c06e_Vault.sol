...
contract CollateralModule is Owner {
...
contract VaultBase is CollateralModule {
    ...
    function _required(uint cdp, address user) internal view {
        ...
        require(CDPInfo[cdp].closed == false, "Vault is closed");
    }
        ...
        returns (uint)
    {
        ...
        amount = ( (amount * ddamount) / (10 ** IERC20Metadata(DD).decimals()) );
...
// File: contracts/lib/security/Initializable.sol
...
abstract contract Initializable {
    ...
    function _init() internal {
        require(!_initialized, "1Contract already initialized");
        _initialized = true;
    }
}
...
contract Vault is VaultBase, Initializable {
    using SafeERC20 for IERC20;

    function initialize(address DD_, address owner_) external {
        _init();
        DD = DD_;
        _owner = owner_;
    }
    ...
    function create(
        ...
        CDPInfo[id] = CDP(user, token, amount, ddAmount, false);
