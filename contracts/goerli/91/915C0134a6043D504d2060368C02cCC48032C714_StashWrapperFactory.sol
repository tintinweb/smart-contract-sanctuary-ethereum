 ...
 */
contract StashWrapperFactory is ITreasury, IStashWrapperFactory, OwnableUpgradeable {
  ...
  address private implementationStashWrapped721;

  uint32 private versionStashWrapped721;

  address private implementationStashWrapped1155;

  uint32 private versionStashWrapped1155;
  ...
  function adminUpdateImplementation(address _implementation) external onlyOwner {
    ...
    if (nftType == NFTType.ERC721) {
      require(_implementation != implementationStashWrapped721, "Implementation is already set");
      ...
      require(_implementation != implementationStashWrapped1155, "Implementation is already set");

      implementationStashWrapped1155 = _implementation;
      // Version will not realistically overflow 32 bits.
      ++versionStashWrapped1155;
      emit ImplementationUpdated(_implementation, nftType, versionStashWrapped1155);
    }
  }
  ...
  function getImplementation(NFTType nftType) external view returns (address implementation) {
    if (nftType == NFTType.ERC721) {
      implementation = implementationStashWrapped721;
    } else {
      implementation = implementationStashWrapped1155;
    }
  }

  function getVersion(NFTType nftType) external view returns (uint256 version) {
    if (nftType == NFTType.ERC721) {
      version = versionStashWrapped721;
    } else {
      version = versionStashWrapped1155;
    }
  }
...
contract $StashWrapperFactory is StashWrapperFactory {
    ...
    constructor(address payable _treasury) StashWrapperFactory(_treasury) {}
