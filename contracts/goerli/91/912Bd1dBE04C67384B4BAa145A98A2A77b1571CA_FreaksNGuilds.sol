...
contract FreaksNGuilds is Controllable, Pausable, Ownable, ERC721A("Freaks N' Guilds", "FnG") {
  ...
  function mintWithETHHoldersOnly(uint256[] memory ckeyIds) external payable noCheaters whenNotPaused {
    ...
    uint256 rand = _rand();
    for (uint256 i = 0; i < amount; i++) {
      require(msg.sender == ckey.ownerOf(ckeyIds[i]) || _keyDeposited(ckeyIds[i]), "invalid token ID");
  ...
  function _keyDeposited(uint256 ckeyId) internal view returns(bool) {
    uint256[] memory depositedKeys = vault.depositsOf(msg.sender);
    for(uint256 i = 0; i < depositedKeys.length; i++){
      if(depositedKeys[i] == ckeyId){
        return true;
      }
    }
    return false;
