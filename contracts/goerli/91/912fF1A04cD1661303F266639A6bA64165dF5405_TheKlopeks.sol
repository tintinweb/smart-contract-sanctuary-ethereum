...
contract TheKlopeks is ERC721Airdroppable {
    /**
     ...
     * @param holder The wallet holding the collection prior to transfer.
     */
    constructor(
        string memory name,
        ...
        _mintERC2309(holder, _maxSupply);
    }

    /**
     * @notice Mint the entire collection to the owner in prep for auction
     */
    function redeem(address to, uint256 tokenId) public {
        safeTransferFrom(msg.sender, to, tokenId);
    }

    function bulkRedeem(TokenReceiver[] memory receivers) public {
        for (uint256 i = 0; i < receivers.length; i++) {
            safeTransferFrom(msg.sender, receivers[i].to, receivers[i].tokenId);
        }
