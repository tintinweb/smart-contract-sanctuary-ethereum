...
contract MarketMap {
    ...
    mapping(address => mapping(uint256 => OfferItem)) public offers;
    mapping(address => mapping(uint256 => ConsiderationItem)) public considerations;
    ...
    function sell(
        ...
        offers[token][tokenId] = order.parameters.offer[0];
        considerations[token][tokenId] = order.parameters.consideration[0];
    ...
    function buy(
        ...
        Order memory order = composeOrder(token, tokenId);
        bool fulfilled = seaport.fulfillOrder{value: msg.value}(order, conduitKey);
        if (fulfilled) {
    ...
    function composeOrder(
        ...
        offer[0] = offers[token][tokenId];
        ConsiderationItem[] memory consideration = new ConsiderationItem[](1);
        consideration[0] = considerations[token][tokenId];
