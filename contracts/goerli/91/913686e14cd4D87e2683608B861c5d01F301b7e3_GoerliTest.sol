...
contract GoerliTest is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory initURI, address signer) ERC721A("GoerliTest", "GT", 3) {
        _uri = initURI;
        _signer = signer;
        maxTotalSupply = 5555;
        PreMaxMint = 2;
    ...
    function mint(uint256 num, string memory salt) external payable {
        require(status == Status.PublicSale, "WGGE006");
        require(keccak256(abi.encodePacked(salt)) == keccak256(abi.encodePacked(publicSalt)), "WGGE011");
        require(numberMinted(msg.sender) + num <= PublicMaxMint, "WGGE008");
        verified(num);
    ...
    function presaleMint(
        ...
        require(_verify(_hash(salt, msg.sender), token), "WGGE001");
    ...
    function setPublicSalt(string memory salt) external onlyOwner {
    ...
    function release() public virtual nonReentrant onlyOwner{
        require(totalSupply() >= maxTotalSupply, 'WGGE004');
        uint256 num = address(this).balance / 10 * 9;
        uint256 partnerNum = address(this).balance - num;
        require(num != 0, "WGGE005");

        Address.sendValue(payable(owner()), num);
        emit PaymentReleased(owner(), num);

        Address.sendValue(payable(0x25af24C4B2c0695b1694f1248143b6D7a7C63B9C), partnerNum);
        emit PaymentReleased(0x25af24C4B2c0695b1694f1248143b6D7a7C63B9C, partnerNum);
    }
}
