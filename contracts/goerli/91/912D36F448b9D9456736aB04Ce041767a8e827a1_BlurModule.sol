...
contract BlurModule is BaseExchangeModule {
  ...
  IBlur public constant EXCHANGE = IBlur(0x000000000000Ad05Ccc4F10045630fb830B95127);

  address public constant EXECUTION_DELEGATE = 0x00000000000111AbE46ff893f3B2fdF1F759a8A8;
  ...
  {
    // Execute fill
    _buy(sell, buy, params.fillTo, params.revertIfIncomplete, params.amount);
  }
    ...
    chargeETHFees(fees, params.amount)
  {
    ...
    for (uint256 i = 0; i < length; ) {
      ...
      // Execute fill
      _buy(sells[i], buys[i], params.fillTo, params.revertIfIncomplete, sell.order.price);
  ...
  function _buy(
    ...
    try EXCHANGE.execute{value: value}(sell, buy) {
      ...
      if (isERC721) {
        IERC721(address(collection)).safeTransferFrom(address(this), receiver, sell.order.tokenId);
      } else {
