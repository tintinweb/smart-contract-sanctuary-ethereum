...
contract Sw3 is Ownable, ERC721A, ReentrancyGuard, Pausable {
    ...
    function whitelistMint(uint256 quantity, bytes32[] memory proof) external payable callerIsUser whenNotPaused {
        ...
        require(price != 0 && whiltelistMintStarted, "whitelist sale has not begun yet");        
        require(isValid(proof, keccak256(abi.encodePacked(msg.sender))), "not eligible for whitelist mint");
    ...
    function setWhitelistPrice(uint32 price_) external onlyOwner {
        whitelistPrice = price_;
    }

    function setPublicPrice(uint32 price_) external onlyOwner {
        publicPrice = price_;
    }
