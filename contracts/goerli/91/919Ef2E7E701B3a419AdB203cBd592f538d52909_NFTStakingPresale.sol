    ...
    UUPSUpgradeable
{
    ...
    function batchCreateNFTs(
        ...
        for (uint256 i; i < quantity; i++) {
            _createNFT(userAddress, _nftType);
        }
    ...
    function _createNFT(address userAddress, uint256 _nftType) internal {
        if (_nftType == 0) {
            _rigel.safeMint(userAddress, "");
        }

        if (_nftType == 1) {
            _sirius.safeMint(userAddress, "");
        }

        if (_nftType == 2) {
            _vega.safeMint(userAddress, "");
        }
    }
