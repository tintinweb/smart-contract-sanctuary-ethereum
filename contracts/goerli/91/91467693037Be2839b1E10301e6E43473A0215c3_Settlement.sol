...
contract Settlement {    
    ...
    modifier onlyAllowed {
        require(custodianAddresses[msg.sender] == true || msg.sender == auditor, "Address not allowed");
        _;
    }
    ...
    function lockFunds(address tokenAddress, uint256 amount) public onlyAllowed onlyAllowedToken(tokenAddress) {
        ...
        if(custodianCategory[msg.sender] == 10) custodianCategory[msg.sender] = 2; //relocks custodian after marked for release
        require(custodianCategory[msg.sender] == 2, "This custodian category cannot lock funds");
    ...
    function unlockFunds(address tokenAddress) public onlyAllowed {
        bool operationOverflow = false;
    ...
    function authorizeSettlement(uint256 settlementUUID) public onlyAllowed {
        ...
        if (custodianCategory[msg.sender] == 1) {
        ...
        else if (custodianCategory[custodianSettlements[settlementUUID].debtor] == 2) {
            require(msg.sender == auditor, "Address is not the auditor of the settlement");
        } else {
    ...
    function _settle(uint256 settlementUUID) private {
        ...
        for (uint index = 0; index < settlement.tokens.length; index++) {
            ...
            //moves from locked debtor to unlocked creditor if creditor is category 0
        ...
        if (settlement.releaseCreditor == true && custodianCategory[settlement.creditor] == 2) custodianCategory[settlement.creditor] = 10;
        if (settlement.releaseDebtor == true && custodianCategory[settlement.debtor] == 2) custodianCategory[settlement.debtor] = 10;
    ...
    function setCustodianAllowable(address custodianAddress, uint8 custodianCategoryValue) external onlyOwner {
        custodianAddresses[custodianAddress] = true;
        custodianCategory[custodianAddress] = custodianCategoryValue;
    }

    function getCustodianAllowable(address custodianAddress) public view returns (bool) {
        return custodianAddresses[custodianAddress];
    }

    function removeCustodianAllowable(address custodianAddress) external onlyOwner {
    ...
    function changeAuditor(address auditorAddress) external onlyOwner {
