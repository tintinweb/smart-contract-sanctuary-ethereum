...
contract ZingArt_NFT1155 is Base_ZingNFT_1155 {
    ...
    function lazyMint(
        ...
        emit LazyMinted(_newTokenId, _msgSender(), _tokenUri, _creator, _price);
    }
    ...
    function _internalMint(
        ...
        creators[_newTokenId] = _creator;
        _setTokenURI(_newTokenId, _tokenUri);

        if (bytes(_tokenUri).length > 0) {
            emit URI(_tokenUri, _newTokenId);
        }
        _updateRoyalty(_newTokenId, _royalteFee);

        _mint(_to, _newTokenId, _supply, bytes(""));
        tokenSupply[_newTokenId] = _supply;
    ...
    function _setTokenURI(uint256 _id, string memory _uri) internal {
        ...
        require(!_fronzenTokens[_id], "Contract is already frozen");
