...
contract Rule is IRule {
    ...
    uint8 public constant NOT_WHITELISTED_CODE = 2;
    uint8 public constant GLOBALLY_BLACKLISTED_CODE = 3;
    uint8 public constant LOCALLY_BLACKLISTED_CODE = 4;
    ...
    function setGlobalList(GlobalList globalList_) external {
        globalList = globalList_;
    }

    function setLocalList(CMTAT localList_) external {
    ...
    function detectTransferRestriction(
        ...
        if (address(0) == _to) return ZERO_ADDRESS_RESTRICTION_CODE;
        if (globalList.isBlacklisted(_from)) return GLOBALLY_BLACKLISTED_CODE;
        if (localList.isBlacklisted(_from)) return LOCALLY_BLACKLISTED_CODE;
        if (!globalList.isWhitelisted(_from) && !localList.isWhitelisted(_from))
            return NOT_WHITELISTED_CODE;
        return SUCCESS_CODE;
    }

    /**
    is there a valid restriction code?
     */
    function canReturnTransferRestrictionCode(uint8 _restrictionCode)
        ...
        returns (bool)
    {
        return _canReturnTransferRestrictionCode(_restrictionCode);
    }
    ...
    function _canReturnTransferRestrictionCode(uint8 _restrictionCode)
        internal
        view
        returns (bool)
    {
    ...
    SnapshotModule
{
    ...
    event SetWhitelist(address[] wallets, bool value);
    event SetBlacklist(address[] wallets, bool value);
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit SetWhitelist(wallets, value);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit SetBlacklist(wallets, value);
    }
