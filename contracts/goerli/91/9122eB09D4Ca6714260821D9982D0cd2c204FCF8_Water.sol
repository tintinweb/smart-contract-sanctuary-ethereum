...
contract Water is ERC20, Ownable {
    ...
    //used to assist in calculating when to swap tax balances from the contract for their purposes
    uint256 private constant TOKEN_VALUE_UPDATE_INTERVAL = 1 days;
    uint256 private _lastTokenValueUpdateTime;
    uint256 private _tokenValueOfETH;
    ...
    constructor () ERC20("Water", "H2O") {
        ...
        // Set the initial tax structures here. Make sure it doesn't exceed 10% for buy tax and 15% for sell tax.
    uint256 buyTaxTotal = 10; // devFee %, partnerships %, maintenance %, autoBurn %
    uint256 sellTaxTotal = 15; // devFee %, partnerships %, autoBurn %, maintenance %
    ...
    function getTaxBalances() public view returns (uint256, uint256, uint256, uint256) {
        return (devFeeBalance, partnershipsBalance, autoBurnBalance, maintenanceBalance);
    }
    ...
    function _updateTokenValueOfETH() private {
        ...
        path[1] = address(this);
        uint256[] memory amounts = uniswapV2Router.getAmountsOut(1e18, path); // Get the value of 1 ETH in tokens
        _tokenValueOfETH = amounts[1];

        _lastTokenValueUpdateTime = block.timestamp;
    }

    function tokenValueOfETH() external view returns (uint256) {
        // Check if update interval has elapsed, if not return the current token value.
            if (block.timestamp < _lastTokenValueUpdateTime + TOKEN_VALUE_UPDATE_INTERVAL) {
            return _tokenValueOfETH;
            } else {
            // Approximate the new token value without changing the state variables
                address[] memory path = new address[](2);
                path[0] = uniswapV2Router.WETH();
                path[1] = address(this);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        // Apply maxWalletAmount check, unless the operation involves the contract owner, the contract itself, Uniswap V2 pair, or exchangeWhitelisted
        if (
            recipient != owner() &&
            recipient != address(this) &&
            recipient != uniswapV2Pair &&
            !exchangeWhitelist[recipient] &&
            balanceOf(recipient) + amount > maxWalletAmount
        ) {
            revert("Transfer amount would exceed the maxWalletAmount");
        }

        // Check if sender or recipient is the contract owner or the contract itself or whitelisted
        bool isExcluded = sender == owner() || sender == address(this) || recipient == owner() || recipient == address(this) || whitelist[sender] || whitelist[recipient];
