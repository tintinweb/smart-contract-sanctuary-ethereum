...
// File contracts/multitoken/MultiToken.sol
...
contract MultiToken is ERC1155URIStorage, Ownable2Step, Pausable, ERC1155Burnable, ERC1155Supply {
    bool public immutable PAUSABLE;
    bool public immutable BURNABLE;

    event ApprovalBurnPermission(address indexed newBurner, bool indexed approved);
    mapping(address => bool) public burnApprovals;
    // NOTE:
    // Having a single name and symbol is not appropriate for the original ERC1155.
    // However, we added it because most network explorers refer to this fields to expose token information.
    string public name;
    ...
    modifier hasPausePermission() {
        require(msg.sender == owner(), "MultiToken: msg.sender does not have permission to control the pause");
        _;
    }
    ...
    function pause() external whenPausableEnabled hasPausePermission {
        _pause();
    }

    function unpause() external whenPausableEnabled hasPausePermission {
        _unpause();
    }
