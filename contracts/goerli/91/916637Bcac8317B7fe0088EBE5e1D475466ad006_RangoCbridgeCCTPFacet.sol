...
contract RangoCbridgeCCTPFacet is IRango, ReentrancyGuard, IRangoCbridgeCCTP {

    /// Storage ///
    /// @dev keccak256("exchange.rango.facets.cbridge.cctp")
    bytes32 internal constant CBRIDGE_CCTP_NAMESPACE = hex"56911998398c5417ac7947c14653ae0e2afc65fe97f279a272eb722b31e2821f";
    uint256 public checkSwap;
    struct CbridgeCCTPStorage {
        /// @notice whitelisted addresses for cbridge circle proxy
        mapping(address => bool) cBridgeCCTPAddresses;
    }

    /// @notice Notifies that some new CCTP bridge addresses are whitelisted
    /// @param _addresses The newly whitelisted addresses
    event CbridgeCCTPBridgesAdded(address[] _addresses);

    /// @notice Notifies that some CCTP bridge addresses are blacklisted
    /// @param _addresses The addresses that are removed
    event CbridgeCCTPBridgesRemoved(address[] _addresses);
    ...
    function cbridgeCctpSwapAndBridge(
        LibSwapper.SwapRequest memory request,
        LibSwapper.Call[] calldata calls,
        IRangoCbridgeCCTP.CbridgeCCTPRequest memory bridgeRequest
    ) external payable nonReentrant {
        checkSwap = checkSwap + 1;
        // uint out = LibSwapper.onChainSwapsPreBridge(request, calls, 0);
        // LibSwapper.approveMax(request.toToken, bridgeRequest.cbridgeProxyAddress, out);
        ...
        //     false,
        //     false,
        //     uint8(BridgeType.CbridgeCCTP),
        //     request.dAppTag
        // );
    }

    function cbridgeCctpBridge(
        IRangoCbridgeCCTP.CbridgeCCTPRequest memory request,
        ...
        LibSwapper.approveMax(bridgeRequest.token, request.cbridgeProxyAddress, amountWithFee);
    ...
    function doDepositForBurn(
        IRangoCbridgeCCTP.CbridgeCCTPRequest memory request,
        address token,
        uint amount)
        internal
    {
        CbridgeCCTPStorage storage s = getCCTPStorage();
        require(
            s.cBridgeCCTPAddresses[request.cbridgeProxyAddress], 
            "Requested cctp address as tokenMessenger not whitelisted"
        );
        ICbridgeCCTP cbridgeProxy = ICbridgeCCTP(
            request.cbridgeProxyAddress
        );
        cbridgeProxy.depositForBurn(
            amount,
            request.destinationChainId,
            request.recipient,
            token
    ...
    function addCCTPBridgesInternal(address[] calldata _addresses) private {
        ...
        for (uint i = 0; i < _addresses.length; i++) {
            ...
            s.cBridgeCCTPAddresses[tmpAddr] = true;
        }

        emit CbridgeCCTPBridgesAdded(_addresses);
    }
    ...
    function removeCCTPBridges(address[] calldata _addresses) external {
        ...
        for (uint i = 0; i < _addresses.length; i++) {
            delete s.cBridgeCCTPAddresses[_addresses[i]];
        }

        emit CbridgeCCTPBridgesRemoved(_addresses);
    }

    /// @dev fetch local storage
    function getCCTPStorage() private pure returns (CbridgeCCTPStorage storage s) {
        bytes32 namespace = CBRIDGE_CCTP_NAMESPACE;
...
/// @title The root contract that handles Rango's interaction with CCTP bridge via cbridge proxy
/// @author Rulo Benini
interface ICbridgeCCTP {
...
interface IRango {
    ...
    enum BridgeType {Across, CBridge, Hop, Hyphen, Multichain, Stargate, Synapse, Thorchain, Symbiosis, Axelar, Voyager, Poly, OptimismBridge, ArbitrumBridge, Wormhole, AllBridge, CbridgeCCTP}
...
/// @title An interface to RangoCbridgeCCTP.sol contract to improve type hinting
/// @author Rulo Benini
interface IRangoCbridgeCCTP {
    struct CbridgeCCTPRequest {
        address cbridgeProxyAddress;
        bytes32 recipient;
        uint64 destinationChainId;
    }
