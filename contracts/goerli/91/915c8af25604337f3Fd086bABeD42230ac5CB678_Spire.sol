...
// genesis text.
contract Spire is GenesisText, AccessControl, Ownable, ReentrancyGuard, Multicall {
    enum Flow {
        Echo,
        Chapter
    }
    ...
    function init() external onlyOwner nonReentrant {
        ...
        _setRoleAdmin(MID_ADMIN_ROLE, SUPER_ADMIN_ROLE);
        _setupRole(MID_ADMIN_ROLE, msg.sender);
        
        initiated = true;
    }

    // To be called after contract deployment by the owner
    function grantRoles(address[] memory _addresses) external onlyOwner nonReentrant {
        uint256 l = _addresses.length;
        for (uint256 i = 0; i < l;) {
            _grantRole(MID_ADMIN_ROLE, _addresses[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function withdrawRoles(address[] memory _addresses) external onlyOwner nonReentrant {
        uint256 l = _addresses.length;
        for (uint256 i = 0; i < l;) {
            _revokeRole(MID_ADMIN_ROLE, _addresses[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function setURI(uint256 tokenId, string calldata _uri) external onlyRole(MID_ADMIN_ROLE) nonReentrant {
    ...
    function _advanceChapterFlowIfPossible() private returns (bool) {
        if (_latestChapterId(currentChapterFlowId) >= GlobalConstants.MAX_CHAPTER_COUNT - 1) {
            return false;
        }
    ...
    function approveChapterContestEntries(uint256[] memory entryIds) external onlyRole(MID_ADMIN_ROLE) nonReentrant {
    ...
    function _setLatestChapterContestWinner(
        ...
        internal
    {
        _setChapterContestWinner(genesisTextId, _latestChapterId(genesisTextId), winningId, losingEntryIds);
    }

    function _setLatestEchoContestWinners(
        uint256 genesisTextId,
        uint256 winningId,
        uint256 echoId,
        uint256[] memory losingEntryIds
        ...
        internal
    {
        _setInitialEchoContestWinners(genesisTextId, _latestEchoId(genesisTextId), echoId, winningId, losingEntryIds);
    }
...
contract Echoes is HasConfigStore, ERC1155Supply {
    ...
    function _mintContestWinnerBatch(address winner, uint256 count) internal {
        ...
        for (uint256 i = 0; i < count; i++) {
            _mint(winner, contestRewardTokenId + i, GlobalConstants.CONTEST_REWARD_AMOUNT - _teamCount, "");
            _mint(_teamWallet, contestRewardTokenId + i, _teamCount, "");
        ...
        internal
    {
        ...
        uint256 l = losingEntryIds.length;
        for (uint256 i = 0; i < l;) {
