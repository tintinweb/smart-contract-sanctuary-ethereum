...
contract EternityVipCards is ERC721Enumerable, Ownable, ReentrancyGuard {
   ...
   */
    ...
    */
	...
	uint16 public totalAmount = 10_000;
	...
	/**
	* @notice The amount of money for business needs
    */
	uint256 public governanceFundAmount = 0;
    ...
    */
	...
	/**
	* @notice DAO address where funds are paid
    */
...
* @dev Fired in addPriceForMint()
  ...
  * @dev Fired in mint(), safeMint()
	...
	*/
	...
	/**
	* @dev Fired in addWhitelistForMint
	*
	* @param _address address which can mint
	* @param tier tier of added whitelist
	* @param amount allowed amount for mint
	*/
   ...
   * @dev Fired in setBaseURI()
	...
	constructor(address payable _daoAddress) ERC721("EternityIO Vip Card", "EVC") payable{
   ...
   * @param _tier tier id
   */
	function mintCards(uint16 _amount, uint16 _tier) external payable nonReentrant {
        ...
        uint256 _cost = tiers[_tier].price * _amount;
		...
		if (tiers[_tier].tierType == TierType.Private) {
			require(whitelistForMint[msg.sender][_tier].amount != 0, "user is not whitelisted");
			require(whitelistForMint[msg.sender][_tier].totalMinted + _amount <= whitelistForMint[msg.sender][_tier].amount, "exceeded limit");
		}

		mint(_amount, _tier);
		governanceFundAmount += _cost;
	}

	/**
   * @notice withdraw amount calculated for business needs
   *
   * @param _amount the withdraw amount
   */
	function withdrawBusinessAmount (uint256 _amount) external payable onlyOwner nonReentrant {
		require(_amount <= governanceFundAmount, "amount can not be greater than current business fund amount");

		daoAddress.sendValue(_amount);
		governanceFundAmount -= _amount;
	}
   ...
   */
	function addTierForMint(uint16 _tier, uint256 _price, uint16 _amount, uint256 _mintStartDate, TierType _type, TierStatus _status) external onlyOwner {
	...
	/**
   * @notice whitelist address for minting some tier
   *
   * @param _address the address need to be whitelisted
   * @param _tier the id of the tier
   * @param _amount the amount of whitelisted cards
   */
	function addWhitelistForMint(address _address, uint16 _tier, uint16 _amount) external onlyOwner {
		require(tiers[_tier].amount != 0, "tier does not exist");
		require(tiers[_tier].lockedAmount + tiers[_tier].totalMinted + _amount <= tiers[_tier].amount, "whitelisted amount exceeds total tier amount");
	...
	*/
	function changeTierStatus(uint16 _tier, TierStatus _status) external onlyOwner {
		...
		if(_status == TierStatus.Active) {
			...
			require(tiers[_tier].amount != tiers[_tier].totalMinted, "tier is fully minted");
			...
			if (activeTierId > 0) {
				...
				if (remainingAmount > 0) {
					...
					tiers[activeTierId].lockedAmount = 0;
	...
	/**
   * @notice gets the info about user current active tier
   *
   * @return the tier id
   * @return the price of the card
   * @return the amount which can be minted
   */
	function getUserActiveTier() external view returns (uint16, uint256, uint16) {
		if (activeTierId == 0 || tiers[activeTierId].mintStartDate > block.timestamp) {
			return (0, 0, 0);
		}

		if(tiers[activeTierId].tierType == TierType.Private) {
			Whitelist memory w = whitelistForMint[msg.sender][activeTierId];
			if (w.amount > 0) {
				return (activeTierId, tiers[activeTierId].price, w.amount - w.totalMinted);
			}
		} else if (tiers[activeTierId].tierType == TierType.Public) {
			return (activeTierId, tiers[activeTierId].price, tiers[activeTierId].amount - tiers[activeTierId].totalMinted);
		}
...
* @notice check if user is whitelisted in tier
   * @param _tier the id of the tier
   */
	function isWhitelisted(uint16 _tier) external view returns (uint8) {
		require(tiers[_tier].amount != 0, "tier does not exist");
   ...
   */
	...
	function changeDaoAddress(address payable _daoAddress) external onlyOwner {
		daoAddress = _daoAddress;
	}
   ...
   */
	function mint(uint16 _mintAmount, uint16 _tier) private {
		...
		if (tiers[_tier].tierType == TierType.Private) {
			...
			tiers[_tier].lockedAmount -= _mintAmount;
		}
