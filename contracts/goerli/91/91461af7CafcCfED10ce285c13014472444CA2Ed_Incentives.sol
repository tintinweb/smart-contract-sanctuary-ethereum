...
/// @title Mainnet bonding and boosting implementation contract
contract Incentives is IncentivesBase {
   
    function initialize(address _gac, address _boost, address _bondToken) external initializer {
        __IncentivesBase_init(_gac, _boost, _bondToken);
    }    
}
...
abstract contract IncentivesBase is Bond {
    Boost public boost;

    /// @notice Contract initialiser
    /// @param _gac Bumper Global Access Controller
    /// @param _boost Protocol Boost Incentive calculatution address
    function __IncentivesBase_init(
        address _gac,
        address _boost,
        address _bondToken
    ...
    /// @notice calculates boost amount
    function calcBoost(int128 w5Max, int128 lr5, int128 rlr5, int128 w1Max, int128 lr1, int128 rlr1) public view returns (int128 boostTaker, int128 boostMaker) {
        (boostTaker, boostMaker) = boost.calcBoost(w5Max, lr5, rlr5, w1Max, lr1, rlr1);
    }

    /// @notice calculates incentive
    function calcIncentive(uint tvlStable) public view returns (int128 incentive) {
        incentive = boost.calcIncentive(tvlStable);
    }
}
...
abstract contract Bond is BondBase {
...
/// @title Boost
contract Boost {
    int128 public boostMax;

    int128 public a;
    int128 public b;
    int128 public c;

    constructor(int128 _boostMax , int128 _a, int128 _b, int128 _c) {
        boostMax = _boostMax;
        
        (a, b, c) = (_a, _b, _c);
    }

    function setBoostMax(int128 _boostMax) external {
        boostMax = _boostMax;
    }

    function setIncentiveConfig(int _a, int _b, int _c) external {
        (a, b, c) = (ABDK.fromInt(_a), ABDK.fromInt(_b), ABDK.fromInt(_c));
    }

    function calcBoost(int128 w5Max, int128 lr5, int128 rlr5, int128 w1Max, int128 lr1, int128 rlr1) external view returns (int128 boostTaker, int128 boostMaker) {
        (boostTaker, boostMaker) = _boost(w5Max, lr5, rlr5, w1Max, lr1, rlr1);
    }

    function _boost(int128 w5Max, int128 lr5, int128 rlr5, int128 w1Max, int128 lr1, int128 rlr1) internal view returns (int128, int128) {
        int128 boostTaker = min(w5Max * 10**18, 10**18 / (lr5 - rlr5)) - min(w1Max * 10**18, 10**18 / (lr1 - rlr1));
        return (boostTaker, boostMax * 10**18 - boostTaker);
    }

    function calcIncentive(uint tvlStable) external view returns (int128 incentive) {
        incentive = _incentive(tvlStable);
    }

    function _incentive(uint tvlStable) internal view returns (int128) {
        return ABDK.add(ABDK.div(ABDK.neg(a), b + ABDK.exp(ABDK.neg(ABDK.add(ABDK.fromUInt(tvlStable), c)))), ABDK.div(a, b));
    }

    function min(int128 _a, int128 _b) private pure returns (int128) {
        return _a < _b ? _a : _b;
    }
}
...
abstract contract BondBase is BACUpgradeable, IBond {
    ...
    function BOND_TOKEN_ADDRESS() public view virtual override returns (address){
        return bondToken;
    }
    ...
    modifier onlyMarket() {
        require(
            hasRole(MARKET_ROLE, msg.sender),
            "BB: not-permitted-market"
        );
        _;
    }
    ...
    function withdraw(uint amount) external virtual override {
        ...
        IERC20(BOND_TOKEN_ADDRESS()).safeTransfer(_msgSender(), amount);
    }
    ...
    function lockForTaker(
        ...
        uint16 risk,
        uint16 term
    ) external virtual override onlyPermittedAdapter returns (uint bondAmount) {
        require(depositedBalances[_owner] >= amount, "BB: exeed-balance");
        bondAmount = takerBond(IClientMarketAdapter(msg.sender).ASSET_ADDRESS(), amount, risk, term);
        _lock(_owner, amount);
    }
    ...
    function lockForMaker(
        ...
        require(depositedBalances[_owner] >= amount, "BB: exeed-balance");
        bondAmount = makerBond(IClientMarketAdapter(msg.sender).ASSET_ADDRESS(), amount, risk, term);
        _lock(_owner, amount);
    }
    ...
    function unlock(
        address _owner,
        uint bondAmount
    ) external virtual override onlyPermittedAdapter {
        ...
        )
    {
        (bondAmount, toTransfer, toReduce) = calcBondSizeForTakerPosition(
            recipient,
            IClientMarketAdapter(msg.sender).ASSET_ADDRESS(),
            amount,
            risk,
            term
        );
    ...
    function lock(
        address addr,
        uint amount
    ) external override onlyPermittedAdapter {
        _lock(addr, amount);
    }
