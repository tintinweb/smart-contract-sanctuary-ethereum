...
contract AASPool {
    ...
    address public constant ANGRY_APES_ADDRESS = 0xde745a45d1fDd48dd3CF752E42d6C1a6d1f9E5eb;
    address public constant ANGRY_RIDERS_ADDRESS = 0x5E55D430541C31Fa46E8017f007CB7Cef68Fb673;
    address public constant PORTAL_PASS_ADDRESS = 0xAfDA903e6Ce5aBacca215e991FEf7d7251B92F5a;
    ...
    function createPoll(string memory topic, string memory question, string[] memory options, uint256 startTime, uint256 endTime) public onlyOwner {
    ...
    function getTotalVotes(uint pollIndex) public view returns (uint totalVotes) {

        for(uint i = 0; i < polls[pollIndex].options.length; i++) {
            totalVotes += polls[pollIndex].results[i];
        }
    }

    function vote(uint pollIndex, uint optionIndex) public {
    // Check if user own AAS NFTs
    require(
    ...
    // Check poll status & if user already voted
    require(block.timestamp >= polls[pollIndex].startTime && block.timestamp <= polls[pollIndex].endTime, "Poll is not active");
    require(!polls[pollIndex].hasVoted[msg.sender], "You have already voted");

    // Finally vote
    polls[pollIndex].hasVoted[msg.sender] = true;
