...
interface ILetsFilController is ILetsFilPackInfo {
...
contract LetsFilFactory is ILetsFilPackInfo, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    struct RaisePlan {
        ...
        address controllerAddr;   // plan controller's address
    }
    // raiseId => raisePlan, all raise plan info
    mapping(uint256 => RaisePlan) public plans;
    address  public toolAddr;

    //计算罚金的相关数据
    //Network base annual interest rate for calculating fine
    uint256 private constant networkAnnual = 100;//网络基础年化利率
    uint256 private constant rateBase = 10000;//基础比率（作为分母）
    uint256 private constant fineCoeff = 30000;//封装期罚息倍数
    //Successfully to entered the delay period
    uint256 private constant sealMinCoeff = 5000;//封装进入展期的最低比例
    //to set the seal delay days
    uint256 private constant sealDelayCoeff = 5000;//封装展期比例
    uint256 private constant protocolSealFineCoeff = 10;//协议罚金系数
    //The minimum maintenance margin coefficient.
    uint256 private constant opsSecurityFundMinCoeff = 500;//运维保证金最小系数
    //raise plan's commission coefficient
    uint256 private constant feeCoeff = 30;//募集成功平台手续费率
    ...
    function createRaisePlan(RaiseInfo memory _raiseInfo, NodeInfo memory _nodeInfo, ExtendInfo memory _extendInfo) public returns (address planAddress) {
        ...
        ILetsFilController(planAddress).initialize(_raiseInfo.id, _raiseInfo, _nodeInfo, _extendInfo);
        ...
        plans[_raiseInfo.id].controllerAddr = planAddress;
    ...
    function deploy(uint256 _raiseID) internal returns (address) {
        ...
        return address(new LetsFilProxy{salt: keccak256(abi.encode(_raiseID))}(toolAddr));
    }
