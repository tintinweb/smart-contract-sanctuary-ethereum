...
contract BatchReader 
{
    ...
    function readWalletOwnedMomentIndividual(address Wallet, uint ProjectID, uint TokensToIterateOver) public view returns (uint[] memory)
    {
        IERC721 Moments = IERC721(0xe745243b82ebC46E5c23d9B1B968612c65d45f3d);
        uint _TokenID = ProjectID * 1000000;
        uint Range = _TokenID + TokensToIterateOver;
        uint[] memory _OwnedIDs = new uint[](TokensToIterateOver);
        uint NumOwned;
        for(_TokenID; _TokenID < Range; _TokenID++)
        {
            try Moments.ownerOf(_TokenID) 
            { 
                if(Moments.ownerOf(_TokenID) == Wallet)
                {
                    _OwnedIDs[NumOwned] = _TokenID;
                    NumOwned++;
                }
            } catch { }
        }
     ...
     */
    function readWalletOwnedMoment(address Wallet, uint ProjectID, uint TokensToIterateOver) public view returns (uint[] memory)
    {
        IERC721 Moments = IERC721(0xe745243b82ebC46E5c23d9B1B968612c65d45f3d);
        uint[] memory temp = new uint[](TokensToIterateOver);
        uint TokenID = ProjectID * 1000000;
        uint NumOwnedTokens;
        for(TokenID; TokenID <= TokenID + TokensToIterateOver; TokenID++)
        {
            try Moments.ownerOf(TokenID) 
            {
                if(Moments.ownerOf(TokenID) == Wallet)
                {
