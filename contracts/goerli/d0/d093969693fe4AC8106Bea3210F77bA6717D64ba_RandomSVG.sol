...
contract RandomSVG is ERC721URIStorage, VRFConsumerBaseV2, Ownable {
    ...
    // SVG parameters
    uint256 public immutable i_maxNumberOfPaths;
    uint256 public immutable i_maxNumberOfPathCommands;
    uint256 public immutable i_size;
    string[] public s_pathCommands;
    string[] public s_colors;
    ...
    event NftRequested(uint256 indexed requestId, uint256 indexed tokenId);
    ...
    ) ERC721("Random SVG", "RSNFT") VRFConsumerBaseV2(_vrfCoordinatorV2) {
        i_vrfCoordinatorV2 = VRFCoordinatorV2Interface(_vrfCoordinatorV2);
        i_gasLane = _gasLane;
        i_subscriptionId = _subscriptionId;
        i_callbackGasLimit = _callbackGasLimit;
        s_tokenCounter = 0;
        i_maxNumberOfPaths = 10;
        i_maxNumberOfPathCommands = 5;
        i_size = 500;
        s_pathCommands = ["M", "L"];
        s_colors = ["red", "blue", "green", "yellow", "black", "white"];
    }
    ...
    function createNFT() public returns (uint256 requestId) {
        ...
        emit NftRequested(requestId, tokenId);
    }
    ...
    function fulfillRandomWords(
        ...
        s_tokenIdToRandomNumber[tokenId] = _randomWords[0];
        emit CreatedUnfinishedRandomSVG(tokenId, _randomWords[0]);
    }
    ...
    function finishMint(uint256 _tokenId) public {
        ...
        // [X] use that imageURI to format a tokenURI
        require(bytes(tokenURI(_tokenId)).length <= 0, "RandomSVG: token URI is already set"); // checks if this has already been done
        require(s_tokenCounter > _tokenId, "RandomSVG: token ID has not been minted yet"); // checks if the token id exists
        require(
            s_tokenIdToRandomNumber[_tokenId] > 0,
            "Random SVG: Please, wait for Chainlink response"
        ); // checks if there is a random number for that token id
    ...
    function generateSVG(uint256 _randomNumber) public view returns (string memory finalSvg) {
        ...
        for (uint256 i = 0; i < numberOfPaths; i++) {
            // this is a good approache because nobody can predict what this number is going to be
            // when the random number is returned nobody can scoop it because it's already been decided its value
            // this method works fine here, but not on the createNFT function because people can brute force the value
    ...
    function generatePath(uint256 _randomNumber) public view returns (string memory pathSvg) {
        uint256 numberOfPathCommands = (_randomNumber % i_maxNumberOfPathCommands) + 1;
        pathSvg = "<path d='";
        for (uint256 i = 0; i < numberOfPathCommands; i++) {
            uint256 newRNG = uint256(keccak256(abi.encode(_randomNumber, i_size + i))); // add i_size to generate a different random number from the one in generateSVG
            string memory pathCommand = generatePathCommand(newRNG);
            pathSvg = string(abi.encodePacked(pathSvg, pathCommand));
        }
        string memory color = s_colors[_randomNumber % s_colors.length]; // gets a random color
    ...
    function generatePathCommand(
        ...
        // It's modded by the size to prevent the parameter to greater than the i_size;
        uint256 parameterOne = uint256(keccak256(abi.encode(_randomNumber, i_size * 2))) % i_size;
        uint256 parameterTwo = uint256(keccak256(abi.encode(_randomNumber, i_size * 2 + 1))) %
            i_size;
        pathCommand = string(
