...
contract DejitaruNeko is ERC20, Ownable {
    ...
    constructor() ERC20(_name, _symbol, _decimals) {
        ...
        marketingWallet = address(0xa3b4c5877F682Ab4B5Fc52Cfd1d0D5f2834D4d02);
        developmentWallet = address(0xa3b4c5877F682Ab4B5Fc52Cfd1d0D5f2834D4d02);
    ...
    function updateDeadline(uint256 _deadline) external isAuth {
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external isAuth returns (bool){
        require(
    ...
    function updateMaxWalletAndTxnAmount(uint256 newTxnNum, uint256 newMaxWalletNum) external isAuth {
        require(
    ...
    function updateFees(uint256 _buyMarketingFee, uint256 _buyLiquidityFee, uint256 _sellMarketingFee, uint256 _sellLiquidityFee) external isAuth {
    ...
    function excludeMultipleFromFees(address[] calldata accounts, bool excluded) external isAuth {
    ...
    function excludeAutomaticWallets(address[] calldata accounts, bool excluded) external isAuth {
        for (uint256 i = 0; i < accounts.length; i++) {
            automaticWallets[accounts[i]] = excluded;
        }
    }

    function updateMarketingWallet(address newWallet) external isAuth {
    ...
    function rescueTokens(address _tokenAddress, address _to, uint256 _amount) public onlyOwner {
