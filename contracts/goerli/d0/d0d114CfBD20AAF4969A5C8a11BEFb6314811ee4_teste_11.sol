...
contract teste_11 is ERC721A, Ownable {
    ...
    /* function mint2(uint256 _mintAmount, bytes calldata _signature, bytes32 _nonce) external returns (bool){
        return keccak256(abi.encodePacked(msg.sender, _mintAmount, saleState, _nonce))
        .toEthSignedMessageHash()
        .recover(_signature) == msg.sender;
    } */
    function mint(uint256 _mintAmount) external payable{ 
        ...
        if(saleState == 1){   //presale
            ...
            require(numberMinted(msg.sender) + _mintAmount <= maxMintAmount, "Cannot mint more");
