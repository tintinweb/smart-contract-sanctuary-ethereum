...
contract CCZoo is ERC721A, Ownable {
    ...
    event animalSet(uint256 indexed tokenId, uint8 animalId);
    ...
    // function setPalettes(bytes3[][] memory _newColors, IExquisiteGraphics.PaletteTypes[] memory _type, string[] memory names) external onlyOwner {
    //     for(uint256 i = 0; i < _newColors.length; i++) {
    //         palettes.push(IExquisiteGraphics.Palette(_newColors[i], _type[i]));
    //     }
    //     traitNames[0] = names;
    // }
    ...
    function _getTokenSVG(uint256[NUM_LAYERS] memory tokenTraits, uint256 tokenId, bytes memory buffer) public view  { //view
        ...
        IExquisiteGraphics.Palette memory _currentPallet = palettes[tokenTraits[0]];
        console.log("redndered animal");
    ...
    function _renderAnimal(Animal memory _currentAnimal) private view returns(bytes memory )  { //view
    ...
    function _getSize(bytes memory data)
    internal
    pure
    returns (uint8 _size)
  {
