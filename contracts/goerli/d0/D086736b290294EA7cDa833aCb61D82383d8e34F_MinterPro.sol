...
contract MinterPro is IMinterPro, Errors, AccessControl, ReentrancyGuard, Pausable {
    ...
    event Minted(address indexed to, bytes32 id);
     ...
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(IMinterPro, AccessControl) returns (bool) {
        return
     ...
     * @param id to emint event and thus identify minted token by Back End
     */
    function mintPro(
        address to,
    ...
    function _hash(
        ...
        return keccak256(abi.encodePacked(addr, numberMinted, expiry));
    }
     ...
     */
    function upgradeCharacterContract(address newContract) external onlyRole(MANAGER_ROLE) returns (address) {
     ...
     * @param newContract Address of the new contract
     */
    function upgradeBagContract(address newContract) external onlyRole(MANAGER_ROLE) returns (address) {
        if (newContract == address(0)) revert InvalidInput(INVALID_ADDRESS);
    ...
    function setMultisig(address _newMultisig) external onlyRole(MULTISIG_ROLE) {
        ...
        if (_newMultisig == _multisig) revert AccessError(ALREADY_ASSIGNED);
    ...
    function setSigner(address newSigner) external onlyRole(MANAGER_ROLE) {
        ...
        if (newSigner == _signer) revert AccessError(ALREADY_ASSIGNED);
        _signer = newSigner;
...
interface ICharacter {
    ...
    function getBurner(uint256) external returns (address);
...
interface IMinterPro {
    ...
    function upgradeBagContract(address) external returns (address);

    function upgradeCharacterContract(address) external returns (address);
