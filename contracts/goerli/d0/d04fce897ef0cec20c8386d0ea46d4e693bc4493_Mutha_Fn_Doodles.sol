...
contract Mutha_Fn_Doodles is Ownable, ERC721A {
    using Strings for uint256;
    IStaking public StakingContract;
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmbDWUAvpXt4cBETsCidvRjYJ2SodTR8N3dHkjmwM6JAE5/";
    uint256 public MFD_MAX = 5000;
    uint256 public MFD_PRICE = 0.01 ether;
    uint256 public MFD_PER_MINT = 20;
    ...
    constructor() ERC721A("Mutha_Fn_Doodles", "MFD") {}
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(tokenMintsCounter + receivers.length <= MFD_MAX, "EXCEED_MAX");
        for (uint256 i = 0; i < receivers.length; i++) {
            tokensLastClaimTime[tokenMintsCounter] = block.timestamp;
    ...
    function founderMint(uint256 tokenQuantity) external onlyOwner {
        require(tokenMintsCounter + tokenQuantity <= MFD_MAX, "EXCEED_MAX");
        tokensLastClaimTime[tokenMintsCounter] = block.timestamp;
    ...
    function mint(uint256 tokenQuantity) external payable callerIsUser {
        ...
        require(tokenMintsCounter + tokenQuantity <= MFD_MAX, "EXCEED_MAX");
        require(tokenQuantity <= MFD_PER_MINT, "EXCEED_PER_MINT");
        require(MFD_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function withdraw() external onlyOwner {
        payable(0xa4F213f5e49D9a423Bb280024B577053b23B6974).transfer(address(this).balance / 10);
        uint256 currentBalance = address(this).balance;
        payable(0x3445544E92EF4a2708000A08D20F14B24BA460F1).transfer(currentBalance * 445/1000);
        payable(0x6b00042B2E924e3E5E8746262aF5981C5FA5D7C8).transfer(address(this).balance);
    }
    ...
    function claimStakingRewards() external {
        ...
        for (
            ...
            require(
                tokensLastClaimTime[balanceIds[msg.sender][index]] > 0,
                "WRONG_TIME"
            );
            totalRewards = totalRewards + earningRate * (block.timestamp - (tokensLastClaimTime[balanceIds[msg.sender][index]]));
            tokensLastClaimTime[balanceIds[msg.sender][index]] = block.timestamp;
        }
    ...
    function transferFrom(
        ...
        tokensLastClaimTime[tokenId] = block.timestamp;
