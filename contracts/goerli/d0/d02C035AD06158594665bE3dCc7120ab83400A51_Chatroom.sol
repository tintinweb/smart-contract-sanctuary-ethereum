...
contract Chatroom is Ownable {
    ...
    MessageStruct[] private allMessages;

    event NewParticipant(ParticipantStruct);
    event NewMessage(MessageStruct);

    constructor() {
        participantList[msg.sender] = ParticipantStruct(
            msg.sender,
            "owner",
            block.timestamp
        );
    ...
    function checkParticipantIdentifierExist(
        address _addr,
        string memory _identifier
    ) public view returns (bool) {
        return
            keccak256(abi.encodePacked(participantList[_addr].identifier)) ==
        ...
        onlyOwner
    {
        ...
        require(
            checkParticipantIdentifierExist(_addr, _identifier),
            "Identifier exist."
        );
        ...
        emit NewParticipant(p);
        participantList[_addr] = p;
    }

    function sendMessage(string calldata _msg) external {
        require(checkIsParticipant(msg.sender));
        MessageStruct memory newMsg = MessageStruct(
            msg.sender,
            block.timestamp,
            _msg
        );
    ...
    function readMessage() external view returns (MessageStruct[] memory) {
        return allMessages;
    }
}
