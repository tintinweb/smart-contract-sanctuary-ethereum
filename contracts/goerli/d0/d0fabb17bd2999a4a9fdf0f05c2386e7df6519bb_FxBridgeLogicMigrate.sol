...
contract FxBridgeLogicMigrate is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20MetadataUpgradeable;
    ...
    function init(bytes32 _fxBridgeId, uint256 _powerThreshold, address _fxOriginatedToken, uint256 _lastEventNonce, bytes32 _lastValsetCheckpoint, uint256 _lastValsetNonce, address[] memory _bridgeToken, uint256[] memory _lastBatchNonce) public initializer {
        __Pausable_init();
        __Ownable_init();
        __ReentrancyGuard_init();
        ...
        state_fxBridgeId = _fxBridgeId;
        state_powerThreshold = _powerThreshold;
        state_fxOriginatedToken = _fxOriginatedToken;
        state_lastEventNonce = _lastEventNonce;
        state_lastValsetCheckpoint = _lastValsetCheckpoint;
        state_lastValsetNonce = _lastValsetNonce;

        require(_bridgeToken.length == _lastBatchNonce.length, "bridge tokens length must equal number of last batch nonce");
        for (uint256 i = 0; i < _bridgeToken.length; i++) {
            if (_bridgeToken[i] != _fxOriginatedToken) {
                bridgeTokens.push(_bridgeToken[i]);
            }
            state_lastBatchNonces[_bridgeToken[i]] = _lastBatchNonce[i];
        }
    }
...
contract FxBridgeLogicBSC is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    using SafeMathUpgradeable for uint256;
