...
contract PolarPlotter is IRenderer {
  uint private constant SLICES = 64;
  uint256 constant PI = 3141592653589793238;
  address private _owner;

  modifier validLength(bytes calldata props) {
    require(props.length % (SLICES * 4) == 0);
    _;
  }
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165) returns (bool) {
    return interfaceId == type(IRenderer).interfaceId ||
  ...
  function _generateHTML(bytes calldata props) validLength(props) internal pure returns (string memory) {
    string memory output = '<svg xmlns="http://www.w3.org/2000/svg" width="570" height="570" style="background:#000000">';

    for (uint i = 0; i < (props.length / 4); i++) {
      uint index = i * 4;
      int rad = int(uint(uint8(props[index])));
      uint8 r = uint8(props[index + 1]);
      uint8 g = uint8(props[index + 2]);
      uint8 b = uint8(props[index + 3]);
      // We don't normally want to divide first but PI is a really big number
      uint256 angle = ((2 * PI / SLICES) * i) + 2 * PI;
      int256 x = Trigonometry.cos(angle);
      int256 y = Trigonometry.sin(angle);
      // normalize x & y to our plot
      int256 xScaled = x * rad / 1e18;
      int256 yScaled = y * rad / 1e18;
      uint256 xNormalized = uint256(285 + xScaled);
      uint256 yNormalized = uint256(285 + yScaled);
      // generate svg code
      output = string(abi.encodePacked(
        output,
        '<circle cx="',
        Strings.toString(xNormalized),
        '" cy="',
        Strings.toString(yNormalized),
        '" r="10" fill="rgb(',
        Strings.toString(r),
        ',',
        Strings.toString(g),
  ...
  function attributes(bytes calldata props) validLength(props) external override pure returns (string memory) {
    return string(abi.encodePacked('{"trait_type": "Data Length", "value":', Strings.toString(SLICES * 4), '}'));
  }
}
