...
contract ChimneyTownGift is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    string public baseURI = "https://metadata.ctdao.io/CTG/";
    string public baseExtension = ".json";
    mapping(address => bool) _operators;

    constructor() ERC721("CHIMNEY TOWN GIFT", "CTG") {
        _operators[msg.sender] = true;
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ChimneyTownGift: URI query for nonexistent token");
    ...
    function mint(address to, uint256 tokenId) external nonReentrant onlyOperator{
        require(tokenId <= maxSupply  , 'ChimneyTownGift: can not mint, over max size');
        _safeMint(to, tokenId);
    }
    ...
    function operatorBurn(uint256 tokenId) external onlyOperator {
        _burn(tokenId);
    }
    ...
    //disable approve
    function approve(address to, uint256 tokenId) public virtual override(ERC721,IERC721) disabled {}
    function setApprovalForAll(address operator, bool approved) public virtual override(ERC721,IERC721) disabled {}
}
