...
contract Snap_Swap is Context, AccessControlEnumerable, Pausable {
    ...
    */
    mapping(uint256 => uint256) private referrals_amount;
    ...
    */
    ...
    event mint_event(address backed_by_token, uint256 amount, uint256 referral);

    // fires when updating a price feed aggregator address
    event AggregatorUpdated(address new_aggregator, bool isUSDT);

    // fires when ETH has been send to this contract
    ...
    /**
    @dev checks argument is not address 0
     */
    modifier NoAddress0(address _address) {
    ...
    */
     ...
     */
    function swap_by_USDT(uint256 _amount, uint256 _referral) public {
        address _USDT = USDT;
        if (IERC20(_USDT).allowance(_msgSender(), address(this)) < _amount) {
            revert AllowanceIsLow();
        }

        require(updateOrReject(_amount));

        IERC20(_USDT).transferFrom(_msgSender(), address(this), _amount);
        uint256 _swap_amount = (USDT_USD_price() * _amount);
        IERC20(token).transfer(_msgSender(), _swap_amount);
        referrals_amount[_referral] += _amount;
        emit mint_event(_USDT, _swap_amount, _referral);
    }
    ...
    */
    ...
    function USDT_USD_price() public view returns (uint256 _USDT_price) {
        ...
        return (uint256(price) * (10**10)); // it already has 8 decimals, we add another 10
    }
    ...
    @param _referral code 
     */
    function swap_by_USDC(uint256 _amount, uint256 _referral) public {
        address _USDC = USDC;
        if (IERC20(_USDC).allowance(_msgSender(), address(this)) < _amount) {
            revert AllowanceIsLow();
        }

        require(updateOrReject(_amount));

        IERC20(_USDC).transferFrom(_msgSender(), address(this), _amount);
        uint256 _swap_amount = (USDC_USD_price() * _amount);
        IERC20(token).transfer(_msgSender(), _swap_amount);
        referrals_amount[_referral] += _amount;
        emit mint_event(_USDC, _swap_amount, _referral);
    }
     ...
     */
    function updateOrReject(uint256 _amount) private returns (bool) {
        if (
    ...
    * NOTE: false is USDC, true is USDT
    ...
    /**
    @dev getting a referral amount
    @param _referralCode which we want to get the amount of
     */
    function getReferralAmount(uint256 _referralCode)
        public
        view
        returns (uint256)
    {
        return referrals_amount[_referralCode];
    }

    /**
    @dev this function is for receiving ETH
     */
    function pay() public payable {
        emit ETHReceived(msg.value, _msgSender());
    }
}
