...
import {StableSwap3PoolHelper} from "../validators/utils/StableSwap3PoolHelper.sol";
import {Meta3PoolHelper} from "../validators/utils/Meta3PoolHelper.sol";
...
contract LUSD3CRVValidatorScript is Script {
    function run() external {
        ...
        address lusdChainlinkFeed = address(
            0xCCe4DafB5c4FC4296359C47c7290E5A9B4228520
        );
        address usdcChainlinkFeed = address(
            0xaC75604863468925691d402AC93359D50aB680a7
        );
        address daiChainlinkFeed = address(
            0xA9a895D11e6893Cbf913dcd12963d706d4e05BbC
        );
        address usdtChainlinkFeed = address(
            0xD11937267348dCD738E91Dd5871C87fc777dE32e
        );
        ...
        StableSwap3PoolHelper stableHelper = new StableSwap3PoolHelper();
        Meta3PoolHelper metaHelper = new Meta3PoolHelper();
        
        LUSD3CRVValidator validator = new LUSD3CRVValidator(helper, stableHelper, metaHelper);
        ...
        oracle.shift(rateId, address(0), 0, 0, value+1, nonce);
...
contract LUSD3CRVValidator is IProofValidator, IValidator {
    ...
    uint256 public constant chainlinkTimeWindow = 48 * 3600;
    ...
    struct ProofWrapper {
        ...
        bytes metaAccount;
        bytes tokenAccount;
        bytes curvePoolProofRlpBytes;
        bytes curveMetaPoolProofRlpBytes;
        bytes curveTokenProofRlpBytes;
    }

    LUSD3CRVHelper public immutable helper;
    StableSwap3PoolHelper public immutable stableSwapPoolHelper;
    Meta3PoolHelper public immutable metaPoolHelper;
    ...
    constructor(LUSD3CRVHelper helper_, StableSwap3PoolHelper stableSwapPoolHelper_, Meta3PoolHelper metaPoolHelper_) {
        helper = helper_;
        stableSwapPoolHelper = stableSwapPoolHelper_;
        metaPoolHelper = metaPoolHelper_;
    }
        ...
        )
    {
        return (
            CURVE3POOL_ADDRESS,
            stableSwapPoolHelper.BALANCES_0_POS(),
            stableSwapPoolHelper.BALANCES_1_POS(),
            stableSwapPoolHelper.BALANCES_2_POS(),
            stableSwapPoolHelper.INITIAL_A_POS(),
            stableSwapPoolHelper.FUTURE_A_POS(),
            stableSwapPoolHelper.INITIAL_A_TIME_POS(),
            stableSwapPoolHelper.FUTURE_A_TIME_POS()
        );
    }
        ...
        )
    {
        return (
            LUSD3POOL_ADDRESS,
            metaPoolHelper.BALANCES_0_POS(),
            metaPoolHelper.BALANCES_1_POS(),
            metaPoolHelper.INITIAL_A_POS(),
            metaPoolHelper.FUTURE_A_POS(),
            metaPoolHelper.INITIAL_A_TIME_POS(),
            metaPoolHelper.FUTURE_A_TIME_POS(),
            metaPoolHelper.RATE_MULTIPLIER_POS(),
            metaPoolHelper.TOTAL_SUPPLY_POS()
        );
    }
    ...
    function validateCurve(
        ...
        emit LogStep(header.usdcRoundId);
        ...
        emit LogStep(blockNumber);

        stableSwapPoolHelper.verifyAccount(CURVE3POOL_ADDRESS_HASH,blockHeader.stateRootHash,proofs.curveAccount, proofs.curvePoolProofRlpBytes);
    }
    ...
    function verifyAccount(
    ...
    )public view returns (StableSwap3PoolHelper.PoolStorage memory poolStorage) {
        ...
        StateProofVerifier.SlotValue memory daiBalance = StateProofVerifier
            .extractSlotValueFromProof(
                stableSwapPoolHelper.BALANCES_0_POS_HASH(),
    ...
    function validate(
        ...
        uint256 computedValue = _computeValue(
            stableSwapPoolHelper.verifyAndExtractStorage(
                CURVE3POOL_ADDRESS_HASH,
                blockHeader.stateRootHash,
                proofs[0],
                proofs[3].toList()
            ),
            metaPoolHelper.verifyAndExtractStorage(
    ...
    function _computeValue(
        ...
        uint256 curve3VirtualPrice = stableSwapPoolHelper.get_virtual_price(
            curve3PoolStorage,
            tokenTotalSupply,
            timestamp
        );
        uint256 metapoolVirtualPrice = metaPoolHelper.get_virtual_price(
            metaPoolStorage,
            curve3VirtualPrice,
            timestamp
        );
