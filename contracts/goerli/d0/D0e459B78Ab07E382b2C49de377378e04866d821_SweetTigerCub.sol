...
// File: contracts/SweetTigerCub.sol
...
/**
 * @author TROISIEME LABS
 * @title Sweet Tiger Cub
 * @dev Deploy a ERC721A NFT Collection
 */
contract SweetTigerCub is ERC721A, Ownable {
    ...
    uint public maxSupplyForSale = 7800;
    ...
    constructor(address payable team) ERC721A("Sweet Tiger Cub", "STC") {
        _team = team;
        //minting for team
        //_mint(team, 1);
    }

    function mintPublicSaleAndWhitelist(uint256 quantity, bytes32[] calldata merkleProof) external payable isWhitelisted(merkleProof) {
        // `_mint`'s second argument now takes in a `quantity`, not a `tokenId`.
        require(!paused, "Sale is paused !");
        require (quantity + balanceOf(msg.sender) <= maxByWallet, "Request too much for one wallet !"); 
        require (quantity + totalSupply() <= maxSupplyForSale, "Max supply reached !");
     ...
     * 
     */
    function setMerkleRootWhitelist(bytes32 newMerkleRoot) external onlyOwner {
