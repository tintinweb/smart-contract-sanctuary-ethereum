...
contract NFTLP is ERC1155, Ownable, ReentrancyGuard {
    ...
    uint256 internal categoriesLeft = NUM_CATEGORIES;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < NUM_CATEGORIES; i++) {
            state |= ((i << 0) | (supplyPerCategory << 3)) << (MASK_SIZE * i);
        }
    }

    modifier withSupply() {
        require(categoriesLeft > 0, "NFTLP: No more tokens");
        _;
    }
    ...
    function leftToTake() public view returns (uint256) {
        return
            (((state >> (MASK_SIZE * 0)) & MASK) >> 3) +
            (((state >> (MASK_SIZE * 1)) & MASK) >> 3) +
            (((state >> (MASK_SIZE * 2)) & MASK) >> 3) +
            (((state >> (MASK_SIZE * 3)) & MASK) >> 3) +
            (((state >> (MASK_SIZE * 4)) & MASK) >> 3);
    }
    ...
    function _mintRandom(address to) internal {
        ...
        uint256 chunk = (state >> (MASK_SIZE * randomIndex)) & MASK;
