...
interface ISetValuer {
  function calculateSetTokenValuation(IDomani _setToken, address _quoteAsset)
    external
    view
    returns (uint256);
}
...
abstract contract ModuleBase is IModule {
  ...
  modifier onlyManagerAndValidSet(IDomani _setToken) {
    _validateOnlyManagerAndValidSet(_setToken);
    _;
  }

  modifier onlySetManager(IDomani _setToken, address _caller) {
    _validateOnlySetManager(_setToken, _caller);
    _;
  }

  modifier onlyValidAndInitializedSet(IDomani _setToken) {
   ...
   */
  modifier onlyModule(IDomani _setToken) {
   ...
   */
  modifier onlyValidAndPendingSet(IDomani _setToken) {
   ...
   */
  function isSetPendingInitialization(IDomani _setToken) internal view returns (bool) {
   ...
   */
  function isSetManager(IDomani _setToken, address _toCheck) internal view returns (bool) {
   ...
   */
  function isSetValidAndInitialized(IDomani _setToken) internal view returns (bool) {
   ...
   */
  function _validateOnlyManagerAndValidSet(IDomani _setToken) internal view {
   ...
   */
  function _validateOnlySetManager(IDomani _setToken, address _caller) internal view {
   ...
   */
  function _validateOnlyValidAndInitializedSet(IDomani _setToken) internal view {
    require(isSetValidAndInitialized(_setToken), "Must be a valid and initialized SetToken");
  }
   ...
   */
  function _validateOnlyModule(IDomani _setToken) internal view {
    require(
      _setToken.moduleStates(msg.sender) == IDomani.ModuleState.INITIALIZED,
   ...
   */
  function _validateOnlyValidAndPendingSet(IDomani _setToken) internal view {
...
library Position {
   ...
   */
  function hasDefaultPosition(IDomani _setToken, address _component) internal view returns (bool) {
   ...
   */
  function hasExternalPosition(IDomani _setToken, address _component) internal view returns (bool) {
   ...
   */
  function getDefaultTrackedBalance(IDomani _setToken, address _component)
    internal
    view
    returns (uint256)
  {
 ...
 * not blocked, after the proposal period, the fund manager can transition the fund to the trading
...
contract DextfTradeModule is ModuleBase, ReentrancyGuard, AccessControl {
  ...
  struct ProposedTrade {
    ...
    // The list of trade components that will be sent to the fund in a trade
    TradeComponent[] inboundTradeComponents;
    // The list of trade components that will be sent to the market maker in a trade
  ...
  struct TradeProposal {
    address[] inboundAddresses;
    uint256[] inboundRealUnitsArray;
    address[] outboundAddresses;
    uint256[] outboundRealUnitsArray;
  }
   ...
   * It can be use both to cancel a proposal or during the trading period.
   *
   * @param _fund   Address of the fund to be transitioned to the regular state
   */
  function revertToRegularState(IDomani _fund) external onlyManagerAndValidSet(_fund) {
   ...
   * from the regular state to the proposal state. There are only few checks on the proposed trade
   * as most checks are left to the approvers and blockers.
   *
   * @param _fund                    Address of the fund subject of the trade
   * @param _proposalConstraints     The constraints for this proposal
   * @param _tradeProposal           It contains : the component addresses entering the fund,
   *                                               the component addresses exiting the fund,
   *                                               the value of the incoming tokens per fund token, in real units,
   *                                               the value of the outgoing tokens per fund token, in real units
   */

  function proposeTrade(
    IDomani _fund,
    ProposalConstraints calldata _proposalConstraints,
    TradeProposal calldata _tradeProposal
    ...
    require(_tradeProposal.inboundAddresses.length > 0, "Inbound addresses cannot be empty");
    require(_tradeProposal.outboundAddresses.length > 0, "Outbound addresses cannot be empty");

    // Check for vector consistency
    require(
      _tradeProposal.inboundRealUnitsArray.length == _tradeProposal.inboundAddresses.length,
      "Mismatch inbound lenghts"
    );
    require(
      _tradeProposal.outboundAddresses.length == _tradeProposal.outboundRealUnitsArray.length,
      "Mismatch outbound lenghts"
    );

    // Make sure there is no null address in either inbound or outbound components
    require(
      !_tradeProposal.inboundAddresses.contains(address(0)),
      "Null address in inbound componets"
    );
    require(
      !_tradeProposal.outboundAddresses.contains(address(0)),
      "Null address in outbound componets"
    );

    // Check that there are no duplicates in the component addresses
    require(
      !_tradeProposal.inboundAddresses.extend(_tradeProposal.outboundAddresses).hasDuplicate(),
    ...
    // Check that fund supply is bigger than 0
    require(fundTotalSupply > 0, "Fund supply must be positive");

    // Keep track of the maxium tokens to be traded
    proposalDetails[_fund].maxTradedFundTokens = fundTotalSupply;
    ...
    for (uint256 i = 0; i < _tradeProposal.inboundAddresses.length; i++) {
      proposalDetails[_fund].inboundTradeComponents.push(
        TradeComponent({
          componentAddress: _tradeProposal.inboundAddresses[i],
          tradeRealUnits: _tradeProposal.inboundRealUnitsArray[i]
        })
      );
    }
    ...
    for (uint256 i = 0; i < _tradeProposal.outboundAddresses.length; i++) {
      proposalDetails[_fund].outboundTradeComponents.push(
        TradeComponent({
          componentAddress: _tradeProposal.outboundAddresses[i],
          tradeRealUnits: _tradeProposal.outboundRealUnitsArray[i]
        })
      );
    }

    // Check that outboud components are compatible with the current fund holdings
    _checkProposalOutboundComponents(_fund, fundTotalSupply);
    ...
    emit TradeProposed(
      ...
      _tradeProposal.inboundAddresses.length,
      _tradeProposal.outboundAddresses.length
    );
  }

  /**
   * ONLY FUND MANAGER: Transition the fund from the proposal state to the trading state if all
   * constraints are satisfied: the minimum proposal time has elapsed, there are enough approval votes
   * and there are not too many blocker votes.
   *
   * @param _fund             Address of the fund for which trading can start
   */
  function startTrading(IDomani _fund) external onlyManagerAndValidSet(_fund) {
    ...
    onlyRoleOrOpenRole(MARKET_MAKER_ROLE)
  {
    ...
    // Check that we have enough tokens for the swap
    _checkTradeOutboundComponents(
      _fund,
      fundTotalSupply,
      scaledOutboundComponents,
  ...
  /**
   * Makes sure that the proposed outbound components do not exceed the current positions
   *
   * @param _fund    Address of the fund subject of the trade
   * @param _totalSupply   Total supply of the fund
   */
  function _checkProposalOutboundComponents(IDomani _fund, uint256 _totalSupply) internal view {
    TradeComponent[] memory outboundTradeComponents = proposalDetails[_fund]
    .outboundTradeComponents;
    uint256[] memory preTradeOutboundBalances = _computePreTradeBalances(
      _fund,
      outboundTradeComponents
    );
    for (uint256 i = 0; i < proposalDetails[_fund].outboundTradeComponents.length; i++) {
      address tradeComponentAddress = outboundTradeComponents[i].componentAddress;
      if (_fund.isComponent(tradeComponentAddress)) {
        uint256 outboundComponentRealUnits = outboundTradeComponents[i].tradeRealUnits;
        ...
        require(
          ...
          "Insufficient holdings for outbound component"
        );
        require(
          outboundComponentRealUnits.preciseMul(_totalSupply) <= preTradeOutboundBalances[i],
  ...
  /**
   * Makes sure that the trade-requested outbound components do not exceed the current positions
   *
   * @param _fund    Address of the fund subject of the trade
   */
  function _checkTradeOutboundComponents(
    IDomani _fund,
    uint256 _fundTotalSupply,
    TradeComponent[] memory _scaledOutboundComponents,
    uint256[] memory preTradeOutboundBalances
  ) internal view {
    // Transfer the outbound components
    for (uint256 i = 0; i < _scaledOutboundComponents.length; i++) {
      // Compute the available component for the given quantity
      uint256 unit = _fund
      .getDefaultPositionRealUnit(_scaledOutboundComponents[i].componentAddress)
      .toUint256();
      uint256 availableQuantity = _fundTotalSupply.preciseMul(unit);
      uint256 tradeRealUnits = _scaledOutboundComponents[i].tradeRealUnits;

      // Check that we have enough tokens to perform the outbound trade
      require(tradeRealUnits <= availableQuantity, "Insufficient holdings for trade");
      // Check that the tokens are actually in the vault
      require(tradeRealUnits <= preTradeOutboundBalances[i], "Insufficient balance for trade");
    }
  }
