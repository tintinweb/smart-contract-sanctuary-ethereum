...
import {IAllowanceTransfer} from "./interfaces/uniswap/permit2/IAllowanceTransfer.sol";
...
contract SkytellerSweeper is ISkytellerSweepDelegate, BaseOwnable, Initializable, Multicall {
    ...
    event SetUniswapRouter(address indexed universalRouter);
    ...
    {
        uint256 destinationMinimum = (BIPS_PRECISION - skytellerSwapFee)
            * minimumAmountOut(address(tokenIn), address(tokenOut), amountIn) / BIPS_PRECISION;
        ...
        inputs[0] = abi.encode(
            universalRouter, amountIn, 1, _routerPath(address(tokenIn), address(tokenOut)), true
        );
        inputs[1] = abi.encode(tokenOut, skytellerSwapFeeTreasury, uint256(skytellerSwapFee));
        inputs[2] = abi.encode(tokenOut, destination, destinationMinimum);

        uint256 treasuryBalanceBefore = tokenOut.balanceOf(skytellerSwapFeeTreasury);
        uint256 destinationBalanceBefore = tokenOut.balanceOf(destination);

        universalRouter.execute(commands, inputs, block.timestamp);

        amountOut = tokenOut.balanceOf(destination) - destinationBalanceBefore;
        emit Sweep(
            address(tokenIn),
            address(tokenOut),
            destination,
            amountIn,
            amountOut,
            tokenOut.balanceOf(skytellerSwapFeeTreasury) - treasuryBalanceBefore
        );
    }
    ...
    /// @param _universalRouter The address of the Uniswap V3 router
    function setUniswapRouter(address _universalRouter) external onlyOwner {
        universalRouter = IUniversalRouter(_universalRouter);
        emit SetUniswapRouter(_universalRouter);
    }
    ...
    function removeToken(address _token) external onlyOwner {
        IERC20(_token).approve(permit2, 0);
    ...
    function _addToken(address _token) internal {
        if (!_tokens.contains(_token)) {
            IERC20(_token).safeIncreaseAllowance(
                permit2, type(uint256).max - IERC20(_token).allowance(address(this), permit2)
            );
            IAllowanceTransfer(permit2).approve(
                _token, address(universalRouter), type(uint160).max, type(uint48).max
            );
            _tokens.add(_token);
        }
    }
