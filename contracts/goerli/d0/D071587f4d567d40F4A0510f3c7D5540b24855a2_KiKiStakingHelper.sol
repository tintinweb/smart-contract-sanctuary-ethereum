...
contract KiKiStakingHelper is IKiKiStakingHelper, RoleManager {
    ...
    bytes32 constant public SWAP_POOL_ROLE = keccak256("SWAP_POOL_ROLE");

    bytes32 internal constant KIKISTAKING_POSITION = keccak256("KiKiStaking.KiKiStakingHelper.kikiStaking");
    
    /// @dev Swap buffer percent, based on 10000 point
    bytes32 internal constant SWAP_BUFFER_PERCENT_POSITION = keccak256("KiKiStaking.KiKiStakingHelper.bufferPercent");
    /// @dev Swap min fee, based on 10000 point
    bytes32 internal constant SWAP_MIN_FEE_POSITION = keccak256("KiKiStaking.KiKiStakingHelper.swapMinFee");
    /// @dev Swap max fee, based on 10000 point
    bytes32 internal constant SWAP_MAX_FEE_POSITION = keccak256("KiKiStaking.KiKiStakingHelper.swapMaxFee");
    /// @dev APR points, based on 10000 point
    bytes32 internal constant APR_POSITION = keccak256("KiKiStaking.KiKiStakingHelper.APR");
    /// @dev Last update APR total pooled ether
    bytes32 internal constant TOTAL_POOLED_ETHER_POSITION = keccak256("KiKiStaking.KiKiStakingHelper.totalPooledEther");
    /// @dev Last update APR block time
    bytes32 internal constant LAST_APR_TIME_POSITION = keccak256("KiKiStaking.KiKiStakingHelper.lastAPRTime");
    ...
    /**
      * @notice Set swap buffer percent to `points`, need manager role
      * @param points Percent points, base on 10000
      */
    function setSwapBufferPercent(uint16 points) external auth(SWAP_POOL_ROLE) {
    ...
    /**
      ...
      */
    function setSwapFeePercent(uint16 minPoints, uint16 maxPoints) external auth(SWAP_POOL_ROLE) {
        require(minPoints <= maxPoints, "KiKiStakingHelper::setSwapFeePercent: need min points <= max points");
    ...
    function updateAPR() public onlyKiKiStaking {
        ...
        // require(_APR <= 10000, "KiKiStakingHelper::updateAPR: abnormal APR");
        _setAPR(_APR);
    ...
    /**
    * @notice Returns the swap buffer percent point, based on 10000
    */
    function getSwapBufferPercent() public view returns (uint16) {
    ...
    */
    function getSwapFeePointsByETH(uint256 etherAmount) public view returns (uint16) {
        address kikiStaking = KIKISTAKING_POSITION.getStorageAddress();
        uint256 bufferPoolLiquidity = IKiKiStaking(kikiStaking).getSwapBufferPoolLiquidity();
        ...
        if (maxFee == minFee) {
            ...
            uint256 totalPooledEther = IKiKiStaking(kikiStaking).getTotalPooledEther();
            uint256 swapPoolSize = totalPooledEther.mul(getSwapBufferPercent()).div(10000);
            if (0 == swapPoolSize)
                return 0;
            uint256 swapPoolLiquidity = IKiKiStaking(kikiStaking).getSwapBufferPoolLiquidity();
    ...
    function getNewAPR(uint256 newTotalPooledEther, uint256 timeElapsed) public view returns (uint256) {
        ...
        require(lastTotalPooledEther > 0, "KiKiStakingHelper::getNewAPR: zero lastTotalPooledEther");
        return newTotalPooledEther.sub(lastTotalPooledEther).mul(365 days).div(lastTotalPooledEther).mul(10000).div(timeElapsed);
    }
...
interface IKiKiStakingHelper {
    function setSwapBufferPercent(uint16 points) external;
    ...
    function getSwapBufferPercent() external view returns (uint16);
...
interface IKiKiStaking {
    ...
    /**
      ...
      */
    function getSwapBufferPoolLiquidity() external view returns (uint256);
