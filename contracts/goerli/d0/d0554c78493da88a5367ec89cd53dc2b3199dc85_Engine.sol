...
contract Engine is ERC721, IEngine {
    ...
    function open(
        ...
        require(collateral > 0, "engine/zero-collateral");
    ...
    function pick(
        ...
        orderId = opens.length;
        opens.push(Order(msg.sender, matchId, amount, collateral, false));
    ...
    function join(Order memory order) internal {
        ...
        if (balances[order.owner] < order.collateral) {
            emit LowTraderCollateral(
                order.matchid,
                order.owner,
                balances[order.owner],
                order.collateral
            );
            return;
        }
        if (balances[maker] < m.collateralM) {
