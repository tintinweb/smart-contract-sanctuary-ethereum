...
contract Casino is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        require(
            CasinoToken.allowance(_msgSender(), address(this)) >= _amount,
            "Transfer Amount Exceeds Allowance"
        );
        require(
            CasinoToken.balanceOf(_msgSender()) >= _amount,
            "Not Enough Funds In Wallet"
        );
        CasinoToken.transferFrom(_msgSender(), address(this), _amount);
    ...
    function withdraw(
        ...
        require(
            CasinoToken.balanceOf(address(this)) >= _amount,
            "Not Enough Funds In Casino Contract"
        );
        Player storage player = _players[_msgSender()];
        require(player.claimed.length == index, "");
        player.claimed.push(_amount);
        emit Withdraw(_msgSender(), address(CasinoToken), _amount);
    }

    function getDepositedList(address _player) public view returns (uint256[] memory) {
        return _players[_player].deposited;
    }

    function getClaimedList(address _player) public view returns (uint256[] memory) {
