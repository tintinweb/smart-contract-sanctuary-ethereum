...
contract GhoPay is ReentrancyGuard {
  ...
  // constructor(address _token, address _SentibleRouterAddress) {
  constructor() {
    owner = msg.sender;
    token = 0xcbE9771eD31e761b744D3cB9eF78A1f32DD99211;
    SentibleRouterAddress = 0x45bD63c9C338c39ea22dAC788Cf0f7dE119c3949;
    SentibleRouter = ISentibleRouterV1(0x45bD63c9C338c39ea22dAC788Cf0f7dE119c3949);
    GhoToken = IERC20(0xcbE9771eD31e761b744D3cB9eF78A1f32DD99211);
    GhoToken.approve(0x45bD63c9C338c39ea22dAC788Cf0f7dE119c3949, type(uint256).max);
  }
  ...
  function depositToRouter(uint256 _amount) external nonReentrant {
    address userAddress = msg.sender;
    require(GhoToken.allowance(userAddress, address(this)) >= _amount && _amount > 0, "Lender: insufficient allowance and amount must be greater than zero");
    GhoToken.safeTransferFrom(userAddress, address(this), _amount);
    SentibleRouter.deposit(address(GhoToken), _amount, userAddress);

    emit Deposit(userAddress, _amount);
  }

  function withdraw(uint256 _amount) external nonReentrant {
    require(_amount > 0, "Lender: amount must be greater than zero");
    SentibleRouter.withdraw(token, _amount, address(this));
    GhoToken.safeTransferFrom(address(this), msg.sender, _amount);

    emit Withdraw(msg.sender, _amount);
  }

  function setGhoToken(address _token) external onlyOwner {
    require(_token != address(0), "Lender: token address is zero");
    token = _token;
    GhoToken = IERC20(_token);
    GhoToken.approve(SentibleRouterAddress, type(uint256).max);
  }
  ...
  function setRouterAddress(address _SentibleRouterAddress) external onlyOwner {
    ...
    GhoToken.approve(_SentibleRouterAddress, type(uint256).max);
  }
 ...
 // Swaps ERC20->ERC20 tokens held by this contract using a 0x-API quote.
  function _fillQuote(
    ...
    uint256 initialSellTokenBalance = sellToken.balanceOf(contractAddress);
    ...
    // Ensure that the contract has sufficient balance of the sellToken before initiating the swap.
    uint256 receivedSellTokenAmount = sellToken.balanceOf(contractAddress) - initialSellTokenBalance;
    require(receivedSellTokenAmount == sellAmount, "Sell token not received from user");
  ...
  function fillQuote(
    ...
    buyToken.safeTransfer(msg.sender, buyTokenReceived);
  }

  // Swaps ERC20->ERC20 tokens held by this contract using a 0x-API quote and deposits the received tokens to SentibleRouter.
  function fillQuoteAndDeposit(
    ...
    SentibleRouter.deposit(address(buyToken), buyTokenReceived, msg.sender);

    emit Deposit(msg.sender, buyTokenReceived);
  }
