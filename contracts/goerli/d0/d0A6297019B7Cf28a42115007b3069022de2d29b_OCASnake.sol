...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    function snakeNameCheck(uint256 tokenId) public view returns (uint256) {
        uint256 snakeNameIndex = (generateAppleSeed(tokenId, 0) %
            snakes.length) + 1;
        uint256 adjectiveIndex = (generateAppleSeed(tokenId, 69) %
            wackyAdjectives.length) + 2;
        uint256 supremeIndex = (generateAppleSeed(tokenId, 420) %
            supremeAdjectives.length) + 1;
        uint256 combineIndex = snakeNameIndex * adjectiveIndex * supremeIndex;

        if (snakeNameIndexes[combineIndex].snakeName == 0) {
            return combineIndex;
        } else {
            return snakeNameCheck(tokenId * 2);
        }
    }
    ...
    function _safeMint(
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            tokenInfoList[startTokenId] = TokenInfo(
                block.timestamp,
                snakeNameSave(startTokenId + block.timestamp)
            );
...
contract username_list is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => NameInformation) public snakeNameIndexes;
