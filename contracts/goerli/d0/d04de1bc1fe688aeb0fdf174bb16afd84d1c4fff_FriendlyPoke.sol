   ...
   function executeOperation(address _reserve, uint256 _amount, uint256 _fee, bytes calldata _params) override external 
    {
        ...
        // lend 4000
        IERC20(_reserve).approve(core, 5000);   
        IUnilendV2Core(core).lend(pool, 4000);
        ...
        (, uint sharetoken1) = IUnilendV2Core(core).shareOfUserToken1(pool, address(this));
        IUnilendV2Core(core).redeem(pool, int(sharetoken1), address(this));
