...
// Phillip Plein's sunglasses core contract
...
contract PleinSunglasses721 is ERC721, Ownable {

    uint64 private constant MAX_SILVER_TOKEN_SUPPLY = 500;
    uint64 private constant MAX_PURPLE_TOKEN_SUPPLY = 1000;
    ...
    */
    function mintSilverToken(address to, uint256 numOfTokens) external onlyOwner() {
        if (to == address(0)) revert CannotMintToZeroAddress();
        if(numOfTokens + nextSilverTokenId > MAX_SILVER_TOKEN_SUPPLY )
            revert TokenOutOfSupply();
        
        uint64 id = nextSilverTokenId;
        ...
        nextSilverTokenId = id;
    }

    /**
    *   Mint purple token function for contract owner.
    */
    function mintPurpleToken(address to, uint256 numOfTokens) external onlyOwner() {
        if (to == address(0)) revert CannotMintToZeroAddress();
        if(numOfTokens + nextPurpleTokenId > MAX_PURPLE_TOKEN_SUPPLY )
            revert TokenOutOfSupply();
        
        uint64 id = nextPurpleTokenId;
