...
contract Facilitator is IFacilitator, Ownable, Pausable, ReentrancyGuard {
        ...
        whenNotPaused
    {
        ...
        listedNFTs[nft] = true;
        listings[nft] = ListingDetails({ basePrices: basePrices, maxTokenIds: maxTokenIdForSeries, series: series, isOpenForPurchase: true });
    ...
    function purchaseNFT(address nft, address receiver) external payable nonReentrant whenNotPaused {
        if (!listedNFTs[nft]) {
            revert NFTNotListed();
        }
        ...
        if (msg.value < _details.basePrices[0]) {
        ...
        uint256 mintFee = _details.basePrices[0];
        // Getting royalty information
        (address rRecv, uint256 rAmt) = I_ERC2981(nft).royaltyInfo(tokenId, _details.basePrices[0]);

        if ((rRecv != address(0) && rAmt != uint256(0)) && rAmt < _details.basePrices[0]) {
            payable(rRecv).transfer(rAmt);
            mintFee = _details.basePrices[0] - rAmt;
        }
        ...
        uint256 remainingBalance = msg.value - _details.basePrices[0];
...
interface IFacilitator {
    ...
    /// @param nft Address of the NFT which need to get purcahse.
    /// @param receiver Address of the receiver.
    function purchaseNFT(address nft, address receiver) external payable;
}
