...
contract PrivateRound{
    ...
    */
    ...
    MilestoneSetup[] memory _mile) public {
        ...
        fundsRequested[_roundId][_investor] = _fundRequested;
        initialPercentage[_roundId][_investor] = _initialPercentage;
        tokenExpected[_roundId][_investor] = _tokenContract;
    }
    ...
    */
    ...
    function tokenStatus(uint _roundId, address _founder, address _investor) public view returns(uint unlockedAmount, uint lockedAmount, uint withdrawnTokensByFounder){
        ...
        for(uint i = 0; i < _milestone[_investor][_roundId].length; i++){
            return (_milestone[_investor][_roundId][i]._num,
            _milestone[_investor][_roundId][i]._num,_milestone[_investor][_roundId][i]._num);
            // if(milestoneApprovalStatus[_roundId][_milestone[_investor][_roundId][i]._num] == 1 && milestoneWithdrawalStatus[_roundId][_milestone[_investor][_roundId][i]._num] == false){
            //     unlockedTokens = remainingTokensOfInvestor[_roundId][_investor] * (_milestone[_investor][_roundId][i]._percent / 100);
            //     if(initialWithdrawalStatus[_roundId][_founder] != true){
            //         unlockedTokens += initialTokensForFounder[_roundId][msg.sender];
            //     }
            // } else if(milestoneApprovalStatus[_roundId][_milestone[_investor][_roundId][i]._num] == 1 && milestoneWithdrawalStatus[_roundId][_milestone[_investor][_roundId][i]._num] == true){
            //     withdrawnTokens = remainingTokensOfInvestor[_roundId][_investor] * (_milestone[_investor][_roundId][i]._percent / 100);
            //     if(initialWithdrawalStatus[_roundId][_founder] == true){
            //         withdrawnTokens += initialTokensForFounder[_roundId][msg.sender];
            //     }
            // } else if(milestoneApprovalStatus[_roundId][_milestone[_investor][_roundId][i]._num] != 1){
            //     lockedTokens = remainingTokensOfInvestor[_roundId][_investor] * (_milestone[_investor][_roundId][i]._percent / 100);
            // }
        }
        // return(
        //     unlockedTokens,
