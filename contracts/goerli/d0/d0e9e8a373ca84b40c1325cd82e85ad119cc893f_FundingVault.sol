...
contract FundingVault is AccessControl {
  ...
  function getTotalPledgeTime() public view returns (int128) {
    (, int128 totalPledgeTime) = _getDistributionState(_intervalTime());
  ...
  function _refreshPledgeTimeCache() internal {
    ...
    if(refreshDuration > 0) {
      ...
      _pledgeTimeCache.totalPledgeTime += int64(_pledgeTimeCache.totalSeverity * refreshDuration);
    }
  }

  function _getDistributionState(uint64 currentTime) internal view returns (uint256, int128) {
    ...
    int128 totalPledgeTime = _pledgeTimeCache.totalPledgeTime + int64(cacheAge * _pledgeTimeCache.totalSeverity);

    return (distributionBalance, totalPledgeTime);
  }

  function _calculatePledgePledgeTime(uint256 distributionBalance, int128 totalPledgeTime, uint64 intervalTime, uint32 pledgeId) internal view returns (uint64, uint64, uint256) {
    ...
    uint256 claimBalance = distributionBalance * (usePledgeTime * pledge.severity) / uint128(totalPledgeTime);
  ...
  function _claim(uint32 pledgeId, uint256 amount, address target) internal returns (uint256) {
    ...
    uint64 intervalTime = _intervalTime();
    (uint256 distributionBalance, int128 totalPledgeTime) = _getDistributionState(intervalTime);
