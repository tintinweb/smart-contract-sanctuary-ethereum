...
contract PixiaAiNFT is ERC721A, Initializable {
    ...
    function tokenURIPakFat(bytes calldata data_) public pure returns (string memory) {
        return string(data_);
    }

    function tokenURIFat(uint256 tokenId) public view returns (bytes memory) {
        require(
            _exists(tokenId),
    ...
    function TEST_BASE_URI(uint256 tokenId) external view returns (string memory){
        return BASE_URI;
    }
