...
contract Staking is EIP712 {
    ...
    IERC721A private _mainContract = IERC721A(0xCC1a0D61770Ea3516F3b03aC061ee9451f3688aC);
    IERC20 golds = IERC20(0xDf5EC40DE46e6DFEd1C44C4426834d715b46ed5e);
    ...
    constructor() EIP712("Staking", "1.0.0") {
    }
    ...
    function withdrawGolds(uint amount, bytes calldata signature) external {
        ...
        golds.transfer(msg.sender, amount);
    ...
    function _hash(address account, uint amount, uint nonce) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("Staking(address account,uint256 amount,uint256 nonce)"),
                        account,
                        amount,
                        nonce
                    )
