...
contract CustomNFT is ERC721, Ownable {
    ...
    function mintBatch(uint256 batchSize, string[] memory hashes) external onlyOwner {
        require(currentTokenId <= MAX_TOKENS, 'All NFTs have been minted');
        require(batchSize == hashes.length, 'batchSize and hashes length should be the same.');

        for (uint i = 0; i < batchSize; i++) {
            if (currentTokenId < MAX_TOKENS) {
                _mint(msg.sender, currentTokenId);
                setTokenHash(currentTokenId, hashes[i]);
                currentTokenId++;
            } else {
                break;
            }
    ...
    function setTokenHash(uint256 _tokenId, string memory _hash) internal {
