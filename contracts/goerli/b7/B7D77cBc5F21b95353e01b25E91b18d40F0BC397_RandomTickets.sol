...
contract RandomTickets is ERC721LazyMint {
    ERC1155Base public receipt;
    uint256 erc1155Id;
    mapping (address=>bool) canLazyMint;
    ...
    function authorizeLazyMint(address _minterAddress) external onlyOwner {
        canLazyMint[_minterAddress] = true;
    }

    function unAuthorizeLazyMint(address _minterAddress) external onlyOwner {
        canLazyMint[_minterAddress] = false;
    }

    function _canLazyMint() internal view virtual override returns (bool) {
        return canLazyMint[msg.sender];
    }
    ...
    {
        require(
            receipt.balanceOf(_claimer, erc1155Id) >= _quantity,
            "You don't own enough Receipt NFTs"
        );
    }

    function updateReceiptContract(address newReceiptAddress) external onlyOwner {
        receipt = ERC1155Base(newReceiptAddress);
    }

    function updateERC1155Id(uint256 newERC1155Id) external onlyOwner {
        erc1155Id = newERC1155Id;
    }
    ...
    function transferTokensOnClaim(address _receiver, uint256 _quantity)
        ...
        returns (uint256)
    {
        receipt.burn(_receiver, erc1155Id, _quantity);
