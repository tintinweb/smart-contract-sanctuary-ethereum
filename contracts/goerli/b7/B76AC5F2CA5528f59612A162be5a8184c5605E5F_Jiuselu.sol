...
contract Jiuselu is ERC721A, Ownable {
    using Address for address;
    using Strings for uint256;
    using ECDSA for bytes32;

    uint256 internal constant MAX_LIMIT = 2570;
    uint256 internal constant MAX_SALE_LIMIT = 2078;
    uint256 internal constant NORMAL_SALE_ONCE_LIMIT = 2;
    uint256 internal constant WL_ONCE_LIMIT = 2;
    ...
    address private ogContractAddress;
    bool private reveal;
    ...
    address private founderAddress;
    mapping(uint256 => bool) public ogTokenMinted;
    mapping(address => uint256) public numWLMinted;
    event SetOGContract(address ogContract);
    event StartWLMint(uint256 amt);
    event StartMint(uint256 amt);
    ...
    constructor(
        ...
        wlPrice = 0.0735 ether;
        normalPrice = 0.09 ether;
        proxyRegistryAddress = _proxyRegistryAddress;
        startedWLMint = false;
        startedMint = false;
    }
    ...
    function setFounderAddress(address _founderAddress) public onlyOwner {
        require(_founderAddress != address(0), "Jiuselu: zero address");
        founderAddress = _founderAddress;
    }

    function setOGContract(address _ogcontract) public onlyOwner {
        ogContractAddress = _ogcontract;
        emit SetOGContract(_ogcontract);
    }
    ...
    function startWLMint(uint256 _mintAmt) public onlyOwner {
        ...
        require(
            ...
            "Jiuselu: reach maximum sale limit!"
        );
        startedWLMint = true;
        allowedWLAmt = _mintAmt;
        emit StartWLMint(_mintAmt);
    }
    ...
    function checkWL(address _sender, bytes calldata _signature)
        public
        view
        returns (bool)
    {
    ...
    function freemint(uint256 num, uint256[] calldata _og_token_ids)
        public
        payable
    {
        require(
            ogContractAddress != address(0),
            "Jiuselu: OG contract address needs to be set first!"
        );
        uint8 to_mint = 0;
        Apsaras aps_contract = Apsaras(ogContractAddress);
        // check if the sender owns APS and those tokens haven't been freeminted yet
        for (uint8 i = 0; i < _og_token_ids.length; i++) {
            address owner_address = aps_contract.ownerOf(_og_token_ids[i]);
            require(
                owner_address == _msgSender(),
                "Jiuselu: freemint sender doesn't own APS token"
            );
            require(
                ogTokenMinted[_og_token_ids[i]] == false,
                "Jiuselu: Aps token already free-minted"
            );
            to_mint++;
            ogTokenMinted[_og_token_ids[i]] = true;
        }
        require(
            num == to_mint,
            "Jiuselu: freemint num doesn't equal to the num allowed"
        );
        _mint(_msgSender(), num, "", false);
    }
    ...
    function wlMint(uint256 num, bytes calldata signature) public payable {
        require(
            mintCounter + num <= MAX_SALE_LIMIT,
            "Jiuselu: reached max sale limit."
        );
        uint256 _mintedAmt = _totalMinted();
        require(_mintedAmt + num <= MAX_LIMIT, "Jiuselu: reached max limit");
        require(startedWLMint, "Jiuselu: Whitelist mint not started");
        require(
            allowedWLAmt - num >= 0,
            "Jiuselu: reach maximum allowed amount during whitelist mint"
        );

        require(
            msg.value == num * wlPrice,
            "Jiuselu: whitelist mint price error, please check msg.value."
        );
        require(
            checkWL(_msgSender(), signature),
            "Jiuselu: Signer address mismatch."
        );
        require(
            _numberMinted(_msgSender()) + num <= WL_ONCE_LIMIT,
            "Jiuselu: WhiteList Account maximum mint allowance reached."
        );
        allowedWLAmt -= num;
        mintCounter += num;
        _mint(_msgSender(), num, "", false);
    ...
    function mint(uint256 num) public payable {
        ...
        uint256 _mintedAmt = _totalMinted();
        require(_mintedAmt + num <= MAX_LIMIT, "Jiuselu: reached max limit");
        require(startedMint, "Jiuselu: Whitelist mint not started");
        require(
        ...
        require(
            ...
            "Jiuselu: over maximum of NFT allowed per user"
        );
        require(
            msg.value >= num * normalPrice,
            "Jiuselu: price error, please check price."
        );
        // public sale limit 2 nft per user
        uint256 user_limit = NORMAL_SALE_ONCE_LIMIT + numWLMinted[_msgSender()];
        require(
            _numberMinted(_msgSender()) + num <= user_limit,
            "Apsaras: over maximum of NFT allowed per user"
        );
