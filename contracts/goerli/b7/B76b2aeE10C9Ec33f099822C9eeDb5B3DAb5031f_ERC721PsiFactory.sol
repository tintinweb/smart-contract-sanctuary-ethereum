...
import "../tokens/ERC721psi.sol";

contract ERC721PsiFactory {
	event ContractCreated (address contractAddress);

    function deploy(string memory name, string memory symbol, uint256 maxSupply) external returns(address) {
        ERC721PsiCore token = new ERC721PsiCore(name, symbol, maxSupply);
        address tokenAddress = address(token);

        emit ContractCreated(tokenAddress);
...
contract ERC721PsiCore is ERC721Psi, Ownable {
	...
	constructor(string memory _name, string memory _symbol, uint256 maxSupply) ERC721Psi(_name, _symbol) {
        MAX_SUPPLY = maxSupply;
	}

	function mint(uint256 _quantity) external payable {
	   require(((totalSupply() + _quantity) <= MAX_SUPPLY) || MAX_SUPPLY == 0, "Max supply reached");
	...
	function tokenURI(uint256 _index) public view override returns(string memory) {
		require(_exists(_index), "Index doesn't exist");

		return string(abi.encodePacked(prefix, baseURI, _index.toString()));
	}
}
