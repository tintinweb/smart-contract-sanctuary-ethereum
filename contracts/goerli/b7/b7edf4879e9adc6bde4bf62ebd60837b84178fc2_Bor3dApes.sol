...
contract Bor3dApes is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("Bor3d Ape Yacht Club", "Bor3dApes",69,10000)
    {
        baseURI = "ipfs://QmdHMA4xTcCDCQ5gRmPD6WaMPz4VnWzJLQAiE5Z3njJnjt/ ";
        MintStartEnabled = true;
    }

    function mint(uint256 qty) external payable
    {
        require(MintStartEnabled , "Ape Info:  Minting Public Pause");
        require(qty <= maxMintPerTx, "Ape Info:  Limit Per Transaction");
        require(totalSupply() + qty <= maxSupply,"Ape Info:  Soldout");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * MintPrice,"Ape Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * MintPrice,"Ape Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
