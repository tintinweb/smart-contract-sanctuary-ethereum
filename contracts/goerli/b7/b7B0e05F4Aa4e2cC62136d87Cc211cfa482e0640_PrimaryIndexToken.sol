...
contract PrimaryIndexToken is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    modifier onlyModeratOrContract() {
        require(msg.sender == address(this) || hasRole(MODERATOR_ROLE, msg.sender), "PIT: Caller is not the Moderator");
        _;
    }
    ...
    function deposit(address projectToken, uint256 projectTokenAmount, address user) public isProjectTokenListed(projectToken) nonReentrant() {
        ...
        calcDepositPosition(projectToken, projectTokenAmount, user);
        emit Deposit(user, projectToken, usdcToken, projectTokenAmount, user);
    }

    function calcAndTransferDepositPosition(address projectToken, uint256 projectTokenAmount, address user, address receiver) external isProjectTokenListed(projectToken) onlyModeratOrContract {
    ...
    function calcDepositPosition(address projectToken, uint256 projectTokenAmount, address user) public isProjectTokenListed(projectToken) onlyModeratOrContract() {
    ...
    function borrow(address projectToken, address lendingToken, uint256 lendingTokenAmount) public isProjectTokenListed(projectToken) isLendingTokenListed(lendingToken) {
        ...
        calcBorrowPosition(msg.sender, projectToken, lendingToken, lendingTokenAmount, _lendingToken);
    ...
    function calcBorrowPosition(address borrower, address projectToken, address lendingToken, uint256 lendingTokenAmount, address currentLendingToken) public {
