...
contract ControllerV1 is Ownable{
    ...
    function initialize() external{
        require(initialized == 0, "ControllerV1: initialized");
        _storageAddress = address(this);
    ...
    function setURI(Kind kind, address tokenAddress, string memory uri) external onlyOwner{
        if(kind == Kind.ERC721){
    ...
    function create(uint256 uuid, Kind kind, string memory name_, string memory symbol_, string memory uri_) external onlyOwner{
        address tokenAddress;
        if(Kind.ERC721 == kind){
    ...
    function mint(uint256 uuid, Token[] calldata tokens) external onlyOwner{
    ...
    function deposit(uint256 uuid, Token[] calldata tokens) external{
        for(uint i;i<tokens.length;i++){
            Token calldata token = tokens[i];
            if(token.kind == Kind.ERC721){
    ...
    function withdraw(uint256 uuid, address to, Token[] calldata tokens) external onlyOwner{
        for(uint i;i<tokens.length;i++){
            Token calldata token = tokens[i];
            if(token.kind == Kind.ERC721){
