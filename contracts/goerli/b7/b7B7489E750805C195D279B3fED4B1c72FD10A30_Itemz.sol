...
contract Itemz is ERC1155, Ownable {
    ...
    uint256 public itemCount = 31;

    uint256 public constant BOUNTY_OR_PERIL = 0;
    uint256 public constant BREAD_BOOSTER = 1;
    uint256 public constant FORTIFICATION = 6;
    uint256 public constant RECALL = 11;
    uint256 public constant FLASH = 16;
    uint256 public constant WITHER = 21;
    uint256 public constant HYPNOSIS = 26;
    ...
    function claim(uint256 _voucherId, address _address, uint256 _tokenId, uint256 _amount, bytes calldata _voucher) external {
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(_voucherId, _address, _tokenId, _amount)
        );
    ...
    function redeem(address _address, uint256 _tokenId, uint256 _amount) external {
        require(!claimPaused, "Claiming has been paused");
        require(msg.sender == _address, "Bad Address");
        _burn(_address, _tokenId, _amount);
    }
    ...
    function walletOfOwner(address _address) external view returns(uint256[] memory) {
        uint256[] memory wallet = new uint256[](itemCount);
        for (uint256 i = 0; i < itemCount; i++) {
            wallet[i] = balanceOf(_address, i);
        }
        return wallet;
    }
}
