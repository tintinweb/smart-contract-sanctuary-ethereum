 ...
 * @author Achthar
 */
contract MoneyMarketFacetGoerli is WithStorage, GoerliCompoundHandler {
    ...
    constructor(
        ...
        address _weth,
        address _router
    ) GoerliCompoundHandler(_factory, _weth, _router) {}
    ...
    function mint(address _underlying, uint256 _amountToSupply) external payable onlyOwner {
    ...
    function redeem(address _underlying, uint256 _cAmountToRedeem) external payable onlyOwner returns (uint256 errorCode) {
    ...
    function redeemEther(uint256 _cAmountToRedeem) external payable onlyOwner {
        cEther().redeem(_cAmountToRedeem);
    }

    function redeemUnderlyingEther(address payable _recipient, uint256 _amountToRedeem) external payable onlyOwner {
    ...
    function swapAndSupplyExactIn(ExactInputToSelfParams calldata uniswapParams) external onlyOwner {
        ...
        mintPrivate(uniswapParams.path.getLastToken(), amountToSupply);
    }

    function swapETHAndSupplyExactIn(ExactInputToSelfParams calldata uniswapParams) external payable onlyOwner {
        IWETH9 _weth = IWETH9(WETH9);
        // wrap eth
        ...
        returns (uint256 amountIn)
    {
        ...
        // deposit received amount to the lending protocol on behalf of user
        mintPrivate(tokenOut, amountOutReceived);
    }

    function swapETHAndSupplyExactOut(ExactOutputToSelfParams calldata _marginSwapParams) external payable onlyOwner returns (uint256 amountIn) {
        IWETH9 _weth = IWETH9(WETH9);
        _weth.deposit{value: msg.value}();
        _weth.approve(router, type(uint256).max);

        // use the swap router to swap exact out
        amountIn = IMinimalSwapRouter(router).exactOutputToSelf(_marginSwapParams);
        // deposit received amount to the lending protocol on behalf of user
        mintPrivate(_marginSwapParams.path.getFirstToken(), _marginSwapParams.amountOut);
    ...
    function withdrawAndSwapExactIn(ExactInputParams calldata uniswapParams) external onlyOwner returns (uint256 amountOut) {
        address tokenIn = uniswapParams.path.getFirstToken();
    ...
    function withdrawAndSwapExactInToETH(ExactInputToSelfParams calldata uniswapParams) external onlyOwner returns (uint256 amountOut) {
        address tokenIn = uniswapParams.path.getFirstToken();
        uint256 amountToWithdraw = uniswapParams.amountIn;
        // withraw and send funds to this address for swaps
        redeemPrivate(tokenIn, amountToWithdraw, address(this));
    ...
    function withdrawAndSwapExactOutToETH(MarginSwapParamsMultiExactOut calldata _marginSwapParams)
        external
        payable
        onlyOwner
        returns (uint256 amountIn)
    {
        (address tokenOut, address tokenIn, uint24 fee) = _marginSwapParams.path.decodeFirstPool();
        uint256 amountOut = _marginSwapParams.amountOut;
        MarginCallbackData memory data = MarginCallbackData({
            path: _marginSwapParams.path,
            tradeType: MarginSwapTradeType.UNISWAP_EXACT_OUT_WITHDRAW,
            user: msg.sender,
    ...
    function borrowAndSwapExactIn(ExactInputParams calldata uniswapParams) external onlyOwner returns (uint256 amountOut) {
        address tokenIn = uniswapParams.path.getFirstToken();
        // borrow and send funds to this address for swaps
        borrowPrivate(tokenIn, uniswapParams.amountIn, address(this));
        // approve minimal router
        TransferHelper.safeApprove(tokenIn, router, type(uint256).max);
    ...
    function borrowAndSwapExactInToETH(ExactInputToSelfParams calldata uniswapParams) external onlyOwner returns (uint256 amountOut) {
        address tokenIn = uniswapParams.path.getFirstToken();
        // borrow and send funds to this address for swaps
        borrowPrivate(tokenIn, uniswapParams.amountIn, address(this));
        // approve minimal router
    ...
    function borrowAndSwapExactOut(MarginSwapParamsMultiExactOut calldata _marginSwapParams) external onlyOwner returns (uint256 amountIn) {
        (address tokenOut, address tokenIn, uint24 fee) = _marginSwapParams.path.decodeFirstPool();
        MarginCallbackData memory data = MarginCallbackData({
            path: _marginSwapParams.path,
    ...
    function borrowAndSwapExactOutToETH(MarginSwapParamsMultiExactOut calldata _marginSwapParams) external onlyOwner returns (uint256 amountIn) {
        (address tokenOut, address tokenIn, uint24 fee) = _marginSwapParams.path.decodeFirstPool();
        MarginCallbackData memory data = MarginCallbackData({
            path: _marginSwapParams.path,
            tradeType: MarginSwapTradeType.UNISWAP_EXACT_OUT_BORROW,
        ...
        IWETH9(WETH9).withdraw(amountOutReceived);
    ...
    function swapAndRepayExactIn(ExactInputToSelfParams calldata uniswapParams) external onlyOwner returns (uint256 amountOut) {
        ...
        amountOut = IMinimalSwapRouter(router).exactInputToSelf(uniswapParams);
        // deposit received amount to the lending protocol on behalf of user
        repayPrivate(uniswapParams.path.getLastToken(), amountOut);
    }

    function swapETHAndRepayExactIn(ExactInputToSelfParams calldata uniswapParams) external payable onlyOwner returns (uint256 amountOut) {
        IWETH9 _weth = IWETH9(WETH9);
        // wrap eth
        _weth.deposit{value: msg.value}();
        _weth.approve(router, type(uint256).max);
    ...
    function swapAndRepayExactOut(MarginSwapParamsMultiExactOut calldata _marginSwapParams) external payable onlyOwner returns (uint256 amountIn) {
        (address tokenOut, address tokenIn, uint24 fee) = _marginSwapParams.path.decodeFirstPool();
        MarginCallbackData memory data = MarginCallbackData({
            path: _marginSwapParams.path,
            tradeType: MarginSwapTradeType.UNISWAP_EXACT_OUT,
    ...
    function swapETHAndRepayExactOut(ExactOutputToSelfParams calldata _marginSwapParams) external payable onlyOwner returns (uint256 amountIn) {
        IWETH9 _weth = IWETH9(WETH9);
        _weth.deposit{value: msg.value}();
        _weth.approve(router, type(uint256).max);
        ...
        // deposit received amount to the lending protocol on behalf of user
        repayPrivate(_marginSwapParams.path.getFirstToken(), _marginSwapParams.amountOut);
