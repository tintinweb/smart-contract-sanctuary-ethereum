...
contract VFTicket is ERC4907VF, VFTokenExtensions {
    ...
    function setUser(
        ...
        _userBalances[user] += 1;
        _userBalances[owner] -= 1;
...
contract ERC4907VF is ERC721VF, IERC4907VF {
    ...
    mapping(address => int256) internal _userBalances;
     ...
     */
    function userBalanceOf(address user) public view virtual returns (uint256) {
        if (user == address(0)) {
            revert ERC721VFAddressZeroIsNotAValidOwner();
        }

        return uint256(_userBalances[user]);
    }
    ...
    function tokensOfUserIn(
        ...
        uint256 tokenCount = balanceOf(user) + userBalanceOf(user);
        ...
        if (tokenCount == 0) {
            ...
            for (index; index <= endIndex; index++) {
                currentUserAddress = _users[index].user;
                currentOwnerAddress = _ownerOf(index);
                if (currentUserAddress == user || currentOwnerAddress == user) {
