...
contract Staker {
  ...
  event Stake(address indexed _from, uint _ammount);
  event Execute();
  event Withdraw(address indexed _from, uint _ammount);
  ...
  function execute() public {
    require(timeLeft() == 0, 'There is still time left');
    require(status == Status.Open, 'Status is not open');
    if (address(this).balance > threshold) {
      status = Status.Closed;
      exampleExternalContract.complete{value: address(this).balance}();
    } else {
      status = Status.WithdrawsOnly;
    }
    emit Execute();
  }
  ...
  function withdraw() public {
    require(status == Status.WithdrawsOnly);
    uint256 senderValueStored = balances[msg.sender];
    require(senderValueStored != 0);
    delete balances[msg.sender];
    (bool sent, ) = payable(msg.sender).call{value: senderValueStored}('');
    require(sent, 'Failed to send Ether');
    emit Withdraw(msg.sender, senderValueStored);
  }
