...
// File: contracts\Interface8\IMasterChefV2.sol
...
// File: contracts\Interface8\IBoostContract.sol
...
// File: contracts\Interface8\IVZinu.sol
...
// File: contracts\ZinuPool.sol


pragma solidity ^0.8.0;
contract ZinuPool is Ownable, Pausable {
    ...
    uint256 public constant MIN_DEPOSIT_AMOUNT = 10000; // 0.00001 ZINU
    uint256 public constant MIN_WITHDRAW_AMOUNT = 10000; // 0.00001 ZINU
    ...
    event NewVZinuContract(address VZinu);
     ...
     * @param _pid: zinu pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(zinuPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the zinu owner address or the operator address.
     */
    modifier onlyOperatorOrZinuOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or zinu owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onZinuPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    /**
     * @notice Unlock user zinu funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrZinuOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Zinu Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VZinu != address(0)) {
            IVZinu(VZinu).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.zinuAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Zinu Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VZinu != address(0)) {
            IVZinu(VZinu).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending ZINU tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingZinu = masterchefV2.pendingZinu(zinuPoolPID, address(this));
        if (pendingZinu > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(zinuPoolPID, 0);
    ...
    /**
     * @notice Set VZinu Contract address
     * @dev Callable by the contract admin.
     */
    function setVZinuContract(address _VZinu) external onlyAdmin {
        require(_VZinu != address(0), "Cannot be zero address");
        VZinu = _VZinu;
        emit NewVZinuContract(VZinu);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Zinu Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingZinuRewards();
     ...
     * @return Returns total pending zinu rewards
     */
    function calculateTotalPendingZinuRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingZinu(zinuPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? ZINU_PRECISION : (((balanceOf() + calculateTotalPendingZinuRewards()) * (ZINU_PRECISION)) / totalShares);
    }
