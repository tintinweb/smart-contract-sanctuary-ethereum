...
contract ExchangeHelperGovernedProxy is NonReentrant, IGovernedContract, IGovernedProxy_New {
    ...
    modifier senderOrigin {
        ...
        require (tx.origin == msg.sender, 'ExchangeHelperGovernedProxy: Only direct calls are allowed!');
        _;
    }
     ...
     */
    function proposeUpgrade(IGovernedContract _newImplementation, uint _period)
        external
        payable
        senderOrigin
        noReentry
    ...
    {
        require (_newImplementation != implementation, 'ExchangeHelperGovernedProxy: Already active!');
        require (_newImplementation.proxy() == address(this), 'ExchangeHelperGovernedProxy: Wrong proxy!');

        ISporkRegistry spork_reg = ISporkRegistry(address(spork_proxy.impl()));
        IUpgradeProposal proposal = spork_reg.createUpgradeProposal.value(msg.value)(_newImplementation, _period, msg.sender);
        ...
        noReentry
    {
        ...
        require (newImplementation != implementation, 'ExchangeHelperGovernedProxy: Already active!');
        // in case it changes in the flight
        require (address(newImplementation) != address(0), 'ExchangeHelperGovernedProxy: Not registered!');
