...
contract BeliFarm is Ownable, ReentrancyGuard {
    ...
    function validateFee(uint16 _harvestFee) internal pure {
        require(_harvestFee <= HARVEST_FEE_UPPER_LIMIT, "harvest fee exceeded");
    }

    function validateHarvestInterval(uint256 _harvestInterval) internal pure {
        require(_harvestInterval <= HARVEST_INTERVAL_UPPER_LIMIT, "harvest interval exceeded");
    }
    ...
    function setEmissionRate(uint256 _newRate) external onlyOwner returns (uint256) {
        require(_newRate >= BELI_PER_BLOCK_LOWER_LIMIT, 'too low');
        require(_newRate <= BELI_PER_BLOCK_UPPER_LIMIT, 'too big');
        if (BELIPerBlock >= BELI_PER_BLOCK_THRESHOLD) {
            require(_newRate >= BELI_PER_BLOCK_THRESHOLD, '< threshold');
        }
    ...
    function setFarmSettings(
        ...
        if (_lockerRate != lockerRate) {
            require(_lockerRate <= MAX_RATE, "locker rate exceeded");
            require(block.number <= lockerLockBlock, "too late");
    ...
    function add(
        ...
        onlyOwner
    {
        validateHarvestInterval(_harvestInterval);
        validateFee(_harvestFee);
        if (_isFeeReceiverReward == true) {
            require(
                _isLpReward == false,
                "fee reward is exclusive"
            );
        }
        if (_isLpReward == true) {
            require(
    ...
    function canHarvest(uint256 _pid, address _user) public view returns (bool) {
        return block.timestamp >= userInfo[_pid][_user].lockedUntil;
    }
    ...
    function harvestFee(uint256 _pid, uint256 _amount) public view returns (uint256) {
        if (block.timestamp >= userInfo[_pid][msg.sender].withdrawalFeeUntil) {
            return 0;
        }
        return _amount.mul(poolInfo[_pid].harvestFee).div(MAX_RATE);
    }

    function harvestFeeUntil(uint256 _pid) external view returns (uint256) {
        return userInfo[_pid][msg.sender].withdrawalFeeUntil;
    }
        ...
        returns (uint256)
    {
        ...
        return userInfo[_pid][_user].shares.mul(wantLockedTotal).div(sharesTotal);
    }

    function vaultWantLockedTotal(uint256 _pid) external view returns (uint256) {
        return IVault(poolInfo[_pid].vault).wantLockedTotal();
    }
    ...
    function transferConvertedEarn(
        ...
        if (userInfo[_pid][_user].shares > 0) {
            uint256 amount = _pendingConvertedEarn(_pid, _user);
            if (amount > 0) {
                IVault(poolInfo[_pid].vault).transferConvertedEarn(_user, amount);
            }
        }
    }
    ...
    function harvestStakerReward(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 pendingBeli = IERC20(pool.vault).totalSupply().mul(pool.accBELIPerShare).sub(pool.rewardDebt).div(1e12);
        uint256 pendingEarn = IERC20(pool.vault).totalSupply().mul(IVault(pool.vault).accEarnBELIPerShare()).sub(pool.earnDebt).div(1e12);

        uint256 feeReceiverAmount = pendingBeli.div(2);
        uint256 lpFeeReceiverAmount = pendingBeli.sub(feeReceiverAmount);
        IERC20(BELI).safeTransfer(address(feeReceiver), feeReceiverAmount);
        IERC20(BELI).safeTransfer(address(feeLpReceiver), lpFeeReceiverAmount);

        uint256 feeReceiverEarn = pendingEarn.div(2);
        uint256 lpFeeReceiverEarn = pendingEarn.sub(feeReceiverEarn);
        IVault(pool.vault).transferConvertedEarn(address(feeReceiver), feeReceiverEarn);
        IVault(pool.vault).transferConvertedEarn(address(feeLpReceiver), lpFeeReceiverEarn);

        pool.rewardDebt = IERC20(pool.vault).totalSupply().mul(pool.accBELIPerShare).div(1e12);
        pool.earnDebt = IERC20(pool.vault).totalSupply().mul(IVault(pool.vault).accEarnBELIPerShare()).div(1e12);
    }
    ...
    function payOrLockupPendingBELI(uint256 _pid, bool _isWithdrawal) internal {
        ...
        if (IERC20(pool.vault).totalSupply() > 0) {
            harvestStakerReward(_pid);
        }
    ...
    function deposit(
        ...
        if (_wantAmt > 0) {
            ...
            _wantAmt = pool.want.balanceOf(address(this)).sub(preBalance);
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        require(user.shares > 0, "0 share");
        require(sharesTotal > 0, "0 shareTotal");
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        // uint256 postBalance = pool.want.balanceOf(address(this));
        // postBalance - preBalance
        amount = pool.want.balanceOf(address(this)).sub(preBalance);
    ...
    function mintShareToReceipt(
        ...
        pool.rewardDebt = pool.rewardDebt.add(
            _amount.mul(pool.accBELIPerShare).div(1e12)
        );
        pool.earnDebt = pool.earnDebt.add(
    ...
    function burnReceiptForShare(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        require(
            IERC20(pool.vault).balanceOf(msg.sender) >= _amount,
        ...
        user.rewardFeeReceiverDebt = user.rewardFeeReceiverDebt.add(
            _amount.mul(pool.accBELIPerShareFromFees).div(1e12)
        );
        user.rewardEarnDebt = user.rewardEarnDebt.add(
            _amount.mul(IVault(pool.vault).accEarnBELIPerShare()).div(1e12)
        );
    }
}
