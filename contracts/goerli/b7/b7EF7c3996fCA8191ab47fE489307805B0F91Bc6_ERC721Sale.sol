...
// TODO
// - Verify withdrawal function

// - Gather correct metadata for ordinals
// - Add a provenance inscription for all Ordinals
// - Create generator for Ticket images
// - Figure out of to randomize tickets if we want to do that
// - IPFS/Arweave (could host in own API to start with)
// - Deploy to mainnet
...
// Modified version of Zora's IERC721Drop: https://github.com/ourzora/zora-drops-contracts/blob/main/src/interfaces/IERC721Drop.sol
// Supports an extra waitlist sale stage

/// @notice Interface for an ERC721 sale contract
interface IERC721Sale {
  ...
  /// @notice Waitlist sale is inactive
  error WaitlistSale_Inactive();
  /// @notice Presale merkle root is invalid
  error Presale_MerkleNotApproved();
  /// @notice Waitlist sale merkle root is invalid
  error WaitlistSale_MerkleNotApproved();
  ...
  /// @notice Too many waitlist sale for address
  error WaitlistSale_TooManyForAddress();
  ...
  struct SalesConfiguration {
    ...
    bytes32 presaleMerkleRoot;
    /// @notice Waitlist sale merkle root
    bytes32 waitlistMerkleRoot;
  }
  ...
  struct SaleDetails {
    ...
    uint64 presaleStart;
    uint64 presaleEnd;
    // Timed sale actions for waitlist
    uint64 waitlistSaleStart;
    uint64 waitlistSaleEnd;
  ...
  struct AddressMintDetails {
    ...
    /// Number of waitlist mints from the given address
    uint256 waitlistMints;
  ...
  /// @notice External purchase waitlist function (takes a merkle proof and matches to root) (payable in eth)
  /// @param quantity to purchase
  /// @param maxQuantity can purchase (verified by merkle root)
  /// @param pricePerToken price per token allowed (verified by merkle root)
  /// @param merkleProof input for merkle proof leaf verified by merkle root
