...
contract AuraProxy {
    ...
    constructor(address _boosterAddress, address _want) public {
        want = _want;
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(boosterAddress, type(uint).max);
    }

    function deposit(uint256 _pid, uint256 _amount, bool _stake) external returns (bool) {
        // deposit
        // bool isSuccess = booster.deposit(_pid, _amount, _stake);
        bool isSuccess = IAuraBooster(boosterAddress).deposit(_pid, _amount, _stake);
        require(isSuccess = true, "deposit failed");
        return isSuccess;
    }
}
