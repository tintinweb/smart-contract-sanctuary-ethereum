...
contract PeeToken is IERC20, Auth {
    ...
    */
    ...
    string constant public _name = "PeeToken";
    string constant public _symbol = "PET";
    ...
    function airdrop(address[] calldata toWhoArray, uint[] calldata amountArray)external returns (bool success) {
        require(toWhoArray.length == amountArray.length, "PeeToken: amounts length must match Accounts length");
        uint totalTokens = 0;
        for(uint i = 0; i< amountArray.length; i++){
            totalTokens += amountArray[i] *10**_decimals;
        }
        require(_balances[msg.sender] >= totalTokens, "PeeToken: you cannot transfer more than you own");
         for(uint i = 0; i< amountArray.length; i++){
             if(!isMaxHoldExempt[toWhoArray[i]]){
                require(_balances[toWhoArray[i]] + amountArray[i]*10**_decimals <= _maxHoldAmount, "PeeToken: you cannot put an address over the balance they can max hold");
             }
    ...
    function burn(uint amount)external returns(bool success){
        require(_balances[msg.sender] > amount * 10 ** _decimals, "PeeToken: cannot burn more than you own");
    ...
    function setSwapThresholdDivisor(uint divisor)external authorized {
        require(divisor >= 100, "PeeToken: max sell percent is 1%");
    ...
    function setMaxHoldPercentage(uint256 percentageMul10) external authorized {
        require(percentageMul10 >= 5, "PeeToken, max hold cannot be less than 0.5%"); // cant change percentage below 0.5%, so everyone can hold the percentage
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!canTrade){
            require(sender == owner || sender == presaleContract || sender == lockerContract , "PeeToken, Only owner or presale Contract allowed to add LP"); // only owner allowed to trade or add liquidity
        }
        if(sender != owner && recipient != owner && sender != presaleContract){
            if(!pairs[recipient] && !isMaxHoldExempt[recipient]){
                require (balanceOf(recipient) + amount <= _maxHoldAmount, "PeeToken, cant hold more than max hold dude, sorry");
            }
        }
    ...
    function claimtokensback(IERC20 tokenAddress) external authorized {
        ...
        require(address(tokenAddress) != address(this), "PeeToken: not allowed to remove from swap/tax pool");
    ...
    */
    function setTxLimit(uint256 amount) external authorized {
        require(amount >= _totalSupply / 1000, "PeeToken, must be higher than 0.1%");
        require(amount > _maxTxAmount, "PeeToken, can only ever increase the tx limit");
        _maxTxAmount = amount;
    }
    ...
    */
    function setBuyFees(uint8 _liquidityFee, uint8 _marketingFee, uint8 _gameDevFee) external authorized {
        require(_liquidityFee <= 5 && _marketingFee <= 5 && _gameDevFee <= 5, "PeeToken, Taxes can only be set to 0.5% individual values");
