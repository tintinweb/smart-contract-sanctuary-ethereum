...
contract BinaryDex is DefaultOperatorFilterer, ERC721A, ERC2981, Ownable {
    ...
    address public reserveAddress = 0x007b73b7e1Cf60022ba9300D2a4661e74289c7E0;
    address public withdrawalAddress = 0x251F8Ac455286e75EAFc7D48eeC537427C588096;
    struct Coupon {
        bytes32 r;
        bytes32 s;
        uint8 v;
    ...
    modifier isLocked() {
        require(phase != SalePhase.Locked, 'Mint phase currently locked');
        _;
    }

    constructor(string memory _baseUri, address _signer) ERC721A('BinTest', 'DAXTest') {
        baseTokenUri = _baseUri;
        adminSigner = _signer;
    }

    function tenTenMint(Coupon calldata coupon) external payable callerIsUser isLocked {
    ...
    function sixtySixtyMint(Coupon calldata coupon) external payable callerIsUser isLocked {
    ...
    function waitListMint(Coupon calldata coupon) external payable callerIsUser isLocked {
    ...
    function publicMint(uint256 quantity) external payable callerIsUser isLocked {
    ...
    function withdraw() external onlyOwner {
        ...
        (bool success, ) = payable(withdrawalAddress).call{value: address(this).balance}(new bytes(0)); 
    ...
    function getMintedLength() external view onlyOwner returns (uint256) {
