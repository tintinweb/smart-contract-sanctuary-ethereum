...
contract Bnpl is NftReceiver, Ownable2Step, ReentrancyGuard {
    ...
    /**
     * @notice The maximum percentage of the price that can be used as the loan principal, measured in basis points
     * (hundredths of a percent). The max allowed value is 10000, it is initialized as 70%.
     */
    uint256 public loanCapBps = 7000;
    ...
    event SetLoanCap(uint256 loanCap);
    event BnplExecuted(
    ...
    error Bnpl__setLoanCapBps_invalidLoanCapBps();

    error Bnpl__execute_invalidAssetType();
    error Bnpl__execute_invalidModule();
    error Bnpl__execute_invalidLoanPrincipalAmount();
    ...
    /**
     * @notice This function can be called by admins to change the loanCap value.
     *
     * @param _loanCapBps - The new loan cap bps.
     */
    function setLoanCapBps(uint256 _loanCapBps) external onlyOwner {
        if (!_isValidLoanCapBps(_loanCapBps)) revert Bnpl__setLoanCapBps_invalidLoanCapBps();
        loanCapBps = _loanCapBps;
    }
    ...
    function execute(Execution calldata _params) external nonReentrant returns (bool) {
        ...
        if (_params.offer.loanPrincipalAmount > getLoanCap(_params.totalPrice))
            revert Bnpl__execute_invalidLoanPrincipalAmount();
    ...
    /**
     * @notice Returns the chain id this contracts is deployed on.
     */
    ...
    function getLoanCap(uint256 _price) public view returns (uint256) {
        return _price > 0 ? (_price * loanCapBps) / HUNDRED_PERCENT : 0;
    }
    ...
    function _isValidLoanCapBps(uint256 _loanCapBps) internal pure returns(bool) {
        return _loanCapBps > 0 && _loanCapBps <= HUNDRED_PERCENT;
    }
}
...
interface IDirectLoanCoordinator {
    ...
    function totalNumLoans() external returns (uint32);
