...
contract AmericanaFactory is ReentrancyGuard {
    ...
    constructor(address payable _feeRecipient, uint8 _maxEscrowDurationInDays, uint8 _marketFee, address _minterAddress) payable {
...
contract AmericanaMinter is ERC721A("Americana", "AMERICANA"), LockRegistry {

    bytes4 private constant _INTERFACE_ID_LOCKABLE = 0xc1c8d4d6;

    modifier onlyAuthorized {
      require(isAuthorized[msg.sender], "AmericanaMinter: Msg.sender not authorized");
      _;
   }

   modifier onlyFactory {
      require(msg.sender == factoryAddress, "AmericanaMinter: Call from non factory address");
      _;
   }

    mapping(address => bool) public isAuthorized; //tracker for authorized addresses

    address public factoryAddress;

    event FactoryUpdate(address _newFactory);
    event NewAuthorization(address _newAuthorized);
    event RemoveAuthorization(address _removeAuthorized);
    ...
    function mint(address payable _to, uint _amount) external onlyFactory{
        _safeMint(_to, _amount);
    }

    function updateFactory(address _newFactory) external onlyAuthorized {
        factoryAddress = _newFactory;

        emit FactoryUpdate(_newFactory);
    }
    ...
    //lock registry / guardian functions
...
abstract contract LockRegistry is Ownable, IERC721LOCK {
...
contract AmericanaMarketplaceV1 {
//this probably be a custom fork of the Seaport marketplace contracts
...
interface ILockERC721A is IERC721A {
	function lockId(uint256 _id) external;
	function unlockId(uint256 _id) external;
	function freeId(uint256 _id, address _contract) external;
}
...
import "../interfaces/ILockERC721A.sol";

contract AmericanaGuardian {
	...
	ILockERC721A public immutable LOCKABLE;
	...
	constructor(address _lockable) {
		LOCKABLE = ILockERC721A(_lockable);
	}
