    ...
    PausableUpgradeable
{
    ...
    bytes32 public constant IMPLEMENTATION_TYPE = keccak256("ERC1155");
    // Admin Role that can start sales and mint nfts
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    // Mint Role that can mint nfts
    ...
    uint96 public royaltyBp;

    // max unique ids that can be minted
    uint256 public maxUniqueIds;
    ...
    mapping(uint256 => uint256) public supplyById;

    // store-front metadata
    string public contractURI;
    // owner of the contract, used for ownable compatibility
    address private _owner;

    mapping(uint256 => string) private _tokenURIs;

    CountersUpgradeable.Counter private _uniqueIdCounter;
     ...
     *
     * @param maxUniqueIds max tokenIds specified in initialization.
     * @param tokenId entered tokenId.
     *
     */
    error ExceededMaxUniqueIds(uint256 maxUniqueIds, uint256 tokenId);

    /**
     * Specified id must be less than the max token supply defined.
     *
     * @param maxSupply max supply specified in initialization.
     * @param tokenId entered tokenId.
     *
     */
    error ExceededMaxSupply(uint256 maxSupply, uint256 tokenId);
     ...
     *
     * @param account owner of token's user address.
     * @param caller caller's address
     */
    error NotApprovedOrOwner(address account, address caller);

    /**
     * maxTokenSupply must be greater than zero.
     *
     * @param maxTokenSupply max supply specified in initialization.
     * @param tokenId entered tokenId.
     *
     */
    error ZeroTokenSupply(uint256 maxTokenSupply, uint256 tokenId);
    ...
    /**
     * @dev Throws InsufficientPermissions if called by any account other than the owner.
     */
    modifier onlyOwner() {
    ...
    {
        /**
         * _baseURI is the base URI for token metadata.
         * _contractURI is the URI for the storefront-level metadata of the contract.
         * _maxUniqueIds max supply of tokenIds available to the contract.
         * _royaltyBp basis points to calculate royalty from transactions.
         * _royaltyReceiver address to pay royalty.
         * _admins list of admin addresses.
         * _minters list of minter addresses.
         */
        (
            string memory _baseURI,
            string memory _contractURI,
            uint256 _maxUniqueIds,
            uint96 _royaltyBp,
        ...
        // _maxUniqueIds of 0 implies no limit
        if (_maxUniqueIds > 0) {
            maxUniqueIds = _maxUniqueIds;
        } else {
            maxUniqueIds = type(uint256).max;
        }

        // set contract options
        contractOptions = _contractOptions;

        // set royalty with ERC2981 and public vars
        if (_royaltyBp > 0 && _royaltyReceiver != address(0)) {
            _setDefaultRoyalty(_royaltyReceiver, _royaltyBp);
    ...
    /**
     * @dev Concatenates the base URI with a tokenURI based on the tokenId.
     *
    ...
    {
        return string(abi.encodePacked(super.uri(0), _tokenURIs[tokenId]));
    }

    /**
     * @dev Returns the amount of unique tokenIds minted.
     *
     * @return uint256 the amount of unique tokenIds minted.
     */
    function uniqueTokenIds() public view returns (uint256) {
        return _uniqueIdCounter.current();
    }

    /**
     * @dev Returns _owner address.
     *
     * @return address the _owner address.
     */
