...
import "./IERC721J.sol";
import "./IERC721JFull.sol";
import "./IERC721JFreeClaims.sol";
import "./IERC721JEnumerable.sol";
    ...
    Ownable
{
    ...
    // Mapping for rarity multipliers. Takes generation number with the multiplier number.
    ...
    event NewSplit(uint256 songId, address payable splitAddress);
    ...
    //Returns the price multiplier for a rarity
    ...
    {
        return _mintPrice * _rarityMultipliers[generation];
    }

    //Sets the price multiplier for a rarity
    function setRarityMultiplier(uint256 generation, uint256 multiplier)
        public
        virtual
        onlyOwner
    {
        _rarityMultipliers[generation] = multiplier;
    }
    ...
    function mintCopy(uint256 tokenId) public payable {
        ...
        uint256 rarityMult = 1;
        if (_rarityMultipliers[songGeneration] > 1)
            rarityMult = _rarityMultipliers[songGeneration];
        //Requires eth
        if (msg.value < _mintPrice * rarityMult) revert NotEnoughEther();
        //Transfer eth
    ...
    function mintCopyToken(uint256 tokenId, address token) public {
        ...
        if (_rarityMultipliers[songGeneration] > 1)
            rarityMult = _rarityMultipliers[songGeneration];
        uint256 rarityPrice = _tokenPrice[token] * rarityMult;
        //Requires token
        ...
        if (address(_songSplits[songId]) != address(0)) {
            ...
            ERC20(token).transferFrom(msg.sender, owner(), rarityPrice);
        }
