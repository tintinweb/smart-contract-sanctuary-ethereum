...
contract ChainPotsTest {
    ...
    uint256 public lastWeeklyPot;
    uint256 public drawTime = 5 minutes;
    address payable[] public playersWeeklyPot;
    uint256 public weeklyPotEntryPrice = 0.001 ether;

    event EntryWeeklyPot(address indexed _from, uint256 _value);
    event WinnerWeeklyPot(
        address indexed _from,
        uint256 _value,
        uint256 indexed _timestap
    );
    event WinnerWeeklyPotEmpty(uint256 indexed _timestap);
    ...
    function enterWeeklyPot(uint256 _count) public payable {
        require(
            msg.value == weeklyPotEntryPrice * _count,
            "incorrent value sent to contract"
        );
        for (uint256 i = 0; i < _count; i++) {
            playersWeeklyPot.push(payable(msg.sender));
        }
        emit EntryWeeklyPot(msg.sender, msg.value);
    }

    function pickWeeklyPotWinner() external {
        require(
            block.timestamp - lastWeeklyPot > drawTime,
            "weekly pot timer is still running"
        );
        uint256 playersWeeklyPotLength = playersWeeklyPot.length;
        lastWeeklyPot = block.timestamp;
        if (playersWeeklyPotLength > 0) {
            uint256 index = randomWeeklyPot() % playersWeeklyPotLength;
            address payable winningAddress = playersWeeklyPot[index];
            uint256 winningAmount = (playersWeeklyPotLength *
                weeklyPotEntryPrice *
                winnerCut) / 100;
            winningAddress.transfer(winningAmount);
            payable(houseAddressOne).transfer(
                (playersWeeklyPotLength * weeklyPotEntryPrice * houseCutOne) / 100
            );
            payable(houseAddressTwo).transfer(
                (playersWeeklyPotLength * weeklyPotEntryPrice * houseCutTwo) / 100
            );
            delete playersWeeklyPot;
            emit WinnerWeeklyPot(winningAddress, winningAmount, lastWeeklyPot);
        }

        emit WinnerWeeklyPotEmpty(lastWeeklyPot);
    }

    function randomWeeklyPot() private view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encode(
    ...
    function getPlayersWeeklyPot()
        public
        view
        returns (address payable[] memory)
    {
        return playersWeeklyPot;
    }

    function setWeeklyEntryPrice(uint256 _newEntryPrice) external restricted {
        weeklyPotEntryPrice = _newEntryPrice;
    }
