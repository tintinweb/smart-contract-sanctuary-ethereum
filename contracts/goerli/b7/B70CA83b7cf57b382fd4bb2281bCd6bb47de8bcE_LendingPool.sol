...
contract LendingPool is Ownable, Math {
        ...
        )
    {
        ...
        for (uint256 index = 0; index < tokens.length; index++) {
            ...
            uint256 usds = usdInUnits.div(1 * 10**(tokens[index].decimal));
            totalLiquidityUSDS += usds.mul(totalLendings[tokenaddress].div(1 * 10**18));
            totalDebtUSDS += usds.mul(totalDebt[tokenaddress].div(1 * 10**18));
            totalStableBorrowUSDS += usds.mul(totalStableDebt[tokenaddress].div(1 * 10**18));
            totalVariableBorrowUSDS += usds.mul(totalVariableDebt[tokenaddress].div(1 * 10**18));
        }
