...
contract PayrollManager is SignatureEIP712 {
    mapping(bytes32 => bool) approvedHash;
    uint256[] packedValues;

    address ALLOWANCE_MODULE;

    function packPayoutNonce(bool value, uint256 nonce) internal {
        uint256 slot = nonce / 256;
        uint256 bitIndex = nonce % 256;

        if (slot >= packedValues.length) {
            packedValues.push(1);
        }

        if (value) {
            packedValues[slot] |= 1 << bitIndex;
        } else {
            packedValues[slot] &= ~(1 << bitIndex);
        }
    }

    function getValue(uint index) internal view returns (bool) {
        uint slotIndex = index / 256;
        uint bitIndex = index % 256;
        if (packedValues.length == 0 || packedValues.length < slotIndex) {
            return false;
        } else {
            return (packedValues[slotIndex] & (1 << bitIndex)) != 0;
        }
    }
    ...
    function validatePayouts(
        ...
        for (uint96 i = 0; i < roots.length; i++) {
            if (!approvedHash[roots[i]]) {
                address signer = validatePayrollTxHashes(
                    roots[i],
                    signatures[i]
                );
                require(isApprover(safeAddress, signer), "Not an Operator");
                approvedHash[roots[i]] = true;
                isNewAdded = true;
            }
        }
    ...
    function executePayout(
        ...
        if (
            ...
            (packedValues.length == 0 || !getValue(payoutNonce))
        ) {
    ...
    function multiSendUnpacked(
        address[] memory tos,
        uint256[] memory values,
        bytes[] memory datas
    ) public payable {
        for (uint96 index = 0; index < datas.length; index++) {
            uint256 dataLength = datas[index].length;
            address to = tos[index];
            uint256 value = values[index];
            bytes memory data = datas[index];
