...
contract Actuator is SynchronV1{
    ...
    */
    ...
    function createOptionWithMint(OptionParam calldata optionParam,uint96 royal) external returns(uint256 optionId){
        _mintHelper(optionParam, royal);
        optionId = createOption(optionParam);
    }
    ...
    function _mintHelper(OptionParam calldata optionParam,uint96 royal) internal{
        ...
        if (IERC165(optionParam.token).supportsInterface(INTERFACE_ID_ERC1155)){
            IERC1155(optionParam.token).safeMint(optionParam.creator, optionParam.identifier, optionParam.amount);
        }
    }
}