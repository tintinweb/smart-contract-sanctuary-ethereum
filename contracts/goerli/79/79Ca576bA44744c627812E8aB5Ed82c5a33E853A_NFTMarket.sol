...
abstract contract NFTMarketExhibition {
  ...
  event SellerAddedToExhibition(uint256 indexed exhibitionId, address indexed seller);
  ...
  function createExhibition(
  ...
  ) external returns (uint256 exhibitionId) {
    ...
    for (uint256 i = 0; i < listLength; ) {
      ...
      emit SellerAddedToExhibition(exhibitionId, seller);
  ...
  function isAllowedSellerForExhibition(uint256 exhibitionId, address user) external view returns (bool allowedSeller) {
    if (user != address(0)) {
      address curator = idToExhibition[exhibitionId].curator;
      if (user == curator) {
        allowedSeller = true;
      } else if (curator != address(0)) {
        allowedSeller = exhibitionIdToSellerToIsAllowed[exhibitionId][user];
      }
    }
  }
