...
contract ChessMarket is Ownable{
   ...
   function _iswon(Match memory _match, address _creater, address _withdrawer, bytes memory _winnercolour, bytes memory _username, bytes memory _usercolour, bytes memory _bottomuser)  internal view returns (address, bool) {
