...
function mintPlanetY(uint256 count) public payable nonReentrant {
    require(saleIsActive, "Sale not active");
    require(msg.value >= PLANET_PRICE, "Insufficient payment");
    require(totalPlanet2Supply() + count - 1 < PLANET2_SUPPLY, "Exceeds max supply");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
    
    PLANET2_SUPPLY++;
    planet2SupplyCounter.increment();
    _safeMint(msg.sender, PLANET2_SUPPLY + PLANET1_SUPPLY);
    payable(_splitter).transfer(msg.value);
}

  function mintPlanetX(uint256 count) public payable nonReentrant {
    require(saleIsActive, "Sale not active");
    // required function ~ totalPlanet1Supply() ~ required constant PLANET1_SUPPLY
    require(totalPlanet2Supply() + count - 1 < PLANET2_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
