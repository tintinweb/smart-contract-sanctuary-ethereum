...
contract Stakable is Ownable {
    ...
    function removeStake(uint256 userIndex, uint256 stakeIndex) internal {
        if (stakeIndex >= _stakeholders[userIndex].userStakes.length) return;

        for (uint256 i = stakeIndex; i < _stakeholders[userIndex].userStakes.length - 1; i++) {
            _stakeholders[userIndex].userStakes[i] = _stakeholders[userIndex].userStakes[i + 1];
        }
        delete _stakeholders[userIndex].userStakes[_stakeholders[userIndex].userStakes.length - 1];
        // _stakeholders[userIndex].userStakes.length--;
        // return _stakeholders[userIndex].userStakes;
    }
}
