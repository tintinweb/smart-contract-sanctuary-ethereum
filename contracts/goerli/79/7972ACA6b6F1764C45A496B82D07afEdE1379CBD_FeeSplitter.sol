...
contract FeeSplitter is Controllable {
    ...
    mapping(uint256 => uint256) public feeProportions;
    ...
    function distributeFees(address _token) external {
        ...
        uint256 arrayLength = feeRecipients.length;
        for (uint256 i; i < arrayLength; ++i) {
            IERC20(_token).transfer(feeRecipients[i], (feeAmount * feeProportions[i]) / Constants.PERCENT_PRECISION);
        }
    }

    function setFeeRecipients(
        address[] calldata _feeRecipients,
        uint256[] calldata _feeProportions
    ) external onlyMultisig {
        // Check that length is correct
        uint256 arrayLength = _feeRecipients.length;
        if (arrayLength != _feeProportions.length) {
            revert lengthMismatch();
        }

        // Set fee proportions. Note that this does not overwrite existing values past the end of the array, but doing so is unnecessary since all
        // operations operate only up to the length of the array.
        uint256 proportionTotal;
        for (uint256 i; i < arrayLength; ++i) {
            proportionTotal += _feeProportions[i];
            feeProportions[i] = _feeProportions[i];
        }
