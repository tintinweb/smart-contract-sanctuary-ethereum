...
contract LlamaPayV2Factory is
    ERC721("LlamaPay V2 Stream", "LLAMAPAY-V2-STREAM")
{
        ...
        returns (LlamaPayV2Payer payerContract)
    {
        ...
        payerContracts[payerCount] = llamapay;
        unchecked {
            payerCount++;
        }
    ...
    /// @notice mint stream position
    /// @param _to send to
    /// @return minted return true on success
    /// @return id resulting token id
    function mint(address _to) external returns (bool minted, uint256 id) {
        (, bool deployed) = calculatePayerAddress(msg.sender);
        if (!deployed) revert NotPayer();
        id = tokenId;
        _safeMint(_to, id);
        tokenToPayerContracts[id] = msg.sender;
        unchecked {
            tokenId++;
        }
        return (true, id);
    ...
    /// @notice burn stream position
    /// @param _id token id
    /// @return burned true on successful burn
    function burn(uint256 _id) external returns (bool burned) {
        if (tokenToPayerContracts[_id] != msg.sender) revert NotPayer();
        tokenToPayerContracts[_id] = address(0);
        _burn(_id);
        return true;
    }
...
contract LlamaPayV2Payer is BoringBatchable {
    ...
    function deposit(address _vault, uint256 _amount) external {
        ...
        uint256 toAdd;
        unchecked {
            toAdd = _amount * (10**(20 - asset.decimals()));
        }
    ...
    function withdraw(uint256 _id, uint256 _amount) public {
        ...
        if (payee == address(0)) revert DoesNotExist();
        ...
        unchecked {
            ...
            if (available < _amount) revert NotAvailable();
    ...
    function cancelStream(uint256 _id) external {
        if (msg.sender != owner) revert NotOwner();
        if (ERC721(factory).ownerOf(_id) == address(0)) revert DoesNotExist();
