...
contract DEX {
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        require(totalLiquidity == 0, "DEX: init - already has liqudity!");
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256) {
        ...
        require(sent, "tokenToEth: Reverted transfering ETH to you!");
        emit TokenToEthSwap(
            msg.sender,
            "Ballons to ETH",
            ethOutput,
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        ...
        tokensDeposit = (msg.value.mul(tokenReserver) / ethReserve).add(1);

        uint256 liquidityMinted = msg.value.mul(totalLiquidity) / ethReserve;
        liquidity[msg.sender] = liquidity[msg.sender].add(liquidityMinted);
        totalLiquidity = totalLiquidity.add(liquidityMinted);

        require(token.transferFrom(msg.sender, address(this), tokensDeposit));
    ...
    function withdraw(uint256 amount) public returns (uint256, uint256) {
        require(
            ...
            "Withdraw: Sender does NOT have enough liquidity!"
        );
        ...
        require(sent, "withdraw(): Reverted transfering ETH to you!");
        require(
            token.transfer(msg.sender, tokenAmount),
            "withdraw(): Reverted transfering tokens to you!"
        );
