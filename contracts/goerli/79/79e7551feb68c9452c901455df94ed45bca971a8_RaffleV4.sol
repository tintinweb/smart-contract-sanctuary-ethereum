...
contract RaffleV4 is RaffleV3 {
    ...
    function keyClaim(uint256[] memory) external override {
        require(false, "This function has been disabled");
    }

    function keyNFTClaim(
        uint256[] calldata ids_,
        uint256[] calldata genesis_
    )
        external
        whenNotPaused
        nonReentrant
    {
        ...
        uint256 genesisLength = genesis_.length;
        ...
        for(uint256 i = 0; i < genesisLength; i++) {
            uint256 current = genesis_[i];
