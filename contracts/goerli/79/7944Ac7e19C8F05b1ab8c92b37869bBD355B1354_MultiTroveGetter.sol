...
contract MultiTroveGetter {
    ...
    function _getMultipleSortedTrovesFromHead(
        ...
        for (uint256 idx = 0; idx < _count; ++idx) {
            ...
            ) = troveManager.getTrove(_asset, currentTroveowner);
...
contract SortedTroves is Ownable, CheckContract, ISortedTroves {
    ...
    function _validInsertPosition(
        ...
        if (_prevId == address(0) && _nextId == address(0)) {
            ...
            return data[_asset].head == _nextId && _NICR >= _troveManager.getNominalICR(_asset, _nextId);
        } else if (_nextId == address(0)) {
            // `(_prevId, null)` is a valid insert position if `_prevId` is the tail of the list
            return data[_asset].tail == _prevId && _NICR <= _troveManager.getNominalICR(_asset, _prevId);
        } else {
    ...
    function _requireCallerIsBOorTroveM(ITroveManager _troveManager) internal view {
        require(
            ...
            "SortedTroves: Caller is neither BorrowerOperations nor TroveManager"
        );
    }
}
