...
contract DssProxyActions is Common {
    ...
    event Repay(address user, uint vault, uint amount);
    ...
    function wipe(
        ...
        VatLike(vat).subDebt(ilk, urn, wad);

        uint feeBorrow = JugLike(jug).getFeeBorrow(ilk) * wad;
        address treasury = JugLike(jug).treasury();
        USBJoinLike(USBJoin).USB().transferFrom(msg.sender, treasury, feeBorrow);

        emit Repay(msg.sender, cdp, wad);
    }
    ...
    function wipeAll(
        ...
        (, uint art, uint debt) = VatLike(vat).urns(ilk, urn);
        ...
        VatLike(vat).subDebt(ilk, urn, debt);

        uint feeBorrow = JugLike(jug).getFeeBorrow(ilk) * debt;
        address treasury = JugLike(jug).treasury();
        USBJoinLike(USBJoin).USB().transferFrom(msg.sender, treasury, feeBorrow);

        emit Repay(msg.sender, cdp, art); 
    }
    ...
    function wipeAndFreeETH(
        ...
        VatLike(vat).subDebt(ilk, urn, wadD);
        ...
        uint feeBorrow = JugLike(jug).getFeeBorrow(ilk) * wadD;
        address treasury = JugLike(jug).treasury();
        USBJoinLike(USBJoin).USB().transferFrom(msg.sender, treasury, feeBorrow);
        ...
        emit Repay(msg.sender, cdp, wadD);
    ...
    function wipeAndFreeGem(
        ...
        uint feeBorrow = JugLike(jug).getFeeBorrow(ilk) * wadC;
        address treasury = JugLike(jug).treasury();
        USBJoinLike(USBJoin).USB().transferFrom(msg.sender, treasury, feeBorrow);
        ...
        emit Repay(msg.sender, cdp, amtC);
        emit Withdraw(msg.sender, cdp, wadC);
    }
