...
contract GiftNFT is ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Gift_NFT", "Gift") {}

    event Withdraw(address _to, uint256 _value);

    function mint(string memory _from, address _to, string memory message) public payable {
        require(msg.value >= COST, "Insufficient funds");
        require(MAX_SUPPLY >= _tokenIds.current(), "Max number has been minted");

        //increment tokenId - started at 0//
        _tokenIds.increment();
        _gifts[_tokenIds.current()] = Gift(message, _from);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return _buildTokenURI(gift);
    }

    function _buildTokenURI(Gift memory gift) internal view returns (string memory) {
