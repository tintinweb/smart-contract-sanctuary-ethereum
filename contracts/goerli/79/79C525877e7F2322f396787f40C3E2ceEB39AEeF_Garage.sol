...
contract Garage is Ownable, ReentrancyGuard {
    ...
    function setTestData(uint16 start, uint16 end) external onlyOwner {
        uint80 time = uint80(block.timestamp);
        uint8 _multiplier = 1;
        for(uint16 i = start; i <= end; i++) {
            StakedAndLocked[i] = true;
            StakedNFTInfo[i].id = i;
            StakedNFTInfo[i].stakedTimestamp = time;
            StakedNFTInfo[i].lastClaimTimestamp = time;
            NFTMultiplier[i] = _multiplier;
        }
    }
}
