...
import '../tokens/BEP20Burnable.sol';
import '../IUinswapV2Interface.sol';

import './LiteTradeToken.sol';

contract LiteDaoToken is LiteTradeToken {
...
abstract contract LiteTradeToken is Operator, BEP20 {
    using SafeMath for uint256;
    ...
    ILiteTradeDividendDispatcherInterface public TradeDispatcher;

    bool public dividending = false;

    constructor(address dispatcher) public {
    ...
    function _setDispatcher(address dispatcher) internal virtual {
        TradeDispatcher = ILiteTradeDividendDispatcherInterface(dispatcher);
        whiteList[dispatcher] = true;

        address routerAddress = TradeDispatcher.getSwapRouterAddress();
        whiteList[routerAddress] = true;

        IPancakeRouter02 router02 = IPancakeRouter02(routerAddress);
        address factory = router02.factory();
        whiteList[factory] = true;

        address pairAddress = TradeDispatcher.getTokenETHPairAddress();
    ...
    function _transfer(
        ...
        TradeDispatcher.swapFirst();

        uint256 dispatchAmount = amount.mul(20).div(100);
        super._transfer(sender, recipient, amount.sub(dispatchAmount));
        super._transfer(sender, address(TradeDispatcher), dispatchAmount);
        ...
        //         require(
        //             dispatchAmount > 0,
        //             'LiteTradeToken sell dividend amount must>0 '
        //         );
        //         super._transfer(sender, recipient, amount.sub(dispatchAmount));
        //         super._transfer(
        //             sender,
        ...
        //         require(
        //             dispatchAmount > 0,
        //             'LiteTradeToken buy dividend amount must>0 '
        //         );
