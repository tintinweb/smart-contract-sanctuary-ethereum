...
contract ArtaNFTFactory is AccessControl, Initializable, IArtaNFTFactory {
    ...
    mapping(address => bool) public BlacklistMarketplaces;
    ...
    function setBlacklistMarketplace(address[] memory markets, bool[] memory approved) public onlyAdmin {
        ...
        for(uint256 i=0; i<markets.length; i++) {
            BlacklistMarketplaces[markets[i]] = approved[i];
        }
    }

    function checkBlacklistMarketplaces(address addr) external override {
        require(BlacklistMarketplaces[addr] == false, "BlackList Marketplace");
    }
...
contract ArtaNFT is ERC721Pausable, IsAdmin, Ownable {
    ...
    function tokensOfOwner(address _owner) public view returns (uint256[] memory){
        uint256 _length = balanceOf(_owner);
