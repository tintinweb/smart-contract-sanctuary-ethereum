...
contract MegaPunksPOP is ERC721A, Ownable, ReentrancyGuard {
        ...
        callerIsUser
    {
        ...
        bytes32 leaf = _leaf(Strings.toString(allowance), string(abi.encodePacked(msg.sender)));
        if (!_verify(leaf, proof, allowlistMerkleRoot)) revert InvalidMerkleProof();

        if (allowance < (_getAux(msg.sender)) + 1) revert NotAllowedToMint();

        _setAux(msg.sender, uint64(_numTokens));
