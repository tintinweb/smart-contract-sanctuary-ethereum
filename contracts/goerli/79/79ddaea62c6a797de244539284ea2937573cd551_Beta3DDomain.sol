...
contract Beta3DDomain is ERC721EnumerableT, Delegated, ReentrancyGuard {
  using Strings for uint;

  /**
    * @dev Data structure of Dreamer
    */
  struct Dreamer {
    address owner;
    bool genesisType;
  ...
  string public notRevealedUri = "ipfs://to-be-updated/hidden.json";
...
/**
  ...
  */
  function balanceOf(address account) public view override returns (uint) {
    require(account != address(0), "DREAMER: balance query for the zero address");
  ...
  */
  function isOwnerOf( address account, uint[] calldata tokenIds ) external view override returns( bool ){
    for(uint i; i < tokenIds.length; ++i ){
      if( dreamers[ tokenIds[i] ].owner != account )
        return false;
    }
...
/**
  ...
  */
  function ownerOf( uint tokenId ) public override (ERC721T,IERC721) view returns( address owner_ ){
    address owner = dreamers[tokenId].owner;
    require(owner != address(0), "DREAMER: query for nonexistent token");
    return owner;
  }
...
/**
  ...
  */
  function tokenByIndex(uint index) external view override returns (uint) {
    require(index < totalSupply(), "DREAMER: global index out of bounds");
    return index;
  }
...
/**
  ...
  */
  function tokenOfOwnerByIndex(address owner, uint index) public view override returns (uint tokenId) {
    uint count;
    for( uint i; i < dreamers.length; ++i ){
      if( owner == dreamers[i].owner ){
        if( count == index )
          return i;
        else
          ++count;
  ...
  */
  function tokenURI(uint tokenId) external view override returns (string memory) {
    require(_exists(tokenId), "DREAMER: URI query for nonexistent token");
...
/**
  ...
  */
  function mint( uint quantity ) external payable nonReentrant {
    require(isMintActive == true,"DREAMER: Minting needs to be enabled.");
    require( msg.value >= PRICE * quantity, "DREAMER: Ether sent is not correct" );

    //If whitelist is active, people in WL can mint 
    //based on the allowable qty limit set by owner/delegates.
    if( isWhitelistActive ){
      require( accessList[ msg.sender ] >= quantity, "DREAMER: Account is less than the qty limit");
      accessList[ msg.sender ] -= quantity;
    } else { 
      //For public, MAX_QTY limit will be applied. 
      //MAX_QTY is determined by the owner/delegates
      require(quantity <= MAX_QTY, "DREAMER:Quantity must be less than or equal MAX_QTY");
    }

    uint supply = totalSupply();
    require( supply + quantity <= MAX_SUPPLY, "DREAMER: Mint/order exceeds supply" );
    for(uint i; i < quantity; ++i){
...
/**
  ...
  */
  function team_mint(uint[] calldata quantity, address[] calldata recipient) external onlyDelegates{
    require(quantity.length == recipient.length, "DREAMER: Must provide equal quantities and recipients" );
    ...
    require( supply + totalQuantity < MAX_SUPPLY, "DREAMER: Mint/order exceeds supply" );
...
/**
  ...
  */
  function setMintingActive(bool mintActive_) external onlyDelegates {
    require( isMintActive != mintActive_ , "DREAMER: New value matches old" );
...
/**
  ...
  */
  function setWhitelistActive(bool isWhitelistActive_) external onlyDelegates{
    require( isWhitelistActive != isWhitelistActive_ , "DREAMER: New value matches old" );
...
/**
  ...
  */
  function setMaxSupply(uint maxSupply) external onlyDelegates{
    require( MAX_SUPPLY != maxSupply, "DREAMER: New value matches old" );
    require( maxSupply >= totalSupply(), "DREAMER: Specified supply is lower than current balance" );
...
/**
  ...
  */
  function setMaxQty(uint maxQty) external onlyDelegates{
    require( MAX_QTY != maxQty, "DREAMER: New value matches old" );
    MAX_QTY = maxQty;
  }
...
/**
  ...
  */
  function setPrice(uint price) external onlyDelegates{
    require( PRICE != price, "DREAMER: New value matches old" );
    PRICE = price;
  }
...
/**
  ...
  */
  function _exists(uint tokenId) internal view override returns (bool) {
    return tokenId < dreamers.length && dreamers[tokenId].owner != address(0);
  }
...
/**
  ...
  */
  function _mint(address to, uint tokenId) internal {
    ...
    dreamers.push(Dreamer(to,false));
    emit Transfer(address(0), to, tokenId);
  }

/**
    * @dev update the dreamer type.
    */
  function updateDreamerType(bool dreamerType, uint[] calldata tokenIds ) external onlyDelegates {
        //update logic to update the dreamerType
        for(uint i=0; i < tokenIds.length; i++) {
            require(_exists(tokenIds[i]), "DREAMER: TokenId not exist");
            dreamers[tokenIds[i]].genesisType = dreamerType;
        }
  }

/**
  * @dev returns the dreamerTypes based on the tokenIds.
  */
  function getDreamerType(uint[] calldata tokenIds) external view returns(bool[] memory dreamerTypes) {
      // return dreamerType true/false
      bool[] memory _dreamerTypes = new bool[](tokenIds.length);
      for(uint i; i < tokenIds.length; i++) {
        _dreamerTypes[i] = dreamers[tokenIds[i]].genesisType;
      }
      return _dreamerTypes;
  }
...
/**
  ...
  */
  function _transfer(address from, address to, uint tokenId) internal override {
    require(dreamers[tokenId].owner == from, "DREAMER: transfer of token that is not owned");

    // Clear approvals from the previous owner
    _approve(address(0), tokenId);
    _beforeTokenTransfer(from, to);

    dreamers[tokenId].owner = to;
...
/**
  ...
  */
    function stake( uint[] calldata tokenIds ) external {
        require( isStakeActive, "DREAMER: Staking is not active" );
        
        Dreamer storage dreamer;
        //Check if TokenIds exist and the dreamer owner is the msge sender
        for( uint i; i < tokenIds.length; ++i ){
            require( _exists(tokenIds[i]), "DREAMER: Query for nonexistent token" );
            dreamer = dreamers[ tokenIds[i] ];
            require(dreamer.owner == msg.sender, "DREAMER: Staking token that is not owned");
  ...
  */
    function unStake( uint[] calldata tokenIds ) external {
        ...
        for( uint i; i < tokenIds.length; ++i ){
            ...
            "DREAMER._unstake: Sender must have staked tokenID");
  ...
  /**
    ...
    */
    function setStakeActive( bool isActive_ ) external onlyDelegates {
      require( isStakeActive != isActive_ , "DREAMER: New value matches old" );
