...
abstract contract BridgeOut is Getters, Setters, TokenHolder {
    ...
    function _bridgeOutInternal(
        ...
        if (tokenType == TokenType.ERC20) {
            ...
            transferInERC20(tokenOwner, token, amount);
        ...
        publishBridgeOutMessage(token, baseType, destinationChainId, amount, instance, recipient);
    }
    ...
    function bridgeOutWithPermit(
        ...
        try IERC20Permit(token).permit(tokenOwner, address(this), amount, deadline, v, r, s) {
            _bridgeOutInternal(tokenOwner, token, amount, 0, destinationChainId, recipient);
    ...
    function transferInERC20(address tokenOwner, address token, uint256 amount) private {
        ...
        uint256 transferredAmount = IERC20(token).balanceOf(address(this)) - balance;
    ...
    function transferInERC721(address tokenOwner, address token, uint256 tokenId) private {
        require(isEnabled(token, DEFAULT_TOKEN_BASE_TYPE), "DISABLED");
...
contract BatchBridgeOut is BridgeOut {
    ...
    function batchBridgeOut(
        ...
        // Bridge setting is initialized to a value that is neither 0 nor 1
        // but will be set to 0 or 1 in the loop below. We do this to optimize
        // the check that all token bridges are either burning or locking.
...
contract BridgeIn is Authority, Setters {
    ...
    function bridgeIn(Structs.VSM calldata message) external {
        ...
        if (payloads.length != 1) {
            ...
            for (uint256 i = 0; i < payloads.length; i++) {
                ...
                // getTokenAndBaseType guarantees that the token is not address(0)
            ...
            bridgeInERC1155Batch(bridgedInToken, tokenIds, quantities, recipient);
    ...
    function bridgeInERC1155Batch(address token, uint256[] memory tokenIds, uint256[] memory amounts, address recipient) internal {
        IGalaGameItems ggi = IGalaGameItems(token);
        // TODO: for now assume only NFTs are bridged in
        // TODO: for now assume that only burning bridge is used
    ...
    function encodeHeader(uint16 destinationChainId, bytes calldata recipient, uint8 numTokens) internal pure returns (bytes memory) {
        return
            bytes.concat(
                ...
                bytes1(0x00), //recipient encoding (0 == text)
