...
contract Airdrop is Validatable {
    ...
    event Airdroped(uint256 indexed id, address indexed user, uint256 indexed amount);
     ...
     */
    function createAirdrop(uint256 startTime, uint256 endTime) external onlyAdmin {
    ...
    function updateAirdrop(
        ...
        uint256 startTime,
        uint256 endTime
    ) external onlyAdmin validAirdropId(id) {
        require(startTime > 0 && startTime < endTime, "Invalid time");
     ...
     */
    function airdrop(uint256 id) external validAirdropId(id) {
        require(airdrops[id].status, "Airdrop was cancel");
        require(
            airdrops[id].startTime <= block.timestamp && block.timestamp <= airdrops[id].endTime,
            "Can not airdrop at this time"
        );
        require(claimable[_msgSender()][id] > 0, "User is not in whitelist");

        uint256 claimAmount = claimable[_msgSender()][id];
        claimable[_msgSender()][id] = 0;
        // Mint Airdrop NFTs
        //slither-disable-next-line unused-return
        genesis.mintBatch(_msgSender(), claimAmount);

        // Emit events
        emit Airdroped(id, _msgSender(), claimAmount);
    }
