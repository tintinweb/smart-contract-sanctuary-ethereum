...
// File: OASStringsUpgradeable.sol
...
library OASStringsUpgradeable  {
    ...
    function toHexStringNoPrefix(uint256 value) internal pure returns (string memory) {
        unchecked {
            return toHexStringNoPrefix(value, MathUpgradeable.log256(value) + 1);
        }
    }
...
contract OASERC721TemplateHosted is Initializable, ERC721Upgradeable, OwnableUpgradeable, IOASERC721TemplateHosted {
    using OASStringsUpgradeable for uint256;


    uint256 private storageKey;
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
        _disableInitializers();
    }

    function initialize(string calldata tokenName, string calldata tokenSymbol,bytes32 _storageKey) initializer public override {
        storageKey = uint256(_storageKey);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        _requireMinted(tokenId);
        return string(abi.encodePacked("https://f.ofs.cab/uc/",storageKey.toHexStringNoPrefix(),"/",tokenId.toString(),".json"));
    }
    
    function contractURI() public view returns (string memory) {
        return string(abi.encodePacked("https://f.ofs.cab/uc/",storageKey.toHexStringNoPrefix(),"/collection.json"));
    }
