    ...
    ERC2981Upgradeable
{
    ...
    PublicSaleConfig private _publicSaleConfig;
    ...
    function initialize(
        ...
        uint96 royalty_,
        PublicSaleConfig calldata publicSaleConfig
    ...
    */
    modifier canMint(
        ...
        unchecked {
            require(_publicSaleConfig.enabled, "Sale not enabled");
            require(
                msg.value >= (amount * _publicSaleConfig.unitPrice),
    ...
    function _guardMint(address, uint256 amount) internal view virtual {
        unchecked {
            require(
            ...
            require(
                amount <= _publicSaleConfig.maxPerTransaction,
    ...
    function publicMint(uint256 amount)
        external
        payable
        canMint(_msgSenderERC721A(), amount)
    {
        uint256 startTokenId = _nextTokenId();
        _guardMint(_msgSenderERC721A(), amount);
    ...
    function getPublicSaleConfig() external view returns (PublicSaleConfig memory) {
        return _publicSaleConfig;
    }

    function setPublicSaleConfig(uint256 maxPerTransaction, uint64 unitPrice)
        public
        onlyOwner
    {
        _publicSaleConfig.maxPerTransaction = uint256(maxPerTransaction);
        _publicSaleConfig.unitPrice = uint64(unitPrice);
    }

    function setPublicSaleStatus(bool enabled) external onlyOwner {
        if (_publicSaleConfig.enabled != enabled) {
            _publicSaleConfig.enabled = enabled;
        }
    }
}
