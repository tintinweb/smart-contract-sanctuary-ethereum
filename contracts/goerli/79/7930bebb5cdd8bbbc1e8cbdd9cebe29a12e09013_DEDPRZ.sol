...
contract DEDPRZ is ERC1155DeltaQueryable, IERC2981, DefaultOperatorFilterer {

    // *** Variables
    address public admin;
    address public treasury;

    uint256 public constant NUM_MAX = 10000;
    uint256 public mintPrice; // mint price
    uint256 public ROYALTIES; // 6.9% royalties
    uint16 internal constant _MAX_MINTS_PER_ADDRESS = 10;

    bool public updatedURI; // allowed one time
    ...
    constructor(
        ...
        ERC1155Delta(
            "ipfs://bafybeid2kp3fun3upa3gieitbez4hpoxq3fkxtugxwunsitojbo5w6dr5a/{id}"
        )
    {
        admin = msg.sender;
        treasury = _treasury;
    ...
    function mint(address to, uint256 amount) external payable {
        require(mintable, "Not mintable"); // check mintable
        require(_totalMinted() + amount <= NUM_MAX, "Minted out"); // max supply
        require(msg.value >= mintPrice * amount, "Insufficient funds"); // mint price
        require(balanceOf(to) + amount <= _MAX_MINTS_PER_ADDRESS, "Limit 10"); // 10 per wallet
    ...
    function royaltyInfo(
        ...
        return (treasury, (_salePrice * ROYALTIES) / 1000);
    }
    ...
    function setURI(string memory newuri) external virtual onlyAdmin {
        require(!updatedURI, "URI already updated");
        _setURI(newuri);
