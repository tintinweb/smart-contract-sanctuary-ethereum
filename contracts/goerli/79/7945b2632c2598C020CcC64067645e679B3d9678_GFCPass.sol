...
contract GFCPass is ERC1155, Ownable, Pausable, DefaultOperatorFilterer {
    //================================================================
    // Setup
    //================================================================
    ...
    mapping(uint256 => string) public passIdToMetadataURI;
    ...
    modifier mintChecks(address _sender, uint256 _quantity, uint256 _id, bool _isWhitelistCall) {
        uint256 value = msg.value;
        if( _quantity == 0 ){ revert CantMintZero(); }
        IDParams memory THIS_PASS = PASS_ID_PARAMS[_id];
        if( THIS_PASS.totalMinted + _quantity > THIS_PASS.totalSupply ){ revert TotalSupplyWillBeReached(); }
        if(_sender != owner() ) {
            if( passIdToMintedPerWallet[_id][_sender] + _quantity > THIS_PASS.maxMintsPerWallet ){ revert WillHaveExceededWalletLimit(); }        
            
            if( _isWhitelistCall && (value < _quantity * THIS_PASS.whitelistMintPrice) ){ revert InsufficientFunds(); }
            if( _isWhitelistCall && !THIS_PASS.whitelistMintIsOpen ){ revert WhitelistMintIsClosed(); }
            
            if( !_isWhitelistCall && (value < _quantity * THIS_PASS.publicMintPrice) ){ revert InsufficientFunds(); }
            if( !_isWhitelistCall && !THIS_PASS.publicMintIsOpen ){ revert PublicMintIsClosed(); }
        }
        _;
    }
        ...
        whenNotPaused 
    {
        ...
        if( THIS_PASS.totalMinted + _qty > THIS_PASS.totalSupply ){ revert TotalSupplyWillBeReached(); }
        if( THIS_PASS.totalMinted == THIS_PASS.totalSupply ){ revert TotalSupplyReached(); }
        if( passIdToMintedPerWallet[_id][_sender] + _qty > THIS_PASS.maxMintsPerWallet ){ revert WillHaveExceededWalletLimit(); }   
    ...
    function airdropPasses(address[] memory _recipients, uint256 _id) external onlyOwner {
        ...
        THIS_PASS.totalMinted += uint16(_recipients.length);
    }
