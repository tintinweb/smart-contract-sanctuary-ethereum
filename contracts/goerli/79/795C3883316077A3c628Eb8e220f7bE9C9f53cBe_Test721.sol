...
import {PremintReady} from "../../PremintReady.sol";

/// @title Test721.sol
/// @author Premint.xyz (https://premint.xyz)
/// @author dievardump ([email protected], https://twitter.com/dievardump)
/// @notice This is a Mock that allows to test the front end
contract Test721 is Ownable, PremintReady, ERC721A("Test721", "Test721") {
	address _premintSigner;
	uint256 _premintMax;
	...
	function contractURI() external pure returns (string memory) {
		return "ipfs://QmYxUtFnPFntZsrxRTCT3rJZUWC7ti949f1hxnnfnzNB2u";
	}

	function tokenURI(uint256) public pure override returns (string memory) {
		return "ipfs://QmNV1gDEFk8fH4dXF4kkqJh9ibz47i4fgVobySRShCBBsF";
	}

	function setPremintSigner(address newSigner) external {
		_premintSigner = newSigner;
	}

	function setPremintMax(uint256 newMax) external {
		_premintMax = newMax;
	}
	...
	/// @dev 0 means no limit. You should override this function in your contract if you wish for a max per wallet
	/// @return the maximal amount of tokens per wallet to mint through premint.xyz
	function premintMax() public view override returns (uint256) {
		return _premintMax;
	}
	...
	/// @dev Called by PremintReady when a config is valid.
	...
	function _premint(address to, uint256 amount) internal override {
		...
		// using batch mints of 5, because ERC721A becomes bad without it
		uint256 times = amount / 5;
		uint256 rest = amount % 5;
