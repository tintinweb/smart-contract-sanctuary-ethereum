...
import { IDepositorOptions } from "../interfaces/IDepositor.sol";
...
import { IConvexToken } from "../../integrations/convex/IConvexToken.sol";
...
import { TokenType, PathNotFoundException, PathNotFoundExceptionTyped } from "../interfaces/IPathFinder.sol";
// DATA
import { Balance, BalanceOps } from "../../libraries/Balances.sol";
import { AddressList } from "../../libraries/AddressList.sol";
import { StrategyPathTask, StrategyPathTaskOps } from "../data/StrategyPathTask.sol";
import { PathFinderComponent } from "../data/PathFinderComponent.sol";

// MULTICALL
import { MultiCall, MultiCallOps } from "../../libraries/MultiCall.sol";
import { ConvexV1_BaseRewardPoolCalls, ConvexV1_BaseRewardPoolMulticaller } from "../../multicall/adapters/convex/ConvexV1_BaseRewardPoolCalls.sol";
import { ConvexV1_BoosterCalls, ConvexV1_BoosterMulticaller } from "../../multicall/adapters/convex/ConvexV1_BoosterCalls.sol";

contract ConvexDepositor is ConvexHelper, IDepositorOptions {
    ...
    PathFinderComponent public override getComponentId =
        PathFinderComponent.CONVEX_DEPOSITOR;

    // Contract version
    uint256 public constant override version = 1;

    constructor(address _booster) ConvexHelper(_booster) {}

    /// @dev Withdraws normal or staked Convex position into underlying Curve LP token
    /// In case of staked position, it also withdraw all earned rewards
    /// @param amountIn amount in ConvexLP token to be withdrawn
    /// @param tokenOut ConvexLP of stake convex token
    function deposit(
        TokenType ttIn,
        address,
        uint256 amountIn,
    ...
    ) external view returns (uint256 amountOut, StrategyPathTask memory) {
        return _deposit(ttIn, amountIn, tokenOut, task);
    }
    ...
    function depositAll(
        ...
        uint256 balance = task.balances.getBalance(tokenIn);
        return _deposit(ttIn, balance, tokenOut, task);
    }
    ...
    function _deposit(
        ...
        address tokenOut,
        StrategyPathTask memory task
    ) internal view returns (uint256 amountOut, StrategyPathTask memory) {
        (
            address adapter,
            bool isStakedToken,
            address boosterAdapter
        ) = findConvexAdapterByToken(tokenOut, task);

        if (ttIn == TokenType.CONVEX_LP_TOKEN && isStakedToken) {
            depositCvx2StakedCvx(tokenOut, amountIn, adapter, task);

            return (amountIn, task);
        } else if (ttIn == TokenType.CURVE_LP_TOKEN) {
            task = (!isStakedToken)
                ? depositCurve2Cvx(
                    tokenOut,
                    amountIn,
        ...
        revert PathNotFoundExceptionTyped(ttIn, tokenOut);
    }

    function depositCurve2Cvx(
        address stakingToken,
        uint256 amount,
        address adapter,
        address boosterAdapter,
        StrategyPathTask memory task
    ) internal view returns (StrategyPathTask memory) {
        address curveLPToken = IConvexV1BaseRewardPoolAdapter(adapter)
            .curveLPtoken(); // F: [CNPF-2]

        bool isAll = task.balances.getBalance(curveLPToken) == amount;

        task.balances.subBalance(curveLPToken, amount);
        task.balances.addBalance(stakingToken, amount);

        MultiCall memory call = isAll
            ? ConvexV1_BoosterMulticaller(boosterAdapter).depositAll(
                IConvexV1BaseRewardPoolAdapter(adapter).pid(),
                false
            )
            : ConvexV1_BoosterMulticaller(boosterAdapter).deposit(
                IConvexV1BaseRewardPoolAdapter(adapter).pid(),
                amount,
                false
            );

        task.calls = task.calls.append(call);

        task.gasUsage += gasUsageByAdapter(
            boosterAdapter,
            curveLPToken,
            stakingToken
        );
    ...
    /// @dev Withdraws Staked convex position into Curvce token
    function depositCurve2StakedCvx(
        ...
        task.balances.subBalance(curveLPToken, amount);
        task.balances.addBalance(stakedPhantomToken, amount);
    ...
    function depositCvx2StakedCvx(
        ...
        bool isAll = task.balances.getBalance(cvxLPToken) == amount;

        task.balances.subBalance(cvxLPToken, amount);
        task.balances.addBalance(stakedPhantomToken, amount);

        MultiCall memory call = isAll
            ? ConvexV1_BaseRewardPoolMulticaller(adapter).stakeAll()
            : ConvexV1_BaseRewardPoolMulticaller(adapter).stake(amount);
    ...
    function depositAllTokens(address tokenOut, StrategyPathTask memory task)
        external
        view
        returns (uint256 amountOut, StrategyPathTask memory)
    {
        (
            address adapter,
            bool isStakedToken,
        ...
        uint256 curveLPBalance = task.balances.getBalance(curveLPToken);

        if (!isStakedToken && curveLPBalance > 1) {
            MultiCall memory call = ConvexV1_BoosterMulticaller(boosterAdapter)
                .depositAll(
                    IConvexV1BaseRewardPoolAdapter(adapter).pid(),
                    false
                );
            ...
            task.balances.subBalance(curveLPToken, curveLPBalance);
            task.balances.addBalance(tokenOut, curveLPBalance);

            amountOut = curveLPBalance;
            ...
            uint256 convexLPBalance = task.balances.getBalance(stakingToken);

            if (curveLPBalance < 2 && convexLPBalance > 1) {
                MultiCall memory call = ConvexV1_BaseRewardPoolMulticaller(
                    adapter
                ).stakeAll();

                task.calls = task.calls.append(call);

                task.balances.subBalance(stakingToken, convexLPBalance);
                task.balances.addBalance(tokenOut, convexLPBalance);

                amountOut = convexLPBalance;
            ...
            } else if (curveLPBalance > 1 && convexLPBalance < 2) {
                MultiCall memory call = ConvexV1_BoosterMulticaller(
                    boosterAdapter
                ).depositAll(
                        IConvexV1BaseRewardPoolAdapter(adapter).pid(),
                        true
            ...
            } else if (curveLPBalance > 1 && convexLPBalance > 1) {
                MultiCall[] memory calls = new MultiCall[](2);
                calls[0] = ConvexV1_BoosterMulticaller(boosterAdapter)
                    .depositAll(
                        IConvexV1BaseRewardPoolAdapter(adapter).pid(),
                        false
                    );
                calls[1] = ConvexV1_BaseRewardPoolMulticaller(adapter)
                    .stakeAll();

                task.balances.subBalance(curveLPToken, curveLPBalance);
                task.balances.subBalance(stakingToken, convexLPBalance);
                task.balances.addBalance(
                    tokenOut,
                    curveLPBalance + convexLPBalance
                );

                task.calls = task.calls.concat(calls);

                amountOut = curveLPBalance + convexLPBalance;
                ...
                task.gasUsage +=
                    ...
                    gasUsageByAdapter(adapter, stakingToken, tokenOut);
            }
        }

        return (amountOut, task);
    }
}
...
import { ILPWorker, ILPWorkerOptions } from "./ILPWorker.sol";

interface IDepositor is ILPWorker {
    function deposit(
        uint256 amountIn,
        address tokenOut,
        StrategyPathTask memory task
    ...
    function depositAll(address tokenOut, StrategyPathTask memory task)
        external
        view
        returns (uint256 amountOut, StrategyPathTask memory);
}

interface IDepositorOptions is ILPWorkerOptions {
    function deposit(
        TokenType ttIn,
        address tokenIn,
        uint256 amount,
...
interface IPathFinder is IGasPricer, IVersion {
    ...
    event ResolverComponentUpdateFailed(address);
...
import { StrategyPathTask, StrategyPathTaskOps } from "../data/StrategyPathTask.sol";
import { AddressList } from "../../libraries/AddressList.sol";
import { ILPWorkerOptions } from "../interfaces/ILPWorker.sol";
import { IConvexToken } from "../../integrations/convex/IConvexToken.sol";
import { IConvexV1BaseRewardPoolAdapter } from "../../interfaces/adapters/convex/IConvexV1BaseRewardPoolAdapter.sol";
import { TokenType, PathNotFoundException, PathNotFoundExceptionTyped } from "../interfaces/IPathFinder.sol";

abstract contract ConvexHelper is GasEstimator, ILPWorkerOptions {
    using AddressList for address[];
    using StrategyPathTaskOps for StrategyPathTask;

    error BoosterAdapterNotFound();

    address public immutable crv;
    IConvexToken public immutable cvx;

    constructor(address _booster) {
        crv = IBooster(_booster).crv();
        cvx = IConvexToken(IBooster(_booster).minter());
    }

    /// @dev Gets underlying token for desired LP token
    /// @param tokenOut ConvexLP token
    /// @param task StratgeyPathTask, used to get adapters
    /// @return tokensIn CurveLP token underlying the Convex pool; and, optionally, Convex LP token
    function getUnderlyings(
        ...
        view
        override
        returns (address[] memory tokensIn, StrategyPathTask memory)
    {
        (address adapter, bool isStakedToken, ) = findConvexAdapterByToken(
            tokenOut,
            task
        );
        ...
        } else if (ttIn == TokenType.CURVE_LP_TOKEN && !isStakedToken) {
            tokensIn = new address[](1);
            tokensIn[0] = IConvexV1BaseRewardPoolAdapter(adapter)
                .curveLPtoken();
        } else if (ttIn == TokenType.CURVE_LP_TOKEN && isStakedToken) {
        ...
        return (tokensIn, task);
    }

    function filterConvexAdapters(address[] memory adapters)
        public
        pure
        returns (address[] memory result, address boosterAdapter)
    {
        uint256 len = adapters.length; // F: [CNPF-1]
        result = new address[](len); // F: [CNPF-1]
        uint256 found;
        unchecked {
            for (uint256 i; i < len; ++i) {
                address adapter = adapters[i]; // F: [CNPF-1]

                if (adapter == address(0)) continue;
                AdapterType adapterType = IAdapter(adapter)
                    ._gearboxAdapterType();

                if (adapterType == AdapterType.CONVEX_V1_BASE_REWARD_POOL) {
                    result[found] = adapter; // F: [CNPF-1]
                    ++found; // F: [CNPF-1]
                }

                if (adapterType == AdapterType.CONVEX_V1_BOOSTER) {
                    boosterAdapter = adapter; // F:[CNPF-1]
                }
            }

            if (found < len) {
                return (result.copy(found), boosterAdapter); // F: [CNPF-1]
            }
        }
    }

    /// @dev Finds adapter by token
    /// @param token ConvexLP or StakedPhantom token
    /// @param task Current task object
    function findConvexAdapterByToken(
        address token,
        StrategyPathTask memory task
    )
        internal
    ...
    {
        boosterAdapter = task.getTokenDepositAdapter(token);
        adapter = task.getTokenWithdrawAdapter(token);

        if (boosterAdapter != address(0) && (adapter != address(0))) {
            if (
                token ==
                IConvexV1BaseRewardPoolAdapter(adapter).stakedPhantomToken()
            ) {
                return (adapter, true, boosterAdapter);
            } else {
                return (adapter, false, boosterAdapter);
            }
        }

        uint256 len = task.adapters.length;

        unchecked {
            for (uint256 i; i < len; ++i) {
                address currentAdapter = task.adapters[i]; //

                if (currentAdapter == address(0)) continue;
                AdapterType adapterType = IAdapter(currentAdapter)
                    ._gearboxAdapterType();

                if (adapterType == AdapterType.CONVEX_V1_BASE_REWARD_POOL) {
                    if (
                        token ==
                        IConvexV1BaseRewardPoolAdapter(currentAdapter)
                            .stakedPhantomToken()
                    ) {
                        adapter = currentAdapter;
                        isStakedToken = true;
                        if (boosterAdapter != address(0)) {
                            task.addTokenDepositAdapter(token, boosterAdapter);
                            task.addTokenWithdrawAdapter(token, adapter);
                            return (adapter, isStakedToken, boosterAdapter);
                        }
                    }
    ...
    /// @dev Computes how much CVX should be minted for provided amount in CRV
    function _computeCRV(uint256 amount) internal view returns (uint256) {
...
interface ILPWorker is IPathFinderComponent {
    function getUnderlying(address tokenOut, StrategyPathTask memory task)
        external
        view
        returns (address tokenIn, StrategyPathTask memory);
}

interface ILPWorkerOptions is IPathFinderComponent {
    function getUnderlyings(
        TokenType ttIn,
        address tokenOut,
        StrategyPathTask memory task
