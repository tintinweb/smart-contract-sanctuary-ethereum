...
import {CreatorEconomyLib} from "../libraries/CreatorEconomyLib.sol";
...
contract CreatorEconomyFacet {
     ...
     */
    function depositDonation(address _creatorAddress) external payable {
        CreatorEconomyLib.depositDonation(_creatorAddress);
    }
    ...
    function withdrawRevenue() external payable {
        CreatorEconomyLib.withdrawRevenue();
    }
}
...
import {CreatorLib} from "./CreatorLib.sol";

library CreatorEconomyLib {
     ...
     * @notice Creators can't deposit funds
     */
    function depositDonation(address _creatorAddress) internal {
        CreatorLib.Creator storage c = CreatorLib.getCreatorsStatsStorage().creators[_creatorAddress];
        CreatorLib.CreatorsStats storage cs = CreatorLib.getCreatorsStatsStorage();
        require(msg.sender != address(0));

        require(c.wallet != msg.sender, "Not Creator: Creators are not allowed to do this for themselves");
        ...
        c.revenue = c.revenue + msg.value;
        c.totalAmountCollected = c.totalAmountCollected + msg.value;
        cs.totalFundsCollected + msg.value;
        emit DonationDeposited(msg.sender, c.wallet, msg.value, block.timestamp);
    }

    /**
     * @dev Withdraw Revenue of a Creator (while paying the platformFee)
     */
    function withdrawRevenue() internal {
        require(msg.sender != address(0));

        CreatorLib.Creator storage c = CreatorLib.getCreatorsStatsStorage().creators[msg.sender];
        require(msg.sender == c.wallet, "Creator: You are not a creator");
        AppLib.AppStorage storage s = AppLib.getStorage();
        // require(c.revenue >= msg.value, "Balance: Insufficient balance");
        // require(address(this).balance >= msg.value, "Balance: Insufficient balance");

        uint256 fee = (msg.value * s.platformFee) / 100;
        uint256 revenueToWithdraw = msg.value - fee;
        uint256 balanceBeforeTransfer = address(this).balance;
        if (c.revenue >= msg.value && address(this).balance >= msg.value) {
            (bool payFee, ) = payable(s.platformFeeRecipient).call{value: fee}("");
            require(payFee);
            s.totalFeesCollected + fee;
            (bool withdraw, ) = payable(c.wallet).call{value: revenueToWithdraw}("");
            require(withdraw);
            c.revenue = c.revenue - msg.value;
        } else {
        ...
        emit RevenueWithdraw(msg.sender, msg.value, block.timestamp);
    }
}
