...
import './../Errors.sol';
import '@openzeppelin/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol';
contract SuperGalaticFactory is Ownable, ReentrancyGuardUpgradeable {
    
    mapping(uint256 => address) public nftContracts;
    mapping(address => bool) public isSuperGalatics;
    /**
     * @notice event when pool is created
     * @param nftIndex Index of the nft
     * @param nftContract address of the nft
     */
    event CreateSuperGalatic(uint256 indexed nftIndex, address indexed nftContract);
    ...
    constructor(address _admin, address _beacon) {
        ...
        for(uint256 index = 0; index < 10; index ++) {
            _createSuperGalaticNFTs(_admin, _beacon, index);
        }
    }

    function _createSuperGalaticNFTs(address _admin, address _beacon, uint256 _index) private {
        bytes memory empty;
        address _superGalatic = address(new BeaconProxy(_beacon, empty));
        ISuperGalatic(_superGalatic).initialize(_admin, address(this), _index);
        nftContracts[_index] = _superGalatic;
        isSuperGalatics[_superGalatic] = true;
        emit CreateSuperGalatic(_index, _superGalatic);
    }

    function mintSuperGalatic(uint256 categoryId, address minter) external nonReentrant {
        require(minter == msg.sender, Errors.ONLY_MINTER);
        address nftContract = nftContracts[categoryId];
        ISuperGalatic(nftContract).createGenesis(minter);
    }
...
interface ISuperGalatic {
    ...
    function initialize(address _admin, address _factory, uint256 _categoryId) external;
