...
contract FogDropERC721Implemention is
    Initializable,
    ERC721AUpgradeable,
    ...
    UUPSUpgradeable
{
    uint256 private _startTokenIdx;
    ...
    function initialize(
        ...
        ERC721AStorage.layout()._currentIndex = _startIdx;
        _startTokenIdx = _startIdx;
        maxMintAmountPerTx = _maxPreTx;
        maxMintAmountPreOwner = _maxPreWallet;
    ...
    modifier _check_mint_compliance(uint256 _mintAmount) {
        require(_msgSender().code.length == 0);
        require(
            cost == 0 || msg.value >= ownerToPay(_msgSender(), _mintAmount)
        );
        require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerTx);
        require(
            _numberMinted(_msgSender()) + _mintAmount <= maxMintAmountPreOwner
        );
        if (maxSupply > 0) {
    ...
    function ownerToPay(
        ...
        uint256 _mintedSoFar = _numberMinted(_owner);
        if (_mintedSoFar >= freeMintAmount) {
    ...
    ) public payable _check_mint_compliance(_mintAmount) {
        // Verify whitelist requirements
        require(whitelistMintEnabled == true);
        require(numberWhitelistMinted(_msgSender()) == 0);
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProofUpgradeable.verify(_merkleProof, merkleRoot, leaf));
    ...
    function mintTo(uint256 _mintAmount, address _receiver) public onlyOwner {
        require(maxSupply == 0 || totalSupply() + _mintAmount <= maxSupply);
        _safeMint(_receiver, _mintAmount);
    }

    /**
     * @dev The owner mint NFT for the receivers
     */
    function airdrop(
        address[] memory _recipients,
        uint256[] memory _amount
    ) public onlyOwner {
        require(_amount.length == 1 || _recipients.length == _amount.length);

        for (uint256 i = 0; i < _recipients.length; i++) {
            uint256 _mintamount = _amount.length == 1 ? _amount[0] : _amount[i];
            if (maxSupply > 0) {
                require(totalSupply() + _mintamount <= maxSupply);
            }
     ...
     */
    function numberWhitelistMinted(
        address _owner
    ) public view returns (uint256) {
    ...
    /**
     * @dev set free mint amount
     */
    function setFreeMintAmount(uint16 _freeMintAmount) public onlyOwner {
        freeMintAmount = _freeMintAmount;
    }
     ...
     */
    function setMaxMintAmountPreOwner(
        uint256 _maxMintAmountPreOwner
    ) public onlyOwner {
