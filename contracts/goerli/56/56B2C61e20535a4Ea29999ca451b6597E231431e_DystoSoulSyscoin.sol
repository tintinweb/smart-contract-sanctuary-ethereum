    ...
    ERC721Upgradeable
{
    ...
    function mint(
        ...
        require(
            _maxTokensPerWallet >= tokensMinted[msg.sender] + _numTokens,
            "Claim limit exceeded for this address."
        );

        if (paymentMethod == address(0)) {
            require(
                msg.value >= price * _numTokens,
                "Not enough SYS to purchase NFTs."
            );
        } else {
        ...
        require(
            signerAddress ==
                _messageHash.toEthSignedMessageHash().recover(signature),
    ...
    function mintTeam(address _receiver, uint256 _numTokens)
        external
        onlyOwner
    {
        tokensMinted[_receiver] += _numTokens;
    ...
    function setExpirationTimestamp(uint256 _expirationTimestamp)
        external
        onlyOwner
    {
        expirationTimestamp = _expirationTimestamp;
