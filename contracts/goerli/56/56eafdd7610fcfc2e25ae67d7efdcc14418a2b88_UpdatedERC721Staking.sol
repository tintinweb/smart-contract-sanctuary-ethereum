...
contract UpdatedERC721Staking is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    function earned(address account) public view returns (uint256) {
        return
            (noOfStakedTokens[account] *
                (block.timestamp - lastUpdateTime) * rewardRate) + rewards[msg.sender];
    }
    ...
    function walletRewardCheck(address account) external view returns (uint256){
        return rewardsToken.balanceOf(account);
    }
    ...
    /// @param _rewardRate The rate of the reward which will be distributed during the entire period until unstaked
    function setRewardRate(uint256 _rewardRate) external onlyOwner updateReward(address(0)) {
        require(_rewardRate != 0, "input reward rate cannot be 0");
        rewardRate = _rewardRate;
        emit RewardRateAdded(rewardRate);
    }
    ...
    event RewardRateAdded(uint256 rewardRate);
