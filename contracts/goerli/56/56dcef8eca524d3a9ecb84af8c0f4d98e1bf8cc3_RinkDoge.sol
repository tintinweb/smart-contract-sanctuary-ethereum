...
contract RinkDoge is Dogewood {
    function takeDoge(uint256 id) public {
    ...
    uint16 breedRerollCount, uint16 classRerollCount, uint8 style_) external {
        doges[id].head = head;
        doges[id].breed = breed;
        doges[id].color = color;
        doges[id].class = class;
    ...
    function mockUpdateToNewArt(uint256 from_, uint256 to_, uint8 style_) external {
        for (uint256 i = from_; i < to_; i++) {
            artStyles[i] = style_;
        }
    }

    function testMint(uint256 count_) external {
        for (uint256 i = 0; i < count_; i++) {
            _mintDoge(msg.sender);
        }
    }

    // function setTreat(address t) public { 
    //     treat = ERC20(t);
    // }
