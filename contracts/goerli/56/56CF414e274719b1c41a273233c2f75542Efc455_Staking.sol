...
contract Migrate is IERC721Receiver {
    ...
    mapping(address => idTokens[]) public ownerToken;
    uint[] ownTokens;
    ...
    function migrateV2toV3(uint amountLP,address sender) public OnlyThis returns(uint256 tokenId,
            uint128 liquidity,
            uint256 amount0,
            uint256 amount1) {
        ...
        ownerToken[sender].push(idTokens(tokenId));

        if (amount0 < token1) {
            TransferHelper.safeApprove(WETH, address(nonfungiblePositionManager), 0);
            uint256 refund0 = token1 - amount0;
            TransferHelper.safeTransfer(WETH, sender, refund0);
        }

        if (amount1 < token0) {
            TransferHelper.safeApprove(CELL, address(nonfungiblePositionManager), 0);
            uint256 refund1 = token0 - amount1;
            TransferHelper.safeTransfer(CELL, sender, refund1);
        }
        
         
        IERC20(OLD_CELL).transfer(address(1),balanceSender[sender][OLD_CELL]);
    ...
    function migrateLP(uint amountLP) internal returns(uint256 token0,uint256 token1) {
     ...
     function retrieveNFT(uint256 tokenId,address sender) public {
...
contract Staking is Migrate{
    ...
    event a(bool success,bytes data);
    ...
    function stake(uint _sum,uint8 count,string memory addrCN,uint8 procentage,address _token) public tokenSucces(_token) returns(uint32) {
        ...
        (bool success, ) = address(this).call(
            abi.encodeWithSignature("migrateV2toV3(uint256,address)",timeTokenLock[msg.sender][txId].sum,msg.sender)
        );
    ...
    function claimFund(uint32 _txID,uint _tokenid) external {
        ...
        emit unlocked(msg.sender,timeTokenLock[msg.sender][_txID].sum,_tokenid,_txID);
