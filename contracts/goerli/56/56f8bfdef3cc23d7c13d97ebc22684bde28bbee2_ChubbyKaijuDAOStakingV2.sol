...
contract ChubbyKaijuDAOStakingV2 is IChubbyKaijuDAOStakingV2, OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function setRarities(uint16[] calldata tokenIds, uint16 rewardType) public onlyOwner{
  ...
  function stakeTokens(address account, uint16[] calldata tokenIds) external whenNotPaused nonReentrant _updateEarnings {
    ...
    for (uint16 i = 0; i < tokenIds.length; i++) {
      ...
      uint16 trait = (tokenIds[i] <= 6666 ? 1 : tokenIds[i] <= 9999 ? 1 : 2);// chubbykaijuGen2.traits(tokenIds[i]); 
      _stakeGEN2(account, tokenIds[i], trait);
      chubbykaijuGen2.transferFrom(msg.sender, address(this), tokenIds[i]);
    }
  
  }

  function _stakeGEN2(address account, uint16 tokenId, uint16 trait) internal {
    if(trait == 0){
      totalZombieStaked += 1;
    }else if(trait == 1){
      totalAlienStaked += 1;
