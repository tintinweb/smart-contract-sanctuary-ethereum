...
contract vMiner is IvMINER, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgMINER public gMINER; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked MINER", "vMINER", 9) ERC20Permit("Staked MINER") {
    ...
    function setgMINER(address _gMINER) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gMINER) == address(0), "gMINER:  gMINER already set");
        require(_gMINER != address(0), "gMINER:  gMINER is not a valid contract");
    ...
    /**
        @notice increases rMINER supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs vMINER of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized vMINER
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "vMINER: insufficient balance");
    }
    ...
    // toG converts an vMINER balance to gMINER terms. gMINER is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gMINER.balanceTo(amount);
    }

    // fromG converts a gMINER balance to vMINER terms. vMINER is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gMINER.balanceFrom(amount);
    }

    // Staking contract holds excess vMINER
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply
                .sub(balanceOf(stakingContract))
                .add(gMINER.balanceFrom(IERC20(address(gMINER)).totalSupply()))
