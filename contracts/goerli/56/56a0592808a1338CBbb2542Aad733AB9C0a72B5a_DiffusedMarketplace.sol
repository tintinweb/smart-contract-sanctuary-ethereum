...
contract DiffusedMarketplace is ReentrancyGuard, Ownable {
    ...
    function withdrawProceeds() public nonReentrant {
        ...
        uint256 totalFee = totalProceeds / 30;
        uint256 finalSum;

        s_proceeds[msg.sender] = 0;

        if (msg.sender != owner()) {
            s_proceeds[address(owner())] += totalFee;
            finalSum = totalProceeds - totalFee;
        } else {
            finalSum = totalProceeds;
        }
        // Fee of 3.33% locks in the contract
