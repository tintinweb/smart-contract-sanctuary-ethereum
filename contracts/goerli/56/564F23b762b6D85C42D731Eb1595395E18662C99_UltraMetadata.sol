...
contract UltraMetadata is Ownable, IForcedOfflineMetadata {
    ...
    function updateDNA(uint from_, uint to_, uint[] calldata dnas_) external onlyOwner {
        require(to_ > from_, "invalid position");
        require(dnas_.length == (from_ - to_), "Length of DNA doesn't match position");
