...
contract AuctionRaffle is Ownable, Config, BidModel, StateModel {
    ...
    function bidWithDiscount(
        ...
        require(MerkleProof.verify(proof, discountRoot, leaf), "AuctionRaffle: discount proof invalid");
        makeBid();
    ...
    function claimProceeds() external onlyOwner onlyInState(State.RAFFLE_SETTLED) {
        ...
        // because there could be discounts applied on raffle winners, we need to subtract discounts
...
abstract contract Config {
    ...
    bytes32 public immutable discountRoot;
...
library MaxHeap {
    ...
    function increaseKey(uint256[] storage heap, uint256 oldValue, uint256 newValue) internal {
        uint256 index = findKey(heap, oldValue);
        increaseKeyAt(heap, index, newValue);
    }

    function increaseKeyAt(uint256[] storage heap, uint256 index, uint256 newValue) internal {
