...
contract ADAO is ADTradeToken {
     ...
     */
    constructor(address dispatcher, address pancakeRouter)
        public
        ADTradeToken(dispatcher,pancakeRouter)
...
abstract contract ADTradeToken is ListenTradeToken {
    ...
    constructor(address dispatcher, address pancakeRouter) public {
        TradeDispatcher = IADTradeDividendDipatcherInterface(dispatcher);

        PancakeRouter = IPancakeRouter02(pancakeRouter);

        // if (openHook) {
        //     addHookETHSwapWithUniswapRouterV2(pancakeRouter);
        // }
        ...
        if (openHook) {
            if (whiteList[sender] || whiteList[recipient]) {
                ...
                if (hookAddress[recipient]) {
                    ...
                    TradeDispatcher.dispatchSellDividend(
                        sender,
                        dispatchAmount
                    );
                    ...
                    TradeDispatcher.dispatchBuyDividend(sender, dispatchAmount);
                } else {
                    super._transfer(sender, recipient, amount);
                }
            }
    ...
    function addHookETHSwapWithUniswapRouterV2(address routerAddress)
        public
        onlyOwner
    {
        ...
        _doAddHookAddress(_uniswapV2Pair);
    }

    function addHookTokenSwapWithUniswapRouterV2(
