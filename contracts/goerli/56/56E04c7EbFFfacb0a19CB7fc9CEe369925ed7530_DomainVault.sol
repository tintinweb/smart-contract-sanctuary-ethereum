...
contract DomainVault is IDomainVault, ERC721Holder, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    *  @notice Buy presale fragment using ETH
    ...
    function initialize(address fragment_,
        ...
        //
        WETH = WETH_;
        //TODO: refactor code, votePassed = false;
        ballotBox = 0;
    ...
    function firstBidUsingETHAndWETH(uint256 currentBidPrice) external payable whenNotPaused nonReentrant {
        ...
        //最小竞拍出价比例
        uint256 bidIncreasePercentage = getBidIncreasePercentage();
        ...
        require(voteEndTimeMapping[ballotBox] == 0 || (voteEndTimeMapping[ballotBox] < _currentTime() && !votePassedMapping[ballotBox]), "DomainVault: voting or voted passed, can't make bid");

        //竞拍结束或未开始（提示：竞拍中，不能发起买断）
        require(auctionEndTimeMapping[ballotBox] < _currentTime(), "DomainVault: auctioning, can't make bid");
        ...
        emit FirstBid(_msgSender(), currentBidPrice);
    }
    ...
    function firstBid(uint256 currentBidPrice) external payable whenNotPaused nonReentrant {
        ...
        //投票未开始 或 结束,且投票未通过
        require(voteEndTimeMapping[ballotBox] == 0 || (voteEndTimeMapping[ballotBox] > 0 && voteEndTimeMapping[ballotBox] < _currentTime() && !votePassedMapping[ballotBox]), "DomainVault: voting or voted passed, can't make offer bid");
    ...
    function vote() external whenNotPaused nonReentrant {
        ...
        //是否已经投票通过
        require(!votePassedMapping[ballotBox], "DomainVault: vote passed, can't vote");

        //投票已开始
        require(voteEndTimeMapping[ballotBox] > 0, "DomainVault: voting has not begun");

        //投票未结束
        require(voteEndTimeMapping[ballotBox] > _currentTime(), "DomainVault: vote closed");

        //竞拍未开始
        require(auctionEndTimeMapping[ballotBox] == 0, "DomainVault: auctioning, can't vote");

        bool voted_ = _whetherVotedMapping[ballotBox][_msgSender()];
        ...
        //用户余额大于0（提示：用户票数不足）
        require(fragmentBalance > 0, "DomainVault: insufficient fragment votesMapping[ballotBox]");
    ...
    function withdrawBidFunds() external payable whenNotPaused nonReentrant {
        ...
        uint256 bidPrice_ = bidPriceMapping[ballotBox];
    ...
    function claimNFT() external whenNotPaused nonReentrant {
        ...
        //有投票(进入过投票阶段)
        require(voteEndTimeMapping[ballotBox] > 0, "DomainVault: voting, can't withdraw bid funds");

        //投票通过
        require(votePassedMapping[ballotBox], "DomainVault: vote no passed, can't claim NFT");

        //竞拍已结束(进入过竞拍阶段，并竞拍已结束)
        require(auctionEndTimeMapping[ballotBox] > 0 && auctionEndTimeMapping[ballotBox] < _currentTime(), "DomainVault: auctioning, can't claim NFT");
    ...
    function claimFunds(uint256 withdrawERC20Amount) public whenNotPaused nonReentrant {
        ...
        //投票通过
        require(votePassedMapping[ballotBox], "DomainVault: vote passed, can't claim funds");
    ...
    //最小竞拍出价比例
    function getBidIncreasePercentage() private returns (uint256) {
