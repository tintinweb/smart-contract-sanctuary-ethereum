...
contract NFTProxyDemo is ERC721A, IReap3rMint {

    string private _baseTokenURI;

    address public _proxySaleAddress;

    uint256 public _teamReservedQuantity = 0;

    uint256 public immutable MAX_TOTAL_SUPPLY = 7000;

    uint256 public immutable MAX_TEAM_HOLD = 334;

    constructor(string memory baseTokenURI, address proxySaleAddress) ERC721A("Test NFT", "TNFT", 100) {
    ...
    function teamAirdrop(address to, uint256 num) public virtual onlyOwner{
        ...
        require(MAX_TEAM_HOLD - totalSupply() >= num, "Airdrop Max supply reached");

        _safeMint(to, num);

        _teamReservedQuantity += num;
    }
