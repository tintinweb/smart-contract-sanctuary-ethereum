...
contract Auction is Ownable{
    ...
    event CreatedAuction(uint auctionID, address _token, uint _tokenAmount, uint _tokenType, uint _endTime, uint256 time, uint _bidPrice);
    ...
    mapping(uint => AuctionStruct) public auctionInfo;
      ...
      function createAuction(address _token, uint _tokenAmount, uint _tokenType, uint _endTime, uint _bidPrice) onlyOwner payable public {
        ...
        if(auctionInfo[auctionID].tokenType == 0){
            ...
            IERC721(auctionInfo[auctionID].token).transferFrom(msg.sender, address(this), auctionInfo[auctionID].tokenAmount);
        }
        auctionID++;
        emit CreatedAuction(auctionID, _token, _tokenAmount, _tokenType, _endTime, block.timestamp, _bidPrice);
    }
    ...
    function bid(uint _auctionID) payable public {
        ...
        require(msg.value >= auctionInfo[_auctionID].bidPrice, "Please send proper amount");
        uint bidCount = msg.value / auctionInfo[_auctionID].bidPrice;
    ...
    function claim(uint _auctionID) payable public {
        ...
        uint remainingValue = ( msg.value + ( auctionInfo[_auctionID].remainingBid * auctionInfo[_auctionID].bidPrice ) - ( auctionInfo[_auctionID].bidCount * auctionInfo[_auctionID].bidPrice ) );
