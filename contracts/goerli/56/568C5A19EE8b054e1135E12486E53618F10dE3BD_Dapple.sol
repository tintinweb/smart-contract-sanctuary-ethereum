...
contract Dapple is ERC20, ERC20Burnable, Ownable  {
    ...
    constructor(address _treasuryWalletAddress) ERC20("Dapple", "DAP") {
    ...
    function setFee(uint8 newBuyFee,uint8 newSellFee,uint8 newTresuryFee) external onlyOwner {
        feeInfo = feeStore({
            buyFee: newBuyFee,
            sellFee: newSellFee,
            treasuryFee: newTresuryFee
        });
    }

    function setTreasuryWallet(address wallet) external onlyOwner{
        require(wallet != address(0), "Dapple: newAddress is a zero address");
    ...
    function _transfer(
        ...
        if(takeFee) {
            uint256 exchangeFee = getFees(from,to);
        	uint256 totalFees = amount * feeInfo.treasuryFee / 100;

            super._transfer(from, treasuryWalletAddress, totalFees);

            if(exchangeFee > 0) {
                uint256 exchangeToken = amount * exchangeFee /  100;
                super._transfer(from, address(this), exchangeToken);
                totalFees = totalFees + exchangeToken;
            }
    ...
    function excludeFromFees(address account, bool status) public onlyOwner {
        require(_isExcludedFromFees[account] != status, "Dapple: Account is already the value of 'excluded'");
    ...
    function getFees(address sender,address receiver) internal view returns (uint256) {
        if(sender == uniswapV2Pair){
            return feeInfo.buyFee;
        }else if(receiver == uniswapV2Pair){
            return feeInfo.sellFee;
        }else {        
            return 0;
        }
    }
