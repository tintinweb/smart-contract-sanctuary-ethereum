...
contract PureFriends is ERC20, Ownable {
    ...
    address public constant USDT = //0xdAC17F958D2ee523a2206206994597C13D831ec7; //Mainnet USDT ERC20
                                0xD87Ba7A50B2E7E660f678A895E4B72E7CB4CCd9C ;// testnet usdc
    ...
    address public  developmentWallet = 0x03e04b02145c0Aca867BC9e0d470ECdD4Fe99454;
    address public marketingWallet = 0x31aa6F19c00fb43114bc3aCd5C0e525dF9e7Eaa8; 
    ...
    function setFee(uint256 _marketingFee, uint256 _developmentFee) public onlyOwner {
        marketingFee = _marketingFee;
        developmentFee = _developmentFee;
        require (marketingFee + developmentFee < 15, "Max Tax limit is 15% ");
    }

    function setMaxSellTransaction(uint256 _maxSellTxAmount) public onlyOwner {
        require (_maxSellTxAmount >= totalSupply()/100, "max sell tx should be greator than equal to 1% of the supply");
        maxSellTransactionAmount = _maxSellTxAmount;
       
    }
    
    function setMaxBuyTransaction(uint256 _maxBuyTxAmount) public onlyOwner {
        require (_maxBuyTxAmount >= totalSupply()/100, "max buy tx should be greator than equal to 1% of the supply");
    ...
    function setMaxWalletAmount (uint256 amount) external onlyOwner {
        require (amount >= totalSupply()/100, "Max wallet amount should be more than 1 percent of the supply");
    ...
    function excludeFromMaxWallet (address _user, bool value) public onlyOwner {
    ...
    function _transfer(
        ...
        if(!automatedMarketMakerPairs[to] && !_isExcludedFromMaxWallet[from] && !_isExcludedFromMaxWallet[to]) {
            require (amount + balanceOf(to) <= maxWalletAmount, "amount exceeds maxwallet Limit");
        }
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromMaxTx[from] && automatedMarketMakerPairs[to]) {
            require(amount <= maxSellTransactionAmount, "amount exceeds the maxSellTransactionAmount.");
        	uint256 _marketingFees = amount.mul(marketingFee).div(100);
            uint256 _developmentfees = amount.mul(developmentFee).div(100);
        	amount = amount.sub(_marketingFees.add(_developmentfees));
            super._transfer(from, address(this), _marketingFees + _developmentfees); 
    ...
    function swapAndSendToWallets (uint256 tokens) private lockTheSwap {
        ...
        uint256 marketingPart = newBalance.mul(marketingFee).div(marketingFee + developmentFee);
        swapETHforUSDT(marketingWallet, marketingPart);
        uint256 developmentPart = newBalance - marketingPart;
