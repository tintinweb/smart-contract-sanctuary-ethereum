...
contract AutoSniper is Ownable {
    ...
    function setUserMaxTip(uint256 maxTipInWei) external {
        if (maxTipInWei < minimumTip) revert TipBelowMinimum();
    ...
    function setUserAllowedNfts(bool guardEnabled, bool nftAllowed, address[] calldata nfts) external {
        sniperGuardrails[msg.sender].nftGuardEnabled = guardEnabled;
        for (uint256 i = 0; i < nfts.length;) {
            sniperGuardrails[msg.sender].allowedNfts[nfts[i]] = nftAllowed;
            unchecked { ++i; }
        }
    }
    ...
    function _transferNftToSniper(ItemType tokenType, address tokenAddress, uint256 tokenId, address sniper) private {
        if (tokenAddress == 0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB) { //cryptopunks
            IPunk(0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB).transferPunk(sniper, tokenId);
    ...
    function _checkSniperGuardrails(address tokenAddress, address marketplace, uint256 tip, address sniper) private view {
        ...
        if (guardrails.nftGuardEnabled && !guardrails.allowedNfts[tokenAddress]) revert TokenContractNotAllowed();
    }
...
struct SniperGuardrails {
    ...
    bool maxTipGuardEnabled;
    bool nftGuardEnabled;
