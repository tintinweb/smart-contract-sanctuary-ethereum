...
contract MandoxGemEarn is Ownable, Pausable {
    ...
    */
    ...
    function totalPendingOwedOLD(uint256[] calldata tokenIds ) external view returns(uint256 allRewards) {
        for (uint i = 0; i < tokenIds.length; i++) {
            allRewards += _allPending(tokenIds[i]);
        }      
    }

    function totalPendingOwed(address _owner) external view returns(uint256 allRewards) {
        uint256[] memory tokenIds = cyborx.tokensOfOwner(_owner);
        for (uint i = 0; i < tokenIds.length; i++) {
            allRewards += _allPending(tokenIds[i]);
        }      
    }
