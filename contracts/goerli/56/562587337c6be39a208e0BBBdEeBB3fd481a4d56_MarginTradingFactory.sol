...
contract MarginTradingFactory is Ownable, IMarginTradingFactory {
    ...
    function getCreateMarginTradingAddress(uint256 _num, uint8 _flag, address _user)
        external
        view
        returns (address _ad)
    {
...
// SPDX-License-Identifier: AGPL-3.0
pragma solidity ^0.8.15;
import {ILendingPool, ILendingPoolAddressesProvider, IFlashLoanReceiver} from "../aaveLib/Interfaces.sol";
...
interface IMarginTradingFactory {
    ...
    function getUserMarginTradingNum(address _user)external
        view
        returns (uint256 _crossNum, uint256 _isolateNum);
