...
contract YOUNG_LEGGENDES_HIDDDEND_DEF is ERC721A, ERC721ABurnable, Ownable {
    using Strings for uint256;
    using ECDSA for bytes32;
    //using SafeMath for uint256;

    constructor() ERC721A("YOUNG LEGGENDES HIDDDEND6", "YLHH") {
        _signerAddress = 0x152683eCA9EbC4Df49a77bfEcc5914c6A512Ed69;
    }
    string public baseURI = "https://storage.googleapis.com/themagictest/metadata/";
    
    uint public MAX_MINTS = 10000;
    uint public constant PRICE = 0.01 ether;
    uint public constant MAX_PER_MINT = 5;
    uint public constant MAX_PER_MINT_WL = 5;
    
    // uint public WLStartTime = 1657216800;
    // uint public PublicStartTime = 0;//9999999999;

    mapping(bytes => uint) public signatureUsed;
    ...
    // PUBLIC_STARTED
    function setStartPublicMint(bool _status) public onlyOwner {
        PUBLIC_STARTED = _status;
    }

    function setStartWhitelistMint(bool _status) public onlyOwner {
        WHITELIST_STARTED = _status;
    }
    ...
    /*function whitelistMint(bytes memory signature) public {
        require(block.timestamp >= WLStartTime, "Sale not started");
        require(totalMints + 1 < MAX_MINTS, "All MINTS have already been minted!");
    ...
    function whitelistMint(uint _count, bytes memory signature) public payable {
        ...
        // require(block.timestamp >= WLStartTime, "Sale not started");
        require(WHITELIST_STARTED == true, "Sale not started");
        require(totalMints + _count < MAX_MINTS, "All MINTS have already been minted!");
        ...
        require(verifyAddressSigner(signature), "Invalid signature");
        require(signatureUsed[signature] + _count <= MAX_PER_MINT, "All mints of this signature has already been used.");
    ...
    function mint(uint _count) external payable {
        ...
        require(PUBLIC_STARTED == true, "Sale not started");
        require(totalMints + _count <= MAX_MINTS, "Not enough NFTs left!");
    ...
    function ownerMint(uint amount) public onlyOwner {
        require(totalMints + amount < MAX_MINTS, "All MINTS have already been minted!");
