...
contract COANFTProxy is Ownable {
    ...
    function digitalArtCreate(
        ...
        require(
            contractFunctionData_.length > 0,
            "NFT contract function data is empty"
        );
        coa.createCertificate(to_, certificate_, signature_);
    ...
    function digitalArtTransfer(
        ...
        require(nftContract_.isContract(), "NFT contract is not a contract");
        coa.safeTransferFrom(certFrom_, to_, certificateId_, 1, "");
    ...
    // we are transfering certificate to the physical art owner
    /*
     * caller must be coa creator
     */
    ...
    function _payment(Payment calldata payment_, bytes32 dataHash_) internal {
        ...
        address signer = payment_.nonce.toEthSignedMessageHash().recover(
            payment_.signature
        );
        require(signer == owner(), "Invalid signature");
        require(!payments[dataHash_], "Payment already processed");
        (bool success, ) = owner().call{value: payment_.amount}("");
