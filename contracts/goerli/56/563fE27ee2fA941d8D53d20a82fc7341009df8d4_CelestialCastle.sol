...
contract CelestialCastle is Ownable, IERC721Receiver{
  ...
  FnGLike public freaksNGuilds;
  /// @notice Freaks bucks token contract.
  FreaksBucksLike public freaksBucks;
  ...
  function initialize(
    ...
    freaksNGuilds = FnGLike(newFreaksNGuilds);
    freaksBucks = FreaksBucksLike(newFreaksBucks);
  }

  /// @notice Travel tokens to L2.
  function travel(uint256[] calldata freakIds, uint256[] calldata celestialIds, uint256 fbxAmount) external {
    bytes[] memory calls = new bytes[]((freakIds.length > 0 ? 1 : 0) + (celestialIds.length > 0 ? 1 : 0) + (fbxAmount > 0 ? 1 : 0));
    uint256 callsIndex = 0;

    if(freakIds.length > 0){
      FnGLike.Freak[] memory freaks = new FnGLike.Freak[](freakIds.length);
    ...
    if(celestialIds.length > 0){
      FnGLike.Celestial[] memory celestials = new FnGLike.Celestial[](celestialIds.length);
  ...
  function retrieveFreakIds(
    ...
    FnGLike.Freak[] calldata freakAttributes
  ) external onlyPortal {
    for(uint256 i = 0; i < freakIds.length; i++) {
      delete ownerOf[freakIds[i]];
      FnGLike(fng).transferFrom(address(this), owner, freakIds[i]);
      FnGLike(fng).setFreakAttributes(freakIds[i], freakAttributes[i]);
    }
  }
  ...
  function retrieveCelestialIds(
    ...
    FnGLike.Celestial[] calldata celestialAttributes
  ) external onlyPortal {
    for(uint256 i = 0; i < celestialIds.length; i++) {
      delete ownerOf[celestialIds[i]];
      FnGLike(fng).transferFrom(address(this), owner, celestialIds[i]);
      FnGLike(fng).setCelestialAttributes(celestialIds[i], celestialAttributes[i]);
    }
  }
  ...
  function retrieveBucks(
    ...
    FreaksBucksLike(fbx).mint(owner, value);
  }
