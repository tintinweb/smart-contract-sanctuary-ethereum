...
contract NFATreasury is ReentrancyGuard {
    ...
    constructor () {
        ...
        daoContract = IdaoContract(0x11fE4B6AE13d2a6055C8D9cF65c55bac32B5d844); //Goerli NFA
        validTokens = [0x11fE4B6AE13d2a6055C8D9cF65c55bac32B5d844]; //Goerli NFA
    }
   ...
   function buyTokenWithEth(address _tokenToBuy, uint256 _amount, string memory _symbol, uint256 _tokenPrice) public onlyOwner {
        ...
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: (_amount) }(5, path, address(this), block.timestamp + 15);

        nextPosition++;
   }

   function sellSpecificTokenForEth(address _tokenToSell) public onlyOwner {
   ...
   function buyBackAndBurn(uint256 _amount, address _tokenToBuy) public onlyOwner {

        burn storage newBurn = Burns[nextBurn];
        newBurn.id = nextBurn;
        newBurn.exists = true;
        newBurn.amountOfEth = _amount;
