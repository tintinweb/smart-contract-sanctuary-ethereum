...
contract Staking is Ownable, Pausable {
    ...
    constructor(address _usdt, address _router, address _weth) {
        ...
        stakers[msg.sender].parent = msg.sender;
        // stakers[msg.sender].contractEndTime = block.timestamp + 300 days;
    ...
    function stakeAble(address _user) public view returns(bool isAble) {
        ...
        if(rank !=0 && stakeAmounts[rank-1]*maxRate <= stakers[_user].earn){
            isAble = true;
        } else if(rank ==10 && stakeAmounts[10]*maxRate <= stakers[_user].earn){
            isAble = true;
    ...
    function changeMaxRate(uint8 _newRate) public onlyOwner {
        maxRate = _newRate;
    }
    ...
    function stakeUsdt(address _ref) public whenNotPaused {
        ...
        require(_ref != msg.sender, "You cant put your address as inviter !");
        require(stakers[_ref].parent != address(0), "The referal address is not a staker in our app");
        ...
        if(stakeAmounts[parent1Rank]*maxRate <= stakers[parents[0]].earn &&
        ...
        if(stakeAmounts[parent2Rank]*maxRate <= stakers[parents[1]].earn &&
        ...
        if(stakeAmounts[parent3Rank]*maxRate <= stakers[parents[2]].earn &&
    ...
    function stakeEth(address _ref) public payable whenNotPaused {
        ...
        require(_ref != address(0), "you should have referral address");
        require(_ref != msg.sender, "You can not put your address as inviter !");
        require(stakers[_ref].parent != address(0), "The referal address is not a staker in our app");
        uint8 rank = stakers[msg.sender].rank;
        uint stakeAmount = stakeAmounts[rank];
        require(stakeAble(msg.sender), "You can not stake again until your rank be increased" );
