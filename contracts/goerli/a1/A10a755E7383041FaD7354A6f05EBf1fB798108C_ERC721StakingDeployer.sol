...
abstract contract ERC20Contract {
    function addToAllowlist(address _address) external virtual;
...
contract ERC721StakingDeployer is Ownable, Referable, ReentrancyGuard {
    ...
    Deployment[] private contracts;

    constructor(address _r2eAddress, address _launchpassAddress, address payable _treasuryAddress) Referable(_r2eAddress) {
    ...
    function setDiscount(uint256 _launchpassId, uint16 _discount) external onlyOwner {
        require(_discount >= 0 && _discount <= 100, "Invalid discount");
        if (_discount > 0) {
            discounts[_launchpassId] = _discount;
        } else {
            delete discounts[_launchpassId];
        }
    }

    function getPrice(uint256 _launchpassId) external view returns (uint256) {
        return price - (price * discounts[_launchpassId]/100);
    }
    ...
    function deploy(
        ...
        // authorize
        require(price - (price * discounts[_launchpassId]/100) <= msg.value, "ETH incorrect");
        require(IERC721(launchpassAddress).ownerOf(_launchpassId) == msg.sender,  "Not owner");

        // deploy contracts
        address _erc20Contract = ERC20Factory(erc20Deployer).deploy(_tokenParams.name, _tokenParams.symbol, _tokenParams.maxSupply);
        address _vaultContract = ERC721VaultFactory(vaultDeployer).deploy(_vaultParams.softStaking, _vaultParams.hardStaking);
        address _rewardContract = ERC721RewardsFactory(rewardDeployer).deploy(_erc20Contract, _vaultContract, _rewardParams.logoUrl, _rewardParams.backgroundUrl, _rewardParams.colors);
        ...
        ERC20Contract(_erc20Contract).addToAllowlist(_rewardContract);
        ...
        // transfer ownership
        // ERC20Contract(_erc20Contract).transferOwnership(msg.sender);
        ...
        // log deployments
        // Deployment memory _deployment = Deployment(_launchpassId, _erc20Contract, _vaultContract, _rewardContract);
        // contracts.push(_deployment);

        // // process payouts
        // if (msg.value > 0) {
        //     if (_referrer != address(0)) Referable.payReferral(msg.sender, _referrer, 1, msg.value);
