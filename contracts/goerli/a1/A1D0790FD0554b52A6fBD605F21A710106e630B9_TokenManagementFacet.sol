...
/******************************************************************************\
/// @title Contract Token Management Facet - manage registered tokens
/// @author Ather Labs
...
contract TokenManagementFacet {
    ...
    event TokenPausedUpdate(uint8 lisitingId, bool paused);

    /// @notice
    event NewToken(address newTokenAddress, uint8 newTokenType, uint256 minLiquidity, uint8 newLisitingId);

    /// @notice
    event SetTokenLimits(uint256 minLiquidity, uint88 limitPerTx, uint8 listingId);

    /* ------ EXTERNAL FUNTIONS ------ */
    /// @notice Pause token deposits for the given token
    /// @param _listingId Token's listingId
    /// @param _tokenPaused Token paused status
    function setTokenPaused(uint8 _listingId, bool _tokenPaused) external {
        LibAccessControl._requireGov();
        if (s.pausedTokens[_listingId] != _tokenPaused) {
            s.pausedTokens[_listingId] = _tokenPaused;
            emit TokenPausedUpdate(_listingId, _tokenPaused);
        }
    }

    /// @notice Add token to the list of registered tokens
    /// @notice Make sure every ERC1155 token supports the IERC1155M interface
    /// @param _tokenAddress Token address
    /// @param _tokenType Token type
    /// @param _minLiquidity the balance minimum limit of this token, 0 mean no limit.
    /// @param _newListingId Token's listing Id
    /*
     * tokenType[listingId] == 0 is native token by default
     * tokenType[listingId] == 1 is ERC20 token
     * tokenType[listingId] == 2 is ERC721 token
     * tokenType[listingId] == 3 is ERC1155 token
     * tokenType[listingId] == 4 is ERC1155C token (for classic Sipher spaceship collections)
     */
    function addToken(
        ...
        LibAccessControl._requireGov();
        require(_tokenType != 0, "TMF: Invalid token type");
        require(_newListingId != 0, "TMF: Invalid listing ID");
        require(s.tokenAddresses[_newListingId] == address(0), "TMF: Contract with listing ID already exists");
        require(s.listingIds[_tokenAddress] == 0, "TMF: Token already exists");

        s.listingIds[_tokenAddress] = _newListingId;
        s.tokenAddresses[_newListingId] = _tokenAddress;
        s.tokenTypes[_newListingId] = _tokenType;
        s.minLiquidity[_newListingId] = _minLiquidity;
        emit NewToken(_tokenAddress, _tokenType, _minLiquidity, _newListingId);
    }
    ...
    function validateTokenAddress(address _tokenAddr) external view returns (uint8) {
        require(_tokenAddr != address(0), "TMF: Invalid token address");
        uint8 listingId = s.listingIds[_tokenAddr];
        require(listingId != 0, "TMF: Listing ID not found");
        return listingId;
    }
    ...
    function validateListingId(uint8 _listingId) external view returns (address) {
        require(_listingId != 0, "TMF: Invalid listing ID");
        address tokenAddr = s.tokenAddresses[_listingId];
        require(tokenAddr != address(0), "TMF: Token address not found");
        return tokenAddr;
    }
}
