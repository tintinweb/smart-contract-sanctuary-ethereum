...
contract ArtDrive is ArtDriveEvent, OwnableUpgradeable, ERC721EnumerableUpgradeable {
    ...
    function freeMintAmount(address _player) external view returns (uint256) {
        if (allowedMintAmount[_player] <= minted[_player]) return 0;
        return allowedMintAmount[_player] - minted[_player];
    }

    // ============= write function =============

    function mint(address _player, uint256 _amount) external payable returns (uint256[] memory) {
        if (msg.sender != owner() && minted[_player] >= allowedMintAmount[_player]) {
            require(msg.value >= (price * _amount), "ArtDrive: msg.value Deficiency");
        }
    ...
    function claimMintAmount(
        ...
        require(allowedMintAmount[_recipient] == 0, "ArtDrive: Already received");
