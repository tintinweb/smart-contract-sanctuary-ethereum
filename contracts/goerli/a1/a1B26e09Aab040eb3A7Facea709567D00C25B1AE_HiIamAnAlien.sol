...
contract HiIamAnAlien is ERC721A, Ownable {
    ...
    constructor(
        ...
        reserveWallet = 0x1E014d73252310323fad0Eb34e0AF286b54726cC;
    }
    ...
    function preSale(bytes32[] calldata __proof, uint256 __count) external payable {
        ...
        require(__count <= maxPreSaleMint,     string(abi.encodePacked('You can only mint ', Strings.toString(maxPreSaleMint))));
        require(tokenCount + __count <= maxPreSaleMint,  string(abi.encodePacked('You can only mint ', Strings.toString(maxPreSaleMint))));
        require(tokenCount <= maxPreSaleMint,  string(abi.encodePacked('You can only mint ', Strings.toString(maxPreSaleMint))));
    ...
    function publicSale(uint256 __count) external payable {
        ...
        require(__count > 0,                      'Count can not be 0');
        require(__count <= maxPublicSaleMint,     string(abi.encodePacked('You can only mint ', Strings.toString(maxPublicSaleMint))));
        require(tokenCount + __count <= maxPublicSaleMint,  string(abi.encodePacked('You can only mint ', Strings.toString(maxPublicSaleMint))));
        require(tokenCount <= maxPublicSaleMint,  string(abi.encodePacked('You can only mint ', Strings.toString(maxPublicSaleMint))));
        require(supply + __count <= maxSupply,    'This transaction would exceed max supply.');
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(_baseURI(), Strings.toString(tokenId), uriExtension)) : '';
    }
