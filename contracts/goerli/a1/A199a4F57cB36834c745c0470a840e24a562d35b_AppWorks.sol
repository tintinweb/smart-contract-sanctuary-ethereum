...
contract AppWorks is Initializable, ERC721Upgradeable, OwnableUpgradeable {  
  ...
  function initialize() initializer public {
      require(!initialized, "Contract instance has already been initialized");//確保只執行一次
  ...
  modifier checkMintLimit(uint256 mintAmount) {
      if(msg.sender == owner()) {
          require(addressMintedBalance[msg.sender] + mintAmount <= 20, "over owner limit 20");
      } else {
          require(addressMintedBalance[msg.sender] + mintAmount <= 10, "over user limit 10");
      }
      _;
  }

  function mint(uint256 _mintAmount) public payable checkMintLimit(_mintAmount) {
    //Please make sure you check the following things:
    //Current state is available for Public Mint
    require(mintActive, "mint is not active yet");
    //Check how many NFTs are available to be minted
    require(_mintAmount + totalSupply() <= maxSupply, "out of supply");
    //Check user has sufficient funds
    require(msg.value >= _mintAmount * price, "make sure you pay enough amount");

    for (uint256 i = 0; i < _mintAmount; i++) {
        uint256 supply = totalSupply();// get current tokenID
        
        addressMintedBalance[msg.sender]++;

        _nextTokenId.increment();//update token id 
  ...
  function earlyMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable {
    ...
    require(earlyMintActive,"Early Mint is Not Active yet");
    //Check how many NFTs are available to be minted
    require(_mintAmount + totalSupply() <= maxSupply, "out of supply");
    //將user地址包成leafNode
    bytes32 leafNode = keccak256(abi.encode(msg.sender));
    //Check user is in the whitelist - use merkle tree to validate
    require(MerkleProofUpgradeable.verify(_merkleProof, merkleRoot, leafNode), "Invalid Merkle Proof");
    //Check user has sufficient funds
    require(msg.value >= _mintAmount * price, "make sure you pay enough amount");
