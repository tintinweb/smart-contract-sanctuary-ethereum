...
import '../../../assets/ERC721/ERC721OwlAttributesRoot.sol';

contract MinterRandomPreset is MinterCore, Payments, KeeperCompatibleInterface {
    struct QueueItem {
        uint256 epochBlock;
        address requester;
    }
    ...
    function __MinterRandomPreset_init(
        address _admin,
        address _mintFeeToken,
        address _mintFeeAddress,
        uint256 _mintFeeAmount,
        ...
        __MinterCore_init(_admin, _nftContractAddr, _forwarder);
        __Payments_init(_mintFeeToken, _mintFeeAddress, _mintFeeAmount);
        __MinterRandomPreset_init_unchained(_dnas, _vrfBeacon);
    }

    function __MinterRandomPreset_init_unchained(uint256[] memory _dnas, address _vrfBeacon) internal onlyInitializing {
        dnas = _dnas;
        vrfBeacon = VRFBeacon(_vrfBeacon);
    }

    function requestMint() external returns (uint256 requestId, uint256 epochBlockNumber) {
        // Requests will only begin to be rejected after all the tokens have
        // been MINTED. There will be a point where there are enough requests in
        // the queue for the amount of mints left. However, that cannot be the
        // parameter for rejection since there is a possibility that a request
        // in the queue fails due to insufficient funds.
        require(dnas.length > 0, 'MinterRandomPreset: there are no more mints left');

        (requestId, epochBlockNumber) = VRFBeacon(vrfBeacon).requestRandomness();

        _collectFee(_msgSender());

        queueItems.push(QueueItem(epochBlockNumber, _msgSender()));
    }

    function checkUpkeep(
        bytes memory /* checkData */
    ) external view override returns (bool upkeepNeeded, bytes memory performData) {
        if (queueItems.length == queueIndex) return (false, '0x');

        uint256 randomness = VRFBeacon(vrfBeacon).getRandomness(queueItems[queueIndex].epochBlock);
    ...
    function performUpkeep(bytes memory performData) external override {
        ...
        _mint(queueItems[queueIndex].requester, randomness);
        queueIndex++;
    }

    function _mint(address requester, uint256 randomSeed) internal override {
        uint256 randIndex = randomSeed % dnas.length;

        uint256 selectedDna = _removeItemByIndex(randIndex);

        MinterCore._mint(requester, selectedDna);
    }

    /**
     * @dev Will return the entire `dnas` array. Intended for debugging purposes.
     * @return dnas array from storage
     */
    function getDnas() external view returns (uint256[] memory) {
        return dnas;
    }

    function _removeItemByIndex(uint256 index) private returns (uint256) {
        uint256 length = dnas.length;
        uint256 finalEl = dnas[length - 1];
        dnas.pop();

        if (length - 1 == index) return finalEl;

        uint256 removedItem = dnas[index];
        dnas[index] = finalEl;
...
contract Payments is OwlBase {
    address public mintFeeToken;
    address public mintFeeAddress;
    uint256 public mintFeeAmount;

    function __Payments_init(
        address _mintFeeToken,
        address _mintFeeAddress,
        uint256 _mintFeeAmount
    ) internal onlyInitializing {
        __Payments_init_unchained(_mintFeeToken, _mintFeeAddress, _mintFeeAmount);
    }

    function __Payments_init_unchained(
        address _mintFeeToken,
        address _mintFeeAddress,
        uint256 _mintFeeAmount
    ) internal onlyInitializing {
    ...
    /**
     * @dev Collect the fee and send it to `mintFeeAddress`
     */
    function _collectFee(address buyer) internal {
        if (mintFeeAmount != 0)
...
abstract contract MinterCore is OwlBase {
    ...
    function __MinterCore_init(
        ...
        __MinterCore_init_unchained(_nftContractAddr);
    }
     ...
     * @param _nftContractAddr NFT address to mint
     */
    function __MinterCore_init_unchained(address _nftContractAddr) private onlyInitializing {
     ...
     */
    function _mint(address buyer, uint256 tokenId) internal virtual {
        // Transfer ERC20
        // if (mintFeeAmount != 0)
        //     SafeERC20Upgradeable.safeTransferFrom(
        //         IERC20Upgradeable(mintFeeToken),
     ...
     * @param tokenId the token id to mint
     */
    function _safeMint(address buyer, uint256 tokenId) internal virtual {
        // Transfer ERC20
        // if (mintFeeAmount != 0)
        //     SafeERC20Upgradeable.safeTransferFrom(
        //         IERC20Upgradeable(mintFeeToken),
...
import './ERC721OwlBase.sol';
import './ERC721OwlAttributes.sol';
...
interface IERC721TopDown {
    ...
    function rootOwnerOf(uint256 _childTokenId) external view returns (address);

    function rootOwnerOfChild(address _childContract, uint256 _childTokenId) external view returns (address);
...
contract ERC721OwlAttributesRoot is IERC721TopDown, IERC721ReceiverUpgradeable, ERC721OwlBase {
    ...
    bytes4 private constant ERC165TAG =
        bytes4(keccak256(abi.encodePacked('OWLProtocol://ERC721OwlAttributesRoot/', _version)));
    ...
    uint8[] public geneConfig;

    // Mapping from root tokenId to inherentDna
    mapping(uint256 => uint256) public inherentDna;

    // Adding 1 allows 0 to be the null value
    // There can only be one child token per contract

    // rootTokenId => (child address => child tokenId)
     ...
     * @param baseURI_ uri
     * @param _childContracts addresses of {ERC721OwlAttributes} contracts who
     * will be the children. If the child does not need to be an NFT, simple pass
     * address(0)
    ...
    function initialize(
        ...
        uint8[] memory _geneConfig,
        address _forwarder,
        address _receiver,
        uint96 _feeNumerator
    ) external initializer {
        __ERC721OwlAttributesRoot_init(
            _admin,
            _name,
            _symbol,
            baseURI_,
    ...
    function __ERC721OwlAttributesRoot_init(
        address _admin,
        string memory _name,
        string memory _symbol,
        string memory baseURI_,
        ...
        __ERC721OwlBase_init(_admin, _name, _symbol, baseURI_, _forwarder, _receiver, _feeNumerator);
        _grantRole(DNA_ROLE, _admin);

        __ERC721OwlAttributesRoot_init_unchained(_childContracts, _geneConfig);
    }

    function __ERC721OwlAttributesRoot_init_unchained(address[] memory _childContracts, uint8[] memory _geneConfig)
        internal
        onlyInitializing
    {
        require(_childContracts.length <= 256, 'ERC721OwlAttributesRoot: invalid child contracts config');
        require(
            _childContracts.length == 0 || _childContracts.length + 1 == _geneConfig.length,
            'ERC721OwlAttributesRoot: invalid gene config'
        );
        childContracts = _childContracts;
        geneConfig = _geneConfig;

        for (uint256 i = 0; i < _childContracts.length; i++) approvedContracts[_childContracts[i]] = true;

        // Start at 1
        nextId.increment();
    }

    function getGeneConfig() external view returns (uint8[] memory) {
        return geneConfig;
    }

    function rootOwnerOf(uint256 _childTokenId) public view returns (address) {
        return rootOwnerOfChild(address(0), _childTokenId);
    }

    function rootOwnerOfChild(address _childContract, uint256 _childTokenId) public view returns (address) {
        // If _childContract == address(0), SKIP the approval check because it
        // is being called through rootOwnerOf(uint256). The check is required
        // in the case that all genes in the dna are non-inherent which means
        // address(0) never gets checked to true in the initialization.
        require(
            approvedContracts[_childContract] || _childContract == address(0),
            'ERC721OwlAttributesRoot: contract not approved'
        );

        address rootOwnerAddress;
        if (_childContract != address(0))
            (rootOwnerAddress, _childTokenId) = ownerOfChild(_childContract, _childTokenId);
        else rootOwnerAddress = ownerOf(_childTokenId);

        // Case 1: Token owner is this contract and token.
        while (rootOwnerAddress == address(this))
            (rootOwnerAddress, _childTokenId) = ownerOfChild(rootOwnerAddress, _childTokenId);

        // Case 2: Token owner is EOA (or address of a destroyed contract,
        // however, this is an unlikely edge case). Also possible (but highly
        // unlikely), a contract in construction buys the root NFT in the
        // constructor then calls rootOwnerOf(). This would return the root NFT
        // contract address as the root owner. While this behaviour is
        // undefined, it presents no security vunerabilities.
        if (!rootOwnerAddress.isContract()) return rootOwnerAddress;

        try ERC721OwlAttributesRoot(rootOwnerAddress).rootOwnerOfChild(address(this), _childTokenId) returns (
            address rootOwner
        ) {
            // Case 3: Token owner is other top-down composable
            return rootOwner;
        } catch {
            // Case 4: Token owner is other contract
        ...
        returns (address parentTokenOwner, uint256 parentTokenId)
    {
        ...
        require(parentTokenId > 0, 'ERC721OwlAttributesRoot: provided child token does not have parent');
        return (ownerOf(parentTokenId), parentTokenId);
    }

    function updateDna(uint256 tokenId, uint256 newDna) external onlyRole(DNA_ROLE) {
        _requireMinted(tokenId);
        uint256 finalDna;
        for (uint256 i = 0; i < childContracts.length; i++) {
            uint256 newGene = _isolateGene(newDna, geneConfig[i], geneConfig[i + 1]);
            address currContr = childContracts[i];
            // This is an attribute, treat it as such
            if (currContr == address(0))
                finalDna |= newGene;
                // This is an attached contract, go set it's DNA
            else {
                uint256 geneBitMask = _get256Bitmask(geneConfig[i], geneConfig[i + 1]);
                uint256 currTokenId = childTokens[tokenId][currContr];
                uint256 currChildDna = ERC721OwlAttributesRoot(currContr).getDna(currTokenId);

                // Copy the dna from the child and clear the gene that will be
                // manipulated by the parent. After those bits are cleared,
                // update the those bits to reflect the new gene. This way, the
                // parent cannot edit any of the child metadata expect what is
                // relevant to itself.
                uint256 newChildDna = (~geneBitMask & currChildDna) | newGene;

                // If there is no change being made, no need to run the update
                if (newChildDna == currChildDna) continue;

                ERC721OwlAttributesRoot(childContracts[i]).updateDna(currTokenId, newChildDna);
            }
        }

        inherentDna[tokenId] = finalDna;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        uint256 finalDna = getDna(tokenId);

        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, finalDna.toString())) : '';
    }

    function getDna(uint256 tokenId) public view returns (uint256 finalDna) {
        if (tokenId == 0) return 0;
        _requireMinted(tokenId);

        finalDna = inherentDna[tokenId];
        for (uint256 i = 0; i < childContracts.length; i++) {
            address currContr = childContracts[i];
            if (currContr == address(0)) continue;

            // Clear other bits so child can have other attributes not displayed
            // on the parent Also, If the child is compromised, only that
            // attribute is compromised.
            uint256 gene = _isolateGene(
                ERC721OwlAttributesRoot(currContr).getDna(childTokens[tokenId][currContr]),
                geneConfig[i],
                geneConfig[i + 1]
            );
    ...
    function mint(address to, uint256 dna) public onlyRole(MINTER_ROLE) {
        safeMint(to, dna, '');
    }

    function safeMint(address to, uint256 dna) public onlyRole(MINTER_ROLE) {
        safeMint(to, dna, '');
    }

    /**
     * @dev Case 1: is root parent
     * Case 2: Is child
     */
    function safeMint(
        address to,
        ...
        // Mint the root NFT
        _safeMint(to, nextId.current(), data);

        // Is parent if data is empty
        if (data.length != 0) inherentDna[nextId.current()] = dna;
        else {
            uint256 _inherentDna;

            // Mint the attributes
            for (uint256 i = 0; i < childContracts.length; i++) {
                uint256 gene = _isolateGene(dna, geneConfig[i], geneConfig[i + 1]);

                if (childContracts[i] == address(0)) _inherentDna |= gene;
                else if (gene != 0)
                    ERC721OwlAttributesRoot(childContracts[i]).safeMint(
                        address(this),
                        gene,
                        abi.encode(nextId.current())
                    );
            }

            inherentDna[nextId.current()] = _inherentDna;
        }
    ...
    function onERC721Received(
        ...
        uint256 _childTokenId,
        bytes memory _data
    ) external override(IERC721ReceiverUpgradeable, IERC721TopDown) returns (bytes4) {
        // Only child contracts can send NFTs into this contract
        require(approvedContracts[_msgSender()], 'ERC721OwlAttributesRoot: contract not approved to send NFTs');

        // Parse rootTokenId from _data

        // Convert up to 32 bytes of_data to uint256, owner nft tokenId
        // passed as uint in bytes
        uint256 tokenId;
        assembly {
            tokenId := calldataload(164)
        }
        if (_data.length < 32) tokenId = tokenId >> (256 - _data.length * 8);

        // SKIP check if this contract is minting
        require(
            (_from == address(0) && _operator == address(this)) || _isApprovedOrRootOwner(_operator, tokenId),
            'ERC721OwlAttributesRoot: Must be owner or approved to attach a child'
        );
    ...
    function detachChild(
        ...
        require(tokenId > 0, 'ERC721OwlAttributesRoot: _childTokenId is not owned');
        require(tokenId == _fromTokenId, 'ERC721OwlAttributesRoot: _childTokenId is owned by a different parent token');

        require(
            _isApprovedOrRootOwner(_msgSender(), tokenId),
            'Must own (or be approved on) root tokenId to transfer children'
        );

        // Update contract storage
        _removeChild(tokenId, _childContract, _childTokenId);

        ERC721OwlBase(_childContract).safeTransferFrom(address(this), _to, tokenId);
        emit DetachedChild(tokenId, _to, _childContract, _childTokenId);
    }

    function _isApprovedOrRootOwner(address spender, uint256 tokenId) internal view returns (bool) {
    ...
    function _removeChild(
        ...
        // require(
        //     childTokens[_tokenId][_childContract] == _childTokenId,
        //     'ERC721OwlAttributesRoot: Child is not owned by parent'
        // );   Redunant, not necessary
    ...
    function _receiveChild(
        ...
        require(
            childTokens[_tokenId][_childContract] == 0,
            'ERC721OwlAttributesRoot: Cannot receive child token because one has already been received.'
        );
    ...
    function _isolateGene(
        uint256 dna,
        uint8 geneStart,
        uint8 geneEnd
    ) private pure returns (uint256) {
        uint256 bitmask = _get256Bitmask(geneStart, geneEnd);
     ...
     */
    function _get256Bitmask(uint16 startBit, uint16 endBit) private pure returns (uint256 bitMask) {
    ...
    /*
     * @dev Required for external contracts to find out how many genes we store.
     */
    function geneConfigLength() public view returns (uint256) {
        return geneConfig.length;
    }

    function childContractsLength() public view returns (uint256) {
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721OwlBase) returns (bool) {
 ...
 * deployment strategy.
 */
contract ERC721Owl is ERC721OwlBase {
    /**********************
        Initialization
    **********************/
 ...
 * deployment strategy.
 */
abstract contract ERC721OwlBase is OwlBase, ERC721BurnableUpgradeable, ERC2981Upgradeable {
    ...
    **********************/
    ...
    function __ERC721OwlBase_init(
        ...
        __ERC721OwlBase_init_unchained(baseURI_);
    }

    function __ERC721OwlBase_init_unchained(string memory baseURI_) internal onlyInitializing {
        (_msgData());
        baseURI = baseURI_;
    }
    ...
    **********************/
     ...
     */
    function setBaseURI(string memory baseURI_) public onlyRole(URI_ROLE) {
        baseURI = baseURI_;
    }
 ...
 * `updateDna()` function.
 */
contract ERC721OwlAttributes is ERC721OwlBase {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using StringsUpgradeable for uint256;
    ...
    **********************/
    ...
    mapping(uint256 => uint256) public dnas;

    // Auto-incrementing tokenIds
    CountersUpgradeable.Counter public nextId;
    ...
    **********************/
     ...
     */
    function mint(address to, uint256 dna) public virtual onlyRole(MINTER_ROLE) {
     ...
     * @param dna of next tokenId
     */
    function safeMint(address to, uint256 dna) public virtual onlyRole(MINTER_ROLE) {
        safeMint(to, dna, '0x');
    }

    /**
     * @dev this function will be called by {ERC721OwlAttributesRoot}
     * @param to address to mint to
     * @param dna of the root token
     * @param data pass `tokenId` of the parent being minted
     */
    function safeMint(
        address to,
        uint256 dna,
    ...
    function getDna(uint256 tokenId) external view returns (uint256) {
        // tokenId = 0 will return null/default value (0)
        require(tokenId == 0 || _exists(tokenId), 'ERC721Metadata: URI query for nonexistent token');
        return dnas[tokenId];
    }
