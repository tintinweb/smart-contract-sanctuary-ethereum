...
import "../interface/IMintSigner.sol";
import "../common/Signer.sol";
import "openzeppelin-contracts/utils/cryptography/EIP712.sol";

/// Signer for Mint transactions. Used to mint LagoStable tokens via the `Minter` contract.
contract MintSigner is Signer, IMintSigner, EIP712 {
    bytes32 public constant MINT_TYPEHASH =
        keccak256(
            "Mint(address minter,address token,address to,uint256 amount,uint256 id)"
        );
    ...
    constructor(
        ...
        EIP712("MintSigner", "0.0.1")
    {}

    /// @inheritdoc IMintSigner
    function enforceMintSigned(
        ...
        bytes32 msgHash = _hashTypedDataV4(
            keccak256(abi.encode(MINT_TYPEHASH, minter, token, to, amount, id))
        );
    ...
    function executeMint(
        ...
        IMinter(minter).mint(token, to, amount, id);
    }
...
interface IMintSigner is ISigner {
