...
contract AMMBase is IAMMBase {
    ...
    //See below comment for reason why not immutable
    ...
    modifier onlyPoolAdmin() {
        require(
            ...
            Errors.ADDRESS_NOT_POOL_ADMIN
        );
        _;
    }

    /// @inheritdoc IAMMBase
    function setFee(DataTypes.PoolFee calldata _newFee) external onlyPoolAdmin {
        poolData.fee.lpBuyFee = _newFee.lpBuyFee;
        poolData.fee.lpSellFee = _newFee.lpSellFee;
        poolData.fee.royaltiesBuyFee = _newFee.royaltiesBuyFee;
        poolData.fee.royaltiesSellFee = _newFee.royaltiesSellFee;
        poolData.fee.protocolBuyRatio = _newFee.protocolBuyRatio;
        poolData.fee.protocolSellRatio = _newFee.protocolSellRatio;
        poolData.fee.royaltiesAddress = _newFee.royaltiesAddress;
        poolData.fee.protocolFeeAddress = _newFee.protocolFeeAddress;
        emit FeeChanged(poolData.fee);
    }

    /// @inheritdoc IAMMBase
    function setPoolIterativeLimits(DataTypes.IterativeLimit calldata _newLimits) external onlyPoolAdmin {
        poolData.iterativeLimit.minimumF = _newLimits.minimumF;
        poolData.iterativeLimit.maxBulkStepSize = _newLimits.maxBulkStepSize;
        poolData.iterativeLimit.iterations = _newLimits.iterations;
        emit PoolIterativeLimitsSet(poolData.iterativeLimit);
    }

    /// @inheritdoc IAMMBase
    function setPoolLiquidityLimits(DataTypes.LiquidityLimit calldata _newLimits) external onlyPoolAdmin {
        poolData.liquidityLimit.poolTvlLimit = _newLimits.poolTvlLimit;
        poolData.liquidityLimit.cooldown = _newLimits.cooldown;
        poolData.liquidityLimit.maxDepositPercentage = _newLimits.maxDepositPercentage;
        poolData.liquidityLimit.maxWithdrawPercentage = _newLimits.maxWithdrawPercentage;
        poolData.liquidityLimit.minFeeMultiplier = _newLimits.minFeeMultiplier;
        poolData.liquidityLimit.maxFeeMultiplier = _newLimits.maxFeeMultiplier;
        poolData.liquidityLimit.addLiqMode = _newLimits.addLiqMode;
        poolData.liquidityLimit.removeLiqMode = _newLimits.removeLiqMode;
        poolData.liquidityLimit.onlyAdminProvisioning = _newLimits.onlyAdminProvisioning;
        emit PoolLiquidityLimitsSet(poolData.liquidityLimit);
    }

    /// @inheritdoc IAMMBase
    function setPoolData(DataTypes.PoolData calldata _newPoolData) external onlyPoolAdmin {
        poolData.useAccessToken = _newPoolData.useAccessToken;
        poolData.accessToken = _newPoolData.accessToken;
        poolData.stableYieldAddress = _newPoolData.stableYieldAddress;
        poolData.coefficientA = _newPoolData.coefficientA;
        poolData.coefficientB = _newPoolData.coefficientB;
        poolData.coefficientC = _newPoolData.coefficientC;
...
contract MME1155 is Initializable, AMMBase, IMME1155, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    ///@notice tokenDistribution is = 0 same as if it doesn't exist
    ...
    function initialize(DataTypes.PoolData memory _poolData, string memory _symbol, string memory _name) external initializer {
        __Pausable_init();
        __Ownable_init();
        poolData = _poolData;
     ...
     * @param _tokenId the id of the access token
     * @param _functionName the name of the function with the modifier
     */
    modifier useAccessNFT(uint256 _tokenId, string memory _functionName) {
        if (poolData.useAccessToken) {
            require(
                IAccessNFT(poolData.accessToken).HasAccessNFT(msg.sender, _tokenId, _functionName),
    ...
    /// @inheritdoc IMME1155
    function getPool(uint256 _index) external view returns (DataTypes.AMMSubPool1155Details memory subpool) {
        subpool.reserve = subPools[_index].reserve;
        subpool.totalShares = subPools[_index].totalShares;
        subpool.V = subPools[_index].V;
        subpool.F = subPools[_index].F;
    }
    ...
    function getQuote(
        ...
        DataTypes.Shares1155Params memory sharesParams = DataTypes.Shares1155Params({amounts: _amounts, tokenIds: _tokenIds});
        quotation = Liquidity1155Logic.getQuote(
                DataTypes.QuoteParams({buy: _buy, useFee: _useFee}),
                sharesParams,
                poolData,
                subPools,
                tokenDistribution
    ...
    function swapStable(
        ...
        DataTypes.Shares1155Params memory sharesParams = DataTypes.Shares1155Params({amounts: _requiredAmounts, tokenIds: _tokenIds});
        Liquidity1155Logic.swapStable(msg.sender, _maxStable, sharesParams, poolData, subPools, tokenDistribution);
    }
    ...
    function swapShares(
        ...
        DataTypes.Shares1155Params memory sharesParams = DataTypes.Shares1155Params({amounts: _amounts, tokenIds: _tokenIds});
        Liquidity1155Logic.swapShares(msg.sender, yieldReserve, sharesParams, poolData, subPools, tokenDistribution);
    }
    ...
    function depositInitial(
        ...
        uint256 _stable,
        uint256 _subPoolIndex
    ) external nonReentrant onlyPoolAdmin {
        DataTypes.Shares1155Params memory sharesParams = DataTypes.Shares1155Params({amounts: _maxAmounts, tokenIds: _tokenIds});
        Liquidity1155Logic.depositInitial(msg.sender, _subPoolIndex, _stable, sharesParams, poolData, subPools, tokenDistribution);
    }
    ...
    function addLiquidityStable(
        ...
        Liquidity1155Logic.addLiquidityStable(msg.sender, _targetLP, _maxStable, poolData, subPools);
    }
    ...
    function addLiquidityShares(
        ...
        DataTypes.Shares1155Params memory sharesParams = DataTypes.Shares1155Params({amounts: _maxAmounts, tokenIds: _tokenIds});
        Liquidity1155Logic.addLiquidityShares(msg.sender, _targetLP, sharesParams, poolData, subPools, tokenDistribution);
    }

    /// @inheritdoc IMME1155
    function removeLiquidityStable(uint256 _targetLP) external nonReentrant whenNotPaused {
        require(poolData.liquidityLimit.removeLiqMode != 1, Errors.LIQUIDITY_MODE_RESTRICTED);
        Liquidity1155Logic.removeLiquidityStable(msg.sender, yieldReserve, _targetLP, poolData, subPools, queuedWithdrawals);
    }
    ...
    /// @inheritdoc IMME1155
    function getTokenIdAvailable(uint256 _tokenId) external view returns (uint256) {
        return subPools[tokenDistribution[_tokenId]].shares[_tokenId];
    }

    function getSubPools(uint256[] memory _tokenIds) external view returns (uint256[] memory) {
        return Pool1155Logic.getSubPools(_tokenIds, tokenDistribution);
    }

    function getSubPoolsSeq(uint256 _startTokenId, uint256 _endTokenId) external view returns (uint256[] memory) {
        return Pool1155Logic.getSubPoolsSeq(_startTokenId, _endTokenId, tokenDistribution);
    }

    function addSubPool(uint256 _V, uint256 _F) external onlyPoolAdmin {
        Pool1155Logic.addSubPool(_V, _F, subPools);
    }

    //TODO: change access to V updater admin if needed
    function updatePoolV(uint256[] calldata _subPoolIds, uint256[] calldata _vs) external onlyPoolAdmin {
        Pool1155Logic.updatePoolV(_subPoolIds, _vs, subPools);
    }

    /// @inheritdoc IMME1155
    function changeSubPoolStatus(uint256[] calldata _subPoolIds, bool _newStatus) external onlyPoolAdmin {
        Pool1155Logic.changeSubPoolStatus(_subPoolIds, _newStatus, subPools);
    }

    /// @inheritdoc IMME1155
    function moveReserve(uint256 _moverId, uint256 _movedId, uint256 _amount) external onlyPoolAdmin {
        Pool1155Logic.moveReserve(_moverId, _movedId, _amount, subPools, poolData);
    }

    /// @inheritdoc IMME1155
    function moveShares(uint256 _startId, uint256 _endId, uint256 _newSubPoolId) external onlyPoolAdmin {
        Pool1155Logic.moveShares(_startId, _endId, _newSubPoolId, subPools,poolData, tokenDistribution);
    }

    /// @inheritdoc IMME1155
    function moveSharesList(uint256 _newSubPoolIds, uint256[] calldata _ids) external onlyPoolAdmin {
        Pool1155Logic.moveSharesList(_newSubPoolIds, _ids, subPools,poolData, tokenDistribution);
    }

    //In v1.1
    // /// @inheritdoc IMME1155
    // function depositIntoStableYield(uint256 _amount) external onlyPoolAdmin {
    //     yieldReserve += Pool1155Logic.depositIntoStableYield(_amount, addressesRegistry, poolData.stableYieldAddress, yieldReserve);
    // }

    // function withdrawFromStableYield(uint256 _amount) external onlyPoolAdmin {
    //     yieldReserve -= Pool1155Logic.withdrawFromStableYield(_amount, addressesRegistry, poolData.stableYieldAddress, yieldReserve);
    // }

    /// @inheritdoc IMME1155
    function RescueTokens(address _token, uint256 _amount, address _receiver) external onlyPoolAdmin {
        Pool1155Logic.RescueTokens(_token, _amount, _receiver, poolData.stable, poolData.poolLPToken);
    }

    function WithdrawFees(address _to, uint256 _amount, DataTypes.FeeType _feeType) external whenNotPaused {
        Pool1155Logic.withdrawFees(msg.sender, _to, _amount, _feeType, poolData);
    }
}
...
interface IMME1155 {
     ...
     */
    event PoolPaused(address _admin);
    /**
     * @dev Emitted when pool is unpaused
     * @param _admin The admin address
     */
    event PoolUnpaused(address _admin);

    function initialize(DataTypes.PoolData memory _poolData, string memory _symbol, string memory _name) external;
     ...
     * @param _amounts The amounts of shares to buy or sell
     * @param _tokenIds The shares token ids
     * @param _buy The directional boolean. If buy direction then true
     * @param _useFee the boolean determining whether to use Fee in the calculation or not in case we want to calculate the value of the shares for liquidity
     */
    function getQuote(
        uint256[] memory _amounts,
        uint256[] memory _tokenIds,
     ...
     * @param _amounts The amounts of token ids outputted
     * @param _tokenIds The token ids outputted
     * @param _maxInput The maximum amount of stablecoin to be spent
     */
    function swapStable(uint256[] memory _amounts, uint256[] memory _tokenIds, uint256 _maxInput) external;
     ...
     * @param _tokenIds The token ids outputted
     */
    function swapShares(uint256[] memory _amounts, uint256[] memory _tokenIds) external;
     ...
     */
    function getPool(uint256 _index) external view returns (DataTypes.AMMSubPool1155Details memory);

    /**
     * @dev Function to add liquidity using Stable coins
     * @param _targetLP The amount of target LPs outputted
     * @param _maxStable The amount of maximum stablecoins to be spent
     */
    function addLiquidityStable(uint256 _targetLP, uint256 _maxStable) external;

    /**
     * @dev Function to add liquidity using shares
     * @param _tokenIds The token ids of shares to be spent
     * @param _maxAmounts The maximum amounts of shares to be spent
     * @param _targetLP The amount of required LPs outputted
     */
    function addLiquidityShares(uint256[] memory _tokenIds, uint256[] memory _maxAmounts, uint256 _targetLP) external;

    /**
     * @dev Function to remove liquidity by shares
     * @param _targetLP The amount of LPs to be burned
     * @param _tokenIds The token ids of shares to be outputted
     * @param _maxAmounts The maximum amounts of shares to be outputted
     */
    function removeLiquidityShares(uint256 _targetLP, uint256[] memory _tokenIds, uint256[] memory _maxAmounts) external;

    /**
     * @dev Function to remove liquidity by stable coins
     * @param _targetLP The amount of LPs to be burned
     */
    function removeLiquidityStable(uint256 _targetLP) external;
     ...
     * @param _startId The start index of the token ids to be moved
     * @param _endId The end index of the token ids to be moved
     * @param _newSubPoolId The id of the new subpool
     */
    function moveShares(uint256 _startId, uint256 _endId, uint256 _newSubPoolId) external;

    /**
     * @dev Function to move shares in an array list to a different sub pool called by the admin
     * @param _ids The array of shares ids to be moved
     * @param _newSubPoolId The id of the new subpool
     */
    function moveSharesList(uint256 _newSubPoolId, uint256[] memory _ids) external;

    /**
     * @dev Function to move enable or disable specific subpools by ids
     * @param _subPoolIds The sub pools ids array
     * @param _newStatus The new status, enabled=true or disabled=false
     */
    function changeSubPoolStatus(uint256[] calldata _subPoolIds, bool _newStatus) external;
     ...
     */
    function moveReserve(uint256 _moverId, uint256 _movedId, uint256 _amount) external;
    ...
    //  */
    // function depositIntoStableYield(uint256 _amount) external;
    ...
    //  */
    // function withdrawFromStableYield(uint256 _amount) external;
...
library Liquidity1155Logic {
     ...
     * @param _stableIn The amount of stablecoin inputted
     * @param _LP The amount of LP token outputted
     * @param _from The address of the msg sender
     * @notice it's here to avoid the stack too deep issue for now
     */
    event AddedLiqStable(uint256 _stableIn, uint256 _LP, address _from);

    /**
     * @dev Emitted when adding liquidity by a liqduity provider using shares
     * @param _LP The amount of LP token outputted
     * @param _from The address of the msg sender
     * @param _subPoolGroups The subpool groups including calculations and shares array
     */
    event AddedLiqShares(uint256 _LP, address _from, DataTypes.SubPoolGroup[] _subPoolGroups);

    /**
     * @dev Emitted when removing liquidity by a liqduity provider
     * @param _stableOut The amount of stablecoin outputted
     * @param _LP The amount of LP token inputted
     * @param _from The address of the msg sender
     * @param queued If transaction is queued = true
     */
    event RemovedLiqStable(uint256 _stableOut, uint256 _LP, address _from, bool queued);
     ...
     * @param _subPoolGroups The subpool groups including calculations and shares array
     */
    event RemovedLiqShares(uint256 _LP, address _from, bool queued, DataTypes.SubPoolGroup[] _subPoolGroups);

    /**
     * @dev Emitted when swap of stable coins occures
     * @param _stable The amount of stablecoin supplied
     * @param _fees The fees collected
     * @param _user The user address
     * @param _subPoolGroups The subpool groups including calculations and shares array
     */
    event SwappedStable(uint256 _stable, DataTypes.FeeReturn _fees, address _user, DataTypes.SubPoolGroup[] _subPoolGroups);

    /**
     * @dev Emitted when swap of shares occures
     * @param _stable The amount of stablecoin outputted
     * @param _fees The fees collected
     * @param _user The user address
     * @param _subPoolGroups The subpool groups including calculations and shares array
     */
     ...
     * @param _user The user that processed the withdrawals
     * @param _transactions The number of transactions processed
     */
    event WithdrawalsProcessed(address _user, uint256 _transactions);
     ...
     * @param _amount The account to deduct the stables from
     * @param _TVL The TVL of the pool
        ...
        for (uint256 i = 0; i < subPools.length; i++) {
            if (subPools[i].status) {
                if (i == subPools.length - 1) {
                    subPools[i].reserve += _remaining;
                } else {
                    if (_TVL == 0) {
                        subPools[i].reserve += _amount / subPools.length;
                        _remaining -= _amount / subPools.length;
                    } else {
                        weighted = (_amount * Pool1155Logic.calculateTotal(subPools, i)) / _TVL;
                        _remaining -= weighted;
                        subPools[i].reserve += weighted;
                    }
                }
                Pool1155Logic.updatePrice(subPools, poolData.coefficientA, poolData.coefficientB, poolData.coefficientC, i);
            }
        }
    }
    ...
    function depositInitial(
        ...
        require(Pool1155Logic.calculateTotal(subPools, _subPoolIndex) == 0, "SUBPOOL_NOT_EMPTY");
        for (uint256 i = 0; i < params.tokenIds.length; i++) {
            if (tokenDistribution[params.tokenIds[i]] != _subPoolIndex) continue;
            subPools[_subPoolIndex].shares[params.tokenIds[i]] += params.amounts[i];
            subPools[_subPoolIndex].totalShares += params.amounts[i];
            IERC1155(poolData.tokens[0]).safeTransferFrom(_user, poolData.poolLPToken, params.tokenIds[i], params.amounts[i], "");
        }
        subPools[_subPoolIndex].reserve += _stable;
        Pool1155Logic.updatePrice(subPools, poolData.coefficientA, poolData.coefficientB, poolData.coefficientC, _subPoolIndex);
        IERC20(poolData.stable).transferFrom(_user, poolData.poolLPToken, _stable);
        ILPToken(poolData.poolLPToken).mint(
            _user,
            MathHelpers.convertToWad(Pool1155Logic.calculateTotal(subPools, _subPoolIndex)) / subPools.getLPPrice(poolData.poolLPToken)
        );
    }

    /**
     * @dev Function to remove liquidity by stable coins
     * @param _user The account to deduct the stables from
     * @param _targetLP The amount of LPs required
     * @param _maxStable the maximum stablecoins to transfer
     * @param poolData The liquidity pool data structure
     * @param subPools The subpools array
     */
    function addLiquidityStable(
        ...
        require(_user != address(0), Errors.ADDRESS_IS_ZERO);
        require(IERC20(poolData.stable).allowance(_user, address(this)) >= _maxStable, Errors.NOT_ENOUGH_APPROVED);
        require(IERC20(poolData.stable).balanceOf(_user) >= _maxStable, Errors.NOT_ENOUGH_USER_BALANCE);
        DataTypes.LiqLocalVars memory vars;
        (vars.TVL, vars.LPPrice) = subPools.getTVLAndLPPrice(poolData.poolLPToken);
        require(poolData.liquidityLimit.poolTvlLimit >= vars.TVL + _maxStable, Errors.TVL_LIMIT_REACHED);
        //if TVL > 0 and deposit > TVL * limitPercentage, then revert where deposit is (requiredLP + totalLPOwned) * price
        //for v1.1
        // require(
        //     vars.TVL == 0 ||
        //         ((MathHelpers.convertFromWad((_targetLP + ILPToken(poolData.poolLPToken).getBalanceOf(_user)) * vars.LPPrice)) <=
        //             MathHelpers.convertFromWadPercentage(vars.TVL * poolData.liquidityLimit.maxDepositPercentage)),
        //     Errors.DEPOSIT_LIMIT_REACHED
        // );
        if ((MathHelpers.convertFromWad(_targetLP * vars.LPPrice)) > _maxStable) {
            vars.LPAmount = MathHelpers.convertToWad(_maxStable) / vars.LPPrice;
            vars.stable = _maxStable;
        } else {
            vars.LPAmount = _targetLP;
            vars.stable = MathHelpers.convertFromWad(_targetLP * vars.LPPrice);
        }
        distributeLiquidityToAll(vars.stable, vars.TVL, poolData, subPools);

        IERC20(poolData.stable).transferFrom(_user, poolData.poolLPToken, vars.stable);
        ILPToken(poolData.poolLPToken).mint(_user, vars.LPAmount);
        emit AddedLiqStable(vars.stable, vars.LPAmount, _user);
        return (vars.stable, vars.LPAmount);
    }

    /**
     * @dev Function to add liquidity by shares while grouping by subpool
     * @param _user The account to deduct stable from
     * @param _targetLP The amount of LPs required
    ...
    function addLiquidityShares(
        ...
        require(IERC1155(poolData.tokens[0]).isApprovedForAll(_user, address(this)), Errors.NOT_ENOUGH_APPROVED);
        DataTypes.LiqLocalVars memory vars;
        (vars.TVL, vars.LPPrice) = subPools.getTVLAndLPPrice(poolData.poolLPToken);
        //for v1.1
        //if TVL > 0 and deposit > TVL * limitPercentage, then revert where deposit is (requiredLP + totalLPOwned) * price
        ...
        // );
        require(
            poolData.liquidityLimit.poolTvlLimit >= vars.TVL + (MathHelpers.convertFromWad(_targetLP * vars.LPPrice)),
            Errors.TVL_LIMIT_REACHED
        );

        vars.remainingLP = _targetLP;
        ...
        for (vars.i = 0; vars.i < vars.counter; vars.i++) {
            ...
            require(subPools[vars.poolId].F >= poolData.iterativeLimit.minimumF, Errors.SELLING_SHARES_DISABLED_DUE_TO_LOW_CONDITIONS);
        ...
        vars.LPAmount = _targetLP - vars.remainingLP;
        ILPToken(poolData.poolLPToken).mint(_user, vars.LPAmount);
        emit AddedLiqShares(vars.LPAmount, _user, vars.subPoolGroups);
        return (vars.LPAmount);
    }

    /**
     * @dev Function to remove liquidity by stable coins
     * @param _user The account to remove LP from
     * @param _yieldReserve The current reserve deposited in yield generators
     * @param _targetLP The amount of LPs to be burned
     * @param poolData The liquidity pool data structure
     * @param subPools The subpools array
    ...
    function removeLiquidityStable(
        ...
        require(_user != address(0), Errors.ADDRESS_IS_ZERO);
        // require(ILPToken(poolData.poolLPToken).allowance(_user, address(this)) >= _targetLP, Errors.NOT_ENOUGH_APPROVED);
        require(ILPToken(poolData.poolLPToken).getBalanceOf(_user) >= _targetLP, Errors.NOT_ENOUGH_USER_BALANCE);
        require(subPools.length > 0, Errors.NO_SUB_POOL_AVAILABLE);
        DataTypes.LiqLocalVars memory vars;
        (vars.TVL, vars.LPPrice) = subPools.getTVLAndLPPrice(poolData.poolLPToken);
        //Check how much stablecoins remaining in the pool excluding yield investment
        vars.stableRemaining = IERC20(poolData.stable).balanceOf(poolData.poolLPToken) - _yieldReserve;
        //Calculate maximum LP Tokens to remove
        vars.remainingLP = _targetLP.min(MathHelpers.convertToWad(vars.stableRemaining) / vars.LPPrice);
        for (vars.i = 0; vars.i < subPools.length; vars.i++) {
            if (subPools[vars.i].status) {
                vars.weighted = vars.remainingLP.min((_targetLP * Pool1155Logic.calculateTotal(subPools, vars.i)) / vars.TVL);
                ...
                Pool1155Logic.updatePrice(subPools, poolData.coefficientA, poolData.coefficientB, poolData.coefficientC, vars.i);
        ...
        if (poolData.liquidityLimit.cooldown == 0) {
            ...
            IERC20(poolData.stable).transferFrom(poolData.poolLPToken, _user, vars.stableTotal);
        } else {
        ...
        ILPToken(poolData.poolLPToken).burn(_user, vars.LPAmount);
        emit RemovedLiqStable(vars.stableTotal, vars.LPAmount, _user, poolData.liquidityLimit.cooldown > 0 ? true : false);
        return (vars.stableTotal, vars.LPAmount);
    }

    /**
     * @dev Function to remove liquidity by shares
     * @param _user The account to burn from
     * @param _targetLP The amount of LPs to be burned
     * @param params The shares arrays (token ids, amounts)
     * @param poolData The data of the liquidity pool
     * @param subPools The subPools array
    ...
    function removeLiquidityShares(
        ...
        for (vars.i = 0; vars.i < vars.counter; vars.i++) {
            ...
            for (vars.y = 0; vars.y < vars.currentSubPool.counter; vars.y++) {
                ...
                subPools[vars.poolId].shares[vars.currentShare.tokenId] -= vars.currentShare.amount;
                //TODO: Re-entrancy?, check with gaurd
                IERC1155(poolData.tokens[0]).safeTransferFrom(
                    poolData.poolLPToken,
                    _user,
                    vars.currentShare.tokenId,
        ...
        if (poolData.liquidityLimit.cooldown > 0) {
            queuedWithdrawals.withdrawals[queuedWithdrawals.nextId].to = _user;
        ...
        //Burn the LP Token
        ILPToken(poolData.poolLPToken).burn(_user, vars.LPAmount);
        emit RemovedLiqShares(vars.LPAmount, _user, poolData.liquidityLimit.cooldown > 0 ? true : false, vars.subPoolGroups);
        return (vars.LPAmount);
    }

    /**
     * @dev Function to process queued withdraw transactions and return number of transactions processed
    ...
    function getQuote(
        ...
        for (vars.i = 0; vars.i < vars.counter; vars.i++) {
            ...
            //return
            // quotation.sharePrices[vars.i] = DataTypes.SharePrice(params.tokenIds[vars.i], vars.sharesData.value);
     ...
     * @param _user The user address to deduct stablecoins
     * @param _maxStable the maximum stablecoins to deduct
     * @param  params The shares arrays (token ids, amounts)
    ...
    function swapStable(
        ...
        for (vars.i = 0; vars.i < vars.counter; vars.i++) {
            vars.currentSubPool = vars.subPoolGroups[vars.i];
            vars.poolId = vars.currentSubPool.id;
            if (subPools[vars.poolId].status == false) continue;
            //Calculate the value of the shares inside this group
        ...
        IERC20(poolData.stable).transferFrom(_user, poolData.poolLPToken, _maxStable - vars.remaining);
        ...
        emit SwappedStable(_maxStable - vars.remaining, vars.fees, _user, vars.subPoolGroups);
    }
     ...
     * @param _user The user address to transfer the shares from
    ...
    function swapShares(
        ...
        (vars.subPoolGroups, vars.counter) = groupBySubpoolDynamic(params, subPools.length, tokenDistribution);
        //Check how much stablecoins remaining in the pool excluding yield investment
        vars.remaining = IERC20(poolData.stable).balanceOf(poolData.poolLPToken) - _yieldReserve;
        //Get the grouped token ids by subpool
        for (vars.i = 0; vars.i < vars.counter; vars.i++) {
            vars.currentSubPool = vars.subPoolGroups[vars.i];
            vars.poolId = vars.currentSubPool.id;
            ...
            // Otherwise, we would need to recalculate using the remaining stable
            // It is better to assume that the user approved more than the shares value
            // if (vars.currentSubPool.sharesCal.value > vars.remaining) continue;
            require(vars.currentSubPool.sharesCal.value <= vars.remaining, Errors.SHARES_VALUE_EXCEEDS_TARGET);
            ...
            //require(vars.currentSubPool.total <= subPools[vars.poolId].totalShares, Errors.SHARES_TARGET_EXCEEDS_RESERVE);
            vars.remaining -= vars.currentSubPool.sharesCal.value;
        ...
        if (vars.stable > 0) {
            ...
            IERC20(poolData.stable).transferFrom(poolData.poolLPToken, _user, vars.stable);
            //Add to the balances of the protocol wallet and royalties address
            poolData.fee.protocolBalance += vars.fees.protocolFee;
            poolData.fee.royaltiesBalance += vars.fees.royalties;
        }
        emit SwappedShares(vars.stable, vars.fees, _user, vars.subPoolGroups);
    }
}
