...
contract MarketplaceDutch is Ownable, ReentrancyGuard
{
    ...
    struct _UserSaleInformation
    {
        ...
        uint _PurchaseAmount;     // [1] -> _PurchaseAmount
        uint _Allocation;         // [2] -> _Allocation
        ...
        bool _BrightListEligible; // [7] -> _BrightListEligible
        bool _SalePaused;         // [8] -> _SalePaused
    }
    ...
    mapping(uint=>SalesArray) private SaleArray;                                       // [SaleIndex] => InternalSaleArray
    ...
    /**
     * @dev Purchases NFTs
     * @param Proofs Contains All Sale Parameters, As Follows
     * note: [0][0] = The Sale Index
     * note: [0][1] = Amount The Amount Of NFTs To Purchase
     * note: [0][2] = Delegation Index (optional delegate.cash) (if opt-out, use 69420 for this value)
     * note: [0][3] = Optional Maximum Brightlist Purchase Per Wallet Limiter
     * note: [1] = Merkle Proof For Priority Discount Eligibility
     * note: [2] = Merkle Proof For Maximum Purchase Amount
     * note: `msg.value` Must Be Sufficient To Purchase NFTs @ The Current Price Of The Dutch Auction
     */
    function Purchase(bytes32[][] calldata Proofs) external payable nonReentrant
    { 
        uint SaleIndex = uint(Proofs[0][0]);  // SaleIndex
        uint Amount = uint(Proofs[0][1]);     // Desired Purchase Amount
        uint DI = uint(Proofs[0][2]);         // Delegation Index
        uint MaxAmount = uint(Proofs[0][3]);  // Maximum Purchasable Amount
        ...
        uint Priority = 69420;
        uint NewUserPurchasedAmount = UserInfo[SaleIndex][Recipient]._PurchaseAmount + Amount;
        bool BrightListEligible;
        if(_IS._ActiveBrightList)
        {
            (BrightListEligible, Priority) = ViewBrightListStatus(SaleIndex, Recipient, Proofs[1]);
            if(BrightListEligible)
            {
                bytes32 _RootHash = SaleArray[SaleIndex]._RootAmounts;
                VerifyAmount(Recipient, MaxAmount, _RootHash, Proofs[2]);
                require(NewUserPurchasedAmount <= MaxAmount, "DutchMarketplace: Purchase Allocation Overflow");
            }
        }
        ...
        UserInfo[SaleIndex][Recipient]._PurchaseAmount = NewUserPurchasedAmount;
    ...
    function __StartSale(
        ...
        SaleArray[_TOTAL_UNIQUE_SALES_DUTCH] = SalesArray(RootsPriority, RootAmounts, DiscountAmounts);
        require(RootsPriority.length == DiscountAmounts.length, "DutchMarketplace: Invalid Merkle Root Length");
        for(uint x; x < SaleArray[_TOTAL_UNIQUE_SALES_DUTCH]._DiscountAmounts.length; x++)
        {
    ...
    function __InitiateRefundsAndProceeds(uint SaleIndex) external nonReentrant onlyAdmin 
    {
        ...
        uint[] memory DiscountAmounts = SaleArray[SaleIndex]._DiscountAmounts;
    ...
    function ___ModifySaleDiscountAmount(uint SaleIndex, uint[] calldata DiscountAmounts) external onlyOwner
    {
        ...
        {
            require(DiscountAmounts[x] <= 100, "DutchMarketplace: Invalid Discount Amount");
            SaleArray[SaleIndex]._DiscountAmounts[x] = DiscountAmounts[x];
        }
    }
    ...
    {
        SaleArray[SaleIndex]._Roots[Priority] = Root;
    }
    ...
    function ViewWalletSaleInformation(
        ...
        bool Verified;
        uint Priority;
        (Verified, Priority) = ViewBrightListStatus(SaleIndex, Wallet, Proof);
        if(
            !(SalesInternal[SaleIndex]._ActiveBrightList || SalesInternal[SaleIndex]._ActivePublic)
            &&
            (Sales[SaleIndex]._MaximumAvailableForSale - SaleState[SaleIndex]._AmountSold > 0)
        ) { SalePaused = true; }     
        return(
            _UserSaleInformation(
                UserInfo[SaleIndex][Wallet]._UserOrderIndexes,
                UserInfo[SaleIndex][Wallet]._PurchaseAmount,
                UserInfo[SaleIndex][Wallet]._Allocation, 
                UserInfo[SaleIndex][Wallet]._ClaimIndex,
                Sales[SaleIndex]._MaximumAvailableForSale - SaleState[SaleIndex]._AmountSold,
                ViewCurrentPrice(SaleIndex),
                Priority,
                Verified,
                SalePaused
            )
    ...
    function ViewSaleStates(uint[] calldata SaleIndexes) public view returns (Sale[] memory)
    {
        ...
        for(uint x; x < SaleIndexes.length; x++) { _Sales[x] = Sales[SaleIndexes[x]]; }
        return _Sales;
    }
     ...
     */
    function ViewAllSaleInformation(uint SaleIndex) public view returns (Sale memory, InternalSale memory, MiscSale memory, SalesArray memory) 
    {
        return ( Sales[SaleIndex], SalesInternal[SaleIndex], SaleState[SaleIndex], SaleArray[SaleIndex] );
    }
    ...
    {
        bool Verified;
        uint Amount = SaleArray[SaleIndex]._Roots.length;
        bytes32 Leaf = keccak256(abi.encodePacked(Recipient));
        for(uint PriorityIndex; PriorityIndex < Amount; PriorityIndex++) 
        { 
            Verified = MerkleProof.verify(Proof, SaleArray[SaleIndex]._Roots[PriorityIndex], Leaf); 
