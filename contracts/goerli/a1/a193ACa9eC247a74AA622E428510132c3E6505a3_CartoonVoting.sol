...
contract CartoonVoting is Ownable, ERC1155{

    uint256 s_ids_supply;

    uint256 s_voting_supply;

    uint256 s_active_voting_id;

    bool isActiveVoting;

    MintPass mint_pass_token;

     /**
     * @notice Info about mint passs tokens used
     */
    mapping(uint256 => bool) public s_mintPassToStatus;

    mapping(uint256 => Voting) public s_idToVoting;

    mapping(uint => string) public idToTokenURI;
    ...
    event Voted(address voter, uint votingId, Vote vote);
    ...
    constructor(address _mintPass)ERC1155("") payable {
       ...
       mint_pass_token = MintPass(_mintPass);
    }
    ...
    function vote(uint256 _passId, Vote _vote) external {
        ...
        require(!s_mintPassToStatus[_passId], "This token was already used for vote");
	...
	function createVoting(string memory _firstTokenURI, string memory _secondTokenURI) external onlyOwner {
        ...
        idToTokenURI[newVoting.firstTokenId] = _firstTokenURI;
        idToTokenURI[newVoting.secondTokenId] = _secondTokenURI;
    ...
    function setURI(uint _id, string memory _uri) external onlyOwner {
       idToTokenURI[_id] = _uri;
     }

     function uri(uint _id) public override view returns (string memory) {
        return idToTokenURI[_id];
     }
...
contract MintPass is Ownable, ERC721Enumerable {

    uint256 s_pass_price = 0 ether;

    uint256 s_max_per_wallet = 1;

    uint256 s_total_supply;

    event Minted(address user, uint timestamp);

    constructor()ERC721("MintPass","MPT") payable {
