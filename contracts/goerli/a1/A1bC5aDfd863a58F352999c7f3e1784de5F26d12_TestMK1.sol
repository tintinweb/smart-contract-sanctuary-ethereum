...
contract TestMK1 is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public currentMaxSupply = 30;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(msg.value >= needToUpdateCost(supply) * _mintAmount, "Not Mad Enough");

    if (msg.sender != owner ()) {
        if(onlyWhitelisted == true){
            require(isWhitelisted(msg.sender),"user is not in the Madlist");
