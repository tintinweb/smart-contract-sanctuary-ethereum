...
contract DepositNFT {
    ...
    uint8 private _tokenIdCounter = 1;
    ...
    event Sell(address from, address to, uint8 tokenId, uint8 price);
    event TransformDepositToBuy(address to, uint8 tokenId);

    constructor(uint8 _maxSupply, uint8 _depositPrice, uint8 _sellGoal, uint256 _decisionTime, address _bankOwner) {
    ...
    function mint() public payable {
        uint8 _tokenId = getTokenIdCounter();
        require(_tokenId < maxSupply, "Sold out");
        require(msg.value >= convertEthToWei(depositPrice), "Deposit is less than the required deposit price");
    ...
    function burn(uint8 _tokenId) public {
    ...
    */
    ...
    function sell(uint8 _tokenId, address _buyer, uint8 _price) public {
        require(tokenMinted[_tokenId].owner == msg.sender, "You do not have this token");
        require(tokenMinted[_tokenId].burned == false, "Token already burned");
        require(_price > convertEthToWei(depositPrice), "Selling price is less than or equal to deposit price");
    ...
    function transformDepositToBuy(uint8 _tokenId) public {
        require(tokenMinted[_tokenId].owner == msg.sender, "You do not have this token");
    ...
    function getTokenIdCounter() public view returns (uint8) {
        return _tokenIdCounter;
    }

    function supplyLeft() public view returns (uint8) {
    ...
    function convertEthToWei(uint8 ethValue) public pure returns(uint256) {
        return ((ethValue / 100) * 1 ether);
    }
    ...
    function _transfer(uint8 _tokenId, address _to) private{
    ...
    function _checkGoal() private {
        if (decisionTimeStartedAt == 0 && _tokenIdCounter > maxSupply) {
            uint8 totalSell = 0;
            uint8 objPercent = maxSupply * 100 / sellGoal;
