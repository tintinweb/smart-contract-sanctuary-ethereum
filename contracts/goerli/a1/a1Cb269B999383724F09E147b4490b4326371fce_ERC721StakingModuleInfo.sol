...
contract ERC721StakingModule is IStakingModule {
    ...
    function stake(
        ...
        emit Staked(sender, address(_token), amount, shares);

        return (bytes32(uint256(uint160(sender))), shares);
    }
    ...
    function unstake(
        ...
        // emit
        uint256 shares = amount * SHARES_PER_TOKEN;
        emit Unstaked(sender, address(_token), amount, shares);
    ...
    function claim(
        ...
        uint256 shares = amount * SHARES_PER_TOKEN;
        emit Claimed(sender, address(_token), amount, shares);
        return (bytes32(uint256(uint160(sender))), sender, shares);
    }
