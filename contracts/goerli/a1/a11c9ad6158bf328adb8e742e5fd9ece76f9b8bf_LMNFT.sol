...
contract LMNFT is IMNFT, ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _qty, address _to) external payable override   {
        ...
        if (isETH){
            require(msg.value >= totalPrice,"Price is not correct");
            ...
            token.transferFrom(_msgSender(), revenueAddress, totalPrice);
        }

        for (uint256 i = 0; i < _qty; i++) {
            _safeMint(_to, totalSupply() + START_ID +1);
        }
    }
    ...
    function multiMint(address[] calldata _wallets) external  onlyOwner {
        ...
        for (uint256 i = 0; i < _wallets.length; i++) {
            ...
            _safeMint(_wallets[i], totalSupply() + START_ID + 1);
        }
    }
    ...
    function setToken(IERC20 newToken_) public onlyOwner {
        token = newToken_;
    }
