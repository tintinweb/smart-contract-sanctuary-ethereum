...
contract Tiro is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseUri, address mintPassContract) ERC721("Time Is Running Out", "TIRO") {
        _baseUri = baseUri;
    ...
    function mint(uint quantity) external payable {
        ...
        require(mintedQuantity + quantity <= MAX_SUPPLY, "All sold out");
        if (isWLActive) {
            require(_wlAccess[msg.sender] == true, "address not in the whitelist");
        }
        require(ownerTokenIds.length + quantity <= maxMintQuantity, "Cannot mint more than maximum quantity per wallet");
    ...
    // airdrop tokens to Mint Pass holders
    function airdropToMintPassHolders() external onlyOwner {
        uint256 mintedQuantity = totalSupply();
        require(mintedQuantity < MAX_SUPPLY, "All sold out");
        uint256 mintPassTotalSupply = ProjectMaybeMintPass(_mintPassContract).totalSupply();
        for (uint256 i=1; i <= mintPassTotalSupply; i++) {
            address owner = ProjectMaybeMintPass(_mintPassContract).ownerOf(i);
            for (uint256 j=0; j < 50; j++) {
                safeMint(owner);
            }
            // GOLD pass can mint 2
            if (i <= 3) {
    ...
    // airdrop tokens to eligible addresses
    function airdrop(address[] memory addresses, uint max) external onlyOwner {
        for (uint256 i=0; i < addresses.length; i++) {
            for (uint j=0; j < max; j++) {
                uint256 mintedQuantity = totalSupply();
                uint256[] memory ownerTokenIds = walletOfOwner(addresses[i]);
    ...
    function toggleSales(bool wl, bool sales) external onlyOwner {
        isWLActive = wl;
        isSalesActive = sales;
    }
