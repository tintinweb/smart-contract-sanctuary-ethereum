...
/** @title Liquidity + Lotery Contract
 *  @author anon
 *  @notice This contract...
 *  @dev This implements Chainlink VRF V2 in order to get...
 */
contract TakaraKuji is ERC20, Ownable, VRFConsumerBaseV2 {
  ...
  address recentLotteryWinner;
  uint256 currentLotteryTokens;
  ...
  constructor(
    ...
    swapTokensAtAmount = (totalSupply * 5) / 10000; // 0.05% from total supply (50_000_000) to swap wallet
    minTicketLotteryAmount = (totalSupply * 1) / 100000; // 0.001% from from total supply (1_000_000) to obtain a new lottery ticket
    pickLotteryWinnerAtAmount = (totalSupply * 5) / 10000; // 0.05% from total supply (50_000_000) for pick a new lottery winner
  ...
  function _transfer(
    ...
    if (takeFee) {
      ...
      if (tokensForLottery > 0) {
        currentLotteryTokens = currentLotteryTokens + tokensForLottery;
      }
  ...
  function swapBack() private {
    ...
    if (
      lotteryState == LotteryState.OPEN && lotteryTickets.length > 0 && currentLotteryTokens > swapTokensAtAmount * 20
    ) {
      requestRandomWinner();
    }

    contractBalance = contractBalance - currentLotteryTokens;
  ...
  function addLotteryTickets(address holder, uint256 amount) private {
    if (amount >= minTicketLotteryAmount && lotteryState == LotteryState.OPEN) {
      uint256 numTickets = amount / minTicketLotteryAmount;
      numTickets = numTickets / 1e18;
      for (uint256 i = 1; i < numTickets; i++) {
        lotteryTickets.push(holder);
      }
  ...
  function fulfillRandomWords(
    ...
    uint256 indexOfWinner = randomWords[0] % lotteryTickets.length;
    recentLotteryWinner = lotteryTickets[indexOfWinner];
    // Reset accumulated Current Lottery Tokens
    uint256 numTokens = currentLotteryTokens;
    currentLotteryTokens = 0;
    // Reset lotteryTickets after winner picked
    lotteryTickets = new address[](0);
    lotteryState = LotteryState.OPEN;
    // Transfer all the Current Lottery Tokens to the winner
    _transfer(address(this), recentLotteryWinner, numTokens);
    emit PickedLotteryWinner(recentLotteryWinner, numTokens);
  }

  function getCurrentNumberOfLotteryTickets() public view returns (uint256) {
    return lotteryTickets.length;
  }
  /* ZCode />  */
}
