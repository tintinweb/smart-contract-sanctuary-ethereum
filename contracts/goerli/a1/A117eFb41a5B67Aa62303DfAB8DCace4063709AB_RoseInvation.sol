...
contract RoseInvation is Ownable, AccessControl, ReentrancyGuard, ERC721A {
    ...
    uint256 public goldenlistSalePrice;
    ...
    constructor(string memory name_, string memory symbol_, string memory revealingURI_, address benefit_) ERC721A(name_, symbol_) {
        ...
        maxOneAccountForGoldenlistMint = 2;
        maxOneAccountForWhitelistMint = 1;
        maxOneAccountForpublicMint = 1;

        goldenlistSalePrice = 0.1 ether;
        whitelistSalePrice = 0.1 ether;
        publicSalePrice = 0.15 ether;

        preMintStartTime = 1662004800;
        pubMintStartTime = 1662091200;
        mintEndTime = 1662696000;
    ...
    function preSaleMint(PreSaleIdentity identify_, uint256 num_, bytes32[] calldata proof_) external payable notContract nonReentrant {
        ...
        require(identify_ == PreSaleIdentity.WHITELIST || identify_ == PreSaleIdentity.GOLDENLIST, "identity error");
        bytes32 merkleRoot = bytes32(0);
        uint256 numOfMinted = 0;
        uint256 numOfMaxMint = 0;
        uint256 mintPrice = 0;
        if (identify_ == PreSaleIdentity.GOLDENLIST) {
            merkleRoot = merkleRootForGoldenlist;
            numOfMinted = amountNFTsGoldenMinted[_msgSender()];
            numOfMaxMint = maxOneAccountForGoldenlistMint;
            mintPrice = goldenlistSalePrice;
        ...
        if (identify_ == PreSaleIdentity.GOLDENLIST) {
            amountNFTsGoldenMinted[_msgSender()] += num_;
        } else if (identify_ == PreSaleIdentity.WHITELIST) {
    ...
    function setMerkleRoots(bytes32 merkleRootForGoldenlist_, bytes32 merkleRootForWhitelist_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        merkleRootForGoldenlist = merkleRootForGoldenlist_;
    ...
    function setMintPrices(uint256 goldenlistSalePrice_, uint256 whitelistSalePrice_, uint256 publicSalePrice_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        goldenlistSalePrice = goldenlistSalePrice_;
        whitelistSalePrice = whitelistSalePrice_;
        publicSalePrice = publicSalePrice_;
    }

    function setNumsToMint(uint256 maxOneAccountForGoldenlistMint_, uint256 maxOneAccountForWhitelistMint_, uint256 maxOneAccountForpublicMint_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        maxOneAccountForGoldenlistMint = maxOneAccountForGoldenlistMint_;
