...
contract MissionsManager is Ownable {
	...
	uint256 public constant ETH_SUPPLY = 250;
	uint256 public constant SC_SUPPLY = 250;
	address public signer;
	YinYangGang yygContract;
	MinterContract minterContract;
	uint256 public ethMinted;
	uint256 public scMinted;
	...
	function claimBurnPass(uint256[] memory _ids, uint256 _season) external {
	...
	function purchasePackETH(uint256 _uniqueId, uint256 amount, bytes memory _signature) external payable {
		if (amount == 0) revert InvalidAmount();
		if (msg.value <  amount * ETH_PRICE) revert InsufficientETH();
		if (ethMinted >= ETH_SUPPLY) revert InsufficientETHSupply();
		uint256 userNonce = nonces[msg.sender];
		bytes memory signData = abi.encode(msg.sender, _uniqueId, userNonce, 'ETH', amount);
		if (!verifySigner(signData, _signature)) revert UnauthorisedSignature();
		nonces[msg.sender]++;
		ethMinted++;
		uint256[] memory tokenIds = minterContract.mint(msg.sender, amount);
		emit PackMint(_uniqueId, msg.sender, tokenIds, 'ETH');
	}

	function purchasePackSC(uint256 _uniqueId, uint256 amount, bytes memory _signature) external {
		if (amount == 0) revert InvalidAmount();
		if (scMinted >= SC_SUPPLY) revert InsufficientSCSupply();
		uint256 userNonce = nonces[msg.sender];
		bytes memory signData = abi.encode(msg.sender, _uniqueId, userNonce, 'SC', amount);
		if (!verifySigner(signData, _signature)) revert UnauthorisedSignature();
		nonces[msg.sender]++;
		scMinted++;
		uint256[] memory tokenIds = minterContract.mint(msg.sender, amount);
