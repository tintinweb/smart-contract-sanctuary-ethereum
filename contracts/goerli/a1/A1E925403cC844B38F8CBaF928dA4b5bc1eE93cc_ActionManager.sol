    ...
    ReentrancyGuardUpgradeable
{
    ...
    /**
     * @dev Address of the PermissionManager contract
     */
    ...
    /**
     * @dev Emitted after `permissionManagerAddress` is set
     */
    event PermissionManagerSet(address indexed _permissionManagerAddress);
     ...
     * @param _permissionManagerAddress sets the permissionManagerAddress address
     */
    function initialize(
    ...
    /**
     * @dev Sets `_permissionManagerAddress` as the permissionManager address
     * @param _permissionManagerAddress PermissionManager address contract
     */
    function setPermissionManager(address _permissionManagerAddress) external onlyAdmin {
        _setPermissionManagerAddress(_permissionManagerAddress);
    }
    ...
    function _setPermissionManagerAddress(address _permissionManagerAddress) internal {
        ...
        emit PermissionManagerSet(_permissionManagerAddress);
    ...
    // this function is tested in the SmtPriceFeed contract
    ...
    // this function is tested in the SmtPriceFeed contract
    // this is just a tribute :) (this is a copy just to make it external for the FE/BE to call it)
    function getAvgAmountFromPools(
        address _assetIn,
        address _assetOut,
        uint256 _assetAmountIn
    ) external view returns (uint256) {
        address[] memory poolAddresses = IBRegistry(bRegistryAddress).getBestPoolsWithLimit(_assetIn, _assetOut, 10);
    ...
    function standardPoolCreation(
        ...
        require(IPermissionManager(permissionManagerAddress).hasTier2(_msgSender()), "Sender is not Tier2");
