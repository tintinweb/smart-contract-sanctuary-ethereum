...
contract SmartWalletFactory {
	...
	event ContractCreated(address owner, address SmartWallet);

	function deploySmartWallet(address owner, address secureExecutor) external {
		SmartWallet wallet = new SmartWallet(owner, secureExecutor);

		wallets[owner] = address(wallet);

		emit ContractCreated(owner, address(wallet));
	}
}