...
contract Strategy{
    ...
    function setPoolDetails(address _pool) external {
        pool = IUniswapV3Pool(_pool);
        tickSpacing = pool.tickSpacing();
    }

    function getSqrtRatioX96AndTick(IUniswapV3Pool __pool)
        internal
        view
        returns (
    ...
    {
        (_sqrtRatioX96, _tick, , observationCardinality, , , ) = __pool.slot0();
    }
    ...
    function getTicks( 
        ...
        require(tickSpacing > 0, "Pool not set"); 
        (, int24 currentTick, ) = getSqrtRatioX96AndTick(pool);

        (int24 _tickLower, int24 _tickUpper)= UniswapLiquidityManagement.getBaseTicks(currentTick,baseThreshold,tickSpacing);
