...
contract LandNFT is ERC721, Ownable {
  ...
  constructor(address adminSigner) payable ERC721("LandNFT", "LND") {
  ...
  function mintPresale(
    ...
    ensureMaxPerWalled(count)
  {
    require(phase == SalePhase.PreSale, "Pre-sale event is not active");
    require(
      count + _walletMints[msg.sender] <= maxPerWallet,
  ...
  function safeMint(address sender_, uint256 quantity_) private {
    ...
    _walletMints[sender_] = _walletMints[sender_] + quantity_;
  }

  modifier ensureAvailabilityFor(uint256 count) {
    require(
      count + totalSupply <= maxSupply,
  ...
  modifier validateEthPayment(uint256 count) {
    require(count * mintPrice == msg.value, "Ether value sent is not correct");
    _;
  }

  modifier ensureMaxPerWalled(uint256 count) {
    require(
      count + _walletMints[msg.sender] <= maxPerWallet,
      "Exceeds number of presale mints allowed"
    );
