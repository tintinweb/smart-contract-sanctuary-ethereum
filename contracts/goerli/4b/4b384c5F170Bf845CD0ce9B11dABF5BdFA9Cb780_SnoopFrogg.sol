...
contract SnoopFrogg is ERC721, AccessControl  {
  ...
  uint256 public costPub = 0.029 ether;
  uint256 public costWL = 0.029 ether;
  ...
  constructor() ERC721("Snoop Frogg", "SNFR") {

     // Grant the contract deployer the default admin role: it will be able
    // to grant and revoke any roles
    _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    //Required for polygon mapping
    _grantRole(PREDICATE_ROLE, 0x932532aA4c0174b8453839A6E44eE09Cc615F2b7);
    owner = msg.sender;
    setHiddenMetadataUri("ipfs://QmYZjoFmHFSH8VSYndgdwhSun77F5ZhDfGboRtNHhnvjPg/hidden.json"); 
  }
   ...
   function whitelistMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    function userMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
        require(!paused, "The contract is paused!");
        if (msg.sender != owner) {
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
    ...
    function setCostPub(uint256 _costpub) public onlyRole(DEFAULT_ADMIN_ROLE) {
        costPub = _costpub;
    }

    function setCostWL(uint256 _costwl) public onlyRole(DEFAULT_ADMIN_ROLE) {
        costWL = _costwl;
    }

    function setMaxMintAmountPerWallet(uint256 _maxMintAmountPerWallet) public onlyRole(DEFAULT_ADMIN_ROLE) {
        maxMintAmountPerWallet = _maxMintAmountPerWallet;
    }

    function setMaxMintAmountPerWalletWL(uint256 _maxMintAmountPerWalletWL) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setWhitelistActive(bool _state) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function withdraw() public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        // You can comment out the next 2 lines if you want, or leave them as is to support MMWABB and his work.
        // =============================================================================
