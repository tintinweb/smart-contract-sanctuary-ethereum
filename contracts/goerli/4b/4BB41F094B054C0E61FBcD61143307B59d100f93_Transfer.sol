...
contract Transfer is IStargateReceiver, Ownable {
    ...
    /// @param _bridgeTokenAmount The amount of bridge token to transfer to the destination chain
        ...
        SafeERC20.safeApprove(IERC20(bridgeTokenAddress), stargateRouterAddress, 0);
        SafeERC20.safeApprove(IERC20(bridgeTokenAddress), stargateRouterAddress, _bridgeTokenAmount);

        // TODO: set lz tx obj properly
        ...
        IStargateRouter(stargateRouterAddress).swap{value:_bridgeTxn.fee}(
            ...
            _bridgeTxn.srcPoolId,                             // the source Stargate poolId //TODO: Hard code these as contract variables?
            ...
            0,                                                // min amount allowed out //TODO: UPDATE THIS
            IStargateRouter.lzTxObj(_bridgeTxn.dstChainGasFee, 0, "0x"),         // default lzTxObj
    ...
    function transfer(
        ...
        if (address(_swapTxn.tokenIn) != NATIVE_TOKEN) {
            ...
            require(msg.value >= _swapTxn.tokenInAmount + _bridgeTxn.fee, "insufficient value for transfer"); //0x swap fee is tokenInAmount for native
        }
    ...
    function sgReceive(
        ...
        if (_token == receivingSwapTxn.dstSwapToken) { 
            // No swap needed on dst chain
            SafeERC20.safeTransfer(IERC20(_token), receivingSwapTxn.dstChainTo, amountLD);
            ...
            uint256 preLocalTokenBalance = IERC20(_token).balanceOf(address(this));
            ...
            uint256 dstSwapTokenBalance = swap(
                ...
                receivingSwapTxn.router, //TODO: verify swap router is 0x router
                receivingSwapTxn.txnCallData
            );

            // Post swap token balances
            uint256 postLocalTokenBalance = IERC20(_token).balanceOf(address(this));
            uint256 localTokenSwappedOutBalance = preLocalTokenBalance - postLocalTokenBalance;
            uint256 leftOverLocalTokenBalance = amountLD - localTokenSwappedOutBalance;
            ...
            if (leftOverLocalTokenBalance > 0) {
                SafeERC20.safeTransfer(IERC20(_token), receivingSwapTxn.dstChainTo, leftOverLocalTokenBalance);
            } 
    ...
    function quoteLayerZeroFee(
        ...
        return IStargateRouter(stargateRouterAddress).quoteLayerZeroFee(
            _dstChainId,
            1, // SWAP function type
            abi.encodePacked(_toAddress),
            abi.encode(_receivingSwapTxn),
            _lzTxParams
        );
    }
