...
contract NounletGovernance is IGovernance {
    ...
    /// @param _vault Address of the vault
    function claimDelegate(address _vault) external {
        uint256 previousBlock = block.number - 1;
        address token = IRegistry(registry).vaultToToken(_vault);
        uint256 callerVotes = Nounlet(token).getPriorVotes(msg.sender, previousBlock);
        uint256 delegateVotes = Nounlet(token).getPriorVotes(
            currentDelegate[_vault],
            previousBlock
        );
        if (delegateVotes >= callerVotes) revert InsufficientVotes();
        currentDelegate[_vault] = msg.sender;
    }
...
contract NounletToken is Clone, ERC1155BCheckpointable, INounlet, PermitBase {
    ...
    mapping(uint256 => INounsSeeder.Seed) seeds;
