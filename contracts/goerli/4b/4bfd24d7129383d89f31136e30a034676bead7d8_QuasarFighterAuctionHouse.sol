    ...
    Ownable
{
    ...
    uint256 public minimumBid = .001 ether; // The minimum price accepted in an auction
    uint256 public minBidIncrement = .001 ether; // The minimum amount by which a bid must exceed the current highest bid
    uint256 public allowListPrice = .001 ether; // The allowlist price
    uint256 public duration = 3600; // 86400 == 1 day The duration of a single auction in seconds
    ...
    uint256 public auctionSupply = 300; // number of auction supply max of raffle ticket
    uint256 public allowlistSupply = 177; // number allowlist supply
    uint256 public maxSupply = 256; // total supply of qf
    uint256 public promoSupply = 39; // promo supply of qf

    uint256 public allowListStartDateTime = block.timestamp; // block.timestamp; 
    uint256 public allowListEndDateTime = allowListStartDateTime + duration; 
    uint256 public auctionStartDateTime = allowListEndDateTime; 
    uint256 public auctionEndDateTime = auctionStartDateTime + duration; 
    ...
    constructor(QuasarFighter _quasarfighter) {
        ...
        setTimes(block.timestamp, duration);
    }
