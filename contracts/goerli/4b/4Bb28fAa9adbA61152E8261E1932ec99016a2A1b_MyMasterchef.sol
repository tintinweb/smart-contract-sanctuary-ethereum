...
// File contracts/MyMasterchef.sol
...
contract MyMasterchef is Ownable {
    struct UserInfo {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 sushiReward = ((block.number - pool.lastRewardBlock) *
                rdxPerBlock *
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSushiPerShare =
            pool.accSushiPerShare +
            ((sushiReward * 10**18) / lpSupply);
        pool.lastRewardBlock = block.number;
        emit UpdatePool(
            _pid,
            sushiReward,
            pool.accSushiPerShare + ((sushiReward * 10**18) / lpSupply)
        );
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = (user.amount * (pool.accSushiPerShare)) /
                1e12 -
                user.rewardDebt;
        ...
        user.rewardDebt = (user.amount * pool.accSushiPerShare) / 10**18;
    ...
    function safeSushiTransfer(address _to, uint256 _amount) internal {
        uint256 sushiBal = rdx.balanceOf(address(this));
        if (_amount > sushiBal) {
            rdx.transfer(_to, sushiBal);
        } else {
