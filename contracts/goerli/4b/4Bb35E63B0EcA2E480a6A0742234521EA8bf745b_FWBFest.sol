...
contract FWBFest is Ownable, ERC721, EIP712MetaTransaction {
    uint256 private _tokenId;

    IERC20 private _requiredTokenAddress;

    uint256 private _requiredTokenAmount;

    mapping(address => uint256) private _claims;

    event Claim(address indexed to, uint256 indexed tokenId);

    constructor(IERC20 requiredTokenAddress, uint256 requiredTokenAmount)
        ERC721("FWBFest", "FWBF")
        EIP712MetaTransaction("FWBFest", "0.1.0")
    {
        _requiredTokenAddress = requiredTokenAddress;
        _requiredTokenAmount = requiredTokenAmount;
    }
    ...
    function isEligible(address user) public view returns (bool) {
        return _requiredTokenAddress.balanceOf(user) >= _requiredTokenAmount;
    }
    ...
    function tokenURI(uint256 tokenId) public pure override returns (string memory) {
        return "https://ipfs.moralis.io:2053/ipfs/QmQS6ZGMHournF3uzXwVNyvLJVmUofrKNAxdZ68RGjnWXg/1.json";
    }
}
