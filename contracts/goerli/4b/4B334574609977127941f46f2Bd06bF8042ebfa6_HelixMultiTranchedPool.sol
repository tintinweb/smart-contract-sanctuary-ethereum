...
contract HelixMultiTranchedPool is BaseUpgradeablePausable, IHelixMultiTranchedPool, IRequiresUID, IVersioned {
  ...
  error TranchMaximumInvestmentExceeds(uint256 current, uint256 amount);
  ...
  function initialize(
    ...
    fundingDate = _days[4]; // funding date 
  ...
  function deposit(
    ...
    if (block.timestamp > fundingDate) {
      revert FundingDatePassed();
    }

    if (trancheInfo.principalDeposited  + amount > trancheInfo.maximumInvestment) {
      revert TranchMaximumInvestmentExceeds(trancheInfo.principalDeposited, amount);
    }
  ...
  function withdrawMultiple(
    ...
    require(tranches.length == amounts.length, "LEN");

    for (uint256 i = 0; i < amounts.length; i++) {
      withdrawWithPermit(tranches[i], amounts[i], deadline[i], v[i], r[i], s[i]);
    }
  }
   ...
   */
  function drawdown() external override onlyLocker notCancelled whenNotPaused returns(uint256) {
    ...
    creditLine.drawdown(amountAvailable, fundingDate);
  ...
  function cancel() external notCancelled onlyLocker {
    /// @dev AC: Already cancelled
    if (cancelled) {
...
//     IHelixCreditLineV2 originalCl = creditLine;
...
//   }
  ...
  function poolFundingEvaluate() public view returns(bool) {
    ...
    uint256 currentInvestmentRatio = juniorTrancheInfo.principalDeposited * PRECISION_POINT / seniorTrancheInfo.principalDeposited;
    ...
    if (
      totalPrincipalDeposited < creditLine.minLimit() 
    ) {
      return false;
    }
  ...
  function _createAndSetCreditLine(
    ...
    creditLine = IHelixCreditLineV2(config.getHelixFactory().createCreditLine());
  ...
  function _withdraw(
    ...
    if (!poolFundingEvaluate()) {
      ...
      /// @dev EA: exceeded amount. Cannot withdraw 0
      if (amount > poolShares.balanceOf(msg.sender)) {
        revert ExceededAmount();
      }

      // require(amount <= poolShares.balanceOf(msg.sender), "EA");
      
      poolShares.burn(msg.sender, amount);
      principalToRedeem = amount;
    } else {
      /// @dev NZ: Not Zero. Cannot withdraw > 0
      require(amount == 0, "NZ");
  ...
  function _lockPool() internal {
    ...
    require(poolFundingEvaluate(), "PE");
  ...
  function _getTrancheInfo(uint256 trancheId) internal view returns (TrancheInfo memory trancheInfo) {
    require(
      trancheId > 0 && trancheId <= numSlices * HelixTranchingLogic.NUM_TRANCHES_PER_SLICE,
      "invalid tranche"
    );
  ...
  event CreditLineMigrated(
    IHelixCreditLineV2 indexed oldCreditLine,
    IHelixCreditLineV2 indexed newCreditLine
  );
