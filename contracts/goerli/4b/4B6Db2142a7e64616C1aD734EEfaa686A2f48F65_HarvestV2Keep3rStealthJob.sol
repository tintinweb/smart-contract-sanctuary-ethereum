...
interface IKeep3rStealthJob is IKeep3rJob {
  ...
  /// @param stealthRelayer Address of the new StealthRelayer contract
  event StealthRelayerSet(address stealthRelayer);
...
interface IKeep3rMeteredJob is IKeep3rJob {
  ...
  /// @param keep3rHelper Address of the new Keep3rHelper contract
  event Keep3rHelperSet(address keep3rHelper);

  /// @notice Emitted when a new gas bonus amount is set
  /// @param gasBonus Amount of gas to add to cover unaccounted gas
  event GasBonusSet(uint256 gasBonus);

  /// @notice Emitted when a new gas bonus multiplier is set
  /// @param gasMultiplier Multiplier that boosts gas record to calculate the keeper reward
  event GasMultiplierSet(uint256 gasMultiplier);

  /// @notice Emitted when a new gas bonus multiplier maximum is set
  /// @param maxMultiplier Maximum acceptable gasMultiplier to be set
  event MaxMultiplierSet(uint256 maxMultiplier);

  /// @notice Emitted when a metered job is worked
  /// @param initialGas First gas record registered
  /// @param gasAfterWork Gas record registered after work
  /// @param bonus Fixed amount of gas added to the accountance
