...
abstract contract MarketplaceCore is IMarketplaceCore {
    ...
    function _accept(uint40 listingId, address[] calldata addresses, uint256[] calldata amounts, uint256 maxAmount) internal virtual {
        ...
        if (MarketplaceLib.isAuction(listingType)) {
            ...
            __accept(listingId, listing, payable(addresses[0]), amounts[0], maxAmount);
            ...
            for (uint i; i < addressLength;) {
                __accept(listingId, listing, payable(addresses[i]), amounts[i], maxAmount);
                unchecked { ++i; }
            }
        }
    }

    function __accept(uint40 listingId, MarketplaceLib.Listing storage listing, address payable offerAddress, uint256 expectedAmount, uint256 maxAmount) private {
...
interface IMarketplaceCore {
     ...
     * @dev Create listing
     */
    function createListing(address payable seller, MarketplaceLib.ListingDetails calldata listingDetails, MarketplaceLib.TokenDetails calldata tokenDetails, MarketplaceLib.ListingReceiver[] calldata listingReceivers, bool enableReferrer, bool acceptOffers) external returns (uint40);
