...
contract NyxQuorumProposal is Ownable, converter {
    string public constant name = "Nyx Proposal - Quorum";
    uint256 public constant nparams = 2;
    ...
    function parseProposalParams(bytes[] memory params)
        public pure
        returns (QuorumProposalParams memory)
    {
        require(params.length == nparams, "wrong format");

        return QuorumProposalParams(converter.bytesToUint(params[0]),
        ...
        returns (QuorumProposalParams memory)
    {
        ...
        return QuorumProposalParams(converter.bytesToUint(param1),
        ...
        returns (QuorumProposalParams memory)
    {
        if (keccak256(params) != keccak256(bytes("")))
        {
            ...
            return QuorumProposalParams(param1,
                param2);
        }
        else
        {
            QuorumProposalParams memory nullProposal;
            return nullProposal;
        }
    }    

    function proposalToBytes(QuorumProposal memory proposal)
        public pure
        returns (bytes memory)
    {
        return abi.encode(proposal.minVoters, proposal.minApproval);
    }
    ...
    function createProposal(uint256 proposalId, bytes[] memory params)
        ...
        returns (bytes memory)
    {
        QuorumProposalParams memory parsedParams = parseProposalParams(params);
        QuorumProposal memory proposal = QuorumProposal(proposalId, parsedParams.minVoters, parsedParams.minApproval);
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        QuorumProposalParams memory params = parseProposalParams2(proposalBytes);
        bytes[] memory prop = new bytes[](nparams);
        prop[0] = abi.encode(params.minVoters);
        prop[1] = abi.encode(params.minApproval);
        return prop;
    }
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            QuorumProposalParams memory params = parseProposalParams2(proposalBytesArr[idx]);
            props[idx] = [abi.encode(params.minVoters), abi.encode(params.minApproval)];
        }
    }
}
