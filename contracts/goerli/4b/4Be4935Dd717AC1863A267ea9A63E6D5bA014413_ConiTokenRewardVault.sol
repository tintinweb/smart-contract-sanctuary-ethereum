...
contract ConiTokenRewardVault is Ownable, Pausable {
    ...
    // events
    event TokensClaimed(address indexed to, uint256 amount, uint256 nonce);
    ...
    function withdraw(uint256 amount, bytes memory signature) public whenNotPaused {
        uint256 walletNonce = _useNonce(msg.sender);
        if (verifySignature(msg.sender, amount, walletNonce, signature) != true) {
        ...
        emit TokensClaimed(msg.sender, amount, walletNonce);
    }
