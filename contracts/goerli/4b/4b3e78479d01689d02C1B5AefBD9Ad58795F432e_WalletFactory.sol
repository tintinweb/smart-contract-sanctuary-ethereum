...
contract WalletFactory {
    ...
    // could be passed in a param in deploy
    ...
    //deployWallet : create2
    //could pass defaultImpl address here which would be main module
    function deployCounterFactualWallet(address _owner) public returns(address proxy){
        bytes32 salt = keccak256(abi.encodePacked(_owner, _owner));
        ...
        BaseImplementation(proxy).init(_owner);
        isWalletExist[_owner] = true;
    }

    function deployWallet(address _owner) public returns(address proxy){ //Proxy vs address in return type
        bytes memory deploymentData = abi.encodePacked(type(Proxy).creationCode, uint(uint160(_defaultImpl)));
        // solhint-disable-next-line no-inline-assembly
        assembly {
    ...
    function getAddressForCounterfactualWallet(address _owner) external view returns (address _wallet) {
    ...
    //generateSalt : also check https://docs.sequence.build/wallet-contracts/modules-and-updates
