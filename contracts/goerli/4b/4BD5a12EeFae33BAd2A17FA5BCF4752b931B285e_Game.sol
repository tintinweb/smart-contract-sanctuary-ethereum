...
contract Game is Buildings, Units {
    ...
    function gatherResource(ResourceType _resource, uint[] memory _workers) public {
        for(uint i=0; i < _workers.length; i++ ) {
            if(playerResources[msg.sender].workers[_workers[i]].isAvail == true) {
               playerResources[msg.sender].workers[_workers[i]].isAvail = false;
               playerResources[msg.sender].workers[_workers[i]].timeDeployed = block.timestamp; 
               playerResources[msg.sender].workers[_workers[i]].resourceType = _resource;
            }
        }
    }
