...
contract AssetContractShared is AssetContract, ReentrancyGuard {
    ...
    ) AssetContract(_name, _symbol, _proxyRegistryAddress, _templateURI, _platformMintingfee, _royaltyFeeRecipient) {
 ...
 */
contract AssetContract is ERC1155Tradable, ERC2981 {
    ...
    constructor(
        ...
        require(_recipient != address(0), "receiver address should not be zeroAddress" );
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155, ERC2981) returns (bool) {
        return (interfaceId == type(IERC2981).interfaceId ||  interfaceId == type(IERC1155).interfaceId ||super.supportsInterface(interfaceId));
    }
     ...
     */
    function setPlatformMintingFee(uint256 _fee) public onlyOwnerOrProxy {
        require(_fee <= _feeDenominator(), "Cannot be more than 100.00%");
        platformMintingFee = _fee;
    }
     ...
     */
     function setRoyaltyFeeRecipient(address _recipient) public onlyOwnerOrProxy {
        require(_recipient != address(0), "receiver should not be zeroAddress" );
        royaltyFeeRecipient = _recipient;
    }
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(templateURI, Strings.toString(_id)));
    }
     ...
     */
    function setRoyaltyFeeLimit(uint256 _royaltyFeeLimit) public onlyOwnerOrProxy {
        require(_royaltyFeeLimit + platformMintingFee <= _feeDenominator(), "Total royalty fee will exceed salePrice");
    ...
    function safeTransferFrom(
        ...
        if(_data.length>0 && _isCreatorOrProxy(_id, _msgSender())) {
            ...
            require(_royaltyAmount <= royaltyFeeLimit, "Royalty fee exceeded royalty fee limit");
