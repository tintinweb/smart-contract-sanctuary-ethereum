...
// File contracts/superfine/SuperfineAirdropV3.sol
pragma solidity ^0.8.18;

contract SuperfineAirdropV3 is Ownable, ReentrancyGuard {
    enum AssetType {
        ERC20,
        ERC721,
        ERC1155
    ...
    constructor(uint256 maxBatchSize, uint256 feePerBatch) Ownable() {
        require(
            maxBatchSize > 0,
            "SuperfineAirdropV3: the size of the batch must be greater than zero"
        );
    ...
    modifier onlyOperators() {
        require(
            ...
            "SuperfineAirdropV3: the caller is not the operator"
        );
        _;
    }
    ...
    function setOperators(
        ...
        require(
            ...
            "SuperfineAirdropV3: the number of operators and the number of statuses must be equal"
        );
    ...
    function createAirdropCampaign(
        ...
        require(
            ...
            "SuperfineAirdropV3: the campaign has already been created before"
        );
        ...
        require(
            ...
            "SuperfineAirdropV3: the paid fee is insufficient"
        );
        if (msg.value > airdropFee) {
            ...
            require(
                success,
                "SuperfineAirdropV3: the excess is failed to be returned"
            );
        }
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            require(
                ...
                "SuperfineAirdropV3: the asset type is invalid"
            );
            if (asset.assetType == AssetType.ERC20)
                require(
                    asset.assetId == 0,
                    "SuperfineAirdropV3: the asset ID of the ERC20 token must be 0"
                );
            else if (asset.assetType == AssetType.ERC721)
                require(
                    asset.amount == 1,
                    "SuperfineAirdropV3: the amount of the ERC721 token must be 1"
                );
        }
    ...
    function updateCampaign(
        ...
        require(
            campaign.creator == msg.sender,
            "SuperfineAirdropV3: the caller is not the owner of the campaign"
        );

        // Make sure that this campaign has not started yet
        require(
            !campaign.airdropStarted,
            "SuperfineAirdropV3: the assets of the campaign cannot be updated since the airdrop process has already started"
        );
        ...
        if (newAirdropFee > campaign.chargedFee) {
            require(
                ...
                "SuperfineAirdropV3:insufficient airdrop fee"
            );
    ...
    function airdrop(
        ...
        require(
            ...
            "SuperfineAirdropV3: the campaign does not exist"
        );
        AirdropCampaign storage campaign = _campaignById[campaignId];
        require(
            assetIndexes.length == recipients.length,
            "SuperfineAirdropV3: the number of assets and the number of recipients must be equal"
        );
        require(
            assetIndexes.length <= campaign.maxBatchSize,
            "SuperfineAirdropV3: the number of assets must not exceed the maximum size of a single batch"
        );
        ...
        for (uint256 i = 0; i < assetIndexes.length; i++) {
            require(
                ...
                "SuperfineAirdropV3: the asset index cannot be larger than the total number of assets in this campaign"
            );
            airdroppedAssets[i] = campaign.assets[assetIndexes[i]];
            Asset storage asset = campaign.assets[assetIndexes[i]];
            require(
                asset.amount > 0,
                "SuperfineAirdropV3: this reward has already been sent before"
            );
            if (asset.assetType == AssetType.ERC20) {
                ...
                require(
                    success,
                    "SuperfineAirdropV3: the transfer process of the ERC20 assets failed"
                );
                asset.amount = 0;
            } else if (asset.assetType == AssetType.ERC721) {
    ...
    function withdrawAirdropFee(address recipient) external onlyOwner {
        ...
        require(
            success,
            "SuperfineAirdropV3: the process of withdrawing airdrop fee is failed"
        );
    }
}