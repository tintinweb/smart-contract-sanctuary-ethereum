...
interface IIconManager {
    function getUsing(address holder)
        external
        view
        returns (address collectionAddress, uint256 tokenId);
...
import "./ICreatorCollectionMinter.sol";
import "./ICollectionFactoryStorage.sol";
...
contract CollectionFactory is W3JEAccessControled, Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;

    ICollectionFactoryStorage public collectionFactoryStorage;
    ...
    function createCollection(string memory name_, string memory symbol_)
        external
        payable
        enoughEth
    {
        CreatorCollection created = new CreatorCollection(
            name_,
            symbol_,
            msg.sender,
            address(ac),
            collectionFactoryStorage.locker(),
            collectionFactoryStorage.tokenUriSupplier(),
            collectionFactoryStorage.iconManager(),
            collectionFactoryStorage.calAddress(),
            collectionFactoryStorage.minter()
        );

        collectionFactoryStorage.addCollections(address(created));

        emit CollectionCreated(msg.sender, address(created), name_, symbol_);
    }

    // ==================================================================
    // For operation
    // ==================================================================
    function setCost(uint256 value) external onlyAdmin {
        cost = value;
    }

    function setCollectionFactoryStorage(address value) external onlyAdmin {
        collectionFactoryStorage = ICollectionFactoryStorage(value);
    }

    // ==================================================================
    // For AccessControlled
    // ==================================================================
    function owner()
        public
        view
        virtual
        override(Ownable, W3JEAccessControled)
        returns (address)
    {
...
import "./ICreatorCollection.sol";
import "../CertOfRegistration/IIconManager.sol";
import "../AccessControl/W3JEAccessControled.sol";
import "./Locker/Lockable.sol";
import "./TokenUriSupplier/ICreatorCollectionTokenUriSupplier.sol";
    ...
    Lockable
{
    ...
    ICreatorCollectionTokenUriSupplier public tokenUriSupplier;

    // ==================================================================
    // Variables
    // ==================================================================
    IIconManager public iconManager;
    ...
    constructor(
        ...
        address ac_,
        address locker_,
        address tokenUriSupplier_,
        address iconManager_,
        address calAddress_,
        address minter_
    ) ERC721Psi(name_, symbol_) {
        _setAc(ac_);
        _setLocker(locker_);
        _setTokenUriSupplier(tokenUriSupplier_);
        _setIconManager(iconManager_);
        _setCAL(calAddress_);
        minter = minter_;
    ...
    function setIconManager(address value) external onlyOwnerOrAdmin {
        _setIconManager(value);
    }

    function _setIconManager(address value) internal {
        iconManager = IIconManager(value);
    }

    // ==================================================================
    // overrive ERC721Psi for Using Registration
    // ==================================================================
    modifier notUsingRegistration(uint256 tokenId) {
        require(
            !iconManager.isUsing(ownerOf(tokenId), address(this), tokenId),
            "this token is using cert of registration."
        );
        _;
    }

    // ==================================================================
    // overrive ICreatorCollection
    // ==================================================================
    modifier onlyMinter() {
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override notUsingRegistration(tokenId) onlyAllowedOperator(from) {
    ...
    {
        return tokenUriSupplier.tokenURI(address(this), tokenId);
    }

    function setTokenUriSupplier(address value) external onlyOwnerOrAdmin {
        _setTokenUriSupplier(value);
    }

    function _setTokenUriSupplier(address value) internal {
        tokenUriSupplier = ICreatorCollectionTokenUriSupplier(value);
    }
    ...
    function setCAL(address calAddress) external onlyOwnerOrAdmin {
        _setCAL(calAddress);
    }

    function setCALLevel(uint256 level) external onlyOwnerOrAdmin {
        CALLevel = level;
    }

    function setEnableRestrict(bool value) external onlyOwnerOrAdmin {
    ...
    // ==================================================================
    function setContractLock(LockStatus lockStatus) external onlyOwnerOrAdmin {
    ...
    function setEnableLock(bool value) external onlyOwnerOrAdmin {
        enableLock = value;
    }
        ...
        returns (bool)
    {
        ...
        return
            ...
            _isLocked(address(this), tokenId);
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721AntiScam, ERC2981)
        returns (bool)
    {
        return
            ERC721Psi.supportsInterface(interfaceId) ||
    ...
    function renounceOwnership() public view override onlyOwner {
        revert(
            "Can not renounceOwnership. In the absence of the Owner, the system will not be operational."
        );
    }
...
interface ICollectionFactoryStorage {
    function calAddress() external view returns (address);

    function minter() external view returns (address);

    function tokenUriSupplier() external view returns (address);

    function locker() external view returns (address);

    function iconManager() external view returns (address);

    function getCollections() external view returns (address[] memory);

    function existsCollections(address collectionAddress)
        external
        view
        returns (bool);

    function addCollections(address collectionAddress) external;

    function setCalAddress(address value) external;

    function setMinter(address value) external;

    function setLocker(address value) external;

    function setTokenUriSupplier(address value) external;

    function setIconManager(address value) external;
}
...
interface ICreatorCollectionMinter {
    function setCost(address collectionAddress, uint256 value) external;

    function mint(address collectionAddress, bytes calldata signature)
        external
        payable;
}
...
abstract contract Lockable is W3JEAccessControled {
    ...
    function _isLocked(address contractAddress, uint256 tokenId)
        ...
        returns (bool)
    {
        return locker.isLocked(contractAddress, tokenId);
    }

    function setLocker(address value) external onlyOwnerOrAdmin {
        _setLocker(value);
    }
...
interface ICreatorCollectionTokenUriSupplier {
    function tokenURI(address contractAddress, uint256 tokenId)
        external
        view
        returns (string memory);
}
