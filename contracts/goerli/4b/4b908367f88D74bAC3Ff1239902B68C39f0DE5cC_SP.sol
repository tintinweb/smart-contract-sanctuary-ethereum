...
// Author: @EvilEye0666
contract SP is Ownable, ReentrancyGuard, ERC721Enumerable {
  // startHoldTime
  mapping(uint256 => uint256) public startHoldTime;
  //whistlist
  ...
  //   require(MerkleProof.verify(proof, root, _leaf(sender)), 'Address not in whitelist.');
  //   require(!minted, 'Address already minted');
  //   require(tokenId + _quantity <= preSaleMax, 'Exceed to max supply of premint.');
  ...
  function pubMint(uint256 _quantity) external payable callerIsUser nonReentrant {
    uint256 tokenId = totalSupply();
    // require(!preMintOpen, 'Public mint not open yet.');
  ...
  function _minNFT(address _addr, uint256 _quantity) internal {
    ...
    for (uint256 i = 0; i < _quantity; i++) {
      startHoldTime[tokenId + i] = block.timestamp;
  ...
  //set token url
  function setBaseURI(uint256 num, string memory _newBaseURI) public onlyOwner {
    require(num <= 3 && num >= 1, 'setURI error.');
    baseURIs[num] = _newBaseURI;
  }
  ...
  // *** get hold period
  function getHoldPeriod(uint256 _tokenid) public view returns (uint256) {
    _requireMinted(_tokenid);
    uint256 nowTime = block.timestamp;
    return startHoldTime[_tokenid] - nowTime;
  }

  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    _requireMinted(tokenId);
    string memory baseURI = _baseURI();
    uint256 period = getHoldPeriod(tokenId);
    // if (period >= 7776000) {
    //   baseURI = baseURIs[3];
  ...
  function withdraw(uint256 _amount) public payable onlyOwner {
    uint256 amount = _amount == 666666 ? address(this).balance : _amount;
