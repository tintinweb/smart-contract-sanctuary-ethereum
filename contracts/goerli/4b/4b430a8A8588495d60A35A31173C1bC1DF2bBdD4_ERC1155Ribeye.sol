...
/**
 * @title ERC1155Ribeye
 * ERC1155Ribeye - ERC-1155 smart contract supporting unique items and splitting profits between shareholders
 */
contract ERC1155Ribeye is ERC1155, ERC1155Holder, Ownable {
    // Constants
     ...
     */
    function mint(uint8 amount, address _to) public payable  validAmount(amount){
        require(msg.value == (mintPrice * amount), "Wrong minting fee");
        require(amount + _currentTokenID <= maxTotalSupply, "Token limit exceeded");
        for(uint8 i=0; i < amount; i++){
            _mint(_to, _currentTokenID + 1, 1, "");
            _currentTokenID++;
        }
    }
