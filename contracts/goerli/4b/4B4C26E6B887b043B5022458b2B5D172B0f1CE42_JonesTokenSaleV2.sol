...
contract JonesTokenSaleV2 {
    ...
    /// @param _amount the amount of ETH the user deposited
    function _setWhitelistedAddressDeposited(address _user, uint256 _amount)
        internal
    {
        uint256 depositAmt = depositsWhitelist[_user];
        if (depositAmt > 0) {
            depositsWhitelist[_user] = depositsWhitelist[_user].add(_amount);
        } else {
            depositsWhitelist[_user] = _amount;
        }
    }
    ...
    receive() external payable isEligibleSender {
        ...
        if (deposits[beneficiary] == 0 && depositsWhitelist[beneficiary] == 0) {
    ...
    function depositForWhitelistedAddress(
        ...
        require(
            depositableLeftWhitelist(index, beneficiary, merkleProof) > 0,
        ...
        _setWhitelistedAddressDeposited(beneficiary, msg.value);

        // Update total sale participants
        if (depositsWhitelist[beneficiary] == 0 && deposits[beneficiary] == 0) {
            totalSaleParticipants = totalSaleParticipants.add(1);
        }
        ...
        weiDepositedWhitelist = weiDeposited.add(msg.value);
        weiDeposited = weiDeposited.add(msg.value);
        ...
        returns (uint256)
    {
        ...
        uint256 userDepo = deposits[beneficiary];

        if (userDepo.add(userDepoWl) == 0) {
            return 0;
        }

        uint256 userClaimableJones = 0;
        // amount of jones tokens allocated to whitelist sale
        uint256 jonesForWl = jonesTokensAllocated
            .mul(weiDepositedWhitelist)
            .div(maxDeposits);
        // amount of jones tokens allocated to public sale
        uint256 jonesForPublic = jonesTokensAllocated
            .mul(maxDeposits.sub(weiDepositedWhitelist))
            .div(maxDeposits);
        // wei deposited during the public sale
        ...
        if (userDepo > 0) {
            userClaimableJones = userClaimableJones.add(
                jonesForPublic.mul(userDepo).div(totalDepoPublic)
            );
        }
    ...
    function claimAmountEth(address beneficiary) public view returns (uint256) {
        ...
        uint256 publicEth = weiDeposited.sub(weiDepositedWhitelist);

        return eccessEth.mul(userDepo).div(publicEth);
    }

    /// View beneficiary's claimable ETH amount
    /// @param beneficiary address to view claimable token amount
    /// @dev claim calculation must be equivalent to the one in claim(address beneficiary)
    function depositableLeftWhitelist(
        uint256 index,
        address beneficiary,
        bytes32[] calldata merkleProof
        ...
        if (isWhitelistedAddressDeposited(beneficiary)) {
            return maxWhitelistDeposit.sub(depositsWhitelist[beneficiary]);
        } else {
            return maxWhitelistDeposit;
        }
    }
