...
//in mainnet connect to uniswapAnhorView(oracle in compound)
    ...
    PausableUpgradeable
{
    ...
    struct User {
        ...
        mapping(address => BorrowSnapshot) borrowedTokens; //borrowed from compound
    ...
    mapping(uint256 => BorrowSnapshot) public borrows;
    ...
    function getUserBorrowedToken(address _user, address borrowedToken)
        public
        view
        returns (uint256)
    {
        User storage user = userInfo[_user];
        BorrowSnapshot storage userBorrow = user.borrowedTokens[borrowedToken];
        Exp memory borrowIndex = Exp({
            mantissa: CToken(borrowedToken).borrowIndex()
        });
        uint256 principalTimesIndex = userBorrow.principal *
            borrowIndex.mantissa;
        return principalTimesIndex / userBorrow.interestIndex;
    }
    ...
    function setUserBorrowedToken(
        ...
        User storage user = userInfo[_user];
        BorrowSnapshot storage userBorrow = user.borrowedTokens[_token];
        ...
        external
    {
        ...
        uint256 exchangerRateStored = ICErc20(_ctoken).exchangeRateCurrent();
        uint256 mantissa = 18 + IERC20Upgradeable(_token).decimals() - 8;
        uint256 usercTokenReceived = (exchangerRateStored / 1**(mantissa)) *
            _numTokenForSupply;
        IERC20Upgradeable(_token).approve(_ctoken, _numTokenForSupply);
    ...
    function chechRedeemAmount(address user, address _cToken)
        public
        view
        returns (uint256)
    {
        uint256 rate = ICErc20(_cToken).exchangeRateStored();
        uint256 amount = getUserReceivedToken(user, _cToken);
        return (rate * amount);
    }
    ...
    function redeemERC20FromCompound(
        ...
        uint256 balanceTokenBefore = IERC20Upgradeable(tokenRec).balanceOf(
            address(this)
        );
        ...
        uint256 balanceTokenAfter = IERC20Upgradeable(tokenRec).balanceOf(
            address(this)
        );
        ...
        returns (uint256)
    {
        ...
        uint256 exhangeRate = ICErc20(_cToken).exchangeRateStored();
        uint256 amountOfToken = (((amountCToken) / 1) *
            (10**18) *
            (exhangeRate / (1 * (10**12))));
    ...
    ////////////////////////////////////////////
    function currentBorrowDuty(address user, address token)
        public
        view
        returns (uint256)
    {
        uint256 amount = calculateAmountOfBorrow(token, user);
        address cToken = tokenForCtoken[token];
        uint256 price = getCTokenprice(cToken);
        return amount * price;
    }
    ...
    ///////////////////////////////////
    function calculateAmountOfBorrow(address borowedToken, address _user)
        public
        view
        returns (uint256)
    {
        Exp memory borrowIndex = Exp({
            mantissa: CToken(borowedToken).borrowIndex()
        });
        uint256 borrowAmount = getUserBorrowedToken(_user, borowedToken);
        address cToken = tokenForCtoken[borowedToken];
        ICErc20(cToken).borrowRatePerBlock();
    }
    ...
    function getCTokenprice(address _cToken) public view returns (uint256) {
...
interface ComptrollerInterfaces {
    ...
    function exitMarket(address[] calldata) external returns (uint256[] memory);
