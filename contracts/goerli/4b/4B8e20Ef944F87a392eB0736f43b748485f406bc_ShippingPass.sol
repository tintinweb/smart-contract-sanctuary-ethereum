...
contract ShippingPass is ERC721, AccessControlEnumerable, DefaultOperatorFilterer, Ownable {
    ...
    uint256 shippingCost1 = 9.5 ether;
    uint256 shippingCost2 = 22 ether;
    uint256 counter;

    mapping(uint256 shippingPassId => Order orderDetails) internal orders;
    ...
    function buyShippingPass(
        ...
        if (bagsBurned <= 5) {
            require(
                msg.value.getConversionRate() >= shippingCost1, "buyShippingPass: SEND MORE ETHEREUM");
        } else if (bagsBurned <= 11 && bagsBurned >= 6) {
            require(
                msg.value.getConversionRate() >= shippingCost2, "buyShippingPass: SEND MORE ETHEREUM");     
        }
        ...
        boringBrewBagsContract.burnBatch(msg.sender, order.tokenIds, order.quantities);
        ...
        onlyOwner
    {
        require(
            ...
            "withdraw: Insufficient balance in contract"
        );
