...
* @notice The address of staking contract
  ...
  /**
   * @notice The array of addresses from which the money when received is NOT distributed to staking
   */
  address[] public eternityAddresses;
 ...
 * @notice Current balance of stakeholders
  ...
  /**
    * @notice The reward percent for staking
   */
  uint32 private stakingRewardPercent;
  ...
  /**
 * @dev Will allow execution only if caller is staking contract
   */
 ...
 * @dev Fired in removeEternityAddress function
  ...
  /**
 * @dev Fired in changeStakingRewardPercent function
   *
   * @param percent percent to change
   */
  event StakingPercentChanged(uint32 percent);

  /**
 * @dev Fired in changeStakingContractAddress function
   *
   * @param stakingAddress changed address of staking contract
   */
  event StakingContractAddressChanged(address stakingAddress);
 ...
 * @dev Automatically distribute funds to shareholders
  ...
  receive() external payable {
    ...
    for (uint32 i = 0; i < eternityAddresses.length; i++) {
      if(msg.sender == eternityAddresses[i]) {
        whitelisted = true;
        distributeToShareholders();
        break;
      }
    ...
    if(!whitelisted) {
      uint256 stakingReward = msg.value * stakingRewardPercent / 100;
      distributeToShareholdersExactAmount(msg.value - stakingReward);
    }
  }
  ...
  /**
 * @dev Add eternity addresses
   *
   * Emits a {EternityAddressAdded} event
   *
   * @param _address one of the eternity infrastructure addresses
   */
  function addEternityAddress(address _address) external onlyOwner {
    eternityAddresses.push(_address);

    emit EternityAddressAdded(_address);
  }

  /**
* @dev Remove eternity address
   *
   * Emits a {EternityAddressRemoved} event
   *
   * @param _address one of the eternity infrastructure addresses
   */
  function removeEternityAddress(address _address) external onlyOwner {
    for (uint16 i = 0; i < eternityAddresses.length; i++) {
      if (eternityAddresses[i] == _address) {
        eternityAddresses[i] = address(0);
      }
    }

    emit EternityAddressRemoved(_address);
  }

  /**
* @dev Change staking reward percent
   *
   * Emits a {StakingPercentChanged} event
   *
   * @param _percent percent to change
   */
  function changeStakingRewardPercent(uint32 _percent) external onlyOwner {
    require(_percent > 0, "percent should be greater than 0");
    require(_percent <= 100, "percent should be in range [0,100]");

    stakingRewardPercent = _percent;

    emit StakingPercentChanged(_percent);
  }

  /**
* @dev get stakeholder reward by address
   *
   * @param _address address of stakeholder
   */
  function getStakeHolderReward(address _address) external view returns(uint256) {
    require(_address != address(0), "address could not be empty");

    return stakeHoldersReward[_address];
  }
