...
contract WBTCCIMarket_goerli is CIMarketStableDef_goerli {
    function TOKEN_ADDRESS() public pure override returns (address) {
        return address(0x74A159Cf8f5CB79D473c12b9f4c24691971EB1Cf);
    }
...
abstract contract CIMarketStableDef_goerli is CIMarket {
    ...
    function STABLE_ADDRESS() public pure override returns (address) {
        return address(0x65dC8EEcdF728ecc5C3323e275cCCD285BEECA40);
    }
    ...
    function BOND_ADDRESS() public view virtual override returns (address) {
        return address(0x533db82CE981D1fCabd7dE3Dc01696fd45Ba2ba0);
    }
...
abstract contract CIMarket is NFTBondMarketBase {
    ...
    function close(address account, uint id) public override updateTakerIndex() {
        super.close(account,id);           
    }
    ...
    function cancel(address account, uint id) public override updateTakerIndex() {
        super.cancel(account,id);           
    }
...
abstract contract NFTBondMarketBase is ManagedMarketBase {    
    ...
    function protect(
        ...
        IPositionNFT(config.getNFTTaker(TOKEN_ADDRESS())).safeMint(account, id);
        return id;
    }

    /// @notice close position (override with NFT burn)
    function close(address account, uint id) public virtual override {
        // Burn position NFT token
        IPositionNFT(config.getNFTTaker(TOKEN_ADDRESS())).safeBurn(id);
        // close position
        super.close(account, id);
    }
    ...
    function cancel(address account, uint id) public virtual override {
        // Burn position NFT token
        IPositionNFT(config.getNFTTaker(TOKEN_ADDRESS())).safeBurn(id);
        // close position
        super.cancel(account, id);
    ...
    function deposit(
        ...
        IPositionNFT(config.getNFTMaker(TOKEN_ADDRESS())).safeMint( account, id);
    ...
    function withdraw(address account, uint id) public virtual override returns (uint amount) {
        // burn position NFT token
        IPositionNFT(config.getNFTMaker(TOKEN_ADDRESS())).safeBurn(id);
...
abstract contract ManagedMarketBase is TransferableMarketBase, IMarketManagement {
    ...
    function setRebalancer(
        ...
        TOKEN().approve(_rebalancer, 0);
        TOKEN().approve(_rebalancer, type(uint).max);
    }
    ...
    function setTreasury(address _treasury) external virtual override onlyGovernance {
        ...
        TOKEN().approve(_treasury, 0);
        TOKEN().approve(_treasury, type(uint).max);
    }
...
interface IMarket is IMarketStorage, IRebalanceable {
    ...
    function cancel(address account, uint id) external;    
    ...
    IMarket
{
    ...
    modifier onlyPermittedAdapter() {
        require(hasRole(ADAPTER_ROLE, msg.sender), "!permitted-adapter");
        _;
    }
    ...
    // tokens    
    function TOKEN_ADDRESS() public pure virtual returns (address);
    function TOKEN() public pure override returns (IERC20) { return IERC20(TOKEN_ADDRESS()); }
    ...
    function protect(
        ...
        bool autorenew,
        uint bumpAmount
    ) public virtual override onlyPermittedAdapter() returns (uint id) {
    ...
    function close(address account, uint id) public virtual override takerTermEnds(id) {
        TakerPosition storage pos = allTakerPositions[id];
        require(account == pos.owner, "!owner");
        ...
        emit Closed(address(this), account, id, amountToReturn, posPremium);
    }

    /// @notice claim stable coins for opened position
    function claim(address account, uint id) public virtual override takerTermEnds(id) onlyPermittedAdapter() returns (uint claimSizeInStable) {
        TakerPosition storage pos = allTakerPositions[id];
        require(account == pos.owner, "!owner");
    ...
    function cancel(address account, uint id) public virtual override {
        ...
        console.log("_premium: %s ", posPremium );
        uint amountToReturn = pos.assetAmount - posPremium;
        TOKEN().safeTransfer(pos.owner, amountToReturn);

        delete allTakerPositions[id];

        emit Canceled(address(this), account, id, amountToReturn, posPremium);
    }
    ...
    function withdraw(address account, uint id) public virtual override makerTermEnds(id) returns (uint amount) {
        ...
        emit Withdrawn(address(this), account, id, pos.stableAmount, _yield);
    ...
    function onAfterRebalance(
        ...
        require(msg.sender == address(rebalancer), "!rebalancer");
...
abstract contract BACUpgradable is AccessControlUpgradeable {
    ...
    function __BACUpgradable__init(address _gac) internal onlyInitializing {
