...
contract VF_NFT is ERC721EnumerableUpgradeable, // metadata + enumarable
                   ERC721RoyaltyUpgradeable,
                   OwnableUpgradeable, AccessControlUpgradeable                
    {
    uint32 public hardCap;
    ...
    function initialize(string calldata name, string calldata symbol, uint32 _hardCap) initializer public {
        initialize_unchained(name, symbol, _hardCap);
    }
     
    function initialize_unchained(string calldata name, string calldata symbol, uint32 _hardCap) initializer public {
        hardCap = _hardCap;
    ...
    */
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721EnumerableUpgradeable, ERC721RoyaltyUpgradeable, AccessControlUpgradeable) returns (bool) {
        return super.supportsInterface(interfaceId);
    }
}


// File contracts/VF_NFT_Factory.sol


// Vertifilms NFT Factory
...
contract VF_NFT_Factory is Initializable, UUPSUpgradeable,
                           OwnableUpgradeable {
    address public implementation;// ERC721 
    address[] public collections; // clones of the implementation

    event collectionCreated(address collectionAddress, string symbol, uint32 hardCap);
    
    function initialize(address implementationAddress) initializer public {
        initialize_unchained(implementationAddress);
     }
     
     function initialize_unchained(address implementationAddress) initializer public {
    ...
    {
        address newCollection = ClonesUpgradeable.clone(implementation);
        VF_NFT(newCollection).initialize(name, symbol, hardCap);
        collections.push(newCollection);        
        emit collectionCreated(newCollection, symbol, hardCap);
