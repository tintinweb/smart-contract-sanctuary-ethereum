...
contract WallStBullsOptionsMarket is ERC721Holder, ChainlinkVRF {
    ...
    function initializeTokenState(uint256 _tokenId, address _owner, State _state, uint32 _order) internal onlyOwner {
        states[_tokenIdToIndex(_tokenId)] = TokenState(_owner, _state, _order);
    }

    // TODO only owner
    function getTokenState(uint256 _tokenId) external view returns (TokenState memory) {
    ...
    function _setTokenState(uint256 _tokenId, address _owner, State _state) internal {
        ...
        states[_tokenIdToIndex(_tokenId)] = TokenState(_owner, _state, token.order + 1);
        emit TokenStateChange(_tokenId, _state, token.order + 1);
        stateCount += 1;
    }
    ...
    function option(uint256 _tokenId) external returns (uint256 requestId) {
        ...
        _setRequestStatus(requestId, msg.sender, _tokenId, true);
        return requestId;
    }

    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        RequestStatus memory request = _getRequestStatus(_requestId);
...
abstract contract ChainlinkVRF is VRFConsumerBaseV2, ConfirmedOwner {
    uint32 public constant DEFAULT_CALLBACK_GAS_LIMIT = 500000;
    uint16 public constant DEFAULT_REQUEST_CONFIRMATIONS = 3;
    uint32 public constant DEFAULT_NUM_ROLLS = 1;
    ...
    event RequestFulfilled(uint256 requestId, uint256 roll);
    ...
    function _getRequestStatus(uint256 _requestId) internal view returns (RequestStatus memory) {
