...
contract OCASnake is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    modifier mustBeOpen(uint256 amount) {
        // if (arcadeIsOpen && balanceOf(msg.sender) == 0) _;
    ...
    function checkCoinInflation(uint256 amount) private view {
        ...
        for (uint256 i = 0; i < amount; i++) {
            if (midnightReleaseIsOn && balanceOf(msg.sender) == 0 && i == 0)
    ...
    function isNeo() private view returns (bool) {
        bool Neo = arcadeDudes[msg.sender].itemType == 1 ||
            msg.sender == arcadeOwner;
        return Neo;
    }

    function midnightRelease(
        uint256 date,
        uint8 _v,
        bytes32 _r,
        bytes32 _s
    ) private view onlyValidAccess(date, "whitelist", _v, _r, _s) {
        require(
            midnightReleaseIsOn,
            "You might have shown up a tad too early."
        );
    }

    function removeLife(uint256 amount) private view {
        require(
    ...
    function insertQuarter(
        ...
        bytes32 _r,
        bytes32 _s
    ) public payable callerIsSender nonReentrant mustBeOpen(amount) {
        if (midnightReleaseIsOn) {
            midnightRelease(date, _v, _r, _s);
        } else
            require(
                !midnightReleaseIsOn && arcadeIsOpen,
                "You missed the midnight party. Come back tomorrow."
            );

        bool Neo = isNeo();
        checkCoinInflation(amount);
        if (!Neo) removeLife(amount);
