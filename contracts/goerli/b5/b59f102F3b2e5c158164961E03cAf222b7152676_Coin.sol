...
contract Coin is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply, ReentrancyGuard {
  ...
  function airdrop(
    ...
    if (processedChunksForAirdrop[chunkNum] || balanceOf(receivers[0], COIN) > 0) revert ChunkAlreadyProcessed();
  ...
  function mintFromBurn(uint256 _amount, address _caller) external {
    require(msg.sender == Trunk1Address, "Only can be called from trunk1 contract");
  ...
  function burnExternal(uint256 _amount, address _caller) external {
    require(tx.origin == _caller, "Cannot burn tokens that do not belong to you");
