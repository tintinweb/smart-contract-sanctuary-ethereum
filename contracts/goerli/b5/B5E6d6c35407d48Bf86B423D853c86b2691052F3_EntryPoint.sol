...
contract EntryPoint {
    ...
    function handleOps(UserOperation[] calldata userOps) external returns(address destination, uint amount, bool success){
        ...
        for (uint i = 0; i < userOps.length; i++) {
            ...
            try IAAWallet(sender).executionFromEntryPoint(callData) returns(address _destination, uint _amount, bool _success) {
...
interface IAAWallet {
    ...
    function executionFromEntryPoint(bytes memory callData) external returns(address destination, uint amount, bool success);
