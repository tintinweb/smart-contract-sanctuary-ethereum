...
contract Traits is Ownable, ITraits {
  ...
  function addZenTraits(uint256 traitTypeIdx, uint256[] calldata traitIds, Trait[] calldata traits) external onlyOwner {
    ...
    require(bytes(zenTraitTypes[traitTypeIdx]).length > 0, "Trait not exists");
  ...
  function addRatTraits(uint256 traitTypeIdx, uint256[] calldata traitIds, Trait[] calldata traits) external onlyOwner {
    require(traitIds.length == traits.length, "Mismatched inputs");
    require(bytes(ratTraitTypes[traitTypeIdx]).length > 0, "Trait not exists");
    for (uint i = 0; i < traits.length; i++) {
  ...
  function addRarities(uint256 rarityIdx, uint256[] calldata _rarities, uint256[] calldata _aliases) external onlyOwner {
    ...
    require(rarityIdx < zenTraitTypes.length + ratTraitTypes.length, "Trait not exists");
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      _isZen ? 'Zen #' : 'Rat #',
      tokenId.toString(),
      '", "description": "OUR CITY NEVER SLEEPS. NOR DO YOU! EARN $CITY COINS, PAY TAXES, BEWARE OF RATS.",',
      '"image": "',
      baseURI,
      tokenId.toString(),
      '",',
