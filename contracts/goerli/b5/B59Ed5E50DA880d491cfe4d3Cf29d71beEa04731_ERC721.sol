...
contract ERC721 is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    uint256 public pMintStartTime = 1673573637;
    uint256 public pMintEndTime = 1673660047;
    ...
    event UpdateElement(address);
    event UpdateNswapMintlimit(uint256);
    ...
    // tofu
    uint256 public mintFromTofuCount = 0;
    uint256 public tofuMintLimit = 400;

    function updateTofuMintLimit(uint256 _limit) external onlyOwner {
        tofuMintLimit = _limit;
    }

    function mintFromTofu(uint256 _quantity) external payable nonReentrant {
        require(saleIsActive, "Not allowed to mint");
        require(_quantity > 0, "Wrong amount of minting");
        // verify start/end time
        require(block.timestamp > pMintStartTime, "Wrong time for public mint");
        ...
        // verify mint max count
        require(totalSupply() + _quantity <= maxSupply);
        require(_quantity + mintFromTofuCount <= tofuMintLimit, "sold out");
        publicTotalMinted += _quantity;
        mintFromTofuCount += _quantity;
        _safeMint(msg.sender, _quantity);
    }
