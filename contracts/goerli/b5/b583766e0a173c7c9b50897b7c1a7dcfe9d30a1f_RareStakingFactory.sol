...
interface IRareStakingFactory {
  ...
  /// @notice Retrieves and returns the staking address associated with {_user} or deploys a staking contract.
  /// @param _user Address of the user to retrieve/deploy staking contract for.
  ...
  /// @notice Set the $RARE ERC20 contract address field to be used.
  /// @param _rare Address of $RARE ERC20 contract.
  function setRare(address _rare) external;
...
contract RareStakingFactory is IRareStakingFactory, OwnableUpgradeable, UUPSUpgradeable {
  /*//////////////////////////////////////////////////////////////////////////
  ...
  /// @inheritdoc IRareStakingFactory
  ...
  /// @inheritdoc IRareStakingFactory
  /// @dev Requires the caller to be the owner of the contract.
  function setRare(address _rare) external onlyOwner {
    rare = _rare;
  }
  ...
  function deployStaking(address _user) public returns (address) {
    address newStaking = ClonesUpgradeable.clone(rareStakingTemplate);

    IRareStake(newStaking).initialize(rare, _user, address(stakingRegistry));

    stakingRegistry.setStakingAddress(_user, newStaking);

    bytes32 statSetterRole = stakingRegistry.STAKING_STAT_SETTER_ROLE();

    IAccessControlUpgradeable(address(stakingRegistry)).grantRole(statSetterRole, newStaking);

    emit IRareStakingFactory.StakingContractCreated(msg.sender, _user, newStaking);
