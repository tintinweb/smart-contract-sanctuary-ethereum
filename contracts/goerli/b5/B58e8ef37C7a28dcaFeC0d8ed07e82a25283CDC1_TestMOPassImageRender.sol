...
library HexGridsMath {
    ...
    function tokenIDRingNum(uint256 tokenId) public pure returns (uint256 n) {
        // tokenId = 3 * n * n + 3 * n + 1;
        n = (Math.sqrt(9 + 12 * (tokenId - 1)) - 3) / (6);
        if ((3 * n * n + 3 * n + 1) == tokenId) {
            return n;
        } else {
            return n + 1;
        }
    ...
    function tokenIDRingPos(uint256 tokenId) public pure returns (uint256) {
        uint256 ringNum = tokenIDRingNum(tokenId) - 1;
        return tokenId - (3 * ringNum * ringNum + 3 * ringNum + 1);
    }

    function tokenIDRingStartCenterPoint(uint256 tokenIDRingNum_) public pure returns (Block memory) {
        int256 tokenIDRingNum__ = int256(tokenIDRingNum_);
        return Block(-tokenIDRingNum__ * 5, tokenIDRingNum__ * 11, -tokenIDRingNum__ * 6);
    }

    function tokenIDCenterPoint(uint256 tokenId) public pure returns (Block memory block_) {
        if (tokenId == 1) {
            return block_;
        }

        uint256 tokenIDRingNum_ = tokenIDRingNum(tokenId);
        int256 tokenIDRingNum__ = int256(tokenIDRingNum_);
        Block memory startblock = tokenIDRingStartCenterPoint(tokenIDRingNum_);
        uint256 tokenIDRingPos_ = tokenIDRingPos(tokenId);
        int256 tokenIDRingPos__ = int256(tokenIDRingPos_) - 1;

        uint256 side = Math.ceilDiv(tokenIDRingPos_, tokenIDRingNum_);
        int256 sidepos = 0;
        if (tokenIDRingNum__ > 1) {
            sidepos = tokenIDRingPos__ % tokenIDRingNum__;
        }
    ...
    function blockIndex(Block memory block_, uint256 tokenId) public pure returns (int256 blockIndex_) {
        Block memory centerPointBlock = tokenIDCenterPoint(tokenId);
        ...
        svg = string(
            bytes.concat(
                ...
                abi.encodePacked(
                    ...
                    '<symbol id="Lv11" viewBox="0 0 20.5 20.5"><circle class="c4" cx="10.25" cy="10.25" r="10" /><circle cx="10.25" cy="10.25" r="4" />',
                    '<animate attributeName="fill" values="#fff;#000;#fff" dur="3.85s" begin="0s" repeatCount="indefinite"/></symbol>',
                    '<symbol id="Lv12" viewBox="0 0 21.6349 18.7503"><polygon points="10.9236 9.3759 7.6204 14.7759 14.0204 14.7759 10.9236 9.3759" /><polygon class="c4" points="10.9588 .5003 .4357 18.5003 21.205 18.5003 10.9588 .5003" />',
                    '<animate attributeName="fill" values="#fff;#000;#fff" dur="3.85s" begin="0s" repeatCount="indefinite"/></symbol>'
                ),
                abi.encodePacked(
...
import "../libs/HexGridsMath.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

contract TestMOPassImageRender {
    using Strings for uint256;

    address public MOPNPass;

    constructor(address MOPNPass_) {
        MOPNPass = MOPNPass_;
    }

    function getImage(uint256 id) public view returns (string memory) {
        HexGridsMath.Block memory _block = HexGridsMath.tokenIDCenterPoint(id);
        uint256 ringNum = HexGridsMath.tokenIDRingNum(id);
        ...
        string memory background = NFTSVG.generateBackground(id, coordinate);

        bytes memory blockLevelsBytes = HexGridsMath.buildBlockLevels(keccak256(abi.encodePacked(MOPNPass, id)));
    ...
    function getAnimationUrl(uint256 id) public view returns (string memory) {
        return getImage(id);
    }
