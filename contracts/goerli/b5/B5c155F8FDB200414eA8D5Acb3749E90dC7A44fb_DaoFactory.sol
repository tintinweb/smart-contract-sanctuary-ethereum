...
contract DaoFactory is Pausable {
    struct Dao { string project_title; string project_description; string project_banner; string project_logo; string name; string symbol; address owner_of; uint256 amount; uint256 limit; uint256 price; address token_address; }
    ...
    */
    event daoCreated(string project_title, string project_banner, string project_logo, string name, string symbol, address owner_of, uint256 presale, uint256 limit, uint256 price, address token_address);

    /**
    * @dev function which creates dao with params.
    * @param metadata includes name of project, project description and project banner and project logo
    ...
    */
    function createDao(Metadata memory metadata, string memory name, string memory symbol, uint256 presale, uint256 limit, uint256 price) public notPaused {
        DaoToken token = new DaoToken(name, symbol, msg.sender, presale, limit, price);
        address token_address = address(token);
        _dao_by_contract[token_address] = Dao(metadata.project_title, metadata.project_description, metadata.project_banner, metadata.project_logo, name, symbol, msg.sender, presale, limit, price, token_address);
        _daos_by_owner[msg.sender].push(token_address);
        _owner_by_dao[token_address] = msg.sender;
        _is_activated[token_address] = true;
        emit daoCreated(metadata.project_title, metadata.project_banner, metadata.project_logo, name, symbol, msg.sender, presale, limit, price, token_address);
    }
    ...
    */
    function addDao(Metadata memory metadata, address token_address) public notPaused {
        ...
        _dao_by_contract[token_address] = Dao(metadata.project_title, metadata.project_description, metadata.project_banner, metadata.project_logo, token.name(), token.symbol(), msg.sender, presale, limit, price, token_address);
        _daos_by_owner[msg.sender].push(token_address);
        _owner_by_dao[token_address] = msg.sender;
        _is_activated[token_address] = true;
        emit daoCreated(metadata.project_title, metadata.project_banner, metadata.project_logo, token.name(), token.symbol(), msg.sender, presale, limit, price, token_address);
    }
