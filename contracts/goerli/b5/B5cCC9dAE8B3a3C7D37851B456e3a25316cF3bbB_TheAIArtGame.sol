...
contract TheAIArtGame is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    function enableWordGeneration() public callerIsNeo {
        wordGeneration = true;
    }

    function generateSeed(uint256 tokenId) private view returns (uint256) {
        uint256 _seed = uint256(
            keccak256(abi.encodePacked(seed, tokenId, uint256(0)))
        );
        return _seed;
    }
    ...
    function battleArena(uint256[5] memory gladiators)
        public
        view
        returns (uint256[5] memory)
    {
        //assigned numbers 1 to 5 to the gladiators randomly.
        uint256[5] memory randomNumbers;
        for (uint256 i = 0; i < 5; i++) {
            randomNumbers[i] = generateNumber(generateSeed(gladiators[i]), 5);
        }
    ...
    {
        string memory json = Base64.encode(abi.encode(tokenId));
