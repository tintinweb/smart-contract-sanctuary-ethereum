    ...
    SilksERC1155Burnable
{
    ...
    internal
    {
        ...
        _partnerships[horseId] = Partnership(addresses, maxPartnershipShares, true);
    ...
    override
    {
        for (uint256 i = 0; i < ids.length; i++){
                ...
                if (from != address(0)){
                    ...
                    if (!addressExists){
                        if (to != owner){
                            // Extend the array.
                            uint256 partnerAddressesLength = partnerAddresses.length;
                            address[] memory newPartnerAddresses = new address[](partnerAddressesLength + 1);
                            for (uint256 j = 0; j < partnerAddressesLength; j++){
            ...
            require(totalSupply <= _partnerships[id].maxPartnershipShares, "MAX-NUM-SHARES-EXCEEDED");
    ...
    {
        _mintPartnershipTokenType(msg.sender, msg.sender, horseId, maxPartnershipShares, "");
    }
    ...
    onlyOwner
    {
        _mintPartnershipTokenType(msg.sender, account, horseId, maxPartnershipShares, "");
    }
    ...
    )
    {
        ...
        address ownerAddress = ERC721(_indexContract.getAddress("Horse")).ownerOf(horseId);
    ...
    function setContractPaused(bool _paused)
    external
    onlyOwner
    {
        if (_paused){
