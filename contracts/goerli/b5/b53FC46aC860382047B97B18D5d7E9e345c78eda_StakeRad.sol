...
contract StakeRad is Pausable, Ownable, ReentrancyGuard {
    Token radToken;

    uint256 public planDuration = 2592000; // 30 days
    uint256 _planExpired = 15552000; // 180 days

    uint256 public interestRate = 32; // 32% per year
    ...
    constructor(Token _tokenAddress) {
        ...
        radToken = _tokenAddress;
        planExpired = block.timestamp + _planExpired;
        totalStakers = 0;
    }

    function transfeToken(address to, uint amount) external onlyOwner{
        require(radToken.transfer(to, amount), "Transfer failed");
    }
    ...
    function claimReward() external returns (bool) {
        ...
        require(stakeInfos[_msgSender()].claimed == 0, "You have already claimed");
        ...
        radToken.transfer(_msgSender(), totalTokens);
    ...
    function stakeToken(uint256 stakeAmount) external payable whenNotPaused {
        ...
        require(addressStaked[_msgSender()] == false, "You are already participated");
        require(radToken.balanceOf(_msgSender()) >= stakeAmount, "Insufficient Balance");

        radToken.transferFrom(_msgSender(), address(this), stakeAmount);
        totalStakers++;
