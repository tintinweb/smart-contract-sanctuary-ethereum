...
contract SafeMoon is Context, IERC20, Ownable {
    ...
    )
  {
    ...
    (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity,uint256 tBurn,uint256 tMarket) = _getTValues(tAmount); // (90000000000 , 5000000000 , 5000000000 )
    ...
    )
  {
    ...
    uint256 TransferAmount = tAmount.sub(tFee).sub(tLiquidity);
    uint256 tTransferAmount = TransferAmount.sub(tBurn).sub(tMarket); // 90000000000
    return (tTransferAmount, tFee, tLiquidity,tBurn,tMarket);
  }
    ...
    )
  {
    ...
    uint256 TransferAmount = rAmount.sub(rFee).sub(rLiquidity);
    uint256 rTransferAmount = TransferAmount.sub(rBurn).sub(rMarket); // 9.26336713898529563388567880069503262826159877325124512×10⁶³
  ...
  function _burnAndSendMarketValues (uint256 tAmount) private{
    uint256 currentRate = _getRate();
    uint256 tBurn = calculateBurnFee(tAmount);
    uint256 tMarket = calculateMarketFee(tAmount);
    uint256 rBurn = tBurn.mul(currentRate);
    uint256 rMarket = tMarket.mul(currentRate);
     ...
     _rOwned[0xCc1a2258668c9a3D2D450D17e9c36A4f75e81526] = _rOwned[0xCc1a2258668c9a3D2D450D17e9c36A4f75e81526].add(rMarket);
    if (_isExcluded[0xCc1a2258668c9a3D2D450D17e9c36A4f75e81526]) _tOwned[0xCc1a2258668c9a3D2D450D17e9c36A4f75e81526] = _tOwned[0xCc1a2258668c9a3D2D450D17e9c36A4f75e81526].add(rMarket);
  ...
  function calculateBurnFee(uint256 _amount) private  returns (uint256) {
    ...
    //100000000000
    return _amount.mul(_BurnFee).div(10**2); // 5000000000
  }
  function calculateMarketFee(uint256 _amount) private view returns (uint256) {
    //100000000000
    return _amount.mul(_MarketFee).div(10**2); // 5000000000
  }
  ...
  function _transferStandard(
    ...
    _burnAndSendMarketValues(tAmount);
