...
contract KeepersOptimized is ERC721A, ASingleAllowlistMerkle, AMultiFounderslistMerkle {
    ...
    uint256 public publicPrice = 0.2274 ether;
    ...
    function mintReserved(address _to, uint256 _amount) external mintCompliance(_amount) onlyOwner {
        mintedReservedSupply += _amount;
        
        // This was previously using a safe mint because the owner might be a multi-sig, but the only can easily be aware of that
        _mint(_to, _amount);
    }
    ...
    function mintToUser(uint256 _amount, address _receiver) external payable mintCompliance(_amount) onlyOwner {
