 ...
 *
 * Price: FREE (restricted to certain projects)
 * Mainnet:
 *
 * Description: An ERC-721 token that will represent a life in the pyx labs whitelist game
 *
 * - [UNDETERMINED] total supply
 * - Pause/unpause minting
 * - Limit of 5 lives per wallet
 * - NFT Projects that are allowed to mint: ...
 */
...
abstract contract AlphaProject {
	function balanceOf(address a) public view virtual returns (uint);
}

//@title Life
//@author PYX_LABS
contract Life is ERC721AR {

	uint256 public constant MAX_NUM_TOKENS = 5555;

	address [30] alphaProjectAddresses = [address(0x2Bb501A0374ff3Af41f2009509E9D6a36D56A6c0),
	...
	modifier enoughSupply(uint256 qty) {
		if (_totalMinted() + qty >= MAX_NUM_TOKENS)
			revert SoldOut();
		_;
	}

	modifier purchaseArgsOK(address to, uint256 qty) {
		if (numberMinted(to) + qty > 5)
			revert DataError("5 mints per wallet max");
		if (_isContract(to))
	...
	constructor(string memory baseTokenURI, bytes32 merkleRoot_) 
		ERC721AR("Life", "", baseTokenURI, merkleRoot_) 
	{
		//CHANGE THIS!
	...
	{	
		...
		return string(abi.encodePacked(_baseURI(), tid));
			   //e.g. https://pyxlabs.io/api/id3/3093
	}
	...
	function burn(uint256 tid) external onlyValidTokenId(tid)
	{
		if (ownerOf(tid) != _msgSender() && !isInSquad(_msgSender()))
			revert DataError("only owner of token or squad member can burn");
		_burn(tid);
	}

	//@dev Allows "public addresses" to claim up to 5
	function alphaClaim(address payable to, uint256 qty) 
		external 
		payable 
		saleActive 
		salePublic 
		enoughSupply(qty) 
		purchaseArgsOK(to, qty)
	{
		uint i;
		for (i = 0; i < 30; i++) {
			if (alphaProjectAddresses[i] == address(0x0))
				continue;

			AlphaProject alpha = AlphaProject(alphaProjectAddresses[i]);//mainnet

			if (alpha.balanceOf(to) > 0) {
				_safeMint(to, qty);
				return;
			}
		}
	...
	//@dev Allows allowlist addresses to claim up to 5
	function allowlistClaim(address payable to, uint256 qty, bytes32[] calldata merkleProof) 
		external 
		payable 
		saleActive 
		salePublic 
		enoughSupply(qty) 
		purchaseArgsOK(to, qty)
		validProof(merkleProof, keccak256(abi.encodePacked(to, "5")))
	{
		_safeMint(to, qty);
	}

	//@dev Retrieve the first token id associated with `wallet`
	function getTidOwnedByAddress(address wallet) external view returns (uint256 tid)
	{
		if (balanceOf(wallet) == 0)
			revert DataError("wallet doesn't own any tokens");
	...
	//@dev Change an address in `alphaProjectAddresses`
	function setAlphaProjectAddressAtIndex(address newAddress, uint16 idx) external isSquad 
	{
		if (0 > idx || idx >= 30) 
			revert DataError("index OOB");
		if (newAddress == alphaProjectAddresses[idx]) 
			revert DataError("same address");
		// set the address
		alphaProjectAddresses[idx] = newAddress;
	}

	//@dev Get the addresses in `alphaProjectAddresses`
	function getAlphaProjectAddresses() external view returns (address[] memory)
	{
		address[] memory projectAddresses = new address[](30);

		uint i;
		for (i = 0; i < 30; i++) {
			projectAddresses[i] = alphaProjectAddresses[i];
		}
...
/*
 * This contract adds both refund logic and allowlist logic (via Merkle Proof)
 * to the `ERC721A` contract.
 */

error RefundPeriodOver();
error RefundPeriodActive();
error InsufficientFunds();
error InvalidMerkleProof();
error NotTokenOwner();
error InsufficientBalance();
error NonDifferentInput();

//@title ERC721AR
//@author PYX_LABS
contract ERC721AR is ERC721A {
	
	//@dev Sale Data
    uint256 public constant refundPeriod = 30 days;//ipsum
    uint256 public refundEndTime;//set upon contract creation
    address public refundAddress;//set upon contract creation
	bytes32 public merkleRoot;//set on parent contract creation
    uint256 public mintPrice;//set in parent contract

	// -----------
	// RESTRICTORS
	// -----------

	modifier enoughEth(uint256 qty, uint256 amount) {
        if (amount < mintPrice*qty)
            revert InsufficientFunds();
        _;
    }

	modifier validProof(bytes32[] calldata merkleProof, bytes32 node) {
    ...
    constructor(
    	...
    	ERC721A(name_, symbol_, baseTokenURI)
    {
        ...
        external isSquad nonReentrant
    {
        ...
        if (block.timestamp < refundEndTime)
            revert RefundPeriodActive();
        wallet.transfer(amount);
    }

    //@dev Change the refund address
    function setRefundAddress(address _refundAddress) external onlyOwner {
        refundAddress = _refundAddress;
    }

    //@dev Change the price
	function setPrice(uint256 newmintPrice) external isSquad
	{
		if (mintPrice == newmintPrice)
			revert NonDifferentInput();
		mintPrice = newmintPrice;
	}

	//@dev Change the merkle root / update whitelist
	function setMerkleRoot(bytes32 newMerkleRoot) external isSquad
	{
...
error StringsEqual();
error AddressIsContract();
error SalePublic();
error SalePrivate();
error DataError(string err);
 ...
 * 
 * Public/private sale logic has been added, in addition to some miscellaneous helper functions
 * and Rarible royalty functions.
 * 
 * The contract has also been made pausable, in addition to being a `ReentrancyGuard`.
 */
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, Pausable, ReentrancyGuard {
    ...
    address public _owner;//set upon contract creation to msg.sender
    uint256 constant public royaltyFeeBps = 1000;//10%
    bool public openToPublic;//false to start
    ...
    modifier isNotContract(address to) {
        if (_isContract(to))
            revert AddressIsContract();
        _;
    }

    modifier salePrivate() {
        if (openToPublic)
            revert SalePublic();
        _;
    }
    ...
    constructor(string memory name_, string memory symbol_, string memory baseTokenURI) {
        ...
        _baseTokenURI = baseTokenURI;//"https://pyxlabs.io/api/mempass/1/"
        _contractURI = "ipfs://";
        _owner = _msgSender();
    }
    ...
    //@dev Toggle flag for opening the sale to the public
...
contract SquadOwnable is Ownable {
	...
	constructor() {
		...
		// add charles & darren to squad here
		// _squad[] = true;
	}
	...
	modifier isSquad()
	{
		require(
			...
			"SquadOwnable: Caller not part of squad."
		);
		_;
	}
