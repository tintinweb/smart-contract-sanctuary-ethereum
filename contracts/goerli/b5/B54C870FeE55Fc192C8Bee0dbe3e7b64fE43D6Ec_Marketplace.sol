...
contract Marketplace is Ownable {
	...
	*/    
    ...
    event ProviderCreated(bytes32 id, string name, string description, string picture, address addr, uint commission);
	...
	*/
    function addProvider(string memory name, string memory description, string memory logo, address addr, uint commission) public onlyOwner {
        ...
        Provider memory p = Provider(id, name, description, logo, payable(addr), commission);

        providers.push(id);
        providerByOwner[addr] = id;
        providerByID[id] = p;

        emit ProviderCreated(p.id, p.name, p.description, p.logo, p.addr, p.commission);
    }
