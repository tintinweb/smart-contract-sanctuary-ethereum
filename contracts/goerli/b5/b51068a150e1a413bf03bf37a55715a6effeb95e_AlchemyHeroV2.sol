 ...
 * @title AlchemyHero
 * @author lukema95
 * @notice AlchemyHero is a NFT game
 */
contract AlchemyHeroV2 is ERC721AUpgradeable, OwnableUpgradeable {  
    using Strings for uint256;

    uint256 public collectionSize;   
    uint256 public amountForDevs;
    uint256 public amountForWhitelist;
    uint256 public maxPerAddressMint; 
    uint256 public maxPerWhitelistAddressMint;
    ...
    RoleAttribute public roleAttribute;
    ...
    mapping(uint256 => StakeConfig) stakeList;
    ...
    modifier isNotPaused() {
        require(!isPaused, "The contract has been suspended");
        _;
    }
    ...
    function initialize(
        ...
        uint256 _maxPerWhitelistAddressMint
    ) 
    initializerERC721A 
    initializer 
    public 
    {
        __ERC721A_init('AlchemyHero', 'AH');
        __Ownable_init();
        collectionSize = _collectionSize;
        amountForDevs = _amountForDevs;
        maxPerAddressMint = _maxPerAddressMint;
        maxPerWhitelistAddressMint = _maxPerWhitelistAddressMint;

        require(_amountForDevs <= _collectionSize, "larger collection size needed");
    ...
    function mint(uint256 quantity, uint256 callerPublicSaleKey, bytes32[] calldata _merkleProof) external payable callerIsUser {
        if(phase == 0) {
            ...
            _whitelistMint(quantity, _merkleProof);
        }else if(phase == 2) {
            _publicSaleMint(quantity, callerPublicSaleKey);
        }else {
            revert("phase error");
        }
    }
    ...
    function stake(uint256 id) external {
        require(phase == 3, "stake has not yet begun"); 
        require(_exists(id), "token id is not existed");
        require(ownerOf(id) == msg.sender, "token owner does not match");
        require(stakeList[id].staker == address(0), "token has staken");
        stakeList[id] = StakeConfig(msg.sender, block.timestamp, 0);

    }

    function _devMint(uint256 quantity) internal onlyOwner {
        require(
    ...
    function _whitelistMint(uint256 quantity, bytes32[] calldata _merkleProof) internal {
    ...
    function _publicSaleMint(uint256 quantity, uint256 callerPublicSaleKey) internal {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (!_exists(tokenId)) revert("token id is not existed");
        if (!isRevealed) {
            return unrevealURI;
        }
