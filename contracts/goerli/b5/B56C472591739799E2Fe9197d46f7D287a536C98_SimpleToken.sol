...
contract SimpleToken is ERC20 {
    ...
    function mint(address account, uint256 amount) public onlyMinters {
        require(
            msg.sender == owner || minters[msg.sender].allowance >= amount,
            "You don't have allowance to mint this much."
        );
        _mint(account, amount);
        minters[msg.sender].allowance -= amount;
    }

    function addMinter(address minterAddress, uint256 allowance)
        public
        onlyOwner
    {
        require(
            minters[minterAddress].canMint == false,
            "This minter is already exists."
        );
    ...
    function removeMinter(address minterAddress) public onlyOwner {
        require(
            minterAddress != owner,
            "Owner can't be removed from the minters."
        );
        require(
            minters[minterAddress].canMint == true,
            "This minter doesn't exist."
        );
        minters[minterAddress].canMint = false;
        minters[minterAddress].allowance = 0;
    }

    function setMintAllowance(address minterAddress, uint256 allowance)
        public
        onlyOwner
    {
        require(
