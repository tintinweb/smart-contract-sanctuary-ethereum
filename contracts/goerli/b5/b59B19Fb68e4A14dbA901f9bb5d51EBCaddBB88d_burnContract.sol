...
contract burnContract is ERC721AQueryable, Ownable, RevokableDefaultOperatorFilterer, ReentrancyGuard {
        ...
        Burn Requirements:
        200 max
        Burn 1 token for a new token
        Choose token from list of token IDs owned
        Can burn multiple (10 MAX)

        SOTY Claim Requirements
        25 max
        Snapshot of holders x hours before claim 
        Must hold 50 or greater tokens to be eligible for the SOTYclaim
        Max 1 mint per wallet
        No burn of any token
