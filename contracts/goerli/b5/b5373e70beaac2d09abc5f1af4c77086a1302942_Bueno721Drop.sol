...
import "./OperatorFilter/upgradeable/OperatorFiltererUpgradeable.sol";
...
struct SaleState {
    ...
    mapping(uint64 => PhaseSettings) phases;
}
    ...
    OperatorFiltererUpgradeable
{
    ...
    mapping(address => mapping(uint64 => uint256)) private amountMintedForPhase;
    ...
    function initialize(
        ...
        OperatorFiltererUpgradeable.__OperatorFilterer_init(
            operatorFilter,
            operatorFilter == address(0) ? false : true // only subscribe if a filter is provided
        );

        uint64 numPhases = uint64(_phases.length);

        if (numPhases > 10 || withdrawAddresses.length > 10) {
    ...
    /**
     * @dev Mint tokens for an allowlisted phase
     *
     * Note: Calling this function for a phase that doesn't have an allowlist will fail
     */
    function mintPhaseAllowlist(
        ...
        if (
            ...
            amountMinted + quantity > phase.maxPerWallet
        ) {
            revert ExceedMaxPerWallet();
        }

        // there can also be a global maxPerWallet
        if (
            baseSettings.maxPerWallet > 0 &&
            _numberMinted(msg.sender) + quantity > baseSettings.maxPerWallet
        ) {
            revert ExceedMaxPerWallet();
        }

        saleState.phases[phaseIndex].amountMinted += quantity;
        amountMintedForPhase[msg.sender][phaseIndex] += quantity;

        _mint(msg.sender, quantity);
    }

    /**
     * @dev Mint tokens for a particular phase.
     *
     * Note: Calling this function for a phase that has an allowlist will fail
     */
    ...
    /**
     * @dev Mint tokens in the public sale
     */
    function mintPublic(uint64 quantity) external payable {
        ...
        if (
            baseSettings.maxPerTransaction > 0 &&
            quantity > baseSettings.maxPerTransaction
        ) {
            revert ExceedMaxPerTransaction();
        }
        ...
        if (msg.value != quantity * baseSettings.price) {
            revert InvalidPrice();
        }
    ...
    /**
     * @dev Perform a batch airdrop for a particular phase. Minted tokens are pulled
     * from the phase that is specified in the airdropper.
     *
     */
    function airdropForPhase(
        ...
        for (uint256 i = 0; i < numRecipients; ) {
            if (phase.amountMinted + quantities[i] > phase.maxSupply) {
                revert SoldOut();
            }

            saleState.phases[phaseIndex].amountMinted += quantities[i];
    ...
    /**
     * @dev Perform a batch airdrop for the public phase.
     */
    function airdropPublic(
        ...
        for (uint256 i = 0; i < numRecipients; ) {
            if (_totalMinted() + quantities[i] > baseSettings.maxSupply) {
                revert SoldOut();
            }
     ...
     */
    function endPhase(uint64 phaseIndex) public onlyOwner {
    ...
    function endPhases(uint64[] calldata phaseIndices) external onlyOwner {
        ...
        if (phaseIndices.length > saleState.numPhases) {
            revert InvalidPhase();
        }
    ...
    function getMintBalance(address owner) external view returns (uint256) {
        return _numberMinted(owner);
    }

    function getAmountMintedForPhase(uint64 phaseIndex, address owner)
        external
        view
        returns (uint256)
    {
        return amountMintedForPhase[owner][phaseIndex];
    }

    function getAmountMintedForOwner(address owner)
        external
        view
        returns (uint256[] memory)
    {
        ...
        for (uint64 i = 0; i < saleState.numPhases; ) {
            amountMintedPerPhase[i] = amountMintedForPhase[owner][i];
        ...
        amountMintedPerPhase[saleState.numPhases] = _numberMinted(owner);

        return amountMintedPerPhase;
    }

    /**
     * @dev Updates the minting rules for a particular phase
     *
     * Note: supply & amountMinted are not changeable
     */
    ...
    /**
     * @dev Updates the the base minting settings
     * The global maxPerWallet setting applies to all phases
     * Pricing and other fields will apply to the public sale
     * Note: maxSupply is not changeable
     */
    function updateBaseSettings(BaseSettings calldata _baseSettings)
        external
        onlyOwner
    {
        ...
        // ensure that the maxSupply value cannot be set
    ...
    function isPhaseActive(uint64 phaseIndex) public view returns (bool) {
        for (uint256 i = 0; i < saleState.activePhaseIds.length; ) {
