...
using Address for address;
    ...
    IERC20 public gasFactoryAddress;
    IERC20 public loveTokenAddress;
    ERC721 CT;
    ...
    address doxxd      = payable(0x1271Dc0C23099D985A7f3455D99AE35a4Be810c8); //70% Test
    address toronto    = payable(0x032F009A2D3E799D229b4612a5E3EF8fB8072672); //30% Test
    ...
    constructor(address _cham, address _gas, address _love) ERC721("",""){
        ...
        gasPrice = 15000 * (10 ** 18) ;//5000;        // it requires update
    ...
    function requireAll(uint256 qty) private view {
        ...
        require((qty + tokenMinted + reservedTokens + reservedChem + reservedGas + reservedLove ) <= maxSupply,
            "Mint: Not enough avaialability"); 
        require(qty <= maxMintPerTx,"Mint: Mint tokens per transaction exceeded");
        require(( tokensMintedby[ _msgSender() ] + qty ) <= maxPerWallet, "Mint: Mint per wallet exceeded.");
    }
    ...
    function mintByGas(uint256 qty, uint256 flag) external reentryLock {
        requireAll(qty);
        require(reservedGas > 0, "Gas reserved tokens end");
        require(flag == 1 || flag == 2, "Wrong gas token used to mint."); 
        if(flag == 1){
    ...
    function totalSupply() external view returns (uint256) {
        return tokenMinted; //stupid bs for etherscan's call
    }
