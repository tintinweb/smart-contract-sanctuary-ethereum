...
contract AtlendisMining is Ownable {
    ...
    constructor(address _nft, address _pools, bytes32 _poolHash) {
    ...
    function rescueRewards(address to) external onlyOwner updateRewards(msg.sender) {
        (, bool defaulted, bool closed , , , , , , , , , ) = IPoolsController(address(pools)).getPoolState(
            poolHash
        );
        require(defaulted || closed, "Pool is neither closed or defaulted");
        uint256[] memory toRescue = new uint256[](tokens.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            toRescue[i] = ERC20(tokens[i]).balanceOf(address(this)) - rewardTokens[tokens[i]].pendingRewards;
...
contract MiningFactory {
    ...
    constructor(AtlendisToken _token, address _positionManager, address _borrowerPools) {
        token = _token;
    ...
    function deployNewMining(
        ...
        emit MiningCreated(address(deployedMining), amount, poolHash, rate);
    }
}
