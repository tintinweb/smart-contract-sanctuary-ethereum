...
import "./TimerofDragons.sol";

contract StakingOfDragons is ERC721Holder, AccessControl, TimerOfDragons {
          //nftId
    ...
    address internal projectOwner = 0x00059Ff7a2c19aAec09E46bd662561938c3Cf651;
    ...
    constructor(DenOfDragons _denofdragons, EggToken _eggtoken) {
        ...
        _setRoleAdmin(CONTRACT_MANAGER_, PROJECT_OWNER_);

        _setupRole(PROJECT_OWNER_, projectOwner);
    }
    ...
    function stake(uint256 _nftId) public {
        ...
        isStaked[_nftId] = true;                                                                                  
        startTimer(_nftId);  //starts the clock!
    ...
    function unstake(uint256 _nftId) public {
        ...
        uint256 amount = weeksStaked[_nftId] / 7;
        mintTokenInternal(amount); 
        emit Unstake(msg.sender, _nftId);
    }

    function mintTokenInternal(uint256 _amount) internal {
        eggToken.mint(msg.sender, (_amount * (10 ** 18))); 
    }
...
contract TimerOfDragons is AccessControl {
    ...
    //remove if staking contract does not inherit functions
    ...
    mapping(uint256 => uint) public weeksStaked;
    mapping(uint256 => bool) public isStaked;
    mapping(uint256 => bool) public hasBeenStaked;


    //add if staking contract does not inherit functions
    //function setStakingContract (address _stakingAddress) public {
    //    require(hasRole(CONTRACT_MANAGER_, msg.sender), "You are not Contract Manager");
    //    stakingContract = _stakingAddress;
    //}


    function startTimer (uint256 _tokenId) public {
        require(msg.sender == stakingContract || hasRole(CONTRACT_MANAGER_, msg.sender), "You are not Contract Manager");
    ...
    function endTimer (uint256 _tokenId) public {
        require(msg.sender == stakingContract || hasRole(CONTRACT_MANAGER_, msg.sender), "You are not Contract Manger");
    ...
    function math (uint256 _tokenId) internal {
        ...
        weeksStaked[_tokenId] = (stakedTime[_tokenId] / 604800);
    }
    ...
    function resetTokenStats (uint256 _tokenId) public {
        require(msg.sender == stakingContract, "You are not Contract Manager");
        startTime[_tokenId] = 0;
        endTime[_tokenId] = 0;
        stakedTime[_tokenId] = 0;
        weeksStaked[_tokenId] = 0;
        isStaked[_tokenId] = false;
...
contract EggToken is ERC20, ERC20Burnable, Pausable, AccessControl {
    ...
    function setStakingContract(address _stakingContractAddress) public {
        ...
        _grantRole(CONTRACT_MANAGER_, _stakingContractAddress);
    }
