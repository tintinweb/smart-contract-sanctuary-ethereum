...
contract Jiniz is ERC721A, Ownable, Pausable {
    Tickets public tickets;
    string public baseTokenURI;
    uint256 public supply = 10000;

    constructor() ERC721A("Jiniz", "Jiniz") {
        _pause();
    }

    function setTickets(address payable _tickets) external onlyOwner {
        tickets = Tickets(_tickets);
    }

    function mint(uint256[] calldata tokenIds) external payable whenNotPaused {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            require(
                tickets.ownerOf(tokenIds[i]) == msg.sender,
                "NOT_TOKEN_OWNER"
            );
        }

        for (uint256 i = 0; i < tokenIds.length; ++i) {
            tickets.burn(tokenIds[i]);
        }
    ...
    DefaultOperatorFilterer
{
    ...
    function mint(uint256 qty) external payable whenNotPaused {
        ...
        require(_totalMinted() + qty <= maxSupply, "MAX_SUPPLY");
