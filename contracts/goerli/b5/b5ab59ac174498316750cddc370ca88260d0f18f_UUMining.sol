...
contract UUMining is Ownable {
    ...
    address usdtAddr = 0x4952018105C30E07DCC4267A0411941ad025dAB1; 
    ...
    function _mint(uint256 _amt, address _referrer, uint256 _usdtPrice) internal onlyValidUsers {
        ...
        // todo: check 1st n 2nd cannot be caller

        if (userInvestedAmt[firstReferrer] >= _usdtPrice) {
            _callMintingMinerNft(firstReferrer, _amt, (_usdtPrice), 0); // mint to 1st referrer
            referralUUReward[firstReferrer] += _amt * _usdtPrice;
        }
        if (userInvestedAmt[secondReferrer] >= _usdtPrice) {
    ...
    function splitNftInto2(uint _id, uint _uuAmt) external onlyValidUsers {
        ...
        require (_uuAmt < remainUU, "input UU-amount out of range"); // todo: ignore input zero _uuamt
        oldNft.valid = false; 
