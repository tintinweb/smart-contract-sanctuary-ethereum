...
contract ERC20 is IERC20, Whitelist {
     ...
     */
    function _mint(address _to, uint256 _amount)  onlyWhitelisted canMint internal {
...
contract Q008 is ERC20 {
    ...
    string private constant _name = "Cool Token";
    string private constant _symbol = "Kul";
    uint8 private constant _decimals = 18;
    uint256 public constant targetSupply = 700000000e18;
    ...
    uint8 constant internal taxDefault = 15; // 15% tax on transfers
    address private allowedEarthVault; //earth vault
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
        ...
        /** @dev Transfers (using customTransferFrom) */
    function customTransferFrom(address _from, address _to, uint256 _value) public returns (bool) {
        require(_to==allowedEarthVault, "not allowed");
        (uint256 adjustedValue, uint256 taxAmount) = calculateTransferTaxes(_from, _value);
    ...
    function calculateTransferTaxes(address _from, uint256 _value) public view returns (uint256 adjustedValue, uint256 taxAmount){
        ...
        if (!_isExcluded[_from]) {
            uint8 taxPercent = taxDefault; // set to default tax 15%
    ...
    function calculateCustomTransferTaxes(address _from, uint256 _value, string payload) private view returns (uint256 adjustedValue, uint256 taxAmount){
        adjustedValue = _value;
        taxAmount = 0;

        if (!_isExcluded[_from]) {
            uint8 taxPercent = 3; // set to 3%
    ...
    */
    ...
    function seAllowedEarthVault(address AllowedAddress) public onlyOwner() {
        allowedEarthVault = AllowedAddress;
    }
