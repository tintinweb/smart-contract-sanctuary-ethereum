...
contract SwayGovernor is AccessControlUpgradeable, OwnableUpgradeable {
  ...
  // Mapping from AsteroidToken to credit used
  mapping (uint256 => bool) private _asteroidTokenCredits;

  // Mapping from CrewToken to credit used
  mapping (uint256 => bool) private _crewTokenCredits;

  // Mapping from address to credit used
  mapping (address => bool) private _testerCredits;

  // Merkle root for claiming tester phase1 sway credits
  bytes32 public testerCreditMerkleRoot;
  ...
  function claimAssignmentSway(uint256[] calldata asteroidTokens, uint256[] calldata crewTokens) public {
    uint total = crewTokens.length * 650_000;

    // validate and update asteroid token credit(s)
    for (uint i = 0; i < asteroidTokens.length; i++) {
      require(asteroidTokenContract.ownerOf(asteroidTokens[i]) == _msgSender(), "Caller not asteroid token owner");
      require(_asteroidTokenCredits[asteroidTokens[i]] != true, "Credit for asteroid token allready claimed");
      ...
      uint factor = 231808622658467920000; // 4 * PI * 2^64
      uint area = (factor * (radius * radius)) / uint(1_000_000); // area convert m2 to km2
      uint modification = (area / uint(18446744073709552000)) * 6922;
    ...
    for (uint i = 0; i < crewTokens.length; i++) {
      require(crewTokenContract.ownerOf(crewTokens[i]) == _msgSender(), "Caller not crew token owner");
      require(_crewTokenCredits[crewTokens[i]] != true, "Credit for crew token allready claimed");
  ...
  function claimTesterPhase1Sway(bytes32[] calldata proof, uint256 amount) public {
    require(testerCreditMerkleRoot != 0, "Merkle root not set");

    // hash the senders address with the amount
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender(), amount));

    // validate the leaf
    bool isValid = MerkleProof.verify(proof, testerCreditMerkleRoot, leaf);

    require(isValid == true, "Invalid caller, proof or amount");
    require(_testerCredits[_msgSender()] != true, "Credit already claimed");
