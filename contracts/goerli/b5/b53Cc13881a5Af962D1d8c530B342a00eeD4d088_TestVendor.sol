...
contract TestToken is ERC20Capped, ERC20Burnable {
    ...
    function stake(uint256 amount) external {
        ...
        require(
            ...
            "User balance must be greater than specified amount"
        );
    ...
    function unstake(uint256 amount) external {
        ...
        require(
            staked[msg.sender] >= amount,
            "Amount must be >= than the amount staked"
        );
        claim();
        staked[msg.sender] -= amount;
...
contract TestVendor is Ownable {
    ...
    function sell(uint256 tokenAmount) public {
        ...
        require(userTokenBalance >= tokenAmount, "Not enough tokens owned");

        uint256 ethAmount = tokenAmount/tokensPerEth;
        require(address(this).balance >= ethAmount, "Not enough eth at vendor");

        (bool sent) = token.transferFrom(user, address(this), tokenAmount);
        require(sent, "Failed");
