...
error Raffle__notInOpenState();
error Raffle__upkeepNotNeeded(uint256 playersLength, uint256 currentRaffleState, uint256 currentBalance);
error Raffle__cannotAnnounceWinnerInOpenState(uint256 currentRaffleState);
error Raffle__winnerPaymentFailed();

/**
 * @title A Simple Raffle / Lottery Contract
 * @notice This is a contract that creates a decentralized Smart Contract
 * @dev This contract has implementations of VRFCoordinatorV2 and VRF Chainlink keepers
...
contract Raffle is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    // functions -- mutational functions
    ...
    function fulfillRandomWords(uint256 /* requestId */, uint256[] memory randomWords) internal override {
        if (s_raffleState != RaffleState.CALCULATING) {
            revert Raffle__cannotAnnounceWinnerInOpenState(uint256(s_raffleState));
        }
        ...
        address recentWinner = players[winnerIndex];

        s_currentWinner = recentWinner;
    ...
    function getWords() public pure returns (uint32) {
        return WORDS;
    }
