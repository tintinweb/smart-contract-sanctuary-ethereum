...
contract PROTOOLSGSC is Context, IERC20, Ownable {
    ...
    address payable private P_WalletAddress = payable(0xdfD89c2933Bd155031E72bBe83d7c9Bf2C12Ad5a);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 totalERCFee = _totalDistributionShares.sub(_liquidityShare.div(2));
        
        uint256 amountERCLiquidity = amountReceived.mul(_liquidityShare).div(totalERCFee).div(2);
        uint256 amountERCTeam = amountReceived.mul(_teamShare).div(totalERCFee);
        uint256 amountERCMarketing = amountReceived.sub(amountERCLiquidity).sub(amountERCTeam);

        if(amountERCMarketing > 0)
            transferToAddressETH(marketingWalletAddress, amountERCMarketing);

        if(amountERCTeam > 0)
            transferToAddressETH(P_WalletAddress, amountERCTeam);
