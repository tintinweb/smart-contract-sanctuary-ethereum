    ...
    UUPSUpgradeable
{
    ...
    mapping(address => mapping(uint256 => bytes32)) internal jobs;
    mapping(address => uint256) public totalJobsPerUser;  

    event JobExecuted(address indexed jobMaker, uint256 jobId, address executor);
    event GasDeposited(address indexed depositer, uint256 value);
    event GasWithrawn(address indexed withdrawer, uint256 value);
    event JobRegistered(address indexed jobMaker, bytes[] jobInfo, address[] targetAddresses, bytes32 jobHash, uint256 jobId);

    function initialize(address _keeperRegistry)public initializer{
        __Ownable_init();
    ...
    /// @notice Use this function to deposit gas to run jobs.
    function depositGas()external payable{
    gasBalance[msg.sender] += msg.value; 
    emit GasDeposited(msg.sender, msg.value);
    }

    ///@notice Use this function to withdraw gas.
    ///@param to is the address of the account where the gas should be transfered.
    ///@param amount is the amount of gas that needs to be withdrawn.
    function withdrawGas(address to,uint256 amount)external payable{
    gasBalance[msg.sender] -= amount; 
    emit GasWithrawn(msg.sender, amount);
    payable(to).transfer(amount);
    }


    ///@notice Use this function to register jobs.
    ///@param _jobInfo are the calldatas(function signature + encoded parameters) for every job.
    ///@param _targetAddresses are the addresses of the contracts on which the jobs needs to be executed.
    function registerJob(bytes[] calldata _jobInfo,address[] calldata _targetAddresses) external{
        require(_jobInfo.length == _targetAddresses.length,'Wrong Address Count');
        bytes32 jobHash = keccak256(abi.encode(_jobInfo,_targetAddresses));
        uint256 totalJobs = totalJobsPerUser[msg.sender];
        totalJobsPerUser[msg.sender]++;
        jobs[msg.sender][totalJobs] = jobHash;
        emit JobRegistered(msg.sender,_jobInfo,_targetAddresses,jobHash,totalJobs);
    }

    ///@notice Use this function to execute Jobs.
    ///@notice Only keepers can call this function.
    ///@param _jobMaker is the address the of user whose jobs needs to be ran.
    ///@param _jobInfo are the calldatas(function signature + encoded parameters) for every job.
    ///@param _targetAddresses are the addresses of the contracts on which the jobs needs to be executed.
    function executeJob(address _jobMaker,uint256 _jobId,bytes[] memory _jobInfo,address[] memory _targetAddresses)external returns(bool){
        uint256 totalGas = gasleft();
        require(IKeeperRegistry(keeperRegistry).isKeeper(msg.sender),'Access Denied');
        bytes32 jobHashPassed = keccak256(abi.encode(_jobInfo,_targetAddresses));

        //Ensure passed params match user registered job
        require(jobHashPassed == jobs[_jobMaker][_jobId],'Wrong Params');
        jobData memory thisJob = jobData({
            calldatas :_jobInfo,
            targetAddresses:_targetAddresses,
            jobId : _jobId,
            jobMaker : _jobMaker
        });
        bytes memory data = abi.encode(thisJob);

        (bool success, ) = address(this).call(
            abi.encodeWithSignature("_executeJob(bytes)", data)
        );

        // Whether or not action succeeded, pull gas from gasBalance for transaction
        uint256 gasUsed = (totalGas - gasleft())*tx.gasprice;
        gasBalance[_jobMaker] -= gasUsed;
    ...
    /**
    * @dev Handles job execution so that a revert in any one method will revert all methods.
    */
    function _executeJob(bytes memory data) external   {
        require(msg.sender == address(this),'Invalid');
        jobData memory thisJob = abi.decode(data,(jobData));

        // For each job, execute, then require that execution was successful.
        uint256 totalJobs = thisJob.targetAddresses.length;
        for (uint256 i = 0; i != totalJobs ; i++) {
            (bool success, ) = thisJob.targetAddresses[i].call(
            thisJob.calldatas[i]
            );
        ...
        emit JobExecuted(thisJob.jobMaker, thisJob.jobId, msg.sender);
    }
}
