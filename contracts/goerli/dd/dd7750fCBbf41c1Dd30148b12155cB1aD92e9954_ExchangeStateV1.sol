...
contract ExchangeDomainV1 {
    ...
    enum AssetType {NO_USE, ETH, ERC20, ERC1155, ERC721}

    struct Asset {
        address token; // 地址(erc20, erc721, erc1155)
        uint tokenId; // erc721/erc1155.tokenId, erc20(0)
        AssetType assetType; // 当前订单是以什么样的代币去购买，
    }
    ...
    struct Order {
        ...
        uint selling; // 出售的数量, erc721 => 1, erc115 => [1-n]
        /* how much wants owner (in wei, or UINT256_MAX if ERC-721) */
        uint buying; // 购买1个需要的token数量，

        // 出售 1个 erc721
        // buying设置的为 100 * 10*18
        // 单价  100 token

        /* fee for selling */
        uint sellerFee; // 手续费 150 / 10000  1.5%
    }
...
contract ExchangeV1 is Ownable, ExchangeDomainV1 {
    ...
    // 购买的手续费
    // 卖方 sellingFee  买房 buyerFee
    // 100 单价， 买卖手续费1% 购买的时候需要的token数量，101=>(99卖方得到的数量, 2平台)
    function exchange(
        ...
        uint amount,
        address buyer // 购买的地址是a， buyer设置为b，b就是接受nft的地址, 0x00000
    ) payable external {
