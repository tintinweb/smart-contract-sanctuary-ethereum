...
contract Vendor is Ownable {
    ...
    function buyTokens() external payable {
        ...
        uint256 numTokens = tokensPerEth * msg.value;
        yourToken.transfer(msg.sender, numTokens);
        emit BuyTokens(msg.sender, msg.value, numTokens);
    }

    function withdraw() external onlyOwner {
        ethToSender(address(this).balance);
    }

    function sellTokens(uint256 amount) external {
        yourToken.transferFrom(msg.sender, address(this), amount);
        uint256 amountInWei = amount / tokensPerEth;

        ethToSender(amountInWei);

        emit SellTokens(msg.sender, amountInWei, amount);
    }

    function ethToSender(uint256 amount) internal {
...
contract YourToken is ERC20 {
  ...
  constructor() ERC20('Gold', 'GLD') {
    ...
    console.log("Initial drop to ", msg.sender);
  }
}
