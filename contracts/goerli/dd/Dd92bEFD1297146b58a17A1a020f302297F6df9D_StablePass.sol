...
contract StablePass is ERC721A, Ownable, PaymentSplitter {
    ...
    mapping(string => uint256) public alreadyMintedEmail;
    ...
    function mintTo(
        ...
        require(alreadyMintedEmail[email] + amount <= maxPerWallet, "Trying to mint over address max minting amount");
        require(proof.verify(merkleRoot, leaf), "You are not in the list");

        alreadyMintedEmail[email] += amount;

        _mint(msg.sender, amount);
    }

    // public minting Crossmint function
    function mintTo(uint256 amount) external payable canMint(amount) {
        address crossmintEth = 0xdAb1a1854214684acE522439684a145E62505233;
    ...
    function mint(
        ...
        require(alreadyMinted[msg.sender] + amount <= maxPerWallet, "Trying to mint over address max minting amount");
