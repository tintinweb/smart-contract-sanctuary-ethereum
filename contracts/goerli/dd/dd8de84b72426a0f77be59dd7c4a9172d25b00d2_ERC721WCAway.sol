...
contract ERC721WCAway is ERC721 {
    ...
    function updateQualifications(uint256[] memory _teamIds, bool[] memory _qualified) public onlyOwner {
        ...
        for (uint256 i = 0; i < _teamIds.length; i ++) {
            if (qualifiedTeams[_teamIds[i]] == true && _qualified[i] == false) {
                // normal advance of tournament
                numQualifiedWithdraw -= numTradableItems[_teamIds[i]];
                qualifiedTeams[_teamIds[i]] = false;
                emit QualificationUpdated(_teamIds[i], false);
            } else if (qualifiedTeams[_teamIds[i]] == false && _qualified[i] == true) {
                // in case admin messes up
                numQualifiedWithdraw += numTradableItems[_teamIds[i]];
                qualifiedTeams[_teamIds[i]] = true;
                emit QualificationUpdated(_teamIds[i], true);
            }
        }
    }
