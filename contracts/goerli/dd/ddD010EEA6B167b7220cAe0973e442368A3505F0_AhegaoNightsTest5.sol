...
contract AhegaoNightsTest5 is ERC721AQueryable, Ownable, ERC2981, ReentrancyGuard {
    ...
    string public unrevealedMetadataURL = "ipfs://QmWn1JdCQmJSXVvsk4QpFyq1GmZE5r8JR5bescd6R78Le5";
    ...
    // Mapping for revealed status of tokenIds.
    mapping(uint256 => bool) private _tokensRevealedStatus;

    constructor() ERC721A("Ahegao Nights Test5","AHNT5") {}
    ...
    /**
     * @notice Smart contract owner only function.
     * Function sets the tokens revealed status to true or false selectively for tokenIds.
     ...
     * For example, the tokenIds array will be like this [3,5,7].
     */ 
    function setTokensRevealedStatusSelectively(uint256[] calldata tokenIds, bool tokensRevealedStatus) public onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            require(tokenIds[i] > 0 && tokenIds[i] <= maxSupply, "tokenId must be greater than zero and smaller than maxSupply");
    ...
    /**
     * @notice Smart contract owner only function.
     * Function sets the tokens revealed status for all tokenIds in an entered range.
    ...
    function setTokensRevealedStatusInRange(uint256 startTokenId, uint256 stopTokenId, bool tokensRevealedStatus) public onlyOwner {
        require(startTokenId > 0 && startTokenId < stopTokenId, "startTokenId must be greater than zero and smaller than stopTokenId");
    ...
    /// @notice Function queries and returns the revealed status of a tokenId.
    function tokenIdRevealedStatus(uint256 tokenId) public view returns (bool revealedStatus) {
        require(tokenId > 0 && tokenId <= maxSupply, "tokenId must be greater than zero and smaller than maxSupply");
        revealedStatus = _tokensRevealedStatus[tokenId];
        return revealedStatus;
    }

    /// @notice Function queries and returns the tokens revealed status for all tokenIds from startTokenId till stopTokenId.
    ...
    function tokenURI(uint256 tokenId) public view virtual override (ERC721A, IERC721A) returns (string memory) {
        ...
        if (_tokensRevealedStatus[tokenId] == false) {
