    ...
    ICoin
{
    ...
    function reveal(
        ...
        require(!CoinStorage.layout().revealed[catTokenId], "Already revealed");
        CoinStorage.layout().coinEXP[coinTokenId] = exp;
        _burn(user, 0, 1);
        _mint(user, coinTokenId, 1, "");
        CoinStorage.layout().revealed[catTokenId] = true;
    }

    function revealBatch(
        address[] calldata users,
        uint256[] calldata coinTokenIds,
        uint256[] calldata catTokenIds,
        uint256[] calldata exps
    ) external onlyRolesOrOwner(MANAGER_ROLE) {
        uint256 length = users.length;
        require(
            length == coinTokenIds.length &&
                length == exps.length &&
                length == catTokenIds.length,
            "Lengths mismatch"
        );
        for (uint256 i = 0; i < length; ) {
            address user = users[i];
            uint256 tokenId = coinTokenIds[i];
            uint256 catTokenId = catTokenIds[i];
            require(
                !CoinStorage.layout().revealed[catTokenId],
                "Already revealed"
            );
