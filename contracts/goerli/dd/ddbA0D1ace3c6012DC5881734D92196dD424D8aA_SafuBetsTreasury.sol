...
contract SafuBetsTreasury is ReentrancyGuard, APIConsumer {
    ...
    // function getSBETPrice() public view returns (uint256) {
    //     SBETPriceOracle oracle = SBETPriceOracle(oracleAddress);
    //     (, uint256 price, ) = oracle.fulfill();
    //     return price;
    // }
    ...
    function _purchaseDiscountedSBET(uint256 amountBUSD) external nonReentrant {
        ...
        // calculate swapRate
        uint256 outAmountSBET = (amountBUSD / SBETPrice) * 1e18;
        // Calculate the discounted amount of SBET
        uint256 discountedAmount = outAmountSBET +
            (outAmountSBET * (swapDiscount / 100));
        ...
        swapAmounts[msg.sender] += discountedAmount;
        vestingDates[msg.sender] = block.timestamp + vestingPeriod;

        emit BondPurchased(msg.sender, discountedAmount);
    }
    ...
    function withdrawOtherTokens(ERC20 token) external onlyAdmin {
        ...
        if (token == SBET) {
            withdrawableBal = token.balanceOf(address(this)) - TVL_SBET;
        } else {
