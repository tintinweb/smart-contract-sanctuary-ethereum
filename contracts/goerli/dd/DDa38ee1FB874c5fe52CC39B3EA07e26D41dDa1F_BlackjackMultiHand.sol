...
contract BlackjackMultiHand is ReentrancyGuard {
    ...
    */
    ...
    mapping(uint256 => mapping(uint8 => Player)) public PlayerHands;
    mapping(uint256 => mapping(uint8 => Player)) public splitPlayerHands;
    mapping(uint256 => Player) public dealer;
    ...
    event CardsDistribution(uint256 betId, uint256[] initialCards);
    ...
    modifier playerChecks(uint256 betId) {
        ...
        require(dealer[betId].hand.length > 0, "Card distribution for this bet id is not completed");
    ...
    */
    function play(uint8 hands, bool isEth, address tokenAddr, uint256 amount) public payable returns(uint256[] memory) {
        ...
        uint256 dCard1 = cardValues[(cards[numOfCards - 1]) % 13];
        dealer[currentBetId].cards.push(cards[numOfCards - 1]);
        dealer[currentBetId].hand.push(dCard1);


        for(uint8 i = 0; i < hands; i++) {
            Player storage hand = PlayerHands[currentBetId][i];
            hand.betAmount = betAmount;
            hand.cards = [cards[i*2],cards[(i*2) + 1]];
            hand.hand = [cardValues[(cards[i*2] % 13)], cardValues[(cards[(i*2) + 1] % 13)]];
            calculate(hand);
        }
    ...
    function drawCard(uint256 betId, uint8 numberOfCards) public view returns(uint256[] memory) {
     ...
     */
    function split(uint256 betId, uint8 handIndex) public payable playerChecks(betId) {
        Player storage bet = PlayerHands[currentBetId][handIndex];
        Player storage sBet = splitPlayerHands[currentBetId][handIndex];
        require(bet.hand.length == 2, 'can only split with two cards');
        require(bet.hand[0] == bet.hand[1], 'First two cards must be same');
        if(isEther[betId]) {
            require(msg.value == bet.betAmount, 'Must match original bet');
        }else {
            IERC20(tokenAddress[betId]).safeTransferFrom(msg.sender, address(this), bet.betAmount);
        }
        sBet.betAmount = bet.betAmount;
        sBet.hand.push(bet.hand[1]);
        sBet.cards.push(bet.cards[1]);
        bet.hand.pop();
        bet.cards.pop();

        uint256[] memory cards = drawCard(currentBetId, 2);

        bet.cards.push(cards[0]);
        sBet.cards.push(cards[1]);

        uint256 pCard2 = (cards[0]) % 13;
        uint256 spCard2 = (cards[1]) % 13;
        
        bet.hand.push(cardValues[pCard2]);
        sBet.hand.push(cardValues[spCard2]);
     ...
     */
    function doubleDown(uint256 betId, uint8 handIndex, uint256 amount) public payable playerChecks(betId) {
        Player storage bet = PlayerHands[currentBetId][handIndex];
        Game storage game = GameId[betId];
        require(bet.hand.length == 2, 'can only double down with two cards');
        if(isEther[betId]) {
            require(msg.value <= bet.betAmount, 'Amount should not be more than original bet');
            bet.betAmount += msg.value;
        }
        else{
            require(amount <= bet.betAmount && amount >= minBet, 'Amount should not be more than original bet');
        ...
        bet.hand.push(cardValues[pCard3]);
        calculate(bet);
        if(bet.score > 21) {
            game.completed = true;
     ...
     */
    function hit(uint256 betId, uint8 handIndex) public playerChecks(betId){
        uint256[] memory cards = drawCard(currentBetId, 1);
     ...
     */
    function revealDealerCard(uint256 betId) public {
        Player storage bet = dealer[betId];
        uint256[] memory card = drawCard(betId, 1);

        uint8 length = uint8(bet.hand.length + 1);
        uint256 dealerCard2 = card[0] * length;
        uint256 cardValue = dealerCard2;
        while(cardValue > 52) {
            cardValue -= 52;
        }
        bet.cards.push(cardValue);
        uint256 dCard2 = dealerCard2 % 13;
        bet.hand.push(cardValues[dCard2]);
        calculate(bet);
        while(bet.score < 17) {
            dealerHit(betId);
        }
    ...
    function dealerHit(uint256 betId) internal {
        ...
        bet.hand.push(cardValues[newCardForDealer]);
        calculate(bet);
    ...
    function checkWinner(uint256 betId) internal {
        ...
        Player storage bet = PlayerHands[currentBetId][i];
        Player storage sBet = splitPlayerHands[currentBetId][i];
        calculatePayOut(betId, bet);
        uint256 winAmount;
        if(sBet.hand.length > 0){
           calculatePayOut(betId, sBet); 
    ...
    function calculatePayOut(uint256 betId, Player storage player) internal {
        ...
        if(player.hand.length == 2 && player.score == 21 && bet.score != 21) {
            player.winningAmount = (25 * player.betAmount)/10;
        }else if(player.score > bet.score || bet.score > 21) {
            player.winningAmount = 2 * player.betAmount;
        }else if(player.score == bet.score) {
