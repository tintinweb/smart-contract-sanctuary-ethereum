...
contract Hopscotch is IHopscotch, Ownable {
    ...
    function payErc20RequestDirect(uint256 requestId) internal {
        ...
        require(
            IERC20(request.recipientToken).transfer(request.recipient, request.recipientTokenAmount),
            "payErc20RequestDirect/transferFailed"
        );
    }
    ...
    function payRequest(PayRequestInputParams calldata params)
        ...
        payable
        returns (
            uint256 excessNativeTokenBalance,
            uint256 excessErc20InputTokenBalance,
            uint256 excessErc20OutputTokenBalance
        )
    {
        ...
        if (nativeTokenBalance > 0) {
            (bool success,) = msg.sender.call{value: nativeTokenBalance}("");
            require(success, "payRequest/refundNative");
        }

        if (erc20InputTokenBalance > 0) {
            require(IERC20(erc20InputToken).transfer(msg.sender, erc20InputTokenBalance), "payRequest/refundErc20Input");
        }

        if (erc20OutputTokenBalance > 0) {
            require(
                IERC20(request.recipientToken).transfer(msg.sender, erc20OutputTokenBalance),
                "payRequest/refundErc20Output"
            );
        }
