...
contract PartitionsAndMetaStore is ContractPrizeRestricted, IPartitionsAndMetaStore {
    ...
    //coenxion con el smart contract SkyTrilogy
    IERC721Enumerable public immutable nft;


    //El limite de nft que se pueden mintar para la preventa
    uint16 private freemintLimit = 100;
    uint16 private whitelistLimit = 500;
    uint16 private publicsaleLimit = 9400;

    //Lista de basesUri segun si es ganador o no
    mapping(uint64 => string) private revealedBaseURIs;
    //Lista de tokens con premio
    mapping(uint256 => string) private winBaseUriPrizes;
    //Por cada nft. Lista de premio que contiene el nft. Key: codigo de premio. Value: cantidad para ese premio
    ...
    string private downSideBaseUri = "ipfs://someHash/downside";
    ...
    /**
     * @dev Setea a un ganador.
     * @dev Cambia la baseURI por la de un token ganador
     */
    ...
    /**
     * @dev Recoge el premio de un nft ganador
     * @dev Comprueba que la firma del que llama al metodo sea el dueño del token sea proxy o no
     */
    ...
    /**
     * @dev Comprueba que la firma que viene por el llamante sea igual 
     */
    ...
    /**
     * @dev Devuelve toda la data de un token
     */
    ...
    /**
     * @dev Incrementa el contador de particiones. Se inicia una nueva particion.
     */
    function checkClosePartition(uint256 tokenId) public override onlyOwnerOrContract returns(uint256){
    ...
    function getCurrentPartition() public override view onlyOwnerOrContract returns(uint256){
        return getPartitionByTkId(nft.totalSupply());
    }

    // Actualiza el map de basesUri a la Uri que revela los tokens para la particion pasada por parametro
    ...
    /**
     * @dev comprueba si el token ha sido revelado a traves del map de revealedBaseUri, si esta es inicializada para esa particion significa que si ha sido revelado
     * @param tokenId id del token
     * @return bool "true" si ha sido revelado y "false" si no ha sido revelado
     */
    ...
    /** 
     * @dev Devuelve el bloque a la que pertenece el token
     * IMPORTANTE: Falta el freemint
     * 
     */
    ...
    /**
     * @dev Recoge la baseUri del token. Si el token ha sido usado devuelve la baseUri. Si el token no ha sido usado devuelve la winBaseURI
     * @param tokenId id del token
     * @return string devuelve la baseUri actual del tokenId @param tokenId
     */
    ...
    /**
     * @dev Descompone la fimra con el prefijo "Geth"
     */
