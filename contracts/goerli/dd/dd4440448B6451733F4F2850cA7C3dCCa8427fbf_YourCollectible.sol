...
abstract contract StakingContest is IERC721Receiver{
...
contract YourCollectible is ERC721, Ownable {
  ...
  constructor() public ERC721("War Eggs", "EGG") {
  ...
  mapping(uint => Eggstats) public stats;
  mapping (uint => address) public eggToOwner;
  mapping (address => uint) public fightsToOwner;
  uint[] public eggPool;


  // function calculate_aura(uint)

  // function update_talents(uint eggId,uint talentId) {
  //   const warrior_link = "
  //   require(msg.sender == eggToOwner[eggId]);
  //   require(stats[eggId].health >= 1);
  //   if (talentId == 1) {
  //     _setTokenURI(eggId)
  //   }
  // }
  ...
  function attack(uint eggId,uint targetId,uint vrf) public {
    require(msg.sender == eggToOwner[eggId]);
    require(stats[eggId].health >= 1);
    require(stats[targetId].health >= 1);
    fightsToOwner[msg.sender]++;
    eggPool.push(eggId);
    Eggstats storage p1 = stats[eggId];
    Eggstats storage p2 = stats[targetId];
    if (vrf > 50) {
      p1.health += p2.health;
      p2.health -= p1.attack;
    } 
  ...
  function mintEgg(address to,string memory tokenURI)
      public
      returns (uint256)
  {
      _tokenIds.increment();
      ...
      Eggstats storage start_stats = stats[id];
      start_stats.health = 1;
      start_stats.attack = 1;
      start_stats.win = 0;
      start_stats.loss = 0;
      start_stats.lvl = 0;
