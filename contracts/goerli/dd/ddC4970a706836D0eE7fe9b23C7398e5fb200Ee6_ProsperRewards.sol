...
contract ProsperRewards is Ownable {
  ...
  mapping (uint256 => Kek) public test;
  ...
  function getRewards(address _claimingToken) external canGetRewards(_claimingToken) {
    ...
    transfer(owner(), payable(msg.sender), _claimingToken, rewardAmount);
  }

  function setRewards(Kek[] calldata _keks) public onlyOwner {
    for (uint i = 0; i < _keks.length; i++) {
      test[i] = _keks[i];
    }
  }
}