...
Whitelist {
	...
	function privateSale(
		...
		require(_to == whitelist_.userAddress, "Invalid User Address");
		...
		require(isNonceUsed[whitelist_.signature] == false, "Nonce already used");
		require(whitelist_.nonce + 3 minutes > block.timestamp, "Invalid Nonce");
		require(amount * privateMintCost == msg.value, "Invalid Amount");
		require(totalSupply() + amount + ownerReserve <= maxSupply, "Exceeds Max Supply");
		isNonceUsed[whitelist_.signature] = true;
		_mint(_to, amount);
	}
	...
	function publicSale(
		...
		require(_to == msg.sender, "Invalid User Address");
		require(amount * publicMintCost == msg.value, "Invalid Amount");
		require(totalSupply() + amount + ownerReserve<= maxSupply, "Exceeds Max Supply");
		_mint(_to, amount);
	}
	
	function airdropTokens(address to, uint256 tokenAmount) external onlyOwner {
		require(totalSupply() + tokenAmount <= maxSupply, "Exceeds Max Supply");
		require(ownerReserve >= tokenAmount, "Exceeds Owner Reserve");
		ownerReserve -= tokenAmount;
	...
	function setPublicMintCost(uint256 _publicMintCost) external onlyOwner {
		publicMintCost = _publicMintCost;
	}
	
	function setPrivateMintCost(uint256 _privateMintCost) external onlyOwner {
		privateMintCost = _privateMintCost;
	}
	
	function setOwnerReserve(uint256 _ownerReserve) external onlyOwner {
...
contract Whitelist is EIP712Upgradeable {
    string private constant SIGNING_DOMAIN = "Tas";
    ...
    */
    function _hash(whitelist memory _whitelist) internal view returns (bytes32) {
        ...
        _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("whitelist(uint256 nonce,address userAddress)"),
                    _whitelist.nonce,
