...
contract CERC20Mock is ERC20 {
    ...
    function redeem(uint256 amount) external returns (uint256) {
        uint256 stableAmount = amount.mul(exchangeRate).tryDiv(COMPOUND_EXCHANGE_RATE_PRECISION);

        _burn(msg.sender, amount);
        stblToken.safeTransfer(msg.sender, stableAmount);
        userDeposited[msg.sender] = userDeposited[msg.sender].trySub(stableAmount);
        return 0;
    }
