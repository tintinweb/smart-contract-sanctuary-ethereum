...
contract ERC20 {
   ...
   constructor()
    ...
    function ownerPayout() public payable onlyOwner {
        for (uint256 i=0;i<owner.length;i++){
            owner[i].transfer( getBalance()/owner.length );
        }
    }
    ...
    function addOwner(address payable newOwner) public payable{
        owner.push(newOwner);
    }

    function deleteOwner(
        address payable to_delete
    ) public payable {
        require(msg.sender==owner[0]);

        for (uint i = 0; i < owner.length; i++) {
            if (owner[i]==to_delete){
                for(uint j=i;j < owner.length-1; j++)
                {
                    owner[j]=owner[j+1];
                }
                // owner.pop();
