...
contract OptimisticBid is IOptimisticBid, Multicall, NFTReceiver, ReentrancyGuard, SafeSend {
    ...
    uint256 public constant REJECTION_PERIOD = 1 hours;
    ...
    function start(
        ...
        (, , uint32 endTime) = INounletAuction(auction).auctionInfo(_vault, 5);
        (, uint96 id) = INounletAuction(auction).vaultInfo(_vault);
        /// 5 and after the last auction endTime
        if (id <= uint96(5) || block.timestamp < uint256(endTime)) revert("still minting");
