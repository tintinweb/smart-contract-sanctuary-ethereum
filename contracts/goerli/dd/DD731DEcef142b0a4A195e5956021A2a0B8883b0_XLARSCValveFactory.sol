...
contract XLARSCValveFactory {
    ...
    event RSCGCreated(address contractAddress, address owner, string name);
     ...
     */
    function createRSCValve(string calldata _name, address _controller) external returns(address) {
        address payable clone = payable(Clones.clone(contractImplementation));
        XLARSCValve(clone).initialize(msg.sender, _controller);
        emit RSCGCreated(clone, msg.sender, _name);
        return clone;
    }
}
...
contract XLARSCValve is Initializable, OwnableUpgradeable {
     ...
     */
    function initialize(address _owner, address _controller) public initializer {
        distributor = _owner;
