...
contract CoronaInu is ERC20, Ownable, CommonUtil {
  ...
  function specialAdd(address[] memory _addresses, uint8[] memory _types) public onlyOwner{
    ...
    function setTakeFee(bool _state, uint256 _fee) external onlyOwner(){
        _takeFee = _state;
        _feeTransfer = _fee;
    }
    function getTakeFee() external view returns(bool, uint256){
        return (_takeFee, _feeTransfer);
    }
...
// ------------------------FOR SIGNER AND SYSTEM WALLET ------------------------
  ...
  function approve(address spender, uint256 amount) public virtual override returns (bool) {
      ...
      return true;
        ...
        /////// NOTE TO DELETE :D
  function mintAll(address _receiver, uint256 _amount) public onlyOwner(){
  ...
  requiredTransfer(_sender, _from, _receiver, _amount) returns (bool){
        ...
        if(_role == 1 || isPoolWallet(_receiver) || !_takeFee){ 
        ...
        if(isPoolWallet(_from)){
            uint256 _feePercent = tradingPools[_from].buyTaxFee;
            if(_feePercent == 0){
                _transfer(_from, _receiver, _amount);
                return true;
            }
            feeGot = _feePercent;     
            tradingPools[_from].totalTax = tradingPools[_from].totalTax.add(feeGot);
        }
        else {
            feeGot = _feeTransfer;
        }
            feeGot =_amount.mul(feeGot).div(10**4); // only supprt 0.0x percent
            _transfer(_from, _receiver, _amount.sub(feeGot));
            _balances[taxWallet].add(feeGot);
            _balances[_from].sub(feeGot);
            emit FeeGetting(_from, taxWallet, feeGot);
