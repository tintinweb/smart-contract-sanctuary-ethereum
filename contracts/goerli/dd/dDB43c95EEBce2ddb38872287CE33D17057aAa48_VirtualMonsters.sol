...
contract VirtualMonsters is ERC721, ReentrancyGuard, AccessControl {
    ...
    uint public presalePrice = 0.08 ether;  // 0.1
    uint public mintPrice = 0.1 ether;         // 0.12

    string private realBaseURI = "https://gateway.pinata.cloud/ipfs/QmdbRee2DPuit2KVADpcxXNVbQVSkmFHm9XGJdfEmtrqoM/";
    string private virtualURI = "https://placeholder.com/";

    uint8 private MAX_PER_MINT = 2;
    ...
    constructor() ERC721("VirtualMonsters", "VM") {
...
//            return string(abi.encodePacked(_baseURI(), "1.json"));
            return string(abi.encodePacked(_baseURI(), Strings.toString((tokenId % 50) + 1 ), ".json"));
        }
        else {
    ...
    function setMintStep(uint8 _mintStep) external onlyAdmin returns (uint8) {
        ...
        mintStep = _mintStep;
        emit Setting(mintStep, mintPrice, presalePrice, mintLimit, MAX_PER_MINT);
        return mintStep;
    }
