...
contract FraxlendPairDeployer is Ownable {
    ...
    function setCreationCode(bytes calldata _creationCode) external onlyOwner {
        ...
        if (_creationCode.length > 13000) {
            bytes memory _secondHalf = BytesLib.slice(
                _creationCode, 13000, _creationCode.length - 13000
            );
    ...
    function setDefaultSwappers(address[] memory _swappers)
        external
        onlyOwner
    {
    ...
    function _deployFirst(
    ...
    ) private returns (address _pairAddress) {
        {
            ...
            bytes memory _creationCode = BytesLib.concat(
                SSTORE2.read(contractAddress1), SSTORE2.read(contractAddress2)
            );
            ...
            require(
                ...
                "FraxlendPairDeployer: create2 failed"
            );
    ...
    function _deploySecond(
        ...
        _fraxlendPair.initialize(
            _name, _approvedBorrowers, _approvedLenders, _rateInitData
        );
    ...
    /// @param _configData abi.encode(address _asset, address _collateral, address _oracleMultiply, address _oracleDivide, uint256 _oracleNormalization, address _rateContract, bytes memory _rateInitData)
    /// @return _pairAddress The address to which the Pair was deployed
    function deploy(bytes memory _configData)
        external
        returns (address _pairAddress)
    {
        (address _asset, address _collateral,,,, address _rateContract,) = abi
            .decode(
            _configData,
            (address, address, address, address, uint256, address, bytes)
        );
        ...
        _deploySecond(
            _name, _pairAddress, _configData, new address[](0), new address[](0)
        );
    ...
    function deployCustom(
        ...
        require(
            _maxLTV <= GLOBAL_MAX_LTV,
            "FraxlendPairDeployer: _maxLTV is too large"
        );
        require(
            IFraxlendWhitelist(FRAXLEND_WHITELIST_ADDRESS)
                .fraxlendDeployerWhitelist(msg.sender),
...
import {SafeERC20 as OZSafeERC20} from
