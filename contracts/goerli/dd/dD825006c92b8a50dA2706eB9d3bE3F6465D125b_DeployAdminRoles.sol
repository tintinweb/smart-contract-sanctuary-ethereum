...
contract AdminRoles {
    string private MarketPlace;
    address private PSAdmin;
    address private MPSAdmin;
    address[] private MPCompilanceAdmin;
    address[] private MPTokenAdmins;
    mapping(address => bool) private isPSuperAdmin;
    mapping(address => bool) private isMPSuperAdmin;
    mapping(address => bool) private isMPCompilanceAdmin;
    mapping(address => bool) private isMPTokenAdmin;
    uint private numConfirmationsRequired; // Mostly 2 Marketplace Compliance Admin and MPSAdmin
    ...
    constructor(string memory _marketplaceName, address _PSAdmin, address _MPSAdmin) {
        ...
        isPSuperAdmin[_PSAdmin] = true;
        isMPSuperAdmin[_MPSAdmin] = true;
    }
    
    // We need addresses of both SuperAdmin & compliance-admin as both should approve it!
    function addMPCompilenceAdmin(address[] memory _owners, uint _numConfirmationsRequired) public {
        if(isMPSuperAdmin[msg.sender]) {    
            if (_owners.length <= 0) {
            ...
            for (uint i = 0; i < _owners.length; i++) {
                ...
                if(isMPCompilanceAdmin[owner]) {
                    revert Admin_should_be_unique();
                }

                isMPCompilanceAdmin[owner] = true;
                MPCompilanceAdmin.push(owner);
            }
    ...
    function addMPTokenAdmin(address[] memory _MPTokenAdmin) public {
        if(isMPSuperAdmin[msg.sender]) {
            for (uint i = 0; i < _MPTokenAdmin.length; i++) {
                ...
                if(isMPTokenAdmin[MPTokenAdmin]) {
                    revert Admin_should_be_unique();
                }

                isMPTokenAdmin[MPTokenAdmin] = true;
                MPTokenAdmins.push(MPTokenAdmin);
            }
        } else {
            revert not_super_admin();
        }
...
import "../../Marketplace/AdminRoles.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

contract DeployAdminRoles is UUPSUpgradeable {
    function deployAdminRolesContract(
        string memory _marketplaceName, 
        address _PSAdmin, 
        address _MPSAdmin
    ) external returns(address) {
        AdminRoles adminRolesContract = new AdminRoles(
            _marketplaceName,
            _PSAdmin,
            _MPSAdmin
        );
