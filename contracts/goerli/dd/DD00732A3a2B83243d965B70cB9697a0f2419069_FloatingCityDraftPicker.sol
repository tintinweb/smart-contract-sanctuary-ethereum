...
contract FloatingCityDraftPicker is FloatingCityVRFHandler {
    ...
    constructor(uint64 _subscriptionId, address _vrfCoordinator, IFloatingCity _floatingCityContract)
       FloatingCityVRFHandler(_subscriptionId, _vrfCoordinator)
    {
        floatingCityContract = _floatingCityContract;
    }

    // constructor()
    //    FloatingCityVRFHandler(3700, 0x2Ca8E0C643bDe4C2E08ab1fA0da3401AdAD7734D)
    // {
    //     floatingCityContract = IFloatingCity(0x28f46cB359aF734dB071E44bcec97DA429E21117);
    // }
    ...
    function getLatestPicks() public onlyOwner view returns(string memory){
        if(!latestRoundPicked) revert LatestPicksNotAppliedYet();
        
        string memory output;
        for(uint i = 0; i < draftRoundPicks[draftRound].length; i++) {
            output = string.concat(output, Strings.toString(draftRoundPicks[draftRound][i]));
            if(i != draftRoundPicks[draftRound].length - 1) {
