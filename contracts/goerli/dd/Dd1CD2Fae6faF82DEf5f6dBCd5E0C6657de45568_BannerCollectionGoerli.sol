...
import './DeferredMutable.sol';

/**
 * @title AccessControlledMutable
 * @author @NiftyMike | @NFTCulture
 * @dev Implements mutable checks with AccessControl backing.
 */
contract AccessControlledMutable is AccessControl, DeferredMutable {
    // Mutability gated behind CREATOR_ROLE.
...
/**
 * @title DeferredMutable
 * @author NFT Culture
 * @dev Abstract class for defining API for access control checks around Mutability.
 * In this case, Mutable checks are deferred to a higher class.
 */
abstract contract DeferredMutable {
 ...
 */
abstract contract ERC1155NFTC is ERC1155Burnable, OperatorFilterer, Ownable, INFTCMinterExperimental {
    bool public operatorFilteringEnabled;

    function setContractURI(string memory newuri) external override onlyOwner {
        _setURI(newuri);
    }
    ...
    function balanceOwnedBy(address account, uint256 tokenId) external view override returns (uint256) {
...
// NFTC Prerelease Contracts
import '../../access/AccessControlledMutable.sol';
import './ERC1155NFTC.sol';
import './FlavorInfoManagerV3Injectable.sol';

/**
 * @title ERC1155NFTCExpandable
 * @author @NiftyMike | @Dr3amLabs
 * @dev ERC1155-type contract with expandable token-type system added on.
 * 
 * NOTE: Some code/approach lifted from OZ ERC1155Supply (token/ERC1155/extensions/ERC1155Supply.sol)
 */
abstract contract ERC1155NFTCExpandable is ERC1155NFTC, FlavorInfoManagerV3Injectable, AccessControlledMutable {
    constructor(
        string memory __contractURI,
        bool __allowMaxSupplyIncrease
    ) ERC1155(__contractURI) FlavorInfoManagerV3Injectable(__allowMaxSupplyIncrease) {
        // Nothing to do.
    }
    ...
    function totalTokensExist(uint256 tokenId) external view virtual returns (uint256) {
        return _getFlavorInfo(tokenId).totalSupply;
    }
    ...
    /**
     * @dev Indicates whether any token exist with a given tokenId, or not.
     */
    function exists(uint256 tokenId) external view virtual returns (bool) {
        return _getFlavorInfo(tokenId).totalSupply > 0;
    }
    ...
    function _canMint(
        ...
        bool enforceValue,
        uint256 valueSent
    ) internal view returns (FlavorInfoV3 memory) {
        FlavorInfoV3 memory tokenFlavor = _getFlavorInfo(flavorId);
    ...
    function _validateMint(
        ...
        (bool validationResult, string memory message) = IFlavorInfoValidatorV3(tokenFlavor.provider).validateMint(
            caller,
            holder,
            count,
            tokenFlavor,
    ...
    function _setFlavorForToken(uint256 tokenId, uint256 flavorId) internal {
        // No-op for ERC1155, since the tokenId is the flavorId.
    }

    /**
     * @dev Get the flavorId for a tokenId
     */
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < tokenIds.length; ) {
                ...
                unchecked {
                    _saveFlavorTotalSupply(tokenId, uint64(_getFlavorInfo(tokenId).totalSupply + amounts[i]));
                    ++i;
                }
            }
        }
        ...
        if (to == address(0)) {
            for (uint256 i = 0; i < tokenIds.length; ) {
                ...
                uint256 supply = _getFlavorInfo(tokenId).totalSupply;
                require(supply >= amount, 'ERC1155: invalid amount');
                unchecked {
                    _saveFlavorTotalSupply(tokenId, uint64(supply - amount));
                    ++i;
                }
            }
        }
...
error NullFlavor();
error FlavorAlreadyCreated();
error FlavorDoesNotExist();
error InvalidValuePayment();
error ExceedsMaxSupplyForFlavor();
error AmountExceedsMaxSupply();

/**
 * @title FlavorInfoManagerV3
 * @author @NFTCulture
 */
abstract contract FlavorInfoManagerV3 is IFlavorInfoV3 {
    // Storage for Token Flavors
    mapping(uint256 => FlavorInfoV3) private _flavorInfo;
    ...
    constructor() {
        _injectFlavors(_getInitialFlavors());
    }

    function _injectFlavors(FlavorInfoV3[] memory) internal virtual;

    function _getInitialFlavors() internal virtual returns (FlavorInfoV3[] memory);

    function getFlavorInfo(uint256 flavorId) external view returns (FlavorInfoV3 memory) {
        return _getFlavorInfo(flavorId);
    }

    function _getFlavorInfo(uint256 flavorId) internal view returns (FlavorInfoV3 memory) {
    ...
    function _createFlavorInfo(FlavorInfoV3 memory tokenFlavor) internal {
        uint256 flavorId = tokenFlavor.flavorId;

        if (flavorId == 0) revert NullFlavor();
        if (_flavorInfo[flavorId].flavorId > 0) revert FlavorAlreadyCreated();

        _flavorInfo[flavorId] = tokenFlavor;
        _flavorIds.push(uint64(flavorId));
    }

    function _updateFlavorInfo(FlavorInfoV3 memory tokenFlavor) internal {
        if (_flavorInfo[tokenFlavor.flavorId].flavorId == 0) revert FlavorDoesNotExist();

        _flavorInfo[tokenFlavor.flavorId] = tokenFlavor;
    }

    function _saveFlavorTotalMinted(uint256 flavorId, uint64 totalMinted) internal {
        _flavorInfo[flavorId].totalMinted = totalMinted;
    }

    function _saveFlavorTotalSupply(uint256 flavorId, uint64 totalSupply) internal {
        _flavorInfo[flavorId].totalSupply = totalSupply;
    }
    ...
    function computeMaxSupply() external view returns (uint256) {
        ...
        for (uint256 idx = 0; idx < _flavorIds.length; idx++) {
            FlavorInfoV3 memory current = _flavorInfo[_flavorIds[idx]];
    ...
    function _decrementMaxSupply(uint256 amount) internal {
        if (amount > _maxSupply) revert AmountExceedsMaxSupply();
        unchecked {
    ...
    function _checkFlavorIsValid(FlavorInfoV3 memory tokenFlavor) internal pure {
        if (tokenFlavor.flavorId == 0) revert NullFlavor();
    }

    function _checkValueIsValid(FlavorInfoV3 memory tokenFlavor, uint256 count, uint256 valueSent) internal pure {
        if (valueSent != tokenFlavor.price * count) revert InvalidValuePayment();
    }

    function _checkCountIsValid(FlavorInfoV3 memory tokenFlavor, uint256 count) internal pure {
...
// NFTC Prerelease Contracts
import './FlavorInfoManagerV3.sol';
import './IFlavorInfoProviderV3.sol';
import './IFlavorInfoValidatorV3.sol';
import './IFlavorInfoV3.sol';
import '../../access/DeferredMutable.sol';
...
/**
 * @title FlavorInfoManagerV3Injectable
 * @author @NiftyMike | @NFTCulture
 * @dev Extension contract for Expandable Token Types.
 */
abstract contract FlavorInfoManagerV3Injectable is FlavorInfoManagerV3, DeferredMutable {
    bool private immutable ALLOW_MAX_SUPPLY_INCREASE;

    constructor(bool __allowMaxSupplyIncrease) FlavorInfoManagerV3() {
        ALLOW_MAX_SUPPLY_INCREASE = __allowMaxSupplyIncrease;
    }

    function _injectFlavors(FlavorInfoV3[] memory __tokenFlavors) internal override {
        uint256 maxSupplyIncrease;
        for (uint256 idx = 0; idx < __tokenFlavors.length; ) {
            FlavorInfoV3 memory current = __tokenFlavors[idx];
            _createFlavorInfo(current);

            unchecked {
                maxSupplyIncrease += current.maxSupply;
                ++idx;
            }
        }
        _incrementMaxSupply(maxSupplyIncrease);
    }

    function injectFlavorsFromProvider(address __externalProviderAddress) external {
        if (!_canCreate()) revert InvalidAccess();

        _injectFlavors(IFlavorInfoProviderV3(__externalProviderAddress).provideFlavorInfos());
    }

    /**
     * @notice Create a new token flavor.
     *
     * @param __provider - The address of the provider that stores the ipfsHash and uriFragment
     */
    function injectFlavorsDirectly(
        ...
        require(
            __flavorIds.length == __prices.length && __flavorIds.length == __maxSupplies.length,
            'Unmatched arrays'
        );

        for (uint256 idx = 0; idx < __flavorIds.length; idx++) {
            FlavorInfoV3 memory newFlavor = FlavorInfoV3(
    ...
    function updateTokenFlavor(
        ...
        FlavorInfoV3 memory previousFlavor = _getFlavorInfo(__flavorId);
        ...
        if (force || __provider != address(0)) {
            previousFlavor.provider = __provider;
        }
    ...
    function updateTokenFlavorPrices(uint256[] calldata __flavorIds, uint256 __price) external {
        ...
        for (uint256 idx = 0; idx < __flavorIds.length; idx++) {
            FlavorInfoV3 memory previousFlavor = _getFlavorInfo(__flavorIds[idx]);
            previousFlavor.price = uint96(__price);
            _updateFlavorInfo(previousFlavor);
        }
    }
    ...
    /**
     * @notice Update provider for a batch of flavors.
     */
    function updateTokenFlavorProviders(uint256[] calldata __flavorIds, address __provider) external {
        if (!_canUpdate()) revert InvalidAccess();
...
contract BannerCollectionGoerli is BannerCollectionGoerliBase, BannerCollectionGoerliSplitsAndRoyalties {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC1155NFTCExpandable, ERC2981) returns (bool) {
        return ERC1155NFTCExpandable.supportsInterface(interfaceId) || ERC2981.supportsInterface(interfaceId);
    }
    ...
    function _getInitialFlavors() internal pure override returns (FlavorInfoV3[] memory) {
        return new FlavorInfoV3[](0);
    }
}
...
// NFTC Prerelease Contracts
import '@nftculture/nftc-contracts-private/contracts/token/v3/ERC1155NFTCExpandable.sol';
    ...
    PrivilegedMinter
{
    ...
    ) ERC1155NFTCExpandable(__baseURI, true) ExpandablePhasedMintThree() PrivilegedMinter(CROSSMINT) {
    ...
    /**
     * @notice ProductManager: reserve tokens for community.
     *
     * @param friends addresses to send tokens to.
     * @param count the number of tokens to mint.
     * @param flavorId the type of tokens to mint.
     */
    function premintTokens(address[] calldata friends, uint256 count, uint256 flavorId) external {
        if (!hasRole(PRODUCT_MANAGER_ROLE, msg.sender)) revert InvalidAccess();

        _reserveMint(friends, count, flavorId);
    }
     ...
     * @param flavorId the type of tokens to mint.
     */
    function reserveTokens(address[] calldata friends, uint256 count, uint256 flavorId) external payable onlyOwner {
        _reserveMint(friends, count, flavorId);
    }

    function _reserveMint(address[] calldata friends, uint256 count, uint256 flavorId) internal {
        if (0 >= count || count > MAX_RESERVE_BATCH_SIZE) revert ExceedsBatchSize();

        FlavorInfoV3 memory updatedFlavor = _canMint(count * friends.length, flavorId, false, 0);
        _saveFlavorTotalMinted(flavorId, updatedFlavor.totalMinted);
    ...
    function publicMintTokens(
        ...
        _publicMintTokens(count, flavorId, validationData, msg.sender, holder, msg.sender);
    }

    /**
     * @notice Same as publicMintTokens(), but with a "destination".
     * Purpose is for fiat+custodial wallet platforms (ex: Crossmint).
     *
     * @param count the number of tokens to mint.
     * @param flavorId the type of tokens to mint.
     * @param validationData extra information required to pass validation.
    ...
    function publicMintTokensTo(
        ...
        _publicMintTokens(count, flavorId, validationData, msg.sender, holder, destination);
    }

    function _publicMintTokens(
        uint256 count,
        uint256 flavorId,
        uint256 validationData,
        address caller,
        ...
        FlavorInfoV3 memory updatedFlavor = _canMint(count, flavorId, true, msg.value);

        if (updatedFlavor.provider != address(0)) {
            _validateMint(caller, holder, count, updatedFlavor, validationData);
        }
