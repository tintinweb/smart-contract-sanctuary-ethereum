...
import { ZERO_BALANCE_INIT_GAS_COST } from "../helpers/GasEstimator.sol";
...
import { IPathFinder, PathNotFoundException, UnsupportedAdapterType } from "../interfaces/IPathFinder.sol";
    ...
    ISwapAggregator
{
    ...
    address public curveSwapper;
    address public uniV2Swapper;
    address public uniV3Swapper;
    address public lidoSwapper;

    /// @notice Pathfinder Component ID
    PathFinderComponent public override getComponentId =
        PathFinderComponent.SWAP_AGGREGATOR;
    ...
    function _updatePathFinderComponents() internal override {
        curveSwapper = pathFinder.componentAddressById(
            PathFinderComponent.CURVE_SWAPPER
        );
        uniV2Swapper = pathFinder.componentAddressById(
            PathFinderComponent.UNISWAP_V2_SWAPPER
        );
        uniV3Swapper = pathFinder.componentAddressById(
            PathFinderComponent.UNISWAP_V3_SWAPPER
        );
        lidoSwapper = pathFinder.componentAddressById(
        ...
        returns (SwapQuote[] memory results)
    {
        ...
        results = new SwapQuote[](adaptersLength);

        for (uint256 i; i < adaptersLength; ) {
            address adapter = adapters[i]; // F:[NTP-3,4,5,6,7]
            address swapper = getSwapper(adapter); // F:[NTP-3,4,5,6,7]

            if (swapper != address(0)) {
                currentQuote = ISwapper(swapper).getBestDirectPairSwap(
                    swapTask,
                    adapter
                ); // F:[NTP-3,4,5,6,7]

                SwapQuote memory bestConnectorQuote = ISwapper(swapper)
                    .getBestConnectorSwap(swapTask, adapter); // F:[NTP-3,4,5,6,7]

                if (bestConnectorQuote.isBetter(swapTask, currentQuote, 0)) {
                    currentQuote = bestConnectorQuote; // F:[NTP-6,7]
                }

                if (currentQuote.found) {
                    results[resultsLength] = currentQuote; // F:[NTP-3,4,5,6,7]
                    resultsLength++; // F:[NTP-3,4,5,6,7]
                }
            }
        ...
        return results.trim(); // F:[NTP-3,4,5,6,7]
    }
    ...
    ) public override returns (StrategyPathTask memory) {
        if (
            typeIn == TokenType.NORMAL_TOKEN &&
            task.targetType == TokenType.NORMAL_TOKEN
        ) {
            uint256 amountOut;
            (amountOut, task) = findBestSwapOrRevert(
                tokenIn,
                amountIn,
                task.target,
                task
    ...
    {
        return swapAllNormalTokens(task, task.target);
    }
    ...
    function findBestSwap(
        ...
        address tokenOut,
        StrategyPathTask memory task
    ) public returns (uint256 amountOut, StrategyPathTask memory) {
        return
            _findBestSwapInternal(
                tokenIn,
                amountIn,
                tokenOut,
                false,
    ...
    function findBestAllInputSwap(
        ...
        uint256 amountIn = task.balances.getBalance(tokenIn);
        return
            _findBestSwapInternal(
                tokenIn,
                amountIn,
    ...
    function _findBestSwapInternal(
        address tokenIn,
        uint256 amountIn,
        address tokenOut,
        bool isAllInput,
        bool revertOnNoPath,
        StrategyPathTask memory task
    ) internal returns (uint256 amountOut, StrategyPathTask memory) {
        SwapQuote memory quote = findBestSwapQuote(
            task.toSwapTask(tokenIn, amountIn, tokenOut, isAllInput),
            task.adapters
        );

        if (!quote.found) {
            if (revertOnNoPath) {
                revert PathNotFoundException(tokenIn, tokenOut);
            } else {
                return (0, task);
            }
        }

        amountOut = task.amountOutWithSlippage(quote.amount);

        task.balances.subBalance(tokenIn, amountIn);
        task.balances.addBalance(tokenOut, amountOut);
        task.gasUsage += quote.gasUsage;
        task.calls = task.calls.append(quote.multiCall);
    ...
    ) public returns (SwapQuote memory bestQuote) {
        SwapQuote memory currentQuote;
        uint256 adaptersLength = adapters.length;

        for (uint256 i; i < adaptersLength; ) {
            address adapter = adapters[i]; // F:
            address swapper = getSwapper(adapter); // F:
            ...
            if (swapper != address(0)) {
                ...
                SwapQuote memory bestConnectorQuote = ISwapper(swapper)
                    .getBestConnectorSwap(swapTask, adapter); // F:

                if (
                    bestConnectorQuote.isBetter(
                        swapTask,
                        currentQuote,
                        pathFinder.getGasPriceTokenOutRAY(swapTask.tokenOut)
                    )
                ) {
                    currentQuote = bestConnectorQuote; // F:
                }

                if (
                    currentQuote.found &&
                    currentQuote.isBetter(
                        swapTask,
                        bestQuote,
                        pathFinder.getGasPriceTokenOutRAY(swapTask.tokenOut)
                    )
                ) {
                    bestQuote = currentQuote; // F:
                }
            }
    ...
    function swapAllNormalTokens(StrategyPathTask memory task, address target)
        ...
        returns (StrategyPathTask memory)
    {
        MultiCall[] memory calls = new MultiCall[](task.balances.length);

        uint256[] memory connectorIndexes = new uint256[](
            task.connectors.length
        );

        uint256 j;
        uint256 mcallLen;

        // WHY IT'S SAFE TO USE UNCHECKED HERE:
        // The block doesn't expect to get overflow
        // All gas operations could not be > uint256
        // mcallLen could not be array size
        // i is limited with connectorLength
        // balances are updated in their own function
        unchecked {
            // Iterates for non-connector tokens
            for (uint256 i; i < task.balances.length; ++i) {
                address token = task.balances[i].token;

                if (
                    token != target &&
                    pathFinder.tokenTypes(token) == TokenType.NORMAL_TOKEN
                ) {
                    // TODO: move before many calls
                    if (
                        task.connectors.includes(token) &&
                        isBestSwapDirect(task, i, target)
                    ) {
                        connectorIndexes[j] = i; // F:[CPF-10,11]
                        ++j; // F:[CPF-10,11]
                    } else {
                        if (task.balances[i].balance > 1) {
                            (
                                SwapQuote memory currentQuote,
                                address tokenOut,

                            ) = findSwapQuote(task, i, target, false); // F:[CPF-10,11]

                            if (!currentQuote.found)
                                if (!task.force) {
                                    revert PathNotFoundException(token, target); // F:[CPF-9]
                                } else {
                                    continue;
                                }

                            task.balances[i].balance = 0; // F:[CPF-10,11]
                            task.balances.addBalance(
                                tokenOut,
                                task.amountOutWithSlippage(currentQuote.amount)
                            ); // F:[CPF-10,11]
                            calls[mcallLen] = currentQuote.multiCall; // F:[CPF-10,11]
                            mcallLen++; // F:[CPF-10,11]
                            task.gasUsage += currentQuote.gasUsage; // F:[CPF-10,11]
                        }
                    }
                }
            }

            for (uint256 i; i < task.connectors.length; ++i) {
                uint256 tokenIndex = connectorIndexes[i]; // F:[CPF-10,11]
                address token = task.balances[tokenIndex].token; // F:[CPF-10,11]

                if (task.balances[tokenIndex].balance > 1 && token != target) {
                    (SwapQuote memory currentQuote, , ) = findSwapQuote(
                        task,
                        tokenIndex,
                        target,
                        true
                    ...
                    task.balances[tokenIndex].balance = 0; // F:[CPF-10]
                    task.balances.addBalance(
                        target,
                        task.amountOutWithSlippage(currentQuote.amount)
                    ); // F:[CPF-10,11]
        ...
        task.calls = task.calls.concat(calls.trim()); // F:[CPF-10]
        return task;
    }
    ...
    function isBestSwapDirect(
        StrategyPathTask memory task,
        uint256 tokenIndex,
        address target
    ) internal returns (bool) {
        if (task.balances[tokenIndex].balance > 1) {
            (SwapQuote memory quote, , bool isConnectorSwap) = findSwapQuote(
                task,
                tokenIndex,
                target,
                false
            ...
            return quote.found && !isConnectorSwap;
        }

        return true;
    }

    function findSwapQuote(
        SwapTask memory swapTask,
        address[] memory adapters,
        ...
        returns (
            ...
            address tokenOut,
            bool isConnectorSwap
        )
    {
        // Try to find direct swap

        uint256 gasPriceTokenOut = pathFinder.getGasPriceTokenOutRAY(
            swapTask.tokenOut
        );

        quote = getBestDirectPairSwap(swapTask, adapters, gasPriceTokenOut); // F:[CPF-6,7,8]

        tokenOut = swapTask.tokenOut; // F:[CPF-6,7,8]

        SwapQuote memory bestOutQuote = quote;

        if (!isConnector) {
            for (uint256 i; i < swapTask.connectors.length; ) {
                uint256 connectorBalanceBefore = balances.getBalance(
                    swapTask.connectors[i]
                ); // F:[CPF-6,7,8]

                (
                    SwapQuote memory connectorTokenInQuote,
                    SwapQuote memory connectorTokenOutQuote
                ) = getComplexPairSwap(
                        swapTask,
                        i,
                        connectorBalanceBefore,
                        adapters
                    ); // F:[CPF-6,7,8]

                if (connectorTokenOutQuote.found) {
                    connectorTokenOutQuote.gasUsage =
                        connectorTokenInQuote.gasUsage +
                        connectorTokenOutQuote.gasUsage +
                        (
                            (connectorBalanceBefore == 0)
                                ? ZERO_BALANCE_INIT_GAS_COST
                                : 0
                        ); // F:[CPF-8]

                    if (
                        connectorTokenOutQuote.isBetter(
                            swapTask,
                            bestOutQuote,
                            bestOutQuote.gasUsage,
                            gasPriceTokenOut
                        )
                    ) {
                        isConnectorSwap = true;
                        bestOutQuote = connectorTokenOutQuote;
                        quote = connectorTokenInQuote; // F:[CPF-6,7,8]

                        // We add here gas cost to init balance
                        quote.gasUsage += (
                            (connectorBalanceBefore == 0)
                                ? ZERO_BALANCE_INIT_GAS_COST
                                : 0
                        ); // F:[CPF-7]
                        tokenOut = swapTask.connectors[i]; // F:[CPF-6,7,8]
                    }
                }
        ...
        )
    {
        ...
        SwapTask memory swapTask = task.toSwapTask(tokenIndex, target);
        return
            findSwapQuote(swapTask, task.adapters, task.balances, isConnector);
    }

    function getComplexPairSwap(
        SwapTask memory swapTask,
        uint256 connectorIndex,
        uint256 connectorCumulativeBalance,
        address[] memory adapters
    )
        public
        returns (
        ...
        )
    {
        ...
        connectorTokenInQuote = getBestDirectPairSwap(
            swapTask.makeConnectorInTask(connectorIndex),
            adapters,
            pathFinder.getGasPriceTokenOutRAY(
                swapTask.connectors[connectorIndex]
            )
        ); // F:[CPF-3,4,5]

        if (connectorTokenInQuote.found) {
            // we use connector swapped amount, cause future rate depends on swap size
            uint256 totalCumulativeAmount = connectorTokenInQuote.amount +
                connectorCumulativeBalance; // F:[CPF-3,4,5]

            connectorTokenOutQuote = getBestDirectPairSwap(
                swapTask.makeConnectorOutTask(
                    totalCumulativeAmount,
                    connectorIndex,
                    connectorCumulativeBalance == 0
                ),
                adapters,
                pathFinder.getGasPriceTokenOutRAY(swapTask.tokenOut)
            ); // F:[CPF-3,4,5]

            connectorTokenOutQuote.amount =
                (connectorTokenOutQuote.amount * connectorTokenInQuote.amount) /
                totalCumulativeAmount; // F:[CPF-3,4,5]
        }
    }
    ...
    function getBestDirectPairSwap(
        ...
        for (uint256 i; i < adaptersLength; ) {
            ...
            address swapper = getSwapper(adapter);

            if (swapper != address(0)) {
                SwapQuote memory bestQuote = ISwapper(getSwapper(adapter))
                    .getBestDirectPairSwap(swapTask, adapter); // F:[NTP-9,10]

                if (bestQuote.isBetter(swapTask, quote, gasPriceInTokenOut)) {
                    quote = bestQuote; // F:[NTP-9,10]
                }
            }
            unchecked {
                ++i;
    ...
    function getSwapper(address adapter) public view returns (address swapper) {
        if (adapter != address(0)) {
            AdapterType adapterType = IAdapter(adapter)._gearboxAdapterType();

            if (adapterType == AdapterType.UNISWAP_V2_ROUTER) {
                swapper = uniV2Swapper; // F:[NTP-2]
            }
            if (adapterType == AdapterType.UNISWAP_V3_ROUTER) {
                swapper = uniV3Swapper; // F:[NTP-2]
            }
            if (
                adapterType == AdapterType.CURVE_V1_EXCHANGE_ONLY ||
                ...
                swapper = curveSwapper; // F:[NTP-2]
            }
            if (adapterType == AdapterType.LIDO_V1) {
                swapper = lidoSwapper;
            }
        }
    }
