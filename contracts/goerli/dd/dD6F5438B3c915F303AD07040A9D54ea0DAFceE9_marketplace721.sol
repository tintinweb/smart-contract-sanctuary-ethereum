...
contract marketplace721 is ReentrancyGuard, ERC721Holder, Initializable, UUPSUpgradeable

{
    ...
    event TransferSingle(address indexed from, address indexed to, uint id);
    ...
    function ListForSale(IERC721 _nft, uint _tokenId, uint _price, uint _amount, bytes calldata _bytes, address _royaltyReceipent, uint256 _royalityPercentage) external nonReentrant returns(uint){
        ...
        _nft.safeTransferFrom(msg.sender, address(this), _tokenId, _bytes);
    ...
    function PurchaseItem(uint _itemId) external payable nonReentrant{
        ...
        emit TransferSingle(address(this), msg.sender, item.tokenId);
    }
    ...
    function SaleAcceptBid(uint _itemId) external payable nonReentrant{
        ...
        item.sold = true;
        item.nft.safeTransferFrom(address(this), msg.sender, item.tokenId);
        emit TransferSingle(address(this), bid.Bidder, item.tokenId);
    }
