...
contract DEX {
    ...
    function init(uint256 tokenAmount) public payable returns (uint256) {
        require(
            totalLiquidity == 0,
            "DEX init liquidity pool already initiated"
        );
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        ...
        //I need xInput, xReserve and yReverse.
        //when we send the money in this current balance already got msg.value so we need to sub.
        uint ethReserve = address(this).balance.sub(msg.value);
        //need to get this token address
        uint tokenReserve = token.balanceOf(address(this));
        tokenOutput = price(msg.value, ethReserve, tokenReserve);
        require(
            token.transfer(msg.sender, tokenOutput),
            "revert because transfer failed"
        );
        emit EthToTokenSwap(
            msg.sender,
            "Eth to Balloons",
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        //find current ratio
        require(msg.value > 0, "deposit() cannot deposit 0 eth");
    ...
    function withdraw(
        ...
        require(
            ...
            "withdraw() amount exceed liquidity"
        );
