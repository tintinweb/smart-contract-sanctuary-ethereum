...
contract ActiveErthling is Erthling {
    constructor(
...
contract Erthling is DotEarthERC721, IErthling, ISoftStaking {
    uint256 public immutable _charges;

    mapping(uint256 => uint256) public override stakingStarted;
    ...
    function charges(uint256 tokenId_) external view override returns (uint256) {
        return _charges;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IERC165, DotEarthERC721Royalty)
        returns (bool)
    {
        return
            interfaceId == type(IErthling).interfaceId ||
            interfaceId == type(ISoftStaking).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    modifier onlyMinterRole() {
        accessController.checkErthlingMinterRole(msg.sender);
        _;
    }

    modifier onlyAdminRole() {
        accessController.checkErthlingAdminRole(msg.sender);
        _;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(stakingStarted[tokenId] == 0, "Erthling: token is staking");
    ...
    function toggleStaking(uint256 tokenId) external override {
        require(
            ...
            "Erthling: not approved or owner"
        );
    ...
    function isStaking(uint256 tokenId) external view override returns(bool) {
        return stakingStarted[tokenId] != 0;
    }
}
...
interface IErthling is IERC721, IDotEarth721 {
    function charges(uint256 tokenId_) external view returns (uint256);
...
interface ISoftStaking {
    ...
    function stakingStarted(uint256 tokenId) external view returns(uint256);
}
 ...
 */
abstract contract DotEarthERC721Base is Context, ERC165, IERC721, IERC721Metadata {
