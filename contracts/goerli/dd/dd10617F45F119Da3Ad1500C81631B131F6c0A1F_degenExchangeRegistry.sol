...
contract degenExchangeRegistry is XchangeBase, GasManager {
     ...
     *          @dev    -   Setters
     */
    function setCallbackGasLimit(uint32 newGaslimit_) public onlyOwner {
    ...
    function setNewVRFInfo(
        uint64 subscriptionId_,
        address vrfCoordinator_,
        bytes32 keyHash_
    ) public override onlyOwner {
        _setNewVRFInfo(subscriptionId_, vrfCoordinator_, keyHash_);
    }
 ...
 *                www.nylonblocks.io | nylonblocks.eth
 */
abstract contract XchangeBase is
    VRFConsumerBaseV2,
    ...
    Ownable
{
    ...
    uint32 private _amountOfNumbersToRequest = 1;
    //  Draws in cue
    ...
    function _requestRandomNumbers() internal {
        ...
        if (requestId_ > 0) {
            ...
            _drawCount++;
            _requestIdToDrawerAddress[requestId_] = requester_;
            _DrawsInLine++;
            _drawTriggers[requestId_] = _drawCount;
            _userDepositCount[_requestIdToDrawerAddress[requestId_]]--;
        } else {
            revert FailedToRequestRandomNumbers(requestId_);
        }
    }
...
interface IXchange {
    ...
    function setNewVRFInfo(uint64 subscriptionId_, address vrfCoordinator_, bytes32 keyHash_)external;
