...
contract animalMain is  ERC721,Ownable {
  ...
  function createAnimal() public {
      ...
      uint dnaModulusstar = 10 ** dnastar;
    
      //使用亂數來產生DNA, 星級, 動物種類
      
      dna=keccak256(msg.sender,ownerAnimalCount[msg.sender]);
      star=uint(keccak256(msg.sender,ownerAnimalCount[msg.sender]))%dnaModulusstar;
      roletype=uint(keccak256(msg.sender,ownerAnimalCount[msg.sender]))%5;
