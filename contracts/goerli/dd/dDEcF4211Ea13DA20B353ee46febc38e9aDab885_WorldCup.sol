// make sure you cant mint group after quarters are released 
...
contract WorldCup is ERC721Base {
    ...
    enum Status { GROUP, ROUND16, QF, COMPLETE }
    Status public status = Status.GROUP;

    // new mapping for redeemed NFTs
    mapping(uint => bool) public redeemed;

    // Store prices for minting each round
    mapping(Status => uint) public prices;
    // Keep track of prize pools for each round 
    mapping(Status => uint) public prizePools;
    ...
    constructor(
        ...
        prices[Status.GROUP] = 0.0069 ether;
        prices[Status.ROUND16] = 0.080085 ether;
        prices[Status.QF] = 0.420 ether;
    }
    ...
    function serializeList(uint[] memory list) public pure returns(string memory res) {
    ...
    function unSerializeList(string memory list) public pure returns(uint[] memory) {
    ...
    /*
        Init: Status = Group
        - can mint group
        - can mint 16  // users not incentivized
        - can mint qf // users not incentivized
        - cannot claim prize

        // admin sets status to 16
        - cannot mint group 
        - can mint 16 
        - can mint qf
        - cannot claim prize

        // admin sets status to QF
        - cannot mint group 
        - cannot mint 16 
        - can mint qf
        - cannot claim prize

        // admin enters results and updates status to complete
        - cannot mint group 
        - cannot mint 16 
        - can mint qf
        - can claim prize on group 
        - can claim prize on 16
        - can claim prize on qf 
    */


    function inferType(uint[] memory bracket) public pure returns(Status) {
        if(bracket.length == 31) {
            return Status.GROUP;
        } else if(bracket.length == 15) {
            return Status.ROUND16;
    ...
    function isWinner(uint tokenId) public view returns(bool) {
        // Check if results match the bet! 
        // make sure we dont have 0 brackets or 0 results
        // Did the bracket match the entire round's bracket
        return arraysEqual(results, brackets[tokenId]);
    }

    // function winning(uint tokenId) public view returns(bool) {
    //     results 
    //     brackets[tokenId]
    // }

    function payout(uint tokenId) public {
        require(status == Status.COMPLETE, "Game is still in progress");
        require(isWinner(tokenId), "This bracket is not a winning bracket");
        ...
        payable(ownerOf(tokenId)).transfer(pool);
