...
contract PVMIdentityVerifier is AdminControl, IIdentityVerifier {
    ...
    function setListingIDs(address marketplace, uint40 meaningListingID, uint40 powerListingID) public adminRequired{
    ...
    function verify(uint40 listingId, address identity, address, uint256, uint24, uint256, address, bytes calldata) external override returns (bool){
        require(msg.sender == _marketplace); // Require that verify function is called by the marketplace
        require(listingId == _meaningListingID || listingId == _powerListingID); // Require that listing is either for Power or Meaning
        ...
        if(listingId == _meaningListingID){
            if(powerOffers[identity] == _powerListingID){
                return false;
            }
            meaningOffers[identity] = _meaningListingID;
        } else if (listingId == _powerListingID){
            if(meaningOffers[identity] == _meaningListingID){
                return false;
            }
