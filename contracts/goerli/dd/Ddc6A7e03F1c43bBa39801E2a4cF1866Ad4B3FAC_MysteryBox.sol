...
ERC721HolderUpgradeable
{
    ...
    function fulfillRandomness(uint256 tokenId, address user, uint256 randomness) internal {
        require(_isApprovedOrOwner(user, tokenId), "MysteryBox: caller is not owner nor approved");
        burn(tokenId);
    ...
    function setApprovalForAll() public onlyRole(ADMIN_ROLE) {
        IERC721Upgradeable(address(nftToken)).setApprovalForAll(address(randomGenerator), true);
    }
}
