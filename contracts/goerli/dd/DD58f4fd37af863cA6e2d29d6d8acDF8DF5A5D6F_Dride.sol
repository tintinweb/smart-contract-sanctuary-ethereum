...
contract Dride is ERC4907, ReentrancyGuard {
    ...
    event unlistedNFT(address unlistSender, uint256 tokenId, uint256 refund);
    ...
    /**
     * @notice Minting an NFT refers to converting digital files into crypto collections
     * or digital assets stored on the blockchain.
     * @dev Modifiers
     * - To protect against reentrancy attacks
     * @param _tokenURI  Token URI metadata
     * @param startDateUNIX  Start rental period
     * @param endDateUNIX  Expiry rental period
     ...
     */
    /**
     * @notice Burning a non-fungible token means destroying it.
     * @dev Modifiers
     * - To protect against reentrancy attacks
     ...
     * @dev Modifiers
     * - To protect against reentrancy attacks
     * @param value Requested value to transfer
     */
    function approveTransaction(address contractAddress, uint256 value)
        external
    {
        uint256 amount = value * 10**6;
    ...
    function rentNFT(
        ...
        // Transfer rental value
        uint256 amount = rentalFee * 10**6;
     ...
     * @param tokenId  Generated token id
     */
    function unlistNFT(uint256 tokenId) external payable nonReentrant {
        Listing storage listing = _listingMap[tokenId];
        require(
        ...
        if (listing.renter != address(0)) {
            ...
            uint256 amount = refund * 10**6;
            token.transfer(listing.renter, amount);
        }
        // Remove user data
        setUser(tokenId, address(0), 0);
        delete _listingMap[tokenId];

        emit unlistedNFT(msg.sender, tokenId, refund);
    }
}
