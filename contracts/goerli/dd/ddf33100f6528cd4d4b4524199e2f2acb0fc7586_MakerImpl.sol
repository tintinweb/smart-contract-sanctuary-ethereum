...
contract MakerImpl is Initializable, OwnableUpgradeable {
    ...
    function openAndBorrow(bytes32 ilk, uint256 amount, uint256 daiToBorrow) external {
        ...
        _exit(DAIILK, msg.sender, daiToBorrow); //moves DAI to msg.sender
    }
    ...
    */
    ...
    /**
        ...
        @param join If True, collateral will be transferred In from msg.sender and Added to VAT
     */
    function manage(bytes32 ilk, int256 amount, int256 _debt, bool join) external {
        CDPInfo memory cdpInfo = cdpByOwner[msg.sender][ilk];
        if(join && amount > 0) {
    ...
    */
    function repayAndWithdraw(bytes32 ilk, uint256 amountToRepay, uint256 withdrawAmount) external {
        ...
        _exit(ilk, msg.sender, withdrawAmount); //DAI is transferred out.   
    }
    ...
    function repayAllAndWithdraw(bytes32 ilk) external {
        ...
        _exit(ilk, msg.sender, withdrawAmountInNativeDec); //DAI is transferred out.   
    }
    ...
    function setJoinAdapter(bytes32 ilk, address adapterAddress) external onlyOwner {
    ...
    function _open(bytes32 ilk) internal returns(uint256 cdpId, address urn){
        require(cdpByOwner[msg.sender][ilk].cdpId == 0, "vault exists");
        ...
        //another way is to use msg.sender while open-ing above and do cdpAllow in frontEnd.
        CDPManager.cdpAllow(cdpId, msg.sender, 1);
        // CDPManager.give(cdpId, msg.sender);

        VAT.hope(JOINByIlk[DAIILK].adapterAddress);//allows DAI-adapter to move DAI from VAT(to mint external DAI)

        cdpByOwner[msg.sender][ilk] = CDPInfo(cdpId, urn);
    }
    ...
    function _join(bytes32 ilk, address urn, uint256 amount) internal {
        ...
        IERC20Upgradeable(joinInfo.collateralAddress).transferFrom(msg.sender, address(this), amount);
