...
// File @openzeppelin/contracts-upgradeable/utils/introspection/I[email protected]
...
contract  AtemReview is ERC1155PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    {  
        ...
        if (_supply[id] == 0) {
            require(
                _verify(
                    keccak256(abi.encodePacked(id, price, contentURI_)), 
                    signature_,
    ...
    ) public payable nonReentrant()
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (_supply[ids[i]] == 0) {
                require(
                    _verify(
                        keccak256(abi.encodePacked(ids[i], prices[i], contentURIs[i])), 
                        signatures[i],
                        _platformAuthorizeAccount),
