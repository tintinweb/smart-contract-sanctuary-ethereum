...
contract LockLPTokens is Ownable {
    ...
    function unstakeLP(uint256 _bondIndex) external {
        Bonds[] storage stakings = stakes[msg.sender];
        Bonds storage bond = stakings[_bondIndex];
    ...
    function withdrawReward(uint256 _bondIndex) public {
        Bonds[] storage stakings = stakes[msg.sender];
        Bonds storage bond = stakings[_bondIndex];
    ...
    function calculateDays(uint256 _depositTime) public view returns (uint256) {
        ...
        return block.timestamp.sub(_depositTime).div(1 seconds);
    }
    ...
    // Only owner should be able to add pairs
    function deletePair(uint256 _index, address _pair) public onlyOwner {
    ...
    function ownerTakeOutRewardTokens() external onlyOwner {
    ...
    function updatePfanAddress(address _newPfanAddress) external onlyOwner {
