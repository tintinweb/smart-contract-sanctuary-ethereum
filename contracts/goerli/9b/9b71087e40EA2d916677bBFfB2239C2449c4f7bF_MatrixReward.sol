...
contract MatrixReward is Ownable, ReentrancyGuard {
    ...
    function payout(uint256 _amount, uint256 _timestamp, uint256 _oldCheckPoint, bytes memory _signature) external nonReentrant {
      ...
      require(verify(wallet, _amount, _timestamp, _signature), "Invalid Signature");
...
function gethash(address to,uint _amount,   uint _timestamp)  public pure  returns (bytes memory )
    {
        return abi.encodePacked(to, _amount,_timestamp);
    }

    function getKeccak(address to,uint _amount,   uint _timestamp)  public pure  returns (bytes32)
    {
        return keccak256(gethash(to, _amount,_timestamp));
    }
    function getKeccakHashed(address to,uint _amount,   uint _timestamp)  public pure  returns (bytes32)
    {
        return getEthSignedMessageHash(keccak256(gethash(to, _amount,_timestamp)));
    }
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_to, _amount, _timestamp));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(_to, _amount,  _timestamp);
