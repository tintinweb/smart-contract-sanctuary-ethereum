...
/**
 * @title ERC721Sirloin
 * ERC721Sirloin - ERC-721 smart contract with lock URI and shareholders
 */
contract ERC721Sirloin is ERC721, Shareholders, LockURI {
    ...
    ) ERC721(_name, _symbol) Shareholders(_shares, _shareholder_addresses){
        _uri = uri;
     ...
     */
    function mint(address _to, uint8 amount) public payable {
        require(msg.value == mintPrice * amount, "Wrong minting fee"); 
        require(amount + _tokenCount <= maxTotalSupply, "Token limit exceeded");
        for(uint8 i=0; i < amount; i++){
            _safeMint(_to, _tokenCount + 1);
            _tokenCount++;
        }
    }
