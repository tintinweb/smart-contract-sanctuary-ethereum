...
contract LendLocalToken is Ownable, ReentrancyGuard {
    ...
    uint256 public MIN_ETH = 1000000000000000;
    uint256 public COMMISION_FEENUM = 300; // Commission Fee 3%
    ...
    function loanToken(
        ...
        uint256 lendTokenAmount = _lendTokenAmount;
        require(
            isCollateralToken(_collateralToken),
        ...
        uint256 colTokenAmountNeeded = countCollateralAmountFromLend(collateralToken, lendToken, limit, lendTokenAmount);
        uint256 balance = IERC20(collateralToken).balanceOf(msg.sender);
        require(balance > colTokenAmountNeeded, "loanToken: not enough collateral token balance on your wallet");

        uint256 beforeBalance = IERC20(collateralToken).balanceOf(
            address(this)
        );
        require(
            TokenTransfer(msg.sender, collateralToken, colTokenAmountNeeded),
        ...
        uint256 lendTokenAmountReal = countLendAmountFromCollateral(collateralToken, lendToken, limit, colTokenAmountReal);
    ...
    function loanEther(address _lendToken, uint256 _lendTokenAmount) public payable nonReentrant {
        address lendToken = _lendToken;
        uint256 lendTokenAmount = _lendTokenAmount;
        require(isCollateralToken(ETHER), "loanEther: No collateral token");
        bool bIsListedPool = isListedPool(ETHER, _lendToken);
        require(bIsListedPool, "loanEther: Loan Pool is not exist");
        require(
        ...
        uint256 etherAmountNeeded = countCollateralAmountFromLend(ETHER, lendToken, limit, lendTokenAmount);
        uint256 beforeETHBalance = address(this).balance;
        require(msg.value >= etherAmountNeeded, "loanEther: no enough ETHER on wallet");
        uint256 afterETHBalance = address(this).balance;
        uint256 ethAmountReal = afterETHBalance - beforeETHBalance;
        uint256 lendTokenAmountReal = countLendAmountFromCollateral(ETHER, lendToken, limit, lendTokenAmount);
        bool isOldUser = false;
        for (uint256 i = 0; i < loanUsers.length; i++) {
            if (loanUsers[i] == msg.sender) {
                isOldUser = true;
        ...
        require(
            checkLoanEthLimit(lendToken, lendTokenAmountReal),
            "loanToken: not enough liquidity or can't borrow over total balance in contract"
        );
        LoanRequest memory newLoan;
        newLoan.borrower = msg.sender;
        ...
        loans[msg.sender][userLoansCount[msg.sender]] = newLoan;
        userLoansCount[msg.sender]++;
        IERC20(lendToken).transfer(msg.sender, lendTokenAmountReal);
    }
