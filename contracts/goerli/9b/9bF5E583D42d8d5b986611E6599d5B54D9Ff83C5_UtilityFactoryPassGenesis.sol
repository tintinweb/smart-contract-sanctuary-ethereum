...
contract UtilityFactoryPassGenesis is Ownable, ERC721A, ReentrancyGuard {
    ...
    constructor() ERC721A("UtilityFactoryPassSeason1", "UFPass1", 10, 1000) {
        typeSupply[TokenType.SILVER] = 300;
        typeSupply[TokenType.GOLD] = 600;

        typePrice[TokenType.SILVER] = 0.003 ether;
        typePrice[TokenType.GOLD] = 0.009 ether;
    ...
    modifier checkSponsor(string calldata sponsorCode, TokenType passType) {
        ...
        if (sponsorWallet[sponsorCode] != address(0)) {
            ...
            uint256 amount = typePrice[passType];
    ...
    function _mint(TokenType passType, uint256 quantity) internal {
        ...
        require(
            typePrice[passType] * quantity == msg.value,
    ...
    function freeMint(uint256 quantity) external payable onlyOwner {
    ...
    // Accessor :
    function getTokenInfos(uint tokenId) public view returns (TokenType, uint, uint) {

        return (tokenType[tokenId], tokenActivationDate[tokenId], tokenEndValidityDate[tokenId]);
    }
