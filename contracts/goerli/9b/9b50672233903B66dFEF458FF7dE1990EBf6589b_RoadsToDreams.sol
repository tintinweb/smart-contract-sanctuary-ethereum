...
contract RoadsToDreams is ERC721AQueryable, Ownable, TimeLock, WhitelistAble {
  ...
  function stats() public view virtual returns (uint256[] memory) {
    ...
    res[10] = uint256(getWhitelistWave());
    return res;
  }
  ...
  function setWhitelistMintData(uint256 whitelistStart, uint256 whitelistEnd, uint16 maxWhitelistNftMintsPerWallet, uint16 whitelistWave) public onlyOwner {
    _setWhitelistData(whitelistStart, whitelistEnd, maxWhitelistNftMintsPerWallet, whitelistWave);
  }
  ...
  function mintWhitelist(uint16 amount, bytes calldata signature)
  public
  payable
  {
    _checkAndFlagWhitelist(msg.sender, signature, _allowListSigner, amount);
    _userMint(msg.sender, amount);
  }
...
abstract contract WhitelistAble {
  ...
  /// @dev The current wave of the whitelist
  uint16 private _whitelistWave;

  /// @notice Set whitelist wave data
  function _setWhitelistData(uint256 whitelistStart, uint256 whitelistEnd, uint16 maxWhitelistNftMintsPerWallet, uint16 whitelistWave) internal {
    _whitelistStart = whitelistStart;
    _whitelistEnd = whitelistEnd;
    _maxWhitelistNftMintsPerWallet = maxWhitelistNftMintsPerWallet;
    _whitelistWave = whitelistWave;
  }
  ...
  function getWhitelistWave() public view returns (uint16) {
    return _whitelistWave;
  }
  ...
  function _checkAndFlagWhitelist(address minter, bytes calldata signature, address allowListSigner, uint16 amount) internal {
    ...
    require(recoverWhitelistSigner(msg.sender, signature, _whitelistWave) == allowListSigner, "W1");
    _whitelistClaimed[minter] += amount;
  }

  /// @dev Returns the signer of the allow message
  function recoverWhitelistSigner(address minter, bytes calldata signature, uint16 whitelistWave) private view returns (address) {
    return keccak256(abi.encodePacked(minter, address(this), whitelistWave)).toEthSignedMessageHash().recover(signature);
  }
}
