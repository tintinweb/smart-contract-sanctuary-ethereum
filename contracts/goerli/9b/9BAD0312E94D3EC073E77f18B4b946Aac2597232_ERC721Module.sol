...
contract ERC721Module is IModule, CoreStorage, Executor {
    ...
    function handleTransaction(address to, bytes memory data)
        external
        payable
        override
        onlyOwner
    ...
    {
        {
            uint256 tokenId = _parseTokenId(data);
            require(!_getLocker()[to][tokenId], "Can not perform this action on locked token.");
        }

        return _execute(to, msg.value, data);
    }
    ...
    function setLockedERC721Token(
        ...
        _getLocker()[collection][tokenId] = isLocked;
        emit SetLockedERC721Token(collection, tokenId, isLocked);
    }

    /**
     * @notice Parse the tokenId from given data.
     * The method returns boolean representing is tokenId found or not and tokenId (0 if not found).
     * @param data Any data to be parsed, mostly calldata of transaction.
     */
    function _parseTokenId(bytes memory data) internal pure returns (uint256) {
        ...
        if (
            ...
            return Utils.getUint256At(data, 0x64);
        }
        revert("Could not parse token id.");
    }

    // TODO try with contract and uses;
    function _getLocker() internal pure returns (mapping(address => mapping(uint256 => bool)) storage result) {
        assembly {
