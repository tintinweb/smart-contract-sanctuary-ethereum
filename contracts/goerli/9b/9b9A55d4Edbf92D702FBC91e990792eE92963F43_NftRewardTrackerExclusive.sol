...
// https://goerli.etherscan.io/address/0x7C06F54167c09dEF0B3fF3e80D0d98ceeafC3B6a
contract NftRewardTracker {
    mapping(address => mapping(uint => bool)) private claimed;
    mapping(address => uint) public shippingFee;
    ...
    function claim(address nftContract, uint[] memory nftIds) public payable {
        require(
            msg.value >= shippingFee[nftContract],
            "You must pay the shipping fee!!!"
        );
        ...
        for (uint i = 0; i < nftIds.length; i++) {
            uint nftId = nftIds[i];
            require(!claimed[nftContract][nftId], "NFT already claimed");
            require(
                nft.ownerOf(nftId) == msg.sender,
                "Caller is not the owner of the NFT"
            );
            claimed[nftContract][nftId] = true;
        }

        emit NFTsClaimed(msg.sender, nftContract, nftIds);
    }
    ...
    function isClaimed(
        ...
        return claimed[nftContract][nftId];
    }
    ...
    function setShippingFee(
        ...
        require(_shippingFee != shippingFee[nftContract], "Already set fee");
        shippingFee[nftContract] = _shippingFee;
    }
