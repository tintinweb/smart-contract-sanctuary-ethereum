...
contract GDCCEthCrossChain is Beneficiary, Pausable, ReentrancyGuard {
    ...
    function tokenDeposit(
        ...
        SafeERC20.safeTransferFrom(IERC20(_token), _msgSender(), address(this), _amount);
        SafeERC20.safeTransfer(IERC20(_token), beneficiary(), feeAmount);
    ...
    function failSafeForEther(address _user, uint256 _amount) external onlyOwner onlyValidAddr(_user) onlyValidAmount(_amount) {
        if (address(this).balance < _amount) revert Insufficient_Contract_Balance();
