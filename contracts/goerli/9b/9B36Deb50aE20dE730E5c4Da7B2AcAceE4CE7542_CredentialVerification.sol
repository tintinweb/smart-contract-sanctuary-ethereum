...
contract CredentialVerification is AccessControl {
    ...
    function getCredentialsByHolder(
        ...
        // require(
        //     msg.sender == _holder || hasRole(ISSUER_ROLE, _msgSender()),
        //     "Only holder is allow to access this function!"
        // );
    ...
    function getAccessToken(bytes32 _credentialHash) public returns (bytes32) {
        require(
            msg.sender == credentials[_credentialHash].holder,
            "You are not allowed to get the access token for this credential!"
        );
        if (
            credentials[_credentialHash].accessTokenValidUntil < block.timestamp
        ) {
            generateAccessToken(_credentialHash);
        }
        return credentials[_credentialHash].accessToken;
    }

    function generateAccessToken(bytes32 _credentialHash) private {
