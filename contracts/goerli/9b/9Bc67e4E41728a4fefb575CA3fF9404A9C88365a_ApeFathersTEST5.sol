...
contract ApeFathersTEST5 is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981 {
    ...
    */
    ...
    struct TokenApproval {
        ...
        // Map to keep track of the owner of each token
    ...
    function burnClaim(uint256[] memory _tokenIds) external {
        ...
        IERC721A ContractOfTokensToBurn = IERC721A(0x7d83f206DE2E16564e5EFD4332a755c9F4615AC1);
        require(address(ContractOfTokensToBurn) != address(0), "INVALID CONTRACT ADDRESS");

        uint256 numberOfTokens = _tokenIds.length;
        assert(numberOfTokens >= 1);

        // Verify that the intended recipient is the token owner for all tokens
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
            address tokenOwner = ownerOf(tokenId);
            require(ContractOfTokensToBurn.ownerOf(tokenId) == address(_msgSender()), "INVALID ADDRESS");
            require(tokenOwner == _msgSender(), "ONLY THE TOKEN OWNER CAN BURN THEIR TOKENS");
            require(address(_msgSender()) == ownerOf(tokenId), "INVALID OWNER ADDRESS");
            require(ContractOfTokensToBurn.getApproved(tokenId) == address(_msgSender()), "INVALID CONTRACT ADDRESS");
        }

        // Approve all tokens at once for this contract and the intended recipient
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
        ...
        // Verify that the burn contract is approved for all tokens
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
            require(tokenApprovals[tokenId].transferApprovals[address(this)], "BURN CONTRACT NOT APPROVED FOR BURNING ONE OR MORE OF THE TOKENS");
        }
        emit ApprovalComplete(_msgSender(), _tokenIds);

        // Burn the token(s) and mint a new Token from this contract "ApeFathers"
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
            uint256 newTokenId = totalSupply() + 1;
            this.transferFrom(ownerOf(tokenId), address(0), tokenId);
            super._safeMint(_msgSender(), newTokenId);
            }
        // Clear the approval status for the burned tokens
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            delete tokenApprovals[_tokenIds[i]];
        }
    }
