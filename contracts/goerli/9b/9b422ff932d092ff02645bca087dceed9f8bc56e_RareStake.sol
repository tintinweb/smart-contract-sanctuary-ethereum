...
interface IRareStake {
  ...
  /// @param account Address to look up the balance for.
  /// @return uint256 The amount of tokens owned by `account`.
  function balanceOf(address account) external view returns (uint256);
...
contract RareStake is IRareStake, ERC20SnapshotUpgradeable, ReentrancyGuard {
  ...
  function unstake(uint256 _amount) external nonReentrant updatePeriod {
    (, uint256 amtStaked) = amountStakedByUser.tryGet(msg.sender);
    uint256 amountRare = calculateSaleReturn(balanceOf(msg.sender), amtStaked, _amount);
  ...
  function calculatePurchaseReturn(uint256 _totalSRare, uint256 _stakedAmount) public pure returns (uint256) {
    uint256 sqrtReturn = sqrt((2 * _stakedAmount) + _totalSRare**2);
    return 10**19 * (sqrtReturn - _totalSRare);
  }
  ...
  /// @inheritdoc IRareStake
  function totalSupply() public view virtual override(IRareStake, ERC20Upgradeable) returns (uint256) {
    return ERC20Upgradeable.totalSupply();
  }

  /// @inheritdoc IRareStake
  function balanceOf(address account) public view override(IRareStake, ERC20Upgradeable) returns (uint256) {
