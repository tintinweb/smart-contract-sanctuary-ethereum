  ...
  EIP712Whitelisting
{
  ...
  function initialize(
    ...
    __AccessControl_init();
    __EIP712Whitelisting_init(platform, tokenName, domainVersion);

    _setupRole(DEFAULT_ADMIN_ROLE, platform);
    _setupRole(MINTER_ROLE, platform);
  ...
  function adminMint(
    ...
    emit NftMinted(ID, tokenURL, recipient);
  }

  function minterMintWithoutUri() external onlyRole(MINTER_ROLE) {
    uint256 ID = _tokenIds.current();
    _tokenIds.increment();

    _safeMint(msg.sender, ID);
    emit NftMinted(ID, _baseURI(), msg.sender);
  }

  function minterMintWithoutUriWithSignature(
  ...
  function minterMint(string calldata tokenURL) external onlyRole(MINTER_ROLE) {
    uint256 ID = _tokenIds.current();
    _tokenIds.increment();
  ...
  function minterMintWithSignature(
    string calldata tokenURL,
    bytes calldata signature
  ) external requiresWhitelist(signature) {
    uint256 ID = _tokenIds.current();
