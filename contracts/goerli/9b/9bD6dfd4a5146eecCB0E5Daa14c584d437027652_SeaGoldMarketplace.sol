...
contract SeaGoldMarketplace is Ownable {
    ...
    function updateRewardCalcAddress(address _rewardCalcAddress) public onlyOwner {
        RewardCalcAddress = _rewardCalcAddress;
    }
    ...
    function acceptBId(
        ...
        if (SeaGoldLaunched) {
        (sellerRewards, buyerRewards) = rewardCalc.getRewardsAL(salestruct.amount);
        }
        else {
            (sellerRewards, buyerRewards) = rewardCalc.getRewardsBL(salestruct.amount);
        }
    ...
    function buyToken(
        ...
        for (uint256 i = 0; i < from.length; i++) {
                ...
                require(goldfee + roy + netamount <= salestruct.amount, "Amount is not equal");
                ...
                if (roy > 0) {
                    RoyaltyRegistry royReg = RoyaltyRegistry(royaltyRegistry);
                    (, royinf.royAddress, ,) = royReg.getRoyaltyInfo(salestruct.conAddr);
    ...
    {
        RoyaltyRegistry royReg = RoyaltyRegistry(royaltyRegistry);
        (royinf.status, , royinf.royPercentage, ) = royReg
            .getRoyaltyInfo(_contractAddress);

        if (royinf.status) {
            salestruct.royaltyPer = royinf.royPercentage * 1e18;
            }
        else {
