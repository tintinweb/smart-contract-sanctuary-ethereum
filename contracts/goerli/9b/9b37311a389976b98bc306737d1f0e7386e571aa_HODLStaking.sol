...
contract HODLStaking is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    struct UserLockInfo {
        uint256 totalActualDeposit;
        uint256 depositTime;
    }

    // The REWARD TOKEN!
    IERC20 public reward;
    //Percentage distributed per day. 2 decimals
    ...
    function canWithdraw(uint8 _lid, address _user) public view returns (bool) {
        return (block.timestamp >= userLockInfo[_lid][_user].depositTime + poolLockInfo[_lid].lockPeriodInSeconds);
    }
    ...
    function deposit(
        ...
        if (pool.isInputNFT == 1) {
            ...
            if (_pid == 0) {
                ...
                UserLockInfo storage userLock = userLockInfo[_lid][_benificiary];

                require(poolLock.multi > 0, "HODL: Invalid lock id");

                userLock.depositTime = block.timestamp;
                userLock.totalActualDeposit += amount;
    ...
    function withdraw(
        ...
        if (pool.isInputNFT == 1) {
            ...
            if (_pid == 0) {
                PoolLockInfo storage poolLock = poolLockInfo[_lid];
                UserLockInfo storage userLock = userLockInfo[_lid][msg.sender];

                require(userLock.totalActualDeposit >= amount, "HODL: Amount exceeds balance");
                uint256 weightedAmount = (amount * poolLock.multi) / DIVISOR;
                user.totalDeposit -= weightedAmount;
                pool.totalDeposit -= weightedAmount;
                ...
                if (canWithdraw(_lid, msg.sender)) {
