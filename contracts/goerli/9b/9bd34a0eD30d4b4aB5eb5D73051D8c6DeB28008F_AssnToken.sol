...
/// @notice Governance token of the Assassin Protocol
contract AssnToken is ERC20, Ownable {
    ...
    /// @notice Creates `_amount` token to owner. Must only be called by the owner.
    function mint(uint256 amount_) external onlyOwner {
        _mint(_msgSender(), amount_);
        _moveDelegates(address(0), _delegates[_msgSender()], amount_);
    }
