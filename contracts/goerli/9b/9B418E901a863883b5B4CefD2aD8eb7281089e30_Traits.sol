...
contract Traits is ERC1155LazyMint {
    ERC1155Base public immutable lootbox;
    uint256 public lootboxRequirement = 1;
    ...
    constructor(
        ...
        lootbox = ERC1155Base(_lootboxAddress);
    }

    // function verifyClaim(address _claimer, uint256 _tokenId, uint256 _quantity) public view virtual override {
    //     uint256 _traitsQuantity = lootboxRequirement * _quantity;
    //     uint256 _lootboxBalance = lootbox.balanceOf(_claimer, _tokenId);
    //     require(_lootboxBalance >= _traitsQuantity, 'You do not have enough Lootboxes to make that transaction.');
    // }

    function claim(address _claimer, uint256 _tokenId, uint256 _quantity) public payable virtual override {
        lootbox.burn(_claimer, _tokenId, _quantity);
        super.claim(_claimer, _tokenId, _quantity);
    }
}