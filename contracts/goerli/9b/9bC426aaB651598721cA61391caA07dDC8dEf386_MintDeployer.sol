...
// OpenZeppelin Contrats v4.4.0 (utils/introspection/IERC165.sol)
...
contract MintDeployer {
  ...
  uint256 public createContractPrice = 0.01 ether;   //= 0 ether; //60000000000000000; // 0.06 eth
  ...
  function SetContractCreationPrice(uint256 price) public restricted {
    createContractPrice = price;
  }

  function DeployBasicMint(uint256 mintPrice, address payable contractOwner) external payable {
    require(msg.value == createContractPrice, "Not the good price");

    Mint mint = new Mint(mintPrice, contractOwner);
    userToMint[msg.sender].push(mint);
  }
...
contract Mint is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 mintPrice, address payable contractOwner) ERC721("CryptoLienzNFT", "CLNFT") {
      price = mintPrice;
