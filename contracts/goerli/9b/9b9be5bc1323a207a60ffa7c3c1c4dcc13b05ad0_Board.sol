...
// who is ownable;
//only DAO can change fee;
//to check who is owner
...
contract Board is Initializable, UUPSUpgradeable, OwnableUpgradeable, PriceConsumerV3 {
    ...
    function changeFee(uint256 _fee) external onlyDAO {
        fee = _fee;

        emit FeeChanged (fee, _fee);  //////////how to inpout old fee??? may be event in start of function code?
    }
    ...
    function publishTask(string calldata _taskName, string calldata _linkAtTaskDescription) external payable {
        ...
        emit PublishTask (arrayTasks.length, _taskName, arrayTasks[arrayTasks.length-1].price); ///////////////////////check at goerli
    }
