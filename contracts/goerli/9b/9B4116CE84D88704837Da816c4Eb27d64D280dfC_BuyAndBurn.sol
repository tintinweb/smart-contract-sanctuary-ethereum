...
contract BuyAndBurn is ReentrancyGuard {
    ...
    // Constants
    address public constant SWAPROUTER_ADDRESS = 0xE592427A0AEce92De3Edee1F18E0157C05861564;
    ...
    constructor(
        ...
        address _plsbPool = IUniswapV3Factory(_factory).getPool(_carnAddress, _plsbAddress, _poolFee);
        address _plsdPool = IUniswapV3Factory(_factory).getPool(_carnAddress, _plsdAddress, _poolFee);
        address _asicPool = IUniswapV3Factory(_factory).getPool(_carnAddress, _asicAddress, _poolFee);
    ...
    function processable(Ops op) public view returns (uint256) {
        uint256 totalReceived = carnContract.balanceOf(address(this)) + totalProcessed();
    ...
    function _pendingProcessing(Ops op, uint256 totalReceived, uint256 alreadyProcessed) private view returns (uint256) {
    ...
    function processCarn(Ops op) external nonReentrant {
        require(_shares[op] > 0, "Op has no shares");

        uint128 amount = uint128(processable(op));
        require(amount != 0, "No dues for this op");

        _carnTotalProcessed += amount;
        unchecked {
            _carnProcessed[op] += amount;
        }

        uint128 callerReward = (amount * 625) / 1000000;
        amount -= callerReward;
        carnContract.safeTransfer(msg.sender, callerReward);
        carnContract.approve(SWAPROUTER_ADDRESS, amount);

        // process op
        if (op == Ops.BURN) {
            carnContract.safeTransfer(DEAD_ADDRESS, amount);
        } else if (op == Ops.SENDTOPLSBNFTREWARDS) {
            carnContract.safeTransfer(plsbNftRewards, amount);
        } else if (op == Ops.BUYPLSDFORWAATCA) {
            uint256 amountReceived = buyPLSD(amount);
            IERC20(PLSD).safeTransfer(waatca, amountReceived);
        } else if (op == Ops.BUYPLSBFORWAATCA) {
            uint256 amountReceived = buyPLSB(amount);
            IERC20(PLSB).safeTransfer(waatca, amountReceived);
        } else if (op == Ops.BUYASICFORWAATCA) {
            uint256 amountReceived = buyASIC(amount);
