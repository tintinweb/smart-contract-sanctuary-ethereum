...
contract ArtDubaiAlexisChristodoulou is ArtDubai {
	IERC721Enumerable private constant KEYPASS_CONTRACT =
		IERC721Enumerable(0x6812f8FCeBe860C9231b210F1036A43DA98527af);
	IERC721Enumerable private constant ALEXIS_ART_CONTRACT =
		IERC721Enumerable(0x3625578aAed73576e492068DBD9746b760E7Bbb7);

	uint8 private immutable i_daytimeRewardTokenId;
	uint8 private immutable i_nighttimeRewardTokenId;

	mapping(uint256 => bool) private s_usedKeypassIds;
	mapping(uint256 => bool) private s_usedAlexisArtIds;
	...
	function mintRewardTokens() external nonReentrant {
		...
		uint256 keypassBalance = KEYPASS_CONTRACT.balanceOf(msg.sender);
		uint256 alexisArtBalance = ALEXIS_ART_CONTRACT.balanceOf(msg.sender);

		uint256 quantity = 0;

		for (uint256 i = 0; i < keypassBalance; i++) {
			uint256 tokenId = KEYPASS_CONTRACT.tokenOfOwnerByIndex(msg.sender, i);
			if (s_usedKeypassIds[tokenId]) continue;

			s_usedKeypassIds[tokenId] = true;
			quantity++;
		}

		for (uint256 i = 0; i < alexisArtBalance; i++) {
			uint256 tokenId = ALEXIS_ART_CONTRACT.tokenOfOwnerByIndex(msg.sender, i);
			if (s_usedAlexisArtIds[tokenId]) continue;

			s_usedAlexisArtIds[tokenId] = true;
			quantity++;
		}
