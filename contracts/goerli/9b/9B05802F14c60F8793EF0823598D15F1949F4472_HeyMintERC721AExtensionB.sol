...
import { BaseConfig, BurnToken, LaunchpadStorage } from '../libraries/LaunchpadStorage.sol';
...
contract HeyMintERC721AExtensionB is
    ERC721AUpgradeable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ...
    RevokableOperatorFiltererUpgradeable
{
    ...
    */
    function setDepositClaimState(bool _depositClaimActive) external onlyOwner {
        require(LaunchpadStorage.state().advCfg.depositClaimActive != _depositClaimActive, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
        if (_depositClaimActive) {
            require(LaunchpadStorage.state().advCfg.depositMerkleRoot != bytes32(0), "MUST_SET_MERKLE_ROOT");
            require(LaunchpadStorage.state().advCfg.depositContractAddress != address(0), "DEPOSIT_CONTRACT_ADDRESS_NOT_SET");
        }
        LaunchpadStorage.state().advCfg.depositClaimActive = _depositClaimActive;
    }
    ...
    */
    function setDepositMerkleRoot(bytes32 _depositMerkleRoot) external onlyOwner {
        LaunchpadStorage.state().advCfg.depositMerkleRoot = _depositMerkleRoot;
    }
     ...
     */
    function setDepositContractAddress(address _depositContractAddress) external onlyOwner {
        LaunchpadStorage.state().advCfg.depositContractAddress = _depositContractAddress;
    }
    ...
    */
    function setRemainingDepositPayment(uint16 _remainingDepositPayment) external onlyOwner {
        LaunchpadStorage.state().advCfg.remainingDepositPayment = _remainingDepositPayment;
    }
    ...
    */
    function burnDepositTokensToMint(uint256[] calldata _tokenIds, bytes32[][] calldata _merkleProofs) external nonReentrant payable originalUser {
        require(LaunchpadStorage.state().advCfg.depositClaimActive, "DEPOSIT_CLAIM_NOT_ACTIVE");
        uint256 numberOfTokens = _tokenIds.length;
        require(numberOfTokens > 0, "NO_TOKEN_IDS_PROVIDED");
        require(numberOfTokens == _merkleProofs.length, "ARRAY_LENGTHS_MUST_MATCH");
        require(totalSupply() + numberOfTokens <= LaunchpadStorage.state().cfg.maxSupply, "MAX_SUPPLY_EXCEEDED");
        require(msg.value == (LaunchpadStorage.state().advCfg.remainingDepositPayment * 10**16) * numberOfTokens, "INCORRECT_REMAINING_PAYMENT");
        IERC721 DepositContract = IERC721(LaunchpadStorage.state().advCfg.depositContractAddress);
        for (uint256 i = 0; i < numberOfTokens; i++) {
            require(MerkleProofUpgradeable.verify(_merkleProofs[i], LaunchpadStorage.state().advCfg.depositMerkleRoot, keccak256(abi.encodePacked(_tokenIds[i]))), "INVALID_MERKLE_PROOF");
            require(DepositContract.ownerOf(_tokenIds[i]) == msg.sender, "MUST_OWN_TOKEN");
    ...
    function determineFundingSuccess() external {
        require(LaunchpadStorage.state().cfg.fundingDuration > 0, 'NOT_ENABLED');
        require(
            address(this).balance >
                LaunchpadStorage.state().cfg.fundingTarget * 10**16,
            'FUNDING_TARGET_NOT_MET'
        );
        require(
            !LaunchpadStorage.state().data.fundingSuccessDetermined,
            'SUCCESS_ALREADY_DETERMINED'
        );
        LaunchpadStorage.state().data.fundingTargetReached = true;
        LaunchpadStorage.state().data.fundingSuccessDetermined = true;
    }
    ...
    function burnToRefund(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        require(
            block.timestamp >=
                (LaunchpadStorage.state().data.contractDeployTime +
                    ((LaunchpadStorage.state().cfg.fundingDuration + 1) *
                        86400)),
            'FUNDING_PERIOD_STILL_ACTIVE'
        );
        require(
            !LaunchpadStorage.state().data.fundingTargetReached,
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                LaunchpadStorage.state().data.pricePaid[_tokenIds[i]] > 0,
            ...
            totalRefund += LaunchpadStorage.state().data.pricePaid[
                _tokenIds[i]
            ];
        }
    ...
    function setLoaningActive(bool _loaningActive) external onlyOwner {
        LaunchpadStorage.state().advCfg.loaningActive = _loaningActive;
    }
    ...
    function loan(uint256 _tokenId, address _receiver) external nonReentrant {
        require(
            LaunchpadStorage.state().data.tokenOwnersOnLoan[_tokenId] ==
                address(0),
            'CANNOT_LOAN_BORROWED_TOKEN'
        );
        require(LaunchpadStorage.state().advCfg.loaningActive, 'NOT_ACTIVE');
        ...
        LaunchpadStorage.state().data.tokenOwnersOnLoan[_tokenId] = msg.sender;
        // Add to the owner's loan balance
        LaunchpadStorage.state().data.totalLoanedPerAddress[msg.sender] += 1;
        LaunchpadStorage.state().data.currentLoanIndex += 1;
        // Transfer the token
    ...
    function retrieveLoan(uint256 _tokenId) external nonReentrant {
        ...
        delete LaunchpadStorage.state().data.tokenOwnersOnLoan[_tokenId];
    ...
    function adminRetrieveLoan(uint256 _tokenId) external onlyOwner {
        address borrowerAddress = ownerOf(_tokenId);
        address loanerAddress = LaunchpadStorage.state().data.tokenOwnersOnLoan[
            _tokenId
        ];
        ...
        delete LaunchpadStorage.state().data.tokenOwnersOnLoan[_tokenId];
        // Subtract from the owner's loan balance
        LaunchpadStorage.state().data.totalLoanedPerAddress[loanerAddress] -= 1;
        LaunchpadStorage.state().data.currentLoanIndex -= 1;
        // Transfer the token back
        _directApproveMsgSenderFor(_tokenId);
    ...
    function totalLoaned() public view returns (uint256) {
        return LaunchpadStorage.state().data.currentLoanIndex;
    }
    ...
    function loanedBalanceOf(address _owner) public view returns (uint256) {
        return LaunchpadStorage.state().data.totalLoanedPerAddress[_owner];
    }
        ...
        returns (uint256[] memory)
    {
        ...
        for (
            ...
            if (LaunchpadStorage.state().data.tokenOwnersOnLoan[i] == _owner) {
    ...
    function refundGuaranteeActive() public view returns (bool) {
        if (LaunchpadStorage.state().cfg.refundDuration == 0) {
            return false;
        }
        return (block.timestamp < LaunchpadStorage.state().data.contractDeployTime + (LaunchpadStorage.state().cfg.refundDuration * 86400));
    }
    ...
    function setRefundAddress(address _refundAddress) external onlyOwner {
      require(_refundAddress != address(0), "CANNOT_SEND_NFTS_TO_ZERO_ADDRESS");
      LaunchpadStorage.state().addrCfg.refundAddress = _refundAddress;
    }

    /**
     * @notice Increase the period of time where token holders can still return their tokens for a refund
     * @param _refundDurationIncrease The amount of days to increase the refund period by
     */
    function increaseRefundDuration(uint8 _refundDurationIncrease) external onlyOwner {
      LaunchpadStorage.state().cfg.refundDuration += _refundDurationIncrease;
    }
    ...
    function refund(uint256 _tokenId) external nonReentrant {
        ...
        if (LaunchpadStorage.state().cfg.fundingDuration > 0) {
            delete LaunchpadStorage.state().data.pricePaid[_tokenId];
        }

        address addressToSendToken = LaunchpadStorage.state().addrCfg.refundAddress != address(0) ? LaunchpadStorage.state().addrCfg.refundAddress : owner();

        safeTransferFrom(msg.sender, addressToSendToken, _tokenId);

        payable(msg.sender).transfer(LaunchpadStorage.state().cfg.refundPrice * 10**16);
    }
     ...
     * @param tokenId The token ID to generate a random hash for
     */
    function _generateRandomHash(uint256 tokenId) internal {
        if (LaunchpadStorage.state().data.randomHashStore[tokenId] == bytes32(0)) {
            LaunchpadStorage.state().data.randomHashStore[tokenId] = keccak256(
                abi.encode(
                    block.timestamp,
                    block.difficulty,
     ...
     */
    function setGenerateRandomHashState(bool _randomHashActive)
        external
        onlyOwner
    {
        require(
            LaunchpadStorage.state().cfg.randomHashActive != _randomHashActive,
    ...
    // Must include on all facets where token transfers occur so these checks are always enforced
    ...
    function _beforeTokenTransfers(
        ...
        uint256 tokenId,
        uint256 quantity
    ) internal override(ERC721AUpgradeable) whenNotPaused onlyAllowedOperator(from) {
        require(
            !LaunchpadStorage.state().advCfg.stakingActive ||
                (LaunchpadStorage.state().data.currentTimeStaked[tokenId] ==
                    0 ||
                    LaunchpadStorage.state().data.stakingTransferActive),
            'TOKEN_IS_STAKED'
        );
        require(
            LaunchpadStorage.state().data.tokenOwnersOnLoan[tokenId] ==
                address(0),
            'CANNOT_TRANSFER_LOANED_TOKEN'
        );
        if (
            LaunchpadStorage.state().cfg.soulbindingActive &&
            !LaunchpadStorage.state().data.soulboundAdminTransferInProgress
        ) {
            require(from == address(0), 'TOKEN_IS_SOULBOUND');
        }
        if (LaunchpadStorage.state().cfg.randomHashActive && from == address(0)) {
...
struct BaseConfig {
    ...
    // If set, amount of days from contract deploy during which tokens are refundable for refundPrice
    ...
    uint16 publicPrice;
    // The price of a token in the presale in centiETH
    uint16 presalePrice;
    // The amount returned to a user in a token refund in centiETH
    uint16 refundPrice;
...
struct Data {
    ...
    bool blocklistPermanentlyDisabled;
    // ============ CONDITIONAL FUNDING ============
