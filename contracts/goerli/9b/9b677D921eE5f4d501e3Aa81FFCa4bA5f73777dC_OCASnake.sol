...
// @@@@@@@@@@@@@@@@@@@@@&[email protected]@@@@@@@&PPPPPPPP[email protected]@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
// @@@@@@@@@@@@@@@@@@@@@@&&&&&&Y?????B&&&@@@@@@@@@&&&PPPPP5J???777775&&&@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    function returnName(uint256 tokenId, uint256 seed)
        public
        view
        returns (string memory)
    {
        ...
        string memory snakeName = string(
            abi.encodePacked(
    ...
    function recursiveNameCheck(uint256 tokenId, uint256 seed)
        public
        view
        returns (string memory)
    {
        //for through all IDS until this one to see if they match as well
        string[1997] memory foundNames;
        string memory foundName;
        for (uint256 i = 0; i < tokenId; i++) {
            string memory snakeName = returnName(i, seed);

            foundNames[i] = snakeName;

            ///end of loop check if name is already in use
            if (i == tokenId) {
                for (uint256 j = 0; j < foundNames.length; j++) {
                    if (
                        keccak256(abi.encodePacked(foundNames[j])) ==
                        keccak256(abi.encodePacked((snakeName)))
                    ) {
                        foundName = snakeName;
                    } else {
                        return recursiveNameCheck(tokenId, 69 * 2);
                    }
                }
            }
        }
    ...
    function registerName(bytes32 name, uint256 tokenId) internal {
    ...
    function snakeNameSave(uint256 tokenId, bool extraName) private {
        ...
        if (isSet(name)) return snakeNameSave(tokenId + block.timestamp, true);
        else {
            registerName(name, tokenId);
        }
    }
