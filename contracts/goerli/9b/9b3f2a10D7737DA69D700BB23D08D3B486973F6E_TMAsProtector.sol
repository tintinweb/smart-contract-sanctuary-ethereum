...
contract TMAsProtector is AccessControl {
    ...
    /**  @dev ステーキング中のトークンをProtectorからアンロックしてしまわないようにここでも状態管理する。加えてAMTの割り当てにも利用する。 */
    // collectionAddress => block.timestamp list
    mapping(address => TimestampStorage.Storage) private _lockTimestamp;

    uint256 public rewardUnit = 1;
    ...
    function lock(NFT.TokenStruct[] memory tokens) external {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            require(
                _lockTimestamp[tokens[i].collectionAddress].get(
                    tokens[i].tokenId
                ) == 0,
                "the token is already protected."
            );
        }

        for (uint256 i = 0; i < tokens.length; i++) {
            _lockTimestamp[tokens[i].collectionAddress].set(
                tokens[i].tokenId,
                current
            );
        }
    ...
    function unlock(NFT.TokenStruct[] memory tokens) external {
        ...
        uint256 amt;
        uint256 _rewardUnit = rewardUnit;
        uint256 currentTimestamp = block.timestamp;
        for (uint256 i = 0; i < tokens.length; i++) {
            uint256 lockTimestamp = _lockTimestamp[tokens[i].collectionAddress]
                .get(tokens[i].tokenId);
            require(lockTimestamp != 0, "the token is not protected.");

            amt += ((currentTimestamp - lockTimestamp) / 1 days) * _rewardUnit;
        }
        ...
        if (amt > 0) {
            address holder = IERC721(tokens[0].collectionAddress).ownerOf(
                tokens[0].tokenId
            );

            rewarder.reward(holder, amt);
        }
    }
    ...
    function isProtected(
        ...
        return _lockTimestamp[collectionAddress].get(tokenId) != 0;
    }

    function getProtectTimestamp(
        address collectionAddress,
        uint256 tokenId
    ) external view returns (uint256) {
        return _lockTimestamp[collectionAddress].get(tokenId);
    }
    ...
    function setRewardUnit(uint256 value) external onlyRole(ADMIN) {
        rewardUnit = value;
    }
}
