...
// Closes 1 days/24hrs after deployment [see timestamp logic]
...
contract Sphinx is Ownable {
  ...
  function entry(bytes32 answer) external payable callerIsUser {
    ...
    if (answer.length > 0) {
      emit Receipt("your transaction hash is:");
    }else{
      emit Loss("something went wrong");
    }
  ...
  function reveal(
    ...
    string memory secretSalt) external onlyOwner {
