...
error RiggedRoll_WithdrawAmountIsHigherThanBalance();
error RiggedRoll_NoEnoughETH();
error RiggedRoll_Return();
...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address receiver, uint256 amount) public onlyOwner {
        if (amount > address(this).balance) {
            revert RiggedRoll_WithdrawAmountIsHigherThanBalance();
        }
    ...
    function riggedRoll() public {
        uint256 requiredBalance = 0.002 ether;
        if (address(this).balance < requiredBalance) {
            revert RiggedRoll_NoEnoughETH();
        }
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(
