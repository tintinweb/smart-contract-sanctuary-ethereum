...
contract FungibleSBTFactory is Ownable, AccessControl {
    ...
    function create() external {
        FungibleSBT collection = new FungibleSBT(msg.sender, address(seller));
...
contract TicketSeller is ReentrancyGuard {
    ...
    function buy(address collection, uint256 id) external payable nonReentrant {
        require(msg.value >= cost[collection][id], "not enough eth.");

        IFungibleSBT(collection).mint(msg.sender, id);
        payable(IFungibleSBT(collection).owner()).sendValue(msg.value);

        // TODO: operation fee

        emit Bought(address(collection), id, msg.sender);
    }
...
contract FungibleSBT is IFungibleSBT, ERC1155, Ownable, AccessControl, TokenUriSupplier {
    using Strings for uint256;

    bytes32 public ADMIN = "ADMIN";
    bytes32 public MINTER = "MINTER";

    constructor(address _owner, address _minter) ERC1155("") {
        _grantRole(ADMIN, _owner);
        _grantRole(ADMIN, _minter);
    ...
    function owner() public override(Ownable, IFungibleSBT) view returns(address) {
