...
contract Payment is AccessControlUpgradeable, UUPSUpgradeable, IPayment {
    ...
    function setPaymentTokens(
        ...
        for (uint256 i = 0; i < tokens_.length; i++) {
            ...
            address router_ = paymentTokenSwapInfo[token_].router;
            uint256 allowance_ = IERC20(token_).allowance(address(this), router_);

            if (statuses_[i]) {
                IERC20(token_).safeIncreaseAllowance(
                    address(swapInfos_[i].router),
            ...
            } else if (paymentTokens.contains(token_)) {
                IERC20(token_).safeDecreaseAllowance(address(router_), allowance_);
    ...
    function setMintToken(address mintToken_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        mintToken = mintToken_;

        emit MintTokenChanged(mintToken_);
    }

    function setDistributor(address newDistributor_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            IERC165Upgradeable(newDistributor_).supportsInterface(type(IFarming).interfaceId),
            "Payment: invalid new distributor"
        );

        address distributor_ = distributor;

        IERC20(IFarming(newDistributor_).rewardToken()).safeIncreaseAllowance(
            newDistributor_,
            type(uint256).max
        );
        if (distributor_ != address(0) && distributor_ != newDistributor_) {
            address rewardToken_ = IFarming(distributor_).rewardToken();
            IERC20(rewardToken_).safeDecreaseAllowance(
                distributor_,
                IERC20(rewardToken_).allowance(address(this), distributor_)
            );
        }

        distributor = newDistributor_;

        emit DistributorChanged(newDistributor_);
    }

    function setHolder(address holder_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        holder = holder_;

        emit HolderChanged(holder_);
    }
    ...
    function pay(
        ...
        if (distributor_ != address(0)) rewardToken_ = IFarming(distributor_).rewardToken();

        uint8 decimals_ = IERC20Metadata(paymentToken_).decimals();
        uint256 toHolderAmount_ = (paymentAmount_ - cashbackAmount_).from18(decimals_);

        if (toHolderAmount_ > 0) {
            _transferToHolder(paymentToken_, rewardToken_, payer_, toHolderAmount_);
        }

        if (cashbackAmount_ > 0) {
            _cashbackTransfer(
                paymentToken_,
                rewardToken_,
                distributor_,
    ...
    function getSwapAmount(uint256 amount_, uint128 increasePercent_)
        public
        pure
        returns (uint256)
    {
        return amount_ + (amount_ * increasePercent_) / PERCENTAGE_100;
    }
    ...
    function _transferToHolder(
        address paymentToken_,
        address rewardToken_,
        address payer_,
        uint256 amount_
    ) private {
        require(holder != address(0), "Payment: holder isn't set");
        require(
            rewardToken_ == paymentToken_ || paymentTokens.contains(paymentToken_),
            "Payment: invalid token"
        );

        IERC20(paymentToken_).safeTransferFrom(payer_, holder, amount_);
    }

    function _cashbackTransfer(
        address paymentToken_,
        address rewardToken_,
        ...
        require(distributor_ != address(0), "Payment: distributor isn't set");

        if (rewardToken_ == paymentToken_) {
            // Transfer `cashbackAmount_` to `this` contract
            IERC20(rewardToken_).safeTransferFrom(
                payer_,
                address(this),
                cashbackAmount_.from18(decimals_)
            );

            // Transfer `cashbackAmount_` from this contract to `distributor_`
            IFarming(distributor_).supply(cashbackAmount_.from18(decimals_));
        } else {
            ...
            uint256 amountInMaximum_ = getSwapAmount(
                cashbackAmount_.from18(decimals_),
                swapInfo.increasePercent
            );
            uint256 amountOut_ = cashbackAmount_.from18(IERC20Metadata(rewardToken_).decimals());
            ...
            _swap(paymentToken_, rewardToken_, amountOut_, amountInMaximum_, payer_, swapInfo);

            // Transfer `cashbackAmount_` from this contract to `distributor_`
            IFarming(distributor_).supply(amountOut_);
        }

        // Mint `cashbackAmount_` for `payer_`
        IToken(mintToken).mint(payer_, cashbackAmount_);
    }
...
interface IPayment {
    ...
    function setDistributor(address newDistributor_) external;

    function setHolder(address holder_) external;
...
interface IFarming {
    ...
    function withdraw(address receiver_) external;
