...
contract veCRS is ERC20VotesUpgradeable, OwnableUpgradeable, UUPSUpgradeable {

    ICeresFactory public ceresFactory;
    mapping(address => bool) public transferable;
    address public governorTimelock;

    event veCRSMinted(address indexed to, uint256 amount);
    event veCRSBurned(address indexed from, uint256 amount);
    ...
    function initialize(uint256 _premint) public initializer {
        __Ownable_init();
        __ERC20_init("veCRS", "veCRS");
        __ERC20Permit_init("veCRS");
        __ERC20Votes_init();
    ...
    function mint(address to, uint256 amount) external {
        require(msg.sender == ceresFactory.ceresReward() || msg.sender == ceresFactory.ceresMiner() ||
            msg.sender == ceresFactory.ceresBank(), "veCRS: No permission!");
        _mint(to, amount);
        emit veCRSMinted(to, amount);
    }

    function burn(address from, uint256 amount) external {
        require(msg.sender == ceresFactory.ceresSwap(), "veCRS: Only CeresSwap!");
        _burn(from, amount);
        emit veCRSBurned(from, amount);
    }

    function _transfer(address from, address to, uint256 amount) internal override {
        require(transferable[from] || transferable[to] || ceresFactory.isStaking(from), "veCRS: Not transferable!");
    ...
    function setTransferable(address[] calldata _addresses, bool[] calldata _statuses) external onlyOwnerOrGovernor {
        require(_addresses.length == _statuses.length, "veCRS: Invalid length!");
        for (uint256 i = 0; i < _addresses.length; i++)
            transferable[_addresses[i]] = _statuses[i];
    }
