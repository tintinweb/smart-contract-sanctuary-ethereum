...
contract Bayplay is ERC721A, Ownable, RoyaltiesV2, DefaultOperatorFilterer {
    ...
    SalesEndTime public salesEndTime = SalesEndTime(0, 0, 0);
    ...
    function pre1Mint(uint256 quantity, bytes calldata signature) external payable {
        require(salesStatus.pre1Active, "pre 1 mint isn't active");

        require(block.timestamp <= salesEndTime.pre1EndAt, "pre 1 mint ended");
    ...
    function pre2Mint(uint256 quantity, bytes calldata signature) external payable {
        require(salesStatus.pre2Active, "pre 2 mint isn't active");

        require(block.timestamp <= salesEndTime.pre2EndAt, "pre 2 mint ended");
    ...
    function publicMint(uint256 quantity) external payable {
        ...
        require(block.timestamp <= salesEndTime.publicEndAt, "public mint ended");
    ...
    function setSalesEndTime(uint256 pre1, uint256 pre2, uint256 pub) external onlyOwner {
        salesEndTime.pre1EndAt = pre1;
        salesEndTime.pre2EndAt = pre2;
        salesEndTime.publicEndAt = pub;
    }
