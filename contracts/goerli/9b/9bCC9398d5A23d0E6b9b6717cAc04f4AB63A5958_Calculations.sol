...
contract Calculations is Initializable, OwnableUpgradeable {
    ...
    function getBiggestMultiplierFromBet(
        ...
        require(
            betAmount <= diceGame.s_maxBetAmount(),
        ...
        returns (uint256 winningChance)
    {
        ...
        uint256 winningChanceBeforeRounding = (((diceGame.MAX_NUMBER() -
            diceGame.s_houseEdge()) * BP) * BP) / multiplier;
    ...
    function getMaxMultiplier() public view returns (uint256 maxMultiplier) {
        maxMultiplier = (diceGame.MAX_NUMBER() - diceGame.s_houseEdge()) * BP;
    }
    ...
    function getMinMultiplier() public view returns (uint256 minMultiplier) {
        ...
        uint16 s_houseEdge = diceGame.s_houseEdge();
        minMultiplier =
        ...
        returns (uint256 maxWinningChance)
    {
        ...
        maxWinningChance = diceGame.MAX_NUMBER() - (diceGame.s_houseEdge() + 1);
    }
    ...
    function estimateChainlinkFee(
        ...
        uint256 priceParsed = (uint256(price) * linkPremium) / BP;
...
interface IDiceGame {
    ...
    function s_houseEdge() external view returns (uint16);

    function s_maxBetAmount() external view returns (uint256);
}
