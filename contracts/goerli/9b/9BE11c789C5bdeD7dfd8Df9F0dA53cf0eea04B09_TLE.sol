...
interface IVault {
    function addManyToVault(address account, uint256[] calldata tokenIds)
    external;

    function claimManyFromVault(uint256[] calldata tokenIds) external;
}
...
contract TLE is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
    ...
    // reference to the Vault for staking
    IVault public vault;

    constructor() ERC721A("TLE", "TLE", MAX_BATCH_SIZE, MAX_TOKENS) {}
    ...
    function mint(uint256 amount) external payable mintGuard(amount) {
        ...
        for (uint256 i = startIndex; i < endIndex; i++) {
            approve(address(vault), i); // totalSupply() acts as tokenId
        }
    }
    ...
    mintGuard(amount)
    {
        ...
        approve(address(vault), totalSupply() - 1); // totalSupply() acts as tokenId
    }
    ...
    //     uint256 tokenId
    // ) public virtual override {
    //     // Hardcode the Vault's approval so that users don't have to waste gas approving
    ...
    function setNotRevealedBaseURI(string calldata _notRevealedTokenUri)
    external
    onlyOwner
    {
