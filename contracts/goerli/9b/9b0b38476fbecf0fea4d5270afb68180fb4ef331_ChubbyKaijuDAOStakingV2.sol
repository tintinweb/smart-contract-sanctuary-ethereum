...
interface IChubbyKaijuDAOStakingV2Old {
  ...
  function lastClaimTimestamp() external view returns (uint48);
  function remaining_crunch() external view returns (uint128);
  function halving_phase() external view returns (uint128);
  function totalCrunchEarned() external view returns (uint128);
}
// File: Strings.sol
...
contract ChubbyKaijuDAOStakingV2 is IChubbyKaijuDAOStakingV2, OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable {
  ...
  function migrateFromOldContract() external onlyOwner {
    ...
    lastClaimTimestamp = gen2StakeOld.lastClaimTimestamp();
    remaining_crunch = gen2StakeOld.remaining_crunch();
    halving_phase = gen2StakeOld.halving_phase();
    totalCrunchEarned = gen2StakeOld.totalCrunchEarned();
