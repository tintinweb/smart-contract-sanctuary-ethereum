...
because we won't have a bridgeable asset for the main token
*/
contract ConxLbpSwapTargetV1 is IXReceiver {

    IConnext public immutable connext;
    address public immutable vaultAddress;
    mapping(string => uint256) private bridgedBalance;
    ...
    function increaseBridgedBalance(address wallet, address asset, uint256 amount) internal {
        string memory keyString = string(abi.encodePacked(wallet, asset));
        // add new bridged balance to existing balance if it exists
        bridgedBalance[keyString] = bridgedBalance[keyString] + amount;
    }

    function decreaseBridgedBalance(address wallet, address asset, uint256 amount) internal {
        string memory keyString = string(abi.encodePacked(wallet, asset));
        // add new bridged balance to existing balance if it exists
        bridgedBalance[keyString] = bridgedBalance[keyString] - amount;
    }

    function withdraw(address asset) external returns (uint256 balance){
        require(asset != address(0), "invalid asset");
        string memory keyString = string(abi.encodePacked(msg.sender, asset));
        uint256 balance = bridgedBalance[keyString];
        require(balance > 0, "no balance");

        TransferHelper.safeTransfer(asset, msg.sender, balance);
    }
    ...
    function xReceive(
    ...
    ) external onlyConnext returns (bytes memory)  { // return type warning, but I think we need to ignore
        ...
        // CREDIT - bridgedBalance to user immediately, in case of any error here the user can request a refund
        increaseBridgedBalance(funds.recipient, singleSwap.assetIn, singleSwap.amount);

        // Validity Checks (For extra security these should also be run on the source contract)
        ...
        require(singleSwap.amount <= _amount, "not enough credit to the swap");
        require(singleSwap.assetIn == _asset, "asset mismatch");
        ...
        try Vault(vaultAddress).swap(singleSwap, funds, limit, deadline) returns (uint _swappedAmount){
            ...
            // DISCREDIT - if swap is successful
            decreaseBridgedBalance(funds.recipient, singleSwap.assetIn, singleSwap.amount);
        } catch Error(string memory reason) {
            TransferHelper.safeTransfer(_asset, funds.recipient, _amount);
            // if swap fails, but we refund the user, reduce bridgedBalance
            return "FF: vault swap failed, call withdraw() for refund";
        }
