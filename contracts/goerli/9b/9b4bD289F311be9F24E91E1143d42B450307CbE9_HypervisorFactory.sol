   ...
   event Deposit(
    ...
    function withdraw(
        ...
        if(amount1 > tmp.totalAmount1 / 2 ){
            ...
            (tmp.amount0_swap, tmp.amount1_swap) = pool.swap(address(this), false, int256(tmp.swapAmount), TickMath.MAX_SQRT_RATIO - 1, tmp.data);
            ...
            (tmp.amount0_swap, tmp.amount1_swap) = pool.swap(address(this), true, int256(tmp.swapAmount), TickMath.MIN_SQRT_RATIO + 1, tmp.data);
            amount0 -= uint256(tmp.amount0_swap);
            amount1 += uint256(tmp.amount1_swap);
        }
