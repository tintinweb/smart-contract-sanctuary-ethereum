...
import "./@chirulabs/ERC721AUpgradeable.sol";
    ...
    ERC2981Upgradeable
{
    ...
    ///@dev mapping that specifies price of token.
    ...
    ///@dev custom error for withdrawal.
    ...
    /**@dev event for when there is a withdrawal.
       @param payee: address to withdraw to
       @param amount: amount that was withdrawed */
    event Withdrawal(address indexed payee, uint256 indexed amount);
    ...
    /**@notice initializes the contract. Required function, since a proxy pattern is used.
       ...
       @param _management: address of Managmente smart contract
    ...
    function initialize(
        ...
        require(_maxSupply > 0, "ERC721Fund: maxSupply cannot be 0");
    ...
    /**@notice mints given amount of NFTs.
       @param _amount: amount of NFTs to be minted */
    function mint(uint256 _amount) external payable whenNotPaused nonReentrant {
        require(msg.sender == tx.origin, "ERC721Fund: BOT");

        require(
            !(_nextTokenId() + _amount > maxSupply),
            "ERC721Fund: max supply reached"
        );

        require(
            !(msg.value < _amount * price),
            "ERC721Fund: not enough value sent"
        );
    ...
    function setTokenPrice(uint256 _tokenId, uint256 _price) external {
        require(
            msg.sender == ERC721AUpgradeable.ownerOf(_tokenId),
            "ERC721Fund: not token owner"
        );
    ...
    /**@notice withdraws all contract's balance to the given payee.
       @param _payee: address to which it will be transfered */
    function withdrawPayments(address payable _payee) external onlyOwner {
        uint256 balance = address(this).balance;
        uint256 royalties = (balance * management.fee()) / _feeDenominator();
        _creatorProRoyalty(royalties);

        ///@custom:question não é melhor usar transfer, para evitar reentrancy?
        (bool transferTx, ) = _payee.call{value: balance - royalties}("");
        if (!transferTx) {
            revert WithdrawTransfer();
        }

        emit Withdrawal(_payee, balance - royalties);
    }
    ...
    /**@dev transfers CreatorPRO royalties to the MultiSig contract.
       @param _value: value to be transfered */
    function _creatorProRoyalty(uint256 _value) internal {
        payable(management.multiSig()).transfer(_value);
    }
    ...
    function transferFrom(
        ...
        require(
            !(block.timestamp - lastTransfer[tokenId] < 30 days),
            "ERC721Fund: transfer deadline still going"
        );
        ...
        // CreatorPRO royalty
        _creatorProRoyalty((msg.value * management.fee()) / _feeDenominator());

        // owner/creator royalty
        (, uint256 royaltyAmount) = royaltyInfo(tokenId, msg.value);
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(ERC721AUpgradeable.tokenURI(_tokenId), ".json")
            );
    }
...
interface IManagement {
    ...
    function newFundCollection() external;
