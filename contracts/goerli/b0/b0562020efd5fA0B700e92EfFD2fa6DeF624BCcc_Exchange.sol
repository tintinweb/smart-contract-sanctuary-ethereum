...
library LibAsset {
    ...
    function _hash(Asset[] memory _assets)
        internal
        pure
        returns (bytes32[] memory)
    {
        bytes32[] memory assetsBytes = new bytes32[](_assets.length);
        for (uint256 i = 0; i < _assets.length; i++) {
            assetsBytes[i] = _hash(_assets[i]);
        }
        return assetsBytes;
    }
}
...
library LibOrder {
    ...
    function _hashKey(Input memory input) internal pure returns (bytes32) {
        bytes32[] memory assetsBytes = LibAsset._hash(input.takeAssets);
        return
            keccak256(
                abi.encode(
                    input.maker,
