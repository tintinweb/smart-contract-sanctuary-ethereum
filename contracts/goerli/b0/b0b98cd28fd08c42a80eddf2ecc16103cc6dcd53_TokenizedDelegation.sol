...
contract TokenizedDelegation is ERC721, ERC2981, INFTFlashLender {
    ...
    /// @notice A mapping pointing NFT ids to Deposit structs
    ...
    event DepositCreated(uint256 indexed depositId, address indexed depositor, address indexed contract_, uint256 tokenId, uint256 expiration);


    constructor(address _DELEGATION_REGISTRY, string memory _baseURI) ERC721("TokenizedDelegation", "RIGHTS") {
    ...
    function transferFrom(address from, address to, uint256 id) public override {
        ...
        require(block.timestamp < deposit.expiration, "deposit expired and cannot be transferred");
        // Reassign delegation powers
        IDelegationRegistry(DELEGATION_REGISTRY).delegateForToken(from, deposit.contract_, deposit.tokenId, false);
        IDelegationRegistry(DELEGATION_REGISTRY).delegateForToken(to, deposit.contract_, deposit.tokenId, true);
        super.transferFrom(from, to, id);
    }

    /// @dev Delegate when a tokenized delegation is minted
    ...
    /// @dev Undelegate when a tokenized delegation is burned
    function _burn(uint256 id) internal override {
        Deposit memory deposit = deposits[id];
        IDelegationRegistry(DELEGATION_REGISTRY).delegateForToken(ownerOf(id), deposit.contract_, deposit.tokenId, false);
        super._burn(id);
    }
    ...
    /// @notice Use this to deposit a timelocked escrow and create a tokenized claim on its delegation rights
    function deposit(address contract_, uint256 tokenId, uint256 expiration) external {
        ...
        _mint(msg.sender, nextDepositId);
        emit DepositCreated(nextDepositId++, msg.sender, contract_, tokenId, expiration);
    }
    ...
    function burn(uint256 depositId) external {
        ...
        require(ownerOf(depositId) == msg.sender || block.timestamp >= deposit.expiration, "INVALID_BURN");
        _burn(depositId);
        ERC721(deposit.contract_).transferFrom(address(this), deposit.depositor, deposit.tokenId);
        delete deposits[depositId];
    }

    /// @notice Flashloan a delegated asset to its tokenized owner. 
    ...
    function flashLoan(
        ...
        Deposit memory deposit = deposits[depositId];
        require(ownerOf(depositId) == msg.sender, "can only flashloan your own tickets");
        ERC721(deposit.contract_).transferFrom(address(this), address(receiver), deposit.tokenId);
        require(
            receiver.onFlashLoan(msg.sender, deposit.contract_, deposit.tokenId, data) == CALLBACK_SUCCESS,
            "FlashLender: Callback failed"
        );
        ERC721(deposit.contract_).transferFrom(address(receiver), address(this), deposit.tokenId);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory attributes = string.concat(
            '"attributes":[',
            '{"trait_type": "Collection Address", "value": "', deposit.contract_.toHexString(), '"', 
            '}, {"trait_type": "Collection Name", "value": "', ERC721(deposit.contract_).name(), '"',
            '}, {"trait_type": "TokenID", "value": "', deposit.tokenId.toString(), '"',
            '}, {"trait_type": "Expiration", "display_type": "date", "value": ', deposit.expiration.toString(),
            '}, {"trait_type": "Depositor Address", "value": "', deposit.depositor.toHexString(), '"'
            '}]'
        );
        ...
        string memory metadataString = string.concat(
            '{"name": "TokenizedDelegation #', tokenId.toString(), 
            '", "description": "TokenizedDelegations are something", ', 
            attributes, 
            ', "image": "', imageUrl,
            '"}'
        );
...
interface INFTFlashLender {
     ...
     * @param depositId The tokenized delegation to flashloan the underlying escrow asset of.
