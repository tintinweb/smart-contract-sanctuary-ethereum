...
interface IUNFT {
   ...
   * @param underlyingAsset_ The address of the underlying asset of this uNFT (E.g. PUNK for bPUNK)
   */
  function initialize(
    ...
    IERC721ReceiverUpgradeable
{
    ...
    function initNft(address nftAsset, address uNftAddress)
        external
        override
        onlyLendPool
    {
    ...
    //update
    function getUserAllLoans(address user, uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        uint256 loanId = _loanIdTracker.current();
        uint256 index;
        uint256 total;
        for (uint256 i = 1; i < loanId + 1; i++) {
            if (_loans[i].borrower == user) {
                total++;
            }
        }
        require(total > 0, "no info");
        DataTypes.LoanData[] memory loanDatas = new DataTypes.LoanData[](total);
        for (uint256 i = 1; i < loanId + 1; i++) {
            if (_loans[i].borrower == user) {
                loanDatas[index] = _loans[i];
                index++;
            }
        }
        uint256 loansLen = 10 > total ? total : 10;
        DataTypes.LoanData[] memory loans = new DataTypes.LoanData[](loansLen);
        (total, loans) = _page(total, pageNum, loanDatas);
        return(total, loans);
        
    }

    function getUserAllActiveLoans(address user, uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        for (uint256 i = 1; i < loanId + 1; i++) {
            if (
                _loans[i].borrower == user &&
                _loans[i].state == DataTypes.LoanState.Active
            ) {
                total++;
            }
        }
    ...
    function getUserAllAuctionLoans(address user, uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        for (uint256 i = 1; i < loanId + 1; i++) {
            if (
                _loans[i].borrower == user &&
                _loans[i].state == DataTypes.LoanState.Auction
            ) {
                total++;
            }
        }
    ...
    function getUserNFTAllLoans(address user, address nft, uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        uint256 total;
        for (uint256 i = 1; i < loanId + 1; i++) {
            if (_loans[i].borrower == user && _loans[i].nftAsset == nft) {
                total++;
            }
        }
        require(total > 0, "no info");
    ...
    function getUserNFTActiveLoans(address user, address nft, uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
    ...
    function getAllActiveLoans(uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        uint256 total;
        for (uint256 i = 1; i < loanId + 1; i++) {
            if (_loans[i].state == DataTypes.LoanState.Active) {
                total++;
            }
        }
        require(total > 0, "no info");
    ...
    // unhealthy
    function getAllUnhealthyLoans(uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        uint256 total;
        for (uint256 i = 1; i < loanId + 1; i++) {
            address nftAsset = _loans[i].nftAsset;
            uint256 nftTokenId = _loans[i].nftTokenId;
            (, , , , , uint256 healthFactor) = ILendPool(
                _addressesProvider.getLendPool()
            ).getNftDebtData(nftAsset, nftTokenId);
    ...
    function getAlmostUnhealthyLoans(uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        for (uint256 i = 1; i < loanId + 1; i++) {
            ...
            if (healthFactor >= 1e18 && healthFactor <= 11e17) {
                total++;
            }
        }
        require(total > 0, "no info");
    ...
    function getAllAuctionLoans(uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        uint256 total;
        for (uint256 i = 1; i < loanId + 1; i++) {
            if (_loans[i].state == DataTypes.LoanState.Auction) {
                total++;
            }
        }
        require(total > 0, "no info");
    ...
    function getUserCanRedeemLoans(address user, uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        for (uint256 i = 1; i < loanId + 1; i++) {
            address nftAsset = _loans[i].nftAsset;
            uint256 nftTokenId = _loans[i].nftTokenId;
            (, , , uint256 redeemEndTimestamp) = ILendPool(
                _addressesProvider.getLendPool()
            ).getNftAuctionEndTime(nftAsset, nftTokenId);
            if (
                _loans[i].state == DataTypes.LoanState.Auction &&
                _loans[i].borrower == user &&
                redeemEndTimestamp > block.timestamp
            ) {
                total++;
            }
        }
    ...
    function getUserCanLiquidateLoans(address user, uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        for (uint256 i = 1; i < loanId + 1; i++) {
            address nftAsset = _loans[i].nftAsset;
            uint256 nftTokenId = _loans[i].nftTokenId;
            (, , uint256 bidEndTimestamp, ) = ILendPool(
                _addressesProvider.getLendPool()
            ).getNftAuctionEndTime(nftAsset, nftTokenId);
            if (
                _loans[i].state == DataTypes.LoanState.Auction &&
                _loans[i].bidderAddress == user &&
                bidEndTimestamp <= block.timestamp
            ) {
                total++;
            }
        }
    ...
    function getUserAllUnhealthyLoans(address user, uint256 pageNum)
        public
        view
        returns (uint256, DataTypes.LoanData[] memory)
    {
        ...
        for (uint256 i = 1; i < loanId + 1; i++) {
            ...
            if (healthFactor < 1e18 && healthFactor != 0 && _loans[i].borrower == user) {
                total++;
            }
        }
        require(total > 0, "no info");
    ...
    function _page(uint256 total, uint256 pageNum, DataTypes.LoanData[] memory loanDatas) internal view returns(uint256, DataTypes.LoanData[] memory) {
        uint256 id;
        uint256 loansLen = 10 > total ? total : 10;
        DataTypes.LoanData[] memory loans = new DataTypes.LoanData[](loansLen);
        if (pageNum > 0) {
          uint256 end = pageNum * 10 > total ? total : pageNum * 10;
            for (uint256 i = (pageNum - 1) * 10; i < end; i++) {
                loans[id] = loanDatas[i];
                id++;
            }
            return (total, loans);
        } else {
            return (total, loanDatas);
        }
    }
