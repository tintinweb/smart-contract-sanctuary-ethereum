...
contract ASkyTrylogy is ERC721Enumerable, Ownable, ReentrancyGuard, ERC2981 {
    ...
    uint256 prizeWeiWL = 150000000000000000;//0.15 ethers
    uint256 prizeWeiPS = 300000000000000000;//0.3 ethers
    // uint256 prizeWeiPS = 150000000000000000;//0.15 ethers
    // uint256 priceWeiWL = 300000000000000000;//0.3 ethers
    ...
    function contractURI() public view returns (string memory) {
        return string(abi.encodePacked(_contractURI, "/", name()));
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        return _partitionsAndMetaStore.getMetadata(tokenId);
    }

    /**
     * @dev Se encarga del minteo de NFT para el fremint
     * @dev Primero comprueba en el modificador que el usuario este en la lista de freemint 
     * @dev Se comprueba que no se haya alcanzado el máximo numero de NFT 
     * @dev Se comprueba que el el numero de NFT del freemint no llegen al limite 
     * @dev Se aumenta el id de freement y se llama al metodo "_mint" 
     */
    ...
    /**
     * @dev Cualquiera puede llamar a a este metodo. Es un metodo payable, el usuario debe pagar para mintear
     * @dev Primero comprueba en el modificador que el usuario este en la lista de whitelist 
     * @dev Se comprueba que no se haya alcanzado el máximo numero de NFT 
     * @dev Se comprueba que el el numero de NFT del freemint no llegen al limite 
     * @dev Se comprueba que el etehreum enviado por aprte del usuario sea indicado, si es menor no se mintea el NFT
     * @dev Se aumenta el id de public sales y se llama al metodo "_mint" 
     */
    function mintWL(uint64 quantity) public payable returns(uint32){
        ...
        require(numMaxMintWhitelist >= quantity, "No mint available for Whitelist");
        require(msg.value >= (quantity * prizeWeiPS), "No enough amount to mint"); 
    ...
    function mintPS(uint64 quantity) public payable returns(uint32){
        ...
        require(numMaxMintPublicsale >= quantity, "No mint available for Public sale");
        require(msg.value >= (quantity * prizeWeiPS), "No enough amount to mint"); 
...
interface IPartitionsAndMetaStore {
    ...
    function checkClosePartition(uint256 tokenId) external returns(uint256);
    ...
    function getCurrentPartition() external view returns(uint256);
...
interface IEarlyMintersRoyalties  {
    ...
    function safeOwnerWithdraw() external payable;
