...
contract Metadata is ERC721A, Ownable, ReentrancyGuard, IERC4907 {
    ...
    uint public rent_Last_Id;
    mapping(uint=>uint) rentTokenId;
    mapping(uint=>uint) rentofOwner;
    mapping(uint => mapping(address => string)) private rents;

    constructor() ERC721A("Metadata Service - Test", "@meta") {
        tokenIDandAddress[_currentIndex]="meta";
        tokenAddressandID["meta"]=_currentIndex;
...
struct UserInfo
    ...
    function setRentUser(uint256 tokenId, address user, uint256 price, string calldata meta, uint64 expires) public virtual{
        ...
        info.countid = rent_Last_Id;
        info.user = user;
        info.price = price;
        info.meta = meta;
        info.expires = expires;
        emit UpdateUser(rent_Last_Id,tokenId,user,meta,expires);
    }
...
//ERC4907 FUNCTIONS FINISH
    ...
    function RentNFT(string calldata metadata, uint256 day) public payable {
            uint256 tokenid=tokenAddressandID[metadata];
            ...
            rent_Last_Id=rent_Last_Id+1;
            rentTokenId[rent_Last_Id]=tokenAddressandID[metadata];
            rentofOwner[rent_Last_Id]=tokenAddressandID[metadata];
            //rents[newLast][msg.sender]=string.concat("?",seller,Strings.toString(block.timestamp),Strings.toString(msg.value));
    }

    function LastRents(uint256 count)    public view
    returns (string[] memory)
  {
    uint256 total = rent_Last_Id;
        ...
        payable
    {
        ...
        require (tokenAddressandID[metadata] == 0 , "This is already taken");
        require(MintStartStatus, "Register is not active!");
        require(msg.value >= price, "Insufficient funds!");
        tokenIDandAddress[_currentIndex]=metadata;
        tokenAddressandID[metadata]=_currentIndex;
         if (is_ref) {
