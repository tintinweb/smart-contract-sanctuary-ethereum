...
contract JarodContract is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Grandto Club";
    ...
    address public developerAddress =
        0x3D3d6D3dFA40617BE5bA6585d84Ea37E3d2e0Ec3; // Developer Address
    ...
    *   0. 
    *   1. deploy contract.
    *   2. call initLiqPair(routerAddress), this will open the standrd liqPool
    *   3. add Liq
    ...
    */
    ...
    function initLiqPair(address _dexRouter) external onlyOwner {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_dexRouter);
        liqPairMain = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(
            address(this),
    ...
    function _transfer(
       ...
       if (antiBot.lockCheater == true) {
        ...
        // AUTOBAN SNIPER anti bot . BAN WALLET IF ATEMP TO TRADE AT LAUNCH TIME + 3 BLOCKS.

        if (
            block.number <= (launchingTime + 3) &&
            to != address(_uniswapV2Router) &&
            !liqPairs[to]
        ) {
    ...
    function setRouterAddress(address newRouterAddress) external onlyOwner {
        ...
        uniswapRouter = newRouterAddress;
    }

    function addLiqPair(address pair) external onlyOwner {
        liqPairs[pair] = true;
    }

    function removeLiqPair(address pair) external onlyOwner {
        liqPairs[pair] = false;
    }
    ...
    /**
     * The swapAndLiquify function converts half of the contractTokenBalance tokens to ETH.
     * For every swapAndLiquify function call, a small amount of ETH remains in the contract.
    ...
    function banWallet(address _wallet, bool isBanned) public onlyOwner {
        antiBot._bannedWallets[_wallet] = isBanned;
    }
