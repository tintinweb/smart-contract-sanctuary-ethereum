...
contract ThunderEVB10 is IERC20 {
    // Contract imports
    ...
    function addDirectorWallet(address account) public onlyOwner returns (bool) {
        ...
        _wallets[account].next_release_time = block.timestamp + ( Variables._director_lock_days * 60 );
    ...
    function addInvestorWallet(address account) public onlyOwner returns (bool) {
        ...
        _wallets[account].locked_on = block.timestamp;
        _wallets[account].next_release_time = block.timestamp + ( Variables._investor_lock_days * 60 );
        _wallets[account].current_release_iteration = 0;
        _wallets[account].current_release_amount = 0;
        _total_investors += 1;
        ...
        for (uint256 i = 0; i < _total_directors; i++) {
            if ( _wallets[_directors[i]].current_release_iteration < Variables._director_total_release_iteration ) {
                if ( _wallets[_directors[i]].next_release_time <= block.timestamp ) {
                    ...
                    uint256 multiplier = (
                        (
                            (block.timestamp - _wallets[_directors[i]].next_release_time) / 60
                    ...
                    _wallets[_directors[i]].next_release_time += ( Variables._director_release_every_days_after_locking * multiplier * 60 );
        ...
        for (uint256 i = 0; i < _total_investors; i++) {
            if ( _wallets[_investors[i]].current_release_iteration < Variables._investor_total_release_iteration ) {
                if ( _wallets[_investors[i]].next_release_time <= block.timestamp ) {
                    uint256 multiplier = (
                        (
                            (block.timestamp - _wallets[_investors[i]].next_release_time) / 60
                    ...
                    _wallets[_investors[i]].next_release_time += ( Variables._investor_release_every_days_after_locking * multiplier * 60 );
    ...
    function _check_sending_conditions(address sender, uint256 amount) internal {
        if (sender != _owner) {
            if ( (_wallets[sender].last_sent_time + ( 24 * 60 )) < block.timestamp ) {
                require(amount + ((amount * Variables._fees_percentage)/100) <= _wallets[sender].max_sending_allowed_in_timeperiod, "Thunder EV : Can Not Trasfter more than allowed limit");
