...
contract HulkCarsNFT is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 usdcCost = 1 ether;
    uint256 usdtCost = 1000000;
    uint256 busdCost = 1 ether;
    ...
    address public immutable USDT = address(0xe583769738b6dd4E7CAF8451050d1948BE717679);
    address public immutable USDC = address(0x07865c6E87B9F70255377e024ace6630C1Eaa37F);
    address public immutable BUSD = address(0xb809b9B2dc5e93CB863176Ea2D565425B03c0540);
    ...
    function mint(uint256 _mintAmount,address _token) external payable callerIsUser{
      ...
      //require(IERC20(_token).allowance(msg.sender, address(this)), "Not approved to send balance requested");
      require(allowedTokens[_token] == true, "Token is not allowed/supported");
      if (_msgSender() == owner() || whiteList[msg.sender] ){
        _safeMint(msg.sender, _mintAmount);
      } else {
        if (_token == USDC){
            uint totalCost = usdcCost * _mintAmount;
            IERC20(_token).transferFrom(msg.sender, owner(), totalCost);
            _safeMint(msg.sender, _mintAmount);
        } else if (_token == USDT){
            uint totalCost = usdtCost * _mintAmount;
            IERC20(_token).transferFrom(msg.sender, owner(), totalCost);
            _safeMint(msg.sender, _mintAmount);
        } else {
            uint totalCost = busdCost * _mintAmount;
            IERC20(_token).transferFrom(msg.sender, owner(), totalCost);
            _safeMint(msg.sender, _mintAmount);
        }
      }
      // else (_msgSender() != owner() && !whiteList[msg.sender] ){
