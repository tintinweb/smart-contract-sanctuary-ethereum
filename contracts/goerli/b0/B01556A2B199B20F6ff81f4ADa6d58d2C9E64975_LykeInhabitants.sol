...
error soldOut();
error saleAlreadyClaimed();
error tooManyTokensPerTx();
...
contract LykeInhabitants is ERC721A, IERC2981, Ownable {
    ...
    // collection details
    uint256 public constant PRICE = 0.0001 ether; //[temp] replace with real amount // only valid for Public Sale
        ...
        external payable 
    {
        ...
        if(msg.value != _quantity * PRICE) revert insufficientPaid();  
        if(_quantity > MAX_MINTS_PER_PUBLIC_TX) revert tooManyTokensPerTx();    
        if(_quantity + totalSupply() > COLLECTION_SIZE) revert soldOut();
    ...
    */
    function saleMint(uint256 _freeQuantity, uint64 _paidQuantity, uint256 _price, uint256 _saleStartTs, bytes calldata _signature)
        external payable
    {
        if(!verifySig(msg.sender, _freeQuantity, _paidQuantity, _price, _saleStartTs, _signature)) revert incorrectSignature();
        if(msg.value != _paidQuantity * _price) revert insufficientPaid();
        if(_paidQuantity + _freeQuantity + totalSupply() > COLLECTION_SIZE) revert soldOut();
        if(_saleStartTs > block.timestamp) revert saleNotStarted();
        if(addressClaimMap[msg.sender]) revert saleAlreadyClaimed();
    ...
    function reservedMint(uint256 _quantity) external onlyOwner {
        if(_quantity + totalSupply() > COLLECTION_SIZE) revert soldOut();
        if(maxReserveMintRemaining < _quantity) revert soldOut();
...
// [TODO] implement supports interface from ERC721A v4 https://chiru-labs.github.io/ERC721A/#/migration?id=supportsinterface
