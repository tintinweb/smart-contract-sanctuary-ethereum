...
contract HEX01 is Ownable, ERC721A  { 
    ...
    bool public _pausedHexKey = true;     
    ...
    address f5 = 0xb38Cf1583306C378a613409ED0eF9d0f815dae0f;
    ...
    function Hex01AlphaMint(uint256 num, bool blnPhysicalClaim, bytes memory signature, string memory nonce) public payable notContract {
        ...
        mintedAddress[msg.sender] += num; 
         ...
         //claim this token. 
        if (blnPhysicalClaim) {
            require( num < 6,   "You can mint a maximum of 5 Digital when claiming a physicals" ); 
            for(uint256 i; i < num; i++){
                _safeLegacyMint( msg.sender, supply + i );
                claimedPhysical[supply + i] = true; 
            }
        }else{
    ...
    function Hex01Mint(uint256 num, bool blnPhysicalClaim, bytes memory signature, string memory nonce) public payable notContract {
        ...
        if (blnPhysicalClaim) { 
            ...
            for(uint256 i; i < num; i++){
                _safeLegacyMint( msg.sender, supply + i );
                //set that the physical was claimed
                claimedPhysical[supply + i] = true; 
            } 
        }else{ 
            require( num < 31,   "You can mint a maximum of 30 Digitals Hexes Only" ); 
    ...
    function pauseHexKey(bool val) public onlyOwner {
        _pausedHexKey = val;
    } 
    ...
    function withdrawAll(uint256 amount) public payable onlyOwner {
        ...
        require(payable(f5).send(percent * 5)); 
    }
