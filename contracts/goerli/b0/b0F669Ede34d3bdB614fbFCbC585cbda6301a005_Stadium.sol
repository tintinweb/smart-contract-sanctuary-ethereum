...
library StadiumUtils {
    ...
    /**
     * @dev Return a recovered address from an ECDSA signature
     */
    ...
    /**
     * @dev Converts a `uint256` to human readable
     */
    function toString(uint256 value) internal pure returns (string memory) {
...
contract Stadium is OwnableUpgradeable, ERC721AUpgradeable {
    ...
    mapping(address => uint) _teamCards;
    ...
    error OneTeamPerAddressError(address team);
    error TeamNeedsPlayerToMint();
    ...
    function withdraw(
        ...
        (bool sent, ) = receiver.call{gas :10000, value: amount}("");
    ...
    /**
     * @dev Mint a named card
     * 
     * Create a card by type and name
     * 
     * Calling conditions:
     *
     * - All givzn types must match their signature
     * 
     * - 'oType' must be a valid type as seen in StadiumUtils
     * - 'name' must be a unique string
     * - 'ipfsHash' must be a valid IPFS hash
     */
    function mintNamed(
        uint8 oType,
        string memory name,
        ...
        if (oType == StadiumUtils.O_PLAYER) {
            if (_playerCards[sender] != 0) revert OnePlayerPerAddressError(sender);
            _playerCards[sender] = _nextTokenId();
        } else if (oType == StadiumUtils.O_TEAM) {
            if (_teamCards[sender] != 0) revert OneTeamPerAddressError(sender);
            if (_playerCards[sender] == 0) revert TeamNeedsPlayerToMint();
            _teamCards[sender] = _nextTokenId();
        }
    ...
    /**
     * @dev Public view to fetch the token name from a given tokenId
     * 
Z     * `tokenId` - The token ID to be used
     *
     */
    ...
    /**
     * @dev Public view to fetch the token type from a given tokenId
     * 
     * `tokenId` - The token ID to be used
     *
     */
    ...
    /**
     * @dev Public view to fetch the IPFS hash from a given tokenId
     * 
     * `tokenId` - The token ID to be used
     *
     * Calling conditions:
     *
     * - 'tokenId' must be a valid ID, when tokenId is 0 it will revert using NoIpfsHashError(tokenId)
     */
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(stadiumTokenURI,  StadiumUtils.toString(tokenId)));
    }
    ...
    /**
     * @dev Public view to fetch tokenID by address and card type
     */
    ...
    /**
     * @dev Public view to check wether a tokenId is retired
     */
    ...
    function _beforeTokenTransfers(
        ...
        // Initial mint is ignored
        if (from == address(0)) return;

        // Restrict transfer if not required
        bool transferrable = _tokenTransferrable[startTokenId];
        require(transferrable, "Card is not transferrable");
    }

    function _afterTokenTransfers(address from, address to, uint256 startTokenId, uint256) internal override {
        _playerCards[from] = 0;
        _playerCards[to] = startTokenId;
    }

    function getRef() public view returns (address) {
        return address(referee);
    }
}
