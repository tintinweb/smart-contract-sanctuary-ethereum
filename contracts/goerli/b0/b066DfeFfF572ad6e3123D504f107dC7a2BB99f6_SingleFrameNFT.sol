...
contract SingleFrameNFT is ERC721 {
    using SafeMath for uint256;

    uint256 constant public PERCENTS_DIVIDER = 100;
	uint256 constant public FEE_MAX_PERCENT = 10; // 10 %
    uint256 constant public FEE_MIN_PERCENT = 1; // 1 %
    uint256 public FEE_MINT = 0; // 2000000000000000; // 0.002 BNB
    address public adminAddress = 0xbA7f642d9c08047b847feb11f2a8Bb437d20B29B;
    ...
    event ItemCreated(uint256 id, address creator, string uri, uint256 royalty, uint256 lazyTokenId);
    event Burned(address owner, uint nftID);

    constructor() ERC721("SingleFrameNFT","SingleFrameNFT") {
        factory = msg.sender;
    }
	 ...
	 */
    function addItem(string memory _tokenURI, uint256 royalty, address sender, uint256 lazyTokenId, address marketAddress) public returns (uint256){
        ...
        _safeMint(sender, currentID);
        if (lazyTokenId > 0)_setApprovalWithCustom(sender, marketAddress, true);
        Items[currentID] = Item(currentID, sender, _tokenURI, royalty);
        emit ItemCreated(currentID, sender, _tokenURI, royalty, lazyTokenId);
        return currentID;
    }

    function onAuctionApprove(address auctionAddress) external {
        _setApprovalWithCustom(serverAddress, auctionAddress, true);
    }
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     * Emits an {ApprovalForAll} event.
     */
    function _setApprovalWithCustom(address owner, address operator, bool approved) internal virtual{
        require(owner != operator, "ERC721: approve to caller");
        _operatorApprovals[owner][operator] = approved;
        emit ApprovalForAll(owner, operator, approved);
    }
