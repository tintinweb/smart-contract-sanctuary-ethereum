...
contract AliveFixedSongMultiUri is AliveCommonSong {
...
function idToTokenUri(uint _id, string memory _uri) public virtual onlyAdmin{
  _idToTokenUri[_id] = _uri;
}
...
mintStopTime = _stopTime;
 ...
 function getPrice(uint256 _quantity, address _to) virtual public view returns (uint256 price) {
    ...
    {   require(block.timestamp > startedAt);
        ...
        uint oneorzero = block.number % 2;
        if(oneorzero==0) {

        _idToTokenUri[totalSupply()] = baseURI;
        }
        else {

            if(exclusiveAmount <= done ) {
        _idToTokenUri[totalSupply()] = baseURI;

            }
            else {
            done++;
            _idToTokenUri[totalSupply()] = baseuri2;
        ...
        returns (string memory)
    {
        ...
        string memory tempBaseURI = _idToTokenUri[tokenId];
        return
