    ...
    DefaultOperatorFilterer
{
    ...
    uint256 public maxMintPerTxForFCFS = 1;
    ...
    // ミントにはAL登録が必須ですが、セールの状況に応じて
    // ウォレット当たり上限なしの早押しもできるようにしてあります
    function publicMint(
        uint256 _mintAmount,
        uint256 _allocated,
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, _allocated));
        require(
        ...
        if (phase == SalePhase.ALSale) {
            require(
                mintCount[msg.sender][id] + _mintAmount <= _allocated,
                "Exceeds your allocation"
            );
        } else if (phase == SalePhase.FCFSSale) {
            require(
                _mintAmount <= maxMintPerTxForFCFS,
                "Exceeds mint limit per Tx"
            );
        }
        ...
        mintCount[msg.sender][id] += _mintAmount;
    }
    ...
    function setMaxMintPerTxForFCFS(uint256 _newAmount) external onlyRole(ADMIN) {
        maxMintPerTxForFCFS = _newAmount;
    }
