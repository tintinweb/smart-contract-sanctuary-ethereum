...
contract sePSP2 is TimeLockedERC20 {
    ...
    function withdrawPSPAndEth(int256 id, uint256 minEthAmount, uint256 minPspAmount) external {
        uint256 bptAmount = _withdraw(id);
        (uint256 ethAmount, uint256 pspAmount) = _exitPool(bptAmount, minEthAmount, minPspAmount, false);
        
        PSP.transfer(msg.sender, pspAmount);
        Utils.transferETH(payable(msg.sender), ethAmount);
    }

    function withdrawPSPAndWeth(int256 id, uint256 minEthAmount, uint256 minPspAmount) external {
        uint256 bptAmount = _withdraw(id);
        (uint256 ethAmount, uint256 pspAmount) = _exitPool(bptAmount, minEthAmount, minPspAmount, true);
    ...
    /// balancer related fn
    function _joinPool(uint256 pspAmount, uint256 ethAmount, uint256 minBptOut, bool isWeth) internal returns (uint256 bptAmount){
        ...
        if(isWeth) {
            ...
            BALANCER_VAULT.joinPool{value: ethAmount}(WETH20_PSP80_POOL_ID,address(this) , address(this), joinRequest);
        }
        
        bptAmount = BPT.balanceOf(address(this)) - bptBalanceBefore;
    }

    function _exitPool(uint256 bptAmount, uint256 minEthAmount, uint256 minPspAmount, bool isWeth) internal returns (uint256 ethAmount, uint256 pspAmount) {
        ...
        uint256 ethBalanceBefore = isWeth ? WETH.balanceOf(address(this)) : address(this).balance;
        uint256 pspBalanceBefore = PSP.balanceOf(address(this));
        
        BALANCER_VAULT.exitPool(WETH20_PSP80_POOL_ID, address(this), payable(address(this)), exitRequest);
        
        ethAmount = (isWeth ? WETH.balanceOf(address(this)) : address(this).balance) - ethBalanceBefore;
        pspAmount = PSP.balanceOf(address(this)) - pspBalanceBefore;
    }
}
...
contract TimeLockedERC20 is ERC20Permit, Ownable, Pausable {
    ...
    constructor(
        ...
        require(_timeLockBlocks >= _minTimeLockBlocks, "Cannot set less timelock than min");
        require(_timeLockBlocks <= _maxTimeLockBlocks, "Cannot set more timelock than max");
        
        asset = _asset;
        timeLockBlocks = _timeLockBlocks;
        minTimeLockBlocks = _minTimeLockBlocks;
    ...
    function changeTimeLock(uint256 newTimeLockBlocks) external onlyOwner {
        require(newTimeLockBlocks >= minTimeLockBlocks, "Cannot set less timelock than min");
        require(newTimeLockBlocks <= maxTimeLockBlocks, "Cannot set more timelock than max");
    ...
    function _withdraw(int256 id) internal returns (uint256) {
        ...
        WithdrawalRequest storage request = userVsWithdrawals[msg.sender][id];
        require(
            request.status == WITHDRAW_STATUS.UNLOCKING && request.releaseBlockNumber <= block.number,
            "Cannot withdraw"
        );
