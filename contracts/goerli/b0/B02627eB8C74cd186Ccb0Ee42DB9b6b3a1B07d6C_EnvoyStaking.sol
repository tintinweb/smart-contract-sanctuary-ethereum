...
contract EnvoyStaking is Ownable {
    ...
    function unStake(uint256 _amount) external returns (uint256) {
        ...
        user.rewardDebt = user.amount.mul(accVOYPerShare).div(1e12);
        uint256 feePercent = _getUnStakeFeePercent(msg.sender);
    ...
    function harvest(bool reStake) external returns (uint256) {
    ...
    function getUnStakeFeePercent(address _user) external view returns (uint256) {
        return _getUnStakeFeePercent(_user);
    }

    function _getUnStakeFeePercent(address _user) internal view returns (uint256) {
        UserInfo storage user = userInfo[_user];
