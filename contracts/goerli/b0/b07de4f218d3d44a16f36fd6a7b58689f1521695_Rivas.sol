    ...
    ERC1155Supply
{
    ...
    function supplyAvailable(
        ...
        uint256 numberOfTokensLength = numberOfTokens.length;
        if (tokenIdsLength != numberOfTokensLength)
            revert UnequalArrayLengths();

        for (uint256 index; index < tokenIdsLength; index++) {
            uint256 id = tokenIds[index];

            if (id >= maxSupply.length) revert InvalidTokenId();
            if (totalSupply(id) + numberOfTokens[index] > maxSupply[id])
        ...
        nonReentrant
    {
        ...
        for (uint256 index; index < airdropLength; index++) {
            ...
            uint256[] calldata tokenIds = data[index].tokenIds;
            uint256[] calldata numberOfTokens = data[index].numberOfTokens;

            // validate
            supplyAvailable(tokenIds, numberOfTokens);
            _mintBatch(receiver, tokenIds, numberOfTokens, "");
        }
    }
