...
contract XProgram is IXProgram, Matrix {
	...
	modifier onlyAccountOwner(uint _AID) {
		require(
			Accounts.RegistrationTime(_AID) != 0 && msg.sender == Accounts.AddressOfAccount(_AID),
	...
	modifier onlyAPI(uint _AID, address _Owner) {
		require(msg.sender == address(TAPI), "XProgram: caller is not API");
		require(
	...
	function ChangeStatusALU(uint _AID) external virtual onlyAccountOwner(_AID) {
		_ChangeStatusALU(_AID);
	}

	function ChangeStatusALU(uint _AID, address _Owner) external virtual onlyAPI(_AID, _Owner) {
		_ChangeStatusALU(_AID);
	}
	...
	function _UpgradeLevel(uint _AID, uint _XPro, uint _LTo) private {
		require(
			Balances.AvailableToUpgrade(_AID) >= Tuktus.PirceOfLevel(_LTo),
	...
	function _ShareReward(uint _AID, uint _XPro, uint _Level) private {
		if (_XPro == X3) {
			...
			uint sr = Tuktus.PirceOfLevel(_Level);
