...
contract HETH is ReentrancyGuardUpgradeable, PausableUpgradeable, HordUpgradable {
    ...
    // Represent instance of TokensFarmSDK contract
    ...
    */
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual {
        tokensFarmSDK.noticeReducedStakeWithoutStakeId(
            from,
            amount
        );

        tokensFarmSDK.deposit(
            to,
            amount
        );
    }
...
contract HordETHStakingManager is HETH {
    ...
    // Represent amount of ETH in reserves
    ...
    event FeeCollected(address account, uint256 amountHETH);
    ...
    initializer
    {
        ...
        totalHETHMinted = totalHETHMinted.add(stakingConfiguration.amountETHInValidator());
    ...
    external
    {
        require(hordCongressMembersRegistry.isMember(msg.sender), "Only congress members");
        _unpause();
    }

    /**
        * @notice  Function set address of TokensFarmSDK contract
        * @dev     Can be only called by HordCongress
     */
    function setTokensFarmSDK(address _tokensFarmSDK) external onlyHordCongress {
        require(_tokensFarmSDK != address(0), "TokensFarmSDK can not be 0x0 address");
        tokensFarmSDK = ISDK(_tokensFarmSDK);
    }
     ...
     */
    function setValidatorStats(uint256 newRewardsAmount, uint256 newTotalETHBalanceInValidators) external onlyMaintainer {
        ...
        if (
            ...
            (newRewardsAmount > totalRewardsCollected.add(rewardsBoundary) || newRewardsAmount < totalRewardsCollected.sub(rewardsBoundary))) ||
        ...
        if (totalRewardsCollected > lastRewardsForFeeCalc.add(tolerancePercentageForFee)) {
            uint256 amountETHForMintingCalc = (totalRewardsCollected.sub(lastRewardsForFeeCalc)).mul(stakingConfiguration.rewardFeePercentage()).div(100);
            uint256 amountHETHForMinting =  getAmountOfHETHforETH(amountETHForMintingCalc, false);

            _mint(stakingConfiguration.feeRecipient(), amountHETHForMinting);

            totalHETHMinted = totalHETHMinted.add(amountHETHForMinting);
            lastRewardsForFeeCalc = totalRewardsCollected;
            ...
            emit FeeCollected(stakingConfiguration.feeRecipient(), amountHETHForMinting);
        }
    }
    ...
    onlyMaintainer
    {
        ...
        liquidationReserves = liquidationReserves.sub(amountToWithdraw);
    ...
    whenNotPaused
    {
        ...
        uint256 amountForMint = getAmountOfHETHforETH(msg.value, true);

        // Mint hETH for user
        _mint(msg.sender, amountForMint);

        // Call deposit function on TokensFarmSDK contract
        tokensFarmSDK.deposit(
            msg.sender,
            amountForMint
        );
        ...
        liquidationReserves = liquidationReserves.add(msg.value);
     ...
     */
    function getAmountOfHETHforETH(uint256 amountETH, bool isContractCall) public view returns (uint256) {
        uint256 totalETHInPossession;

        if(isContractCall) {
            totalETHInPossession = totalBalanceETHInValidators.add(address(this).balance).sub(amountETH);
        } else {
            totalETHInPossession = totalBalanceETHInValidators.add(address(this).balance);
        }
    ...
    function getExecutionLayerRewards() public view returns(uint256) {
        return address(this).balance.sub(liquidationReserves);
    }
