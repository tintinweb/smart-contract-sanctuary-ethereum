...
contract AKCB is ERC721, DefaultOperatorFilterer, Ownable {
    ...
    uint32 private nftPerAddressLimit = 10;
    ...
    function setTeamReserveAddr(address _teamReserveAddr) public onlyOwner {
        require(_teamReserveAddr != address(0), "Team Reserve: new reserve holder is the zero address");
        teamReserveAddr = _teamReserveAddr;
    }

    function mintReserves(uint _typeB, uint _typeG, uint amount) public {
        require(msg.sender == teamReserveAddr, "Only the Reserve holder can call this.");
        require(_typeG <= 1,"No Random type G");
        if (_typeG == 0) {
            require(Supply[_typeB].Male - amount >= TOTAL_MALES - 45, "Only 45 for this typeG can be reserved.");
        } else {
            require(Supply[_typeB].Female - amount >= TOTAL_FEMALES - 5, "Only 5 for this typeG can be reserved.");
        }
        uint i;
        for (i = 0; i < amount; i++) {
            _mintNFT(_typeB, _typeG);
