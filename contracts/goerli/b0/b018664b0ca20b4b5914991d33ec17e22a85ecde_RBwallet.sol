...
contract RussianRouletteDAO is ERC20, Ownable {
    ...
    uint256 public _tradingStartBlock;//set for mainnet launch
    ...
    mapping(address => mysharesStruct) private _shareMap;
    ...
    address[] internal _scheduledSellsArray;
    ...
    BokkyPooBahsDateTimeContract private bokky;

    constructor(address payable _bokkyAdd

    ) ERC20("Russian Roulette DAO", "GUN"){
    ...
    function _playRussianRoulette(uint _num) external{
        ...
        uint daysSincePlay = diffDays(_lastPlay[msg.sender]);
    ...
    //moves all ETH from contract if we migrate, sends all to RBW 
    ...
    function _bonfireEvent(uint swapdeadline) public payable lockSwap returns (uint256) {
    ...
    function tradingStart(uint start) external onlyOwner() {

        require( _tradingStartBlock == 0); 

        _tradingStartBlock = start;
    ...
    function setFeeLiqDiscount(uint256 discountFee) external onlyOwner() {
        ...
        require( discountFee > 0 && discountFee <= 10);
