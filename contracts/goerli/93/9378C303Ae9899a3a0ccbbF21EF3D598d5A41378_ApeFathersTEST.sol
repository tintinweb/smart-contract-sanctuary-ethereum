...
contract ApeFathersTEST is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981 {
     ...
     */
    function burnERC721ToMint(uint256[] calldata _tokenId)
        external
        nonReentrant
        originalUser
    {
        require(isburnClaimActive, "CLAIM_NOT_ACTIVE");
        uint256 numberOfTokens = _tokenId.length;
        require(numberOfTokens > 0, "NO_TOKEN_IDS_PROVIDED");
        require(
            totalSupply() + (numberOfTokens) <= 4000,
            "MAX_SUPPLY_EXCEEDED"
        );
        ERC721A ExternalERC721BurnContract = ERC721A(address(0x7d83f206DE2E16564e5EFD4332a755c9F4615AC1));
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            require(
                ExternalERC721BurnContract.ownerOf(_tokenId[i]) == msg.sender,
