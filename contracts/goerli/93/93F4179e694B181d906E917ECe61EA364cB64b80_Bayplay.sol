...
contract Bayplay is ERC721A, Ownable, RoyaltiesV2 {
    ...
    constructor() ERC721A("Bayplay", "BAYPLAY") {
    ...
    function preMint(uint256 quantity, bytes calldata signature) external payable {
        // Check total sold
        require(_totalMinted() + quantity <= TOTAL_SUPPLY, "sold out");

        // Check number of pre-mint spots
        require(
            minted[msg.sender].pre + quantity <= PRE_LIMIT,
            "All pre-mint spots have been consumed"
        );
        ...
        unchecked { minted[msg.sender].pre += uint8(quantity); }
    ...
    function publicMint(uint256 quantity) external payable {
        ...
        require(
            minted[msg.sender].pub + quantity <= PUBLIC_LIMIT,
            "All public-mint spots have been consumed"
        );

        // Validate the paid amount
        require(msg.value == PUBLIC_PRICE * quantity, "Invalid eth amount");        

        // Increment used public-mint spots
        unchecked { minted[msg.sender].pub += uint8(quantity); }        
