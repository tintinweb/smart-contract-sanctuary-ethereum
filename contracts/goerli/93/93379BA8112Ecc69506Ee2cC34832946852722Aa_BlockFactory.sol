...
/**
 * @title : Block Land contract
 * 
 * @dev Single _M x N_ plot assets suppoert ERC-4907, whitelist mint
...
abstract contract Governance {
    ...
    address _keygovernance = address(0x3cd23e3fC14EC26Dbb96db285CF8CC7E9F04E5ea);
...
contract Nft_erc721 is ERC4907 {
   ...
   uint256 royaltyValue = 0.05 ether;
   ...
   struct Tile {
    ...
    constructor( address _address, string memory name, string memory symb) ERC4907( name, symb) {
        ...
        _setBaseURI(string(abi.encodePacked("https://pfp.allimeta.world/lands/block/", symb, "/")));
    }
    ...
    function X2Bytes(uint256 second, bool sw) internal pure returns (bytes memory b) {
            return abi.encode(second, sw);
    }
    
    function LeaseTransfer( address to, uint256 tokenId, uint256 times, bool enable ) external {

        safeTransferFrom( msg.sender, to, tokenId ,X2Bytes(times, enable) ) ;
        emit ToLease( msg.sender, to, tokenId ,times, enable );
    }
...
interface IALLIKEY {
    function metamint(address to) external returns (uint256); 
}
interface IGOVERLAND {
    function newmeta(address metaaddress, uint256 key_tokenid) external;
}

contract BlockFactory is Governance {
    event deployAddress(string _name, address _address);

    uint256 public currentDeployPrice;
    uint256 public currentTokenCost;
    address payable public withdrawWallet = payable(0x0);
    address public ErcToken =  address(0x0);
    address public AlliKeyAddr =  address(0x0);
    address public GoverLand = address(0x0);
    ...
    PrjInfo[] public allprj;

    mapping(bytes32 => bool) internal records;
    mapping(bytes32 => address) internal newnftaddr;
    ...
    function deployCollection(
        ...
        public payable
    {
         require(msg.value >= currentDeployPrice, "deploy value not enough!");
         require(amount >= currentTokenCost, "token amount not enough!");

         bytes32 hash = keccak256(abi.encodePacked(_projectname, this));
         require(records[hash] == false , "Invalid name"); 
         records[hash] = true;
        
        Nft_erc721 childContract = new Nft_erc721(address(msg.sender),_projectname,_projectsymbol);
        newnftaddr[hash] = address(childContract);

        if(ErcToken != address(0x0) && currentTokenCost > 0)
        {
            IERC20Token _ercx =  IERC20Token(ErcToken);
           _ercx.transferFrom(msg.sender, address(this), amount);
        }

        allprj.push( PrjInfo(msg.sender,_projectname,block.number) );
        emit deployAddress(_projectname, address(childContract));

        uint256 tokenid = 0;
        if( AlliKeyAddr != address(0x0) && GoverLand != address(0x0))
        {
            IALLIKEY _keycontract = IALLIKEY(AlliKeyAddr);
            tokenid = _keycontract.metamint(msg.sender);
            
            IGOVERLAND _gov = IGOVERLAND(GoverLand);
            _gov.newmeta( address(childContract), tokenid);
        }

    }

    function derived( string memory _projectname) public view returns (address addr)
    {
       bytes32 hash = keccak256(abi.encodePacked(_projectname, this));
       return newnftaddr[hash];
    }
    ...
    function setDepolyPrice(uint256 _price) public onlyGovernance {
        currentDeployPrice = _price;
    }
    ...
    function setTokenCost(uint256 _cost) external onlyGovernance {
        currentTokenCost = _cost;
    }
    
    function setAllikeyGovernance(address _keyaddr, address _coveraddr) public onlyGovernance {
        AlliKeyAddr = _keyaddr;
        GoverLand = _coveraddr;
    }

    function allSupply() public view returns(uint256) {
        return allprj.length;
    }

    function ownerSupply(address _owner) public view returns(uint256) {
        uint256 sum=0;
        for(uint256 i=0;i<allprj.length;i++)
        {
            if( allprj[i].prjowner == _owner)
            {   
                sum = sum + 1;
            }
        }
    ...
    function allProjects() public view returns(address[] memory) {
        address[] memory _addrs = new address[](allprj.length);
        for(uint256 i=0;i<allprj.length;i++)
        {
            _addrs[i] = newnftaddr[ keccak256(abi.encodePacked(allprj[i].prjname, this))];
        }
        return _addrs;
    }

    function ownerProjects(address _owner) public view returns(address[] memory) {
        uint256 j=0;
        address[] memory _addrs = new address[](ownerSupply(_owner));
        for(uint256 i=0;i<allprj.length;i++)
        {
            if( allprj[i].prjowner == _owner)
            {   
                address adr = newnftaddr[ keccak256(abi.encodePacked(allprj[i].prjname, this))];
                _addrs[j] = adr;
                j=j+1;
            }
        }
