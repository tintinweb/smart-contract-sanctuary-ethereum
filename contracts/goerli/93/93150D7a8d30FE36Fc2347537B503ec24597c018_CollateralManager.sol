...
library Bytes32SetLib {
    ...
    function getPage(Bytes32Set storage set, uint256 index, uint256 pageSize) internal view returns (bytes32[] memory) {
        // NOTE: This implementation should be converted to slice operators if the compiler is updated to v0.6.0+
        uint256 endIndex = index + pageSize; // The check below that endIndex <= index handles overflow.
...
contract CollateralManager is ICollateralManager, Owned, Pausable, MixinResolver {
    ...
    function getRatesAndTime(
        uint256 index
    ) public view returns (uint256 entryRate, uint256 lastRate, uint256 lastUpdated, uint256 newIndex) {
