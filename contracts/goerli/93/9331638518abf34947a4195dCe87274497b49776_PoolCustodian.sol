...
library RevolvingCreditLineErrors {
    ...
    error RCL_EXCLUDED_PHASE(DataTypes.OrderBookPhase excludedPhase); // "Pool must not be in the target phase"
...
library RevolvingCreditLineDeployer {
    function deploy(
        ...
        address instance = address(
            new RevolvingCreditLine(governance, custodian, feeConfigs, parametersConfig, name, symbol)
        );
        return instance;
    }
}
...
library TickLogic {
    ...
    function calculateYieldFactor(
        DataTypes.Tick storage tick,
        uint256 timestamp,
        uint256 rate
    ) internal view returns (uint256 newYieldFactor, uint256 accrualRate) {
        if (timestamp == tick.lastAccrualTimestamp) return (tick.yieldFactor, 0);
        accrualRate = (rate * (timestamp - tick.lastAccrualTimestamp)) / SECONDS_PER_YEAR;
        newYieldFactor =
            tick.yieldFactor +
            (tick.baseEpochsAmounts.borrowed + tick.baseEpochsAmounts.swappedAccrualsPaid).mul(accrualRate).div(
    ...
    function accrueYield(
        ...
        // get actualized yield values
        uint256 accrualRate;
        (tick.yieldFactor, accrualRate) = tick.calculateYieldFactor(timestamp, rate);
    ...
    function prepareTickForNextLoan(DataTypes.Tick storage tick, uint256 currentLoanId) external {
        ...
        tick.baseEpochsAmounts.available =
            ...
            tick.newEpochsAmounts.swappedAccrualsInterestsPaid +
            tick.baseEpochsAmounts.swappedAccrualsInterestsPaid +
        ...
        tick.adjustedDeposits =
            ...
            tick.newEpochsAmounts.swappedAccrualsInterestsPaid.div(tick.yieldFactor) -
        ...
        tick.newEpochsAmounts.swappedAccrualsInterestsPaid = 0;
        tick.baseEpochsAmounts.swappedAccrualsInterestsPaid = 0;
        tick.baseEpochsAmounts.swappedAccrualsPaid = 0;
        tick.baseEpochsAmounts.signalledAdjusted = 0;
        tick.newEpochsTotalAmountToBeAdjusted = 0;
        tick.loanAccruedInterests = 0;
        tick.newEpochsAmounts.signalledDeposits = 0;
        tick.newEpochsAmounts.signalledEarnings = 0;
    }
    ...
    function getAdjustedAmount(DataTypes.Tick storage tick, DataTypes.Position storage position)
        internal
        view
        returns (uint256 adjustedAmount)
    {
        DataTypes.Epoch storage epoch = tick.epochs[position.epochId];
        adjustedAmount = position.baseDeposit.div(tick.getEquivalentRatio(epoch));
    }
    ...
    function getPositionCurrentValue(
        ...
        DataTypes.Epoch storage epoch = tick.epochs[position.epochId];
        // a signalled position retains its value if its associated loan has been repaid
        bool isSignalledPositionAssociatedLoanRepaid = (position.signalLoanId > 0) &&
            ((position.signalLoanId > tick.lastLoanId) || (tick.baseEpochsAmounts.borrowed == 0));
        if (isSignalledPositionAssociatedLoanRepaid) {
            positionValue = tick.getAdjustedAmount(position).mul(tick.endOfLoanYieldFactors[position.signalLoanId]);
        }
        // current epoch is never borrowed
        else if (position.epochId == tick.currentEpochId) {
            positionValue = position.baseDeposit;
        }
        // new epochs for currently ongoing loan share a part of the expected fees
        else if (
            (position.epochId > tick.loanStartEpochId) && (currentMaturity > 0) && (tick.lastLoanId == currentLoanId)
        ) {
            uint256 unrealizedInterests = ((epoch.borrowed * (currentMaturity - block.timestamp)) / SECONDS_PER_YEAR)
                .mul(position.rate);
            positionValue = position.baseDeposit + epoch.endOfLoanEarnings - unrealizedInterests;
        }
        // all base epochs verify the position value = adjusted value * current yield factor invariant
        else {
            positionValue = tick.getAdjustedAmount(position).mul(yieldFactor);
        }
    }

    /// @dev Only holds when the position is currently borrowed
    function getPositionStartOfLoanValue(DataTypes.Tick storage tick, DataTypes.Position storage position)
        internal
        view
        returns (uint256 positionValue)
    {
        ...
        if (position.epochId <= tick.loanStartEpochId) {
            positionValue = tick.getAdjustedAmount(position).mul(
                tick.endOfLoanYieldFactors[epoch.precedentEpochLoanId]
            );
        } else {
            positionValue = position.baseDeposit;
        }
    ...
    function getPositionEndOfLoanValue(
        ...
        if (position.epochId <= tick.loanStartEpochId) {
            uint256 endOfLoanYieldFactor = tick.yieldFactor +
                ((tick.baseEpochsAmounts.borrowed * (currentMaturity - block.timestamp)) / SECONDS_PER_YEAR)
                    .mul(position.rate)
                    .div(tick.adjustedDeposits);
            positionValue = tick.getAdjustedAmount(position).mul(endOfLoanYieldFactor);
        } else {
            uint256 shareOfEpoch = position.baseDeposit.div(epoch.deposited);
            uint256 endOfLoanInterest = shareOfEpoch.mul(epoch.endOfLoanEarnings);
            positionValue = position.baseDeposit + endOfLoanInterest;
        }
    }

    function getPositionRepartition(DataTypes.Tick storage tick, DataTypes.Position storage position)
        internal
        view
        returns (uint256 unborrowedAmount, uint256 borrowedAmount)
    {
        DataTypes.Epoch storage epoch = tick.epochs[position.epochId];
        if (position.epochId <= tick.loanStartEpochId) {
            uint256 adjustedAmount = tick.getAdjustedAmount(position);
            uint256 shareOfEpoch = adjustedAmount.div(tick.adjustedDeposits);
            unborrowedAmount = shareOfEpoch.mul(tick.baseEpochsAmounts.available);
            borrowedAmount = shareOfEpoch.mul(tick.baseEpochsAmounts.borrowed);
        } else {
            uint256 shareOfEpoch = position.baseDeposit.div(epoch.deposited);
            unborrowedAmount = shareOfEpoch.mul(epoch.available);
            borrowedAmount = shareOfEpoch.mul(epoch.deposited - epoch.available);
        }
    }
    ...
    function deposit(DataTypes.Tick storage tick, uint256 amount) external {
        // TODO if the base epoch was exit, this condition does not guarantee that there is an ongoing loan
        if (tick.baseEpochsAmounts.borrowed > 0) {
            ...
            tick.epochs[tick.currentEpochId].available += amount;
        } else {
    ...
    function withdraw(
        ...
        if (position.signalLoanId > 0) {
            withdrawnAmount = tick.getPositionCurrentValue(position, currentMaturity, currentLoanId, tick.yieldFactor);
        }
        // when there's no loan ongoing, all positions are part of the base epoch
        else if (tick.baseEpochsAmounts.borrowed == 0) {
            withdrawnAmount = tick.getPositionCurrentValue(position, currentMaturity, currentLoanId, tick.yieldFactor);
            if (withdrawnAmount > tick.baseEpochsAmounts.available) {
                tick.baseEpochsAmounts.available = 0;
            } else {
                tick.baseEpochsAmounts.available -= withdrawnAmount;
            }
            tick.adjustedDeposits -= withdrawnAmount.div(tick.yieldFactor);
        }
        // when a loan is ongoing, only the current epoch is not borrowed
            ...
            withdrawnAmount = position.baseDeposit;
            tick.newEpochsTotalAmountToBeAdjusted -= withdrawnAmount;
        ...
        )
    {
        ...
        // get position values
        uint256 currentPositionValue = tick.getPositionCurrentValue(
            position,
            currentMaturity,
            currentLoanId,
            tick.yieldFactor
        );
        uint256 startOfLoanPositionValue = tick.getPositionStartOfLoanValue(position);
        uint256 endOfLoanPositionValue = tick.getPositionEndOfLoanValue(position, currentMaturity);
        uint256 borrowedAmount;
        (unborrowedAmount, borrowedAmount) = tick.getPositionRepartition(position);
        realizedInterests = currentPositionValue - startOfLoanPositionValue;
        endOfLoanBorrowEquivalent = endOfLoanPositionValue - realizedInterests - unborrowedAmount;
        ...
        // register the exit for new epochs
            ...
            epoch.available -= unborrowedAmount;
            /// @dev toExit does not need updating here because if it was already stored then it was based on the right proportions
            epoch.endOfLoanEarnings -= endOfLoanPositionValue - startOfLoanPositionValue;

            /// @dev precision issues can lead to overflows, hence we floor at zero
            bool isPositionAllNewEpochs = (currentPositionValue > tick.newEpochsTotalAmountToBeAdjusted) &&
                (currentPositionValue - tick.newEpochsTotalAmountToBeAdjusted) < 10;
            if (isPositionAllNewEpochs) currentPositionValue = tick.newEpochsTotalAmountToBeAdjusted;
            tick.newEpochsTotalAmountToBeAdjusted -= currentPositionValue;
        }
        tick.loanAccruedInterests -= realizedInterests;
    }
        ...
        )
    {
        ...
        if (tick.baseEpochsAmounts.available + tick.newEpochsAmounts.available > 0) {
            ...
            if (tick.baseEpochsAmounts.available > 0) {
                ...
                swappedAccruedInterests = (((args.totalAccrualsToSwap - remainingAccruals) *
                    (args.currentMaturity - block.timestamp)) / SECONDS_PER_YEAR).mul(args.rate);
                tick.baseEpochsAmounts.swappedAccrualsPaid += args.totalAccrualsToSwap - remainingAccruals;
                tick.baseEpochsAmounts.swappedAccrualsInterestsPaid += swappedAccruedInterests;
            }
            // else tap into new epoch potential partial fill
                ...
                if (!epoch.isBaseEpoch && epoch.available > 0 && epoch.borrowed > 0) {
                    (epochBorrowed, remainingAccruals, swappedAccruedInterests) = tick.borrowFromNew({
                        epoch: epoch,
                        toBeBorrowed: args.totalToBorrow,
                        rate: args.rate,
                        currentLoanId: args.currentLoanId,
            ...
            totalSwappedAccrualsInterests += swappedAccruedInterests;
        ...
        )
    {
        ...
        // TODO accrualsToBeAllocated accumulate end of loan earnings paid by exiting lender
        ...
        remainingAccruals = accruals - accrualsToBeAllocated;
        swappedAccrualsInterests = (((accruals - remainingAccruals) * (currentMaturity - block.timestamp)) /
            SECONDS_PER_YEAR).mul(rate);
        tick.newEpochsAmounts.swappedAccrualsInterestsPaid += swappedAccrualsInterests;
    }
}
...
interface IRCLLenders {
     ...
     * @param positionId The ID of the position
     */
    function signal(uint256 positionId) external;

    /**
     * @notice Retrieve the unborrowed and borrowed amounts of a position
     * @param positionId ID of the position
     * @return unborrowedAmount Unborrowed amount within the position
     * @return borrowedAmount Unborrowed amount within the position
     */
...
abstract contract RCLBorrowers is IRCLBorrowers, RCLOrderBook {
    ...
    function borrow(address to, uint256 amount) external onlyBorrower onlyInPhase(DataTypes.OrderBookPhase.OPEN) {
        ...
        while (remainingAmount > 0 && rate <= MAX_RATE) {
            (uint256 borrowedAmount, , ) = ticks[rate].borrow(
    ...
    function repay() external onlyBorrower notInPhase(DataTypes.OrderBookPhase.DEFAULT) {
...
abstract contract RCLGovernance is IRCLGovernance, RCLOrderBook {
    ...
    function close() external onlyOwner onlyInPhase(DataTypes.OrderBookPhase.OPEN) {
        orderBookPhase = DataTypes.OrderBookPhase.CLOSED;
    }

    function open() external onlyOwner onlyInPhase(DataTypes.OrderBookPhase.CLOSED) {
...
abstract contract RCLLenders is IRCLLenders, RCLOrderBook, ERC721 {
        ...
        returns (uint256 unborrowedAmount, uint256 borrowedAmount)
    {
        ...
        return tick.getPositionRepartition(position);
    }
    ...
    function getPositionCurrentValue(uint256 positionId) external view returns (uint256 positionCurrentValue) {
        ...
        (uint256 newYieldFactor, ) = tick.calculateYieldFactor(block.timestamp, position.rate);
        positionCurrentValue = tick.getPositionCurrentValue(position, currentMaturity, currentLoanId, newYieldFactor);
    }
    ...
    function withdraw(uint256 positionId) external notInPhase(DataTypes.OrderBookPhase.DEFAULT) {
        DataTypes.Position storage position = positions[positionId];
        DataTypes.Tick storage tick = ticks[position.rate];

        validateWithdraw(tick, positionId);

        uint256 withdrawableAmount = tick.withdraw(position, currentMaturity, currentLoanId);
    ...
    function updateRate(uint256 positionId, uint256 newRate) external onlyInPhase(DataTypes.OrderBookPhase.OPEN) {
        DataTypes.Position storage position = positions[positionId];
        DataTypes.Tick storage tick = ticks[position.rate];

        validateWithdraw(tick, positionId);
        uint256 updatedAmount = tick.withdraw(position, currentMaturity, currentLoanId);

        validateDepositRate(newRate);
        ticks[newRate].deposit(updatedAmount);

        position.baseDeposit = updatedAmount;
        position.rate = newRate;
        position.epochId = ticks[newRate].currentEpochId;
    ...
    function signal(uint256 positionId) external onlyInPhase(DataTypes.OrderBookPhase.OPEN) {
        ...
        if (position.epochId <= tick.loanStartEpochId) {
            tick.baseEpochsAmounts.signalledAdjusted += tick.getAdjustedAmount(position);
        }
        // if the position is from a new epoch, the exact earnings to be removed at the end of the loan will be computed later
        else {
            epoch.signalled += position.baseDeposit; // TODO only necessary when the target new epoch is partially filled
            tick.newEpochsAmounts.signalledDeposits += position.baseDeposit;
            tick.newEpochsAmounts.signalledEarnings += position.baseDeposit.mul(epoch.endOfLoanEarnings).div(
                epoch.deposited
            );
        }
    ...
    function exit(uint256 positionId) external notInPhase(DataTypes.OrderBookPhase.DEFAULT) {
        DataTypes.Position storage position = positions[positionId];
        DataTypes.Tick storage tick = ticks[position.rate];

        if (ownerOf(positionId) != _msgSender()) revert RevolvingCreditLineErrors.RCL_ONLY_OWNER();
        if (tick.baseEpochsAmounts.borrowed == 0 || (position.epochId == tick.currentEpochId))
            revert RevolvingCreditLineErrors.RCL_POSITION_NOT_BORROWED();
        if (position.signalLoanId > 0) revert RevolvingCreditLineErrors.RCL_HAS_SIGNAL_EXIT();
        if (block.timestamp > currentMaturity) revert RevolvingCreditLineErrors.RCL_CANNOT_EXIT();

        // compute position values to exit over other ticks
        (uint256 endOfLoanBorrowEquivalent, uint256 realizedAccruals, uint256 unborrowedAmount) = tick.registerExit(
            position,
            currentMaturity,
            currentLoanId
        );
        ...
        uint256 substitutedAccrualsToBuyer;
        uint256 remainingRealizedAccrualsToExit = realizedAccruals;
        uint256 rate = MIN_RATE;
        ...
        while (endOfLoanBorrowEquivalent > 10 && remainingRealizedAccrualsToExit > 10 && rate <= MAX_RATE) {
            // the switch between end of loan target and current tick equivalent is to conserve the same amount to repay at the end of the loan
            uint256 tickToBorrowEquivalent = toTickEquivalent(endOfLoanBorrowEquivalent, currentMaturity, rate);

            (uint256 tickBorrowed, uint256 accrualsRemaining, uint256 swappedInterestsAccruals) = ticks[rate].borrow(
                DataTypes.BorrowInput({
                    totalToBorrow: tickToBorrowEquivalent,
                    totalAccrualsToSwap: remainingRealizedAccrualsToExit,
                    currentMaturity: currentMaturity,
                    currentLoanId: currentLoanId,
                    rate: rate
                })
            ...
            totalSwapped += tickBorrowed;
            substitutedAccrualsToBuyer += swappedInterestsAccruals;

            // register data for next iteration
            endOfLoanBorrowEquivalent -= toEndOfLoanEquivalent(tickBorrowed, currentMaturity, rate);
            remainingRealizedAccrualsToExit = accrualsRemaining;
            rate += RATE_SPACING;
        }

        if (endOfLoanBorrowEquivalent > 10 || remainingRealizedAccrualsToExit > 10)
            revert RevolvingCreditLineErrors.RCL_NO_LIQUIDITY();

        _burn(positionId);
        delete positions[positionId];
        CUSTODIAN.withdraw(totalSwapped + unborrowedAmount + realizedAccruals - substitutedAccrualsToBuyer, msg.sender);

        emit Exit(totalSwapped + unborrowedAmount + realizedAccruals, position.rate, position.epochId);
    }
    ...
    function validateWithdraw(DataTypes.Tick storage tick, uint256 positionId) internal view {
        if (ownerOf(positionId) != _msgSender()) revert RevolvingCreditLineErrors.RCL_ONLY_OWNER();
        if (positions[positionId].creationTimestamp == block.timestamp) revert RevolvingCreditLineErrors.RCL_TIMELOCK();
        // a signalled position can't be withdrawn if its associated loan is still active
        // TODO if the base epoch was exit, this condition does not guarantee that there is an ongoing loan
        if ((positions[positionId].signalLoanId == currentLoanId) && (tick.baseEpochsAmounts.borrowed > 0))
            revert RevolvingCreditLineErrors.RCL_HAS_SIGNAL_EXIT();
    }

    function toTickEquivalent(
        uint256 endOfLoanEquivalent,
        uint256 currentMaturity,
        uint256 rate
    ) internal view returns (uint256 tickEquivalent) {
        uint256 currentValueToEndOfLoanMultiplier = ONE +
            ((ONE * (currentMaturity - block.timestamp)) / SECONDS_PER_YEAR).mul(rate);
        tickEquivalent = endOfLoanEquivalent.div(currentValueToEndOfLoanMultiplier);
    }

    function toEndOfLoanEquivalent(
        uint256 tickEquivalent,
        uint256 currentMaturity,
        uint256 rate
    ) internal view returns (uint256 endOfLoanEquivalent) {
        uint256 currentValueToEndOfLoanMultiplier = ONE +
            ((ONE * (currentMaturity - block.timestamp)) / SECONDS_PER_YEAR).mul(rate);
        endOfLoanEquivalent = tickEquivalent.mul(currentValueToEndOfLoanMultiplier);
    }
}
...
abstract contract RCLOrderBook is IRCLOrderBook, Ownable {
    ...
    function getTickGlobals(uint256 rate)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        return (
            tick.yieldFactor,
            tick.loanStartEpochId,
            tick.currentEpochId,
            tick.lastAccrualTimestamp,
            tick.loanAccruedInterests
        );
    }
    ...
    modifier notInPhase(DataTypes.OrderBookPhase excludedPhase) {
        if (orderBookPhase == excludedPhase) revert RevolvingCreditLineErrors.RCL_EXCLUDED_PHASE(excludedPhase);
        _;
    }
}
...
contract RCLFactory is IProductFactory, ERC165 {
    ...
    function deploy(
        ...
        instance = RevolvingCreditLineDeployer.deploy(
            governance,
            custodian,
            feeConfigs,
            parametersConfigs,
