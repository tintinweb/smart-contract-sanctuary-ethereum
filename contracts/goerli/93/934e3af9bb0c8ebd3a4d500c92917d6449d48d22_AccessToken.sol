...
contract AccessToken is ERC1155(''), Ownable {
    mapping(uint256 => address) public art;
    mapping(uint256 => address) public names;
    mapping(uint256 => address) public descriptions;
    ...
    function uri(uint256 tokenId) public override view returns (string memory) {
        require(tokenId < 1, "token id invalid (0 or 1)");
    ...
    function buildMetadata(uint256 tokenId) public view returns(string memory) {
        return string(abi.encodePacked(
              'data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
                          '{"name": "', string(SSTORE2.read(names[tokenId])), 
                          '", "description":"', 
                          string(SSTORE2.read(descriptions[tokenId])),
                          '", "image": "', 
                          'data:image/svg+xml;base64,', 
                          string(SSTORE2.read(art[tokenId])),
                          '"}')))));
    }

    function setItAll(uint256 tokenId, string memory svg, string memory name,
                            string memory desc) external onlyOwner {
        setArt(tokenId, svg);
        setName(tokenId, name);
        setDescription(tokenId, desc);
    }

    function setArt(uint256 tokenId, string memory svg) public onlyOwner {
        art[tokenId] = SSTORE2.write(bytes(svg));
    }

    function setName(uint256 tokenId, string memory name) public onlyOwner {
        names[tokenId] = SSTORE2.write(bytes(name));
    }

    function setDescription(uint256 tokenId, string memory desc) public onlyOwner {
        descriptions[tokenId] = SSTORE2.write(bytes(desc));
    }
}