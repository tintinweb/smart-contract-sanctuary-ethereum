...
// interface IArtworks {
//     function selectArtwork(uint artId) external view returns (string memory artwork);
// }
interface IOnChain {
    function getArtwork(uint tokenId) external view returns (PrmntArtwork memory a);
}

contract PrmntGenerative is ERC721Base{ // , Ownable 
    // mapping (uint256 => uint256) internal tokenHue;   
    ...
    function claim(uint256 tokenId, uint256 _amount, address _counter ) public {
        // require(_amount > 0 && _amount < 6);
        string memory tempURI = getURI(tokenId, _counter);
        mintTo(msg.sender, tempURI);
    }
    
    function getArtworkFromObject(address _counter, uint tokenId) public view returns (string memory artwork) {
        return IOnChain(_counter).getArtwork(tokenId).image;    
        // return PrmntArtwork({
        //     title: 'test',
        //     creator: 'test',
        //     description: 'test',
    ...
    function getArtwork(address _counter, uint hue, string memory seed, uint minId, uint maxId) public view returns (string memory artwork) {
        // IOnChain(_counter).getArtwork(tokenId).image,
        string memory hueColor = string.concat('hsl(', Strings.toString(hue), ', 60%, 60%)');
        string memory parts = string.concat('<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" width="640" height="640" viewBox="0 0 640 640" class="pa" ',
        ' style="background: ',
        string.concat('hsl(', Strings.toString(hue), ', 30%, 50%)'),
        ';">',
        // '>',
        createSymbol(_counter, seed, 'd1', minId, maxId),
        createSymbol(_counter, seed, 'd2', minId, maxId),
        createSymbol(_counter, seed, 'd3', minId, maxId),
        createSymbol(_counter, seed, 'd4', minId, maxId),
        '<use href="#d1" x="0" y="0" fill="',hueColor,'"  /><use href="#d2" x="320" y="0" fill="',hueColor,'" /><use href="#d3" x="0" y="320" fill="',hueColor,'" /><use href="#d4" x="320" y="320" fill="',hueColor,'" />',
        '</svg>');
        return parts;
    }

    function createSymbol(address _counter, string memory seed, string memory iteration, uint minId, uint maxId) internal view returns (string memory){
        string memory svg = IOnChain(_counter).getArtwork(randomInRange(minId,maxId, string.concat(seed, iteration))).image;
        return string.concat('<symbol id="',iteration, '" width="320" height="320"  viewBox="0 0 640 640" >',
            svg,
        '</symbol>');
    }
    
    function randomInRange(uint min, uint max, string memory input) internal view returns (uint256) {
        uint randomness = uint256(keccak256(abi.encodePacked(block.timestamp, input)));
        uint output = randomness % (max - min + 1) + min;
        return output;
    }
    
    function getURI(uint256 tokenId, address _counter) public view returns (string memory){
        string memory output = string.concat('data:application/json;base64,', 
            Base64.encode(
            abi.encodePacked(
                '{"name": "Generative Art", "description": "PRMNT generative art.", "image": "data:image/svg+xml;base64,', 
                Base64.encode(
                    abi.encodePacked(getArtwork(_counter,  tokenId, 'seedphase', 0, 5))
                ), 
                '"}'
            )
            )
