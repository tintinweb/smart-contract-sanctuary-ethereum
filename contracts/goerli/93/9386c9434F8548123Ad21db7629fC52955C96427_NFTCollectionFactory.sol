 ...
 */
contract NFTCollectionFactory is ICollectionFactory, Initializable, Gap1000 {
   ...
   */
  uint32 public versionCollection;
   ...
   * @param name The name of the collection contract created.
   * @param symbol The symbol of the collection contract created.
   * @param preRevealURIOrRevealedBaseURI The full path to the pre-reveal image of the collection
  ...
  event NFTDropCollectionCreated(
    ...
    string name,
    string symbol,
    string preRevealURIOrRevealedBaseURI,
  ...
  function adminUpdateCollectionImplementation(address _implementation) external onlyAdmin {
    ...
    INFTCollectionInitializer(_implementation).initialize(
      ...
      string.concat("NFT Collection Template v", versionCollection.toString()),
      string.concat("NFTv", versionCollection.toString())
    );

    emit ImplementationNFTCollectionUpdated(_implementation, versionCollection);
  }
  ...
  function createCollection(
    ...
    emit NFTCollectionCreated(collection, msg.sender, versionCollection, name, symbol, nonce);
  }
  ...
  function createNFTDropCollection(
    string calldata name,
    string calldata symbol,
    string calldata preRevealURIOrRevealedBaseURI,
    bytes32 postRevealBaseURIHash,
    uint32 maxTokenId,
