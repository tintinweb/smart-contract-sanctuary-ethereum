...
contract worldcup is ERC1155, Ownable, ReentrancyGuard {
    string public name;
    ...
    uint128 freeSupply = 1000;
    uint256 price = 0.01 ether;
    uint256 numTeams = 32;
    uint256 winningTeam;
    uint256 totalSupply;
    uint256 initialPrizeCapital;
    uint256 prizePool;
    bool publicActive;
    bool freeMintActive;
    bool contestCompleted;
    bool ownerHasWithdrawn;

    // teamID/tokenID => amount minted
    mapping(uint256 => uint256) public numMintedForTeam;
    mapping(address => uint8) public freeMintCounter;
    ...
    function setFreeSupply(uint128 _supply) external onlyOwner {
        freeSupply = _supply;
    }
    ...
    function getPrizePool() public view returns (uint256) {
        if (ownerHasWithdrawn) {
            ...
            return ((address(this).balance / 4) * 3);
        }
    }

    // random number for free mint
    function random() public view returns (uint256) {
        ...
        return (seed - ((seed / numTeams) * numTeams));
    }
        ...
        nonReentrant
    {
        ...
        require(totalSupply + 1 < freeSupply, "No more free mints allocated");
        require(freeMintCounter[msg.sender] == 0, "You have already minted");
        // check proof & mint
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, freeMintRoot, leaf),
            "Invalid MerkleProof"
        );
        freeMintCounter[msg.sender] = 1;
        totalSupply++;
        uint256 _tokenID = random();
        numMintedForTeam[_tokenID] += 1;
    ...
    function publicMint(uint8 quantity, uint128 _tokenID)
        ...
        nonReentrant
    {
        require(publicActive, "Pre mint is not active");
    ...
    function claimWinnings() external callerIsUser nonReentrant {
        ...
        uint256 winningPercent = balance / numMintedForTeam[winningTeam];
        _burn(msg.sender, winningTeam, balance);
        payable(msg.sender).transfer(winningPercent * prizePool);
    }

    // withdraw to owner wallet - 25% of proceeds not counting original ETH seeded into contract
    function withdraw() external onlyOwner {
        uint256 balance = (address(this).balance - initialPrizeCapital) / 4;
