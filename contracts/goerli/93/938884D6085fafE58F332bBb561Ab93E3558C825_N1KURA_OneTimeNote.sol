...
contract N1KURA_OneTimeNote {
    ...
    uint256 public contractBalance; // variable for storing contract balance
    
    constructor() payable {
        owner = msg.sender;
        contractBalance = msg.value; // initialize contract balance with initial ether sent to the contract
    }
    ...
    function deleteNote() public onlyOwner {
        ...
        contractBalance = 0; // zero out the contract balance after transferring all funds
    }
    ...
    function withdraw() public onlyOwner {
        ...
        contractBalance = 0; // zero out the contract balance after withdrawing all funds
    }
    
    // Receive Ether to the contract and update the contract balance
    function receiveEther() payable public {
        contractBalance += msg.value;
    }
    
    // Spend Ether from the contract to a recipient's address
    function spendEther(address payable recipientAddress, uint256 amount) public onlyOwner {
