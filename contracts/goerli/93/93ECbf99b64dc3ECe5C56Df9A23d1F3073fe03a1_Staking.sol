...
import "./library/ArrayAddresses.sol";
...
contract Staking is AccessControl, Pausable {
    ...
    using ArrayAddresses for address[];
    ...
    */
    function getForecast(uint256 daysForForecast) public view returns (uint256 amountOfAllRewards) {
        for (uint256 i = 0; i < _allAccounts.length; i++) {
            uint256 countStakes = getStakesCount(_allAccounts[i]);
            for (uint256 j = 0; j < countStakes; j++) {
                (uint256 rewardsSubValue, ) = getRewards(_allAccounts[i], j);
                amountOfAllRewards += (rewardsSubValue * daysForForecast);
            }
        }
    }
    ...
    */
    function claimAll() external whenNotPaused returns (bool) {
        uint256 countStakes = getStakesCount(msg.sender);
    ...
    */
    function _claim(uint256 id) internal {
        ...
        stake_.closed = true;
        _allAccounts.removeElement(caller);
