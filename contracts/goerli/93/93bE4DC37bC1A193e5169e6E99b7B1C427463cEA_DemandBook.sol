...
contract DemandBook is IDemandBook, Initializable, AbstractDependant {
    ...
    uint256 public constant override OPEN_PERIOD = 72 hours;
    uint256 public constant override PENDING_PERIOD = 24 hours;
    ...
    EnumerableSet.UintSet internal _lastDemandIndexes;
    mapping(uint256 => LostInDefi) internal _lostInDefi; // lastDemandIndex -> LostInDefi
    ...
    function demandStartTime(uint256 demandIndex) external view override returns (uint256) {
        return demandInfo[demandIndex].startTime;
    }
        ...
        )
    {
        ...
        currentLiquidity = _getCurrentLiquidity(demandIndex);
    ...
    function _getCurrentLiquidity(uint256 demandIndex)
        internal
        view
        returns (uint256 currentLiquidity)
    {
        currentLiquidity = demandInfo[demandIndex].depositedLiquidity.sub(
            demandInfo[demandIndex].lostLiquidity
        );
    }
    ...
    /// @dev use with countMyDemands() if listOption == ListOption.CREATED
    ...
    function getListDemands(
        ...
        for (uint256 i = offset; i < to; i = uncheckedInc(i)) {
            ...
            publicDemandInfo[newIndex].demandIndex = _demandsToIndex[info.demander][info.protocol];
            publicDemandInfo[newIndex].demander = info.demander;
            publicDemandInfo[newIndex].protocol = info.protocol;
            publicDemandInfo[newIndex].coverageAmount = info.coverageAmount;
            publicDemandInfo[newIndex].currentLiquidity = _getCurrentLiquidity(demandIndex);
            publicDemandInfo[newIndex].premiumAmount = info.premiumAmount;
            publicDemandInfo[newIndex].epochAmount = info.epochAmount;
            publicDemandInfo[newIndex].creationTime = info.creationTime;
            publicDemandInfo[newIndex].startTime = info.startTime;
            publicDemandInfo[newIndex].endTime = info.endTime;
            publicDemandInfo[newIndex].status = getDemandStatus(demandIndex);
            if (publicDemandInfo[newIndex].status == DemandStatus.OPEN) {
                publicDemandInfo[newIndex].time = info.creationTime.tryAdd(OPEN_PERIOD).trySub(
                    block.timestamp
                );
            } else if (publicDemandInfo[newIndex].status == DemandStatus.PENDING) {
                publicDemandInfo[newIndex].time = info
                    .creationTime
                    .tryAdd(OPEN_PERIOD)
                    .tryAdd(PENDING_PERIOD)
                    .trySub(block.timestamp);
            } else if (publicDemandInfo[newIndex].status == DemandStatus.ACTIVE) {
                publicDemandInfo[newIndex].time = info.endTime.tryAdd(STILL_CLAIMABLE_FOR).trySub(
                    block.timestamp
                );
            }
        }
    ...
    function updateLostInDefi(uint256 _lostPercentage) external override onlyCapitalPool {
        _lostInDefi[_demandIndex].percentageLost = _lostPercentage;
        _lastDemandIndexes.add(_demandIndex);
    }

    function _countLostInDefi() internal view returns (uint256) {
        return _lastDemandIndexes.length();
    }

    function excludeDefiLostLiquidity(uint256 demandIndex)
        external
        override
        onlyDemandBookLiquidity
    {
        _excludeDefiLostLiquidity(demandIndex, _countLostInDefi());
    }

    /// @dev triggered before requesting withdraw and submitting a claim
    /// @dev in case of previous defi loss, we update lost liquidity before any withdraw (liq or claim)
    function _excludeDefiLostLiquidity(uint256 demandIndex, uint256 limit) internal {
        for (uint256 i = 0; i < limit; i = uncheckedInc(i)) {
            uint256 lastDemandIndex = _lastDemandIndexes.at(i);
            if (
                demandIndex < lastDemandIndex &&
                !_lostInDefi[lastDemandIndex].demandsLossApplied.contains(demandIndex)
            ) {
                uint256 currentLiquidity = _getCurrentLiquidity(demandIndex);
                uint256 lostLiquidity =
                    currentLiquidity.mul(_lostInDefi[lastDemandIndex].percentageLost).uncheckedDiv(
                        PERCENTAGE_100
                    );
                demandInfo[demandIndex].lostLiquidity = demandInfo[demandIndex].lostLiquidity.add(
                    lostLiquidity
                );
                _lostInDefi[lastDemandIndex].demandsLossApplied.add(demandIndex);
            }
        }
    }
    ...
    /// @dev update liquidity from claiming at withdrawClaim()
    ...
    function getClaimApprovalAmount(uint256 demandIndex) external view override returns (uint256) {
    ...
    function _submitClaimAndInitializeVoting(
        ...
        _excludeDefiLostLiquidity(demandIndex, _countLostInDefi());
        uint256 coverage =
            Math.min(demandInfo[demandIndex].coverageAmount, _getCurrentLiquidity(demandIndex));
...
interface IDemandBookLiquidity {
    ...
    function getAllPendingWithdrawalRequestsAmount(bool isRebalancing, uint256 _limit)
        external
        view
        returns (uint256 totalLiquidityAmount);
