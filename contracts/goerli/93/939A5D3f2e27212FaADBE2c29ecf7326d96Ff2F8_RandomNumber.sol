...
contract RandomNumber is EIP712, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function validateAmountFunction(string memory _aString, string memory _aPoint, bytes memory _signature) internal view returns (address){
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
                keccak256("Number(string aString)"),
                keccak256(bytes(_aString)),
                keccak256(bytes(_aPoint))
            )));
    ...
    function _getRandomNumberInRange(string memory _tokenFromUniswap, string memory _point, uint _from, uint _to, bytes memory _signature) external view returns (uint)  {
      ...
      function _getRandomNumberWithNonceNumber(string memory _tokenFromUniswap, string memory _point, uint _from, uint _to, bytes memory _signature) external {
        require(_to > _from, "Range is not valid");
        require(validateAmountFunction(_tokenFromUniswap,_point, _signature) == msg.sender, "Not invalid");
        uint randomNumber = uint(
            keccak256(
