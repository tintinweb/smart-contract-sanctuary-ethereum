...
contract UniswapCrossFlash {
    ...
    function placeTrade(
        ...
        return amountReceived;
         ...
         // Check if the given Pair part of UNISWAP
    function checkUniSwapPossiblePair(
        address _fromToken,
        address _toToken,
        address UniSwapfactory
        
    )  public view returns (address _pair) {
        address pair = IUniswapV2Factory(UniSwapfactory).getPair(_fromToken, _toToken);
        return pair;
    }

             // Check if the given Pair part of SUSHISWAP
    function checkSushiSwapPossiblePair(
        address _fromToken,
        address _toToken,
        address SushiSwapfactory
        
    )  public view returns (address _pair) {
        address pair = IUniswapV2Factory(SushiSwapfactory).getPair(_fromToken, _toToken);
        return pair;
    }


    function checkTriangularPossibleTrade(
        address _Token1,
        address _Token2,
        address _Token3,
        uint256 _amountIn,
        ...
        pair = IUniswapV2Factory(Factory).getPair(_Token2,_Token3);
        console.log(pair);
        path[0] = _Token2 ;
        path[1] = _Token3;

        uint256 amountRequired2 = IUniswapV2Router01(Router).getAmountsOut(
            amountRequired,
            path
        )[1];


        pair = IUniswapV2Factory(Factory).getPair(_Token3,_Token1);
        console.log(pair);
        path[0] = _Token3;
        path[1] = _Token1;

        uint256 amountRequired3 = IUniswapV2Router01(Router).getAmountsOut(
            amountRequired2,
            path
        )[1];
        
        if(amountRequired3 > _amountIn)
        {
            result=true;
        }
        ...
        function checkSquarePossibleTrade(
        address _fromToken,
        address _toToken,
        uint256 _amountIn,
        address Factory,
        ...
        pair = IUniswapV2Factory(Factory).getPair(_toToken,_fromToken);
        console.log(pair);
        path[0] = _toToken ;
        path[1] = _fromToken;
     ...
     // Check if the swap is possible between UNISWAP and SUSHISWAP for a given pair
     
    function checkCrossPossibleTrade(
        address _fromToken,
        address _toToken,
        uint256 _amountIn,
        address UniSwapfactory,
        ...
        //IERC20(_fromToken).safeApprove(address(UNISWAP_ROUTER), MAX_INT);
        //IERC20(_toToken).safeApprove(address(UNISWAP_ROUTER), MAX_INT);
        address pair = IUniswapV2Factory(UniSwapfactory).getPair(_fromToken, _toToken);
        require(pair != address(0), "Pool does not exist");
        ...
        uint256 amountRequired = IUniswapV2Router01(UniSwaprouter).getAmountsOut(
            _amountIn,
            path
        )[1];
        
        pair = IUniswapV2Factory(SushiSwapfactory).getPair(_toToken,_fromToken);
        console.log(pair);
        path[0] = _toToken ;
        path[1] = _fromToken;

        uint256 amountRequired2 = IUniswapV2Router01(SushiSwaprouter).getAmountsOut(
            amountRequired,
            path
        )[1];
    ...
    function Test(uint256 _input)public view returns(uint256 _out){
        return 1;
    }
    ...
    function uniswapV2Call(
        ...
        bool reuslt = checkCrossPossibleTrade(
            USDC,
            LINK,
            loanAmount,
            UNISWAP_FACTORY,
