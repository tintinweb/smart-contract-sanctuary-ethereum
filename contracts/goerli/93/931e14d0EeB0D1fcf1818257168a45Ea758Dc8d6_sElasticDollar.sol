...
contract sElasticDollar is IsELASTIC, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgELASTIC public gELASTIC; // additional staked supply (governance token)

    Rebase[] public rebases; // past rebase data

    uint256 private constant MAX_UINT256 = type(uint256).max;
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 600_000_000 * 10**9;
    ...
    constructor() ERC20("Staked ELASTIC", "sELASTIC", 9) ERC20Permit("Staked ELASTIC") {
    ...
    function setgELASTIC(address _gELASTIC) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gELASTIC) == address(0), "gELASTIC:  gELASTIC already set");
        require(_gELASTIC != address(0), "gELASTIC:  gELASTIC is not a valid contract");
        gELASTIC = IgELASTIC(_gELASTIC);
    }
    ...
    /**
        @notice increases rELASTIC supply to increase staking balances relative to profit_
    ...
    function rebase(uint256 profit_, uint256 epoch_) public override onlyStakingContract returns (uint256) {
        ...
        _totalSupply = _totalSupply.add(profit_);
    ...
    // this function is called by the treasury, and informs sELASTIC of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sELASTIC
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sELASTIC: insufficient balance");
    }
    ...
    // toG converts an sELASTIC balance to gELASTIC terms. gELASTIC is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gELASTIC.balanceTo(amount);
    }

    // fromG converts a gELASTIC balance to sELASTIC terms. sELASTIC is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gELASTIC.balanceFrom(amount);
    }

    // Staking contract holds excess sELASTIC
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gELASTIC.balanceFrom(IERC20(address(gELASTIC)).totalSupply())).add(
