...
error Staker__NotOpen();
error Staker__NotComplete();
error Staker__TimeOut();
error Staker__NotYetTime();
error Staker__CanNotWithdraw();
error Staker__NoEnoughMoney();
...
contract Staker {
    ...
    StakerStats private s_staker_stats;
    ...
    constructor(address exampleExternalContractAddress) {
        ...
        s_staker_stats = StakerStats.OPENING;
    }
    ...
    function updateState() private {
        ...
        if (address(this).balance < THRESHOLD) {
            s_staker_stats = StakerStats.WITHDRAW;
            return;
        }
        s_staker_stats = StakerStats.COMPLETE;
    }
    ...
    function execute() public {
        ...
        if (s_staker_stats != StakerStats.COMPLETE) {
            return;
        }
    ...
    function withdraw() public {
        if (s_staker_stats != StakerStats.WITHDRAW) {
            revert Staker__CanNotWithdraw();
        }
        if (balances[msg.sender] == 0) {
            revert Staker__NoEnoughMoney();
        }
