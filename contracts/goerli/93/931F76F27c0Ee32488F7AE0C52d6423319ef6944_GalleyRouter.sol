...
contract GalleyRouter is ERC721Holder {
    function swapNFTsForToken(
        ILSSVMRouter.PairSwapSpecific[] calldata firstSwapList,
        ILSSVMRouter firstRouter,
        uint256 minOutput1,
        ILSSVMRouter.PairSwapSpecific[] calldata secondSwapList,
        ILSSVMRouter secondRouter,
        uint256 minOutput2,
        address tokenRecipient,
        uint256 deadline
    ) external returns (uint256 firstOutputValue, uint256 secondOutputValue) {
        uint256 numSwaps = firstSwapList.length;
        for (uint256 i; i < numSwaps; ) {
            ILSSVMRouter.PairSwapSpecific memory swapList = firstSwapList[i];
        ...
        uint256 numSwaps2 = secondSwapList.length;
        for (uint256 i; i < numSwaps2; ) {
            ILSSVMRouter.PairSwapSpecific memory swapList = secondSwapList[i];
            IERC721 nft = swapList.pair.nft();
            uint256 numNfts = swapList.nftIds.length;
            for (uint256 j; j < numNfts; ) {
                nft.safeTransferFrom(
        ...
        firstOutputValue = firstRouter.swapNFTsForToken(
            firstSwapList,
            minOutput1,
            tokenRecipient,
            deadline
        );
        secondOutputValue = secondRouter.swapNFTsForToken(
            secondSwapList,
            minOutput2,
            tokenRecipient,
            deadline
    ...
    {
        firstRemainingValue = firstRouter.swapETHForSpecificNFTs{value: value1}(
            firstSwapList,
            ethRecipient,
            nftRecipient,
            deadline
        );
        secondRemainingValue = secondRouter.swapETHForSpecificNFTs{
            value: value2
        }(secondSwapList, ethRecipient, nftRecipient, deadline);
    }

    function robustSwapETHForSpecificNFTsAndNFTsToToken(
        ILSSVMRouter.RobustPairNFTsFoTokenAndTokenforNFTsTrade
            calldata firstParams,
        ILSSVMRouter firstRouter,
        ILSSVMRouter.RobustPairNFTsFoTokenAndTokenforNFTsTrade
            calldata secondParams,
        ILSSVMRouter secondRouter
    )
        external
        payable
    ...
    {
        uint256 numSwaps = firstParams.nftToTokenTrades.length;
        for (uint256 i; i < numSwaps; ) {
            ILSSVMRouter.PairSwapSpecific memory swapList = firstParams
                .nftToTokenTrades[i]
                .swapInfo;
            IERC721 nft = swapList.pair.nft();
            uint256 numNfts = swapList.nftIds.length;
        ...
        uint256 numSwaps2 = secondParams.nftToTokenTrades.length;
        for (uint256 i; i < numSwaps2; ) {
            ILSSVMRouter.PairSwapSpecific memory swapList = secondParams
                .nftToTokenTrades[i]
                .swapInfo;
            IERC721 nft = swapList.pair.nft();
            uint256 numNfts = swapList.nftIds.length;
        ...
        (firstRemainingValue, firstOutputAmount) = firstRouter
            .robustSwapETHForSpecificNFTsAndNFTsToToken{
            value: firstParams.inputAmount
        }(firstParams);
        (secondRemainingValue, secondOutputAmount) = secondRouter
            .robustSwapETHForSpecificNFTsAndNFTsToToken{
            value: secondParams.inputAmount
        }(secondParams);
    }
