...
abstract contract DefaultOperatorFilterer is OperatorFilterer {
    ...
    contract TestnetTESTare is ERC721A, DefaultOperatorFilterer , Ownable {
    using Strings for uint256;
  ...
  constructor() ERC721A("TestnetTestare", "TSTA") {
  }


  function preSale(bytes32[] calldata __proof, uint8 _mintAmount) external payable  {
    ...
    require(isPreSaleActive, "Pre-sale is not yet active!");
  ...
  function publicSale(uint8 _mintAmount) external payable  {
     uint16 totalSupply = uint16(totalSupply());
     uint8 nft = NFTPerPublicAddress[msg.sender];
    require(totalSupply + _mintAmount <= maxSupply, "Exceeds max supply.");
    require(_mintAmount + nft <= maxMintAmountPerTx, "Exceeds max per transaction.");

    require(isPublicSaleActive, "Public sale is not yet active!");
