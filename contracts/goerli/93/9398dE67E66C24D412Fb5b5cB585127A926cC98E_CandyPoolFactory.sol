...
import "./CandyPool.sol";

// File: contracts/CandyPoolFactory.sol

contract CandyPoolFactory is Ownable {
    uint16 public constant MAX_FEE = 2000;

    uint16 public defaultDepositFee = 0;
    uint16 public defaultUnlockFee = 0;
    bool public defaultIsFlexibleLock = true;
    uint256 public defaultLockPeriod = 0;
    address public defaultFeeAddress =
        0x19E53469BdfD70e103B18D9De7627d88c4506DF2;

    event NewCandyPoolContract(address indexed smartChef);
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(CandyPool).creationCode;
        ...
        CandyPool(smartChefAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        CandyPool(smartChefAddress).updateFeePercentage(
            defaultDepositFee,
            defaultUnlockFee
        );
        CandyPool(smartChefAddress).updateLockProps(
            defaultLockPeriod,
            defaultIsFlexibleLock
        );
        CandyPool(smartChefAddress).updateFeeAddress(defaultFeeAddress);
        CandyPool(smartChefAddress).transferOwnership(_admin);

        emit NewCandyPoolContract(smartChefAddress);
    }
}
...
/// @notice Candy pool contract
/// Users stake token to earn other token. It supports native currency as well
contract CandyPool is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20Metadata;
    using UniversalERC20 for IERC20Metadata;

    // The address of the smart chef factory
    address public immutable CANDY_POOL_FACTORY;
    ...
    function initialize(
        ...
        require(msg.sender == CANDY_POOL_FACTORY, "Not factory");
        require(_admin != address(0), "Invalid admin");

        _stakedToken.universalBalanceOf(address(this));
        _rewardToken.universalBalanceOf(address(this));
        ...
        uint256 decimalsRewardToken = uint256(rewardToken.universalDecimals());
    ...
    function deposit(uint256 _amount) external payable nonReentrant {
        ...
        require(
            poolLimitPerUser == 0 || user.amount + _amount <= poolLimitPerUser,
        ...
        // Update user amount and total staked supply first against the exception case when the staking token is native currency
        if (_amount > 0) {
            _amount = stakedToken.universalTransferFromSenderToThis(_amount);
            uint256 feeAmount = 0;

            if (depositFee > 0) {
                feeAmount = (_amount * depositFee) / 10000;
                if (feeAmount > 0) {
                    stakedToken.universalTransfer(feeAddress, feeAmount);
                    _amount -= feeAmount;
                }
            }
            ...
            emit Deposited(msg.sender, _amount, feeAmount);
        }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (block.timestamp < user.lastDepositAt + lockPeriod) {
                ...
                withdrawFee = (_amount * unlockFee) / 10000;
                if (withdrawFee > 0) {
                    stakedToken.universalTransfer(feeAddress, withdrawFee);
                    _amount -= withdrawFee;
                }
            }
            if (_amount > 0) {
                stakedToken.universalTransfer(msg.sender, _amount);
            }

            emit Withdrawn(msg.sender, _amount, withdrawFee);
        }
    ...
    function safeRewardTransfer(address _to, uint256 _amount)
        ...
        returns (uint256)
    {
        uint256 rewardBalance = rewardToken.universalBalanceOf(address(this));
        ...
        rewardToken.universalTransfer(_to, _amount);
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            stakedToken.universalTransfer(msg.sender, amountToTransfer);
        }
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        require(
            ...
            "Not allowed while pool is live"
        );
        ...
        onlyOwner
    {
        ...
        IERC20Metadata(_tokenAddress).universalTransfer(
            msg.sender,
            _tokenAmount
        );
    }

    /// @notice Stop reward in the pool
    /// @dev Callable by owner only
    function stopReward() external onlyOwner {
        require(
            startBlock < block.number && block.number <= bonusEndBlock,
            "Pool not opened"
        );
     ...
     */
    function updatePoolLimitPerUser(uint256 _poolLimitPerUser)
        external
        onlyOwner
    {
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        uint256 pending = (user.amount * adjustedTokenPerShare) /
            PRECISION_FACTOR -
            user.rewardDebt;
