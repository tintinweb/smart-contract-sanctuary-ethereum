...
contract LucidLands is Ownable, BEP20{

    uint private feePercentage;

    mapping(address=>bool) private willGiveFee;

    address payable dev;


    constructor(uint _feePercentage, address payable _dev,  uint256 _initialSupply) BEP20("LucidLand", "LL"){
    ...
    /*
     ...
     @param _amount in wai the are 18 decimal points.
    ...
    */
    function transfer(address _to, uint256 _amount) public virtual override returns (bool) {
        ...
        if(willGiveFee[msg.sender] == false){
            uint256 fee = (_amount) * feePercentage / 100;
            
            
            uint _tranferableToken = _amount-fee;
            _balances[_to] += _tranferableToken;

            _balances[dev] += fee;
            emit Transfer(_msgSender(), _to, _amount-fee);
            //emit Transfer(_msgSender(), dev, fee);
        }else{
    ...
    function extractBEP20(address tokenAddress, address recipient)
        public
        onlyOwner
    {
        IBEP20 t = IBEP20(tokenAddress);
        t.transfer(recipient, t.balanceOf(address(this)));
    }



    function extractBNB(address _recipient) public onlyOwner {
