...
// File: nilliv1.1/RefundEscrowNilli.sol
...
contract RefundEscrowNilli is ConditionalEscrow {
    ...
    struct crowdContract {
        ...
        string condition2;
        string condition3;
        bool[4] conditionFlags; //3 conditions and 4th is jury decision
        uint targetAmount;
        uint expirationDate;
        uint qtySigners;
        State curState;
        ...
        uint expirationDate) 
    {
        _crowdContract = crowdContract({
            ...
            conditionFlags : [false, false, false, false],
     ...
     * further deposits.
     */
    function stateChangeContract(uint8 newState) public virtual onlyOwner {
        ...
        if (newState == 1) {
            _crowdContract.curState = State.Unlocked;
        }
        else if (newState == 2) {
            _crowdContract.curState = State.Refunded;
        }
        else if (newState == 3) {
            _crowdContract.curState = State.Delivered;
        }
        else {
            _crowdContract.curState = State.Locked;
        }
...
contract nilli is Ownable {
    ...
    event NilliContract(
        ...
        string condition2,
        string condition3,
        bool[4] conditionFlags, //3 conditions and jury decision is 4th
        uint targetAmount,
        uint expirationDate,
        uint qtySigners,
        State curState
     ...
     */
    function stateChangeContract(RefundEscrowNilli contractAddress, State newState) public onlyOwner {
        if (newState == State.Unlocked) {
            contractAddress.stateChangeContract(1);
        }
        else if (newState == State.Refunded) {
            contractAddress.stateChangeContract(2);
        }
        else if (newState == State.Delivered) {
            contractAddress.stateChangeContract(3);
        }
        else {
            contractAddress.stateChangeContract(0);
        }
    ...
    function withdrawTokenContract(RefundEscrowNilli contractAddress, IERC20 token) public {
        ...
        // TODO: need to add in event here???
    }
    ...
    //  /**
    //  * @dev if endowment Closed, the beneficiary can withdraw ETH from endowment.
    //  */
    // function withdrawETHOwner(RefundEscrowNilli contractAddress) public onlyOwner {
    //     contractAddress.withdrawETHOwner(payable(msg.sender));
    //     //TODO: need to add an event here???
    //     //TODO: or will this occur directly on RefundEscrowNilli contract?!?
    // }
