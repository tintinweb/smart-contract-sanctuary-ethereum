    ...
    PausableUpgradeable
{
        ...
        returns (uint256 totalValue)
    {
        ...
        if (_amount + totalTokensSold > rounds[0][currentStep]) {
            ...
            if (linearStartTime == 0 || linearStartTime >= block.timestamp) {
                ...
                if (!changeLinearStartTimeFlag) {
                    uint256 priceStep = (block.timestamp - linearStartTime) /
                        (12 * 60 * 60);
                    priceStep += 1;
                    USDTAmount =
                        (rounds[1][currentStep] + (priceStep * increment)) *
                        _amount;
                } else {
                    uint256 priceStep = (block.timestamp - linearStartTime) /
                        (7 * 24 * 60 * 60);
    ...
    function updateFromBSC(
        ...
        UserDeposits[] calldata _userDeposits
    ) external onlyOwner {
     ...
     */
    function updateInvestmentBonus(uint256[][2] memory _tokenQuantity)
        public
        onlyOwner
    {
        require(
    ...
    function changeLinearStartTime(uint256 _linearStartTime, uint256 _increment)
        external
    {
