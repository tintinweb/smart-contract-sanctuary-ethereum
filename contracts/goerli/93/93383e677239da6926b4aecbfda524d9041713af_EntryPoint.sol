...
contract EntryPoint is StakeManager, Ownable, ERC2771Context {
     ...
     */
    function handleOp(UserOperation calldata op, address payable beneficiary)
        public
    {
        uint preGas = gasleft();
        ...
        unchecked {
            ...
            ) = _validatePrepayment(0, op, requestId);
            ...
            try this.internalHandleOp(op, opInfo, context) returns (
                uint _actualGasCost
            ) {
                actualGasCost = _actualGasCost;
            } catch {
                uint actualGas = preGas - gasleft() + opInfo.preOpGas;
                actualGasCost = handlePostOp(
                    0,
    ...
    function internalHandleOp(
        UserOperation calldata op,
        UserOpInfo calldata opInfo,
        bytes calldata context
    ) external returns (uint actualGasCost) {
        uint preGas = gasleft();
    ...
    function simulateValidation(UserOperation calldata userOp)
        external
        returns (uint preOpGas, uint prefund)
    {
        uint preGas = gasleft();
    ...
    function _getPaymentInfo(UserOperation calldata userOp)
        internal
        view
        returns (uint requiredPrefund, PaymentMode paymentMode)
    {
    ...
    function getSenderAddress(bytes memory initCode, uint _salt)
        public
        view
        returns (address)
    {
    ...
    function _validateWalletPrepayment(
        ...
        PaymentMode paymentMode
    ) internal returns (uint gasUsedByValidateUserOp, uint prefund) {
        unchecked {
            uint preGas = gasleft();
    ...
    function _validatePaymasterPrepayment(
        ...
        unchecked {
            ...
            if (
                !isPaymasterStaked(
                    op.paymaster,
                    paymasterStake + requiredPreFund
                )
            ) {
            ...
            try
                IPaymaster(op.paymaster).validatePaymasterUserOp{gas: gas}(
                    op,
                    requestId,
                    requiredPreFund
                )
    ...
    {
        uint preGas = gasleft();
        uint maxGasValues = userOp.preVerificationGas |
        ...
        (gasUsedByValidateUserOp, prefund) = _validateWalletPrepayment(
            opIndex,
            userOp,
            requestId,
            requiredPreFund,
    ...
    function handlePostOp(
        ...
        bytes memory context,
        uint actualGas
    ) private returns (uint actualGasCost) {
        uint preGas = gasleft();
    ...
    function isPaymasterStaked(address paymaster, uint stake)
        public
        view
        returns (bool)
    {
        ...
        returns (uint)
    {
        unchecked {
            ...
            return
                min(
                    tx.gasprice,
                    min(maxFeePerGas, maxPriorityFeePerGas + block.basefee)
                );
        }
    }
