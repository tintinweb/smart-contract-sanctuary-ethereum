    ...
    Initializable
{
    ...
    event ClaimItem(ClaimInfo, RewardType);
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "ClaimInfo(address claimer,bytes32 itemId,uint256 shipPrice,uint256 timestamp)"
                        ),
                        _info.claimer,
                        _info.itemId,
                        _info.shipPrice,
    ...
    function claimItem(ClaimInfo calldata _info, RewardType _type) public payable nonReentrant {
        uint256 buyTime = _info.timestamp;
        uint256 limitTime = buyTime + 1 minutes;

        //verify
        address signer = _verify(_info);
        require(signer == signWallet, "not signed");
        require(block.timestamp <= limitTime, "Time's over");
        uint256 _amount = 0.1 ether;

        //transfer
        if(_type == RewardType.WBTC) {
            IWBTC.transferFrom(rewardWallet, _info.claimer, _amount);
        } else if (_type == RewardType.ETH) {
            address payable receiver = payable(msg.sender);
            (bool sent, ) = receiver.call{value: _amount}("");
            require(sent, "Failed to Receiver Wallet");
        } else if (_type == RewardType.USDT) {
            IUSDT.transferFrom(rewardWallet, _info.claimer, _amount);
        } else if (_type == RewardType.PHYS) {
            require(msg.value >= _info.shipPrice, "eth not enough");
            (bool sent, ) = adminWallet.call{value: _info.shipPrice}("");
            require(sent, "Failed to Admin Wallet");
        }

        emit ClaimItem(_info, _type);
    }
