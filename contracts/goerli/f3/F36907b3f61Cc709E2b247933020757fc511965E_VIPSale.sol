...
contract VIPSale is ERC721Psi, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 _mintAmount, uint256 _pid) public payable callerIsUser {
        TokenInfo storage tokens = AllowedCrypto[_pid];
        IERC20 paytoken;
        paytoken = tokens.paytoken;
        uint256 cost;
        cost = tokens.costvalue;
        uint256 presalePrice = cost * _mintAmount / (10**12);
        require(!paused,"The contract is currently paused, try again later");
        require(_mintAmount > 0, "You can't buy nothing");
        require(_mintAmount <= maxMintAmount, "Sorry, you just want too much");
        require(totalSupply() + _mintAmount <= maxSupply, "We don't sell that much");
        require(walletMints[msg.sender] + _mintAmount <= maxMintAmount, "Sorry, only ten NFTs per user");
        
        // Check USDC allowance
        uint256 ourAllowance = paytoken.allowance(
            _msgSender(),
            address(this)
        );
        require(presalePrice <= ourAllowance, "Make sure to add sufficient USDC allowance");

        // Check USDC balance
        uint256 balance = paytoken.balanceOf(
            _msgSender()
        );
        require(balance >= presalePrice);

            // Do the magic
            paytoken.transferFrom(msg.sender, address(this), presalePrice);
