...
contract ArbitrAI is IERC20Metadata, Ownable
{
    ...
    function _transfer(address sender, address recipient, uint amount) private {
        ...
        if (excludedFromFees[sender] || excludedFromFees[recipient])
            ...
            _LimitlessFunctionTransfer(sender, recipient, amount);
        }
        else {
            //once trading is enabled, it can't be turned off again
            require(LaunchTimestamp > 0, "trading not yet enabled");
    ...
    function _taxedTransfer(address sender, address recipient, uint amount) private {
        ...
        uint contractToken = _calculateFee(amount, tax, marketingTax + minerTax + liquidityTax);
    ...
    function _LimitlessFunctionTransfer(address sender, address recipient, uint amount) private {
        uint senderBalance = _balances[sender];
        require(senderBalance >= amount, "Transfer exceeds balance");
    ...
    function _swapContractToken(bool ignoreLimits) private lockTheSwap {
        ...
        uint totalTax = minerTax + marketingTax + liquidityTax;
        ...
        uint marketingETH = (address(this).balance * marketingTax) / TAX_DENOMINATOR;
        uint minerETH = (address(this).balance * minerTax) / TAX_DENOMINATOR;
        //Sends all the marketing ETH to the marketingWallet
        (bool sentMarketing,) = marketingWallet.call{value : marketingETH}("");
        (bool sentMiner,) = marketingWallet.call{value : minerETH}("");
        sentMarketing = true;
        sentMiner = true;
    }
