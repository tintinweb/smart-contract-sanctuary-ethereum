...
// minted bitmap
// transfer function
// once minted out -- allow admin mints with any id
// set URI functions
// set names + attribute functions

import "../Deployed/Random.sol";
...
contract Candies is ERC721, ERC721Royalty, AccessControl {
    ...
    string baseURI = "https://mustard-labs.s3.amazonaws.com/candies/wrapped/";
    string movURI = "https://mustard-labs.s3.amazonaws.com/candies/wrapped/";
    string unwrappedURI = "https://unwrapped-uri/";
    mapping(uint => string) private names;
    mapping(uint => string) private attributes;
    mapping(uint => uint) public codes;
    mapping(uint => uint) public counts;
    uint public totalCount;

    // uint private strawberryCount;
    // uint private butterscotchCount;
    // uint private redMintCount;
    // uint private greenMintCount;
    // Random.Manifest private _collectionDeck;
    // Random.Manifest private _strawberryDeck;
    // Random.Manifest private _butterscotchDeck;
    // Random.Manifest private _redMintDeck;
    // Random.Manifest private _greenMintDeck;
...
// Public
    ...
    function mint() public {
        ...
        // uint _pull = _collectionDeck.draw();
        // console.log("Pulled:", _pull, " type: ", getType(_pull));
        // _safeMint(to, _pull);
    }
...
// Admin Setup
    ...
    function add(uint _startIndex, uint8[] memory arr) public onlyRole(MINTER_ROLE) {
        ...
        for (uint i = _startIndex; i < arr.length; i++) {
            codes[i] = arr[i];
        }
    }

    function init() public onlyRole(MINTER_ROLE) {
        // _strawberryDeck.setup(strawberryCount);
        // _butterscotchDeck.setup(butterscotchCount);
        // _redMintDeck.setup(redMintCount);
        // _greenMintDeck.setup(greenMintCount);
        // totalCount = strawberryCount + butterscotchCount + redMintCount + greenMintCount;
        // _collectionDeck.setup(totalCount);
        // console.log("STARTING... Total count = ", totalCount);
    }

// View

    function getMetadataJson(uint _code, uint256 tokenId) public view returns (string memory) {
    
        string memory _attributes = string(abi.encodePacked('[\n{"value": "',names[_code / 10],'"},\n'));
        _attributes = string(abi.encodePacked(_attributes,attributes[_code % 10]));

        string memory meta = string(
            abi.encodePacked(
            '{\n"name": "Candy #', tokenId.toString(),
        ...
        meta = string(
            abi.encodePacked(
                meta,
                ',\n"external_url": "http://candies.shop/candies/', tokenId.toString(),
                '",\n"image": "', baseURI, tokenId.toString(),'.gif"',
                ',\n"video": "', movURI,  tokenId.toString(),'.mp4"'
            )
        );
...
// Admin
    ...
    function setCode(uint id, uint8 val) public onlyRole(MINTER_ROLE) {
        codes[id] = val;
    }
