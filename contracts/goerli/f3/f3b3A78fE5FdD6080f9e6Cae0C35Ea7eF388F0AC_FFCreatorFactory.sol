...
import "./FFCreatorV1A.sol";

contract FFCreatorFactory {
    event FFCollectionCreated(address creator, address indexed contractAddress);
    ...
    function createCollection(
        ...
        emit FFCollectionCreated(msg.sender, address(clone));
        FFCreatorV1A(payable(clone)).initialize(
            _name,
            _symbol,
            _placeholderURI
        );
        FFCreatorV1A(payable(clone)).transferOwnership(msg.sender);
    ...
    OwnableUpgradeable
{
    ...
    // TODO: Add string tokenURI - URI of the token minted
    ...
    // Function to receive ether, msg.value is not empty
    receive() external payable {
        require(msg.value > 0, "Insufficient input balance (0)");
    }

    // Fallback to receive ether, msg.value is not empty
    fallback() external payable {
        require(msg.value > 0, "Insufficient input balance (0)");
    }

    // @notice Release funds from the contract to the owner
    ...
    // @notice this will enable mint()
    function enableMinting() external onlyOwner {
        if (mintingActive) revert MintingActive();
        bool prev = mintingActive;
        mintingActive = true;
        emit UpdatedMintStatus(prev, mintingActive);
    }

    // @notice this will disable mint()
