...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    // Liquidity pool provider routerguar
    ...
    */
    ...
    function setMinTokensBeforeSwap(uint256 minTokensBeforeSwap_) public onlyOwner {
        ...
        /**
     * @dev Sets the max Tx amount.
     * Distribute transaction amount * `taxReward_` amount of tokens each transaction when enabled.
     *
     * Emits a {SetMaxTx} event.
     *
     * Requirements:
     *
     * Enabled by default. 
    */
    ...
    function changeSellTaxIncrease(uint32 amount) public onlyOwner {
        require(amount >= (_taxBurn + _taxReward + _taxMarketing) && amount <= 5, "Tax rate must be greater than total buy tax and lower than 5.");
...
contract MEMEMAFIA is ExtendedReflections {

    uint256 private _tokenSupply = 1_000_000_000;

    uint32 private _taxBurn = 1;
    uint32 private _taxReward = 1;
    uint32 private _taxMarketing = 1;

    uint32 private _sellTaxIncrease = 5;

    uint32 private _taxDecimals = 0;
    uint32 private _decimals = 18;

    uint256 private _minTokensBeforeSwap = 100_000 * (10 ** _decimals);

    address private _routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address private _marketingAddress = 0xCB90132346ba5723AD187FFfEB9235D3685e2e1b;

    constructor () ExtendedReflections("Meme Mafia", "MEMEMAFIA", _decimals, _tokenSupply) {
