...
contract NFTStaking is Ownable, Pausable{
    ...
    function withdrawAction(uint256 tokenID) internal {
        UserInfo storage info = userInfos[msg.sender];
    ...
    function rewardPeriodForToken(address user, uint256 tokenID) external view returns(uint256){
        return userInfos[user].rewardPeriod[tokenID];
    }
    ...
    function gettingRemainingTime(address user, uint256 tokenID) public view returns(uint256) {
        UserInfo storage info = userInfos[user];
        uint256 restDays = info.rewardPeriod[tokenID] - ((block.timestamp-info.staked[tokenID])%info.rewardPeriod[tokenID]);
        return restDays;
    }
