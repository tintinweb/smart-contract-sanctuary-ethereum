...
contract Trinity is ERC721A, ERC2981, Ownable {

  bool public isWhitelistMintingOpened;
  bool public isPublicMintingOpened;
  bool public isFreeMintingOpened;

// Il y a une limite de 3 NFT par wallet en prévente.
// En phase de vente publique, la limite est de 5 NFT par jour et par wallet.
// Les trinity sont vendus par 108 par jour.

// Attention : ne pas mélanger les mint vendus et ceux par burn
  uint16 public constant MAXSALES = 3000;
  uint16 public dailyMaxMint = 5;
  uint16 public maxWhiteListMint = 1000;
  ...
  address public unityAddress;
  address public deployer;

  event WhitelistMintFlipped(bool wlMint);
  event PublicMintFlipped(bool publicMint);
  event FreeMintFlipped(bool publicMint);
  event BaseURIUpdated(string newURI);
  event DailyMaxMintLimitUpdated(uint16 newLimit);
  ...
  event UnityAddressSet(address _a);

  constructor(
    address owner,
    string memory _uri,
    address _proxyRegistryAddress)
    ERC721A("AstroData Trinity","TRINITY")
    {
      require(owner != address(0), "Please provide a valid owner");
      baseURI = _uri;
      deployer = msg.sender;
      setDefaultRoyalty(owner, 500); // 5% fees
  ...
  function updateDailyMaxMint(uint16 newLimit) external onlyOwner {
    dailyMaxMint = newLimit;
    emit DailyMaxMintLimitUpdated(newLimit);
  }
  ...
  function _batchMint(address to, uint16 count) private {
    // TODO : changer ça - on ne doit pas mélanger les ventes avec les burn
    require(totalSupply() + count <= MAXSALES, "Collection is sold out");
    require(count <= dailyMaxMint, "Limit per transaction exeeded");
  ...
  /// @notice set the trinity contract's address for burning tokens
  /// @param _a the new address
  /// @dev can only be set once
  function setUnityAddress(address _a) external {
    require(msg.sender==deployer, "Only deployer");
    require(unityAddress==address(0), "Address already set");
    unityAddress = _a;
    emit UnityAddressSet(_a);
  }

  function claimTrinity(address to, uint16 count) external {
    require(msg.sender==unityAddress, "Only unity");
    _safeMint(to, count);
  }
  ...
  function merkleTreeWLMint(uint16 _count, bytes32[] memory _proof) external payable {
    ...
    // Part du principe que le while list mint a lieu avant les autres mints...
    require(totalSupply() + _count <= maxWhiteListMint, "Presale is sold out");
