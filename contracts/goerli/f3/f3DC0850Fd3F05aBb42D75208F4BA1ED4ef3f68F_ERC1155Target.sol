    ...
    IRoyaltySplitter
{
    ...
    function changeTokenUri(uint256 tokenId, string memory newuri) public onlyRole(DEFAULT_ADMIN_ROLE) {
        if (bytes(tokenURIs[tokenId]).length == 0) revert TokenDoesNotExist();
    ...
    function setOwner(address owner) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function mintWithUri(
        ...
        if (bytes(tokenURIs[id]).length > 0) revert TokenAlreadyExists();
        tokenURIs[id] = uri;
    ...
    function tokenSupplyTotal(uint256 tokenId) public view returns (uint256) {
    ...
    function setUpRecipients(
        ...
        for (uint256 i = 0; i < _royaltyRecipients.length; i++) {
            // todo: what is PayoutZeroAddress?
    ...
    function setRecipients(Recipient[] calldata _recipients) public onlyRole(MANAGER_ROLE)
    {
    ...
    */
    ...
    function setRoyaltyRecipient(address royaltyRecipient) public onlyRole(MANAGER_ROLE) {
...
/// @title Minimalist EIP2981 implementation.
/// @notice To be used within Quantum, as it was written for its needs.
/// @author exp.table / JCBDEV
abstract contract EIP2981 {
