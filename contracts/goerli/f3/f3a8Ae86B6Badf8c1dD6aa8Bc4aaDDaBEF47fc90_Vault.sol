...
library ArrayUint256Util {
  function indexOf(uint256[] storage values, uint256 value) internal view returns(bool, uint256) {
  ...
  function removeByIndex(uint256[] storage values, uint256 index) internal {
...
library ArrayAddressUtil {
  function indexOf(address[] storage values, address value) internal view returns(bool, uint256) {
    for (uint256 i = 0; i < values.length; i += 1) {
      if (values[i] == value) {
        return (true, i);
      }
  ...
  function removeByIndex(address[] storage values, uint256 index) internal {
    for (uint256 i = index; i < values.length - 1; i += 1) {
      values[i] = values[i+1];
    }
