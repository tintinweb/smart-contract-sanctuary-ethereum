 ...
 * @dev implementation of few functionalities are remaining
 * @custom:developmment contract is currently under development
 */
contract OTCDesk is ERC165, ReentrancyGuard, Pausable, AccessControl {
    using SafeERC20 for IERC20;
    using ERC165Checker for address;

    //EIP712 Initialization
    bytes32 internal DOMAIN_SEPARATOR;
    string internal MESSAGE_TYPE;
    ...
    constructor() {
        ...
        MESSAGE_TYPE = "Order(uint256 nonce,address maker,address tokenToSell,uint256 sellAmount,address tokenToBuy,uint256 buyAmount)";
    ...
    function supportsInterface(
        bytes4 interfaceID
    ) public view override(AccessControl, ERC165) returns (bool) {
        return
    ...
    function isValidPecentage(uint16 percentage) internal view returns (bool) {
    ...
    function tokenBlacklisting(
        ...
        require(isInvalidAddress(_token) == false, "token is not valid");
    ...
    function changePlatformFees(uint16 _platformFees) external onlyAdmin {
        require(
            isValidPecentage(_platformFees) == true,
            "fee can be b/w 0 and 100"
        );
    ...
    function setPlatformFeesRecipient(address _address) external onlyAdmin {
        require(isInvalidAddress(_address) == false, "address is not valid");
    ...
    function withdrawPlatformFees(
        ...
        require(
            isInvalidAddress(platformFeesRecipient) == false,
            "set platform fee recipient"
        );
        require(
            _tokens.length == percetageAmounts.length,
            "array lengths mismatch"
        );

        for (uint16 i = 0; i < _tokens.length; i++) {
            require(
                isValidPecentage(percetageAmounts[i]),
    ...
    function verifyOrder(
        ...
        require(_signature.length == 65, "wrong signature passed");
    ...
    function swap(
        ...
        require(
            (tokenBlacklist[tokenToBuy] == false) &&
                (tokenBlacklist[tokenToSell] == false),
            "tokens entered is/are blacklisted"
        );
        require(
            isInvalidInterface(tokenToBuy) == false,
            "invalid tokenToBuy interface"
        );
        require(
            isInvalidInterface(tokenToSell) == false,
            "invalid tokenToSell interface"
        );
        require(isInvalidAddress(maker) == false, "invalid maker address");
        require(
            isInvalidAddress(tokenToBuy) == false,
            "invalid tokenToBuy address"
        );
        require(
            isInvalidAddress(tokenToSell) == false,
            "invalid maker address"
        );
        require(tokenToBuy != tokenToSell, "both the tokens cannot be same");
        require(
            nonces[maker][nonce] == false,
            "nonce is similar to a pre-existing order"
        );
        require(
            verifyOrder(
                nonce,
        ...
        emit sellOrder(orderID, maker, tokenToSell, sellAmount);
        emit buyOrder(orderID, msg.sender, tokenToBuy, buyAmount);
    }
}