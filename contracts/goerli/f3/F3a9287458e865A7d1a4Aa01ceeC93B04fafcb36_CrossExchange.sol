...
contract CrossExchange is Initializable, ReentrancyGuardUpgradeable {
    ...
    struct Exchange {
        uint256 amount;
        address signContractAddress; // eth to bsc -> eth
        uint256 orderType; //1-eth,56-bsc,137-polygon
        uint256 toAmount;
        uint256 toChain;
        bytes signature;
    }

    string private constant names = "Exchange-signature";
    ...
    event CrossTokenExchange(
        address to,
        uint256 amount,
        uint256 network,
        address[] path
    );
    event ExecuteCoinPayment(
        address from,
        address contractAddress,
        address to,
        uint256[6] details,
    ...
    event ExecuteCrossCoinSwap(address to, uint256 network, uint256 amount);
    ...
    function initialize(address _owner, address _vault) external initializer {
        owner = _owner;
        vault = _vault;
        // Uni = IUni(0x8cfD38eb0163BC597fA3BdA5381165972dc2e967);
        // USDT = address(0x83aD7BBC3961b2b2e80C7c1A5E803ef171B966B8);
        // Uni = IUni(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        // USDT = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
        Uni = IUni(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        USDT = address(0x07865c6E87B9F70255377e024ace6630C1Eaa37F);
    }
    ...
    function setUSDT(address usdt) external nonReentrant onlyOwner {
        USDT = usdt;
    }
    ...
    {
        require(msg.value >= _amount, "amount not enough");
        uint256 usdtAmountExpected = _swap(
            OrderType.EthForTokens,
            path,
            msg.value,
            1,
            vault,
    ...
    function executeTokenExchange(
        ...
        OrderType orderType,
        OrderType crossOrderType,
        uint256[] calldata details, //[assetInOffered,minExpectedAmount,deadline,_network,_toAmount,_toNetwork]
        uint8 v,
        bytes32 r,
        bytes32 s
    ) public payable {
        if (orderType == OrderType.EthForTokens) {
            require(msg.value >= (details[0]), "Payment = assetInOffered");
        }
    ...
    function crossCoinPayment(
        ...
        bytes32 r,
        bytes32 s
    ) external whenNotPaused isBlacklist(msg.sender) {
    ...
    function executeCrossCoinExchange(
        address signContractAddress,
        address user,
        address[] calldata path,
        uint256[] calldata detials, //[amount,_network,toAmount,toChain,usdtAmount,_deadline]
        uint8 v,
        bytes32 r,
        bytes32 s
    ) external whenNotPaused isBlacklist(msg.sender) {
        ...
        uint256 expectedCoin = _swap(
            OrderType.TokensForEth,
            path,
            detials[4],
            1,
            user,
        ...
        emit ExecuteCrossCoinSwap(user, detials[3], expectedCoin);
    }

    function executeCrossTokenExchange(
        address signContractAddress,
        address user,
        address[] memory path,
        OrderType crossOrderType,
        uint256[] calldata details, //[usdtAmount,minExpectedAmount,deadline,_network,_toAmount,_toNetwork,assetInOffered]
        uint8 v,
        bytes32 r,
        bytes32 s
    ) public payable {
        ...
        if (path[path.length - 1] == USDT) {
            ...
            emit CrossTokenExchange(user, details[0], details[5], path);
        } else {
            uint256 minAmountExpected = _swap(
                crossOrderType,
                path,
            ...
            emit CrossTokenExchange(user, minAmountExpected, details[5], path);
        }
    }
    ...
    function executeSetIfSignatureMatch(
        ...
        bytes32 eip712DomainHash = keccak256(
            abi.encode(
                ...
                keccak256(bytes("Exchange-signature")),
                keccak256(bytes("1"))
            )
        );
        ...
        bytes32 hashStruct = keccak256(
            abi.encode(
                keccak256(
                    "Permit(uint256 amount,address signContractAddress,uint256 orderType,uint256 toAmount,uint256 toChain)"
                ),
                amount,
                signContractAddress,
                orderType,
