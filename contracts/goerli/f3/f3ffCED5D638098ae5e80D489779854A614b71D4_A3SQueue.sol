...
contract A3SQueue is IA3SQueue, Ownable {
    ...
    function updateBMPlayer(uint256 newBMPlayer) external override onlyOwner {
        ...
        emit UpdateSwitches(newBMPlayer, gameStartSwitch, prioritySwitch, outQueueSwitch);
    }
    
    /**
     * @dev See {IA3SQueue-updateLockingDays}.
     */
    function updateLockingPeriod(uint32 newlockingPeriod)
        external
        override
        onlyOwner
    {
    ...
    function updateQueueLenExtCountThld(uint64 maximumQLExtThld)
        external
        onlyOwner
    {
    ...
    function pushIn(address addr) external override ONLY_GAMESTART {
        ...
        if(!prioritySwitch){
            ...
            uint256 tokenId = IA3SWalletFactoryV3(A3SWalletFactory).walletIdOf(addr);
     ...
     */
    function batchMint(address[] memory addrs)
        external
        override
        ONLY_GAMESTART
    {
    ...
    function _pushOut() internal {
        ...
        emit PushedOut(
            _cur_addr,
            addressNode[_cur_addr].outQueueTime,
            headIdx,
            tailIdx,
            curQueueLength
        );
    ...
    function _jumpToTail(address _addr) internal {
        ...
        require(
            ...
            "A3S: invalid queue status - must be inqueue to jump"
        );
        ...
        require(
            IERC20(token).transferFrom(msg.sender, vault, _payment),
            "A3S: Jump to tail payment failed!"
        );
...
library A3SQueueHelper {
    ...
    function _steal(
        ...
        //Update balance for stealing address(tail): add stolen balance to the tailIdx address
        uint256 new_balance = _addressNode[_tailIdx].balance + _balance;
        _addressNode[_tailIdx].balance = new_balance;

        emit Steal(_tailIdx, _addr, _balance);
    }

    function _getJumpToTailFee(uint256 _inQueueTimestamp)
        internal
        view
        returns (uint256)
    {
    ...
    function _getTokenAmount(
        ...
        uint256 _diffID = a3sContract.tokenIdCounter() - _currID + 1;
        //N = 1.1 + 0.1 * ()
...
interface IA3SQueue {
    ...
    event UpdateSwitches(uint256 tokenId, bool gameStartSwitch, bool prioritySwitch, bool outQueueSwitch);
}
