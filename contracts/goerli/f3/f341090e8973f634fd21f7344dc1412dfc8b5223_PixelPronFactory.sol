...
contract ERC721A is IERC721A {
    ...
    uint256 immutable _maxSupply = 2000;
...
function _mintIdWithoutBalanceUpdate(address to, uint256 tokenId) private {
    ...
    function _mint(address to, uint _numToMint) internal virtual {
    ...
    function _safeMint(
        ...
        _mintIdWithoutBalanceUpdate(to, _maxSupply - _numAvailableTokens);
...
contract PixelPronFactory is ERC721A, Ownable {
    uint256 MAX_MINTS = 500;
    uint256 MAX_SUPPLY = 2000;
    uint256 public PRICE = 0.005 ether;
    bool PAUSED = true;
    string public uriPrefix = "https://pixels.mypinata.cloud/ipfs/QmZYiJM45XhUfFtqEkxtyVtQ8mVDJkcDEvFT2SAyMCT1cc/{";
    string public uriSuffix = ".json";
    bytes public data;

    constructor() ERC721A("Pixel Pron", "SHRIMP") {}

    function mint(uint256 quantity) public payable {
        
        require(quantity + _numberMinted(msg.sender) <= MAX_MINTS, "Over Mint Limit!");
        require(totalSupply() + quantity <= MAX_SUPPLY, "No more tokens to mint.");
        require(PAUSED == false, "MINT PAUSED");
        require(msg.value >= quantity * PRICE);
        _randomSafeMint(msg.sender, quantity, data);
    }
        ...
        require(quantity + _numberMinted(msg.sender) <= MAX_MINTS, "Over Mint Limit!");
        require(totalSupply() + quantity <= MAX_SUPPLY, "No more tokens to mint.");
        _safeMint(msg.sender, quantity, data);
    }
