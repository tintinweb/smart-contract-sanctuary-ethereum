...
// File: contracts/regcontract.sol
...
contract GGNFTUploadSample is ERC721A, Ownable {
  ...
  mapping(uint256 => string) public NFTMap;
  ...
  mapping(string => uint256) public DailyContractCount;
  mapping(string => uint256) public MonthlyContractCount;
  bool public isContractPaused = false;


  constructor(
  )  ERC721A("GGAgriNFT", "GGANFT") {
    OrgAdmins[_msgSender()] = 1;
  }

 
  function isOrgAdmin(address _add) public view returns (bool) {
    return OrgAdmins[_add] > 0
        ? true
        : false;
  }

  function addOrgAdmin(address _orgAdmin) public onlyOwner {
     OrgAdmins[_orgAdmin] = 1;
  }


  function removeOrgAdmin(address _orgAdmin) public onlyOwner {
     OrgAdmins[_orgAdmin] = 0;
  }

  function setContractState(bool _pauseState) public {
    require(isOrgAdmin(msg.sender) == true, 'Only admins can set state.');
    isContractPaused = _pauseState;
  }

  function setPrice(uint256 _price) public {
    require(isOrgAdmin(msg.sender) == true, 'Only admins can set price.');
  ...
  function addContract(string memory _category,
                      ...
                      string memory _IPFSURI, string memory _month) public payable {
    uint256 supply;
    ...
    MonthlyContractCount[_month] += 1;
    supply = supply + 1;
    NFTMap[supply] = _SHA256;
    mint();
  }
  ...
  function getContractsCount_month(string memory _createmth) public view returns ( uint256 ) {
    return MonthlyContractCount[_createmth];
  }
  ...
  function getContractbyCreator(address _add, uint256 _index) public view returns ( structContractInfo memory ) {
    unchecked {
      ...
      require(_add == msg.sender || isOrgAdmin(_add) == true, 'Only owner can see contracts.');
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    structContractInfo memory nftInfo;
    nftInfo = ContractsMap[NFTMap[_tokenId]];

    currentBaseURI = nftInfo.IPFSURI;

    return bytes(currentBaseURI).length > 10 ? currentBaseURI : "";
