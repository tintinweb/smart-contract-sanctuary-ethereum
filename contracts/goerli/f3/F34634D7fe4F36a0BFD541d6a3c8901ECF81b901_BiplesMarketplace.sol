...
interface IBiplesRoyaltyRegistry {
...
interface IBiplesTokenRegistry {
    function enabled(address) external view returns (bool);
}

interface IBiplesPriceFeed {
    function wETH() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract BiplesMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event UpdateServiceFee(uint16 serviceFee);
    event UpdateServiceFeeRecipient(address payable serviceFeeRecipient);
    ...
    function initialize(address payable _feeRecipient, uint16 _serviceFee)
        public
        initializer
    {
    ...
    function _buyItem(
        ...
        uint256 feeAmount = (price * serviceFee) / 1e3;
        ...
        IBiplesRoyaltyRegistry royaltyRegistry = IBiplesRoyaltyRegistry(
        ...
        if (minter != address(0) && royaltyAmount != 0) {
            ...
            feeAmount = feeAmount + royaltyAmount;
        }
        IERC20Upgradeable(_payToken).safeTransferFrom(
            _msgSender(),
            _owner,
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IBiplesPriceFeed priceFeed = IBiplesPriceFeed(
    ...
    /**
     @notice Method for updating service Fee
     @dev Only admin
     @param _serviceFee uint16 the service Fee to set
     */
    ...
    /**
     @notice Method for updating service Fee address
     @dev Only admin
     @param _serviceFeeRecipient payable address the address to sends the funds to
     */
    function updateServiceFeeRecipient(address payable _serviceFeeRecipient)
        external
        onlyOwner
    {
        feeReceipient = _serviceFeeRecipient;
        emit UpdateServiceFeeRecipient(_serviceFeeRecipient);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IBiplesTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
