...
contract CrossChainCoinBook is ICrossChainCoinBook, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 private initialId = 1000000000001;
    ...
    ) external override onlyMatcher nonReentrant returns (
        address seller, 
        uint256 taxAmount, 
        uint256 finalAmount
    ) {
        require(_orderStatus(_id) == 1 && !orderId[_id].settled, "Order settled canceled or queued");
     ...
     */
    function executeOrder(uint256 _id, 
        uint16 orderChain, 
        uint256 _index, 
        IERC20 _token, 
        uint256 _amount
        ...
        emit OrderQueued(_id, orderChain, msg.sender, _token, _index, _amount);
    }
    ...
    function finalizeQueuedOrder(
        ...
        require(orderQueued[_id] && queuedOrder[_id].buyer == buyer, "Order can not be unqueued now");
        ...
        emit OrderFinalized(_id, orderChain, buyer, seller, amount);
    }
