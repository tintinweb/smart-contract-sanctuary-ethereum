...
contract SxTCommunity is ISxTCommunity, SxTCommunityStorage, ERC1155, Ownable, ReentrancyGuard, Pausable {
    ...
    function mintNFT(uint256 id) external override whenNotPaused nonReentrant{   
        require(!compareStrings(tokenDetails[id].tokenUri, ""), "SxTCommunity: URI nonexistent token");    
        ...
        require(!isPreviouslyMintedFrom[id][_msgSender()], "SxTCommunity: Already minted this NFT once to this account");
        require(whitelist[_msgSender()], "SxTCommunity: This address is not in the whitelist. Go to the Space and Time discord and comment your wallet address in the whitelist channel before minting!");
        isPreviouslyMintedFrom[id][_msgSender()] = true;
        Token storage currentToken = tokenDetails[id];
        currentToken.currentTokenSupply += 1;
        _mint(_msgSender(), id, AMOUNT_BUYABLE, "");
        emit NftMinted(id, _msgSender());
    }
    ...
    function mintNFTUsingEth(uint256 id) external override payable whenNotPaused nonReentrant{
        require(!compareStrings(tokenDetails[id].tokenUri, ""), "SxTCommunity: URI nonexistent token");  
        ...
        currentToken.currentTokenSupply += 1;
        _mint(_msgSender(), id, AMOUNT_BUYABLE, "");
        emit NftMintedUsingEth(id, _msgSender(), tokenDetails[id].ethPrice );
    }
    ...
    function mintNFTUsingERC20(uint256 id) external override whenNotPaused nonReentrant{
        ...
        require(!checkMaxSupplyReached(id), "SxTCommunity: Total supply exceeded maximum supply");
        require(!isPreviouslyMintedFrom[id][_msgSender()], "SxTCommunity: Already minted this NFT once to this account");
        require(sxtToken.balanceOf(_msgSender()) >= tokenDetails[id].erc20Price, "SxTCommunity: Insufficient ERC20 token balance");
        require(whitelist[_msgSender()], "SxTCommunity: This address is not in the whitelist. Go to the Space and Time discord and comment your wallet address in the whitelist channel before minting!");
        isPreviouslyMintedFrom[id][_msgSender()] = true;
        bool sent = sxtToken.transferFrom(_msgSender(), address(this), tokenDetails[id].erc20Price);
        require(sent, "SxTCommunity: Failed to send ERC20Token");
        Token storage currentToken = tokenDetails[id];
        currentToken.currentTokenSupply += 1;
        _mint(_msgSender(), id, AMOUNT_BUYABLE, "");
        emit NftMintedUsingERC20(id, _msgSender(), tokenDetails[id].erc20Price);
    }
    ...
    function addToWhitelist(address toAddAddresses) external override onlyOwner {
        require(address(toAddAddresses) != ZERO_ADDRESS, "SxTCommunity: Address Cannot be Zero Address");
        whitelist[toAddAddresses] = true;
        sxtCommunityAddresses.push(toAddAddresses);
    }

    /// @dev add an array of addresses to the whitelist

    function addArrayToWhitelist(address[] calldata toAddAddresses) external onlyOwner {
        for (uint i = 0; i < toAddAddresses.length; i++) {
            if(address(toAddAddresses[i]) != ZERO_ADDRESS){
                whitelist[toAddAddresses[i]] = true;
                sxtCommunityAddresses.push(toAddAddresses[i]);
            }
        }
    }

    /// @dev removes an adress from the whitelist

    function removeFromWhitelist(address[] calldata toRemoveAddresses) external override onlyOwner {
        for (uint i = 0; i < toRemoveAddresses.length; i++) {
            delete whitelist[toRemoveAddresses[i]];
        }
    }

    /// @dev returns the sxt community addresses

    function returnSXTCommunity() public view returns(address[] memory) {
        return sxtCommunityAddresses;
    }
...
interface ISxTCommunity {
    ...
    function mintNFTUsingEth(uint256 id) external payable;
    ...
    function mintNFTUsingERC20(uint256 id) external;
    ...
    function addToWhitelist(address toAddAddresses) external;
...
contract SxTCommunityStorage {
    ...
    //Whitelist of address's
    mapping(address => bool) public whitelist;
    
    //Array for address retrieval 
    address[] public sxtCommunityAddresses;
}
