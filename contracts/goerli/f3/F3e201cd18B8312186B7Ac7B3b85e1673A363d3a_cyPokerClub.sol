...
contract cyPokerClub is Initializable, ERC721Upgradeable, EIP712Upgradeable,ERC721URIStorageUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    string private constant SIGNING_DOMAIN = "CPC-Voucher";
    ...
    mapping(uint256 => string) private _tokenURIs;
    // unleash it!!!
    mapping(uint256 => bool) public leashed;
    mapping(uint256 => bool) public ableToUnleash;
    event AbleToUnleash(uint256 tokenId);
    event Leashed(uint256 tokenId);
    ...
    function initialize(
        ...
        __ERC721_init("Cyber Poker Club", "CPC");
    ...
    function redeem(NFTVoucher calldata voucher) public returns (uint256) {
        ...
        leashed[tokenId] = false;
        ableToUnleash[tokenId] = false;
    ...
    function safeMintByOwner(address userAddr) public onlyOwner {
    ...
    //unleash the mistery!!
    function unleashBatch (uint256 [] memory tokenIds ) 
            public {
    ...
    function unleash(uint256 tokenId ) public{
        _requireMinted(tokenId);
        require(ableToUnleash[tokenId] == true, "not allowed to leash");
        require(ownerOf(tokenId)== msg.sender, "not the owner");
        leashed[tokenId] = true;
        emit Leashed(tokenId);
    } 

    function permitToLeash(uint256 id) public onlyOwner{
        require(ableToUnleash[id] == false, "already unleash");
        ableToUnleash[id] = true;
        emit AbleToUnleash(id);
    } 

    function permitToLeashByBatch(uint256 [] memory ids) public onlyOwner{
    ...
    function tokenURI(uint256 tokenId) public view virtual 
        ...
        if (leashed[tokenId] == false){
            return("https://raw.githubusercontent.com/zhubuz/web3.github.io/gh-pages/mistery_0.json");
        }
        else if (leashed[tokenId] == true){
...
//   }
  ...
  /// @param voucher An NFTVoucher to hash.
    function _hash(NFTVoucher calldata voucher) public view returns (bytes32) {
        ...
        keccak256("NFTVoucher(address recipient)"),
