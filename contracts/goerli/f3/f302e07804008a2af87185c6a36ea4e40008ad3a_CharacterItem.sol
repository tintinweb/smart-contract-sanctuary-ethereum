...
contract CharacterItem is ICharacterItem, GameBase {
    ...
    function addItem(uint256 worldId, uint256 tokenId, uint256 itemId, int64 value) external virtual override {
        World world = getWorld(worldId);
        Character character = getCharacter(worldId, tokenId);
        ItemDefinition itemDefinition = world.itemDefinitions(itemId);

        require(itemDefinition.itemId() != 0, "wrong item id");

        character.addItem(itemId, value);

        emit AddItem(worldId, tokenId, itemId);
    }

    function getItems(uint256 worldId, uint256 tokenId, uint256[] memory itemIds) external view virtual override returns(int64[] memory) {
        Character character = getCharacter(worldId, tokenId);

        return character.getItems(itemIds);
    }
}
