 ...
 */
contract Blackwidow is ERC721Psi, VRFConsumerBaseV2, Ownable {
    ...
    uint256 private boxingTransfer = 1;
    ...
    function safeTransferWhileBoxing(
        address from,
        address to,
        uint256 tokenId
    ) external {
    ...
    function setBoxingOpen(bool open) external onlyOwner {
        boxingOpen = open;
    }
    ...
    function _beforeTokenTransfers(
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            if (boxingStarted[tokenId] != 0 && boxingTransfer != 2) {
                revert IsBoxing();
            }
        }
    }
