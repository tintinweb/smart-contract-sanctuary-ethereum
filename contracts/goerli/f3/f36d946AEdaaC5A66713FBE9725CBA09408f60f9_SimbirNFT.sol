...
contract SimbirNFT is Ownable{
  ...
  function addressHasToken(address addr) public view returns (bool){
      return _minted[addr];
  }

  function getTokenIdByAddress(address addr) public view returns (uint256){
      require(addressHasToken(addr), "The address does not contain a token");
      return _tokens[addr];
  }
  ...
  function mint(address _to) external onlyOwner{
    require(!_minted[_to], "Token for this address already minted");
