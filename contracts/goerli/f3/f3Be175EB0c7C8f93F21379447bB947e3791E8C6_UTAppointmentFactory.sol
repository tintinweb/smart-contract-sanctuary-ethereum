...
contract UTAppointmentStorage is Ownable {
...
contract UTAppointmentController {
    ...
    function setValue(uint16[] memory tokenId, uint8 value, bytes32 eventId, address traitStorage) public {
        require(
            !locked,
            "Appointment Controller: contract locked."
        );
        require(
            tokenId.length > 0,
            "Appointment Controller: at least 1 token."
        );
        require(
            getTimestamp() > startTime,
            "Appointment Controller: before start time."
        );
        require(
            getTimestamp() < endTime,
            "Appointment Controller: after end time."
        );
        ...
        for(uint8 i = 0; i < tokenId.length; i++) {
            ...
            if(currentValue == 1 && value == 2) {
                ...
                require(
                    ...
                    "Appointment Controller: not owner of token."
                );
                // Is this event still open?
                require(ITraitStorage(traitStorage).getTokenForEvent(eventId) == 0, "Appointment Controller: this appointment is already taken!");
                ITraitStorage(traitStorage).setValue(currentTokenId, 2, eventId);
            }
        }
    }
    ...
    */
    ...
    function toggleLock() public {
        require(
            ...
            "ApointmentController: Not authorised"
        );
        locked = !locked;
    }
}
