...
contract Lottery is ReentrancyGuard, Initializable, VRFv2Consumer, ILotteryEvents {
    ...
    mapping(uint256 => uint256) private requestIdPerRaffle;
     ...
     * @dev Sets the values for 
        ...
        IUniswapV2Router02 _router, * UNISWAP or any other DEX Router contract, for the swap functions.
    ...
    function initialize(
        ...
        require(_profitPercent + _burnPercent < _basisPoints, "Cannot be more than 100%");
        require(_profitSplit1BP < _basisPoints, "Cannot be more than 100%");
        operator = _operator;
        admin = _admin;
        router = _router;
     ...
     * note: Lines 231-237 must be un-commented when testing this function and Lines 239-243 should be commented while testing.    
     */
    ...
    function declareWinner(uint256 _raffleNumber) external nonReentrant onlyOperator {
        ...
        require(
            uint256(keccak256(abi.encodePacked(block.timestamp))) > 0,
            "Random number not generated."
        );
        uint256 random = (uint256(keccak256(abi.encodePacked(block.timestamp)))) % 10000;
        uint256 totalTicketsSold = raffleInfo.ticketCounter;
        uint256 winnerTicketNumber = (random % totalTicketsSold) + 1;
...
interface ILotteryEvents {
    ...
    event RewardTokenUpdate(uint _raffleNumber, address _rewardToken);
}
