...
/**
 *   /'\_/`\                         /\ \__                  /\ \/\ \  __                    /\ \                        
 *  /\      \    ___     ___     ____\ \ ,_\    __   _ __    \ \ \/'/'/\_\    ___      __    \_\ \    ___     ___ ___    
 *  \ \ \__\ \  / __`\ /' _ `\  /',__\\ \ \/  /'__`\/\`'__\   \ \ , < \/\ \ /' _ `\  /'_ `\  /'_` \  / __`\ /' __` __`\  
 *   \ \ \_/\ \/\ \L\ \/\ \/\ \/\__, `\\ \ \_/\  __/\ \ \/     \ \ \\`\\ \ \/\ \/\ \/\ \L\ \/\ \L\ \/\ \L\ \/\ \/\ \/\ \ 
 *    \ \_\\ \_\ \____/\ \_\ \_\/\____/ \ \__\ \____\\ \_\      \ \_\ \_\ \_\ \_\ \_\ \____ \ \___,_\ \____/\ \_\ \_\ \_\
 *     \/_/ \/_/\/___/  \/_/\/_/\/___/   \/__/\/____/ \/_/       \/_/\/_/\/_/\/_/\/_/\/___L\ \/__,_ /\/___/  \/_/\/_/\/_/
 *                                                                                     /\____/                           
 *                                                                                     \_/__/                      
 */                                                                                  
...
contract MonsterKingdom is ERC721A, Ownable {
    ...
    ) ERC721A("Monster Kingdom NFT", "MK NFT"){
    ...
    function setStage(uint stageType, uint256 startTime, uint256 endTime, uint _maxQuantity, uint256 price) public onlyExecutor {
        ...
        require(startTime < endTime, "The Stage Start Time And Sale Time Must Be Less Than End Time");
        require(endTime > block.timestamp, "The Stage End Time Must Be More Than Current Time");
        require(_maxQuantity != 0, "The Stage quantity has not zero");
        uint stageNum = stageList.length;
        if (stageType >= stageNum) {
            for (uint i; i < stageType-stageNum; i++) {
                stageList.push(Stage(0, 0, 0, 0, 0));
            }
        }
        uint index = stageType - 1;
    ...
    function getStage(uint stageType) public view onlyExecutor returns (uint _stageType, uint256 startTime, uint256 endTime, uint _maxQuantity, uint mintNum, uint256 price){
        ...
        Stage memory stage = stageList[stageType - 1];
    ...
    function currentStage(address to) public view returns (uint _stage, bool _mint, uint256 _price, uint256 startTime, uint256 endTime){
        uint stageNum = stageList.length;
        ...
        for (uint i; i < stageNum; i++) {
            ...
            if (currentTime >= stage.startTime && currentTime <= stage.endTime) {
                if (to == address(0)) {
                    // has not login
                    return (stageType, false, stage.price, stage.startTime, stage.endTime);
                }
                ...
                bool mintable = !mintedNum[stageType][to];
                return (stageType, mintable, stage.price, stage.startTime, stage.endTime);
            }
        }
    ...
    function updateStage(uint stageType, address to, uint256 quantity, uint256 price) internal {
        ...
        uint256 stageMintedNum = stage.mintedNum;
        uint256 stageMaxQuantity = stage.maxQuantity;
        require(stageMintedNum+quantity <= stageMaxQuantity, "Minted more than max quantity in stage");

        require(quantity * price <= msg.value, "value error");

        Stage storage _stage = stageList[index];
        _stage.mintedNum = stageMintedNum + quantity;
        stageList[index] = _stage;
        emit UpdateStageEvent(stageType, to, _stage.mintedNum);
    }

    function mint(uint stageType, uint256 _timestamp, bytes memory _signature) public payable nftIsOpen stageOpen {
        require(msg.sender == tx.origin, "only origin");
    ...
    function isInWhiteList(address to, uint256 _timestamp, bytes memory _signature) public view returns (bool){
        (uint stageType, bool mintable, uint256 _price,,) = currentStage(to);
        if(!mintable)
            return false;
        address signer = signatureMint(stageType, to, 1, _price, _timestamp, _signature);
        return (signer == whitelistAddress);
    }
