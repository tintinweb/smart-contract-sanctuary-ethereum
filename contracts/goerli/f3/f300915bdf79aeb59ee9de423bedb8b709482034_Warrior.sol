    ...
    ERC2981
{
	...
	function mint(uint16 mintAmount) external payable nonReentrant {
        ...
        for (i = (supply + 1); i < (supply + mintAmount + 1); i++) {
            faction[i] = uint16(uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, (i + 1) * (i + 1)))) % factionNames.length);
        }
    ...
    function addFaction(string calldata newFaction) external onlyOwner {
        factionNames.push(newFaction);
    }
    ...
    function showFaction(uint16 id) external view returns (string memory) {
        return factionNames[faction[id]];
    }
