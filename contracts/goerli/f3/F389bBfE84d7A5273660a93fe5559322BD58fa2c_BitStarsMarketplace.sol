...
contract BitStarsMarketplace is Ownable, ReentrancyGuard {
    ...
    function createMarketItem(
        ...
        address creator = STARS(nftContractAddress).getTokenCreatorById(
            tokenId
        );
    ...
    function getRoyaltyFee(address nftContractAddress, uint256 tokenId)
        public
        view
        returns (uint256)
    {
        // address creator = STARS(nftContractAddress).getTokenCreatorById(tokenId);
        uint256 royaltyFeeInBips = STARS(nftContractAddress)
            .getRoyaltyFeeByTokenId(tokenId);
        uint256 royaltyFee = (100 * royaltyFeeInBips) / 10000;
        return royaltyFee;
    }

    /**
     * @dev Creates a market sale by transfering msg.sender money to the seller and NFT token from the
     * marketplace to the msg.sender. It also sends the royalty fee to the marketplace owner.
     */
        ...
        nonReentrant
    {
        ...
        uint96 royaltyFeeInBips = STARS(nftContractAddress)
            .getRoyaltyFeeByTokenId(tokenId);
        uint256 royaltyFee = (msg.value * royaltyFeeInBips) / 10000;
        address seller = marketItemIdToMarketItem[marketItemId].seller;

        marketItemIdToMarketItem[marketItemId].seller.transfer(
        ...
        // STARS(nftContractAddress).approve(address(this), tokenId);
