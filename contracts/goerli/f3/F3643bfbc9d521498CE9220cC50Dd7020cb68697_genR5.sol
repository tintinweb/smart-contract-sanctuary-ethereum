...
, GelatoRelayContext
{
  ...
  //SIGNER-WITHDRAWERS-mapping (address => uint256) pendingWithdrawals;

  //REVEALURI-mapping(uint256 => bool) private _reavealedTokens;
  ...
  */
  ...
  function increasePendingWithdrawals(uint256 val, address signer) internal {
    //SIGNER-WITHDRAWERS-pendingWithdrawals[signer] += val;
    pendingWithdrawals += val;
  }

  //SIGNER-WITHDRAWERS-/// @notice Transfers all pending withdrawal balance to the caller. Reverts if the caller is not an authorized minter.
  //SIGNER-WITHDRAWERS-function withdraw() public {
  //SIGNER-WITHDRAWERS-  require(hasRole(MINTER_ROLE, msg.sender), "Only authorized minters can withdraw");
  //SIGNER-WITHDRAWERS-
  //SIGNER-WITHDRAWERS-  // IMPORTANT: casting msg.sender to a payable address is only safe if ALL members of the minter role are payable addresses.
  //SIGNER-WITHDRAWERS-  address payable receiver = payable(msg.sender);
  //SIGNER-WITHDRAWERS-
  //SIGNER-WITHDRAWERS-  uint amount = pendingWithdrawals[receiver];
  //SIGNER-WITHDRAWERS-  // zero account before transfer to prevent re-entrancy attack
  //SIGNER-WITHDRAWERS-  pendingWithdrawals[receiver] = 0;
  //SIGNER-WITHDRAWERS-  receiver.transfer(amount);
  //SIGNER-WITHDRAWERS-}
  ...
  //REVEALURI-//Update the URI when the contract is revealed (only once by token id)
  //REVEALURI-function setRevealedURI(uint256 tokenId, string memory _tokenURI) public  {
  //REVEALURI-    require(msg.sender == owner(), "WNERC721: only owner can reveal URI");
  //REVEALURI-    require(_exists(tokenId), "can't reveal nonexistent token");
  //REVEALURI-    require(_reavealedTokens[tokenId] != true, "WNERC721: already revealed");
  //REVEALURI-
  //REVEALURI-    _reavealedTokens[tokenId] = true;
  //REVEALURI-    _setTokenURI(tokenId, _tokenURI);
  //REVEALURI-}
