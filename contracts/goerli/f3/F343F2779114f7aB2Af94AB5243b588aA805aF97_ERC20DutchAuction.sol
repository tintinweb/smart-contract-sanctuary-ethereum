...
contract ERC20DutchAuction {
    ...
    returns (uint256) {
        ...
        int256 price_diff = int(auctionDetails[_auctionID].startPrice - auctionDetails[_auctionID].reservePrice) * -1; // -1 make the slope dropping
        int slope = int(price_diff/time_diff); //startDate, endDate are sec since Unix epoch
        int intercept = int(auctionDetails[_auctionID].startPrice) - (slope* (int(auctionDetails[_auctionID].startDate)));
        // intercept goes back to origin Unix epoch time axis
        int price = int(slope * int(block.timestamp) + intercept); // [intercept [// price at unix epoch] + (slope [// negative slope] * sec)] - simpler just find current sec since epoch
    ...
    onlyNonAuctionOwners(_auctionID)
    {
        ...
        uint256 price = currentPrice(_auctionID); //price = current price 
        require(msg.value >= price * _amount, "more money required"); //deposit current * amount
        auctionDetails[_auctionID].biddingPrice = price; // set biddingPrice = current price
