...
contract PrimaryIndexTokenLeverage is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    event LeveragedBorrow(address indexed user, address projectToken, address lendingToken, uint notionalExposure, uint lendingAmount, uint margin, uint addingAmount, uint totalDepositedAmount, uint borrowAmount);
    ...
    modifier isPrimaryIndexToken() {
        require(msg.sender == address(primaryIndexToken), "PIT: caller is not primaryIndexToken");
        _;
    }
    ...
    function deferLiquidityCheck(address user, address projectToken, address lendingToken) internal view {
        ...
        require(totalOutstandingInUSD <= pit, "PIT: lendingTokenAmount exceeds pit remaining");
    ...
    function nakedBorrow(address user, address lendingToken, uint lendingTokenAmount, address projectToken) internal nonReentrant {
    ...
    function checkIsValidPosition(address user, address projectToken, address lendingToken, uint marginCollateralAmount) internal view {
        ...
        require(marginCollateralAmount >= depositedProjectTokenAmount, "PIT: invalid amount");
    }
    ...
    function leveragedBorrow(address projectToken, address lendingToken, uint notionalExposure, uint marginCollateralAmount, uint maxSlippage, bytes memory buyCalldata) public {
        ...
        deferLiquidityCheck(msg.sender, projectToken, lendingToken);

        if(!isLeveragePosition[msg.sender][projectToken]){
            isLeveragePosition[msg.sender][projectToken] = true;
        }
        emit LeveragedBorrow(msg.sender, projectToken, lendingToken, notionalExposure, lendingTokenCount, marginCollateralAmount, addingAmount, totalCollateral, amountRecive);
    }
