...
contract DeathCoin is
    ...
    PausableUpgradeable
 {
    ...
    address facoryAddress;
    address wethAddress;
    address deathAddress;
    ISwapRouter swapRouter;

    mapping (address => mapping (uint256 => UserStake)) public userStakeMap; 
    uint256 public stakeCountAll;
    uint256 userStakeMaxLimit;
    ...
    function initialize() external initializer {
        ...
        __Pausable_init();
        facoryAddress=0x1F98431c8aD98523631AE4a59f267346ea31F984;
        wethAddress=0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6;
        deathAddress=0xf1f98CcBCbb924A11D96C36afc4012CB985ADCdD;
        swapRouter=ISwapRouter(0xE592427A0AEce92De3Edee1F18E0157C05861564);
        userStakeMaxLimit=10;
    }
    ...
    //exchange
    function exchangeDeath(address tokenAddress,address fissionAddress,uint256 tokenAmount,uint256 day)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        dN.k=k;
        dN.tokenDecimals=ERC20Upgradeable(tokenAddress).decimals();
        dN.deathDecimals=ERC20Upgradeable(deathAddress).decimals();
        uint256 deathNumber=countDeathNumber(dN); 
        uint256 deathNumberFission=deathNumber.div(100);

        //check user balanceOf
        require(ERC20Upgradeable(tokenAddress).balanceOf(msg.sender)>0,"Insufficient Balance");

        //Send Death for user
        uint256 deathBalance = ERC20Upgradeable(deathAddress).balanceOf(address(this));
        require(deathBalance > stakeCountAll,"Death Token Insufficient supply");
        deathBalance-=stakeCountAll;
        ...
        if(fissionAddress!=address(0)){
            if(myUserData.fissionAddress==address(0)){
                if(fissionAddress==msg.sender){
                    ...
                    ERC20Upgradeable(deathAddress).safeTransfer(fissionAddress, deathNumberFission);
                }
            }else{
                require(deathNumber.add(deathNumberFission)<=deathBalance,"Low death balance");
                myUserData.userDeathCount=myUserData.userDeathCount.add(deathNumber);
                ...
                ERC20Upgradeable(deathAddress).safeTransfer(myUserData.fissionAddress, deathNumberFission);
            }
        }else{
            require(deathNumber<=deathBalance,"Low death balance");
            myUserData.userDeathCount=myUserData.userDeathCount.add(deathNumber);
        ...
        transDeath(tokenAddress,tokenAmount,deathNumber,day);

        //0.1ether pass
        uint256 tokenBalance = ERC20Upgradeable(tokenAddress).balanceOf(address(this));
        if(tokenBalance.mul(ethPriceToken).div(10**ERC20Upgradeable(tokenAddress).decimals())>=(1e18/10)){
    ...
    function swapTokens(
        ...
        ERC20Upgradeable(tokenIn).approve(address(swapRouter), amountNumber);
    ...
    function transDeath(address tokenAddress,uint256 tokenAmount,uint256 deathNumber,uint256 day) internal {
        // transferFrom  ERC20token to recipient address(this)
        ERC20Upgradeable(tokenAddress).safeTransferFrom(msg.sender, address(this), tokenAmount);
        if(day>0){
            saveUserStake(deathNumber,day);
        }else{
            // safeTransfer DeathToken to user
            ERC20Upgradeable(deathAddress).safeTransfer(msg.sender, deathNumber);
        }
    }
    ...
    function getSwapPoolData(
        ...
        //get balanceOf
        uint256 tokenBalance0 = ERC20Upgradeable(pool.token0()).balanceOf(poolAddress);
        uint256 tokenBalance1 = ERC20Upgradeable(pool.token1()).balanceOf(poolAddress);
        (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();

        uint256 p =  uint256(sqrtPriceX96).mul(1e18).div(2**96).mul(sqrtPriceX96).div(2**96);
        swapPrice.ethPrice = (p * 10 ** ERC20Upgradeable(pool.token0()).decimals()).div(10 ** ERC20Upgradeable(pool.token1()).decimals());
    ...
    function getSwapPrice(
        ...
        (uint160 sqrtPriceX96,, , , , , ) = pool.slot0();
        uint256 p =  uint256(sqrtPriceX96).mul(1e18).div(2**96).mul(sqrtPriceX96).div(2**96);
        ethPrice = (p * 10 ** ERC20Upgradeable(pool.token0()).decimals()).div(10 ** ERC20Upgradeable(pool.token1()).decimals());
        if(wethAddress==pool.token0()){
    ...
    function getK(uint256 day) public pure returns (uint256 k) {
        require(day<=20,"Invalid n");
        return day * 5e16 + 1e18;
    }
    ...
    //withdraw
    function withdraw() external returns ( uint256 totalAmountAvailable ) {
        for(uint i = 0 ; i < userStakeMaxLimit ; i++ ){
            if(userStakeMap[msg.sender][i].stakeTime>0){
                if(userStakeMap[msg.sender][i].stakeTime < block.timestamp){
                    totalAmountAvailable=totalAmountAvailable.add(userStakeMap[msg.sender][i].stakeCount);
                    delete userStakeMap[msg.sender][i];
                }
            }
        }
        require(totalAmountAvailable>0 && stakeCountAll>=totalAmountAvailable,"Withdraw fail");
        stakeCountAll-=totalAmountAvailable;
        //transf totalAmountAvailable
        ERC20Upgradeable(deathAddress).safeTransfer(msg.sender, totalAmountAvailable);
    }



    //Save userStake
    function saveUserStake(uint256 stakeCount,uint256 day) internal {
        require(day<=20 && day>0,"Invalid days");
        require(stakeCount>0,"Invalid reward");
        UserStake memory userStake;
        userStake.account=msg.sender;
        userStake.stakeCount=stakeCount;
        userStake.stakeTime=block.timestamp + day * 1 days;
        setUserStake(userStake);
    }

    //Set UserStake
    function setUserStake(UserStake memory userStake) internal {
        for ( uint i=0; i<userStakeMaxLimit; i++ ) {
            if(userStakeMap[userStake.account][i].stakeTime==0){
                userStakeMap[userStake.account][i]=userStake;
                stakeCountAll=stakeCountAll.add(userStake.stakeCount);
                return;
            }
        }
        revert("Save fail");
    ...
    //get user stake data
    function getUserStakeData(address account) external view returns (UserStake [] memory urArray,uint256 totalAmountAvailable,uint256 availableStakeNumber){
        for(uint i = 0 ; i < userStakeMaxLimit ; i++ ){
            if(userStakeMap[account][i].stakeTime>0){
                if(userStakeMap[account][i].stakeTime < block.timestamp){
                    totalAmountAvailable=totalAmountAvailable+userStakeMap[account][i].stakeCount;
                }
                UserStake memory userStake=userStakeMap[account][i];
                urArray=pushToArray(urArray,userStake);
            }else{
                availableStakeNumber++;
            }
        }

        return (urArray,totalAmountAvailable,availableStakeNumber);
    }

    //push to array
    function pushToArray(UserStake [] memory arr,UserStake memory userState) internal pure returns (UserStake [] memory) {
        UserStake [] memory usArray=new UserStake [](arr.length+1);
        for (uint i=0;i<arr.length;i++){
            usArray[i]=arr[i];
        }
        usArray[usArray.length-1]=userState;
        return usArray;
    }
