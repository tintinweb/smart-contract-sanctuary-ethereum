...
contract PccToken is ERC20,  Ownable, ExclusivesYieldMap {
    ...
    event AddedClaimBucket(string indexed _name, uint16 _dailyYield, uint64 _maxYield);
    ...
    uint256 public constant START_TIME = 1659816914;
    ...
    function addTokenBucket(string memory _name, uint32 _startNumber, uint16 _dailyYield, uint64 _maxYield) public {
        TokenBucket memory bucket = TokenBucket(_startNumber, _dailyYield, _maxYield, 0, uint80(block.timestamp));
    ...
    function addNewContract(IERC721 _contract, uint256 _yield) public onlyOwner {
        require(contractCount < 5, "cannot add any more yield contracts");
    ...
    function claimCommunitityAirdrop(bytes32[] calldata _merkleProof, uint256 _amount) public {
    ...
    function currentTokenToClaim(address _owner, IERC721[] calldata _contracts, uint256[] calldata _ids) external view returns (uint256){
    ...
    function availableFromBucket(string calldata _name) public view returns (uint256){
        ...
        amountToMint += uint256((block.timestamp - bucket.LastClaimTimestamp) * (bucket.DailyYield)) / 86400 ;

        return amountToMint;
    }

    function bucketMint(string calldata _name, uint256 _amount) public onlyOwner {
        ...
        amountToMint += uint256((block.timestamp - bucket.LastClaimTimestamp) * (uint256(bucket.DailyYield))) * SECOND_RATE_FOR_ONE_PER_DAY / 86400;

        bucket.CurrentYield += uint64(_amount);
        require(bucket.CurrentYield < bucket.MaxYield && amountToMint >= _amount, "cannot mint this many from this bucket");
    ...
    function getYield(IERC721 _contract, uint256 _id, address _operator)
        private
        view
        returns (uint256)
    {
        ...
        require(
            owner == _operator ||
                _contract.isApprovedForAll(owner, _operator),
            "not eligible"
        );
    ...
    function getExclusivesDailyYield(uint256 _id) public view returns(uint256){
...
uint256[] public YIELD_MAP = [127689209163621340800975369692887101994514425976037930576731308074077257728,
...
contract PccTierTwoItem is ERC1155, Ownable {
    ...
    function purchaseTierTwo(uint256 _ticketId) public {
        uint256 price = ItemPrice[_ticketId];
        require(price > 0, "sale not open for this item");

        _burn(msg.sender, _ticketId, 1);
        TierTwo.mint(_ticketId, msg.sender);
    }

    function purchaseTicket(uint256 _ticketId) public {
        uint256 price = ItemPrice[_ticketId];
        require(price > 0, "sale not open for this item");

        TokenContract.payForTierTwoItem(msg.sender, price);
        _mint(
        msg.sender,
        _ticketId,
        1,"");
    ...
    function setPricePerTicket(uint256 _ticketId, uint256 _price) external onlyOwner {
...
contract PccTierTwo is ERC721, Ownable {
    ...
    function mint(uint256 _ticketId, address _to) public {
        require(msg.sender == TicketContract, "not authorised");
        uint256 remaining = MAX_NUMBER_PER_TYPE - CurrentSupplyByType[_ticketId] - 1;
        uint256 index = getRandomNumber(remaining, uint256(block.number));
            
        _safeMint(_to, ((Ids[_ticketId][index] == 0) ? index : Ids[_ticketId][index]) + (MAX_NUMBER_PER_TYPE * _ticketId));

                       
        Ids[_ticketId][index] = Ids[_ticketId][remaining] == 0 ? remaining : Ids[_ticketId][remaining];
