...
contract Wsb is ERC1155Supply, Ownable{
    uint256 public constant WSB_SUPPLY = 50;

    uint256 public constant WSB = 0;

    uint256 public WSB_PRICE = 0.25 ether;

    uint256 public constant WSB_LIMIT = 4;
    ...
    address public constant ADDRESS_1 = 0x8cC005F1624a06B3e459C3516735F83f65911a28; // WENWL TEAM
    ...
    function mintPublic(uint256 numberOfTokens, address _owner) external isSecured(2) payable{
        require(numberOfTokens + totalSupply(WSB) <= WSB_SUPPLY,"NOT_ENOUGH_PRESALE_SUPPLY");
        require(mintedWSB[msg.sender] + numberOfTokens <= WSB_LIMIT,"MINTED_RUBY_ALREADY");

        require(msg.value == WSB_PRICE , "WRONG_ETH_VALUE");
        addressBlockBought[msg.sender] = block.timestamp;
        mintedWSB[msg.sender] += numberOfTokens;

        _mint(_owner, WSB, numberOfTokens, "");
    }
    ...
    function setRubyPrice(uint256 _price) external onlyOwner {
      WSB_PRICE = _price;
    }
    //Essential
