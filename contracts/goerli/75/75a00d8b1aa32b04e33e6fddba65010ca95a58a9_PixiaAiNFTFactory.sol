...
// File: contracts/PixiaAiNFTFactory.sol
...
interface ISingleNFT {
	function initialize(address creator, uint16 royaltyBasisPoints) external;
}
...
contract PixiaAiNFTFactory{
	...
	uint16 royaltyBasisPoints = 300; //3%
	address adminAddr = 0x79543b9490110668c32c7EC5a0dE1a8BCA661b17;
	...
	function setAdminAddr(address _adminAddr) external {
		require(msg.sender == factory, "Admin can change the address.");
	...
	function setTaxFee(uint16 _fee) external {
	...
	function createCollection(bool isFree) external payable returns(address collection)  {
		if (!isFree)payable(adminAddr).transfer(PRICE);
