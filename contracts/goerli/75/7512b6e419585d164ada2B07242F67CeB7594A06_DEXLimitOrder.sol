...
contract DEXLimitOrder is ReentrancyGuard {
    ...
    * @dev List limit order for token
    ...
    {
        require(inAmt > 0, 'Enter valid input amt');
        require(outAmt > 0, 'Enter required output amt');
    ...
    * @dev List limit order for eth swap
    ...
    nonReentrant payable 
    {
        require(msg.value > 0, 'Enter valid amt');
        require(outAmt > 0, 'Enter required output amt');
        LimitOrder memory limitOrder = LimitOrder(
            wethAddress,
            outTokenAddr,
    ...
    * @dev able to cancel listing if swap not settled
    ...
    nonReentrant payable 
	{
        LimitOrder memory limitOrder = _limitOrders[orderId];

        require(limitOrder.limitOrderStatus == false, "Already settled");
        ...
        if (limitOrder.inTokenAddr == wethAddress) {
            (bool sent,) = limitOrder.owner.call{value: amt}("");
            require(sent, "Failed to send ether");
        }
        else IERC20(limitOrder.inTokenAddr).transfer(msg.sender, amt);
