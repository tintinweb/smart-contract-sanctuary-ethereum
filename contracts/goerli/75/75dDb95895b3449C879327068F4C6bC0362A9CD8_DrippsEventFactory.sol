...
contract DrippsEventFactory is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event NewEvent(address instance);
		event NewEventImplementation(address instance);

		function setEventImplementation(
			address newImplementation
		) external onlyOwner {
				eventImplementation = newImplementation;
		}

    function createEvent(bytes calldata initdata) public payable {
        _initAndEmit(eventImplementation.clone(), initdata);
    }
    ...
    function predictDeterministicAddress(bytes32 salt) public view returns (address predicted) {
        return eventImplementation.predictDeterministicAddress(salt);
    }
