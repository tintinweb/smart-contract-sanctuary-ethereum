...
// ABI coder v2 is activated by default, but we specify explicitly nevertheless.
// See: https://docs.soliditylang.org/en/v0.8.0/080-breaking-changes.html
pragma abicoder v2;
...
// import "./WampTypes.sol";


interface IWMPTokenRelayInterface {
    function getRelayAuthority() external view returns (address);
}

// FIXME: using WampTypes.isContract and WampTypes.splitSignature
// Error in plugin @matterlabs/hardhat-zksync-solc:
// Contract `contracts/Wmp.sol:WMP` compiling error: The contract
// `contracts/Wmp.sol:WMP` LLVM IR generator definition pass error:
// Library `contracts/WampTypes.sol:WampTypes` not found in the project

/// Check if the given address is a contract.
function isContract(address adr) view returns (bool) {
    uint256 codeLength;
...
function splitSignature (bytes memory signature_rsv) pure returns (uint8 v, bytes32 r, bytes32 s) {
...
/**
 * The `WMP Token <https://github.com/crossbario/xbr-protocol/blob/master/contracts/WMPToken.sol>`__
 * is a `ERC20 standard token <https://eips.ethereum.org/EIPS/eip-20>`__ defined by:
 *
 * * "WMP" as symbol
 * * fixed total supply of 10**9 ("one billion") WMP
 * * 18 decimals
 * * address (mainnet): tbd
 *
 * The WMP Token is using the (unmodified) `OpenZeppelin <https://openzeppelin.org/>`__
...
contract WMP is ERC20 {
    ...
    /**
     * The WMP Token has a fixed supply of 1 billion and uses 18 decimal digits.
     */
     ...
     * The WMP Token uses the symbol "WMP" and 18 decimal digits.
     */
    constructor() ERC20("WMPToken", "WMP") {
    ...
    function approveFor (address sender, address relayer, address spender, uint256 amount, uint256 expires,
        ...
        require(
            (relayer == address(0x0)) ||
            (!isContract(relayer) && msg.sender == relayer) ||
            (isContract(relayer) && msg.sender == IWMPTokenRelayInterface(relayer).getRelayAuthority()),
            "INVALID_RELAYER"
        );
