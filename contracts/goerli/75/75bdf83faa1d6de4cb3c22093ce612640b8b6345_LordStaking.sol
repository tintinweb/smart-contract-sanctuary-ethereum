...
contract LordStaking is Ownable {
    ...
    constructor() {
        ...
        lordSeniorNFT = IERC721Enumerable(
            0x0Bb74e3bC7D61CE05Cf8caE73d3103ae7922D60E
        );
        timeStep = 1 minutes;
    ...
    function unstakeNFT(uint256 _tokenId) public {
        ...
        uint256 _amount = ((block.timestamp -
            user.nftRecords[_tokenId].lastClaimed) / timeStep) * rewardPerDay;

        if (_amount > 0) {
            lordToken.transferFrom(owner(), msg.sender, _amount);
            user.totalclaimedReward += _amount;
        }
    ...
    function Breeding(
        ...
        bool eligible = viewBreedingEligibility(
            msg.sender,
            _tokenId1,
            _tokenId2
        );
        ...
        nftPerentsId[_tokenId1] = true;
        nftPerentsId[_tokenId2] = true;
        nftBreederWallets[msg.sender] = true;
        lordJuniorNFT.safeMint(msg.sender, _juniorUri);
    }

    function viewBreedingEligibility(
        address _account,
        uint256 _tokenId1,
        uint256 _tokenId2
    ) public view returns (bool eligible) {
        return (
            (!nftPerentsId[_tokenId1] &&
                !nftPerentsId[_tokenId2] &&
                !nftBreederWallets[_account])
                ? !eligible
                : eligible
        );
    }
