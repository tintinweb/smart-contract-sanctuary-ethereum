...
contract ChubbyKaijuDAOSerum is ERC1155, Ownable {
    ...
    uint128 public constant RADIO_ETH_PRICE = 0.65 ether;
    uint128 public constant SERUM_ETH_PRICE = 0.85 ether;
    ...
    uint16 public constant MAX_RADIO = 3330;
    uint16 public constant MAX_SERUM = 3330;
    ...
    function freeRadio(bytes memory signature) external {
        ...
        require(radioMinted+1<3331,"All radios are minted");
        uint256[] memory ids;
        ids[0] = TYPE_RADIO;
    ...
    function preSale(bytes memory signature, uint256 id, uint256 amount) external payable {
        ...
        if(ispreSale1){
            ...
            if(id==TYPE_RADIO){
                require(radioMinted+amount<3331,"All radios are minted");
                ...
                require(serumMinted+amount<3331,"All serums are minted");
    ...
    function setgen2ContractAddress(address gen2ContractAddress)
        external
        onlyOwner
    {
        gen2Contract = gen2ContractAddress;
    }

    function burnSerumForAddress(uint256 typeId, address burnTokenAddress)
        external
    {
        require(msg.sender == gen2Contract, "Invalid burner address");
