...
contract CollateralAggregator {
    ...
    function refreshCollateral(
        ...
        _collateralByIssuerAggregation[collateralKey][account] = value;
    }
}
...
contract DebtShareAggregator {
    ...
    function refreshDebtShare(address account, uint value) external {
        _debtShareTotalSupply = _debtShareTotalSupply + value - __debtShareBalanceOf[account];
        __debtShareBalanceOf[account] = value;
    }

    function refreshDebtShareTotalSupply(uint value) external {
        _debtShareTotalSupply = value;
    }
}
...
contract RewardEscrowV2Aggregator {
    ...
    function refreshTotalEscrowedBalance(bytes32 key, uint value) external {
        _totalEscrowedBalance[key] = value;
    }
    ...
    function refreshTotalEscrowedAccountBalance(
        ...
        _totalEscrowedAccountBalance[key][account] = value;
    }
}
...
contract SynthAggregator {
    ...
    function refreshSynthTotalSupply(bytes32 key, uint value) external {
        _synthTotalSupply[key] = value;
    }
    ...
    function refreshSynthBalanceOf(
        ...
        _synthBalanceOf[key][account] = value;
    }
}
...
contract SynthrBridge is BaseBridge, CollateralAggregator, DebtShareAggregator, LiquidatorRewardsAggregator, RewardEscrowV2Aggregator, SynthAggregator {
    ...
    // Events
    event SynthMint(address indexed account, bytes32 indexed collateralKey, bytes32 indexed synthKey, uint collateralAmount, uint synthAmount, uint debtShare);
    event CollateralWithdraw(address indexed account, bytes32 indexed collateralKey, uint amount);
    event SynthBurn(address indexed account, bytes32 indexed synthKey, uint synthAmount, uint debtShare);
    event SynthTransfer(bytes32 indexed currencyKey, address indexed from, address indexed to, uint amount);
    event SynthExchange(address indexed sourceAccount, bytes32 indexed sourceKey, bytes32 indexed destKey, uint sourceAmount, address destAccount, uint destAmount, uint fee);
    event Liquidate(address indexed account, bytes32 indexed collateralKey, uint collateralAmount, uint debtShare, uint notifyAmount);
    event REAppend(bytes32 indexed currencyKey, address account, uint amount);
    event Vest(bytes32 indexed currencyKey, address indexed account, uint amount); 
    ...
    function _nonblockingLzReceive(
        ...
        if (packetType == PT_SYNTH_MINT) {
            ...
            if (_destChainId == _selfLZChainId) {
                ISynthrIssuer(IAddressResolver(_synthrAddressResolver).getAddress("Issuer")).destIssue(_accountForSynth, _synthKey, _synthAmount, _debtShare);
            }
            _sendMint(_accountForCollateral, _collateralKey, _collateralAmount, _accountForSynth, _synthKey, _synthAmount, _debtShare);
    ...
    function _sendMint(
        ...
        if (_synthKey != bytes32(0) && _synthAmount != 0) {
            issueSynth(_synthKey, _accountForSynth, _synthAmount);
            mintDebtShare(_accountForSynth, _debtShare);
            updateEntry(_collateralKey, _debtShareBalanceOf(_accountForSynth), _accountForSynth);
        }

        emit SynthMint(_accountForCollateral, _collateralKey, _synthKey, _collateralAmount, _synthAmount, _debtShare);
    }
    ...
    function _sendWithdraw(
        ...
        emit CollateralWithdraw(account, collateralKey, amount);
    }
    ...
    function _sendBurn(
        ...
        emit SynthBurn(accountForSynth, synthKey, synthAmount, debtShare);
    }
    ...
    function _sendTransferFrom(
        ...
        emit SynthTransfer(currencyKey, from, to, amount);
    }
    ...
    function _sendExchange(
        ...
        emit SynthExchange(sourceAccount, sourceKey, destKey, sourceAmount, destAccount, destAmount, fee);
    }
    ...
    function _sendLiquidate(
        ...
        emit Liquidate(account, collateralKey, collateralAmount, debtShare, notifyAmount);
    }
    ...
    function _sendREAppend(
        ...
        emit REAppend(currencyKey, account, amount);
    }
    ...
    function _sendVest(
        ...
        emit Vest(currencyKey, account, amount);
    }
