...
contract UTILToken is ERC20Detailed, ERC20Pausable {
    struct LockInfo {
    ...
    constructor() public ERC20Detailed("UTIL Token", "UTIL", 18) {
        ...
        if (timelockList[owner].length > 0) {
            for (uint256 i = 0; i < timelockList[owner].length; i++) {
                totalBalance = totalBalance.add(
                    timelockList[owner][i]._remainingAmount
                );
            }
        }
    ...
    function transferWithSliceLock(
        address holder,
        uint256 value,
        uint256 releaseStart,
        uint256 termOfRound,
    ...
    function _releaseTimeLock(address _holder, uint256 _idx)
        internal
        returns (bool)
    {
        ...
        releaseAmount = releaseAmount.add(
            sinceRound.mul(info._unlockAmountPerRound)
        );
        ...
        if (releaseAmount >= info._remainingAmount) {
            ...
            timelockList[_holder][_idx] = timelockList[_holder][
                timelockList[_holder].length.sub(1)
            ];
            ...
            timelockList[_holder][_idx]._releaseTime = timelockList[_holder][
                _idx
            ]._releaseTime.add(sinceRound.add(1).mul(info._termOfRound));
            timelockList[_holder][_idx]._remainingAmount = timelockList[
                _holder
