...
abstract contract AccountControl is RoleManager {
     ...
     */
    function freezeAccount(address account) external onlyRole(ISSUER_ROLE) {
     ...
     */
    function whitelistAccount(address account) public onlyRole(ISSUER_ROLE) {
     ...
     */
    function greylistAccount(address account) public onlyRole(ISSUER_ROLE) {
...
contract SX1155NFT is Pausable, SX1155NFTBase, RoleManager, AuthorizeManager, AccountControl {
    ...
    constructor(
        ...
        require(_admin != address(0), "SX1155: Invalid admin address");
    }
    ...
    function forceBurnBatch(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            ...
            emit ForceBurn(msg.sender, _accounts[i], _ids[i], reason);
        }
    }
...
contract SX1155NFTFactory is AccessControl {
    ...
    function deploySxNft(
        string memory _name,
        string memory _symbol,
        address _admin
    ) public onlyRole(NFT_DEPLOYER_ROLE) {
        SX1155NFT newSX1155 = new SX1155NFT(_name, _symbol, _admin);
