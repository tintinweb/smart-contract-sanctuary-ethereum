...
contract Wrap is Ownable {
    ...
    function withdraw() public {
        ...
        if (usdtBalance > 0) {
            usdt.transfer(address(AAA), usdtBalance);
        }
        uint256 MainTokenBalance = AAA.balanceOf(address(this));
        if (MainTokenBalance > 0) {
            AAA.transfer(address(AAA), MainTokenBalance);
        }
    }
