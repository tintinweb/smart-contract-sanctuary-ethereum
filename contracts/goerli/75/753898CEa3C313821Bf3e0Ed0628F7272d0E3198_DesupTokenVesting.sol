...
contract DesupTokenVesting is Ownable, ReentrancyGuard {
    ...
    function getWithdrawableAmount(address holder_address)
        public
        view
        returns (uint256[] memory withdrawble_amount)
    {
        require(token_published, "Token is not published yet");
        require(vsDefined(holder_address), "Vesting Schedule is not set yet");

        uint256[] memory withdrawable_amount = new uint256[](
            holders[holder_address].histories.length
        );

        for (uint16 i = 0; i < holders[holder_address].histories.length; i++) {
            withdrawable_amount[i] =
                getAllowedAmount(holder_address, i) -
                holders[holder_address].histories[i].withdrawed_amount;
        }
    ...
    function getInvestorHistory(address holder_address)
        public
        view
        returns (History[] memory)
    {
        require(isInHolderlists(holder_address), "Not a token holder");
        return holders[holder_address].histories;
    }
