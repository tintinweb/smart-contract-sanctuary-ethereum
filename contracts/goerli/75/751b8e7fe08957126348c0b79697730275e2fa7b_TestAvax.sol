...
contract TestAvax is ERC721Enumerable, ERC721URIStorage, IERC2981, Pausable, Ownable, ERC721Burnable, NonblockingReceiver, Randomizer {
    ...
    // MODIFIERS
    modifier onlyDevs() {
        require(devFees[msg.sender].percent > 0, "Dev Only");
        _;
    }
    ...
    // CONSTANTS
    uint public constant MAX_SUPPLY = 110;
    ...
    constructor(
        ...
        uint[] memory _fees,
        address _lzEndpoint
    ) ERC721("TestAvax", "Test") Randomizer(maxSupply) {
    ...
    function mint(uint amount) public payable whenNotPaused {
        ...
        require(amount > 0 && amount <= maxPerTx, "Error: Only 2 per Tx");
        require(balanceOf(msg.sender) + 1 <= maxPerPerson, "Error: Max 10 per Wallet");
        if (price > 0) require(msg.value == price * amount, "Error: Need more Avax");
