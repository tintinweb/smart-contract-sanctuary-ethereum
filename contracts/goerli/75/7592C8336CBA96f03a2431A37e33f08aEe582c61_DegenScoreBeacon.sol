...
import "./SoulboundERC721ERC1155.sol";
    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        secondaryTraitURI = _secondaryURI;
    }
    ...
    function submitTraits(UserPayload calldata payload, bytes memory signature) external payable whenNotPaused {
        ...
        for (uint256 i = 0; i < payload.traits.length; i++) {
            uint192 traitId = payload.traits[i].id;

            uint64 oldTraitValue = _traits[payload.account][payload.traits[i].id].value;
            uint64 newTraitValue = payload.traits[i].value;
        ...
        beaconIds[payload.beaconId] = payload.account;

        // emit mint event if submitted data for the first time
        if (isFirstSubmission) {
            emit Transfer(address(0), payload.account, payload.beaconId);
        }
    ...
    function burn() external override {
        ...
        emit Transfer(msg.sender, address(0), metadata.beaconId);
    ...
    {
        uint256[] memory maxAges = new uint256[](accounts.length);

        return _getTraitBatch(accounts, ids, maxAges);
    }

    function getAllTraitsOf(address _account)
        public
        view
        override
        returns (
        ...
        )
    {
        ...
        BeaconData memory data = beaconData[_account];
        uint256[] memory _traitValues = new uint256[](data.traitIds.length);

        for (uint256 i = 0; i < data.traitIds.length; ++i) {
            _traitValues[i] = _getTrait(_account, data.traitIds[i], 0);
        }
    ...
    function uri(uint256) external view override returns (string memory) {
        return primaryTraitURI;
    }

    // User functions

    function beaconDataOf(address _account) public view override returns (BeaconData memory) {
        require(_account != address(0), "address zero is not a valid owner");
        return beaconData[_account];
    }

    function ownerOfBeacon(uint64 beaconId) public view returns (address owner) {
        return _ownerOfBeacon(beaconId);
    }

    function ownerOf(uint256 beaconId) public view override returns (address owner) {
        return _ownerOfBeacon(uint64(beaconId));
    }

    function tokenURI(uint256) public view override returns (string memory) {
        return secondaryTraitURI;
    }

    function balanceOf(address owner) public view override returns (uint256 balance) {
        require(owner != address(0), "address zero is not a valid owner");
        BeaconData memory metadata = beaconData[owner];
    ...
    function setSecondaryTraitURI(string calldata _uri) public onlyOwner {
        secondaryTraitURI = _uri;
    }
    ...
    function _ownerOfBeacon(uint64 beaconId) internal view returns (address account) {
...
abstract contract SoulboundERC721ERC1155 is
    IERC165Upgradeable,
    IERC1155,
    IERC1155MetadataURI,
    IERC721,
    IERC721Metadata
{
    ...
    function setApprovalForAll(address, bool) public virtual override(IERC1155, IERC721) {
        revert SoulBoundContract(REVERT_ERROR);
    }

    // approval functinos only return false?
    function isApprovedForAll(address, address) public pure virtual override(IERC1155, IERC721) returns (bool) {
        return false;
    }
...
interface IDegenScoreBeaconReader {
    ...
    // Returns the userId of that address. Or 0 if no Id exists.
    function beaconDataOf(address _account) external view returns (BeaconData memory);

    function ownerOfBeacon(uint64 beaconId) external view returns (address owner);

    // Returns the trait if it is not older than 'daysValid' else 0
    function getTrait(
        address account,
        uint256 traitId,
        uint256 maxAge
...
interface IDegenScoreBeaconWriter {
    ...
    event SubmitTraits(uint128 beaconId, uint64 createdAt);

    event Burn(uint128 beaconId);

    // Called by user to submit new trait data.
...
interface IERC721 is IERC165Upgradeable {
     ...
     * Commented out since ERC1155 has the same event
     */
