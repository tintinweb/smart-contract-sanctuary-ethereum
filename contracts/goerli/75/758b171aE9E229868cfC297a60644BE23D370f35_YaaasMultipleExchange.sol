...
contract YaaasMultipleExchange is
    ...
    function _addOffer(
        ...
        emit Listed(offerId, _msgSender(), newOffer._collection, newOffer._assetId, newOffer._price, newOffer._amount);
    }

    function setOfferPrice(bytes32 offerID, uint256 price)
        override
        external
    ...
    function setForSell(bytes32 offerID, bool isForSell)
        override
        external
        onlyOfferOwner(offerID)
    {
    ...
    function setForAuction(bytes32 offerID, bool isForAuction)
        override
        external
        onlyOfferOwner(offerID)
    {
    ...
    function setExpiresAt(bytes32 offerID, uint256 expiresAt)
        override
        external
        onlyOfferOwner(offerID)
    {
    ...
    function cancelOffer(bytes32 offerID) override external onlyOfferOwner(offerID) {
    ...
    function buyOffer(bytes32 id, uint256 amount) override external payable {
    ...
    */
    function delegateBuy(bytes32 id, uint256 amount, address buyer) external payable {
        Offer memory offer = offers[id];
    ...
    function _buyOffer(
        ...
        (bool success, bytes memory data) = address(offer.collection).call("isLoyalty");
        if (success) {
    ...
    function safePlaceBid(
        ...
        _createBid(_offer_id, _price, _amount);
    }
    ...
    function cancelBid(bytes32 _offerId, address _bidder) override external {
    ...
    function acceptBid(bytes32 _offerID, address _bidder)
        override
        external
...
interface IYaaasMultipleExchange {
     ...
     */
    function setOfferPrice(bytes32 _offerId, uint256 _price) external;
     ...
     */
    function setForSell(bytes32 _offerId, bool _isForSell) external;
     ...
     */
    function setForAuction(bytes32 _offerId, bool _isForAuction) external;
     ...
     */
    function setExpiresAt(bytes32 _offerId, uint256 _expiresAt) external;
     ...
     */
    function buyOffer(bytes32 _offerId, uint256 amount) external payable;
     ...
     */
    function cancelBid(bytes32 _offerId, address _bidder) external;
     ...
     * @param _bidder bidder address
     */
    function acceptBid(bytes32 _offerId, address _bidder) external;
    ...
    event SetOfferPrice(bytes32 _offerId, uint256 price);
    event SetForSell(bytes32 _offerId, bool isForSell);
    event SetForAuction(bytes32 _offerId, bool isForAuction);
    event SetExpireAt(bytes32 _offerId, uint256 expiresAt);
    event CancelOffer(bytes32 _offerId);
}
