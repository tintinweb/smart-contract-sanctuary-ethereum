    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getGamesOf(
        ...
        Game[] memory gamesOfArray = new Game[](gamesOf[player].length);
        for (uint i = 0; i < gamesOf[player].length; i++) {
            gamesOfArray[i] = games[gamesOf[player][i]];
        }
        return gamesOfArray;
    }
    ...
    function _ghostsChosen(
        ...
        uint256 left = Random.numberChosen(0, 2, nonceTemp.add(reaperChosen));
        uint256 number;
...
contract GameEngine is GameBase {
        ...
        returns (uint256)
    {
        ...
        gamesOf[_msgSender()].push(gameID);
        if (playersOf[gameID][0].id == address(this)) {
            gamesOf[address(this)].push(gameID);
        }
    ...
    function claimAllPrize() external virtual override {
        for (uint i = 0; i < gamesOf[_msgSender()].length; i++) {
            uint256 gameID = gamesOf[_msgSender()][i];
            Game memory game = games[gameID];
            if (
    ...
    function claimAllPrizeReaper()
        ...
        whenNotPaused
    {
        for (uint i = 0; i < gamesOf[address(this)].length; i++) {
            uint256 gameID = gamesOf[address(this)][i];
            Game storage game = games[gameID];
            if (
                block.timestamp >= game.endedAt &&
                game.winner == address(this) &&
    ...
    function _removeGameOf(address account, uint256 gameID) internal virtual {
        ...
        for (uint256 i = 0; i < gamesOf[account].length; i++) {
            Game memory game = games[gamesOf[account][i]];
            if (game.id == gameID) {
                indexOf = int256(i);
            }
        }
