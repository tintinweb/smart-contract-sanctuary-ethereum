...
contract CentherMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event CreateItem(address indexed _collection, string _category, uint256 indexed _tokenID, string _tokenURI, bool _isAuction, uint256 _price, uint256 _endAt, address _creator);
    ...
    function createItems(address collection, string memory category, string memory tokenURI, uint32 supply, bool isAuction, uint256 price, uint256 period) public payable onlyRegisterUser {
        ...
        for(uint32 i = 0; i < supply; i++) {
            ...
            emit CreateItem(address(collection), category, tokenID, tokenURI, isAuction, price, block.timestamp + period, msg.sender);
        }
    }
    ...
    function createAuction(address collection, uint256 tokenID, uint256 startPrice, uint256 auctionPeriod) public onlyRegisterUser{
        ...
        require(startPrice > 0, "Price should be greater than zero.");
