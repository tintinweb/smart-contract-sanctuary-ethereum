...
contract PrudysDeployer is ERC721, ERC2981, Pausable, Ownable, ReentrancyGuard, OperatorFiltererKillSwitchBlues {
   ...
   uint256 public shenanigansInWild = 0;
   ...
   constructor() ERC721("Prudys", "PRUDS") {
      // console.log("\n\n>In the 'MooseysDeployer' constructor()!!!\n");
      ...
      console.log("\n\n>Still in MAIN 'Prudys' 'constructor()' \n   >'usingOSRegistryService' = %s', \n   >defaultRoyalty.royaltyFraction' was just set to '750' - which should mean 7.5% !!! \n", usingOSRegistryService);
   ...
   function approve(address operator, uint256 tokenId) public override {
      ...
      if(usingOSRegistryService) {
         console.log("\n>In Master contract's 'approve()' --> Got through the 'IF usingOSRegistryService == true' statement!!!");
   ...
   function safeFreeMint(address to, string memory metadataURI, uint96 leRoyaltyAmount) public onlyOwner() {
      ...
      _safeMint(to, shenanigansInWild);
      setTokenURI(shenanigansInWild, metadataURI);
      ...
      if(leRoyaltyAmount == 0) {
         _setTokenRoyalty(shenanigansInWild, to, 750);  // 7.5%
      }
      else {
         _setTokenRoyalty(shenanigansInWild, to, leRoyaltyAmount / 10000000000000000);
      }

      console.log("\n>SUMMARY: Just minted token # %s to address: %s", shenanigansInWild, to);  // <<--------- <--------- THIS CODE IS NOT NECESSARY!!! 

      // Iterate the Counter of NFTs that were minted:
      shenanigansInWild++;
      // console.log("\n>AND last but not least: Incremented 'shenanigansInWild' to %s!\n>Which means the next Token that will be minted will have an ID of %s", shenanigansInWild,  shenanigansInWild);
  
      emit ShenaniganMinted(shenanigansInWild, metadataURI);
   ...
   function updateTokenRoyalty(uint256 tokenId, uint96 newRoyaltyAmount) public onlyOwner() {
      console.log("\n\nIn 'updateTokenRoyalty()'!!!");
      console.log("  >Incoming 'tokenId' = %s \n    >Incoming 'newRoyaltyAmount' = %s", tokenId, newRoyaltyAmount);

      // 1. Make sure this Token exists:
      require(_exists(tokenId), "ERROR!!! Trying to set URI of nonexistent token!");

      // 2. Get the Token's current OWNER:
      // address currentOwner = ownerOf(tokenId);

      // -->If I somehow got here WITHOUT passing in ANY value, I need to just set 'leRoyaltyAmount' to a default value of 750:
      if(newRoyaltyAmount == 0) {
         _setTokenRoyalty(tokenId, ownerOf(tokenId), 750);  // 7.5%
      }
      else {
         _setTokenRoyalty(tokenId, ownerOf(tokenId), newRoyaltyAmount / 10000000000000000);
      }

      // emit TokenRoyaltyUpdated(tokenId, theTokenURI);
   }
...
*/
contract OperatorFiltererKillSwitchBlues is Ownable {
// abstract contract OperatorFiltererKillSwitchBlues {
   ...
   function registerAndSubscribeToOS() public onlyOwner() {
      if (address(OPERATOR_FILTER_REGISTRY).code.length > 0) {
         console.log("Going to Register OURSELVES via 'registerAndSubscribe()'!!!");
         OPERATOR_FILTER_REGISTRY.registerAndSubscribe(address(this), 0x3cc6CddA760b79bAfa08dF41ECFA224f810dCeB6);
      }
   ...
   function unregisterFromOS(address addr) public onlyOwner() {
      console.log("\n\n>>In 'unregisterFromOS()'!!! \n>addr =", addr);
      OPERATOR_FILTER_REGISTRY.unregister(addr);
   }
