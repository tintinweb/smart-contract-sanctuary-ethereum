...
contract CryptoAvatarsExchange is ICryptoAvatarsExchange, ReentrancyGuard, Ownable {
    ...
    event TakerBid(
        ...
        bool isRemix,
        bytes dataRemix,
        bool isCryptoAvatars,
        bytes dataCryptoAvatars,
        uint256 price // final transacted price
    );
    ...
    constructor(
        ...
        eventEmitter = IEventEmitter(_proxyEventEmitter);
    }
    ...
    function cancelAllOrdersForSender(uint256 minNonce) external {
        ...
        eventEmitter.emitEvent("CancelAllOrders", abi.encode(msg.sender, minNonce));
    ...
    function cancelMultipleMakerOrders(uint256[] calldata orderNonces) external {
        ...
        eventEmitter.emitEvent("CancelMultipleOrders", abi.encode(msg.sender, orderNonces));
    }
    ...
    function matchAskWithTakerBidUsingETHAndWETH(
        ...
        bytes32 askHash = makerAsk.hashMakerOrder();
        bytes32 bidHash = takerBid.hashTakerOrder();
        _validateOrder(makerAsk, askHash, takerBid, bidHash);

        // Check the taker bid order
        //...
        ...
        // Emit event
        eventEmitter.emitEvent("TakerBid", abi.encode(
            makerAsk,
            takerBid));
        ...
        nonReentrant
    {
        ...
        // Execution part 1/2
        _transferFeesAndFunds(makerAsk,takerBid,_msgSender());
        ...
        nonReentrant
    {
        ...
        bytes32 bidHash = makerBid.hashMakerOrder();
        bytes32 askHash = takerAsk.hashTakerOrder();
        _validateOrder(makerBid, bidHash, takerAsk, askHash);
        ...
        // Execution part 2/2
        _transferFeesAndFunds(makerBid,takerAsk, _msgSender());


        // Emit event
        eventEmitter.emitEvent("TakerAsk", abi.encode(makerBid,takerAsk));
    }
    ...
    function updateCurrencyManager(address _currencyManager) external onlyOwner {
        ...
        currencyManager = ICurrencyManager(_currencyManager);
        eventEmitter.emitEvent("NewCurrencyManager", abi.encode(_currencyManager));
    }
    ...
    function updateExecutionManager(address _executionManager) external onlyOwner {
        ...
        executionManager = IExecutionManager(_executionManager);
        eventEmitter.emitEvent("NewExecutionManager", abi.encode(_executionManager));
    }
    ...
    function updateCollectionManager(address _collectionManager) external onlyOwner {
        ...
        collectionManager = ICollectionManager(_collectionManager);
        eventEmitter.emitEvent("NewCollectionManager", abi.encode(_collectionManager));
    }
    ...
    function updateProtocolFeeRecipient(address _protocolFeeRecipient) external onlyOwner {
        protocolFeeRecipient = _protocolFeeRecipient;
        eventEmitter.emitEvent("NewProtocolFeeRecipient", abi.encode(_protocolFeeRecipient));
    }
    ...
    function updateRoyaltyFeeManager(address _royaltyFeeManager) external onlyOwner {
        ...
        royaltyFeeManager = IRoyaltyFeeManager(_royaltyFeeManager);
        eventEmitter.emitEvent("NewRoyaltyFeeManager", abi.encode(_royaltyFeeManager));
    }
    ...
    function updateTransferSelectorNFT(address _transferSelectorNFT) external onlyOwner {
        ...
        eventEmitter.emitEvent("NewTransferSelectorNFT", abi.encode(_transferSelectorNFT));
    }
    ...
    function _transferFeesAndFunds(
        ...
        uint256 finalSellerAmount = makerOrder.amount;

        // 1. Protocol fee
        {
            uint256 protocolFeeAmount = _calculateProtocolFee(makerOrder.strategy, makerOrder.amount);
        ...
        // 2. Royalty fee
        {
        if(!takerOrder.isRemix){
            if(takerOrder.isCryptoAvatars){
                (address creator , uint256 royaltyFee) = abi.decode(takerOrder.dataCryptoAvatars, (address, uint256));
                uint256 royaltyCreatorFeeAmount = (makerOrder.amount * royaltyFee) / 10000 ;
                ...
                (address royaltyFeeRecipient, uint256 royaltyFeeAmount) = royaltyFeeManager
                    .calculateRoyaltyFeeAndGetRecipient(makerOrder.collection, makerOrder.tokenId, makerOrder.amount);

                // Check if there is a royalty fee and that it is different to 0
                if ((royaltyFeeRecipient != address(0)) && (royaltyFeeAmount != 0)) {
                    IERC20(makerOrder.currency).safeTransferFrom(from, royaltyFeeRecipient, royaltyFeeAmount);
                    finalSellerAmount -= royaltyFeeAmount;
                    eventEmitter.emitEvent("RoyaltyPayment", abi.encode(makerOrder.collection , makerOrder.tokenId , royaltyFeeRecipient, makerOrder.currency, royaltyFeeAmount));
            ...
            uint256 royaltyCreatorFeeAmount = (makerOrder.amount * royaltyCreatorFee) / 10000 ;
            uint256 royaltyOwnerFeeAmount =  (makerOrder.amount * royaltyOwnerFee) / 10000 ;
        ...
        require((finalSellerAmount * 10000) >= (makerOrder.minPercentageToAsk * makerOrder.amount), "Fees: Higher than expected");

        // 3. Transfer final amount (post-fees) to seller
        {
            IERC20(makerOrder.currency).safeTransferFrom(from, makerOrder.signer, finalSellerAmount);
        }
    }
    ...
    function _transferFeesAndFundsWithWETH(
       ...
       OrderTypes.TakerOrder calldata takerOrder
        ...
        if(!takerOrder.isRemix){
            ...
            if(takerOrder.isCryptoAvatars){
                ...
                if ((royaltyFeeRecipient != address(0)) && (royaltyFeeAmount != 0)) {
                    ...
                    eventEmitter.emitEvent("RoyaltyPayment", abi.encode(makerOrder, takerOrder));
                }
            }
        }
        else{
        ...
        // 3. Transfer final amount (post-fees) to seller
        {
            IERC20(WETH).safeTransfer(makerOrder.signer, finalSellerAmount);
        }
    }
     ...
     * @param makerOrder maker order
     * @param makerOrderHash computed hash for the maker order
     * @param takerOrder taker order
     * @param takerOrderHash computed hash for the taker order
     */
    function _validateOrder(OrderTypes.MakerOrder calldata makerOrder, bytes32 makerOrderHash, 
                            OrderTypes.TakerOrder calldata takerOrder, bytes32 takerOrderHash) internal view {
        ...
        require(
            SignatureChecker.verifyMakerOrder(
                makerOrderHash,
                makerOrder.signer,
                makerOrder.v,
                makerOrder.r,
        ...
        require(
            SignatureChecker.verifyTakerOrder(
                takerOrderHash,
                takerOrder.signer,
                takerOrder.v,
                takerOrder.r,
...
library OrderTypes {
    ...
    //keccak256("TakerOrder(bool isOrderAsk,address taker,address signer,uint256 price,uint256 tokenId,uint256 maxValidTime,uint256 minPercentageToAsk,bytes params,bool isCryptoAvatars,bytes dataCryptoAvatars,bool isRemix,bytes dataRemix)")
    bytes32 internal constant TAKER_ORDER_HASH = 0x7f11b29477759477edb96bdaa7d8479b3ba587ce264d4d49d433d300fa1e9839;
    struct MakerOrder {
    ...
    struct TakerOrder {
        ...
        bool isCryptoAvatars; // check if the NFT is a CryptoAvatars NFT
        bytes dataCryptoAvatars; // data for CryptoAvatars NFT
        bool isRemix; // check if the NFT is a remix and if so, the remix owner for royalties
