...
contract FreaksNGuildsMigrated is Controllable, Ownable, Pausable, ERC721("Freaks N Guilds Migrated", "FnG"), ERC721Enumerable{
  ...
  //////////////////////////////////////////////////////////////*/ 
    ...
    function transferOriginalFnGOwnership(address newOwner) external onlyOwner {
        fngOriginal.transferOwnership(newOwner);
    }
    ...
    function migrate(uint256[] calldata tokenIds) external whenNotPaused{
        ...
        for(uint256 i = 0; i < tokenIds.length; i++){
            require(fngOriginal.ownerOf(tokenIds[i]) == msg.sender, "you don't own this token sir");
    ...
    function migrateAndHunt(uint256[] calldata freakIds, uint256 pool) external whenNotPaused {
        require(freakIds.length > 0, "No token IDs provided");
        for(uint256 i = 0; i < freakIds.length; i++){
            require(fngOriginal.ownerOf(freakIds[i]) == msg.sender, "you don't own this token sir");
    ...
    function migrateAndObserve(uint256[] calldata celestialIds) external whenNotPaused {
        require(celestialIds.length > 0, "No token IDs provided");
        for(uint256 i = 0; i < celestialIds.length; i++){
            require(fngOriginal.ownerOf(celestialIds[i]) == msg.sender, "you don't own this token sir");
            require(!fngOriginal.isFreak(celestialIds[i]), "not a celestial");
    ...
    */
    function mint(uint256 amount) external whenNotPaused{
        require(_currentIndex + amount <= (maxSupplyL1 + 1), "maximum supply reached");
    ...
    */
    function getSpecies(uint256 tokenId) external view returns (uint8 species) {
        require(isFreak(tokenId), "token not a freak");
