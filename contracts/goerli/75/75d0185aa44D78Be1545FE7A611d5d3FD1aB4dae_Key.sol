...
contract Key is ERC721URIStorage, ERC721Burnable, AccessControl, IKey {
    using Strings for uint256;
    ...
    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE,msg.sender), "Access restricted to admin.");
        _;
    }

    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE,msg.sender), "Access restricted to minters.");
        _;
    }
    ...
    function mint(address recipient) public onlyMinter returns(uint) {
        uint newItemId = _tokenIds.current();
        _safeMint(recipient, newItemId);
        _setTokenURI(newItemId, _keyURI);
        _tokenIds.increment();
        return newItemId;
    }

    function setKeyURI(string memory _tokenURI) public onlyAdmin {
        uint keyCounter = _tokenIds.current();
        for (uint i = 0; i < keyCounter; i++) {
