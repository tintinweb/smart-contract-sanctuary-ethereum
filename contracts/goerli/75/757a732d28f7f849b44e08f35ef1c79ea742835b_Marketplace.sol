...
contract Marketplace is OwnableUpgradeable, PausableUpgradeable {
     ...
     */
    function initialize(address[] calldata _nftAddresses, address _swapperAddress) public initializer {
        __Ownable_init();
        __Pausable_init();
     ...
     */
    function getOffers(address _nftAddress, uint256 _tokenId) external view returns (Offer[] memory) {
        Offer[] memory offersActive = offers[_nftAddress][_tokenId];
        for(uint256 i = 0; i < offersActive.length;){
            if(offersActive[i].endTime < block.timestamp){
                delete offersActive[i];
            }
