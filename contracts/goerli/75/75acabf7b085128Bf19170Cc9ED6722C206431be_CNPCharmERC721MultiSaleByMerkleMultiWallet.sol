...
import "../../mock/CNPC.sol";

contract CNPCharmERC721MultiSaleByMerkleMultiWallet is
    ...
    Ownable
{
    ...
    ) external payable enoughEth(amount) callerIsUser {
    ...
    function exchange(
        ...
        uint256 allowedAmount,
        bytes32[] calldata merkleProof
    ) external payable enoughEth(burnTokenIds.length) callerIsUser {
...
contract CNPC is Ownable, RevokableDefaultOperatorFilterer ,ERC2981, ERC721A, AccessControl {
    ...
    function setBaseExtension(string memory _newBaseExtension) external onlyRole(ADMIN) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        if(address(tokenuri) == address(0)) {
            ...
            return bytes(_baseURI()).length != 0 ? string(abi.encodePacked(_baseURI(), _toString(tokenId), baseExtension)) : '';
        } else {
    ...
    //CAL
    function setCalContract(IContractAllowListProxy _cal) external onlyRole(ADMIN) {
        cal = _cal;
    }
    ...
    function setRoyaltyAddress(address _royaltyAddress) external onlyRole(ADMIN) {
        ...
        internal
    {
        ...
        _safeMint(0x0Ed35594FDb513f955cddE0B0B54a12d619d109c, 1);
        // _safeMint(member2, 1);
        // _safeMint(member3, 1);
        // _safeMint(member4, 1);
        // _safeMint(member5, 1);
