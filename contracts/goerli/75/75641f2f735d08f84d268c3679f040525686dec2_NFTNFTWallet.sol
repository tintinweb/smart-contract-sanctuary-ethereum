...
contract NFTNFTWallet is CallbackHandler, ISignatureValidator {
    ...
    event ExecutionSuccess(address indexed to, uint256 value, bytes data);
    ...
    function execTransaction(
        ...
        uint256 value,
        bytes calldata data
    ) external payable virtual onlyOwner returns (bool success) {
        require(policy.isMethodAllowed(to, data) == policy.isMethodAllowed.selector, "Prohibited transaction");
        success = execute(to, value, data, gasleft());

        if (success) emit ExecutionSuccess(to, value, data);
        else emit ExecutionFailure(to, value, data);
    }
...
contract CallbackHandler is IERC1155Receiver, IERC777Recipient, IERC721Receiver {
