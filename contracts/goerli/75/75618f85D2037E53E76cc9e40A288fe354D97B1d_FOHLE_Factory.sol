...
contract FOHLE_Factory is IFOHLE_Factory {
    ...
    function set30daysPR(address _pair, uint256 _30daysPR) external { //TEST NEEDED
        //S'ha de comprobar que canvii realment el valor en el parell al fer el test
        require(msg.sender == setter, 'FOHLE: FORBIDDEN');
        ...
        require(_pairMatched == true, 'FOHLE: PAIR_NOT FOUND');
        IFOHLE_Pair(_pair).updateDays30PR(_30daysPR);
    }
}
...
interface IFOHLE_Pair {
    ...
    function getSlots() external view returns (uint112 liquiditySlots, uint112 fullSlots);
...
interface IFOHLE_ERC20 {
    ...
    //function lockTime(address owner) external view returns (uint256); no se sabe si aqui o al IERC20
}
...
contract FOHLE_Pair is IFOHLE_Pair, FOHLE_ERC20 {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    if (liquidity > 0) _mint(feeTo, liquidity); //LP sended, not fees directly
                }
            }
        } else if (_kLast != 0) {
            kLast = 0;
    ...
    function updateLiquiditySlots() internal {
        ...
        uint256 _days30Fees0=0;
        uint256 _days30Fees1=0;
    ...
    function liquidityLock(address sender, uint256 amountLP) external lock returns (uint256 _lockTime){
        ...
        fullSlots = uint112(uint256(fullSlots).add(amountLP));
        if (activeA) {
    ...
    function autoEndBooster (address lockedAddress, uint256 lockedLP) internal returns (address updatedAddress) {
        if (block.timestamp > lockTime[lockedAddress] && lockedAddress != address(0)) {
            ...
            fullSlots = uint112(uint256(fullSlots).sub(lockedLP));
