...
abstract contract AccountControl is RoleManager {
    ...
    ///@dev Throws an error after passing a zero address as an account address
    modifier isNotZeroAddress(address account) {
        require(account != address(0), "AC: account cannot be a zero address");
        _;
    }
     ...
     */
    function unFreezeAccount(address account) external onlyRole(ISSUER_ROLE) isNotZeroAddress(account) {
     ...
     */
    function unWhitelistAccount(address account) public onlyRole(ISSUER_ROLE) isNotZeroAddress(account) {
     ...
     * Emits AccountGreylisted event.
     */
    function bulkGreylistAccount(address[] calldata accounts) external {
        for (uint256 i = 0; i < accounts.length; i++) {
            address account = accounts[i];
     ...
     * Emits AccountGreylisted event.
     */
    function unGreylistAccount(address account) public onlyRole(ISSUER_ROLE) isNotZeroAddress(account) {
     ...
     * Emits AccountGreylisted event.
     */
    function bulkUnGreylistAccount(address[] calldata accounts) external {
        for (uint256 i = 0; i < accounts.length; i++) {
            address account = accounts[i];
...
contract RoleManager is AccessControl {
    ...
    /// @dev Constructor for assigning default admin role
    /// @param admin address to be assigned DEFAULT_ADMIN role
    constructor(address admin) {
        // require(admin != address(0), "SX1155: Invalid admin address");
        // Default Admin
...
contract SX1155NFT is Pausable, SX1155NFTBase, RoleManager, AuthorizeManager, AccountControl {
    ...
    constructor(
        ...
        address _admin
    ) SX1155NFTBase(_name, _symbol) RoleManager(_admin) {}
    ...
    /// @dev Returns pause status of the contract.
    ...
    function requestTransfer(
        ...
        emit GreylistTransferRequestCreated(reqId, from, to, tokenId, quantity);
        return reqId;
    }
    ...
    function approveTransferRequest(uint256 reqId) external onlyRole(ISSUER_ROLE) {
        ...
        emit GreylistTransferApproved(reqId, _msgSender(), req.tokenId);
    }
    ...
    function forceBurnBatch(
        ...
        require(
            (_accounts.length == _ids.length) && (_accounts.length == _quantities.length),
            "SX1155: Invalid inputs"
        );

        for (uint256 i = 0; i < _accounts.length; i++) {
            _burnToken(_accounts[i], _ids[i], _quantities[i]);
            emit ForceBurn(_msgSender(), _accounts[i], _ids[i], reason);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (hasRole(ISSUER_ROLE, _msgSender())) return;
...
contract SX1155NFTBase is ERC1155 {
    ...
    /// @dev Burns a batch of token ids, called by owner/operator of the tokens
