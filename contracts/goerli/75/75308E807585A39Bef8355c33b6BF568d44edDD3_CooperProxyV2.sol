...
contract CooperProxyV2 is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    event InsufficientFunds(address token, address feeRecipient, uint256 feeAmount);
    ...
    function createLBP(PoolConfig memory poolConfig) external returns (address) {
        ...
        ERC20Token(poolConfig.tokens[0]).transferFrom(msg.sender, address(this), poolConfig.amounts[0]);
        ERC20Token(poolConfig.tokens[1]).transferFrom( msg.sender, address(this), poolConfig.amounts[1]);
        ERC20Token(poolConfig.tokens[0]).approve(VAULT, poolConfig.amounts[0]);
        ERC20Token(poolConfig.tokens[1]).approve(VAULT, poolConfig.amounts[1]);
     ...
     */
    function _transferTokenToPoolOwner(address pool, address token, uint256 amount) public {
        if (ERC20Token(token).balanceOf(token) >= amount) {
                ERC20Token(token).transfer(
                msg.sender,
                amount
            );
            ...
            emit InsufficientFunds(token, msg.sender, amount);
        }
    }
    ...
    function _distributeSafeFee(address pool, address fundToken, uint256 totalFeeAmount) private {
        // ERC20Token(fundToken).approve(owner(), totalFeeAmount);
        if (ERC20Token(fundToken).balanceOf(fundToken) >= totalFeeAmount) {
            ERC20Token(fundToken).transfer(owner(), totalFeeAmount);
            emit TransferredFee(pool, fundToken, owner(), totalFeeAmount);
        } 
        else {
            emit InsufficientFunds(fundToken, owner(), totalFeeAmount);
        }
    }
    ...
    function _distributePlatformAccessFee(address pool, address fundToken, uint256 totalFeeAmount) private {
        ...
        for (uint256 i = 0; i < recipientsLength; i++) {
            ...
            uint256 balance = ERC20Token(fundToken).balanceOf(owner());
            if(balance < proportionalAmount){
                revert("Insufficient balance");
            }
            if (ERC20Token(fundToken).balanceOf(fundToken) >= proportionalAmount) {
                ERC20Token(fundToken).transfer(recipientAddress, proportionalAmount);
                emit TransferredFee(pool, fundToken, recipientAddress, proportionalAmount);
            } 
            else {
                emit InsufficientFunds(fundToken, recipientAddress, proportionalAmount);
            }
...
contract ERC20Token is ERC20, ERC20Burnable, Pausable, Ownable, ERC20Permit {
    constructor() ERC20("ERC20Token", "MTK") ERC20Permit("ERC20Token") {}
