...
contract RiggedRoll is Ownable {
    ...
    address private diceGameAddressCopy; 
    constructor(address payable diceGameAddress) {
        diceGame = DiceGame(diceGameAddress);
        diceGameAddressCopy = diceGameAddress;
    }
    ...
    function withdraw(address _addr, uint256 _amount) external onlyOwner {
      ...
      require(ownerBalance > 0, "No ether to withdraw");
      require(_amount <= ownerBalance, "Not enough funds available to withdraw that amount");
    ...
    function riggedRoll() external {
        require(address(this).balance >= .002 ether, "Not enough ether in contract to call");
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash,  diceGame, diceGame.nonce()));
        uint256 roll = uint256(hash) % 16;
