...
contract ArtTokenVaultFactory is Ownable, Pausable, Whitelist {
    ...
    mapping(uint32 => address) public vaults;
    ...
    constructor(ILisaSettings _settings) {
        ...
        _grantRole(WHITELISTER_ROLE, _settings.platformAdmin());
    }
    ...
    function deployTokenVault(
    ...
    ) external whenNotPaused onlyRole(PARTICIPANT_ROLE) returns (uint32) {
        ...
        uint256 nftTokenId = nftContract.mintItem(vault, nftTokenURI);
...
contract InitializedProxy {
    ...
    constructor(address _logic, bytes memory _initializationCalldata) {
        require(
            address(_logic) != address(0),
            "_logic address should not be 0"
        );
        logic = _logic;
