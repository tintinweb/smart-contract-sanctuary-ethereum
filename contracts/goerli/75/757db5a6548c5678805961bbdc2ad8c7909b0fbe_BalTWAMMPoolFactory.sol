...
contract BalTWAMMPoolFactory is BasePoolFactory {
  ...
  function create(
    ...
    requireErrCode(_poolType < 3, CronErrors.INVALID_POOL_TYPE);
    ...
    getPool[token1][token0][_poolType] = pool;
...
contract BalTwamm is IMinimalSwapInfoPool, BalancerPoolToken, ReentrancyGuard {
  ...
  modifier senderIsArbPartnerOrAdmin() {
    _senderIsArbPartnerOrAdmin(); // #contractsize
    _;
  }
  ...
  function onSwap(
    ...
    if (swapType == SwapType.LongTermSwap) {
      _longTermSwap(from, tokenIn, amount, argument);
    } else {
      ...
      if (swapType == SwapType.RegularSwap) {
        ...
        requireErrCode(arbPartnerAddrMap[from], CronErrors.SENDER_NOT_PARTNER);
      } else if (swapType == SwapType.KDAOSwap) {
        requireErrCode(Rook(rookWhiteList).isWhitelistedKeeper(from), CronErrors.SENDER_NOT_KEEPER);
      } else {
    ...
    returns (uint256[] memory amountsInU112, uint256[] memory dueProtocolFeeAmounts)
  {
    ...
    requireErrCode(_currentBalancesU112.length == 2 && amountsInU112.length == 2, CronErrors.INVALID_ARGUMENTS);

    // #savesize #savegas
    uint256 token0In = amountsInU112[TOKEN0_IDX];
    uint256 token1In = amountsInU112[TOKEN1_IDX];
    uint256 token0Balance = _currentBalancesU112[TOKEN0_IDX];
    uint256 token1Balance = _currentBalancesU112[TOKEN1_IDX];

    uint256 amountLP;
    if (joinType == JoinType.Join) {
      // Stack Too Deep Opt.:
      amountLP = _mintWrapper(_sender, _recipient, token0In, token1In, token0Balance, token1Balance);
      ...
      _executeVirtualOrders(token0Balance, token1Balance, block.number);
    } else {
    ...
    (slot4, dueProtocolFeeAmounts[TOKEN0_IDX], dueProtocolFeeAmounts[TOKEN1_IDX]) = BitPackingLib.zeroPairU96(slot4);

    _updateBalancerFee(_protocolFeeDU1F18);
  }
    ...
    returns (uint256[] memory amountsOutU112, uint256[] memory dueProtocolFeeAmounts)
  {
    ...
    if (exitType == ExitType.FeeWithdraw) {
      (token0Out, token1Out) = _withdrawCronFiFees(_sender);
    } else {
      // Stack too deep opt.:
      {
        (uint256 token0Reserve, uint256 token1Reserve) = _executeVirtualOrders(
  ...
  /* TODO: Begin Remove / optimize this section for space */

  // Slot 1:
  //
  function shortTermFeeBP() external view returns (uint256) {
    return BitPackingLib.unpackU10(slot1, S1_OFFSET_SHORT_TERM_FEE_BP);
  }

  function partnerFeeBP() external view returns (uint256) {
    return BitPackingLib.unpackU10(slot1, S1_OFFSET_PARTNER_FEE_BP);
  }

  function longTermFeeBP() external view returns (uint256) {
    return BitPackingLib.unpackU10(slot1, S1_OFFSET_LONG_TERM_FEE_BP);
  }

  function orders() external view returns (uint256 token0, uint256 token1) {
    (token0, token1) = BitPackingLib.unpackPairU112(slot1);
  }

  // Slot 2:
  //
  function oracleTimeStamp() external view returns (uint256) {
    return BitPackingLib.unpackOracleTimeStampS2(slot2);
  }

  function proceeds() external view returns (uint256 token0, uint256 token1) {
    (token0, token1) = BitPackingLib.unpackPairU112(slot2);
  }

  // Slot 3:
  //
  function feeShiftU3() external view returns (uint256) {
    return BitPackingLib.unpackFeeShiftS3(slot3);
  }

  function holdingPenaltyBP() external view returns (uint256) {
    return BitPackingLib.unpackU10(slot3, S3_OFFSET_HOLDING_PENALTY_BP);
  }

  function holdingPeriodSec() external view returns (uint256) {
    return BitPackingLib.unpackHoldingPeriodSecS3(slot3);
  }

  function cronFiFees() external view returns (uint256 token0, uint256 token1) {
  ...
  function collectBalancerFees() external view returns (bool) {
    return BitPackingLib.unpackBit(slot4, S4_OFFSET_COLLECT_BALANCER_FEES) != FALSE;
  }

  function balancerFeeDU1F18() external view returns (uint256) {
    return BitPackingLib.unpackBalancerFeeS4(slot4);
  }

  function balancerFees() external view returns (uint256 token0, uint256 token1) {
    (token0, token1) = BitPackingLib.unpackPairU96(slot4);
  }

  /* TODO: End Remove / optimize this section for space */
  ...
  /// @notice TODO
  /// @dev Sorry - this is ugly AF, but saves ~200 bytes of contract size.
  ///
  function setParameter(uint256 _paramTypeU, uint256 _value) public senderIsAdmin nonReentrant {
    ...
    } else if (paramType == ParamType.CollectCronFiFee) {
      offset = S4_OFFSET_FEE_ENABLED;
    } else if (paramType == ParamType.CollectBalancerFee) {
      offset = S4_OFFSET_COLLECT_BALANCER_FEES;
    }
    ...
    if (_paramTypeU <= 2 && offset != 0 && _value <= MAX_FEE_BP) {
    ...
    } else if (paramType == ParamType.FeeSharesLP) {
      // Slot 3:
      ...
      slot3 = BitPackingLib.packFeeShiftS3(slot3, value);
    ...
    } else if (_paramTypeU <= 7 && offset != 0 && _value <= MAX_BOOL) {
      // Slot 4:
      slot4 = BitPackingLib.packBit(slot4, _value, offset);
    } else {
      revertErrCode(CronErrors.PARAM_ERROR);
    }
  ...
  function transferMintEvent(
    ...
    requireErrCode(msg.sender != _dest, CronErrors.CANNOT_TRANSFER_TO_SELF);
    requireErrCode(_dest != NULL_ADDR, CronErrors.CANNOT_TRANSFER_TO_NULL);
  ...
  /// @notice TODO:
  function _senderIsArbPartnerOrAdmin() internal view {
    requireErrCode(adminAddrMap[msg.sender] || arbPartnerAddrMap[msg.sender], CronErrors.SENDER_NOT_ADMIN_OR_PARTNER);
  }
  ...
  function _shortTermSwap(
    ...
    if (_tokenIn == TOKEN0) {
      ...
      slot4 = BitPackingLib.incrementWithClampU96(localSlot4, balancerFeeU112, S4_OFFSET_TOKEN0_BALANCER_FEES);
      ...
      slot4 = BitPackingLib.incrementWithClampU96(localSlot4, balancerFeeU112, S4_OFFSET_TOKEN1_BALANCER_FEES);
    }
  }
  ...
  function _longTermSwap(
    ...
    uint256 salesRateOffset = (token0To1) ? P_OFFSET_TOKEN0_SALES_RATE : P_OFFSET_TOKEN1_SALES_RATE;

    ops.currentSalesRates = BitPackingLib.incrementU112(ops.currentSalesRates, sellingRate, salesRateOffset);

    ops.salesRatesEndingPerBlock[orderExpiry] = BitPackingLib.incrementU112(
    ...
    virtualOrders.orderMap[orderId] = Order(
      token0To1,
      uint112(sellingRate),
      uint128(BitPackingLib.unpackU128(ops.scaledProceeds, token0To1)),
      _sender,
      orderExpiry
    );
    ...
    slot1 = BitPackingLib.incrementU112(
      slot1,
      sellingRate * tradeBlocks,
      (token0To1 ? S1_OFFSET_TOKEN0_ORDERS : S1_OFFSET_TOKEN1_ORDERS)
    );
  ...
  /// @notice TODO
  function _withdrawCronFiFees(address _sender) private returns (uint256 token0Out, uint256 token1Out) {
    ...
    if (zeroCronFiFees != FALSE) {
      ...
      (slot3, token0Out, token1Out) = BitPackingLib.zeroPairU96(slot3);
    ...
    )
  {
    ...
    if (_cancel) {
      ...
      uint256 salesRateOffset = (token0To1) ? P_OFFSET_TOKEN0_SALES_RATE : P_OFFSET_TOKEN1_SALES_RATE;
      ops.currentSalesRates = BitPackingLib.decrementU112(ops.currentSalesRates, stakedAmount, salesRateOffset);
      ops.salesRatesEndingPerBlock[expiry] = BitPackingLib.decrementU112(
      ...
      slot1 = BitPackingLib.decrementU112(
        slot1,
        refundU112,
        (token0To1) ? S1_OFFSET_TOKEN0_ORDERS : S1_OFFSET_TOKEN1_ORDERS
      );
    ...
    slot2 = BitPackingLib.decrementU112(
      slot2,
      proceedsU112,
      (token0To1) ? S2_OFFSET_TOKEN1_PROCEEDS : S2_OFFSET_TOKEN0_PROCEEDS
    );
  ...
  function _mintWrapper(
    ...
    amountLP = (lTotalSupply == 0)
      ? _initialMint(_token0In, _token1In)
      : _mint(_token0In, _token1In, lTotalSupply, _token0Balance, _token1Balance);
  ...
  ///     #savegas
  function _initialMint(uint256 _token0In, uint256 _token1In) private returns (uint256 amountLP) {
    // NOTE: There are no orders, proceeds or fees collected yet so no need to check that the sum of the aforementioned
    //       exceeds MAX_U112 with the amount being minted.
    requireErrCode(_token0In <= MAX_U112, CronErrors.TOO_MUCH_TOKEN0_LIQUIDITY); // R1
    requireErrCode(_token1In <= MAX_U112, CronErrors.TOO_MUCH_TOKEN1_LIQUIDITY); // R2
    requireErrCode(
  ...
  function _mint(
    ...
    //
    requireErrCode(
      _token0In <= MAX_U112 && (_token0In + token0Reserve) <= MAX_U112,
      CronErrors.TOO_MUCH_TOKEN0_LIQUIDITY
    ); // R1
    requireErrCode(
      _token1In <= MAX_U112 && (_token1In + token1Reserve) <= MAX_U112,
      CronErrors.TOO_MUCH_TOKEN1_LIQUIDITY
    ); // R2

    _amountLP = Math.min(
      _token0In.mul(_lTotalSupply).divDown(token0Reserve),
      _token1In.mul(_lTotalSupply).divDown(token1Reserve)
    );
  }
  ...
  function _updateBalancerFee(uint256 _protocolFeeDU1F18) private {
    if (_protocolFeeDU1F18 <= ONE_DU1_18) {
      ...
      if (currentBalancerFee != _protocolFeeDU1F18) {
        slot4 = BitPackingLib.packBalancerFeeS4(localSlot4, _protocolFeeDU1F18);
      }
    } else {
  ...
  {
    (uint256 token0TwammRes, uint256 token1TwammRes) = calcTwammReserves(_token0Balance, _token1Balance);
...
library BitPackingLib {
  ...
  /// @notice TODO
  /// TODO: What makes sense when checking increment maximum on clamped values?
  ///       - A value > MAX_U96 but less than MAX_U112?
  function incrementWithClampU96(
    uint256 _slot,
    uint256 _increment,
    uint256 _offset
  ) internal pure returns (uint256 slot) {
    requireErrCode(_increment <= MAX_U96, CronErrors.INCREMENT_TOO_LARGE);

    uint256 valueU96 = ((_slot >> _offset) & MAX_U96) + _increment;

    if (valueU96 > MAX_U96) {
      valueU96 = MAX_U96;
    }

    // NOTE: No need create set mask as valueU96 above checked against MAX_U96/clamped.
    uint256 clearMask = ~(MAX_U96 << _offset);
    slot = (_slot & clearMask) | (valueU96 << _offset);
  }
  ...
  function incrementPairWithClampU96(
    ...
    uint256 value0U96 = ((_slot >> 96) & MAX_U96) + _increment0;
    uint256 value1U96 = (_slot & MAX_U96) + _increment1;
  ...
  function zeroPairU96(uint256 _slot)
    internal
    pure
    returns (
      uint256 slot,
  ...
  /// @notice TODO
  function incrementU112(
    uint256 _slot,
    uint256 _increment,
    uint256 _offset
  ) internal pure returns (uint256 slot) {
    requireErrCode(_increment <= MAX_U112, CronErrors.INCREMENT_TOO_LARGE);

    uint256 valueU112 = ((_slot >> _offset) & MAX_U112) + _increment;

    requireErrCode(valueU112 <= MAX_U112, CronErrors.OVERFLOW);

    // NOTE: No need create set mask as valueU112 above checked against MAX_U112.
    uint256 clearMask = ~(MAX_U112 << _offset);
    slot = (_slot & clearMask) | (valueU112 << _offset);
  }

  /// @notice TODO
  function decrementU112(
    uint256 _slot,
    uint256 _decrement,
    uint256 _offset
  ) internal pure returns (uint256 slot) {
    uint256 valueU112 = ((_slot >> _offset) & MAX_U112);

    requireErrCode(valueU112 >= _decrement, CronErrors.UNDERFLOW);

    valueU112 -= _decrement;

    // NOTE: No need create set masks as valueU112 above less than MAX_U112 (correct by
  ...
  function incrementPairU112(
    ...
    uint256 value0U112 = ((_slot >> 112) & MAX_U112) + _increment0;
    uint256 value1U112 = (_slot & MAX_U112) + _increment1;
...
library VirtualOrderLib {
  ...
  function executeVirtualOrdersToBlock(
    ...
    while (expiryBlock < _blockNumber) {
      ...
      _self.scaledProceedsAtBlock[expiryBlock] = BitPackingLib.packPairU128(
  ...
  function updateFees(
    ...
    if (feeShareU60 == 0) {
      ...
      uint256 feeShiftU4 = _evoMem.feeShiftU4; // #savegas #savesize
      lpFeeT0 = feeShareT0 << feeShiftU4;
      lpFeeT1 = feeShareT1 << feeShiftU4;
