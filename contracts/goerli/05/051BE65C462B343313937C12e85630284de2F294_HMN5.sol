...
import "./lib/DefaultOperatorFilterer.sol";
import "erc721a/contracts/ERC721A.sol";

contract HMN5 is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {

    mapping(uint256 => bool) public steezyCloneTracker;
    mapping(uint256 => uint256) public cloneOf;
    mapping(address => uint256) public allowListTracker;
    ...
    function allowListMint(uint256 mintQuantity, uint256[] calldata steezyTokenIds, uint256 totalGenesis, uint totalPioneer, uint totalGiveaway, bytes32[] calldata proof) external nonReentrant payable {
        
        require(MerkleProof.verify(proof, merkleRoot, keccak256(abi.encodePacked(msg.sender, totalGenesis, totalPioneer, totalGiveaway))), "Invalid proof");
        
        uint256 quantity = steezyTokenIds.length + mintQuantity;
        uint256 cloneQuantity = steezyTokenIds.length;
        uint256 totalFree = (totalGenesis * 2) + (totalPioneer * 2) + totalGiveaway;
        uint256 freeRemaining = totalFree - allowListTracker[msg.sender];
        uint256 quantityPaid = freeRemaining > quantity ? 0 : quantity - freeRemaining;
        uint256 actualFree = quantity - quantityPaid;

        allowListTracker[msg.sender] += actualFree;
        
        require(msg.value == clonePrice * quantityPaid, "Invalid ETH Amount");

        if(cloneQuantity > 0) {
            if(address(okinaLabs) != address(0)) {
                okinaLabs.burn(msg.sender, 1, cloneQuantity);
            }
    ...
    function clone(uint256[] calldata steezyTokenIds) external nonReentrant payable {    
        ...
        require(quantity > 0, "Must clone at least one Steezy Ape Gang");
        require(msg.value == clonePrice * quantity, "Invalid ETH Amount");

        if(address(okinaLabs) != address(0)) {
            okinaLabs.burn(msg.sender, 1, quantity);
        }
    ...
    function _setCloneData(address account, uint256[] calldata steezyTokenIds) internal {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            require(steezyCloneTracker[steezyTokenId] == false, "This SteezyApeGang has already been cloned");
            steezyCloneTracker[steezyTokenId] = true;
            cloneOf[_totalMinted() + i + 1] = steezyTokenId;
        }
    }
    ...
    uint256 public cloneTransactionLimit = 20;
    ...
    uint256 public mintTransactionLimit = 20;
    ...
    function setCloneOf(uint256 tokenId, uint256 steezyApeGangTokenId) external onlyOwner {
        ...
        if(steezyApeGangTokenId == 0) {
            steezyCloneTracker[steezyApeGangTokenId] = false;
        } else {
            steezyCloneTracker[steezyApeGangTokenId] = true;
        }
    }
    ...
    function releaseFunds() external onlyOwner {
        ...
        uint okina = balance / 100 * 85;
        uint dev = balance / 100 * 15;
        Address.sendValue(payable(0xA6021476F90FDE5b4Fc2bC4C22cA26008F2A23f7), okina);
