...
contract GreatNFT is ERC721AUpgradeable, PausableUpgradeable, OwnableUpgradeable, DefaultOperatorFiltererUpgradeable {
    ...
    function initialize(bytes32 _root, uint256 _maxMint, uint256 _maxSupply) public initializerERC721A initializer {
        root = _root;
        maxMint = _maxMint;
    ...
    function mint(uint256 quantity, bytes32[] memory proof) external payable {
        require(!isPreSaleOn || !isPublicSaleOn, "Sale is on");
        require(totalSupply() <= maxSupply, "Max supply exceeded");
        require(isWhiteListed(proof, keccak256(abi.encodePacked(msg.sender))), "Not whitelisted");
        require(qtyMintedPerUser[msg.sender] <= maxMint, "Max wallet exceeded");
    ...
    function buyPresale(uint256 quantity, bytes32[] memory proof) external payable {
        require(isPreSaleOn && !isPublicSaleOn, "PreSale is NOT on");
        require(totalSupply() <= maxSupply, "Max supply exceeded");
    ...
    function buyPublicsale(uint256 quantity, bytes32[] memory proof) external payable {
        require(!isPreSaleOn && isPublicSaleOn, "PublicSale is NOT on");
        require(totalSupply() <= maxSupply, "Max supply exceeded");
    ...
    function startPresale() external onlyOwner {
        require(!isPreSaleOn && !isPublicSaleOn, "Can't start");
        isPreSaleOn = true;
    }

    function startPublicsale() external onlyOwner {
        require( !isPublicSaleOn && !isPreSaleOn, "Can't start");
        isPublicSaleOn = true;
    }
    ...
    function stopPublicsale() external onlyOwner {
        require(isPublicSaleOn, "Publicsale is not on");
