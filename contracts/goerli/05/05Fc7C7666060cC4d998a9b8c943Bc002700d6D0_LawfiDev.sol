...
contract LawfiDev is Ownable2Step, ERC721Enumerable {
  ...
  constructor() ERC721("LFDAODEV", "LawFi DAO Dev") {}
  ...
  function claim(
    ...
    bytes32 message = keccak256(abi.encodePacked("lawfi-claim", msg.sender, _maxAmount));
  ...
  function mint(bytes[] calldata _tokens) external onlyOwner {
    for (uint256 i = 0; i < _tokens.length; i++) {
      (uint32 tokenType, address receiver) = abi.decode(_tokens[i], (uint32, address));
      uint256 tokenId = totalSupply();
      _safeMint(receiver, tokenId);
      tokens[tokenId].tokenType = tokenType;
    }
  }

  // verified
  function transferBatch(bytes[] calldata _tokens) external onlyOwner {
    for (uint256 i = 0; i < _tokens.length; i++) {
      (address from, address to, uint256 tokenId) = abi.decode(_tokens[i], (address, address, uint256));
  ...
  // verified
  function setTokenType(uint256[] calldata _tokenIds, uint32[] calldata _types) external onlyOwner {
    require(_tokenIds.length == _types.length, "Invalid input");
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      tokens[_tokenIds[i]].tokenType = _types[i];
    }
  }
