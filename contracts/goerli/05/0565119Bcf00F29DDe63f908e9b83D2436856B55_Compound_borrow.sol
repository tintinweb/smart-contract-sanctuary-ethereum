...
contract Compound_borrow is AccessControl{
    ...
    ////поставка токенов для compound c Token
    function supply(IERC20 token,uint _amount) external {
        address CToken = tokenForCtoken[address(token)];
        require(CErc20(CToken).redeemUnderlying(_amount) == 0);   
        token.transferFrom(msg.sender, CToken, _amount);
        token.approve(CToken, _amount);
    ...
    function getCTokenBalance(address _cToken) external view returns (uint) {
        return CErc20(_cToken).balanceOf(msg.sender);
    }
    function balanceOfUnderlying(address _cToken) external returns (uint) {
        return CErc20(_cToken).balanceOfUnderlying(msg.sender);
    }
    ...
    // collateral
    function getCollateralFactor(address _cToken) external view returns (bool,uint,bool) {
        (bool isListed, uint colFactor, bool isComped) = comptroller.markets(address(_cToken));
    ...
    function getAccountLiquidity()external view returns (uint liquidity, uint shortfall){
        ...
        (uint error, uint _liquidity, uint _shortfall) = comptroller.getAccountLiquidity(msg.sender);
  ...
  // open price feed - USD price of token to borrow
    ...
    function borrow(address _cTokenToBorrow, uint _decimals) external {
        ...
        cTokens[0] = address(_cTokenToBorrow);
        uint[] memory errors = comptroller.enterMarkets(cTokens);
        require(errors[0] == 0, "Comptroller.enterMarkets failed.");
        // check liquidity
        (uint error, uint liquidity, uint shortfall) = comptroller.getAccountLiquidity(msg.sender);
        require(error == 0, "error");
