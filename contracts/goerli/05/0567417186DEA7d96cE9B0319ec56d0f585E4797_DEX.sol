...
contract DEX {
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        ...
        // We want the balance before the user has transfer ETH tokens for computation.
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        require(tokenInput > 0, "cannot swap of Balloons");
        uint256 tokenReserve = token.balanceOf(address(this));
        ...
        require(sent, "sending Eth to msg.sender failed");
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        require(msg.value > 0, "Deposit ETH value must be greater than 0.");

        uint256 ethReserve = address(this).balance.sub(msg.value);
        uint256 tokenReserve = token.balanceOf(address(this));
        uint256 tokenDeposit = msg.value.mul(tokenReserve) / ethReserve + 1;
