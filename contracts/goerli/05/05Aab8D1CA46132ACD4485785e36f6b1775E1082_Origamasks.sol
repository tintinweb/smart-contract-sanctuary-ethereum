    ...
    VRFV2WrapperConsumerBase
{
        ...
        VRFV2WrapperConsumerBase(linkAddress_, wrapperAddress_)
    {
        ...
        tokensMinted[1] = origamasksAddress_; // secure spot for reserve
    }
     ...
     */
    function batchMints(address[] memory tos_) 
        external
        onlyOwner
        nonReentrant
        isSaleState(SaleState.Finished)
    {
        // Loop through owners' addresses
        for (uint256 i = 0; i < tos_.length; i++) {
            ...
            for (uint256 j = 0; j < _tokenIds.length; j++) {
                ...
                emit Minted(_receiver, SaleState.Finished, 0, _tokenIds[j]);
            }
        }
    }
    ...
    */
    ...
    /*
    // @dev Dynamic tokenURI based on token level
    */
    ...
    */
    ...
    //level => require "days" to level up
     ...
     */
    function setNumberReservedToken(uint256 numberReserved_) external onlyOwner {
        // if(totalSupply + numberReserved_ > collectionSize) revert ExceedCollectionSize();
    ...
    function withdraw() external onlyOwner {
        (bool success, ) = payable(origamasksAddress).call{value: address(this).balance}(
            ""
        );
    ...
    /**
     * @dev Withdraw for Airdrop fee
     */
    function withdrawForAirdrop(uint256 amountInWei_) external onlyOwner {
        (bool success, ) = payable(msg.sender).call{value: amountInWei_}("");
        if (!success) revert TransferFailed();
    }
