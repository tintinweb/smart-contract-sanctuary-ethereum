...
contract HCB_NFT is ERC721Enumerable, Ownable {
    ...
    modifier isValidMintAmount(uint256 _mintAmount, uint256 _supply) {
        ...
        require(supply + _mintAmount <= _supply);
    ...
    payable
    onlyOwner {
        require(!drop, "Only once");
        require(members.length > 0, "Empty members");
        uint256 tokenNum = dropSupply / members.length;
        drop = true;
        for(uint i = 0; i < members.length; i++) {
            Member memory m = members[i];
            uint256 supply = totalSupply();
            for(uint j = 1; j <= tokenNum; j++) {
                _safeMint(m.account, supply + j);
            }
        }
    }
