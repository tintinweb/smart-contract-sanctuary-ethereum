...
contract SubjectRoot is SubjectWithGeneChanger, ISubjectRoot, TunnelEnabled {
    ...
    // [Comment for audit]: it wouldn't be used on mainnet deployed contracts, needed just for current test structure
    ...
    * @notice Invokes `bulkBuy` if presale has started and msg.sender is whitelisted.
        ...
        onlyDAO
    {
        ...
        for (uint256 i = 0; i < entries.length; i++) {
            ...
            // [Comment for audit]: ensuring that we're not rewriting _bossTokens mapping, so we really get 10 boss tokens
        ...
        nonReentrant
    {
        ...
        if (genePosition > 1 && genePosition < 7) {
            ...
            if ((geneGenerator.random() % 100) + 1 <= _rareTraitsChances[genePosition]) {
                ...
                // Clean up rare trait
                _genes[tokenId] = replaceGene(oldGene, 0, genePosition + 5);
            }
        } else {
