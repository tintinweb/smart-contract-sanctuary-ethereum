 ...
 * @notice Drivers is celebrated motion artist Everfresh’s first Fount Gallery release.
 *
 * Features:
 *   - Open edition NFTs
 ...
 */
contract DriversOpenEditions is ERC1155Base, Withdraw {
    /* ------------------------------------------------------------------------
       S T O R A G E
    ------------------------------------------------------------------------ */
    ...
    error CannotDeleteTokenDataWhenTokenHasSold();
    ...
    constructor(
        ...
        address metadata_,
        address fountCard_
    ) ERC1155Base(owner_, admin_, royaltiesAmount_, metadata_, fountCard_) {}
    ...
    function _collectEdition(
        ...
        if (tokenData.mintPerAddress > 0) {
            // Check the amount doesn't exceed the per wallet allowance to mint
            if (amount > tokenData.mintPerAddress) revert AmountExceedsMaxWalletMint();

            // Get the accounts remaining balance for this edition
            uint256 currentBalance = balanceOf[to][id];
            uint256 remainingAllowance = currentBalance >= tokenData.mintPerAddress
                ? 0
                : tokenData.mintPerAddress - currentBalance;
        ...
        unchecked {
            tokenData.minted += uint32(mintAmount);
        }
    ...
    function addTokenForSale(
        ...
        tokenData.mintPerAddress = mintPerAddress;
    ...
    function setTokenSaleTimes(
        ...
        // exists. Setting to zero will effectively "delete" the token data for other functions.
        // Must use `deleteTokenData` if a deletion is needed.
        if (startTime == 0) revert CannotSetStartTimeToZero();
    ...
    /**
     * @notice Admin function to delete the token data for a token
     * @dev Used if the edition needs to be removed before any mints are recorded.
     *
     * Reverts if:
     *  - the token data does not exist
     *  - if the token data already has mints recorded
     *
     * @param id The token id
     */
    function deleteTokenData(uint256 id) external onlyOwnerOrAdmin {
        TokenData memory tokenData = _idToTokenData[id];
        if (tokenData.startTime == 0) revert TokenDataDoesNotExist();

        // Check there are no mints already
        if (tokenData.minted != 0) revert CannotDeleteTokenDataWhenTokenHasSold();

        // Delete the token data
        delete _idToTokenData[id];
    }
    ...
    function tokenMintedCount(uint256 id) external view returns (uint256) {
        return _idToTokenData[id].minted;
    }

    function tokenMintAllowancePerAddress(uint256 id) external view returns (uint256) {
        return _idToTokenData[id].mintPerAddress;
    }

    function tokenRemainingMintAllowanceForAddress(uint256 id, address account)
        external
        view
        returns (uint256)
    {
     ...
     * @dev Withdraws to the `to` address
    ...
    OperatorFilterer
{
    ...
    constructor(
        ...
        Royalties(owner_, royaltiesAmount_)
