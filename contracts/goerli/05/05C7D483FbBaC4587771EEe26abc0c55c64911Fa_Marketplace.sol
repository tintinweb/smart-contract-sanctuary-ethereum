...
contract Marketplace is IMarketplace, Ownable, ERC4907 {
	mapping(address => bool) public isAdvisor;
	mapping(bytes32 => Offer) public offerData;
	uint256 private _saleFeeAmount;
	uint256 private _rentFeeAmount;
	...
	modifier onlyActor(address _sender_addr, address _seller, address _buyer) {
		require(
			isAdvisor[_sender_addr] || _sender_addr == _seller || _sender_addr == _buyer,
			'The caller must be one of Seller, Buyer and Advisor.'
		);
		_;
	}
	...
	function sendOffer(
		...
		uint64 _expires
	) public onlyActor(msg.sender, _seller, _buyer) {
		require(
		...
		offerData[_generatedId] = Offer(
			...
			_buyerAcceptStatus,
			_sellerAcceptStatus,
			0,	// waiting for both buyer/seller to accept this offer
			_expires
		);
	...
	function acceptOffer(bytes32 _offerId) public onlyContractor(_offerId, msg.sender) {
		...
		if (
			...
			uint256 _fee = (_offer.price * _saleFeeAmount) / (100 * _factor);
			if (_offer.offerType == 1) {
				_fee = (_offer.price * _rentFeeAmount) / (100 * _factor);
			}
			uint256 _realPrice = _offer.price - _fee;

			address _admin = owner();

			if (_offer.offerType == 0) {	// Sale
				IERC721(_offer.collection).transferFrom(_offer.seller, _offer.buyer, _offer.assetId);
			}
			else { 	// Rent
				_mint(_offer.seller, _offer.assetId);
				setUser(_offer.assetId, _offer.buyer, _offer.expires);
			}
	...
	function cancelOffer(bytes32 _offerId) public {
		...
		require(_offer.status != 2, 'The offer already canceled.');
		require(_offer.status == 0, "");
		require(msg.sender == _offer.offerSender, 'Only the offer creator could cancel this offer.');

		if (msg.sender != _offer.buyer) { 	// If Seller or Advisor wants to cancel offer
			require(_offer.buyerAcceptStatus == 0, '');
		}
		if (msg.sender != _offer.seller) { 	// If Buyer or Advisor wants to cancel offer
			require(_offer.sellerAcceptStatus == 0, '');
		}
	...
	function declineOffer(bytes32 _offerId) public onlyContractor(_offerId, msg.sender) {
		// Only Buyer or Seller can decline offer.
		require(offerData[_offerId].status != 2, 'The offer already canceled.');
		require(offerData[_offerId].status != 3, 'The offer has already been declined.');
		require(offerData[_offerId].status == 0, ''); 	// Offer status has been updated. status = 1

		if(msg.sender == offerData[_offerId].buyer) {
			require(offerData[_offerId].buyerAcceptStatus != 1, 'Could not decline this offer, because you have already accepted it.');
		} else if(msg.sender == offerData[_offerId].seller) {
			require(offerData[_offerId].sellerAcceptStatus != 1, 'Could not decline this offer, because you have already accepted it.');
		}
...
interface IMarketplace {
	struct Offer {
		uint256 offerType; 	// 0: sale offer, 1: rent offer
		address seller;
		address buyer;
		address collection;
		uint256 assetId;
		...
		uint256 price;
		address offerSender;
		uint256 buyerAcceptStatus; 	// 0: awaiting, 1: accepted, 2: declined
		uint256 sellerAcceptStatus;	// 0: awaiting, 1: accepted, 2: declined
		uint256 status;	// 0: awaiting, 1: accepted, 2: cancelled, 3: declined
		uint64 expires;	// 0: no expiration time
	}
}
