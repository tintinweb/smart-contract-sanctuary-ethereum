...
uint8 constant INVEST_PAUSED        = 1 << 0;
uint8 constant INVEST_TOSS_PAUSED   = 1 << 1;
uint8 constant WITHDRAW_PAUSED      = 1 << 2;
uint8 constant WITHDRAW_TOSS_PAUSED = 1 << 3;

interface IBridgeMinMax {
...
contract ExtranetService is IMCeler, CollectTokens, Shutdown {
    ...
    ExtranetToken public immutable extranetToken;

    uint256 private investMinAmount;
    uint256 private investMaxAmount;
    uint256 private investQueueMinAmount;
    uint256 private withdrawMinAmount;
    uint256 private withdrawMaxAmount;
    ...
    uint256 private minExtranetTokenBalanceToSkipLockedPower;
    uint256 private minLockedPower;
    uint256 private minLockedPowerPerQuoteTokenAmount;

    uint8 private pauses;
    uint16 private maxQueueLength = 10;
    ...
    event RewardReceived(bytes32 rewardMessageId, uint256 amount);
    ...
    IMCeler(_messageBus)
    {
        ...
        extranetToken = ExtranetToken(_extranetToken);
    ...
    function getQueueLengths() public view returns (
        ...
        whenNotPaused(INVEST_PAUSED)
    {
        ...
        require(amount >= investMinAmount && amount > 0, "MIN_AMOUNT");
        require(investMaxAmount == 0 || amount <= investMaxAmount, "MAX_AMOUNT");

        IBridgeMinMax bridge = IBridgeMinMax(IMessageBus(messageBus).liquidityBridge());
        require(amount >= bridge.minSend(address(quoteToken)), "MIN_SEND");

        uint256 maxSend = bridge.maxSend(address(quoteToken));
        require(maxSend == 0 || amount <= maxSend, "MAX_SEND");

        checkIfHasMinLockedPower(amount);
        ...
        emit InvestQueued(investRoundtripId, msg.sender, amount);
    }
        ...
        whenNotPaused(WITHDRAW_PAUSED)
    {
        ...
        require(amount > 0 && amount >= withdrawMinAmount, "MIN_AMOUNT");
        require(withdrawMinAmount == 0 || amount <= withdrawMaxAmount, "MAX_AMOUNT");
    ...
    function tossInvestQueue() public payable whenNotPaused(INVEST_TOSS_PAUSED) {
        require(investQueue.length > 0, "EMPTY");

        uint256 investQueueTotalAmount = _calculateQueueTotalAmount(investQueue);
        require(investQueueTotalAmount >= investQueueMinAmount, "MIN_AMOUNT");

        bytes memory message = abi.encode(investRoundtripId, MESSAGE_KIND_ROUNDTRIP, investQueueTotalAmount, investQueue);
        uint256 fee = sendMessageWithTransfer(address(quoteToken), investQueueTotalAmount, message);
    ...
    function clearInvestQueue() public {
        delete investQueue;
    }

    function tossWithdrawQueue() public payable whenNotPaused(WITHDRAW_TOSS_PAUSED) {
        require(withdrawQueue.length > 0, "EMPTY");

        uint256 withdrawQueueTotalAmount = _calculateQueueTotalAmount(withdrawQueue);
        bytes memory message = abi.encode(withdrawRoundtripId, MESSAGE_KIND_ROUNDTRIP, withdrawQueueTotalAmount, withdrawQueue);
    ...
    function clearWithdrawQueue() public {
        delete withdrawQueue;
    }
    ...
    function onMessageReward(bytes memory message) internal {
        (bytes32 rewardMessageId, , uint256 rewardAmount) = abi.decode(message, (bytes32, uint8, uint256));

        extranetToken.onReward(rewardAmount);

        emit RewardReceived(rewardMessageId, rewardAmount);
    }

    function onMessageRoundtrip(bytes memory message) internal {
    ...
    function onMessageWithTransfer(bytes memory message, address token, uint256 amount) override internal {
        require(token == address(quoteToken), "QUOTE");

        (, uint8 messageKind) = abi.decode(message, (bytes32, uint8));
        require(messageKind == 1, "MESSAGE_KIND");
    ...
    function getLimits() public view returns (
        uint256 _investQueueMinAmount,
        uint256 _investMinAmount,
        uint256 _investMaxAmount,
        uint256 _withdrawMinAmount,
        uint256 _withdrawMaxAmount,

        uint256 _minLockedPower,
        uint256 _minLockedPowerPerQuoteTokenAmount,
        uint256 _minExtranetTokenBalanceToSkipLockedPower,

        uint256 _maxQueueLength,

        uint8 _pauses
    ) {
        IBridgeMinMax bridge = IBridgeMinMax(IMessageBus(messageBus).liquidityBridge());

        _investMaxAmount = bridge.maxSend(address(quoteToken));

        if (investMaxAmount < _investMaxAmount) {
            _investMaxAmount = investMaxAmount;
        }

        _investMinAmount = bridge.minSend(address(quoteToken));
        if (investMinAmount > _investMinAmount) {
            _investMinAmount = investMinAmount;
        }

        _withdrawMinAmount = withdrawMinAmount;
        _withdrawMaxAmount = withdrawMaxAmount;

        _investQueueMinAmount = investQueueMinAmount;

        _minLockedPower = minLockedPower;
        _minLockedPowerPerQuoteTokenAmount = minLockedPowerPerQuoteTokenAmount;
        _minExtranetTokenBalanceToSkipLockedPower = minExtranetTokenBalanceToSkipLockedPower;
    ...
    function setLimits(
        uint16 _maxQueueLength,
        uint256 _investQueueMinAmount,
        uint256 _investMinAmount, uint256 _investMaxAmount,
        uint256 _withdrawMinAmount, uint256 _withdrawMaxAmount
    ) public {
        maxQueueLength = _maxQueueLength;
        investMinAmount = _investMinAmount;
        investMaxAmount = _investMaxAmount;
        investQueueMinAmount = _investQueueMinAmount;
        withdrawMinAmount = _withdrawMinAmount;
        withdrawMaxAmount = _withdrawMaxAmount;

        emit ConfigurationUpdated();
    }

    function setPauses(uint8 _pauses) public {
        pauses = _pauses;

        emit ConfigurationUpdated();
    }

    function setLockingContractAndMinLockedPower(address _locking, uint256 _minLockedPower, uint256 _minLockedPowerPerQuoteTokenAmount, uint256 _minExtranetTokenBalanceToSkipLockedPower) public {
        locking = Locking(_locking);
        minLockedPower = _minLockedPower;
        minLockedPowerPerQuoteTokenAmount = _minLockedPowerPerQuoteTokenAmount;
        minExtranetTokenBalanceToSkipLockedPower = _minExtranetTokenBalanceToSkipLockedPower;
    ...
    function generateInvestRoundtripId() internal {
        investRoundtripId = keccak256(abi.encode(1, block.timestamp, block.number, address(this), block.chainid, roundtripNonce)); // solhint-disable-line not-rely-on-time
        roundtripNonce++;
    }

    function generateWithdrawRoundtripId() internal {
        withdrawRoundtripId = keccak256(abi.encode(2, block.timestamp, block.number, address(this), block.chainid, roundtripNonce)); // solhint-disable-line not-rely-on-time
        roundtripNonce++;
    }

    function _calculateQueueTotalAmount(QueueEntry[] memory queue) internal pure returns (uint256 total) {
        total = 0;
        for (uint i=0; i<queue.length; i++) {
    ...
    function checkIfHasMinLockedPower(uint256 quoteTokenAmount) internal view {
        if (address(locking) == address(0) || minLockedPower == 0) {
            return;
        }

        uint256 requiredPower = minLockedPower * quoteTokenAmount / minLockedPowerPerQuoteTokenAmount;

        require(
            locking.powerBy(msg.sender) >= requiredPower ||
            extranetToken.balanceOf(msg.sender) >= minExtranetTokenBalanceToSkipLockedPower,
            "MIN_LOCKED"
        );
    }
}
...
contract ExtranetToken is ERC20, Shutdown {
    using SafeERC20 for IERC20;
    ...
    constructor(string memory name, string memory symbol, uint8 __decimals, address _rewardToken, address _rewardCustodian) ERC20(name, symbol) {
    ...
    function onReward(uint256 amount) public onlyMinter {
        if (totalSupply() == 0) {
            return;
        }
    ...
    function pendingRewardAmount(address account) internal view returns (uint256) {
...
contract CollectTokens {
    ...
    function collectNative(address to) public {
...
abstract contract IMCeler is IMessageReceiverApp, AccessControl {
    ...
    mapping (bytes32 => bool) public seenRoundtripId;
    ...
    modifier onlyUnique(bytes memory message) {
        bytes32 roundtripId = abi.decode(message, (bytes32));

        if (seenRoundtripId[roundtripId]) {
            revert("UNIQUE");
        }

        seenRoundtripId[roundtripId] = true;
        ...
        returns (ExecutionStatus)
    {
        ...
        bytes32 roundtripId = abi.decode(incomingMessage, (bytes32));
    ...
    function tossIncomingMessageQueue() public payable {
        ...
        payable(msg.sender).transfer(msg.value - feePaid); // underflow verified by solidity 0.8
    }

    function sendMessage(bytes memory message) internal returns (uint256 fee) {
    ...
    function sendMessageWithTransfer(address token, uint256 amount, bytes memory message) internal returns (uint256 fee) {
        fee = IMessageBus(messageBus).calcFee(message);
        require(address(this).balance >= fee, "CELER_FEE");
    ...
    function executeMessageWithTransferForce(address token, uint256 amount, bytes calldata message) public payable {
        onMessageWithTransfer(message, token, amount);
    }

    function executeMessageForce(bytes calldata message) public payable {
        onMessage(message);
    }

    function clearIncomingMessageQueue() public {
        delete incomingMessageQueue;
    }
    ...
    function markRoundtripId(bytes32 roundtripId, bool isUsed) public {
        if (isUsed) {
            seenRoundtripId[roundtripId] = true;
            return;
        }

        delete seenRoundtripId[roundtripId];
    }
}
...
uint8 constant LOCK_PAUSED   = 1 << 0;
uint8 constant UNLOCK_PAUSED = 1 << 0;

contract Locking is Shutdown, CollectTokens {
    using SafeERC20 for IERC20;
    ...
    constructor(address _token, uint8 _minLockDuration, uint8 _maxLockDuration) {
        token = IERC20(_token);
    ...
    function lock(uint256 amount, uint8 lockDuration) public whenNotPaused(LOCK_PAUSED) {
        require(lockDuration >= minLockDuration && lockDuration <= maxLockDuration, "DURATION");
        require(amount > 0, "ZERO");

        uint32 unlockAfter = uint32(block.timestamp) + (uint32(lockDuration) * 7 days);
        uint256 power = amount * uint256(lockDuration);
    ...
    function unlockable(address account) public view returns (uint256) {
    ...
    function unlockAt(uint256 from, uint256 count) public whenNotPaused(UNLOCK_PAUSED) {
        UnlockData[] storage userUnlockData = unlockData[msg.sender];

        require(count > 0, "C");
        require(from + count <= userUnlockData.length, "C");
        ...
        if (count == userUnlockData.length) {
            delete unlockData[msg.sender];

        } else {
            UnlockData[] memory newUserUnlockData = new UnlockData[](userUnlockData.length);

            uint256 lastPos = 0;
            for (uint256 pos=0; pos<userUnlockData.length; pos++) {
                UnlockData memory entry = userUnlockData[pos];
            ...
            for (uint i=0; i<lastPos; i++) {
                unlockData[msg.sender].push(newUserUnlockData[i]);
            }
        }
    ...
    function unlockDataLength(address account) public view returns (uint256) {
    ...
    function setLockDuration(uint8 _minLockDuration, uint8 _maxLockDuration) public {
        minLockDuration = _minLockDuration;
        maxLockDuration = _maxLockDuration;
    }
}
