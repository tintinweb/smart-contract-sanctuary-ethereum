  ...
  BlackholePrevention
{
  ...
  ITaggrNftFactory internal _lazy721Factory;
  ITaggrNftFactory internal _preMint721Factory;
  address internal _nftDistributor;

  // Customer Account => Is Registered
  mapping (address => bool) internal _customers;
  ...
  function createCustomerAccount() public override {
    // Collect Membership Fee
    uint256 fee = _taggrSettings.getMembershipFee();
    if (fee > 0) {
      address feeToken = _taggrSettings.getMembershipFeeToken();
      _collectAssetToken(_msgSender(), feeToken, fee);
    }

    // Create Member
    _createCustomerAccount(_msgSender());
  }
  ...
  function setLazy721Factory(address nftFactory) external onlyRole(MANAGER_ROLE) {
    require(nftFactory != address(0), "Invalid address");
    _lazy721Factory = ITaggrNftFactory(nftFactory);
    emit Lazy721FactorySet(nftFactory);
  }

  function setPreMint721Factory(address nftFactory) external onlyRole(MANAGER_ROLE) {
    require(nftFactory != address(0), "Invalid address");
    _preMint721Factory = ITaggrNftFactory(nftFactory);
    emit PreMint721FactorySet(nftFactory);
  }

  function managerCreateCustomerAccount(address customerAccount) external onlyRole(MANAGER_ROLE) {
    // Create Member
    _createCustomerAccount(customerAccount);
  }
  ...
  function _validateCustomerProject(address customerAccount, bytes32 projectIdHash) internal view {
    // Validate Customer
    require(_customers[customerAccount], "Not a customer");
  ...
  function _createCustomerAccount(address customerAccount) internal {
    // Create Member
    _customers[customerAccount] = true;
  ...
  function _launchNewProject(
    ...
    if (contractType == 1) {
      contractAddress = _lazy721Factory.deploy(customerAccount, _nftDistributor, projectName, projectSymbol, maxSupply, royaltiesPct);
    } else if (contractType == 2) {
      contractAddress = _preMint721Factory.deploy(customerAccount, _nftDistributor, projectName, projectSymbol, maxSupply, royaltiesPct);
    }
...
interface ITaggr {
  ...
  event Lazy721FactorySet(address indexed nftFactory);
  event PreMint721FactorySet(address indexed nftFactory);
  ...
  function createCustomerAccount() external;
  function launchNewProject(
    string memory projectId,
    string memory projectName,
    string memory projectSymbol,
