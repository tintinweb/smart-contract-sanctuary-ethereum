...
contract Tok is IERC20, Ownable {
    ...
    mapping(address => uint256) private _tOwned; // tokens owned if excluded from rewards
    ...
    string private constant _name = "Tok";
    ...
    Fees public _taxRates = Fees({buyTotalFee: 80, 
                                  sellTotalFee: 80, 
                                  reflectionBuy: 10, 
                                  marketingBuy: 40, 
                                  liquidityBuy: 20, 
    ...
    FeeReceivers public _FeeReceivers =
        FeeReceivers({marketing: payable(DEAD), buyBack: payable(DEAD)});
    
    Limits public limits = Limits({buyLimit: 50, sellLimit: 100, maxWallet: 50});
    ...
    event TaxesUpdated(
                       uint16 buyTotalFee, uint16 sellTotalFee, 
                       uint16 reflectionBuy, uint16 marketingBuy, 
                       uint16 liquidityBuy, uint16 buyBackBuy,
                       uint16 reflectionSell, uint16 marketingSell,
                       uint16 liquiditySell, uint16 buyBackSell
                    );
    event SwapAndLiquify(
                         uint256 tokensSwapped, uint256 buyBackBalance, 
                         uint256 liquidityBalance, uint256 marketingBalance
                        );
    ...
    event FeeReceiversUpdated(address payable marketing, address payable buyBack);
    ...
    function _transfer(
        ...
        if (lpPairs[from]) {
            ...
            sell = true;
            if( !_isExcludedFromLimits[from] && amount > _tTotal / limits.sellLimit ){
                revert MaxTxExceeded("Max sell amount exceeded.");
            }
        } else {
            if(balanceOf(to) + amount > _tTotal / limits.maxWallet && !_isExcludedFromLimits[to]){
                revert MaxWalletExceeded("Max wallet amount exceeded.");
            }
     ...
     * @param contractTokenBalance amount of tokens to be swapped
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        if (fees.sellTotalFee == 0) {
            return;
        }

        uint256 amountToLiquify = (contractTokenBalance * fees.liquiditySell) /
            fees.sellTotalFee / 2;
        ...
        bool success;
        uint256 buyBackBalance = (amtBalance * fees.buyBackSell) /
            fees.sellTotalFee;
        uint256 marketingBalance = amtBalance * fees.marketingSell /
            fees.sellTotalFee;
        uint256 liquidityBalance = (amtBalance * fees.liquiditySell) /
            fees.sellTotalFee;


        if (liquidityBalance > 0) {

            try uniswapRouter.addLiquidityETH{value: liquidityBalance}(
                address(this),
                amountToLiquify,
                0,
                0,
        ...
        if (_FeeReceivers.marketing == address(0) || _FeeReceivers.marketing == DEAD
            || _FeeReceivers.buyBack == address(0) || _FeeReceivers.buyBack == DEAD
        ) revert ();
        ...
        if (success && fees.buyBackSell > 0) {
            (success, ) = _FeeReceivers.buyBack.call{
                value: buyBackBalance,
                gas: 35000
            }("");
        }
        emit SwapAndLiquify(contractTokenBalance, buyBackBalance, liquidityBalance, marketingBalance);
    }
    /**
    ...
    function finalizeTransfer(
        ...
        bool takeFee = true;
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || other == true) {
            takeFee = false;
        }
    ...
    function startTrading(uint256 _deadBlocks) external onlyOwner {
        if(tradingStarted) revert StartTradingError("Trading already enabled!");
        if(!_hasLiqBeenAdded) revert StartTradingError("Liquidity must be added.");
     ...
     */
    function sweepTokens(uint256 percentual, address token) external onlyOwner {
        IERC20 tokenContract = IERC20(token);
        if (_FeeReceivers.marketing == address(0) || _FeeReceivers.marketing == DEAD) revert ();
        tokenContract.transfer(
            _FeeReceivers.marketing,
            tokenContract.balanceOf(address(this)) * percentual / 100
            );
    }
    ...
    function setLimits(
        ...
        if( maxSell > 200 || maxSell < 100 || 
            maxBuy > 200 || maxBuy < 50 || 
            maxWallet > 200 || maxWallet < 50
        )
            revert InvalidLimitsSettings("Cannot exceed 2% or be lower than 0.5%");
     ...
     * @param buyBackBuy The buyback fee for buy transactions.
     ...
     * @param buyBackSell The buyback fee for sell transactions.
    ...
    function setFees(
        ...
        _taxRates.buyBackBuy = buyBackBuy;
        _taxRates.reflectionSell = reflectionSell;
        _taxRates.marketingSell = marketingSell;
        _taxRates.liquiditySell = liquiditySell;
        _taxRates.buyBackSell = buyBackSell;
        _taxRates.buyTotalFee = liquidityBuy + marketingBuy + buyBackBuy;
        _taxRates.sellTotalFee = liquiditySell + marketingSell + buyBackSell;
        if(
            _taxRates.buyTotalFee > 80 || _taxRates.sellTotalFee > 80
        )
            revert InvalidFeesSettings("Fees cannot exceed 8%.");

        emit TaxesUpdated(_taxRates.buyTotalFee, _taxRates.sellTotalFee,
                          reflectionBuy, liquidityBuy, 
                          buyBackBuy, marketingBuy, 
                          reflectionSell, liquiditySell, 
                          buyBackSell, marketingSell
                        );
    }
    /**
     * @dev Set the fee receivers.
     * @dev The marketing and buyBack addresses cannot be the zero address.
     * @notice This function can only be called by the contract owner.
     * @param marketing The address of the marketing wallet.
     * @param buyBack The address of the buyBack wallet.
    ...
    function setFeeReceivers(
        ...
        if( marketing == address(0) || buyBack == address(0) || marketing == DEAD || buyBack == DEAD )
            revert InvalidFeeReceivers("Fee receivers cannot be zero address.");
        _FeeReceivers.marketing = payable(marketing);
        _FeeReceivers.buyBack = payable(buyBack);
        emit FeeReceiversUpdated(marketing, buyBack);
    }
    ...
    function setContractSwapSettings(
        ...
        if( _swapAndLiquifyEnabled ) {
            ...
            if( swapThreshold < _tTotal / 100_000 || swapThreshold > _tTotal / 100 )
                revert InvalidContractSwapSettings( "SwapThreshold must be between 0.0001% and 1% of total supply.");       
        }
     ...
     */
    function getLimits() external view returns (uint256 maxBuy, uint256 maxSell, uint256 maxWallet) {
        return ( 
    ...
    /**
     * @dev Returns if the struct of fees.
     */
