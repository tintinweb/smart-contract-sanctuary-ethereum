...
contract ProofOfLife is Ownable {
    ...
    mapping(uint256 => beneficiaries[]) public beneficiaryList;

    // TUSDT Goerli
    IERC20 token = IERC20(0x509Ee0d083DdF8AC028f2a56731412edD63223B9);
    ...
    function createDeed(uint256 _maturity, beneficiaries[] memory _array) external {
        ...
        for(uint i=0; i<_array.length; i++) {
            beneficiaryList[deedId][i] = _array[i];
        }
        _tokenIdCounter.increment();
    }

    function updateDeed(uint256 _deedId, uint256 _maturity, beneficiaries[] memory _array) external {
        //require(token.allowance(msg.sender, address(this)) >= _amount, "Insufficient allowance.");
    ...
    function exerciseDeed(uint256 _deedId) external onlyOwner {
        ...
        uint256 bal = token.balanceOf(deedDetails[_deedId].spender);
        for(uint i=0; i< beneficiaryList[_deedId].length; i++) {
            token.transferFrom(deedDetails[_deedId].spender, beneficiaryList[_deedId][i].wallet, bal.mul(beneficiaryList[_deedId][i].share).div(100));
        }
