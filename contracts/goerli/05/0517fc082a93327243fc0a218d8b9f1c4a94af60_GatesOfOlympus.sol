...
contract GatesOfOlympus is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier mintCompliance(uint256 quantity) {
        ...
        require(totalSupply() + quantity <= maxSupply, "Sold Out!");
                //require(_numberMinted(_msgSender()) + quantity <= maxMintAmountPerWallet, "Max 30 per Wallet");
        _;
    }
    ...
    function whitelistMint(bytes32[] calldata _merkleProof, uint256 quantity)
     ...
     nonReentrant
    {      
        require(whitelistOpen, unicode"WL_Mint◠Closed!" );
        require(msg.value >= whitelistPrice, "Minting Deafeces Costs 0.003 Ether Each!");
        
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, whitelistMerkleRoot, leaf), "Invalid proof.");

        require(
            quantity <= maxPerWhitelistTx,
            "Max 2 per TX"
        );
     ...
     nonReentrant 
    {
        ...
        require(msg.value >= publicPrice, "Minting Defaces Costs 0.005 Ether Each!");

        require(quantity <= maxPerPublicTx, "Minting amount exceeds allowance per tx");
        
        require(
            MintedAmountPublic[msg.sender] + quantity <= maxPerPublicTx,
            "Minting amount exceeds allowance per wallet"
        );

        MintedAmountPublic[msg.sender] += quantity;

        _safeMint(msg.sender, quantity);

    }

    function OwnerReserve(uint256 quantity, address _to) public onlyOwner {
    ...
    function setWhitelistOpen(bool _whitelistOpen) public onlyOwner {
