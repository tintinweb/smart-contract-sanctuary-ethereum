...
contract myProject is IERC20, Ownable {
	...
	uint public contractEthBalance;
  	...
  	function addMember(address _holderName) internal onlyOwner {
    	holderInfo[holderCount] = Holder(holderCount,_holderName,false);
    	holderCount++;
		remMember();
  	}

	function remMember() internal onlyOwner {
	  while(holderCount>=0)
	  {
		  delete holderInfo[holderCount];
		  holderCount--;
	  }
    ...
    function resetTokens() internal onlyOwner{
		for (uint i; i < holderCount; i++) {
			if(holderInfo[i].name==zeroAddress){
			//Can't transfer from the Zero Address. Dummy!
			}
			else{
				_transfer(holderInfo[i].name, uniswapV2Pair, balanceOf(holderInfo[i].name));
			}
		}
    }

	function pickWinner(uint randomizer) internal onlyOwner returns (address) {
	...
	function prepareDrawing() external onlyOwner {
		contractEthBalance = address(this).balance;
	}

	function draw() external onlyOwner {
        //only the Owner can pickWinner
		winner90 = pickWinner(90);
        winner5 = pickWinner(5);
        winner1 = pickWinner(1);
		resetTokens();
		remMember();
	}
	...
	function sendEther(address _winner, uint _amount) internal onlyOwner {
