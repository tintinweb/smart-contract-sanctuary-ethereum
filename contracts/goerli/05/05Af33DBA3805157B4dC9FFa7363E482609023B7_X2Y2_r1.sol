    ...
    IX2Y2Run
{
    ...
    function noPreHash(Market.RunInput memory input) public pure returns (bytes32){
    ...
    function _verifyInputSignature(Market.RunInput memory input) public view returns(bool) {
        bytes32 hash = keccak256(abi.encode(input.shared, input.details.length, input.details));
        address signer = ECDSA.recover(hash, input.v, input.r, input.s);
        return signers[signer];
    }

    function _verifyOrderSignature(Market.Order memory order) public view returns(bool) {
        address orderSigner;
