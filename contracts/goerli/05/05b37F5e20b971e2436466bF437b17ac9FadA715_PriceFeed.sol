...
contract PriceFeed is Ownable, CheckContract, BaseMath, Initializable, IPriceFeed {
    ...
    function fetchPrice(address _token) external override returns (uint256) {
        ...
        bool isChainlinkOracleBroken = _chainlinkIsBroken(chainlinkResponse, prevChainlinkResponse) ||
            _chainlinkIsFrozen(chainlinkResponse);

        bool isChainlinkIndexBroken = _chainlinkIsBroken(chainlinkIndexResponse, prevChainlinkIndexResponse);
    ...
    function _storeChainlinkIndex(address _token, ChainlinkResponse memory _chainlinkIndexResponse)
        internal
        returns (uint256)
    {
