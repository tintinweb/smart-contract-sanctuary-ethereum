...
import 'slicer-so-hooks/contracts/extensions/Purchasable/SlicerPurchasable.sol';
import 'erc721a/contracts/ERC721A.sol';

contract GenerativeNounsPass is ERC721A, SlicerPurchasable {
    error MaxSupplyExceeded();

    uint256 public constant MaxSupply = 2000;
    address public constant RoyaltyReceiver = address(0x8a97426C1a720a45B8d69E974631f01f1168232B);
    uint256 public constant RoyaltyRate = 500;

    uint256 internal _slicerProductId;

    constructor() ERC721A('Generative Nouns minting pass', 'GNMP') {
        _productsModuleAddress = address(0xcA6b9D59849EC880e82210e9cb8237E1d0cAA69e);
        _slicerId = 8;
        _slicerProductId = 5;
    }

    function _baseURI() internal pure override returns (string memory) {
        return 'ipfs://QmbR85G1HqkV4rbqEGxv5uo7AvGDKynJeqRLyWWxjP9Jqq';
    }
    ...
    function contractURI() public pure returns (string memory) {
        return 'ipfs://QmZwoAgtnmsMSTnCgwefR6ULSc4GJ2N4fY6NDBN2PCT6XN';
    }
    ...
    function royaltyInfo(
        ...
        if (salePrice == 0 || ownerOf(tokenId) != address(0)) {
            receiver = address(0);
            royaltyAmount = 0;
        } else {
            receiver = RoyaltyReceiver == address(0) ? address(this) : RoyaltyReceiver;
            royaltyAmount = (salePrice * RoyaltyRate) / 10_000;
        }
    }
