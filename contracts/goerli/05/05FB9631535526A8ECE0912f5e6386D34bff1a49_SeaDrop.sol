...
abstract contract ReentrancyGuard {
    event helpMe(string data, uint256 num);

    uint256 private locked = 1;

    modifier nonReentrant() virtual {
        emit helpMe("entering critical section", locked);
        require(locked == 1, "REENTRANCY");
        locked = 2;
        emit helpMe("changed the value to 2" , locked);

        _;
        locked = 1;
        emit helpMe("changing it back to 1" , locked);
    }
}
