...
contract Box is ERC1155SupplyUpgradeable, ERC1155PresetMinterPauserUpgradeable {
    event NewBox(uint256 tokenId, uint256 num, uint256 startTimee);

    using StringsUpgradeable for uint256;

    string public _uri;

    function initialize(string memory __uri, address _playground)
        public
        initializer
    {
        super.initialize("");
        _setupRole(MINTER_ROLE, _playground);
        _uri = __uri;
    }

    function uri(uint256 tokenId) public view override returns (string memory) {
        return string.concat(_uri, string.concat(tokenId.toString(), ".json"));
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC1155PresetMinterPauserUpgradeable, ERC1155Upgradeable)
        returns (bool)
    {
        return super.supportsInterface(interfaceId);
    }
