...
// File: ERC20FixedTermStack_flat.sol
...
// File: contracts/ERC20FixedTermStack.sol
...
contract ERC20FixedTermStack is Ownable, ReentrancyGuard {
    ...
    function initialized()private {
        ...
        uint _id0 = poolInfos.length;
        PoolInfo storage p0 = poolInfos.push();
        p0.aprs.push(AprInfo(200, block.timestamp));
        p0.duration = 5 * 1 minutes;
        p0.id = _id0;
        p0.amount = 0;

        uint _id1 = poolInfos.length;
        PoolInfo storage p1 = poolInfos.push();
        p1.aprs.push(AprInfo(400, block.timestamp));
        p1.duration = 8 * 1 minutes;
        p1.id = _id1;
        p1.amount = 0;

        uint _id2 = poolInfos.length;
        PoolInfo storage p2 = poolInfos.push();
        p2.aprs.push(AprInfo(600, block.timestamp));
        p2.duration = 10 * 1 minutes;
        p2.id = _id2;
        p2.amount = 0;
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 reward) {
        ...
        for (uint i = user.offset;i< len-1;i++){
            ...
            if (!flag){
                ...
                reward = multiplier * pool.aprs[i].apr * user.amount / SECONDSPERYEAR / BASE;
                flag = true;
                continue ;
            }
            ...
            reward += tempNumber * pool.aprs[i].apr * user.amount / SECONDSPERYEAR / BASE;  
    ...
    function calculateReward(uint256 _pid, address _user) internal view returns (uint256 reward) {
        ...
        if (pool.aprs[len-1].time < user.lockStartTime){
            reward = termOfValidity * pool.aprs[len-1].apr * user.amount / SECONDSPERYEAR / BASE;
            return reward;
        }
        for (uint i = user.offset;i< len-1;i++){
            ...
            if (!flag){
                ...
                if (multiplier > termOfValidity){
                    reward = termOfValidity * pool.aprs[i].apr * user.amount / SECONDSPERYEAR / BASE;
                    return reward;
                    }
                reward = multiplier * pool.aprs[i].apr * user.amount / SECONDSPERYEAR / BASE;
                flag = true;
            ...
            if (tempNumber + multiplier > termOfValidity){
                reward += (termOfValidity - multiplier) * pool.aprs[i].apr * user.amount / SECONDSPERYEAR / BASE;
                return reward;
                }    
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0){
            ...
            if(user.lockStartTime + pool.duration < block.timestamp){
                ...
                if(pending > 0) {
                    IERC20(pdt).safeTransfer(msg.sender, pending);
                }
        ...
        IERC20(pdt).safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(uint256 _pid) external nonReentrant {
        ...
        uint256 pending;
        if(user.lockStartTime + pool.duration < block.timestamp){
            pending = calculateReward(_pid,msg.sender);   
        }

        uint256 amount = user.amount;
        IERC20(pdt).safeTransfer(msg.sender, amount + pending);
        user.amount = 0;
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant {
        ...
        uint256 amount = user.amount;
        IERC20(pdt).safeTransfer(msg.sender, amount);
        user.amount = 0;
        user.lockStartTime = 0;
        user.offset = 0;
        pool.amount -= amount;
    ...
    function getRewardBalance()public view returns(uint256 rewardBalance){
        rewardBalance = pdt.balanceOf(address(this)) - allAmount;
        require(rewardBalance > 0,"FixedTermStack: NO_MORE_REWARDS");
        return rewardBalance;
    }


    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)external onlyOwner {
        require(_tokenAddress != address(pdt), "Cannot be staked token");
    ...
    function refundReward()external onlyOwner {
        ...
        IERC20(pdt).safeTransfer(msg.sender, rewardBalance);
    }
