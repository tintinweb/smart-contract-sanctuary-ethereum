...
contract StakingPool is OwnableUpgradeable {
    ...
    function addStakerIfInexistent(address depositingStaker) private {
        for (uint256 i; i < stakersArray.length; i++) {
            address existingStaker = stakersArray[i];
            if (existingStaker == depositingStaker) return;
        }
        stakersArray.push(depositingStaker);
    }

    function stake(address _token, uint256 _amount) external {
        require(_amount > 0, "Insufficient amount");
        require(isAllowed[_token], "Token not allowed");
        addStakerIfInexistent(msg.sender);
