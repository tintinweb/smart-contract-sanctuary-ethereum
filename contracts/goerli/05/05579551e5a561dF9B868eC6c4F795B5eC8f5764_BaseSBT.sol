...
HasSecondarySaleFees
{
    ...
    mapping(uint256 => NFT) private nftInfos;
    mapping(bytes32 => uint256) private nftToSbts;
    ...
    function addNftInfo( uint256 _tokenId, uint256 _chainId, address _contractAddress, uint256 _nftId, string memory _url) public onlyOwner {
        ...
        // add SBT's certified NFT info 
        NFT storage _nft = nftInfos[_tokenId];
        _nft.chainId = _chainId;
        _nft.contractAddress = _contractAddress;
        _nft.nftId = _nftId;
        _nft.url = _url;

        // add NFT's SBT
        bytes32 nftKey = getNFTKeyHash(_chainId,_contractAddress, _nftId);
        nftToSbts[nftKey] = _tokenId;
    }

    function getNftInfo(uint256 _tokenId) public view returns (uint256,address,uint256,string memory)  {
        NFT storage _nft = nftInfos[_tokenId];
        return (_nft.chainId, _nft.contractAddress, _nft.nftId, _nft.url);
    }

    function getSBTbyNft(uint256 _chainId, address _contractAddress, uint256 _nftId) public view returns (uint256)  {
        bytes32 nftKey = getNFTKeyHash(_chainId,_contractAddress, _nftId);
        return nftToSbts[nftKey];
    }
