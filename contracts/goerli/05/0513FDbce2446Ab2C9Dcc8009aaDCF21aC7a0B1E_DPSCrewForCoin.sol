...
contract DPSCrewForCoin is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function borrowDPS(uint256 _tokenId) external nonReentrant {
        ...
        doubloons.transferFrom(msg.sender, asset.lender, asset.doubloonsPerEpoch * asset.epochs);

        lDPS.mint(msg.sender, asset.targetId);

        emit AssetBorrowed(address(dps), msg.sender, asset.lender, asset.doubloonsPerEpoch * asset.epochs);
    }

    function redeemDPS(uint256 _tokenId) external nonReentrant {
        Asset storage asset = assets[dps][_tokenId];
        if (asset.borrower == address(0)) revert NotListed();
        if (asset.endTime >= block.timestamp) revert NotExpired();
        bool burned;

        if (lDPS.ownerOf(_tokenId) == address(this) || lDPS.ownerOf(_tokenId) == asset.borrower) {
            burned = true;
            lDPS.burn(_tokenId);
        }
        ...
        emit AssetRedeemed(address(dps), msg.sender, _tokenId, burned);
    }
    ...
    function borrowFlagship(uint256 _tokenId) external nonReentrant {
        ...
        emit AssetBorrowed(address(lFlagship), msg.sender, asset.lender, asset.doubloonsPerEpoch * asset.epochs);
    }

    function redeemFlagship(uint256 _tokenId) external nonReentrant {
        Asset storage asset = assets[flagship][_tokenId];
        if (asset.borrower == address(0)) revert NotListed();
        if (asset.endTime >= block.timestamp) revert NotExpired();
        bool burned;

        // should never skip this if but just in case we transfer the lent flagship out of the contract
        if (lFlagship.ownerOf(_tokenId) == address(this) || lFlagship.ownerOf(_tokenId) == asset.borrower) {
            burned = true;
        ...
        emit AssetRedeemed(address(flagship), msg.sender, _tokenId, burned);
    }
    ...
    function afterTransferHookDPS(
        ...
        if (asset.endTime < block.timestamp && _to == asset.borrower) IERC721Full(msg.sender).burn(_tokenId);
    }
    ...
    event AssetRedeemed(address indexed _target, address indexed _lender, uint256 _tokenId, bool _burned);
