...
contract LendLocal is Ownable, ReentrancyGuard {
    ...
    string public constant name = 'LendLocal';
    ...
    mapping(address => mapping(uint32 => uint32)) userLoanIds;
    mapping(uint32 => LoanRequest) public loans;
    ...
    function permit(address owner, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public {
        require(deadline >= block.timestamp, 'LendLocal:: EXPIRED');
        bytes32 digest = keccak256(
            abi.encodePacked(
                '\x19\x01',
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(PERMIT_TYPEHASH, owner, value, nonces[owner]++, deadline))
            )
        );
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'LendLocal: INVALID_SIGNATURE');
    }
    ...
    function loanTokenWithPermit(
        ...
        permit(msg.sender, _value, _deadline, _v, _r, _s);
        nonces[msg.sender]++;
        ...
        uint32 tempCount = userLoansCount[msg.sender];
        loans[totalLoanCount] = newLoan;
        userLoanIds[msg.sender][tempCount] = totalLoanCount;
    ...
    function loanEther(uint256 _kiosk) external payable nonReentrant {
    ...
    function setFiatPaidUser(address _user, uint32 _id)
        ...
        onlyOwnerOrOperator
    {
        uint32 loanId = userLoanIds[_user][_id];
