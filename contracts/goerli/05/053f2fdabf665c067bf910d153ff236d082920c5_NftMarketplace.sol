...
error NftMarketplace__Already_listed(address nftAddress, uint256 tokenId, address owner);
...
contract NftMarketplace is ReentrancyGuard {
    ...
    event ItemUpdated(address indexed nftAddress, uint256 indexed tokenId, uint256 price, address indexed seller);
    ...
    function listItem(
        ...
        uint256 tokenId,
        uint256 price
    ) external notAlreadyListed(nftAddress, tokenId, msg.sender) isOwner(nftAddress, tokenId, msg.sender) {
        if (price <= 0) {
    ...
    function getProceeds(address buyerAddress) public view returns (uint256 proceeds) {
