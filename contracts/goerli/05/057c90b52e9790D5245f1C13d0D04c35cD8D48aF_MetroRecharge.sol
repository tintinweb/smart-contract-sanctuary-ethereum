...
interface IMetropolitanNFT{
    function safeMint(address addressTo, uint256 idNFT) external;
    function vendidoNFT() external;
}
    ...
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        cantNFTVendidos = 0;
        limiteCantidadTkn = 100;
        tokenPEN = IERC20(0x020bCBEEB5CB98491911F3543880CBfa69Aea204);
        tokenNFT = IMetropolitanNFT(0x1D92c07F92A6B8Dd50D709eB39b87a4D0433cF1c);
        swapperAddress = 0xaF8C9098Dfd8281e0DF3670BBBc8378857c632CE;
        swapper = IMetropolitanSwap(swapperAddress);
    }

    function setMiPrimerTokenAdd(
        address _miPrimerToken
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        miPrimerToken = IERC20Upgradeable(_miPrimerToken);
    }

    function setGnosisSafeWalletAdd(
        address _gnosisSafeWallet
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        gnosisSafeWallet = _gnosisSafeWallet;
    }

    function setLimitePremioNFT(uint256 _limite) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_limite > 0 , "Limite no puede ser menor que cero");
        limiteCantidadTkn = _limite;
    }
    ...
    function comprarMtptkPorPEN(uint amountPEN) public {
        ...
        if (amountPEN >= limiteCantidadTkn*10**2) {
            //Obtener NFT por logro...
            uint256 idNftAleatorio = _getRandomNftId();
            // if (tokenNFT.vendidoNFT()[idNftAleatorio] != address(0)) {
            //     //Ya fue minteado a otro address...
            // } else {
                tokenNFT.safeMint(msg.sender, idNftAleatorio);
            // }
        }
    }
