...
contract NFTStaking is Ownable, Pausable, ReentrancyGuard{
    ...
    function rewardForTokenInUSD(address user, uint256 tokenID) public view returns(uint256) {
        UserInfo storage info = userInfos[user];
        uint256 stakedPeriod = (block.number-info.staked[tokenID])/info.rewardPeriod[tokenID];
    ...
    function gettingRemainingTime(address user, uint256 tokenID) public view returns(uint256) {
        UserInfo storage info = userInfos[user];
        uint256 restDays = info.rewardPeriod[tokenID] - ((block.number-info.staked[tokenID])%info.rewardPeriod[tokenID]);
        return restDays;
    }
