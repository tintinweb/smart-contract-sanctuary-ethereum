...
interface ITrinity {
  function claimTrinity(address to, uint16 count) external;
}

contract Unity is ERC721A, ERC2981, Ownable {

  uint16 public collectionSize = 1000;
  bytes32 public merkleRoot;

  string public baseURI;
  address public proxyRegistryAddress;
  address public trinityAddress;
  ...
  /// @notice allow to burn 3 unity to gain 1 trinity
  /// @param ids list of unity id to burn
  /// @dev 800 items maximum per transaction
  function burnForTrinity(uint256[] calldata ids) external {
    uint count = ids.length / 3;
    require(count > 0, "3 tokens required");
    for (uint i = 0; i<count*3; i++) {
      require(ownerOf(ids[i]) == msg.sender, "Not all your tokens");
      _burn(ids[i]);
    }
    ITrinity(trinityAddress).claimTrinity(msg.sender, uint16(count));
  }
  ...
  function claimReward(uint16 _maxCount, bytes32[] memory _proof) external payable {
    require(isWhitelisted(msg.sender, _proof), "Address not whitelisted");

    // TODO : changer pour compter compbien peut être mint au maximum
    _batchMint(msg.sender, _maxCount);
  }
