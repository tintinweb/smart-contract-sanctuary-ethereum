...
contract Sexg is ERC2981, ERC721AQueryable, Ownable {
    ...
    uint256 public _wlPrice = 0.0001 ether;
    uint256 public _publicPrice = 0.0002 ether;
    ...
    bool public _rootabi = true;

    constructor(string memory metadataURI) ERC721A("wemadfl", "wemadfl") {
    ...
    function mintWL(uint32 amount,bytes32[] calldata merkleProof)
    external payable isValidMerkleProof(merkleProof, _saleMerkleRoot, _rootabi) {
    ...
    function identityVerification(bytes32[] calldata merkleProof) public view returns (bool) {
        return MerkleProof.verify(merkleProof, _saleMerkleRoot, keccak256(abi.encodePacked(msg.sender)),_rootabi);
    }
