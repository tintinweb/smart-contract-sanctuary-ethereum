...
contract swap is Ownable {
    ...
    IUniswapV3Pool public customPool;

    event SwapPerformed(uint256 In, uint256 Out);
    event testestimateAmountout(uint256 amountout);
    ...
    constructor(
        ...
        address _pool = IUniswapV3Factory(uniswapV3factoryAddress).getPool(
            linkTokenAddress,
            wethAddress,
            3000
        );
        ...
        customPool = IUniswapV3Pool(_pool);
    }
    ...
    function getTwap() public view returns (int24) {
        uint32 _twapDuration = 20;
        uint32[] memory secondsAgo = new uint32[](2);
        secondsAgo[0] = _twapDuration;
        secondsAgo[1] = 0;

        (int56[] memory tickCumulatives, ) = customPool.observe(secondsAgo);
        return int24((tickCumulatives[1] - tickCumulatives[0]) / int56(20));
    }

    function swapMaticForLink() external {
        uint256 amountIn = contractLinkBalance();
        // notice: swap returns Wrapped Matic, which is unwrapped in withdraw function call
        uint256 amountOut = _swapExactInputSingle(amountIn);
        emit SwapPerformed(amountIn, amountOut);
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        (int24 twapTick, ) = OracleLibrary.consult(
            address(customPool),
            uint32(30)
        );
        uint256 estimatedAmountOut = OracleLibrary.getQuoteAtTick(
            twapTick,
            uint128(amountIn),
            address(LinkToken),
        ...
        // TEST
        emit testestimateAmountout(estimatedAmountOut);
        // TEST
        // perform the swap
    ...
    function withdrawLinkTokens() external onlyOwner {
        LinkToken.transfer(owner(), contractLinkBalance());
    }

    function withdrawBalance() external onlyOwner {
        uint256 amount = contractWETHBalance();
        WETHcontract.withdraw(amount);
        (bool s, ) = payable(owner()).call{value: address(this).balance}("");
        require(s, "tx failed");
    }

    // required for WETH unwrapping
