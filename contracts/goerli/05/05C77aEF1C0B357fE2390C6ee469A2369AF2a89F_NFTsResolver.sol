...
abstract contract OpenGetter is IOpenGetter, OpenChecker {
    ...
    {
        collectionInfos = _getCollectionInfos(collection, new bytes4[](0), msg.sender);
    }
    ...
    function getCollectionsInfos(
        ...
        bytes4[] memory interfaceIds,
        address account
    ) public view override(IOpenGetter) returns (CollectionInfos[] memory collectionsInfos) {
        collectionsInfos = new CollectionInfos[](collections.length);
        for (uint256 i = 0; i < collections.length; i++) {
            collectionsInfos[i] = _getCollectionInfos(collections[i], interfaceIds, account);
        }
    }
    ...
    function _getCollectionInfos(
        ...
        bytes4[] memory interfaceIds,
        address account
    ) private view returns (CollectionInfos memory collectionInfos) {
...
abstract contract OpenResolver is IOpenResolver, OpenRegistry, OpenGetter {
    ...
    {
        collectionsInfos = getResolverCollectionsInfos(account, new bytes4[](0));
    }

    function getResolverCollectionsInfos(address account, bytes4[] memory interfaceIds)
        public
        view
        override(IOpenResolver)
        returns (CollectionInfos[] memory collectionsInfos)
    {
        CollectionInfos[] memory collectionsInfosAll = getCollectionsInfos(_addresses, interfaceIds, account);
