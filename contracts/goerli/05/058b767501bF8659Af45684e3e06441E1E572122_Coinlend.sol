...
/* Errors */
error Coinlend__InsufficientFunds();
error Coinlend__InsufficientCollateral();
error Coinlend__InsufficientTokenWithdraw();

/**
 * @title The Conlend DeFi
 * @author Jan Pumpe
 * @notice This contract is for creating of decentralized loans
...
contract Coinlend {
    ...
    struct Transfer {
        ...
        address token; // Address of the token they receive
        uint256 tokenAmount; // Amount to lend
        uint8 transferType; // 0 -> Deposit, 1 -> Withdraw
        uint256 timestamp; // When transfer was created
    }
    ...
    function withdrawToken(address _token, uint256 _amount) public {
        ...
        if (tokens[_token][msg.sender] < _amount) {
            revert Coinlend__InsufficientTokenWithdraw();
        }
    ...
    function createLoanLend(
        address tokenLend,
        uint256 tokenLendAmount,
        address tokenCollateral,
        uint256 tokenCollateralAmount,
        uint256 interestRate,
        uint256 loanDuration
    ) public {
        if (balanceOf(tokenLend, msg.sender) < tokenLendAmount) {
            revert Coinlend__InsufficientFunds();
        }
        ...
        tokens[tokenLend][msg.sender] -= tokenLendAmount;
    ...
    function createLoanBorrow(
        ...
        if (balanceOf(tokenCollateral, msg.sender) < tokenCollateralAmount) {
            revert Coinlend__InsufficientFunds();
        }
        ...
        tokens[tokenCollateral][msg.sender] -= tokenCollateralAmount;
    ...
    function cancelLoan(uint256 _id) public {
        ...
        require(address(_loan.user) == msg.sender, "Addresses not matching");

        // Ensue the loan is in state open
        require(_loan.loanState == LoanState.OPEN, "Only open loans can be cancelled");

        // Update loan state
        _loan.loanState = LoanState.CANCELED;
    ...
    function takeLoan(uint256 _id) public {
        // Fetch loan
        Loan storage _loan = loans[_id];

        // Loan must exist
        require(_loan.id == _id);
        require(_loan.loanState == LoanState.OPEN, "Only open loans can be taken");

        _loan.loanState = LoanState.ACTIVE;

        // If user takes a lending offer -> check if he has enough collateral
        if (_loan.borrower == address(0)) {
            require(
                balanceOf(_loan.tokenCollateral, msg.sender) >= _loan.tokenCollateralAmount,
        ...
        if (_loan.lender == address(0)) {
            require(
                balanceOf(_loan.tokenLending, msg.sender) >= _loan.tokenLendingAmount,
                "Not enough lending funds"
            );
        }
    ...
    function payBackLoan(uint256 _id) public {
        // Fetch loan
        Loan storage _loan = loans[_id];

        // Ensure the caller of the function is the owner of the loan
        require(address(_loan.borrower) == msg.sender, "Addresses not matching the borrower");

        // Loan must exist
        require(_loan.id == _id);

        require(_loan.loanState == LoanState.ACTIVE, "Only active loans can be paid back");

        _loan.loanState = LoanState.COMPLETE;

        // Calculate interest

        console.log("Loan creation timestamp: ", _loan.timestamp);
        console.log("Current timestamp: ", block.timestamp);
        console.log("Currency: ", _loan.tokenLending);
        console.log("amount: ", _loan.tokenLendingAmount);
        console.log("Interest rate: ", _loan.interestRate);

        uint256 interestAmount = (((_loan.tokenLendingAmount * _loan.interestRate) *
            (block.timestamp - _loan.timestamp)) /
            365 /
            24 /
            60 /
            60 /
        ...
        uint256 fee = (interestAmount * feePercent) / 10000;

        console.log("fee: ", fee);

        tokens[_loan.tokenLending][feeAccount] += fee;

        //100*500*1/365/24/60/60/10000*sec
