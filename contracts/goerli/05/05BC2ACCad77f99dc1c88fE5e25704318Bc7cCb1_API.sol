...
contract API {
	...
	function InitializeAPI(address _Tuktu, address _XProgram) external {
		...
		if (!Initialized) {
			Tuktus.InitializeTuktu(XPrograms, Matrixes);
			XPrograms.InitializeXProgram(Accounts, Balances, Tuktus);
		}
	}
...
interface ITuktu {
	...
	function InitializeTuktu(IXProgram _XProgram, IMatrix _Matrix) external;
}
...
interface IXProgram {
	...
	function InitializeXProgram(IAccount _Account, IBalance _Balance, ITuktu _Tuktu) external;
