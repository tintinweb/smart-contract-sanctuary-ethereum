...
contract Pool is Ownable, ReentrancyGuard, Pausable, Core, IPool {
    ...
    /// @dev Modifier ensuring that certain function can only be called by owner or rewarder
    modifier onlyOwnerOrRewarder() {
        require(owner() == msg.sender || rewarder == msg.sender, "FORBIDDEN");
        _;
    }
    ...
    /**
     * @notice Changes the contract rewarder. Can only be set by the contract owner.
     * @param rewarder_ new contract rewarder address
     */
    function setRewarder(address rewarder_) external onlyOwner {
        require(rewarder_ != address(0), "ZERO");
        emit DevUpdated(rewarder, rewarder_);
        rewarder = rewarder_;
    }
    ...
    function noHaircutSwap(
        ...
        address to,
        uint256 deadline
    ) external override ensure(deadline) nonReentrant whenNotPaused onlyOwnerOrRewarder returns (uint256 actualToAmount) {
        require(fromToken != address(0), "ZERO");
        require(toToken != address(0), "ZERO");
        require(fromToken != toToken, "SAME_ADDRESS");
        require(fromAmount > 0, "ZERO_FROM_AMOUNT");
