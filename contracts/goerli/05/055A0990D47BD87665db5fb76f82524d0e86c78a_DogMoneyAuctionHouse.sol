    ...
    UniversalPaymentReceiver
{
    ...
    function getNextAuction(
        bool canRestartCycle
    ) public view returns (IDogMoneyAuctionHouse.Auction memory) {
        if (nextAuctionHead >= 0 && nextAuctionHead < numNextAuctions) {
    ...
    function _reactivateAuction(uint256 auctionId) internal {
        ...
        uint256 endTime = startTime + auction.auctionPack.duration;

        auction.startTime = startTime;
        auction.endTime = endTime;
        auction.lastAttempt = startTime;
    ...
    function _createAuction(AuctionPack memory auctionPack) internal {
        ...
        uint256 startTime = block.timestamp;
        uint256 endTime = startTime + auctionPack.duration;
     ...
     */
    function setNextAuctions(
        AuctionPack[] memory auctionPacks
    ) external onlyOwner {
        nextAuctionHead = 0;
