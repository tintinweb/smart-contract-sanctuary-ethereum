...
contract Particulatim is AdminControl, ICreatorExtensionTokenURI{
    ...
    function addChunk(uint256 tokenId, string calldata chunk) public adminRequired {
        require(minted[tokenId] == false, "token already minted");
        tokenData[tokenId].push(chunk);
    }

    function mint() public adminRequired {
      uint256 tokenId = IERC721CreatorCore(_creator).mintExtension(msg.sender);
      minted[tokenId] = true;
    }
    ...
    function tokenURI(address creator, uint256 tokenId) external view override returns (string memory) {
        ...
        require(minted[tokenId], "Not yet minted");
        string memory output = "";

        for (uint8 i = 0; i < tokenData[tokenId].length; i++) {
            output = string.concat(output, tokenData[tokenId][i]);
        }
