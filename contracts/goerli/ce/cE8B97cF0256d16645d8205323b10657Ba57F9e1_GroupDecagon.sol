...
contract GroupDecagon is AccessControl, ReentrancyGuard {
  ...
  ) public view returns (bytes32 colourHash) {
    colourHash = keccak256(abi.encode(_address, block.timestamp));
  }
  ...
  function _mint(
    ...
    bytes32 colourHash = generateColourHash(_friends);
...
error ExceedsMaxFriends();
error GroupHashAlreadyUsed();