    ...
    ERC721BurnableUpgradeable
{
    ...
    function addRewardsSet(
        ...
        RewardTypesInfo memory rewardSet;
        rewardSet.name = setName;
        rewardSet.rangeMax = rangeMax;
        rewardSet.rangeMin = rangeMin;
        rewardSet.distributedRewardAmount = 0;
        rewardTypes.push(rewardSet);
    }
    ...
    function updateRewardsSet(
        ...
        RewardTypesInfo storage rewardSet = rewardTypes[rewardsSetId];
        rewardSet.name = setName;
        rewardSet.rangeMax = rangeMax;
        rewardSet.rangeMin = rangeMin;
    }
    ...
    function revealRewards(
        ...
        for (uint256 i = 0; i < amount; i++) {
            range = getWinningRewardsSet(i);
            lastRangeUsedId =
                rewardTypes[range].distributedRewardAmount +
                rewardTypes[range].rangeMin;
            rewardTypes[range].distributedRewardAmount += 1;
    ...
    function setMaximumSupplyLimit(uint256 new_supplyLimit)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _supplyLimit = new_supplyLimit;
    }
    ...
    function getRandomValue(uint256 amount) public view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
    ...
    function getWinningRewardsSet(uint256 seed) public view returns (uint256) {
        uint256 index;
        uint256 randomID = getRandomValue(seed) % _supplyLimit;
