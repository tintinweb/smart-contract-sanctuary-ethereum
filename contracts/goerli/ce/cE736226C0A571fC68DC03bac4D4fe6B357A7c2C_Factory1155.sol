/** @notice The Factory contracts (here, Factory1155) have the responceblity for deploying 1155 own contracts which can be used for  Minting NFTs , transferring , burning NFTs from the wallets
...
import "./RAJNFTUser1155Token.sol";

contract Factory1155 {

    //@notice Deployed the event is emited at the time of  factory contract invoke. 
    ...
    //@param _salt 
    //@param name name of the ERC1155 token
    //@param symbol symbol of the ERC1155 token
    //@param tokenURIPrefix base URI of the ERC1155 token
    //@return address of the deployed contract. 
    //@dev see{RAJNFTUser1155Token}.
    ...
    function deploy(
        ...
        addr = address(
            new RAJNFTUser1155Token{salt: _salt}(name, symbol, tokenURIPrefix)
        );
        RAJNFTUser1155Token token = RAJNFTUser1155Token(address(addr));
        token.transferOwnership(msg.sender);
        emit Deployed(msg.sender, addr);
    }
}

/** @notice The RAJNFTUser1155Token own contracts have the responceblity for Minting NFTs , transferring , burning NFTs from the wallets
    assign operators for NFTs, returning Ipfs target path(tokenURIs) of given tokenId,..etc
*/
...
contract RAJNFTUser1155Token is
    Context,
    ERC1155Burnable,
    ERC1155Supply,
    AccessControl
