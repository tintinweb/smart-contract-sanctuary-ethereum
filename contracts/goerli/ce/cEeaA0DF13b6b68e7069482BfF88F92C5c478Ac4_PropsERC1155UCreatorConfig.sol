    ...
    AccessControlEnumerableUpgradeable
{
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function setIPFSAt(uint256 _index)
        external
    {
        require(hasMinRole(CONTRACT_ADMIN_ROLE));
        ipfsAt = _index;
    ...
    function getIPFSAt() public view returns (uint256) {
        return ipfsAt;
    }
    ...
    function getTokenURI(uint256 _tokenId) external view returns (string memory) {
        if(getIPFSAt() >= _tokenId && bytes(baseURI_).length > 0 ){
            return string(abi.encodePacked(baseURI_, _tokenId.toString(), ".json"));
        }
        return getToken(_tokenId).baseURI;
    }

    function getTokenCheck(IPropsCreatorConfig.ERC1155Token memory _token) external view returns (string memory) {
    ...
    function getPrimaryCheck(IPropsCreatorConfig.Split memory _primarySplit) external view returns (string memory) {
     ...
     function getRoyaltyCheck(IPropsCreatorConfig.Split memory _royaltySplit) external view returns (string memory) {
  ...
  function revertOnAllocationCheckFailure(IPropsCreatorConfig.AllocationCheck calldata check) external view {
   ...
   function revertOnUnauthorizedSignature(IPropsCreatorConfig.SignatureRequest calldata _inputs ) external view {
     if(isSanctioned(_msgSender())) revert Sanctioned();
     if(ECDSAUpgradeable.recover(keccak256(abi.encodePacked(_inputs.tokenCheck, _inputs.primaryCheck, _inputs.royaltyCheck, _inputs.configuration, _inputs.quantity, _inputs.price, _inputs.nonce)).toEthSignedMessageHash(), _inputs.signature) !=  signatureVerifier) revert InvalidSignature();
   }
