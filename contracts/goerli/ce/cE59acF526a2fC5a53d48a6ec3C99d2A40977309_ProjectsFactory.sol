...
contract ProjectsFactory {
    ...
    constructor() {}
    function createNewProject(string memory projectName, uint256 projectBudget) payable public {
        require(availableProjects[projectName] == address(0), 'This Project Already Exists');
        ProjectFunding project = new ProjectFunding(projectName, projectBudget, msg.sender);
...
contract ProjectFunding is Ownable {
    ...
    constructor(string memory _projectName, uint256 _budget, address _owner) {
        projectName = _projectName;
        budget = _budget;
        transferOwnership(_owner);
        rvlToken = RVLToken(0x6B0809b99a8035C483c55ccFfCe781FC12ff1bBF);
    }

    modifier budgetNotReached {
        require(remainingBalance() < 0, 'Action Forbidden, Project Budget Already Reached !');
        _;
    }

    modifier acceptsFunding {
        require(remainingBalance() <= 0, 'Action Forbidden, Amount Exceeds Remaining Budget !');
        _;
    }

    function fundProject(uint256 amount) payable public {
        // implement transfer from token
        rvlToken.approve(msg.sender, amount);
        rvlToken.transferFrom(msg.sender, address(this), amount);
        investors[msg.sender] += amount;
    }

    function testTransfer() public view returns(uint256) {
        uint256 allowance = rvlToken.allowance(address(this), msg.sender);
        return allowance;
    }
    ...
    function remainingBalance() public view returns (uint256 remainingAmount) {
        remainingAmount = getBudget() - address(this).balance;
