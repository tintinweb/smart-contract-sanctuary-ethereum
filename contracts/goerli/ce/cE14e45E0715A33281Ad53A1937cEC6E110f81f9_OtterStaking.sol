...
contract OtterStaking is ReentrancyGuard, Ownable {
  ...
  // UNSTAKE FUNCTIONS
  function unstake(uint256[] memory _tokenIds, uint64 expireTime, bytes memory sig) external {
    bytes32 digest = keccak256(abi.encodePacked(msg.sender, expireTime));
  ...
  //  SNAPSHOT
  function snapshot(uint256[] memory _tokenIds, uint64 expireTime, bytes memory sig) external returns(uint256) {
    bytes32 digest = keccak256(abi.encodePacked(msg.sender, expireTime));
    require(isAuthorized(sig,digest),"CONTRACT_NOT_ALLOWED");
    uint256 totalRewards = 0;
