...
library Orders {
    ...
    event WithdrawEnqueuedWithData(
        uint256 indexed orderId,
        uint8 unwrapAndFailure,
        uint32 gasLimit,
        uint112 liquidity,
    ...
    struct Data {
        ...
        mapping(uint256 => uint8) unwrapAndFailure;
    }
    ...
    function getOrderStatus(
        ...
        uint256 orderId,
        uint32 validAfterTimestamp
    ) internal view returns (OrderStatus orderStatus) {
    ...
    function getDepositOrder(Order calldata order)
        public
        pure
        returns (
            DepositOrder memory returnOrder,
            uint256 amountLimit0,
            uint256 amountLimit1
        )
    {
        require(order.orderType == DEPOSIT_TYPE, 'OS32');
        returnOrder.pairId = order.pairId;
        returnOrder.share0 = order.value0;
        returnOrder.share1 = order.value1;
        returnOrder.minSwapPrice = float32ToUint(order.minSwapPrice);
        returnOrder.maxSwapPrice = float32ToUint(order.maxSwapPrice);
        returnOrder.unwrap = getUnwrap(order.unwrapAndFailure);
        returnOrder.swap = order.swap;
        returnOrder.to = order.to;
        returnOrder.gasPrice = order.gasPrice;
        returnOrder.gasLimit = order.gasLimit;
        returnOrder.validAfterTimestamp = order.validAfterTimestamp;
        returnOrder.priceAccumulator = order.priceAccumulator;
        returnOrder.timestamp = order.timestamp;

        amountLimit0 = order.amountLimit0;
        amountLimit1 = order.amountLimit1;
    }

    function getWithdrawOrder(Order calldata order) public pure returns (WithdrawOrder memory returnOrder) {
        require(order.orderType == WITHDRAW_TYPE, 'OS32');
        returnOrder.pairId = order.pairId;
        returnOrder.liquidity = order.liquidity;
        returnOrder.amount0Min = order.value0;
        returnOrder.amount1Min = order.value1;
        returnOrder.unwrap = getUnwrap(order.unwrapAndFailure);
        returnOrder.to = order.to;
        returnOrder.gasPrice = order.gasPrice;
        returnOrder.gasLimit = order.gasLimit;
    ...
    function getSellOrder(Order calldata order)
        public
        pure
        returns (SellOrder memory returnOrder, uint256 amountLimit)
    {
        require(order.orderType == SELL_TYPE || order.orderType == SELL_INVERTED_TYPE, 'OS32');
        returnOrder.pairId = order.pairId;
        returnOrder.inverse = order.orderType == SELL_INVERTED_TYPE;
        returnOrder.shareIn = order.value0;
        returnOrder.amountOutMin = order.value1;
        returnOrder.unwrap = getUnwrap(order.unwrapAndFailure);
        returnOrder.to = order.to;
        returnOrder.gasPrice = order.gasPrice;
        returnOrder.gasLimit = order.gasLimit;
        ...
        amountLimit = order.amountLimit0;
    }

    function getBuyOrder(Order calldata order) public pure returns (BuyOrder memory returnOrder, uint256 amountLimit) {
        require(order.orderType == BUY_TYPE || order.orderType == BUY_INVERTED_TYPE, 'OS32');
        returnOrder.pairId = order.pairId;
        returnOrder.inverse = order.orderType == BUY_INVERTED_TYPE;
        returnOrder.shareInMax = order.value0;
        returnOrder.amountOut = order.value1;
        returnOrder.unwrap = getUnwrap(order.unwrapAndFailure);
        returnOrder.to = order.to;
        returnOrder.gasPrice = order.gasPrice;
        returnOrder.gasLimit = order.gasLimit;
    ...
    function enqueueDepositOrder(
        ...
        data.orderQueue[newestOrderId] = getOrderDigest(order);

        emit DepositEnqueued(newestOrderId, validAfterTimestamp, gasPrice);
        emit DepositEnqueuedWithData(
            newestOrderId,
            order.unwrapAndFailure,
            order.timestamp,
            order.gasLimit,
            order.value0,
            order.value1,
    ...
    function isRefundFailed(Data storage data, uint256 orderId) internal view returns (bool) {
        uint8 unwrapAndFailure = data.unwrapAndFailure[orderId];
        return unwrapAndFailure == UNWRAP_FAILED || unwrapAndFailure == KEEP_FAILED;
    }

    function markRefundFailed(Data storage data) internal {
        uint256 lastProcessedOrderId = data.lastProcessedOrderId;
        data.unwrapAndFailure[lastProcessedOrderId] = data.unwrapAndFailure[lastProcessedOrderId] == UNWRAP_NOT_FAILED
            ? UNWRAP_FAILED
            : KEEP_FAILED;
    }
    ...
    function dequeueDepositOrder(Data storage data, Order calldata order)
        external
        returns (
            DepositOrder memory returnOrder,
            uint256 amountLimit0,
    ...
    {
        ++data.lastProcessedOrderId;
        require(order.orderId == data.lastProcessedOrderId, 'OS5C');
        (returnOrder, amountLimit0, amountLimit1) = getDepositOrder(order);
    }

    function dequeueWithdrawOrder(Data storage data, Order calldata order)
        external
        returns (WithdrawOrder memory returnOrder)
    {
        ++data.lastProcessedOrderId;
        require(order.orderId == data.lastProcessedOrderId, 'OS5C');
        returnOrder = getWithdrawOrder(order);
    }

    function dequeueSellOrder(Data storage data, Order calldata order)
        external
        returns (SellOrder memory returnOrder, uint256 amountLimit)
    {
        ++data.lastProcessedOrderId;
        require(order.orderId == data.lastProcessedOrderId, 'OS5C');
        (returnOrder, amountLimit) = getSellOrder(order);
    }

    function dequeueBuyOrder(Data storage data, Order calldata order)
        external
        returns (BuyOrder memory returnOrder, uint256 amountLimit)
    {
        ++data.lastProcessedOrderId;
        require(order.orderId == data.lastProcessedOrderId, 'OS5C');
        (returnOrder, amountLimit) = getBuyOrder(order);
    }
    ...
    function verifyOrder(Data storage data, Order memory order) external view {
        require(getOrderDigest(order) == data.orderQueue[order.orderId], 'OS5B');
    }
}
