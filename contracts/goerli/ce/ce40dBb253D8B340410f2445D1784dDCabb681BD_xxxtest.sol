...
contract xxxtest is ERC721A, EIP712, Ownable {
    // EIP712 Feature
    ...
    modifier onlyOwnerOrTeam() {
        require(
            core1Address == msg.sender ||
                core2Address == msg.sender ||
                core3Address == msg.sender ||
                core4Address == msg.sender ||
                core6Address == msg.sender ||
                owner() == msg.sender,
    ...
    function _mintMeowListReservedMerkle(
        ...
        require(
            totalSupply() + amount < TOTAL_MEOWLIST_SUPPLY_PLUS_ONE,
            "Exceeds max supply"
        );

        uint256 totalClaim = premintedAmount[msg.sender] + amount;
        require(
            totalClaim < MAX_PRESALE_MINTS_PLUS_ONE,
            "Over max presale allowed"
        );

        uint256 generalAmount = premintedAmount[msg.sender] + amount - 1;
        require(
            generalMeowMinted + generalAmount < generalMeowListSuppy,
            "Exceeds General Meowlist"
        );
        ...
        require(
            MerkleProof.verify(proof, merkleRootTier1, _generateMerkleLeaf(to)),
            "User not in WL"
        );
    ...
    /**
        MeowList Sale with Merkle Tree
     */
    function mintMeowListGeneralMerkle(uint256 amount, bytes32[] calldata proof)
        external
        payable
    {
    ...
    function _mintMeowListGeneralMerkle(
        ...
        require(presaleOn, "Presale Not Live");
        require(
            premintedAmount[to] + amount < MAX_PRESALE_MINTS_PLUS_ONE,
            "Over max presale allowed"
        );
        require(
            generalMeowMinted + amount < generalMeowListSuppy,
            "Exceeds General Meowlist"
        );
        ...
        require(
            MerkleProof.verify(proof, merkleRootTier2, _generateMerkleLeaf(to)),
            "User not in WL"
        );
     ...
     */
    function mintMeowListCockadoodlelHash(
        uint256 amount,
        uint256 _passAmount,
        uint8 vSig,
        bytes32 rSig,
        ...
        uint256 generalAmount = premintedAmount[msg.sender] +
            amount -
            _passAmount;
     ...
     */
    function mintPublicMerkle(uint256 amount, bytes32[] calldata proof)
        external
        payable
    {
    ...
    function mint(address to, uint256 amount) external payable callerIsUser {
        ...
        require(
            totalSupply() + amount < MAX_SUPPLY_PLUS_ONE,
            "Exceeds max supply"
        );
