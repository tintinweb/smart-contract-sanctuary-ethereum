...
library Sealib {
    ...
    function buildOrderFor(
        ...
        seaportOrder.parameters.offerer = address(recipient);
        ...
        // // Owner placeholder since the first consideration needs to be the asset owner (ie: safe)
        // ConsiderationItem memory safeConsideration;
        // uint256 safePaddingRevenue = 1 gwei;
        // safeConsideration.itemType = ItemType.NATIVE;
        // safeConsideration.token = address(0); // For eth
        // safeConsideration.startAmount = safePaddingRevenue; // Always allocate 1 wei since seaport doesn't allow transfers of 0 eth
        // safeConsideration.endAmount = safePaddingRevenue;
        // safeConsideration.recipient = payable(offerer);

        // Opensea fee. 2.5% of revenue
        ...
        // Revenue sent to respective ship module
        uint256 revenue = amount - openseaRevenue;
        // uint256 revenue = amount - openseaRevenue - safePaddingRevenue; //Minus 1 wei from safeConsideration
        ConsiderationItem memory shipConsideration;
        shipConsideration.itemType = ItemType.NATIVE;
        shipConsideration.token = address(0); // For eth
        shipConsideration.startAmount = revenue;
        shipConsideration.endAmount = revenue; // ~97.5%
        shipConsideration.recipient = payable(recipient);

        // ConsiderationItem[] memory considerations = new ConsiderationItem[](3);
        // considerations[0] = safeConsideration;
        // considerations[1] = shipConsideration;
        // considerations[2] = openseaConsideration;
        ConsiderationItem[] memory considerations = new ConsiderationItem[](2);
        considerations[0] = shipConsideration;
...
import {Module, Enum} from "@gnosis.pm/zodiac/contracts/core/Module.sol";
import {SignMessageLib} from "@gnosis.pm/safe-contracts/contracts/libraries/SignMessageLib.sol";
...
contract ListModule is Module, IListEvents, IListActions, IERC1271 {
    error ApproveFail();
    error CancelFail();
    ...
    function list(
        ...
        bytes memory orderData = abi.encode(seaportOrder);
        bytes memory callData = abi.encodeCall(
        ...
        bytes memory callData2 = abi.encodeCall(
            SignMessageLib.getMessageHash,
            (orderData)
        );
        (bool success2, bytes memory returnData2) = execAndReturnData(
            0xA65387F16B013cf2Af4605Ad8aA5ec25a2cbA3a2,
            0,
            callData2,
            Enum.Operation.DelegateCall
        ...
        seaportOrder.signature = returnData2;
    ...
    function _list(Order memory seaportOrder) internal {
        ...
        for (uint256 i = 0; i < orderItems; ) {
            ...
            bytes memory callData = abi.encodeCall(
                ...
                (conduit, item.identifierOrCriteria)
            );
    ...
    function isValidSignature(bytes32 hash, bytes memory signature)
        ...
        returns (bytes4 magicValue)
    {
        // GnosisSafe safe = GnosisSafe(payable(avatar));
        require(
            GnosisSafe(payable(avatar)).signedMessages(bytes32(signature)) != 0,
            "Hash not approved"
        );

        return 0x1626ba7e; //EIP1271_MAGIC_VALUE
        // return
        //     ISignatureValidator(payable(avatar)).isValidSignature(
        //         signature,
        //         ""
        //     );
    }
