...
contract IsekaiBattleStake is IERC721Receiver, ReentrancyGuard, Context, AccessControlEnumerable {
    ...
    function addRegionInfo(RegionInfo memory info) external virtual onlyRole(INFO_SETTER_ROLE) {
        regionInfos.push(info);
    }

    function setRegionInfo(uint256 index, RegionInfo memory info) external virtual onlyRole(INFO_SETTER_ROLE) {
        regionInfos[index] = info;
    }

    function setMaxStakingCharacters(uint256 value) external virtual onlyRole(INFO_SETTER_ROLE) {
        maxStakingCharacters = value;
    }

    function setMinGetSeedCharacters(uint256 value) external virtual onlyRole(INFO_SETTER_ROLE) {
        minGetSeedCharacters = value;
    }

    function getRegionInfosLength() external view virtual returns (uint256) {
        return regionInfos.length;
    }

    function stakingTokenIdsLength(address account, uint64 regionId) external view virtual returns (uint256) {
        return stakingTokenIds[account][regionId].length;
    }

    function stake(StakingRequest[] calldata _stakingInfo) external virtual {
        if(!ISB.isApprovedForAll(_msgSender(), address(this))) revert NotApproved();
        actionCounts[_msgSender()].stake++;

        StakingInfo memory stakingInfo = stakingInfos[_msgSender()];

        if (stakingInfo.stakingCharactorCounts > 0) {
            claim();
            for (uint256 i = 0; i < stakingInfo.stakingRegionIds.length; i++) {
                _unstake(stakingTokenIds[_msgSender()][stakingInfo.stakingRegionIds[i]]);
                emit Unstake(
                    _msgSender(),
                    stakingInfo.stakingRegionIds[i],
                    stakingTokenIds[_msgSender()][stakingInfo.stakingRegionIds[i]]
                );
            }
        }

        delete stakingInfo.stakingRegionIds;
        stakingInfos[_msgSender()] = stakingInfo;
        uint256 allTokenCount = 0;

        for (uint256 i = 0; i < _stakingInfo.length; i++) {
            allTokenCount += _stakingInfo[i].tokenIds.length;
            if(allTokenCount > maxStakingCharacters) revert MAXCharacter();
    ...
    function claim() public virtual nonReentrant {
        StakingInfo memory stakingInfo = stakingInfos[_msgSender()];
        if(stakingInfo.stakingRegionIds.length <= 0) revert NotStake();
        uint256 elapsed = block.timestamp - stakingInfo.claimTimes;
        uint256 stakingCharactorCount = stakingInfo.stakingCharactorCounts;
        ActionCount memory origin = actionCounts[_msgSender()];
        ActionCount memory actionCount = actionCounts[_msgSender()];

        for (uint256 i = 0; i < stakingInfo.stakingRegionIds.length; i++) {
            uint64 regionId = stakingInfo.stakingRegionIds[i];
            ...
            if (
                ...
                origin.claimPlus == actionCount.claimPlus
            ) {
                actionCount.claimPlus++;
            }
            ...
            if (weaponCount > 0) {
                ...
                if (origin.wpnClaim == actionCount.wpnClaim) {
                    actionCount.wpnClaim++;
                }
                actionCount.wpn += uint64(weaponCount);
            ...
            if (armorCount > 0) {
                ...
                if (origin.armClaim == actionCount.armClaim) {
                    actionCount.armClaim++;
                }
                actionCount.arm += uint64(armorCount);
            ...
            if (stakingCharactorCount >= minGetSeedCharacters && seedPer > 0) {
                ...
                for (uint256 j = 0; j < tokenIds.length; j++) {
                    ...
                    if (random < seedPer) {
                        ...
                        actionCount.seed++;
                        if (origin.seedClaim == actionCount.seedClaim) {
                            actionCount.seedClaim++;
                        }
        ...
        actionCount.claim++;
        actionCounts[_msgSender()] = actionCount;
        stakingInfo.claimTimes = block.timestamp;
        stakingInfos[_msgSender()] = stakingInfo;
