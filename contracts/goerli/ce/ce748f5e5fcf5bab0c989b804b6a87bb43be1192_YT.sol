...
error YTError(string reason);

contract YT is IYT, ERC20PermitUpgradeable {
    using SafeMathUpgradeable for uint256;

    IPrincipalToken private principalToken;
     ...
     * @param name_ The name of the yt token.
     * @param symbol_ The symbol of the yt token.
     * @param pt The address of the principalToken associated with this yt token.
     */
    function initialize(
        string calldata name_,
        string calldata symbol_,
    ...
    * @notice checks for msg.sender to be principalToken and then calls _burn of ERC20Upgradeable.
     ...
     */
    function burnWithoutUpdate(address from, uint256 amount) external {
        if (msg.sender != address(principalToken)) {
            revert YTError("Caller should be the Future Vault");
        }
        _burn(from, amount);
    }
    ...
    * @notice updates the yield of the caller and then calls _burn of ERC20Upgradeable.
    ...
    function burn(uint256 amount) public {
        principalToken.updateYield(msg.sender);
    ...
    function transfer(
        address to,
        uint256 amount
    ) public virtual override(IYT, ERC20Upgradeable) returns (bool success) {
        principalToken.beforeYtTransfer(msg.sender, to);
    ...
    function transferFrom(
        ...
        principalToken.beforeYtTransfer(from, to);
    ...
    /** @dev See {IYT- balanceOf} */
    function balanceOf(
        address account
    ) public view override(IYT, ERC20Upgradeable) returns (uint256) {
        return
            (block.timestamp < principalToken.maturity())
                ? super.balanceOf(account)
                : 0;
    }

    /** See {IYT- actualBalanceOf} */
    function actualBalanceOf(
        address account
