...
contract RAIR721_Contract is IERC2981, ERC165, IRAIR721_Contract, ERC721, AccessControl {
	...
	/// @notice	Creates a subdivision of the collection
	/// @dev 	Like the collections, ranges are sequential
	/// @param	collectionIndex 	Index of the collection on the contract
	/// @param	_rangeLength 		Number of tokens in the range
	/// @param	_allowedTokens 		Number of tokens allowed to be sold
	/// @param	_lockedTokens 		Number of tokens that have to be minted in order to unlock transfers
	/// @param	_price 				Price for the tokens in the range
	...
	) external onlyRole(DEFAULT_ADMIN_ROLE) collectionExists(collectionIndex) {
	...
	/// @param	rangeIndex 	Index of the collection on the contract
	/// @param	_allowedTokens 		Number of tokens allowed to be sold
	/// @param	_lockedTokens 		Number of tokens that have to be minted in order to unlock transfers
	/// @param	_price 				Price for the tokens in the range
	/// @param	_name 				Name of the range
	function updateRange(
		...
		uint _price,
		string calldata _name
	) external onlyRole(DEFAULT_ADMIN_ROLE) rangeExists(rangeIndex) {
		range storage selectedRange =  _ranges[rangeIndex];

		require(_price > 100, "RAIR ERC721: Minimum price for a range is 100");

		selectedRange.tokensAllowed = _allowedTokens;
		selectedRange.lockedTokens = _lockedTokens;
		selectedRange.rangePrice = _price;
		selectedRange.rangeName = _name;
