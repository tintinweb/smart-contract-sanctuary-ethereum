...
contract Seedlings is ERC721AQueryable, ERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 public alMaxPerTxn;

    uint256 public ogMaxPerTxn;
    ...
    function setMaxPerTxns(uint256 _maxPerTxn, uint256 _alMaxPerTxn, uint256 _ogMaxPerTxn) external onlyOwner {

        maxPerTxn = _maxPerTxn;
	alMaxPerTxn = _alMaxPerTxn;
	ogMaxPerTxn = _ogMaxPerTxn;
    }
   ...
   function ogMint(bytes32[] calldata _merkleProof, uint256  ogQuantity_) external payable nonReentrant {
	...
	uint256 totals = totalSupply();
        ...
        require(ogMinted[msg.sender].add(ogQuantity_) <= ogMaxPerTxn, "Maximum OG Mint exceeded.");
   ...
   function allowlistMint(bytes32[] calldata _merkleProof, uint256  alQuantity_) external payable nonReentrant {
	...
	require(alMinted[msg.sender].add(alQuantity_) <= alMaxPerTxn, "Maximum Allowlist Mint exceeded.");
