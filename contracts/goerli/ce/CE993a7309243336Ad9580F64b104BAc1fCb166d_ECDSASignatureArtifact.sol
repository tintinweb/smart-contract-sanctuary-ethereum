...
contract ECDSASignatureArtifact is ECDSASignatureBase, IECDSASignatureArtifact{
    ...
    function hashingModifyDerrickTypeMessage(uint256 id, string memory name, uint bps, uint priceModifier) external pure returns (bytes32) {
        return keccak256(abi.encodePacked(id, name, bps, priceModifier));
    }
    ...
    function hashingRegisterPotionTypeMessage(string memory name, uint duration, uint role,uint priceModifier) external pure returns (bytes32) {
        return keccak256(abi.encodePacked(name, duration, role, priceModifier));
    }
    ...
    function hashingModifyPotionTypeMessage(uint256 id, string memory name, uint duration, uint role, uint priceModifier) external pure returns (bytes32) {
        return keccak256(abi.encodePacked(id, name, duration, role, priceModifier));
    }
