    ...
    IFlashLoanReceiver
{
    ...
    function _proxyClose(
        ...
        Close memory _proxyClose = proxyCloses[_closeType];
        require(_proxyClose.priceOracle > 0, "proxyClose is null");
        uint256 _tokenOraclePrice = IMarginTradingFactory(owner()).getPrice(
            _proxyClose.assetToken
        );
        //3：止损【预言机报价 < 止损价格】  4：止盈【预言机报价 > 止盈价格】
        require(
            !(_closeType == 3 && _tokenOraclePrice < _proxyClose.priceOracle) ||
                !(_closeType == 4 &&
                    _tokenOraclePrice > _proxyClose.priceOracle),
        ...
        if (_swapParams.length > 0) {
            ...
            require(
                _proxyClose.flag == 2 &&
                    _proxyClose.swapToAssetTokenAmount >
                    IERC20(_proxyClose.swapToAssetToken).balanceOf(
                        address(this)
                    ),
    ...
    function addProxyClose(
        ...
        require(
            (_closeType == 3 || _closeType == 4) && _priceOracle > 0,
            "addProxyClose error"
        );
        proxyCloses[_closeType] = Close(
            _assetToken,
            _priceOracle,
            _swapToAssetToken,
            _swapToAssetTokenAmount,
            _flag
        );
        emit AddProxyClose(
            _closeType,
