...
contract Tracking is ERC721URIStorage {
    ...
    event RegisterTrackingSent(address addressWallet, uint256 timestmap);
    event RegisterTrackingOpen(address addressWallet, uint256 timestmap);
    event RegisterTrackingRead(address addressWallet, uint256 timestmap);
    event RegisterTrackingFinished(address addressWallet, uint256 timestmap);
    ...
    function mint(address walletAdrress, string memory _urlMetaData) payable public returns(uint256) {
        _tokenIds.increment();
        uint256 id = _tokenIds.current();
        _mint(walletAdrress, id);
        _setTokenURI(id, _urlMetaData);

        Register storage post = idToPost[id];

        post.idToken = id;
        post.wallet = walletAdrress;
        post.SENT = true;
        post.OPEN = false;
        post.READ = false;
        post.FINISHED = false;
        ...
        emit RegisterTrackingOpen(walletAdrress, block.timestamp);
    ...
    function open(uint256 id) payable public returns (uint256) {
        ...
        if (post.SENT == true) {
            ...
            emit RegisterTrackingSent(msg.sender, block.timestamp);
        }
    ...
    function read(uint256 id) payable public returns (uint256) {
        ...
        if (post.OPEN == true) {
            ...
            emit RegisterTrackingRead(msg.sender, block.timestamp);
        }
    ...
    function finished(uint256 id) payable public returns (uint256) {
        ...
        if (post.READ == true) {
            ...
            emit RegisterTrackingFinished(msg.sender, block.timestamp);
        }
