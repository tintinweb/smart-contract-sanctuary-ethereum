...
// Seapad is a decentralized marketplace for NFT Collectible Packs.
// Built by Mleinad. 2022.
...
interface SeapadPayments {
    ...
    function insertNewProject(
        ...
        uint256 basePayment,
        uint256 amountPaidToLaunch //EXCLUDING ORACLE FEE
    ) external payable;
...
contract Seapad is VRFV2WrapperConsumerBase, ChainlinkClient {
    ...
    bytes32 jobId;
    address owner;
    uint256 ORACLE_FEE_PACK_GENERATION; //CHAINLINK FEE FOR THE
    address ORACLE_FEE_ADDRESS;
    ...
    function updateContract(
        ...
        uint256 _ORACLE_FEE_PACK_GENERATION,
        ...
        ORACLE_FEE_PACK_GENERATION = _ORACLE_FEE_PACK_GENERATION;
    ...
    function updateAddresses(
        ...
        ORACLE_FEE_ADDRESS = _ORACLE_FEE_ADDRESS;
    ...
    function getProjectPriceInBaseToken(uint256 projectId)
        public
        view
        returns (address contractAddress, uint256 pricePerPack)
    {
        Projects storage project = projects[projectId];
        return (project.contractAddress, project.pricePerPack);
    }
    ...
    function purchasePack(
        ...
        address toFinal = to == address(0) ? msg.sender : to;
        ...
        pb.ownedBy = toFinal;
        // if()
        if (project.whitelisted && msg.sender != owner) {
            require(project.whitelist[toFinal] == true, "5"); //not whitelisted
        }
        (
            ,
            address paymentMethodAddress,
        ...
        (
            uint256 pricePerPackInChoosenPaymentMethodAfterSavedFeeRemoved, //this is price of pack, after the fee for seapad is removed
            ,
            ,
            ,
            uint256 feeAmountRemainingForSeapadFromOriginalPackPrice //this is the fee for seapad, that is removed from the pack price (which goes to seapad.)
        ...
        pack.mintNewToken(toFinal, packsBoughtId);
    ...
    function splitRemainingFunds(
        ...
        // send transaction to seapad and the owners. (Convert if nececerry)
        uint256 seapadReceived;
        uint256 ownerReceived;

        if (!keep) {
            token.transfer(paymentAddress, remaindingForSeapad);
            //we receive ETH with this function.
            seapadReceived = Payments.swapAndTransferTo(
                paymentMethodAddress,
                uniswapV2Router.WETH(),
                address(this), //keep the funds on the contract.
                remaindingForSeapad
            );
            payable(paymentAddress).transfer(seapadReceived);
            SeapadPayments(paymentAddress).updateProfitForSeapad(
                seapadReceived,
                0
            );
        } else {
            seapadReceived = remaindingForSeapad;
            token.transfer(paymentAddress, remaindingForSeapad);
            SeapadPayments(paymentAddress).updateProfitForSeapad(
                seapadReceived,
                _paymentMethod
        ...
        if (!isBasePayment) {
            ...
            ownerReceived = Payments.swapAndTransferTo(
                paymentMethodAddress,
                basePaymentMethodAddress,
                project.contractAddress, //PROFITS FOR OWNER OF PROJECT IS SENT TO HIS/HERS NFT CONTRACT
                remainingForO
            );
        } else {
            ownerReceived = remainingForO;
            token.transfer(project.contractAddress, remainingForO); //PROFITS FOR OWNER OF PROJECT IS SENT TO HIS/HERS NFT CONTRACT
        }
        ...
        pb.seapadCurrency = keep
            ? paymentMethodAddress
            : uniswapV2Router.WETH();
        pb.ownerRecieved = ownerReceived;
        pb.ownerCurrency = _basePaymentMethodAddress;
        pb.seapadRecieved = seapadReceived;
    ...
    function mintOpenedPack(uint256 projectid, uint256 packSoldID_) public {
        ...
        NFT minter_ = NFT(payable(project.contractAddress));
    ...
    */
    ...
    function generateWinnings(
        ...
        for (uint256 i = 0; i < project.amountToPayOutPerPack; i++) {
            uint256 _randomResult = randomness[i] % project.itemsLeft;
            generateWinnerNumber(projectid, _randomResult, requestid);
        }
    }
    ...
    function generateWinnerNumber(
        ...
        isValidProject(projectid);
        Projects storage project = projects[projectid];
        uint256 itemsAmounts = project.itemsLeft;
        PacksBoughtRequest storage pbr = packsBoughtRequests[requestid];
        PacksBought storage pb = project.packsBought[pbr.packSoldID];
        ...
        if (itemsAmounts == randomResult) {
            uint256 winner = project.items[randomResult] != 0
                ? project.items[randomResult]
                : randomResult;
            pb.wonItems.push(winner);
        } else {
            uint256 winner = project.items[randomResult] != 0
                ? project.items[randomResult]
                : randomResult;
            pb.wonItems.push(winner);
            if (project.items[randomResult] == 0) {
                project.items[winner] = project.items[itemsAmounts] != 0
                    ? project.items[itemsAmounts]
                    : itemsAmounts;
            } else {
                project.items[randomResult] = project.items[itemsAmounts] != 0
                    ? project.items[itemsAmounts]
                    : itemsAmounts;
            }
        }
    ...
    function createToken(
        ...
        require(totalItems % amountToPayOutPerPack == 0, "1"); //not divisible
        require(amountToPayOutPerPack <= MAX_ITEMS_PER_PACK, "2"); //tomanyitems
        require(
            totalItems / amountToPayOutPerPack <= MAX_PACKS,
            "3" //tomanypacks
        );
        require(
            totalItems / amountToPayOutPerPack > 0,
            "4" //toolittlepacks
        );
        require(LAUNCH_ENABLED, "5"); //launches_not_enabled
        ...
        emit ProjectCreated(
            ...
            project.totalItems, //collection id's
    ...
    function createTokenPart2(
        ...
        uint256 launchTime,
        uint256[] calldata paymentMethods_,
        uint256 basePayment // uint256 slippage // uint16 gasLane
    ) public payable {
        ...
        uint256 seapadProfit = msg.value - ORACLE_FEE_PACK_GENERATION;

        payable(ORACLE_FEE_ADDRESS).transfer(ORACLE_FEE_PACK_GENERATION);

        Payments.insertNewProject{value: seapadProfit}(
            msg.sender,
            pid,
            paymentMethods2,
            bp,
        ...
        address projectAddress = project.packAddress;
        makeChainLinkRequest(
            ...
            bid,
            pbg,
            SeapadHelper.toAsciiString(projectAddress)
        );
        bool _isJson = projects2.isJson;
        projects2.id = pid;
        uint256 ppp = project.pricePerPack;
        uint256 lt = projects2.launchTime;
        emit ProjectCreatedP2(pid, bid, pbg, ppp, lt, _isJson, projectAddress);
    }
    ...
    function makeChainLinkRequest(
        ...
        return sendOperatorRequest(request, 0);
    }
...
contract Pack is ERC721, Ownable {
    ...
    modifier onlyCollectionOwnerOrSeapad() {
        _;
        require(
            msg.sender == packCollectionOwner || msg.sender == seapadOwner,
            "Only contract owner or Seapad Owner can call this (Pack.sol)."
        );
    }
    ...
    // WITHDRAW FUNCTION. (Remains public in case no funds for owner, he can request hes buddy to withdraw but to owners wallet.)
    function WithdrawAndSplitETHFunds() public {
        payable(packCollectionOwner).transfer(amountSeapadWithdrawable);
        payable(seapadAddress).transfer(amountOwnerWithdrawable);
        amountSeapadWithdrawable = 0;
        amountOwnerWithdrawable = 0;
    }

    // WITHDRAW FUNCTION. (Remains public in case no funds for owner, he can request hes buddy to withdraw but to owners wallet.)
    function WithdrawAndSplitTokenFunds(address tokenAddress) public {
        IERC20 token = IERC20(tokenAddress);
        //calculate seapad profit
        uint256 seapadAmount = (token.balanceOf(address(this)) *
            seapadRoyaltyPercentage) / 10000;
        uint256 ownerAmount = token.balanceOf(address(this)) - seapadAmount;
        token.transfer(packCollectionOwner, ownerAmount);
        token.transfer(seapadAddress, seapadAmount);
    }
