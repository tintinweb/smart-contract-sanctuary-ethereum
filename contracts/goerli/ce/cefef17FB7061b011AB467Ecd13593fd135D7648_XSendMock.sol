...
contract XSendMock {
  ...
  address dao;
  address public xprovider; // XProviderMock
  ...
  function xSendSomeValue(uint256 _value, uint16 _destinationDomain) external {
    // should later on be changed to onlyDao/ onlyKeeper
    IXProviderMock(xprovider).xSend(_value, _destinationDomain);
  }
  ...
  function xTransferFunds(
    ...
    IXProviderMock(xprovider).xTransfer{value: relayerFee}(
      to,
      asset,
      destinationDomain,
      amount,
...
interface IXProviderMock {
  ...
  function xSend(uint256 _value, uint16 _destinationDomain) external; // sending a (permissioned) value crosschain.
  ...
  function xTransfer(
    ...
    uint256 amount,
    uint256 relayerFee
  ) external payable; // transfer funds crosschain.
