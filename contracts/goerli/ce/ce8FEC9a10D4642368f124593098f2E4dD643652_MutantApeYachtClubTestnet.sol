...
contract MutantApeYachtClubTestnet is MutantApeYachtClub {
    ...
    constructor(
        ...
        address baycAddress,
        address baccAddress
    ) MutantApeYachtClub(name, symbol, baycAddress, baccAddress) {}

    function mintMutants(uint256 id)
        public
        payable
        override
        whenPublicSaleActive
        nonReentrant
    {
        ...
        require(
            id <= PS_MAX_MUTANTS,
            "Minting would exceed max number"
        );
        require(
            numMutantsMinted + numMutants <= PS_MAX_MUTANTS,
            "Minting would exceed max supply"
 ...
 */
contract _ERC721 is ERC165Storage, Context, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        require(_msgSender() == owner || _ERC721.isApprovedForAll(owner, _msgSender()),
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        ...
        address owner = _ERC721.ownerOf(tokenId);
        return (spender == owner || getApproved(tokenId) == spender || _ERC721.isApprovedForAll(owner, spender));
    }
    ...
    function _burn(uint256 tokenId) internal virtual {
        address owner = _ERC721.ownerOf(tokenId); // internal owner
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(_ERC721.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own"); // internal owner
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(_ERC721.ownerOf(tokenId), to, tokenId); // internal owner
    }
 ...
 */
contract BoredApeYachtClub is _ERC721, Ownable {
    using SafeMath for uint256;
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply, uint256 saleStart) _ERC721(name, symbol) {
    ...
    * Mints Bored Apes
    */
    function mintApe(uint numberOfTokens) public payable virtual {
...
contract BoredApeYachtClubTestnet is BoredApeYachtClub {
    constructor(string memory name, string memory symbol, uint256 maxNftSupply, uint256 saleStart) BoredApeYachtClub(name, symbol, maxNftSupply, saleStart) {}

    function mintApe(uint id) public payable override {
        uint numberOfTokens = 1;
        require(id <= MAX_APES, "Number exceed max value");
