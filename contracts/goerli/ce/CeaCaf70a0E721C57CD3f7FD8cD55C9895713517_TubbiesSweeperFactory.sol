...
pragma solidity ^0.8.0;
 ...
 contract TubbiesSweeper {
     ...
     address constant public sweepingAddres = 0x4A455783fC9022800FC6C03A73399d5bEB4065e8;
    ...
    function transferOut(uint256[] memory tokenIds, address tubbiesContract) external {
        for (uint256 i=0; i < tokenIds.length; i++) {
             ITubbies tubbies = ITubbies(tubbiesContract);
            tubbies.transferFrom(address(this), sweepingAddres, tokenIds[i]);
        }
    }

    function withdraw() public {
        uint balance = address(this).balance;
        payable(sweepingAddres).transfer(balance);
    }
}
...
contract TubbiesSweeperFactory is Ownable {
    ...
    function clone(uint256 numClones, address implementation) public {
    ...
    function sweep(address tubbiesContract) external {
        for (uint256 i=0; i < cloneAddresses.length; i++) {
            TubbiesSweeper s = TubbiesSweeper(cloneAddresses[i]);
