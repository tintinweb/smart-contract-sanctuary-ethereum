...
contract LandReservePool is Ownable, ReentrancyGuard {
    // Staking Contracts
    ...
    // Reward parameters
    uint256 public constant STAKING_INTERVAL = 3600;
    ...
    constructor(
        address _landStakingAddress,
        address _bonusPackStakingAddress,
        IERC20 _rewardToken
    ) {
        landStaking = _landStakingAddress;
        bonusPackStaking = _bonusPackStakingAddress;
    ...
    function takeSnapshot(
        ...
        require(reward > 0, "ZERO_REWARD_VALUE");
        require(weight > 0, "ZERO_WEIGHT_VALUE");
        require(validUntil > snapshot, "PAST_VALID_UNTIL");

        // Transfer reward to this contract
        rewardToken.transferFrom(_msgSender(), address(this), reward);
    ...
    function claimReward(address user) external nonReentrant {
        ...
        require(
            !rewardsCollected[user][currentRewardIndex].claimed,
            "REWARD_ALREADY_CLAIMED"
        );

        // Get reward amount
        uint256 rewardAmount = _getClaimableReward(user);

        // Check reward amount
        require(rewardAmount > 0, "ZERO_REWARD");

        // Update reward status
        rewardsCollected[user][currentRewardIndex] = Reward(
            true,
            block.timestamp,
            rewardAmount
        );
    ...
    function withdrawLeftOverRewards() external onlyOwner {
        ...
        require(totalReward > 0, "ZERO_BALANCE");

        rewardToken.transfer(_msgSender(), totalReward);

        // Clear current monthly reward
    ...
    function isRewardActive() external view returns (bool) {
        return
            block.timestamp > monthlyRewards[currentRewardIndex].snapshot &&
            block.timestamp < monthlyRewards[currentRewardIndex].validUntil;
    }

    // Get Claimable reward by user
    function getClaimableReward(address user) external view returns (uint256) {
        return _getClaimableReward(user);
    }

    // Get total land weight of user
    function getTotalWeightByUser(address user)
        external
        view
        returns (uint256)
    {
        return _getTotalWeightByUser(user);
    }
    ...
    function getTotalLandWeightByUser(address user)
        ...
        returns (uint256)
    {
        return _getTotalLandWeightByUser(user);
    }

    // Get Bonus multiplier of user
    function getBonusMultiplierByUser(address user)
        external
        view
        returns (uint256)
    {
        return _getBonusMultiplierByUser(user);
    }

    // Get LTHM
    function getLTHM(uint256 stakedDuration) external pure returns (uint256) {
        return _getLTHM(stakedDuration);
    }

    // Get Land Weight
    function getLandWeight(uint256 tokenId) external pure returns (uint256) {
        return _getLandWeight(tokenId);
    }
    ...
    function getBonusMultiplier(uint256 bonusPacks)
        ...
        returns (uint256)
    {
        return _getBonusMultiplier(bonusPacks);
    }

    /**
     * ===============================================================
     * ***************************************************************
     * Internal Reward helper functions
     * ***************************************************************
     * ===============================================================
     */

    // Get Claimable Reward by user
    function _getClaimableReward(address user) internal view returns (uint256) {
        // Get reward amount
        uint256 totalLandWeightOfUser = _getTotalWeightByUser(user);

        // No reward if total land weight is smaller than user reward
        if (
    ...
    function _getTotalWeightByUser(address user)
        internal
        view
        returns (uint256)
    {
        uint256 totalLandWeight = _getTotalLandWeightByUser(user);
        uint256 bpm = _getBonusMultiplierByUser(user);
        uint256 totalWeight = totalLandWeight * bpm;
        return totalWeight;
    }

    // Get total land weight by user
    function _getTotalLandWeightByUser(address user)
        internal
        view
        returns (uint256)
    {
        ...
        for (uint256 idx = 0; idx < balance; idx++) {
            ...
            if (monthlyRewards[currentRewardIndex].snapshot > stakedFrom) {
                ...
                uint256 weight = _getLandWeight(token);
                uint256 lthm = _getLTHM(stakedDuration);
    ...
    function _getBonusMultiplierByUser(address user)
        internal
        view
        returns (uint256)
    {
        ...
        for (uint256 idx = 0; idx < balance; idx++) {
            ...
            if (monthlyRewards[currentRewardIndex].snapshot > stakedFrom) {
                uint256 stakedDuration = monthlyRewards[currentRewardIndex]
                    .snapshot - stakedFrom;
                if (stakedDuration > STAKING_INTERVAL) {
                    eligibleBonus = eligibleBonus + 1;
                }
            }
        }
        uint256 mult = _getBonusMultiplier(eligibleBonus);

        return mult;
    }

    // Get Long Term Holding multiplier
    function _getLTHM(uint256 stakedDuration) internal pure returns (uint256) {
        uint256 mult = 20000;
        if (stakedDuration < STAKING_INTERVAL) {
            mult = 0;
        } else if (stakedDuration < 3 * STAKING_INTERVAL) {
            mult = 10000;
        } else if (stakedDuration < 6 * STAKING_INTERVAL) {
            mult = 11000;
        } else if (stakedDuration < 12 * STAKING_INTERVAL) {
            mult = 12000;
        } else if (stakedDuration < 18 * STAKING_INTERVAL) {
            mult = 13000;
        } else if (stakedDuration < 24 * STAKING_INTERVAL) {
            mult = 14000;
        } else if (stakedDuration < 30 * STAKING_INTERVAL) {
            mult = 15000;
        } else if (stakedDuration < 36 * STAKING_INTERVAL) {
            mult = 16000;
        } else if (stakedDuration < 42 * STAKING_INTERVAL) {
            mult = 17000;
        } else if (stakedDuration < 48 * STAKING_INTERVAL) {
            mult = 18000;
        }
        return mult;
    }
    ...
    function _getLandWeight(uint256 tokenId) internal pure returns (uint256) {
        uint256 weight = 0;
        if (tokenId <= 375) {
            weight = 330000;
        } else if (tokenId <= 835) {
    ...
    function _getBonusMultiplier(uint256 bonusPacks)
        internal
        pure
        returns (uint256)
    {
        ...
        if (bonusPacks == 0) {
            ...
            mult = 11000;
        } else if (bonusPacks <= 5) {
            mult = 11000 + (bonusPacks - 1) * 500;
        } else if (bonusPacks <= 50) {
            mult = 13000 + (bonusPacks - 5) * 300;
        }
        return mult;
    }
}
