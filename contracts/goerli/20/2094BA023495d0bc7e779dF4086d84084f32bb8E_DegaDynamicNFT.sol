...
library LibString {
    function stringToUint(string memory s) internal pure returns (uint, bool) {
        bool hasError = false;
...
contract BaseNFT is ERC721, Ownable, ERC2771Recipient {
    ...
    function _mint(address _to) internal returns (uint256) {
        ...
        require(tokenIds <= maxSupply, "Exceed MaxSupply");
        _safeMint(_to, tokenIds);

        emit Minted(tokenIds, _to);
        return tokenIds;
    }
    ...
    function setDescription(string memory _newDesc) external onlyOwner {
...
contract DegaDynamicNFT is BaseNFT {
    using Strings for uint256;
    using LibString for string;

    string public dynamicSvg;
    uint256 public propertyCount;
    /// @dev propertyId => property name
    mapping(uint256 => string) public propertyNames;
    /// @dev tokenId => propertyId => property Value;
    mapping(uint256 => mapping(uint256 => string)) private properties;
    ...
    constructor(
        ...
        address _owner,
        address _forwarder
    ) BaseNFT(_name, _symbol, _contractURI, _maxSupply, _owner, _forwarder) {}
    ...
    function setPropertyNames(uint256 count, string[] memory names)
        external
        onlyOwner
    {
        require(count > 0 && count == names.length, "invalid names");
    ...
    function setPropertyValue(
        ...
        uint256 propertyId,
        string memory value
    ) external virtual onlyOwner existingTokenId(tokenId) {
