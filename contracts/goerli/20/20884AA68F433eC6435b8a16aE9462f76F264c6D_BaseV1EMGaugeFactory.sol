...
contract EarnMore is Gauge {
    ...
    constructor(address _stake, 
                ...
                address _treasury) Gauge(_stake, __ve, _voter) {
        excludePercent = _excludePercent;
        earnMorePercent = _earnMorePercent;
        treasury = _treasury;
    }
    ...
    function derivedBalance(address account) public override view returns(uint) {
    	...
    	if (earnMore[account]) {
        	derived = derived * (earnMorePercent + DIVISOR) / DIVISOR;
        }
    ...
    function calculateTokenBalance(uint256 lpValue) public view returns(uint) {
    	Vault vault = Vault(stake);

    	return vault.pricePerShare() * lpValue / 10 ** vault.decimals();
    }

    function earnMoreDeposit(uint amount, uint tokenId) external {
    ...
    function _excludeProfit(address account, uint256 amount) internal returns(uint) {
    	...
    	uint256 profit = curTokenBalance - (baseTokenBalance[account] * amount)/ balanceOf[account]; 
    	uint256 baseValueToExclude = (profit * excludePercent) / DIVISOR;
    	...
    	_safeTransfer(stake, treasury, valueToExclude);
    ...
    function deposit(uint amount, uint tokenId) external override {
    ...
    function fromGaugeToEarn() external {
        require(!earnMore[msg.sender], "You already have EarnMore deposit");
        earnMore[msg.sender] = true;
...
contract BaseV1EMGaugeFactory {
    ...
    function createEMGauge(
        ...
        returns (address) 
    {
        last_gauge = address(new EarnMore(_stake, _ve, msg.sender, _excludePercent, _earnMorePercent, _treasury));
        return last_gauge;
    }
    ...
    function createEMGaugeSingle(
        ...
        returns (address) 
    {
        last_gauge = address(new EarnMore(_stake, _ve, _voter, _excludePercent, _earnMorePercent, _treasury));
        return last_gauge;
    }
}
