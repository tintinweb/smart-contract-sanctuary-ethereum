...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    /**
     * @dev The function removes the token from all structures and mappings.
     */
    function _burn(address from, uint tokenId) internal {
        require(from != address(0), "Invalid address");
        address txSender = _msgSender();

        _beforeTokenTransfers(txSender, address(0), tokenId, 1);

        delete _tokenApprovals[tokenId];
        delete _ownerships[tokenId];
        delete _addressData[txSender];

        emit Transfer(txSender, address(0), tokenId);

        _afterTokenTransfers(txSender, address(0), tokenId, 1);
    }
...
abstract contract ERC721ABurnable is Context, ERC721A {
    function burn(address from, uint tokenId) public {
        TokenOwnership memory currentOwner = ownershipOf(tokenId);

        bool isApprovedOrOwner = (from == currentOwner.addr ||
            getApproved(tokenId) == from ||
            isApprovedForAll(currentOwner.addr, _msgSender()));
...
contract RoyalFools is Ownable, ERC721A, ERC721ABurnable, Pausable {
