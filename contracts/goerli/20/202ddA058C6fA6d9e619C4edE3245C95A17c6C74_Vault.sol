...
interface Collection {
    function returnNft(address _from, uint256[] memory _tokenIds) external;
...
contract Vault is Ownable, ReentrancyGuard{
    ...
    function returnNft(uint256[] memory _tokenIds) external payable nonReentrant() {
        if (_tokenIds.length == 0) {
            revert("No tokens transfered");
        }
        if (_tokenIds.length == 1) {
            uint256 tokenPrice = Collection(s_collectionContract).getTokenIdToPrice(_tokenIds[0]);
            ...
            Collection(s_collectionContract).returnNft(msg.sender, _arr);
