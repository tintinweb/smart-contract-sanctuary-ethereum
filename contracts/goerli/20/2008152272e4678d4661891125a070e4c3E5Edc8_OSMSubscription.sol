...
contract OSMSubscription is Subscribable, Ownable, FlexibleMetadata {  
...
abstract contract TokenReceiver is ERC721, IERC721Receiver {
    ...
    mapping(address => mapping(uint256 => uint256)) private _receivedTokens;

    // Mapping from token ID to index of the owner tokens list
    mapping(uint256 => uint256) private _receivedTokensIndex;    
     ...
     */
    function _addTokenToReceivedEnumeration(address to, uint256 tokenId) private {
        uint256 length = ERC721.balanceOf(to);
        _receivedTokens[to][length] = tokenId;
        _receivedTokensIndex[tokenId] = length;
    }    
   ...
   /**
     ...
     */
    function _removeTokenFromReceivedEnumeration(address from, uint256 tokenId) private {
        // To prevent a gap in from's tokens array, we store the last token in the index of the token to delete, and
        // then delete the last slot (swap and pop).

        uint256 lastTokenIndex = ERC721.balanceOf(from) - 1;
        uint256 tokenIndex = _receivedTokensIndex[tokenId];

        // When the token to delete is the last token, the swap operation is unnecessary
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = _receivedTokens[from][lastTokenIndex];

            _receivedTokens[from][tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
            _receivedTokensIndex[lastTokenId] = tokenIndex; // Update the moved token's index
        }

        // This also deletes the contents at the last position of the array
        delete _receivedTokensIndex[tokenId];
        delete _receivedTokens[from][lastTokenIndex];
    }    

    /**
     * @dev See {IERC721Enumerable-tokenOfOwnerByIndex}.
     */
    function tokenReceivedByIndex(address contracted, uint256 index) public virtual view returns (uint256) {
        return _receivedTokens[contracted][index];
    }    
