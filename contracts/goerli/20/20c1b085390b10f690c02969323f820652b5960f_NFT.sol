...
// File: contracts/SDLC/elachiNft.sol
...
contract NFT is ERC721Enumerable, Ownable {
    ...
    constructor(string memory name, string memory symbol, uint256 _maxNftSupply) ERC721(name, symbol) {
        MAX_NFT_SUPPLY = _maxNftSupply;
        // setBaseURI(_baseURI);

    }

    function reserveNFT(uint256 numberOfNFT, address _user) public onlyOwner { 
        require(saleIsActive, "Sale must be active to mint NFTs");
        require(totalSupply().add(numberOfNFT) <= MAX_NFT_SUPPLY, "Reserve nft would exceed max supply of NFTs");
    ...
    function mint(uint256 numberOfNFT) external{
        require(saleIsActive, "Sale must be active to mint NFTs");
        require(totalSupply().add(numberOfNFT) <= MAX_NFT_SUPPLY, "Purchase would exceed max supply of NFTs");
        require(balanceOf(msg.sender).add(numberOfNFT) <= MAX_NFT_WALLET, "Purchase exceeds max allowed per wallet");
