...
contract BrandExtenderMinter is OwnableUpgradeable, ERC721struct, ERC5169Upgradable, VerifySignature, UriChangerBase, DerivedERC2981RoyaltyUpgradeable, OptimizedEnumerableUpgradeable, ParentContractsUpgradeable, SharedHolders, Minter, UUPSUpgradeable {
    ...
    uint constant contractMintRoyalty = 500;

    address RoyaltyReceiver;
    address mintFundsReceiver;
 
    uint256 mintStartTimestamp;
    uint256 mintEndTimestamp;

    uint256 mintPriceUpTo3;
    uint256 mintPrice3Plus;
    mapping(address => uint256) internal mintedByWallet;
    bool internal collectContracts;
    ...
    event MintPricesUpdated(uint _mintPriceUpTo3, uint _mintPrice3Plus);
    ...
    function initialize(address _rr, address _newUriChanger) external initializer {
        __Ownable_init();
        ...
        // 0.025 ETH
        mintPrice3Plus = 1 ether / 1000 * 25; 

        collectContracts = true;

    }

    function switchCollectContractsFlow() public onlyOwner {
        collectContracts = !collectContracts;
    }

    function getMinterAndParent(uint tokenId) external view returns(address minter, address parent){
        minter = getMinter(tokenId);
        ERC721s memory _parent = getParent(tokenId);
        parent = getRoyaltyBeneficiary(_parent.erc721);
    }

    function getMinterParentHolder(uint tokenId) external view returns(address minter, address parent, address holder){
        minter = getMinter(tokenId);
        ERC721s memory _parent = getParent(tokenId);
        parent = _parent.erc721;
        holder = _getTokenOwner(parent, tokenId);
    }

    function setMintPrices(uint _mintPriceUpTo3, uint _mintPrice3Plus) external onlyUriChanger {
        emit MintPricesUpdated(_mintPriceUpTo3, _mintPrice3Plus);
        mintPriceUpTo3 = _mintPriceUpTo3;
        mintPrice3Plus = _mintPrice3Plus;
    }
    ...
    function validateMintRequest(mintRequestData calldata data) internal view returns(bool){
        ...
        bytes memory toSign = abi.encodePacked(address(this), msg.sender, balanceOf(msg.sender), erc721, data.tokenId, block.chainid, data.tokenURI );
    ...
    function mintedBy(address wallet) public view returns (uint){
        return mintedByWallet[wallet];
    }

    function processPriceAndCounter(uint mintNumber) internal returns (uint){
        uint mintSum;
        uint balance = mintedByWallet[msg.sender];
        mintedByWallet[msg.sender] = balance + mintNumber;
        if (balance > 3) {
    ...
    function mintDerived(mintRequestData[] calldata data) external virtual payable {
        ...
        uint balance = processPriceAndCounter(data.length);
        ...
        for (i=0; i < data.length; i++) {
            ...
            if (!collectContracts){ 
                ...
                if (mintPrice > 0){
                    uint contractRoyaltyAmount = mintPrice * contractMintRoyalty / 10000;
    ...
    function withdraw() external {
        ...
        require(mintFundsReceiver != address(0), "Receiver not set");

        _pay(mintFundsReceiver, balance);
    }
    ...
    // required to solve inheritance
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC5169, OptimizedEnumerableUpgradeable, DerivedERC2981RoyaltyUpgradeable) returns (bool) {
        return 
    ...
    function _getTokenOwner(address erc721, uint tokenId) internal view returns (address owner) {
        owner = ERC721(erc721).ownerOf(tokenId);
    }

    function getParentNftHolder( uint tokenId) public view returns (address owner) {
...
interface iMinterAndParent is ERC721struct {
    function getMinterAndParent(uint tokenId) external view returns(address minter, address parent);
    function getMinterParentHolder(uint tokenId) external view returns(address minter, address parent, address holder);
    function getParentNftHolder(uint tokenId) external view returns( address holder);
