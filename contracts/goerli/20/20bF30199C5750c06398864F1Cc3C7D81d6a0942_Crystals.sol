    ...
    IERC5050Receiver
{
    ...
    function _claimCrystalMana(uint256 tokenId) internal ownsCrystal(tokenId) {
        ...
        if (spellCasts[tokenId].spellId != 0) {
    ...
    function refocusMana(uint256 tokenId) external view returns (uint32) {
        ...
        if (mana > (crystal.focus * getResonance(tokenId))) {
            if (spellCasts[tokenId].spellId != 0) {
                return (mana - (crystal.focus * getResonance(tokenId))) / 2;
            }
    ...
    function _castWithCharacter(uint256 crystalId, uint256 spellId, address collection, uint256 characterId)
        internal 
    {
        ...
        iMana.burn(_msgSender(), 50 * crystalsMap[crystalId].focus * crystalsMap[crystalId].attunement);
    ...
    function clearSpell(uint256 crystalId, address collection, uint256 characterId)
        ...
        iMana.transfer(spellCasts[crystalId].from, 20 * crystalsMap[crystalId].focus * crystalsMap[crystalId].attunement);
    ...
    {
        require(
            msg.sender == spellsContract,
            "sender not spells"
        );
        require(
            action.selector == _CAST_SELECTOR, 
            "not cast"
        );
        require(
        ...
        _castWithCharacter(action.to._tokenId, action.to._tokenId, collection, characterId);
    }
