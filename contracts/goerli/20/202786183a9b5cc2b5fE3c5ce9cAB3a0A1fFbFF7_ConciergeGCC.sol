...
contract ConciergeGCC is Ownable, ReentrancyGuard {
    constructor() {
        if (block.chainid == 1) {
            ...
            // goerli
            gccNftAddress = 0x346898dAB86d9bdD3881f248a192D529b818766f;
        }
    ...
    /// @notice Concierge token purchase price
    uint256 public purchasePrice = 0.01 ether;

    /// @notice Set Concierge token purchase price
    ...
    /// @notice Function to stake GCC token
    /// @param tokenId GCC NFT ID to stake
    function stakeAndPurchase(uint256 tokenId) public payable nonReentrant {
        require(
            msg.value >= purchasePrice,
            "ConciergeGCC: Did not send enough"
        );
        require(stakingInfo[msg.sender].staker == address(0), "ConciergeGCC: Already staked once from this address");
        require(
            gccNftAddress != address(0),
            "ConciergeGCC: GCC NFT Address not set"
        );
        require(
            gccNft.ownerOf(tokenId) == msg.sender,
            "ConciergeGCC: Only the owner of the token can stake it"
        );
        ...
        emit ConciergeTokenPurchased(msg.sender, tokenId, msg.value);
    }

    /// @notice Unstake GCC NFT
    /// @param tokenId GCC NFT Token ID
    ...
    function unstake(uint256 tokenId) public nonReentrant {
        ...
        gccNft.transferFrom(address(this), gccNft.ownerOf(tokenId), tokenId);
