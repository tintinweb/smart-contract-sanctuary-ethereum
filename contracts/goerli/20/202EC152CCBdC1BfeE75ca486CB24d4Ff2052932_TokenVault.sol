    ...
    ERC1155HolderUpgradeable
{
        ...
        initializer
    {
        ...
        token = params.token;
        id = params.id;
        (address _nftGovernor, , ) = ISettings(settings).getGovernorSetting(
            token
        );
        ...
        if (nftGovernor != address(0)) {
            require(
                params.treasuryBalance >=
                    ((params.supply * ISettings(settings).votingMinTokenPercent()) /
                        10000),
    ...
    function initializeGovernorToken() external onlyOwner {
        if (nftGovernor != address(0)) {
            IVotesUpgradeable(token).delegate(address(this));
        }
    ...
    function _checkAfterTargetCall() public view virtual {
        require(
            IERC721(token).ownerOf(id) == address(this),
...
library TokenVaultLogic {
        ...
        returns (uint256, uint256)
    {
        ...
        require(
            exitPrice == 0 ||
                (newPrice <= exitPrice * 5 && newPrice >= exitPrice / 5),
        ...
        // previously they were not voting
            ...
            uint256 averageExit = exitTotal / votingTokens;

            uint256 exitPriceMin = (averageExit *
                ISettings(settings).minExitFactor()) / 1000;
            require(newPrice >= exitPriceMin, Errors.VAULT_PRICE_TOO_LOW);
            uint256 exitPriceMax = (averageExit *
                ISettings(settings).maxExitFactor()) / 1000;
            require(newPrice <= exitPriceMax, Errors.VAULT_PRICE_TOO_HIGHT);
        ...
        // they are updating their vote
            ...
            uint256 averageExit = (exitTotal - (oldPrice * weight)) /
                (votingTokens - weight);
