...
contract BettingAdmin is Storage, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function initialize(address usdcContract_, address vaultContract_, address signer_, address proxy_) public initializer {
    ...
    function createPool(uint256 numberOfTeams_, string memory eventName_, uint256 startTime_, uint256 duration_, string[] memory teams_, string memory uri_) external onlyRole(MULTISIG_ROLE) {
    ...
    function gradePool(uint256 poolId_, uint256 winnerId_) external onlyRole(MULTISIG_ROLE) validPool(poolId_) {
        Pool storage pool = pools[poolId_];
        require(pool.status == PoolStatus.Closed, "BettingAdmin: Pool status should be Closed");
    ...
    function updatePoolMintAddress(uint256 poolId_, address mintAddress_) external onlyRole(MULTISIG_ROLE) validPool(poolId_) {
        Pool storage pool = pools[poolId_];
