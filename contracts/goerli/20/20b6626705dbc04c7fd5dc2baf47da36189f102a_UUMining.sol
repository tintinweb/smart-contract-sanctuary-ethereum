...
contract UUMining is Ownable {
    ...
    uint public CONST_UURATE = 1000; // num of days to finish distributing. 0.1% per day
    ...
    function claimUU(uint _id) public onlyValidUsers {
        ...
        require (UUtoken.balanceOf(address(uuTokenWallet)) >= available2ClaimUU, 'Insuffient UU for claiming');
    ...
    function openTreasures(uint256 _id) public onlyValidUsers {
    ...
    function withdrawErc20Token (address _tokenAddr, uint256 _withdrawAmt) public onlyOwner {
        ...
        require (erc20token.balanceOf(address(this)) >= _withdrawAmt, 'insufficent token for withdrawal');
        erc20token.transfer(address(owner()), _withdrawAmt);
    }
