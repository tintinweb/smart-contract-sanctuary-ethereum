...
contract ARB is Context, IERC20, Ownable {
    ...
    function excludeAccounts(address[] memory accounts) external onlyOwnerAndAdmin {
        ...
        require(!_isExcluded[accounts[i]], "TOKEN: Account is already excluded");
    ...
    function includeAccounts(address[] memory accounts) external onlyOwnerAndAdmin {

        require(accounts.length < 10,"accounts length must be less then 10");

        for (uint256 i = 0; i < accounts.length; i++) {
            require(_isExcluded[accounts[i]], "TOKEN: Account is already included");
    ...
    function collectFee(
        ...
        address to,
        uint256 amount,
        uint256 rate) private returns (uint256) {
        uint256 transferAmount = amount;
        //This 2% fee is applicable when the  contract is going to reward to the token holders.
         if(redistributionFee!= 0 && (to == UniswapV2Pair|| enableAllFee)){
        ...
        if (liquidityFee != 0 && (to == UniswapV2Pair|| enableAllFee)) {
        ...
        if (burnFee != 0 &&( to == UniswapV2Pair|| enableAllFee )) {
    ...
    function setAcquisitionsWallet(address wallet) external onlyOwnerAndAdmin {
        require(wallet != address(0),"Invalid Address, Address should not be zero");
        acquisitionsWallet = wallet;
    }

    function setAdvisorsAndPartnersWallet(address wallet) external onlyOwnerAndAdmin {
        require(wallet != address(0),"Invalid Address, Address should not be zero");
        advisorsAndPartnersWallet = wallet;
    }
    ...
    function setPublicSaleAddress(address wallet) external onlyOwnerAndAdmin {
        require(wallet != address(0),"Invalid Address, Address should not be zero");
        publicSaleAddress = wallet;
    }

    function setPrivateSaleAddress(address wallet) external onlyOwnerAndAdmin {
        require(wallet != address(0),"Invalid Address, Address should not be zero");
        privateSaleAddress = wallet;
    }

    function setSeedAddress(address wallet) external onlyOwnerAndAdmin {
        require(wallet != address(0),"Invalid Address, Address should not be zero");
        seedAddress = wallet;
    }

    function setPreSeedAddress(address wallet) external onlyOwnerAndAdmin {
        require(wallet != address(0),"Invalid Address, Address should not be zero");
        preSeedAddress = wallet;
    }


    function setAllTeamWalletAddress(address _teamWallet,address _developmentWallet,address _acquisitionsWallet,address _advisorsAndPartnersWallet,address _preSeedAddress,address _seedAddress,address _privateSaleAddress,address _publicSaleAddress,address _marketingWallet) public onlyOwnerAndAdmin {
        require(_teamWallet != address(0) && _developmentWallet != address(0) && _acquisitionsWallet != address(0) && _advisorsAndPartnersWallet != address(0) && _preSeedAddress != address(0) && _seedAddress != address(0) && _privateSaleAddress != address(0) && _publicSaleAddress != address(0) && _marketingWallet != address(0) ,"Invalid Address, Address should not be zero");

        preSeedAddress = _preSeedAddress;
        seedAddress = _seedAddress;
        privateSaleAddress = _privateSaleAddress;
        publicSaleAddress = _publicSaleAddress;
        marketingWallet = _marketingWallet;
        advisorsAndPartnersWallet = _advisorsAndPartnersWallet;
    ...
    function removeERC20() public onlyOwnerAndAdmin {
        IERC20(address(this)).transfer(_msgSender(),IERC20(address(this)).balanceOf(address(this)));
    }
}
