...
// This function allows a user to stake one or more NFT tokens
function stake(uint256[] calldata _tokenIds) external nonReentrant {
    ...
    for (uint256 i = 0; i < _tokenIds.length; i++) {
        ...
        require(minLockTime == 0 || block.timestamp >= minLockTime, "Token is still locked");
        ...
        if (staker.lockTime == 0 || staker.lockTime > block.timestamp + LOCK_TIME) {
