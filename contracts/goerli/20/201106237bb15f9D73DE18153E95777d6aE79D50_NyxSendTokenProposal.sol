...
import "./interfaces/INyxSendTokenProposal.sol";

contract NyxSendTokenProposal is NyxProposal, INyxSendTokenProposal {
    string public constant name = "NyxSendTokenProposal";
        ...
        returns (SendTokenProposalParams memory)
    {
        ...
        return SendTokenProposalParams(InvestmentType(bytesToUint(params[0])),
            bytesToString(params[1]),
            bytesToUint(params[2]),
            payable(bytesToAddress(params[3])),
            bytesToAddress(params[4]));
    }
        ...
        returns (SendTokenProposalParams memory)
    {
        ...
        {
            SendTokenProposalParams memory nullProposal;
            return nullProposal;
        }
    }
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        SendTokenProposalParams memory params = parseProposalParams(proposalBytes);
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            SendTokenProposalParams memory params = parseProposalParams(proposalBytesArr[idx]);
...
// @title INyxSendTokenProposal for Nyx DAO
// @author TheV
...
interface INyxSendTokenProposal {
    // Enums
    ////////////////////
    ...
    function parseProposalParams(bytes[] memory params) external pure returns (SendTokenProposalParams memory);
    function parseProposalParams(bytes memory params) external pure returns (SendTokenProposalParams memory);
    function proposalToBytes(SendTokenProposal memory proposal) external pure returns (bytes memory);
