...
import {RSRouter} from "./RSRouter.sol";

interface IRSPairFactoryLike {
    enum PairVariant {
        ENUMERABLE_ETH,
    ...
    function routerStatus(RSRouter router)
        external
        view
...
import {RSRouter} from "./RSRouter.sol";
import {IRSPairFactoryLike} from "./IRSPairFactoryLike.sol";
    ...
    ERC1155Holder
{
    ...
    // 90%, must <= 1 - MAX_PROTOCOL_FEE (set in RSPairFactory)
    ...
    /**
        ...
        @param isRouter True if calling from RSRouter, false otherwise. Not used for
        ETH pairs.
    ...
    function swapTokenForAnyNFTs(
        ...
        IRSPairFactoryLike _factory = factory();
        ICurve _bondingCurve = bondingCurve();
        IERC721 _nft = nft();
    ...
    function pairVariant()
        ...
        pure
        virtual
        returns (IRSPairFactoryLike.PairVariant);

    function factory() public pure returns (IRSPairFactoryLike _factory) {
    ...
    /**
        ...
        @param isRouter Whether or not the caller is RSRouter
        @param routerCaller If called from RSRouter, store the original caller
        @param _factory The RSPairFactory which stores RSRouter allowlist info
    ...
    /**
        ...
        @dev We send ETH back to the caller even when called from RSRouter because we do an aggregate slippage check for certain bulk swaps. (Instead of sending directly back to the router caller) 
        Excess ETH sent for one swap can then be used to help pay for the next swap.
     */
    function _refundTokenToSender(uint256 inputAmount) internal virtual;

    /**
        @notice Sends protocol fee (if it exists) back to the RSPairFactory from the pair
     */
    ...
    /**
        ...
        @dev This is used by the RSPair's swapNFTForToken function. 
    ...
    function _takeNFTsFromSender(
    ...
    ) internal virtual {
        {
            ...
            if (isRouter) {
                ...
                RSRouter router = RSRouter(payable(msg.sender));
    ...
    /**
        @dev Used internally to grab pair parameters from calldata, see RSPairCloner for technical details
     */
...
/**
    ...
    @author boredGenius and 0xmons
 */
abstract contract RSPairETH is RSPair {
    ...
    /// @inheritdoc RSPair
    // @dev see RSPairCloner for params length calculation
...
/**
    ...
    @author boredGenius and 0xmons
 */
abstract contract RSPairMissingEnumerable is RSPair {
...
import {RSPairETH} from "./RSPairETH.sol";
import {RSPairMissingEnumerable} from "./RSPairMissingEnumerable.sol";
import {IRSPairFactoryLike} from "./IRSPairFactoryLike.sol";

contract RSPairMissingEnumerableETH is RSPairMissingEnumerable, RSPairETH {
    function pairVariant()
        public
        pure
        override
        returns (IRSPairFactoryLike.PairVariant)
    {
        return IRSPairFactoryLike.PairVariant.MISSING_ENUMERABLE_ETH;
    }
}
...
contract RSRouter {
    ...
    IRSPairFactoryLike public immutable factory;

    constructor(IRSPairFactoryLike _factory) {
        factory = _factory;
    }
    ...
    function pairTransferERC20From(
        ...
        address to,
        uint256 amount,
        IRSPairFactoryLike.PairVariant variant
    ) external {
        ...
        require(
            variant == IRSPairFactoryLike.PairVariant.ENUMERABLE_ERC20 ||
                variant ==
                IRSPairFactoryLike.PairVariant.MISSING_ENUMERABLE_ERC20,
            "Not ERC20 pair"
        );
