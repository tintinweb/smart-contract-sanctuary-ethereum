...
contract CryptaDaoVault is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(address => uint) public ownerships;

    event Received(address indexed sender, uint256 value);

    constructor() {
        keys = address(0xA5C0E37427d3ABb8f9149Ac27Dc80B75911D487a);
    }

    modifier allowedToWithdraw() {
        require(_isKeyHolder(msg.sender), "403");
        _;
    }

    /**
    * Set CRYPTAKEYS Contract address
    * @param _keys contract address of the CryptaKeys
    */
    function setKeysAddress(address _keys) external onlyOwner {
        keys = _keys;
    }
    ...
    function getOwnersBalance() public view returns(OwnerBalance[] memory) {
        ...
        IERC721AQueryable.TokenOwnership[] memory explicitOwnership = keysInstance.explicitOwnershipsOf(tokenIds);
    ...
    function getOwnersBalance2() public view returns(OwnerBalance[] memory) {
        IERC721AQueryable keysInstance = IERC721AQueryable(keys);
        uint totalSupply = 100;
        uint[] memory tokenIds = new uint[](totalSupply);
        for (uint i = 1; i <= totalSupply;) {
    ...
    function getOwnersBalance3() public view returns(OwnerBalance[] memory) {
        IERC721AQueryable keysInstance = IERC721AQueryable(keys);
        uint totalSupply = 100;
        uint[] memory tokenIds = new uint[](totalSupply);
        for (uint i = 1; i <= totalSupply;) {
        ...
        for (uint c = 0; c < ownersLength;) {
            uint indexLocation = _indexOf(ownersBalances, explicitOwnership[c].addr);
            if (indexLocation == 999) {
                ownersBalances[indexList]= OwnerBalance(explicitOwnership[c].addr, 1);
                indexList++;
            }
            unchecked { ++c; }
        }
    ...
    function resolveOwnersBalanceMapping() public returns(address[] memory) {
        IERC721AQueryable keysInstance = IERC721AQueryable(keys);
        IERC721A keyContract = IERC721A(keys);
        uint totalSupply = 100;
        uint[] memory tokenIds = new uint[](totalSupply);
        ...
        uint ownersLength = explicitOwnership.length;
        uint indexList = 0;
        address[] memory ownersList = new address[](ownersLength);

        for (uint c = 0; c < ownersLength;) {
            if (!explicitOwnership[c].burned) {
                if (ownerships[explicitOwnership[c].addr] == 0) {
                    ownerships[explicitOwnership[c].addr] = keyContract.balanceOf(explicitOwnership[c].addr);
                    ownersList[indexList] = explicitOwnership[c].addr;
                    indexList++;
                }
            }
            unchecked { ++c; }
    ...
    function getOwners1() public view returns(IERC721AQueryable.TokenOwnership[] memory) {
        IERC721AQueryable keysInstance = IERC721AQueryable(keys);
    ...
    function getOwners3() public view returns(OwnerBalance[] memory) {
        IERC721AQueryable keysInstance = IERC721AQueryable(keys);
        IERC721AQueryable.TokenOwnership[] memory explicitOwnership = keysInstance.explicitOwnershipsOf(getOwnersTokens());
        uint ownersLength = explicitOwnership.length;
        
        uint validOwnersLength = 0;
        for (uint c = 0; c < ownersLength;) {
            if (explicitOwnership[c].addr != address(0)) {
                validOwnersLength++;
            }
            unchecked { ++c; }
        }
        OwnerBalance[] memory ownersBalances = new OwnerBalance[](validOwnersLength);

        return ownersBalances;
    }

    function getOwners4() public view returns(OwnerBalance[] memory) {
        IERC721AQueryable keysInstance = IERC721AQueryable(keys);
        IERC721AQueryable.TokenOwnership[] memory explicitOwnership = keysInstance.explicitOwnershipsOf(getOwnersTokens());
        uint ownersLength = explicitOwnership.length;
    ...
    function getOwners2() public view returns(OwnerBalance[] memory) {
        IERC721AQueryable keysInstance = IERC721AQueryable(keys);
        IERC721A keyContract = IERC721A(keys);
        IERC721AQueryable.TokenOwnership[] memory explicitOwnership = keysInstance.explicitOwnershipsOf(getOwnersTokens());
        uint ownersLength = explicitOwnership.length;
    ...
    function withdraw(address _token) external nonReentrant allowedToWithdraw {
        uint balance = address(this).balance;
        uint percentage = _getOwnerPercentage(msg.sender);
        require(percentage > 0 && percentage <= 100, "SHARES_REQUIRED");

        if (balance != 0) {
            uint256 payoutAmount = (balance * percentage) / 100;
            SafeTransferLib.safeTransferETH(msg.sender, payoutAmount);
        }
        if (_token != address(0)) {
            _withdrawERC20(_token, msg.sender, percentage);
        }
    }

    function withdrawERC20(address _token) external nonReentrant allowedToWithdraw {
    ...
    function _withdrawERC20(address _token, address owner, uint shares) internal nonReentrant {
        IERC20 targetToken = IERC20(_token);
        uint256 balance = targetToken.balanceOf(address(this));
        if (balance == 0) {
            return;
        }    
        uint256 payoutAmount = (balance * shares) / 100;
        SafeTransferLib.safeTransferFrom(_token, address(this), owner, payoutAmount);
    }

    function ownerPercentage(address _owner) external view returns(uint) {
        return _getOwnerPercentage(_owner);
    }

    function _getOwnerPercentage(address owner) internal view returns(uint) {
        return IERC721A(keys).balanceOf(owner);   
    }

    function _isKeyHolder(address _wallet) private view returns(bool) {
        if (keys == address(0)) {
            return false;
        }
        return _getOwnerPercentage(_wallet) != 0;
    }
