...
interface IPancakeV3PoolWithLmPool is IPancakeV3Pool {
    function setLmPool(IPancakeV3LmPool lmPool) external;
}

/// @title Pancakeswap factory owner
/// @notice Manages ownership and control over factory and pools
contract PancakeV3FactoryOwner {
    ...
    function setLmPoolDeployer(address _lmPoolDeployer) external onlyOwner {
        require(lmPoolDeployer == address(0), 'already initialized');
    ...
    function setFeeProtocol(IPancakeV3Pool pool, uint8 feeProtocol0, uint8 feeProtocol1) external onlyOwner {
    ...
    function setLmPool(address pool, address lmPool) external onlyOwnerOrLmPoolDeployer {
        IPancakeV3PoolWithLmPool(pool).setLmPool(IPancakeV3LmPool(lmPool));
    }
}