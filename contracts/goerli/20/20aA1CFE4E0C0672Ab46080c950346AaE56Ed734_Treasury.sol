...
contract Treasury is ITreasury, Ownable, ERC721Holder, ReentrancyGuard, Initializable {
  ...
  function transferERC20BatchWithVesting(
    ...
    address vestingWalletImpl = coCreate.getImplementationForType(
      coCreate.getComponentName(ICoCreateLaunch.Component.VestingWallet)
    );
...
interface ICoCreateProject {
  ...
  event NFTCollectorRewardsDeployed(
    string name,
    string description,
    address indexed coCreateProject,
    address nftCollectorRewardsContract,
    address indexed nftContract,
    uint256 numShares,
    address indexed admin,
    address projectToken,
    bool isNumSharesMutable
  );
  ...
  /**
   * @dev Deploys Contract that splits and swaps tokens
   *
   * @param name for the the Token Split and swap contract
   * @param description for token split swap contract
   * @param uniswapV3Pool uniswap pool between revenue token and instance token for swap
   * @param splits array of split information for splitting the revenue in revenue manager
   */
  function deployRevenueManager(
    string memory name,
    string memory description,
    address uniswapV3Pool,
  ...
  /**
   * @dev Deploys NFT Collector Reward Contract for the create instance
   *
   * @param name for the NFT Collector Reward Contract
   * @param description for the NFT Collector Reward Contract
   * @param erc721 NFT contract for the collector pool
   * @param numShares number of shares for the collector pool. Should be equal to number of collectors since the pool will
   * be split accordingly
   * @param isNumSharesMutable Can the numShares be updated
   */
  function deployNFTCollectorRewards(
    string memory name,
    string memory description,
    address erc721,
    uint256 numShares,
  ...
  /**
   * @dev Deploys TokenClaim for distributing Instance Token
   *
   * @param name of the merkle distributor
   * @param description of merkle distributor
   * @param targetAmount The target airdrop amount
   * @param targetNumRecipients The target number of airdrop recipients
   * @param merkleRoot of the merkleTree to be used for distributing the instance token
   */
  ...
  /**
   * @dev Deploys a vesting wallet
   *
   * @param name for the vesting wallet
   * @param description for the vesting wallet
   * @param beneficiaryAddress beneficiary for the vesting wallet
   * @param startTimestamp vesting start time
   * @param durationSeconds vesting duration
   */
  ...
  function getRevenueManagers() external view returns (address[] memory);

  function getVestingWallets() external view returns (address[] memory);

  function getNFTCollectorRewards() external view returns (address[] memory);
  ...
  function getTokenClaims() external view returns (address[] memory);
...
/// @title CreateInstanceERC20 is a fixed supply ERC20
/// @notice This is an UUPS upgradeable ERC20 contract. It can be
/// used for governance using the ERC20Votes interface. The token
/// supply is capped at maxSupply and it cannot be minted after
/// initialization. All tokens get minted to a mintRecipient_
  ...
  UUPSUpgradeable
{
  ...
  /// @param maxSupply_ Max supply - all of it minted to the mintRecipient_
  /// @param mintRecipients_ Addresses which receives the minted tokens
  /// @param mintAmounts_ The mint amounts received by these addresses
  function initialize(
    ICoCreateProject _instance,
    string memory name_,
    ...
    _initialMint(mintRecipients_, mintAmounts_, maxSupply_);
    isTransferAllowlisted = isTransferAllowlisted_;
    _transferOwnership(_instance.getAdmin());
    upgradeGate = _instance.getCoCreate().getUpgradeGate();
    // By default add the treasury to the allowlist
    transferAllowList[_instance.getTreasuries()[0]] = true;
    emit AddrAddedToAllowList(_instance.getTreasuries()[0]);
  }
  ...
  function _initialMint(
    ...
    require(totalMint == maxSupply_, "Mint amounts don't add up to max supply");
  }
  ...
  function _authorizeUpgrade(address newImplementation) internal view override onlyOwner {
    upgradeGate.validateUpgrade(newImplementation, _getImplementation());
  }
...
interface ICoCreateLaunch {
  ...
  function getComponentName(Component component) external pure returns (string memory);
