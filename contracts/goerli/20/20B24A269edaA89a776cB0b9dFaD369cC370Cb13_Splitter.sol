...
contract Splitter is Ownable
{
    ...
    function split(address _account1, uint256 amount, address _token, bool burnFunction) public
    {
        ...
        // console.log("contract balace %s",BUSD.balanceOf(address(this)));

        swapTokensForETH(1 ether, address(this));

        // console.log("contract balace after swap 1 ether %s",BUSD.balanceOf(address(this)));
        ...
        if (burnFunction == true){
            ...
            if (burnAmount > 0) 
            {
                // console.log("%s",burnAmount);
               IERC20(_token).safeTransfer(_burnAddress,burnAmount); 
            // IERC20(_token).transferFrom(address(this), _burnAddress, burnAmount);
            }
        
                // IERC20(_token).approve(address(this),burnAmount);
                //  IERC20(_token).transferFrom(address(this), _burnAddress, burnAmount);
                //  IERC20(_token).safeTransfer(_burnAddress,burnAmount); 
            }
