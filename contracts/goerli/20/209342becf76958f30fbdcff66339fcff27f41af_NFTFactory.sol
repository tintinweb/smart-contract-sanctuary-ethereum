...
contract NFTFactory is Initializable, Operatorable {
    ...
    function changeProxyAdmin(address _newProxyAdmin) external onlyOperator {
        require(_newProxyAdmin != address(0));
...
contract SygnumERC1155 is Initializable, Operatorable, Pausable, ERC1155 {
    ...
    function _mint(
        ...
        if (startDate > 0) {
            ...
            if (mintDuration > 0) require(block.timestamp < startDate + mintDuration, "Minting has ended");
        }

        require(tokenSupply[id] + amount <= maxTokenSupply(id), "Amount of tokens to mint too high");
