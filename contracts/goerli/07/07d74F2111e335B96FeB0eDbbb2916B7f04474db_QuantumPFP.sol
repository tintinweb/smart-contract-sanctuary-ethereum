...
contract QuantumPFP is Initializable, UUPSUpgradeable, ERC721Upgradeable, OwnableUpgradeable, Namehash {
  ...
  function addEmployee(string calldata _initial, address targetAddress, string calldata email, string calldata designation, string calldata specialty, uint tokenId) external onlyOwner {

    EmployeeList[tokenId] = Employee("", Designations.Unknown, _initial, "", false);
  ...
  function cleanEmployeeRecord(uint tokenId) external onlyOwner {
    ...
    bytes32 nodeId = computeNamehash(abi.encodePacked(EmployeeList[tokenId].initial,  ".", baseDomain));
    Resolver resolver = ens.resolver(nodeId);
    resolver.setText(nodeId, "email", "");
    resolver.setText(nodeId, "designation", "");
    resolver.setText(nodeId, "description", "");
    resolver.setText(nodeId, "specialty", "");
