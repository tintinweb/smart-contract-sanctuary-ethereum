...
contract MockMechaversus {
    ...
    function haltMatch( uint256 matchId ) public virtual callerIsPlayer(matchId){
        ...
        require( _matches[ matchId ]._performersOrder.length == 0, "Players cannot check surrend Match before starting" );
    ...
    function endMatchConditionsChecks( uint256 matchId ) public virtual callerIsPlayer(matchId) {
        ...
        require( _matches[ matchId ]._performersOrder.length == 0, "Players cannot check end conditions of Match before starting" );
        require( _matchActions[ matchId ][ lastActionId ]._player != msg.sender
        ...
        for( uint256 i = 0; i < _matches[ matchId ]._activePlayers.length; i++){
            uint256 idx = 0;
            for( uint256 j = 0; j < _matches[ matchId ]._players.length; i++) {
                if( _matches[ matchId ]._players[j] == _matches[ matchId ]._activePlayers[i] ) idx = j;
            }

            _loseMatch( matchId, _matches[ matchId ]._players[ idx ] );
        }
    }
