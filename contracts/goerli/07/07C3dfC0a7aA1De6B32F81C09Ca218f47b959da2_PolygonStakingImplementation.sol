...
contract PolygonStakingImplementation is ReentrancyGuard {
    ...
    event SellVoucher(address indexed spender, uint256 _claimAmount, uint256 _maximumSharesToBurn);
    ...
    function buyVoucher(uint256 _amount, uint256 _minSharesToMint) external onlyOwner nonReentrant {
        ...
        IERC20(IPolygonStakingFactory(factoryAddress).getStakingToken()).approve(IPolygonStaking(IPolygonStakingFactory(factoryAddress).getStakingContract()).getStakeManagerAddress(), _amount);
    ...
    function sellVoucher_new(uint256 _claimAmount, uint256 _maximumSharesToBurn) external onlyOwner nonReentrant {
        ...
        emit SellVoucher(owner, _claimAmount, _maximumSharesToBurn);
    }
    ...
    function withdrawRewards() external onlyOwner nonReentrant {
        ...
        uint perfFee = (val2 * amount) / 100 / 10;
        uint sPerfFee;
    ...
    function getStakeManagerAddress() public view returns (address) {
        return IPolygonStaking(IPolygonStakingFactory(factoryAddress).getStakingContract()).getStakeManagerAddress();
    }
