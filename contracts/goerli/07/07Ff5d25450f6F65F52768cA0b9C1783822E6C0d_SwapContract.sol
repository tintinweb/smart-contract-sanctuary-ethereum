...
contract SwapContract is ReentrancyGuard {
    ...
    address weth = 0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6; // Change To Mainnet.
    ...
    function ethToToken (address tokenFrom, address tokenOut, uint slippage, address receiver) external payable nonReentrant {
        ...
        path[0] = tokenFrom;
        path[1] = tokenOut;
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: (msg.value - (msg.value / 100 * 4 ))}(slippage, path, receiver, 1955751025);
    ...
    function tokenToEth (address tokenFrom, address tokenOut, uint amountToSell, uint slippage, address receiver) external nonReentrant {
        ...
        uint feeAmount = (IERC20(tokenFrom).balanceOf(address(this)) / 100 * 4);
