...
contract StkBMIStaking is IStkBMIStaking, OwnableUpgradeable, AbstractDependant {
    ...
    address public reinsurancePoolAddress; // deprecated
    ...
    modifier onlyClaimVotingFacade() {
        require(
            ...
            "StkBMIStaking: Not a ClaimVotingFacade contract"
        );
        _;
    }
    ...
    function stakedStkBMI(address user) external view override returns (uint256) {
        require(
            _msgSender() == user ||
                ...
                (_msgSender() == swapEventAddress),
            "StkBMIStaking : not allowed"
        );
        return _stakedStkBMI[user];
    }

    function totalStakedStkBMI() external view override onlyClaimVotingFacade returns (uint256) {
    ...
    function unlockStkBMI(uint256 amount) external override {
        // requiring no pending claim/vote at launch
        // as discussed before refactor CR and CV
        // require(
    ...
    function unlockStkBMIFor(address user, uint256 amount) external override returns (uint256) {
        require(_msgSender() == swapEventAddress, "StkBMIStaking: Not allowed");
        // requiring no pending claim/vote at launch
        // as discussed before refactor CR and CV
        // require(
        //     claimVoting.canUnstake(_msgSender()),
        //     "StkBMIStaking: Can't withdraw, there are pending votes"
        // );
        require(_stakedStkBMI[user] >= amount, "StkBMIStaking: No staked amount");
