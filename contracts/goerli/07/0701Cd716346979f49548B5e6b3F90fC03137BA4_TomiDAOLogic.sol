...
contract TomiDAOLogic is Initializable, TomiDAOStorageV1, TomiDAOEvents {
    ...
    /**
     * @notice Gets whether or not NFTs have atleast 1 NFT that has not voted on a given proposal
     */
    ...
    function _setDeepProposalCriteria(string memory signature, address target, Criteria memory criteria) external {
        require(msg.sender == admin || msg.sender == pendingAdmin, "TomiDAO::_setDeepProposalCriteria: admin only");
    ...
    function _setShallowProposalCriteria(string memory signature, Criteria memory criteria) external {
        require(msg.sender == admin || msg.sender == pendingAdmin, "TomiDAO::_setShallowProposalCriteria: admin only");
    ...
    function _setGeneralProposalCriteria(Criteria memory criteria) external {
        require(msg.sender == admin || msg.sender == pendingAdmin, "TomiDAO::_setGeneralProposalCriteria: admin only");
    ...
    function setAdmin(address admin_) external {
        ...
        require(msg.sender == pendingAdmin && admin_ != address(0), 'TomiDAO::setAdmin: pending admin only');
