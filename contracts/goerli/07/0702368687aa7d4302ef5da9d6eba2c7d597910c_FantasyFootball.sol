...
contract FantasyFootball {
  ...
  mapping(uint256 => uint256) public playersToPoints;  
  ...
  constructor() {
    owner = msg.sender;
    token = 0xd0A2e008eb1f160fa6055A28D5224034d8f527F7;
  }
  ...
  function depositToken(uint256 amount) public payable {
    ...
    require(amount >= PARTICIPATION_FEE, "Amount must be greater than or equal to the participation feee");

    tokenContract.transferFrom(msg.sender, address(this), amount);
  }

  function submitTeam(uint256[] memory _players) public {
    
    require(balances[msg.sender] >= PARTICIPATION_FEE, "You must deposit tokens to participate");
    require(_players.length == 11, "Team must have 11 players and 4 substitutes");
    require(teams[msg.sender].length == 0, "You already have a team");

    uint256[] memory oldTeam = teams[msg.sender];

    if (oldTeam.length != 0) {
      uint256 previousPoints = getTeamPoints(oldTeam);
      addressToPoints[msg.sender] += previousPoints;
    }
  ...
  function getTeamPoints(uint256[] memory team) public view returns (uint256) {
  ...
  function setPlayerData(uint256[] memory _ids, uint256[] memory _points) public onlyOwner {
  ...
  function setSinglePlayerData(uint256 _id, uint256 _points) public onlyOwner {
    playersToPoints[_id] = _points;
  }

  function widthrdawTo(uint256 amount) public onlyOwner {
  ...
  function withdrawToAll(uint256[] memory amounts, address[] memory participants) public onlyOwner {
    ...
    for (uint256 i = 0; i < participants.length; i++) {
      ERC20(token).transferFrom(address(this), participants[i], amounts[i]);
    }
  }
}