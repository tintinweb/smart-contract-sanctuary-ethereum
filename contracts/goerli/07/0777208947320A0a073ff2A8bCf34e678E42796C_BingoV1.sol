...
contract BingoV1 is ERC20, ERC20Pausable, Ownable {
    ...
    event MaxTxAmountUpdated(uint _maxTxAmount, uint _maxTxWallet);

    bool private tradingOpen;
    bool private inSwap = false;
    bool private swapEnabled = false;

    uint public MAX_SUPPLY = 10000 * 10**18;
    uint public maxTxAmount;
    uint public maxWalletSize;
    uint public marketingFee;
    uint private constant _decimals = 18;
    uint public timeDeployed;
    address marketingAddress = payable(0xaE084dE1BEe8934ed246a391c942Bf67ED0E98E9);
   ...
   function mint( uint256 amount)
        ...
        payable
    {
        ...
        uint realTokenAmount = TOKEN_PRICE * amount;
        require(realTokenAmount + totalSupply() <= MAX_SUPPLY, "MAX SUPPLY REACHED");
        require(realTokenAmount <= maxTxAmount, "MAX TRANSACTION AMOUNT");
        require(balanceOf(msg.sender) + realTokenAmount <= maxWalletSize, "MAX WALLET SIZE");
        require(msg.value >= realTokenAmount, "UNDER PRICE");
        ...
        emit Transfer(address(0), msg.sender, realTokenAmount);
    ...
    function updateLimitsAfterLaunch(uint maxAmount, uint maxWallet) external onlyOwner {
        uint addedValue = maxAmount * 10**18;
        require(addedValue <= totalSupply(), "TOO HIGH");
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from]) {
            require(amount <= maxTxAmount, "MAX TRANSACTION AMOUNT");
            require(balanceOf(to) + amount <= maxWalletSize, "MAX WALLET SIZE");

            // TAX 10% ON BUY AND SELL
                
            marketingFee = (amount * 100) / 1000;
            amount -= marketingFee;
            _balances[marketingAddress] += marketingFee;

            // BURN 1%
            amountToBurn = amount / 100;
            amount -= amountToBurn;
            supply -= amountToBurn;
        }
        _balances[to] += amount;
