...
contract LearnETH is ERC721, Ownable {
    ...
    uint256 monthDuration = 900;
	uint256 monthDuration1 = 120;
    ...
    function registerAsPartner(uint256 _referrerTokenId, uint256 _months) public payable {
        ...
        //Students and Partners can add new partners
        require(nodeDetails[_referrerTokenId].nodeType == 0 || nodeDetails[_referrerTokenId].nodeType >= 2,"Incorrect Referrer Token Id");
        //require(msg.value == partnerFee,"Incorrect value");

        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);
        (uint256 _level, uint256 _position) = getNextNode(_referrerTokenId);
        uint256 f_position;
        ...
        nodeDetails[tokenId].subscribedTill = block.timestamp.add(monthDuration);
        partnerNodeTokenMapping[_level][_position] = tokenId;
        nodeDetails[_referrerTokenId].totalRecruits+=1;

        emit partnerAdded(tokenId, _referrerTokenId, _father1, _level, _position);
    ...
    function upgradeInstructorAsPartner(uint256 _instructorTokenId, uint256 _months) public payable {
        ...
        uint256 _referrerTokenId = nodeDetails[_instructorTokenId].personalReferrer;
    ...
    function upgradeStudentAsPartner(uint256 _tokenId, uint256 _months) public payable {
        ...
        uint256 currentFather1 = nodeDetails[_tokenId].father1;

        (uint256 _level, uint256 _position) = getNextNode(currentFather1);
    ...
    function getRefund(uint256 _tokenId, uint256 _paymentId) public {
        ...
        require(block.timestamp.sub(monthDuration1) > paymentDetails[_paymentId].paidAt, "Refund eligibility duration lapsed");
    ...
    function upgradeStudentAsInstructor(uint256 _studentTokenId) public {
        ...
        uint256 _personalReferrer = nodeDetails[_studentTokenId].personalReferrer;
    ...
    function unlockPayment(uint256 _paymentId) external {
        ...
        require(paymentDetails[_paymentId].paidAt.add(monthDuration1) < block.timestamp,"Still in refund eligibility window");
        ...
        distributeUplineFee(studentTokenId, studentUplineFee);
        distributeUplineFee(instructorTokenId, instructorUplineFee);
    ...
    function distributePartnerCourseBalance(uint256 _tokenId, uint256 _partnerCourseId) external {
        ...
        distributeUplineFee(_tokenId, partnerCourseDetails[_partnerCourseId].balance.mul(25).div(100));
    ...
    function distributePartnerCourseFee(uint256 _tokenId, uint256 _months, uint256 _amount) internal {
        ...
        for(uint i=0; i< _months; i++) {
            nodeDetails[_tokenId].subscribedTill = nodeDetails[_tokenId].subscribedTill.add(monthDuration);
        ...
        distributeUplineFee(_tokenId, twelth.mul(9));
    }

    function distributeUplineFee(uint256 _tokenId, uint256 _amount) public {
