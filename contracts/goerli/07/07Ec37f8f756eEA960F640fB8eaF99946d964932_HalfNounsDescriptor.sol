...
/// @title The HalfNouns NFT descriptor
...
import { IHalfNounsDescriptor } from './interfaces/IHalfNounsDescriptor.sol';
import { IHalfNounsSeeder } from './interfaces/IHalfNounsSeeder.sol';
import { NFTDescriptor } from './libs/NFTDescriptor.sol';
import { MultiPartRLEToSVG } from './libs/MultiPartRLEToSVG.sol';

contract HalfNounsDescriptor is IHalfNounsDescriptor, Ownable {
    ...
    // Whether or not new HalfNoun parts can be added
    ...
    // HalfNoun Color Palettes (Index => Hex Colors)
    mapping(uint8 => string[]) public override palettes;

    // HalfNoun Backgrounds (Hex Colors)
    string[] public override backgrounds;

    // HalfNoun Bodies (Custom RLE)
    bytes[] public override bodies;

    // HalfNoun Accessories (Custom RLE)
    bytes[] public override accessories;

    // HalfNoun Heads (Custom RLE)
    bytes[] public override heads;

    // HalfNoun Glasses (Custom RLE)
    ...
    /**
     * @notice Get the number of available HalfNoun `backgrounds`.
     */
    ...
    /**
     * @notice Get the number of available HalfNoun `bodies`.
     */
    ...
    /**
     * @notice Get the number of available HalfNoun `accessories`.
     */
    ...
    /**
     * @notice Get the number of available HalfNoun `heads`.
     */
    ...
    /**
     * @notice Get the number of available HalfNoun `glasses`.
     */
    ...
    /**
     * @notice Batch add HalfNoun backgrounds.
    ...
    /**
     * @notice Batch add HalfNoun bodies.
     * @dev This function can only be called by the owner when not locked.
     */
    ...
    /**
     * @notice Batch add HalfNoun accessories.
     * @dev This function can only be called by the owner when not locked.
     */
    ...
    /**
     * @notice Batch add HalfNoun heads.
     * @dev This function can only be called by the owner when not locked.
     */
    ...
    /**
     * @notice Batch add HalfNoun glasses.
     * @dev This function can only be called by the owner when not locked.
     */
    ...
    /**
     * @notice Add a HalfNoun background.
     * @dev This function can only be called by the owner when not locked.
     */
    ...
    /**
     * @notice Add a HalfNoun accessory.
     * @dev This function can only be called by the owner when not locked.
     */
    ...
    /**
     * @notice Lock all HalfNoun parts.
    ...
    /**
     * @notice Given a token ID and seed, construct a token URI for an official HalfNouns DAO halfNoun.
     * @dev The returned value may be a base64 encoded data URI or an API URL.
     */
    function tokenURI(uint256 tokenId, IHalfNounsSeeder.Seed memory seed) external view override returns (string memory) {
        if (isDataURIEnabled) {
    ...
    /**
     * @notice Given a token ID and seed, construct a base64 encoded data URI for an official HalfNouns DAO halfNoun.
     */
    function dataURI(uint256 tokenId, IHalfNounsSeeder.Seed memory seed) public view override returns (string memory) {
        string memory halfNounId = tokenId.toString();
        string memory name = string(abi.encodePacked('HalfNoun ', halfNounId));
        string memory description = string(abi.encodePacked('HalfNoun ', halfNounId, ' is a member of the HalfNouns DAO'));
     ...
     */
    function generateSVGImage(IHalfNounsSeeder.Seed memory seed) external view override returns (string memory) {
    ...
    /**
     * @notice Get all HalfNoun parts for the passed `seed`.
     */
    function _getPartsForSeed(IHalfNounsSeeder.Seed memory seed) internal view returns (bytes[] memory) {
