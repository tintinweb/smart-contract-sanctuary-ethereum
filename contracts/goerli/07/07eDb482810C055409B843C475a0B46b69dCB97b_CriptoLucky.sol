...
contract CriptoLucky is VRFConsumerBaseV2 {
  ...
  constructor(uint64 subscriptionId,address _tokenReward) VRFConsumerBaseV2(vrfCoordinator) {
  ...
  function createLottery(uint _cost,uint _maxTicketsPlayer,uint32 _Nwinners,uint _maxTickets,uint _percentage)public onlyOwner{
  ...
  function join(uint tickets)public payable{
    ...
    require(_ActualLottery.NTicketsOwner[msg.sender]+tickets<= _ActualLottery.maxTicketsPlayer,"you exceeded the limit of tickets of your account");
    ...
    for(uint i=_ActualLottery.totalTickets;i<_ActualLottery.totalTickets+tickets;i++){
      _ActualLottery.TicketOwner[i]=msg.sender;
    }
    _ActualLottery.totalTickets+=tickets;
    _ActualLottery.NTicketsOwner[msg.sender]+=tickets;
    ...
    tokenReward._mintMinerReward(msg.sender,ticketsRewarded);
  }
 ...
 function setWinners() public onlyOwner {
  ...
  function withdrawByWinner(uint _idGame)public{
    ...
    for(uint i=0;i<PastLotteries[_idGame].Nwinners;i++){
      if(msg.sender==PastLotteries[_idGame].winners[i] && !PastLotteries[_idGame].WithdrawWinners[i]){
        isWinner=true;
        place=i;
    ...
    (bool success,)=msg.sender.call{value:PastLotteries[_idGame].prize*(PercentageWinners[PastLotteries[_idGame].Nwinners][place+1])/100}("");
