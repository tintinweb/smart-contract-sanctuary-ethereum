...
contract DeviewsPool is Ownable, Init {
    ...
    ///@dev Proportion of support events 0：team 1:treasury 2:staker 3:burn 4:pool
    uint256[5] private sup;
    ...
    function updateProportion(
        ...
        sup[3] = burn;
        sup[4] = pool;
        emit UpdateProportion(sup[0], sup[1], sup[2], sup[3], sup[4]);
    }
    ...
    function rewardsMultisig(
        ...
        require(block.timestamp <= deadline, "Expired");
        require(vs.length == rs.length, "Unauthorized");
        require(ss.length == rs.length, "Unauthorized");
        require(ss.length == signerNumber, "Unauthorized");
    ...
    /**
     * Add bonus to a project to support it, trigger Support event, listen to the amount of bonus added
    ...
    function support(
        ...
        uint256 _total = (sup[0] + sup[1] + sup[2] + sup[3] + sup[4]);
        // _transfer(_msgSender(), contractAddress, amount);
        //the amount to the team
        uint256 ta = (amount * sup[0]) / _total;
        //the amount to the treasury
        uint256 tra = (amount * sup[1]) / _total;
        //the amount to staker
        uint256 sa = (amount * sup[2]) / _total;
        //the amount for burning
        uint256 ba = (amount * sup[3]) / _total;
        ...
        IERC20(token).transfer(_team, ta);
        IERC20(token).transfer(_treasury, tra);
        IERC20(token).transfer(_staker, sa);
        IERC20(token).transfer(address(0), ba);
        IERC20(token).transfer(address(this), poolAmount);
    ...
    function viewTeam() public view returns (address) {
        return _team;
    }

    function viewStaker() public view returns (address) {
        return _staker;
    }
    ...
    function recoverSigner(
        ...
        bytes32 _hash = keccak256(
            abi.encode(toAddress, value, deadline, addressNonce)
        );
        return ECDSA.recover(_hash, v, r, s);
    }

    ///@notice Get the nonce of the target address and nonce add 1
    ///@param toAddress target address
    ///@return nonce of the target address
    function _useNonce(address toAddress) internal returns (uint256) {
        uint256 _nonce = poolNonce[toAddress];
        poolNonce[toAddress] += 1;
        return _nonce;
    }
}
