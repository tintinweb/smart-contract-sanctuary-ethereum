...
contract HDGenerator is ERC721Upgradeable, OwnableUpgradeable {
     ...
     */
    function airdrop(address[] calldata _receivers) external {
        if (_receivers.length > maxSupply) revert NotEnoughTokensAvailable();
        for (uint256 i = 0; i < _receivers.length; i++) {
            _safeMint(_receivers[i], i + 1);
        }
    }
