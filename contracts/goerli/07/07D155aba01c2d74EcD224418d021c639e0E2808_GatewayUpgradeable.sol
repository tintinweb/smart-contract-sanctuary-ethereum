...
contract AssetVault is ReentrancyGuard {
    ...
    function deposit(uint256 amount, address caller) nonReentrant external  {
    ...
    function handleWithdraw(uint256 amount, address recipient) nonReentrant external {
        require(msg.sender == gateway, "!Gateway");
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setBridgeFees(
       ...
       function setVault(
        ...
        require(_vaultAddress != address(0),"empty address");
    ...
    function requestToRouter(
        ...
        require(msg.value >= requestToRouterDefaultFee, "insufficient fees sent");
        address txOrigin;
        assembly {
            txOrigin := origin()
        }
        ...
        if (routeAmount > 0){
            require(keccak256(abi.encodePacked(routeRecipient)) != keccak256(abi.encodePacked("")),"empty recipient");
        ...
        _requestToRouterEmitter(routeAmount,routeRecipient,payload,routerBridgeContract,gasLimit,feePayer,txOrigin);
       
        
        return eventNonce;
    }

function _requestToRouterEmitter(
     uint256 routeAmount,
        bytes memory routeRecipient, 
        bytes memory payload,
        string memory routerBridgeContract,
    ...
    function requestToDest(
        ...
        require(msg.value >= requestToDestDefaultFee, "insufficient fees sent");
...
interface IGateway {
    function requestToRouter(
        ...
        uint256 gasLimit,
        Utils.FeePayer feePayerEnum
    ) payable external returns (uint64);
