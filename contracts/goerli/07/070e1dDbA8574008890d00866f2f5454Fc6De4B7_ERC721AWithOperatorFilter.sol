...
import "./extensions/StudioRoyalties.sol";
import "../../TokenId.sol";
...
contract ERC721AWithOperatorFilter is
    StudioRoyalties,
    ...
    ICollection
{
    ...
    constructor(
        ...
        _payoutAddresses = [params.creator];
        _payoutSplits = [10000];
    ...
    /// @param newMaxSupply maximum items in the drop
    function setSupply(uint256 newMaxSupply) public onlyRole(MANAGER_ROLE) {
        _maxSupply = newMaxSupply;
    }

    /// @notice overrides the auto self payment royalty distribution to set address manually
    /// @param recipient address of the recipient
    function overrideRoyaltyRecipient(address recipient)
        public
        onlyRole(MANAGER_ROLE)
    {
        _overrideRoyaltyRecipient = recipient;
    }
    ...
    */
    ...
    function mint(
        ...
        if (_maxSupply == 0 || totalSupply() + 1 > _maxSupply)
            revert MintExceedsDropSupply();
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721ABase, StudioRoyalties, AccessControlEnumerable)
        returns (bool)
    {
        return
            ERC721ABase.supportsInterface(interfaceId) ||
            StudioRoyalties.supportsInterface(interfaceId) ||
    ...
    /**
     * @dev See {StudioRoyalties-_beforeSetSplits}.
     *
     * Requirements:
     *
     * - Can only be executed by manager role
     */
    function _beforeSetSplits(
        address[] calldata recipients,
        uint256[] calldata fees
    ) internal override onlyRole(MANAGER_ROLE) {
        super._beforeSetSplits(recipients, fees);
    }
...
error AddressAndSplitsMismatch();
error MissingSplitAmount(uint256 shortfall);
error SplitAmountsToLarge(uint256 surplus);
error NoBalanceToWithdraw(address addr);

/// @title Specialized ERC2981 implementation for Quantum Studio which implements royalty splits.
/// @notice To be used within Quantum, as it was written for its needs.
/// @author JCBDEV (Quantum Art)
abstract contract StudioRoyalties is IStudioRoyalties {
    using Address for address payable;
    event RoyaltyPaymentReceived(address from, uint256 amount);
    // event __Debug(uint256 amount);

    /// @dev one global fee for all royalties.
    uint256 internal _royaltyFee;
    /// @dev one global recipient for all royalties.
    address internal _overrideRoyaltyRecipient;

    address[] internal _payoutAddresses;
    uint256[] internal _payoutSplits;
    mapping(address => uint256) internal _payoutBalances;
    uint256 internal _unassignedBalance; // Used to store value if payment splits have not been configured yet
    ...
    {
        receiver = _overrideRoyaltyRecipient == address(0)
            ? address(this)
            : _overrideRoyaltyRecipient;
    ...
    {
        return
            interfaceId == type(IStudioRoyalties).interfaceId ||
            interfaceId == 0x01ffc9a7 || // ERC165 Interface ID for ERC165
    ...
    function setSplits(address[] calldata recipients, uint256[] calldata splits)
        ...
        virtual
    {
        _beforeSetSplits(recipients, splits);
        if (recipients.length != splits.length) {
            revert AddressAndSplitsMismatch();
        }
        uint256 totalFee;
        ...
        if (totalFee < 10000) {
            revert MissingSplitAmount(10000 - totalFee);
        } else if (totalFee > 10000) {
            revert SplitAmountsToLarge(totalFee - 10000);
        }
        delete _payoutAddresses;
        delete _payoutSplits;
        _payoutAddresses = recipients;
        _payoutSplits = splits;
        // Assign any balance received whilst payment splits were not configured
        for (uint256 i = 0; i < _payoutAddresses.length; i++) {
            uint256 amount = (_unassignedBalance * _payoutSplits[i]) / 10000;
            _payoutBalances[_payoutAddresses[i]] += amount;
        }
        _unassignedBalance = 0;
    }

    /**
     * @dev Hook executed before splits are set
     *      can be used to assert permission or logic etc.
     *
     */
    function _beforeSetSplits(
        address[] calldata recipients,
    ...
    receive() external payable virtual {
        ...
        // require(_payoutAddresses.length != 0, "Blonpy");
        if (_payoutAddresses.length == 0) {
            _unassignedBalance += msg.value;
            return;
        }
        for (uint256 i = 0; i < _payoutAddresses.length; i++) {
            uint256 amount = (msg.value * _payoutSplits[i]) / 10000;
            // emit __Debug(amount);
            _payoutBalances[_payoutAddresses[i]] += amount;
        }
    }

    function withdrawRoyalties() external virtual {
        uint256 amount = _payoutBalances[msg.sender];
        if (amount <= 0) revert NoBalanceToWithdraw(msg.sender);
        _payoutBalances[msg.sender] = 0;
