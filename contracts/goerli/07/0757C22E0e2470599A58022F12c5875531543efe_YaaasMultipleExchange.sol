    ...
    ERC1155Holder
{
    ...
    constructor(address loyaltyAddr){
        loyalty = ILoyalty(loyaltyAddr);
    }
    function addLoyaltyOffer(
        CreateLoyaltyOffer calldata loyaltyOffer
    )override external {
        loyalty.addLoyalty(loyaltyOffer._collection, loyaltyOffer._assetId, _msgSender(), loyaltyOffer._loyaltyPercent);
        CreateOffer memory newOffer = CreateOffer(
            loyaltyOffer._id,
            loyaltyOffer._seller,
            loyaltyOffer._collection,
            loyaltyOffer._assetId,
            loyaltyOffer._token,
            loyaltyOffer._price,
            loyaltyOffer._amount,
            loyaltyOffer._isForSell,
            loyaltyOffer._isForAuction,
            loyaltyOffer._expiresAt,
            loyaltyOffer._shareIndex
        );
        _addOffer(
            newOffer
        );
    ...
    function _addOffer(
        ...
        require(!offers[newOffer._id].exists, "Offer exists already");
        ...
        emit Listed(newOffer._seller, newOffer._collection, newOffer._assetId, newOffer._amount, newOffer._price);
    }
    ...
    function cancelOffer(uint256 offerID) override external onlyOfferOwner(offerID) {
    ...
    function buyOffer(uint256 id, uint256 amount) override external payable {
    ...
    */
    ...
    function _buyOffer(
        ...
        sellerAmount = loyalty.sendLoyaltyToCreatorFromETH(
            offer.collection,
            offer.assetId,
            offer.seller,
            sellerAmount
    ...
    function cancelBid(uint256 _offerId, address _bidder) override external {
        ...
        onlyOfferOwner(_offerID)
    {
        ...
        // check seller
        sellerAmount = loyalty.sendLoyaltyToCreatorFromERC20Token(
            offer.collection,
            offer.assetId,
            offer.seller,
            sellerAmount,
