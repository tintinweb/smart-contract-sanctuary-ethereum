...
import {sub, wdiv} from "fiat/utils/Math.sol";
...
import {mul} from "fiat/utils/Math.sol";
...
abstract contract OptimisticOracle is IOptimisticOracle, Guarded {
    ...
    function activateRateId(bytes32 rateId) public checkCaller {
        ...
        // set the blank (initial) proposal that will be referenced during the first shift
        proposals[rateId] = computeProposalId(
            rateId,
            address(0),
            0,
    ...
    function shift(
        ...
        if (prevNonce != 0) _push(rateId, prevValue);
...
interface IOptimisticOracle {
    ...
    event Push(bytes32 indexed rateId, uint256 value);
    ...
    function canDispute(bytes32 nonce) external returns (bool);
