...
error Dobermans__ZeroAddressProhibited();
error Dobermans__NotACreator();
error Dobermans__NothingToWithdraw();

error Dobermans__TokenIsLockedInStaking();

contract Dobermans is Neutron {
	address payable private immutable i_creator1;
	address payable private immutable i_creator2;
	address payable private immutable i_creator3;
	...
	) Neutron("Dobermans", "DOBE", 0x88f19e6e1d5487d3, 0x323Dc358909BdfcA136481DFf8C486F22aFEcA4A) {
		if (creator1 == address(0) || creator2 == address(0) || creator3 == address(0))
			revert Dobermans__ZeroAddressProhibited();
		i_creator1 = creator1;
		i_creator2 = creator2;
		i_creator3 = creator3;
	}
	 ...
	 */
	function setStakingContract(address stakingContract) external onlyManager {
		s_stakingContract = IStaking(stakingContract);
	}

	/**
	 * @dev Withdraw the balance of the contract to the creators.
	 * The first creator gets 80% of the balance, the second and the third get 10%.
	 */
	function withdraw() external nonReentrant {
		bool isCreator = msg.sender == i_creator1 ||
			msg.sender == i_creator2 ||
			msg.sender == i_creator3;
		if (!isCreator) revert Dobermans__NotACreator();

		uint256 balance = address(this).balance;
		if (balance == 0) revert Dobermans__NothingToWithdraw();

		i_creator1.transfer((balance * 80) / 100);
		i_creator2.transfer((balance * 10) / 100);
		i_creator3.transfer((balance * 10) / 100);
	}

	/**
	 * @dev Get the addresses of the creators.
	 *
	 * @return creators The addresses of the creators.
	 */
	function getCreatorAddresses() external view returns (address payable[3] memory creators) {
		creators = [i_creator1, i_creator2, i_creator3];
	}

	/// @dev Override the _beforeTokenTransfers function to check if the token is locked in staking.
	function _beforeTokenTransfers(
		...
		for (uint256 tokenId = startTokenId; tokenId < startTokenId + quantity; tokenId++) {
			if (s_stakingContract.isTokenStaked(tokenId))
				revert Dobermans__TokenIsLockedInStaking();
		}
	}
}
