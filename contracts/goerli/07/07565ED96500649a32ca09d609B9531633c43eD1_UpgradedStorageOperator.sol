 ...
 */
contract UpgradedStorageOperator is KeeperCompatibleInterface, Ownable {
    ...
    function _deductStorageCost(IERC777 token, IBWGToken iBWGToken, ITokenTransferOperator tokenTransferOperator) internal {   
        ...
        for (uint256 index = lastIndex; index < batchLastIndex; index++) {
            ...
            // Costfree wallets are not allowed to deduct tokens as costs.
            if (!iBWGToken.isCostFreeWallet(_balanceWallet)) {
                ...
                uint256 _cost = ((_balance * _storageCostRate)/10000) + transactionFee;
                ...
                _totalCost = _totalCost.add(_cost);
    ...
    function getNextStorageCostRunDetails() external view returns ( uint256 totalStorageCost, uint256 totalWallet, uint256 totalWalletUnderThreshold ){
        IERC777 token = IERC777(baseContract);
        IBWGToken iBWGToken = IBWGToken(baseContract);             
        address[] memory costableWallets = iBWGToken.costableWallets();                 
        
        for (uint256 index = 0; index < costableWallets.length; index++) {
            address balanceWallet = costableWallets[index];
            uint256 balance = token.balanceOf(balanceWallet);
            if (!iBWGToken.isCostFreeWallet(balanceWallet)) {
