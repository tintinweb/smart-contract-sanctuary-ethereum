...
contract PunkGizmos is Context, ERC721Tradeable {
  ...
  mapping (uint256 => StakingEvent) stakeholders;
  ...
  constructor(address _proxyRegistryAddress) ERC721Tradeable("PunkGizmos", "PUNKG", _proxyRegistryAddress) {
    ...
    function stakeGizmo(uint256 gizmo) public {
      ...
      require(stakeholders[gizmo].epochOfStake == 0, "Already staked");
      _burn(gizmo);
      stakeholders[gizmo] = StakingEvent(block.number, 1, 0, _msgSender(), 0);
    }
    ...
    function unstakeGizmo(uint256 gizmo) public {
      // mint stage 3 means the mint phase is over
      require(mintStage == 3, "Mint not finished");
      require(stakeholders[gizmo].epochOfStake != 0, "Already unstaked");
      require(_msgSender() == stakeholders[gizmo].holder, "Not owner of token");
      _remintTo(_msgSender(), gizmo);
      delete stakeholders[gizmo];
    }
    ...
    function calculateRewards() public {
      ...
      require(block.timestamp > lastCalculationEpoch + 72, "Too early, need to wait at least 72 blocks since last calculation");
      // sum all time differences
      uint256 sumEpochs = 0;
      for (uint256 i = 0; i < staked.length; i++) {
        uint256 y = ((currentEpoch - stakeholders[staked[i]].epochOfStake).mul(gizmoToWeights[staked[i]])).div(rarityScale);
        sumEpochs = sumEpochs + y;
        stakeholders[staked[i]].y = y;
      }
      uint256 currentContractBalance = address(this).balance;
      // calculate the reward for each gizmo
      for (uint256 i = 0; i < staked.length; i++) {
        StakingEvent memory gizmo = stakeholders[staked[i]];
    ...
    function claimRoyalties() public {
      uint256 due = 0;
      for (uint256 i; i < balanceOf(_msgSender()); i++){
         uint256 gizmo = tokenOfOwnerByIndex(_msgSender(), i);
         if (stakeholders[gizmo].weiDue > 0) {
           due = due.add(stakeholders[gizmo].weiDue);
           stakeholders[gizmo].weiDue = 0;
           stakeholders[gizmo].epochOfCalculation = 0;
         }
      }
...
contract ERC721Tradeable is ERC721Enumerable, ContextMixin, NativeMetaTransaction, Ownable {
  using SafeMath for uint256;
  using Counters for Counters.Counter;
