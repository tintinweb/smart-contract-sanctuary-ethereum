...
contract StakingLockable is LockSettings, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    function earned(address _account) virtual public view returns (uint256) {
        ...
        // `b.userRewardPerTokenPaid` is at most == rewardPerToken()
        // so when we doing below calculation: rewardPerToken() >= b.userRewardPerTokenPaid, we will not underflow
        ...
        updateReward(_user)
    {
        ...
        // `_amount` is what we will transferFrom contract, so we can not overflow `totalSupply.totalBalance`
        totalSupply.totalBalance += uint96(_amount);
        // `_bonus` is max possible rUMB supply * 5 (max multiplier) => 200_000_000 * 5 => 1_000_000_000
        // 1e9 tokens can be stored with 90bits, so we are ok with 128
    ...
    function _withdrawUnlockedTokens(
        ...
        internal
    {
        // we working with internal variables here (input is not from user), so there is no option to under/over-flow
