...
contract NomadMinting is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function mint(address to, uint256[] calldata tokenIds) public payable {
    ...
    for (uint256 index = 0; index < tokenIds.length; index++) {
      ...
      require(tokenIds[index] < 10000, "Above limit");
      require(tokenIds[index] > 0, "Below limit");
  ...
  {
    address[] memory ret = new address[](tokens.length);
    for (uint256 index = 0; index < tokens.length; index++) {
      ...
      bytes32 labelhash = keccak256(bytes(label));
      bytes32 ensId = _makeNode(parentNode, labelhash);
      ret[index] = NameWrapper(nameWrapper).ownerOf(uint256(ensId));
    }
    return ret;
  }
  ...
  function freeMints(
    ...
    for (uint256 index = 0; index < amount.length; index++) {
      require(amount[index] < 10000, "Above limit");
      require(amount[index] > 0, "Below limit");
