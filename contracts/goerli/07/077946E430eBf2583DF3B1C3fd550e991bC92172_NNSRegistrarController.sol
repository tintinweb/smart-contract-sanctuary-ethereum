...
contract NNSRegistrarController is Ownable {
    ...
    bytes32 internal constant _passNFT_ = "passNFT";
    ...
    */
    ...
    mapping (bytes32 => uint) public conf;  //_
    ...
    {
       // __NNSRegistrarController_init(_base,_prices,_gov);
       __Ownable_init();
  ...
  /*  function __NNSRegistrarController_init(
    ...
    {
        __Ownable_init();
        __NNSRegistrarController_init_unchained(_base,_prices,_gov);
    }
...
/*        if(vars.resolver != address(0)) {
   ...
   function RegisterInter(Vars memory vars) internal {
        if(vars.resolver != address(0)) {
            // Set this contract as the (temporary) owner, giving it
            // permission to set up the resolver.
            vars.expires = base.registerWithName(vars.tokenId, address(this), vars.duration,vars.name);
            ...
            require(vars.addr == address(0));
            vars.expires = base.registerWithName(vars.tokenId, vars.owner, vars.duration,vars.name);
        }
        emit NameRegistered(vars.name, vars.label, vars.owner, vars.cost, vars.expires);
    ...
    function renew(string calldata name, uint duration) external payable {
        uint cost = rentPrice(name, duration);
        address pass = address(uint160(conf[_passNFT_]));
        if (pass != address(0)){
            IERC721 nft = IERC721(pass);
   ...
   /* function setConf(uint _minCommitmentAge, uint _maxCommitmentAge,uint _wlBegin,uint _wlEnd,uint _pubBegin,NftWlInfo calldata _nftWlInfo) public onlyOwner {
    ...
    function batchRegister(string[] memory names,uint[] calldata durations, address[] calldata  owners,address resolver) public  { // ora:address owner,address resolver, address addr
        require(conf[_airDropAcc_]==uint(uint160(msg.sender)),"E");
        require((names.length == durations.length)&&(names.length == owners.length),"len!");
        Vars memory vars;
