...
contract KLStaking is ReentrancyGuard {
    ...
    // 质押状态信息（ 0: deposit  1: staked  2: unstaked  3: withdrawal  4: staking  5: unstaking ）
    mapping(uint256 => address) private _stakeUsers;
    mapping(uint256 => uint8) private _stakeStatus;
    ...
    // 转移事件
    event OnTransfer(address indexed sender, address indexed token, uint256[] ids, uint256 time, uint256 amount, uint8 way);
    ...
    // 提取代币并销毁锚定币
    function withdraw(string memory name, uint256 id, uint256 amount) public nonReentrant {
        ...
        require(_stakeStatus[id] == 2, "You cannot withdraw tokens right now");// 2:unstaked 状态才能提现

        // 验证质押数量（地址为0则认为退还ETH代币，反之则认为退还ERC20代币）
        bool isEthToken = address(token.input) == address(0);
        uint256 returned = amount.mul(token.denominator).div(token.numerator);
        if(isEthToken){
            uint balance = address(this).balance;
            require(balance>=returned,"Insufficient balance");
        }else{
        ...
        // 转移锚定币
        token.output.safeTransferFrom(msg.sender, address(this), amount); 

        // 发送质押币（不能删除状态，万一他只提部分呢）
        if(isEthToken){
            (bool success,) = msg.sender.call{value: returned}("");
            require(success);
        }else{
            token.input.safeTransfer(msg.sender,returned);
        }

        // 销毁锚定币
        token.output.burn(address(this), amount);
        
        emit OnWithdraw(msg.sender, address(token.input), id, block.timestamp, amount, returned);
    }

    // 系统转移质押代币并创建节点
    function transferOut(string memory name, uint256 amount, uint256[] memory ids) public nonReentrant {
        ...
        // 验证合约账户余额（地址为0则认为转移ETH代币，反之则认为转移ERC20代币）
        bool isEthToken = address(token.input) == address(0);
        if(isEthToken){
            uint balance = address(this).balance;
        ...
        emit OnTransfer(msg.sender, address(token.input), ids, block.timestamp, amount, 0);
    }

    // 系统转移质押代币并创建节点
    function transferIn(string memory name, uint256 amount, uint256[] memory ids) public payable nonReentrant {
        ...
        // 区分转入代币类型（地址为0则认为转入ETH代币，反之则认为转入ERC20代币）
        bool isEthToken = address(token.input) == address(0);
        if(isEthToken){
        ...
        // 修改用户质押状态（0: deposit  1: staked  2: unstaked  3: withdrawal  4: staking  5: unstaking ）
        for(uint i=0;i<ids.length;i++){
            _stakeStatus[ids[i]] = 2;//unstaked
        }

        emit OnTransfer(msg.sender, address(token.input), ids, block.timestamp, amount, 1);
    }
    ...
    // 获取存款状态
    function getStatusInfo(uint256 id) public view returns (uint256) {
