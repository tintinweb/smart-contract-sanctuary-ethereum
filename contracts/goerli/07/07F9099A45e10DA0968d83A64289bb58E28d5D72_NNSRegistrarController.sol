...
contract NNSRegistrarController is Ownable {
    ...
    {
        __NNSRegistrarController_init(_base,_prices,_minCommitmentAge,_maxCommitmentAge,_wlBegin,_wlEnd,_pubBegin,_nftWlInfo);
    }    
    ...
    {
        __Ownable_init();
        __NNSRegistrarController_init_unchained(_base,_prices,_minCommitmentAge,_maxCommitmentAge,_wlBegin,_wlEnd,_pubBegin,_nftWlInfo);
    }
    ...
    function __NNSRegistrarController_init_unchained(
        ...
        uint _wlEnd,
        uint _pubBegin,
        NftWlInfo calldata _nftWlInfo) internal onlyInitializing {
    ...
    function costEth(string memory name, uint duration,uint nftNum) view public returns(uint) {
        ...
        uint nftValue = prices.attoUSDToWei(nftNum*160*1e18);
        if (cost<=nftValue)
            return 0;
    ...
    function passToEth() view public returns(uint) {
        return prices.attoUSDToWei(160*1e18);
    }
    ...
    function wlRegister(string calldata name, address owner, uint duration, bytes32 secret,uint wlNum,uint[] calldata tokenIds,bytes32[] calldata _merkleProof) external payable {
        ...
        vars.nftValue = passToEth()*tokenIds.length; //prices.attoUSDToWei(tokenIds.length*160*1e18);
    ...
    function registerWithConfig(string memory name, address owner, uint duration, bytes32 secret, address resolver, address addr) public payable {
        ...
        uint cost = _consumeCommitment(name, duration, commitment,0);
