...
contract OrganisationRegistry is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    // todo: take it module by module; start with token and add the rest
    // todo: we might not be able to generalize this one
    ...
    function deployTokenV1(
        bytes32 id,
        IToken.TokenConfig memory cfg,
        address roleAdmin
    ) public payable {
        Organisation storage org = _getOrganisationOrRevert(id);
        require(org.owner == msg.sender, "OrganisationRegistry: caller is not the owner");
        require(org.token == address(0), "OrganisationRegistry: token already deployed");

        org.modules.push(Modules.TOKEN_V1);

        org.token = factory.deployModuleClone{value : msg.value}(
            Modules.TOKEN_V1,
    ...
    function deployTokenV1Upgradeable(
        bytes32 id,
        ...
        org.token = factory.deployModuleUUPS{value : msg.value}(
            Modules.TOKEN_V1,
            abi.encodeWithSelector(
    ...
    function deployDAOV1Staking(
        ...
        Organisation storage org = _getOrganisationOrRevert(id);
        require(org.owner == msg.sender, "OrganisationRegistry: caller is not the owner");
        require(org.token != address(0), "OrganisationRegistry: token module is required");
        require(org.daoStaking == address(0), "OrganisationRegistry: DAO Staking already deployed");

        org.modules.push(Modules.DAO_V1_STAKING);

        org.daoStaking = factory.deployModuleClone{value : msg.value}(
    ...
    function deployDAOV1StakingUpgradeable(
        bytes32 id,
        address roleAdmin,
        address upgrader
    ) public payable {
        Organisation storage org = _getOrganisationOrRevert(id);
        require(org.owner == msg.sender, "OrganisationRegistry: caller is not the owner");
        require(org.token != address(0), "OrganisationRegistry: token is required");
        require(org.daoStaking == address(0), "OrganisationRegistry: DAO Staking already deployed");

        org.modules.push(Modules.DAO_V1_STAKING);

        org.daoStaking = factory.deployModuleUUPS{value : msg.value}(
            Modules.DAO_V1_STAKING,
            abi.encodeWithSelector(
    ...
    function deployDAOV1Governance(
        ...
        Organisation storage org = _getOrganisationOrRevert(id);
        require(org.owner == msg.sender, "OrganisationRegistry: caller is not the owner");
        require(org.daoStaking != address(0), "OrganisationRegistry: DAO Staking is required");
        require(org.governance == address(0), "OrganisationRegistry: DAO Governance already deployed");

        cfg.daoStakingAddr = org.daoStaking;

        org.modules.push(Modules.DAO_V1_GOVERNANCE);

        org.governance = factory.deployModuleClone{value : msg.value}(
    ...
    function deployDAOV1GovernanceUpgradeable(
        bytes32 id,
        IGovernance.GovernanceConfig memory cfg
    ) public payable {
        Organisation storage org = _getOrganisationOrRevert(id);
        ...
        org.governance = factory.deployModuleUUPS{value : msg.value}(
            Modules.DAO_V1_GOVERNANCE,
            abi.encodeWithSelector(
                IGovernance.initialize.selector,
                cfg
    ...
    function _getOrganisationOrRevert(bytes32 id) internal view returns (Organisation storage) {
        require(organisations[id].exists == true, "OrganisationRegistry: organisation does not exist");
