 ...
 */
contract IP3 is Initializable, OwnableUpgradeable {
    ...
    mapping(bytes32 => ClaimInfo) claimableMap; // hashedCerticate => isClaimed
    uint256 managedFeeBalance; //balance of platform manage fee
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.Bytes32Set;
    EnumerableSetUpgradeable.Bytes32Set private nftOnRecord; // set for NFT have transactional record (hash nft)
    ...
    bool claimRevenueLock;
    bool claimManageFeeLock;
    ...
    function initialize(
        IERC20 instanceAddress,
        address manageFeeAddress,
        address _signer
    ) public initializer {
        __Ownable_init();
        acceptedERC20token = instanceAddress;
        priceParams = PriceDynamicsParams({
            maxDiscount: 5*10**17,
            alpha: 5*10**18, // 0.5 WAD,
    ...
    function purchaseAuthorization(
        ...
        // Verify signature
        require(
            isValidSignature(authorizedNFT, term, signature),
            "invalid signature"
        );
    ...
    function purchaseByAmount(
        ...
        Term memory newTerm = Term(block.timestamp, 0, count, isByDuration);
        ...
        acceptedERC20token.transferFrom(msg.sender, address(this), termedPrice);
    }
    ...
    function purchaseByDuration(
        ...
        // first get approved amount from USDT approve, then can purchase this
        bytes32 hashedAuthorizeNFTbyDuration = hashAuthorizeNFT(
            authorizedNFTSubmit,
            isByDuration
        );
        ...
        if (
            nftOnRecord.contains(hashedNft) &&
            latestStartPriceByDurationRecord != 0
        ) {
            // get the latest start price from record on chain
            currentStartPrice = getCurrentStartingPricePerNFT(
                hashedNft,
        ...
        if (term.isByDuration) {
            result = keccak256(
                abi.encodePacked(
                    ...
                    authorizedNFT.authorizedByDurationInfo.listStartTime,
                    authorizedNFT.authorizedByDurationInfo.listEndTime,
                    authorizedNFT.authorizedByDurationInfo.lastActive,
                    term.authorizedStartTime,
                    term.authorizedEndTime,
                    term.isByDuration
                )
            ...
            result = keccak256(
                abi.encodePacked(
                    ...
                    authorizedNFT.authorizedByCountInfo.listStartTime,
                    authorizedNFT.authorizedByCountInfo.listEndTime,
                    authorizedNFT.authorizedByCountInfo.lastActive,
                    term.count,
                    term.isByDuration
                )
            );
    ...
    /*//////////////////////////////////////////////////////////////
                    CLAIM REVENUE
    //////////////////////////////////////////////////////////////*/
    function claimRevenue(bytes32[] memory hashedCertifcates) external {
        require(!claimRevenueLock, "LOCKED");

        uint256 totalBalance = 0;
        uint256 size = hashedCertifcates.length;

        for (uint256 i = 0; i < size; i++) {
            if (isValidClaim(msg.sender, hashedCertifcates[i])) {
                claimableMap[hashedCertifcates[i]].isClaimed = true;
                totalBalance += claimableMap[hashedCertifcates[i]]
                    .claimableAmount;
            }
        }
        require(totalBalance > 0, "ZERO BALANCE");

        acceptedERC20token.transfer(msg.sender, totalBalance);
    ...
    function updatePurchaseState(
        ...
        Metadata memory metadata = newAuthorizeCertificate.metadata;
        ...
        claimableMap[hashedCertificate].claimableAmount = authorizerOwn;
    ...
    /*//////////////////////////////////////////////////////////////
                    SETTER FUNCTIONS
    //////////////////////////////////////////////////////////////*/
    function setPriceParams(PriceDynamicsParams memory _priceParams)
        public
        onlyOwner
    {
    ...
    function getCurrentStartPrice(bytes32 hashedNft)
        public
        view
    ...
    function getPriceParams()
        external
        view
        returns (PriceDynamicsParams memory)
    {
        return priceParams;
    }
    ...
    function getClaimableAmount(
        ...
        bytes32[] memory hashedCertifcates
    ) public view returns (uint256) {
        uint256 totalAmount = 0;
        uint256 size = hashedCertifcates.length;
        for (uint256 i = 0; i < size; i++) {
            if (isValidClaim(claimAddress, hashedCertifcates[i])) {
                totalAmount += claimableMap[hashedCertifcates[i]]
                    .claimableAmount;
            }
        }
    ...
    function isValidClaim(address claimAddress, bytes32 hashedCertifcate)
        internal
        view
        returns (bool)
    {
        address nftHolder = authorizeCertificateMap[hashedCertifcate]
            .authorizedNFT
            .authorizer
            .nftHolder;

        address claimAddressOnRecord = authorizeCertificateMap[hashedCertifcate]
            .authorizedNFT
            .authorizer
            .claimAddress;

        // filter if the certifcates alreay claimed
        if (claimableMap[hashedCertifcate].isClaimed) {
            return false;
        }

        // filter if the authroizer or claim has current own the NFT addreess
        NFT memory nft = authorizeCertificateMap[hashedCertifcate]
            .authorizedNFT
            .nft;

        address currentHolder = IERC721Upgradeable(nft.NFTAddress).ownerOf(
            nft.tokenId
        );

        // if token transfer out, check the current addr if hold this token
        if (nftHolder != currentHolder) {
            if (claimAddress != currentHolder) {
                return false;
            }
        } else {
            // if current holder is same as the holder on record
            // check the if claim addr same as on record
            if (claimAddressOnRecord != claimAddress) {
                return false;
            }
        }

        // check the term based on authorzer nft type (by Duration, by amount)
        Term memory term = authorizeCertificateMap[hashedCertifcate].term;
        if (!isValidByTerm(term)) {
            return false;
        }
    ...
    function isValidByTerm(Term memory term) internal view returns (bool) {
        // if by duration
        if (term.isByDuration) {
            if (term.authorizedEndTime > block.timestamp) {
                return false;
            }
            return true;
        } else {
            // by amount: check if within 7 days
            if (term.authorizedStartTime + 86400 * 7 > block.timestamp) {
                return false;
            }
            return true;
        }
    ...
    function claimManageFee() external {
        ...
        managedFeeBalance = 0;
        acceptedERC20token.transfer(msg.sender, claimBalance);
    ...
    function getManageFee() external view returns (ManageFee memory) {
        return manageFee;
    }
    ...
    /*//////////////////////////////////////////////////////////////
                    Lock Set
    //////////////////////////////////////////////////////////////*/
    function setClaimRevenueLock(bool flag) external onlyOwner {
        claimRevenueLock = flag;
    }

    function setClaimManageFeeLock(bool flag) external onlyOwner {
...
contract IP3v2 is IP3{

    function getCurrentStartPriceOneNFT(NFT memory nft) external view returns (uint256[] memory) {
        bytes32 hashedNft = getNftHash(nft);
        uint256[] memory priceBothOptions = new uint256[](2);
        priceBothOptions[0] = getCurrentStartingPricePerNFT(hashedNft, true);
        priceBothOptions[1] = getCurrentStartingPricePerNFT(hashedNft, false);
...
import {discountFactor} from "../utils/PriceFunction.sol";
...
library SmartPriceEngine {
    ...
    function computeAuthroizedPriceByCount(
        ...
        uint256 authorizedPrice = currentStartPrice * count * uint256(discountFactor(int256(maxDiscount), int256(count * WAD), int256(alpha))) / WAD;
        
        // currentStartPrice *  count * discountFactor(maxDiscount, count, alpha);
        // discountFactor(int256 maxDiscount, int256 x, int256 alpha)
    ...
    function computeAuthroizedPriceByDuration(
        ...
        uint256 authorizedPrice = currentStartPrice * duration / TIME_INTERVAL * uint256(discountFactor(int256(maxDiscount), int256(duration / TIME_INTERVAL * WAD), int256(alpha))) / WAD;
...
import {wadExp, unsafeWadDiv, wadDiv, wadLn} from "../../lib/solmate/src/utils/SignedWadMath.sol";


// both x and alpha in WAD unit
// maxDiscount: less than 1 WAD unit but must be in a range, may be 0.5 WAD  to 0.8 WAD
function sigmoidAlphaDiscount(int256 maxDiscount, int256 x, int256 alpha) pure returns (int256) {
    return wadDiv(2*maxDiscount, 10**18 + wadExp(-wadDiv(x,alpha))) - maxDiscount;
}

// discount factor, max is 1 WAD means no discount at all
function discountFactor(int256 maxDiscount, int256 x, int256 alpha) pure returns (int256) {
    x -= 10**18;
    return 10**18 -  sigmoidAlphaDiscount(maxDiscount, x,alpha);
}

// back to 6 decimal values
function converBackSixDecimal(int256 x) pure returns (uint256) {
    return uint256(x/ 10**12);
}
