...
contract ShmidgetNFTMinter is ERC721, ERC2981, Pausable, Ownable, ReentrancyGuard {
// contract PKWidgetNFTMinter is ERC721, ERC721Enumerable, ERC2981, Pausable, Ownable, ReentrancyGuard {
   ...
   uint256 public PKWidgetCIGsPrice = 0.05 ether;   // <<----------------------------------------------- CIGs not ETH!!!
   ...
   event NFTBurned(uint256 indexed burnedTokenID, address indexed burnerAddress);     // <<----------------------------------------------- BURN EVENT - IMPLEMENT IT!!!!!
   ...
   constructor() ERC721("JITTERS NFT", "JTRS") {
   ...
   // (had "callerIsUser"...)
   function regularMint(bool payingWithETH) external payable whenNotPaused() {
   // function regularMint(bool payWithETH, uint256 amountReceived) external payable whenNotPaused() {
      console.log("\n>In 'regularMint()'");
      console.log("  >Is user paying ETH? %s", payingWithETH);
      console.log("  >User sent this much ETH: %s", msg.value);
      ...
      if(payingWithETH == true) {
         require(msg.value >= PKWidgetETHPrice, "'regularMint()' ERROR! Insufficient ETH funds!");
      }
      else {
         // require(msg.value >= PKWidgetCIGsPrice, "CIGs 'regularMint()' ERROR! Insufficient CIGs funds!");   
         // REMINDER: function transferFrom(address sender, address recipient,uint256 amount) external returns (bool); 

         // Get the amount of ERC20 Token ALLOWED to be spent:
         uint256 currentAllowance = SillyCoinContractInstance.allowance(msg.sender, address(this));

         if(currentAllowance != type(uint256).max) {
            SillyCoinContractInstance.approve(address(this), (currentAllowance - PKWidgetCIGsPrice));
            require(currentAllowance >= PKWidgetCIGsPrice, "ERC20: insufficient allowance");
            // unchecked {
            // }
         }

         // bool sillyTokenApproved = SillyCoinContractInstance.approve(msg.sender, address(this));
         SillyCoinContractInstance.transferFrom(msg.sender, address(this), PKWidgetCIGsPrice);
   ...
   /* 
      ...
      I SHOULD NOT APPEND TO THE ARRAY BELOW, I SHOULD WIPE OUT AND START FROM SCRATCH!
   ...
   */ 
   ...
   function getWidgetPrices() public view returns(uint256 priceInETH, uint256 priceInCIGs) {
      return(PKWidgetETHPrice, PKWidgetCIGsPrice);
   }
   ...
   */
   ...
   function setWidgetPrice(uint ethPrice, uint cigPrice) public onlyOwner {
      console.log("In 'setWidgetPrice()' \nIncoming Price =  %s", ethPrice);

      PKWidgetETHPrice = ethPrice; // * 1 ether;
      PKWidgetCIGsPrice = cigPrice * 7; // ether;  // <------------------------  FIX THIS to get REAL TIME CIGs PRICE!

      // console.log("\n  >'PKWidgetETHPrice' was set to:  %s", PKWidgetETHPrice);
      // console.log("\n  >'PKWidgetCIGsPrice' was set to:  %s", PKWidgetCIGsPrice);
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
   ...
   */
   function _safeMint(address to, uint256 tokenId) internal virtual {
      console.log("\n  >In ERC721 '_safeMint()'!!! 'tokenId' = %s", tokenId);
      _safeMint(to, tokenId, "");
   }
