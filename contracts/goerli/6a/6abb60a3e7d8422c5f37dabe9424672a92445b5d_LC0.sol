...
contract ERC20 is Context {
     ...
     */
    event Transfer(address indexed _from, address indexed _to, uint256 _value);//--moni
     ...
     */
     event Approval(address indexed _owner, address indexed _spender, uint256 _value);//--moni
    ...
    function transferFrom(
        ...
        require(to != address(0));
        require(_balances[from] >= amount,"amount more than balnce");
        require(_allowances[from][msg.sender] >= amount,"amount more than allowence");
        require(_allowances[from][msg.sender] != type(uint256).max);
       ...
       // _balances[to] += amount;
        ...
        //_approve(from,msg.sender,_allowances[from][msg.sender]);
       // emit Transfer(from,to,amount);
        //emit Approval(from, to, _allowances[from][msg.sender]);
        return true;
    ...
    /** TODO
     @dev Creates `amount` tokens and assigns them to `accoun5t`, increasing
     * the total supply.
     *
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(
                ...
                currentAllowance >= amount,"spend allowance less than amount"
            );
            //_allowances[owner][spender] -= amount;
            unchecked {
...
contract LC0 is LuckyToken {
    // TODO: replace names
    constructor() LuckyToken("LuckyTokenMEK", "LCMEK") {}
}