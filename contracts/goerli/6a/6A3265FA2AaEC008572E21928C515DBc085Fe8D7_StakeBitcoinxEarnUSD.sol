...
contract StakeBitcoinxEarnUSD is ReentrancyGuard{
    ...
    mapping(address => uint256) public lockedDuration;

    uint256 public rewardPerBlock = 40000000000000000; //wei in USD Per 1M BTCX (4000 per block/1M) 
    ...
    function doStake(uint256 amount) external nonReentrant{
        ...
        lockedDuration[msg.sender] = block.timestamp + 7 days;
    ...
    function unStakeToken(uint256 amount) external payable nonReentrant{
        require(amount >= baseFee, 'Low Gas Fee');
        _updateReward(msg.sender);
        console.log('Locked: ', lockedDuration[msg.sender]);
        console.log('Current Timestamp: ', block.timestamp);
        if(block.timestamp < lockedDuration[msg.sender]){
    ...
    function getTotalStakedTokenInSmartContract() public view returns(uint256){
