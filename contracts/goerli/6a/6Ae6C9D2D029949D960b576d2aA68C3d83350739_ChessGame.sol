 ...
 * @author gmspacex
 */
contract ChessGame is Ownable, BaseGame {
     ...
     */
    function getGameResults(bytes32 _gameId) public view returns (address) {
    ...
    function newGame(
        ...
        require(player1.ethBalance >= fee, "CHESS: Insufficient Player1 Eth Balance");
        require(player2.ethBalance >= fee, "CHESS: Insufficient Player2 Eth Balance");

        bytes32 gameId = keccak256(abi.encode(address(this), player1, player2, nonce));
        require(gameId.toEthSignedMessageHash().recover(signature) == verifier, "CHESS: Invalid signature");
    ...
    function endGame(
        ...
        require(_hash.toEthSignedMessageHash().recover(signature) == verifier, "CHESS: Invalid signature");
        ...
        require(winner == player1.player || winner == player2.player, "CHESS: Invalid winner address");
    ...
    function emergencyEndGame(bytes32 gameId) external override onlyOwner {
        ...
        require(game.player1.player != address(0x0), "CHESS: Invalid game id");
        require(game.winner == address(0x0), "CHESS: It's a finished game");
