...
contract CallPool is ICallPool, Pausable {
    ...
    uint256[4] private DURATION = [3 days, 7 days, 14 days, 28 days];
    ...
    function checkAvailable(uint256 tokenId) public view returns (bool) {
        if (nftStatus[tokenId].available) {
            ...
            if (CallToken(callToken).getCallInfo(tokenId).endTime < block.timestamp) {
                return true;
            } else {
                return false;
            }
    ...
    function _deposit(
        ...
        uint8 lowerStrikePriceGapIdx,
        uint8 upperDurationIdx,
        uint256 lowerLimitOfStrikekPrice
    ) internal {
        ...
        nftStatus[tokenId] = NFTStatus(true, true, lowerStrikePriceGapIdx, upperDurationIdx, lowerLimitOfStrikekPrice);
   ...
   */
    ...
    function previewOpenCall(
        ...
        if(!(status.ifOnMarket) || !(checkAvailable(tokenId))){
    ...
    function _openCallInteractions(uint256 tokenId, uint256 strikePrice, uint256 durationIdx) internal {
        ...
        if (status.available) {
            CallToken(callToken).mint(_msgSender(), tokenId, strikePrice, DURATION[durationIdx], exercisePeriodProportion);
        } else {
            CallToken(callToken).reopen(_msgSender(), tokenId, strikePrice, DURATION[durationIdx], exercisePeriodProportion);
        }
    }
...
contract CallToken is ERC721, ICallToken, Ownable {
    address public immutable override nft;
    ...
    function mint(
        ...
        _safeMint(user, tokenId);
        emit Mint(user, tokenId, strikePrice, exercisePeriodBegin, exercisePeriodEnd);
    }
...
interface ICallToken {
    ...
    event Mint(address indexed user, uint256 indexed tokenId, uint256 exercisePrice, uint256 exercisePeriodBegin, uint256 exercisePeriodEnd);

    function mint(address user, uint256 tokenId, uint256 strikePrice, uint256 duration, uint256 exercisePeriodProportion) external;

    event Burn(address indexed user, uint256 indexed tokenId);

    function burn(uint256 tokenId) external;

    function reopen(address user, uint256 tokenId, uint256 strikePrice, uint256 duration, uint256 exercisePeriodProportion) external;
}
