...
contract NYXSoulmateAI is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    address payable internal po =
        payable(0x9933Fc3Ae2188Ff5C53AEB6a24f31104733CD87F);
    address payable internal dev =
        payable(0x9933Fc3Ae2188Ff5C53AEB6a24f31104733CD87F);

    uint256 public MINT_COST = 0.00088 ether; // 0.088 ether;
    uint256 public MAX_SUPPLY = 5; // 10000;
    ...
    function presaleMint(
        ...
        if (hasFreeMint || addressToFreeMint[msg.sender]) {
            require(
                addressToMinted[msg.sender] + count <= MAX_PER_WALLET + 1,
                "Exceeds your allowance"
            );
    ...
    function setMaxSupply(uint256 maxSupply) external onlyOwner {
        require(maxSupply < MAX_SUPPLY, "Check value");
        MAX_SUPPLY = maxSupply;
    }

    function setMintCost(uint256 mintCost) external onlyOwner {
        require(mintCost < MINT_COST, "New value must be < MINT_COST");
        MINT_COST = mintCost;
    }
