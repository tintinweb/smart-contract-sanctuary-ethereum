...
contract ProjectLucifer is ERC721Enumerable, Ownable {  
    ...
    uint256 private publiceSaleCounter = 0;
    ...
    function claimTokens(uint256[] calldata _ownedACTokens, uint256[] calldata _ownedSSDTokens) public {
        ...
        if(_ownedACTokens.length > 0) {
            for (uint256 i = 0; i < _ownedACTokens.length; i++) {
                ...
                tokensIds[count] = 4444 + _ownedACTokens[i];
                count = count + 1;
            }
        }
        ...
        if(_ownedSSDTokens.length > 0) {
            for (uint256 i = 0; i < _ownedSSDTokens.length; i++) {
                require(ac.ownerOf(_ownedSSDTokens[i]) == msg.sender,                            "Require to be token holder");
        ...
        for (uint256 i = 0; i < tokensIds.length; i++) {
            if(tokensIds[i] > 4444) {
                isAcTokenIdMinted[tokensIds[i] - 4444] = true;
            } else {
    ...
    function mintToken(uint256 _amount, bytes32[] calldata _merkleProof) public payable {
        ...
        require( publiceSaleCounter + _amount <= publicSaleLimit,                               "Can't mint more than max supply" );
        ...
        for (uint256 i = 0; i < _amount; i++) {
            ...
            publiceSaleCounter = publiceSaleCounter + 1;
            _safeMint( msg.sender, 5555 + publiceSaleCounter );
    ...
    function setClaimActiveActive(bool val) public onlyOwner {
        claimActive = val;
    }
    ...
    function getSsdTokensOfOwner(address addr) public view returns(uint256[] memory) {
        return ssd.tokensOfOwner(addr);
    }
