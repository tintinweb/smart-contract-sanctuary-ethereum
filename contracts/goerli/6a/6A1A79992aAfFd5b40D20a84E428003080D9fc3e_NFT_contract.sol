...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     * Emits a {Transfer} event.
     */
    mapping (uint256 => uint256) public initialMintTime;
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        _balances[to] += 1;
        _owners[tokenId] = to;
        initialMintTime[tokenId]=block.timestamp;
     ...
     * Emits a {Transfer} event.
     */
    mapping (address =>uint256) public lastsend;
    uint256 limit= 250 days;
    ...
    function _transfer(
        ...
        if(to==staking||from == staking||excludefromlimits[to]==true||excludefromlimits[from]==true){
        ...
        _owners[tokenId] = to;
               ...
               require(block.timestamp>=initialMintTime[tokenId]+limit,"You are not allowed to send before limit");
                _beforeTokenTransfer(from, to, tokenId);
...
abstract contract Ownable is Context {
 ...
 contract NFT_contract is ERC721Enumerable, Ownable   {
  ...
  uint256 public ValueinULE = 1e18;
  ...
  uint256 public MinitngPricein_token=100;    // BUSD 


 /////////////////whitelist prices///////////////////////////// 
  uint256 public WhitelistMintingPricein_eth=500000000000000000; //0.5 eth
  uint256 public WhitelistMinitngPricein_token=500000000000000000;    // 0.5 Token
  uint256 public WhitelistMinitngPricein_BUSD=500000000000000000;    // 0.5 BUSD
  ...
  function mint_with_token(uint256 _count ,uint256 amount) public 
     {
        ...
        // uint256 eth = Valueineth;
        uint256 ULE  = ValueinULE;

         uint256 totalamount = ULE*_count;
        if(iswhitelist[msg.sender]==true){

        require(_count<=Whitelist_MaxLimitPerTransaction,"Whitelist Max Limit Per Transaction Exceed!");
        require(amount>=_count*WhitelistMinitngPricein_token ,"please enter token1");  
  ...
  function mint_with_eth(uint256 _count ) public payable
     {
        require(totalSupply()<=maxsupply,"Minitng Limit Exceeded!");
        require( paused == false ,"paused by owner");
         require(ethmint == true , "enable minting");
        ...
        if(iswhitelist[msg.sender]==true){
            require(msg.value>=_count*WhitelistMintingPricein_eth ,"please enter eth1");
            require(_count<=Whitelist_MaxLimitPerTransaction,"Whitelist Max Limit Per Transaction Exceed!");
            for (uint i = 0; i < _count; i++) 
              {
                 uint mintIndex = totalSupply();
         ...
         mintedNFTs[msg.sender] += _count;
            ...
            require(msg.value>=_count*eth  ,"please enter eth2");
            require(_count<=MaxLimitPerTransaction,"Max Limit Per Transaction Exceed!");
            for (uint i = 0; i < _count; i++) 
              {
                 uint mintIndex = totalSupply();
...
/////////////////////////Whitelist value////////////////////////////////
  ...
  function SetWhitelistMintingPricein_eth(uint256 _value) external onlyOwner{
      WhitelistMintingPricein_eth=_value;
  }

  function SetWhitelistMinitngPricein_Token(uint256 _value) external onlyOwner{
  ...
  function setULEprice(uint256 value) external onlyOwner 
  {
      ValueinULE = value;
  }
  ...
  function setenablemintwitheth() public onlyOwner{
      ethmint=true;
  } 
   function setenablemintwithtoken() public onlyOwner{
      tokenmint=true;
  } 
   function setenablemintwithbusd() public onlyOwner{
      busdmint=true;
  } 
  function setDisablemintwitheth() public onlyOwner{
      ethmint=false;
  } 
   function setDisablemintwithtoken() public onlyOwner{
      tokenmint=false;
  } 
   function setDisablemintwithbusd() public onlyOwner{
      busdmint=false;
  } 
