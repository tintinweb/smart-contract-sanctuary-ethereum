...
contract Fanverse is ERC1155URIStorageUpgradeable, ERC2981Upgradeable, EIP712Upgradeable{
    ...
    IERC20 public fanvToken;  //Fanverse token
    ...
    mapping(uint256 => TokenId) public detailsNFT;
    event primaryFixBuy(address indexed seller, address indexed buyer, uint256 tokenId, uint256 nftBatchAmount, uint256 nftPrice);
    event primaryAuction(address indexed seller, address indexed buyer, uint256 tokenId, uint256 nftPrice);

    
    function initialize(string memory _uri, uint256 _marketFee, address _fanvToken, address _adminAddress) external initializer {
        __ERC1155_init(_uri);
...
*/
    ...
    function priListingHash(FNVvoucher.priListing memory voucher) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256(
                "priListing(uint160 nftOwner,uint256 tokenId,uint256 unitprice,uint256 countervalue,uint256 amount,bool listed,bool isEth)"
            ),
            voucher.nftOwner,
            voucher.tokenId,
            voucher.unitprice,
...
*/
    ...
    function primaryBuy(FNVvoucher.mintVoucher memory mintvoucher, FNVvoucher.priListing memory _voucher, address redeemer, uint256 nftamount, uint256 _fanvAmount) public payable{
        fixBuyFlow(mintvoucher, _voucher, redeemer, nftamount, _fanvAmount);
        emit primaryFixBuy(address(mintvoucher.nftOwner), redeemer, mintvoucher.tokenId, mintvoucher.amount,_voucher.unitprice*(nftamount));
    }

    function fixBuyFlow(FNVvoucher.mintVoucher memory mintvoucher, FNVvoucher.priListing memory _voucher, address redeemer, uint256 nftamount, uint256 _fanvAmount) internal {
            ...
            if(priListAmonutLeft[_voucher.countervalue] == 0){
                ...
                if(_voucher.isEth){
                        require((_voucher.unitprice*(nftamount)) == msg.value , "pay more eth");
                        distributeAmountInEth(signer, msg.value);     
                } else {
                        require((_voucher.unitprice*(nftamount)) == _fanvAmount , "pay more fanv token");
                        distributeAmountInFanv(signer, _fanvAmount);
                }
    }
/**
...
*/
    ...
    function distributeAmountInFanv(address _receiver, uint256 FanvAmount) internal {
...
*/
    ...
    function distributeAmountInEth(address _receiver, uint ethAmount) internal {
...
*/
    ...
    function _hashAucSeller(FNVvoucher.auctionItemSeller memory voucher) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256(
                "auctionItemSeller(address nftAddress,address owner,address royaltyKeeper,uint256 royaltyFees,uint256 tokenId,uint256 nftBatchAmount,string tokenURI,uint256 minimumBid,uint256 startTime,uint256 endTime,bool started,bool ended,bool isEth)"
            ),
            voucher.nftAddress,
            voucher.owner,
            voucher.royaltyKeeper,
...
*/
    ...
    */    
    ...
    function primaryAucWinner (FNVvoucher.auctionItemSeller memory seller, FNVvoucher.auctionItemBuyer memory buyer) public payable {
       buyAuctionFlow(seller, buyer);
       emit primaryAuction(seller.owner, buyer.buyer, seller.tokenId, buyer.pricePaid);
    }

    function buyAuctionFlow(FNVvoucher.auctionItemSeller memory seller, FNVvoucher.auctionItemBuyer memory buyer) internal{
        ...
        if(seller.isEth){                                                                                                   
            require(seller.minimumBid <= buyer.pricePaid && buyer.pricePaid == msg.value, "Invalid Bid of Eth");                      
            distributeAmountInEth(signer, msg.value);                                                                      
        } else{                                                                             
            require(seller.minimumBid <= buyer.pricePaid, "Invalid Bid of Fanv Token");             
