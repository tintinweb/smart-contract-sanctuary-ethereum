...
contract PrincipalToken is
    ...
    IPrincipalToken
{
    error PrincipalTokenError(string reason);
    ...
    modifier notExpired() virtual {
        if (block.timestamp > expiry) {
            revert PrincipalTokenError("Principal Token expired");
        }
        _;
    }
    ...
    modifier afterExpiry() virtual {
        if (block.timestamp < expiry) {
            revert PrincipalTokenError("Called before expiry");
        }
        _;
    }
    ...
    function initialize(
        ...
        if (_ytBeacon == address(0)) {
            revert PrincipalTokenError(
                "YT beacon address should not be equal to zero"
            );
        }
        if (_ibt == address(0)) {
            revert PrincipalTokenError("Cannot set ibt to zero address");
        }
        ...
        if (_max_fees > MAX_FEES) {
            revert PrincipalTokenError("Max fees should be less than 100%");
        }
    ...
    function deposit(
        ...
        if (shares < minShares) {
            revert PrincipalTokenError("Deposit slippage protection");
        }
        return shares;
    }
    ...
    function depositWithIBT(
        ...
        if (shares < minShares) {
            revert PrincipalTokenError("ERC5143: Deposit slippage protection");
        }
        return shares;
    }
    ...
    function mint(
        ...
        if (assets > maxAssets) {
            revert PrincipalTokenError("ERC5143: Mint slippage protection");
        }
        return assets;
    }
    ...
    {
        if (owner != msg.sender) {
            revert PrincipalTokenError("Caller should be the owner of shares");
        }
        if (shares > maxRedeem(owner)) {
            revert PrincipalTokenError("Redeem more than max");
        }
    ...
    function redeem(
        ...
        if (assets < minAssets) {
            revert PrincipalTokenError("ERC5143: Redeem slippage protection");
        }
        return assets;
    }
        ...
        returns (uint256 shares)
    {
        ...
        if (maxAssetsToWithdraw < assets) {
            revert PrincipalTokenError("Input amount exceeds withdraw limit");
        }
        ...
        if (assetsInIBT > yieldInIBT - feesInIBT) {
            ...
            if (yieldInIBT - feesInIBT == 0) {
                revert PrincipalTokenError(
                    "Net yield is 0 whereas supposed to cover the withdraw"
                );
            }
    ...
    function withdraw(
        ...
        if (shares > maxShares) {
            revert PrincipalTokenError("Withdraw slippage protection");
        }
        return shares;
    }
    ...
    function claimFees() external returns (uint256) {
        if (msg.sender != feeCollector) {
            revert PrincipalTokenError("Caller should be FEE_COLLECTOR");
        }
    ...
    function claimYieldOfAmount(
        ...
        if (yieldToTransfer != 0 && (_amount != 0 || userYTBalance == 0)) {
            ...
            if (yieldToTransfer > yieldOfUserInIBT[_user]) {
                revert PrincipalTokenError(
                    "YieldToTransfer exceeds user yield"
                );
            }
    ...
    function beforeYtTransfer(address _from, address _to) external override {
        if (msg.sender != address(yt)) {
            revert PrincipalTokenError("Caller should be a YT");
        }
        updateYield(_from);
        updateYield(_to);
    }
    ...
    function setFeeCollector(address _feeCollector) public {
        ...
        if (_feeCollector == address(0)) {
            revert PrincipalTokenError(
                "Cannot set fee collector to zero address"
            );
        }
        ...
        returns (uint256 _ibtRate, uint256 _ptRate)
    {
        if ((ratesAtExpiryStored)) {
            revert PrincipalTokenError("Rates at expiry already stored");
        }
    ...
    function previewRedeem(
        ...
        if (ratesAtExpiryStored) {
            ...
            revert PrincipalTokenError("Rates at expiry not stored");
        }
    }
    ...
    function getIBTRateAtExpiry() external view override returns (uint256) {
        if (!ratesAtExpiryStored) {
            revert PrincipalTokenError("Rates at expiry not stored yet");
        }
        return ibtRate;
    }
    ...
    function _computeYield(
        ...
        if (_oldPTRate == _ptRate && _ibtRate == _oldIBTRate) {
            ...
            if (_oldPTRate > _ptRate) {
                ...
                revert PrincipalTokenError(
                    "PT rate increased or no PT depeg but IBT rate decreased!"
                );
            }
        }
