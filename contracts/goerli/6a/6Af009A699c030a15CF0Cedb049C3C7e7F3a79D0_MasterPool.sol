...
contract MasterPool is IMasterPool, Ownable {
    ...
    function unstake(uint256 stakingId_) external {
        ...
        uint256 unlockTime = pool.stakeDuration * 30 + pool.stakeTimestamp;
        ...
        if (totalAmount == 0) {
            ...
            for (uint256 i = 0; i < stakingId; i++) {
                if (
                    ...
                    leftDuration = leftDuration / 30 >= stakingPool.stakeDuration ? stakingPool.stakeDuration : leftDuration / 30;
    ...
    function _buyBack(uint256 auctionAmount_) internal {
        uint256 amountForBuyBack = (auctionAmount_ * PERCENT_FOR_BUYBAK) /
            100;
