    ...
    ERC1820Implementer
{
     ...
     *
     * Requirements: onlyOwner can remove Contracts; Contract needs to be ERC721 compliant
        ...
        onlyOwner
    {
        ...
        for (uint256 i; i < walletsToRewardLength; i = unsafe_inc(i)) {
            if (
                ...
                stakers[walletsToRewardInRound[currentRoundId][i]]
                    .unclaimedRewards +=
                    (sharesForWalletInRound[currentRoundId][
                        walletsToRewardInRound[currentRoundId][i]
                    ] / totalShares) *
    ...
    /**
     * @dev Function to estimate rewards for specific token on a contract.
     ...
     * - unaccountedRewards: Unclaimed + Rewards that are not ready to be claimed
     * because the current period did not finish yet.
    ...
    /**
     * @dev Emergency Unstake Function: Unstake without any rewards. (NOT RECOMMENDET)
     *
     * @param _tokens Array of TokenData(contractAddress, tokenId)
     */
