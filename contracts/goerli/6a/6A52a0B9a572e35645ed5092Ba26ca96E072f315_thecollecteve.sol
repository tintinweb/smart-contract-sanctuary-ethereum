...
contract thecollecteve is ERC721A, Ownable, DefaultOperatorFilterer  {
  using SafeMath for uint256;
    ...
    address private wallet1 = 0xB81123539f87Dbc3fa6B63486A41aD628F9D0Fa5; // Wallet 1 - Project
    ...
    uint256 public maxPurchaseWL2 = 4;
    uint256 public maxPurchaseWL3 = 5;
	  uint256 public maxTxWL = 1;
	  uint256 public maxTxVIP = 2;
    uint256 public maxTxWL2 = 4;
    uint256 public maxTxWL3 = 5;
    uint256 public maxTx = 5;

    constructor() ERC721A("thecollecteve", "thecollecteve") {}
    ...
    function whitelistMint2(uint256 numberOfTokens, bytes32[] calldata merkleProof ) payable external callerIsUser {
        require(whitelistSaleIsActive, "Whitelist Sale must be active to mint");
        require(totalSupply().add(numberOfTokens) <= MAX_WL_SUPPLY, "Total WL2 Supply has been minted");
        require(numberOfTokens > 0 && numberOfTokens <= maxTxWL2, "Can only mint upto 1 NFTs in a transaction");
        require(msg.value == WL_PRICE.mul(numberOfTokens), "Ether value sent is not correct");
        require(numberMinted(msg.sender).add(numberOfTokens) <= maxPurchaseWL2,"Exceeds Max mints allowed per whitelisted wallet");
    ...
    function whitelistMint3(uint256 numberOfTokens, bytes32[] calldata merkleProof ) payable external callerIsUser {
        require(whitelistSaleIsActive, "Whitelist Sale must be active to mint");
        require(totalSupply().add(numberOfTokens) <= MAX_WL_SUPPLY, "Total WL3 Supply has been minted");
        require(numberOfTokens > 0 && numberOfTokens <= maxTxWL3, "Can only mint upto 1 NFTs in a transaction");
        require(msg.value == WL_PRICE.mul(numberOfTokens), "Ether value sent is not correct");
        require(numberMinted(msg.sender).add(numberOfTokens) <= maxPurchaseWL3,"Exceeds Max mints allowed per whitelisted wallet");
