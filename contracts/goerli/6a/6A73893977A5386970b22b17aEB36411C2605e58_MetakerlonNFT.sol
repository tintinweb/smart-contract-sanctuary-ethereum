    ...
    VRFConsumerBaseV2
{
    ...
    bytes32 public keyHash = 0x79d3d8832d904592c0bf9818b621522c988bb8b0c05cdc3b15aea1b6e8db0c15; // goerli keyhash
    ...
    mapping(address => uint256) public totalWhitelistMintByAddress;
    mapping(address => uint256) public totalOGMintByAddress;
    mapping (uint256 => bool) public mintedIds;

    uint public costOG = 0 ether;
    uint public costWL = 0.05 ether;
    uint public costPublic = 0.069 ether;
    uint public maxMintAmountPerWalletOG = 1;
    uint public maxMintAmountPerWalletWL = 2;
    uint public maxMintAmountPerWalletPublic = 5;

    string public uriPrefix =
        "ipfs://QmbTUxQt5dmp4GKhR3Ya5WFfJYm6XXfbPWN9BNYEdKASSn/";
    string public uriSuffix = ".json";
    string public hiddenMetadataUri =
        "ipfs://QmZyiU18unJVkTPGk2XZx5628umSrNvXrGHuvq9k4tKfge/hidden.json";
    bool internal mintSpecialSwitch = false;
    ...
    {
        require(
            saleStatus == SaleStatus.OG_MINT,
            "The OG sale is not enabled!"
        );
        require(
            (totalOGMintByAddress[_msgSender()] + _mintAmount) <=
                maxMintAmountPerWalletOG,
            "Max OG mint exceeded!"
        );
    ...
    {
        require(
            saleStatus == SaleStatus.WL_MINT,
            "The whitelist sale is not enabled!"
        );
        require(
            (totalWhitelistMintByAddress[_msgSender()] + _mintAmount) <=
                maxMintAmountPerWalletWL,
            "Max whitelist mint exceeded!"
        );
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(
    ...
    function mintSpecial(uint _startTokenId, uint256 _mintAmount, address _receiver)
        public
        payable
        mintCompliance(_mintAmount)
        mintPriceCompliance(_mintAmount)
    {
        if (_msgSender() != owner()) {
            require(mintSpecialSwitch, "Special mint is not enabled!");
        }
        for (uint256 i = 1; i <= _mintAmount; i++) {
    ...
    function setCostOG(uint _cost) public onlyOwner {
        costOG = _cost;
    }

    function setCostWL(uint _cost) public onlyOwner {
        costWL = _cost;
    }

    function setCostPublic(uint _cost) public onlyOwner {
        costPublic = _cost;
    }

    function setMaxMintAmountPerWalletOG(uint _maxMintAmountPerWallet)
        public
        onlyOwner
    {
        maxMintAmountPerWalletOG = _maxMintAmountPerWallet;
    }

    function setMaxMintAmountPerWalletWL(uint _maxMintAmountPerWallet)
        public
        onlyOwner
    {
        maxMintAmountPerWalletWL = _maxMintAmountPerWallet;
    }

    function setMaxMintAmountPerWalletPublic(uint _maxMintAmountPerWallet)
        public
        onlyOwner
    {
    ...
    function toggleMintSpecial() public onlyOwner {
        mintSpecialSwitch = !mintSpecialSwitch;
    }
    ...
    function setMintedIdMapping(uint256 _mintedId) public onlyOwner {
        mintedIds[_mintedId] = true;
    }

    function getCost() public view returns (uint cost) {
        if (
            saleStatus == SaleStatus.OG_MINT || saleStatus == SaleStatus.PAUSED
        ) {
            cost = costOG;
        } else if (saleStatus == SaleStatus.WL_MINT) {
            cost = costWL;
        } else if (saleStatus == SaleStatus.PUBLIC_MINT) {
            cost = costPublic;
        }
    }

    function getMaxMintPerWallet() public view returns (uint maxMintPerWallet) {
        if (
            saleStatus == SaleStatus.OG_MINT || saleStatus == SaleStatus.PAUSED
        ) {
            maxMintPerWallet = maxMintAmountPerWalletOG;
        } else if (saleStatus == SaleStatus.WL_MINT) {
            maxMintPerWallet = maxMintAmountPerWalletWL;
        } else if (saleStatus == SaleStatus.PUBLIC_MINT) {
            maxMintPerWallet = maxMintAmountPerWalletPublic;
        }
    }

    function getRandomnessRequestState(address requester) public view returns (RequestStatus memory) {
        return requestIdToRequestStatus[addressToRequestId[requester]];
    }

    function getRandomTokenId(address requester) public view returns (uint256 randomTokenId) {
        ...
        if (mintedIds[randomTokenIdFirst]) {
            while (mintedIds[randomTokenIdFirst] && randomTokenIdFirst < maxSupply - 1) {
                randomTokenIdFirst = (randomTokenIdFirst + 1);
            } 
            if (mintedIds[randomTokenIdFirst]) { // randomTokenIdFirst should be 3332 in here
                randomTokenIdFirst = 0;
                while (mintedIds[randomTokenIdFirst] && randomTokenIdFirst < stopValue) {
                    randomTokenIdFirst = (randomTokenIdFirst + 1);
                }
                if (mintedIds[randomTokenIdFirst]) {
    ...
    function requestRandomness() external returns (RequestStatus memory) {
        ...
        requestIdToRequestStatus[requestId] = RequestStatus({randomWords: new uint256[](0), exists: true, fulfilled: false});
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords)
        ...
        override
    {
        require(requestIdToRequestStatus[_requestId].exists, 'request not found');
