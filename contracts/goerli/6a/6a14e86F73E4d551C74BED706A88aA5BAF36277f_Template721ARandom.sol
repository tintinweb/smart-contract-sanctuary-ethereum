...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    event MintTokenIdToAddress(uint256, address);
    ...
    function _mint(
        ...
        unchecked {
            ...
            emit MintTokenIdToAddress(startTokenId, to);
...
abstract contract OperatorFilterer {
    ...
    bool public disableOperatorFilter = false;
...
contract Template721ARandom is RegisterFilter721, Initializable, Pausable {
    ...
    function mint(uint256 amount) external payable whenNotPaused {
        ...
        // require(useReserveForTeam && _isTeamNFTsMinted, "Contract is not ready for public mint yet");
        //require(!_isTeamNFTsMinted, "Contract is not ready for mint.");
    ...
    function claimAirdrop() public {
        // require(useReserveForAirdrop, "UseReserveForAirdrop is inactive");
        require(isAirdropActive, "Airdrop is inactive");
        require(isAddressInAirdropWhitelist(msg.sender), "You not have airdrops to claim");

        uint256 quantity = 1;
        do {
            require(totalMinted() + quantity <= maxSupply, "Max supply exceeded");
            ...
            removeAirdropWhitelistAddress(msg.sender);
            claimedAirdropAddresses.push(msg.sender);

        } while (isAddressInAirdropWhitelist(msg.sender));
    }
    ...
    function isAddressInAirdropWhitelist(address _address) public view returns (bool) {
        for (uint i = 0; i < airdropWhitelistAddresses.length; i++) {
            if (airdropWhitelistAddresses[i] == _address) {
                return true;
            }
        }
        return false;
    ...
    function addAirdropWhitelistAddresses(address[] calldata _addresses) external onlyOwner {
        uint256 count = airdropWhitelistAddresses.length + _addresses.length + claimedAirdropAddresses.length;
        require(count <= airdropNFTsCount, "Max supply exceeded");
        for (uint i = 0; i < _addresses.length; i++) {
            airdropWhitelistAddresses.push(_addresses[i]);
        }
    }

    function removeAirdropWhitelistAddress(address _address) internal {
        for (uint i = 0; i < airdropWhitelistAddresses.length; i++) {
            if (airdropWhitelistAddresses[i] == _address) {
                airdropWhitelistAddresses[i] = airdropWhitelistAddresses[airdropWhitelistAddresses.length - 1];
                airdropWhitelistAddresses.pop();
                break;
            }
        }
    }

    function removeAirdropWhitelistAddresses(address[] calldata _addresses) external onlyOwner {
        require(airdropWhitelistAddresses.length > 0, "Empty airdrop whitelist");

        for (uint i = 0; i < _addresses.length; i++) {
            address _address = _addresses[i];

            uint j = 0;
            while(j < airdropWhitelistAddresses.length) {
                if (airdropWhitelistAddresses[j] == _address) {
                    airdropWhitelistAddresses[j] = airdropWhitelistAddresses[airdropWhitelistAddresses.length - 1];
                    airdropWhitelistAddresses.pop();
                } else {
                    j++;
                }                
  ...
  function getMintTransection(address user_ , uint index_) public view returns(uint[] memory){
   ...
   function getLastedMintTransection(address user_) public view returns(uint[] memory){
    ...
    function isClaimedAirdrop(address address_)  public  view  returns(bool) {
        bool result = false;
               for (uint i = 0; i <  claimedAirdropAddresses.length; i++) {
                    if(claimedAirdropAddresses[i] ==  address_){
                         result = true;
                    }
            }
        return result;
