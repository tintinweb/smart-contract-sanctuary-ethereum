...
contract RandomTestNewMix2 is ERC721A, ERC721AQueryable, ERC721APausable, ERC721ABurnable, Ownable, ReentrancyGuard {

  string public        baseURI;
  uint public          mintWinner;
  uint public          Winner;
  ...
  constructor() ERC721A("RandomTestNewMix2", "RandomTestNewMix2"){}
  ...
  function mint(uint256 amt) whenNotPaused external payable
  {
    uint cost = price;
...
uint[] private mintWinnersList; 


    function tellMintWinner() external onlyOwner {
        mintWinner =  uint256 (keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
        msg.sender))) % maxSupply;
        mintWinnersList.push(mintWinner);
    }

    function getMintWinners() external view returns(uint[] memory) {
        return mintWinnersList;
    }

    uint[] private WinnersList; 


    function tellWinner() external onlyOwner {
        Winner =  uint256 (keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
        msg.sender))) % maxSupply;
  ...
  function _setOwnersExplicit(uint256 quantity) internal {
      ...
      require(_nextOwnerToExplicitlySet < _currentIndex, "all ownerships have been set");
