...
contract CrossFarming is MessageSenderApp, MessageReceiverApp {
    ...
    /// @notice executor receiver which call cross-chain receiver contract on dest chain for cross-chain users.
    ...
    function sendFarmMessage(
        ...
        // uint256 destFee = (tx.gasprice * estimateGaslimit(_account, _msgType) * exchangeRate * compensationRate) /
        // (EXCHANGE_RATE_PRECISION * COMPENSATION_PRECISION);

        uint256 fee = IMessageBus(messageBus).calcFee(message);
        // receiver.transfer(feeBNB);
        // stack too deep, the dest chain fee local variable
        require(
            msg.value >=
                fee +
                    (tx.gasprice * estimateGaslimit(_account, _msgType) * exchangeRate * compensationRate) /
                    (EXCHANGE_RATE_PRECISION * COMPENSATION_PRECISION),
            "insufficient fee"
        );
