...
contract Token is ERC20, AccessController {

  bytes32 private constant MINTER_ROLE = "MINTER_ROLE";
  bytes32 private constant BURNER_ROLE = "BURNER_ROLE";
  ...
  AccessController(
    ...
    _mint(_initialHolder, _initialAmount);
  }

  function mint(address _recipient, uint _amount)
  external
  onlyRole(MINTER_ROLE, msg.sender) {
    _mint(_recipient, _amount);
  }

  function burn(address _from, uint _amount)
  external
  onlyRole(BURNER_ROLE, msg.sender) {
    _burn(_from, _amount);
  }
