...
contract LandNFT is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    function setMintList(
        ...
        onlyOwner
    {
        // 1)Owner権限から販売可能トークンIDリスト(tokenMerkleRoot)を更新する
        // 2)リスト更新イベントを発火
    }
    */
    ...
    function buy(
        ...
        // TODO: 4)購入金額をチェック
        // ・ユーザがディスコードロール登録者であれば、代金xx%引き
        // ⇒大名：5% / 侍・忍者：3% / 農民・商人：2%
        // ★ロールの割引率が変更されるか、もしくは追加・変更を今後見込む必要があるか、登録者何名想定でいればよいか、ロールの設定ルールは？
        // ・それ以外であれば、通常通り500USDC/1land
        ...
        onlyOwner
    {
        for (uint256 i; i < count; i++) {
            // 1)引数のtokenIdsとnonMint_tokenIdsを比較し、想定したTokenID以外が存在しないかチェック
            require(isNonMintTokenIds[tokenIds[i]], "LandNFT: mintTo: Error.");
            // 2)事前に用意したairdrop対象者のaddressと購入数リストを引数にして、対象者へowner側からLandをmintする
            // 3)トークンと購入者アドレスをマッピング（mapping(uint256 => address) private _owners;）
            // ※airdropなので代金は頂かない
            // ※total_supply を1加算していけない
            // ※上限は設けない
    ...
    function transferTokens(uint256 _mintNum, uint256 _tokenPrice) internal {
        for (uint256 i; i < _mintNum; i++) {
            for (uint256 index; index < MAX_SUPPLY_MINT; index++) {
                // ・TokenIdのリスト格納前にランダムで設定してリストの上から発行
                uint256 tokenId = tokenIdList[tokenIdListIndex];

                if (!isNonMintTokenIds[tokenId]) {
                    // エアドロ用の除外リストのIdが次に発行するTokenIdだった場合、リスト上から飛ばして、次のIdを見に行く
                    // ⇒このとき、TokenIdリストのインデックスをインクリメントする。
                    _safeMint(msg.sender, tokenId);

                    // 下記の処理は、ERC721EnumerableUpgradeableで対応
                    // ・既に払い出し済のLANDではないかどうかをチェッeク
                    // ・トークンとユーザアドレスのマッピング（mapping(uint256 => address) private _owners;）
                    // ・total_supplyを1加算する
                    tokenIdListIndex++;
                    break;
                } else {
