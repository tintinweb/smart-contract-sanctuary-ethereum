...
interface IDrawCalculator {
     ...
     */
    function calculateNumberOfUserPicks(address user, uint32[] calldata drawIds)
        external
        view
 ...
 * @author Asymetrix Protocol Inc Team
 * @notice The PrizeDistributor contract holds Tickets (captured interest) and distributes tickets to users with winning draw claims. An admin account can indicate the winners that will receive the payment of the prizes
 */
contract PrizeDistributor is Initializable, IPrizeDistributor, Ownable {
     ...
     */
    function initialize(address _owner, IERC20Upgradeable _token)
        public
        initializer
    {
        __PrizeDistributor_init_unchained(_owner, _token);
    }
    ...
    /* ============ External Functions ============ */
    /**
     * @notice Allows the payment to the winners of an unpaid draw.
     * @param _drawId          DrawId uint32
     * @param _winners         Winners[] address
     */
    function payWinners(uint32 _drawId, address[] memory _winners)
        external
        onlyOwner
    {
        // TODO: make sure the _drawId is unpaid
        // Make sure the addresses of _winners are not zero
        // Make sure the length of the addresses is equals to the amount of places/tiers
        uint256 totalPayout = 0; // Obtained from the available yield balance
        for (uint32 i = 0; i < _winners.length; i++) {
            uint256 _amount = 0; // TODO: Specify this amount.
            _awardPayout(_winners[i], _amount);
        }
        emit PaidDraw(_drawId, totalPayout, _winners, uint64(block.timestamp));
    }
