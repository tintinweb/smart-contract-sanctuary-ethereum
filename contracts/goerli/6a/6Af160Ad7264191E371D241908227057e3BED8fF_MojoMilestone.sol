...
import "./interface/IMojoTokenClaim.sol";

contract MojoMilestone is ERC1155Upgradeable, AccessControlUpgradeable, ERC165StorageUpgradeable, IERC2981Upgradeable {
    ...
    IMojoTokenClaim mojoTokenClaim;

    function initialize(string memory uri_, address royaltyAddress_, uint256 royaltyPercentage_, address mojoClaimAddress) external initializer {
        ...
        mojoTokenClaim = IMojoTokenClaim(mojoClaimAddress);
    ...
    function _beforeTokenTransfer(
        ...
        for (uint i = 0; i < ids.length; i++) {
            mojoTokenClaim.onTransfer(from, to, ids[i], amounts[i]);
        }
    }
}
...
interface IMojoTokenClaim {
    ...
    event TokenWeightUpdated(uint256 indexed tokenId, uint256 weight);

    function onTransfer(address from, address to, uint256 tokenId, uint256 amount) external;
}
