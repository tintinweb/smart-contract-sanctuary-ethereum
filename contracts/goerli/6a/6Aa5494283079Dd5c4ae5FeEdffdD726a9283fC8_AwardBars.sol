...
contract AwardBars is ReentrancyGuard, ERC20, OnlyAuthorized, IOperableAward {
    bool private development = true; //Allows you to change contract addresses or make allowed changes without having to redeploy
    IOilWell private oilWell;
    IOperable private bars;
    uint256[] private _awards;

    event Exploded(uint256 amount, uint256 rewardsCru, address indexed fortunate);

    constructor() ERC20("AwardBars", "A-BAR") {}

    function mint(uint256 amount) external {
        require(
            msg.sender == address(bars),
            "only the bars can create award bars"
        );
    ...
    function raise(uint256 amount) public view returns (uint256) {
        return amount * 10 ** decimals();
    }

    function setBarsAddress(IOperable _bars) external {
        require(address(_bars) != address(0), "Address cannot be zero");
        require(
            address(bars) == address(0) || development,
            "You can't change the address again"
        );
        bars = _bars;
    }

    function setOilWellAddress(address oilwellAddress) external onlyAuthorized {
        require(oilwellAddress != address(0), "Address cannot be zero");
        require(
            address(oilWell) == address(0) || development,
            "You can't change the address again"
        );
        oilWell = IOilWell(oilwellAddress);
    }

    function explodeBars(uint256 amount) external nonReentrant {
        uint256 cruAmount = 0;
        uint256 playerBalance = balanceOf(msg.sender);
        uint256 barsAmount = raise(amount);

        require(
            barsAmount <= playerBalance,
            "You don't have enough award bars"
        );

        for (uint256 i = 0; i < amount; i++) {
            cruAmount += _awards[_awards.length - 1];
            _awards.pop();
        }

        _burn(msg.sender, barsAmount);

        oilWell.explodeBars(cruAmount, msg.sender);

        emit Exploded(amount, cruAmount, msg.sender);
    }

    function setAwardSupply(uint256 supply) external {
        require(msg.sender == address(bars), "unauthorized call");
        _awards.push(supply);
    }
