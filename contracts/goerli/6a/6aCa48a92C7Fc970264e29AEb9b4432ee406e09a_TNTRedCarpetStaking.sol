...
contract TNTRedCarpetStaking is BaseContract, ERC721Upgradeable
{
    /**
    ...
    {
        __BaseContract_init();
        __ERC721_init("TNT Red Carpet", "TNTRC");
    }
    ...
    uint256 public totalTntBurned;
    uint256 public currentTntStaked;
    uint256 public totalUsdcClaimed;
    uint256 public outstandingUsdc;
    uint256 public currentlyServing;
    ...
    mapping(uint256 => uint256) public tokenTntValue;
    mapping(uint256 => uint256) public tokenUsdcValue;
    mapping(uint256 => uint256) public tokenUsdcEarned;

    /**
     * Events.
     */
    event TokenCreated(uint256 indexed id, uint256 tntValue, uint256 usdcValue);
    event TokenClaimed(uint256 indexed id, uint256 usdcValue);

    function setup() external override
    {
        tnt = IERC20(addressBook.get("tnt"));
        usdc = IERC20(addressBook.get("usdc"));
        swap = ITNTSwap(addressBook.get("swap"));
    }

    /**
     * Token exists.
     * @param id_ Token IDimport "../interfaces/IAutoCompound.sol";
     * @return bool True if token exists.
     */
    function tokenExists(uint256 id_) public view returns (bool)
    {
        return id_ > 0 && id_ <= _tokenIdTracker && tokenClaimed[id_] == 0;
    }
    ...
    {
        require(balanceOf(owner_) > index_, "TNTRedCarpetStaking: Index out of bounds");
    ...
    {
        require(tokenExists(id_), "TNTRedCarpetStaking: Token does not exist");
        return _tokenOwner[id_];
    }

    function stake(uint256 amount_) external
    {
        require(tnt.transferFrom(msg.sender, address(this), amount_), "TNTRedCarpetStaking: USDC transfer failed");
        _tokenIdTracker++;
        totalSupply++;
        totalTntStaked += amount_;
        currentTntStaked += amount_;
        uint256 _usdcValue_ = swap.sellOutput(amount_) * 2;
        outstandingUsdc += _usdcValue_;
        _tokenBalance[msg.sender]++;
        _tokenOwner[_tokenIdTracker] = msg.sender;
        tokenCreated[_tokenIdTracker] = block.timestamp;
        tokenTntValue[_tokenIdTracker] = amount_;
        tokenUsdcValue[_tokenIdTracker] = _usdcValue_;
        _mint(msg.sender, _tokenIdTracker);
        emit TokenCreated(_tokenIdTracker, tokenTntValue[_tokenIdTracker], tokenUsdcValue[_tokenIdTracker]);
    }
}
