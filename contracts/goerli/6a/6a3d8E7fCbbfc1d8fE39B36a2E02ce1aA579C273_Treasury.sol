...
contract Treasury is Ownable {
    ...
    IAsset public IAssetNether;
    IAsset public IAssetWeth;
    IAsset public Zero_ETH;

    uint256 private seniorageThreshold = 1010000000000000000;
    uint256 private bondThreshold = 980000000000000000;
    ...
    event BondsIssued(address, uint256);
    event BondsBurned(uint256);
    event Seniorage(uint256);



    function setTokenParams(address _nether, address _core, address _nBond, address _balancerWeth,
    address _zero_ETH) external onlyOwner {
        nether = INether(_nether);
        balancerWeth = IERC20(_balancerWeth);
        core = IERC20(_core);
        nBond = INBond(_nBond);
        Zero_ETH = IAsset(_zero_ETH);
        IAssetNether = IAsset(_nether);
        IAssetWeth = IAsset(_balancerWeth);
    }



    function setBalancerPoolParams(address _vault, bytes32 _poolId) external onlyOwner {
        poolId = _poolId;
        vault = IVault(_vault);
    }


    function setNetherParams(address _oracle, address _weaver) external onlyOwner {
        oracle = NetherOracle(_oracle);
        weaver = IWeaver(_weaver);
    }
    ...
    function sellNBonds(uint256 _amount) external payable {
        ...
        nBond.mint(sender, mintNBonds);
        balancerWeth.approve(address(vault), _amount);
        uint256 receivedNether = trade(IAssetWeth, IAssetNether, _amount);
        nether.burn(receivedNether);
        emit BondsIssued(msg.sender, mintNBonds);
    }
    ...
    function seniorage() external {
        ...
        nether.mint(address(this), mintAmount);

        uint256 receivedWETH = trade(IAssetNether, IAssetWeth, mintAmount);
        balances = getPoolBalances();
        assert(balances[1] > balances[0]);
                     if(bonusExpansion > 0) {
                    nether.mint(address(this), bonusExpansion * mintAmount / 100);
                    receivedWETH += trade(IAssetNether,IAssetWeth,bonusExpansion * mintAmount / 100);
                }
        ...
        if (bondAllocation < circulatingBondAmount) {
            ...
            if (receivedWETH > uncollateralizedBond) {
                ...
                balancerWeth.approve(address(weaver),receivedWETH-uncollateralizedBond);
                weaver.allocateSeigniorage(receivedWETH-uncollateralizedBond);
            } else {
                bondAllocation += receivedWETH;
            }
        } else if (bondAllocation >= circulatingBondAmount) {
            balancerWeth.approve(address(weaver),receivedWETH);
            weaver.allocateSeigniorage(receivedWETH);
        }
        deltas[0] = 0;
        deltas[1] = 0;
        emit Seniorage(receivedWETH);
    }
    ...
    function getSwapFee() external returns (uint256) {
        ...
        for (uint256 i = 3; i >= 0; i--) {
            if (netherPrice < swapFeePriceLevels[i]) {
                ...
                break;
            } else if (netherPrice > bondThreshold) {
                poolSwapFee = 50000000000000000;
            }
        }
        return poolSwapFee;
    }
    ...
    function reAllocateBondFundsWETH(address _address) external onlyOwner {
        uint256 wethBalance = balancerWeth.balanceOf(address(this));
        balancerWeth.transfer(_address,
         wethBalance);
    }

    function reAllocateBondFunds(address _address) external onlyOwner {
        uint256 ethBalance = address(this).balance;
        bool sent = payable(_address).send(ethBalance);
...
contract NetherOracle is Epoch {
    ...
    uint256 public poolImbalance;

    constructor(
        uint256 _epochPeriod
    ) Epoch(_epochPeriod, block.timestamp, 0) {
    ...
    function setBalancerParams(address _pool, address _vault, bytes32 _poolId) external onlyOperator {
        pool = IMetaStablePool(_pool);
        vault = IVault(_vault);
        poolId = _poolId;
    }
    ...
    function getTwap() external checkEpoch returns (uint256) {
        ...
        if (oracleIndex < 1024) {
            (, uint256[] memory balances) = getPoolBalances();
            if ((balances[0] * poolImbalance) / 100 < balances[1]) {
                twap = 2 ether;
                return twap;
            }
        } else {
            IMetaStablePool.OracleAverageQuery[]
                memory queries = new IMetaStablePool.OracleAverageQuery[](1);
            IMetaStablePool.OracleAverageQuery memory query = IMetaStablePool
                .OracleAverageQuery({
