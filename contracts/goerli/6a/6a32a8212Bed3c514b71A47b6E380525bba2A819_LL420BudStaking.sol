...
contract LL420BudStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdrawToWallet(uint256 _amount, uint256 _from) external nonReentrant {
        ...
        require(_from == 0 && user.reward > _amount, "No bud reward to withdraw");
        ...
        if (_from == 0) user.reward -= _amount;
        else gkRewardOf[_msgSender()] -= _amount;
    ...
    function _pendingGKReward(address _user) internal view returns (uint256) {
        ...
        /// calculate reward per each duration, 1080 seconds 2160 x 40 = 3600 * 24
        /// reward per day 10,00 $HIGH, 25 per every 2160 sec
        UserInfo memory user = _userInfo[_user];
        uint256 checkpoint = user.lastCheckpoint > 1676782770 ? user.lastCheckpoint : 1676782770;
        return ((block.timestamp - checkpoint) / 1080) * 25 * user.gameKeyCount;
    }
