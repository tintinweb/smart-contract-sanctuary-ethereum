...
contract Commission is Ownable, Pausable {
	...
	mapping(address => Claim[]) public historyCommission;
	...
	function claim(
		...
		historyCommission[_to].push(Claim(_amount, block.timestamp));
		emit ClaimEvent(_to, _amount, block.timestamp, _message);
	...
	function getHistoryCommissionLength(address _user)
	public
	view
	returns (uint256)
	{
		return historyCommission[_user].length;
	}
