...
contract NFToken is ERC721, SupportsInterface {
    ...
    function ownerOfInternal(uint256 _tokenId)
        internal
        view
        returns (address _owner)
    {
...
contract AnimeMetaverseMysteryBox is NFTokenMetadata, Ownable {
    ...
    /**
     * @dev Map of how many nfts are minted per address.
     */
     ...
     */
    uint256 public maxAirdropLimit = 100;

    /**
     * @dev Base URI for NFT metadata.
     */
    string private metaAddress = "https://example.server.com/metadata/";

    /**
     * @dev Postfix for metadata URI of NFT
     */
    string private constant jsonAppend = ".json";
    ...
    /**
     * @dev constractor for AnimeMetaverseMysteryBox. During the contract creation set the tokenName
     * and tokenSymbol.
     */
    constructor() {
        nftName = "Soulmate Giftbox";
        nftSymbol = "AMMB";
    }

    /**
     * @dev validate tokenID and token exists or not. Provide the tokenURI for the requested tokenID.
     * Token URIs are generated dynamically on view requests. This is to allow easy server changes and
     * reduce gas fees for minting. -ssa2
     * @param tokenID NFT token ID for getting tokenURI.
     */
    ...
    /**
     * @dev Get how many nfts are minted by a particular address.
     */
    ...
    function setMaxAirdropLimit(uint256 _airdropLimit) external onlyOwner {
        ...
        /**
         * @dev Throws if airdropLimit is more than maxNft.
         */
        require(_airdropLimit <= maxNft, "Can not set airdropLimit more than maxNft.");
        
        maxAirdropLimit = _airdropLimit;
    }

    /**
     * @notice The call (contract owner) is responsible for providing '_receiver' address and '_nftCount'.
     * @dev Airdrop a requested number of NFT to the reveiver address. Validate the requested number of
     * airdrop is greater than max suppy or not.
    ...
    function burn(uint256 _tokenId) external {
        address onwerOfToken = this.ownerOf(_tokenId);
        require(msg.sender == onwerOfToken, "Sender is not the owner of the token.");
        super._burn(_tokenId);
    }

    /**
     * @notice The caller (Contract Owner) is responsible to confirm that `_URI` is the base URL for
     * token metadata and it provides metadata with format <_URI><tokenID>.json
    ...
    /**
     * @dev Takes care of converting an integer into the raw bytes of it's abi-encodable string.
     * @param v NFT token ID to calculate byte.
     */
