...
contract Escrow is IEscrow, Initializable, ReentrancyGuard, Multicall {
    ...
    event Confirmed(address sender, address token, uint256 amount, uint8 vote);
    event Released(address to, address token, uint256 amount, uint256 released);
    event Refunded(address to, address token, uint256 amount);
    ...
    function deposit(uint256 _releaseAmount)
        external
        payable
        onlyInitialized
        nonReentrant
    ...
    function resetVotes(
        ...
        isConfirmed[_type][hash][provider] = false;
        ...
        nonReentrant
    {
        if (countVotes(REFUND, address(0), _amount)) {
            ...
            emit Refunded(client, address(0), _amount);
        }
    }

    /// @notice Refunds tokens to client
    function refund(address _token, uint256 _amount)
        external
        override
        onlyParty
        onlyInitialized
        nonReentrant
    {
        if (countVotes(REFUND, _token, _amount)) {
            ...
            emit Refunded(client, _token, _amount);
        }
    }

    /// @notice Releases eth to provider and 15% to resolver
    function release(uint256 _amount)
        ...
        nonReentrant
    {
        bool isClient = msg.sender == provider;
        if (isClient || countVotes(RELEASE, address(0), _amount)) {
            _release(_amount);
        }
    }

    /// @notice Releases tokens to provider and 15% to resolver
    function release(address _token, uint256 _amount)
        public
        override
        onlyParty
        onlyInitialized
    ...
    function _release(uint256 _amount) internal {
        released += _amount;
        emit Released(provider, address(0), _amount, released);
        uint256 resolverShare = calcShare(_amount);
        Address.sendValue(payable(resolver), resolverShare); // 15% to resolver
        Address.sendValue(payable(provider), _amount - resolverShare); // 85% to provider
    }

    function _release(address _token, uint256 _amount) internal {
        uint256 resolverShare = (_amount / 10_000) * 1500;
        IERC20(_token).safeTransfer(resolver, resolverShare); // 15% to resolver
        IERC20(_token).safeTransfer(provider, _amount - resolverShare); // 85% to provider

        released += _amount;
        resetVotes(RELEASE, _token, _amount);
        emit Released(provider, _token, _amount, released);
    }
    ...
    function calcShare(uint256 _amount) internal returns (uint256) {
        return (_amount / 10_000) * 1500;
    }
}
