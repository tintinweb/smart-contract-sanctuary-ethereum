...
contract NFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable {
    /**
    ...
    *
    * @dev If revealURIMode is true, all token id will use this as their token URI
    *
    */
    ...
    */
    ...
    string previewURI = "ipfs://QmfWNxuDRUGqxCDQJiUW5B7BFuxuTK5cjMXEmWwg9BhyVn/";
    ...
    */
    ...
    modifier isExactAmount(uint256 _amount) {
        require(msg.value == _amount, "ERR: IC"); // Incorrect Price
        _;
    }

    modifier isMaxSupplyNotReached() {
        require(totalSupply() <= MAX_SUPPLY, "ERR: MR"); // Max Reached
        _;
    }
    ...
    function safeMint(uint256 tokenId, address to) 
    public payable
    whenNotPaused
    isMaxSupplyNotReached
    isExactAmount(mintPrice)
    {       
    ...
    function enableURIMode() external onlyOwner {
        revealURIMode = true;
    }

    function disableURIMode() external onlyOwner {
        revealURIMode = false;
    }
        ...
        returns (string memory)
    {
        ...
        if (revealURIMode) {
            return bytes(_tokenBaseURI).length > 0 ? string(abi.encodePacked(_tokenBaseURI, Strings.toString(tokenId), tokenURISuffix)) : "";
        } else {
            return string(abi.encodePacked(previewURI, "hidden", tokenURISuffix));
        }
    }

    function setBaseTokenURI(string memory _tokenBaseURI)
        external
        onlyOwner
    {
