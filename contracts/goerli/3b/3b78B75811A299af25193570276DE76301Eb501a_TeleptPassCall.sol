...
interface WhitelistContract {
    function invitationList(address addr) external view returns (bool);

    function balanceOf(address owner) external view returns (uint256);
}

contract TeleptPassCall is ERC721URIStorage, Ownable {
    ...
    string baseURI = "https://pass-test.telept.xyz/token/";

    mapping(address => uint256) mintCount;

    WhitelistContract whitelistContractInstance;

    constructor() ERC721("TeleptPassCall", "TELEPT") {}
    ...
    function mintInternal(address to) internal returns (uint256) {
        ...
        if (requireInvitation) {
            require(
                checkIsInInvitationListByOtherContract(to),
                "not in the invitation list of the whitelist contract"
            );
        }
    ...
    function instaniateWhitelistContract(
        address whitelistContractAddress
    ) public onlyOwner {
        whitelistContractInstance = WhitelistContract(whitelistContractAddress);
    }

    function checkIsInInvitationListByOtherContract(
        address addr
    ) public view returns (bool) {
        bool isInWhite = whitelistContractInstance.invitationList(addr);
        return isInWhite;
    }

    function checkHasWhitelistNft(address addr) public view returns (bool) {
        uint256 balance = whitelistContractInstance.balanceOf(addr);
        return balance > 0;
    }
