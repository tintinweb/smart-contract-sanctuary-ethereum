...
contract testMap is ERC721A, ReentrancyGuard, Ownable, DefaultOperatorFilterer {
    constructor(string memory _customBaseURI) ERC721A( "testMap", "MAP" ){ customBaseURI = _customBaseURI; }


    mapping(address => uint256) public mintedAmount;
    mapping(address => uint256) public mintedFree;
    uint[] public itemClasses;
    uint classMIN = 0;
    uint classMAX = 3;
    uint256[] public priceByClassWL = [  10000000000000,  20000000000000,  30000000000000,  40000000000000 ];
    uint256[] public priceByClass   = [ 100000000000000, 200000000000000, 300000000000000, 400000000000000 ];
    ...
    function addItems( uint _count, uint _class ) internal {
	for (uint256 i = 0; i < _count; i++) {
	    itemClasses.push( _class );
	}
    }


    function mintOne(address _to, uint256 _count, uint _class) external onlyOwner {
	...
	addItems( _count, _class );
    }


    function mintWL(uint256 _count, uint _class, bytes32[] memory _proof) public payable nonReentrant {
	bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
	...
	uint256 _total = 0;
	if( _freeAmount == 0 && _class == 0 ){
	    _total = priceByClass[_class] * ( _count - 1 );
	}else{
	    _total = priceByClass[_class] * ( _count );
	}
	require( msg.value >= _total, "Insufficient payment"  );
    ...
    function mint(uint256 _count, uint _class) public payable nonReentrant {
    ...
    function tokenClass( uint _id ) public view returns (uint) {
	if( _id < itemClasses.length ){
	    return itemClasses[_id];
	}else{
	    return 666;
	}
    }
