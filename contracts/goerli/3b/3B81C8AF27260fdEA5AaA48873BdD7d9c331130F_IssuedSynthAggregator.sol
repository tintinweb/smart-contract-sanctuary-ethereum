...
contract IssuedSynthAggregator is Owned, MixinSystemSettings, IIssuedSynthAggregator {
    using SafeMath for uint256;
    using SafeDecimalMath for uint256;

    bytes32 public constant CONTRACT_NAME = "IssuedSynthAggregator";

    ISynthAggregator[] public availableSynthAggregators;
    mapping(bytes32 => ISynthAggregator) public syAggregators;
    mapping(address => bytes32) public syAggregatorsByAddress;
    ...
    function totalIssuedSynths() external view returns (uint256 _issuedSynths) {
        for (uint256 ii = 0; ii < availableSynthAggregators.length; ii++) {
            ISynthAggregator syAggregator = availableSynthAggregators[ii];
            bytes32 currencyKey = syAggregatorsByAddress[address(syAggregator)];
            if (currencyKey == sUSD) {
                uint256 totalSynths = syAggregator.totalSupply();
                _issuedSynths = _issuedSynths.add(totalSynths);
            } else {
                (uint256 currencyRate,) = exchangeRates().rateAndInvalid(currencyKey);
                uint256 totalSynths = syAggregator.totalSupply();
                _issuedSynths = _issuedSynths.add(totalSynths.multiplyDecimal(currencyRate));
            }
        }
    }
    ...
    function totalIssuedSynthsPerIssuer(address account) external view returns (uint256 _issuedSynths) {
        for (uint256 ii = 0; ii < availableSynthAggregators.length; ii++) {
            ...
            bytes32 currencyKey = syAggregatorsByAddress[address(syAggregator)];
            if (currencyKey == sUSD) {
                uint256 totalSynths = syAggregator.balanceOf(account);
                if (totalSynths > 0) {
                    _issuedSynths = _issuedSynths.add(totalSynths);
                }
            } else {
    ...
    function totalIssuedSynthPerAsset(bytes32 currencyKey) external view returns (uint256 _issuedSynth) {
        ISynthAggregator syAggregator = syAggregators[currencyKey];
        if (currencyKey == sUSD) {
            _issuedSynth = syAggregator.totalSupply();
        } else {
            (uint256 currencyRate,) = exchangeRates().rateAndInvalid(currencyKey);
            uint256 totalSynths = syAggregator.totalSupply();
            _issuedSynth = totalSynths.multiplyDecimal(currencyRate);
        }
    }
    ...
    function totalIssuedSynthPerAssetAndIssuer(bytes32 currencyKey, address account) external view returns (uint256 _issuedSynth) {
        ISynthAggregator syAggregator = syAggregators[currencyKey];
        if (currencyKey == sUSD) {
            _issuedSynth = syAggregator.balanceOf(account);
        } else {
            (uint256 currencyRate,) = exchangeRates().rateAndInvalid(currencyKey);
            uint256 totalSynths = syAggregator.balanceOf(account);
            _issuedSynth = totalSynths.multiplyDecimal(currencyRate);
    ...
    function _addSynthAggregator(bytes32 currencyKey, ISynthAggregator syAggregator) internal {
        require(syAggregators[currencyKey] == ISynthAggregator(0), "Synth exists");
        require(syAggregatorsByAddress[address(syAggregator)] == bytes32(0), "Synth address already exists");

        availableSynthAggregators.push(syAggregator);
        syAggregators[currencyKey] = syAggregator;
        syAggregatorsByAddress[address(syAggregator)] = currencyKey;

        emit SynthAggregatorAdded(currencyKey, address(syAggregator));
    }

    function addSynthAggregator(bytes32 currencyKey, ISynthAggregator syAggregator) external onlyOwner {
        _addSynthAggregator(currencyKey, syAggregator);
    }

    function _removeSynthAggregator(bytes32 currencyKey) internal {
        address syAggregatorToRemove = address(syAggregators[currencyKey]);
        require(syAggregatorToRemove != address(0), "Synth does not exist");
        require(currencyKey != sUSD, "Cannot remove synth");

        // Remove the synth from the availableSynths array.
        for (uint256 i = 0; i < availableSynthAggregators.length; i++) {
            if (address(availableSynthAggregators[i]) == syAggregatorToRemove) {
                delete availableSynthAggregators[i];

                availableSynthAggregators[i] = availableSynthAggregators[availableSynthAggregators.length - 1];

                // Decrease the size of the array by one.
                availableSynthAggregators.length--;
        ...
        delete syAggregatorsByAddress[syAggregatorToRemove];
        delete syAggregators[currencyKey];

        emit SynthAggregatorRemoved(currencyKey, syAggregatorToRemove);
    }

    function removeSynthAggregator(bytes32 currencyKey) external onlyOwner {
        _removeSynthAggregator(currencyKey);
    }

    /* ========== EVENTS ========== */

    event SynthAggregatorAdded(bytes32 currencyKey, address synthAggregator);
    event SynthAggregatorRemoved(bytes32 currencyKey, address synthAggregator);
}
...
interface ISynthAggregator {
    ...
    function sendIssue(address account, uint256 amount) external;

    function sendBurn(address account, uint256 amount) external;
...
import "./ISynthAggregator.sol";
interface IIssuedSynthAggregator {
    function syAggregators(bytes32 currencyKey) external view returns (ISynthAggregator);
    function syAggregatorsByAddress(address syAggregator) external view returns (bytes32);
    function totalIssuedSynths() external view returns (uint256 _issuedSynths);
    function totalIssuedSynthsPerIssuer(address account) external view returns (uint256 _issuedSynths);
    function totalIssuedSynthPerAsset(bytes32 currencyKey) external view returns (uint256 _issuedSynth);
    function totalIssuedSynthPerAssetAndIssuer(bytes32 currencyKey, address account) external view returns (uint256 _issuedSynth);
