...
contract StrategyTest {
    ...
    event Failure(uint256 code);

    constructor() {
    }

    function sellErc(address cToken, address wrapToken, uint256 inputAmt) external {
        
        // transfer cToken/comp from wrapped token contract and burn the wrapped tokens 
        IWrappedToken(wrapToken).burn(inputAmt);
        uint256 redeemResult = ICErc20(cToken).redeem(inputAmt);
        if (redeemResult != 0) {
            emit Failure(redeemResult);
        }
    }
