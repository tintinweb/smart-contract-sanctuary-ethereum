...
contract FactoryCloneContract is Ownable {
    ...
    function createDAO(
        ...
        uint256 _depositFunctioningDays,
        bool _feeUSDC,
        uint256 _quorum,
        uint256 _threshold,
        ...
        require(_depositFunctioningDays > 0, "Days cannot be 0");

        require(_ownerFeePerDeposit >= 0, "Owner fee cannot be less than 0");
        require(_ownerFeePerDeposit < 100, "Owner fee cannot exceed 100");

        require(_gnosisAddress != address(0), "Owner cannot be null");

        require(USDCAddress != address(0), "USDC cannot be null");

        require(_minDepositPerUser > 0, "Min deposit should be grater than 0");

        require(_maxDepositPerUser > 0, "Max deposit should be grater than 0");
        require(
        ...
        require(
            _totalRaiseAmount > _maxDepositPerUser,
        ...
        address owner = owner();
        proxy = address(new ProxyContract(ImplementationAddress, owner, data));
    ...
    function createDaoERC721(
        ...
        require(
            _totalSupplyOfToken > 0,
            "Total supply of tokens can't be zero"
        );

        require(_maxTokensPerUser > 0, "Max tokens per user can't be zero");

        data = abi.encodeWithSignature(
            "initializeERC721(uint256,uint256,uint256,uint256,address,address,address,address[],uint256, bool)",
            _ownerFeePerDeposit,
            _depositFunctioningDays,
            _quorum,
            _threshold,
        ...
        ERC20NonTransferable(proxy).setNftAddressFunction(nftContractAddress);
...
contract Nft is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mintNft(address _to, string memory _tokenURI) public {
        ...
        //     );
        //     require(block.timestamp < mintingStopDate, "Minting is over");
        // }
