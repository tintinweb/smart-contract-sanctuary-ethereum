...
contract SpaceRouter {
    ...
    constructor(SpaceCoin _spaceCoin, SpaceLP _spaceLP) {
    ...
    function addLiquidity(uint256 spc) external payable { 
        // get values
        (uint ethReserve, uint spaceCoinReserve) = spaceLP.getReserves();
        uint spcToSend;
        uint ethToSend = msg.value;

        if(ethReserve == 0 && spaceCoinReserve == 0) {
            // There is no reserves, so the iniial deposit is whatever is sent in
            spcToSend = spc;
        } else {
            /// @notice we prioritise calculating the amount of eth sent, unless it would require more spaceCoin than desired
            // get the amount of spaceCoin to send if we sent the full amount of eth along
            spcToSend = _getDepositAmount(msg.value, ethReserve, spaceCoinReserve);
            if(spcToSend > spc) {
                //the eth value would require more tokens than desired, so lets reduce the amount of eth to send, and send the rest back
                ethToSend = _getDepositAmount(spc, spaceCoinReserve, ethReserve);
                //set the amount of tokens to send back to what was specified
                spcToSend = spc;
            }

            // this may be an unnecessary feature, but it is a major quality of life improvement if is spacecoin is set to tax as extra eth deposited would be donated to the pool
            if(spaceCoin.toTax()) {
                /**
                @dev we know the percent that it is taxed at, if we didn't we could transfer the token to the lp contract,
                then check the change in balance then use that as the amount sent for determining amount of eth to send
                but we can calculate the tax directly and adjust balances accordingly
                */
                // spaceCoin taxes 2% on transfer, so adjust ethToSend based on what the lp pool will recieve after tax
                uint spaceCoinDepositedAterTax = (spcToSend * 98) / 100;
                ethToSend = _getDepositAmount(spaceCoinDepositedAterTax, spaceCoinReserve, ethReserve);
            }
        }
        
        address spaceLpAddress = address(spaceLP);
        // send proper amount of spaceCoin to lp contract
        spaceCoin.transferFrom(msg.sender, spaceLpAddress, spcToSend);
        // send proper amount of eth to lp contract
        (bool success, ) = spaceLpAddress.call{value : ethToSend}("");
        if(success == false) revert EthFailedToSend();
        // call deposit which will mint lp tokens for the caller based on percent of liqudity they just added
        spaceLP.deposit(msg.sender);
        if(msg.value > ethToSend) {
            // we used less eth than was sent, so send the rest back
            (success, ) = msg.sender.call{value : msg.value - ethToSend}("");
            if(success == false) revert EthFailedToSend();
        }
    }
    ...
    function removeLiquidity(uint256 lpToken) external {
        // send the desired amount of lp tokens to the lp contract
        spaceLP.transferFrom(msg.sender, address(spaceLP), lpToken);
        // withdraws liquidity to the called based on the balance of lp tokens now held in the contract
    ...
    function swapETHForSPC(uint256 spcOutMin) external payable {
        // grab values
        address spaceLpAddress = address(spaceLP);
        uint spaceCoinReserve = spaceLP.spaceCoinReserve();
        uint currentSpaceCoin = spaceCoin.balanceOf(spaceLpAddress);
        uint userSpaceCoinBefore = spaceCoin.balanceOf(msg.sender);

        if(currentSpaceCoin - spaceCoinReserve > 0) {
            // spaceCoin is out of sync, so lets sync the reserve values in the lp contract
            spaceLP.sync();
        }
        // send eth into the lp contract
        (bool success, ) = spaceLpAddress.call{value : msg.value}("");
        if(success == false) revert EthFailedToSend();
        // swap eth sent in for spaceCoin
        spaceLP.swap(msg.sender);
        // get the amount of spaceCoin the user received
        uint spcOut = spaceCoin.balanceOf(msg.sender) - userSpaceCoinBefore;
        // make sure the amount received is not lower than the minimum expected amount
        if(spcOut < spcOutMin) revert LowerThanMin(spcOutMin, spcOut);
    ...
    function swapSPCForETH(uint256 spcIn, uint256 ethOutMin) external {
        // get values
        address spaceLpAddress = address(spaceLP);
        uint ethReserve = spaceLP.ethReserve();
        uint currentEth = spaceLpAddress.balance;
        uint userEthBefore = msg.sender.balance;

        if(currentEth - ethReserve > 0) {
            // eth is out of sync, so lets sync the reserve values in the lp contract
            spaceLP.sync();
        }
        // transfer spaceCoin into the lp pool
        spaceCoin.transferFrom(msg.sender, spaceLpAddress, spcIn);
        // swap spaceCoin sent in for eth
        spaceLP.swap(msg.sender);
        // get amount of eth the user received
        uint ethOut = msg.sender.balance - userEthBefore;
        // make sure the amount received is not lower than the minimum expected amount
        if(ethOut < ethOutMin) revert LowerThanMin(ethOutMin, ethOut);
    }

    // helper functions //

    /**
    @notice Returns the amount of eth to deposit alongside a given amount of spaceCoin
    @param amountSpaceCoin The amount of spaceCoin wanting to be deposited
    */
    function getAmountEthToDeposit(uint amountSpaceCoin) external view returns(uint) {
        (uint ethReserve, uint spaceCoinReserve) = spaceLP.getReserves();
        return _getDepositAmount(amountSpaceCoin, spaceCoinReserve, ethReserve);
    }

    /**
    @notice Returns the amount of spaceCoin to deposit alongside a given amount of ETH
    @param amountEth The amount of ETH wanting to be deposited
    */
    function getAmountSpaceCoinToDeposit(uint amountEth) external view returns(uint) {
        (uint ethReserve, uint spaceCoinReserve) = spaceLP.getReserves();
        return _getDepositAmount(amountEth, ethReserve, spaceCoinReserve);
    }

    /**
    @notice Returns the amount of spaceCoin to deposit alongside a given amount of ETH
    @param amount The amount of ETH wanting to be deposited
    @param reserveEntered The reserve of the entered amount
    @param reserveOther The reserve of the other token
    */
    function _getDepositAmount(uint amount, uint reserveEntered, uint reserveOther) internal pure returns(uint) {
        return((amount * reserveOther) / reserveEntered);
    }

    // Errors //
    error LowerThanMin(uint min, uint actual);
    error EthFailedToSend();
