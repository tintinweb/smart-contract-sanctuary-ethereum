...
constructor ( address _nftreserveaddress ) {
        
        DaiAddress = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F; // testnet DAI
   ...
   function exchangeNFT ( uint256 _amount ) public {
       
       NFTReserve nft = NFTReserve ( nftreserveaddress );
       nft.transferFrom ( msg.sender, burnAddress, _amount );
       dai.transfer ( msg.sender, exchangeAmount ( _amount ));
   }
  
   function exchangeAmount ( uint256 _amount ) public returns(uint256) {
       return _amount * floorPrice();
   }
    
   function currentSupply() public  returns(uint256){
        NFTReserve nft = NFTReserve ( nftreserveaddress );
        return nft.totalSupply().sub( nft.balanceOf(burnAddress));
   } 
   ...
   function floorPrice() public  returns ( uint256 ){
