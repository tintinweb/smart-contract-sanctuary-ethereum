...
contract Numbers is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  function fuse(uint64[] memory tokens) public {
        ...
        baseColor[tokens[0]] = randoNumbers(tokens[0], 1, 4);
  }

  function getNumbers(uint64 tokenId) public view returns (uint64) {
        if (!_exists(tokenId)) {
            return 0;
