...
interface IDrawBeacon {
     ...
     * @return The timestamp at which the next beacon period would start
     */
    function calculateNextBeaconPeriodStartTime(uint64 time)
        external
        view
        returns (uint64);
...
interface IDrawBuffer {
     ...
     */
    function getDraw(uint32 drawId)
        external
        view
     ...
     * @return Draw.drawId
     */
    function setDraw(IDrawBeacon.Draw calldata newDraw)
        external
        returns (uint32);
}
...
library RingBufferLib {
     ...
     * @return TWAB index.
     */
    function wrap(uint256 _index, uint256 _cardinality)
        internal
        pure
        returns (uint256)
    {
...
interface IPrizeTierHistory {
    ...
    function getPrizeTier(uint32 drawId)
        external
        view
        returns (PrizeTier memory prizeTier);
