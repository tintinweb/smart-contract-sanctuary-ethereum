...
contract SynthrBridge is BaseBridge, CollateralAggregator, DebtShareAggregator, LiquidatorRewardsAggregator, RewardEscrowV2Aggregator, SynthAggregator {
    ...
    function initialize(address addressResolver, address __exchangeFeeAddress, uint16 __selfLZChainId) external onlyOwner {
    ...
    function _sendMint(
        ...
        if (_synthKey != bytes32(0) && _synthAmount != 0) {
            ...
            updateEntry(
                _collateralKey,
                _debtShareBalanceOf(_accountForSynth),
                _accountForSynth
            );
        }
    }
    ...
    function _sendLiquidate(
        ...
        if (currentDebtShareBalance > debtShare) {
            updateAccumulatedShare(
                collateralKey,
                notifyAmount / _debtShareBalanceOf(account)
            );
        }
