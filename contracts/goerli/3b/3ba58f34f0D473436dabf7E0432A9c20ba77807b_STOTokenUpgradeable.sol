...
contract STOTokenUpgradeable is
    Initializable,
    ...
    STOErrors
{
    ...
    function __STOTokenDividend_init(address stoToken, address newPaymentToken, string memory _name, string memory _symbol)
        internal
        onlyInitializing
    {
    ...
    function addDistDividend(uint256 _totalAmount)
        external
        nonReentrant
    {
        ...
        returns (uint256 amount)
    {
        ...
        if ((index == type(uint256).max) || (index == numberOfDistributions)) revert NotAmountToClaim(_claimer);
