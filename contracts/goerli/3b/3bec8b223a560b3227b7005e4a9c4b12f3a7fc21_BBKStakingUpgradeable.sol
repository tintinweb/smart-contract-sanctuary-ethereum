...
contract BBKStakingUpgradeable is
    Initializable,
    ...
    PausableUpgradeable
{
    ...
    function stake(
        ...
        //set Approval for all required before staking
    ...
    //gas cost to unstake 1: 65944, 90370
    function unstake(
        uint256[] calldata genTokenIds,
        uint256[] calldata gen2TokenIds
    ...
    function claimRewards() external nonReentrant whenNotPaused {
        ...
        //if nfts are unstaked before claiming and an amount of tokens are deducted from claimable balance, the total reward could be negative.
    ...
    function deductFromBalanceOf(address account, uint256 amount)
        ...
        whenNotPaused
    {
        //requires allowance of $Neuron for caller (doesnt decrease allowance after)
        require(
    ...
    */
    ...
    function calculateRewards(address account) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < tokenIds.length(); i++) {
            uint256 curTokenId = tokenIds.at(i);
            //replace with calculateRewardForTokenId?
            uint256 stakedSince = genToTimestamp[curTokenId];
            uint256 timePassed = block.timestamp - stakedSince;
            uint256 _rate = isLegendaryToken[curTokenId]
                ? legendaryRate
