...
contract MoonLabsReferral is Ownable {
    /*|| === STATE VARIABLES === ||*/
    int public INDEX;
    ...
    function checkIfReserved(string memory _code) public view returns (bool) {
        ...
        for (uint16 i = 0; i < RESERVED_CODES.length; i++) {
            // Comapre two strings
            if (
                keccak256(abi.encodePacked(_c)) ==
                keccak256(abi.encodePacked(RESERVED_CODES[i]))
            ) return true;
        }
        return false;
    }
    ...
    function createCode(string calldata _code) external {
        ...
        require(
            keccak256(abi.encodePacked(ADDRESS_CODE[msg.sender])) ==
    ...
    function setCodeAddress(string calldata _code, address _address) external {
        ...
        require(
            keccak256(abi.encodePacked(ADDRESS_CODE[_address])) ==
                keccak256(abi.encodePacked("")),
            "Address in use"
        );
