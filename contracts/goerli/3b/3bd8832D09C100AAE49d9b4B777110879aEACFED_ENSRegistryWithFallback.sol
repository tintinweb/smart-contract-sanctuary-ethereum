...
contract ENSRegistry is ENS {
    ...
    // 操作人的映射(所有人=>操作人=>是否有权限)
    mapping(address => mapping(address => bool)) internal operators;

    // 需要owner或者operators权限
    // 比如.eth顶级域名的管理权限
    modifier authorised(bytes32 node) {
        address owner = records[node].owner;
        require(owner == msg.sender || operators[owner][msg.sender], "unauthorized");
        _;
    }

    /**
     * @dev Constructs a new ENS registrar.给0x0节点设置owner，用于初始化合约
     */
    constructor() {
