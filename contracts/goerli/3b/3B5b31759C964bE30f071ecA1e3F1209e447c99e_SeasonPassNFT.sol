...
contract SeasonPassNFT is ERC721URIStorage, ERC721Royalty, Ownable {
    ...
    uint256 public constant MAX_SEASONPASS = 4000;

    uint256 public constant MAX_TX_MINT_SEASONPASS = 10;

    uint256 public PRICE_SEASONPASS = 20 * 10**15; // .020 eth

    address payable target = payable(0x353d96a060391F8Dc88c44e660372cbBe604B70b);

    event CreateSeasonPassNFT(uint256 indexed id);

    constructor() ERC721("Season Pass", "PASS") {}
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://bafybeihwxbagj7zurvzsrgib3rvpdmyqwlde4u2dzdxha4t62z6qgqalby/";
    }
        ...
        //   returns (uint256)
    {
        ...
        require(msg.value >= priceSeasonPass(_count), "Not enough ETH sent");
        require(_count > 0, "Must mint 1 or more per transaction");
        require(_count <= MAX_TX_MINT_SEASONPASS, "Exceeds max mint number per transaction");
        require(_seasonPassTokenCount._value <= MAX_SEASONPASS, "Member tokens sold out");
        require(_seasonPassTokenCount._value + _count <= MAX_SEASONPASS, "Exceeds the number remaining. Try again, requesting a smaller number.");
