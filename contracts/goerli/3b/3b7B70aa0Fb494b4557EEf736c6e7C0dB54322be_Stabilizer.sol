...
contract Stabilizer is Owned {
  ...
  function invest(uint usdx_amount) external onlyOwner {
    if(usdx_amount == 0) { revert InputIsZero(); }
    if(usdx_balance < usdx_amount) { revert NotEnoughBalance(); }

    usdx_balance -= usdx_amount;
    TransferHelper.safeApprove(address(usdx), address(asset), usdx_amount);
    asset.deposit(address(usdx), usdx_amount);

    emit Invested(usdx_amount, address(asset));
  }

  // gets <usdx_amount> back from the asset investment
  function payback(uint usdx_amount) external {
    if(usdx_amount == 0) { revert InputIsZero(); }

    asset.withdraw(address(usdx), usdx_amount);
    usdx_balance += usdx_amount;
...
interface IAsset {
  function deposit(address collateral, uint amount) external;
  function withdraw(address collateral, uint amount) external;
