...
import { FukuStorage, EIP712Domain, EIP712Hashes, FukuTokenizedOptionsStorage, TokenAddressStorage } from "./libraries/FukuStorage.sol";
...
contract FukuInit {
     ...
     * @param punkToken The punk token address
     */
    function init(
        ...
        // create option offer type hash
        hashes.optionOffer = keccak256(
            "OptionOffer(address buyer,address collection,uint256 tokenId,uint256 strikePrice,uint256 premium,uint256 duration,uint256 nonce)"
        );
        ...
        tokenAddressStorage.punkToken = punkToken;
        tokenAddressStorage.wethToken = wethToken;
    }
}
...
contract TokenizedOptions is ERC721, Ownable, ITokenizedOptions {
    ...
    // base uri for fetch metadata, token ID gets appended to tokenURI
     ...
     * @param seller The option seller
     * @param buyer The option buyer and the recipient of the tokenized options
     * @return tokenizedId The tokenized option Id
     */
    function mintOption(
        Option calldata option,
        address seller,
        address buyer
    ) external override onlyOwner returns (uint256 tokenizedId) {
        tokenizedId = nextOptionId++;

        // mint and send to recipient
        _safeMint(buyer, tokenizedId);

        // add to mapping
        options[tokenizedId] = TokenizedOption(option, seller, block.timestamp);
    ...
    /**
     * @notice Function called by Fuku exchange contract to set baseURI
     * @dev todo functionality to set base URI in the diamond contract
     *
