...
contract MergeCanvas is IMergeCanvas {
    ...
    function _changePixelColor(
        ...
        // Require min bid if old_price != 0
        if (old_price != 0 && old_price + MIN_BID_AMOUNT > _new_price) {
            revert MergeCanvas_InsufficientBid(_new_price, old_price);
        }
        ...
        // emit PixelChangePrior(_x, _y, old_color, old_owner, old_price);

        // Refund all new money to previous owner
        // at last for CIE pattern
        if (_new_price > 0) {
    ...
    function changePixelColor(
        ...
        SufficientBid(_x, _y)
    {
        _changePixelColor(_x, _y, _new_color, msg.value);
    }
        ...
        NotAtMaxPixelCapacity
    {
        ...
        for (uint i = 0; i < _x.length; i++) {
            _changePixelColor(_x[i], _y[i], _new_color[i], _prices[i]);
        }
    }
