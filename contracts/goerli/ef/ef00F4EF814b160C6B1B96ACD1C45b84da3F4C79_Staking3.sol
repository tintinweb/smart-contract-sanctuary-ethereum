...
contract Staking3 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function deposit(uint128 _amount, uint128 _stakeType) external nonReentrant {
        ...
        if(stakes.stakeAmount == 0) {
            ...
            stakes.fullWithdrawlTime = uint128(uint128(block.timestamp).add(stakeType.stakePeriod));            
            ...
            emit Deposit(msg.sender, actualAmount, _stakeType, stakeType.stakePeriod, block.timestamp);
            ...
            uint128 totalDepositAmount = totalRewardTillNow + actualAmount;
            emit Deposit(msg.sender, totalDepositAmount, _stakeType, stakeType.stakePeriod, block.timestamp);
    ...
    function compound(uint128 _stakeType) external nonReentrant {
        ...
        uint128 stakeTimeTillNow;
            ...
            emit Compound(msg.sender, totalRewardTillNow, _stakeType, block.timestamp);
    ...
    function withdraw(uint128 _amount, uint128 _stakeType) external nonReentrant {
        ...
        emit Withdraw(msg.sender, actualAmount, _stakeType, rewardTillNow, block.timestamp);
    ...
    function claim(uint128 _stakeType) external nonReentrant returns(bool) {
        ...
        emit Claim(msg.sender, totalRewardTillNow, _stakeType, block.timestamp);
        return true;
