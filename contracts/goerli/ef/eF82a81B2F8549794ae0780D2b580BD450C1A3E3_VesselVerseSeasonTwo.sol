...
/// @title The VesselVers Season Two ERC-721 token
...
contract VesselVerseSeasonTwo is
    ERC721ACommon,
    BaseTokenURI,
    FixedPriceSeller,
    SignerManager,
    AccessControlEnumerable
{
    ...
    /**
    @notice Role of administrative users allowed to expel a VesselVerse Season Two token from the
    rack.
    ...
    constructor(
        ...
        FixedPriceSeller(
            ...
            Seller.SellerConfig({
                ...
                freeQuota: freeQuota,
                lockFreeQuota: false,
                reserveFreeQuota: reserveFreeQuota
            }),
            beneficiary
        )
    {
    ...
    function setMintPhase(MintPhase _mintPhase) external onlyOwner {
        if (_mintPhase == MintPhase.PUBLIC_SALE) {
            // Public sale price is 0.01 ETH
            setPrice(0.01 ether);
            // Update seller config to allow for indiivdual wallets to mint more tokens during public sale
            setSellerConfig(
                Seller.SellerConfig({
                    totalInventory: sellerConfig.totalInventory,
                    lockTotalInventory: sellerConfig.lockTotalInventory,
                    maxPerAddress: sellerConfig.totalInventory,
                    maxPerTx: sellerConfig.totalInventory,
                    freeQuota: sellerConfig.freeQuota,
                    lockFreeQuota: sellerConfig.lockFreeQuota,
                    reserveFreeQuota: sellerConfig.reserveFreeQuota
                })
            );
        }
    ...
    /**
    @notice Returns the length of time, in seconds, that the VesselVerse Season Two Token has
    racked.
    @dev Racking is tied to a specific VesselVerse Season Two Token, not to the owner, so it doesn't
    reset upon sale.
    @return racking Whether the VesselVerse Season Two Token is currently racking. MAY be true with
    ...
    @return total Total period of time for which the VesselVerse Season Two has racked across
    its life, including the current period.
     */
    function rackingPeriod(
        uint256 tokenId
    ) external view returns (bool racking, uint256 current, uint256 total) {
    ...
    /**
    @notice Transfer a token between addresses while the VesselVerse Season Two Token is minting,
    ...
    function safeTransferWhileRacking(
        ...
        require(
            ...
            "VesselVerse Season Two: Only owner"
        );
        rackingTransfer = 2;
    ...
    function _beforeTokenTransfers(
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            require(
                ...
                "VesselVerse Season Two: racking"
            );
        }
    }

    /**
    @dev Emitted when a VesselVerse Season Two begins racking.
     */
    event Racked(uint256 indexed tokenId); // Racked

    /**
    @dev Emitted when a VesselVerse Season Two stops racking; either through standard means or
    by expulsion.
     */
    event Unracked(uint256 indexed tokenId); // Unracked

    /**
    @dev Emitted when a VesselVerse Season Two is expelled.
     */
    ...
    /**
    @notice Changes the VesselVerse Season Two's racking status.
    */
    function toggleRacking(
        ...
        uint256 start = rackingStarted[tokenId];
        if (start == 0) {
            require(rackingOpen, "VesselVerse Season Two: racking closed");
    ...
    /**
    @notice Changes multiple VesselVerse Season Two's racking status
     */
    ...
    /**
    @notice Admin-only ability to expel a VesselVerse Season Two from the rack.
    ...
    VesselVerse Season Two.
     */
    function expelFromRack(uint256 tokenId) external onlyRole(EXPULSION_ROLE) {
        require(rackingStarted[tokenId] != 0, "VesselVerse Season Two: not racked");
    ...
    function burn(uint256 tokenId) public virtual onlyApprovedOrOwner(tokenId) {
        require(burningOpen, "VesselVerse Season Two: burning not enabled");
        _burn(tokenId);
    }
