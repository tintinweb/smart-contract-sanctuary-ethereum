...
interface Manifold {
    function mintBatch(address creatorContractAddress, uint256 claimIndex, uint16 mintCount, uint32[] calldata mintIndices, bytes32[][] calldata merkleProofs, address mintFor) external;
}

contract Minter is Ownable {
    address public immutable kudasai = 0xECD0CBBdbFB07986E22981C8D78e17a952605854;
    ...
    modifier onlyHolder() {
        require(IERC721(kudasai).balanceOf(msg.sender) > 0, "Agenai");
        _;
    }
    
    
    function createAddress(uint256 _quantity) public onlyOwner {
    ...
    function sendETH(uint256 _startId, uint256 _endId) public payable onlyOwner {
        require(_startId <= childCount[msg.sender] && _endId <= childCount[msg.sender], "Invalid ID");
        uint256 beforeBalance = address(this).balance;
        uint256 value = msg.value / (_endId - _startId + 1);
        for (uint256 i = _startId; i <= _endId; i++) {
            payable(child[msg.sender][i]).transfer(value);
        }
        // refund
    ...
    function withdrawETH(uint256 _startId, uint256 _endId) public {
        require(_startId <= childCount[msg.sender] && _endId <= childCount[msg.sender], "Invalid ID");
        uint256 beforeBalance = address(this).balance;
        for (uint256 i = _startId; i <= _endId; i++) {
    ...
    function withdrawNFT(uint256 _startId, uint256 _endId, address _contract, uint256[] calldata tokenId) public {
        require(_startId <= childCount[msg.sender] && _endId <= childCount[msg.sender], "Invalid ID");
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            minterChaild(payable(child[msg.sender][i])).withdrawNFT(_contract, tokenId[idx], msg.sender);
            idx++;
        }
    }

    function withdrawETHOnlyOwner() public onlyOwner() {
        payable(owner()).transfer(address(this).balance);
    }

    function withdrawERC20OnlyOwner(address _contract) public onlyOwner() {
        IERC20(_contract).transfer(msg.sender, IERC20(_contract).balanceOf(address(this)));
    }

    function withdrawNFTOnlyOwner(address _contract, uint256 _tokenId) public onlyOwner {
...
contract minterChaild is IERC721Receiver {
    ...
    function withdrawNFT(address _contract, uint256 tokenId, address _to) public onlyOwner {
        IERC721(_contract).transferFrom(address(this), _to, tokenId);
    }
