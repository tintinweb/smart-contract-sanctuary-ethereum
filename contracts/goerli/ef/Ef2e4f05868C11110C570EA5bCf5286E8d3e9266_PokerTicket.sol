...
contract PokerTicket is ERC721A, Ownable, ReentrancyGuard {
    ...
    event MerkleRootConfigChanged(MerkleRootConfig config);
    ...
    function alphaSale(bytes32[] calldata proof_) external callerIsUser nonReentrant {
        require(status == Status.Alpha, "alpha not enabled");
        require(verifyAddress(_msgSender(), proof_, TreeRootType.Alpha), "not alpha or invalid proof");
        require(_numberMinted(_msgSender()) < 1, "already minted in alpha state");
        _sale(1, 0);
    }

    function whitelistSale(uint64 numberOfTokens_, bytes32[] calldata proof_) external payable callerIsUser nonReentrant {
        require(status == Status.WhitelistSale, "whitelist sale not enabled");
        require(verifyAddress(_msgSender(), proof_, TreeRootType.Whitelist), "not in whitelist or invalid proof");
        uint64 whitelistMinted = _getAux(_msgSender()) + numberOfTokens_;
        require(whitelistMinted <= 3, "Only allowed 3 in whitelist");
    ...
    function publicSale(uint64 numberOfTokens_) external payable callerIsUser nonReentrant {
        require(status == Status.PublicSale, "public sale not enabled");
    ...
    function upgradePass(uint256 tokenId_, uint64 updatedLevel_, bytes memory signature_) external callerIsUser nonReentrant{
        require(_exists(tokenId_),"tokenId not exist");
        TokenOwnership memory ownership = _ownerships[tokenId_];
        require(ownership.addr == _msgSender(), "only owner");
        require(ownership.level < updatedLevel_, "can only upgrade");
        require(verfiySignature(tokenId_, updatedLevel_, signature_), "not a valid signature");
    ...
    function setMerkleRootConfig(MerkleRootConfig calldata merkleRootConfig_) external onlyOwner {
        merkleRootConfig = merkleRootConfig_;
        emit MerkleRootConfigChanged(merkleRootConfig_);
    }
