...
contract NOSE is ERC20("BIGNOSE", "NOSE"), ERC20Burnable, ERC20Permit, Pausable, EnsResolve {
  ...
  constructor(
    ...
    _pause();
    require(totalSupply() == 10000000 ether, "NOSE: incorrect distribution");
  }

  modifier onlyGovernance() {
    require(_msgSender() == governance, "NOSE: only governance can perform this action");
    _;
  }

  function changeTransferability(bool decision) public onlyGovernance {
    require(blockTimestamp() > canUnpauseAfter, "NOSE: cannot change transferability yet");
    if (decision) {
      _unpause();
    } else {
      _pause();
  ...
  function _beforeTokenTransfer(
    ...
    require(!paused() || allowedTransferee[from] || allowedTransferee[to], "NOSE: paused");
    require(to != address(this), "NOSE: invalid recipient");
  }
  ...
  function rescueTokens(
    ...
    require(_to != address(0), "NOSE: can not send to zero address");
    ...
    if (_token == IERC20(0)) {
      ...
      uint256 balance = _balance == 0 ? totalBalance : Math.min(totalBalance, _balance);
      require(balance > 0, "NOSE: trying to send 0 balance");
