...
/**
 * @title XXX, LLC. IMintable Contract
 * @dev Implements Openzeppelin Audited Contracts
 *
...
/**
 * @title XXX, LLC. IBurnable Contract
 * @dev Implements Openzeppelin Audited Contracts
 *
...
/**
 * @title XXX, LLC. IBlacklist Contract
 * @dev Implements Openzeppelin Audited Contracts
 *
...
/**
 * @title XXX, LLC. Stablecoin Contract
 * @dev Implements Openzeppelin Audited Contracts
 *
...
contract Stablecoin is Initializable, ERC20, ProxyOwnable, IMintable, IBurnable, IBlacklist {
  //mapping to determine if address is blacklisted
  mapping (address => bool) private _blacklist;

  /**
   * @notice Initializes the contract by setting `name`, `symbol`, and 'decimals'
   */
  function initialize(string memory name_, string memory symbol_, uint8 decimal_, address owner_) public initializer() returns (bool) {
    initializeERC20(name_, symbol_, decimal_);
    createOwner(owner_);
    return true;
  }

  function updateName(string memory name_) public onlyOwner() returns (bool) {
    _name = name_;
    emit Name(_name);
    return true;
  }

  function updateSymbol(string memory symbol_) public onlyOwner() returns (bool) {
    _symbol = symbol_;
    emit Symbol(_symbol);
    return true;
  }
  ...
  * @param 'newDescription' is the new '_description'
  * @return a boolean value indicating whether the operation succeeded.
  */
  function updateDescription(string memory description_) public onlyOwner() returns (bool) {
   ...
   * Emits a {Transfer} event.
   */
  function mintTo(address account, uint256 amount) public override onlyOwner() returns (bool) {
   ...
   * Emits a {Transfer} event.
   */
  function burnFrom(address account, uint256 amount) public override onlyOwner() returns (bool) {
  ...
  */
  ...
  /**
  * @notice Strips a blacklisted address of its funds
  *
  * Emits a {Transfer} event.
  */
