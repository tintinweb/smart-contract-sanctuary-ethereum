...
contract Vesting is IVesting, ISaleListener, AccessControl {
    ...
    {
        Rules storage rule = rules[Group.Public];
        Allocation storage alloc = allocations[Group.Public][_beneficiary];
        ...
        return ISaleListener.onPublicSale.selector;
    }
    ...
    function getPublicAllocation(address _holder)
        ...
        returns (uint256 amount)
    {
        return allocations[Group.Public][_holder].total;
    }
    ...
    /**
     * Allows admins to claim leftover amounts from Public sale, since those cannot be normally allocated via addAllocations
     *
     * @notice Only callable once, and only once vesting has already started
     */
    function claimPublicSaleLeftover() external onlyRole(ALLOCATOR_ROLE) {
        Rules storage rule = rules[Group.Public];

        if (rule.remaining == 0 || block.timestamp < start) {
            revert NotAuthorized();
        }

        uint256 amount = rule.remaining;
        rule.remaining = 0;
        rule.total -= amount;
    ...
    function allGroups() public pure returns (Group[] memory groups) {
        groups = new Group[](3);
        groups[0] = Group.Public;
        groups[1] = Group.Private;
        groups[2] = Group.Seed;
        ...
        returns (uint256 amountOut)
    {
        ...
        amountOut = (alloc.total * pct) / mul - alloc.claimed;
    }
