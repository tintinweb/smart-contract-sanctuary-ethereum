...
contract ChainpassTickets_1_0 is ERC721iEnumerable, Initializable, MetaTxMixin, OwnableUpgradeable, ReentrancyGuard {
    ...
    function issueTickets(uint256 amount, uint256 price) external {
        ...
        adminSetApprovalForAll(_preMintReceiver, address(cpMarketplace), true);
    ...
    function eventRegistry() private view returns (IChainpassEventFactory) {
        return IChainpassEventFactory(registry().getEventFactoryContractAddress());
    }
    ...
    function setRoyalty(Royalty memory _royalty) external onlyOwner {
        require(_royalty.bps <= 8_000, "BPS cannot be hire than 8000.");
        royalty = _royalty;
    }
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function adminSetApprovalForAll(address owner, address operator, bool approved) internal {
        _operatorApprovals[owner][operator] = approved;
        emit ApprovalForAll(owner, operator, approved);
    }
}
