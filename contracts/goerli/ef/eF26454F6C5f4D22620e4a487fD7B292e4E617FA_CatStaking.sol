...
contract CatStaking is Ownable, ReentrancyGuard, Pausable {
    using SafeERC20 for IERC20;

    /**
     * @dev The ERC20 Reward Token that will be distributed to stakers.
     */
    IERC20 public immutable tears;

    /**
     * @dev The ERC721 Collection that will be staked.
     */
    IERC721 public immutable catCollection;
    ...
    uint256 public rewardsPerHour = 3600;
		uint256 public startTime = 1675785600;
		uint256 public finishTime = 1675789200;
    ...
    function stake(uint256[] calldata _tokenIds) external whenNotPaused {
        ...
        for (uint256 i; i < len; ++i) {
            require(
                catCollection.ownerOf(_tokenIds[i]) == msg.sender,
                "Can't stake tokens you don't own!"
            );

            catCollection.transferFrom(msg.sender, address(this), _tokenIds[i]);
    ...
    function withdraw(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        for (uint256 i; i < lenToWithdraw; ++i) {
            ...
            catCollection.transferFrom(address(this), msg.sender, _tokenIds[i]);
        }
    ...
    function claimRewards() external {
        ...
        tears.safeTransfer(msg.sender, rewards);
    }
    ...
    function setRewardsPerHour(uint256 _newValue) public onlyOwner {
		...
		/**
		 * @notice Function used to set start time of staking
		 * @param _newValue - The new value of the startTime variable.
		 *
		 */
		function setStartTime(uint256 _newValue) public onlyOwner {
			startTime = _newValue;
		}

		/**
		 * @notice Function used to set finish time of staking
		 * @param _newValue - The new value of the finishTime variable.
		 *
		 */
		function setFinishTime(uint256 _newValue) public onlyOwner {
			finishTime = _newValue;
		}
        ...
        returns (uint256 _rewards)
    {
        Staker memory staker = stakers[_staker];
				uint256 timestart = max(staker.timeOfLastUpdate, startTime);
				uint256 timeend = min(block.timestamp, finishTime);
				uint256 timeElapsed = max(timeend - timestart, 0);

        return 
        ((((timeElapsed * staker.stakedTokenIds.length)) * rewardsPerHour) / SECONDS_IN_HOUR);
    }
    ...
    function updateRewards(address _staker) internal {
		...
		/**
		 * @notice Function used to get the max value of two numbers
		 * @param _a - The first number
		 * @param _b - The second number
		 * @return - The max value of the two numbers
		 */
		...
		/**
		 * @notice Function used to get the min value of two numbers
		 * @param _a - The first number
		 * @param _b - The second number
		 * @return - The min value of the two numbers
		 */
