...
interface IKeyNFT {
	function isAccreditedAddress(address _address) external view returns (bool);    
}
...
contract SingleFixed is OwnableUpgradeable, ERC721HolderUpgradeable {
	...
	function setKeyNFT(address _keyNFT) external onlyOwner {
        require(_keyNFT != address(0x0), "invalid address");		
        keyNFT = _keyNFT;		
    }
    ...
    function singleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _price) OnlyItemOwner(_collection,_tokenId) public {
		...
		// filter that user is accredited
        require(IKeyNFT(keyNFT).isAccreditedAddress(_msgSender()), "can not mint nft for unaccredited address");

		IInvestNFT nft = IInvestNFT(_collection);  
		// filter floor Price
        uint256 floorPrice = nft.getFloorPrice();
        require(floorPrice <= _price, "startPrice have to be greater than floorPrice");        
    ...
    function singleDelist(uint256 _id) external {        
        ...
        IInvestNFT(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].tokenId);        
    ...
    function singleBuy(uint256 _id) external payable {
		...
		IInvestNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.tokenId);
	...
	modifier OnlyItemOwner(address tokenAddress, uint256 tokenId){
        IInvestNFT tokenContract = IInvestNFT(tokenAddress);
