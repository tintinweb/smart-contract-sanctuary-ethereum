...
\____/_/ /_/_/   \____/_/ /_/_/\___/_/\___/____/  /_____/\____/\____/\__/_.___/\____/_/|_|  
    ...
    Rare Token ID:     1001    Guaranteed Rare
...
contract CoiwLootbox is AbstractERC1155Factory {
    ...
    /*
     * Mint parameters
     * NOTE: We likely won't use these because we're only planning to airdrop CLBs, not let people mint CLBs.
     */
    ...
    /**
    * Init setup. Edit all params for a token id
    *
    ...
    */
    ...
    function burnToMint(uint256 tokenId, uint16 desiredDepartment) external payable returns (uint256) {
        require(tokenId == 1001 || tokenId == 2001 || desiredDepartment == 0, "Only Rare and Uncommon CLBs can choose a Department");
    ...
    /**
     * Safety function
     * @notice Purchase tokens during public sale.  This isn't expected to be used since we airdrop.
     *
    ...
    /**
     * Safety function 
     * @notice purchase tokens using multi-signature, REGARDLESS of active status.  This isn't expected to be used since we airdrop.
     *
     */
    function signedPurchase(uint256 tokenId, uint256 amount, uint256 _timestamp, bytes memory _signature) external payable whenNotPaused {
        ...
        require(_timestamp >= block.timestamp - 30, "Signature expired, out of time");
        _purchase(tokenId, amount);
    }
