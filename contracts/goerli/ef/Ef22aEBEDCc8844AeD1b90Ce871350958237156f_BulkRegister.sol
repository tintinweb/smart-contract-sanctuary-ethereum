...
contract BulkRegister {
    ...
    function availableAll(string[] calldata names) external view returns(string[] memory){
        string[] memory namesAvailable = new string[](names.length);
        ETHRegistrarController controller = getController();
        for(uint i = 0; i < names.length; i++) {
            bool res =  controller.available(names[i]);
            if(res) {
                namesAvailable[i] = names[i];
            }
        }
    ...
    function commitAll(string[] calldata names, address owner, bytes32 secret) external payable {
        ETHRegistrarController controller = getController();
        for(uint i = 0; i < names.length; i++) {
            bytes32 cmt = controller.makeCommitment(names[i], owner, secret);
            controller.commit(cmt);
        }
    }
