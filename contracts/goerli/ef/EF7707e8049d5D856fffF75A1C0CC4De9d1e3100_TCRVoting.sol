...
contract TCRVoting is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function isCuratorVotedForRegistry(bytes32 registryId, address curator)
        ...
        returns (bool)
    {
        uint256 curatorAllVotes = curatorVotedRegistries[curator].length;
        bool isVoted = false;
        for (uint256 i = 0; i < curatorAllVotes; i++) {
            if (curatorVotedRegistries[curator][i] == registryId) {
                isVoted = true;
            }
        }
        return isVoted;
    ...
    function acceptRegistration(bytes32 registryId) external nonReentrant {
        ...
        uint256 rewardsCalculation = successSubmissionRewards + registries[registryId].submissionInfo.submissionAmount;
        ...
        nonReentrant
    {
        ...
        registries[registryId].challengedBy = msg.sender;
        registries[registryId].isChallenged = true;
        allChallengeRegistries.push(registryId);
        curatorChallengedRegistries[msg.sender].push(registryId);
