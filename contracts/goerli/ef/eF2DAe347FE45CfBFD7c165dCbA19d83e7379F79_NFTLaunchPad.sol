...
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
 ...
 contract NFTLaunchPad is Initializable, OwnableUpgradeable {
     ...
     //  *@param _collectionArgs struct of integer values used to create launchPad
     ...
     function createLaunchPad(
         NFTCollection.CollectionArgs memory _collectionArgs,
         NFTCollection.PhaseArgs[] calldata _phaseArgs
     ) external returns (address _launchpad) {
         require(
             (_collectionArgs.royalty >= 0 && _collectionArgs.royalty <= 10000),
             "NFTLaunchPad: Royalty should be less than 10000"
         );
         uint256 totalNFTCap = 0;
         for (uint256 i = 0; i < _phaseArgs.length; i++) {
             ...
             totalNFTCap += _phaseArgs[i].nftCap;
         }
 
         require(
             totalNFTCap <= _collectionArgs.maxSupply,
             "NFTLaunchpad: Phases nftCap should less than maxSupply"
         );
         ...
         emit CreateLaunchpad(
             ...
             block.timestamp,
             _collectionArgs.name,
             _collectionArgs.symbol,
             _collectionArgs.contractURI,
             _collectionArgs.maxSupply
         );
     ...
     function setGlobalBaseURI(string calldata baseURI_) external  {
...
import "./VerifyUser.sol";

contract NFTCollection is ERC721A, ERC2981, Ownable, VerifyUser {
    ...
    struct Phase {
        ...
        mapping(address => uint256) nftMinted; 
        bool isWhitelisted;
        uint256 startTime;
        uint256 endTime;
        uint256 maxNFTPerUser;
        uint256 maxQuantity; //maxnftpertransaction
        uint256 nftCap;
        uint256 totalMinted;
    }
    ...
    constructor(
        address _creator,
        CollectionArgs memory _collectionArgs,
        PhaseArgs[] memory _phaseArgs
    ) ERC721A(_collectionArgs.name, _collectionArgs.symbol) {
        maxSupply = _collectionArgs.maxSupply;
        creator = _creator;
        baseURI = _collectionArgs.baseURI;
        contractURI = _collectionArgs.contractURI;
        launchpad = INFTLaunchPad(msg.sender);
        _setDefaultRoyalty(creator, _collectionArgs.royalty);
        ...
        for (uint256 i = 0; i < _phaseArgs.length; i++) {
            ...
            for (uint256 j = 0; j < _phaseArgs[i].currencies.length; j++) {
                ...
                phases[i].isWhitelisted = _phaseArgs[i].isWhiteListed;

                phases[i].startTime = _phaseArgs[i].startTime;
                phases[i].endTime = _phaseArgs[i].endTime;

                phases[i].maxNFTPerUser = _phaseArgs[i].maxNFTPerUser;
                phases[i].maxQuantity = _phaseArgs[i].maxQuantity;
                phases[i].nftCap = _phaseArgs[i].nftCap;
            ...
            emit CollecctionPhase(
                ...
                _phaseArgs[i].startTime,
                _phaseArgs[i].endTime,
    ...
    function getTotalMinted(uint256 _phaseId) external view returns (uint256) {
        return phases[_phaseId].totalMinted;
    }

    function getNftCap(uint256 _phaseId) external view returns (uint256) {
        return phases[_phaseId].nftCap;
    }

    function getNftMiintedPerPhase(uint256 _phaseId)external view returns(uint256) {
        return phases[_phaseId].nftMinted[msg.sender];
    }
    ...
    function mint(
        ...
        require(
            _phase.startTime < block.timestamp &&
                _phase.endTime > block.timestamp,
            "NFTCollection: Phase is not started yet or end!"
        );

        require(
            _phase.totalMinted + _quantity <= _phase.nftCap,
            "NFTCollection: Phase reached max mint Capicity"
        );
        ...
        require(
            _phase.nftMinted[msg.sender] + _quantity <=
                _phase.maxNFTPerUser,
        ...
        require(
            _quantity > 0 && _quantity <= _phase.maxQuantity,
            "NFTCollection: Max quantity reached"
        );

        if (phases[_phaseId].isWhitelisted) {
            ...
            require(WhiteListed, "NFTCollection: only white list user is allowed");
        }
        uint256 mintFee = _phase.currenciesPrice[_currency];
        uint256 brokerage = _getBrokerage(address(_currency));

        require(
            _phase.currenciesPrice[_currency] > 0 ||
                _phase.currencies[_currency],
            "NFTCollection: Currency not Supported for minting"
        );
        ...
        _phase.totalMinted = _phase.totalMinted + _quantity;
...
contract VerifyUser{
     ...
     */
    function _getPublicKey(address _launchpad) private view returns (address) {

      
        return INFTLaunchPad(_launchpad).getPublicKey();
    }
    ...
    function verifyWhiteListUser(
        ...
        require(
            verifiedUser == _getPublicKey(launchpad),
