    ...
    RewardPlanner
{
    ...
    PoolInfo public pool; // for multiple token rewards in future
        ...
        notZeroAddress(_initParams.rewardToken)
    {
        ...
        __ERC165_init();
        __RewardPlanner_init();

        accessRegistry = IAccessControl(_initParams.accessRegistry);

        require(
            IERC165(_initParams.accessRegistry).supportsInterface(
                type(IAccessControl).interfaceId
            ),
            "Staking: UNSUPPORTED_ADDRESS"
        );

        accessRegistry = IAccessControl(_initParams.accessRegistry);
        stakeToken = _initParams.stakeToken;
        pool = PoolInfo(_initParams.rewardToken, 0, 0, 0);
    }
        ...
        returns (uint reward)
    {
        ...
        uint _poolRewardPerToken = pool.snapshotRewardPerToken +
            _incRewardPerToken();
        reward = _userReward(_user, _poolRewardPerToken);
    }
        ...
        nonReentrant
    {
        ...
        _updatePool();
        UserInfo storage _user = _updateUser(msg.sender);
        _claim(_user);
     ...
     */
    function claim() external notPaused(pauseInfo.pausedWithdraw) nonReentrant {
        _updatePool();
        UserInfo storage _user = _updateUser(msg.sender);
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        uint _requiredAmount = totalRewardAmount - pool.paidAmount;
        uint _balance = IERC20(pool.rewardToken).balanceOf(address(this));
        uint _toTransfer = _requiredAmount > _balance
    ...
    function refundUnusedTokens()
        ...
        nonReentrant
    {
        IERC20 _token = IERC20(pool.rewardToken);
        ...
        emit RefundUnused(pool.rewardToken, msg.sender, _unusedAmount);
    }
    ...
    function _updatePool() private {
        ...
        pool.snapshotRewardPerToken += _incRewardPerToken();
        pool.lastUpdateTime = block.timestamp.toUint32();
    }

    function _incRewardPerToken() internal view returns (uint) {
        if (totalStakes > 0) {
            uint _reward = deltaReward(
                pool.lastUpdateTime,
    ...
    function _updateUser(address _account)
        private
        returns (UserInfo storage user)
    {
        UserInfo storage _user = users[_account];
        _user.reward = _userReward(_user, pool.snapshotRewardPerToken);
        _user.snapshotRewardPerToken = pool.snapshotRewardPerToken;

        return _user;
    }

    function _claim(UserInfo storage _userInfo) private {
        uint reward = _userInfo.reward;
        if (reward > 0) {
            _userInfo.reward = 0;
            pool.paidAmount += reward;

            IERC20(pool.rewardToken).safeTransfer(msg.sender, reward);

            emit Claim(msg.sender, pool.rewardToken, reward);
        }
    }
...
interface IPlannedStakingV1 {
    ...
    function refundUnusedTokens() external;
}
...
abstract contract RewardPlanner is
    IRewardPlanner,
    CommonChecks,
    Initializable,
    ERC165Upgradeable
{
    ...
    function __RewardPlanner_init() internal onlyInitializing {
        // do nothing
    }
    ...
    function deltaReward(uint32 _from, uint32 _to) public view returns (uint) {
    ...
    /// @return prevSum sum of amounts of all preceeding intervals
    ...
    {
        require(_index < intervals.length, "RewardPlanner: OUT_OF_BOUND");
        IntervalInfo memory _info = intervals[_index];
        (prevSum, amount, start, end) = (
            _info.prevSum,
            _info.amount,
            _info.start,
            _info.end
    ...
    function _appendIntervals(Interval[] memory _data) internal virtual {
        require(_data.length > 0, "RewardPlanner: NO_DATA");
        require(
            _data[0].start > block.timestamp,
            "RewardPlanner: INVALID_START"
        );

        uint _sum = totalRewardAmount;
        uint32 _prevEnd = intervals.length == 0
            ? 0
            : intervals[intervals.length - 1].end;

        for (uint i = 0; i < _data.length; i++) {
            Interval memory _curr = _data[i];
            _checkInterval(_curr);
            _checkOverlap(_prevEnd, _curr.start);
            ...
            emit IntervalAdded(_curr.amount, _curr.start, _curr.end);
        }

        totalRewardAmount = _sum;
    }
    ...
    function _removeIntervals(uint _index) internal {
        ...
        require(
            ...
            "RewardPlanner: ALREADY_STARTED"
        );

        uint sum;
        while (intervals.length > _index) {
            IntervalInfo memory _last = intervals[intervals.length - 1];
            sum += _last.amount;
            emit IntervalRemoved(_last.amount, _last.start, _last.end);
            intervals.pop();
        }
    ...
    function _checkInterval(Interval memory _interval) internal pure {
        require(_interval.start < _interval.end, "RewardPlanner: INVALID_ARG");
        require(_interval.amount > 0, "RewardPlanner: ZERO_AMOUNT");
    }

    function _checkOverlap(uint32 _start, uint32 _end) internal pure {
        require(_start <= _end, "RewardPlanner: SEGMENT_OVERLAP");
    }
        ...
        returns (IntervalInfo memory _info)
    {
        ...
        require(length > 0, "RewardPlanner: NO_INTERVALS");
    ...
    function _valueAt(uint32 _x) internal view returns (uint) {
        ...
        IntervalInfo memory item = _findInterval(_x);
        if (_x <= item.start) {
            return item.prevSum;
        }
        if (_x >= item.end) {
            return item.prevSum + item.amount;
        }

        return
            item.prevSum +
            ((_x - item.start) * item.amount) /
            (item.end - item.start);
    }
...
interface IRewardPlanner {
    ...
    function deltaReward(uint32 _from, uint32 _to) external view returns (uint);
