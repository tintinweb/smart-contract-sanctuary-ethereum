...
contract EthPortal is IPortal, NonblockingLzApp {
    ...
    constructor(
        ...
        address _endpoint, 
        address _admin
    ) NonblockingLzApp(_endpoint, _admin) {
        POW = IERC20(_pow);
    }

    function setL2ChainId(uint16 _id) external onlyRole(USER_ROLE) {
        l2ChainId = _id;
        emit L2ChainIdSet(_id);
    }

    function withdrawEth(address to) external onlyRole(USER_ROLE) {
    ...
    function bridge(
        ...
        _lzSend(
            ...
            msg.value // gas fee from user
        );

        // get outbound nonce from Endpoint contract
        uint256 _nonce = lzEndpoint.getOutboundNonce(l2ChainId, address(this));

        emit Bridged(_nonce, l2ChainId, msg.sender, _amountToBridge);
    }
    ...
    function _nonblockingLzReceive(
        ...
        emit Received(_nonce, _srcChainId, _user, _amount);
    }
...
interface IPortal is IERC165 {
    ...
    function bridge(uint256 _amountToBridge) external payable;
}
...
abstract contract NonblockingLzApp is LzApp {
    constructor(address _endpoint, address _admin) LzApp(_endpoint, _admin) {}
...
abstract contract LzApp is AccessControl, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    constructor(address _endpoint, address _admin) {
    ...
    function setConfig(uint16 _version, uint16 _chainId, uint _configType, bytes calldata _config) external override onlyRole(USER_ROLE) {
        lzEndpoint.setConfig(_version, _chainId, _configType, _config);
    }

    function setSendVersion(uint16 _version) external override onlyRole(USER_ROLE) {
        lzEndpoint.setSendVersion(_version);
    }

    function setReceiveVersion(uint16 _version) external override onlyRole(USER_ROLE) {
        lzEndpoint.setReceiveVersion(_version);
    }

    function forceResumeReceive(uint16 _srcChainId, bytes calldata _srcAddress) external override onlyRole(USER_ROLE) {
        lzEndpoint.forceResumeReceive(_srcChainId, _srcAddress);
    }

    // allow owner to set it multiple times.
    function setTrustedRemote(uint16 _srcChainId, bytes calldata _srcAddress) external onlyRole(USER_ROLE) {
        trustedRemoteLookup[_srcChainId] = _srcAddress;
        emit SetTrustedRemote(_srcChainId, _srcAddress);
    }

    function setMinDstGasLookup(uint16 _dstChainId, uint _type, uint _dstGasAmount) external onlyRole(USER_ROLE) {
