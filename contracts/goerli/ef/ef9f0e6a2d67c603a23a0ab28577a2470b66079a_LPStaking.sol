...
contract LPStaking is OwnableUpgradeable, ERC20Upgradeable, ReentrancyGuard {
    uint256 private constant _EIGHTEEN_DECIMALS = 10**18;
    uint256 private constant _DAY_TO_SECONDS = 24 * 60 * 60;
    uint256 private _minDays;
    uint256 private _maxDays;
    uint256 private _minBoost;
    uint256 private _maxBoost;

    uint256 private _tLPBalanceThis;
    ...
    mapping(address => uint256) private _tLP;
    mapping(address => uint256) private _sLP;
    ...
    /// @param name Name of staked LP token
    /// @param symbol Symbol of staked LP token
    function initialize(
        address rewardToken,
        address owner,
        address lpToken,
        ...
        require(
            minBoost >= 100,
            "LPStaking: Min boost must be bigger or equal to 1"
        );
        ...
        _minBoost = minBoost;
        _maxBoost = maxBoost;
        emit LPStakingInit(lpToken, _minDays, _maxDays, _minBoost, _maxBoost);
    }
    ...
    /// @param numberOfDays Number of weeks you want to stake
    ...
    function _updateBeforeStakeStart(
        ...
        _lockedUntil[user] = block.timestamp + (numberOfDays * _DAY_TO_SECONDS);
        uint256 tempLPAmount = (amount * getBoost(numberOfDays));
        uint256 tempLPBalance = _tLPBalanceThis;
        ...
        _tLPBalanceThis += tempLPAmount;
        _tLP[user] += tempLPAmount;
        _sLP[user] += amountsLPToMint;
        ...
        returns (uint256 stakedLP)
    {
        uint256 tempLPBalance = _tLPBalanceThis;
        uint256 sLPSupply = totalSupply();
        uint256 share = _sLP[user];
        _sLP[user] = 0;
        uint256 tempLPToReturn = ((share * tempLPBalance) / (sLPSupply));
        _burn(user, share);
        uint256 reward = tempLPToReturn - _tLP[user];
        _tLP[user] = 0;
        _tLPBalanceThis -= tempLPToReturn;
        require(
            _rewardToken.transfer(user, reward),
            "LPStaking: Couldn't transfer reward"
        );
    ...
    function deposit(uint256 amount, uint256 numberOfDays) public nonReentrant {
        ...
        require(
            IERC20(_LPToken).transferFrom(_msgSender(), address(this), amount),
            "LPStaking: Couldn't transfer LI"
        );
        require(
            0 <= numberOfDays && numberOfDays <= _maxDays,
    ...
    function withdraw() public nonReentrant {
        ...
        require(
            IERC20(_LPToken).transfer(_msgSender(), stakedLP),
            "LPStaking: Couldn't transfer LP on withdrawal"
        );
    }
    ...
    function addReward(uint256 amount) external nonReentrant {
        require(
            ...
            "LPStaking: Couldn't add reward"
        );
        _tLPBalanceThis += amount;
    ...
    /// @return Boost (should be divided by 10^4 to get correct decimals)
    ...
    function getUnclaimedReward(address wallet) public view returns (uint256) {
        uint256 tempLPToReturn = ((_sLP[wallet] * _tLPBalanceThis) /
            totalSupply());
        return tempLPToReturn - _tLP[wallet];
    }
