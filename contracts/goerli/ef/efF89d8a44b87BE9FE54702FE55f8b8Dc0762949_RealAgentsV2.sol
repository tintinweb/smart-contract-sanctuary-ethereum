...
error Claimed();
contract RealAgentsV2 is ERC721AQueryable, ERC721ABurnable, ERC2981, Pausable, Ownable, ReentrancyGuard {
    using Strings for uint256;

    string public constant NAME = "REAL AGENTS V2";
    string public constant SYMBOL = "AGENT V2";
    ...
    constructor(string memory _uri, address _contract) ERC721A(NAME, SYMBOL) {
    ...
    function upgradeKey(uint256[] calldata _assetIds) external nonReentrant whenNotPaused {
        for (uint256 i = 0; i < _assetIds.length; i++) {
            require(ERC721(_v1Contract).ownerOf(_assetIds[i]) == _msgSender(), "Real Agents: not token holder");
            if (claimed[_assetIds[i]]) revert Claimed();

            ERC721(_v1Contract).safeTransferFrom(_msgSender(), msg.sender, _assetIds[i]);
    ...
    function approve(address to, uint256 tokenId) public payable virtual override {
        if (_marketplaceProtection) {
            require(_approvedMarketplaces[to], "Real Agents V2: invalid Marketplace");
        }
        super.approve(to, tokenId);
    }
    
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(_approvedMarketplaces[operator], "Real Agents V2: invalid Marketplace");
