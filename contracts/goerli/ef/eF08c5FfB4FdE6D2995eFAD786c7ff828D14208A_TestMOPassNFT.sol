...
interface IMOPassImageRender {
    function getImage(uint256 id) external view returns (string memory);

    function getAnimationUrl(uint256 id) external view returns (string memory);
}
...
contract TestMOPassNFT is ERC721A, Ownable {
    ...
    address public mopImageRenderAddress;
    ...
    function setImage(address _mopImageRenderAddress) external onlyOwner {
        require(_isContract(_mopImageRenderAddress), "Invalid address");
        mopImageRenderAddress = _mopImageRenderAddress;
    }
    ...
    function _mintPass(address to, uint256 quantity) internal {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            _buildBlockLevels(tokenId);
            tokenId++;
        }
        _safeMint(to, quantity);
    }

    function _buildBlockLevels(uint256 id) internal {
        require(passBlockLevels[id].length == 0, "Cannot be set");
        bytes memory passBlockLevel = HexGridsMath.buildBlockLevels(keccak256(abi.encodePacked(address(this), id)));

        passBlockLevels[id] = passBlockLevel;
    }
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        PassType _passType = getPassTypeById(id);

        string memory typeStr = "Regular";
        ...
        string[3] memory coordinateStr;

        coordinateStr[0] = _int2str(_block.x);

        coordinateStr[1] = _int2str(_block.y);

        coordinateStr[2] = _int2str(_block.z);

        string memory image = "";
        string memory animationUrl = "";

        if (mopImageRenderAddress != address(0)) {
            IMOPassImageRender imageRender = IMOPassImageRender(mopImageRenderAddress);
            image = imageRender.getImage(id);
            animationUrl = imageRender.getAnimationUrl(id);
        }
    ...
    function getPassTypeById(uint256 id) public view returns (PassType _passType) {
        require(_exists(id), "not exist");
