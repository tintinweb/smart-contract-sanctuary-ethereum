...
contract BSKR is BaseBSKR {
    ...
    uint32[6] private _currFees; // 16x6 bits
    ...
    function _getValues(uint256 tAmount, uint256 feeMultiplier)
        private
        view
        returns (uint256[13] memory response)
    {
    ...
    function _transferTokens(
        ...
        uint256[13] memory response = _getValues(tAmount, reducedFees);
    ...
    function giftReflection(uint256 tAmount) external whenNotPaused {
        ...
        uint256[13] memory response = _getValues(tAmount, 10);
        require(
            _rBalances[sender] > response[uint256(Field.rAmount)],
            "B: Gift too large"
        );
...
contract Utils {
    ...
    //     for (uint256 index; index < charCount; ++index) {
