...
// Executes successive swaps along an encoded uniswap path.
    function _swap(
        ...
        while (true) {
            bool hasMultiplePools = PathHelper.hasMultiplePools(encodedPath);
            isPathMultiHopPublic = hasMultiplePools;
            sellAmount = _swapExactInputInternal(
                sellAmount,
                hasMultiplePools ? address(this) : recipient, // for intermediate swaps, this contract custodies
                0,
                SwapCallbackData({path: PathHelper.getFirstPool(encodedPath), source: source})
            );

            if (hasMultiplePools) {
                source = address(this);
                encodedPath = PathHelper.skipToken(encodedPath);
            } else {
        ...
        require(amountOut >= minBuyAmount, "Router: insufficient amountOut");
    }
    ...
    function _swapExactInputInternal(
        ...
        (address tokenIn, address tokenOut, uint24 fee) = PathHelper.decodeFirstPool(data.path);
        intoken = tokenIn;
        outtoken = tokenOut;

        bool isFromToken0 = tokenIn < tokenOut;
        zeroForOnep = isFromToken0;
        IOcarioPool temp = _getPool(tokenIn, tokenOut,fee);
        poolTest = temp;
        datatemp =  abi.encode(data);
        (int256 amount0, int256 amount1) = _getPool(tokenIn, tokenOut,fee).swap(
            recipient,
            int256(amountIn),
            isFromToken0,
            limitSqrtP == 0 ? (isFromToken0 ? MIN_PRICE_SQRT_RATIO + 1 : MAX_PRICE_SQRT_RATIO - 1) : limitSqrtP,
            abi.encode(data)
        );
