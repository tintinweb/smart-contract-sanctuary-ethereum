...
UUPSUpgradeable
{
    ...
    struct Sales {
        uint256 salesType;      // 1: Fixed, 2: Dutch Auction (Fixed Rate Decrease), 3: Dutch Auction(% Decrease)
        uint256 startDate;
        uint256 endDate;
        uint256 mintPrice;
        uint256 maxPool;
    ...
    function getPrice(uint8 n, uint256 sales) public view returns (uint256) {
        ...
        if (_salesPhase.salesType == 1) {
            ...
            for (uint i = 0; i < numDecreases; i ++) {
                _price = _price - _salesPhase.auction.decreaseSize * numDecreases;
            }

            return n * MathUpgradeable.max(_price, _salesPhase.auction.numDecreases);
        } else if(_salesPhase.salesType == 3) {
            uint256 numDecreases = 0;
    ...
    function getSales(uint256 sales) public view returns (Sales memory) {
        ...
        if (_salesPhase.salesType > 1 && block.timestamp >= _salesPhase.endDate && salesPhaseRestart[sales] == 1) {
            uint256 auctionPeriod = (_salesPhase.endDate - _salesPhase.startDate);
    ...
    function setSalesPhase(Sales memory _salesPhase, uint8 fRestart, uint8 position) public onlyRole(ADMIN_ROLE) {
        ...
        if (uint256(position) > latestSalesIndex) {
            latestSalesIndex = position;
        }
    }
    ...
    uint256 public latestSalesIndex;
}
