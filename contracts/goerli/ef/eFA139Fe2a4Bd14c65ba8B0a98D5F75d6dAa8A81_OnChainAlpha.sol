...
contract OnChainAlpha is ERC721A, IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    function getLayerNames() public view returns (string[] memory) {
        return LAYER_NAMES;
    }
    ...
    function tokensAreDuplicates(uint256 tokenIdA, uint256 tokenIdB)
        public
        view
        returns (bool)
    {
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < NUM_LAYERS; i++) {
            ...
            } else if (_hideLayer[_tokenId][i] == true) {
                hash[i] = TIERS[i].length - 1;
            } else {
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < NUM_LAYERS; i++) {
            ...
            if (LibString.runeCount(_traitDetails[i][thisTraitIndex].name) > 1 ) {
                ...
                if (i == NUM_LAYERS - 1) {
                    metadataBytes.appendSafe("]");
                } else if (i == 10 && (_hideLayer[_tokenId][11] && _hideLayer[_tokenId][12] && _hideLayer[_tokenId][13] && _hideLayer[_tokenId][14]) == true ){
                    metadataBytes.appendSafe("]");
                } else if (i == 11 && (_hideLayer[_tokenId][12] && _hideLayer[_tokenId][13] && _hideLayer[_tokenId][14]) == true ){
                    metadataBytes.appendSafe("]");
                } else if (i == 12 && (_hideLayer[_tokenId][13] && _hideLayer[_tokenId][14]) == true ){
                    metadataBytes.appendSafe("]");
                } else if (i == 13 && _hideLayer[_tokenId][14] == true ){
                    metadataBytes.appendSafe("]");
                } else {
                    metadataBytes.appendSafe(",");
                }
    ...
    function setProfile(
        ...
        emit AttributesUpdated(
            _tokenId,
            idValues[_tokenId].AlphaProfile.userName,
            idValues[_tokenId].AlphaProfile.social,
            idValues[_tokenId].AlphaProfile.website,
    ...
    {
        return idValues[_tokenId].AlphaProfile;
    }
    ...
    function setLabelsValues(uint256 _tokenId, uint _labelNum, string memory _label, string memory _value) external {
        ...
        emit LabelsValuesUpdated(_tokenId, _labelNum, idValues[_tokenId].labels[_labelNum], idValues[_tokenId].values[_labelNum]);
    }
    ...
    function toggleNewLayers(uint256 _tokenId, bool[] memory states) public {
        require(msg.sender == ownerOf(_tokenId));
        for (uint256 i = 0; i < NUM_LAYERS; i++) {
            _revealLayer[_tokenId][i] = states[i];
        }

        emit LayersRevealed(_tokenId, states);
    }
    ...
    function normalize(string memory word)
        private
        pure
        returns (string memory)
    {
