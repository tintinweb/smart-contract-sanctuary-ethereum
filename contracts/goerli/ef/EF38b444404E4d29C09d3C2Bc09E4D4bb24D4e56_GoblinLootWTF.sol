...
contract GoblinLootWTF is ERC721A, Ownable, PullPayment {
    ...
    /// @dev num initial tokens mintable for free
    uint256 public constant freeMintAmount = 5000;
    ...
    constructor() ERC721A("goblin-loot.wtf", "GOBLOOT") {
        contractMetadataURI = "";
        baseTokenURI = "https://goblin-loot-metadata-proxy.herokuapp.com/api/token-metadata/";
    ...
    function mint(uint256 quantity) external payable {
        ...
        if (newTotal > freeMintAmount) {
            if (totalMinted < freeMintAmount ) {
                uint256 payableQuantity = newTotal.sub(freeMintAmount);
                price = payableQuantity.mul(mintPrice);
            }
            else {
                price = quantity.mul(mintPrice);
            }
        }
