...
import "./interfaces/IFlashExecPermits.sol";

contract FlashExecPermits is AccessControl, IFlashExecPermits {
    mapping(bytes => bool) _permits;

    function addPermit(
        address target,
        bytes4 selector
    ) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        bytes memory key = _getMappingKey(target, selector);
        if (!_permits[key]) {
            _permits[key] = true;
            emit AddPermit(target, selector);
        }
    }
    ...
    function removePermit(
        ...
        if (_permits[key]) {
            delete _permits[key];
            emit RemovePermit(target, selector);
        }
    }
    ...
    function isPermitted(
        ...
        return _permits[_getMappingKey(target, selector)];
    }
    ...
    function _getMappingKey(
        ...
        return abi.encode(target, selector);
    }
