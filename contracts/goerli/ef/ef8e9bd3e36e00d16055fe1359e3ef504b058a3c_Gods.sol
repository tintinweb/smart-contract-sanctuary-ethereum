    ...
    SignatureVerifier
{
    ...
    address[1000] public tokenLedger;

    uint256 regularMintCost;
    uint256 privateMintCost;
    uint16 maxSupply;
    uint16 supply;
    bool regularMintActive;
    bool isRevealed;
    ...
    function initialize() external initializer {
        ...
        __ERC721_init("", "");
        regularMintCost = 0.1 ether;
        privateMintCost = 0.3 ether;
        regularMintActive = false;
    ...
    function mint(bytes calldata sig)
        ...
        nonReentrant
        whenNotPaused
        mintingChecks(sig, regularMintActive, regularMintCost, regularSigner)
    {
        _safeMint(msg.sender, ++supply);
        mintingLedger[msg.sender] = true;
        emit tokenMinted(msg.sender, supply);
    }
    ...
    function privateMint(bytes calldata sig)
        ...
        nonReentrant
        whenNotPaused
        mintingChecks(sig, privateMintActive, privateMintCost, privateSigner)
    {
        _safeMint(msg.sender, ++supply);
        mintingLedger[msg.sender] = true;
        tokenLedger[supply] = msg.sender;
        emit tokenMinted(msg.sender, supply);
    }
    ...
    function setRegularMintCost(uint256 _cost) external onlyOwner {
        regularMintCost = _cost;
    }
    ...
    function setRegularMintActive(bool _state) external onlyOwner {
        regularMintActive = _state;
    }
    ...
    function getTokenLedger() external view returns (address[1000] memory) {
        return tokenLedger;
    }
