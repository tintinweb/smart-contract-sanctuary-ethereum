...
contract ExchangeChallenge is IExchangeChallenge, Ownable, ERC1155Holder{
    ...
    uint256 public PRIMARY_TOKEN_FEE_PERCENT = 5;
    uint256 public SECONDARY_TOKEN_FEE_PERCENT = 7;
    ...
    function addChallenge(        
        ...
        uint256 eventIdAddChallenge)external override{
            _addChallenge(collection, assetId, amount, airdropStartAt, airdropEndAt, eventIdAddChallenge);
    }
    /**
    ...
    function _addChallenge(        
        ...
        uint256 airdropStartAt,
        uint256 airdropEndAt,
        uint256 eventIdAddChallenge) internal returns (bytes32){
            ...
            emit AddChallenge(challengeId, _msgSender(), collection, assetId, amount, airdropStartAt, airdropEndAt, eventIdAddChallenge);            
    ...
    * @param eventIdAirDropChallenge a tracking id used to sync db
    ...
    function airdropChallenge(
        ...
        uint256 eventIdAirDropChallenge
            ...
            emit AirDropChallenge(challengeId, receiver, amount, eventIdAirDropChallenge);
    ...
    function withdrawChallenge(bytes32 challengeId, uint256 eventIdWithdrawChallenge)external  override returns (bool){
    ...
    function addTokenChallenge(        
        ...
        uint256 airdropStartAt,
        uint256 airdropEndAt,
        uint256 eventIdAddTokenChallenge)external override returns (bytes32){
            require(token != address(0), "Challenge Exchange: Token address not valid");
            IERC20 challengeToken = IERC20(token);
            uint256 tokenAirdropFee = _getTokenAirdropFee(token, tokenAmount);

            require(challengeToken.balanceOf(_msgSender()) >= uint256(winnerCount).mul(tokenAmount.add(tokenAirdropFee)) , "Challenge Exchange: Insufficient balance");
            require(airdropStartAt > block.timestamp, "Challenge Exchange: invalid start at airdrop");
            require(airdropEndAt > airdropStartAt, "Challenge Exchange: invalid start at airdrop");
                                
            _challengeIdCounter.increment();
            bytes32 challengeId = keccak256(
                abi.encodePacked(block.timestamp, msg.sender, _challengeIdCounter.current())
            );
            ...
            emit AddTokenChallenge(challengeId, _msgSender(), token, winnerCount, tokenAmount, airdropStartAt, airdropEndAt, tokenAirdropFee, eventIdAddTokenChallenge);

            bool success = challengeToken.transferFrom(_msgSender(), address(this), winnerCount.mul(tokenAmount));
            require(success, "Challenge Exchange: Token transfer did not succeeded ");            
            return challengeId;
    }
    ...
    function airdropTokenChallenge(
        ...
        uint256 eventIdAirDropTokenChallenge
            ...
            TokenChallenge storage challenge = tokenChallenges[challengeId];
            require(challenge.seller == _msgSender(),"Challenge Exchange: caller not an owner");
            require(challenge.winnerCount > 0, "Challenge Exchange: Airdrop done");
            require(block.timestamp >= challenge.airdropStartAt, "Challenge Exchange: invalid start at airdrop");

            uint256 tokenAirdropFee = _getTokenAirdropFee(challenge.token, challenge.tokenAmount);
            IERC20 challengeToken = IERC20(challenge.token);
            require(challengeToken.balanceOf(_msgSender()) >=  tokenAirdropFee, "Challenge Exchange: Insufficient balance");      

            challenge.winnerCount =  uint256(challenge.winnerCount).sub(1); 

            bool success = challengeToken.transferFrom(_msgSender(), owner(), tokenAirdropFee);
            require(success, "Challenge Exchange: Airdrop fee transfer did not succeeded ");

            bool success1 = challengeToken.transferFrom(_msgSender(), receiver, challenge.tokenAmount);
            require(success1, "Challenge Exchange: Airdrop transfer did not succeeded ");
            
            emit AirDropTokenChallenge(challengeId, receiver, challenge.tokenAmount, eventIdAirDropTokenChallenge);
            return true;
    }

    function withdrawTokenChallenge(bytes32 challengeId, uint256 eventIdWithdrawTokenChallenge)external  override returns (bool){
        TokenChallenge memory challenge = tokenChallenges[challengeId];
        require(challenge.seller == _msgSender(),"Challenge Exchange: caller not an owner");
        require(challenge.airdropEndAt < block.timestamp, "Challenge exchange airdrop not ended");

        require(challenge.winnerCount > 0, "Challenge Exchange: No token left.");

        IERC20 challengeToken = IERC20(challenge.token);
        bool success = challengeToken.transferFrom(address(this), _msgSender(), challenge.winnerCount.mul(challenge.tokenAmount));
            require(success, "Challenge Exchange: Token transfer did not succeeded ");   
    ...
    function getAirdropFeePercent(address token) external view returns (uint256){
       return _getAirdropFeePercent(token);
    }

    function _getAirdropFeePercent(address token) internal view returns (uint256){
        if (token == address(MARKET_TOKEN)) {
    ...
    function _getTokenAirdropFee(address token, uint256 tokenAmount) internal view returns (uint256){
        uint256 airdropFeePercent = _getAirdropFeePercent(token);

        return airdropFeePercent.mul(tokenAmount).div(100);
    }

    function setPrimaryTokenPercent(uint256 percent) onlyOwner() external  override returns (bool){
        PRIMARY_TOKEN_FEE_PERCENT = percent;
        emit SetPrimaryTokenPercent(percent);
        return true;
    }

    function setSecondaryTokenPercent(uint256 percent) onlyOwner() external  override returns (bool){
...
interface IExchangeChallenge {
     ...
     */
    event AddChallenge(bytes32 challengeId, address indexed seller, address indexed collection, uint256  assetId, uint256  amount, uint256  airdropStartAt,  uint256 airdropEndAt, uint256 eventIdAddChallenge);
     ...
     function addChallenge(        
        ...
        uint256 airdropStartAt,
        uint256 airdropEndAt,
        uint256 eventIdAddChallenge) external;
    /**
     ...
     * @dev Emitted when `challengeId` challenge is created.
     */
    event AirDropChallenge(bytes32 challengeId, address receiver, uint256 amount, uint256 eventIdAirDropChallenge);
    /**
     * @dev airdrop NFT token
     */
    function airdropChallenge(bytes32 challengeId, address receiver, uint256 amount, uint256 eventIdAirDropChallenge) external returns (bool);
    /**
    ...
    * @dev withdraw nft when airdrop is ended
     */
    function withdrawChallenge(bytes32 challengeId, uint256 eventIdWithdrawChallenge) external returns (bool);
    
    event WithdrawChallenge(bytes32 challengeId, uint256 eventIdWithdrawChallenge);
    ...
    event AddTokenChallenge(bytes32 challengeId, address indexed seller, address indexed token, uint256 winnerCount, uint256 tokenAmount, uint256  airdropStartAt,  uint256 airdropEndAt, uint256 tokenAirdropFee, uint256 eventIdAddTokenChallenge);
     ...
     function addTokenChallenge(        
        ...
        uint256 airdropStartAt,
        uint256 airdropEndAt,
        uint256 eventIdAddTokenChallenge) external returns (bytes32);

    event AirDropTokenChallenge(bytes32 challengeId, address receiver, uint256 tokenAmount, uint256 eventIdAirDropTokenChallenge);

    function airdropTokenChallenge(bytes32 challengeId, address receiver, uint256 eventIdAirDropTokenChallenge) external returns (bool);

    event WithdrawTokenChallenge(bytes32 challengeId, uint256 eventIdWithdrawTokenChallenge);

    function withdrawTokenChallenge(bytes32 challengeId, uint256 eventIdWithdrawTokenChallenge) external returns (bool);

    event SetPrimaryTokenPercent(uint256 percent);
    function setPrimaryTokenPercent(uint256 percent)external   returns (bool);

    event SetSecondaryTokenPercent(uint256 percent);
