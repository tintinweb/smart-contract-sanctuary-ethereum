...
contract AtlendisMining is Ownable {
        ...
        returns (uint256, uint256)
    {
        ...
        return (_user.earned, _user.lastLiquidityRatio);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 lastLiquidityRatio = user.lastLiquidityRatio == 0
            ? 1e18
            : user.lastLiquidityRatio;
        ...
        if (totalDeposits > 0) {
            ...
            newLiquidityRatio =
                token.liquidityRatio +
                (newRewards * 1e18) /
                token.totalAdjustedDeposits;
        }

        return
            user.earned +
            (userDeposits * newLiquidityRatio) /
            lastLiquidityRatio -
            userDeposits;
    }
    ...
    modifier updateRewards(address from) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            uint256 lastLiquidityRatio = user.lastLiquidityRatio == 0
                ...
                : user.lastLiquidityRatio;
            user.earned +=
                (userDeposits * token.liquidityRatio) /
                lastLiquidityRatio -
                userDeposits;

            // register user earnings impact over whole distribution
            uint256 oldAdjustedBalance = (userDeposits * 1e18) /
                lastLiquidityRatio;
            uint256 newAdjustedBalance = (userDeposits * 1e18) /
                token.liquidityRatio;
            token.totalAdjustedDeposits -=
                oldAdjustedBalance -
                newAdjustedBalance;
    ...
    function withdraw(uint256 tokenId) external updateRewards(msg.sender) {
        require(
            msg.sender == stakedNFTs[tokenId].owner,
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (totalDeposits == 0) {
                ...
                token.totalAdjustedDeposits -=
                    (stakedNFTs[tokenId].deposit * 1e18) /
                    token.liquidityRatio;
            }
        }
