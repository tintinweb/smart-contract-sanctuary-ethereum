...
contract Raffle is Ownable, IERC721Receiver {
    ...
    function transferNftToRaffle(address _nftContractAddress, uint256 _nftTokenId) external onlyOwner {
    ...
    function buyTicket(uint256 _amountOfTickets) external payable {
    ...
    function drawWinner(uint256 _gweiForFulfillPrayer) external payable onlyOwner returns (uint256) {
    ...
    // refund ticket 
    function refundTickets() external {
