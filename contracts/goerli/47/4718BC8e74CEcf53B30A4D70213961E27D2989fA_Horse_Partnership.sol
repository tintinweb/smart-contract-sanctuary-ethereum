    ...
    ERC1155Burnable
{
    ...
    constructor(address indexContract)
        ERC1155("https://portal.dev.silks.io/api/HorseFracEqu/HorseFracEqu/")
    {
        fractNumber = 9;
        baseURI = "https://portal.dev.silks.io/api/HorseFracEqu/HorseFracEqu/";
        tokenCount = 1;
    ...
    function fractionalize(uint256 horseID) external whenNotPaused {
        ...
        require(
            ...
            "HORSE CANNOT BE IN COMMUNITY STABLE"
        );
        HorseContract.transferFrom(msg.sender, address(this), horseID);
        mintfract(msg.sender, horseID, fractNumber, "");
        if (!(HorseGovContract.exists(horseID))) {
            HorseGovContract.EQMint(msg.sender, horseID);
        } else if (
            HorseGovContract.exists(horseID) &&
            HorseGovContract.ownerOf(horseID) == address(this)
        ) {
    ...
    function reconstitute(uint256 horseID) external whenNotPaused {
        ERC721 HorseContract = ERC721(Index.getAddress("Horse"));
        ERC721 HorseGovContract = ERC721(Index.getAddress("HorseGovernance"));
        require(
            balanceOf(msg.sender, horseID) == fractNumber,
        ...
        burn(msg.sender, horseID, fractNumber);
