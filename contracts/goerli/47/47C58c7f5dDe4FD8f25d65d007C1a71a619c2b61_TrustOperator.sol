...
contract TrustOperator {
    FeedRegistryInterface constant priceFeedRegistry = FeedRegistryInterface(0x47Fb2585D2C56Fe188D0E6ec628a38b74fCeeeDf);
    ...
    // it needs to mint erc-6551 to create new trust

    // it transfers ownership when terms are met
    ...
    event TimeTrustMinted(uint tokenId, address indexed creator, address indexed beneficiary, uint indexed releaseTime);
    event PriceTrustMinted(uint tokenId, address indexed creator, address indexed beneficiary, uint releasePrice);
    ...
    function createTimeTrustFund(address beneficiary, uint256 releaseTime) external returns(address) {
        ...
        emit TimeTrustMinted(tokenId, msg.sender, beneficiary, releaseTime);
        return erc6551Account;

    }

    function createPriceTrustFund(address beneficiary, address base, address quote, uint256 releasePrice)  external returns (address) {
        address priceFeed = address(priceFeedRegistry.getFeed(base, quote));
        require(priceFeed != address(0), "Invalid Base or Quote");

        uint256 tokenId = trustFund.mintNewTrust(address(this));
        TrustTerms storage tokenToTerms = TokenToTerms[tokenId];
        tokenToTerms.beneficiary = beneficiary;
        tokenToTerms.creator = msg.sender;
        tokenToTerms.base = base;
        tokenToTerms.quote = quote;
        tokenToTerms.releasePrice = releasePrice;

        address erc6551Account = createERC6551(tokenId);
        tokenToTerms.trustFundAccount = erc6551Account;
        emit PriceTrustMinted(tokenId, msg.sender, beneficiary, releasePrice);
        return erc6551Account;
    ...
    function setTerms() external {}

    function setNewBenficiary() external {}

    function releaseTrustFund(uint256 tokenId) public {
        // check token id exists
        TrustTerms memory tokenToTerms = TokenToTerms[tokenId];
        if(tokenToTerms.releaseTime == 0) {
            // require current asset price to be >= releasePrice
             (
            /* uint80 roundID */,
            int256 price,
            /*uint startedAt*/,
            ...
            ) = priceFeedRegistry.latestRoundData(tokenToTerms.base, tokenToTerms.quote);
        ...
        trustFund.transferFrom(address(this), tokenToTerms.beneficiary, tokenId);
        emit TrustReleased(tokenId, tokenToTerms.creator, tokenToTerms.beneficiary);
    }
    ...
    // chainlink check on if anything has met terms yet, maybe chainlink off-chain computation can recieve the info and check
