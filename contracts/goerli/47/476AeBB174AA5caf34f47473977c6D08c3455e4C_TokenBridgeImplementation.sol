...
contract TokenBridgeImplementation is TokenBridge {
...
import "./TokenBridgeGovernance.sol";

contract TokenBridge is TokenBridgeGovernance, ReentrancyGuard {
    using BytesLib for bytes;

    /*
     *  @notice Send ERC20 token_bridge through portal.
     */
    function transferTokens(
        ...
        TokenBridgeStructs.TransferResult memory transferResult = _transferTokens(token, amount);
     ...
     */
    function _transferTokens(address token, uint256 amount) internal returns (TokenBridgeStructs.TransferResult memory transferResult) {
        // determine token_bridge parameters
        ...
        // query own token_bridge balance before transfer
        ...
        // query own token_bridge balance after transfer
        ...
        SafeERC20.safeApprove(IERC20(token), address(liquidityPool()), amount);
        liquidityPool().deposit(token, amount);

        // normalize amounts decimals
        uint256 normalizedAmount = normalizeAmount(amount, decimals);

        //        // track and check outstanding token_bridge amounts
        //        bridgeOut(token, normalizedAmount);

        transferResult = TokenBridgeStructs.TransferResult({
    ...
    function logTransfer(
        ...
        TokenBridgeStructs.Transfer memory transfer = TokenBridgeStructs.Transfer({
        payloadID : 1,
        amount : amount,
        tokenAddress : tokenAddress,
        tokenChain : tokenChain,
        ...
        address dstAddress = address(uint160(uint256(bridgeContracts(recipientChain))));
        sequence = zkBridge().send{value : callValue}(recipientChain, dstAddress, encodeTransfer(transfer));
    }

    /*
     * @notice Complete a transfer of an ERC20 token_bridge.
     *
     * @dev The msg.sender gets paid the associated fee.
     *
     */
    function completeTransfer(uint16 srcChainId, bytes32 srcBlockHash, bytes memory mptProof) public {
        _completeTransfer(srcChainId, srcBlockHash, mptProof);
    }
    ...
    function _completeTransfer(uint16 srcChainId, bytes32 srcBlockHash, bytes memory mptProof) internal {
        (IZKBridge.VM memory vm, bool valid, string memory reason) = zkBridge().validateTransactionProof(srcChainId, srcBlockHash, mptProof);
        require(valid, reason);
        require(vm.srcAddress == bridgeContracts(vm.srcChainId), "invalid emitter");
        require(vm.dstAddress == address(this), "invalid destination");

        TokenBridgeStructs.Transfer memory transfer = parseTransfer(vm.payload);
        ...
        ILiquidityPool pool = liquidityPool();
        address chainToken = pool.getChainToken(transfer.tokenChain, _truncateAddress(transfer.tokenAddress));
        //
        //        //
        //        //        require(
        //        //            unwrapWETH == false || address(transferToken) != address(WETH()),
        //        //            "invalid token_bridge, can only unwrap WETH"
        //        //        );
        //
        // query decimals
        (, bytes memory queriedDecimals) = address(chainToken).staticcall(abi.encodeWithSignature("decimals()"));
        uint8 decimals = abi.decode(queriedDecimals, (uint8));

        // adjust decimals
        uint256 transferAmount = deNormalizeAmount(transfer.amount, decimals);

        pool.transferToken(chainToken, transferRecipient, transferAmount);
    }
    ...
    function bridgeOut(address token, uint256 normalizedAmount) internal {
        ...
        _setOutstandingBridged(token, outstanding + normalizedAmount);
    }

    function bridgedIn(address token, uint256 normalizedAmount) internal {
        _setOutstandingBridged(
            token,
    ...
    function encodeTransfer(TokenBridgeStructs.Transfer memory transfer)
    public
    pure
    ...
    public
    pure
    returns (TokenBridgeStructs.Transfer memory transfer)
    {
        uint256 index = 0;
...
import "@openzeppelin/contracts/proxy/ERC1967/ERC1967Upgrade.sol";
import "./TokenBridgeGetters.sol";
import "./TokenBridgeSetters.sol";

contract TokenBridgeGovernance is TokenBridgeGetters, TokenBridgeSetters, ERC1967Upgrade {
    ...
    function setLiquidityPool(address liquidityPool) public onlyOwner {
        _setLiquidityPool(liquidityPool);
    }
...
contract TokenBridgeGetters is TokenBridgeState {
    ...
    function bridgeContracts(uint16 chainId) public view returns (bytes32) {
        return _state.bridgeImplementations[chainId];
    }

    function liquidityPool() public view returns (ILiquidityPool) {
        return ILiquidityPool(_state.liquidityPool);
    }
...
contract TokenBridgeSetters is TokenBridgeState {
    function _setInitialized(address implementatiom) internal {
    ...
    function _setLiquidityPool(address liquidityPool) internal {
        _state.liquidityPool = liquidityPool;
    }
    ...
    function _setZkBridge(address zh) internal {
        _state.zkBridge = payable(zh);
    }

    function _setOutstandingBridged(address token, uint256 outstanding) internal {
...
interface IZKBridge is Structs {
    ...
    function send(uint16 dstChainId, address dstContractAddress, bytes memory payload) external payable returns (uint64 sequence);

    function validateTransactionProof(uint16 srcChainId, bytes32 srcBlockHash, bytes memory mptProof) external view returns (Structs.VM memory, bool, string memory);
}
...
contract TokenBridgeStorage {
    ...
    struct State {
        ...
        uint256 lockTime;
        Provider provider;
        // Mapping of consumed token_bridge transfers
        ...
        // Mapping of token_bridge contracts on other chains
        mapping(uint16 => bytes32) bridgeImplementations;
    }
}

contract TokenBridgeState {
    TokenBridgeStorage.State _state;
...
contract TokenBridgeStructs {
    struct Transfer {
        // PayloadID uint8 = 1
        uint8 payloadID;
        // Amount being transferred (big-endian uint256)
        uint256 amount;
        // Address of the token_bridge. Left-zero-padded if shorter than 32 bytes
        bytes32 tokenAddress;
        // Chain ID of the token_bridge
        uint16 tokenChain;
