...
contract FrankenMonsters is ERC721, Owned {
    ...
    address internal _frankenPunksContractAddr;

    /**
     * @param baseTokenURI A string you want the token URI to be set to, will be used as placeholder URI until reveal
     * @param frankenPunksContractAddr The contract address to access FrankenPunks ownership from
     */
    constructor(
        string memory baseTokenURI,
        address frankenPunksContractAddr
    ) ERC721("FrankenMonsters", "FM") Owned(msg.sender) {
        _baseTokenURI = baseTokenURI;
        _frankenPunksContractAddr = frankenPunksContractAddr;
    }
    ...
    /**
     * @notice Allows the contract owner to mint legendary FrankenMonseters and to airdrop all tokens to existing FrankenPunks holders.
     */
    function mintAllTokens() external onlyOwner {
        if (_totalSupply == MAX_SUPPLY) {
            revert AllTokensMinted();
        }

        IFrankenPunks frankenPunks = IFrankenPunks(_frankenPunksContractAddr);

        for (uint16 i = 0; i < MAX_SUPPLY; i++) {
            uint16 tokenId = STARTING_INDEX + i;
            address receiver = msg.sender;

            if (i >= LEGENDARY_SUPPLY) {
                address frankenPunksOwner = frankenPunks.ownerOf(i - LEGENDARY_SUPPLY);

                if (frankenPunksOwner != address(0)) {
