...
import "IERC721.sol";
import "IChubbyKaijuDAOSerum.sol";
/***************************************************************************************************
...
contract ChubbyKaijuDAOGen2 is ERC721Enumerable, Ownable{
    ...
    uint256 zombieMinted;
    uint256 alienMinted;
    uint256 undeadMinted;
    uint256 superMinted;

    uint256 public constant ALIEN_BASE = 6666;
    uint256 public constant UNDEAD_BASE = 10000;
    ...
    IChubbyKaijuDAOSerum private chubbykaijudaoserum;

    constructor(address _serumAddress) ERC721("ChubbyKaijuDAOGen2", "CKAIJU2"){
        chubbykaijudaoserum = IChubbyKaijuDAOSerum(_serumAddress);
    }
    ...
    function mutateWithSerum(uint256 typeId, uint256 zombieId) external {
        ...
        require(chubbykaijudaoserum.balanceOf(msg.sender, typeId)>0,"Must own this type of gear");

        if(typeId == 0){ // RADIO
            chubbykaijudaoserum.burnSerumForAddress(typeId, msg.sender);
            alienMinted++;
            ...
            chubbykaijudaoserum.burnSerumForAddress(typeId, msg.sender);
            superMinted++;
            _mint(msg.sender, superMinted+SUPER_BASE);
            traits[superMinted+SUPER_BASE] = 1;
        }else if(typeId == 3) { // SUPER SERUM
            chubbykaijudaoserum.burnSerumForAddress(typeId, msg.sender);
            _burn(zombieId);
            superMinted++;
            _mint(msg.sender, superMinted+SUPER_BASE);
            traits[superMinted+SUPER_BASE] = 2;
            traits[zombieId] = 10;
        }
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function validTokens() public returns (uint256[] memory){
        return _allTokens;
    }
}
...
interface IChubbyKaijuDAOSerum {
    function burnSerumForAddress(uint256 typeId, address burnTokenAddress) external;
