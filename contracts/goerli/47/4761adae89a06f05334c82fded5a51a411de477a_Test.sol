    ...
    PausableUpgradeable
{
    ...
    function blocklist(address _account) public onlyRole(BLACKLISTER_ROLE) {
        blocklisted[_account] = true;
        emit Blocklisted(_account);
    }

    function blocklistBulk(address[] memory accounts)
        external
        onlyRole(BLACKLISTER_ROLE)
    {
        for (uint256 i = 0; i < accounts.length; i++) {
            blocklist(accounts[i]);
        }
    }

    function unBlocklist(address _account) external onlyRole(BLACKLISTER_ROLE) {
    ...
    function foundOnlyOnUpgrade() public view returns (bool) {
        return true;
    }
}