...
//        mintStatus = MintStatus.Phunks;
    ...
    function maxMintsInternal(
        ...
        if (mintStatus == MintStatus.PreMint) {
            ...
            return (eligibleForAllowlist(wallet, _alProof) ? 1 : 0) +
            (eligibleForPhunk(wallet, _phunkProof) ? 1 : 0);
        }
    ...
    function getNumFree(
        ...
        else if (mintStatus == MintStatus.PublicExtended || mintStatus == MintStatus.ReserveOnly) {
            return _generalGetNumFree(wallet, quantity, _phunkProof, _alProof);
        }
