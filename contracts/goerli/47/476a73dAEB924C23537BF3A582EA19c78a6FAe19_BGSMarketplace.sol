...
contract BGSMarketplace is ReentrancyGuard {
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < totalAuctions; i++) {
            if (
                IdToAuction[i + 1].is_started == true &&
    ...
    function createAuction(
        ...
        require(_startingPrice > _endingPrice, "Ending price must be less than starting price");
    ...
    // get dutch auction price at certain timestamp
    function getPrice(uint256 _auctionId) public view returns (uint) {
        uint256 timeElapsed = block.timestamp - IdToAuction[_auctionId].startAt;
        uint256 discountRate = (IdToAuction[_auctionId].startingPrice - IdToAuction[_auctionId].endingPrice) / (IdToAuction[_auctionId].endAt - IdToAuction[_auctionId].startAt);
        uint256 discount = discountRate * timeElapsed;
        return IdToAuction[_auctionId].startingPrice - discount;
    }

    // enter to auction

    function bid(uint256 _auctionId) public payable nonReentrant {
        bool is_tarted = IdToAuction[_auctionId].is_started;
        bool is_ended = IdToAuction[_auctionId].is_ended;

        require(is_tarted, "not started");
        require(is_ended == false, "ended");
        ...
        ERC721(nftAddress).transferFrom(address(this), msg.sender, nftId);
