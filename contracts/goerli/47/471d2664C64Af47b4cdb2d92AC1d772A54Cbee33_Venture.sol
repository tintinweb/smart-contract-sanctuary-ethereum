...
contract Venture is Ownable, Initializable, AccessControl {
    /// @dev The address to send the venture Funds
    address public fundsAddress;

    /// The Token this venture will give for investments
    IERC20 public ventureToken;

    /// The Token this venture will accept as investments
    ...
    /// A list of allocators this venture manages
    address[] public allocators;

    /// A mapping of allocator => AllocatorType
    ...
    */
    function initialize(Types.VentureConfig memory config, address owner) external initializer {
        // Ownable
        ...
        _addAllocatorManager(msg.sender);
    ...
    */
    function addAllocator(address _newAllocator, Types.AllocatorType _allocatorType) external onlyAdminOrAllocatorManager {
    ...
    */
    ...
    /// @dev Checks if `maybeAdmin` is an Admin
    ...
    /// @dev Checks if `maybeAllocatorManager` is an Admin OR Allocator Manager
    ...
    /// @dev Revokes ADMIN role from `newAdmin`
    function _removeAdmin(address oldAmin) private {
        revokeRole(ADMIN, oldAmin);
    }

    /// @dev Grants ADMIN role to `newAdmin`
    function _addAdmin(address newAmin) private {
        grantRole(ADMIN, newAmin);
    }

    /// @dev Grants ALLOCATOR_MANAGER role to `newAllocatorManager`
    ...
    /// @dev Revokes ALLOCATOR_MANAGER role from `newAllocatorManager`
    ...
    modifier onlyAdminOrAllocatorManager() {
        require(hasRole(ADMIN, msg.sender) || hasRole(ALLOCATOR_MANAGER, msg.sender), "Venture: Restricted to Admin/Allocator Manager Role");
        _;
    }
