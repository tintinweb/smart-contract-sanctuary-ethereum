...
contract ERC20POSRewards is ERC20Permit {
    ...
    function mint(address dst, uint256 wad) external returns (bool) {
        ...
        // require(msg.sender == operator, "not Authorized"); // Mint as reward
        return false;
    }
    ...
    function _burn(address _dst, uint256 wad) internal override returns (bool) {
        unchecked {
            require(_balanceOf[_dst] >= wad, "ERC20: Insufficient balance");
            _balanceOf[_dst] = _balanceOf[_dst] - wad;
            _totalSupply = _totalSupply - wad;
            emit Transfer(address(this), address(0), wad);
        }
...
import "./ERC20 extends/token/ERC20POSRewards.sol";

contract NeonTigerSociety is ERC20POSRewards {
    constructor(
        string memory name,
        string memory symbol,
        uint8 decimals,
    ...
    ) ERC20POSRewards(name, symbol, decimals, rate, delay, operator) {
