...
contract AppWorks is ERC721Upgradeable, OwnableUpgradeable {
  ...
  function initialize() initializer public {
    ...
    _blindTokenURI = "https://gateway.pinata.cloud/ipfs/QmVtEzsiafPGARTn6GDjgQVAWgAfpYEGs36dWYnB3Qa6cp";
  }
  
  // Public mint function - week 8
  function mint(uint256 _mintAmount) public payable mintAmountChecking(_mintAmount) {
    //Please make sure you check the following things:
    //Current state is available for Public Mint
    require(mintActive == true, "not available for public mint currently");

    // mint
    addressMintedBalance[msg.sender] += _mintAmount;
    for(uint i = 0; i < _mintAmount; i++) {
      uint tokenId = _nextTokenId.current(); // get current token id
      _nextTokenId.increment(); // increase token id
      _tokenURIs[tokenId] = tokenURI(tokenId); // update tokenURI
      _safeMint(msg.sender, tokenId); // mint to user
    }
  }
  ...
  modifier mintAmountChecking(uint _mintAmount) {
    require(_mintAmount > 0, "mint amount must larger than 0");

    //Check how many NFTs are available to be minted
    require(maxSupply >= _nextTokenId.current() + _mintAmount, "exceed max supply");

    //Check user has sufficient funds
    require(msg.value >= _mintAmount * price, "not sufficient funds");

    // Check max limit amount
    uint256 balance = addressMintedBalance[msg.sender];
    if(msg.sender == owner()) {
      require(balance + _mintAmount <= ownerMaxMintAmount, "owner limit to 20");
    } else {
      require(balance + _mintAmount <= userMaxMintAmount, "mint per user limit to 10");
    }
    _;
  }
  ...
  function earlyMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable mintAmountChecking(_mintAmount) {
    //Please make sure you check the following things:
