...
contract MoneyInstant is IMoneyInstant, ReentrancyGuard, Ownable {
    ...
    function createInstantTransfer(
        ...
        require(deposits.length > 0, 'length of deposits is zero');
        require(recipients.length > 0, 'length of recipients is zero');
        require(
            deposits.length == recipients.length,
            'length of recipients is not equal to length of deposits'
        );
...
//0x9fAcc753aC0BCEC5143065004175841228278396
...
interface IMoneyInstant {
    ...
    /**
     * @notice Data object for a token and its balance
     */
    struct TokenBalance {
        address tokenAddress;
        uint256 balance;
     ...
     * @param recipients An array of recipients addresses.
     * @param deposits The amount of tokens to be streamed for each recipient.
