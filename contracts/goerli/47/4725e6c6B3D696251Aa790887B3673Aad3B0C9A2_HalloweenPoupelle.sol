...
contract HalloweenPoupelle is ERC721A, IHalloweenNFT, ReentrancyGuard, Ownable {
    ...
    IContractAllowListProxy CAL; //Contract Allow List 
    ...
    uint256 public calLevel = 1; //Contract Allow List
//    address public withdrawAddress = 0x4262098A3a607b263c414F3D16ABf2cc8C1F3711;
    address public withdrawAddress = 0xcD58376dE988F3a8B131dE70D120C6d59f8A9096;
    bytes32 public whiteListSaleRoot;
    bytes32 public addWhiteListSaleRoot;
    string public baseExtension = '.json';
    string public baseURI = 'https://ipfs.io/ipfs/QmWs8k7hugmpR72Bjqua9ETtmp4GUqeBTdUXfARFy6aqkw/';
    ITokenURI public tokenuri;
    mapping(address => uint256) public VIPList;
    mapping(uint256 => bool) public LockList; // tokenLock
    mapping(address => bool) _lockOperators; // tokenLock
    bool public enableTokenLock = true; // tokenLock

    constructor() ERC721A("Halloween Poupelle", "HWP") {
        grantLockerRole(msg.sender);
        grantLockerRole(withdrawAddress);
...
//        _safeMint(msg.sender, 100);
   ...
   function wlMint(uint64 _saleType, uint256 _mintCount, uint256 _maxCount, bytes32[] calldata _merkleProof)
    ...
    //
    function setContractAllowListLevel(uint256 _value)
        external
        onlyOwner
    {
        calLevel = _value;
    ...
    function setICAL(address _address)
        external
        onlyOwner
    {
   ...
   function setApprovalForAll(address operator, bool approved)
        ...
        override(ERC721A)
    {
        if (address(CAL) != address(0)) {
            bool result = CAL.isAllowed(operator,calLevel);
    ...
    {
        if (address(CAL) != address(0)) {
            bool result = CAL.isAllowed(to,calLevel);
            require(result, "approve is prohibited");
        }
    ...
    //Token Lock section
    //
    function setEnableTokenLock(bool _state)
        external
        onlyOwner
    {
        enableTokenLock = _state;
    }

    function grantLockerRole(address candidate)
        public
        onlyOwner
    {
        require(!_lockOperators[candidate],'account is already has an operator role');
        _lockOperators[candidate] = true;
    }

    function revokeLockerRole(address candidate)
        external
        onlyOwner
    {
        checkLockerRole(candidate);
        delete _lockOperators[candidate];
    }

    function checkLockerRole(address operator) public view {
        require(_lockOperators[operator],'account is not an locker');
    }

    function setLock(uint256[] calldata tokenIds, bool[] calldata locks) external {
        require(tokenIds.length == locks.length, 'invalid value');
        require(enableTokenLock, 'token lock disabled');

        for (uint256 i = 0; i < tokenIds.length; i++) {
            address tokenOwner = ownerOf(tokenIds[i]);
            require( tokenOwner == msg.sender);
            LockList[tokenIds[i]] = locks[i];
            emit TokenLock(tokenOwner, msg.sender, tokenIds[i], locks[i]);
        }
    }

    function setAdminLock(uint256[] calldata tokenIds, bool[] calldata locks) external onlyLocker {
        require(tokenIds.length == locks.length, 'invalid value');
        for (uint256 i = 0; i < tokenIds.length; i++) {
            address tokenOwner = ownerOf(tokenIds[i]);
            LockList[tokenIds[i]] = locks[i];
    ...
    // unlock token is return tokenid;
    // lock token is return (tokenid * -1);
    function tokensAndLocksOfOwner(address owner) external view returns (int256[] memory) {
        unchecked {
            uint256 tokenIdsIdx;
            address currOwnershipAddr;
            uint256 tokenIdsLength = balanceOf(owner);
            int256[] memory tokenIds = new int256[](tokenIdsLength);
            ...
            for (uint256 i = _startTokenId(); tokenIdsIdx != tokenIdsLength; ++i) {
                ...
                if (currOwnershipAddr == owner) {
                    tokenIds[tokenIdsIdx] = int256(i);
                    if(LockList[i] && enableTokenLock){
                        //lock
                        tokenIds[tokenIdsIdx] *= -1;
                    }
                    tokenIdsIdx++;
                }
            }
    ...
    function _beforeTokenTransfers(
        ...
        if (enableTokenLock && from != address(0)) {
            require(LockList[startTokenId] == false , "LOCKED");
        }
    }
...
interface IHalloweenNFT{
    event TokenLock(address indexed owner, address indexed from, uint256 indexed tokenId, bool isLock);
}
