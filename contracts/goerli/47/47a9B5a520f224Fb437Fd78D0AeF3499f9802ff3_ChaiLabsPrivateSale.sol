...
contract ChaiLabsPrivateSale is Ownable {
    ...
    struct Sale {
        ...
        bool enabled;
        // if true, users can claim their tokens
        bool finalized;
    ...
    // sale id => sale round id => amount raised
    mapping(uint256 => mapping(uint256 => uint256)) public saleRoundRaisedAmounts;
    ...
    event SaleCreated(address indexed saleToken, address indexed admin, address paymentRecipient, uint256 rounds);
    ...
    function getSaleRounds(uint256 saleId) public view returns (SaleRound[] memory) {
        return saleRounds[saleId];
    }
    ...
    function createSale(
        ...
        emit SaleCreated(sale.saleToken, sale.admin, sale.paymentRecipient, saleRounds[nextSaleId].length);

        nextSaleId++;
    }

    function addSaleRound(uint256 saleId, SaleRound[] memory rounds) external validateSale(saleId) {
        _addSaleRound(saleId, rounds);
    }
    ...
    function contribute(
        ...
        _canContribute(saleId, roundId, amount);
    ...
    /**
        This function can be used to manually move the round if there is no hardcap on round
     */
    function moveToNextRoundManually(uint256 saleId, uint256 roundId) external validateSale(saleId) {
        SaleRound memory saleRound = saleRounds[saleId][roundId];

        require(saleRound.startTime > block.timestamp, "ChaiLabs: round not started yet");
        require(saleRounds[saleId].length - roundId > 1, "ChaiLabs: no rounds");
        require(_softCapFilled(saleId, roundId), "ChaiLabs: softcap not filled");
        require(saleRound.hardCap == 0, "ChaiLabs: hardcap not zero");

        saleRound.closed = true;
        saleRounds[saleId][roundId + 1].startTime = block.timestamp;
        emit SaleRoundClosed(saleId, roundId, block.timestamp);
    }

    function finalizeSale(uint256 saleId) external validateSale(saleId) {
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            ...
            if (rounds[i].paymentToken == address(0)) {
                payable(sale.paymentRecipient).transfer(totalDepositedAmounts[saleId][i]);
            } else {
                IERC20(rounds[i].paymentToken).safeTransfer(sale.paymentRecipient, totalDepositedAmounts[saleId][i]);
            }
        }
    ...
    /**
        ...
        user can claim there tokens back when sale ends
     */
    function claim(uint256 saleId) external validateSale(saleId) {
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            uint256 claimAmount = userContributions[msg.sender][saleId][i] * rounds[i].price;
            ...
            if (claimAmount > 0 && _softCapFilled(saleId, i)) {
                ...
                emit UserClaim(saleId, i, msg.sender, userContributions[msg.sender][saleId][i], claimAmount);
            }
        }
    }
    ...
    function claimBackContribution(uint256 saleId) external validateSale(saleId) {
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            ...
            require(rounds[i].startTime + rounds[i].saleDuration < block.timestamp, "ChaiLabs: round not finished yet");
            if (!_softCapFilled(saleId, i)) {
                ...
                if (rounds[i].paymentToken == address(0)) {
                    ...
                    IERC20(rounds[i].paymentToken).safeTransferFrom(address(this), msg.sender, depositAmount);
                }
    ...
    function _addSaleRound(uint256 saleId, SaleRound[] memory rounds) private {
        ...
        require(!sales[saleId].finalized, "ChaiLabs: finalized");
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            ...
            console.log(lastRoundEndTime, rounds[i].startTime);
            require(
                lastRoundEndTime < rounds[i].startTime && rounds[i].startTime != rounds[i].saleDuration,
