...
contract TherundownLPConsumer_v2_1 is ChainlinkClient {
    ...
    //constants
    uint internal minimumBet = 1 ether / 10;
    ...
    // sportId => (gameId => bets[])
    mapping(uint256 => mapping(bytes32 => Bet[])) bets;
    ...
    function placeBet(uint256 _sportId, bytes32 _gameId, uint256 _amount, uint8 _betType) public {
        require(_betType == 1 || _betType == 2, "Invalid bet type");
        bets[_sportId][_gameId].push(Bet(msg.sender, _amount, _gameId, _sportId, _betType));
    }
     ...
     * @param _days the date for the games to be queried (format in days (1,2,3,4,5,6,7)).
     */
    function requestGames(
        ...
        uint256 daysToTimestamp = _days * 24 * 60 * 60; 
        req.addUint("date", block.timestamp + daysToTimestamp);
