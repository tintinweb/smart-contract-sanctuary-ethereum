...
contract Investment is ERC20, ERC20Burnable, Ownable {
    ...
    function _swapTokens(address[] memory v2Path, bytes memory v3Path, uint256 amountIn, uint256 amountOutMinimum) internal returns (uint256) {
        ...
        if(v2Path.length != 0){
            ...
            (address inToken, uint24 fee) = abi.decode(v3Path, (address, uint24));
            IERC20(inToken).approve(0xE592427A0AEce92De3Edee1F18E0157C05861564, amountIn);
    ...
    function _adjustTokensHeld(address[] memory v2Path, bytes memory v3Path, uint256 amountIn) internal {
        ...
        if(v2Path.length != 0){
            ...
            if(bytesLength == 96){
                (address decodedTokenIn, uint24 fee, address decodedTokenOut) = abi.decode(v3Path, (address, uint24, address));
        ...
        if(IERC20(tokenOut).balanceOf(address(this)) == 0){
            if(isV3 == false){
                ...
                poolAddress = PoolAddress.computeAddress(uniswapV3Factory, PoolAddress.PoolKey(uniswapV2Router.WETH(), tokenOut, fee));
            }
