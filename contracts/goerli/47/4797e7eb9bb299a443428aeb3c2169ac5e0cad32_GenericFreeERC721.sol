...
// File: contracts/GenericFreeERC721.sol
...
contract GenericFreeERC721 is ERC721, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 _quantity) public {
    ...
    require(amountMinted[msg.sender] == 0, "You can not mint more than one token per wallet.");
    require(supply.current() + 1 <= maxSupply, "All tokens have been minted.");
    require(_quantity == 1, "You may only mint one token.");
    ...
    returns (string memory)
  {
    ...
    return string(abi.encodePacked(currentBaseURI, _tokenId.toString(), suffix));
  }
