...
contract OURGATEWAYGenesisWrapper is Ownable{
  string public baseURI;

  uint public price = 0.15 ether;

  uint public constant MAX_TOKENS_FOR_SALE = 1111;

  uint public numMinted = 0;

  address public keyPassContract;
  address public cardContract;
  address public crossmintAddress;

  function mint(uint num) public payable {
    uint256 amountRequired = price * num;
    ...
    IKeyPass(keyPassContract).mint(num, msg.sender);
    ICard(cardContract).mint(num, msg.sender);

    numMinted = newTotal;
  }

  function crossmint(address _to, uint num) public payable {
    uint256 amountRequired = price * num;
    require(msg.value >= amountRequired, "Not enough funds sent");
    ...
    IKeyPass(keyPassContract).mint(num, _to);
    ICard(cardContract).mint(num, _to);
