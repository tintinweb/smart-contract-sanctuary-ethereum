...
contract ZetaMPIEth is ZetaMPIBase {
    constructor(
    ...
    function send(ZetaInterfaces.SendInput calldata _input)
        external
        override
        whenNotPaused
    {
        bool success = IERC20(zetaToken).transferFrom(
            msg.sender,
            address(this),
            _input.zetaAmount
        );
        require(success == true, "ZetaMPI: error transferring Zeta");

        emit ZetaSent(
            msg.sender,
            _input.destinationChainId,
            _input.destinationAddress,
            _input.zetaAmount,
            _input.gasLimit,
            _input.message,
            _input.zetaParams
    ...
    function onReceive(
        ...
        bool success = IERC20(zetaToken).transfer(
            destinationAddress,
            zetaAmount
        );
        require(success == true, "ZetaMPI: error transferring Zeta");
...
contract ZetaMPIBase is Pausable {
    ...
    modifier onlyTssAddress() {
        require(
            msg.sender == tssAddress,
            "ZetaMPI: only TSS address can call this function"
        );
        _;
    }
    ...
    modifier onlyTssUpdater() {
        require(
            ...
            "ZetaMPI: only TSS updater can call this function"
        );
        _;
    }

    // update the TSS Address in case of Zeta blockchain validator nodes churn
    function updateTssAddress(address _address) external onlyTssUpdater {
        require(_address != address(0), "ZetaMPI: invalid tssAddress");
    ...
    function renounceTssAddressUpdater() external onlyTssUpdater {
        require(tssAddress != address(0), "ZetaMPI: invalid tssAddress");
    ...
    function send(ZetaInterfaces.SendInput calldata _input) external virtual {}
