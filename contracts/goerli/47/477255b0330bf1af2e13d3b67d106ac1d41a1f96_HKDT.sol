...
contract HKDT is ERC20 {
  ...
  string private constant _message = "this is HKDT v0.6"; 
  uint256 private constant USDT_TO_HKDT_RATE = 778;
  ...
  function deposit() public payable {
    uint256 hkdtAmount = msg.value * USDT_TO_HKDT_RATE;
    _mint(_msgSender(), hkdtAmount);
    emit Deposit(_msgSender(), msg.value);
  }

  function withdraw(uint256 hkdtAmount) public {
    uint256 usdtAmount = hkdtAmount / USDT_TO_HKDT_RATE;
    
    _burn(_msgSender(), hkdtAmount);
    payable(_msgSender()).transfer(usdtAmount);
