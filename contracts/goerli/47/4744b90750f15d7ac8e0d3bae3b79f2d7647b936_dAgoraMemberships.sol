    ...
    ReentrancyGuard
{
    ...
    function mintFreeClaim() public isPaused isNotMember nonReentrant {
        uint256 tokenId = totalSupply() + 1;
        requestAccessCheck(
            msg.sender,
            rewardedRole,
            this.fulfillFreeClaim.selector,
            abi.encode(msg.sender, tokenId)
        );
        emit ClaimRequested(msg.sender);
    }

    function fulfillFreeClaim(bytes32 requestId, uint256 access)
        public
        checkRole(requestId, access)
    {
        ...
        emit MembershipMinted(
            receiver, tokenId, tokenTier[tokenId], expires[tokenId]
            );
