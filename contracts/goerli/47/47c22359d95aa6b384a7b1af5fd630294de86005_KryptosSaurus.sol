...
contract KryptosSaurus is ERC721Enumerable, Ownable 
{
    ...
    string public baseURI = "ipfs://QmVAHhRbtWarFkpW2VZVsfFanashkMsGKrdtHiMcknsrfq/";
  ...
  constructor() ERC721("Kryptos Saurus", "KS") {}

  function _baseURI() internal view virtual override returns (string memory) {
    return baseURI;
  }
    function WhitelistMint(uint256 amount, bytes32[] memory proof) external payable {
  
  require(preSaleIsActive || saleIsActive, "Whitelist Sale must be active to mint");
    require(!whitelist || verify(proof), "Address not whitelisted");
    require(msg.value >= 0 ether, "Ether value sent is not correct");  
    require(!preSaleIsActive || presaleTokensSold + amount <= PRESALE_LIMIT, "Purchase would exceed max supply");
   require(amount > 0 && amount <= 3, "Max 3 KS per transaction");
    require(addressMintedBalance[msg.sender] + amount <= perAddressLimit, "Max KS per address exceeded");
   ...
   function publicMint(uint256 amount) public payable {
    ...
    require(saleIsActive, " public sale must be Open to mint" );
    require(totalSupply() + amount < MAX_TOKENS + 1, "We Are Sold Out!!!");
...
for (uint256 i = 1; i <= amount; i++) 
   {
    ...
    address private constant payoutAddress1 =
       0x4F930fAdBb9667A3E3dd8BCFABD546e4927e2FC2;
