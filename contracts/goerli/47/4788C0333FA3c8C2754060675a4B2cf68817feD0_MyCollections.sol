...
// File contracts/contract.sol
        ...
        DefaultOperatorFilterer
    {
        ...
        uint256 public maxPreSaleMintAmountPerTx       = 2;
        uint256 public maxPublicSaleMintAmountPerTx    = 2;
        ...
        uint256 private _publicSaleDiscountCode;
        uint256 private _preSaleDiscountCode;
        uint256 private _publicSaleDiscountValue;
        uint256 private _preSaleDiscountValue;
        ...
        modifier checkQuantity(uint256 quantity) {
            ...
            require(
                ...
                "Would exceed max supply of Tokens"
            );
            _;
        }

        modifier price(uint256 pricePer, uint256 quantity) {
            require(pricePer * quantity == msg.value, "Incorrect funds");
            _;
        }
        ...
        function reserveMint(
            uint256 quantity
        ) public onlyOwner checkQuantity(quantity) {
            require(
                totalSupply() + quantity <= MAX_SUPPLY,
            ...
            checkQuantity(quantity) 
        {
            ...
            require(quantity <= maxPreSaleMintAmountPerTx, "exceeds max per transaction");
            if(_cProof == _preSaleDiscountCode ) {
                uint256 tokenPrice = (PRESALE_PRICE * _preSaleDiscountValue) / 100;
                require(tokenPrice * quantity == msg.value, "Insufficient funds");
            } else {
                require(PRESALE_PRICE * quantity == msg.value, "Insufficient funds");
            }
            require(
            ...
            checkQuantity(quantity)
        {
            ...
            require(quantity <= maxPublicSaleMintAmountPerTx, "exceeds max per transaction");
            if(_cProof == _publicSaleDiscountCode ) {
                uint256 tokenPrice = (PUBLIC_PRICE * _publicSaleDiscountValue) / 100;
                require(tokenPrice * quantity == msg.value, "Insufficient funds");
            } else {
                require(PUBLIC_PRICE * quantity == msg.value, "Insufficient funds");
            }
            require(
        ...
        function setMaxPreSaleMintAmountPerTx(uint256 _newQty) public onlyOwner
        {
            maxPreSaleMintAmountPerTx = _newQty;
        }

        function setMaxPublicSaleMintAmountPerTx(uint256 _newQty) public onlyOwner
        {
            maxPublicSaleMintAmountPerTx = _newQty;
        }
        ...
        function setPublicSaleDiscount(uint256 _newCode, uint256 _newVal) public onlyOwner {
            _publicSaleDiscountCode = _newCode;
            _publicSaleDiscountValue = _newVal;
        }

        function setPreSaleDiscount(uint256 _newCode, uint256 _newVal) public onlyOwner {
            _preSaleDiscountCode = _newCode;
            _preSaleDiscountValue = _newVal;
        }
        ...
        struct MaxTrxMintInfo {
            uint256 maxPreSaleMintAmountPerTx;
            uint256 maxPublicSaleMintAmountPerTx;
        }
        ...
        function getMintInfo() external view 
            returns(
                ...
                MaxTrxMintInfo memory maxTrxMintInfo,

                uint256 _totalSupply,
                uint256 _maxSupply,
                bytes32 _merkleRoot
        ) {
            saleInfo = SaleInfo(_preSaleActive, _publicSaleActive);
            priceInfo = PriceInfo(PRESALE_PRICE, PUBLIC_PRICE);
            maxMintInfo = MaxMintInfo(maxPreSaleMintAmount, maxPublicSaleMintAmount);
            maxTrxMintInfo = MaxTrxMintInfo(maxPreSaleMintAmountPerTx, maxPublicSaleMintAmountPerTx);
