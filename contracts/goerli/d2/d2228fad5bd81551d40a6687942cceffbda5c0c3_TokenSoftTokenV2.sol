...
// File: contracts/capabilities/Buyable.sol
pragma solidity 0.6.12;

contract Buyable is ERC20, MinterRole {
  event Buy(
  ...
  function _buy(address minter, address to, uint256 amount, uint256 tier, uint256 orderChainId, bytes32 order) internal returns (bool) {
      ERC20._mint(to, amount);
      emit Buy(minter, to, amount, tier, orderChainId, order);
      return true;
  }

  /**
   * Capture buy order information. i.e. Moving value into Ethereum.
   * Old value is custodied, new value is minted. thus requires a minter to initiate
  */
  function buy(address account, uint256 amount, uint256 tier, uint256 orderChainId, bytes32 order) public onlyMinter returns (bool) {
      return Buyable._buy(msg.sender, account, amount, tier, orderChainId, order);
  }
}
...
// File: contracts/capabilities/Sellable.sol
pragma solidity 0.6.12;

contract Sellable is ERC20, BurnerRole {
  event Sell(
  ...
  function _sell(address burner, address from, uint256 amount, uint256 tier, uint256 settlementChainId, string memory settlementAddress, bytes32 order) internal returns (bool) {
      ERC20._burn(from, amount);
      emit Sell(burner, from, amount, tier, settlementChainId, settlementAddress, order);
      return true;
  }

  /**
    Capture a sell order information, i.e. transfering value away from Ehtereumm
    Value on ethereum is burnt, thus requires a burner to initiate.
  */
  function sell(address account, uint256 amount, uint256 tier, uint256 settlementChainId, string memory settlementAddress, bytes32 order) public onlyBurner returns (bool) {
      return _sell(msg.sender, account, amount, tier, settlementChainId, settlementAddress, order);
  }
}
...
contract TokenSoftToken is Proxiable, ERC20Detailed, ERC1404, OwnerRole, Whitelistable, Mintable, Burnable, Revocable, Pausable, Buyable, Sellable {
