...
import "./utils/AnonUtils.sol";
import "./utils/AnonErrors.sol";
  ...
  AccessControlUpgradeable
{
  ...
  bytes32 public constant STEAL_ROLE = keccak256("STEAL_ROLE");
  ...
  bool public anonEthEnabled;

  bool public anonErc20Enabled;

  bool public customEthEnabled;

  bool public customErc20Enabled;
  ...
  // AnonID to Anon
  mapping(uint256 => AnonData) public anones;

  mapping(uint256 => Native) public natives;

  mapping(uint256 => Custom) public customs;

  mapping(uint256 => uint256[]) public stealIds;
  ...
  // Custom strings to array of anonIDs of that string
  ...
  function toggleAnonEth() external onlyRole(DEFAULT_ADMIN_ROLE) {
    anonEthEnabled = !anonEthEnabled;
  }

  function toggleAnonErc20() external onlyRole(DEFAULT_ADMIN_ROLE) {
    anonErc20Enabled = !anonErc20Enabled;
  }

  function toggleCustomEth() external onlyRole(DEFAULT_ADMIN_ROLE) {
    customEthEnabled = !customEthEnabled;
  }

  function toggleCustomErc20() external onlyRole(DEFAULT_ADMIN_ROLE) {
    customErc20Enabled = !customErc20Enabled;
  }
  ...
  function addSteal(
    ...
    uint256 _stealId,
    bytes32 _entropy
  ) external onlyRole(STEAL_ROLE) {
    if (!_exists(_anonId)) revert NonexistentToken();
    stealIds[_anonId].push(_stealId);
    anones[_anonId].entropy = _entropy;
    anones[_anonId].level++;
  }
  ...
  {
    _entropy = anones[_anonId].entropy;
    _level = anones[_anonId].level;
    _anonType = anones[_anonId].anonType;
    _arweaveId = anones[_anonId].arweaveId;
    _ownership = natives[_anonId].ownership;
    _contract = natives[_anonId].contract_;
    _id = natives[_anonId].id;
    _target = customs[_anonId].target;
  }
  ...
  // Returns array of all anonIDs associated with a given Custom
  ...
  function getStealIds(uint256 _anonId)
    external
    view
    returns (uint256[] memory)
  {
    if (!_exists(_anonId)) revert NonexistentToken();
    return stealIds[_anonId];
  }

  function getType(uint256 _anonId) external view returns (AnonType) {
    if (!_exists(_anonId)) revert NonexistentToken();
    return anones[_anonId].anonType;
  }

  function getLevel(uint256 _anonId) external view returns (uint128) {
    if (!_exists(_anonId)) revert NonexistentToken();
    return anones[_anonId].level;
  }
  ...
  function storeBasic(
    ...
    anones[_anonId].arweaveId = _arweaveId;
  ...
  function anon(
    ...
    if (
      !AnonUtils.verify(
        keccak256(
          abi.encodePacked(_contract, _id, _arweaveId, _expiry, msg.sender)
        ),
        _signature,
        signer
      )
    ...
    storeBasic(totalSupply, _arweaveId, _signature);
    children[_contract][_id].push(totalSupply);
    anones[totalSupply].anonType = AnonType.NATIVE;
    natives[totalSupply].contract_ = _contract;
    natives[totalSupply].id = _id;
    natives[totalSupply].ownership = AnonUtils.isOwned(
      _contract,
      _id,
      msg.sender
    );
    ...
    emit AnonCreated(totalSupply, _contract, _id, _arweaveId);
    return totalSupply;
  }

  function mintAnonEth(
    address _contract,
    uint256 _id,
  ...
  function mintAnonErc20(
    ...
    if (!anonErc20Enabled) revert Erc20Disabled();
    tokenId = anon(_contract, _id, _arweaveId, _expiry, _signature);
  ...
  // Takes a string (base64 encoded JSON) and Anones it
  function custom(
    string memory _target,
    string memory _arweaveId,
    uint256 _expiry,
    ...
    if (
      !AnonUtils.verify(
        keccak256(abi.encodePacked(_target, _arweaveId, _expiry, msg.sender)),
        _signature,
        signer
      )
    ) revert ProofInvalid();
    ...
    customChildren[keccak256(bytes(_target))].push(totalSupply);

    customs[totalSupply].target = _target;
    anones[totalSupply].anonType = AnonType.CUSTOM;

    _mint(msg.sender, totalSupply);

    emit CustomCreated(totalSupply, _target, _arweaveId);
    return totalSupply;
  }
  ...
  function mintCustomEth(
    ...
    if (!customEthEnabled) revert EthDisabled();
    if (msg.value < priceEth) revert InsufficientPayment();
    tokenId = custom(_target, _arweaveId, _expiry, _signature);
    (bool sentTreasury, ) = treasury.call{value: msg.value}("");
    if (!sentTreasury) revert TransferFailed();
  }
  ...
  function mintCustomErc20(
    ...
    if (!customErc20Enabled) revert Erc20Disabled();
    tokenId = custom(_target, _arweaveId, _expiry, _signature);
    SafeERC20Upgradeable.safeTransferFrom(
      erc20,
      msg.sender,
  ...
  function burn(uint256 _anonId) external {
    _burn(_anonId);
  }

  function _burn(uint256 _anonId) internal override(ERC721Upgradeable) {
    if (!_exists(_anonId)) revert NonexistentToken();
    if (msg.sender != ownerOf(_anonId)) revert NotOwnedToken();
    super._burn(_anonId);
  }
...
interface IERC721UpgradeableV1 {
  function supportsInterface(bytes4 interfaceId) external view returns (bool);
}
...
library AnonUtils {
  ...
  function isOwned(
    ...
    if (
      !IERC721UpgradeableV1(_contract).supportsInterface(0x80ac58cd) &&
