...
contract PersonaNFTStake is IPersona, Ownable {
    ...
    event StakedNFT(address indexed holder, uint256 tokenID);
    event ClaimedCredit(address indexed holder, uint16 amount);
    ...
    function stake(uint256[] memory _tokenIds) external {
        for (uint i; i < _tokenIds.length; i++) {
            ...
            emit StakedNFT(msg.sender, _tokenIds[i]);
        }
    }
    ...
    function claimCredit() public {
        ...
        emit ClaimedCredit(msg.sender, uint16(credit));
    }
