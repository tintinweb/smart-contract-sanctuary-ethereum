...
contract EmitWinner {
    ...
    mapping(address => uint) hoeTokens;
    mapping (address => mapping (address => uint256)) allowed;

    //Approve Emit Winner to spend hoetoken (Do this on etherscan in contract)
    function approve(address spender, uint256 _amount) public {
     ...
     function drop(uint256 _amount) external {
        bool success = ERC20(erc20).transferFrom(msg.sender, address(this), _amount);
        require(success, "Transaction Failed");
        Bucket(bucketAddress).drop(address(this), _amount);
    }
    ...
    //     hoeTokens[msg.sender] += amount;
    //     //allowed[address(this)][msg.sender] -= amount;
    // }


    // function transferFrom(uint256 _amount) external payable {
    //     erc20.transferFrom(msg.sender, bucketAddress, _amount);
    // }
