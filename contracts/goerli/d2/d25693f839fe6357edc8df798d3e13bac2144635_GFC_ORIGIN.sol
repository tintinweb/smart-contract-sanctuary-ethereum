...
contract GFC_ORIGIN is ERC1155, AccessControl, RrpRequesterV0 {
    ...
    // [ ] figure out which vars to hard set vs set in constructor, scope of vars, etc.
    // [ ] merkle root WL for STEPN/S1/S2/DP (4 potential roots?)
    // [ ] make sure WL via merkle tree will work with marketing partners like Bubblehouse
    // [ ] send hyperlane messages when adminMint is called too!!!! this is bad for now.
    ...
    address public inboxAddress; // to goerli from mumbai: 0x666a24F62f7A97BA33c151776Eb3D9441a059eB8
    address public outboxAddress; //goerli: 0xDDcFEcF17586D08A5740B7D91735fcCE3dfe3eeD
    address public paymasterAddress; //goerli: 0x44b764045BfDC68517e10e783E69B376cef196B2
    ...
    address public DOGE_POUND_ADDRESS = 0xf5de760f2e916647fd766B4AD9E85ff943cE3A2b; //optimism goerli testing addresses (ALL ERC721). Replace with Mainnet Addresses before deployment
    address public S1_CITIZEN_ADDRESS = 0xf5de760f2e916647fd766B4AD9E85ff943cE3A2b;
    address public S2_CITIZEN_ADDRESS = 0xf5de760f2e916647fd766B4AD9E85ff943cE3A2b; 

    bool public mintIsPaused = true;
    bool public whitelistMintIsPaused = true;
    bool public utilityTokenIsDeployed = false;
    ...
    string public name = "Origin Pass";
    string public symbol = "GFCO";

    uint32 public destinationDomain; //mumbai: 80001, polygon: ?
    uint256 public hyperlaneV1GasFee = 100000000000000; //0.0001 eth, adjust as needed
    uint256 public hyperlaneV2GasFee = 100000000000000;
    uint256 public hyperlaneVersion = 1;
    uint256 public apiCallForwardingValue = 0.005 ether;
    uint256 public hyperlaneForwardingValue = 0.005 ether;
    uint256 public totalSupply = 10000;
    uint256 public goldSupply = 500;
    uint256 public goldSpacing = Math.mulDiv(totalSupply, 1, goldSupply); //totalSupply must be divisible by goldSupply
    uint256 public maxMintsPerTxn = 20; //change later, to like 10?
    uint256 public mintPrice = 0.001 ether; //0.001 ether for testing only 0.2 -> 0.3? make adjustable?
    uint256 public whitelistMintPrice = 0.001 ether; //0.001 ether for testing only 0.2 -> 0.3? make adjustable?
    ...
    function isMintPaused() public view returns (string memory) {
        if(mintIsPaused) {
            return "mint is not currently active";
        }
        return "";
    }
    ...
    function requestMint(address _minter, uint256 _mintQuantity) public payable {
        require(!mintIsPaused || hasRole(ADMIN_ROLE, _minter), "mint is not currently active");
        ...
        require(msg.value >= (mintPrice * _mintQuantity) + apiCallForwardingValue + hyperlaneForwardingValue, "not enough ether sent");
        
        //check that this is required beyond msg.value (i.e. both will not result in more funds being requested than intended)
        (bool fwd1, ) = sponsorWallet.call{value: apiCallForwardingValue }(""); 
        (bool fwd2, ) = address(this).call{value: hyperlaneForwardingValue }(""); 
        require(fwd1 && fwd2, "Forward failed");
        
        sendQrngRequest(_minter, _mintQuantity);
    }
    ...
    function requestWhitelistMint(
        ...
        require(!whitelistMintIsPaused,"whitelist mint is not currently active");
        require(isValidMerkle(_proof, _leaf, _minter) == true || hasRole(ADMIN_ROLE, _minter), "invalid merkle proof");
        require( totalSupply > 0, "max supply reached");
        require(_mintQuantity > 0, "can't mint zero");
        require(_mintQuantity <= maxMintsPerTxn, "can't mint more than max");
        require(msg.value >= (whitelistMintPrice * _mintQuantity) + apiCallForwardingValue + hyperlaneForwardingValue, "not enough ether sent");
    ...
    function sendQrngRequest(address _minter, uint256 _mintQuantity) private {
        ...
        onlyAirnodeRrp
    {
        ...
        for (uint256 i = 0; i < mintQuantity; i++) {
            ...
            if(utilityTokenIsDeployed){
                sendOwnerInfo(transferSignalToChild);
    ...
    function getTierFromQrnAndManageSupply(uint256 _QRN) private returns (uint256) {
        uint256 thisRandomId = (_QRN % totalSupply) + 1; //inclusive of 1...
        uint256 thisTier;
        ...
        if(hyperlaneVersion == 1) {
            uint256 messageId = outbox.dispatch(destinationDomain, destinationContractBytes, _message);
                hyperlaneV1Paymaster.payGasFor{
                    value: hyperlaneV1GasFee //may need to be adjusted later when dynamic fees are implemented
                }(
                    outboxAddress,
                    messageId,
                    destinationDomain
            ...
            bytes32 messageId = mailboxV2.dispatch(destinationDomain, destinationContractBytes, _message);
                hyperlaneV2Paymaster.payForGas{
                    value: hyperlaneV2GasFee //may need to be adjusted later when dynamic fees are implemented
                }(
                    messageId,
                    destinationDomain,
                    hyperlaneV2GasFee,
    ...
    // need to add address of sender EOA not the inbox address
    ...
    function handle(
        ...
        require(_origin == destinationDomain && msg.sender == address(inbox), "Only the inbox can call this method"); //check if inbox is the correct address
    ...
    function updatePartnerNFTBalancesOnUtilityToken(address _requester) public {
        require(msg.sender == address(inbox) || msg.sender == address(this) || hasRole(ADMIN_ROLE, msg.sender), "Only the inbox or admin can call this method"); //check if inbox is the correct address
        
        uint256 userS1Balance = IERC721(S1_CITIZEN_ADDRESS).balanceOf(_requester);
        uint256 userS2Balance = IERC721(S2_CITIZEN_ADDRESS).balanceOf(_requester);
        uint256 userDogePoundBalance = IERC721(DOGE_POUND_ADDRESS).balanceOf(_requester);
        
        require(userS1Balance > 0 || userS2Balance > 0 || userDogePoundBalance > 0, "You must own at least one S1, S2, or Doge Pound NFT to register for M2E via Partner Collection");
        
        bytes memory transferSignalToChild;
        transferSignalToChild = abi.encode(block.timestamp, 0, _requester, address(0), userS1Balance, userS2Balance, userDogePoundBalance);
    ...
    function setDeployedSendPendingMessages() public onlyRole(ADMIN_ROLE) {
        require(!utilityTokenIsDeployed, "Utility token is already deployed");
        require(address(this).balance >= pendingMessagesForUtilityToken.length * hyperlaneForwardingValue, "Not enough for forwarding payment for Hyperlane requests");
        utilityTokenIsDeployed = true;
        for(uint256 i=0; i<pendingMessagesForUtilityToken.length; i++) {
            sendOwnerInfo(pendingMessagesForUtilityToken[i].message);
    ...
    function sendMessageManually(bytes memory _message, address _userAddress) public onlyRole(ADMIN_ROLE) {
        sendOwnerInfo(_message);
        sentMessagesForUtilityToken.push(MessageLog(_userAddress, block.timestamp, _message));
    }

    //---------------------------------------------
    // OVERRIDES, SETTERS, MISC
    //---------------------------------------------
    ...
    function setPartnerNftAddresses(address _a1, address _a2, address _a3) external onlyRole(ADMIN_ROLE) {
        S1_CITIZEN_ADDRESS = _a1;
        S2_CITIZEN_ADDRESS = _a2;
        DOGE_POUND_ADDRESS = _a3;
    }

    /// @dev admin control of mint tiers for payment and marketing purposes
    function adminMintTo(uint256[] memory _tierArray, address _recipient) external onlyRole(ADMIN_ROLE) {
    ...
    function decreaseSupply(uint256 _amount) external onlyRole(ADMIN_ROLE) {
        require(totalSupply - _amount >= 0, "Cannot decrease supply below amount already minted");
        totalSupply -= _amount;
    }

    function burnPass(uint256 _id) public {
        require(_id==1 || _id==2, "Invalid pass ID");
        require(balanceOf(msg.sender, _id) > 0, "You do not own this pass");
        _burn(msg.sender, _id, 1);
        totalSupply++;
        if(_id==2) {
            goldSupply++;
            goldIds.push(goldIds[goldIds.length-1]+goldSpacing);
        }
    }
