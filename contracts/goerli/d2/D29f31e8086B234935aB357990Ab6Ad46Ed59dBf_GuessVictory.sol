...
contract GuessVictory is ReentrancyGuard, Ownable {
    ...
    struct TeamStruct {
        mapping(uint256 => uint256) flatAmount; // 队伍胜平负pool
        mapping(address => mapping(uint256 => uint256)) usersFlat; // 用户已抵押金额
        mapping(address => uint256) usersAmount; // 用户参与总额
        mapping(address => uint256) usersRewarded; // 已领取奖励用户
        uint256 totalAmount; // 总竞猜金额
        uint256 totalReward; // 已领取总金额
        address[] users; // 用户地址
        bool turnOn; // 开奖
        uint256 flat; // 1 胜利 2 失败  3 平局
    }
    ...
    address feeAddr; // 手续费钱包地址
    address poolAddr; // 池子钱包地址

    // 当前竞猜的队伍
    uint256 public currentTeam = 1;
    ...
    // 查询队伍的胜平负池子金额
    function getTeamTeamFlatPool(uint256 teamId)
        public
        view
        returns (
            uint256,
    ...
    // 查询队伍总竞猜金额
    function getTeamTeamTotalAmount(uint256 teamId)
        public
        view
        returns (uint256)
    {
    ...
    // 查询队伍总已领取的金额
    function getTeamTeamTotalReward(uint256 teamId)
        public
        view
        returns (uint256)
    {
    ...
    // 查询队伍是否已开奖
    function getTeamTeamTurnOn(uint256 teamId) public view returns (bool) {
        return teamsData[teamId].turnOn;
    }

    // 查询队伍胜平负结果
    function getTeamTeamFlat(uint256 teamId) public view returns (uint256) {
    ...
    // 竞猜队伍
    function deposit(uint256 amount, uint256 flat) public nonReentrant {
        require(!teamsData[currentTeam].turnOn, "deposit off");

        require(currentTeam > 0, "Team id error");
        ...
        require(
            (teamsData[currentTeam].usersAmount[msg.sender] + amount) <=
                amountMax,
            "Amount limit"
        );
        ...
        teamsData[currentTeam].usersFlat[msg.sender][flat] += sa;

        teamsData[currentTeam].flatAmount[flat] += sa;

        teamsData[currentTeam].totalAmount += sa;

        teamsData[currentTeam].usersAmount[msg.sender] += amount;

        addTeamUser(msg.sender, currentTeam);

        emit DepositEvent(msg.sender, currentTeam, sa, flat);
    }
    ...
    // 设置当前竞猜队伍
    function setCurrentTeam(uint256 teamId) public onlyOP nonReentrant {
        require(!teamsData[teamId].turnOn, "team deposit off");
        currentTeam = teamId;
    }
    ...
    // token 0x643C565D11Ec7975018Bc9f81B69E00cA11A59D2
    // feeAddr 0x284CE33481f205A8250a487CEe031B82429413F1
    // poolAddr 0x85CDC44b0fb92090AA0C3Bf0e66D4217d963A8fd

    // constructor(ERC20 _token, address _feeAddr, address _poolAddr) Ownable() {
    //     token = _token;
    //     feeAddr = _feeAddr;
    //     poolAddr = _poolAddr;
    // }
