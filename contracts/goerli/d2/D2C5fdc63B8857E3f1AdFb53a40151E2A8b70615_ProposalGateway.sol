    ...
    UUPSUpgradeable
{
    ...
    function createTGEProposal(
        ...
        bool isPreferenceTokenCreation
    ) external onlyPoolShareholder(pool) returns (uint256 proposalId) {
        IDispatcher(dispatcher).validateTGEInfo(info, address(pool.token()));

        proposalId = pool.proposeSingleAction(
            address(pool.service()),
            0,
            abi.encodeWithSelector(IService.createSecondaryTGE.selector, info, isPreferenceTokenCreation),
            description,
