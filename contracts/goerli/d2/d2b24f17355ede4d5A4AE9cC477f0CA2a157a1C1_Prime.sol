...
contract Prime is Initializable, OwnableUpgradeable, IPrime, NZAGuard {
    ...
    /// @custom:oz-renamed-from penaltyRatePerDay
    ...
    function __Prime_init(
        ...
        require(penaltyRatePerYear_ > 0, "ZVL");
        __Prime_init_unchained(stablecoins, blocksPerDay_, penaltyRatePerYear_);
    }
    ...
    function penaltyRate(
        ...
        return (penaltyRatePerYear * blocksFromSeconds) / blocksPerYear;
    }
    ...
    function isAssetAvailable(
        address asset
    ) external view override nonZA(asset) returns (bool) {
    ...
    function whitelistMember(
        address _member,
        uint256 _riskScore
    ) external nonZA(_member) onlyOwner {
        Member storage member = _members[_member];
    ...
    function blacklistMember(
        address _member
    ) external nonZA(_member) onlyMember(_member) onlyOwner {
        Member storage member = _members[_member];
    ...
    function changeProtocolRateCollector(
        address protocolCollector_
    ) external nonZA(protocolCollector_) onlyOwner {
    ...
    function updatePenaltyRatePerYear(uint256 newValue) external onlyOwner {
        require(newValue > 0, "ZVL");

        uint256 currentValue = penaltyRatePerYear;
        require(currentValue != newValue, "AAD");

        penaltyRatePerYear = newValue;
