...
contract NinfaMarketplace is AccessControl, RoyaltyEngineV1 {
    ...
    /// @notice factory contract for deploying self-sovreign collections. Needed for access control in `isWhitelisted` modifier and as a placeholder for self-sovregn collections in `_secondaryMarketFee` mapping for determining their secondary market fee BPS.
    address private _factory;
    /// @notice Ninfa Marketplace fee percentage for primary sales for orders, expressed in basis points
    uint256 private _primaryOrdersFee;
    /// @notice Ninfa Marketplace fee percentage for primary sales for offers, expressed in basis points
    uint256 private _primaryOffersFee;
    /// @notice in order to be traded on Ninfa marketplace, collections MUST be manually approved by Ninfa's multisig contract
    mapping(address => uint256) private _secondaryMarketFee;
    ...
    modifier isWhitelisted(address _collection) {
        if (_secondaryMarketFee[_collection] == 0) {
    ...
    function _trade(
        ...
        if (royaltyAmounts.length > 0) {
            ...
            if (royaltyAmount == 0) {
                ...
                |*  # IF SECONDARY MARKET                                   *|
                |*----------------------------------------------------------*/
                //  `_secondaryMarketFee[_factory]` is used as a fees BPS placeholder for all Ninfa self sovereign erc1155 collections.
                //  if it can't be found in the _secondaryMarketFee (whitelist) then it must be one of Ninfa's communal collections (721 or 1155).
                //  Ninfa factory clones, defaults to whatever BPS value is stored at corresponding to the clones Factory address, used as placeholder.

                /*----------------------------------------------------------*|
                |*  # CALCULATE MARKETPLACE FEE AMOUNT                      *|
                |*----------------------------------------------------------*/
                /// @dev checks `_secondaryMarketFee[_order.collection]` before checking _secondaryMarketFee[_factory]
                //  this way secondary market fees from factory contracts may be overridden individually as the collection's address takes precedence
                // TODO
                // if()
                marketplaceAmount =
                    (_order.unitPrice *
                        (
                            _secondaryMarketFee[_order.collection] > 0
                                ? _secondaryMarketFee[_order.collection] // NINFA_ERC721_V2 or any external whitelisted collection
                                : _secondaryMarketFee[_factory]
                        )) /
                    10000;
    ...
    /**
     * @param factory_ is used as a placeholder to store secondary fees BPS for all of Ninfa's self-sovreign ERC-721 and ERC-1155 collections
     */
    function setFactory(
        address factory_
     ...
     * @param secondaryMarketFee_ secondary sales fees BPS; if uint is positive the isWhitelisted modifier will return true, the uint corresponds to the secondary fee basis points, the maximum should be < 8501.
     */
    function whitelistCollection(
        address _collection,
        uint256 secondaryMarketFee_
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _secondaryMarketFee[_collection] = secondaryMarketFee_;

        emit CollectionWhitelisted(_collection, secondaryMarketFee_);
    }
...
contract RoyaltyEngineV1 {
        ...
        returns (address payable[] memory recipients, uint256[] memory amounts)
    {
        try
             ...
             */
            this._getRoyalty{gas: 1000000}(tokenAddress, tokenId, value)
        returns (
