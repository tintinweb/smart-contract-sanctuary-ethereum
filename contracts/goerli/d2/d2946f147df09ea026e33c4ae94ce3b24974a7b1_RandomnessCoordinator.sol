...
contract RandomnessCoordinator is Ownable {
    ...
    function addContractToSub(uint subId, address contractAddress) external {
        require(
            msg.sender == ownerOfSub[subId],
            "You are not the owner of subscription"
        );
    ...
    function removeContractFromSub(uint subId, address contractAddress)
        external
    {
        require(
            msg.sender == ownerOfSub[subId],
