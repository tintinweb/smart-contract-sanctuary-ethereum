...
contract JumpRateModelV3 is Initializable, InterestRateModel, AccessControlUpgradeable {
    ...
    function getInterestRateChange(uint cash, uint borrows, uint reserves) public view returns(int interestRateChange) {
        /* E = Cu - Tu
    ...
    function getBorrowRatePerBlock(uint cash, uint borrows, uint reserves, address blendingToken) public view returns(uint interestRatePerBlock) {
        uint lir = lastInterestRate[blendingToken];
        uint dt = getElapsedBlocks(blendingToken);
        int dir = getInterestRateChange(cash, borrows, reserves);
        int ir = dt > 0 ? int(lir/dt) + (dir/2) : int(lir); 
        interestRatePerBlock = ir > 0 ? uint(ir) : 0;
    }
