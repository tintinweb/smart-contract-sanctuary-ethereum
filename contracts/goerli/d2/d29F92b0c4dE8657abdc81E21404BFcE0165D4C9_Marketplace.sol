    ...
    Ownable
{
    ...
    event MarketplaceTransfer(address recipient, uint256 amount);
    ...
    isReady(collectionId, _qty)
    {
        ...
        //Deduct the service fee
        uint256 serviceFee = msg.value.mul(_qty).mul(SERVICE_FEE).div(100);
        uint256 royalty = msg.value.mul(_qty).mul(collection.creatorFee).div(100);
        uint256 finalAmount = msg.value.mul(_qty).sub(serviceFee.add(royalty));

        s_platformEarning += serviceFee;
        s_totalAmount += msg.value.mul(_qty);
    ...
    function _sendFunds(address payable _recipient, uint256 _amount) internal {
        ...
        emit MarketplaceTransfer(_recipient, _amount);
    }
