  ...
  ReentrancyGuard
{
  constructor(
    ...
    Distributor(_token, total, 10000, uri)
  {}
...
abstract contract TrancheVesting is Distributor {
  event SetTranche(uint256 indexed index, uint128 time, uint128 vestedBips);
  ...
  constructor(
    ...
    for (uint i = 0; i < _tranches.length;) {
      ...
      emit SetTranche(i, lastTime, lastVestedBips);
  ...
  function _getTranche(uint i) public view returns (Tranche memory) {
    return tranches[i];
  }

  function _getTranches() public view returns (Tranche[] memory) {
    return tranches;
  }
}
