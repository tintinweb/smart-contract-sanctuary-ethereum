  ...
  ERC721Burnable
{
  ...
  constructor() ERC721('Immutacord', 'IMC') {
  ...
  {
    return 'ipfs://QmabKCMJsRAxdSBtVKHhJW6NQRXXLvCxorPvZTPbKwdeNE/';
  }
  ...
  //   return
  //     keccak256(abi.encodePacked(_hashedMessageId))
  //       .toEthSignedMessageHash()
  //       .recover(_signature) == _signer;
  // }

  function mint(string memory _hashedMessageId) external payable whenNotPaused {
    require(
    ...
    // Verify signature
    // require(verify(_hashedMessageId, _signature, signer), 'Invalid signature');
    ...
    _setTokenURI(tokenId, string(abi.encodePacked(_hashedMessageId, '.json')));
  ...
  function getMessageByHash(string memory _hash)
    external
    view
    returns (uint256)
  {
