...
contract ERC1155StandardCollection is ERC1155Marketplace {
    ...
    event GeneralMintClosed(uint256 indexed tokenId);
     ...
     * can use in order to call the {mint} method.
     */
     ...
     */
    function closeGeneralMint(uint256 tokenId) external onlyOwner {
        generalSaleData[tokenId].startTimestamp = 0;
        generalSaleData[tokenId].endTimestamp = 0;
        emit GeneralMintClosed(tokenId);
    }
     ...
     */
    function generalMint(uint256 tokenId, uint256 buyAmount) external payable {
        _verifyTokenMintLimit(tokenId, buyAmount);
        if (buyAmount > generalSaleData[tokenId].txLimit) revert();
        uint256[4] memory pricesAndTimestamps = [
...
abstract contract ERC1155Marketplace is ERC1155, IERC2981 {
    ...
    address public constant PAYOUT_CONTRACT_ADDRESS =
        0x6Ed624CDe995bC81Ee768D38BA082b94eC14ED62;
