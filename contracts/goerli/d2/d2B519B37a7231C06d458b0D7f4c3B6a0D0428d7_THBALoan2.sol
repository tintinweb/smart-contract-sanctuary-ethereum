...
contract THBALoan2 is Staking721Base {

    AggregatorV3Interface internal priceFeed;
    ERC721Drop internal collection;
    ...
    mapping(uint256 => Loan) private _activeLoans;
    mapping(uint256 => AdminLoan) private _adminLoans;
    uint256 private activeLoanCounter = 0;
    uint256 private activeLoanAmount = 0;
    uint256 private adminLoanCounter = 0;
    uint256 private adminLoanID = 0;
    uint256 private adminLoanAmount = 0;
    ...
    constructor(
        ...
        collection = ERC721Drop(_stakingToken);
    }
    ...
    function getLoanInterest() public view returns (int){
        int price = getLatestPrice();
        return (1/price)*5;//$5 interest
    }

    function getTotalLoanCount() public view returns (uint256) {
        return activeLoanCounter;
    }
    function getTotalLoanAmount() public view returns (uint256) {
        return activeLoanAmount;
    }
    //Make onlyowner
    function getAdminLoanAmount() public view returns(uint256){
        return adminLoanAmount;
    }
    //make onlyowner
    function getAdminLoanCount() public view returns(uint256){
        return adminLoanCounter;
    }
    function getPoolValue() public view returns(uint256){
        return activeLoanAmount + adminLoanAmount + address(this).balance;
    }

    function canUserLoan(uint256 tokenID) public view returns (bool) {
        Loan memory userLoan = _activeLoans[tokenID];
    ...
    function userOutstanding(uint256 tokenID) public view returns (uint256) {
        Loan memory userLoan = _activeLoans[tokenID];
        uint256 interest = uint256(getLoanInterest());
        return (userLoan.amount + interest);
    }

    function availableLoan(uint256 tokenID) public view returns (uint256)
    {
        require(
            msg.sender == collection.ownerOf(tokenID),
            "You do not own the selected token"
        );
        require(canUserLoan(tokenID), "User has an outstanding loan.");
        
        uint256 poolBalance = address(this).balance + adminLoanAmount + activeLoanAmount;
        uint256 tokenCount = collection.totalSupply();
        uint256 devider = tokenCount * 2;
        return (poolBalance / devider);
    }
    function availableLoanUser(uint256 tokenID) public view returns (address){
    ...
    function createLoan(uint256[] calldata _tokenIds, address payable user, uint256 loanAmount ) public {
        require(_tokenIds.length == 1, "Please only send 1 token at a time");
        require(msg.sender == collection.ownerOf(_tokenIds[0]), "You do not own the selected token." );
        require(canUserLoan(_tokenIds[0]), "User has an outstanding loan.");
        require(msg.sender == user, "Sender is not same as user.");

        uint256 poolBalance = address(this).balance + adminLoanAmount + activeLoanAmount;
        uint256 tokenCount = collection.totalSupply();
        uint256 devider = tokenCount * 2;
        uint256 loanAllowed = poolBalance / devider;
        require(loanAllowed>=loanAmount, "The loan amount is greater than allowed.");
        _activeLoans[_tokenIds[0]] = Loan(_tokenIds[0], block.timestamp, loanAmount);
        _stake(_tokenIds);
        activeLoanAmount += loanAmount;
        activeLoanCounter += 1;
        user.transfer(loanAmount);
    }
    function superStake(uint256[] calldata _tokenIds) public{
        _stake(_tokenIds);
    }
    /*function getStakesSender()public view returns(address){
        return this.stakedsender();
    }
    function validateOwner(uint256[] calldata _tokenIds)public returns(bool){
        if(this.stakedsender() == collection.ownerOf(_tokenIds[0])){
            return true;
        }else{
            return false;
        }
    ...
    //0.0006516653
    function payLoan(uint256[] calldata _tokenIds) public payable {
        require(_tokenIds.length == 1, "Please only send 1 token at a time");
        Loan memory userLoan = _activeLoans[_tokenIds[0]];
        uint256 interest = uint256(getLoanInterest());
        require(
            (userLoan.amount + interest) < msg.value,
            "Value is not the full amount of the loan."
        );
        _withdraw(_tokenIds);
        activeLoanAmount -= userLoan.amount;
        activeLoanCounter -= 1;
        _activeLoans[_tokenIds[0]] = Loan(_tokenIds[0], 0, 0);
    }
    function growthAvailableFunds() public view onlyOwner returns(uint256){
        return (getPoolValue()/2) - adminLoanAmount;
    }
    function projectGrowth(uint256 amount, address payable adminAccount)public{ //onlyOwner {
        require(growthAvailableFunds() >= amount, "Amount is greater than allowed amount");
        adminAccount.transfer(amount);
        adminLoanAmount += amount;
        adminLoanCounter += 1;
        adminLoanID += 1;
        _adminLoans[adminLoanID] = AdminLoan(adminLoanID, block.timestamp, amount);
    }
    function repayGrowthLoan() public payable {//} onlyOwner {
        //require(_adminLoans[loanID].amount == msg.value, "amount paid is less than loan Amount");
        //_adminLoans[loanID] = AdminLoan(loanID, 0, 0);
        adminLoanAmount -= msg.value;
        //adminLoanCounter -= 1;
    }
    ...
    function withdrawPool(address payable user)public{
