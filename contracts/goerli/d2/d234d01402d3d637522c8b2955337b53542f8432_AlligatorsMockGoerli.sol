...
contract Alligators is
    ...
    mapping(uint256 => uint8) internal NFTLvl;
   ...
   struct NFT_Anatomy {
    ...
    uint256 internal MIN_CHANCE_VALUE_PRIZE = 922;
    ...
    function verify(address whitelisted, uint8 v, bytes32 r, bytes32 s) public virtual {
        ...
        require(isWhitelisted[recoveredAddress] == 1, "Invalid signer" );
   ...
   /**
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        if (saleStatus == SaleStatus.PUBLIC || saleStatus == SaleStatus.PRESALE) {
            ...
            if (moddedRng < chanceArracy[0]) {
                ...
                alligatorMint(publicMinter, quantity, _requestId);

            } else {
                vrf_requests[_requestId].jokerMint = 1;
                jokerMint(publicMinter, _requestId);
                if (vrf_requests[_requestId].quantity > 1) {
                    alligatorMint(publicMinter, vrf_requests[_requestId].quantity - 1, _requestId);
                }
            }

            if (moddedRngPrize > chanceArrayPrize[0]) {
                vrf_requests[_requestId].jokerMintPrize = 1;
                alligatorMint(publicMinter, 1, _requestId);
            }
    ...
    function alligatorMint(address to, uint256 _quantity, uint256 _requestId) internal {

        if (AlligatorIds.current() > COLLECTION_SIZE) revert callErr();
        
        uint256[] memory _randomWords = vrf_requests[_requestId].randomWords;

        for (uint index = 1; index < _quantity; index++) {
    ...
    function jokerMint(address to, uint256 _requestId) internal {
        ...
        NFTLvl[_currentIndex] = uint8(_randomWords[9] % 4);
    ...
    function merge(uint256 _1st, uint256 _2nd, uint256 _3rd, address _owner) external onlyMerger {
        ...
        if (uint8(_lvl[_currentIndex].trait1Lvl) == uint8(_lvl[_currentIndex].trait2Lvl) &&
            uint8(_lvl[_currentIndex].trait1Lvl) == uint8(_lvl[_currentIndex].trait3Lvl) &&
            uint8(_lvl[_currentIndex].trait1Lvl) == uint8(_lvl[_currentIndex].trait4Lvl) &&
            uint8(_lvl[_currentIndex].trait1Lvl) == uint8(_lvl[_currentIndex].trait5Lvl) &&
            uint8(_lvl[_currentIndex].trait1Lvl) == uint8(_lvl[_currentIndex].trait6Lvl) &&
            uint8(_lvl[_currentIndex].trait1Lvl) == uint8(_lvl[_currentIndex].trait7Lvl)
        ) {

                if (NFTLvl[_currentIndex] >= 5) revert Mergefailed();
                NFTLvl[_currentIndex]++;
            }
