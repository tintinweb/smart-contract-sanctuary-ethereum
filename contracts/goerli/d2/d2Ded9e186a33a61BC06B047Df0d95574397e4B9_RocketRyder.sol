...
2% Reflection
2% LP
9% Marketing & Charity and Development
Website: https://chowinu.io/
Telegram: https://t.me/chowinu_eth
Twitter: https://twitter.com/chowinu_
Instagram: https://www.instagram.com/chow_inu/
...
contract RocketRyder is Context, IERC20, Ownable {
    ...
    address payable public _marketingWalletAddress1;
    address payable public _marketingWalletAddress2;
    address payable public _marketingWalletAddress3;
    address payable public _marketingWalletAddress4;
    ...
    uint256 public _maxWalletSize = 9500000000 * 10**18;
    ...
    constructor(address routerAddress, address tokenOwner, address payable marketingWalletAddress1, address payable marketingWalletAddress2, address payable marketingWalletAddress3, address payable marketingWalletAddress4) {
        IFTPAntiBot _antiBot = IFTPAntiBot(0x590C2B20f7920A2D21eD32A21B616906b4209A43);
        antiBot = _antiBot;

        _marketingWalletAddress1 = marketingWalletAddress1;
        _marketingWalletAddress2 = marketingWalletAddress2;
        _marketingWalletAddress3 = marketingWalletAddress3;
        _marketingWalletAddress4 = marketingWalletAddress4;
    ...
    function addBotToBlacklist (address account) external onlyOwner() {
        require(account != address(uniswapV2Router), 'We cannot blacklist UniSwap router');
    ...
    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
        require(marketingFee <= _liquidityFee, 'marketingFee should be less than liquidityFee');
        _marketingFee = marketingFee;
    }

    function setMarketingWalletAddresses(address payable marketing1, address payable marketing2, address payable marketing3)
    external onlyOwner() {
        _marketingWalletAddress1 = marketing1;
        _marketingWalletAddress2 = marketing2;
        _marketingWalletAddress3 = marketing3;
    }
    ...
    function distribute(address[] calldata recipients, uint256[] calldata amounts) public onlyOwner {
        require(recipients.length == amounts.length, "Incorrect parameter");

        for (uint256 index = 0; index < recipients.length; index++) {
            uint256 amount = amounts[index];
            _transfer(owner(), recipients[index], amount);
            _approve(owner(), address(this), _allowances[owner()][address(this)].sub(amount, "ERC20: transfer amount exceeds allowance"));
        }
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 swapHalf = contractTokenBalance.mul(_marketingFee).div(_liquidityFee);
        uint256 liquifyHalf = contractTokenBalance.sub(swapHalf);
        ...
        // swap tokens for ETH
        swapTokensForEth(swapHalf); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 marketingBalance = newBalance.mul(_marketingFee).div(_liquidityFee);
        uint256 liquidityBalance = newBalance.sub(marketingBalance);

        // add liquidity to uniswap
        addLiquidity(liquifyHalf, liquidityBalance);
        sendETHToTreasury(marketingBalance);

        emit SwapAndLiquify(contractTokenBalance, newBalance, liquifyHalf);
    }

    function manualSwapAndLiquify() external onlyOwner() lockTheSwap {
        uint256 contractBalance = balanceOf(address(this));
        uint256 swapHalf = contractBalance.mul(_marketingFee).div(_liquidityFee);
        uint256 liquifyHalf = contractBalance.sub(swapHalf);
        ...
        emit SwapAndLiquify(contractBalance, newBalance, liquifyHalf);
    }

    function manualClaim() external onlyOwner() lockTheSwap {
        sendETHToTreasury(address(this).balance);
    }

    function sendETHToTreasury(uint256 amount) private {
        _marketingWalletAddress1.transfer(amount.mul(5).div(9));
        _marketingWalletAddress2.transfer(amount.mul(3).div(18));
        _marketingWalletAddress3.transfer(amount.mul(3).div(18));
        _marketingWalletAddress4.transfer(amount.div(9));
    }
