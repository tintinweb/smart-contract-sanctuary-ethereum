...
contract Bridge is IBridge, IBridgeReceiver, Ownable, ReentrancyGuard, ERC165 {
    ...
    /// @dev specifying a feeRecipient other than this contract's address will forward fee (if fee is supplied with the call)
    function sendMessage(address destination, bytes calldata message, address payable feeRecipient)
        external
        payable
        override
    {
        ...
        if (msg.value > 0) { // message has fee
            if (feeRecipient != address(this)) { // forward fee to specified recipient
                feeRecipient.transfer(msg.value);
            } else {
                // bridge contract is fee recipient
                accumulatedMessageFees += msg.value;
            }
        }
        emit SendMessage(sentEventId++, msg.sender, destination, message, msg.value, feeRecipient);
    }
