...
contract LuxFractionalizationToken is ERC20, ILuxFractionalizationToken, FractionalizationAccessControl {
    address public controller;
    
    constructor(address[2] memory _admins) ERC20("LuxVesting", "LVX") FractionalizationAccessControl(_admins) {}

    modifier onlyController() {
        require(msg.sender == controller, "not verified to call func");
        _;
    }

    function setController(address _controller) external override onlyRole(ADMIN_ROLE) {
        controller = _controller;
        emit ChangeController(msg.sender, _controller, block.timestamp);
    }

    function mintTokens(uint256 _amount) external override onlyController() {
        _mint(controller, _amount);
    }

    function transfer(address to, uint256 amount) public virtual override(ERC20, ILuxFractionalizationToken) returns (bool) {
    ...
    function transferFrom(
        ...
        address to,
        uint256 amount
    ) public virtual override(ERC20, ILuxFractionalizationToken) returns (bool) {
