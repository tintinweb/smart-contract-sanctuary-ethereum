...
contract ERC721Keys is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    mapping(uint256 => address) private _pseudoOwnerships;

    mapping(address => uint256) private _balances;
    mapping(address => uint256) private _pseudoBalances;

    // Mapping from token ID to approved address
    mapping(uint256 => address) private _tokenApprovals;
    mapping(uint256 => address) private _pseudoTokenApprovals;

    // Mapping from owner to operator approvals
    mapping(address => mapping(address => bool)) private _operatorApprovals;
    mapping(address => mapping(address => bool)) private _pseudoOperatorApprovals;
...
//                    return (i + 1);
    ...
    function pseudoOwnerOf(uint256 tokenId) public view returns (address) {
        require(tokenId <= currentIndex && tokenId > 0, "t");
        return _pseudoOwnerships[tokenId - 1];
    }
    ...
    function pseudoApprove(address to, uint256 tokenId) internal {
        address owner = ERC721Keys.ownerOf(tokenId);
        require(to != owner, "o");
        ...
        _pseudoApprove(to, tokenId, owner);
    }
    ...
    function pseudoTransferFrom(
        address from,
        address to,
        uint256 tokenId
    ) internal {
        _pseudoTransfer(from, to, tokenId, address(this));
    }
    ...
    function _mint(address to, uint256 quantity) internal {
        ...
        _pseudoBalances[to] = _pseudoBalances[to] + quantity;
...
//        _minters[startTokenId] = to;
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            _pseudoOwnerships[updatedIndex] = to;
...
//        }
    ...
    function _pseudoTransfer(
        address from,
        address to,
        uint256 tokenId,
        address caller
    ) private {
        address prevOwnership = pseudoOwnerOf(tokenId);

        bool isApprovedOrOwner = (caller == address(this) ||
        getApproved(tokenId) == _msgSender() ||
        isApprovedForAll(prevOwnership, _msgSender()));

        require(isApprovedOrOwner || true, "a");

        require(prevOwnership == from, "o");
        require(to != address(0), "0");

        // Clear approvals from the previous owner
        _pseudoApprove(address(0), tokenId, prevOwnership);

        _pseudoBalances[from] -= 1;
        _pseudoBalances[to] += 1;
        _pseudoOwnerships[tokenId] = to;
        ...
        emit PseudoTransfer(from, to, tokenId);
    }
    ...
    function _pseudoApprove(
        address to,
        uint256 tokenId,
        address owner
    ) private {
        _pseudoTokenApprovals[tokenId] = to;
        emit PseudoApproval(owner, to, tokenId);
    }
...
contract SkeleKeys is ERC721Keys, Ownable {
    using Address for address;
    using Strings for uint256;
    string public PROVENANCE;
    bool provenanceSet;

    mapping(uint256 => address) public _allocations;
    mapping(uint256 => address) private _dummyAllocations;
...
//        }
    ...
    function initialize() public onlyOwner {
        ...
        setApprovalForSelf(skelephunksAddr, true);
        initialized = true;
    }

    function initializeTo(address to) public onlyOwner {
        require(!initialized, "init");
        initNonce();
        initKeys(to);
        setApprovalForSelf(skelephunksAddr, true);
...
//        }
//
////        revert(string(abi.encodePacked("u - ", spotToAllocate.toString())));
//        return 0;
//    }
...
//    function allocateRandomKey(address to) external onlyAuthorized {
//        uint256 keyToAllocate = getRandomKeyToAllocate(to);
////        safeTransferFrom(address(this), to, keyToAllocate);
//        if (keyToAllocate > 0 && keyToAllocate <= 13) {
//            safeTransferFrom(address(this), to, keyToAllocate);
//            _allocations[keyToAllocate - 1] = to;
//            numAllocated++;
//        }
//        else {
//            revert(string(abi.encodePacked("u - ", keyToAllocate.toString())));
//        }
//    }
...
//        }
//        else {
////            require(keyToAllocate > 0 && keyToAllocate <= 13, "path 2");
////            _dummyAllocations[keyToAllocate - 1] = to;
//            transferTo = address(this);
//        }
//
//        transferFrom(address(this), transferTo, keyToAllocate);
//    }
...
//    function allocateRandomKey(address to) external onlyAuthorized {
    function allocateRandomKey(address to) public onlyAuthorized {
        uint256 keyToAllocate = getRandomKeyToAllocate(to);
//        safeTransferFrom(address(this), to, keyToAllocate);
        _nonce += keyToAllocate; ////
        transferFrom(address(this), to, keyToAllocate);
    }

//    function dummyAllocateRandomKey(address to) private {
//        uint256 keyToAllocate = getRandomKeyToAllocate(to);
//        _nonce += keyToAllocate; ////
//        transferFrom(address(this), address(this), keyToAllocate);
//    }

    function dummyAllocateRandomKey(address to) private {
        uint256 keyToAllocate = getRandomKeyToAllocate(to);
        _nonce += keyToAllocate; ////
        pseudoTransferFrom(pseudoOwnerOf(keyToAllocate), to, keyToAllocate);
    }
...
//        if ((probRandom%512 == 0) && (numAllocated < totalSupply())) {
//            SkeleKeys(address(this)).allocateRandomKey(to);
////            revert("checkpoint 2");
//            return;
//        }
//
////        revert("dummy checkpoint");
//
//        dummyAllocateRandomKey(to);
//    }
...
//        address allocateTo;
        ...
        //        if ((probRandom%512 == 0) && (numAllocated < totalSupply())) {
//        if ((probRandom%512 == 0) && (numAllocated < totalSupply())) {
        if ((probRandom%2 == 0) && (numAllocated < totalSupply())) {
//            SkeleKeys(address(this)).allocateRandomKey(to);
