...
contract BST is ERC20Upgradeable, BACUpgradeable, IBST {
    using SafeERC20 for IERC20;

    bytes32 public constant ADAPTER_ROLE = keccak256("ADAPTER_ROLE");

    modifier onlyAdapter() {
        require(hasRole(ADAPTER_ROLE, msg.sender), "BST: adapter-not-permitted");
        _;
    }
    ...
    function withdraw(address stable, uint amountInBST, address burnFrom, address to) external override onlyAdapter {
        _burn(burnFrom, amountInBST);        
        IERC20(stable).safeTransfer(to, amountInBST * (10**IERC20Metadata(stable).decimals()) / (10**18) );
    }
}
