...
contract Zorggles is ERC721A, Ownable {
    ...
    */
    ...
    address public allowListContractAddress;
    ...
    */
    ...
    function setZorggleType(uint32 tokenId, uint16 zorggleTypeIndex) public {
        address ownerAddress = ownerOf(tokenId);
        require(msg.sender == ownerAddress, "Rejected: not token owner");
        require(
            zorggleTypeIndex <= descriptor.getZorggleTypesCount(),
            "Rejected: must be a valid zorggle type index"
        );
        nogSeeds[tokenId].zorggleType = zorggleTypeIndex;

        emit ZorggleTypeUpdated(tokenId, zorggleTypeIndex, ownerAddress);
    }
