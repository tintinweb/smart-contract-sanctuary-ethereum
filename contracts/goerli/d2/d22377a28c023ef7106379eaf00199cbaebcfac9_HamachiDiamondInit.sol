...
import {ERC165Storage} from "./libraries/LibERC165.sol";
...
contract HamachiDiamondInit is IDiamondInit {
  ...
  function init() external {
    ...
    LibERC165.addSupportedInterface(type(IAccessControl).interfaceId);
    LibAccessControl.grantRole(DEFAULT_ADMIN_ROLE, msg.sender);

    // configure excluded from fee role
    LibAccessControl.grantRole(EXCLUDED_FROM_FEE_ROLE, msg.sender);
    LibAccessControl.grantRole(EXCLUDED_FROM_FEE_ROLE, address(this));
    LibAccessControl.grantRole(EXCLUDED_FROM_FEE_ROLE, LibHamachi.DS().liquidityWallet); // protocol added liquidity

    // configure excluded from antiwhale role
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, msg.sender);
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, address(this));
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, address(0));
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, BURN_ADDRESS);
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, LibHamachi.DS().liquidityWallet);

    LibAccessControl.grantRole(EXCLUDED_FROM_REWARD_ROLE, msg.sender);
    LibAccessControl.grantRole(EXCLUDED_FROM_REWARD_ROLE, address(this));
    LibAccessControl.grantRole(EXCLUDED_FROM_REWARD_ROLE, address(0));
    LibAccessControl.grantRole(EXCLUDED_FROM_REWARD_ROLE, BURN_ADDRESS);
    LibAccessControl.grantRole(
      EXCLUDED_FROM_REWARD_ROLE,
      0x926C609Cb956b1463d964855D63a89cBBE3aC8c0
    ); // locked liquidity

    // configure ERC20
    LibHamachi.DS().name = "Hamachi";
    LibHamachi.DS().symbol = "HAM";

    // configure hamachi facet
    LibAccessControl.grantRole(EXCLUDED_FROM_MAX_WALLET_ROLE, LibHamachi.DS().defaultRouter);
  }
}
