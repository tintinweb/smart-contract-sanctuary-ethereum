...
interface OpenerMintInterface is IERC721 {
    function getMarketplaceDistributionForERC721(uint256 tokenId) external view returns(uint256[] memory, address[] memory);
}
...
contract Bundles is ERC721, Ownable, BundlesInterface {
    ...
    mapping(uint256 => Bundle) private lpidToBundle;
    ...
    function getBundleDistribution(uint256 tokenId) external view override returns(uint256[][] memory, address[][] memory, uint256) {
        return (lpidToBundle[tokenId].fees, lpidToBundle[tokenId].addresses, lpidToBundle[tokenId].numberOfNFTs);
    }
    

    event Deposit(uint256 indexed lpid, address[] indexed nftAddress, uint256[] indexed ids);
    event Withddraw(uint256 indexed lpid, address[] indexed nftAddress, uint256[] indexed ids);
    ...
    function depositNFTs(address[] memory nftAddresses,  uint256[] memory ids, string memory bundleName) external {
        ...
        require(ids.length <= 50 && ids.length == nftAddresses.length, "Up to 50 nfts allowed");
        {
            uint256[][] memory test1;
            address[][] memory test2;
            lpidToBundle[LPidCounter] = Bundle(LPidCounter, bundleName, ids.length, nftAddresses, ids, test1, test2);
        }
        uint256[] memory tempFees;
        address[] memory tempAddys;
        ...
        for(uint256 i = 0 ; i < ids.length; i++) {
            ...
            (tempFees, tempAddys) = OpenerMintInterface(nftAddresses[i]).getMarketplaceDistributionForERC721(ids[i]);
            lpidToBundle[LPidCounter].fees[i] = tempFees;
            lpidToBundle[LPidCounter].addresses[i] = tempAddys;
        }

        _mint(msg.sender, LPidCounter);

        emit Deposit(LPidCounter++, nftAddresses, ids);
    }
    ...
    function withdrawNFTs(uint256 lpid) external {
        require(!paused, "Bundles contract is currently paused");
        require(IERC721(address(this)).ownerOf(lpid) == msg.sender, "Now owner");
        IERC721(address(this)).transferFrom(msg.sender, dead, lpid);
        for(uint256 i = 0 ; i < lpidToBundle[lpid].ids.length; i++) {
            IERC721(lpidToBundle[lpid].nftAddresses[i]).transferFrom(address(this), msg.sender, lpidToBundle[lpid].ids[i]);
        }

        emit Withddraw(lpid, lpidToBundle[lpid].nftAddresses, lpidToBundle[lpid].ids);
    }

    function getBundle(uint256 bundleId) external view returns(uint256, string memory, address[] memory, uint256[] memory) {
        return (lpidToBundle[bundleId].lpid, lpidToBundle[bundleId].name, lpidToBundle[bundleId].nftAddresses, lpidToBundle[bundleId].ids);
    }
