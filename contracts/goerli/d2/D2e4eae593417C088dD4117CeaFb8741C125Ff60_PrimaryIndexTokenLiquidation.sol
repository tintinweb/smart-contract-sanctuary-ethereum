...
contract PrimaryIndexTokenLiquidation is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function liquidateFromModerator(address _account, address _projectToken, address _lendingToken, uint256 _lendingTokenAmount, address liquidator) public isProjectTokenListed(_projectToken) isLendingTokenListed(_lendingToken) onlyRelatedContracts() returns(uint256) {
        return liquidateInternal(_account, _projectToken, _lendingToken, _lendingTokenAmount, liquidator);
         
    }

    function liquidateInternal(address _account, address _projectToken, address _lendingToken, uint256 _lendingTokenAmount, address liquidator) internal isProjectTokenListed(_projectToken) isLendingTokenListed(_lendingToken) returns(uint256) {
        ...
        uint256 projectTokenLiquidatorReceived = distributeReward(_account, _projectToken, projectTokenToSendToLiquidator, liquidator);
    ...
    function distributeReward(address _account, address _projectToken, uint256 projectTokenToSendToLiquidator, address liquidator) internal returns(uint256){
