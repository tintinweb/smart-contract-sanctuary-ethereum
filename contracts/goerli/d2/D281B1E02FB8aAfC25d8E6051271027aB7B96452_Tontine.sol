...
contract Tontine is ERC721URIStorage, Ownable {
    ...
    function mint() external payable {
        ...
        subMint(msg.sender);
        totalMintAmount += msg.value; // track mint funds
    }
    ...
    function splitBalance() external {
        ...
        require(balance > 1000, "Insufficient balance for payout");
            ...
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        ...
                        uintToString(id),
                        "</text>",
                        '<text x="50%" y="95%" text-anchor="middle" fill="white" font-size="1.25px" font-family="Courier New">Sacrificed NFTs:',
                        uintToString(totalBalance),
                        "</text>",
                        "</svg>"
                    )
                )
    ...
    function deadManSwitch(uint256[] memory tokens) external onlyOwner {
    ...
    function ownerClaim() external onlyOwner {
        require(ownerClaimed < 2, "Cannot claim twice, mfer.");

        uint256 balance = address(this).balance;
        uint256 claimable = (totalMintAmount * SPLIT_PERCENT) / 100;
        require(balance > claimable, "No funds available to claim");

        // Transfer the claimable amount to the owner
        (bool success, ) = payable(msg.sender).call{value: claimable}("");
        require(success, "Transfer failed.");

        totalMintAmount -= claimable; // reduce the amount
