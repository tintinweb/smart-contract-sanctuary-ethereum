...
contract StakingOfDragons is ERC721Holder {
    ...
    function getStakedNFTs () public view returns (uint256[] memory) {
        uint256[] memory ownedNFTs = denOfDragons.walletOfOwner(msg.sender);
        uint256[] memory stakedNFTs = new uint256[](stakedNFTCount[msg.sender]);
        uint256 matches = 0;
        uint256 counter = 0;
        while (matches < stakedNFTCount[msg.sender]) {
            uint256 id = ownedNFTs[counter];
            if (isStaked[id] == true) {
                stakedNFTs[matches] = id;
                matches++;
            }
            counter++;
        }
    ...
    function viewTrueTokenBalance () public view returns (uint256) {
        uint256 tokenBal = rewardOwned();
        uint256[] memory stakedNFTs = getStakedNFTs();
        uint combinedSecsStaked = 0;
        uint256 counter = 0;
        while (counter > stakedNFTs.length) {
            combinedSecsStaked += stakedNFTs[counter];
            counter++;
        }
        uint combinedReward = (combinedSecsStaked * (10 ** 18)) / 86400;
    ...
    function rewardOwned () public view returns (uint256) {
        uint256 tokens = eggToken.balanceOf(msg.sender);
        return(tokens);
    }
