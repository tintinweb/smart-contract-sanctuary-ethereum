 ...
 *  @author @itsalexey
 *  @notice Decentralized NFT and ETH Lottery
...
contract Lottery is Ownable, VRFConsumerBaseV2, AutomationCompatibleInterface, ERC721Holder {
    ...
    function enterRaffle(uint256 entries) public payable {
        ...
        s_treasuryBalance += ((msg.value * (s_feePercent * 1e2)) / 1e4);
        s_raffleBalance += ((msg.value * (1e4 - (s_feePercent * 1e2))) / 1e4);
        for (uint256 i = 0; i < entries; i++) {
            s_players.push(msg.sender);
            points[msg.sender] += (msg.value / 1e15) * s_rate;
            weight[msg.sender] += 1;
    ...
    function fulfillRandomWords(
        ...
        if (LoadedNFTs.length > 0) {
            ...
            if (
                ...
                emit Winnings(tokenId, address(nftRaffled), 0);
    ...
    function withdrawETHForNFTs() external onlyOwner {
