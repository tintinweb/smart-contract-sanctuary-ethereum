...
contract dAMM is IStargateReceiver, ILayerZeroReceiver, Message, ERC20 {
    ...
    function addStargateTrustedBridge(
        uint16 chainId,
        address remote,
        address local
    ) external {
    ...
    function sgReceive(
        ...
        require(
            msg.sender == IdAMMFactory(factory).stargateRouter(),
            "NOT STARGATE"
        );
        require(
        ...
        (uint256 earmarkedAmount, uint256 exactBridgedAmount) = abi.decode(
            _payload,
            (uint256, uint256)
        );
        ...
        if (partialSync.token == address(0)) {
            partialSyncs[_srcChainId] = PartialSync(
                _token,
                exactBridgedAmount,
                earmarkedAmount
            );
    ...
    function lzReceive(
        ...
        if (messageType == MessageType.BurnVoucher) {
            ...
            (, address token, address user, uint256 amount) = abi.decode(
                _payload,
                (MessageType, address, address, uint256)
            );
    ...
    function syncL2(uint16 _destChainId, address _amm) external payable {
        ...
        ILayerZeroEndpoint endpoint = ILayerZeroEndpoint(
            IdAMMFactory(factory).lzEndpoint()
        );
    ...
    function _syncFromL2(
        ...
        uint256 newreserve0 = reserve0 +
            bridged0 -
            (earmarked0 - marked0[source]);
        uint256 newreserve1 = reserve1 +
            bridged1 -
            (earmarked1 - marked1[source]);
