...
// File: ../libraries/LibAssetClasses.sol
...
// File: ../libraries/LibAssetTypes.sol
...
// File: ../libraries/LibOrderTypes.sol
...
// File: ../libraries/LibFillTypes.sol
...
// File: ../libraries/LibPartTypes.sol
...
// File: ../libraries/LibFeeSideTypes.sol
 
// Copyright 2022 Energi Core
library LibFeeSideTypes {
    enum FeeSide {NONE, MAKE, TAKE}
}
// File: ../libraries/LibOrderDataV1Types.sol
...
// File: ../interfaces/IWrappedCoin.sol
...
interface IExchange {
    ...
    function getRoyaltiesRegistry() external view returns (address);
    function getFeeReceiver(address _token) external view returns (address);
    function getOrderFill(bytes32 orderKeyHash) external returns (uint);
    function getOrdersFills(bytes32[] calldata ordersKeyHashes) external view returns (uint[] memory);
    function isERC20AssetAllowed(address _erc20AssetAddress) external view returns (bool);
}
// File: helper/IExchangeHelper.sol
...
interface IExchangeStorage {
    ...
    function getFill(bytes32 _orderKeyHash) external view returns (uint);
    function isERC20AssetAllowed(address _erc20AssetAddress) external view returns (bool);
    function getProtocolFee() external view  returns (uint);
    ...
    function setRoyaltiesRegistry(address _royaltiesRegistry) external;
    ...
    function setERC20AssetAllowed(address _erc20AssetAddress, bool _isAllowed) external;
    function setProtocolFee(uint _newProtocolFee) external;
}
// File: ../interfaces/IExchangeGovernedProxy.sol
...
contract ExchangeStorage is StorageBase, IExchangeStorage {
    ...
    address private royaltiesRegistry; // Registers details of royalties to be paid when an NFT asset is sold
    ...
    mapping(bytes32 => uint) private fills; // Orders fills (basis points: 10000 <=> 100%) indexed by orders hashes
    ...
    uint private protocolFee; // Protocol fee (basis points: 10000 <=> 100%) to be paid by seller of ERC721/ERC1155 asset
    constructor(
    ...
    // Getter functions
    //
    function getHelperProxy() external override view returns (address) {
        return helperProxy;
    }
    function getWhitelistProxy() external override view returns (address) {
        return whitelistProxy;
    }
    function getOrderBook() external override view returns (address) {
        return orderBook;
    }
    function getDefaultFeeReceiver() external override view  returns (address) {
        return defaultFeeReceiver;
    }
    function getRoyaltiesRegistry() external override view returns (address) {
        return royaltiesRegistry;
    }
    function getFeeReceiver(address _token) external override view  returns (address) {
    ...
    function getFill(bytes32 _orderKeyHash) external override view returns (uint) {
        return fills[_orderKeyHash];
    }
    function isERC20AssetAllowed(address _erc20AssetAddress) external override view returns (bool) {
        return allowedERC20Assets[_erc20AssetAddress];
    }
    function getProtocolFee() external override view  returns (uint) {
        return protocolFee;
    }
    ...
    function setRoyaltiesRegistry(address _royaltiesRegistry) public override requireOwner {
        royaltiesRegistry = _royaltiesRegistry;
    }
    ...
    function setERC20AssetAllowed(address _erc20AssetAddress, bool _isAllowed) external override requireOwner {
        allowedERC20Assets[_erc20AssetAddress] = _isAllowed;
    }
    function setProtocolFee(uint _newProtocolFee) public override requireOwner {
...
contract Exchange is Pausable, NonReentrant, ExchangeAutoProxy, IExchange {
    ...
    function getExchangeHelperImpl() private view returns(IExchangeHelper _helper) {
        _helper = IExchangeHelper(address(IGovernedProxy(address(uint160(_storage.getHelperProxy()))).impl()));
    }
    ...
    function getRoyaltiesRegistryImpl() private view returns(address _royaltiesRegistry) {
        _royaltiesRegistry = address(IGovernedProxy(address(uint160(_storage.getRoyaltiesRegistry()))).impl());
    }
    // Exchange functions
    //
    // Cancel an order by setting its fill to Max(uint256) so that it can't ever be matched
    function cancelOrder(LibOrderTypes.Order memory order) public override requireWhitelisted whenNotPaused {
    ...
    function matchOrders(
        ...
        uint matchLeftBeforeBlock, // Block after which matching taker order is not allowed by order-book
        ...
        uint matchRightBeforeBlock, // Block after which matching maker order is not allowed by order-book
        bytes memory orderBookSignatureRight // Order-book signature for maker order matchAllowance
    ) external override payable requireWhitelisted whenNotPaused {
        ...
        (bytes32 leftOrderKeyHash, bytes32 rightOrderKeyHash) = getExchangeHelperImpl().validateMatch(
            orderLeft,
            orderRight,
    ...
    function matchAndTransfer(
        ...
        LibFillTypes.FillResult memory newFill = calculateFills(orderLeft, orderRight, leftOrderKeyHash, rightOrderKeyHash);
        // Process ETH and WETH conversions and transfers to proxy, and calculate totalETHAmountWithFees
        uint totalETHAmountWithFees;
        (totalETHAmountWithFees, makerAssetType) = processEthAndWeth(makerAssetType, takerAssetType, orderLeft, orderRight, newFill);
        // Transfer assets
        (   uint totalMakeValue, // Total value sent by maker to taker
            uint totalTakeValue // Total value sent by taker to maker
        ) = doTransfers(
            takerAssetType,
            makerAssetType,
            newFill,
    ...
    function checkERC20TokensAllowed(
        ...
        if (orderRight.makeAsset.assetType.assetClass == LibAssetClasses.ERC20_ASSET_CLASS) {
            require(_storage.isERC20AssetAllowed(abi.decode(orderRight.makeAsset.assetType.data, (address))),
        ...
        if (orderRight.takeAsset.assetType.assetClass == LibAssetClasses.ERC20_ASSET_CLASS) {
            require(_storage.isERC20AssetAllowed(abi.decode(orderRight.takeAsset.assetType.data, (address))),
        ...
        if (orderLeft.makeAsset.assetType.assetClass == LibAssetClasses.ERC20_ASSET_CLASS) {
            require(_storage.isERC20AssetAllowed(abi.decode(orderLeft.makeAsset.assetType.data, (address))),
        ...
        if (orderLeft.takeAsset.assetType.assetClass == LibAssetClasses.ERC20_ASSET_CLASS) {
            require(_storage.isERC20AssetAllowed(abi.decode(orderLeft.takeAsset.assetType.data, (address))),
    ...
    function checkCounterparties(
        ...
        if (orderLeft.taker != address(0)) {
            require(orderRight.maker == orderLeft.taker, 'Exchange: leftOrder.taker verification failed');
        }
        if (orderRight.taker != address(0)) {
            require(orderRight.taker == orderLeft.maker, 'Exchange: rightOrder.taker verification failed');
        }
    }
    function processEthAndWeth(
    ...
    ) internal returns (uint totalETHAmountWithFees, LibAssetTypes.AssetType memory _makerAssetType) {
        ...
        if (msg.value > 0) {
            ...
            if (makerAssetType.assetClass == LibAssetClasses.ETH_ASSET_CLASS) {
                ...
                (bool success, bytes memory data) = proxy.call{ value: msg.value }(abi.encodeWithSelector(0x3ecfd51e));
                require(
            ...
            LibOrderDataV1Types.DataV1 memory leftOrderData = getExchangeHelperImpl().parse(orderLeft);
            ...
            LibOrderDataV1Types.DataV1 memory rightOrderData = getExchangeHelperImpl().parse(orderRight);
            totalETHAmountWithFees = getExchangeHelperImpl().calculateTotalAmount(
                newFill.leftOrderTakeValue,
    ...
    function calculateFills(
        ...
        uint leftOrderTakeAssetFill = getOrderFill(orderLeft, leftOrderKeyHash);
        uint rightOrderTakeAssetFill = getOrderFill(orderRight, rightOrderKeyHash);
    ...
    function getOrderFill(LibOrderTypes.Order memory order, bytes32 hash) internal view returns (uint fill) {
        if (order.salt == 0) {
    ...
    function doTransfers(
    ...
    )  internal returns (uint _totalMakeValue, uint _totalTakeValue) {
        // Determine fee side (and fee asset)
        LibFeeSideTypes.FeeSide feeSide = getExchangeHelperImpl().getFeeSide(_makerAssetType.assetClass, _takerAssetType.assetClass);
        ...
        LibOrderDataV1Types.DataV1 memory rightOrderData = getExchangeHelperImpl().parse(_rightOrder);
        if (feeSide == LibFeeSideTypes.FeeSide.MAKE) {
            ...
            transferPayouts(_makerAssetType, _fill.leftOrderTakeValue, _leftOrder.maker, rightOrderData.payouts, TO_MAKER);
            transferPayouts(_takerAssetType, _fill.rightOrderTakeValue, _rightOrder.maker, leftOrderData.payouts, TO_TAKER);
        }
    }
    function doTransfersWithFees(
    ...
    ) internal returns (uint _totalAmount) {
        ...
        _totalAmount = getExchangeHelperImpl().calculateTotalAmount(_amount, _feePayerOrderData.originFees);
        // Transfer protocol fee (ERC721/ERC1155 seller pays protocol fee by receiving less than the order amount)
        uint rest = transferProtocolFee(_totalAmount, _amount, _from, _otherAssetType, _transferDirection);
        // Transfer royalties
        rest = transferRoyalties(_otherAssetType, _feePayerAssetType, rest, _amount, _from, _transferDirection);
        ...
        // amount
        (rest,) = transferFees(_otherAssetType, rest, _amount, _feePayerOrderData.originFees, _from, _transferDirection, ORIGIN);
        (rest,) = transferFees(_otherAssetType, rest, _amount, _otherOrderData.originFees, _from, _transferDirection, ORIGIN);
    ...
    function transferProtocolFee(
        ...
        (uint _rest, uint _protocolFee) = getExchangeHelperImpl().subFeeInBp(
            _totalAmount,
            _amount,
        ...
        if (_protocolFee > 0) {
            ...
            if (_assetType.assetClass == LibAssetClasses.ERC20_ASSET_CLASS
                ...
                if (tokenAddress == weth) {
                    if (_assetType.assetClass == LibAssetClasses.PROXY_WETH_ASSET_CLASS) {
                        ...
                        IExchangeGovernedProxy(proxy).safeTransferERC20(IERC20(_storage.getWETH()), address(this), _protocolFee);
                    } else {
                        // Transfer WETH from user to implementation via proxy
                        IExchangeGovernedProxy(proxy).safeTransferERC20From(IERC20(tokenAddress), _from, address(this), _protocolFee);
                    }
                    // Redeem ETH for WETH
                    IWrappedCoin(weth).withdraw(_protocolFee);
                    // Forward ETH to proxy via receiveETH function (proxy holds funds)
                    // selector = bytes4(keccak256(bytes('receiveETH()')))
                    (bool success, bytes memory data) = proxy.call{ value: _protocolFee }(abi.encodeWithSelector(0x3ecfd51e));
                    require(
                        success && (data.length == 0 || abi.decode(data, (bool))),
                ...
                uint tokenId;
                (tokenAddress, tokenId) = abi.decode(_assetType.data, (address, uint));
            }
    ...
    function transferRoyalties(
        ...
        address _from,
        bytes4 _transferDirection
    ) internal returns (uint _newRest) {
        ...
        _newRest = _rest;
        // Transfer royalties
        uint totalRoyaltiesBps;
        if (royalties.length == 0) {
            // Try to get royalties from the token contract itself assuming it implements EIP-2981 standard
            (address tokenAddress, uint tokenId) = abi.decode(_feePayerAssetType.data, (address, uint));
            ...
            try IERC165(tokenAddress).supportsInterface(INTERFACE_ID_ERC2981) returns (bool isSupported) {
                if (isSupported) {
                    ...
                    (address _royaltyReceiver, uint _royaltyAmount) = IERC2981Royalties(tokenAddress).royaltyInfo(tokenId, _amount);
    ...
    function transferFees(
    ...
    ) internal returns (uint _newRest, uint _totalFeesBps) {
        _totalFeesBps = 0;
        _newRest = _rest;
        for (uint256 i = 0; i < _fees.length; i++) {
            ...
            (uint newRestValue, uint feeValue) = getExchangeHelperImpl().subFeeInBp(_newRest, _amount, _fees[i].value);
    ...
    function transferPayouts(
        ...
        for (uint256 i = 0; i < _payouts.length - 1; i++) {
            ...
            uint payoutAmount = getExchangeHelperImpl().bp(_amount, _payouts[i].value);
    ...
    function transfer(
        ...
        require (
            _to != address(0),
            'Exchange: can not transfer to zero address'
        );
        require (
            _asset.value != 0,
            'Exchange: transfer amount can not be zero'
        );
        if (_asset.assetType.assetClass == LibAssetClasses.ETH_ASSET_CLASS) {
            ...
            IExchangeGovernedProxy(proxy).safeTransferERC20(IERC20(_storage.getWETH()), _to, _asset.value);
        } else if (_asset.assetType.assetClass == LibAssetClasses.WETH_ASSET_CLASS) {
            // Transfer WETH from user via proxy
            IExchangeGovernedProxy(proxy).safeTransferERC20From(IERC20(_storage.getWETH()), _from, _to, _asset.value);
            ...
            IExchangeGovernedProxy(proxy).safeTransferERC20From(IERC20(token), _from, _to, _asset.value);
            ...
            IExchangeGovernedProxy(proxy).safeTransferERC721From(IERC721(token), _from, _to, tokenId);
        } else if (_asset.assetType.assetClass == LibAssetClasses.ERC1155_ASSET_CLASS) {
            (address token, uint tokenId) = abi.decode(_asset.assetType.data, (address, uint256));
            // Transfer ERC1155 token from user via proxy
            IExchangeGovernedProxy(proxy).safeTransferERC1155From(IERC1155(token), _from, _to, tokenId, _asset.value, '');
        } else {
    ...
    //
    // Set protocol fee
    function setProtocolFee(uint newProtocolFee) external override onlyOwner {
    ...
    function setERC20AssetAllowed(address _erc20AssetAddress, bool _isAllowed) external override onlyOwner {
        _storage.setERC20AssetAllowed(_erc20AssetAddress, _isAllowed);
    }
    // External getter functions
    //
    function getWhitelistProxy() external override view returns (address _whitelistProxy) {
        _whitelistProxy = _storage.getWhitelistProxy();
    }
    function getOrderBook() external override view returns (address _orderBook) {
    ...
    function getRoyaltiesRegistry() external override view returns (address) {
        return _storage.getRoyaltiesRegistry();
    }
    function getFeeReceiver(address _token) external override view returns (address) {
        // Use address(0) as token address for ETH
        return _storage.getFeeReceiver(_token);
    }
    function getOrderFill(bytes32 orderKeyHash) external override view returns (uint) {
        return _storage.getFill(orderKeyHash);
    }
    function getOrdersFills(bytes32[] calldata ordersKeyHashes) external override view returns (uint[] memory) {
        uint[] memory ordersFills = new uint[](ordersKeyHashes.length);
        // Loop over ordersKeyHashes array
        for (uint i = 0; i < ordersKeyHashes.length; i++) {
