...
contract MissionsManager is Ownable {
	...
	event PackMint(string indexed uniqueId, address indexed owner, uint256[] tokenIds, string method);
	...
	function purchasePackETH(string memory _uniqueId, uint256 amount, bytes memory _signature) external payable {
	...
	function purchasePackSC(string memory _uniqueId, uint256 amount, bytes memory _signature) external {
		if (amount == 0) revert InvalidAmount();
