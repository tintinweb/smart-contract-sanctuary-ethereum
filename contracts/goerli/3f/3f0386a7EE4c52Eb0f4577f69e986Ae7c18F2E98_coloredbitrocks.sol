...
error bitrocksFactoryNotOpenYet();
error NobitrocksForYou();
error ExceedsSupply();
...
abstract contract bitrocksFactory {
    function bitrocks(address to, uint256 boxId) public virtual returns (uint256);
}

contract coloredbitrocks is ERC721AQueryable, ReentrancyGuard, ERC2981, Ownable {
    using ECDSA for bytes32;

    address private bitrocksContract;
    bool public canbitrocks;
    ...
    constructor() ERC721A('Bitrocks', 'BR') {}
    ...
    function bitrocks(uint256 id_) public nonReentrant returns (uint256) {
        if (!canbitrocks) {
            if (msg.sender != owner()) revert NobitrocksForYou();
        }
        ...
        bitrocksFactory factory = bitrocksFactory(bitrocksContract);

        _burn(id_, true);

        uint256 newId = factory.bitrocks(destinationAddress, id_);
        return newId;
    }
    ...
    function toggleCanbitrocks() external onlyOwner {
        if (bitrocksContract == address(0)) revert bitrocksFactoryNotOpenYet();
        canbitrocks = !canbitrocks;
    }
    ...
    function setbitrocksContract(address contractAddress_) external onlyOwner {
        bitrocksContract = contractAddress_;
    }
