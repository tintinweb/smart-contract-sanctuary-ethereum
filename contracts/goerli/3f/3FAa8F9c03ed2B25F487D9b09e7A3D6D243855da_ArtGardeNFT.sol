...
contract ArtGardeNFT is Ownable, ERC721URIStorage, EIP712, IERC721Receiver {
    bytes32 private constant _MINT_TYPEHASH =
        keccak256("MintRequest(string uri,address minter)");
    ...
    constructor(address relayer)
        ERC721("ArtGardeNFT", "AGN")
        EIP712("ArtGardeNFT", "1.0.0")
    {
        require(relayer != address(0), "ArtGardeNFT: invalid relayer address");
        _relayer = relayer;
    }

    function changeRelayer(address newRelayer) external onlyOwner {
        require(
            newRelayer != address(0),
            "ArtGardeNFT: invalid relayer address"
        );
    ...
    function mint(string calldata tokenURI, bytes calldata signature) external {
        require(
            _verify(_relayer, _hash(msg.sender, tokenURI), signature),
            "ArtGardeNFT: invalid signature"
        );
        require(
            !_uriExists[tokenURI],
            "ArtGardeNFT: token already exists with the given URI"
        );
        _tokenId++;
        _uriExists[tokenURI] = true;
        _safeMint(address(this), _tokenId);
        _setTokenURI(_tokenId, tokenURI);
        emit Mint(msg.sender, _tokenId, tokenURI);
    }

    function claim(address to, uint256 tokenId) external {
        require(msg.sender == _relayer, "ArtGardeNFT: only relayer can call");
    ...
    function _hash(address minter, string calldata tokenURI)
        private
        view
        returns (bytes32)
    {
