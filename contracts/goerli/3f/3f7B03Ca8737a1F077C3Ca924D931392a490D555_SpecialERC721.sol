...
contract SpecialERC721 is ERC721Enumerable, Ownable {
...
contract Market is Ownable {
    ...
    mapping (address => mapping(uint256 => fixedMarketItem)) public marketFixedItem;
    
    mapping (address => mapping(uint256 => auctionMarketItem)) public marketAutionItem;
    ...
    function purchaseAuction(
      ...
      require(tokenAddr != address(0), "Please set the token address");
      require(nftInstance.getApproved(tokenId)==address(this), "Nft Not Approved By the owner");
  
        if(burn_Method == true) {
          for(uint j = 0; j < marketFixedItem[nftAddress][tokenId].selltype.length; j++) {
            if(marketFixedItem[nftAddress][tokenId].selltype[j] == tokenAddr)
            {
              require(marketAutionItem[nftAddress][tokenId].auction_Price[j] <= bidAmount, "Please enter the greater bidAmount");

              require(token.allowance(msg.sender, address(this)) >= bidAmount, "Payable is not allowed by buyer");

              uint256 burnPrice = bidAmount * 98 / 100;

              token.transferFrom(msg.sender, burnAddr, burnPrice); 
              token.transferFrom(msg.sender, owner, (bidAmount - burnPrice));
              break;
            }
          }
        ...
        nftInstance.transferFrom(owner, to, tokenId);
           ...
           for(uint k = 0; k < marketFixedItem[nftAddress][tokenId].selltype.length; k++) {
            if(marketFixedItem[nftAddress][tokenId].selltype[k] == tokenAddr)
            {
              require(marketAutionItem[nftAddress][tokenId].auction_Price[k] <= bidAmount, "Please enter the greater bidAmount");

              require(token.allowance(msg.sender, address(this)) >= bidAmount, "Payable is not allowed by buyer");

              uint256 burnPrice = bidAmount * 2 / 100;
    ...
    function purchaseWithFixedPrice (
      ...
      if(tokenAddr == address(0)) {
        for(uint i = 0; i < marketFixedItem[nftAddress][tokenId].selltype.length; i++) {
          if(marketFixedItem[nftAddress][tokenId].selltype[i] == address(0))
          {
            uint256 price = marketFixedItem[nftAddress][tokenId].fixed_Price[i];
      ...
      nftInstance.transferFrom(owner, to, tokenId);
        ...
        if(burn_Method == true) {
          for(uint j = 0; j < marketFixedItem[nftAddress][tokenId].selltype.length; j++) {
            if(marketFixedItem[nftAddress][tokenId].selltype[j] == tokenAddr)
            {
              uint256 price = marketFixedItem[nftAddress][tokenId].fixed_Price[j];

              require(token.allowance(msg.sender, address(this)) >= price, "Payable is not allowed by buyer");

              uint256 burnPrice = price * 98 / 100;

              token.transferFrom(msg.sender, burnAddr, burnPrice); 
              token.transferFrom(msg.sender, owner, (price - burnPrice));
              break;
            }
          }
        ...
        nftInstance.transferFrom(owner, to, tokenId);
           ...
           for(uint k = 0; k < marketFixedItem[nftAddress][tokenId].selltype.length; k++) {
            if(marketFixedItem[nftAddress][tokenId].selltype[k] == tokenAddr)
            {
              uint256 price = marketFixedItem[nftAddress][tokenId].fixed_Price[k];
    ...
    function putOnAuction(
      ...
      marketAutionItem[nftAddress][tokenId] = auctionMarketItem (
        nftAddress,
        tokenId,
        payableAddress,
        selltype,
    ...
    function putOnFixedPriceSale (
      ...
      require(auctionTimestamp > block.timestamp, "Time should be greater than current time");
      ...
      marketFixedItem[nftAddress][tokenId] = fixedMarketItem (
        nftAddress,
        tokenId,
        payableAddress,
        selltype,
