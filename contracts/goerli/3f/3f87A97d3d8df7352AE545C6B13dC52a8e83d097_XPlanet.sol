...
contract XPlanet is ERC721Enumerable, Ownable {
   ...
   // address public constant USDC_CONTRACT = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F;
    ...
    uint256 public MAX_MINT_PHASE = 1;

    bool public whitelistSale = false;
     
    bool public publicSale = false; // DA CAMBIARE ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    function mint(uint256  _numberOfTokens) external {
        ...
        usdc.transferFrom(_user, address(this), _amount * 1000000);
    ...
    function airDrop( uint256 _numberOfTokens, address _user ) public onlyOwner{ 
        require(
            totalSupply() + _numberOfTokens <= MAX_MINT_PHASE,
            "max-supply-reached"
        );
        for (uint256 i = 0; i < _numberOfTokens; i++) {
            if (totalSupply() < MAX_MINT_PHASE) {
