...
contract HAaveProtocol is HandlerBase, IFlashLoanReceiver {
  address public constant POOLADDRESS = 0x4bd5643ac6f66a5237E18bfA7d47cF22f1c9F210;
  ...
  function deposit(
    ...
    ILendingPoolV2(POOLADDRESS).deposit(asset, amount, onBehalfOf, REFERRAL_CODE);
  }
  ...
  function withdraw(
    ...
    return ILendingPoolV2(POOLADDRESS).withdraw(asset, amount, to);
  }
  ...
  function borrow(
    ...
    ILendingPoolV2(POOLADDRESS).borrow(asset, amount, interestRateMode, REFERRAL_CODE, onBehalfOf);
  ...
  function repay(
    ...
    IERC20(asset).transferFrom(msg.sender, address(this), amount);
    IERC20(asset).approve(POOLADDRESS, amount);
    ILendingPoolV2(POOLADDRESS).repay(asset, amount, rateMode, onBehalfOf);
  }
  ...
  //   console.log("Amount Value",msg.value);
  //   amount = _getBalance(NATIVE_TOKEN_ADDRESS, amount);
  //   console.log("Amount",amount);
  //   IWETH9(WETH).deposit{ value: amount }();
  //   // IERC20(WETH).transferFrom(msg.sender, address(this), msg.);
  //   IERC20(WETH).approve(POOLADDRESS, amount);
  //   ILendingPoolV2(POOLADDRESS).deposit(WETH, amount, address(this), REFERRAL_CODE);
  //   _updateToken(WETH);
  ...
  function flashLoan(
    ...
    try ILendingPoolV2(POOLADDRESS).flashLoan(address(this), assets, amounts, modes, onBehalfOf, params, REFERRAL_CODE) {} catch Error(string memory reason) {
    ...
    for (uint256 i = 0; i < assets.length; i++) {
      _tokenApproveZero(assets[i], POOLADDRESS);
  ...
  function executeOperation(
    ...
    _requireMsg(msg.sender == POOLADDRESS, "executeOperation", "invalid caller");

    _requireMsg(initiator == address(this), "executeOperation", "not initiated by the proxy");

    (address[] memory tos, bytes32[] memory configs, bytes[] memory datas) = abi.decode(params, (address[], bytes32[], bytes[]));
    ...
    for (uint256 i = 0; i < assets.length; i++) {
      ...
      _tokenApprove(assets[i], POOLADDRESS, amountOwing);
    }
    return true;
  }
   ...
   function _deposit(address asset, uint256 amount) internal {
    ...
    try ILendingPoolV2(pool).deposit(asset, amount, address(this), REFERRAL_CODE) {} catch Error(string memory reason) {
