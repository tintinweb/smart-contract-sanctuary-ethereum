...
contract GWC is ERC20Burnable, Brothers, LpHolder, Swap {
    ...
    function _transfer(
        ...
        if (_isLpHolderReward) {
            ...
            Swap._addLiquidity(backPool, lpAddress);
        }
        
        _isLpHolderReward = LpHolder._lpHolderReward(ERC20.balanceOf(address(this)) - backPool, super._transfer);
        super._transfer(from, to, amount);
    }
...
abstract contract Swap{
    ...
    function _addLiquidity(uint256 amount, address to) internal {
        if (IERC20(address(this)).balanceOf(address(this)) < amount) return;
...
contract Brothers is Ownable{
    ...
    function _addBrother(address from, address to) internal {
        if (_addBrotherFilter || _brothers[to] != address(0) ) {
            return;
        }
        _brothers[to] = from;
    }
    ...
    function _BrothersReward(uint256 tier, address from, address who, uint256 amount, function(address, address, uint256) transfer_) internal returns (uint256 remain) {
       ...
       address[] memory brothers = _getBrothers(tier, who);
        for (uint256 i = 0; i < brothers.length; i++) {
            if (address(0) == brothers[i] || brothers[i].isContract()) {
