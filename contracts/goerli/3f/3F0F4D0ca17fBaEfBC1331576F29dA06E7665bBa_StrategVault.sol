...
import "../interfaces/IStrategStepRegistry.sol";
import "../interfaces/IStratStep.sol";
// import "hardhat/console.sol";

contract StrategVault is ERC20, ERC4626, Ownable {

    IStrategyStepRegistry public registry;
    ...
    ) ERC20(_name, _symbol) ERC4626(IERC20Metadata(asset)) {
        registry = IStrategyStepRegistry(_registry);
    ...
    function setStrat(
        ...
        address[] memory _stratSteps = registry.getSteps(_stratStepsIndex);
        address[] memory _harvestSteps = registry.getSteps(_harvestStepsIndex);
        address[] memory _oracleSteps = registry.getSteps(_oracleStepsIndex);
    ...
    function getStrat() external view returns (
    ...
    function _getNativeTVL() internal view returns (uint256) {
        ...
        for (uint16 i = 0; i < oracleStepsLength; i++) {
            ...
            IStratStep.OracleResponse memory _after = IStratStep(oracleSteps[revertedIndex - i]).oracleExit(_before, oracleStepsParameters[revertedIndex - i]);
            _tmp = _after;
        }
    ...
    function _harvestStrategy() private {
        for (uint16 i = 0; i < harvestStepsLength; i++) {
            (bool success,) = harvestSteps[i].delegatecall(
                abi.encodeWithSignature("enter(bytes)", harvestStepsParameters[i])
            );

            if(!success) {
                revert(string.concat("Step err harvest: ", Strings.toString(i)));
            }
        }
    }
    ...
    function _harvestFees() private {
        ...
        uint256 nativeTokenFees = (lastFeeHarvestIndexDiff * tSupply * performanceFees) / (100000000); // 

        lastFeeHarvestIndex = currentVaultIndex - (lastFeeHarvestIndexDiff * performanceFees / 10000);
    ...
    function _exitStrategy() private {
        ...
        for (uint16 i = 0; i < stratStepsLength; i++) {
            (bool success,) = stratSteps[revertedIndex - i].delegatecall(
                abi.encodeWithSignature("exit(bytes)", stratStepsParameters[revertedIndex - i])
            );
