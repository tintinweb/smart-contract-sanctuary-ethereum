...
contract Test is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    function MintPhase2(uint256 amount) public payable nonReentrant {
        require(MintPhase2Active, "MintPhase2 is not enabled");
        require(_minted + amount <= MAX_SUPPLY, "Exceed max supply");
     //   require(isDVDAHolder(msg.sender), "Not DVDA Holder.");
    ...
    function MintPhase3(uint256 amount,  bytes32[] memory proof) public payable nonReentrant {
        require(MintPhase3Active, "MintPhase3 is not enabled");
        require(_minted + amount <= MAX_SUPPLY, "Exceed max supply");
        require(isChiliTravelsHolder(msg.sender, proof) /* || isGHNHolder(msg.sender) || isAHCHolder(msg.sender) || isMGHNHolder(msg.sender) */, "Not Holder of Collections.");
...
//change smart contracts
    function isDVDAHolder(address _address) public returns(bool){
        dvda = IERC721(0xd9145CCE52D386f254917e481eB44e9943F39138);
        bool isOwner = false;
    ...
    function isGHNHolder(address _address) public returns(bool){
        ghnft = IERC721(0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8);
        bool isOwner = false;
        if (ghnft.balanceOf(_address) > 0){
            isOwner = true;
        }
        return isOwner;
    }
    ...
    function isMGHNHolder(address _address) public returns(bool){
        mghnft = IERC721(0xf8e81D47203A594245E36C48e151709F0C19fBe8);
        bool isOwner = false;
        if (mghnft.balanceOf(_address) > 0){
            isOwner = true;
        }
        return isOwner;
    }

    function isAHCHolder(address _address) public returns(bool){
        ahc = IERC721(0xD7ACd2a9FD159E69Bb102A1ca21C9a3e3A5F771B);
        bool isOwner = false;
