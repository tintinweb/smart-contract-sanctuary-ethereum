    ...
    ERC1155PausableUpgradeable
{
    ...
    mapping(uint256 => uint256) public saleStatus; // NFT Sale Status - 0,1,2
    mapping(uint256 => libERC1155Fee.Part) public auction; // NFT Auction Details - bidder and bid amount
    ...
    function placeBid(
        ...
        // _setAuctionPayMode(collectableId, libERC1155Fee.PAY_TYPE.CRYPTO);
    ...
    function placeFiatBid(
        ...
        require(
            amount > auction[collectableId].value,
            "Amount is lesser than the previous bid"
        );

        // require(!msg.sender.isContract(), "Contracts cannot do bidding");

        _returnCurrentBid(collectableId);
        _setAuctionPayMode(collectableId, libERC1155Fee.PAY_TYPE.FIAT);
        _setBidder(collectableId, payable(msg.sender), amount, msg.value);
    ...
    function releaseFiatBidNft(address bidder, uint256 collectableId)
        public
        onlyAdmin
        nonReentrant
    {
        require(
            bidder == auction[collectableId].account,
            "Wrong bidder address"
        );
        ...
        require(
            (auction[collectableId].mode == libERC1155Fee.PAY_TYPE.FIAT),
            "Can't able to release the token"
        );
