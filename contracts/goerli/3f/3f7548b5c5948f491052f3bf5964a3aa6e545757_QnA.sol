...
contract QnA {
   ...
   /*enum Status {questionRegistration, cancellation, registeringAnswer, completion}
   ...
   function question(string memory _questionContent string memory _questioner) public returns(string memory _answerContent) {
       status = Status.questionRegistration;
       require(msg.sender !== questioner);
       return _answerContent;
       status = Status.registeringAnswer;
       _answerContent = Status.completion;
       //한 질문에 대해 답변은 1개만 등록할 수 있다 부터 막혔습니다.
   }*/
}