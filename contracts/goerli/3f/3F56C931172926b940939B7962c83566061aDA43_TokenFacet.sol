...
import "../interfaces/IOpenseaSeaportConduitController.sol";
...
library Constants {
	...
	uint256 public constant MIN_ETH_BALANCE = 0 ether;

	string public constant NAME = "Chiripto + test";
	string public constant SYMBOL = "chiripto";
	string public constant VERSION = "1.0.0";
	string public constant DESCRIPTION = "jfjfjjfjfjfjffjfjfjfj";
}
	...
	BaseContract
{
    ...
    constructor(string memory name_, string memory symbol_) {
        getState().name = name_;
        getState().symbol = symbol_;
        getState().currentIndex = _startTokenId();
    }
    ...
    function totalSupply() public view override returns (uint256) {
        ...
        unchecked {
            return getState().currentIndex - getState().burnCounter - _startTokenId();
        }
    }
    ...
    function _totalMinted() internal view returns (uint256) {
        ...
        unchecked {
            return getState().currentIndex - _startTokenId();
        }
    }
    ...
    function balanceOf(address owner) public view override returns (uint256) {
        ...
        return getState().packedAddressData[owner] & BITMASK_ADDRESS_DATA_ENTRY;
    }
    ...
    function _numberMinted(address owner) internal view returns (uint256) {
        return (getState().packedAddressData[owner] >> BITPOS_NUMBER_MINTED) & BITMASK_ADDRESS_DATA_ENTRY;
    }
    ...
    function _numberBurned(address owner) internal view returns (uint256) {
        return (getState().packedAddressData[owner] >> BITPOS_NUMBER_BURNED) & BITMASK_ADDRESS_DATA_ENTRY;
    }
    ...
    function _getAux(address owner) internal view returns (uint64) {
        return uint64(getState().packedAddressData[owner] >> BITPOS_AUX);
    }
    ...
    function _setAux(address owner, uint64 aux) internal {
        uint256 packed = getState().packedAddressData[owner];
        uint256 auxCasted;
        ...
        getState().packedAddressData[owner] = packed;
    }
    ...
    function _packedOwnershipOf(uint256 tokenId) private view returns (uint256) {
        ...
        unchecked {
            if (_startTokenId() <= curr)
                if (curr < getState().currentIndex) {
                    uint256 packed = getState().packedOwnerships[curr];
                    // If not burned.
                    if (packed & BITMASK_BURNED == 0) {
                        ...
                        while (packed == 0) {
                            packed = getState().packedOwnerships[--curr];
                        }
                        return packed;
                    }
                }
    ...
    function _ownershipAt(uint256 index) internal view returns (TokenOwnership memory) {
        return _unpackedOwnership(getState().packedOwnerships[index]);
    }
    ...
    function _initializeOwnershipAt(uint256 index) internal {
        if (getState().packedOwnerships[index] == 0) {
            getState().packedOwnerships[index] = _packedOwnershipOf(index);
        }
    }
    ...
    function approve(address to, uint256 tokenId) public override {
        ...
        getState().tokenApprovals[tokenId] = to;
    ...
    function getApproved(uint256 tokenId) public view override returns (address) {
        ...
        return getState().tokenApprovals[tokenId];
    }
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        ...
        getState().operatorApprovals[_msgSenderERC721A()][operator] = approved;
    ...
    function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
        return getState().operatorApprovals[owner][operator];
    }
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        return
            ...
            tokenId < getState().currentIndex && // If within bounds,
            getState().packedOwnerships[tokenId] & BITMASK_BURNED == 0; // and not burned.
    }
    ...
    function _safeMint(
        ...
        unchecked {
            if (to.code.length != 0) {
                uint256 end = getState().currentIndex;
                ...
                if (getState().currentIndex != end) revert();
            }
        }
    }
    ...
    function _mint(address to, uint256 quantity) internal {
        uint256 startTokenId = getState().currentIndex;
        ...
        unchecked {
            ...
            getState().packedAddressData[to] += quantity * ((1 << BITPOS_NUMBER_MINTED) | 1);
            ...
            getState().packedOwnerships[startTokenId] = _packOwnershipData(
                to,
    ...
    function _mintERC2309(address to, uint256 quantity) internal {
        ...
        unchecked {
            ...
            getState().currentIndex = startTokenId + quantity;
        }
        _afterTokenTransfers(address(0), to, startTokenId, quantity);
    }
    ...
    {
        mapping(uint256 => address) storage tokenApprovalsPtr = getState().tokenApprovals;
    ...
    function transferFrom(
        ...
        unchecked {
            ...
            --getState().packedAddressData[from]; // Updates: `balance -= 1`.
            ++getState().packedAddressData[to]; // Updates: `balance += 1`.
            ...
            getState().packedOwnerships[tokenId] = _packOwnershipData(
                to,
            ...
            if (prevOwnershipPacked & BITMASK_NEXT_INITIALIZED == 0) {
                ...
                if (getState().packedOwnerships[nextTokenId] == 0) {
                    // If the next slot is within bounds.
                    if (nextTokenId != getState().currentIndex) {
                        // Initialize the next slot to maintain correctness for `ownerOf(tokenId + 1)`.
                        getState().packedOwnerships[nextTokenId] = prevOwnershipPacked;
                    }
                }
            }
        }
    ...
    function _burn(uint256 tokenId, bool approvalCheck) internal virtual {
        ...
        unchecked {
            ...
            getState().packedAddressData[from] += (1 << BITPOS_NUMBER_BURNED) - 1;
    ...
    function _setExtraDataAt(uint256 index, uint24 extraData) internal {
        uint256 packed = getState().packedOwnerships[index];
        ...
        getState().packedOwnerships[index] = packed;
    }
