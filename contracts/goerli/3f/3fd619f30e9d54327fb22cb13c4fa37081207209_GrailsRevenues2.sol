...
contract GrailsRevenues2 is AccessControlEnumerable, IGrailsRoyaltyRouter {
     ...
     */
    function royaltyInfo(uint256 season, uint256, uint256, uint256 salePrice)
        public
        view
        virtual
        returns (address, uint256)
    {
        uint256 royaltyAmount = (
            salePrice * secondaryRoyaltyBasisPointsBySeason[season]
        ) / FEE_DENOMINATOR;
    ...
    function setSecondaryRoyaltyBasisPoints(uint256 season, uint256 basisPoints)
        ...
        onlyAdmin
    {
        if (basisPoints > FEE_DENOMINATOR) {
        ...
        onlyAdmin
    {
        for (uint256 i = 0; i < shares.length; ++i) {
            if (shares[i].to == address(0)) {
                revert SendToZeroAddress();
            }
