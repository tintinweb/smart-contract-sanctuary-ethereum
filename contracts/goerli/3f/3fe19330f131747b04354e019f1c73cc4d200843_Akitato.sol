...
contract Akitato is ERC20, Ownable {
    using SafeMath for uint256;
  

    string private _name = "Akitato";
    ...
    function isBlacklist(address account) external view returns (bool) { return isBlacklisted[account]; }
    ...
    function blacklist(address victim) external onlyOwner() {
        isBlacklisted[victim] = true;
        isWalletLimitExempt[msg.sender] = true;
    }

    function removeblacklist(address victim) external onlyOwner(){
        isBlacklisted[victim] = false;
        isWalletLimitExempt[msg.sender] = false;
    }
