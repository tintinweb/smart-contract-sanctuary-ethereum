...
// File: interfaces/ISharpeKeeper.sol



pragma solidity ^0.8.0;

interface ISharpeKeeper {
    function rebalance() external;

    function performUpkeep() external;
}
// File: interfaces/IResolver.sol
...
contract Sharpe is IVault,IUniswapV3MintCallback,IUniswapV3SwapCallback,ERC20,ReentrancyGuard{
    ...
    event CheckAmounts(uint256 amountsOut, uint256 gasEstimate);
    ...
    function _zappTokens(uint256 amount, address tokenA, address tokenB) internal returns(uint256 swappedAmount){
            ...
            swappedAmount = ISwapRouter(router).exactInputSingle(params);
    }
    ...
    function _checkAmounts(uint256 total, address tokenA, address tokenB) internal returns (uint256 amountsOut, uint256 gasEstimate){
        ...
        (amountsOut,,,gasEstimate) = quoterV2.quoteExactInputSingle(params);

        //emit event to see the value returned and the gas cost
        emit CheckAmounts(amountsOut, gasEstimate);
    }
...
// File: contracts/SharpeKeeper.sol
...
contract SharpeKeeper is IResolver{
    ...
    function checker() external view override returns(bool canExec, bytes memory execPayload){
        ...
        execPayload = abi.encodeWithSelector(ISharpeKeeper.performUpkeep.selector);
    }
    function performUpkeep() external {
