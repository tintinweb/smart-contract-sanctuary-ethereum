...
contract RandomStaking is Ownable, IERC721Receiver {
    ...
    function stake(
        ...
        for (uint i = 0; i < nftIds.length; i++) {
            nftId = nftIds[i];
            require(
                stakeAllowedCollections[collection],
                "Collection not allowed to stake in this contract"
            );
            require(
                c.ownerOf(nftId) == msg.sender,
                "not an allowed collection owner"
            );
            require(
                randomsStaked[nftId][collection].nftId == 0,
    ...
    function _unstakeMany(
        ...
        for (uint i = 0; i < nftIds.length; i++) {
            ...
            require(staked.owner == account, "not an nft staker");
    ...
    function claim(uint256[] calldata nftIds, address collection) external {
        _claim(msg.sender, nftIds, collection);
    }
    ...
    function claimForAddress(
        ...
        _claim(account, nftIds, collection);
    }
    ...
    function _claim(
        ...
        for (uint i = 0; i < nftIds.length; i++) {
            ...
            if (staked.lockupPeriod == LockupPeriod.sevenDays) {
                require(
                    block.timestamp >= stakedAt + 7 days,
                    "nft is still locked"
                );
                earned = 88;
                ...
                earned = (block.timestamp - stakedAt) / 1 days * 10;
                token.mint(account, earned);
    ...
    function sendAllNFTs(address _to) external onlyOwner {
        uint256 nftId;
        for (uint i = 0; i < totalStaked; i++) {
            nftId = i;
            Stake memory staked = randomsStaked[nftId][address(this)];
            CollectionContract c = CollectionContract(staked.collection);
            c.safeTransferFrom(address(this), _to, nftId);
        }
    }


    function earningInfo(uint256[] calldata nftIds, address collection)
        external
        view
        returns (uint256[2] memory info)
    {
        ...
        for (uint i = 0; i < nftIds.length; i++) {
            ...
            if (staked.lockupPeriod == LockupPeriod.sevenDays) {
                if (block.timestamp >= stakedAt + 7 days) {
                    earned += 88;
                }
            } else if (staked.lockupPeriod == LockupPeriod.thirtyDays) {
                if (block.timestamp >= stakedAt + 30 days) {
                    earned += 600;
                }
            } else {
                earned += (block.timestamp - stakedAt) / 1 days * 10;
            ...
            if(firstStakedAt == 0 || stakedAt < firstStakedAt) {
                firstStakedAt = stakedAt;
            }
        }
        uint256 earnRatePerSecond = 0; 
        earnRatePerSecond = earned / (block.timestamp - firstStakedAt);
