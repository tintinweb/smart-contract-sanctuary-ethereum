...
contract supplychain {
        ...
        )
    {
        return (
            ...
            rdMapping[_id].name,
            soldMapping[_id],
            block.timestamp >= medMapping[_id].exp,
            medMapping[_id].expdate
        );
    }
    ...
    function sendmedtodistributor(
        ...
        for (uint256 i = sid; i <= lid; i++) {
            ...
            require(
                ...
                "medicine already sent to someone else"
            );
    ...
    function destroymed(uint256 sid, uint256 lid) public {
        for (uint256 i = sid; i <= lid; i++) {
            require(
                mdMapping[i].user == msg.sender ||
                    rdMapping[i].own == msg.sender ||
