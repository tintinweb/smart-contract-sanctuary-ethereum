...
abstract contract Market is MarketPositionManager, MarketManager {
    ...
    function premiumOnClose(uint id) public view override returns (uint premiumByCI, uint fee, uint extraPremium) {
        return IRiskCalculation(calc).premiumOnClose(id);
    }

    /// @inheritdoc MarketCore
    function premiumOnClaim(uint id) public view override returns (uint premiumByCI, uint fee, uint extraPremium) {
        return IRiskCalculation(calc).premiumOnClaim(id);
    }

    /// @inheritdoc MarketCore
    function premiumOnCancel(uint id) public view override returns (uint premiumByCI, uint fee, uint extraPremium) {
    ...
    IMarket
{
        ...
        returns (uint premium, Payout memory payout)
    {
        ...
        (uint premiumByCI, uint fee, uint extraPremium) = premiumOnClose(id);
    ...
    returns (uint premium, Payout memory payout)
    {
        ...
        (uint premiumByCI, uint fee, uint extraPremium) = premiumOnCancel(id);
        premium = premiumByCI + fee + extraPremium;
        ...
        returns (uint id)
    {
        ...
        (pos.shares, pos.Q) = IState(state).newMakerPosition(account,pos.stableAmount, pos.tier, pos.term);
        ...
        returns (int yield, Payout memory payout)
    {
        ...
        require( liquidation || _makerPositionNotEnded(id), "MC: maker-position-not-ended"  );    
    ...
    function premiumOnClose(uint id)
        ...
        virtual
        override
        returns (uint premiumByCI, uint fee, uint extraPremium);
