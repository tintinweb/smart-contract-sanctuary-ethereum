...
contract CsToken is ERC20 {
    ...
    /**
     * @notice Logs event for setting clayMain contract.
     *
     * @param setter : Address of entity setting clayMain.
     * @param claymain : Address of clayMain contract.
     */
    event LogClayMainSet(address setter, address claymain);

    /// @notice Check if the clayMain init is already done or not
    modifier onlyOnce() {
        require(msg.sender == owner && !setInit, "Already Initialized");
        _;
    }
    ...
    /**
     * @dev Initializes the values for `name`, `symbol` and `owner`.
    ...
    /**
     * @dev Sets the clayMain contract address to `_clayMain`
     * Emits a {LogClayMainSet} event.
     *
     * @notice only `owner` callable & can only be called once.
     *
     * Requirements:
     * - `_clayMain` cannot be the zero address.
     *
     * @param _clayMain : Address of clayMain contract.
     */
    function setClayMain(address _clayMain) external onlyOnce {
        require(_clayMain != address(0x0), "ClayMain address can not be zero address");
        clayMain = _clayMain;
        setInit = true;
        emit LogClayMainSet(owner, _clayMain);
    }
