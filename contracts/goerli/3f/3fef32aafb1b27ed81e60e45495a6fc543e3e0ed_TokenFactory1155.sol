    ...
    NFTV1Storage1155
{
    ...
    function BuyNFT(uint256[] memory _saleId, uint256[] memory _amount)
        external
        payable
        nonReentrant
    {
        uint256 numberOfSales = _saleId.length;
        require(numberOfSales <= 50);
        require(numberOfSales == _amount.length);
        uint256 totalAmount;

        for (uint256 i = 0; i < numberOfSales; i++) {
            LibMeta1155.checkBuy(_tokenMeta[_saleId[i]], _amount[i]);
            totalAmount += _tokenMeta[_saleId[i]].price * _amount[i];
        }
        require(msg.value == totalAmount);

        for (uint256 i = 0; i < numberOfSales; i++) {
            LibMeta1155.executeBuy(
                _tokenMeta[_saleId[i]],
                _amount[i],
                PNDC1155Address
            );

            ERC1155(_tokenMeta[_saleId[i]].collectionAddress).safeTransferFrom(
                address(this),
                msg.sender,
    ...
    function sellNFT(
        ...
        LibMeta1155.checkSale(_collectionAddress, _tokenId, _price, _amount);
        for (uint256 i = 0; i < _tokenId.length; i++) {
            _tokenIdTracker.increment();

            //needs approval on frontend
            ERC1155(_collectionAddress).safeTransferFrom(
                msg.sender,
                address(this),
                _tokenId[i],
                _amount[i],
    ...
    function cancelSale(uint256[] memory _saleId) external nonReentrant {
        require(_saleId.length <= 50);
        for(uint256 i = 0; i < _saleId.length; i ++) {
            LibMeta1155.executeSaleCancel(_tokenMeta[_saleId[i]]);

            ERC1155(_tokenMeta[_saleId[i]].collectionAddress).safeTransferFrom(
            address(this),
            _tokenMeta[_saleId[i]].currentOwner,
            _tokenMeta[_saleId[i]].tokenId,
            _tokenMeta[_saleId[i]].numberOfTokens,
            ""
        );
...
library LibMeta1155 {
    ...
    function transfer(TokenMeta storage token, uint256 _numberOfTokens) public {
    ...
    function checkSale(
        ...
        require(numberOfTokens <= 50);
        require(numberOfTokens == prices.length);
        require(numberOfTokens == amounts.length);
        for (uint256 i = 0; i < numberOfTokens; i++) {
            require(amounts[i] > 0);
            require(
                IERC1155(collectionAddress).balanceOf(
                    msg.sender,
                    tokenIds[i]
                ) >= amounts[i]
            );
    ...
    function checkBidSale(address collectionAddress, uint256[] memory tokenId, uint256[] memory price, uint256[] memory amount) external view {
        require(collectionAddress != address(0));
        uint256 numberOfSales = tokenId.length;
        require(numberOfSales <= 50);
        require(numberOfSales == price.length);
        require(numberOfSales == amount.length);
        for(uint256 i = 0; i < numberOfSales; i ++) {
            require(price[i] > 0);
            require(amount[i] > 0);
            require(IERC1155(collectionAddress).balanceOf(msg.sender, tokenId[i]) >= amount[i]);
        }
    }

    function checkBuy(TokenMeta storage meta, uint256 amount) external view {
        require(meta.status);
        require(msg.sender != address(0) && msg.sender != meta.currentOwner);
        require(!meta.bidSale);
        require(meta.numberOfTokens >= amount);
    }

    function executeBuy(
        TokenMeta storage meta,
        uint256 numberOfTokens,
        address PNDC1155Address
    ) external {
        ...
        uint256 sum = meta.price * numberOfTokens;
        uint256 val = meta.price * numberOfTokens;

        transfer(meta, numberOfTokens);
    ...
    function checkBid(TokenMeta storage meta, uint256 numberOfTokens) external {
        require(meta.currentOwner != msg.sender);
        require(meta.status);
        require(meta.bidSale);
        require(msg.value % numberOfTokens == 0);
        require(msg.value / numberOfTokens >= meta.price);
        require(meta.numberOfTokens >= numberOfTokens);
    }

    function executeSaleCancel(TokenMeta storage meta) external {
...
contract NFTBid1155 is NFTFactoryContract1155 {
    ...
    function Bid(uint256 _saleId, uint256 _amount) external payable {
        LibMeta1155.checkBid(_tokenMeta[_saleId], _amount);
    ...
    function SellNFT_byBid(address _collectionAddress, uint256[] memory _tokenId, uint256[] memory _price, uint256[] memory _amount)
        external
        nonReentrant
    {   
        LibMeta1155.checkBidSale(_collectionAddress, _tokenId, _price, _amount);
        for(uint256 i = 0; i < _tokenId.length; i ++) {
            _tokenIdTracker.increment();

            //needs approval on frontend
            ERC1155(_collectionAddress).safeTransferFrom(msg.sender, address(this), _tokenId[i], _amount[i], "");
