...
contract ArbDogeSale is Ownable, ReentrancyGuard {
	...
	event Purchased(address indexed sender, uint256 raise, uint256 allocation);
	...
	function purchase() external payable nonReentrant {
		...
		require(
			allocation + totalAllocation <= MAX_IDO,
			"Exceed MAX_IDO"
		);
	...
	function claim() external nonReentrant {
		...
		UserInfo storage user = userInfo[msg.sender];
		require(user.allocation > 0, "ZERO  ALLOCATION");
	...
	function claimTransfer(address token, address to) external onlyOwner {
    ...
    function setTotal(uint256 raised,uint256 allocation) public onlyOwner {
		totalRaised = raised;
