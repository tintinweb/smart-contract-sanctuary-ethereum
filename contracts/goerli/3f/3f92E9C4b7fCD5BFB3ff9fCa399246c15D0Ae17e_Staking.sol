...
contract Staking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, IStaking {
     ...
     */
    // uint256 public constant totalBlocksPerYear = (uint256(365 days) * (86400) * (10)) / (131); // for mainnet
    // uint256 public constant totalBlocksPerYear = 1210; // for local
    uint256 public constant totalBlocksPerYear = 60005; // for goerli
    ...
    function initialize(
        ...
        endBlock = block.number + (totalBlocksPerYear);
        stakingEndBlock = block.number + (totalBlocksPerYear - _lockinBlocks);
     ...
     */
    function depositTo(address _to, uint256 amount) external override beforeStakingEnds nonReentrant {
        depositCounter++;
        uint256 blockNumber = block.number;
        uint256 unlockBlock = blockNumber + (lockinBlocks);
        deposits[depositCounter] = Deposit(amount, blockNumber, unlockBlock, blockNumber, _to, DepositState.DEPOSITED, 0, 0);
        tvl = tvl + (amount);

        emit Deposited(depositCounter, _to, amount, unlockBlock);
     ...
     * @param amount Amount of tokens to be staked
     */
    function deposit(uint256 amount) external override beforeStakingEnds nonReentrant {
        depositCounter++;
        uint256 blockNumber = block.number;
        uint256 unlockBlock = blockNumber + lockinBlocks;
        deposits[depositCounter] = Deposit(amount, blockNumber, unlockBlock, blockNumber, msg.sender, DepositState.DEPOSITED, 0, 0);
        tvl = tvl + (amount);
    ...
    function claimPlasmaFromFactory(uint256[] calldata depositNumbers, address depositor) external override onlyFactory {
        ...
        _mintAndSendPlasma(depositor, totalPlasmaToMint);
    }
        ...
        returns (uint256 amount)
    {
        ...
        deposits[depositNumber].plasmaLastClaimedAt = block.number;
    }

    function claimPlasmaMultiple(uint256[] calldata depositNumbers) external {
        uint256 totalAmount;
        for (uint256 index = 0; index < depositNumbers.length; index++) {
            totalAmount += _claimPlasmaMultiple(depositNumbers[index], msg.sender);
        }
        _mintAndSendPlasma(msg.sender, totalAmount);
    }
        ...
        nonReentrant
    {
        ...
        deposits[depositNumber].plasmaLastClaimedAt = _getCurrentBlockWrtEndBlock();
        _mintAndSendPlasma(user, amount);
    }
     ...
     */
    function withdrawUfoMultiple(uint256[] calldata depositNumbers) external {
        require(depositNumbers.length <= MAX_LOOP_ITERATIONS, Errors.EXCEEDS_MAX_ITERATION);
        ...
        for (uint256 index = 0; index < depositNumbers.length; index++) {
            (uint256 a, uint256 b) = _withdrawUfoMultiple(depositNumbers[index], msg.sender, totalPoolShare);
            totalPlasmaToClaim += a;
            totalTokensToWithdraw += b;
        }
        IERC20Upgradeable(stakingToken).safeTransfer(msg.sender, totalTokensToWithdraw);
        _mintAndSendPlasma(msg.sender, totalPlasmaToClaim);
        IStakingFactory(factory).updateTVL(tvl);
    }
     ...
     * @param depositNumbers Deposit Numbers to withdraw
     */
    function withdrawPartialUfoMultiple(uint256[] calldata depositNumbers, uint256 fraction) external beforeStakingEnds {
        ...
        for (uint256 index = 0; index < depositNumbers.length; index++) {
            (uint256 a, uint256 b, uint256 c) = _withdrawPartialUfoMultiple(depositNumbers[index], msg.sender, fraction, totalPoolShare);
            totalPlasmaToClaim += a;
            totalTokensToWithdraw += b;
        ...
        )
    {
        ...
        _deposit.vestedRewardUnlockBlock = blockNumber + (totalBlocksPerYear);

        uint256 numberOfBlocksStaked = _deposit.unlockBlock - _deposit.startBlock;

        uint256 vestedReward = getVestedRewards(totalPoolShare, _deposit.amount, numberOfBlocksStaked);
    ...
    function _withdrawUfoMultiple(
        ...
        onlyWhenDeposited(depositNumber)
        onlyDepositor(depositNumber, depositor)
        returns (uint256 plasmToClaim, uint256 stakedTokensToWithdraw)
    {
        Deposit storage _deposit = deposits[depositNumber];
        require(block.number > _deposit.unlockBlock, Errors.ONLY_AFTER_END_BLOCK);
        ...
        nonReentrant
    {
        ...
        _mintAndSendPlasma(msg.sender, amount);
        IStakingFactory(factory).updateTVL(tvl);
    }
        ...
        nonReentrant
    {
        ...
        _transferVestedRewards(msg.sender, _deposit.vestedRewards);
    }
    ...
    function _mintAndSendPlasma(address receiver, uint256 amount) internal {
        ...
        IRootChainManager(maticBridge).depositFor(receiver, plasmaToken, abi.encode(amountMinted));
    }
     ...
     */
    function updatePlasmaPerBlockPerToken(uint256 _plasmaPerBlockPerToken) external override onlyFactory {
        plasmaPerBlockPerToken = _plasmaPerBlockPerToken;
...
interface IStaking {
    ...
    function updatePlasmaPerBlockPerToken(uint256 _plasmaPerBlockPerToken) external;
