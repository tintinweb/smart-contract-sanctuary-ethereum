...
import './TokenOffRampStorage.sol';
import '@openzeppelin/contracts/token/ERC20/IERC20.sol';
import '@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol';
import '@openzeppelin/contracts/security/Pausable.sol';

/// @title Kiara Exchange Logic Contract
...
contract TokenOffRamp is ITokenOffRamp, Pausable {
    TokenOffRampStorage immutable config;

    using SafeERC20 for IERC20;
    using TokenOffRampTypes for TokenOffRampTypes.TradeStatus;
    using TokenOffRampTypes for TokenOffRampTypes.Trade;

    /// @param _config Storage Contract
    constructor(TokenOffRampStorage _config) Pausable() {
        config = _config;
    }

    modifier onlyProtocolOwner() {
        require(msg.sender == config.protocolOwner(), 'Only Owner wallet can access');
        _;
    }

    modifier onlyAdmin() {
        require(
            msg.sender == config.protocolBackend() || msg.sender == config.protocolOwner(),
    ...
    modifier onlyCustomerOrAdmin(bytes32 tradeId) {
        (, address from, , , , ) = config.tradeForId(tradeId);
        require(
            msg.sender == config.protocolBackend() ||
                msg.sender == config.protocolOwner() ||
                msg.sender == from,
    ...
    modifier isTradeIDAvailable(bytes32 tradeId) {
        (TokenOffRampTypes.TradeStatus status, , , , , ) = config.tradeForId(tradeId);
        require(status == TokenOffRampTypes.TradeStatus.NOT_CREATED, 'Trade already exists');
        _;
    }

    modifier isTradeProcessable(bytes32 tradeId) {
        (TokenOffRampTypes.TradeStatus status, , , , , ) = config.tradeForId(tradeId);
        require(
            status == TokenOffRampTypes.TradeStatus.TRANSFER_CONFIRMED,
    ...
    modifier onlySupportedFiatCurrency(bytes8 currencyCode) {
        require(config.supportedFiatCurrencies(currencyCode), 'Fiat Currency is not supported');
        _;
    }
    ...
    ) public override whenNotPaused isTradeIDAvailable(id) onlySupportedFiatCurrency(toCurrency) {
        require(
            submittedTimestamp + config.createTradeExpiryPeriod() > block.timestamp,
            'Trade deposit period has expired!'
        );
        require(config.deposit(msg.sender, fromTokenAddress, fromAmount));

        config.setTradeForID(
            id,
            TokenOffRampTypes.TradeStatus.TRANSFER_CONFIRMED,
            msg.sender,
            fromTokenAddress,
            toCurrency,
            fromAmount,
            submittedTimestamp + config.expiryTimePeriod()
        );
        emit CreateTrade(id);
    }
    ...
    function refundTrade(bytes32 id)
        ...
        isTradeProcessable(id)
    {
        require(config.withdraw(id), 'Failed to refund trade');
        emit RefundTrade(id);
    }
    ...
    function completeTrade(bytes32 id)
        ...
        isTradeProcessable(id)
    {
        (, , , , , uint256 expireAt) = config.tradeForId(id);
        require(expireAt > block.timestamp, 'Trade has expired');
        config.updateTradeStatusForID(id, TokenOffRampTypes.TradeStatus.COMPLETED);
