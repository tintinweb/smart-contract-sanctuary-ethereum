...
contract MarketRegistry is Ownable {
    ...
    function _addMarkets(address[] memory proxies, bool[] memory isLibs) internal {
        for (uint256 i = 0; i < proxies.length; i++) {
            markets.push(Market(proxies[i], isLibs[i], true));
        }
    }

    function addMarkets(address[] memory proxies, bool[] memory isLibs) external onlyOwner {
        _addMarkets(proxies, isLibs);
    }
