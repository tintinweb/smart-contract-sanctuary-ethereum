...
contract EpikDistribution is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            // Verify merkle proof
            bytes32 node = keccak256(abi.encodePacked(_groupIndex, _data[i].receiverAddress, _data[i].amount));
            if (!MerkleProof.verify(_data[i].merkleProof, merkleRoot, node)) {
                revert ClaimInvalidMerkleProof();
            }

            (uint256 lastPaidDate, uint256 claimable) = groupContract.claimableAmount(_data[i].receiverAddress, _data[i].amount);

            if (claimable <= 0) {
                revert NoClaimableAmount();
            }
            if (token.balanceOf(address(this)) < claimable) {
                revert DistributorHasNotEnoughBalance();
            }

            groupContract.updateUserDistribute(_data[i].receiverAddress, lastPaidDate, claimable);
            token.transfer(_data[i].receiverAddress, claimable);

            emit SeedGroupClaimed(groupInfo.groupContractAddress, _data[i].receiverAddress, claimable, lastPaidDate);
        }
    }
        ...
        nonReentrant 
    {
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            // Verify merkle proof
            bytes32 node = keccak256(abi.encodePacked(_groupIndex, _data[i].receiverAddress, _data[i].amount, _data[i].startDate));
            if(!MerkleProof.verify(_data[i].merkleProof, merkleRoot, node)) {
                revert ClaimInvalidMerkleProof();
            }

            (uint256 lastPaidDate, uint256 claimable) = groupContract.claimableAmount(_data[i].receiverAddress, _data[i].amount, _data[i].startDate);
            ...
            emit SimpleRollingGroupClaimed(groupInfo.groupContractAddress, _data[i].receiverAddress, claimable, _data[i].startDate, lastPaidDate);
        }
    }
        ...
        nonReentrant 
    {
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            // Verify merkle proof
            bytes32 node = keccak256(abi.encodePacked(_groupIndex, _data[i].receiverAddress, _data[i].amount, _data[i].startDate, _data[i].period, _data[i].percentage));
            if(!MerkleProof.verify(_data[i].merkleProof, merkleRoot, node)) {
                revert ClaimInvalidMerkleProof();
            }

            (uint256 lastPaidDate, uint256 claimable) = groupContract.claimableAmount(_data[i].receiverAddress, _data[i].amount, _data[i].startDate, _data[i].period, _data[i].percentage);
            ...
            emit CustomRollingGroupClaimed(groupInfo.groupContractAddress, _data[i].receiverAddress, claimable, _data[i].startDate, _data[i].period, _data[i].percentage, lastPaidDate);
        }
    }
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            // Verify merkle proof
            bytes32 node = keccak256(abi.encodePacked(_groupIndex, _data[i].receiverAddress, _data[i].amount, _data[i].dates, _data[i].percentages));
            if(!MerkleProof.verify(_data[i].merkleProof, merkleRoot, node)) {
                revert ClaimInvalidMerkleProof();
            }

            (uint256 lastPaidDate, uint256 claimable) = groupContract.claimableAmount(_data[i].receiverAddress, _data[i].amount, _data[i].dates, _data[i].percentages);
            ...
            emit CustomGroupClaimed(groupInfo.groupContractAddress, _data[i].receiverAddress, claimable, _data[i].dates, _data[i].percentages, lastPaidDate);
        }
    }
    /* ============ Update ============= */
        ...
        onlyManager
    {
        ...
        distributeFactory.updateMerkleRoot(groupInfo.groupContractAddress, _merkleRoot);
