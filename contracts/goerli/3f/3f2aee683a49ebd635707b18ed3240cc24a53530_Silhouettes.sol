...
contract Silhouettes is ERC721Enumerable, Ownable, DefaultOperatorFilterer
{
    ...
    uint256 public WLPRICE = 30000000000000000; //0.03 eth
    uint256 public PUBLICPRICE = 35000000000000000; //0.035 eth
    ...
    constructor() ERC721("Silhouettes", "SLHT") {}
    ...
    function WLmint(uint256 amount, bytes32[] memory proof) external payable
    {
        ...
        require(amount > 0 && amount <= 1, "Max 1 NFTs per transaction");
...
function SLmint(uint256 amount, bytes32[] memory proof) external payable
    {
        require(!whitelist || verify(proof), "Address not whitelisted");
        require(msg.sender == tx.origin, "No transaction from smart contracts!");
        require(preSaleIsActive || saleIsActive, "Sale must be active to mint");
    ...
    function Publicmint(uint256 amount) external payable
    {
        ...
        require(amount > 0 && amount <= 2, "Max 2 NFTs per transaction");
        require(totalSupply() + amount <= MAX_TOKENS - (NUMBER_RESERVED_TOKENS - reservedTokensMinted), "Purchase would exceed max supply");
        require(msg.value >= PUBLICPRICE * amount, "Not enough ETH for transaction");
        require(addressMintedBalance[msg.sender] + amount <= perAddressLimit, "Max NFT per address exceeded");
    ...
    function setRootWL(bytes32 _root) external onlyOwner
    {
        root = _root;
    }
     function setRootSL(bytes32 _root) external onlyOwner
    {
        root = _root;
    }
