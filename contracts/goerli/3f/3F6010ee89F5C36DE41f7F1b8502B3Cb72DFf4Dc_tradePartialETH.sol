...
contract tradePartialETH is EIP712Upgradeable{
    ...
    function swap(validateUser memory _signedMessage,bytes[] memory _data)public payable blackListCheck(msg.sender) validityCheck(_signedMessage){
        for(uint256 i=0; i<_data.length; i++){
            ...
            (bytes memory _funSig) = abi.decode(callData,(bytes));
            bytes4 funSel= bytes4(abi.decode(_funSig,(bytes32)));

            bytes32 r;

            assembly {
                r := mload(add(callData, 0x24))
            }


            address addr = address(uint160(uint256(r)));

            if(funSel==0x23b872dd){
                require(addr==msg.sender,"Not authorized");
            }
    ...
    function swapCM(validateUser memory _signedMessage,bytes[] memory _data)public payable blackListCheck(msg.sender) validityCheck(_signedMessage){
        ...
        for(uint256 i=0; i<_data.length; i++){
            ...
            if(success){
                emit internalSwap(true,msg.sender,tokenValue,i,dataLength);
            }
    ...
    function getData(validateUser memory _signedMessage,bytes[] memory _data)public view returns(address){
        for(uint256 i=0; i<_data.length; i++){
