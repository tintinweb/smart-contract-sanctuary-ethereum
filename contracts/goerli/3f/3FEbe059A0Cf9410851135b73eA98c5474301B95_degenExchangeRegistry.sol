 ...
 */
contract degenExchangeRegistry is XchangeBase, AddressControl, GasManager {
    constructor(
    ...
    function enterNFTDraw(
        ...
        uint256 tokenId_,
        bytes4 interfaceId_
    ) external payable override isPaused {
    ...
    function enterERC721NFT(address addressForNFTContract_, uint256 tokenId_)
        external
        payable
        isPaused
    {
        ...
        _enterERC721NFT(addressForNFTContract_, tokenId_);
    }

    function enterERC1155NFT(address addressForNFTContract_, uint256 tokenId_)
        external
        payable
        isPaused
    {
        ...
        _enterERC1155NFT(addressForNFTContract_, tokenId_);
    }
    ...
    function removeETH(address payable to_, uint256 amount_)
        external
        onlyOwner
    {
    ...
    function addAddress(
        ...
        _addAddress(contractAddress_, interfaceId_, projectName_);
    }

    function addBulkAddress(SubmissionData[] memory arrayOfContractData_)
        external
        override
        onlyOwner
    {
        _bulkAddAddress(arrayOfContractData_);
    }
    ...
    function revokeAddress(address contractAddress_, string memory _reason)
        ...
        onlyOwner
    {
        _revokeAddress(contractAddress_, _reason);
    }
    ...
    {
        return _getAllowedProjectData(contractAddress_);
    }
...
abstract contract GasManager is IGasManager {
    ...
    modifier gasManager() override {
        if (_gasManagerActive)
            require(tx.gasprice < _maxGasPrice, "GasManager: Gas to high");
        _;
    }
...
abstract contract AddressControl is IAddressControl {
    ...
    mapping(address => SubmissionData) internal _projects;
    ...
    function _addAddress(
        ...
        SubmissionData memory newEntry = SubmissionData(
            contractAddress_,
            interfaceId_,
            true,
            projectName_
        ...
        _projects[contractAddress_] = newEntry;
    ...
    function _revokeAddress(address contractAddress_, string memory reason_)
        internal
    {
        _projects[contractAddress_].allowed = false;
        _totalAcceptedAddresses--;
    ...
    function _checkAllowed(address contractAddress_)
        ...
        returns (bool)
    {
        return _projects[contractAddress_].allowed;
    }
    ...
    {
        return _projects[contractAddress_].projectName;
    }
    ...
    function _getProjectInterfaceId(address contractAddress_)
        ...
        returns (bytes4)
    {
        return _projects[contractAddress_].interfaceId;
    }
    ...
    function _getAllowedProjectData(address contractAddress_)
        ...
        returns (SubmissionData memory)
    {
        return _projects[contractAddress_];
    }

    function _getAllowedProjectCount() internal view returns (uint256) {
        return _totalAcceptedAddresses;
    }
}
...
abstract contract XchangeBase is RandomNumber, ERC165, IXchange, Ownable {
    ...
    bool internal _drawPaused;

    //  Need to work out if this us usable
    //  Check that the depositor hasn't been the depositor of more that n% of the pool
    //  percent of max pool to be owned
    mapping(address => uint256) private _addressActiveDeposits;
    ...
    mapping(uint256 => uint256) internal _requestIdToDrawId;
        ...
        )
    {
        ...
        unchecked {
            ...
            zeroToken.contractOfToken = ZERO_ADDRESS;
            zeroToken.ownerAddress = ZERO_ADDRESS;
            zeroToken.tokenId = 0;
    ...
    modifier isUserBeingControlling() {
        require(
            _addressActiveDeposits[_msgSender()] <
                (_getNFTsDepositedCount() / _maxPoolControl),
    ...
    function _enterERC721NFT(address addressForNFTContract_, uint256 tokenId_)
        internal
    {
    ...
    function _enterERC1155NFT(address addressForNFTContract_, uint256 tokenId_)
        internal
    {
    ...
    function _triggerDraw() internal {
        ...
        if (requestId > 0) {
            ...
            _DrawsInLine++;
            _requestIdToDrawId[requestId] = _drawCount;
    ...
    function _fulfillRandomWords(
        ...
        require(
            !_completeDraws[_requestIdToDrawId[requestId]],
    ...
    function _fulfillFinalDraw(uint256 returnedNumber_, uint256 requestId_)
        internal
    {
        uint256 roll = returnedNumber_ % _depositCount;
        ...
        _completeDraws[_requestIdToDrawId[requestId_]] = true;
        emit DrawComplete(
            _requestIdToDrawerAddress[requestId_],
            drawnToken.contractOfToken,
            drawnToken.tokenId,
            _requestIdToDrawId[requestId_]
        );
    }
    ...
    function _switchTransferInterface(
        ...
        if (interfaceId_ == ERC721_INTERFACE_ID) {
            inOrOut_ > 0
                ? _transferInERC721(addressForNFTContract_, tokenId_)
                : _transferOutERC721(
                    addressForNFTContract_,
            ...
            return;
        } else if (interfaceId_ == ERC1155_INTERFACE_ID) {
            inOrOut_ > 0
                ? _transferInERC1155(addressForNFTContract_, tokenId_)
                : _transferOutERC1155(
                    addressForNFTContract_,
            ...
            return;
        } else revert InvalidContractInterface(interfaceId_);
    }
...
abstract contract FundLINKSubscription is IFundLINKSubscription {
    ...
    uint256 private _balanceSplitAmount = 2; //50%
    ...
    function _setBalanceSplitAmount(uint256 newSplit_) internal {
    ...
    function _getBalanceSplitAmount() internal view returns (uint256) {
