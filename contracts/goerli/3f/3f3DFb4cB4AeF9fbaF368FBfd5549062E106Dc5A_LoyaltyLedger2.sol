...
/// @title Passage Loyalty Ledger v2
/// @notice Loyalty Ledger v2 ERC-1155 Token

contract LoyaltyLedger2 is
    ...
    ILoyaltyLedger2
{
    ...
    I1155BeforeTransfersModule public beforeTransfersModule;

    // ---- modifiers ----

    modifier onlyAuthorizedUpgrader() {
        if (isManaged()) {
            require(address(passageRegistry) == _msgSender(), "T1");
        } else {
    ...
    function initialize(
        ...
        passageRegistry = IPassageRegistry2(_msgSender());
        ...
        _setupRoles(_creator, _msgSender());
        emit LoyaltyLedgerInitialized(_creator, address(this), _royaltyWallet, _royaltyBasisPoints);
    }
    ...
    /// @param mmIndex uint256 Desired index of minting module
    /// @param tokenIds uint256[] Held token ID(s) of tokens to claim/mint for
    /// @param claimAmounts uint256[] Amount of tokens to claim for each held token
    ...
    function claim(
        ...
        require(address(tokens[id].mintingModules[mmIndex]) != address(0), "T22");
        uint256 amount = IMintingModule(tokens[id].mintingModules[mmIndex]).canMint(
            _msgSender(),
            msg.value,
            tokenIds,
            claimAmounts,
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        ...
        if (address(renderModule) != address(0)) {
            return renderModule.tokenURI(_id);
        }
    ...
    function hasUpgraderRole(address _address) public view override(ILoyaltyLedger2, PassageAccess) returns (bool) {
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC1155Upgradeable, AccessControlUpgradeable, ERC2981Upgradeable, ILoyaltyLedger2)
        returns (bool)
    {
        return
            ...
            interfaceId == type(ILoyaltyLedger2).interfaceId;
    }

    /// @notice Returns Loyalty Ledger implementation version
    /// @return version number
    function loyaltyLedgerVersion() external pure virtual returns (uint256) {
        return 0;
    }
    ...
    function eject() external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    /// @param _maxSupply Max supply of tokens; specify 0 for no max supply
    /// @return ID of created token
    function createToken(string memory _name, uint256 _maxSupply) external onlyRole(MANAGER_ROLE) returns (uint256) {
        ...
        emit TokenCreated(id, _name, _maxSupply);
        return id;
    }
    ...
    function setTokenMaxSupply(uint256 _id, uint256 _maxSupply) external onlyRole(MANAGER_ROLE) {
        require(exists(_id), "T4");
        require(_maxSupply >= tokens[_id].totalMinted, "T16");
    ...
    function setRenderModule(address _contractAddress) external onlyRole(MANAGER_ROLE) {
    ...
    function setBeforeTransfersModule(address _contractAddress) external onlyRole(MANAGER_ROLE) {
        _setBeforeTransfersModule(_contractAddress);
    }

    /// @notice Allows manager to set the minting module(s) of a token
    /// @param _id ID of token to update
    ...
    function setTokenMintingModule(
        ...
        require(exists(_id), "T4");
        _setTokenMintingModule(_id, _index, _contractAddress);
    }

    /// @notice Allows manager to get the minting module(s) from a token
    /// @param _id ID of token to update
    /// @param _index index of mintingModules to get
    function getTokenMintingModule(uint256 _id, uint256 _index) external view returns (address) {
        require(exists(_id), "T4");
        return address(tokens[_id].mintingModules[_index]);
    }

    function upgradeTo(address newImplementation) external override(UUPSUpgradeable, ILoyaltyLedger2) onlyProxy {
    ...
    function upgradeToAndCall(address newImplementation, bytes memory data)
        external
        payable
        override(UUPSUpgradeable, ILoyaltyLedger2)
        onlyProxy
    {
        _authorizeUpgrade(newImplementation);
    ...
    function _setBeforeTransfersModule(address _contractAddress) internal {
        beforeTransfersModule = I1155BeforeTransfersModule(_contractAddress);
    ...
    function _setTokenMintingModule(
        uint256 _id,
        uint256 _index,
        address _contractAddress
    ) internal {
        tokens[_id].mintingModules[_index] = IMintingModule(_contractAddress);
        emit MintingModuleSet(_id, _index, _contractAddress);
    }

    function _checkMint(uint256 _id, uint256 _amount) private view {
        require(exists(_id), "T4");
        if (tokens[_id].maxSupply > 0) require(tokens[_id].totalMinted + _amount <= tokens[_id].maxSupply, "T17");
    }
    ...
    function _mint(
        ...
        tokens[_id].totalMinted += _amount;
        _mint(_to, _id, _amount, "");
    }
    ...
    function _mintBatch(
        ...
        for (uint256 i = 0; i < _ids.length; ) {
            ...
            tokens[_ids[i]].totalMinted += _amounts[i];
            unchecked {
                ++i;
            }
        }
    ...
    function _baseURI(uint256 _id) internal view returns (string memory) {
    ...
    function _beforeTokenTransfer(
        ...
        if (address(beforeTransfersModule) != address(0)) {
            beforeTransfersModule.beforeTokenTransfers(from, to, ids, amounts);
        }
