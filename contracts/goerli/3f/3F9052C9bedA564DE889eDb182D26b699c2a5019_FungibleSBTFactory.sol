...
contract TicketSeller is ReentrancyGuard, AccessControl, Ownable {
    ...
    function repayment(
        ...
        uint256 id,
        uint256 maxIndex
    ) external onlyRole(ACCOUNTANT) notCompleteAccount(collection, id) {
        _paused[collection].setTo(id, true);

        EnumerableSalesSet.Sales[] memory sales = _salesSet[collection][id].values();
    ...
    TokenUriSupplier
{
    ...
    constructor(
        ...
        contractURI = string(
            abi.encodePacked(
                "https://storage.googleapis.com/ticket-nft-dev/",
