...
contract AuctionTreasury is Initializable, OwnableUpgradeable {
    ...
    uint256 public  RATIO_PRECISION;
...
//    }
    ...
    function transferLGO(address from, address _for, uint256 _amount) external {
        require(msg.sender == LGOAuctionFactory, "only LGOAuctionFactory");
        console.log("transfer from : %s, %s", _for, _amount);
//        LGO.safeTransfer( _for, _amount);
        console.log("LGOGranted: %s, %s, %s", _for, _amount, msg.sender);
        LGO.transferFrom(from , _for, _amount);
        console.log("LGOGranted: %s, %s", _for, _amount);
