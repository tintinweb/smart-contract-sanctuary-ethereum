...
contract SurvivorSeries2 is CometDrop, ReentrancyGuard {
    ...
    /// @notice The Crossmint admint for credit card processing.
    address private _crossmintAdmin;

    /// @notice The partner contract address.
    address private _partnerContractAddress;
    ...
    function setPartnerContractAddress(
        ...
        _partnerContractAddress = contractAddress;
     ...
     */
    function setCrossmintAdministrator(address newWallet) external onlyOwner {
        if (newWallet == address(0)) {
            revert NewAdministratorIsZeroAddress();
        }

        _crossmintAdmin = newWallet;

        emit CrossmintAdministratorUpdated(newWallet);
    }

    function setPhases(PhaseConfig[] memory phaseConfigs) external onlyOwner {
        for (uint256 i = 0; i < phaseConfigs.length; i++) {
            PhaseConfig memory config = phaseConfigs[i];

            _phases[config.phaseIndex] = config;
            _phaseArray.push(config.phaseIndex);
        }
    }

    function getPartnerBalance() public view returns (uint256) {
        address sender = _msgSender();
        ERC721 partnerContract = ERC721(_partnerContractAddress);

        return partnerContract.balanceOf(sender);
    }
    ...
    // function getPhases() public pure returns (PhaseConfig[] memory) {
    //     uint256[] memory arr = _phaseArray;
    //     uint256 n = arr.length;
    //     uint256 temp;
    //     PhaseConfig[] configs = new PhaseConfig[];
    ...
    function mintAccessPass(uint256 quantity) external payable nonReentrant {
        ...
        uint256 partnerBalance = partnerContract.balanceOf(sender);
        ...
        _checkPayment(quantity, config.price);

        // Safe mint checks total supply.
        _internalMint(sender, quantity);

        // emit event
        emit AccessMint(sender, _partnerContractAddress, quantity, msg.value);
    ...
    function mint(uint256 quantity) external payable nonReentrant {
        ...
        SafeTransferLib.safeTransferETH(_beneficiary, msg.value);
    ...
    function mintTo(
        ...
        if (config.isPublic) {
            ...
            emit AccessMint(to, _partnerContractAddress, quantity, msg.value);
        }
    }
}
