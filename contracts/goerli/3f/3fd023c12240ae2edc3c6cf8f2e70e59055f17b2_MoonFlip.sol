...
contract MoonFlipVault {
    ...
    function withdraw() public {
        require(playerOwner == msg.sender, "must be the owner to call this function.");
...
contract MoonFlip is Ownable, ReentrancyGuard {
    ...
    function clearPendingCommit(address addr, uint256 gameId) external onlyOwner {
    ...
    function setBetRange(uint256 min, uint256 max) external onlyOwner {
        minBet = min;
        maxBet = max;
    }
    ...
    modifier gameChecks() {
        ...
        require(!msg.sender.isContract() && msg.sender == tx.origin, "no contracts...");
        ...
        require(
            msg.value >= checkForFee(minBet) &&
                msg.value <= checkForFee(maxBet),
    ...
    function flipCoin(CoinSide guess) public payable gameChecks nonReentrant {
        require(_pendingCommits[msg.sender][_gameNonces[msg.sender]].blockNumber != block.number, "cannot play two games in the same block."  );
        ...
        (bool ok, ) = payable(houseAddress).call{value: (address(this).balance  >= liquidityThreshold ? msg.value : fee )  }("");
        require(ok, "failed to send payment");
    }
