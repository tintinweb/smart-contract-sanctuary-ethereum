...
contract MintingManager is ERC2771Context, MinterRole, Blocklist, Pausable, IMintingManager {
    ...
    function _mintSLD(
        ...
        if (_useCNS(tld)) {
            cnsMintingController.mintSLDWithResolver(address(auction), label, address(cnsResolver));
        } else {
            unsRegistry.mint(address(auction), tokenId, _uri(tld, label));
        }

        auction.setOnAuction(payable(to), tokenId);
    }
