    ...
    OwnableUpgradeable
{
    ...
    modifier isValidSign(bytes calldata sig, uint256 index) {
        bytes32 dataHash = keccak256(abi.encodePacked(msg.sender, stagesID, index));
    ...
    function _verifySig(bytes32 ethSigHash, bytes calldata sig) internal view returns (bool) {
    ...
    function claimable(
        ...
        bytes32 dataHash = keccak256(abi.encodePacked(account, stagesID, index));
        bytes32 ethSigHash = dataHash.toEthSignedMessageHash();
    ...
    function setSigner(address value) external onlyTimelock {
        signer = value;
    }
    ...
    function mintForGift(address to, bytes calldata sig, uint256 index)
        external
        payable
        isValidSign(sig, index)
        nonReentrant
        whenNotPaused
    {
        require(msg.value == price, "Invalid price");
