...
// import '../libraries/LimitOrders.sol';
import '../libraries/Orders.sol';
...
interface ITwapLimitOrder {
    ...
    event MaxGasLimitSet(uint256 maxGasLimit);
    event OrderExecuteFailed(uint256 indexed orderId, string reason);
    event BuyLimitOrderSubmitted(uint256 indexed orderId, uint256 price);
    event SellLimitOrderSubmitted(uint256 indexed orderId, uint256 price);
    event OrderCancelled(uint256 indexed orderId, uint256 price);
    event OrderRefund(uint256 indexed orderid);
    event BuyLimitOrderEnqueued(uint256 indexed orderId, uint32 expiration, uint32 twapInterval, uint256 price);
    event SellLimitOrderEnqueued(uint256 indexed orderId, uint32 expiration, uint32 twapInterval, uint256 price);
    ...
    struct Data {
        mapping(uint256 => StoredOrder) orders;
        address delay;
        address factory;
        uint256 maxGasLimit;
    ...
    function getTwapPrice(address _pairAddress, uint32 _twapInterval) external view returns (uint256);
    ...
    function execute(uint256[] calldata orderId) external;
    ...
    function shouldExecute(uint256 orderId) external view returns (bool);
}
...
contract TwapLimitOrder is ITwapLimitOrder {
    ...
    uint256 private constant MAX_GAS_LIMIT = 5_000_000;
    uint256 private constant MAX_GAS_LIMIT_UPPER_BOUND = 10_000_000;
    uint256 private constant GAS_MULTIPLIER = 2;

    uint8 private locked;
    address public override owner;

    Data internal orders;
    ...
    constructor(
        ...
        require(_delay != address(0) && _factory != address(0) && _weth != address(0), 'LO02');
        owner = msg.sender;
        tokenShares.weth = _weth;
        isBot[_bot] = true;
        orders.delay = _delay;
        orders.factory = _factory;
        orders.gasMultiplier = GAS_MULTIPLIER;
        orders.maxGasLimit = MAX_GAS_LIMIT;
        emit OwnerSet(msg.sender);
        emit BotSet(_bot, true);
        emit DelaySet(_delay);
        emit FactorySet(_factory);
        emit MaxGasLimitSet(MAX_GAS_LIMIT);
    }

    modifier lock() {
        require(locked == 0, 'LO06');
        locked = 1;
        _;
        locked = 0;
    }
    ...
    function delay() external view override returns (address) {
        return orders.delay;
    }

    function setDelay(address _delay) external override {
        require(msg.sender == owner, 'LO00');
        require(_delay != orders.delay, 'LO01');
        require(_delay != address(0), 'LO02');
        orders.delay = _delay;
        emit DelaySet(_delay);
    }
    ...
    function setFactory(address _factory) external override {
        require(msg.sender == owner, 'LO00');
        require(_factory != orders.factory, 'LO01');
        require(_factory != address(0), 'LO02');
        orders.factory = _factory;
        emit FactorySet(_factory);
    }
    ...
    function setOwner(address _owner) external override {
        require(msg.sender == owner, 'LO00');
        require(_owner != owner, 'LO01');
        require(_owner != address(0), 'LO02');
        owner = _owner;
    ...
    function setBot(address _bot, bool _isBot) external override {
        require(msg.sender == owner, 'LO00');
        require(_isBot != isBot[_bot], 'LO01');
        isBot[_bot] = _isBot;
    ...
    function setPriceTolerance(address _pair, uint32 _tolerance) external override {
        require(msg.sender == owner, 'LO00');
        require(_tolerance != priceTolerance[_pair], 'LO01');
        require(_tolerance < PRICE_TOLERANCE_PRECISION, 'LO54');
    ...
    function gasPrice() public view override returns (uint256) {
        return ITwapDelay(orders.delay).gasPrice();
    }
    ...
    function setMaxGasLimit(uint256 _maxGasLimit) external override {
        require(msg.sender == owner, 'LO00');
        require(_maxGasLimit != orders.maxGasLimit, 'LO01');
        require(_maxGasLimit <= MAX_GAS_LIMIT_UPPER_BOUND, 'LO2B');
        orders.maxGasLimit = _maxGasLimit;
        emit MaxGasLimitSet(_maxGasLimit);
    }

    function isOrderExpired(uint256 orderId) public view override returns (bool) {
        return orders.orders[orderId].expiration < block.timestamp;
    }

    function getTwapPrice(address pairAddress, uint32 twapInterval) public view virtual override returns (uint256) {
    ...
    function sellWithExpiration(
        ...
        uint32 twapInterval,
        uint32 expiration
    ) public payable override lock returns (uint256 orderId) {
        require(expiration <= EXPIRATION_UPPER_LIMIT && expiration >= EXPIRATION_LOWER_LIMIT, 'LO03');
        require(twapInterval > 0, 'LO56');
        orderId = sellLimitOrder(
            sellParams,
            uint256(expiration).add(block.timestamp).toUint32(),
            twapInterval,
            price,
            msg.sender
        );
    ...
    function buyWithExpiration(
        ...
        require(expiration <= EXPIRATION_UPPER_LIMIT && expiration >= EXPIRATION_LOWER_LIMIT, 'LO02');
        require(twapInterval > 0, 'LO56');
        orderId = buyLimitOrder(
            buyParams,
            uint256(expiration).add(block.timestamp).toUint32(),
            twapInterval,
            price,
    ...
    function cancelOrder(uint256 orderId) external override lock {
        require(
            orders.orders[orderId].status != LimitOrderStatus.NonExistent && !isOrderFailed(orderId),
            'LO41'
        );
        require(isBot[msg.sender] || orders.orders[orderId].submitter == msg.sender, 'LO00');
        if (orders.orders[orderId].status == LimitOrderStatus.Waiting) {
            performRefund(orderId, true);
            forgetOrder(orderId);
        } else {
            ITwapDelay(orders.delay).cancelOrder(orders.orders[orderId].delayOrderId);
            forgetOrder(orderId);
        }
    }

    function getPriceWithTolerance(uint256 price, address pair) private view returns (uint256) {
    ...
    function testBuy( Orders.BuyParams calldata buyParams,
        uint256 price,
        uint32 twapInterval,
        uint32 expiration) external returns(uint256 orderId) {
        emit BuyLimitOrderEnqueued(block.number, expiration, twapInterval, price);
        return  block.number;
    }

    function testExecuteBuy(uint256[] calldata orderIds) external {
        bytes memory a = "a";
        emit OrderExecuted(block.number, true, a, 1, 1);
    }


    function shouldExecute(uint256 orderId) public override view returns (bool) {
        StoredOrder memory order = orders.orders[orderId];
        (address pairAddress, bool inverted) = getPair(order.tokenIn, order.tokenOut);
        uint256 currentPrice = getTwapPrice(pairAddress, order.twapInterval);
        bool isSellOrder = order.orderType == LimitOrderType.Sell;
        if (inverted == isSellOrder) {
            if (currentPrice <= getPriceWithTolerance(order.price, pairAddress)) {
                return true;
            }
        } else if (getPriceWithTolerance(currentPrice, pairAddress) >= order.price) {
            return true;
        }
        return false;
    }

    function _execute(uint256 orderId) external {
        require(msg.sender == address(this), 'LO00');
        require(!isOrderExpired(orderId), 'LO07');
        _executeOrder(orderId);
    }

    function executeOrder(uint256 orderId) internal {
        uint256 gasStart = gasleft();
        if (!shouldExecute(orderId)) {
            return;
        }
        StoredOrder memory order = orders.orders[orderId];
        (bool executionSuccess, bytes memory data) = address(this).call{ gas: order.gasLimit }(
            abi.encodeWithSelector(this._execute.selector, orderId)
        );
        if (!executionSuccess) {
            if (!refundToken(order.tokenIn, order.to, order.shares, order.wrapUnwrap)) {
                markOrderRefundFail(orderId);
            } else {
                markOrderFail(orderId);
            }
        }
        (uint256 gasUsed, uint256 leftOver) = refund(
            executionSuccess,
            order.gasLimit,
            order.gasPrice,
            gasStart,
        ...
        emit OrderExecuted(orderId, executionSuccess, data, gasUsed, leftOver);
    }

    function execute(uint256[] calldata orderIds) external override lock {
        require(isBot[msg.sender], 'LO00');
    ...
    function retryRefund(uint256 orderId) external override lock {
        if (orders.orders[orderId].status == LimitOrderStatus.RefundFail) {
            performRefund(orderId, false);
        } else if (orders.orders[orderId].status == LimitOrderStatus.Submitted) {
            ITwapDelay(orders.delay).retryRefund(orders.orders[orderId].delayOrderId);
        } else {
            revert('LO14');
        }
    }
    ...
    function getOrderStatus(uint256 orderId) external view override returns (LimitOrderStatus) {
        if (orders.orders[orderId].status == LimitOrderStatus.Submitted) {
            Orders.OrderStatus status = ITwapDelay(orders.delay).getOrderStatus(orders.orders[orderId].delayOrderId);
            return LimitOrderStatus(uint256(status));
        }
        return orders.orders[orderId].status;
    }
    ...
    function getDelayOrderId(uint256 orderId) external view override returns (uint256) {
        return orders.orders[orderId].delayOrderId;
    }

    function forgetOrder(uint256 orderId) internal {
    ...
    function refund(
        ...
        uint256 feeCollected = executionSuccess
            ? gasLimit.mul(gasPriceInOrder)
            : gasLimit.mul(gasPriceInOrder).mul(orders.gasMultiplier);

        gasUsed = gasStart.sub(gasleft()).add(Orders.REFUND_BASE_COST);
        uint256 actualRefund = Math.min(feeCollected, gasUsed.mul(ITwapDelay(orders.delay).gasPrice()));
        leftOver = feeCollected.sub(actualRefund);
        require(refundEth(msg.sender, actualRefund), 'LO40');
        refundEth(payable(to), leftOver);
    }

    function performRefund(uint256 orderId, bool shouldRefundEth) internal {
        StoredOrder memory order = orders.orders[orderId];

        require(refundToken(order.tokenIn, order.to, order.shares, order.wrapUnwrap), 'LO14');
        if (shouldRefundEth) {
            uint256 value = order.gasPrice.mul(order.gasLimit).mul(orders.gasMultiplier);
            require(refundEth(payable(order.to), value), 'LO40');
        }
    }
    ...
    function refundEth(address payable to, uint256 value) internal returns (bool success) {
        ...
        success = TransferHelper.transferETH(to, value, ITwapDelay(orders.delay).getTransferGasCost(address(0)));
    ...
    function refundToken(
        ...
        (bool success, bytes memory data) = address(this).call{
            gas: ITwapDelay(orders.delay).getTransferGasCost(token)
    ...
    function _refundToken(
        ...
        if (token == tokenShares.weth && unwrap) {
            ...
            TransferHelper.safeTransferETH(to, amount, ITwapDelay(orders.delay).getTransferGasCost(address(0)));
        } else {
    ...
    function buyLimitOrder(
        ...
        uint256 tokenTransferCost = ITwapDelay(orders.delay).getTransferGasCost(buyParams.tokenIn);
        require(tokenTransferCost != 0, 'LO0F');
        require(buyParams.amountOut != 0, 'LO23');
        checkOrderParams(
            buyParams.to,
            buyParams.gasLimit,
            buyParams.submitDeadline,
            Orders.ORDER_BASE_COST.add(tokenTransferCost * 2)
        );
        uint256 value = msg.value;
        // allocate gas refund
        if (buyParams.tokenIn == tokenShares.weth && buyParams.wrapUnwrap) {
            value = value.sub(buyParams.amountInMax, 'LO1E');
        }
        uint256 _gasPrice = gasPrice();
        allocateGasRefund(value, buyParams.gasLimit, _gasPrice);
        uint256 shares = tokenShares.amountToShares(buyParams.tokenIn, buyParams.amountInMax, buyParams.wrapUnwrap);
        StoredOrder storage buyOrder = orders.orders[orders.newestOrderId + 1];
        ...
        buyOrder.gasPrice = _gasPrice;
        buyOrder.amountOut = buyParams.amountOut.toUint112();
        buyOrder.amountIn = buyParams.amountInMax.toUint112();

        buyOrder.twapInterval = twapInterval;
        buyOrder.shares = shares.toUint112();

        buyOrder.inverse = buyParams.tokenIn > buyParams.tokenOut;
        buyOrder.wrapUnwrap = buyParams.wrapUnwrap;
        buyOrder.to = buyParams.to;
        buyOrder.tokenIn = buyParams.tokenIn;
        buyOrder.tokenOut = buyParams.tokenOut;
        buyOrder.price = price;
        buyOrder.submitter = submitter;

        enqueueOrder(buyOrder);
        emit BuyLimitOrderEnqueued(orders.newestOrderId, expiration, twapInterval,  price);
        return orders.newestOrderId;
    }
    ...
    function sellLimitOrder(
        ...
        uint256 tokenTransferCost = ITwapDelay(orders.delay).getTransferGasCost(sellParams.tokenIn);
        require(tokenTransferCost != 0, 'LO0F');
        require(sellParams.amountIn != 0, 'LO24');
        checkOrderParams(
            sellParams.to,
            sellParams.gasLimit,
            sellParams.submitDeadline,
        ...
        // allocate gas refund
        if (sellParams.tokenIn == tokenShares.weth && sellParams.wrapUnwrap) {
            value = value.sub(sellParams.amountIn, 'LO1E');
        }
        uint256 _gasPrice = gasPrice();
        allocateGasRefund(value, sellParams.gasLimit, _gasPrice);
        uint256 shares = tokenShares.amountToShares(sellParams.tokenIn, sellParams.amountIn, sellParams.wrapUnwrap);
        StoredOrder storage sellOrder = orders.orders[orders.newestOrderId + 1];
        ...
        sellOrder.gasPrice = _gasPrice;
        sellOrder.amountOut = sellParams.amountOutMin.toUint112();
        sellOrder.amountIn = sellParams.amountIn.toUint112();

        sellOrder.twapInterval = twapInterval;
        sellOrder.shares = shares.toUint112();

        sellOrder.inverse = sellParams.tokenIn > sellParams.tokenOut;
        sellOrder.wrapUnwrap = sellParams.wrapUnwrap;
        sellOrder.to = sellParams.to;
        sellOrder.tokenIn = sellParams.tokenIn;
        sellOrder.tokenOut = sellParams.tokenOut;
        sellOrder.price = price;
        sellOrder.submitter = submitter;

        enqueueOrder(sellOrder);
        emit SellLimitOrderEnqueued(orders.newestOrderId, expiration, twapInterval, price);
        return orders.newestOrderId;
    }
    ...
    function allocateGasRefund(
        ...
        futureFee = _gasPrice.mul(gasLimit).mul(orders.gasMultiplier);
        require(value >= futureFee, 'LO1E');
        if (value > futureFee) {
            TransferHelper.safeTransferETH(
                msg.sender,
                value.sub(futureFee),
                ITwapDelay(orders.delay).getTransferGasCost(address(0))
            );
        }
    }

    function markOrderFail(uint256 orderId) internal {
        orders.orders[orderId].status = LimitOrderStatus.Fail;
    }

    function markOrderRefundFail(uint256 orderId) internal {
        orders.orders[orderId].status = LimitOrderStatus.RefundFail;
    }

    function isOrderFailed(uint256 orderId) internal view returns (bool) {
        return
            orders.orders[orderId].status == LimitOrderStatus.RefundFail ||
            orders.orders[orderId].status == LimitOrderStatus.Fail;
    }
    ...
    function _executeOrder(
        ...
        StoredOrder storage order = orders.orders[orderId];
        require(order.status == LimitOrderStatus.Waiting, 'LO21');
        uint256 delayOrderId;
        uint256 value = order.gasPrice.mul(order.gasLimit);
        uint256 amountIn = tokenShares.sharesToAmount(order.tokenIn, order.shares, 0, order.to);
        if (ITwapERC20(order.tokenIn).allowance(address(this), orders.delay) < amountIn) {
            TransferHelper.safeApprove(order.tokenIn, orders.delay, type(uint112).max);
        }
        if (order.orderType == LimitOrderType.Buy) {
            delayOrderId = ITwapDelay(orders.delay).buy{ value: value }(
                Orders.BuyParams(
                    order.tokenIn,
                    order.tokenOut,
                    amountIn,
                    ...
                    order.to,
                    order.gasLimit,
                    order.submitDeadline
                )
            );
            emit BuyLimitOrderSubmitted(orderId, order.price);
        } else {
            delayOrderId = ITwapDelay(orders.delay).sell{ value: value }(
                Orders.SellParams(
                    order.tokenIn,
                    order.tokenOut,
                    amountIn,
            ...
            emit SellLimitOrderSubmitted(orderId, order.price);
        }
        orders.orders[orderId].status = LimitOrderStatus.Submitted;
        orders.orders[orderId].delayOrderId = delayOrderId;
    }

    function enqueueOrder(StoredOrder memory order) internal {
        ++orders.newestOrderId;
        orders.orders[orders.newestOrderId] = order;
    }
    ...
    function checkOrderParams(
        ...
        require(submitDeadline >= block.timestamp, 'LO04');
        require(gasLimit <= orders.maxGasLimit, 'LO3E');
        require(gasLimit >= minGasLimit, 'LO3D');
        require(to != address(0), 'LO26');
    }
    ...
    function getPair(
        ...
        inverted = tokenA > tokenB;
        (address token0, address token1) = inverted ? (tokenB, tokenA) : (tokenA, tokenB);
        pair = ITwapFactory(orders.factory).getPair(token0, token1);
        require(pair != address(0), 'LO17');
    }
