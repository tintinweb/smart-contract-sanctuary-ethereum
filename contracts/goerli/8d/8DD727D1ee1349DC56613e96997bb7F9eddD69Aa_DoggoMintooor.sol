    ...
    NFTGate1155
{
    ...
    function mint(
        ...
        // Only non-keyholders must pay.
        if (!_checkNFTHolder(_msgSender())) {
...
abstract contract NFTGate1155 is Context {
    struct NFTGate {
        address erc1155;
        uint256 tokenId;
    }

    NFTGate internal gate;

    modifier onlyNFTHolder() {
        require(_checkNFTHolder(_msgSender()), "NFTGate: !holder");
        _;
    }

    function _setNFTGate(address _erc1155, uint256 _tokenId) internal {
        gate.erc1155 = _erc1155;
        gate.tokenId = _tokenId;
    }

    function passesNFTGate(address holder) external view returns (bool) {
        return _checkNFTHolder(holder);
    }

    function _checkNFTHolder(
        address holder
    ) internal view virtual returns (bool) {
        return
            gate.erc1155 == address(0) ||
            IERC1155(gate.erc1155).balanceOf(holder, gate.tokenId) > 0;
    }
}
...
contract UniversalPaymentReceiver is Context {
    ...
    function _configurePaymentSettings(
        ...
        acceptedCurrenciesArray = paymentSettings.acceptedCurrencies;

        for (uint16 i = 0; i < paymentSettings.acceptedCurrencies.length; i++) {
            acceptedCurrenciesMap[paymentSettings.acceptedCurrencies[i]] = true;
            poolFeesMap[paymentSettings.acceptedCurrencies[i]] = paymentSettings
                .poolFees[i];
        }
