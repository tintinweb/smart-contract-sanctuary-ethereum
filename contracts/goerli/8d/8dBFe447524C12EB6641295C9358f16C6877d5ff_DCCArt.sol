    ...
    DefaultOperatorFilterer
{
    ...
    constructor() ERC721A("Dream Cars Art Collection", "DCCArt") {
        admins[msg.sender] = true;
    }

    modifier onlyAdmin() {
        require(admins[_msgSender()], "DCCArt: Caller is not the admin");
        _;
    }
    ...
    function tokenURI(
        ...
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, tokenURIs[tokenId - 1]))
                : "";
    }
    ...
    function giftMint(
        string[] memory _artTypes,
        uint256[] memory _tokenAmounts,
        address[] memory _addresses
    ) public onlyAdmin {
        uint256 totalQuantity = 0;
        uint256 total = totalToken();
        for (uint256 i = 0; i < _artTypes.length; i++) {
            totalQuantity += _tokenAmounts[i];
            for (uint256 j = 0; j < _tokenAmounts[i]; j++) {
                tokenURIs.push(_artTypes[i]);
            }
        }
        require(total + totalQuantity <= MAX_ELEMENTS, "DCCArt: Max limit");
    ...
    function mintForAirdrop(
        ...
        tokenIdTracker = tokenIdTracker + totalQuantity;
        _safeMint(_msgSender(), totalQuantity);

        for (uint256 i = 0; i < _addresses.length; i++) {
            safeTransferFrom(_msgSender(), _addresses[i], total + 1);
            total++;
        }
    }
