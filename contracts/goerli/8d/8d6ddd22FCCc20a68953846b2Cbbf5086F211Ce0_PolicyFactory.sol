 ...
 * @title PolicyFactory
 * @author @InsureDAO
 * @notice InsureDAO's Depeg insurance factory of fungible policy
 **/
import {
    IPolicyFactory, IFungibleInitializer, IOwnership
} from "../interfaces/AllInterfaces.sol";
import "../securities/AbstractOwner.sol";

contract PolicyFactory is IPolicyFactory, AbstractOwner {
    ...
    function approveTemplate(IFungibleInitializer _template, bool _approval) override external onlyOwner {
    ...
    function createFungible(
        ...
        IFungibleInitializer _fungible = IFungibleInitializer(_createClone(address(_template)));

        _fungible.initialize(_name, _symbol, _params, _references);
        emit FungibleCreated(address(_fungible), address(_template), _name, _symbol, _params, _references);
        return address(_fungible);
    }
