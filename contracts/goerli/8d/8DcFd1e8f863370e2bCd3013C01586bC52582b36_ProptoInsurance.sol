...
contract ProptoInsurance is IProptoInsurace, Ownable {
    ...
    constructor(
        ...
        nextResizeReservesVRFIndex = IPVRF(_standardPVRF).getPresentIndex();
    }
    ...
    function addInsuranceShare(uint256 amount) public {
        ...
        require(
            nextResizeReservesVRFIndex == IPVRF(standardPVRF).getPresentIndex(),
            "Previous round reserves not resized"
        );
        IERC20(reservesContract).transfer(address(this), amount);
        incomingReservesAmountMap[nextResizeReservesVRFIndex].amount += amount;
        ...
        inoutReservesMap[msg.sender]
            .list[inoutReservesMap[msg.sender].detailNum]
            .PVRFIndex = nextResizeReservesVRFIndex;
        inoutReservesMap[msg.sender]
            .list[inoutReservesMap[msg.sender].detailNum]
            .direction = true;
    ...
    function removeInsuranceShare(uint256 amount) public {
        ...
        outgoingReservesAmountMap[nextResizeReservesVRFIndex]
            .ITokenAmount += amount;
        inoutReservesMap[msg.sender].detailNum++;
        inoutReservesMap[msg.sender]
            .list[inoutReservesMap[msg.sender].detailNum]
    ...
    function buyInsurance(
        ...
        (uint256 payment, uint8 hour) = calcBuyInsurancePaymeny(
            ppi.PVRFContract,
            ppi.PVRFIndex,
            ppi.amount,
            valuation
    ...
    function calcBuyInsurancePaymeny(
        ...
        return (payment1 + payment2 + payment3, uint8(hour));
    }

    function resizeReserves() public {
        calcVRFRound();
        if (nextResizeReservesVRFIndex == IPVRF(standardPVRF).getPresentIndex())
            return;
        ...
        IToken(ITokenContract).burn(
            outgoingReservesAmountMap[nextResizeReservesVRFIndex].ITokenAmount
        );
        ...
        incomingReservesAmountMap[nextResizeReservesVRFIndex]
            .ITokenAmount = ITokenAmount;
        IToken(ITokenContract).mint(address(this), ITokenAmount);
        totalReserves += incomingReservesAmountMap[nextResizeReservesVRFIndex]
            .amount;

        emit ResizeReserves(
            nextResizeReservesVRFIndex,
            true,
            incomingReservesAmountMap[nextResizeReservesVRFIndex].amount,
            ITokenAmount
        );
    ...
    {
        // 0 Underwriting 1 Done Settlement 2 Expired 3 Processing
        insuranceData memory id = insuranceDatas[index];
        if (id.settlement == true) {
            id.status = 1;
        } else {
            uint8 VStatus = IProptoPredict(id.proptoPredictContract)
