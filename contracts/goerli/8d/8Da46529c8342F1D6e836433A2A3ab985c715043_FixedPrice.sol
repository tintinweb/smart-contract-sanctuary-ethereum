...
contract Base is OwnableUpgradeable {
    string baseURI_;
    ...
    function tokenURI(uint256) external virtual view returns (string memory) {
        return baseURI_;
    }
}

contract Escher721 is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, AccessControlUpgradeable, ERC2981Upgradeable {
    ...
    function initialize(address _creator, address _uri, string memory _name, string memory _symbol) initializer public {
    ...
    {
        return Base(tokenUriDelegate).tokenURI(tokenId);
    }
...
contract FixedPrice is Initializable, OwnableUpgradeable, ERC1155Holder {

    event End(Sale saleInfo);
    event Buy(address indexed buyer, uint256 amount, uint256 value, Sale saleInfo);
    ...
    function price() public view returns(uint256) {
        return uint256(sale.price);
    }

    /// @notice the start time of the sale
    function startTime() public view returns(uint256) {
        return uint256(sale.startTime);
    }
    ...
    function available() public view returns(uint256) {
        return uint256(sale.finalId - sale.currentId);
    }

    /// @notice buy from a fixed price sale after the sale starts
    /// @param amount the amount of editions to buy
    function buy(uint48 amount) external payable {
        Sale memory temp = sale;
        Escher721 nft = Escher721(temp.edition);
        ...
        for (uint48 x = temp.currentId + 1; x <= newId; x++) {
        ...
        if (newId == temp.finalId) {
            temp.feeReceiver.transfer(address(this).balance / 20);
            _end();
        }
    }
...
contract FixedPriceFactory is CloneFactory, Ownable {

    event NewFixedPriceContract(address indexed creator, address indexed edition, uint256 price, uint256 startTime, uint256 amount, address indexed saleContract);
    ...
    constructor() Ownable() {
        ...
        uint96 y = type(uint96).max;
        address payable z = payable(address(0));
        FixedPrice.Sale memory init = FixedPrice.Sale(x, x, z, y, z, y, z);
        implementation = address(new FixedPrice());
        FixedPrice(implementation).initialize(init);
    ...
    function createFixedSale(FixedPrice.Sale calldata sale) external returns(address clone){
        require(Escher721(sale.edition).hasRole(Escher721(sale.edition).DEFAULT_ADMIN_ROLE(), msg.sender), "NOT AUTHORIZED");
        require(sale.feeReceiver == feeReceiver, "INVALID FEE RECEIVER");
        require(sale.startTime >= block.timestamp, "START TIME IN PAST");
        require(sale.finalId > sale.currentId, "FINAL ID BEFORE CURRENT");

        clone = createClone(implementation);
        FixedPrice(clone).initialize(sale);

        emit NewFixedPriceContract(msg.sender, sale.edition, sale.price, sale.startTime, sale.finalId - sale.currentId, clone);
    }
