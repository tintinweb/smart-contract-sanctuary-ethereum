...
contract ERC3525MintableUpgradeable is Initializable, ContextUpgradeable, ERC3525Upgradeable {
    ...
    function mint(address mintTo_, uint256 tokenId_, uint256 slot_, uint256 value_) external virtual {
        ERC3525Upgradeable._mint(mintTo_, tokenId_, slot_, value_);
    }

    function mintValue(uint256 tokenId_, uint256 value_) external virtual {
...
contract FoxPoint is Initializable, ContextUpgradeable, ERC3525BurnableUpgradeable, OwnableUpgradeable {
    uint256[] private _allSlots;
    uint256 private constant _SLOTNULL = 0;
    ...
    function initialize(
        ...
        __FoxPoint_init(name_, symbol_, decimals_);
        __Ownable_init();
    }

    // solhint-disable-next-line
    function __FoxPoint_init(string memory name_, string memory symbol_, uint8 decimals_) internal onlyInitializing {
        __ERC3525_init_unchained(name_, symbol_, decimals_);
    }

    function mint(address mintTo_, uint256 tokenId_, uint256 slot_, uint256 value_) external override onlyOwner {
        ERC3525Upgradeable._mint(mintTo_, tokenId_, slot_, value_);
    }

    function mintValue(uint256 tokenId_, uint256 value_) external override onlyOwner {
        ERC3525Upgradeable._mintValue(tokenId_, value_);
    }

    function setNewSlot(uint256 expiration_) external onlyOwner {
        // TODO: check that expiration day should be end of month
        _allSlots.push(expiration_);
    }

    function balanceValueOf(address owner_) external view returns (uint256) {
        return _balanceValueOf(owner_, _SLOTNULL);
    }

    function balanceValueOf(address owner_, uint256 slot_) external view returns (uint256) {
        return _balanceValueOf(owner_, slot_);
    }

    function _balanceValueOf(address owner_, uint256 slot_) private view returns (uint256 balanceValue) {
        require(owner_ != address(0), "ERC3525: balance query for the zero address");
        uint256[] memory _ownedTokens = _addressData[owner_].ownedTokens;
        balanceValue = 0;
        for (uint i = 0; i <= _ownedTokens.length; i++) {
            TokenData memory token = _allTokens[_allTokensIndex[_ownedTokens[i]]];
            if (_isAvailable(token.slot)) {
                if ((slot_ == _SLOTNULL) || (token.slot == slot_)) {
    ...
    function allTokens() external view returns (TokenData[] memory) {
        return _allTokens;
    }

    function allSlots() external view returns (uint256[] memory) {
        return _allSlots;
    }

    function _isAvailable(uint256 slot_) private view returns (bool) {
        // solhint-disable-next-line
        return block.timestamp < slot_;
    }
}
