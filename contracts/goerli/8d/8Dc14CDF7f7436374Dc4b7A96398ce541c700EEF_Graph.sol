...
contract Graph is Tenderizer {
    ...
    function processUnstake() external onlyGov {
        ...
        assembly {
            let newNode := sload(NEW_NODE_SLOT)
            if gt(newNode, 0) {
                sstore(node.slot, newNode)
            }
            sstore(NEW_NODE_SLOT, 0)
        }
    }
    ...
    function _processNewStake() internal override returns (int256 rewards) {
        ...
        assembly {
            pendingMigration := sload(PENDING_MIGRATION_SLOT)
        }
...
abstract contract Tenderizer is Initializable, ITenderizer, SelfPermit {
    ...
    uint256 internal constant PENDING_MIGRATION_SLOT =
        0xd8effa4b0e415a75773d1fa7f0984b339d1594adb84907222f439e472a0836a2;

    uint256 internal constant NEW_NODE_SLOT =
        0xccc5d19dff56a8c78665c4cec7d6597093caa4b4b31acb65b41236aa61db4581;
    ...
    function _initialize(
        ...
        require(tenderToken_.initialize(_symbol, _symbol, ITotalStakedReader(address(this))));
    ...
    function unstake(uint256 _amount) external override returns (uint256) {
        ...
        require(tenderToken.burn(msg.sender, _amount));
    ...
    function migrateUnlock(address _newNode) external onlyGov returns (uint256 lockID) {
        uint256 amount = _tokensToMigrate(node);

        // Check that there's no pending migration
        uint256 pendingMigration;
        assembly {
            let reason := "PENDING_MIGRATION"
            pendingMigration := sload(PENDING_MIGRATION_SLOT)
            if gt(pendingMigration, 0) {
                revert(reason, 32)
            }
            sstore(PENDING_MIGRATION_SLOT, amount)
            sstore(NEW_NODE_SLOT, _newNode)
        }
        // set new node
    ...
    function migrateWithdraw(uint256 _unstakeLockID) external onlyGov {
        ...
        assembly {
            sstore(PENDING_MIGRATION_SLOT, 0)
        }
        _withdraw(address(this), _unstakeLockID);
    }
    ...
    function _depositHook(address _for, uint256 _amount) internal {
        ...
        // mint tenderTokens
        require(tenderToken.mint(_for, amountOut));
