...
// Author: https://github.com/SnikeZ
...
contract RewarderManager{
    ...
    IERC1155 RobotParts;
    event Claim(address indexed receiver, address indexed caller, uint hypoSessionId);
    ...
    function setServers(address server, bool value) public {
    ...
    function setRobotPartsAddress (address robotparts) public{
        require(msg.sender == owner, "You are not an owner");
    ...
    function claimLoot(transaction[] calldata _txs, bytes calldata sign) public {
        claimLootByServer (_txs, sign, msg.sender);
    }

    function claimLootByServer (transaction[] calldata _txs, bytes calldata sign, address receiver) public {
        IERC721 Hypo = IERC721(hypoContract);

        require(servers[msg.sender] == true || msg.sender == receiver, "You are not the owner of the loot");

        require(Hypo.ownerOf(_txs[0].hypoId) == receiver, "You are not an owner of hypo");
        require(areAllTxsSigned(_txs, sign), "RewarderManager: wrong signature");

        emit Claim(receiver, msg.sender, hypoNonce[_txs[0].hypoId]);

        hypoNonce[_txs[0].hypoId] += 1;
        uint _id = _txs[0].hypoId;
        uint256[] memory robotPartsAmount = new uint[](5);
        for (uint i = 0; i < _txs.length; i++){
            require(_id == _txs[i].hypoId, "Quests was done by not this hypo");
            for(uint k = 0; k < 5; k++){
                robotPartsAmount[k] += _txs[i].rewards[k];
            }
            if (_txs[i].address_ != address(0)){
                IERC721 NFT = IERC721(_txs[i].address_);
                NFT.safeTransferFrom(dropStorage, receiver, _txs[i].id, "");
            }
        }
        ...
        ids[3] = 3;
        ids[4] = 4;
        RobotParts.safeBatchTransferFrom(dropStorage, receiver, ids, robotPartsAmount, "");
    }
