...
contract OilV2 {
    ...
    function unFreeze(
        ...
        (uint256 stakedIndex, bool isOwned) = isOwnedByStaker(msg.sender, royalId_, true);

        require(isOwned, "You do not own the royal specified.");

        uint256 royalStakedTimestamp = royalStakers[msg.sender].royals[stakedIndex].stakedTimestamp;
        
        // Gather timestamp once so it can be re-used.
        uint256 currentTimestamp = block.timestamp;

        require(currentTimestamp - royalStakedTimestamp > unFreezeTimer, "You have not staked your royal long enough to unfreeze.");

        // Ensure that the caller is eligble to unfreeze based on timer. 
        require(RoyalsUnFrozen[royalId_].lastClaim + unFreezeTimer <= currentTimestamp);
        ...
        // Ensure staking doesn't occur twice.
        for (uint256 i; i < RoyalsUnFrozen[royalId_].UnfrozenHabibiz.length;) {
            uint256 tokenId = RoyalsUnFrozen[royalId_].UnfrozenHabibiz[i].tokenId;
            if (tokenId == habibiz_) revert("TokenId already unfrozen.");
            unchecked {
                ++i;
            }
        }

        // Initiate Oil Transfer and burn tokens.
    ...
    function getUnfrozen(uint256 royalId_) public view returns (uint256[] memory unFrozenIds_) {
        uint256 amount = RoyalsUnFrozen[royalId_].UnfrozenHabibiz.length;
        uint256[] memory unFrozenIds = new uint256[] (amount);
        for (uint256 i; i < amount;) {
            uint256 tokenId = RoyalsUnFrozen[royalId_].UnfrozenHabibiz[i].tokenId;
            unFrozenIds[i] = tokenId;
            unchecked {
                ++i;
            }
        }
