...
import "./IOzuGenerative.sol";

contract OzuGenerative is
  IOzuGenerative,
  Initializable,
  ...
  OwnableUpgradeable
{
  ...
  // address of the minter, only one minter allowed
  ...
  modifier onlyMinter() {
    require(msg.sender == _minter, "OzuCuts: Caller not minter");
    _;
  }

  /**
   * @dev Disable Constructor, the contract will be deployed as proxy every time
   */
  constructor() {
   ...
   * @param __symbol the symbol for the token
   * @param __baseURI the base uri used to easily fetch token metadata
   * @param __minter The address of the contract that will call the mint functions
   */
   ...
   * @param newMinter The address that will receive the minted tokens.
   * @return A boolean that indicates if the operation was successful.
   */
