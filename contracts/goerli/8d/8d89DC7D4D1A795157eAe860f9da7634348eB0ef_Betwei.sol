...
contract Betwei is VRFConsumerBaseV2 {
  ...
  mapping(uint256 => mapping(address => bool)) waitingClaimRewardGame;
  ...
  function _selectWinner(uint256 requestId, uint256[] memory randomWords) internal {
    ...
    waitingClaimRewardGame[gameIndex][winnerAddress] = true;
  }
  ...
  function withdrawGame(uint256 _gameId) external gameExists(_gameId) returns(bool) {
    ...
    require(waitingClaimRewardGame[_gameId][msg.sender], 'Player not winner');
    require(
    ...
    game.balance = 0;
    waitingClaimRewardGame[_gameId][msg.sender] = false;
    // save game
  ...
  function hasClaimReward(uint _gameId, address beneficiary) external view returns(bool) {
      return waitingClaimRewardGame[_gameId][beneficiary];
  }
