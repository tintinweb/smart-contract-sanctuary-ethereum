...
contract LaunchpadDriver is Ownable {
    ...
    function callFunctions(uint _id, bytes[] memory data) public onlyOwner {
        LaunchpadProjectInfo(projectsList[_id].projectAddress).callFunctions(data);
        for(uint i; i < data.length; i++){                  
            if (bytes4(data[i]) == bytes4(0x1adff0ee)) {
                projectsList[_id].approved = false;
                break;
            }
        }
    } 

    //Исправить косяк. Не становится true в маппинге и при отмене внутри контракта не становится false в маппинге
    function approveProject(uint _id, bytes[] memory data) external onlyOwner {
        callFunctions(_id, data);
        projectsList[_id].approved = true;
        _deleteIndex(_id);
    }
...
interface IDataToBytes {
    ...
    function nextStage() external pure returns(bytes memory);
}