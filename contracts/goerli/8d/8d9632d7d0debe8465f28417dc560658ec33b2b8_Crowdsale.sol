...
// File: contracts/launchpad/Crowdsale.sol
...
contract Crowdsale is ReentrancyGuard, Ownable {
    ...
    function updateTokenURL(string memory _url) external onlyOwner {
        tokenURL = _url;
        emit URLUpdated(_url);
    }

    function updateInputTokenRate(address _inputToken, uint256 _rate) external onlyOwner {
        require(_getNow() < crowdsaleStartTime, "Cannot update token rate after crowdsale is started");
