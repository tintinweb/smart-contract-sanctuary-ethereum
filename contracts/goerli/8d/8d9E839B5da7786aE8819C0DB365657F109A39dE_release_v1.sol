...
contract release_v1 is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using strings for *;

    /// @notice The maximum number of PoS Phunks available for minting.
    uint256 public constant MAX_TOKENS = 9;

    /// @notice The maximum number of PoS Phunks that can be minted at a time.
    uint256 public constant MAX_TOKENS_PER_PURCHASE = 1000;

    /// @notice The cost of one PoS Phunk.
    ...
    /// @notice The description you would like your PoS Phunk to have.
    string[10000] private description;

    constructor() ERC721("Releasev1", "RV1") Ownable() {}
    ...
    /// @notice Mint a PoS Phunk.
    /// @param _count The number of PoS Phunks to mint.
        ...
        nonReentrant
    {
        ...
        // NEED TO SORT ACCURACY
        // set max token to 9999 or to 10,000
    ...
    /// @notice The PoS Phunks contract owner can change the mint price.
    /// @param _newPrice The new price set for a PoS Phunk.
    function setPrice(uint256 _newPrice) public onlyOwner() {
        price = _newPrice;
    }

    /// @notice Get the current price of a PoS Phunk.
    /// @return The price of a PoS Phunk in Wei.
    function getPrice() public view returns (uint256){
        return price;
    }

    /// @notice The PoS Phunk contract owner can withdraw ETH accumulated through mints.
    function withdraw() public onlyOwner {
        uint256 balance = address(this).balance;
        payable(owner()).transfer(balance);
    }

    /// @notice Enter your wallet address to see which PoS Phunks you own.
    /// @param _owner The wallet address of a PoS Phunk owner.
    /// @return An array of tokenIds of the PoS Phunks owned by the address.
    ...
    /// @param input A string value inputted to genrate the random output.
    ...
    /// @return An integer value used to select an orignal CryptoPunk.
    /// @dev Using keccak256 hash of the work "punk".
    function randomPunk(uint256 tokenId) public pure returns (uint16) {
        uint256 v = uint(keccak256(abi.encodePacked("4c71ce6ba2ee0cfaa5acee977e8e67e2cd9b456dcdf1ab291519d32de27f4ece", toString(tokenId)))) % 10001;
        uint16 original = uint16(v);
        return original;
    }

    /// @notice Combine PoS Phunk attributes with original CryptoPunk attributes.
    /// @param tokenId The minting tokenId for which attributes are being generated.
    /// @return A json string with the combined attributes for a PoS Phunk.
    function makeAttributes(uint256 tokenId) private view returns (string memory) {
        ...
        string memory attributes = string(abi.encodePacked(traits[0], ',', traits[1], ',', traits[2], ',', traits[3]));

        return attributes;
    }

    /// @notice Retreive svg values of a CryptoPunk while trimming unused values.
    /// @param tokenId The tokenId value used to generate the random CryptoPunk selection.
    /// @return An string containing the svg representation of an original CryptoPunk.
    ...
    /// @param tokenId The tokenId value used to generate the original CryptoPunk selection.
    ...
    /// @return A six character string useable as a gradient or background color hex code value.
    /// @dev Uses keccak256 has of "time is only now".
    ...
    /// @notice Generate a random color hex code value using current blockchain details.
    /// @return A six character string useable as a gradient or background color hex code value.
    /// @dev Uses keccak256 has of "you create you".
    ...
    /// @notice Generate the tokenURI for each PoS Phunk.
    /// @param tokenId The tokenId of a Pos Phunk used to select stored value and generate attributes.
    /// @return A base64 encoded string representing a PoS Phunk.
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        string[7] memory p;

        p[0] = '<svg transform="scale(-1,1) translate(-0,0)" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.2" viewBox="0 0 600 600"><defs><linearGradient id="backgroundGradient" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stop-color="#';
        ...
        p[4] = '"/></linearGradient></defs><style type="text/css"></style><rect viewBox="0 0 600 600" width="600" height="600" fill="url(#backgroundGradient)" />';
