...
contract Marketplace is ReentrancyGuard, Ownable, Pausable {
    ...
    address treasury = 0x9035AbF650f35070B8B210aA6f62Aa930F84b745;
        ...
        whenNotPaused
    {
        ...
        uint256 swapfee = calculmarketfees(price);
        uint256 Price = price.sub(swapfee);
    ...
    function acceptOffer(uint256 offerId) public nonReentrant whenNotPaused {
        ...
        uint256 swapfee = calculmarketfees(offer.price);
        uint256 Price = offer.price.sub(swapfee);
    ...
    function calculmarketfees(uint256 _amount) internal view returns (uint256) {
        uint256 result = _amount.div(100);
        uint256 swapFees = result.mul(fees);
        return swapFees;
    }

    function sendFees(uint256 _amount, address _buyer) public {
        uint256 tradeFees = calculmarketfees(_amount);
