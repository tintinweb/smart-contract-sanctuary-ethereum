...
contract MinterWhitelist is Ownable {
    ...
    mapping(address => uint) public whitelistA;
    mapping(address => bool) public whitelistB;

    uint public whitelistSize;
    uint public mintedA;
    uint public mintedB;
    uint public mintQuantityB;
    ...
    function viewAllocationB() public view returns (uint) {
        return erc721.max() - erc721.totalSupply() - whitelistSize;
    }

    function mint() public {
        require(wlMint, "mint not started");
        require(whitelistA[msg.sender] > 0 || whitelistB[msg.sender], "Address not whitelisted");
    ...
    function setMintQuantityB(uint _quantity) public onlyOwner {
    ...
    function setPublicMint(bool _isTrue) public onlyOwner {
        publicMint = _isTrue;
    }

    function setWhitelistA(address[] memory _whitelist, uint[] memory _quantities) public onlyOwner {
        for (uint i = 0; i < _whitelist.length; i++) {
            whitelistSize = whitelistSize + _quantities[i] - whitelistA[_whitelist[i]];
