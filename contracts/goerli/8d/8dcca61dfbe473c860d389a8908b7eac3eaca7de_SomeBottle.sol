...
// TODO maybe leftover withdraw function?
// TODO maybe refactor functions in subfiles
...
contract SomeBottle is MintpassValidator, ERC721A, ERC2981, Ownable {
    ...
    uint256 public TOKEN_LIMIT = 3000;
    ...
    bool public redeemingEnabled = false;
    bool public allowlistMintEnabled = false;
    bool public publicMintEnabled = false;

    // Mint Price
    // int256 public mintUSDPrice = 150000000000; // = 1,500 USD
    int256 public mintUSDPrice = 150000000; // = 1.5 USD
    // int256 public discountMintUSDPrice = 130000000000; // = 1,300 USD
    int256 public discountMintUSDPrice = 130000000; // = 1.3 USD
    // int256 public redeemUSDPrice = 10000000000; // = 100 USD
    int256 public redeemUSDPrice = 10000000; // = 0.1 USD

    string public _baseTokenURI;

    // Sharing is Caring
    mapping(address => uint256) public shares;
    uint256 internal shareA;
    uint256 internal shareB;
    address internal shareABeneficiary;
    address internal shareBBeneficiary;
        ...
        ERC721A(name, symbol)
    {
        ...
        ); // Chainlink ETH/USD GOERLI
        // 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419 // Chainlink ETH/USD MAINNET
        shareA = 1000;
        shareB = 1000;
        shareABeneficiary = 0xCeE2e56a98E967F85A5f63Aa286835128b88Ba36;
        shareBBeneficiary = 0xe01c10b9b4943D9582d8a9621bE9B5afDC96f16b;
        SIGNER_WALLET = 0x04179d7CEE7ea8d93AaDA996e3a912d95711d32f;
    }
    ...
    function withdraw() external onlyBeneficiary {
        require(
            shares[msg.sender] > 0,
            "SomeBottle: Beneficiary has nothing to withdraw"
        );
    ...
    function redeemBottle(uint256 tokenId) public payable {
        require(
            ...
            "SomeBottle: Bottle Redeeming is currently not enabled"
        );
        ...
        require(
            ...
            "SomeBottle: Bottle already redeemed"
        );
        ...
        shares[owner()] = shares[owner()] + msg.value;
    }
    ...
    function resetBottleRedemption(uint256 tokenId) public virtual onlyOwner {
        require(
            redeemedBottles[tokenId] != address(0),
            "SomeBottle: Bottle not redeemed yet"
        );
    ...
    function allowlistMint(
        ...
        require(
            ...
            "SomeBottle: Allowlist Minting is not Enabled"
        );
        ...
        require(TOKEN_LIMIT >= _nextTokenId(), "SomeBottle: All Tokens minted");
    ...
    /**
     * Adds shares to the beneficiary accounts
     */
    function accountShares(uint256 totalReceived) internal {
        ...
        uint256 ownerPayout = totalReceived - shareAPayout - shareBPayout;
        shares[shareABeneficiary] = shares[shareABeneficiary] + shareAPayout;
        shares[shareBBeneficiary] = shares[shareBBeneficiary] + shareBPayout;
        shares[owner()] = shares[owner()] + ownerPayout;
    }
    ...
    /**
     * @dev Can be called to reduce the maximum Token supply. Since we sell a
     * physical product it might happen that not all Tokens can be sold.
     *
    ...
    function reduceTokenLimit(uint256 tokenLimit) public virtual onlyOwner {
        require(
            ...
            "SomeBottle: New Limit must be below current Limit."
        );
        require(
            tokenLimit >= _nextTokenId(),
            "SomeBottle: New Limit must be higher than current Token Supply"
        );
        TOKEN_LIMIT = tokenLimit;
    }
    ...
    modifier onlyBeneficiary() {
        require(
            ...
            "SomeBottle: Caller is not a Beneficiary"
        );
        _;
    }
...
abstract contract MintpassValidator is EIP712 {
    constructor() EIP712("SomeBottle", "1") {}
