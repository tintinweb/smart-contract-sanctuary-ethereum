...
contract MergeGators is
    ...
    function verify(address owner_, uint8 v, bytes32 r, bytes32 s) public virtual {
   ...
   /**
    ...
    function VRFMint(address to, uint256 _quantity) internal {
        if (saleStatus != SaleStatus.PUBLIC) revert callErr();
    ...
    function whitelistMint(address to, uint count) external {  
        ...
        if (saleStatus == SaleStatus.PRESALE) {
            //check coupon
            if (!hasPassedCAPTCHA[to]) revert invalidSigner();
