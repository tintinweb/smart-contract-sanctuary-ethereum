...
contract Compound_borrow is AccessControl{
    ...
    function supply(address token,uint _amount) external {
    address _cToken = tokenForCtoken[token];
    IERC20(token).transferFrom(msg.sender, address(this), _amount);
    IERC20(token).approve(_cToken, _amount);
  ...
  // collateral
  function getCollateralFactor() external view returns (bool,uint,bool) {
    (bool isListed, uint colFactor, bool isComped) = comptroller.markets(address(cToken));
    return (isListed,colFactor,isComped); // divide by 1e18 to get in %
  }
