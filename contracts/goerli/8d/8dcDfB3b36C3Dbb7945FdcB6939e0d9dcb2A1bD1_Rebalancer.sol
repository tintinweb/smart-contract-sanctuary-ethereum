...
contract Rebalancer is BACUpgradable, IRebalancer {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    struct SwapInfo {
        bytes buySwapEncodedPath;
        bytes sellSwapEncodedPath;
        address swapAdapter;
    }

    bytes32 public constant ARBITRAGER_ROLE = keccak256("ARBITRAGER_ROLE");

    /// @notice swap info for specific market
    ...
    function initialize(address protocolConfig) external initializer {
        ...
        returns (bool)
    {
        (
            ...
            uint sellToken,
            uint sellStable
        ) = IRebalanceCalculation(IMarket(_market).getRiskCalc())
                .rebalanceAmount();
    ...
    function _rebalance(
        ...
        SwapInfo memory swap = swapInfo[_market];
        require(swap.swapAdapter != address(0), "Invalid market");

        if (sellToken > 0) {
            IERC20Upgradeable token = IERC20Upgradeable(
                address(IMarket(_market).token())
            );

            token.safeTransferFrom(_market, address(this), sellToken);

            _approveIfNotMaxAllowance(token, swap.swapAdapter);

            deltaCP = int(
                ISwap(swap.swapAdapter).swapExactTokensForTokensMult(
                    swap.sellSwapEncodedPath,
                    sellToken,
                    0,
                    _market
                )
        ...
        if (sellStable > 0) {
            IERC20Upgradeable stable = IERC20Upgradeable(
                address(IMarket(_market).stable())
            );

            stable.safeTransferFrom(_market, address(this), sellStable);

            _approveIfNotMaxAllowance(stable, swap.swapAdapter);

            deltaAP = int(
                ISwap(swap.swapAdapter).swapExactTokensForTokensMult(
                    swap.buySwapEncodedPath,
                    sellStable,
                    0,
                    _market
                )
        ...
        IMarket(_market).onAfterRebalance(deltaAP, deltaAR, deltaCP, deltaCR);
    ...
    function setSwap(
        ...
        // swap[_market] = _swap;
        // buySwapEncodedPath[_market] = _buySwapEncodedPath;
        // sellSwapEncodedPath[_market] = _sellSwapEncodedPath;
    }
    ...
    function _approveIfNotMaxAllowance(IERC20Upgradeable token, address spender)
        internal
    {
        if (token.allowance(address(this), spender) != type(uint).max) {
