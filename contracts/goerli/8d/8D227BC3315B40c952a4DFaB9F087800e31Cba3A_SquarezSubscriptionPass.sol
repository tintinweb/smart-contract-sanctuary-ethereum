...
contract SquarezSubscriptionPass is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public whitelistMintMaxSize=269; // 40 for team, 111 for genesis.
    address signerAddress = 0x8083ddC308eE77E72DfaCD0F38595FB7F16916Ac;
  ...
  function isAuthorized(address sender, bytes calldata signature) private view returns (bool){
...
library DateTime{
    ...
    function fillZero(uint256 i) internal pure returns(string memory){
        if (i < 10){
            return string(abi.encodePacked('0', Strings.toString(i)));
        }
        else{
        ...
        returns (string memory)
    {
        ...
        return string(abi.encodePacked(Strings.toString(year), '/', fillZero(month), '/', fillZero(day), ' ', fillZero(hour), ':', fillZero(minute), ':', fillZero(second)));
    }
}
