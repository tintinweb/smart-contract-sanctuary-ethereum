...
contract ShibaTicket is VRFConsumerBase, ReentrancyGuard, Ownable, Pausable {
    ...
    uint ticketcount;
    uint ticketprice = 132052;
    uint256 internal balance;
    address tokenAddress = 0x8C7190802Ae13025adE069D2a11Eb9dF2F52A071;
    ...
    constructor() Pausable()
        VRFConsumerBase(
            ...
            0x63bfb2118771bd0da7A6936667A7BB705A06c1bA  // goerli LINK token address
        ) {
    ...
    // when called it returns the current ticket count ( a.k.a TICKET NUMBER )
    function ticketCount() public view returns (uint) {
        return ticketcount;
    }

    // Allows for ticketcount to be updated from outside source
    function updateTicketCount() public {
    ...
    function showTicketPrice() public view returns (uint) {
        return ticketprice * 10**18;
    }
    ...
    function winnerByLottery(uint lottery) public view returns (address payable) {
        return lotteryHistory[lottery];
    }

    // returns the amount of the contracts ERC20 balance
    function TokenBalance() public view returns (uint) {
        return IERC20(tokenAddress).balanceOf(address(this));
    }

    // this displays all the addresses of the current ticket holders
    function currentPlayers() public view returns (address payable[] memory) {
        return players;
    }
    ...
    // Only the owner can call this function which will send remaining ERC20 
    // balance to the owner
    function finalizeFunds(IERC20 token) whenPaused() public onlyOwner {
        uint256 erc20balance = IERC20(tokenAddress).balanceOf(address(this));
        token.transfer(msg.sender, erc20balance);
        emit TransferSent(tokenAddress, msg.sender, erc20balance);
    }

    // This function is used by the owner to pay the winner their percentage of winnings
    function payWinner(IERC20 token) whenPaused() public onlyOwner {
        
        // Calculates 10 percent of the pot to be given to the winner
        require(randomResult > 0, "Must have a source of randomness before paying a winner");
        uint256 erc20Payoutbalance = (IERC20(tokenAddress).balanceOf(address(this)) / 1000) * 100;
        uint index = randomResult % players.length;
        token.transfer(players[index], erc20Payoutbalance);
        emit TransferSent(msg.sender, players[index], erc20Payoutbalance);
