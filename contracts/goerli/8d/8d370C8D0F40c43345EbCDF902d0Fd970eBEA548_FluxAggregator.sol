...
contract FluxAggregator is AggregatorV2V3Interface, Owned {
    ...
    private
  {
    ...
    recordedFunds = funds;
    oracles[msg.sender].withdrawable = funds.available.sub(payment);
    recordedFunds.allocated = recordedFunds.allocated.sub(payment);

    assert(linkToken.transfer(msg.sender, uint256(payment)));

    emit AvailableFundsUpdated(funds.available);
    emit TestEvent(funds.available, funds.allocated, payment);
  }
