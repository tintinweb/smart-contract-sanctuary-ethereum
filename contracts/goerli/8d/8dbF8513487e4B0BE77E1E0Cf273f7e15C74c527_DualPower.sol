...
contract DualPower is Ownable, ERC721A, ERC2981, PaymentSplitter {
  ...
  function checkMintEligibility(
    ...
    if (block.timestamp < (allowlist ? mintConfig.allowlistMintTime : mintConfig.publicMintTime)) return false;
    if (block.timestamp > (allowlist ? mintConfig.publicMintTime : mintConfig.mintEndTime)) return false;
    if (_getAux(recipient) + amount > (allowlist ? mintConfig.allowlistMintLimit : mintConfig.publicMintLimit)) return false;
    if ((totalSupply() + amount) > mintConfig.supplyLimit) return false;
  ...
  function paperAllowlistMint(
    ...
    if (msg.value < amount * MINT_PRICE) revert IncorrectPayment();
