...
contract VaultFactory is UpgradeableBeacon, IVaultFactory {
    ...
    bytes32 private immutable _bytesArr0 = bytes32(uint256(64));
    bytes32 private immutable _bytesArr1 = bytes32(0);

    event Deployed(address _proxy, address _sender, bytes32 _salt);
    event FeeToChanged(address prevFeeTo, address newFeeTo);
    ...
    function setFee(uint256 newFee) public {
        ...
        require(_msgSender() == feeTo, "VaultFactory: not fee receiver");
        require(newFee <= FEE_BASIS, "VaultFactory: newFee must be less than FEE_BASIS");
     ...
     */
    function predict(address deployer, bytes32 salt) public view virtual returns (address) {
