...
// File contracts/CERUSVesting.sol

// CERUS INVESTOR CONTRACT
// allows a whitelisted investor to buy CERUS at a set price
// and claim at scheduled times
//
//
...
contract CERUSVesting is AccessControl, ReentrancyGuard {
    ...
    // Info of each user.
    struct InvestorInfo {
        uint256 depositedAmount; // How much an investor has invested usdc.
        uint256 vestedAmount; // How much an investor must be paid.
        uint256 claimedEpochs; // How many times an investor has claimed.
    }
    mapping(address => InvestorInfo) public investorInfo;

    // TOKENS
    address public cerusToken; /// @dev undeployed, must be set later.
    address public buyToken; //  0xEA32A96608495e54156Ae48931A7c20f0dcc1a21 - m.usdc on metis
    uint256 public constant CERUS_DECIMALS = 18;

    // SALE
    uint256 public constant MAX_BURNED = 100000 * 10**CERUS_DECIMALS; // Amount to burn if not sold.
    uint256 public totalCerus; // Total amount to sell.
    uint256 public cerusPrice; /// @dev in m.usdc
    uint256 public minBuy; /// fx. 5000 m.USDC
    uint256 public totalVested = 0; /// @dev total amount of CERUS sold

    uint256 public saleStart; // unix time
    uint256 public saleEnd; // uinx time

    address public treasury;

    // CLAIM
    uint256 public claimStart; // unix time
    uint256 public claimEnd; // claimStart + totalEpochs * epochLength
    uint256 public totalEpochs;
    uint256 public totalClaimed; // counter for the UI.
    uint256 public epochLength; //  = 4 weeks;
    ...
    // INIT CONTRACT
    /// @dev Instead we set per contract vars here.
    function initialize(
        ...
        claimEnd = _claimStart + (_epochs * _epochLength);
    ...
    function deposit(uint256 _depositAmount) external nonReentrant {
        ...
        uint256 _vestAmount = (_depositAmount * (10**CERUS_DECIMALS)) /
            cerusPrice;
        totalVested += _vestAmount;

        investorInfo[msg.sender].depositedAmount += _depositAmount;
        investorInfo[msg.sender].vestedAmount += _vestAmount;

        emit Deposit(msg.sender, _vestAmount);
    }

    function claim() external nonReentrant {
        _canClaim(msg.sender);

        (uint256 _claimableEpochs, uint256 _claimableAmount) = pendingEpochs(
            msg.sender
        );

        investorInfo[msg.sender].claimedEpochs += _claimableEpochs;
        investorInfo[msg.sender].vestedAmount -= _claimableAmount;
        totalClaimed += _claimableAmount;
    ...
    function withdrawUnsold() external {
        ...
        uint256 _unsold = totalCerus - totalVested;
        uint256 _balance = IERC20(cerusToken).balanceOf(address(this));
        require(_balance >= _unsold, "withdrawUnsold: not enough tokens!");
        hasWithdrawnUnsold = true;

        uint256 _toBurn = (_unsold <= MAX_BURNED) ? _unsold : MAX_BURNED;
        uint256 _toTransfer = (_unsold > MAX_BURNED) ? _unsold - MAX_BURNED : 0;
        if (_toBurn > 0) {
            ERC20Burnable(cerusToken).burn(_toBurn);
        }
        if (_toTransfer > 0) {
            IERC20(cerusToken).transfer(treasury, _toTransfer);
        }

        emit WithdrawUnsold(_unsold);
    }
    ...
    function currentEpoch() public view returns (uint256) {
        ...
        /// @notice we count claimable epochs from 1
    ...
    function _canDeposit(address _investor, uint256 _amount) private view {
        ...
        require(whitelisted[_investor], "deposit: not whitelisted!");
        require(isSale(), "deposit: Sale is not live!");
        require(
            _amount + investorInfo[_investor].vestedAmount >= minBuy,
            "deposit: not enough!"
        );
        ...
        require(
            ...
            "deposit: not enough tokens left!"
        );
        require(
    ...
    function _canClaim(address _investor) private view {
        require(cerusToken != address(0), "Cerus: not set!");
        ...
        uint256 balance = IERC20(cerusToken).balanceOf(address(this));

        require(balance > 0, "claim: not enough tokens!");
    }
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 _claimableEpochs = currentEpoch() - investor.claimedEpochs;

        if (_claimableEpochs == 0) return (0, 0);

        uint256 _unclaimedEpochs = totalEpochs - investor.claimedEpochs;
        uint256 _claimableAmount = (investor.vestedAmount * _claimableEpochs) /
            _unclaimedEpochs;

        return (_claimableEpochs, _claimableAmount);
    }
