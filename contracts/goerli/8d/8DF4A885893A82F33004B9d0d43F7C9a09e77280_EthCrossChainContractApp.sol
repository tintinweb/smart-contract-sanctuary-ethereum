...
contract EthCrossChainContractApp {
    ...
    /// @dev Contract address by string
    string internal _CrossContract;
    ...
    function initialize(string memory chainCode,
        address[] memory managersList,
        address payable crossContract) external {
        require(_Init == false, "contract initialized");
        require(crossContract == address(this), "contract not equal address(this)");
        _Owner = msg.sender;
        _Version = "1.0.0";
        ...
        _Init = true;
        _CrossTxNum = 0;
        _CrossContract = LibString.addressToString(crossContract);
    }
    ...
    //    function transfer(string memory assetKey, address to, uint256 value) external {
    //        //require(to != address(0x0), "Invalid address");
    //        require(value > 0, "Value > 0");
    //        require(to != msg.sender, "Transfer assets to yourself");
    //
    //        if (LibString.contains(assetKey, _ERC721)) {
    //            _HolderTokens[abi.encodePacked(assetKey, msg.sender)].remove(value);
    //            assetErc721Mint(assetKey, to, value);
    //            return;
    //        }
    //
    //        uint256 senderValue = _Asset[
    //        abi.encodePacked(assetKey, msg.sender)
    //        ];
    //        require(
    //            senderValue > value,
    //            "asset not enough"
    //        );
    //
    //        uint256 toValue = _Asset[
    //        abi.encodePacked(assetKey, to)
    //        ];
    //        toValue = toValue + value;
    //        _Asset[abi.encodePacked(assetKey, to)] = toValue;
    //
    ...
    function subStartTxTransferSgas(
        ...
        (uint8  assetType,uint256 amount,string memory contractAddress,uint256  tokenId, string memory contractAddressDest,
        string memory extra1, string memory extra2) = abi.decode(
            payload,
            (uint8, uint256, string, uint256, string, string, string)
        );

        if (assetType == 1) {
            require(amount == msg.value, "Amount not same");
        } else if (assetType == 2) {
            erc20TransferFrom(srcAddress, address(this), amount, payable(LibString.toAddress(contractAddress)));
        } else if (assetType == 3) {
            erc721TransferFrom(srcAddress, address(this), tokenId, payable(LibString.toAddress(contractAddress)));
        } else if (assetType == 4) {
            erc20TransferFrom(srcAddress, address(this), amount, payable(LibString.toAddress(contractAddress)));
        } else if (assetType == 5) {
            erc20TransferFrom(srcAddress, address(this), amount, payable(LibString.toAddress(contractAddress)));
    ...
    function startTx(
        ...
        if (payloadType == 2) {
            ...
            subStartTxTransferSgas(srcAddress, srcChainCode, payload);
        }
        ...
        crossTxObj.SrcCrossContract = _CrossContract;
    ...
    function assetMint(string memory assetKey, address destAddress, uint256 amount) internal {

        _Asset[abi.encodePacked(assetKey, destAddress)] = _Asset[abi.encodePacked(assetKey, destAddress)] + amount;

        emit AssetMintEvent("mint", assetKey, address(this), destAddress, amount);
    }

    /// @notice Casting main chain points
    function assetErc721Mint(string memory assetKey, address destAddress, uint256 tokenId) internal {
        _HolderTokens[abi.encodePacked(assetKey, destAddress)].add(tokenId);
        emit AssetMintEvent("mint", assetKey, address(this), destAddress, tokenId);
    }
    ...
    function subSendTxTransferSgas(address destAddress, string memory srcChainCode, bytes memory payload) internal {
        ...
        (uint8  assetType,uint256 srcAmount,string memory contractAddress,uint256  tokenId, string memory contractAddressDest,
        string memory extra1, string memory extra2) = abi.decode(
            payload,
            (uint8, uint256, string, uint256, string, string, string)
        );
        if (assetType == 1) {
            assetMint(srcChainCode, destAddress, srcAmount);
        } else if (assetType == 2) {
            string memory assertKey = LibString.concat(LibString.concat(srcChainCode, _ERC20), contractAddress);
            assetMint(assertKey, destAddress, srcAmount);
        } else if (assetType == 3) {
            string memory assertKey = LibString.concat(LibString.concat(srcChainCode, _ERC721), contractAddress);
            assetErc721Mint(assertKey, destAddress, tokenId);
        } else if (assetType == 4) {
            erc20Approve(destAddress, srcAmount, payable(LibString.toAddress(contractAddressDest)));
        } else if (assetType == 5) {
            erc20Approve(destAddress, srcAmount, payable(LibString.toAddress(contractAddressDest)));
        } else if (assetType == 6) {
            erc721Approve(destAddress, tokenId, payable(LibString.toAddress(contractAddressDest)));
        }
        //
    }
    ...
    internal
    {
        (bool success,) = ercContractAddress.call(abi.encodeWithSignature("transferFrom(address,address,uint256)", fromAddress, toAddress, amount));
        require(success, "Contract20 call failed");
    }

    /// @notice erc20 asset transferFrom
    function erc20Approve(address spender, uint256 amount, address ercContractAddress)
    internal
    {
        (bool success,) = ercContractAddress.call(abi.encodeWithSignature("approve(address,uint256)", spender, amount));
        require(success, "Contract20 call failed");
    }
    ...
    internal
    {
        (bool success,) = ercContractAddress.call(abi.encodeWithSignature("transferFrom(address,address,uint256)", fromAddress, toAddress, tokenId));
        require(success, "Contract721 call failed");
    }

    /// @notice erc20 asset transferFrom
    function erc721Approve(address to, uint256 tokenId, address ercContractAddress)
    internal
    {
        (bool success,) = ercContractAddress.call(abi.encodeWithSignature("approve(address,uint256)", to, tokenId));
        require(success, "Contract721 call failed");
    }
    ...
    function balanceOf(string memory assetKey, address addr) external view returns (uint256) {
        //require(addr != address(0), " balance query for the zero address");
        if (LibString.contains(assetKey, _ERC721)) {
            return _HolderTokens[abi.encodePacked(assetKey, addr)].length();
        } else {
            return _Asset[abi.encodePacked(assetKey, addr)];
        }
    }
