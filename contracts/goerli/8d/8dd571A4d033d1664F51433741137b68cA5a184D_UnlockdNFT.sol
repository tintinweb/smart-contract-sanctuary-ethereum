    ...
    UUPSUpgradeable
{
    ...
    uint64 private keySupporterSaleIndex;
    uint64 private publicWhiteListSaleIndex;
    ...
    function initialize(
        ...
        __ERC721_init("The Lockey", "LOCKY");
        __Ownable_init();
        ...
        nonReentrant
    {
        ...
        require(publicWhiteListCount[_msgSender()] >= _count, "already minted");
        require(publicWhiteListSaleIndex < 2526, "whitelist all minted");
        require(_count < 6, "whitelist mint balance over");
        require(mintFee * _count <= msg.value, "!enough mintFee");
        for (uint64 i = 0; i < _count; ++i) {
            _mint(_to, publicWhiteListSaleIndex);
            ++publicWhiteListSaleIndex;
        }
        publicWhiteListCount[_msgSender()] -= _count;
    }
    ...
    function mintNFT(
        ...
        if (_count > 0) {
            require(!_reentrancyGuard, "Reentrant public NFT mint call");
            ...
            for (uint64 i = 0; i < _count; ++i) {
                _mint(_msgSender(), genericNFTPublicSaleIndex);
