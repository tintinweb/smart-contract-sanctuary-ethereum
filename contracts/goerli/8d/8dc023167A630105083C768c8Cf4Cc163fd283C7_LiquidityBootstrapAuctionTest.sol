...
import "./LiquidityBootstrapAuction.sol";
import "hardhat/console.sol";

contract LiquidityBootstrapAuctionTest is LiquidityBootstrapAuction {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 amount,
        uint256 startTime
    ) LiquidityBootstrapAuction(multisig, _asto, _usdc, amount, startTime) {}
    ...
    // this function requires calling approve() before!
    function setTotalDepositedUSDC(uint256 newDeposited) external {
        if (newDeposited > totalDepositedASTO) {
            uint256 diff = newDeposited.sub(totalDepositedUSDC);
            usdc.transferFrom(msg.sender, address(this), diff);
        }

        if (totalDepositedASTO > newDeposited) {
            uint256 diff = totalDepositedUSDC.sub(newDeposited);
            usdc.transferFrom(address(this), msg.sender, diff);
        }

        totalDepositedUSDC = newDeposited;
    }

    // this function requires calling approve() before!
    function setTotalDepositedASTO(uint256 newDeposited) external {
        if (totalDepositedASTO > newDeposited) {
            uint256 diff = totalDepositedASTO.sub(newDeposited);
            asto.transferFrom(address(this), msg.sender, diff);
        }

        if (newDeposited > totalDepositedASTO) {
            uint256 addition = newDeposited.sub(totalDepositedASTO);
            asto.transferFrom(msg.sender, address(this), addition);
        }

        totalDepositedASTO = newDeposited;
    }

    function withdrawUSDCForUser(address user) external {
        uint256 amount = withdrawableUSDCAmount(user);
        require(usdcWithdrawAllowed(), "withdraw not allowed");
        require(amount > 0, "withdrawal is not possible");

        if (currentTime() >= usdcWithdrawLastDay()) {
            usdcWithdrawnOnDay7[user] = true;
        } else if (currentTime() >= usdcDepositEndTime()) {
            usdcWithdrawnOnDay6[user] = true;
        }

        depositedUSDC[user] -= amount;
        totalDepositedUSDC -= amount;

        emit USDCWithdrawn(user, amount, stats(user));

        usdc.transfer(user, amount);
    }

    // This function isn't safe!
    // It doesn't take into account LBA's stages.
    // Use withdrawUSDCForUser() instead.
    function resetUserDepositedUSDC(address user) external {
        uint256 currentBalance = depositedUSDC[user];
        if (currentBalance > 0) {
            usdc.transfer(user, currentBalance);
            depositedUSDC[user] = 0;
            totalDepositedUSDC -= currentBalance;
        }
    }

    // This function isn't safe!
    // It doesn't take into account LBA's stages
    function resetUserDepositedASTO(address user) external {
        uint256 currentBalance = depositedASTO[user];
        if (currentBalance > 0) {
            asto.transfer(user, currentBalance);
            depositedASTO[user] = 0;
            totalDepositedASTO -= currentBalance;
        }
    }

    function setLPTokenBalance(uint256 amount) external {
        lpTokenAmount = amount;
    }

    /**
     * @notice Set the ASTO amount deposited from an address.
     */
    function setDepositedASTO(address addr, uint256 newDeposited) external {
        depositedASTO[addr] = newDeposited;
    }

    /**
     * @notice Set the USDC amount deposited from an address.
     */
    function setDepositedUSDC(address addr, uint256 newDeposited) external {
        depositedUSDC[addr] = newDeposited;
    }

    /**
     * @notice Sets the auctionStartTime to match the Lp token release time
     */
    function setLpTokenReleaseTime(uint256 timestamp) external {
        auctionStartTime = timestamp - 7 days - 12 weeks;
    }
}
