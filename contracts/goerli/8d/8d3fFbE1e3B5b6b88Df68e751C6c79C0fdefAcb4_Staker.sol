...
contract Staker {
    ...
    function _getUnstakedRewards() private view returns (uint256) {
        return IValidator(validatorContractAddress).getLiquidRewards(address(this));
    }
    ...
    function _stake(uint256 _amount) private {
        IValidator(validatorContractAddress).buyVoucher(_amount, 2423);
    ...
    function _unbond(uint256 _amount) private returns (uint256 unbondNonce) {
        ...
        IValidator(validatorContractAddress).sellVoucher_new(_amount, _amount);

        return IValidator(validatorContractAddress).unbondNonces(address(this));
    }

    function _claimStake( uint256 _unbondNonce) private {
        // transfer all unstaked MATIC to vault in original unbonding call
        IValidator(validatorContractAddress).unstakeClaimTokens_new(_unbondNonce);
    }

    function _restake() private returns (uint256) {
        (uint256 amountRestaked,) = IValidator(validatorContractAddress).restake();
...
interface IValidator {
    ...
    function unbondNonces(address) external returns (uint256); // automatically generated getter of a public mapping
}
