...
interface IPrizeSplit {
    ...
    ) external view returns (PrizeSplitConfig memory);
    ...
    function setPrizeSplits(
        PrizeSplitConfig[] calldata newPrizeSplits
    ) external;
...
abstract contract PrizeSplit is IPrizeSplit, Ownable {
    ...
    function __PrizeSplit_init_unchained(address owner)
        internal
        onlyInitializing
    {
        ...
        onlyOwner
    {
        ...
        require(
            newPrizeSplitsLength <= type(uint8).max,
            "PrizeSplit/invalid-prizesplits-length"
        );
    ...
    /// @inheritdoc IPrizeSplit
    function setPrizeSplit(
        PrizeSplitConfig memory _prizeSplit,
        uint8 _prizeSplitIndex
    ) external override onlyOwner {
        require(
            _prizeSplitIndex < _prizeSplits.length,
            "PrizeSplit/nonexistent-prizesplit"
        );
        require(
            _prizeSplit.target != address(0),
            "PrizeSplit/invalid-prizesplit-target"
        );
     ...
     */
    function _awardPrizeSplitAmount(address _target, uint256 _amount)
        internal
        virtual;
...
contract PrizeSplitStrategy is PrizeSplit, IStrategy {
    ...
    function initialize(address _owner, IPrizePool _prizePool)
        public
        virtual
        initializer
    {
        __PrizeSplitStrategy_init_unchained(_owner, _prizePool);
    }

    function __PrizeSplitStrategy_init_unchained(
        address _owner,
        IPrizePool _prizePool
    ) internal onlyInitializing {
        __PrizeSplit_init_unchained(_owner);
        require(
     ...
     * @param _amount Amount of minted tokens.
     */
    function _awardPrizeSplitAmount(address _to, uint256 _amount)
        internal
        override
    {
