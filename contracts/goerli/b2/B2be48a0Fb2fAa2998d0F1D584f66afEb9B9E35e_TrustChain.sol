  ...
  ERC721URIStorageUpgradeable
{
  ...
  function initialize(
    ...
    _grantRole(DEFAULT_ADMIN_ROLE, __holder);
    ...
    returns (MixinStructs.TrustChainCredential memory)
  {
    if (_issuerCredentialContract != address(0)) {
      // not root credential
      TrustChain issuerTrustChain = TrustChain(_issuerCredentialContract);
    ...
    return
      MixinStructs.TrustChainCredential(
        MixinStructs.TrustChainCredentialData(
          ...
          0,
          1,
          "TrustChain Root Credential",
          "This is the root credential of TrustChain"
        ),
        owner(),
        _tokenId,
        ""
  ...
  function issue(
    ...
    address _trustChainTemplateImpl = _trustChainTemplateImpls[
    ...
    // it's to fix compile error: CompilerError: Stack too deep.
    MixinStructs.TrustChainCredentialType _credentialType = credentialType;
    // trigger event
  ...
  function adminCredentialByOwner(
    ...
    require(_tokenIdByAdmin[owner] > 0, "Cannot find admin credential");
    uint256 tokenId = _tokenIdByAdmin[owner];
    return credentialByTokenId(tokenId);
  }
  ...
  function _saveAdminTokenId(address to, uint tokenId) private {
  ...
  function trustChainTemplateVersions(
    address _impl
  ) external view onlyOwner returns (uint16) {
  ...
  function upgradeTrustChain(
    ...
    // check version
    TrustChain trustChain = TrustChain(trustChainAddress);
