...
contract GFC_TOKEN is ERC20, ERC20Snapshot, ERC20Burnable, AccessControl, ReentrancyGuard, ChainlinkClient, ConfirmedOwner, GFC_Helpers, Taxable {
    ...
    string public getURL = "https://arcane-sands-66304.herokuapp.com/test/";   
    ...
    function receiveClaim(uint256 _claimAmount) public nonReentrant {
        ...
        _mint(sendTo, _claimAmount);

        emit RewardClaimed(sendTo, _claimAmount);
    }

    //-----------------------------------------------------------------------------
    //-----------TOKEN TRANSFER AND TAXES------------------------------------------
    //-----------------------------------------------------------------------------
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override(ERC20) nonReentrant  {        
        if(hasRole(EXEMPT_ROLE, from) || hasRole(EXEMPT_ROLE, to) || !taxed()) { // If to/from a tax excluded address or if tax is off...
            super._transfer(from, to, amount); // Transfers 100% of amount to recipient.
        } else { // If not to/from a tax excluded address & tax is on...
            require(balanceOf(from) >= (amount + amount*thetax()/10000)); // Makes sure sender has the required token amount for the total.
            ...
            super._transfer(from, to, amount); // Transfers the remainder to the recipient.
        }
    }  
