...
contract PaymentContract {
    ...
    function CancelPaymentSubscription(uint256 index) public {
        if (payments[index].payer != msg.sender)
            revert PaymentContract__SenderNotOwner();

        (bool success, ) = payable(msg.sender).call{value: payments[index].fundsAmount}("");
