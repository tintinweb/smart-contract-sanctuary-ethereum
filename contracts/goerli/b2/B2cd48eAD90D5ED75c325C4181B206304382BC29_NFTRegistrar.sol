...
interface INFTController {
    function available(string memory name, bool isNum) external view returns (bool);
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import { INFTController } from "./INFTController.sol";
import { IOwnedResolver } from "./IOwnedResolver.sol";
import { IPriceOracle } from "./IPriceOracle.sol";
import { BaseRegistrar } from "./BaseRegistrar.sol";
import "hardhat/console.sol";

contract NFTRegistrar is BaseRegistrar {
    // ************************************* Constants **************************************** //

    // ************************************* Variables **************************************** //

    INFTController public registrarController;
    ...
    event MaxTxnUpdated(uint256 previewsMaxTxn, uint256 newMaxTxn);
    ...
    constructor(address _registrarController) {
        registrarController = INFTController(_registrarController);
    }
    ...
    function available(string[] memory names, bool[] memory isNums) external view returns (bool[] memory) {
        bool[] memory results = new bool[](names.length);
        for (uint256 i = 0; i < names.length; i++) {
            results[i] = registrarController.available(names[i], isNums[i]);
        }
        return results;
    }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < names.length; i++) {
            prices[i] = registrarController.rentPrice(names[i], durations[i]);
        }
        return prices;
    }
    ...
    function getCommitments(
        ...
        for (uint256 i = 0; i < _names.length; i++) {
            ...
            timestamps[i] = registrarController.commitments(commitment);
        }
        return timestamps;
    }
    ...
    function setMaxTxn(uint256 _maxTxn) external onlyOwner {
        require(_maxTxn > 0 && _maxTxn < 200, "Max txn err");
        uint256 oldMaxTxn = maxTxn;
        maxTxn = _maxTxn;
        emit MaxTxnUpdated(oldMaxTxn, maxTxn);
    }
    ...
    function register2(
        ...
        uint256 totalCost = 0;
        for (uint256 i = 0; i < _names.length; i++) {
            prices[i] = registrarController.rentPrice(_names[i], _durations[i]);
            totalCost += prices[i];
        }
        require(msg.value >= totalCost, "Error: Ether not enough");
        ...
        for (uint256 i = 0; i < _names.length; i++) {
            registrarController.RegisterWithConfig{ value: prices[i] }(_names[i], _durations[i], _secret, addrs);
        }
    }
