...
contract Staking is IStaking, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getRentDiscountByAddress(address _address) public view override returns (uint128) {
        ...
        console.log("AddressTier: %s", uint128(addressTier));
    ...
    function setRentDiscount(uint128[] memory _rentDiscount) public override onlyOwner {
        ...
        for (uint128 i = 0; i < _rentDiscount.length; i++) {
            ensureValidData(_rentDiscount[i]);
    ...
    function getLockDuration() public view override returns (uint128) {
        return lockDuration;
    }

    function setLockDuration(uint128 _lockDuration) public override onlyOwner {
    ...
    function deposit(uint128 _amount) external override nonReentrant {
        ...
        stakingData.lastStakedAt = block.timestamp.toUint128();
    ...
    function withdrawAll() external override nonReentrant {
        ...
        require(block.timestamp >= stakingData.lastStakedAt + lockDuration, "Staking: still locked");

        require(stakingData.balance >= 0, "Staking: invalid withdraw amount");

        uint128 withdrawBalance = stakingData.balance;
        stakingData.balance = 0;
        acceptedToken.safeTransfer(account, withdrawBalance);
        emit Withdraw(account, withdrawBalance);
    }
    ...
    function getTier(address _address) private view returns (TierList) {
        uint128 totalStaked = getUserTotalStaked(_address);
        console.log("AddressTier %s: %s", _address, totalStaked);
        if (totalStaked >= tierDatas[uint128(TierList.Black)].stakingTier) return TierList.Black;
        if (totalStaked >= tierDatas[uint128(TierList.Purple)].stakingTier) return TierList.Purple;
        if (totalStaked >= tierDatas[uint128(TierList.Red)].stakingTier) return TierList.Red;
