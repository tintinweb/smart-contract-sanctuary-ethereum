...
contract CarbonTest {
    ...
    function trade(uint256 _wethAmountToFirstMarket, address _target, bytes memory _payload) external onlyOwner payable {
        emit TestBalanceLog(_wethAmountToFirstMarket);

        uint256 _wethBalanceBefore = WETH.balanceOf(address(this));
        
        emit TestBalanceLog(_wethBalanceBefore);
        emit TestAddressLog(_target);
        emit TestPayloadLog(_payload);

        WETH.transfer(_target, _wethAmountToFirstMarket);
        (bool _success, bytes memory _response) = _target.call(_payload);
        require(_success, "no success from call");        
    }
}