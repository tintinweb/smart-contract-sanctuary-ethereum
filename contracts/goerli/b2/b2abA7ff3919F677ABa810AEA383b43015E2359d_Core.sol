...
contract Core is Ownable{
    ...
    function addProtocol(string memory _protocol, bool _isLP) external onlyOwner {
        ...
        lendingProtocol storage protocol = supportedProtocols[keccak256(abi.encode(_protocol))];
        protocol.isSupported = true;
        protocol.isLP = _isLP;
    }
    ...
    function expireEpoch(
        ...
        require(IFuture(_epochAddress).expiry() < block.timestamp, "ERR_STREAM_CONTAINS_ACTIVE_EPOCH");
