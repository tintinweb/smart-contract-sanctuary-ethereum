...
contract xborgStaker is Ownable {
    ...
    uint256 public currentStakeIndex = 0;
    ...
    function stakeNFT(uint256[] calldata tokenIds, uint256[] calldata tokenPoints) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds.length == tokenPoints.length , "input information is not correct");
            require(msg.sender == xborgContract.ownerOf(tokenIds[i]) , "stake caller is not owner");
            xborgContract.stakeToken(tokenIds[i]);
            _stakerInfo[msg.sender].point += tokenPoints[i];
        }
    }

    function unstakeNFT(uint256 tokenId) external {
        require(msg.sender == xborgContract.ownerOf(tokenId) , "unstake caller is not owner");
        xborgContract.unstakeToken(tokenId);
    }
    ...
    function checkStakerPoint(address stakerAddr) public view returns (uint256) {
        return _stakerInfo[stakerAddr].point;
    }
}