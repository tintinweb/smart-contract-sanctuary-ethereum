...
contract SolidBlock is IERC20, Ownable, AccessControl {
    ...
    address public burnAddress = 0x056f62064Df299FD124631350fa94C42b62e9cB4;
    ...
    uint256 public presaleMaxWalletAmount = 100_000_000;
    ...
    Taxes public taxes = Taxes(10,40,20,10,50);
    ...
    constructor () {
        ...
        _setOwner(0x4dDd407bdfdD35f74131E38E7dBdf7baf4e8CBdE);
        ...
        _setupRole(BLOCK_ROLE, msg.sender);
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(isPresale == false, "You cannot perform this function in the pre-sale");
    ...
    function blockAccount(address account) external{
        require(hasRole(BLOCK_ROLE, msg.sender), "Caller is not a blocker");
    ...
    function setWhitelist(bytes32 _whitelistMerkleRoot) external {
    ...
    function setTaxes(uint256 _rfi, uint256 _treasury, uint256 _development, uint256 __burn, uint256 _liquidity) public {
        taxes.rfi = _rfi;
    ...
    function _getValues(uint256 tAmount, uint8 takeFee) private  returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rTreasury,to_return.rDevelopment, to_return.rBurn, to_return.rLiquidity) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, uint8 takeFee) private returns (valuesFromGetValues memory s) {
        ...
        if(takeFee == 0) {
          ...
          return s;
            ...
            ETHAmount.development += s.tLiquidity*splitETH.development/taxes.liquidity;
            s.tTransferAmount = tAmount-s.tRfi-s.tTreasury-s.tLiquidity-s.tDevelopment-s.tBurn;
            return s;
        } else {
            s.tRfi = tAmount*taxes.rfi/1000;
            s.tDevelopment = tAmount*taxes.development/1000;
            s.tBurn = tAmount*taxes.burn/1000;
            s.tLiquidity = tAmount*splitETH.development/1000;
            ETHAmount.development += s.tLiquidity;
            s.tTransferAmount = tAmount-s.tRfi-s.tLiquidity-s.tDevelopment-s.tBurn;
            return s;
        }
        
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, uint8 takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rTreasury,uint256 rDevelopment,uint256 rBurn,uint256 rLiquidity) {
        ...
        if(takeFee == 0) {
          return(rAmount, rAmount, 0,0,0,0,0);
            ...
            rTransferAmount =  rAmount-rRfi-rTreasury-rLiquidity-rDevelopment-rBurn;
            return (rAmount, rTransferAmount, rRfi,rTreasury,rDevelopment,rBurn,rLiquidity);
        }
        else{
            rRfi = s.tRfi*currentRate;
            rLiquidity = s.tLiquidity*currentRate;
            rDevelopment = s.tDevelopment*currentRate;
            rBurn = s.tBurn*currentRate;
            rTransferAmount =  rAmount-rRfi-rLiquidity-rDevelopment-rBurn;
            return (rAmount, rTransferAmount, rRfi,0,rDevelopment,rBurn,rLiquidity);
        }
    ...
    function purchasePresale(address to, uint256 amount, bytes32[] calldata _proof) external payable {
        require(inWhitelist(_proof, msg.sender), "You are not in presale");
        require(isPresale == true, "Presale is not active");
        require((balanceOf(to) + amount) <= presaleMaxWalletAmount, "SolidBlock: Expected wallet amount exceeds the PresaleMaxWalletAmount.");

        _transfer(_msgSender(), to, amount);(msg.sender, amount);
    }
    ...
    function updateBurnWallet(address newWallet) external onlyOwner addressValidation(newWallet) {
        require(burnAddress != newWallet, 'SolidBlock: Wallet already set');
