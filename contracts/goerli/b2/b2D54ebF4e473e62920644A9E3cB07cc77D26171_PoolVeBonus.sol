...
contract PoolVeBonus is IJellyContract {    
     ...
     */
    function claimAndLock() external returns (uint) {
        IJellyPoolWrapper tokenPool = IJellyPoolWrapper(poolAddress);
        ...
        if (tokenIds.length > 0) {
            for (uint256 i = 0; i < tokenIds.length; i++) {
                ...
                rewardsLocked[tokenIds[i]] += amountAfter - amountBefore;
                require(rewardsLocked[tokenIds[i]] <= amountAfter, "Locking more tokens than has been earnt.");
            }
        }   
     ...
     */
    function claimAndLockToken(uint _tokenId) external returns (uint) {
        require(IERC721(poolAddress).ownerOf(_tokenId) == msg.sender, "Must own tokenId");
    ...
    function lockAmount(uint _tokenId, uint256 _amount) public returns (uint) {
        require(IERC721(poolAddress).ownerOf(_tokenId) == msg.sender, "Must own tokenId");
        require(_amount > 0);
    ...
    function _lockAmount(address _user, uint256 _amount) internal returns (uint) {
        require(_amount > 0);
        uint256 lockedAmount = _amount * (bonusPercentage + PERCENTAGE_PRECISION) / uint256( PERCENTAGE_PRECISION);
        ...
        return IVeToken(veToken).create_lock_for(lockedAmount, lockDuration, msg.sender);
    }
