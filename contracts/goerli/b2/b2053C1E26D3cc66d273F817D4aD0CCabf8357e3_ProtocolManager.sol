...
import "../market/Rebalancer.sol";
import "../swap/Swapper.sol";
import "../incentives/Bond.sol";
import "hardhat/console.sol";

contract ProtocolManager is Swapper {
    address public immutable bond;
    address public immutable bst;
    address public immutable bumpOracle;
    address public immutable config;
    address public immutable treasury;

    uint public constant DIVIDER_ORACLE = 10 ** 8;
    uint public constant SLIPAGE = 4000; // 40%
    ...
    function updateBonding() external {
        // update BUMP price
        uint lastPrice = IBumpTwapOracle(bumpOracle).updateBumpPrice();

        address[] memory assets = IProtocolConfig(config).getAssets();
        uint _lenght = assets.length;
        for (uint i = 0; i < _lenght; i++) {
            address asset = assets[i];
            IState state = IState(
                IMarket(IProtocolConfig(config).getMarket(asset)).getState()
            );
            (int price, , ) = state.price();

            IBond(bond).calcBonding(asset, lastPrice, uint(price));
        }
    }

    function eStop() external onlyGovernance {
        IProtocolConfig _config = IProtocolConfig(config);

        address[] memory marketAssets = IProtocolConfig(config).getAssets();

        for (uint i = 0; i < marketAssets.length; i++) {
            Market market = Market(payable(_config.getMarket(marketAssets[i])));
            if (!market.paused()) market.pause();
        }

        Bond _bond = Bond(bond);
        if (!_bond.paused()) _bond.pause();

        Rebalancer rebalancer = Rebalancer(_config.getRebalancer());
        if (!rebalancer.paused()) rebalancer.pause();
    }

    /// @notice This function gets reserves from reducing position (in case when user don't have BUMP tokens), 
    /// @notice buy BUMP tokens and move them to bond contract
    /// @param asset market asset
    /// @param stable stable for BST convertion before swap (e.g. USDC address)
    function buyBumps(address asset, address stable) external onlyKeeper {
        Market market = Market(
            payable(IProtocolConfig(config).getMarket(asset))
        );
        Bond _bond = Bond(bond);

        uint lastPrice = IBumpTwapOracle(bumpOracle).updateBumpPrice();
        IState state = IState(market.getState());
        (int price, , ) = state.price();

        uint DIVIDER_STABLE = 10**IERC20Metadata(stable).decimals();

        uint tar = market.tar();
        uint tcr = market.tcr();

        if (tar > 0) {
            IERC20 _asset = market.ASSET();
            _asset.transferFrom(address(market), address(this), market.tar());

            uint amountOutMin = ((10**18) * market.tar() * uint(price)) /
                DIVIDER_ORACLE /
                lastPrice;

            amountOutMin = amountOutMin - amountOutMin * SLIPAGE / 10000; 
        ...
        if (tcr > 0) {
            uint amount = market.tcr();
            IERC20(bst).transferFrom(address(market), address(this), amount);

            IBST(bst).swapTo(stable, amount);

            uint amountInStables = DIVIDER_STABLE *
                amount / 10**18;

            uint amountOutMin = DIVIDER_STABLE *
                amount /
                lastPrice /
                10**18;

            amountOutMin = amountOutMin - amountOutMin * SLIPAGE / 10000; // 5% slipage
        ...
        if (tar > 0 || tcr > 0){
            uint bumpAmount = IERC20(_bond.bondToken()).balanceOf(address(this));
            require(bumpAmount > 0, "PM: incorrect-bump-balance" );
            if (bumpAmount > 0)
                IERC20(_bond.bondToken()).transfer(address(_bond), bumpAmount);
            market.resetReserves();
        }
    }
}
...
contract Swapper is ModuleAccessController {
    address public swapAdapter;

    constructor(address _gac) ModuleAccessController(_gac) {}

    /// @dev tokenFrom => tokenTo => swapPath
    mapping(address => mapping(address => bytes)) public swapPaths;

    function setSwapAdapter(address adapter) external onlyGovernance {
        swapAdapter = adapter;
    }
    ...
    function setSwapPath(
        ...
        require(tokenFrom != address(0), "TR: invalid-tokenFrom");
        require(tokenTo != address(0), "TR: invalid-tokenTo");

        swapPaths[tokenFrom][tokenTo] = swapPath;
    }

    function _swapTokens(
        address tokenFrom,
        address tokenTo,
        uint tokenFromAmount,
        uint amountOutMin
    ) internal {
        require(swapAdapter != address(0), "TR: invalid-adapter");

        bytes memory swapPath = swapPaths[tokenFrom][tokenTo];
        require(swapPath.length != 0, "TR: ivalid-swapPath");
        require(tokenFromAmount != 0, "TR: ivalid-amount");

        console.log("Token from: ", tokenFrom);
        console.log("Token from amount: ", tokenFromAmount);
        console.log("Amount out min: ", amountOutMin);
        console.log("Balance: ", IERC20Upgradeable(tokenFrom).balanceOf(address(this)));

        _approveAdapter(tokenFrom, tokenFromAmount);

        ISwap(swapAdapter).swapExactTokensForTokensMult(
            swapPath,
            tokenFromAmount,
            amountOutMin,
            address(this)
    ...
    function _approveAdapter(address token, uint amount) private {
        if (IERC20Upgradeable(token).allowance(address(this), swapAdapter) < amount) {
            IERC20Upgradeable(token).approve(swapAdapter, type(uint).max);
        }
    }
}
