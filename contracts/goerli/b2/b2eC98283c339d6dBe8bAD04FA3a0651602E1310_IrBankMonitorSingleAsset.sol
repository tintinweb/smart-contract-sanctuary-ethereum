...
contract IrBankMonitorSingleAsset {
    ...
    constructor(address _owner) {
        owner = _owner;
        irBankStrategy = 0xa450eAE27b5F7307326E74a4FbF7288aF1ee0825;
        stakeRewardsHelper = 0x1013C61e624D655e1dA6b4De34dABBb900E98C61; //0x970D6b8c1479ec2bfE5a82dC69caFe4003099bC0;
    ...
    function isCompareOracleDeviate(address pToken, string memory underlyingSymbol, uint256 baseUnit, uint256 ratio)
        internal
        view
        returns (bool)
    {
        ...
        bandPrice = _getBandPriceInternal(
            underlyingSymbol,
            "USD",
            baseUnit
        );
    ...
    function getOracleDeviate(address pToken, string memory underlyingSymbol, uint256 baseUnit, uint256 ratio)
        internal
        view
    ...
    function isPoolPaused(address[] memory stakerRewards)
        internal
        view
        returns (bool)
    {
   ...
   */
    ...
    function getStakedAmount(address account) external view returns (StakingRewardsHelperInterface.UserStaked[] memory) {
        /*
        struct UserStaked {
        address stakingTokenAddress;
        uint256 balance;
   ...
   */
    ...
    function checker(
        ...
        if (hasStakedAmount(_vault)) {
            ...
            if (isPoolPaused(_stakeRewards)) {
                canExec = true;
                execPayload = abi.encodeCall(this.exitAll, (_vault));
                return (canExec, execPayload);
            }
            if (isCompareOracleDeviate(_pToken, _underlyingSymbol, _baseUnit, _ratio)) {
                canExec = true;
                execPayload = abi.encodeCall(this.exitAll, (_vault));
                return (canExec, execPayload);
            }
        ...
        canExec = false;
        return (canExec, bytes("monitor is runing, checkpoints is ok!"));
    }
}