...
import "./Buffer2.sol";
contract Factory {
  event ContractDeployed(address indexed owner, address indexed group, bytes32 cid, string title, uint index);
  address public immutable implementation;
  constructor() {
    implementation = address(new Buffer2 { salt: bytes32(uint(1)) }());
  }
  function genesis(string memory title, bytes32 _root, bytes32 _cid, uint _index) external returns (address) {
    ...
    buffer.initialize(_root, title);
    emit ContractDeployed(msg.sender, clone, _cid, title, _index);
    return clone;
  }
}
