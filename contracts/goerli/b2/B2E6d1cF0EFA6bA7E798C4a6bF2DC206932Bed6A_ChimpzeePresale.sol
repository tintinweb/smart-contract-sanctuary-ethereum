...
contract ChimpzeePresale is Ownable, Pausable, ReentrancyGuard {
    ...
    IERC20 public USDTInterface = IERC20(0x509Ee0d083DdF8AC028f2a56731412edD63223B9);
    ...
    function calculatePrice(uint256 _amount) public view returns (uint256) {
        ...
        if (_amount + totalTokensSold > stages[0][currentStage] - excessCoins || block.timestamp >= stages[2][currentStage]) {
            ...
            if (block.timestamp >= stages[2][currentStage]) {
                ...
                uint256 tokenAmountForCurrentPrice = (stages[0][currentStage] - excessCoins) - totalTokensSold;
    ...
    function changeSaleTimes(uint256 _startTime, uint256 _endTime) external onlyOwner {
        ...
        uint256 prevStartValue = startTime;
        uint256 prevEndValue = endTime;
        startTime = _startTime;
        endTime = _endTime;
        emit SaleTimeUpdated(bytes32('START'), prevStartValue, _startTime, block.timestamp);
        emit SaleTimeUpdated(bytes32('END'), prevEndValue, _endTime, block.timestamp);
    }
    ...
    function buyWithUSDT(uint256 amount) external checkSaleState(amount) whenNotPaused returns (bool) {
        ...
        totalTokensSold += newAmount;
        if (totalTokensSold > stages[0][currentStage] - excessCoins || block.timestamp >= stages[2][currentStage]) {
            if (block.timestamp >= stages[2][currentStage]) {
                excessCoins += (stages[0][currentStage] - excessCoins) - totalTokensSold;
            }
            currentStage += 1;
        }
     ...
     */
    function ethHelper(uint256 amount) external view returns (uint256 ethAmount) {
        uint256 usdPrice = calculatePrice(amount);
     ...
     * @param amount Number of tokens to buy
     */
    function usdtHelper(uint256 amount) external view returns (uint256 usdPrice) {
