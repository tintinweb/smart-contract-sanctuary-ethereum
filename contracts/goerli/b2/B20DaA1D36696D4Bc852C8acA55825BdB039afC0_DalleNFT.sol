...
contract DalleNFT is ERC721, Ownable {
  ...
  constructor(address owner_, address signer_) ERC721("DalleNFT", "DalleNFT") {
  ...
  bytes32 public constant PERMIT_TYPEHASH = keccak256("mintSigned(string metadata,string dalleId)");
  ...
  function mintSigned(
    ...
    bytes32 digest = keccak256(
      abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, keccak256(abi.encode(PERMIT_TYPEHASH, keccak256(bytes(metadata)), keccak256(bytes(dalleId)))))
    );
