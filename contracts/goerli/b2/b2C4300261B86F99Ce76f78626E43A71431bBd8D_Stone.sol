    ...
    Ownable
{
    ...
    // Constants
    uint32 public constant MAX_NFTS = 100000; // Maximum allowed number of NFTs

    // Private variables
    string private _contractURI;
    mapping(address => bool) private _blacklistedOperators;
    ...
    function _mint(address to, uint256 tokenId)
        ...
        override(ERC721)
    {
        require(totalSupply() < MAX_NFTS, "Cannot mint more NFTs");
    ...
    function devMint(
        ...
        require(
            tos_.length == tokenIds_.length,
            "Unequal addresses and token ids"
        );

        require(tos_.length == uris_.length, "Unequal addresses and uris");

        for (uint256 i = 0; i < tos_.length; i++) {
            _safeMint(tos_[i], tokenIds_[i]);
            _setTokenURI(tokenIds_[i], uris_[i]);
        }
    }
    ...
    // A function to add some operators to the blacklist
    function blacklistOperators(address[] calldata operators)
        external
        onlyRole(MINTER_ROLE)
    {
        for (uint256 i = 0; i < operators.length; i++) {
            _blacklistedOperators[operators[i]] = true;
        }
    }

    // A function to remove some operators from the blacklist
    function unBlacklistOperators(address[] calldata operators)
        external
        onlyRole(MINTER_ROLE)
    {
        for (uint256 i = 0; i < operators.length; i++) {
            _blacklistedOperators[operators[i]] = false;
        }
    }

    function _isOperatorBlacklisted(address operator)
        internal
        view
        returns (bool)
    {
        return _blacklistedOperators[operator];
    }
    ...
    function transferFrom(
        ...
        require(
            !_isOperatorBlacklisted(_msgSender()),
