...
contract ARTBYJARTHURDRAWINGS_EXTENSION is CreatorExtensionBasic {
    ...
    constructor(
        ...
        configure_contract(creator, owner, artist, artist_percentage);
    }
    ...
    function rollDicePublic() external payable {
        if (msg.value != rollPriceWei) {
            revert("Incorrect amount of ETH sent");
        }
        _rollDice();

        // split the ETH between the owner and the artist
        uint256 artistAmount = (msg.value * _artist_percentage) / 100;
        payable(_artist_wallet_address).transfer(artistAmount);
        payable(_owner_wallet_address).transfer(msg.value - artistAmount);
    }

    function rollDiceAdmin() external adminRequired {
        _rollDice();
    }
    ...
    function _rollDice() internal {
        ...
        uint256 nb_to_reveal = (random % 6) + 1;
        uint256 new_timestamp = block.timestamp + reveal_time;
        for (uint256 i = 0; i < nb_to_reveal; i++) {
            tokens[_currentRevealedIndex]
                .revealed_until_timestamp = new_timestamp;
            _currentRevealedIndex++;
            if (_currentRevealedIndex > nb_tokens_configured - 1) {
                _currentRevealedIndex = 0;
            }
        }
        emit DiceRolled(nb_to_reveal);
    }
    ...
    function getTokens() external view returns (Token[] memory) {
        Token[] memory tokens_temp = new Token[](nb_tokens_configured);
        for (uint256 i = 0; i < nb_tokens_configured; i++) {
