...
contract CollectionsCreator {
    function createToken(string memory uri)
        public
        returns (address tokenAddress)
    {
        MarketplaceERC721 collection_address = new MarketplaceERC721(uri);
        emit NewCollection(msg.sender, address(collection_address));
        return address(collection_address);
    }
    ...
    Pausable
{
    ...
    constructor(string memory uri) ERC721A("Test collection contract", "CCC") {
        baseURI = uri;
    }
    ...
    function public_mint(uint16 amount, address recipient) 
        external
        payable
        nonReentrant
        whenNotPaused {
