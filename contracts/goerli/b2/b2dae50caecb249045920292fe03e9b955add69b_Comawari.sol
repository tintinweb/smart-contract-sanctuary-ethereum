...
contract Comawari is ERC1155, Ownable {
  ...
  function mintBatchWithUri(uint[] memory _ids,string[] memory _uri) external onlyOwner {
    ...
    _mintBatch(owner(), _ids, amounts, "");

    for (uint256 i; i < _ids.length; i++) {
        tokenURI[_ids[i]] = _uri[i];
        creatorOf[_ids[i]] = owner();
    }
  }
