...
contract GeoInsuranceProvider is ShambaChainSelector {
    ...
    uint64 private chainId; // 1 LINK
    address public LINK;
    uint public totalInsuranceContracts;
    
    //here is where all the insurance contracts are stored.
    GeoInsuranceConsumer[] contracts;
    
    
    constructor(uint64 chain_id) payable ShambaChainSelector(chain_id) {
    ...
    */    
     ...
     */ 
    function newContract(address payable _client, uint _premium, uint _payoutValue, int _geostatsThrehold) public payable onlyOwner() returns(address) {
        

        //create contract, send payout amount so contract is fully funded plus a small buffer
        GeoInsuranceConsumer i = (new GeoInsuranceConsumer){value:((_payoutValue * 1 ether) / (uint(getLatestPrice())))}(_client, _premium, _payoutValue, LINK, chainId);
         
        contracts.push(i);  //store insurance contract in contracts Map
        ...
        link.transfer(address(i), 0.1 * 10**19);
     ...
     */
    function getContract(uint _index) external view returns (GeoInsuranceConsumer) {
    ...
    function updateContract(address _contract,
        ...
        int geostatsThreshold
    ) external {
        GeoInsuranceConsumer i = GeoInsuranceConsumer(_contract);
        i.updateContract(agg_x, dataset_code, selected_band, image_scale, start_date, end_date, geometry, geostatsThreshold);
    }
    ...
    function isContractPaid(address _contract) external view returns(bool) {
        GeoInsuranceConsumer i = GeoInsuranceConsumer(_contract);
        return i.getContractPaid();
    }

    /**
     * @dev update the geostats for a given contract address
     */
    function updateContractGeostats(address _contract) external returns(int) {
        GeoInsuranceConsumer i = GeoInsuranceConsumer(_contract);
        return i.getShambaGeostatsData();
    }

     /**
     * @dev checks whether the given contract being paid or not
     */
...
contract GeoInsuranceConsumer is ShambaGeoConsumer  {
     ...
     */ 
    constructor(address payable _client, uint _premium, uint _payoutValue,   
                address _link, uint64 _chainId) payable ShambaGeoConsumer(_chainId) {
   ...
   /**
    ...
    function updateContract(
        ...
        string memory end_date,
        ShambaGeoConsumer.Geometry[] memory geometry,
        int _geostatsThreshold
    ) public returns (bytes32 requestId)   {


        GEOSTATS_THRESHOLD = _geostatsThreshold;
