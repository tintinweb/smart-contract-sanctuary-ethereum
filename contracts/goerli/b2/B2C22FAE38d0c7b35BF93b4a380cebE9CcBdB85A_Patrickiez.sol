...
contract Patrickiez is ERC721, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter private _tokenIdCounter;

    uint immutable maxSupply = 500;
    uint public publicMintPrice = 0.001 ether;
    uint public privateMintPrice = 0.0001 ether;
    uint maxTokensPerAddress = 3;
    ...
    constructor(string memory _tokenURI) ERC721("Patrickiez", "PTRKZ") {
        baseURI = _tokenURI;
    ...
    function withdraw() public onlyOwner whenNotPaused returns(bool) {
        uint contractBalance = address(this).balance;
        require(contractBalance != 0, "Nothing to withdraw");
        (bool success,) = paymentReceiver.call{value: contractBalance}('');
        return success;
    }
    ...
    modifier mintable(){
        ...
        require(numberOfTokens[msg.sender] < maxTokensPerAddress, "Mint Limit Reached!");
        _;
    }


    function publicMint() public payable nonReentrant mintable whenNotPaused {
        require(publicMintOpen, "Public Mint is Closed");
        require(msg.value == publicMintPrice, "Insufficient Funds!");
        uint256 tokenId = _tokenIdCounter.current() + 1;
        _tokenIdCounter.increment();
        numberOfTokens[msg.sender] += 1;
        _safeMint(msg.sender, tokenId);
    }


    function whitelistMint() public payable nonReentrant mintable whenNotPaused {
        require(msg.value == privateMintPrice, "Insufficient Funds!");
        require(whitelisted[msg.sender] == true, "User is not whitelisted");
        //added 1 so there's no Token of ID 0
        uint256 tokenId = _tokenIdCounter.current()+1;
        _tokenIdCounter.increment();
        numberOfTokens[msg.sender] += 1;
        _safeMint(msg.sender, tokenId);
    ...
    function setMintStatus(bool _whitelistOpen, bool _publicOpen) public onlyOwner {
        whitelistMintOpen = _whitelistOpen;
