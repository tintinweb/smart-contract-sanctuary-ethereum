...
contract HEXGONETWORK is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    /// @param many how many nfts per address
    function airdrop(address[] memory addresses, uint256 many)
        external
        onlyOwner
        whenNotPaused
    {
        require(addresses.length > 0, "No addresses to receive NFTs");

        for (uint256 i = 0; i < addresses.length; i++) {
            address recipient = addresses[i];
            uint256 nftAmount = many;
            _mint(recipient, nftAmount);
        }
    }
