...
import { AutomationGuard } from "./security/AutomationGuard.sol";

/**
 * @title RPCPredictionV2
 */
contract RPCPredictionV2 is Ownable, Pausable, ReentrancyGuard, AutomationGuard {
  ...
  uint256 public minBetAmount = 1000000000000000; // 0.001 ETH, minimum betting amount (denominated in wei)
  ...
  constructor(address _oracleAddress) {
    oracle = AggregatorV3Interface(_oracleAddress); // 0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e
  ...
  /**
   * @notice Chainlink Automation Callback
   */
  // function scheduleCallback() external whenNotPaused onlyAutomation {
  ...
  /**
   * 開始下一輪(原邏輯在 executeRound() 內，但為了 scheduleCallback()，獨立 function 讓內部呼叫用)
   */
  ...
  function claimTreasury() external nonReentrant onlyOwner {
    ...
    treasuryAmount = 0;
    _safeTransferBNB(owner(), currentTreasuryAmount);
   ...
   * @dev Callable by admin
   */
  function setBufferAndIntervalSeconds(uint256 _bufferSeconds, uint256 _intervalSeconds) external whenPaused onlyOwner {
...
/**
 * 確認是由 Chainlink Automation 呼叫的
 */
abstract contract AutomationGuard is Ownable {
  address private registry;

  event NotAutomationCall(address);
  ...
  /**
   * 只能由 Chainlink Automation 呼叫
   * @notice 須先設定 registry address
   */
  modifier onlyAutomation() {
    if (registry != msg.sender) {
      emit NotAutomationCall(msg.sender);
      revert("not automation call");
    }
    _;
  }
}
