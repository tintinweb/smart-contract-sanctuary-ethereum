...
contract ChainPark  {
  ...
  function getClaimAmmount() public view returns (uint) {

    uint usersLastClaimTime = lastClaimed[msg.sender];
    ...
    if (periodsSinceClaim == 0 || periodsSinceClaim > parksSinceClaim[msg.sender]) {
      return 0;
    }

    return (periodsSinceClaim - parksSinceClaim[msg.sender]) * dailyIncome;
  }
