...
contract StaderSSVManager is Initializable, OwnableUpgradeable {
    ETHX public ethX;
    uint256 public validatorCount;
    address payable public ethValidatorDeposit;
    uint256 public minDeposit;
    uint256 public maxDeposit;
    uint256 public rewards;

    /// @notice event emitted after call function stake
    event Staked(address indexed to, uint256 EthReceived, uint256 ethXMinted);

    struct Validators {
        bytes pubKey;
        bytes[4] publicShares;
        bytes[4] encyptedShares;
        uint256[4] operatorIDs;
    }

    mapping(uint256 => Validators) validatorsRegistry;

    function initialize(ETHX _ethX) external initializer {
        ethX = _ethX;
        validatorCount = 0;
        minDeposit = 0;
        maxDeposit = 10 * 10**18;
    ...
    function stake() external payable {
        ...
        // Gets the amount of Eth locked in the contract
        // uint256 totalEth = getTotalEth();
        // Gets the amount of ethX in existence
        // uint256 totalShares = ethX.totalSupply();
        // If no ethX exists, mint it 1:1 to the amount put in
        ...
        // else {
        //     amountToSend = (amount * (totalShares)) / (totalEth);
        //     ethX.mint(msg.sender, amountToSend);
        // }
        emit Staked(msg.sender, amount, amountToSend);
    }

    function getTotalEth() internal view returns (uint256) {
        uint256 totalETH = address(this).balance +
            rewards +
            (validatorCount * 32 * 10**18);
        return totalETH;
    }
    ...
    function addToValidatorRegistry(
        ...
        bytes[4] memory _publicShares,
        bytes[4] memory _encyptedShares,
        uint256[4] memory _operatorIDs
    ) public {
        validatorCount++;
