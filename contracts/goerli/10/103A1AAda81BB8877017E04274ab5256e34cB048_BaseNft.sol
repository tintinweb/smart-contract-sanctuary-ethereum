...
contract BaseNft is ERC721URIStorage, Ownable {
    ...
    mapping(address => uint256) numberOfBaseNftsOwned;
    mapping(uint256 => address) ownerOfBaseNft;
    mapping(uint256 => string) keyForBaseNft;

    uint256 totalSupplyBaseNfts = 50;
    string svgPartOne =
    ...
    event NewBaseNftMinted(address sender, uint256 tokenId);
    ...
    function mintBaseNft() public {
        ...
        keyForBaseNft[newItemId] = nftKey;
        ...
        string memory jsonForNft = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
        ...
        string memory finalTokenUri = string(
            abi.encodePacked("data:application/json;base64,", jsonForNft)
        );
        ...
        // console.log("An Base NFT w/ ID %s has been minted to %s", newItemId, msg.sender);
        numberOfBaseNftsOwned[msg.sender]++;
        ownerOfBaseNft[newItemId] = msg.sender;

        _tokenIds.increment();
        emit NewBaseNftMinted(msg.sender, newItemId);
    }

    function setTotalSupplyBaseNft(uint256 _newTotalSupply) external onlyOwner {
        totalSupplyBaseNfts = _newTotalSupply;
    }

    function getTotalSupplyBaseNft() public view returns (uint256) {
        return totalSupplyBaseNfts;
    }
    ...
    function getNumberOfBaseNftsOwned(address _senderAddress) public view returns (uint256) {
        return numberOfBaseNftsOwned[_senderAddress];
    }

    function getOwnerOfBaseNft(uint256 _tokenId) public view returns (address) {
        return ownerOfBaseNft[_tokenId];
    }

    function getKeyForBaseNft(uint256 _tokenId) public view returns (string memory) {
        return keyForBaseNft[_tokenId];
    }
}
