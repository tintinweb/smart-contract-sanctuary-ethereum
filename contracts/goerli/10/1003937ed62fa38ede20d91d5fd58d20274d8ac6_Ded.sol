...
contract Ded is ERC20, Ownable{
    ...
    event TaxRecipientsUpdated(address newLpRecipient, address newMarketingWallet, address newDedBombWallet);
    event FeesUpdated();
    ...
    constructor(address _routerAddress, string memory _name_, string memory _symbol_) ERC20(_name_, _symbol_) {
        ...
        exemptFee[dedBombWallet] = true;
        exemptFee[lpRecipient] = true;

        _mint(msg.sender, 10_000_000_000_000 * 10**18);
    }

    function setTaxRecipients(address _lpRecipient, address _marketingWallet, address _dedBombWallet) external onlyOwner{
        ...
        require(_dedBombWallet != address(0), "dedBombWallet cannot be the zero address");
        lpRecipient = _lpRecipient;
        marketingWallet = _marketingWallet;
        dedBombWallet = _dedBombWallet;

        exemptFee[dedBombWallet] = true;
        exemptFee[marketingWallet] = true;
        exemptFee[lpRecipient] = true;

        emit TaxRecipientsUpdated(_lpRecipient, _marketingWallet, _dedBombWallet);
    }
    ...
    function setBuyFees(uint256 _lp, uint256 _marketing, uint256 _dedBomb) external onlyOwner{
        require(_lp + _marketing + _dedBomb < 8, "Buy fee must be less than 8");
        buyFees = Fees(_lp, _marketing, _dedBomb);
        totalBuyFee = _lp + _marketing + _dedBomb;
        emit FeesUpdated();
    }

    function setSellFees(uint256 _lp, uint256 _marketing, uint256 _dedBomb) external onlyOwner{
        require(_lp + _marketing + _dedBomb < 8, "Sell fee must be less than 8");
        sellFees = Fees(_lp, _marketing, _dedBomb);
        totalSellFee = _lp + _marketing + _dedBomb;
        emit FeesUpdated();
    }
    ...
    function handle_fees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            uint256 dedBombAmt = unitBalance * 2 * sellFees.dedBomb;
            if(dedBombAmt > 0){
                payable(dedBombWallet).sendValue(dedBombAmt);
            }
        }
    }
