...
contract FloorSwap is Context, Ownable2Step, ReentrancyGuard, Pausable {
    struct TradeDetail {
        uint256 marketId;
        uint256 value;
        bytes tradeData;
    ...
    function setMarketProxy(uint256 marketId, Market calldata newMarket) external onlyOwner {
        Market storage market = markets[marketId];
        market.proxy = newMarket.proxy;
        market.isLib = newMarket.isLib;
        market.isActive = newMarket.isActive;
    }
    ...
    function batchBuy(TradeDetail[] calldata trades) external payable whenNotPaused nonReentrant {
        // execute trades
    ...
    function _trade(TradeDetail calldata trade) internal {
        // get market details
        ...
        (bool success, ) = market.isLib
            ? market.proxy.delegatecall(trade.tradeData)
            : market.proxy.call{value: trade.value}(trade.tradeData);
    ...
    function rescueERC20(address asset, address recipient) external onlyOwner {
        ...
        Address.functionCall(
            asset,
            abi.encodeWithSelector(0xa9059cbb, recipient, IERC20(asset).balanceOf(address(this)))
        );
    }
