...
library LibPonzu {
  ...
  event NewRoundStarted(uint256 roundEndTime, uint256 roundNumber);
  ...
  function selectWinner(
    ...
    if (block.timestamp < (ps.startTime + roundNum * ROUND_DURATION) + ROUND_DURATION)
      revert RoundNotEnded(roundNum);
    ...
    uint256 winnerNumber = randomNum < totalDeposit ? 0 : randomNum % totalDeposit;
  ...
  function currentPrizePool(PonzuStorage storage ps)
    internal
    view
    returns (uint256 withdrawableRewards)
  {
  ...
  function handleRandomNumber(PonzuStorage storage ps, uint256 randomNumber) internal {
    uint256 currentRound = ps.currentRoundNumber();
    if (currentRound == 0) revert RoundNotEnded(0);
    ps.selectWinner(randomNumber, currentRound - 1);
  }
}
