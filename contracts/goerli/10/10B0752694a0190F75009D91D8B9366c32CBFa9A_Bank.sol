...
contract Bank {
    ...
    function safeTransferWithApprove(uint256 amountIn, uint tokenInIndex, address routerAddress) internal {
        TransferHelper.safeTransferFrom(tokenAddresses[tokenInIndex], msg.sender, address(this), amountIn);
        TransferHelper.safeApprove(tokenAddresses[tokenInIndex], routerAddress, amountIn);
    }

    function swapExactInput(uint256 amountIn, uint tokenInIndex, uint tokenOutIndex) external returns (uint256 amountOut) {
        safeTransferWithApprove(amountIn, tokenInIndex, address(swapRouter));
