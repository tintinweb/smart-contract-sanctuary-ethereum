...
IRandomConsumer
{
    ...
    function revealAll(uint from_, uint to_) onlyRole(ADMIN_ROLE) whenPaused external {
    ...
    function _fulfillReveal(uint256 tokenId_, address user_, uint256 randomness_) internal {
        uint index = _randModulus(user_, randomness_, nftIds.length());
        uint nftId = nftIds.at(index);
        nftIds.remove(nftId);
        nftToken.reveal(nftId);
        IERC721Upgradeable(address(nftToken)).safeTransferFrom(address(this), user_, nftId);
