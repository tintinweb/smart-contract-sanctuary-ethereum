...
contract NTRDAOPresale is OwnableUpgradeable {
    ...
    event RefRewardClaimBUSD(address indexed sponsorAddress, uint256 amount);
    event RefRewardClaimNTR(address indexed sponsorAddress, uint256 amount);
    event SetRefRewardBUSD(address indexed sponsorAddress, address refererAddress, uint8 round, uint256 amount);
    event SetRefRewardNTR(address indexed sponsorAddress, address refererAddress, uint8 round, uint256 amount);
    ...
    function tokenPurchaseWithBUSD(uint256 _busdAmount) public {
        ...
        for(uint8 i = 0; i < referralDeep; i++) {
            ...
            refRewardByBUSD[sponsorAddresses[i]] += bonus;
            busdForOwner -= bonus;
            emit SetRefRewardBUSD(sponsorAddresses[i], msg.sender, uint8(_round), bonus);
        }
    ...
    function tokenPurchaseWithNtr(uint256 _ntrAmount) public {
        ...
        for(uint8 i = 0; i < referralDeep; i++) {
            ...
            refRewardByNTR[sponsorAddresses[i]] += bonus;
            ntrForOwner -= bonus;
            emit SetRefRewardNTR(sponsorAddresses[i], msg.sender, uint8(_round), bonus);
        }
    ...
    function claimTokensFromBusd(uint8 _round) external {
        ...
        ntrdao.transfer(msg.sender, tokenAmount);
