...
/// @notice bToken for market
contract BToken is ERC20, ModuleAccessController, IBToken {
    uint8 internal immutable _decimals;
    address internal market;

    /// @dev global governance role
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "BT: not-minter" );
        _ ;
    }

    mapping(address => uint) public override marketSupply;
    ...
    function mint(address market, address to, uint amount) external override onlyMinter {
        _mint(to, amount);
        marketSupply[market] += amount;
        console.log("Mint: %s, to %s", amount, to );
    }

    function burn(address market, address from, uint amount) external override onlyMinter {
        console.log("Burn: %s, from %s", amount, from );        
        _burn(from, amount);
        marketSupply[market] -= amount;
    }    
}
