...
/**
 * @title MegoTickets
 * MegoTickets - ERC721 contract with on-chain metadata
 */
contract MegoTickets is ERC721, Ownable, Pausable {
    struct Ticket {
        bool exists;
        bool active;
        string name;
    ...
    mapping(address => address) public _haloToOwner;
    mapping(address => bool) public _proxies;
    mapping(uint256 => address) public _claims;
    ...
    constructor() ERC721("MegoTickets", "MEGOTICKET") {}
    ...
    function tokensOfHalo(address _halo)
        external
        view
        returns (uint256[] memory ownerTokens)
    {
        address _owner = _haloToOwner[_halo];
        uint256 tokenCount = balanceOf(_owner);
        if (tokenCount == 0) {
            // Return an empty array
            return new uint256[](0);
    ...
    function linkHalo(address _halo) external {
        require(_haloToOwner[_halo] == address(0), "Halo already linked");
        _haloToOwner[_halo] = msg.sender;
    }

    function unlinkHalo(address _halo) external {
        require(_haloToOwner[_halo] == msg.sender, "Can't manage this halo");
        _haloToOwner[_halo] = address(0);
    }
    ...
    function manageTickets(
        ...
        require(
            !_tickets[tier].exists || _tickets[tier].owner == msg.sender,
            "Can't manage this tier."
        );
        _tickets[tier].owner = msg.sender;
