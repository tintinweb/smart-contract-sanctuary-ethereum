...
contract Ethernaut is Ownable {
  ...
  uint256 public endTime = MAX_INT;
  ...
  // Players may only create and submit level instances during the specified time frame.
  function setTimeFrame(uint256 start, uint256 end) public onlyOwner {
    require(start <= end, "Invalid Time Frame");
    startTime = start;
    endTime = end;
  }

  modifier timeSensitive() {
    require(startTime <= block.timestamp, "Competition has not started yet.");
    require(endTime >= block.timestamp, "Competition has ended.");
    _;
  }
  ...
  function createLevelInstance(Level _level) public payable timeSensitive {
  ...
  function submitLevelInstance(address payable _instance) public timeSensitive {
