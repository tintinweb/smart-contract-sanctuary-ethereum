...
contract StrategiesJob {
  ...
  event Worked(address _strategy, uint256 _workedAt);
  ...
  function addMultipleStrategies(address[] calldata _strategies) external {
  ...
  function work(
    ...
    if (!workable(_strategy, _trigger)) revert NotWorkable();
    workHard(_loop);
    lastWorkAt[_strategy] = block.timestamp;
    emit Worked(_strategy, block.timestamp);
  }

  function workable(address _strategy, bool _trigger) public view returns (bool) {
    if (!_availableStrategies.contains(_strategy)) revert StrategyNotAdded();
    return block.timestamp > (lastWorkAt[_strategy] + workCooldown) && _trigger;
  }
