...
error ConsecutiveTransfersUnsupported();
error InvalidOwnerOrUnapproved();

abstract contract HDMintExtended is Initializable, ERC721Upgradeable {
    mapping(uint256 => string) private _tokenURIs;
    mapping(address => mapping(uint256 => uint256)) internal _ownedTokens;

    function __HDMintExtended_init() internal onlyInitializing {}
    ...
    function burn(uint256 tokenId) external {
        if (!_isApprovedOrOwner(_msgSender(), tokenId)) revert InvalidOwnerOrUnapproved();
    ...
    function _beforeTokenTransfer(
        ...
        if (batchSize > 1) {
            revert ConsecutiveTransfersUnsupported();
        }

        if (to != from) {
            if (to != address(0)) _addTokenToOwnerEnumeration(to, firstTokenId);
            if (from != address(0)) _removeTokenFromOwnerEnumeration(from, firstTokenId);
        }
    }
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        _ownedTokens[to][balanceOf(to)] = tokenId;
    }
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        while (tokenIndex <= lastTokenIndex) {
            if (_ownedTokens[from][tokenIndex] == tokenId) break;
            unchecked {
                tokenIndex++;
            }
        }
...
import "./extensions/HDMintExtended.sol";
    ...
    IHDMint
{
    ...
    function walletOfOwner(address _owner) external view returns (uint256[] memory) {
        ...
        while (i < tokenCount) {
            tokensId[i] = _ownedTokens[_owner][i];
            unchecked {
                i++;
            }
        }
