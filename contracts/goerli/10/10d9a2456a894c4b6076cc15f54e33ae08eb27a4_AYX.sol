...
contract AYX is ERC721Enumerable, ReentrancyGuard, Ownable, Pausable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant MAX_NFT = 10000;
    uint256 public MINT_PRICE = 0.05 ether;
    uint256 public lastMintPriceUpdatePoint;
    ...
    uint256 public publicSaleDuration = 10 minutes;

    constructor() ERC721("Anonymous x club", "AYX") {        
    }    
    ...
    // Function to update public sale duration
    function updateSaleDuration(uint256 duration) external onlyOwner {
    ...
    function mint(uint256 _numOfTokens) public payable whenNotPaused nonReentrant {
        if (publicSaleStartTimeStamp + publicSaleDuration < block.timestamp) isPublicSaleActive = false;
        require(balanceOf(msg.sender) + _numOfTokens <= PURCHASE_LIMIT, "Exceed max wallet limit");
        require(_numOfTokens + totalSupply() <= MAX_NFT, "Exceed max NFT limit");
        if (isVIP[msg.sender]){
            require(isPublicSaleActive || isPreSaleActive, "Sale not active");
            for(uint256 i = 0; i < _numOfTokens; i++) {
                _mint(msg.sender, totalSupply());                
            }
        }
        else if (isWhiteList[msg.sender]){
            require(isPreSaleActive || isPreSaleActive, "Sale not active");
            require(MINT_PRICE.mul(_numOfTokens) == msg.value, "Invalid Amount");
            for(uint256 i = 0; i < _numOfTokens; i++) {
                _mint(msg.sender, totalSupply());                
            }
        }
        ...
        // reward if own 3 NFT
        if (balanceOf(msg.sender) == PURCHASE_LIMIT){
            _mint(msg.sender, totalSupply());
        }
        if (totalSupply() >= lastMintPriceUpdatePoint + 5){
            MINT_PRICE = MINT_PRICE.add(0.015 ether);
            lastMintPriceUpdatePoint = lastMintPriceUpdatePoint + 5;
        }
    }    

    // Function to airdrop unclaimed
    function airdropNFT(address _to, uint256 _numOfTokens) external onlyOwner {
        require(_numOfTokens + totalSupply() <= MAX_NFT, "Exceed max NFT limit");
        for(uint256 i=0; i < _numOfTokens; i++) {            
