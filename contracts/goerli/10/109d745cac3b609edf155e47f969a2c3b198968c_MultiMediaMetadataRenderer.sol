...
/// @notice Edition renderer that supports multiple types of media
contract MultiMediaMetadataRenderer is
    IMetadataRenderer,
    MetadataRenderAdminCheck
{
    ...
    event EditionInitialized(
        ...
        string description,
        string[] imageURIs,
        string[] animationURIs,
        bytes32 randomSeed
    );

    // it's like an edition but with…lots of media
    ...
    /// @notice Add a pair of media URIs to the token
    /// @param target target for contract to update metadata for
    /// @param imageURIs new image uri addresses
    /// @param animationURIs new animation uri addresses
    function addMedia(
        address target,
        string[] calldata imageURIs,
        string[] calldata animationURIs
    ) external requireSenderAdmin(target) {
        TokenEditionInfo storage tokenInfo = tokenInfos[target];

        uint256 imagesLength = imageURIs.length;
        uint256 animationsLength = animationURIs.length;

        require(
            imagesLength == animationsLength,
            "arrays should be same length"
        );

        for (uint256 i = 0; i < imagesLength; ) {
            tokenInfo.imageURIs.push(imageURIs[i]);
            tokenInfo.animationURIs.push(animationURIs[i]);
    ...
    function info(address target)
        public
        view
        returns (TokenEditionInfo memory)
    {
        TokenEditionInfo memory tokenInfo = tokenInfos[target];
    ...
    function initializeWithData(bytes memory data) external {
        // data format: description, imageURIs, animationURIs, assignRandomish
        (
            string memory description,
            bytes memory imageURIsEncoded,
            bytes memory animationURIsEncoded,
            bool assignRandomish
        ) = abi.decode(data, (string, bytes, bytes, bool));

        string[] memory imageURIs = abi.decode(imageURIsEncoded, (string[]));
        string[] memory animationURIs = abi.decode(
            animationURIsEncoded,
            (string[])
        );
    ...
    function contractURI() external view override returns (string memory) {
        ...
        if (bytes(tokenInfos[target].imageURIs[0]).length > 0) {
            imageSpace = abi.encodePacked(
                '", "image": "',
                tokenInfos[target].imageURIs[0]
            );
        }
        return
            string(
        ...
        returns (string memory)
    {
        ...
        uint256 choices = tokenInfo.imageURIs.length;

        uint256 choice;
        unchecked {
            if (tokenInfo.randomSeed != bytes32(0x0)) {
                choice =
                    (((uint256(tokenInfo.randomSeed) / 1000) * (tokenId + 3)) %
                        choices) +
                    1;
            } else {
                choice = (tokenId - (1 % choices)) + 1;
            }
        }

        string memory imageUrl = tokenInfo.imageURIs[choice];
        string memory animationUrl = tokenInfo.animationURIs[choice];
