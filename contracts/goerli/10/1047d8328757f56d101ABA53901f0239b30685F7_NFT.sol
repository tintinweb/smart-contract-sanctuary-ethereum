...
contract NFT is ERC721URIStorage {
    ...
    function getNftsCreatedByMe(address owner)
        external
        view
        returns ( NFTs[] memory)
    {
        ...
        uint256[] memory tokenid = _myTokenIds[msg.sender];

        NFTs[] memory items = new NFTs[](numberOfTokensOwned);
        
        // uint256 currentIndex = 0;

        for (uint256 i = 0; i < numberOfTokensOwned; i++) {
              uint256 token = tokenid[i];
