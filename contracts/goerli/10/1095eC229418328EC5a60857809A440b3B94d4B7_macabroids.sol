...
contract macabroids is ERC721A, Ownable {
    ...
    address t2 = 0xf93DB85964a30F38f05DcBFD9dD23dfD9D9004d8;
    ...
    function mint(uint256 _count, bytes32[] calldata _merkleProof) external payable {
    ...
    function getCost (address _address, uint256 _count, bytes32[] calldata _merkleProof) public view returns (uint256) {
    ...
    function canClaim(address _wallet, bytes32[] calldata _merkleProof) public view returns (bool) {
        return(
            mintedByAddress[_wallet] < freePerWallet && totalSupply() < maxFreeSupply && isWhitelisted(_wallet, _merkleProof)
            );
    }
