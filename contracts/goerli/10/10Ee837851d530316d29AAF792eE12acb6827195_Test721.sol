...
contract Test721 is PremintReady, ERC721A("Test721", "Test721") {
...
///
///         2) Implement the function _premint(address to, uint256 amount) so when it is called it mints `amount`
///            tokens to the address `to`
///
///         3) Connect to https://premint.xyz with the current contract Owner and set up all the configuration your need.
///
///         4) Premint will then know how to communicate with your contract and offer safe minting through the Premint interface
...
abstract contract PremintReady is IPremintReady, EIP712 {
	...
	mapping(address => mapping(bytes32 => uint256)) public allowanceUsed;
	...
	function premint(PremintConfig calldata config, uint256 amount) public payable virtual {
		...
		// Account check: allowance must be for current msg.sender
		if (msg.sender != config.allowance.account) {
		...
		// Amount check: the account can not mint more than their allowance (even if they mint in more than one call)
		if (allowanceUsed[msg.sender][config.allowance.listId] + amount > config.allowance.amount) {
		...
		// use allowance before any other interaction, blocks reEntrancy
		allowanceUsed[msg.sender][config.allowance.listId] += amount;
	...
	/// @dev this is the function that you need to implement in your contract for them to be PremintReady
	/// @param to the recipient of the mint
	/// @param amount the amount of items to mint
