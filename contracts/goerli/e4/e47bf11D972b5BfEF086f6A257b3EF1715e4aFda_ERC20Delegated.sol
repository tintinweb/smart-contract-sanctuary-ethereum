...
import "../WhitelistPing/Shared/Delegated.sol";
...
contract ERC20Delegated is Delegated, ERC20 {
  ...
  function withdraw(address token, uint256[] calldata tokenIds) external onlyDelegates{
  ...
  function mintTo( uint[] calldata quantities, address[] calldata accounts ) external onlyDelegates{
    require( quantities.length == accounts.length, "equal counts required" );
