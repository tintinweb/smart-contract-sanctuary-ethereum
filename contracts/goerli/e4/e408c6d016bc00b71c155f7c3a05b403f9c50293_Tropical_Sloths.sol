...
contract Tropical_Sloths is ERC721A, Ownable {
  ...
  function mintPublic(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
   ...
   require(totalSupply() + _mintAmount <= 12000, "Public mint is over");
...
//Can only be called by given msg.sender
   function crossmint_Public(uint256 _mintAmount, address _to) public payable {
   // require(msg.value >= cost * _mintAmount, "Insufficient funds!");
    require(_numberMinted(_to) + _mintAmount <= MAxmintWL , "Exceeds Per wallet limit!");
