...
contract PepePills is IERC721A { 
    ...
    modifier mintPriceCompliance(uint256 amount) {
        ...
        if(MAX_FREE_PER_WALLET > 0 && freeMintClaimed[_caller] < MAX_FREE_PER_WALLET) {
            require(msg.value >= COST * (amount-(MAX_FREE_PER_WALLET - freeMintClaimed[_caller])), 'Insufficient funds!');
            if(amount <= MAX_FREE_PER_WALLET - freeMintClaimed[_caller]) {
                freeMintClaimed[_caller] += amount;
            } else {
                freeMintClaimed[_caller] = MAX_FREE_PER_WALLET;
            }
            _;
        } else {
