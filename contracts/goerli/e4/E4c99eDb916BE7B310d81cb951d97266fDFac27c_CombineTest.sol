...
contract CombineTest is ERC721A, Ownable, IERC4906 {
    ...
    mapping(uint256 => uint256) newValues;
    mapping(uint256 => uint256) baseColors;

    constructor() ERC721A("CombineTest", "CBT") {}
    ...
    function combine(uint256[] memory tokens) public {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            newValue =
                newValue *
                10**utils.numDigits(getValue(tokens[i])) +
                getValue(tokens[i]);
        }
        if (newValue > 9999) {
            revert("number must be 9999 or less");
        }
        if (luckyNumber != 0 && newValue == luckyNumber) {
            revert(
                string.concat(
                    "can't combine because lucky number is ",
                    utils.uint2str(luckyNumber)
                )
            );
        }
    ...
    function randomLuckyNumber() public onlyOwner {
        luckyNumber = utils.random(block.timestamp, 1000, 9999);
    }
...
library segments {
    ...
    function getNumbers(uint256 input, uint256 length)
        internal
        pure
        returns (Number memory result)
    {
        if (length == 1) {
            result.d = input;
        } else if (length == 2) {
    ...
    function getNumberStyle(uint256 position, uint256 input)
        internal
        pure
        returns (string memory)
    {
    ...
    function renderSvg(uint256 value, uint256[3] memory rgbs)
        internal
        pure
        returns (string memory svg)
    {
        ...
        uint256 length = bytes(utils.uint2str(value)).length;
...
library utils {
    ...
    function numDigits(uint256 _num) internal pure returns (uint256) {
        uint256 digits = 0;
        while (_num > 0) {
            digits++;
            _num /= 10;
