...
contract OffChainAsset is Owned, IAsset {
  ...
  uint public redeem_amount;

  uint public current_value;
  uint public valuation_time;
   ...
   * @param amount The amount to deposit in the asset.
   * @dev tracks the time when current_value was updated.
   */
  function deposit(address token, uint amount) public onlyStabilizer {
    if(wallet == address(0)) { revert WalletNotSet(); }
    TransferHelper.safeApprove(token, msg.sender, amount);
    TransferHelper.safeTransferFrom(token, stabilizer, wallet, amount);
    current_value += token == address(sweep) ? SWEEPinUSDX(amount, sweep.target_price()) : amount;
   ...
   * @dev tracks the time when current_value was updated.
   */
  function withdraw(address token, uint amount) public onlyStabilizer {
    redeem_token = token;
