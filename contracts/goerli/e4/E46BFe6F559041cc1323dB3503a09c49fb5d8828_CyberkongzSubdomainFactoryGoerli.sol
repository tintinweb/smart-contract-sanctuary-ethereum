...
contract CyberkongzSubdomainFactoryGoerli is Ownable {
	...
	// Used to update the name attached to an asset. Must be of equal length or longer
	// function updateSubdomain(string calldata _label, uint256 _tokenId, address _nft, address _bind) external {
	// 	(bytes32 nameHash, bytes32 subnode, bytes32 nftData) = _checkParams(_label, _tokenId, _nft);
	// 	bytes32 attachedHash = nftToHash[nftData];
	// 	uint256 labelLength = bytes(_label).length;
	// 	require(labelLength >= bytes(hashToStr[attachedHash]).length); // new subdomain must be of equal length or bigger
	// 	require(nftDataToExpiryDate[attachedHash] > 0); // subdomain doesn't exist

	// 	hashToNft[nameHash] = nftData;
	// 	nftToHash[nftData] = nameHash;
	// 	nftDataToExpiryDate[nameHash] = nftDataToExpiryDate[attachedHash];
	...
	// 	if (attachedHash != bytes32(0)) {
	// 		delete hashToNft[attachedHash];
	// 		delete nftDataToExpiryDate[attachedHash];
	// 		resolver.setAddr(keccak256(abi.encodePacked(KONGZ_NODE, attachedHash)), address(0));
	// 	}
	// 	resolver.setAddr(subnode, _bind);
	// }
