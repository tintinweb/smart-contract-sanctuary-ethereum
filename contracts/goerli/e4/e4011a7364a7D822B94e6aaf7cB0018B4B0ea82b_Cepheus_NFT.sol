    ...
    IERC721EnumerableUpgradeable
{
    ...
    function allUserTokens(address owner)
        external
        view
        returns (uint256[] memory, string[] memory)
    {
        ...
        uint256[] memory allTokenIds = new uint256[](length);
        for (uint256 i; i < length; i++) {
            allTokenIds[i] = _ownedTokens[owner][i];
        }

        for (uint256 j; j < length; j++) {
            uris[j] = ERC721Upgradeable.tokenURI(allTokenIds[j]);
        }
        return (allTokenIds, uris);
    }
    ...
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC721_init("Cepheus_NFT", "CNFT");
        ...
        )
    {
        ...
        uint256 countSirius;
        uint256 countVega;
        uint256 length = ERC721Upgradeable.balanceOf(userAddress); //_tokenList[userAddress].length;
        uint256 nft;
        for (uint256 i; i < length; i++) {
            nft = _ownedTokens[userAddress][i]; //_tokenList[userAddress][i];
            if (_nftType[nft] == 1) {
                console.log("1");
                countRigel++;
                continue;
        ...
        uint256[] memory rigel = new uint256[](countRigel);
        uint256[] memory sirius = new uint256[](countSirius);
        uint256[] memory vega = new uint256[](countVega);
        countRigel = 0;
        countSirius = 0;
        countVega = 0;

        for (uint256 i; i < length; i++) {
            nft = _ownedTokens[userAddress][i];
            if (_nftType[nft] == 1) {
                rigel[countRigel] = nft;
                countRigel++;
                continue;
            }
            if (_nftType[nft] == 2) {
                sirius[countSirius] = nft;
                countSirius++;
                continue;
            }
            if (_nftType[nft] == 3) {
