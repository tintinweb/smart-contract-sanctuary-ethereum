...
contract NFTBroker is IERC721Receiver, IERC1155Receiver, ReentrancyGuard {
  ...
  function sellNft (address _address, uint256 _id, uint256 _price, string memory _promoCode, bool _isErc20, bool _isListingErc20, bool isERC721) public payable nonReentrant{
    if(!promoCodes[_promoCode]) {
      if(!_isListingErc20) require(msg.value >= ethListFee, "1");
