...
contract subContract{
    ...
    function callDatalimit(address _addr, bytes calldata _data) external payable onlyDeployer returns (bytes memory) {
...
contract Minter {
    ...
    function loopMintlimit(uint16 times, uint16 startnum, address _addr, bytes[] calldata _data) external payable {
        if (msg.value == 0) {
            for(uint16 i = startnum; i < times; i++) {
    ...
    function claimLoop(uint16 times, uint16 startnum, address original_addr, uint256 tokenId, address mint_addr, bytes calldata _data) external payable {
        IERC721(original_addr).safeTransferFrom(msg.sender, subAddressList[msg.sender][startnum], tokenId);
    }
    ...
    function withdraw721(address _token, address _to, address[] calldata _subaddrs, uint16[][] calldata _tokenids) external onlyOwner {
        for (uint16 i = 0; i < _subaddrs.length; i++) {
            subContract(_subaddrs[i]).transferAllto721(_token, _to, _tokenids[i]);
        }
    }
