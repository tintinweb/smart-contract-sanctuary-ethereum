...
contract DigitsRedeemer is IDigitsRedeemer, Ownable {
    ...
    mapping(uint => bool) private stakedTokens;
    ...
    constructor() {
        DigitsAgents = IERC721Enumerable(0xa7721FD27D49f6de9b8655A902DC97f87b32A141);
    }
    ...
    */
    ...
    function stake(uint[] calldata tokenIds, bool isFoundingAgents, uint timePeriod, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) external {
        ...
        if (isFoundingAgents) {
            ...
            for (uint i = 0; i < tokenIds.length; i++) {
                ...
                require(!stakedTokens[tokenIds[i]], "Token is already Staked!");
    ...
    function emergencyUnstake(address[] calldata unstakers, bool isFoundingAgents) external {
    ...
    function _unstake(address unstaker, bool isFoundingAgents) internal {
        if (isFoundingAgents) {
            for (uint i; i < addressToFoundingAgentsStaked[unstaker].tokens.length; i++) {
                stakedTokens[addressToFoundingAgentsStaked[unstaker].tokens[i]] = false;
            }
            ...
            for (uint i; i < addressToDigitsAgentsStaked[unstaker].tokens.length; i++) {
                stakedTokens[addressToDigitsAgentsStaked[unstaker].tokens[i]] = false;
            }
