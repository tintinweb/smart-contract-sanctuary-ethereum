...
contract IDOContract is Ownable, ReentrancyGuard {
    ...
    function verifySignature(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(_buyer, _currency, _amountIn, _amountOut, _amountOutLimit, _price, _validBefore, _nonce)
        );
    ...
    function buy(
        ...
        require(
            verifySignature(
                _buyer, _currency, _amountIn, _amountOut, _amountOutLimit, _price, _validBefore, _nonce, v, r, s
            )
        );
        lockERC20(_amountIn, _currency);
        totalAmountSold += _amountOut;
        emit BuyEvent(_buyer, _currency, _amountIn, _amountOut, _amountOutLimit, _price, _validBefore, _nonce);
    }
    ...
    function withdraw(address payable _to, uint256 amount) public payable onlyOwner nonReentrant {
        assert(address(this).balance >= amount);
        _to.transfer(amount);
    }

    function withdraw(address _to, uint256 amount, address asset) public onlyOwner nonReentrant {
