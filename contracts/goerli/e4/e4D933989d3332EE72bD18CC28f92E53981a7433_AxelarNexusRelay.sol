...
abstract contract AbstractNexusRelay is Ownable, INexusRelay {
	...
	constructor(address _nexusAddress, uint8 _providerId) {
	...
	function transmit(
		...
		_transmit(destinationChainId, message, _token, amount);
	}
...
contract AxelarNexusRelay is IAxelarExecutable, AbstractNexusRelay {
	...
	constructor(
		address _owner,
		uint8 _providerId
	) AbstractNexusRelay(_owner, _providerId) {
		_fillChainDetails();
