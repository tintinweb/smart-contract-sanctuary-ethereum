...
contract TestTokenVesting is Ownable {
    ...
    function startVesting() external onlyOwner{
        require(vestingStartTS == 10000000000, "Vesting start timestamp already set");
    ...
    {
        require(privateRoundInvestors[_beneficiary].amount > 0, "Not correct beneficiary");
        require(
        ...
        returns (uint256)
    {
        ...
        if (currentTime < _privateRoundInvestor.start + slicePeriodSeconds * 2) {
            return 0;
        } else if (currentTime >= _privateRoundInvestor.start + vestingSchedules[1].duration) {
