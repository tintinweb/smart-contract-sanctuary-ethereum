...
contract EarlyMint is ReentrancyGuard, Ownable {
    ...
    event SpotReserved(uint256 batchId, address minter, uint8 quantity, uint256 quantityOwned, uint256 spotsTaken);
    ...
    function createBatch(string memory _externalId, uint256 _price, uint256 _maxSpots, uint256 _spotsPerWallet, address _nftContractAddress, address _creator) public onlyOwner nonReentrant returns (uint) {
    ...
    function reserveSpot(uint256 _batchId, uint8 _requestedSpotQuantity) public payable nonReentrant {
        ...
        emit SpotReserved(_batchId, msg.sender, _requestedSpotQuantity, paidSpots[_batchId][msg.sender], batchesList[_batchId].spotsTaken);
    }
