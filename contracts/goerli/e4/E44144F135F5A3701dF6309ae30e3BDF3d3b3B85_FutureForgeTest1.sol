...
contract FutureForgeTest1 is AccessControl, ERC1155Holder {
  using Strings for uint256;

  uint256 constant _dragonId = 1;
  ...
  function forge(uint256 _dragonQuantity) public returns (uint256) {
    ...
    require(_dragonQuantity % 3 == 0, "Dragon quantity must be divisible by 3");

    require(
      _futureQuestContract.balanceOf(msg.sender, _dragonId) >= _dragonQuantity,
    ...
    uint256 artifactId = _futureQuestContract.create(1);

    console.log("[Forge] artifactId", artifactId);

    mintArtifact(msg.sender, artifactId);

    return artifactId;
  }

  function mintArtifact(address _user, uint256 _artifactId) public {
