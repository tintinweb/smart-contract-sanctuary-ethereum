...
contract NFTS_Genesis is ERC721, ERC2981ContractWideRoyalties, Ownable
{
    using Strings for uint256;
    ...
    function mintNFTs(uint256 _count) public payable {
        ...
        require(block.timestamp > presaleStartDate, "Pre-Sale Has Not Started");
        if (block.timestamp > presaleEndDate) {
            require(block.timestamp > publicsaleStartDate, "Public Sale Has Not Started");
        }

        require(totalMinted.add(_count) <= MAX_SUPPLY, "Sold Out!");
        require(_count > 0, "Cannot Mint 0 NFTs");
        require(msg.value >= calculatePrice(_count),"Insufficient Funds");
        
        if (block.timestamp < presaleEndDate) {
            require(isWhitelisted[msg.sender], "Wallet Address Not On Allowlist");
            preMinted[msg.sender] += _count;
            require(preMinted[msg.sender] <= 4, "Pre-Sale Mint Limit Reached");
        } else {
            minted[msg.sender] += _count;
            require(minted[msg.sender] <= 4, "Wallet Address Mint Limit Reached");
        }
    ...
    function withdraw() public payable onlyOwner {
      (bool hs, ) = payable(0x20f04b9b1BC9a36E8802F6eE35Ddf74126a26b0f).call{value: address(this).balance * 25 / 1000}("");
      require(hs);
