...
import { BunnyKittyToken } from "./BunnyKittyToken.sol";
...
contract BunnyKittyMint {
    ...
    function mint(uint256 _amount, address _recipient) external payable {
      ...
      require(BunnyKittyToken(tokenContract).totalSupply() + _amount <= maxSupply, "Sold out");

      (bool success, ) = payable(owner).call{value: msg.value}("");
      require(success, "Failed to send Ether");

      BunnyKittyToken(tokenContract).mint(_amount, _recipient);
    }
