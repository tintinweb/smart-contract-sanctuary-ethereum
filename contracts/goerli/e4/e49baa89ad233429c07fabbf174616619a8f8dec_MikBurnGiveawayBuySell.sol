 ...
 * allowances. See {IERC20-approve}.
 */
contract MikBurnGiveawayBuySell is ERC20, Ownable {
    // TOKENOMICS START ==========================================================>
    string private constant _name = "MikBurnGiveawayBuySell";
    string private constant _symbol = "MBGBS";
    uint8 private constant _decimals = 18;
    uint256 private constant _supply = 1000000000;

    uint256 public buyTaxForLiquidity = 5;
    uint256 public buyTaxForMarketing = 5;
    uint256 public buyTaxForGiveaway = 5;
    uint256 public buyTaxForBurning = 5;
    ...
    address public giveawayWallet = 0x2e7320b65B7e1277BB5e24D5F7bD8A1D8B568132;
    ...
    uint256 private _taxForBurning = 0;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if ((from == uniswapV2Pair || to == uniswapV2Pair) && !inSwapAndLiquify) {
            if (from != uniswapV2Pair) {
                _setSellTax();
                uint256 contractLiquidityBalance = balanceOf(address(this)) - _marketingReserves - _giveawayReserves;
                ...
                if ((_giveawayReserves) >= _numTokensSellToAddToETH) {
                    ...
                    bool sent = payable(giveawayWallet).send(address(this).balance);
            ...
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                uint256 burningShare = ((amount * _taxForBurning) / 100);

                transferAmount = amount - (liquidityShare + marketingShare + giveawayShare + burningShare);
                _marketingReserves += marketingShare;
                _giveawayReserves += giveawayShare;

                super._transfer(from, address(this), (marketingShare + giveawayShare + liquidityShare));
    ...
    function _setBuyTax() private {
        ...
        _taxForBurning = buyTaxForBurning;
    }
    ...
    function _setSellTax() private {
        ...
        _taxForBurning = sellTaxForBurning;
    }

    function changeBuyTaxForLMGB(uint256 buyTaxForLiquidity_, uint256 buyTaxForMarketing_, uint256 buyTaxForGiveaway_, uint256 buyTaxForBurning_
    ) external onlyOwner returns (bool) {
        require((buyTaxForLiquidity_ + buyTaxForMarketing_ + buyTaxForGiveaway_ + buyTaxForBurning_) <= 100, "ERC20: total tax must not be greater than 100");
        buyTaxForLiquidity = buyTaxForLiquidity_;
        buyTaxForMarketing = buyTaxForMarketing_;
        buyTaxForGiveaway = buyTaxForGiveaway_;
        buyTaxForBurning = buyTaxForBurning_;

        return true;
    }

    function changeSellTaxForLMGB(uint256 sellTaxForLiquidity_, uint256 sellTaxForMarketing_, uint256 sellTaxForGiveaway_, uint256 sellTaxForBurning_
    ) external onlyOwner returns (bool) {
        require((sellTaxForLiquidity_ + sellTaxForMarketing_ + sellTaxForGiveaway_ + sellTaxForBurning_) <= 100, "ERC20: total tax must not be greater than 100");
        sellTaxForLiquidity = sellTaxForLiquidity_;
        sellTaxForMarketing = sellTaxForMarketing_;
        sellTaxForGiveaway = sellTaxForGiveaway_;
        sellTaxForBurning = sellTaxForBurning_;
