...
/**
 * @title A collection of NFTs for a fixed price mint drop.
 */
contract FixedPriceCollection is
  AccessControlUpgradeable,
  ERC721BurnableUpgradeable,
  ERC721RoyaltyUpgradeable,
  IFixedPriceCollection,
  IGetRoyalties,
  IGetFees,
  IRoyaltyInfo
{
  ...
  // TODO: Re-arrange variables below for optimal packing.

  /**
   * @notice Base URI for the Collection.
   * @dev TokenId is naively concatenated with the baseURI to obtain an NFT in the collection.
   * e.g. The address for `tokenId`: '1' with `baseURI`: "ipfs://foo/" is "ipfs://foo/1".
   */
  string public baseURI;
  /**
   * @notice Hash of the `baseURI` for the collection.
   * @dev The value is set bytes(0) if the collection is public by default.
   */
  ...
  */
  ...
  /**
   * @notice The creator of this NFT collection.
   */
   ...
   */
  ICollectionFactory public immutable collectionDropFactory;

  /**
   * @notice Role with minting privileges.
   */
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

  /**
   * @notice Default royalty cut paid out on secondary sales.
   * @dev Set to 10% of the seconday sale.
   */
   ...
   * @param baseURI The baseURI for the collection.
   * @param baseURIHash The hash of the revealed baseURI for the collection.
   * @dev If baseURIHash is signals that the collection has been revealed.
   * When both baseURI and baseURIHash are set, baseURI is the pre-reveal content
   * and means the collection has not been revelaed yet.
   */
  event URIUpdated(string baseURI, bytes32 baseURIHash);

  /**
   * @notice Initialize the template's immutable variables.
   * @param _collectionDropFactory The factory which will be used to create collection contracts.
   */
  constructor(address _collectionDropFactory) {
    require(_collectionDropFactory.isContract(), "FixedPriceCollection: collectionDropFactory is not a contract");
    collectionDropFactory = ICollectionFactory(_collectionDropFactory);
  }
  ...
  function initialize(
    ...
    require(msg.sender == address(collectionDropFactory), "FixedPriceCollection: Must be created via the factory");
    require(bytes(_symbol).length > 0, "FixedPriceCollection: `_symbol` must be set");
    require(_creator != address(0), "FixedPriceCollection: `_creator` must be set");
    require(_paymentAddress != payable(0), "FixedPriceCollection: `_paymentAddress` must be set");
    require(bytes(_tokenURIPreReveal).length > 0, "FixedPriceCollection: `_tokenURIPreReveal` must be set");
    require(_maxTokenId > 0, "FixedPriceCollection: `_maxTokenId` must be set");
    _setDefaultRoyalty(_paymentAddress, ROYALTY_IN_BASIS_POINTS);
    __ERC721_init(_name, _symbol);
    _setupRole(DEFAULT_ADMIN_ROLE, _creator);
    _setupRole(MINTER_ROLE, _creator);
    if (_additionalMinterAddress != address(0)) {
      _setupRole(MINTER_ROLE, _additionalMinterAddress);
    }

    creator = _creator;
    baseURI = _tokenURIPreReveal;
   ...
   */
  function burn(uint256 tokenId) public virtual override(ERC721BurnableUpgradeable) onlyRole(DEFAULT_ADMIN_ROLE) {
    super.burn(tokenId);
  }
   ...
   */
  function mintCountFor(uint256 count, address to) external onlyRole(MINTER_ROLE) returns (uint256 firstTokenId) {
    require(latestTokenId + count <= maxTokenId, "FixedPriceCollection: Exceeds max tokenId");
   ...
   * @dev a) The admin for the collection can reveal/change the baseURI
   *         for the collection anytime and any number of times.
   *      b) use updatePreRevealContent() to update pre-reveal content not reveal()
   */
  function reveal(string memory _baseURI) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(bytes(_baseURI).length != 0, "FixedPriceCollection: `_baseURI` must be set.");
    delete baseURIHash;
    baseURI = _baseURI;
    emit URIUpdated(baseURI, baseURIHash);
  }
  ...
  function selfDestruct() external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(totalSupply() == 0, "FixedPriceCollection: Any NFTs minted must be burned first");
   ...
   * @dev a) Use reveal() to reveal collection not this fn.
   *      c) This fn should only be used if:
   *         i) the unrevealed baseURI has changed and the baseURIHash needs to
   *            be updated (set `_baseURIHash`)
   *         ii) pre-reveal URI needs to be updated (set `_baseURI`)
   */
  function updatePreRevealContent(string memory _baseURI, bytes32 _baseURIHash) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(baseURIHash != bytes32(0), "FixedPriceCollection: Already revealed");
    require(_baseURIHash != bytes32(0), "FixedPriceCollection: use `reveal` instead");
    // require baseURI be set, just like in the initializer to avoid empty preReveal URIs.
    require(bytes(_baseURI).length > 0, "FixedPriceCollection: `_baseURI` must be set");
   ...
   * @dev Once this value has been set, it may be decreased but can't be increased if a sale has happened.
   * @param _maxTokenId The max tokenId to set, all NFTs must have a tokenId less than or equal to this value.
   */
  function updateMaxTokenId(uint32 _maxTokenId) external onlyRole(DEFAULT_ADMIN_ROLE) {
    _updateMaxTokenId(_maxTokenId);
  }

  function _updateMaxTokenId(uint32 _maxTokenId) internal {
    require(_maxTokenId != 0, "FixedPriceCollection: Max tokenID may not be 0");
    require(_maxTokenId < maxTokenId || totalSupply() == 0, "FixedPriceCollection: Max tokenID may not increase");
    require(latestTokenId + 1 <= _maxTokenId, "FixedPriceCollection: Max tokenID must be greater than last mint");
   ...
   */
  function isRevealed() public view returns (bool revealed) {
    revealed = baseURIHash == bytes32(0);
  }
    ...
    returns (bool interfaceSupported)
  {
    if (
      ...
      interfaceId == type(IFixedPriceCollection).interfaceId ||
      interfaceId == type(ICollectionMint).interfaceId
    ) {
  ...
  function royaltyInfo(
    uint256, /*tokenId*/
    uint256 salePrice
  ) public view virtual override(IRoyaltyInfo, ERC2981Upgradeable) returns (address receiver, uint256 royaltyAmount) {
  ...
  /**
   * @notice Returns the total minted supply for the collection.
   */
