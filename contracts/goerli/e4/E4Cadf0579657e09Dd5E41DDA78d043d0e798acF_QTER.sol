...
contract QTER is ERC20, Ownable, ReentrancyGuard {
    ...
    function snakeTokenOwnerOf(uint256 tokenId) private view returns (bool) {
    ...
    function stakeSnakeTokens(uint256[] memory tokenIds) public {
        ...
        oca.stakeTokens(tokenIds.length, msg.sender, tokenIds);
        // oca.tokenInfoList(tokenId).stakeDate = block.timestamp;
    }
}
...
contract OCASnake is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    function stakeTokens(
        ...
        uint256 tokenId = tokenIds[index - 15];
        bool isSender = ownerOf(tokenIds[tokenId]) == sender;
        ...
        if (index > 0) {
            return this.stakeTokens(index - 1, sender, tokenIds);
        }
    }
