...
contract NftyDreamsBenefactors is ERC1155, Ownable {
    ...
    modifier mintCheck(
        ...
        require(
            term <= _maxTermLimit,
            "Exceeding max term limit"
        );
        require(
            minted[msg.sender][tokenType] + amount <= _maxPerWallet,
    ...
    function _init() internal {
        ...
        _price[1] = 0.015 ether; //0.15 ether; //Pioneer
        _price[2] = 0.015 ether; //1.5 ether;  //Patron
        _price[3] = 0.015 ether; //15 ether;   //Nexus
        _price[4] = 0.015 ether; //150 ether;  //Visionary
    }
    ...
    function mintPublic(
        ...
        minted[msg.sender][tokenType] += amount;
    ...
    function burnBatch(address _to, uint256[] memory _ids, uint256[] memory _amounts) external {
        require(msg.sender == _redeemAddress, "Not authorized");
