...
import {BaseConfig, AdvancedConfig, TokenConfig, HeyMintStorage, BurnToken} from "../libraries/HeyMintStorage.sol";
...
contract HeyMintERC1155Base is HeyMintERC1155Upgradeable, IERC2981Upgradeable {
    ...
    function updateFullConfig(
        BaseConfig memory _baseConfig, TokenConfig[] memory _tokenConfigs
    ) external onlyOwner {
        this.updateBaseConfig(_baseConfig);
        for (uint256 i = 0; i < _tokenConfigs.length; i++) {
            this.upsertToken(_tokenConfigs[i]);
        }
        // TODO: Also update advanced config w/ this function once it's added.
    }
