...
contract ZRRTest is ERC721G, Ownable, ERC721Holder{
    using Strings for uint256;

    bool public isPublic;
    uint256 public maxSupply;
    uint256 public constant PRIV_PRICE = 1000000000000000; //0.001 ETH
    uint256 public constant PUB_PRICE = 1000000000000000; //0.001 ETH
    string private _baseTokenURI;
    constructor() ERC721G("Zodiac Rankings Race", "ZRR", 1, 3) {
        maxSupply = 999;
    }

    function chgToPub() external onlyOwner {
        isPublic = !isPublic;
    }
    ...
    function mint(uint256 _amount) payable external{
        ...
        if (!isPublic) {
            require(msg.value == PRIV_PRICE * _amount, "ZRR: Priv Insufficient amount");
            require(_amount + totalSupply() <= 666, "ZRR: End for private sales");

        }
        else {
            require(msg.value == PUB_PRICE * _amount, "ZRR: Insufficient amount");
