...
interface IUtility {
     ...
     */
    function getIncomeTokenFromETH(uint256 bnbNumber)
        external
        view
        returns (uint256);
     ...
     */
    function getDollarFromETH(uint256 bnbAmount)
        external
        view
        returns (uint256);
}
...
contract MiningCenterETH is ERC1155Holder {
    ...
    function groupStakingUnStaking(
        ...
        require(
            msg.value >= utility.getETHTokenFromDollar(2 * 10**18),
            "not enough gas fee"
        );
        ...
        )
    {
        ...
        if (different >= stakingType[stakedType].stakingTime) {
            ownerIncomeAmount = utility.getIncomeTokenFromETH(
                utility.getETHTokenFromDollar((_earnedUSDT * 11) / 10)
            );
        } else {
            ownerIncomeAmount = utility.getIncomeTokenFromETH(
                utility.getETHTokenFromDollar(
                    (((_earnedUSDT * 11) / 10) * (100 - marketingPros)) / 100
                )
            );

            marketingIncomeAmount = utility.getIncomeTokenFromETH(
                utility.getETHTokenFromDollar(
