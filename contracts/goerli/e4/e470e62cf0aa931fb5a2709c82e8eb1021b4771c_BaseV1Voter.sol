...
contract BaseV1Voter {
    ...
    address internal immutable baseStable;
    ...
    constructor(address __ve, address _baseStable, address _factory, address  _gauges, address _bribes) {
        _ve = __ve;
        factory = _factory;
        base = ve(__ve).token();
    ...
    function whitelist(address _token, address _pool, uint _tokenId) public {
        ...
        if (_tokenId == 1) {
            ...
            require(_pool == IBaseV1Factory(factory).getPair(_token, base, false) || _pool == IBaseV1Factory(factory).getPair(_token, baseStable, true));
            //require(pool == IBaseRouter(router).pairFor(_token, _paired, true) || pool == IBaseRouter(router).pairFor(_token, _paired, false));
