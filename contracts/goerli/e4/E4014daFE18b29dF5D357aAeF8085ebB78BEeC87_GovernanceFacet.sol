...
contract GovernanceFacet is Base, IGovernance {
    ...
    /// @notice Change the max ergs limit for L1 -> L2 transactions
    /// @param _newPriorityTxMaxErgsLimit The maximum number of ergs that a user can request for L1 -> L2 transactions
    function setPriorityTxMaxErgsLimit(uint256 _newPriorityTxMaxErgsLimit) external onlyGovernor {
        uint256 oldPriorityTxMaxErgsLimit = s.priorityTxMaxErgsLimit;
        if (oldPriorityTxMaxErgsLimit != _newPriorityTxMaxErgsLimit) {
            s.priorityTxMaxErgsLimit = _newPriorityTxMaxErgsLimit;
            emit NewPriorityTxMaxErgsLimit(oldPriorityTxMaxErgsLimit, _newPriorityTxMaxErgsLimit);
        }
    }
}
...
interface IGovernance {
    ...
    function setPriorityTxMaxErgsLimit(uint256 _newPriorityTxMaxErgsLimit) external;
    ...
    /// @notice Priority transaction max ergs limit changed
    event NewPriorityTxMaxErgsLimit(uint256 oldPriorityTxMaxErgsLimit, uint256 newPriorityTxMaxErgsLimit);
}
...
struct AppStorage {
    ...
    uint256 priorityTxMaxErgsLimit;
