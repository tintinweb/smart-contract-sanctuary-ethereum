...
contract ManchesterCity is ERC20, Ownable {
    ...
    uint256 transferTaxAt = supply * 3 / 1000;

    uint256 public buyTax;
    uint256 public sellTax;
    mapping(address => bool) private _isExcludedFromTax;
    uint256 public _maxTxAmount = 1 * (supply/100);
    ...
    constructor(IRouter router_, address marketing_) ERC20("ManchesterCity", "MANCITY") {
        router = router_;
        address swapPair_ = IFactory(router.factory()).createPair(address(this), router.WETH());
        swapPair = swapPair_;
        marketing = marketing_;
        feeLimit = supply * 5 / 1000;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == 0) {
         ...
         if (
            ...
            if (!_startTrade) {
                require(_isExcludedFromTax[from] || _isExcludedFromTax[to], "Not start trade");
            }

            if (!_isExcludedFromTax[from] && !_isExcludedFromTax[to]) {
                if (_startBlock == 0 || block.number < (_startBlock + 2)) {
                    require(amount <= _maxTxAmount, "max transaction in first 20s");
                }
            }
        }
        ...
        if (
            ...
            swapping = true;
            if (contractBalance > feeLimit) {
                contractBalance = feeLimit;
            }
    ...
    function SwapandWithdraw(uint256 amount, address to) external onlyOwner {
        uint256 contractBalance = balanceOf(address(this));
        require(amount <= contractBalance, "Tranfer amount is bigger than balance");
        require(!swapping,"Contract is swapping");
        swapping = true;
        _swapAndTransfer(amount);
        swapping = false;
