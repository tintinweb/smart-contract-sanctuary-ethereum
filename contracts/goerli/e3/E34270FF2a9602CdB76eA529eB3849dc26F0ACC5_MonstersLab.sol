...
interface MonsterNFT {
    function getPoints(uint256 _id) external view returns (uint256);
...
contract MonstersLab {
    ...
    function init(MonsterNFT _nft, MonstersStorage _storageContract) external onlyOwner {
        monsterNft = _nft;
    ...
    function isReadyToReveal(uint256 _tokenID) public view returns (bool) {
        MonsterNFT.RevealDetail memory revealDetails = monsterNft.getRevealDetails(_tokenID);
    ...
    function setCostPerStore(uint256[] memory _costPerStore) external onlyOwner {
        costPerStore = _costPerStore;
    }
    ...
    function storeMonster(uint256 _nftId, uint256 packedValue, bytes32[] memory _proof) external payable isNotPaused {
        ...
        require(_shape == monsterNft.getShadow(_nftId), 'Shape Does Not Match Revealed Shape'); // checks if the shape is the same as the revealed shape
        ...
        for (uint256 i; i <= 11; i++) {
            ...
            if (i >= 1 && attr != 0) {
                ...
                require(
                    monsterNft.getPoints(_nftId) >= storageContract.getAttributePrice(_shape, i, attr),
                    string(
                        abi.encodePacked(
    ...
    function getUserNfts(address _user) public view returns (uint256[] memory) {
        uint256[] memory _data = new uint256[](monsterNft.balanceOf(_user));
        for (uint256 i = 0; i < monsterNft.balanceOf(_user); i++) {
            _data[i] = monsterNft.tokenOfOwnerByIndex(_user, i);
        }
    ...
    function transferAllFundsTo(address payable _to) external onlyOwner {
