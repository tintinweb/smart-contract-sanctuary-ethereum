...
interface CustomIERC20 is IERC20 {
    function changeUserSellTax(address _user, uint8 _sellTaxPercentage)
        external;
...
contract KIZOINU is ERC20, Ownable {
    ...
    constructor() ERC20("Token Name", "Token Symbol") {
        ...
        changeLpPairStatus(uniswapV2Pair, true);
    ...
    function checkUserTax(address _user) public view returns (uint256 tax) {
...
contract OptimizedStaking is ERC20, ReentrancyGuard, Ownable {
    ...
    mapping(address => uint256) private userSellTax;

    constructor(address _token0)
        ERC20("Staking Token Name", "Staking Token Symbol")
    {
        //staking token with adjustable sell tax
        token0 = IERC20(_token0);
    }

    function stake(uint256 _amount, uint256 lockTime) external nonReentrant {
        require(
            lockTime == 604800 ||
                lockTime == 1209600 ||
                lockTime == 1814400 ||
        ...
        require(
            _amount > minStakeAmount * (10**18), //static value for decimals, very important that it matches the staking token,
        ...
        uint256 tax = CustomIERC20(address(token0)).checkUserTax(msg.sender);
        ...
        if (lockTime == 604800) {
            ...
            if (tax == 0) {
                CustomIERC20(address(token0)).changeUserSellTax(msg.sender, 10);
            }
            ...
            if (tax > 5 || tax == 0) {
                CustomIERC20(address(token0)).changeUserSellTax(msg.sender, 5);
            }
            ...
            if (tax > 1 || tax == 0) {
                CustomIERC20(address(token0)).changeUserSellTax(msg.sender, 1);
            }
        }
        totalStaked += _amount;
    }
    ...
    function unstake() external nonReentrant {
        ...
        for (uint256 i = 0; i < userStakingInstances; i++) {
            ...
            if (
                ...
                uint256 userReward = userObject.stakingReward;
                uint256 userAmountStaked = userObject.amountStaked;
    ...
    function userStaked(address _user) public view returns (uint256 staked) {
        //goes through all active staking instances with unlocked rewards, where the timelock period has expired
        for (uint256 i = 0; i < userStakes[msg.sender].length; i++) {
            StakingObject memory userObject = userStakes[_user][i];
