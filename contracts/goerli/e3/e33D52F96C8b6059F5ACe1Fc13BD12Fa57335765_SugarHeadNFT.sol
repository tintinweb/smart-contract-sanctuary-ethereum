...
contract SugarHeadNFT is ERC721URIStorage, Ownable, Pausable, DefaultOperatorFilterer {
    ...
    function buyNFTWithChadinu(uint id) external onlyOneFree {
    ...
    function buyNFTForFree(uint[] memory ids) external {
        require(currentPhase == Phase.FREE_MINT, "No Free Sale");
        require(
    ...
    function buyNFTEarly(uint32 _amount, uint[] memory ids)
        external
        payable
    ...
    function buyNFTsInPublic(uint32 _amount, uint[] memory ids)
        external
        payable
        onlyValidAmount(_amount)
    {
