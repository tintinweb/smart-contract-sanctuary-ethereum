...
contract ClientMarketAdapter is ModuleAccessController, IClientMarketAdapter {
    ...
    function protect(
        ...
        // apply incentives
        bumpAmount +=
            incentives.boostForTaker(address(asset), amount) +
            incentives.coordinationForTaker(address(asset), amount);
        // open position and transfer tokens
        uint192 flags = OptionFlagsHelper.EMPTY.setAutorenew(autorenew);
        return
            market.protect(msg.sender, amount, risk, term, bumpAmount, flags);
    }
    ...
    function close(
        ...
        // close position
        market.close(msg.sender, id, unwrap, address(stable));
    }
        ...
        onlyTakerPositionOwner(id)
    {
        ...
        // claim position
        market.claim(msg.sender, id, address(stable));
    }
    ...
    function cancel(
        ...
        // close position
        market.cancel(msg.sender, id, unwrap, address(stable));
    }
    ...
    function deposit(
        ...
        // apply incentives
        bumpAmount +=
            incentives.boostForMaker(address(asset), bstAmount) +
            incentives.coordinationForMaker(address(asset), bstAmount);
        uint192 flags = OptionFlagsHelper.EMPTY.setAutorenew(autorenew);
        ...
        onlyMakerPositionOwner(id)
    {
        ...
        market.abandon(msg.sender, id, address(stable));
    }
...
library OptionFlagsHelper {
    ...
    uint192 private constant NUMBER1_OFFSET = 1;
    uint192 private constant NUMBER1_MASK = uint192(0xFF) << NUMBER1_OFFSET; //size 8
    ...
    function getNumber1(uint192 flags) internal pure returns (uint8) {
        return uint8((flags & NUMBER1_MASK) >> NUMBER1_OFFSET);
    }

    function setNumber1(uint192 flags, uint8 value)
        internal
        pure
        returns (uint192)
    {
        return (flags & ~NUMBER1_MASK) | (uint192(value) << NUMBER1_OFFSET);
    }
