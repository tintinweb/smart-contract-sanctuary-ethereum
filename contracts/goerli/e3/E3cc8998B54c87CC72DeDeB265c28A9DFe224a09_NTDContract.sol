    ...
    CantBeEvil
{
    ...
    //Events
    event newName(uint256 _date, string _name, uint256 _tokenId);

    event newDIP(uint256 _tokenId);
     ...
     * @dev
     */
    function nameTheDip(uint256 date, string memory name) public {
        string memory proposedNameFormatted = toLowercase(name);
        require(!nameExists[proposedNameFormatted], "name has been used");
        require(dateExists[date], "date is not a DIP");
        require(
            ownerOf(dateToTokenId[date]) == msg.sender,
            "you are not the owner of the DIP"
        );
        require(isSafeName(name), "not a valid name");
        require(
            keccak256(abi.encodePacked(dateToName[date])) ==
                keccak256(abi.encodePacked("none")),
            "DIP is named"
        );
        uint256 _tokenId = dateToTokenId[date];
        tokenIdToName[_tokenId] = name;
        dateToName[date] = name;
        string memory nameFormatted = toLowercase(name);
        nameExists[nameFormatted] = true;
        emit newName(date, name, _tokenId);
    }
