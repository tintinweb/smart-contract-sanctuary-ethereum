...
// File: contracts/newupdatedcontractnft_b.sol
...
contract ERC1155Lockable is
    ...
    **/
     ...
     * acceptance magic value.
     */
    function mint(address to, uint256 amount, bytes calldata uri_, bytes calldata key, 
...
// File: contracts/CreatorContract.sol
...
contract Collections is ReentrancyGuard {
    // ERC1155Lockable public ERC1155;
    
    //recording the collections details
    ...
    function create_collection(string memory name, NftType N_type, uint256 supply, bytes calldata uri_, bytes calldata key, 
        string[] calldata cdn_, uint256 royaltyValue) public payable nonReentrant {
            // require(msg.value >= 0, "Not enough amount for the Fees");
            ERC1155Lockable ERC1155 =new ERC1155Lockable();
            CollectionsRecord[CollectionCount] = CollectionsDetails(ERC1155, name, msg.sender, block.timestamp, N_type);
            collectionOwners[address(msg.sender)].ids.push(CollectionCount);
            ERC1155.mint(msg.sender, supply, uri_, key, cdn_, msg.sender, royaltyValue);
