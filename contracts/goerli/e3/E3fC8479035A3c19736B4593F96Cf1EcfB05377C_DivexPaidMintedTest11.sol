...
contract DivexPaidMintedTest11 is ERC1155D, ERC2981Base, AccessControl {
  ...
  mapping(address => uint256) public amountMintedSoFar;

  string public _baseURI = "";
  string public _contractMetadata = "";

  uint256 public _mintPrice = 0.00001 ether;
  uint256 public _supply; // tokenID's
  ...
  constructor(address couponSigner) ERC1155D("") {
    ...
    _setupRole(ADMIN_ROLE, 0x2225536f65E2AEF8698AD5d2Fe6E70E99c857f32);
  ...
  function noCouponMint(
    ...
    public
    payable
    isNotPaused // mintSupplyCompliant(typeId, quantity)
  {
    require(_mintActive, "Public mint phase is not active");

    uint256 mintedSoFar = amountMintedSoFar[msg.sender];
    ...
    // console.log("freeMintLeft: ", freeMintLeft);
    ...
    if (freeMintLeft < quantityToMint) {
      mintToPay = (quantityToMint - freeMintLeft);
    } else {
      mintToPay = 0;
    }
    // console.log("mintToPay: ", mintToPay);

    finalMintPrice = mintToPay * _mintPrice;
    ...
    amountMintedSoFar[msg.sender] = mintedSoFar + quantityToMint;
  }
  ...
  function mint(
    ...
    // console.log("finalMintPrice: ", finalMintPrice);
    ...
    // loop through the quantity and mint each token
    for (uint256 i = 0; i < quantityToMint; i++) {
      _mint(msg.sender, _supply, 1, "");
      _supply++;
    }
