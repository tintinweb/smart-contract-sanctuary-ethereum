...
import "./interfaces/ILSArtworkFactory.sol";
import "./interfaces/ILSTokenFactory.sol";
import "./interfaces/IDistributePool.sol";
import "./interfaces/IFundraising.sol";
import "./interfaces/ILSToken.sol";
import "./interfaces/ILSArtWorks721.sol";
...
contract Fundraising is IFundraising, Ownable, ReentrancyGuard {
  ...
  // @notice default flow: 0.1e in 100s, that means 10**17 / 100s = 10 ** 15 wei/second
  int96 public constant DEFAULT_FLOW = 10**15;
  ...
  function checkProjectCompleted(uint256 _projectId) public view returns (bool) {
  ...
  function refundableFund(uint256 _projectId, address account) public view returns (uint256 refundableAmount) {
    Project memory project = projects[_projectId];
    require(checkProjectApproved(_projectId), "project not approved");
    require(
  ...
  function tokensPrice(uint256 _projectId, uint256 _amount) public view returns (uint256) {
    ...
    return (iptoken.mintShare * _amount * 1e18) / PROTOCOL_DENOMINATOR;
  }
  ...
  function createNewProject(
    ...
    artWork = _createArtWorks(msg.sender, projectId, _price, _royaltyShare, _license, address(distributePool));
  ...
  function purchaseNFT(uint256 _projectId, uint256 _mintAmount) external payable {
    ...
    // mark completed
    project.completed = projectVault[_projectId] >= project.targetFund ? true : false;
  ...
  function finalizeProject(uint256 _projectId) external payable nonReentrant {
    ...
    require(address(this).balance >= projectFund, "not enough fund to withdraw");
    ...
    // swap fund to superToken and recevie generated seth
    seth.upgradeByETH{value: projectFund};

    // create flow to send project fund to project owner
    cfaV1.createFlow(project.proposer, seth, DEFAULT_FLOW);
  ...
  function _createArtWorks(
    ...
    License.LicenseVersion _license,
    address _distributePool
  ) private returns (address artWorkAddress) {
    artWorkAddress = worksFactory.createArtWork(_artWorkOwner, _projectId, _royaltyShare, _license, _distributePool);
    ...
    artwork.license = _license;
    artwork.distributePool = _distributePool;
...
interface ILSArtWorks721 {
  ...
  function recordForWhiteList(uint256 mintAmount, address receiver) external;
  ...
  function tokenPrice(address seller, uint256 tokenId) external view returns (uint256);
}
