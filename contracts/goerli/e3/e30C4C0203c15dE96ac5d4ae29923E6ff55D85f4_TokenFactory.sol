...
// File: NFTfactory/TokenFactory.sol
...
error InvalidAmount(uint256 sentAmount, uint256 requiredAmount);
error AtleastOneWei();
error NullAddress();
...
contract NFT is ERC721A, Ownable {
    ...
    function setCollectionUri (string memory _uri) public adminModifier {
        uri = _uri;
    }
    function safeTransferFrom(address _from, address _to,  uint256 _tokenId) external {
        transferFrom(_from,_to,_tokenId);
    }
    function putForSale(uint _tokenId) public {        
        approve(admin, _tokenId);
    }
    ...
    modifier adminModifier {
        require(admin == msg.sender,"Admin only can change the uri");
        _;
    }
}
contract TokenFactory is Ownable {
    ...
    uint256 public nftContractsCount;
        ...
        returns (address)
    {
        ...
        NFT t = new NFT(name, symbol, admin);
    ...
    // to change DeployFee
    function setDeployFee(uint256 _newDeployFee)
        external
        onlyOwner
        returns (bool)
    {
