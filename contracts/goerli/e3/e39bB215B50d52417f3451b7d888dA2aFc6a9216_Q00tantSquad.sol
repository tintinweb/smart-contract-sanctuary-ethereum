...
error NotAQ00nicornSquad();
error Q00tantNotDeposited();
error NotEnoughQ00tants();
error Q00tantSquadDefeated();
error OwnershipLocked();

contract Q00tantSquad is OwnableEpochContract {
    IQ00nicornSquad q00nicornSquad;
    ...
    uint256 public costToImbue = 100_000000000000000000;
    bool private defeated;
    mapping(uint256 => address) ownerOf;

    constructor() {}

    function imbueSquad() external {
        glitter.transferFrom(msg.sender, address(this), costToImbue);   

        costToImbue = costToImbue * 2;     
        unchecked { squadPower += 50; }
    }
    ...
    function defend() external view returns (uint16) {
      if (q00tants.balanceOf(address(this)) < 5) revert NotEnoughQ00tants();
      if (defeated) revert Q00tantSquadDefeated();
    ...
    function transferFrom(address, address to, uint256 tokenId) external onlyEpoch {
      if (ownerOf[tokenId] == address(0)) revert Q00tantNotDeposited();
      ownerOf[tokenId] = to;
    }

    function withdrawQ00tant(uint256 tokenId) external onlyEpoch {
      q00tants.transferFrom(address(this), ownerOf[tokenId], tokenId);
    }
