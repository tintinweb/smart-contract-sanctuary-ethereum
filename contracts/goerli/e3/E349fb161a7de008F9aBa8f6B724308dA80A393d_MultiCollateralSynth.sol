...
contract ExternStateToken is Owned, Proxyable {
    ...
    function _internalTransfer(address from, address to, uint256 value) internal returns (bool) {
...
contract MultiCollateralSynth is Synth {
     ...
     */
    function issue(address account, uint256 amount) external onlyInternalContracts {
     ...
     * @param amount Number of synths
     */
    function burn(address account, uint256 amount) external onlyInternalContracts {
...
contract Synth is Owned, IERC20, ExternStateToken, MixinResolver, ISynth {
    ...
    /* ========== MUTATIVE FUNCTIONS ========== */
    function transfer(address to, uint256 value) public onlyProxyOrInternal returns (bool) {
        ...
        if (to == address(0)) {
            synthrBridge().sendTransferFrom(currencyKey, messageSender, to, value);
    ...
    function transferAndSettle(address to, uint256 value) public onlyProxyOrInternal returns (bool) {
    ...
    function transferFrom(address from, address to, uint256 value) public onlyProxyOrInternal returns (bool) {
        _ensureCanTransfer(from, value);

        return _internalTransferFrom(from, to, value);
    }

    function transferFromAndSettle(address from, address to, uint256 value) public onlyProxyOrInternal returns (bool) {
        // Exchanger.settle() ensures synth is active
    ...
    function _transferToFeeAddress(address to, uint256 value) internal returns (bool) {
        ...
        if (currencyKey == "sUSD") {
            ...
            (amountInUSD) = exchanger().exchange(messageSender, address(0), bytes32(0), args);
        }
    ...
    function _internalTransferFrom(address from, address to, uint256 value) internal returns (bool) {
        ...
        if (tokenState.allowance(from, messageSender) != uint256(-1)) {
            // Reduce the allowance by the amount we're transferring.
            // The safeSub call will handle an insufficient allowance.
            tokenState.setAllowance(from, messageSender, tokenState.allowance(from, messageSender).sub(value));
        }
        synthrBridge().sendTransferFrom(currencyKey, from, to, value);
...
interface ICollateralManager {
    ...
    function getRatesAndTime(
        uint256 index
    ) external view returns (uint256 entryRate, uint256 lastRate, uint256 lastUpdated, uint256 newIndex);
