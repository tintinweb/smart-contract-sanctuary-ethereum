...
interface IRareStakingFactory {
  function getOrDeployStaking(address _user) external returns (address);
...
ToDo:
    - natspec comments
    - make upgradeable UUPS
*/

contract RareStakingFactory is IRareStakingFactory, OwnableUpgradeable {
  /*//////////////////////////////////////////////////////////////////////////
  ...
  function getOrDeployStaking(address _user) public returns (address) {
    IRareStakingRegistry.Info memory info = stakingRegistry.getStakingInfoForUser(_user);

    if (info.stakingAddress != address(0)) {
      return info.stakingAddress;
    }
...
interface IRareStakingRegistry {
  ...
  function getAllStakingContract() external view returns (address[] memory);
}
...
interface IRareStake {
  ...
  /// @notice Error emitted when
  error ClaimNotActive();

  /// @notice Error emitted when
  error ZeroContractBalance(address _currencyAddress);
