...
contract TheFarmGameMint is Ownable, ERC165Storage, ReentrancyGuard, Pausable {
  ...
  // PreSale
  uint256 public preSaleTime = 1665068400; // Thursday, October 6, 2022 3:00 PM UTC / 11:00 AM EST
  uint256 public allowListTime = 1665072000; // Thursday, October 6, 2022 4:00 PM UTC / 12:00 PM EST
  uint256 public preSaleFee = 0.025 ether;
  uint256 public preSaleMaxQty = 4;
  uint256 public preSaleStakeFee = 0.02 ether;
  ...
  // Public Sale Gen 0
  uint256 public publicTime = 1665075600; // Thursday, October 6, 2022 5:00 PM UTC / 1:00 PM EST
  uint256 public publicFee = 0.035 ether;
  uint256 public publicStakeFee = 0.03 ether;
  ...
  // Auto-liquidity
  uint256 liqudityAlreadyPaid = 1; // Count of tokens that liquidity is already paid for
  ...
  /**
   * @dev Mint team reserve
   */
  function mintReserve(
    address recipient,
    uint256 quantity,
    bool stake
  ) external payable whenNotPaused nonReentrant onlyController {
    uint16 minted = farmAnimalsNFT.minted();
    uint256 maxSupply = farmAnimalsNFT.maxSupply();

    require(quantity <= teamReserve, 'Qty greater than reserve');

    require(
      minted + quantity + teamReserve + specialMint.getSpecialMintReserve() <= maxSupply,
      'Qty greater than maxSupply'
    );

    teamReserve = teamReserve - quantity;

    _mintStake(recipient, quantity, stake, minted);
  }
  ...
  // HACK: This should have onlyController, removed for testing
  // This should be called from randomizer upkeep
  function addCommitRandom(uint256 seed) external onlyController {
    _commitRandoms[_commitId] = seed;
    _commitId += 1;
  }

  // Get the Seed of the a commit id
  // HACK: Remove this before prod deploy
  function getCommitRandom(uint16 commitId) external view onlyController returns (uint256) {
    return _commitRandoms[commitId];
  }
  ...
  function _selectRecipient(
    ...
    // HACK: Local Dev set to 50% theif rate, set back to 10% for launch
    // if (minted <= gen0Supply || ((seed >> 245) % 10) != 0) return _msgSender();
    if (minted <= gen0Supply || ((seed >> 245) % 10) < 3) {
  ...
  function withdraw(address payable to) external onlyOwner {
    ...
    if (liqudityAlreadyPaid < gen0Supply) {
      uint256 tokenCountToBeAdded = gen0Supply - liqudityAlreadyPaid;
