...
contract CustomBond is Ownable {
    ...
    address immutable vaultIncDAO;
    address vaultIncTreasury; // receives fee
    ...
    address immutable subsidyRouter; // pays subsidy in payout token to custom treasury
    ...
    constructor(
        ...
        address _vaultIncTreasury,
        address _subsidyRouter, 
        address _initialOwner, 
        address _vaultIncDAO,
        ...
        require( _vaultIncTreasury != address(0) );
        vaultIncTreasury = _vaultIncTreasury;
        require( _subsidyRouter != address(0) );
        subsidyRouter = _subsidyRouter;
        require( _initialOwner != address(0) );
        policy = _initialOwner;
        require( _vaultIncDAO != address(0) );
        vaultIncDAO = _vaultIncDAO;
    ...
    /**
     *  @notice change address of VaultInc Treasury
     *  @param _vaultIncTreasury uint
     */
    function changeVaultIncTreasury(address _vaultIncTreasury) external {
        require( msg.sender == vaultIncDAO, "Only VaultInc DAO" );
        vaultIncTreasury = _vaultIncTreasury;
    }
    ...
    function deposit(uint _amount, uint _maxPrice, address _depositor) external returns (uint) {
        ...
        uint fee = payout.mul( currentVaultIncFee() ).div( 1e6 );
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            payoutToken.transfer(vaultIncTreasury, fee);
        }
    ...
    function trueBondPrice() public view returns ( uint price_ ) {
        price_ = bondPrice().add(bondPrice().mul( currentVaultIncFee() ).div( 1e6 ) );
    }
    ...
    /**
     *  @notice calculate user's interest due for new bond, accounting for VaultInc Fee
     *  @param _value uint
     *  @return uint
     */
    function payoutFor( uint _value ) external view returns ( uint ) {
        ...
        return total.sub(total.mul( currentVaultIncFee() ).div( 1e6 ));
    }

    /**
     *  @notice calculate current ratio of debt to payout token supply
     *  @notice protocols using VaultInc Pro should be careful when quickly adding large %s to total supply
    ...
    /**
     *  @notice current fee VaultInc takes of each bond
     *  @return currentFee_ uint
     */
    function currentVaultIncFee() public view returns( uint currentFee_ ) {
