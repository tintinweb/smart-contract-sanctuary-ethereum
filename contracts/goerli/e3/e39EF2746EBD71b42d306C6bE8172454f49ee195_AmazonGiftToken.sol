...
import "../lz_contracts/lzApp/NonblockingLzApp.sol";
/*
    LayerZero Mumbai
...
contract AmazonGiftToken is ERC20, Ownable, ReentrancyGuard, NonblockingLzApp {
    ...
    constructor(
        ...
        uint256 limit,
        address _lzEndpoint
    ) NonblockingLzApp(_lzEndpoint) ERC20(name, symbol) {
        limitAmount = limit;
    ...
    function changeLimitAmount(uint256 limit) external onlyOwner {
        limitAmount = limit;
    }
    ...
    function addToWhitelist(address toAddAddresses) external onlyOwner {
        ...
        for (uint i = 0; i < whitelist.length; i++) {
            if(toAddAddresses == whitelist[i]) {
                exist = true;
                break;
            }
        }
        if(!exist) {
            whitelist.push(toAddAddresses);
        }
    }
    ...
    function removeFromWhitelist(address toRemoveAddresses) external onlyOwner {
        ...
        bool exist = false;
        for (uint i = 0; i < whitelist.length; i++) {
            if(toRemoveAddresses == whitelist[i]) {
                index = i;
                exist = true;
                break;
            }
    ...
    function mint(uint256 amount) external nonReentrant {
        require(amount != 0, "Amount cannot be equal to 0.");
    ...
    function _beforeTokenTransfer(
        ...
        require(exist, "NOT_IN_WHITELIST");
        require(amount != 0, "Amount cannot be equal to 0.");
        require(amount >= limitAmount, "Insufficient limit amount");
    }
