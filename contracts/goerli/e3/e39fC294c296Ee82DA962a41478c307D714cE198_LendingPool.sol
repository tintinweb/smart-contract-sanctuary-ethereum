...
library PeriodStaking {
    ...
    function calculateWalletRewardsYieldPeriod(
        ...
        if (rewardPeriod.totalRewards != 0 || rewardPeriod.id != periodStakingStorage.currentRewardPeriodId) {
        ...
        uint256 walletStakingScorePresentMoment = walletStakingScore - (rewardPeriod.end - block.number) * periodStakingStorage.walletStakedAmounts[wallet].stakedBalance;

        uint256 yieldBearingCapitalPresentMoment = walletStakingScorePresentMoment / (block.number - rewardPeriod.start);
