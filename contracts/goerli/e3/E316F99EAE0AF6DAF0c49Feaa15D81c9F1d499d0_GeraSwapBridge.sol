...
contract GeraSwapBridge is Context {
    ...
    uint256 public feePercentageInGera;
    ...
    constructor(
        ...
        require(
            fee_PerGera <= hunderedPercent,
            "feePercentageInGera: SHould be less then 100"
        );
        feePercentageInGera = fee_PerGera;
        owner = payable(msg.sender);
        feeReceiver = fee_Receiver;
        superAdmin = super_Admin;
        burntPercentage = hunderedPercent.sub(fee_PerGera);
    }
     ...
     */
    function setSwapAmountToDivGERA(uint256 _amountDiv) external onlyOwner {
        require(
    ...
    /**
     * Returns set minimum swap fee in GERA from BEP20 to other chains
     */
    function setSwapFeePercentageOfGERA(uint256 _feePerGera)
        external
        onlyOwner
    {
        require(
            _feePerGera <= hunderedPercent,
            "amountDivisonPercentage: Greater than 100 %"
        );
        feePercentageInGera = _feePerGera;
        burntPercentage = hunderedPercent.sub(_feePerGera);
    }
        ...
        )
    {
        ...
        feeInGera = remainingAmount.mul(feePercentageInGera);
        feeInGera = feeInGera.div(hunderedPercent);

        amountToBurn = remainingAmount.sub(feeInGera);
        return (amountToSwap, feeInGera, amountToBurn);
    }
    ...
    function fillSwap(
        ...
        if (amountDivisonPercentage == hunderedPercent) {
            ...
            (swapAmount, feeInGera, amountToBurn) = deductFeeAndBurntAmount(
                amount
            );
            _token.transfer(toAddress, swapAmount);
            _token.transfer(feeReceiver, feeInGera);
    ...
    function swapToken(
        ...
        if (amountDivisonPercentage == hunderedPercent) {
            ...
            _token.transferFrom(msg.sender, address(this), swapAmount);
            _token.transferFrom(msg.sender, feeReceiver, feeInGera);
            _token.burn(amountToBurn);
        }
        ...
        )
    {
        ...
        return (swapAmount, feeInGera, amountToBurn);
    }
