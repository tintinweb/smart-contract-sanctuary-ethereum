...
contract WrappedERC4626YearnV2Vault is ERC4626
{
	...
	constructor (IYearnV2Vault _vault, string memory name, string memory symbol) ERC4626(IERC20Metadata(_vault.token())) ERC20(name, symbol)
	{
		vault = _vault;
		IERC20(asset()).safeIncreaseAllowance(address(_vault), type(uint256).max);
	}
	...
	function _convertYearnSharesToShares(uint256 yearnShares) internal view returns(uint256 shares) {
		shares = yearnShares * (10 ** decimals()) / (10 ** vault.decimals());
	}

	function _convertSharesToYearnShares(uint256 shares) internal view returns(uint256 yearnShares) {
