...
interface IERC721 is IERC165 {
    ...
    function isNFThasRoyalty(uint256 _tokenid) external view returns (bool);
...
contract ZellerStakingContract is IERC721Receiver, Ownable {
    ...
    constructor(address _nftContractAddress, address _rewardTokenAddress) {
        tokenRewardsPerMinute = 1 ether;
    ...
    function totalAccumulatedRewards(address _userAddress)
        external
        view
        returns (uint256)
    {
        uint256 totalRewards;
        uint256[] memory _ids = getStakedTokenIDs(_userAddress);
        for (uint256 i = 0; i < _ids.length; i++) {
    ...
    function getAccumulatedrewardPerTokenStaked(uint256 _tokenId)
        public
        view
        returns (uint256)
    {
        ...
        if (stakedaddress[_tokenId] != address(0)) {
            ...
            if (lastrewardClaim[_tokenId] != 0) {
                minuteslastClaimTime =
                    (block.timestamp - lastrewardClaim[_tokenId]) /
                    60; // 27382745.2167
            } else {
    ...
    function getStakedTokenIDs(address _userAddress)
        public
        view
    ...
    function _unstake(uint256 _tokenId, address _userAddress) internal {
        require(
            ...
            "user does not stake this id"
        );
        uint256 getClaimAmnt = getAccumulatedrewardPerTokenStaked(_tokenId);
        ...
        uint256[] storage arr = tokenIdsStaked[_userAddress];
        ...
        NFTContract.safeTransferFrom(address(this), _userAddress, _tokenId);
    }

    function _stake(uint256 _tokenId, address _userAddress) internal {
        require(
            NFTContract.ownerOf(_tokenId) == _userAddress,
            "Sender is not the owner of the tkn"
        );
        stakedaddress[_tokenId] = _userAddress;
        ...
        NFTContract.safeTransferFrom(_userAddress, address(this), _tokenId);
    ...
    //Admin Functions
    function updateTokenRewardPerMinutes(uint256 _newRewardPerMinute)
        external
        onlyOwner
    {
    ...
    function updateNFTToken(address _newNFTToken) external onlyOwner {
        NFTContract = IERC721(_newNFTToken);
    }
...
contract RewardToken is ERC20, ERC20Burnable, Ownable {
    constructor() ERC20("CHROMA", "CRM") {}

    address public NFT_STAKE_CONTRACT;

    function updateNFTStakeAddress(address _nft_stake_address)
        external
        onlyOwner
    {
        NFT_STAKE_CONTRACT = _nft_stake_address;
    }

    function mint(address to, uint256 amount) external onlyNFtStakeContract {
        _mint(to, amount);
    }

    modifier onlyNFtStakeContract() {
        require(
            (NFT_STAKE_CONTRACT == _msgSender()) || (owner() == _msgSender()),
            "Gov: caller is not the stakin contract"
        );
        _;
    }
}