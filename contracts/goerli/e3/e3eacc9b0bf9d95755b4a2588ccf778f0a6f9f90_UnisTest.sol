...
contract UnisTest is Context, IERC20, Ownable {
    ...
    IERC20 token;
    uint256 _profitStakeByYear = 200; // profit in 1 year
    uint256 _limitTimeWithdrawFee = 20; // with 20 % is 20, limit 100
    uint256 _limitTimeWithdraw = 7; // unit is days, with 1 month is 30, with 6 month is 180, with 1 year is 365,
    struct Stake{
        address user;
        uint256 amount;
        uint256 time;
    ...
    function stake(uint256 _amount) public {
      ...
      require(holderIndex[_msgSender()] == 0, "STAKE: You have already stake");
    ...
    function profitStakeByYear() external view returns(uint256) {
        return _profitStakeByYear;
    }
    function limitTimeWithdrawFee() external view returns(uint256) {
        return _limitTimeWithdrawFee;
    }
    function limitTimeWithdraw() external view returns(uint256) {
        return _limitTimeWithdraw;
    }
}