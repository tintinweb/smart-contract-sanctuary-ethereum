...
contract Validator is Initializable, ReentrancyGuardUpgradeable, Transfers, Auth {
    ...
    function initialize(address _currencyManager, address _collectionManager, address _feeManager, address _pool) public initializer {
    ...
    function setServices(bool swap, bool auction, bool offer, bool collectionOffer) external authorized {
        services = Services(swap, auction, offer, collectionOffer);
    }
    ...
    function createAuction(
        ...
        uint256 _endTime,
        address _SENDER
    ) external onlyMarketplace auctionActive {
        ...
        _transferNFT(_collection, _SENDER, MARKETPLACE, _id);
    }
    ...
    function bidOnAuction(
        ...
        bool _valid,
        address _owner,
        address _lastBidFrom,
        uint256 _lastBidPrice,
        address _SENDER
    ) external payable onlyMarketplace auctionActive nonReentrant {
        require(_valid, "Invalid Auction");
        require(_owner != _SENDER, "Owner cannot bid");
        require(block.timestamp < _end, "Auction is over");
        ...
        if (_lastBidPrice > 0) {
            require(
                _amount >=
                    _lastBidPrice.mul(IFeeManager(feeManager).divider() + IFeeManager(feeManager).bidIncrement()).div(
                        IFeeManager(feeManager).divider()
                    ),
                "Too small amount"
            );
        }
        require(
            _amount >=
                _startPrice.mul(IFeeManager(feeManager).divider() + IFeeManager(feeManager).bidIncrement()).div(
                    IFeeManager(feeManager).divider()
                ),
            "Too small amount"
        );
        ...
        if (_curr != address(0)) {
            ...
            require(IERC20(_curr).transferFrom(_SENDER, address(this), _amount), "Insufficient token");

            // 2 - Refund last bidder
            if (_lastBidPrice > 0) {
                require(IERC20(_curr).transfer(_lastBidFrom, _lastBidPrice), "Refund failed");
            }
        } else {
            ...
            // 2 - Refund last bidder
            if (_lastBidPrice > 0) {
                (bool result, ) = payable(_lastBidFrom).call{value: _lastBidPrice}("");
    ...
    function finalizeAuction(
        ...
        require(_valid, "Invalid Auction");
        require(_SENDER == _owner, "Not owner");

        // if have bids, transfer NFT to auction winner
        if (_lastBidPrice > 0) {
            _transferNFT(_col, MARKETPLACE, _lastBidFrom, _id);
        } else {
            // Transfer platform fee, royalties and seller amount
            _transferFeesAndFunds(_col, _id, _curr, _owner, _lastBidPrice);
            // Transfer NFT to seller
            _transferNFT(_col, MARKETPLACE, _owner, _id);
        }
    }
    ...
    function claimAuction(
        ...
        require(_valid, "Invalid Auction");
        require(block.timestamp > _endTime, "Auction is active");
        require(_lastBidPrice > 0, "No auction winner");
        require(_SENDER == _lastBidFrom, "Not auction winner");
    ...
    modifier auctionActive() {
        _serviceActive(services.auction);
        _;
    }
