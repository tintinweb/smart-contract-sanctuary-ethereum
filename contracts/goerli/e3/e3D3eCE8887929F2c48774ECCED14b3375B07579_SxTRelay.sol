...
contract SxTRelay is Admin, String, Initializer, ReentrancyGuard, ChainlinkClient, ISxTRelay {

    using Chainlink for Chainlink.Request;

    /// @dev Fee for Chainlink request
    /// uint256 public constant FEE = (1 * LINK_DIVISIBILITY) / 10000000000;
    uint256 public constant override FEE = 1;
     ...
     */
    function setChainlinkOperator(address newOperator) external override onlyAdmin {
     ...
     */
    function withdrawChainlink(address to, uint256 amount) external override onlyAdmin nonReentrant {
        bool transferResult = ChainlinkTokenInterface(chainlinkTokenAddress()).transfer(
            to,
            amount
        );
     ...
     */
    // function setChainlinkJobID(string memory jobId) external override onlyAdmin {
     ...
     */
    function requestQuery(string memory query, string memory resourceId, address callerContract, bytes4 callbackFunctionId, bytes32 chainlinkJobId)
        external
        override
        checkAllowance(FEE)
        nonReentrant
        returns (bytes32)
    {
        require(ChainlinkTokenInterface(chainlinkTokenAddress()).transferFrom(msg.sender, address(this), FEE), "SxTRelay: Insufficient LINK tokens");
     ...
     */
    function queryResponse(bytes32 requestId, bytes calldata data)
        external
        override
        nonReentrant
        recordChainlinkFulfillment(requestId)
    {
        ...
        (bool success, ) = callerAddress.call(
    ...
    modifier checkAllowance(uint amount) {
        require(ChainlinkTokenInterface(chainlinkTokenAddress()).allowance(msg.sender, address(this)) >= amount, "SxTRelay: Insufficient allowance");
        _;
    }
}
...
// SPDX-License-Identifier: MIT
// github.com/OpenZeppelin/[email protected]0b70ab89073d5d0b6e01b0778f7e7d6
