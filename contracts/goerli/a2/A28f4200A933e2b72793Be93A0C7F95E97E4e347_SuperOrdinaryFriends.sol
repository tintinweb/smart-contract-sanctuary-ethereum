...
contract SuperOrdinaryFriends is ERC721Base, PaymentSplitter, AccessControl, Pausable, ReentrancyGuard, Ownable {
    ...
    uint private maxAirDrop = 50;
    uint private currAirDrop = 0;

    // Signer address
    address private freeMintSignerAddr;
    ...
    function whitelistMint(uint256 amount, uint256 maxAmount, bytes memory signature) external payable whenNotPaused nonReentrant {
        ...
        require(whitelistClaimed[msg.sender] + amount <= maxAmount, "Exceed max per wallet");
    ...
    function whitelistFreeMint(uint amount, uint256 maxAmount, bytes memory signature) external whenNotPaused nonReentrant {
        uint256 supply = totalSupply();
        
        require(isFreeMint, "Whitelist mint is not open");
        require(freeMintClaimed[msg.sender] + amount <= maxAmount, "Exceed max per wallet");
        require(amount > 0, "amount too little");
        require(msg.sender != address(0), "empty address");
        require(supply + amount <= MAX_SUPPLY, "exceed max supply");
        require(freeMintSignerAddr != address(0), "Signer address is not set");
        require(Signature.verify(maxAmount, msg.sender, signature) == freeMintSignerAddr, "Invalid signature");
    ...
    function setFreeMintSignerAddr(address signer) external onlyRole(DEFAULT_ADMIN_ROLE){
        freeMintSignerAddr = signer;
    }
    ...
    function setFreeMint(bool _isFreeMint) external onlyRole(DEFAULT_ADMIN_ROLE){
