...
// Declare events to emit when users stake, withdraw, or claim rewards
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        // require(!isLocked[msg.sender], "Withdraw is locked");
    ...
    function claimRewards() external nonReentrant {
        uint256 reward = calculateReward(msg.sender);
        Rewardbalances[msg.sender] += reward;
        require(Rewardbalances[msg.sender] > 0 ,"NO Reward to claim") ;
        require(token.transfer(msg.sender, Rewardbalances[msg.sender]), "Reward transfer failed");
             uint256 Claim = Rewardbalances[msg.sender];
    ...
    function calculateReward(address user) public view returns (uint256) {
        uint256 t = (block.number - lastBlock[user]) / 10; // number of 10-block periods that have passed

        if (t == 0) {
            return 0;
        }

        uint256 reward = (balances[user] * interestRate * t) / 100;
        return reward;
    }
    function numberBL() public view returns(uint256){
        return block.number ;
    }
