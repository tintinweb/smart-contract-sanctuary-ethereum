...
contract SpecialSlothItem is Initializable, ERC721AQueryableUpgradeable, OwnableUpgradeable, ISlothItemV2, IERC2981, RevokableOperatorFiltererUpgradeable {
  error InvalidArguments();

  mapping(uint256 => ItemType) public itemType;
  mapping(uint256 => uint16)  public specialType;
  ...
  function ownerItemMintForEgg(uint8 quantity) payable public onlyOwner {
    require(quantity > 0, "quantity must be greater than 0");
    uint256 nextTokenId = _nextTokenId();
    _mint(msg.sender, quantity);
    for(uint256 i = 0; i < quantity; i++) {
      itemType[nextTokenId] = ItemType.CLOTHES;
      specialType[nextTokenId] = 0;
      nextTokenId += 1;
    }
  }

  function ownerItemMintForSpecial1(uint8 quantity) payable public onlyOwner {
    require(quantity > 0, "quantity must be greater than 0");
    uint256 nextTokenId = _nextTokenId();
    _mint(msg.sender, quantity);
    ItemType _nextItemType = ItemType.CLOTHES;
    for(uint256 i = 0; i < quantity; i++) {
      itemType[nextTokenId] = _nextItemType;
      specialType[nextTokenId] = 1;
      if (_nextItemType == ItemType.CLOTHES) {
  ...
  function batchTransferToMultipleWallets(address[] calldata tos, uint256[] calldata tokenIds) public onlyOwner {
  ...
  // TODO: ownerMint関数を作成する
  // function ownerItemMint(uint8 quantity, ItemType itemType, string specialType) payable external onlyOwner {
  //   // TODO specialTypeが_specialTypesに存在するかチェックする
  //   require(quantity > 0, "quantity must be greater than 0");
  //   require(specialType != "", "specialType must not be empty");

  //   uint256 nextTokenId = _nextTokenId();

  //   _mint(msg.sender, quantity);
  //   itemType[nextTokenId] = ItemType.CLOTHES;
  //   specialType[nextTokenId] = specialType;

  //   // コラボのアイテムが複数タイプあったらなら何を入れればいいかわからないか・・・？
  //   for(uint256 i = 0; i < quantity; i++) {
  //     itemType[nextTokenId] = _nextItemType;
