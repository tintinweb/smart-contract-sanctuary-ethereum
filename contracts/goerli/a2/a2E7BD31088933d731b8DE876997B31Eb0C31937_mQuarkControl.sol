...
contract mQuarkControl is ReentrancyGuard, PullPayment, AccessControl {
  ...
  /**
   *  @dev Mapping from 'Project ID' to Project Struct
   **/
  ...
  function mint(
    ...
    _registeredProjects[projectId].balance += (msg.value * creatorPercentage) / 100;
  ...
  function mintBatchWithURISlot(
    ...
    require(templateIds.length == amounts.length, "amount mismatch");
    require(_registeredProjects[projectId].id != 0, "unregistered project");
    // require(_registeredProjects[projectId].id == _projectId, "project id mismatch");
    // require(_registeredProjects[projectId].wallet == projectId, "wallet mismatch");
   ...
   *@param projectIds:   struct ProjectParams {
   *                                uint256[] _projectIds: The number of mints for each IDs.
  ...
  function mintWithURISlots(
    ...
    string[] memory _projectMetadataTemplate = new string[](projectIds.length);
    ...
    for (uint8 i = 0; i < projectCount; ) {
      ...
      _metadataSlotPrice = _projectSlotPrices[projectIds[i]];
      _projectMetadataTemplate[i] = (_registeredProjects[projectIds[i]].projectSlotDefaultURI);
      _totalMetadataSlotPriceAmount += _metadataSlotPrice;
      _registeredProjects[projectIds[i]].balance += (_metadataSlotPrice * creatorPercentage) / 100;
    ...
    mQuark.mintWithURISlots(msg.sender, templateId, collectionId, projectIds, _projectMetadataTemplate);
    _registeredProjects[projectIds[0]].balance += ((_templateMintPrice * creatorPercentage) / 100);
    adminBalance[admin] += ((msg.value * adminPercentage) / 100);

    /** @notice Base mint price should be considered at zero index! */
    emit MintBatchSlotFundsDeposit(msg.value, creatorPercentage, projectIds, _projectsMetedataPriceShares);
  }
  ...
  function addBatchURISlotsToNFT(uint256 tokenId, uint256[] calldata projectIds) external payable nonReentrant {
    ...
    for (uint256 i = 0; i < _projects; ) {
      ...
      _projectMetadataTemplate[i] = (_registeredProjects[projectIds[i]].projectSlotDefaultURI);
      _totalAmount += _price;
      _registeredProjects[projectIds[i]].balance += (_price * creatorPercentage) / 100;
    ...
    adminBalance[admin] += (msg.value * adminPercentage) / 100;
    mQuark.addBatchURISlotsToNFT(msg.sender, tokenId, projectIds, _projectMetadataTemplate);
    emit BatchSlotFundsDeposit(msg.value, creatorPercentage, projectIds, _projectsShares);
  }
  ...
  function resetProjectURISlot(
    ...
    require(msg.sender == _registeredProjects[projectId].wallet, "unauthorized access");
    require(_registeredProjects[projectId].id == projectId, "unregistered project");

    // mQuark.resetProjectURI(tokenId, projectId, _registeredProjects[project].projectSlotDefaultURI);
  }
  ...
  function removeProject(uint256 prjectId) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(_registeredProjects[prjectId].id != 0, "deleting unregistered project");
   ...
   *@param mQuarkAddr: Contract address of NFT Contract
   */
  ...
  */
  ...
  //   require(_registeredProjects[wallet].creator == msg.sender || _registeredProjects[msg.sender].wallet == msg.sender, "unauthorized access");
    ...
    returns (uint256 totalAmount)
  {
    ...
    for (uint8 i = 0; i < _templateIdsLength; ) {
      ...
      totalAmount += (_templateMintPrices[templateIds[i]] * amounts[i]);
      unchecked {
        ++i;
      }
    }
    ...
    returns (uint256 totalAmount)
  {
    ...
    for (uint8 i = 0; i < _amountsLength; ) {
      totalAmount += (_projectSlotPrices[projectId] * amounts[i]);
      unchecked {
        ++i;
      }
    }
  ...
  function calculateMintWithURISlotsPrice(uint256 templateId, uint256[] calldata projectIds)
    public
    view
    returns (uint256)
  {
  ...
  function getRegisteredProjects(uint256 projectId) external view returns (Project memory) {
    return _registeredProjects[projectId];
  }

  function getProjectId(address projectAddress) external view returns (uint256) {
    return _projectIds[projectAddress];
  }
  ...
  function getProjectSlotPrices(uint256 projectId) external view returns (uint256) {
