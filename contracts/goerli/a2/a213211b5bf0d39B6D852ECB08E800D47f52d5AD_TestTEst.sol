...
contract TestTEst {
    ...
    */
    function depositTokensToPool(address _token, uint256 _amount) external {
        // require(msg.sender == m_Admins[_token]); // uncomment to restrict pool deposits to team only
    ...
    */  
    function depositTokensToStake(address _token, uint256 _amount) external {     
        ERC20(_token).transferFrom(msg.sender, address(this), _amount);
    ...
    function reStake(address _token) external {
    ...
    function withdraw(address _token) external {
        ...
        _amount += _claimDividends(msg.sender, _token);
        m_Earnings[msg.sender][_token] += _amount;
    ...
    function _claimDividends(address _address, address _token) internal Lock returns (uint256) {
    ...
    function _getCurrentEarnings(address _address, address _token) internal view returns (uint256) {
