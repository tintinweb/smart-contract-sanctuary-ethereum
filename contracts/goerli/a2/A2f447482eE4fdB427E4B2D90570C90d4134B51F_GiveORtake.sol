 ...
 */
contract GiveORtake is ReentrancyGuard, GameTools {
    ...
    event GiveHandled(uint256 indexed roomId);
    event TakeHandled(uint256 indexed roomId);
     ...
     * @param user identifies user user = 0 -> player1, user = 1 -> player2
     */
    modifier validUser(uint16 user) {
        require(user >= 0 && user <= 1, "Invalid User");
        _;
    }
        ...
        gameRoomClosed(roomId)
    {
        ...
        if (gameRoom.result == 0) {
            IERC20(gameRoom.erc20Address).transferFrom(
                gameRoom.player1,
                address(this),
                gameRoom.betAmount * 3
            );
        } else if (gameRoom.result == 1) {
            IERC20(gameRoom.erc20Address).transferFrom(
                gameRoom.player2,
                address(this),
                gameRoom.betAmount * 3
     ...
     * @param roomId game room id
     * @param actor the protector 0->player1, 1->player2
     */
    function Give(uint256 roomId, uint16 actor)
        external
        whenNotPaused
        validUser(actor)
        validRoomId(roomId)
     ...
     * @param actor the protector 0->player1, 1->player2
     */
    function Take(uint256 roomId, uint16 actor)
        external
        whenNotPaused
        validUser(actor)
        validRoomId(roomId)
