...
contract MasterPool is IMasterPool, Ownable {
    ...
    uint256 constant PERCENT_FOR_AUCTION = 97;  // 85%
    uint256 constant PERCENT_FOR_BUYBAK = 1;    // 5%
    uint256 constant PERCENT_FOR_DEV = 1;       // 5%
    uint256 constant PERCENT_FOR_LIQUIDITY = 1; // 5%
    ...
    constructor (
        ...
        BUSD = IERC20(0xb809b9B2dc5e93CB863176Ea2D565425B03c0540);
        isLock = false;
    ...
    function getAuctionState(address user_) external view returns (
        uint256 auctionDay,
        uint256 leftAuctionDay,
        uint256 tokenXPool,
        uint256 busdPool,
        uint256 participantsCnt,
        uint256 auctionPrice,
        uint256 busdDividendsAmount,
        uint256 totalBidAmount,
        ...
        auctionDay = tokenX.leftDays() + 1;
        uint256 auctionId = auctionDay - 1;
        leftAuctionDay = 100 - auctionDay;
        tokenXPool = tokenX.supplyAmount(auctionId);
        totalBidAmount = totalAuctioned[auctionId];
        participantsCnt = auctionStakers[auctionId].length;
        auctionPrice = tokenXPool == 0 ? 0 : busdPool * 1e18 / tokenXPool;
        busdDividendsAmount = auctionId == 0 ? 0 : totalAuctioned[auctionId - 1] * PERCENT_FOR_AUCTION / 100;
        myTotalAuctionBid = userInfos[account].totalBidAmount;
        myAuctionBidToday = userInfos[account].bidAmountPerAuction[auctionId];

        uint256 lastAuctionId = lastAuctionIds[account];
        uint256 lastClaimDividensId = lastClaimDividensIds[account];
        uint256 cnt = 0;
        for (uint256 i = lastClaimDividensId + 1; i <= lastAuctionId; i ++) {
            AuctionInfo memory auctionInfo = auctionInfos[i][account];
            if (auctionInfo.auctionAmount > 0 && auctionInfo.claimed == false) {
                cnt ++;
            }
        }

        auctionHistory = new AuctionHistory[](cnt);
        uint256 index = 0;
        for (uint256 i = lastClaimDividensId + 1; i <= lastAuctionId; i ++) {
            AuctionInfo memory auctionInfo = auctionInfos[i][account];
            if (auctionInfo.auctionAmount > 0 && auctionInfo.claimed == false) {
                auctionHistory[index ++] = AuctionHistory({
                    auctionId: i,
                    auctionAmount: auctionInfo.auctionAmount,
                    stakingDuration: auctionInfo.stakeDuration,
