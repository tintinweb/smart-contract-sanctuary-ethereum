...
contract CombineNFT is Initializable, UUPSUpgradeable, ContextUpgradeable {
    ...
    */
     ...
     */
    function mint(uint256 isleId, uint256 exteriorId, uint256 interiorId, uint256 landScapeId, uint256 stakeWeight, address maker) public onlyRole(MINTER_C_ROLE) returns(uint256){
        require(isleId > 0,"NFT/isleId_is_0");

        _nfts[++_currentId].isleId = isleId;
        _nfts[_currentId].exteriorId = exteriorId;
        _nfts[_currentId].interiorId = interiorId;
        _nfts[_currentId].landScapeId = landScapeId;
        _nfts[_currentId].stakeWeight = stakeWeight;
