...
contract Sporto is AccessControl {
    ...
    event CommissionPayment(address indexed from, uint256 indexed commisionAmount);
    ...
    constructor(address _erc20, address _treasury) {
    ...
    function makeBet(uint256 id, Result choice, uint256 ticketCount) external {
        ...
        if (bet.commissionPerTicket > 0) {
            erc20.transferFrom(msg.sender, treasury, ticketCount * bet.commissionPerTicket);
            emit CommissionPayment(msg.sender, ticketCount * bet.commissionPerTicket);
        }
        erc20.transferFrom(msg.sender, address(this), ticketCount * bet.pricePerTicket);

        bet.choices[choice].push(msg.sender);
        bet.counts[choice].push(ticketCount);
        bet.accumulated[choice] += ticketCount * bet.pricePerTicket;
    ...
    function openBet(
        ...
        bet.endsAt = endsAt;
        bet.pricePerTicket = pricePerTicket;
        bet.prizePerTicket = 0;
        bet.commissionPerTicket = commissionPerTicket;
        bet.result = Result.NONE;
        bet.status = Status.OPEN;
        emit OpenBet(id, startsAt, endsAt, pricePerTicket);
    }
    ...
    function injectBet(uint256 id, uint256 amount) external onlyRole(INJECTOR_ROLE) {
        Bet storage bet = bets[id];
        require(bet.status == Status.OPEN || bet.status == Status.CLOSED, "Not open or closed");
        erc20.transferFrom(msg.sender, address(this), amount);
        bet.injectedAmount = amount;
        emit InjectBet(id, amount);
    }
