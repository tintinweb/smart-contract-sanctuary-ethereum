...
contract Stader is Timelock{
    ...
    event Staked(address indexed sender, uint256 amount, uint256 ethXMinted, address referral);
    ...
    modifier onlyScriptOrOwner {
        require(msg.sender == timelockOwner || msg.sender == scriptManager, "ERR_NOT_AUTHORIZED");
        _;
    }
    ...
    * @param _referral address of referral.
     */
    function stake(address _referral) external whenNotPaused payable {
    ...
    function distributeFee(uint256 _totalRewards) internal {
        ...
        // token shares.
        uint256 shares2mint = (
            _totalRewards.mul(feePercentage).mul(getTotalSupply())
            .div(
                getTVL().mul(100)
                .sub(feePercentage.mul(_totalRewards))
            )
        );
...
contract Timelock is Initializable{
  ...
  function initializeTimelock(address _timelockOwner) public onlyInitializing
  {
  ...
  function acceptTimelockOwnership() external{
    ...
    require(timestamp+lockedPeriod >= block.timestamp, "Locking period not expired");
  ...
  function cancelTimelockOwnerProposal() external{
    if (timelockOwnerCandidate != msg.sender && timelockOwner != msg.sender) revert('You are not the owner');
