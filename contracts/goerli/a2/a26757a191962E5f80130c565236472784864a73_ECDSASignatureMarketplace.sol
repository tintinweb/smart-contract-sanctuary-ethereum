...
import "../interfaces/IECDSASignatureMarketplace.sol";

contract ECDSASignatureMarketplace is ECDSASignatureBase, IECDSASignatureMarketplace {
    using ECDSA for bytes32;
   
    constructor(address signature) ECDSASignatureBase(signature) {}
    
    function hashingInsertMessage(address nftContract, uint256 tokenId, uint256 price) external pure returns (bytes32) {
        return keccak256(abi.encodePacked(nftContract, tokenId, price));
    }
    
    function hashingChangePriceBase(uint256 price, bytes32 typeNameContract) external pure returns (bytes32){
        return keccak256(abi.encodePacked(price, typeNameContract));
    }
...
interface IECDSASignatureMarketplace {
    function hashingInsertMessage(address nftContract, uint256 tokenId, uint256 price) external pure returns (bytes32);
    function hashingChangePriceBase(uint256 price, bytes32 typeNameContract) external pure returns (bytes32);
    function verifyMessage(bytes32 messageHash, uint256 nonce, uint256 timestamp, bytes[] memory signatures) external;
}
