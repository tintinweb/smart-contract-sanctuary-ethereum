...
contract SingleFixed is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    function singleList(address _collection, uint256 _tokenId, uint256 _price) OnlyItemOwner(_collection,_tokenId) public {
		...
		address tokenAdr = nft.getTokenAddr();
		...
		pairs[currentPairId].tokenAdr = tokenAdr;		
    ...
    function singleBuy(uint256 _id) external payable {
		...
		// filter floor Price
        uint256 floorPrice = IInvestNFT(pairs[_id].collection).getFloorPrice();        
		uint256 totalAmount = pair.price;
		if (pair.price < floorPrice) {
			totalAmount = floorPrice;
		}
