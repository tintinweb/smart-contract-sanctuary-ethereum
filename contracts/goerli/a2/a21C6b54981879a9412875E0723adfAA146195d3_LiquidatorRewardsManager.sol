...
interface ILiquidatorRewardsAggregator {
    function earned(bytes32 currencyKey, address account, uint256 debtShare) external view returns (uint256);
}
...
contract LiquidatorRewardsManager is Owned, MixinSystemSettings {
    ...
    bytes32 private constant CONTRACT_LIQUIDATORREWARDS_AGGREGATOR = "LiquidatorRewardsAggregator";
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[2] = CONTRACT_DEBT_AGGREGATOR;
        newAddresses[3] = CONTRACT_LIQUIDATORREWARDS_AGGREGATOR;
    ...
    function liquidatorRewardsAggregator() internal view returns (ILiquidatorRewardsAggregator) {
        return ILiquidatorRewardsAggregator(requireAndGetAddress(CONTRACT_LIQUIDATORREWARDS_AGGREGATOR));
    }
    ...
    function totalLiquidateEarned(address account) external view returns (uint256) {
        ...
        for (uint256 ii = 0; ii < availableCollaterals.length; ii++) {
            uint256 earned = liquidatorRewardsAggregator().earned(availableCollaterals[ii], account, debtShare);
            if (earned > 0) {
    ...
    function totalLiquidateEarnedPerCurrencyKey(bytes32 currencyKey, address account) external view returns (uint256) {
        uint256 debtShare = debtShareAggregator().balanceOf(account);
        uint256 earned = liquidatorRewardsAggregator().earned(currencyKey, account, debtShare);
