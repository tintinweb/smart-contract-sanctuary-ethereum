...
contract ExchangeCircuitBreaker is Owned, MixinSystemSettings, IExchangeCircuitBreaker {
    ...
    uint256 public constant CIRCUIT_BREAKER_SUSPENSION_REASON = 65;
    ...
    mapping(bytes32 => uint256) internal _lastExchangeRate;
    ...
    function rateWithInvalid(bytes32 currencyKey) external view returns (uint256, bool) {
        (uint256 rate, bool invalid) = _exchangeRates().rateAndInvalid(currencyKey);
    ...
    function resetLastExchangeRate(bytes32[] calldata currencyKeys) external onlyOwner {
        (uint256[] memory rates, bool anyRateInvalid) = _exchangeRates().ratesAndInvalidForCurrencies(currencyKeys);
    ...
    function _isDeviationAboveThreshold(uint256 base, uint256 comparison) internal view returns (bool) {
     ...
     */
    function _isRateOutOfBounds(bytes32 currencyKey, uint256 currentRate) internal view returns (bool) {
        if (currentRate == 0) {
            return true;
        }

        uint256 lastRateFromExchange = _lastExchangeRate[currencyKey];
        ...
        (uint256[] memory rates, ) = _exchangeRates().ratesAndUpdatedTimeForCurrencyLastNRounds(currencyKey, 4, 0);
