...
import "./ChildChainManager.sol";
import "@openzeppelin/contracts/token/ERC20/extensions/ERC20Pausable.sol";

contract AGGToken is
    ERC20Pausable,
    Ownable,
    TimeLock,
    IChildToken,
    ChildChainManager
{
    constructor(
        ...
        bool _isEthereumNetwork
    ) public ERC20(_name, _symbol) {
        if (_isEthereumNetwork) {
            uint256 initSupply = _initSupply * (10**uint256(_decimals));
            mint(initSupply);
        } else {
            setChildChainManager(_childChainManager);
        }
    }
    ...
    function withdraw(uint256 amount) external onlyChildChainManager {
...
contract ChildChainManager is Ownable {
    address private childChainManagerProxy;

    modifier onlyChildChainManager() {
        require(
            address(0) != childChainManagerProxy,
            "childChainManagerProxy is zero address"
        );
        require(
            msg.sender == childChainManagerProxy,
            "msg.sender is not childChainManagerProxy"
        );
        _;
    }

    function setChildChainManager(address _proxy) public onlyOwner {
        require(
            _proxy != address(0),
            "setChildChainManager: proxy is the zero address"
        );

        childChainManagerProxy = _proxy;
    }
}
