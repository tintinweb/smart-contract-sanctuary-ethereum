...
contract Atom is ERC721A, Authorized {
  ...
  Counters.Counter private _paperCounter;

  mapping(uint256 => string) private _paperURIs;

  constructor() ERC721A("Atom Protocol", "ATOM") {}

  function publish(string calldata paperURI) public {
    _paperCounter.increment();

    _safeMint(msg.sender, _paperCounter.current());
    setPaperURI(_paperCounter.current(), paperURI);
  }

  function setPaperURI(
    uint256 paperId,
    string memory paperURI
  ) internal onlyOwner {
    require(_exists(paperId), "setPaperURI: URI set of nonexistent paper");
    _paperURIs[paperId] = paperURI;
  }
  ...
  function walletOfOwner(
    address account
  ) external view returns (uint[] memory wallet) {
    uint count;
