    ...
    VRFConsumerBaseV2
{
    ...
    uint256 totalCount = 10000;
    uint256 purchaseLimit = 20;
    ...
    function openBox(uint256[] memory _ids) public nonReentrant {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            if (boxNumber >= 1 && boxNumber <= 1000) {
                GameNft gameNft = GameNft(nftAddressByMinted[0]);
                gameNft.mint(tokenOwner);
            } else if (boxNumber > 1000 && boxNumber <= 2000) {
                GameNft gameNft = GameNft(nftAddressByMinted[1]);
                gameNft.mint(tokenOwner);
            } else if (boxNumber > 2000 && boxNumber <= 3000) {
                GameNft gameNft = GameNft(nftAddressByMinted[2]);
                gameNft.mint(tokenOwner);
            } else if (boxNumber > 3000 && boxNumber <= 4000) {
                GameNft gameNft = GameNft(nftAddressByMinted[3]);
                gameNft.mint(tokenOwner);
            } else if (boxNumber > 4000) {
                GameNft gameNft = GameNft(nftAddressByMinted[4]);
                gameNft.mint(tokenOwner);
            }
        }
    }
