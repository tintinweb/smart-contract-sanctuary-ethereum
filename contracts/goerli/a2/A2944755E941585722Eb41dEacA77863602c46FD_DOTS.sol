...
contract DOTS is ERC721AQueryable, AccessControl, VRFConsumerBaseV2 {
    ...
    function sendVRFRequests (
        ...
        while (_tokenAmount > 0) {
            ...
            emit VRFRequested(s_requestId);

        }

    }

    function externalFulfill(uint256 requestId, uint256[] memory randomWords) external {
    ...
    function fulfillRandomWords(
        ...
        uint256 tokenId = vrfRequest.fromTokenId;

        for(uint256 i; i < randomWords.length;) {

            metadata[tokenId].genes = randomWords[i];

            emit DOTRevealed(tokenId);
    ...
    function getMetadata(uint256 _tokenId) external view returns (Metadata memory) {
        return Metadata({
            evoNum: metadata[_tokenId].evoNum,
            genes: _normalize(metadata[_tokenId].genes)
        });            
    } 
