...
contract BasicAMB is BasicBridge, VersionableAMB {
    ...
    bytes32 internal constant ALLOWED_MEDIATOR = 0x0663c2f596d549523cd16d233f6ea03a179da6a59887705779f4429c84ed02ea; // keccak256(abi.encodePacked("allowedMediator"))
    ...
    function initialize(
        ...
        _setAllowedMediator(_allowedMediator);
        _setOwner(_owner);
        setInitialize();
    ...
    function allowedMediator() external view returns (address) {
        return addressStorage[ALLOWED_MEDIATOR];
    }
    ...
    /**
     * Sets the mediator contract that is allowed to pass messages between chains.
     * @param _allowedMediator address.
     */
    function setAllowedMediator(address _allowedMediator) external onlyOwner {
        _setAllowedMediator(_allowedMediator);
    }
    ...
    /**
     * Internal function to store allowed mediator address.
     * @param _allowedMediator address.
     */
    function _setAllowedMediator(address _allowedMediator) internal {
        addressStorage[ALLOWED_MEDIATOR] = _allowedMediator;
    }
...
contract MessageDelivery is BasicAMB, MessageProcessor {
    ...
    * If allowed mediator is zero address = everyone can pass messages.
    * If allowed mediator is any other address = only that address can pass messages.
    ...
    */
    function requireToPassMessage(address _contract, bytes memory _data, uint256 _gas) public returns (bytes32) {
        if (addressStorage[ALLOWED_MEDIATOR] != address(0)) {
            require(msg.sender == addressStorage[ALLOWED_MEDIATOR]);
        }
