...
contract Lottery is OwnableUpgradeable {
    ...
    uint8 constant keyLengthForEachBuy = 3;
    ...
    mapping (uint256 => uint8[2]) public lotteryTicketInfo;
    mapping (uint256 => uint256) public lotteryTicketAmount;
    mapping (uint256 => uint256) public issueIndexTicketMap;
    ...
    function internalBuy(uint256 _price, uint8[2] memory _numbers) internal {
        require (!drawed(), 'drawed, can not buy now');
        for (uint i = 0; i < 2; i++) {
        ...
        lotteryTicketInfo[newItemId] = _numbers;
        lotteryTicketAmount[newItemId] = _price;
        issueIndexTicketMap[newItemId] = issueIndex;
    ...
    function buy(uint256 _price, uint8[2] memory _numbers) external inDrawingPhase{
        ...
        lotteryTicketAmount[newItemId] = _price;
        issueIndexTicketMap[newItemId] = issueIndex;
        ownerOfTicket[newItemId] = msg.sender;
    ...
    function multiBuy(uint256 _price, uint8[2][] memory _numbers) external inDrawingPhase {
        require (_price >= minPrice, 'price must above minPrice');
    ...
    function claimReward(uint256 _tokenId) external {
        require(msg.sender == ownerOfTicket[_tokenId], "not from owner");
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        for (uint i = 0; i < _tickets.length; i++) {
            require (msg.sender == ownerOfTicket[_tickets[i]], "not from owner");
    ...
    function getRewardView(uint256 _tokenId) public view returns(uint256) {
        uint256 _issueIndex = issueIndexTicketMap[_tokenId];
        uint8[2] memory lotteryNumbers = lotteryTicketInfo[_tokenId];
        ...
        uint256 reward = 0;
        if (matchingNumber > 0) {
            uint256 amount = lotteryTicketAmount[_tokenId];
