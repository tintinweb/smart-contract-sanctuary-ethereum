...
contract TicketExs is ERC721Enumerable, Ownable, ERC2981, ReentrancyGuard {
    using Strings for uint256;
    ...
    mapping(uint256 => bool) public isPunched;
    ...
    modifier onlyWhitelisted() {
        require(
            UserInfo(userInfo).isWhitelisted(msg.sender),
    ...
    function buyTickets(uint256 _qty) external payable onlyWhitelisted {
        uint256 totalAmount = _qty * mintPrice;
    ...
    function punchTicket(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(!isPunched[_tokenIds[i]], "Ticket already punched");
            attendees.push(_msgSender());
            _tokenURIs[_tokenIds[i]] = _assignTokenURI();
            isPunched[_tokenIds[i]] = true;
