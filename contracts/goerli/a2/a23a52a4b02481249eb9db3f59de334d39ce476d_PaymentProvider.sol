...
contract PaymentProvider is Ownable, ReentrancyGuard {
    ...
    function adminGiftPlan(address[] calldata _addresses, uint256 _days) public onlyOwner {
    ...
    function removeSubscriptions(address[] calldata _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            address _address = _addresses[i];
            uint256 addressExpirationTimestamp = expirationTime[_address];
    ...
    // Set a new base fee
    function setBaseFee(uint256 _newBaseFee) public onlyOwner {
