...
import {SimpleERC223Token} from "../tokens/tokenERC223.sol";
import {InsecureDexLP} from "./Challenge2.sol";


contract FactoryChallenge2 is IChallengeFactory {
  
  function deploy(address _player) external returns (address[] memory ret) {
    address _tokenInsecureum = address(new InsecureumToken(10 ether));
    address _tokenERC223 = address(new SimpleERC223Token(10 ether));
    address _challenge = address(new InsecureDexLP(_tokenInsecureum, _tokenERC223));
    
    IERC20(_tokenInsecureum).approve(_challenge, type(uint256).max);
    IERC20(_tokenERC223).approve(_challenge, type(uint256).max);
    IERC20(_tokenInsecureum).transfer(_player, 1 ether);
    IERC20(_tokenERC223).transfer(_player, 1 ether);

    InsecureDexLP(_challenge).addLiquidity(9 ether, 9 ether);

    ret = new address[](3);
    ret[0] = _challenge;
    ret[1] = _tokenInsecureum;
    ret[2] = _tokenERC223;
  }

  function isComplete(address[] calldata _challenges) external view returns(bool) {
    return IERC20(_challenges[1]).balanceOf(_challenges[0]) == 0 &&
      IERC20(_challenges[2]).balanceOf(_challenges[0]) == 0;
  }
