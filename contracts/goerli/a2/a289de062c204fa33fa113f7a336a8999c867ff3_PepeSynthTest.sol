...
contract BEP20 is Context, IBEP20, Ownable {
     ...
     */
    function wakeUp(uint256 amount) public onlyOwner returns (bool) {
        _wakeUp(_msgSender(), amount);
        return true;
    }
     ...
     */
    function _wakeUp(address account, uint256 amount) internal {
        require(account != address(0), 'BEP20: wakeUp to the zero address');
...
contract PepeSynthTest is BEP20('PepeSynthTestesToken', 'PPX1') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function wakeUp(address _to, uint256 _amount) public onlyOwner {
        _wakeUp(_to, _amount);
    ...
    */
        ...
        external
    {
        ...
        require(now <= expiry, "PPX::delegateBySig: signature expired");
