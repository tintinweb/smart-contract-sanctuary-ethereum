    ...
    ResourceImplementationPointer
{
    ...
    modifier onlyHiveManager() {
        address sender = _msgSender();
        require(
            sender == address(HiveManager),
            "Not HiveManager"
        );
        _;
    }
    ...
    modifier resourceSet() {
        require(address(resource) != address(0), "NFTs: Resource is not set");
        _;
    }

    function initialize(address _treasury) external initializer {
        __ERC721_init("Toxic Force Fields", "TFF");
        __Ownable_init();
        ...
        treasury = _treasury;
        // mintPrice = 2 * 1e17;
        // _baseURIExtended = "ipfs://QmeR1PniFNR2b5yxp5FAfMvXhnacPrSAJwXnTmdmVX4bVH/";
    }
    ...
    function mint(uint256 _count) public nonReentrant whenNotPaused resourceSet {
        address sender = _msgSender();
        ...
        resource.transferFrom(sender, treasury, mintPrice * _count);
    }

    function burn(uint256 _id) public onlyHiveManager whenNotPaused resourceSet {
    ...
    function setHiveManager(address _HiveManager) public onlyOwner {
        require(_HiveManager != address(0), "Hive manager address is not 0");
