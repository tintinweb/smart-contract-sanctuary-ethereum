...
contract EnglishAuction is ERC721A, ERC2981, ReentrancyGuard, AccessControl, Ownable {
    bytes32 public constant SUPPORT_ROLE = keccak256('SUPPORT');
    bytes32 public constant REFUND_ROLE = keccak256('REFUND');
    
    uint256 public constant MAX_SUPPLY = 200;
    uint256 public minimumContribution = 0.05 ether;
    ...
    constructor(address payable withdrawAddress_) ERC721A("AuctionContract", "AUC") {
    ...
    */
    function _internalMint(address to, uint256 numberOfTokens) internal {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce))) % 151;
            nonce++;
            while (usedIds[tokenId]) {
                tokenId = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce))) % 151;
                nonce++;
            }
