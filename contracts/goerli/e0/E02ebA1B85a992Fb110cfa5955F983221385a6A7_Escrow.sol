...
contract Escrow is Ownable{
    ...
    address[] public admins; //Admins list
    ...
    */
    function createContract(uint _uid,uint _amount, address payable _buyer, address payable _seller, address _currency) public payable{
        ...
        require(_amount>0, "Transaction value can not be empty");
        require(agreements[_uid].amount==0,"Can not overwrite transaction");
    ...
    */
    function acceptContract(uint _uid) public onlySeller(_uid) onlyValidated(_uid){
        require(agreements[_uid].stage==Stage.initiated,"Wrong stage" );
    ...
    */
    function claim(uint _uid) public onlyBuyer(_uid) onlyValidated(_uid){
        ...
        require(agreements[_uid].stage==Stage.denied || (agreements[_uid].stage==Stage.initiated && block.timestamp>agreements[_uid].lastTime), "Can not claim at this stage or time");
        agreements[_uid].validated=false;
    ...
    */
    function markProductSent(uint _uid) public {
        ...
        require(agreements[_uid].stage==Stage.accepted, "Wrong stage");
    ...
    */
    function markProductreceived(uint _uid) public {
        ...
        require(agreements[_uid].stage==Stage.productSent, "Wrong stage");
    ...
    @custom:events this emits "Complete" event from route.
    */
    function mintAndMarkComplete(uint _uid, string memory _tokenURI) public onlySeller(_uid) {
        require(agreements[_uid].stage==Stage.productReceived, "Wrong stage");
    ...
    */
    function reaiseDispute(uint _uid) public {
        ...
        require(block.timestamp>agreements[_uid].lastTime,"time now has to be more than the minimum time.");
        //Status has to be either accepted or productSent.
        require(agreements[_uid].stage==Stage.accepted || agreements[_uid].stage==Stage.productSent, "Wrong stage");
    ...
    */
    function awardDispute(uint _uid, address payable _winner) public onlyAdmin(msg.sender) {
        require(agreements[_uid].stage==Stage.onDispute, "Wrong stage");
    ...
    */
    ...
    /**  
    @notice Function used to refund money as admin
    @param _uid Predefined unique identifier for contract/agreement.
    @custom:events this emits "Delete" event.
    */
    function refund(uint _uid) public onlyAdmin(msg.sender){
        address payable wallet = payable(agreements[_uid].buyer);
        if (agreements[_uid].currency==address(0x0000000000000000000000000000000000000000)){
            wallet.transfer(agreements[_uid].amount);
        }else{
            IERC20 currency = IERC20(agreements[_uid].currency);
            currency.transfer(wallet, (agreements[_uid].amount));
        }
        delete agreements[_uid];
        emit Delete(_uid);
    }
    ...
    */
    ...
    /**  
    @notice Changes fee for dispute
    @param _fee Fee amount
    */
    function changeDisputeFee(uint _fee) public onlyOwner {
        disputFee = _fee;
    }

    /**  
    @notice Changes oracle address
    @param _oracle _oracle address
    */
    ...
    /**  
    @notice Changes NFT address
    @param _NFT _NFT address
    */
    function changeNFT(NFTCOLLECTION _NFT) public onlyOwner {
        NFT = _NFT;
    }
}
