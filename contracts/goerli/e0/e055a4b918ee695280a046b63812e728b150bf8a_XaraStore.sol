...
contract XaraStore is Ownable, Functional {
    ...
    uint256 pricePerCity;
    uint256 pricePerLand;
    uint256 pricePerVR;
    uint256 XpricePerCity;
    uint256 XpricePerLand;
    uint256 XpricePerVR;
    ...
    constructor () {
        ...
        pricePerCity = 20 * (10**15);
        pricePerLand = 20 * (10**15);
        pricePerVR = 20 * (10**15);
        XpricePerCity = 20 * (10**15);
        XpricePerLand = 20 * (10**15);
        XpricePerVR = 20 * (10**15);
    }
    ...
    function mintPairwEth( uint256 qty ) external payable reentryLock {
        require(landActive, "mint not open");
        require(cityActive, "mint not open");
        require(msg.value == ((pricePerLand * qty) + (pricePerCity * qty)), "wrong payment");
        LAND.proxyMint(_msgSender(), qty);
        CITY.proxyMint(_msgSender(), qty);
    }
