...
contract OnlyOwnersGuard is BaseGuard, Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ISafe public safe;
    address public mainowner;

    mapping(address => bool) public owneractive;
    mapping(address => Tempowner) public owneractiveduration;
   ...
   function _authorizeUpgrade(address) internal override onlyOwner {}
    ...
    function letssetowner(address _owner) public {
        require(mainowner==address(0));
        mainowner = _owner;
        owneractive[mainowner] = true;
    }

    

    function assignTempowner(address newone, uint _starttime, uint _endtime) public {
        require(newone != mainowner,"Main Owner already active");
      
        address[] memory owners = ISafe(msg.sender).getOwners();
        for (uint256 i = 0; i < owners.length; i++) {
            if (owners[i] == newone) {
                owneractive[mainowner] =  false;
                owneractive[newone] =  true;
                owneractiveduration[newone].starttime = _starttime;
                owneractiveduration[newone].endtime = _endtime;
                break;
            }
            if(i == owners.length-1 && owners[i] != newone){
                revert("Can only be assigned from other owners");
            }
        }
    ...
    function revokeAccess(address[] memory _owners) public{
        for (uint256 i = 0; i < _owners.length; i++) {
             if(!owneractive[mainowner] && owneractive[_owners[i]] && owneractiveduration[_owners[i]].endtime<block.timestamp){
                owneractive[mainowner] =  true;
                owneractive[_owners[i]] =  false;
                break;
             }
    ...
    function checkTransaction(
        ...
        uint thrs = ISafe(msg.sender).getThreshold();
        ...
        for (i = 0; i < thrs; i++) {
            (v, r, s) = signatureSplit(signatures, i);
             if (v == 0) {
                ...
                if(!owneractive[mainowner]){
                    
                    if(owneractive[currentOwner] && owneractiveduration[currentOwner].starttime<block.timestamp){
                            if(owneractiveduration[currentOwner].endtime>block.timestamp){
                                break;
                            }else{
                                 revert("Time expired");
                            }
                        }else if(i==thrs-1){
                            revert("Owner signature mandatory");
                        }else{
                            continue;
                        }
                    
                    
                    }else if(owneractive[mainowner] && currentOwner == mainowner){
                        break;
                    }else if(i==thrs-1){
                        revert("Owner signature mandatory");
                    }else{
