 ...
 * @title Pledge
 * @author lixin
 * @notice Pledge Handle all NFT related operations in the pledged loan module
 */
contract Pledge is PledgeInterface, Manageable ,ReentrancyGuard{

    // Capital pool for storing NFT
    address public pledgeCapitalPool;

    address public loan;

    //NFTAddr => NFTId => MortgageNote
    mapping(address => mapping(uint256 => MortgageNote)) public mortgageNote;

    /**
     * @notice Constructor of pledge NFT module
     *
     */
    constructor(address newCapitalPool) {
        setPledgeCapitalPool(newCapitalPool);
    }
     ...
     */
    function setPledgeCapitalPool(
        address newCapitalPool
    ) public onlyManager {
        address oldLoanRation = pledgeCapitalPool;
        pledgeCapitalPool = newCapitalPool;
        emit CapitalPoolChanged(oldLoanRation, newCapitalPool);
    }
    ...
    function pledgeNFT(
        ...
        NFTInterface(NFTAddr).safeTransferFrom(_msgSender(), pledgeCapitalPool, NFTId, bytes(businessId));
        if (NFTInterface(NFTAddr).ownerOf(NFTId) != pledgeCapitalPool) {
            revert NFTTransferFailed();
        }

        (, uint256 tokenAmount) = NFTInterface(NFTAddr).checkPrice(NFTId);
        uint256 loanRatio = NFTInterface(NFTAddr).loanRatio();
        MortgageNote storage note = mortgageNote[NFTAddr][NFTId];
        note.mortgagor = msg.sender;
        note.NFTAddr = NFTAddr;
        note.NFTId = NFTId;
        note.FTAddr = FTAddr;
        note.NFTPrice = tokenAmount;
        note.capitalPool = address(pledgeCapitalPool);
        note.loanRatio = loanRatio;

        emit PledgeNFT(businessId, NFTAddr, NFTId, FTAddr, pledgeCapitalPool);
        return true;
    }
    ...
    function releaseNFT(
        ...
        if (!verify(NFTAddr, NFTId, managerSign)) {
            revert IllegalManagerSign();
        }

        bool status = LoanInterface(loan).checkLoanStatus(NFTAddr, NFTId);
        if (!status) {
            revert OverdueLoan();
        }

        (,uint256 repaymentTotal, uint256 repaymentAmount) = LoanInterface(loan).getLoanAmount(NFTAddr, NFTId);
        if (repaymentTotal != repaymentAmount) {
            revert LoanNotFinished();
        }

        address mortgagor = mortgageNote[NFTAddr][NFTId].mortgagor;
        if (mortgagor != msg.sender) {
            revert NOTYOURNFT();
        }

        MortgageNote memory note = mortgageNote[NFTAddr][NFTId];
        CapitalPoolInterface(note.capitalPool).withdrawNFT(businessId, NFTAddr, NFTId, _msgSender());
        delete mortgageNote[NFTAddr][NFTId];
        emit ReleaseNFT(businessId, NFTAddr, NFTId, _msgSender());
        return true;
    }
    ...
    function verify(
        ...
        bytes32 hash = keccak256(abi.encode(tokenAddr, tokenId, _txOrigin()));
    ...
    /**
     * @notice Force to close out the user's NFT and transfer it to the address specified by the manager.
     *         Use the safeTransferFrom method of ERC721.
     *
     * @param businessId Used as business differentiation.
     * @param NFTAddr The ERC721 contract address.
    ...
    function closeOutNFT(
        ...
        bool status = LoanInterface(loan).checkLoanStatus(NFTAddr, NFTId);
        if (status) {
            revert NOTOverdueLoan();
        }


        MortgageNote memory note = mortgageNote[NFTAddr][NFTId];
        CapitalPoolInterface(note.capitalPool).withdrawNFTManager(businessId, NFTAddr, NFTId, recipient);
        delete mortgageNote[NFTAddr][NFTId];
        emit CloseOutNFT(businessId, NFTAddr, NFTId, recipient);
        return true;
    }
    ...
    function checkMortgageNote(
        ...
        MortgageNote memory mNote = mortgageNote[NFTAddr_][NFTId_];
        note.mortgagor = mNote.mortgagor;
        note.NFTPrice = mNote.NFTPrice;
        note.FTAddr = mNote.FTAddr;
        note.loanRatio = mNote.loanRatio;
        return note;
    }
