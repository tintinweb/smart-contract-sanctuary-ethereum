...
// @title contract which allows early investor to deposit ETH
// @notice send purchase orders to staakeToken contract when staake team `withdrawAll`
contract EarlySale is ISale, PriceCalculator, AccessControl {
    ...
    function buy() external payable {
        ...
        require(
            ...
            "amount should be at most 50 ETH"
        );

        uint256 stk = getPriceConversion(msg.value);
        require(stk <= availableToken, "Not enough token available");
    ...
    /**
     * @notice initialize receiver
     * makes `WithdrawAll()` available
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        require(
            ...
            "address is not a valid receiver"
        );
        receiver = IEarlySaleReceiver(_address);
    }

    /**
     * @notice Withdraw all ETHs
     * close the sale & deposit buy order on StakeSale Contract
     * you can only call this function ONCE
     * and only if StakeSale address is set
     */
    function withdrawAll() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            address(receiver) != address(0),
            "StaakeSale address not specified"
        );
    ...
    /**
     * @notice function called to allow an address to invest
     * @param _earlyInvestors, array of address of the investor
     */
    ...
    /**
     * @notice function called to revoke `buy` access from an address
     * @param _earlyInvestors, array of address of the investor
     */
    ...
    /**
     * @notice function called to toggle is Public
     */
