// SPDX-License-Identifier: MIT
    ...
    uint256 public decimalNumber = 18;
    ...
    // Contract Addresses
    address _nft_Contract = 0x6982fc3779ADf331b3E67b5eB5A299A299B4e851;
    address _token_Contract = 0x3d3A68Ce8DD368af6133e6505B61fDA470A10396;
    ...
    function buyItem(uint256 itemCategoryNumber, string memory sendingAddress, uint256 quantity) public payable {
               
         
           if(itemCategoryNumber == 1){
           require(token.balanceOf(msg.sender) >= quantity * sweatshirts * 10**18); 
           token.transferFrom(msg.sender, address(this), quantity * sweatshirts * 10**18);       

        }

          if(itemCategoryNumber == 2){
           require(token.balanceOf(msg.sender) >= quantity * tShirts * 10**18); 
           token.transferFrom(msg.sender, address(this), quantity * tShirts * 10**18);
        }

          if(itemCategoryNumber == 3){
           require(token.balanceOf(msg.sender) >= quantity * seatAtToonHall * 10**18); 
           token.transferFrom(msg.sender, address(this), quantity * seatAtToonHall * 10**18);
        }

          if(itemCategoryNumber == 4){
           require(token.balanceOf(msg.sender) >= quantity * createACustomSticker * 10**18); 
           token.transferFrom(msg.sender, address(this), quantity * createACustomSticker * 10**18);
        }

          if(itemCategoryNumber == 5){
           require(token.balanceOf(msg.sender) >= quantity * writeYourStory * 10**18); 
           token.transferFrom(msg.sender, address(this), quantity * writeYourStory * 10**18);
        }

          if(itemCategoryNumber == 6){
           require(token.balanceOf(msg.sender) >= quantity * featureYourNFTInComic * 10**18); 
           token.transferFrom(msg.sender, address(this), quantity * featureYourNFTInComic * 10**18);
        }

        Order memory newOrder = Order(orderCount, block.timestamp, msg.sender, itemCategoryNumber, sendingAddress, quantity, token.balanceOf(msg.sender));
    ...
    function recordSentOrders(uint256 orderNumber) public {
