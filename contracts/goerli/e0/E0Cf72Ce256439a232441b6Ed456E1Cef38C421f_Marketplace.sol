...
contract Marketplace {
    ...
    function createListing(
        ...
        // allow marketplace to access nft
        require(price > 0);
        require(
        ...
        isListed(nftAddress, tokenID)
    {
        ...
        uint initBalance = msg.sender.balance;

        (bool callSuccess, ) = payable(listing.seller).call{value: msg.value}(
            ""
        );
        require(callSuccess);

        require(initBalance + msg.value == msg.sender.balance);
    ...
    function getListingSeller(address nftAddress, uint tokenID)
        external
        view
        isListed(nftAddress, tokenID)
        returns (address)
