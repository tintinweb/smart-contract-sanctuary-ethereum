...
contract Fartz is Ownable, ERC721A, IERC2981, ReentrancyGuard {
    ...
    uint64 public MAX_SUPPLY = 4_269;
    uint64 public MAX_MINT_QUANTITY = 5;
    uint64 public MAX_MINT_ALLOWANCE = 10;

    uint256 public PUBLIC_SALE_PRICE = 42 ether;
    ...
    string public DESCRIPTION = "We all know what you did... metadata and img fully on chain, forever.";
    ...
    string[] private _traitsSmell = [
        ...
        "burnt brown sugar and peanuts"
    ];
    string[] private _traitsWetness = [
        "tropical",
        "like a full blown monsoon",
        "greasy",
        "dry",
    ...
    string[] private _traitsDuration = [
        "just a squeak",     
        "short",
        "a juicy crunch with punctuation",
        "a jackhammer",
        "delong one",
        "uncomfortably long"
    ];
    string[] private _traitsWithWho = [
        "your step mom",
        "your favourite cam girl",
        "your life coach",
        "your wifes boyfriend",
        "a crowd of strangers",
        "your egirl crush",
        "a hot date jk you dont get dates",
        "scotts mom",
        "zaks mom",
        "the canto conmunity"
    ];
    string[] private _traitsWhereAreYou = [
        ...
        "while walking a small feminine dog",
        "while waiting for the fucking bridge",
        "at your grandparents funeral",
        "in an elevator",
        "on a plane",
        "on an escalator",
        "in line at the starbucks",
        "in zaks moms bed",
        "at your girlfriends parents dinner table",
        "while aping an ohm fork"
    ];
    ...
    constructor(address owner_) ERC721A("Fartz", "FRTZ") {
    ...
    function _generateFart(address receiver_, uint256 tokenId_) internal view returns (bytes32) {
        ...
        // 4 - who are you with
        b[4] = _generateRandomIndex(seed_, _raritiesWithWho, 4);

        // 5 - where are you
        b[5] = _generateRandomIndex(seed_, _raritiesWhereAreYou, 5);

        // 6 - intentionality
        b[6] = _generateRandomIndex(seed_, _raritiesIntentionality, 6);

        // 7 - level of relief
        b[7] = _generateRandomIndex(seed_, _raritiesRelief, 7);
    ...
    function _buildFart(uint256 tokenId_) public view returns (string memory properties, string memory svg) {
        ...
        return (
            ...
            _renderFart(
                ...
                _traitsWithWho[uint8(dna[4])],
                _traitsWhereAreYou[uint8(dna[5])],
                _traitsIntentionality[uint8(dna[6])],
                _traitsRelief[uint8(dna[7])]
            )
        );
    }
    ...
    function _fartProperties(bytes32 dna) internal view returns (string memory properties) {
        return string(
            abi.encodePacked(
                ...
                _packProperty("who are you with",    _traitsWithWho[uint8(dna[4])], false),   
                _packProperty("where are you",       _traitsWhereAreYou[uint8(dna[5])], false),   
                _packProperty("intentionality",      _traitsIntentionality[uint8(dna[6])], false),   
                _packProperty("level of relief",     _traitsRelief[uint8(dna[7])], true)
            )
        );
    }
        ...
        return string(
            abi.encodePacked(
                ...
                abi.encodePacked(
                    ...
                    whereAreYou,
                    '<tspan class="c">, </tspan></tspan></text><text class="i" style="text-align: center;" transform="translate(143.11 438.95)"><tspan class="h"> ',
                    intentiality,
                    '</tspan></text><text class="i" style="text-align: center;" transform="translate(143.11 472.34)"><tspan class="c">In front of</tspan><tspan class="h"> ',
                    withWho,
                    '</tspan></text><text class="i" transform="translate(143.11 505.73)"><tspan class="c">After eating</tspan><tspan class="f"> ',
                    cuisine
                ),
                abi.encodePacked(
                    '</tspan></text><text class="i" transform="translate(143.11 539.12)"><tspan class="c">It was</tspan><tspan class="h"> ',
                    duration,
                    '</tspan><tspan class="c"> and</tspan></text><text class="i" transform="translate(143.11 572.51)"><tspan class="c">smelled like</tspan><tspan class="h"> ',
                    smell,
                    '</tspan></text><text class="i" transform="translate(143.11 605.9)"><tspan class="c">It felt</tspan><tspan class="h"> ',
                    wetness,
                    '</tspan></text><text class="j" transform="translate(143.11 685.79)"><tspan x="0" y="0">~ ',
                    relief
                ),
                svgEnd
            )
    ...
    function withdraw() public {
        (bool success_,) = _owner.call{value : address(this).balance}("");
