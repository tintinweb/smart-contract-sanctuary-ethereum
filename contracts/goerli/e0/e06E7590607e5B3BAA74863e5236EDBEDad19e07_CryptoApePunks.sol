...
contract CryptoApePunks is ERC721r {
    uint256 public constant MAX_TOKENS = 100;
    uint256 private _numTokensIssued;

    // Mapping that records the number of NFTs generated for each wallet address
    mapping(address => uint256) private _numTokensPerWallet;

    constructor () ERC721r("CryptoApePunks", "APEPUNK", MAX_TOKENS) {}

    // You can only mint 12 tokens per wallet
    function mint(uint256 quantity) external {
        require(_numTokensIssued + quantity <= MAX_TOKENS, "Exceeded maximum number of tokens that can be issued");
        require(_numTokensPerWallet[msg.sender] + quantity <= 12, "Exceeded maximum number of tokens per wallet");
        _mintRandom(msg.sender, quantity);
        _numTokensPerWallet[msg.sender] += quantity;
