...
/**
 * @title PolemosStaking
 *
 * @notice handle protocol revenue distribution
 *
 */
contract PolemosStaking {
    using SafeERC20 for IERC20;
    IPool tPlms;
    IPoolBase tPlmsStakingPool;

  constructor(
    IPool _tPlms,
    IPoolBase _tPlmsStakingPool
  ) {
    // verify the inputs are set
    require(address(_tPlms) != address(0), 'tPLMS address not set');
    require(address(_tPlmsStakingPool) != address(0), '_tPlmsStakingPool address not set');

    tPlms = _tPlms;
    tPlmsStakingPool = _tPlmsStakingPool;
  }

  function stake(uint256 _amount, uint64 _lockUntil) external {
      IERC20 underlyer = IERC20(tPlms.underlyer());

      underlyer.safeTransferFrom(msg.sender, address(this), _amount);
      underlyer.approve(address(tPlms), _amount);
      tPlms.deposit(_amount);

      IERC20(address(tPlms)).safeApprove(address(tPlmsStakingPool), _amount);
      tPlmsStakingPool.stakeToPoolFor(msg.sender, _amount, _lockUntil);
    }
}
