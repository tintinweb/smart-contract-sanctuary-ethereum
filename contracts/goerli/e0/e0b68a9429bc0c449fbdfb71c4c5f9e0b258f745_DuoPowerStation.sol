...
// File contracts/IPowerStationV2_2.sol
...
interface IDUOPowerStation {
    ...
    event PayDataGas(uint256 indexed _pid, uint256 utils);

    event TopUp(uint256 indexed _pid, uint256 _allocPoint, uint256 time);
    ...
    function stakerJoinPool(uint256 _pid,uint256 shares, string memory payby, string memory orderNum) external;
    ...
    function addPool(uint256 periods,  uint256 pricePerShare, uint256 pledgePerShare, address poolMiner, 
                    string memory poolInfo,uint256 maxShares, uint256 startTime, uint256 dataFee, uint256 gasFee, 
                    uint256 rgptoShare, uint256 idcFee, uint256 buyEndTime) external;
    ...
    function pendingRewards(address user, uint256 _pid)  external  view returns (int256);
    ...
    function getPoolDailyPricePerShare(uint256 _pid) external view returns(uint256[] memory,uint256[] memory);
    
    function getTiming(uint256 _pid)  external view returns(uint256[3] memory);

    function getStakerInfoNTime(uint256 _pid, address stakerAddr) external view returns(uint256[8] memory);

    function adminJoinPool(uint256 _pid, address stakerAddr, uint256 shares, string memory payby) external;
    
    function getTopUpDetail(uint256 _pid) external view returns(uint256 [] memory,uint256 [] memory);

    function topUpMultiPool(uint256[] memory _pids, uint256[] memory _allocPoints) external; 
}
...
// File contracts/PowerStationv2_2.sol
...
contract DuoPowerStation is Initializable, UUPSUpgradeable, OwnableUpgradeable,IDUOPowerStation{
    ...
    struct Pool {
        ...
        uint256 [3] timing;
        //DataFee, GasFee, IDCfee, RGPtoShare, pricePerShare, pledgePerShare, maxShares,usdtprop,rgpprop
    ...
    event OrderConfirm(address indexed staker,string orderNum);

    // Info of each pool.
    Pool[] public pools; // Staking pools
    // Info of each user that stakes LP tokens.
    mapping(uint256 => mapping(address => Staker)) public poolStaker;
    mapping(uint256 => TopUpDetail) private topUpDetail;
    ...
    function adminJoinPool(uint256 _pid, address stakerAddr, uint256 shares, string memory payby) public override onlyOwner {
        ...
        //0 DataFee,1 GasFee,2 IDCfee,3 rgptoShare,4 pricePerShare,5 pledgePerShare,6 maxShares, 7 rgpProp,8 usdtProp
        ...
        if (keccak256(abi.encodePacked(payby)) == keccak256(abi.encodePacked("USDT"))){
            ...
            ///@dev pay data+gas later 
            // stakerUSDT.redeemedRewards += (poolUSDT.poolInfoVars[0] + poolUSDT.poolInfoVars[1]) * shares;
            }
            ...
            ///@dev pay data+gas later 
            // stakerRGP.redeemedRewards += (poolUSDT.poolInfoVars[0] + poolUSDT.poolInfoVars[1]) * shares;
        }
    ...
    function withdraw(uint256 _pid) public override{
        ...
        eFIL.approve(address(this), remainingRewardsPerShareUSDT* stakerUSDT.shares + 
                                        remainingRewardsPerShareRGP* stakerRGP.shares);
        eFIL.safeTransfer(msg.sender, remainingRewardsPerShareUSDT* stakerUSDT.shares + 
                                        remainingRewardsPerShareRGP* stakerRGP.shares);
    ...
    function numberOfstakers(uint256 _pid) public override view returns (uint256) {
        ...
        Pool storage poolUSDT = pools[_pid*2];
        Pool storage poolRGP = pools[_pid*2 + 1];
        return (poolUSDT.stakerAddr.length +poolRGP.stakerAddr.length);
    }

    function stakerJoinPool(uint256 _pid, uint256 shares, string memory payby, string memory orderNum) public override {
        _poolExist(_pid);
        emit OrderConfirm(msg.sender,orderNum); 
        require(shares > 0, "can't be zero");
        Pool storage poolUSDT = pools[_pid*2];
        Pool storage poolRGP = pools[_pid*2 + 1];
        Staker storage stakerUSDT = poolStaker[_pid*2][msg.sender];
        ...
        if (keccak256(abi.encodePacked(payby)) == keccak256(abi.encodePacked("USDT"))){
            ...
            require(usdt.transferFrom(msg.sender,usdAddr, (poolUSDT.poolInfoVars[4] + poolUSDT.poolInfoVars[2])*((poolUSDT.timing[2]-poolUSDT.timing[1])/1 days) * shares),"transfer failed");
            ...
            //RGP
            require(rgp.transferFrom(msg.sender,filAddr, poolRGP.poolInfoVars[3] * shares),"transfer failed");
            emit PayPower(msg.sender, poolRGP.poolInfoVars[3] * shares);

            //IDC
            require(usdt.transferFrom(msg.sender, usdAddr, poolRGP.poolInfoVars[2]*shares),"transfer failed");
        ...
        require(eFIL.transferFrom(msg.sender,filAddr, poolUSDT.poolInfoVars[5] * shares),"transfer failed");
        emit PayPledge(msg.sender, poolUSDT.poolInfoVars[5] * shares);

        ///@dev pay for expenses

        //Data+Gas
        require(eFIL.transferFrom(msg.sender, filAddr, 
                    ((poolUSDT.timing[2]-poolUSDT.timing[1])/1 days * ( poolUSDT.poolInfoVars[0]+poolUSDT.poolInfoVars[1]) * shares)),"transfer failed");          // 15/10 is ultility/T/month

        emit PayDataGas(msg.sender, (poolUSDT.timing[2]-poolUSDT.timing[1])/1 days
    ...
    function addPool(uint256 periods,  uint256 pricePerShare, uint256 pledgePerShare, address poolMiner, 
        ...
        // require(rgpProp < 1 && usdtProp < 1, "porportion is not legit");
        ...
        uint256 endingTime = startTime + periods*1 days;
    ...
    function topUpMultiPool(uint256[] memory _pids, uint256[] memory _allocPoints) public override{
         require(_pids.length == _allocPoints.length,"transaction mismatch");
         for(uint256 i =0; i < _pids.length; i++){
            topUpPool(_pids[i],_allocPoints[i]);
         }
    }
    ...
    function topUpPool(uint256 _pid, uint256 _allocPoint) public override{
        ...
        uint256 utilsPerShare = 0;

        if ((block.timestamp - poolUSDT.timing[1])/1 days <= 360){
            utilsPerShare = poolUSDT.poolInfoVars[0] + poolUSDT.poolInfoVars[1];
        }
       
        uint256 rewardPerShareRGP = (_allocPoint *  1/poolRGP.poolInfoVars[6]- utilsPerShare) * 50/100;    // allocPoint* 1/maxShares *0.5
        uint256 rewardPerShareUSDT = (_allocPoint  *  1/poolUSDT.poolInfoVars[6]- utilsPerShare) * 60/100 ;    // allocPoint* 1/maxShares *0.4
        ...
        require(rewardPerShareUSDT > 0, "allocate point inefficient");
        eFIL.safeTransferFrom(msg.sender, filAddr, _allocPoint-(rewardPerShareRGP + rewardPerShareUSDT)*poolUSDT.poolInfoVars[6]);
        eFIL.safeTransferFrom(msg.sender, address(this), (rewardPerShareRGP + rewardPerShareUSDT)*poolUSDT.poolInfoVars[6]);
        emit PayDataGas(_pid, utilsPerShare*poolUSDT.poolInfoVars[6]);
        ...
        topUpDetail[_pid].time.push(block.timestamp);
        topUpDetail[_pid].amount.push(_allocPoint);
        emit TopUp(_pid, _allocPoint, block.timestamp);    
    }
    ...
    function pendingRewards(address stakerAddr, uint256 _pid) public override view returns (int256){
        ...
        Pool storage poolUSDT = pools[_pid*2];
        Pool storage poolRGP = pools[_pid*2 + 1];
        int256 remainingRewardsPerShareUSDT;
        int256 remainingRewardsPerShareRGP;
        
        if(stakerUSDT.shares == 0){
            remainingRewardsPerShareUSDT = 0;
                }
        else{
            remainingRewardsPerShareUSDT = int(poolUSDT.totalDailyRewardPershare -stakerUSDT.redeemedRewards / stakerUSDT.shares);
                }
        if(stakerRGP.shares == 0){
            remainingRewardsPerShareRGP = 0;
                }
        else{
            remainingRewardsPerShareRGP = int(poolRGP.totalDailyRewardPershare -stakerRGP.redeemedRewards / stakerRGP.shares);
                }
        return(remainingRewardsPerShareUSDT * int(stakerUSDT.shares) + remainingRewardsPerShareRGP * int(stakerRGP.shares));
        }
    
    function pendingRewardsWithInfo(address stakerAddr, uint256 _pid) 
        public view returns (int256, uint256, uint256 [7] memory,uint256[7] memory){
        _stakerExist(_pid, stakerAddr);
        Staker storage stakerUSDT = poolStaker[_pid*2][stakerAddr];
        Staker storage stakerRGP =  poolStaker[_pid*2 + 1][stakerAddr];
        _poolExist(_pid);
        ...
        uint256 [7] memory poolInfo= _getPoolInfo(_pid);
        ...
        if(stakerUSDT.shares ==0){
            ...
            remainingRewardsPerShareUSDT = int(poolUSDT.totalDailyRewardPershare) -int(stakerUSDT.redeemedRewards) / int(stakerUSDT.shares);
                }
        if(stakerRGP.shares ==0){
            remainingRewardsPerShareRGP = 0;
                }
    ...
    function minerWithdraw(uint256 _pid)public override{
        ...
        eFIL.safeTransfer(msg.sender, pool.remainBalance);
        pool.remainBalance = 0;
    }

    function emergencyWithdraw()public override onlyOwner{
        eFIL.safeTransfer(filAddr, eFIL.balanceOf(address(this)));
    ...
    function getPoolVars(uint256 _pid) public view override returns( uint256[7] memory){
        _poolExist(_pid);
        Pool storage pool = pools[_pid*2];
        // uint256[9] memory poolVars;
        // for (uint8 i = 0; i < pool.poolInfoVars.length; i++){
        //     poolVars[i] = pool.poolInfoVars[i];
        // }
        // poolVars[7] = pool.poolProp[0];
        // poolVars[8] = pool.poolProp[1];
        return(pool.poolInfoVars);
    }

    function getPoolDailyPricePerShare(uint256 _pid) public override view returns(uint256[] memory,uint256[] memory){
        _poolExist(_pid);
        Pool storage poolUSDT = pools[_pid*2];
        Pool storage poolRGP = pools[_pid*2 + 1];
        return(poolUSDT.dailyRewardPershare, poolRGP.dailyRewardPershare);
    }
    
    function getStakerInfoNTime(uint256 _pid, address stakerAddr) public override view returns(uint256[8] memory){
        _stakerExist(_pid,stakerAddr);
        Staker storage stakerUSDT = poolStaker[_pid*2][stakerAddr];
        Staker storage stakerRGP =  poolStaker[_pid*2 + 1][stakerAddr];
        Pool storage poolUSDT = pools[_pid*2];
        ...
        uint256 rgpToShare = poolVars[3];
        uint256 pricePerShare = poolVars[4];
        uint256 startTime  = poolTime[1];
        uint256 endTime = poolTime[2];
        uint256[8] memory data = [stakerUSDT.shares, stakerRGP.shares, pricePerShare,
                rgpToShare, startTime, endTime, poolUSDT.totalDailyRewardPershare, poolRGP.totalDailyRewardPershare];
        return(data);
    }
    ...
    function _getPoolInfo(uint256 _pid) private view returns(uint256 [7] memory){
        _poolExist(_pid);
        Pool storage pool = pools[_pid*2];
    ...
    function getTopUpDetail(uint256 _pid) public override view returns(uint256 [] memory,uint256 [] memory){
        _poolExist(_pid);
        return (topUpDetail[_pid].time,topUpDetail[_pid].amount);
