...
contract DecentrapadLaunch is Ownable, Pausable {
        ...
        whenNotPaused
    {
        ...
        if (launchInfo.paymentToken != address(0)) {
            ERC20(launchInfo.paymentToken).safeTransferFrom(
                msg.sender,
                address(this),
                paymentAmount
            );
    ...
    function userClaim() external {
        ...
        if (poolData.status == PoolStatus.ACTIVE) {
            if (poolData.totalRaised < launchInfo.softCap) {
                ...
                if (launchInfo.paymentToken != address(0)) {
                    ERC20(launchInfo.paymentToken).safeTransfer(
                        platformAddress,
                        earlyClaimPenalty
                    );
                    ERC20(launchInfo.paymentToken).safeTransfer(
        ...
        if (poolData.status == PoolStatus.COMPLETED) {
            ...
            ERC20(launchInfo.tokenAddress).safeTransfer(
                msg.sender,
                minAmountReceives
            );
    ...
    function finalizeLaunch() external onlyOwner {
        ...
        uint256 amountRaisedAferRefers = totalAmountRaised.sub(
            totalReferReward
        );

        uint256 platformShare = calculatePlatformShare(amountRaisedAferRefers);
        // cut platform fees
        if (launchInfo.paymentToken != address(0)) {
            ERC20(launchInfo.paymentToken).safeTransfer(
                platformAddress,
        ...
        (uint256 token0Amount, uint256 token1Amount) = calculateLPShare(
        ...
        require(
            raisedAfterPlatformFee > token1Amount,
    ...
    function claimReferReward() external {
        ...
        uint256 userReward = referComission[msg.sender];
    ...
    //working
    // calculate tokens and payment currency
    function calculateLPShare(uint256 _amountRaised)
        public
        view
        returns (uint256, uint256)
    {
        // uint256 tokens = calculateAmount(_amountRaised, dexInfo.listingRate);
        // calculate the amount of tokens sold
        uint256 tokens0Sold = calculateAmount(
            _amountRaised,
            launchInfo.presaleRate
        );
        // uint256 tokensSold;
        // calculate the ratio of LP on basis of tokens sold
        uint256 tokens0RequiredForLP = tokens0Sold.mul(1000).div(
            dexInfo.lpTokensRatio
        );
        //changed above to this
        //uint256 tokens0RequiredForLP = tokens0Sold.mul(dexInfo.lpTokensRatio).div(1000);

        uint256 token1RequiredForLP = calculatetoken0LP(
            tokens0RequiredForLP,
            dexInfo.listingRate
        );
        // check whether the ratio is less than max LP tokens allocated
        require(
            tokens0RequiredForLP < dexInfo.maxTokensForLiquidity,
            "LP tokens greater than max allocated"
        );
        // return the amount to add LP function, so LP can be added
        return (tokens0RequiredForLP, token1RequiredForLP);
    }

    function calculatetoken0LP(uint256 _amount, uint256 _rate)
        public
        view
        returns (uint256 amountOfToken1)
    {
        if (launchInfo.paymentToken == address(0)) {
            uint256 scale = 10**18;
            return amountOfToken1 = (_amount.mul(scale)).div(_rate);
        } else {
    ...
    //working
    function calculateAmount(uint256 _amount, uint256 _rate)
        public
        view
        returns (uint256)
    {
        uint256 decimals = ERC20(launchInfo.tokenAddress).decimals();
        uint256 scale = 10**decimals;
    ...
    function calculatePlatformShare(uint256 _amountRaised)
        public
        view
        returns (uint256)
    {
    ...
    function addNativeLiquidity(uint256 tokenAmount, uint256 ethAmount)
        private
    {
