...
contract GameSystem is AccessControl, ERC721Holder, ERC1155Holder, Ownable {
    ...
    mapping(address => uint256) public nonces; //  user address =>
    mapping(address => mapping(TokenStandard => mapping(uint256 => uint256))) public balances; //  user address => tokenStandard => tokenid => balance

    event ClaimDeplayUpdated(uint256 value);
    event EmergencyLockUpdated(bool status);
    event MinValidSignatureCountUpdated(uint256 count);
    event BlacklistUpdated(address indexed account, bool status);
    event TokenClaimed(address indexed account, uint8 tokenStandard, uint256 tokenId, uint256 amount);
    event TokenDeposited(address indexed account, uint8 tokenStandard, uint256 tokenId, uint256 amount);
    ...
    function claim(TokenClaimData calldata _claimData) public notBlacklisted {
        ...
        require(balances[msg.sender][_claimData.tokenStandard][_claimData.tokenId] >= _claimData.amount, "GameSystem: Not enough balance");
        ...
        emit TokenClaimed(msg.sender, uint8(_claimData.tokenStandard), _claimData.tokenId, _claimData.amount);
    }
    ...
    function deposit(TokenDepositData calldata _depositData) public notBlacklisted {
        ...
        emit TokenDeposited(msg.sender, uint8(_depositData.tokenStandard), _depositData.tokenId, _depositData.amount);
    }
    ...
    function updateClaimDelay(uint256 _delay) public onlyOwner {
        claimDelay = _delay;
        emit ClaimDeplayUpdated(_delay);
    }
}
