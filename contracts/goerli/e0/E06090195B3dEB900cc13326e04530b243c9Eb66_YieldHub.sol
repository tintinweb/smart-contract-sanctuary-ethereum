...
// File: contracts/YieldHub/SafeMath.sol
...
// File: contracts/YieldHub/IERC165.sol
...
// File: contracts/YieldHub/IERC1155.sol
...
// File: contracts/YieldHub/IERC721.sol
...
// File: contracts/YieldHub/Context.sol
...
// File: contracts/YieldHub/Ownable.sol
...
// File: contracts/YieldHub/IERC20.sol
...
// File: contracts/YieldHub/ERC20.sol
...
// File: contracts/YieldHub/YLD.sol
...
contract YieldHub is Ownable {
	...
	//REEMPLAZA EL VALOR QUE ESTÁ ENTRE PARENTESIS POR EL ADDRESS DEL NFT
	IPNUT public constant lecProxyContractAddress = IPNUT(0xfADcFa486A8Fa4431eF966774610B8F7c67dBa1E);
	...
	// admin //
	///////////
	function updatePeanut(address _nut) external onlyOwner {
		peanut = _nut;
	}
	...
	function getTokenReward(address _token) public {
		uint256 balOf = lecProxyContractAddress.balanceOf(msg.sender);
	...
	function getTotalClaimable(address _user, address _token) external view returns(uint256) {
		...
		bal = lecProxyContractAddress.balanceOf(_user);
	...
	function getReward(address _user) public {
		uint256 balOf = lecProxyContractAddress.balanceOf(msg.sender);
		updateUserToken(_user, peanut, balOf);
		_getReward(peanut, _user);
	}

	// called on transfers
	function updateReward(address _from, address _to) external {
		require(msg.sender == address(lecProxyContractAddress), "!peanut caller");
	...
	function updateUser(address _user, uint256 _tokensFarmed) internal {
		uint256 balOf = lecProxyContractAddress.balanceOf(_user);
	...
	// needs to burn new peanut
	function burn(address _from, uint256 _amount) external {
		require(msg.sender == address(lecProxyContractAddress), "!peanut contract");
		IMintable20(peanut).burnFrom(_from, _amount);
	}
}