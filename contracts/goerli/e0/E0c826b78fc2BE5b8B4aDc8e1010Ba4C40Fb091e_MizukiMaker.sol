 ...
 *
 *   Follow-up to the original Azuki Milady derivative project, enabling
 *   holders to mint a soulbound, customizable NFT that can be set to any
...
contract MizukiMaker is ERC721AQueryable, Ownable {
    ...
    function updateMetadata(uint256 tokenId, string memory cid) public {
        if(ownerOf(tokenId) != msg.sender) revert MustOwnThisMizkiMaker();

        if(frozen[tokenId]) revert MetadataFrozen();
