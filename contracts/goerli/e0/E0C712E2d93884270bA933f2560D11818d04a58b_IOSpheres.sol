...
contract tokenInitialData is OwnableUpgradeable, ERC1155Upgradeable {
    ...
    function createSphere(uint256 maxSupply, uint256 initSupply, uint256 maxMintingPerTx, bytes memory data) public onlyOwner returns (uint256 tokenId) {
...
contract IOSpheres is Initializable, ERC1155SupplyUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, OperatorFiltererUpgradeable, tokenInitialData {
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
        _disableInitializers();
    }
  
    address private IOGarageContract;
    ...
    function initialize(string memory _name, string memory _symbol) initializer public {
        ...
        __OperatorFilterer_init(_msgSender(),true);
    ...
    function mintIOSphere(address to, uint256 qty, uint256 _id) external virtual nonReentrant returns(uint256) {
        if (_id > tokenIdTracker) revert nonExistentTokenId();
        if (IOGarageContract == address(0)) revert IOGarageContractIsNotDefined();
        if (_msgSender() != IOGarageContract) revert notCalledUponRevealGarage();
        if ((AllTokens[_id].maxMintingPerTx > 0 && qty > AllTokens[_id].maxMintingPerTx) || qty <= 0 ) revert exceedMaxMinting();
        if (AllTokens[_id].totalSupply + qty > AllTokens[_id].maxSupply) revert mintExceededMaxSupply();

        unchecked { AllTokens[_id].totalSupply += qty; }
    ...
    function uri(uint256 _tokenId) public view virtual override returns (string memory) {
        if(!exists(_tokenId)) revert nonExistentTokenId();
    ...
    function getIOGarageContract() external view onlyOwner returns(address){
        return IOGarageContract;
    }
