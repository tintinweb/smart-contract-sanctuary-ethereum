...
/**
 * @title DelegateMinter
 * @author @NiftyMike | @NFTCulture
 * @dev Control functions for supporting a delegate minter that mints to other, typically custodial wallets.
 */
abstract contract DelegateMinter is Context {
    address internal _delegateMinter;

    modifier onlyDelegateMinter() {
        require(_delegateMinter == _msgSender(), 'DM: caller is not delegate');
        _;
    }

    constructor(address __defaultDelegateMinter) {
        _delegateMinter = __defaultDelegateMinter;
    }

    function _setDelegateMinter(address __newDelegateMinter) internal virtual {
        _delegateMinter = __newDelegateMinter;
    }

    function getDelegateMinter() external view returns (address) {
        return _delegateMinter;
    }
}
...
import './GenerativeArtProjectBase.sol';
import './GenerativeArtProjectSplitsAndRoyalties.sol';

// For goerli only
import '@nftculture/nftc-contract-library/contracts/interfaces/IOperatorFiltererRegistry.sol';

/**
 * @title GenerativeArtProject
 */
contract GenerativeArtProject is GenerativeArtProjectSplitsAndRoyalties, GenerativeArtProjectBase {
    constructor()
        GenerativeArtProjectBase(
            'GenerativeArtProject',
            'GAP',
            'https://api.dr3amlabs.xyz/api/v1/cosmic-bloom/metadata/', // Dr3am Labs API V1 (Immediate Reveal Service).
            addresses,
            splits,
            0.8 ether,
            0.8 ether,
...
// NFTC Prerelease Contracts
import '@nftculture/nftc-contract-library/contracts/token/DelegateMinter.sol';
    ...
    DelegateMinter
{
    ...
    function setDelegateMinter(address __newDelegateMinter) external onlyOwner {
        _setDelegateMinter(__newDelegateMinter);
    }
    ...
    function publicMintTokensTo(
        uint256 count,
        address to
    ) external payable nonReentrant isPublicMinting onlyDelegateMinter {
        if (0 >= count || count > PUBLIC_MINT_BATCH_SIZE) revert ExceedsBatchSize();
        if (msg.value != publicMintPricePerNft * count) revert InvalidPayment();
        if (_totalMinted() + count > PUBLIC_MINT_SUPPLY_CAP) revert ExceedsSupplyCap();
    ...
    function phaseTwoMintTokens(bytes32[] calldata proof, uint256 count) external payable nonReentrant isPhaseTwo {
        ...
        _proofMintTokens_PhaseTwo(msg.sender, proof, newBalance, count);
    }
...
contract GenerativeArtProjectSplitsAndRoyalties is ERC2981 {
    address[] internal addresses = [
        0x05Ed4cf991c4ed7606930AB54dDbF27836C1f590 // For Goerli Only
    ];

    uint256[] internal splits = [100];  // For Goerli Only
