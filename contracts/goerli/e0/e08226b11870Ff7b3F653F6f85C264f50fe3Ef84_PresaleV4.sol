...
contract PresaleV4 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function update(uint256 amount, uint256 lockup_months , uint256 _linearPriceUsdRaised) internal returns (uint256) {
    ...
    function updateForWert(uint256 amount, uint256 lockup_months , uint256 _linearPriceUsdRaised, address _user) internal returns (uint256) {
    uint256 quantityBonus = checkTokenQuantityBonus(amount);
    (uint256 lockupBonus, uint256 time) = checkLockupBonus(amount, lockup_months);
    totalTokensSold += amount;
    if (totalTokensSold > rounds[0][currentStep]) {
  ...
  function buyWithETHWert(address user, uint256 amount, uint256 lockup_months) external payable checkSaleState(amount) whenNotPaused nonReentrant returns (bool) {
    ...
    require(msg.value >= ethAmount, 'Less payment');
    uint256 excess = msg.value - ethAmount;
    uint256 newBonus = updateForWert(amount, lockup_months , usdEq, user);
