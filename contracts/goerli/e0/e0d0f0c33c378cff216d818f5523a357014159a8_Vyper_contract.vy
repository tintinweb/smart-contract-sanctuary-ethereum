# @version 0.3.1
"""
@title Everbloom Token Vault
@license GNU AGPLv3
@author Everbloom
@notice
    Everbloom Token Vault. Holds an underlying token, and allows users to interact
    with the Everbloom ecosystem through Strategies connected to the Vault.
...
interface Whitelist:
    def isWhitelisted(addr: address) -> bool: view
...
whitelistContractAddress: public(address)
...
def initialize(
    ...
    @dev
        If `nameOverride` is not specified, the name will be 'ebVault'
        combined with the name of `token`.

        If `symbolOverride` is not specified, the symbol will be 'eb'
    ...
    if nameOverride == "":
        self.name = concat(DetailedERC20(token).symbol(), " ebVault")
    else:
        self.name = nameOverride
    if symbolOverride == "":
        self.symbol = concat("eb", DetailedERC20(token).symbol())
    else:
    ...
    self.whitelistContractAddress = whitelist
    log WhitelistUpdated(whitelist)
    ...
    self.DOMAIN_SEPARATOR = keccak256(
        concat(
            DOMAIN_TYPE_HASH,
            keccak256(convert("EB Vault", Bytes[11])),
...
def deposit(_amount: uint256 = MAX_UINT256, recipient: address = msg.sender) -> uint256:
    ...
    assert Whitelist(self.whitelistContractAddress).isWhitelisted(msg.sender), 'This address is not whitelisted'
...
def withdraw(
    ...
    # Checks if withdrawer address is on whitelist
    assert Whitelist(self.whitelistContractAddress).isWhitelisted(msg.sender), 'This address is not whitelisted'
...
@external
def setWhitelist(whitelistContractAddress: address):
    """
    @notice
        Changes the whitelist contract address.
        This may only be called by governance.
    @param whitelistContractAddress The contract that is used for whitelisting assertion
    """
    assert msg.sender == self.governance
    self.whitelistContractAddress = whitelistContractAddress
    log WhitelistUpdated(whitelistContractAddress)
...
def clone(
    ...
    clonedAddr: address = create_forwarder_to(self)
    Vault(clonedAddr).initialize(
        token,
        governance,
        rewards,
        whitelist,
