...
contract ZoomanSpecialEdition is ERC721Enumerable, ERC721Royalty, AccessControl {
    using Strings for uint256;

    string baseURI = 'https://zoo-man.com/nftmetadata/timechangenft/';
    ...
    constructor() ERC721('ZOO-MAN SPECIAL EDITION', 'ZSE') {
        ...
        returns(string memory)
    {
        ...
        if(hour < nightStartTime) {
            return string(abi.encodePacked(baseURI, tokenId.toString(), "/morning.json"));
        } else if(hour >= nightEndTime) {
            return string(abi.encodePacked(baseURI, tokenId.toString(), "/morning.json"));
        } else {
            return string(abi.encodePacked(baseURI, tokenId.toString(), "/night.json"));
        }
    }
}