...
contract Timelock is ITimelock {
    ...
    function updateUsdgSupply(uint256 usdgAmount) external onlyKeeperAndAbove {
        address usdg = IDatManager(datManager).usdg();
        uint256 balance = IERC20(usdg).balanceOf(datManager);

        IUSDG(usdg).addVault(address(this));

        if (usdgAmount > balance) {
            uint256 mintAmount = usdgAmount.sub(balance);
            IUSDG(usdg).mint(datManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdgAmount);
            IUSDG(usdg).burn(datManager, burnAmount);
        }
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        IDatManager(datManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }

    function setDatCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        IDatManager(datManager).setCooldownDuration(_cooldownDuration);
    }
