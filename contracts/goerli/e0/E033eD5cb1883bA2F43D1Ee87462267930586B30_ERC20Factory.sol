...
contract ERC20Factory is Ownable {
    ...
    ERC20Contract[] private contracts;
    ...
    function updateLaunchPass(address _launchpassAddress) external onlyOwner {
        launchpassAddress = _launchpassAddress;
    }

    function getDeployments() external view returns (ERC20Contract[] memory) {
        return contracts;
    }
    ...
    function deploy(
        ...
        ERC20Contract _contract = new ERC20Contract(_name, _symbol, _payees, _shares, msg.sender);
        deployments[_launchpassId] = address(_contract);
...
contract ERC20Contract is ERC20, Ownable, PaymentSplitter {
    ...
    function burn(address _from, uint256 _amount) public onlyAllowlist {
        require(totalSupply() - _amount >= 0, "Insufficient supply");
        _burn(_from, _amount);
    }

    function mint(address _to, uint256 _amount) public onlyAllowlist {
        require(totalSupply() + _amount <= maxSupply, "Insufficient supply");
        _mint(_to, _amount);
    }
