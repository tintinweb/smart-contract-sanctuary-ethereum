...
contract LiquidityMining is Ownable {
    ...
    /// `rewardDebt` The amount of USDC entitled to the user.
    ...
    /// Also known as the amount of USDC to distribute per block.
    struct PoolInfo {
    ...
    event LogSetPool(uint256 indexed pid, uint256 rewardPerBlock);
    event LogUpdatePool(
        uint256 indexed pid,
        uint64 lastRewardBlock,
        uint256 lpSupply,
    ...
    function add(
        ...
        uint256 maxRewards = uint256(end).sub(block.number).mul(rewardPerBlock);
        require(
            USDC.balanceOf(address(this)) >= 1000000 ether,
        ...
        emit LogPoolAddition(lpToken.length.sub(1), rewardPerBlock, _lpToken);
    }

    /// @notice Update the given pool's USDC allocation point. Can only be called by the owner.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _rewardPerBlock New rewards of the pool.
    /// disabled for now
    // function set(uint256 _pid, uint256 _rewardPerBlock) public onlyOwner {
    //     poolInfo[_pid].rewardPerBlock = _rewardPerBlock;
    //     emit LogSetPool(_pid, _rewardPerBlock);
    // }

    /// @notice View function to see pending USDC on frontend.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _user Address of user.
    /// @return pending USDC reward for a given user.
        ...
        returns (uint256 pending)
    {
        ...
        if (block.number > pool.lastRewardBlock && pool.supply != 0) {
            uint256 lastValidBlock = block.number > pool.end
                ? pool.end
                : block.number;
            uint256 blocks = lastValidBlock.sub(pool.lastRewardBlock);
            uint256 sushiReward = blocks.mul(pool.rewardPerBlock);
            accRewardPerShare = accRewardPerShare.add(
                sushiReward.mul(ACC_INST_PRECISION) / pool.supply
            );
        }
        pending = uint256(
            int256(user.amount.mul(accRewardPerShare) / ACC_INST_PRECISION).sub(
                user.rewardDebt
            )
        );
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (
            ...
            if (pool.supply > 0) {
                ...
                // USDC.mint(address(this), sushiReward);
                uint256 rewardFactor = sushiReward.mul(ACC_INST_PRECISION) /
                    pool.supply;
    ...
    /// @notice Deposit LP tokens to LM for USDC allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    function deposit(
        ...
        user.rewardDebt = user.rewardDebt.add(
            int256(amount.mul(pool.accRewardPerShare) / ACC_INST_PRECISION)
        );
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param to Receiver of USDC rewards.
    function harvest(uint256 pid, address to) public onlyUnlocked(pid) {
        ...
        int256 accumulatedReward = int256(
            user.amount.mul(pool.accRewardPerShare) / ACC_INST_PRECISION
        );
        uint256 _pendingRewards = uint256(
            accumulatedReward.sub(user.rewardDebt)
        );
        ...
        if (_pendingRewards != 0) {
            ...
            USDC.transfer(to, _pendingRewards);
        }
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount LP token amount to withdraw.
    /// @param to Receiver of the LP tokens and USDC rewards.
    function withdrawAndHarvest(
        ...
        USDC.transfer(to, _pendingReward);
