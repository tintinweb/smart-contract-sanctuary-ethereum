...
contract TangleswapPool is ITangleswapPool, NoDelegateCall {
    ...
    function swap(
        ...
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            ...
            if (INonfungiblePositionManager(ITangleswapFactory(factory).nonfungiblePositionManager()).balanceOf(recipient) > 0) {
            ...
            if (state.liquidity > 0) {
                ...
                state.feeGrowthGlobalX128 += FullMath.mulDiv(step.feeAmount - step.chargedBurnFeeAmount, FixedPoint128.Q128, state.liquidity);
            }
    ...
    function checkPriceDiff(bool zeroForOne, uint160 sqrtPriceX96) internal view returns (bool) {
        ...
        if (iPriceOracle.isOracleEnabled() && iPriceOracle.isOracleOperative()) {
            ...
            bool isPriceDiffInRange = priceDiff.mul(10000) > lpPrice.mul(minimumThreshold) &&
            ...
            if (isPriceDiffInRange) {
                if (zeroForOne) { // case 1: token0 -> token1 (price of token0  will decrease, price of token1 will increase)
                    if (isRatioToken1OverToken0) { // token0 price
                        if (uint256(oraclePrice) < lpPrice) {
                ...
                } else { // case 2: token1 -> token0 (price of token0 will increase, price of token1 will decrease)
                    if (isRatioToken1OverToken0) { // token0 price
                        if (uint256(oraclePrice) >= lpPrice) {
                            isSwapDecreasingPriceDiff = true;
                        }
