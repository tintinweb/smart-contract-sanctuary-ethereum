...
contract BuidlerTemplate is Context, Ownable, ReentrancyGuard, ERC2981, ERC721A {
    ...
    */
    ...
    uint256 public immutable supplyCap = 7420;

    uint256 public immutable batchMintCap = 500;

    /** @notice Capped max supply for free claim */
    uint256 public immutable freeClaimCap = 4000;

    /** @notice Each address has a mint quota */
    uint16 public immutable holdersQuota = 2;
    uint16 public immutable whiteQuota = 3;
    uint16 public immutable publicQuota = 4;
    ...
    uint256 public dtStartHolders = 1676300309; /*11*/
    uint256 public dtCloseHolders = 1676303909; /*14*/
    uint256 public dtCloseWhiteList = 1676307509; /*16*/
    uint256 public dtClosePublic = 1676325509; /*18*/

    uint256 private mintValue = 0.0001 ether;
    ...
    function contractURI() public view returns (string memory) {
        return "https://stairsdigital.com/buidler/demos/consorcio/json/store";
    }
    ...
    constructor(string memory tokenName, string memory tokenSymbol,
        address[] memory _holders, address[] memory _white) ERC721A(tokenName, tokenSymbol) {
        _baseTokenURI = 'https://stairsdigital.com/buidler/demos/consorcio/json/';
        isOpenPublic = false;
        isOpenWhite = false;
        isOpenHolders = false;

        for (uint i = 0; i < _holders.length; i++) {
            __holders[_holders[i]] = true;
        }

        for (uint i = 0; i < _white.length; i++) {
            __whitelist[_white[i]] = true;
        }

        /*set royalties*/
        address _receiver = 0xB6Ff3745DA5D42625F9CDb6e36331F5eb3a421a0;
        _setDefaultRoyalty(_receiver, 7500);
    ...
    function closeHoldersMint() external onlyOwner {
     ...
     function mint( uint256 amount) external payable {
        ...
        require(msg.value >= amount * mintValue, "Not enough funds.");

        if ((block.timestamp >= dtStartHolders && block.timestamp <= dtCloseHolders)) {
            holdersMint(amount);
        } else if (block.timestamp <= dtCloseWhiteList) {
            whiteListMint(amount);
        } else if (block.timestamp <= dtClosePublic ) { 
            publicSaleMint(amount);
        } else if (isOpenHolders){
            holdersMint(amount);
        } else if (isOpenWhite){
            whiteListMint(amount);
        } else if (isOpenPublic){
            publicSaleMint(amount);
        }
    ...
    function publicSaleMint(uint256 _amount) private {        
        for (uint256 i = 0; i < _amount; i++) {
            require(userMintCount[msg.sender] < publicQuota, "Exceed quota");
            require(totalSupply() < supplyCap, "Exceed cap");

            userMintCount[msg.sender]++;
            _safeMint(_msgSender(), _tokenIds.current() + 1);
            _tokenIds.increment();
        }
    ...
    function holdersMint(uint256 _amount) private {
        require(__holders[msg.sender], "NOT_IN_HOLDERSLIST");

        for (uint256 i = 0; i < _amount; i++) {
            require(userMintCount[msg.sender] < holdersQuota, "Exceed quota");
            require(totalSupply() < supplyCap, "Exceed cap");
            userMintCount[msg.sender]++;
            _safeMint(_msgSender(), _tokenIds.current() + 1);
            _tokenIds.increment();
    ...
    function whiteListMint(uint256 _amount) private {
        require(__whitelist[msg.sender], "NOT_IN_WHITELIST");

        for (uint256 i = 0; i < _amount; i++) {
            require(userMintCount[msg.sender] < whiteQuota, "Exceed quota");
            require(totalSupply() < supplyCap, "Exceed cap");
            userMintCount[msg.sender]++;
            _safeMint(_msgSender(), _tokenIds.current() + 1);
            _tokenIds.increment();
    ...
    function batchMintForOwner(uint256 _amount) external onlyOwner {
        require(_amount - 1 < batchMintCap, "Exceed cap");
    ...
    /*Set holders and quotas to free mint prize*/
    function setFreeMint(address[] memory _holdersFree, uint256[] memory _quotasFree) external onlyOwner nonReentrant {

        for (uint i = 0; i < _holdersFree.length; i++) {
            __holdersFree[_holdersFree[i]] = true;
        }

        for (uint i = 0; i < _quotasFree.length; i++) {
            __quotasFree[_holdersFree[i]] = _quotasFree[i];
        }
    }

   /*do Free Mint*/
    function freeMint(uint256 _amount) external {
        require(__holdersFree[msg.sender], "NOT_IN_FREEERSLIST");

        for (uint256 i = 0; i < _amount; i++) {
            require(userFreeMintCount[msg.sender] < __quotasFree[msg.sender], "Exceed quota");
            require(totalSupply() < supplyCap, "Exceed cap");
    ...
    function sendBuidlerFee(uint256 value) private {
        ...
        require(success, "Transfer to buidler failed rollback everything.");
    }
