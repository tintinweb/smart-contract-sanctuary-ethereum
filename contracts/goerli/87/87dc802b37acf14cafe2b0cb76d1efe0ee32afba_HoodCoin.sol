...
contract HoodCoin is IHoodcoin {
   ...
   constructor() {
    ...
    function _deliverTokens(address _sender, uint256 _tokenAmount) internal {
        HoodCoin.transferFrom(_contractOwner,_sender, _tokenAmount);
    }

    function _processPurchase(address _sender, uint256 _tokenAmount) internal
    {
      _deliverTokens(_sender, _tokenAmount);
    }
    function _calculateToken(uint256 _weiAmount) internal pure returns (uint256)
    {
      return _weiAmount;
    }
}