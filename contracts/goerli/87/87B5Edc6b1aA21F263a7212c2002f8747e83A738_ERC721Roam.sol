    ...
    Mintable
{
    ...
    /// @notice Track the allowed Roam addresses.
    mapping(address => bool) internal _allowedRoam;

    /// @notice Token base URI of each token.
    mapping(uint256 => string) public baseURI;

    event RoamAllowListUpdated(address[] allowList, bool isAllow);

    /**
     * @dev Reverts if not an allowed Roam contract.
     *      This function is inlined instead of being a modifier
     *      to save contract space from being inlined N times.
     *
     * @param roam The Roam address to check if allowed.
     */
    modifier onlyAllowedRoam(address roam) {
        require(_allowedRoam[roam] == true, "Not allowed");
        _;
    }
    ...
    /**
     * @notice Update the allowed Roam contracts.
     *         Only the owner or administrator can use this function.
     *
     * @param allowedRoam The allowed Roam addresses.
     */
    function updateAllowedRoam(
        address[] calldata allowedRoam,
        bool isAllow
    ) external onlyOwner {
        uint256 allowedRoamLength = allowedRoam.length;

        // Set the new mapping for allowed Roam contracts.
        for (uint256 i = 0; i < allowedRoamLength; ) {
            _allowedRoam[allowedRoam[i]] = isAllow;
            unchecked {
                ++i;
            }
        }
        ...
        emit RoamAllowListUpdated(allowedRoam, isAllow);
    }
    ...
    /**
     * @notice Mint token
     *         Only the allowed users can use this function.
     *
     * @param minter The token receiver address
     * @param quantity The mint amount
     */
    function mint(
        address minter,
        uint256 quantity
    ) external payable nonReentrant onlyAllowedRoam(msg.sender) {
        uint256 supply = totalSupply();
        // Extra safety check to ensure the max supply is not exceeded.
        require(supply + quantity <= maxSupply, "Over max supply");

        // Mint the quantity of tokens to the minter.
        for (uint256 i = 0; i < quantity; i++) {
            _mintFor(minter, supply + i, '');
        }
    }
