...
contract $RegenBingo is RegenBingo {
    ...
    event return$requestRandomness(uint256 requestId);

    constructor(string memory _name, string memory _symbol, uint256 _mintPrice, uint256 _drawTimestamp, uint256 _drawNumberCooldownSeconds, string memory _donationName, address payable _donationAddress, address _metadataGenerator, address _linkAddress, address _wrapperAddress) RegenBingo(_name, _symbol, _mintPrice, _drawTimestamp, _drawNumberCooldownSeconds, _donationName, _donationAddress, _metadataGenerator, _linkAddress, _wrapperAddress) {}
    ...
    function $VRF_CALLBACK_GAS_LIMIT() external pure returns (uint32) {
        return VRF_CALLBACK_GAS_LIMIT;
    }

    function $VRF_REQUEST_CONFIRMATIONS() external pure returns (uint16) {
        return VRF_REQUEST_CONFIRMATIONS;
    }

    function $VRF_NUMBER_OF_WORDS() external pure returns (uint32) {
        return VRF_NUMBER_OF_WORDS;
    }

    function $VRF_REREQUEST_COOLDOWN_BLOCKS() external pure returns (uint256) {
        return VRF_REREQUEST_COOLDOWN_BLOCKS;
    }
    ...
    function $lastRequestBlockNumber() external view returns (uint256) {
        return lastRequestBlockNumber;
    }
    ...
    function $VRF_V2_WRAPPER() external view returns (VRFV2WrapperInterface) {
        return VRF_V2_WRAPPER;
    }

    function $_tokenSeed(uint256 tokenId) external view returns (uint256 ret0) {
        (ret0) = super._tokenSeed(tokenId);
    }
    ...
    function $_requestDrawSeed() external {
        super._requestDrawSeed();
    }

    function $requestRandomness(uint32 _callbackGasLimit,uint16 _requestConfirmations,uint32 _numWords) external returns (uint256 requestId) {
        (requestId) = super.requestRandomness(_callbackGasLimit,_requestConfirmations,_numWords);
        emit return$requestRandomness(requestId);
    }
