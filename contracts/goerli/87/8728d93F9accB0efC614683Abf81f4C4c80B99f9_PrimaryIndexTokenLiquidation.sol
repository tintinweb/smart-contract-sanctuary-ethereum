...
contract PrimaryIndexTokenLiquidation is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function liquidate(address _account, address _projectToken, address _lendingToken, uint256 _lendingTokenAmount) public isProjectTokenListed(_projectToken) isLendingTokenListed(_lendingToken) {
        ...
        uint lendingTokenAmountMul = getTokenPrice(_lendingToken, _lendingTokenAmount) * 10 ** LIQUIDATOR_REWARD_FACTOR_DECIMAL;
    ...
    function getMaxLiquidatorRewardAmount(address _account, address _projectToken, address _lendingToken) public view returns (uint256 maxLA) {
        ...
        maxLAParams.maxLACompare = maxLAParams.calculatedMaxLA > totalOutstandingInUSDMul ? totalOutstandingInUSDMul : maxLAParams.calculatedMaxLA / 10**LIQUIDATOR_REWARD_FACTOR_DECIMAL;
        maxLA = (maxLAParams.maxLACompare * 10 **LIQUIDATOR_REWARD_FACTOR_DECIMAL) / getTokenPrice(_lendingToken, 10 ** ERC20Upgradeable(_lendingToken).decimals());
    }
    ...
    function getMinLiquidatorRewardAmount(address _account, address _projectToken, address _lendingToken) public view returns (uint256) {
        ...
        return Math.min(maxLrf, minPartialLiquidationAmount * 10 ** LIQUIDATOR_REWARD_FACTOR_DECIMAL) / getTokenPrice(_lendingToken, 10 ** ERC20Upgradeable(_lendingToken).decimals());
    }
}