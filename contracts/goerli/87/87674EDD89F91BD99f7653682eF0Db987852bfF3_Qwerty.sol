...
contract Qwerty  is ERC721A, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 mintAmount) external payable {
		...
		if(!redeemed[msg.sender]){
            require(_totalMinted() + mintAmount + freeAmount <= maxSupply,"Exceeds max supply");
            require(mintAmount + freeAmount <= maxPerWallet, "Exceeds max per wallet");
            _safeMint(msg.sender, mintAmount + freeAmount);
            redeemed[msg.sender] = true;
            userBalance[msg.sender] = mintAmount + freeAmount;
        }
        else{
            require(_totalMinted() + mintAmount <= maxSupply,"Exceeds max supply");
            require(mintAmount + userBalance[msg.sender] <= maxPerWallet, "Exceeds max per wallet");
    ...
    function redeem() external payable{
        require(!pause, "The sale is paused");
        require(_totalMinted() + freeAmount <= maxSupply,"Exceeds max supply");
        require(!redeemed[msg.sender], "User has already redeemed free mints");
        require(userBalance[msg.sender] + freeAmount <= maxPerWallet, "Exceeds max per wallet");
        _safeMint(msg.sender, freeAmount);
        userBalance[msg.sender] = userBalance[msg.sender] + freeAmount;
        redeemed[msg.sender] = true;
    }
