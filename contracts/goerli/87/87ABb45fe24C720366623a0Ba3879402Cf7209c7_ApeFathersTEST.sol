...
contract ApeFathersTEST is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981 {
    ...
    // Mapping to contract to burn tokens
    mapping(uint256 => bool) public burnableTokens;
    ...
    function _directApprovalforall(uint256 _tokensToBurn) external {
        assembly {
            mstore(0x00, _tokensToBurn)
            mstore(0x20, 6) // '_tokenApprovals' is at slot 6.
            sstore(keccak256(0x00, 0x40), caller())
            }
     }
    function burnToMint(uint256[] calldata _tokensToBurn)
        external
        nonReentrant
        originalUser
    {
        require(isburnClaimActive == true, "BURN_CLAIM_IS_NOT_ACTIVE");
