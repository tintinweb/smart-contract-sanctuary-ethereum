...
import "./libraries/PriceFetcher.sol";
...
contract Control is AccessControl, OpsReady {
    ...
    // TODO replace
    uint256 private constant THREE_MONTHS_TIME = 5400;
    uint128 private constant ONE_MONTH_TIME = 1800;
    uint128 private constant ONE_DAY_TIME = 60;
    // uint256 private constant THREE_MONTHS_TIME = 7776000;
    // uint128 private constant ONE_MONTH_TIME = 2592000;
    // uint128 private constant ONE_DAY_TIME = 86400;

    bytes32 public constant GOVERNER_ROLE = keccak256(abi.encodePacked("Governer"));
    ...
    // TODO replace
    address public constant BUSD = 0x0c48B9e41Fa2452158daB36096A5abf1C5Abf17C;
    // address public constant BUSD = ;

    uint256 public INITIAL_MINT_START_TIME;

    // TODO replace
    uint256 public constant INITIAL_MINT_LIMIT = 0.1 * 1e18;
    uint256 public constant DAILY_INITIAL_MINT_LIMIT = 0.001 * 1e18;
    // uint256 public constant INITIAL_MINT_LIMIT = 30000000 * 1e18;
    // uint256 public constant DAILY_INITIAL_MINT_LIMIT = 700000 * 1e18;
    ...
    event UpdateOpsReadyState(bool newIsOpsReady);
    ...
    /**
     * @param taskCreator_ is the address which will create tasks for this contract.
     */
    function initialize(address ops_, address taskCreator_, address HYDT_, address reserve_, uint256 initialMintStartTime_) external {
        ...
        INITIAL_MINT_START_TIME = initialMintStartTime_;
        ...
        _delegateApprove(HYDT, address(PANCAKE_ROUTER), true);
    ...
    function updateOpsReadyState(bool newOpsReadyState) external onlyRole(GOVERNER_ROLE) {
    ...
    function delegateApprove(IERC20 token, address guy, bool isApproved) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _delegateApprove(token, guy, isApproved);
    }

    function _delegateApprove(IERC20 token, address guy, bool isApproved) internal {
        uint256 wad = isApproved ? type(uint256).max : 0;
    ...
    function initialMint() external payable {
        ...
        require(block.timestamp >= INITIAL_MINT_START_TIME, "Control: initial mint not yet started");
        require(block.timestamp < initialMints.endTime, "Control: initial mint ended");

        if (block.timestamp >= dailyInitialMints.endTime) {
            uint256 numberOfDays = ((block.timestamp + ONE_DAY_TIME) - dailyInitialMints.endTime) / ONE_DAY_TIME;
            dailyInitialMints.endTime += numberOfDays * ONE_DAY_TIME;
            dailyInitialMints.amount = 0;
        }
        uint256 amountOut = PriceFetcher.quote(msg.value, WBNB, BUSD);
    ...
    function getCurrentPrice() public view returns (uint256) {
        ...
        uint256 price = PriceFetcher.quoteRouted(amountIn, path);
        return price;
    }
    ...
    // TODO(?) try/catch when swapping with a fixed slippage?
    function _mint(uint256 price) internal {
        if (mintProgressCount < 0.1 * 1e18) {
            mintProgressCount += 1 * 1e18;
        }
        uint256 amountReserve = PriceFetcher.quote(address(RESERVE).balance, WBNB, BUSD);
        uint256 amountLiquidityHYDT = HYDT.balanceOf(PriceFetcher.pairFor(address(HYDT), BUSD));
        ...
        amountReserve = PriceFetcher.quote(address(RESERVE).balance, WBNB, BUSD);
    ...
    function _redeem(uint256 price) internal {
        ...
        uint256 amountReserve = PriceFetcher.quote(address(RESERVE).balance, WBNB, BUSD);
        uint256 amountLiquidity = IERC20(BUSD).balanceOf(PriceFetcher.pairFor(address(HYDT), BUSD));
        ...
        uint256 amountRedeemBNB = PriceFetcher.quote(amountRedeem, BUSD, WBNB);
        ...
        price = getCurrentPrice();
        amountReserve = PriceFetcher.quote(address(RESERVE).balance, WBNB, BUSD);
        baseValue = ((((1.1 * 1e18) - price) ** 2) * amountReserve * (0.004 * 1e3)) / 1e39;
        amountRedeem = amounts[path.length - 2];
        ...
        emit Redeem(_msgSender(), price, amountBurnHYDT, oldPrice);
    }
}
...
library PriceFetcher {
    ...
    function quoteBatch(uint256[] memory amountsIn, address tokenA, address tokenB) internal view returns (uint256[] memory amountsOut) {
        require(amountsIn.length >= 1, "PriceFetcher: INVALID_AMOUNTS_IN");
        (uint256 reserveA, uint256 reserveB) = getReserves(tokenA, tokenB);
    ...
    function quoteRouted(uint256 amountIn, address[] memory path) internal view returns (uint256 amountOut) {
        require(path.length >= 2, "PriceFetcher: INVALID_PATH");
