...
interface ICeresBank {
    ...
    function redeem(address collateral, address[] calldata stakers, uint256 ascAmount) external;
...
interface ICeresReward {
    ...
    function minStakeAmount(address staking) external view returns (uint256);
...
interface IMintsReceiver {

    function notifyMints(address[] memory stakers, uint256[] memory amounts, uint256 total) external;
}
...
interface IRedeemsReceiver {

    function notifyRedeems(address[] calldata stakers, uint256 ascTotal, uint256 veCRSTotal, uint256 colValueTotal) external;
...
interface IStakerBook {
    ...
    function stakers(address staking, uint256 index) external returns (address);
    function getStakers(address staking) external view returns (address[] memory);
    function getStakersLength(address staking) external view returns (uint256);
...
library CeresLibrary {
    ...
    function randomIndex(uint256 length) internal view returns (uint256){
        return uint256(keccak256(abi.encodePacked(msg.sender, block.difficulty, block.timestamp, length))) % length;
    }
...
import "../interfaces/IRedeemsReceiver.sol";
import "../interfaces/ICeresBank.sol";
import "../interfaces/IMintsReceiver.sol";
import "../library/CeresLibrary.sol";

contract CeresStakingASC is CeresStakingRewards, IRedeemsReceiver, IMintsReceiver, UUPSUpgradeable {

    mapping(address => uint256) public redeemVeCRS;
    mapping(address => uint256) public redeemColValue;

    /* ---------- Events ---------- */
    event RedeemPaid(address indexed account, uint256 veCrsAmount, uint256 colValue);
    ...
    function stake(uint256 amount) public override nonReentrant updateReward(msg.sender) {
        require(amount >= ICeresReward(factory.ceresReward()).minStakeAmount(address(this)),
            "CeresStaking: must stake more than minimum stake amount!");
        asc.transferFrom(msg.sender, address(this), amount);
        _stake(msg.sender, amount);
    }

    function withdraw(uint256 amount) public override nonReentrant updateReward(msg.sender) {
        require(accountLockTime[msg.sender] <= block.timestamp, "CeresStaking: staking lock time is not over yet.");
        require(amount > 0 && amount <= stakingBalanceOf[msg.sender], "CeresStaking: your staking balance is not enough!");

        totalStaking -= amount;
        stakingBalanceOf[msg.sender] -= amount;
        asc.transfer(msg.sender, amount);
    ...
    function _claimRedeem() internal {
        ...
        if (_redeemColValue > 0) {
            ...
            uint256 _redeemAmount = CeresLibrary.toAmountActual(_redeemToken, _redeemColValue);
            SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(_redeemToken), msg.sender, _redeemAmount);
        }
        emit RedeemPaid(msg.sender, _redeemVeCRS, _redeemColValue);
    }

    /* ---------- Notifies ---------- */

    function notifyMints(address[] memory stakers, uint256[] memory amounts, uint256 total) external override onlyCeresMiner {
        rewardPerTokenStored = rewardPerToken();
        lastUpdateTime = lastTimeRewardApplicable();

        uint256 _lockTime = block.timestamp + ICeresReward(factory.ceresReward()).stakingLockTime();
        for (uint256 i = 0; i < stakers.length; i++) {
            ...
            // update rewards
            rewards[account] = stakingBalanceOf[account] * (rewardPerTokenStored - accountRewardPerTokenPaid[account]) / 1e18 + rewards[account];
            accountRewardPerTokenPaid[account] = rewardPerTokenStored;
            // update new stakings
            stakingBalanceOf[account] += amounts[i];
            accountLockTime[account] = _lockTime;
            stakerBook.stake(account);
        }

        totalStaking += total;
        require(asc.balanceOf(address(this)) >= totalStaking, "CeresStaking: ASC balance is not enough!");
    }

    function notifyRedeems(address[] calldata stakers, uint256 ascTotal, uint256 veCrsTotal, uint256 colValueTotal) external override onlyCeresBank {
        rewardPerTokenStored = rewardPerToken();
        lastUpdateTime = lastTimeRewardApplicable();

        uint256 ascEach = ascTotal / stakers.length;
        uint256 veCrsEach = veCrsTotal / stakers.length;
        uint256 colValueEach = colValueTotal / stakers.length;
        for (uint256 i = 0; i < stakers.length; i++) {
            address account = stakers[i];
            require(stakingBalanceOf[account] >= ascEach, "CeresStaking: staker balance is not enough to redeem!");
            // update rewards
            rewards[account] = stakingBalanceOf[account] * (rewardPerTokenStored - accountRewardPerTokenPaid[account]) / 1e18 + rewards[account];
            accountRewardPerTokenPaid[account] = rewardPerTokenStored;
            // update balances
            stakingBalanceOf[account] -= ascEach;
            // update redeems
            redeemVeCRS[account] += veCrsEach;
            redeemColValue[account] += colValueEach;
        }

        totalStaking -= ascTotal;
        require(asc.balanceOf(address(this)) >= totalStaking, "CeresStaking: ASC balance is not enough!");
    }
...
abstract contract CeresStakingRewards is CeresBaseUpgradeable, ICeresStaking, ReentrancyGuardUpgradeable {
    ...
    mapping(address => uint256) public override stakingBalanceOf;
    ...
    mapping(address => uint256) public accountRewardPerTokenPaid;
    ...
    function accountStakingRatio(address account) public view override returns (uint256){
        return totalStaking > 0 ? stakingBalanceOf[account] * CERES_PRECISION / totalStaking : 0;
    }
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return rewardPerTokenStored + ((lastTimeRewardApplicable() - lastUpdateTime) * rewardRate * 1e18 / totalStaking);
    }

    function earned(address account) public view override returns (uint256) {
        return stakingBalanceOf[account] * (rewardPerToken() - accountRewardPerTokenPaid[account]) / 1e18 + rewards[account];
    }
    ...
    function yieldAPR() public view override returns (uint256){
        ...
        if (totalStaking == 0 || rewardRate == 0)
            return 999999999;
    ...
    function _stake(address account, uint256 amount) internal {
        ...
        totalStaking += amount;
        stakingBalanceOf[account] += amount;
        stakerBook.stake(account);
    ...
    function withdrawAll() external override {
        withdraw(stakingBalanceOf[msg.sender]);
    }
