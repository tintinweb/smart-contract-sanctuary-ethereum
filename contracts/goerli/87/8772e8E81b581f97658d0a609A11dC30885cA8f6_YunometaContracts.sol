...
contract YunometaContracts is Ownable {
  ...
  function getContractbyCreator(address _add, uint256 _index) public view returns ( structContractInfo memory ) {
    unchecked {
      require(_index > 0, 'Incorrect index value');
      require(_index < ContractCreatorMap[_add].length, 'Incorrect index value');
      
      return ContractsMap[ContractCreatorMap[_add][_index - 1]];
    }
  }
