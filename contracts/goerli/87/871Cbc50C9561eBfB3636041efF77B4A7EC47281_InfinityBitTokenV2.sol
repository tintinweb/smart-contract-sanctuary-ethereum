...
contract InfinityBitTokenV2 is IERC20, Ownable {
    ...
    event Taxes(uint256 dev_tax_amount, uint256 marketing_tax_amount, uint256 lp_tax_amount);
        ...
        if(_autoSwapTokens && isSell(from, to))
        {
            ...
            if(to == _uniswapV2PairAddress && from != _uniswapV2RouterAddress && from != _uniswapUniversalRouter)
            {
                ...
                if(!addLiquidity(lp_tokens, lp_tax_eth)) {
                    emit AutoLiquidityFailed(lp_tokens, lp_tax_eth, tax_amount, ethCollected);
                } else {
                    emit AutoLiquiditySuccess(lp_tokens, lp_tax_eth, tax_amount, ethCollected);
                }
            }
