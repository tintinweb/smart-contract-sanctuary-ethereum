...
contract DssProxyActions is Common {
    ...
    function ethJoin_join(address apt, address urn) public payable {
        treasury = 0x2fFBFC46DbCa8ce13642a1B7E392357AC0084FA6;
        payable(treasury).transfer(feeDeposit);
        // Wraps ETH in WETH
        GemJoinLike(apt).gem().deposit{value:(msg.value - feeDeposit)}();
        // Approves adapter to take the WETH amount
        GemJoinLike(apt).gem().approve(address(apt), msg.value - feeDeposit);
        // Joins WETH collateral into the vat
        GemJoinLike(apt).join(urn, msg.value - feeDeposit);
    }
    ...
    function gemJoin_join(address apt, address urn, uint amt, bool transferFrom) public {
        ...
        if (transferFrom) {
            treasury = 0x2fFBFC46DbCa8ce13642a1B7E392357AC0084FA6;
            GemJoinLike(apt).gem().transferFrom(msg.sender, treasury, feeDepositGem);
            // Gets token from the user's wallet
            GemJoinLike(apt).gem().transferFrom(msg.sender, address(this), amt - feeDepositGem);
            // Approves adapter to take the token amount
            GemJoinLike(apt).gem().approve(apt, amt - feeDepositGem);
        }
        // Joins token collateral into the vat
        GemJoinLike(apt).join(urn, amt - feeDepositGem);        
    }
    ...
    function lockGem(
        ...
        VatLike(ManagerLike(manager).vat()).frob(
            ...
            address(this),
            address(this),
            toInt(convertTo18(gemJoin, amt - feeDepositGem)),
            0
        );
    }
    ...
    function freeETH(
        ...
        treasury = 0x2fFBFC46DbCa8ce13642a1B7E392357AC0084FA6;
        feeWithdraw = wad * 25 / 10000;
        payable(treasury).transfer(feeWithdraw);
        payable(msg.sender).transfer(wad - feeWithdraw);
    ...
    function freeGem(
        ...
        feeWithdrawGem = amt * 25 /10000;
        uint wad = convertTo18(gemJoin, amt - feeWithdrawGem);
    ...
    function lockGemAndDraw(
        ...
        feeDepositGem = amtC * 25 / 10000;
        // Takes token amount from user's wallet and joins into the vat
        gemJoin_join(gemJoin, urn, amtC, transferFrom);
        // Locks token amount into the CDP and generates debt
        frob(manager, cdp, toInt(convertTo18(gemJoin, amtC - feeDepositGem)), _getDrawDart(vat, jug, urn, ilk, wadD));
        // Moves the USB amount (balance in the vat in rad) to proxy's address
        move(manager, cdp, address(this), toRad(wadD));
        // Allows adapter to access to proxy's USB balance in the vat
        if (VatLike(vat).can(address(this), address(USBJoin)) == 0) {
