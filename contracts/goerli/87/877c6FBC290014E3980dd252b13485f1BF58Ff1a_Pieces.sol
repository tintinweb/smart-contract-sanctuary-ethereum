...
contract Pieces is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable {
    ...
    event ArtworkAdded(uint256 indexed id, address indexed creator, string indexed name, uint80 price, uint16 maxSupply);
    ...
    function createToken(uint16 maxSupply, uint80 price, uint256 mintAmount, bytes memory data, uint16 destLen, string memory name)
        public
    {
        ...
        if(mintAmount > 0) _mint(msg.sender, _tokenId, mintAmount, bytes("0"));
