...
contract MarketplaceDutch is Ownable, ReentrancyGuard
{
    ...
    struct _UserSaleInformation
    {
        ...
        bool _SalePaused;              // [9] -> _SalePaused              | If The Sale Is Paused
    }
    ...
    function ViewWalletSaleInformation(
        ...
        if(DN != 69420) { Recipient = IDelegationRegistry(_DR).getDelegationsByDelegate(Wallet)[DN].vault; }
        (Verified, Priority) = ViewBrightListStatus(SaleIndex, Recipient, ProofEligibility);
        if(
        ...
        bool VerifiedAmount = VerifyAmount(Recipient, MaxAmount, SaleParams[SaleIndex]._RootsAmounts[Priority], ProofAmount); // If The User Has Passed A Valid MaxAmount
        uint CurrentPrice = ViewCurrentPrice(SaleIndex);
        return(
            _UserSaleInformation(
                UserInfo[SaleIndex][Recipient]._UserOrderIndexes,            // The User's Order Indexes
                UserInfo[SaleIndex][Recipient]._PurchasedAmount,             // The User's Total Purchase Amount For `SaleIndex`
                MaxAmount - UserInfo[SaleIndex][Recipient]._PurchasedAmount, // The User's Remaining Purchase Allocation
                UserInfo[SaleIndex][Recipient]._ClaimIndex,                  // The User's Claim Index
                ...
                SalePaused                                                                                      // If The Sale Is Paused
            )
        );
    }
