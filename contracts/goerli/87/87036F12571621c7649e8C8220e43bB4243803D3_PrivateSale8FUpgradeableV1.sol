...
contract PrivateSale8FUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    modifier onlyTokenOwner(uint256 tokenId) {
        address ownerOfToken = PrivateSaleNft8FContractV1(nftContractAddress).ownerOf(tokenId);
    ...
    function openToken(uint256 tokenId) public onlyTokenOwner(tokenId) {
        balances[tokenId].payment.opened = true;
    }
    ...
    function buyToken(uint amount) public {
        ...
        uint nftTokenId = PrivateSaleNft8FContractV1(nftContractAddress).privateSaleMint(address(tx.origin), "");
    ...
    function isOpened(uint256 tokenId) public view returns (bool) {
        return this.getPayment(tokenId).opened;
    }
    ...
    function calculateWithdrawAvailable(uint256 tokenId) public view vestingStarted onlyTokenOwner(tokenId) returns (uint, uint)  {
        uint256 amount;
        if (block.timestamp - vestingStartTime >= vestingTime) {
            amount = balances[tokenId].payment.tokensAmount - balances[tokenId].payment.withdrawed;
        } else {
            amount = (
                (
                    multiplier * (block.timestamp - vestingStartTime)
                    /
                    vestingTime
                )
    ...
    function claimMyMoney (uint256 tokenId) public vestingStarted onlyTokenOwner(tokenId) {
    ...
    function contractUsdtBalance() public onlyOwner returns (uint) {
        return ERC20Interface(usdContractAddress).balanceOf(address(this));
    }

    //function getMyFullAvailableWithdraw() public view returns (uint256) {
      //  uint256 amount = 0;
        //uint256[] memory myTokens = PrivateSaleNft8FContractV1(nftContractAddress).getMyTokens();
        //for(uint256 i = 0; i <= myTokens.length; i++) {
            //amount += balances[myTokens[i]].payment.tokensAmount;
        //}
        //return balances[myTokens[1]].payment.tokensAmount;
    //}
}
...
contract PrivateSaleNft8FContractV1 is Initializable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function getMyTokens () public view returns (uint256[] memory){
        uint256[] memory _tokensOfOwner = new uint256[](balanceOf(tx.origin));
        uint256 userTokenId = 0;
        for (uint256 i = 1; i <= _tokenIds.current(); i++) {
            if (ownerOf(i) == tx.origin) {
