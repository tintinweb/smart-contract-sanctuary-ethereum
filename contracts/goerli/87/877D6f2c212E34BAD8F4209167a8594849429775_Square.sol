...
/*
 * ?  Non Fungible Tokenの性質を持つIPFS上にMetadataを持つ
 * OK 任意の他アドレスにトークンをtransferできる
 * OK 任意の他アドレスに「引き出し権限」を付与/剥奪できる
 * OK 「引き出し権限」の設定状況を参照できる
 * OK トークン保有者アドレスを参照できる
 * OK トークンtransfer履歴を参照できる
 * OK トークンの発行権限を、特定のコントラクトに制限する
 * OK トークンの消却権限を、特定のコントラクトに制限する
 * OK トークン累計発行数と現存数を参照できる
 * その他要件は開発Phaseで定義
 */
contract Square is
    Ownable,
    ...
    ISquare
{
    ...
    uint256 private _maxSquarePerValidator = 10;
    uint256 private _lastTokenId = 0;
    //    mapping(uint256=>address) private _tokenValidator;
    TokenInfo[] public tokenInfo;
    IKnightsOfTheRoundTable private _roundContract;
    mapping(address => uint256) private _numIssued;

    function setMaxSquarePerValidator(uint256 maxSquarePerValidator)
        external
        onlyOwner
    {
        _maxSquarePerValidator = maxSquarePerValidator;
    }

    function setKnightsOfTheRoundTable(address _address) external onlyOwner {
        _roundContract = IKnightsOfTheRoundTable(_address);
    }

    function getKnightsOfTheRoundTable() external view returns (address) {
        return address(_roundContract);
    }
    ...
    function tokens(uint256 tokenId) external view returns (TokenInfo memory) {
        require(
            tokenId > 0 && tokenId <= tokenInfo.length,
            "Square: invalid tokenId"
        );
        require(
            tokenInfo[tokenId - 1].validator != address(0),
            "Square: invalid tokenId"
        );
        return tokenInfo[tokenId - 1];
    }

    function mint(address to, string calldata _metadata)
        external
        returns (uint256)
    {
        require(
            _roundContract.isValidator(msg.sender),
            "Square: caller is not validator"
        );
        require(
            _numIssued[msg.sender] < _maxSquarePerValidator,
            "Square: exceeds maximum square token for the validator"
        );
        _mint(to, tokenInfo.length + 1);
        tokenInfo.push(TokenInfo(msg.sender, _metadata));
        _numIssued[msg.sender]++;
        return tokenInfo.length;
    }

    function burn(uint256 tokenId) external {
        require(
            msg.sender == validatorOf(tokenId) || isBurner(msg.sender),
            "Square: burner is not a validator of token nor burner"
        );
        _burn(tokenId);
        _numIssued[msg.sender]--;
        tokenInfo[tokenId - 1].validator = address(0);
    ...
    function numIssued(address validator) external view returns (uint256) {
        return _numIssued[validator];
    }

    /**
     * @notice tokenを取り上げる
     * 指定したtokenIdのvalidatorのみが実行可能
     */
    function takeAway(uint256 tokenId) external {
        require(
            validatorOf(tokenId) == msg.sender,
            "Square: not a validator of the token"
        );
        _transfer(ownerOf(tokenId), msg.sender, tokenId);
    }

    /**
     * @notice tokenのvalidatorを返す。
     */
    function validatorOf(uint256 tokenId)
        public
        view
        override
        returns (address)
    {
        ...
        return tokenInfo[tokenId - 1].validator;
    }
