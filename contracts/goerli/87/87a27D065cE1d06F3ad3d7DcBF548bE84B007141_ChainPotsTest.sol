...
contract ChainPotsTest {
    ...
    uint256 public dailyPotMaxAmount = 0.02 ether;
    uint256 public dailyPotMaxEntries = 2;
    ...
    modifier isDailyPotFull() {
        require(
            dailyPotMaxEntries == playersDailyPot.length,
            "dailyPot pot is not full"
        );
        _;
    }
    ...
    function enterDailyPot(uint256 _count) public payable {
        ...
        require(
            playersDailyPot.length + _count <= dailyPotMaxEntries,
    ...
    function dailyPotTimer() external {
        require(
            block.timestamp - lastRun > 2 minutes,
