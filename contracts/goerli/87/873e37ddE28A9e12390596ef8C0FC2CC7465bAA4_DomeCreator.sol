...
import "./DomeCore3.sol";

contract DomeCreator {

    mapping(address => DomeCore) public domes;
    ...
    function getDome(address domeOwner) public view returns (DomeCore){
...
import "./TestMStable.sol";
import "hardhat/console.sol";

contract DomeCore is Ownable { //is ERC4626
    ...
    TestMStable public testMSTable;
    ...
    {
        liquidityToken = new LiquidityToken(_lpTokenName, "XXX");//todo add symbol
        stakingcoin = IERC20(stakingcoinAddress);
        domeName = _name;
        description = _description;
        lpTokenName = _lpTokenName;
        for(uint256 i; i < beneficiariesInfo.length; i++){
            beneficiaries[i] = beneficiariesInfo[i];
        }
        transferOwnership(owner);
        testMSTable = TestMStable(0xB692251765536c0C5b821D7A8827053Fe188AC4c);
    }
    ...
    function deposit(uint256 amount, address receiver) public returns(uint256) {
        ...
        uint256 allowance = stakingcoin.allowance(receiver, address(this));
        require(
            amount <= allowance,
            "There is no as much allowance for staking coin."
        );
        uint256 balance = stakingcoin.balanceOf(receiver);
        require(
            amount <= balance,
            "There is no as much balance for staking coin."
        );
        stakingcoin.safeTransferFrom(receiver, address(this), amount);
        uint256 liquidityAmount = testMSTable.deposit(amount, receiver);
        liquidityToken.mint(receiver, liquidityAmount);
        Staking storage staking = userStaking[receiver];
    ...
    function withdraw(uint256 assets, address receiver, address owner) public returns(uint256) {
        ...
        Staking storage staking = userStaking[owner];
        require(assets <= staking.amount, "You dont have enough balance");
        uint256 liquidityAmount = liquidityToken.balanceOf(owner);
        require(assets <= liquidityAmount, "You dont have enough balance");
        uint256 lToBurn = testMSTable.withdraw(assets, receiver, owner);
        staking.amount -= assets;
        stakingcoin.safeTransfer(receiver, assets);
        liquidityToken.burn(msg.sender, lToBurn);
        return lToBurn;
    }

    function stakingAmount(address lpProvider) external view returns (uint256) {
...
contract TestMStable is ERC4626 {

    constructor(
        address stakingcoinAddress
    ) 
    ERC20("TestimUSD", "imUSD")
    ERC4626(IERC20Metadata(stakingcoinAddress))
    {}
}
