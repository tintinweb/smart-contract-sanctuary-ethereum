...
contract NFTSale {
    ...
    // Offer valid until not expired and Buer balance is suvvicient
    ...
    //expired sale is also considered unset
    function isBuyNowPriceNotSet(
        address _nftContractAddress,
        uint256 _tokenId,
        address _nftSeller
    ) internal view returns (bool) {
        return (
            directSales[_nftContractAddress][_tokenId][_nftSeller].saleEnd == 0 ||
        ...
        isFeePercentagesLessThanMaximum(_feePercentages)
    {
        ...
        _transferNftToSaleContract(_nftContractAddress, _tokenId, msg.sender);
    ...
    {
        require(
            isBuyNowPriceNotSet(_nftContractAddress, _tokenId, msg.sender),
            "BuyNow price is already set"
        );
   ...
   function buyNFT(
        ...
        amountMeetsRequirements(_nftContractAddress, _tokenId, _nftSeller, _erc20Token, _tokenAmount) 
    {
        ...
        require(
            !isBuyNowPriceNotSet(_nftContractAddress, _tokenId, _nftSeller),
            "BuyNow price is not set"
        );

        _transferNftToSaleContract(_nftContractAddress, _tokenId, _nftSeller);
        _transferNftAndPaySeller(
            _nftContractAddress, 
            _tokenId,
            _nftSeller, 
