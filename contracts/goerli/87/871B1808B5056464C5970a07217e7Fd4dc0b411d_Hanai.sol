...
contract Hanai is HanaiBase, ReentrancyGuard {
    // ===== Provenance ======
     ...
     */
    constructor(address authority, address admin, address royaltyAddress) HanaiBase(admin, royaltyAddress) {
     ...
     * @param tokenIds ids of token to create
     *
     * Requirements:
     *
     * - `saleOn` must be false,
     * - the caller must be `_admin`.
     */
    function privateMint(address[] memory tos, uint256[] memory tokenIds) external onlyAdmin isSaleOn(false) {
        if (tos.length != tokenIds.length) { revert ArrayLengthMismatch(); }

        for (uint256 i = 0; i < tos.length; i++) {
            if (tokenIds[i] <= 0 || tokenIds[i] > maxSupply) { revert InvalidTokenId(); }
     ...
     * - `saleOn` must be true,
     */
    function publicMint(uint256 tierId, uint256[] memory tokenIds, bytes memory signature) external payable isSaleOn(true) validTime(tierId) {        
        bytes32 data = keccak256(abi.encode(address(this), msg.sender, tierId, tokenIds));
        if (!SignatureChecker.isValidSignatureNow(_authority, data, signature)) { revert InvalidSignature(); }

        uint256 ticketNum = tokenIds.length;
        
        // all public sales share same counter mapping, contract trusts that all signature addresses are unique
        publicMints[msg.sender] += ticketNum;
        if (publicMints[msg.sender] > tiers[tierId].maxTicketNum) { revert ExceedMaxTicket(); }

        uint256 totalPrice = tiers[tierId].ticketPrice * ticketNum;
...
error TransferLockedByLocker();
error RoyaltyPercentageExceed();
error ArrayLengthMismatch();

contract HanaiBase is ERC721, IERC2981, Ownable {
     ...
     */
    modifier onlyLocker() {
        if (!lockers[_msgSender()])  { revert NonLocker(); }
        _;
    }

    /**
     * @dev Sets an address as admin role
    ...
    /**
     * @dev Sets whether an address is locker role
     * @param locker address to set
     * @param valid if the address is valid locker
     *
     * Requirements:
     *
     * - the caller must be `owner`.
     */
    function updateLocker( address locker, bool valid) external onlyOwner {
        lockers[locker] = valid;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked("https://metadata.thefwenclub.com/hanai/", tokenId.toString()));
    }
     ...
     * - the caller must be `owner`.
     */
    function toggleFlag(uint256 flag) public virtual onlyAdmin {
    ...
    function lockTransfer (uint256 tokenId, bool locked) external {
        if (!_isApprovedOrOwner(_msgSender(), tokenId)) { revert NonOwnerOrApproved(); }
        if (transferLocks[tokenId] == LockStatus.LockedByLocker) { revert TransferLockedByLocker(); }

        transferLocks[tokenId] = locked ? LockStatus.LockedByTokenOwner : LockStatus.Unlock;
     ...
     * - the caller must be `_admin`.
     */
    function lockTransfers (uint256[] memory tokenIds, bool[] memory locks) external onlyLocker {
        if (tokenIds.length != locks.length) { revert ArrayLengthMismatch(); }

        for (uint256 i = 0; i < tokenIds.length; i++) {
            transferLocks[tokenIds[i]] = locks[i] ? LockStatus.LockedByLocker : LockStatus.Unlock;
