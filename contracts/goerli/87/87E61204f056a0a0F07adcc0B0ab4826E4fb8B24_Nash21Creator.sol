...
/// @title The interface of the Nash21OffChainData contract
/// @notice Stores off chain data about the NFTs
interface INash21OffChainData {
    /// EVENTS

    /// @notice Emitted when the off chain hash is set
    /// @param tokenId The token id
    /// @param cid The IPFS hash
    event SetContractHash(uint256 tokenId, string cid);
    ...
    /// @notice Sets the off chain hash for a token
    /// @param tokenId The token id
    /// @param cid The hash
    function setContractHash(uint256 tokenId, string memory cid) external;

    /// @notice Gets the off chain hash for a token
    /// @param tokenId The token id
    /// @return cid The hash
    function contractHashByTokenId(uint256 tokenId) external view returns (string memory cid);
}
...
contract UUPSUpgradeableByRole is AccessControlProxyPausable, UUPSUpgradeable {
    // solhint-disable-next-line
    function _authorizeUpgrade(address newImplementation) internal view override onlyRole(keccak256("UPGRADER_ROLE")) {}
}
...
import {INash21OffChainData} from "../interfaces/INash21OffChainData.sol";
import {INash21Manager} from "../interfaces/INash21Manager.sol";

contract Nash21Creator is UUPSUpgradeableByRole {
    bytes32 private constant _CREATOR_ADMIN_ROLE = keccak256("CREATOR_ADMIN_ROLE");
    ...
    function initialize() public initializer {
        __AccessControlProxyPausable_init(_msgSender());
    }
    ...
    function authAndCreate(
        ...
        string memory uri,
        string memory cid
    ) external onlyRole(_CREATOR_ADMIN_ROLE) returns (uint256 tokenId) {
        INash21Manager managerInterface = INash21Manager(manager);
        INash21Factory factoryInterface = INash21Factory(managerInterface.get(keccak256("FACTORY")));
        INash21OffChainData offChainDataInterface =
            INash21OffChainData(managerInterface.get(keccak256("OFFCHAIN_DATA")));
        tokenId = factoryInterface.authAndCreate(hashId, account, value, currency, startDate, endDate, uri);
        offChainDataInterface.setContractHash(tokenId, cid);
    }
}