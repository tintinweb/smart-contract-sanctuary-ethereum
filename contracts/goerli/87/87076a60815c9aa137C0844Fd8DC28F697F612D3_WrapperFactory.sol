...
contract WrapperFactory is Owned, MixinResolver, IWrapperFactory {
    ...
    uint256 internal constant WRAPPER_VERSION = 1;
    ...
    function isWrapper(address possibleWrapper) external view returns (bool) {
        return flexibleStorage().getUIntValue(CONTRACT_NAME, bytes32(uint256(address(possibleWrapper)))) > 0;
    }

    function feesEscrowed() public view returns (uint256) {
    ...
    function createWrapper(
        ...
        flexibleStorage().setUIntValue(CONTRACT_NAME, bytes32(uint256(address(wrapper))), WRAPPER_VERSION);
    ...
    function distributeFees() external {
        ...
        uint256 amountSUSD = feesEscrowed();
...
contract Wrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IWrapper {
    ...
    uint256 public targetSynthIssued;
    ...
    function capacity() public view returns (uint256 _capacity) {
        // capacity = max(maxETH - balance, 0)
        uint256 balance = getReserves();
        uint256 maxToken = maxTokenAmount();
        if (balance >= maxToken) {
            return 0;
        }
    ...
    function totalIssuedSynths() public view returns (uint256) {
    ...
    function calculateMintFee(uint256 amount) public view returns (uint256, bool) {
        int256 r = mintFeeRate();

        if (r < 0) {
            return (amount.multiplyDecimalRound(uint256(-r)), true);
        } else {
            return (amount.multiplyDecimalRound(uint256(r)), false);
        }
    }

    function calculateBurnFee(uint256 amount) public view returns (uint256, bool) {
        int256 r = burnFeeRate();
    ...
    function mint(uint256 amountIn) external notPaused issuanceActive {
        ...
        uint256 currentCapacity = capacity();
        require(currentCapacity > 0, "Contract has no spare capacity to mint");

        uint256 actualAmountIn = currentCapacity < amountIn ? currentCapacity : amountIn;

        (uint256 feeAmountTarget, bool negative) = calculateMintFee(actualAmountIn);
        uint256 mintAmount = negative ? actualAmountIn.add(feeAmountTarget) : actualAmountIn.sub(feeAmountTarget);
    ...
    function burn(uint256 amountIn) external notPaused issuanceActive {
        ...
        (uint256 burnFee, bool negative) = calculateBurnFee(targetSynthIssued);
        ...
        if (negative) {
            ...
            amountOut = burnAmount.multiplyDecimal(
                ...
                uint256(int256(SafeDecimalMath.unit()) - burnFeeRate())
            );
        } else {
        ...
        uint256 feeAmountTarget = negative ? 0 : burnAmount.sub(amountOut);
    ...
    function _mint(uint256 amount) internal {
        ...
        uint256 excessAmount = reserves > targetSynthIssued.add(amount) ? reserves.sub(targetSynthIssued.add(amount)) : 0;
        uint256 excessAmountUsd = exchangeRates().effectiveValue(currencyKey, excessAmount, sUSD);
    ...
    function _burn(uint256 amount) internal {
        ...
        uint256 excessAmount = reserves.add(amount) > targetSynthIssued ? reserves.add(amount).sub(targetSynthIssued) : 0;
    ...
    function _setTargetSynthIssued(uint256 _targetSynthIssued) internal {
    ...
    function _safeTransferFrom(
        ...
        uint256 msgSize = msgData.length;
