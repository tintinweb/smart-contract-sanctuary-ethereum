    ...
    ERC1155Burnable
{
    ...
    function lazyMint(
        ...
        if (_exists(_tokenId)) {
            require(
                ...
                _voucher.tokenUri == _tokenURIs[_tokenId] &&
                    ...
                    // The `_voucher` signer MUST be the original artist or at least a royalty recipient for the user-supplied `_tokenId`,
            ...
            require(
                hasRole(MINTER_ROLE, signer) && _value <= _voucher.totalValue
            );
            // important, since the user-supplied `_tokenId` can't be right, it MUST be reassigned, or else the rest of the function will refer to a non-existent tokenId
            _tokenId = _totalSupply.length;
            _totalSupply.push(_value);
            _tokenURIs.push(_voucher.tokenUri);
            // royaltyRecipients mapping only exists in communal editions, in self-sovreign editions there is a single contract-wide royalty recipient
        ...
        _sendValue(_marketFeeRecipient, marketFeeAmount); // `_marketFeeRecipient` is a trusted admin multisig

        /*----------------------------------------------------------*|
        |*  # CHECK-EFFECTS-INTERACTIONS                            *|
        |*----------------------------------------------------------*/
        // leave external calls to untrusted addresses last
