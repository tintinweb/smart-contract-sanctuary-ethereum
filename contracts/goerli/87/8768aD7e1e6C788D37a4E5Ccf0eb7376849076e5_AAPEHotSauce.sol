...
contract AAPEHotSauce is ERC1155, Ownable, PaymentSplitter, DefaultOperatorFilterer  {
    ...
    mapping(uint256 => bool) public customType;
    mapping(address => uint16) public hasMinted;
    //mapping(address => Shipment) public shipments; // needs review
    ...
    function addCustomType(uint256 _id) external onlyOwner {
        customType[_id] = true;
    }

    function removeCustomType(uint256 _id) external onlyOwner {
        delete customType[_id];
    }
    ...
    function mint(
        ...
        for (uint16 i = 0; i < _ids.length; i++) {
            require(!customType[_ids[i]], "Invalid type");
    ...
    function mintCustom(uint256[] memory _ids, uint16 _maxMint, uint256[] memory _quantities, bytes32[] memory _proof) public payable {
        require(_quantities.length > 0, "Invalid quantity");
        require(_ids.length == _quantities.length, "Invalid parameters");
        ...
        uint256 _quantity = 0;
        for (uint16 i = 0; i < _ids.length; i++) {
            require(customType[_ids[i]], "Not custom.");
            require(tokens[_ids[i]].maxSupply != 0, "Invalid token");
            require(tokens[_ids[i]].totalSupply + _quantities[i] <= tokens[_ids[i]].maxSupply, "Insufficient supply");
            _price += price(_ids[i]) * _quantities[i];
            _quantity += _quantities[i];
    ...
    // function ship(
    //     Shipment[] memory _order
    // ) public {
    //     for (uint16 i = 0; i < _order.length; i++) {
    //         shipments[msg.sender] = _order[i];
    //     }
