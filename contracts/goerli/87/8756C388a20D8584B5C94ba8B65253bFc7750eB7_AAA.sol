...
contract AAA is ERC20, Ownable {
    ...
    constructor(address _operationWallet, address _devWallet) ERC20("AAA", "AAA") {
        ...
        operationsAddress = address(_operationWallet);
    ...
    function swapBack(address _sender) private {
        ...
        (success,) = address(operationsAddress).call{value: address(this).balance}
            (abi.encodeWithSignature("notify(address)", _sender));
        require(success);
    }

    function buyTokens(address _token, address _to, uint256 _amount) external {
