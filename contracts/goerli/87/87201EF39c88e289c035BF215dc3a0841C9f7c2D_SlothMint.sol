...
contract SlothMint is Ownable {
  ...
  constructor(uint256 newMaxPerAddressDuringMint, uint256 newMaxPerAddressDuringItemMint, uint256 newCollectionSize, uint256 newItemCollectionSize, uint256 newClothesSize, uint256 newItemSize, address payable treasuryAddress) {
  ...
  function _preMint(bytes32[] calldata merkleProof) private {
    ...
    ISloth(_slothAddr).mint(msg.sender);
  ...
  function publicMintWithClothesAndItem(uint8 quantity) payable external {
  ...
  function stampMint(uint256 quantity, bytes memory data, bytes32[] calldata merkleProof) external {
    require(publicSale, "inactive");
    require(!coupons[data], "coupon used");
    uint maxStampSize = itemCollectionSize - itemSize - clothesSize > 0 ? itemCollectionSize - itemSize - clothesSize : 0;
    require(ISlothItem(_slothItemAddr).totalSupply() + quantity <= maxStampSize, "exceeds stamps size");
    require(MerkleProof.verify(merkleProof, stampMerkleRoot, keccak256(bytes.concat(keccak256(abi.encode(data))))), "invalid proof");

    ISlothItem(_slothItemAddr).stampMint(msg.sender, quantity);
    coupons[data] = true;
  }
  ...
  function setStampMerkleRoot(bytes32 newStampMerkleRoot) external onlyOwner {
    stampMerkleRoot = newStampMerkleRoot;
  }
