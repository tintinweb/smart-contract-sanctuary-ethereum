...
abstract contract Account is IAccount, ReentrancyGuard {
	...
	uint public constant CFID = type(uint256).max; // Community fund id
	...
	function InitializeAccount() public notAccountInitialized {
		_InitAccount(CFID, msg.sender); // Init community fund
		XPrograms.InitializeXProgram(IAccount(address(this)), IBalance(address(this)), ITuktu(address(this)));
...
abstract contract Balance is IBalance, Account {
	...
	function InitializeBalance() public notBalanceInitialized {
		InitializeAccount();
...
contract Tuktu is ITuktu, Balance {
	...
	function InitializeTuktu(IXProgram _XProgram, IMatrix _Matrix) public notTuktuInitialized {
		XPrograms = _XProgram;
		Matrixes = _Matrix;
		InitializeBalance();
