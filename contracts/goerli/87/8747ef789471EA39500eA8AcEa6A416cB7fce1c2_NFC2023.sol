...
contract NFC2023 is ERC721, ERC721Enumerable, ERC2981ContractWideRoyalties, Ownable {
    ...
    */
    ...
    string public constant PERSISTENT_IPFS_HASH = "QmdjxKfEbY5xH2BACS9VvbWYYybLnRQRadiTevkmFKvHrc";
    string public constant PERSISTENT_ARWEAVE_HASH = "jWr16e0Yt9H__uzxCf8ACHKbmh8Q7RScNZMoFunuo98";
    string public constant PERSISTENT_SHA256_HASH_PROVENANCE = "d6357cf6261516ad796bde082f473fdbeeab66b5221044e0c91cc58ef4516ba3";
    ...
    modifier onlyPublicSales {
        require(phase == 0, "Only during public sales.");
        _;
    }

    constructor(address _paidWallet, bytes32 _merkleRoot, uint256 _phase, uint256 _basePrice, string memory _jsonForFetchingOnly)
    ...
    function updateJsonFetching(string calldata _jsonForFetchingOnly) public onlyOwner {
        jsonForFetchingOnly = _jsonForFetchingOnly;
    }
    ...
    function getRedeem(address _nft_contract, uint256 _nft_id) public view returns (bool) {
        return coupons[_nft_contract].redeemed[_nft_id];
    }

    function append721Coupon(address[] calldata _nft_contract, uint256[] calldata _phase) public onlyOwner {
        for (uint256 i = 0; i < _nft_contract.length;) {
            if (_phase[i] <= 100 && NFCLibrary.check721Compliancy(_nft_contract[i])) {
                if (coupons[_nft_contract[i]].added == false) {
                    if (NFCLibrary.check721EnumerableCompliancy(_nft_contract[i])) {
    ...
    function discountedMint(address _nft_contract, uint256 _nft_id) public payable {
    ...
    function normalMint() public payable onlyPublicSales {
        require(!salesLocked, "Sales are closed.");
    ...
    function freeMint(bytes32[] calldata _merkle) public {
        require(!whitelistClaimed[msg.sender], "Already freemint.");
