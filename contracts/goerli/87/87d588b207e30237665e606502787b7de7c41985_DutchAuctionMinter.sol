...
import "./LightYearsPasses.sol";
import "./Pausable.sol";
import "./PaymentDistributor.sol";

contract DutchAuctionMinter is Pausable, PaymentDistributor {
    uint256 public constant walletLimit = 3;
    string private constant limitRevertMessage = "Limited to 3 purchases per wallet";

    /// @notice ERC-721 pass contract whose tokens are minted by this auction
    /// @dev Must implement mint(address)
    LightYearsPasses public passes;
    ...
    constructor(
        ...
        require(passes_.supportsInterface(0x6a627842), "Pass contract must implement mint(address)");
    ...
    function mint() external payable started whenNotPaused {
        // CHECKS inputs
        ...
        uint256 id = passes.mint(msg.sender);
    ...
    function mintMultiple(uint256 quantity) external payable started whenNotPaused {
        // CHECKS inputs
        uint256 alreadyMinted = mintCount[msg.sender];
        require(quantity > 0, "Must mint at least one pass");
        require(alreadyMinted + quantity <= walletLimit, limitRevertMessage);
        ...
        uint256 firstId = passes.mint(msg.sender);
        emit Purchase(msg.sender, firstId, price + (payment % quantity));
        uint256 failed = 0;
        for (uint256 i = 1; i < quantity; i++) {
            try passes.mint(msg.sender) returns (uint256 nextId) {
                emit Purchase(msg.sender, nextId, price);
            } catch {
                // Don't revert, just give the sender what was already minted
                unchecked {
                    // Unchecked arithmetic: i always less than quantity
                    failed = quantity - i;
                    quantity = i;
                    // Unchecked arithmetic: mintCount cannot exceed walletLimit
                    mintCount[msg.sender] = alreadyMinted + i;
                }
            }
        }
        ...
        if (failed > 0) {
            uint256 refund;
            unchecked {
                // Unchecked arithmetic: refund will be less than payment
                refund = price * failed;
            }
            (bool refunded, ) = msg.sender.call{value: refund}("");
            require(refunded, "Mint failure refund reverted");
        }
    }
    ...
    function setPasses(LightYearsPasses passes_) external unstarted onlyOwner {
        // CHECKS inputs
        require(address(passes_) != address(0), "Pass contract must not be the zero address");
        require(passes_.supportsInterface(0x6a627842), "Pass contract must support mint(address)");
        // EFFECTS
        passes = passes_;
    }
