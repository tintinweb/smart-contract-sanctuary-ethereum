...
contract subscriptionFactory {
    ...
    function createSubscription(uint96[] memory _price, string memory _name, address _owner, bool _USDc, bool _Dai, bool _USDt, bool _recurring, bool _streaming) external {
        Subscription subscription = new Subscription(_price, _name, _owner, _USDc, _Dai, _USDt, _recurring, _streaming);
    ...
    function userDeployedContracts(address _user) external view returns(address[] memory) {
    ...
    function addressOfUserByIndex(address owner, uint256 index) internal view returns(address addr){
        uint count;
...
contract Subscription {
    ...
    function getDetails() public view returns(uint256[] memory) {
        ...
        uint256 sumBalance = uint256(USDc.balanceOf(address(this)))/1e6 + uint256(Dai.balanceOf(address(this)))/1e18 + uint256(USDt.balanceOf(address(this)))/1e18 + uint256(USDcx.balanceOf(address(this)))/1e18 + uint256(Daix.balanceOf(address(this)))/1e18 + uint256(USDtx.balanceOf(address(this)))/1e18;
    ...
    //     detailList[0] = uint256(USDc.balanceOf(address(this)))/1e6; //usdc uses 1e6
    //     detailList[1] = uint256(Dai.balanceOf(address(this)))/1e18; //everything else uses ether
    //     detailList[2] = uint256(USDt.balanceOf(address(this)))/1e18;
    //     detailList[3] = uint256(USDcx.balanceOf(address(this)))/1e18;
    //     detailList[4] = uint256(Daix.balanceOf(address(this)))/1e18;
    //     detailList[5] = uint256(USDtx.balanceOf(address(this)))/1e18;
