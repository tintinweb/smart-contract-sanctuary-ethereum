...
contract Staking3 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Deposit(uint128 stakeAmount, uint128 stakeType, uint128 stakePeriod);
    event Withdraw(uint128 stakeAmount, uint128 stakeType, uint128 rewardAmount);
    event Compound(uint128 rewardAmount, uint128 stakeType);
    event Claim(uint128 rewardAmount, uint128 stakeType);
    ...
    function deposit(uint128 _amount, uint128 _stakeType) external nonReentrant {
        ...
        DetailsLibraryUpdated.eachTransaction storage stakes = userStakesData[msg.sender][_stakeType];
        ...
        if(stakes.stakeAmount == 0) {
            ...
            stakes.fullWithdrawlTime = uint128(block.timestamp) + stakeType.stakePeriod;            
            ...
            if(uint128(block.timestamp) < userStakesData[msg.sender][_stakeType].fullWithdrawlTime) {
                stakeTimeTillNow = uint128(block.timestamp) - userStakesData[msg.sender][_stakeType].lastClaimTime;
                totalRewardTillNow = rewardCalculation(actualAmount, _stakeType, stakeTimeTillNow);                

            } else {

                stakeTimeTillNow = userStakesData[msg.sender][_stakeType].fullWithdrawlTime - userStakesData[msg.sender][_stakeType].lastClaimTime;
                if(stakeTimeTillNow > 0 ) totalRewardTillNow = rewardCalculation(actualAmount, _stakeType, stakeTimeTillNow); 

            }
            userStakesData[msg.sender][_stakeType].stakeAmount += totalRewardTillNow;

            stakeType.totalStakedIn += actualAmount + totalRewardTillNow;
            userTotalPerPool[msg.sender][_stakeType] +=  actualAmount + totalRewardTillNow;
            ...
            emit Deposit(totalRewardTillNow, _stakeType, stakeType.stakePeriod);
        ...
        if(fees > 0) 
        {
            success = stakedToken.transferFrom(msg.sender, ownerWallet, fees);
            if(!success) revert();
            
        }

         success = stakedToken.transferFrom(msg.sender, address(this), actualAmount);
        if(!success) revert();
        emit Deposit(actualAmount, _stakeType, stakeType.stakePeriod);

    }

    function compound(uint128 _stakeType) external nonReentrant {
        require(stakeTypeExist[_stakeType], "STAKE_TYPE_DOES_NOT_EXIST");
        ...
        uint128 stakeTimeTillNow;
            ...
            userTotalPerPool[msg.sender][_stakeType] +=  totalRewardTillNow;
            totalStaked += totalRewardTillNow;
            if(totalRewardTillNow > 0)
            claimReward(address(this), totalRewardTillNow);

            emit Compound(totalRewardTillNow, _stakeType);


    }

    function withdraw(uint128 _amount, uint128 _stakeType) external nonReentrant {

        DetailsLibraryUpdated.StakeTypeData storage stakeType = stakeTypesList[_stakeType];

        require(_amount > 0, "WITHDRAW_MORE_THAN_ZERO");
        require(stakeTypeExist[_stakeType], "STAKE_TYPE_DOES_NOT_EXIST");
        require(_amount <= userTotalPerPool[msg.sender][_stakeType], "CANT_WITHDRAW_MORE_THAN_STAKED");   // --------

        DetailsLibraryUpdated.eachTransaction storage stakes = userStakesData[msg.sender][_stakeType];

        require(block.timestamp > stakes.fullWithdrawlTime, "CANT_UNSTAKE_BEFORE_LOCKUP_TIME");

        uint128 stakeTimeTillNow = stakes.fullWithdrawlTime - stakes.lastClaimTime;
        ...
        if(rewardTillNow > 0) claimReward(msg.sender, rewardTillNow);
        bool success;
        if(fees > 0) 
        {
        ...
        if(actualAmount > 0) {
            success = stakedToken.transfer(msg.sender, actualAmount);
            if(!success) revert();
        }

        emit Withdraw(actualAmount, _stakeType, rewardTillNow);


    }

    function claim(uint128 _stakeType) external nonReentrant returns(bool) {
        ...
        if(totalRewardTillNow > 0)
        claimReward(msg.sender, totalRewardTillNow);

        emit Claim(totalRewardTillNow, _stakeType);
        return true;
    ...
    function rewardCalculation(uint128 _amount, uint128 _stakeType, uint128 _time) public view returns(uint128) {
        ...
        uint128 interest = (_amount * rate * 10 * _time) / (1000 * 3650);
        return interest;
