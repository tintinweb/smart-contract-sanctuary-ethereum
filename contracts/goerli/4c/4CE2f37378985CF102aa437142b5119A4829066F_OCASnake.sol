...
contract OCASnakeArt is Ownable, ReentrancyGuard {
    ...
    function funkCreator(
        ...
        return
            abi.encodePacked("rgba(", r, ",", g, ",", b, ",", opacity[a], ")");
    }
        ...
        returns (string memory)
    {
        ...
        string[] memory test = generateAllColors(tokenId, selectedColors);
        ...
        string memory base64 = Base64.encode(
            ...
            funkSnakeCreator(
                [
                    test[generateAppleSeed(tokenId, 786) % uint256(19)],
                    test[generateAppleSeed(tokenId, 69) % uint256(19)],
                    selectDifficulty(),
                    test[generateAppleSeed(tokenId, 23) % uint256(19)],
                    test[0],
                    test[generateAppleSeed(tokenId, 1337) % uint256(19)],
                    test[generateAppleSeed(tokenId, 420) % uint256(19)],
                    test[generateAppleSeed(tokenId, 143) % uint256(19)]
                ]
            )
        );
    ...
    function generateAllColors(uint256 tokenId, string[30] memory colors)
        public
        view
        returns (string[] memory)
    {
        uint256 rn = randomNumber(
            10,
            uint256(generateAppleSeed(tokenId, 420)) % uint256(colors.length)
        );
        string[] memory result = new string[](rn);
        for (uint256 i = 0; i < rn; i++) {
            result[i] = colors[generateColors(tokenId, rn + i)];
        }
    ...
    function retrofy(uint256 tokenId) private view returns (string[30] memory) {
        ...
        for (uint256 i = 0; i < parts.length; i++) {
            ...
            if (i == 1)
                parts[1] = string(
                    funkCreator(colors[0], colors[1], colors[2], 0)
                );
            else
                parts[i] = string(
