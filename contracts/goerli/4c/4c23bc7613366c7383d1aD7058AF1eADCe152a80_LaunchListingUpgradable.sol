    ...
    IERC165Upgradeable
{
    ...
    modifier validateParams(uint256[] memory nftId) {
        require(nftId.length > 0, "no nfts selected");
        _;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return interfaceId == IID_IERC2981;
    }
    ...
    function addFnftToListingV2(
        address fnft,
        uint256[] memory nftId,
        uint256 price
    ) public validateParams(nftId) {
        for (uint256 i = 0; i < nftId.length; ++i) {
            isCallerOwner(fnft, nftId[i]);
            fnftListingV2[msg.sender][fnft][nftId[i]][price] = true;
        }

        emit FNFTListedV2(fnft, nftId, msg.sender, price, block.timestamp);
    }

    function removeFnftFromListingV2(address fnft, uint256[] memory nftId, uint256[] memory price)
        public
        validateParams(nftId)
    {
        require(nftId.length == price.length,"invalid arr length");
        for (uint256 i = 0; i < nftId.length; ++i) {
                isCallerOwner(fnft, nftId[i]);
                delete fnftListingV2[msg.sender][fnft][nftId[i]][price[i]];
        }

        emit FNFTUnListedV2(fnft, nftId, msg.sender, block.timestamp);
    }

    function addNftToListingV2(
        address nft,
        uint256[] memory nftId,
        uint256 price
    ) public validateParams(nftId) {
        for (uint256 i = 0; i < nftId.length; ++i) {
            isCallerOwner(nft, nftId[i]);
            nftListingV2[msg.sender][nft][nftId[i]][price] = true;
        }

        emit NFTListedV2(nft, nftId, msg.sender, price, block.timestamp);
    }

    function removeNftFromListingV2(address nft, uint256[] memory nftId, uint256[] memory price)
        public
        validateParams(nftId)
    {
        for (uint256 i = 0; i < nftId.length; ++i) {
            ...
            delete nftListingV2[msg.sender][nft][nftId[i]][price[i]];
        }

        emit NFTUnListedV2(nft, nftId, msg.sender, block.timestamp);
    }
    ...
    function calculateRoyaltyFees(
        ...
        uint256 _tokenId
    ) public view returns (address _receiver, uint256 _amount) {
        IERC2981 _token = IERC2981(_tokenAddress);
        (_receiver, _amount) = _token.royaltyInfo(_tokenId, _salePrice);
    ...
    function purchaseNFTInETHV2(
        address[] memory _nft,
        uint256[] memory _nftId,
        uint256[] memory prices,
        ContractType _contractType
    ) external payable nonReentrant validateParams(_nftId) {
        require(msg.value > 0, "Insufficient amount");
        require(_nftId.length == _nft.length, "invalid arr length");
        uint256 platformFee;
        uint256 _platformFeeInPer = ILaunchSettings(launchSetting)
            .getPlatformFee(address(0));
        uint256 _originalAmt = msg.value;
        for (uint256 i = 0; i < _nft.length; i++) {
            uint256 currId = _nftId[i];
            address currAd = _nft[i];
            uint256 currPrice = prices[i];
            address _owner = IERC721Upgradeable(currAd).ownerOf(currId);
            if (_contractType == ContractType.FNFT && fnftListingV2[_owner][currAd][currId][currPrice]) {
                // getting price from mapping
                
                uint256 _platformFeePerNft = (currPrice * _platformFeeInPer) /
                    10000;
                if (_originalAmt >= currPrice) {
                    _originalAmt = _originalAmt - currPrice;
                    //Summing platform fee
                    platformFee = platformFee + _platformFeePerNft;
                    // deduct platformfee per nft from price of nft
                    uint256 _amt = currPrice - _platformFeePerNft;

                    ILaunchNfti launchNfti = ILaunchNfti(currAd);

                    if (launchNfti.curator() != _owner) {
                        (
                            address _receiver,
                            uint256 _royaltyAmt
                        ) = calculateRoyaltyFees(_amt, currAd, currId);

                        uint256 _saleAmt = _amt - _royaltyAmt;

                        // transfer _royaltyAmt to curator
                        payable(_receiver).transfer(_royaltyAmt);
                        // transfer _saleAmt price to owner
                        payable(_owner).transfer(_saleAmt);
                    } else {
                        // transfer remainingAmt to owner/curator
                        payable(_owner).transfer(_amt);
                    }
                    launchNfti.safeTransferFrom(_owner, msg.sender, currId, "");
                    delete fnftListingV2[_owner][currAd][currId][currPrice];
                }
                
            } else if (_contractType == ContractType.NFT && nftListingV2[_owner][currAd][currId][currPrice]) {
                uint256 _platformFeePerNft = (currPrice * _platformFeeInPer) /
                    10000;
                if (_originalAmt >= currPrice) {
                    _originalAmt = _originalAmt - currPrice;
                    ...
                    // deduct platformfee per nft from price of nft
                    uint256 _amt = currPrice - _platformFeePerNft;
                    IERC721Upgradeable erc721Token = IERC721Upgradeable(currAd);

                    // if (currAd.supportsInterface(IID_IERC2981)) {
                    if (
                        IERC165Upgradeable(currAd).supportsInterface(
                            IID_IERC2981
                        )
                    ) {
                        (
                        ...
                        // transfer _royaltyAmt to receiver
                        payable(_receiver).transfer(_royaltyAmt);
                        // transfer _saleAmt price to owner
                        payable(_owner).transfer(_saleAmt);
                    } else {
                    ...
                    delete nftListingV2[_owner][currAd][currId][currPrice];
                }
            } // else if
    ...
    function purchaseNFTV2(
        ...
        ContractType _contractType
    ) external validateParams(_nftId) {
        require(_nftId.length == _nft.length, "invalid arr length");
        uint256 multiplier = getLatestPrice(_feed);
        uint256 originalAmt;
        address buyInAd = _buyIn;
        uint256 platformFee;
        // uint256 _platformFeeInPer = ILaunchSettings(launchSetting)
        //     .getPlatformFee(address(0));
        uint256 _platformFeeInPer = ILaunchSettings(launchSetting)
            .getPlatformFee(buyInAd);
        IERC20Upgradeable erc20Token = IERC20Upgradeable(buyInAd);

        for (uint256 i = 0; i < _nft.length; i++) {
            uint256 currId = _nftId[i];
            address currAd = _nft[i];
            uint256 currPrice = _prices[i];
            address _owner = IERC721Upgradeable(currAd).ownerOf(currId);
            if (_contractType == ContractType.FNFT && fnftListingV2[_owner][currAd][currId][currPrice]) {
                    // getting price from mapping
                   
                    uint256 _price = (currPrice * 10**18) /  multiplier;
                    uint256 _platformFeePerNft = (_price * _platformFeeInPer) / 10000;
                    if (erc20Token.balanceOf(msg.sender) >= _price) {
                        originalAmt = originalAmt + _price;

                        //Summing platform fee
                        platformFee = platformFee + _platformFeePerNft;
                        // deduct platformfee per nft from price of nft
                        uint256 _amt = _price - _platformFeePerNft;

                        ILaunchNfti launchNfti = ILaunchNfti(currAd);

                        if (launchNfti.curator() != _owner) {
                            (address _receiver, uint256 _royaltyAmt) = calculateRoyaltyFees(_amt, currAd, currId);
                            ...
                            // transfer remainingAmt to owner/curator
                            erc20Token.transferFrom(msg.sender, _owner, _amt);
                        }
                    ...
                    uint256 _price = (currPrice *
                    10**18) / multiplier;
                    uint256 _platformFeePerNft = (_price * _platformFeeInPer) /
                        10000;
                    if (erc20Token.balanceOf(msg.sender) >= _price) {
                        originalAmt = originalAmt + _price;
                        ...
                        // deduct platformfee per nft from price of nft
                        uint256 _amt = _price - _platformFeePerNft;
                        IERC721Upgradeable erc721Token = IERC721Upgradeable(
                            currAd
                        );
                        ...
                        if (currAd.supportsInterface(IID_IERC2981)) {
                            (address _receiver,uint256 _royaltyAmt) = calculateRoyaltyFees(_amt, currAd, currId);
                                ...
                                // transfer _saleAmt price to owner
                                erc20Token.transferFrom(msg.sender, _owner, _saleAmt);
    ...
    function isCallerOwner(address _nft, uint256 _nftId)
        internal
        view
        returns (bool _success)
    {
        require(
            IERC721Upgradeable(_nft).ownerOf(_nftId) == msg.sender,
            "not owner of nft"
        );
        _success = true;
    }
    ...
    function purchaseNFTInETH(
        ...
        if (_contractType == ContractType.FNFT) {
            // it is checking the price of given token id is 0
            require(
            ...
            if (launchNfti.curator() != launchNfti.ownerOf(_tokenId)) {
                // deduct royalty
                (address _receiver, uint256 _royaltyAmt) = calculateRoyaltyFees(
                    _remainingAmt,
                    _tokenAddress,
                    _tokenId
                );
    ...
    function purchaseNFT(
        ...
        if (_contractType == ContractType.FNFT) {
            uint256 amt = (fnftListing[_tokenAddress][_tokenId] * 10**18) /
                multiplier;
            require(
            ...
            if (launchNfti.curator() != oldOwner) {
                // deduct royalty
                (address _receiver, uint256 _royaltyAmt) = calculateRoyaltyFees(
                    _remainingAmt,
                    _tokenAddress,
                ...
                // transfer _royaltyAmt to curator
                IERC20(_buyIn).transferFrom(msg.sender, _receiver, _royaltyAmt);

                // transfer _saleAmt price to owner
                IERC20(_buyIn).transferFrom(msg.sender, oldOwner, _saleAmt);
            } else {
                // transfer remainingAmt to owner/curator
                IERC20(_buyIn).transferFrom(
                    msg.sender,
            ...
            launchNfti.safeTransferFrom(oldOwner, msg.sender, _tokenId, "");
            ...
            uint256 amt = (nftListing[_tokenAddress][_tokenId] * 10**18 ) /
                multiplier;
