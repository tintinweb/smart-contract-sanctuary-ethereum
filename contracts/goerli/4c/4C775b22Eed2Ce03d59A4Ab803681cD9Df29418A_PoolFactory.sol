 ...
 * @title Pool
 *
 * @notice this implements the CML core Pool contract. It supports generalized
...
contract Pool is IPool, IEvents, ReentrancyGuard, OwnerController {
    ...
    function stake(
        ...
        (address account, uint256 shares) = _staking.stake(
            msg.sender,
            amount,
            stakingdata
        );
    ...
    function unstake(
        ...
        (address account, uint256 shares) = _staking.unstake(
            msg.sender,
            amount,
            stakingdata
        );
    ...
    function claim(
        ...
        (address account, uint256 shares) = _staking.claim(
            msg.sender,
            amount,
            stakingdata
        );
     ...
     * @inheritdoc IPool
     */
    function cmlBalance() external view override returns (uint256) {
        return _cmlVested;
    }
    ...
    function withdraw(uint256 amount) external override {
        requireController();
        require(amount > 0, "p1");
        require(amount <= _cmlVested, "p2");

        // do transfer
        _cml.safeTransfer(msg.sender, amount);

        _cmlVested = _cmlVested - amount;

        emit CMLWithdrawn(amount);
    }
    ...
    /**
     * @dev private method to process CML spending and vesting
     * @param spent number of tokens to unstake
     * @param vested data passed to staking module
     */
    function _processCML(uint256 spent, uint256 vested) private {
        // spending
        if (spent > 0) {
            _cml.safeTransferFrom(msg.sender, address(this), spent);
        }
        ...
        if (vested > 0) {
            ...
            if (fee > 0) {
                _cml.safeTransfer(_factory.treasury(), fee);
            }
            _cmlVested = _cmlVested + vested - fee;
        }
    }
}
...
contract PoolFactory is IPoolFactory, OwnerController {
    ...
    address[] public list;
    address private _cml;
    address private _treasury;
    uint256 private _fee;
    mapping(address => ModuleFactoryType) public whitelist;

    /**
     * @param cml_ address of CML token
     */
    constructor(address cml_, address treasury_) {
        _cml = cml_;
        _treasury = treasury_;
        _fee = MAX_FEE;
    }
    ...
    function create(
        ...
        // create pool
        Pool pool = new Pool(stakingModule, rewardModule, _cml, address(this));
    ...
    /**
     * @notice update the CML treasury address
    ...
    /**
     * @notice update the global CML spending fee
     * @param fee_ new value for CML spending fee
     */
...
interface IPool {
    ...
    /**
     * @return CML usage ratio for Pool
     */
    ...
    /**
     * @return CML balance available for withdrawal
     */
    function cmlBalance() external view returns (uint256);

    /**
     * @notice withdraw CML tokens applied during unstaking
     * @param amount number of CML to withdraw
     */
...
interface IPoolFactory {
    /**
     * @return CML treasury address
     */
    function treasury() external view returns (address);

    /**
     * @return CML spending fee
     */
...
abstract contract IRewardModule is OwnerController, IEvents {
    ...
    /**
     * @return CML usage ratio for reward module
     */
     ...
     * @return amount of cml spent
     * @return amount of cml vested
     */
    function stake(
        address account,
        address user,
