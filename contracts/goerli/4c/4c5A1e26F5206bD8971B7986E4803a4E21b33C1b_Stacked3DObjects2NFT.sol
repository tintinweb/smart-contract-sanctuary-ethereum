...
contract Stacked3DObjects2NFT is ERC721 {
    ...
    function tokenURI(
        ...
        IScriptyBuilder scriptBuilder = IScriptyBuilder(scriptyBuilderAddress);

        uint256 bufferSize = scriptBuilder.getBufferSizeForHTMLWrapped(requests);

        bytes memory encodedHTML = scriptBuilder.getEncodedHTMLWrapped(requests, bufferSize);
