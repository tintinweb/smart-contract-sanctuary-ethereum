...
contract APRRewardController is BaseRewardController, IAPRRewardController {
    ...
    function claimRewards(address account) external onlyRole(POOL_ROLE) returns (uint reward) {
    ...
    function getDailyRewards(address account) external view returns (uint reward) {
        UserInfo memory user = users[account];
        reward = interestRate * user.amount * 1 days / 365 days;
    }

    function _getDeltaTime(UserInfo memory _user) internal view returns (uint _deltaTime) {
        uint latestCheckPoint = getUserLatestCheckPoint(_user.stakedTime);
        uint endTime = latestCheckPoint > block.timestamp ? block.timestamp : latestCheckPoint;
        uint rewardTime = endTime < disableTime ? endTime : disableTime;
        _deltaTime = rewardTime - _user.lastRewardTime;
    }
