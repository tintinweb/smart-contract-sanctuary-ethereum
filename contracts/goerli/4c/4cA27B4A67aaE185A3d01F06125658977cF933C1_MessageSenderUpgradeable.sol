...
contract MessageSenderUpgradeable is IMessageSender, OwnableUpgradeable {
	...
	event SenderUpdated(address sender);
	...
	modifier onlySender() {
		require(msg.sender == sender, 'MessageSenderUpgradeable: caller is not a sender');
		_;
	}
	...
	) external payable onlySender returns (bytes32) {
	...
	function _setSender(address _sender) internal {
		sender = _sender;
		emit SenderUpdated(_sender);
	}
