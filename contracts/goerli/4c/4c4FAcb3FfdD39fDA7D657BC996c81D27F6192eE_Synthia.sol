...
contract Synthia is Ownable, ERC721Enumerable {
  ...
  uint16 public publicClaimed; // public + WL
  ...
  string public constant hashKey = "synthia-drp";
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    if (!isBlank(uri[uint16(_tokenId)])) {
  ...
  function whitelistMint(
    ...
    require(block.timestamp >= whitelistSalesStart, "Whitelist sales inactive");
    require(block.timestamp <= publicSalesStart, "Whitelist sales ended");
    require(publicClaimed < PUBLIC_SUPPLY, "Supply exhausted");
    require(publicClaimed + _amount <= PUBLIC_SUPPLY, "Amount exceeds public supply");
    require(whitelistAmounts[msg.sender] > 0, "Not whitelisted");
    require(nClaimed[msg.sender] < whitelistAmounts[msg.sender], "Whitelist amount exceeded");
    require(nClaimed[msg.sender] < MAX_PER_WALLET, "Already claimed max");
    require(_verifySignature(msg.sender, _amount, MAX_PER_WALLET, _signature), "Invalid signature");
  ...
  function mint(
    ...
    require(tx.origin == msg.sender, "Not allowed");
    require(block.timestamp >= publicSalesStart, "Public sales inactive");
    require(publicClaimed < PUBLIC_SUPPLY, "Supply exhausted");
    require(publicClaimed + _amount <= PUBLIC_SUPPLY, "Amount exceeds public supply");
    require(nClaimed[msg.sender] < MAX_PER_WALLET, "Already claimed max");
    require(_verifySignature(msg.sender, _amount, MAX_PER_WALLET, _signature), "Invalid signature");
  ...
  function teamClaim() external onlyOwner {
    ...
    for (uint16 i = PUBLIC_SUPPLY; i < MAX_SUPPLY; i++) {
  ...
  // setters
  // set WL amounts
  function setWhitelistedAddresses(address[] calldata _addresses, uint16[] calldata _amounts) external onlyOwner {
    require(_addresses.length == _amounts.length, "Amount mismatch");
    for (uint16 i = 0; i < _addresses.length; i++) {
      whitelistAmounts[_addresses[i]] = _amounts[i];
    }
  }
  ...
  // signatures
  function _validSignature(bytes32 _message, bytes memory _signature) internal view returns (bool) {
    bytes32 ethSignedMessageHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", _message));
    (uint8 v, bytes32 r, bytes32 s) = _splitSignature(_signature);
    // console.log("ecrecover", ecrecover(ethSignedMessageHash, v, r, s), "verifier", verifier);
  ...
  function _verifySignature(
    ...
    bytes32 message = keccak256(abi.encodePacked(hashKey, _amount, _maxAmount, msg.sender));
    return _validSignature(message, _signature) && nClaimed[_account] + _amount <= _maxAmount;
  }
