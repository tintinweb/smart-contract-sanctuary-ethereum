...
contract TankNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    ) ERC721("Fortress-Arena NFT", "FortressTank") Ownable() {
        _signer = signer_;
        _receiver = receiver_;
    ...
    function updateMaxPublicId(uint16 _newMax) external onlyOwner {
        require(
            ...
            "TankNFT: Can not set MAX_PUBLIC_ID lower than the current id"
        );
        require(
            _newMax <= MAX_COUNT,
            "TankNFT: Can not set more than MAX_COUNT"
        );
    ...
    function freeClaim(
        ...
        require(
            _publicTokenId + 1 <= MAX_PUBLIC_ID,
            "TankNFT: Can not mint more than MAX_PUBLIC_ID"
        );
    ...
    function preMint(
        ...
        require(
            _publicTokenId + _count <= MAX_PRE_ID,
            "TankNFT: Can not mint more than MAX_PRE_ID"
        );
        require(
            preMints[_msgSender()] + _count <= MAX_PRE_MULTI,
        ...
        nonReentrant
    {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (!_exists(tokenId)) {
                require(tokenId > 0);
                require(
                    tokenId <= FOUNDER_TANKS_COUNT || tokenId > MAX_PUBLIC_ID,
                    "TankNFT: id is out of range"
                );
        ...
        nonReentrant
    {
        require(
            ...
            "TankNFT: token count does not match recipient count"
        );
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            if (!_exists(tokenId)) {
    ...
    function getMintingState() external view returns (uint8) {
        if (state == State.Setup) {
            ...
            state == State.PublicMint && _publicTokenId < MAX_PUBLIC_ID
        ) {
            return 2;
        } else {
            return 3;
