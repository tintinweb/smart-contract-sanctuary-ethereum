...
contract SeraphStorage {
    ...
    /// @notice Address of the stage contract
    address public state_contract;
...
contract Seraph is SeraphStorage {
    ...
    modifier onlySeraphAdmin(){
        ...
        /**
     * @notice Modifier used to verify that the sender is the KMS administrative wallet or the state contract
     */
    modifier onlySeraphAdminorState(){
        require(msg.sender == admin || msg.sender==state_contract, "Seraph: Only Seraph KMS or state contract wallet allowed");
        _;
    }
     ...
     */
    function setClientProtected(bytes32 clientId, bool value) external onlySeraphAdminorState {
    ...
    function setFunctionProtected(address contractAddress, bytes4 functionSelector, bool value) external onlySeraphAdmin contractExists(contractAddress) functionExists(contractAddress, functionSelector) {
     ...
     /**
     * @notice It will change the address of Seraph state contract.
     *
     * @param newStateContract The contract address for the state contract
     */

    function setStateContract (address newStateContract) external onlyOwner {

         state_contract = newStateContract;
    }
