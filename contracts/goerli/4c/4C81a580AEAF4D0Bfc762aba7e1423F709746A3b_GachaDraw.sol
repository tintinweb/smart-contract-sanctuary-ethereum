...
contract GachaDraw is Ownable {
        ...
        validActivity(_activityId)
    {
        ...
        totalCompleteDraw++;
        uint256[] memory selectedTokenAmount = new uint256[](maxRewardTokenId);
        ...
        for (uint256 i = 0; i < _ticketAmount; i++) {
            ...
            for (uint256 j = 0; j < activity.rewardTokenIds.length; j++) {
                uint256 remaining = activity.maximumRewardSupply[j] 
                - activity.totalRewardSupply[j];
                indexCount += remaining;

                uint256 rewardTokenId = activity.rewardTokenIds[j];

                if (remaining > 0 && indexCount >= randomIndex) {
                    selectedTokenId = rewardTokenId;
                    activity.totalRewardSupply[j]++;
                    break;
                }
            }

            selectedTokenAmount[selectedTokenId - 1]++;
