...
contract DSCOLL is Context, AccessControlEnumerable, ERC1155BurnableX, ERC1155PausableX {
    ...
    function updateNonReselableTokens(uint256[] calldata tokenIds, bool[] calldata reselable) external onlyRole(DEFAULT_ADMIN_ROLE){
        require(tokenIds.length == reselable.length, "Nb addys doesn't match nb bools.");
        for (uint256 i; i < tokenIds.length; ++i) {
            _updateNonReselableTokens(tokenIds[i], reselable[i]);
        }
    }
...
contract ERC1155X is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function _updateNonReselableTokens(uint256 tokenId, bool reselable) internal virtual {
        non_resalable_tokens[tokenId]=reselable;
        emit NonReselableTokenUpdate(tokenId, reselable);
    }    
