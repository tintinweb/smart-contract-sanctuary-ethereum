...
contract CosmoGangItems is ERC1155Supply, Ownable {
    ...
    function canMintBatchNew(uint256[] calldata tokenIds)
        public view
        returns (bool)
    {
        uint256[] memory _tokenIdLastMint = tokenIdLastMint;
    ...
    // Mint Logic Batch
    // DO NOT SET role in the cosmo gang nft
    function _mintNFTBatch2New(address recipient, uint8 v, bytes32 r, bytes32 s, uint256[] calldata fromTokenIds, uint256 deadline, bool useProba)
        private
        returns (bool)
    {
        Faction faction = getFactionBatch(fromTokenIds, v, r, s, deadline);
        require(canMintBatchNew(fromTokenIds), "tokenId already minted his periodic Cosmo Gang Item");
        for (uint256 idx = 0; idx < fromTokenIds.length; idx++)
        {
            uint tokenId = fromTokenIds[idx];
            tokenIdLastMint[tokenId] = block.timestamp;
    ...
    // Normal Mint Bacth
    function mintNFTBatch2New(address recipient, uint8 v, bytes32 r, bytes32 s, uint256[] calldata fromTokenIds, uint256 deadline)
        public payable
        returns (bool)
    {
        uint256 nMint = fromTokenIds.length;
       ...
       return _mintNFTBatch2New(recipient, v, r, s, fromTokenIds, deadline, true);
    }
