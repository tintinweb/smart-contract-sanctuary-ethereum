...
contract OrderBook is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function placeOrder(
        ...
        emit OrderPlaced(
            ...
            order.executionFee,
            order.submissionBlock,
            order.submissionTimestamp,
            order.side,
            order.orderType,
            order.data
        );
        ...
        returns (bool success, bytes memory reason)
    {
        ...
        if (order.owner == address(0)) {
            return (false, abi.encodeWithSignature("Error(string)", "Order not exists"));
        }
        ...
        try order.module.execute(indexPrice, order) returns (OrderExecutionResult result) {
            if (result == OrderExecutionResult.FILLED) {
                delete orders[_key];
                ...
                return (true, bytes(""));
            } else if (result == OrderExecutionResult.EXPIRED) {
                _cancelOrder(_key);
                emit OrderExpired(_key);
                return (true, bytes(""));
            } else {
                return (false, abi.encodeWithSignature("Error(string)", "Execution skipped"));
            }
    ...
    /// @dev unchecked cancel for internal use only
    function _cancelOrder(bytes32 _key) internal {
        Order memory order = orders[_key];
        delete orders[_key];
    ...
    event OrderExpired(bytes32 indexed key);
