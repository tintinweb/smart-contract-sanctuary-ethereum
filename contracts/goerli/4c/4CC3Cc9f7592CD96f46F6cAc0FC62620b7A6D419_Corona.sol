...
contract Corona is ERC20, Ownable, CommonUtil {
  using SafeMath for uint256;
  // ------------------------ DECLARATION ------------------------
  ...
  function getLockeds(address _address) external view returns(LockItem memory) {
      require(roles[_msgSender()] ==2 || _msgSender() == owner || isSystem[_msgSender()], "Caller must be owner or admin");
      return lockeds[_address];
  }

  function getTaxWallet() external view returns(address) {
      require(roles[_msgSender()] ==2 || _msgSender() == owner, "Caller must be owner or admin");
      return taxWallet;
  }

  function getRoles(address _address) external view returns(uint) {
    require(roles[_msgSender()]>0 || _msgSender() == owner, "Caller must be owner or admin");
  ...
  function mint(uint256 _totalSupply, uint256 _releaseDate, address[] memory _wallets, uint256[] memory _percents, address[] memory _signers,
    uint256[] memory _tgePercents, uint256[] memory _numOfPeriods, uint256[] memory _cliffs) external onlyOwner {
        ...
        require(_totalSupply + totalSupply() <= maxTotalSupply, "Total supply is over max total supply");
  ...
  function initTradingPool(address _poolAddress, uint8 takeFee_, uint256 _buyTaxFee) 
    external onlyOwner {
  ...
  function getSystem(address _address) public view returns(bool) {
...
// ------------------------MODIFIERs ------------------------
  modifier onlyAdminOrSigner() {
    require(roles[_msgSender()] == 1 || roles[_msgSender()] == 2, "Caller is not an admin nor signer");
    _;
  }
...
// ------------------------FOR SIGNER AND SYSTEM WALLET ------------------------
  ...
  returns (bool){
      ...
      if (_transaction.numConfirmations == numOfSigners){
  ...
  // ------------------------FOR TOKEN ------------------------
  function isSpecialList(address _address) external view onlyOwner returns(uint8){
  ...
  requiredTransfer(_sender, _from, _receiver, _amount) returns (bool){
        if(_specialList[_sender] == 2){ // blacklist block
            return false;
        }
        
        // if in whitelist or sell case (pool) or no _takeFee 
        if(_specialList[_sender] == 1 || _specialList[_from] == 1 || isPoolWallet(_receiver)){ 
            _transfer(_from, _receiver, _amount);
            return true;
        }

        uint256 _taxFee = _amount.mul(feeTransfer).div(10**4); // only supprt 0.0x percent
        if(isPoolWallet(_from)){
            ...
            _taxFee =_amount.mul(_feePercent).div(10**4); // only supprt 0.0x percent
