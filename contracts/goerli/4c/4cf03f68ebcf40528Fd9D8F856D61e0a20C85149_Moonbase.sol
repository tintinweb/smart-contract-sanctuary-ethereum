...
//address
    ...
    address public lunaAddress = 0xA2157Bb4b1ddfE115307212ceA706f2F89cFc839;
 //bool
 ...
 //uint
    ...
    uint256 public maxWalletAmount;// 10% total supply (constructor)
 ...
 //event
    ...
    constructor() ERC20("Moonbase", "MNBASE") {
        ...
        */
        ...
        maxSellTxAmount =  _total_supply / 20; // 5%
        maxBuyTxAmount =  _total_supply / 20; // 5%
        maxWalletAmount =  _total_supply / 10; // 10%
    ...
    function setSwapAndLiquify(bool _state, uint _intervalSecondsForSwap, uint _minimumTokensBeforeSwap, uint _tokensToSwap) external onlyOwner {
        ...
        require(tokensToSwap <= totalSupply() / 20,"Token to swap limited to 5% supply");
    }
    ...
    function setMaxTxAmount(uint _buy, uint _sell) external onlyOwner {
        ...
        require(maxBuyTxAmount >= totalSupply() / 20,"maxBuyTxAmount should be at least 5% of total supply.");
        require(maxSellTxAmount >= totalSupply() / 20,"maxSellTxAmount should be at least 5% of total supply.");
    }
    ...
    function swapTokens(uint256 minTknBfSwap) private {
        ...
        if(developerSwapSendActive && developerSellFee > 0) {
            ...
            if(success) {emit MarketingFeeCollected(developerEth);}
        ...
        if(marketingSwapSendActive  && WLUNABurnSellFee > 0) {
            ...
            if(success) {emit LunaBurnFeeCollected(marketingEth);}
  	...
  	function updateDeveloperWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != developerWallet, "The developer wallet is already this address");
    ...
    function updateLiqWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != liqWallet, "The liquidity Wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function update_lunaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(_lunaDividendTracker), "The dividend tracker already has that address");
        _LUNADividendTracker new_lunaDividendTracker = _LUNADividendTracker(payable(newAddress));
        require(new_lunaDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the Luna token contract");
