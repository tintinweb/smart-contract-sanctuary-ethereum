...
contract SwapAdapterV3 is SwapAdapterBase {
    constructor(address routerAddress) SwapAdapterBase(routerAddress) {}
...
abstract contract SwapAdapterBase is ISwap {
...
library SwapLibV3 {
    ...
    function _swapExactTokensForTokensV3(
        ...
        if (tokenIn != IUniswapV3Router(router).WETH9()) {
        ...
        amountOut = IUniswapV3Router(router).exactInputSingle{value: msg.value}(
            ISwapRouter.ExactInputSingleParams(
                ...
                fee,
                recipient,
                SwapDeadlineLib.getRelativeSwapDeadlineTimeStamp(),
                amountIn,
                amountOutMin,
                0
            )
...
/// @title SwapDeadlineLib.
library SwapDeadlineLib {
    uint internal constant SWAP_DEADLINE = 10 seconds;

    /// @return swapDeadline - relative time of swap's ending.
    function getRelativeSwapDeadlineTimeStamp()
        internal
        view
        returns (uint swapDeadline)
    {
        swapDeadline = block.timestamp + SWAP_DEADLINE;
    }
}
