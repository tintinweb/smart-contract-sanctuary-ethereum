...
contract ANT {
    ...
    function initialize(
        ...
        initialized = true;
        nonce = 0;
        matingDates = [1657486800, 1658091600, 1658696400, 1659301200];
    }
    ...
    function feedLarva(uint256 _larvaAmount) public {
        ...
        for (uint256 i = 0; i < _larvaAmount; i++) {
            if (larva.idToFed(larvaeList[i]) == false) {
    ...
    function hatch(uint256 _amount) public {
        ...
        console.log("Larva addr in ANT:", address(larva));
        ...
        returns (uint256 _homelessAntCount)
    {
        ...
        uint256 _homelessMaleCount = worker.getHomelessCount(_user);
    ...
    function layEggs(uint256 _index) public {
        require(queen.idToFertility(_index) == true, "Queen not fertile.");
        ...
        uint256 _epoch = queen.getEpoch(_index);
        uint256 _level = queen.idToLevel(_index);
        if (_level == 1 && _epoch > 5) {
            queen.setFertility(_index, false);
        } else if (_level == 2 && _epoch > 9) {
            queen.setFertility(_index, false);
        } else if (_level == 3 && _epoch > 12) {
            queen.setFertility(_index, false);
        }
    }

    function feedQueen(uint256 _index) public {
        uint256 timeElapsed = block.timestamp - queen.idToTimestamp(_index);
        uint256 epochsElapsed = timeElapsed / 360; // test value
        uint256 _level = queen.idToLevel(_index);
        if (_level == 1) {
            epochsElapsed = epochsElapsed > 5 ? 5 : epochsElapsed;
        } else if (_level == 2) {
            epochsElapsed = epochsElapsed > 9 ? 9 : epochsElapsed;
        } else {
            epochsElapsed = epochsElapsed > 12 ? 12 : epochsElapsed;
        }

        uint256 _amount = epochsElapsed * queen.PORTION_FEE() * 1e18;
        funghi.transferFrom(msg.sender, address(this), _amount);
        queen.setTimestamp(_index, block.timestamp);
        queen.setFertility(_index, true);
    ...
    function increaseCapacity(address _user) public {
        playerToAvailableSpace[_user] += 10;
    }
    ...
    function claimAndIncreaseSpace(uint256 _missionId) public {
        uint256[] memory _workerList = worker.getWorkers(msg.sender);
        uint256 _speed = getSpeed(msg.sender);
        uint256 _now = block.timestamp;
        for (uint256 i; i < _workerList.length; i++) {
            if (
                worker.idToOnBuildMission(_workerList[i]) &&
                worker.BUILD_DURATION() / _speed <=
                _now - worker.idToBuildDate(_workerList[i])
            ) {
        ...
        worker.finalizeMission(msg.sender, _missionId);
    }
    ...
    function revealTarget(uint256 _missionId) public {
        uint256 _end = soldier.getMissionEnd(msg.sender, _missionId);
        require(_end < block.timestamp, "Mission is not over yet.");
        uint256 prob = uint256(keccak256(abi.encodePacked(msg.sender, nonce))) %
            participants.length;
        address _target = participants[prob];
        uint256 targetSoldierCount = soldier
            .getAvailableSoldiers(_target)
            .length;
        uint256[] memory targetLarvae = larva.getLarvae(_target);
        uint256 attackerSoldierCount = soldier.getMissionPartipants(
            msg.sender,
            _missionId
        );
        uint256 prize = soldier.battle(
            attackerSoldierCount,
            targetSoldierCount
        );
        require(prize > 0, "Battle lost.");
        uint256[] memory larvaToBeBurnt = new uint256[](prize);
        uint256 larvaeAdded = 0;
        for (uint256 i = 0; i < prize; i++) {
            larvaToBeBurnt[larvaeAdded] = targetLarvae[i];
        }
        claimStolenLarvae(_missionId, _target, larvaToBeBurnt);
    }

    function claimStolenLarvae(
        uint256 _missionId,
        address _target,
        uint256[] memory larvaToBeBurnt
    ) public {
        uint256[] memory _soldierList = soldier.getSoldiers(msg.sender);
        uint256 _speed = getSpeed(msg.sender);
        for (uint256 i; i < _soldierList.length; i++) {
            if (
                soldier.idToOnRaidMission(_soldierList[i]) &&
                soldier.RAID_DURATION() / _speed <=
                block.timestamp - soldier.idToRaidDate(_soldierList[i])
            ) {
        ...
        larva.mint(msg.sender, larvaToBeBurnt.length);
        for (uint256 i = 0; i < larvaToBeBurnt.length; i++) {
            larva.burn(_target, larvaToBeBurnt[i]);
        }
    }
    ...
    function healSoldier(uint256 _soldierAmount) public {
        ...
        uint256[] memory _infectedSoldierList = soldier.getInfectedSoldiers(
            msg.sender
        );
        require(
        ...
        if (_healedSoldiers < _soldierAmount) {
            for (uint256 i = 0; i < _soldierList.length; i++) {
                uint256 _soldierDamage = soldier.idToDamageCount(
                    _soldierList[i]
                );
                if (_soldierDamage == 2) {
                    funghi.transferFrom(
    ...
    function claimQueen(uint256 _amount) public {
        ...
        for (uint256 week = 0; week < 4; week++) {
            if (
                _now >= matingDates[week] &&
                _now <= (matingDates[week] + 172800)
            ) {
                seasonBonus = 60;
            } else {
                seasonBonus = 0;
        ...
        uint256 speed = getSpeed(msg.sender);
        for (uint256 i; i < _amount; i++) {
            uint256 _timeElapsed = _now -
                princess.idToMateTime(_matedPrincesses[i]);
            if (_timeElapsed > princess.MATE_DURATION() / speed) {
