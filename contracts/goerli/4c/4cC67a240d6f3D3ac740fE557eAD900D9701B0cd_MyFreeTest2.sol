...
contract MyFreeTest2 is ERC721AQueryable, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    if(maxFreeMintAmount > 0 && freeMintClaimed[msg.sender] < maxFreeMintAmount) {
      if(_mintAmount <= maxFreeMintAmount - freeMintClaimed[msg.sender]) {
        require(msg.value >= cost * 0, 'Insufficient funds!');
        freeMintClaimed[msg.sender] += _mintAmount;
      } else {
        require(msg.value >= cost * (_mintAmount-(maxFreeMintAmount - freeMintClaimed[msg.sender])), 'Insufficient funds!');
        freeMintClaimed[msg.sender] = maxFreeMintAmount;
      }
      _;
    } else {
