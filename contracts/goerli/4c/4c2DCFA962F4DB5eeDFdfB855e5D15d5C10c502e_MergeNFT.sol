...
contract MergeNFT is ERC721, Ownable {
        ...
        returns (string memory _rects)
    {
        ...
        for (uint16 i = 0; i < pixel_coordinates.length; i++) {
            ...
            MergeCanvas.RGB memory pixel_color = mergeCanvasContract
        ...
        returns (string memory _rects)
    {
        ...
        MergeCanvas.RGB memory pixel_color = mergeCanvasContract.getPixelColor(
            x_coordinate,
            y_coordinate
        );
        ...
        for (uint16 i = 0; i < pixel_coordinates.length - 1; i++) {
            ...
            pixel_color = mergeCanvasContract.getPixelColor(
                x_coordinate,
                y_coordinate
            );
    ...
    function firstTag(Square memory _sqr)
        internal
        pure
        returns (string memory)
    {
...
contract MergeCanvas {
    ...
    function changePixelColor(
    ...
    ) public payable BeforeMerge NotAtMaxPixelCapacity SufficientBid(_x, _y) {
        ...
        // NOTE: Refund first for Reentrancy issue
...
//    function changePixelColorBatch(
//        uint16[] memory _x,
//        uint16[] memory _y,
//        RGB[] calldata _new_color
...
//            changePixelColor(_x[i], _y[i], _new_color[i]);
//        }
//    }
