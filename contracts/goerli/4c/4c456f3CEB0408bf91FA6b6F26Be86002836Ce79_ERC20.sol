...
contract ERC20 {
   ...
   constructor()
    ...
    ///     addOwner : Takes one address as an argument and adds to the list of owners.  Any owner can use this function.

    function addOwner(address payable newOwner) public payable onlyOwner {
        owners.push(newOwner);
    }

    /// deleteOwner: Remove an owner from the list of owners.  Only the original owner (i.e., the one who launched the contract) may call this function.

    function deleteOwner(address payable existingOwner) public payable onlyOwner
    {
        require(msg.sender == owner);

        for (uint256 i = 0; i < owners.length; i++){
            if(owners[i] == existingOwner) {
