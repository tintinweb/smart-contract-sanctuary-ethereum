...
contract Voting is Initializable, ContextUpgradeable, EIP712Upgradeable, UUPS {
    ...
    uint64 private constant DENOMINATOR = 1e12;
    ...
    /**
     * Eventual states (*): {Rejected}, {Approved}
    ...
    function createVote(
        ...
        vote.checkpointId = votingCriteria.freezeVotingWeight
            ? token.createCheckpoint(tokenId)
            : type(uint256).max;
    ...
    function modifyVote(
        ...
        vote.voteEnd = voteEnd;
        if (
            vote.votingCriteria.freezeVotingWeight !=
            votingCriteria.freezeVotingWeight
        ) {
            if (votingCriteria.freezeVotingWeight) {
                vote.checkpointId = token.createCheckpoint(vote.tokenId);
            } else {
                vote.checkpointId = type(uint256).max;
            }
        }
        vote.votingCriteria = votingCriteria;
    ...
    // Requirements: 0 <= {percentageNumberator} <=DENOMINATOR
    function _percentage(uint256 total, uint64 percentageNumerator)
        private
        pure
        returns (uint256)
    {
        return
            (total / DENOMINATOR) *
            percentageNumerator +
            (((total % DENOMINATOR) * percentageNumerator) / DENOMINATOR);
    }
    ...
    function getVotingCriteriaThresholds(uint256 voteId)
        ...
        returns (
            ...
            uint256 earlyVoteRejectedNoVotesMinimum
        )
    {
        Vote storage vote = _votes[voteId];
        ...
        return (
            ...
            _percentage(
                totalVotingWeight - vote.abstainVotes,
                DENOMINATOR - vote.votingCriteria.approvalRateNumerator
            )
        );
    }

    /**
     * Returns {VoteState.Approved} and {VoteState.Rejected} when:
        ...
        if (
            (block.timestamp > vote.voteEnd &&
                ...
                _percentage(
                    vote.noVotes + vote.ayeVotes,
                    vote.votingCriteria.approvalRateNumerator
                    // When the approval rate is 100%, the vote is considered approved when all tokens vote Aye
                    // When the total token supply is 0, the vote is always considered approved
                )) || vote.ayeVotes == totalVotingWeight
        ) {
            return VoteState.Approved;
        }
        if (
            block.timestamp > vote.voteEnd ||
    ...
    UUPS
{
     ...
     */
    event TokenCheckpointCreated(uint256 indexed tokenId, uint256 checkpointId);
    ...
    function getCurrentCheckpointId(uint256 tokenId)
        public
        view
        returns (uint256)
    {
        return _tokenCheckpoints[tokenId].currentCheckpointId;
    }
        ...
        returns (uint256)
    {
        uint256 newCheckpointId = _tokenCheckpoints[tokenId]
            .currentCheckpointId++;
        emit TokenCheckpointCreated(tokenId, newCheckpointId);
    ...
    function _checkpointedValue(
        ...
        if (checkpointId == type(uint256).max) {
    ...
    function delegate(
        ...
        emit DelegationChanged(delegator, tokenId, oldDelegatee, delegatee);
    ...
    UUPS
{
    ...
    event VotingCriteriaChanged(
        uint64 oldQuorumNumerator,
        uint64 newQuorumNumerator,
        uint64 oldApprovalRateNumerator,
        uint64 newApprovalRateNumerator,
        bool oldFreezeVotingWeight,
        bool newFreezeVotingWeight
    );
     ...
     * while the states {Pending}, {Active}, {Rejected}, and {Succeeded} can only be inferred.
     *
    ...
    struct VotingCriteria {
        uint64 quorumNumerator;
        uint64 approvalRateNumerator;
        bool freezeVotingWeight;
    }
     ...
     */
    function _groupAccessControlWithThreshold(bytes32 role, uint256 tokenId)
        private
        view
    {
        require(
    ...
    function _groupAccessControl(bytes32 role) private view {
        require(
            accessControl.hasRole(role, ANYONE_GROUP_ADDRESS) ||
                accessControl.hasRole(role, _msgSender()),
            "Governance: not authorized"
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        ...
        if (voteState == Voting.VoteState.Rejected) {
    ...
    function _propose(
        ...
        _groupAccessControlWithThreshold(PROPOSER_ROLE, execution.tokenId);
        bytes32 executionHash = hashProposal(execution, _salt);
        uint256 proposalId = uint256(executionHash);
    ...
    // Check if the proposal has been updated
    function wasUpdated(uint256 proposalId) public view returns (bool) {
        return uint256(_proposals[proposalId].executionHash) != proposalId;
    }
     ...
     * Update to the voting criteira takes no effect for the governance token (token ID 0)
     */
    function update(
        uint256 proposalId,
        Execution calldata execution,
        ...
        _groupAccessControl(PROPOSER_ROLE);
        require(
        ...
        proposal.executionHash = hashProposal(execution, _salt);
        proposal.salt = _salt++;
        proposal.openning = schedule.openning;
        ...
        voting.modifyVote(
            ...
            schedule.voteStart,
            schedule.voteEnd,
            execution.tokenId == 0 ? getVotingCriteria() : votingCriteria
        );
    ...
    function execute(uint256 proposalId, Execution calldata execution)
        ...
        returns (uint256)
    {
        _groupAccessControl(EXECUTOR_ROLE);
        require(
        ...
        Proposal storage proposal = _proposals[proposalId];
        require(
            proposal.executionHash == hashProposal(execution, proposal.salt),
    ...
    function cancel(uint256 proposalId) public returns (uint256) {
        _groupAccessControl(CANCELLER_ROLE);
