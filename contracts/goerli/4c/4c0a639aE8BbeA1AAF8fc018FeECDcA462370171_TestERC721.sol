...
contract TestERC721 is ERC721A, OpenSeaDefaultOperatorFilterer, Ownable, Paid {
    modifier onlyOwnerOrAllowedOperator() {
        address __msgSender = _msgSenderERC721A();
    ...
    function _baseURI() internal view override returns (string memory) {
        return "ipfs://bafybeie5zkvb77u63zfxlxtdeu4gkqdzywbeb277zuunzxu6slt2jqhj3i/";
    }

    constructor()
        ERC721A("NFT EsTest", "NFTEsT")
    {
    ...
    function mint() external payable {
        if (msg.value >= OPEN_MINT_PRICE) {
            _mint(msg.sender, msg.value / OPEN_MINT_PRICE);
        }
    }

    function mintTo(address to_, uint quantity_) 
        external payable onlyPaid(OPEN_MINT_PRICE * quantity_) 
    {
        _mint(to_, quantity_);
    }
