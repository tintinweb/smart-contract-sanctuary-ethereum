        ...
        )
    {
        ...
        for (uint256 i; i < outLength; i++) {
            ...
            if (address(0) == tempBalanceOut[i].token) {
                tokenName = "ETH";
                tempBalanceOut[i].amount = tempBalanceOut[i].amount / 1 ether;
            } else {
        ...
        for (uint256 i; i < inLength; i++) {
            ...
            if (address(0) == tempBalanceIn[i].token) {
                tokenName = "ETH";
                tempBalanceIn[i].amount = tempBalanceIn[i].amount / 1 ether;
            } else {
                try IERCToken(tempBalanceIn[i].token).name() {
                    tokenName = IERCToken(tempBalanceIn[i].token).name();
                } catch {
                    tokenName = "Unknown";
                }
            }
