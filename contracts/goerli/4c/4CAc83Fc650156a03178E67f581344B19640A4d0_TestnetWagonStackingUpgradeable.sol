...
contract TestnetWagonStackingUpgradeable is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, UUPSUpgradeable  {
    ...
    event Stake(uint256 indexed period, address indexed staker, uint256 amount);
    event Unstake(uint256 indexed period, address indexed staker, uint256 amount);
    event DistributeProfitSharing(uint256 indexed period, uint256 amount);
    ...
    event RemoveAutoCompound(address indexed staker, uint indexed period, uint256 amount);
    ...
    modifier isAbleToUncompound() {
        ...
        uint256 currentLockTime = getCurrentPeriod();
        uint256 nextLockTime = BokkyPooBahsDateTimeLibrary.addSeconds(firstLockTime, currentLockTime * lockDuration);
    ...
    function updateUserLatestLock() private {
        ...
        userLatestLocks[msg.sender].totalLocked = getCurrentTotalUserLocked(msg.sender);
    ...
    function getCurrentTotalUserStake(address _address) public view returns(uint256) {
        uint256 currentPeriod = getCurrentPeriod();
        uint256 totalUserStaked = stakings[currentPeriod][_address];
    ...
    // Get total user current locked WAG.
    function getCurrentTotalUserLocked(address _address) public view returns(uint256) {
        uint256 lockedPeriod = getCurrentLockedPeriod();
        uint256 totalUserLocked = getTotalUserPeriodLocked(lockedPeriod, _address);
    ...
    function getTotalUserPeriodLocked(uint256 _lockedPeriod, address _address) public view returns(uint256) {
        if(_lockedPeriod == 0) return 0;
        if(_lockedPeriod == 1) return stakings[1][_address];

        userLatestLock memory userLock = userLatestLocks[_address];
        ...
        if(latestLockedPeriod > 0 && latestLockedPeriod <= _lockedPeriod) {
            totalUserLocked += calculateReward(latestLockedPeriod, totalUserLocked);
            totalUserLocked -= withdraws[latestLockedPeriod][_address];
        }

        uint256 i = latestLockedPeriod + 1;
        for(i; i <= _lockedPeriod; i++) {
            totalUserLocked += stakings[i][_address];
            if (i == _lockedPeriod) continue;
            totalUserLocked += calculateReward(i, totalUserLocked);
            totalUserLocked -= withdraws[i][_address];
        }
    ...
    function getTotalUserReward(address _address) public view returns(uint256) {
        ...
        for(i; i <= currentLockedPeriod; i++) {
            ...
            reward = calculateReward(i, totalUserLocked);
    ...
    function getUserReward(address _address) public view returns(uint256) {
        ...
        uint256 userReward = getUserPeriodReward(period, _address);
    ...
    function getUserPeriodReward(uint256 _period, address _address) public view returns(uint256) {
        if(_period == 0) return 0;

        uint256 totalUserLocked = getTotalUserPeriodLocked(_period, _address);
        uint256 totalStaked = getTotalStaked(_period);
    ...
    function getTotalUserWithdrawable(address _address) public view returns(uint256 totalWithdrawable) {
        uint256 lockedPeriod = getCurrentLockedPeriod();
        if(lockedPeriod == 0) return 0;

        totalWithdrawable = getCurrentTotalUserLocked(_address);
        totalWithdrawable += calculateReward(lockedPeriod, totalWithdrawable);
        totalWithdrawable -= withdraws[lockedPeriod][_address];
    ...
    function removeAutoCompound(uint256 _amount) public isAbleToUncompound nonReentrant{
        ...
        uint256 totalUserWithdrawable = getTotalUserWithdrawable(msg.sender);
