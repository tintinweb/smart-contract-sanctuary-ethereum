...
// File: contracts/y00tsMagnumWrapper.sol
...
contract y00tsAnchorClubWrapper is IERC721Receiver {

    mapping(address => mapping(uint256 => bool)) public owners;

    YachtClub YClub;
    y00tsAnchorClub AClub;

    constructor(YachtClub _YClub, y00tsAnchorClub _AClub) {
    ...
    function y00t(uint256[] memory tokenIds) public {
    ...
    function uny00t(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i< tokenIds.length; i++){
            uint256 tokenId = tokenIds[i];
            require(AClub.unwrapActive(), "unWrap not y00tin");
            require(AClub.ownerOf(tokenId) == _msgSenderERC721A(), "y00ts don't own this");
