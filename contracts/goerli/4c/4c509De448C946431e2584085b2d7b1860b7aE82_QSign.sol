...
contract QSign is Sign, IQSign {
    ...
    function chainIdConfig(bytes32 walletTypeId, uint256 chainId, bool support) external virtual override onlyRole(DEFAULT_ADMIN_ROLE) walletTypeGuard(walletTypeId) {
        if(support){
        ...
        emit ChainIdSupport(walletTypeId, chainId, support);
    }
...
interface ISign is IAccessControl {
    ...
    function isChainIdSupported(bytes32 walletTypeId, uint256 chainId)
        external
        view
        returns (bool);
    ...
    ISign
{
    ...
    mapping(bytes32 => mapping(uint256 => bool)) internal supportedChainIds; //0 for chains without chainIds
    ...
    modifier onlyOvm {
        require(
            hasRole(OVM_ROLE, _msgSender()) == true,
            "qs::onlyOvm:caller not authorized"
        );
        _;
    }
    ...
    modifier chainIdGuard(bytes32 walletTypeId, uint256 chainId) {
        require(
            supportedChainIds[walletTypeId][chainId] == true,
    ...
    {
        if(broadcast){
            require(isHash == false, "qs::requestSignature:cannot be broadcasted");
        }
        _requestSignature(
            walletTypeId,
            _msgSender(),
    ...
    function _broadcast(
        ...
        uint256 publicKeyIndex,
        bytes memory payload
    ) internal virtual chainIdGuard(walletTypeId, dstChainId) {
        emit Broadcast(
            walletTypeId,
            dstChainId,
            owner,
...
abstract contract AccessControl is Context, IAccessControl, ERC165 {
    ...
    function grandRoleAdmin(bytes32 role, bytes32 adminRole)
        public
        virtual
