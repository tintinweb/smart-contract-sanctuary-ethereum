...
contract Exchange is ERC20 {
    ...
    function getPrice(uint256 inputReserve, uint256 outputReserve) public pure returns (uint256) {
        ...
        uint256 denumerator = outputReserve;
    ...
    function getOutputAmount(uint256 inputAmount, uint256 inputReserve, uint256 outputReserve) public pure returns (uint256) {
        ...
        uint256 denumerator = inputReserve + inputAmount;
        return numerator / denumerator;
    } 
      ...
      function getOutputAmountWithFee(uint256 inputAmount, uint256 inputReserve, uint256 outputReserve) public pure returns (uint256) {
        ...
        // ERC20 -> ETH
    function etherToTokenSwap(uint256 _mintTokens) public payable {
       
        // calculate amount out (zero fee)
        uint256 outputAmount = getOutputAmountWithFee(msg.value, address(this).balance - msg.value, token.balanceOf(address(this)) );

        require(outputAmount >= _mintTokens , "Insufficient outputamount");
    ...
    function tokenToTokenSwap(uint256 _tokenSold, uint256 _minTokenBought, uint256 _minEthBought, address _tokenAddress) public {
        ...
        // calculate amount out (zero fee)
        uint256 ethOutputtAmount = getOutputAmountWithFee(_tokenSold, token.balanceOf(address(this)), address(this).balance);

        require(ethOutputtAmount >=_minEthBought, "Insufficient outputamount");
        ...
        //새로운 인터페이스
        IExchange(toTokenExchangeAddress).ethToTokenTransfer{ value: ethOutputtAmount}(_minTokenBought, msg.sender);
...
interface IExchange {
    ...
    function etherToTokenSwap(uint256 _mintTokens) external payable;
