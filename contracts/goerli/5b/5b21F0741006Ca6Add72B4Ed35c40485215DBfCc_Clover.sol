...
contract Clover is VRFConsumerBaseV2 {
    ...
    function withdraw() external returns (bool) {
        ...
        if (stepToPlayer[step.ONE] == msg.sender) stepToPlayer[step.ONE] = address(0);
        if (stepToPlayer[step.TWO] == msg.sender) stepToPlayer[step.TWO] = address(0);
        if (stepToPlayer[step.THREE] == msg.sender) stepToPlayer[step.THREE] = address(0);
        if (stepToPlayer[step.FOUR] == msg.sender) stepToPlayer[step.FOUR] = address(0);
        if (stepToPlayer[step.FIVE] == msg.sender) stepToPlayer[step.FIVE] = address(0);
        if (stepToPlayer[step.SIX] == msg.sender) stepToPlayer[step.SIX] = address(0);
        if (stepToPlayer[step.SEVEN] == msg.sender) stepToPlayer[step.SEVEN] = address(0);
        playerToAmount[msg.sender] = 0;
        if (balance == 0) revert Clover__EmptyBalance();
        (bool success, ) = payable(msg.sender).call{value: balance}("");
        if (!success) revert Clover__WithdrawFailed();
