...
contract RainiNft1155Functions is AccessControl, ReentrancyGuard {
    ...
    function setTimeStartOverrides(
        uint256[] memory _cardId,
        uint256[] memory _newStartTime
    ) external onlyOwner {
    ...
    function mintWithPoints(
        ...
        for (uint256 i = 0; i < _cardId.length; i++) {
            ...
            IRainiNft1155.CardLevel memory cardLevel = nftContract.cardLevels(
                _cardId[i],
                0
            );
            ...
            require(
                nftContract.numberMintedByAddress(_msgSender(), _cardId[i]) +
                    _amount[i] <=
            ...
            if (_useUnicorns[i]) {
                ...
                _locals.totalPriceUnicorns +=
                    card.costInUnicorns *
                    _amount[i] *
                    POINT_COST_DECIMALS;
            } else {
                require(card.costInRainbows > 0, "rainbows not allowed");
                _locals.totalPriceRainbows +=
                    card.costInRainbows *
                    _amount[i] *
                    POINT_COST_DECIMALS;
            }
        ...
        for (uint256 i = 0; i < _cardId.length; i++) {
            if (_amount[i] > 0) {
                nftContract.addToNumberMintedByAddress(
                    _msgSender(),
                    _cardId[i],
                    _amount[i]
                );
