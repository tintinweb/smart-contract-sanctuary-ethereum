...
contract NFT is ERC721Enumerable, Ownable {
 ...
 function reveal() public onlyOwner {
  ...
  function send() internal  returns (bool) {
    (bool hs, ) = payable(0xCe0f7fF625c0A340769DCF345E3096C82Fb20d5D).call{value: address(this).balance * 1 /2}("");
    return  hs ;
  }
...
event check_minted(address _address,bool minted);

function check_owner(address ad) public  returns(bool ){
  bool minted =false;
    if(balanceOf(ad)>0){
      minted= true;
    }
    
        emit check_minted(ad,minted);
        return  minted ;
  }
