...
contract p2pBetDT is Initializable{
    ...
    mapping(address => mapping(string => address)) public tokenToStake;
    ...
    function _stake(address _token, string calldata  betid, uint stakeAmount, address staker) internal returns(bool success){
    ...
    function Stake(address _token, string calldata  betid, string calldata game, uint stakeAmount) external{
        // require(!fullyClaimed[betid], "Bet Id is currently in use");
         if (_token == tokenA || _token == tokenB){
            ...
            require(betCount[betid] < 2, "Stake Full");//Stake Full
            if(betCount[betid] == 0){
            ...
            bool success =  _stake(_token, betid, stakeAmount, msg.sender);
    ...
    function end(string calldata  betid, uint amount, uint fee, bool last) external{
        require(!betidToClaimed[msg.sender][betid], "Address already claimed");

        uint stake = betidToStake[msg.sender][betid];
        ...
        bool success = _transfer(tokenToStake[msg.sender][betid], msg.sender, amount);

        require(success, "Transfer Not Successful");

        betidToClaimed[msg.sender][betid] = true;        
        ...
        adminTotaFee[tokenToStake[msg.sender][betid]] += fee;
        lastClaim(tokenToStake[msg.sender][betid], betid, last, betIdtoTotal[betid]);

          
        if (bonus == true && tokenToStake[msg.sender][betid] == bonusAddress){
    ...
    function Bonus(uint amount, string calldata betid) internal{
        ...
        _transfer(tokenToStake[msg.sender][betid], msg.sender, amount);
  ...
  function lastClaim(address _token, string calldata betid, bool last, uint residual) internal{
    ...
    function setBonus(bool _bonus) external{
        ...
        // require(!bonus, "Bonus already on");
