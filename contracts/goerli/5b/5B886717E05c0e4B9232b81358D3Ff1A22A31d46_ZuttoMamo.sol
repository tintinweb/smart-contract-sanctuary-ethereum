...
interface IZuttoMamo is IERC721A, IERC721Lockable, IERC5192PLTop {
	// function getBirthDay(uint256 _tokenId) external view returns (uint256);

	// function getBirthDayTimestamp() external view returns (uint256);
...
interface IZuttoMamoStage {
	...
	function setHighSchoolerLock(uint256 _tokenIds, DataType.LockStatus _lockStatus) external;

	function setFamilyLock(uint256 _tokenIds, DataType.LockStatus _lockStatus) external;

	function setOldAgeLock(uint256 _tokenIds, DataType.LockStatus _lockStatus) external;

	function getGrowingUpToHighSchoolerTime() external view returns (uint256);

	function getGrowingUpToFamilyTime() external view returns (uint256);

	function getGrowingUpToOldAgeTime() external view returns (uint256);
}
...
library DataType {
	...
	struct StandardPeriod {
		uint256 highSchooler;
		uint256 workingAdult;
		uint256 marriage;
	}

	struct MarriageDayPeriod {
		uint256 family;
		uint256 oldAge;
		uint256 tomb;
	}
	...
	struct AllStageParams {
		...
		MarriageDayPeriod marriageDayPeriod;
	}
}
...
abstract contract ZuttoMamoConfig {
	...
	// uint256 public birthDayTimestamp;

	// unix 2024.2.10 12:00:00
	// uint256 public highSchoolerLockStageTimestamp = 1707534000;
	...
	IERC4906
{
	...
	// =============================================================
	//   EIP2981 ROYALTYOVERRIDE
	// =============================================================

	// function setTokenRoyalties(TokenRoyaltyConfig[] calldata royaltyConfigs) external override onlyRole(ADMIN_ROLE) {
	// 	_setTokenRoyalties(royaltyConfigs);
	// }

	// function setDefaultRoyalty(TokenRoyalty calldata royalty) external override onlyRole(ADMIN_ROLE) {
	...
	function _birthWithSleeping(address _to, uint256 _amount) private {
		...
		for (uint256 i = 0; i < _amount; i++) {
			...
			// birthDay[tokenId] = 0;
			// zuttoMamoStage.setLockStage(tokenId, DataType.LockStatus.UnLock);
		}
	}
	...
	// function setBirthDayTimestamp() external onlyRole(ADMIN_ROLE) {
	// 	birthDayTimestamp = block.timestamp;
	// }

	// function setBirthDayTimestamp(uint256 _value) external onlyRole(ADMIN_ROLE) {
	// 	birthDayTimestamp = _value;
	// }

	// function setLockStageTimestamp(uint256 _value) external onlyRole(ADMIN_ROLE) {
	// 	highSchoolerLockStageTimestamp = _value;
	// }
...
contract ZuttoMamo is ZuttoMamoAdmin, RevokableDefaultOperatorFilterer {
	...
	function _afterTokenTransfers(address _from, address _to, uint256 _tokenId, uint256 _quantity) internal override {
		if (tokenLocation[_tokenId] == DataType.TokenLocation.Operator) {
			// if (highSchoolerLockStageTimestamp <= block.timestamp) {
			if (zuttoMamoStage.getGrowingUpToHighSchoolerTime() <= block.timestamp) {
				zuttoMamoStage.setHighSchoolerLock(_tokenId, DataType.LockStatus.Lock);
			}
			// birthDay[_tokenId] = birthDayTimestamp;
