...
contract FjordLbpProxyV5 is FjordLbpAbstractV1 {
     ...
     */
    modifier onlyNFTPoolOwner(address pool) {
     ...
     */
    function setSwapEnabled(address pool, bool swapEnabled) external onlyNFTPoolOwner(pool) {
     ...
     */
    function transferPoolOwnership(address pool, address newOwner) external onlyNFTPoolOwner(pool) {
        require(blockListAddress != address(0), "no blocklist address set");
    ...
    function exitPool(
        ...
        bool isStandardFee
    ) external onlyNFTPoolOwner(pool) {
     ...
     */
    function createWeightedPoolForLBP(WeightedPoolConfig memory weightedPoolConfig)
        external
        onlyNFTPoolOwner(weightedPoolConfig.lbpAddress)
        returns (address)
    {
        // 1: deposit tokens and approve vault
        ...
        require(
            _poolData[weightedPoolConfig.lbpAddress].weightedPoolAddress == address(0),
            "Weighted pool already exists for this LBP"
        );
        ...
        // 2: pool creation
        address pool = WeightedPoolFactory(WeightedPoolFactoryAddress).create(
        ...
        // 3: store pool data
        _poolData[weightedPoolConfig.lbpAddress].weightedPoolAddress = pool;
        ...
        emit JoinedWeightedPool(pool, weightedPoolConfig.tokens, weightedPoolConfig.amounts, userData);
        return pool;
    }
}
