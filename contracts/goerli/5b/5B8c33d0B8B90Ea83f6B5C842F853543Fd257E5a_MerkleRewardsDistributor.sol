...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => uint256)) public claimedTokensPerUser;
    ...
    function claim(
        ...
        /// @dev ensure not claiming older leaf after already claiming newer leaf
        uint256 claimedSoFar = claimedTokensPerUser[currentEpoch][msg.sender];
        uint256 tokensToDistribute = (amountToClaim > claimedSoFar)
            ? (amountToClaim - claimedSoFar)
            : 0;

        if(tokensToDistribute == 0) revert NoRewardsAvailable();

        claimedTokensPerUser[currentEpoch][msg.sender] = amountToClaim;
        claimed[leaf] = true;

        tokenContract.transfer(msg.sender, tokensToDistribute);

        emit Claimed(msg.sender, currentEpoch, tokensToDistribute);
    }
     ...
     */
    function setTreasuryFeeBps(uint16 _treasuryFeeBps) external onlyOwner {
    ...
    function swapMultiple(
        ...
        for (uint256 i = 0; i < tokensAttributed.length;) {
            ...
            /// @dev if the token is our reward token, skip swapping since no need
            if(currentTokenToSwap == address(tokenContract)) {
                /// @dev only need to apply fee in this case
                _applyFee(tokenAmount);
            }
            else {
