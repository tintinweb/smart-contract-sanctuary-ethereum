...
/**
 * (c) 2021 Boulevard, All Rights Reserved
 */
...
contract STARSP is ERC721URIStorage, ERC2981, Ownable {
    using SafeMath for uint256;

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    mapping(address => bool) private isAddressAllowlisted;

    mapping(uint256 => address) private creators;
    mapping(uint256 => uint256) private prices;
    mapping(uint256 => mapping(address => bool)) private purchasers;

    event TokenMinted(uint256 indexed tokenId, address ownerAddress, uint256 price);
    event PriceUpdated(uint256 indexed tokenId, uint256 newPrice);
    event Purchase(uint256 indexed tokenId, address purchaserAddress);

    constructor() ERC721("BitStars Paid Content", "STARSP") {
        // noop
    }

    function setTokenPrice(uint256 tokenId, uint256 price) public {
        require(creators[tokenId] == msg.sender, "Unauthorized sender.");
        prices[tokenId] = price;
        emit PriceUpdated(tokenId, price);
    }

    function setIsAddressAllowlisted(address _newAddress) public onlyOwner {
        isAddressAllowlisted[_newAddress] = true;
    }

    function setIsAddressAllowlisted(
        address _newAddress,
        bool _value
    ) public onlyOwner {
        isAddressAllowlisted[_newAddress] = _value;
    }
    ...
    function purchase(uint256 tokenId) public payable {
        require(msg.value == prices[tokenId], string.concat("Must pay purchase price. Sent value: ", string.concat(Strings.toString(msg.value), string.concat(", Required value: ", Strings.toString(prices[tokenId])))));
        payable(creators[tokenId]).transfer(prices[tokenId]);
        purchasers[tokenId][msg.sender] = true;
        emit Purchase(tokenId, msg.sender);
    }

    function isPurchaser(uint256 tokenId, address _address) public view returns (bool) {
        return purchasers[tokenId][_address];
    }
