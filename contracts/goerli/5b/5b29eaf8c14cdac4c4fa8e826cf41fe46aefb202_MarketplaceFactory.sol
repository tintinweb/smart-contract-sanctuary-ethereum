...
interface IBooksNFT {
    function transferBook(address buyer) external returns(uint);
    function secondaryTransferBooks(address buyer, uint256 bookSaleTicketId) external;
    function getAuthor() external view returns (address);
    function getBookPrice() external view returns (uint256);
    function ownerOf(uint256 tokenId) external view returns (address owner);
    function getSellingPrice(uint256 bookCopyId) external view returns (uint256);
}
...
contract BooksMarketplace {
    ...
    IBSPToken private _token;
    IBooksNFT private _book;

    address private _author;

    constructor(address token, address book) {
        _token = IBSPToken(token);
        _book = IBooksNFT(book);
        _author = _book.getAuthor();
    }

    event Purchase(address indexed buyer, address seller, uint256 bookId);
    ...
    function purchaseBook() public {
        ...
        uint bookId = _book.transferBook(buyer);

        emit Purchase(buyer, address(this), bookId);
    }
...
contract BooksNFT is Context, AccessControl, ERC721URIStorage {
     ...
     */
    function mint(address operator, string memory tokenURI)
        internal
        virtual
        isMinterRole
        returns (uint256)
    {
        ...
        _setTokenURI(newBookCopyId, tokenURI);
     ...
     * 
     */
    function bulkMintBookCopies(uint256 numOfBookCopies, string[] memory uris, address operator)
        public
        virtual
    {
        // Important so that no one runs the for loop like too many times
        require(
            (bookCounts() + numOfBookCopies) <= 25,
            "Exceeds max count"
        );

        for (uint256 i = 0; i < numOfBookCopies; i++) {
            mint(operator, uris[i]);
        }
    }
     ...
     */
    function transferBook(address buyer) public returns (uint) {
...
// File: contracts/MarketplaceFactory.sol
