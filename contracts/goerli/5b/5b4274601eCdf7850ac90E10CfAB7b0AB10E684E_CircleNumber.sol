...
contract CircleNumber is ERC721, Ownable {
    ...
    constructor(string memory _baseUri) ERC721("pitests1", "PIT") {
    ...
    function withdraw() external onlyOwner {
        (bool success, ) = withdrawalWallet.call{value: address(msg.sender).balance}('');
    ...
    function airdrop(uint256 quantity_) public onlyOwner {
        for(uint256 i = 0; i < quantity_; i++){
            totalSupply++;
            _safeMint(withdrawalWallet, totalSupply);
        }
    }
}