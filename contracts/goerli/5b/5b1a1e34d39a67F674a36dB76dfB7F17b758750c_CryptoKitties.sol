...
contract ERC721 is Context {
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        address owner = ownerOf(tokenId);
        return (spender == owner || getApprovedAddress(tokenId) == spender);
    }
...
contract CryptoKitties is ERC721("CryptoKitties", "CryptoKitties") {
    ...
    function tokensOfOwner(address _owner) external view returns(uint256[] memory ownerTokens) {
        ...
        if (tokenCount == 0) {
            ...
            ownerTokens = new uint256[](0);
        } else {
            ownerTokens = new uint256[](tokenCount);
            uint256 total = totalSupply();
            uint256 resultIndex = 0;

            for (uint256 i = 1; i <= total; i++) {
                if (ownerOf(i) == _owner) {
                    ownerTokens[resultIndex] = i;
                    resultIndex++;
                }
            }
        }
    ...
    function mintTo(address to, uint256 count) external {
        for (uint256 i = 0; i < count; i++) {
