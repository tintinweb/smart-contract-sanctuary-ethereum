...
abstract contract OpenERC173 is IERC173, OpenERC165 {
    ...
    function _initializeOpenERC173(address owner_) internal {
...
abstract contract OpenERC721Metadata is IERC721Metadata, OpenERC721 {
    ...
    function _initializeOpenERC721Metadata(string memory name_, string memory symbol_) internal {
...
abstract contract OpenCloneable is IOpenCloneable, OpenERC165 {
    ...
    function _initializeOpenCloneable(string memory template_, uint256 version_) internal {
    ...
    OpenCloneable
{
    ...
    /// @notice _initializeOpenNFTs
    /// @param name_ name of the NFT Collection
    /// @param symbol_ symbol of the NFT Collection
    /// @param owner_ owner of the NFT Collection
    // solhint-disable-next-line comprehensive-interface
    function _initializeOpenNFTs(string memory name_, string memory symbol_, address owner_)
        internal
    {
        tokenIdNext = 1;
        _initializeOpenERC173(owner_);
        _initializeOpenCloneable("OpenNFTs", 4);
        _initializeOpenERC721Metadata(name_, symbol_);
    }
...
interface IOpenNFTsV4 {
    ...
    function mint(address minter, string memory tokenURI) external payable returns (uint256 tokenID);
...
contract OpenNFTsV4 is IOpenNFTsV4, OpenERC721Metadata, OpenERC721Enumerable, OpenERC173, OpenCloneable {
    ...
    function initialize(
        ...
        tokenIdNext = 1;
        _initializeOpenERC173(owner_);
        _initializeOpenCloneable("OpenNFTsV4", 4);
        _initializeOpenERC721Metadata(name_, symbol_);
        open = options[0];
    }
    ...
    function mint(address minter_, string memory tokenURI_)
        public
        payable
    ...
    {
        tokenID = tokenIdNext++;
        _mint(minter_, tokenURI_, tokenID);
    }
