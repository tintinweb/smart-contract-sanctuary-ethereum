...
contract SquareMetadata is Initializable, SquareMetadataState, ISquareMetadata {
	...
	{   
		...
		return string(abi.encodePacked(baseURI ,_tokenId.toString(), '_', re, '_', '.json'));
	}
	...
	function setRevealedPosition(uint256 _tokenId, uint256 _x, uint256 _y)
		override
		external
		onlySquare
	{
		RevealedPosition memory revealedPos = RevealedPosition(
			_x, // X-axis position
			_y // Y-axis position
		);
		tokenIdToRevealedPos[_tokenId] = revealedPos;
	}

	function setInitialPosition(uint256 _tokenId, uint256 _x, uint256 _y) 
		override
		external
		onlySquare 
	{
		InitialPosition memory initialPos = InitialPosition(
			_x, // X-axis position
			_y // Y-axis position
		);
		tokenIdToInitialPos[_tokenId] = initialPos;
	}
	...
	modifier onlySquare() {
		require(squares.contains(msg.sender), "Not a square contract");
...
interface ISquareMetadata {
    ...
    function setInitialPosition(uint256 _tokenId, uint256 _x, uint256 _y) external;

    function setRevealedPosition(uint256 _tokenId, uint256 _x, uint256 _y) external;
}
...
abstract contract SquareMetadataState is Initializable, AdminableUpgradeable {
    ...
    mapping(uint256 => RevealedPosition) public tokenIdToRevealedPos;
    mapping(uint256 => InitialPosition) public tokenIdToInitialPos;
...
- Emit metrics/events 
 - setting initial
 - setting revealed 
- Who should call the set position, Square or Square Manager? 
