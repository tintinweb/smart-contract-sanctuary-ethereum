...
contract WhereMyVansGo is ERC721Creator {
    ...
    function lockMerkleRoot() public adminRequired {
        require(merkleRoot != 0, "Merkle root has not yet been set");
        merkleRootLocked = true;
    }

    function setContractURI(string calldata _contractURI) public adminRequired {
    ...
    function sendTokenToPurgatory(
        ...
        require(
            purgatory != address(0),
            "Purgatory address cannot be the 0x0 address"
        );
        require(
            originalContract != address(0),
            "Original contract address cannot be the 0x0 address"
        );

        ERC1155Partial baseContract = ERC1155Partial(originalContract);
        require(
            baseContract.isApprovedForAll(msg.sender, address(this)),
            "Contract not yet approved for all transfers from original contract"
        );
        ...
        require(
            ...
            "Invalid proof"
        ); // Ensures original and new tokenIds are valid
    ...
    function mint(
        ...
        uint8 tokenId,
        string calldata uri
    ) public virtual nonReentrant adminRequired returns (uint256) {
        require(
            tokenId < collectionSize,
            "Token ID must be less than max collection size"
        );
        require(
            tokenId < migrationRange[0] || tokenId > migrationRange[1],
            "Token ID must be outside of migration range"
        );
        require(bytes(uri).length > 0, "URI cannot be empty");
    ...
    function generateLeaf(uint256 originalTokenId, uint256 newTokenId)
        public
        pure
        returns (bytes32)
    {
        ...
        string memory leaf = string(
            abi.encodePacked(originalTokenStr, " ", newTokenStr)
        );
        return keccak256(abi.encodePacked(leaf));
    }

    function convertUintToString(uint256 _i)
        internal
        pure
