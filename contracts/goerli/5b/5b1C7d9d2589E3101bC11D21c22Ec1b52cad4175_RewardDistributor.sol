...
contract RewardDistributor is IRewardDistributor, Ownable, EIP712 {
    ...
    mapping(address user => uint256 claimedAmt) public claimed;
    mapping(address user => uint256 ethAmount) public ethClaimed;
    ...
    function claim(Reward calldata reward, bytes calldata signature) public {
        _validateClaimRequest(reward, signature);
        if (reward.token != address(rewardToken)) {
            revert InvalidRewardTokenAddress(reward.token);
        }
        userNonce[reward.recipient] += 1;
        claimed[reward.recipient] += reward.rewardAmount;
        rewardToken.transfer(reward.recipient, reward.rewardAmount);

        emit Claimed(reward.recipient, reward.token, reward.rewardAmount, reward.nonce);
    }

    function claimETH(Reward calldata reward, bytes calldata signature) public {
        _validateClaimRequest(reward, signature);
        if (reward.token != ETH) {
            revert InvalidRewardTokenAddress(reward.token);
        }
        userNonce[reward.recipient] += 1;
        ethClaimed[reward.recipient] += reward.rewardAmount;
        (bool success,) = payable(reward.recipient).call{ value: reward.rewardAmount }(new bytes(0));
    ...
    function _validateClaimRequest(Reward calldata reward, bytes calldata signature) private {
        bytes32 structHash = keccak256(
            abi.encode(
                "Reward(address recipient,uint256 rewardAmount, uint256 nonce)",
                reward.recipient,
                reward.rewardAmount,
                reward.nonce
            )
        ...
        bool isValid = SignatureCheckerLib.isValidERC1271SignatureNowCalldata(validator, digest, signature);
        if (!isValid) {
            revert InvalidSignature();
        }

        if (reward.nonce < userNonce[reward.recipient]) {
            revert InvalidNonce(reward.nonce);
        }
        uint256 currentRewardAmt = rewardToken.balanceOf(address(this));
        if (currentRewardAmt < reward.rewardAmount) {
            revert InsufficientReward(currentRewardAmt);
        }
    }
