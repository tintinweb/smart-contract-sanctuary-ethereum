...
contract StreamSpreader is Ownable {
    ...
    function withdrawFunds(address receiver, uint amount, ISuperToken superToken) external onlyOwner {
        superToken.transfer(receiver, amount);
    }

    function createFlow(address payable receiver) external onlyOwner {
        require(!usedAddresses[receiver], "receiver already received stream");
        usedAddresses[receiver] = true;
        //send around 1500/month
        uint256 i;
        unchecked {
            for(i; i < streamTokens.length; ++i) {
                ISuperToken superToken = streamTokens[i];
                ...
                cfaV1.createFlow(receiver, streamTokens[i], 578703703703703);
            }
        }
    }

    //allows owner to delete flows at any time
    function deleteFlow(address receiver, ISuperToken superToken) external onlyOwner {
