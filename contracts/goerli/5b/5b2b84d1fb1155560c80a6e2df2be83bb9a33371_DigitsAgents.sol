...
contract DigitsAgents is ERC721A, ReentrancyGuard {
    ...
    uint256 private maxSalePlusOne = 1000;
    uint256 public tokenPriceFoundingAgent = 0.99 ether;
    uint256 public tokenPriceSeeker = 1.29 ether;
    uint256 public tokenPricePublic = 1.59 ether;

    bytes32 public merkleRootPresale;
    bytes32 public merkleRootFoundingAgent;
    bytes32 public merkleRootSeeker;

    uint public mintAllowancePlusOne = 3;
    ...
    constructor() ERC721A("Digits Agents", "DA") {
        ...
        merkleRootFoundingAgent = 0x935323dfdc2225bd42c05cdcbeb7d4502e6a24cf363845ee8538427fca3329db; //0x should be prepended
        merkleRootSeeker = 0x935323dfdc2225bd42c05cdcbeb7d4502e6a24cf363845ee8538427fca3329db; //0x should be prepended
        ...
        correctValue(tokenPricePublic * quantity)
    {
        ...
        mintsPerAddress[4][msg.sender] += quantity;
    }
     ...
     * @param proof The Merkle proof used to validate the leaf is in the root.
     */
    function mintFoundingAgent(uint256 quantity, bytes32[] calldata proof, bytes32 leaf)
        external
        payable
        noBots
        isContractState(ContractState.ON)
        withinMintLimit(quantity)
        correctValue(tokenPriceFoundingAgent * quantity)
    {
        require(_numberMinted(msg.sender) + quantity < mintAllowancePlusOne, "Maximum number of NFTs allowed to Mint reached!");
        require(verify(merkleRootFoundingAgent, leaf, proof), "Not a valid proof!");
        _safeMint(msg.sender, quantity);
        mintsPerAddress[2][msg.sender] += quantity;
    }

    function mintSeeker(uint256 quantity, bytes32[] calldata proof, bytes32 leaf)
        external
        payable
        noBots
        isContractState(ContractState.ON)
        withinMintLimit(quantity)
        correctValue(tokenPriceSeeker * quantity)
    {
        require(_numberMinted(msg.sender) + quantity < mintAllowancePlusOne, "Maximum number of NFTs allowed to Mint reached!");
        require(verify(merkleRootSeeker, leaf, proof), "Not a valid proof!");
        _safeMint(msg.sender, quantity);
    ...
    /**
     * Set the Founding Agent sale Merkle root.
     * @dev The Merkle root is calculated from [address, allowance] pairs.
     * @param merkleRoot_ The new merkle roo
     */
    function setMerkleRootFoundingAgent(bytes32 merkleRoot_) external onlyOwner {
        merkleRootFoundingAgent = merkleRoot_;
    }

    /**
     * Set the Seeker sale Merkle root.
     * @dev The Merkle root is calculated from [address, allowance] pairs.
     * @param merkleRoot_ The new merkle roo
     */
    function setMerkleRootSeeker(bytes32 merkleRoot_) external onlyOwner {
        merkleRootSeeker = merkleRoot_;
    }
