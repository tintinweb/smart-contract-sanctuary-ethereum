...
contract ProjectFactory is Ownable, BlackHolePrevention {
    ...
    constructor(
        ...
        chainid = id;
        projectIdOffset = id * ( 10 ** 10 );

        if(chainid == 1337 || chainid == 31337) {
            TheRegistry = IRegistryConsumer(0x7B3c35D7D339A3A0f70D06b8852a35D674bEB1dC);
        }
    ...
    function getProjectIdWithOffset(uint256 _id) public view returns (uint256) {
        return projectIdOffset + _id;
    }

    function getInternalProjectId(uint256 _publicId) public view returns (uint256) {
        uint256 internalId = _publicId - projectIdOffset;
        require(internalId > 0 && internalId < (10 ** 10) , "ProjectFactory: Id out of bounds.");
        return internalId;
    }
    ...
    function LaunchProject(
        ...
        projectCount++;
        uint256 newProjectId = getProjectIdWithOffset(projectCount);
        saleConfig.projectID = newProjectId;
        tokenConfig.projectID = newProjectId;
        ...
        emit NewProject(newProjectId, name);
    }
    ...
    function UpdateProjectDetails(
        ...
        _projectId = getInternalProjectId(_projectId);
