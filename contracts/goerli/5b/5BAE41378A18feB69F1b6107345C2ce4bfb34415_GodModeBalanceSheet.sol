...
contract GodModeBalanceSheet is BalanceSheetV2 {
    function __godMode_setBondList(address account, IHToken[] memory bondList)
        external
    {
    ...
    function __godMode_setCollateralList(
        address account,
        IERC20[] memory collateralList
    ) external {
    ...
    IERC20Recover // one dependency
{
    ...
    error HToken__RedeemInsufficientLiquidity(
        uint256 underlyingAmount,
        uint256 totalUnderlyingReserve
    );
    ...
    error HToken__WithdrawUnderlyingUnderflow(
        address depositor,
    ...
    event SetBalanceSheet(
        ...
        IBalanceSheetV2 oldBalanceSheet,
        IBalanceSheetV2 newBalanceSheet
    );
    ...
    function getDepositorBalance(address depositor)
        external
        view
...
interface IFintroller is IOwnable {
    ...
    function getCollateral(IERC20 collateral)
        external
        view
    ...
    function getCollateralCeiling(IERC20 collateral)
        external
        view
        returns (uint256);
    ...
    function getCollateralRatio(IERC20 collateral)
        external
        view
        returns (uint256);
    ...
    /// @return bool true = allowed, false = not allowed.
    function getDepositCollateralAllowed(IERC20 collateral)
        external
        view
        returns (bool);
    ...
    /// @param bond The bond to make the check against.
    /// @return bool true = allowed, false = not allowed.
    function getDepositUnderlyingAllowed(IHToken bond)
        external
        view
        returns (bool);
    ...
    function getLiquidationIncentive(IERC20 collateral)
        external
        view
        returns (uint256);
    ...
    /// @param bond The bond to make the check against.
    /// @return bool true = allowed, false = not allowed.
    function getLiquidateBorrowAllowed(IHToken bond)
        external
        view
        returns (bool);
    ...
    /// @param newCollateralRatio The new collateral ratio.
    function setCollateralRatio(IERC20 collateral, uint256 newCollateralRatio)
        external;
    ...
    /// @param state The new state to put in storage.
    function setDepositCollateralAllowed(IERC20 collateral, bool state)
        external;
...
interface IBalanceSheetV2 is IOwnableUpgradeable {
    ...
    error BalanceSheet__CollateralCeilingOverflow(
    ...
    error BalanceSheet__LiquidityShortfall(
        address account,
        uint256 shortfallLiquidity
    );
    ...
    error BalanceSheet__RepayBorrowInsufficientBalance(
        IHToken bond,
        uint256 repayAmount,
    ...
    error BalanceSheet__RepayBorrowInsufficientDebt(
        IHToken bond,
        uint256 repayAmount,
        uint256 debtAmount
    );
    ...
    function getBondList(address account)
        external
        view
        returns (IHToken[] memory);
    ...
    /// @dev It is not an error to provide an invalid address.
    /// @param account The borrower account to make the query against.
    function getCollateralList(address account)
        external
        view
    ...
    function getDebtAmount(address account, IHToken bond)
        external
        view
    ...
    function depositCollateral(IERC20 collateral, uint256 depositAmount)
        external;
    ...
    function withdrawCollateral(IERC20 collateral, uint256 withdrawAmount)
        external;
}
    ...
    SBalanceSheetV2 // no dependency
{
    ...
    function initialize(IFintroller fintroller_, IChainlinkOperator oracle_)
        public
        initializer
    {
        ...
        returns (uint256 excessLiquidity, uint256 shortfallLiquidity)
    {
        ...
        for (uint256 i = 0; i < vars.collateralListLength; i++) {
            ...
            if (collateralModify != collateral) {
                vars.collateralAmount = vaults[account].collateralAmounts[
                    collateral
                ];
            } else {
            ...
            if (vars.collateralDecimals != 18) {
                vars.normalizedCollateralAmount = vars.collateralAmount.div(
            ...
            vars.normalizedCollateralPrice = oracle.getNormalizedPrice(
                collateral.symbol()
            );

            // Calculate the USD value of the collateral amount.
            vars.collateralValueUsd = vars.normalizedCollateralAmount.mul(
                vars.normalizedCollateralPrice
            );
            ...
            vars.weightedCollateralValueUsd = vars.collateralValueUsd.div(
                vars.collateralRatio
            );

            // Add the previously calculated USD value of the weighted collateral to the totals.
            vars.totalWeightedCollateralValueUsd += vars
                .weightedCollateralValueUsd;
        }
        ...
        for (uint256 i = 0; i < vars.bondListLength; i++) {
            ...
            vars.normalizedUnderlyingPrice = oracle.getNormalizedPrice(
                bond.underlying().symbol()
            );

            // Calculate the USD value of the collateral amount.
            vars.debtValueUsd = vars.debtAmount.mul(
                vars.normalizedUnderlyingPrice
            );
        ...
        unchecked {
            if (vars.totalWeightedCollateralValueUsd > vars.totalDebtValueUsd) {
                excessLiquidity =
                    vars.totalWeightedCollateralValueUsd -
                    vars.totalDebtValueUsd;
            } else {
                shortfallLiquidity =
                    vars.totalDebtValueUsd -
                    vars.totalWeightedCollateralValueUsd;
    ...
    function getRepayAmount(
        ...
        if (collateralDecimals != 18) {
            normalizedSeizableAmount = seizableCollateralAmount.div(
        ...
        // Grab the normalized USD price of the collateral.
        uint256 normalizedCollateralPrice = oracle.getNormalizedPrice(
            collateral.symbol()
        );

        // Grab the normalized USD price of the underlying.
        uint256 normalizedUnderlyingPrice = oracle.getNormalizedPrice(
            bond.underlying().symbol()
        );

        // Calculate the top part of the equation.
        uint256 numerator = normalizedSeizableAmount.mul(
            normalizedCollateralPrice
        );

        // Calculate the repay amount.
        uint256 liquidationIncentive = fintroller.getLiquidationIncentive(
            collateral
        );
        repayAmount = numerator.div(
            liquidationIncentive.mul(normalizedUnderlyingPrice)
        );
    }
    ...
    function getSeizableCollateralAmount(
        ...
        uint256 numerator = repayAmount.mul(
            liquidationIncentive.mul(normalizedUnderlyingPrice)
        );

        // Calculate the normalized seizable collateral amount.
        uint256 normalizedSeizableCollateralAmount = numerator.div(
            normalizedCollateralPrice
        );
    ...
    function borrow(IHToken bond, uint256 borrowAmount) public override {
        ...
        if (newTotalSupply > debtCeiling) {
            revert BalanceSheet__DebtCeilingOverflow(
                newTotalSupply,
                debtCeiling
            );
        }
        ...
        uint256 newDebtAmount = vaults[msg.sender].debtAmounts[bond] +
            borrowAmount;
        ...
        if (vaults[msg.sender].debtAmounts[bond] == 0) {
            ...
            unchecked {
                uint256 newBondListLength = vaults[msg.sender].bondList.length +
                    1;
                uint256 maxBonds = fintroller.maxBonds();
                if (newBondListLength > maxBonds) {
                    revert BalanceSheet__BorrowMaxBonds(
                        bond,
                        newBondListLength,
                        maxBonds
                    );
        ...
        if (hypotheticalShortfallLiquidity > 0) {
            revert BalanceSheet__LiquidityShortfall(
                msg.sender,
                hypotheticalShortfallLiquidity
            );
        }
        ...
        override
    {
        ...
        uint256 collateralAmount = vaults[msg.sender].collateralAmounts[
            collateral
        ];
        ...
        vaults[msg.sender].collateralAmounts[collateral] =
            collateralAmount +
            depositAmount;
    ...
    function liquidateBorrow(
        ...
        if (!bond.isMatured()) {
            ...
            (, uint256 shortfallLiquidity) = getCurrentAccountLiquidity(
                borrower
            );
        ...
        uint256 vaultCollateralAmount = vaults[borrower].collateralAmounts[
            collateral
        ];
        uint256 seizableCollateralAmount = getSeizableCollateralAmount(
            bond,
            repayAmount,
            collateral
        );
        ...
        unchecked {
            newCollateralAmount =
                vaults[borrower].collateralAmounts[collateral] -
                seizableCollateralAmount;
        }
    ...
    /// @inheritdoc IBalanceSheetV2
    function setFintroller(IFintroller newFintroller)
        external
        override
        onlyOwner
    {
    ...
    /// @inheritdoc IBalanceSheetV2
    function setOracle(IChainlinkOperator newOracle)
        external
        override
        onlyOwner
    {
        ...
        override
    {
        ...
        // Checks: there is enough collateral in the vault.
        uint256 vaultCollateralAmount = vaults[msg.sender].collateralAmounts[
            collateral
        ];
        if (vaultCollateralAmount < withdrawAmount) {
            revert BalanceSheet__WithdrawCollateralUnderflow(
                msg.sender,
                vaultCollateralAmount,
                withdrawAmount
            );
    ...
    function removeCollateralFromList(address account, IERC20 collateral)
        internal
    {
        // Load into memory for faster iteration.
        ...
        storedCollateralList[collateralIndex] = storedCollateralList[
            length - 1
        ];
    ...
    function repayBorrowInternal(
        ...
        if (debtAmount < repayAmount) {
            revert BalanceSheet__RepayBorrowInsufficientDebt(
                bond,
                repayAmount,
                debtAmount
            );
        ...
        if (hTokenBalance < repayAmount) {
            revert BalanceSheet__RepayBorrowInsufficientBalance(
                bond,
                repayAmount,
                hTokenBalance
            );
