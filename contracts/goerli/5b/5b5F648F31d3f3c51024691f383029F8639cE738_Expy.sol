...
contract Expy {
    ...
    event DepositNative(address indexed from, address indexed to, uint256 amount);
    event WithdrawNative(address indexed reciever, uint256 amount);
    event Withdraw(address indexed reciever, address indexed token, uint256 amount);
    ...
    receive() external payable {
        emit Expy.DepositNative(msg.sender, address(this), msg.value);
    }
    ...
    function withdrawNative(address receiver, uint256 amount) external notPaused onlyOwner noReentrant{
        require(receiver != address(0), "Reciever Address is Zero!");
        require(amount > 0, "Value is Zero!");
        
        payable(receiver).transfer(amount);

        emit Expy.WithdrawNative(receiver, amount);
    }
    ...
    function withdraw(address receiver, address token, uint256 amount) external notPaused onlyOwner noReentrant{
        ...
        emit Expy.Withdraw(receiver, token, amount);
    }
