...
contract OpinToken is Ownable{
    string public constant name = "CarRentToken"; //название токена
    string public constant symbol = "Rent"; //скоращение токена
    ...
    function emission(address to, uint value) onlyOwner public { // фунцкия появления новых токенов (доступна только владельцу контракта)
        require(_TotalSupply + value >= _TotalSupply && balances[to] + value >= balances[to]);
        balances[to] += value;
        _TotalSupply += value;
    }
