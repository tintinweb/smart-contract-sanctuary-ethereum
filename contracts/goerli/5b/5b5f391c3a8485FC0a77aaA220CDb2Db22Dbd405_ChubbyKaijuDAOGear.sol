...
contract ChubbyKaijuDAOGear is ERC1155, Ownable {
    ...
    uint16 public constant MAX_FREE_RADIO = 748;
    ...
    function preSale(bytes memory signature, uint256 id, uint256 amount) external payable {
        ...
        if(ispreSale1){
            ...
            if(id==TYPE_RADIO){
                require(radioMinted+amount<MAX_RADIO+1-MAX_FREE_RADIO,"All radios are minted");
    ...
    function setinvasionContract(address invasionAddress)
        external
        onlyOwner
    {
        invasionContract = invasionAddress;
    }
    ...
    function burnGearForAddress(uint256 typeId, address burnTokenAddress)
        external
    {
        require(msg.sender == invasionContract, "Invalid burner address");
    ...
    function setInvasionContrac(address _address) external onlyOwner{
