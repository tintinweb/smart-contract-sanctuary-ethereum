...
contract EnglishAuctionHouse is IWCNFTErrorCodes, ReentrancyGuard {
    struct EnglishAuction {
        uint256 lotSize;
        uint256 highestBid;
        uint256 outbidBuffer;
        uint256 startTime;
        uint256 endTime;
    ...
    uint256 private constant _MAXIMUM_START_DELAY = 604800; // 1 week
    uint256 private constant _MAXIMUM_DURATION = 604800; // 1 week
    uint256 private constant _MINIMUM_DURATION = 3600; // 1hr, to avoid block time skew
    uint256 private constant _MINIMUM_TIME_BUFFER = 7200; // cannot stop auction within 2 hours of end
    ...
    /// Bid must exceed the current highest bid, plus the minimum buffer price
    error BidTooLow();

    /// Auctions cannot be stopped within 2 hours of their end time
    error CannotStopAuction();

    /// attempting to initialize an auction that has already been initialized
    error EnglishAuctionAlreadyInitialized();

    /// requested English auction has ended
    error EnglishAuctionHasEnded();

    /// cannot complete request when the English auction is active
    error EnglishAuctionIsActive();

    /// The requested EnglishAuction is not currently accepting bids
    error EnglishAuctionIsNotBiddable();

    /// The requested auction has already been settled
    error EnglishAuctionIsSettled();

    /// The requested EnglishAuction has not ended.
    /// It is not necessarily active, as the start may be delayed.
    error EnglishAuctionNotEnded();

    /// The requested auction has not been settled
    error EnglishAuctionNotSettled();

    /// Cannot create auction with duration greater than _MAXIMUM_DURATION.
    /// Edit _MAXIMUM_DURATION with editMaxDuration();
    error ExceedsMaximumDuration();

    /// Cannot start too far in the future. Edit delay with editMaxStartDelay()
    error ExceedsMaximumStartDelay();

    /// Insufficient funds to process withdrawal. Contact the team
    error InsufficientContractFunds();

    /// Duration must exceed _MINIMUM_DURATION. Edit with editMinDuration()
    error InsufficientDuration();

    /// The requested EnglishAuction instance does not exist in this contract
    ...
    /// emitted when an English auction is started
    ...
    /// emitted when an auction is force-stopped
    ...
    /// emitted when a new high bid is received
    event NewHighBid(
        uint256 indexed auctionId,
    ...
    /// emitted when an auto refund fails
    event RefundLodged(
        uint256 indexed auctionId,
    ...
    /**
     * @dev edit the maximum gas sent with _gasLimitedCall().
     ...
     *  fail. Then all users who are outbid will have to pull their own refunds
     *  with claimRefund().
     * @param maxGas maximum gas units for the call
     */
    function editMaxRefundGas(uint256 maxGas) public {
        ...
        returns (uint256)
    {
        ...
        ea.outbidBuffer = outbidBufferWei;
    ...
    /**
     * @dev force stop an English auction, useful if an incorrect parameters
     ...
     */
    function _forceStopEnglishAuction(uint256 auctionId) internal nonReentrant {
        _revertIfEnglishAuctionNotValid(auctionId);
    ...
    /**
     * @notice bid on an auction. If you are the highest bidder already,
     * extra bids are ADDED to your existing bid.
     *
     * @param auctionId identifier for the auction
     */
    function _bidEnglish(uint256 auctionId) internal nonReentrant {
        _revertIfEnglishAuctionNotValid(auctionId);
        ...
        returns (bool)
    {
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        // TODO:
        // This call must not revert and get the contract stuck.
        // Either:
        //      leave as it is: limit gas to a modifiable value, in case of
        //       opcode cost changes in the future
        //      or: limit to a permanent value
        //      or: remove limit and forward all gas to the receiver
        //      or: make ALL outbid users withdraw their bids
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        returns (bool)
    {
        ...
        if (_englishAuctions[auctionId].endTime > block.timestamp) {
            return true;
        } else {
            return false; // not started || ended
        }
    }
     ...
     * @param auctionId identifier for the auction
     * @return biddable true (live-and-biddable) or false (ended or not started)
     */
        ...
        returns (bool)
    {
        ...
        if (ea.startTime <= block.timestamp && block.timestamp < ea.endTime) {
            return true;
        } else {
            return false;
        }
        ...
        returns (bool)
    {
        ...
        if (ea.endTime != 0 && ea.endTime <= block.timestamp) {
            return true; // auction has ended
        } else {
            return false; // auction is active or has not been initialized
        }
    }
     ...
     * @param auctionId identifier for the auction
     */
    function _englishAuctionSettled(uint256 auctionId)
        internal
        view
        returns (bool)
    {
...
contract MerkleFourParams {
    ...
    /**
     * @dev adds the number of tokens to an address's total for a given listKey
...
import "./lib/auctions/SteppedDutchAuctionMulti.sol";
...
contract FinsClaim is
    IWCNFTErrorCodes,
    DefaultOperatorFilterer,
    SteppedDutchAuctionMulti,
    EnglishAuctionHouse,
    MerkleFourParams,
    Ownable,
    WCNFTToken,
    ERC721
{
    ...
    struct Suburb {
        uint256 id; // 1, 2, 3... etc. local to each suburb, starts at 1
        uint256 cityId; // parent city
        uint256 dutchAuctionId; // identifier of current DutchAuction, see SteppedDutchAuctionMulti.sol
    ...
    uint256 private constant _MAX_DUTCH_AUCTION_MINTS = 1;
    ...
    /// cannot add new city if previous has no suburb > addSuburb or removeCity
     ...
     * @param dutchAuctionId auction ID, see SteppedDutchAuctionMulti.sol
     */
    ...
    function _revertIfAnyAuctionActive(Suburb storage s) internal view {
        ...
        if (englishAuctionActive(s.englishAuctionId)) {
            revert EnglishAuctionIsActive();
        }
    }
        ...
        returns (uint256)
    {
        ...
        // also means first token is ID #1 - consistent with cityId/suburbId
     ...
     * @return newSuburbId ID for the newly created Suburb.
     */
    function addSuburb(
        uint256 cityId_,
        ...
        // recent city
        if (cityId_ != numberOfCities) revert NotTheMostRecentCity();
     ...
     *
     * @param cityId the city to be removed. Must be the most
    ...
    function removeCity(uint256 cityId) external onlyRole(SUPPORT_ROLE) {
        ...
        if (cityId != numberOfCities) revert NotTheMostRecentCity();
        ...
        onlyRole(SUPPORT_ROLE)
    {
        ...
        if (suburbId != c.numberOfSuburbs) revert NotTheMostRecentSuburb();
     ...
     * @return Suburb tuple of data for the suburb
     */
    ...
    /**
     * @dev start and stop the public for a suburb.
     ...
     *  duration is correct by calling getDutchAuctionInfoInSuburb(cityId, suburbId)
     *
     * @param cityId ID number for suburb's parent city
     * @param suburbId ID number for the suburb to host the Dutch auction
        ...
        returns (uint256)
    {
        ...
        // create the auction, see {SteppedDutchAuctionMulti}
     ...
     *  calling getDutchAuctionInfoInSuburb(cityId, suburbId)
     *
     ...
     *  be ended and settled before setting this new one up.
     *
     * @param cityId city to host the English auction
        ...
        returns (uint256)
    {
        ...
        _revertIfAnyAuctionActive(s);
        uint256 englishId = s.englishAuctionId;
        
        if (
            _englishAuctionEnded(englishId) &&
            !_englishAuctionSettled(englishId)
        ) {
            revert EnglishAuctionNotSettled();
        }
        ...
        uint256 newId = _setUpEnglishAuction(
            lotSize_,
            startingBidInWei_,
            outbidBufferInWei_
        );

        s.englishAuctionId = newId;
        emit EnglishAuctionCreatedInSuburb(cityId, suburbId, newId);
        return newId;
    }
        ...
        onlyRole(SUPPORT_ROLE)
    {
        ...
        // auction ended checks done in _markSettled()
        ...
        if (winner != address(0)) {
            ...
            // to prevent the _safeMint reverting maliciously and sticking the
            // contract, we do not check onERC721Received(). Contracts bidding
            // in this English Auction do so at their own risk and should
            // ensure they can use their tokens once received, i.e. using
     ...
     *  Cannot do this within the last 2 hours of an auction.
     * @param cityId parent city of the auctioning Suburb
     * @param suburbId the suburb hosting the English auction
     */
     ...
     * @dev using MerkleTwoKey to manage a multiparameter merkle tree
     ...
     * @param tokenQuotaInSuburb the total quota for this address in this tier,
     *  regardless of the number minted so far
     * @param merkleProof merkle proof for this address and tier combo
     */
        ...
        payable
    {
        ...
        _setAllowListMinted(msg.sender, numberOfTokens, cityId, suburbId);
     ...
     * @dev refund if transaction hangs across a price step
     * @param cityId the aucioning suburb's parent city
     * @param suburbId the suburb hosting the Dutch auction
        ...
        payable
    {
        ...
        // check auction is active - see {SteppedDutchAuctionMulti}
    ...
    /**
     * @notice Bid on an English auction for any suburb. If you are the highest
     *  bidder already, extra bids are ADDED to your current bid.
     * @dev refunds previous highest bidder when new highest bid received.
     * @param cityId parent city of the auctioning suburb
     * @param suburbId suburb hosting the auction
     */
    function bidEnglish(uint256 cityId, uint256 suburbId) external payable {
 ...
 */
contract SteppedDutchAuctionMulti {
    struct DutchAuction {
    ...
    error ComputedDurationIsTooLong();
    ...
    /**
     * @dev reverts when an uninitialised Dutch auction is requested
     ...
     *  as a tuple.
     *  See {SteppedDutchAuctionMulti-DutchAuction} for details.
     */
    ...
    /**
     * @dev initialise a new Dutch auction and return its ID numberpriceStep
        ...
        returns (uint256)
    {
        ...
        // startPrice_ greater than finalPrice_, should not underflow
        uint256 duration = ((startPrice_ - finalPrice_) * timeStepSeconds_) /
            priceStep_;

        if (duration > type(uint80).max) revert ComputedDurationIsTooLong();
     ...
     *  DOES NOT CHECK if the auction exists (saves an SLOAD).
     ...
     * @param quantity increment the counter by this much
     */
