...
contract Lottery is RrpRequesterV0, Ownable {
  ...
  bytes32 private s_endpointIdUint256;
  address private s_sponsorWallet;
  mapping(bytes32 => bool) private s_waitingFulfillment;
  ...
  function setRequestParameters(
    ...
    s_airnode = airnode;
    s_endpointIdUint256 = endpointIdUint256;
    s_sponsorWallet = sponsorWallet;
  }
  ...
  function requestWinner() external onlyOwner {
    ...
    s_waitingFulfillment[requestId] = true;
  ...
  function fulfillWinner(bytes32 requestId, bytes calldata data) external onlyAirnodeRrp {
    if (!s_waitingFulfillment[requestId]) {
      revert Lottery__UnknowRequest();
    }
    s_waitingFulfillment[requestId] = false;
    uint256 qrngUint256 = abi.decode(data, (uint256));

    uint256 indexOfWinner = qrngUint256 % s_players.length;
