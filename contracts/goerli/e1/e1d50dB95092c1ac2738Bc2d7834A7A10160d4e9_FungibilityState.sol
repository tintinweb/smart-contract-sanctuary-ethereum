...
contract FungibilityState is CIState {
    ...
    function newTakerPosition(
        ...
        shares =
            (calc.takerShare(assetAmount, risk, term, floor) *
                (assetAmount + bAsset.marketSupply(market))) /
            UDIVIDER;
    ...
    function newMakerPosition(
        ...
        shares =
            (calc.makerShare(stableAmount, tier, term) *
                (stableAmount + bStable.marketSupply(market))) /
            UDIVIDER;
...
abstract contract CIState is ClaimState {
    ...
    /// @inheritdoc IState
    function calcCI() public view override returns (uint cia, uint cis) {
        uint papsInAsset;
        uint papsInStable;
    ...
    function _newTakerPosition(
        ...
        _addToClaimState(
            (floor * DIVIDER_ORACLE) / DIVIDER_STABLE,
            (assetAmount * DIVIDER_STABLE) / DIVIDER_ASSET
        );
    ...
    function _closeTakerPosition(
        ...
        _removeFromClaimState(
            (floor * DIVIDER_ORACLE) / DIVIDER_STABLE,
            (assetAmount * DIVIDER_STABLE) / DIVIDER_ASSET
        );

        if (payout.assetAmount > 0) {
            (int deltaAP, int deltaAR) = calc.rebalanceTokenPool(
                -int(payout.assetAmount)
            );
            AP = uint(int(AP) + deltaAP);
            AR = uint(int(AR) + deltaAR);
        }
        ...
        if (payout.stableAmount > 0) {
            (int deltaCP, int deltaCR) = calc.rebalanceStablePool(
                -int(payout.stableAmount)
            );
...
abstract contract ClaimState is IClaimState, State {
    ...
    function setTickLib(address _tickLib) external onlyAdmin {
        tickLib = Ticks(_tickLib);
    }

    function initializeExpectedClaims(
        int24 _lastTick,
        uint _upperLiquidity,
        uint _claim
    ) external onlyAdmin {
    ...
    function getClaims() external view override returns (uint) {
        return expectedClaims.claim;
    }

    function moveToTick(int24 tick, ExpectedClaims memory _expectedClaims)
        public
        view
        override
        returns (ExpectedClaims memory)
    {
        ...
        if (tick > _expectedClaims.tick) {
            // price moves up
            console.log(
                "moveToTick forward number ticks %s",
                uint(int(tick - _expectedClaims.tick))
            );
            for (
                int24 _tick = int24(_expectedClaims.tick);
                _tick < tick;
                _tick++
            ) {
                if (ticks[_tick] > 0) {
                    ...
                    uint deltaClaim = (ticks[_tick] * tickPrice) /
                        DIVIDER_ORACLE;
            ...
            // price moves down
            console.log(
                "moveToTick backward number ticks %s",
                uint(int(_expectedClaims.tick - tick))
            );
            for (
                int24 _tick = int24(_expectedClaims.tick) - 1;
                _tick >= tick;
                _tick--
            ) {
                uint tickPrice = tickLib.getPriceAtTick(_tick);
                _expectedClaims.claim +=
                    (ticks[_tick] * tickPrice) /
                    DIVIDER_ORACLE;
                _expectedClaims.upperLiquidity += ticks[_tick];
    ...
    function moveToPrice(uint price, ExpectedClaims memory _expectedClaims)
        public
        view
        override
        returns (ExpectedClaims memory)
...
import "../math/TakerPositionRate.sol"; // remove Alpha to run tests
...
abstract contract State is
    ...
    Precisions,
    TakerPositionRate, // remove Alpha to run tests
    IState
{
    /// @notice adapter role
    ...
    modifier onlyAdapter() {
        require(
            IAccessControl(market).hasRole(ADAPTER_ROLE, msg.sender),
            "CS: not-adapter"
        );
        _;
    }
...
contract Ticks {
    ...
    function sqrtu(uint x) internal pure returns (uint) {
        unchecked {
            if (x == 0) return 0;
            else {
                uint xx = x;
