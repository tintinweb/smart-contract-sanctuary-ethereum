...
contract WarzIco is Ownable {
    ...
    IERC721 private immutable _rblNft;
    IERC721 private immutable _candyNft;

    uint256 private _startDate = 1671188400; // When to start ICO - Dec 16, 2022 11:00:00 UTC
    uint256 private _endDate = 1671451200; // When to end ICO - Dec 19, 2022 12:00:00 UTC
    uint256 private _claimDate = 1671458400; // When to claim ICO - Dec 19, 2022 14:00:00 UTC

    uint256 private _hardcap = 1000000 ether; // hard cap
    uint256 private _softcap = 500000 ether; // softcap

    uint256 private _level1Price = 0.00227 ether; // ICO price when holds Rebel Punk NFT
    uint256 private _level2Price = 0.0025 ether; // ICO price when holds Bored Candy NFT
    uint256 private _level3Price = 0.00278 ether; // Public ICO price

    uint256 private _minPerUser = 1000 ether; // min amount per user
    uint256 private _maxPerUser = 1000000 ether; // max amount per user, same as hardcap in default
    ...
    constructor(
        ...
        require(rblNft_.supportsInterface(0x80ac58cd), "Invalid ERC721");
        require(candyNft_.supportsInterface(0x80ac58cd), "Invalid ERC721");
        _icoToken = icoToken_;
        _rblNft = rblNft_;
        _candyNft = candyNft_;
    }
    ...
    function claimTokens() external {
        ...
        uint256 userRequiredAmount = (userContributedAmount *
            10**(_icoToken.universalDecimals())) / viewIcoPrice(_msgSender());

        if (userRequiredAmount > 0) {
            _icoToken.universalTransfer(_msgSender(), userRequiredAmount);
        }
     ...
     */
    function finalizeIco(uint256 claimDate_) external onlyOwner {
    ...
    function withdrawRemainedTokens() external onlyOwner {
        ...
        // 1 year after ico finished, owner can withdraw all tokens from the ico contract
        if (block.timestamp >= _endDate + 365 days) {
            _icoToken.universalTransfer(
                _msgSender(),
                _icoToken.universalBalanceOf(address(this))
            );
        } else {
            uint256 contractTokens = _icoToken.balanceOf(address(this));
            uint256 unclaimedTokens = ((_totalContributed - _totalClaimed) *
                10**(_icoToken.universalDecimals())) / _level1Price; // We assume that all unclaimed contributors are holding Rebel Punk NFT

            _icoToken.universalTransfer(
                _msgSender(),
                contractTokens - unclaimedTokens
            );
        }
    }
    ...
    function withdrawFunds() external onlyOwner {
        ...
        payable(_msgSender()).sendValue(_totalContributed);
     ...
     *
     * @param level1Price_ ICO price when the contributor holds Rebel Punk NFT
     * @param level2Price_ ICO price when the contributor holds Bored Candy NFT
     * @param level3Price_ Public ICO price
     *
     * Only owner is allowed to call this function
     */
    function updateIcoPrice(
        uint256 level1Price_,
        uint256 level2Price_,
        uint256 level3Price_
    ) external onlyOwner {
        require(block.timestamp < _startDate, "ICO already started");
        require(
            level1Price_ > 0 && level2Price_ > 0 && level3Price_ > 0,
            "Invalid price"
        );
        _level1Price = level1Price_;
        _level2Price = level2Price_;
        _level3Price = level3Price_;
    }

    /**
     * @notice View ICO level1 / leve2 / level3 prices
     */
    function viewIcoPrices()
        external
        view
        returns (
            uint256,
    ...
    {
        return (_level1Price, _level2Price, _level3Price);
    }

    function viewIcoPrice(address account_) public view returns (uint256) {
        if (_rblNft.balanceOf(account_) > 0) {
            return _level1Price;
        } else if (_candyNft.balanceOf(account_) > 0) {
            return _level2Price;
        } else {
            return _level3Price;
        }
