...
contract TokenBank {
    ...
    constructor (
        ...
        // constructorは引数を変更しなくてもmemoryを指定。
    ...
    /// @dev Tokenの名前を返す
    // privateになっている定数や変数は閲覧できないのでそれを表示するfunctionを作成する。
    ...
    function transfer(address to, uint256 amount) public onlyMember {
        if (owner == msg.sender) {
            require(_balances[owner] - _bankTotalDeposit >= amount, "Amounts greater than the total supply connot be transferred");
        }
