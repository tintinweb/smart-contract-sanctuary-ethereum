...
// this contract is a helper for the diamond factory. it makes creating a diamond token easier.
// it does this by internally creating the DiamondInit contract so that the user doesnt need to pass it in,
// and it also handles the creation of the diamond token, using the parameters it was initialized with when it was deployed.

import "./libraries/DiamondLib.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "./DiamondFactory.sol";

contract DiamondFactoryHelper is Initializable, Ownable {

    address private diamondFactory;
    address private svgManager;

    function initialize(address factory, address _svgManager) external initializer {
        diamondFactory = factory;
        svgManager = _svgManager;
    }

    function create(
       DiamondSettings memory settings
    ) public onlyOwner returns (address payable tokenAddress) {
        bytes memory _calldata;
        tokenAddress = payable(
            DiamondFactory(diamondFactory).create(settings, address(0), _calldata)
        );
        Diamond(tokenAddress).transferOwnership(msg.sender);
  ...
  function _getDiamondAddress(DiamondFactoryStorage storage, address factoryAddress, string memory symbol, bytes memory creationCode)
    ...
    function create(
        ...
        IDiamondElement(_diamondAddress).initialize(
            msg.sender,
            params,
            self.contractData.facetsToAdd[self.contractData.defaultFacetSet],
            diamondInit,
            _calldata
        );
    }
