...
contract CustomWallet is Ownable, ReentrancyGuard, Pausable { 
    ...
    function withdraw(address _token, uint _amount) public nonReentrant {                
        Token storage token = tokens[_token];
        require(token.balances[msg.sender] >= _amount, "Not enough token balance");
        IERC20(_token).safeTransfer(msg.sender, _amount);
        token.balance = token.balance - _amount;
        token.balances[msg.sender] = token.balances[msg.sender] - _amount;
    }

    function withdrawEther(address payable _to, uint _amount) public nonReentrant {
        require(_amount <= balanceOfEther(), "Not enough money in contract");
    ...
    function clientBalance(address _token) public view returns(uint) {
