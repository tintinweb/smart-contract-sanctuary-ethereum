...
contract UserRedemption is Ownable {
    ...
    FactoryInterface immutable factory;
    ...
    function burn(
        ...
        bytes32 _hash = keccak256(abi.encode(msg.sender, amount, user_req_nonce[msg.sender]));

        require(signer == recoverSigner(_hash, signature), "invalid signer");

        user_req_nonce[msg.sender]++;

        req.push(Req({amount: amount, requester: msg.sender, completed: false}));
    ...
    function finalizeBurn(uint256 nonce, bool completed) external only(approver) {
        ...
        if (completed) {
            ...
            factory.burn(_req.amount, "USER_REDEMPTION");
        } else {
            req[nonce].completed = true;
