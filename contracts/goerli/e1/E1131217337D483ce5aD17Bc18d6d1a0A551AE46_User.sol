...
contract User is Pausable, Ownable {
    ...
    function register(address _referrer) public {
        if (referrers[msg.sender] == address(0)) {
            require(
                ...
                "User: you cannot introduce myself"
            );
    ...
    function updateRefUser(address account, address newRefAccount)
        public
        onlyOwner
    {
        // remove item listRef
        ...
        // update
        userRefCount[referrers[account]] = userRefCount[referrers[account]].sub(
            1
        );
