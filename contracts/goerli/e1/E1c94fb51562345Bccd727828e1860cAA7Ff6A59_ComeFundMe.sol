...
contract ComeFundMe is Ownable, Pausable {
    struct Campaign {
        bool isAlive; // Is there an ongoing campaign?
        address initiator;
        uint256 fundsRaised; // Total amount of money raised
        string title; // Blockchain is not the best place to store these. Usually takes a lot of space
        string description;
    }
    mapping(bytes32 => Campaign) private _campaigns; //Point from campaign ID (bytes32) to the campaign struct
    ...
    function togglePause() external onlyOwner {
        paused() ? _unpause() : _pause(); // you are asking "is it paused"?
        // Is it something __ ? If yes: If no
        //If the first part is true, then it returns the first, if false then the second
    }
    ...
    function startCampaign(
        ...
        string calldata description //does it save data correctly
    ) external whenNotPaused {
        bytes32 campaignId = getCampaignId(_msgSender(), title, description);
        Campaign storage c = _campaigns[campaignId];
        require(!c.isAlive && c.initiator == address(0), "Campaign exists"); //check if campaign is alive and initiator is equal to zero. If either aren't true, then it reverts
        c.isAlive = true;
        c.initiator = _msgSender(); //why _msg.sender, because when inheriting info from OpenZeppelin you should always use _msg.sender because it may give you the relayer instead
        c.title = title;
    ...
    function endCampaign(bytes32 campaignId) external whenNotPaused {
        // only the initiator can end it, have to check that it is not already ended
        Campaign storage c = _campaigns[campaignId];
        require(c.initiator == _msgSender(), "Not campaign owner");
        require(c.isAlive, "Already ended");
        c.isAlive = false;
        payable(c.initiator).transfer(c.fundsRaised); // sending all the money to the initator aka the sender
    ...
    function donateToCampaign(
        bytes32 campaignId // method to donate to the campaign
    ) external payable whenNotPaused {
        Campaign storage c = _campaigns[campaignId];
        require(c.isAlive, "Already ended"); // has to be alive, filters out ended/non-existent campaigns
        c.fundsRaised += msg.value; // adding msg.value to the c.fundsRaiser
        emit CampaignDonationReceived(campaignId, _msgSender(), msg.value); //emits the event
    }
