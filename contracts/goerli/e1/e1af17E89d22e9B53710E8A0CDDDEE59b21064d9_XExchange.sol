    ...
    XExchangeStorage
{
    ...
    function matchAskWithTakerBidUsingWeth(
        ...
        require(makerAsk.currency == address(weth), "XE: weth only");
        ...
        if (fulfillment.price > msg.value) {
            weth.safeTransferFrom(
                takerBid.taker,
                address(this),
                fulfillment.price - msg.value
            );
        } else {
    ...
    function cancelMultipleOrders(bytes32[] calldata orderItemHashes)
        external
        override
    {
        for (uint256 i = 0; i < orderItemHashes.length; i++) {
            require(!orderStatus[orderItemHashes[i]], "XE: orderItemHash used");
            orderStatus[orderItemHashes[i]] = true;
        }
    ...
    function _transferFeesFromExchange(
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            if (fees[i] != 0 && receivers[i] != address(0)) {
                remainingValue -= fees[i];
                IERC20Upgradeable(currency).safeTransfer(receivers[i], fees[i]);
            }
        }
        (receivers, fees) = royaltyEngine.getRoyalty(
            collection,
        ...
        IERC20Upgradeable(currency).safeTransfer(to, remainingValue);
    }
...
interface IXExchange {
    ...
    event CancelMultipleOrders(bytes32[] orderHashes);
    ...
    function cancelMultipleOrders(bytes32[] calldata orderHashes) external;
}
