...
contract TeleptPassMerkle is ERC721URIStorage, Ownable {
    ...
    function mintNFT(
        ...
        return mintInternal(msg.sender, proof);
    }
    ...
    function mintInternal(
        ...
        if (requireInvitation) {
            require(
                checkIsAddressInTree(to, proof),
                "not in the invitation list of the merkle tree"
            );
        }
    ...
    function airdrop(
        ...
        for (uint256 i = 0; i < wAddresses.length; i++) {
            mintInternal(wAddresses[i], proof);
        }
    }
    ...
    function getTreeLeaf(address account) public pure returns (bytes32) {
    ...
    function checkIsAddressInTree(
        address account,
        bytes32[] calldata proof
    ) public view returns (bool) {
        return verifyTree(getTreeLeaf(account), proof);
    }
