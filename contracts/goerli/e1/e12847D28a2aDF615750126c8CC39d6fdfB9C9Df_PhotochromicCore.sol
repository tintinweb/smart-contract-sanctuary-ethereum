...
contract PhotochromicCore is Ownable {
    ...
    struct DomainInfo {
        // The base node of the user identity.
        bytes32 baseNode;
        // The user identifier for which the the ticket was bought. The mapping between a user id and their
        // corresponding address can be found in `userIdToAddress`.
        string userId;
    ...
    function upgradeResolver(address _resolver) external onlyOwner {
        require(_resolver != address(0));
        resolver = PhotochromicResolver(_resolver);
    }

    function setTicketValidity(uint256 _ticketValidity) external onlyOwner {
        ticketValidity = _ticketValidity;
    }
    ...
    function setPricePerYear(uint256 _pricePerYear) external onlyOwner {
        pricePerYear = _pricePerYear;
    }

    function lastLiveness(bytes32 node) public view returns (uint32) {
    ...
    /**
      ...
      */
    function renew(bytes32 node, uint256 yrs) public payable {
    ...
    function updateSignerAddress(address _photochromicSignerAddress) external onlyOwner {
        photochromicSignerAddress = _photochromicSignerAddress;
    }
    ...
    function decodeDomainInfo(bytes memory bs) public pure returns (DomainInfo memory) {
        DomainInfo memory domainInfo;
        if (bs.length < 39) return domainInfo;

        // node
        bytes32 node;
        assembly {
            node := mload(add(bs, 32))
        }
        domainInfo.baseNode = node;

        // purchaseTime
        domainInfo.purchaseTime = (uint32(uint8(bs[32])) << 24) | (uint32(uint8(bs[33])) << 16)
                                | (uint32(uint8(bs[34])) << 8)  |  uint32(uint8(bs[35]));
        domainInfo.yrs = uint8(bs[36]);

        // profile
        domainInfo.profileNum = uint8(bs[37]);

        // userId
        bytes memory userId = new bytes(bs.length - 38);
        for (uint256 i = 38; i < bs.length; i++) {
            userId[i - 38] = bs[i];
        }
    ...
    /**
     * Sets the profile prices (in USD).
     */
    function setProfiles(Profile[] calldata _details) external onlyOwner {
        require(_details.length != 0);
        delete profiles;
        for (uint i = 0; i < _details.length; i++) {
            profiles.push(Profile({name: _details[i].name, price:_details[i].price, info: _details[i].info}));
        }
    }
    ...
    function purchase(
        ...
        // Is there already a ticket for this userId?
        bytes32 node = namehash(baseNode, userId);
        if (baseNode != registrar.baseNode()) {
            // The sender needs to be the owner of the node if it is not a
            // PhotoChromic identity.
            require(ens.owner(node) == msg.sender);
        }

        // Does someone already own a ticket for this userId?
        // This check is ignored if the no address owns the userId yet.
        ...
        if (expiryTime != 0) {
            // Someone owns the user id already...
            // Did it expire already or is the userId still inside its grace period?
            require(
            ...
            // It did expire!
            _burn(node, baseNode, userId, ens.owner(node));
        }

        // Does the user already own a ticket for a userId (any, can be different from this one) and is it still valid?
        ...
        if (bytes(domainInfo.userId).length != 0) {
            ...
            _burnTicket(namehash(domainInfo.baseNode, domainInfo.userId), msg.sender);
        }

        // Create a new ticket for the given userId.
        nodeToAddress[node] = msg.sender;
        uint32 currentTime = uint32(block.timestamp);
        addressToDomainInfo[msg.sender] = encodeDomainInfo(currentTime, yrs, uint8(getProfileNum(profile)), baseNode, userId);
        emit Ticket(msg.sender, node, userId, profile, yrs, currentTime);
    }

    // available checks whether the given node is still available.
    ...
    // isValidTicket checks whether there is a valid userId ticket for the given requester.
    // 1. The requester != 0x00.
    // 2. There is a ticket owned by the requester.
    // 3. The ticket is still valid.
    function isValidTicket(bytes32 node, address requester) public view returns (bool) {
        address addresOwningUserId = nodeToAddress[node];
        DomainInfo memory domainInfo = decodeDomainInfo(addressToDomainInfo[addresOwningUserId]);
    ...
    /**
     * Removes the ticket for the given userId.
     */
    function burnTicket(bytes32 baseNode, string memory userId) external {
        bytes32 node = namehash(baseNode, userId);
    ...
    function _burnTicket(bytes32 node, address owner) internal {
        delete nodeToAddress[node];
        delete addressToDomainInfo[owner];
    }

    /**
     * Burns the userId at the registrar.
     */
    function burn(bytes32 baseNode, string memory userId) external {
        bytes32 node = namehash(baseNode, userId);
        ...
        _burn(node, baseNode, userId, nodeOwner);
    }
    ...
    function _burn(bytes32 node, bytes32 baseNode, string memory label, address owner) internal {
        registrar.burn(label, baseNode);
        registrar.removeNode(owner);
        resolver.clearPCRecords(node);
        resolver.deleteValidityInfo(node);
    }
    ...
    function getTicketAddress(bytes32 node) public view returns (address) {
        return nodeToAddress[node];
    }

    function getTicketNode(address userAddress) public view returns (bytes32 baseNode, string memory label) {
        DomainInfo memory domainInfo = decodeDomainInfo(addressToDomainInfo[userAddress]);
        return (domainInfo.baseNode, domainInfo.userId);
    }

    /**
     * Returns the user id corresponding to the {msg.caller} address anywhere in the minting process.
     */
    function getTicketNode() public view returns (bytes32 baseNode, string memory label) {
        return getTicketNode(msg.sender);
    }

    /**
     * Returns the profile corresponding to the ticket of {msg.caller} address.
     */
    function getTicketProfile() public view returns (string memory profile, uint8 yrs) {
        DomainInfo memory domainInfo = decodeDomainInfo(addressToDomainInfo[msg.sender]);
    ...
    /**
     * Creates a the given userId if the sender has a ticket for the given userId and the signature matches
    ...
    function mint(
        ...
        bytes32 node = namehash(data.baseNode, data.userId);
        // 1. Check ticket exists for the sender.
        require(nodeToAddress[node] == msg.sender, "need to purchase a ticket first");
        DomainInfo memory domainInfo = decodeDomainInfo(addressToDomainInfo[msg.sender]);
        // 2. Check if not exceeded amount of allowed socials.
        ...
        registrar.register(msg.sender, address(resolver), data.userId, domainInfo.baseNode, data.ipfsHash);
    ...
    function validPhotoChromicRecord(address sender, PhotoChromicRecord calldata data) internal view returns (bool) {
        bytes32 h = keccak256(abi.encode(sender, data.livenessTime, data.contents, data.baseNode, data.userId, data.ipfsHash));
        address signer = ecrecover(h, data.sig.v, data.sig.r, data.sig.s);
        return signer == photochromicSignerAddress;
    }

    function namehash(bytes32 baseNode, string memory label) internal pure returns (bytes32) {
...
contract PhotochromicRegistrar is ERC721Enumerable, Controllable {
    ...
    /// Mapping from tokenId=ENS namehash to IPFS metadata hash.
    mapping(uint256 => bytes32) hashes;
    // owner of photochromic identity to ENS namehash
    ...
    function burn(string calldata userId, bytes32 baseNodeUser) public onlyController {
        bytes32 label = keccak256(abi.encodePacked(userId));
        uint256 tokenId = uint256(keccak256(abi.encodePacked(baseNodeUser, label)));
        _burn(tokenId);
        delete hashes[tokenId];
        if (baseNodeUser == baseNode) {
            ens.setSubnodeRecord(baseNode, label, address(this), address(0), 0);
        }
    }

    function nodeOwnedBy(bytes32 node, address owner) public view returns (bool) {
        return nodes[owner] == node;
    }

    function getNode(address owner) public view returns (bytes32) {
        return nodes[owner];
    }

    function removeNode(address owner) public onlyController {
        delete nodes[owner];
    }
    ...
    function isUserIdAvailable(uint256 userId) public view returns (bool) {
        uint256 namehash = uint256(keccak256(abi.encodePacked(baseNode, bytes32(userId))));
        return available(namehash);
    }

    /**
     * user: photochromic user
     * userId: label (label.photochromic.eth)
     * ipfsHash: pointing to a json adhering to ERC721 Metadata JSON Schema (see https://eips.ethereum.org/EIPS/eip-721)
     */
    function register(
        address user,
        address resolver,
        ...
        bytes32 label = keccak256(abi.encodePacked(userId));
        uint256 tokenId = uint256(keccak256(abi.encodePacked(userBaseNode, label)));
        require(available(tokenId),"ENS name already has a photochromic identity");
        _safeMint(user, tokenId);
        if (userBaseNode == baseNode) {
            ens.setSubnodeRecord(baseNode, label, user, resolver, 0);
        }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        string memory ipfsHash = IPFSTool.ipfsToString(hashes[tokenId]);
...
contract PhotochromicResolver is Resolver {
    ...
    function clearPCRecords(bytes32 node) external onlyOwner {
        ...
        // question: Do we need to emit events for this?
    }

    function setValidatedRecords(
        bytes32 node,
        ValidatedTextRecord[] calldata texts,
        ValidatedAddrRecord[] calldata addresses
    ) external authorised(node) {
        _setValidatedTextRecords(node, texts);
        _setValidatedAddrRecords(node, addresses);
    }
    ...
    function _setValidatedTextRecords(bytes32 node, ValidatedTextRecord[] calldata list) internal {
        ...
        for (uint i = 0; i < list.length; i++) {
            bytes32 h = keccak256(abi.encode(owner, list[i].key, list[i].value, list[i].timestamp));
    ...
    function _setValidatedAddrRecords(bytes32 node, ValidatedAddrRecord[] calldata list) internal {
        address owner = ens.owner(node);
        for (uint i = 0; i < list.length; i++) {
            bytes32 h = keccak256(abi.encode(owner, list[i].coinType, list[i].value, list[i].timestamp));
            address signer = ecrecover(h, list[i].sig.v, list[i].sig.r, list[i].sig.s);
            uint32 t = signer == signerAddress ? list[i].timestamp : 1; // 1 == invalid
     ...
     */
    function isValidNode(bytes32 node) public view returns (bool) {
...
abstract contract ResolverValidated is AddrResolver, TextResolver, Ownable {
    ...
    function validatedText(bytes32 node, string calldata key) public view returns (ValidationStatus, string memory, uint32) {
        ...
        if (bytes(value).length == 0 && keccak256(abi.encodePacked(key)) == keccak256("avatar")) {
            ...
            return (ValidationStatus.VALIDATED, string(abi.encodePacked("eip155:1/erc721:", a, "/", nodeString)), 0);
        }
