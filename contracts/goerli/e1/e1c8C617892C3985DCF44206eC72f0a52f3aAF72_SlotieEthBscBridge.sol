...
contract SlotieEthBscBridge is Ownable, Pausable {
    ...
    event WattsReceived(address indexed user, uint256 amount, uint256 timestamp);
    ...
    function bridgeWattss(uint256 amount) external whenNotPaused {
    ...
    function _handleReceive(address user, uint256 amount, bytes32 randomHex, bytes memory signature) internal {
        ...
        emit WattsReceived(user, amount, block.timestamp);
    }
