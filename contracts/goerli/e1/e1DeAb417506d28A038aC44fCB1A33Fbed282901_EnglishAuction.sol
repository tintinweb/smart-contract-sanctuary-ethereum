...
contract EnglishAuction is Ownable {
    ...
    event Start(uint indexed bidId, uint indexed nftId, address indexed seller, uint startTime, uint duration);
    event Cancel(uint indexed bidId);
    event Bid(uint indexed bidId, address indexed bidder, uint bidAmount, uint time, uint _amount, uint indexed nftId);
    event End(uint indexed bidId, address indexed highestBidder, uint highestBid);
    event ClaimRefund( uint indexed bidId, address indexed Bidder, uint RefundAmount);
    ...
    function initNFT(IERC721 initNFTAddr) public onlyOwner {
        nft = initNFTAddr;
    }
    ...
    function doBid(uint _bidId, uint _amount) external {
        BidInfo storage bid = bids[_bidId];
        require(_bidId <= _bidIdCount.current(), "EA: The bid does not exist");
        ...
        emit Bid(_bidId, _msgSender(), _amount, block.timestamp, _amount, bid.nftId);        
    }
