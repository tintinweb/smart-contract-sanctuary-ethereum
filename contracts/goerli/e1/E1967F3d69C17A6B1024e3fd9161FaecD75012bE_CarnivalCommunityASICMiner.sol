...
contract CarnivalCommunityASICMiner is ReentrancyGuard {
    ...
    uint256 public constant MIN_CARN_DEPOSIT = 10 * 1e12;
    ...
    constructor(
        ...
        nextMiningStartTime = block.timestamp + 21 days;
        currentSessionId = 1;
    }

    // Functions
    // Deposit function
    function deposit(uint256 _asicAmount, uint256 _carnAmount)
    public
    payable
    nonReentrant
    {
        ...
        require(
            _carnAmount >= MIN_CARN_DEPOSIT,
            "At least minimum CARN deposit required"
        );
        ...
        emit Deposit(msg.sender, currentSessionId, _asicAmount, _carnAmount);
    ...
    // function to trigger the mining session
    ...
    function startReloadPeriod() public {
        ...
        // PulseBitcoin.MinerStore[] memory _minerStore = PulseBitcoin(PLSB).minerList(address(this));
        uint40 _minerId;
    ...
    // function to claim rewards once mining ends
    ...
    // function to deposit CARN tokens to the trapped ASIC release pool
    function depositCARNToTrappedPool(uint256 _carnAmount)
    external
    nonReentrant
    {
