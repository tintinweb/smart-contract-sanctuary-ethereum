...
/// @title Soulbound Tokens for Scalene Academy (e.g. certificates)
/// @author Scalene
...
contract ScaleneSoulboundTokens is ERC1155, Ownable2Step {
    /// merkle roots for each token
    mapping(uint256 => bytes32) public whitelists;
    /// whether an user has already minted
    ...
    constructor(string memory uri_) ERC1155(uri_) Ownable2Step() {}

    /// @dev We overwrite the ERC1155 standard because we're unable to follow it: https://docs.openzeppelin.com/contracts/4.x/api/token/erc1155#ERC1155-_setURI-string-
    function uri(uint256 id) public view override returns (string memory) {
        return uris[id];
    }

    function setUriForToken(uint256 id, string calldata newuri) external onlyOwner {
        require(bytes(uris[id]).length == 0, "ScaleneSoulboundTokens: Token URI can only be set once");
        uris[id] = newuri;
    }
    ...
    function mintOneToSelf(uint256 id, bytes32[] calldata _merkleProof) external {
        require(isWhitelisted(id, _merkleProof), "ScaleneSoulboundTokens: can't mint, you must be whitelisted");
        require(hasMinted[id][msg.sender] == false, "ScaleneSoulboundTokens: can't mint, you've already minted");
        // mark as minted
    ...
    function _beforeTokenTransfer(
        ...
        require(
            // can only mint
            from == address(0),
            "ScaleneSoulboundTokens: The tokens are soulbound. They cannot be transferred - only minted"
        );
    }
}
