...
contract Kore is iKore, protected {
    ...
    function addLiquidity(address token_1, address token_2, 
        ...
        if (token_1 == WETH) {
            require(msg.value == amount_1, "KORE: Wrong amount of ETH sent");
        } else {
            (bool _success) = IERC20(token_1).transferFrom(msg.sender, address(this), amount_1);
            require(_success, "KORE: Transfer failed");
        }
        if (token_2 == WETH) {
            require(msg.value == amount_2, "KORE: Wrong amount of ETH sent");
        } else {
            (bool _success) = IERC20(token_2).transferFrom(msg.sender, address(this), amount_2);
            require(_success, "KORE: Transfer failed");
        }
        return liquidity;
    ...
    function removeLiquidity(address token_1, address token_2, 
        ...
        require(liquidities[liquidity].token_1 != address(0), "KORE: Liquidity does not exist");
        // Checking if the liquidity provider owns enough tokens
        uint actual_owned_token_1;
        uint actual_owned_token_2;
        (actual_owned_token_1, actual_owned_token_2) = _percentageToOwnedTokens(liquidity, msg.sender);
        require(actual_owned_token_1 >= amount_1 && actual_owned_token_2 >= amount_2, "KORE: Not enough tokens");
        // Removing liquidity
        liquidities[liquidity].amount_1 -= amount_1;
        liquidities[liquidity].amount_2 -= amount_2;
        // Calculate the percentage of liquidity owned by the liquidity provider
        ...
        if (token_1 == WETH) {
            ...
            (bool _success) = IERC20(token_1).transfer(msg.sender, amount_1);
            require(_success, "KORE: Transfer failed");
        }
        if (token_2 == WETH) {
            ...
            (bool _success) = IERC20(token_2).transfer(msg.sender, amount_2);
            require(_success, "KORE: Transfer failed");
        }
        // Emitting event
    ...
    function swap(address token_1,
        ...
        // Transferring the tokens (native if weth)
        if (token_1 == WETH) {
            require(msg.value == amount, "KORE: Wrong amount of ETH sent");
        } else {
            (bool _success) = erc20_in.transferFrom(msg.sender, address(this), amount);
            require(_success, "KORE: Transfer failed");
        }
        ...
        // Sending the tokens to the user (native if weth)
        ...
        if (token_2 == WETH) {
            ...
            (bool _success) = erc20_out.transfer(msg.sender, amountOut);
            require(_success, "KORE: Transfer failed");
        }
        // Emitting the event
