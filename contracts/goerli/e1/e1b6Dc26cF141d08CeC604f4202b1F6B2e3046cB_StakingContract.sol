...
contract StakingContract is Ownable, ReentrancyGuard {
    ...
    constructor(address _rToken, address _nftAddress) {
        rtoken = IERC20(_rToken);
        allowedToStake[_nftAddress] = true;
    }
    ...
    function fullReward1collection(
        ...
        IERC721Enumerable _nft = IERC721Enumerable(_collection);
        //how many they own
        uint256 howMany = ERC721(_collection).balanceOf(_user);
        uint256[] memory whichIds = new uint256[](howMany);

        uint256 fullPendingRewards1Collection;
        //then do a loop of that many times getting token
        for (uint256 i = 0; i < howMany; i++) {
            whichIds[i] = _nft.tokenOfOwnerByIndex(_user, i);
        }
        for (uint256 i = 0; i < howMany; i++) {
            fullPendingRewards1Collection += pendingRewardNew(
                _collection,
                whichIds[i]
            );
        }
        return fullPendingRewards1Collection;
    }
    ...
    function FullPendingTotal(
        ...
        for(uint256 i = 0; i < _collection.length; i++){
            ...
            IERC721Enumerable _nft = IERC721Enumerable(_collection[i]);
            //how many they own
            ...
            for (uint256 x = 0; x < howMany; x++) {
                whichIds[x] = _nft.tokenOfOwnerByIndex(_user, x);
            }
            for (uint256 x = 0; x < howMany; x++) {
