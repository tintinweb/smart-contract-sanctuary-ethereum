...
contract Lease is LeaseInterface, Manageable, ReentrancyGuard {
     ...
     * @param startTimestamp  lease creation timestamp in seconds
     * @param endTimestamp lease end timestamp in seconds
     * @param endTimestamp lease end timestamp in seconds
    ...
    function lease(
        ...
        require(verify(businessId, NFTAddr, NFTId, FTAddr, startTimestamp, endTimestamp, managerSign));
        ...
        leaseList.startTimestamp = startTimestamp;
        leaseList.endTimestamp = endTimestamp;

        emit Lease(businessId, NFTAddr, NFTId, FTAddr, startTimestamp,endTimestamp);
    ...
    function verify(
        ...
        bytes32 hash = keccak256(abi.encode(businessId, NFTAddr, NFTId, FTAddr, startTimestamp, endTimestamp, _msgSender()));
    ...
    * @notice Settle the rent of a month. Only manager can call this function.
    ...
    function _withdrawNFT(
        ...
        if (leaseList.endTimestamp <= block.timestamp || leaseList.startTimestamp >= block.timestamp) {// 正常到期
            if (FTAmount != 0) {
                revert NOTChangePenalty();
            }
        } else {// 提前解除
            require(FTInterface(FTAddr).transferFrom(_msgSender(), leaseCapitalPool, FTAmount), "FT transfer failed");
            CapitalPoolInterface(leaseCapitalPool).depositFT(businessId, FTAddr, FTAmount);
        }
    ...
    function withdrawNFTManager(
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            if (leaseList.endTimestamp <= block.timestamp || leaseList.startTimestamp >= block.timestamp) {
...
interface LeaseInterface is LeaseStructs {
    ...
    event Lease(string businessId, address NFTAddr, uint256 NFTId, address FTAddr, uint256 startTimestamp, uint256 endTimestamp);
     ...
     * @param FTAddr Fungible Token to lease
     * @param startTimestamp  lease creation timestamp in seconds
     * @param endTimestamp Rental term in seconds
     * @param managerSign manager Sign
     */
    function lease(
        string calldata businessId,
...
contract CapitalPool is CapitalPoolInterface, Manageable, ReentrancyGuard {
    ...
    * @notice Only manager can withdraw a FT. Only the manager can call this function.
    ...
    function withdrawFTManager(
        ...
        balance = FTInterface(tokenAddr).balanceOf(address(this));
        balanceOf[tokenAddr] = balance;
        emit WithdrawFT(businessId, tokenAddr, tokenAmount, recipient);
        return true;
    }
 ...
 */
 ...
 interface LeaseStructs {
        ...
        struct LeaseList{
            ...
            uint256 startTimestamp;
            uint256 endTimestamp;// Rental term in seconds
        }
