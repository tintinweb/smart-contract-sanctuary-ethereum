...
import "./interfaces/IDebtCacheBridge.sol";
...
contract BaseDebtCache is Owned, MixinSystemSettings, IDebtCache {
    ...
    bytes32 private constant CONTRACT_DEBTCACHE_BRIDGE = "DebtCacheBridge";
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[5] = CONTRACT_ISSUED_SYNTH_AGGREGATOR;
        newAddresses[6] = CONTRACT_DEBTCACHE_BRIDGE;
    ...
    function debtCacheBridge() internal view returns (IDebtCacheBridge) {
        return IDebtCacheBridge(requireAndGetAddress(CONTRACT_DEBTCACHE_BRIDGE));
    }
        ...
        returns (uint256[] memory values)
    {
        ...
        for (uint256 i = 0; i < numValues; i++) {
            ISynthAggregator syAggregator = issuedSynthAggregator().syAggregators(currencyKeys[i]);
            require(address(syAggregator) != address(0), "Synth does not exist");
            uint256 supply = syAggregator.totalSupply();
    ...
    function _onlyIssuerOrExchanger() internal view {
        require(
            msg.sender == address(issuer()) || msg.sender == address(exchanger()) || msg.sender == address(debtCacheBridge()),
    ...
    function _onlyDebtIssuer() internal view {
        ...
        require(isOwner, "Only debt issuers may call this");  // || isWrapper
    }
...
contract DebtCache is BaseDebtCache {
    ...
    function updateCachedSynthDebtWithRate(bytes32 currencyKey, uint256 currencyRate) external onlyIssuer {
        ...
        if (msg.sender != address(debtCacheBridge())) {
            debtCacheBridge().updateCachedSynthDebtsWithRates(synthKeyArray, synthRateArray);
        }
    }
        ...
        onlyIssuerOrExchanger
    {
        ...
        if (msg.sender != address(debtCacheBridge())) {
            debtCacheBridge().updateCachedSynthDebtsWithRates(currencyKeys, currencyRates);
        }
    }
