...
contract ECDSASignature2 is IECDSASignature2 {
    using ECDSA for bytes32;
    uint256 constant _timePeriodInSeconds = 1800; //30 minutes
    ...
    function makeHashToSign(
        ...
        require(checkNonce(nonce) == false, "nonce was already used");
    ...
    function signatureStatus(
        ...
        bool result = verifyByTheSigners(
    ...
    function verifyByTheSigners(
        bytes32 markedHash,
        bytes[] memory signatures
    ) private view returns (bool) {
        if (signatures.length < signers.length)
            revert("Signatures are missing");
        ...
        if (validSignatures == signatures.length) return true;
