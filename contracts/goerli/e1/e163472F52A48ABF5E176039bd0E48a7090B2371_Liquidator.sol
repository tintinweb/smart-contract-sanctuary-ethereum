...
contract Liquidator is Owned, MixinSystemSettings, ILiquidator {
    ...
    function isLiquidationOpen(address account, bool isSelfLiquidation) external view returns (bool) {
        uint256 accountCollateralisationRatio = issuer().collateralisationRatio(account);
    ...
    function calculateAmountToFixCollateral(
        ...
        uint256 divisor = unit.sub(unit.add(penalty).multiplyDecimal(ratio));
    ...
    function flagAccountForLiquidation(address account) external {
        ...
        uint256 accountsCollateralisationRatio = issuer().collateralisationRatio(account);

        // if accounts issuance ratio is greater than or equal to liquidation ratio set liquidation entry
        require(accountsCollateralisationRatio >= getLiquidationRatio(), "Account issuance ratio is less than liquidation ratio");

        uint256 deadline = now.add(getLiquidationDelay());
    ...
    function _storeLiquidationEntry(address _account, uint256 _deadline, address _caller) internal {
