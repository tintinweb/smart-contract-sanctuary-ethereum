...
contract YonkoStaking is Ownable, Pausable, ReentrancyGuard{
    ...
    uint256 public rewardCycle = 30 days / 12; // 1 block take approx 12 sec to mine on etherscan
    ...
    uint256 public rewardPeriod = 5 minutes / 12;
    ...
    function updateReward(uint256[4] memory _rewardForRarity, uint256 _rewardCycle) external onlyOwner {
        require(rewardCycle > 0, "Invalid setting");
        rewardForRarity = _rewardForRarity;
    ...
    function updateBonusRate(uint256 _bonusRate, uint256 _bonusPeriod) external onlyOwner {
        require(_bonusRate >= 0 && _bonusPeriod > 0, "Invalid setting");
    ...
    function depositAction(uint256 tokenID, uint256 minTokenAmount) internal {
        ...
        require(block.number > lastDepositBlock, "Current time is before last deposit time.");
    ...
    function rewardForTokenInUSD(address user, uint256 tokenID) public view returns(uint256) {
        UserInfo storage info = userInfos[user];
        uint256 stakedPeriod = (block.number-info.staked[tokenID])/rewardPeriod;
        ...
        return (stakedPeriod*rewardPeriod*rewardPerCycle*bonus/(rewardCycle*10000));
    }
    ...
    function claimAction(uint256 extraTokenAmount) internal{
        UserInfo storage info = userInfos[msg.sender];

        uint256 rewardAmount = pendingRewardInUSD(msg.sender);
        if(rewardAmount == 0) {
            if(extraTokenAmount > 0)
                rewardToken.transfer(msg.sender, extraTokenAmount);
            return;
        }
        ...
        if(rewardTokenBalance < rewardAmount + extraTokenAmount) revert("Not enought reward token balance");
        info.totalWithdrawn += rewardAmount;
        rewardToken.transfer(msg.sender, rewardAmount + extraTokenAmount);
