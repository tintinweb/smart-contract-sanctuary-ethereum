...
contract FundraiserFactory is Ownable {
    ...
    // Compte le nombre de levée de fond
    function createFundraiser (string memory name, string memory url, string memory imageURL, string memory description, address payable beneficiary) public onlyOwner {
...
contract Fundraiser is Ownable {
    ...
    mapping(address => bool) private _donors;
    ...
    function donate(string memory _message) public payable {
        require(msg.value > 0, "You can't invest 0 ! ");
        Donation memory donation = Donation({
            value: msg.value,
            date: block.timestamp,
            // PEUT ETRE DESCRIPTION
            message: _message
        ...
        // si la personne a deja donné alors on ne fait pas donationsCount++
         if (!_donors[msg.sender]) {
        // Increment donationsCount if donor has not made a donation before
        donationsCount++;
        // Mark donor as having made a donation
