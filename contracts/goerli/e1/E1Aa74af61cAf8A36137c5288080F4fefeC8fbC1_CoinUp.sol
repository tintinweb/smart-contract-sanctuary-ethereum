...
contract CoinUp is AccessControl  {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    address public nft = 0xEffD96FFA527EFaa9e27c19F526ad218Ae085633;
    ...
    uint256 public unconditional;
    uint256 public unconditionalNumber;
    uint256 public quota;
    // uint256 public oversold;
    uint256 public activateNumber;
    mapping(address=>uint256) public userQuota;
    mapping(uint256=>uint256) public oversold;
    // mapping(uint256=>uint256) public activityNumber;
    ...
    event SetUnconditional(address indexed sender,uint256 number,uint256 oldNumber);
    event SetQuota(address indexed sender,uint256 number,uint256 oldNumber);
    event SetNft(address indexed sender,address newNft,address oldNft);
    ...
    function safeMint(uint256 class,uint256 number,bytes32[] calldata proofs) payable external {
        ...
        if(unconditionalNumber<unconditional){
            unconditionalNumber = unconditionalNumber.add(number);
        }else{
            bytes32 _leaf = keccak256(abi.encodePacked(_msgSender()));
            bool _verify = MerkleProof.verify(proofs, root, _leaf);
            
            if(getResidue()<=0 && !_verify){
                revert NonBuy();
            }
            if(getResidue()>0){
                if(number>getResidue()){
                    oversold[getDay()] = oversold[getDay()-1].add(number-getResidue());
                }
                activateNumber = activateNumber.add(number);
            }else if(_verify){
                require((userQuota[_msgSender()]+number)<=quota,"Whitelisted limit buy");
                userQuota[_msgSender()] = userQuota[_msgSender()].add(number);
            }else{
                revert NonBuy();
            }
           
        }
        tokenIds =  NFT(nft).safeMint{value:msg.value}(_msgSender(), class, number);
    ...
    function setGuestInfo(GuestInfo memory gInfo) external onlyRole(MINTER_ROLE) {
        require(NFT(nft).getTotalSurplus() >= gInfo.max,"exceed the limit");
        require(gInfo.max>0,"number fail");
        guestInfo = gInfo;
        emit SetGuestInfo(_msgSender(),gInfo);
    }

    function setNft(address  _nft) external onlyRole(MINTER_ROLE) {
        require(_nft!=address(0),"nft address fail");
        require(_nft!=nft,"nft address is not dif");
        emit SetNft(_msgSender(), _nft, nft);
        nft = _nft;
    }
    function setUnconditional(uint256 number) external onlyRole(MINTER_ROLE) {
            emit SetUnconditional(_msgSender(), number, unconditional);
            unconditional = number;
    }
    function setQuota(uint256 number) external onlyRole(MINTER_ROLE) {
            emit SetQuota(_msgSender(), number, quota);
            quota = number;
    }
    ...
    function getResidue() public view returns(uint256) {
        if(getActiveTotal()>activateNumber){
            return getActiveTotal().sub(activateNumber);
        }
        return uint256(0);
    }

    function getResidueMints() external view returns(uint256) {
        if(startTime == 0){
            return 0;
        }
        uint256 resideue = 0;
        if(unconditional>unconditionalNumber){
            resideue = unconditional.sub(unconditionalNumber);
        }

        return getResidue().add(resideue);
    }

    function getActiveTotal() public view returns(uint256) {
        uint256 activeNumber = getDay().mul(guestInfo.max);
        if(getDay()==0){
            return activeNumber;
        }
        
        return activeNumber.add(oversold[getDay()-1]);
    }

    function getDay() public view returns(uint256){
        if(guestInfo.startTime==0){
            return 0;
        }
        return block.timestamp.div(guestInfo.startTime);
    }
...
contract NFT is ERC721PresetMinterPauserAutoId {
    ...
    string private BaseUri = "ipfs://bafybeia3yimpigjas2q5kk4bwu7yguoq4nxaxf7kp6pvzaoifzj47bvy7a/";
    ...
    modifier txOwnerExists(uint _txId){
        require(_txId<ownerVotes.length,"Owner tx dows not exist");
        _;
    }
    ...
    function safeMint(address sender,uint256 class,uint256 number) payable public onlyRole(MINTER_ROLE) returns(uint256[] memory) {
        ...
        tokenIds = inerMint(sender,class, number);
        emit SafeMint(_msgSender(),sender,tokenIds,number,block.timestamp);
        return tokenIds;
    }


    function inerMint(address sender,uint256 class,uint256 number) private returns(uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](number);
    ...
    function getOwnerVoteCount() external view returns(uint256){
        return ownerVotes.length;
    }
