    ...
    UUPSUpgradeable
{
    ...
    function initialize(string memory baseUri, uint256 blockTransferUntil)
        public
        initializer
    {
        __ERC721_init("Cepheus Vega", "VEGA");
        ...
        _baseUri = baseUri;
        _blockTransferUntil = block.timestamp + blockTransferUntil;
    }
    ...
    function extendBlockTime(uint256 extendTimeInSecond) external {
        _blockTransferUntil += extendTimeInSecond;
    }

    function decreaseBlockTime(uint256 decreaseTimeInSecond) external {
        _blockTransferUntil -= decreaseTimeInSecond;
    }
