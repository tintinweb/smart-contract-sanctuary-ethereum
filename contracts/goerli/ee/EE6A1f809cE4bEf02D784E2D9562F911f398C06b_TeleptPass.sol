...
contract TeleptPass is ERC721URIStorage, Ownable {
    ...
    string public baseURI = "https://pass.telept.xyz/token/";
    ...
    function mintNFT() public payable returns (uint256) {
        ...
        if (requireInvitation) {
            require(invitationList[msg.sender], "not in the invitation list");
        }
        ...
        if (maxMintCount > 0) {
            require(
                mintCount[msg.sender] < maxMintCount,
        ...
        _setTokenURI(
            newItemId,
            string(abi.encodePacked(baseURI, Strings.toString(newItemId)))
        );
    ...
    function addToInvitationList(address[] memory addresses) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            invitationList[addresses[i]] = true;
        }
    }

    function removeFromInvitationList(address[] memory addresses)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
    ...
    function setBaseURI(string memory baseURIUpdate) public onlyOwner {
        baseURI = baseURIUpdate;
    }
