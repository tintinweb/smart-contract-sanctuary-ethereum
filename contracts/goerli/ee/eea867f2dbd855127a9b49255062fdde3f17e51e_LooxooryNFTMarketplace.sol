...
contract LooxooryNFTMarketplace is ReentrancyGuard,Ownable {
    ...
    function createCampaign(
      CreateCampaignStruct memory _campaignDTO
        ...
        require(filteredNfts.length>0,"Provided NFTs are already added in another campaign");
  ...
  function removeUnsoldNfts(uint256 _campaignId) external onlyOwner {
      require(campaigns[_campaignId].campaignID != 0,"Invalid campaign id");

      uint8[] memory emptyArr = new uint8[](0);
      filteredNfts = emptyArr;
      for(uint256 i=0;i<campaigns[_campaignId].nfts.length;i++){
        if(!nftItems[campaigns[_campaignId].nfts[i]].sold){
          nftItems[campaigns[_campaignId].nfts[i]].campaignID = 0;
        }else{
          filteredNfts.push(campaigns[_campaignId].nfts[i]);
        }
      }

      campaigns[_campaignId].nfts = filteredNfts;
      campaigns[_campaignId].totalNfts = filteredNfts.length;
  }
}