...
contract PolygonBSG {
        ...
        if (upline != address(0)) {
            ...
            if(userInfo[upline].membership >= 4){
                ...
                if(userInfo[upline].level < 5){
                    upRewards.capitals = upRewards.capitals
            ...
            }else if(userInfo[upline].membership >= 4){
                //blue diamond reward
                upRewards.capitals = upRewards.capitals
    ...
    function _updateMembership(address _user) private {
        ...
        if(user.teamNum >= 2 && user.teamTotalDeposit >= user.maxDeposit.mul(2)){
            if(user.maxDeposit >= 2500e6 && user.teamNum >= 400 && 
            user.teamTotalDeposit >= 15000e6 && user.teamTotalVolume >= 500000e6){
                user.membership = 4;
            }else if(user.maxDeposit >= 2500e6 && user.teamNum >= 180 && 
            user.teamTotalDeposit >= 10000e6 && user.teamTotalVolume >= 100000e6){
                user.membership = 3;
            }else if(user.maxDeposit >= 1000e6 && user.teamNum >= 40 && 
            user.teamTotalDeposit >= 10000e6 && user.teamTotalVolume >= 100000e6){
                user.membership = 2;
            }else{
                user.membership = 1;
    ...
    function _checkClaimable(address _user) private returns(bool, uint256){
        OrderInfo[] memory orderlist = orderInfos[_user];
        uint256 _now = block.timestamp;
        uint256 i;
        for(i = 0;i < orderlist.length;i++){
            OrderInfo memory _order = orderlist[i];
            if(!_order.isClaimed && _order.cycle == rewardInfo[_user].cycleNumber.sub(1) && _now >= _order.unfreeze){
                return (true,i);
            }
        }
        return (false,i);
