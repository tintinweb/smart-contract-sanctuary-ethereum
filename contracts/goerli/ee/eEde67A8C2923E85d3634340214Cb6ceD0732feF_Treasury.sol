 ...
 * https://blog.openzeppelin.com/reentrancy-after-istanrose/[Reentrancy After Istanrose].
 */
...
interface IBoardroom {
    ...
    function getRosePrice() external view returns (uint256);
...
contract Treasury is ContractGuard {
    ...
    function setBootstrap(uint256 _bootstrapSupplyExpansionPercent) external onlyOperator {
    ...
    function getRoseCirculatingSupply() public view returns (uint256) {
        IERC20 roseErc20 = IERC20(rose);
        uint256 totalSupply = roseErc20.totalSupply();
        uint256 lockerSupply = ILocker(locker).totalStakedAmount();
        return totalSupply.add(lockerSupply);
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        uint256 roseSupply = getRoseCirculatingSupply();
        _sendToBoardroom(roseSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
    }
