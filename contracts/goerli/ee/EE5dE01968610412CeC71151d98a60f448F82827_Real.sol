...
contract Real{
    ...
    struct Details{
        ...
        //State variable 1 - Auction , 2 - Listing , 3 - Donation , 4 - Venture 
        uint state;
        uint acquire;
        //uint lastSale;
    }
    ...
    function updateAuction(uint tokenId, uint startValue, uint stepValue) public{
        user storage temp = allUsers[msg.sender];
        Auction storage temp1 = temp.allAuctions[tokenId];
        require(temp1.state == 1, "Auction Not Live");
        require(stepValue > startValue*5/100, "StepValue Must be greater than 5 %");
        temp1.startingValue = startValue;
        temp1.step = stepValue;
    }
    ...
    function assertAuction(uint tokenId, address buyer) public payable{
        require(allUsers[msg.sender].allAuctions[tokenId].state == 1, "Auction Not Live");
        user storage temp = allUsers[msg.sender];
        Auction storage temp1 = temp.allAuctions[tokenId];
    ...
    function endAuction(uint tokenId) external payable{
        user storage temp = allUsers[msg.sender];
        Auction storage temp1 = temp.allAuctions[tokenId];
        temp1.state = 2;
    ...
    function placeBid(address auc, uint tokenId) public payable{
        require(allUsers[auc].allAuctions[tokenId].state == 1 ,"The Auction is not live"); 
    ...
    function cancelBid(address auc, uint tokenId) public payable {
        require(allUsers[auc].allAuctions[tokenId].state == 1, "Auction Not Live");
        user storage temp = allUsers[auc];
        Auction storage temp1 = temp.allAuctions[tokenId];
    ...
    function donateProperty(address reciever, uint tokenId) public{
        //Have to think of a way to confirm if the tokenId is of this venture
        require(VentureDetails[tokenId].state == 4, "Invalid Token ID");
        ERC721(NFTaddress).safeTransferFrom(msg.sender, reciever, tokenId);
    ...
    function buyListing(address auc, uint tokenId) public payable{
        require(allUsers[auc].allListings[tokenId].state == 1, "Listing not Live");
    ...
    function cancelListing(uint tokenId) public{
        require(allUsers[msg.sender].allListings[tokenId].state == 1, "Lisiting not Live");
        user storage temp = allUsers[msg.sender];
        Listing storage temp1 = temp.allListings[tokenId];
        temp1.state = 3;
        ERC721(NFTaddress).safeTransferFrom(address(this), msg.sender, tokenId);
    ...
    function allAuctions() public view returns(string[] memory){
        user storage temp = allUsers[msg.sender];
        string[] memory result = new string[](temp.allTokens.length);
        uint pointer = 0;
        for(uint i=0; i<temp.allTokens.length; i++){
            Auction storage temp1 = temp.allAuctions[temp.allTokens[i]];
            string memory out = "";
            out = string(abi.encodePacked(out, "{", "Starting Value :", temp1.startingValue, ", Step Value :", temp1.step, ", Highest Bidder :", temp1.highestBidder, ", Highest Bidder Value :", temp1.highestBidderValue));
            result[pointer] = out;
            pointer++;
        }
        return result;
    ...
    function allListings() public view returns(string[] memory){
        user storage temp = allUsers[msg.sender];
        string[] memory result = new string[](temp.allTokensListing.length);
        uint pointer = 0;
        for(uint i=0; i<temp.allTokensListing.length; i++){
            Listing storage temp1 = temp.allListings[temp.allTokensListing[i]];
            string memory out = "";
            out = string(abi.encodePacked(out, "{", "Listing Value :", temp1.listingValue, " }"));
            result[pointer] = out;
            pointer++;
        }
        return result;
    ...
    function viewProperties() public view returns(string[] memory){
        string[] memory result = new string[](allVentureDetails.length);
        for(uint i=0; i<allVentureDetails.length; i++){
            Details memory temp = VentureDetails[allVentureDetails[i]];
            string memory out = "";
            out = string(abi.encodePacked(out, "{", "Current Owner :", temp.currentOwner, ", Acquired State :", temp.acquire, "}"));
            result[i] = out;
        }
        return result;
    }

    // function viewWhole() public view returns(string[] memory, string[] memory){
    //     string[] resultAuctions;
    //     string[] memory resultListings;
    ...
    //             string memory out1 = "";
    //             out1 = string(abi.encodePacked(out1, "{", "State :", tempA.state, ", Starting Value :", tempA.startingValue, ", Step :", tempA.step, ", Highest Bidder Address :", tempA.highestBidder, ", Highest Bidder Value :", tempA.highestBidderValue));
    //             resultAuctions.push(out1);
    //         }
    //     }
    // }
}
