...
abstract contract Lockable is Ownable, ERC165, IERC721 {    
    ...
    constructor(string memory tokenName, string memory symbolic) {
        findable.name = tokenName;
        findable.symbol = symbolic;
    }  



    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        address owner = findable.tokens[tokenId].owner;
        if (owner == address(0)) {
    ...
    function getApproved(uint256 tokenId) public view virtual override returns (address) {
        ...
        return findable.tokens[tokenId].approval;
    }            
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return findable.tokens[tokenId].exists;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {     
        ...
        findable.tokens[tokenId].approval = to;
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        findable.tokens[tokenId].owner = to;

        emit Transfer(address(0), to, tokenId);

        findable._addTokenToEnumeration(to,tokenId);
    ...
    function _transfer(
        ...
        delete findable.tokens[tokenId].approval;
        ...
        findable._addTokenToEnumeration(to,tokenId);
        findable._removeTokenFromEnumeration(from,tokenId);
...
contract OSMSubscription is Subscribable {  
    ...
    string private revealedBaseURI="https://private.horde-ai.xyz/api/osm-metadata"; 

    constructor() Subscribable(tokenName,tokenSymbol) {
        subs.promoDiscounts[""] = PromoDiscount(10,0,0x3f1849C19394CfaA0BC85734F3ADEeff155424F5);
    }      

    function standardFee(uint256 numberOfDays) public view returns (uint256) {
        return super.determineFee(numberOfDays, " ");        
    }  
    ...
    function mint(uint numberOfDays) external payable {  
        ...
        validSubscription(numberOfDays);
        uint256 fee = determineFee(numberOfDays," ");
        HORDE_AI_WALLET.transfer(fee);
        _mintTokenFor(msg.sender,numberOfDays);     
    }  

    function discountMint(uint numberOfDays, string calldata discountCode) external payable {  
        if (balanceOf(msg.sender) > 0) {
            revert OnlyOneSubscriptionPerWallet();
        }
        validSubscription(numberOfDays,discountCode);
    ...
    function mintForRecipient(address recipient, uint numberOfDays) external onlyOwner { 
    ...
    function _mintTokenFor(address recipient, uint numberOfDays) internal {
...
import "./SetSubscribable.sol";

abstract contract Subscribable is TokenReceiver {
    using SetSubscribable for SubscribableData; // this is the crucial change
    SubscribableData internal subs;
    event SubscriptionUpdate(uint256 indexed tokenId, uint256 expiration);
    
    address payable internal HORDE_AI_WALLET = payable(0x3f1849C19394CfaA0BC85734F3ADEeff155424F5);

    constructor(string memory tokenName, string memory symbol) Lockable(tokenName, symbol) {
        subs.setSpans(28, 84, 168, 336); // 4 weeks, 12 weeks, 24 weeks, 48 weeks
        subs.setFeeStructure(4, 25, 52); // base multiplier, base discount, annual period
        subs.promoDiscounts[" "] = PromoDiscount(0,0,0x3f1849C19394CfaA0BC85734F3ADEeff155424F5);
    }  
    ...
    function validSubscription(uint256 numberOfDays) internal view {
        subs.validateSubscription(numberOfDays);
    }

    function validSubscription(uint256 numberOfDays, string calldata discountCode) internal view {
    ...
    function addPromoDiscount(string calldata discountCode, uint256 amount, uint256 commission, address partner) external onlyOwner {    
        subs.promoDiscounts[discountCode] = PromoDiscount(amount,commission,partner);
    }   

    function setRateParams(uint256 multiplier, uint256 discount) external onlyOwner {
        subs.setSpans(28, 84, 168, 336);
        subs.setFeeStructure(multiplier, discount, 52);
    }

    function commitSubscription(uint256 tokenId, uint numberOfDays) internal {
        uint256 expiration;
        if (block.timestamp > subs.subscriptions[tokenId]) {
            expiration = block.timestamp + numberOfDays * 1 days;
        } else {
            expiration = subs.subscriptions[tokenId] + numberOfDays * 1 days;
        }        
        subs.subscriptions[tokenId] = expiration;
    ...
    function cancelSubscription(uint256 tokenId) external payable {
        ...
        delete subs.subscriptions[tokenId];
