...
contract Battling is Ownable, SignerVerifiable {
    ...
    function initiateBattleERC20(uint256 _amount, string memory _message, string memory _battle_id, uint256 _deadline, address _erc20_token, bytes memory _signature) external frozen callerVerified(_amount, _message, _battle_id, _deadline, _erc20_token, _signature) {
        require(battle_contestants[_battle_id].player_one == address(0) || battle_contestants[_battle_id].player_two == address(0), "Battle is full");
        ...
        if (battle_contestants[_battle_id].value == 0) { // battle creation
            ...
            require(battle_contestants[_battle_id].player_one != msg.sender, "Cannot join own battle");
        ...
        if (battle_contestants[_battle_id].player_one == address(0)) {
    ...
    function claimWinnings(string memory _message, string memory _battle_id, uint256 _deadline, bytes memory _signature) external frozen callerVerified(battle_contestants[_battle_id].value, _message, _battle_id, _deadline, battle_contestants[_battle_id].token_address, _signature) {
        require(!battle_contestants[_battle_id].paid, "Rewards already claimed for battle");
        require(!battle_contestants[_battle_id].cancelled, "Battle was cancelled, cannot claim winnings");
        ...
        if (battle_contestants[_battle_id].token_address == address(0)) {
            payable(msg.sender).transfer(2 * (100 - battle_fee_precent) * battle_contestants[_battle_id].value / 100);
            payable(TREASURY).transfer(2 * battle_fee_precent * battle_contestants[_battle_id].value / 100);
        } else {
            IERC20(battle_contestants[_battle_id].token_address).transfer(msg.sender, 2 * (100 - battle_fee_precent) * battle_contestants[_battle_id].value / 100);
            IERC20(battle_contestants[_battle_id].token_address).transfer(TREASURY, 2 * battle_fee_precent * battle_contestants[_battle_id].value / 100);
        }
    }
