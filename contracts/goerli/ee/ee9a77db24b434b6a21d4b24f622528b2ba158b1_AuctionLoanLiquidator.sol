...
///         Once liquidated, it sends the proceeds to the vault. Mainly
///         taking Zora's implementation.
    ...
    ReentrancyGuard
{
    ...
    error VaultNotExistsError(address _vaultAddress);

    error AuctionNotExistsError(address _contract, uint256 _tokenId);

    error MinBidError(uint256 _minBid);

    error AuctionOverError(uint48 _expiration);

    error AuctionNotOverError(uint48 _expiration);

    error NoBidsError();

    constructor(address vaultFactory) Owned(msg.sender) {
    ...
    function liquidateLoan(
        ...
        if (!VaultFactory(_vaultFactory).vaultExists(_vaultAddress)) {
            revert VaultNotExistsError(_vaultAddress);
        }
        Auction storage auction = _auctions[_contract][_tokenId];

        auction.loanId = _loanId;
        auction.loanAddress = msg.sender;
        auction.vaultAddress = _vaultAddress;
        auction.duration = abi.decode(_extraPayload, (uint48));
        auction.asset = _asset;
        auction.startTime = uint48(block.timestamp);
    ...
    function placeBid(
        ...
        if (
            currentHighestBid.mulDivDown(10000 + MIN_INCREMENT_BPS, 10000) >=
            _bid
        ) {
            revert MinBidError(_bid);
        }

        uint48 expiration = auction.startTime + auction.duration;
        if (block.timestamp > expiration && currentHighestBid > 0) {
            revert AuctionOverError(expiration);
        }
        ...
        emit BidPlaced(_contract, _tokenId, newBidder, _bid);
    }

    /// @notice On settlement, the NFT is sent to the highest bidder and the proceeds
    ///         to the vault. Calls loan liquidated for accounting purposes.
    function settleAuction(address _contract, uint256 _tokenId)
        external
        nonReentrant
    {
        Auction storage auction = _auctions[_contract][_tokenId];
        ...
        if (block.timestamp < expiration) {
            revert AuctionNotOverError(expiration);
        }
        if (auction.highestBidder == address(0)) {
            revert NoBidsError();
        }
        if (!VaultFactory(_vaultFactory).vaultExists(auction.vaultAddress)) {
            revert VaultNotExistsError(auction.vaultAddress);
        }
        ...
        ERC20(auction.asset).safeTransfer(auction.vaultAddress, highestBid);

        ILoan(auction.loanAddress).loanLiquidated(
            auction.loanId,
            auction.highestBid
        );
