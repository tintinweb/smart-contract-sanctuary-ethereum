...
contract OASERC721Template is Initializable, ERC721Upgradeable, OwnableUpgradeable, IOASERC721CounterTemplate {
    using StringsUpgradeable for uint256;

    string private _baseURIInput;
    ...
    function initialize(string calldata inputContractURI, string calldata baseURI, string calldata tokenURISuffix, string calldata tokenName, string calldata tokenSymbol) initializer public override {
        _baseURIInput = baseURI;
        _contractURI = inputContractURI;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(_baseURIInput).length > 0 ? string(abi.encodePacked(_baseURIInput, tokenId.toString(),_tokenURISuffix)) : "";
    }
