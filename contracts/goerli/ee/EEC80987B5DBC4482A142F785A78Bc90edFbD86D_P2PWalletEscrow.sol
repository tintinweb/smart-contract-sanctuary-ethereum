...
contract P2PWalletEscrow is Ownable {
    ...
    modifier IsValidSellOrderId(uint sellOrderId) {
        require(
            sellOrderId > 0 && sellOrderId <= totalSellOrders,
    ...
    /****************************************************************************
     @notice This Method Is Used To Create Sell Order Or Lock Users Token In Contract
     @param tokenToSell, Address Of Token To Deposit. 
     @param amount, Number Of Tokens To Deposit.
     @return It Will Return Sell Order Id.
    *****************************************************************************/
    function createSellOrder(IERC20 tokenToSell, uint amount)
        public
        OnlyWhiteListedUser
        returns (uint)
    {
    ...
    *********************************************************************************************/
    ...
    function cancelSellOrder(uint sellOrderId)
        public
        OnlyWhiteListedUser
        IsValidSellOrderId(sellOrderId)
    {
    ...
    /****************************************************************************************************
     @notice This Method Is Used To Create Buy Request,
     When Any User Will Call This Method So Specified Tokens Will Locked To Their Address,
     Seller's Deposited Token Will Be Locked And They Will Not Able To Withdraw That Amount Of Tokens.
     @param sellOrderId, Id Of Sell Order.
    ...
    *****************************************************************************************************/
    ...
    function createBuyRequest(
    ...
    ) public OnlyWhiteListedUser IsValidSellOrderId(sellOrderId) {
        SellOrder memory tempSellOrderData = sellOrders[sellOrderId];
        require(
        ...
        address msgSender = msg.sender;
        require(
            msgSender != tempSellOrderData.seller,
        ...
        usersData[tempSellOrderData.seller].buyRequestsReceived.push(
            newBuyRequestId
        );
    ...
    **************************************************************************/
    ...
    function acceptBuyRequest_Batch(
        uint[] memory buyRequestIds,
        uint[] memory sellOrdersIds
    ) external {
        require(
            buyRequestIds.length == sellOrdersIds.length,
            "Invalid Inputs, Make Sure That You Are Passing Same Number Of BuyRequestIds And SellOrdersIds"
        );
        uint totalIterations = buyRequestIds.length;
        for (uint i = 0; i < totalIterations; ) {
            acceptBuyRequest(buyRequestIds[i], sellOrdersIds[i]);
            unchecked {
                i++;
            }
        }
    ...
    function acceptBuyRequest(uint buyRequestId, uint sellOrderId)
        public
        IsValidSellOrderId(sellOrderId)
    {
        require(
            buyRequestId > 0 && buyRequestId <= totalBuyRequests,
        ...
        require(
            sellOrders[sellOrderId].seller == msg.sender,
    ...
    /************************************************************************************
     ...
     Locked Tokens Of Seller Will Be Released And They Will Be Able To Withdraw Them.
     @param buyRequestIds, Ids (Array Of Ids) Of Buy Request.
    ************************************************************************************/

    function cancelBuyRequest_Batch(
        uint[] memory buyRequestsIds,
        uint[] memory sellOrdersIds
    ) external onlyOwner {
        require(
            buyRequestsIds.length == sellOrdersIds.length,
            "Invalid Inputs, Make Sure That You Are Passing Same Number Of BuyRequestIds And SellOrdersIds"
        );
        uint totalIterations = buyRequestsIds.length;
        for (uint i = 0; i < totalIterations; ) {
            cancelBuyRequest(buyRequestsIds[i], sellOrdersIds[i]);
            unchecked {
                i++;
            }
        }
    ...
    function cancelBuyRequest(uint buyRequestId, uint sellOrderId)
        private
        IsValidSellOrderId(sellOrderId)
    {
        require(
        ...
        BuyRequest storage tempBuyRequestData = buyRequests[buyRequestId];
        require(
            isWhiteListedUser(sellOrders[sellOrderId].seller) == true,
            "Seller Is Not WhiteListed, Seller Must Be WhiteListed"
        );
        require(
            tempBuyRequestData.requestStatus == BuyRequestStatus.Requested,
        ...
        require(
            ...
            "Buy Request Cannot Cancelled Before Buy Request"
        );
        tempBuyRequestData.requestStatus = BuyRequestStatus.Dispute;
        sellOrders[sellOrderId].totalUnsoldTokens += tempBuyRequestData
            .tokenAmount;
        sellOrders[sellOrderId].totalSoldTokens -= tempBuyRequestData
            .tokenAmount;
    ...
    ********************************************************************************************************/
    ...
    function getUsersSellOrderCreated(address userAddress)
        external
        view
        returns (uint[] memory)
    {
    ...
    function getUsersBuyRequestReceived(address userAddress)
        external
        view
        returns (uint[] memory)
    {
    ...
    function getUsersBuyRequestCreated(address userAddress)
        external
        view
        returns (uint[] memory)
    {
