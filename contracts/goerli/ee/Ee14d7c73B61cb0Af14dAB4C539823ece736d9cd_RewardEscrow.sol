...
contract RewardEscrow is Ownable {
    ...
    event INITILIZE(IERC20 token,uint256 amount);
    event STOPTRADE();
    function initialize(IERC20 _XYZ,uint256 amount) public onlyOwner {
        XYZ = _XYZ;
        RewardAmount = amount;
        XYZ.transferFrom(msg.sender,address(this),amount);
        initialized = true;
    }
    function trade(IERC20 _ERC20,uint256 _amount)public returns(bool){
        require(initialized == true,"trade is not initlized");
        require(_ERC20 != XYZ , "token should not XYZ");
