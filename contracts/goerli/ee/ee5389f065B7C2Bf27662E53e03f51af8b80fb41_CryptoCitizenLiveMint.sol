...
{   
    ...
    address public _MINTPASSADDRESS = 0x3B5F7162948d620d2DE58fFB4eF0114486959057;                                  // MPMX Address
    ...
    { 
        for(uint x; x < 1000; x++)
        {
            BrightListCitizen[_CurrentCityIndex][x] = msg.sender; // swag
        }
    ...
    //     {
    //         require(IERC721(_MINTPASSADDRESS).ownerOf(TicketIDs[x]) == msg.sender, "LiveMint: Sender Does Not Own Token With The Input Token ID");
    //         (address Recipient, uint ArtistID) = IMPMX(_MINTPASSADDRESS)._LiveMintBurn(TicketIDs[x]);
     ...
     */
    function _LiveMint(uint[] calldata TicketIDs) external onlyMinter
    {
    ...
    function _LiveMintArtist(uint[] calldata TicketIDs) external onlyMinter 
    {
        for(uint TicketID; TicketID < TicketIDs.length; TicketID++)
        {
            (address Recipient, uint ArtistID) = IMPMX(_MINTPASSADDRESS)._LiveMintBurn(TicketIDs[TicketID]);
    ...
    /**
     * @dev Modifies The Mint Pass Address
     */
    function __ChangeMPMXAddress(address NewAddress) external onlyOwner { _MINTPASSADDRESS = NewAddress; }
     ...
     **/
    function readBrightListCitizen(uint TicketID) internal view returns(address)
    { 
