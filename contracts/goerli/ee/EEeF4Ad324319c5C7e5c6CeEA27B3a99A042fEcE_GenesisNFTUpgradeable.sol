...
abstract contract MintingSalesUpgradeable is Initializable, ContextUpgradeable {
    ...
    struct SalesInfo {
        ...
        // uint8 mintLimitPerAccount;   // Limit per account for minting
    ...
    function __MintingSales_init(address addr) internal onlyInitializing {
        __MintingSales_init_unchained(addr);
    }

    function __MintingSales_init_unchained(address addr) internal onlyInitializing {
        _setCollector(addr);
    }
...
abstract contract WhitelistVoucherUpgradeable is Initializable, ContextUpgradeable {
    using SafeMathUpgradeable for uint256;
     ...
     * @param accountList a batch of addresses to add in whitelist.
     */
    event AddWhitelistBatch(address[] accountList);
    ...
    function _addWhitelist(address account) internal virtual {
        _whitelist[account].add(1);
        emit AddWhiteList(account);
    }

    /**
     * @dev add a batch of addresses in whitelist.
     * @param accountList a batch of address to add in whitelist.
     */
    function _addWhitelistBatch(address[] calldata accountList) internal virtual {
        for (uint256 i = 0; i < accountList.length ;i++) {
            _whitelist[accountList[i]].add(1);
        }
        emit AddWhitelistBatch(accountList);
    }
    ...
    function _removeWhitelist(address account) internal virtual {
        _whitelist[account].sub(1, "Safe Math: incorrect calculation");
...
import "../lib/whitelist/WhitelistVoucherUpgradeable.sol";

contract GenesisNFTUpgradeable is
    Initializable,
    ERC721Upgradeable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable,
    MintingSalesUpgradeable,
    WhitelistVoucherUpgradeable,
    UUPSUpgradeable
{
    using SafeMathUpgradeable for uint256;

    string private _baseTokenURI;
    string private _revealTokenURI;
    uint256 private _revealBlock;
    uint256 public discountRate;
    uint8 private _whitelistLimitPerOnce;
    mapping(uint256 => uint64) private _lock;
    ...
    function initialize(
        ...
        string calldata revealTokenURI_,
        uint256 revealBlock_,
        uint256 discountRate_,
        address collector_,
        uint8 whitelistLimitPerOnce_
    ) 
        public initializer
    {
        ...
        _revealTokenURI = revealTokenURI_;
        _revealBlock = revealBlock_;
        discountRate = discountRate_;
        _whitelistLimitPerOnce = whitelistLimitPerOnce_;
    }
    ...
    function addWhitelistBatch(address[] calldata accountList) external onlyOwner {
        _addWhitelistBatch(accountList);
    }
    ...
    function mintBatch(address[] calldata accountList, uint256 startIndex) external onlyOwner {
        for (uint256 i = 0; i < accountList.length; i++) {
            _mint(accountList[i], startIndex);
            startIndex++;
        }
    }
    ...
    function setupMinting(
        ...
        external onlyOwner
    {    
        // require(block.number > saleInfo().mintEndBlockNumber, "Sales: minting sales already exist.");
        _setupMinting(
            antibotInterval,
            mintLimitPerOnce,
            // mintLimitPerAccount,
    ...
    function whitelistMinting(uint256 requestedCount)
        ...
        salesValidator(requestedCount, balanceOf(_msgSender()))
    {
        require(requestedCount > _whitelistLimitPerOnce, "Sales: too many request or zero request");
        require(msg.value == salesInfo().mintPrice.mul(100 - discountRate).div(100).mul(requestedCount), "Sales: incorrect ETH for payment");
    ...
    function _baseURI() internal view override returns (string memory) {
        return block.number >= _revealBlock ? _revealTokenURI : _baseTokenURI;
    }

    function _isLock(uint256 tokenId) public virtual returns(bool) {
        return _lock[tokenId] > block.timestamp;
    }

    function _setLock(uint256 tokenId, uint64 expires) internal virtual {
        _lock[tokenId] = expires;
    }

    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        require(!_isLock(tokenId), "Token is lock");
