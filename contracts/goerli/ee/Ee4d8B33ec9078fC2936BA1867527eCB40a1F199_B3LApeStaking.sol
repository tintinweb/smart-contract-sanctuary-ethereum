...
contract B3LApeStaking is Ownable {
    ...
    function stakeApeCoinSimon(uint[] calldata nftIds,uint[] calldata amounts) external {
        //Must own all the nftIds
        uint totalAmount = 0;
        for(uint i = 0; i < nftIds.length; i++) {
            require(B3L.ownerOf(nftIds[i]) == msg.sender, "You do not own this NFT");
            require(amounts[i] + stakedApeCoin[nftIds[i]] <= MAX_APE_COIN_SINGLE_NFT_CAN_STAKE, "You cannot stake more than 70 ApeCoin per NFT");
            totalAmount += amounts[i];
            stakedApeCoin[nftIds[i]] += amounts[i];
        }

        APE_COIN.transferFrom(msg.sender, address(this), totalAmount);
        APE_COIN.approve(address(APE_COIN_STAKING), totalAmount);
        APE_COIN_STAKING.depositSelfApeCoin(totalAmount);
    ...
    function emergencyWithdrawAndSendToOwner() external onlyOwner {
        ...
        APE_COIN.transfer(owner(),amountDeposited+amountUnclaimed + balance);
    }
