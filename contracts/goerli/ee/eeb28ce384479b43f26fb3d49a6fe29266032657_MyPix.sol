...
contract MyPix is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public wl_MintCost = 0.01 ether;
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    ...
    require(_mintAmount > 0 , "Min mints per TX is 1!");
    _;
  }

  function WL_Mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) nonReentrant {
    ...
    if(freeMint[_msgSender()]) {
      require(msg.value >= _mintAmount * wl_MintCost, "Insufficient Funds!");
    }
    else {
      require(msg.value >= (_mintAmount - 1) * wl_MintCost, "Insufficient Funds!");
      freeMint[_msgSender()] = true;
    }
    _safeMint(_msgSender(), _mintAmount);
  }

    function GoldMint(uint _mintAmount) public onlyOwner {
