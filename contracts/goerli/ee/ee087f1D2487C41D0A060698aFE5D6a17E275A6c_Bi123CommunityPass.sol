...
contract Bi123CommunityPass is ERC721, Pausable, Ownable, ERC721Burnable {
    ...
    uint256 PRICE_MONTHLY = 0.1 ether;

    // Quarterly Passcard Price
    uint256 PRICE_QUARTERLY = 0.25 ether;

    // Annually Passcard Price
    uint256 PRICE_ANNUALLY = 0.8 ether;

    uint256 private constant _UNIT = 0.000001 ether;
    ...
    function monthlyPrice() public view returns (uint256) {
        return PRICE_MONTHLY;
    }

    function quarterlyPrice() public view returns (uint256) {
        return PRICE_QUARTERLY;
    }

    function annuallyPrice() public view returns (uint256) {
        return PRICE_ANNUALLY;
    }
    ...
    function expirationTimestampOf(
        uint256 tokenId
    ...
    function updatePrice(uint8 cardType, uint256 price) public onlyOwner {
        if (1 == cardType) {
            PRICE_MONTHLY = price * _UNIT;
        } else if (2 == cardType) {
            PRICE_QUARTERLY = price * _UNIT;
        } else if (4 == cardType) {
            PRICE_ANNUALLY = price * _UNIT;
        }
    }
}