...
contract InfinityBitTokenV2 is IERC20, Ownable {
    ...
    address payable immutable private _legacyMarketingWallet = payable(0xb9B23Cc434596ABa6f83d5C6662DD3Cf4B1e099C);

    // Developer Tax
    uint8 private _devTax = 18; // 1.8%
    address payable private _devWallet = payable(0x4Af89d72497F1F0a6e9D4779b28B499c6cD8d3D2);
    address payable immutable private _legacyDevWallet = payable(0xe83842e599b9BC6951A265dC75c3745bA9B88201);
    ...
    function autoSwapConditionCheck(address from, address to, uint256 amount) private view returns (bool) {
        if(!_autoSwapTokens) {
            return false;
        }
    ...
    function _takeTokensFromMarketingAndDevWallets(uint256 max_extra_autoswap) private returns (uint256 amount_taken) {
        ...
        if(_balances[_marketingWallet] + _balances[_devWallet] < 100000_00000000)
        {
            return 0;
        }
