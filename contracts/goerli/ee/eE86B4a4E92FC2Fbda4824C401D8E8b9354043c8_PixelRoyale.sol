...
contract PixelRoyale is ERC721A, Ownable {
    ...
    address public abashoCollective = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2; // ---> Needs to be set - development = test wallet
    ...
    uint256 public timeLimit = 1662463800; // 1671667200; // Thursday, 22. December 2022 00:00:00 GMT
    ...
    //---------- Player Vars ----------//
    struct Pixel {
        //bytes8 seed; //only for on-chain art gen
        int256 health;
        bool status;
        //address killer; //keep track of killer if needed/wanted
    }
    ...
    function mint(uint256 _amount) external {
        ...
        for (uint j; j < _amount; j++) { 
                Pixel memory newPixel = Pixel(
                    //bytes8(keccak256(abi.encodePacked(block.timestamp, j+1, msg.sender))),
                    0,
                    true);
    ...
    //---------- Team Claim ----------//
     function teamClaim() external onlyOwner {
        ...
        for (uint j; j < CREATORCLAIMAMOUNT; j++) {
            ...
            Pixel memory newPixel = Pixel(//bytes8(keccak256(abi.encodePacked(block.timestamp, j+1, msg.sender))),
            0,
            true);
            pixelList[total+j+1] = newPixel;
        }
    ...
    function pseudoRandom(uint _number, string memory _specialSalt) public view returns(uint number){
    ...
    function setHP(uint256 _tokenId, int256 _amount) external payable {
        ...
        if(_amount < 0){
            require(pixelList[_tokenId].health+_amount>-2,"Try less attacks");
        ...
        emit ItemBought(msg.sender,mostAttacks,_tokenId,_amount); // ---> buyer, current Highscore Leader, Interacted token, amount of protections/attacks
        ...
        if ( !isAlive(_tokenId) ) {
            ...
            //emit DropOut event
            emit DropOut(msg.sender,_tokenId); // ---> Killer, Killed Token
        }
    ...
    function tryJackpot() internal {
        if(pseudoRandom(8,"") == 4) { // ---> 12,5% winning chance
            ...
            emit MiniJackpotWin(msg.sender, currentPot()); // ---> emits jackpot amount and winner when hit
        }
        else {
