...
contract Roulette is VRFConsumerBaseV2{
  ...
  */
  ...
  event ChainLinkGeneratorRandom(uint256 randomNumberFormChainLink);
  ...
  function getRandomNumberFromVRF() external{
    require(!isAlreadyRandom, "already request random number!");
    // requestRandomWords();
  ...
  function fulfillRandomWords(
    ...
    isAlreadyRandom = true; // isAlreadyRandom 改為 ture 
      
    // lottery.random = randomWords[0] % lottery.totalParticipants;
    res1 = randomWords[0];
    res2 = randomWords[0] % 37;
    emit ChainLinkGeneratorRandom(randomWords[0]);
  }

  uint256 public res1 = 1;
  uint256 public res2 = 1;
  
  function getRes1() public view returns(uint) {
    return res1; 
  }
  
  function getRes2() public view returns(uint) {
    return res2; 
  }
