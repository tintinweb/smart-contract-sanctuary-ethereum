    ...
    IBaseMetadata
{
    ...
    uint256 public constant FIRST_SALE_SUPPLY = 22;
    ...
    mapping (address => uint256) private allowlistClaimedQuantities;
    mapping (uint256 => bytes32) private objects; /// @dev objects or _seed

    /// @dev Must change name, symbol and func name of require.
    constructor() ERC721("TestNFT", "TNFT") {}

    function allowlistMint(bytes32[] calldata _merkleProof, uint256 _quantity, uint256 _mintQuantity) external payable nonReentrant {
        require(isAllowlistSaleActive, "TestNFT: Allowlist sale is not active");
        require(MerkleProof.verify(_merkleProof, merkleRoot,  keccak256(abi.encodePacked(msg.sender, _quantity))), "TestNFT: Invalid Merkle Proof");
        require(_mintQuantity <= _quantity - allowlistClaimedQuantities[msg.sender] , "TestNFT: Already claimed the maximum allowed quantity");
        require(msg.value >= _mintQuantity * mintPrice, "TestNFT: ETH is not enough");

        _commonMint(msg.sender, _mintQuantity);
        allowlistClaimedQuantities[msg.sender] += _mintQuantity;
    }

    function publicMint() external payable nonReentrant {
        require(isPublicSaleActive, "TestNFT: Public sale is not active");
        require(!publicClaimed[msg.sender], "TestNFT: Already claimed"); 
        require(msg.value >= mintPrice, "TestNFT: ETH is not enough");
    ...
    function _commonMint(address _to, uint256 _mintQuantity) internal {
        require(totalSupply + _mintQuantity <= _getMaxSupply(), "TestNFT: Already the max supply of this sale");
    ...
    function setIsAllowlistSaleActive(bool _state) external onlyOwner {
        require(merkleRoot != bytes32(0), "TestNFT: MerkleRoot is not set");
    ...
    modifier onlyNotFrozen {
        require(!isMetadataFrozen, "TestNFT: Already frozen"); 
        _;
    }
    ...
    modifier onlyBaseAddress {
        require(baseAddress == msg.sender, "TestNFT: Caller is not generateMetadata contract");
        _;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "TestNFT: Nonexistent token");
        return IGenerateMetadata(baseAddress).tokenMetadata(tokenId);
    }
    
    function burn(uint256 tokenId) external {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "TestNFT: Caller is not owner nor approved");
        _burn(tokenId);
    }
    ...
    /// @dev DefaultOperatorFilterer
    /// @dev 前回からどのように変わったのか？を確認する, 特にonlyAllowedOperatorApproval
