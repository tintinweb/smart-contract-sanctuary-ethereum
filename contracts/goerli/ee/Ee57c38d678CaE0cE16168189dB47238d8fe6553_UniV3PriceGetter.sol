...
library UniV3PriceGetter {
    ...
    //Ape LP addresses
    address constant USDC_WETH_PAIR = 0x07A4f63f643fE39261140DF5E613b9469eccEC86; // usdc is token0

    address constant USDT_WETH_PAIR = 0x4e68Ccd3E89f51C3074ca5072bbAC773960dFa36; // usdt is token1
    address constant DAI_WETH_PAIR = 0xC2e9F25Be6257c210d7Adf0D4Cd6E3E881ba25f8; // dai is token0
    ...
    function getRawPrice(address token) public view returns (uint256) {
        ...
        return getRawPrice(token, getETHPrice(10));
    }

    //returns the prices of multiple tokens, zero on failure

    function getRawPrices(address[] memory tokens) public view returns (uint256[] memory prices) {
        prices = new uint256[](tokens.length);
        uint256 bnbPrice = getETHPrice(10);
    ...
    function getRawLPPrice(address token) internal view returns (uint256) {
        ...
        return getRawLPPrice(token, getETHPrice(10));
    }
    ...
    function getETHPrice(uint32 _secondsAgo) public view returns (uint256 price) {
        // uint32[] memory data = new uint32[](2);
        // data[0] = 0;
        // data[1] = _secondsAgo;
        // (int56[] memory tickCumulatives, uint160[] memory secondsPerLiquidityCumulativeX128s) = IUniswapV3Pool(USDC_WETH_PAIR).observe(data);
        // uint256 sqrtPriceX96 = (uint256(tickCumulatives[1]) - uint256(tickCumulatives[0])) / _secondsAgo; //usdc has 6 decimals. 18-6 = 12

        // (int56[] tickCumulatives, uint160[] secondsPerLiquidityCumulativeX128s) = IUniswapV3Pool(USDT_WETH_PAIR).observe([0, _secondsAgo]);
        // int256 priceWethInUsdt = 10001 ** (tickCumulatives[1] - tickCumulatives[0]) * 10 ** 12; //usdt has 6 decimals. 18-6 = 12

        // (int56[] tickCumulatives, uint160[] secondsPerLiquidityCumulativeX128s) = IUniswapV3Pool(DAI_WETH_PAIR).observe([0, _secondsAgo]);
        // int256 priceWethInDai = 10001 ** (tickCumulatives[1] - tickCumulatives[0]);

        // uint256 price = (priceWethInUsdc + priceWethInUsdt + priceWethInDai) / 3 / 10 ** 4;

        uint256 sqrtPriceX96;
        if (_secondsAgo == 0) {
            // return the current price if _secondsAgo == 0
            (sqrtPriceX96, , , , , , ) = IUniswapV3Pool(USDC_WETH_PAIR).slot0();
        } else {
            uint32[] memory secondsAgos = new uint32[](2);
            secondsAgos[0] = _secondsAgo; // from (before)
            secondsAgos[1] = 0; // to (now)

            (int56[] memory tickCumulatives, ) = IUniswapV3Pool(USDC_WETH_PAIR).observe(secondsAgos);
    ...
    function getRawLPPrice(address lp, uint256 bnbPrice) internal view returns (uint256) {
        ...
        try IApePair(lp).getReserves() returns (uint112 reserve0, uint112 reserve1, uint32) {
            ...
            uint256 totalValue = getRawPrice(token0, bnbPrice) * reserve0 + getRawPrice(token1, bnbPrice) * reserve1;
    ...
    function getRawPrice(address token, uint256 bnbPrice) internal view returns (uint256 rawPrice) {
        uint256 pegPrice = pegTokenPrice(token, bnbPrice);
    ...
    function pegTokenPrice(address token, uint256 bnbPrice) private pure returns (uint256) {
        if (token == USDT || token == USDC || token == DAI) return PRECISION;

        if (token == WETH) return bnbPrice;
    ...
    function pegTokenPrice(address token) private view returns (uint256) {
        ...
        if (token == WETH) return getETHPrice(10);
