...
contract OZGURTOKEN is ERC1363 {
    using SafeMath for uint256;
    using Address for address;

    address private owner;
    string _name = "OZGUR TOKEN";
    string _symbol = "OZGUR";
    ...
    constructor() ERC20(_name, _symbol) {
        owner = _msgSender(); // '_msgSender()' is sender of current call, contract deployer for a constructor
        _mint(_msgSender(), 10000000000 * 10 ** 18);
    }

    function mint(uint amount) external onlyOwner {
        require(_msgSender() == owner, "ERC20: mint from the zero address");
        _mint(_msgSender(), amount);
    }

    function burn(address account, uint256 amount) external onlyOwner {
        require(_msgSender() == owner, "ERC20: burn from the zero address");
        _burn(account, amount);
    }

    function transferOwnership(address newOwner) external  onlyOwner {
        require(_msgSender() == owner, "ERC20: Require transfer owner address");
