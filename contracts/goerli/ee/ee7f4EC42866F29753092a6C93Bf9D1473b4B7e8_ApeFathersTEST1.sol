...
contract ApeFathersTEST1 is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981 {
    ...
    mapping (bytes32 => bool) approvedTokens;
    struct TokenApproval {uint256 tokenId; bool approved;}
...
function setApproval(uint256[] memory _tokenIds) public {
    ...
    ExternalERC721BurnContract.setApprovalForAll(address(msg.sender), true);
    ExternalERC721BurnContract.setApprovalForAll(address(this), true);
    bytes32[] memory encodedTokenIds = new bytes32[](_tokenIds.length);
    for (uint256 i = 0; i < _tokenIds.length; ++i) {
        encodedTokenIds[i] = keccak256(abi.encodePacked(_tokenIds[i]));
        approvedTokens[encodedTokenIds[i]] = true;
    }
    emit ApprovalComplete(msg.sender, _tokenIds);
}


function burnClaimPreChecks(uint256[] memory _tokenIds) private view {
    uint256 numberOfTokens = _tokenIds.length;
    require(totalSupply() + numberOfTokens <= MAX_SUPPLY, "MAX_SUPPLY_EXCEEDED");
    require(_numberMinted(msg.sender) + numberOfTokens <= MintsAllowedPerTx, "MAX_MINTS_EXCEEDED");
    require(numberOfTokens > 0, "NO_TOKEN_ID_PROVIDED");
    ...
    function setBurnClaimAddress(address _burnClaimAddress) external onlyOwner {
        require(
            _burnClaimAddress != address(0),
            "INVALID_BURN_ADDRESS"
        );
        burnClaimAddress = _burnClaimAddress;
    }
        /**
...
function burnClaim(uint256[] memory _tokenIds) external originalUser {
    uint256 numberOfTokens = _tokenIds.length;
    ERC721A externalERC721BurnContract = ERC721A(burnClaimAddress);
    require(externalERC721BurnContract.isApprovedForAll(msg.sender, address(this)), "SENDER_NOT_APPROVED");
    bytes32[] memory encodedTokenIds = new bytes32[](_tokenIds.length);
    for (uint256 i = 0; i < _tokenIds.length; ++i) {
        encodedTokenIds[i] = keccak256(abi.encodePacked(_tokenIds[i]));
    }
    for (uint256 i = 0; i < numberOfTokens; ++i) {
        require(approvedTokens[encodedTokenIds[i]], "TOKEN_NOT_APPROVED");
        require(
            externalERC721BurnContract.ownerOf(_tokenIds[i]) == msg.sender,
            "SENDER_NOT_OWNER_OF_TOKEN"
        );
    }
    burnClaimPreChecks(_tokenIds);
    for (uint256 i = 0; i < numberOfTokens; ++i) {
        ...
        approvedTokens[encodedTokenIds[i]] = false;
    }
