  ...
  AccessControlUpgradeable
{
  ...
  bool public anonEthPayments;

  bool public anonErc20Payments;
  ...
  function initialize(
    ...
    signer = _signer;
    baseUri = _baseUri;
    emit AnonInitalized(address(this), _priceEth, _priceErc20);
  }
  ...
  function toggleAnonEthPayments() external onlyRole(DEFAULT_ADMIN_ROLE) {
    anonEthPayments = !anonEthPayments;
  }

  function toggleAnonErc20Payments() external onlyRole(DEFAULT_ADMIN_ROLE) {
    if (address(erc20) == address(0)) revert AddressNotSet();
    anonErc20Payments = !anonErc20Payments;
  }
  ...
  function addSteal(
    ...
    emit StealAdded(_anonId, _stealId, anones[_anonId].gen++, _entropy);
  }
  ...
  function getStealIds(
    ...
    if (len > stealIds[_anonId].length - _cursor) {
      len = stealIds[_anonId].length - _cursor;
    }
    ids = new uint256[](len);
    for (uint256 i = 0; i < len; i++) {
      ids[i] = stealIds[_anonId][_cursor + i];
    }
  ...
  function storeBasic(
    ...
    anones[_anonId].anonType = _anonType;
    anones[_anonId].arweaveId = _arweaveId;
    usedSignature[_signature] = true;
  }

  // Takes a contract and tokenID and anones them
  function anon(
    address _recipient,
    address _contract,
    uint256 _id,
    ...
    uint256 _expiry,
    bytes memory _signature
  ) private returns (uint256 anonId) {
    ...
    storeBasic(anonId, AnonType.NATIVE, _arweaveId, _signature);
    children[_contract][_id].push(anonId);
    natives[anonId].contract_ = _contract;
    natives[anonId].id = _id;
    natives[anonId].ownership = AnonUtils.isOwned(_contract, _id, msg.sender);

    if (_contract == address(this)) {
      anones[_id].entropy = keccak256(
        abi.encodePacked(anonId, anones[_id].gen, msg.sender, block.timestamp)
      );
      anones[_id].gen++;
    }
    _mint(_recipient, anonId);

    emit AnonCreated(_recipient, anonId, _contract, _id, _arweaveId);
  }
  ...
  ) external payable nonReentrant returns (uint256 anonId) {
    if (!anonEthPayments) revert EthDisabled();
    if (msg.value < priceEth) revert InsufficientPayment();
    anonId = anon(_recipient, _contract, _id, _arweaveId, _expiry, _signature);
  ...
  function mintAnonErc20(
    ...
    uint256 _expiry,
    bytes calldata _signature
  ) external nonReentrant returns (uint256 anonId) {
    if (!anonErc20Payments) revert Erc20Disabled();
  ...
  function custom(
    ...
    storeBasic(customId, AnonType.CUSTOM, _arweaveId, _signature);
