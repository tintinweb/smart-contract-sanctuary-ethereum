...
interface IFabricaValidator is IERC165 {
    // On-chain data update
    event UpdateMetadataBaseUri(string newData);

    function metadataUri(uint256 id) external view returns (string memory);
}
...
contract FabricaToken is Context, ERC165, IERC1155, IERC1155MetadataURI, Ownable, Pausable {
     ...
     * @dev See {IERC1155MetadataURI-uri}.
     *
     * The Fabrica token contract implements IERC1155MetadataURI#uri() by returning the
     *  result of calling IFabricaValidator#metadataUri() on the contract found at the
     *  passed-in (by ID) token's Property#validator value. The same token ID is passed to
     *  the IFabrica metadataUri
     */
    function uri(uint256 id) override public view returns (string memory) {
        address validator = _property[id].validator;
        if (ERC165Checker.supportsInterface(validator, type(IFabricaValidator).interfaceId)) {
            return IFabricaValidator(validator).metadataUri(id);
        }
        return "";
    }
    ...
    /**
     * @dev generate token id (to avoid front-running)
     */
