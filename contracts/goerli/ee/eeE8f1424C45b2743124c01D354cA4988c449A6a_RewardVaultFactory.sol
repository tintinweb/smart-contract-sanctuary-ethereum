...
contract RewardVaultFactory is IRewardVaultFactory, OwnableUpgradeable {
    ...
    /// @notice Operators who can launch new reward vault contracts for the projects
    ...
    modifier onlyOperator(address account_) {
        if (!_operators[account_]) revert Unpermitted();
        _;
    }
    ...
    /// @notice Update operator permission
    ...
    /// @notice Check account if it has operator permission
    function isOperator(address account_) external view returns (bool) {
