...
contract AutoLandVaultV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    IERC20 public immutable token; // LAND token
    IERC20 public immutable receiptToken; // LAND Reward token

    ILandTokenStakingV3 public immutable masterchef;
    ...
    constructor(
        ...
        ILandTokenStakingV3 _masterchef,
        address _admin,
        address _treasury
    ) {
        token = _token;
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_masterchef), MAX_INT);
    }
     ...
     */
    function withdrawAll() external notContract nonReentrant {
    ...
    function harvest() external notContract whenNotPaused nonReentrant {
        ILandTokenStakingV3(masterchef).withdraw(0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        ILandTokenStakingV3(masterchef).emergencyWithdraw();
    }
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = ILandTokenStakingV3(masterchef).pendingReward(address(this));
    ...
    function withdraw(uint256 _shares) public notContract nonReentrant{
        ...
        if (bal < currentAmount) {
            ...
            ILandTokenStakingV3(masterchef).withdraw(balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = ILandTokenStakingV3(masterchef).userInfo(address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            ILandTokenStakingV3(masterchef).deposit(bal);
        }
    }
