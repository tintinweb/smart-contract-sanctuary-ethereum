...
contract CimoStaking is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    uint256 constant ONE_DAY = 60;//86400; //total seconds in one day
    ...
    constructor(ICimo _cimo, address _router) {
        ...
        Cimo.approve(router, Cimo.totalSupply()); //approve staking-router
        numdays = 13;
    }
    ...
    */
    ...
    function withdraw(uint32 _pid, uint256 _amount) public nonReentrant {
        ...
        require(
            ...
            "withdraw: Tokens locked, if you're trying to claim your rewards use the deposit function"
        );
    ...
    */
    function addLiquidityNoFeeAndStake(
        ...
        require(
            (lockTime >= 0 &&
                lockTime <= 90 * ONE_DAY &&
                user.lockedUntil <= lockTime + block.timestamp),
            "addLiquidityNoFeeAndStake : Lock out of range"
        );
        updatePool(0);
        if (user.weightedBalance > 0) {
    ...
    function reinvestETHRewards(uint256 amountOutMin) public nonReentrant {
        ...
        updatePool(1);
        uint256 ETHPending = ((user.weightedBalance *
            poolInfo[1].accETHPerShare) / 1e12) -
            user.ETHrewardDebt +
            user.claimableETH;
    ...
    function withdrawNFT(
        ...
        userInfo[_pid][sender].nftBoost -= pointsPerNFT;
    ...
    function boostWithNFT(
        ...
        userInfo[_pid][msg.sender].nftBoost += pointsPerNFT;
        updateUserWeightedBalance(_pid, msg.sender);
    ...
    function lockTokens(
        ...
        user.lockTimeBoost = uint16((2 * 1000 * lockTime) / (90 * ONE_DAY)); // 0 - 2000
        updateUserWeightedBalance(_pid, sender);
