...
contract VoteWin is Pausable, Ownable {
    ...
    function getMatchs(uint _day, uint _matchIndex) external view returns(uint teamA, uint teamB, uint totalVoteTeamA, uint totalVoteTeamB, uint startTime, uint result,  string memory description) {
        Match storage m = matchs[_day][_matchIndex];
        return (m.team[1], m.team[2], m.totalVote[1], m.totalVote[2], m.startTime, m.result, m.description);
    }
