...
contract mysteryNFT is ERC721, ERC721URIStorage, ERC721Burnable, Ownable, DefaultOperatorFilterer {
    using Counters for Counters.Counter;
    Counters.Counter private NFTtokenId;

    //list of events setup
    event Freeze(address account, bool value);
    event Unfreeze(address account, bool value);
    ...
    constructor(string memory name, string memory symbol, uint256 _privateTokenLimit, 
    ...
    ERC721(name, symbol) {
       ...
       _symbol = symbol;
        ...
        privateMintTokenLimit = _privateTokenLimit;
    ...
    //Whitelisted user process
    //below function is used to add whiteListed user
    function addWuser(address userAddress) public returns (bool) {
        require(whitelistAdmin == msg.sender, "Caller is not authorized");
        whiteListedUser[userAddress] = true;
        return true;
    }

      //array function to add list of whitelisted users
    function addListedOfWusers(address[] memory _userAddress) public {
        require(whitelistAdmin == msg.sender, "Caller is not authorized");
        for(uint i=0 ; i < _userAddress.length ; ++i) {
        whiteListedUser[_userAddress[i]] = true;
        }
    }

    //below function is used to remove the whitlisted user
    function removeWuser(address userAddress) public returns (bool) {
        require(whitelistAdmin == msg.sender, "Caller is not authorized");
        whiteListedUser[userAddress] = false;
        return true;
    }

    //get whitelistedUser
    function isWhiteListedUser(address userAddress) public view returns(bool) {
        return whiteListedUser[userAddress];
    }
    ...
    //set the pause status for the contract function and only Owner can make it
    function freeze() external returns(bool) {
        require(msg.sender == whitelistAdmin, "Caller is not authorized");
        _freeze = true;
        emit Freeze(msg.sender, _freeze);
        return _freeze;
      
    }
    //set unpaused state and only owner can change it
    function unFreeze() external returns(bool) {
        require(msg.sender == whitelistAdmin, "Caller is not authorized");
        _freeze = false;
        emit Unfreeze(msg.sender, _freeze);
        return _freeze;
    }

    function setPrivateTokenLimit(uint256 _tokenLimit) public onlyOwner {
        privateMintTokenLimit = _tokenLimit;
    }

    function setPublicTokenLimit(uint256 _tokenLimit) public onlyOwner {
        publicMintTokenLimit = _tokenLimit;
    }
    
    function setPrivateSaleTokenPrice(uint256 _tokenPrice) public onlyOwner {
        tokenPrivateSalePrice = _tokenPrice;
    }

    function setPublicSaleTokenPrice(uint256 _tokenPrice) public onlyOwner {
        tokenPublicSalePrice = _tokenPrice;
    }

    //get current tokenId or minted token count
    function getCurrentTokenCount() public view returns(uint) {
        require(msg.sender == whitelistAdmin, "Caller is not authorized");
     return NFTtokenId.current();
    }

    function safePrivateMint(string memory metadataUrl) payable public {
       //check the caller/minter is whitelisted user
       require(whiteListedUser[msg.sender] == true, "Caller is not white listed");
       require(NFTtokenId.current() < privateMintTokenLimit, "Token reaches the initial limit");
       require(_freeze == false, "Private miniting is freezed"); 
        ...
        if(tokenPrivateSalePrice > 0) {
            admin.transfer(tokenPrivateSalePrice);
        }
    }

    function safePublicMint(string memory metadataUrl) payable public {
        require(NFTtokenId.current() < (privateMintTokenLimit + publicMintTokenLimit), "Token reaches the initial limit");
        NFTtokenId.increment();
        uint256 tokenId = NFTtokenId.current();
        _safeMint(msg.sender, tokenId);
        _setTokenURI(tokenId, metadataUrl);
        ...
        //transfer the service amount to the owner
        address payable admin = payable(owner());
        if(tokenPublicSalePrice > 0) {
           admin.transfer(tokenPublicSalePrice);
        }
    }
    ...
    function setMetaDataURL(uint _tokenId, address _to, string memory _metaDataUrl) public {
        require(whitelistAdmin == msg.sender, "Caller is not authorized");
        require(_exists(_tokenId),"TokenId is not exist");
        //token should owned by the user _to
        require(balances[_tokenId][_to] >= 1, "insufficient Token Balance");
    ...
    //creator-fee on-chain enforcement tool
