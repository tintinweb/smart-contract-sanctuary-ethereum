...
import "./ante/AnteTest.sol";

/// @title Average Block Time Ante Test
/// @notice Ante Test to check the average block time since the deployment of the contract
contract AnteAverageBlockTimeTest is AnteTest("Average Block Time Test") {

    uint256 public immutable deploymentBlockTimestamp;
    uint256 public immutable deploymentBlockNumber;
    uint256 public immutable targetBlockTime;
    uint256 public immutable targetBlockTimeToleratedDeviation;
    uint256 public immutable toleranceUnit;
    constructor(uint256 _targetBlockTime, uint256 _targetBlockTimeToleratedDeviation, uint256 _toleranceUnit) {
        deploymentBlockTimestamp = block.timestamp;
        deploymentBlockNumber = block.number;
        targetBlockTime = _targetBlockTime;
        targetBlockTimeToleratedDeviation = _targetBlockTimeToleratedDeviation;
        toleranceUnit = _toleranceUnit;
        
        protocolName = "Network";
        testedContracts = [address(0)];
    }

    function averageBlockTime() public view returns (uint256) {
        return  (block.timestamp - deploymentBlockTimestamp) * toleranceUnit / (block.number - deploymentBlockNumber);
    }
    
    /// @notice test to check the average block time since the deployment of the contract
    /// @return true if the average block time is less than the target average block time
    function checkTestPasses() public view override returns (bool) {
        if(block.number <= deploymentBlockNumber) {
            return true;
        }
        
        uint256 average = averageBlockTime();
        return average <= targetBlockTime * toleranceUnit + targetBlockTimeToleratedDeviation;
    }
}