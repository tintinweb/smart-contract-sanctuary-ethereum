...
contract MagpieListManager is Initializable, OwnableUpgradeable, Pausable, ERC2771ContextUpgradeable {
    ...
    modifier onlyLiquidityPool() {
        require(_msgSender() == address(magpieProviders), "ERR__UNAUTHORIZED");
        _;
    }

    modifier onlyLpNft() {
        require(_msgSender() == address(magpieLPToken), "ERR__UNAUTHORIZED");
        _;
    }
    ...
    function _beforeAddingLiquidity(
        ...
        require(
            ifEnabled(magpieProviders.getLiquidityProvidedByToken(_token) + _amount <= perTokenTotalCap[_token]),
    ...
    function setTotalCap(address _token, uint256 _totalCap) public tokenChecks(_token) onlyOwner {
        require(magpieProviders.getLiquidityProvidedByToken(_token) <= _totalCap, "ERR__TOTAL_CAP_LESS_THAN_SL");
    ...
    function getMaxCommunityLpPositon(address _token) external view returns (uint256) {
        uint256 totalSupply = magpieLPToken.totalSupply();
        uint256 maxLp = 0;
        for (uint256 i = 1; i <= totalSupply; ++i) {
            uint256 liquidity = totalLiquidityByLp[_token][magpieLPToken.ownerOf(i)];
            if (liquidity > maxLp) {
                maxLp = liquidity;
            }
        }
