...
contract NFTOpenSale is AccessControl {
    ...
    constructor(INFTBase nftBase_) {
        nftBase = INFTBase(nftBase_);
    ...
    function open(uint256 id, uint256 price, uint256 round) public onlyRole(ADMIN_ROLE) {
        require(nftBase.isApprovedForAll(_msgSender(), address(this)), "NFTOpenSale/not_approved");
        require(price  > 0, "NFTOpenSale/open_0_price");
        require(nftBase.balanceOf(_msgSender(), id) > 0, "NFTOpenSale/balance_zero");
    ...
    function openBatch(uint256[] memory ids, uint256[] memory prices, uint256[] memory rounds) public onlyRole(ADMIN_ROLE) {
        ...
        for(uint256 i=0; i<prices.length; i++){
            ...
            require(nftBase.balanceOf(_msgSender(), ids[i]) > 0, "NFTOpenSale/balance_zero");
    ...
    function buy(uint256 id) public payable returns (bool){
        ...
        if(_rounds[_sales[id].round].isPrivate)
            require(whiteList[_msgSender()],"NFTOpenSale/has_not_whitelist");
        //check seller balance
        require(nftBase.balanceOf(_sales[id].seller, id) > 0, "NFTOpenSale/balance_zero");
        ...
        //nft <-> ether
        nftBase.safeTransferFrom(_sales[id].seller, _msgSender(), id, 1, "");
...
interface INFTBase is IERC1155Upgradeable, IAccessControlUpgradeable {
    ...
    function combinedMint(uint256 isleId, uint256 exteriorId, uint256 interiorId, uint256 landScapeId, uint256 stakeWeight) external returns(uint256);
    function buildingMint(uint256 isleId, uint256 landId, uint256 combinedId, uint256 durability, uint256 stakeWeight) external returns(uint256);
    function getLand(uint256 id) external view returns(LandNFT memory);
    function getExterior(uint256 id) external view returns(ExteriorNFT memory);
    function getInterior(uint256 id) external view returns(InteriorNFT memory);
    function getLandScape(uint256 id) external view returns(LandScapeNFT memory);
    function getCombined(uint256 id) external view returns(CombinedNFT memory);
    function getBuilding(uint256 id) external view returns(BuildingNFT memory);
