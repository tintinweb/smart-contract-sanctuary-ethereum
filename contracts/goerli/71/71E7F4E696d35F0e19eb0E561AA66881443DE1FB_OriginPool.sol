...
contract OriginPool is SuperAppBase {
    ...
    function _sendFlowMessage(uint streamActionType, address sender, address receiver,  int96 flowRate, uint256 relayerFee, uint256 slippage, uint256 cost) external payable {
        IERC20(token.getUnderlyingToken()).approve(address(this), type(uint256).max);
