...
contract GoldStaking is Ownable {
    ...
    function updateTokenStakeAPR(uint256 lockType, uint256 apr) external onlyOwner {
        require(lockType < 6, "lockType should be less than 6");
        tokenStakeAPR[lockType] = apr;
    }

    function updateLpStakeAPR(uint256 lockType, uint256 apr) external onlyOwner {
        require(lockType < 6, "lockType should be less than 6");
        lpStakeAPR[lockType] = apr;
    }

    function updatePenaltyAPR(uint256 lockType, uint256 penalty) external onlyOwner {
        require(lockType < 6, "lockType should be less than 6");
        penaltyForEarlyClaim[lockType] = penalty;
    }

    function updateLockTerm(uint256 lockType, uint256 month) external onlyOwner {
        require(lockType < 6, "lockType should be less than 6");
    ...
    function getTokenStakeData(address user, uint8 lockType) external view returns(META_DATA[] memory) {
        return tokenStakeData[user][lockType];
    }

    function getLpStakeData(address user, uint8 lockType) external view returns(META_DATA[] memory) {
    ...
    function tokenStake(uint256 amount, uint8 lockType) external {
        ...
        META_DATA memory temp = META_DATA(amount, block.timestamp, 0);
    ...
    function tokenStakeReward(uint8 lockType, uint256 index) public view returns (uint256) {
        require(index < tokenStakeData[msg.sender][lockType].length, "Invalid claim index");
        META_DATA memory temp = tokenStakeData[msg.sender][lockType][index];

        uint256 lastClaimTime = temp.claimTime == 0 ? temp.stakeTime : temp.claimTime;
        return temp.amount * tokenStakeAPR[lockType] / 100 * (block.timestamp - lastClaimTime) / 360 days;
    }

    function lpStakeReward(uint8 lockType, uint256 index) public view returns (uint256) {
        require(index < lpStakeData[msg.sender][lockType].length, "Invalid claim index");
        META_DATA memory temp = lpStakeData[msg.sender][lockType][index];

        uint256 tokenPrice = getTokenPrice();
        uint256 lpPrice = getLpPrice();

        uint256 lastClaimTime = temp.claimTime == 0 ? temp.stakeTime : temp.claimTime;
        return (temp.amount * lpPrice / tokenPrice) * lpStakeAPR[lockType] / 100 * (block.timestamp - lastClaimTime) / 360 days;
    }
    ...
    function tokenClaim(uint8 lockType, uint256 index) external {
        ...
        uint256 originReward = tokenStakeReward(lockType, index);

        uint256 nextClaimableTime = temp.claimTime == 0 ? temp.stakeTime + lockTerm[lockType] : temp.claimTime + lockTerm[lockType];
        if(lockType == 0 || nextClaimableTime <= block.timestamp)
            goldToken.transfer(msg.sender, originReward);
        else {
            goldToken.transfer(msg.sender, originReward * penaltyForEarlyClaim[lockType] / 100);
        }
    ...
    function lpClaim(uint8 lockType, uint256 index) external {
        ...
        uint256 originReward = lpStakeReward(lockType, index);
    ...
    function tokenUnstake(uint256 amount, uint8 lockType, uint256 index) external {
        ...
        uint256 nextClaimableTime = temp.claimTime == 0 ? temp.stakeTime + lockTerm[lockType] : temp.claimTime + lockTerm[lockType];
        if(lockType == 0 || nextClaimableTime <= block.timestamp)
            goldToken.transfer(msg.sender, originReward + amount);
        else
            goldToken.transfer(msg.sender, originReward * penaltyForEarlyClaim[lockType] / 100 + amount);
    ...
    function lpUnstake(uint256 amount, uint8 lockType, uint256 index) external {
        require(amount > 0, "Amount should be not 0.");
        require(index < lpStakeData[msg.sender][lockType].length, "Invalid claim index");
