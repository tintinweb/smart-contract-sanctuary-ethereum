...
contract DAOCommittee is StorageStateCommittee, AccessControl, IDAOCommittee {
    ...
    function setSeigManager(
        address _seigManager
    ) external override onlyOwner nonZero(_seigManager) {
    ...
    function setCandidatesCommittee(
        address[] calldata _candidateContracts,
        address _committee
    ) external override onlyOwner nonZero(_committee) {
        for (uint256 i = 0; i < _candidateContracts.length; i++) {
    ...
    function setDaoVault(
        address _daoVault
    ) external override onlyOwner nonZero(_daoVault) {
    ...
    function setLayer2Registry(
        address _layer2Registry
    ) external override onlyOwner nonZero(_layer2Registry) {
    ...
    function setAgendaManager(
        address _agendaManager
    ) external override onlyOwner nonZero(_agendaManager) {
    ...
    function setActivityRewardPerSecond(
        uint256 _value
    ...
    function increaseMaxMember(
        ...
        require(
            ...
            "DAOCommittee: You have to call decreaseMaxMember to decrease"
        );
    ...
    function changeMember(
    ...
    ) external override validMemberIndex(_memberIndex) returns (bool) {
        ...
        require(
            ICandidate(msg.sender).totalStaked() >
                ICandidate(prevMemberContract).totalStaked(),
            "not enough amount"
        );

        CandidateInfo storage prevCandidateInfo = _candidateInfos[prevMember];
        prevCandidateInfo.indexMembers = 0;
        prevCandidateInfo.rewardPeriod = uint128(
            uint256(prevCandidateInfo.rewardPeriod).add(
                block.timestamp.sub(prevCandidateInfo.memberJoinedTime)
            )
        );
        ...
        returns (bool)
    {
        ...
        candidateInfo.rewardPeriod = uint128(
            uint256(candidateInfo.rewardPeriod).add(
                block.timestamp.sub(candidateInfo.memberJoinedTime)
            )
        );
        candidateInfo.memberJoinedTime = 0;
    ...
    function decreaseMaxMember(
        ...
        uint256 _quorum
    ) external override onlyOwner validMemberIndex(_reducingMemberIndex) {
        address reducingMember = members[_reducingMemberIndex];
        CandidateInfo storage reducingCandidate = _candidateInfos[
            reducingMember
        ];
        ...
        reducingCandidate.rewardPeriod = uint128(
            uint256(reducingCandidate.rewardPeriod).add(
                block.timestamp.sub(reducingCandidate.memberJoinedTime)
            )
        );
    ...
    function setQuorum(
        uint256 _quorum
    ) public override onlyOwner validAgendaManager {
    ...
    function setCreateAgendaFees(
        uint256 _fees
    ) external override onlyOwner validAgendaManager {
    ...
    function castVote(
        ...
        (uint256 yes, uint256 no, uint256 abstain) = agendaManager
            .getVotingCount(_agendaID);

        if (quorum <= yes) {
            // yes
            agendaManager.setResult(_agendaID, LibAgenda.AgendaResult.ACCEPT);
            agendaManager.setStatus(
                _agendaID,
                LibAgenda.AgendaStatus.WAITING_EXEC
            );
        } else if (quorum <= no) {
            // no
    ...
    function updateSeigniorage(
        ...
        address candidateContract = _candidateInfos[_candidate]
            .candidateContract;
    ...
    ) internal view returns (AgendaCreatingData memory data) {
        (
            data.target,
    ...
    function payCreatingAgendaFee(address _creator) internal {
        ...
        require(
            IERC20(ton).transferFrom(_creator, address(this), fee),
            "DAOCommittee: failed to transfer ton from creator"
        );
        require(
            IERC20(ton).transfer(address(1), fee),
            "DAOCommittee: failed to burn"
        );
    }
    ...
    function totalSupplyOnCandidateContract(
        address _candidateContract
    ) public view override returns (uint256 totalsupply) {
        require(
            _candidateContract != address(0),
    ...
    function isExistCandidate(
        address _candidate
    ) public view override returns (bool isExist) {
        return _candidateInfos[_candidate].candidateContract != address(0);
    }

    function getClaimableActivityReward(
        address _candidate
