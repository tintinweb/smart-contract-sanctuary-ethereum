                                   ...
                                   AccessControlUpgradeable 
{
    ...
    function initialize(address _uniswapV3Factory, address _usdc, uint24 fee) public initializer {
    ...
    function _getPoolForRoute(PoolAddress.PoolKey memory _poolKey) public view returns (address pool) {
    ...
    function changeActive(address token, bool active) public override onlyModerator {
        ...
        require(tokenInfo.pairAsset != address(0) && tokenInfo.pool != address(0), "UniswapV2PriceProvider: token is not listed!");
    ...
    function setTokenAndPair(address token, address pool, uint24 _poolFee, uint32 _period) public onlyModerator {
        require(token != address(0) && pool != address(0) && _period > 0,"UniswapV2PriceProvider: Invalid address");
        require(IUniswapV3Pool(pool).fee() == _poolFee && IUniswapV3Pool(pool).token0() == token || IUniswapV3Pool(pool).token1() == token, "UniswapV2PriceProvider: Invalid pair!");
        UniswapV3Metadata storage tokenInfo = uniswapV3Metadata[token];
    ...
    function getPrice(address token) public override view returns (uint256 price, uint8 priceDecimals) {
        ...
        require(tokenInfo.pool != address(0) && tokenInfo.isActive, "UniswapV2PriceProvider: token is not active");
        address uniswapPair = tokenInfo.pool;
        ...
        int24 timeWeightedAverageTick = getTWAT(tokenInfo);
    ...
    function getTWAT(UniswapV3Metadata memory tokenInfo) public view returns (int24 twat) {
        address pool = tokenInfo.pool;
        uint32 period = tokenInfo.period;
        twat = OracleLibrary.consult(pool, period);
    }
    ...
    function getSpot(address _pool) public view returns(int256 spotTick) {
