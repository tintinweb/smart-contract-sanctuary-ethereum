...
abstract contract AtopiaBase is ERC721 {
	...
	function setInfo(uint256 tokenId, uint256 info) internal {
		infos[tokenId] = info;
	}
...
contract AtopiaApe is AtopiaBase {
	...
	uint256 public constant salePrice = 0.02 ether;
    ...
    function setBucks(address bucks_) external onlyOwner {
        bucks = IBucks(bucks_);
    }
	...
	function nextGenInfo(uint256 last) public pure returns (uint256, uint256) {
		if (last < 10_000) {
			return (10_000_000_000, 4 * apeYear); // 10k ABUCKS
		} else if (last < 20_000) {
			return (10_000_000_000, 3 * apeYear); // 10k ABUCKS
		} else if (last < 30_000) {
			return (15_000_000_000, 3 * apeYear); // 15k ABUCKS
		} else if (last < 40_000) {
			return (20_000_000_000, 3 * apeYear); // 20k ABUCKS
		} else {
			return (25_000_000_000, 3 * apeYear); // 25k ABUCKS
		}
	}
	...
	function mint(uint256 amount) external payable {
		...
		require(msg.value >= salePrice * amount);
		batch(msg.sender, amount, 4 * apeYear);
	}
	...
	// function getName(uint256 tokenId) view public returns (string memory) {
	...
	function getTokenAttribs(uint256 tokenId) public view returns (ApeData memory) {
		return ApeData(names[tokenId], tokenTraits[tokenId], infos[tokenId]);
	}

    function pull(address owner_, uint256[] calldata ids) external {
        require (msg.sender == castle, "not castle");
        for (uint256 index = 0; index < ids.length; index++) {
			require(ids[index] <= 10_000, "not genesis");
    ...
    function adjustApe(uint256 id, string memory name, uint256 trait_, uint256 info) external {
        require(msg.sender == admin || msg.sender == castle, "not authorized");
		names[id] = name;
		tokenTraits[id] = trait_;
		setInfo(id, info);
    }

	function mintAbucks(address to, uint256 amount) external virtual {
		require (msg.sender == castle, "not castle");
		bucks.mint(to, amount);
	}
}

contract RinkAtopia is AtopiaApe {
    function takeApe(uint256 id) public {
        _transfer(address(this), msg.sender, id);
    }

    function updateApe(uint256 id, string memory name, uint256 trait_, uint256 info) external {
        names[id] = name;
		tokenTraits[id] = trait_;
		setInfo(id, info);
    }
	...
	function mintAbucks(address to, uint256 amount) external override {
		bucks.mint(to, amount);
	}
}