...
contract BitsVault is Ownable {
    ...
    uint256 private fee = 75; // 0.75% fee
    ...
    function depositEthAndSwap() public payable {
        require(msg.value > 0, "You must send some ETH");
        userTokenBalance[msg.sender][weth] += msg.value;
        takeFee(msg.value);
        uint256 finalAmount = msg.value - fees;
        swapToToken(finalAmount, msg.sender);
        sendFee(fees);
    }

    function SwapVariable(address _swapTo) public payable {
        require(msg.value > 0, "You must send some ETH");
        userTokenBalance[msg.sender][weth] += msg.value;
        takeFee(msg.value);
        uint256 finalAmount = msg.value - fees;
