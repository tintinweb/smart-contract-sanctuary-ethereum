...
contract Proxy is Ownable, IERC721Receiver {
    ...
    event ERC721Transaction(address receiver, uint256 assetId);
    Storage private st;
    ...
    function proxy(address coin, address target, uint amount, bytes calldata data) payable public returns (bytes memory) {
        require(Address.isContract(target), "[WPROXY] target address must be smartcontract");

        require(st.checkAccess(msg.sender), "[WPROXY] sender is not allowed");

        IERC20(coin).approve(target, amount);
    ...
    function proxyDERC721(address coin, address target, address receiver, uint amount, bytes calldata data) external {
        proxy(coin, target, amount, data);
        ...
        IERC721(addr).safeTransferFrom(address(this), receiver, asset);
        emit ERC721Transaction(receiver, asset);
    }
    ...
    function transferTokens(address coin, address to, uint amount) external onlyOwner returns (bool success){
        return IERC20(coin).transfer(to, amount);
    }

    function transferETH(address payable to, uint amount) external payable onlyOwner {
