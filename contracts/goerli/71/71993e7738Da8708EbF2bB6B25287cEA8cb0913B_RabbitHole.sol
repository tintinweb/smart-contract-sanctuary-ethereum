...
contract RabbitHole is Ownable, ReentrancyGuard {
    ...
    // will not auto rebase if `autoRebase` is False
    bool public autoRebase = false;
    // the chef role of the token, need to be set before add pool
    ...
    function add(
        ...
        require(CARROT.hasRole(CHEF_ROLE, address(this)), "CHEF_ADD:: need to set chef role of token first");
        if (_withUpdate) {
            massUpdatePools();
        }
