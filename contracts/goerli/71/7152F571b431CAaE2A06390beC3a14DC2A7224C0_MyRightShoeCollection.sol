...
import "./templates/MyRightShoe.sol";

contract MyRightShoeCollection is MyRightShoe {
    constructor(
...
Initializable
{
    ...
    struct DeploymentConfig {
        ...
        // Use `withdrawFees()` to transfer contract balance to the treasury and pool addresses.
    ...
    /// Contract name
    string public NAME = "My Right Shoe";
    ...
    external
    payable
    paymentProvided(_runtimeConfig.publicMintPrice)
    {
        require(mintingActive(), "Minting has not started yet");

            _tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            _mintTokens(msg.sender, _tokenId);
            _setTokenURI(_tokenId, uri);
    }

    /// Mint tokens if the wallet has been whitelisted
    function presaleMint(string memory uri, bytes32[] calldata proof)
    external
    payable
    paymentProvided(_runtimeConfig.presaleMintPrice)
    {
    ...
    function reserveMint(string memory uri)
    external
    onlyRole(ADMIN_ROLE)
    {
        require( 1 <= reserveRemaining, "Not enough reserved");
    ...
    function withdrawFees() external onlyRole(ADMIN_ROLE) {
        ...
        _deploymentConfig.poolAddress.sendValue( _contractBalance * _deploymentConfig.poolPercentage / 100);
    ...
    function _mintTokens(address to, uint256 tokenId) internal {
        require( 1 <= _deploymentConfig.tokensPerMint, "Amount too large");
    ...
    pure
    {
        ...
        require(
            config.poolAddress != address(0),
            "Pool address cannot be null"
        );
    ...
    function poolAddress() public view returns (address) {
        return _deploymentConfig.poolAddress;
    }
