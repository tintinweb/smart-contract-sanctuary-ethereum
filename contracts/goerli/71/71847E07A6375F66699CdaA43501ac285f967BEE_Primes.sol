...
contract Primes is ERC721r, Ownable {
    ...
    uint256 private constant MAX_SUPPLY = 30; // 1576
    uint256 public gifts = 25;
    ...
    function mint(uint256 _quantity) external payable isUser {
        ...
        require(_quantity <= maxPerAddress, "Primes: can't mint that quantity");
        ...
        returns (string memory)
    {
        ...
        string
            memory description = "One of 1575 unique Primes, the first collection of scalable, animated and interactive graphics by g56d.";
    ...
    function generatePaths(uint256 _tokenId)
        ...
        returns (string memory)
    {
        uint256 start_count = _tokenId * 81 - 80;
        uint256 x = 0;
        uint256 y = 0;
        ...
        for (y = 0; y <= 800; y += 100) {
            for (x = 0; x <= 800; x += 100) {
                colorIndex = x / 100;
                if (isPrime(uint256(start_count), 2)) {
                    paths = strConcat(
                        paths,
                        string(
                            abi.encodePacked(
