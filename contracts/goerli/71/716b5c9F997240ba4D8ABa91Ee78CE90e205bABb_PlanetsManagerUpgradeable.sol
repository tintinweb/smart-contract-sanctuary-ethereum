    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    function createPlanetWithTokens(
        ...
        require(
            bytes(planetName).length > 3 && bytes(planetName).length < 32,
            "Planets: Name size invalid"
        );
        require(
            planetValue >= creationMinPrice,
            "Planets: Planet value set below creationMinPrice"
        );
        require(
        ...
        // (, uint256 feeAmount) = getProcessingFee(planetValue);
    ...
    function isCompoundable(PlanetEntity memory planet)
        private
        view
        returns (bool)
    {
