...
contract LIFECRYPTO is AccessControlEnumerable, Ownable{
    ...
    //Write Method
    function createFreeUsername(address wAddress) public returns (bool) {
        string memory genUname = string.concat("user",Strings.toString(block.timestamp)); //'block.timestamp' for unique number
        _setUserName(genUname,wAddress);
        return true;
    }
