...
contract ICO {
    ...
    error CannotWithdrawNotInOpenPhase();
    ...
    /**
     * Used by owner to pause the contract
     * @param _isPaused True for pause, false for not pause
     */
    ...
    function redeem(address recipient) external isNotPaused {
        ...
        // transfer status wont be checked as spaceCoin is deployed by us
    ...
    function withdraw(address to) external onlyOwner {
        if (currentPhase != Phase.Open) {
            revert CannotWithdrawNotInOpenPhase();
        }

        uint256 _ethBalance = address(this).balance;
        (bool success, ) = to.call{value: _ethBalance}("");
        if (!success) {
            revert WithdrawUnsuccessful();
        }

        emit Withdraw(to, _ethBalance);
    }
