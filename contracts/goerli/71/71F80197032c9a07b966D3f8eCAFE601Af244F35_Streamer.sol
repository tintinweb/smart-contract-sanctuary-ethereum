...
contract Streamer is Ownable {
    ...
    function withdrawEarnings(Voucher calldata voucher) public onlyOwner {
        ...
        */
        address clientAddress = ecrecover(
            prefixedHashed,
            voucher.sig.v,
            voucher.sig.r,
            voucher.sig.s
        ...
        require(balances[clientAddress] > voucher.updatedBalance);
        uint256 payment = balances[clientAddress] - voucher.updatedBalance;
        balances[clientAddress] = balances[clientAddress] - payment;

        address owner = owner();
        payable(owner).transfer(payment);

        emit Withdrawn(clientAddress, payment);
    }
    ...
    */
    ...
    function defundChannel() public {
        require(canCloseAt[msg.sender] != 0, "channel hasn't been challenged");
        require(
