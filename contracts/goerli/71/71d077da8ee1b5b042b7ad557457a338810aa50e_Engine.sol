...
library QueuePickOrders {
    ...
    {
        res = x.data[x.currentIdToRead + id];
        ...
        returns (uint256 id)
    {
        ...
        id = x.currentIdToWrite - x.currentIdToRead;
        x.currentIdToWrite++;
    }
...
contract Engine is ERC721, IEngineEvents {
    ...
    function cancelOrder(uint256 queueId) external {
        ...
        emit OrderCanceled(msg.sender, queueId, o.matchId);
        o.canceled = true;
    }
    ...
    function join(uint256 n) internal {
        ...
        if (m.maker == 0) {
            emit MatchInexistant(order.matchId, opens.length());
            return;
        }
        ...
        if (order.owner == maker) {
            emit MakerIsTrader(order.matchId, maker, opens.length());
            return;
        }
