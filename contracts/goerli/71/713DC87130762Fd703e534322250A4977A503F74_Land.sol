...
contract Land is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    function farmConditionVerifier(
        ...
        uint256 minStableTerm = FarmContract.minStableTerm();
        uint256 maxTerm = nums[1];
        uint256 maxStableTerm = FarmContract.maxStableTerm();
        uint256 maxFarmSize = FarmContract.maxFarmSize();
        uint256 destablingFee = nums[2];
        uint256 minDestablingFee = FarmContract.minDestablingFee();
        uint256 ownerFee = nums[3];
        uint256 maxOwnerFee = FarmContract.maxOwnerFee();

        if (ids.length != 1) {
            for (uint256 i = 0; i < ids.length; i++) {
                if (i < ids.length - 1) {
                    require(
                        keccak256(abi.encodePacked(landTypes[ids[i]])) ==
                            keccak256(abi.encodePacked(landTypes[ids[i + 1]])),
