    ...
    CommonChecks
{
    ...
    function state(uint _proposalId) public view returns (ProposalState) {
        ...
        require(proposal.voteStart != 0, "Governor: INVALID_ID");
        ...
        if (proposal.rejected) {
            ...
            quorumReached(
                proposal.forVotes,
                token.getPastTotalSupply(proposal.voteStart) *
            ...
            quorumReached(
                proposal.forVotes,
                token.getPastTotalSupply(proposal.voteStart) * quorumNumerator
            ) && proposal.forVotes > proposal.againstVotes
        ) {
            return ProposalState.Succeeded;
        } else {
     ...
     * @param _supplyMulNumerator Supply multiplied by numerator of the proposal
     */
    function quorumReached(uint128 _forVotes, uint _supplyMulNumerator)
        public
        view
        returns (bool)
    {
        return _supplyMulNumerator / denominator <= _forVotes;
    }
    ...
    function propose(
        ...
        require(
            token.getPastVotes(_msgSender(), block.number - 1) >=
                proposalThreshold ||
    ...
    function _countVote(
        ...
        if (_support == uint8(VoteType.Against)) {
            ...
            revert("Governor: INVALID_SUPPORT");
        }
    }
