...
contract PreSale is Validatable {
    ...
    /**
     *  @notice mapping user address => id > bool is white list of user
     */
    mapping(address => mapping(uint256 => bool)) public isWhiteList;
    ...
    event SetWhitelist(uint256 indexed id, address[] users, bool allow);
    ...
    function createPreSale(
        ...
        uint256 endTime,
        uint256 price
    ) external onlyAdmin notZero(maximum) notZero(maxPerUser) {
    ...
    function updatePreSale(
        ...
        uint256 endTime,
        uint256 price
    ) external onlyAdmin validPreSaleId(id) notZero(maximum) notZero(maxPerUser) {
        require(startTime > 0 && startTime < endTime, "Invalid time");
        require(admin.isPermittedPaymentToken(paymentToken), "Invalid payment token");
        ...
        preSaleInfo.maxPerUser = maxPerUser;
    ...
    function setWhitelist(
        ...
        bool allow
    ) external onlyAdmin validPreSaleId(id) {
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            ...
            isWhiteList[receivers[i]][id] = allow;
        }

        emit SetWhitelist(id, receivers, allow);
    }
     ...
     * emit {Bought} events
     */
    function buy(uint256 id, uint256 times) external payable notZero(times) validPreSaleId(id) {
        require(isWhiteList[_msgSender()][id], "User is not white list");
        PreSaleInfo storage preSaleInfo = preSales[id];
        require(preSaleInfo.status, "PreSale was cancel");
        require(
        ...
        require(preSaleInfo.boughtQty < preSaleInfo.maximum, "Sold out");
        require(preSaleInfo.boughtQty + times <= preSaleInfo.maximum, "Exceed the allowed qty");
        require(boughtAmountOf[_msgSender()][id] + times <= preSaleInfo.maxPerUser, "Limit times each user");

        boughtAmountOf[_msgSender()][id] += times;

        // Payment
        TransferHelper._transferToken(preSaleInfo.paymentToken, times * preSaleInfo.price, _msgSender(), treasury);
