...
contract Spaceship is ERC721EnumerableUpgradeable, NonblockingReceiverUpgradeable, IMelaxyEventsAndErrors {
    ...
    modifier onlyBlindBox() {
        require(msg.sender == blindBox, "not blindBox");
        _;
    }
    ...
    // rarity => cardIds
    mapping(uint => uint[]) private rarityCardIdsMap;
    mapping(uint => uint) private rarityCardNumMap;
    ...
    mapping(uint => bool) public forMerge;
    mapping(uint => MergeInfo) public mergeMap;
    //rarity=>scales
    mapping(uint => uint) private scalesMap;
    //rarity=>base merge price
    mapping(uint => uint) public mergePriceMap;
    IERC20Upgradeable  public mergeToken;
    // mergeIds
    EnumerableSetUpgradeable.UintSet private mergeIds;

    function initialize(address _layerZeroEndpoint, uint _startMintId, uint _endMintId) external initializer {
        __ERC721_init_unchained("Spaceship", "SHIP");
        ...
        if (_endMintId > 0) {
            ...
            MAX_MINT_ETHEREUM = 200000;
        }
    }

    function setBlindBox(address blindBox_) onlyOwner public {
        if (blindBox_ == address(0)) {
    ...
    function setCardInfos(uint _maxRarity, uint[] calldata _cardIds, uint[] calldata _cardPic, uint[] calldata _cardRarity) external onlyOwner {
        if (_cardIds.length == 0) {
            revert ArrayIsNull();
        }
        if (_cardIds.length != _cardPic.length || _cardIds.length != _cardRarity.length) {
        ...
        for (uint i = 0; i < length; i++) {
            ...
            delete cardInfoes[cardId];
            cardIds.remove(cardId);
        }
        for (uint i = 0; i < maxRarity; i++) {
            delete rarityCardNumMap[i + 1];
            delete rarityCardIdsMap[i + 1];
        }
        for (uint i = 0; i < _cardIds.length; i++) {
            require(_cardIds[i] != 0);
            cardInfoes[_cardIds[i]] = CardInfo({
            cardId : _cardIds[i],
            pic : _cardPic[i],
            rarity : _cardRarity[i]
            });
            cardIds.add(_cardIds[i]);
            rarityCardNumMap[_cardRarity[i]] = rarityCardNumMap[_cardRarity[i]] + 1;
            rarityCardIdsMap[_cardRarity[i]].push(_cardIds[i]);
        }
    ...
    function mint(uint cardId_) onlyBlindBox public returns (uint256) {
        if (cardId_ == 0 || cardInfoes[cardId_].rarity < 1) {
            revert WrongCardId();
        }
        uint tokenId = nextTokenId++;
        if (tokenId > MAX_MINT_ETHEREUM) {
            revert MintExceedsSupply();
        }
        _mint(blindBox, tokenId);
        cardIdMap[tokenId] = cardId_;
        rarityMap[tokenId] = cardInfoes[cardId_].rarity;
        return tokenId;
    }

    function mintMulti(uint cardId_, uint amount_) onlyOwner external {
    ...
    // Merge Methods
    function startMerge(IERC20Upgradeable _mergeToken, uint[] calldata _mergePrices, uint[] calldata rarityScales) onlyOwner external {
        if (mergeActive) {
            revert MergeHasAlreadyBegun();
        }
        if (_mergePrices.length != (maxRarity - 1) || rarityScales.length != (maxRarity - 1)) {
            revert ArrayLengthError();
        }
        ...
        for (uint i = 0; i < _mergePrices.length; ++i) {
            mergePriceMap[i + 1] = _mergePrices[i];
        }
        //remove previous quota first
        for (uint i = 0; i < rarityScales.length; ++i) {
            scalesMap[i + 1] = rarityScales[i];
        }
    }
    ...
    function addMerge(uint tokenIdA, uint tokenIdB, uint price) payable onlyEOA whenMergerActive external {
        ...
        uint rarityA = rarityMap[tokenIdA];
        uint rarityB = rarityMap[tokenIdB];
        if (tokenIdA == tokenIdB || rarityA != rarityB || rarityA >= maxRarity) {
        ...
        uint basePrice = mergePriceMap[rarityA];
        if (price < basePrice) {
        ...
        if (address(operator) != _msgSender()) {
            approve(address(operator), tokenIdA);
            approve(address(operator), tokenIdB);
        }
    ...
    function getCardIdBySeed(uint[] storage cardIds, address user, bytes32 seed, bytes32 transactionHash, uint index) internal view returns (uint) {
        uint mod = cardIds.length;
        uint rand = uint(keccak256(abi.encodePacked(
                block.timestamp,
                block.difficulty,
                mod,
    ...
    function toMerge(uint tokenIdA, bytes32 randomness_, bytes32 transactionHash) onlyOperator external {
        ...
        uint rarityA = rarityMap[tokenIdA];
        bool mergeResult = getMergeResult(scalesMap[rarityA], mergeInfo.user, randomness_, transactionHash);
        bool success = false;
        if (mergeResult) {
            success = true;
            uint newRarity = rarityA + 1;
            rarityMap[tokenIdA] = newRarity;
            //ModeL ONE
            uint index = 1;
            uint cardId = getCardIdBySeed(rarityCardIdsMap[newRarity], mergeInfo.user, randomness_, transactionHash, index);
            cardIdMap[tokenIdA] = cardId;
        }
        ...
        emit MergeSuccess(tokenIdA, mergeInfo.tokenIdB, mergeInfo.user, rarityMap[tokenIdA], cardIdMap[tokenIdA], success, transactionHash);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != to) {
            MergeInfo memory mergeInfo = mergeMap[tokenId];
            if (mergeInfo.tokenIdA == tokenId) {
                delete forMerge[tokenId];
                delete forMerge[mergeInfo.tokenIdB];
                delete mergeMap[tokenId];
                mergeIds.remove(tokenId);
            }
        }
    }
