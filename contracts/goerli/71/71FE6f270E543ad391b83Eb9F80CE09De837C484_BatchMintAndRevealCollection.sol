 ...
 * with collaborators.
 */
contract BatchMintAndRevealCollection is
  AccessControlUpgradeable,
  ERC721BurnableUpgradeable,
  ERC721RoyaltyUpgradeable,
  IBatchMintAndRevealCollection,
  IGetRoyalties,
  IGetFees,
  IRoyaltyInfo,
  ITokenCreator
{
   ...
   * @dev Notes:
   *   a) when collection isn't revealed, baseURI is set to full path of the default pre-reveal image
   *      of the collection. e.g. `www.foodomain.com/my/default/pre/reveal/image.jpg`
   *   b) TokenId is naively concatenated with the baseURI to obtain an NFT in the collection.
   ...
   * @dev At collection creation the `creator` is given `admin` and `minter` roles by defaulyt.
   */
   ...
   */
  address private immutable collectionDropFactory;
   ...
   * @dev If baseURIHash is empty, signals that the collection has been revealed.
   * When both baseURI and baseURIHash are set, baseURI is the pre-reveal content
   * and signals the collection has not been revealed yet.
   */
  ...
  constructor(address _collectionDropFactory) {
    require(_collectionDropFactory.isContract(), "BatchMintAndRevealCollection: Factory is not a contract");
    collectionDropFactory = _collectionDropFactory;
  }
   ...
   * @param _additionalMinterAddress An additional address to grant MINTER_ROLE.
   * @param _name The name of this collection.
   * @param _symbol The symbol for this collection.
   * @param _baseURIHash The hash of the revealed baseURI for the collection.
   * @param _tokenURIPreReveal The full path to the pre-reveal image of the collection.
   * @param _maxTokenId The max token id for this collection.
   * @dev Notes:
   *   a) `_creator` is granted `MINTER_ROLE` and `DEFAULT_ADMIN_ROLE` by default.
   *   b) Set `_additionalMinterAddress` to address(0) if not needed.
   *   c) For a collection without a reveal, set `_tokenURIPreReveal`
   *      to the actual baseURI (e.g. "ipfs://foo/") and do not set `_baseURIHash`.
  ...
  function initialize(
    ...
    require(
      msg.sender == address(collectionDropFactory),
      "BatchMintAndRevealCollection: Must be created via the factory"
    );
    require(bytes(_symbol).length > 0, "BatchMintAndRevealCollection: `_symbol` must be set");
    require(_paymentAddress != payable(0), "BatchMintAndRevealCollection: `_paymentAddress` must be set");
    require(bytes(_tokenURIPreReveal).length > 0, "BatchMintAndRevealCollection: `_tokenURIPreReveal` must be set");
    require(_maxTokenId > 0, "BatchMintAndRevealCollection: `_maxTokenId` must be set");
    ...
    if (_additionalMinterAddress != address(0)) {
      _grantRole(MINTER_ROLE, _additionalMinterAddress);
    }
   ...
   * @param marketToApprove The address of the contract to approve operator privelledges
  ...
  ) external onlyRole(MINTER_ROLE) returns (uint256 firstTokenId) {
    require(latestTokenId + count <= maxTokenId, "BatchMintAndRevealCollection: Exceeds max tokenId");
  ...
  function reveal(string memory _baseURI) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(baseURIHash != bytes32(0), "BatchMintAndRevealCollection: Already revealed");
    require(bytes(_baseURI).length != 0, "BatchMintAndRevealCollection: `_baseURI` must be set.");
    delete baseURIHash;
    baseURI = _baseURI;
  ...
  function selfDestruct() external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(totalSupply() == 0, "BatchMintAndRevealCollection: Minted NFTs must be burned first");
  ...
  function updatePreRevealContent(string memory _baseURI, bytes32 _baseURIHash) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(baseURIHash != bytes32(0), "BatchMintAndRevealCollection: Already revealed");
    require(_baseURIHash != bytes32(0), "BatchMintAndRevealCollection: use `reveal` instead");
  ...
  function _updateMaxTokenId(uint32 _maxTokenId) internal {
    require(_maxTokenId != 0, "BatchMintAndRevealCollection: `_maxTokenId` may not be 0");
    require(
      _maxTokenId < maxTokenId || latestTokenId == 0,
      "BatchMintAndRevealCollection: `_maxTokenId` may not increase"
    );
    require(latestTokenId <= _maxTokenId, "BatchMintAndRevealCollection: _maxTokenId must >= latestTokenId");
    maxTokenId = _maxTokenId;
  ...
  /**
   * @notice Returns whether the collection has been sold out.
   */
  function isSoldOut() external view returns (bool soldOut) {
    soldOut = latestTokenId == maxTokenId;
  }
    ...
    returns (bool interfaceSupported)
  {
    if (
      ...
      interfaceId == type(IBatchMintAndRevealCollection).interfaceId
    ) {
