...
contract HelixMultiTranchedPool is BaseUpgradeablePausable, IHelixMultiTranchedPool, IRequiresUID, IVersioned {
  ...
  function deposit(
    ...
    require(block.timestamp <= fundingDate, "funding date is not reached");
    require(trancheInfo.principalDeposited  + amount <= trancheInfo.maximumInvestment, "Exceeds maximum amount");
   ...
   */
  function drawdown() external override onlyLocker whenNotPaused returns(uint256) {
...
//   }
  ...
  function currentInvestmentRatio() public view returns(uint256) {
     PoolSlice storage currentSlice = _poolSlices[numSlices - 1];
  ...
  function _withdraw(
    ...
    require(
     poolShares.approve(address(this), amount) && hasAllowedUID(msg.sender),
      "NA"
    );
    /// @dev IA: invalid amount. Cannot withdraw 0
    require(amount > 0, "IA");
    /// @dev IA: exceeded amount. Cannot withdraw 0
    ...
    if (block.timestamp < fundingDate || !poolFundingEvaluate()) {
      poolShares.burn(msg.sender, amount);
      principalToRedeem = amount;
    } else {
      (interestToRedeem, principalToRedeem) = poolShares.withdrawableFundsOfByShares(msg.sender, amount);
    }
