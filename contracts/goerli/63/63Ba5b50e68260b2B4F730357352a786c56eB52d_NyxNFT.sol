...
contract NyxNFT is ERC1155Supply, Ownable
{
    ...
    function getMaxSupplyFromMintableOLDOLD(uint256 supply)
        public view
        returns (uint256)
    {
        uint256 max_supply = supply - supply % nyx_mint_pct;
        if (supply % nyx_mint_pct > supply * nyx_mint_pct / 100)
        {
            max_supply += supply % nyx_mint_pct;
        }
        else
        {
            max_supply += supply * nyx_mint_pct / 100;
        }
         return max_supply;
    }

    function getMaxSupplyFromMintableOLD(uint256 supply)
        public view
        returns (uint256)
    {
        uint256 max_supply1 = supply - supply % nyx_mint_pct + supply / nyx_mint_pct;
        // uint256 max_supply2 = supply;
        uint256 max_supply2 = supply + supply / nyx_mint_pct;
        if (max_supply1 > max_supply2)
        {
            return max_supply1;
        }
        else
    ...
    function getMaxSupplyFromMintable(uint256 supply)
        public view
        returns (uint256)
    {
        return supply + supply / nyx_mint_pct;
    }
        ...
        private
    {
        ...
        waveMaxSupply[wave] = getMaxSupplyFromMintable(_waveMintableSupply);
        waveWithdrawl[wave] = 0;
        waveTokenId[wave] = currentTokenId;
        waveVotingPower[wave] = votingPower;
        waveIsRunning[wave] = true;
        ...
        public onlyApproved
    {
        ...
        uint256 mint_amount = getMaxSupplyFromMintable(waveCurrentSupply[wave]) - waveCurrentSupply[wave];
        if (mint_amount > 0)
        {
