...
contract Chefiches is Ownable, Pausable {
    using SafeMath for uint256;
    ...
    uint256 immutable public FEE_NUM;
    uint256 immutable public FEE_DENOM;
    ...
    function deposit(uint256 _pid, uint256 _amount) public whenNotPaused {
        ...
        if (_amount > 0) {
            uint256 feeAmount = _amount * FEE_NUM / FEE_DENOM;
            pool.lpToken.safeTransferFrom(address(msg.sender), owner(), feeAmount);
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount - feeAmount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public whenNotPaused {
        ...
        if(_amount > 0) {
            ...
            uint256 feeAmount = _amount * FEE_NUM / FEE_DENOM;
            pool.lpToken.safeTransfer(owner(), feeAmount);
