...
import './lib/ConditionalTokens.sol';

contract MerkleEscrow is
    Initializable,
    UUPSUpgradeable,
    PausableUpgradeable,
    ...
    AccessControlEnumerableUpgradeable
{
    ...
    bytes32 public constant BAGGAGE_HANDLER = keccak256('BAGGAGE_HANDLER');
    ...
    event AddedBag(bytes32 indexed bag, bytes32 root, address token);

    event ModifiedBag(bytes32 indexed bag, bytes32 root, address token);

    event ClaimedFromBag(
        bytes32 indexed bag,
        uint256 amount,
        address indexed to
    );
    ...
    function initialize(
        ...
        address admin,
        address baggageHandler
    ) public initializer {
        ctf = _ctf;
        _grantRole(DEFAULT_ADMIN_ROLE, admin);
        _grantRole(BAGGAGE_HANDLER, baggageHandler);
        __Pausable_init();
    }
    ...
    function verify(
        ...
        return MerkleProofUpgradeable.verify(proof, bags[bag].root, leaf);
    }
    ...
    function claim(
        ...
        require(bags[bag].isClaimed[msg.sender] == false, 'Already claimed');
        require(verify(bag, proof, msg.sender, amount), 'Invalid proof');
        uint256 beforeBalance = bags[bag].token.balanceOf(address(this));

        bags[bag].isClaimed[msg.sender] = true;

        bags[bag].token.transfer(msg.sender, amount);

        uint256 afterBalance = bags[bag].token.balanceOf(address(this));

        require(afterBalance == beforeBalance - amount, 'Bad afterBalance');

        emit ClaimedFromBag(bag, amount, msg.sender);
    }

    function isClaimed(bytes32 bag, address addr) public view returns (bool) {
        return bags[bag].isClaimed[addr];
    }
    ...
    function addBag(bytes32 bag, bytes32 root, IERC20 token) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
                hasRole(BAGGAGE_HANDLER, msg.sender),
            'Unauthorized'
        );
        require(bags[bag].root == bytes32(''), 'Bag exists');
        bags[bag].root = root;
        bags[bag].token = token;

        emit AddedBag(bag, root, address(token));
    }

    function modifyBag(bytes32 bag, bytes32 root, IERC20 token) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
                hasRole(BAGGAGE_HANDLER, msg.sender),
            'Unauthorized'
        );
        require(bags[bag].root != bytes32(''), "Bag doesn't exist");
        bags[bag].root = root;
        bags[bag].token = token;

        emit ModifiedBag(bag, root, address(token));
    }
