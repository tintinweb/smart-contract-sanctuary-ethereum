...
contract TodoList {
    ...
    function getTodoIdsByUser(address userAddress) external view returns(uint256[] memory){
    ...
    modifier isOwnerTodoItem(uint256 id){
        require(todoList[id].owner == msg.sender, "Sender is not owner todo item");
        _;
    }
    ...
    function addTodoItem(uint256 id, string memory title, string memory content, bool status) external {
        require(todoList[id].id == 0, "Todo item is exists");
        address owner = msg.sender;
        uint256 createdAt = block.timestamp;
        uint256 updatedAt = block.timestamp;
        TodoItem memory todoItem = TodoItem(id, owner, title, content, createdAt, updatedAt, status);
        todoList[id] = todoItem;
        todoIdsByUser[owner].push(id);
        indexTodoIds[id] = todoIdsByUser[owner].length - 1;
        emit AddTodoList(todoItem);
    }

    // Sửa todo item theo id
    function editTodoItem(uint256 id, string memory title, string memory content, bool status) external isTodoItemExists(id){
