...
contract BuyBack is BuyBackInterface, Manageable {
    ...
    * @notice Guarantor repurchase user NFT.
    ...
    function _transferCoffer(
        ...
        bool success;
        if(FTFrom == address(this)){
            success = FTInterface(FTAddr).transfer(coffer, serviceFee);
        }else{
            success = FTInterface(FTAddr).transferFrom(FTFrom, coffer, serviceFee);
        }
    ...
    * @notice Guarantor repurchase user NFT.
    ...
    function _transfer(
        ...
        success = FTInterface(FTAddr).transfer(NFTOwner, userAmount);
        if (!success) {
            revert FTTransferFailed();
        }
