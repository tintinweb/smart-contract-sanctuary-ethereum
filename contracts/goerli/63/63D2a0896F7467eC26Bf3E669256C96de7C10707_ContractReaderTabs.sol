...
contract ContractReaderTabs is ICreatorExtensionTokenURI {

    uint public constant MAX_PER_WALLET = 3;
    mapping (address => uint) public addressToMintCount;
    address _creator = 0xfB96F5E3C01129f9a8Bfa87413C29b2162Afc1db;

    error CantMintZero();
    error MaxMintExceeded();
    error NoContracts();

    // todo: add open and closing mint blocks
    function mint(uint _amount) external { // TODO: ensure mint is opeon
        if (tx.origin != msg.sender) revert NoContracts();
        if (_amount == 0) revert CantMintZero();
        if (_amount > 3) revert MaxMintExceeded();

        uint count = addressToMintCount[msg.sender];
        if (count + _amount > 3) revert MaxMintExceeded();
    ...
    function generateSVG() public pure returns (string memory) {
        return string.concat(
            '<svg width="1660" height="1300" viewBox="0 0 1660 1300" fill="none" xmlns="http://www.w3.org/2000/svg">',
                ...
                '<stop offset="0" stop-color="#BE00F2"/>',
    ...
    function tokenURI(address creator_, uint256 tokenId) external view override returns (string memory) {
        require(creator_ == _creator, "Bad creator");
        ...
        string memory json = Base64.encode(
            bytes(
                string.concat(
                    '{"name": "Contract Reader Tabs #', toString(tokenId),
                    '", "description": "The first feature-NFT minted from [ContractReader.io](https://contractreader.io), the better way to read and understand smart contracts. This NFT commemorates the launch of the Tabs feature, where you can read contracts side-by-side, explore linked contracts, and subcontracts.", "image": "data:image/svg+xml;base64,',
     ...
     function generateHTML() public pure returns (bytes memory) {
        return abi.encodePacked(
            ...
            '<head>',
                ...
                '<title>Contract Reader Tabs</title>',
                '<style>',
                    'html,',
                    'body {',
                        'margin: 0;',
            ...
            '</head>',
            '<body>',
            abi.encodePacked(generateSVG()),
            '</body>',
            '</html>'
        );
    }

    // Thanks w1nt3r_eth and [hot-chain-svg](https://github.com/w1nt3r-eth/hot-chain-svg)
