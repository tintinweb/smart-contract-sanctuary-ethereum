...
contract NFT is ERC721Consecutive, Ownable {
    using Strings for uint256;
    string internal _uri;

    constructor(
        uint96 premintAmount,
        address preOwner,
        string memory newURI,
        string memory _name,
        string memory _symbol
    ) ERC721(_name, _symbol) {
        require(preOwner != address(0), "preOwner address is zero");
        require(0 < premintAmount, "_maxTotalSupply is zero");
        _uri = newURI;
        _mintConsecutive(preOwner, premintAmount);
    }
