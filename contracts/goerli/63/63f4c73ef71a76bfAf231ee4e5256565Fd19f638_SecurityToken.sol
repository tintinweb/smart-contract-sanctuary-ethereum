...
import "./extensions/ERC20Security.sol";
import "./extensions/MintableSecurity.sol";
import "./ISecurityToken.sol";
import "../Lockdrop/extensions/TokenRecoveryUpgradeable.sol";

/**
 * @notice Implementation of the {ISecurityToken} interface.
 */
contract SecurityToken is ISecurityToken, ERC20Security, MintableSecurity, TokenRecoveryUpgradeable {
    /**
     * @inheritdoc ISecurityToken
     */
    function initialize(string memory domain, string memory name, string memory symbol) external virtual initializer {
        __ERC20Security_init(domain, name, symbol);
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC20Security, MintableSecurity, TokenRecoveryUpgradeable)
        returns (bool)
    {
    ...
    /**
     * @notice Returns a boolean indicating if the contracts are paused.
     */
    function paused() public view virtual override(ERC20Security, SomaContractUpgradeable) returns (bool) {
        return super.paused();
    }

    function _mint(address to, uint256 amount) internal override(ERC20Security, MintableSecurity) {
        super._mint(to, amount);
    }

    function _burn(address from, uint256 amount) internal override(ERC20Security, MintableSecurity) {
...
import "./ERC20HistoryUpgradeable.sol";
import "./ERC20Guard.sol";
import "./SeizableSecurity.sol";

abstract contract ERC20Security is
    ERC20PausableUpgradeable,
    ERC20HistoryUpgradeable,
    ERC20PermitUpgradeable,
    SeizableSecurity,
    ERC20Guard
{
    function __ERC20Security_init(string memory domain, string memory name, string memory symbol)
        internal
        onlyInitializing
    {
        ...
        __ERC20History_init_unchained();
        __EIP712_init_unchained(domain, "1");
        __ERC20Permit_init_unchained(domain);
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(SeizableSecurity, ERC20Guard)
        returns (bool)
    {
        return interfaceId == type(IERC20Upgradeable).interfaceId
            || interfaceId == type(IERC20PermitUpgradeable).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function _burn(address from, uint256 amount) internal virtual override(ERC20Upgradeable, ERC20HistoryUpgradeable) {
        super._burn(from, amount);
    }

    function _afterTokenTransfer(address from, address to, uint256 amount)
        internal
        override(ERC20Upgradeable, ERC20HistoryUpgradeable)
    {
        super._afterTokenTransfer(from, to, amount);
    }

    function _seize(address from, address to, uint256 amount) internal override {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount)
        internal
        virtual
        override(ERC20Upgradeable, ERC20PausableUpgradeable, ERC20Guard)
    {
...
abstract contract MintableSecurity is IMintableSecurity, AccessibleUpgradeable {
    ...
    /**
     * @notice Modifier to restrict a function caller to accounts that have the GLOBAL_MINT_ROLE or LOCAL_MINT_ROLE.
     */
    modifier onlyMintRole() {
        address sender = _msgSender();
        require(hasRole(LOCAL_MINT_ROLE, sender) || hasRole(GLOBAL_MINT_ROLE, sender), "MintableSecurity: UNAUTHORIZED");
        _;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IMintableSecurity).interfaceId || super.supportsInterface(interfaceId);
    }

    /**
     * @inheritdoc IMintableSecurity
     */
    function mint(uint256 amount) public override onlyMintRole {
        address mintTo = SOMA.mintTo();
        _mint(mintTo, amount);
        emit Minted(_msgSender(), mintTo, amount);
    }

    /**
     * @inheritdoc IMintableSecurity
     */
    function burn(uint256 amount) public override onlyMintRole {
        address mintTo = SOMA.mintTo();
        _burn(mintTo, amount);
        emit Burned(_msgSender(), mintTo, amount);
    }
 ...
 * @author SOMA.finance
 * @notice A guarded, mintable and seizable security token.
 */
interface ISecurityToken {
    /**
     * @notice Initialize function for the Security Token contract.
...
import "./IERC20HistoryUpgradeable.sol";
...
/**
 * @notice Implementation of the {IERC20HistoryUpgradeable} interface.
 */
abstract contract ERC20HistoryUpgradeable is Initializable, IERC20HistoryUpgradeable, ERC20Upgradeable {
    /**
     * @notice Initializer for extended contracts.
     */
    function __ERC20History_init() internal onlyInitializing {
        __Context_init_unchained();
        __ERC20History_init_unchained();
    }
     ...
     */
    function __ERC20History_init_unchained() internal onlyInitializing {}

    /**
     * @notice Checkpoint structure.
     * @param fromBlock The block marking the start of the checkpoint.
     * @param balance The balance of an account in a given checkpoint.
     */
    struct Checkpoint {
        uint32 fromBlock;
        uint224 balance;
     ...
     * @param account The account to return the checkpoints of.
     * @param pos The index of the checkpoint to return.
     * @return The checkpoint structure representing the `pos`-th checkpoint for `account`.
     */
    ...
    function _mint(address account, uint256 amount) internal virtual override {
        ...
        require(totalSupply() <= _maxSupply(), "ERC20History: total supply risks overflowing votes");
    ...
    /**
     * @dev Move balance when tokens are transferred.
     *
    ...
    function _moveBalance(address src, address dst, uint256 amount) private {
        if (src != dst && amount > 0) {
            if (src != address(0)) {
                ...
                emit BalanceChanged(src, oldWeight, newWeight);
            }

            if (dst != address(0)) {
                (uint256 oldWeight, uint256 newWeight) = _writeCheckpoint(_checkpoints[dst], _add, amount);
                emit BalanceChanged(dst, oldWeight, newWeight);
            }
        }
    }
        ...
        if (pos > 0 && ckpts[pos - 1].fromBlock == block.number) {
            ...
            ckpts.push(
                Checkpoint({
                    ...
                    balance: SafeCastUpgradeable.toUint224(newWeight)
                })
            );
        }
    }
...
abstract contract ERC20Guard is IERC20Guard, ERC20Upgradeable, GuardableUpgradeable {
     ...
     */
    function canTransferFrom(address _from, address _to, uint256) public view virtual returns (bool) {
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IERC20Guard).interfaceId || super.supportsInterface(interfaceId);
    }
...
abstract contract SeizableSecurity is ISeizableSecurity, AccessibleUpgradeable {
    ...
    modifier onlySeizeRole() {
        address sender = _msgSender();
        require(
            hasRole(LOCAL_SEIZE_ROLE, sender) || hasRole(GLOBAL_SEIZE_ROLE, sender), "SeizableSecurity: UNAUTHORIZED"
        );
        _;
    }
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(ISeizableSecurity).interfaceId || super.supportsInterface(interfaceId);
    }

    /**
     * @inheritdoc ISeizableSecurity
     */
    function seize(address tokenHolder, uint256 amount) public override enableSeizing onlySeizeRole {
        address seizeTo = SOMA.seizeTo();
        _seize(tokenHolder, seizeTo, amount);
        emit Seized(_msgSender(), tokenHolder, seizeTo, amount);
    }

    function _seize(address from, address to, uint256 amount) internal virtual;
}
...
/**
 * @title SOMA ERC20 History Upgradeable Contract.
 * @author SOMA.finance
 * @notice An ERC20 extension to keep a history of each account's voting balance.
 */
interface IERC20HistoryUpgradeable {
    /**
     * @notice Emitted when a token transfer or delegate change results in changes to a delegate's number of votes.
     * @param account The account that had a change in balance.
     * @param previousBalance The previous balance of `account`.
     * @param newBalance The new balance of `account`.
     */
    event BalanceChanged(address indexed account, uint256 previousBalance, uint256 newBalance);

    /**
     * @notice Returns the balance for `account` at the end of `blockNumber`.
     * @param account The account to return the past balance of.
     * @param blockNumber The block number to return ``account``'s balance at.
     * @custom:requirement `blockNumber` must have been already mined.
     * @return The balance of `account` at `blockNumber`.
     */
    function getPastBalance(address account, uint256 blockNumber) external view returns (uint256);

    /**
     * @notice Returns the total supply of votes available at the end of a past block (`blockNumber`).
     * @dev This value is the sum of all available votes, which is not necessarily the sum of all delegated votes.
     * Votes that have not been delegated are still part of total supply, even though they would not participate in a
     * vote.
     * @param blockNumber The block number to return the total supply at.
     * @custom:requirement `blockNumber` must have been already mined.
...
library SOMAlib {
     ...
     */
    ISOMA public constant SOMA = ISOMA(0x0960e25931156C3648631Cc2608648d2D81E3e56);
}
...
interface ISeizableSecurity {
     ...
     */
    event Seized(address indexed operator, address indexed from, address indexed to, uint256 value);
...
interface IMintableSecurity {
     ...
     * @param operator The address of the operator.
     * @param to The seize to address that receives minted tokens.
     * @param value The amount of tokens minted.
     */
    event Minted(address indexed operator, address indexed to, uint256 value);

    /**
     * @notice Emitted after a token is burned.
     * @param operator The address of the operator.
     * @param from The seize to address that receives minted tokens.
     * @param value The amount of tokens minted.
     */
    event Burned(address indexed operator, address indexed from, uint256 value);

    /**
     * @notice Mints a mintable security token.
     * @param amount The amount of tokens to mint.
     * @custom:emits Minted
     * @custom:requirement The function caller must have the GLOBAL_MINT_ROLE or LOCAL_MINT_ROLE.
     */
    function mint(uint256 amount) external;

    /**
     * @notice Burns a mintable security token.
