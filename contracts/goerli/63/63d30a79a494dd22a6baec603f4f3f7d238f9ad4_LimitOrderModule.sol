...
contract LimitOrderModule is IModule {
    function execute(uint256 indexPrice, Order memory order) external pure returns(OrderExecutionResult) {
        ...
        bool isPriceValid = triggerAboveThreshold ? indexPrice >= limitPrice : indexPrice <= limitPrice;
        return isPriceValid ? OrderExecutionResult.FILLED : OrderExecutionResult.SKIP;
    }
