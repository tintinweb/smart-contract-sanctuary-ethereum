...
import './interface/IForwarderFactory.sol';
 ...
 *
 */
contract Forwarder is IERC165, IForwarder, Pausable {
  ...
  function init(
    ...
    // auto wrap, approve, & deposit
    _wrapAndDeposit(ethBal);
  }

  function onlyRelayer() internal view {
    require(_msgSender() == relayerAddress, "relayer only");
  }

  function onlyOwner() internal view {
    require(_msgSender() == IForwarderFactory(factoryAddress).owner(), "owner only");
  }
  ...
  function _approve(address _tokenAddress) internal {
    SafeERC20.safeApprove(IERC20(_tokenAddress), handlerAddress, type(uint256).max);
  }
  ...
  function _deposit(
    ...
    IBridge(bridgeAddress).deposit(_destinationDomainID, _resourceID, _depositData, _feeData);
  }

  // used for native only
  function _wrapAndDeposit(uint256 _ethBalance) internal whenNotPaused {
    // wrap native
    IWETH(wethAddress).deposit{value: _ethBalance}();

    // deposit to bridge
    _deposit(
      100, // destinationDomainID to edge
      bytes32(bytes("WETH")), // resourceID
      abi.encode( // depositData
        address(this), // cVA address
        _ethBalance, // eth balance
        18 // weth decimal
      ),
      new bytes(0) // feeData
    );
  ...
  // used only at 2nd deposit and so on, when ERC20 is already transferred in VA
  function deposit(
    uint8 _destinationDomainID,
    bytes32 _resourceID,
    bytes memory _depositData,
  ...
  function updateConfigAddresses() external whenPaused {
    // owner check
    onlyOwner();

    IForwarderFactory factory = IForwarderFactory(factoryAddress);

    bridgeAddress = factory.bridgeAddress();
    handlerAddress = factory.handlerAddress();
    relayerAddress = factory.relayerAddress();
  }

  /**
   * Default function; Gets called when Ether is deposited with no data, and wrapped it
   */
...
interface IBridge {
    function deposit(uint8, bytes32, bytes calldata, bytes calldata) external payable;
}
