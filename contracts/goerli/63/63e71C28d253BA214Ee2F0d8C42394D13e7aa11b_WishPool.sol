...
contract WishPool is Initializable, Pausable, Ownable {
    ...
    function redeem(uint256 tokenId, RedeemData memory redeemData) public 
        whenNotPaused needInit payable {
        require(!appConf.validBlacklist(_msgSender()), "can not redeem");

        // burn redeem nft
        IBurnNft(appConf.getRedeemNftToken()).burn(tokenId);

        // nft info
        (, uint8 redeemNftGen,) = IFactory(appConf.getNftFactoryAddr()).tokenDetail(Model.NFT_TYPE_WISH, tokenId);
