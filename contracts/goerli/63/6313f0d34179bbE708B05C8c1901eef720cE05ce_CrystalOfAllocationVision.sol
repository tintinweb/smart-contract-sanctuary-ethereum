...
contract CrystalOfAllocationVision is ERC721, ICrystalOfPower {
    ...
    constructor(
        ...
        ERC721(
            "Crystal of Allocation Vision Power",
            "Crystal of Allocation Vision Power"
        )
    {
    ...
    modifier checkMint() {
        ...
        if (gold_list_addresses[msg.sender]) {
            require(msg.value >= 5 * 4 * 10**17, "Not enough funds send");
        } else if (white_list_addresses[msg.sender]) {
            require(msg.value >= 5 * 45 * 10**16, "Not enough funds send");
        } else {
            require(msg.value >= 5 * 55 * 10**16, "Not enough funds send");
        }
        require(
            IERC721(shard_crystal_address).balanceOf(msg.sender) >= 5,
            "Not enough MetaSkynet Shard Crystals in your wallet. Min: 5."
        );
        _;
    }
