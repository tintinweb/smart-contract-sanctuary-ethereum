...
contract MiharaSilverToken is IMiharaToken, ERC721, Ownable, ReentrancyGuard {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory){
        require(_exists(tokenId), "Mihara Token: URI query for nonexistent token");

        return string(
            abi.encodePacked(
                'data:application/json;base64,',
                Base64.encode(bytes(abi.encodePacked('{"name":"Mihara Token #',  Strings.toString(tokenId),'","description": "","image": "https://arweave.net/vYzbo0tGvMUtVnMZJjj9aL1peVTfIXN7Czd0DIJDFZs","animation_url": "https://arweave.net/-ANZ2UOLFHcDL8iSM_tCzUB5OF0X8nzMzuDKvKI1blw"}')))
            )
        );
    }
