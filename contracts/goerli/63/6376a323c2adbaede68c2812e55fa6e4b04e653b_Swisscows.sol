...
contract Swisscows is Ownable, ERC721Enumerable, PaymentSplitter{
    uint256 public MAX_AMOUNT = 8888;
    uint256 public _max_tokens_per_account = 4;
    ...
    PaymentSplitter(team, team_shares)
    {
        ...
        accounts[admins[i]] = Account(0,true);
      }
      ...
      modifier onlyAdmin() {
        require(accounts[msg.sender].isAdmin == true, "No admin");
        _;
    }
    function setListingPrice(uint256 price) external onlyAdmin {
      _listing_price = price;
    }

    function setMaxAmountPerAccount(uint256 maxAmount) external onlyAdmin {
      _max_tokens_per_account = maxAmount;
    }
    ...
    function setRoot(bytes32 newRoot) external onlyAdmin {
    _allowedRoot = newRoot;
    }

    function setAllowListRequired() external onlyAdmin {
    ...
    function mint(uint256 _amount, bytes32[] calldata proof) external payable {
        ...
        require(_amount > 0, "Mint between 1 and 4 Tokens");
        require(totalSupply() + _amount <= (MAX_AMOUNT), "supply limited");
        require(msg.value >= (_listing_price * _amount), "Wrong Ether value");
        require((_amount + accounts[msg.sender].alreadyMinted) <= _max_tokens_per_account, "Max 4 per wallet");
        ...
        for (uint i = 0; i < _amount; i++) {
            id++;
            accounts[msg.sender].alreadyMinted++;
