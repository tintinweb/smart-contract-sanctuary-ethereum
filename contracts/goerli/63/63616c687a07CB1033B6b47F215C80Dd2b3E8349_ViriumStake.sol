...
contract ViriumStake is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    uint256 public stakingDate;
    uint256 public earningDate;
    bool public stakingPanel;
    bool public mintPad;
    ...
    function initialize() initializer public {
        ...
        setDate(block.timestamp, block.timestamp);
        setStakingPanel(true);
        setMintPad(true);
    }

    function setDate(uint256 stakingDate_, uint256 earningDate_) public onlyRole(MAINTAIN_ROLE) {
        stakingDate = stakingDate_;
        earningDate = earningDate_;
    }

    function setStakingPanel(bool stakingPanel_) public onlyRole(MAINTAIN_ROLE) {
        stakingPanel = stakingPanel_;
    }

    function setMintPad(bool mintPad_) public onlyRole(MAINTAIN_ROLE) {
        mintPad = mintPad_;
    }
    ...
    function getEarned(address user) external view returns(uint256){
        return _userShare[user] * (rewardPerShare() - _userRewardPerShareStored[user]) + _rewards[user];
    }
    ...
    function getReward(uint256 t2, uint256 t1) private view returns (uint256){
        if (block.timestamp < earningDate) {
            return 0;
        }
        ...
        uint256 duration = t2 - earningDate;
        if (duration / FIFTY_YEARS > 0) {
            return 0;
        }
        return g(duration) - g(t1 - earningDate);
    }
    ...
    function getCurrentStatus() public view returns (Status){
        if (block.timestamp < stakingDate) {
    ...
    function stake(uint256[][] calldata tokenIds) external updateReward {
        require(getCurrentStatus() == Status.Running, "ViriumStake: Status error");
