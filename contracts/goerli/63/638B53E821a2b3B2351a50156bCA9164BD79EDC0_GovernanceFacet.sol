...
contract GovernanceFacet is IGovernance {
    ...
    function setFees(
        uint256 _startPoolId,
        uint96 _protocolFee,
        uint96 _settlementFee
        ...
        // Store in memory to optimize gas
        uint8 _maxUpdateIdToFees = gs.maxUpdateIdToFees;

        // Check validity of `_startPoolId`
        _isValidStartPoolId(
            _startPoolId,
            gs.updateIdToFees[_maxUpdateIdToFees].startPoolId
        );

        // Store new fee regime in the mapping under a new index incremented by 1
        gs.updateIdToFees[_maxUpdateIdToFees + 1] = LibDIVAStorage.Fees({
            startPoolId: _startPoolId,
            protocolFee: _protocolFee,
            settlementFee: _settlementFee
        });

        // Increment max index
        ++gs.maxUpdateIdToFees;
    ...
    function setSettlementPeriods(
        ...
        // Check validity of new fee periods
        ...
        // Store in memory to optimize gas
        uint8 _maxUpdateIdToSettlementPeriods = gs
            .maxUpdateIdToSettlementPeriods;

        // Check validity of `_startPoolId`
        _isValidStartPoolId(
            _startPoolId,
            gs
                .updateIdToSettlementPeriods[_maxUpdateIdToSettlementPeriods]
                .startPoolId
        );

        // Store new settlement period regime in the mapping under a new index incremented by 1
        gs.updateIdToSettlementPeriods[
            _maxUpdateIdToSettlementPeriods + 1
        ...
        // Increment max index
        ++gs.maxUpdateIdToSettlementPeriods;

        // Log the new settlement period as well as the address that initiated the change
        emit SettlementPeriodSet(
            msg.sender,
            _submissionPeriod,
            _startPoolId,
    ...
    function setFallbackDataProvider(address _newFallbackDataProvider)
        external
        override
        onlyOwner
    {
    ...
    function _isValidFee(uint96 _fee) private pure {
        if (_fee > 0) {
            ...
            if (_fee > 25000000000000000) revert FeeAboveMaximum(); // 2.5% = 0.025
        }
    }

    function _isValidPeriod(uint256 _period) private pure {
        if (_period < 1 days || _period > 15 days) revert OutOfBounds();
    }

    function _isValidStartPoolId(
        uint256 _startPoolId,
        uint256 _latestStartPoolId
    ) private view {
        uint256 _latestPoolId = LibDIVA._getLatestPoolId();
        if (_startPoolId <= _latestStartPoolId) revert InvalidStartPoolId();
        if (_startPoolId <= _latestPoolId) revert InvalidStartPoolId();
    }
}
...
interface IGovernance {
    ...
    // Thrown if startPoolId is smaller than or equal to the previous one
    // smaller than the latest poolId + 100
    error InvalidStartPoolId();

    // Thrown if 0 < protocol/settlement fee < 0.01% (non-zero minimum)
    error FeeBelowMinimum();

    // Thrown if the protocol/settlement fee > 2.5% (maximum)
    error FeeAboveMaximum();

    // Thrown if contract owner attempts to set a settlement related period
    // to less than 1 day or more than 15 days
    error OutOfBounds();

    // Thrown if contract owner attempts to set treasury or the fallback provider
    // to the zero address
    error ZeroAddress();

    // Thrown if contract owner attempts to pause `redeemPositionToken` and
    // `removeLiquidity` before the two day delay period has passed
    error TooEarlyToPauseAgain();

    /**
     * @notice Emitted when the protocol or settlement fee is set.
     * @param from The address that initiated the change (contract owner).
     * @param fee The fee amount in % of the collateral expressed as an integer
     * with 18 decimals.
     * @param startPoolId The poolId at which the new fee regime comes into effect.
    ...
    /**
     * @notice Emitted when a settlement related period is set.
     * @param from The address that initiated the change (contract owner).
     * @param period New length of the final reference value submission period.
     * @param startPoolId The poolId at which the new settlement regime comes into effect.
     * @param periodType Settlement period type.
     */
    event SettlementPeriodSet(
        address indexed from,
        uint24 period,
        uint256 startPoolId,
    ...
    /**
     * @notice Function to set both the protocol and settlement fee in one single tx.
     * @dev Execution restricted to contract owner.
     * @param _startPoolId The poolId at which the new fee regime comes into effect.
    ...
    /**
     * @notice Function to set all settlement related periods in one call.
     * @dev Execution restricted to contract owner.
     * @param _startPoolId The poolId at which the new settlement regime comes into effect.
     ...
     */
    function setFallbackDataProvider(address _fallbackDataProvider) external;
     ...
     * @param _pause Boolean indicating whether to pause or unpause the return
     * of collateral.
     */
