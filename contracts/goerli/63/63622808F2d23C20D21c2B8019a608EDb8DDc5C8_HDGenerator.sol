...
contract HDGenerator is ERC721A, Ownable, Stateable {
    ...
    function revealCollection(string memory _newUri) external onlyOwner {
        if (isRevealed) revert AlreadyRevealed();
        setUriPrefix(_newUri);
        isRevealed = true;
    }

    function setMaxSupply(uint96 _maxSupply) external onlyOwner {
        maxSupply = _maxSupply;
        ...
        notInState(State.Paused)
    {
        ...
        requiredAmount = _mintQuantity * getConversionRate();
    ...
    function adminMint(address _receiver, uint256 _mintQuantity)
        external
        onlyOwner
        notInState(State.Paused)
    {
    ...
    function mint(address _receiver, uint256 _mintQuantity) internal {
        ...
        if (currentSupply + _mintQuantity > maxSupply) revert NotEnoughTokensAvailable();
        _safeMint(_receiver, _mintQuantity);
        for (uint256 i = currentSupply; i < (currentSupply + _mintQuantity); i++) {
            ownerTokens[_receiver].push(i + 1);
        }
    }

    function getConversionRate() internal view returns (uint256) {
        (, int256 price, , , ) = priceFeed.latestRoundData();
        uint256 ethAmountInWei = ((1 * 10**26) / uint256(price)) * mintPrice;
...
contract Stateable {
    ...
    function toggleState(State _state) internal virtual {
        currentState = _state;
