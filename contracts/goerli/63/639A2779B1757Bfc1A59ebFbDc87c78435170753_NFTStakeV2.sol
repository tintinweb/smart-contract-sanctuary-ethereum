...
contract NFTStakeV2 is Ownable{
    ...
    mapping(address => bool) public _migrateV1Rewards;
    ...
    function stakeInfos(uint256 tokenId) public view returns(StakeInfo memory infov2){
        ...
        if(v1.blackTokens(tokenId)) infov2.claimedReward = 0;
    }
    ...
    function stakeInfoAndReward(uint16 tokenId, address _owner) public view returns(uint192 totalReward, StakeInfo memory infov2){
        ...
        if(!v1.blackTokens(tokenId) && infov2.owner == _owner){
            totalReward = (infov2.stakeTime == 0) ? 0 : v1.getReward(infov2.stakeTime, block.timestamp, v1.getPower(tokenId));
        }
    }
    
    function _unstake(uint16 tokenId) internal returns(uint192 reward){
        address _owner = _checkOwner(tokenId);
