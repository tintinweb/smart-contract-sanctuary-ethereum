...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/master/contracts/utils/structs/EnumerableSetUpgradeable.sol
...
abstract contract ABSToken is ERC20, Ownable {
    ...
    // 买入手续费钱包
    address private _buyWallet;
    // 卖出手续费钱包
    address private _sellWallet;
    // 买入滑点
        ...
        ERC20(name_, symbol_)
    {
        ...
        _sellSlippage = 3;
        // usdt地址
        usdt = 0xf869dBf444d0C03aE18706BDFE9770327137Aa92;
        // 给路由授予最大值
        ...
        _feeWhiteList[_buyWallet] = true;
        _feeWhiteList[_sellWallet] = true;
    }
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
            ...
            if (_swapPairList[from] || _swapPairList[to]) {
                ...
                if (_swapPairList[from]) {
                    buyFell = amount.mul(_buySlippage).div(100);
                    amount = amount.sub(buyFell);
                    // sendChangeFee(buyFell);
                }
                // 卖出
                if (_swapPairList[to]) {
                    sellFell = amount.mul(_buySlippage).div(100);
                    amount = amount.sub(sellFell);
                    // sendChangeFee(sellFell);
                }
            }
        }
    ...
    function sendChangeFee(
        uint256 amount
    ) private lockTheSwap {
        if (amount > 0) {
            tokenToUSDT(_buyWallet, amount);
        }
        if (amount > 0) {
            tokenToUSDT(_sellWallet, amount);
        }
    }

    function tokenToUSDT(address to, uint256 amount) private {
...
contract SHIBDAO is ABSToken {
    constructor() ABSToken("Shib lnu DAO", "SHIBDAO") {
        _mint(msg.sender, 589000000000000 * 10**decimals());
    }
}