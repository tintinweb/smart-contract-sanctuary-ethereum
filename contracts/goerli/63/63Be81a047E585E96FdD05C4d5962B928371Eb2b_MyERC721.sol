...
contract MyERC721 is ImmutableERC721Preset {
    ...
    uint256 public constant ADMIN_FEE_PERCENTAGE = 30;
    ...
    function pickWinner() public {
        ...
        uint256 prize = _ticketPool.length.sub(TICKET_PRICE.mul(_ticketPool.length).mul(ADMIN_FEE_PERCENTAGE).div(100));

        // Transfer the prize to the winner and the admin fee to the admin
        payable(winner).transfer(prize.mul(70).div(100));
        payable(super.owner()).transfer(prize.mul(30).div(100));

        // Remove the winning ticket from the pool and delete its mapping
        _ticketPool[winnerIndex] = _ticketPool[_ticketPool.length.sub(1)];
        _ticketPool.pop();
        delete _ticketOwners[winnerId];

        // Transfer the NFT to the winner
        _transfer(ownerOf(winnerId), winner, winnerId);
