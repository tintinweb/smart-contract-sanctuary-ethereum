...
import {INeuralAutomataEngine, NCAParams} from "./INeuralAutomataEngine.sol";
import {Base64} from "./Base64.sol";

contract ZooOfNeuralAutomata is ERC1155, Owned, Base64 {
    ...
    constructor(address _engine) Owned(msg.sender) {
        engine = _engine;
    }
    ...
    function updateParams(uint256 _id, NCAParams memory _params) external onlyOwner {
        tokenParams[_id] = _params;
    }

    function updateMinter(uint256 _id, address _minter) external onlyOwner {
        tokenMinter[_id] = _minter;
    }

    function updateBurner(uint256 _id, address _burner) external onlyOwner {
        tokenBurner[_id] = _burner;
    }

    function updateBaseURI(uint256 _id, string memory _baseURI) external onlyOwner {
        tokenBaseURI[_id] = _baseURI;
    }
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < idsLength; ) {
            require(msg.sender == tokenMinter[_ids[i]]);
            unchecked {
                ++i;
            }
        }
    ...
    function batchBurn(
        ...
        for (uint256 i = 0; i < idsLength; ) {
            require(msg.sender == tokenBurner[_ids[i]]);
            unchecked {
                ++i;
            }
        }
