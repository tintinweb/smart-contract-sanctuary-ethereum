...
// Project Name - Mad Mythics
// Artist - Dr Bonkers 
// Development - Blockgeni3.com
...
contract MadMythics is ERC721A, Ownable {
    ...
    address payable immutable private artaddress = payable(0xddC3A364260e619316E0A5dE60ef00326E8F164d);
    address payable immutable private sladdress = payable(0xddC3A364260e619316E0A5dE60ef00326E8F164d);
    address payable immutable private mkaddress = payable(0xddC3A364260e619316E0A5dE60ef00326E8F164d);
    address payable immutable private gdtaddress = payable(0xddC3A364260e619316E0A5dE60ef00326E8F164d);
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedURI, bytes32 _root) ERC721A("Mad Mythics", "MM") {
    ...
    modifier callerIsUser() {
        require(tx.origin == _msgSender(), "[Error] Function cannot be called by a contract");
        _;
    }

    // ===== Check Mint Compliance =====
    modifier maxWalletCheck(uint256 quantity) {
        require(numberMinted[_msgSender()] + quantity <= MAX_PER_WALLET, "[Error] Max Per Wallet Reached");
        _;
    }
    ...
    // ===== Whitelist Mint =====
    function whitelistMint(bytes32[] memory proof, uint256 quantity) external payable maxWalletCheck(quantity) supplyCheck(quantity) callerIsUser {
        require(!publicMintStarted && whitelistMintStarted, "[Error] Private Mint Not Started");
        require(proof.verify(whitelistMerkleRoot, keccak256(abi.encodePacked(_msgSender()))), "[Error] You are not on the whitelist");

        if(numberMinted[_msgSender()] == 0) {
        ...
        numberMinted[_msgSender()] += quantity;
    ...
    // ===== Mint =====
    function mint(uint256 quantity) external payable maxWalletCheck(quantity) supplyCheck(quantity) callerIsUser {
        require(publicMintStarted, "[Error] Public Mint Not Started");
        require(msg.value >= cost * quantity, "[Error] Not enough funds supplied");
    ...
    // ===== Turn on whitelist mint =====
    function turnOnWhitelistMint() external onlyOwner {
        publicMintStarted = false;
    ...
    // ===== Change Mint Price =====
    function setPublicMintPrice(uint256 value) external onlyOwner {
        require(value > 0, "[Error] Value cannot be 0");
        cost = value;
    }

    // ===== Change Whitelist Mint Price =====
    function setWhitelistMintPrice(uint256 value) external onlyOwner {
        require(value > 0, "[Error] Value cannot be 0");
        whitelistCost = value;
    }
    ...
    function sendFunds(uint256 _totalMsgValue) internal {
        ...
        (bool s2,) = artaddress.call{value: (_totalMsgValue * 30) / 100}("");
        (bool s3,) = sladdress.call{value: (_totalMsgValue * 20) / 100}("");
        (bool s4,) = mkaddress.call{value: (_totalMsgValue * 5) / 100}("");
        (bool s5,) = gdtaddress.call{value: (_totalMsgValue * 20) / 100}("");
        require(s1 && s2 && s3 && s4 && s5, "[Error] Payment Splitter Failure");
    }
