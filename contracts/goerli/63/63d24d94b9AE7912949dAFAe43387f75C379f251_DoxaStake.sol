...
interface Wallet {
    ...
    function depositTokens(address, string memory, uint) external;
...
contract DoxaStake is OwnableUpgradeable,whitelistChecker{
    ...
    function stake(uint256 amount, uint noOfDays,Whitelist memory stakeo) external isblocklist(msg.sender){
        ...
        if(stakeo.id == 0) {
            ...
            wallet.withdraw(msg.sender, Strings.toString(Stakers[msg.sender].length), amount);
        } else {
            revert("Improper stake type");
        }
    ...
    function unstake(uint id, Whitelist memory stakeo) internal {
        ...
        uint stakeType=Stakers[msg.sender][id].stakeType;
        if(stakeType == 0) {
            stakeToken.transfer(msg.sender, amount);
        } else {
            rewardToken.approve(address(wallet), amount);
            wallet.depositTokens(msg.sender, Strings.toString(id), amount);
        }
    ...
    function getReward(uint id,Whitelist memory stakeo) external  updateReward(msg.sender, id) isblocklist(msg.sender){
        ...
        if (reward > 0) {
            ...
            givenRewards+=reward;
           if(Stakers[msg.sender][id].stakeType == 0) {
                rewardToken.transfer(msg.sender, reward);
            } else {
                rewardToken.approve(address(wallet), reward);
                wallet.depositTokens(msg.sender, Strings.toString(id), reward);
            }
	...
	function initialize(address _stakeToken, address _RewardToken,address _walletAddress) external initializer{
          stakeToken=IERC20Upgradeable(_stakeToken);
		  rewardToken=IERC20Upgradeable(_RewardToken);
		  __Ownable_init();
	       signer=msg.sender;
           wallet=Wallet(_walletAddress);
           accuracyFactor = 10 ** 10;
           day = 60;
