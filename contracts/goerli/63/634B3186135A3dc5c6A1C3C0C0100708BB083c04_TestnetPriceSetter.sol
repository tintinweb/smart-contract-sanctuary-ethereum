...
contract TestnetPriceSetter {
    ...
    function setPrices() public {
        ...
        oracle.setDirectPrice(WBTC, uint256(price) * 10**(18 + IERC20Metadata(WBTC).decimals() - decs));

        decs = AggregatorV3Interface(DAI_USD).decimals();
        (, price, , , ) = AggregatorV3Interface(DAI_USD).latestRoundData();
        oracle.setDirectPrice(DAI, uint256(price) * 10**(18 + IERC20Metadata(DAI).decimals() - decs));

        decs = AggregatorV3Interface(USDC_USD).decimals();
        (, price, , , ) = AggregatorV3Interface(USDC_USD).latestRoundData();
        oracle.setDirectPrice(USDC, uint256(price) * 10**(18 + IERC20Metadata(USDC).decimals() - decs));

        decs = AggregatorV3Interface(USDC_USD).decimals();
        (, price, , , ) = AggregatorV3Interface(USDC_USD).latestRoundData();
        oracle.setDirectPrice(USDT, uint256(price) * 10**(18 + IERC20Metadata(USDT).decimals() - decs));
