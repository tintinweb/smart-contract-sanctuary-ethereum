...
interface ITheShopMT is IERC1155Upgradeable {
...
contract NFTCollections is ERC721Upgradeable, DefaultOperatorFiltererUpgradeable, OwnableUpgradeable {
    ...
    ITheShopMT public materialsAndToolsContract;
    ...
    mapping (bytes32 => uint256) public couponHashToDiscountValue;
    ...
    function initialize(address warmXYZ, address delegateCash) public initializer {
        __ERC721_init("NFTCollection", "NFTCollection");
        __Ownable_init();
    ...
    function mintFromCollection(uint256 collectionId, uint256 tokenIdInCollection, string calldata coupon) external payable {
        ...
        if (bytes(coupon).length != 0) {
            ...
            expectedPrice -= couponHashToDiscountValue[couponHash];
        }
    ...
    function mintFromCollectionMultiple(uint256[] calldata collectionIds, uint256[] calldata tokenIdsInCollection, uint256[] calldata proxyTypes, string calldata coupon) external payable {
        ...
        if (bytes(coupon).length != 0) {
            bytes32 couponHash = keccak256(abi.encodePacked(coupon));
            couponHashToUses[couponHash]--;
            sum -= couponHashToDiscountValue[couponHash] * collectionIds.length;
        }
    ...
    function setCoupons(bytes32[] calldata couponHashes, uint256[] calldata uses, uint256[] calldata discounts) external onlyOwner {
        for (uint256 i=0; i<couponHashes.length; i++) {
            couponHashToDiscountValue[couponHashes[i]] = discounts[i];
    ...
    function setMaterialsAndToolsContract(address _contract) external onlyOwner {
        materialsAndToolsContract = ITheShopMT(_contract);
    }
    ...
    /**
     * @dev Mint tokens by specifying collection ids and token ids within collections (callable by owner)
     */
    function runAirdrop(uint256[] calldata collectionIds, uint256[] calldata tokenIds) external {
        require(isAdmin[msg.sender], "Not authorized");
        require(collectionIds.length == tokenIds.length, "Bad lengths");
        
        for (uint256 i = 0; i < collectionIds.length;) {
            require(collections[collectionIds[i]].collectionType == 0, "Bad type");
            address wallet = IERC721Upgradeable(collections[collectionIds[i]].baseToken).ownerOf(tokenIds[i]);
            internalMintFromCollection(collectionIds[i], tokenIds[i], wallet, true, 0);
            unchecked{i++;}
        }
    }

    /**
     * @dev Mint tokens in type 1 collections by specifying collection id and token indexes (callable by owner)
     */
    function runAirdropBasedOnCollection(uint256[] calldata collectionIds, uint256[] calldata tokenIds) external {
        require(isAdmin[msg.sender], "Not authorized");
        require(collectionIds.length == tokenIds.length, "Bad lengths");
        
        for (uint256 i = 0; i < tokenIds.length;) {
            require(collections[collectionIds[i]].collectionType == 1, "Bad type");
            address wallet = ownerOf(collectionIdAndBaseTokenIdToTokenId(collections[collectionIds[i]].baseCollection, tokenIds[i]));
            internalMintFromCollection(collectionIds[i], tokenIds[i], wallet, true, 0);
            unchecked{i++;}
        }
    }
     ...
     */
    function runAirdropFreeForAll(uint256[] calldata collectionIds, uint256[] calldata tokenIds, address[] calldata owners) external {
        require(isAdmin[msg.sender], "Not authorized");
        require(collectionIds.length == tokenIds.length && tokenIds.length == owners.length, "Bad lengths");
        
        for (uint256 i = 0; i < tokenIds.length;) {
            require(collections[collectionIds[i]].collectionType == 2, "Bad type");
