...
contract recurringFactory {
    ...
    function createMetaCoin(uint256 _price, uint256 _numsubscriptions, string memory _name, address[] memory _stableCoins) external {
        Subscription subscription = new Subscription(_price, _numsubscriptions, _name, _stableCoins);
...
contract Subscription is Ownable {
    ...
    constructor(uint256 _rate, uint256 _numsubscriptions, string memory _name, address[] memory _stableCoins) {
        ...
        USDc = iUSDc(_stableCoins[0]);
        Dai = iDai(_stableCoins[1]);
        USDt = iUSDt(_stableCoins[2]);
    }
