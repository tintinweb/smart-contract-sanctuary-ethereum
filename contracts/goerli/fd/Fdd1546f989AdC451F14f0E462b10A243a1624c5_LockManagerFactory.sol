...
abstract contract GovernorMiniBravo is IGovernorMiniBravo, Roles {
  ...
  function queue(uint256 _method, bytes memory _parameters) public {
    ...
    if (_proposal.timelockExpiry != 0) {
      revert ProposalAlreadyQueued(_method, _proposal.id);
    }
    if (!_quorumReached(_proposal.open, _proposal.forVotes)) {
      revert QuorumNotReached(_method, _proposal.id);
    }
    if (keccak256(_proposal.params) != keccak256(_parameters)) {
      revert ParametersMismatch(_method, _proposal.params, _parameters);
    }
  ...
  /// @inheritdoc IGovernorMiniBravo
  function quorumReached(uint256 _method) external view returns (bool __quorumReached) {
    Proposal storage _proposal = _getLatest(_method);
    __quorumReached = _quorumReached(_proposal.open, _proposal.forVotes);
  }
  ...
  function execute(uint256 _method, bytes memory _parameters) public {
    Proposal storage _proposal = _getLatest(_method);
    if (!_isExecutable(_proposal.open, _proposal.timelockExpiry)) {
      revert ProposalNotExecutable(_method, _proposal.id);
    }
    if (keccak256(_proposal.params) != keccak256(_parameters)) {
      revert ParametersMismatch(_method, _proposal.params, _parameters);
    }
  ...
  function _propose(uint256 _method, bytes memory _parameters) internal {
    ...
    Proposal memory _newProposal =
      Proposal({id: _latestId + 1, params: _parameters, forVotes: 0, open: true, timelockExpiry: 0});
   ...
   */
  function _acceptProposal(uint256 _method, bytes memory _params, address _voter) internal {
    Proposal storage _proposal = _getLatest(_method);
    if (keccak256(_proposal.params) != keccak256(_params)) {
      revert ParametersMismatch(_method, _proposal.params, _params);
    }
    if (_userVotes[_method][_proposal.id][_voter] > 0) {
      revert AlreadyVoted(_method, _proposal.id);
    }
...
abstract contract LockManagerGovernor is ILockManagerGovernor, GovernorMiniBravo {
   ...
   */
  function _transferVotes(address _sender, address _receiver, uint256 _votes) internal {
    for (uint256 _i; _i < uint256(Methods.LatestMethod); ++_i) {
      if (_proposals[_i].length > 0) {
        ...
        if (_proposal.open) {
          ...
          if (_senderVotes > 0) {
            if (_senderVotes < _votes) {
              revert LockManager_ArithmeticUnderflow();
            }
 ...
 */
// solhint-disable defi-wonderland/non-state-vars-leading-underscore
...
library GasCheckLib {
  ...
  uint256 internal constant _SLOT0_GAS_USAGE = 6200;
    ...
    returns (uint256 _amount0, uint256 _amount1)
  {
    ...
    if (_priceOracle.isManipulated(_pool)) {
      revert GasCheckLib_PoolManipulated();
    }
    ...
    view
  {
    ...
    if (_collectCostWeth > _totalCollectedWeth) {
      revert GasCheckLib_InsufficientFees();
    }
  }
}
...
contract LockManager is ERC20, ILockManager, LockManagerGovernor {
  ...
  function withdraw(address _receiver) external {
    ...
    if (!_withdrawalData.withdrawalsEnabled) {
      revert LockManager_WithdrawalsNotEnabled();
    }
    address _user = msg.sender;
    uint256 _userBalance = balanceOf[_user];
    if (_userBalance == 0) {
      revert LockManager_ZeroBalance();
    }
    ...
    userRewards[_user] =
      UserRewards({wethAvailable: 0, tokenAvailable: 0, wethPaid: _wethPerLockedWeth, tokenPaid: _tokenPerLockedWeth});
  ...
  function unwind(uint256 _positionsCount) external {
    if (!deprecated) {
      revert LockManager_DeprecationRequired();
    }
    uint256 _positionsLength = _positionsList.length;
    if (_positionsLength == 0) {
      revert LockManager_NoPositions();
    }
    if (_positionsLength < _positionsCount) {
      revert LockManager_NoPositions();
    }

    IPriceOracle _priceOracle = POOL_MANAGER_FACTORY.priceOracle();
    if (_priceOracle.isManipulated(POOL)) {
      revert LockManager_PoolManipulated();
    }
    ...
    while (_positionsLength > _lastIndexToCheck) {
      ...
      if (_currentLiquidity > 0) {
        ...
        (uint128 _amount0Collect, uint128 _amount1Collect) =
  ...
  function _getSymbol(IERC20 _token, uint256 _index) internal view returns (string memory _symbol) {
    _symbol = string(
      abi.encodePacked('LockedWETHv1-', _token.symbol(), '-', Strings.toString(FEE), '-', Strings.toString(_index))
    );
  }
  ...
  function transfer(address _to, uint256 _amount) public override (ERC20, IERC20) returns (bool _success) {
    if (msg.sender == _to) {
      revert LockManager_InvalidAddress();
    }
    ...
    if (!_success) {
      revert LockManager_TransferFailed();
    }
  }
  ...
  function claimRewards(address _to) external returns (uint256 _rewardWeth, uint256 _rewardToken) {
    if (_to == address(0)) {
      revert LockManager_ZeroAddress();
    }

    address _account = msg.sender;
    (_rewardWeth, _rewardToken) = _claimable(_account);

    if (_rewardWeth == 0 && _rewardToken == 0) {
      revert LockManager_NoRewardsToClaim();
    }
  ...
  function _addRewards(uint256 _wethAmount, uint256 _tokenAmount) internal {
    if (_wethAmount == 0 && _tokenAmount == 0) {
      revert LockManager_ZeroAmount();
    }
    uint256 _totalLocked = totalSupply;
    if (_totalLocked == 0) {
      revert LockManager_NoLockedWeth();
    }
  ...
  function mintPosition() external notDeprecated {
    ...
    _positionsLiquidity[_positionToMint.lowerTick][_positionToMint.upperTick] =
      _currentLiquidity + _positionToMint.liquidity;
  ...
  function burnPosition(IStrategy.Position calldata _position) external notDeprecated {
    ...
    (uint128 _totalCollect0, uint128 _totalCollect1) = POOL.collect(
      address(this), _positionToBurn.lowerTick, _positionToBurn.upperTick, type(uint128).max, type(uint128).max
    );
   ...
   * @return _positionToBurn The position that has to be burned
   */
  function _getPositionToBurn(IStrategy.Position calldata _position)
    internal
    view
    returns (IStrategy.LiquidityPosition memory _positionToBurn)
  {
    ...
    _positionToBurn = STRATEGY.getPositionToBurn(
      _position, _positionsLiquidity[_position.lowerTick][_position.upperTick], _lockManagerState
    );
  }
    ...
    notDeprecated
  {
    if (msg.sender != address(POOL)) {
      revert LockManager_OnlyPool();
    }

    uint256 _owedWeth = IS_WETH_TOKEN0 ? _amount0Owed : _amount1Owed;
    concentratedWeth += _owedWeth;

    if (concentratedWeth > totalSupply) {
      revert LockManager_OverLimitMint(totalSupply, concentratedWeth);
    }
  ...
  function collectFees(IStrategy.Position[] calldata _positions) external {
    ...
    if (address(_feeCollectorJob) == msg.sender) {
      if (_poolManagerFactory.priceOracle().isManipulated(_pool)) {
        revert LockManager_PoolManipulated();
      }
      uint256 _collectMultiplier = _feeCollectorJob.collectMultiplier();
      uint256 _slot0CostPerPosition = GasCheckLib._SLOT0_GAS_USAGE / _positionsCount;
      (uint160 _sqrtPriceX96,,,,,,) = _pool.slot0();
      ...
      while (_positionIndex < _positionsCount) {
        ...
        (_amount0, _amount1) =
          _pool.collect(address(this), _position.lowerTick, _position.upperTick, type(uint128).max, type(uint128).max);
  ...
  function _feesDistribution(uint256 _totalToken0, uint256 _totalToken1) internal {
    if (_totalToken0 == 0 && _totalToken1 == 0) {
      return;
    }
    IFeeManager _feeManager = POOL_MANAGER.feeManager();

    (uint256 _totalWeth, uint256 _totalToken) =
      IS_WETH_TOKEN0 ? (_totalToken0, _totalToken1) : (_totalToken1, _totalToken0);
   ...
   */
  function _burnPosition(int24 _lowerTick, int24 _upperTick, uint128 _currentLiquidity)
    internal
    returns (uint256 _amount0Burn, uint256 _amount1Burn)
  {
  ...
  /// @inheritdoc IGovernorMiniBravo
  function votingPower(address _user)
    public
    view
    override (GovernorMiniBravo, IGovernorMiniBravo)
