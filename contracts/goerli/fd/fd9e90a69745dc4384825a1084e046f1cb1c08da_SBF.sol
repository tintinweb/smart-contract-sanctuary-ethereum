...
contract SBF  is ERC721A, Ownable {
    ...
    function publicMint(uint32 mintAmount) public payable callerIsUser{
        require(totalSupply() + mintAmount <= MaxSupplyNumber,"sold out");
        require(mintAmount <= maxPerTx,"max 10 mintAmount");
        if(freeMinted[msg.sender])
        {
            require(msg.value >= mintAmount * cost,"insufficient");
        }
        else 
        {
            freeMinted[msg.sender] = true;
            require(msg.value >= (mintAmount-1) * cost,"insufficient");
        }
