...
import {Order, Status,Side,AssetType,Settle,Input,Fee,Bid} from "./lib/OrderStruct.sol";

contract AlvinMarketplaceV2 is 
    ReentrancyGuard,
    Ownable,
    Pausable
{
    ...
    function execute(Input memory input) external payable nonReentrant whenNotPaused {
        require(msg.sender == input.settle.user, 'Marketplace:invalid caller');
        require(input.settle.deadline > block.timestamp, 'Marketplace: settle deadline reached');
    ...
    //internal functions
    function _execute(Order memory order, Settle memory settle) internal returns(uint256){
        uint amount = 0;
            require(
        ...
        if(Side(order.side) == Side.BUY){
            ...
            require(
                ...
                order.tokenId == settle.tokenId &&
                order.amount == settle.amount,
        ...
        }else if(Side(order.side) == Side.AUCTION){
            require(order.status == Status.AUCTION && orderStatus[orderHash] == Status.NEW,'Marketplace:order already exist');
            require(order.endTime < block.timestamp || order.endTime >= block.timestamp, 'Marketplace: order deadline reached');
            require(!_isNative(IERC20(order.currencyAddress)), 'native token not supported');
            require(order.bid.highestBid > order.price , 'bid price > order price');
            require(order.bid.highestBidder != address(0), 'bidder is addres 0');

            amount = _takePayment(IERC20(order.currencyAddress),order.user, order.bid.highestBid);
            require(
                order.executionDelegate.executeSell(order.tokenAddress, settle.user,order.user,order.tokenId,order.amount,'AUCTION'),
                'delegation error'
            );
            _distributeFeeAndProfit(
                orderHash, //
                order.user, //funds transfer to
                IERC20(order.currencyAddress), //currencyaddress
                settle.price,
                order.fee
            );
