...
contract TomiStates {
    ...
    IERC20 public constant usdToken = IERC20(0x85fcb519c06cb48A74fd0baD6438ABF25D42cdb4);
    IERC20 public constant wethToken = IERC20(0x85fcb519c06cb48A74fd0baD6438ABF25D42cdb4);
    IERC20 public constant tomiToken = IERC20(0x85fcb519c06cb48A74fd0baD6438ABF25D42cdb4);
    ...
    uint256 public votingDelay = 10 minutes;
...
contract TomiTokenDao is TomiEvents, TomiStates, Ownable {
    ...
    function getProposalsChangeEmissionData(uint256 id) external view returns (ProposalChangeEmissionData memory) {
        return proposalsChangeEmissionData[id];
    }

    function getProposalsWalletData(uint256 id) external view returns (ProposalWalletData memory) {
        return proposalsWalletData[id];
    }
    ...
    function processTomiTokenPayments() internal {
        ...
        uint256[] memory fee = priceOfToken(tomiMainDao.baseFee().div(10**8), path);
    ...
    function castVote(uint256 proposalId, uint8 support, uint256[] calldata nftIds) external {
        ...
        uint8 status = uint8(state(proposalId));
        emit VoteCast(_msgSender(), proposalId, support, votes, status);
    }
