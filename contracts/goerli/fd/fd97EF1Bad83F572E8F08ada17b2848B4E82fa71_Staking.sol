...
contract Staking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, IStaking {
     ...
     */
    uint256 public constant totalBlocksPerYear = 2346585 + 7000; // for mainnet (extra 7k blocks for allowing users to lock in 52 week pool)
    ...
    function claimPlasmaMultiple(uint256[] calldata depositNumbers, address plasmaRecipient) external {
        uint256 totalAmount;
        _updatePlasmaPerBlockPerToken();
        for (uint256 index = 0; index < depositNumbers.length; index++) {
