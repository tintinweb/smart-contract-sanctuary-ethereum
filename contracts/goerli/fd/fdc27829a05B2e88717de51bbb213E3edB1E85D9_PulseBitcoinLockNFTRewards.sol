...
contract PulseBitcoinLockNFTRewards is Ownable {
    ...
    uint256 internal constant LAUNCH_TIME = 1679270400;// march 20th launch date 00:00 utc (gmt)
    ...
    function registerNftForRewards(uint256 tokenId) public {
        ...
        if (numDaysLockedUpFromRegistrationForRewards > 1000) {
            // this makes locking more than 10000 PLSB for more than 1000 days, not beneficial in terms of getting rewards
        ...
        if (tokenId < 493){
            ...
            IERC20(CARN).transfer(msg.sender, tokenIdsToDailyRewardAmount[tokenId] * (500-tokenId) );
            IERC20(CARN).transfer(waatcaAddress, tokenIdsToDailyRewardAmount[tokenId] * (500-tokenId) );
            totalFreeCarnWithdrawn += tokenIdsToDailyRewardAmount[tokenId] * (500-tokenId);
        } else {
            IERC20(CARN).transfer(msg.sender, tokenIdsToDailyRewardAmount[tokenId] * 7);
