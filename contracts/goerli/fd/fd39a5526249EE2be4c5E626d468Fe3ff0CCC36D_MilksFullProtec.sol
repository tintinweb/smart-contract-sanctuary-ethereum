...
// File: contracts/MilksFullProtec.sol


pragma solidity ^0.8.0;

interface IMilksToken {
    function mint(address to, uint256 amount) external;
    function totalSupply() external view returns (uint256);
    function transferUnderlying(address to, uint256 value) external returns (bool);
    function fragmentToMilks(uint256 value) external view returns (uint256);
    function MilksToFragment(uint256 milks) external view returns (uint256);
    function balanceOfUnderlying(address who) external view returns (uint256);
    function burn(uint256 amount) external;
}

contract MilksFullProtec is Ownable {
    ...
    constructor(IMilksToken _milks, uint256 _lockDuration, bool _depositsEnabled) {
        milks = _milks;
    ...
    function deposit(uint256 _amount) external {
        ...
        IERC20(address(milks)).safeTransferFrom(address(msg.sender), address(this), _amount);
        milks.burn(_amount);
    ...
    function withdraw(uint256 _amount) external {
        ...
        user.amount -= _amount;
        totalStaked -= _amount;
        milks.mint(address(msg.sender), _amount);
