  ...
  UUPSUpgradeable
{
   ...
   * @param fundTransferProxy_ IFundTransferProxy
   */
  function updateFundTransferProxy(IFundTransferProxy fundTransferProxy_)
    public
    onlyAdmin
  {
    require(
   ...
   * @param nftTransferProxy_ INftTransferProxy
   */
  function updateNftTransferProxy(INftTransferProxy nftTransferProxy_)
    public
    onlyAdmin
  {
    require(
  ...
  function _checkParameterForProcessOrder(
    ...
    require(
      order.collectionAddress != address(0),
    ...
    // In case of lazy mint only
    if (lazyMintData.nftId != 0) {
      require(
        lazyMintData.creator != address(0),
        "UzuExchange: invalid lazyMintData.creator"
      );
      require(
        lazyMintData.amount != 0,
        "UzuExchange: invalid lazyMintData.amount"
      );
      require(
        bytes(lazyMintData.uri).length > 0,
        "UzuExchange: invalid lazyMintData.uri"
      );
      require(
        lazyMintData.royaltyReceiver != address(0),
        "UzuExchange: invalid lazyMintData.royaltyReceiver"
      );
    }
  }
  ...
  function processOrder(
    ...
    _checkParameterForProcessOrder(order, lazyMintData);

    bool isLazyMint = (lazyMintData.nftId != 0);
    if (isLazyMint) {
      require(
        lazyMintSign.length == 65,
        "UzuExchange: invalid lazyMintData.signature"
      );
    }
    ...
    if (msg.value > 0) {
      fundTransferProxy.depositEthToVault{value: msg.value}(_msgSender());
    }
    ...
    if (Domain.isCryptoCurrencyType(sellerPayment.currencyType)) {
      if (sellerPayment.tokenAddress == address(0)) {
        // This is the case of ETH native payment
        fundTransferProxy.transferEthFromVault(
          order.seller,
   ...
   */
  function isOrderProcessed(uint256 orderId)
    public
    view
    virtual
    override
...
library Domain {
  ...
  bytes32 private constant MINT_DATA_TYPEHASH =
    keccak256(
      "MintData(address creator,uint256 nftId,uint256 amount,string uri,bytes data,address royaltyReceiver,uint96 royaltyFeeNumerator)"
    );
