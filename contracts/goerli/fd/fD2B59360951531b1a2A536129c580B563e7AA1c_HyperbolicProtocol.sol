...
contract HyperbolicProtocol is IHyperbolicProtocol, UniswapV3FeeERC20 {
  ...
  uint32 public override poolToMarketCapTarget = DENOMENATOR * 2;
  bool public taxesEnabled = true;
  bool public taxesOnTransfers;
  bool public taxesOnBuys = true;
  bool public taxesOnSells = true;
    ...
    returns (uint256 lendPoolETHBal, uint256 marketCapETH)
  {
    ...
    marketCapETH = _token0 == address(this)
      ...
      : (totalSupply() * 2 ** (96 * 2)) / _priceX96 / 2 ** 96;
  }

  // _initialPriceX96 = initialPrice * 2**96
  // initialPrice = token1Reserves / token0Reserves
  function lpCreatePool(uint256 _initialPriceX96) external onlyOwner {
    ...
    if (token0 != address(this)) {
      _initialPriceX96 = 2 ** (96 * 2) / _initialPriceX96;
    }
  ...
  function lpCreatePosition(
    uint64 _totalSupply // supply without decimals
  ) external payable onlyOwner {
    require(msg.value > 0, 'ADDLP: need ETH for LP');
    require(_totalSupply > 0, 'ADDLP: need token supply');
    ...
    address[] memory _pools = _createLiquidityPositions(
      balanceOf(address(this)),
      msg.value
    );
    for (uint256 _i = 0; _i < _pools.length; _i++) {
      amms[_pools[_i]] = true;
      lendingPool.toggleWhitelistCollateralPool(_pools[_i]);
    }
  }
  ...
  PeripheryImmutableState
{
  ...
  constructor(
    ...
    _lpPoolFees.push(3000); // 0.3%
    _lpPoolFees.push(10000); // 1%
  }
  ...
  function _createLiquidityPools(
    address _token0,
    address _token1,
    ...
    for (uint256 _i = 0; _i < _lpPoolFees.length; _i++) {
      ...
      increasePoolTwapObservations(_newPool, _initPriceObservations);
    }
  }

  function _createLiquidityPositions(
    uint256 _amount0,
    uint256 _amount1
  ) internal returns (address[] memory _pools) {
    require(!liquidityPosInitialized, 'CREATELP: already initialized');
    (address token0, address token1) = _getToken0AndToken1();
    // if the token placements are different than what was passed in,
    // update the amounts to reflect the adjustment
    uint256 _amountETHTotal = _amount1;
    if (token0 != address(this)) {
      uint256 _cachedAmount0 = _amount0;
      _amount0 = _amount1;
      _amount1 = _cachedAmount0;
      _amountETHTotal = _amount1;
    }
    ...
    uint256 _amount0PerPool = _amount0 / _lpPoolFees.length;
    uint256 _amount1PerPool = _amount1 / _lpPoolFees.length;
    uint256 _amountETHPerPool = _amountETHTotal / _lpPoolFees.length;

    _pools = new address[](_lpPoolFees.length);
    for (uint256 _i = 0; _i < _lpPoolFees.length; _i++) {
      ...
      INonfungiblePositionManager.MintParams
        memory params = INonfungiblePositionManager.MintParams({
          ...
          amount0Desired: _amount0PerPool,
          amount1Desired: _amount1PerPool,
          amount0Min: 0,
          amount1Min: 0,
      ...
      ) = lpPosManager.mint{ value: _amountETHPerPool }(params);
      ...
      if (token0 == address(this)) {
        if (amount0Actual < _amount0PerPool) {
          uint256 refund0 = _amount0PerPool - amount0Actual;
          TransferHelper.safeTransfer(token0, owner(), refund0);
        }
      } else if (amount1Actual < _amount1PerPool) {
        uint256 refund1 = _amount1PerPool - amount1Actual;
  ...
  // https://docs.uniswap.org/contracts/v3/reference/core/interfaces/pool/IUniswapV3PoolActions#increaseobservationcardinalitynext
  function increasePoolTwapObservations(
    address _pool,
    uint16 _observations
  ) public {
    IUniswapV3Pool(_pool).increaseObservationCardinalityNext(_observations);
  }
