    ...
    LimitOrdersStorage
{
    ...
    function cancelOrder(address _poolAddress, uint256 _tokenId)
        public
        allowedOrder(_tokenId, _poolAddress)
    {
        Order storage order = orders[_poolAddress][_tokenId];

        require(order.poolData.token0 != address(0), "CANO1");
    ...
    function checkClosability(address _poolAddress, uint256 _tokenId)
        internal
        view
        returns (bool)
    {
        (bool closable, , , , , ) = checkOrder(_poolAddress, _tokenId);
        return closable;
    }
    ...
    {
        uint256 balanceOfToken0Before = IERC20Upgradeable(
            _order.poolData.token0
        ).balanceOf(address(this));
        uint256 balanceOfToken1Before = IERC20Upgradeable(
            _order.poolData.token1
        ).balanceOf(address(this));
        ...
        uint256 balanceOfToken0After = IERC20Upgradeable(
            _order.poolData.token0
        ).balanceOf(address(this));
        uint256 balanceOfToken1After = IERC20Upgradeable(
            _order.poolData.token1
        ).balanceOf(address(this));

        if (balanceOfToken0After > balanceOfToken0Before) {
            IERC20Upgradeable(_order.poolData.token0).transfer(
                _order.owner,
                balanceOfToken0After - balanceOfToken0Before
            );
        }
        if (balanceOfToken1After > balanceOfToken1Before) {
            IERC20Upgradeable(_order.poolData.token1).transfer(
                _order.owner,
                balanceOfToken1After - balanceOfToken1Before
            );
        }
        _order.transferedToken0 = balanceOfToken0After > balanceOfToken0Before
            ? balanceOfToken0After - balanceOfToken0Before
            : 0;
        _order.transferedToken1 = balanceOfToken1After > balanceOfToken1Before
            ? balanceOfToken1After - balanceOfToken1Before
            : 0;
        return (
            balanceOfToken0After > balanceOfToken0Before
