...
contract PolimiVoting is AccessControl, Ownable {
    ...
    constructor (address _recipient1, address _recipient2, address _recipient3, address _recipient4) {
        ...
        for (uint i = 0; i < recipientCount; i++) {
            for (uint j = i+1; j < recipientCount; j++) {
                require(recipients[i] != recipients[j], "Sono richiesti 4 indirizzi diversi");
            }
        }  
    ...
    function changeVotingState() external onlyOwner {
        ...
        if (!isVotingOpen) {
            ...
            if (winner != address(0)) {
                ...
                emit VotingClosed("Votazione chiusa. L'indirizzo vincente puo ora ritirare i fondi");
            }            
        }
    }
    ...
    function voteFor(uint _recipientId, uint256 _tokenId) external {   
        require(_recipientId < recipientCount, "Questo indirizzo non esiste nel contratto");    
        require(recipients[_recipientId] != address(0), "Questo indirizzo non puo essere votato");
        require(isVotingOpen, "Le votazioni sono al momento chiuse");
        address tokenOwner = tokenIdOwner(_tokenId);
        require(tokenOwner == msg.sender, "Non sei detentore del NFT che ti da diritto al voto. Ricontrolla il tuo token ID");
    ...
    function claimFunds() public payable onlyRole(CLAIM_WINNER_ROLE) {
        require(address(this).balance > 0, "Non ci sono fondi da ritirare");
        (bool myTransfer,) = msg.sender.call{value: address(this).balance}("");
        require(myTransfer, "Trasferimento fallito");
    }
