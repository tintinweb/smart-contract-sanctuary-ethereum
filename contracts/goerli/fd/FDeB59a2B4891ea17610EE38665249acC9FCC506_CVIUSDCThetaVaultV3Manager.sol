...
contract ThetaVaultV3Manager is KeepersBased, IThetaVaultV3Manager
{
    ...
    function perform() internal override {
        ...
        uint256 currVolTokenPrice = uint256(currCVI) * 1e6 / 100; // Move from cvi value to UDSC price

        uint160 sqrtMinPriceX96 = uint160(sqrt(currVolTokenPrice - cviPriceWindow) * Q96 / 10 ** (VOL_TOKEN_DECIMALS / 2));
        uint160 sqrtMaxPriceX96 = uint160(sqrt(currVolTokenPrice + cviPriceWindow) * Q96 / 10 ** (VOL_TOKEN_DECIMALS / 2));
