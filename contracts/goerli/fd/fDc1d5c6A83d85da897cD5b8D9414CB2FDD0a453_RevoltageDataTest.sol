...
contract RevoltageDataTest is Ownable {
    ...
    function stakeTokens(uint256 amount) public returns (bool) {
        ...
        // require(
        //     tokenfarmBalance >= total,
        //     "staking contract balance is too low for this amount"
        // );
    ...
    function unstakeTokens(address account, uint256 index) public {
        require(msg.sender != address(0), "sender cannot be the zero address");

        Staker memory staker = stakes[account];

        require(
            msg.sender == staker.owner,
            "sendmsg.sender is different to owner"
        );

        StakeItem memory item = staker.items[index];
        ...
        item.end = end;
        revoltage.transfer(staker.owner, total);
        //Remove from balance
        balance[staker.owner] = balance[staker.owner] - item.amount;
    }

    function getStakes() public view returns (StakeItem[] memory) {
        return _getStakes(msg.sender);
    }

    function getStakesForAddress(address account)
        public
        view
        onlyOwner
        returns (StakeItem[] memory)
    {
        return _getStakes(account);
    }

    // function getStakesOverview(bool running)
    //     public
    //     view
    //     onlyOwner
    //     returns (StakeItem[] memory)
    // {
    //     uint256 itemsCount = stakes[msg.sender].items.length;
    ...
    function _getStakes(address account)
        internal
        view
        returns (StakeItem[] memory)
    {
