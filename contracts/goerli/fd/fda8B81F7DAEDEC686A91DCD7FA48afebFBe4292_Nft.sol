...
contract Nft is ERC721URIStorage{
    ...
    constructor() payable ERC721("BattleGround","BTGE") {
    ...
    function buyBox(BoxType boxType) external payable boxCost(boxType) returns (uint[3] memory acquiredTokens ) { 
        ...
        (uint[3] memory addonsGenerated,uint[3] memory numbersGenerated) = generateAddons(boxType);

        for(uint i = 0; i < 3;i++){

             if(uint8(boxType)==0){
                 uint index = numbersGenerated[i]%listofCommonChars[addonsGenerated[i]].length;
                _setTokenURI(tokenCounter + i, listofCommonChars[addonsGenerated[i]][index]);       
             }
             else if(uint8(boxType)==1){
                 uint index = numbersGenerated[i]%listofEpicChars[addonsGenerated[i]].length;
                _setTokenURI(tokenCounter + i, listofEpicChars[addonsGenerated[i]][index]);       
             }
             else if(uint8(boxType)==2){
                 uint index = numbersGenerated[i]%listofLegendaryChars[addonsGenerated[i]].length;
                _setTokenURI(tokenCounter + i, listofLegendaryChars[addonsGenerated[i]][index]);       
             }
                
        }

        emit NftPurchase(addonsGenerated[0], addonsGenerated[1], addonsGenerated[2], msg.sender);//remover
    ...
    function compareTwoStrings(string memory a, string memory b) public pure returns(bool equal){
