...
contract LostHamsters is ERC721, Ownable, PullPayment {
    ...
    string public baseTokenURI;
		...
		Counters.Counter private currentDoubleId;
    ...
    function canMint(uint256 _count) public view returns (bool) {
			if (_count > MAX_MINT || _count< 1) {
				return false;
			}

			return currentTokenId.current() < (MAX_MINT - _count);
    }

    function canDouble() public view returns (bool) {
			address _sender = _msgSender();
			if (currentDoubleId.current() > TOTAL_DOUBLE || _doubles[_sender] == 1) {
				return false;
			}
			return true;
    }
		...
		function totalSupply() public view returns (uint256) {
			return currentDoubleId.current();
		}
