...
contract LostKidsStaking is Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
...
*/
    ...
    function createStake(uint256[] calldata tokenIds) external nonReentrant {
        ...
        // function internal to private

    function _addNftToRegister(address account, uint256 tokenId) private whenNotPaused {
    ...
    */
    function claimStakes(uint256[] calldata tokenIds, bool unstake) external nonReentrant whenNotPaused {
        uint256 rewards = 0;
        for (uint i = 0; i < tokenIds.length; i++) {
    ...
    */
    function _claimNftFromRegister(uint256 tokenId, bool unstake) private returns (uint256 rewards) {
        ...
        require(stake.tokenId == tokenId, "Not Staked");
            ...
            rewards = uint256((block.timestamp - stake.stakeTime) / 1 minutes * ( rewardPerKid));
