...
contract MarketplaceDutch is Ownable, ReentrancyGuard
{
    ...
    struct _UserSaleInformation
    {
        ...
        bool _BrightListEligible;      // [7] -> _BrightListEligible      | If The User Is Eligible For The BrightList
        bool _MaxAmountVerified;       // [8] -> _MaxAmountVerified       | If The User Passed MaxAmount Is Valid From The Merkle Tree
        bool _Active;                  // [9] -> _Active                  | If The Sale Is Active
    }
    ...
    event Purchased(address Purchaser, uint Amount, uint MessageValue, uint CurrentPurchaseValue, uint NewAmountSold, bool BrightList, uint Priority);
     ...
     * @param DN               | Delegation Index (optional delegate.cash) (if opt-out, use 69420 for this value)
    ...
    function Purchase (
        ...
        if(DN != 69420) { Recipient = IDelegationRegistry(_DR).getDelegationsByDelegate(msg.sender)[DN].vault; }
        ...
        if(_IS._ActiveBrightList)
        {
            ...
            if(BrightListEligible)
            {
                ...
                require(VerifyAmount(Recipient, MaxAmount, _RootHash, ProofAmount), "DutchMarketplace: Invalid Max Amount Merkle Proof");
                require(NewUserPurchasedAmount <= MaxAmount, "DutchMarketplace: User Has Purchased All Allocation For Provided Merkle Priority");
            }
        }
        ...
        Orders[SaleIndex].push(Order(msg.sender, CurrentPurchaseValue, Amount, Priority, BrightListEligible, false));
        ...
        if(SalesInternal[SaleIndex]._Simple)
        {
        ...
        else if (_IS._MintPack)
        {
            uint _SaleStrip = Sales[SaleIndex]._SaleStrip;
            uint _Amount = _IS._CurrentIndex;
            for(uint x; x < Amount; x++)
            {
            ...
            SalesInternal[SaleIndex]._CurrentIndex += _Amount;
        }
        else { revert("DutchMarketplace: Incorrect Sale Configuration"); }
        emit Purchased(Recipient, Amount, msg.value, CurrentPurchaseValue, NewAmountSold, BrightListEligible, Priority);
    }
    ...
    function __StartSale(
        ...
        // require(!InitializedSale[_InternalSale._NFT], "DutchMarketplace: Sale Already Initiated With This NFT Address");
        InitializedSale[_InternalSale._NFT] = true;
    ...
    function ViewSaleStates(uint[] calldata SaleIndexes) public view returns (Sale[] memory, Info[] memory)
    {
        ...
        { 
            Ended = SalesInternal[SaleIndexes[x]]._ActivePublic || SalesInternal[SaleIndexes[x]]._ActiveBrightList;
