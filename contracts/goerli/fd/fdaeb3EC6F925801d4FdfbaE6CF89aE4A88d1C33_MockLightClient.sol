...
interface ILCAMB {
    ...
    function getFinalizedStateRoots(uint64 slot) external view returns (bytes32);
...
contract LCAMB is LCAMBStruct {
    ...
    function executeMessage(
        ...
        if (vars.storageRoot == bytes32(0)) {
            ...
            require(vars.finalizedExecutionStateRoots != bytes32(0), "LCAMB: executionStateRoot not found");
            {
    ...
    function getFinalizedStateRoots(uint64 slot) public view returns (bytes32) {
        return lightClient.finalizedExecutionStateRoots(slot);
    }
}
...
contract MockLightClient is IEthereumLightClient {
    ...
    mapping(uint64 => bytes32) public stateRootsMapping; // slot => header

    function submitHeader(uint64 slot, bytes32 root, bytes32 stateRoots) external {
        latestSlot = slot;
        stateRootsMapping[slot] = stateRoots;
    }


    function finalizedExecutionStateRoots(uint64 slot) view public returns (bytes32) {
        return stateRootsMapping[slot];
    }
}
...
contract MockLCAMB {
    ...
    function setup(uint64 slot, address _lcambAddress, address _mockLightClient) public {
        lcamb = LCAMB(_lcambAddress);
        lightClient = MockLightClient(_mockLightClient);
        lcamb.setup(address(lightClient), 2000000, address(0xb44ea27353D96890CB6adD8D5F7De6837bD3322a));
        lightClient.submitHeader(
            slot, 
    ...
    function viewData(uint64 slot) view public returns (bytes32) {
        return lcamb.getFinalizedStateRoots(slot);
    }
    ...
    function testExecutedMessage(
        ...
        return lcamb.executeMessage(slot, message, accountProof, storageProof);
    }
}
...
interface IEthereumLightClient {
    function finalizedExecutionStateRoots(uint64 slot) external view returns (bytes32);
}