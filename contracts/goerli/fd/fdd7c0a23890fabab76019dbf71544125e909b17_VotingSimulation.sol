...
contract VotingSimulation {
    ...
    function startVoting(uint256[] memory randomNumbers) public{
       ...
       for (uint256 i; i<10;i++){
        ...
        for (uint256 i; i< s_politicalPartiesArray.length-1; i++){
            if(s_politicalPartiesArray[i+1].numberOfVotes > s_politicalPartiesArray[i].numberOfVotes){
                newWinner = s_politicalPartiesArray[i+1]; 
            }
        }
        s_winner = newWinner;
    }
