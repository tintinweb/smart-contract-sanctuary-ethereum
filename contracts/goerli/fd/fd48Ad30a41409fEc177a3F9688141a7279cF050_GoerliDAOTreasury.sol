...
contract GoerliDAOTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SGDAO }
    ...
    address public sGDAO;
    uint public sGDAOQueue; // Delays change to sGDAO address
    ...
    constructor (
        ...
        isLiquidityToken[ _GDAODAI ] = true;
        liquidityTokens.push( _GDAODAI );
    ...
    /**
        @notice allow approved address to deposit an asset for GDAO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint GDAO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( GDAO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn GDAO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IGDAOERC20( GDAO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGDAO ).balanceOf( msg.sender ); // Can only borrow against sGDAO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GDAO
        @param _amount uint
     */
    function repayDebtWithGDAO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGDAOERC20( GDAO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, GDAO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( GDAO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( GDAO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns GDAO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match GDAO decimals
            value_ = _amount.mul( 10 ** IERC20( GDAO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGDAO ) { // 9
            sGDAOQueue = block.number.add( blocksNeededForQueue );
        } else return false;
