...
interface IERC4907 {
    ...
    event UpdateUser(uint256 countid, uint256 indexed tokenId, address indexed user, string indexed meta, uint64 expires);
    ...
    function setRentUser(uint256 tokenId, address user, uint256 rentprice, string calldata meta, uint64 expires) external ;
...
contract Metadata is ERC721A, Ownable, ReentrancyGuard, IERC4907 {
    using Strings for uint256;
    uint256 public RegisterCost = 20000000000000000;
    uint256 private ref = 20;
    uint256 public rentFeePercentage = 10;
    uint256 private ref_discount = 20;
    ...
    bytes _allowChars = "0123456789_abcdefghijklmnopqrstuvwxyz";
    uint public rentLastId;
    mapping(uint=>uint) rentTokenId;
    mapping(uint => mapping(address => string)) private rents;

    constructor() ERC721A("Metadata Service - Test", "@meta") {
        metadataIDtoName[_currentIndex]="meta";
        metadataNametoID["meta"]=_currentIndex;
...
struct UserInfo
    {
        uint256 countid; // unix timestamp, user expires
    ...
    /// @param user  The new user of the NFT
    /// @param expires  UNIX timestamp, The new user could use the NFT before expires
    function setRentUser(uint256 tokenId, address user, uint256 price, string calldata meta, uint64 expires) public virtual{
        ...
        info.countid = rentLastId;
        info.user = user;
        info.price = price;
        info.meta = meta;
        info.expires = expires;
        emit UpdateUser(rentLastId,tokenId,user,meta,expires);
    }
...
function rentOwner(address _owner)
    ...
    returns (string[] memory)
  {
    ...
    while (ownedTokenIndex < ownerTokenCount) {
      address currentTokenOwner = rentTokenID(currentTokenId);
...
/// ERC4907 Standard Finish
    ...
    function RentNFT(string calldata metadata, uint256 day) public payable {
            ...
            setRentUser(tokenid, msg.sender, msg.value, metadata, uint64(block.timestamp+(86400*day)));
    ...
    returns (string[] memory)
  {
    ...
    function setRegisterPrice(uint256 customPrice) external onlyOwner {
        RegisterCost = customPrice;
    }
  ...
  /**
    1) Register cost without reference is 0.01 ETH, with reference value is 0.008 ETH.
  ...
  */
    ...
    function Register(address ref_address, string memory metadata)
        ...
        payable
    {
        uint256 price = RegisterCost;
        bool is_ref=false;
        uint256 ref_cost=0;
        require(bytes(metadata).length>0,"Write a Metadata Name");
        require(CheckMetadata(metadata), "Invalid Metadata Name");
...
function LastMetadatas(uint256 count)
    ...
    returns (string[] memory)
  {
    ...
    require(currentId>=0,"Invalid");
    while (total > currentId) {
        lastAddr[ownedTokenIndex] = metadataIDtoName[total];
        ownedTokenIndex++;
      total--;
    }
...
function setUploadAllowlist(bytes32 merkleForAllowlist) external onlyOwner {
 ...
 function CheckMetadata(string memory metadata) public view returns(bool){
        uint allowedChars =0;
    ...
    function withdraw() public onlyOwner nonReentrant {
        uint256 balance = address(this).balance;
        Address.sendValue(payable(0x7Ab2f70A0590B0872Db9e0EC689A4d8F310b0116),balance);
        }
