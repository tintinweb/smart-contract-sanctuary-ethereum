...
contract ICO {
    ...
    event SeedFunderApproved(address user);
    event ContributionReceived(address from, uint amount);
    event TokensRedeemed(address from, uint amount);
    ...
    constructor(address _owner, address _treasury) {
        owner = _owner;
        spc = new SpaceCoin(_owner, _treasury, address(this));
    }
     ...
     * @param user A seed funder to approve.
     */
    function approveSeedFunder(address user) external onlyOwner {
        approvedSeedFunders[user] = true;
        emit SeedFunderApproved(user);
    }
    ...
    function redeem() external onlyUnpaused {
        ...
        spc.releaseFromICO(msg.sender, tokensToRedeem);
    }
...
contract SpaceCoin is ERC20 {
    ...
    event ICOTokensReleased(address by, address to, uint amount);
     ...
     */
    constructor(address _owner, address _treasury, address _icoContract) ERC20("SpaceCoin","SPC") {
        owner = _owner;
        treasury = _treasury;
    ...
    /**
    * @dev Utility function invoked by ICO contract to fulfill user redemptions. Transfers tokens.
    * @param to Recipient
    * @param amount The amount of SPC to release.
    */
    function releaseFromICO(address to, uint256 amount) public {
        if (msg.sender != icoContract) {
            revert("Forbidden action.");
        }

        emit ICOTokensReleased(msg.sender, to, amount);

        /// @dev This is a design decision to transfer tokens rather than increasing allowance. It's still ultimately invoked in a pull-based way.
        super._transfer(owner, to, amount);
    }

    /**
     * @dev Overrides ERC20 -> transfer(), implementing tax.
     * @param to Recipient
    ...
    /**
     * @dev Overrides ERC20 -> transferFrom(), implementing tax.
     * @param from Sender
     * @param to Recipient
     * @param amount The amount of SPC to transfer.
     * @return always true
