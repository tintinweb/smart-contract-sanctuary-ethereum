...
interface Token {
    ...
    function updateRewardOnMint(address from) external;
}

contract YieldingNft is ERC721A, Ownable {
    address token = 0x7D09831373B4f4B89e4E9F57044F14618802aDdC;
   ...
   function _startTokenId() internal view override virtual returns (uint256) {
    ...
    function mint() external payable {
        require(msg.value >= .01 ether, "not sending 1 ether");
        uint256 count = (msg.value / (.01 ether));
        require(totalSupply() + count <= 10);
        _safeMint(msg.sender, count);
		Token(token).updateRewardOnMint(msg.sender);
    }
