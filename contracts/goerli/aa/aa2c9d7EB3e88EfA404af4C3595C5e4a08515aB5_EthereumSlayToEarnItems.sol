...
import "@maticnetwork/pos-portal/contracts/root/RootToken/DummyMintableERC1155.sol";
import "@openzeppelin/contracts/utils/Address.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "./ISlayToEarnItems.sol";

contract EthereumSlayToEarnItems is ISlayToEarnItems, ERC1155, AccessControlMixin, NativeMetaTransaction, ContextMixin, IMintableERC1155 {
    bytes32 public constant INVENTORY_ADMIN_ROLE = keccak256("INVENTORY_ADMIN_ROLE");

    constructor(string memory tokenMetadataBaseUri) public ERC1155(tokenMetadataBaseUri) {
        _setupContractId("EthereumSlayToEarnItems");
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());

        _initializeEIP712(tokenMetadataBaseUri);

        _setupRole(INVENTORY_ADMIN_ROLE, _msgSender());
    ...
    function setTokenMetadataUri(string memory newUri) public only(DEFAULT_ADMIN_ROLE) {
        _setURI(newUri);
    }
    ...
    function mint(
        ...
        uint256 amount,
        bytes calldata data
    ) external override only(INVENTORY_ADMIN_ROLE) {
    ...
    function mintBatch(
        ...
        bytes calldata data
    ) external override(IMintableERC1155, ISlayToEarnItems) only(INVENTORY_ADMIN_ROLE) {
        require(ids.length == amounts.length, "The length of items and amounts must match.");
    ...
    function burnBatch(
        ...
        uint256[] memory items,
        uint256[] memory amounts
    ) public override only(INVENTORY_ADMIN_ROLE) {
