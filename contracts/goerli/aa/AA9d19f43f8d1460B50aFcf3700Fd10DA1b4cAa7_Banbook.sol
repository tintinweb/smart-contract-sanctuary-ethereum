...
contract Banbook is ERC721AQueryable, Ownable {
    ...
    mapping(uint256 => uint256) public banbookMaxLife;
    mapping(uint256 => uint256) public banbookLife;
    ...
    constructor(address ashesAddress, address bagicAddress)
        ERC721A("Banbook", "BBK")
    {
    ...
    function mint(uint256 quantity) external payable {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            uint256 _life = (uint256(
                ...
                // ) % 1048576) + 2; // ban and unban himself/herself
            ) % 1048576) + 16384; // 1.5625%
            banbookMaxLife[totalSupply() + i] = _life;
            banbookLife[totalSupply() + i] = _life;
        }
        _safeMint(msg.sender, quantity);
    }

    function banUnban(address target, uint256 tokenId) external {
        require(ownerOf(tokenId) == msg.sender, "not token owner");
        require(banCount[target] < 18, "Reached Max BanCount");

        // same cost to ban and unban
        // increse by power of 4 after each banUnban
        uint256 cost = 4**(banCount[target] / 2);
        // overflow error if not enough book life
        banbookLife[tokenId] -= cost;

        uint256 _banCount = banCount[target] + 1;
        banCount[target] = _banCount;

        // convert to ashes
        ASH.mint(msg.sender, cost);
    ...
    // https://stackoverflow.com/questions/72582656/invalid-contract-specified-in-override-list
    function transferFrom(
        address from,
        address to,
        uint256 tokenId
    ...
    function stakeAshes(uint256 quantity) external {
        claimAshes();
        ASH.transferFrom(msg.sender, address(this), quantity);
        stakedAshes[msg.sender] += quantity;
    }

    function unstakeAshes(uint256 quantity) external {
        claimAshes();
        ASH.transfer(msg.sender, quantity);
        stakedAshes[msg.sender] -= quantity;
    }

    // transform Ashes to Bagic
    function claimAshes() public {
        // 0.5**((block.timestamp - stakedAshesTimeStamps[msg.sender])/3 days)
        ...
        // for testing
        // uint claimAmount = stakedAshes[msg.sender]
    ...
    // recharge the banbook using Bagic
    function rechargeBanbook(uint256 tokenId, uint256 quantity) public {
        BG.burn(msg.sender, quantity);
        banbookLife[tokenId] += quantity;
        require(
            banbookMaxLife[tokenId] >= banbookLife[tokenId],
            "reached max life"
        );
    }

    function lowerBanCount(address target, uint256 tokenId) external {
        require(ownerOf(tokenId) == msg.sender, "not token owner");
