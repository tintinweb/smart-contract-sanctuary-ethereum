...
contract FGP is ERC721A, ERC2981, Ownable, ReentrancyGuard, OperatorFilterer {
  ...
  uint256 public maxSupply = 370;
  uint256 public WlSupply = 370;
  ...
  constructor (string memory name, string memory symbol, address payable royaltiesReceiver)
    ERC721A(name, symbol)
  {
  ...
  function mint(uint256 tokens) public payable nonReentrant {
    ...
    require(!preSale, "The public sale has not started yet");
    require(tokens <= MaxperWallet, "Max Limit per Wallet!");
    require(totalSupply() + tokens <= maxSupply, "Max supply exceeded!");
    require(PublicMinted[_msgSenderERC721A()] + tokens <= MaxperWallet, "Max Limit per Wallet!");
  ...
  function presalemint(uint256 tokens, bytes32[] calldata merkleProof) public payable nonReentrant {
    ...
    require(PresaleMinted[_msgSenderERC721A()] + tokens <= MaxperWalletWl, "Max Limit per Wallet!");
    ...
    require(msg.value >= wlcost * tokens, "Insufficient funds!");
       ...
       PresaleMinted[_msgSenderERC721A()] += tokens;
      _safeMint(_msgSenderERC721A(), tokens);
