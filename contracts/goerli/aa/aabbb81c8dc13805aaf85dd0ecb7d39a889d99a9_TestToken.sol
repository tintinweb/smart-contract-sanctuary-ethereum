...
contract TestToken is ERC721("TEST", "TeST"), Ownable {
    ...
    string internal base =
        "ipfs://bafybeihlqmw4pbsgas444orfx2hahpbr4tns5s55vp2nkjmd675wqlmdeu/1";
    ...
    constructor() {
        ...
        _setBlockedOperators(blockedOperatorsList, true);
    }

    function setBlockedOperators(address[] memory operators, bool status) external onlyOwner {
        _setBlockedOperators(operators, status);
    }

    function _setBlockedOperators(address[] memory operators, bool status)
        internal
    {
        for (uint256 i = 0; i < operators.length; i++) {
            if (status) {
                blockedOperators.add(operators[i]);
            } else {
                blockedOperators.remove(operators[i]);
            }
        }
    }
