...
contract AssetTrade is Initializable,Ownable {
    ...
    event SellAssets(uint256 index, address sellAddr, address nftAddr, uint256 nftTokenId, uint256 price, uint256 sta, uint256 sellId, address[] tokenAddr);
    event CancleSellAssets(uint256 index, uint256 sta);
    event UpdateSellAssets(uint256 index, uint256 price, uint256 sellId, address[] tokenAddr);
    event UpdateBuyAddr(uint256 index, uint256 bidPrice, address bidBuyAddr, uint256 expireTime, uint256 sellId);
    ...
    function updateTokenFeed(address _tokenAddr, address _tokenFeed) external onlyOwner{
        tokenFeed[_tokenAddr] = _tokenFeed;
    }
    ...
    {   
        ...
        _tradeMsg.sta = 1;
        emit SellAssets(_tradeNum, _tradeMsg.sellAddr, nftAddr, nftTokenId, price, _tradeMsg.sta, sellId, tokenAddrs);
    }
    ...
    function updateBuyAddr(
        ...
        _tradeMsg.bidBuyAddr = bidBuyAddr;
        _tradeMsg.bidPrice = bidPrice;
        _tradeMsg.expireTime = block.timestamp + expireTime;
        emit UpdateBuyAddr(index, bidPrice, bidBuyAddr, _tradeMsg.expireTime, sellId);
    }
        
    function updateSellAssets(
        uint256 index, 
        address[] calldata tokenAddrs, 
        uint256 price,
        uint256 sellId
        ...
        _tradeMsg.price = price;
        emit UpdateSellAssets(index, price, sellId, tokenAddrs);
    }
    ...
    {   
        ...
        if(HGBPAddr == paymentToken){
            ...
            _amount = amounts[1].mul(10000).mul(paymentAmount).div(10**18);
            IFeed feed =  IFeed(tokenFeed[stableToken]);
            (, int256 answer, , ,) = feed.latestRoundData();
            _amount = uint256(answer).mul(10 ** (uint256(22).sub(feed.decimals()))).mul(_amount).div(10**18);
        }else {
            IFeed feed =  IFeed(tokenFeed[paymentToken]);
            (, int256 answer, , ,) = feed.latestRoundData();
            _amount = uint256(answer).mul(10 ** (uint256(22).sub(feed.decimals()))).mul(paymentAmount).div(10**18);
        }
        ...
        uint256 price = _tradeMsg.price;
        if(_tradeMsg.expireTime > block.timestamp){
            require(_tradeMsg.bidBuyAddr ==  msg.sender, "not bid buyer"); 
            price = _tradeMsg.bidPrice;
        }
        require(price.mul(10000 + rangeRate) > _amount  && price.mul(10000 - rangeRate) < _amount, "price error"); 
