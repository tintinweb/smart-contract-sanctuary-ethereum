...
import "./interfaces/ICrystalOfPower.sol";

contract CrystalOfGenerativeAssets is ERC721, ICrystalOfPower {
    uint256 private quantity;
    string private project_uri;
    address private shard_crystal_address;
    address private project_address;
    uint256 private drop_amount;
    uint256 private limit = 1000;
    ...
    ) ERC721("Crystal of Generative Assets", "Crystal of Generative Assets") {
    ...
    modifier checkMint() {
        ...
        if (gold_list_addresses[msg.sender]) {
            require(msg.value >= 2 * 4 * 10**17, "Not enough funds send");
        } else if (white_list_addresses[msg.sender]) {
            require(msg.value >= 2 * 45 * 10**16, "Not enough funds send");
        } else {
            require(msg.value >= 2 * 55 * 10**16, "Not enough funds send");
        }
        require(
            IERC721(shard_crystal_address).balanceOf(msg.sender) >= 2,
            "Not enough MetaSkynet Shard Crystals in your wallet. Min: 2."
        );
        _;
    }
