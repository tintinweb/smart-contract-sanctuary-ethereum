...
contract GeneralERC721Factory is AccessControl {
    ...
    function deployERC721(string memory name, string memory symbol, string memory baseTokenURI) public returns (address) {
        GeneralERC721 newContract = new GeneralERC721();
        require(address(newContract) != address(0), "GeneralERC721Factory: Contract not deployed");
        newContract.initialize(name, symbol, baseTokenURI, msg.sender, _defaultMintingFee);
        
        uint256 totalDeployed_ = _totalDeployed;
        _deployedContracts[totalDeployed_] = address(newContract);

        emit ContractDeployed(msg.sender, address(newContract), totalDeployed_);

        _totalDeployed = totalDeployed_ + 1;
...
contract GeneralERC721 is 
    ...
    function redeem(address redeemer, NFTVoucher calldata voucher) public payable returns (uint256) {
        ...
        require(msg.value == _mintingFee + voucher.minPrice, "GeneralERC721: Incorrect funds sent");

        (bool sent, ) = payable(_feeTarget).call{value: _mintingFee}("");
