...
contract RecipeFactory is IRecipeFactory {
    ...
    function makeRecipe(bytes32[] memory _ingredients) external override {
        ...
        clean(_ingredients);
        recipes[hash] = address(new Recipe{salt : hash}(_ingredients, ens, hash));
    }
...
contract Recipe is IRecipe {
    ...
    constructor(bytes32[] memory _ingredients, address _ens, bytes32 hash) {
...
interface IRecipe {
    /// @notice Emitted when a recipe is completed
    event RecipeCompleted();

    /// @notice Emitted when a recipe is created
    /// @param hash The hash of the ingredients needed for the recipe
    event RecipeCreated(bytes32 hash);

    /// @notice Emitted when an ingredient is added
