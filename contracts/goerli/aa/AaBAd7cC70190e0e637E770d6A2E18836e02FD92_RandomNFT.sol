...
contract RandomNFT is Ownable {
    ...
    function makeLotery(
        ...
        require(!colAdrToId[collection][NftID], "Already participating it lottery");
        ...
        idToAdrToLotId[AllLotsAmount] = infoLot(msg.sender, adrToLotAmount[msg.sender]);
    ...
    function isInArray(address[] memory array, address adr) internal pure returns (bool) {
    ...
    function closeLottery(
        ...
        colAdrToId[adrToLotIdToInfo[adr][lotId].collection][adrToLotIdToInfo[adr][lotId].id] = false;
    }
    ...
    function myGiveaways(address user, uint256 startPoint) external view returns (mygives[] memory myInfo) {
        ...
        uint256[] memory endTime = new uint256[](adrToLotAmount[user]);
        for (uint16 i = 1; i <= adrToLotAmount[user]; i++) {
            ...
            endTime[i - 1] = adrToLotIdToInfo[user][i].endDate;
        }
        ...
        if (IdArray.length < startPoint + amount) {
            ...
            mygives[] memory myStruct = new mygives[](IdArray.length - newStartPoint);
            ...
            for (uint256 i = newStartPoint; i < IdArray.length; i++) {
                ...
                myStruct[count].endTime = endTime[i];
                count++;
            }
            return (myStruct);
        } else {
    ...
    function lotInfo(address adr, uint256 lotId) external view returns (lot memory) {
    ...
    function isInArrayAdr(address[] memory array, address element) internal pure returns (bool) {
