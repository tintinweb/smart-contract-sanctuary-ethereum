...
contract Params {
    ...
    uint256 public constant MinimalStakingCoin = 0.001 ether;
...
contract Validators is Params {
    ...
    function initialize(address[] calldata vals) external onlyNotInitialized {
        ...
        lastRewardTime = now;
        reflectionPercentSum[lastRewardTime] = 0;
        ...
        returns (bool)
    {
        ...
        stakeTime[staker][validator] = lastRewardTime;
    ...
    function withdrawStakingReward(address validator) public returns(bool)
    {
        ...
        require(stakeTime[msg.sender][validator] > lastRewardTime, "no reward yet");
        StakingInfo storage stakingInfo = staked[msg.sender][validator];
        uint validPercent = reflectionPercentSum[lastRewardTime] - reflectionPercentSum[stakeTime[msg.sender][validator]];
        require(validPercent > 0, "still no reward");
        stakeTime[msg.sender][validator] = lastRewardTime;
        uint reward = stakingInfo.coins * validPercent / 100  ;
        ...
        payable
    {
        ...
        contractOwnersPart += msg.value * 2 / 10;
        // 10% to burn
        address(0).transfer(msg.value/10);
        lastRewardTime = now;
        reflectionPercentSum[lastRewardTime] += calculateReflectionPercent(validatorInfo[val].coins, msg.value * 2 / 10);
