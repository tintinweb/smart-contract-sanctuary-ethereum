...
contract VoteWin is Pausable, Ownable {
    ...
    mapping(uint => Match[]) public matchs; // day => match
    ...
    function getMatchs(uint __day, uint _matchIndex) external view returns(uint teamA, uint teamB, uint totalVoteTeamA, uint totalVoteTeamB, uint startTime, uint result,  string memory description) {
        uint _day = __day;
        return (matchs[_day][_matchIndex].team[1], matchs[_day][_matchIndex].team[2], matchs[_day][_matchIndex].totalVote[1], matchs[_day][_matchIndex].totalVote[2], matchs[_day][_matchIndex].startTime, matchs[_day][_matchIndex].result, matchs[_day][_matchIndex].description);
    }
    function getMatchsLength(uint _day) external view returns(uint) {
        return matchs[_day].length;
    }
    function setMatch(uint _day, uint _matchIndex, uint startTime, uint teamIndexA, uint teamIndexB) external onlyOwner whenPaused {
        require(matchDetails[_day][_matchIndex].startTime == 0 ||
            block.timestamp > matchDetails[_day][_matchIndex].startTime, 'VoteWin::setMatch: match time closed');
    ...
    function setMatchResult(uint _day, uint _matchIndex, uint _result) external onlyOwner whenPaused {
        require(block.timestamp - matchDetails[_day][_matchIndex].startTime >= 7200, 'VoteWin::setMatch: match running');
