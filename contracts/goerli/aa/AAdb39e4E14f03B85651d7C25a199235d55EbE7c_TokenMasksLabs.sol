...
contract TokenMasksLabs is ERC721A, Ownable {
    ...
    constructor(
    ) ERC721A ('ETH MASKS LAB230310','EM') {  // コレクション名：　ETH MASKS、　トークンの名前（シンボル名）：EM
        setBaseURI('https://bafybeid7pzdaajwimvavlbf3wphipsk3mhr3a5jpahwmkkesaaqja3jq6a.ipfs.nftstorage.link/');
    }
    /**
    * コンストラクタで間違えた場合に変更できるようにしておく。
    */
    ...
    */
    function mint(uint _mintAmount) public payable {
        // セールが始まっているかどうか。pausedがtrueであれば止める
        if(paused) revert("the contract is paused");
        // totalSupplyは、ERC721Aで設定している現在発行されている枚数を取ってくる
        uint supply = totalSupply();
        // 0以下の場合は1枚以上発行するよう促す
        if(!(_mintAmount > 0)) revert("Please specify 1 or more");
        // 最大発行枚数より多い場合は減らすように促す
        ...
        // オーナー以外のミントの場合は、枚数分のガス代が足りないときのエラー
        if(msg.sender != owner()){
            if(msg.value < cost * _mintAmount) revert("insufficient funds");
        }
        // ミント本体
        _safeMint(msg.sender, _mintAmount); // msg.sender：実行者のアドレス、_mintAmount：ミント枚数
    }
    /**
    * 無料で誰かに配布したい、たくさんの人に一気に配るときに使う。
    */
    function airdropMint(address[] calldata _airdropAddresses, uint[] memory _userMintAmount) public onlyOwner {
        // totalSupplyは、ERC721Aで設定している現在発行されている枚数を取ってくる
        uint supply = totalSupply();

        uint _mintAmount = 0;   // 何枚発行するか
    ...
    /**
    * トークンURIを返す、NFTのメタ（画像やステータス）をチェックさせるJSONのURI
    */
    function tokenURI(uint _tokenId) public view virtual override returns (string memory) {
        return string(abi.encodePacked(ERC721A.tokenURI(_tokenId), baseExtension)); // abiはSolidityが用意してくれている関数。
    }
    /**
    * NFTの価格更新
    */
    ...
    */
    ...
    /**
    * コントラクトからいくら取り出すか。
    * ユーザーのmint時のガス代はコントラストに貯まる
    * bool os：正常に実行されたかどうかのフラグ
    */
    ...
    */
    ...
    /**
    * ERC721Aにある関数のオーバーライド
    * オーバーライドしておかないと新しく設定するbaseURIがセットされなくなる
    */
