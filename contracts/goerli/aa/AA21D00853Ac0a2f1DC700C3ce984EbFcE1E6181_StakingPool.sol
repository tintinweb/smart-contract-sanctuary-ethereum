...
interface IStakingPool {
    ...
    error TokensLockedUntil(uint256 unlockTimestamp);

    function factory() external view returns (IStakingFactory);

    function lockedUntil(address user) external view returns (uint256);
}

// File erc20-extensions/contracts-upgradeable/lib/[email protected]
...
// File erc20-extensions/contracts-upgradeable/staking/[email protected]
...
contract StakingPool is IStakingPool, StakingUpgradeable {
    IStakingFactory public factory;
    // user => unlock timestamp
    mapping(address => uint256) public lockedUntil;

    function initialize(address token_) public initializer {
        factory = IStakingFactory(msg.sender);
        string memory name_ = string.concat("Volted ", IERC20MetadataUpgradeable(token_).name());
        string memory symbol_ = string.concat("v", IERC20MetadataUpgradeable(token_).symbol());
        __Staking_init(name_, symbol_, token_);
    }

    /// @dev locks tokens for a specific amount of time after deposit
    /// @dev depositing more tokens will reinitialize the lock period
    function deposit(uint256 amount) public override returns (uint256) {
        uint256 unlockTimestamp = block.timestamp + factory.lockPeriod();
        lockedUntil[_msgSender()] = unlockTimestamp;
        emit TokensLocked(_msgSender(), unlockTimestamp);
        return super.deposit(amount);
    }

    /// @dev should only allow withdraw if tokens are unlocked
    function withdraw(uint256 amount, bool claim) public override returns (uint256) {
        uint256 unlockTimestamp = lockedUntil[_msgSender()];
        if (block.timestamp < unlockTimestamp) revert TokensLockedUntil(unlockTimestamp);
        return super.withdraw(amount, claim);
    }

    /// @dev should only allow transfers of overlying staking token if lock period is over
    function _isTransferable() internal view override returns (bool) {
        return block.timestamp >= lockedUntil[_msgSender()];
    }
}