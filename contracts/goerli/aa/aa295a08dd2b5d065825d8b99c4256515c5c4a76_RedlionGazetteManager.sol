...
function initialize(
  ...
  function claim(uint256 _issue) external {
  ...
  function claimArtdrop(uint[] calldata _tokenIds) external {
    IRedlionArtdrops ADC = IRedlionArtdrops(AD);
    for (uint i = 0; i < _tokenIds.length; i++) {
      uint tId = _tokenIds[i];
      ADC.mint(msg.sender, tId);
      emit ArtdropClaimed(getRLGA().tokenToIssue(tId), tId, msg.sender);
    }
  }
  ...
  function canDeliver(
    ...
    address _target
        ...
        ISubscriptionsManager SUBSC = ISubscriptionsManager(REDA);
    IRedlionGazette RLG = IRedlionGazette(RLGA);
    ISubscriptionsManager.SubInfo memory subInfo = SUBSC.subscriptionInfo(
      _target
    );
    ...
    if (DELIVERIES[_issue][subInfo.subId]) return false;
  ...
  function claimableIssues(
    ...
    IRedlionGazette.Issue memory currentIssue = RLG.timeToIssue(block.timestamp);
...
interface IRedlionGazetteManager {
  ...
  function getRLGA() external view returns (IRedlionGazette);

  function getRLLGA() external view returns (IRedlionLegendaryGazette);
}
