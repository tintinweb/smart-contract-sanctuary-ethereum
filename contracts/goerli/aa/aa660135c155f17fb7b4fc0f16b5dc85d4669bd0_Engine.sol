...
library LeverageMath {
    ...
    function getLeverage(
        ...
        leverage =
            ((amount * currentPrice) *
                10**(decimals.leverage + decimals.collateral)) /
            (collateral_plus_realizedPnL *
                10**(decimals.amount + decimals.oracle));
    }
}
...
contract Engine is ERC721Upgradeable, IEngineEvents {
    ...
    function join(
        ...
        if (
            order.collateral <= traderFees ||
            (premium > 0 && order.collateral < (uint256(premium) + traderFees))
        ) {
        ...
        returns (bool)
    {
        Match storage m = matches[matchId];
        PriceData memory priceData = oracle.getPrice();
        // if the match is deleted, it is not liquidatable
    ...
    function liquidate(
        ...
        for (uint256 i = 0; i < matchIds.length; i++) {
            ...
            bool _isLiquidatable = isLiquidatable(matchId, tokenId);
            if (!_isLiquidatable) continue;
            Match storage m = matches[matchId];
            PriceData memory priceData = oracle.getPrice();
            int256 pnl_ = m.pnl(
                tokenId,
            ...
            if (tokenId == m.maker) {
                ...
                int256 remainingCollateral = int256(m.collateralT) + pnl_;
                ...
                vault.assign(
                    uint256(int256(m.collateralM) - pnl_),
                    _ownerOf(m.maker)
                );
                totalOpenedAmount -= m.amount;
                emit MatchLiquidated(matchId, -pnl_, oracle.getPrice().price);
                delete matches[matchId].maker;
            }
        }
    }
