...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
    ...
    // Multiplier for saving redemptionTokenPerVaultToken so that the contract can keep track of
    // redemptionTokenPerVaultToken in case redemptionToken price is less than vaultToken price
    uint256 constant REDEMPTION_TOKEN_PER_VAULT_TOKEN_PRECISION = 1e18;
    ...
    function withdraw(address account, uint256 redemptionEpochId) whenNotPaused external {
        ...
        reentrancyGuard = 2;
        bool success = redemptionEpoch.redemptionToken.transfer(account, amount * redemptionEpoch.redemptionTokenPerVaultToken / REDEMPTION_TOKEN_PER_VAULT_TOKEN_PRECISION);
        if (!success) {
            revert TransferFailed();
        }
        reentrancyGuard = 1;
    ...
    */
    function rollRedemptionEpoch() external onlyOwner {
        uint256 numerator = REDEMPTION_TOKEN_PER_VAULT_TOKEN_PRECISION * vaultPrice * (ONE_HUNDRED_PERCENT_BPS - redemptionFeeBPS);
        uint256 denominator = tokenPrices[nextRedemptionToken] * ONE_HUNDRED_PERCENT_BPS;
        uint256 redemptionTokenPerVaultToken = numerator / denominator;
        redemptionEpochs.push(RedemptionEpoch({
            state: RedemptionEpochState.Pending,
            redemptionToken: nextRedemptionToken,
            vaultTokenTotalAmount: nextRedemptionEpochVaultTokenTotalAmount,
            redemptionTokenPerVaultToken: redemptionTokenPerVaultToken
        }));
    ...
    */
    function settleRedemptionEpoch(uint256 redemptionEpochId) external onlyOwner {
        RedemptionEpoch storage redemptionEpoch = redemptionEpochs[redemptionEpochId];
        require(redemptionEpochs[redemptionEpochId].state == RedemptionEpochState.Pending);

        bool success = redemptionEpoch.redemptionToken.transferFrom(owner(), address(this), totalRedemptionTokenAmount(redemptionEpochId));
        if (!success) {
            revert TransferFailed();
        }
    ...
    function accruePerformanceFee(uint256 performanceFeePeriodCount) external onlyOwner {
        ...
        performanceFeeAccruedVaultTokenAmount += _accurablePerformanceFee() / netVaultPrice();
        if (_performanceFeePnL() < 0) {
            // Only non-negative int256s could be converted to uint256 and there won't be any exceptions below.
    ...
    /* ------------------------------------------------------------------- */
    /*              External Owner Functions for Fee Settings              */
    /* ------------------------------------------------------------------- */
    ...
    function totalRedemptionTokenAmount(uint256 redemptionEpochId) public view returns (uint256) {
        return redemptionEpochs[redemptionEpochId].vaultTokenTotalAmount * redemptionEpochs[redemptionEpochId].redemptionTokenPerVaultToken / REDEMPTION_TOKEN_PER_VAULT_TOKEN_PRECISION;
    }
    ...
    function netVaultPrice() public view returns (uint256) {
        if (0 < _performanceFeePnL()) {
            ...
            return vaultPrice - (_accurablePerformanceFee() / totalSupply());
        } else {
            return vaultPrice;
        }
    }
    ...
    function _accurablePerformanceFee() internal view returns (uint256) {
        if (0 < _performanceFeePnL()) {
...
interface IManagedVault {
    ...
    // Only used in RedemptionEpoch structs. When a redemption epoch is created its status is 0 (Pending). When the
    // owner liquidates the required amount of redemption tokens, they call the settleRedemptionEpoch function. If the
    // callsucceedes, the state is set as 1 (Settled) and the investors are free to withdraw their redemptions.
    ...
    // Redemptions are settled in epochs. Investors can redeem their funds anytime. When the owner calls the
    // rollRedemptionEpoch one of the structs below is pushed to the redemptionEpochs array. Has the following
    // properties:
    // * state is initially 0 (Pending). When the ownersettles the epoch, the state is set to 1 (Settled).
    // * redemptionToken is the token this redemption epoch is paid in.
    // * vaultTokenTotalAmount is the total amount of tokens registered for redemption in this epoch.
    // * redemptionTokenPerVaultToken is the unit for how much redemption tokens are withdrawable per a vault token
    //   amount, multiplied by REDEMPTION_TOKEN_PER_VAULT_TOKEN_PRECISION. The precision is for being able to store
    //   fractional amounts with integers and the value must be divided to the precision in corresponding calculations.
    struct RedemptionEpoch {
        RedemptionEpochState state;
        IERC20 redemptionToken;
        uint256 vaultTokenTotalAmount;
        uint256 redemptionTokenPerVaultToken;
    }
    ...
    event SetRedemptionFeeBPS(uint256 indexed bps);
