...
contract Test is ERC721A, Pausable, Ownable {
  enum SaleStates {
    CLOSED,
    PUBLIC,
    WHITELIST
  ...
  uint256 public maxWhitelistTokens = 11;
  uint256 public publicSalePrice = 0.011 ether;

  uint64 public maxPublicTokensPerWallet = 1;
  ...
  {
    require(
      _totalMinted() + numberOfTokens <= maxWhitelistTokens,
      "Minted the maximum no of public tokens"
    );

    uint64 userAuxiliary = _getAux(msg.sender);
    require(
      userAuxiliary + numberOfTokens <= maxWLTokensPerWallet,
      "Maximum minting limit exceeded"
    );

    _setAux(msg.sender, userAuxiliary + numberOfTokens);
  ...
  {
    require(
      (_numberMinted(msg.sender) - _getAux(msg.sender)) + numberOfTokens <=
  ...
  function mintTo(address to, uint256 numberOfTokens)
    external
    canMint(numberOfTokens)
    onlyOwner
  {
  ...
  function setMaxWhitelistTokens(uint256 _maxWhitelistTokens)
    external
    onlyOwner
  {
