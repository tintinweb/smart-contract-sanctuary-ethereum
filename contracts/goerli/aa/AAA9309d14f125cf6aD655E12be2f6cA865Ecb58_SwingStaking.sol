...
contract SwingStaking is ERC721TokenReceiver {
    ...
    constructor() {
        ...
        if (block.chainid == 1) {
            _token = IERC20(0x55ACbBA71AbBf3653A4F12dD7EA71F0cB1ac3E5c);
            _nftReward = IERC721(0x29B59Ddd190F30d8bEffF8F55ed13562Eb48E6AA);
        ...
        stakes[StakeType.SIMPLE1].info.timeBetweenRewards = 1 minutes; //24 hours;
        stakes[StakeType.SIMPLE1].info.rewardPercentage = (12 * 10 ** accuracy) / uint256(365);
        stakes[StakeType.SIMPLE1].info.minimumStakeTime = 30 minutes; //30 days;
        ...
        stakes[StakeType.SIMPLE2].info.timeBetweenRewards = 1 minutes; //24 hours;
        stakes[StakeType.SIMPLE2].info.rewardPercentage = (14 * 10 ** accuracy) / uint256(365);
        stakes[StakeType.SIMPLE2].info.minimumStakeTime = 90 minutes; // 90 days;
        ...
        stakes[StakeType.STAKE2MINT].info.timeBetweenRewards = 1 minutes; //24 hours;
        stakes[StakeType.STAKE2MINT].info.rewardPercentage = (18 * 10 ** accuracy) / uint256(365);
        stakes[StakeType.STAKE2MINT].info.minimumStakeTime = 180 minutes; //180 days;
    ...
    function claimWalletRewards(address wallet) private {
        ...
        for (uint256 i = 0; i < ownerStakings[wallet].length; i ++) {
            Map storage m = ownerStakings[wallet][i];
            if (stakes[m.stakeType].info.enabled && hasPenalty(stakes[m.stakeType].stakings[m.index]) == false) {  
                (uint256 owed, uint256 time) = rewardsOwed(m.stakeType, stakes[m.stakeType].stakings[m.index]);
                if (owed > 0) {
                    totalOwed += owed;
                    stakes[m.stakeType].stakings[m.index].lastClaimTime = stakes[m.stakeType].stakings[m.index].lastClaimTime + time;
        ...
        if (totalOwed > 0) {
            _token.transfer(wallet, totalOwed);
        }
    }
