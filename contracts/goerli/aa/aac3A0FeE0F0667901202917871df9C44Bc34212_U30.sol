...
contract U30 is AbstractERC1155Factory {
    ...
    // Max number of transactions per wallet per token id
    //uint8[] maxTxPublic;
   ...
   /**
    * Edit the pause status
    *
    * @param tokenId the token collection to edit
    * @param _isPaused paused or not?
    */
    function setPaused(uint256 tokenId, bool _isPaused) external onlyOwner {
        isPaused[tokenId] = _isPaused;
    }
    ...
    */
    function setMaxPerTx(uint256 tokenId, uint8 _maxPerTx) external onlyOwner {
        maxPerTx[tokenId] = _maxPerTx;
    }

    /**
    * Edit all params for a token collection
    *
    * @param tokenId the token collection to edit
    * @param _maxSupply the max number of tokens that can be minted
    * @param _mintPrice the new price in wei
    * @param _maxPerTx the number of tokens that can be minted per transaction
    */
    function setParams(uint256 tokenId, uint256 _maxSupply, uint256 _mintPrice, uint8 _maxPerTx, bool _isPaused) external onlyOwner {
        maxSupply[tokenId] = _maxSupply;
        mintPrice[tokenId] = _mintPrice;
        maxPerTx[tokenId] = _maxPerTx;
        isPaused[tokenId] = _isPaused;
    ...
    *
    * @param tokenId the token collection
