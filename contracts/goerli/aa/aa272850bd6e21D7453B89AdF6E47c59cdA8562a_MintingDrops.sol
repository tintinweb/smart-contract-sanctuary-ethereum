...
contract MintingDrops is Ownable {
    ...
    bool immutable public isPrivate;
    /// @notice Collection contract.
    IERC11554KDrops immutable public collection;
    /// @notice Controller contract.
    IERC11554KController immutable public controller;
    /// @notice Items frequencies to determine item id in case of unequal items.
    uint256[] public itemsFreqs;
    /// @notice Maximum items to mint per mint for collection.
    uint256 immutable public maxItemsPerMint;
    /// @notice Maximum items to mint per ID for collection.
    uint256 immutable public maxItemsPerID;
    /// @notice Maximum items to mint in the drop.
    uint256 immutable public maxItems;
    /// @notice Maximum items to mint per mint for collection.
    uint256 immutable public guardianFeeAmountPerItem;
    /// @notice Maximum items to mint per ID for collection.
    uint256 immutable public guardianClassIndex;
    /// @notice Maximum items to mint in the drop.
    uint256 immutable public serviceFeePerItem;
    /// @notice Items variations in case of non-NFT drop.
    uint256 immutable public variations;
    /// @notice Drop type.
    DropType immutable public dropType;
    /// @notice Guardian that vaults items during drop.
    address immutable public managingGuardian;
    ...
    /// @notice Minted drop
    event MintedDrop(uint256 id, uint256 randomUriID, uint256 amount, address minter);
    ...
    constructor(
        ...
        controller_.paymentToken().approve(address(controller_), type(uint256).max);
        controller_.paymentToken().approve(IGuardians(controller_.guardians()).feesManager(), type(uint256).max);
    }
     ...
     */
    function withdrawEther(uint256 amount, address payable receiver) external payable onlyOwner {
     ...
     * @param receiver address to send ETH.
     */
    function withdrawPaymentToken(uint256 amount, address receiver) external payable onlyOwner {
        IERC20(address(controller.paymentToken())).safeTransfer(receiver, amount);
    }
    ...
    function mint(
        ...
        if (isPrivate && !MerkleProof.verifyCalldata(allowlistProof, allowlistMerkleRoot, keccak256(abi.encodePacked(msg.sender)))) {
            revert AccessDenied();
        }
