...
// File: IOLOLottery.sol


pragma solidity ^0.8.9;

interface IOLOLottery {
    /**
...
contract RandomNumberGenerator is VRFConsumerBaseV2, IRandomNumberGenerator, Ownable {
    ...
    struct RequestStatus {
     ...
     /* requestId --> requestStaus */
    mapping(uint256 => RequestStatus) public allRequestStatus;
    // All Past requests Ids.
    uint256[] public allRequestIds;
     // bytes32 public latestRequestId;
    uint256 public latestRequestId;

    address public oloLottery;
     ...
     *  @param _subscriptionId: Integer Value for your ChainLINK VRF Subcriptionn Id
    ...
    /**
     * @notice Set the address for the OLOLottery
     * @param _oloLottery: address of the OLO lottery
     */
    function setLotteryAddress(address _oloLottery) external onlyOwner {
        oloLottery = _oloLottery;
    }
     ...
     */
      ...
      function getRandomWords() external override returns (uint256) {
        // require(msg.sender == oloLottery, "Only OLOLottery");
        ...
        allRequestStatus[latestRequestId] = RequestStatus({
            randomWords: new uint256[](0),
            exists: true,
            fulfilled: false
        });
        allRequestIds.push(latestRequestId);
     ...
     */
       function fulfillRandomWords(
        ...
        // require(latestRequestId == _requestId, "Wrong requestId");
         require(allRequestStatus[_requestId].exists, "Wrong requestId");
         allRequestStatus[_requestId].fulfilled = true;
         allRequestStatus[_requestId].randomWords = _randomWords;
         uint256  mostRandomRandom;
        for (uint256 i = 0; i < _randomWords.length; i++) {
                   
            // randomResult = uint32(1000000 + (_randomWords[i] % 1000000));
            mostRandomRandom = _randomWords[i];
        }
        // to get a random number in a range from 1000000 to 1999999.
        // randomResult = uint32(1000000 + (_randomWords[0] % 1000000));

        //Below retuned 1505195 but we wanted to retrun 6 digit 505195
        //  randomResult = uint32(1000000 + (mostRandomRandom % 1000000));
        //Currently hardcoing and returing it
        randomResult = 505195;
        latestLotteryId = IOLOLottery(oloLottery).viewCurrentLotteryId();
    ...
    {
        require(allRequestStatus[_requestId].exists, "request not found");
        RequestStatus memory request = allRequestStatus[_requestId];
