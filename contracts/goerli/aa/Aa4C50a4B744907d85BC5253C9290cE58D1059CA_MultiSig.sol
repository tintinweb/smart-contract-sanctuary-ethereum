...
import "contracts/VariablesSetter.sol";
...
*/
contract MultiSig is VariablesSetter {
    ...
    constructor(
        ...
        uint256 _delay, 
        uint256 _lifetime
    ) VariablesSetter(_delay, _lifetime) 
    {
        require(_signersNeeded > 1, "MS: Requires > 1 signer");
        require(_signers.length >= _signersNeeded, "MS: signersNeeded <= total");
        require(_signers.length >= _fullConsensusNumber, "MS: ConsensusNumber <= total");
        for (uint i=0; i<_signers.length; i++) {
            require(activeSigners[_signers[i]] == false, "MS: Duplicate in signers");
    ...
    */
        ...
        isActive(_id)
    {
        ...
        require(block.number < _tx.blockNumber + requestLifetime, "tx timed out");
    ...
    */
        ...
        isActive(_id)
    {
        ...
        // if tx fails, should it automatically be rejected?
    ...
    */
    function _checkActiveTxs() private view {
        for (uint i=0; i<activeTransactionIds.length; i++) {
            uint256 _index = activeTransactionIds[i];
     ...
     * @dev Overrides createUpdateSigners in VariablesSetter.sol
    ...
    */
        ...
        returns (uint256 _id) 
    {
        ...
        _id = super.createUpdateSigners(_newSigners);
    }

    /**
     * @notice Create a request to update the signersNeeded variable
     * @dev Overrides createSignersNeeded in VariablesSetter.sol
    ...
    */
        ...
        returns (uint256 _id) 
    {
        _checkActiveTxs();
        if (activeTransactionIds.length > 0) delete activeTransactionIds;
        _id = super.createSignersNeeded(_newCount);
    }

    /**
     * @notice To be called by a signer that wants to update the fullConsensusNumber variable
     * @dev Overrides createFullConsensusNumber in VariablesSetter.sol
    ...
    */
        ...
        returns (uint256 _id) 
    {
        _checkActiveTxs();
        if (activeTransactionIds.length > 0) delete activeTransactionIds;
        _id = super.createFullConsensusNumber(_newCount);
    }
...
contract VariablesSetter {

    uint256 internal immutable blockOffset;
    uint256 internal immutable requestLifetime;
    ...
    Request internal activeRequest; // right now, neither of these are checked in tests
    ...
    constructor(uint256 _delay, uint256 _lifetime) {
    ...
    */
        ...
        returns (uint256 _id) 
    {
        ...
        // Try iterating through arrays and pushing values instead of assigning full array and compare gas
    ...
    */
    ...
    {
        Request storage _request = variableUpdateRequests[_id];
        require(_request.requestType == RequestType.UpdateSigners, "not update signers request");
        require(block.number > _request.blockNumber + blockOffset, "request delayed");
        require(block.number < _request.blockNumber + requestLifetime, "request timed out");
        _request.approvedSignatures.push(msg.sender);
        _request.approvedCount++;
        hasApprovedRequest[_id][msg.sender] = true;
        if (_request.approvedCount >= fullConsensusNumber) {
            ...
            for (uint i=0; i<_request.newSigners.length; i++) {
            ...
            fullConsensusNumber = _length < fullConsensusNumber ? _length : fullConsensusNumber;
        }
    }
    ...
    */
    ...
    {
        Request storage _request = variableUpdateRequests[_id];
        require(_request.requestType == RequestType.SignersNeeded, "not signers needed request");
        require(block.number > _request.blockNumber + blockOffset, "request delayed");
        require(block.number < _request.blockNumber + requestLifetime, "request timed out");
        _request.approvedSignatures.push(msg.sender);
        _request.approvedCount++;
    ...
    */
    ...
    {
        Request storage _request = variableUpdateRequests[_id];
        require(_request.requestType == RequestType.FullConsensusNumber, "not full consensus request");
        require(block.number > _request.blockNumber + blockOffset, "request delayed");
        require(block.number < _request.blockNumber + requestLifetime, "request timed out");
        _request.approvedSignatures.push(msg.sender);
        _request.approvedCount++;
    ...
    */
    modifier notApproved(uint256 _id) {
        require(!hasApprovedRequest[_id][msg.sender], "already approved by caller");
        _;
    }
