    ...
    IgNFT
{
    ...
    function _checkLiquidity(address[] memory markets, TokenType tokenType) internal {
        ...
        for (uint256 i; i < len; ) {
            ...
            totalValue += _quoteLiquiditySingle(
                market,
                oracle,
                ICToken(market).balanceOfUnderlying(msg.sender),
                false
            );
    ...
    function _quoteLiquiditySingle(
        ...
        uint256 decimals = reverse
            ? ICToken(market).decimals()
            : IERC20MetadataUpgradeable(ICToken(market).underlying()).decimals();
        uint256 factor;
        unchecked {
            factor = DEFAULT_DECIMALS - decimals;
        }
        if (decimals > DEFAULT_DECIMALS) {
            factor = decimals - DEFAULT_DECIMALS;
        }
        factor = 10**factor;

        if (reverse) {
            return (amount * EXP_ORACLE) / price / factor;
        }
        return (amount * factor * price) / EXP_ORACLE;
    }
