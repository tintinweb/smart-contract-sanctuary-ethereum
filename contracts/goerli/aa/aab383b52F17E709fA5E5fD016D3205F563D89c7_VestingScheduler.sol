...
contract VestingScheduler is IVestingScheduler, SuperAppBase {
    ...
    mapping(bytes32 => Schedule) public vestingSchedules; // id = keccak(sender, receiver, superToken)
    ...
    function createVestingSchedule(
        ...
        uint32 startDate,
        uint32 startDateValidFor,
        uint32 cliffDate,
        int96 flowRate,
        uint256 cliffTransferAmount,
        uint32 endDate,
        uint32 endDateValidBefore,
        bytes memory ctx
        ...
        // pick stream starting date
        uint32 effectiveStartDate = cliffDate == 0 ? startDate : cliffDate;
        if(effectiveStartDate <= block.timestamp ||
            effectiveStartDate >= endDate ||
            effectiveStartDate + startDateValidFor >= endDate - endDateValidBefore ||
            startDateValidFor == 0 || endDateValidBefore == 0
        ) revert TimeWindowInvalid();

        bytes32 hashConfig = keccak256(abi.encodePacked(sender,receiver,superToken));
        if(vestingSchedules[hashConfig].endDate == 0) {
            vestingSchedules[hashConfig] = Schedule(
                cliffDate,
                startDateValidFor,
                endDate,
                endDateValidBefore,
    ...
    function deleteVestingSchedule(
        ...
        if(vestingSchedules[configHash].endDate != 0) {
            ...
            emit DeleteVestingSchedule(sender,receiver,superToken);
        } else {
            revert ScheduleDontExist();
        }
    }

    /// @dev IVestingScheduler.executeVesting implementation.
    function executeVesting(
        address sender,
        address receiver,
        ISuperToken superToken
    ) external {
        bytes32 configHash = keccak256(abi.encodePacked(sender,receiver,superToken));
        Schedule memory schedule = vestingSchedules[configHash];
        if(schedule.effectiveStartDate > block.timestamp ||
            schedule.effectiveStartDate + schedule.startDateValidFor < block.timestamp
        ) revert TimeWindowInvalid();
        // invalidate configuration, avoid re-executions and reentry
        delete vestingSchedules[configHash].effectiveStartDate;
        delete vestingSchedules[configHash].startDateValidFor;
        delete vestingSchedules[configHash].cliffTransferAmount;
        // adjusted transfer amount based on block.timestamp
        uint256 adjustedAdditionalAmount = (block.timestamp - schedule.effectiveStartDate) * uint96(schedule.flowRate);
        // if we need to send tokens by cliffTransferAmount or adjustedAmount we send always that amount to receiver
        if (schedule.cliffTransferAmount > 0 || adjustedAdditionalAmount > 0) {
            superToken.transferFrom(
                sender,
                receiver,
                schedule.cliffTransferAmount + adjustedAdditionalAmount
            );
        }

        // Create a flow accordingly as per the stream schedule data.
        cfaV1.host.callAgreement(
            cfaV1.cfa,
            abi.encodeCall(
                cfaV1.cfa.createFlowByOperator,
                (superToken, sender, receiver, schedule.flowRate, new bytes(0))
            ),
            ""
        );
        ...
        emit ExecuteVesting(
            ...
            schedule.startDateValidFor,
            schedule.flowRate,
            schedule.cliffTransferAmount,
            adjustedAdditionalAmount
        );
    }


    /// @dev IVestingScheduler.executeCloseVesting implementation.
    function executeCloseVesting(
        address sender,
        address receiver,
        ISuperToken superToken
    ) external {
        bytes32 configHash = keccak256(abi.encodePacked(sender,receiver,superToken));
        Schedule memory schedule = vestingSchedules[configHash];
        if(schedule.endDate - schedule.endDateValidBefore > block.timestamp) revert TimeWindowInvalid();
        // remove schedule configuration
        delete vestingSchedules[configHash];
        uint256 adjustedMissingAmount = schedule.endDate > block.timestamp ? (schedule.endDate - block.timestamp) * uint96(schedule.flowRate) : 0 ;
        bool didTransferFail;
        if(adjustedMissingAmount > 0) {
            // try catch this, if user don’t have tokens to adjustedAmount we should close the stream anyway
            try superToken.transferFrom(sender,receiver,adjustedMissingAmount)
            {} catch {
        ...
        emit ExecuteClosingVesting(
            sender,
                receiver,
                superToken,
                schedule.endDate,
                schedule.endDateValidBefore,
                adjustedMissingAmount,
                didTransferFail
        );
    ...
    function getVestingSchedule(address sender, address receiver, address supertoken) external view returns (Schedule memory) {
        return vestingSchedules[keccak256(abi.encodePacked(sender,receiver,supertoken))];
    }
    ...
    function _getSender(bytes memory ctx) internal view returns(address sender) {
        ...
        if(sender == address(0)) revert AccountInvalid();
    }
...
interface IVestingScheduler {
    ...
    error ScheduleExist();
    error ScheduleDontExist();

    /**
     * @dev Vesting configuration provided by user.
     * @param effectiveStartDate Date that will be used to start vesting.
     * @param startDateValidFor Account who will be receiving the stream.
     * @param endDate The superToken to be streamed.
     * @param endDateValidBefore Timestamp when the stream should start (or 0 if starting not required)
     * @param flowRate For the stream (or 0 if starting not required)
     * @param cliffTransferAmount Amount you would like to transfer when you start streaming.
     */
    struct Schedule {
        uint32 effectiveStartDate;
        uint32 startDateValidFor;
        uint32 endDate;
        uint32 endDateValidBefore;
        int96 flowRate;
    ...
    /**
     * @dev Vesting order event executed by bot.
     ...
     * @param startDate The timestamp when the vesting should start
     * @param startDateValidFor How many seconds the stream is allowed to start after startDate / cliffDate
     * @param cliffDate Timestamp when the stream should start
     * @param flowRate The flowRate for the stream
     * @param endDate The timestamp when the stream should stop
     * @param endDateValidBefore How many seconds the stream is allowed to be closed before endDate
     * @param cliffTransferAmount The amount you would like to transfer at the startDate when you start streaming
     */
    ...
    /**
     * @dev Setup a vesting schedule
     * @dev Timestamp to start the stream is pick between cliffDate and startDate.
     * @dev Inclusion of two dates is for indexing propose only.
     * @param receiver Accounts who will be receiving the stream + cliffTokenAmount.
     * @param superToken SuperToken to be streamed
     * @param startDate The timestamp when the vesting should start.
     * @param startDateValidFor How many seconds the stream is allowed to start after startDate / cliffDate.
     * @param cliffDate The timestamp when the stream should start.
     * @param flowRate The flowRate for the stream.
     * @param cliffTransferAmount The amount you would like to transfer at the cliffDate when you start streaming
    ...
    /**
     * @dev Stream event executed by bot.
     ...
     * @param effectiveStartDate The timestamp when the stream should start (or 0 if starting not required)
     * @param startDateValidFor How many seconds the stream is allowed to start after startDate
     * @param flowRate The flowRate for the stream (or 0 if starting not required)
     * @param cliffTransferAmount The amount you would like to transfer at the startDate when you start streaming
     * @param adjustedAmount Adjusted amount transferred to receiver. (elapse time from config and tx timestamp)
     */
    event ExecuteVesting(
        address indexed sender,
        address indexed receiver,
        ISuperToken indexed superToken,
        uint32 effectiveStartDate,
    ...
    /**
     * @dev Executes a create stream to single or multi receivers.
    ...
    /**
     * @dev Vesting event executed by bot.
     * @param sender The account who will be send the stream
     * @param receiver The account who will be receiving the stream
     * @param superToken The superToken to be streamed
     * @param endDate The timestamp when the stream should stop
     * @param endDateValidBefore How many seconds before endDate the stream is allowed to be closed.
     * @param closingTransferAmount adjusted close amount transferred to receiver.
     * @param didTransferFail adjusted close amount transfer fail.
     */
    event ExecuteClosingVesting(
        address indexed sender,
        address indexed receiver,
        ISuperToken indexed superToken,
        uint32 endDate,
        uint32 endDateValidBefore,
        uint256 closingTransferAmount,
        bool didTransferFail
    );

    /**
     * @dev Executes a delete stream schedule.
     * @param sender The account who will be sending the stream
     * @param receiver The account(s) who will be receiving the stream
     * @param superToken The superToken to be streamed
     */
    function executeCloseVesting(
        address sender,
    ...
    /**
     * @dev Delete schedule event executed by bot.
     * @param sender The account who will be send the stream
     * @param receiver The account who will be receiving the stream
     * @param superToken The superToken to be streamed
     */
    event DeleteVestingSchedule(
        address indexed sender,
        address indexed receiver,
        ISuperToken indexed superToken
    );
    ...
    /**
     * @dev Get save data from vestingSchedule mapping.
     * @param sender The account who will be sending the stream
     * @param receiver The account who will be receiving the stream
     * @param superToken The superToken to be streamed
     */
