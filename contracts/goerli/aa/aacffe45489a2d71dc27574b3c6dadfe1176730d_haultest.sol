...
contract haultest is ERC721Enumerable, Ownable {
      Pass private pass;
  ...
  uint256 public setBlockTimestamp;
    ...
    constructor( address dependentContractAddress, uint256 maxNftSupply) ERC721("haultest", "HT") {
        maxPasses = maxNftSupply;
        pass = Pass(dependentContractAddress);
    }

  function trySetStartingIndexBlock() private {
    if (
      collectionStartingIndexBlock == 0 &&
      (totalSupply() == maxPasses || block.timestamp >= setBlockTimestamp)
    ) {
      collectionStartingIndexBlock = block.number;
    }
  }

 function Claim(uint256 passTokenId) public {
    require(totalSupply() < maxPasses, "Purchase would exceed max supply");
    require(passTokenId < maxPasses, "Requested tokenId exceeds upper bound");
    require(pass.ownerOf(passTokenId) == msg.sender, "Must be Owner of Pass for requested tokenId to claim");
    
    _safeMint(msg.sender, passTokenId);
