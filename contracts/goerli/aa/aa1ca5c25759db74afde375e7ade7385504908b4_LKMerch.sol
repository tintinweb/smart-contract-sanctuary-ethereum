...
contract LKMerch is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard, IERC2981 {
  ...
  mapping(uint256 => bool) public merchRedeemed;
  ...
  event MerchItemRedeemed(address indexed owner, uint256 indexed tokenId);
  ...
  function mintFreeNFT(address recipient, uint8 numTokens) public {
    for (uint8 i = 0; i < numTokens; i++) {
      _tokenIds.increment();
      _safeMint(recipient, _tokenIds.current());
      evolutionLevel[_tokenIds.current()] = 3;
  ...
  function redeemMerchItem(uint256 _id) public {
    require(msg.sender == super.ownerOf(_id), 'Access denied');
    require(!merchRedeemed[_id], 'Already redeemed');
    merchRedeemed[_id] = true;
    emit MerchItemRedeemed(msg.sender, _id);
  }
