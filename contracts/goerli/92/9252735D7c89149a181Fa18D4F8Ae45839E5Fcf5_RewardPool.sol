...
import "./utils/VerifySigner.sol";

contract RewardPool is Pausable, AccessControl, ReentrancyGuard, VerifySigner {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");

    bytes32 public constant CLAIM_TYPEHASH =
        keccak256(
            "claim(uint256 _amount, uint256 _nonces, bytes memory _signature)"
        );
    ...
    event ClaimEvent(address _receiver, uint256 _amount);

    constructor(address _token, address _signer) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);
        _setOracleSigner(_signer);
    ...
    function claim(
        ...
        public
        nonReentrant
        onlySignerWithUint256(CLAIM_TYPEHASH, _amount, _nonces, _signature)
    {
        require(
...
contract VerifySigner is AccessControl {
    using ECDSA for bytes32;
    address private _signer;

    bool internal _bEnableSequence = true;
    ...
    function setOracleSigner(address _account)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function _validate(
        ...
        if (_bEnableSequence)
            require(
                _nonce == _sequence[msg.sender],
                "Oracle: Invalid sequence"
            );
