...
contract ExmentMarketPlace {
     ...
     */
    modifier onlyAdmin() {
        require(admin == msg.sender, "Err: caller is not admin");
        _;
    }
    ...
    function CreateCollection(
        ...
        bytes32 salt = keccak256(abi.encodePacked(_seller, collections.length));
        assembly {
        ...
        emit CollectionCreated(newCollection, _seller, collections.length);
    }
}
...
contract ExmentCollection is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    function initialize(string memory name_, string memory symbol_, address seller_, uint256 price_) external {
