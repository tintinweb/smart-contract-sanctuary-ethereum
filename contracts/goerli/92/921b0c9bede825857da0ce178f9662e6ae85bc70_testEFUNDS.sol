...
contract testEFUNDS is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;
    
    string private _name     = "testFREEDOM FUND TOKEN";
    string private _symbol   = "tEFUNDS";
    uint8  private _decimals = 18;
    
    IUniswapV2Router02 public _uniswapV2Router; 
    address public _uniswapV2LiquidityPair; 
    
    address public _nftReflectionAddress = 0xE1C0e0565A681461c37896Eae65E4dce66A8ec59; 
    address payable public _growthFundAddress = payable(0x02AF948Ba23Cd9dC14c3AAc4dCa19a6bb9Aaeb18); 
    ...
    uint256 public reflectionFee = 85; //0.85%
    uint256 public nftReflectionFee = 15; //0.15%
    uint256 public growthFee = 50; // 0.5%
    uint256 public liquidityFee = 100; // 1%
    uint256 public burnFee = 50; //0.5%
    uint256 public  _totalTax = reflectionFee.add(nftReflectionFee).add(liquidityFee).add(growthFee).add(burnFee); 
    uint256 private _previousTaxFee;

    uint256 public _maxTxAmount        = 60000000 * 10**18;  
    uint256 public _tokenSwapThreshold = 30000 * 10**18; 
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount) private {
		...
		if (tFee > 0) {
	    	uint256 swapTokens = tFee.mul(liquidityFee.add(growthFee)).div(_totalTax);
            uint256 reflectionTokens = tFee.mul(reflectionFee).div(_totalTax);
            uint256 nftReflectionTokens = tFee.mul(nftReflectionFee).div(_totalTax);
            ...
            // nftReflection wallet fee 
             if(nftReflectionTokens > 0) {
                _nftReflectionTokens(nftReflectionTokens);
                 emit Transfer(sender, _nftReflectionAddress, nftReflectionTokens);
             }
            
            // Reflect some of the taxed tokens 
    		_reflectTokens(reflectionTokens);
            
            // Take the rest of the taxed tokens for liquidity and growth reward
            if(swapTokens > 0) {
    ...
    function _nftReflectionTokens(uint256 nftFee) private {
        uint256 currentRate = _getRate();
        uint256 rTakeAmount = nftFee.mul(currentRate);
         _rOwned[_nftReflectionAddress] = _rOwned[_nftReflectionAddress].add(rTakeAmount);
        if(_isExcluded[_nftReflectionAddress]) {
            _tOwned[_nftReflectionAddress] = _tOwned[_nftReflectionAddress].add(nftFee);
        }
    }
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        uint256 liquidityTokenShare = tokenAmount.mul(liquidityFee).div(liquidityFee.add(growthFee));
        uint256 growthFund = tokenAmount.sub(liquidityTokenShare);
        ...
        swapTokensForEth(growthFund); 
        payable(_growthFundAddress).transfer(address(this).balance);
    ...
    // for 1% put 100
    function setTaxFeePercent(uint256 reflection, uint256 nftReflection, uint256 growth, uint256 liquidity, uint256 burn) external onlyOwner() {
        reflectionFee = reflection;
        nftReflectionFee = nftReflection;
        growthFee = growth;
        liquidityFee = liquidity;
        burnFee = burn;
        _totalTax = reflectionFee.add(nftReflectionFee).add(liquidityFee).add(growthFee).add(burnFee);
        require(_totalTax <= 1000, "tax too high");
    }
    ...
    function setGrowthFundAddress(address growthAddress) external onlyOwner() {
        _growthFundAddress = payable(growthAddress);
    }

    function setNftReflectionAddress(address nftReflectionAddr) external onlyOwner() {
        _nftReflectionAddress = nftReflectionAddr;
    }
