...
contract XPodsAuction is ERC721AQueryable, ReentrancyGuard, AccessControl, Ownable {
    using Math for uint256;

    bytes32 public constant SUPPORT_ROLE = keccak256("SUPPORT");

    // initial stage
    AuctionStage public stage = AuctionStage.NOT_STARTED;
    ...
    uint256 public constant MAX_TEAM_MINT = 30;

    // this applies only during auction distribution of items own
    uint256 public constant MAX_PODS_WIN_PER_WALLET = 10;

    // the current minimum bid
    uint256 public minimumBid = 0.05 ether;
    ...
    uint256 podSalesWithdrawnByOwner = 0 ether;
    ...
    struct User {
        uint216 totalBid; // cumulative sum of ETH bids
        bool claimed; // tracker for claimed tokens
        uint256 refundedFunds; // has user been refunded yet
    }
     ...
     */
    function startClaimsAndRefunds() external onlyRole(SUPPORT_ROLE) {
        // storage to memory
        AuctionStage _stage = stage;

        // revert if the stage is not "bidding closed"
        if (_stage != AuctionStage.BIDDING_CLOSED) {
            revert StageMustBeBiddingClosed(_stage);
        }
    ...
    function bid() external payable {
        ...
        bidder.totalBid = uint216(totalUserBid);
     ...
     */
    function _refundAmount(uint256 userBid, uint256 _price)
        internal
        pure
        returns (uint256)
    {
    ...
    function sendRefund(address to) public onlyRole(SUPPORT_ROLE) {
        ...
        if (refundValue > 0) {
            ...
            user.refundedFunds = refundValue;
            // reassign
    ...
    function claimOnBehalfOfBatch(address[] calldata addresses)
        external
        onlyRole(SUPPORT_ROLE)
    {
        for (uint256 i; i < addresses.length; ++i) {
     ...
     */
    function setXDroidsContract(address xDroidsContractAddress_)
        external
        onlyRole(SUPPORT_ROLE)
    {
