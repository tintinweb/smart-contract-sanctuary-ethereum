...
contract TradFiLinesColor is ERC721, ERC721Enumerable, Ownable {
    ...
    function validateWrapWindow(uint hour, uint minute) public view returns(bool) {
        return validWrapWindows[hour][minute];
    }
    ...
    function wrap(uint tokenToWrap) external {
        ...
        if(limitedWrapping) {
            require(validateWrapWindow(hour, minute), 'Not within any wrapping window');
        }
