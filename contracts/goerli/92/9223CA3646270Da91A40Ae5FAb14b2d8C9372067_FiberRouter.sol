...
contract FiberRouter is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    event WithdrawSignedAndSwap(
        address to,
        uint256 swapAmount,
        uint256 withdrawalAmount,
        address[] path
    );
    event Withdrawal(address tokenAddress, address to, uint256 amount);
    event FiberSwap(
    ...
    function swap(
        ...
        address[] memory path;
        path[0] = token;
        emit FiberSwap(targetAddress, msg.sender, amount, amount, getChainId(), targetNetwork, path, targetToken);
    }
    ...
    */
    ...
    function withdrawSigned(
        ...
        emit Withdrawal(token, payee, amount);
    }
    ...
    function withdrawSignedAndSwapToFoundry(
        ...
        emit Withdrawal(targetFoundry, payee, amountOut);
    }
    ...
    function withdrawSignedAndSwap(
        ...
        emit WithdrawSignedAndSwap(to, amountIn, amountOutMin, path);
        emit Withdrawal(path[path.length - 1], to, amountOutMin);
    }
