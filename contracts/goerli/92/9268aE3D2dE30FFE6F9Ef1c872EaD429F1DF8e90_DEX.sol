...
contract DEX {
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        ...
        require(success, "DEX: ethOutput transfer did not transact");
        require(
            token.transferFrom(msg.sender, address(this), tokenInput),
            "DEX: tokenInput transfer did not transact"
        );
        emit TokenToEthSwap(
            msg.sender,
            "Balloons to Eth",
    ...
    function withdraw(
        ...
        require(
            ...
            "cannot transfer liquidity you dont have"
        );
        uint256 currentRatio = totalLiquidity / token.balanceOf(address(this));
        eth_amount = amount;
        token_amount = currentRatio.mul(eth_amount);
        (bool success, ) = payable(msg.sender).call{value: eth_amount}("");
        require(success, "DEX: withdraw: ETH send transfer failed");
        require(
            token.transfer(msg.sender, token_amount),
            "DEX: withdraw: token send failed"
        );
        totalLiquidity = address(this).balance;
