...
contract BookieMain is IBookieMain, AccessHandler {
    ...
    function createBet(BetData.Bet memory bet) private {
        ...
        (uint256 pot, bytes32 betHash) = betHistory.createBet(bet);
        emit BetWagered(bet.owner, bet.token, pot, bet, betHash);
    }
    ...
    function settleBet(bytes32 betHash) external override {
        ...
        if (paidToBetter == 0 && paidToLP == 0) {
            emit BetNothingToSettle(better, token);
            return;
        }
        console.log("Ready To PAY:", paidToBetter.div(1 ether), "and to return:", paidToLP.div(1 ether));
        ...
        emit BetSettled(better, token, paidToBetter, betHash);
    }
   ...
   /**
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if (paidToBetter == 0 && paidToLP == 0) {
            emit BetNothingToCancel(better, token);
            return;
        }
        ...
        emit BetCanceled(better, betHash, token, paidToBetter);
    }
...
library BetData {
    ...
    uint256 public constant MAX_ODDS = 1e3;
     ...
     */
    function assertBetExists(Bet storage bet) internal view {
        require(
            bet.amount > 0,
            "BET_NOT_FOUND"
        );
        ...
        returns (string memory)
    {
        ...
        if (bet.token == address(0)) {return "BAD_TOKEN";}
        return "OK";
    }
     ...
     * @param bet The bet to check.
     */
    function assertValidParams(Bet calldata bet) internal view {
        require(
            bet.amount > 0,
            "TOTAL_BET_SIZE_ZERO"
        );
        require(bet.decimalOdds > 0 && bet.decimalOdds > ODDS_PRECISION.mul(MAX_ODDS),
            "INVALID_DECIMAL_ODDS"
        );
        require(bet.token != address(0), "INVALID_BASE_TOKEN");
     ...
     */
    function assertValidAsSender(Bet calldata bet, address sender)
        internal
        view
    {
        assertValidParams(bet);
