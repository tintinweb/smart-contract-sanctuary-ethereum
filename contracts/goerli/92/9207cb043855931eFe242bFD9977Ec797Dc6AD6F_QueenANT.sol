...
contract QueenANT is ERC721Upgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => bool) public idToFertility;
    ...
    function getEpoch(uint256 _index) public view returns (uint256) {
        uint256 timeElapsed = block.timestamp - idToTimestamp[_index];
        uint256 epochsElapsed = timeElapsed / duration;

        return (epochsElapsed);
    }
    ...
    function setFertility(uint256 _index, bool _value) public {
        idToFertility[_index] = _value;
    }
        ...
        returns (uint256 deservedEggs)
    {
        ...
        uint256 epochsElapsed = (fertilityWindow / 3600) * 6;
        uint256 noOfEggs;
        ...
        for (uint256 i = 1; i <= epochsElapsed; i++) {
            uint256 eggsLaid = 6 + bonus - i;
            noOfEggs += eggsLaid;
        }
        if (idToFertility[_index] == true) {
