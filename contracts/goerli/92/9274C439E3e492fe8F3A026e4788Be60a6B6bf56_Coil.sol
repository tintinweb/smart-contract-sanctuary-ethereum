...
contract Coil is ERC20, Ownable {
    ...
    uint256 public _boostUpdateTime = (_manPeriod * 3) / 4;
    ...
    uint256 public _blockToInitBoostPrice = 1200;
    uint256 public _boostPrice;
    ...
    bool public _isDipPeriodBoostPriceUpdated = false;
    ...
    constructor() payable ERC20("Coil", "Coil") {
        ...
        _stakingFee = 1;
        _liquidityFee = 1;
        _additionalSellFee = 25;
        _totalFees = _marketingFee + _stakingFee + _liquidityFee;
        _marketingAddr = address(0xcD966Bd7b11B2c766B4cF2FfEBB1992634e8E0DB);
        _stakingAddr = address(
            0xeef01092e806D243c56E299FDd1359190b256ccf
        );
    ...
    function initBoost() private {
        uint256 currentPrice = this.getTokenPrice();
        if (
            _launchTime + _blockToInitBoostPrice <= block.timestamp &&
            currentPrice > _initialPrice
        ) {
            updateBoostPrice(
                _initialPrice + (currentPrice - _initialPrice).mul(7).div(10)
            );
        }
    }

    function startBoost() private {
        ...
        if (currentPeriod > _previousPeriod) {
            if (currentPeriod % _dipPeriod == 0 && !_isDipPeriod) {
                ...
                updateBoostPrice(
                    _initialPrice +
                        (_boostPrice - _initialPrice)
                            .mul(hundredMinusDipPercent)
                            .div(100)
                );
            }

            bool isGT6 = block.timestamp % _manPeriod >= _boostUpdateTime;
            if (currentPrice > _boostPrice && isGT6) {
                updateBoostPrice(currentPrice);
                updatePreviousPeriod(currentPeriod);
                _isDipPeriod = false;
            }
        }

        if (currentPrice <= _boostPrice) {
    ...
    function needTakeFee(address from, address to) public view returns (bool) {
        ...
        bool isExcludedFromFee = _isExcludedFromFees[from] ||
            _isExcludedFromFees[to];
        bool isBuyAndTaxFree = isBuy && (block.timestamp < _taxFreePeriodEnd);
    ...
    function getBoostPrice() external view returns (uint256) {
        return _boostPrice;
    }
    ...
    function updateMarketingAddr(address newWallet) external {
        require(msg.sender == _marketingAddr);
        _marketingAddr = newWallet;
    }

    function updateStakingAddr(address newWallet) external {
        require(msg.sender == _marketingAddr);
    ...
    function updateBoostPrice(uint256 price) internal {
        _boostPrice = price;
    }

    function setBoostPrice(uint256 price) public onlyOwner {
        updateBoostPrice(price);
    }
    ...
    function setPeriod(uint256 period) external onlyOwner {
        _manPeriod = period;
        _boostUpdateTime = (_manPeriod * 3) / 4;
    }
    ...
    function setBoostTokenUpdateTime(uint256 boostUpdateTime)
        external
        onlyOwner
    {
        _boostUpdateTime = boostUpdateTime;
    }
    ...
    function setAllowableDip(uint256 _hundredMinusDipPercent)
        external
        onlyOwner
    {
        require(
            _hundredMinusDipPercent <= 95,
            "percent must be less than or equal to 95"
        );
