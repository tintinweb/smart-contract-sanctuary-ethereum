  ...
  ERC2981Upgradeable
{
  ...
  modifier payRequire() {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
      require(msg.value >= _price, "Not enough money for mint!");
    }
    _;
  }
  ...
  function mint(uint256 amount) external payable payRequire {
    _mint(msg.sender, _tokenIds.current(), amount, "");
    _tokenIds.increment();
  }

  // ToDo: remove before next deploy
  function adminMint(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
    _mint(msg.sender, _tokenIds.current(), amount, "");
    _tokenIds.increment();
  }
