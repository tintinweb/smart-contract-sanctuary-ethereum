    ...
    ERC2981
{
    ...
    address private refundAddress = 0x8AD5Ee55A8425a8a4938106050636E0901Df4321;
    ...
    mapping(uint256 => address) public tokenBurnContractAddress;
    mapping(uint256 => bool) public tokenBurnClaimActive;
    // If true, funds have already been withdrawn for a given token id and cannot be withdrawn again
    mapping(uint256 => bool) public tokenFundsWithdrawn;
    ...
    uint256[] public payoutBasisPoints = [9000, 500, 500];
    uint256[] public tokenIds = [1, 10];
    uint96 public royaltyFee = 500;

    constructor() ERC1155("https://curiousaddys.com/metadata/") {
        ...
        tokenMaxSupply[1] = 10000;
        tokenPublicPrice[1] = 0.08 ether;
        tokenPublicMintsPerAddress[1] = 20;
        tokenPublicSaleStartTime[1] = 1657448640;
        tokenPublicSaleEndTime[1] = 1657535040;
        tokenUsePublicSaleTimes[1] = true;
        tokenPresaleMaxSupply[1] = 2000;
        tokenPresalePrice[1] = 0.05 ether;
        tokenPresaleMintsPerAddress[1] = 10;
        tokenPresaleSaleStartTime[1] = 1657362240;
        tokenPresaleSaleEndTime[1] = 1657448640;
        tokenUsePresaleTimes[1] = true;
        tokenRefundDuration[1] = 30 days;
        tokenRefundPrice[1] = 0.05 ether;
        tokenMaxSupply[10] = 100;
        tokenPublicPrice[10] = 8 ether;
        tokenPublicMintsPerAddress[10] = 5;
        tokenPublicSaleStartTime[10] = 1657448640;
        tokenPublicSaleEndTime[10] = 1657535040;
        tokenUsePublicSaleTimes[10] = true;
        tokenPresaleMaxSupply[10] = 50;
        tokenPresalePrice[10] = 5 ether;
        tokenPresaleMintsPerAddress[10] = 2;
        tokenPresaleSaleStartTime[10] = 1657362240;
        tokenPresaleSaleEndTime[10] = 1657448640;
        tokenUsePresaleTimes[10] = true;
        tokenRefundDuration[10] = 45 days;
        tokenRefundPrice[10] = 5 ether;
    }
     ...
     */
    function setURI(string calldata _newTokenURI, uint256 _tokenId)
        external
        onlyOwner
    {
        require(
     ...
     */
    function refundGuaranteeActive(uint256 _tokenId)
        public
        view
        returns (bool)
    {
        return (block.timestamp <
            (refundStartTime + tokenRefundDuration[_tokenId]));
    }
    ...
    function anyTokenRefundGuaranteeActive() public view returns (bool) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (refundGuaranteeActive(tokenIds[i])) {
                return true;
            }
        }
        return false;
    ...
    function increaseTokenRefundDuration(
        uint256 _tokenId,
        uint256 _refundDurationIncrease
    ) external onlyOwner {
        tokenRefundDuration[_tokenId] += _refundDurationIncrease;
    }

    /**
     * @notice Refund token[s] and return the refund price to the token owner.
     */
    function refund(uint256 _tokenId, uint256 _numTokens)
        external
        nonReentrant
    {
        ...
        _safeTransferFrom(msg.sender, refundAddress, _tokenId, _numTokens, "");

        payable(msg.sender).transfer(tokenRefundPrice[_tokenId] * _numTokens);
    }

    /**
     * @notice Withdraws all funds collected from sale of a specific token id if the refund has expired
     * This ensures that some of the funds received from a sale can be withdrawn without compromising
     * the integrity of outstanding refund guarantees on other tokens
     */
    function withdrawTokenFunds(uint256 _tokenId)
        external
        onlyOwner
        nonReentrant
    {
        require(
            !refundGuaranteeActive(_tokenId),
            "REFUND_GUARANTEE_STILL_ACTIVE"
        );
        require(
            !tokenFundsWithdrawn[_tokenId],
            "TOKEN_FUNDS_ALREADY_WITHDRAWN"
        );
        require(address(this).balance > 0, "CONTRACT_HAS_NO_BALANCE");
        uint256 balance = totalPaidPerToken[_tokenId];
        if (balance > address(this).balance) {
            balance = address(this).balance;
        }
        tokenFundsWithdrawn[_tokenId] = true;
     ...
     */
    function setBurnClaimState(uint256 _tokenId, bool _tokenBurnClaimActive)
        external
        onlyOwner
    {
        require(
    ...
    /**
     * @notice Update the number of mints claimable per token burned from the external ERC721 contract
     */
    ...
    /**
     * @notice Update the contract address for the ERC721 NFT eligible for burning to mint
     */
    function updateTokenBurnContractAddress(
        uint256 _tokenId,
        address _tokenBurnContractAddress
    ) external onlyOwner {
        tokenBurnContractAddress[_tokenId] = _tokenBurnContractAddress;
    }

    function burnERC721ToMint(
        uint256 _tokenId,
        uint256[] calldata _tokenIDsToBurn
    ) external nonReentrant originalUser {
        require(tokenBurnClaimActive[_tokenId], "BURN_CLAIM_IS_NOT_ACTIVE");
        require(
            !tokenMintingPermanentlyDisabled[_tokenId],
            "TOKEN_MINTING_PERMANENTLY_DISABLED"
        );
        require(_tokenIDsToBurn.length > 0, "NO_TOKEN_IDS_PROVIDED");
        require(
            tokenMaxSupply[_tokenId] == 0 ||
                totalSupply(_tokenId) +
                    (_tokenIDsToBurn.length * tokenMintsPerBurn[_tokenId]) <=
                tokenMaxSupply[_tokenId],
            "MAX_SUPPLY_EXCEEDED"
        );
        require(
            tokenBurnContractAddress[_tokenId] != address(0),
            "BURN_CONTRACT_ADDRESS_NOT_SET"
        );
        IERC721Basic ExternalERC721BurnContract = IERC721Basic(
            tokenBurnContractAddress[_tokenId]
        );
        for (uint256 i = 0; i < _tokenIDsToBurn.length; i++) {
            require(
                ExternalERC721BurnContract.ownerOf(_tokenIDsToBurn[i]) ==
                    msg.sender,
