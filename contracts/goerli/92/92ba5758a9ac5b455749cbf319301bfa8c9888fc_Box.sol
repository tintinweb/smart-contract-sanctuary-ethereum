    ...
    ERC1155PresetMinterPauserUpgradeable
{
    ...
    CountersUpgradeable.Counter public _idTracker;

    mapping(uint256 => uint256) public metaId;
    ...
    function mintBox(
        ...
        require(hasRole(MINTER_ROLE, _msgSender()), "sender err");
        _idTracker.increment();
        uint256 tokenId = _idTracker.current();
        bytes memory data;
        mint(boxReceiver, tokenId, amount, data);
        metaId[tokenId] = _metaId;
        emit NewBox(tokenId, amount, _metaId, block.timestamp);
        return tokenId;
    }

    function uri(uint256 tokenId) public view override returns (string memory) {
        uint256 _metaId = metaId[tokenId];
        return string.concat(_uri, string.concat(_metaId.toString(), ".json"));
    }

    function setMetaId(uint256 tokenId, uint256 _metaId) public onlyOwner {
        metaId[tokenId] = _metaId;
    }
    ...
    function curTokenId() public view returns (uint256) {
    ...
    function totalSupply(uint256 id)
        public
        view
        override(IBox, ERC1155SupplyUpgradeable)
        returns (uint256)
    {
