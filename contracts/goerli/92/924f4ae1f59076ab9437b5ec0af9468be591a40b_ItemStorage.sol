...
//Circuit structs an mapps/arrays
    ...
    mapping(uint => string) circuitMap;
...
//Circuit functions
    ...
    function getSLAOfCircuit(uint _circuitIdentifier) public view returns (string memory, SLARange[] memory) {
        string memory _name = circuitMap[_circuitIdentifier]; 
        require(SLAMap[_name].length > 0);
    
        SLARange[] memory returnedRanges = new SLARange[](SLAMap[_name].length);

        for(uint i = 0; i < SLAMap[_name].length; i++){
            returnedRanges[i] = SLAMap[_name][i];
        }
        return (_name, returnedRanges);

    }

    function AddSLAToCircuit(string memory _name, uint _circuitIdentifier) public {
        require (SLAMap[_name].length > 0);
        circuitMap[_circuitIdentifier] = _name;
