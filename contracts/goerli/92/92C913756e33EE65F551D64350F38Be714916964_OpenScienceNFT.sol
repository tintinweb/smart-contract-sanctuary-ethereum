...
contract OpenScienceNFT is ERC721, Ownable {
     ...
     */
    constructor(string memory _baseURI, address _admin)
        ERC721("OpenScienceNFTStage", "SCINFTSSTAGE")
    {
     ...
     * - invocation can be done, only by the contract owner.
     */
    function mint(address[] memory _receivers) external onlyOwnerOrAdmin {
        ...
        for (uint256 i = 0; i < receiverLength; i++) {
            ...
            _safeMint(_receivers[i], mintIndex);
            tokenIds += 1;
        }
    }
     ...
     * @dev To get all mined nfts
     */
    function sciNFTsOF(address _owner)
        external
        view
    ...
    function _transfer(
        ...
        // CHECK FOR 0xffff... if it exists in `to` then put the tokenId in that slot
