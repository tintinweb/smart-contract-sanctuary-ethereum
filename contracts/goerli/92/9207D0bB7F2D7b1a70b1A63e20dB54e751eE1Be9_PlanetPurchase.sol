...
contract PlanetPurchase is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function parseBytes(bytes memory invoice, uint256 count, uint8 startIdx) internal pure returns(bytes[] memory, uint8) {
        bytes[] memory retData = new bytes[](count);
        uint8 tokenLength = 0;
        uint8 idx = 0;

        for(uint8 pos = startIdx; pos < invoice.length; pos++) {
            if (invoice[pos] == 0x2c || invoice[pos] == 0x7c) {                 
            ...
            if (pos == invoice.length -1)
                retData[idx] = Utils.slice(invoice, startIdx,tokenLength);
        }
        return (retData, startIdx);
    }
    ...
    function get_salesCompleteCount() external view returns (uint) {
        return SafeMath.sub(_publicSalesIndex, 101);
    }
    ...
    function get_lastPlanetID() external view returns(uint) {
