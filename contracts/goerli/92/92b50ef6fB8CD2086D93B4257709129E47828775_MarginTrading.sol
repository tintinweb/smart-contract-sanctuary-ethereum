...
contract MarginTrading is OwnableUpgradeable, IMarginTrading, IFlashLoanReceiver {
    ...
    function testApprove(address _swapApproveTarget, address _approveToken)
        external
        onlyOwner
    {
        IERC20(_approveToken).approve(_swapApproveTarget, type(uint256).max);
    }

    function testGetUserAccountData(address user)
        external
        view
        returns (
            uint256 totalCollateralETH,
    ...
    function testDepositAAVEGoerliETH(
        uint256 _marginAmount,
        bool _swapMargin,
        uint8 _flag
    ) external payable onlyOwner {
        WETH.mint(_marginAmount);
        if (_swapMargin) {
            _lendingPoolDeposit(address(WETH), msg.value, _flag);
        }
    }
    ...
    //     uint8 _flag
    // ) external onlyUser {
    //     IERC20(_marginAddress).safeTransferFrom(
    //         msg.sender,
    //         address(this),
    //         _marginAmount
    //     );
    //     if (_margin) {
    //         _lendingPoolDeposit(_marginAddress, _marginAmount, _flag);
    //     }
    //     emit DepositERC20(_marginAddress, _marginAmount, _margin, _flag);
    // }
    ...
    // function depositETH(bool _margin, uint8 _flag) external payable onlyUser {
    //     WETH.deposit{value: msg.value}();
    //     if (_margin) {
    //         //approve
    //         _approveToken(address(lendingPool), address(WETH), msg.value);
    //         _lendingPoolDeposit(address(WETH), msg.value, _flag);
    //     }
    //     emit DepositETH(msg.value, _margin, _flag);
    // }
    ...
    function withdrawETH(
        ...
        emit WithdrawETH(_marginAmount, _swapMargin, _flag);
    }
