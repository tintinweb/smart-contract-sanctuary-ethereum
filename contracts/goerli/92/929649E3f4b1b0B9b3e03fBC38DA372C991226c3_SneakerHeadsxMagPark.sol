...
error SneakerHeadsxMagPark__TokenIdOutOfRange();
 ...
 */
contract SneakerHeadsxMagPark is
    ERC1155,
    ERC1155Supply,
    Manageable,
    DefaultOperatorFilterer
{
    ...
    string private constant BASE_TOKEN_URI = "ipfs://tatata/{id}";
    uint8 private constant MIN_TOKEN_ID = 1;
    uint8 private constant MAX_TOKEN_ID = 11;

    bytes8 private constant HASH_SALT = 0xa4dae93d75b4175c;
    address private constant SIGNER_ADDRESS =
        0xFc8931C9bD3bf8F8D4Ae10215168e4062F25Aa57;
    ...
    mapping(SALE_STAGE => mapping(address => uint256))
        private s_numberMintedDuringSaleStage;

    constructor() ERC1155(BASE_TOKEN_URI) {}
    ...
    function mint(
        ...
        if (saleStage == SALE_STAGE.CLOSED)
            revert SneakerHeadsxMagPark__SalesAreClosed();
        if (saleStage == SALE_STAGE.ENDED)
            revert SneakerHeadsxMagPark__SalesHaveEnded();

        if (tokenId < MIN_TOKEN_ID || tokenId > MAX_TOKEN_ID)
            revert SneakerHeadsxMagPark__TokenIdOutOfRange();
        ...
        if (
            _mintOperationHash(msg.sender, quantity, maxQuantity, nonce) != hash
        ) revert SneakerHeadsxMagPark__HashComparisonFailed();
        if (!_isTrustedSigner(hash, signature))
            revert SneakerHeadsxMagPark__UntrustedSigner();
        if (s_usedNonces[nonce])
            revert SneakerHeadsxMagPark__SignatureAlreadyUsed();
     ...
     */
    function withdrawMoney(address payable to) external onlyManager {
        if (address(this).balance == 0)
            revert SneakerHeadsxMagPark__NothingToWithdraw();
     ...
     */
    function setAllowlistSaleStartTime(
    ...
    /**
     * @dev This function is used for getting metadata URI of a token
     *
     * @param tokenId Token ID to get metadata URI of
     *
    ...
    function getCurrentSaleStage() public view returns (SALE_STAGE) {
        if (block.timestamp >= s_allowlistSaleStartTime)
            return SALE_STAGE.ENDED;
        
        if (block.timestamp >= s_allowlistSaleStartTime)
            return SALE_STAGE.ALLOWLIST;

        if (block.timestamp >= s_vipSaleStartTime) 
            return SALE_STAGE.VIP;
    ...
    // Overrides for ERC1155Supply support
