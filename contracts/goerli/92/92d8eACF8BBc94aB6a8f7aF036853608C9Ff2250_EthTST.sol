...
import "./MagpieTokenDisplayInfo.sol";

 contract EthTST is MagpieTokenDisplayInfo {
    using Strings for uint256;

    constructor(uint256 _decimals) MagpieTokenDisplayInfo(_decimals) {}
...
abstract contract MagpieTokenDisplayInfo is Ownable {
  using Strings for uint256;
  ...
  function _truncateDigitsFromRight(uint256 _number, uint256 _digitsCount)
    internal
    pure
    returns (uint256)
  {
  ...
  function _divideByPowerOf10(
    ...
    uint256 digitsToTruncateCount = _getDigitsCount(fractionalPartTemp) +
      leadingZeroesToAddBeforeDecimal >
  ...
  function _calculatePercentage(uint256 _num, uint256 _denom)
    internal
    pure
    returns (string memory)
  {
