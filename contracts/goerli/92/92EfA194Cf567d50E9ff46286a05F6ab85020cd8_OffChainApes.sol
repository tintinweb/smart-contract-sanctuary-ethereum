...
contract OffChainApes is ERC721Burnable, ERC2981, Verifier {
    using Strings for uint256;
    ...
    constructor(bytes32 _root) ERC721("OffChainApes", "OCA") Verifier(_root) {}
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: black; font-family: serif; font-size: 24px; }</style><rect width="100%" height="100%" fill="#ffffff" /><text x="30" y="175" class="base">';
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "OffChainApes #',
                        tokenId.toString(),
                        '", "description": "This is one of 10,000 NFTs created by Verdomi as part of an art project that serves as a commentary on the fact that the artwork featured on most NFTS are not stored on chain. The original idea that served as inspiration for this project was posted by dankmfer.eth on Twitter. There is a 10% royalty on the collection, half of the royalty is sent to Verdomi and the other half is sent to dankmfer.", "image": "data:image/svg+xml;base64,',
