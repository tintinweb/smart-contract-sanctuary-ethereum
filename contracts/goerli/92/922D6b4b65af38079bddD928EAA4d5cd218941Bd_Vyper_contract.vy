interface IStarknetCore:
    ...
    def sendMessageToL2(toAddress: uint256, selector: uint256, payload: DynArray[uint256, 2]) -> (bytes32): nonpayable

    #Consumes a message that was sent from an L2 contract.
    #Returns the hash of the message.
    def consumeMessageFromL2(fromAddress: uint256, payload: DynArray[uint256, 3]) -> (bytes32): nonpayable
...
def withdraw(l2ContractAddress: uint256, user: uint256, amount: uint256):
    ...
    payload: DynArray[uint256, 3] = [MESSAGE_WITHDRAW, user, amount]
...
def deposit(l2ContractAddress: uint256, user: uint256, amount: uint256):
    ...
    payload: DynArray[uint256, 2] = [user, amount]
