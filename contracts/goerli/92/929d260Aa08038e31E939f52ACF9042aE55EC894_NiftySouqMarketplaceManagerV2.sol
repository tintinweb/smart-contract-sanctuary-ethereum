    ...
    PausableUpgradeable
{
    ...
    modifier isNiftyMarketplace() {
        if (
            msg.sender != address(_niftySouqErc721V2) &&
            msg.sender != address(_niftySouqErc1155V2) &&
    ...
    ) external onlyRole(CFO_ROLE) whenNotPaused {
        if (!isAdmin(msg.sender)) revert GeneralError("NS:101");
        if (blockList[msg.sender] == true) revert GeneralError("NS:126");
     ...
     */
    function pause() public onlyRole(COO_ROLE) {
        _pause();
    }
     ...
     * @dev Only by pauser role
     */
    function unpause() public onlyRole(COO_ROLE) {
        _unpause();
    }
     ...
     */
    function blockListUpdate(address _address, bool _value) public onlyRole(COO_ROLE){
     ...
     *@param niftySouqErc721V1_ address of niftySouqErc721V1.
     *@param niftySouqErc1155V1_ address of niftySouqErc1155V1.
     *@param niftySouqErc721V2_ address of niftySouqErc721V2.
     *@param niftySouqErc1155V2_ address of niftySouqErc1155V2.
     */
    function setContractAddress(
        ...
        address niftySouqFixedPrice_,
        address niftySouqAuction_
    ) external {
        if (!isAdmin(msg.sender)) revert GeneralError("NS:101");
        if (blockList[msg.sender] == true) revert GeneralError("NS:126");
        if (niftySouqMarketplace_ != address(0))
            _marketplace = niftySouqMarketplace_;
        ...
        if (niftySouqFixedPrice_ != address(0))
            _niftySouqFixedPrice = NiftySouqIFixedPrice(niftySouqFixedPrice_);
        if (niftySouqAuction_ != address(0))
            _niftySouqAuction = NiftySouqIAuction(niftySouqAuction_);
    }
    ...
    function addCryptoToken(
        ...
        if (!isAdmin(msg.sender)) revert GeneralError("NS:101");
        if (blockList[msg.sender] == true) revert GeneralError("NS:126");
        if (tokenExist(tokenName_)) revert GeneralError("NS:111");
        if (!_is20(address_)) revert GeneralError("NS:113");
        if (
            keccak256(abi.encodePacked(tokenName_)) ==
            keccak256(abi.encodePacked(""))
        ) revert GeneralError("NS:114");
        if (tokenValue_ == 0) revert GeneralError("NS:115");
    ...
    function editCryptoToken(
        ...
        if (!isAdmin(msg.sender)) revert GeneralError("NS:101");
        if (blockList[msg.sender] == true) revert GeneralError("NS:126");
        if (!tokenExist(tokenName_)) revert GeneralError("NS:112");
        if (!_is20(address_)) revert GeneralError("NS:113");
        if (
            keccak256(abi.encodePacked(tokenName_)) ==
            keccak256(abi.encodePacked(""))
        ...
        whenNotPaused
    {
        ...
        if (enable_ && _cryptoTokenList[tokenName_].isEnabled)
            revert GeneralError("NS:116");
        if (!enable_ && !_cryptoTokenList[tokenName_].isEnabled)
            revert GeneralError("NS:117");
    ...
    function isAdmin(address caller_) public view returns (bool isAdmin_) {
        if (
            hasRole(DEFAULT_ADMIN_ROLE, caller_) || hasRole(CFO_ROLE, caller_)
        ) return true;
        else return false;
    }
        ...
        )
    {
        ...
        niftySouqMarketplace_ = _marketplace;
        niftySouqErc721V1_ = address(_niftySouqErc721V1);
        niftySouqErc1155V1_ = address(_niftySouqErc1155V1);
        niftySouqErc721V2_ = address(_niftySouqErc721V2);
        niftySouqErc1155V2_ = address(_niftySouqErc1155V2);
    }
     ...
     *@return contractType_ type of contract such as NIFTY_V1,NIFTY_V2,COLLECTOR,EXTERNAL,UNSUPPORTED.
    ...
    {
        if (contractAddress_ == address(_niftySouqErc721V1))
            return (ContractType.NIFTY_V1, false);
        else if (contractAddress_ == address(_niftySouqErc1155V1))
            return (ContractType.NIFTY_V1, true);
        else if (contractAddress_ == address(_niftySouqErc721V2))
            return (ContractType.NIFTY_V2, false);
        else if (contractAddress_ == address(_niftySouqErc1155V2))
            return (ContractType.NIFTY_V2, true);
        else {
            ...
            if (success) return (ContractType.COLLECTOR, false);
                ...
                if (_is721(contractAddress_))
                    return (ContractType.EXTERNAL, false);
    ...
    /**
     *@notice verifying the signature for lazyminting in auction.
     *@param lazyData_ contains tokenAddress,uri,seller address,creater addresses, royalties percentage, investors addresses, revenue percentage
     *start time of aunction,duration,start bid price,reserve price and signature in bytes.
    ...
    function _isNftTransferApproved(address seller_, address nftContract_)
        ...
        returns (bool)
    {
        if (nftContract_ == address(_niftySouqErc721V2)) return true;
