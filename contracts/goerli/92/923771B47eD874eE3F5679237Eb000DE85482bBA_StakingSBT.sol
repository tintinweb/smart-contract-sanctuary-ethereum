...
contract StakingSBT is ERC721Psi, IERC4906, Ownable {
    ...
    //from TIM for testing
    string public domain = "abc123.xyz";
    string public description = "StakingSBT";

    string public nft_name = "STKR";
    ...
    constructor() ERC721Psi("StakingReceipt", "STKR") {
        header = string(abi.encodePacked(
                ...
                '<text style="white-space:pre;fill:rgb(32,194,14);font:normal 32px Courier, Monospace, serif;" x="7" y="17">'));
    ...
    function getSVGTextLine(uint256 _startingY, uint256 _rank) public pure returns (string memory) {
        return string(
            abi.encodePacked(
                '<tspan x="7" y="', 
                Strings.toString(_startingY),
                '">',
                Strings.toString(_rank),
                '</tspan>'));
    }

    function buildImage(uint256 _thisRank) public view returns (string memory) {
        return
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        header,
                        getSVGTextLine(60,_thisRank),
                        '</text>',
                        footer,
                        '</svg>'
                    )
    ...
    function buildMetadata(uint256 _thisRank)
        public
        view
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                buildImage(_thisRank),
                                '", "attributes": ',
                                "[",
                                '{"trait_type": "STKR-ID",',
                                '"value":"',
                                Strings.toString(_thisRank),
                                '"}',
                                "]",
                                "}"
                            )
