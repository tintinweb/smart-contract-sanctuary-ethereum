...
contract QRNGFacet is WithRoles, IQRNGReceiver, WithAirnodeRrp {
  ...
  function closeRound() external onlyRole(REQUEST_NUMBER_ROLE) returns (bytes32) {
...
library LibPonzu {
  ...
  event RoundClosed(uint256 roundNumber);
  ...
  function currentRoundNumber(PonzuStorage storage ps) internal view returns (uint256) {
    return (block.timestamp < ps.startTime) ? 0 : (block.timestamp - ps.startTime) / ROUND_DURATION;
  }
  ...
  function selectWinner(PonzuStorage storage ps, uint256 randomNum) internal {
    bool firstRound = ps.roundWinners[0].winner == address(0);
    uint256 roundNum = firstRound ? 0 : ps.lastClosedRound + 1;
    ...
    uint256 totalDeposit = ps.sumTotalValidDeposits();

    uint256 winnerNumber = randomNum % totalDeposit;
    ...
    if (IERC20(DS().rewardToken).balanceOf(winnerAddress) + winnerAmount > MAX_DEPOSIT) {
  ...
  function endRoundWithoutWinner(PonzuStorage storage ps) internal {
    ...
    ps._endRoundWithoutWinner();
  }

  function _endRoundWithoutWinner(PonzuStorage storage ps) internal {
