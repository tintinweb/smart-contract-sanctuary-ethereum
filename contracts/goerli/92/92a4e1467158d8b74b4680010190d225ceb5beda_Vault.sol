...
contract Vault is Ownable {
    ...
    struct Employee{
        ...
        address token;
        uint256 frequency;
        bool deleted; // 1 for monthly and 2 for bi-weekly
    }
    ...
    function sendSalaryBiWeekly() external{
        require(msg.sender == chainlinkUpkeepAddress, "You are not authorized to call this function");
        for(uint256 i=0; i<employees.length; i++){
            if(employeeDetails[employees[i]].frequency == 2 && employeeDetails[employees[i]].deleted == false){
    ...
    function sendSalaryMonthly() external{
        require(msg.sender == chainlinkUpkeepAddress, "You are not authorized to call this function");
        for(uint256 i=0; i<employees.length; i++){
            if(employeeDetails[employees[i]].frequency == 1 && employeeDetails[employees[i]].deleted == false){
                uint256 amount = employeeDetails[employees[i]].salary;
                IERC20(employeeDetails[employees[i]].token).transfer(employees[i], amount);
            }
        }
    ...
    function addEmployees(address _employeeAddress, uint256 _employeeSalary, address _token, uint256 _frequency) external onlyOwner{
        employeeDetails[_employeeAddress].salary = _employeeSalary;
        employeeDetails[_employeeAddress].token = _token;
        employeeDetails[_employeeAddress].frequency = _frequency;
        employees.push(_employeeAddress);
    }

    function editEmployees(address _employeeAddress, uint256 _employeeSalary, address _token, uint256 _frequency) external onlyOwner{
        employeeDetails[_employeeAddress].salary = _employeeSalary;
        employeeDetails[_employeeAddress].token = _token;
        employeeDetails[_employeeAddress].frequency = _frequency;
    }
    ...
    // function getAllEmployeeDetails() public{
    //     Employee[] emps;
    //     for(uint256 i=0; i<employees.length; i++){
    ...
    function getJobIds() public view returns(uint256[] memory){
        return cronFactory.getActiveCronJobIDs();
    }
