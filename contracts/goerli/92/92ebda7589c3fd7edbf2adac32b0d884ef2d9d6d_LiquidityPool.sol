...
library BytesLib{
    ...
    function toAddressPayable(bytes memory _bytes, uint256 _start)  internal pure returns (address payable) {
        require(_bytes.length >= _start + 20, "toAddress_outOfBounds");
    ...
    ERC2771ContextUpgradeable
{
    ...
    IWormhole public bridge;
    address public bridgeAddress;
    address public uniswapAddress;
    // uint deadline = block.timestamp + 1500;
    // uint256 public arbiterFeeValue =0;
    IWormholeCore public coreBridge;
    ...
    function swapInitialize( address _uniswap, address _bridgeAddress, address _coreBridge) public{
    ...
    function swapInToken(UserInput memory ui) external payable returns(uint64 coreSequence){
        uint[] memory amounts;
        uint[] memory amountsOut;
        uint32 nonceValue = uint32(block.timestamp % 2**32);
        address _origToken = ui.path[0];
        IERC20Upgradeable originalToken = IERC20Upgradeable(_origToken);
        // {
        ...
        IERC20Upgradeable token = IERC20Upgradeable(swapTokenAddress);
        SafeERC20Upgradeable.safeIncreaseAllowance(token, bridgeAddress, amounts[amounts.length-1]);
        /*change the below to ERC20*/
        depositErc20(ui.recipientChainId,swapTokenAddress, ui.recipient, amounts[amounts.length-1]);
        }
        ...
        {Payload memory payload = Payload({tokenAddress: ui.path[0], 
                                          amount: amounts[amounts.length-1],
                                          receiver: ui.recipient,
                                          tokenGasPrice: ui.gasPrice,
                                          fromChainId: ui.recipientChainId});
        bytes memory encoded = abi.encodePacked(payload.tokenAddress, payload.amount,  payload.receiver, payload.tokenGasPrice, payload.fromChainId);
...
/*struct Payload{
    ...
    function parsePayload(bytes memory encoded) public pure returns(Payload memory payload){
        uint index = 0;
        payload.tokenAddress = encoded.toAddress(index);
        index += 20;
        payload.amount = encoded.toUint256(index);
        index += 32;
        payload.receiver = encoded.toAddressPayable(index);
        index += 20;
        payload.tokenGasPrice = encoded.toUint256(index);
        index += 32;
        payload.fromChainId = encoded.toUint256(index);
        index += 32;
    ...
    function depositNative(
        ...
        emit Deposit(_msgSender(), NATIVE, receiver, toChainId, msg.value + rewardAmount, rewardAmount);
    }
    ...
    function sendFundsToUser(
        ...
        function swapOutToken(bytes memory encodedVmCore) external {
        ...
        if (valid == true){
            ...
            // address swapToken = payload.tokenAddress;
            // IERC20Upgradeable token = IERC20Upgradeable(swapToken);
            // require(token.approve(address(uniswap), payload.amount), 'approve failed.');
            // address[] memory path;
            // path[0] = payload.tokenAddress;
            // targetAmountsOut = uniswap.getAmountsOut(payload.amount, path);
            sendFundsToUser(payload.tokenAddress, payload.amount, payload.receiver, payload.tokenGasPrice, payload.fromChainId);            
        }
    }
