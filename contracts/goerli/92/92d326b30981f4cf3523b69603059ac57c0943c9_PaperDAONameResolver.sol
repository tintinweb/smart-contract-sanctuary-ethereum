...
// File: contracts/PaperDAONameLocator.sol
...
contract PaperDAONameResolver is Ownable {
    ...
    function setStakingContractAddress(address stakingContractAddress) external onlyOwner {
        if (address(stakingContractAddress) != address(0)) {
            stakingContract = AcrocalypseStakingContract(stakingContractAddress);
        }
    }

    function getTokenAddress(uint256 tokenid) internal view returns (address) {
        try stakingContract.stakedTokens(tokenid) returns (AcrocalypseStakingContract.StakedToken memory _stakingOwnerAddress) {
            if (address(_stakingOwnerAddress.owner) != address(0)) {
                return _stakingOwnerAddress.owner;
            }
        } catch {
            return address(0);
        }

        try tokenContract.ownerOf(tokenid) returns (address _tokenOwnerAddress) {
    ...
    function setTokenContractAddress(address tokenContractAddress) external onlyOwner {
        if (address(tokenContractAddress) != address(0)) {
            tokenContract = AcrocalypseTokenContract(tokenContractAddress);
        }
    }
}