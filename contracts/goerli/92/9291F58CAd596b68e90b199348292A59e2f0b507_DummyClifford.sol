...
contract DummyClifford is ERC721A, Ownable, VRFConsumerBaseV2 {
  ...
  uint256 public constant MAX_SUPPLY = 10; // 6_000;
  ...
  uint private constant AUCTION_LENGTH = 1 weeks; // 5 minutes;

  // Time after auction ends that users can claim their nfts and refunds
  // Also the time cypher holders have to claim their NFT
  uint private constant CLAIM_PERIOD = 1 weeks; // 5 minutes;

  uint private constant TIME_OFFSET = 2 weeks - 2 minutes;  // 0;

  uint private constant BID_EXTENSION_LENGTH = 15 minutes; // 5 minutes;

  uint private constant BID_INCREASE_PERCENT = 10;

  ICypher internal constant CYPHER_CONTRACT = ICypher(0xccCFDb354FA15Dc05335559659B62aaB06Eb12b4);
  ...
  constructor() ERC721A("Clifford", "Cliff") VRFConsumerBaseV2(vrfCoordinator) {
    // _metadata = metadata;
  }
  ...
  // TODO: Remove this function, used for auction testing only
  function resetAuction() public onlyOwner {
    ...
    // delete allBidders;
    // delete cypherClaims;
    numOfCyphersClaimed = 0;

    // restart cypher claim
    // startCypherClaimPeriod();
  }
  ...
  function startCypherClaimPeriod() external onlyOwner {
    // Auction settings
    startedAt = block.timestamp + CLAIM_PERIOD - TIME_OFFSET;
    endAt = block.timestamp + CLAIM_PERIOD + AUCTION_LENGTH - TIME_OFFSET;
	}
