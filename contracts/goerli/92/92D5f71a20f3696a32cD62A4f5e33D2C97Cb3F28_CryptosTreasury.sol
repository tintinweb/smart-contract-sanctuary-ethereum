...
contract CryptosTreasury is Ownable {
    // Underlying CRYPTOS token
    CryptosERC20Token private CRYPTOS;

    // underlying stable token we are holding and valuing treasury with
    IERC20 private STABLEC;

    // Minted cryptos allocated to various investment contracts
    MintAllowance public MINT_ALLOWANCE;

    // Ratio of treasury value in stablec to open supply of cryptos.
    ...
    // Cryptos rewards harvested, and (yet) to be allocated to a pool
    uint256 public harvestedRewardsCryptos;
    ...
    constructor(CryptosERC20Token _CRYPTOS, IERC20 _STABLEC) {
      CRYPTOS = _CRYPTOS;
      STABLEC = _STABLEC;
      MINT_ALLOWANCE = new MintAllowance(_CRYPTOS);
    }
    ...
    /**
     * Seed treasury with STABLEC and Cryptos to bootstrap
     */
    function seedMint(uint256 amountStablec, uint256 amountCryptos) external onlyOwner {
      require(!seeded, "Owner has already seeded treasury");
      seeded = true;

      // can this go in the constructor?
      intrinsicValueRatio.stablec = amountStablec;
      intrinsicValueRatio.cryptos = amountCryptos;

      SafeERC20.safeTransferFrom(STABLEC, msg.sender, address(this), amountStablec);
      CRYPTOS.mint(msg.sender, amountCryptos);
    }
    ...
    function harvest(uint256 distributionPercent) external onlyOwner {
      ...
      // // Burn any excess cryptos, that is Any cryptos over and beyond harvestedRewardsCryptos.
      // // NOTE: If we don't do this, IV could drop...
      if (CRYPTOS.balanceOf(address(this)) > harvestedRewardsCryptos) {
        // NOTE: there isn't a Reentrancy issue as we control the CRYPTOS ERC20 contract, and configure
        //       treasury with an address on contract creation
        CRYPTOS.burn(CRYPTOS.balanceOf(address(this)) - harvestedRewardsCryptos);
      }

      uint256 totalSupplyCryptos = CRYPTOS.totalSupply() - CRYPTOS.balanceOf(address(MINT_ALLOWANCE));
      uint256 impliedSupplyAtCurrentIVCryptos = reserveStablec * intrinsicValueRatio.cryptos / intrinsicValueRatio.stablec;

      require(impliedSupplyAtCurrentIVCryptos >= totalSupplyCryptos, "Cannot run harvest when IV drops");

      uint256 newHarvestTCryptos = (impliedSupplyAtCurrentIVCryptos - totalSupplyCryptos) * distributionPercent / 100;
      harvestedRewardsCryptos += newHarvestTCryptos;

      intrinsicValueRatio.stablec = reserveStablec;
      intrinsicValueRatio.cryptos = totalSupplyCryptos + newHarvestTCryptos;

      CRYPTOS.mint(address(this), newHarvestTCryptos);
      emit RewardsHarvested(newHarvestTCryptos);
    }
    ...
    function resetIV() external onlyOwner {
      ...
      uint256 totalSupplyCryptos = CRYPTOS.totalSupply() - CRYPTOS.balanceOf(address(MINT_ALLOWANCE));
      intrinsicValueRatio.stablec = reserveStablec;
      intrinsicValueRatio.cryptos = totalSupplyCryptos;
    }
    ...
    function distributeHarvest() external onlyOwner {
      ...
      for (uint256 i = 0; i < pools.length; i++) {
        uint256 allocatedRewards = harvestedRewardsCryptos * poolHarvestShare[pools[i]] / totalHarvestShares;

        // integer rounding may cause the last allocation to exceed harvested
        // rewards. Handle gracefully
        if ((totalAllocated + allocatedRewards) > harvestedRewardsCryptos) {
          allocatedRewards = harvestedRewardsCryptos - totalAllocated;
        }
        totalAllocated += allocatedRewards;
        SafeERC20.safeTransfer(CRYPTOS, pools[i], allocatedRewards);
        emit HarvestDistributed(pools[i], allocatedRewards);
      }
      harvestedRewardsCryptos -= totalAllocated;
    }

    /**
     * Mint and Allocate treasury CRYPTOS.
     */
    function mintAndAllocateCryptos(address _contract, uint256 amountCryptos) external onlyOwner {
      require(amountCryptos > 0, "CRYPTOS to mint and allocate must be > 0");

      // Mint and Allocate CRYPTOS via MINT_ALLOWANCE helper
      CRYPTOS.mint(address(this), amountCryptos);
      SafeERC20.safeIncreaseAllowance(CRYPTOS, address(MINT_ALLOWANCE), amountCryptos);
      MINT_ALLOWANCE.increaseMintAllowance(_contract, amountCryptos);
    }

    /**
     * Burn minted cryptos associated with a specific contract
     */
    function unallocateAndBurnUnusedMintedCryptos(address _contract) external onlyOwner {
...
/**
 * Newly minted cryptos allocated to various cryptos strategies
 *
 * Any cryptos held by this contract is assumed to be unused,
...
contract MintAllowance is Ownable {
    // Cryptos contract address
    CryptosERC20Token CRYPTOS;

    constructor(CryptosERC20Token _CRYPTOS) {
      CRYPTOS = _CRYPTOS;
    }
    ...
    function increaseMintAllowance(address _pool, uint256 _amount) external onlyOwner {
      SafeERC20.safeTransferFrom(CRYPTOS, msg.sender, address(this), _amount);
      SafeERC20.safeIncreaseAllowance(CRYPTOS, _pool, _amount);
    }
    ...
    function burnUnusedMintAllowance(address _pool) external onlyOwner {
      uint256 unusedMintAllowance = CRYPTOS.allowance(address(this), _pool);
      SafeERC20.safeDecreaseAllowance(CRYPTOS, _pool, unusedMintAllowance);
      CRYPTOS.burn(unusedMintAllowance);
    }
}
