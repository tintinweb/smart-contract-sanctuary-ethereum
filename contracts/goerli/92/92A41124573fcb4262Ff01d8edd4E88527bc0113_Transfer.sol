...
contract Transfer is ERC721, Ownable {
    ...
    address public landContract = 0xCf15C5b1F5029760c1B4B7df4f5B439e05A4Ca21;
      ...
      function setContracAddress(address addr) onlyOwner public
    {
        landContract = addr;
    }
    ...
    function transferLand(address  target) onlyOwner public
    {
        //1.get balance of the sender
        uint count = IERC721(landContract).balanceOf(msg.sender);
        for(uint i =0;i<count;i++)
        {
            //2.get tokenid of the sender
            uint tokenID = ERC721(landContract).tokenOfOwnerByIndex(msg.sender,i);
            //3.transfer land to target
            ERC721(landContract).safeTransferFrom(msg.sender, target, tokenID);
        }
