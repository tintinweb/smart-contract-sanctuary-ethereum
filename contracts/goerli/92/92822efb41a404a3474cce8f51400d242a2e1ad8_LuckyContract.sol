...
contract LuckyContract is ReentrancyGuard, Ownable {
    ...
    event winner(uint256 indexed round, address winner, uint256 winAmount,uint256 feeRate);
    ...
    function clacWinner(uint256 _len,uint256 _feeRate) external onlyOwner {
        ...
        curInfo.luckyNumber = number.mod(curInfo.participationCounts);
        ...
        emit winner(round, winnerAddress, winAmount,_feeRate);
        round++;
        tokenWei = 10**_len;
    }
    ...
    function participationCount(uint256 _round)
        public
        view
        returns (uint256)
    {
