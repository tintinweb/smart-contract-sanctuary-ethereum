...
contract TokenBank {
    ...
    /// @dev アカウントアドレスごとのToken残高 addressを指定するとuint256の残高数字を返す
    mapping(address => uint256) private _balances;

    /// @dev TokenBankが預かっているToken残高　addressを指定するとuint256の残高数字を返す
    ...
    /// @dev Token預入時のコメントToken残高
    event TokenDeposit(
        address indexed from,
        uint amount
    );

    /// @dev Token引き出し時のコメントToken残高
    event TokenWithdraw(
        address indexed from,
        uint amount
    );
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        if (msg.sender == owner) {
            require(_balances[owner] - _bankTotalDeposit >= amount, "Amount greater than the total cannot be transferred");
        }
        require(to != address(0), "'to' = Zero address!");
        uint256 frombalance = _balances[from];
        
        require(frombalance >= amount, "from zandaka husoku!");
...
/// @dev Tokenを引き出す
    function withdraw(uint256 amount) public onlyMember notOwner {
        ...
        require(toTokenBankBalance >= amount, "An amount greter than your token balance!");
