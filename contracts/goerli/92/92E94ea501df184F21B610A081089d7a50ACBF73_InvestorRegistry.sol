...
*/
contract InvestorRegistry is CertificateHolder {
    ...
    function addInvestorToRegistry(address payable _investorAddress, bool _isKycComplete, bytes32 _cert, string calldata _domicileCountry) external onlyOwner() {
        ...
        createCert(_cert, _investorAddress);
        emit InvestorAddedToRegistry(_investorAddress);
    }

    function isThisRegSEnabledInvestor(address _investorAddress, bytes32 _cert) onlyIfInvestorExistsInRegistry(_investorAddress) onlyOwner public returns (bool){
        require(isUserCertified(_cert, _investorAddress), "User Certificate does not match one in registry");
    ...
    function removeInvestorFromRegistry(address _investorAddress, bytes32 _existingCert) public onlyOwner onlyIfInvestorExistsInRegistry(_investorAddress) {
    ...
    function updateCertificate(address _investorAddress, bytes32 _existingCert, bytes32 _newCert) public onlyOwner onlyIfInvestorExistsInRegistry(_investorAddress) {
    ...
    function isInvestorCertified(bytes32 _cert, address _investorAddress)  view onlyOwner external returns (bool){
        return isUserCertified(_cert, _investorAddress);
    }
