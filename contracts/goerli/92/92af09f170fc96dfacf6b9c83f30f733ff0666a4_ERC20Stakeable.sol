...
// File: contracts/FundingContract.sol
...
contract ERC20Stakeable is Ownable,IERC721Receiver{
    ...
    */
    function showReward() public view returns(uint result)
    {
        if (UserRate[msg.sender].check)
        {
            uint RewardEst = (Details[msg.sender].deposited * UserRate[msg.sender].userRate) * ((block.timestamp - Details[msg.sender].timestake)/60);
        ...
        {
            uint RewardEst = (Details[msg.sender].deposited * rate) * ((block.timestamp - Details[msg.sender].timestake)/60);
            uint TotalReward = RewardEst / 100;
            result = TotalReward + Details[msg.sender].deposited;
            return result;
        }
