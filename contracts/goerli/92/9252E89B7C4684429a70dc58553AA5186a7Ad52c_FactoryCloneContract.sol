...
contract FactoryCloneContract is Ownable {
        ...
        returns (address)
    {
        ...
        require(_params.quorum > 0, "Quorum should be greater than 0");
        require(
            _params.quorum <= 100,
            "Quorum should be less then or equal to 100"
        );

        require(_params.threshold > 0, "Threshold should be greater than 0");
        require(
            _params.threshold <= 100,
            "Threshold should be less then or equal to 100"
        );

        require(_params.depositFunctioningDays > 0, "Days cannot be 0");

        require(
            _params.ownerFeePerDeposit >= 0,
            "Owner fee cannot be less than 0"
        );
        require(
            _params.ownerFeePerDeposit < 100,
            "Owner fee cannot exceed 100"
        );

        require(_params.gnosisAddress != address(0), "Owner cannot be null");

        require(USDCAddress != address(0), "USDC cannot be null");

        if (!_params.isTemplateErc721) {
            require(
                _params.minDepositPerUser > 0,
                "Min deposit should be grater than 0"
            );

            require(
                _params.maxDepositPerUser > 0,
                "Max deposit should be grater than 0"
            );
            require(
                _params.maxDepositPerUser > _params.minDepositPerUser,
                "Amount should be grater than min amount"
            );

            require(
                _params.totalRaiseAmount > _params.maxDepositPerUser,
            ...
            require(
                _params.totalSupplyOfToken > 0,
                "Total supply of tokens can't be zero"
            );

            require(
                _params.maxTokensPerUser > 0,
                "Max tokens per user can't be zero"
            );

            data = abi.encodeWithSignature(
                "initializeERC721(uint256,uint256,uint256,uint256,address,address,address,address[],uint256)",
                _params.ownerFeePerDeposit,
                _params.depositFunctioningDays,
                _params.quorum,
                _params.threshold,
            ...
            address nftContractAddress = address(
                new Nft(
                    _params.DaoName,
                    _params.DaoSymbol,
                    _params.maxTokensPerUser,
                    _params.totalSupplyOfToken,
                    _params.isNftTransferable,
                    _params.isNftTotalSupplyUnlimited,
                    proxy
                )
            );

            ERC20NonTransferable(proxy).setNftAddressFunction(
                nftContractAddress
            );

            Emitter(emitterAddress).createDaoNftTemplate(
                nftContractAddress,
                _params.gnosisAddress,
                proxy,
                _params.DaoName,
    ...
    IERC721Receiver
{
    ...
    bool public isTemplateErc721 = false;
    ...
    uint256 public priceOfNft = 0;

    bool setNftAddress = false;
    ...
    function updateMinMaxDeposit(
        ...
        require(!isTemplateErc721, "Template is not ERC20");
    ...
    function disableTokenGating() external onlyOwner {
    ...
    function deposit(
        ...
        if (isTemplateErc721) {
            require(priceOfNft == _totalAmount, "Amount sent is invalid");
        }
        ...
        if (feeUSDC) {
            ...
            if (isTemplateErc721) {
                mintNftToken(msg.sender, _tokenURI);
            } else {
            ...
            Emitter(emitterContractAddress).newUser(
                ...
                _totalAmount,
                block.timestamp,
                isTemplateErc721 ? 1 : totalGtTokens,
                false
            );
        } else {
    ...
    function setNftAddressFunction(address _nftContractAddress) external {
        require(!setNftAddress, "Nft Contract Address already set");
    ...
    function mintNftToken(address _to, string memory _tokenURI) internal {
        Nft(nftContractAddress).mintNft(_to, _tokenURI);
    }
...
contract Nft is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public maxTokensPerUser = 0;

    uint256 public totalSupplyOfToken = 0;
    ...
    bool public onlyAllowWhitelist = false;

    mapping(uint256 => string) private _tokenURIs;

    bool public isNftTransferable = false;

    bool public isNftTotalSupplyUnlimited = false;

    // uint256 public mintingStartDate = 0;

    // uint256 public mintingStopDate = 0;

    // bool public isMintingTimeBound = false;

    constructor(
        string memory name,
        string memory symbol,
        // uint256 _pricePerToken,
        // bool _isMintingTimeBound
        uint256 _maxTokensPerUser,
        uint256 _totalSupplyOfToken,
        ...
        totalSupplyOfToken = _totalSupplyOfToken;
        // isMintingTimeBound = _isMintingTimeBound;
        implementationAddress = _implementationAddress;
        isNftTransferable = _isNftTransferable;
        isNftTotalSupplyUnlimited = _isNftTotalSupplyUnlimited;
        isAdmin[_implementationAddress] = true;
    ...
    function makeAddressAdmin(address _address) public onlyOwner {
        isAdmin[_address] = true;
    }

    function removeAddressFromAdmin(address _address) public onlyOwner {
        delete isAdmin[_address];
    }

    // function updatePricePerToken(uint256 _pricePerToken) public onlyOwner {
    //     pricePerToken = _pricePerToken;
    // }

    function setOnlyAllowWhitelist() public onlyAdmins {
        onlyAllowWhitelist = true;
    }

    function unsetOnlyAllowWhitelist() public onlyAdmins {
        onlyAllowWhitelist = false;
    }

    function updateMaxTokensPerUser(uint256 _maxTokensPerUser)
        public
        onlyAdmins
    {
        maxTokensPerUser = _maxTokensPerUser;
    ...
    function updateTotalSupplyOfToken(uint256 _totalSupplyOfToken)
        public
        onlyAdmins
    {
        totalSupplyOfToken = _totalSupplyOfToken;
    ...
    function startMinting() public onlyAdmins {
        // if (isMintingTimeBound) {
        //     require(mintingStartDate > 0, "Minting start date not set");
        //     require(mintingStopDate > 0, "Minting stop date not set");
        // }
        isMintingOpen = true;
    }

    function stopMinting() public onlyAdmins {
        isMintingOpen = false;
    }

    function whitelistAddress(address _address) public onlyAdmins {
        require(onlyAllowWhitelist, "Contract does not support whitelisting");
        allowMint[_address] = true;
    }

    function removeWhitelistAddress(address _address) public onlyAdmins {
        delete allowMint[_address];
    }
    ...
    // function setMintingStartDate(uint256 _mintingStartDate) public onlyOwner {
    //     require(isMintingTimeBound, "Minting is not time bound");
    //     require(_mintingStartDate > 0, "Date can be 0");
    //     require(block.timestamp < mintingStartDate, "Date already passed");
    //     mintingStartDate = _mintingStartDate;
    // }

    // function setMintingStopDate(uint256 _mintingStopDate) public onlyOwner {
    //     require(isMintingTimeBound, "Minting is not time bound");
    //     require(_mintingStopDate > 0, "Date can be 0");
    //     require(_mintingStopDate > block.timestamp, "Date has to be in future");
    //     require(_mintingStopDate > mintingStopDate, "Invalid date");
    //     mintingStopDate = _mintingStopDate;
    // }

    // function setIsMintingTimeBound() public onlyOwner {
    //     isMintingTimeBound = true;
    // }

    // function unsetIsMintingTimeBound() public onlyOwner {
    //     isMintingTimeBound = false;
    //     mintingStartDate = 0;
    //     mintingStopDate = 0;
    // }
    ...
    function mintNft(address _to, string memory _tokenURI) public {
        require(
            ...
            "Not allowed to call the function"
        );

        require(isMintingOpen, "Minitng is not open");

        require(
            balanceOf(_to) < maxTokensPerUser,
            "Max tokens for user already minted"
        );

        if (!isNftTotalSupplyUnlimited) {
            require(
                _tokenIdTracker + 1 < totalSupplyOfToken,
                "Max tokens already minted"
            );
        }

        if (onlyAllowWhitelist) {
            require(allowMint[_to], "Not whitelisted");
        }

        // if (isMintingTimeBound) {
        //     require(
        //         block.timestamp > mintingStartDate,
        //         "Miniting not yet started"
        //     );
        //     require(block.timestamp < mintingStopDate, "Miniting is over");
        // }

        _tokenIdTracker += 1;
        _safeMint(_to, _tokenIdTracker);
        _setTokenURI(_tokenIdTracker, _tokenURI);
        tokenOwners[_to].push(_tokenIdTracker);
    }
    ...
    function transferFrom(
        ...
        require(isNftTransferable, "NFT Non Transferable");
