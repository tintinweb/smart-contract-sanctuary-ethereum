...
contract CryptoAxus is
    ...
    EIP712
{
    constructor() ERC1155("") EIP712("CryptoAxus", "1") {}
    ...
    modifier denyContract() {
        require(
            ...
            "CryptoAxus :: Call from contract is not allowed."
        );
        _;
    }

    function recover(LazyVoucher calldata voucher)
        public
        view
        returns (address)
    {
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "LazyVoucher(uint256 tokenId,uint256 quantity,uint256 price,string uri)"
                    ),
                    voucher.tokenId,
                    voucher.quantity,
                    voucher.price,
    ...
    function mint(uint256 id, uint256 amount) public denyContract {
        require(!exists(id), "CryptoAxus::Token alreday exists");

        _mint(msg.sender, id, amount, "");
    }

    function redeem(address redeemer, LazyVoucher calldata voucher)
        external
        payable
    {
        require(
            msg.value >= voucher.price,
            "CryptoAxus::Not enough funds sent to mint"
        );
        require(!exists(voucher.tokenId), "CryptoAxus::Token alreday exists");

        address artist = recover(voucher);
        payable(artist).transfer(voucher.price);
        _mint(artist, voucher.tokenId, voucher.quantity, "");
        _safeTransferFrom(
            artist,
            redeemer,
            voucher.tokenId,
