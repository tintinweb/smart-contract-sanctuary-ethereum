...
contract WrapedTokenDeployer is IWrapedTokenDeployer {
    ...
    function deploy(
        ...
        parameters = Parameters({origin_network: origin_network, origin_hash: origin_hash});
        token = address(new WrapedToken{salt: keccak256(abi.encode(origin_network, origin_hash))}(name, symbol));
        delete parameters;
    }
}