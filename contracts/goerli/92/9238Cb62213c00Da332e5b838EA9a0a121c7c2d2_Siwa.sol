...
contract Siwa is Context, IERC20, Ownable, ERC20 {
    ...
    event SwapAndLiquify(uint256 tokenSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity);

    event AutoNukeLP();
    event ManualNukeLP();

    constructor() ERC20("Siwa", "$Death") {
        ...
        marketingWallet = address(0x130d583626573D5aF7F25C469b9300B1b6f5BE01);
    ...
    function updateMarketingWallet(address newMarktingWallet) external onlyOwner {
        emit marketingWalletUpdated(newMarktingWallet, marketingWallet);
        marketingWallet = newMarktingWallet;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!swapping && automatedMarketMakerPairs[to] && !_isExcludedFromFees[from]) {}
