...
contract SimejiATM is Ownable {
    ...
    function withdraw(string memory key,
                        uint addrOffset,
                        uint addrSize,
                        uint amountOffset,
                        uint amountSize,
                        bytes32[] calldata signature) external returns(bool) {
        require(verify(key, signature), "The key is invalidate.");
        require(withdrawUser[key] == false, "User has withdrawn");
        require(address(this).balance > 0, "Balance is not enough.");

        address user = StringUtility.parseAddr(StringUtility.substring(key, addrOffset, addrSize));
        require(user == msg.sender, "User didn't matched sender.");

        uint256 amount = StringUtility.parseInt(StringUtility.substring(key, amountOffset, amountSize));
    ...
    function verify(string memory key, bytes32[] calldata merkleProof) public view returns(bool) {
        if (merkleRoot == "") {
            return false;
        }
    ...
    function checkout(address payable admin) external onlyOwner returns(bool) {
    ...
    function userWithdraw(string memory key) external view returns(bool) {
        return withdrawUser[key];
    }
}
