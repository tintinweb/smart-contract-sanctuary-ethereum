...
library CompleteRequestIdentifiers {
    ...
    uint256 constant TASKID_MASK = ((uint256(1) << TASKID_BITS) - 1) ^ ASKTIME_MASK;

    function taskId(uint256 _id) internal pure returns (uint256) {
        return _id & TASKID_MASK;
    }
...
contract XYZ is Ownable {
    ...
    event CompleteTaskRequestEvent(address tasker, address principal, uint256 requestId);
    ...
    /**
     * @dev queryTask : only the principal can query their own Task.
    ...
    function queryTask(uint256[] memory _taskIds) external view returns (Task[] memory) {
        ...
        for (uint256 i = 0; i < taskIdsLength; i++) {
            require(allTasks[_taskIds[i]].principal == msg.sender, "only principal can query.");
    ...
    function completeTask(address _principal, uint256 _taskId, uint256 _requestId, string calldata _proofLink) external {
        ...
        Task memory task = allTasks[_requestId.taskId()];
        ...
        emit CompleteTaskRequestEvent(msg.sender, _principal, _requestId);
    }

    /**
     * @dev queryCompleteRequests : only the principal & tasker can query their own requests.
     */
    function queryCompleteRequests(uint256[] calldata _askIds) external view returns (CompleteTaskRequest[] memory returnCompleteTaskRequests) {
        returnCompleteTaskRequests = new CompleteTaskRequest[](_askIds.length);
        for (uint256 i = 0; i < _askIds.length; i++) {
            CompleteTaskRequest memory completeTaskRequest = completeTaskRequests[_askIds[i]];
