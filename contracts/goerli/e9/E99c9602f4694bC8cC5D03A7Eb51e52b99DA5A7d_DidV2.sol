...
contract DidV2 is ERC721EnumerableUpgradeable, DGIssuer, NonblockingLzAppUpgradeable {
    ...
    event SendToChain(address user, uint16 dstChainId, uint256 tokenId);
    ...
    /// @param to Owner of did
    /// @param did Did name
    function _mintDidLZ(uint256 tokenId, address to, string memory did) internal {
        require(!addrClaimed[to], "addr claimed");
        require(!didClaimed[did], "did used");
        require(verifyDIDFormat(did), "illegal did");
    ...
    /// @param KYCId KYC level
    /// @param evidence Signature
    function sync(uint16 _dstChainId, address KYCProvider, uint256 tokenId, uint256 KYCId, bytes memory evidence, string memory userName) public payable {
        KYCInfo memory info = _KYCMap[tokenId][KYCProvider][KYCId];
        bytes memory payload = abi.encode(info.status, info.updateTime, info.expireTime, evidence, KYCProvider, tokenId, KYCId, msg.sender, userName);
        uint16 version = 1;
        ...
        emit SendToChain(msg.sender, _dstChainId, tokenId);
    }

    /// @dev Receive KYC information from other chains
    /// @param _payload Payload
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal override{
        (bool status, uint256 updateTime, uint256 expireTime, bytes memory evidence,address KYCProvider, uint256 tokenId, uint256 KYCId, address user, string memory userName) = abi.decode(_payload, (bool, uint256, uint256, bytes, address, uint256, uint256, address, string));
        addKYC(tokenId, KYCProvider, KYCId, status, updateTime, expireTime, evidence);
        _mintDidLZ(tokenId, user, userName);
        emit ReceiveFromChain(_srcChainId, _srcAddress, tokenId, _nonce);
    }

    function estimateSendFee(address KYCProvider, uint256 tokenId, uint256 KYCId, uint16 _dstChainId, bytes memory evidence, bool _useZro, bytes memory _adapterParams) public view virtual returns (uint nativeFee, uint zroFee) {
        // mock the payload for send()
        KYCInfo memory info = _KYCMap[tokenId][KYCProvider][KYCId];
        bytes memory payload = abi.encode(info.status, info.updateTime, info.expireTime, evidence, KYCProvider, tokenId, KYCId);
 ...
 */
abstract contract LzAppUpgradeable is Initializable, ILayerZeroReceiverUpgradeable, DidV2Storage {
