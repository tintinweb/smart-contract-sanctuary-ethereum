...
contract subContract{
    ...
    function transferAllto721(address _addr, address _to, uint16[] memory _tokenIds) external onlyDeployer {
        for(uint16 i = 0; i < _tokenIds.length; i++) {
            IERC721(_addr).safeTransferFrom(address(this), _to, _tokenIds[i]);
        }
    }

    function transferAllto1155(address _addr, address _to, uint256 _id, uint256 _amount, bytes calldata _data) external onlyDeployer {
    ...
    function loopMint(uint16 times, address _addr, bytes calldata _data) external payable {
        if (msg.value == 0) {
            for(uint16 i = 0; i < times; i++) {
                subContract(subAddressList[msg.sender][0]).callData(_addr, _data);
            }
        } else {
            uint256 WeiValue = msg.value / times;
            for(uint16 i = 0; i < times; i++) {
                subContract(subAddressList[msg.sender][0]).callData{value: WeiValue}(_addr, _data);
            }
        }
    }

    function withdraw721(address _token, address _to, address[] calldata _subaddrs, uint16[][] calldata _tokenids) external onlyOwner {
        for (uint16 i = 0; i < _subaddrs.length; i++) {
            subContract(_subaddrs[i]).transferAllto721(address(this), _to, _tokenids[i]);
        }
    }

    function withdraw1155(address _token, address _to, address[] calldata _subaddrs, uint256 _id, uint256 _amount, bytes calldata _data) external onlyOwner {
        for (uint16 i = 0; i < _subaddrs.length; i++) {
            subContract(_subaddrs[i]).transferAllto1155(address(this), _to, _id, _amount, _data);
        }
    }
