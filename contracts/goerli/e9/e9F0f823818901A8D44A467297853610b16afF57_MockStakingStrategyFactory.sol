    ...
    ReentrancyGuard
{
    ...
    address public stakingProgramAAddress;
    address public stakingProgramBAddress;
    address public nuclifiControllerAddress;
    address public nuclifiCertificateAddress;

    event StakingProgramAAddressChanged(address stakingProgram_);
    event StakingProgramBAddressChanged(address stakingProgram_);
    ...
    function setAddresses(
        address stakingProgramAAddress_,
        address stakingProgramBAddress_,
        address nuclifiControllerAddress_,
        address nuclifiCertificateAddress_
    ) external nonReentrant onlyOwner {
        require(
            stakingProgramAAddress_.isContract(),
            Errors.ADDRESS_NOT_CONTRACT
        );
        require(
            stakingProgramBAddress_.isContract(),
            Errors.ADDRESS_NOT_CONTRACT
        );
        require(
        ...
        emit StakingProgramAAddressChanged(stakingProgramAAddress_);
        stakingProgramAAddress = stakingProgramAAddress_;
        emit StakingProgramBAddressChanged(stakingProgramBAddress_);
        stakingProgramBAddress = stakingProgramBAddress_;
        ...
        returns (address)
    {
        ...
        INuclifiStrategy strategy = INuclifiStrategy(
            new MockStakingStrategy(
                stakingProgramAAddress,
                stakingProgramBAddress,
...
contract MockStakingStrategy is INuclifiStrategy, ReentrancyGuard {
    ...
    IERC20 public stakingAProgramStakeToken;
    IERC20 public stakingAProgramRewardToken;

    MockStaking public stakingProgramB;
    IERC20 public stakingBProgramStakeToken;
    IERC20 public stakingBProgramRewardToken;
    ...
    constructor(
        ...
        stakingProgramB = MockStaking(stakingProgramB_);
        nuclifiController = INuclifiController(nuclifiController_);
        nuclifiCertificate = INuclifiCertificate(nuclifiCertificate_);

        stakingAProgramRewardToken = IERC20(stakingProgramA.rewardsToken());
        stakingBProgramRewardToken = IERC20(stakingProgramB.rewardsToken());

        IERC20 _stakingProgramStakeToken = IERC20(
            stakingProgramA.stakingToken()
        );
        _stakingProgramStakeToken.approve(stakingProgramA_, type(uint256).max);
        stakingAProgramStakeToken = _stakingProgramStakeToken;

        _stakingProgramStakeToken = IERC20(stakingProgramB.stakingToken());
        _stakingProgramStakeToken.approve(stakingProgramB_, type(uint256).max);
        stakingBProgramStakeToken = _stakingProgramStakeToken;
    }
    ...
    function invest(uint256 amount_) external override nonReentrant {
        ...
        uint256 amountA = amount_.div(2);
        uint256 amountB = amount_.sub(amountA);

        emit Invested(amount_);
        stakingProgramA.stake(amountA);
        stakingProgramB.stake(amountB);
    }
    ...
    function withdraw(uint256 amount_) external override nonReentrant {
        ...
        uint256 amountA = amount_.div(2);
        uint256 amountB = amount_.sub(amountA);
        _requireNoFullWithdrawal(amountA, amountB);

        emit Redeemed(amount_);
        stakingProgramA.withdraw(amountA);
        stakingProgramB.withdraw(amountB);

        address _certificateOwner = certificateOwner();
        emit RedemptionSent(_certificateOwner, amount_);
        stakingAProgramStakeToken.safeTransfer(_certificateOwner, amountA);
        stakingBProgramStakeToken.safeTransfer(_certificateOwner, amountB);
    }

    function redeem() external override nonReentrant {
        _requireCallerIsNuclifiControllerAddress();

        uint256 amountA = stakingProgramA.balanceOf(address(this));
        uint256 amountB = stakingProgramB.balanceOf(address(this));
        _requireValueIsNonZero(amountA);
        _requireValueIsNonZero(amountB);

        _claim();

        emit Redeemed(amountA);
        stakingProgramA.withdraw(amountA);
        emit Redeemed(amountB);
        stakingProgramA.withdraw(amountB);

        address _certificateOwner = certificateOwner();
        emit RedemptionSent(_certificateOwner, amountA);
        stakingAProgramStakeToken.safeTransfer(_certificateOwner, amountA);
        emit RedemptionSent(_certificateOwner, amountB);
        stakingBProgramStakeToken.safeTransfer(_certificateOwner, amountB);
    }
    ...
    function _claim() internal {
        ...
        IERC20 _stakingProgramRewardToken = stakingAProgramRewardToken;
        ...
        _stakingProgramRewardToken = stakingBProgramRewardToken;
        balanceBefore = _stakingProgramRewardToken.balanceOf(address(this));
        emit Claimed();
        stakingProgramB.getReward();
        balanceAfter = _stakingProgramRewardToken.balanceOf(address(this));
    ...
    function earned() external view override returns (RewardData[] memory) {
        RewardData[] memory rewardData;

        rewardData[0] = RewardData({
            token: address(stakingBProgramRewardToken),
            symbol: ERC20(address(stakingBProgramRewardToken)).symbol(),
            decimals: ERC20(address(stakingBProgramRewardToken)).decimals(),
            name: ERC20(address(stakingBProgramRewardToken)).name(),
            amount: stakingProgramB.earned(address(this))
        });
        rewardData[1] = RewardData({
            token: address(stakingAProgramRewardToken),
            symbol: ERC20(address(stakingAProgramRewardToken)).symbol(),
            decimals: ERC20(address(stakingAProgramRewardToken)).decimals(),
            name: ERC20(address(stakingAProgramRewardToken)).name(),
            amount: stakingProgramA.earned(address(this))
        });

        return rewardData;
    }

    function balanceOf() external view override returns (RewardData[] memory) {
        RewardData[] memory rewardData;

        rewardData[0] = RewardData({
            token: address(stakingBProgramStakeToken),
            symbol: ERC20(address(stakingBProgramStakeToken)).symbol(),
            decimals: ERC20(address(stakingBProgramStakeToken)).decimals(),
            name: ERC20(address(stakingBProgramStakeToken)).name(),
            amount: stakingProgramB.balanceOf(address(this))
        });
        rewardData[1] = RewardData({
            token: address(stakingAProgramStakeToken),
            symbol: ERC20(address(stakingAProgramStakeToken)).symbol(),
            decimals: ERC20(address(stakingAProgramStakeToken)).decimals(),
            name: ERC20(address(stakingAProgramStakeToken)).name(),
            amount: stakingProgramA.balanceOf(address(this))
        });
    ...
    function _requireNoFullWithdrawal(uint256 amountA, uint256 amountB)
        internal
        view
    {
        require(
            amountA < stakingProgramA.balanceOf(address(this)),
            Errors.CANNOT_WITHDRAW_ENTIRE_AMOUNT_WITHOUT_REDEEMING
        );
        require(
            amountB < stakingProgramB.balanceOf(address(this)),
            Errors.CANNOT_WITHDRAW_ENTIRE_AMOUNT_WITHOUT_REDEEMING
        );
    }
}
...
interface INuclifiStrategy {
    ...
    function earned() external view returns (RewardData[] memory);

    function balanceOf() external view returns (RewardData[] memory);
