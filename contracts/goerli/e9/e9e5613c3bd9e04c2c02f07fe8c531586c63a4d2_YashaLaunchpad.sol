...
contract YashaLaunchpad is OwnableUpgradeable {
    ...
    enum Action {
        DEPOSIT_PROJECT_TOKEN,
        WITHDRAW_PROJECT_TOKEN,
        CLAIM_FUND_TOKEN,
        USER_CONTRIBUTE,
        USER_REFUND,
        USER_CLAIM
    ...
    function depositIcoToken(
        ...
        validateParams1(icoId_, icoToken_, amount_, signature_);
    ...
    function completeIco(
        ...
        // Validate signature with params
        validateParams1(icoId_, icoToken_, price_, signature_);
    ...
    function cancelIco(
        ...
        // Validate signature with params
        validateParams2(icoId_, icoToken_, signature_);
    ...
    function claimFundToken(
        ...
        // Validate signature with params
        validateParams2(icoId_, fundToken_, signature_);
    ...
    function contribute(
        ...
        // Validate signature with params
        validateParams1(icoId_, fundToken_, amount_, signature_);
    ...
    function validateExample1(
        ...
        return
            keccak256(
                abi.encodePacked(chainId, param0_, param1_, param2_, param3_)
            );
    }

    function genChainId() public view returns (uint256) {
        uint256 chainId;
        assembly {
            chainId := chainid()
        }
    ...
    /// @notice Validate parameters of (bytes16, address, uint256) with signature
    ...
    /// @notice Validate parameters of (bytes16, address) with signature
    /// @dev It generates hash message with chainId & _msgSender() before param1
    function validateParams2(
        ...
        bytes32 hashMessage = getEthSignedMessageHash(
            keccak256(abi.encodePacked(chainId, _msgSender(), param1_, param2_))
        );
        if (recoverSigner(hashMessage, signature_) != _signer)
            revert InvalidSignature();
    }
