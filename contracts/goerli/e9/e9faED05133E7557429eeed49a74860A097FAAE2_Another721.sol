...
contract Another721 is ERC721ABv2, ERC721ABErrors {
    ...
    mapping(uint256 => Phase[4]) public phasesPerDrop;
    ...
    IERC721 private immutable AB_PFP;

    // Range used to calculate new token index (allow for a maximum of 10k token per drop)
    uint256 private constant TOKEN_INDEX_RANGE = 10000;
    ...
    ) ERC721ABv2(_dropManager, _name, _symbol) {
    ...
    function mintPfpHolder(
        ...
        if (mintedPerDropPerPhase[drop.dropId][_to][0] + _quantity > maxMint)
            revert MaxMintPerAddress();

        mintedPerDropPerPhase[drop.dropId][_to][0] += _quantity;

        uint256 tokenIndex;
        for (uint256 i = 0; i < _quantity; ++i) {
            tokenIndex = _dropId * TOKEN_INDEX_RANGE + drop.sold + i;
    ...
    function mintSeasonHolder(
        ...
        Phase memory phase = phasesPerDrop[_dropId][1];

        // Check that the first phase has started (revert otherwise)
        if (block.timestamp < phase.phaseStart) revert SaleNotStarted();
        if (block.timestamp > phasesPerDrop[_dropId][2].phaseStart)
            revert PhaseCompleted();
        ...
        // Check that user did not mint the maximum amount per address for the current phase
        if (
            mintedPerDropPerPhase[drop.dropId][_to][1] + _quantity >
            phase.maxMint
        ) revert MaxMintPerAddress();
    ...
    function mintAllowlist(
        ...
        Phase memory phase = phasesPerDrop[_dropId][2];

        // Check that the first phase has started (revert otherwise)
        if (block.timestamp < phase.phaseStart) revert SaleNotStarted();
        if (block.timestamp > phasesPerDrop[_dropId][3].phaseStart)
            revert PhaseCompleted();

        // Check that user did not mint the maximum amount per address for the current phase
        if (
            mintedPerDropPerPhase[drop.dropId][_to][2] + _quantity >
            phase.maxMint
        ) revert MaxMintPerAddress();
    ...
    function mintPublic(
        ...
        Phase memory phase = phasesPerDrop[_dropId][3];
        ...
        // Check that user did not mint the maximum amount per address for the current phase
        if (
            mintedPerDropPerPhase[drop.dropId][_to][3] + _quantity >
            phase.maxMint
        ) revert MaxMintPerAddress();
        ...
        for (uint256 i = 0; i < _quantity; ++i) {
            tokenIndex = uint256(
                keccak256(abi.encodePacked(drop.sold + i, _dropId))
            );
            dropIdPerToken[tokenIndex] = drop.dropId;
            _safeMint(_to, tokenIndex);
        }
     ...
     */
    function setDropPhases(
        uint256 _dropId,
        Phase[4] memory _phases
    ) external onlyOwner {
        phasesPerDrop[_dropId][0] = _phases[0];
        phasesPerDrop[_dropId][1] = _phases[1];
        phasesPerDrop[_dropId][2] = _phases[2];
        phasesPerDrop[_dropId][3] = _phases[3];

        emit UpdatedPhase(_dropId);
    }
