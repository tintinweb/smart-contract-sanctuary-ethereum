...
contract DssProxyActions is Common {
    ...
    address public constant config = 0xe0111370CE5B757c98aeCc3edE8351A88EdfAa60;

    //events
    event Deposit(address user, uint vault, uint amount, uint fee);
    event Withdraw(address user, uint vault, uint amount, uint fee);
    event Repay(address user, uint vault, uint amount, uint fee);
    event Borrow(address user, uint vault, uint amount, uint fee);
    ...
    function approve(address gem, uint amount) internal {
        if(GemLike(gem).allowance(msg.sender, address(this)) < amount) {
                GemLike(gem).approve(address(this), amount);
        }
    }
    ...
    function lockETH(
        ...
        emit Deposit(msg.sender, cdp, msg.value, _fee);
    }
    ...
    function lockGem(
        ...
        emit Deposit(msg.sender, cdp, amt, _fee);
    }
    ...
    function freeETH(
        ...
        emit Withdraw(msg.sender, cdp, wad, _fee);
    }
    ...
    function freeGem(
        ...
        emit Withdraw(msg.sender, cdp, amt, _fee);
    }
    ...
    function draw(
        ...
        if(_fee > 0) {
            // approve(address(USBJoinLike(USBJoin).USB()), _fee);
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _fee);
        }
        emit Borrow(msg.sender, cdp, wad, _fee);
    }
    ...
    function wipe(
        ...
        emit Repay(msg.sender, cdp, wad, _fee);
    }
    ...
    function wipeAll(
        ...
        emit Repay(msg.sender, cdp, art, _fee); 
    }
    ...
    function lockETHAndDraw(
        ...
        emit Deposit(msg.sender, cdp, msg.value, _fee);
        emit Borrow(msg.sender, cdp, wadD, _feeBorrow);
    }
    ...
    function lockGemAndDraw(
        ...
        emit Deposit(msg.sender, cdp, amtC, _fee);
        emit Borrow(msg.sender, cdp,wadD, _feeBorrow);
    }
    ...
    function wipeAndFreeETH(
        ...
        emit Repay(msg.sender, cdp, wadD, _feeRepay);
        emit Withdraw(msg.sender, cdp, wadC, _fee);
    }
    ...
    function wipeAllAndFreeETH(
        ...
        uint _fee = wadC * _caculatedFee / decimal;
        transferFeeWithdrawETH(msg.sender, Config(config).getTreasury(), wadC, _fee);
        emit Repay(msg.sender, cdp, art, _feeRepay);
        emit Withdraw(msg.sender, cdp, wadC, _fee);
    }
    ...
    function wipeAndFreeGem(
        ...
        emit Repay(msg.sender, cdp, amtC, _feeRepay);
        emit Withdraw(msg.sender, cdp, wadC, _fee);
    }
    ...
    function wipeAllAndFreeGem(
        ...
        emit Repay(msg.sender, cdp, art, _feeRepay);
        emit Withdraw(msg.sender, cdp, amtC, _fee);
    }
}
