...
import "./IERC2981.sol"; // royalties standard

contract DEDPRZ is ERC1155DeltaQueryable, IERC2981, OperatorFilterer {
    address public admin;
    address public treasury;

    uint256 mintPrice; // mint price
    uint256 ROYALTIES; // 6.9% royalties

    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a; // royaltyInfo()

    bool updatedURI; // allowed one time
    ...
    function mint(address to, uint256 amount) external payable {
        ...
        require(_totalMinted() + amount <= 10000, "Minted out"); // max supply
        require(msg.value >= mintPrice * amount, "Insufficient funds"); // mint price
        require(balanceOf(to) + amount <= 10, "Limit 10"); // 10 per wallet
