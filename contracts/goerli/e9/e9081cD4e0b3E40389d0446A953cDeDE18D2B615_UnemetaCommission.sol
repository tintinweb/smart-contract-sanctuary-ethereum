...
contract UnemetaCommission is Pausable,ReentrancyGuard ,Ownable{
    ...
    whenNotPaused
    {
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender,_amounts,_id,_treeid));
        require(MerkleProof.verifyCalldata(_proof, _trees, leaf),"verify error ");
        require(_amounts <=getBalance(),"Insufficient balance");
        _claimed[msg.sender][_id] = true;
        (bool success,) = payable(msg.sender).call{value: _amounts}("");
        require(success,"error payable");
        emit Claim(_id,msg.sender,_amounts);
    }
