...
contract Paytusker is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
//custom
    IUniswapV2Router02 public uniswapV2Router;
//string
    string private _name = "Paytusker Token";
...
//address
    ...
    address public _MarketingWalletAddress = 0x756B7775c93C6Fb0ef7207f566402FE1cC87e4Ce; // 2% Buy/Sell
    address public _devWalletAddress = 0x503CF12afE78A998B8C90206EC19e3C70f25afca; // 2% Buy/Sell
...
//uint
    ...
    uint public buyFee = buyReflectionFee + buyDevFee + buyLiqFee + buyMarketingFee;
    uint public sellFee = sellReflectionFee + sellDevFee + sellLiqFee + sellMarketingFee;
    uint public buySecondsLimit = 5;
    uint public maxBuyTxAmount;
    uint public maxSellTxAmount;
    uint public minimumTokensBeforeSwap = 130 * (10**18);
    uint private MarketActiveAt;
    uint private constant MAX = ~uint256(0);
    uint private _tTotal = 1000000 * (10**18);
...
// constructor
    constructor() {
        ...
        uint total_supply = 1000000 * 10 ** decimals();
        // set gvars
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        uniswapV2Router = _uniswapV2Router;
        maxSellTxAmount = total_supply / 200; // 0.5% supply
        maxBuyTxAmount = total_supply / 40; // 2.5% supply
        excludedFromFees[address(this)] = true;
        excludedFromFees[owner()] = true;
        premarketUser[owner()] = true;
        excludedFromFees[_MarketingWalletAddress] = true;
        excludedFromFees[_devWalletAddress] = true;
        //spawn pair
    ...
    function setFees() private {
        buyFee = buyReflectionFee + buyDevFee + buyLiqFee + buyMarketingFee;
        sellFee = sellReflectionFee + sellDevFee + sellLiqFee + sellMarketingFee;
    }
    ...
    function setDevFee(uint buy, uint sell) external onlyOwner() {
        buyDevFee = buy;
        sellDevFee = sell;
        setFees();
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 _tDev, uint256 tMarketing) {
        (tTransferAmount, tFee, tLiquidity, _tDev, tMarketing) = _getTValues(tAmount);
        (rAmount, rTransferAmount, rFee) = _getRValues(tAmount, tFee, tLiquidity, _tDev, tMarketing, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, _tDev, tMarketing);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 _tDev, uint256 tMarketing) {
        tFee = calculateReflectionFee(tAmount);
        tLiquidity = calculateLiquidityFee(tAmount);
        _tDev = calculateDevFee(tAmount);
        tMarketing = calculateMarketingFee(tAmount);
        tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(_tDev).sub(tMarketing);
        return (tTransferAmount, tFee, tLiquidity, _tDev, tMarketing);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 _tDev, uint256 tMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rDev = _tDev.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rDev).sub(rMarketing);
    ...
    function _takeDev(uint256 _tDev) private {
        ...
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(_tDev);
    }
...
//set functions
    ...
    function setDevAddress(address _value) external onlyOwner {
        _devWalletAddress = _value;
    }
...
// mappings functions
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 _tDev, uint256 tMarketing) = _getValues(tAmount);
