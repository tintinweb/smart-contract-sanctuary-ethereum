...
contract LotteryTicketNftFactory {
    ...
    function createLottery(string memory _baseUri, uint256 _ticketPrice, uint256 _lotteryDurationInMinutes) public returns (address deployed) {
        bytes32 salt = keccak256(abi.encodePacked(_baseUri, _ticketPrice, _lotteryDurationInMinutes));
        deployed = Clones.cloneDeterministic(blueprintImplementation, salt);
        new BeaconProxy(
            address(beacon),
            abi.encodeWithSelector(LotteryTicketNft(deployed).initialize.selector, _baseUri, _ticketPrice, _lotteryDurationInMinutes)
        );
...
contract LotteryTicketNft is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(
    ...
    uint256 _lotteryDurationInMinutes
      ...
      require(lotteryEndDate > 0, "Value for 'lotteryEndDate' must be greater than0.");
      baseURI = _baseUri;
