 ...
 */
contract BlockchainNuggets is ERC721Burnable {
    ...
    constructor() ERC721("Blockchain Nuggets Baby", "BNC") {
        passSpecs[Size.Standard] = PassSpec(6000, 1);
        passSpecs[Size.Copper] = PassSpec(1500, 6001);
        passSpecs[Size.Silver] = PassSpec(1000, 7501);
        passSpecs[Size.Gold] = PassSpec(500, 8501);
        passSpecs[Size.Home] = PassSpec(500, 9001);
        passSpecs[Size.Special] = PassSpec(500, 9501);
    }
    ...
    modifier mintComplianceWithWL(Size size) {
        ...
        require(totalSupplyBySize(size) + maxMintPerWL <= pass.maxSupply, "over supply");
        _;
    }
    ...
    function upgrade(Size size, uint256 tokenId) external {
        ...
        require(whitelistBySize[size][msg.sender] > 0, "Address does not exist in the white list");
        require(tokenId < getSpec(size).startingTokenId, "Size too high");

        _burn(tokenId);
        _safeMintSize(size, msg.sender);
        whitelistBySize[size][msg.sender] -= maxMintPerWL;
    }
    ...
    function walletOfOwner(Size size, address _owner) public view returns (uint256[] memory) {
        ...
        while (ownedTokenIndex < ownerTokenCount && currentTokenId < totalSupplyBySize(size) + getSpec(size).startingTokenId) {
