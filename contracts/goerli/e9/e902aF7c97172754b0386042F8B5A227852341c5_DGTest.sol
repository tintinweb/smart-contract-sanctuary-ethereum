...
contract DGTest is ERC721AUpgradeable, DefaultOperatorFiltererUpgradeable,
    ...
    */
    ...
    function checkMintedByPhase(address wallet) internal view returns(uint256 minted){
        if(phase == 1){
            return SNMinted[wallet];
        }else if(phase == 2){
            return DLMinted[wallet];
        }else if(phase == 3){
            return WLMinted[wallet];
        }
    }

    function mint(bytes32[] calldata merkleProof, uint256 allowance, uint256 quantity) external payable{
        require(verify(getRoot(), merkleProof, msg.sender, allowance), "Merkle proof doesn't match");
        require(quantity <= allowance - checkMintedByPhase (msg.sender), "This exceeds max amount for this wallet");
        require(msg.value >= getPrice() * quantity, "Not enough funds sent");
