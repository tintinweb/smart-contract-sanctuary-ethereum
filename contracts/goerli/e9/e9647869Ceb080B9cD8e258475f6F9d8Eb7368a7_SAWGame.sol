...
contract SAWGame is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("SAW Game", "SAW_GAME") {
    }
    ...
    function fiatAllowlistMint(address addr, uint256 quantity) external payable {
        publicMints += quantity;
        _safeMint(addr, quantity);
    }

    // Mint method for ETH minting w/o allowlist
    function mint(address addr, uint256 quantity) external payable {
        require(saleOpen, "Public sale not open");
    ...
    function allowlistMint(bytes32 hash, bytes memory signature, string memory nonce, string memory list, uint256 maxQuantity, uint256 quantity) external payable {
        ...
        // TODO: We can just check the quantity in the /api file, don't need to do it here
