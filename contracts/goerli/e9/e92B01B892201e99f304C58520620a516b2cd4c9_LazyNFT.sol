...
error LazyNFT__InvalidTokenId();
error LazyNFT__MaketplaceError();
...
contract LazyNFT is ERC721URIStorage, EIP712 {
    ...
    function redeem(
        ...
        (bool success /*bytes memory data*/, ) = nftMarketplace.call{value: msg.value}(
            //  function nftVoucherBought(address nftAddress, uint256 tokenId, uint256 price, address seller)
            abi.encodeWithSignature(
                "nftVoucherBought(address,uint256,uint256,address)",
                address(this),
                voucher.tokenId,
                msg.value,
                signer
        ...
        if (!success) {
            revert LazyNFT__MaketplaceError();
        }
