    ...
    UUPSUpgradeable
{
    ...
    /// @notice gets the drop's collection contract address
    /// @param dropId The id of the drop.
    ...
    /// @notice public utility to check if an item is part of a drop
    /// @param dropId The id of the drop.
    ...
    function publishDrop(
        ...
        if (fp.drops[dropId].collection != address(0))
            revert DuplicateDrop(dropId);
        if (start < block.timestamp) revert InvalidDropStart();
        if (numItems == 0) revert ItemsRequired();

        FixedPriceDrop storage newDrop = fp.drops[dropId];
    ...
    /// @param price The price of the drop.
    /// @param start The block timestamp at which the drop starts
    /// @param numItems The list of item id's that will be part of the drop
    ...
    /// @param data The data to pass to the collection contract
    function _mint(
        ...
        if (isSoldOut(mintAuth.dropId)) revert DropSoldOut(mintAuth.dropId);
