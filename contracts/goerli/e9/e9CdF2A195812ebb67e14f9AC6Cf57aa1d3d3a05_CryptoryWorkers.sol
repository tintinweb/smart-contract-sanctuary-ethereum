...
contract CryptoryWorkers is ERC721A, Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 private constant MAX_SUPPLY = 24;
    // uint8 private constant MAX_PUBLIC_MINTABLE_AMOUNT = 3;

    // uint8 private constant WL1_STEP = 0;
    // uint8 private constant WL2_STEP = 1;
    // uint8 private constant PUBLIC_STEP = 2;
    // uint8 private constant STEP_COUNT = 3;

    uint8 private teamReservedAmount = 2;
    uint256 private firstWhitelistReservedAmount = 12;
    ...
    ) ERC721A("Cryptory Workers", "Cryptory Workers") {
        // Commented to test
        // metadataUri = _metadataUri;
        // _setSaleInfo(_saleInfoList);
        metadataUri = "https://marketsaigon.s3.ap-southeast-1.amazonaws.com/CW/metadatas/";
        // _saleInfo.push(SaleInfo(1, 1665116108));
        // _saleInfo.push(SaleInfo(1, 1665116108));
        // _saleInfo.push(SaleInfo(1, 1665116108));
        // _setSaleInfo(_saleInfo);
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        if (!isRevealed) {
            return string(abi.encodePacked(metadataUri, "unreveal"));
        }
    ...
    //function mint(uint8 amount, uint8 step) external payable nonReentrant whenNotPaused isNotContract {
        ...
        // if (step == WL1_STEP) {
        //     require(_totalMinted() + amount <= firstWhitelistReservedAmount, "Sold out in this step");
        //     require(allowedAmounts[msg.sender].raffle - amount >= 0, "Don't have mint authority on the 1st WH");
        //     allowedAmounts[msg.sender].raffle -= amount;
        // } else if (step == WL2_STEP) {
        //     require(allowedAmounts[msg.sender].premint - amount >= 0, "Don't have mint authorityon the 2nd WH");
    ...
    // function setSecondWhitelistAllowedAmount(address[] calldata _addressList, uint8[] calldata _amountList) external onlyOwner {
    //     require(_addressList.length == _amountList.length, "Invalid argument");

    //     for (uint256 i = 0; i < _addressList.length; i++) {
    //         _setAllowedAmount(_addressList[i], _amountList[i], WL2_STEP);
    //     }
    // }

    // function setFirstWhiteListAllowedAmount(address[] calldata _addressList, uint8[] calldata _amountList) external onlyOwner {
    //     require(_addressList.length == _amountList.length, "Invalid argument");

    //     for (uint256 i = 0; i < _addressList.length; i++) {
    //         _setAllowedAmount(_addressList[i], _amountList[i], WL1_STEP);
    //     }
    // }

    // function setFirstWhitelistReservedAmount(uint256 _amount) external onlyOwner {
    //     firstWhitelistReservedAmount = _amount;
    // }
    ...
    //     uint8 amount = uint8(MAX_SUPPLY - _totalMinted());
    //     if (amount > 0) {
    //         return amount;
    //     } 
    //     return 0;
    ...
    //     return _totalMinted() == firstWhitelistReservedAmount;
    // }
