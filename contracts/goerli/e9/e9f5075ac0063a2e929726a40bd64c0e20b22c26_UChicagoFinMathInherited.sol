...
contract ERC20 {
   ...
   constructor()
    ...
    event OwnerAdd(address indexed newOwner);
    event OwnerDelete(address indexed Owner);
    ...
    modifier isAdmin(){
        require(msg.sender == admin,"Caller is not the original owner");
        _;
    }
    ...
    function addOwner(address newOwner) public isOwner {
        ...
        emit OwnerAdd(newOwner);
        emit MessageLog("New Owner added!");
    }


    //delete owners
    function deleteOwner(address existingOwner) public isAdmin {
        require(existingOwner != admin,"The admin should not delete itself as owner!");
        owners[existingOwner] = false;
        emit OwnerDelete(existingOwner);
        emit MessageLog("Owner deleted!");
    }
    ...
    function ownerPayout() public payable isOwner {
...
contract UChicagoFinMathInherited is ERC20 {
    ...
    //fun function: change admin (the original owner)
    function changeAdmin(address newOwner) public isAdmin {
