...
// will be transferred to a governance smart contract once GTEST is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract GTESTSingleStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // Info of each user.
    struct UserInfo {
        uint256 amount;         // How many GTEST tokens the user has provided.
        uint256 rewardDebt;     // Reward debt. See explanation below.
        uint256 rewardLockedUp;  // Reward locked up.
        uint256 lastDepositTime; // when user deposited

        //
        // We do some fancy math here. Basically, any point in time, the amount of GTEST
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTokenPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws GTEST tokens to a pool. Here's what happens:
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint;         // How many allocation points assigned to this pool. GTEST to distribute per block.
        uint256 lastRewardBlock;    // Last block number that GTEST distribution occurs.
        uint256 accTokenPerShare;   // Accumulated GTEST per share, times 1e12. See below.
    ...
    // The GTEST TOKEN!
    IBEP20 public token;
    // GTEST tokens created per block.
    uint256 public tokenPerBlock;
    // Info of each pool.
    PoolInfo[] public poolInfo;
    // Info of each user that stakes GTEST tokens.
    mapping(uint256 => mapping(address => UserInfo)) public userInfo;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when GTEST mining starts.
    ...
    constructor() {
        token = IBEP20(0x19FF1C2dAea681a08AFD5B0f0D8C9D2ec9276ae4);
    ...
    // Update the given pool's GTEST allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending GTEST on frontend.
    ...
    // View function to see if user can harvest GTEST.
    ...
    // Deposit GTEST tokens to Farm for GTEST allocation.
    ...
    // Withdraw GTEST tokens from Farm.
    ...
    function setRewardPerBlock( uint256 _tokenPerBlock) external onlyOwner {
        require(_tokenPerBlock > 1 ether);
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough GTEST.
