...
contract SourceGreeterWithTokenAuthenticated {
  ...
  function xUpdateGreeting(address target, uint32 destinationDomain, string memory newGreeting, uint256 amount, uint256 relayerFee) external payable {
    ...
    connext.xcall{value:relayerFee}(destinationDomain, target, address(token), msg.sender, amount, slippage, callData);
  }
}