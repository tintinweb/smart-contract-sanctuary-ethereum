...
contract Minter {
    ...
    function claimLoop(uint16 times, uint16 startnum, address original_addr, uint256 tokenId, address mint_addr, bytes calldata _data) external payable {
        IERC721(original_addr).safeTransferFrom(owner, subAddressList[msg.sender][startnum], tokenId);
        ...
        IERC721(original_addr).safeTransferFrom(subAddressList[msg.sender][startnum + times - 1], owner, tokenId);
    }
