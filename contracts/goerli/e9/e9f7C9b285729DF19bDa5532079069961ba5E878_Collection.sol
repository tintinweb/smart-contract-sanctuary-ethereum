 ...
 */
contract Collection is ProxyBaseStorage, ERC721, DefaultOperatorFilterer, Ownable,CollectionStorage{
using Strings for uint256;
...
function isWhitelisted(address account, bytes32[] calldata proof, uint256 quantity, uint256 whiteListType) public view returns (bool) {
    ...
    function safeMint(address to, uint256 countNFTs_) internal   returns(uint256 start, uint256 end) {
          start = ++tokenIds;
     end = tokenIds + countNFTs_;
 ...
 */
  function mint(address to_, uint256 countNFTs_) isValid() public returns(uint256, uint256) {
    ...
    (uint256 start, uint256 end) = safeMint(to_, countNFTs_);
  ...
  function claimDeemoNFT() external {
    ...
    (uint256 from, uint256 to) =  safeMint(msg.sender,  limit);
    for (uint256 index = from; index <= to; index++) {
        if(supplyPerRarity[1] <= mintedPerRarity[1] + 1){
          ...
          rand = uint256(keccak256(abi.encode(seed + limit + totalSupply(),seed + index, block.timestamp, msg.sender, owner()))) % 100;
  ...
  function craftDeemo(uint256[] memory tokenIds) public {
    ...
    (uint256 from,) = safeMint(msg.sender, 1);
    tokenRarity[from] = 2;
     mintedPerRarity[3]++;
