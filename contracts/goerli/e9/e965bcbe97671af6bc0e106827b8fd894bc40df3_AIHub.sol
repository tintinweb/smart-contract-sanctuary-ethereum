...
contract AIHubERC20 is ERC20, Ownable {
    uint8 internal _decimals = 9;
    uint256 internal _supply = 900 * 10**6;
    uint256 internal _capFarm = 100 * 10**6;
    uint256 private _farmRewarded;
    mapping(address => bool) public stakingSys;
    
    address public marketingWallet = 0xB568C2ceE72Eb666144fe52BBD8723d552769d7f;
    ...
    uint256 public numTokensCap = 200000 * 10**_decimals;

    modifier onlyStakingSystem() {
        require(stakingSys[_msgSender()], "AI-Hub: caller is not the staking system");
        _;
    }

    constructor(string memory name, string memory symbol) ERC20(name, symbol) { }
    
    function setStakingSystem(address _address, bool _status) public onlyOwner {
        require(stakingSys[_address] != _status, "AI-Hub: already set to current status");
        stakingSys[_address] = _status; 
    }

    function stakeReward(address recipient, uint256 amount) external onlyStakingSystem {
        require(_capFarm > _farmRewarded, "AI-Hub: Over cap farm");
        require(recipient != address(0), "AI-Hub: 0x is not accepted here");
        require(amount > 0, "AI-Hub: not accept 0 value");

        _farmRewarded = _farmRewarded + amount;
        if (_farmRewarded <= _capFarm) {
            _mint(recipient, amount);
        }
        else {
            uint256 availableReward = _farmRewarded - _capFarm;
            _mint(recipient, availableReward);
            _farmRewarded = _capFarm;
        }
    }
}
 ...
 * allowances. See {IERC20-approve}.
 */
contract AIHub is AIHubERC20, ReentrancyGuard {
     ...
     * construction.
     */
    constructor() AIHubERC20("AI-Hub", "aiHub") {
        ...
        if ((from == uniswapV2Pair || to == uniswapV2Pair) && !inSwap) {
            if (from != uniswapV2Pair) {
                if ((_marketingReserves) >= numTokensCap) {
                    ...
                    require(sent, "AI-Hub: Failed to send ETH");
                }
            }
    ...
    function setExcludedFromFee(address _address, bool _status) external onlyOwner {
        require(_isExcludedFromFee[_address] != _status, "AI-Hub: already set to current status");
    ...
    function setMarketingReceiver(address _address) public onlyOwner returns (bool) {
        require(_address != DEAD, "AI-Hub: LP Pair cannot be the Dead wallet!");
        require(_address != address(0), "AI-Hub: LP Pair cannot be the 0 wallet!");
        marketingWallet = _address;
        return true;
    }

    function setTax(uint256 _taxMarketing) public onlyOwner returns (bool) {
        require(_taxMarketing <= 100, "AI-Hub: tax must not be greater than 10%");
        taxMarketing = _taxMarketing;
        return true;
    }

    function setFeeLevel(uint256 _amount) public onlyOwner returns (bool) {
        require(_amount <= (_supply / 100) * 2, "AI-Hub: Cannot tax more than 2% of the supply at once!");
