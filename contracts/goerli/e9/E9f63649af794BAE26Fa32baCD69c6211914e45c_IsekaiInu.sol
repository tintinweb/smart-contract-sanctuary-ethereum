...
contract IsekaiInu is ERC20, Ownable {
    ...
    // TEMP: all transfer calls
    event TempTokenTransfer(address from, address to, uint256 amount);
    ...
    function _transfer(
        ...
        // TEMP
        emit TempTokenTransfer(from, to, amount);
        ...
        if (limitsInEffect) {
            if ((from != owner() && to != owner()) && !inSwapping) {
                ...
                // at launch transfer delay (only on buys)
                if (transferDelayEnabled) {
                    if (
                        ...
                        // TODO: check if below block check makes sense (relax it vs. make it stricter)
                        require(
        ...
        bool canSwap = contractTokenBalance >= _tokensSwapThreshold; // || contractTokenBalance >= _tokensLiqThreshold;
