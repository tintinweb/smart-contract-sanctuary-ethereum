...
contract Generator is IGenerator {
    ...
    function generateSVG(
        ...
        svg = string.concat(svg, _generateFill(_pot));
        if (
    ...
    function _generateFill(uint256 _pot) internal pure returns (string memory) {
        string memory fill;
        if (_pot >= 10 ether) {
            fill = GREEN;
...
interface IRoshambo {
    ...
    event JoinGame(uint256 indexed _gameId, address indexed _player, uint256 indexed _wager);
    event Commit(
    ...
    event Cancel(uint256 indexed _gameId, address indexed _player, uint256 indexed _wager);
    ...
    function joinGame(uint256 _gameId, bytes32 _commitment) external payable;

    function newGame(bytes32 _commitment) external payable returns (uint256);

    function rake() external view returns (uint256);

    function reveal(uint256 _gameId, Choice _choice, string calldata _secret) external;
