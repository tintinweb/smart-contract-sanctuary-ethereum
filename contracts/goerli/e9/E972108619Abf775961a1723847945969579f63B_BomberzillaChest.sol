...
contract BomberzillaChest is Rescueable {
    ...
    function buyCharacterChest(uint256 quantity) external payable {
        if (!freeMintAvailable(_msgSender())) {
            paymentTransfer(msg.sender, characterChestPrice * quantity);
        } else {
            freeMintCount[_msgSender()] += quantity;
        }
    ...
    function setPaymentToken(IBurnable _paymentToken) external onlyOwner {
    ...
    function freeMintAvailable(address _account) public view returns (bool) {
        return freeMintCount[_account] < freeMintLimit;
    }
}
