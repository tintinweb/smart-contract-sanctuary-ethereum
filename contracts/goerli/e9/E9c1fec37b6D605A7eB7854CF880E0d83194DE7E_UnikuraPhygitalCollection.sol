...
library UnikuraPhygitalCollectionStorage {
    struct Layout {
        ...
        address _unikuraMothershipContract;
 ...
 */
contract UnikuraPhygitalCollection is ERC721AUpgradeable, OwnableUpgradeable {
    using UnikuraPhygitalCollectionStorage for UnikuraPhygitalCollectionStorage.Layout;

    /**
     * @dev Emit an event when the URI for the Unikura Mothership Contract address upgraded
     */
    event UnikuraMothershipContract(address newUnikuraMothershipContract);

    /**
     * @dev Emit an event when the URI for when the mint price is set
     */
    event MintPrice(uint256 newMintPrice);

    event SalesAddress(address newSalesAddress);
    ...
    function initialize(
        ...
        address unikuraMothershipContract_,
        ...
        UnikuraPhygitalCollectionStorage
            .layout()
            ._unikuraMothershipContract = unikuraMothershipContract_;
     ...
     * @dev It is overidden if the address is the zero address or `salesAddress`
     ...
     * @dev This function performs an external call to the UnikuraMembership contract and decodes the returned data.
    ...
    function ownsMembershipNFT(address account) public view returns (bool) {
        (
            bool getUnikuraMothershipSuccess,
            bytes memory getUnikuraMothershipData
        ) = getUnikuraMothershipContract().staticcall(
                abi.encodeWithSignature("getUnikuraMembershipContract()")
            );

        require(
            getUnikuraMothershipSuccess,
            "Failed to call getUnikuraMembershipContract() function"
        );

        address unikuraMembershipContract = abi.decode(
            getUnikuraMothershipData,
            (address)
        );

        (
            bool ownsMembershipSuccess,
            bytes memory ownsMembershipData
        ) = unikuraMembershipContract.staticcall(
                abi.encodeWithSignature("ownsMembership(address)", account)
            );

        require(
            ownsMembershipSuccess,
            "Failed to call ownsMembership() function"
        );

        return abi.decode(ownsMembershipData, (bool));
    }
    ...
    /**
     * @dev Transfer tokens from deployer wallet to `to` address (minter).
     *
     */
    function mint(address to, uint256 quantity) public payable {
        ...
        getVelvettFeeRecipient().transfer(totalFee);

        // Mint tokens
        for (uint256 i = 0; i < quantity; i++) {
            _mintTransferFrom(
                to,
                UnikuraPhygitalCollectionStorage.layout()._totalMintedTokens + 1
            );
    ...
    function calculateFee(uint256 mintPrice) public view returns (uint256) {
        address unikuraMothershipContract = UnikuraPhygitalCollectionStorage
            .layout()
            ._unikuraMothershipContract;

        (bool success, bytes memory data) = unikuraMothershipContract
            .staticcall(abi.encodeWithSignature("getFeePercentage()"));

        require(success, "Failed to call getFeePercentage function");

        uint8 feePercentage = abi.decode(data, (uint8));

        return (mintPrice * feePercentage) / 100;
    }

    function getVelvettFeeRecipient() public view returns (address payable) {
        address unikuraMothershipContract = UnikuraPhygitalCollectionStorage
            .layout()
            ._unikuraMothershipContract;

        (bool success, bytes memory data) = unikuraMothershipContract
            .staticcall(abi.encodeWithSignature("getVelvettFeeRecipient()"));

        require(success, "Failed to call getVelvettFeeRecipient function");

        return abi.decode(data, (address));
    }

    /**
     * @dev Transfers `tokenId` from `salesAddress` to `to`.
     *
     * Requirements:
     *
    ...
    /**
     * @notice Sets the burn address to `newUnikuraMothershipContract`.
     * @dev This function can only be called by the contract owner.
     * @param newUnikuraMothershipContract The address to be set as the new unikura Mothership contract.
     */
    function setUnikuraMothershipContract(
        address newUnikuraMothershipContract
    ) external onlyOwner {
        UnikuraPhygitalCollectionStorage
            .layout()
            ._unikuraMothershipContract = newUnikuraMothershipContract;

        // Emit an event with the update.
        emit UnikuraMothershipContract(newUnikuraMothershipContract);
    }

    /**
     * @notice Returns the current unikura Mothership contract address.
     ...
     */
    function getUnikuraMothershipContract() public view returns (address) {
        return
            UnikuraPhygitalCollectionStorage
                .layout()
                ._unikuraMothershipContract;
     ...
     */
    function setSalesAddress(
        address payable newSalesAddress
    ) external onlyOwner {
        UnikuraPhygitalCollectionStorage
            .layout()
            ._salesAddress = newSalesAddress;

        // Emit an event with the update.
        emit SalesAddress(newSalesAddress);
    }
    ...
    function setMintPrice(uint256 newMintPrice) external onlyOwner {
        UnikuraPhygitalCollectionStorage.layout()._mintPrice = newMintPrice;

        // Emit an event with the update.
        emit MintPrice(newMintPrice);
    }
    ...
    function getIsTokenMinted(uint256 tokenId) public view returns (bool) {
        return
    ...
    function setBurnAddress(address newBurnAddress) external onlyOwner {
        UnikuraPhygitalCollectionStorage.layout()._burnAddress = newBurnAddress;
    }
    ...
    function getBurnAddress() public view returns (address) {
        return UnikuraPhygitalCollectionStorage.layout()._burnAddress;
    }
    ...
    function burn(uint256 tokenId) external {
        if (
            msg.sender != UnikuraPhygitalCollectionStorage.layout()._burnAddress
        ) {
