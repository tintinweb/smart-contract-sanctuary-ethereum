...
import "@openzeppelin/contracts/token/common/ERC2981.sol";
import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import "./EphemeryaStack.sol";

contract Ephemerya is
    BaseTokenURI,
    SignerManager,
    ERC2981,
    EphemeryaStack
{
    ...
    constructor(
        ...
        BaseTokenURI("https://gateway.pinata.cloud/ipfs/QmQFF9vWewGxzLksLiRGYqBebibTV8wymZSDL7SXEU7s1b/")
        EphemeryaStack(
            name,
            symbol,
            beneficiary,
    ...
    /**
    @notice Override of Seller.cost() with constant logic.
    ...
    **/
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC2981, EphemeryaStack)
        returns (bool)
    {
...
abstract contract EphemeryaStack is 
Seller,
ERC721ACommon,
AccessControlEnumerable
{
    ...
    // /**
    // @dev tokenId to stacking start time (0 = not stacking).
    //  */
    // mapping(uint256 => uint256) private stackingStarted;

    // /**
    // @dev Cumulative per-token stacking, excluding the current period.
    //  */
    // mapping(uint256 => uint256) private stackingTotal;

    // /**
    // @notice Returns the length of time, in seconds, that the token has
    // been stacked.
    // @dev Stacking is tied to a specific token, not to the owner, so it doesn't
    // reset upon sale.
    // @return stacking Whether the token is currently stacking. MAY be true with
    // zero current stacking if in the same block as stacking began.
    // @return current Zero if not currently stacking, otherwise the length of time
    // since the most recent stacking began.
    // @return total Total period of time for which the token has been stacked across
    // its life, including the current period.
    //  */
    // function stackingPeriod(uint256 tokenId)
    //     external
    //     view
    //     returns (
    //         bool stacking,
    ...
    //     )
    // {
    //     uint256 start = stackingStarted[tokenId];
    //     if (start != 0) {
    //         stacking = true;
    //         current = block.timestamp - start;
    //     }
    //     total = current + stackingTotal[tokenId];
    // }

    /**
    @dev Block transfers while stacking.
     */
    function _beforeTokenTransfers(
        address,
        address,
    ...
    // /**
    // @dev Emitted when a token begins stacking.
    //  */
    // event Stack(uint256 indexed tokenId);

    // /**
    // @dev Emitted when a token stops being stacking; either through standard means or
    // by expulsion.
    //  */
    // event Unstack(uint256 indexed tokenId);

    // /**
    // @dev Emitted when a token is expelled from the stacking.
    //  */
    // event Expelled(uint256 indexed tokenId);

    // /**
    // @notice Whether stacking is currently allowed.
    // @dev If false then stacking is blocked, but unstacking is always allowed.
    //  */
    // bool public stackingOpen = false;

    // /**
    // @notice Toggles the `stackingOpen` flag.
    //  */
    // function setStackingOpen(bool open) external onlyOwner {
    //     stackingOpen = open;
    // }

    // /**
    // @notice Changes the token's stacking status.
    // */
    // function toggleStacking(uint256 tokenId)
    //     internal
    //     onlyApprovedOrOwner(tokenId)
    // {
    //     uint256 start = stackingStarted[tokenId];
    //     if (start == 0) {
    //         require(stackingOpen, "Ephemerya: stacking closed");
    //         stackingStarted[tokenId] = block.timestamp;
    //         emit Stack(tokenId);
    //     } else {
    //         stackingTotal[tokenId] += block.timestamp - start;
    //         stackingStarted[tokenId] = 0;
    //         emit Unstack(tokenId);
    //     }
    // }

    // /**
    // @notice Changes the EphemeryaStack' stacking statuss (what's the plural of status?
    // statii? statuses? status? The plural of sheep is sheep; maybe it's also the
    // plural of status).
    // @dev Changes the EphemeryaStack' stacking sheep (see @notice).
    //  */
    // function toggleStacking(uint256[] calldata tokenIds) external {
    //     uint256 n = tokenIds.length;
    //     for (uint256 i = 0; i < n; ++i) {
    //         toggleStacking(tokenIds[i]);
    //     }
    // }

    // /**
    // @notice Admin-only ability to expel a token from the stacking.
    // @dev As most sales listings use off-chain signatures it's impossible to
    // detect someone who has been stacked and then deliberately undercuts the floor
    ...
    // because stacking would then be all-or-nothing for all of a particular owner's
    // Ephemerya.
    //  */
    // function expelFromStacking(uint256 tokenId) external onlyRole(EXPULSION_ROLE) {
    //     require(stackingStarted[tokenId] != 0, "Ephemerya: not been stacked");
    //     stackingTotal[tokenId] += block.timestamp - stackingStarted[tokenId];
    //     stackingStarted[tokenId] = 0;
    //     emit Unstack(tokenId);
