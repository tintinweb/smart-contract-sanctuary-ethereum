    ...
    ERC721Holder
{
    ...
    function getHashAndRecoveredSigner(
        LibLoan.LoanTerms calldata _terms,
        bytes calldata _signature
    ) external view returns (bytes32 hash, address recoveredSigner) {
        bytes32 termsHash = LibLoan.getLoanTermsHash(_terms);
        hash = _hashTypedDataV4(termsHash);
        recoveredSigner = ECDSA.recover(hash, _signature);
    }
