...
contract xTST is ERC20, Ownable, Pausable, APIConsumer{
  ...
  function _changeBuff(uint256 buff) public onlyOwner {
   ...
   function mint(uint256 amount) public whenNotPaused onlyWhitelist {
    if(msg.sender != owner()) {
       require(amount <= _mintableTokens, "Not enough mintable tokens available");
       uint256 _spend = amount * (price.mul(_buff).div(10**18).div(100) + price);
       require(ERC20(usdc).balanceOf(msg.sender) >= _spend, "Not enough USDC balance");
       ERC20(usdc).transferFrom(msg.sender, address(this), _spend);
 ...
 function isWhitelist(address to) public view returns (bool) {
  ...
  function withdrawUSDC(uint256 _withdrawAmount) public onlyOwner {
      ERC20(usdc).transfer(msg.sender, _withdrawAmount);
  }
}