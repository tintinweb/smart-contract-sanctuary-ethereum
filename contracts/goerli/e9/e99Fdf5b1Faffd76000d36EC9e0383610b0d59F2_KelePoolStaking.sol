...
// Kelepool ETH2.0 staking service
contract KelePoolStaking is Initializable, UUPSUpgradeable {
    // constants
    ...
    address public constant ETH2_DEPOSIT_ADDRESS =
        0xff50ed3d0ec03aC01D4C79aAd74928BFF48a7b2b; // eth2.0 contract address 0x00000000219ab540356cBB839Cbe05303d7705Fa
    ...
    struct System {
        ...
        uint256 fee; // fee 0.05 ETH
        uint256 minimum; // deposit minimum 0.01
        uint256 balance; // deposit amount
        uint256 devfee; // dev fee
    }
    System private _system;
    mapping(address => uint256) public micros;
    mapping(address => uint256) public whales;

    // events
    event OnDeposit(address indexed payer, uint256 amount, uint256 time);
    event OnWithdraw(
        address indexed owner,
        address indexed receiver,
        uint256 amount
    );
    event OnValidatorCreated(
        address indexed owner,
        uint8 role,
        uint256 amount,
        uint256 time,
    ...
    function initialize() public initializer {
        ...
        _system.foundation = msg.sender;
        _system.status = true;
        _system.minimum = 1e16;
    ...
    function createValidator(
        ...
        require(
            ...
            "You should deposit at least one validator"
        );
        require(
            pubkeys.length <= PUBKEY_LENGTH * MAX_VALIDATORS,
            "You can deposit max 100 validators at a time"
        );
        require(
        ...
        require(
            ...
            "Data count don't match"
        );

        // check role
        if (role == 0) {
            // normal
            require(msg.sender == _system.operator, "Error operator");
            require(
                address(this).balance >= DEPOSIT_AMOUNT &&
                    _system.balance >= DEPOSIT_AMOUNT,
                "Invalid balance"
            );
            _system.balance -= DEPOSIT_AMOUNT;
        } else if (role == 1) {
            ...
            require(
                msg.value == amount,
                "Amount is not aligned with pubkeys number"
            );
            _system.devfee += _system.fee * pubkeyCount;
            whales[msg.sender] += DEPOSIT_AMOUNT * pubkeyCount;
        } else {
            require(false, "Invalid staking role");
        }
    ...
    // take out fee
    function takeOutFee(address receiver) public onlyFoundation {
        require(receiver != address(0), "Invalid receiver");
        require(
            _system.devfee > 0 && address(this).balance >= _system.devfee,
            "Invalid fee"
        );
    ...
    modifier onlyFoundation() {
        require(_system.foundation == msg.sender, "Error foundation");
        _;
    }
