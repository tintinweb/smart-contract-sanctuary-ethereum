...
contract SeedSales is Ownable {

    uint256 public constant priceDecimal = 1000;

    IERC721 public immutable cryptoHunters;
    ...
    uint256 public constant rewardPercentage = 17;
    ...
    constructor(
        ...
        cryptoHunters = IERC721(cyptoHuntersAddress_);
    }

    /** 
     * @dev setting seedSalesEnabled variable
     * @param enabled boolean True if enables, False otherwise
    */

    function setSeedSalesEnabled(bool enabled) external onlyOwner {
        seedSalesEnabled = enabled;
    }
    ...
    */
    modifier whenAllowed() {
        require(isAllowed[msg.sender] || cryptoHunters.balanceOf(msg.sender) > 0, "SeedSales: address is not allowed to call this function");
        _;
    }
    ...
    */
    ...
    /**
     * @dev Buy the coins in seed sales Coins supply with USDC coin
    ...
    */
    ...
    function buy(uint256 amount, uint256 splitNFTCount, Coin coinType) external whenAllowed {
        require(seedSalesEnabled, "SeedSales: sale is not enabled");
        require(splitNFTCount <= getMaxSplitCount(amount), "SeedSales: split count exceed it's limit");
        require(amount >= priceLimit, "SeedSales: amount should be greater than price limit ");
    ...
    /**
     * @dev airdrop the coins from seed sales supply, with USDC/BUSD/USDT coin
     * @param amount USDC coins to be transferred
     * @param splitNFTCount nft count
     * @param coinType type of coin: USDC, USDT, BUSD
    */
