...
contract MetaBrain is IMetaBrain, IERC721ReceiverUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function blah(uint teamSize, uint anftTokenId, uint playerId) external view returns (bytes memory) {
        return abi.encodePacked(epochRandNums[anftTokenId], playerId);
    }
    ...
    function _fightBoss(
        ...
        uint randNum = brainOracle.getRandFightNum(anftTokenId);
    ...
    function newBoss(
        ...
        epochRandNums[anftTokenId] = brainOracle.getRandFightNum(anftTokenId);
    ...
    function sendToRandomPlayer(
        ...
        uint rand = brainOracle.getRandFightNum(anftTokenId);
