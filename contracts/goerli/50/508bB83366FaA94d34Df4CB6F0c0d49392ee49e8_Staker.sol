...
contract Staker {
  ...
  function stake() public payable {
    require(timeLeft() != 0, 'No staking after deadline!');
  ...
  function execute() external notCompleted {
    require(block.timestamp > deadline, "Deadline has not been passed yet!");
    
    // If deadline passed, we check if threshold met, if so send to external,
    // else allow everyone to withdraw
    if(address(this).balance >= threshold) {
      console.log("calling example contract , threshold met");
      exampleExternalContract.complete{value: address(this).balance}();
    } else {
      console.log("now open fo rwithdraw, threshold not met");
  ...
  function withdraw() external notCompleted {
    require(openForWithdraw, "Not available for withdraw");
