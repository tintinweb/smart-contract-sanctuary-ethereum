...
contract Vault is Ownable {
    ...
    WhitelistedToken[] whitelistedTokens;

    function whitelist(address _token) external onlyOwner {
        whitelistedTokens.push(WhitelistedToken(_token, ERC20(_token).name(), ERC20(_token).symbol()));
    }

    function getWhitelistedAddresses() public view returns( WhitelistedToken [] memory){
        return whitelistedTokens;
    }

    function isWhitelisted(address _tokenAddress) internal view returns(bool) {
        bool tokenIsWhistelisted = false;
        for (uint i = 0 ; i < whitelistedTokens.length ; i++ ) {
            if (whitelistedTokens[i].tokenAddress == _tokenAddress) {
                tokenIsWhistelisted = true;
            }
        }
        return tokenIsWhistelisted;
    }

    function depositTokens(uint256 _amount, address _tokenAddress) external {
        require(isWhitelisted(_tokenAddress), "Is not whitelisted");
    ...
    function withdrawTokens(uint256 _amount, address _tokenAddress) external {
        require(_amount <= Balances[msg.sender][_tokenAddress], 'Not enough funds');
    ...
    function getWalletBalance(address _walletAddress, address _tokenAddress) public view onlyOwner returns(uint256) {
