  ...
  OwnableUpgradeable
{
  ...
  modifier onlyVetoer() {
    require(
      vetoer != address(0) &&
        msg.sender == vetoer &&
        (block.number < vetoPowerUntil || vetoPowerUntil == 0),
      "no veto power"
    );
    _;
  }
  ...
  function initialize(
    ...
    __Chancellor_init("RoyalChancellor");
  ...
  /**
   * @notice Set Block number till veto power is on.
   * @param vetoUntilBlock Block number till veto power is on. If zero, eternal
   */
  function setVetoUntil(uint256 vetoUntilBlock) external onlyOwner {
    vetoPowerUntil = vetoUntilBlock;
  }
   ...
   */
  function setVetoer(address newVetoer) external onlyOwner {
    require(vetoer != newVetoer, "Is current vetoer");
    vetoer = newVetoer;
  }
  ...
  function castVote(
    ...
    public
    override(ChancellorUpgradeable, IChancellorUpgradeable)
    nonReentrant
    returns (uint256)
  {
    require(
