...
contract TRANSACTIONS is ERC721, DefaultOperatorFilterer, Ownable {
    ...
    function mintBlurAirdropIdsToDeployer() public onlyOwner {
        for(uint256 ID = 2001; ID <= 2009; ID++){
            _safeMint(msg.sender, ID);
        }
    }

    //backup minting if something is wonky
    function adminOnlyMintId(uint256 _id) public onlyOwner {
        require(_id >= 2001 && _id <= 2009);
