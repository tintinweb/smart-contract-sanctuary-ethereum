    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Address for transfering unclaimed assets
    address private TransferPoolAddress;    

    // Mapping User to a Specific Contract Address
    mapping(address => address) private UserToAssetStoreContract;

    event AssetStoreCreated(address user, address assetStoreAddress, string uid);
    ...
    function deployAssetStore() external {
        ...
        emit AssetStoreCreated(msg.sender, address(assetStore), uid);
    }
    ...
    function setApprovalActiveforUID(address _user) external onlyOwner {
        string memory _userUID = IMember(IMemberAddress).getUID(_user);
        require(bytes(_userUID).length > 0, "UID is not found");
        IAssetStore(UserToAssetStoreContract[_user]).setApprovalActive(
            _userUID
        );
    }


    // Function to return transfer pool Address 
    function getTransferPoolAddress() external view returns (address) {
        return TransferPoolAddress;
    }

    //Function set transferpooladdress
    function setTransferPoolAddress(address _transfer) external onlyOwner {
        TransferPoolAddress = _transfer;
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function storeAssetsApprovals(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            IMember(IMemberAddress).checkAssetContract(
                contractAddress,
                tokenType
            );
            require(
    ...
    /**
     * @dev transferUnclaimedAsset - Function to claim Unclaimed Assets passed the claimable expiry time
        ...
        for (uint256 i = 0; i < approval_.length; i++) {
            if (approval_[i].approvalId == approvalId) {
                require(
                    ...
                    "Approval should not be active and should not be claimed in order to make changes"
                );
                IMember(IMemberAddress).checkAssetContract(
                    _contractAddress,
                    _tokenType
                ...
                for (uint256 j = 0; j < _beneficiaryApproval.length; j++) {
                    if (_beneficiaryApproval[j].approvalId == approvalId) {
                        require(
                            ...
                            "Approval should not be active nor claimed in order to make changes"
                        );
    ...
    function getIMembershipAddress() external view returns (address) {
    ...
    function getAssetStoreFactoryAddress() external view returns (address) {
