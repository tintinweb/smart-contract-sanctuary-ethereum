...
// import "./chainlink/v2/VRFV2WrapperConsumerBase.sol";
import "@chainlink/contracts/src/v0.8/interfaces/LinkTokenInterface.sol";
import "@chainlink/contracts/src/v0.8/interfaces/VRFCoordinatorV2Interface.sol";
import "@chainlink/contracts/src/v0.8/VRFConsumerBaseV2.sol";


contract Curve is VRFConsumerBaseV2 {
    ...
    uint32 constant NUM_WORDS = 1; // number of words for VRF
    ...
    constructor(
        ...
        address _link,
        uint32 _callbackGasLimit, // 500 000
    ...
    function setChainlinkParams(uint32 _callbackGasLimit) public {
        callbackGasLimit = _callbackGasLimit;
    }
    ...
    */
        ...
        returns (uint256 _requestId)
    {
        ...
        // _requestId = requestRandomness(callbackGasLimit, requestConfirmations, NUM_WORDS);
        _requestId = COORDINATOR.requestRandomWords(KEY_HASH, sSubscriptionId, requestConfirmations, callbackGasLimit, NUM_WORDS);
        nftsCount++;

        emit RequestSent(_requestId, NUM_WORDS);
        ...
        if (requests[requestId].isMint) {
            ...
            if (isRare(tokenId)) {
                ...
                if (
                    ...
                    burnPrice = reserve;
                    gameEnded = true;
                    emit Lottery(tokenId, randomness[0], true, burnPrice);
                }
            }

            nft.burn(requests[requestId]._address, tokenId);
            nftsCount--;
            emit Lottery(tokenId, randomness[0], false, burnPrice);
    ...
    function burn(uint256 tokenId) external virtual NftInitialized {
        // uint256 _requestId = requestRandomness(callbackGasLimit, requestConfirmations, NUM_WORDS);
        uint256 _requestId = COORDINATOR.requestRandomWords(KEY_HASH, sSubscriptionId, requestConfirmations, callbackGasLimit, NUM_WORDS);
    ...
    function setSubId(uint64 _sSubscriptionId) public {
        sSubscriptionId = _sSubscriptionId;
    }

    /// @dev CHAINLINK VRF HELPER FUNCTIONS
    ...
    /// @dev CHAINLINK VRF HELPER FUNCTIONS
    /// @dev increase LINK tokens in order to call a random
    /// @param amount numbers of LINK tokens // 1000000000000000000 = 1 LINK
    ...
    /// @param receivingWallet address where the money will be returned after canceling the subscription.

    function cancelSubscription(address receivingWallet) external {
        require(msg.sender == admin, "Unauthorized");
        uint64 _sSubscriptionId = sSubscriptionId;
        sSubscriptionId = 0;
        COORDINATOR.cancelSubscription(_sSubscriptionId, receivingWallet);
    }
