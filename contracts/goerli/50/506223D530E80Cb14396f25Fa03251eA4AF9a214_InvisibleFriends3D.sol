...
contract InvisibleFriends3D is ERC721A, ERC2981, Ownable {
  ...
  mapping(uint256 => bool) private _alreadyMintedIFIDs;
  mapping(uint256 => bytes32) private _lists;
  mapping(address => uint256) private _alreadyListMinted;
  ...
  function setListRoot(uint256 listNumber, bytes32 root) external onlyOwner {
    _lists[listNumber] = root;
  }
  ...
  modifier verifyListExists(uint256 listNumber) {
    if (_lists[listNumber] == "") revert UnknownListError();
    _;
  }

  // Minting

  function alreadyMintedIFIDs(uint256[] calldata tokenId) public view returns (bool[] memory) {
    bool[] memory states = new bool[](tokenId.length);
    for (uint256 i = 0; i < tokenId.length; i++) {
      states[i] = _alreadyMintedIFIDs[tokenId[i]];
    }
    return states;
  }

  function alreadyListMinted(address account) public view returns (uint256) {
    return _alreadyListMinted[account];
  }
  ...
  function mintForInvisibleFriends(
    ...
    external
    payable
    verifySaleState(SaleState.Owners)
    verifyAmount(originalIds.length)
    verifyAvailableSupply(originalIds.length)
  {
    _checkOwnershipAndAvailability(originalIds, _msgSender());
    ...
    verifyAvailableSupply(originalIds.length)
  {
    ...
    _checkOwnershipAndAvailability(originalIds, vault);
    _mint(_msgSender(), originalIds.length);
  }
  ...
  function mintListed(
    ...
    external
    payable
    verifySaleState(SaleState.Listed)
    verifyAmount(amount)
    verifyAvailableSupply(amount)
    verifyListExists(listNumber)
  {
    if (!_verifyMerkleProof(listNumber, merkleProof, _msgSender(), maxAmount)) revert InvalidProofError();
    if (amount > maxAmount - _alreadyListMinted[_msgSender()]) revert InsufficientListAmountError();

    _alreadyListMinted[_msgSender()] += amount;
  ...
  function _checkOwnershipAndAvailability(uint256[] calldata originalIds, address proposedOwner) private {
    uint256 tokenId;
    for (uint256 i = 0; i < originalIds.length; i++) {
      ...
      if (_alreadyMintedIFIDs[tokenId]) revert TokenIdAlreadyMintedError(tokenId);

      _alreadyMintedIFIDs[tokenId] = true;
    }
  }
  ...
  function _verifyMerkleProof(
    ...
    return MerkleProof.verify(merkleProof, _lists[listNumber], leaf);
  }
