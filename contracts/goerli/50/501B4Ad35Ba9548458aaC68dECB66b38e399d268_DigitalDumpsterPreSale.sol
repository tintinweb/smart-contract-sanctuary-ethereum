...
function claimTokens() public nonReentrant icoNotActive returns (bool) {
    ...
    uint256 totalTokensForSale = 14000000 * (10**18); // Total 14,000,000 tokens for Pre-Sale
    ...
    if (claimedTokens[msg.sender] == 0) {
        tokensToClaim = tokensRemaining.mul(50).div(100); // first time claiming, allow to claim 50%
    } else {
        tokensToClaim = tokensRemaining.mul(10).div(100); // subsequent claims, allow to claim 10%
    }
...
function totalAllocation(address user) public view returns (uint256) {
    ...
    return userContribution.mul(_weiRaised).div(10 ** _tokenDecimals);
}

function unclaimedTokens(address user) public view returns (uint256) {
    uint256 userContribution = _contributions[user];
    uint256 userShare = userContribution.mul(_weiRaised).div(10 ** _tokenDecimals);
    uint256 claimed = claimedTokens[user];
    return userShare.sub(claimed);
}
