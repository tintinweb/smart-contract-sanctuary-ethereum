...
/// It's encoded as a fixed point number with a scaling factor of `2^64`.
/// In other words the amount is multiplied by `2^64` and can have fractions.
...
library DripsConfigImpl {
    ...
    /// It must be encoded as a fixed point number with a scaling factor of `2^64`.
    /// In other words the passed value must be multiplied by `2^64` and can have fractions.
...
library Drips {
    ...
    function _addDelta(
        ...
        unchecked {
            ...
            int256 amtPerCycle = (amtPerSec * int256(cycleSecs)) / (1 << 64);
            int256 amtSplitCycle = (amtPerSec * int256(timestamp % cycleSecs)) / (1 << 64);
            thisCycleDelta += amtPerCycle - amtSplitCycle;
            nextCycleDelta += amtSplitCycle;
            require(
    ...
    function _drippedAmt(
        ...
        // The entire function costs 139 gas per execution as of Solidity 0.8.13,
        // but when rewritten in unchecked Solidity it costed 396 gas.
        // solhint-disable-next-line no-inline-assembly
        assembly {
            let endedCycles := sub(div(end, cycleSecs), div(start, cycleSecs))
            let amtPerCycle := shr(64, mul(amtPerSec, cycleSecs))
            amt := mul(endedCycles, amtPerCycle)
            let amtEnd := shr(64, mul(amtPerSec, mod(end, cycleSecs)))
            amt := add(amt, amtEnd)
            let amtStart := shr(64, mul(amtPerSec, mod(start, cycleSecs)))
