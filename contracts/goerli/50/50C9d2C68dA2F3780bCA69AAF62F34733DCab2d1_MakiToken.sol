...
contract MakiToken is ERC20Permit, VaultOwned {

    using SafeMath for uint256;
    address public dao;
    address private _headChef;
    uint256 public buyFee;
    uint256 public sellFee;
    mapping(address => bool) public pair;
    mapping(address => bool) public tasteApproved;
    bool private tasteVerificationRequired = true;

    event TasteVerification(address trialChef);

    constructor(address _dao, address headChef) ERC20("uramaki test", "MEST", 9) {
      dao = _dao;
      _headChef = headChef;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
      ...
      if(pair[msg.sender]) {
        if (tasteVerificationRequired)
          require(tasteApproved[recipient], "Taste verification not passed yet.");
        fee = amount.mul(buyFee).div(10000);
      }
      if(fee > 0)
        _transfer(msg.sender, dao, fee);
      _transfer(msg.sender, recipient, amount.sub(fee));
      return true;
    }
    ...
    function setFee(uint256 _buy, uint256 _sell) public onlyOwner {
      require(buyFee <= 10000 && sellFee <= 10000, "invalid fee value");
      buyFee = _buy;
      sellFee = _sell;
    }

    function setPair(address _pair, bool _en) public onlyOwner {
      pair[_pair] = _en;
    }

    function setTasteVerificationRequired(bool required) public onlyOwner {
        tasteVerificationRequired = required;
    }

    function tasteVerification(uint8 _v, bytes32 _r, bytes32 _s) public {
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 hash = keccak256(abi.encodePacked(address(this), msg.sender, "TASTING..."));
        bytes32 prefixedHash = keccak256(abi.encodePacked(prefix, hash));
        address recovered = ecrecover(prefixedHash, _v, _r, _s);

        if (recovered == _headChef) {
            tasteApproved[msg.sender] = true;
            emit TasteVerification(msg.sender);
        } else {
            revert("Taste verification not passed.");
        }
    }
}