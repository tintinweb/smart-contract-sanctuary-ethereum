...
contract Diamonds is ERC721x("CoDiamonds", unicode"💎"), Merkle {
	...
	IAggregatorV3Interface immutable public EUR_FEED_USD = IAggregatorV3Interface(0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e);
	uint256 immutable public MAX_MINTABLE;
	address immutable public PASS;
	uint256 immutable public PASS_COOLDOWN = 3600 * 24;
	...
	uint256[] public diamondStagesCounts;


	mapping(uint256 => uint256) public carbonBitMap;
	mapping(uint256 => uint256) public diamondStages;
	mapping(uint256 => uint256) public stagePrice;
	mapping(uint256 => uint256) public passCD;
	...
	function pushNewStage(uint256 _price) external onlyOwner {
	...
	// 	if (!publicMint) revert("!public");
	// 	if (counter + 1 > MAX_MINTABLE) revert("!supply");
	// 	_mint(msg.sender, ++counter);
	// }

	function mintWithProof(uint256 _index, uint256 _prio, bytes32[] calldata _proof) external {
	...
	function buyCarbonAtoms(uint256 _tokenId, uint256 _amount, uint256 _passId) external payable {
		if (ownerOf(_tokenId) != msg.sender) revert NotOwnerOfNft();
		...
		if (lastStage < 3 && lastStage + _amount >= 3)
			passId = _tryMintPass();
		if (lastStage + _amount >= 3) {
			if (IERC721(PASS).ownerOf(_passId) != msg.sender) revert("!pass owner");
			if (block.timestamp - passCD[_passId] < PASS_COOLDOWN) revert("cd");
			passCD[_passId] = block.timestamp;
		}
		diamondStages[_tokenId] += _amount;
		if (lastStage > 0)
			diamondStagesCounts[lastStage]--;
		diamondStagesCounts[lastStage + _amount]++;
	...
	function getEURPriceInEth(uint256 _price) public view returns(uint256) {
		(,int256 ethPrice,,,) = ETH_FEED_USD.latestRoundData(); // 1_870_9924_2260
		(,int256 eurPrice,,,) = EUR_FEED_USD.latestRoundData();
		return _price * uint256(eurPrice) * 1e18 / uint256(ethPrice);
	}

	function _tryMintPass() internal returns(uint256 passId) {
		if (passMinted < 700) {
			passId = ++passMinted;
			GenesisPass(PASS).mint(msg.sender, 77 + passId);
		}
	}

	// TODO have gaspacho's format for uri, needs more work
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
		...
		uint256 weight = diamondStages[tokenId];
        return bytes(baseURI).length > 0 ?
			string(abi.encodePacked(baseURI, bytes('?id='), tokenId.toString(), bytes('&size='), weight.toString())) : "";
    }
