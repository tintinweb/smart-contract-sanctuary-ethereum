...
interface claimLogic {
    function registerForRoyalty(address _hodler, uint256 _amount) external;
    function updateHolder(address _hodler, uint256 _amount) external;
    function reduceHoldings(address _hodler, uint256 _amount) external;
}

contract ChookyInu is ERC20, Ownable {
    using Address for address payable;

    IRouter public router;
    address public pair;
    address public referralContract = 0xe62431e240A8d036E74906E0b8D5a2b30229B653;
    ...
    uint256 public maxSellLimit = 21_000 * 10**18;
    ...
    address public marketingWallet = 0xb172C5bF1c12B0ecDFb04192C40095a095cdEf43;
    ...
    Taxes public sellTaxes = Taxes(6, 0, 4);

    mapping(address => bool) public exemptFee;
    mapping(address => bool) public impatientChooky;
    mapping(address => bool) public existingHolder;

    //Anti Dump
    mapping(address => uint256) private _lastSell;
    bool public coolDownEnabled = true;
    bool public referralActive = true;
    ...
    constructor() ERC20("Chooky Inu", "CHOOKY") {
        ...
        exemptFee[referralContract] = true;
    ...
    function _transfer(
        ...
        require(!impatientChooky[sender] && !impatientChooky[recipient], "Snipers not allowed!");
        ...
        if(sender == pair && referralActive) {
            referralLogic(referralContract).referralBuy(sender, amount);
        }

        //Register buyer as a holder into royalty contract and update his balance when holder buys more or sells
        if(recipient == msg.sender && !existingHolder[recipient]) {
            claimLogic(claimContract).registerForRoyalty(sender, amount);
        }
        else if (recipient == msg.sender && existingHolder[recipient]){
            claimLogic(claimContract).updateHolder(sender, amount);
        }
        else if (sender == msg.sender) {
            claimLogic(claimContract).reduceHoldings(sender, amount);
        }
    ...
    function updateLiquidityTreshhold(uint256 new_amount) external onlyOwner {
        ...
        require(new_amount <= 210_000 && new_amount > 0, "Swap threshold amount should be lower or euqal to 1% of tokens");
    ...
    function SetBuyTaxes(
        ...
        require((_marketing + _liquidity +  _burn) <= 25, "Must keep fees at 25% or less");
    }
    ...
    function updateClaimContract(address _newClaimContract) external onlyOwner {
        require(_newClaimContract != address(0),"Fee Address cannot be zero address");
    ...
    function bulkAddImpatientChookies(address[] memory accounts, bool state) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            impatientChooky[accounts[i]] = state;
        }
    }
