...
contract NonTradeable721A is Ownable, ReentrancyGuard, ERC721A {
    ...
    constructor() payable ERC721A("NonTradeable721A", "NT721A") {
        maxSupply = 10000;
        maxPerWallet = 5;
    ...
    /*/////////////////////////////
        Mint Functions
    /*/
    function makeTokenTransferable() external onlyOwner {
        isTransferable = true;
    }
    ...
    function internalMint(uint256 _amount) internal {
        ...
        if (totalSupply() + _amount > maxSupply)
            revert("Not enough supply remaining");
        if (mintsToWallet[msg.sender] + _amount > maxPerWallet)
            revert("Amount exceeds max per wallet");
    ...
    {
        if (!_exists(_tokenId)) revert("Token does not exist");
    ...
    function safeTransferFrom(
        ...
        if(!isTransferable) revert TokenNotTransferable();
