...
//    uint256 public feeETH = 0.001 ether;
    address public usdt;
    mapping (address => uint256) private rewardsByReferral;
    ...
    function mintByUSDT(
        ...
        if (_referral1 != address(0)) {
            rewardsByReferral[_referral1] += (fee * _amount) / 10;
        }
        if (_referral2 != address(0)) {
            rewardsByReferral[_referral2] += (fee * _amount) / 20;
        }
    }
    ...
    function setIsClaimable(bool _isClaimable) public onlyAdmin {
        isClaimable = _isClaimable;
    }

    function checkRewards(address _user) public view returns (uint256) {
        return rewardsByReferral[_user];
    }

    function claimRewards() public {
        require(isClaimable, 'Claim is not enabled for now');
        require(rewardsByReferral[_msgSender()] > 0, 'No rewards to be claimed');
        uint256 balance = rewardsByReferral[_msgSender()];
        rewardsByReferral[_msgSender()] = 0;
