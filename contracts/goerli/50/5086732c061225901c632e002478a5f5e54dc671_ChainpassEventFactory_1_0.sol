...
import "./ChainpassTicketsProxy.sol";
...
contract ChainpassEventFactory_1_0 is MetaTxMixin, ReentrancyGuard {
    struct EventInfo {
        bool exists;
        uint32 customChainpassFee;

        // internal used for overriding the customBPS
        uint32 eventHostProposedFee;
        uint32 chainpassProposedFee;
    }

    event EventCreated(address eventHost, address ticketContractAddress);
    ...
    function createEvent(address eventHost) external nonReentrant {
        ...
        address chainpassTickets = address(new ChainpassTicketsProxy_1_0(
            registry().getTicketsBaseContractAddress(),
            abi.encodeWithSignature("initialize(address,address)", chainpassRegistry, eventHost)
        ));
        ...
        eventInfoByTicketContractAddress[chainpassTickets] = eventInfo;
        eventTicketContractAddresses.push(chainpassTickets);

        IOwnable(chainpassTickets).transferOwnership(eventHost);

        emit EventCreated(eventHost, chainpassTickets);
    }
    ...
    function globalRegistry() private view returns (IChainpassRegistry) {
        return IChainpassRegistry(chainpassRegistry);
    }

    /*
     *  Handling Chainpass fee.
     */
    function getChainpassFee(address ticketContractAddress) public view returns (uint256) {
        EventInfo memory eventInfo = getEventInfo(ticketContractAddress);
        if (eventInfo.customChainpassFee != 0) {
            return eventInfo.customChainpassFee;
        }

        return globalRegistry().getChainpassDefaultFee();
    }

    function proposeFee(address ticketContractAddress, uint32 feeBps) external {
        require(feeBps < 10_000, "BPS must be less than 10000");

        EventInfo memory eventInfo = getEventInfo(ticketContractAddress);
        if (_msgSender() == chainpassWallet()) { // Chainpass is proposing a fee.
            if (eventInfo.eventHostProposedFee != 0 && eventInfo.eventHostProposedFee == feeBps) { // Event host has already approved this fee.
                overrideChainpassPrimaryBPS(ticketContractAddress, feeBps);
            
            } else { // Simply just record this proposal
                eventInfoByTicketContractAddress[ticketContractAddress].chainpassProposedFee = feeBps;
            }
            
            return;
        }
        
        IOwnable ticketContract = IOwnable(ticketContractAddress);
        if (_msgSender() == ticketContract.owner()) { // The event host is proposing a fee.
            if (eventInfo.chainpassProposedFee != 0  && eventInfo.chainpassProposedFee == feeBps) { // Chainpass has already approved this fee.
                overrideChainpassPrimaryBPS(ticketContractAddress, feeBps);
            
            } else { // Simply just record this proposal
                eventInfoByTicketContractAddress[ticketContractAddress].eventHostProposedFee = feeBps;
            }
            
            return;
        }

        revert("Only Chainpass or the event host can propose a new primary fee.");
    }

    function overrideChainpassPrimaryBPS(address ticketContractAddress, uint32 feeBps) private {
        eventInfoByTicketContractAddress[ticketContractAddress].customChainpassFee = feeBps;
        eventInfoByTicketContractAddress[ticketContractAddress].chainpassProposedFee = 0;
        eventInfoByTicketContractAddress[ticketContractAddress].eventHostProposedFee = 0;
    }
    ...
    function chainpassWallet() private view returns (address) {
        return globalRegistry().getChainpassWallet();
    }
...
/*
 *  ChainpassTicketsProxy V1.0
 */
contract ChainpassTicketsProxy_1_0 is ERC1967Proxy {
