...
import "./StringsBytes32.sol";

contract Blitkin is ERC721{
    ...
    struct ScrambleParents {
        uint256 compositionId;
        uint256 paletteId;
    }

    mapping(uint256 => ScrambleParents) private _tokenParentIndex;
    ...
    constructor() ERC721("Blitkin", "BTK") {
        _owner = msg.sender;
    }
     ...
     function addInscriptions(
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < newInscriptions.length; ++i) {
                inscriptions[count + i] = newInscriptions[i];
            }
            // update count
            inscriptionsCount = count + newInscriptions.length;
        }
    }
    ...
    function mintScramble(uint256 compositionId, uint256 paletteId) public payable {
        //require(msg.value == 0.05 ether, "Not enough eth");
        require(inscriptions[compositionId].originalBlitmapId != paletteId, "Trying to mint original");
        require(compositionId < inscriptionsCount && blitmap.tokenIsOriginal(paletteId),  "Must combine originals");

        // a given pair can only be minted once
        bytes32 pairHash = keccak256(abi.encodePacked(compositionId, '-', paletteId));
        require(_tokenPairs[pairHash] == false);

        uint256 scrambleTokenId = _tokenId;

        _tokenId++;

        ScrambleParents memory parents;
        parents.compositionId = compositionId;
        parents.paletteId = paletteId;

        _tokenParentIndex[scrambleTokenId] = parents;
        _tokenPairs[pairHash] = true;
        _safeMint(msg.sender, scrambleTokenId);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        _requireMinted(tokenId);
        uint256 inscriptionId = _tokenParentIndex[tokenId].compositionId;
        Inscription memory inscription = inscriptions[inscriptionId];
        uint256 blitmapPaletteId = _tokenParentIndex[tokenId].paletteId;
