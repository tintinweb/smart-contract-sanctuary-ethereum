...
contract SerumMintHelper is Ownable, ReentrancyGuard, ERC1155Holder {
  Dogc public immutable dogc;
  DogeSerum public immutable serum;
  ...
  bool public m1MintStateV2 = false;
  bool public m2MintStateV2 = false;

  bool public m1PublicMintStateV2 = false;
  bool public m2PublicMintStateV2 = false;
  uint256 public publicMintPrice = 0.18 ether;

  mapping(address => bool) public m1Claimed;

  function balance(uint256 id) public view returns (uint256) {
    return serum.balanceOf(address(this), id);
  }

  function withdrawSerum(address to, uint256 id) external onlyOwner {
    serum.safeTransferFrom(
      address(this),
      to,
      id,
      balance(id),
  ...
  function mintM1BatchV2(uint256 _amount, bytes32[] calldata _merkleProof)
    external
    payable
    nonReentrant
  {
    require(m1MintStateV2, "Exclusive mint haven't start");
    require(balance(serum.M1_SERUM()) > 0, "Insufficient remains");
    require(msg.value >= serum.price() * _amount, "Insufficient payment");
    require(!m1Claimed[msg.sender], "Already claimed");

    bytes32 leaf = keccak256(abi.encodePacked(msg.sender, _amount));

    if (MerkleProof.verify(_merkleProof, merkleRoot, leaf)) {
      m1Claimed[msg.sender] = true;
      serum.safeTransferFrom(
        address(this),
        msg.sender,
        serum.M1_SERUM(),
  ...
  function flipM1MintStateV2() external onlyOwner {
    m1MintStateV2 = !m1MintStateV2;
  }

  function flipM2MintStateV2() external onlyOwner {
    m2MintStateV2 = !m2MintStateV2;
  }

  function flipM1PublicMintStateV2() external onlyOwner {
    m1PublicMintStateV2 = !m1PublicMintStateV2;
  }

  function flipM2PublicMintStateV2() external onlyOwner {
    m2PublicMintStateV2 = !m2PublicMintStateV2;
  }

  function mintM2BatchV2(uint256 _amount, bytes32[] calldata _merkleProof)
    external
    payable
    nonReentrant
  {
    require(m2MintStateV2, "Exclusive mint haven't start");
    require(balance(serum.M2_SERUM()) > 0, "Insufficient remains");
    require(msg.value >= serum.price() * _amount, "Insufficient payment");
  ...
  function m1PublicMintV2(uint256 _amount) external payable nonReentrant {
    require(m1PublicMintStateV2, "Public mint haven't started");
    require(balance(serum.M1_SERUM()) > 0, "Insufficient remains");

    require(msg.value >= publicMintPrice * _amount, "Insufficient payment");
  ...
  function m2PublicMintV2(uint256 _amount) external payable nonReentrant {
    require(m2PublicMintStateV2, "Public mint haven't started");
    require(balance(serum.M2_SERUM()) > 0, "Insufficient remains");
