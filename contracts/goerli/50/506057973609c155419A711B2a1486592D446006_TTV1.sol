    ...
    UUPSUpgradeable
{
        ...
        public initializer
    {
        ...
        _whitelistPrice = whitelistPrice_;
    ...
    function addWhitelist(WhitelistRequest calldata whitelist) external onlyOwner {
        _addWhitelist(whitelist);
    }

    function addWhitelistBatch(WhitelistRequest[] calldata whitelistArr) external onlyOwner {
        _addWhitelistBatch(whitelistArr);
    }
...
abstract contract WhitelistVoucherUpgradeable is Initializable, ContextUpgradeable {
     ...
     * @param whitelist The address and amount to add in whitelist.
     */
    event AddWhiteList(WhitelistRequest whitelist);
     ...
     * @param whitelist The address to add in whitelist.
     */
    function _addWhitelist(WhitelistRequest calldata whitelist) internal virtual {
        require(whitelist.account != address(0), "Whitelist: address is tho zero address");

        _whitelist[whitelist.account] = _whitelist[whitelist.account].add(whitelist.amount);
        emit AddWhiteList(whitelist);
    }

    /**
     * @dev add a batch of addresses in whitelist.
     * @param whitelistArr a batch of addresses to add in whitelist.
     */
    function _addWhitelistBatch(WhitelistRequest[] calldata whitelistArr) internal virtual {
        for (uint256 i = 0; i < whitelistArr.length ;i++) {
            require(whitelistArr[i].account != address(0), "Whitelist: address is tho zero address");

            _whitelist[whitelistArr[i].account] = _whitelist[whitelistArr[i].account].add(whitelistArr[i].amount);
            emit AddWhiteList(whitelistArr[i]);
        }
    }
