...
contract BSKR_VX is BaseBSKR {
    ...
    function __BSKR_VX_init(
        ...
        _excludeFromRfi(_msgSender()); // Use this function to exclude any account from Rfi
    ...
    function _excludeFromRfi(address account) private {
        if (_rBalances[account] > 0) {
            require(
                _rBalances[account] <= _rTotal,
                "Amount must be less than total reflections"
            );
            _balances[account] = _rBalances[account] / _getRate();
        }
