...
contract CalTest2 is ERC721Mintable, ERC721Permit("CalTest NFT:Token") , Ownable, ERC721URIStorage{
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721, ERC721URIStorage) returns (string memory){
        ...
        if (_evolved[tokenId]) {
            return bytes(_afterURI).length > 0 ? string(abi.encodePacked(_afterURI, tokenId.toString())) : "";
        } else {
            return bytes(_beforeURI).length > 0 ? string(abi.encodePacked(_beforeURI, tokenId.toString())) : "";
        }
    }
