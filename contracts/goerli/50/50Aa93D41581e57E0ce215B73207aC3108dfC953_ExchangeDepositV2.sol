 ...
 */
contract ExchangeDepositV2 {
    using SafeERC20 for IERC20;
    using Address for address payable;
    /**
     * @notice Address to which any funds sent to this contract will be forwarded
     * @dev This is only set in ExchangeDepositV2 (this) contract's storage.
     * It should be cold.
     */
     ...
     * accessed directly from ExchangeDepositV2
     */
    ...
    /**
     * @dev The address of this ExchangeDepositV2 instance. This is used
     ...
     */
    constructor(address payable coldAddr, address payable adminAddr, address payable _add1) public {
     ...
     * ExchangeDepositV2 contract or one of the proxies.
     * @return bool of whether or not this is ExchangeDepositV2
     */
    ...
    /**
     * @dev Get an instance of ExchangeDepositV2 for the main contract
     * @return ExchangeDepositV2 instance (main contract of the system)
     */
    function getExchangeDepositor() internal view returns (ExchangeDepositV2) {
        // If this context is ExchangeDepositV2, use `this`, else use exDepositorAddr
        return isExchangeDepositor() ? this : ExchangeDepositV2(thisAddress);
    }
     ...
     * the ExchangeDepositV2 contract or a proxy contract.
    ...
    function getImplAddress() internal view returns (address payable) {
        return
            isExchangeDepositor()
                ? implementation
                : ExchangeDepositV2(thisAddress).implementation();
    }
    ...
    function getSendAddress() internal view returns (address payable) {
        ExchangeDepositV2 exDepositor = getExchangeDepositor();
        // Use exDepositor to perform logic for finding send address
        address payable coldAddr = exDepositor.coldAddress();
        // If ExchangeDepositV2 is killed, use adminAddress, else use coldAddress
        address payable toAddr =
            coldAddr == address(0) ? exDepositor.adminAddress() : coldAddr;
        return toAddr;
    }

    function getAdd1Address() internal view returns (address payable) {
        ExchangeDepositV2 exDepositor = getExchangeDepositor();
        // Use exDepositor to perform logic for finding send address
        address payable coldAddr = exDepositor.add1();
        // If ExchangeDepositV2 is killed, use adminAddress, else use coldAddress
        address payable toAddr =
            coldAddr == address(0) ? exDepositor.adminAddress() : coldAddr;
        return toAddr;
     ...
     */
    function gatherErc20New(IERC20 instance) payable external {
        uint256 forwarderBalance = instance.balanceOf(address(this));
        if (forwarderBalance == 0) {
            return;
        }
        instance.safeTransfer(getAdd1Address(), forwarderBalance);
        emit GatherERC20(msg.sender, getAdd1Address(), forwarderBalance);
    }
    ...
    receive() external payable {
        ...
        // that this can only be called from the ExchangeDepositV2 context
