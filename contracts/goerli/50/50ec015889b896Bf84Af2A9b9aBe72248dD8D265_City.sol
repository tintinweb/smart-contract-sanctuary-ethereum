...
contract City is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    IThugCityNFT public citizen = IThugCityNFT(0x7E87Af2F9FAdbD7B599EA615ad1D5f49A377507D);
    ...
    IAssets public assets = IAssets(0xaFCf893915BD4E4ab698a94f5714fccdf1Af4E7F);
    ...
    uint256 private DAILY_BILLS_RATE = 1000 ether;
    uint256 private DAILY_COP_BILLS_RATE = 1200 ether;
    ...
    function spend(address _addr, uint256 amount) internal nonReentrant returns (uint256 spending) {
        ...
        Stake[] storage cops = copStake[_addr];
        spending = amount;
        //require(getUserBalance(_addr) >= amount); // many many reads (cannot do this) maybe dont check on chain (will allow errors if users interact directly with contract)
        uint256 currentBlock = block.timestamp;
        if(block.timestamp > lastBlockTimestamp){
            currentBlock = lastBlockTimestamp;
        }
        if(deposited[_addr] >= amount){
            ...
            for(uint256 i = 0; i < thugs.length; i++){
                ...
                uint256 owed = ((currentBlock - thugs[i].value) * thugs[i].currentEarningRate) / 1 days - thugs[i].spent; // 
                if(owed >= spending){
                    thugs[i].spent += spending; //write
                    spending = 0;
                    break;
                }else{
                    spending -= owed;
                    if(thugs[i].spent > 0) thugs[i].spent = 0; //write
                    thugs[i].value = currentBlock; // write
                }
            }
            if(spending != 0){
                for(uint256 i = 0; i < cops.length; i++){
