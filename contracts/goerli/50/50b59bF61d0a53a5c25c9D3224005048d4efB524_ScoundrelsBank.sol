...
/// @title Scoundrels Bank
/// @author aceplxx (https://twitter.com/aceplxx)

contract ScoundrelsBank is IERC721Receiver, FxBaseRootTunnel, Ownable {

    /* ========== STORAGE ========== */

    /// @notice ERC721 instance of the Scoundrels contract.
    ERC721 public scoundrelsContract;

    struct UserInfo {
        uint256[] stakedScoundrels;
        mapping(address => uint256) stakedWhitelistCount;
        mapping(address => uint256[]) wlTokenIds;
    }
    ...
    constructor(
        address _scoundrelsContract,
        address checkpointManager,
        address fxRoot
    ) FxBaseRootTunnel(checkpointManager, fxRoot) {
        scoundrelsContract = ERC721(_scoundrelsContract);
    }
    ...
    event StakeScoundrels(address user, uint256 tokenId, bool stake);
    event StakeWhitelist(address user, address wlContract, uint256 tokenId, bool stake);

    //@dev Emitted when `contract` is whitelisted/N.
    event ContractWhitelist(address wlContract, bool wl);


    /* ========== OWNER FUNCTIONS ========== */

    /// @notice Set the contract addresses for all contract instances.
    /// @param _contract The contract address of Scoundrels.
    function setScoundrelsContract(address _contract) external onlyOwner{
        scoundrelsContract = ERC721(_contract);
    }
    ...
    function balanceOf(address owner) external view returns (uint256) {
        uint[] memory scoundrels = userInfo[owner].stakedScoundrels;
        if(scoundrels.length == 0) return 0;
        return 1;
    }

    function whitelistContract(address _contract, uint256 _baseReward ) external onlyOwner{
        if(_baseReward > 0){
    ...
    function getStakedScoundrels(address user)
        public
        view
        returns (
            uint256[] memory scoundrels
        )
    {
        UserInfo storage info = userInfo[user];
        return (
            info.stakedScoundrels
        );
    }

    function getStakedWhitelist(address _user, address _contract)
        public
        view
        returns (
            uint256[] memory stakedWhitelist
        )
    {
        UserInfo storage info = userInfo[_user];
        return (
            info.wlTokenIds[_contract]
        );
    }

    /* ========== PUBLIC MUTATIVE ========== */

    function stakeScoundrels(uint256[] memory _tokenIds) public notPaused {
        UserInfo storage info = userInfo[msg.sender];

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(scoundrelsContract.ownerOf(_tokenIds[i]) == msg.sender, "Not owner");
            info.stakedScoundrels.push(_tokenIds[i]);
            scoundrelsContract.transferFrom(
                msg.sender,
                address(this),
                _tokenIds[i]
            );
            emit StakeScoundrels(msg.sender, _tokenIds[i], true);
        }
        _sendMessageToChild(
            abi.encode(
                msg.sender,
                address(scoundrelsContract),
                _tokenIds.length,
                true
            )
        );
    ...
    function unstakeScoundrels(uint256[] memory _tokenIds) public notPaused {
        UserInfo storage info = userInfo[msg.sender];

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 index = _verifyOwnership(info.stakedScoundrels, _tokenIds[i]);
            _clearIndex(info, index);
            scoundrelsContract.transferFrom(
                address(this),
                msg.sender,
                _tokenIds[i]
            );
            emit StakeScoundrels(msg.sender, _tokenIds[i], false);
        }
        _sendMessageToChild(
            abi.encode(
                msg.sender,
    ...
    function stakeWhitelists(address _contract, uint256[] memory _tokenIds) external notPaused{
        require(contractWhitelist[_contract], "Not whitelisted");

        UserInfo storage info = userInfo[msg.sender];

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(IERC721(_contract).ownerOf(_tokenIds[i]) == msg.sender, "Not owner");
            info.stakedWhitelistCount[_contract]++;
            info.wlTokenIds[_contract].push(_tokenIds[i]);
            IERC721(_contract).transferFrom(
                msg.sender,
                address(this),
                _tokenIds[i]
            );
            emit StakeWhitelist(msg.sender, _contract, _tokenIds[i], true);
        }
        _sendMessageToChild(
            abi.encode(
                msg.sender,
    ...
    function unstakeWhitelists(address _contract, uint256[] memory _tokenIds) external notPaused{
        UserInfo storage info = userInfo[msg.sender];

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 index = _verifyOwnership(info.wlTokenIds[_contract], _tokenIds[i]);
            _clearWlIndex(info, _contract, index);
            IERC721(_contract).transferFrom(
                address(this),
                msg.sender,
                _tokenIds[i]
            );
            emit StakeWhitelist(msg.sender, _contract, _tokenIds[i], false);
        }
        _sendMessageToChild(
            abi.encode(
                msg.sender,
    ...
    function _verifyOwnership(uint256[] memory _stakedArr, uint256 _tokenId) internal pure returns (uint256){
        uint256 _index = 0;
        while (_stakedArr[_index] != _tokenId) {
            _index++;
            require(_index < _stakedArr.length, "index out of bound");
        }
        return _index;
    }

    function _clearIndex(UserInfo storage _info, uint256 _index) internal {
        if (_info.stakedScoundrels.length > 1) {
            for (uint256 i = _index; i < _info.stakedScoundrels.length - 1; i++) {
                _info.stakedScoundrels[i] = _info.stakedScoundrels[i + 1];
            }
        }
        _info.stakedScoundrels.pop();
    }

    function _clearWlIndex(UserInfo storage _info, address _contract, uint256 _index) internal {
        if (_info.wlTokenIds[_contract].length > 1) {
            for (uint256 i = _index; i < _info.wlTokenIds[_contract].length - 1; i++) {
                _info.wlTokenIds[_contract][i] = _info.wlTokenIds[_contract][i + 1];
            }
        }
        _info.wlTokenIds[_contract].pop();
    }
