    ...
    UUPSUpgradeable
{
    ...
    uint256 internal MAX_MINT_BATCH_SIZE;
    uint256 internal MAX_WHITELIST_MINTS;
    uint256 internal MINT_COST_PER_NFT;
    ...
    error ExceededMaxQuantityPerMint();
    ...
    event UpdateMintStatus(bool _mintActive);
    event UpdateWhitelistMerkleRoot(bytes32 merkleRoot);
    ...
    function initialize(
        ...
        __ERC721A_init(_contractName, _contractSymbol); // (_contractName, _contractSymbol) from param
        __Ownable_init();
        __ERC2981_init();
        __ReentrancyGuard_init();
        MAX_MINT_BATCH_SIZE = 20;
        MAX_WHITELIST_MINTS = 1;
        MINT_COST_PER_NFT = 0.01 ether;
        _whitelistMerkleRoot = "placeholderhash";
        _publicMintActive = false;
        CANONICAL_OPERATOR_FILTER_REGISTRY_ADDRESS = 0x000000000000AAeB6D7670E522A718067333cd4E;
        CANONICAL_CORI_SUBSCRIPTION = 0x3cc6CddA760b79bAfa08dF41ECFA224f810dCeB6;
    ...
    function mint(uint256 quantity) external payable callerIsUser nonReentrant {
        if (!_publicMintActive) revert PublicMintNotActive();
        if (quantity > MAX_MINT_BATCH_SIZE) revert ExceededMaxQuantityPerMint();
        if (msg.value < quantity * MINT_COST_PER_NFT)
    ...
    function whitelistMint(
        ...
        if (!verifyAddressSigner(signature)) revert SignatureValidationFailed();
        if (_getAux(msg.sender) >= MAX_WHITELIST_MINTS)
            revert MaxWhitelistMintsReached();
    ...
    function setBaseURI(string calldata baseURI) external onlyOwner {
        ...
        emit BatchMetadataUpdate(_startTokenId(), _totalMinted() - 1);
    }

    function setMintPrice(uint256 _price) external onlyOwner {
        MINT_COST_PER_NFT = _price;
        emit UpdateMintPrice(_price);
    }

    function setMintStatus(bool _mintActive) external onlyOwner {
        // may not be internal variable
        _publicMintActive = _mintActive;
        emit UpdateMintStatus(_mintActive);
    }

    function setWhitelistMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        _whitelistMerkleRoot = merkleRoot;
        emit UpdateWhitelistMerkleRoot(merkleRoot);
    }

    // NOTE: feeDenominator is 100000 so for 5% royalties, feeNumerator should be 500
    function setRoyalty(
        address receiver,
        uint96 feeNumerator
    ) external onlyOwner {
    ...
    function supportsInterface(
        ...
        returns (bool)
    {
        // relook at this. pretty sure there's more things that needs to be here
        return
            super.supportsInterface(interfaceId) ||
            ERC721AUpgradeable.supportsInterface(interfaceId);
    }

    // Operator Filterer Override
...
//  struct SoulmatesWhiteListData {
//         address SOULMATE_HOLDER;
//         uint256 ALLOWED_V1_MINTS;
//         uint256 ALLOWED_V2_MINTS;
//     }

//     struct SoulmatesHolder {
//         uint256 ALLOWED_V1_MINTS;
//         uint256 CLAIMED_V1_MINTS;
//         uint256 ALLOWED_V2_MINTS;
//         uint256 CLAIMED_V2_MINTS;
//     }
//     mapping(address => SoulmatesHolder) whitelistInfo;

//  function updateSoulmatesWhitelist(
//         SoulmatesWhiteListData[] calldata soulmateHolderList
//     ) external onlyOwner {
//         for (uint i = 0; i < soulmateHolderList.length; ++i) {
//             whitelistInfo[
//                 soulmateHolderList[i].SOULMATE_HOLDER
//             ] = SoulmatesHolder(soulmateHolderList[i].ALLOWED_V1_MINTS, 0,soulmateHolderList[i].ALLOWED_V2_MINTS, 0 );
//             soulmatesWhitelist.push(soulmateHolderList[i].SOULMATE_HOLDER);
//         }
//     }        

//     function getWhitelistInfo(address calldata walletAddress ) public view returns (SoulmatesHolder memory) {
//         return whitelistInfo[walletAddress]
//     }
