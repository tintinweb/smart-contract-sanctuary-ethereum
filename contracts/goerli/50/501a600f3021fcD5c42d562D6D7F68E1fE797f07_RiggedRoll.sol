...
contract DiceGame {
    ...
    function rollTheDice() public payable {
        ...
        // console.log("\t", "   Dice Game previous hash:", uint256(prevHash));
        bytes32 hash = keccak256(
            abi.encodePacked(prevHash, address(this), nonce)
        );
        // console.log("\t", "   Dice Game current hash:", uint256(hash));
...
contract RiggedRoll is Ownable {
    ...
    function riggedRoll() public payable {
        require(address(this).balance >= 0.002 ether, "Must send 0.002 ETH");

        bytes32 prevHash = blockhash(block.number - 1);
        // console.log("\t", "   Current block number:", block.number);

        // console.log("\t", "   RiggedHash previous hash:", uint256(prevHash));
        bytes32 hash = keccak256(
            abi.encodePacked(prevHash, address(diceGame), nonce)
        );
        // console.log("\t", "   Rigged Hash current hash:", uint256(hash));

        uint256 roll = uint256(hash) % 16;

        console.log("\t", "   Predicted result:", roll);
