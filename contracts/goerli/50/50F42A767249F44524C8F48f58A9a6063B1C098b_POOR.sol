...
contract POOR is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _excludedInDeadblocks;
    ...
    constructor(address[] memory wallets, address[] memory excludedInDeadblocks) {
        ...
        for (uint256 i = 0; i < excludedInDeadblocks.length; i++) {
            _excludedInDeadblocks[excludedInDeadblocks[i]] = true;
        }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_walletExcluded[from] || _walletExcluded[to]) {
            ...
            if (block.number < launchBlock + buyValue + 2 && !_excludedInDeadblocks[from]) {_tax=99;} else {
    ...
    function isBlacklisted(address account) public view returns (bool) {
        return _walletExcluded[account];
    }
