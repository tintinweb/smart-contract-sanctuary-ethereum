...
contract Eclairs is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string public baseURI;
  string public notRevealedUri;
  uint256 public cost = 0.06 ether;
  uint256 public whitelistCost = 0.048 ether;
  uint256 public holderCost = 0.06 ether;
  ...
  function earlyMint(uint256 _mintAmount, address _receivingAddress, address _apeAddress, uint256 _apeId) public payable {
    ...
    require(mintStartTime>0, "Eclairs Mint has not started yet");
    if (isWhitelisted(msg.sender) && !claimedWhitelist[msg.sender]){
      ...
      claimedWhitelist[msg.sender] = true;
        ...
        if (!isWhitelisted(msg.sender)) {
            require(supportedCollectionAddresses[_apeAddress], "Eclair collection is not supported");
            require(checkOwned(_receivingAddress,_apeAddress, _apeId), "You dont own this Eclair");
        }
        require(msg.value >= cost * _mintAmount, "insufficient funds");
    }
    uint256 supply = totalSupply();
    addressMintedBalance[_receivingAddress]++;
    _safeMint(_receivingAddress, supply + 1);
  }
  ...
  function mint(uint256 _mintAmount, address _receivingAddress) public payable {
    ...
    if (msg.sender != owner()) {
        require(!paused, "the contract is paused");
        require((block.timestamp-mintStartTime)>3600, "Eclairs public mint has not started yet");
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      addressMintedBalance[_receivingAddress]++;
      _safeMint(_receivingAddress, supply + i);
    }
  }
