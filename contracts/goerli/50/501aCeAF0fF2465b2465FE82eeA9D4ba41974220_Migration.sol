...
contract Migration is Governable, ReentrancyGuard {
    ...
    function migrate(address _account, uint256 _amount, bytes32[] calldata _proof) external whileUnpaused nonReentrant {
        // check
        ...
        // verify proof
        bytes32 node = keccak256(bytes.concat(keccak256(abi.encodePacked(_account, _amount))));
        require(MerkleProof.verify(_proof, merkleRoot, node), "invalid account");
