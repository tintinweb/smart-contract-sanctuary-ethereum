...
import "./ArtDubai.sol";
import "./reward_tokens/IRewardTokensCalculator.sol";

error ArtDubaiAlexisChristodoulou__NoRewardTokens();
...
contract ArtDubaiAlexisChristodoulou is ArtDubai {
	IRewardTokensCalculator private immutable i_rewardTokensCalculator;

	uint8 private immutable i_daytimeRewardTokenId;
	uint8 private immutable i_nighttimeRewardTokenId;

	constructor(
		IRewardTokensCalculator rewardTokensCalculator,
		uint8 daytimeRewardTokenId,
		uint8 nighttimeRewardTokenId
	) ArtDubai(0xaca187d2a59b04b4, 0x89994365E8538F61a906c84cb15B6d415051fed0) {
		i_rewardTokensCalculator = rewardTokensCalculator;

		i_daytimeRewardTokenId = daytimeRewardTokenId;
		i_nighttimeRewardTokenId = nighttimeRewardTokenId;
	}

	/**
	 * @dev Mint reward tokens for the caller
	 */
	function mintRewardTokens() external nonReentrant {
		uint8 tokenIdToMint = _itIsDaytime() ? i_daytimeRewardTokenId : i_nighttimeRewardTokenId;

		SaleConditions memory conditions = s_saleConditions[tokenIdToMint];
		if (block.timestamp < conditions.startTime) revert ArtDubai__SalesNotStarted();

		uint256 quantity = i_rewardTokensCalculator.calculateRewardTokensCount(msg.sender);
		if (quantity == 0) revert ArtDubaiAlexisChristodoulou__NoRewardTokens();
		i_rewardTokensCalculator.markAvaliableRewardTokensAsUsed(msg.sender);

		_mint(msg.sender, tokenIdToMint, quantity, "");
	}

	/**
	 * @dev Proxy call to the reward calculator contract. Calculate the number of reward tokens to mint for the wallet
	 *
	 * @param owner The wallet to calculate the number of reward tokens for
	 *
	 * @return count The number of reward tokens available
	 */
	function calculateRewardTokensCount(address owner) external view returns (uint256 count) {
		count = i_rewardTokensCalculator.calculateRewardTokensCount(owner);
	}

	/**
	 * @dev Get the reward token ID, that's minted during the day
	 *
	 * @return rewardTokenId The daytime reward token ID
	 */
	function getDaytimeRewardTokenId() public view returns (uint256 rewardTokenId) {
		rewardTokenId = i_daytimeRewardTokenId;
	}

	/**
	 * @dev Get the reward token ID, that's minted during the night
	 *
	 * @return rewardTokenId The nighttime reward token ID
	 */
	function getNighttimeRewardTokenId() public view returns (uint256 rewardTokenId) {
		rewardTokenId = i_nighttimeRewardTokenId;
	}

	/// @inheritdoc ArtDubai
	function isTokenForSale(uint8 tokenId) public view override returns (bool isForSale) {
		isForSale =
			(tokenId != i_daytimeRewardTokenId) &&
			(tokenId != i_nighttimeRewardTokenId) &&
...
error ArtDubai__SalesNotStarted();
	...
	ReentrancyGuard
{
	...
	mapping(uint8 => SaleConditions) public s_saleConditions;
	mapping(uint8 => mapping(address => uint16)) private s_numberMinted;
	 ...
	 *
	 * @param hash The hash of the operation for security.
	 * @param signature The signature of the operation for security.
	 * @param nonce The nonce of the operation for security.
	...
	function mint(
		...
		if (s_numberMinted[tokenId][msg.sender] + quantity > conditions.maxTokensPerWallet)
			revert ArtDubai__ExceedingTokensPerWalletLimit();

		if (_mintOperationHash(msg.sender, tokenId, quantity, nonce) != hash)
			revert ArtDubai__HashComparisonFailed();
		if (!_isTrustedSigner(hash, signature)) revert ArtDubai__UntrustedSigner();
		if (s_usedNonces[nonce]) revert ArtDubai__SignatureAlreadyUsed();

		s_numberMinted[tokenId][msg.sender] += quantity;
	...
	function setSaleConditions(
		...
		SaleConditions[] memory saleConditions
	...
	/**
	 * @dev Get the number of tokens remaining for a token.
	 *
	 * @param tokenId The id of the token.
	 *
	 * @return remainingSupply The number of tokens available to mint.
	 */
	function getRemainingSupply(uint8 tokenId) external view returns (uint16 remainingSupply) {
		SaleConditions memory conditions = s_saleConditions[tokenId];

		if (!isTokenForSale(tokenId)) revert ArtDubai__TokenNotForSale();

		uint256 supplyLimit = conditions.supplyLimit;
		uint256 totalTokenSupply = totalSupply(tokenId);

		if (totalTokenSupply >= supplyLimit) {
			remainingSupply = 0;
		} else {
			remainingSupply = uint16(supplyLimit - totalTokenSupply);
		}
	}
	 ...
	 * @param quantity The quantity of tokens to mint.
	 * @param nonce The nonce of the mint operation.
	 *
	 * @return The hash of the mint operation.
	 */
	function _mintOperationHash(
		...
		return
			keccak256(
				abi.encodePacked(i_hashSalt, minter, uint64(block.chainid), tokenId, quantity, nonce)
			);
	}
...
/**
 * @title Reward tokens calculator interface
 * @author DeployLabs.io
 *
 * @dev This interface is used to calculate the number of reward tokens to mint for Alexis Christodoulou Art Dubai collection
 */
interface IRewardTokensCalculator {
	function markAvaliableRewardTokensAsUsed(address owner) external;

	function calculateRewardTokensCount(address wallet) external view returns (uint256);
}