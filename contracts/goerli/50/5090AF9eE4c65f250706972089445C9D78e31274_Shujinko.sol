...
contract Shujinko is ERC721AQueryable, ERC721ABurnable, Ownable {
    ...
    // @dev The merkle root of ob list
    ...
    // @dev An address mapping for oblis mints
    ...
    // @dev The whitelist supply (n - 1)
    uint256 public whitelistSupply = 3756;

    // @dev The reserved supply (n - 1)
    uint256 public reservedSupply = 1801;

    // @dev Track whitelist mint count
    uint256 whitelistMinted = 0;

    // @dev Track oblist mint count
    uint256 oblistMinted = 0;

    constructor() ERC721A("Shujinko", "Shujinko") {
        _mintERC2309(treasury, 55); // Team mints
    }

    /**
     * @notice Set the live timestamps for mint phases
    ...
    // @dev Check if whitelist mint is live
    function isWhitelistLive() public view returns (bool) {
        return block.timestamp > whitelistLiveAt;
    }

    // @dev Check if public mint is live
    function isPublicLive() public view returns (bool) {
        return block.timestamp > publicLiveAt;
    }

    // @dev Check if oblis mint is live
    function isOblisLiveAt() public view returns (bool) {
        return block.timestamp > oblisLiveAt;
    }

    /**
     * @notice Whitelisted minting function which requires a merkle proof (max per wallet)
    ...
    /**
     * @notice Oblist FREE minting function which requires a merkle proof (max per wallet)
     * @param _amount The amount to mint
     * @param _proof The bytes32 array proof to verify the merkle root
     */
    function oblistMint(uint256 _amount, bytes32[] calldata _proof) external {
        require(isOblisLiveAt(), "0");
        require(oblistMinted + _amount < reservedSupply, "1");
    ...
    /**
     * @dev Check remaining oblist mints
     * @param _address mint address lookup
     */
    function remainingOblistMints(address _address)
        public
        view
        returns (uint256)
    {
    ...
    /**
     * @notice Sets the merkle root for the mint
     * @param _obMerkleRoot The merkle root to set
     */
     ...
     * @param _whitelistSupply The whitelist supply of the collection
     * @param _reservedSupply The reserved supply of the collection
     */
