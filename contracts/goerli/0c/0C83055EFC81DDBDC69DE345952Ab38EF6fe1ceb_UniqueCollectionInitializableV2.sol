...
interface IMintConstraintV1 is IERC165Upgradeable {
     ...
     * Solidity doesn not have a `any` or generic type so we can
     * not define a function with undefined return type.
     *
     * function unpackInitializerData(
     *     bytes memory _data
    ...
    PausableUpgradeable
{
    ...
    function _passMintConstraint(
        ...
        if(address(mp.mintConstraint) != address(0x00)) {
            require(
                mp.mintConstraint.canMint(_msgSender(), _data),
    ...
    */
    ...
    {
        // require(mintPhases.length > _mintPhaseIndex, "mintphase not exists");
        MintPhase memory mp = mintPhases[_mintPhaseIndex];
        _passMintConstraint(mp, _data);
        _phaseMint(_mintPhaseIndex, mp, _msgSender());
    }
    ...
    function _phaseMint(
        ...
        require(msg.value >= _mp.mintPrice, "price too low");
        _canStillMintInPhase(_mintPhaseIndex, _mp, _to, 1);
        minted[_to]+=1;
        phaseMinted[_to][_mintPhaseIndex]+=1;
        totalPhaseMinted[_mintPhaseIndex]+=1;
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
