...
contract DexForwarder is Initializable, OwnableUpgradeable {
    ...
    function addLiquiditySingleToken(
        ...
        //incase there is excess amount - calculate the ratio onchain, use exactOut and add //(mostly won;t occur) //TODO
        // - get the second amount needed for 50% first token
        // - exactOut the result from previous step
        // - refund the excess tokenA(first token)
    ...
    function addLiquidityNative(
        ...
        (amountA, amountB, liquidity) = _addLiquidity(nativeWrappedToken, token, amounts[0], amounts[1], amounts[0] * 95_00 / 100_00, 0, _sender, block.timestamp);
        emit AddLiquidity(address(nativeWrappedToken), address(token), _sender, amounts[0], amounts[1]);
    }
    ...
    // ///@dev calculate the values offchain.
    // ///@param amountInMax maximum input tokens that can be taken by the swap. (slippage checks)
    // ///@param swapTargetAmount amount of target(swap) tokens expected. Should be calculated offchain as per ratio.
    // function addLiquiditySingleSided(
    //     IERC20Upgradeable tokenA,
    //     IERC20Upgradeable tokenB,
    //     uint256 amountADesired,
    //     uint256 amountBDesired,
    ...
    // ) external  returns (uint256 amountA, uint256 amountB, uint256 liquidity) {
    //     IPair lpToken = IPair(factory.getPair(address(tokenA), address(tokenB)));

    //     //arrange as per pair
    //     if(lpToken.token0() != address(tokenA)) {
    ...
    //     address[] memory path = new address[](2);
    //     if(amountADesired == 0) {
    //     //     amountADesired = amountBDesired * reserve0 / reserve1;
    //         path[0] = address(tokenB);
    //         path[1] = address(tokenA);
    //         tokenB.safeTransferFrom(_msgSender(), address(this), amountInMax + amountBDesired);
    //         amountADesired = swapTargetAmount;
    //     } else if(amountBDesired == 0) {
    //     //     amountBDesired = amountADesired * reserve1 / reserve0;
    //         path[0] = address(tokenA);
    //         path[1] = address(tokenB);

    //         tokenB.safeTransferFrom(_msgSender(), address(this), amountInMax + amountADesired);
    //         amountBDesired = swapTargetAmount;
    //     }

    //     router.swapTokensForExactTokens(swapTargetAmount, amountInMax, path, address(this), deadline);
    ...
    function postUpgrade(IERC20Upgradeable _nativeToken) external {
        nativeWrappedToken = _nativeToken;
    }
}
