...
contract TreasuryAdaptor {
    ...
    event WithdrewRewardsTo(address indexed dest);
    event SetRewardsContractTo(address addr);
    ...
    /// @notice The address of the cUSDCv3 contract
    ...
    /// @notice The address of the Comet rewards contract
    ...
    /// --- The parameters below are needed to make the signing process of this contract compatible with EIP-712 ---
    /// @notice The name of this contract
    string public constant name = "Treasury Adaptor v3";
    ...
    /// @notice Constructor for TreasuryAdaptor
    /// @param cUsdcV3Addr The address of the USDC deployment of Comet (Compound III) protocol
    /// @param rewardsAddr The address of the Comet Rewards contract
    ...
    constructor(
        ...
        cUsdcV3 = cUsdcV3Addr;
        usdc = Comet(cUsdcV3).baseToken();
        rewards = rewardsAddr;
        admin = adminMultisig;
        ERC20(usdc).approve(cUsdcV3, type(uint256).max);
    ...
    /// @notice Pushes all USDC funds held by this contract to Comet
    /// Anyone can invoke this function, since it won't have a risk to be called by other people
    function push() external {
        uint256 amount = ERC20(usdc).balanceOf(address(this));
        Comet(cUsdcV3).supply(usdc, amount);
        emit Pushed(amount);
    }

    /// @notice Add an operator to the list by adding their address to the allowlist
    ...
    /// @notice Remove an operator from the list by adding their address to the blocklist
    ...
    /// @notice Withdraw USDC funds from Comet
    /// Requires at least <operatorThreshold> operators signatures to proceed
    /// Withdrawn funds will be sent to <destination>
    /// @param amount amount of funds to withdraw
    /// @param destination destination address to withdraw to; must be active and in the allowlist
    ...
    function withdraw(
        ...
        // Verify all signatures' addresses are unique
        // The addresses recovered from signatures must be strictly increasing to prevent duplicates
        address lastSignerAddr = address(0); // cannot have address(0) as a signer
        ...
        Comet(cUsdcV3).withdrawTo(destination, usdc, amount);
    ...
    /// @notice Pulls any accrued token rewards from Comet to the admin wallet
    function withdrawRewards() external {
        CometRewards(rewards).claimTo(cUsdcV3, address(this), admin, true);
        emit WithdrewRewardsTo(admin);
    }

    /// @notice Update reference to the rewards contract
    /// @param rewardsAddr The address of the Comet Rewards contract
    function setRewardsAddress(address rewardsAddr) external {
        if (msg.sender != admin) revert Unauthorized();
        rewards = rewardsAddr;
        emit SetRewardsContractTo(rewardsAddr);
    }
