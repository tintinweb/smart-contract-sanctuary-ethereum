...
contract ASTTokenRewards is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, DateTime {
    ...
    function claim() external nonReentrant {
        ...
        for (uint256 i; i < nftBalance; i++) {
            ...
            rewards += amount;
            emit HoldngRewardsClaimed(id, _rewards, CATEGORY(x));
        }
        ...
        userDetails.lastRewardCliamed = Actual_claimedRewards;
        userDetails.totalRewardsClaimed += Actual_claimedRewards;
        userDetails.dueRewards = dueRewards;
        userDetails.toClaim = 0;
        token.transfer(_msgSender(), Actual_claimedRewards);
        emit TotalRewardsClaimedToday(Actual_claimedRewards, userDetails.toClaim);
    }
    ...
    function getRewardsCalc(
        ...
        if (dayCount != 0) {
            rewardAmount = dayCount <= 365
                ? dayCount * RewardsMap[1][category]
        ...
        if (user.lastClaim[_id] > purchaseTime) {
            uint256 claimDays = (user.lastClaim[_id] - purchaseTime) / 1 days;
...
interface IASTNftSale is IERC721EnumerableUpgradeable {
    ...
    function getLastPurchaseTime(uint256 tokenId, address user) external view returns (uint256);
}
