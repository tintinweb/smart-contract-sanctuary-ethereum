...
contract FND is ERC721, Ownable {
    ...
    address constant public fundOfTheArmedForcesOfUkraine = 0xcccdBb8F9BE160a70D24a1448f22cC9b28BcDcE3;

    event Mint(address indexed owner, uint indexed tokenId);
    event DonatedToHelpTheArmedForcesOfUkraine(address indexed sender, uint256 indexed amount);
    ...
    function mint() external payable {
        ...
        require(amount <= maxItemsPerTx, "Mint: Surpasses maxItemsPerTx");

        _mintWithoutValidation(msg.sender, amount);

        donateToHelpTheArmedForcesOfUkraine();
    }
    ...
    /**
     * Donate 50% of minting NFT
     */
    function donateToHelpTheArmedForcesOfUkraine() internal {
        uint256 donationAmount = msg.value / 2;

        (bool success,) = fundOfTheArmedForcesOfUkraine.call{value: donationAmount}("");
        require(success, "DonateToHelpTheArmedForcesOfUkraine: Failed to send ether");

        emit DonatedToHelpTheArmedForcesOfUkraine(msg.sender, donationAmount);
    }
    ...
    function isOpen() external view returns (bool) {
        return block.timestamp >= startTimestamp && !mintPaused && totalSupply < maxItems;
    }
