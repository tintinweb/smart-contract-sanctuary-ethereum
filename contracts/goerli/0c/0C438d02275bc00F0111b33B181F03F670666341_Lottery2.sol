...
contract Lottery2 {
    ...
    function joinLottery2() public payable {
        require(msg.value == pool2_value, "value should be 0.5 ether");
        require(user2[msg.sender] == false, "user already participiant");
        _user2.push(msg.sender);
        user2[msg.sender] = true;
        payable(address(this)).transfer(pool2_value);
        totalAmount2 = totalAmount2 + pool2_value;
    }

    function random2() private view returns (uint256) {
        return uint256(keccak256(abi.encode(block.timestamp, _user2)));
    }

    function pickWinner2() public {
        require(_user2.length != 0, "No participient");
        uint256 index2 = random2() % _user2.length;
        uint256 winnerprize = totalAmount2.mul(winner_prize).div(100);
        owner_prize1 = totalAmount2.mul(owner_prize1).div(100);
        owner_prize2 = totalAmount2.mul(owner_prize2).div(100);
        owner_prize3 = totalAmount2.mul(owner_prize3).div(100);
        payable(_user2[index2]).transfer(winnerprize);
    ...
    function changevale2(uint256 poolprice) public {
        require(msg.sender == owner, "owner can call ");
        pool2_value = poolprice;
    }

    function reset() internal {
        // iterate over the array to iterate over the mapping and reset all to value
        for (uint i = 0; i < _user2.length; i++) {
            user2[_user2[i]] = false;
        }
    }
