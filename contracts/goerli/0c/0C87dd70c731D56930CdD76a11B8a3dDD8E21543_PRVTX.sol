...
contract PRVTX is Context, Ownable, IERC20 {
    ...
    address public usdtToken = 0x0EA47EDa67415b406A4F15333367EBC516Cd1c13;
    ...
    constructor () {
        ...
        if (block.chainid == 56) {
            ...
            swapRouter = IRouter02(0x4c11DBbaeFe87B8E7492724180D1907e79695a83);
        ...
        // address pairUSDT = IFactoryV2(swapRouter.factory()).createPair(address(this), usdtToken);
        isLpPair[lpPair] = true;
        _approve(msg.sender, address(swapRouter), type(uint256).max);
        _approve(address(this), address(swapRouter), type(uint256).max);

        IERC20(usdtToken).approve(address(swapRouter), type(uint256).max);
        // IERC20(usdtToken).approve(address(pairUSDT), type(uint256).max);
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        // uint256 marketingBal = address(this).balance * sellTaxes.marketing / rate;
        // uint256 rewardsBal = address(this).balance - marketingBal;

        // (success,) = marketingAddress.call{value: marketingBal, gas: 35000}("");
        // (success,) = rewardsAddress.call{value: rewardsBal, gas: 35000}("");
