...
contract HoneyFarmQueenDeluxe is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(BEE_LEVEL => uint8) rewardsPerBeeLevel; // 10 = 1
    ...
    event Test(string message, uint256 value);
    ...
    function initialize() public initializer {
        ...
        rewardsPerBeeLevel[BEE_LEVEL.COMMON_ACTIVE] = 20;
        rewardsPerBeeLevel[BEE_LEVEL.COMMON_WORKER] = 30;

        rewardsPerBeeLevel[BEE_LEVEL.UNCOMMON_ACTIVE] = 25;
        rewardsPerBeeLevel[BEE_LEVEL.UNCOMMON_WORKER] = 40;

        rewardsPerBeeLevel[BEE_LEVEL.RARE_ACTIVE] = 35;
        rewardsPerBeeLevel[BEE_LEVEL.RARE_WORKER] = 50;

        rewardsPerBeeLevel[BEE_LEVEL.EPIC_ACTIVE] = 45;
        rewardsPerBeeLevel[BEE_LEVEL.EPIC_WORKER] = 60;

        rewardsPerBeeLevel[BEE_LEVEL.LEGENDARY_ACTIVE] = 55;
        rewardsPerBeeLevel[BEE_LEVEL.LEGENDARY_WORKER] = 70;
    }
    ...
    function keepBeesActive(uint16[] calldata _beesIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < _beesIds.length; i++) {
            ...
            // if (honeyCostMultiplier == 0) honeyCostMultiplier = 1;
    ...
    function levelUpBee(
        ...
        for (uint256 i; i < _honeyCombs.length; i++) {
            ...
            emit Test("quantities", _quantities[i]);
            emit Test("rarity", rarity);
            emit Test("rarity + 1", rarity + 1);

            uint256 newLevels = (rarity + 1) * _quantities[i];

            emit Test("newLevels", newLevels);


            uint256 nextLevel = newLevels + currentLevel;

            emit Test("nextLevel", nextLevel);


            //if leveling up quantity does not go beyond max level
            if (currentLevel + nextLevel >= MAX_BEE_LEVEL) {
            ...
            } else if (currentLevel + nextLevel < MAX_BEE_LEVEL) {
...
abstract contract HoneyCombsDeluxeI is Ownable, IERC1155 {
    ...
    function getRarity(uint16 _tokenId) external view virtual returns (HONEY_COMB_RARITY);
