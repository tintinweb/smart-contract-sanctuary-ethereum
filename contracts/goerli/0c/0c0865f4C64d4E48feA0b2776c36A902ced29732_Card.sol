...
contract Card is ERC721, ERC721Enumerable, ERC721Burnable, ERC2981, Pausable, Ownable, ReentrancyGuard {
    /* ==================== VARIABLES SECTION ==================== */
    /**
    ...
    */
    uint256 maxMintPerTransaction = 5;
    ...
    */
    ...
    string public previewURI = "ipfs://QmUfkXJHASK1fxb9hRyc6Ez2pJMpic7Nwkkay8TSaGDj3D/";  
    ...
    */
    string public baseURI = "https://www.ipfs.io/nft/";
    ...
    */
    modifier isCallerAllowed() {
        require(tx.origin == msg.sender || owner() == msg.sender, "ER: IC"); // Invalid Caller
        _;
    }

    /**
    * @dev Reverts if withdrawalAddress is incorrect    
    */
    modifier isValidWithdrawalAddress() {
        require(withdrawalAddress == msg.sender, "ER: IR"); // Invalid Recipient
        _;
    }

    /**
    * @dev Reverts if the balance of the wallet address is 0
    */
    modifier isBalanceNotZero() {
        require((address(this).balance) > 0, "ER: NB"); // No Balance
        _;
    }

    /**
    * @dev Reverts if the payment is not equal to the mint price for tier
    * **NOTE: Magic happening! 2 requires in one modif haha
    */
    modifier isExactPayment(uint256[] memory _tiers) {
        uint256 _correctPayment;
        for (uint256 x = 0; x <= (_tiers.length - 1); x++) {
            require(_tiers[x] >= 0 && _tiers[x] <= (mintPrices.length - 1), "ER: ITP"); // Incorrect Tier Price
            _correctPayment += mintPrices[ _tiers[x]];
        }
        require(msg.value == _correctPayment, "ER: IP"); // Incorrect Payment
        _;
    }
    ...
    */
    modifier isSameLength(uint256 _id, uint256 _tier) {        
        require(_id == _tier, "ER: NSL"); // Not Same Length
        _;
    }

    /**
    * @dev Reverts if the quantity is beyond the max supply
    */
    modifier isMaxSupplyAllowed(uint256 _quantity) {
        uint256 value = totalSupply() + _quantity;
        require(value <= MAX_SUPPLY, "ER: MR"); // Max Reached
        _;
    }

    /**
    * @dev Reverts if quantity is greater than the max mint per transaction
    */
    modifier isMaxMintPerTxAllowed(uint256 _quantity) {
        require(_quantity <= maxMintPerTransaction, "ER: MI"); // Max Invalid
        _;
    }
    ...
    */
    modifier isTokenExists(uint256 _id) {
        require(_exists(_id), "ER: TNE"); // Token Not Exist
        _;
    }
    /* ==================== MODIFIERS SECTION ==================== */
    ...
    */
    ...
    /* ==================== REVEAL SECTION ==================== */
    /**
    * @dev update the revealURIMode value to true.
    * @dev it will allow to reveal the final metadata    
    */
    function enableURIMode() external 
        onlyOwner 
    {
        revealURIMode = true;

        emit SetBaseURIMode(revealURIMode);
    }

    /**
    * @dev update the revealURIMode value to true.
    * @dev it will show the unrevealed metadata    
    */
    function disableURIMode() external 
        onlyOwner 
    {
        revealURIMode = false;
    ...
    */
    function mint(address _to, uint256[] calldata _ids, uint256[] calldata _tiers, string[] memory _uris)
        external
        payable
        whenNotPaused
        isCallerAllowed
        isMaxMintPerTxAllowed(_ids.length)
        isMaxSupplyAllowed(_ids.length)
        isSameLength(_ids.length, _tiers.length)
        isExactPayment(_tiers)     
    {
    ...
    */
    ...
    /* ==================== (EXTERNAL) WITHDRAW SECTION ==================== */
    /**
    * @dev send contract balance to withdrawal address
    *
    ...
    * @param _uris metadata for each token
    */
    function safeBatchMint(address _to, uint256[] calldata _ids, uint256[] calldata _tiers, string[] memory _uris)
        private
    {
    ...
    *      see https://docs.openzeppelin.com/contracts/4.x/api/token/erc721#ERC721Burnable-burn-uint256-
    *      for more details
    *
