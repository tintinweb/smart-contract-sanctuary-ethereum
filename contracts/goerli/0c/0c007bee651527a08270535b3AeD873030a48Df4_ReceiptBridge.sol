...
import "contracts/base/ReceiptECDSA.sol";
import "contracts/base/Roles.sol";
import "contracts/base/Whitelist.sol";
import "./NFT.sol";

contract ReceiptBridge is Roles, ReceiptECDSA, Whitelist, IReceipt {
    using SafeERC20 for IERC20;

    uint256 constant PERCENT_100 = 100e9;
    uint96 feePercent;
    ...
    constructor(
        ...
        address _nft,
        address _owner,
        address _legitimateSigner,
        address _treasure
    ) Roles(_legitimateSigner, _treasure) ReceiptECDSA("ReceiptBridge") {
        feePercent = uint96(_feePercent);
        nft = NFT(_nft);
    ...
    function pay(
        ...
        uint256 feePayed = distributeInvoice(
            pr.token,
            pr.amount,
            msg.sender,
            pr.recipient
    ...
    function verifySignatureForNameChanging(
        uint256 _tokenId,
        address _operator,
    ...
    function setFeePercent(uint256 _newPercentE9) external onlyOwner {
        feePercent = uint96(_newPercentE9);
    }

    function changeTokensIsWhitelisted(address[] calldata _tokens)
        external
        onlyOwner
    {
        for (uint256 i; i < _tokens.length; i++) changeWhitelisted(_tokens[i]);
    }

    function calculateInvoiceFee(uint256 _amount)
        public
        view
        returns (uint256 fee_)
    {
        fee_ = (_amount * feePercent) / PERCENT_100;
    }
    ...
    function distributeInvoice(
        ...
        fee_ = calculateInvoiceFee(_amount);

        IERC20(_token).safeTransferFrom(_from, _to, _amount - fee_);
        IERC20(_token).safeTransferFrom(_from, treasure, fee_);
    }
}
...
import "contracts/base/Roles.sol";
import "contracts/errors/ReceiptECDSA.sol";

abstract contract ReceiptECDSA is EIP712, Roles {
    using ECDSA for bytes32;

    uint256 public requestLifetime = 15 minutes;

    bytes32 private constant RECEIPT_SCHEMA_HASH =
        keccak256(
    ...
    bytes32 private constant UPDATE_NAME_SCHEMA_HASH =
        keccak256(
    ...
    constructor(string memory _contractName) EIP712(_contractName, "1.0") {}

    function setRequestLifetime(uint256 _new) external onlyOwner {
        requestLifetime = _new;
    }
    ...
    function _useSignature(
        ...
        doubleSpendGuard(bytes32(_receiptId));
        ...
        view
    {
        ...
        if (recovered != signer) revert WrongSigner(recovered);
    }
...
abstract contract Roles is Ownable {
    ...
    constructor(address _signer, address _treasure) {
        signer = _signer;
        treasure = _treasure;
    }
...
import "contracts/errors/Whitelist.sol";

abstract contract Whitelist {
    mapping(address => bool) public whitelisted;

    function changeWhitelisted(address _target) internal {
        whitelisted[_target] = !whitelisted[_target];
    }

    function requireWhitelisted(address _target) internal view {
        if (!whitelisted[_target]) revert AddressIsNotWhitelisted(_target);
    }
}
...
import "contracts/base/OwnableImmutable.sol";
import "contracts/libraries/NFTDescriptor.sol";
import "./ReceiptBridge.sol";

contract NFT is ERC721Holder, ERC721A, OwnableImmutable {
    ...
    function updateNickname(
        ...
        ReceiptBridge(owner()).verifySignatureForNameChanging(
            _tokenId,
            msg.sender,
            _newName,
            _signTimestamp,
