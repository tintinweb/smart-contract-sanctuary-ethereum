...
contract DiceGame is Initializable, RandomNumber {
    ...
    // This game allows a user to guess in which range a random number will be generated, the players bet for that range if they win they get a prize, otherwise they loose their bet
    ...
    function playGame(
        ...
        if (token == IERC20Upgradeable(address(0))) {
            ...
            contractBalance = address(this).balance - paidFee;
        } else {
        ...
        require(
            ...
            "DiceGame: Estimated prize is equal or less than the bet amount"
        );
        ...
        (bool success, ) = s_feeRecipient.call{value: chainlinkFeeNativeToken}(
            ""
        );
        ...
        returns (uint256)
    {
        ...
        uint256 priceParsed = (uint256(price) * s_linkPremium) / 1e18;
    ...
    function getAvailablePrize(IERC20Upgradeable token)
        ...
        returns (uint256)
    {
        uint256 available = s_totalInBetsPerToken[address(token)];
        if (token == IERC20Upgradeable(address(0))) {
            return address(this).balance - (available);
        } else {
            return token.balanceOf(address(this)) - available;
        }
    }
}
...
contract RandomNumber is OwnableUpgradeable, VRFConsumerBaseV2Upgradeable {
        ...
        override
    {
        ...
        // uint16 winningNumber = uint16(randomWords[0] % 10001);
