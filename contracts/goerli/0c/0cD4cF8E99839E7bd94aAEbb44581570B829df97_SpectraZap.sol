...
interface ISpectraZap {
    ...
    error FailedToFetchExpectedLPToken();
    error LpTokenAmountLessThanMinimumValue();
    error PrincipalTokenFactoryAddressNotFound();
...
library CurvePoolUtil {
    ...
    error FailedToConverge();
    error UnsafeValueForX0();
    error UnsafeValueForX1();
    error UnsafeValueForDeposits();
    error DepositStrategyFailed();
    ...
    function _newtonD(Args memory args) internal pure returns (uint256) {
        ...
        **/
        ...
        if(!(x[0] > 1e9 - 1 && x[0] < 1e15 * 1e18 + 1)) {
            revert UnsafeValueForX0();
        }
        if(!((x[1] * 1e18) / x[0] > 1e14 - 1)) {
            revert UnsafeValueForX1();
        }
...
contract SpectraZap is ISpectraZap, Initializable {
    ...
    function zapInToCurvePoolWithAsset(
        ...
        if ((factory == address(0))) {
            revert PrincipalTokenFactoryAddressNotFound();
        }
        ...
        if (lpAmount < minLpAmount) {
            revert LpTokenAmountLessThanMinimumValue();
        }
    ...
    function _calcExpectedLpTokenAmount(
        ...
        if (!success) {
            revert FailedToFetchExpectedLPToken();
        }
