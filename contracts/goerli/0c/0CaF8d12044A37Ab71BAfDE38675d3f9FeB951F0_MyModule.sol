...
contract MyModule is Module {
    IERC5564Messenger public messengerContract;


    constructor(address _owner, address _messengerContract) {
        bytes memory initializeParams = abi.encode(_owner, _messengerContract);
    ...
    function setUp(bytes memory initializeParams) public override initializer {
        __Ownable_init();
        (address _owner, address _messengerContract) = abi.decode(initializeParams, (address, address));
        messengerContract = IERC5564Messenger(_messengerContract);
        setAvatar(_owner);
        setTarget(_owner);
    ...
    /// @param ephemeralPubKey Is broadcasted by an immutable implementation of messengerContract
    ...
    function privateETHTransfer(
        ...
        require(safe.execTransactionFromModule(stealthAddress, amount, "", Enum.Operation.Call), "Could not execute ether transfer");
        messengerContract.announce(schemeId, stealthAddress, ephemeralPubKey, metadata);
    }    
}
