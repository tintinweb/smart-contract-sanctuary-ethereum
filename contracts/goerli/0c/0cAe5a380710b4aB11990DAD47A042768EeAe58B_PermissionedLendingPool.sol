...
import {eddsaVerifier} from './eddsaVerifier.sol';

contract eddsaCredentialManager is ReentrancyGuard {
    event wallet_kyc(address account);

    address verifier_address;
    uint pub_x;
    uint pub_y;
    
    constructor(address verifier_add, uint pub_x_in, uint pub_y_in) public { 
        verifier_address = verifier_add; 
        pub_x = pub_x_in;
        pub_y = pub_y_in;
    }
    // Checks to ensure you are not re-registering with the same leaf
    mapping(uint => address) public registered_users;
    // mapping(address => bool) public registered_wallets;
    function add_credential(
        uint[2] memory a,
        uint[2][2] memory b,
        uint[2] memory c,
        uint[5] memory input //input[0] = pk_x, 1 = pk_y, 2 = wallet, 3 = time, 4 = leaf
    ) external payable nonReentrant {
        uint leaf = input[4];
        // verify proof - this verifies signature
        eddsaVerifier verifier = eddsaVerifier(verifier_address);
        require(verifier.verifyProof(a, b, c, input), "proof no good");
        
        // Potential vulnerability
        // require(input[2] == uint256(uint160(tx.origin)), "wallet address wrong");
        require(input[2] == uint256(tx.origin), "wallet address wrong");
        //check pub key and cred_id
        require(input[0] == pub_x, "pub key x value no good");
        require(input[1] == pub_y, "pub key y value no good");

        // Check proof is not being used for different wallet
        require(registered_users[leaf] == address(0) || registered_users[leaf] == tx.origin, "KYC already used on a different wallet");
        registered_users[leaf] = tx.origin;
        // registered_users[leaf] = true;
        // registered_wallets[tx.origin] = true;
        emit wallet_kyc(tx.origin);
    }

    function registeredAddress(uint leaf) external view returns (address) {
        return registered_users[leaf];
    }
}
...
contract eddsaVerifier {
    ...
    function verifyingKey() internal pure returns (VerifyingKey memory vk) {
        ...
        vk.delta2 = Pairing.G2Point(
            [2125317541198013108743056644036512505929254518761379841745540674759279349174,
             9359235537496458729379721975413392545379097246546897182402537725196756490302],
            [6776335322576360167323569188537156501520776653269910555090746783372346567164,
             10102644717143124839166124597444885312653671420158964525346752912897688198860]
        );
        vk.IC = new Pairing.G1Point[](6);
        
        vk.IC[0] = Pairing.G1Point( 
            1775235192657277357787138575196465354813441193275357727823369282300091785037,
            5891411517854233581681270309681770698262835927607619671939322854651393157692
        );                                      
        
        vk.IC[1] = Pairing.G1Point( 
            19979509910450319958774900227705799145627240902162919275983807408116330785494,
            8139528002688773785814149267875699952848386380386934931681614261095404219827
        );                                      
        
        vk.IC[2] = Pairing.G1Point( 
            19655784644866820272735306453897793721778492149411102545191495564391780587939,
            3821642183785451963286549124803438547192636389221441554092643008601268363628
        );                                      
        
        vk.IC[3] = Pairing.G1Point( 
            20436197312112093575120412791932279215752175038908060060106986253037025778182,
            13328724041125966405796237762839478512539193278144845637555564874865193071177
        );                                      
        
        vk.IC[4] = Pairing.G1Point( 
            18071996848758153974227288640655971920669045825253113635247653235493862809543,
            11654086088624925433696020455096708555825548587182970926032071686161370908355
        );                                      
        
        vk.IC[5] = Pairing.G1Point( 
            21763493888954969853698613244637848017337660966324025378625132523240546535663,
            10381879644860972087662240258552444538218186202495534780412701534048583272047
        );                                      
...
import {Ownable} from '../../dependencies/openzeppelin/contracts/Ownable.sol';
import {eddsaCredentialManager} from '../configuration/eddsaCredentialManager.sol';
...
contract PermissionManager is IPermissionManager, Ownable {
  ...
  mapping(address => uint) public time_of_kyc;
  ...
  // Notebook set manager address
  function setManagerAddress(address manager) external override onlyOwner {
    manager_address = manager;
  }

    // Notebook add credential
  function addCredential(uint256 role, uint[2] memory a, uint[2][2] memory b, uint[2] memory c, uint[5] memory input)
    external
    override
  {

      uint256 permissions = _users[tx.origin].permissions;

      require(role < MAX_NUM_OF_ROLES, 'INVALID_ROLE');

      eddsaCredentialManager manager = eddsaCredentialManager(manager_address);
      manager.add_credential(a, b, c, input);

      _users[tx.origin].permissions = permissions | (1 << role);
      _users[tx.origin].permissionAdmin = address(0);
      time_of_kyc[tx.origin] = input[3];

      emit RoleSet(tx.origin, role, msg.sender, true);
    }
...
interface IPermissionManager {
  ...
  // Notebook add credential
  function addCredential(uint256 role, uint[2] memory a, uint[2][2] memory b, uint[2] memory c, uint[5] memory input) external;
...
contract PermissionedLendingPool is IPermissionedLendingPool, LendingPool {
  ...
  function seize(
    ...
    address[] calldata assets,
    address to
  ) public virtual override(IPermissionedLendingPool) whenNotPaused {
    // Make sure sender is a permissioned admin
    IPermissionManager permissionManager = IPermissionManager(_addressesProvider.getAddress(PERMISSION_MANAGER));
    require(permissionManager.isPermissionsAdmin(msg.sender), Errors.PLP_INVALID_PERMISSION_ADMIN);
