...
contract BharatNFTMarketplace is Ownable, ReentrancyGuard {
    ...
    function getRoyalties721(uint256 tokenId, uint256 price) private view returns (address receiver, uint256 royaltyAmount){
    ...
    function getRoyalties1155(uint256 tokenId, uint256 price) private view returns (address receiver, uint256 royaltyAmount){
...
//-------------------------------------------------------------------------------ERC721(Offer)-------------------------------------------------------------------------------//
    ...
    function acceptOffer(address _nft, uint256 _tokenId, address _offerer,uint256 indexId) external nonReentrant {
    ...
    function rejectAllOffer(address _nft, uint256 _tokenId) external nonReentrant {
    ...
    function rejectOffer(address _nft, uint256 _tokenId,uint256 indexId) external nonReentrant {
        Offer memory offer = offerNfts[_nft][_tokenId];

        require(offerNfts[_nft][_tokenId].offerers[indexId] == msg.sender, "Only the owner is allowed to delete offer");

        uint256 offerersLastIndex = offer.offerers.length - 1;
        uint256 offerAmountsLastIndex = offer.offerers.length - 1;

        offerNfts[_nft][_tokenId].offerers[indexId] = offerNfts[_nft][_tokenId].offerers[offerersLastIndex];
        offerNfts[_nft][_tokenId].offerers.pop();

        offerNfts[_nft][_tokenId].offerAmounts[indexId] = offerNfts[_nft][_tokenId].offerAmounts[offerAmountsLastIndex];
        offerNfts[_nft][_tokenId].offerAmounts.pop();
    }
...
//----------------------------------------------------------------------------ERC721--------------------------------------------------------------------------------//
        ...
        nonReentrant
    {
        ...
        require(msg.value >= listedNft.price,"Please submit the asking price in order to complete the purchase");
