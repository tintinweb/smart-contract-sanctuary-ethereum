...
contract RCT is ERC721, Ownable {
  ...
  constructor() ERC721("RCTEST8", "RCT8") {}
  ...
  function batchMintForAddress(uint256[] memory _mintAmounts, address[] memory _receivers)
    public
    onlyOwner
  {
    require(_mintAmounts.length == _receivers.length, "Invalid input array");

    for (uint256 i = 0; i < _mintAmounts.length; i++) {
      mintForAddress(_mintAmounts[i], _receivers[i]);
    }
  }
  ...
  function batchWithdrawNfts(uint256[] memory tokenIds, address receipter) external onlyOwner {
