...
contract Vendor is Ownable {
  ...
  function sellTokens(uint amount) public {
    require(yourToken.balanceOf(msg.sender) >= amount, "You don't have enough tokens to sell.");
    bool tokensSent = yourToken.transferFrom(msg.sender, address(this), amount);
    require(tokensSent, 'Failed to send tokens correctly.');

    uint ethAmount = amount / tokensPerEth;
    (bool ethSent, ) = msg.sender.call{value: ethAmount}('');
    require(ethSent, 'Failed to withdraw.');
  }
}
