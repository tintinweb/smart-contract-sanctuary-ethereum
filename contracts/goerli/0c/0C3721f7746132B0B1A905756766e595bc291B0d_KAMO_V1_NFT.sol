...
contract KAMO_V1_NFT is ERC721Enumerable, Ownable, RoyaltiesV2Impl {
    using Strings for uint256;
  ...
  //configuration
    ...
    constructor(
    ) ERC721 ("KAMO_V1", "KAMOV1") {
        console.log("create KAMO_V1");
    }
    ...
    function makeAnEpicNFT(uint256 _mintAmount) public payable {
        // Get the current tokenId, this starts at 0.
        _mintAmount = _tokenIds.current();
        uint256 supply = totalSupply();
            require(!paused);
            require(balanceOf(msg.sender) < maxMintAmount, "@@@  Per wallet can only mint one NFT  @@@");
            require(_mintAmount >= 0);
            require(supply <= maxSupply, "@@@  SOLD OUT: Theres a mint limit of 10 @@@");

            console.log("_mintAmount", _mintAmount);

          if (msg.sender != owner()) {
            require(msg.value == cost, "Need to send exactly 0.01 ether");
          }
            _safeMint(msg.sender, supply);
            // Return the NFT's metadata
            tokenURI(_mintAmount);
            // Increment the counter for when the next NFT is minted.
              _tokenIds.increment();
              emit NewEpicNFTMinted(msg.sender, _mintAmount);
        }

    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
    require(_exists(_tokenId));
    console.log("An NFT w/ ID %s has been minted to %s", _tokenId, msg.sender);
    return "https://ipfs.io/ipfs/QmatxYD6c4mfWLGHLVoSHd3zx5mFgAcVX5UNdKwDbUu7T7?filename=metadata.json";
    }
