...
contract Tippr is ERC721URIStorage {
  ...
  function tipImageOwner(uint256 _id) public payable {
    ...
    require(_id > 0 && _id <= imageCount, "Invalid post id");
    // Fetch the image
    Image memory _image = images[_id];
    require(_image.author != msg.sender, "Cannot tip your own post");
    // Fetch the author
  ...
  function getAllImages() external view returns (Image[] memory _images) {
      _images = new Image[](imageCount);
      for (uint256 i = 0; i < _images.length; i++) {
          _images[i] = images[i + 1];
      }
  }
