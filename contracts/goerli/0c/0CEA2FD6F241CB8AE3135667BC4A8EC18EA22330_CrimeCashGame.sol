...
contract CrimerInfo is Governable {
  ...
  event eventAddNewCrimer(address crimer, uint8 joinRoundDay, uint256 initBank, uint256 initCash, uint256 initAttack, uint256 initDefense);
  event eventAddNewCrimerByReferral(address crimer, address byReferral, uint8 roundDay, uint256 initBank, uint256 initCash, uint256 initAttack, uint256 initDefense, uint256 referralBankBonus, uint256 referralAttackBonus);
  event eventAddBonusByReferralClick(address referralCrimer, uint8 roundDay, uint256 referralBankBonus, uint256 referralAttackBonus);
  ...
  event applyNFTInGame(address crimer, uint256[] nftWeaponIdList, uint256[] nftPerkIdList);
  ...
  function addReferralTo(address _addTo) external { 
    ...
    emit eventAddBonusByReferralClick(_addTo, roundDay, referralCashReward, referralAttackReward);
  }

  function createNewCrimer() external {
    onlyCrimer();
    _createNewCrimer(_msgSender());
    emit eventAddNewCrimer(_msgSender(), roundDay, initBank.mul(10**2), initCash.mul(10**2), initAttack, initDefense);
  }
  ...
  function _createNewCrimerByReferral(address _crimer, address _byReferral) private {
    ...
    emit eventAddNewCrimerByReferral(_crimer, _byReferral, roundDay, initBank.mul(10**2), initCash.mul(10**2), initAttack, initDefense, referralCashReward, referralAttackReward);
  }
  ...
  function setNfts(uint256[] calldata _nftWeaponIds, uint256[] calldata _nftPerkIds) external { 
    ...
    emit applyNFTInGame(msg.sender, _nftWeaponIds, _nftPerkIds);
  }
...
contract CrimeMarketplace is CrimERC1155 {
    ...
    function getNftByIdList(uint256[] calldata _nftIdList) external view returns(CrimeNFT[] memory nftInfoList) {
        require(_nftIdList.length > 0, "Invalid NFT Id list amount");
        nftInfoList = new CrimeNFT[](_nftIdList.length);
        for (uint i = 0; i < _nftIdList.length; i++) {
            uint256 nftId = _nftIdList[i];
            require(nftId < nfts.length, "Invalid NFT id");
            CrimeNFT memory nftInfo = nfts[nftId];
...
contract CrimeCash is ERC20Detailed, ERC20Mintable, Governable {
  ...
  constructor(Storage _storage, address[] memory _initMinters) public ERC20Detailed("CrimeCash", "CCASH", 2) Governable(_storage) {
    ...
    // TODO: Add router address to param of constructor
  ...
  function _partialAddToLiquidity(uint256 amount, address sender) internal returns (uint256) {
    ...
    if(!inSwapAndLiquify && sender != store.owner() && sender != uniswapV2Pair && swapAndLiquifyEnabled)
    {
      amountForLiquidity = amount.mul(_liquidityFee).div(100);
  ...
  function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
    ...
    uniswapV2Router.addLiquidityETH{value: ethAmount} (
      ...
      0, // slippage is unavoidable
      store.owner(), // The LP tokens will be added to owner address.
      block.timestamp
    );
  }
}
