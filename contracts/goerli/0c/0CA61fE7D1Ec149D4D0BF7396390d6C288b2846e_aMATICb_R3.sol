...
contract aMATICb_R3 is OwnableUpgradeable, ERC20BondBase, IinternetBond_R1 {
    ...
    function unlockShares(uint256 bonds) external override {
        _unlockShares(msg.sender, bonds, true);
    }

    function unlockSharesFor(address account, uint256 bonds) external override onlyBondMinter {
        _unlockShares(account, bonds, false);
    }

    function _unlockShares(address account, uint256 bonds, bool takeFee) internal {
        uint256 shares = balanceToShares(bonds);
        ...
        super._transfer(account, address(this), shares - fee);
        if (fee != 0) {
            super._transfer(account, swapFeeOperator, fee);
        }
    ...
    function _lockShares(address account, uint256 shares, bool takeFee) internal {
        ...
        super._transfer(address(this), account, shares - fee); // can not fail as _balance[address(this)] always equals to amount of aFTMc minted
        if (fee != 0) {
            super._transfer(address(this), swapFeeOperator, fee);
        }
    }
