...
contract DEX {
    ...
    function price(
        ...
        //0.3% fee
        uint256 yOutPut = yReserves - (k / (xReserves + (xInput * 997) / 1000));
        return yOutPut;
    }
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        ...
        // when it is from contract use transfer
        token.transfer(msg.sender, tokenOutput);
        emit EthToTokenSwap(msg.sender, "EthToBallon", msg.value, tokenOutput);
        return tokenOutput;
    }
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        ...
        token.approve(address(this), xInput);
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        uint256 xReserves = address(this).balance - msg.value;
        uint256 yReserves = token.balanceOf(address(this));
        tokensDeposited = (msg.value * yReserves) / xReserves;
        token.transferFrom(msg.sender, address(this), tokensDeposited);
        uint256 liquidityMinted = (msg.value * totalLiquidity) / xReserves;
