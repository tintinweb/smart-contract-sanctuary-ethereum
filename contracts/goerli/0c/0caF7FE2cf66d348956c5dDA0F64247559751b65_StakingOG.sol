...
contract StakingOG is ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => string) public tokenSpecificAttributes;
    mapping(uint256 => uint256) public rankAttributions; // which NFT has which rank
    mapping(uint256 => string) public tokenRanks; // what is the name of the rank of rank ID
    ...
    constructor() ERC721("Ownest Staking OGs", "OWNG"){
        StakedMatic = IERC20(0x1B09895C2AD5a0C6Be4053E5627C3575757C6705); // dev: 0x1B09895C2AD5a0C6Be4053E5627C3575757C6705, prod: 0xD14a87025109013B0a2354a775cB335F926Af65A
        mintEnabled = true;
        setNodeAPR(500);
        setNodeCommission(500);
        setBaseImageURI("https://api.onigiri.art/ownest/staking/OGs/"); // TODO: change for ex: https://images.owalt.io/img/staking-og/
        setNftDescription("Holding this NFT gives refunds of commission fees and the special OG title as initial delegators on Ownest validator node of Polygon: Ahoy there, adventurer! Have you heard of the mystical card that grants its owner the power to vanquish the vexing commissions on the Ownest validator node 47 of the treacherous Polygon network? With this mighty card in hand, you can boldly navigate the perilous waters of the blockchain and reap the rewards that await you at the end of your journey. The riches you seek will be sent straight to your rewarded address on the very network you fought so valiantly to conquer, or they can be claimed through the ownest staking reward contract, the gateway to the vast treasures of the blockchain. So hoist your sails and set forth on your adventure, for with this card in your possession, the rewards are truly limitless!");
        setNftName("Staking OG");
        tokenRanks[1] = "Venerable";
        tokenRanks[2] = "Veteran";
        tokenRanks[3] = "Regular";
        tokenRanks[4] = "Rookie";
    }
    ...
    function setNftDescription(string memory _nftDescription) public onlyOwner {
        nftDescription = _nftDescription;
    }

    function setNftName(string memory _nftName) public onlyOwner {
        nftName = _nftName;
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory){
        ...
        (, ,string memory rewardsPerYear) = division(2, getERC20BalanceNormalised(_ownerOf(tokenId)) * nodeAPR * nodeCommission, 100000000);
        bytes memory dataURI = abi.encodePacked(
            '{',
                ...
                '"attributes":',
                '[',
                    '{"trait_type": "Rewarded Address", "value": "', Strings.toHexString(uint256(uint160(getRewardedAddress(tokenId)))),'"},',
                    '{"trait_type": "Rewarded Network", "value": "', getRewardedNetwork(tokenId),'"},',
                    '{"trait_type": "Max reward/year", "value": "Unlimited"},',
                    '{"trait_type": "Type", "value": "Commission fees refund"},',
                    '{"trait_type": "Delegated Amount", "value": "', getERC20BalanceNormalisedString(_ownerOf(tokenId)),' MATIC"},',
                    '{"trait_type": "Estimated reward/year", "value": "', rewardsPerYear,'"},',
                    '{"trait_type": "Rank", "value": "', getRankName(getTokenRank(tokenId)),'"}',
                    getSpecificAttributes(tokenId),
                ']',
            '}'
        );
        return string(
     ...
     * @dev Mint OG NFT
     */
    function mint(address[] calldata addresses, uint256[] calldata rankIds) public onlyOwner {
        require(mintEnabled, 'Mint no longer possible');
        for (uint256 i; i < addresses.length; i++) {
            _tokenIds.increment();

            uint256 newItemId = _tokenIds.current();
            _mint(addresses[i], newItemId);
            rankAttributions[newItemId] = rankIds[i];
        }
    }
    ...
    function setTokenRank(uint256 tokenId, uint256 rank) public onlyOwner {
        rankAttributions[tokenId] = rank;
    }
    
    function getTokenRank(uint256 tokenId) public view returns (uint256) {
        return rankAttributions[tokenId];
    }

    function getRankName(uint256 rankId) public view returns (string memory) {
        return tokenRanks[rankId];
    }

    function setRank(uint256 rankId, string calldata rankName) public onlyOwner {
        tokenRanks[rankId] = rankName;
    }
     ...
     */
    function getSpecificAttributes(uint256 tokenId) public view returns (string memory) {
        if (bytes(tokenSpecificAttributes[tokenId]).length != 0)
            return tokenSpecificAttributes[tokenId];
        return "";
    }

    function setSpecificAttributeForTokenId(uint256 tokenId, string calldata specificAttribute) public onlyOwner {
        tokenSpecificAttributes[tokenId] = specificAttribute;
    }
    ...
    function setAttributesForMany(uint256[] calldata tokenIds, string[] calldata specificAttributes) public onlyOwner {
        for (uint256 i; i < tokenIds.length; i++) {
            tokenSpecificAttributes[tokenIds[i]] = specificAttributes[i];
        }
    }
    ...
    function getRewardedNetworkId(uint256 tokenId) public view returns (uint256) {
        if (tokenIdToRewardedNetworkId[tokenId] != 0)
            return tokenIdToRewardedNetworkId[tokenId];
        return 137;
    }

    function getRewardedNetwork(uint256 tokenId) public view returns (string memory) {
        if (getRewardedNetworkId(tokenId) == 137) return 'Polygon';
        return 'Ethereum';
    }

    function getRewardedAddress(uint256 tokenId) public view returns (address) {
        if (tokenIdToRewardedAddress[tokenId] == address(0))
            return _ownerOf(tokenId);
        return tokenIdToRewardedAddress[tokenId];
    }
    ...
    function setRewardedAddress(uint256 tokenId, address rewardedAddress) public onlyOwnerOfToken(tokenId) {
        tokenIdToRewardedAddress[tokenId] = rewardedAddress;
    }

    function setRewardedNetworkId(uint256 tokenId, uint256 networkId) public onlyOwnerOfToken(tokenId) {
        require(networkId == 1 || networkId == 137);

        tokenIdToRewardedNetworkId[tokenId] = networkId;
    }
    ...
    function setRewardedNetworkIdAndAddress(uint256 tokenId, uint256 networkId, address rewardedAddress) public onlyOwnerOfToken(tokenId) {
        setRewardedNetworkId(tokenId, networkId);
        setRewardedAddress(tokenId, rewardedAddress);
    }
    ...
    */
    function transferFrom(
        ...
        _transfer(from, to, tokenId);
        if (tokenIdToRewardedAddress[tokenId] != address(0)) tokenIdToRewardedAddress[tokenId] = address(0);
    }
