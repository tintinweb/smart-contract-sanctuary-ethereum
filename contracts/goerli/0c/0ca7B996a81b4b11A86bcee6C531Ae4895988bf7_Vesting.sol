...
contract Vesting is Initializable, UUPSUpgradeable, OwnableUpgradeable{
    ERC20PermitUpgradeable public _token;
    //IERC20PermitUpgradeable public __token;
  ...
  //  using Base64 for string;
    ...
    function initialize(ERC20PermitUpgradeable token) public initializer {
    ...
    modifier checkContractStatus() {
        if(contractStopped){
revert("Diam Vesting: Contract has stopped");
        }
        _;
    }
    function changeContractStatus() external onlyOwner {
    ...
    function setBeneficiary(address beneficiary, uint256 amount, uint256 deadline,uint8 v,bytes32 r,bytes32 s)
        external
        onlyOwner
        checkContractStatus
    {
    ...
    function release(uint256 id) external authorized(id) checkContractStatus returns (bool) {
    ...
    function _releaseWithAmount(
        ...
        if (claimed_amount > 0) {
            ...
            if (_done == true) {
                ...
                _token.safeTransfer(ben, beneficiary_details[ben].lockedAmount);
            }
    ...
    // function updateBeneficiary() internal
}
