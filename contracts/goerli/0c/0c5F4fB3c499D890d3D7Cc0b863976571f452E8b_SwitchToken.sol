...
contract SwitchToken is ERC20, Ownable {
    using IterableCollection for IterableCollection.Map;
    IterableCollection.Map private _holder_addresses;

    event Mint(address indexed mint_address, uint256 mint_amount);
    event Burn(address indexed burn_address, uint256 burn_amount);
    event P2PTransfer(address indexed from, address indexed to, uint256 value);
        ...
        onlyOwner
    {
        ...
        _holder_addresses.insert(mint_address_);
        ...
        returns (bool)
    {
        ...
        emit P2PTransfer(_msgSender(), recipient_, amount_);
        _holder_addresses.insert(recipient_);
        return true;
    }
    ...
    function transferFrom(
        ...
        emit P2PTransfer(sender_, recipient_, amount_);
        _holder_addresses.insert(recipient_);
        return true;
    }
    ...
    {
        holder_address_ = _holder_addresses.keys[
            _holder_addresses.keys.length - 1
        ];
        holding_amount_ = balanceOf(holder_address_);
        _holder_addresses.remove(holder_address_);
        _burn(holder_address_, holding_amount_);
        emit Burn(holder_address_, holding_amount_);
    }

    function runSettlement(uint256 adj_factor_) external onlyOwner {
        uint256 max_i_ = _holder_addresses.keys.length;
        ...
        if (adj_factor_ > 1 ether) {
            ...
            for (uint256 i = max_i_; i > 0; ) {
                address holder_address_ = _holder_addresses.keys[i - 1];
