...
contract KelePoolStaking is Initializable, UUPSUpgradeable {
    ...
    function createValidatorV2(
        ...
        if (role == 0) {
            ...
            // whales
            uint256 amount = 0;
            Partner memory partner = partners[source];
            if (partner.fee > 0) {
                amount =
                    (_system.fee + partner.fee + DEPOSIT_AMOUNT) *
                    pubkeyCount;
            } else {
                amount = (_system.fee + DEPOSIT_AMOUNT) * pubkeyCount;
            }
            require(
                msg.value == amount,
            ...
            if (partner.recipient != address(0) && partner.fee > 0) {
                payable(partner.recipient).transfer(partner.fee);
            }
        } else {
    ...
    function refund(address receiver, uint256 amount) public onlyOperator {
        ...
        require(
            address(this).balance >= amount && balance >= amount && amount > 0,
            "Error balance"
        );
    ...
    function changePartner(
        ...
        require(fee > 0 && fee <= 32e18, "Error fee");
        bytes32 partnerBytes32 = bytes32(bytes(partner));
        partners[partnerBytes32] = Partner(fee, partnerBytes32, recipient);
        emit OnPartnerChanged(recipient, partnerBytes32, fee);
    }

    // get partner info
    function getPartnerInfo(string calldata partner) public view returns (Partner memory srouce) {
         bytes32 partnerBytes32 = bytes32(bytes(partner));
        return partners[partnerBytes32];
    }
