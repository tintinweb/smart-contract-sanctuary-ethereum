...
//ownable -- contract for ownership of contract.
//openzeplin interface
abstract contract Ownable {
    ...
    //abstract contract with all virtual functions
    //public is for internal as well external use
    //external is for external use only
    ...
    //check message.sender against contract owner for validation
    function _checkOwner() internal view virtual {
        require(owner() == msg.sender, "Ownable: caller is not the owner");
    }

    //renounce ownership here will remove the owner of the contract and the contract will have no owner.
    function renounceOwnership() public virtual onlyOwner {
        _transferOwnership(address(0));
    }

    //check if transfer the ownership works
...
//    using SafeMath for uint256;
    ...
    struct User {
        address userAddress;        //staking user addr
    ...
    mapping (uint8=>uint256) lockTimes;                 //lockchoice => lockTime
    mapping (address=>User) usersList;                  //public address => user
    mapping (uint256=>UserStake) stakesList;            //unique stakeId=> userStake
    mapping (address => uint256[]) userStakes;          //address => mutiple stakes done by the suer
    mapping (uint8=>uint256) interestRates;             //interest rate 0 1 2
    mapping (address=>uint256) userTotalStakeBalance;   //address => all stakes for particular user.

    
    uint256 stakeCount;                 //stake Id
    uint256 public totalTokensStaked;   //total staked for paticular address.
    ...
    //staking opened / close only owner
    ...
    // redeeem referal balance fro user account.
