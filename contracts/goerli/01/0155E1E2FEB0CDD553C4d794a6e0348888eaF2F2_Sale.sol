...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => mapping(address => mapping(uint256 => bool))) public tokenMultiIsBoughts;
    mapping(uint256 => mapping(address => mapping(uint256 => uint256))) public amountMultiTokenBought;
    ...
    mapping(uint256 => mapping(uint256 => uint256)) private amountMultiTokenOfPacks;

    event Creates(uint256 indexed projectId, SaleInfo[] sales);
    event Bid(BidEvent bidEvent);
    ...
    /**
     * @dev Throws if caller not is the manager.
     */
    ...
    /**
     * @dev Throws if caller not not is the controller.
     */
    ...
    /**
     * @dev Throws if caller not is the Project contract.
     */
    ...
    /**
     * @dev Throws if sale not valid when call bid function.
     */
    ...
    function creates(address _caller, bool _isCreateNewToken, bool _isSaleWithPacks, ProjectInfo memory _projectInfo, SaleInput[] memory _saleInputs) external onlyProject returns (uint256) {
        ...
        for (uint256 i; i < _saleInputs.length; i++) {
            (SaleInfo memory saleInfo, uint256 amountSale) = _createSale(_caller, ++id, _isCreateNewToken, _isSaleWithPacks, _projectInfo, _saleInputs[i]);
            _sales[i] = saleInfo;
            totalAmount += amountSale;
        }
    ...
    function _createSale(address _caller, uint256 _saleId, bool _isCreateNewToken, bool _isSaleWithPacks, ProjectInfo memory _project, SaleInput memory _saleInput) private returns (SaleInfo memory, uint256) {
        if (_isSaleWithPacks) require(_saleInput.tokenIds.length > 0 && _saleInput.tokenIds.length == _saleInput.amounts.length, "Invalid saleInput");

        if (!_project.isRaise) {
            require(_saleInput.maxPrice > _saleInput.minPrice, "Invalid price");
        ...
        if (_isSaleWithPacks) {
            uint256 tokenIdsLength = _saleInput.tokenIds.length;
            sale.packTokenIds = _saleInput.tokenIds;
            sale.warePackTokenIds = _saleInput.tokenIds;
            
            if (_project.isSingle) {
                totalAmount += tokenIdsLength;
                for (uint256 i; i < tokenIdsLength; i++) {
                    _mintOrTransfer721FromToSale(_project.token, _caller, _isCreateNewToken, _saleInput.tokenIds[i]);
                }
            } else { 
                for (uint256 i; i < tokenIdsLength; i++) {
                    require(_saleInput.amounts[i] > 0, "Invalid amount");
                    _mintOrTransfer1155FromToSale(_project.token, _caller, _isCreateNewToken, _saleInput.tokenIds[i], _saleInput.amounts[i]);
                    amountMultiTokenOfPacks[_saleId][_saleInput.tokenIds[i]] = _saleInput.amounts[i];
                    totalAmount += _saleInput.amounts[i];
                }
            }
        } else {
            if (!_project.isSingle) require(_saleInput.amount > 0, "Invalid amount");
            totalAmount += _project.isSingle ? 1 : _saleInput.amount;
            _project.isSingle ? _mintOrTransfer721FromToSale(_project.token, _caller, _isCreateNewToken, _saleInput.tokenId) : _mintOrTransfer1155FromToSale(_project.token, _caller, _isCreateNewToken, _saleInput.tokenId, _saleInput.amount);
        }
    ...
    function _mintOrTransfer721FromToSale(address _token, address _transferFrom, bool _isMint, uint256 _tokenId) private {
        _isMint ? IOSB721(_token).mint(address(this), _tokenId) : IOSB721(_token).safeTransferFrom(_transferFrom, address(this), _tokenId);
    }

    function _mintOrTransfer1155FromToSale(address _token, address _transferFrom, bool _isMint, uint256 _tokenId, uint256 _amount) private {
        _isMint ? IOSB1155(_token).mint(address(this), _tokenId, _amount) : IOSB1155(_token).safeTransferFrom(_transferFrom, address(this), _tokenId, _amount, "");
    }
    ...
    function bid(uint256 _saleId, bytes32[] memory _merkleProof, uint256 _amount) external payable nonReentrant saleValid(_saleId) {
        ...
        TokensBought memory tokensBought; // use to pack

        if (!_project.isSaleWithPacks) require(_project.isSingle ? _amount == 1 : _amount > 0 && _amount <= sale.amount, "Invalid amount");
        else require(_amount > 0 && _amount <= sale.amount, "Invalid amount");
        ...
        if (_project.isInstantPayment && soldAmount == _project.amount) {
            project.end(_project.id);
        } 
        ...
        if (_project.isSaleWithPacks) tokensBought = _bidPack(_project, sale, _amount);

        if (!_project.isSaleWithPacks && _project.isInstantPayment) {
        ...
        _sharing(_project, sale, tokensBought, _amount, _price); 
    }

    function _bidPack(ProjectInfo memory _project, SaleInfo storage _sale, uint256 _amount) private returns (TokensBought memory) {
        _sale.warePackTokenIds = Helper.shuffle(_sale.warePackTokenIds);
        uint256 sentByIndex = _sale.warePackTokenIds.length - 1;
        uint256 amountSent = _amount;
        TokensBought memory tokensBought;

        if (_project.isSingle) {
            tokensBought.tokenIds = new uint256[](_amount);
            for (uint256 i; i < amountSent; i++) {
                if (_project.isInstantPayment) IOSB721(_project.token).safeTransferFrom(address(this), _msgSender(), _sale.warePackTokenIds[sentByIndex - i]);
                tokensBought.tokenIds[i] = _sale.warePackTokenIds[sentByIndex - i];
                _sale.warePackTokenIds.pop();
            }
        } else {
            tokensBought.tokenIds = new uint256[](_sale.warePackTokenIds.length);
            tokensBought.amounts = new uint256[](tokensBought.tokenIds.length);
            for (uint256 i; i < tokensBought.tokenIds.length; i++) {
                uint256 tokenId = _sale.warePackTokenIds[sentByIndex - i];
                tokensBought.tokenIds[i] = tokenId;
                tokensBought.length += 1;
                uint256 currentTokenAmount = amountMultiTokenOfPacks[_sale.id][tokenId];
                uint256 amountSend;

                if (currentTokenAmount > amountSent) {
                    amountMultiTokenOfPacks[_sale.id][tokenId] -= amountSent;
                    amountSend = amountSent;
                } else {
                    amountMultiTokenOfPacks[_sale.id][tokenId] = 0;
                    amountSend = currentTokenAmount;
                    _sale.warePackTokenIds.pop();
                }

                tokensBought.amounts[i] = amountSend;
                amountSent -= amountSend;
                amountMultiTokenBought[_sale.id][_msgSender()][tokenId] += amountSend;
                if (_project.isInstantPayment) IOSB1155(_project.token).safeTransferFrom(address(this), _msgSender(), tokenId, amountSend, "");
                if (amountSent == 0) break;
            }
        }
    ...
    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, TokensBought memory _tokensBought, uint256 _amount, uint256 _price) private {
        ...
        if (_project.minSales == 0 && _project.isInstantPayment) {
            ...
            if (_project.isSaleWithPacks) {
                if (_project.isSingle) {
                    for (uint256 i; i < _tokensBought.tokenIds.length; i++) {
                        billInfo.packIds.push(_tokensBought.tokenIds[i]);
                    }
                } else {
                    for (uint256 i; i < _tokensBought.length; i++) {
                        if (!tokenMultiIsBoughts[_sale.id][_msgSender()][_tokensBought.tokenIds[i]]) {
                            billInfo.packIds.push(_tokensBought.tokenIds[i]);
                            tokenMultiIsBoughts[_sale.id][_msgSender()][_tokensBought.tokenIds[i]] = true;
                        } 
                    }
                }
            }
        ...
        BidEvent memory bidEvent;
        bidEvent.saleId = _sale.id;
        bidEvent.tokenId = _sale.tokenId;
        bidEvent.tokenIds = _tokensBought.tokenIds;
        bidEvent.amounts = _tokensBought.amounts;
        bidEvent.amountRequest = _amount;
        bidEvent.profitShare = _project.profitShare;
        bidEvent.supperAdminProfit = supperAdminProfit;
        bidEvent.royaltyProfit = royaltyProfit;
        bidEvent.sellerProfit = sellerProfit;
        bidEvent.residualPrice = residualPrice;
        bidEvent.buyer = _msgSender();
        ...
        for (uint256 i; i < _accountBuyers.length; i++) {
            if (getBill(_sale.id, _accountBuyers[i]).account == _accountBuyers[i]) {
                ...
                if (!_project.isSaleWithPacks) {
                    ...
                    if (_project.isSingle) {
                        for (uint256 j; j < billInfo.packIds.length; j++) {
                            IOSB721(_project.token).safeTransferFrom(address(this), receiver, billInfo.packIds[j]);
                        }
                    } else {
                        for (uint256 j; j < billInfo.packIds.length; j++) {
                            IOSB1155(_project.token).safeTransferFrom(address(this), receiver, billInfo.packIds[j], amountMultiTokenBought[_sale.id][billInfo.account][billInfo.packIds[j]], "");
                        }
                    }
                }
            }
    ...
    function getAmountMultiTokenOfPacks(uint256 saleId, uint256 _tokenId) external view returns (uint256) {
        return amountMultiTokenOfPacks[saleId][_tokenId];
    }
...
library Helper {
	...
	function shuffle(uint256[] memory numberArr) internal pure returns(uint256[] memory) {
        for (uint256 i = 0; i < numberArr.length; i++) {
            uint256 n = i + uint256(keccak256(abi.encodePacked(i))) % (numberArr.length - i);
...
interface ISale {
    ...
    function creates(address _caller, bool _isCreateNewToken, bool _isSaleWithPacks, ProjectInfo memory _projectInfo, SaleInput[] memory _saleInputs) external returns (uint256);
    function getAmountMultiTokenOfPacks(uint256 saleId, uint256 _tokenId) external view returns (uint256);
}
...
contract $Helper {
    ...
    function $shuffle(uint256[] calldata numberArr) external pure returns (uint256[] memory) {
        return Helper.shuffle(numberArr);
    }
}