...
contract EquityPool is Pausable, AccessControl, ReentrancyGuard {
    ...
    constructor(
        ...
        // we add the interval to the supplied _timedeployed (unix timestamp in seconds)
        // as we only start the interval after a buffer of `intervalSeconds` seconds
        // to allow time to add and update new stakeholders
        lastEndOfIntervalTimestamp = _timedeployed.add(intervalSeconds);
    }
