...
// File: contracts\Apemotorcycleclubtest.sol
...
contract TESYUU is ERC721, Ownable {
    using Strings for uint256;
    ...
    function whitelistMint(uint256 _quantity) external payable nonReentrant {
        ...
        require(
            _quantity > 0 && (balanceOf(_to) + _quantity) < WL_MAX_PER_WALLET,
    ...
    function publicMint(uint256 _quantity) external payable nonReentrant {
        ...
        address _to = msg.sender;
        require(
            _quantity > 0 && (balanceOf(_to) + _quantity) < PUB_MAX_PER_WALLET,
            "Invalid mint quantity."
        );
