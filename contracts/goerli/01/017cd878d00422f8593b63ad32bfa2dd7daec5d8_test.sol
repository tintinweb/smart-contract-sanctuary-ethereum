...
contract test is ERC721Enumerable, Ownable, MerkleProof, ReentrancyGuard, ERC2981 {
  using Strings for uint256;
  string baseURI;
  uint256 public wlMintPrice = 0.001 ether; //本番デプロイ時変更
  uint256 public publicMintPrice = 0.003 ether; //本番デプロイ時変更
  uint256 public maxMintsPerPublic = 10; //本番デプロイ時変更
  uint256 public maxMintsPerWL = 5; //本番デプロイ時変更
  uint256 public maxSupply = 50; //本番デプロイ時変更
  ...
  mapping(address => uint256) internal _publicMinted;
  ...
  function setPublicPrice(uint256 newPrice) external virtual onlyOwner {
  ...
  function publicMinted(address _address) external view virtual returns (uint256){
    return _publicMinted[_address];
  }
  ...
  function setPublicMaxMints(uint256 _max) external virtual onlyOwner {
    maxMintsPerPublic = _max;
  }
  ...
  function publicMint(uint256 _amount) external payable virtual nonReentrant {
    ...
    require(maxMintsPerPublic >= _amount, "publicMint: Over max mints per wallet");
    require(maxMintsPerPublic >= _publicMinted[msg.sender] + _amount, "You have no publicMint left");
    require(msg.value == publicMintPrice * _amount, "ETH value is not correct");
    require((_amount + totalSupply()) <= (maxSupply), "No more NFTs");
      
    _publicMinted[msg.sender] += _amount;
    _safeMint(msg.sender, _amount);
  }

  //Free mint
  function freeMint(uint256 _amount, uint256 freeMintCount, bytes32[] memory proof_) external virtual nonReentrant {
    require(isFreeMintEnabled, "Free Mint is Paused");
    require(isValidFreeCount(msg.sender, freeMintCount, proof_), "Invalid Vip Count!");
    require(freeMintCount > 0, "You have no VIP!");
    require(freeMintCount >= _amount, "FreeMint: Over max mints per wallet");
    require(freeMintCount >= _freeMinted[msg.sender] + _amount, "You have no FreeMint left");
    require((_amount + totalSupply()) <= (maxSupply), "No more NFTs");
