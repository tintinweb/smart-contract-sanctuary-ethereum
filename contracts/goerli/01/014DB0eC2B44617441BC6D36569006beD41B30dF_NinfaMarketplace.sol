    ...
    RoyaltyEngineV1
{
    ...
    modifier onlyWhitelisted(address _collection) {
        // ERC721 and ERC1155 must be whitelisted.
        require(
            marketplaceFees[_collection] > 0 ||
                NinfaERC1155Factory.exists(_collection)
        );
        _;
    }
    ...
    ) public onlyWhitelisted(_collection) {
     ...
     * todo if this internal function is still being called only from one place, then remove it and copy the code inside the calling function, i.e. submitOrderDeploySplitter
     */
    ...
    function submitOffer(
        ...
        uint256 _tokenId,
        uint256 _amount
    ) external payable onlyWhitelisted(_collection) {
        require(msg.value > 0);
     ...
     */
    function cancelOffer(uint256 _offerId) external onlyOfferOwner(_offerId) {
    ...
    //     acceptOffer(_offerId, _orderId, commissionReceiver, commissionShares);
    // }
    ...
    function _trade(
        ...
        if (trade.orderId != 0) {
            ...
            require(msg.sender == orders[trade.orderId].from);
    ...
    function initAddresses(
        ...
        // adds NinfaERC721 to whitelist for access control in `onlyWhitelisted` modifier, also sets secondary fees for NinfaERC721 and all NinfaERC1155 to 5%. In order to reset secondary fees call whitelistCollection() from an ADMIN_ROLE address
     ...
     * @dev this function needs to be called immediately after deployment in order to set marketplace fees for NinfaERC721 to 500bps or 5%. Required for access control by `onlyWhitelisted` modifier, also required in order to set secondary sales fees for NinfaERC721 and all NinfaERC1155 to 5%.
     * @param _collection address of collection ERC721 or ERC1155
     * @param _collectionFees adds a uint to a mapping, if uint is positive the onlyWhitelisted modifier will return true, the uint corresponds to the secondary fee basis points, the maximum should be < 8501.
     */
...
contract RoyaltyEngineV1 {
     ...
     */
    address private constant ROYALTY_REGISTRY =
        ...
        returns (address payable/*[] memory*/ recipients, uint256/*[] memory*/ amounts)
    {
        ...
        try
            ...
            this._getRoyalty{gas: 50000 /*100000*/}(tokenAddress, tokenId, value)
        returns (
            address payable[] memory _recipients,
            uint256[] memory _amounts
        ) {
            return (_recipients[0], _amounts[0]);
        } catch {
