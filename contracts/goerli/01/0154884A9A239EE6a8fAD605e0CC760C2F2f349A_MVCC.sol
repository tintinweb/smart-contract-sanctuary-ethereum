...
interface IMVCCRenderer {
    function tokenURI(uint256 tokenId, uint256 dna)
        external
        view
        returns (string memory);

    function getIndexes(uint256 dna)
        external
        view
        returns (string memory indexes);

    function setUniquesDrawn(bool state, uint256 randomNumber) external;
}
...
import "./interfaces/IMVCCRenderer.sol";
...
contract MVCC is ERC721A, Ownable, ReentrancyGuard, ERC2981, OperatorFilterer {
    ...
    uint8[6] public numSkillsPerTrait;
    mapping(address => uint256) public publicMinted;
    mapping(address => uint256) public whitelistMinted;
    mapping(uint256 => TokenInfo) public tokenInfo;

    // Playing info
    bool public playingOpen = true;
    mapping(uint256 => uint256) public playingStarted;
    mapping(uint256 => uint256) public playingTotal;
    ...
    function mint(uint256 quantity, uint256[6][] calldata skills)
        ...
        nonReentrant
    {
        if (saleState != Period.PUBLIC) revert Errors.SaleNotStarted("Public");
        if (publicMinted[msg.sender] + quantity > publicMintLimit) {
            revert Errors.MintLimitReached();
        }
        if (quantity != skills.length) revert Errors.LengthMismatch();
        if (msg.value < quantity * price) revert Errors.PriceTooLow();
        if (quantity > batchLimit) revert Errors.BatchLimitReached();
        if (!_validSum(quantity, skills)) revert Errors.InvalidStats();
    ...
    function whitelistMint(
        ...
        /* Checks */
        if (saleState != Period.WHITELIST)
            revert Errors.SaleNotStarted("whitelist");
        if (whitelistMinted[msg.sender] + quantity > whitelistMintLimit)
            revert Errors.MintLimitReached();
        if (quantity != skills.length) revert Errors.LengthMismatch();
        if (msg.value < quantity * whitelistPrice) revert Errors.PriceTooLow();
        if (quantity > batchLimit) revert Errors.BatchLimitReached();
        if (!_validSum(quantity, skills)) revert Errors.InvalidStats();
        if (totalSupply() + quantity > maxSupply)
            revert Errors.MaxSupplyReached();
    ...
    function getRefund(uint256 id) external nonReentrant {
        if (ownerOf(id) != msg.sender) revert Errors.NotOwner();

        TokenInfo storage info = tokenInfo[id];
        bytes32 hash = blockhash(info.mintedAt);
        bytes32 random = keccak256(abi.encodePacked(hash, id));

        if (uint256(hash) == 0 || info.refunded)
            revert Errors.RefundUnavailable();
        if (totalRefunded >= (maxSupply * maxRefundBasis) / denominator) {
            revert Errors.RefundUnavailable();
        }

        if (uint256(random) % 100 < refundChance) {
            info.refunded = true;
            totalRefunded += 1;
        ...
        returns (bool refundAvailable)
    {
        ...
        if (
            ...
            refundAvailable = uint256(random) % 100 < refundChance
                ? true
                : false;
        }
    }
    ...
    function _setSkill(
        ...
        if (skillIndex >= numSkillsPerTrait[typeIndex])
            revert Errors.InvalidStats();
    ...
    function togglePlaying(uint256 tokenId) internal {
        ...
        if (start == 0) {
            if (!playingOpen) revert Errors.PlayingNotAllowed();
        ...
        returns (string memory)
    {
        ...
        IMVCCRenderer renderer = IMVCCRenderer(renderingContract);

        return renderer.tokenURI(tokenId, info.dna);
    }

    function getDNAIndexes(uint256 tokenId)
        public
        view
        returns (string memory indexes)
    {
        ...
        indexes = renderer.getIndexes(info.dna);
    }
        ...
        if (from != address(0)) {
            for (uint256 i; i < quantity; i++) {
                if (playingStarted[startTokenId + i] != 0) {
                    revert Errors.CantTransferWhilePlaying();
                }
            }
        }
    }
    ...
    function _validSum(uint256 amount, uint256[6][] calldata skills)
        internal
        pure
        returns (bool)
    {
    ...
    function getBlock() external onlyOwner {
        if (drawBlock != 0) revert Errors.AlreadyCommitted();
        drawBlock = block.number + 1;
    }

    function randomDraw() external onlyOwner {
        if (block.number <= drawBlock || drawBlock == 0)
            revert Errors.MissingCommit();
        uint256 random = uint256(blockhash(drawBlock));
        IMVCCRenderer(renderingContract).setUniquesDrawn(true, random);
    }
    ...
    function expelFromCourse(uint256 tokenId) external onlyOwner {
        if (playingStarted[tokenId] == 0) revert Errors.NotPlaying();
    ...
    function airdrop(
        ...
        if (_quantity != skills.length) revert Errors.LengthMismatch();
        if (!_validSum(_quantity, skills)) revert Errors.InvalidStats();
        ...
        for (uint256 i; i < _quantity; ) {
            _setTokenData(_user, nextId, 0, false);
            _setSkills(skills[i], nextId);
    ...
    function setMaxPublicMint(uint256 newMax) external onlyOwner {
        if (newMax == 0) revert Errors.ZeroNotAllowed();
    ...
    function setBatchSize(uint256 _amount) external onlyOwner {
        if (_amount == 0) revert Errors.ZeroNotAllowed();
    ...
    function setSaleState(uint256 _state) external onlyOwner {
        if (_state > 2) revert Errors.OutOfBounds();
...
library Errors {
    error MintLimitReached(); //0x303b682f
    error PriceTooLow(); //0xdbbbe822
    error MaxSupplyReached(); //0xd05cb609
    error BatchLimitReached(); //0x479290cf
    error SaleNotStarted(string expected); //0x1ceedf60
    error TransferFailed(); //0x90b8ec18
    error NotOwner();
    error RefundUnavailable(); //0x551a40dc
    error InvalidSkill(); //0x6c97757c
    error CantTransferWhilePlaying(); //
    error PlayingNotAllowed(); //0x1e042eae
    error LengthMismatch(); //0xff633a38
    error InvalidStats(); //0x270b168d
    error MissingCommit();
...
contract MVCCOnChainMetadata is Ownable {
    ...
    address public MVCC;
    bool public uniquesDrawn;
    uint256 public randomHash;
    string public name;
    string public description;

    uint256[3] public uniques;
    ...
    constructor(string memory _name, string memory _description) {
        ...
        skillLayers[2] = ["187", "202", "219", "242", "269", "286"];
        skillLayers[3] = [
            "Ball Topper",
            "Chili Dipper",
            "Texas Wedge Professional",
            "Bump N' Run Loyalist",
            "Bunker Warrior",
            "Flop Shot Expert"
        ];
    ...
    function tokenURI(uint256 tokenId, uint256 _dna)
        ...
        returns (string memory)
    {
        uint256 raceIndex = uniquesDrawn && tokenId == uniques[0]
            ? 12
            : uniquesDrawn && tokenId == uniques[1]
            ? 13
            : uniquesDrawn && tokenId == uniques[2]
            ? 14
            : 0;
        (
    ...
    function getIndexes(uint256 tokenId, uint256 _dna)
        public
        view
        returns (string memory indexes)
    {
        uint256 raceIndex = uniquesDrawn && tokenId == uniques[0]
            ...
            ? 14
            : 0;
        (, , indexes) = getTokenData(_dna, raceIndex);
    }

    function getTokenData(uint256 _dna, uint256 _raceIndex)
        public
        view
        returns (
            string[NUM_LAYERS] memory traits,
        ...
        )
    {
        ...
        uint256[] memory selection = new uint256[](NUM_LAYERS);

        uint256 raceIndex;
        Races race;

        if (uniquesDrawn && _raceIndex != 0) {
            raceIndex = _raceIndex;
            race = Races.UNIQUE;
        } else {
            raceIndex = getRaceIndex(dna[1]);
            race = getRace(raceIndex);
        }

        for (uint8 i; i < NUM_LAYERS; i++) {
            if (i == 1) {
                selection[i] = raceIndex;
                traits[i] = layers[1][raceIndex];
            } else {
                selection[i] = getLayerIndex(dna[i], i, race);

                if (selection[i] == RACEWEIGHT[race][i].length) {
                    traits[i] = "";
                } else {
                    traits[i] = layers[i][selection[i]];
                }
            }
        }

        uint256 clothesIndex = selection[2];
        uint256 mouthIndex = selection[3];
        uint256 hatIndex = selection[5];

        // @audit - I should change the indexes string in these cases
        // CLOTHES - Red Hoodie /w Bean
        if (clothesIndex == 26 && hatIndex != 22) {
            traits[5] = layers[5][22];
            selection[5] = 22;
        }

        if (hatIndex == 22 && clothesIndex != 26) {
            traits[2] = layers[2][26];
            selection[2] = 26;
        }

        // Hoodie Up hat exclusions
        if (
            (hatIndex == 22 &&
                (mouthIndex == 11 ||
                    mouthIndex == 12 ||
            ...
            uint256 newIndex = mouthIndex % 7;
            traits[3] = layers[3][newIndex];
            selection[3] = newIndex;
        }

        for (uint256 i; i < NUM_LAYERS; i++) {
            if (i == 1) {
                indexes = string(
                ...
                indexes = bytes(indexes).length == 0
                    ? string(abi.encodePacked(selection[i].toString(), ","))
                    : i == NUM_LAYERS - 1
                    ? string(abi.encodePacked(indexes, selection[i].toString()))
                    : string(
                        abi.encodePacked(indexes, selection[i].toString(), ",")
                    );
            }
        }
    ...
    function setLayer(
        ...
        layers[layerIndex][itemIndex] = _name;
    }
    ...
    function setMVCCAddress(address _contract) external onlyOwner {
        MVCC = _contract;
    }

    function setUniquesDrawn(bool state, uint256 random) external {
        if (msg.sender != MVCC) revert NotAuthorized();
        uniquesDrawn = state;
        randomHash = random;

        uniques[0] = ((randomHash % 3332) + 1);
        uniques[1] = ((randomHash % 3333) + 3333);
        uniques[2] = ((randomHash % 3334) + 6666);
    }
