...
contract TestSubdomains is Initializable, ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    struct DomainData { bool enable; string name; }
    ...
    function _getMessageHash(address to, string memory label, bytes32 baseNode, address _contract, uint256 serial) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(
    ...
    function _getMessage(address to, string memory label, bytes32 baseNode, address _contract, uint256 serial) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(to, label, baseNode, _contract, serial));
    }

    function recover(address to, string memory label, bytes32 baseNode, uint256 serial, bytes memory signature ) public view returns (bool) {
    ...
    function setResolver(bytes32 _baseNode, address _resolver) external onlyAdmin {
    ...
    function tokenURI(uint256 ensID) public view override returns (string memory) {
        ...
        string memory tokenDescription = string(abi.encodePacked(tokenName,' a Zingot ID and fully functional ENS Subdomain.\n\nGet yours at https://dojicrew.com/ids'));
