...
contract PoolFactory is IPoolFactory, OwnableUpgradeable, EIP712Upgradeable, UUPSUpgradeable {
    ...
    bytes32 internal constant _CREATE_TYPEHASH =
        keccak256(
            "Create(address guildMaster,uint256 maxTokensHardCap,uint256 investmentCategoryId,uint256 rewardsCategoryId,bytes32 tokenName,bytes32 tokenSymbol)"
        );
    ...
    mapping(address => InvestmentPoolInfo) internal _investmentPoolsInfo;
    ...
    function __PoolFactory_init(
        ...
        for (uint256 i = 0; i < categoryNames_.length; i++) {
            _updateCategory(type(uint256).max, categoryNames_[i], categoryTokens_[i], true);
        }
    ...
    function deployInvestmentPool(
        ...
        bytes32 structHash_ = keccak256(
            abi.encode(
                ...
                keccak256(bytes(deployParams_.tokenName)),
                keccak256(bytes(deployParams_.tokenSymbol))
            )
        );
        ...
        IInvestmentPool(newPool_).__InvestmentPool_init(
            ...
            deployParams_.tokenName,
            deployParams_.tokenSymbol
        );

        _investmentPoolsInfo[newPool_] = InvestmentPoolInfo(
            _investmentPools.length(),
            deployParams_.guildMaster
        );
    ...
    function updateCategory(
        ...
        _updateCategory(categoryId_, categoryName_, tokensArr_, isAdding_);
    }
    ...
    function getInvestmentPoolInfo(address poolAddr_)
        external
        view
        override
        returns (InvestmentPoolInfo memory)
    {
        return _investmentPoolsInfo[poolAddr_];
    }
    ...
    function _updateCategory(
        ...
        if (bytes(categoryName_).length > 0) {
            _categoryInfo.categoryName = categoryName_;
        }

        _updateAddressSet(_categoryInfo.categoryTokens, categoryTokens_, _isAdding);

        emit CategoryUpdated(currentCategoryId_, categoryName_, categoryTokens_, _isAdding);
    }
...
interface IPoolFactory {
    ...
    /**
     * @notice A structure that stores information about a certain investment pool
     * @param poolIndex an index of the cuurent investment pool
     * @param guildMaster a guild master of the cuurent investment pool
     */
     ...
     * @param tokenName a name of the token, which is a new investment pool
     * @param tokenSymbol a symbol of the token, which is a new investment pool
     */
     ...
     * @param tokenName a token name of the deployed pool
     * @param tokenSymbol a token symbol of the deployed pool
     */
     ...
     * @param categoryTokens a category tokens that have been updated
     * @param isAdding true if tokens were added, false - otherwise
     */
    event CategoryUpdated(
        uint256 categoryId,
        string categoryName,
        address[] categoryTokens,
        bool isAdding
    );
     ...
     * @param tokensArr_ an addresses of tokens to add/remove (pass an empty array if you don't want to update the tokens)
     * @param isAdding_ true to add new tokens, false to remove
     */
    function updateCategory(
        uint256 categoryId_,
        string calldata categoryName_,
    ...
    /**
     * @notice Function to get the information about investment pool by pool address
     * @param poolAddr_ an address of the pool for which you want to get information
     * @return InvestmentPoolInfo structure
     */
    function getInvestmentPoolInfo(address poolAddr_)
        external
        view
...
interface IInvestmentPool {
     ...
     * @param name_ a name of the ERC721 token
     * @param symbol_ a symbol of the ERC721 token
     */
