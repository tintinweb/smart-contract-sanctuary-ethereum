...
contract NftfiBundler is ERC9981155Extension, ERC998ERC20Extension, IBundleBuilder {
    ...
    function _addBundleElements(uint256 bundleId, BundleElements memory _bundleElements) internal {
        require(
    ...
    function _removeBundleElements(uint256 bundleId, BundleElements memory _bundleElements) internal {
        require(
            _bundleElements.erc721s.length > 0 ||
                _bundleElements.erc20s.length > 0 ||
                _bundleElements.erc1155s.length > 0,
        ...
        for (uint256 i = 0; i < _bundleElements.erc721s.length; i++) {
            address erc721Contract = _bundleElements.erc721s[i].tokenContract;
            uint256 tokenId = _bundleElements.erc721s[i].id;
        ...
        for (uint256 i = 0; i < _bundleElements.erc20s.length; i++) {
            address erc20Contract = _bundleElements.erc20s[i].tokenContract;
            uint256 balance = _bundleElements.erc20s[i].amount;
            _removeERC20(bundleId, erc20Contract, balance);
            IERC20(erc20Contract).safeTransfer(msg.sender, balance);
            emit TransferERC20(bundleId, msg.sender, erc20Contract, balance);
        }

        for (uint256 i = 0; i < _bundleElements.erc1155s.length; i++) {
            address erc1155Contract = _bundleElements.erc1155s[i].tokenContract;
            for (uint256 j = 0; j < _bundleElements.erc1155s[i].ids.length; ++j) {
                uint256 tokenId = _bundleElements.erc1155s[i].ids[j];
                uint256 tokenAmount = _bundleElements.erc1155s[i].amounts[j];
                _remove1155Child(bundleId, erc1155Contract, tokenId, tokenAmount);
            }
