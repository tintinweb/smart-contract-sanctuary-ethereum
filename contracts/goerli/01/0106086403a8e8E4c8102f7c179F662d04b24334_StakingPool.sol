...
contract StakingPool is ReentrancyGuard, Ownable {
    ...
    // reward duration set for 30 days
    uint256 public rewardsDuration = 86400 * 30;
    ...
    modifier updateReward(address _account) {
        ...
        uint256 balance = _balances[_account].total;
        for (uint i = 0; i < rewardTokens.length; i++) {
            token = rewardTokens[i];
            rewardData[token].rewardPerTokenStored = _rewardPerToken(token);
            rewardData[token].lastUpdateTime = lastTimeRewardApplicable(token);
            if (_account != address(0)) {
                rewards[_account][token] = _earned(_account, token, balance);
    ...
    /**
     * @dev User can claim all reward tokens and it transfers reward tokens to user's address
     *
     */
    ...
    /**
     * @dev Total balance of an user account
     * @param _user user's account address
     *
     */
    function totalBalance(
        address _user
    ) external view returns (uint256 amount) {
        return _balances[_user].total;
    }
    ...
    /**
     * @dev Shows the amount of all rewards tokens any particular user can claim
     * @param _account user's account address
     */
    function claimableRewards(
        address _account
    ) external view returns (RewardData[] memory rewardsData) {
    ...
    function _notifyReward(address _rewardsToken, uint256 _reward) internal {
        if (block.timestamp >= rewardData[_rewardsToken].periodFinish) {
            rewardData[_rewardsToken].rewardRate = _reward / rewardsDuration;
        } else {
        ...
        return
            ((_balance *
                (_rewardPerToken(_rewardsToken) -
                    userRewardPerTokenPaid[_user][_rewardsToken])) / 1e18) +
