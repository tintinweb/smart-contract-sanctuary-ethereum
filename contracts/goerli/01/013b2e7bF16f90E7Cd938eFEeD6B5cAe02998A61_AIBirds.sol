...
contract AIBirds is ERC721A, Ownable {
    ...
    function feedBird(uint256 tokenId, uint256 action) external {
    ...
    function presalePhase1(uint256 totalFree, uint256 totalPaid, bytes32[] memory proof) external payable {
        ...
        require(((presaleWalletTracker[msg.sender] == 0 ? totalPaid : presaleWalletTracker[msg.sender] + totalPaid - totalFree) < _presalePaidLimit), "Exceeds presale limit");
        require(presalePaidCounter + totalPaid < _totalPresalePaidAvailable, "No more presale paid mints available");
        uint256 quantity = presaleWalletTracker[msg.sender] == 0 ? totalFree + totalPaid : totalPaid;
    ...
    function presalePhase2(uint256 quantity, bytes32[] memory proof) external payable {
    ...
    uint private _maxSupply = 10_001; // 10,000
    function maxSupply() external view returns (uint256) {
        return _maxSupply - 1;
    }

    uint256 private _totalPresalePaidAvailable = 0;
