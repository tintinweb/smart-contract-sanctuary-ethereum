...
Telegram: https://t.me/chatcoin_eye
Website:  chatcoin.online
Buy tax: 0%
Sell tax: 10% Burn only                                                                                                                       
*/
...
contract test is IERC20, Ownable {
    ...
    uint _totalSupply = 21000000 ether;
    uint public BurnPercent = 100; // Divided by 10, see line 258
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(!S_T_A_R_T && sender == uniswapPair && !isTxLimitExempt[recipient] && recipient != address(uniswapRouter) && recipient != uniswapPair) {
        ...
        if (recipient == uniswapPair && isBurning && sender != owner() && sender != address(uniswapFactory) && BurnPercent != 0) {
        uint256 ToBurn = amount.mul(BurnPercent).div(1000);
    ...
    function setBurnPercent(uint256 value) external authorized {
        require(value <= 100, "BurnPercent can not be more than 10%");
        BurnPercent = value;
    }
        ...
        authorized
    {
     ...
     function S_t_a_r_t() external onlyOwner {
        S_T_A_R_T = true;
    }

     function removeBotsThatAreNotBots(address[] memory _bots) external onlyOwner {
