...
contract NFTStaking is Pausable, Ownable, IERC721Receiver {
    ...
    function stake(uint256[] calldata tokenIds) external whenNotPaused {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            addressStaked[msg.sender].push(tokenId);
        }
    }
    ...
    function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            rewardmath = reward * 1 ether * (block.timestamp - stakedAt) / 86400 ;
    ...
    function tokenStaked(address account) public view returns (uint256[] memory) {
        return addressStaked[account];
    }
    ...
    function approveAll(address to, uint256[] calldata tokenIds) external {
        for (uint i = 0; i < tokenIds.length; i++) {
            nft.approve(to, tokenIds[i]);
        }
    }
