...
interface IDatetime {
    function DAY_IN_SECONDS() external view returns (uint);
...
//
    function activatedByCron() public returns (string memory){
        ...
        for (uint i = 0; circuitsArray.length > i; i++) {  //for by every circuit
            ...
            // uint seconds_to_add = days_in_month * IDatetime(datetimeAddress).DAY_IN_SECONDS(); //second of the last month with an email sended
