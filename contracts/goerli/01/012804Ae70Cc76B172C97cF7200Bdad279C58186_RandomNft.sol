...
error RandomNft__OutOfBound();
error RandomNft__TranferFail();
...
contract RandomNft is ERC721URIStorage, VRFConsumerBaseV2, Ownable {
    ...
    event NftMinted(Breed indexed dogBreed, address indexed owner);
    ...
    function withdraw() public onlyOwner {
        ...
        (bool isSuccessful, ) = payable(msg.sender).call{value: amount}("");
        if (!isSuccessful) {
            revert RandomNft__TranferFail();
        }
    }
    ...
    function getChanceArray() private pure returns (uint256[4] memory) {
        return [0, 10, 40, MAX_CHANCE_VALUE];
    }
    ...
    function getBreedFromModdedRng(uint256 moddedRng) private pure returns (Breed) {
        ...
        for (uint256 i = 1; i < chanceArray.length; ++i) {
            if (moddedRng >= chanceArray[i - 1] && moddedRng <= chanceArray[i]) {
                return Breed(i - 1);
            }
        }
        revert RandomNft__OutOfBound();
    }

    /**
     * @dev Below are view & pure functions
     */
    ...
    function requestIdToSender(uint256 requestId) public view returns (address) {
