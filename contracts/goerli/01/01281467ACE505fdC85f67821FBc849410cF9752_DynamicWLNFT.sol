...
contract DynamicWLNFT is ERC721A, Ownable, MerkleWhitelist{
    ...
    function mintPublicWL(bytes32[] calldata merkleProof, uint256 numberOfTokens)
        public
        payable 
        onlyPublicWhitelist(merkleProof, NORMAL)
    {
        require(claimedAmount[msg.sender] + numberOfTokens <= MAX_MINTS, "Total amount claimed exceeds max 5 NFTs!");
    ...
    function mintSpecialWL(bytes32[] calldata merkleProof, uint256 numberOfTokens)
        public
        payable 
        onlyPublicWhitelist(merkleProof, SPECIAL) //TODO: add SpecialWL
    {
        require(claimedAmount[msg.sender] + numberOfTokens <= MAX_MINTS, "Total amount claimed exceeds max 5 NFTs!");
        claimedAmount[msg.sender] += numberOfTokens;
        _safeMint(msg.sender, numberOfTokens);
...
contract MerkleWhitelist is Ownable {
    ...
    mapping(uint256 => bytes32) public publicWhitelistMerkleRoot;
        ...
        returns (uint256)
    {
        if (
            _verify(
                ...
                publicWhitelistMerkleRoot[SPECIAL]
            )
        ) {
            return SPECIAL;
        } else if (
            _verify(
                proof,
                toBytes32(msg.sender),
                publicWhitelistMerkleRoot[NORMAL]
            )
        ) {
            return NORMAL;
        } else {
  ...
  */
    ...
    function setPublicWhitelistMerkleRoot(
        bytes32 merkleRoot,
        uint256 whitelistId
    ) external onlyOwner {
        publicWhitelistMerkleRoot[whitelistId] = merkleRoot;
    }

    /*
  MODIFIER
  */
    modifier onlyPublicWhitelist(bytes32[] memory proof, uint256 whitelistId) {
        require(
            _verifyPublicSender(proof) == whitelistId,
