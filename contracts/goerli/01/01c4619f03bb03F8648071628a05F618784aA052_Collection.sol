...
mapping(address => bool) _allowAddress;
 ...
 mapping(address => mapping(uint256 => bool)) internal isClaimed;
 ...
 */
  ...
  function reserveDeemoNFT(bytes32[] calldata proof, uint256 limit)  payable external {
  ...
  function claimDeemoNFT() external {
   ...
   if (status == 1) require(isWhitelisted(msg.sender, proof, quantity,whiteListType), "not authorize");
    ...
    require((MaxSupply - mintedPerRarity[4]) >=   (_currentIndex - _burnCounter) + limit, "soldout");
  ...
  function craftDeemo(uint256[] memory tokenIds) public {
    ...
    (uint256 from,) = _safeMint(msg.sender, 1);
    tokenRarity[from] = 2;
     mintedPerRarity[3]++;
