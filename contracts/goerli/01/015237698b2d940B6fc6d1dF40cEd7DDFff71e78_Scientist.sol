...
// File: contracts\erc721-tradable-upgradeable\meta-transactions\ContentMixin.sol
...
// File: contracts\erc721-tradable-upgradeable\meta-transactions\EIP712Base.sol
...
// File: contracts\erc721-tradable-upgradeable\meta-transactions\NativeMetaTransaction.sol
...
// File: contracts\interfaces\IOperator.sol
...
// File: contracts\helpers\timelock-access\TimelockAccess.sol
...
// File: contracts\erc721-tradable-upgradeable\ERC721TradableUpgradeable.sol
...
interface IScientistRepository {
    event AddScientist(ScientistData.Scientist scientist, uint256 timestamp);
    event UpdateScientist(
        ScientistData.Scientist currentScientist,
        ScientistData.Scientist newScientist,
        uint256 timestamp
    );
    event RemoveScientist(ScientistData.Scientist scientist, uint256 timestamp);

    function addScientist(address account, uint256 tokenId, ScientistData.Scientist memory scientistData) external;
     ...
     */
    function getScientistsIndexes(address _user)
        external
        view
...
// File: contracts\scientist\abstracts\AScientistRepository.sol
...
abstract contract AScientistRepository is IScientistRepository, Multicall {
    ...
    function _addScientist(
        ...
        EnumerableSet.add(idSet, _tokenId);
        addressToMap[_account][_tokenId] = _scientist;
        userIndexesArray[_account].push(_tokenId);
        emit AddScientist(_scientist, block.timestamp);
    }

    function _removeScientist(address _account, uint256 _tokenId) internal {
        ScientistData.Scientist memory _scientist = _getScientist(_tokenId);
        address _owner = ownerOf(_tokenId);
        ...
        userIndexesArray[_account][indexInArray] = userIndexesArray[_account][
            userIndexesArray[_account].length - 1
        ];
        userIndexesArray[_account].pop();
        delete addressToMap[_account][_tokenId];
        emit RemoveScientist(_scientist, block.timestamp);
    }
    ...
    function _updateScientist(
        ...
        ScientistData.Scientist memory scientist = _getScientist(_tokenId);
        require(_account != address(0), "Token not exists");
        require(_account == ownerOf(_tokenId), "wrong owner of Scientist");
        emit UpdateScientist(scientist, _scientist, block.timestamp);
        scientist = _scientist;
        addressToMap[_account][_tokenId] = scientist;
    }
...
// File: contracts\scientist\interfaces\IScientistCharacteristic.sol
...
// File: contracts\scientist\Scientist.sol
    ...
    OwnableUpgradeable
{
    ...
    // Constants
    uint8 public constant LENGTH_OF_SCIENTIFIC_FIELDS = 7;
    ...
    modifier isValidMerkleProof(bytes32[] calldata merkleProof, bytes32 root) {
        bytes32 leaf = getKeccakScientist(msg.sender);
    ...
    function claimable(bytes32[] calldata merkleProof, address account)
        ...
        returns (bool)
    {
        bytes32 leaf = getKeccakScientist(account);
        bytes32 hashProof = keccak256(abi.encodePacked(merkleProof));
        return
     ...
     */
    function getKeccakScientist(address account)
        public
        pure
    ...
    function _transfer(
        ...
        _addScientist(to, tokenId, scientist);
    }
    ...
    function getLeaf(address account) external pure returns (bytes32) {
        bytes32 validateData = getKeccakScientist(account);
        bytes32 leaf = keccak256(abi.encodePacked(validateData));
        return leaf;
    }

    function _create(address _account) internal {
        require(tokenIdCount.current() < MAX_MINTABLE, "Can not create more Scientist");
        ...
        _addScientist(_account, tokenId, newScientist);
    ...
    function addScientist(address account, uint256 tokenId, ScientistData.Scientist memory scientistData)
        external
        override
        isOperator
    {
        _addScientist(account, tokenId, scientistData);
    }
