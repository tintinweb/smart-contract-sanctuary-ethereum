...
/*
    ...
    Base for Building ERC721 by Degens
    Utility without fluff.
*/
...
contract TestSMF is IERC721, Ownable, Functional {
    ...
    address doxxd      = payable(0x032F009A2D3E799D229b4612a5E3EF8fB8072672); //70% Test
    address toronto    = payable(0x1271Dc0C23099D985A7f3455D99AE35a4Be810c8); //30% Test
    ...
    constructor() {
        _name = "TestSMF";
        _symbol = "TSMF";
        _baseURI = "https://strippmfers.xyz/metadata/";
        
        price = 2 * (10 ** 16); // Replace leading value with price in finney = 0.02 Eth
        gasPrice = 10 ;//* (10 ** 18) ;//5000;        // it requires update
        lovePrice = 10; //* (10 ** 18);//2500;       // it requires update
        maxSupply = 2222;
        reservedTokens = 100;
        reservedChem = 400;
        reservedGas = 100;
        ...
        CT = ERC721(0xC273350237368E9274d426E9DBF0e3eaA9ACd01e); //Chemeleons contract address.
        gasFactoryAddress = IERC20(0x44BA41150920C14a3E2C4a8ff707ad22bcF8c712);
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == TestSMF.onERC721Received.selector;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        bool success;
        //loveTokenAddress.transfer(msg.sender, loveTokenAddress.balanceOf(address(this)));
        //gasFactoryAddress.transfer(msg.sender, gasFactoryAddress.balanceOf(address(this)));
    ...
    function requireAll(uint256 qty) private view {
        ...
        require((qty + reservedTokens + tokenMinted + reservedChem) <= maxSupply,"Mint: Not enough avaialability"); 
    ...
    function publicMint(uint256 qty) external payable reentryLock {
        ...
        /*require((qty + reservedTokens + tokenMinted + reservedChem) <= maxSupply, 
        ...
        for(uint256 i = 0; i < qty; i++) {
            ...
            //_owners[mintSeedValue + i] = _msgSender();// ???
            //tokenMinted++;
        }*/
        tokensMint(qty);
    }
    ...
    function mintByChameleons(uint256 [] memory qty) external reentryLock {
        ...
        /*require((qty.length + reservedTokens + tokenMinted + reservedChem) <= maxSupply, 
            "Mint by Chemeleon: Not enough avaialability"); 
        require(qty.length <= maxMintPerTx,
            "Mint by Chemeleon: Mint tokens per transaction exceeded");
        require(( tokensMintedby[ _msgSender() ] + qty.length ) <= maxPerWallet, 
            "Mint by Chemeleon: Mint per wallet exceeded.");
            */
        requireAll(qty.length);
        require(reservedChem > 0, "Mint by Chemeleon: Not enough available");
    ...
    function mintByGas(uint256 qty, uint256 flag) external reentryLock {
        ...
        require(( tokensMintedby[ _msgSender() ] + qty ) <= maxPerWallet, 
            "Mint by Gas: Mint per wallet exceeded.");
        require((reservedGas - qty) > 0,"Gas Mint not enough available");*/
        requireAll(qty);
    ...
    /*
    function mintByGas(uint256 qty) external reentryLock {
        require(mintActive, "Mint is not Active.");
        //require((qty + reservedTokens + tokenMinted + reservedChem) <= maxSupply,"Mint: Not enough avaialability"); 
        require(qty <= maxMintPerTx,"Mint: Mint tokens per transaction exceeded");
        require(( tokensMintedby[ _msgSender() ] + qty ) <= maxPerWallet, 
            "Mint by Gas: Mint per wallet exceeded.");
        require((reservedGas - qty) > 0,"Gas Mint not enough available");
        /*if(loveTokenAddress.approve(address(this), lovePrice * qty)){
            loveTokenAddress.transferFrom(msg.sender, address(this), lovePrice * qty);
        }else if (gasFactoryAddress.approve(address(this), gasPrice * qty)){
            gasFactoryAddress.transferFrom(msg.sender, address(this), gasPrice * qty);
        }
        gasFactoryAddress.transferFrom(msg.sender, address(this), gasPrice * qty);
        uint256 mintSeedValue = tokenMinted;
    ...
    function mintByLove(uint256 qty) external reentryLock {
        require(mintActive, "Mint is not Active.");
        //require((qty + reservedTokens + tokenMinted + reservedChem) <= maxSupply,"Mint: Not enough avaialability"); 
        require(qty <= maxMintPerTx,"Mint: Mint tokens per transaction exceeded");
        require(( tokensMintedby[ _msgSender() ] + qty ) <= maxPerWallet, "Mint: Mint per wallet exceeded.");
        require((reservedlove - qty) > 0,"Gas Mint not enough available");
        loveTokenAddress.transferFrom(msg.sender, address(this), lovePrice * qty);
        uint256 mintSeedValue = tokenMinted;
        tokenMinted += qty;
        reservedlove -= qty;
...
/*    
    function getBalance(address tokenAddress) view external returns (uint256) {
        ...
        return tokensMintedby[tokenAddress]; //temporary fix to report in finneys
    }
*/
    /**
