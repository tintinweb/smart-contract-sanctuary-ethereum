...
import '@uniswap/v2-core/contracts/interfaces/IARYZE_Factory.sol';
import '@uniswap/lib/contracts/libraries/TransferHelper.sol';

import './interfaces/IARYZE_Router02.sol';
import './libraries/ARYZE_Library.sol';
...
interface IARYZE_Factory {
    ...
    function feeToRYZEVaultSetter() external view returns (address);

    function feeToRewardsVaultSetter() external view returns (address);
    ...
    function setFeeToRYZEVaultSetter(address) external;

    function setFeeToRewardsVaultSetter(address) external;
}
...
import './IARYZE_Router01.sol';

interface IARYZE_Router02 is IARYZE_Router01 {
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        address token,
        uint256 liquidity,
        uint256 amountTokenMin,
...
import '@uniswap/v2-core/contracts/interfaces/IARYZE_Pair.sol';
import './SafeMath.sol';

library ARYZE_Library {
    using SafeMath for uint256;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'ARYZE_Library: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'ARYZE_Library: ZERO_ADDRESS');
    }
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex'63d3f269cf65fdbd214e67ac19d205a6f7e98778d7266b46cbdaf2b5abc4a2ab' // init code hash keccak256(ARYZE_Pair bytecode)
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IARYZE_Pair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, 'ARYZE_Library: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'ARYZE_Library: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, 'ARYZE_Library: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'ARYZE_Library: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, 'ARYZE_Library: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'ARYZE_Library: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, 'ARYZE_Library: INVALID_PATH');
...
interface IARYZE_Router01 {
    function factory() external pure returns (address);

    function WNT() external pure returns (address);
