...
import "@openzeppelin/contracts/token/ERC20/ERC20.sol"; // openzeppelin 4.5 (for solidity 0.8.x)
...
contract OpenchainContract {
    ...
    function swapToETH(
        ...
        // transferFrom
        ERC20 sellTokenContract = ERC20(_tokenIn);
        // uint amountIn = _amountIn * 10 ** 18; //sellTokenContract.decimals();
        TransferHelper.safeTransferFrom(_tokenIn, msg.sender, address(this), _amountIn);
        // approve
        // require(sellTokenContract.approve(address(this), amountIn), 'approve for the contract failed.');

        // uint256 allowance = sellTokenContract.allowance(msg.sender, address(this));
        // string memory errorText = Strings.toString(allowance);
        // errorText = strConcat("Check the ERC20 token allowance: ", errorText);
        // require(allowance >= _amountIn, errorText);
        // sellTokenContract.transferFrom(msg.sender, address(this), amountIn);
        // require(sellTokenContract.transferFrom(msg.sender, address(this), amountIn), 'transferFrom failed.');
