...
contract Game1 is Ownable  {
   ...
   // core game
    ...
    struct NftList {
     ...
     NftList[] private nftListed;
     uint256[] private allTokenIdListed;
     uint256[] private priceTokenIdListed;
     mapping(address => uint256[]) private walletToTokenIdListed;

     modifier IsOwner(uint256 _tokenId) {
         require(nftAddress.ownerOf(_tokenId) == _msgSender(), "GAME: You do not own this NFT");
         _;
     }
     function isListByTokenId(uint256 _tokenId) public view returns (bool) {
         bool _bool;
         for(uint8 i; i < nftListed.length; i++) {
             if(nftListed[i].tokenId == _tokenId){
                 _bool = true;
             }
         }
         return _bool;
     }
     function lastSaleByTokenId(uint256 _tokenId) public view returns (uint256){
         uint256 _price;
         for(uint32 i;i < nftListed.length; i++) {
             if(nftListed[i].tokenId == _tokenId){
                _price = nftListed[i].price;
             }
         }
         return _price;
     }
     function indexOfTokenId (uint256 _tokenId) public view returns (uint256){
         uint256 _index;
         for(uint32 i; i < nftListed.length; i++) {
             if(nftListed[i].tokenId == _tokenId){
                _index = i;
     ...
     function sell (uint256 _tokenId, uint256 _price) external IsOwner(_tokenId) {
        require(isListByTokenId(_tokenId) == false, "MARKETPLACE: NFT listed");
        nftListed.push(NftList(_msgSender(), _tokenId, _price, lastSaleByTokenId(_tokenId), block.timestamp));
        walletToTokenIdListed[_msgSender()].push(_tokenId);
        allTokenIdListed.push(_tokenId);
        priceTokenIdListed.push(_price / (10**9));
        //------transfer
        nftAddress.transferFrom(_msgSender(), address(this), _tokenId);
    }

    function cancel(uint256 _tokenId) external {
        require(nftListed[indexOfTokenId(_tokenId)].owner == _msgSender(), "MARKETPLACE: You do not own Nft");
        require(isListByTokenId(_tokenId) == true, "MARKETPLACE: NFT do not listed");
        nftListed[indexOfTokenId(_tokenId)] = NftList(address(0),0,0,0,0);
        walletToTokenIdListed[msg.sender][indexOfTokenId(_tokenId) - 1] = 0;
        allTokenIdListed[indexOfTokenId(_tokenId) -1 ] = 0;
        priceTokenIdListed[indexOfTokenId(_tokenId) - 1 ] = 0;
        //----transfer
        nftAddress.transferFrom(address(this), _msgSender(), _tokenId);
    }

    function buy(uint256 _tokenId) external {
        require(isListByTokenId(_tokenId) == true, "MARKETPLACE: NFT is not listed");
        require(nftListed[indexOfTokenId(_tokenId)].owner != _msgSender(), "MARKETPLACE: You can not buy your Nft");
        require(tokenAddress.balanceOf(_msgSender()) >= nftListed[indexOfTokenId(_tokenId)].price, "MARKETPLACE: Insufficient balance");
         walletToTokenIdListed[msg.sender][indexOfTokenId(_tokenId) - 1] = 0;
         uint256 _price = nftListed[indexOfTokenId(_tokenId)].price;
        nftListed[indexOfTokenId(_tokenId)]=NftList(address(0),0,0,_price, 0);
        allTokenIdListed[indexOfTokenId(_tokenId) - 1 ] = 0;
        priceTokenIdListed[indexOfTokenId(_tokenId) - 1 ] = 0;
        //----transfer
        nftAddress.transferFrom(address(this), _msgSender(), _tokenId);
        tokenAddress.transfer(nftListed[indexOfTokenId(_tokenId)].owner, (_price * 96)/100);
    }

    function getInfoTokenListed(uint256 _tokenId) public view returns(address, uint256, uint256, uint256, uint256) {
        NftList storage _nftListed = nftListed[indexOfTokenId(_tokenId)];
        return (_nftListed.owner,_nftListed.tokenId,_nftListed.price,_nftListed.lastSale,_nftListed.time);
    }

    function tokenIdListedByOwner(address _owner) public view returns(uint256[] memory) {
        return walletToTokenIdListed[_owner];
    }

    function allNftListed() public view returns(uint256[] memory) {
        return allTokenIdListed;
    }
    function priceListed() public view returns(uint256[] memory) {
        return priceTokenIdListed;
    }
