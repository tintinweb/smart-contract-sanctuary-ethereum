...
contract OMRedPackets is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function version() pure external returns(uint){
        return 1; // version number for updating purpose
    }
        ...
        payable
    {
        ...
        // revert if there are already too many claimers o
        ...
        // revert if there user has claimed once already
        ...
        // check validity of signature. This should resolve to the packet Id with the packet's senders address
        require(_verifyClaimSignature(p.sender, p.id, signature), "Incorrect claim hash provided!");
    ...
    function retrieveFund(uint256 packetId) external payable {
        ...
        address payable receiver = payable(msg.sender);
        receiver.transfer(p.amountLeft);
        p.amountLeft = 0;    
    }
     ...
     */
    function _verifyClaimSignature(address sender, uint256 packetId, bytes memory sig) internal pure returns (bool) {
        bytes32 msgHash = keccak256(abi.encodePacked("Share my packet", packetId));
        bytes32 signedHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", msgHash));
        (bytes32 r, bytes32 s, uint8 v) = _splitSignature(sig); 
        return ecrecover(signedHash, v, r, s) == sender;
    }
