...
contract WalletFactory {
    ...
    // Just for now: can be hardcoded later on
    function getBytecode() public view returns (bytes memory) {
        bytes memory bytecode = type(BeaconProxy).creationCode;

        //return abi.encodePacked(bytecode, abi.encode(address(beacon), ""));
        return abi.encodePacked(bytecode, abi.encode(address(beacon), abi.encodeWithSelector(Wallet(address(0)).initialize.selector, address(0))));
    }

    function createWallet(bytes32 _salt) external returns(address) {
        // BeaconProxy walletProxy = new BeaconProxy{salt: _salt}(address(beacon),
        // "");
        BeaconProxy walletProxy = new BeaconProxy{salt: _salt}(address(beacon),
        abi.encodeWithSelector(Wallet(address(0)).initialize.selector, address(0)));
