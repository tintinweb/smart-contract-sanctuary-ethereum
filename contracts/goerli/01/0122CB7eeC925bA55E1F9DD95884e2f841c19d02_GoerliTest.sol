...
contract GoerliTest is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory initURI) ERC721A("GoerliTest", "GT", 3) {
        _uri = initURI;
        maxTotalSupply = 5555;
        PreMaxMint = 2;
        PublicMaxMint = 3;
    ...
    function _verify(bytes32 hash, bytes memory token, uint256 num) internal view returns (bool)
    {
        return _signerOfNum[_recover(hash, token)] >= num;
    }
    ...
    function mint(uint256 num, string calldata salt, bytes calldata token) external payable {
        ...
        require(_verify(_hash(salt, msg.sender), token, 1), "WGGE001");
    ...
    function presaleMint(
        ...
        require(_verify(_hash(salt, msg.sender), token, amount), "WGGE001");
