...
contract ChainpassMarketplace_1_0 is AuthHelper, ReentrancyGuard {
    ...
    function setSecondaryAsks(
        ...
        MetaTransactionInput calldata metaTxInput
    ) external {
        ...
        require(
            address(ticketContract) == msg.sender ||
                isSignerOrCaller(ticketOwner, MetaTransaction(metaTxInput, abi.encode("setSecondaryAsks", ticketContractAddress, tokenIds, prices))),
    ...
    function buyTickets(
        ...
        // transfer CP cut
        uint256 chainpassCut = (totalPrice * getChainpassFee(ticketContractAddress)) / 10_000;
        transferUSDC(buyer, getChainpassWallet(), chainpassCut);
    
        // transfer the rest to the event host
        transferUSDC(buyer, ticketContract.owner(), totalPrice - royaltyAmount - chainpassCut);
    ...
    function getChainpassFee(address ticketContractAddress) private view returns (uint256) {
        return eventFactory().getChainpassFee(ticketContractAddress);
    }
