...
// STAX (investments/frax-gauge/temple-frax/StaxLPStaking.sol)
...
/**
  * @notice Contract for users to stake xLP and receive proportional rewards.
  * @dev Based on synthetix BaseRewardPool.sol & convex cvxLocker.
  * Modified for use by TempleDAO 
  */
contract StaxLPStaking is IStaxLPStaking, Ownable {
    ...
    error OnlyMigrator(address caller);
    error OnlyDistributor(address caller);
    ...
    /// @notice Set distributor of rewards
    ...
    /// @notice Balance of staked xLP for a particular user
    function balanceOf(address account) public view returns (uint256) {
        return _balances[account];
    }

    /// @notice Add a token which will be used for future rewards
    function addReward(address _rewardToken) external onlyOwner {
        if (rewardData[_rewardToken].lastUpdateTime != 0) revert CommonEventsAndErrors.InvalidToken(_rewardToken);
    ...
    /// @notice Remove a token which will no longer be used for future rewards
    function removeReward(address _rewardToken) external onlyOwner {
        if (rewardData[_rewardToken].lastUpdateTime == 0) revert CommonEventsAndErrors.InvalidToken(_rewardToken);
        delete rewardData[_rewardToken];
        
        // Create a new array for the remaining tokens.
        // Called rarely so not worried about gas because of the inneficient copy
        address[] memory newRewardTokens = new address[](rewardTokens.length-1);
        uint256 index;
        for (uint256 i=0; i<rewardTokens.length;) {
            if (rewardTokens[i] != _rewardToken) {
                newRewardTokens[index] = rewardTokens[i];
                unchecked { index++; }
            }
            unchecked { i++; }
        }
    ...
    function stakeFor(address _for, uint256 _amount) public override {
        if (_amount == 0) revert CommonEventsAndErrors.InvalidParam();
    ...
    function notifyRewardAmount(
        ...
        if (msg.sender != rewardDistributor) revert OnlyDistributor(msg.sender);
        if (_amount == 0) revert CommonEventsAndErrors.InsufficientBalance(_rewardsToken, 1, 0);
        if (rewardData[_rewardsToken].lastUpdateTime == 0) revert CommonEventsAndErrors.InvalidToken(_rewardsToken);
    ...
    function recoverToken(address _token, address _to, uint256 _amount) external onlyOwner {
        if (_token == address(stakingToken)) revert CommonEventsAndErrors.InvalidToken(_token);
    ...
    modifier onlyMigrator() {
        if (msg.sender != migrator) revert OnlyMigrator(msg.sender);
        _;
    }
