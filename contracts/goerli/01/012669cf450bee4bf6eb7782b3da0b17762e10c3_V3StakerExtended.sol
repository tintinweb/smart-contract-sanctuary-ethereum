...
// see: https://github.com/MoodyCoins/v3-staker-extended/blob/main/src/V3StakerExtended.sol
...
interface IUniswapV3StakerExtended {
    ...
    function increaseIncentive(IUniswapV3Staker.IncentiveKey memory key, uint256 tokenChange) external;
...
contract V3StakerExtended is IUniswapV3Staker, IUniswapV3StakerExtended, Multicall, Ownable {
    ...
    /// @inheritdoc IUniswapV3StakerExtended
    function userDeposits(address user, uint256 index) external view override returns (uint256 tokenId) {
    ...
    function userDepositsIndex(uint256 tokenId) external view override returns (uint256 index) {
        require(deposits[tokenId].owner != address(0), 'UniswapV3StakerExtended: token not deposited');
    ...
    receive() external payable {
        require(msg.sender == address(nonfungiblePositionManager), 'UniswapV3Staker::receive: Not WETH9');
    }
    ...
    /// @inheritdoc IUniswapV3StakerExtended
    function increaseIncentive(IncentiveKey memory key, uint256 tokenChange) external override onlyOwner {
        bytes32 incentiveId = IncentiveId.compute(key);
        Incentive storage incentive = incentives[incentiveId];

        TransferHelperExtended.safeTransferFrom(address(key.rewardToken), msg.sender, address(this), tokenChange);
    ...
    /// @inheritdoc IUniswapV3Staker
    function endIncentive(IncentiveKey memory key) external override onlyOwner returns (uint256 refund) {
        ...
        )
    {
        ...
        (, , address token0, address token1, , , , , , , , ) = nonfungiblePositionManager.positions(id);

        bool eth0 = token0 == address(WETH9) && msg.value > 0;
        bool eth1 = token1 == address(WETH9) && msg.value > 0;

        // collect necessary tokens
        if (eth0) require(msg.value >= params.amount0Desired, 'UniswapV3Staker: Not enough ETH sent');
        else TransferHelper.safeTransferFrom(token0, msg.sender, address(this), params.amount0Desired);

        if (eth1) require(msg.value >= params.amount1Desired, 'UniswapV3Staker: Not enough ETH sent');
        else TransferHelper.safeTransferFrom(token1, msg.sender, address(this), params.amount1Desired);
