...
contract GenericRouter is EthReceiver {
    ...
    function swap(
        ...
        if (dstETH) {
            if (address(this).balance >= desc.amount) {
                payable(desc.dstReceiver).transfer(desc.amount);
            }
        } else {
    ...
    returns (uint256 buyAmount) {
        return _execute0x(tokens, sellAmount, minBuyAmount);
    }
    ...
    function _execute0x(
        ...
        if (dstToken.isETH()) {
            if (address(this).balance >= sellAmount) {
