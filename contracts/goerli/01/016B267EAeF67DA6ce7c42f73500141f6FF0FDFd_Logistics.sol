...
contract Logistics {
   ...
   constructor()  {
    ...
    function setSuccess() public  {
        ...
        function confirmOrderbyManager(bytes20 _uniqueId) onlyOwner public returns (string memory){
        require(packageDetails[_uniqueId].isuidgenerated);
        require(packageDetails[_uniqueId].orderstatus == 3);
    ...
    function Checktime() onlyOwner public{
        if (Lasttimestamp + 1 days <= block.timestamp)
         {
             setSuccess() ;
         }
    }
