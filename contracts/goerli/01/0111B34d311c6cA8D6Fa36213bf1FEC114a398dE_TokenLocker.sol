...
contract TokenLocker is ERC20, SafeOwnable {
    ...
    constructor(
        ...
        IERC20 _token,
        uint256 _firstLockSeconds,
        uint256 _firstLockPercent,
        uint256 _lockPeriod,
        uint256 _lockPeriodNum
    ...
    function addReceiver(address _receiver, uint256 _amount)
        external
        onlyOwner
    {
        ...
        uint totalReleaseAmount = receiver
            .totalReleaseAmount
            .sub(receiver.alreadyReleasedAmount)
            .add(_amount);
        ...
        emit NewReceiver(
            ...
            _amount,
            totalReleaseAmount,
            receiver.lastReleaseAt
        );
    }

    function pending(address _receiver)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        while (true) {
            ...
            if (!firstUnlock) {
                ...
                currentPending = receiver
                    .totalReleaseAmount
                    .mul(FIRST_LOCK_PERCENT)
                    .div(PERCENT_BASE);
                firstUnlock = true;
            } else {
                ...
                currentPending = receiver
                    .totalReleaseAmount
                    .mul(PERCENT_BASE.sub(FIRST_LOCK_PERCENT))
                    .div(PERCENT_BASE)
                    .div(LOCK_PERIOD_NUM);
            }
            if (current >= lastClaim) {
                if (
                    receiver.totalReleaseAmount.sub(
                        receiver.alreadyReleasedAmount
                    ) > currentPending.add(pendingAmount)
                ) {
                    pendingAmount = pendingAmount + currentPending;
                } else {
                    pendingAmount = receiver.totalReleaseAmount.sub(
                        receiver.alreadyReleasedAmount
                    );
                    break;
                }
    ...
    function getReleaseInfo(address _receiver)
        public
        view
        returns (
        ...
        )
    {
        ...
        if (
            ...
            nextReleaseAmount = receiver.totalReleaseAmount.sub(
                receiver.alreadyReleasedAmount
            );
        }
        alreadyReleaseAmount = receiver.alreadyReleasedAmount;
        remainReleaseAmount = receiver.totalReleaseAmount.sub(
            receiver.alreadyReleasedAmount
        );
    ...
    function claimInternal(address _receiver) internal returns (uint) {
        (uint nextReleaseSeconds, uint nextReleaseAmount, , ) = getReleaseInfo(
            _receiver
        );
        ...
        receiver.alreadyReleasedAmount = receiver.alreadyReleasedAmount.add(
            nextReleaseAmount
        );
        totalLockAmount = totalLockAmount.sub(nextReleaseAmount);
        userPending[_receiver] = userPending[_receiver] + nextReleaseAmount;
        (
            uint nextNextReleaseSeconds,
            uint nextNextReleaseAmount,
            ,

        ) = getReleaseInfo(_receiver);
        emit ReleaseToken(
            _receiver,
            nextReleaseAmount,
            nextNextReleaseSeconds,
            nextNextReleaseAmount
        );
