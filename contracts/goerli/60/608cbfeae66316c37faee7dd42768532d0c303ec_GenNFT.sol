...
contract GenNFT is ERC721Enumerable, Ownable, Pausable {
  ...
  uint64 public constant MAX_SUPPLY = 24000000000; // Max Supply. Almost Earth Population * 3
  ...
  event TokenMinted(address indexed owner, uint256[3] ids);
  ...
  function mint() external {
    ...
    require(!walletMinted[msg.sender], 'Already minted');
    uint256 tokenId = _tokenIds.current();
    emit TokenMinted(msg.sender, [tokenId+1, tokenId+2, tokenId+3]);
...
function migrateNFT(uint256 tokenId, address recipient) external onlyOwner {
    // require(!walletMinted[recipient], 'Already minted');
    // emit TokenMinted(recipient, [tokenId, tokenId+1, tokenId+2]);
    walletMinted[recipient] = true;
    // for (uint8 i = 0; i < 3; i++) {
        _tokenIds.increment();
...
function flipMintActive() external onlyOwner {
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    uint256 tokenIdMod3;
    tokenIdMod3 = (tokenId % 3);
    if(tokenIdMod3 == 0) {
        tokenIdMod3 = 3;
    }
    string memory baseContractURI = _baseURI();
    return bytes(baseContractURI).length > 0 ? string(abi.encodePacked(baseContractURI, tokenIdMod3.toString(), tokenSuffixURI)) : '';
  }
