...
contract LuckyWins is ReentrancyGuard, ILuckyWins, Ownable {
    ...
    function _buyTicket(BuyTicketInfo calldata ticket) internal {
        ...
        uint256 ticketValue = _getValueInUsd(ticket.amount);
        require(ticketValue>=MIN_VALUE_USD&&ticketValue<=MAX_VALUE_USD,"The ticket amount is out range!");
    ...
    function _getValueInUsd(uint256 _amount) public view returns(uint256 value){
        value = _amount*_getLatestPrice()/1e18;
    }

    /**
     * Returns the latest price.
     */
    function _getLatestPrice() public view returns (uint256 price) {
        ...
        if(!_usdIsToken0()){
            ...
            price = 10**(token1Decimals+token0Decimals)/result;
        }
