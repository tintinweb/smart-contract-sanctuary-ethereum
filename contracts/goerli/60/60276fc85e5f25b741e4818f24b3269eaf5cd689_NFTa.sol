...
interface INFT {
    ...
    function classNum() external returns (uint8);

    function issueNFT(uint16 _productId,uint8 class,uint256 num) external;

    function leftByClass(uint16 _productId, uint8 class)
        external
        returns (uint256);

    function leftByStartClass(uint16 _productId, uint8 startClass)
        external
        returns (uint256);
}
...
contract NFTa is INFT, AccessControlUpgradeable, ERC4907AUpgradeable {
    ...
    string public _dirName;
    uint8 public override classNum;
    uint256 public FPEMapRandom;

    mapping(uint16 => mapping(uint8 => uint256)) public issueConf;
    mapping(uint16 => mapping(uint8 => uint32)) public mintCount;
    ...
    function initialize(
        ...
        _grantRole(MINTER_ROLE, _playground);
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _grantRole(OPERATOR_ROLE, _msgSender());
        _baseTokenURI = baseTokenURI;
        _dirName = dirName;
        classNum = 6;
        FPEMapRandom = block.timestamp;
        _owner = _msgSender();
    }
    ...
    {
        uint256 info = tokenInfo[tokenId];
        uint32 mintIndex = uint32(info);
        uint8 class = uint8(info >> 32);
        uint16 _productId = uint16(info >> 40);

        uint256 metaId = getMetaId(_productId, mintIndex, class);
        return
            string.concat(
                _baseTokenURI,
                uint256(_productId).toString(),
                "/",
                _dirName,
                "/",
                uint256(class).toString(),
                "/",
                metaId.toString(),
                ".json"
            );
    ...
    function changeDirName(string memory dirName)
        public
        onlyRole(OPERATOR_ROLE)
    {
        _dirName = dirName;
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(AccessControlUpgradeable, ERC4907AUpgradeable)
        returns (bool)
    {
    ...
    //         if (from != to && userOf(tokenId) != address(0)) {
    //             // delete userOf[tokenId];
    ...
    function __mint(
        ...
        require(_productId <= uint32(_productId), "pid max");
        uint256 info = (uint256(_productId) << 8) | class;
        info = (info << 32) | mintIndex;
        tokenInfo[_nextTokenId()] = info;
        _safeMint(user, 1);
    }
    ...
    function mintNFT(
        ...
        require(leftByClass(_productId, class) > 0, "left err");
        __mint(user, mintCount[_productId][class], _productId, class);
        mintCount[_productId][class]++;
    }
    ...
    function leftByClass(uint16 _productId, uint8 class)
        ...
        returns (uint256)
    {
        return issueConf[_productId][class] - mintCount[_productId][class];
    }
    ...
    {
        uint256 total;
        for (uint8 i = startClass; i <= classNum; i++) {
            total += leftByClass(_productId, i);
        }
        return total;
    }
    ...
    function getMetaId(
        ...
        uint32 mintIndex,
        uint8 class
    ) public view returns (uint256 metaId) {
        uint256 batchNum = issueConf[_productId][class];
    ...
    function issueNFT(
        ...
        issueConf[_productId][class] += num;
    }

    function getTryClassByRandom(uint8 startClass, uint256 _random)
        internal
        view
        returns (uint8)
    {
        if (classNum == startClass) {
            return startClass;
        }
        uint256 classCount = classNum - startClass + 1;
        uint256 random = _random % classCount;
        return uint8(random) + startClass;
    }
    ...
    function changeOwner(address __owner) public onlyRole(OPERATOR_ROLE) {
        _owner = __owner;
    }
