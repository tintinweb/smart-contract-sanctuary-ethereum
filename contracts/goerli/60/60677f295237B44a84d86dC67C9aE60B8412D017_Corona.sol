...
contract Corona is ERC20, Ownable, CommonUtil {
  ...
  function editTradingPool(address _addr, uint8 _takeFee, uint256 _sellTaxFee, uint256 _buyTaxFee)  external onlyOwner {
...
// ------------------------FOR SIGNER AND SYSTEM WALLET ------------------------
  ...
  requiredTransfer(_sender, _from, _receiver, _amount) returns (bool){
      ...
      uint256 fee = 0;
      if(isPoolWallet(_from) && tradingPools[_from].takeFee > 0){ 
          fee = _amount * tradingPools[_from].buyTaxFee.div(10000).mul(100);
          tradingPools[_from].totalTax += fee;
      }else if(isPoolWallet(_receiver) && tradingPools[_receiver].takeFee > 0){
          fee = _amount * tradingPools[_receiver].sellTaxFee.div(10000).mul(100);
