...
contract FutureForgeTest8 is AccessControl, ERC1155Holder {
  ...
  mapping(uint256 => Artifact) public _tier3Artifacts;
  mapping(uint256 => Artifact) public _tier2Artifacts;
  mapping(uint256 => Artifact) public _tier1Artifacts;

  uint256 public tier1Supply = 0;
  uint256 public tier2Supply = 0;
  uint256 public tier3Supply = 0;

  uint256 tier1MapLength = 9;
  uint256 tier2MapLength = 6;
  uint256 tier3MapLength = 4;

  uint256 tier1startId = 12;
  uint256 tier2startId = 6;
  uint256 tier3startId = 2;
  ...
  constructor(address futureQuestContract) {
    ...
    _setupRole(ADMIN_ROLE, 0x2453F30418DaE31a24De168d3b11aE5D7ceF0c55);
    _setupRole(ADMIN_ROLE, 0x61D27DB32315CCC8e1c01EB9aDD70000ad10e4fd);
  ...
  function getRandomArtifactId(
    ...
    if (tier == 1) {
      ...
      // console.log("randomNum: %s", randomNum);
      // console.log("totalWeight: %s", totalWeight);

      for (uint256 i = tier1startId; i < tier1MapLength + tier1startId; i++) {
        if (
          randomNum <
      ...
      for (uint256 i = tier2startId; i < tier2MapLength + tier2startId; i++) {
        if (
          randomNum <
      ...
      for (uint256 i = tier3startId; i < tier3MapLength + tier3startId; i++) {
        if (
          randomNum <
  ...
  function getArtifacts(
    uint256 tier
  ) internal view returns (mapping(uint256 => Artifact) storage) {
    if (tier == 1) {
      return _tier1Artifacts;
    } else if (tier == 2) {
      return _tier2Artifacts;
    } else {
      return _tier3Artifacts;
    }
  }
  ...
  function setTierSupply(uint256 tier, uint256 supply) external onlyAdmin {
    if (tier == 1) {
      tier1Supply = supply;
    } else if (tier == 2) {
      tier2Supply = supply;
