...
contract StakingStream is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public secondsInDay = 120;
     ...
     */
    constructor(IERC20 _stakingToken) ERC20("Staked PAKC", "stPAKC") {
        require(address(_stakingToken) != address(0), "StakingStream: staking token is the zero address");
     ...
     */
    function stake(StakeData memory _stakeData) external nonReentrant {
        require(_stakeData.amount > 0, "StakingStream: Staking amount is required");
        uint256 _stakingId = stakingId.current();
        stakingId.increment();
        stakingToken.safeTransferFrom(msg.sender, address(this), _stakeData.amount);
     ...
     */
    function Unstake(UnstakeData memory _unstakeData) external nonReentrant onlyOwner {
        Stake storage stakeRef = stakings[_unstakeData.stakingId];
        require(stakeRef.stakedTimestamp != 0, "StakingStream: nothing is staked");
        require(stakeRef.stakedTimestamp + uint48(stakeRef.period) * secondsInDay > getCurrentTime());
        ...
        stakingToken.safeTransfer(_unstakeData.user, stakeRef.amount);
        emit Unstaked(_unstakeData.stakingId, stakeRef.amount);
    }

    function withdrawProfit(WithdrawProfitData memory _withdrawProfitData) external nonReentrant onlyOwner {
        ...
        stakingToken.safeTransfer(_withdrawProfitData.user, profitToTransfer);
        withdrawnProfit[_withdrawProfitData.userId] = withdrawnProfit[_withdrawProfitData.userId] + _withdrawProfitData.amount;
        emit WithDrawnProfit(_withdrawProfitData.user, _withdrawProfitData.amount);
    }
