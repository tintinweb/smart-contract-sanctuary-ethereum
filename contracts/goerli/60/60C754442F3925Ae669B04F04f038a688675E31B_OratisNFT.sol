...
contract OratisNFT is ERC721A, ERC721AQueryable, ERC4907A, Ownable, ReentrancyGuard {
	...
	// USD has 6 decimals so $1 = 1150000 so $100 = 100_000000
	uint256 public usdcPublicPrice = 115_000000;
	 ...
	 * to the contract prior to calling.
	 */
	function mint(address _recipient, bytes calldata signature, uint256 _amount) external nonReentrant originalUser {
		...
		require(isValidSignature(_recipient, signature), "HASH_FAIL");
		require(totalSupply() + _amount <= MAX_SUPPLY, "MAX_SUPPLY_EXCEEDED");

		uint256 priceInWei = usdcPublicPrice * _amount;
		...
		emit TokensMinted(_recipient, _amount, priceInWei);
	}
