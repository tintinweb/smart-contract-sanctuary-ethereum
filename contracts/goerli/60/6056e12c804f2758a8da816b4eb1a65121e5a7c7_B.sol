...
contract B {
    enum Status {register, pass, fail}
    struct poll {
        uint number;
        string title;
        string contents;
    ...
    struct user {
        ...
        mapping(string => string) voted; // 문제
    }

    mapping(address => user) users;

    function setPoll(string memory _title, string memory _contents) public {
        Polls[_title] = poll(index++, _title, _contents, msg.sender, 0,0,Status.register);
    ...
    function getUser2(string memory _a) public view returns(string memory) {
        return users[msg.sender].voted[_a];
    }

    
    function hash(string memory s) public returns(bytes32){
        return keccak256(bytes(s));
    }

    function vote(string memory _title, bool _b) public {
        // 입력한 _title과 각 요소의 title이 일치하는지 확인 (for 문은 이제 필요 없음)
        require(hash(users[msg.sender].voted[_title]) == hash(""));
        if(hash(Polls[_title].title) == hash(_title) )  { //title과 _title 비교, string은 직접 비교가 아닌 hash값으로
            // 비교 후 통과하면 찬반여부 판단
            // 찬성이면 
            if(_b == true) {
    ...
    function finish(string memory _title) public {
        if(Polls[_title].agree + Polls[_title].disag >=10 && Polls[_title].agree >= (Polls[_title].agree + Polls[_title].disag)*7/10){
            Polls[_title].status = Status.pass;
        }else{
             Polls[_title].status = Status.fail;
        }
    }
}