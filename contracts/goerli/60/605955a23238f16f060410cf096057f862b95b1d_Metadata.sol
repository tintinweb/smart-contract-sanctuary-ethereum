...
interface IERC4907 {
    ...
    function setUser(uint256 tokenId, address user, string calldata meta, uint64 expires) external ;
...
contract Metadata is ERC721A, Ownable, ReentrancyGuard, IERC4907 {
    using Strings for uint256;
    uint256 private register_cost = 20000000000000000;
...
struct UserInfo
    ...
    /// @param user  The new user of the NFT
    /// @param expires  UNIX timestamp, The new user could use the NFT before expires
    function setUser(uint256 tokenId, address user, string calldata meta, uint64 expires) public virtual{
...
//ERC4907 FUNCTIONS FINISH
    ...
    returns (string[] memory)
  {
    ...
    function setRegisterPrice(uint256 customPrice) external onlyOwner {
        register_cost = customPrice;
    }

    function setRegisterActive(bool saleIsActive) external onlyOwner {
        SALE_ACTIVE = saleIsActive;
        IS_ALLOWLIST_ACTIVE = saleIsActive;
    }


    function register(address ref_address, string memory metadata_name)
        public
        payable
    {   
        uint256 price = register_cost;
        bool is_ref=false;
        uint256 ref_cost=0;
        require(bytes(metadata_name).length<=maxCharSize,"Long name");
        require(bytes(metadata_name).length>0,"Write a name");
        require(checkMetadata(metadata_name), "Invalid name");
        ...
        require (tokenAddressandID[metadata_name] == 0 , "This is already taken"); 
        require(SALE_ACTIVE, "Sale is not active!");
        require(msg.value >= price, "Insufficient funds!");
        tokenIDandAddress[_currentIndex]=metadata_name;
        tokenAddressandID[metadata_name]=_currentIndex;
         if (is_ref) {
     ...
     function allowList(string memory metadata_name, bytes32[] calldata _merkleProof)
        public
        payable
    {      
