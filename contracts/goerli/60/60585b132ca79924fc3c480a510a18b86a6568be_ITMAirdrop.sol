...
contract ITMAirdrop is ERC721A, Ownable, ERC2981ContractWideRoyalties {
    ...
    function walletOfOwner(address _owner, uint256 start, uint256 end) public view returns (uint256[] memory) {
        uint256 ownerTokenCount = balanceOf(_owner);
        uint256[] memory ownedTokenIds;
        uint256 currentTokenId = start;
        uint256 ownedTokenIndex = 0;

        while (ownedTokenIndex < ownerTokenCount && currentTokenId <= end) {
