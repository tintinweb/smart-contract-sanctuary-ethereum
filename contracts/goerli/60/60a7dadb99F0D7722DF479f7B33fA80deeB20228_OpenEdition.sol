...
contract OpenEdition is OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, ERC1155URIStorageUpgradeable {
    ...
    modifier MintSettings(uint256 _tokenID, uint256 _qty){
        ...
        if((tokenData[_tokenID].price > 0)){
            require((tokenData[_tokenID].price * _qty) > msg.value, "OpenEdition :: Not enough payment!");  // price must be equal to quantity they bought
        }
    ...
    function setUpTokenData(uint256 _tokenID, uint256 _maxSupply, uint256 _price) external onlyOwner{
    ...
    function setUpMintData(uint256 _tokenID, MintPhase _mintPhase, uint256 _maxWallet, uint256 _maxSupply, bytes32 _merkleRoot) external onlyOwner{
