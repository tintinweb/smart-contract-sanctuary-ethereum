...
contract BlockPiNFT is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, EIP712Upgradeable, ERC721VotesUpgradeable {
    ...
    mapping(uint256 =>BPMeta) public bpMetas;

    function initialize() initializer public {
        __ERC721_init("BlockPi NFT", "BPN");
        __ERC721URIStorage_init();
        __ERC721Burnable_init();
        __Ownable_init();
        __EIP712_init("BlockPi NFT", "1");
        __ERC721Votes_init();
    }

    function safeMint(address to, string memory uri,uint level,uint weight,uint value) public onlyOwner {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        BPMeta memory newMonkey = BPMeta({  
            level:level,
            weight:weight,
            value :value
        });
        bpMetas[tokenId] = newMonkey;
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
    }
    function getNftLevel(uint256 tokenId)public view returns(uint){
        return bpMetas[tokenId].level;
    }
    function getNftWeight(uint256 tokenId)public view returns(uint){
        return bpMetas[tokenId].weight;
    }
    function getNftValue(uint256 tokenId)public view returns(uint){
        return bpMetas[tokenId].value;
    }
