...
abstract contract Tokenomics {
    ...
    string public constant NAME = "FraxInu";
    string public constant SYMBOL = "FXI";

    uint16 public constant FEES_DIVISOR = 10 ** 3;
    uint8 public constant DECIMALS = 18;
    uint256 public constant ZEROES = 10 ** DECIMALS;

    uint256 private constant MAX = ~uint256(0);
    uint256 public constant TOTAL_SUPPLY = 299792458 * ZEROES;
    uint256 public _reflectedSupply = (MAX - (MAX % TOTAL_SUPPLY));

    uint256 public constant maxTransactionAmount = TOTAL_SUPPLY / 100;

    uint256 internal constant maxWalletBalance = 0;

    uint256 internal constant numberOfTokensToSwapToLiquidity = TOTAL_SUPPLY / 100;

    // --------------------- Fees Settings ------------------- //

    address public teamAddress = 0x65b4eF486971839517d6FF08Af90fD69F26FbB1B;
...
abstract contract Liquifier is Ownable, Manageable {
    ...
    function initializeLiquiditySwapper(Env env, uint256 maxTx, uint256 liquifyAmount) internal {
        _env = env;
        if (_env == Env.MainnetV2) _setRouterAddress(_mainnetRouterV2Address); /*(_env == Env.Testnet)*/
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        (uint256 tokenAmountSent, uint256 ethAmountSent, uint256 liquidity) =
            _router.addLiquidityETH{value: ethAmount}(address(this), tokenAmount, 0, 0, owner(), block.timestamp);
...
abstract contract ReflectToken is BaseRfiToken, Liquifier, Antiwhale {
    using SafeMath for uint256;
...
contract FraxInu is ReflectToken {
    constructor() ReflectToken(Env.Testnet) {
