...
contract CryptaDaoVault is Ownable, Pausable, ReentrancyGuard {
    ...
    uint public treasuryAmount = 1.3 ether;
    ...
    */
    ...
    function setTreasuryAmount(uint _amount) external adminOrOwner {
    ...
    function withdraw(address _token) external {
        ...
        // Proceed with withdrawal if balane is > 0
        if (balance != 0) {
            _withdrawTreasury();
            for (uint c; c < length;) {
    ...
    function _withdrawERC20(address _token, OwnerBalance[] memory ownersList) internal nonReentrant {
    ...
    function _withdrawTreasury() internal {
        ...
        if (treasuryAmount != 0 
            && balance != 0 
            && (treasuryLastWithdrawal + treasuryInterval < block.timestamp)
            && treasury != address(0)
        ){
            uint treasuryBalance = balance > treasuryAmount ? balance - treasuryAmount : balance;
            SafeTransferLib.safeTransferETH(treasury, treasuryBalance);
        }
    }
