...
contract NFL721 is ERC721A, Pausable, Ownable {
  ...
  constructor(uint teamMax, uint rprice, uint sprice, string memory __baseURI) ERC721A("NFL Rarity League", "NFL") {
    TEAM_MAX = teamMax;
    rPRICE = rprice;
    sPRICE = sprice;
    BASE_URI = __baseURI;
  ...
  function setRPrice(uint _rprice) public onlyOwner {
    rPRICE = _rprice;
  }

  function setSPrice(uint _sprice) public onlyOwner {
    sPRICE = _sprice;
  }
  ...
  function _getRandomTeamOnSale() internal view returns (uint8) {
    require(_teamsOnSale.length > 0, "must be during sale");
    uint rand = uint(keccak256(abi.encodePacked(blockhash(block.number - 1), _totalMinted())));
    uint8 teamIndex = uint8(rand % _teamsOnSale.length);
    return _teamsOnSale[teamIndex];
  }
  ...
  function randomMint(uint quantity) external payable whenNotPaused {
    require(quantity < 51, "limit of 50 per transaction");
    require(msg.value >= rPRICE * quantity, "incorrect eth sent");
    require(_teamsOnSale.length > 1, "not enough teams on sale");
    require(_totalMinted() + quantity <= SALE_MAX, "cannot mint more than allotted supply");
    for (uint i = 0; i < quantity; i++) {
      uint8 teamId = _getRandomTeamOnSale();
      // update team supply
      _teamData[teamId].supply += 1;
  ...
  function specificMint(uint quantity, uint8 teamId) external payable whenNotPaused {
    ...
    require(msg.value >= sPRICE * quantity, "incorrect eth sent");
    // update team supply
    _teamData[teamId].supply += quantity;
    emit MintSupplyRemaining(teamId,  SELLING_AMOUNT - _teamData[teamId].supply);
    if (_teamData[teamId].supply == SELLING_AMOUNT) {
