...
contract SingleAssetVault is ERC4626, Ownable
{
	...
	mapping (address => bool) public isVaultExisting;
	...
	{
		require(!isVaultExisting[address(vault)], "prevent double addition of one vault");

		isVaultExisting[address(vault)] = true;
		vaults.push(VaultInfo(vault, false));
		IERC20(asset()).safeIncreaseAllowance(address(vault), type(uint256).max);
	}

	function switchVaultStatus(uint256 indexVault) external onlyOwner
	{
		vaults[indexVault].isDeprecated = !vaults[indexVault].isDeprecated;
	} 
	...
	function totalPortfolioScore() public view returns (uint256 total)
	{
		...
		{
			uint256 portfolioScore = oracle.getPortfolioScore(address(vaults[j].vault));
	...
	{
		IERC4626 vault = vaults[vaultIndex].vault;
		...
		return int256(1000 * portfolioScore / totalPortfolioScore()) - int256(shareOfVault);
	}
	...
	/** 
	 ...
	 @param fromVault Describes from which SingleAssetVault to take funds
	 @param toVault Describes to which SingleAssetVault to put funds
	...
	) internal virtual override
	{
		...
		require(!vaultInfo.isDeprecated, "vault is deprecated now");
	...
	{
		VaultInfo storage vaultInfo = vaults[getVaultWithMaxScoreDeviation(false)];
