...
contract AssetPool is IAssetPool, Initializable {
    ...
    function deposit(address token, uint amount) public payable {
        ...
        emit Deposit(user, token, amount, liquidityAdded);
    }
    ...
    function withdraw(address token, uint amount) external {
        ...
        (,uint liquidityWithdrawable) = this.withdrawableAmount(user, token);
        uint shareBurnt;
        if (liquidityRemoved == liquidityWithdrawable) {
            shareBurnt = userShare(user); 
        }
        else {
            shareBurnt = _getShareBurnt(liquidity, liquidityRemoved);
        }
        ...
        emit Withdraw(user, token, amount, liquidityRemoved);
    }
...
interface IAssetPool {
    ...
    event Deposit(address user, address token, uint amount, uint amountUsd);
    event Withdraw(address user, address token, uint amount, uint amountUsd);
