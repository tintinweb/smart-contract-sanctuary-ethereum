...
contract TheAIArtGame is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    string public seed = "beforerevealtest";
    ...
    string[2] private website = [
        '<html><style>*{box-sizing: border-box;}html, body{height: 100%; width: 100%; background-color: rgb(255, 255, 255);}.container{font-weight: 700; font-style: normal; width: 100%; height: 100%; position: relative; padding: 4em; filter: contrast(1.25); background-color: rgb(255, 255, 255); overflow: hidden;}h1{color: red; font-size: 8rem; font-weight: 700; text-transform: uppercase; line-height: 1; filter: blur(0.5rem); animation: letterspacing 10s infinite alternate cubic-bezier(.2, 0, 0, 1); display: block; position: absolute; left: 50%; top: 50%; transform: translate3d(-50%, -50%, 0);}@keyframes letterspacing{0%{letter-spacing: -5rem; filter: blur(0.5rem);}50%{filter: blur(0.5rem);}100%{letter-spacing: 1rem; filter: blur(1.5rem);}}</style><div class="container"> <h1>',
        "</h1></div></html>"
    ];
    ...
    function animationCreator(string[1] memory vars)
        private
        view
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(abi.encodePacked(animationBytes, website[1]))
                )
            );
    }
        ...
        returns (string memory)
    {
        ...
        string[3] memory metadataVars = [
            ...
            unhideWord
                ? animationCreator([grabWord(tokenId)])
        ...
        string
            memory animationUrl = '","animation_url":"https://i.pinimg.com/originals/eb/bd/f7/ebbdf7ce4f7f502d1f28b96b5cbd7a1f.gif';
        if (!unhideWord) {
            bytes memory test;
            ...
            animationUrl = string(
                abi.encodePacked("data:text/html;base64,", Base64.encode(test))
            );
        }
