...
contract FungibleFlexWaitinglist is Ownable {
   ...
   // }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            if (
                !isOccupied(
                    establishments[_estID].displayIds[i]
                )
                )
            {
                displaysArray[counter] = establishments[_estID].displayIds[counter];
    ...
    function isOccupied(uint256 _displayId) public view virtual returns (bool) {
        ...
        for (uint256 counter = 0; counter < display[_displayId].endTime.length; counter++) {
    ...
    function getDisplay(uint256 _displayId) public view virtual returns (address, uint256) {
        require(
            display[_displayId].displayOwner != address(0),
            "Display does not exist"
        );
        for (uint256 counter = 0; counter < display[_displayId].endTime.length; counter++) {
            if (display[_displayId].endTime[counter]> block.timestamp) {
                return (display[_displayId].collectionAddress[counter], display[_displayId].tokenId[counter]);
            }
