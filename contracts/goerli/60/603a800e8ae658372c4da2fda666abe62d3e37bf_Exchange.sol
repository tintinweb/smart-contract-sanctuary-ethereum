...
abstract contract AssetMatcher is Initializable {
    ...
    function _matchAssetOneSide(
        ...
        if (classLeft == LibAsset.ETH_ASSET_CLASS) {
            ...
            if (msg.sender == order.maker && classRight == LibAsset.ERC20_ASSET_CLASS) {
...
contract Exchange is Initializable, OwnableUpgradeable, ExchangeCore {
    ...
    PausableUpgradeable
{
    ...
    //state of the orders extension
    mapping(bytes32 => bool) public isOrderExtensionCancel;
        ...
        returns (LibOrderData.Data memory dataOrder)
    {
        ...
        } else if (order.dataType == LibOrderData.DEFAULT) {} else {
            revert("Unknown Order data type");
        }
        dataOrder.recipient = payable(order.maker);
    }

    function cancelExtension(LibOrder.Input memory input)
        external
        override
        nonReentrant
        whenNotPaused
    {
        ...
        bytes32 orderHash = LibOrder._hashKey(input);
        require(!isOrderExtensionCancel[orderHash], "already canceled");
        isOrderExtensionCancel[orderHash] = true;

        // bytes4(keccak256("LISTING"))
        emit Cancel(input.salt, 0xebdc9abe);
    }
        ...
        whenNotPaused
    {
        ...
        if (
            ...
            // bytes4(keccak256("OFFER"))
            typeOrder = 0x768a5be4;
        }

        emit Cancel(order.salt, typeOrder);
    }

    /**
        @notice verify signature and match orders 
        @param input the left order of the match
        @param signature the signature of listing
        @param orderRight the right order of the match
     */
    function matchOrderExtension(
        LibOrder.Input memory input,
        bytes memory signature,
        LibOrder.Order memory orderRight
    ) external payable override nonReentrant whenNotPaused {
        bytes32 orderHash = LibOrder._hashKey(input);
        require(!isOrderExtensionCancel[orderHash], "listing is canceled");
        require(input.dataType == LibOrderData.V1, "Invalid match order");

        _validateExtentionFull(input, signature);
        ...
        LibOrder.Order memory orderLeft = LibOrder.Order(
            input.maker,
            input.makeAsset,
            input.taker,
            input.takeAssets[orderRight.typeMint],
            input.salt,
            input.start,
            input.end,
            input.dataType,
        ...
        _matchAndTransferExtension(orderLeft, orderRight);
    ...
    /**
        ...
        @param orderRight the right order of the match
    */
    function _matchAndTransferExtension(
        ...
        LibFill.FillResult memory newFill = LibFill.FillResult(
            1,
        ...
        emit Match(
            msg.sender,
            orderLeft.salt,
            0xd6c39a87, //bytes4(keccak256("SALE_LISTING")),
            newFill.rightValue,
            newFill.leftValue
        );
    }
    ...
    function _matchAndTransfer(
        ...
        if (signatureRight.length == 65) {
            emit Match(
                msg.sender,
                orderRight.salt,
                0x3c7f72f2, // bytes4(keccak256("SALE_OFFER")),
                newFill.rightValue,
                newFill.leftValue
            );
        } else {
...
interface IExchange {
    //events
    event Cancel(uint256 salt, bytes4 typeOrder);
    event Match(
        address from,
        uint256 salt,
        bytes4 typeOffer,
    ...
    function cancelExtension(LibOrder.Input memory input) external;
...
library LibAsset {
    ...
    bytes4 constant ETH_ASSET_CLASS = 0xaaaebeba;
    // bytes4(keccak256("ERC20"))
    bytes4 constant ERC20_ASSET_CLASS = 0x8ae85d84;
    // bytes4(keccak256("ERC721"))
    bytes4 constant ERC721_ASSET_CLASS = 0x73ad2146;
    // bytes4(keccak256("ERC721_LAZY"))
    bytes4 constant ERC721_LAZY_ASSET_CLASS = 0xd8f960c1;
    ...
    // keccak256("Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)")
    bytes32 constant ASSET_TYPEHASH =
        0x2b5d221b1c8e4eba031fd290f19916cb845b0b880956c3c5289654454e369b31;
...
library LibOrder {
    ...
    // keccak256("Order(address maker,Asset makeAsset,address taker,Asset takeAsset,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data,uint64 typeMint)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)")
    bytes32 constant ORDER_TYPEHASH =
        0x8a52423eebf0127faa87c8b03d44e451375af417e24b076e08405a3ae3d0d766;

    // keccak256("Input(address maker,Asset makeAsset,address taker,Asset[] takeAssets,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)")
    bytes32 constant INPUT_TYPEHASH =
        0x2e79377bb4a3ac7b8b74a0c944d66df335d1448fa257bb55031ad281a791650c;
    ...
    function _hashKey(Input memory input) internal pure returns (bytes32) {
        bytes32[] memory assetsBytes = new bytes32[](input.takeAssets.length);
        for (uint256 i = 0; i < input.takeAssets.length; i++) {
            assetsBytes[i] = LibAsset._hash(input.takeAssets[i]);
        }
        return
            keccak256(
                abi.encode(
                    input.maker,
                    LibAsset._hash(input.makeAsset),
                    input.taker,
...
library LibOrderData {
    ...
    bytes4 public constant DEFAULT = 0xffffffff;
...
library LibPart {
    ...
    function _hash(Part memory part) internal pure returns (bytes32) {
    ...
    EIP712Upgradeable
{
    ...
    function _verifySignatureExtention(
        LibOrder.Input memory input,
        bytes memory signature
    ) internal view {
        if (input.salt != 0) {
...
abstract contract TransferExecutor {
    ...
    function _transferNFT(
        ...
        if (asset.assetType.assetClass == LibAsset.ERC721_ASSET_CLASS) {
            ...
            require(asset.value == 1, "erc721 value error");
            INFT721(asset.token).mint(to, typeMint);
        } else {
            revert("asetClass is invalid");
        }
    }
