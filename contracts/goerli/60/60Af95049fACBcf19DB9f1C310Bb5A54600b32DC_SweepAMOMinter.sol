...
// =========================== SweepAMOMinter ==========================
// ====================================================================
// globalCollateralValue() in Swep.sol is gassy because of the loop and all of the AMOs attached to it. 
...
import '../AMOs/IAMO.sol';

contract SweepAMOMinter is Owned {
    ...
    // Max amount of SWEEP this contract can mint
    int256 public sweep_mint_cap = int256(100000000e18);

    // Minimum collateral ratio needed for new SWEEP minting
    uint256 public min_cr = 810000;

    // SWEEP mint balances
    mapping(address => int256) public sweep_mint_balances; // Amount of SWEEP the contract minted, by AMO
    int256 public sweep_mint_sum = 0; // Across all AMOs
    ...
    // SWEEP balance related
    uint256 public sweepDollarBalanceStored = 0;
    ...
    // [amo_address][0] = AMO's sweep_val_e18
    ...
    function dollarBalances() public view returns (uint256 sweep_val_e18, uint256 collat_val_e18) {
        sweep_val_e18 = sweepDollarBalanceStored;
    ...
    function sweepTrackedGlobal() external view returns (int256) {
        return int256(sweepDollarBalanceStored) - sweep_mint_sum - (collat_borrowed_sum * int256(10 ** missing_decimals));
    }

    function sweepTrackedAMO(address amo_address) external view returns (int256) {
        (uint256 sweep_val_e18, ) = IAMO(amo_address).dollarBalances();
        int256 sweep_val_e18_corrected = int256(sweep_val_e18) + correction_offsets_amos[amo_address][0];
        return sweep_val_e18_corrected - sweep_mint_balances[amo_address] - ((collat_borrowed_balances[amo_address]) * int256(10 ** missing_decimals));
    }
    ...
    function syncDollarBalances() public {
        uint256 total_sweep_value_d18 = 0;
        uint256 total_collateral_value_d18 = 0; 
        for (uint i = 0; i < amos_array.length; i++){ 
            // Exclude null addresses
            address amo_address = amos_array[i];
            if (amo_address != address(0)){
                (uint256 sweep_val_e18, uint256 collat_val_e18) = IAMO(amo_address).dollarBalances();
                total_sweep_value_d18 += uint256(int256(sweep_val_e18) + correction_offsets_amos[amo_address][0]);
                total_collateral_value_d18 += uint256(int256(collat_val_e18) + correction_offsets_amos[amo_address][1]);
            }
        }
        sweepDollarBalanceStored = total_sweep_value_d18;
    ...
    // on the main SWEEP contract
    function mintSweepForAMO(address destination_amo, uint256 sweep_amount) external onlyByOwnGov validAMO(destination_amo) {
        int256 sweep_amt_i256 = int256(sweep_amount);

        // Make sure you aren't minting more than the mint cap
        require((sweep_mint_sum + sweep_amt_i256) <= sweep_mint_cap, "Mint cap reached");
        sweep_mint_balances[destination_amo] += sweep_amt_i256;
        sweep_mint_sum += sweep_amt_i256;

        // Mint the SWEEP to the AMO
        SWEEP.minter_mint(destination_amo, sweep_amount);

        // Sync
        syncDollarBalances();
    }

    function burnSweepFromAMO(uint256 sweep_amount) external validAMO(msg.sender) {
        int256 sweep_amt_i256 = int256(sweep_amount);

        // Burn first
        SWEEP.minter_burn_from(msg.sender, sweep_amount);

        // Then update the balances
        sweep_mint_balances[msg.sender] -= sweep_amt_i256;
        sweep_mint_sum -= sweep_amt_i256;
    ...
    function refreshSweepTargetPrice() external validAMO(msg.sender) {
        // Refresh target price
        SWEEP.refreshTargetPrice();
    }
    ...
    function addAMO(address amo_address, bool sync_too) public onlyByOwnGov {
        ...
        (uint256 sweep_val_e18, uint256 collat_val_e18) = IAMO(amo_address).dollarBalances();
        require(sweep_val_e18 >= 0 && collat_val_e18 >= 0, "Invalid AMO");
        ...
        // Mint balances
        sweep_mint_balances[amo_address] = 0;
    ...
    function setTreasury(address _treasury_address) external onlyByOwnGov {
        require(_treasury_address != address(0), "Treasury address cannot be 0");        
        treasury_address = _treasury_address;
    }

    function setSweepMintCap(uint256 _sweep_mint_cap) external onlyByOwnGov {
        sweep_mint_cap = int256(_sweep_mint_cap);
    }

    function setAMOCorrectionOffsets(address amo_address, int256 sweep_e18_correction, int256 collat_e18_correction) external onlyByOwnGov {
        correction_offsets_amos[amo_address][0] = sweep_e18_correction;
...
interface ISweep {
  ...
  function refreshTargetPrice() external;
