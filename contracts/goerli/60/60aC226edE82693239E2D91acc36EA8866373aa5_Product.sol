...
contract Product is ERC721URIStorage, ERC721Enumerable, ReentrancyGuard {
  ...
  address immutable crowdmuseHub = address(0);
  ...
  mapping(uint256 => ContributionInformation) public contributionByContributionId;
  mapping(uint256 => uint256) public NFTByType; // mapping that keeps the NFT type for each  NFT id
  mapping(address => bool) public contributors;

  constructor(
    address _owner,
    uint256 _projectSource,
  ...
  function createTasks(
    ...
    for (uint256 i = 0; i < _contributionValues.length; i++) {
      ...
      if (_taskStatus[i] == TaskStatus.Complete) {
        ...
        mintContributorNFT(_taskContributors[i]);
      }
    }
  }

  function assignTask(uint256 _taskId, address _contributorAddress) public {
    require(taskByTaskId[_taskId].taskStatus == TaskStatus.Open, "Task not open");
    require(taskByTaskId[_taskId].taskOwnerAddress == msg.sender, "Not task owner");
    taskByTaskId[_taskId].taskStatus = TaskStatus.Assigned;
    taskByTaskId[_taskId].taskContributor = _contributorAddress;
  }

  function submitContribution(uint256 _taskId, string calldata _submissionUri) public returns (uint256) {
    require(taskByTaskId[_taskId].taskContributor == msg.sender, "Not correct contributor");
    contributionId.increment();
    uint256 _contributionId = contributionId.current();
    contributionByContributionId[_contributionId].submissionUri = _submissionUri;
    contributionByContributionId[_contributionId].contributorAddress = msg.sender;
  ...
  function approveContribution(
    ...
    require(taskByTaskId[_taskId].taskOwnerAddress == msg.sender, "Not task owner");
    // This allows for a different contribution to be accepted onto  the task even if ir was assigned to someone else
    taskByTaskId[_taskId].submissionUri = _submissionUri;
    taskByTaskId[_taskId].taskStatus = TaskStatus.Complete;
    taskByTaskId[_taskId].taskContributor = _contributorAddress;
  }

  function buyNFT(address _to) public payable returns (uint256 _tokenId) {
    require(productStatus == ProductStatus.Complete, "Product not complete");
    require(msg.value >= buyNFTPrice, "NFT costs more than this");
  ...
  // function batchBuyNFT(uint256 _quantity) public payable  returns (uint256) {
  //   require(productStatus == ProductStatus.Complete, "Product not complete");
  //   require(_quantity > 0, "quantity cant be 0");
  //   require(_quantity < 10, "quantity less than 10");
  //   require(msg.value >= buyNFTPrice.mul(_quantity), "NFT costs more than this");
  ...
  function mintContributorNFT(address to) private {
    // tokenId.increment();
    // _tokenId = tokenId.current();
    // _safeMint(to, _tokenId);
    // NFTByType[_tokenId] = contributor;
    contributors[to] = true;

  }

  function submitProduct(uint256 _buyNFTPrice) public onlyOwner {
    require(_buyNFTPrice > 0, "Price should not be 0");
