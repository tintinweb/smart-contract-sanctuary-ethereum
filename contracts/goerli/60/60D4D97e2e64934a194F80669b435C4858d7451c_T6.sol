...
contract T6 is ERC721A, Ownable, ReentrancyGuard {
    ...
    //presale settings
    uint256 public presaleDate = 1657817100;
    uint256 public publicMintDate = 1660384213;
    ...
    constructor(
        ...
        setSignatureAddress(_signatureAddress);
    }

    

    // internal
    function setSignatureAddress (address _signatureAddress) public onlyOwner {
    ...
    function viewSignatureAddress() public onlyOwner view {
      signatureAddress;
    }
    ...
    // public minting
    function koalaMint(address _sendTo, uint256 _mintAmount)
        public
        payable
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _mintAmount; ++i) {
            _safeMint(_sendTo, s + i, "");
        }
        delete s;
    }

    function koalaPresaleMint(address _sendTo, uint256 _mintAmount)
        public
        payable
        nonReentrant
    {
        ...
        uint256 s = totalSupply();
        uint256 reserve = presaleWhitelist[_sendTo];
        require(!status, "Off");
        ...
        presaleWhitelist[_sendTo] = reserve - _mintAmount;
        delete reserve;
        for (uint256 i = 0; i < _mintAmount; i++) {
            _safeMint(_sendTo, s + i, "");
        }
    ...
    function mintPresale(uint256 _mintAmount,bytes32 hash, bytes memory signature) external payable {
        require(presaleDate <= block.timestamp, "Not yet");
        require(publicMintDate >= block.timestamp, "Too late");
        uint256 s = totalSupply();
        uint256 reserve = presaleWhitelist[msg.sender];
        require(status, "Off");
        require(recoverSigner(hash, signature)==signatureAddress,"You are not on the list!");
        require(presaleMaxMint - reserve >= _mintAmount, "Try less");
        // require(_mintAmount <= reserve, "Try less");
        require(s + _mintAmount <= maxSupply, "More than max");
        require(cost * _mintAmount == msg.value, "Wrong amount");
    ...
    function mintPresale2(uint256 _mintAmount,bytes32 hash, bytes memory signature) external payable {
        require(presaleDate <= block.timestamp, "Not yet");
        require(publicMintDate >= block.timestamp, "Too late");
        uint256 s = totalSupply();
        uint256 reserve = presaleWhitelist[msg.sender];
        require(status, "Off");
        require(recoverSigner(hash, signature)==owner(),"You are not on the list!");
        require(presaleMaxMint - reserve >= _mintAmount, "Try less");
        // require(_mintAmount <= reserve, "Try less");
        require(s + _mintAmount <= maxSupply, "More than max");
        require(cost * _mintAmount == msg.value, "Wrong amount");
