...
/// @title TheFarm DAO Logic interfaces and events
...
contract TheFarmDAOEvents {
  ...
  /// @notice An event emitted when a proposal has been queued in the TheFarmDAOExecutor
  event ProposalQueued(uint256 id, uint256 eta);

  /// @notice An event emitted when a proposal has been executed in the TheFarmDAOExecutor
...
contract TheFarmDAOStorageV1 is TheFarmDAOProxyStorage {
  ...
  /// @notice The basis point number of votes required in order for a voter to become a proposer. *DIFFERS from GovernorBravo
  uint256 public proposalThresholdBPS;

  /// @notice The basis point number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed. *DIFFERS from GovernorBravo
  uint256 public quorumVotesBPS;

  /// @notice The total number of proposals
  uint256 public proposalCount;

  /// @notice The address of the TheFarm DAO Executor TheFarmDAOExecutor
  ITheFarmDAOExecutor public timelock;

  /// @notice The address of the FarmAnimals NFT tokens
  ...
  struct Proposal {
    ...
    address proposer;
    /// @notice The number of votes needed to create a proposal at the time of proposal creation. *DIFFERS from GovernorBravo
    uint256 proposalThreshold;
    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed at the time of proposal creation. *DIFFERS from GovernorBravo
    uint256 quorumVotes;
...
/// @title The TheFarm DAO proxy contract
...
import './TheFarmDAOInterfaces.sol';
