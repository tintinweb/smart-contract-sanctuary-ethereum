...
contract Advertise is ERC721, Ownable {
	...
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		...
		uint256 price = priceByTokenId[tokenId];
		string memory txt = txtByTokenId[tokenId];
		string memory payerAddress = payerByTokenId[tokenId];
		string memory priceStr = DecimalStrings.decimalString((price / 10000000000000) * 10000000000000, 18, false);
		...
		bytes memory json = abi.encodePacked(
			...
			Base64.encode(abi.encodePacked(renderer.render(tokenId, payerAddress, address(this), time, priceStr))),
			'", "attributes": [',
