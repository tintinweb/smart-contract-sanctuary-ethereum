    ...
    ErrorCodes
{
   ...
   /**
    ...
    function returnTimestamp(uint256 time) public view returns(uint256, uint256, bool){
        bool result;
        if(block.timestamp <= time + 30 minutes){
            result = true;
        }
        else{
            result = false;
        }
        return (block.timestamp, time + 30 minutes, result);
    }
