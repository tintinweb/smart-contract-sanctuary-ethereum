...
contract ERC1155StakingPool is LockCycle, BaseStakingPool, ERC1155Holder
{
	...
	function deposit(uint256[] calldata _ids, uint256[] calldata _amounts, bool _claimRewards) external nonReentrant
	{
		if (_claimRewards) {
	...
	function withdraw(uint256[] calldata _ids, uint256[] calldata _amounts, bool _claimRewards) external nonReentrant
	{
		if (_claimRewards) {
			_harvestAll(msg.sender);
		}
