...
contract Staking is Migrate{
    ...
    address constant tokenLP = 0x49dF46326D45166Ff50664b73Cf7FE4de787BD61;
    bool pause;
    uint time;
    uint endTime;
    uint32 txId;
    ...
    function stake(uint _sum,uint8 count,string memory addrCN,uint8 procentage,address _token) public  returns(uint32) {
        require(procentage >= 0 && procentage <= 100,"Max count procent 100");
        //require(_sum >= 10 ** IERC20(_token).decimals(),"Minimal stake 1 token");
        require(_token == tokenLP,"Error address token");
