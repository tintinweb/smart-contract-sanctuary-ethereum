...
contract OcxToken is ERC20, OcxAdmin, Ownable {
    ...
    constructor()
        ERC20("Liquidity Provider Coin for OpenchainDex Local Pool", "OCX")
    {}

    function mint(uint256 amount) external payable onlyAdmin {
    ...
    function burn(uint256 amount) public onlyAdmin {
        require(
            ...
            "Insufficient OCX balance to burn"
        );
        super._burn(msg.sender, amount);
        emit burned(amount);
    }

    function addPoolAddress(address addr) public onlyAdmin {
        poolAddress.push(addr);
    }

    function isPoolAddress(address addr) public view returns (bool) {
        for (uint256 i = 0; i < poolAddress.length; i++)
            if (poolAddress[i] == addr) return true;
        return false;
    }
        ...
        returns (bool)
    {
        ...
        } else fee = amount.mul(8).div(1000);
        super.transfer(owner(), fee);
        super.transfer(to, amount.sub(fee));
        return true;
    }
