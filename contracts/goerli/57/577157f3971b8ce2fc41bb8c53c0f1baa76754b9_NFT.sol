...
contract NFT is ERC721, Ownable {
    ...
    constructor(string memory _unrevealedUri, address _signer)
        ERC721("NFT", "NFT")
    {
    ...
    /// @param _start unix timestamp of mint start time
    /// @param _duration unix timestamp of length of mint

    function setMintWindow(uint256 _start, uint256 _duration) public onlyOwner {
        start = _start;
        end = _start + _duration;
    }

    function mint(uint256 _amount) public {
        require(block.timestamp >= start, "mint not started");
        require(block.timestamp <= end, "mint ended");
