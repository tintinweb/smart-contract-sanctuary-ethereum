...
contract ASTILO is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function preSaleBuy(bytes32[] calldata _proof) public payable whenNotPaused nonReentrant checkWhitelist(_proof) {
        ...
        require(detail.raisedIn < detail.cap);
        UserToken memory userToken;
        uint256 _tokens = calculateToken(msg.value, detail.rate);
        uint256 limit = userToken.createdOn == 0 ? detail.thresHold : detail.thresHold - userToken.tokens;
        require(_tokens <= limit, "buying more than max allowed");
