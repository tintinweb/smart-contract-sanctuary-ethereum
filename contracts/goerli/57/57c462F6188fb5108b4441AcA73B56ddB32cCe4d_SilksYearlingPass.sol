    ...
    WhitelistPricing
{
    ...
    ) ERC721Redeemable(_name, _symbol, _payeeAddress) {
    ...
    function publicPurchase() public payable whenNotPaused {
        (
            uint256 price,
            bool paused,
            uint256 maxPerTx,
    ...
    function tokenGatedPurchase(
        ...
        (
            ...
            uint256 maxPerWallet,
            bool valid
        ) = getCollaboration(_contractAddress);
        require(valid && !paused, "NOT_VALID_OR_PAUSED");
        checkPurchaseConditions(msg.sender, price, maxPerTx, maxPerWallet);
    ...
    function _checkAndMintPasses(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                !checkIfTokenUsed(_contractAddress, _tokenIds[i]),
                "USED_TOKEN"
            );
    ...
    function checkIfTokenUsed(address _contractAddress, uint256 _tokenId)
        public
        view
        returns (bool)
    {
...
contract CollaborationPricing is Ownable {
    ...
    function getCollaboration(address _contractAddress)
        public
        view
        returns (
            uint256 price,
...
contract WhitelistPricing is Ownable {
    ...
    function getWhitelist(string calldata _id)
        public
        view
        returns (
            uint256 price,
...
contract ERC721Redeemable is ERC721Enumerable, Ownable, Pausable {
    ...
    function setRedeemInfo(address _redeemContractAddress, bool _redeemStarted)
        external
        onlyOwner
    {
