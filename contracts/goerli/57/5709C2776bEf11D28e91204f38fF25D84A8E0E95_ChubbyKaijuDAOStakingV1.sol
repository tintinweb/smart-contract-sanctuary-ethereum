...
contract ChubbyKaijuDAOStakingV1 is IChubbyKaijuDAOStakingV1, OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function _test() internal returns(uint48){
    return 1000;
  }
  ...
  modifier _updateEarnings() {
    if (totalCrunchEarned < MAXIMUM_CRUNCH) {
      uint48 time = uint48(block.timestamp);
      uint128 temp = (1*1*(time - lastClaimTimestamp)*CRUNCH_EARNING_RATE*rewardtypeStaked.uncommon_gen1);
      uint128 temp2 = 100000000+6*(time - lastClaimTimestamp);
      temp *= temp2;
      totalCrunchEarned += temp;

      temp = (1*3*(time - lastClaimTimestamp)*(CRUNCH_EARNING_RATE/2)*rewardtypeStaked.uncommon_gen1);
      temp2= 100000000+6*(time - lastClaimTimestamp);
      totalCrunchEarned += temp;
