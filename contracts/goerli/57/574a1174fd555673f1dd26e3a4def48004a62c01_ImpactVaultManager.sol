    ...
    ReentrancyGuardUpgradeable
{
    ...
    function registerVault(address _vaultToAdd)
        ...
        onlyOwner
        whenNotPaused
        onlyNotRegisteredVault(_vaultToAdd)
    {
        vaults.push(ImpactVault(_vaultToAdd));
    }
    ...
    function deregisterVault(address _vaultToRemove)
        ...
        onlyOwner
        whenNotPaused
        onlyRegisteredVault(_vaultToRemove)
    {
        for (uint256 i = 0; i < vaults.length; i++) {
            if (address(vaults[i]) == _vaultToRemove) {
    ...
    modifier onlyRegisteredVault(address _vault) {
        require(isRegisteredVault(_vault), "VAULT_NOT_REGISTERED");
        _;
    }

    modifier onlyNotRegisteredVault(address _vault) {
        require(!isRegisteredVault(_vault), "VAULT_ALREADY_REGISTERED");
        _;
    }

    function isRegisteredVault(address _vault)
        public
        view
        returns (bool isRegistered)
    {
        for (uint256 i = 0; i < vaults.length; i++) {
            if (address(vaults[i]) == _vault) {
                isRegistered = true;
                break;
            }
        }
    ...
    function claimGovernanceTokens() external whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < vaults.length; i++) {
            totalYieldUSD += vaults[i].transferYieldToManager(_msgSender());
        }
    ...
    {
        for (uint256 i = 0; i < vaults.length; i++) {
            uint256 y = vaults[i].getYieldUSD(_address);
            totalYieldUSD += y;
        }
