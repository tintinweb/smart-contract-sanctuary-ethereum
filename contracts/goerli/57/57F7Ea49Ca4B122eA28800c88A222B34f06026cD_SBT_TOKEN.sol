...
contract SBT_TOKEN is ERC4973, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _university_Counter;
    Counters.Counter private _certificate_Counter;
    /**
     * @dev Set contract deployer as owner
     */

    string VERSION = "1.0";
    string NAME = "SBT_TOKEN";
    string SYMBOL = "SBT";
    ...
    struct CERTIFICATE {
        ...
        string student_name;
        string course;
        uint startDate; //= 1638352800; // 2012-12-01 10:00:00 // DATE SHOULD BE IN EPOCH FORMAT
        uint endDate;
        string department;
        string contact_number;
        string cgpa;
        string tenure;
        string certificate_uri;
        uint certificate_id;
        uint university_id;
    }
    ...
    function ADD_NEW_UNIVERSITY(string memory UNI_NAME) public {
        uint256 uniId = _university_Counter.current();
        _university_Counter.increment();
        CREATE_UNIVERCITY(uniId, UNI_NAME, msg.sender);
    }

    function CREATE_UNIVERCITY(
        uint ID,
        string memory _NAME,
        address UNI_ADDRESS
    ) private {
        universities[ID].uni_owner = UNI_ADDRESS;
        universities[ID].uni_name = _NAME;
        universities[ID].whitelisted_accounts.push(UNI_ADDRESS);
        emit UNIVERSITY_ADDED(UNI_ADDRESS, ID, _NAME);
    }

    function GET_WHITELISTED_ACCOUNTS(
        uint id
    ) public view returns (address[] memory) {
        require(
            universities[id].uni_owner == msg.sender,
            "Only UNIVERSITY Can check the whitelisted Accounts"
        );
        return universities[id].whitelisted_accounts;
    }

    function WHITELIST_ADDRESS_FOR_UNIVERSITY(
        uint id,
        address whitelist_account
    ) public {
        require(
            universities[id].uni_owner == msg.sender,
            "Only UNIVERSITY owner can whitelist accounts"
        );
        universities[id].whitelisted_accounts.push(whitelist_account);
    }
    ...
    function GENERATE_CERTIFICATE(
        ...
        string memory _certificate_uri,
        uint _uni_id
    ) public returns (string memory message) {
        require(
            _isWhiteListedAccount(msg.sender, _uni_id),
            "Only whitelisted Accounts can generate token"
        );
        uint256 cert_id = _certificate_Counter.current();
        _certificate_Counter.increment();
        certificates[cert_id].student_name = _student_name;
        certificates[cert_id].owner = _student_address;
        certificates[cert_id].course = _course;
        certificates[cert_id].startDate = _startDate;
        certificates[cert_id].endDate = _endDate;
        certificates[cert_id].department = _department;
        certificates[cert_id].contact_number = _contact_number;
        certificates[cert_id].cgpa = _cgpa;
        certificates[cert_id].certificate_uri = _certificate_uri;
        certificates[cert_id].tenure = _tenure;
        _mint(msg.sender, _student_address, cert_id, _certificate_uri);
    ...
    function _isWhiteListedAccount(
        ...
        address[] memory accounts = universities[id].whitelisted_accounts;
    ...
    // function GET_ALL_UNIVERSITIES() public returns(UNIVERSITY[] memory universities_listed) {
    //     uint totalUniversities = _university_Counter.current();
    //     UNIVERSITY memory universities_listed[];
    //     for(uint i=0;i< totalUniversities;i++){
    //         universities_listed.push(universities[i]);
    //     }
    //     return universities_listed;
    // }
    function GET_ALL_UNIVERSITIES() public view returns (UNIVERSITY[] memory) {
        uint uni_count = _university_Counter.current();
        UNIVERSITY[] memory id = new UNIVERSITY[](uni_count);
        for (uint i = 0; i < uni_count; i++) {
            UNIVERSITY storage university = universities[i];
            id[i] = university;
        }
        return id;
    }

    function GET_ALL_CERTIFICATES() public view returns (CERTIFICATE[] memory) {
        uint cert_count = _certificate_Counter.current();
        CERTIFICATE[] memory id = new CERTIFICATE[](cert_count);
        for (uint i = 0; i < cert_count; i++) {
            CERTIFICATE storage certificate = certificates[i];
            id[i] = certificate;
        }
        return id;
    }

    function getMYCertificate() public view returns (CERTIFICATE[] memory) {
        uint cert_count = _certificate_Counter.current();
        CERTIFICATE[] memory id = new CERTIFICATE[](cert_count);
        for (uint i = 0; i < cert_count; i++) {
        ...
        require(id.length > 0, "NO Certificates found");
        return id;
    }
}
