...
contract NFT is ERC721URIStorage, ERC721Enumerable, Permissions {
    ...
    uint public _patentFees = 4600000000000000;
    uint public _transferCommission = 5;
    mapping(uint => TokenInfo) public _tokenList;
    mapping(address => uint[]) public _ownedTokens;
    mapping(address => uint) public _ownedTokensCount;
    mapping(uint => address) public _initialTokenOwnerOf;
    mapping(uint => address) public _currentTokenOwnerOf;
    mapping(uint => string) public _tokenURIOf;
    mapping(uint => bool) public _feePaymentStatus;
    mapping(uint => address) public _tokenBuyer;
    mapping(uint => uint)public _transferCount;
    ...
    function mint(uint _tokenId, string memory _tokenURI, address _owner) external onlyAuthority {
        require(_feePaymentStatus[_tokenId] = true, "Fees has not been paid for this patent." );
        tokenCount ++;
    ...
    function listTokensOf(address _address) external view returns(TokenInfo[] memory){
        uint ownedTokensCount = _ownedTokensCount[_address];
        uint[] memory ownedTokens = _ownedTokens[_address];
        TokenInfo[] memory ownTokens = new TokenInfo[](ownedTokensCount);
        for(uint i=0;i<ownedTokensCount;i++){
    ...
    function setTransferCommission(uint _percent) external onlyAuthority{
        _transferCommission = _percent;
    }

    function setPatentFees(uint _amount) external onlyAuthority{
        _patentFees = _amount;
    }

    function payPatentFees(uint _tokenId) external payable{
        require(_feePaymentStatus[_tokenId] == false, "Fees for this patent has already been paid");
...
//Should be called by the token owner
    function giveApprovalToContract( uint _tokenId) public {
        approve(address(this), _tokenId);
    }

    function setTokenBuyer(uint _tokenId, address _buyer) external onlyAuthority{
...
//Should be called(by buyer) after getting approval from token owner
    function buyNft(uint _tokenId) external payable {
        require(msg.sender == _tokenBuyer[_tokenId], "Transfer not authoried by Authority.");
        require(msg.value > 0, "Please enter value more than zero.");
        ...
        uint _commission = _totalPrice * _transferCommission / 100;
        uint _remainingAmount = _totalPrice - _commission;
        ...
        emit TransferNFT(_tokenId, _tokenOwner, _buyer, _totalPrice, _commission);
    }
