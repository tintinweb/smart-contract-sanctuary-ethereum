...
contract SRGF3 is SpaceRidersDockable {
    ...
    function mint(uint256 _id, bytes32[] calldata proof) external nonReentrant {
    ...
    function mintMultipleToAddress(
        ...
        for (
            ...
            tokenTypes[nextTokenId] = _id;
        }
        minted[_id] += _amount;
        _safeMint(_address, _amount);
    }

    function mintRemainingSupplyToAddress(address _address) external onlyOwner {
        for (
            uint256 tokenTypeId = 0;
            tokenTypeId < supplies.length;
            ++tokenTypeId
        ) {
            uint256 remainingSupply = remainingSupplyOfTokenType(tokenTypeId);
            if (remainingSupply > 0) {
                uint256 mintIndex = _nextTokenId();
                for (
                    uint256 nextTokenId = mintIndex;
                    nextTokenId < (mintIndex + remainingSupply);
                    ++nextTokenId
                ) {
                    tokenTypes[nextTokenId] = tokenTypeId;
                }
                minted[tokenTypeId] += remainingSupply;
                _safeMint(_address, remainingSupply);
            }
        }
    }
}