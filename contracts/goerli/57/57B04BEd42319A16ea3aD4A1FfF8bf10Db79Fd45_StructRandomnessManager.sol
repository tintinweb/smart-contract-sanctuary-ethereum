...
import "../interfaces/IRandomnessManager.sol";
import "../interfaces/IPaymentManager.sol";

error StructRandomnessManager_CallerNotOwner();
error StructRandomnessManager_IndexAlreadySet();
error StructRandomnessManager_IndexNotSet();
error StructRandomnessManager_InsufficientPermissions();
 ...
 */
contract StructRandomnessManager is IRandomnessManager, VRFConsumerBaseV2, AccessControl {
    ...
    IPaymentManager internal immutable _PAYMENT_MANAGER;
    ...
    bytes32 public constant VRF_ALLOWED_ROLE = keccak256("VRF_ALLOWED_ROLE");
     ...
     * @param slot TODO
     */
    function _getIndex(address collection, uint256 slot) internal pure returns (bytes32) {
    ...
    function isSet(address collection, uint256 slot) external view returns (bool) {
        return _indexToRandomness[_getIndex(collection, slot)] != 0;
    }
    ...
    function randomness(address collection, uint256 slot) external view returns (uint256) {
        ...
        if (_indexToRandomness[index] == 0) revert StructRandomnessManager_IndexNotSet();

        return _indexToRandomness[index];
    }
    ...
    /**
     * @notice Set randomness using Chainlink VRF
     * @param collection TODO
     * @param slot TODO
     */
    function setWithVRF(address collection, uint256 slot) external {
        ...
        if (tx.origin != Ownable(collection).owner()) revert StructRandomnessManager_CallerNotOwner();
        if (_indexToRandomness[index] != 0) revert StructRandomnessManager_IndexAlreadySet();
        if (!_PAYMENT_MANAGER.hasRole(VRF_ALLOWED_ROLE, tx.origin))
            revert StructRandomnessManager_InsufficientPermissions();

        _requestsToIndex[_COORDINATOR.requestRandomWords(
...
interface IPaymentManager {
    ...
    function validateDeployment(Receipt calldata receipt, bytes calldata signature) external;
    function validateRole(Receipt calldata receipt, bytes calldata signature) external;
    function validateOp(Receipt calldata receipt, bytes calldata signature) external;
