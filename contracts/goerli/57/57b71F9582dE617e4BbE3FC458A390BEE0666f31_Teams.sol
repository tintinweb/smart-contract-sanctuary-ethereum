...
import "@thirdweb-dev/contracts/interfaces/drop/IDropERC1155.sol";
import "@thirdweb-dev/contracts/base/ERC1155Drop.sol";
import "@openzeppelin/contracts/metatx/ERC2771Context.sol";

error Teams__AlreadyHaveTeam(address user);
...
contract Teams {
    ...
    mapping(address => Team) private s_teams;

    address[] private s_users;

    // Players contract address
    address private s_playersContract;

    ////////////////
    //// EVENTS ////
    ////////////////
    event NewTeam(address indexed user, Team team);

    ///////////////////
    //// MODIFIERS ////
    ///////////////////
    modifier onlyOneTeam(address user) {
        if (s_teams[user].score > 0) {
            revert Teams__AlreadyHaveTeam(user);
        }

        _;
    }

    constructor(address _playersContract) {
        s_playersContract = _playersContract;
    }
    ...
    function createTeam(
        ...
        uint256 _player2,
        uint256 _sub
    ) public onlyOneTeam(_msgSender()) {
        // s_listings[nftAddress][tokenId] = Listing(price, msg.sender);
        Team memory newTeam = Team(_player1, _player2, _sub, 0);
        s_teams[user] = newTeam;
        s_users.push(user);

        emit NewTeam(user, newTeam);
    }

    function modifyTeam(address user, uint256 _player1, uint256 _player2, uint256 _sub) public {
        s_teams[user].player1 = _player1;
        s_teams[user].player2 = _player2;
        s_teams[user].substitute = _sub;

        // emit TeamModified(user, s_teams[user]);
    }

    function addScore(uint256 player, uint256 points) public {
        // Loop through all teams in the mapping
        for (uint256 i = 0; i < s_users.length; i++) {
            // Get the current team
            Team memory team = s_teams[s_users[i]];

            // Check if the team has the specified player as player1 or player2
            if (team.player1 == player || team.player2 == player) {
                // Add the points to the team's score
                s_teams[s_users[i]].score += points;
            }
        }
    }

    function getTeam(address user) public view returns (Team memory) {
        return s_teams[user];
    }

    function getAllTeams() public view returns (Team[] memory) {
        Team[] memory allTeams = new Team[](s_users.length);
        for (uint256 i = 0; i < s_users.length; i++) {
            allTeams[i] = s_teams[s_users[i]];
        }
        return allTeams;
    }

    function getTeamScore(address user) public view returns (uint256) {
        return s_teams[user].score;
    }

    function getPlayersContract() public view returns (address) {
        return s_playersContract;
    }
