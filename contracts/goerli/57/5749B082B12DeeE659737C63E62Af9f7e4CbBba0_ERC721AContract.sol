...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer {
    ...
    function registerPaperKey(address _paperKey) external onlyOwner {
        require(paperKeyManager.register(_paperKey), "Registration error");
    }
    ...
    function checkClaimEligibility(address _address, uint256 _quantity) external view returns (string memory) {
    ...
    function mint(
        ...
        if(_preSaleIsActive) {
            ...
            if (token.preSalePhase == 1) {
                isAllowed = mintpass.balanceOf(msg.sender, 1) >= _quantity;
                if (isAllowed) mintpass.burnForAddress(1, _quantity, msg.sender);
            }
        ...
        if (!isAllowed) {
            ...
            require(token.pubSalePrice * _quantity <= msg.value, "ETH incorrect");
            require(_quantity <= token.maxPerTransaction, "Invalid quantity");
        }
        _safeMint(msg.sender, _quantity);
        _payReferral(msg.sender, _referrer, _quantity, msg.value);
    }
    ...
    function fiatMint(address _address, uint256 _quantity, address payable _referrer, uint256 _value) internal {
        require(token.pubSaleIsActive, "Sale inactive");
    ...
    function crossmint(address _address, uint256 _quantity, address payable _referrer) external payable onlyCrossmint {
        fiatMint(_address, _quantity, _referrer, msg.value);
    }

    function paper(address _address, uint256 _quantity, address payable _referrer, bytes32 _nonce, bytes calldata _signature) external payable 
        onlyPaper(keccak256(abi.encode(_address, _quantity, _referrer)), _nonce, _signature)
    {
        fiatMint(_address, _quantity, _referrer, msg.value);
    }

    function claimFree(uint16 _maxMint, uint16 _quantity, bytes32[] memory _proof) external {
