 ...
 * {ERC721Enumerable}.
 */
contract ERC721 is Context, IERC721, IERC721Metadata, ERC165, ERC2981 {
...
contract NFT721 is ERC721Permit, Ownable {
    using Strings for address;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    string public baseURI;

    constructor(string memory name_, string memory symbol_, string memory baseURI_, uint96 defaultRoyalty_) 
        ERC721Permit(name_, symbol_) {
            setDefaultRoyalty(defaultRoyalty_);
    ...
    function batchTransfer(address[] memory recipients, uint256[] memory tokenIds) public {
        require(recipients.length == tokenIds.length, "Recipients and tokenIds length not match");
        for (uint i = 0; i < tokenIds.length; i++) {
            _transfer(_msgSender(), recipients[i], tokenIds[i]);
        }
    }
    ...
    function mint(address to_, uint96 royalty_) public onlyOwner returns (uint256) {
        ...
        _setTokenRoyalty(newItemId, to_, royalty_);
        return newItemId;
    }
