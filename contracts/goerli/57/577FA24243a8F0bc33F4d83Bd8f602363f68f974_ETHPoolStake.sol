...
contract ETHPoolStake is Ownable {
    ...
    /**
     *  @dev Accumulated Reward amount for a user.
     *      Real reward is subtraction of this accuRewards from the reward that just summarized.
     */
    mapping(address => uint256) private accuRewards;

    /**
     *  @dev We use UNIT to make calculation of fixed point numbers to be calculation of integer.
     */
     ...
     */
    event RewardDeposit(uint256 _amount);
    ...
    /**
     *  @notice Function for user to deposit some amount of ETH
     *  @dev we calculate stakedBalances, totalStakedBalance and accuRewards here.
     *      Calculating accuRewards is main focus here:
     *      accuRewards[msg.sender] += totalRewardRate * msg.value
     */
    function deposit() external payable {
        ...
        uint256 _lastRevertedReward = totalRewardRate.mul(msg.value).div(UNIT);
        accuRewards[msg.sender] = accuRewards[msg.sender].add(
            _lastRevertedReward
        );
     ...
     *  @dev we calculate totalRewardRate here:
     *      totalRewardRate += msg.value / totalStakedBalance
     *  @dev Throws when totalStakedBalance is zero
     */
    function rewardDeposit() external payable onlyOwner {
        require(totalStakedBalance > 0, "No user has deposited yet");

        uint256 _lastRewardRate = msg.value.mul(UNIT).div(totalStakedBalance);
        totalRewardRate = totalRewardRate.add(_lastRewardRate);
     ...
     *  @dev _withdrawReward = _stakedBalance * totalRewardRate - accuRewards[staker]
     *  @dev Throws when staked Balance is zero.
     *  @param user Address of user(Staker)
     *  @return _withdrawReward Real Reward that staker can get so far.
     */
    function getRewardAmount(address user)
        public
        view
        returns (uint256 _withdrawReward)
    {
        require(stakedBalances[user] > 0, "You have no balance to withdraw");

        uint256 _rewardForStaked = stakedBalances[user]
            .mul(totalRewardRate)
            .div(UNIT);
        _withdrawReward = _rewardForStaked.sub(accuRewards[user]);
    }
}
