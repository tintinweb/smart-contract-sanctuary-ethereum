...
contract DPSDocks is ERC721Holder, ERC1155Holder, Ownable {
    ...
    function lockVoyageItems(
        ...
        for (uint256 index; index < _lockedVoyages.length; index++) {
            ...
            bytes memory uniqueId = abi.encodePacked(msg.sender, "LOCK_VOYAGE", randomRequestIndex, block.timestamp);
            randomRequestIndex++;
     ...
     * @param _voyage the voyage address we claim the rewards for
     * @param _pirate the pirate used to claim the voyages
     */
    function claimRewards(
        uint256[] calldata _voyageIds,
        DPSVoyageIV2 _voyage,
        IERC721Metadata _pirate
    ) external {
        if (nonReentrant == 2 || !voyages[_voyage] || !pirates[_pirate]) revert Unauthorized();
        ...
        for (uint256 i; i < _voyageIds.length; i++) {
            ...
            LockedVoyageV2 storage lockedVoyage = lockedVoyages[owner][voyageId];
            VoyageConfigV2 memory voyageConfig = _voyage.getVoyageConfig(voyageId);
            ...
            if (keccak256(bytes(_pirate.symbol())) == keccak256(bytes("DPS")))
                (, features) = dpsFeatures.getTraitsAndSkills(uint16(lockedVoyage.dpsId));
            else {
                features[0] = 150;
                features[1] = 150;
                features[2] = 150;
            ...
            LockedVoyageV2 memory cached = lockedVoyage;

            //TODO not sure if this should be afterwards. need to check
            cleanLockedVoyage(lockedVoyage.voyageId, owner);

            awardRewards(voyageResult, voyageConfig.typeOfVoyage, cached, owner, _voyage, _pirate);
    ...
    function checkVoyageState(
        ...
        DPSVoyageIV2 _voyage,
        IERC721Metadata _pirate
    ) external view returns (VoyageResult memory voyageResult) {
        LockedVoyageV2 storage lockedVoyage = lockedVoyages[ownerOfLockedVoyages[_voyageId]][_voyageId];

        // not started
        if (_voyage.ownerOf(_voyageId) != address(this) || lockedVoyage.voyageId == 0) revert WrongState(1);

        VoyageConfigV2 memory voyageConfig = _voyage.getVoyageConfig(_voyageId);
        voyageConfig.sequence = lockedVoyage.sequence;
        ...
        return gameEngine.computeVoyageState(lockedVoyage, voyageConfig.sequence, randomNumber, features);
    }
    ...
    function awardRewards(
        ...
        _pirate.safeTransferFrom(address(this), _owner, _lockedVoyage.dpsId);
        ...
        _voyage.burn(_lockedVoyage.voyageId);
    }
    ...
    function setContract(
        ...
        else if (_target == 10) chest = DPSChestsIV2(_contract);
        else if (_target == 11) gameEngine = DPSGameEngineI(_contract);
