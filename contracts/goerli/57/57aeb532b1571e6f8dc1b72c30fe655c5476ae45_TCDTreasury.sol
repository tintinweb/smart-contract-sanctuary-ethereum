...
//  the rebasing tokens mania was proof that people want sustainable Up Only investments that guarantee value retention
...
//  but elastic supply was just one of the many innovative money concepts that will come to this space
...
contract TestContractDAO is ERC20, Ownable {
    ...
    uint256 public _discountRate = 5;//discount for helping us liquidate fee tokens when you use _buy() function
...
contract TCDTreasury {//not ownable but checks main contract owner as admin



    TestContractDAO private token; // Token contract
	...
	constructor(address payable _tokenAdd, address _bokkyAdd) {
		...
		token = TestContractDAO(_tokenAdd);
