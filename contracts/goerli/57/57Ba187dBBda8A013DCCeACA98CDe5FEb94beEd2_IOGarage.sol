...
error exceedmaxPublicMintPerTx();
...
error publicMintPriceCantBeZero();
    ...
    OperatorFilterer
{
    ...
    address private IORidesContract;
    address private IOSpheresContract;
    ...
    constructor(string memory _name, string memory _symbol)
        ...
        OperatorFilterer(msg.sender, true)
    {}
    ...
    function mintOnlyOwner(address _to, uint256 qty)
        public
        mintCompliance(qty)
        nonReentrant
        onlyOwner
    ...
    function publicMint(
        ...
        if (
            maxPublicMintPerTx > 0 &&
            (publicMinted[msg.sender] + _mintAmount) > maxPublicMintPerTx
        ) revert exceedmaxPublicMintPerTx();
        if (
            publicMintSupply > 0 &&
    ...
    function setPublicMintSupply(uint256 _publicMintSupply) external onlyOwner {
    ...
    function setPublicMintPrice(uint256 _price) external onlyOwner {
        if (_price <= 0) revert publicMintPriceCantBeZero();
...
interface IOperatorFilterRegistry {
    ...
    function subscriptionOf(address addr) external view returns (address registrant);
