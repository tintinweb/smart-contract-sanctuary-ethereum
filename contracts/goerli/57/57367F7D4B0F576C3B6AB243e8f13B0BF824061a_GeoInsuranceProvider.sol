...
contract GeoInsuranceProvider is ShambaChainSelector {
    ...
    */    
    ...
    function newContract(address payable _client, uint _premium, uint _payoutValue) public payable onlyOwner() returns(address) {
        ...
        GeoInsuranceConsumer i = (new GeoInsuranceConsumer){value:((_payoutValue * 1 ether) / (uint(getLatestPrice())))}(_client, _premium, _payoutValue, LINK, ORACLE_PAYMENT);
...
contract GeoInsuranceConsumer is ShambaGeoConsumer  {
    ...
    uint premium;
    uint payoutValue;
    int constant public GEOSTATS_THRESHOLD = 0.5 * 10**18;
    ...
    constructor(address payable _client, uint _premium, uint _payoutValue, 
                address _link, uint256 _chainId) payable ShambaGeoConsumer(_chainId) {
