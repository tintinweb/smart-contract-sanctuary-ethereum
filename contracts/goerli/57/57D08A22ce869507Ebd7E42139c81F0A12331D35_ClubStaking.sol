...
contract ClubStaking is Initializable, OwnableUpgradeable {
    ...
    /**
     * @notice to get current user's staking amount, reward amount
     * @param _stakeholder The stakeholder address.
     */
    function getStakingStatus(address _stakeholder)
        public
        view
        returns (uint256 stakeAmount, uint256 rewardAmount)
    {
        StakeList storage _personStakeStatus = stakeLists[_stakeholder];
        uint256 _reward = updateReward(_stakeholder);

        return (_personStakeStatus.stakesAmount, _reward);
    }
