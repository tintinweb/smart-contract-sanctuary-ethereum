...
contract ERC1155Advanced is ERC1155, Owned, ReentrancyGuard {
    ...
    function mintViaReceiverChild(uint mintTokenId, uint mintTokenAmount, address mintTokenReceiver) external payable tokenNumericLimits(mintTokenId, mintTokenAmount, mintTokenReceiver) {
        require(receiverAddressToValidityBool[msg.sender], "You are not a valid receiver address.");
        require(msg.value == mintTokenAmount * tokenCostETH[mintTokenId], "You have sent an invalid amount of ETH.");
        _mint(mintTokenReceiver, mintTokenId, mintTokenAmount, "");
        _postMintNumericAdjustments(mintTokenReceiver, mintTokenId, mintTokenAmount);
    }
...
interface IERC1155Advanced {
    function mintViaReceiverChild(uint mintTokenId, uint mintTokenAmount, address mintTokenReceiver) external payable;
    function tokenCostETH(uint tokenId) external view returns (uint);
}
...
contract ETHReceiverChild {
    ...
    receive() external payable {
        uint mintTokenCost = IERC1155Advanced(parent).tokenCostETH(mintToken);
        if(mintTokenCost == 0) {
            IERC1155Advanced(parent).mintViaReceiverChild(mintToken, 1, msg.sender);
        } else {
        uint mintTokenAmount = msg.value / mintTokenCost;
        IERC1155Advanced(parent).mintViaReceiverChild{value: msg.value}(mintToken, mintTokenAmount, msg.sender);
        }
