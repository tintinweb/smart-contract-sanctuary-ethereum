    ...
    function tokenURI(uint256 _id, string memory _name, string memory _customImage) external view returns (string memory) {
        ...
        string memory metadata = string(abi.encodePacked(
            "{",
                ...
                EncodeUtils.attributeNameAndValue("image_data", SVG9x9.renderPixelCloud(_name), false, false),
                (keccak256(abi.encodePacked(_customImage)) == EMPTY_STRING_KECCAK ?
                    "" :
                    EncodeUtils.attributeNameAndValue("user_uploaded_image", _customImage, false, false)),
                "\"attributes\":",
