...
contract NFTLaunchPad is Initializable, OwnableUpgradeable {
    ...
    event currencySupport(
        address[] indexed currencies,
        address[] indexed whiteListCurrencies,
...
contract NFTCollection is ERC721A, ERC2981, Ownable {
    ...
    function _setCurrencyForwhiteList(
        ...
        for (uint256 i = 0; i < _whiteListCurrencies.length; i++) {
            require(
                _currenciesPriceWhiteList[i] > 0,
                "NFTCollection: WhiteListCurrency not Supported"
            );
    ...
    */
    ...
    function mint(
        ...
        require(
            currenciesPrice[_currency] > 0 || currencies[_currency] == true,
            "NFTCollection: Currency not Supported"
        );
        ...
        if (
            ...
            if (whitelistedFee > 0) {
                uint256 brokerage = _getBrokerage(address(currency));
