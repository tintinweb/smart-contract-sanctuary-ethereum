...
library NFTSVG {
  function generateFullSVG(
    uint256 _tokenId,
    Params.Character[][8] memory _characters,
    Params.SVGParams memory _params
  ...
  function generateFilterGrey(Params.SVGParams memory _params) internal pure returns (bytes memory) {
    return
      bytes.concat(
        bytes.concat('<filter id="FG-0">'), //
        bytes.concat('<feColorMatrix values="', Combine.getMatrixByPaletteAndOffset(_params.color.palette, 0), '"/>'),
        "</filter>",
        bytes.concat('<filter id="FG-1">'), //
        bytes.concat('<feColorMatrix values="', Combine.getMatrixByPaletteAndOffset(_params.color.palette, 1), '"/>'),
        "</filter>",
        bytes.concat('<filter id="FG-2">'), //
        bytes.concat('<feColorMatrix values="', Combine.getMatrixByPaletteAndOffset(_params.color.palette, 2), '"/>'),
        "</filter>"
      );
  }
  ...
  function generateFilterDSA() internal pure returns (bytes memory) {
    return
      bytes.concat(
        ...
        bytes.concat('<feOffset dx="2" dy="2" height="100%" in="SourceGraphic" result="offset1"/>' '<feComposite in="SourceGraphic" in2="offset1" operator="out" width="100%" height="100%" result="composite1"/>'),
  ...
  function generateBackground(Params.SVGParams memory _params) internal pure returns (bytes memory) {
    ...
    return
      bytes.concat(
        ...
        bytes.concat('<g filter="url(#FG-', bytes(Strings.toString(_params.pattern_grey_id % 3)), ')">'),
        bytes.concat('<rect width="100%" height="100%" class="colR"/>'),
        bytes.concat('<g filter="url(#FDSA)">'),
        bytes.concat('<rect width="100%" height="100%" fill="url(#pattern)" />'),
        "</g>",
        "</g>",
        bytes.concat('<g filter="url(#FG-', bytes(Strings.toString(_params.noise.grey_id % 3)), ')">'),
        tmp,
        "</g>",
        "</g>"
      );
  ...
  function generateScreen() internal pure returns (bytes memory) {
    return
      bytes.concat(
        ...
        '<rect x="620" y="815" width="196" height="33"/>',
        "</g>",
        "</g>",
        "</g>"
      );
  ...
  function generateTitle(
    ...
    return
      bytes.concat(
        ...
        "</g>",
        "<g>",
        abi.encodePacked('<text class="colB" style="font-size:32px;font-weight:bold;font-family: \'Courier New\',monospace;" text-anchor="middle" transform="translate(718,840)">', Combine.generateTokenId(_tokenId), "</text>"),
        "</g>",
        "</g>",
        "</g>"
      );
...
library Combine {
  ...
  index 1 - 3
   */
  function getMatrixByPaletteAndOffset(uint8 _palette, uint8 _index) internal pure returns (bytes memory) {
    _palette = _palette % 3;
    _index = (_index % 3) + 1;
    bytes memory row;

    bytes[3] memory red = [bytes("0.3"), "0.053", "0.008"];
  ...
  function generateTokenId(uint256 _tokenId) internal pure returns (string memory) {
    bytes[10] memory mappings = [bytes("0"), "1", "2", "W", "C", "5", "P", "U", "A", "X"];

    uint8 wordSize = 0;
    bytes memory output;
    while (_tokenId != 0) {
      wordSize++;
      output = bytes.concat(mappings[_tokenId % 10], output);
      _tokenId /= 10;
    }

    while (wordSize != 5) {
      wordSize++;
      output = bytes.concat(mappings[0], output);
    }
    return string(bytes.concat('T_', output, '_T'));
  }
}