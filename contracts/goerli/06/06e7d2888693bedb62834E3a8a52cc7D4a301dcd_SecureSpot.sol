...
contract SecureSpot is Ownable {
    ...
    function _hasPyxis(address to) private view returns (bool) {
        return pyxis.getToken(to) > 0;
    }
        ...
        payable
    {
        ...
        require(_hasPyxis(msg.sender), "No pyxis found");
    ...
    function _isAirdropActive() private view returns (bool) {
        uint256 buffer = 0; // 5 days // owner 0 others 5 days
    ...
    function airdropBulk(uint256 limit) public {
        ...
        for (;i <= _lastAirdropIndex; i++) {
            ...
            try mintContract.mintReservedNFT{value: 0}(to) {
                ...
                emit Airdrop("failed", to); // 10 -> event 8 the fail > unReserve 8 > run all 
                revert(
    ...
    function setStartTime(uint256 _time) public onlyOwner {
        require(_time > block.timestamp && _time > startTime, "Can't set older time");
        startTime = _time;
    }

    function setEndTime(uint256 _time) public onlyOwner {
        require(_time > startTime, "Should be greater than start time");
    ...
    function withdraw() public onlyOwner {
        uint256 buffer = 0; // no buffer
        require(endTime + buffer < block.timestamp, "not ready for withdraw");
