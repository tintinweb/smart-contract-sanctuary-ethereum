...
contract FritzTheCoin is ERC20, Ownable {
    ...
    mapping(address => uint256) public migratePerUser;
    ...
    function migrateToV2(uint256 amount) external{
        require(migratePerUser[_msgSender()] >= amount, "there is no token allowed to migrate");
        migratePerUser[_msgSender()] -= amount;
        _mint(_msgSender(), amount * 1e18);
        emit MintedTo(_msgSender(), amount);
    }

    function setMigratePerUser(address target, uint256 amount) external onlyOwner{
        require(target != address(0), "invalid user");
        migratePerUser[target] += amount;
    }
