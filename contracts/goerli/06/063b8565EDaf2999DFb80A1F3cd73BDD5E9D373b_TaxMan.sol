...
contract TaxMan is ERC721A, Ownable, OperatorFilterer, ERC2981 {
    ...
    bool saleLive;
    bool whitelistSaleLive;
    bool isRevealed;
    bool public operatorFilteringEnabled;

    uint256 public constant maxSupply = 8888;
    uint256 public constant whiteListSupply = 4444;
    uint256 public constant maxPerTxPublic = 5;
    uint256 public constant maxWhitelistMint = 2;
    uint256 private price = 0.025 ether;
    //Signer address for the hash
    bytes32 private merkleRoot;

    event UpdateSaleStatus(bool saleLive);
    event UpdateWhitelistSaleStatus(bool saleLive);
    event UpdateRevealStatus(bool isRevealed);
    event UpdateMerkleRoot(bytes32 merkleRoot);

    constructor(string memory _BaseURI, string memory _NotRevealedUri, bytes32 merkleRoot_) ERC721A("TaxMan", "TMAN") {
        setBaseURI(_BaseURI);
    ...
    function setSaleState(bool _state) public onlyOwner {
        saleLive = _state;
        emit UpdateSaleStatus(saleLive);
    }

    function setWhitelistSaleState(bool _state) public onlyOwner {
        whitelistSaleLive = _state;
        emit UpdateWhitelistSaleStatus(whitelistSaleLive);
    }

    function reveal(bool _state) public onlyOwner {
        isRevealed = _state;
        emit UpdateRevealStatus(isRevealed);
    }
    ...
    function whitelistMint(uint64 amount, bytes32[] calldata _merkleProof) public payable {
        if (!whitelistSaleLive) revert SaleNotLive(); // Check if sale is live
        if (totalSupply() + amount > whiteListSupply) revert OverMintLimit(); // This is to limit the supply to whiteListSupply
        if (msg.value < amount * price) revert InvalidMsgValue(); // Check Price is correct
        uint64 whiteListMinted = _getAux(msg.sender) + amount;
        if (_getAux(msg.sender) > 1) revert OverMintLimit();
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        if (!MerkleProofLib.verify(_merkleProof, merkleRoot, leaf)) revert InvalidProof();
        _setAux(msg.sender, whiteListMinted);
    ...
    function publicMint(uint256 _mintAmount) public payable callerIsUser {
        ...
        if (_mintAmount > maxPerTxPublic) revert OverMintLimit(); //Ensure 5/tx
    ...
    function teamMint(uint256 _amount) external payable onlyOwner {
        if (totalSupply() + _amount > maxSupply) revert OverMintLimit();
