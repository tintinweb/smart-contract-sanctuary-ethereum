...
contract BinaryVaultManager is Ownable, IBinaryVaultManager {
    ...
    function registerVault(IBinaryVault vault) external onlyOwner {
        IERC20 underlyingToken = vault.underlyingToken();
        if (vaults[address(underlyingToken)] != address(0)) {
            // Duplicated. Replace
            vaults[address(underlyingToken)] = address(vault);

            emit VaultAdded(address(vault), address(underlyingToken), false);
        } else {
            // New register
            vaults[address(underlyingToken)] = address(vault);
            underlyingTokens.push(address(underlyingToken));

            emit VaultAdded(address(vault), address(underlyingToken), true);
        }
    }
...
interface IBinaryVaultManager {
    function registerVault(IBinaryVault vault) external;
}
