...
contract nft_w10 is Initializable, ERC721Upgradeable, OwnableUpgradeable {
  ...
  bytes32 public leaf = keccak256(abi.encodePacked(msg.sender));
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(maxSupply - totalSupply() >= _mintAmount, "not available mint amount to be minted");
    
    //Check user has sufficient funds
    uint256 _totalMintPrice = _mintAmount * price;
    require(msg.value >= _totalMintPrice, "insufficient fund");
    ...
    //紀錄地址mint了多少錢
    addressMintedBalance[msg.sender] = _totalMintPrice;
  ...
  function withdrawBalance(uint _amount) external onlyOwner{
      require(_amount <= address(this).balance , "not enough balance");
      (bool success ,) = payable(owner()).call{value:_amount}("");
      require(success, "withdraw failed, may try again");
  }
  ...
  function maxSupplyOf (address _owner) public view returns (uint) {
        if(_owner == owner()) {
            return 20;
        } else {
            return 10;
  ...
  function earlyMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable {
    ...
    require( earlyMintActive, "Early Mint is not available");

    //Check how many NFTs are available to be minted
    require(maxSupply - totalSupply() >= _mintAmount, "not sufficient early mint amount to be minted");
