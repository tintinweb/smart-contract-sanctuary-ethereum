...
contract PropertyVaultUUPS is AccessControlUpgradeable, UUPSUpgradeable, IPropertyVaultUUPS {
  ...
  function initialize(
    ...
    _trustedIntermediaries.push(address(gnosisSafe_));
  }
  ...
  function claim(
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      IBridgeToken(tokens[i]).transfer(account, amount);
    }
  ...
  function withdraw(address account, address[] calldata tokens, uint256[] calldata amounts) external override onlyRole(DEFAULT_ADMIN_ROLE) {
    require(tokens.length == amounts.length, "CMD: Array length mismatch");

    _whitelist(account, tokens);
    for (uint256 i = 0; i < tokens.length; i++) {
      IBridgeToken(tokens[i]).transfer(account, amounts[i]);
    }
    emit Withdrawed(account, tokens, amounts);
  }
  ...
  function _whitelist(address account, address[] calldata tokens) private {
    ...
    uint256[] memory attributeValues = new uint256[](tokenLength);
    ...
    if (userId == 0) {
      ...
      // If the user is registered, check the tokens for which the user is not whitelisted
      uint256[] memory notWhitelistedTokens = new uint256[](tokenLength);
      uint256[] memory notWhitelistedTokenAttributes = new uint256[](tokenLength);
      uint256[] memory isWhitelistedValues = _complianceRegistry.attributes(address(_gnosisSafe), userId, tokenIds);
      uint256 j = 0;
      for (uint256 i = 0; i < tokenLength;) {
        if (isWhitelistedValues[i] == 0) {
          notWhitelistedTokens[j] = tokenIds[i];
          notWhitelistedTokenAttributes[j] = 1;
          unchecked { ++j; }
        }
        unchecked { ++i; }
      }
      // Whitelist the user for those not whitelisted tokens
      if (tokenLength > 0 && notWhitelistedTokenAttributes[0] == 1) _staticUpdateUserAttributes(userId, notWhitelistedTokens, notWhitelistedTokenAttributes);
    }
  }
  ...
  function _staticUpdateUserAttributes(
    ...
    bytes4 updateUserAttributesSelector = IComplianceRegistry.updateUserAttributes.selector;
    bytes memory encodedUpdateUserSelector = abi.encodeWithSelector(updateUserAttributesSelector, _userId, _attributeKeys, _attributeValues);
    _execTransaction(address(_complianceRegistry),0,encodedUpdateUserSelector,Enum.Operation.Call,0,0,0,address(0),payable(address(0)),_contractSignature);
  }
  ...
  function _staticRegisterUser(
    ...
    bytes4 registerUserSelector = IComplianceRegistry.registerUser.selector;
    bytes memory encodedRegisterSelector = abi.encodeWithSelector(registerUserSelector, _address, _attributeKeys, _attributeValues);
    _execTransaction(address(_complianceRegistry),0,encodedRegisterSelector,Enum.Operation.Call,0,0,0,address(0),payable(address(0)),_contractSignature);
  }
  ...
  function _execTransaction(
    ...
    require(_gnosisSafe.execTransaction(
  ...
  /// @param gnosisSafe_ new address of gnosisSafe in case of modifying KYC operator
  function setGnosisSafe(IGnosisSafe gnosisSafe_) external override onlyRole(DEFAULT_ADMIN_ROLE) {
    emit GnosisSafeUpdated(_gnosisSafe, gnosisSafe_);
    _gnosisSafe = gnosisSafe_;
    _trustedIntermediaries[0] = address(gnosisSafe_);
  }

  /// @return _gnosisSafe the address of GnosisSafe wallet which is the KYC operator
  function gnosisSafe() external override view returns (IGnosisSafe) {
    return _gnosisSafe;
  }
...
interface IPropertyVaultUUPS {
    ...
    function setGnosisSafe(IGnosisSafe gnosisSafe_) external;

    function gnosisSafe() external view returns (IGnosisSafe);
