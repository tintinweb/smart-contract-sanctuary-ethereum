...
contract MoonForceHQ is Ownable, ReentrancyGuard {
    ...
    mapping(address => EnumerableSet.UintSet) WastelandGeneralsOfWallet;
   ...
   */
    function addManyToStakingPool(address account, uint16[] calldata tokenIds) external nonReentrant notContract() {
    ...
    function sendGeneralToWastelands(uint16[] calldata _ids) external payable notContract() {
        ...
        for (uint16 i = 0; i < numWords;) {
            ...
            if (HUB.alienCount() > 0 && (seed[i]) % 100 < 25) { // stolen
                ...
                WastelandGeneralsOfWallet[msg.sender].add(_ids[i]);
