...
contract AAPEHotSauce is ERC1155, Ownable, PaymentSplitter, DefaultOperatorFilterer  {
    ...
    mapping(uint256 => bool) public customSauces;
    ...
    function setCustomSauce(uint256 _id) external onlyOwner {
        customSauces[_id] = true;
    }

    function unsetCustomSauce(uint256 _id) external onlyOwner {
        delete customSauces[_id];
    }

    function updateSaleState(
        bool _saleIsActive,
        bool _customIsActive
    ) public onlyOwner {
        if (_customIsActive) require(merkleRoot != "", "Root undefined");
        saleIsActive = _saleIsActive;
        customIsActive = _customIsActive;
    }
    ...
    function mintSauce(
        ...
        for (uint16 i = 0; i < _ids.length; i++) {
            require(!customSauces[_ids[i]], "Invalid type");
    ...
    function mintCustomSauce(uint256[] memory _ids, uint16 _maxMint, uint256[] memory _quantities, bytes32[] memory _proof) public payable {
        require(_quantities.length > 0, "Invalid quantity");
        require(_ids.length == _quantities.length, "Invalid parameters");
        require(customIsActive, "Custom sale inactive");
        uint16 _hasMinted = hasMinted[msg.sender];
        uint256 _price;
        uint256 _quantity;
        for (uint16 i = 0; i < _ids.length; i++) {
            require(customSauces[_ids[i]], "Not custom.");
            _price += price[_ids[i]] * _quantities[i];
            _quantity += _quantities[i];
        }
        require(totalMinted + _quantity <= maxSupply, "Insufficient supply");
        ...
        uint16 _mintable = _maxMint - _hasMinted;
        require(_quantity <= _mintable, "Invalid quantity");
        totalMinted += _quantity;
        hasMinted[msg.sender] = _hasMinted + uint16(_quantity);
        _mintBatch(msg.sender, _ids, _quantities, "");
    }
    ...
    function order(
        ...
        for (uint16 i = 0; i < _items.length; i++) {
            _totals[_items[i].sauceType]++;
        }
        uint256 _quantity;
        for (uint16 i = 0; i < _ids.length; i++) {
            require(_quantities[i] > 0, "Quantity is zero");
            require(price[_ids[i]] > 0, "Invalid type");
            require(_quantities[i] == _totals[_ids[i]], "Invalid quantities");
