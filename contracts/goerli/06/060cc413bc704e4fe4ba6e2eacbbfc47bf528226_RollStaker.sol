...
interface IRollStaker {
     ...
     * @param balances mapping of epoch ids => balance in epoch.
     * @dev   **IMPORTANT** a zero balance DOES NOT mean the user had no balance, it simply means the user did not deposit
     ...
     * @param totalLocked Staking tokens locked by the user. Inactive users will have current balance of zero.
     * @dev   Not all locked tokens are active, tokens deposited this epoch activate next epoch.
     *        Use getCurrentBalanceForUser to get tokens activated *this* epoch.
     */
    struct UserStake {
...
contract RollStaker is IRollStaker, AccessControl, Pausable, ReentrancyGuard {
    ...
    /// @notice the token to be staked in the contract. Set during contract deploy.
    ...
    /**
     * @notice list of historical epoch balances by epoch Id + pending deposits next epoch
     * @dev    when next epoch begins, the balance of the current epoch will be added to pending deposits and rolled forward
     */
    ...
    function getProjectedNextEpochBalance() public view returns (uint256) {
        return epochBalances[currentEpochId] + epochBalances[currentEpochId + 1];
    }
    ...
    /**
     * @notice fetches the balance of staking tokens at a given epochId.
     *         if passed the next epoch id, will return a projection.
     */
    function getEpochBalanceWithProjection(uint8 _epochId) public view returns (uint256) {
        return _epochId == currentEpochId + 1 ? getProjectedNextEpochBalance() : epochBalances[_epochId];
    }

    /// @notice fetch the epoch balances array 'as-is', with no projections
    function getEpochBalances() public view returns (uint256[] memory) {
    ...
    /**
     * @notice fetches balances for the user up to nextEpoch.
     * @dev    If a balance is empty, check to see if the user was active in that epoch, then if so,
     *         look at the next available balance. See the `getHistoricalBalance` for an example.
     * @dev    Required because we use a mapping vs a contiguous array.
     *         This function can get quite expensive so best to leave it as a view
     */
    function getUserBalances(address _user) public view returns (uint256[] memory) {
        uint256[] memory balances = new uint256[](epochBalances.length);
        for (uint8 epoch; epoch < epochBalances.length;) {
            balances[epoch] = userStakes[_user].balances[epoch];
            unchecked {
                ++epoch;
            }
        }
    ...
    /// @notice assuming no deposits nor withdrawals, fetches what the user's balance will be starting from next epoch.
    function getProjectedBalanceForUser(address _user) public view returns (uint256 balance) {
        return userStakes[_user].totalLocked;
    }

    /// @notice get epoch balance for the current epoch, for a given user.
    function getCurrentBalanceForUser(address _user) public view returns (uint256 balance) {
        return getHistoricalBalanceForUser(_user, currentEpochId);
    }

    /**
     * @notice fetches the actual balance for a user at a given epoch.
     * @dev    if the user's balance was zero but they were active, walks back through epochs to find the last known value
     */
    function getHistoricalBalanceForUser(address _user, uint8 _epoch) public view returns (uint256 balance) {
        // if inactive, the user's balance is zero
        if (!userStakes[_user].activations.isActive(_epoch)) return 0;

        // assign to local variable to avoid changing state
        uint8 epoch = _epoch;
        // empty in this epoch does not mean the user's balance is zero
        // so we walk back through the balances to fetch the last known epoch balance
        while (balance == 0) {
            balance = userStakes[_user].balances[epoch];
            if (epoch == 0) break;
            unchecked {
                epoch--;
            }
    ...
    function getActivations(address _user) public view returns (Bitfields.Bitfield memory) {
        return userStakes[_user].activations;
    }

    /// @notice is the user currently active
    function userIsActive(address _user) public view returns (bool) {
        return userStakes[_user].activations.isActive(currentEpochId);
    }

    /// @notice was the user active for a particular epoch
    function userIsActiveForEpoch(address _user, uint8 _epoch) public view returns (bool) {
    ...
    function lastEpochUserWasActive(address _user) public view returns (uint8) {
    ...
    function deposit(uint256 _amount) public nonReentrant whenNotPaused {
        ...
        // cache variable but point to storage so we can modify
        UserStake storage u = userStakes[msg.sender];
        ...
        if (u.balances[nextEpochId] == 0) {
            // if there is no balance, we bring the last known balance forward
            u.balances[nextEpochId] = u.totalLocked + _amount;
        } else {
            // else we just increment the amount
            u.balances[nextEpochId] += _amount;
        }

        // update the user's total locked, tokens
        u.totalLocked += _amount;

        // balances for the next epoch will be the delta of new deposits until activated
        epochBalances[nextEpochId] += _amount;

        stakingToken.safeTransferFrom(msg.sender, address(this), _amount);
        emit Deposited(msg.sender, nextEpochId, _amount);
    }
    ...
    function depositWithSignature(uint256 _amount, uint256 _deadline, uint8 v, bytes32 r, bytes32 s) external {
        IERC20Permit(address(stakingToken)).permit(msg.sender, address(this), _amount, _deadline, v, r, s);
        deposit(_amount);
    }

    /// @notice removes all staked tokens from the contract, including pending deposits next epoch.
    function quit() external {
        // revert deposit will fail if nothing to withdraw
        if (userStakes[msg.sender].balances[currentEpochId + 1] > 0) revertDeposit();
        withdraw(getCurrentBalanceForUser(msg.sender));
        emit Quit(msg.sender, currentEpochId);
    }

    /**
     * @notice removes any tokens newly deposited in the next epoch, without affecting the current staking.
     */
    function revertDeposit() public nonReentrant whenNotPaused {
        uint8 nextEpochId = currentEpochId + 1;
        UserStake storage u = userStakes[msg.sender];
        uint256 nextEpochBalance = u.balances[nextEpochId];

        if (nextEpochBalance == 0) revert NothingToWithdraw(msg.sender);

        // if the user is currently inactive, then deactivate them next epoch
        if (!u.activations.isActive(currentEpochId)) u.activations.deactivateFrom(nextEpochId);

        // NOTE: can this OOG if user has not deposited in a long time - probably
        uint256 availableToWithdraw = nextEpochBalance - getCurrentBalanceForUser(msg.sender);

        u.balances[nextEpochId] = 0;
        u.totalLocked -= availableToWithdraw;

        // remove from the delta
        epochBalances[nextEpochId] -= availableToWithdraw;

        stakingToken.safeTransfer(msg.sender, availableToWithdraw);
        emit DepositReverted(msg.sender, nextEpochId, availableToWithdraw);
    }

    /**
     * @notice withdraw tokens from the contract. If withdrawing all, the user will be set as inactive
    ...
    function withdraw(uint256 _amount) public nonReentrant whenNotPaused {
        ...
        if (u.totalLocked == 0) revert NothingToWithdraw(msg.sender);
        if (_amount > u.totalLocked) revert InvalidWithdrawalAmount(msg.sender, _amount);

        // adds a lot of complexity if we try and handle both next and current epoch
        require(u.balances[currentEpochId + 1] == 0, "Unstake next epoch first");

        if (!u.activations.isActive(currentEpochId)) revert Inactive(msg.sender, currentEpochId);

        // if current epoch balance is zero, this just means a deposit will not have been made in the previous
        if (_amount == u.totalLocked) {
            // users forfeit rewards in current epoch
            u.activations.deactivateFrom(currentEpochId);
            emit Exited(msg.sender, currentEpochId);
        }

        // zero balance is possible if the user didn't deposit but was still active
        // how to handle deposits into next epoch but then withdrawing
        if (u.balances[currentEpochId] > 0) u.balances[currentEpochId] -= _amount;

        // not possible that totalLocked is zero
        unchecked {
            u.totalLocked -= _amount;
        }

        // withdrawals takes from the current delta
        epochBalances[currentEpochId] -= _amount;

        stakingToken.safeTransfer(msg.sender, _amount);
        emit Withdrawn(msg.sender, currentEpochId, _amount);
    }
     ...
     * @dev    until this function is called, the balance of the next epoch only represents pending deposits.
     */
    function activateNextEpoch() external onlyRole(OPERATOR_ROLE) {
        epochBalances[currentEpochId + 1] += epochBalances[currentEpochId];
        currentEpochId++;
        // now that the epoch id has been incremented, zero initialize the deposits for the *new* next epoch
        epochBalances.push(0);
