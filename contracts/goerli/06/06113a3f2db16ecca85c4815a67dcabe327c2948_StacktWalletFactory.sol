...
import "./guards/StacktOperatorGuard.sol";
import "./StacktWallet.sol";
import "./proxies/StacktProxyFactory.sol";
import "./proxies/StacktProxy.sol";

contract StacktWalletFactory {

    event LogCreateStacktWallet(address indexed creator, address proxyAddress, address singleton);

    address public immutable proxyFactoryAddress;
    address public immutable stacktWalletSingletonAddress;
    address public immutable stacktOperatorGuardSingletonAddress;
    address public immutable fallbackAddress;

    constructor(address _proxyFactoryAddress, address _stacktWalletSingletonAddress, address _stacktOperatorGuardSingletonAddress, address _fallbackAddress) {
        proxyFactoryAddress = _proxyFactoryAddress;
        stacktWalletSingletonAddress = _stacktWalletSingletonAddress;
        stacktOperatorGuardSingletonAddress = _stacktOperatorGuardSingletonAddress;
    ...
    struct Setup { 
        ...
        uint256 fundPrice; 
        uint256 funderLimit; 
        address[] additionalOperators;
    }

    function createWallet(Setup calldata setup) public payable returns (StacktProxy stacktWalletProxy) {

        // proxy factor
        StacktProxyFactory proxyFactory = StacktProxyFactory(proxyFactoryAddress);

        // operator guard
        bytes memory operatorGuardSetupData = abi.encodeCall(StacktOperatorGuard.setup, (
            msg.sender,
            setup.additionalOperators
        ));

        StacktProxy operatorGuardProxy = proxyFactory.createProxy(stacktOperatorGuardSingletonAddress, operatorGuardSetupData);
        address operatorGuardAddr = address(operatorGuardProxy);
        
        // setup data
        bytes memory stacktWalletSetupData = abi.encodeCall(StacktWallet.setup, (
            StacktWallet.StacktWalletSetup({
                name: setup.name, 
                description: setup.description, 
                image: setup.image, 
                msgSender: msg.sender, 
                startFundingTime: setup.startFundingTime, 
                fundingDuration: setup.fundingDuration, 
                fundPrice: setup.fundPrice, 
                funderLimit: setup.funderLimit
            }), 
            fallbackAddress, 
            operatorGuardAddr
        ));
        ...
        // stackt wallet
        stacktWalletProxy = proxyFactory.createProxy{value: msg.value}(stacktWalletSingletonAddress, stacktWalletSetupData);

        //
        StacktOperatorGuard(operatorGuardAddr).setStacktWalletAddress(address(stacktWalletProxy));
        
        emit LogCreateStacktWallet(msg.sender, address(stacktWalletProxy), stacktWalletSingletonAddress);

        return stacktWalletProxy;
    }
}
...
contract StacktWallet is Singleton, StacktOwnerManager, FallbackManager, StacktGuardManager {
    ...
    function execTransaction(
        ...
        /* -------------------------------- execution ------------------------------- */
        {
            ...
            _execute(to, value, data, operation, gas == 0 ? (gasleft() - 2500) : gas);
        ...
        /* ---------------------------------- emit ---------------------------------- */
        emit ExecutedTransaction(to, value, data, operation, gas, refundGas, signatures, _nonce, tx.origin, threshold, ownerCount);
    }
    ...
    /// from: https://github.com/superfluid-finance/protocol-monorepo/blob/dev/packages/ethereum-contracts/contracts/libs/CallUtils.sol
    function _revertFromReturnedData(bytes memory returnedData) internal pure {
    ...
    function _execute(
        ...
        // revert
        if (!success) {
            _revertFromReturnedData(result);
        }
    }
}
...
contract StacktWalletWithFee is StacktWallet, StacktFeeCollector {
    function execTransaction(
        ...
        bool refundGas,
        bytes memory signatures
    ) public override payable virtual {
        collectFee();
        super.execTransaction(to, value, data, operation, gas, refundGas, signatures);        
    }
}
