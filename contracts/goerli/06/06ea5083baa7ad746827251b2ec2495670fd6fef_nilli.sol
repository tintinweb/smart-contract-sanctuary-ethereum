...
// File: nilliv1/RefundEscrowNilli.sol
...
contract RefundEscrowNilli is ConditionalEscrow {
    ...
    string public _tagline;
    string public _condition;
     ...
     */
    constructor(address payable beneficiary_, string memory tagline_, string memory condition_) {
    ...
    /**
     * @dev adjust the benficiary address of the contract.
     */
    function adjustBeneficiary(address payable newBeneficiary) public onlyOwner {
        require(state() == State.Active, "RefundEscrow: must be in Active state to update beneficiary");
    ...
    function depositToken(IERC20 token, address depositor, uint256 amount) public onlyOwner {
        require(state() == State.Active, "RefundEscrow: can only deposit token while active");
    ...
    function withdrawToken(IERC20 token, address withdrawer) public onlyOwner {
        require(state() == State.Refunding, "RefundEscrow: can only refund token when the state is refunding");
     ...
     */
    function beneficiaryWithdrawToken(IERC20 token) public onlyOwner {
        require(state() == State.Closed, "RefundEscrow: beneficiary can only withdraw token what closed");
     ...
     */
    function nftDepositEndow(IERC721 token, address from, uint256 tokenId) public onlyOwner {
        require(state() == State.Active, "RefundEscrow: can only deposit NFT while active");
     ...
     * @param tokenId The NFT tokenID specific to this NFT collection.
     */
    function nftWithdrawBeneficiaryEndow(IERC721 token, uint256 tokenId) public onlyOwner {
        require(state() == State.Closed, "RefundEscrow: beneficiary can only withdraw NFT when closed");
     ...
     * @param tokenId The NFT tokenID specific to this NFT collection.
     */
    function nftRefundEndow(IERC721 token, address depositor, uint256 tokenId) public onlyOwner {
        require(state() == State.Refunding, "RefundEscrow: depositors can only withdraw NFT refunds when the state is refunding");
...
contract nilli is Ownable {
    ...
    // Event that will be emitted whenever an endowment is closed
    event EndowmentClosed(
        address endowAddress
    );

    // Event that will be emitted whenever an endowment has refunds enabled
    event EndowmentRefundsEnabled(
        address endowAddress
    );

    // Event that will be emitted whenever a benficary address is adjusted
    event BeneficaryAdjust(
        address endowAddress,
     ...
     * @param beneficiary The address that will act as recipient/beneficiary
     * if the endowment is successful (for the athlete)
     */
    function startEndowment(
        ...
        RefundEscrowNilli newEndowment = new RefundEscrowNilli(beneficiary, tagline, condition);
        // newEndowment.transferOwnership(address(this));
     ...
     * @param endowAddress The specific endowment contract address that will receive the deposit.
     */
    function depositToEndowment(RefundEscrowNilli endowAddress) public payable returns (uint amt) {
        endowAddress.deposit{value: msg.value}(msg.sender);
        emit EndowDeposit(
            address(endowAddress),
            msg.sender,
            address(0),
     ...
     * @param endowAddress The specific endowment contract to be closed.
     */
    function closeEndowment(RefundEscrowNilli endowAddress) public onlyOwner {
        endowAddress.close();
        emit EndowmentClosed(
            address(endowAddress)
        );
    }

    /**
     * @dev enableRefunds will allow depositors to extract their ETH contributed to the endowment.
     * @param endowAddress The specific endowment contract to interact with.
     */
    function enableRefunds(RefundEscrowNilli endowAddress) public onlyOwner {
        endowAddress.enableRefunds();
        emit EndowmentRefundsEnabled(
            address(endowAddress)
        );
    }
     ...
     * @param endowAddress The specific endowment contract to interact with.
     */
    function depTokentoEndow(RefundEscrowNilli endowAddress, IERC20 token, uint amount) public returns (uint amt) {
        endowAddress.depositToken(token, msg.sender, amount);
        emit EndowDeposit(
            address(endowAddress),
            msg.sender,
            address(token),
     ...
     * @param endowAddress The specific endowment contract to interact with.
     */
    function withdrawTokenEndow(RefundEscrowNilli endowAddress, IERC20 token) public {
        endowAddress.withdrawToken(token, msg.sender);
    }

    /**
     * @dev if endowment Closed, the beneficiary can withdraw ERC-20 balances from endowment.
     * @param endowAddress The specific endowment contract to interact with.
     */
    function withdrawTokenBeneficiary(RefundEscrowNilli endowAddress, IERC20 token) public {
        endowAddress.beneficiaryWithdrawToken(token);
    }
     ...
     * @param endowAddress The specific endowment contract to interact with.
     */
    function depNFTtoEndow(RefundEscrowNilli endowAddress, IERC721 token, uint tokenId) public {
        endowAddress.nftDepositEndow(token, msg.sender, tokenId);
        emit EndowDeposit(
            address(endowAddress),
            msg.sender,
            address(token),
     ...
     * @param endowAddress The specific endowment contract to interact with.
     */
    function withdrawNFTEndow(RefundEscrowNilli endowAddress, IERC721 token, uint tokenId) public {
        endowAddress.nftRefundEndow(token, msg.sender, tokenId);
    }
     ...
     * @param endowAddress The specific endowment contract to interact with.
     */
    function withdrawNFTBeneficiary(RefundEscrowNilli endowAddress, IERC721 token, uint tokenId) public {
        endowAddress.nftWithdrawBeneficiaryEndow(token, tokenId);
    }

    // adjust beneficary address of the crowd contract
    function adjustBeneficiary(RefundEscrowNilli endowAddress, address payable newBeneficiary) public onlyOwner {
        endowAddress.adjustBeneficiary(newBeneficiary);
        emit BeneficaryAdjust(address(endowAddress), newBeneficiary);
    }

    // typical renounce ownership - will likely totally shutdown contract as only owners can interact.
    function renounceEndowOwnership(RefundEscrowNilli endowAddress) public onlyOwner {
        endowAddress.renounceOwnership();
    }

    // typical transfer of ownership
    function xferEndowOwnership(RefundEscrowNilli endowAddress, address newOwner) public onlyOwner {
        endowAddress.transferOwnership(newOwner);
    }
