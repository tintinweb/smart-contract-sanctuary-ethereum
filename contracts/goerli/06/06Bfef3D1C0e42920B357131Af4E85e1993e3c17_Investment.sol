...
library InvestmentLibrary {
    ...
    address constant WETHAddress = 0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6;
            ...
            for (uint i = 0; i < _tokensHeld.length; i++){
                if(_tokensHeld[i].tokenAddress == WETHAddress){
                    ...
                    if(_tokensHeld[i].isV3 == false){
                        (uint reserveA, uint reserveB) = UniswapV2Library.getReserves(uniswapV2Factory, _tokensHeld[i].tokenAddress, WETHAddress);
                        ...
                        contractValue += OracleLibrary.getQuoteAtTick(arithmeticMeanTick, uint128(IERC20(_tokensHeld[i].tokenAddress).balanceOf(address(this))), _tokensHeld[i].tokenAddress, WETHAddress);
                    }
                }
            }
        }
    ...
    function removeFromPool(uint256 amountToRemove, InvestmentLibrary.exitSwapParams[] memory exitSwapParams) public {
        ...
        for (uint i = 0; i < tokensHeldSnapshot.length; i++){
            ...
            if(tokensHeldSnapshot[i].tokenAddress == WETHAddress){
                wethPayout += (tokenContract.balanceOf(address(this)) * userShare / 10**18) - wethPayout;
            } else {
    ...
    function processFee(address tokenIn, uint256 amount, uint24 uniPoolfee) internal {
        ...
        if(tokenIn != WETHAddress){
            if(uniPoolfee == 0){
                ...
                address[] memory path;
                feeAmount = _swapTokens(path, abi.encode(tokenIn, uniPoolfee, WETHAddress), amount, 0);
            }
            
        }

        IERC20(WETHAddress).approve(_feeHandlerAddress, feeAmount);
    ...
    function _adjustTokensHeld(address[] memory v2Path, bytes memory v3Path, uint256 amountIn) internal {
        ...
        if(IERC20(tokenOut).balanceOf(address(this)) == 0 && InvestmentLibrary.isTokenHeld(_tokensHeld, tokenOut) == false){
            if(tokenOut == WETHAddress){
                ...
                if(isV3 == false){
                poolAddress = UniswapV2Library.pairFor(uniswapV2Factory, WETHAddress, tokenOut);
                } else {
                    poolAddress = IUniswapV3Factory(uniswapV3Factory).getPool(WETHAddress, tokenOut, fee);
                }
            }   
