...
contract Vincents is Ownable, ERC721A, ReentrancyGuard, Pausable{
    ...
    uint64 public publicSalePrice = 1 ether;
    uint32 public whitelistSaleStartTime = 0;
    uint64 public whitelistPrice = 1 ether;
    
    
    mapping(address => uint256) public whitelist;
    mapping(address => uint256) public blocklist;


    constructor() ERC721A("Vincents", "VINCENTS", 10000, 10) {
    ...
    function setPublicSalePrice (uint64 newPrice) 
    external onlyOwner {
        
        publicSalePrice = newPrice;
    }

    function setWhitelistSalePrice (uint64 newPrice) 
    public onlyOwner {
        whitelistPrice = newPrice;
    }

    function setPublicSaleStartTime(uint32 startTime) 
    external onlyOwner {
        publicSaleStartTime = startTime;
    }

    function setWhitelistSaleStartTime(uint32 startTime) 
    external onlyOwner {
        whitelistSaleStartTime = startTime;
    }

    function publicMint(uint256 qty) 
    external payable isUserWallet {
        require(totalSupply() + qty <= collectionSize, 
        "Max Supply Reached");
        require(numberMinted(msg.sender) + qty <= maxBatchSize, 
        "More than max no. of NFT allowed");
        require(msg.value >= publicSalePrice * qty, 
        "Not enough Eth");
        require(isPublicSaleOn(), 
        "Public sale has not started yet" );
        _safeMint(msg.sender, qty);
    }

    function whitelistMint(uint256 qty) 
    external payable isUserWallet {
         require(totalSupply() + qty <= collectionSize, 
         "Max Supply Reached");
        require(numberMinted(msg.sender) + qty <= maxBatchSize, 
    ...
    function isWhitelistSaleOn() public view returns (bool) {
        return whitelistSaleStartTime > 0 && block.timestamp >= whitelistSaleStartTime;
    }

    function isWalletOnWhitelist(address owner) 
    public view returns (bool) {
        return whitelist[owner] != 0;
    }
    ...
    external onlyOwner {
        require(addresses.length == numSlots.length, "Addresses length does not match with slots length");
  ...
  function crossmint(address _to) public payable {
    require(msg.value >= publicSalePrice, "Incorrect ETH value sent");
