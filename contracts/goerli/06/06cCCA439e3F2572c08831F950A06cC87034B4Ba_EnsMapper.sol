    ...
    Ownable
{
    ...
    string public domainLabel = "testg";
    ...
    event RegisterSubdomain(address registrar, uint256 token_id, string label);

    event SubdomainReset(address registrar, uint256 token_id);
        ...
        returns (address payable)
    {
        ...
        return payable(tokenIdToClaimer[token_id]);
    }
        ...
        returns (bytes memory)
    {
        ...
        if (coinType == 60) {
            // ETH
            return abi.encodePacked(tokenIdToClaimer[token_id]);
        } else {
    ...
    function setDomain(
        ...
        bytes32 old_hash = tokenHashmap[token_id];

        if (tokenHashmap[token_id] != 0x0 && old_hash != big_hash) {
            _reset(token_id);
            emit AddrChanged(old_hash, address(0));
            emit AddressChanged(old_hash, 60, abi.encodePacked(address(0)));
        }

        require(
            !ens.recordExists(big_hash) ||
            (ens.recordExists(big_hash) && hashToIdMap[big_hash] == 0) ||
            (ens.recordExists(big_hash) && hashToIdMap[big_hash] == token_id) ||
                msg.sender == owner(),
