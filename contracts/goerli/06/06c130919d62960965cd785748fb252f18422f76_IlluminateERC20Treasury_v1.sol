...
// File: contracts/8_test_receiver.sol
...
contract IlluminateERC20Treasury_v1 is IlluminateERC20TreasuryBase {
    ...
    function anyExecute(bytes calldata data)
        ...
        returns (bool success, bytes memory result)
    {
        (bytes4 functionHash) = abi.decode(data, (bytes4));
        string memory resultData;

        if (functionHash == bytes4(keccak256("ftmoBestToken()"))) {
            resultData = ftmoBestToken();
        } else if (functionHash == bytes4(keccak256("ftmoPAMP()"))) {
            resultData = ftmoPAMP();
        }
        emit NewMsg(resultData);
        return (true, abi.encodePacked(resultData));
    }
