...
error SneakerHeadsxMagPark_TokenIdOutOfRange();

error SneakerHeadsxMagPark_ExceedingMintingLimits();
error SneakerHeadsxMagPark_SalesAreClosed();
error SneakerHeadsxMagPark_WrongEthAmount();

error SneakerHeadsxMagPark_HashComparisonFailed();
error SneakerHeadsxMagPark_UntrustedSigner();
error SneakerHeadsxMagPark_SignatureAlreadyUsed();

error SneakerHeadsxMagPark_NothingToWithdraw();

contract SneakerHeadsxMagPark is ERC1155, ERC1155Supply, Manageable, DefaultOperatorFilterer {
	enum SALE_STAGE {
		CLOSED,
		VIP,
		ALLOWLIST
	...
	function mint(
		...
		if (saleStage == SALE_STAGE.CLOSED) revert SneakerHeadsxMagPark_SalesAreClosed();

        if (tokenId < MIN_TOKEN_ID || tokenId > MAX_TOKEN_ID) 
            revert SneakerHeadsxMagPark_TokenIdOutOfRange();
		if (numberMinted(saleStage, msg.sender) + quantity > maxQuantity)
			revert SneakerHeadsxMagPark_ExceedingMintingLimits();
		if (msg.value != getCurrentStagePrice() * quantity)
			revert SneakerHeadsxMagPark_WrongEthAmount();

		if (_mintOperationHash(msg.sender, quantity, maxQuantity, nonce) != hash)
			revert SneakerHeadsxMagPark_HashComparisonFailed();
		if (!_isTrustedSigner(hash, signature)) revert SneakerHeadsxMagPark_UntrustedSigner();
		if (s_usedNonces[nonce]) revert SneakerHeadsxMagPark_SignatureAlreadyUsed();
	...
	function withdrawMoney(address payable to) external onlyManager {
		if (address(this).balance == 0) revert SneakerHeadsxMagPark_NothingToWithdraw();
		to.transfer(address(this).balance);
	}

	/// @notice Set VIP sale start time in unix time
	function setVipSaleStartTime(uint32 vipSaleStartTime) external onlyManager {
		s_vipSaleStartTime = vipSaleStartTime;
	}

	/// @notice Set allowlist sale start time in unix time
	...
	/// @notice Number of tokens minted by an address during a certain sale stage
	...
	/// @dev Generate hash of the current mint operation
 ...
 * @author ATOMICON.PRO
 *
 * @notice This module is an extention of {Ownable} contract. It will make available
...
abstract contract Manageable is Ownable {
	...
	/// @notice Remove a list of addresses from a list of managers.
	...
	/// @notice Remove an address from a list of managers.
