...
contract PunkChecks is DefaultOperatorFilterer, ERC721, Owned {
    ...
    // @dev Goerli testnet contracts
    address constant PUNKS_V1_ADDRESS = address(0x7a68F8f78F08E85e1835F42a93756a34775DC275);
    address constant PUNKS_V2_ADDRESS = address(0xb9dE406C7A0D4A061FefDD5daD06F56D6B2fB155);
    address constant PUNKS_V1_WRAPPER_ADDRESS = address(0x282BDD42f4eb70e7A9D9F40c8fEA0825B7f68C5D);
    address constant PUNKS_V2_WRAPPER_ADDRESS = address(0xb7F7F6C52F2e2fdb1963Eab30438024864c313F6);
    ...
    function mint(uint256 punkId, address vaultV1, address vaultV2, uint256 v) external payable {
        if (mintType == 5) revert MintNotOpen();
        if (msg.value != price) revert WrongEtherAmount();
        if (v < 1 || v > 4) revert InvalidVersion();
        if (
            mintType == 5 || (mintType == 1 && v != 1 && v != 3) || (mintType == 2 && v != 2 && v != 3)
                || (mintType == 3 && v != 3) || (mintType == 4 && v == 4)
        ) {
            revert MintNotOpenForVersion();
        }
    ...
    /// OpenSea's Operator Filter overrides
    ...
    function contractURI() public view returns (string memory) {
        return IPunkChecksRenderer(renderingContractAddress).contractURI();
    }
}
