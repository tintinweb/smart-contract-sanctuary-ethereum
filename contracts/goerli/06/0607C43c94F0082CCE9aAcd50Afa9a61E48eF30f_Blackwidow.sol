...
contract Blackwidow is ERC721Psi, VRFConsumerBaseV2, Ownable {
    ...
    ) ERC721Psi("Blackwidow", "BLW") VRFConsumerBaseV2(vrfCoordinator) {
    ...
    function publicMint(uint256 quantity) external payable callerIsUser {
        ...
        if (numberMinted(msg.sender) + quantity > maxMint) {
            revert MintMoreThanAllowed();
        }
        ...
        returns (string memory)
    {
        ...
        return
            randomseedRequested
                ? string(
                    abi.encodePacked(_baseURI(), getMetadata(tokenId), ".json")
                )
                : _baseURI();
    }
