...
//Records functions
    ...
    function getReturnedPorcentages() public view returns (returnedPorcentages[] memory){
        ...
        for (uint i = 0 ; returnedPorcentagesArray.length > i; i++){
            ...
            returnedPorcentages memory porcentageToReturn = returnedPorcentages({circuitId:returnedPorcentagesArray[i].circuitId, date:returnedPorcentagesArray[i].date, upTimePorcentage:returnedPorcentagesArray[i].upTimePorcentage, porcentage:returnedPorcentagesArray[i].porcentage});
    ...
    function calculateDownTime(string memory _circuitId, uint _startDate, uint _endDate) public view returns (uint upTimePorcentage, uint percentage){ //testeado
        Record[] memory recordsArray = this.getRecords(_circuitId, _startDate, _endDate);
        uint downtime = 0;
        if(recordsArray.length == 0){    //testeado
            if (recordMap[_circuitId].length > 0 && recordMap[_circuitId][recordMap[_circuitId].length - 1].isDowntime){
                return (0, 100000);
            }
            else{
                return (100000, 0);
        ...
        for(uint i = 0; SLAMap[actualSLA.SLAName].length > i; i++){    //testeado
            if(SLAMap[actualSLA.SLAName][i].max >= upTimePorcentage && SLAMap[actualSLA.SLAName][i].min < upTimePorcentage){
                return (upTimePorcentage, SLAMap[actualSLA.SLAName][i].percentage);
            }
        }
        return (upTimePorcentage, SLAMap[actualSLA.SLAName][SLAMap[actualSLA.SLAName].length-1].percentage); //if not found returns the last percentage
    }
    ...
    function DoCalculations() public{    //testeado
        ...
        for (uint i = 0; circuitsArray.length > i; i++) {//testeado    //for by every circuit
            ...
            for (uint j = 0; lastEmailSendMap[circuitId].length > j; j++){//testeado       //Resend emails for months with changes
                if (lastEmailSendMap[circuitId][j].hasChanged){
                    if (lastEmailSendMap[circuitId].length == j -1){ //if last position calculate endtime of calculations
                        (returnerUpTimePorcentage, returnedporcentage) = this.calculateDownTime(circuitId, lastEmailSendMap[circuitId][j].date, lastEmailSendMap[circuitId][j].date + seconds_to_add);
                    }
                    else{//testeado
                        (returnerUpTimePorcentage, returnedporcentage) = this.calculateDownTime(circuitId, lastEmailSendMap[circuitId][j].date, lastEmailSendMap[circuitId][j + 1].date -1);
                    }

                    returnedPorcentagesArray.push(returnedPorcentages({circuitId:circuitId, date:lastEmailSendMap[circuitId][j].date, upTimePorcentage:returnerUpTimePorcentage, porcentage:returnedporcentage}));
            ...
            // if(last_month > IDatetime(datetimeAddress).getMonth(block.timestamp)){
                if(lastEmailSendMap[circuitId].length < 4){  //testeado
                    if (newCircuit == true){ //testeado
                        lastSendedEmail memory newLastSendedEmail = lastSendedEmail({date:IDatetime(datetimeAddress).toTimestamp(last_year, last_month, 1) + seconds_to_add, hasChanged:false});
            ...
            (returnerUpTimePorcentage, returnedporcentage) = this.calculateDownTime(circuitId, lastEmailSendMap[circuitId][last_position].date, lastEmailSendMap[circuitId][last_position].date + seconds_to_add);
            returnedPorcentagesArray.push(returnedPorcentages({circuitId:circuitId, date:lastEmailSendMap[circuitId][last_position].date, upTimePorcentage:returnerUpTimePorcentage, porcentage:returnedporcentage}));
