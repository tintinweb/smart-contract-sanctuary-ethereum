...
contract STFU is Context, IERC20, Ownable {
    ...
    address public devAddress = 0x855f73bbdf5FeC3ce34e10B98b845216A001eA52;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isBot[from] && !_isBot[to], "Dude, stop using a bot");
    ...
    function updatMaxTx(uint256 amount) external onlyOwner {
    ...
    function updateCoolDown(bool _enabled, uint256 _timeInSeconds) external onlyOwner {
    ...
    // Rescue BNB that is sent to the contract by mistake
    function rescueBNB(uint256 weiAmount) external onlyOwner {
        require(address(this).balance >= weiAmount, "insufficient BNB balance");
        payable(msg.sender).transfer(weiAmount);
    }
    // Rescue BEP20 tokens that are sent to the contract by mistake
