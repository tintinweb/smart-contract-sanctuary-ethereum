    ...
    ReentrancyGuard
{
    ...
    // How many tokens an investor gets per one unit of cash
    uint256 public preIcoRate;
    uint256 public icoRate;
    uint256 public currentRate;

    // Minimum and maximum amount of cash an investor is allowed to spend
     ...
     * @param cashAmount cash paid for purchase
        ...
        validAddress(_wallet)
    {
        ...
        require(_icoRate > 0 && _preIcoRate > _icoRate,
            "ExenoTokenIco: preICO rate needs to be higher then ICO rate");
        
        require(_minCap > 0 && _minCap < _maxCap,
            "ExenoTokenIco: min cap needs to be lower then max cap");
     ...
     */
    function _setStage(Stage stage)
        internal
    {
        if (stage == Stage.PreICO) {
            currentRate = preIcoRate;
        } else if (stage == Stage.ICO) {
            currentRate = icoRate;
        }
        currentStage = stage;
    }
    ...
    function buyTokens(address beneficiary)
        public payable validAddress(beneficiary) whenNotPaused nonReentrant
    {
        ...
        uint256 tokenAmount = _getTokenAmount(cashAmount);
        ...
        uint256 existingPayment = tokenPayments[beneficiary];
        uint256 newPayment = existingPayment.add(cashAmount);

        require(newPayment >= minCap,
            "ExenoTokenIco: investment is below min cap");

        require(newPayment <= maxCap,
            "ExenoTokenIco: investment is above max cap");

        cashRaised = cashRaised.add(cashAmount);
        assert(address(this).balance >= cashRaised);

        totalTokensPurchased = totalTokensPurchased.add(tokenAmount);
        uint256 existingPurchase = tokenPurchases[beneficiary];
        uint256 newPurchase = existingPurchase.add(tokenAmount);

        emit TokenPurchase(msg.sender, beneficiary, cashAmount, tokenAmount);

        tokenPayments[beneficiary] = newPayment;
    ...
    function getUserContribution(address beneficiary)
        public view validAddress(beneficiary) returns(uint256)
    {
    ...
    /**
     * @notice Allows onwer to update the stage
     */
    function nextStage()
        public onlyOwner
    {
    ...
    /**
     * @notice Allows owner to update the rate
     * @param rate New rate
     */
    ...
    {
        require(currentStage == Stage.PreICO || currentStage == Stage.ICO,
            "ExenoTokenIco: updating rate is only allowed in preICO and ICO");
        
        require(rate < currentRate,
            "ExenoTokenIco: only decreasing rate is allowed");
        
        if (currentStage == Stage.PreICO) {
            require(rate >= icoRate,
                "ExenoTokenIco: during preICO the rate cannot be less than the ICO rate");
        }
    ...
    /**
     * @notice Allows owner to forward cash to the company wallet
     */
