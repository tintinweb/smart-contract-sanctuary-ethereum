...
contract Deployer {
    ...
    ERC1155Minter minter;

    constructor(ERC1155Minter _minter) {
        minter = _minter;
    }
    ...
    function deploy1155(
        ...
        ERC1155MinterProxy proxy = new ERC1155MinterProxy(
            address(minter),
            msg.sender,
            abi.encodeWithSignature(
                "initialize(string,uint256,uint256,uint256,bytes32,bool,address[],uint256[])",
                _tokenUri,
                _maxSupply,
                _price,
                _startTime,
        ...
        ERC1155Minter(payable(address(proxy))).transferOwnership(owner);

        emit DeployContract(msg.sender, address(proxy), address(minter));
    ...
    PaymentSplitterUpgradeable
{
    ...
    /**
     * @dev Event for setting a new uri
     */
        ...
        payable
    {
        ...
        _mint(msg.sender, DEFAULT_TOKEN_ID, quantity, "");
    }
    ...
    function airdrop(address[] memory addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; ) {
            _mint(addresses[i], DEFAULT_TOKEN_ID, DEFAULT_QUANTITY, "");
...
contract ERC1155MinterProxy is TransparentUpgradeableProxy {
    constructor(
        address _logic,
        address _admin,
        bytes memory _data
