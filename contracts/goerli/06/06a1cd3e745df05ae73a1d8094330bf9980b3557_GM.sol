...
contract GM is IERC20, Ownable {
    ...
    uint8 private _devTaxPercent = 1;
    ...
    function _transferSupportingFee(address sender, address recipient, uint256 amount) private {
        /* transfers tokens while taking fees.
            ...
            returns:
                none
        */
        uint256 marketingTK = amount.mul(_currentTaxPercent).div(100);
        uint256 devTK = amount.mul(_devTaxPercent).div(100);

        uint256 amountRecieved = amount.sub(marketingTK).sub(devTK);
        ...
        // handle taxes
        _balances[address(this)] = _balances[address(this)].add(marketingTK);
        _balances[_devWallet] = _balances[_devWallet].add(devTK);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        */
        ...
        // checks if sender/reciever is exluded from these checks.
