...
contract SceneContractV2 is SceneStorage {
    // Initialization function
    function initialize() public initializer {
        __ERC721_init("Astroscenes", "ASTROSCENES");
        __Ownable_init();
    ...
    /**
     * @dev staking officially started by owner.
     *
     * Requirements:
     * - msg.sender must be owner of the contract.
     *
     */
    function stakeOnOff(bool _status) external virtual onlyOwner {
        require(stakeStatus != _status, "SceneContract: Already set the status");
        stakeStatus = _status;
        if(stakeStatus){
            for(uint256 i = 1; i <= tokenCounter; i++){
                address _owner = ownerOf(i);
                ownedTokens[_owner].push(i);
                StakingDetails[_owner][i].tokenId = i;
                (StakingDetails[_owner][i].startTime).push(block.timestamp);
            }
        }else if(!stakeStatus){
            for(uint256 i = 1; i <= tokenCounter; i++){
                address _owner = ownerOf(i);
                (StakingDetails[_owner][i].endTime).push(block.timestamp);
            }
        }
     ...
     *
     * @param _user token id.
     *
     * Returns
     * - token URI.
     *
     */
    function viewStakePointsPerToken(
        address _user,
        uint256 _token_id
    ) public view virtual returns (uint256 Points) {
        for(uint256 i = 0; i < (StakingDetails[_user][_token_id].startTime).length; i++){
            if(i < (StakingDetails[_user][_token_id].endTime).length){
                Points += pointsPerDay * uint256((StakingDetails[_user][_token_id].endTime[i] - StakingDetails[_user][_token_id].startTime[i]) / 2);
            }else if(i == (StakingDetails[_user][_token_id].endTime).length){
                Points += pointsPerDay * uint256((block.timestamp - StakingDetails[_user][_token_id].startTime[i]) / 2);
            }
        }
    }
     ...
     *
     */
    function viewStakePoints(
        address _user
    ) public view virtual returns (uint256 Points) {
        for(uint256 i = 0; i < ownedTokens[_user].length; i++){
            if(ownedTokens[_user][i] != 0){
                Points += viewStakePointsPerToken(_user, ownedTokens[_user][i]);
            }
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        batchSize;
        if(stakeStatus){
            (StakingDetails[from][firstTokenId].endTime).push(block.timestamp);
            (StakingDetails[to][firstTokenId].startTime).push(block.timestamp);
        }
        for(uint256 i = 0; i < ownedTokens[from].length; i++){
            if(ownedTokens[from][i] == firstTokenId){
                delete ownedTokens[from][i];
                break;
            }
        }
...
contract SceneStorage is ERC721RoyaltyUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => mapping(uint256 => StakingData)) public StakingDetails;
