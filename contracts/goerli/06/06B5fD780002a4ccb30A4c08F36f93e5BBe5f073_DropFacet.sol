...
contract DropFacet is InternalOwnableRoles, InternalERC721AUpgradeable {
    ...
    modifier onlyMintable(uint64 quantity) {
        ...
        require(quantity <= ds._maxPerMint, "Exceeded max per mint");
        if (ds._maxAmount > 0 && _totalSupply().add(quantity) > ds._maxAmount) {
    ...
    {
        DropStorage.Layout storage ds = DropStorage.layout();
        require(!ds._presaleActive, "Presale active");
        require(ds._saleActive, "Sale not active");
        require(
            _getAux(recipient) + quantity <= ds._maxPerWallet,
    ...
    function presaleMintTo(
        ...
        require(ds._presaleActive, "Presale not active");
        require(ds._merkleRoot != "", "Presale not set");
        require(mintQuantity <= ds._maxPerWallet, "Exceeded max per wallet");
    ...
    function startSale(
        ...
        DropStorage.Layout storage ds = DropStorage.layout();
        ds._saleActive = true;
        ds._presaleActive = presale;

        ds._maxAmount = newMaxAmount;
        ds._maxPerMint = newMaxPerMint;
        ds._maxPerWallet = newMaxPerWallet;
        ds._price = newPrice;
    }

    function stopSale() external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        DropStorage.Layout storage ds = DropStorage.layout();
        ds._saleActive = false;
        ds._presaleActive = false;
    }

    function maxAmount() external view returns (uint256) {
        DropStorage.Layout storage ds = DropStorage.layout();
        return ds._maxAmount;
    }

    function maxPerMint() external view returns (uint256) {
        DropStorage.Layout storage ds = DropStorage.layout();
        return ds._maxPerMint;
    }

    function maxPerWallet() external view returns (uint256) {
        DropStorage.Layout storage ds = DropStorage.layout();
        return ds._maxPerWallet;
    }
    ...
    function presaleActive() external view returns (bool) {
        DropStorage.Layout storage ds = DropStorage.layout();
        return ds._presaleActive;
    }

    function saleActive() external view returns (bool) {
        DropStorage.Layout storage ds = DropStorage.layout();
        return ds._saleActive;
    }

    function _purchaseMint(uint64 quantity, address to) internal {
        DropStorage.Layout storage ds = DropStorage.layout();
        require(ds._price.mul(quantity) <= msg.value, "Value incorrect");

        unchecked {
            ds._totalRevenue = ds._totalRevenue.add(msg.value);
        }

        _setAux(to, _getAux(to) + quantity);
        BaseStorage.layout()._niftyKit.addFees(msg.value);
        _mint(to, quantity);
    }
}
