...
contract Invoice is Initializable, IInvoice {
  ...
  // MODIFIERS
  modifier onlyValidProposer(address pool) {
    require(
      IPool(pool).isValidProposer(msg.sender),
      ExceptionsLibrary.NOT_VALID_PROPOSER
    );
    _;
  }

  modifier onlyInvoiceCreator(address pool, uint256 invoiceId) {
    require(
      invoices[pool][invoiceId].createdBy == msg.sender,
  ...
  function payInvoice(
    ...
    if (invoice.core.whitelist.length > 0) {
      ...
      for (uint256 i = 0; i < invoice.core.whitelist.length; i++) {
        if (invoice.core.whitelist[i] == msg.sender) isWhitelisted = true;
      }
    ...
    if (invoice.core.unitOfAccount == address(0)) {
      require(msg.value == invoice.core.amount, ExceptionsLibrary.WRONG_AMOUNT);
  ...
  /**
   * @notice create Invoice for given pool (only validProposer)
   * @param pool Pool address
  ...
  ) external onlyValidProposer(pool) {
    //validation
  ...
  /**
   * @notice Cancel Invoice by invoice id (only Invoice Creator)
   * @param pool Pool address
   * @param invoiceId invoiceId
   */
  function cancelInvoice(
    address pool,
    uint256 invoiceId
  ) external onlyInvoiceCreator(pool, invoiceId) {
  ...
  function validateInvoiceCore(
    ...
    require(core.expirationBlock > block.number , ExceptionsLibrary.WRONG_BLOCK_NUMBER);
