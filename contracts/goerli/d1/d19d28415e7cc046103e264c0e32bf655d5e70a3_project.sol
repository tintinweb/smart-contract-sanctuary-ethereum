 ...
 * {ERC721Enumerable}.
 */
abstract contract ERC721 is Context, IERC165, ERC165, IERC721, IERC721Metadata {
...
abstract contract MarketPlace is IMarketPlace, ERC721, Ownable {
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        // _balances and _owners need to be changed from private to interal if inheriting ERC721
        _balances[from] -= 1;
        _balances[to] += 1;
        _owners[tokenId] = to;
    ...
    function delistItem(uint256 tokenId) external {
        require(_msgSender() == ownerOf(tokenId), "Invalid Lister");
        _delistItem( tokenId );
    ...
    function buyItem(uint256 tokenId) external payable {
        ...
        if (royalty > 0){
            ...
            (success, ) = owner().call{value: royaltyPrice}("");
            require(success, "Transaction Unsuccessful");
        }
    ...
    // CONTRACT OWNER
    function setRoyalty(uint royaltyPercent) external onlyOwner {
        // royaltyPercent = the percentage amount * 1000, thus 10% is 10000
...
abstract contract ApprovalBlacklist is Ownable {
    //REQUIRES ADDITION OF wlOnly TO ALL APPROVAL FUNCTIONS

}

abstract contract ApprovalWhitelist is Ownable {
