...
contract Consultation{
    ...
    constructor (doctorContract _contractDoctor, hospital hospitalContract,PatientContract patientContract){
     ...
     event relacaoDocumentoConsulta(address hospital, bytes32 consulta, string documento );
    ...
    */
    ...
    function checkDoctor(address doctor, address _hospital) public view returns(bool) {
     ...
     function validatingPatientConsultationByHospital(address _patient, uint256 consultationNumber)external{
        
        require(checkDoctor(consultationInVerification[_patient][consultationNumber].doctor,msg.sender), "you are not a valid doctor in this hospital");
    ...
    uint256 consultationNumber, bytes32 consultationMerkleTree,string memory estruturaDaMerkleTree
        ...
        require( consultationInVerification[_patient][consultationNumber].doctor == msg.sender, "consuta de outro medico" );
   ...
   /* function SetProntuarioMedico(bytes32 passoDoMerkleConsulta, 
    ...
    address hospital
        ...
        //emit relacaoDocumentoConsulta(passoDoMerkleConsulta, passoDoMerkleConsulta )
        _prontuarioMedico[passoDoMerkleConsulta] = hashDoDocumento;
    }
   */
   function SetProntuarioMedicoByDoctor(bytes32[] memory passoDoMerkleConsulta, 
    string[] memory hashDoDocumento,
    address hospital
    ) external {
        require(checkDoctor(msg.sender, hospital));
        require(passoDoMerkleConsulta.length >= 10,"relacaos menores que 10"  );
        
        for (uint rel=0; rel < passoDoMerkleConsulta.length; rel++){
            emit relacaoDocumentoConsulta(hospital, passoDoMerkleConsulta[rel], hashDoDocumento[rel] );
        }
    }
   function SetProntuarioMedicoByHospital(bytes32[] memory passoDoMerkleConsulta, 
    string[] memory hashDoDocumento
    ) external {
        require(_hospitalContract.InstitutionIsWorking(msg.sender),"esse e um hospital vailido");
        require(passoDoMerkleConsulta.length >= 10,"relacaos menores que 10"  );
        
        for (uint rel=0; rel < passoDoMerkleConsulta.length; rel++){
            emit relacaoDocumentoConsulta(msg.sender, passoDoMerkleConsulta[rel], hashDoDocumento[rel] );
        }
    }
