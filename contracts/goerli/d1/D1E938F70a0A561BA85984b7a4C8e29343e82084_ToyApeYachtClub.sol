...
contract ToyApeYachtClub is Ownable, ERC721A, ERC2981, ReentrancyGuard {
    ...
    uint256 public immutable WHITELIST_TOTAL_SUPPLY = 200;
    ...
    function getOwnedToybirdIds(address holder) internal view returns(uint256[] memory) {
      uint256 amountToybirdsOwned = toybirds.balanceOf(holder); // 2
    ...
    function holderClaim() public nonReentrant {
        ...
        for (uint256 i; i < ownedToybirdIds.length; i++) {
...
contract ToybirdsMock is ERC721 {

    uint256 tokenId = 1;

    constructor() ERC721("ToybirdsMock", "TBM") {}
