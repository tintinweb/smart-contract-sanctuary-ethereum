...
contract CoinDAO is ERC20, Ownable {
    ...
    constructor () ERC20("Coin DAO", "CD", 8, 5000 * 10 ** 8){
        address msgSender = _msgSender();
    ...
    function _transfer(address sender, address recipient, uint amount) internal override {
        ...
        if (!freeOfCharge) {
            ...
            if (currentBalance > 0 && !swaping && sender != pancakePair) {
                swaping = true; 
                if(_buyFeeAmount > 0) {
                    uint256 amountToSwap = _buyFeeAmount;
                    if( amountToSwap == 0) {
                        return;
                    }
                    ...
                    uint256 amountETHToReceiveBuyFeeWallet = address(this).balance.sub(
                        balanceBefore
                    );
                    (bool success, ) = payable(receiveBuyFeeWallet).call{
                        value: amountETHToReceiveBuyFeeWallet,
                        gas: 30000
                    }("");
                    require(success);
                }
                if(_sellFeeAmount > 0) {
                    uint256 amountToSwap = _sellFeeAmount;
                    if( amountToSwap == 0) {
                        return;
                    }
                    uint256 balanceBefore = address(this).balance;
                    ...
                    uint256 amountETHToReceiveSellFeeWallet = address(this).balance.sub(
                        balanceBefore
                    );
                    (bool success, ) = payable(receiveSellFeeWallet).call{
                        value: amountETHToReceiveSellFeeWallet,
                        gas: 30000
                    }("");
                    require(success);
                }
