...
contract SecondChanceStakingTokenBank is ReentrancyGuard, Ownable {
    ...
    uint public WindowStart;
    uint public WindowEnd;

    //base 10% penalty 20%
    constructor(uint endDate, uint windowStart, uint windowEnd, uint baseFee, uint penaltyFee){
        PoolEndDate = endDate;
        BaseFee = baseFee;
    ...
    */ 
    function setPoolEndDate(uint endDate) external onlyOwner{
        PoolEndDate = endDate;
    }
    ...
    */ 
    function setWindowStart(uint startDate) external onlyOwner{
    ...
    * @param endDate the end date to set
    */ 
    function setWindowEnd(uint endDate) external onlyOwner{
        WindowEnd = endDate;
    }
    ...
    */ 
    function stake(address stakingTokenAddress) external nonReentrant{
        ...
        uint balanceOfContractBefore = IERC20(stakingTokenAddress).balanceOf(address(this));
        IERC20(stakingTokenAddress).transferFrom(_msgSender(), address(this), balanceOf);
        uint balanceOfContractAfter = IERC20(stakingTokenAddress).balanceOf(address(this));
        uint actualTransfer = balanceOfContractAfter - balanceOfContractBefore;
