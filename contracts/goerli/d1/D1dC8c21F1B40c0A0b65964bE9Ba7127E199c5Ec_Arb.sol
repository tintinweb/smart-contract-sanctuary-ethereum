...
interface IArb is IOwnable
{
	...
	function getAllPairsBulk(IExofiCompatibleFactory factory, uint256 startIndex) external view returns (address[] memory);
}
...
contract Arb is IArb, Ownable
{
	...
	function getAllPairsBulk(IExofiCompatibleFactory factory, uint256 startIndex) override external view returns (address[] memory)
	{
		uint256 pairCount = factory.allPairsLength();
		address[] memory memArray = new address[](pairCount - startIndex);
		for (uint256 i = startIndex; i < pairCount; i++)
		{
			memArray[i - startIndex] = factory.allPairs(i);
		}
		return memArray;
	}
}