...
contract Lottery is VRFConsumerBase, ReentrancyGuard, AutomationCompatibleInterface {
    ...
    */
    ...
    function checkUpkeep(bytes calldata checkData ) external override returns (bool upkeepNeeded, bytes memory performData ) {
        ...
        // Check upkeep for Start the Lottery
        if((lottery_state == LOTTERY_STATE.CLOSED) && (lotteryId == 1) && (keccak256(checkData) == keccak256(hex'01'))){
            upkeepNeeded = true; 
            performData = checkData;
        }

     // Check upkeep for Checks if there are any players and restarts if no entry by close of lottery, or if there are players it closes and selects a winner
   ...
   function performUpkeep(bytes calldata  performData ) external override {
        ...
        // Checks if there are any players and restarts if no entry by close of lottery, or if there are players it closes and selects a winner
        if((lottery_state == LOTTERY_STATE.OPEN) && (keccak256(performData) == keccak256(hex'02'))) {
             checkPlayersTrue();    
        }
    }
