    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant FEE_PERCENT_PRECISION = 1e18;
    ...
    function setPeriodROI(
        uint256 _day,
        uint256 _timestamp,
        uint256 _roi
    ) public whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) {
        require(periodROIs[_day].isExist == false, "Period: already exist");
        periodROIs[_day] = PeriodROI(_timestamp, _roi, true, true);
        periodROIsList.push(_day);
    }

    function updatePeriodROI(
        uint256 _day,
        uint256 _timestamp,
        uint256 _roi
    ) public whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        whenNotPaused
    {
        ...
        require(_day != 0, "Date: non zero");
        require(getPeriodROI(_day).isExist == true, "Date: isn't exist");
        require(getPeriodROI(_day).isActive == true, "Date: isn't active");

        token.safeTransferFrom(msg.sender, address(this), _amount);

        uint256 _stakeId = getStakesByOwner(msg.sender).length;
        uint256 _rewards = calculateReward(_amount / 10e18, _day);
    ...
    function calculateReward(uint256 _amount, uint256 _day)
        ...
        returns (uint256)
    {
        uint256 _roi = (getPeriodROI(_day).roi).mul(100);

        uint256 _percentage = (10e18 * uint256(_roi)) / 10_000;
        uint256 _roiPerDay = _percentage / 365;

        uint256 _rewards = _roiPerDay * _amount;
