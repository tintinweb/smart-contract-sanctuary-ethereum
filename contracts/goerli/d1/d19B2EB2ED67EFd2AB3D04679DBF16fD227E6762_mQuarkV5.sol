...
// contract mQuarkV5 is ERC1155, IERC1155Receiver, IERC2981 {
contract mQuarkV5 is ERC721, IERC721Receiver, IERC2981 {
  using EnumerableSet for EnumerableSet.UintSet;

  /*==================================================== Events =============================================================*/
  // event TemplateCreated(uint256 tempalteId, uint256 amount, uint256 minId, uint256 maxId, string uri);
  // event TemplatesSelected(address metaverse, uint256[] templateIds, uint256[] amounts, uint256 globalTokenIdVariable); //*** */
  event TemplatesSelected(address metaverse, 
    uint256 templateId, 
  ...
  event MetaverseSlotAdded(uint256 tokenId, uint256 metaverseId, string uri);
  event MetaverseBatchSlotAdded(uint256[] tokenId, uint256 metaverseId, string uri);
  event MetaverseSlotMetadataReset(uint256 tokenId, uint256 metaverseId);
  event MetaverseMetadataUpdated(bytes signature, uint256 metaverseId, uint256 tokenId, string newUri);
  event NFTMinted(address metaverse, uint256 templateId, uint256 collectionIndex, uint256 tokenId, address to); ///*** */
   ...
   *@param collectionIndex: Index of selected template among collections in Metaverse list
  ...
  mapping(uint256 => string) public templateURI;
  ...
  //Authorized to register metaverses
  mapping(address => bool) public authorizedContracts;

  //metaverse wallet -> teamplate id => collection index
  mapping(address => mapping(uint256 => uint256)) metaverseCollectionIndex;

  //metaverse wallet -> teamplate id -> collection index  => TemplateData
  mapping(address => mapping(uint256 => mapping(uint256 => TemplateData))) public metaverseTemplates;
  ...
  */
  ...
  constructor() ERC721("mQuark", "MQRK") {
    admin = msg.sender;
  }
   ...
   */
  // function createTemplate(uint256 _amount, string calldata _uri) external onlyAdmin {
  function createTemplate(string calldata _uri) external onlyAdmin {
    uint256 _templateId = ++templateIdCounter;
    templateURI[_templateId] = _uri;
    templateIds.add(_templateId);

    emit TemplateCreated(_templateId, _uri);
  }
   ...
   */
  function createBatchTemplate(string[] calldata _uris) external onlyAdmin {
    require(_uris.length <= 100, "Invalid amount of template creation!");

    uint256 _templateId = templateIdCounter;
    for (uint256 i = 0; i < _uris.length; i++) {
      ++_templateId;
      templateURI[_templateId] = _uris[i];
      templateIds.add(_templateId);
   ...
   *@param _templateId: The Template ID of the NFT that will be minted by the user
   *@param _collectionIndex:
   */
  function mint(
    address _to,
    address _metaverse,
    ...
    // require(templateIds.contains(_templateId), "Not a valid ID!");
    TemplateData memory _tempData = metaverseTemplates[_metaverse][_templateId][_collectionIndex];
    require(_tempData.minTokenId != 0, "Can't mint an unexisting token!");
    require(_tempData.mintCount <= _tempData.amount, "Can't mint anymore, please contact admins!");

    uint256 _tokenId = _tempData.minTokenId + _tempData.mintCount;
    _mint(_to, _tokenId);
    tokenIdURI[_tokenId] = templateURI[_templateId];
    ++metaverseTemplates[_metaverse][_templateId][_collectionIndex].mintCount;

    emit NFTMinted(_metaverse, _templateId, _collectionIndex, _tokenId, _to);
  }
  ...
  function mintBatch(
    ...
    TemplateData memory _tempData;
    uint256 _tokenId;

    for (uint256 i; i < _mintParams._templateIds.length; i++) {
      _tempData = metaverseTemplates[_metaverse][_mintParams._templateIds[i]][_mintParams._collectionIndexes[i]];
      require(_mintParams._templateIds[i] != 0 && _mintParams._amounts[i] != 0, "Invalid template ID or amount!");
      require(_mintParams._amounts[i] <= 10, "Can't mint more than 10!");
      require(_tempData.minTokenId != 0, "Can't mint an unexisting token!");
      require((_tempData.mintCount + _mintParams._amounts[i]) <= (_tempData.amount), "Can't mint this amount, please contact admins!");

      for (uint256 ii; ii < _mintParams._amounts[i]; ii++) {
        _tokenId = _tempData.minTokenId + _tempData.mintCount;
        _mint(_to, _tokenId);
        ++_tempData.mintCount;
        tokenIdURI[_tokenId] = templateURI[_mintParams._templateIds[i]];
        emit NFTMinted(_metaverse, _mintParams._templateIds[i], _mintParams._collectionIndexes[i], _tokenId, _to);
      }

      metaverseTemplates[_metaverse][_mintParams._templateIds[i]][_mintParams._collectionIndexes[i]].mintCount += _mintParams._amounts[i];
    }
  }
  ...
  function mintBatchWithMetadataSlot(
    ...
    for (uint256 i; i < _mintParams._templateIds.length; i++) {
      require(_mintParams._templateIds[i] != 0 && _mintParams._collectionIndexes[i] != 0, "Invalid template ID or index!");
      _tempData = metaverseTemplates[_metaverse][_mintParams._templateIds[i]][_mintParams._collectionIndexes[i]];
      // require(_amounts[i] <= 10, "Can't mint more than 10!");
      require(_tempData.minTokenId != 0, "Can't mint an unexisting token!");
      require((_tempData.mintCount) <= (_tempData.amount), "Can't mint this amount, please contact admins!");

      for (uint256 j; j < _mintParams._amounts[i]; j++) {
        _tokenId = _tempData.minTokenId + _tempData.mintCount;
        _mint(_to, _tokenId);
        ...
        ++_tempData.mintCount;
        emit NFTMinted(_metaverse, _mintParams._templateIds[i], _mintParams._collectionIndexes[i], _tokenId, _to);
        emit MetaverseSlotAdded(_tokenId, _metaverseId, _metaverseMetadataTemplate);
      }

      metaverseTemplates[_metaverse][_mintParams._templateIds[i]][_mintParams._collectionIndexes[i]].mintCount += _mintParams._amounts[i];
    }

    // emit NFTBatchMinted(_templateIds, _tokenIds, _to);
    // emit MetaverseBatchSlotAdded(_tokenIds, _metaverseId, _metaverseMetadataTemplate);
  }
  ...
  function mintWithMetadaSlots(
    ...
    string[] calldata _metaverseMetadataTemplates
  ) external onlyAuthorizedContract {
    TemplateData memory _tempData = metaverseTemplates[_metaverses[0]][_templateId][_collectionIndex];
    require(_tempData.mintCount <= _tempData.amount, "Can't mint anymore, please contact admins!");
    uint256 _tokenId = _tempData.minTokenId + _tempData.mintCount;
    _mint(_to, _tokenId);
    tokenIdURI[_tokenId] = templateURI[_templateId];
    ++metaverseTemplates[_metaverses[0]][_templateId][_collectionIndex].mintCount;
    addBatchMetaverseSlotsToTemplate(_to, _tokenId, _metaverseIds, _metaverseMetadataTemplates);
    emit NFTMinted(_metaverses[0], _templateId, _collectionIndex, _tokenId, _to);
  }
  ...
  function addMetadataSlotToTemplate(
    ...
    require(ownerOf(_tokenId) == _owner, "You don't have the token!");
  ...
  function selectTemplates(
    ...
    uint256 _collecIndex;

    for (uint256 i; i < _templateIds.length; i++) {
      _collecIndex = ++metaverseCollectionIndex[_metaverse][_templateIds[i]];
      TemplateData memory _tempData = metaverseTemplates[_metaverse][_templateIds[i]][_collecIndex];
      _tempData.id = _templateIds[i];
      _tempData.amount = _amounts[i];
      _tempData.minTokenId = _globalTokenIdVariable + 1;
      _globalTokenIdVariable = _globalTokenIdVariable + _amounts[i];
      _tempData.maxTokenId = _globalTokenIdVariable;
      _tempData.collectionIndex = _collecIndex;
      // _tempdata.isPurchasable = true;
      // metaverseTemplateMintPrices[msg.sender][_templateIds[i]] = templateMintPrice[_templateIds[i]];
      _tempData.templateURI = templateURI[_templateIds[i]];
      metaverseTemplates[_metaverse][_templateIds[i]][_collecIndex] = _tempData;

      emit TemplatesSelected(_metaverse, _templateIds[i], _collecIndex, _amounts[i], _tempData.minTokenId, _tempData.maxTokenId);
    }
    globalTokenIdVariable = _globalTokenIdVariable;
    // emit TemplatesSelected(_metaverse, _templateIds, _amounts, _globalTokenIdVariable);
  }
  ...
  function updateURI(
    ...
    if (isVerified) {
      ...
      emit MetaverseMetadataUpdated(_signature, _metaverseId, _tokenId, _newURI);
  ...
  function resetMetaverseURI(
    ...
    emit MetaverseSlotMetadataReset(_tokenId, _metaverseId);
  }
  ...
  function supportsInterface(bytes4 interfaceId) public pure override(ERC721, IERC165) returns (bool) {
    return (interfaceId == type(IERC721Receiver).interfaceId) || (interfaceId == type(IERC2981).interfaceId);
  }
