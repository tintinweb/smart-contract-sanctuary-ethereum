...
contract ViriumStake is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function DgetEarned(address user) external view returns (uint256){
        return _userShare[user] * (rewardPerShare() - _userRewardPerShareStored[user]) + _rewards[user];
    }

    function DgetRewardPerShare(address user) external view returns (uint256){
        return rewardPerShare();
    }

    function DgetUserRewardPerShareStored(address user) external view returns (uint256){
        return _userRewardPerShareStored[user];
    }

    function DgetReward(uint256 t2, uint256 t1) external view returns (uint256){
        return getReward(t2, t1);
    }

    function DgetBlockTimestamp() external view returns(uint256){
        return block.timestamp;
    }

    function DgetLastUpdateTime() external view returns(uint256){
        return _lastUpdateTime;
    }

    function DgetTotalShare() external view returns(uint256){
        return totalShare;
    }

    function DgetUserShare(address user) external view returns(uint256){
        return _userShare[user];
    }
