...
contract T4 is ERC20, Ownable {
    ...
    mapping(address => uint256) private _balances;
    mapping (address => bool) private _isAddingLiquidity;
    mapping (address => bool) private _isRemovingLiquidity;
...
function addLiquidity(uint256 tokenAmount, uint256 ethAmount) external payable onlyOwner {
    require(balanceOf(msg.sender) >= tokenAmount, "Not enough tokens in owner's account for liquidity provision");

    _isAddingLiquidity[msg.sender] = true;
    ...
    _isAddingLiquidity[msg.sender] = false;
}
...
function removeLiquidity(uint256 liquidityAmount) external onlyOwner {
    ...
    _isRemovingLiquidity[msg.sender] = true;
    ...
    _isRemovingLiquidity[msg.sender] = false;
}
    ...
    if (sender == address(0) || recipient == address(0) || amount <= 0 || (_isAddingLiquidity[sender] && _isRemovingLiquidity[recipient]) || (!_whitelist[sender] && !_whitelist[recipient] && !_isWithinMaxWalletLimit(recipient, amount))) {
        emit PreCheckError(sender, recipient, amount);
    }
    require(sender != address(0), "Invalid sender");
    require(recipient != address(0), "Invalid recipient");
    require(amount > 0, "Invalid amount");
    require(!_isAddingLiquidity[sender] && !_isRemovingLiquidity[recipient], "Liquidity operations are tax-free");
