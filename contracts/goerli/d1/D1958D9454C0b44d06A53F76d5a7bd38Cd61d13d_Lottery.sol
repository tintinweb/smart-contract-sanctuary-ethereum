...
import "./Random.sol";
import "./model/Models.sol";

// for debugging
import "hardhat/console.sol";

// Author: @David Duong
contract Lottery is Ownable, Pausable, AccessControl {
     ...
     */
    uint256 public constant MIN_DRAWING_INCREMENT = 1000000000000000000; // 1 Token; min amount to enter lottery.
    ...
    */
     ...
     */
    modifier isNewPlayerValid(IERC20 token, uint amount) {
        if (amount < MIN_DRAWING_INCREMENT) {
     ...
     */
    function setMaxPlayersAllowed(
        uint256 maxPlayersAllowed_
    ) external onlyOwner {
        maxPlayersAllowed = maxPlayersAllowed_;
        emit LogMaxPlayersAllowedUpdated(maxPlayersAllowed);
    }

    function setAcceptedToken(IERC20 token) external onlyOwner {
        acceptedToken = token;
    }
    ...
    function initLottery(
        ...
        external
        isNewLotteryValid // TASK: add onlyOwner and re-test
    {
     ...
     */
    function transferERC20ToContract(IERC20 token, uint amount) public payable {
        require(
    ...
    function mintLotteryTickets(
        uint amount
    ) external isNewPlayerValid(acceptedToken, amount) {
        // TODO: - check amount of token is multiply of MIN_DRAWING_INCREMENT
        uint256 _numTicketsToMint = amount / (MIN_DRAWING_INCREMENT);
        ...
        prizeAmount = prizeAmount + amount; // update the pot size
     ...
     */
    function withdraw(uint256 lotteryId_) external {
        ...
        require(
            acceptedToken.balanceOf(address(this)) >=
                _pendingCurrentUserWithdrawal,
            "Not enough token to withdraw"
        );
        bool sent = acceptedToken.transfer(
            msg.sender,
            _pendingCurrentUserWithdrawal
        );
        if (sent == false) {
...
import "./LotteryStruct.sol";
import "./TicketDistributionStruct.sol";
import "./WinningTicketStruct.sol";
