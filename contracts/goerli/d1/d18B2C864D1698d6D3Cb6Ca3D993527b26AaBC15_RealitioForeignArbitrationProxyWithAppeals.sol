...
contract RealitioForeignArbitrationProxyWithAppeals is IForeignArbitrationProxy, IDisputeResolver, FxBaseRootTunnel {
    ...
    function receiveArbitrationAcknowledgement(bytes32 _questionID, address _requester) external override onlyBridge {
        ...
        emit ArbitrationCreated(_questionID, _requester, 11);
    ...
    function executeFromChild(bytes memory _data) public {
        // solhint-disable-next-line avoid-low-level-calls
        (bool success, ) = address(this).call(_data);
        require(success, "Failed to call contract");
    }
...
abstract contract FxBaseRootTunnel {
    ...
    function getData(bytes memory inputData) public returns (RLPReader.RLPItem[] memory) {
        return inputData.toRlpItem().toList();
    }

    function getReceipt(bytes memory inputData) public returns (RLPReader.RLPItem[] memory) {
        RLPReader.RLPItem[] memory inputDataRLPList = inputData.toRlpItem().toList();
        return inputDataRLPList[6].toBytes().toRlpItem().toList();
    }

    function getLogRLP(bytes memory inputData) public returns (RLPReader.RLPItem memory) {
        RLPReader.RLPItem[] memory inputDataRLPList = inputData.toRlpItem().toList();
        RLPReader.RLPItem[] memory receiptRLPList = inputDataRLPList[6].toBytes().toRlpItem().toList();
        return receiptRLPList[3].toList()[
            inputDataRLPList[9].toUint() // receiptLogIndex
        ];
    }

    function getLogRLPList(bytes memory inputData) public returns (RLPReader.RLPItem[] memory) {
        RLPReader.RLPItem[] memory inputDataRLPList = inputData.toRlpItem().toList();
        RLPReader.RLPItem[] memory receiptRLPList = inputDataRLPList[6].toBytes().toRlpItem().toList();
        ...
        return logRLP.toList();
    }

    function getLogRLPListTopic(bytes memory inputData) public returns (RLPReader.RLPItem[] memory) {
        RLPReader.RLPItem[] memory inputDataRLPList = inputData.toRlpItem().toList();
        RLPReader.RLPItem[] memory receiptRLPList = inputDataRLPList[6].toBytes().toRlpItem().toList();
        RLPReader.RLPItem memory logRLP = receiptRLPList[3].toList()[
            inputDataRLPList[9].toUint() // receiptLogIndex
        ...
        return logRLPList[1].toList();
    }

    function _validateAndExtractMessage(bytes memory inputData) public returns (bytes memory) {
        RLPReader.RLPItem[] memory inputDataRLPList = inputData.toRlpItem().toList();
