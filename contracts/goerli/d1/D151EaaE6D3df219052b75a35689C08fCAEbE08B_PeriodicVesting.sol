...
interface IPeriodicVesting is IVesting {
    struct VestingPeriod {
        ...
        uint256 cadence; // frequency of token vesting in seconds
    ...
    struct VestingDetails {
        ...
        uint256 vestingStart; // start time of vesting period
        VestingPeriod[] periods; // vesting periods
    }

    /**
     * @dev Returns vesting details
     */
...
interface IVesting {
    /**
     * @dev Emitted when token released from vesting contract
    ...
    /**
     * @dev Returns number of tokens available to be released by beneficiary
     */
    function releasable() external view returns (uint256);

    /**
     * @dev Withdraws vested tokens from contract
     * @param amount Amount of tokens to withdraw
     */
    function release(uint256 amount) external;

    /**
     * @dev Returns amount of vested tokens
     * @param timestamp Timestamp for which to return vested tokens
     */
...
import "./interfaces/IPeriodicVesting.sol";
...
contract PeriodicVesting is IPeriodicVesting, ERC165 {
    ...
    function release(uint256 amount) external virtual {
        ...
        require(amount <= releasable(), "Not enough tokens vested");
    ...
    function vestedAmount(uint64 timestamp) public view virtual returns (uint256) {
        if (timestamp < start) {
            return 0;
        } else if (timestamp >= start + totalDuration) {
            return totalAllocation;
        } else {
            IPeriodicVesting.VestingPeriod[] memory _periods = periods;

            uint256 vested = 0;
            uint256 periodStart = start;

            uint256 index = 0;
            for (; index < _periods.length; index++) {
                uint256 newPeriodStart = periodStart + _periods[index].duration;
                if (timestamp < newPeriodStart) {
                    break;
                } else {
                    periodStart = newPeriodStart;
                    vested += _periods[index].allocation;
                }
            }
            ...
            IPeriodicVesting.VestingPeriod memory period = _periods[index];
            if (timestamp < periodStart + period.cliff) {
                return vested;
            }

            uint256 elapsedSeconds = timestamp - periodStart;
            uint256 vestedCadences = elapsedSeconds / period.cadence;
            uint256 vestedSeconds = vestedCadences * period.cadence;

            return vested + (period.allocation * vestedSeconds) / period.duration;
        }
    }
    ...
    function getDetails() external view returns (IPeriodicVesting.VestingDetails memory) {
        return IPeriodicVesting.VestingDetails(token, beneficiary, start, periods);
    }
    ...
    function _validateSchedule(IPeriodicVesting.VestingPeriod[] memory periods_) internal pure {
        for (uint256 i = 0; i < periods_.length; i++) {
