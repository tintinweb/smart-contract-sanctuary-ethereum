...
contract Service {
        ...
        returns (bool)
    {
        ...
        if (
            ...
            subscription.cost * _days <
            IERC20(subscription.token).balanceOf(subscription.user)
        ) {
            return true;
        }
    ...
    function unpaidPendingTime(bytes32 _hash) public view returns (uint256) {
        if (block.timestamp > lastPaid[_hash]) {
    ...
    // close subscription
    function subscriptionStop(bytes32 _hash) external {
        ...
        emit Close(subscription.hash);

        delete subscriptions[_hash];
    }

    // process subscription
    function subscriptionExecute(bytes32 _hash, uint256 _days) external {
        Plan memory subscription = subscriptions[_hash];
