...
contract RaffleTicketMachine is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) private tokenIdToRaffleName;
    ...
    constructor(address _token, bool _contractPaused) ERC721("RaffleTicketMachine", "RDMRFL") {
        baseURI = "https://raffleticketmachine.com/api/";
    ...
    modifier createRaffleCompliant(uint256 _raffleName) {
        bool raffleExists = false;
        for (uint256 i = 0; i < raffles.length; i++) {
            if (_raffleName == raffles[i].raffleName) {
                raffleExists = true;
            }
        }
    ...
    modifier isPaused(uint256 _raffleName) {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                require(raffles[i].paused, "Raffle is not paused");
            }
        }
        _;
    }

    modifier isEthMintEnabled(uint256 _raffleName) {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
    ...
    modifier isRandomMintEnabled(uint256 _raffleName) {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                require(
    ...
    modifier isSplitMintEnabled(uint256 _raffleName) {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                require(
    ...
    modifier isQuantityAvailable(uint256 _raffleName, uint256 _quantity) {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                require(
                    raffles[i].quantity >= _quantity,
    ...
    modifier isQuantityPerMintValid(uint256 _raffleName, uint256 _quantity) {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                require(
                    raffles[i].maxQuantityPerMint >= _quantity,
                    "Quantity per mint is too high"
                );
            }
        }
        _;
    ...
    function getRaffle(uint256 _raffleName)
        public
        view
        returns (
            uint256 name,
    ...
    function getRaffleQuantity(uint256 _raffleName)
        public
        view
        returns (uint256 quantity)
    {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                return raffles[i].quantity;
            }
        }
    }

    function getRaffleMaxQuantity(uint256 _raffleName)
        public
        view
        returns (uint256 maxQuantity)
    {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                return raffles[i].maxQuantity;
            }
        }
    }

    function getRafflePaused(uint256 _raffleName)
        public
        view
    ...
    function getRaffleSplitMintEnabled(uint256 _raffleName)
        public
        view
    ...
    function getRaffleEthMintEnabled(uint256 _raffleName)
        public
        view
    ...
    function getRaffleRandomsMintEnabled(uint256 _raffleName)
        public
        view
    ...
    function getRaffleSplitPrice(uint256 _raffleName)
        public
        view
    ...
    function getRaffleTicketPrice(uint256 _raffleName)
        public
        view
        returns (uint256 eth, uint256 randoms)
    {
    ...
    function getRaffleMaxQuantityPerMint(uint256 _raffleName)
        public
        view
    ...
    function getRaffleTicketPriceEth(uint256 _raffleName)
        public
        view
        returns (uint256 eth)
    {
    ...
    function getRaffleTicketPriceRandoms(uint256 _raffleName)
        public
        view
    ...
    function getRaffleSplitPriceEth(uint256 _raffleName)
        public
        view
        returns (uint256 eth)
    {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                return raffles[i].splitPrice.eth;
            }
        }
    }

    function getRaffleSplitPriceRandoms(uint256 _raffleName)
        public
        view
        returns (uint256 randoms)
    {
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                return raffles[i].splitPrice.randoms;
            }
        }
    }

    function ethMint(uint256 _raffleName, uint256 _quantity)
        public
        payable
        ...
        for (uint256 i = 0; i < raffles.length; i++) {
            if (raffles[i].raffleName == _raffleName) {
                ...
                for (uint256 j = 0; j < _quantity; j++) {
                    uint256 raffleName = raffles[i].raffleName;
                    uint256 tokenId = raffles[i].quantity + 1;
                    uint256 fullTokenId = raffleName * (10**18) + tokenId;
                    _safeMint(msg.sender, fullTokenId);
                    raffles[i].quantity += 1;
                    tokenIdToRaffleName[fullTokenId] = raffles[i].raffleName;
                }
            }
        }
    }

    function randomsMint(uint256 _raffleName, uint256 _quantity)
        public
        isPaused(_raffleName)
    ...
    function splitMint(uint256 _raffleName, uint256 _quantity)
        public
        payable
        isPaused(_raffleName)
        ...
        returns (string memory)
    {
        ...
        uint256 raffleName = tokenIdToRaffleName[_tokenId];
        _tokenId = _tokenId % (10**18);
        return
            string(
                abi.encodePacked(
                    baseURI,
