...
contract Revolution is Ownable, AccessControl {
    ...
    modifier hasVotesOrAdmin() {
        require(
            daoToken.getVotes(msg.sender) >= MIN_VOTES || hasRole(ADMIN_ROLE, msg.sender),
    ...
    function grantAdmin(address account) public {
        require(hasRole(ADMIN_ROLE, msg.sender), "Only admins can grant roles");
    ...
    function setMinVotes(uint256 newMinVotes) public isAdmin {
    ...
    function setFundsRecipient(address payable newFundsRecipient) public isAdmin {
    ...
    function rejectQueuedDrop() external isAdmin {
        delete queuedDrop;
    }

    function deployRevolutionDrop() external isAdmin returns (address) {
