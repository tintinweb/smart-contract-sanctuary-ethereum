...
import "../Helpers.sol";
import "../artPass/IArtOnFtsMintPass.sol";
import "./ReentrancyGuard.sol";
import "./Signable.sol";
import "./Errors.sol";

contract FtsNFT is ERC721, ReentrancyGuard, Signable, AccessControl {
    ...
    IArtOnFtsMintPass private _artOnFtsMintPass;
    ...
    function preSaleMint(
        ...
        _mintLogic(amount);
        _mintArtOnFtsLogic(msg.sender, amount);
    }
    ...
    function setBaseURI2(string calldata baseURI_) external onlyOwner {
    ...
    function setArtOnFtsMintPass(address artOnFtsMintPass_)
        external
        onlyOwner
    {
        _artOnFtsMintPass = IArtOnFtsMintPass(artOnFtsMintPass_);
    }
    ...
    function _mintArtOnFtsLogic(address to, uint256 amount) private {
        if (address(_artOnFtsMintPass) == address(0))
            revert Errors.ArtOnFtsMintPassNotSet();

        _artOnFtsMintPass.mint(to, amount);
    }

    // Contract by Alexander Zimin
}
