...
interface IMetaWealthModerator is IMetaWealthAccessControlled {
    ...
    event VendorWhitelistRootUpdated(bytes32 oldRoot, bytes32 newRoot);
    ...
    function isTrustedForwarder(address account) external view returns (bool);
    ...
    function defaultCurrency() external view returns (address defaultCurrency);
    ...
    function toggleSupportedCurrency(
        address token
    ) external returns (bool newState);

    /// @notice Returns all the currencies supported by platform
    /// @return currencies is the array of currency addresses
    ...
    function updateVendorWhitelistRoot(bytes32 _newRoot) external;
    ...
    function defaultUnlockPeriod() external view returns (uint64 unlockPeriod);
    ...
    function setAssetDepositShareholdersFee(uint32 _fee) external;

    function setExchangeFee(uint32 eFee) external;

    function setFundraiseVendorFee(uint32 _fee) external;

    function setFundraiseInvestorFee(uint32 _fee) external;

    function treasuryWallet() external returns (address);

    function assetDepositShareholdersFee() external view returns (uint32);

    function fundraiseVendorFee() external view returns (uint32);

    function fundraiseInvestorFee() external view returns (uint32);

    function exchangeFee() external view returns (uint32);
    ...
    IMetaWealthModerator
{
    ...
    function setTrustedForwarder(address _newTrustForwarder) public onlyAdmin {
        _trustedForwarder = _newTrustForwarder;
    }
