...
contract NTRewardSystem is Ownable, ReentrancyGuard {
    ...
    function rewardBytesPeerToPeerHandleTwoSplit(
        ...
        if(_split[0] + _split[1] > 100 * 1 ether) revert SplitPercentageHasToEqualOneHundred();
        BYTES.transferFrom(
            msg.sender,
            _recipient[0],
        ...
        if (100 * 1 ether - _split[0] + _split[1] > 0) {
            BYTES.transferFrom(msg.sender, _recipient[0], 100 * 1 ether - _split[0] + _split[1]);
        }
    ...
    function rewardBytesPeerToPeerHandleThreeSplit(
        ...
        if(_split[0] + _split[1] + _split[2] > 100 * 1 ether) revert SplitPercentageHasToEqualOneHundred();
        BYTES.transferFrom(
            msg.sender,
            _recipient[0],
            _amount.mul(_split[0]).div(100 * 1 ether)
    ...
    function rewardBytesSpecifcWithoutCustomSplit(
        ...
        if (!vaultActive) {
            if (_recipient.length == 3) {
                rewardBytesPeerToPeerHandleThree(
                    _recipient,
                    _amount
                );
            } else {
                if (_recipient.length == 2) {
                    rewardBytesPeerToPeerHandleTwo(
                        _recipient,
                        _amount
                    );
                } else {
    ...
    {   
        ...
        if (
            _calculateReward[1] - 1 < _amount && _amount < calculateRewardTwo
        ) {
