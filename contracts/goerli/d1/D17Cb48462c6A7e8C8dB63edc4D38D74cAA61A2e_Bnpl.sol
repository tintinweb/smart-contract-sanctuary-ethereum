...
import { AllowedMarketModules } from "./AllowedMarketModules.sol";
...
contract Bnpl is NftReceiver, Ownable2Step {
    using SafeERC20 for IERC20;
    using Address for address;

    uint16 public constant HUNDRED_PERCENT = 10000;

    AllowedMarketModules public allowedMarketModules;
    ...
    /**
     * @notice A percentage of the price at which the token is offered in the market where the purchase will be made,
     * taken by the contract admin's as a fee on top of the buyer portion, measured in basis points (hundredths of a
     * percent). The max allowed value is 10000.
     */
    struct ServiceFee {
        uint256 serviceFee;
        uint256 nonce;
    ...
    // ========== Custom Errors ===========
    error Bnpl__setAllowedMarketModules_invalidAddress();

    error Bnpl__setServiceFee_invalidServiceFee();
    ...
    constructor(address _allowedMarketModules) {
        _setAllowedMarketModules(_allowedMarketModules);
    }

    // ========== External functions ==========

    function setAllowedMarketModules(address _allowedMarketModules) external onlyOwner {
        _setAllowedMarketModules(_allowedMarketModules);
    }

    function execute(Execution calldata _params) external returns (bool) {
        if (!allowedMarketModules.isModuleAllowed(_params.module)) revert Bnpl__execute_invalidModule();

        // _validateServiceFee(_params);

        // get buyer portion of the total
        uint256 serviceFeeAmount = getServiceFee(_params.totalPrice, _params.serviceFeeData.serviceFee);
        uint256 buyerDeposit = serviceFeeAmount + _params.totalPrice - _params.offer.loanPrincipalAmount;
        ...
        IERC20(_params.offer.loanERC20Denomination).safeApprove(_params.module, _params.totalPrice);

        // buy nft using corresponding module
        if (!IMarketModule(_params.module).buyAsset(_params.assetType, _params.buyData, _params.offer))
            revert Bnpl__execute_unsuccessfulBuy();
        ...
        // transfer loan principal and service fee to lender at once
        IERC20(_params.offer.loanERC20Denomination).safeTransferFrom(
            address(this),
            _params.lenderSignature.signer,
            _params.offer.loanPrincipalAmount + serviceFeeAmount
        );
    ...
    function getServiceFee(uint256 _amount, uint256 _serviceFee) public pure returns (uint256) {
        return _serviceFee > 0 ? (_amount * _serviceFee) / HUNDRED_PERCENT : 0;
    }
    ...
    function _validateServiceFee(Execution calldata _params) internal {
        ...
        if (_params.lenderSignature.signer == address(0)) revert Bnpl__validateServiceFee_invalidSigner();
        if (!isValidNonce(_params.lenderSignature.signer, _params.serviceFeeData.nonce))
            revert Bnpl__validateServiceFee_invalidNonce();

        _invalidateNonce(_params.lenderSignature.signer, _params.serviceFeeData.nonce);

        bytes32 message = keccak256(
            abi.encodePacked(
                _params.serviceFeeData.serviceFee,
                _params.lenderSignature.signer, // the lender is the same that determines the service fee
    ...
    function _setAllowedMarketModules(address _allowedMarketModules) internal {
        if (!_allowedMarketModules.isContract()) revert Bnpl__setAllowedMarketModules_invalidAddress();
        allowedMarketModules = AllowedMarketModules(_allowedMarketModules);
        emit SetAllowedMarketModules(_allowedMarketModules);
    }
}
...
contract AllowedMarketModules is Ownable2Step {
    using Address for address;
    ...
    // ========== Custom Errors ===========
    error AllowedMarketModules__setModuleAllowance_invalidAddress();

    function setModuleAllowance(address _module, bool _allowed) external onlyOwner {
        if (!_module.isContract()) revert AllowedMarketModules__setModuleAllowance_invalidAddress();
