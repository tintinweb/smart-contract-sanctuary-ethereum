...
// todo: rename contract, constructor, file
contract Agency is ERC721A, ERC2981, Ownable {

  uint public constant EARLY_SUPPLY = 5000;
  uint public constant MAX_SUPPLY = 7_700;
  uint public constant TEAM_MINT_SUPPLY = 201;

  uint public price = 0.0001 ether; // todo: update

  uint public max_per_txn = 7;

  address public theAgency = 0x4D5949BE178270B031c18FE86663d8bA5Ea97204;

  bool public earlyRecruitmentIsOpen = true; // todo: false
  bool public itsRecruitingSeason = true; // todo: false
  bool public teamHasBeenRecruited;
  bool public receiveable = true; // todo: false

  mapping(address => bool) addressHasMinted;
  mapping(address => uint8) theSpecialists; // save gas to be uint256?
  ...
  event ShouldReceiveUpdate(address indexed _fromAndTo, uint256 indexed _tokenId);

  // Have a seat. Let's begin.

  constructor() ERC721A("INTERNATIONAL", "INTL") { // todo: update these
    ...
    external
  {
    ...
    uint8 count = theSpecialists[msg.sender] > 0 ? theSpecialists[msg.sender] : 1;
  ...
  function setTheSpecialists(
    address[] calldata _whom,
  ...
  function onERC721Received(address, address from, uint256 tokenId, bytes memory)
    ...
    returns(bytes4)
  {
    if (receiveable == false) revert ProgramNotYetBegun();

    // Transfer it back to the address that sent it to this contract
    safeTransferFrom(address(this), from, tokenId);

    // Emit an event. This is paired with a listener on our server that can do things like update metadata and force a refresh of the token id on OpenSea
    emit ShouldReceiveUpdate(from, tokenId);
