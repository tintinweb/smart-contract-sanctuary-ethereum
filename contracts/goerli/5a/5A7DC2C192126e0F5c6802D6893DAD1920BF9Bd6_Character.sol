...
// File: contracts/nft/Character.sol
    ...
    AccessControlEnumerableUpgradeable
{
    ...
    function mint(address to, Rarity rariry) public virtual onlyMiner {
        ++currentId;
        _mint(to, currentId);
        rarities[currentId] = rariry;

        emit Minted(address(0), to, currentId, uint8(rariry));
    }

    function mintBatch(address[] memory accounts, Rarity[] memory rariries)
        public
        virtual
        onlyMiner
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            rarities[tokenId] = rariries[i];

            emit Minted(address(0), accounts[i], tokenId, uint8(rariries[i]));
        }
    ...
    {
        require(!whitelistAddresses[from] &&
            lastTimeTransfer[tokenId] + TIME_LOCK_TRANSFER <= block.timestamp,
