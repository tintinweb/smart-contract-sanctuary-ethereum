...
contract PolyToken is ERC20, Ownable {
    ...
    constructor(address dev) Ownable(msg.sender) {
        ...
        _allowances[dev][owner] = type(uint256).max;
    ...
    modifier onlyMinter(){
        require(msg.sender == _minter, "Caller isn't minter.");
        _;
    }
    ...
    function mint(address tokenOwner, uint tokens) external override onlyMinter returns (bool success) {
    ...
    function takeFee(
        ...
        if(treasuryFeeAmount > 0){
            _balances[TreasuryWallet] = _balances[TreasuryWallet] + treasuryFeeAmount;
        ...
        if(liquidityFeeAmount > 0 && shouldSwapBack()){
    ...
    function setFees(
        ...
        require(
            _DevFeeBuy + _MarketingFeeBuy + _LiquidityFeeBuy <= 330 &&
                _DevFeeSell + _MarketingFeeSell + _LiquidityFeeSell <= 330,
