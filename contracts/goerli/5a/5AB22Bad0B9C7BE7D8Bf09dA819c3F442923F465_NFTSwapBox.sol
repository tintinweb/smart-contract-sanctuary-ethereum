    ...
    ERC1155Holder
{
    ...
    */
    ...
    function createBox(
        ...
        if(_fixedPrice.amount > 0 && _fixedPrice.tokenAddr != address(0)) {
           INFTSwapBoxFees(NFTSwapBoxFees)._checkWhiteListToken(_fixedPrice.tokenAddr);
        }
        ...
        _transferERC20Fee(erc20swapFees, msg.sender, address(this), true);
        ...
        nonReentrant
    {
        ...
        emit SwapBoxOffer(uint32(listBoxID), uint32(offerBoxID),chainName);
    }
    /**
    ...
    */
    ...
    function purchaseBox(uint256 BoxID) payable public isOpenForSwap {
        require(swapBoxes[BoxID].state == 1, "Not Allowed");
        uint256 tokenFee = INFTSwapBoxFees(NFTSwapBoxFees)._checkPaymentTokenFee(fixedPrice[BoxID], msg.sender);
        if(fixedPrice[BoxID].tokenAddr == address(0)){
            require(msg.value == fixedPrice[BoxID].amount + tokenFee, "Insufficient Balance");
            payable(swapBoxes[BoxID].owner).transfer(fixedPrice[BoxID].amount);
            payable(withdrawOwner).transfer(tokenFee);
        }
        else {  
            require(
        ...
        isOpenForSwap
    {
        ...
        if(keccak256(abi.encodePacked(chainName)) == keccak256(abi.encodePacked("ETH")))
            INFTSwapBoxHistory(NFTSwapBoxHistory).addHistoryUserSwapFees(
                ...
                swapBoxes[listBoxID].owner,
                swapBoxes[offerBoxID].owner,
                nft_gas_SwapFee[listBoxID],
                nft_gas_SwapFee[offerBoxID],
                erc20Fees[listBoxID],
                erc20Fees[offerBoxID]
            );
        _historyCounter++;
        emit Swaped(
            _historyCounter,
        ...
        nonReentrant
    {
        ...
        emit SwapBoxWithDrawOffer(uint32(listBoxID), uint32(offerBoxID),chainName);
    }
...
interface INFTSwapBoxHistory is INFTSwap {
    function addHistoryUserSwapFees(uint256, uint256, uint256, address, address, uint256, uint256, ERC20Fee[] calldata, ERC20Fee[] calldata) external;
}
