...
interface IChecks {
    ...
    function compositeMany(uint256[] memory, uint256[] memory) external;

    function infinity(uint256[] memory) external;

    function getCheck(uint256) external view returns (Check memory);

    function mint(uint256[] memory, address recipient) external;
}
...
library ChecksArt {
    ...
    function getCheck(uint256 tokenId, IChecks.Checks storage checks)
        public
        view
        returns (IChecks.Check memory check)
    {
    ...
    function getCheck(
        ...
        check.seed = (uint256(keccak256(abi.encodePacked(randomness, stored.seed))) %
            type(uint128).max);
        ...
        check.composite = !check.isRoot && divisorIndex < 7
            ? stored.composites[divisorIndex - 1]
            : 0;
    ...
    function gradientIndex(IChecks.Check memory check, uint8 divisorIndex)
        public
        pure
        returns (uint8)
    {
    ...
    /// @param divisorIndex The check divisor in question.
    function colorBandIndex(IChecks.Check memory check, uint8 divisorIndex)
        public
        pure
        returns (uint8)
    {
        ...
        if (check.hasManyChecks) {
            if (gradient > 0) {
                ...
                for (uint256 i = 1; i < checksCount; ) {
                    indexes[i] =
                        (indexes[0] + (((i * gradient * colorBand) / checksCount) % colorBand)) %
                        80;
                    unchecked {
                        ++i;
                    }
    ...
    /// @param checks The DB containing all checks.
    function colors(IChecks.Check memory check, IChecks.Checks storage checks)
        public
        view
    ...
    function fillAnimation(
        ...
        return
            abi.encodePacked(
                ...
                '" ',
                'dur="',
                Utilities.uint2str((count * 2) / data.check.speed),
    ...
    function generateChecks(CheckRenderData memory data) public pure returns (bytes memory) {
        ...
        for (uint8 i; i < checksCount; i++) {
            ...
            string memory translateX = Utilities.uint2str(
                data.rowX + data.indexInRow * data.spaceX
            );
    ...
    /// @param checks The DB containing all checks.
    function collectRenderData(IChecks.Check memory check, IChecks.Checks storage checks)
        public
        view
        returns (CheckRenderData memory data)
    {
    ...
    /// @param checks The DB containing all checks.
    function generateSVG(IChecks.Check memory check, IChecks.Checks storage checks)
        public
        view
        returns (bytes memory)
    {
        ...
        return
            abi.encodePacked(
                ...
                ">",
                "<defs>",
                '<path id="check" fill-rule="evenodd" d="',
                CHECKS_PATH,
                '"></path>',
...
library ChecksMetadata {
    ...
    /// @param checks The DB containing all checks.
    function tokenURI(uint256 tokenId, IChecks.Checks storage checks)
        public
        view
        ...
        return
            abi.encodePacked(
                showVisualAttributes
                    ? trait(
                        "Color Band",
                        colorBand(ChecksArt.colorBandIndex(check, check.stored.divisorIndex)),
                        ","
                    )
                    : "",
                showVisualAttributes
                    ? trait(
                        "Gradient",
                        gradients(ChecksArt.gradientIndex(check, check.stored.divisorIndex)),
                        ","
                    )
                    : "",
                showAnimationAttributes
                    ? trait(
                        "Speed",
                        check.speed == 4 ? "2x" : check.speed == 2 ? "1x" : "0.5x",
                        ","
                    )
                    : "",
                showAnimationAttributes
                    ...
                    : "",
                check.isRevealed == false ? trait("Revealed", "No", ",") : "",
...
library Utils {
    ...
    function seed(uint256 nonce) public view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(msg.sender, block.coinbase, nonce)));
    }
