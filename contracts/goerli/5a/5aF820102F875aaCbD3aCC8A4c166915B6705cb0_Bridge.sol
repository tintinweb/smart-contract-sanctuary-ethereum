...
contract Bridge is ERC721A, Ownable {
    ...
    MINT_STAGE public mintStage;

    uint256[] public discountQuantities;
    address[] public mintingDistributionRecipients;
    address[] public nftHolders;

    event UpdateMaxPerWallet(uint256 oldMax, uint256 newMax);
    event UpdatedNftPrice(uint256 oldPrice, uint256 newPrice);
    event UpdatedMerkleRoot(bytes32 oldRoot, bytes32 newRoot);
    event AddedDistributionRecipient(address recipient, uint256 percentage);
    event RemovedDistributionRecipient(address recipient);
    event UpdatedTokenURI(string oldURI, string newURI);
    event PublicSaleActivated();
    event UpdatedMaxPreSale(uint256 oldMax, uint256 newMax);
    ...
    constructor(
        //Position 0 - BUSD address; Position 1 - bridge Factory Address
        address[] memory _criticalAddresses,
        //Position 0 - is pre sale; Position 1 - is easy transfer
        bool[] memory _booleans,
        //Position 0 - token name; Position 1 - token symbol; 
        ...
        if(_booleans[0]){
            mintStage = MINT_STAGE.PRESALE;
        }else{
            mintStage = MINT_STAGE.PUBLIC;
        }

        BUSD = _criticalAddresses[0];
        BridgeFactoryAddress = _criticalAddresses[1];
        ...
        //Opitmize by using local variable in for loop
    ...
    function preMint(uint256 _quantity, uint256 _price, bytes32[] calldata _merkleProof, uint256 _companyID, uint256 _projectId) external {
        require(mintStage == MINT_STAGE.PRESALE, "Not in presale");
        ...
        _mint(_quantity, _price, _companyID, _projectId);
        
    }

    function publicMint(uint256 _quantity, uint256 _price, uint256 _companyID, uint256 _projectId) external {
        require(mintStage == MINT_STAGE.PUBLIC, "Not in public mint");
    ...
    function addAddressToMintingProceeds(
        ...
        emit AddedDistributionRecipient(_newRecipient, _newPercentage);
    }
    ...
    function removeAddressFromMintingProceeds(
        ...
        emit RemovedDistributionRecipient(_removedAddress);
    }
    ...
    function setPreSale(uint256 _newMax) external onlyOwner {
        uint256 oldMax = maxPreSale;
        maxPreSale = _newMax;

        emit UpdatedMaxPreSale(oldMax, _newMax);
    }
    ...
    function getDiscountedPrice(uint256 _quantity) public view returns (uint256) {
        uint256 discountQuantitesLength = discountQuantities.length;
        uint256 discountQuantityIndex;
        uint256 discountPercentage;

        for (uint256 x = 0; x < discountQuantitesLength; x++) {
            if (_quantity < discountQuantities[0]) {
                discountPercentage = 0;
                break;
            } else if (_quantity < discountQuantities[x]) {
                discountQuantityIndex = discountQuantities[x - 1];
                discountPercentage = discountQuantityToPercentage[discountQuantityIndex];
                break;
            } else if (_quantity >= discountQuantities[discountQuantitesLength - 1]) {
                discountQuantityIndex = discountQuantities[discountQuantitesLength - 1];
                discountPercentage = discountQuantityToPercentage[discountQuantityIndex];
                break;
            }
        }
    ...
    function _mint(uint256 _quantity, uint256 _price, uint256 _companyID, uint256 _projectId) internal {

        IBridgeFactory factory = IBridgeFactory(BridgeFactoryAddress);
        ...
        //Sending of actually minting funds
        for(uint256 x = 0; x < mintingDistributionRecipientsLength; x++){
        ...
        //Need to add the number of NFTs sold and value to the company mapping
        factory.addCompanyNFTsSold(_companyID, _quantity, _projectId);
        factory.addTotalRevueToCompany(_companyID, _price, _projectId);
...
interface IBridgeFactory {
    ...
    function addCompanyNFTsSold(uint256 _companyID, uint256 _quantity, uint256 _projectId) external;
