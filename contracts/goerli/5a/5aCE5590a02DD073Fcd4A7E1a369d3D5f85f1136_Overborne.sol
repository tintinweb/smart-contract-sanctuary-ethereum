...
contract Overborne is ERC721A, Ownable {
    ...
    function whitelistMerkleMint(bytes32[] calldata _merkleProof, uint256 _numberOfMints) external payable verifySupply(_numberOfMints) {
        require(mintStatus == MintStatus.WHITELIST, "Whitelist closed.");
        require(msg.value >= priceWhitelist * _numberOfMints, "Incorrect ether" );
        require(_numberMinted(msg.sender) + _numberOfMints <= maxMintsPerPersonWhitelist, "Exceeds max");
    ...
    function publicMint(uint256 _numberOfMints) external payable verifySupply(_numberOfMints) {
        ...
        require(_numberMinted(msg.sender) + _numberOfMints <= maxMintsPerPersonPublic, "Exceeds max");
    ...
    string private baseUnrevealedURI;
    string private baseRevealedURI;

    function _baseURI() internal view virtual override returns (string memory) {
        return revealEnabled ? baseRevealedURI : baseUnrevealedURI;
    }
    ...
    function setBaseUnrevealedURI(string memory _uri) external onlyOwner {
        baseUnrevealedURI = _uri;
    }

    function setBaseRevealedURI(string memory _uri) external onlyOwner {
        baseRevealedURI = _uri;
    }
