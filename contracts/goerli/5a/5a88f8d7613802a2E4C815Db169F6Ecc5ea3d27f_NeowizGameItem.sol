    ...
    OwnableUpgradeable
{
    ...
    modifier onlyAllowed(address _launchpad) {
        if (!allowed[_launchpad]) revert NotAllowedLaunchpad();
        _;
    }
    ...
    function migrate(address _launchpad, uint256 _tokenId)
        external
        override
        onlyAllowed(_launchpad)
    {
        ...
        IERC721Metadata(_launchpad).transferFrom(
            tokenOwner,
            address(this),
            _tokenId
        );
    ...
    function lock(uint256 _tokenId)
        external
        override
        onlyOwnerOrTokenOwner(_tokenId)
    {
    ...
    function _beforeTokenTransfer(
        ...
        if (locked[tokenId]) revert Locked();
