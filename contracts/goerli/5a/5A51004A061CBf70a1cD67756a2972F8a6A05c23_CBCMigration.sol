...
contract CBCMigration is Ownable {
    ...
    IERC20 public newCBC;

    /// @notice old cbc token to be migrated
    IERC20 public oldCBC;
    ...
    /// migration event
    event Migration(address indexed recepient, uint256 indexed amount, uint256 swappedAt);
     ...
     * @param _expirationTime time we don't want to accept the signature
    ...
    function migrate(
        ...
        require(
            ...
            "migrate: User cannot exceeed allowed swap amount"
        );
        require(msg.sender != address(0), "migrate: Caller cannot be zero address!");
        ...
        require( _expirationTime >= block.timestamp, "migrate: This transaction signature is expired");

        // ensure that the sender is allowed by the validator
        address recovered = ECDSA.recover(
            keccak256(
                abi.encodePacked(_amount, _totalAmount, _nonce, msg.sender, address(this), _expirationTime)
            ),
            _signature
        );
        ...
        require(
            ...
            "User didn't approve enough amount to transfer on old CBC contract"
        );

        require(
            newCBC.allowance(adminWallet, address(this)) >= _amount,
            "Admin didn't approve enough amount to transfer on new CBC contract"
        );

        require( oldCBC.transferFrom(msg.sender, burnWallet, _amount), "Token Transfer failed");

        newCBC.transferFrom(adminWallet, msg.sender, _amount);
        nonceToValidated[_nonce] = true;
        swapPerAddress[msg.sender] += _amount;

        emit Migration(msg.sender, _amount, block.timestamp);
    }
