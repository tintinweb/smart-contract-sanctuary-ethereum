...
contract Krakatoa is Ownable, Additional {
    ...
    function processOrdersByAdmin(uint64[] calldata orders, bool[] calldata isWithdraw, bytes calldata signature) external onlyAdmin {
        ...
        for (uint8 i = 0; i < orders.length; ++i) {
            if(isWithdraw[i]) {
                _withdrawOrder(orders[i], true);
            } else {
                _refundOrder(orders[i], true);
            }
        }
    }
    ...
    function _refundOrder(uint64 orderId, bool skipRecipient) internal {
        Order memory r = _orders[orderId];

        require(r.status == Status.Pending, "KR: Order already withdrawed");
         require(skipRecipient || r.buyer == _msgSender(), "KR: Recipient of order is not buyer");
    ...
    function _withdrawOrder(uint64 orderId, bool skipRecipient) internal {
        Order memory r = _orders[orderId];

        require(r.status == Status.Pending || r.status == Status.WithdrawnFee, "KR: Order already processed");
        require(skipRecipient || r.seller == _msgSender(), "KR: Recipient of order is not seller");
