...
contract MasterPool is IMasterPool, Ownable {
    ...
    uint256 constant PERCENT_FOR_AUCTION = 90; // 97%
    uint256 constant PERCENT_FOR_BUYBAK = 5; // 1%
    uint256 constant PERCENT_FOR_DEV = 5; // 1%
    uint256 constant PERCENT_FOR_LIQUIDITY = 0; // 1%

    mapping(uint256 => StakingPool) public stakingPools;
    mapping(uint256 => uint256) public totalAuctioned;
    mapping(address => uint256) public lastAuctionIds;
    mapping(uint256 => uint256) public totalAuctionStaked;
    mapping(address => uint256) public lastClaimDividensIds;
    mapping(address => PendingRewards) public pendingRewards;
    mapping(uint256 => mapping(address => AuctionInfo)) public auctionInfos;
    mapping(uint256 => address[]) public auctionStakers;
    ...
    function auction(uint256 auctionAmount_, uint256 stakeDuration_)
        external
        lock
    {
        ...
        require(
            stakeDuration_ <= 1000 && stakeDuration_ % STAKE_MIN_PERIOD == 0,
    ...
    function claimAuctionRewards(uint256 auctionId_) external {
        address sender = msg.sender;
        ...
        AuctionInfo storage auctionInfo = auctionInfos[auctionId_][sender];
        ...
        uint256 rewards = totalAuctionedAmount == 0
            ? 0
            : (supplyAmount * auctionedAmount) / totalAuctionedAmount;

        require(rewards > 0, "no rewards");
        auctionInfo.claimed = true;
        auctionInfo.auctionAmount = 0;
        tokenX.mint(address(this), rewards);
        _stake(sender, rewards, auctionInfo.stakeDuration);
    }

    function getAuctionReward(uint256 auctionId_)
        public
        view
        returns (uint256)
    {
    ...
    function claimableDividendsRewards(address account_)
        public
        view
        returns (uint256)
    {
        ...
        uint256 rewards = 0;
        lastAuctionId = auctionId == lastAuctionId
            ? lastAuctionId - 1
            : lastAuctionId;
    ...
    function stake(uint256 amount_, uint256 duration_) external {
        ...
        IERC20(address(tokenX)).safeTransferFrom(
            sender,
            address(this),
            amount_
        );
    ...
    function unstake(uint256 stakingId_) external {
    ...
    function getUserStakingState(address user_)
        external
        view
        returns (
            uint256 userBalance,
        ...
        )
    {
        ...
        if (totalAmount == 0) {
            ...
            for (uint256 i = 0; i < stakingId; i++) {
                if (
                    stakingPools[i].staker == user_ &&
                    stakingPools[i].claimed == false
                ) {
                    StakingPool memory stakingPool = stakingPools[i];
                    uint256 leftDuration = (block.timestamp -
                        stakingPool.stakeTimestamp);
                    leftDuration = leftDuration / 1 days >=
                        stakingPool.stakeDuration
                        ? stakingPool.stakeDuration
                        : leftDuration / 1 days;
    ...
    function getAuctionRewardsState(address user_)
        external
        view
        returns (
    ...
    function getGlobalAuctionState()
        external
        view
        returns (
            uint256 auctionDay,
        ...
        )
    {
        ...
        if (auctionId > 99) {
            ...
            curAuctionPrice = todayTokenXPool == 0
                ? 0
                : (todayBUSDPool * 1e18) / todayTokenXPool;
            totalBUSDDividens = auctionId == 0
                ? 0
                : (totalAuctioned[auctionId - 1] * PERCENT_FOR_AUCTION) / 100;
        }
    }

    function getUserAuctionState(address user_)
        external
        view
        returns (
        ...
        )
    {
        ...
        myAuctionBidToday = auctionId > 99
            ? 0
            : userInfos[account].bidAmountPerAuction[auctionId];
        ...
        auctionHistory = new AuctionHistory[](
            auctionId == 100 ? 100 : auctionId
        );
        for (uint256 i = 0; i < auctionId; i++) {
            uint256 totalAmount = totalAuctioned[i];
            uint256 tokenXPool = tokenX.supplyAmount(i);
            uint256 auctionPrice = tokenXPool == 0
                ? 0
                : (totalAmount * 1e18) / tokenXPool;
            auctionHistory[i] = AuctionHistory({
                auctionId: i,
                tokenXPool: tokenXPool,
                totalBUSDDividens: i == 0
                    ? 0
                    : (totalAuctioned[i - 1] * PERCENT_FOR_AUCTION) / 100,
    ...
    function _unstake(address account_, uint256 stakingId_) internal {
        StakingPool storage pool = stakingPools[stakingId_];
        // uint256 rewards = (pool.stakeDuration / STAKE_MIN_PERIOD) *
        //     pool.stakeAmount;
        uint256 rewards = pool.stakeAmount * 10; // 10X
    ...
    function _getClaimableDividendsBUSD(address account_, uint256 auctionId_)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 totalAmount = totalAuctioned[auctionId_];
        if (totalAmount == 0) {
            return 0;
        }
    ...
    function _calcRewards(address account_, uint256 auctionId_)
        internal
        view
    ...
    function _getTotalAuctionStakedAmount(uint256 auctionId_)
        internal
        view
        returns (uint256)
    {
    ...
    function _actionForTokenX(uint256 auctionAmount_) internal {
    ...
    function _addliquidity(uint256 auctionAmount_) internal {
        uint256 amountForLiquidity = (auctionAmount_ * PERCENT_FOR_LIQUIDITY) /
            100;
    ...
    function _buyBack(uint256 auctionAmount_) internal {
        uint256 amountForBuyBack = (auctionAmount_ * PERCENT_FOR_BUYBAK) / 100;
