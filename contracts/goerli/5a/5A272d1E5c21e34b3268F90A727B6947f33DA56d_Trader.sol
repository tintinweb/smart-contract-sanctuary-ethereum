...
contract HashflowRouterMock is IHashflowQuote {
	...
	function tradeSingleHop (RFQTQuote calldata quote) public payable {
    	...
    	(bool success, ) = maker.call{value: quote.effectiveBaseTokenAmount}("");
			require(success, "native baseToken transfer failed");
		}
	...
	function tradeXChain (
		...
		IERC20(quote.baseToken).safeTransferFrom(quote.trader, quote.srcPool, quote.baseTokenAmount);
	}
...
import {IHashflowQuote} from "../mocks/HashflowRouterMock.sol";
import {Sweepable} from "../utilities/Sweepable.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract Trader is IHashflowQuote, Ownable, Sweepable {
	using SafeERC20 for IERC20;
	...
	function tradeXChain(
		...
		if (quote.baseToken != address(0)) {
			// this is an ERC20 transfer
			IERC20(quote.baseToken).safeTransferFrom(msg.sender, address(this), fee);
		} else {
			// this is a native token transfer
