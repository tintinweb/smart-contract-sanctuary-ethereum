  ...
  modifier CalibVendMan(){
    ...
    function createTool(uint256 _assetTag, string memory _Type, address _vendor, bool _status, uint256 _lastCalibDate, uint256 _nextCalibDate) public CalibMang returns (bool){
        for(uint i = 0 ; i<TMDE.length ; i++){
            require(_assetTag != TMDE[i].assetTag, "Tool registered already!");
        }
    ...
    function updateTool(uint256 _assetTag, string memory _Type, bool _status, uint256 _lastCalibDate, uint256 _nextCalibDate) public CalibMang returns (bool){
        address v;
        for(uint i = 0 ; i<TMDE.length ; i++){
...
//For Calibration and manager to view all tmde
    ...
    mapping (uint256 => tmde) public tagToTool;
