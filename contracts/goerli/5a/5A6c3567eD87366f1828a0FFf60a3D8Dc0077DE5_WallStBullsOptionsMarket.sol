...
contract WallStBullsOptionsMarket is ERC721Holder, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    VRFConsumerBaseV2(0x2Ca8E0C643bDe4C2E08ab1fA0da3401AdAD7734D) // Goerli
    ...
    function option(uint256 _tokenId) external returns (uint256 requestId) {
        ...
        return _option(msg.sender, _tokenId);
    }

    function _option(address _sender, uint256 _tokenId) private returns (uint256 requestId) {
        ...
        emit RequestSent(requestId, chainLinkConfig.numWords);
        return requestId;
    }

    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        require(requests[_requestId].exists, 'request not found');
        uint256 roll = (_randomWords[0] % 100) + 1;
        requests[_requestId].fulfilled = true;
        requests[_requestId].randomWords = _randomWords;
        requests[_requestId].roll = roll;

        RequestStatus memory request = requests[_requestId];
        State state = _rollToState(_roll(request.tokenId));

        if (state == State.DEAD) {
            erc721.transferFrom(address(this), graveyardWallet, request.tokenId);
        } else if (state == State.PRISON) {
            erc721.transferFrom(address(this), prisonWallet, request.tokenId);
        } else {
            erc721.transferFrom(address(this), request.sender, request.tokenId);
        }

        _setTokenState(request.tokenId, request.sender, state);
    ...
    function getRequestStatus(uint256 _requestId) external view returns (
        ...
        require(requests[_requestId].exists, 'request not found');
        RequestStatus memory request = requests[_requestId];
        return (request.fulfilled, request.randomWords, request.roll);
    }

    function undo(uint256 _tokenId) external onlyOwner {
