...
contract Gamble is IERC20, IERC20Metadata, AccessControl {
    ...
    event PotTransferredToWinnerOnBuy(address indexed receiver, uint256 amount);
    event PotTransferredToWinnerOnSell(address indexed receiver, uint256 amount);
    event PotTransferreredToWinnerOnTransfer(address indexed receiver, uint256 amount);
        ...
        if (_pot > 0 && pair != address(0)) {
            ...
            // } else if (num % _threshold2 == 0) {
               ...
               **/
                ...
                _transferPotToWinner(sender, recipient);
    ...
    function _transferPotToWinner(address sender, address receiver) internal {
        ...
        if (sender == pair) {
            ...
            emit PotTransferredToWinnerOnBuy(receiver, potAmount);
        } else if (receiver == pair) {
            _balances[sender] += potAmount;
            emit PotTransferredToWinnerOnSell(sender, potAmount);
        } else {
            _balances[sender] += potAmount;
            emit PotTransferreredToWinnerOnTransfer(sender, potAmount);
        }
