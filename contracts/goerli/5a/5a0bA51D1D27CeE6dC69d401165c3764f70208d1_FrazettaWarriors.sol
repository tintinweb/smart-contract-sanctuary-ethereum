 ...
 *         registrant's entries in the OperatorFilterRegistry.
 */
abstract contract OperatorFiltererV2 {
    error OperatorNotAllowed(address operator);
...
import {RevokableOperatorFiltererV2} from "./RevokableOperatorFiltererV2.sol";
import {OperatorFilterer} from "./OperatorFilterer.sol";
import {OperatorFiltererV2} from "./OperatorFiltererV2.sol";
...
/**
 * @title  ConfigurableOperatorFilterer
 * @notice Inherits from OperatorFilterer and subscribes to configured registry and subscription.
 */
abstract contract RevokableConfiguredOperatorFilterer is RevokableOperatorFiltererV2 {
    constructor(address registry, address subscription) OperatorFiltererV2(registry, subscription, true) {}
}
 ...
 *         isOperatorFilterRegistryRevoked flag in order to permanently bypass the OperatorFilterRegistry checks.
 */
abstract contract RevokableOperatorFiltererV2 is OperatorFiltererV2 {
    error OnlyOwner();
    error AlreadyRevoked();
...
contract ERC721Base is ERC721Core {
    ...
    constructor(
        ...
        uint256 _initialMaxTokens,
        uint256 _initialPriceWei,
        uint256 _initialReservedTokens,
        address _initialFilterRegistryAddress,
        address _initialFilterRegistrySubscription
    )
    ERC721Core(
        _initialName,
        _initialSymbol,
        _initialFilterRegistryAddress,
        _initialFilterRegistrySubscription
    ){
        MAX_TOKENS = _initialMaxTokens;
        PRICE = _initialPriceWei;
        RESERVED_TOKENS = _initialReservedTokens;
    ...
    function mintReserved(address _to, uint256 _tokenCount) external onlyOwner {
        require(totalMinted[msg.sender] + _tokenCount <= RESERVED_TOKENS, "All reserved tokens have been minted");
        _mintBatch(_to, _tokenCount);
    }

    function mint(uint256 _tokenCount) external payable {
        require(publicSaleActive, "Public sale is not active");
        require(PRICE * _tokenCount == msg.value, "ETH amount is incorrect");

        _mintBatch(msg.sender, _tokenCount);
    }

    function _mintBatch(address _to, uint256 _tokenCount) internal {
        uint256 totalSupply = totalSupply();
        require(totalSupply + _tokenCount <= MAX_TOKENS, "All tokens have been minted");
        require(_tokenCount > 0, "Must mint at least one token");

        for (uint256 i = 1; i <= _tokenCount; i++) {
            totalMinted[msg.sender] += 1;
            super._mint(_to, totalSupply + i);
        }
    }
}
...
import "../../opensea/RevokableDefaultOperatorFilterer.sol";
...
abstract contract ERC721Core is
    Ownable,
    ERC721EnumerableV3,
    RevokableConfiguredOperatorFilterer,
    ERC2981Setter
{
    constructor(
        string memory _initialName,
    ...
    ERC721(_initialName, _initialSymbol)
    RevokableConfiguredOperatorFilterer(_initialFilterRegistryAddress, _initialFilterRegistrySubscription) {}

    /**
     * @notice overrides supportInterface to support both ERC721Enumerable and ERC2981
     */
    ...
    /**
     * @notice overrides setApprovalForAll for OperatorFilterer that only allow use by approved operators from the
    ...
    /**
     * @notice overrides approve for OperatorFilterer that only allow use by approved operators from the
     *         configured OperatorFilterRegistry
     */
    ...
    /**
     * @notice overrides transferFrom for OperatorFilterer that only allow use by approved operators from the
     *         configured OperatorFilterRegistry
     */
    ...
    /**
     * @notice overrides safeTransferFrom for OperatorFilterer that only allow use by approved operators from the
     *         configured OperatorFilterRegistry
     */
    ...
    /**
     * @notice overrides safeTransferFrom for OperatorFilterer that only allow use by approved operators from
    ...
    /**
     * @notice overrides owner for RevokableOperatorFilterer that only allow use by approved operators from
     *         the configured OperatorFilterRegistry
     */
    function owner() public view virtual override (Ownable, RevokableOperatorFiltererV2) returns (address) {
...
contract FrazettaWarriors is ERC721Base {
    string private constant CONTRACT_NAME = "Frazetta Warriors";
    string private constant CONTRACT_SYMBOL = "FRAWAR";
    uint256 private constant INITIAL_MAX_TOKENS = 10000;
    uint256 private constant INITIAL_PRICE = 0.1 ether;
    uint256 private constant INITIAL_RESERVED_TOKENS = 100;
