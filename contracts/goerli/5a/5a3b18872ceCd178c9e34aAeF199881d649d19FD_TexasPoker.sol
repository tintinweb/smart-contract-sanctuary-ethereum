...
contract TexasPoker is Ownable, IERC721Receiver {
    ...
    function freePledgeNFT(address token, uint256 tokenId) public {
        require(tokenId > 0, "TokenId error");
    ...
    function freeRedeemNFT(address token, uint256 tokenId) public {
        require(userPledgeNFTInfos[msg.sender][token][tokenId], "TokenId error");
    ...
    function freeSettlement(address owner, address token, uint256 tokenId, address winner) public onlyOwner {
        require(userPledgeNFTInfos[owner][token][tokenId], "TokenId error");
        require(winner != address(0), "winner error");

        delete userPledgeNFTInfos[owner][token][tokenId];
        ERC721(token).safeTransferFrom(address(this), winner, tokenId);
