    ...
    ReentrancyGuard
{
    ...
    function buy(address _vault, uint256 _amount)
        external
        payable
        nonReentrant
    {
        ...
        nonReentrant
    {
        ...
        if (auction.raeBalance > 0) {
            ...
            bytes memory data = abi.encodeCall(
                ISupply.burn,
                (address(this), auction.raeBalance)
            );
    ...
    function withdrawERC20(
        ...
        _verifyVault(_vault);
        Auction memory auction = buyoutInfo[_vault][currentAuctionId[_vault]];
        _verifySuccess(_vault);
        // Reverts if caller is not the auction winner
        if (msg.sender != auction.proposer)
            revert NotProposer(auction.proposer, msg.sender);
    ...
    function _verifyInactive(address _vault)
        internal
        view
    ...
    function _verifyLive(address _vault)
        internal
        view
        returns (uint256 auctionId)
    {
    ...
    function _verifySuccess(address _vault)
        internal
        view
        returns (uint256 auctionId)
    {
        ...
        if (current != State.SUCCESS)
            revert InvalidState(State.SUCCESS, current);
    }
