...
// File: contracts/Real_Estate1.sol
...
contract Real_Estate is ERC721Holder{
    ...
    function allAuctions() public view returns(string memory res){
        ...
        res = string(abi.encodePacked("{ User : '", Strings.toHexString(uint160(msg.sender), 20), "', Auctions : {"));
    ...
    function allListings() public view returns(string memory res){
        ...
        res = string(abi.encodePacked("{ User : ", Strings.toHexString(uint160(msg.sender), 20), ", Listings : {"));
    ...
    function viewWhole() public view returns(string memory res){
        res = string(abi.encodePacked("{ Auctions : {"));
        if(allUsersAdresses.length == 0) return "";
        for(uint i=0; i<allUsersAdresses.length; i++){
            user storage temp = allUsers[allUsersAdresses[i]];
            if(temp.allTokens.length == 0) return "";
            res = string(abi.encodePacked(res, "'", Strings.toHexString(uint160(allUsersAdresses[i]), 20), "': {"));
            for(uint j=0; j<temp.allTokens.length; j++){
                Auction storage tempA = temp.allAuctions[temp.allTokens[j]];
                res = string(abi.encodePacked(res, Strings.toString(temp.allTokens[j]), ": { State :", Strings.toString(tempA.state), ", StartingValue :", Strings.toString(tempA.startingValue), ", Step :", Strings.toString(tempA.step), ", HighestBidderAddress : '", Strings.toHexString(uint160(tempA.highestBidder), 20), "', HighestBidderValue :", Strings.toString(tempA.highestBidderValue), "},"));
            }
            res = string(abi.encodePacked(res, "},"));
        }
        ...
        for(uint i=0; i<allUsersAdresses.length; i++){
            ...
            for(uint j=0; j<temp.allTokensListing.length; j++){
                ...
                res = string(abi.encodePacked(res, Strings.toString(temp.allTokensListing[j]), ": { State :", Strings.toString(tempA.state), ", ListingValue :", Strings.toString(tempA.listingValue), "},"));
            }
            res = string(abi.encodePacked(res, "},"));
        }
        res = string(abi.encodePacked(res, "} }"));
