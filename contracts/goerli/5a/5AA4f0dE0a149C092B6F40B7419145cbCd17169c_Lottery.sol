 ...
 * @dev This implements the Chainlink VRF Version 2. Money are deposited to Aave. Finish of Lottery is done via Chainlink keeper
 * @dev To make contract upgradeable, we should get rid of immutable variables, as there is no constructor
 */
contract Lottery is
    Initializable,
    ...
    KeeperCompatibleInterface
{
    ...
    uint16 private s_request_confirmations;
    uint32 private s_num_words;

    // Aave Pool
    IPool private s_aavePool;
    IWeth private s_wethContract;
    ...
    function __Lottery_init_unchanged(
        uint256 _entranceFee,
        uint256 _interval,
        address _priceFeedAddress,
        address _vrfCoordinator,
        ...
        s_aavePool = IPool(_aavePoolAddress);
        s_wethContract = IWeth(_wethContractAddress);
        s_request_confirmations = 3;
        s_num_words = 1;
    }
    ...
    function enterLottery() public payable lotteryOpen {
        ...
        s_wethContract.deposit{value: msg.value}();
        ...
        )
    {
        ...
        bool hasPrizes = s_prizePool > 0;
        upkeepNeeded = (timePassed && isOpen && hasPlayers && hasPrizes);
    ...
    function emrgStopLottery() public onlyOwner {
        ...
        uint256 wethBalance = s_wethContract.balanceOf(address(this));
    ...
    /**
     * @dev Contract is called by VRFConsumerBaseV2Upgradable and provide Random words
     * @inheritdoc VRFConsumerBaseV2Upgradable
     */
    ...
    function supplyFundsAave(uint256 amount) internal {
        s_wethContract.approve(address(s_aavePool), amount);
        s_aavePool.supply(address(s_wethContract), amount, address(this), 0);
    }

    function withdrawAaveFunds(uint256 amount) internal {
        s_wethContract.approve(address(s_aavePool), amount);
        s_aavePool.withdraw(address(s_wethContract), amount, address(this));
    }

    function changeToEth(uint256 amount) internal {
        s_wethContract.approve(address(this), amount);
        s_wethContract.withdraw(amount);
    }
    ...
    function getEntranceFee() public view returns (uint256) {
        ...
        (, int256 conversionRate, , , ) = s_priceFeed.latestRoundData();
    ...
    function getTimeToNextFinish() public view returns (uint256) {
        ...
        returns (uint256, uint256)
    {
        ...
        ) = s_aavePool.getUserAccountData(address(this));
