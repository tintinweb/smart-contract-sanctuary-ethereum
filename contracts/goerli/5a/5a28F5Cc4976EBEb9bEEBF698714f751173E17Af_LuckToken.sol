...
import './ILuckTokenInterface.sol';

import './LuckTradeToken.sol';

contract LuckToken is LuckTradeToken {
    using SafeMath for uint256;

    /**
     * @notice Constructs the Basis Cash ERC-20 contract.
     */
    constructor(address _usdt, address _routerV2)
        public
        BEP20('LuckDao Token', 'LUCK')
        LuckTradeToken(_usdt, _routerV2)
    {}
    ...
    function _mintOne(address recipient_, uint256 amount_)
        internal
        returns (bool)
    {
        uint256 balanceBefore = balanceOf(recipient_);
    ...
    function mintArray(
        address[] memory recipientArray_,
        uint256[] memory amountArray_
    ) public onlyOperator {
        require(recipientArray_.length == amountArray_.length, 'length not equals');

        for (uint112 i = 0; i < recipientArray_.length; i++) {
            _mintOne(recipientArray_[i], amountArray_[i]);
        }
    }
}
...
interface ILuckTokenInterface is IBEP20
{
    function getAddressTradeSum(address sender) external view returns (uint256 buyAmount,uint256 sellAmount,uint256 totalAmount);
}
...
abstract contract LuckTradeToken is ListenTradeToken {
    using SafeMath for uint256;
    ...
    constructor(address usdt_, address routerv2) public {
        pancakeRouter = IPancakeRouter02(routerv2);
        USDT = IBEP20(usdt_);

        _createTokenETHTPair();
        _createTokenUSDTPair();
    }

    function changeOpenTrade(bool value) external onlyOwner {
        openTrade = value;
    }

    function changeEnableWhitelist(bool value) external onlyOwner {
    ...
    function addWhitelistArray(address[] calldata users) external onlyOwner {
        for (uint112 i = 0; i < users.length; i++) {
    ...
    function _createTokenUSDTPair() internal virtual {
        address _usdtTokenPair = IPancakeFactory(pancakeRouter.factory())
            .createPair(address(USDT), address(this));
        _changeHookAddress(_usdtTokenPair, true);
    }

    function _createTokenETHTPair() internal virtual {
        address _ethTokenPair = IPancakeFactory(pancakeRouter.factory())
            .createPair(pancakeRouter.WETH(), address(this));
        _changeHookAddress(_ethTokenPair, true);
    }
}
...
abstract contract ListenTradeToken is Operator, BEP20 {
    ...
    mapping(address => bool) listenAddress;
    ...
    function changeOpenListen(bool value) onlyOwner external
    {
        openListen = value;
    }
