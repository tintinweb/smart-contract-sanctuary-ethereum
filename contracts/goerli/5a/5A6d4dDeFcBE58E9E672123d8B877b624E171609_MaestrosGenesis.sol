...
contract MaestrosGenesis is ERC721, ERC2981, Ownable {
    ...
    modifier whenNotComposing(uint256 _token) {
     ...
     */
    function compose(uint256 _token) external whenNotComposing(_token) {
    ...
    function getShuffledTokenId(uint256 tokenId) public view returns (uint256) {
        if (tokenId < offsetMinId || tokenId > offsetMaxId) return tokenId;

        uint256 newId = tokenId + offset;
    ...
    function totalSupply() external view returns (uint256) {
        // when none minted: (1 - 1) + (1159 - 1159) + (1307 - 1307) = 0
        // when all minted: (1159 - 1) + (1307 - 1159) + (1556 - 1307) = 1555
        return (nextTicket - 1) + (nextTreasury - 1307);
    }
    ...
    function notesBalance(uint256 _token) external view returns (uint256) {
        uint256 composeTime = tokenToComposeTime[_token];
        uint256 duration = composeTime > 0 ? block.timestamp - composeTime : 0;
        return duration;
    }
