...
contract Activator1 is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    struct Account {
    ...
    function deposit(uint256 amount) external {
        TokenUtils.safeApprove(syntheticToken, address(this), amount);
        _updateAccount(
            UpdateAccount({
                user: msg.sender,
    ...
    function getUserData(address user) external view returns (uint256, uint256) {
        Account storage _account = accounts[user];
