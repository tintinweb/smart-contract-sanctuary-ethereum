...
contract Primes is ERC721r, Ownable {
    ...
    mapping(uint256 => Definitions) public Paths;
    mapping(uint256 => Attributes) public Stats;
    ...
    function getD(uint256 _position) internal view returns (string memory d) {
        d = Paths[_position].d;
    }
    ...
    function tokenURI(
        ...
        // string memory numerator = getNumerator(statsKey);
        // string memory percentage = getPercentage(statsKey);
        return
            string(
                abi.encodePacked(
        ...
        for (y = 0; y <= 800; y += 100) {
            for (x = 0; x <= 800; x += 100) {
                ...
                if (isPrime(uint256(count))) {
                    string memory d = getD(index);
                    string memory values = getValues(index);
