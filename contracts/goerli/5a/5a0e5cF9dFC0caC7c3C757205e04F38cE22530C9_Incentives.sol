...
contract Incentives is Boosting, Coordination, IIncentives {
        ...
        returns (uint)
    {
        ...
        return uint(SignedMath.min( 
            int(boostMax), 
            int(boost[token].taker * amount) / int(IState(state).B() + amount) / _price 
            ));
    }
        ...
        returns (uint)
    {
        ...
        return uint(SignedMath.min( 
            int(boostMax),
            int(boost[token].maker * amount) / int(IState(state).D() + amount))
            );
    }
        ...
        returns (uint)
    {
        ...
        return uint(SignedMath.min( 
                int(incentiveMax), 
                int(calculateCoordination() * amount) / int(IState(state).B() + amount) / _price 
            )
        );        
    }
        ...
        returns (uint)
    {
        ...
        return uint(SignedMath.min( 
                int(incentiveMax), 
                int(calculateCoordination() * amount) / int(IState(state).D() + amount)
            )
        );
    }
}
...
abstract contract Coordination is IncentivesBase {
    ...
    uint public incentiveMax;

    function setIncentiveConfig(uint _incentiveMax, uint _scale, uint _inflection)
        external
        onlyGovernance
    {
        incentiveMax = _incentiveMax;
        thetaScale = _scale;
...
abstract contract Boosting is IncentivesBase {
    uint public boostMax;
    uint public thetaBoost;
    ...
    function setBoostConfig(uint _boostMax, uint _thetaBoost)
        external
        onlyGovernance
    {
        boostMax = _boostMax;
