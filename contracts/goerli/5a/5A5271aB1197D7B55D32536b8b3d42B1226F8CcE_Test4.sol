...
contract Test4 is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public tmCost = 0.1 ether;
  uint256 public maxSupply = 10000;
  uint256 public MaxperWallet = 2;
  uint256 public MaxperWalletTM = 2;
  ...
  function publicSaleMint(uint256 tokens) public payable nonReentrant {
    ...
    require(tokens <= MaxperTX, "max mint amount TX exceeded");
    ...
    if (!(_numberMinted(_msgSender()) >= 1 && (msg.value >= cost * tokens) ))
	revert InsufficientFunds(); 
    if (!(_numberMinted(_msgSender()) < 1 && (msg.value >= firstCost * tokens) ))
	revert InsufficientFunds();
