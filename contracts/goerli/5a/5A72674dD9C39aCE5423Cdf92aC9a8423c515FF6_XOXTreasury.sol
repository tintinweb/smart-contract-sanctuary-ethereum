...
contract XOXTreasury is Initializable {
    ...
    uint256 public blockStartFarming;
    ...
    function initialize(
        ...
        levelLength = 11;
        isActiveFarming = false;
    }

    /* ========== EVENTS ========== */
    event PreSale(address indexed buyer,address indexed ref,uint256 amountBuyer,uint256 amountRef);
    event SwapUSDToXOX(
    ...
    modifier onlyPreSale() {
        require(
            msg.sender == preSale,
            "XOX Treasury: Only XOX PreSale can call this function"
        );
        _;
    }
    ...
    function setXOXPreSale(address sale_) external onlyAdmin {
        preSale = sale_;
    }
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        uint256 blockDelta = user.lastBLockUpdated > blockStartFarming
            ? block.number.sub(user.lastBLockUpdated)
            : block.number.sub(blockStartFarming);
    ...
    ) external onlyPreSale returns (uint256) {
        UserInfo storage buyer = userInfo[from];
        uint256 amountBonus;
        uint256 amountRef;
        if (ref != address(0)) {
            ...
            reffer.amount = reffer.amount.add(amountRef);
            amountBonus = amount.mul(2).div(100);
        }
        amountBonus = amountBonus.add(amount.mul(rewardPercent).div(100));
        // logic update farming
        buyer.amount = buyer.amount.add(amountBonus);
        emit PreSale(from, ref, amountBonus, amountRef);
        return amountBonus;
    }
    ...
    function ActiveFarming() external {
        isActiveFarming = true;
        blockStartFarming = block.number;
    }
}
