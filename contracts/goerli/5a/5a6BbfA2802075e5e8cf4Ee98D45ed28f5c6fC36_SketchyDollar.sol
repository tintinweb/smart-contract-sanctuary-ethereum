...
contract SketchyDollar is ERC721A, Ownable, ReentrancyGuard, CantBeEvil(LicenseVersion.COMMERCIAL_NO_HATE) {
    ...
    constructor(address _project_wallet) ERC721A("Sketchy Dollar","SKTCHY") {

            myContractStateData.maxTokens =  5050;
            myContractStateData.allowListMintPrice =  0.05 ether;
            myContractStateData.publicMintPrice =  0.08 ether;
    ...
    modifier mintCheck(uint256 _mintAmount) {
        ...
        require(activeSaleStateInt >0, 'Allowlist mint not open');
    ...
    function allowlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCheck(_mintAmount) {
        uint256 activeSaleStateInt = uint(myContractStateData.salestate);
        require(activeSaleStateInt == 1, 'Allowlist mint not open');
        require(msg.value >= myContractStateData.allowListMintPrice * _mintAmount, 'Insufficient funds!');
    ...
    function publicMint(uint256 _mintAmount) public payable mintCheck(_mintAmount) {
        uint256 activeSaleStateInt = uint(myContractStateData.salestate);
        require(activeSaleStateInt == 2, 'Public mint not open');
