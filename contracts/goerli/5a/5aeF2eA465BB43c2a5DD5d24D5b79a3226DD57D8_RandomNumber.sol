...
contract RandomNumber is EIP712, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function _getRandomNumberInRange(string memory _tokenFromUniswap, uint point, uint _from, uint _to, bytes memory _signature) external view returns (uint)  {
      ...
      function _getRandomNumberWithNonceNumber(string memory _tokenFromUniswap, uint point, uint _from, uint _to, bytes memory _signature) external {
        require(_to > _from, "Range is not valid");
        require(validateAmountFunction(_tokenFromUniswap, _signature) == msg.sender, "Not invalid");
        uint randomNumber = uint(
            keccak256(
