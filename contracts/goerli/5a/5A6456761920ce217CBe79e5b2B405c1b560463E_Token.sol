...
contract Token is StartTokenIdHelper, ERC721ABurnable, Ownable, ReentrancyGuard {
    ...
    SaleState public state = SaleState.Locked;
    ...
    function addContract(address contract_) external onlyOwner {
    ...
    function configureSaleState(SaleState state_, uint256 price_, uint16 transactionLimit_) external onlyOwner {
        require(limitSupply != 0, "Token: Limit supply should be defined.");
        require(state_ != SaleState.Locked, "Token: Cannot lock sale.");

        if (state_ == SaleState.Presale) {
            require(contractAddress != address(0), "Token: Contract address should be defined.");
        }

        if (state != SaleState.Locked) {
            emit SalePaused(block.timestamp, state);
        }
        ...
        emit SaleStart(block.timestamp, state, price, transactionLimit);
    }

    function pauseAnySale() external onlyOwner {
        SaleState _state = state;
        state = SaleState.Locked;
        emit SalePaused(block.timestamp, _state);
    }
    ...
    function _preValidateMint(uint256 tokensAmount) internal {
        ...
        require(totalSupply() + tokensAmount <= limitSupply, "Token: Limited amount of tokens.");
    ...
    function mint(uint16 tokensAmount) external payable nonReentrant {
        require(state == SaleState.Sale, "Token: Sale is not active.");
    ...
    function presaleMint(uint256[] memory tokens_) external payable nonReentrant {
        require(state == SaleState.Presale, "Token: Presale is paused.");
