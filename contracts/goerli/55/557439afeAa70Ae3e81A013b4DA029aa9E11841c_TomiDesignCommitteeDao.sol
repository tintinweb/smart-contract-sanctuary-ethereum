...
contract TomiStates {
    ...
    /// @notice address of the committee contract
    IDesignDao public designDao;
    ...
    struct ProposalDesignCommitteeData {
        /// @notice Unique id for looking up a proposal
        uint256 id;
    ...
    mapping (uint256 => ProposalDesignCommitteeData) internal proposalsDesignCommitteeData;
...
contract TomiDesignCommitteeDao is TomiEvents, TomiStates, Ownable {
    using SafeMath for uint256;

    constructor(ITomiMainDao _tomiMainDao, address _treasury, IDesignDao _designDao) {
        tomiMainDao = _tomiMainDao;
        treasury = _treasury;
        designDao = _designDao;
    }
    ...
    function getProposalsDesignCommitteeData(uint256 proposalId) external view returns (ProposalDesignCommitteeData memory) {
        return proposalsDesignCommitteeData[proposalId];
    }
    ...
    function proposeChangeCommittee(
        ...
        require(7 <= functionId && functionId <= 8, "incorrect function id");
        ...
        ProposalDesignCommitteeData storage newProposalData = proposalsDesignCommitteeData[tomiMainDao.proposalCount()];
    ...
    function executeChangeCommittee(uint256 proposalId) external {
        Proposal storage proposal = proposals[proposalId];

        require(7 <= proposal.functionId && proposal.functionId <= 8, "incorrect function id");
        ...
        if (state(proposalId) == ProposalState.Succeeded) {
            ...
            address[] memory executionTargets = proposalsDesignCommitteeData[proposal.id].executionTargets;
            if (proposal.functionId == 7) {
                designDao.AddComitteMemberBatch(executionTargets);
            } else if (proposal.functionId == 8) {
                designDao.RemoveComitteMemberBatch(executionTargets);
            }
        }
    ...
    function testUpdateDesignDao(address _designDao) external {
        designDao = IDesignDao(_designDao);
    }
}
...
interface IDesignDao {

    function safeTransferFrom(address from, address to, uint256 tokenId) external;

    function AddComitteMemberBatch(address[] calldata _addresses) external;

    function addCommitteeMember(address _address) external;

    function RemoveComitteMemberBatch(address[] calldata _addresses) external;
