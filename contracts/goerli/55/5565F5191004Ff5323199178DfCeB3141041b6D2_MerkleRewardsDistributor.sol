...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    event MerkleProofCIDUpdated(bytes _newMerkleCDI);
    ...
    function setRoot(bytes32 _merkleRoot, bytes calldata _newMerkleCDI)
        public
        onlyOwner
    {
    ...
    function claim(
        ...
        claimedTokensPerUser[userAddress] = amountToClaim; // is it = or += ?
    ...
    function endEpoch(
        ...
        require(_etherAmount >= 0, "Ether is non-positive");
        if (_etherAmount > 0) {
        ...
        require(
            ...
            "tokensAttributed len should be equal to amountsAttributed len"
        );
    ...
    function swapMultiple(
        ...
        require(tokensToBeSwappedLen > 0 || msg.value > 0, "ETH or ERC-20 tokens should be present for the swap");
        if (msg.value > 0) {
