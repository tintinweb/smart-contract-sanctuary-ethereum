...
error PresaleRequired();
error NotPresale();
error NotEnoughPresaleSlots();
error ExceedsMaxPresaleMints();
error WrongPayment();
error InvalidMintSize();

contract FFCreatorV1A is ERC721A, ERC2981Collection, PaymentSplitter, Ownable {
    ...
    // @notice Presale functionality
    bool private presaleActive;
    bytes32 public presaleMerkleRoot;
    uint64 private maxPresaleMints;

    event UpdatedMintPrice(uint256 _prev, uint256 _new);
    event UpdatedMintSize(uint256 _prev, uint256 _new);
    event UpdatedMaxPresaleMints(uint256 _prev, uint256 _new);
    event UpdatedMaxPublicMints(uint256 _prev, uint256 _new);
    event UpdatedMintStatus(bool _prev, bool _new);
    event UpdatedRoyalties(address newRoyaltyAddress, uint256 newPercentage);
    event UpdatedPresaleStatus(bool _prev, bool _new);
    event UpdatedPresaleEnd(uint256 _prev, uint256 _new);
    event PayeesLocked(bool _status);
    event UpdatedPresale(bytes32 _prev, bytes32 _new);
    ...
    constructor(
        ...
        uint256 _mintPrice,
        uint256 _mintSize,
        uint64 _maxPresaleMints,
        uint64 _maxPublicMints,
        bool _multi,
        ...
        if (_maxPresaleMints == 0) {
            maxPresaleMints = 2;
        }

        if (_maxPublicMints == 0) {
            maxPublicMints = 10;
        }

        if (_presaleMerkleRoot != 0) {
        ...
        payable
    {
        ...
        uint64 numPresaleMinted = _getAux(_msgSender()) + amount;
        if (numPresaleMinted > maxPresaleMints) revert NotEnoughPresaleSlots();
        _safeMint(_msgSender(), amount);
        _setAux(_msgSender(), numPresaleMinted);
    }
    ...
    function publicMint(uint256 amount) external payable {
        if (presaleActive) revert PresaleRequired();
        if (!mintingActive) revert MintingNotActive();
        if (msg.value != mintPrice * amount) revert WrongPayment();
        if (totalSupply() + amount > mintSize) revert WouldExceedMintSize();
    ...
    // @notice this will enable presale or "if" in publicMint()
    function enablePresale() public onlyOwner {
        if (presaleActive) revert PresaleRequired();
        bool old = presaleActive;
        presaleActive = true;
        emit UpdatedPresaleStatus(old, presaleActive);
    }

    // @notice this will disable presale or "else" in publicMint()
    ...
    // @notice this will set a new Merkle root used to verify presale membership
    ...
    function setPresaleMerkleRoot(bytes32 _merkleRoot) public onlyOwner {
        bytes32 old = presaleMerkleRoot;
        presaleMerkleRoot = _merkleRoot;
        emit UpdatedPresale(old, presaleMerkleRoot);
    }

    // @notice this will set the maximum number of tokens a presale user can mint.
    // @param uint256 _amount - max amount of tokens
    function setMaxPresaleMints(uint64 _amount) public onlyOwner {
        uint256 oldAmount = maxPresaleMints;
        maxPresaleMints = _amount;
        emit UpdatedMaxPresaleMints(oldAmount, maxPresaleMints);
    }
    ...
    // @notice will return presale status of Minter
    ...
    // @notice will return presale status of Minter
    function presaleMaxMints() external view returns (uint256) {
        return maxPresaleMints;
    }
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    struct AddressData {
        ...
        // (e.g. number of presale mint slots used).
    ...
    /**
     * Returns the auxillary data for `owner`. (e.g. number of presale mint slots used).
     */
    ...
    /**
     * Sets the auxillary data for `owner`. (e.g. number of presale mint slots used).
     * If there are multiple variables, please pack them into a uint64.
     */
...
abstract contract Ownable is Context {
    ...
    function acceptOwner() public virtual {
        require(
            ...
            "New Owner: new Owner is the only caller"
        );
...
/* PAYMENT SPLITTER
 *
 * Functionality to force release to all addresses added as payees.
 */
...
/***
 * Interface for the ERC2981 token.
 * Zach Burks, James Morgan, Blaine Malone, James Seibel, "EIP-2981: NFT Royalty Standard,"
