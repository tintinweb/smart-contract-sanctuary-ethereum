...
/*
 * Auction V2 Prototype for cryptoWine project
 *
...
/*
 * cryptoWine Auction V2 contract, can own currency and wine NFTs.
 *
 * Preparation steps:
...
contract AuctionV2 is ERC165, AuctionV2DeployI, ReentrancyGuard {
    ...
    uint256 public amountScheduledToDistribute;
    address public constant eolGuard = address(1);
    address public highestBidder;
    address public lastPrepDistributionBidder;
    ...
    mapping(address => bytes32) public bidTaxRegionHash;
    mapping(address => uint256) public bidTimestamp;
    mapping(address => uint256) public bidTotalWei;  // 0 for non-crypto bids
    mapping(address => PaymentStatus) public bidPaymentStatus;
    mapping(address => uint256) public bidToDistributeAmount;
    ...
    event NewBid(address indexed bidder, uint256 amount, uint256 netWeiPerAsset, uint256 totalBidWei, string taxRegionIdentifier, PaymentStatus paymentStatus);
    event BidChanged(address indexed bidder, uint256 newTotalAmount, uint256 netWeiPerAsset, uint256 totalBidWei);
    event RequestExternalPayment(address indexed bidder);
    ...
    event PaymentStatusChanged(address indexed bidder, PaymentStatus indexed newPaymentStatus);
    ...
    requireActive
    {
        ...
        agriData = AuctionV2FactoryI(msg.sender).agriData();
    }
    ...
    /*** Auction-related properties / view functions ***/
    ...
    // Convert wei to EUR cent with rounding.
    function wei2EurcentRounded(uint256 _weiValue)
    internal view
    returns (uint256)
    {
        // Normally, we would multiply by 100 and divide by the rate.
        // Here, multiply by 1000 first and then decide to round up or down on the following division by 10 based on the modulo.
        uint256 baseConvertedValue = _weiValue * 1000 / eurRate();
        uint256 roundedDownEurcent = baseConvertedValue / 10;
        if (baseConvertedValue % 10 >= 5) {
            return roundedDownEurcent + 1;
        }
        return roundedDownEurcent;
    }
    ...
    returns (uint256)
    {
        require(assetToken().balanceOf(address(this)) <= amountScheduledToDistribute, "Price is not known yet");
        if (lastPrepDistributionBidder == eolGuard) {
        ...
        return bidNetWeiPerAsset[lastPrepDistributionBidder];
    }
    ...
    returns (uint256)
    {
        return wei2EurcentRounded(netSalePriceWei());
    }
    ...
    // Returns true if distribution preparation is finished.
    function readyForDistribution()
    public view
    requireActive
    returns (bool)
    {
        return startTimestamp > 0 && amountScheduledToDistribute >= assetToken().balanceOf(address(this));
    }
    ...
    requireNotFinished
    {
        ...
        _recordBid(msg.sender, _amount, msg.value, netWeiPerAsset, vatPermil, _taxRegionIdentifier, _candidateHigherBidder);
    }

    // Make a bid via the API, for some bidder.
    function bidViaApi(address _bidder, uint256 _amount, uint256 _bidTotalEurCent, string memory _taxRegionIdentifier, address _candidateHigherBidder)
    public
    onlyAPI
    requireActive
    requireStarted
    requireNotFinished
    {
        require(bidAssetAmount[_bidder] == 0, "You con only make one bid.");
        require(_amount > 0, "You need to bid on at least one asset.");
        require(_amount <= assetToken().balanceOf(address(this)), "You cannot bid on more assets than this auction sells.");
        if (_eurRate == 0) {
            // Rate gets fixed when the first bid is received.
        ...
        uint256 vatPermil = taxRegions().vatPermilForRegionId(_taxRegionIdentifier);
        require(vatPermil > 0, "The region is not supported.");
        uint256 virtualBidTotalWei = _bidTotalEurCent * _eurRate / 100;
        uint256 netWeiTotal = virtualBidTotalWei * 1000 / (1000 + vatPermil);
        // Compare the total net bid in rounded EUR cent so we have as little chance of rounding problems as possible.
        require(wei2EurcentRounded(netWeiTotal) >= minBidEurCent * _amount, "You need to bid more than the minimum bid plus VAT.");
        uint256 netWeiPerAsset = netWeiTotal / _amount;
        _recordBid(_bidder, _amount, 0, netWeiPerAsset, vatPermil, _taxRegionIdentifier, _candidateHigherBidder);
    }

    function _recordBid(address _bidder, uint256 _amount, uint256 _bidTotalWei, uint256 _netWeiPerAsset, uint256 _vatPermil, string memory _taxRegionIdentifier, address _candidateHigherBidder)
    internal
    {
        if (_candidateHigherBidder == address(0)) {
            // Compare to highest bidder.
            if (highestBidder != address(0) && hasHigherBid(highestBidder, _netWeiPerAsset)) {
                _candidateHigherBidder = highestBidder;
            }
        }
        else {
            require(hasHigherBid(_candidateHigherBidder, _netWeiPerAsset), "Candidate needs to actually have a higher bid.");
        }
        ...
        if (_candidateHigherBidder == address(0)) {
            ...
            while (hasHigherBid(nextLowerBidder[_candidateHigherBidder], _netWeiPerAsset)) {
        ...
        bidCount += 1;
        bidAssetAmount[_bidder] = _amount;
        bidNetWeiPerAsset[_bidder] = _netWeiPerAsset;
        bidVATPermil[_bidder] = _vatPermil;
        bidTimestamp[_bidder] = block.timestamp;
        bidTotalWei[_bidder] = _bidTotalWei;
        if (_bidTotalWei > 0) {  // On crypto payments, mark as successfully done right away.
            bidPaymentStatus[_bidder] = PaymentStatus.Success;
        }
        bidTaxRegionHash[_bidder] = taxRegions().getRegionHash(_taxRegionIdentifier);
        emit NewBid(_bidder, _amount, _netWeiPerAsset, _bidTotalWei, _taxRegionIdentifier, bidPaymentStatus[_bidder]);
    }
    ...
    // `_previousNextHigherBidder` ideally is the one exactly above this current bid previously, use the zero address if it's the highest bidder.
    ...
    requireNotFinished
    {
        require(bidAssetAmount[msg.sender] > 0 && bidTotalWei[msg.sender] > 0, "You need to have an active crypto bid.");
        ...
        uint256 newTotalWei = bidTotalWei[msg.sender] + msg.value;
        uint256 netWeiPerAsset = newTotalWei * 1000 / (1000 + bidVATPermil[msg.sender]) / _newTotalAmount;
        ...
        _recordAdjustedBid(msg.sender, _newTotalAmount, newTotalWei, netWeiPerAsset, _candidateHigherBidder, _previousNextHigherBidder);
    }

    // Make a bid via the API, for some bidder.
    function adjustBidViaApi(address _bidder, uint256 _newTotalAmount, uint256 _newBidTotalEurCent, address _candidateHigherBidder, address _previousNextHigherBidder)
    public
    onlyAPI
    requireActive
    requireStarted
    requireNotFinished
    {
        require(bidAssetAmount[_bidder] > 0 && bidTotalWei[_bidder] == 0, "You need to have an active non-crypto bid.");
        require(_newTotalAmount < assetToken().balanceOf(address(this)), "You cannot bid on more assets than this auction sells.");
        require(_newTotalAmount >= bidAssetAmount[_bidder], "You cannot reduce the amount of assets.");
        uint256 oldTotalWei = bidNetWeiPerAsset[_bidder] * bidAssetAmount[_bidder] * (1000 + bidVATPermil[_bidder]) / 1000;
        uint256 newTotalWei = _newBidTotalEurCent * eurRate() / 100;
        require(newTotalWei >= oldTotalWei, "You cannot reduce the bid.");
        uint256 netWeiPerAsset = newTotalWei * 1000 / (1000 + bidVATPermil[_bidder]) / _newTotalAmount;
        require(netWeiPerAsset >= bidNetWeiPerAsset[_bidder], "You cannot reduce the bid per asset.");  // Because of that, we're always over the min bid here.
        _recordAdjustedBid(_bidder, _newTotalAmount, 0, netWeiPerAsset, _candidateHigherBidder, _previousNextHigherBidder);
    }

    function _recordAdjustedBid(address _bidder, uint256 _amount, uint256 _bidTotalWei, uint256 _netWeiPerAsset, address _candidateHigherBidder, address _previousNextHigherBidder)
    internal
    {
        if (_bidder == highestBidder) {
            ...
            if (_previousNextHigherBidder == address(0)) {
                // Compare to highest bidder.
                if (hasHigherBid(highestBidder, bidNetWeiPerAsset[_bidder])) {
                    _previousNextHigherBidder = highestBidder;
                }
            }
            else {
                require(_previousNextHigherBidder != _bidder && hasHigherBid(_previousNextHigherBidder, bidNetWeiPerAsset[_bidder]), "Previous needs to actually have a higher bid.");
            }
            ...
            if (_candidateHigherBidder == address(0)) {
                // Compare to highest bidder.
                if (hasHigherBid(highestBidder, _netWeiPerAsset)) {
                    _candidateHigherBidder = highestBidder;
                }
            }
            else {
        ...
        bidTimestamp[_bidder] = block.timestamp;
        bidTotalWei[_bidder] = _bidTotalWei;
        emit BidChanged(_bidder, _amount, _netWeiPerAsset, bidTotalWei[_bidder]);
    }

    // Mark a bid paid via the API, for some bidder.
    function markExternalPayment(address _bidder, PaymentStatus _newStatus)
    public
    onlyAPI
    requireFinished
    {
        require(readyForDistribution(), "Need to have preparation finished.");
        require(bidAssetAmount[_bidder] > 0 && bidTotalWei[_bidder] == 0, "You need to have an active non-crypto bid.");
        require(_newStatus != PaymentStatus.Open, "Cannot set back to open.");
        bidPaymentStatus[_bidder] = _newStatus;
        emit PaymentStatusChanged(_bidder, _newStatus);
    }

    // Prepare for distribution of tokens and funds, will need to go through all bidders.
    function prepareDistribute(uint256 _maxSteps)
    public
    requireActive
    requireFinished
    {
        for (uint256 i = 0; i < _maxSteps; i++) {
            if (lastFundsDistributionBidder != eolGuard) {
                _prepareDistributeOneStep();
            }
        }
    }

    // Run distribution preparation for a single step.
    // This determines who should get how many tokens and finally, what the call price is for this auction.
    function _prepareDistributeOneStep()
    internal
    {
        if (amountScheduledToDistribute < assetToken().balanceOf(address(this))) {
            if (lastPrepDistributionBidder == address(0)) {
                // We haven't distributed anything yet, start with the highest bidder.
                lastPrepDistributionBidder = highestBidder;
            }
            else if (lastPrepDistributionBidder != eolGuard &&
                     bidToDistributeAmount[lastPrepDistributionBidder] >= bidAssetAmount[lastPrepDistributionBidder]) {
                // This one did get all its tokens, move to the next bid.
                lastPrepDistributionBidder = nextLowerBidder[lastPrepDistributionBidder];
            }
            if (lastPrepDistributionBidder != eolGuard) {
                bidToDistributeAmount[lastPrepDistributionBidder]++;
            }
            amountScheduledToDistribute++;
            if (bidTotalWei[lastPrepDistributionBidder] == 0 &&
                lastPrepDistributionBidder != eolGuard &&
                (bidToDistributeAmount[lastPrepDistributionBidder] >= bidAssetAmount[lastPrepDistributionBidder] ||
                 amountScheduledToDistribute >= assetToken().balanceOf(address(this)))) {
                emit RequestExternalPayment(lastPrepDistributionBidder);
            }
        }
    }
        ...
        if (assetToken().balanceOf(address(this)) > 0) {
            ...
            if (lastTokenDistributionBidder == address(0)) {
                ...
                lastTokenDistributionBidder = highestBidder;
                     ...
                     bidDistributedAmount[lastTokenDistributionBidder] >= bidToDistributeAmount[lastTokenDistributionBidder]) {
                // This one did get all its tokens, move to the next bid.
            ...
            if (lastTokenDistributionBidder == eolGuard) {
                ...
                // As it has not been sold, the NFT is sent to leftovers address for manual distribution.
                assetToken().safeTransferFrom(address(this), agriData.getAddress("leftovers"), tokenId);
            }
            else {
                PaymentStatus payStatus = bidPaymentStatus[lastTokenDistributionBidder];
                if (payStatus == PaymentStatus.Open) {
                    revert("Bid needs to be paid!");  // We expect the API to always be available to mark the success or failure.
                }
                else if (payStatus == PaymentStatus.Success) {
                ...
                else {  // Payment failed! We need to mark it as distributed for the loop to continue, but fail the actual distribution.
                    bidDistributedAmount[lastTokenDistributionBidder]++;
                    emit DistributionFailed(lastTokenDistributionBidder, tokenId);
                    // As the payment has failed, the NFT is sent to leftovers address for manual distribution.
                    assetToken().safeTransferFrom(address(this), agriData.getAddress("leftovers"), tokenId);
                }
            }
        }
        else {
            // Distribution Phase 2: Move actual payment funds to beneficiary, return overpayments and failed bid funds.
            ...
            if (bidTotalWei[lastFundsDistributionBidder] > 0 && lastFundsDistributionBidder != eolGuard) {
    ...
    requireFinished
    {
        ...
        require(bidAssetAmount[msg.sender] > 0 && bidTotalWei[msg.sender] > 0, "You need to have a crypto bid on this auction.");
