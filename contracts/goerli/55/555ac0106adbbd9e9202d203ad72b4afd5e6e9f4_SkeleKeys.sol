...
contract SkeleKeys is ERC721Y, Ownable {
    ...
    mapping(uint256 => address) private _allocations;
    uint256 public numAllocated;
    ...
    mapping(address => bool) private _isBurnAddress;
    ...
    function initKeys() private {
//        _mint(address(this), 13);
        _mintRandom(address(this), 13);
    }
    ...
    function getRandomKeyToAllocate(address to) private view returns(uint256) {
        uint256 supply = totalSupply();
        uint256 numRemaining = supply - numAllocated;
        uint256 spotToAllocate = getRandomNumber(to)%numRemaining;
//        uint256 spotToAllocate = 1%numRemaining;

        uint256 curIndex = 0;
        for (uint256 i = 0; i < supply; i++) {
            if (_allocations[i] == address(this)) {
    ...
    function allocateRandomKey(address to) private {
        uint256 keyToAllocate = getRandomKeyToAllocate(to);
//        safeTransferFrom(address(this), to, keyToAllocate);
        safeTransferFrom(address(this), to, keyToAllocate + 1);
        _allocations[keyToAllocate] = to;
    }

    function tryToAllocateRandomKey(address to) public onlyAuthorized {
        uint256 probRandom = getRandomNumber(to);
        _nonce += (probRandom>>10)%1024;

//        if ((probRandom%512 == 0) && (numAllocated < totalSupply())) {
        if ((probRandom%512 < 512) && (numAllocated < totalSupply())) {
            allocateRandomKey(to);
        }
    }

    function redeemKeyForSkelephunks(uint256 keyId, uint256 _genderDirection) public {
        require(msg.sender == tx.origin, "no contracts fam");
        require(msg.sender == ownerOf(keyId), "not ur key, not ur phunks");
        SkelephunksProto(skelephunksAddr).mintReserve(msg.sender, 3, _genderDirection);
        transferFrom(msg.sender, address(this), keyId);
//        _allocations[keyId] = address(this);
        _allocations[keyId - 1] = address(this);
    }
}
