    ...
    IEscrow
{
    ...
    function lock(
        ...
        IERC721(drop).safeTransferFrom(from, address(this), nftId);
    }

    function releaseFund(address from, uint256 price)
        external
        override
        onlyOrderBookOrAdmin
    {
        sendEth(payable(from), price);

        emit ReleaseFund(from, price);
    }
    ...
    function releaseNFT(
        ...
        IERC721(drop).safeTransferFrom(address(this), to, nftId);

        emit ReleaseNFT(to, drop, nftId);
    }
        ...
        onlyOwner
    {
        ...
        if (isManager) {
            ...
            renounceRole(ESCROW_MANAGER_ROLE, account);
        }
    }
...
interface IEscrow {
    ...
    function releaseFund(address from, uint256 price) external;
    ...
    event ReleaseNFT(address to, address drop, uint256 nftId);
}
