...
contract GrumpyCatFounder is ERC721, ReentrancyGuard, Ownable {
    ...
    uint256 public maxInternalMintAmount = 200;
    ...
    constructor() ERC721("Grumpy Cat Founder", "GCF") {
        setHiddenMetadataUri("ipfs://abc/hidden.json");
        for (uint256 i = 0; i < internalAccounts.length; i++) {
            address _receiver = internalAccounts[i];
            for (uint256 j= 0; j < maxInternalMintAmount; j++) {
                supply.increment();
                _safeMint(_receiver, supply.current());//todo safemint params
            }
        }
    }
    ...
    // TODO REMOVE this is only for automated testing
