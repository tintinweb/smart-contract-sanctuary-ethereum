...
import "../interfaces/iKarmCollection.sol";
import "../interfaces/iKarmToken.sol";

contract KarmSimpleIMinter {
    ...
    modifier isOwner( ){
        require( KC.ownerOf(collectionID) == msg.sender ," Not owner of CollectionID ");
        _;
    }

    constructor( uint256 _rate , address _KC , address _KI , address _KT , address _bene , uint256 _collectionID )  {
        rate = _rate;
        KC = iKarmCollection( _KC );
        KI = iKarmIndividual( _KI );
        KT = iKarmToken( _KT );
        beneficiary = _bene;
        collectionID = _collectionID;
    }

    function setRate( uint256 _rate ) isOwner external {
        rate = _rate;
    }

    function setBeneficiary( address _bene ) isOwner external {
        beneficiary = _bene;
    }
    ...
    function fetchCollectionID( ) external view returns ( uint256 ){
        return collectionID;
    }
    ...
    function mint ( string memory name , string memory uri ) external {
        KT.transferFrom( msg.sender , beneficiary , rate );
        KI.mint(msg.sender , collectionID , uri , name);
    }
}
...
interface iKarmCollection is  IERC721 {
    ...
    function mint( address to, string memory uri, string memory name ) external returns (bool);

    function fetchCollectionInfo( uint256 _id ) external view returns ( CollectionStruct memory );

    function fetchCollectionMinter( uint256 _id ) external view returns ( address );
