...
contract PositionSwapRouter {
    ...
    mapping(uint256 => bool) public bidCanceled;
    ...
    function create_position_bid(
        ...
        if (_underlyingOrderType == 0) {
            ...
            uint256 makers_taker_id = derivswapOrderbook.makers_taker_ID(_underlyingOrderID);
            HDGXStructs.TakerOrder memory taker = fetch_orderbook_taker_order(makers_taker_id);
            require(taker.user!= msg.sender, "Cannot place a bid order on your own position.");
            require(
            ...
            require(
                (taker.user != msg.sender && maker.user != msg.sender),
                "Cannot place a bid order on your own position."
            );
            require(
    ...
    function accept_position_bid(uint256 _bidID) public {
        ...
        require(
            bidCanceled[bid.positionBidID] == false,
        ...
        if (success == true) {
            TakeBid memory take_bid = TakeBid(
                msg.sender,
                bid.positionBidID,
                block.timestamp
            );
    ...
    function accept_position_ask(uint256 _askID) public payable {
        ...
        if (ask.underlyingIsMaker == true) {
            ...
            if (success == true) {
                delete makersAsk[maker.order_ID];
                makerForSale[maker.order_ID] = false;
            }
            ...
            if (success == true) {
                delete takersAsk[taker.order_ID];
                takerForSale[taker.order_ID] = false;
            }
        }
        if (success == true) {
            TakeAsk memory take_ask = TakeAsk(
                msg.sender,
                ask.positionAskID,
                block.timestamp
            );
    ...
    function external_settle_order_cleanup(
        ...
        require(msg.sender == address(drvs_vault));
        maker_order_cleanup(maker_order_id);
        taker_order_cleanup(taker_order_id);
    }

    function maker_order_cleanup(uint256 _underlyingOrderID) internal{
        if (makerForSale[_underlyingOrderID] == true) {
                delete makersAsk[_underlyingOrderID];
            }
            uint256 makers_bids_length = bid_lengths(_underlyingOrderID, 0);
            for (uint256 ix = 0; ix < makers_bids_length; ix++) {
                PositionBid memory bid = makersBids[_underlyingOrderID][ix];
                if (bidCanceled[bid.positionBidID] == false) {
                    if (bidAccepted[bid.positionBidID] == false) {
                        bidCanceled[bid.positionBidID] = true;
                        payable(bid.user).transfer(bid.ethAmount);
                        emit BidRefunded(bid.positionBidID, bid.user, bid.ethAmount, block.timestamp);
                    }
                }
            }
            if (makersBids[_underlyingOrderID].length > 0) {
                delete makersBids[_underlyingOrderID];
            }
    }

    function taker_order_cleanup(uint256 _underlyingOrderID) internal {
        if (takerForSale[_underlyingOrderID] == true) {
                delete takersAsk[_underlyingOrderID];
            }
            uint256 takers_bids_length = bid_lengths(
                _underlyingOrderID,
                1
            );
            for (uint256 ix = 0; ix < takers_bids_length; ix++) {
                PositionBid memory bid = takersBids[_underlyingOrderID][ix];
                if (bidCanceled[bid.positionBidID] == false) {
                    if (bidAccepted[bid.positionBidID] == false) {
                        bidCanceled[bid.positionBidID] = true;
                        payable(bid.user).transfer(bid.ethAmount);
            ...
            if (takersBids[_underlyingOrderID].length > 0) {
                delete takersBids[_underlyingOrderID];
            }
    }
    ...
    function user_lengths(uint256 _option)
        public
        view
...
contract HedgeXv3 {
    ...
    uint256 initialLiqThreshold = 10 * (10**18);
    ...
    function provideLiquidity() internal {
        ...
        hedgexLiqManager.mintNewPosition((10000 * (10**18)), initialLiqThreshold);
