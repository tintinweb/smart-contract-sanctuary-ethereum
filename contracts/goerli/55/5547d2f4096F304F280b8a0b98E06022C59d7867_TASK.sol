...
contract TASK is ERC721r, Ownable {
    ...
    mapping(address => UserPreSaleCounter)  public  _perWalletCounter;
    mapping(address => bool)                public  _addressExist;
    
    constructor() ERC721r("task", "TSK",100) {
    ...
    function mint(uint256 quantity) public {
        ...
        require(quantity > 0, "zero not allowed");
            if (_addressExist[msg.sender] == false) {
                _perWalletCounter[msg.sender] = UserPreSaleCounter({
                    counter: 0
                });
                _addressExist[msg.sender] = true;
            }
        require(_perWalletCounter[msg.sender].counter + quantity <= maxPerWallet, "Sorry can not mint more than maxwallet");
        
        _mintRandom(msg.sender, quantity);
        _perWalletCounter[msg.sender].counter += quantity;
