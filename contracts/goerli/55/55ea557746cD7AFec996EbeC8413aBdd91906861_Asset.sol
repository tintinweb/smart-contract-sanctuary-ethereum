...
abstract contract Mintable is Ownable, IMintable {
    address public glink;
    mapping(uint256 => bytes) public blueprints;

    event AssetMinted(address to, uint256 id, bytes blueprint);

    constructor(address _owner, address _glink) {
        glink = _glink;
        require(_owner != address(0), "Owner must not be empty");
        transferOwnership(_owner);
    }

    modifier onlyOwnerOrglink() {
        require(msg.sender == glink || msg.sender == owner(), "Function can only be called by owner or glink");
        _;
    }
    ...
    function mintFor(
        ...
        uint256 quantity,
        bytes calldata mintingBlob
    ) external override onlyOwnerOrglink {
...
contract Asset is ERC721, Mintable {
    constructor(
        ...
        address _glink
    ) ERC721(_name, _symbol) Mintable(_owner, _glink) {}
