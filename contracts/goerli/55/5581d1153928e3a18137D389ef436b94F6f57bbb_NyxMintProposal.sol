...
contract NyxMintProposal is Ownable, converter {
    ...
    {
        return abi.encode(proposal.action, proposal.mintPrice, proposal.mintSupply, proposal.reason);
    }
        ...
        returns (bytes memory)
    {
        ...
        MintProposal memory proposal = MintProposal(proposalId, parsedParams.action, parsedParams.mintPrice, parsedParams.mintSupply, parsedParams.reason);
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        for (uint256 idx = 0; idx < proposalBytesArr.length; idx++)
        {
            ...
            props[idx] = [abi.encode(params.action), abi.encode(params.mintPrice), abi.encode(params.mintSupply), abi.encode(params.reason)];
        }
    }
}
