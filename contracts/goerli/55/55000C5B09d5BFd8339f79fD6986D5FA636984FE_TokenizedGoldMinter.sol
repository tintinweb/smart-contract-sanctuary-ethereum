...
contract TokenizedGoldMinter is Ownable {
    ...
    function mintTokens(
        ...
        uint256 amount = _mintBars(beneficiary_, barMinter_, barVault_ ,barIdentifier_, barWeight_);
        if (chainlinkActive) {
            _chainlink(amount);
        }
...
contract BullionBar is ERC721PresetMinterPauserAutoId, Ownable {
    ...
    event BarMinted(uint256 tokenId, address beneficiary, string barComodity, string barMinter, string barVault, string barIdentifier, uint256 barWeight);
    ...
    uint256 barWeight_) external onlyMinter {
        ...
        emit BarMinted(tokenId, beneficiary_, barCommodity_, barMinter_, barVault_, barIdentifier_, barWeight_);
