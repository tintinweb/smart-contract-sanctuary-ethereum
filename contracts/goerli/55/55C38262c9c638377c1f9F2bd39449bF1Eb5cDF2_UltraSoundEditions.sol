    ...
    DefaultOperatorFilterer
{
    ...
    function opacity(uint256 position) internal view returns (string memory o) {
        uint256(
            position > 1
                ? (
                    position > 60
    ...
    function proofOfStake() internal view returns (string memory parts) {
        uint256 pi = 0;
        ...
        for (uint256 r = 0; r < 10; r++) {
            for (uint256 c = 0; c < 8; c++) {
                ...
                string memory part = string.concat(
                    "<g fill=",
                    pi > 1 || block.basefee < 16400 ? colors[pi % 12] : "#000",
