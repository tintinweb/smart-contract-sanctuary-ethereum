...
//This contract inherit ERC721 and Ownable interface/Library
contract PyraDooDev is ERC721, Ownable {
    using Strings for uint256;
    using SafeMath for uint256;

    uint256 private serviceValue;
    uint256 private serviceValuePYN;
    string private _currentBaseURI;
    uint256 public _tid;
    uint256 public minimumTokenLimit;
    uint256 public discountPercentage;
    uint256 public decimal;
    //whitelist
    address public whiteUserAdmin;
    bool public whiteListedUser;
    mapping(address => bool) public whiteListedUserArray;
    ...
    mapping(string => address) private tokenCategory;
    ...
    //get the token name and symbol at the time of crateing ERC721 contract which will receive in ERC721 interface
    constructor() ERC721("PyraDooDev Market", "PDD") {
        _tid = 1;
        serviceValue = 2500000000000000000;
        serviceValuePYN = 2500000000000000000;
        decimal = 18;
        minimumTokenLimit = 1000000000000000000000;
        discountPercentage = 5000000000000000000;
        whiteListedUserArray[msg.sender] = true;
    }

    function getServiceFeePYN() public view returns (uint256) {
        return serviceValuePYN;
    }

    function setServiceFeePYN(uint256 _serviceValuePYN) public onlyOwner {
        serviceValuePYN = _serviceValuePYN;
    }
    ...
    //set white listed user/allowed user to view the nft
    function enableWhiteListedUser(address _user, bool _approved) public onlyOwner returns(bool) {
        whiteListedUserArray[_user] = _approved;
        return _approved;
    }

    //get white listed user
    function getWhiteListedUser(address _user) public view returns(bool) {
        return whiteListedUserArray[_user];
    }
    function setServiceValue(uint256 _serviceValue) public onlyOwner {
    ...
    function getTokenAddress(string memory _tokenSymbol)
        ...
        returns (address)
    {
        return tokentype[_tokenSymbol];
    }

    function setTokenType(string memory _tokenSymbol, address _tokenAddress)
        public
        onlyOwner
    {
        tokentype[_tokenSymbol] = _tokenAddress;
    }
    ...
    //Only owner can change the baseURI (base URI is the unique Identifier of the token)
        ...
        returns (string memory ipfsmetadata)
    {
        ...
        require(whiteListedUserArray[msg.sender] == true, "Caller is not white listed!");
        Metadata memory data = token_id[tokenId];
        ipfsmetadata = data.ipfsmetadata;
        return ipfsmetadata;
    }

    //Below function is used Set _tokenURI as the tokenURI of the token
    function _setTokenURI(uint256 _tokenId, string memory _tokenURI)
        internal
        virtual
    {
        //check the tokenId is exist before set the tokenURI for that tokenId
        require(
            _exists(_tokenId),
            "ERC721Metadata: URI set of nonexistent token"
        );
        //if tokeid is exist then set the tokenURI as tokenURI for the given tokeId
        _tokenURIs[_tokenId] = _tokenURI;
    }

    //call to see what is the baseURI extended by the owner
    //this baseURI is used to compute the tokenURI by resulting URI
    //for each token will be concatenated baseURI with tokenId.
    ...
    //In this function need to mint the token and set the the tokenURI for the tokenId
    function mint(
        uint256 tokenId,
        string memory name,
    ...
    function enableTokenStatus(bool _status, uint256 _tokenId)
        public
        returns (bool)
    {
        require(_creator[_tokenId] == msg.sender, "User Not Authorized");
    ...
    function _saleToken(
        ...
        require(msg.value >= val && val > 0, "Insufficient Balance total");
        require(
            amount >= order_place[from][tokenId].price &&
                order_place[from][tokenId].price > 0,
            "Insufficent found"
        );
    ...
    function acceptBId(
        ...
        emit Acceptbid(msg.sender, bidaddr, tokenId, amount);
    }
    ...
    function _acceptBId(
        ...
        uint256 serviceVal = serviceValue;
        if (keccak256(bytes(tokenSymbol)) == keccak256(bytes("PYN"))) {
            serviceVal = serviceValuePYN;
        }

        uint256 val = pERCent(amount, serviceVal).add(amount);
        IERC20 t = IERC20(tokentype[tokenSymbol]);
        uint256 Tokendecimals = decimal.sub(t.decimals());
        uint256 approveValue = t.allowance(from, address(this));
        require(
            approveValue >= val.div(10**Tokendecimals),
            "Insufficient Balance"
        );
        ...
        require(
            approveValue >=
                (_adminfee.add(roy.add(netamount))).div(10**Tokendecimals),
            "Insufficient Balance"
        );
        if (_adminfee != 0) {
        ...
        override(ERC721)
    {
     ...
     function burnToken(uint256 id, address from) public {
        require(
            balances[id][from] == 1,
            "You are Not a Token Owner or insufficient Token Balance"
        );
       burn(id);
