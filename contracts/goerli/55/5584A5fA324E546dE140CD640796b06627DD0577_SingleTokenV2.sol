...
contract SingleTokenV2 is ERC721EnumerableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    event ContractDeployed(address indexed owner, string baseUri, string name, string symbol);

    /// @dev Emit an event when mint success
    event Mint(address indexed to, uint256 indexed tokenId, uint256 indexed quantity);
    ...
    /// @dev Emit an event when setMintFee success
     ...
     * @param _mintFee Fee for mint
     * @param _baseUri Contract URI metadata
     * @param _name Token name
     * @param _symbol Token symbol
     */
    function initialize(address _owner, uint256 _mintFee, string memory _baseUri, string memory _name, string memory _symbol) public initializer {
        ...
        emit ContractDeployed(_owner, _baseUri, _name, _symbol);
    }
     ...
     */
    function mint(address _to, uint256 _quantity) public payable returns (uint256 tokenId) {
        require(msg.value >= mintFee, "Invalid mint fee");
        lastId.increment();
        _safeMint(_to, lastId.current());
        tokenId = lastId.current();
        emit Mint(_to, tokenId, _quantity);
    }
...
import "../contracts/SingleTokenV2.sol";

contract $SingleTokenV2 is SingleTokenV2 {
    constructor() {}
