...
//    .:-:.         ....:::.....                        ..     ...     ..:::..       ...      ..    ...   ...    ..    ....     .::.....    ..
//
/// @title Zero LifeTime Pass NFT
/// @author audie.eth
/// @notice An NFT for lifetime access to the Zero application by Probably0.
...
contract PRBLYLifetimePassNFT is ERC721, ERC721Burnable, DefaultOperatorFilterer, ERC2981, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _nextTokenId;

    // the maximum number that can be minted
    string private _baseTokenURI =
        'https://probably0.mypinata.cloud/ipfs/QmZhfo4sCWtg9Wdsw6Yb5WZxvedSJ8EiAYR2bHXSApBF8v';

    // the total number that have been minted
    uint256 private _mintedSupply;
    mapping(address => bool) private _claims;

    string public contractURI = 'https://probably0.mypinata.cloud/ipfs/QmbWwXCdDCDsLkMTFtcasX4YYkSfF5g2E39krbVYBZibgr';
    uint256 public constant maxSupply = 350;
    ...
    /**
     *  @notice Public mint function. Just checks the mint is open and sent enough ETH
     */
    function mint() external payable {
        // only the address on the merkle tree can mint, no others - means tree can be public
        address account = _msgSender();
        require(mintOpen == true, 'Mint not open');
        require(msg.value >= mintPrice, 'Not enough ETH');
        _mintOne(account);
    }

    /**
     *  @notice Given a caller with a matching proof, mints the next NFT
     */
    function allowListMint(bytes32[] calldata proof) external payable {
        // only the address on the merkle tree can mint, no others - means tree can be public
        address account = _msgSender();
        require(allowListMintOpen == true, 'AllowListMint not open');
        require(_claims[account] == false, 'Claimed Already');
        require(_verify(proof, _leaf(account)), 'Invalid Proof');
        require(msg.value >= allowListMintPrice, 'Not enough ETH');
    ...
    function ownerMint(address account, uint256 numberToMint) external onlyOwner {
        uint256 i;
        do {
            if (_nextTokenId.current() <= maxSupply) {
                _mintOne(account);
            }
            unchecked{++i;}
     ...
     *  @param newMintOpen The boolean value to determine whether the mint is open
     */
     ...
     *  @param newAllowListMintOpen The boolean value to determine whether the allowListMint is open
     */
    ...
    /**
     *   @notice Shows total existing supply
    ...
    /**
     *  @notice Confirms an address, and proof is on the allowList
     *  @param account The address to use as the first part of a merkle leaf
     *  @param proof The merkle proof you want to check is valid - should come from contract's merkle root
     */
    ...
    function _mintOne(address account) internal {
        require(_nextTokenId.current() <= maxSupply, 'Cannot mint beyond supply limit');
