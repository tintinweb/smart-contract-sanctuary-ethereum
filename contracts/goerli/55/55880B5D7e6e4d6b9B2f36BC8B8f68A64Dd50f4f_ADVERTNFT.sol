...
;
    ...
    function mint(string calldata _json_url)
    ...
    function wlmint(uint256 index,bytes32[] calldata merkleProof,string calldata _json_url)
    nonReentrant external payable
    {
        require(blockTimestamp()>=nftMintStartTime,'Mint not start yet');
    ...
    function _mint(string memory _json_url)
    internal
    {
    ...
    function claimTokens(uint256 fromindex,uint256 max)nonReentrant external {
    ...
    function claim(uint256 tokenId)nonReentrant public {
        ...
        address(uint160(ownerOf(tokenId))).send(amount-claimMap[tokenId]);
    ...
    function editmetadata(uint256 tokenId,string calldata _json_url) external{
        require(_exists(tokenId),"not correct tokenid");

        require(msg.sender==ownerOf(tokenId),"not owner");
        IERC20(erc20).burnFrom(msg.sender, 1000 ether);
        NFTs[tokenId]=_json_url;
    ...
    function truthURI(uint256 tokenId) public view returns (string memory) {
        ...
        string memory img=string(abi.encodePacked(uri_prefix,"/",NFTs[tokenId]));
