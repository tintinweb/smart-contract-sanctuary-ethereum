...
contract AssetRouter is IAssetRouter, AccessControl, ReentrancyGuard {
    ...
    function _sendVouchers(
        ...
        require(cp.active, "Stargate: counter chainPath is not ready");
        cp.kbp += cp.vouchers;
        c.optimalDstBandwidth = (pool.totalLiquidity * cp.weight) / pool.totalWeight;
        c.vouchers = cp.vouchers;
        console.log("address this send ", address(this));

        console.log("cp.kbp send ", cp.kbp);
        console.log("cp.kbp send _srcPoolId ", _srcPoolId);
        console.log("cp.kbp send _dstChainId ", _dstChainId);
        console.log("cp.kbp send _dstPoolId ", _dstPoolId);
        cp.vouchers = 0;
    ...
    function swapRemote(
        ...
        console.log("cp kbp ", cp.kbp);
        console.log("_s kbpRemove ", _s.kbpRemove);
        cp.kbp -= _s.kbpRemove;
    ...
    function createChainPath(
        ...
        for (uint256 i; i < chainPaths[_srcPoolId].length; ++i) {
            ...
            require(!exists, "Stargate: can't createChainPath of existing dstChainId and _dstPoolId");
        }
    ...
    function setFeeLibrary(address _feeHandler) external {
        require(_feeHandler != address(0), "Stargate: fee library cant be 0x0");
    ...
    function activateChainPath(uint16 _srcPoolId, uint16 _dstChainId, uint16 _dstPoolId) external {
        ChainPath storage cp = getAndCheckCP(_srcPoolId, _dstChainId, _dstPoolId);
        require(cp.active == false, "Stargate: chainPath is already active");
        cp.active = true;
    }
    ...
    function getAndCheckCP(
        ...
        require(chainPaths[_srcPoolId].length > 0, "Stargate: no chainpaths exist");
