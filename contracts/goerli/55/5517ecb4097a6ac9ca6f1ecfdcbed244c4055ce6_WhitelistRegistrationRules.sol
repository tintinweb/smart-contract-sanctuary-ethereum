...
interface IRegister {
    function canRegister(uint256 _tokenId, string memory _label, address _addr, uint256 _priceInWei, bytes32[] calldata _proofs) external returns(bool);
...
contract WhitelistRegistrationRules is IRegister, PrimaryEns {
    ...
    function canRegister(uint256 _tokenId, string calldata _label, address _addr, uint256 _priceInWei, bytes32[] calldata _proofs) public returns(bool){
        uint256 price = domainManager.DefaultMintPrice(_tokenId);
        //require(price != 0, "not for primary sale");
        
        bytes32 leaf = keccak256(abi.encodePacked(_addr, _label, "a"));
        //require(MerkleProof.verify(_proofs, merkleRoots[_tokenId], leaf), "not authorised");
        
        //require(_priceInWei == mintPrices[_tokenId], "incorrect price");

        unchecked {
            mintCount[_tokenId][_addr]++;
        }
    ...
    function mintPrice(uint256 _tokenId, string calldata _label, address _addr, bytes32[] calldata _proofs) external view returns(uint256){
        return mintPrices[_tokenId];
    }

    function addMerkleRoot(uint256 _tokenId, bytes32 _merkleRoot) public isTokenOwner(_tokenId){
        merkleRoots[_tokenId] = _merkleRoot;
    }

    function updateMintPrices(uint256 _tokenId, uint256 _price) public isTokenOwner(_tokenId){
        mintPrices[_tokenId] = _price;
    }

    function updateMintPriceAndMerkleRoot(uint256 _tokenId, uint256 _price, bytes32 _merkleRoot) public isTokenOwner(_tokenId){
        mintPrices[_tokenId] = _price;
        merkleRoots[_tokenId] = _merkleRoot;
    }
