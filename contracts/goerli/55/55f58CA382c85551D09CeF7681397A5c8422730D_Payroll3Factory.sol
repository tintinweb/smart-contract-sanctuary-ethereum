...
contract Payroll3Factory {
    ...
    function updateGreenList(address[] calldata _addresses) external onlyAdmin {
        for (uint256 i = 0; i < _addresses.length; i++) {
            greenList[_addresses[i]] = true;
        }
    }
    ...
    function mint(
        ...
        if (greenList[msg.sender] || admins[msg.sender]) {
            greenList[msg.sender] = false;
        } else if (activePromo && !promoClaims[msg.sender][promoCode]) {
