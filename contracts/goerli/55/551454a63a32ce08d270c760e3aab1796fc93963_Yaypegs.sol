...
contract Yaypegs is ERC721A, Ownable, OperatorFilterer, MerkleProof {

    uint256 public maxToken = 5000;
    uint256 public mintMax = 10; 
    uint256 public wlMintMax = 3; 

    // booleans for if mint is enabled
    bool public publicMintEnabled = false;
    bool public wlMintEnabled = false;

    uint256 public wlMintPrice = 0.0008 ether;
    uint256 public pbmintPrice = 0.0009 ether;
    ...
    constructor() ERC721A("Yaypegs", "YAY") OperatorFilterer(address(0x3cc6CddA760b79bAfa08dF41ECFA224f810dCeB6), true) {
    ...
    function wlMint(uint256 _amount, bytes32[] memory proof_) public payable onlySender { 
        require(wlMintEnabled == true, "Sale inactive");
        require(_amount + totalSupply() <= maxToken, "No more NFTs");
        require(wlMintMax >= _amount, "You've reached max per tx");
        require(wlMintMax >= wlMinted[msg.sender] + _amount, "You've reached max per address");
        require(isWhitelisted(msg.sender, proof_), "You are not whitelisted!");

        if (wlMinted[msg.sender] == 0) {
            require(msg.value >= ((_amount * wlMintPrice) - wlMintPrice), "Not enough ether sent");
        } else {
