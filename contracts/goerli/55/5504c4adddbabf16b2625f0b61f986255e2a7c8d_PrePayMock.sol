 ...
 */
contract PrePayMock is ERC721Airdroppable, Terms, ReentrancyGuard {
    ...
    uint256 private _prePayAllocated;

    // The next token ID to be minted.
    uint256 private _redeemPointer;

    /**
     * @dev Emit an event when the max supply is updated.
     */
    event PrePayTotalUpdated(uint256 value);

    /**
     * @notice PrePayMock constructor.
     *
     * @param name The token name.
     * @param symbol The token symbol.
     */
    ...
    {
        // Initial maxSupply is 5000
        _maxSupply = 5000;

        // Initial maxSupply is 1100
        _prePayAllocated = 1000;
     ...
     * @notice Return total quantity of tokens from an array of receivers.
     *
     * @return uint256 - the total quantity within the reservations array
     */
    function _totalQuantity(Reservation[] memory reservations)
        private
        pure
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < reservations.length; i++) {
            totalQuantity += reservations[i].quantity;
        }

        return totalQuantity;
    }

    function prePay(uint256 quantity) public payable nonReentrant {
        address sender = _msgSender();
        uint256 newTotalReservations = _totalQuantity(_reservations) + quantity;
        // require(msg.value == price * quantity, msgConfig.BAD_AMOUNT);
        require(newTotalReservations <= _prePayAllocated);

        _reservations.push(Reservation(sender, quantity));
        // _reservations[sender] += quantity;

        emit PrePayTotalUpdated(newTotalReservations);
    }

    function prePayCredit(uint256 quantity, address to)
        public
        payable
        nonReentrant
    {
        ...
        _reservations.push(Reservation(to, quantity));
     ...
     * @notice Mint next available token(s) to addres using ERC721A _safeMint
     *
     * @param reservation The address to mint to.
     */
    function _mintReservation(Reservation memory reservation) private {
        require(
            totalSupply() + reservation.quantity <= _maxSupply,
            MessageConstants.MAX_SUPPLY
        );

        _safeMint(reservation.to, reservation.quantity);
    }

    function redeem(uint256 count) public {
        uint256 total = _redeemPointer + count;

        if (total > _reservations.length) {
            total = _reservations.length;
        }

        for (uint256 i = _redeemPointer; i < total; i++) {
            _mintReservation(_reservations[i]);
        }

        _redeemPointer = total;
    }
}
