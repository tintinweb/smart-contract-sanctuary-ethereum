...
// File: Aventus/nft-marketplace-contracts/contracts/libraries/structs.sol


pragma solidity ^0.8.9;

enum PriceStrategy{ SIMPLIFIED, TIERS }
...
struct BatchPriceRule {
    uint idFrom; // Which tokenId id starts the rule - inclusive
    uint idTo; // Which tokenId  ends the rule - inclusive
    uint minMint; // How much min supply needed for rule to take effect - zero means no min
    address[] tokens;
    uint[] prices;
}
...
// File: Aventus/nft-marketplace-contracts/contracts/helpers/Whitelistable.sol
...
abstract contract Whitelistable is AccessControl {
  ...
  function isAvailable(uint256 _start, uint256 _end) internal view returns (bool available) {
  ...
  function setTime (uint256 _startTime, uint256 _endTime, bool _whitelist) external onlyAdmin {
    if (_whitelist) {
...
// File: Aventus/nft-marketplace-contracts/contracts/helpers/Exchangeable.sol
...
abstract contract Exchangeable is Whitelistable {
    ...
    function getPricingStrategy() external view returns (uint) {
        return uint(pricestrategy);
    }
    ...
    function resetSimplifiedPricing (
        ...
        if (_acceptedTokens.length != _acceptedPrices.length) revert InvalidAction("Tokens and prices arrays not matching");
...
// File: Aventus/nft-marketplace-contracts/contracts/helpers/Priceable.sol
...
abstract contract Priceable is Exchangeable  {
    ...
    function _getPrice(
        ...
        if (pricestrategy == PriceStrategy.TIERS && tierRules[_tokenId].minMint > 0) {
...
// File: Aventus/nft-marketplace-contracts/contracts/helpers/ERC1155Customizable.sol
...
abstract contract ERC1155Customizable is Priceable {
    ...
    function setHiddenMetadata(string memory _hiddenMetadataUri) external onlyAdmin {
    ...
    function setBaseExtension(string calldata _extension) external onlyAdmin {
        if (bytes(_extension).length == 0) revert InvalidAction("Invalid BaseExtension");
        baseExtension = _extension;
    }

    function setTokenUris(uint256[] calldata _tokenIds, string[] calldata _tokenURIs) external onlyAdmin {
        if (_tokenIds.length != _tokenURIs.length) revert InvalidAction("TokenIds and tokenURIs must equal length");
    ...
    function getTotalSupplies(uint256 from, uint256 to) external view returns(uint256[] memory, uint256) {
    ...
    function setMaxSupply(uint256 _newSupply) external onlyAdmin {
        if (_newSupply < currentMaxTokenId) {
            revert InvalidAction( "New max supply cannot be lower than highest mint");
        }
    ...
    function setMaxQuantity(uint256[] calldata _tokenIds, uint256[] calldata _newQuantities) external onlyAdmin {
        if (_tokenIds.length != _newQuantities.length) revert InvalidAction("TokenIds and tokenURIs must equal length");
...
// File: Aventus/nft-marketplace-contracts/contracts/helpers/Withdrawable.sol
...
abstract contract Withdrawable is ERC1155Customizable {
    ...
    function getPayees() external view returns (address[] memory, uint256[] memory, uint256){
    ...
    function withdraw(address _paymentToken) external onlyAdmin {
        ...
        for (uint256 j = 0; j < payees.length; j++) {
            ...
            if (_paymentToken == address(0)) {
                (bool success, ) = payable(payees[j].destination).call{ value: weight }("");
                require(success, "Payment failure");
            } else {
                IERC20(_paymentToken).safeTransfer(payees[j].destination, weight);
            }
        }
    }
}

// File: Aventus/nft-marketplace-contracts/contracts/helpers/ERC1155Mintable.sol
...
contract ERC1155AMintable is ERC1155Burnable, Withdrawable {
  ...
  function init(
      ...
      if (_globalSupplyConfigs.length < 3) revert InvalidAction("Incomplete _globalSupplyConfigs");
  ...
  function uri(uint256 id) public view virtual override returns (string memory uriString) {
      ...
      if (bytes(uriString).length == 0) {
        uriString = string.concat(baseURI, Strings.toString(id), baseExtension);
      }
  ...
  function getTotalCost(
      ...
      for (uint256 x = 0; x < _tokensIds.length; x++) {
        uint256 price = _getPrice(_tokensIds[x], _mintAmounts[x], _tokenAddress);
  ...
  function verifySupply(uint256 _amount, uint256 _tokenId) internal view {
    ...
    if (
      ((maxMintPerTx[_tokenId] > 0 && _amount > maxMintPerTx[_tokenId]) || (maxMintPerTx[_tokenId] == 0 && _amount > maxMintsPerTxGlobal) )
      || ((maxQuantity[_tokenId] > 0 && newQuantity > maxQuantity[_tokenId]) || (maxQuantity[_tokenId] == 0 && newQuantity > maxQuantityGlobal))
      || _tokenId >= maxSupply
    ) {
  ...
  modifier verifyMintAmount(uint256[] memory _tokenIds, uint256[] memory _mintAmounts) {
...
// File: Aventus/nft-marketplace-contracts/contracts/helpers/MogulERC1155Mintable.sol
...
contract MogulERC1155Mintable is ERC1155AMintable, ReentrancyGuard {
  ...
  function _payment(
    ...
    if (!isPaymentToken[_paymentToken]) revert InvalidAction("Invalid token");

    if (_merkleProof.length > 0) {
      if (!isAvailable(startTimeWhitelist, endTimeWhitelist)) revert InvalidAction("Mint unavailable");

      if (whitelistClaimed[msg.sender]) revert InvalidAction("Whitelist Claimed");

      //Verify provided _merkelProof via APi call
      if (!isWhiteListed(_merkleProof, msg.sender)) revert InvalidAction("Invalid proof");
...
/**
  ...
  function mintBatchMultipleRecipients(
    ...
    if (tokenIds.length != amounts.length || tokenIds.length != tos.length) revert InvalidAction("Arrays do not match");
