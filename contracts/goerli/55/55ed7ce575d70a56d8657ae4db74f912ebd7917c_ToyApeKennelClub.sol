...
enum TAKCContractStatus {
  disable,
  claim
}

contract ToyApeKennelClub is Ownable, ERC721A, ERC2981, ReentrancyGuard {
    // address to toybirds contract
    ToyApeYachtClub _toyApeYachtClub;

    error OwnerIndexOutOfBounds();

    TAKCContractStatus public CONTRACT_STATUS = TAKCContractStatus.disable;

    uint256 public immutable TOTAL_SUPPLY = 170;
    ...
    constructor(address taycContract) ERC721A("Toy Ape Kennel Club", "TAKC") {
        _toyApeYachtClub = ToyApeYachtClub(taycContract);
        // royalty
        _setDefaultRoyalty(owner(), 750);
    }
    ...
    function getOwnedToyApeIds(address holder) external view returns(uint16[] memory) {
      uint256 amountToyApesOwned = _toyApeYachtClub.balanceOf(holder);
      require(amountToyApesOwned > 0, "Address does not own any Toy Apes");
      uint256 totalToyApes = _toyApeYachtClub.totalSupply();
      uint16[] memory ownedToyApeIds = new uint16[](amountToyApesOwned);
      uint16 idx = 0;

      for(uint16 i = 1; i <= totalToyApes; i++) {
        if(_toyApeYachtClub.ownerOf(i) == holder && !isClaimed(i)) {
          ownedToyApeIds[idx] = i;
          idx++;
        }
      }

      require(ownedToyApeIds[0] != 0, "No tokens to claim");

      return ownedToyApeIds;
    }

    function holderClaim(uint16[] calldata ownedToyApeIds) public nonReentrant {
        require(_toyApeYachtClub.balanceOf(msg.sender) > 0, "Address does not own any Toy Apes");
        require(CONTRACT_STATUS == TAKCContractStatus.claim, "Claim is not active.");
        require(ownedToyApeIds.length > 0, "No IDs given");

        uint16 numTokensClaimed;
         
        for (uint16 i = 0; i < ownedToyApeIds.length; i++) {
            if (ownedToyApeIds[i] != 0 && _toyApeYachtClub.ownerOf(ownedToyApeIds[i]) == msg.sender && !isClaimed(ownedToyApeIds[i])) {
                _setClaimed(ownedToyApeIds[i]);
                numTokensClaimed++;
            }
        }
    ...
    function setStatus(TAKCContractStatus status) external onlyOwner {
        CONTRACT_STATUS = status;
    }
