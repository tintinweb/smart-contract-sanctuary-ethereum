...
import "./interfaces/AutomationBase.sol";
import "./interfaces/IStaderExecutionLayerRewards.sol";
import "./interfaces/ISSVStakingPool.sol";
...
contract Stader is Timelock, AutomationBase, AutomationCompatibleInterface {
    using SafeMath for uint256;
    ETHX public ethX;
    address payable ssvPoolAddress;
    address payable public daoAddress;
    address payable public staderExecutionLayerRewardContract;
    ...
    // Amount of Eth Rewards  from Execulation Layer Contract to Stader Contract
    event ReceivedExecutionLayerRewards(uint256 amount);

    /// @notice check the owner or ssvPoolAddress
    modifier onlyScriptOrOwner() {
        require(
            msg.sender == timelockOwner || msg.sender == ssvPoolAddress,
            "ERR_NOT_AUTHORIZED"
        );
        _;
    }
     ...
     * @param _daoAddress daoAddress address
     * @param _timelockOwner multi sig owner of the contract
     * @param _ssvPoolAddress Staking Pool Manager contract
     * @param _staderExecutionLayerRewardContract stader execution layer reward contract address
     */
    function initialize(
        ETHX _ethX,
        address payable _daoAddress,
        address _timelockOwner,
        address payable _ssvPoolAddress,
        address payable _staderExecutionLayerRewardContract
    ) public initializer checkZeroAddress(_timelockOwner) {
        Timelock.initializeTimelock(_timelockOwner);
        ethX = _ethX;
        daoAddress = _daoAddress;
        ssvPoolAddress = _ssvPoolAddress;
        staderExecutionLayerRewardContract=_staderExecutionLayerRewardContract;
        initialSetup();
    }
     ...
     */
    function stake(address _referral) external payable whenNotPaused {
        ...
        emit Staked(msg.sender, amount, _referral);
    }

    /**
     * @notice Transfer ETH to staking pool once balance cross 32 ETH mark
     * @dev send fund to staking pool, only owner or script Manager can initiate
     */
    function registerValidator() public onlyScriptOrOwner {
        ISSVStakingPool(ssvPoolAddress).receiveFund{value:address(this).balance}();
    }

    /**
    * @notice A payable function for execution layer rewards. Can be called only by StaderExecutionLayer Contract 
    */
    function receiveExecutionLayerRewards() external payable {
        require(msg.sender == staderExecutionLayerRewardContract);
        totalELRewards = totalELRewards+ msg.value;
        emit ReceivedExecutionLayerRewards(msg.value);
    }
    ...
    function distributeFee(uint256 _totalRewards) internal {
        ...
        // token shares.
        uint256 shares2mint = (
            _totalRewards.mul(feePercentage).mul(getTotalSupply()).div(
                getTVL().mul(100).sub(feePercentage.mul(_totalRewards))
            )
        );
        ...
        ethX.mint(address(this), shares2mint);
    ...
    */
    function handleOracleReport(uint256 _beaconValidators, uint256 _beaconBalance) external {
        ...
        uint256 rewardBase = (appearedValidators*deposit_size) + beaconBalance;
        ...
        if (staderExecutionLayerRewardContract != address(0)) {
            executionLayerRewards = IStaderExecutionLayerRewards(staderExecutionLayerRewardContract).withdrawRewards(
               10 //(_getTotalPooledEther() * EL_REWARDS_WITHDRAWAL_LIMIT_POSITION.getStorageUint256()) / TOTAL_BASIS_POINTS
            );

            if (executionLayerRewards != 0) {
                bufferedETH = bufferedETH+executionLayerRewards;
            }
        }
    ...
    function checkUpkeep(
        ...
        return (address(this).balance >= 0.0001 ether, bytes(""));
    }

    function performUpkeep(
        bytes calldata /* performData */
    ) external override {
        // require(address(this).balance >= 1 ether, "ERR_NOT_ENOUGH_ETH");
        ISSVStakingPool(ssvPoolAddress).receiveFund{value:address(this).balance}();
    }
    ...
    /// @notice return the total supply of ethX token
    function getTotalSupply() public view returns (uint256) {
        return ethX.totalSupply();
    }

    /**********************
     * Setter functions   *
     **********************/
    /// @dev set address of script Manager
    function setSSVPoolAddress(address payable _ssvPoolAddress) public checkOwner {
        ssvPoolAddress = _ssvPoolAddress;
    }

    function setTVL(uint256 _totalTVL) external checkOwner {
        totalTVL = _totalTVL;
    }
...
abstract contract Timelock is Initializable {
    ///@notice time in secs for withholding transfer transaction
    ///@dev unlocks in 2 hours
    uint256 public lockedPeriod;

    ///@dev timestamp of change owner request, owner can be changed after waiting for this period
    uint256 timestamp;

    /// @notice address of multisig admin account which act as the owner for contract operations
    address timelockOwner;

    ///@notice address of new multisig admin account which will act as the new owner for contract operations
    ...
    function initializeTimelock(address _timelockOwner)
        internal
        onlyInitializing
    {
    ...
    function acceptTimelockOwnership() external {
        if (timelockOwner != msg.sender) revert("You are not the owner");
        require(
     ...
     * Setter functions   *
     **********************/
    /// @notice Set new multisig owner for the transfer of Hbar to new version
    /// @param _timelockOwner the new owner of Hbar withdrawal to new version
...
interface IStaderExecutionLayerRewards {
    event ETHReceived(uint256 amount);

    function STADER() external view returns (address);

    function withdrawRewards(uint256 _maxAmount)
        external
...
/*
[{"inputs":[{"internalType":"address","name":"_stader","type":"address"}],"stateMutability":"nonpayable","type":"constructor"},{"anonymous":false,"inputs":[{"indexed":false,"internalType":"uint256","name":"amount","type":"uint256"}],"name":"ETHReceived","type":"event"},{"inputs":[],"name":"STADER","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"uint256","name":"_maxAmount","type":"uint256"}],"name":"withdrawRewards","outputs":[{"internalType":"uint256","name":"amount","type":"uint256"}],"stateMutability":"nonpayable","type":"function"},{"stateMutability":"payable","type":"receive"}]
*/
...
interface ISSVStakingPool {
    ...
    function chainlink(uint256) external view returns (bytes memory);

    function depositEthtoDepositContract() external;

    function ethValidatorDeposit() external view returns (address);

    function getChainlink() external view returns (bytes[] memory);
    ...
    function registerValidatortoSSVNetwork(uint256 tokenFees) external;
    ...
    function validatorsRegistry(uint256)
        external
        view
        returns (
            bytes memory pubKey,
...
// THIS FILE WAS AUTOGENERATED FROM THE FOLLOWING ABI JSON:
/*
[{"anonymous":false,"inputs":[{"indexed":false,"internalType":"uint8","name":"version","type":"uint8"}],"name":"Initialized","type":"event"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"previousOwner","type":"address"},{"indexed":true,"internalType":"address","name":"newOwner","type":"address"}],"name":"OwnershipTransferred","type":"event"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"bytes","name":"pubKey","type":"bytes"},{"indexed":false,"internalType":"uint256","name":"indexOfValidatorinRegistry","type":"uint256"}],"name":"addedToValidatorRegistry","type":"event"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"bytes","name":"pubKey","type":"bytes"}],"name":"depositToDepositContract","type":"event"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"bytes","name":"pubKey","type":"bytes"},{"indexed":false,"internalType":"uint256","name":"index","type":"uint256"}],"name":"reRegisteredValidatortoSSVNetwork","type":"event"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"bytes","name":"pubKey","type":"bytes"},{"indexed":false,"internalType":"uint256","name":"indexOfValidatorinRegistry","type":"uint256"}],"name":"registeredValidatortoSSVNetwork","type":"event"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"bytes","name":"pubKey","type":"bytes"}],"name":"removedValidatorfromSSVNetwork","type":"event"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"bytes","name":"pubKey","type":"bytes"},{"indexed":false,"internalType":"uint256","name":"index","type":"uint256"}],"name":"updatedValidatortoSSVNetwork","type":"event"},{"inputs":[{"internalType":"bytes","name":"_pubKey","type":"bytes"},{"internalType":"bytes[]","name":"_publicShares","type":"bytes[]"},{"internalType":"bytes[]","name":"_encyptedShares","type":"bytes[]"},{"internalType":"uint32[]","name":"_operatorIDs","type":"uint32[]"},{"internalType":"bytes","name":"_withdrawal_credentials","type":"bytes"},{"internalType":"bytes","name":"_signature","type":"bytes"},{"internalType":"bytes32","name":"_deposit_data_root","type":"bytes32"}],"name":"addToValidatorRegistry","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"uint256","name":"","type":"uint256"}],"name":"chainlink","outputs":[{"internalType":"bytes","name":"","type":"bytes"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"depositEthtoDepositContract","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"ethValidatorDeposit","outputs":[{"internalType":"contract IDepositContract","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"getChainlink","outputs":[{"internalType":"bytes[]","name":"","type":"bytes[]"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"bytes","name":"_publicKey","type":"bytes"}],"name":"getValidatorIndexByPublicKey","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"contract ISSVNetwork","name":"_ssvNetwork","type":"address"},{"internalType":"contract IERC20","name":"_ssvToken","type":"address"},{"internalType":"contract IDepositContract","name":"_ethValidatorDeposit","type":"address"}],"name":"initialize","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"owner","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"bytes","name":"_pubKey","type":"bytes"},{"internalType":"uint256","name":"tokenFees","type":"uint256"}],"name":"reRegisterValidatortoSSVNetwork","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"readRegistry","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"receiveFund","outputs":[],"stateMutability":"payable","type":"function"},{"inputs":[{"internalType":"uint256","name":"tokenFees","type":"uint256"}],"name":"registerValidatortoSSVNetwork","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"registeredValidatorCount","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"bytes","name":"publicKey","type":"bytes"}],"name":"removeValidatorfromSSVNetwork","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"renounceOwnership","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"ssvNetwork","outputs":[{"internalType":"contract ISSVNetwork","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"ssvToken","outputs":[{"internalType":"contract IERC20","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"newOwner","type":"address"}],"name":"transferOwnership","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"contract IDepositContract","name":"_ethValidatorDeposit","type":"address"}],"name":"updateEthDepositAddress","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"contract ISSVNetwork","name":"_ssvNetwork","type":"address"}],"name":"updateSSVNetworkAddress","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"contract IERC20","name":"_ssvToken","type":"address"}],"name":"updateSSVTokenAddress","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"bytes","name":"_pubKey","type":"bytes"},{"internalType":"bytes[]","name":"_publicShares","type":"bytes[]"},{"internalType":"bytes[]","name":"_encyptedShares","type":"bytes[]"},{"internalType":"uint32[]","name":"_operatorIDs","type":"uint32[]"},{"internalType":"uint256","name":"tokenFees","type":"uint256"}],"name":"updateValidatortoSSVNetwork","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"validatorCount","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"uint256","name":"","type":"uint256"}],"name":"validatorsRegistry","outputs":[{"internalType":"bytes","name":"pubKey","type":"bytes"},{"internalType":"bytes","name":"withdrawal_credentials","type":"bytes"},{"internalType":"bytes","name":"signature","type":"bytes"},{"internalType":"bytes32","name":"deposit_data_root","type":"bytes32"},{"internalType":"bool","name":"registrationStatus","type":"bool"}],"stateMutability":"view","type":"function"},{"stateMutability":"payable","type":"receive"}]
*/
