...
contract Auctions {
    ...
    function getSingleAuction(address lock, uint nftId) public view returns(Auction memory) {
        (uint index, address seller) = getSellerIndexAuc(lock, nftId);
        Auction memory auction = activeAuctions[seller][index];
        return auction;
    }
    function getAllAuctionsBySeller(address seller) public view returns(Auction[] memory) {
        return activeAuctions[seller];
    }
    function auctionExists(address lock, uint nftId) public view returns(bool) {
        return activeAuctions[sellersAuc[lock][nftId]][indexesAuc[lock][nftId]].lock == lock;
    }
...
contract Orders {
    ...
    //library - public functions
    function getSingleOrder(address lock, uint nftId) public view returns(List memory) {
        (uint index, address seller) = getSellerIndexOrd(lock, nftId);
        List memory list = activeListings[seller][index];
        return list;
    }
    function getAllOrdersBySeller(address seller) public view returns(List[] memory) {
        return activeListings[seller];
    }
    function orderExists(address lock, uint nftId) public view returns(bool) {
        return activeListings[sellers[lock][nftId]][indexes[lock][nftId]].lock == lock;
    }
    function walletPurchasedAmount(address buyer, uint orderId) public view returns(uint) {
...
contract Marketplace is BaseGovernanceUpgradeable, Orders, Auctions {
    ...
    function buyNowOrder(address lock, uint nftId, uint split) external inProgressGuard(lock, nftId) {
        ...
        if(split < order.remainingPart){
            ...
            splitParts[1] = split;
            addresses[1] = buyer;
            uint[] memory newIDs = ILock(lock).split(order.nftId, splitParts, addresses);
