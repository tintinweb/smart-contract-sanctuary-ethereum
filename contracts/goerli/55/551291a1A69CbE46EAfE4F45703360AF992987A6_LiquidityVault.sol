...
//   ====================================== 
    ...
    function _swapProcess (uint256 swapAmount) private {
        ...
        IERC20Metadata(_baseToken).approve(address(swapRouter), swapAmount);
    ...
    {
        // Approve token transfer to cover all possible scenarios
        IERC20Metadata(_baseToken).approve(address(swapRouter), tokenAmount);   
...
//   ======================================
    ...
    function getTokenPrice () public view returns(uint256) {
        ...
        IERC20Metadata T0 = IERC20Metadata(IUniswapV2Pair(_liquidityPair).token0());
        IERC20Metadata T1 = IERC20Metadata(IUniswapV2Pair(_liquidityPair).token1());
    ...
    function releaseTokenReserves () external onlyRole(Financial_Controller) {
        ...
        if (_actionNeed) {
            (uint256 tradedTokens, uint256 tradedCoins) = _liquidityProcess(_tokenBalance, _coinBalance);
             emit LiquidityIncreased (_msgSender(), tradedTokens, tradedCoins); 
             _tokenBalance = IERC20Metadata(_baseToken).balanceOf(address(this));
        }

        if (_tokenBalance > 0 ){
            IERC20Metadata(_baseToken).safeTransfer(externalSafe, _tokenBalance);
    ...
    function liquidityStatus () public view returns (uint256 tokenBalance, uint256 coinBalance, bool actionNeed){
        ...
        tokenBalance = IERC20Metadata(_baseToken).balanceOf(address(this)); 
