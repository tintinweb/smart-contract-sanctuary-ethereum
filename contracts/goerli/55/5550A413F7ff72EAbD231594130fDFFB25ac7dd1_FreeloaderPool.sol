...
contract FreeloaderPool is Ownable {
    ...
    // 注意:如果作恶者新发一个同名代币,这时会导致挖矿用户的累计实现收益和已提取收益出现混乱(两个价值不同的同名代币计算在一起)
    ...
    // 映射:某挖矿对某轮次每个结算周期新增的质押挖矿份额数量:
    mapping(string => mapping(string => mapping(uint256 => mapping(uint256 => uint256)))) public shareUpdateMap;

    // 映射:某挖矿对某轮次某用户质押与取消质押情况:
    mapping(string => mapping(string => mapping(uint256 => mapping(address => AllStakeRecord)))) private allStakeRecordMap;

    // 映射:某挖矿对某轮次某用户质押所在结算周期和质押数量记录:
    // 质押与取消质押可能出现以下这种情况:
    ...
    mapping(string => mapping(string => mapping(uint256 => mapping(address => mapping(uint256 => uint256))))) public stakeCycleRecordMap;

    // 映射:某挖矿对某轮次每个结算周期参与收益结算的总份额变化数:
    // 注意:如果要获取每个结算周期参与收益结算的总份额,就直接累加就可以了,有点UTXO获取比特币余额的意思了
    mapping(string => mapping(string => mapping(uint256 => mapping(uint256 => int256)))) public finalShareMap;

    // 映射:某挖矿对某轮次某用户每个结算周期参与收益结算的总份额变化数:
    // 注意:如果要获取某用户每个结算周期参与收益结算的总份额,就直接累加就可以了,有点UTXO获取比特币余额的意思了
    mapping(string => mapping(string => mapping(uint256 => mapping(address => mapping(uint256 => int256))))) public finalShareByStakerMap;
    ...
    function queryAllStakeRecord(
        ...
        return(_queryAllStakeRecord(_stakeSymbol, _rewardSymbol, _address, _round));
    }
    ...
    function queryRewardShareByCycle(
        ...
        return(_queryRewardShareByCycle(_stakeSymbol, _rewardSymbol, _cycle, _round));
    }
    ...
    function updateMinePair(
        ...
        _updateMinePair(_stakeToken, _rewardToken, _rewardTotalSupply, _totalMineTime, _settlementCycle, _leastShare);
    }
    ...
    function _queryMineInfo(
        ...
        uint256 _round = _info.round;
       
        // 计算当前结算周期每份额产币量:
        uint256 _rewardPerShare = _countRewardPerShare(_stakeSymbol, _rewardSymbol, _round);
    
        // 创建mineInfo结构体:
        mineInfo memory _currentInfo;

        _currentInfo.minePair = _info.minePair;
        _currentInfo.round = _info.round;
    ...
    function _countRewardPerShare(
        ...
        // 计算当前周期参与收益分配的总份额数:
        uint256 _shareByReward = _addUpShareByReward(_stakeSymbol, _rewardSymbol, _round) / 1e18;
    ...
    function _addUpShareByReward(
        ...
        return(_totalShare - shareUpdateMap[_stakeSymbol][_rewardSymbol][_round][_countMineCycle + 1]);
    }
    ...
    function _queryWithdrawInfo(
        ...
        // 获取累计实现收益:
        uint256 _round = _getRound(_stakeSymbol, _rewardSymbol);
        uint256 _totalRewardByStaker = _getTotalReward(_stakeSymbol, _rewardSymbol, _address, _round);

        // 获取可提取收益:
        uint256 _canWithdrawReward = _getCanWithdrawReward(_stakeSymbol, _rewardSymbol, _address, _round);
    ...
    function _queryAllStakeRecord(
        ...
        return(allStakeRecordMap[_stakeSymbol][_rewardSymbol][_round][_address]);
    }
    ...
    function _queryRewardShareByCycle(
        ...
        return(finalShareMap[_stakeSymbol][_rewardSymbol][_round][_cycle]);
    }
    ...
    function _addMinePair(
        ...
        _isFinishOrNotByAdd(_stakeSymbol, _rewardSymbol);
    ...
    // 内部辅助函数,用来启动挖矿对第N(N≥2)轮挖矿:
    function _updateMinePair(
        address _stakeToken,
        address _rewardToken,
        uint256 _rewardTotalSupply,
        uint256 _totalMineTime,
        ...
        // 再检查挖矿对是否已结束一轮挖矿:
        _isFinishOrNotByUpdate(_stakeSymbol, _rewardSymbol);

        // 获取挖矿对更新人地址:
        address _user = _msgSender();

        // 检查更新者是否是最初添加挖矿对的人:
        _isCreator(_stakeSymbol, _rewardSymbol, _user);

        // 将未撤销质押的用户的代币全额返还:
        _returnStakeToken(_stakeSymbol, _rewardSymbol, _stakeToken);
        ...
        // 将更新者的收益代币转到本合约:
        // 注意需要先在ERC20合约中对本合约地址进行approve授权  
        _transferRewardToken(_user, _rewardToken, _rewardTotalSupply);

        // 更新mineInfoMap映射:
        mineInfo memory _info = mineInfoMap[_stakeSymbol][_rewardSymbol];
        mineInfo memory _currentInfo;
        _currentInfo.minePair = _info.minePair;
        _currentInfo.rewardTotalSupply = _rewardTotalSupply;
        _currentInfo.totalMineTime = _totalMineTime;
        _currentInfo.startTimeStamp = 0;
        _currentInfo.alreadyMineTime =  0;
        _currentInfo.alreadyMineAmount = 0;
        _currentInfo.settlementCycle = _settlementCycle;
        _currentInfo.totalShare = 0;      
        _currentInfo.rewardPerShare = 0;
        _currentInfo.leastShare = _leastShare;
        _currentInfo.creator = _info.creator;

        mineInfoMap[_stakeSymbol][_rewardSymbol] = _currentInfo;
    }
    ...
    function _stake( 
        ...
        // 再检查该挖矿对是否已经结束挖矿:
        _isFinishOrNotByStake(_stakeSymbol, _rewardSymbol);
        ...
        // 更新对应的信息(包括份额,质押人信息):
        uint256 _round = _getRound(_stakeSymbol, _rewardSymbol);
        _updateStakeInfo(_stakeSymbol, _rewardSymbol, _amount, _staker, _round);
    ...
    function _unStake(
        ...
        // 更新对应的信息(包括份额,质押人信息)
        uint256 _round = _getRound(_stakeSymbol, _rewardSymbol);
        _updataUnStakeInfo(_stakeSymbol, _rewardSymbol, _amount, _unStaker, _round);
    ...
    function _withdraw(
        ...
        // 检查是否有足够数量的代币可供提取:
        uint256 _round = _getRound(_stakeSymbol, _rewardSymbol);
        _isEnoughByWithdraw(_stakeSymbol, _rewardSymbol, _amount, _user, _round);
    ...
    function _getTotalReward(
        ...
        for(uint256 i = 1; i <= _mineCycle; i++) {
            uint256 _userCycleShare = _countCycleShareByStaker(_stakeSymbol, _rewardSymbol, i, _user, _round);
            uint256 _totalCycleShare = _countShareByCycle(_stakeSymbol, _rewardSymbol, i, _round);
    ...
    function _getCanWithdrawReward(
        ...
        // 计算用户该挖矿对累计实现收益:
        uint256 _userTotalReward = _getTotalReward(_stakeSymbol, _rewardSymbol, _user, _round);
    ...
    // 内部辅助函数,用于启动新一轮挖矿时,将未取消质押的用户的质押代币全额返还:
    function _returnStakeTokenToStaker(
        address _stakeToken,
        address _staker,
        uint256 _amount
        ) internal virtual {
        IERC20 _token = IERC20(_stakeToken);
        _token.transfer(_staker, _amount);
    }
    ...
    function _isFinishOrNotByAdd(
        ...
        // 获取现在的时间戳:
        uint256 _currentTime = _getTimeStamp();

        require(_info.startTimeStamp + _info.totalMineTime < _currentTime, 
                "Sorry, this minePair already completed at least a round of mining, please call the update function");
    }


    // 内部辅助函数,用来检查某挖矿对是否已完成至少一轮挖矿活动(供_updateMinePair函数调用):
    function _isFinishOrNotByUpdate(
        string memory _stakeSymbol,
        string memory _rewardSymbol
        ...
        require(_info.startTimeStamp + _info.totalMineTime >= _currentTime,
                "Sorry, this minePair is mining now, you can't update");
    }


    // 内部辅助函数,用来检查某挖矿对是否已经结束挖矿(供_stake函数调用):
    function _isFinishOrNotByStake(
        string memory _stakeSymbol,
        string memory _rewardSymbol
        ) public virtual view {
    ...
    // 内部辅助函数,用来获取当前某挖矿对的挖矿轮次:
    function _getRound(
        string memory _stakeSymbol,
        string memory _rewardSymbol
        ) internal virtual view returns(uint256) {
        return mineInfoMap[_stakeSymbol][_rewardSymbol].round;
    }
    ...
    function _isCreator(
        ...
        // 检查:
        require(_user == _info.creator, "Sorry, you are not the creator of this minePair");
    }


    // 内部辅助函数,将未撤销质押的用户的代币全额返还:
    function _returnStakeToken(
        string memory _stakeSymbol,
        string memory _rewardSymbol,
        address _stakeToken
        ) internal virtual {    
        
        // 遍历所有质押账户的地址,然后依次返还质押代币:
        address[] memory _allStakeAddr = minerAddr[_stakeSymbol][_rewardSymbol];

        for(uint i = 0; i < _allStakeAddr.length; i++) {

            // 获取质押用户的份额:
            uint256 _amount = minerShareMap[_stakeSymbol][_rewardSymbol][_allStakeAddr[i]];
            _returnStakeTokenToStaker(_stakeToken, _allStakeAddr[i], _amount);

            // 清除份额
            delete minerShareMap[_stakeSymbol][_rewardSymbol][_allStakeAddr[i]];
        }

        // 清除所有质押者地址:
        delete minerAddr[_stakeSymbol][_rewardSymbol];       
    }
    ...
    function _updateStakeInfo(
        ...
        _updateFinalShareByStake(_stakeSymbol, _rewardSymbol, _amount, _countMineCycle, _staker, _round);
    
        // 更新allStakeRecordMap映射:
        _updateAllStakeRecordByStake(_stakeSymbol, _rewardSymbol, _amount, _countMineCycle, _staker, _round);

        // 更新份额信息:
        minerShareMap[_stakeSymbol][_rewardSymbol][_staker] += _amount;
        shareUpdateMap[_stakeSymbol][_rewardSymbol][_round][_countMineCycle + 1] += _amount;
    ...
    function _updataUnStakeInfo(
        ...
        _updateFinalShareByUnStake(_stakeSymbol, _rewardSymbol, _amount, _countMineCycle, _unStaker, _round);

        // 更新allStakeRecordMap映射:
        _updateAllStakeRecordByUnStake(_stakeSymbol, _rewardSymbol, _amount, _countMineCycle, _unStaker, _round);  
    ...
    function _updateFinalShareByStake(
        ...
        if(_countMineCycle == 0) {
            ...
            if(_state) {
                finalShareMap[_stakeSymbol][_rewardSymbol][_round][2] += int256(_amount);
                finalShareByStakerMap[_stakeSymbol][_rewardSymbol][_round][_staker][2] += int256(_amount);
                stakeCycleRecordMap[_stakeSymbol][_rewardSymbol][_round][_staker][2] += _amount;
            } else {
                finalShareMap[_stakeSymbol][_rewardSymbol][_round][1] += int256(_amount);
                finalShareByStakerMap[_stakeSymbol][_rewardSymbol][_round][_staker][1] += int256(_amount);
                stakeCycleRecordMap[_stakeSymbol][_rewardSymbol][_round][_staker][1] += _amount;
            }
        } else {
            // 如果已结算期数不为0,则新加入的质押份额算在第_countMineCycle + 2结算周期内:
            finalShareMap[_stakeSymbol][_rewardSymbol][_round][_countMineCycle + 2] += int256(_amount);     
            finalShareByStakerMap[_stakeSymbol][_rewardSymbol][_round][_staker][_countMineCycle + 2] += int256(_amount);                 
            stakeCycleRecordMap[_stakeSymbol][_rewardSymbol][_round][_staker][_countMineCycle + 2] += _amount;      
        }  
    }
    ...
    function _updateFinalShareByUnStake(
        ...
        // 下结算周期才产生收益的份额数量:
        uint256 _nextCycleShare = stakeCycleRecordMap[_stakeSymbol][_rewardSymbol][_round][_unStaker][_countMineCycle + 2];
        ...
        if(_totalShareByUnStaker - _nextCycleShare < _amount) {
            ...
            finalShareMap[_stakeSymbol][_rewardSymbol][_round][_countMineCycle + 1] -= int256(_tmp);     
            finalShareMap[_stakeSymbol][_rewardSymbol][_round][_countMineCycle + 2] -= int256(_amount - _tmp);  
            finalShareByStakerMap[_stakeSymbol][_rewardSymbol][_round][_unStaker][_countMineCycle + 1] -= int256(_tmp);
            finalShareByStakerMap[_stakeSymbol][_rewardSymbol][_round][_unStaker][_countMineCycle + 2] -= int256(_amount - _tmp);  
        } else {
            finalShareMap[_stakeSymbol][_rewardSymbol][_round][_countMineCycle + 2] -= int256(_amount);    
            finalShareByStakerMap[_stakeSymbol][_rewardSymbol][_round][_unStaker][_countMineCycle + 2] -= int256(_amount);                  
        }
    }   
    ...
    function _updateAllStakeRecordByStake(
        ...
        allStakeRecordMap[_stakeSymbol][_rewardSymbol][_round][_staker].allRecord.push(_newRecord);
    }
    ...
    function _updateAllStakeRecordByUnStake(
        ...
        allStakeRecordMap[_stakeSymbol][_rewardSymbol][_round][_unStaker].allRecord.push(_newRecord);
    }
    ...
    function _countShareByCycle(
        ...
        for(uint256 i = 1; i <= _cycle; i++) {
            int256 _singleCycleShare = finalShareMap[_stakeSymbol][_rewardSymbol][_round][i];
    ...
    function _countCycleShareByStaker(
        ...
        // 遍历每个结算周期收益变动,然后累加:
        for(uint256 i = 1; i <= _cycle; i++) {
            int256 _singleCycleShare = finalShareByStakerMap[_stakeSymbol][_rewardSymbol][_round][_staker][i];
            _totalShareByCycle += _singleCycleShare;    
        }
    ...
    function _isEnoughByWithdraw(
        ...
        // 获取可提取收益:
        uint256 _canWithdrawReward = _getCanWithdrawReward(_stakeSymbol, _rewardSymbol, _user, _round);
