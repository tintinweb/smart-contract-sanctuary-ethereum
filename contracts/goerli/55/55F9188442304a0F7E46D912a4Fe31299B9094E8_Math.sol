...
contract BlockchainRoulette {
    ...
    event TriggerClicked(uint gameIndex);
    event PlayerEliminated(uint gameIndex, address player);

    constructor(uint _minBetAmount) {
        minBetAmount = _minBetAmount;
    }

    function createGame(string memory _betAmount) public payable {
        uint betAmountInWei = parseEther(_betAmount);
        require(betAmountInWei >= minBetAmount, "Bet amount is less than the minimum bet amount.");
        require(msg.value == betAmountInWei, "Send correct bet amount.");

        games.push();
        Game storage newGame = games[games.length - 1];
        newGame.betAmount = betAmountInWei;
        newGame.players.push(Player(payable(msg.sender), betAmountInWei, false));

        emit GameCreated(games.length - 1);
    }

    function parseEther(string memory _amount) public pure returns (uint) {
        return uint(keccak256(abi.encodePacked(_amount))) / 1e18;
    }
    ...
    function pullTrigger(uint gameIndex) public {
        ...
        if (playerIndex == games[gameIndex].bulletPosition) {
            ...
            emit PlayerEliminated(gameIndex, games[gameIndex].players[playerIndex].wallet);
        } else {
            emit TriggerClicked(gameIndex);
        }
    }
}