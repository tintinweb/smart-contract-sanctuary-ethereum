...
contract NFTFactory is Ownable, INFTFactory {
    ...
    /// @param  _royaltyDefaultReceiver Default royalty receiver
    /// @param  _feeNumeratorDefault Default royalty percentage.
    function createNFT(
        string memory _baseUri,
        string memory _name,
        string memory _symbol,
        string[] calldata _series,
        uint256[] calldata _mintPrices,
        uint256[] calldata _maxTokenIdForSeries,
        address _royaltyDefaultReceiver,
        uint96 _feeNumeratorDefault
    ) external onlyOwner returns (address clone) {
        // Create clone
        clone = Clones.clone(proxyImplementation);
        // Initialize the contract
        INFT(clone).initialize(
            _maxTokenIdForSeries.length == 1 ? _maxTokenIdForSeries[0] : _getMaxTokenId(_maxTokenIdForSeries),
            _baseUri,
            _name,
            _symbol,
            ...
            _feeNumeratorDefault
        );
        _createdClones.push(clone);
        // Add NFT in primary market
        IFacilitator(facilitator).addNFTInPrimaryMarket(clone, _mintPrices, _series, _maxTokenIdForSeries);
    ...
    function setGlobalRoyaltyInfo(address _receiver, uint96 _feeNumerator) external onlyOwner {
        require(_feeNumerator <= 10000, "ERC2981: royalty fee will exceed salePrice");
    ...
    function getGlobalRoyaltyInfo() external view returns(GlobalRoyaltyInfo memory) {
    ...
    function _getGlobalRoyaltyInfo() internal view returns(GlobalRoyaltyInfo memory) {
        return GlobalRoyaltyInfo(globalRoyaltyReceiver, feeNumerator);
    }

    function _getMaxTokenId(uint256[] memory _maxTokenIds) internal pure returns(uint256 _maxTokenId) {
        uint256 len = _maxTokenIds.length;
        for (uint256 i = 0; i < len - 1; i++) {
            if (_maxTokenIds[i+1] < _maxTokenIds[i]) {
                revert UnOrderedTokenIds();
            }
            _maxTokenId += _maxTokenIds[i];
        }
