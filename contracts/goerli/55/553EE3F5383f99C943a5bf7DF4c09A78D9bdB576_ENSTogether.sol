...
contract ENSTogether is ReentrancyGuard, Ownable {
    IENSRegistryWithFallback ensRegistry;
    IReverseRegistrar ensReverseRegistrar;
    IENSTogetherNFT ensTogetherNFT;

    uint256 private cost = 0.08 ether;
    uint256 private updateStatusCost = 0.04 ether;
    uint32 public timeToRespond = 1 weeks;
    ...
    constructor(address ensReverseRegistrar_, address ensRegistry_) {
        ensReverseRegistrar = IReverseRegistrar(ensReverseRegistrar_);
        ensRegistry = IENSRegistryWithFallback(ensRegistry_);
    }
    ...
    function lookupENSName(address addr) public view returns (string memory) {
        ...
        if (resolverAddress == address(0) && addr == msg.sender) {
            revert("You dont own an ENS name");
        } else if (resolverAddress == address(0) && addr != msg.sender) {
            revert("No ENS name for that address");
        }
        IDefaultReverseResolver ensResolver = IDefaultReverseResolver(
            resolverAddress
        );
        return ensResolver.name(node);
    }

    function respondToProposal(Proposal response)
        external
        payable
    {
        ...
        //Only the receiver can respond the proposal
        require(
        ...
        acceptOrDecline.createdAt = uint32(block.timestamp);
        ...
        //ACCEPT SCENARIO AND GET UNITED
            ...
            string memory ens1 = lookupENSName(unionWith[msg.sender].from);
            string memory ens2 = lookupENSName(unionWith[msg.sender].to);
            ...
            emit GotUnited(
                ...
                acceptOrDecline.registryNumber
            );
    ...
    function updateUnion(Status newStatus) external payable {
        ...
        unionUpdated.createdAt = uint32(block.timestamp);
    ...
    function modifyTimeToRespond(uint32 t) external onlyOwner {
        timeToRespond = t;
    }
