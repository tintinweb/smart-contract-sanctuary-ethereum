...
library PoolLogic {
  ...
  function executeResetIsolationModeTotalDebt(
    ...
    require(reservesData[asset].config.getDebtCeiling() == 0, Errors.DEBT_CEILING_NOT_ZERO);
...
library ReserveConfig {
  ...
  function setStableRateBorrowingEnabled(
    DataTypes.ReserveConfigMap memory self,
    bool enabled
  ) internal pure {
    self.data =
   ...
   **/
  function getFlags(DataTypes.ReserveConfigMap memory self)
    internal
    pure
    returns (
      bool,
   ...
   **/
  function getParams(DataTypes.ReserveConfigMap memory self)
    internal
    pure
    returns (
      uint256,
   ...
   **/
  function getCaps(DataTypes.ReserveConfigMap memory self)
    internal
    pure
 ...
 * @author Monetaria
 * @notice Defines the error messages emitted by the different contracts of the Monetaria protocol
 */
library Errors {
  ...
  string public constant CALLER_NOT_MTOKEN = '11'; // 'The caller of the function is not an MToken'
  ...
  string public constant MTOKEN_SUPPLY_NOT_ZERO = '54'; // 'MToken supply is not zero'
...
library DataTypes {
  ...
  struct ReserveConfigMap {
    ...
    //bit 212-251 debt ceiling for isolation mode with (ReserveConfig::DEBT_CEILING_DECIMALS) decimals
    //bit 252-255 unused

    uint256 data;
  }

  struct UserConfigMap {
    /**
  ...
  struct ReserveCache {
    ...
    ReserveConfigMap reserveConfig;
    address mTokenAddress;
    address stableDebtTokenAddress;
    address variableDebtTokenAddress;
...
library ReserveLogic {
  ...
  function updateInterestRates(
    ...
    (
      ...
      DataTypes.CalculateInterestRatesParams({
        unbacked: reserveCache.reserveConfig.getUnbackedMintCap() != 0
          ? reserve.unbacked
          : 0,
    ...
    returns (DataTypes.ReserveCache memory)
  {
    ...
    reserveCache.reserveConfig = reserve.config;
    reserveCache.reserveFactor = reserveCache.reserveConfig.getReserveFactor();
...
library ValidationLogic {
  ...
  function validateWithdraw(
    ...
    (bool isActive, , , , bool isPaused) = reserveCache.reserveConfig.getFlags();
    require(isActive, Errors.RESERVE_INACTIVE);
    require(!isPaused, Errors.RESERVE_PAUSED);
  }
  ...
  function validateBorrow(
    ...
    (
      ...
      vars.isPaused
    ) = params.reserveCache.reserveConfig.getFlags();
    ...
    vars.reserveDecimals = params.reserveCache.reserveConfig.getDecimals();
    vars.borrowCap = params.reserveCache.reserveConfig.getBorrowCap();
    unchecked {
    ...
    if (params.isolationModeActive) {
      ...
      require(
        params.reserveCache.reserveConfig.getBorrowableInIsolation(),
        Errors.ASSET_NOT_BORROWABLE_IN_ISOLATION
      );

      require(
        reservesData[params.isolationModeCollateralAddress].isolationModeTotalDebt +
          (params.amount / 10**(vars.reserveDecimals - ReserveConfig.DEBT_CEILING_DECIMALS))
            .toUint128() <=
    ...
    if (params.userEModeCategory != 0) {
      require(
        params.reserveCache.reserveConfig.getEModeCategory() == params.userEModeCategory,
    ...
    if (params.userConfig.isBorrowingAny()) {
      ...
      if (vars.siloedBorrowingEnabled) {
        ...
        require(
          !params.reserveCache.reserveConfig.getSiloedBorrowing(),
   ...
   * @param reserveCache The cached data of the reserve
   * @param userConfig The user reserves config
  ...
  function validateFlashloan(
    ...
    for (uint256 i = 0; i < assets.length; i++) {
      DataTypes.ReserveConfigMap memory config = reservesData[assets[i]]
        .config;
      require(!config.getPaused(), Errors.RESERVE_PAUSED);
      require(config.getActive(), Errors.RESERVE_INACTIVE);
    }
  }
  ...
  function validateFlashloanSimple(DataTypes.ReserveData storage reserve) internal view {
    DataTypes.ReserveConfigMap memory config = reserve.config;
    require(!config.getPaused(), Errors.RESERVE_PAUSED);
    require(config.getActive(), Errors.RESERVE_INACTIVE);
  }
  ...
  function validateLiquidationCall(
    ...
    vars.isCollateralEnabled =
      collateralReserve.config.getLiquidationThreshold() != 0 &&
  ...
  function validateHealthFactor(
    ...
    mapping(uint256 => address) storage reservesList,
    mapping(uint8 => DataTypes.EModeCategory) storage eModeCategories,
    DataTypes.UserConfigMap memory userConfig,
    address user,
    uint8 userEModeCategory,
    uint256 reservesCount,
    address oracle
  ...
  function validateHFAndLtv(
    ...
    require(
      !hasZeroLtvCollateral || reserve.config.getLtv() == 0,
  ...
  function validateTransfer(DataTypes.ReserveData storage reserve) internal view {
    require(!reserve.config.getPaused(), Errors.RESERVE_PAUSED);
  }
   ...
   * @param reservesData The state of all the reserves
   * @param reservesList The addresses of all the active reserves
   * @param eModeCategories a mapping storing configs for all efficiency mode categories
      ...
      unchecked {
        for (uint256 i = 0; i < reservesCount; i++) {
          if (userConfig.isBorrowing(i)) {
            DataTypes.ReserveConfigMap memory config = reservesData[reservesList[i]]
              .config;
            require(
  ...
  function validateUseAsCollateral(
    ...
    mapping(uint256 => address) storage reservesList,
    DataTypes.UserConfigMap storage userConfig,
    DataTypes.ReserveConfigMap memory reserveConfig
...
library GenericLogic {
    ...
    )
  {
    ...
    while (vars.i < params.reservesCount) {
      ...
      ) = currentReserve.config.getParams();
...
/**
 * @title IMonetariaIncentivesController
 * @author Monetaria
 * @notice Defines the basic interface for a Monetaria Incentives Controller.
 **/
interface IMonetariaIncentivesController {
  /**
  ...
  /**
   * @notice Returns the config of the distribution for a certain asset
  ...
  /**
   * LEGACY **************************
   * @dev Returns the config of the distribution for a certain asset
   * @param asset The address of the reference asset of the distribution
...
interface IPool {
  ...
  /**
   * @dev Mints an `amount` of mTokens to the `onBehalfOf`
   ...
   * into consideration. For further details please visit https://developers.monetaria.com
...
interface IReserveInterestRateStrategy {
  ...
  /**
   * @notice Calculates the interest rates depending on the reserve's state and configs
 ...
 * @author Monetaria
 * @notice Interface for the Monetaria price oracle.
 **/
...
library UserConfig {
  ...
  function setBorrowing(
    ...
    unchecked {
      require(reserveIndex < ReserveConfig.MAX_RESERVES_COUNT, Errors.INVALID_RESERVE_INDEX);
...
library EModeLogic {
  ...
  /**
   * @notice Gets the eMode config and calculates the eMode asset price if a custom oracle is configured
