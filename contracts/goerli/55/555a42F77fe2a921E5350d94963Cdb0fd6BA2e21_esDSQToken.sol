 ...
 * @dev     Only whitelisted addresses can tranfer, transferFrom, burn or burnFrom during restrictedTransfer
 * @author  HessianX
...
contract esDSQToken is ERC20, ERC20Burnable, AccessControl {
    ...
    event NewRestrictedTransfer(bool oldValue, bool newValue);
    ...
    /// @notice Mapping of permanently whitelisted addresses
    mapping(address => bool) private permanentWhitelist;
    ...
    function getWhitelistStatus(address _address) external view returns (bool, bool) {
        return (whitelist.contains(_address), permanentWhitelist[_address]);
    }
    ...
    function setRestrictedTransfer(bool _restrictedTransfer) external onlyRole(DEFAULT_ADMIN_ROLE) {
        emit NewRestrictedTransfer(restrictedTransfer, _restrictedTransfer);
    ...
    function addWhitelistAddress(address _address, bool _permanent) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (_permanent) permanentWhitelist[_address] = true;
    ...
    function removeWhitelistAddress(address _address) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!permanentWhitelist[_address], "esDSQToken: Address is permanently whitelisted");
