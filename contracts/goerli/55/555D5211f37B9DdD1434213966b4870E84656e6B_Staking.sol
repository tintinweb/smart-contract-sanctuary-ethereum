...
contract Staking is Initializable{
    ...
    function initialize(address _shidoToken)external initializer{
        ...
        lockInPeriod = 60; //30days for mainnet
    ...
    function stake(uint256 _amount) external{
        ...
        if(userStake[msg.sender].amount==0){
            ...
            userStake[msg.sender].depositTime = uint128(block.timestamp);
        }

        userStake[msg.sender] = Stake(userStake[msg.sender].amount += _amount,userStake[msg.sender].unlockTime,userStake[msg.sender].depositTime,userStake[msg.sender].claimedReward,userStake[msg.sender].stakedShido);
    ...
    function emergencyExit(uint256 _amount) external {
        ...
        if(_amount == userStakeData.amount){
            ...
            fee = _amount*emergencyExitFees / 100; // 10% fee
    ...
    function stakeShidoReward()external{
        uint256 currentReward = calculateReward(msg.sender, userStake[msg.sender].amount);
        if(currentReward == 0){revert customErrors.lessThanZeroAmount(currentReward);}

        totalStakes += currentReward;
        userStake[msg.sender] = Stake(userStake[msg.sender].amount + currentReward,userStake[msg.sender].unlockTime,uint128(block.timestamp),userStake[msg.sender].claimedReward,userStake[msg.sender].stakedShido + uint128(currentReward));
    ...
    function getUserReward(address _add)external view returns(uint256 _reward){
    ...
    function calculateReward(address user,uint256 _amount) public view returns (uint256) {
        ...
        uint256 reward = (_amount * rewardRate * time) / (300 * 100 * 100);
        return reward;
    }
