...
contract AnimalCrowdsale is AccessControl, PullPayment, Ownable {
    ...
    PubMintState public pState;
    MintState public mState;
    ...
    event PubMintStateChange(PubMintState state);
    ...
    function mint(uint256 _potionId, uint256 _halflingId) external {
        require(mState == MintState.ALLOW ,"AnimalCrowdsale: Wrong state for this action");
        require(
            mintNum[msg.sender].add(1) <= oLimit,
    ...
    function pubMint(uint256 _amount) external payable {
        require(pState == PubMintState.ALLOW ,"AnimalCrowdsale: Wrong state for this action");
        require(
            _amount <= sLimit,
        ...
        Animal(token).mint(msg.sender, _amount);
    ...
    function setMintState(MintState _state) external onlyRole(CROWD_ROLE) {
        mState = _state;
        emit MintStateChange(_state);
    }
    function setPubMintState(PubMintState _state) external onlyRole(CROWD_ROLE) {
        pState = _state;
        emit PubMintStateChange(_state);
    }
    ...
    function saleState() external  view returns (MintState,PubMintState,uint256) {
        return (mState,pState,salePrice);
    }
    ...
    NativeMetaTransaction
{
    ...
    constructor() ERC721A("NRJTigerStyle", "NRJT") {
        _initializeEIP712("NRJTigerStyle");
        baseTokenURI = "https://cdn.nftstar.com/neymar/animal/metadata/";
        collectionURI = "https://cdn.nftstar.com/neymar/animal/contract.json";
    ...
    NativeMetaTransaction
{
    ...
    constructor() ERC721A("NRJPotionX", "NRJPX") {
        _initializeEIP712("NRJPotionX");
        baseTokenURI = "https://cdn.nftstar.com/neymar/potionx/metadata/";
        collectionURI = "https://cdn.nftstar.com/neymar/potionx/contract.json";
        // Grant the contract deployer the default admin role: it will be able to grant and revoke any roles
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSenderERC721A());
        _setupRole(MINER_ROLE, _msgSenderERC721A());
        _setupRole(PAUSER_ROLE, _msgSenderERC721A());
    ...
    NativeMetaTransaction
{
    ...
    constructor() ERC721A("NRJHalfling", "NRJH") {
        _initializeEIP712("NRJHalfling");
        baseTokenURI = "https://cdn.nftstar.com/neymar/halfling/metadata/";
        collectionURI = "https://cdn.nftstar.com/neymar/halfling/contract.json";
        // Grant the contract deployer the default admin role: it will be able to grant and revoke any roles
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSenderERC721A());
        _setupRole(MINER_ROLE, _msgSenderERC721A());
        _setupRole(PAUSER_ROLE, _msgSenderERC721A());
