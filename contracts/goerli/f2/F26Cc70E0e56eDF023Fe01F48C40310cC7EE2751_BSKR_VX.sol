...
contract BSKR_VX is BaseBSKR {
    ...
    uint256 private _max_tx_amount; // 40 million - 4% of the total supply
    uint256 private _num_tokens_for_lp; // 1 million - 0.1% of the total supply
    uint256 private totalReflection;
    uint32[6] private _currFees;
    ...
    function __BSKR_VX_init(
        ...
        // _max_tx_amount = 0x2116545850052128000000; // 40 million - 4% of the total supply
        // _num_tokens_for_lp = 0xD3C21BCECCEDA1000000; // 1 million - 0.1% of the total supply
        ...
        if (
            ...
            _max_tx_amount = 0x813F3978F894098440000000; // 40 billion
            _num_tokens_for_lp = 0x33B2E3C9FD0803CE8000000; // 1 billion
        } else {
        ...
        _addLPEnabled = true;
        // _rTotal = (type(uint256).max - (type(uint256).max % _totalSupply)); // this might have changed and we want to preserve it
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            require(amount <= _max_tx_amount, "B: Exceeds max tx size");
        }

        if (!isV3Enabled) {
            require(!v3PairInvolved(from, to), "B: UniV3 is not supported!");
        }
