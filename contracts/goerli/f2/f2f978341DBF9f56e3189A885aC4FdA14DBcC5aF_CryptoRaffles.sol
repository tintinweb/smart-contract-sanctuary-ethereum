...
contract RaffleStorage is Ownable, ReentrancyGuard {
    ...
    mapping (address => address) internal playerToAffilate;
    ...
    event PlayerJoined(address player, address affilate, uint256 numOfTickets);
    ...
    error UnableToDetermineTokenOwner();
     ...
     */
    function read(uint256 raffleId, bytes4 selector, address key) internal view returns (uint256) {
        return raffles[raffleId]._storage[selector][key];
    }

    /**
     * @dev Internal function to return owner of token id for specific raffle id
     */
    function ownershipOf(uint256 raffleId, uint256 tokenId) internal view returns (address) {
        ...
        revert UnableToDetermineTokenOwner();
    }
...
contract CryptoRaffles is RaffleStorage, VRFAdministrator {
        ...
        isState(raffleId, RAFFLE_STATE.OPEN)
    {
        ...
        emit PlayerJoined(_msgSender(), affilate, numOfTokens);
    }
    ...
    function startRaffle(
        ...
        returns(uint256)
    {
        // TIME VALUE MUST BE CHANGED BEFORE DEPLOYMENT - ONLY FOR TESTING PURPOSES !!!
    ...
    function withdrawPrize(uint256 raffleId) external isState(raffleId, RAFFLE_STATE.RAFFLED) nonReentrant {
        ...
        if (read(raffleId, playerToGrandPrizeTokenSelector, _msgSender()) != 0) {
            ...
            grandAmount = calcMargin(raffles[raffleId].treasury, raffles[raffleId].grandPrizeMargin);
            ...
            minorAmount = numTokens * (totalAmount / raffles[raffleId].numBonusWins);
            // clears players prize token to protect from multiple withdrawals
        ...
        uint256 amount = grandAmount + minorAmount;
        ...
        returns (uint256 amount)
    {
        ...
        if (read(raffleId, affilateToNumOfTokensSelector, _msgSender()) != 0) {
            uint256 totalAmount = read(raffleId, affilateToNumOfTokensSelector, _msgSender()) * raffles[raffleId].entryFee;
            amount1 = calcMargin(totalAmount, affilateMargin);
        } 
        
        if (read(raffleId, playerToGrandPrizeTokenSelector, _msgSender()) != 0) {
            amount2 = calcMargin(raffles[raffleId].treasury, raffles[raffleId].grandPrizeMargin);
        } 
        
        if (read(raffleId, playerToMinorPrizeNumTokensSelector, _msgSender()) != 0) {
            uint256 totalAmount = calcMargin(raffles[raffleId].treasury, raffles[raffleId].minorPrizeMargin);
            uint256 numTokens = read(raffleId, playerToMinorPrizeNumTokensSelector, _msgSender());
            amount3 = numTokens * (totalAmount / raffles[raffleId].numBonusWins);
        } 
        
        if (raffles[raffleId].owner == _msgSender()) {
            uint256 minorPrizeAmount = calcMargin(raffles[raffleId].treasury, raffles[raffleId].minorPrizeMargin);
            uint256 grandPrizeAmount = calcMargin(raffles[raffleId].treasury, raffles[raffleId].grandPrizeMargin);
            uint256 affilateAmount = raffles[raffleId].affilateTokensCounter * raffles[raffleId].entryFee;
            uint256 affilateMarginAmount = calcMargin(affilateAmount, affilateMargin);
            amount4 = raffles[raffleId].treasury - (minorPrizeAmount + grandPrizeAmount + affilateMarginAmount);
        }
