 ...
 * 2. Localhost에 배포 후, 한쪽에는 deploy.js // 한쪽에는 hardhat.config.js 
 * // 밑의 terminal에서는 한쪽에는 같은 컨트랙트를 4번 실행(2번은 그냥, 2번은 localhost에) 
 * // 한쪽에는 hardhat. node 실행 -> 주소 변화 강조하면 더 좋음
...
contract A {
    ...
    /* Update array */
    function pushToArr(uint _num) public {
        arr.push(_num);
    }
    ...
    /* Compare */
    function getLargerNum(uint _a, uint _b) public pure returns(uint) {
        if (_a >= _b) {
            return _a;
        } else {
            return _b;
