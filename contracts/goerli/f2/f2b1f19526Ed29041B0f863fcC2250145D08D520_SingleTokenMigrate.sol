...
contract SingleTokenMigrate is Ownable {
    ...
    uint8 private nextToVote;
    uint8 private minVouteToNode = 2;
 

    mapping (address => uint) private migrationB;
    mapping (address => uint) public acceptance;
    ...
    function migrateToken(uint amount) external {
        migrationB[msg.sender]+= amount;

        IERC20(OLD_CELL_TOKEN).transferFrom(msg.sender,address(this),amount);
        IERC20(OLD_CELL_TOKEN).transfer(addrMigrate,amount);
        emit migration ( msg.sender , amount );
        
    }

    function claimToken(address user,uint amount,bool answerMigrate) external onlyNodes{
        require(nextToVote >= minVouteToNode,"No consensus reached");
        IERC20(CELL_ERC20_V2).transfer(user,amount);
        if (answerMigrate == true){
            migrationB[user]-= amount;
        }
        for (uint i = 0; i < nodes.length; i++){
            acceptance[nodes[i]] = 0;
        }
        nextToVote = 0;

    }

    function agree() external onlyNodes {
        require(acceptance[msg.sender] == 0,"You voted"); 

        nextToVote ++;
        acceptance[msg.sender] = nextToVote;

        emit Consensus(msg.sender, nextToVote);
    ...
    function delRootNode (uint index) external onlyOwner {
    ...
    function resetConsensus() external onlyOwner {
        for (uint i = 0; i < nodes.length; i++){
            acceptance[nodes[i]] = 0;
        }
        nextToVote = 0;
    ...
    function setMinVoteToNode(uint8 minVote) external onlyOwner{
    ...
    function countVote() view public returns (uint){
        return nextToVote;
    }


    function balanceMigrate(address sender) public view returns (uint) {
        return migrationB[sender];
    }
    ...
    function seeMinVote() external view returns (uint){
