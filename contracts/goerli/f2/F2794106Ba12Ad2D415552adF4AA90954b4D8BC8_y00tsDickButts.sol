...
contract ClaimBitmap {
    ...
    function isClaimed(uint256 tokenId) public view returns (bool) {
      ...
      /**
     * @notice checks to see if a token id has been used for FREE claimed
     * @param tokenId the token id
     */
    function isUsedForFreeClaim(uint256 tokenId) public view returns (bool) {
        uint256 claimedWordIndex = tokenId / 256;
        uint256 claimedBitIndex = tokenId % 256;
        uint256 claimedWord = claimedBitmap[claimedWordIndex];
        uint256 mask = (1 << claimedBitIndex);
    ...
    Ownable
{
    ...
    {
        uint256 reservePublicSupply = maxPublicSupply;
        require((reservePublicSupply < numberOfTokens), "No more public Supply Available ");
        if (reservePublicSupply < numberOfTokens) revert ExceedMaximumPublicSupply();

        maxPublicSupply = reservePublicSupply - numberOfTokens;
