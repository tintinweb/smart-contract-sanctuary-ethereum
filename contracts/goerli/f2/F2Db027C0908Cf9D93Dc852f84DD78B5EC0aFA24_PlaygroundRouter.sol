...
contract PlaygroundRouter is IPlaygroundRouter, FeeUtil {
        ...
        returns (uint256[] memory amounts)
    {
        ...
        amounts = PlaygroundLibrary.getAmountsOut(factory, msg.value, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "PlaygroundRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        if (path[0] == pairFeeAddress[pair]) {
            uint256 appliedFee = (amountIn * fee) / (10000);
        ...
        amountOutMin;
        if (path[1] == pairFeeAddress[pair]) {
            uint256 appliedFee = (amountOut * fee) / (10000);
...
abstract contract FeeUtil is Ownable {
    ...
    function setFee(address _feeTo, uint256 _fee) external onlyOwner {
        require(_fee <= 10, "Playground::FeeUtil: Fee exceeds 1%");
        feeTo = _feeTo;
        fee = _fee;
    }
}
...
library PlaygroundLibrary {
    ...
    function pairFor(
        ...
        bytes32 _data = keccak256(
            abi.encodePacked(
                ...
                hex"32d9f641aa50574c6546fd47095c87187b52cf80984243e2db04f432ede80718"
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IPlaygroundPair(
