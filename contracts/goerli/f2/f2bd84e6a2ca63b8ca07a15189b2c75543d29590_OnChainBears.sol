...
contract OnChainBears is IOnChainBears, Ownable, ERC721AQueryable {
    ...
    /// `pixels`: A string representation of the pixel placement associated with this trait.
    struct Trait {
        string name;
        string value;
        string pixels;
    ...
    /// need to minus 97, hence our `_ASCII_OFFSET` value.
    ///
    ...
    function _getMetadata(uint256 id, Trait[] memory _traits) internal pure returns (string memory) {
        return string(abi.encodePacked(
            ...
            abi.encodePacked(
                '{"name":"On Chain Bears #', _toString(id),
                '","description":"On Chain Bears is a passion project inspired by the concept of on-chain NFTs, NFTs with no dependence on the outside world or external services such as IPFS. Rendered directly from the blockchain and destined to remain there until the collapse of Ethereum itself. In the true spirit of decentralisation; CC0, 100% on-chain and 0% royalties.',
