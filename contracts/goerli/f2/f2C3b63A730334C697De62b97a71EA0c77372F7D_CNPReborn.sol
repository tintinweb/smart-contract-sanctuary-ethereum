    ...
    ERC721MultiSaleByMerkleMultiWallet
{
    ...
    function getLastTransferOrGimmickDay(uint256 tokenId)
        ...
        returns (uint256)
    {
        return lastTransferOrGimmickDay[tokenId];
    }
    ...
    function setWalletLock(address to, LockStatus lockStatus) external {
        ...
        bool beforeIsWalletLock = walletLock[to] == LockStatus.Lock;
        uint256[] memory tokenIds = this.tokensOfOwner(to);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (
                beforeIsWalletLock &&
                tokenLock[tokenIds[i]] == LockStatus.UnSet &&
                lockStatus != LockStatus.Lock
            ) {
                unlockTimestamp[tokenIds[i]] = block.timestamp;
            }
        }
        ...
        returns (bool)
    {
        return
            ...
            (unlockTimestamp[tokenId] + unlockLeadTime > block.timestamp);
    }
