...
contract KyvuNFT is ERC721URIStorage, EIP712 {
    ...
    function changeActiveAmount(uint x) onlyOwner external {
    ...
    struct offersTracker {
        uint256 noOfElements;
        mapping(uint256 => offers) listOfOffers;
    }
    mapping(address => mapping(string => offersTracker)) availableOffers;
    mapping(string => uint) uriToTokenID;
        ...
        payable
    {
        ...
        require(uriToTokenID[voucher.uri]==0, "This URI already exists on chain");

        require(msg.value >= voucher.minPrice, "Value sent is lower than min. price");
        //Doubt: Should I just mint it to the redeemer?
        _mint(redeemer, currentTokenId);
    ...
    function makeOffer(
        ...
        availableOffers[_creator][_tokenURI].noOfElements++;
        uint256 x = availableOffers[_creator][_tokenURI].noOfElements;
        availableOffers[_creator][_tokenURI].listOfOffers[x] = offer1;
    }

    function acceptOffer(
        address _creator,
        string memory _tokenURI,
        uint256 indexOfOffer
    ) public {
        require(_creator == msg.sender, "Only creators can mint.");
        require(
            block.timestamp <
                availableOffers[_creator][_tokenURI]
                    .listOfOffers[indexOfOffer]
                    .endTime
        );
        address receiverOfNFT = availableOffers[_creator][_tokenURI]
            .listOfOffers[indexOfOffer]
            .offerer;
        ...
        if (activeFee) {
            uint256 valueX = (availableOffers[_creator][_tokenURI]
                .listOfOffers[indexOfOffer]
                .approvedTokenAmount * effectiveAmount) / 100;
            IERC20(weth).transferFrom(msg.sender, _creator, valueX);
            IERC20(weth).transferFrom(
                msg.sender,
                owner,
                availableOffers[_creator][_tokenURI]
                    .listOfOffers[indexOfOffer]
                    .approvedTokenAmount - valueX
            );
        } else {
            IERC20(weth).transferFrom(
                msg.sender,
