...
contract Sanctum is ERC721A, Ownable{
    ...
    function Whitelist_mint(uint _count) external payable{ 
       ...
       // require(MerkleProof.verify(merkleProof,whitelistMerkleRoot,keccak256(abi.encodePacked(msg.sender))),"You are not in Presale List.");
        require(_count <= maxPerWalletPresale, "max per transaction 2");
        require(totalSupply() + _count <= maxSupply, "Sold Out!");
     ...
     function Genesis_mint(uint _count) external{ 
        require(Genesis_status == true, "Sale is Paused.");
       // require(MerkleProof.verify(merkleProof,GenesisMerkleRoot,keccak256(abi.encodePacked(msg.sender))));
        ...
        require(totalSupply() + _count<= GenesismaxSupply, "Genesis Collection Sold out");
         require((totalGenesisMint[msg.sender] +_count) <= maxPerGenesis, "1 tokens per wallet allowed in presale");
      ...
      function Genesis_checker(address walletAddress, bytes32[] calldata merkleProof) public view returns (bool){ 
        ...
        function Genesis_holders(address walletAddress) public view returns (uint){ 
            uint value=0;
            for(uint i=1; i<=GenesismaxSupply; i++)
            {
