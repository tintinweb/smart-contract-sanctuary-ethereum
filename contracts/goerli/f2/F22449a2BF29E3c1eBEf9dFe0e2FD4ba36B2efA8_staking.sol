...
/** 
 *  SourceUnit: /Users/kenzman/Desktop/code/Solidity/Staking contract/contracts/staking.sol
*/
...
function stake(uint _amount) public {
    ...
    function claimReward(uint _amount) public {
        calculateReward();   
        stakerDetail storage staker = stakers[msg.sender];
        uint claimAbleAmount = staker.tokenEarned;
        ...
        if (_amount == claimAbleAmount){
            rewardToken.transfer(msg.sender, claimAbleAmount);
             staker.tokenEarned = 0;
        }else {
            rewardToken.transfer(msg.sender, _amount);
           uint rewardLeft = claimAbleAmount - _amount;
            staker.tokenEarned = rewardLeft;
        }
    }

    function withdrawStaking()public {
        calculateReward();
       uint staked = stakers[msg.sender].amount;
       uint reward = stakers[msg.sender].tokenEarned;
       ...
       stakers[msg.sender].initialStaketime = 0;
       stakers[msg.sender].stakeStatus = false;
       stakers[msg.sender].tokenEarned = 0;
    }
