...
contract DZooStaking is ReentrancyGuard, Ownable {
    ...
    uint256 public fractionalizationRate;
    ...
    constructor(
        ...
        uint256 _fractionalizationRate
    ) {
        rewardsToken = IDZooNFT(_rewardsToken);
        stakingToken = IERC20(_stakingToken);
        fractionalizationRate = _fractionalizationRate;
    }
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        uint256 claimable = reward.mod(fractionalizationRate); // whole "integer" of caller
        rewards[msg.sender] = reward.sub(claimable.mul(fractionalizationRate)); // update remainder fractional rewards
        rewardsToken.mint(msg.sender, claimable);
        nftsMintedviaStake.add(claimable);
    ...
    function notifyRewardAmount(
        ...
        nftsAssignedtoStake.add(nfts);

        // Ensure the provided reward amount is not more than the remaining NFTs to assign.
        // This keeps the reward rate in the right range, preventing overflows due to
        // very high values of rewardRate in the earned and rewardsPerToken functions;
        // Reward + leftover must be less than 2^256 / 10^18 to avoid overflow.
        uint256 balance = (nftsAssignedtoStake.sub(nftsMintedviaStake)).mul(
            fractionalizationRate
        );
        //uint256 balance = (rewardsToken.MAX_SUPPLY.sub(rewardsToken.totaSupply)).mul(fractionalizationRate);
        require(
