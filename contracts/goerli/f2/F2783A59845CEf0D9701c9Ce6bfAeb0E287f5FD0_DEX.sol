...
contract DEX {
     ...
     * NOTE: since ratio is 1:1, this is fine to initialize the totalLiquidity (wrt to balloons) as equal
     * to eth balance of contract.
     */
    function init(uint256 tokens) public payable returns (uint256) {
        require(totalLiquidity == 0, "Init balance more than 0");
     ...
     * NOTE: parameter is the msg.value sent with this function call. That amount is used
     * to determine the amount of $BAL needed as well and taken from the depositor.
     * NOTE: user has to make sure to give DEX approval to spend their tokens on their
     * behalf by calling approve function prior to this function call.
     * NOTE: Equal parts of both assets will be removed from the user's wallet with respect
     * to the price outlined by the AMM.
     */
    function deposit() public payable returns (uint256 tokensDeposited) {
        require(msg.value > 0, "Must send value when depositing");

        uint256 ethReserve = address(this).balance - msg.value;
        uint256 tokensDeposit = (token.balanceOf(address(this)) * msg.value) /
            ethReserve +
            1;

        uint256 mintedLiquidity = (totalLiquidity * msg.value) / ethReserve;
     ...
     *  very little back if the liquidity is super low in the pool. I guess
     *  they could see that with the UI.
     */
    function withdraw(
        uint256 amount
        ...
        require(
            ...
            "You don't have enough liquidity"
        );
