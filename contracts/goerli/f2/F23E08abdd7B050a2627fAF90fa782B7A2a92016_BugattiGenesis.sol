...
contract BugattiGenesis is ERC721A, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
    ...
    event TokenClaimed(uint256[] indexed _tokenIds, address _address);
    event TokenBlended(uint256 indexed _newTokenId, uint256 _newTokenType, address _address);

    constructor(string memory _hiddenURI, address _server) ERC721A("Bugatti Genesis", "BG") {
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        require(totalMinted + _mintAmount <= maxSupply, "MINT: Max Supply Exceeded.");
        _;
    }
    ...
    function setMaxSupply(uint256 _newMaxSupply) public onlyOwner {
        require(_newMaxSupply >= totalMinted && _newMaxSupply <= maxSupply, "Invalid Max Supply.");
    ...
    function blend(uint256 _recipeId, uint256[] calldata _tokenIds, bytes memory _signature) external notPaused {
        ...
        emit TokenBlended(totalMinted, _recipeId + 1, _msgSender());
    }
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        bool success;
        (success, ) = payable(0x1560D5fa5Ee6AA197655e075063BAdb1aA6596C9).call{value: balance}("");
