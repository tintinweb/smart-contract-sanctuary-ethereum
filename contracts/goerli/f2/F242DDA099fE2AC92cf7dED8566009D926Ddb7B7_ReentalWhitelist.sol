...
contract ReentalWhitelist is UUPSUpgradeableByRole, EIP712Upgradeable, RefundableUpgradeable {
  ...
  function addBatch(address[] memory accounts, uint256[] memory retentions) public onlyRole(_WHITELIST_ADMIN_ROLE) {
    require(accounts.length == retentions.length, "ReentalWhitelist: length of arrays must match");
    for(uint256 i = 0; i < accounts.length; ++i) {
      address account = accounts[i];
      uint256 retention = retentions[i];
      _add(account, retention);
    }
  }
  ...
  function removeBatch(address[] memory accounts) public onlyRole(_WHITELIST_ADMIN_ROLE) {
    for(uint256 i = 0; i < accounts.length; ++i) {
      address account = accounts[i];
  ...
  function _add(address account, uint256 retention) internal whenNotPaused {
    if(_whitelisted[account] && retentionPercent[account] == retention) {
      revert("ReentalWhitelist: account already whitelisted");
    }
    if(!_whitelisted[account]) {
      _whitelisted[account] = true;
      idOf[account] = accounts;
      accountOf[accounts] = account;
      ++accounts;
      emit Added(account, retention);
    }
    if(retentionPercent[account] != retention) {
      _updateRetention(account, retention);
    }
  }
  ...
  function _remove(address account) internal {
    ...
    address lastAccount = accountOf[accounts - 1];
    uint256 lastId = idOf[lastAccount];

    idOf[lastAccount] = 0;
    accountOf[lastId] = address(0);

    --accounts;

    accountOf[idOf[account]] = lastAccount;
    idOf[account] = lastId;

    _whitelisted[account] = false;
    retentionPercent[account] = 0;
