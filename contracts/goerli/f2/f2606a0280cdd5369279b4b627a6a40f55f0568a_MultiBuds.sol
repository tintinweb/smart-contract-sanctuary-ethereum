...
contract MultiBuds is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x34d0781B41C52bFD109f6C91525764bef8ea4F4b);
    address payable private _feeAddress = payable(0x5B02Bd67b5B1bDaC824A78749eb7000900d2777b);

    uint256 public MAX_SUPPLY = 10000;
    uint256 public PRICE = 2499 ether;
    bool    private REVEAL_ENABLED = true;
    bool    private isPaused = false;
    uint256 public pubMaxMintCnt = 1000;
    uint256 public startTokenId = 0; // Starting Token ID
    ...
    constructor() ERC721("MultiBuds", "MTB") {
    }
    ...
    function airdrop(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
        ...
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 tokenId = _publicCounter.current() + startTokenId;
    ...
    function airdropSpecial(address to, uint256[] memory tokenIds) external onlyOwner {
        require(_publicCounter.current() < MAX_SUPPLY,"Purchase would exceed MAX_SUPPLY");

        for (uint256 k = 0 ; k < tokenIds.length ; k++){
            _publicCounter.increment();
            if (!_exists(tokenIds[k])) _safeMint(to, tokenIds[k]);
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {//Public Sale
        ...
        require(PRICE * numberOfTokens <= msg.value,"Your Balance is not sufficient");
        ...
        if (msg.value > 0) {
            ...
            _PaymentAddress.transfer(msg.value - fee);
        }
