...
import "./OpenSeaRoyalties/DefaultOperatorFilterer.sol";
import "./INFCStorage.sol";

contract NFC2023 is ERC721, ERC721Enumerable, ERC2981, DefaultOperatorFilterer, Ownable {
   ...
   using Strings for uint256;
    ...
    string public constant ANIMATION_SHA256_PROVENANCE = "d6357cf6261516ad796bde082f473fdbeeab66b5221044e0c91cc58ef4516ba3";
    ...
    modifier payWallets() {
        _;
        payable(paidWallet).transfer(msg.value * 95/100);
        payable(primaryRoyaltyWallet).transfer(msg.value * 5/100);
    }
    ...
    constructor(
        ...
        string memory _jsonForFetchingOnly,
    ...
    function tokenURIarweave(uint256 _tokenId) external view ifExist(_tokenId) returns (string memory) {
        return string(abi.encodePacked("https://arweave.net/jWr16e0Yt9H__uzxCf8ACHKbmh8Q7RScNZMoFunuo98/", _tokenId.toString(), ".json"));
    }

    function tokenURIipfs(uint256 _tokenId) external view ifExist(_tokenId) returns (string memory) {
        return string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/QmdjxKfEbY5xH2BACS9VvbWYYybLnRQRadiTevkmFKvHrc/", _tokenId.toString(), ".json"));
    }

    function tokenURI(uint256 _tokenId) public view override ifExist(_tokenId) returns (string memory) {
        return bytes(jsonForFetchingOnly).length > 0 ? string(abi.encodePacked(jsonForFetchingOnly, _tokenId.toString(), ".json")) : "";
    }

    function discountedMint(address _nft_contract, uint256 _nft_id) external payable onlyFromWallet payWallets {
    ...
    function normalMint() external payable onlyFromWallet payWallets {
    ...
    function emergencyMint() external payable onlyFromWallet payWallets {
