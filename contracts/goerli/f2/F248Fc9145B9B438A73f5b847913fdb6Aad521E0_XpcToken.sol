...
/**
 * @notice $XPC token contract (upgradeable with Transparent Proxy pattern)
 *
 ...
 */
contract XpcToken is ERC20Upgradeable, OwnableUpgradeable {
    /**
    ...
    address private _projectFeeReciever;
     ...
     * @param lockToken_ address of the $XPCL token
     * @param projectFeeReciever address of the project fee receiever wallet
     */
    function initialize(
        string memory name_,
        string memory symbol_,
        address lockToken_,
        address projectFeeReciever
    ) public initializer {
        __ERC20_init(name_, symbol_);
        __Ownable_init();

        require(projectFeeReciever != address(0), "ERC20: no fees reciever");
        ...
        _isExcludedFromFee[projectFeeReciever] = true;
        _isExcluded[projectFeeReciever] = true;
        ...
        _projectFeeReciever = projectFeeReciever;
    ...
    function reflectionFromToken(uint256 tAmount, bool deductFee) public view returns (uint256) {
    ...
    function _tokenFromReflection(uint256 rAmount) internal view returns (uint256) {
        require(rAmount <= _rTotal, "Xpc: exceeds reflected total");
    ...
    function _takeProjectFee(uint256 tProjectFee) private {
        ...
        _rOwned[_projectFeeReciever] = _rOwned[_projectFeeReciever] + rProjectFee;

        if (_isExcluded[address(this)]) _tOwned[_projectFeeReciever] = _tOwned[_projectFeeReciever] + tProjectFee;
    }
