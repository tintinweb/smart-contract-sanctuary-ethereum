...
//need accesscontrol logic as well
contract routerGater {
    address goldfinchUIDAddress;
    uint8[] goldfinchID_TYPE = [0, 1, 2, 3, 4];
    uint256 goldfinchID_TYPE_LENGTH = goldfinchID_TYPE.length;
    ...
    function goldfinchLogic(address _account)
        public
        view
        returns (bool _gatedStatus)
    {
        IERC1155 goldfinchUID = IERC1155(goldfinchUIDAddress);

        for (uint8 i = 0; i < goldfinchID_TYPE_LENGTH; i++) {
            if (
                goldfinchUID.balanceOf(_account, i) > 0 &&
                acceptedGoldfinchID_TYPE[i]
            ) {
                return true;
            }
        }
    ...
    function updateAcceptedID_TYPE(uint8 _ID_TYPE, bool _valid) public {
