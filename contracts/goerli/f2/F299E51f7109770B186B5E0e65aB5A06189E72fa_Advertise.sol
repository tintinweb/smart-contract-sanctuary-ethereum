...
contract Advertise is ERC721, ReentrancyGuard, Ownable {
	...
	event update(address indexed from, uint256 timestamp, uint256 tokenId, string txt, uint256 price);
	...
	function validate(string memory txt) private pure returns (bool) {
		uint256 len = bytes(txt).length;
		if (len <= 0 || len > 10000) return false;
		return true;
	}

	function getCurrentTxt() public view returns (string memory) {
		uint256 tokenId = this.totalSupply();
		return (tokenId > 0) ? txtByTokenId[tokenId] : "";
	}
	...
	function getTxtById(uint256 tokenId) public view returns (string memory) {
		return txtByTokenId[tokenId];
	}
	...
	function mint(string memory txt) public payable nonReentrant {
		require(msg.value >= getPrice(), "Incorrect payable amount");
		require(validate(txt), "Invalid text");
		...
		emit update(msg.sender, block.timestamp, tokenId, txt, msg.value);
	}
	...
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		...
		bytes memory json = abi.encodePacked(
			...
			Base64.encode(
				abi.encodePacked(
					renderer.render(
						...
						payerAddress,
						address(this),
						timeByTokenId[tokenId],
						priceByTokenId[tokenId]
					)
				)
			),
			'", "attributes": [',
			...
			txtByTokenId[tokenId],
			'","timestamp":"',
			Strings.toString(timeByTokenId[tokenId]),
			'","price":',
			priceStr,
			"}}"
		);
...
contract SVGRenderer {
		...
		return
			string(
				abi.encodePacked(
					...
					'</text><text x="27" y="556" font-size="110px">ETH</text><text x="33" y="731">PAYER ADDRESS:</text><text x="33" y="785">',
					payerAddress,
	...
	function getHSLStr(uint256 seed) private pure returns (string memory main, string memory sub) {
		...
		uint256 maxMainHue = 450;
		uint256 minMainHue = 130;

		uint256 mainHue = randomRange(seed, ++count, minMainHue, maxMainHue);
		string memory mainSatStr = randomRangeStr(seed, ++count, 80, 100);
		string memory mainLumStr = randomRangeStr(seed, ++count, 40, 60);
		string memory subSatStr = randomRangeStr(seed, ++count, 80, 100);
		string memory subLumStr = randomRangeStr(seed, ++count, 40, 60);
		...
		main = string(abi.encodePacked(Strings.toString(mainHue), ",", mainSatStr, "%,", mainLumStr, "%"));
		sub = string(abi.encodePacked(Strings.toString(subHue), ",", subSatStr, "%,", subLumStr, "%"));
	}
