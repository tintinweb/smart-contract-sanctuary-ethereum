    ...
    ERC721URIStorage
{
    ...
    function mintBatch(
        ...
        require(_tokenIdTracker + _amount <= 10,"NFT is already limited");
        _tokenIdTracker = _tokenIdTracker + _amount;
        _safeMint(recipients, _amount);
    }

    function mint(address _send) external onlyRole(MINTER_ROLE){
        require(!isContract(msg.sender), "It is contracts");
        require(_tokenIdTracker + 1 <= 10,"NFT is already limited");
