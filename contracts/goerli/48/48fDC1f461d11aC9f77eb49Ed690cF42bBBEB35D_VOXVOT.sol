...
abstract contract ownerdataInterface {
    function setChildPurpose(address _from, address _to, uint256 _tokenID, address _owner) public payable virtual;
}

//tested contract
contract VOXVOT is ERC721Enumerable, Ownable, Pausable{
  using Strings for uint256;
  ...
  uint256 public maxSupplyPerAddress = 5;
  ...
  function publicMint(uint256 amount) public payable whenNotPaused {
    require(!isOnlyWhitelisted, "Now only WL mint");
    uint256 supply = totalSupply();
    require(amount <= publicMaxMintAmount);
  ...
  function whitelistMint(bytes32[] calldata proof) public payable whenNotPaused {
    require(isOnlyWhitelisted, "Now only public mint");
    require(!WLMinted[msg.sender]);
