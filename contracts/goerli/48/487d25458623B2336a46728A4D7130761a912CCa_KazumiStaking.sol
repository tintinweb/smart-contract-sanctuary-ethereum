...
contract KazumiStaking is Ownable, ReentrancyGuard {
    ...
    // address public token = 0x1852baAF94015634d948862D03fF04489772Cf65;
    address public token = 0xCB90572b897591249A84c2CDf71E09625118943A;
    ...
    function withdrawAmount(uint256 amount) external nonReentrant {
        ...
        for (uint256 i; i < locks.length;) {
            if (block.timestamp >= tokenLocks[locks[i]].unlockTimestamp && tokenLocks[locks[i]].tokenAmount > 0) {
                if (withdrawableAmount + tokenLocks[locks[i]].tokenAmount >= amount) {
                    unchecked {
                        tokenLocks[locks[i]].tokenAmount -= (amount - withdrawableAmount);
                    }
                    withdrawableAmount = amount;
                    break;
                } else {
                    unchecked {
                        withdrawableAmount += tokenLocks[locks[i]].tokenAmount;
                    }
                    tokenLocks[locks[i]].tokenAmount = 0;
                }
            }
            unchecked {i++;}
        }
    ...
    /// @dev Get withdrawable lock ids for staker
    /// @param staker the address of the staker
    function getWithdrawableLocks(address staker) external view returns (uint256[] memory) {
        uint256[] memory locks = getLocksByAddress(staker);
        uint256[] memory withdrawableLocks;
        uint256 counter;
        for (uint256 i; i < locks.length;) {
            if (block.timestamp >= tokenLocks[locks[i]].unlockTimestamp && tokenLocks[locks[i]].tokenAmount > 0) {
                withdrawableLocks[counter] = locks[i];
                unchecked {counter++;}
            }
            unchecked {i++;}
        }
        return withdrawableLocks;
    }

    /// @dev Get withdrawable lockboxes for staker
    /// @param staker the address of the staker
    function getWithdrawableLockBoxes(address staker) external view returns (LockBox[] memory) {
        uint256[] memory locks = getLocksByAddress(staker);
        LockBox[] memory lockBoxes;
        uint256 counter;
        for (uint256 i; i < locks.length;) {
            if (block.timestamp >= tokenLocks[locks[i]].unlockTimestamp && tokenLocks[locks[i]].tokenAmount > 0) {
                lockBoxes[counter] = tokenLocks[locks[i]];
                unchecked {counter++;}
            }
            unchecked {i++;}
        }
