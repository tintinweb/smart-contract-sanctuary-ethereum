...
contract NFortress is OwnableUpgradeable {
    ...
    function balanceOfERC721(address nftAddr, bytes memory message, bytes calldata signature) external view returns (uint256) {
        if(nftAddr == address(0)) revert InvalidAddress("nftAddr", nftAddr);
        address wallet = verifyMsg(message, signature);
    ...
    function ownerOfERC721(address nftAddr, uint256 tokenId, bytes memory message, bytes calldata signature) external view returns (bool) {
        if(nftAddr == address(0)) revert InvalidAddress("nftAddr", nftAddr);
        address wallet = verifyMsg(message, signature);
    ...
    function balanceOfERC1155(address nftAddr, uint256 tokenId, bytes memory message, bytes calldata signature) external view returns (uint256) {
        if(nftAddr == address(0)) revert InvalidAddress("nftAddr", nftAddr);
        address wallet = verifyMsg(message, signature);
