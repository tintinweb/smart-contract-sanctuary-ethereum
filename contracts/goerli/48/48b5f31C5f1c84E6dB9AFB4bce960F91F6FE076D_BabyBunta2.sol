...
abstract contract BabyBunta2Core {
    // 割当数とミント済み数を構造体で保存
    ...
    uint8 public constant MAX_FOR_FREE_MINT = 2;
    uint8 public constant MAX_FOR_ALLOW_LIST = 5;
    uint16[3] public MAX_MINT_AMOUNT = [
        MAX_FOR_FREE_MINT,
        MAX_FOR_ALLOW_LIST,
        1
    ];
    ...
    ERC721Psi
{
    ...
    function setCost(uint256 _newCost, SalePhase _target) external onlyOwner {
    ...
    function setDescriptor(IDescriptor _new) external onlyOwner {
        descriptor = _new;
    }
    ...
    function setLoyalityAddress(address _new) external onlyOwner {
        if (_new == address(0)) revert ZeroAddress();
        royaltyAddress = _new;
    }

    function setLoyalityFee(uint96 _new) external onlyOwner {
    ...
    modifier whenValidMintAmount(uint256 amount) {
        // mint数自体の確認
        if (amount > MAX_MINT_AMOUNT[uint256(salePhase)])
...
contract BabyBunta2 is BabyBunta2Admin {
        ...
        returns (string memory)
    {
        ...
        uint256 id = tokenId;
        if (!revealed) {
            id = uint256(keccak256(abi.encodePacked(id))) % 3;
        }
