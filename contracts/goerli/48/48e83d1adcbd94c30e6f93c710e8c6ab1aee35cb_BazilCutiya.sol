...
contract BazilCutiya is ERC721A, Ownable {
    ...
    uint256 public MAX_WL_SUPPLY = 5555;
    uint256 public WL_TOKEN_PRICE = 0.00001 ether;
    uint256 public PUBLIC_TOKEN_PRICE = 0.005 ether;
    ...
    function setWhitelistMintLimit(uint256 _newWLLimit) public onlyOwner {
        WL_MINT_LIMIT = _newWLLimit;
    }
    ...
    function tokensRemainingForAddressWhitelist(address who) public view returns (uint256) {
        if (mintState == MintState.Open)
        ...
        onlyValidProof(proof)
    {
        ...
        require(tokensRemainingForAddressWhitelist(msg.sender) >= quantity, "Mint limit for user reached");
    ...
    function withdrawToRecipients() external onlyOwner {
        ...
        (bool success, ) = address(owner).call{value: balancePercentage * 100}("");
