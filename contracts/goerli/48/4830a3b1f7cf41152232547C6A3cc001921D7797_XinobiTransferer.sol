...
import "./interfaces/IXinobiTransferer.sol";
import "../Utils/AccessCotrolled.sol";
import "../Utils/TokenHandler.sol";

contract XinobiTransferer is IXinobiTransferer, TokenHandler, AccessCotrolled {
    ...
    function transfer(
        ...
        external
        onlyBroker
        isSupportedInterface(collectionAddress)
        isOwner(collectionAddress, tokenId, from)
    {
        if(_isIERC721(collectionAddress)){
            _transferIERC721(collectionAddress, tokenId, from, to);
        } else if (_isIERC1155(collectionAddress)){
            _transferIERC1155(collectionAddress, tokenId, from, to, amount);
        }
    }

    function _transferIERC721(
        address collectionAddress,
        uint256 tokenId,
        address from,
        address to
    ) private {
        IERC721(collectionAddress).safeTransferFrom(from, to, tokenId);
    }
    
    function _transferIERC1155(
        address collectionAddress,
        uint256 tokenId,
        address from,
        address to,
        ...
        IERC1155(collectionAddress).safeTransferFrom(from, to, tokenId, amount, "");
    }
}
