...
contract CHATGPT is ERC721AQueryable, Ownable, ReentrancyGuard {
...
function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
  ...
  if (balanceOf(_msgSender()) == 0) {
    // This is the first NFT that the user is minting, so it is free
    whitelistClaimed[_msgSender()] = true;
    _safeMint(_msgSender(), _mintAmount);
  } else {
    // This is not the first NFT that the user is minting, so check if they are paying enough
    require(msg.value >= cost * _mintAmount, 'Insufficient funds!');
    whitelistClaimed[_msgSender()] = true;
    _safeMint(_msgSender(), _mintAmount);
  }
