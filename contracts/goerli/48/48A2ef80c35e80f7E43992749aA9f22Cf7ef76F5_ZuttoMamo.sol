...
interface IERC5192PLConnector {
	...
	function getParentLinkSbtAddressList(string memory _symbol) external view returns (address parentLinkSbt);

	function getMarriageDay(uint256 _tokenId) external view returns (uint256);
}

// SPDX-License-Identifier: MIT
pragma solidity >=0.8.10 <0.9.0;

interface IParentLinkSbt {
	function mintParentLinkSbt(address _to, uint256 _tokenId) external payable;

	function mintCost() external view;
...
import "./ZuttoMamoAdmin.sol";
import "./interface/IParentLinkSbt.sol";

/**
 *  @dev Zutto Mamoru is the main part of NFT.
 *  You can switch between metadata by using time lapse and specific trigger NFTs.
 */
// contract ZuttoMamo is ZuttoMamoAdmin {
contract ZuttoMamo is ZuttoMamoAdmin, RevokableDefaultOperatorFilterer {
	...
	constructor() ERC721A("Zutto Mamoru", "ZM") {
		...
		// _birthWithSleeping(_ownerAddress, 100);
	}
	...
	/**
	 *  @dev Set minted time to token ID.
	 */
	function _setBirthDay(uint256 _tokenId) private {
		birthDay[_tokenId] = block.timestamp;
	}
	...
	/**
	 *  @dev Minted time is set to 0 to avoid growth and send NFT to the address.
	 */
	...
	function setTokenLock(uint256[] calldata tokenIds, LockStatus newLockStatus) external {
	...
	function setWalletLock(address to, LockStatus lockStatus) external {
		...
		if (walletLock[to] == LockStatus.Lock && lockStatus != LockStatus.Lock) {
	...
	function _isTokenLockToUnlock(uint256 tokenId, LockStatus newLockStatus) private view returns (bool) {
	...
	function _isWalletTimeLock(uint256 tokenId) private view returns (bool) {
		return unlockWalletTimestamp[ownerOf(tokenId)] + unlockLeadTime > block.timestamp;
	}

	function isLocked(uint256 tokenId) public view override(IERC721Lockable, ERC721Lockable) returns (bool) {
		return ERC721Lockable.isLocked(tokenId) || _isTokenTimeLock(tokenId) || _isWalletTimeLock(tokenId);
	}

	function setUnlockLeadTime(uint256 value) external onlyRole(ADMIN_ROLE) {
	 ...
	 * If parent link sbt are tied to Zutomamo NFT, they are moved together.
	 */
	function transferFrom(
		...
		uint256 totalParentLinkSbtContractsAmount = connector.getTotalParentLinkSbtContracts(_tokenId);

		if (totalParentLinkSbtContractsAmount != 0) {
			for (uint256 i = 0; i < totalParentLinkSbtContractsAmount; i++) {
				address parentLinkSbtContract = connector.getParentLinkSbtContractByIndex(_tokenId, i);
				uint256 parentLinkSbtTokenId = connector.getParentLinkSbtTokenByIndex(
					_tokenId,
					parentLinkSbtContract,
					0
				);
				require(
					msg.sender == ownerOfParentLinkSbt(parentLinkSbtContract, parentLinkSbtTokenId),
					"not token owner"
				);
				IParentLinkSbt(parentLinkSbtContract).setIsLocked(false);
				IParentLinkSbt(parentLinkSbtContract).safeTransferFrom(_from, _to, parentLinkSbtTokenId);
			}
	...
	function tokenURI(uint256 tokenId) public view virtual override(ERC721A, IERC721A) returns (string memory) {
		...
		if (address(zuttoMamoStage) == address(0)) {
	 ...
	 */
	function ownerOf(uint256 _tokenId) public view override(ERC721A, IERC721A) returns (address tokenOwner) {
		tokenOwner = tokenIdToTokenOwner[_tokenId];
		require(tokenOwner != address(0), "tooken owner is 0");
		return tokenOwner;
	}
	...
	function ownerOfParentLinkSbt(
		...
		return tokenIdToTokenOwner[parentTokenId];
	}
	...
	function _afterTokenTransfers(
		...
		if (birthDay[_startTokenId] == 0) {
			birthDay[_startTokenId] = block.timestamp;
		}
		super._afterTokenTransfers(_from, _to, _startTokenId, _quantity);
	}
...
import "./ZuttoMamoConfig.sol";
import "./interface/IZuttoMamoStage.sol";

// import "hardhat/console.sol";

/**
 *  @dev Functions that can only be operated by authorized users and functions that can retrieve values are grouped together.
 */
abstract contract ZuttoMamoAdmin is ZuttoMamoConfig, Ownable, AccessControl, ERC721AntiScam, ERC2981 {
	// abstract contract ZuttoMamoAdmin is ZuttoMamoConfig, AccessControl, ERC721AQueryable, ERC2981 {
	// =============================================================
	//                     SUPPORTS INTERFACE
	// =============================================================
	...
	function supportsInterface(
		bytes4 interfaceId
	) public view virtual override(ERC721AntiScam, AccessControl, ERC2981) returns (bool) {
		return
	...
	/**
	 *  @dev Grant the role of admin to the address.
	 */
	...
	/**
	 *  @dev Revoke the role of admin to the address.
	 */
	...
	function setRoyaltyFee(uint96 value) external onlyRole(ADMIN_ROLE) {
		royaltyFee = value;
		_setDefaultRoyalty(withdrawAddress, royaltyFee);
	}
	...
	function setCALLevel(uint256 level) external override onlyRole(ADMIN_ROLE) {
		CALLevel = level;
	}

	function setCAL(address calAddress) external onlyRole(ADMIN_ROLE) {
		_setCAL(calAddress);
	}
	...
	/**
	 *  @dev Move all of the funds to the fund manager contract.
	 */
	 ...
	 */
	function setWithdrawAddress(address payable _ownerAddress) external onlyRole(ADMIN_ROLE) {
	...
	/**
	 *  @dev Set the base URI extension for all token IDs.
	 */
...
/**
 *  @dev Variables required for Zutto Mamo.
 */
abstract contract ZuttoMamoConfig {
	...
	address public withdrawAddress = 0x20E68da8d4B86573E329203613719664543B4f35;
