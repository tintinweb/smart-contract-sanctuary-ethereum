    ...
    ContextMixin
{
    ...
    function royaltyInfo(
        ...
        if (minter != address(0) && royalties[_tokenId] != 0) {
            royaltyFee += (_salePrice * royalties[_tokenId]) / 10000;
            return (minter, royaltyFee);
        }

        // return (receiver, (_salePrice * 400) / 10000);
        return (address(0), 0);
    }
...
contract ZingArt_NFT721 is Base_ZingNFT_721 {
    ...
    event UpdateMintFee(uint256 mintFee);
    ...
    function mint(
        ...
        _internalMint(_to, _tokenUri, newTokenId, _royalteFee, _msgSender());
        emit Minted(newTokenId, _to, _tokenUri, _msgSender());
    }

    /** Lazy Mints
     * @dev Mints a new token to an address with a tokenURI.
     * @param _creator address of the future owner of the token
     * @param _tokenUri string tokenURI of the token to be minted
     * @param _royalteFee uint16 royalty fee
     * @param _price uint16 price
     */
    function lazyMint(
        address _creator,
        string calldata _tokenUri,
        ...
        require(msg.value >= mintFee + _price, "Insufficient funds to mint.");

        // Send  Mint fee to platform address
        (bool success2, ) = treasuryAddress.call{value: mintFee}("");
        if (!success2) {
            revert("Failed to pay mint fee");
        }

        (bool success, ) = _creator.call{value: (msg.value - mintFee)}("");
        if (!success) {
            revert("Failed to pay royalty");
        }
        ...
        emit LazyMinted(newTokenId, _msgSender(), _tokenUri, _creator, _price);
    }
    ...
    function _internalMint(
        ...
        _updateRoyalty(_newTokenId, _royalteFee, _creator);
    }
    ...
    */
    ...
    function isApprovedForAll(
        ...
        // Whitelist Zenft auction, marketplace, contracts for easy trading.
    ...
    /**
     * Override _isApprovedOrOwner to whitelist Zenft contracts to enable gas-less listings.
     */
    ...
    function _setTokenURI(
        ...
        require(!_fronzenTokens[tokenId], "Contract is already frozen");
    ...
    function freeze(
        ...
        require(!_fronzenTokens[_tokenId], "Contract is already frozen");
