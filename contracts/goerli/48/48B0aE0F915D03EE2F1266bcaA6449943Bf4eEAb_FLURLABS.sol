...
contract FLURLABS is ERC721A, ERC721ABurnable, ERC721AQueryable, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;

    uint256 constant maxSupply = 50;
    uint256 constant mintPrice = 0.001 ether;
    ...
    constructor() ERC721A("Flur Labs", "FLURLABS") {
    }
    ...
    function checkNumberMinted(address _wallet) public view returns (uint256) {
        return _numberMinted(_wallet);
    }

    function checkTotalAvailableToMint(address _wallet) public view returns (uint256) {
        return flurAlphaWhale[_wallet];
    }

    function mint(uint256 _quantity) external payable {
        require(letsgolive, "FlurLabs: Mint Not Active");
        require(flurAlphaWhale[msg.sender] > 0, "You are NOT a whale");
        require(_quantity <= flurAlphaWhale[msg.sender], "FlurLabs: Trying to mint more than Allowed");
        require(totalSupply() + _quantity <= maxSupply, "FlurLabs: Mint Supply Exceeded");
        require(_numberMinted(msg.sender) + _quantity <= flurAlphaWhale[msg.sender], "FlurLabs: Exceeds Max For Your Whale Wallet");
