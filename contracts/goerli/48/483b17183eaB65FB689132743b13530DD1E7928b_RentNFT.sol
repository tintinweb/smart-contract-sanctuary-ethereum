...
contract RentNFT is IERC721Receiver, AccessControl {
    ...
    uint256 public buySellCommisionFunds;
    ...
    function unlistNFTForRent(uint8 _itemID) external {
        require(
            isavailableforrent(_itemID, block.timestamp, 2**256 - 1),
            "already booked for future period"
        );
    ...
    function rentnftinternal(
...

        {
            ...
            nftdetail.rentcollected = rent - commision;
    ...
    function buyListedNFT(uint256 _itemID) external {
        ...
        uint256 commision = (inventorymarketplace.price * buySellCommision) /
            100;

        buySellCommisionFunds = buySellCommisionFunds + commision;
        ...
        returns (uint256 rent)
    {
        Inventory memory nftinventory = inventoryinstance.get(_itemID);
        rent =
            nftinventory.rentcollected -
            inventoryinstance.getRentWithdrawn(_itemID);

        if (nftinventory.expiresAt > block.timestamp) {
            return rent;
        } else {
            uint256 rentpersecond = nftinventory.expiresAt -
                nftinventory.startAt /
                nftinventory.rentcollected;
            uint256 timeLeft = nftinventory.expiresAt - block.timestamp;
            uint256 rentearned = timeLeft * rentpersecond;

            if (rentearned > inventoryinstance.getRentWithdrawn(_itemID)) {
                rent = rentearned - inventoryinstance.getRentWithdrawn(_itemID);
                return rent;
            }
        }
    }
    ...
    function withDrawFundsInventory(uint256 _itemID) external {
        ...
        uint256 rent = nftinventory.rentcollected -
            inventoryinstance.getRentWithdrawn(_itemID);
        if (nftinventory.expiresAt > block.timestamp) {
            inventoryinstance.updateRentWithdrawn(
                _itemID,
                inventoryinstance.getRentWithdrawn(_itemID) + rent
            );
            emit Withdrawn(rent, address(msg.sender), _itemID);

            IERC20(feetoken).transferFrom(address(this), msg.sender, rent);
        } else {
            uint256 rentpersecond = nftinventory.expiresAt -
                nftinventory.startAt /
                nftinventory.rentcollected;
    ...
    function withDrawFundsOwner() external onlyOwner {
        emit Withdrawn(
            buySellCommisionFunds + rentCommisionFunds,
            address(msg.sender),
            0
        );
...
contract NFTInventory is Ownable {
        ...
        onlyOwner
    {
        rentWithdrawn[_itemID] = _rentWithdrawn;
         ...
         function getRentWithdrawn( uint256 _itemID)
        public view
        returns(uint256)
    {
