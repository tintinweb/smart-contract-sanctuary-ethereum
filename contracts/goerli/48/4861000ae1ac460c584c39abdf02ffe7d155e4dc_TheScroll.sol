...
contract TheScroll is ERC721, ERC721Burnable, Ownable {
    using Counters for Counters.Counter;
    
    event NewColorUnlock(uint256 color, string colorName, address patron);
    event NewSizeUnlock(uint256 newSize, address patron);
    event NewTile(uint256 canvasId, bytes pixels, address author);
    // this makes me think RSS-like thoughts
    ...
    // current parameters
    Counters.Counter private _tileCounter;
    Counters.Counter private _tileSize;

    // mapping(uint256 => address) private _sizePatrons;
    
    mapping(address => bool) private _hosts;
    mapping(address => bool) private _banned;

    string[] private _colors = ["FFFFFF", "000000"];
    string[] private _colorNames = ["white", "black"];
    // string[] private _colorPatrons;

    // pixels
    mapping(uint256 => address) private _svgData;

    // block number of last mint? 

    // the id of the canvas this canvas is responding to (default=0)
    mapping(uint256 => uint256) private _riff;
    ...
    function getColors() public view returns (string[] memory) {
        // include names somehow? dict/json?
        return _colors;
    }

    function getColorNames() public view returns (string[] memory) {
        return _colorNames;
    }

    function drawTile(bytes calldata pixels, uint256 riffId) public returns (uint256) {
        safeMint(msg.sender);

        require(_banned[msg.sender] == false, "you are banned from drawing");

        // todo: require to own a tile
            // exclude: addresses in _hosts
        
        // todo: require dupcilate pixels to not already exist in svgData array
            // exclude: addresses in _hosts

        // todo: require only uses colors in _colors array

        // todo: require correct size based on _size

        // todo: require riffId is valid?

        uint256 currentId = _tileCounter.current();

        _svgData[currentId] = SSTORE2.write(pixels);
        _riff[currentId] = riffId;

        // emit events
        emit NewTile(currentId, pixels, msg.sender);

        _tileCounter.increment();

        return currentId;
    }

    // todo: make payable
    // function drawTileUnlockColor(bytes calldata pixels, uint256 riffId, string memory newColor, string memory newColorName) public returns (uint256) {
    //     safeMint(msg.sender);
    ...
    //     // todo: require duplicate pixels to not already exist in svgData array
    //         // exclude: addresses in _hosts
    ...
    //         require(keccak256(abi.encodePacked(_colors[i])) != keccak256(abi.encodePacked(newColor)), "color already exists");
    //     }
    //     // does uniqueness on names matter?

    //     _colors.push(newColor);
    //     _colorNames.push(newColorName);
    //     // _colorPatrons.push(msg.sender);
    ...
    //     // emit events
    //     emit NewTile(currentId, pixels, msg.sender);
    //     emit NewColorUnlock(newColor, newColorName, msg.sender);
    ...
    // todo: make payable
    function drawTileUnlockSize(bytes calldata pixels, uint256 riffId) public returns (uint256) {
        safeMint(msg.sender);
        ...
        // does this structure work across two different types? 
        // _patrons.push(msg.sender);

        // self

        uint256 newTileId = _tileCounter.current();

        _svgData[newTileId] = SSTORE2.write(pixels);
        _riff[newTileId] = riffId;

        emit NewTile(newTileId, pixels, msg.sender);
        emit NewSizeUnlock(_tileSize.current(), msg.sender);

        return newTileId;
    }

    function getTileSVG(uint256 tileId) public view returns (string memory) {
        return gfx.draw(
            SSTORE2.read(_svgData[tileId])
        );
    }
    ...
    // todo function addHost, removeHost
        ...
        returns (string memory)
    {
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "The Scroll", "description": "", "image": "', 
                        output, 
                        '"}'
                    )
                )
    ...
    function safeMint(address to) public {
        uint256 tileId = _tileCounter.current();
        _tileCounter.increment();
        _safeMint(to, tileId);
    }
}
