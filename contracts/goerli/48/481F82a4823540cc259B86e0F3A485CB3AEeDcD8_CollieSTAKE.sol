...
contract CollieContext {
    ...
    uint256 public maxTxAmount = 1000000000000000000000; // if 0, no limit
    ...
    event TierListUpdated(uint256 level, uint256 percentage, uint256 apy);
}
...
contract CollieSTAKE is CollieContext, Ownable, ReentrancyGuard {
    ...
    constructor() {
        token = IERC20(0x78d0B340639aab3b200E7eBE38e701930e8850D4);
        rewardingWallet = 0x294d0487fdf7acecf342ae70AFc5549A6E90f3e0;
        pools[1] = Pool(30, 1000, 1);
        pools[2] = Pool(60, 1500, 2);
        pools[3] = Pool(90, 2000, 3);
        pools[4] = Pool(120, 2500, 4);
        pools[5] = Pool(150, 3000, 5);
        pools[6] = Pool(180, 3500, 6);
        pools[7] = Pool(210, 4000, 7);
        pools[8] = Pool(240, 4500, 8);
        pools[9] = Pool(270, 5000, 9);
        pools[10] = Pool(300, 5500, 10);
    }
    ...
    function updatePool(uint256 lockPeriod, uint256 apy, uint256 tier) public onlyOwner{
        pools[tier] = Pool(lockPeriod, apy, tier);
        emit TierListUpdated(tier, lockPeriod, apy);
    }
    

    function stake(uint256 amount, uint256 tier) public nonReentrant {
        require(tier > 0 && tier <=10, "Incorrect Tier");
        require(amount >= minTxAmount && amount <= maxTxAmount, "Amount Should Be Between Min and Max Limit");
    ...
    function _unstakeTokens(address _address, uint256 tier) internal {
        ...
        wUser.totalAmount = wUser.totalAmount.sub(wUser.stakedAmount);
        wUser.depositTime = block.timestamp;
    ...
    function EmergencyUnstake(uint256 tier) public nonReentrant{
        ...
        totalStaked-=wUser.stakedAmount;
        deposit[tier][_msgSender()] = User(0, 0, 0, 0, 0, Pool(0,0,0));
