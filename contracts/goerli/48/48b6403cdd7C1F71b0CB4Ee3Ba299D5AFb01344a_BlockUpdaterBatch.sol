...
contract BlockUpdaterBatch {
    ...
    function parse_input(uint256[8] memory inputs) public pure returns (ParsedInput memory) {
    ...
    function updateBlock(
        ...
        if (canonical.length == 0) {
            // init
            setGenesisBlock(currentMerkleRoot,blockSize,blockNumber,totalDifficulty);
        } else {
    ...
    function setGenesisBlock(bytes32 merkleRoot, uint256 blockSize, uint256 blockNumber, uint256 totalDifficulty) internal {
        ...
        genesisBlockNumber = blockNumber - blockSize + 1;
    }
}
