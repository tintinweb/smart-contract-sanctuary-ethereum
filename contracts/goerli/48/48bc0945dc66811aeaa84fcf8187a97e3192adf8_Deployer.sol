...
contract Deployer {
    ...
    function deploy1155(
        ...
        ERC1155Minter minter = new ERC1155Minter(
            _tokenUri,
            _maxSupply,
            _price,
            _startTime,
        ...
        emit DeployContract(msg.sender, address(minter));

        return payable(address(minter));
    }
}
    ...
    PaymentSplitter
{
    ...
    constructor(
        ...
        minterPayees = _payees;
    }

    /* END USER */

    function mintAllowList(uint256 quantity, bytes32[] calldata _merkleProof)
        external
        payable
    {
        require(
        ...
        require(msg.value == price, "The price does not match the amount paid");
    ...
    function _beforeTokenTransfer(
        ...
        require(
            ...
            "ERC1155Minter: Transfer not allowed"
        );

        if (from == address(0)) {
            require(
                totalSupply(ids[0]) + amounts[0] <= maxSupply,
                "ERC1155Minter: Max supply reached"
            );
        }
    ...
    function _setApprovalForAll(
        ...
        require(transferable, "ERC1155Minter: Approval not allowed");
