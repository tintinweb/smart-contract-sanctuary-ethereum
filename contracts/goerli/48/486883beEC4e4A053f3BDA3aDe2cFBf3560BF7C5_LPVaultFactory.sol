...
import "../interfaces/ILPVaultFactory.sol";
import "../interfaces/ILPVault.sol";

// Module to deploy Vault
contract LPVaultFactory is ILPVaultFactory, Ownable2StepUpgradeable {
    /* State
     *****************************************************************************************************************/

    address private registry;
    mapping(address => bool) private implementationRegistry;
    mapping(address => address[]) private lpVaultsByPrincipalToken;
    ...
    /**
     * @inheritdoc ILPVaultFactory
     */
    function deployLPVault(
        address _implementation,
        address _principalToken,
        bytes memory _data
    ) external override returns (address lpVault) {
        if (!isImplementationValid(_implementation)) {
            revert ImplementationInvalidError();
        }
        lpVault = address(new BeaconProxy(_implementation, _data));
        if (ILPVault(lpVault).getPrincipalToken() != _principalToken) {
            // This makes it easier for developers while not compromising security.
            // If we were to extract the principalToken directly from the encoded data,
            // we'd have to know in advance the signature of the implementation initializer
            revert PrincipalTokenMismatchError();
        }
        emit LPVaultDeployed(lpVault, _implementation, _principalToken);
        lpVaultsByPrincipalToken[_principalToken].push(lpVault);
        // Transfer ownership of LPVault from LPVaultFactory to owner of LPVaultFactory
        Ownable2StepUpgradeable(lpVault).transferOwnership(owner());
    }

    /**
     * @inheritdoc ILPVaultFactory
     */
    function setRegistry(address _registry) public override {
        _checkOwner();
        if ((_registry == address(0))) {
            revert RegistryZeroAddressError();
        }
     ...
     * @inheritdoc ILPVaultFactory
     */
    function addImplementation(address _implementation) public override {
        _checkOwner();
        if ((_implementation == address(0))) {
            revert ImplementationZeroAddressError();
        } else if (isImplementationValid(_implementation)) {
            revert ImplementationAlreadyAddedError();
        }
        emit ImplementationAdded(_implementation);
        implementationRegistry[_implementation] = true;
    }

    /**
     * @inheritdoc ILPVaultFactory
     */
    function removeImplementation(address _implementation) public override {
        _checkOwner();
        if (!isImplementationValid(_implementation)) {
            revert ImplementationInvalidError();
        }
        emit ImplementationRemoved(_implementation);
        delete implementationRegistry[_implementation];
    }
     ...
     * @inheritdoc ILPVaultFactory
     */
    function isImplementationValid(
        address _implementation
    ) public view override returns (bool) {
        return implementationRegistry[_implementation];
    }

    /**
     * @inheritdoc ILPVaultFactory
     */
    function getLPVaultsForPrincipalToken(
        address _principalToken
    ) external view override returns (address[] memory) {
        return lpVaultsByPrincipalToken[_principalToken];
    }
}
...
// TODO: Should this be 4626? Are there any LPVaults which wouldn't be 4626? Probably not
interface ILPVault {
    /**
     * @notice Getter for the Principal Token associated to the LPVault
     */
...
interface ILPVaultFactory {
    error PrincipalTokenMismatchError(); // Security in case the deployer passes one PT to the factory and another as encoded data
    error RegistryZeroAddressError();
    error ImplementationZeroAddressError();
    error ImplementationAlreadyAddedError();
    error ImplementationInvalidError(); // When the implementation is not in the internal registry
     ...
     * @param _data The data to be passed during deployment.
     * For example, the Curve LP Vault will take an initialize with parameters:
     * abi.encodeWithSelector(initialize.selector, ...params)
     * @return lpVault The address of the LPVault deployed.
     */
    function deployLPVault(
        address _implementation,
        address _principalToken,
        bytes memory _data
    ) external returns (address lpVault);
    ...
    /**
     * @notice Adds an implementation to the list of valid implementations.
     * An implementation will need to be added to the factory before it can be used to create vaults.
     * @param _implementation The address of the implementation to add.
     */
     ...
     * @param _implementation The address of the implementation to check.
     * @return true if the implementation has been added, false otherwise.
     */
    function isImplementationValid(
        address _implementation
     ...
     * @param _principalToken The address of the principalToken.
     * @return lpVaults The list of LPVaults deployed for the given principalToken.
     * LPVaults are returned without any particular sorting or filtering.
     * They may also have different implementations.
     */
    function getLPVaultsForPrincipalToken(
        address _principalToken
