...
import "grails/season-02/Grails2MintPass.sol";
import "grails/season-02/Grails2.sol";

contract DevGrails2 is Grails2 {
    constructor()
        Grails2(
            "DevGrails II",
            "DG2",
    ...
    function devmint(address to, uint8[] memory grailIds) public {
        uint256 nextTokenId = totalSupply();

        for (uint256 idx = 0; idx < grailIds.length; ++idx) {
            uint8 grailId = grailIds[idx];

            if (grailId >= NUM_GRAILS) {
                revert InvalidGrailId();
            }

            _grailByTokenId[nextTokenId++] =
                Grail({id: grailId, edition: numEditionsByGrailId[grailId]++});
    ...
    function devmint(uint8[] memory grailIds) public {
        devmint(msg.sender, grailIds);
    }
}

contract DeployDevGrails is Script {
    function setUp() public {}

    function run() public {
        vm.startBroadcast();
        DevGrails2 grails = new DevGrails2();

        uint8[] memory ids = new uint8[](1);
        ids[0] = 1;
        grails.devmint(msg.sender, ids);
    }
}
...
contract Grails2 is ERC721ACommon, BaseTokenURI {
    ...
    function mintTreasuryReserve(address to) external onlyOwner {
        if (treasuryReserveMinted) {
            revert ReserveAlreadyMinted();
        }
     ...
     * @dev Passing controll to our own contracts is effectively not an
     * interaction,
    ...
    modifier onlyAfterTreasuryReserveMinted() {
        if (!treasuryReserveMinted) {
            revert TreasuryReserveNotYetMinted();
        }
        _;
    }
    ...
    function _afterTokenTransfers(
        ...
        for (uint256 tokenId = startTokenId; tokenId < end;) {
            ...
            try transferListener.onTransfer{gas: 100_000}(from, to, tokenId) {}
                catch {}
...
contract Grails2MintPass is ERC721ACommon, BaseTokenURI {
    ...
    function airdrop(
        ...
        if (_totalMinted() > NUM_MAX_PASSES) {
            revert AirdropExceedsMaxSupply();
        }
        if (total != expectedTotal) {
            revert WrongAirdropChecksum();
        }
