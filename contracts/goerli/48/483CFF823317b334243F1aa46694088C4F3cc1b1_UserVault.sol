...
contract ConfigurableRightsPool is PCToken, DesynOwnable, DesynReentrancyGuard, WhiteToken {
    ...
    function rebalance(
        ...
        SmartPoolManager.rebalanceHandle(bPool, etype == SmartPoolManager.Etypes.CLOSED, etfStatus.collectEndTime, etfStatus.closureEndTime, rights.canChangeWeights, tokenA, tokenB);
    ...
    function joinPool(
        ...
        _mintPoolShare(actualPoolAmountOut);
        _pushPoolShare(msg.sender, actualPoolAmountOut);
        if (totalBegin > 0) {
            SmartPoolManager.Fund storage fund = beginFund[msg.sender];
            fund.etfAmount = DesynSafeMath.badd(beginFund[msg.sender].etfAmount, actualPoolAmountOut);
    ...
    function exitPool(uint poolAmountIn, uint[] calldata minAmountsOut) external logs lock needsBPool {
        ...
        for (uint i = 0; i < poolTokens.length; i++) {
            ...
            emit LogExit(msg.sender, t, finalAmountOut);
            ...
            if (_actualRedeemFee != 0 || (profitRate > 0 && etfStatus.perfermanceFee != 0)) {
                ...
                bool returnValue = IERC20(t).safeApprove(vaultAddress, redeemAndPerformanceFeeReceived);
...
contract UserVault is DesynOwnable {
    ...
    function managerClaim(address pool) external {
        ...
        if (this.isClosePool(pool)) {
            ...
            ClaimtokensInfo memory recordToken;
