    ...
    UUPSUpgradeable
{
    ...
    uint256[3] private _nftBuyPrices; //regal - 0 //sirius - 1 //vega - 2
    address private collectorAddress; //regal-0, serius-1, vega-2
    ...
    function initialize(
        ...
        uint256[3] memory nftQuan, //regal-0, serius-1, vega-2
    ...
    function setRefralLinks(bytes32[] memory _setRefLink) public onlyOwner {
    ...
    function setNFTPresaleQuant(uint256[3] memory _increaseNFT)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < 3; i++) {
            _nftInfo[i].total = uint16(_increaseNFT[i]);
        }
    }
    ...
    function getRefLinkPurchaseDataWithType(bytes32 _Link, uint256 _type)
        public
        view
        returns (uint256)
    {
        return totalPurchasePerLinkPerType[_Link][_type];
    }
    ...
    function batchCreateNFTs(
        uint256 _nftType, //regal - 0 //sirius - 1 //vega - 2
        uint256 quantity,
        bytes32 _Link
    ) external {
        uint256 balOfAstro = _astro.balanceOf(msg.sender);
        if (isAstroNftNeccesary) {
            require(balOfAstro >= 1, "Can't buy nft without Astro nft");
        }
