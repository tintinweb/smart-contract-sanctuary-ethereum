...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    // ============ FUNCTION TO ADD CREATOR ADDRESS + TOKENID  ============
    function addCreatorAddress(uint256 _tokenId, address _creatorAddress) internal {
        // use the _tokenId as the key in the mapping to store the creator address
        tokenCreator[_tokenId] = _creatorAddress;
    }

    // ============ FUNCTION TO CHECK CREATOR ADDRESS OF TOKEN  ============
    function getCreatorAddress(uint256 _tokenId) public view returns (address) {
    // check if the tokenId exists in the mapping
    require(tokenCreator[_tokenId] != address(0), "Token with the given ID does not exist");

    // return the creator address for the tokenId
    ...
    function mintWithQTY(address _creatorAddress, string calldata _valid, uint256 _numberOfTokens, uint256 _stage)
        public
        payable
        ...
        nonReentrant
    {
        ...
        addCreatorAddress(firstId, _creatorAddress);

        emit TokenMinted(firstId, _numberOfTokens, address(this), msg.sender);
    }

    // ============ PUBLIC MINT FUNCTION FOR NORMAL USERS ============
    function mintWithID(address _creatorAddress,string calldata _valid, uint256 _tokenId, uint256 _stage)
        public
        payable
        isCorrectPayment(_valid, _stage)
        canMint(1, _tokenId)
