...
contract GeweemERC721 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable,NativeMetaTransactionUpgradeable,ContextMixin {
    ...
    function initialize(string memory name, string memory symbol,string memory baseURI,uint initReleaseCount,uint maxSupply,uint getFreeBuyCount,uint getFree,uint promoBefore) initializer public {
    ...
    function mintAsset(uint256 _mintAmount) public payable {
        ...
        if(_getFreeBuyCount >0 && _getFree > 0 && _mintAmount >= _getFreeBuyCount && totalSupply() < _promoBefore ){
            _mintAmount =  _mintAmount + (_mintAmount/_getFreeBuyCount);
        }
