...
contract BountyMint is ERC721A, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_PRE_MINT = 3; // This actually means 2 mints but we say 3 for minor gas saving
    uint256 public constant MAX_MINT = 5; // This actually means 4 mints but we say 5 for minor gas saving
    ...
    function presaleMint(uint256 qty, bytes32[] calldata merkleProof) checkTokenSupplyAvailable(qty) validPayment(qty, PRESALE_PRICE) addressWhitelisted(merkleProof, whitelistRoot) nonReentrant external payable {
