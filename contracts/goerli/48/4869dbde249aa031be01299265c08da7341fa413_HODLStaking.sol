...
contract HODLStaking is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    //Percentage distributed per day. 2 decimals / 10000
    uint16 public percPerDay = 7;
    ...
    function setPercentagePerDay(uint16 _perc) external onlyOwner {
        percPerDay = _perc;
    }
    ...
    function getLockTermsOfUser(address _user, uint8 _lid)
        external
        view
        returns (uint256 count, UserLockInfo[] memory)
    {
...
interface IVesting {
    ...
    function vesting(address, uint256)
        external
        view
        returns (
            uint256 time,
