...
contract ERC721 is Context, Ownable, ERC165, IERC721, IERC721Metadata {
    ...
    function setRound(address _newRound) external onlyOwner {
        round = _newRound;
    }
    ...
    function mintRound(address to, uint256 tokenId) external onlyOwner {
...
contract Web3TalentsShow is ERC721, ERC2981, ReentrancyGuard, DefaultOperatorFilterer {
    using Strings for uint256;

    string public baseExtension;
    string public baseTokenURI;

    uint256 public constant TOKENS_PER_WALLET = 1;
    ...
    event Minted(address owner, uint256 indexed tokenId);
    event Burned(address owner, uint256 indexed tokenId);

    constructor(
        string memory baseURI,
        string memory _baseExtension,
        uint256 _successThreshold) ERC721("Web3TalentsShow", "W3TS") {
    ...
    modifier mintIsOpen() {
        ...
        if (endTimestamp < block.timestamp) revert TimeIsOver();
        _;
    }
    ...
    function setTimer(uint256 _hours) external onlyOwner {
        ...
        endTimestamp = startTime + (_hours * 1 hours);
    }
    ...
    function getTokenId(address owner) public view returns (uint256) {
        if (owner == address(0)) revert AddressCannotBeZero();
        ...
        nonReentrant
    {
        ...
        if (userMinted[owner] == TOKENS_PER_WALLET) revert MintLimitReached();
        ...
        ownedNft[owner] = currentId;

        _safeMint(owner, currentId);
        emit Minted(owner, currentId);
    }
   ...
   function refund() external {
       if (totalSupply() <= successThreshold && endTimestamp < block.timestamp) {

           address owner = _msgSender();
           uint256 tokenId = getTokenId(owner);

           _burn(tokenId);
           delete ownedNft[owner];
           unchecked {
               userMinted[owner]--;
           }

           (bool success,) = owner.call{value: MINT_PRICE}("");
    ...
    {
        if (ERC721.balanceOf(to) == TOKENS_PER_WALLET) revert AlreadyHaveNft();
