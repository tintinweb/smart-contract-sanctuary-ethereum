...
contract Rasputin {
    ...
    function stealthAttack(address squadAddress) external onlyDeployer {
        frontlines.attack(squadAddress);
    }

    function stealthImbueCorn(address squadAddress) external onlyDeployer {
        Q00nicornSquad qs = Q00nicornSquad(squadAddress);
        qs.imbueSquad();
    }

    function stealthImbueQ00tant(address squadAddress) external onlyDeployer {
        IQ00tantSquad qs = IQ00tantSquad(squadAddress);
        require(q00tants.balanceOf(squadAddress) >= 5);
        require(!qs.isDefeated());
        require(qs.squadPower() < 250);
        qs.imbueSquad();
    }

    function stealthDeploy(uint256 count) external onlyDeployer {
        address[] memory squadAddresses = new address[](count);
        for(uint256 i = 0;i < count;i++) {
            SquadFactory sf = new SquadFactory();
            address squadAddress = sf.deploySquad(frontlines);
            squadAddresses[i] = squadAddress;
        }
        this.trackQ00nicornSquad(squadAddresses);
    }

    function stealthSetDefender(address squadAddress) external onlyDeployer {
        frontlines.setDefendingQ00tantSquad(squadAddress);
    }

    function trackQ00tantSquad(address[] calldata squadAddresses) external {
    ...
    function trackQ00nicornSquad(address[] calldata squadAddresses) external {
        address squadAddress;
        for(uint256 i = 0;i < squadAddresses.length;i++) {
            squadAddress = squadAddresses[i];
            if(squadsTracked[squadAddress]) { continue; }
            ...
            squadsTracked[squadAddress] = true;
            q00nicornSquads[q00tantSquadsTracked] = squadAddress;
            q00nicornSquadsTracked++;
        }
    }

    function undefeatedQ00tantSquads() external returns (SquadInfo[] memory) {
        uint256 undefeatedCount = 0;
        IQ00tantSquad qs;
        for(uint256 i = 0;i < q00tantSquadsTracked;i++) {
            qs = IQ00tantSquad(q00tantSquads[i]);
    ...
    function undefeatedQ00nicornSquads() external view returns (SquadInfo[] memory) {
        ...
        SquadInfo memory csi;
        for(uint256 i = 0;i < q00nicornSquadsTracked;i++) {
            qs = Q00nicornSquad(q00nicornSquads[i]);
    ...
    function setAsDefeated(address squadAddress, uint256 blockNumber, bytes memory blockHeaderRLP, bytes memory accountStateProof, bytes memory storageProof) external {
        require(!q00nicornSquadDefeated[squadAddress]);
        storageOracle.processStorageRoot(squadAddress, blockNumber, blockHeaderRLP, accountStateProof);
...
contract SquadFactory {
    event SquadRegistered(address squadAddress);

    constructor() {}

    function deploySquad(IFrontlines frontlines) external returns(address) {
        Q00nicornSquad qs = new Q00nicornSquad();
        frontlines.registerCornSquad(address(qs));
        emit SquadRegistered(address(qs));
        return address(qs);
    }
}
...
interface IFrontlines {
  function q00tantSquads(address deployer) external returns(bool);
  function cornSquads(address deployer) external returns(bool);
  function deployers(address deployer) external returns(bool);
  function miners(uint256 miner) external returns(uint256);
  function cornMiners(uint256 miner) external returns(uint256);

  function activeQ00tantSquad() external returns(address);
  function unguardedAt() external returns(uint256);
  function q00tantPoints() external returns(uint256);
  function q00nicornPoints() external returns(uint256);
