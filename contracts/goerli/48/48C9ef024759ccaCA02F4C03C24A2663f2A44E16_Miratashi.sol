...
contract Miratashi is ERC721M, Ownable, ERC2981, ReentrancyGuard {
    ...
    address public constant FOUNDER_ADDRESS =
        0xB2d3b575619A9Eab1cb8c0f5f2a34333695f0B5a;

    // Team treasury address
    address public constant TEAM_TREASURY_ADDRESS =
        0x754a7ACc432Da9596a4E972Cba622391196FdCd0;
    ...
    // Active variable
    bool public isOGPhaseActive = false;

    // The quantity for OG Phase mint
    uint256 public ogPhaseQuantity;
    ...
    uint256 public ogPhaseDuration = 7200;
    ...
    // Ending price (wei)
    uint256 public auctionEndingPrice = 0;
    ...
    constructor() ERC721M("Miratashi", "MIRA") {
        _setDefaultRoyalty(TEAM_TREASURY_ADDRESS, 500);
    }
     ...
     */
    function mintOG() external payable callerIsUser {
        // Check if OG phase is active
        require(isOGPhaseActive, "Miratashi: OG phase is not active.");
        ...
        // Max supply
        require(
            ogPhaseMinted + 1 <= ogPhaseQuantity,
            "Miratashi: Over OG Phase mint supply limit."
        );

        // Max address OG phase capacity
        require(
            walletOGPhaseMinted[msg.sender] < walletOGPhaseCapacity[msg.sender],
            "Miratashi: Reached mint limit."
        );
        ...
        // Require enough ETH
        require(msg.value <= ogPhasePrice, "Miratashi: Not enough ETH.");

        // Increase wallet addesss minted count
        walletOGPhaseMinted[msg.sender]++;
    ...
    /**
     * @notice Allows the whitelist people to mint a NFT.
     */
    function mintWhitelist() external payable callerIsUser {
        // Check if whitelist phase is active
        require(isWhitelistActive, "Miratashi: Whitelist phase is not active.");

        // Check if wallet was in whitelist
        require(
            walletWhitelist[msg.sender],
            "Miratashi: You are not Whitelist."
        );

        // Max supply
        require(
            whitelistMinted + 1 <= whitelistQuantity,
        ...
        require(
            ...
            "Miratashi: You can mint only once during the first 30 minutes."
        );

        // Mint only 2 times
        require(
            walletWhitelistMinted[msg.sender] <= 2,
            "Miratashi: Reached mint limit."
        );
    ...
    function mintDutchAuction(uint8 _quantity) external payable callerIsUser {
        // Check is active
        require(isAuctionActive, "Miratashi: Dutch auction is not active.");
        ...
        require(
            ...
            "Miratashi: Dutch auction has not begun yet."
        );

        // Require max 2 per tx
        require(
            _quantity <= 2,
            "Miratashi: Over max per transaction. (Limit 2)"
        );
        ...
        if (
            totalSupply() + _quantity == (whitelistQuantity + auctionQuantity)
        ) {
    ...
    function remainingSupply() public view returns (uint256) {
        return (whitelistQuantity + auctionQuantity) - totalSupply();
    }
    ...
    /**
     * @notice Allows the owner to set active status of OG phase.
     */
    function setOGPhaseActive(bool _isActive) external onlyOwner {
        isOGPhaseActive = _isActive;
    }

    /**
     * @notice Allows the owner to set a new price of OG phase.
     */
    function setOGPhasePrice(uint256 _newPrice) external onlyOwner {
        ogPhasePrice = _newPrice;
    }

    /**
     * @notice Allows the owner to set quantity of OG phase.
     */
    function setOGPhaseQuantity(uint256 _quantity) external onlyOwner {
        require(
            (whitelistQuantity + auctionQuantity + _quantity) < MINT_CAPACITY,
            "Miratashi: Over max total supply."
        );
        ogPhaseQuantity = _quantity;
    }

    /**
     * @notice Allows the owner to set specified time of OG Phase phase.
     */
    function setOGPhaseStartTime(uint256 _startTime) external onlyOwner {
        ogPhaseStartingTime = _startTime;
        // Dutcn Auction starting time is ogPhaseStartingTime + ogPhaseDuration + 15 minutes.
        auctionStartingTime = ogPhaseStartingTime + ogPhaseDuration + 900;
    }
    ...
    function addToWhitelist(
        ...
        for (uint256 i = 0; i < _toAddAddresses.length; i++) {
            walletWhitelist[_toAddAddresses[i]] = true;
        }
    }
    ...
    function removeFromWhitelist(
        ...
        for (uint256 i = 0; i < _toRemoveAddresses.length; i++) {
            delete walletWhitelist[_toRemoveAddresses[i]];
        }
    }
    ...
    /**
     * @notice Allows the owner to set quantity of whitelist phase.
     */
    function setWhitelistQuantity(uint256 _quantity) external onlyOwner {
        require(
            (ogPhaseQuantity + auctionQuantity + _quantity) < MINT_CAPACITY,
            "Miratashi: Over max total supply."
        );
        whitelistQuantity = _quantity;
    }
    ...
    function setWhitelistStartTime(uint256 _startTime) external onlyOwner {
        ...
        // Dutcn Auction starting time is whitelistStartingTime + whitelistDuration + 15 minutes.
        auctionStartingTime = whitelistStartingTime + whitelistDuration + 900;
    }

    /**
     * @notice Allows the owner to set active status of dutch auction phase.
     */
    function setAuctionActive(bool _isActive) external onlyOwner {
        isAuctionActive = _isActive;
    }

    /**
     * @notice Allows the owner to set price, decrement and frequency of dutch auction phase price.
     */
    function setAuctionPrice(
        ...
        auctionStartingPrice = _price * (1 ether);
        auctionPriceDecrement = _decrement * (1 ether);
        auctionDecrementFrequency = _frequency;
    }

    /**
     * @notice Allows the owner to set quantity of dutch auction phase.
     */
    function setAuctionQuantity(uint256 _quantity) external onlyOwner {
        require(
            (ogPhaseQuantity + whitelistQuantity + _quantity) < MINT_CAPACITY,
            "Miratashi: Over max total supply."
        );
        auctionQuantity = _quantity;
    }

    /**
     * @notice Allows the owner to set specified time of Dutch auction phase.
     */
    function setAuctionStartTime(uint256 _startTime) external onlyOwner {
