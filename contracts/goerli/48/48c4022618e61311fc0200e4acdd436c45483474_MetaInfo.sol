...
contract MetaInfo is Meta {
    ...
    function getNodeBaseInfo(bytes32 node, bytes32[] memory keys) external view returns (
        ...
        for (uint256 i = 0; i < keys.length; i++) {
            values[i] = metaDB.getNodeItem(node, keys[i]);
        }
    }

    function getNodeBaseInfoString(bytes32 node, bytes32[] memory keys) external view returns (
        uint256 owner_balance,
        uint256 locked_balance,
        uint256 staked_balance,
        uint256 gas_balance,
        ...
        for (uint256 i = 0; i < keys.length; i++) {
            values[i] = abi.decode(metaDB.getNodeItem(node, keys[i]), (string));
        }
    }
