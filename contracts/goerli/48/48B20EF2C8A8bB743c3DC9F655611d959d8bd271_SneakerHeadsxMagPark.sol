...
contract SneakerHeadsxMagPark is ERC721A, Manageable, DefaultOperatorFilterer, ReentrancyGuard {
	...
	uint32 private s_vipSaleStartTime = 1674032400;
	uint32 private s_allowlistSaleStartTime = 1674118800;
	uint32 private s_saleEndTime = 1674205200;

	uint256 private s_vipSalePrice = 0.08 ether;
	uint256 private s_allowlistSalePrice = 0.12 ether;

	mapping(SALE_STAGE => mapping(address => uint256)) private s_numberMintedDuringSaleStage;

	string private s_baseTokenURI = "...";

	constructor() ERC721A("SneakerHeads x MagPark", "SHxMP") {}
	...
	/**
	 * @dev This function is used for changing VIP sale start time
	 *
	 * @param vipSaleStartTime New VIP sale start time in unix time
	 */
	function setVipSaleStartTime(uint32 vipSaleStartTime) external onlyManager {
		s_vipSaleStartTime = vipSaleStartTime;
	}

	/**
	 * @dev This function is used for changing allowlist sale start time
	 *
	 * @param allowlistSaleStartTime New allowlist sale start time in unix time
	 */
	function setAllowlistSaleStartTime(uint32 allowlistSaleStartTime) external onlyManager {
		s_allowlistSaleStartTime = allowlistSaleStartTime;
	}

	/**
	 * @dev This function is used for changing sale end time
	 *
	 * @param saleEndTime New sale end time in unix time
	 */
	function setSaleEndTime(uint32 saleEndTime) external onlyManager {
		s_saleEndTime = saleEndTime;
	}

	/**
	 * @dev This function is used for changing VIP sale price
	 *
	 * @param vipSalePrice New VIP sale price in wei
	 */
	function setVipSalePrice(uint256 vipSalePrice) external onlyManager {
		s_vipSalePrice = vipSalePrice;
	}

	/**
	 * @dev This function is used for changing allowlist sale price
	 *
	 * @param allowlistSalePrice New allowlist sale price in wei
	 */
	function setAllowlistSalePrice(uint256 allowlistSalePrice) external onlyManager {
		s_allowlistSalePrice = allowlistSalePrice;
	}
	...
	function getCurrentSaleStage() public view returns (SALE_STAGE) {
		if (block.timestamp >= s_saleEndTime) return SALE_STAGE.ENDED;
		if (block.timestamp >= s_allowlistSaleStartTime) return SALE_STAGE.ALLOWLIST;
		if (block.timestamp >= s_vipSaleStartTime) return SALE_STAGE.VIP;
	...
	function getCurrentStagePrice() public view returns (uint256) {
		...
		if (saleStage == SALE_STAGE.VIP) return s_vipSalePrice;
		return s_allowlistSalePrice;
	}
	 ...
	 */
	function getVipSaleStartTime() public view returns (uint32) {
		return s_vipSaleStartTime;
	}
	 ...
	 */
	function getAllowlistSaleStartTime() public view returns (uint32) {
		return s_allowlistSaleStartTime;
	}

	/**
	 * @dev This function is used for getting sale end time
	 *
	 * @return Sale end time in unix time
	 */
	function getSaleEndTime() public view returns (uint32) {
		return s_saleEndTime;
	}
