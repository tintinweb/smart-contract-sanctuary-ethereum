...
contract SquareManager is Initializable, SquareManagerContracts {
    ...
    {
        // require(
        //     iSquare.areSquaresFilled(),
        //     "SquareManager: Not filled yet"
        // );

        setHomeTeam();
        setAwayTeam();
        // setRevealedPositions(); May not need
 ...
 //    {
	...
	// 	uint256 requestId = (3 * timeStamp) % randomRequestLimit;
	...
	// }
     ...
     //    function setRandomRequestLimit(uint256 _randomRequestLimit) 
 //        external 
 //        onlyAdminOrOwner 
 //    {
    //  require(
    ...
    // }
        ...
        //Set the metadata for revealed position
    // function setTokenPositions(uint256[] calldata _tokenIds,
    //     uint256[] calldata _xPos, uint256[] calldata _yPos)
    //     private
    ...
    //         "SquareManager: Not filled yet"
    //     );
    //     //Have to create square method

    //     for (uint256 i = 0; i < _tokenIds.length; i++) {
    //         iSquare.setRevealed(_tokenIds[i], _xPos[i], _yPos[i]);
    //     }
    // }
    ...
    modifier tableIsSet() {
        require(tableSet, 
            "SquareManager: Table is not set"
        );
        _;
    }
}
...
abstract contract SquareManagerContracts is Initializable, SquareManagerState {
        ...
        onlyAdminOrOwner 
    {
        ...
        // MAGIC = MAGIC = IERC20Upgradeable(_magicAddress);
...
{   
    ...
    function __SquareManagerState_init() internal initializer {
        ...
        maxTokensPerWallet = 1; // by default, will increase depending on users
        tableSet = false;
    }
}
...
/**
    ...
    Holds all ERC20, Square ownership knowledge
...
interface ISquareManager {
    ...
    // function sendWinningsTo(address _owner) external;

    // function setHomeTeamNumbers(uint256[] calldata _numbers) external;

    // function setAwayTeamNumbers(uint256[] calldata _numbers) external;

    // function updateOwnerOfSquare() external;
...
interface ISquare {
	...
	// 			  uint256 _amount,
	// 			  uint256[] calldata _xPos,
	// 			  uint256[] calldata _yPos) external;
	...
	function setRevealed(uint256 _tokenId, uint256 _xPos, uint256 _yPos)
		external
		returns(bool);
}
