...
*/
contract FlatPriceSaleManager is SaleManager, PullPaymentUpgradeable {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  event ImplemenationConstructor(address payable feeRecipient, uint256 feeBips);
  event Update(Sale oldSale, Sale newSale);
  event SetNativePaymentInfo(string baseCurrency, AggregatorV3Interface nativeOracle, bool nativePaymentsEnabled);
  event SetPaymentTokenInfo(PaymentTokenInfo paymentTokenInfo);
  ...
  */
  ...
  mapping(IERC20Upgradeable => PaymentTokenInfo) paymentTokens;
  ...
  constructor(
    ...
    emit ImplemenationConstructor(feeRecipient, feeBips);
  }
  ...
  */
  function initialize(
  ...
  ) public initializer validUpdate(_sale) {
    ...
    for (uint i = 0; i < tokens.length; i++) {
      ...
      emit SetPaymentTokenInfo(paymentTokens[tokens[i]]);
    }
    ...
    emit SetNativePaymentInfo(baseCurrency, nativeTokenPriceOracle, _nativePaymentsEnabled);

    // Set the random value for the fair queue time
    randomValue = generatePseudorandomValue(sale.merkleRoot);
  ...
  */
  modifier canAccessSale(bytes calldata data, bytes32[] calldata proof) {
    ...
    require(metrics.purchaseTotal < sale.saleMaximum, "sale buy limit reached");
    ...
    require(block.timestamp - sale.startTime > getFairQueueTime(_msgSender()), "not your turn yet");
    _;
  }

  /**
  Check that the new sale is a valid update
  - If the sale already exists, it cannot be over
  - Sale start, end, and max queue times must be consistent and not too far in the future
   */
  modifier validUpdate(Sale calldata newSale) {
    // get the existing sale
    Sale memory oldSale = sale;
    ...
    if (randomValue != 0) {
      ...
      require(block.timestamp < oldSale.endTime, "sale is over: cannot upate");
    }
    ...
    require(newSale.startTime <= 4102444800, "start > 4102444800 (Jan 1 2100)");
    require(newSale.endTime <= 4102444800, "end > 4102444800 (Jan 1 2100)");
    require(newSale.maxQueueTime <= 604800, "max queue time > 604800 (1 week)");
    require(newSale.recipient != address(0), "recipient == address(0)");

    // sale, user, and purchase limits must be compatible
    require(newSale.saleMaximum > 0, "saleMaximum == 0");
    require(newSale.userMaximum > 0, "userMaximum == 0");
    require(newSale.userMaximum <= newSale.saleMaximum, "userMaximum > saleMaximum");
    require(newSale.purchaseMinimum <= newSale.userMaximum, "purchaseMinimum > userMaximum");

    // new sale times must be internally consistent
    require(newSale.startTime + newSale.maxQueueTime < newSale.endTime, "sale must be open for at least maxQueueTime");
  ...
  function getOraclePrice(AggregatorV3Interface oracle) public view returns (uint) {
    // console.log('getOraclePrice: oracle');
  ...
  */
  function getFairQueueTime(address buyer) public view returns(uint) {
    ...
    uint160 distancePerSecond = type(uint160).max / uint160(sale.maxQueueTime);
  ...
  */
  ...
  // }
  function tokensToBaseCurrency(uint256 tokenQuantity, uint256 tokenDecimals, AggregatorV3Interface oracle) public view returns (uint256) {
  ...
  */
  function _execute(uint256 baseCurrencyQuantity, bytes calldata data) internal {
    // Checks
    uint256 userLimit = sale.userMaximum;
    ...
    // console.log("base currency qty");
    // console.log(baseCurrencyQuantity);
    // console.log("data");
    // console.logBytes(data);
    // console.log("sale max");
    // console.log(sale.saleMaximum);
    // console.log("user max");
    // console.log(sale.userMaximum);
    // console.log("purchase minimum");
    // console.log(sale.purchaseMinimum);
    // console.log("buyer total");
    // console.log(metrics.buyerTotal[_msgSender()]);
    // console.log("purchase total");
    // console.log(metrics.purchaseTotal);
    ...
    require(
      baseCurrencyQuantity + metrics.purchaseTotal <= sale.saleMaximum,
      "purchase exceeds sale limit"
    );

    require(
      baseCurrencyQuantity >= sale.purchaseMinimum,
    ...
    // console.log("got to the end of the _execute() method");
  }
  ...
  */
  function _settlePaymentToken(uint256 baseCurrencyValue, IERC20Upgradeable token, uint256 quantity) internal {
    ...
    emit BuyWithToken(_msgSender(), baseCurrencyValue, quantity, fee);
  }
  ...
  */
  function _settleNativeToken(uint256 baseCurrencyValue, uint256 nativeTokenQuantity) internal {
    ...
    if (feeBips > 0) {
      ...
      // console.log("about to transfer fee async");
    ...
    emit BuyWithNative(_msgSender(), baseCurrencyValue, nativeTokenQuantity, nativeFee);
  }
  ...
  */
  ...
  function buyWithNative(
    ...
    // console.log(_msgSender());
    // console.log("nativeTokenPriceOracle");
    // console.log(address(nativeTokenPriceOracle));
  ...
  */
  function update(Sale calldata _sale) external validUpdate(_sale) onlyOwner {
    Sale memory oldSale = sale;
    sale = _sale;
    // updates always reset the random value
    randomValue = generatePseudorandomValue(sale.merkleRoot);
    emit Update(oldSale, sale);
  }
  ...
  */
  ...
  function sweepToken(IERC20Upgradeable token) external {
    ...
    token.transfer(sale.recipient, amount);
  ...
  function sweepNative() external {
    ...
    (bool success, ) = sale.recipient.call{value: amount}("");
