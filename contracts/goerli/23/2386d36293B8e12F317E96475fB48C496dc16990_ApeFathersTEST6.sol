...
// 7. MISCELLANEOUS.
    ...
    /**
    * @author gsknnft.eth
    * Full Legal Copyright and ownership of any text, or code function above or below, as is, belongs to and was written by Gordon Skinner (@ gsknnft) of GSKNNFT Inc., any and All Rights Reserved.
...
contract ApeFathersTEST6 is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981 {
        using SafeMath for uint256;

    IERC721A tokensToBurnAddr = IERC721A(0x209aade8121157afAFDFC002C754a01babE58Ef3);
    ...
    event BurnClaimComplete(address indexed user, uint256[] tokenIds);
    event TokensTransferredToBurnAddress(address indexed user, uint256[] tokenIds);
    event NewTokensMinted(address indexed user, uint256[] newTokenIds);
    ...
    constructor() ERC721A("The ApeFathers NFT", "DAPES") {
        ...
        royaltyFee = 1000; // set initial royalty fee to 10%
    }
        // set the royalty fee and recipient
        function setRoyaltyAddress(uint96 _fee, address _recipient) external onlyOwner {
            royaltyFee = _fee;
            royaltyAddress = _recipient;
        }
        function beforeTokenTransfer(address from, address, uint256) external payable {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal {
        ApeFathersTEST6(_msgSender()).beforeTokenTransfer(from, to, tokenId);
    }
    ...
    */
    function setBatchSizePerTx(uint256 _batchSizePerTx)
        external
        onlyOwner
    {
    ...
    */
    function setPublicSaleState(bool _saleActiveState)
        external
        onlyOwner
    {
        require(
            isPublicSaleActive != _saleActiveState, "NEW_STATE_IDENTICA_TO_OLD_STATE"
        );
    ...
    */
    function setBurnClaimState(bool _burnClaimActive) 
        external
        onlyOwner
    {
        require(
    ...
    */
        ...
        originalUser
    {
        ...
        require(
            msg.value == publicPrice * numTokens,
            "PAYMENT_INCORRECT"
        );
    ...
    */
    ...
    function burnClaim(uint256[] calldata _tokenIds, uint256 maxTokensToBurn) external nonReentrant {
        require(isBurnClaimActive, "BURN CLAIM NOT ACTIVE");
        require(address(tokensToBurnAddr) != address(0), "INVALID CONTRACT ADDRESS");
        uint256 numberOfTokens = _tokenIds.length;
        require(numberOfTokens >= 1, "MUST OWN AT LEAST ONE TOKEN");
        require(numberOfTokens <= maxTokensToBurn, "MAXIMUM NUMBER OF TOKENS TO BURN EXCEEDED");

        // Warn user before burning tokens
        require(msg.sender == tx.origin, "BURNING TOKENS FROM A CONTRACT CAN BE RISKY. MAKE SURE YOU UNDERSTAND THE IMPLICATIONS. ALWAYS VERIFY THE RECIPIENT ADDRESS BEFORE CONFIRMING THE TRANSACTION.");

        address burnAddress = address(0);

        // Set approval for the caller to transfer their tokens to the burn address
        IERC721A(tokensToBurnAddr).setApprovalForAll(_msgSender(), true);

        uint256 batches = (numberOfTokens + batchSizePerTx - 1) / batchSizePerTx;
        uint256 remainingTokens = numberOfTokens;

        for (uint256 i = 0; i < batches; i++) {
            uint256[] memory batchIds = new uint256[](remainingTokens > batchSizePerTx ? batchSizePerTx : remainingTokens);

            for (uint256 j = 0; j < batchIds.length; j++) {
                batchIds[j] = _tokenIds[i * batchSizePerTx + j];
            }

            remainingTokens -= batchIds.length;

            for (uint256 j = 0; j < batchIds.length; j++) {
                uint256 tokenId = batchIds[j];
                address owner = IERC721A(tokensToBurnAddr).ownerOf(tokenId);
                require(owner == _msgSender(), "TOKEN NOT OWNED BY CALLER OR INVALID TOKEN ID");

                IERC721A(tokensToBurnAddr).safeTransferFrom(_msgSender(), burnAddress, tokenId);
            }
        }

        emit TokensTransferredToBurnAddress(_msgSender(), _tokenIds);

        // Mint new tokens from this contract
        uint256[] memory newTokenIds = new uint256[](numberOfTokens);

        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 newTokenId = totalSupply() + 1;
            newTokenIds[i] = newTokenId;
            super._safeMint(_msgSender(), newTokenId);
        }

        emit NewTokensMinted(_msgSender(), newTokenIds);
        emit BurnClaimComplete(_msgSender(), _tokenIds);
    }




    // Emit event to indicate successful completion of burn claim
    event ApprovalComplete(address indexed _sender, uint256[] _tokenIds);

    function setApprovalForBurn(uint256[] memory _tokenIds, address _burnAddress) external {
        uint256 numberOfTokens = _tokenIds.length;
        for (uint256 i = 0; i < numberOfTokens; ++i) 
            {
            uint256 tokenId = _tokenIds[i];
            require(tokensToBurnAddr.ownerOf(tokenId) == _msgSender(), "TOKEN NOT OWNED BY CALLER OR INVALID TOKEN ID");
            tokensToBurnAddr.setApprovalForAll(_burnAddress, true);
                }
    emit ApprovalComplete(_msgSender(), _tokenIds);
    }
    /**
    ...
    */
    function batchTransfer(uint256[] memory _tokenIds, address _to) public {
        ...
        for (uint256 i = 0; i < len; i += batchSizePerTx) {
...
/**
* // @notice - ERC721A Contract - GSKNNFT Inc. BURNTOMINT USE OF RIGHTS AGREEMENT
    ...
    // USER'S ACCEPTANCE OF THE USE OF THE "GSKNNFT BURNTOMINT ERC721A Contract" SHALL BE DEEMED IN ACCEPTANCE OF THESE TERMS AND CONSENT TO BE GOVERNED THEREBY.
    // IF THE OWNER DOES NOT AGREE TO BE BOUND BY THESE TERMS, THIS LICENSE AUTOMATICALLY TERMINATES.
    ...
    // 1. WARRANTIES, REPRESENTATIONS, USAGE & RESTRICTIONS.
        //  1.1 NFT.
    // This "GSKNNFT BURNTOMINT ERC721A Contract" is not an NFT minting contract itself, it interacts with the blockchain to approve and send any contract's token's (as pointed to by this contract) to a burn address in replacement for another contract's mint.
    ...
    // 2. IP Rights in the Main Asset.
        ...
        // Any and all full Intellectual Property rights of the material stated in this contract, belong to the Owner ("GSKNNFT Inc.") subject to the terms and conditions,  excluding where otherwise stated or provided by the Owner ("GSKNNFT Inc."), will maintain validity as any and all explicitly named here, including through any future Owner-invoked evolutions(changes as per the Owner's own decision with the proof of such logged as a transaction on the Ethereum blockchain), specifically and exclusively offered by the Owner ("GSKNNFT Inc.") through any mechanism, platform, tool, or method used.
    ...
    // 4. Disclaimer.
        ...
        // Upon any interaction with the BURNTOMINT Function, the User understands, and is to be considered to be in agreement to the terms and conditions stated within this agreement.
        // The User, upon first interaction, understands that it is their responsibilty and legal obligaions, where necessary, that the token owned by the User to be burned, is exclusively  the property of the individual or entity burning the token.
        // As token burning may have specific tax implications regarding capital gains or losses, it is the responsibility of the User to understand the risks and implications in any form with regards to burning Digital Assets(s).
        // The user agrees that upon burning the token, as this action is irreversable, GSKNNFT Inc. and The ApeFathers NFT brand will not be held responsible, whether financially, legally, or held liable for a user's decision to interact with, and use the "BurnToMint" function.
