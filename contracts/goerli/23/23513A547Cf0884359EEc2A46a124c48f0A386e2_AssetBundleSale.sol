...
contract AssetBundleSale is IERC1155Receiver, IERC721Receiver, Ownable {
    ...
    modifier onlyManager() {
        require(_msgSender() == _governance.getManager(), "You don't have permission");
        _;
    }

    function updateGovernance(address newGovernance) external onlyManager {
        require(newGovernance != address(_governance), "Nothing change");

        emit UpdateGovernance(newGovernance, address(_governance));
    ...
    function withdrawSale(uint256 saleId, address to) external {
        ...
        ERC1155ERC721(sale.asset).safeBatchTransferFrom(address(this), to, ids, amounts, "");
    }
    ...
    function buyBundleWithToken(
        ...
        /** Buyer transfer fund to seller and feeReceiver */
        payment(sale.paymentToken, sale.seller, _msgSender(), priceRequired);

        /** Seller transfer asset to buyer */
        ERC1155ERC721(sale.asset).safeBatchTransferFrom(address(this), to, sale.ids, sale.amounts, "");

        emit BundleSold(saleId, _msgSender(), sale.asset, sale.paymentToken, sale.ids, sale.amounts, sale.pricePerPack);
    }
    ...
    function payment(
        ...
        /** Calculate Fee */
        uint256 feeSale = FullMath.mulDiv(priceRequired, _fee, FEE_DENOMINATION);

        /** Using ERC20 */
        if(priceRequired > 0 && paymentToken != address(0)) {
            require(msg.value == 0, "No need to transfer native coin");
            ERC20BaseToken(paymentToken).safeTransferFrom(_msgSender(), seller, priceRequired - feeSale);

            if(_feeReceiver != address(0) && feeSale > 0) {
                ERC20BaseToken(paymentToken).safeTransferFrom(_msgSender(), _feeReceiver, feeSale);
            }
        }

        /** Using native token */
        if(priceRequired > 0 && paymentToken == address(0)) {
            require(msg.value >= priceRequired, "Insufficient Fund");
            payable(seller).transfer(priceRequired - feeSale);

            if(_feeReceiver != address(0) && feeSale > 0) {
                payable(_feeReceiver).transfer(feeSale);
            }

            /* Allow overshoot for variable-price, refund difference. */
            uint256 diff = msg.value - priceRequired;
            if (diff > 0) {
                payable(buyer).transfer(diff);
            }
        }
    }
    ...
    function onERC721Received(
        ...
        _setupBundle(from, ids, amounts, data);
    ...
    function onERC1155BatchReceived(
        ...
        _setupBundle(from, ids, values, data);
    ...
    function _setupBundle(
        ...
        require(data.length > 0, "Metadata is not set");
        (uint256 pricePerPack, uint256 assetBytes, uint256 paymentTokenBytes) = abi.decode(
            data,
            (uint256, uint256, uint256)
        );
        address asset = address(uint160(assetBytes));
        address paymentToken = address(uint160(paymentTokenBytes));

        require(validateSale(ids, amounts, asset, paymentToken), "Sale is invalid");
        ...
        emit BundleSale(saleId, from, asset, paymentToken, ids, amounts, pricePerPack);
    }
    ...
    function validateSale(
        ...
        if(!_governance.getAcceptedPayments(paymentToken)) {
            return false;
        }
