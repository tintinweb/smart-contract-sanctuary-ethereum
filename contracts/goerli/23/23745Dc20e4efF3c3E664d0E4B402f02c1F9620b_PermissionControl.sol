...
contract PermissionControl is IPermissionControl, Initializable, EIP712Upgradeable {
    enum VerifyType {
        MinterWhitelist,
        CanvasCreatorWhitelist,
        MinterBlacklist,
        CanvasCreatorBlacklist
    }

    mapping(bytes32 => Whitelist) public whitelists;
    mapping(bytes32 => Blacklist) public blacklists;
    ...
    bytes32 internal constant ADDPERMISSION_TYPEHASH = keccak256(
        abi.encodePacked(
            ...
            abi.encodePacked("Blacklist(bytes32 minterMerkleRoot,bytes32 canvasCreatorMerkleRoot)"),
            abi.encodePacked(
                "Whitelist(",
    ...
    bytes32 internal constant BLACKLIST_TYPEHASH = keccak256(
        abi.encodePacked(abi.encodePacked("Blacklist(bytes32 minterMerkleRoot,bytes32 canvasCreatorMerkleRoot)"))
    );
    ...
    function addPermission(bytes32 daoId, Whitelist calldata whitelist, Blacklist calldata blacklist) external {
        require(msg.sender == ownerProxy.ownerOf(daoId), "PermissionControl: not DAO owner");
        _addPermission(daoId, whitelist, blacklist);
    }

    function _addPermission(bytes32 daoId, Whitelist calldata whitelist, Blacklist calldata blacklist) internal {
        // add whitelist
        whitelists[daoId] = whitelist;
        emit WhitelistModified(daoId, whitelist);

        // add blacklist
        blacklists[daoId] = blacklist;
        emit BlacklistModified(daoId, blacklist);
    }

    /**
     * @dev After the DAO is created, can modify whitelist and blacklist directly
     */
    function modifyPermission(bytes32 daoId, Whitelist calldata whitelist, Blacklist calldata blacklist) external {
        require(msg.sender == ownerProxy.ownerOf(daoId), "PermissionControl: not DAO owner");
                ...
                abi.encode(
                    ...
                    keccak256(
                        abi.encode(
                            ...
                            whitelist.minterNFTHolderPass,
                            whitelist.canvasCreatorMerkleRoot,
                            whitelist.canvasCreatorNFTHolderPass
                        )
                    ),
                    keccak256(
                        abi.encode(BLACKLIST_TYPEHASH, blacklist.minterMerkleRoot, blacklist.canvasCreatorMerkleRoot)
                    )
                )
            )
        );
     ...
     */
    function inMinterBlacklist(bytes32 daoId, address _account, bytes32[] calldata _proof)
        external
        view
        returns (bool)
    {
        return _verifyProof(daoId, _account, _proof, VerifyType.MinterBlacklist);
    }
     ...
     * @param _account The address to check
     */
    function inCanvasCreatorBlacklist(bytes32 daoId, address _account, bytes32[] calldata _proof)
        external
        view
        returns (bool)
    {
        return _verifyProof(daoId, _account, _proof, VerifyType.CanvasCreatorBlacklist);
    }

    /**
     * @dev If merkleRoot of DAO is set, throws if given _merkle proof is not correct
     *      If NFTHolderPass of DAO is set, throws if given _account is not the owner of the NFT
     * @param daoId DAO id
     * @param _account The address to check
    ...
    function inMinterWhitelist(bytes32 daoId, address _account, bytes32[] calldata _proof)
        ...
        returns (bool)
    {
        _verifyProof(daoId, _account, _proof, VerifyType.MinterWhitelist);
    }

    /**
     * @dev Throws if _merkleRoot is not correct
     *      If merkleRoot of DAO is set, return `false` if given _merkle proof is not correct
     *      If NFTHolderPass of DAO is set, return `false` if given _account is not the owner of the NFT
     * @param daoId DAO id
     * @param _account The address to check
     * @param _proof The merkle proof
     */
    function inCanvasCreatorWhitelist(bytes32 daoId, address _account, bytes32[] calldata _proof)
        ...
        returns (bool)
    {
        _verifyProof(daoId, _account, _proof, VerifyType.CanvasCreatorWhitelist);
    }

    function _verifyProof(bytes32 daoId, address _account, bytes32[] calldata _proof, VerifyType _type)
        internal
        view
        returns (bool)
    {
        if (_type == VerifyType.MinterBlacklist) {
            Blacklist memory blacklist = blacklists[daoId];

            if (blacklist.minterMerkleRoot == bytes32(0)) return true;
            if (
                MerkleProofUpgradeable.verifyCalldata(
                    _proof, blacklist.minterMerkleRoot, keccak256(bytes.concat(keccak256(abi.encode(_account))))
                )
            ) {
                return true;
            }
            return false;
        } else if (_type == VerifyType.CanvasCreatorBlacklist) {
            Blacklist memory blacklist = blacklists[daoId];

            if (blacklist.canvasCreatorMerkleRoot == bytes32(0)) return true;
            if (
                MerkleProofUpgradeable.verifyCalldata(
                    _proof, blacklist.canvasCreatorMerkleRoot, keccak256(bytes.concat(keccak256(abi.encode(_account))))
                )
            ) {
                return true;
            }
            return false;
        } else if (_type == VerifyType.MinterWhitelist) {
            Whitelist memory whitelist = whitelists[daoId];

            if (whitelist.minterMerkleRoot == bytes32(0) && whitelist.minterNFTHolderPass == address(0)) return true;
            if (
                MerkleProofUpgradeable.verifyCalldata(
            ...
            if (IERC721Upgradeable(whitelist.minterNFTHolderPass).balanceOf(_account) > 0) {
                return true;
            }
            return false;
        } else if (_type == VerifyType.CanvasCreatorWhitelist) {
            Whitelist memory whitelist = whitelists[daoId];

            if (whitelist.canvasCreatorMerkleRoot == bytes32(0) && whitelist.canvasCreatorNFTHolderPass == address(0)) {
                return true;
            }
            if (
                MerkleProofUpgradeable.verifyCalldata(
            ...
            if (IERC721Upgradeable(whitelist.canvasCreatorNFTHolderPass).balanceOf(_account) > 0) {
                return true;
            }
            return false;
        }
    ...
    function setOwnerProxy(ID4AOwnerProxy _ownerProxy) external {
        require(
            IAccessControlUpgradeable(address(ownerProxy)).hasRole(keccak256("DEFAULT_ADMIN_ROLE"), msg.sender),
            "Not owner"
        );
        ownerProxy = _ownerProxy;
    }
...
interface IPermissionControl {
    ...
    event BlacklistModified(bytes32 indexed daoId, Blacklist blacklist);
