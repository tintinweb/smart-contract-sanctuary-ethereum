...
contract Streamer is Ownable {
    event Opened(address, string);
    event Challenged(address);
    event Withdrawn(address, string);
    ...
    function fundChannel() public payable {
       ...
       require(balances[msg.sender] == 0, "Ruby already has a running channel");
       balances[msg.sender] = msg.value;
       canCloseAt[msg.sender] = block.timestamp + 24 hours;
       emit Opened(msg.sender, "Opened");

    }

    function timeLeft(address channel) public view returns (uint256) {
        if (balances[channel] > 0) {
    ...
    function withdrawEarnings(Voucher calldata voucher) public {
        ...
        signer = ecrecover(prefixedHashed, voucher.sig.v, voucher.sig.r, voucher.sig.s);
            // - require that the recovered signer has a running channel with balances[signer] > v.updatedBalance
        require(balances[signer] > voucher.updatedBalance,"Signer has not a running channel with balance > udated balance");
            // - calculate the payment when reducing balances[signer] to v.updatedBalance
        payout = balances[signer] - voucher.updatedBalance;
   ...
   function challengeChannel() public{
        require(balances[msg.sender] > 0, "No channel is opened.");
   ...
   function defundChannel() public{
        ...
        require(canCloseAt[msg.sender] < block.timestamp, "It's early to close the channel");
