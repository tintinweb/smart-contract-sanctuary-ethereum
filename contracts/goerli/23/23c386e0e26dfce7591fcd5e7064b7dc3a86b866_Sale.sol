...
contract Sale is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function unlockToken(uint256 _id) public view checkPresaleId(_id) onlyOwner{
        require(block.timestamp >= presale[_id].endTime,"You can only unlock on finalize");
    }
     ...
     * @param _id Presale id
     */
    function claim(address user, uint256 _id) public checkPresaleId(_id) returns (bool) {
        ...
        if(block.timestamp >= presale[_id].endTime  ){
          ...
          uint256 payableAmount = userVesting[user][_id].totalAmountCopy * 50/100;
          userVesting[user][_id].claimedAmount += payableAmount;
          bool status = IERC20Upgradeable(presale[_id].saleToken).transfer(
           ...
           emit TokensClaimed(user, _id, payableAmount, block.timestamp);
        }
        }

        if( block.timestamp >= presale[_id].endTime + 30 days){
            if(userVesting[user][_id].isFirstClaim == false){
          uint256 payableAmount  =userVesting[user][_id].totalAmountCopy*5/100;
          userVesting[user][_id].claimedAmount += payableAmount;
          bool status = IERC20Upgradeable(presale[_id].saleToken).transfer(
            user,
            payableAmount
        ...
        // if( block.timestamp >= presale[_id].endTime){
        //    if(userVesting[user][_id].counterForCycle)
        // uint256 payableAmount  =amount*5/100;
