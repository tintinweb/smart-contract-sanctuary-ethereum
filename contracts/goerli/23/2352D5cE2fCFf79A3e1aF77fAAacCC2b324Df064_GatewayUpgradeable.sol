    ...
    ReentrancyGuardUpgradeable
{
    ...
    // default fee for "requestToRouter"
    uint256 public requestToRouterDefaultFee;
    // default fee for "requestToDest"
    uint256 public requestToDestDefaultFee;
    ...
    bytes32 constant CROSS_TALK_ACK_METHOD_NAME = 0x63726F737354616C6B41636B0000000000000000000000000000000000000000;
    ...
    event RequestToRouterEvent(
        Utils.InboundSourceInfo inboundSourceInfo,
    ...
    event CrossTalkAckReceivedEvent(
        string relayerRouterAddress,
        uint64 indexed eventNonce,
        uint64 indexed eventIdentifier,
        uint64 chainType,
    ...
    function makeCheckpoint(
        Utils.ValsetArgs memory _valsetArgs,
    ...
    function setBridgeFees(
        uint256 _requestToRouterDefaultFee,
        uint256 _requestToDestDefaultFee
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // default fee for "requestToRouter"
        requestToRouterDefaultFee = _requestToRouterDefaultFee;
        // default fee for "requestToDest"
        requestToDestDefaultFee = _requestToDestDefaultFee;
    }
    ...
    function requestToRouter(
        ...
        // "insufficient fees sent" => "C03"
        require(msg.value >= requestToRouterDefaultFee, "C03");
        address txOrigin;
        assembly {
            txOrigin := origin()
        }
        ...
        if (routeAmount > 0) {
            // "empty recipient" => "C04"
            require(keccak256(abi.encodePacked(routeRecipient)) != keccak256(abi.encodePacked("")), "C04");
            vault.deposit(routeAmount, msg.sender);
        }

        address feePayer = msg.sender;
        if (feePayerEnum == Utils.FeePayer.USER) {
            feePayer = txOrigin;
        } else if (feePayerEnum == Utils.FeePayer.NONE) {
            feePayer = address(0);
        }
        eventNonce++;

        emit RequestToRouterEvent(
            Utils.InboundSourceInfo (
                routeAmount,
                routeRecipient,
                eventNonce,
                chainType,
    ...
    function requestToDest(
        ...
        require(msg.value >= requestToDestDefaultFee, "C03");
        ...
        Utils.SourceChainParams memory srcChainParams = Utils.SourceChainParams(
            destChainNonces[destChainParams.destChainId][destChainParams.destChainType],
            requestArgs.expTimestamp,
            requestArgs.isAtomicCalls,
            chainType,
            chainId
        );
        ...
        address feePayer = msg.sender;
        if (requestArgs.feePayerEnum == Utils.FeePayer.USER) {
            feePayer = txOrigin;
        } else if (requestArgs.feePayerEnum == Utils.FeePayer.NONE) {
            feePayer = address(0);
        }
    ...
    function requestFromRouter(
        ...
        // "request message already handled" => "C05"
        require(outboundNonceMapping[requestPayload.routerBridgeAddress][requestPayload.outboundTxNonce] == 0, "C05");
        ...
        if (requestPayload.routeAmount > 0) {
            ...
            vault.handleWithdraw(requestPayload.routeAmount, toAddress(requestPayload.routeRecipient));
        }
        ...
        address shieldModuleAddress = toAddress(requestPayload.asmAddress);
        bool isValidRequest = true;
        if (isContract(shieldModuleAddress)) {
            AddOnShieldModule asm = AddOnShieldModule(shieldModuleAddress);
            isValidRequest = asm.verifyRequestFromRouter(
                requestPayload.routerBridgeAddress,
                requestPayload.outboundTxNonce,
                requestPayload.isAtomic,
                requestPayload.chainTimestamp,
        ...
        if (block.timestamp > requestPayload.expTimestamp || !isValidRequest) {
    ...
    function executeHandlerCalls(
        ...
        for (uint8 i = 0; i < handlers.length; i++) {
            ...
            execData[i] = data;
            if (isAtomic && !success) {
                revert ExecuteCallsRevert({ execFlags: execFlags, execData: execData });
            }
        }
    ...
    function requestFromSource(
        ...
        // chainId, chainType data is present inside the signature validation
        // valsetArgs validation will be handled by the makeCheckPoint function
        // "cross-talk request message already handled" => "C06"
        require(
            crossTalkNonceMapping[crossTalkPayload.sourceParams.chainId][crossTalkPayload.sourceParams.chainType][
...

        {
            bytes memory encodedABI = abi.encode(
                ...
                crossTalkPayload.chainTimestamp,
                crossTalkPayload.expTimestamp,
                crossTalkPayload.asmAddress,
        ...
        bool isValidRequest = true;
        {
            address shieldModuleAddress = toAddress(crossTalkPayload.asmAddress);

            if (isContract(shieldModuleAddress)) {
                AddOnShieldModule asm = AddOnShieldModule(shieldModuleAddress);
                isValidRequest = asm.verifyRequestFromSource(
                    crossTalkPayload.sourceParams.caller,
                    crossTalkPayload.sourceParams.chainId,
                    crossTalkPayload.sourceParams.chainType,
                    crossTalkPayload.eventIdentifier,
        ...
        uint64 crossTalkNonce = destChainNonces[crossTalkPayload.sourceParams.chainId][
            crossTalkPayload.sourceParams.chainType
        ];
    ...
    function executeCrossTalkCalls(
        ...
        for (uint8 i = 0; i < handlers.length; i++) {
            address handler = toAddress(handlers[i]);
            execFlags[i] = isContract(handler);

            if (execFlags[i]) {
                if (!isReadCall) {
                    (execFlags[i], execData[i]) = handler.call(
                        abi.encodeWithSelector(
                            ICrossTalkApplication.handleRequestFromSource.selector,
                            srcContractAddress,
                            payloads[i],
                            srcChainId,
                            srcChainType
                    ...
                    (execFlags[i], execData[i]) = handler.call(payloads[i]);
                }
            }
            // execFlags[i] = success;
            // execData[i] = data;
            if (isAtomic && !execFlags[i]) {
                revert ExecuteCallsRevert({ execFlags: execFlags, execData: execData });
            }
        }
        return (execFlags, execData);
    ...
    function crossTalkAck(
        ...
        // valsetArgs validation will be handled by the makeCheckPoint function
        require(
            crossTalkNonceMapping[crossTalkAckPayload.destChainId][crossTalkAckPayload.destChainType][
        ...
        emit CrossTalkAckReceivedEvent(
            crossTalkAckPayload.relayerRouterAddress,
            eventNonce,
            crossTalkAckPayload.eventIdentifier,
            chainType,
            chainId,
    ...
    function executeCrossTalkAck(
        ...
        if (success) {
            (success, data) = handler.call(
                abi.encodeWithSelector(
                    ICrossTalkApplication.handleCrossTalkAck.selector,
                    eventIdentifier,
                    execFlags,
                    execData
                )
    ...
    function toBytes32(address _address) public pure returns (bytes32) {
        bytes32 convertedAddress = bytes32(uint256(uint160(_address)));
