...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    event RaffleRandomWordsRequest(uint256 indexed requestId);
    ...
    //* Functions - simple / payable
    function enterRaffle() public payable {
        if (msg.value < i_entranceFee) {
            revert Raffle__NotEnoughEth();
        }

        if (s_raffleState != RaffleState.OPEN) {revert Raffle__NotOpenYet();}
    ...
    function checkUpkeep(bytes memory) public override returns (bool upkeepNeeded, bytes memory) {
        ...
        bool timePassed = ((block.timestamp - s_lastBlockTime) > i_interval);
    ...
    function performUpkeep(bytes calldata) external override {
        ...
        emit RaffleRandomWordsRequest(requestId);
    }
