...
contract RollupHandler is IGetRollupInfo, IExecuteProposal, HandlerHelpers {
    ...
    mapping(uint128 => Metadata) public rollupMetadata;
        ...
        onlyBridge
    {
        ...
        uint128 nonceAndID = (uint128(md.nonce) << 64) | uint128(md.domainID);
    ...
    {
        address l1TargetAddress = _resourceIDToTokenContractAddress[resourceID];
        require(l1TargetAddress != address(0), "invalid resource ID");

        uint128 nonceAndID = (uint128(nonce) << 64) | uint128(originDomainID);
        Metadata memory md = rollupMetadata[nonceAndID];
        require(md.resourceID == resourceID, "different resource IDs");

        return (l1TargetAddress, md.rootHash, md.totalBatches);
    }
}
