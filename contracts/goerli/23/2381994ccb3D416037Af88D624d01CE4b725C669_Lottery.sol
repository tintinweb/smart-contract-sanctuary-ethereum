 ...
 * @author - Roman Scher
 */
contract Lottery is VrfConsumer, AutomationCompatibleInterface {
    error NotOpen();
    ...
    constructor(
        uint _entranceFee,
        uint _roundInterval,
        address vrfCoodinatorAddress,
        address linkTokenAddress,
        bytes32 vrfGasLane,
        uint32 vrfCallbackGasLimit,
        uint16 vrfNumberOfRequestConfirmations
    )
        VrfConsumer(
            ...
            vrfGasLane,
            vrfCallbackGasLimit,
            vrfNumberOfRequestConfirmations
        )
    {
        isOpen = true;
        round = 1;
...
abstract contract VrfConsumer is VRFConsumerBaseV2 {
    ...
    uint16 private immutable vrfNumberOfRequestConfirmations;
    uint32 private constant VRF_NUMBER_OF_RANDOM_VALUES = 1;
    ...
    constructor(
    ...
    ) VRFConsumerBaseV2(vrfCoodinatorAddress) {
        vrfCoordinator = VRFCoordinatorV2Interface(vrfCoodinatorAddress);
        ...
        vrfNumberOfRequestConfirmations = numberOfRequestConfirmations;
    }

    function fundVrfSubscription(uint amount) external {
