    ...
    Ownable
{
    ...
    function mint(uint256 tokenId, uint256 qty) external payable {
        doMint(tokenId, configurations[tokenId].tokenPrice, qty, msg.value, msg.sender);        
    }
    ...
    function whitelistMint(bytes32[] memory proof,uint256 tokenId, uint256 qty)  external payable  {
        // verify whitelisted
        require(merkleVerify(proof,tokenId), 'Auth failed');
        doMint(tokenId, configurations[tokenId].tokenPrice - 30000000000000000, qty, msg.value, msg.sender);
    }

    function merkleVerify(bytes32[] memory proof,uint256 tokenId) public view returns(bool result){
    ...
    function doMint(
        ...
        if (mintedPerWallet[tokenId][recipient] + qty > config.maxAllowance)
            revert ExceededAllowance();
        _mint(recipient, tokenId, qty, "");
        mintedPerWallet[tokenId][recipient] += qty;    
    ...
    function setRoot(uint256 tokenId,bytes32 _root)  external  onlyOwner{
