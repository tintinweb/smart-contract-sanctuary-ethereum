...
contract MyToken is ERC721,ERC721Enumerable, ERC2981, ERC721URIStorage, Ownable {
   // public uint balance[] = [1, 2, 3];
    ...
    string public baseURI = "https://gateway.pinata.cloud/ipfs/Qmcf4CLw8x8oCy3svzUQSTah6unrNBHjoXkuALLHbRGrPJ/";
    uint public MAX_SUPPLY = 500;
    string public contractURI;
    /*function contractURI() public pure returns (string memory) {
        return "https://api.npoint.io/91c61c485a95e8a3666c/";
    }*/
    constructor(string memory name, string memory symbol, uint96 _royaltyFeesInBips, string memory _contractURI) ERC721(name, symbol) {
    ...
    function BulkAirDropNFT(address[] calldata to, uint256 [] calldata amounts)public onlyOwner{
        require(to.length == amounts.length, "Receivers and Amount are diffrent");
        for(uint256 i = 0; i < to.length; i++){
            
            if(amounts[i]>2){
                for(uint j = 0; j < amounts[i]; j++ ){
                    uint256 tokenId= rarenft[rarenft.length - 1];
                    MinttheNft(to[i],tokenId , 0);
                    rarenft.pop();    
                }
            }else{
                uint256 tokenId= rarenft[rarenft.length - 1];
