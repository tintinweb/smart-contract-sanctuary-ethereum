...
contract Metaisland is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable {
    ...
    constructor() ERC721("Metaisland", "2Dpfp") {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://test.wok-os.com/metadata/";
    }
   ...
   function mintRent(address to, string memory uri) public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
    ...
    function mintGift(address to, string memory uri) public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
    ...
    function buyNFT(address to, string memory uri) public payable {
        require(msg.value >= _price, "Ether insufficient funds");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
