...
error RandomWinner__OnlyOwner();
error RandomWinner__upkeepNotNeeded();

contract RandomWinner is VRFConsumerBaseV2, KeeperCompatibleInterface {
    VRFCoordinatorV2Interface private immutable i_VRFcoordinator;
    bytes32 private immutable i_keyHash;
    uint32 private immutable i_callbackGasLimit;
    uint16 private constant REQUEST_CONFIRMATION = 3;
    uint32 private constant CANT_RANDOM_NUMBERS = 1;
    uint64 private immutable i_subscriptionId;
    address private immutable i_owner;
    Raffle private immutable i_raffleContract;
    uint256 private s_lastTimestamp;
    uint256 private s_interval;

    modifier onlyOwner() {
        if (msg.sender != i_owner) {
            revert RandomWinner__OnlyOwner();
        }
        _;
    }
    ...
    constructor(
        ...
        i_raffleContract = Raffle(raffleAddress);
        s_interval = interval;
    ...
    function fulfillRandomWords(
        ...
        i_raffleContract.pickRandomWinner(randomWords[0]);
        s_lastTimestamp = block.timestamp;
    }
    ...
    {
        bool isOpen = i_raffleContract.isRaffleOpen();
        bool isIntervalPassed = (block.timestamp - s_lastTimestamp) >
            s_interval;
        bool isAtLeastTwoPlayers = i_raffleContract.getPlayerListLength() > 1;
        upkeepNeeded = isOpen && isIntervalPassed && isAtLeastTwoPlayers;
    ...
    function performUpkeep(
        ...
        if (upkeepNeeded) {
            i_raffleContract.setRaffleToCalculating();
            ...
            revert RandomWinner__upkeepNotNeeded();
        }
    }
    ...
    function getRaffle() external view returns (Raffle) {
