...
contract BearWithUs is ERC721A, Ownable {
    ...
    // constructor(string memory _baseUri) ERC721A("Bear WithUs", "BWUS") {
    constructor() ERC721A("Bear WithUs", "BWUS") {
        string memory _baseUri = "https://sampleonly.com/api/bwus/";
        setBaseURI(_baseUri);
    }
    ...
    function mint(uint256 _mintQuantity) public payable {
        ...
        if (msg.sender != owner()) {
            require(
                ...
                "Exceeded the maximum amount to be minted per transaction."
            );

            uint256 ownerMintedCount = balanceOf(msg.sender);
            require(
                ownerMintedCount + _mintQuantity <= nftPerAddressLimit,
                "Exceeded the maximum NFTs allowed per address"
            );
    ...
    function freeMint() public {
        ...
        bool hasFreeMint = isFreeMint(msg.sender);

        require(supply + 1 <= maxSupply, "max NFT limit exceeded");
        require(hasFreeMint, "No free mints available for this account.");
    ...
    function setFreeMints(address[] calldata _freeMints, bool _condition)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _freeMints.length; i++) {
            freeMints[_freeMints[i]] = _condition;
        }
    }
