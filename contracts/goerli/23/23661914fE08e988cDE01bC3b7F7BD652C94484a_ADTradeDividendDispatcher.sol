...
contract ADTradeDividendDispatcher is Father {
    ...
    function _getSwapHelerAddress() external view returns (address) {
        return address(swapHelper);
    }

    function _dividendToDead(uint256 amount) internal virtual {
        ADAOToken.safeTransfer(deadAddress, amount);
        globalDeadTokenAmount = globalDeadTokenAmount.add(amount);
        emit SellDividendDeadDisptached(deadAddress, amount);
    }

    function _dividendToFund(uint256 amount) internal virtual {
        ADAOToken.safeTransfer(fundAddress, amount);
        globalFundTokenAmount = globalFundTokenAmount.add(amount);
        emit SellDividendFundDisptached(fundAddress, amount);
    }

    function _dividendToHolding(uint256 amount) internal virtual {
        ADAOToken.safeTransfer(holdingRewardAddress, amount);
        globalHoldingTokenAmount = globalHoldingTokenAmount.add(amount);
        emit SellDividendHoldingDisptached(holdingRewardAddress, amount);
    }

    function _dividendToHoldingForBuy(uint256 amount) internal virtual {
        uint256 initETHBalance = address(this).balance;
        swapHelper.swapADAOToBNB(amount, address(this));
        uint256 swapETHBalance = address(this).balance.sub(initETHBalance);

        address(uint160(holdingRewardAddress)).transfer(swapETHBalance);

        globalHoldingBNBAmount = globalHoldingBNBAmount.add(swapETHBalance);
        globalHoldingTokenAmount = globalHoldingTokenAmount.add(amount);
        emit BuyDividendHoldingDisptached(holdingRewardAddress, amount);
    }
    ...
    function _dividendToLiquidity(uint256 amount) internal virtual {
        ...
        uint256 initETHBalance = address(this).balance;
        swapHelper.swapADAOToBNB(swapAmount, address(this));

        uint256 swapETHBalance = address(this).balance.sub(initETHBalance);
        swapHelper.addADAOLiquidity{value: swapETHBalance}(liquidityAmount);
    }
    ...
    function addLiquidityExternalManual(uint256 amount)
        ...
        onlyOwner
    {
        swapHelper.addADAOLiquidity{value: msg.value}(amount);
    }

    function addLiquidityInternalManual() external onlyOwner {
        swapHelper.addADAOLiquidity{value: address(this).balance}(
            ADAOToken.balanceOf(address(this))
        );
    }
