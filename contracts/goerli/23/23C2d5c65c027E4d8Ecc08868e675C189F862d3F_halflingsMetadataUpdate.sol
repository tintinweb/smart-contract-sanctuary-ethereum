...
contract halflingsMetadataUpdate is Ownable {
      ...
      event _buyNft(address user,uint256 nftid,uint256 halflingId,uint256 amount);
    ...
    {
        require(!_isPause, "currently contract is paus");
        bytes32 hash = keccak256(abi.encodePacked(nftid,time,msg.value,seed));
        bytes32 message = ECDSA.toEthSignedMessageHash(hash);
        address receivedAddress = ECDSA.recover(message, signature);
        require(receivedAddress == owner(), "Invalid signature");
        require(!_isBuy[halflingId][nftid], "already buy");
        require(time>block.timestamp, "Time over");
        _isBuy[halflingId][nftid]=true;
         payable(owner()).transfer(msg.value);
        ERC1155(halflingsItems).mint(_msgSender(),nftid,1);
        emit _buyNft(_msgSender(),nftid,halflingId,msg.value);
    }
