...
contract SNKFactory is SNKFactoryBase {
    constructor(address snackTokenAddress, uint64 duration)
        SNKFactoryBase(snackTokenAddress, duration)
    {}
    ...
    function deployVestingWallet(
        ...
        newVestingWallet.transferOwnership(beneficiary);
...
abstract contract SNKFactoryBase is Ownable, Pausable {
    ...
    constructor(address snackTokenAddress, uint64 durationSeconds) {
    ...
    function addBeneficiary(
        ...
        _addBeneficiary(Beneficiary(addr, amount, start));
    }
    ...
    function _addBeneficiary(Beneficiary memory beneficiary) private {
        require(
        ...
        _beneficiaries.push(beneficiary.addr);
    ...
    function createVestingWallet()
        ...
        returns (address)
    {
        address newVestingWallet = deployVestingWallet(
            msg.sender,
            address(snackToken),
            vestingWalletRecords[msg.sender].start
        );

        vestingWalletRecords[msg.sender].addr = newVestingWallet;
