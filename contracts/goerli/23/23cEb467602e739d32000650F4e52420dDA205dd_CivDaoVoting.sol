...
contract CivDaoVoting is Ownable, ReentrancyGuard {
    IERC20 public token;
    ...
    ICivReflection public reflectionToken;
    ...
    constructor(IERC20 _token, address _reflection) {
        token = _token;
        reflectionToken = ICivReflection(_reflection);
    }
    ...
    function vote(uint8 _answer) public {
        ...
        require(election.voters[msg.sender]==0);
        ...
        if( _answer == 1 ) {
            election.yesPower += token.balanceOf(msg.sender);
        } else {
            election.noPower += token.balanceOf(msg.sender);
        }
        election.voters[msg.sender] = _answer;
    ...
    function finishCurrentVote() public nonReentrant returns(uint8){
        ...
        uint8 winnerId;
        if( election.yesPower > election.noPower) winnerId = 1;
        else winnerId = 2;
