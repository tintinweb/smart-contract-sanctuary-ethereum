    ...
    PausableUpgradeable
{
    ...
    function getActivePlayersBy(
        ...
        uint256 size = game.playersInGame;
        players = new Player[](size);
        for (uint i = 0; i < size; i++) {
            NumberChosen memory numberChosen = playersOf[gameId][i];
            Userable.UserDescription memory userDescriptor;
            if (
                numberChosen.tokenID != tokenIDBotA &&
                numberChosen.tokenID != tokenIDBotB
            ) {
                userManager.getUserDescription(numberChosen.tokenID);
            } else {
        ...
        )
    {
        ...
        if (gameID > 0) {
            ...
            players = new Player[](game.playersInGame);
            for (uint i = 0; i < game.playersInGame; i++) {
                NumberChosen memory nbchosen = playersOf[gameID][i];
                Userable.UserDescription memory userDescriptor;
                if (
                    nbchosen.tokenID != tokenIDBotA &&
                    nbchosen.tokenID != tokenIDBotB
                ) {
                    userDescriptor = userManager.getUserDescription(
                        nbchosen.tokenID
                    );
                } else {
                    userDescriptor.name = "The Reapers";
