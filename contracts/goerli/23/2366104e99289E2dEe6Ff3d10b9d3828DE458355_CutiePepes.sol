...
contract CutiePepes is ERC721A, ReentrancyGuard, Ownable {
    ...
    address internal constant openseaConduit =
    ...
    function _mintTokens(
        ...
        if (!isFree) {
            require(
                _value >=
                    totalMintPrice(_quantity + _numberMinted(_to)) -
                        totalMintPrice(_numberMinted(_to)),
                "Insufficient funds"
            );
        }
    ...
    function totalMintPrice(uint256 _quantity) public view returns (uint256) {
        if (_quantity == 0) {
            return 0;
        }
    ...
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        if (isOpenSeaProxyActive && operator == openseaConduit) {
            return true;
        }
