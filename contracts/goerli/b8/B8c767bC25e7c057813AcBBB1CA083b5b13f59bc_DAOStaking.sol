...
contract DAOStaking is IModule, IDAOV1Staking, Initializable, AccessControlUpgradeable {
    ...
    function initialize(DAOStakingConfig memory cfg, address roleAdmin) public initializer {
        __DAOStaking_init(cfg, roleAdmin);
    }

    function __DAOStaking_init(DAOStakingConfig memory cfg, address roleAdmin) internal onlyInitializing {
        __AccessControl_init();

        __DAOStaking_init_unchained(cfg, roleAdmin);
    }

    function __DAOStaking_init_unchained(DAOStakingConfig memory cfg, address roleAdmin) internal onlyInitializing {
        require(cfg.govToken != address(0), "DAOStaking: govToken address must not be 0x0");

        govToken = IERC20Upgradeable(cfg.govToken);
    ...
    function enforcePrerequisites(address registryAddr, bytes32 orgId) external view {
        ...
        require(!registry.hasModule(orgId, Modules.DAO_V1_STAKING), "Prerequisites: module already exists");

        require(registry.hasModule(orgId, Modules.TOKEN_V1), "Prerequisites: module TOKEN_V1 required");
        require(
            address(govToken) == registry.getModuleValue(orgId, Modules.TOKEN_V1, 0),
            "Prerequisites: govToken address must match registry"
        );
    }
    ...
    function enforceSafeRemove(address registryAddr, bytes32 orgId) external view {
        ...
        require(!registry.hasModule(orgId, Modules.DAO_V1_GOVERNANCE), "SafeRemove: required by DAO_V1_GOVERNANCE");
        require(!registry.hasModule(orgId, Modules.DAO_V1_REWARDS), "SafeRemove: required by DAO_V1_REWARDS");
    }

    // deposit allows a user to add more governance token to his staked balance
    function deposit(uint256 amount) public {
        require(amount > 0, "DAOStaking: amount must be greater than 0");

        uint256 allowance = govToken.allowance(msg.sender, address(this));
        require(allowance >= amount, "DAOStaking: token allowance too small");

        _beforeBalanceChange(msg.sender);

        uint256 newBalance = balanceOf(msg.sender) + amount;
        _updateUserBalance(userStakeHistory[msg.sender], newBalance);
        _updateLockedGovToken(govTokenStakedAtTs(block.timestamp) + amount);

        address delegatedTo = userDelegatedTo(msg.sender);
        if (delegatedTo != address(0)) {
            uint256 newDelegatedPower = delegatedPower(delegatedTo) + amount;
            _updateDelegatedPower(delegatedPowerHistory[delegatedTo], newDelegatedPower);
    ...
    function withdraw(uint256 amount) public {
        require(amount > 0, "DAOStaking: amount must be greater than 0");
        require(userLockedUntil(msg.sender) <= block.timestamp, "DAOStaking: user balance is locked");

        uint256 balance = balanceOf(msg.sender);
        require(balance >= amount, "DAOStaking: insufficient balance");

        _beforeBalanceChange(msg.sender);

        _updateUserBalance(userStakeHistory[msg.sender], balance - amount);
        _updateLockedGovToken(govTokenStakedAtTs(block.timestamp) - amount);
        ...
        emit Withdraw(msg.sender, amount, balance - amount);
    }

    // lock a user's currently staked balance until timestamp & add the bonus to his voting power
    function lock(uint256 timestamp) public {
        require(timestamp > block.timestamp, "DAOStaking: timestamp must be in the future");
        require(timestamp <= block.timestamp + MAX_LOCK, "DAOStaking: timestamp too big");
        require(balanceOf(msg.sender) > 0, "DAOStaking: sender has no balance");

        Stake[] storage checkpoints = userStakeHistory[msg.sender];
        Stake storage currentStake = checkpoints[checkpoints.length - 1];

        require(timestamp > currentStake.expiryTimestamp, "DAOStaking: new timestamp lower than current lock timestamp");
    ...
    function delegate(address to) public {
        require(msg.sender != to, "DAOStaking: can't delegate to self");

        uint256 senderBalance = balanceOf(msg.sender);
        require(senderBalance > 0, "DAOStaking: no balance to delegate");

        emit Delegate(msg.sender, to);

        address delegatedTo = userDelegatedTo(msg.sender);
        if (delegatedTo != address(0)) {
            uint256 newDelegatedPower = delegatedPower(delegatedTo) - senderBalance;
            _updateDelegatedPower(delegatedPowerHistory[delegatedTo], newDelegatedPower);
        ...
        if (to != address(0)) {
            uint256 newDelegatedPower = delegatedPower(to) + senderBalance;
            _updateDelegatedPower(delegatedPowerHistory[to], newDelegatedPower);
    ...
    function enableNewRewardsContract(address _rewards) public {
        require(hasRole(MANAGER_ROLE, msg.sender), "DAOStaking: only owner can change rewards address");
        require(_rewards != address(0), "DAOStaking: rewards address must not be 0x0");

        for (uint256 i = 0; i < rewards.length; i++) {
            require(address(rewards[i]) != _rewards, "DAOStaking: rewards contract already enabled");
        }

        rewards.push(IDAOV1Rewards(_rewards));
    }
    ...
    function stakeAtTs(address user, uint256 timestamp) public view returns (Stake memory) {
        Stake[] storage stakeHistory = userStakeHistory[user];
    ...
    function votingPowerAtTs(address user, uint256 timestamp) public view returns (uint256) {
        ...
        if (stake.delegatedTo != address(0)) {
            ...
            ownVotingPower = balance * multiplier / BASE_MULTIPLIER;
        }

        uint256 delegatedVotingPower = delegatedPowerAtTs(user, timestamp);

        return ownVotingPower + delegatedVotingPower;
    }

    // govTokenStaked returns the total raw amount of governance token staked at the current block
    function govTokenStaked() public view returns (uint256) {
        return govTokenStakedAtTs(block.timestamp);
    }

    // govTokenStakedAtTs returns the total raw amount of governance token users have deposited into the contract
    // it does not include any bonus
    function govTokenStakedAtTs(uint256 timestamp) public view returns (uint256) {
        return _checkpointsBinarySearch(govTokenStakedHistory, timestamp);
    }
    ...
    function delegatedPowerAtTs(address user, uint256 timestamp) public view returns (uint256) {
        return _checkpointsBinarySearch(delegatedPowerHistory[user], timestamp);
    }
    ...
    function numRewardContracts() public view returns (uint256) {
    ...
    function _checkpointsBinarySearch(Checkpoint[] storage checkpoints, uint256 timestamp) internal view returns (uint256) {
    ...
    function _stakeMultiplier(Stake memory stake, uint256 timestamp) internal pure returns (uint256) {
        ...
        return BASE_MULTIPLIER + diff * BASE_MULTIPLIER / MAX_LOCK;
    }
    ...
    // _updateLockedGovToken stores the new `amount` into the governance token locked history
    function _updateLockedGovToken(uint256 amount) internal {
        if (govTokenStakedHistory.length == 0 || govTokenStakedHistory[govTokenStakedHistory.length - 1].timestamp < block.timestamp) {
            govTokenStakedHistory.push(Checkpoint(block.timestamp, amount));
        } else {
            Checkpoint storage old = govTokenStakedHistory[govTokenStakedHistory.length - 1];
            old.amount = amount;
        }
    }

    function _beforeBalanceChange(address user) internal {
        for (uint256 i = 0; i < rewards.length; i++) {
            rewards[i].registerUserAction(user);
        }
    }
...
interface IDAOV1Rewards {
    ...
    function pullFeature() external view returns (address source, uint256 startTs, uint256 endTs, uint256 totalDuration, uint256 totalAmount);
}
