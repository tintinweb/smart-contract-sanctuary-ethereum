...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer, ReentrancyGuard, Referable {
    ...
    function setSaleRoot(bytes32 _root) external onlyOwner {
        merkleRoots[MerkleRoot.Sale] = _root;
    }

    function seClaimRoot(bytes32 _root) external onlyOwner {
        merkleRoots[MerkleRoot.Claim] = _root;
    }
    ...
    function isEligible(
        ...
        if(token.claimIsActive) {
                _isEligibleClaim = _hasSupply && (_quantity <= (_maxMint - _hasClaimed)) && _maxMint <= 50 &&
                    MerkleProof.verify(_proof, merkleRoots[MerkleRoot.Claim], keccak256(abi.encode(_address, _maxMint)));
        }
        if(token.preSaleIsActive) {
            ...
            if (token.preSalePhase == 2) {
                _isEligiblePreSale = _hasSupply && (_quantity <= (_maxMint - _hasMinted)) && _maxMint <= 50 &&
                    MerkleProof.verify(_proof, merkleRoots[MerkleRoot.Sale], keccak256(abi.encode(_address, _maxMint)));
            }
        }
        if (token.pubSaleIsActive) {
            if (token.pubMintType == PublicMintType.PerWallet) {
                _isEligiblePubSale = _hasSupply && _quantity <= token.pubMaxMint && (_quantity <= (token.pubMaxMint - _hasMinted));
            } else {
