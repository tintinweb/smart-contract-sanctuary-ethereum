...
import {DateTimeLibrary as TimeLib} from "./DateTimeLibrary.sol";
...
interface ISubTokenDescriptor {
    function tokenURI(address merchantToken, address planManager, address subInfoManager, uint256 subTokenId, address subTokenOwner) external view returns (string memory);
}
...
contract SubTokenManager is ISubTokenManager, Initializable ,ERC721Upgradeable, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        __ERC721_init("S10N Subscription Token", "SST");
        __Ownable_init();
        nextTokenId = 1;
    }
    ...
    function setMerchantTokenManager(address _merchantToken) external onlyOwner override {
    ...
    function tokenURI(uint tokenId) public view override returns (string memory uri) {
        ...
        uri = ISubTokenDescriptor(subTokenDescriptor).tokenURI(merchantTokenManager, planManager, subInfoManager, tokenId, owner);
    }
