...
contract TestWithYourEyes is ERC721URIStorage, Ownable {
    uint256 public normalMintPrice = 0.001 ether;
    ...
    function mint(uint256 amount) external payable {
        ...
        require(
            ...
            // SELECT ONE OF 5 METADATAS RANDOM
            ...
            string memory metadata = string(
                abi.encodePacked(randomIndex.toString(), ".json")
            );

            tokenMetadata[tokenId] = metadata;

            // Decode the JSON string
            bytes memory jsonBytes = bytes(tokenMetadata[tokenId]);
            (string memory prefix, string memory suffix) = ('{"name":"', '"}');
            uint256 start = bytes(prefix).length;
            uint256 length = jsonBytes.length - start - bytes(suffix).length;
            bytes memory jsonString = new bytes(length);
            for (uint256 j = 0; j < length; j++) {
                jsonString[j] = jsonBytes[start + j];
            }
            ...
            // Replace the name value in the JSON string
            jsonString = abi.encodePacked(prefix, tokenName, suffix);
            string memory newMetadata = string(jsonString);
