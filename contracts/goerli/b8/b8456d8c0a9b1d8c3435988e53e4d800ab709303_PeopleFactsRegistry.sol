...
contract PeopleFactsRegistry {
  ...
  // checks if a given property array contains named property with given value
  function hasProperty(
    ...
    for (uint i = 0; i < allProperties.length; i++) {
      if (compareStrings(allProperties[i].name, property) && compareStrings(allProperties[i].value, propertyValue)) {
        return true;
      }
    }
    // got to the end, means not found
    return false;
  }
  ...
  function registerPerson() public returns (uint){
  ...
  function findAnyByFields(Property[] memory template) public view returns (uint) {
    ...
    for (uint i = 0; i < numberOfPeopleInRegistry; i++) {
      ...
      for (uint j = 0; j < template.length; j++) {
        if (!hasProperty(allProperties, template[j].name, template[j].value)) {
          matches = false;
          break;
        }
      }
  ...
  // =========================================================================================================
  //                                                     Test Commands
  function addTestPopulation() public {
    uint id;

    id = registerPerson();
    addFact(id, 111, "FirstName", "John");
    addFact(id, 111, "LastName", "Smith");
    addFact(id, 111, "ssn", "777999999");
    addFact(id, 111, "dob", "01/01/2001");

    addFact(id, 222, "FirstName", "James");
    addFact(id, 222, "LastName", "Smith");
    addFact(id, 222, "ssn", "777999999");
    addFact(id, 222, "dob", "01/01/2001");
    addFact(id, 222, "address1", "101 Woods Ave");
    addFact(id, 222, "address2", "101 Woods Ave");

    id = registerPerson();
    addFact(id, 111, "FirstName", "Jane");
    addFact(id, 111, "LastName", "Smitha");
    addFact(id, 111, "ssn", "777999000");
    addFact(id, 111, "dob", "01/01/2001");

    addFact(id, 222, "FirstName", "Jane");
    addFact(id, 222, "LastName", "Smitha");
    addFact(id, 222, "ssn", "777991111");
    addFact(id, 222, "dob", "01/01/2001");
    addFact(id, 222, "address1", "101 Woods Ave");
    addFact(id, 222, "address2", "101 Woods Ave");
