...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer, ReentrancyGuard, Referable {
    ...
    function setApprovalForAll(address operator, bool approved) public override onlyAllowedOperatorApproval(operator) {
        require(!token.soulbound, "Soulbound");
    ...
    function seClaimRoot(bytes32 _root) external onlyOwner {
    ...
    function isEligible(
        ...
        if(!_isEligibleClaim && token.preSaleIsActive) {
            if (token.preSalePhase == 1) {
                _isEligiblePreSale = _hasSupply && mintpass.balanceOf(_address, 1) >= _quantity;
            }
            if (token.preSalePhase == 2 && (_quantity <= (_maxMint - hasMinted[_address])) && _value <= token.preSalePrice) {
    ...
    function _mintTo(
        ...
        (bool _isEligibleClaim, bool _isEligiblePreSale, bool _isEligiblePubSale) = isEligible(_address, uint16(_quantity), _maxMint, _proof, _value);
