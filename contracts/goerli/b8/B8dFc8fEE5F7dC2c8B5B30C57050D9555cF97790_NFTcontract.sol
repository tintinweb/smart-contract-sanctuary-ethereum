...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) private numOfTokens;
    uint256 private lengthOfBaseURIs = 0;
    ...
    function updateBaseURI(uint256[] calldata _numOfTokens, string[] calldata _baseURIs) external {
        ...
        for (uint256 i =0; i < _numOfTokens.length; i++) {
            lengthOfBaseURIs++;
            numOfTokens[i] = _numOfTokens[i];
            baseURIs[i] = _baseURIs[i];
        }
    }
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < lengthOfBaseURIs; i++) {
            if (tokenId <= numOfTokens[i]) {
                return  string(
                        abi.encodePacked(baseURIs[i], Strings.toString(tokenId), ".json")
                    );
            }
        }
        return "";
