...
contract RebasePair_Allowance is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function setPair(
        ...
        _pair = pair;
        _oracleNT = oracleNT;
        if (oracle0 != address(0)) {
            uint8 oracle0Decimal = IDataFeed(oracle0).decimals();
            require((oracle0Decimal == 8 || oracle0Decimal == 18), "Invalid oracle0 decimals");
            _oracle0Decimal = oracle0Decimal;
        } else {
            _oracle0Decimal = 0; // default value for USD
        }
        if (oracle1 != address(0)) {
            uint8 oracle1Decimal = IDataFeed(oracle1).decimals();
            require((oracle1Decimal == 8 || oracle1Decimal == 18), "Invalid oracle1 decimals");
            _oracle1Decimal = oracle1Decimal;
        } else {
            _oracle1Decimal = 0; // default value for USD
        }
        if (oracleNT != address(0)) {
            uint8 oracleNTDecimal = IDataFeed(oracleNT).decimals();
        ...
        )
    {
        ...
        if (dif0 > dif1 && profitUSD > rebaseFeeUSD) {
            ...
            amountInMax = (dif0 * 10**_oracle1Decimal) / fx1; // _getAmountInMax(amountOut, reserve0, reserve1);
            path[0] = _token1;
            path[1] = _token0;
        ...
        if (dif1 > dif0 && profitUSD > rebaseFeeUSD) {
            ...
            amountInMax = (dif1 * 10**_oracle0Decimal) / fx0; // _getAmountInMax(amountOut, reserve1, reserve0);
            path[0] = _token0;
            path[1] = _token1;
            netProfitUSD = profitUSD - rebaseFeeUSD;
        }
