...
contract KeepersOptimized is ERC721A, ASingleAllowlistMerkle, AMultiFounderslistMerkle {
    ...
    uint256 public listPrice = 0.125 ether;
    ...
    constructor(address _owner, bytes32 _allowlistMerkleRoot, bytes32 _foundersListMerkleRoot)
        ERC721A("Keepers", "KPR")
    ...
    /// @dev free
    function mintFounderslist(bytes32[] calldata _merkleProof, uint16 _entitlementAmount, uint256 _amount) external onlyFounderslisted(_merkleProof, _entitlementAmount) whenNotPaused {
        ...
        uint256 newSupply = mintedFounderslistSupply + _amount;
        if (newSupply > MAX_FOUNDERSLIST_SUPPLY) revert ExceedingFoundersSupply();

        // TODO - Multiplying the quantity by 1 adds 82 gas per call, leaving it for now to help keep the bit order straight

        mintedFounderslistSupply += _amount;
        _setAux(msg.sender, _getAux(msg.sender) + uint64(_amount * 4));

        // TODO - this was previously using a safe mint because the owner might be a multi-sig?
        _safeMint(msg.sender, _amount, "");
    }

    /// @notice Safely mints NFTs from allowlist.
    /// @dev pays the lowest auction price
    function mintAllowlist(bytes32[] calldata _merkleProof, uint256 _amount) external payable onlyAllowlisted(_merkleProof) whenNotPaused {
        if (msg.value < listPrice) revert InsufficientETH();
        ...
        uint256 newSupply = mintedAllowlistSupply + _amount;
        if (newSupply > MAX_ALLOWLIST_SUPPLY) revert ExceedingAllowlistSupply();

        // TODO - Multiplying the quantity by 1 adds 82 gas per call, leaving it for now to help keep the bit order straight

        mintedAllowlistSupply += _amount;
        _setAux(msg.sender, _getAux(msg.sender) + uint64(_amount * 1));

        refundOverpay(listPrice);
     ...
     */
    function mintPublic(uint256 _amount, bytes32 _nonce, bytes32 _hash, uint8 v, bytes32 r, bytes32 s) external payable whenNotPaused {
        ...
        uint256 newSupply = mintedPublicSupply + _amount;
        if (newSupply > MAX_PUBLIC_SUPPLY) revert ExceedingMaxPublicSupply();

        // TODO - Multiplying the quantity by 262144 uses less gas than the setBits option(s)

        mintedPublicSupply += _amount;
        _setAux(msg.sender, _getAux(msg.sender) + uint64(_amount * 262144));

        refundOverpay(publicPrice);

        _mint(msg.sender, _amount);

        // Not emiting this saves 2045 gas
        emit Buy(msg.sender, publicPrice, _amount);
    }
    ...
    function mintToUser(uint256 _quantity, address _receiver) external payable onlyOwner {
        if ((totalSupply() + _quantity) > MAX_SUPPLY) revert ExceedingMaxSupply();
