...
contract WeLoveMP is ERC1155, Ownable {
    ...
    constructor(address[] memory initialAuthorizedWallets, uint256[] memory initialTokenIds, uint256[] memory initialTokenAmounts, bytes memory initialData)
        ERC1155("ipfs://QmYkyLzJnc1yw6d18KWEKKLZv4zXLBv8rTvBW3SDLgu9Y1/1.json")
    {
    ...
    for (uint i = 1; i < initialAuthorizedWallets.length; i++) {
        authorizedReceivers[initialTokenIds[i]][initialAuthorizedWallets[i]] = true;
    ...
    function mintBatch(address[] memory recipients, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public onlyOwner {
        require(recipients.length == ids.length && recipients.length == amounts.length, "Arrays length mismatch");
        for (uint i = 0; i < recipients.length; i++) {
            _mint(recipients[i], ids[i], amounts[i], data);
        }
    }

    function authorizeReceivers(address[] memory receivers, uint256 tokenId) public onlyOwner {
        for (uint i = 0; i < receivers.length; i++) {
            authorizedReceivers[tokenId][receivers[i]] = true;
        }
    }

    function revokeReceivers(address[] memory receivers, uint256 tokenId) public onlyOwner {
        for (uint i = 0; i < receivers.length; i++) {
            authorizedReceivers[tokenId][receivers[i]] = false;
        }
    }


    function isAuthorized(address receiver, uint256 tokenId) public view returns (bool) {
        return authorizedReceivers[tokenId][receiver];
    }

    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _amount, bytes memory _data) public override {
        require(authorizedReceivers[_id][_to], "Receiver not authorized");
    ...
    function safeBatchTransferFrom(address _from, address _to, uint256[] memory _ids, uint256[] memory _amounts, bytes memory _data) public override {
        for (uint i = 0; i < _ids.length; i++) {
            require(authorizedReceivers[_ids[i]][_to], "Receiver not authorized");
        }
    ...
    function _beforeTokenTransfer(address operator, address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal override {
        for (uint i = 0; i < ids.length; i++) {
            if (to != address(0)) {
                require(authorizedReceivers[ids[i]][to], "Receiver not authorized");
            }
        }
