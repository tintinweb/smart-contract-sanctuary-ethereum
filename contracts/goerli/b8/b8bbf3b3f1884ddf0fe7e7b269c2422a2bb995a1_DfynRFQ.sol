...
contract DfynRFQ is Ownable {
    ...
    */
    function swapTokenToNative(
        ...
        IERC20(tokens[1]).safeTransferFrom(custodian, feeAddress, feeAmount);
        IERC20(tokens[1]).safeTransferFrom(custodian, address(this), amounts[1].sub(feeAmount));
        IWETH(WETH).withdraw(amounts[1].sub(feeAmount));
        payable(msg.sender).transfer(amounts[1].sub(feeAmount));
