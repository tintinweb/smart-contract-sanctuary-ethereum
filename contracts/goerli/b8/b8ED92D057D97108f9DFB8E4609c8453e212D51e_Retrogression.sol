...
contract Retrogression is ERC721A, ERC2981, Ownable {
  ...
  uint256 public ogMaxMint; // max mint each address can mint
  ...
  bool public isOgSale; // a boolean to handle stages of og sale
  ...
  constructor() ERC721A('Retrogression', 'RTGN') {
    mintRate = 98000000000000000; // 0.098eth
    whitelistMintRate = 98000000000000000; // 0.098eth
    ogMaxMint = 2;
    whiteListMaxMint = 2;
    totalMaxSupply = 5050;
    ogMaxSupply = 566;
    whiteListMaxSupply = 100;
    isOgSale = false;
    isWhitelistSale = false;
    isPublicSale = false;

    // @dev setting the royalty fee for retrogression address.
    _setDefaultRoyalty(0xb8623497431893Fc4820eC708003f27DE086FEF1, 1500);
  }
  ...
  function ogMint(uint256 _quantity, bytes32[] calldata _proof)
    ...
    payable
    isOgLive
    isAddressVerified(_proof, ogMerkleRoot)
    isWithinMintLimit(_quantity, ogMaxMint)
    isEnoughTokensLeft(_quantity)
  {
    if (totalOgSupply.add(_quantity) > ogMaxSupply) revert ExceededLimit();

    if (ogUsedAddresses[msg.sender].add(_quantity) > ogMaxMint) {
      revert OgListUsed();
    }

    ogUsedAddresses[msg.sender] = ogUsedAddresses[msg.sender].add(_quantity);
