...
contract OcarioFeature is IFeature, IOcarioFeature, FixinCommon,RouterTokenHelper {
    ...
    returns (uint256 amountOut)
  {
    ...
    while (true) {
      bool hasMultiplePools = PathHelper.hasMultiplePools(params.path);

      params.amountIn = _swapExactInputInternal(
        params.amountIn,
        hasMultiplePools ? address(this) : params.recipient, // for intermediate swaps, this contract custodies
        0,
        SwapCallbackData({path:PathHelper.getFirstPool( params.path), source: source})
      );

      if (hasMultiplePools) {
        source = address(this);
        params.path = PathHelper.skipToken(params.path);
      } else {
  ...
  function _swapExactInputInternal(
    ...
    if (recipient == address(0)) recipient = address(this);
    databytes = PathHelper.getFirstPool(data.path);
