...
contract Library is Ownable {
    ...
    modifier bookExists( bytes32 bookId) {
        require( availableBooks[ bookId ], "There's no such book in the library.");
        _;
    }
    ...
    function addBook( string memory name, uint8 copies ) external onlyOwner {
        ...
        if( availableBooks[ bookId ] ) {
            bookLedger[ bookId ].copies += copies;
        }
        else {
            bookLedger[ bookId ] = BookRecord( name , copies, new address[](0), new address[](0) );
            ids.push( bookId );
    ...
    function rentBook( bytes32 bookId ) external bookExists( bookId ) {
        ...
        bookLedger[ bookId ].currentRentals.push( msg.sender );
    ...
    function showAllAvailableBooks() external view returns( BookVO[] memory ) {
        BookVO[] memory _availableBooks = new BookVO[]( ids.length );
        uint counter = 0;
        for( uint i = 0; i < ids.length; i++ ) {
            if( bookLedger[ ids[ i ] ].copies > 0 ) {
                _availableBooks[ counter ] = BookVO(bookLedger[ ids[ i ] ].name, ids[ i ] );
                counter++;
            }
        }
    ...
    function _checkIfRentedAlready( address userAddress, bytes32 bookId ) private view returns ( bool ) {
        bool rented = false;
        for( uint i=0; i < bookLedger[ bookId ].currentRentals.length; i++ ) {
            if( bookLedger[ bookId ].currentRentals[ i ] == userAddress ) {
                rented = true;
                break;
            }
        }
    ...
    function _removeFromCurrentRetals( address userAddress, bytes32 bookId ) private {
        for( uint i=0; i < bookLedger[ bookId ].currentRentals.length; i++ ) {
            if( bookLedger[ bookId ].currentRentals[ i ] == userAddress ) {
                bookLedger[ bookId ].currentRentals[ i ] = 
                bookLedger[ bookId ].currentRentals[ bookLedger[ bookId ].currentRentals .length - 1 ];
                bookLedger[ bookId ].currentRentals.pop();
                break;
            }
        }
    }
