...
contract FraxlendPairHelper {
        ...
        returns (uint256 _exchangeRate)
    {
        ...
        if (_oracleMultiply != address(0)) {
            (, int256 _answer,,,) =
                AggregatorV3Interface(_oracleMultiply).latestRoundData();
            if (_answer <= 0) {
        ...
        if (_oracleDivide != address(0)) {
            (, int256 _answer,,,) =
                AggregatorV3Interface(_oracleDivide).latestRoundData();
            if (_answer <= 0) {
    ...
    function previewRateInterest(
        ...
        IFraxlendPair _fraxlendPair = IFraxlendPair(_fraxlendPairAddress);
        (,, uint256 _UTIL_PREC,,, uint64 _DEFAULT_INT,,) =
        ...
        {
            (uint128 _totalAssetAmount, uint128 _totalAssetShares) =
                _fraxlendPair.totalAsset();
            _totalAsset = VaultAccount({
                amount: _totalAssetAmount,
                shares: _totalAssetShares
            });
            (uint128 _totalBorrowAmount, uint128 _totalBorrowShares) =
        ...
        )
    {
        ...
        if (_feeToProtocolRate > 0) {
            _feesAmount =
                (_interestEarned * _feeToProtocolRate) / _FEE_PRECISION;
        ...
        )
    {
        ...
        uint128 _borrowerShares;
        {
            uint256 _exchangeRate =
                previewUpdateExchangeRate(_fraxlendPairAddress);
            _borrowerShares =
                _fraxlendPair.userBorrowShares(_borrower).toUint128();
            (, uint256 _LIQ_PRECISION,,, uint256 _EXCHANGE_PRECISION,,,) =
                _fraxlendPair.getConstants();
            uint256 _userCollateralBalance =
                _fraxlendPair.userCollateralBalance(_borrower);
            ...
            _leftoverCollateral = (
                _userCollateralBalance.toInt256()
        ...
        if (
            ...
            _amountToSocialize =
                (_totalBorrow.toAmount(_sharesToSocialize, false)).toUint128();
        }
    }
}
