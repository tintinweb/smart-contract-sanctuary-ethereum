 ...
 */
  function mint(address to_, uint256 countNFTs_) isValid() public returns(uint256, uint256) {
    ...
    for (uint256 index = start; index < start + countNFTs_; index++) {
       _safeMint(to_, tokenIds);
       tokenIds++;
    }
    
    uint256 end = tokenIds - 1;
  ...
  function preOrderCryptoNFT(bytes32[] calldata proof, uint256 limit)  payable external {
    ...
    reservedNFT[whiteListType] += limit;
    userBought[msg.sender][whiteListType] += limit;

    emit ReserverNFT(mainPrice, price, seller, msg.sender, limit);

  }
  function claimCryptoNFT() external {
    require(msg.sender == tx.origin, "101");
    uint256 limit=  userBought[msg.sender][2].sub(userBought[msg.sender][3]);
   ...
   uint256 from = tokenIds;
    ...
    userBought[msg.sender][3] += limit;
     emit ClaimNFT( from,  to,   msg.sender);
  }

   function mintPenpenzNFT(bytes32[] calldata proof, uint256 limit, uint256 whiteListType) external {
    require(msg.sender == tx.origin, "101");
    require(_saleDetails[whiteListType].startTime <= block.timestamp, "sale not started yet");
    require(_saleDetails[whiteListType].endTime > block.timestamp, "sale has ended");
  ...
  function unLockGas(uint256[] memory tokenIds) payable external {
    uint256 depositAmount = msg.value;
    (bool success,) = payable(gasCollector).call{value: depositAmount}("");
    if(!success) revert("unable to receive eth");
    emit LockGasPaid(tokenIds,msg.sender,depositAmount);
  }
 ...
 /**
  ...
  event LockGasPaid(uint256[] tokenIds, address from, uint256 price);
