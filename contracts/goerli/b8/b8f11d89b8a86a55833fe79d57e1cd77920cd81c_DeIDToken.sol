...
abstract contract DeIDOffering is ERC777, AccessControl, ReentrancyGuard  {
    ...
    function investorPurchase(address investorWallet) public payable onlyRole(INVESTORS_ROLE) {
    ...
    function investorSell(address investorWallet, uint256 tokensToSell) public payable onlyRole(INVESTORS_ROLE) {
    ...
    function contributorPurchase(address contributorWallet) public payable onlyRole(CONTRIBUTORS_ROLE) {
        require(currentState != State.pauseAll || currentState != State.pauseBuy);
        uint256 amountWei = msg.value;
        require(amountWei > 0, "No ETH Sent");
    ...
    function contributorSell(address contributorWallet, uint256 tokensToSell) public payable onlyRole(CONTRIBUTORS_ROLE) {
        require(currentState != State.pauseAll || currentState != State.pauseSell);
    ...
    function incubationPurchase(address incubatorWallet) public payable onlyRole(INCUBATION_ROLE) {
        require(currentState != State.pauseAll || currentState != State.pauseBuy);
        uint256 amountWei = msg.value;
        require(amountWei > 0, "No ETH Sent");
    ...
    function incubationSell(address incubatorWallet, uint256 tokensToSell) public payable onlyRole(INCUBATION_ROLE) {
        require(currentState != State.pauseAll || currentState != State.pauseSell);
        require(tokensToSell > 0, "No tokens sent for sale, You need to sell at least 1 token");
    ...
    function customPurchase(address walletFrom, address walletTo) public payable onlyRole(CUSTOMSALE_ROLE) {
        require(currentState != State.pauseAll || currentState != State.pauseBuy);
        uint256 amountWei = msg.value;
        require(amountWei > 0, "No ETH Sent");
    ...
    function transferGeneralTokens(uint256 amountWei) internal {
    ...
    function timeLockContract(address tokenOwner, uint256 tokens, address beneficiary_, uint64 releaseDuration) public nonReentrant onlyRole(FINANCE_ROLE) {
        ...
        TokenTimelock newToken = new TokenTimelock(thisToken,beneficiary_,releaseTime_);
        this.transferFrom(address(tokenOwner),address(newToken),tokens);   
    }
