...
// import "@manifoldxyz/creator-core-extensions-solidity/contracts/manifold/lazyclaim/ERC1155LazyPayableClaim.sol";
// import "https://github.com/manifoldxyz/creator-core-extensions-solidity/blob/main/contracts/manifold/lazyclaim/ERC1155LazyPayableClaim.sol";
// import "@manifoldxyz/creator-core-extensions-solidity/contracts/manifold/lazyclaim/ERC1155LazyPayableClaim.sol";

import "https://github.com/manifoldxyz/creator-core-solidity/blob/main/contracts/core/IERC1155CreatorCore.sol";

contract RealMinter  {
    struct Season{
      uint256 numberOfItem;
      uint256 mintLimit;
      uint256 packedProbablity;
      uint256 normalFloor;
      uint256 stratTime;
      uint256 endTime;
      uint256 price;
    }
    Season private _season;
    // event NewSeas
    ...
    function setCreatort(address cr) public {
      require(_creator == address(0),"this function can be called only once");
      _creator = cr;
    }
    function mintQ(uint256 _quantity) public {
        uint256[] memory _amounts = new uint256[](_season.numberOfItem);
        address[] memory _recipients = new address[](1);
        uint256[] memory _tokenIds = new uint256[](_season.numberOfItem);
        uint256 _rand = random();
        uint256 _randId;
        _recipients[0] = msg.sender;

        for(uint256 i; i<_season.numberOfItem;i++){
          
          
          _tokenIds[i] = lastTokenId - _season.numberOfItem + i;

        }

        for(uint256 i; i<_quantity;i++){
          _randId = (_rand >> (16*i)) % _season.normalFloor;
          _amounts[calculateId(_randId)] += 1;
          

        }

      IERC1155CreatorCore(_creator).mintExtensionExisting(_recipients, _tokenIds, _amounts);
    }
    function calculateId(uint256 _randId) public view returns(uint256){
      uint256 treshhold;
      for(uint256 i;i<_season.numberOfItem;i++){
        treshhold += (_season.packedProbablity >> (i*8)) & 255;
        if(_randId < treshhold){
          return i;
        }
      }
      revert();
    ...
    function random() public view returns(uint256){
      return uint256(keccak256(abi.encodePacked(block.number,"asd")));
    }
    function getSeason() public view returns(Season memory){
      return _season;
    }
  
    // adminrequierd
    function createSeason(uint256 _numberOfItems, uint256 _mintLimit,uint256[] memory _prob,uint256 _startTime,uint256 _endTime,uint256 _price) external{
      require(_prob.length == _numberOfItems,"lenght of probablity and Items don't match");
      require(_mintLimit != 0,"mint limit cannot be zero");
      require(_endTime > _startTime,"end should be greater than start");
      uint256 _packedProb;
      uint256 _normalFloor; 
      for(uint i=0; i< _prob.length ;i++){
        _packedProb += _prob[i] * 256**i;
        _normalFloor += _prob[i];
      }
      _season = Season({numberOfItem: _numberOfItems, mintLimit : _mintLimit ,packedProbablity: _packedProb,normalFloor : _normalFloor,stratTime:_startTime, endTime: _endTime, price: _price});
