...
import "./interfaces/INyxInvestmentProposal.sol";

contract NyxInvestmentProposal is NyxProposal, INyxInvestmentProposal {
    string public constant name = "NyxInvestmentProposal";
    uint256 public constant nparams = 9;
    ...
    function parseProposalParams(bytes[] memory params)
        public pure
        // returns (InvestmentType, string memory, address, uint256, uint256, uint256, string memory, string memory)
        returns (InvestmentProposalParams memory)
    {
        require(params.length == nparams, "wrong format");

        return InvestmentProposalParams(InvestmentType(bytesToUint(params[0])),
            bytesToString(params[1]),
            payable(bytesToAddress(params[2])),
            bytesToUint(params[3]),
            Side(bytesToUint(params[4])),
            bytesToUint(params[5]),
            bytesToUint(params[6]),
            bytesToString(params[7]),
            bytesToString(params[8]));
    }
        ...
        returns (InvestmentProposalParams memory)
    {
        ...
        {
            (uint256 param1, string memory param2, address param3,
                uint256 param4, uint256 param5, uint256 param6, uint256 param7,
                string memory param8, string memory param9) = abi.decode(params, (uint256, string, address, uint256, uint256, uint256, uint256, string, string));
            return InvestmentProposalParams(InvestmentType(param1),
                param2,
                payable(param3),
                param4,
                Side(param5),
        ...
        {
            InvestmentProposalParams memory nullProposal;
            return nullProposal;
        }
    }

    function proposalToBytes(InvestmentProposal memory proposal)
        public pure
        returns (bytes memory)
    {
        return abi.encode(uint256(proposal.assetType), proposal.tokenName, proposal.tokenAddress, proposal.tokenIdOrAmountETH, proposal.side,
            proposal.price, proposal.maxDelta, proposal.osLink, proposal.projectLink);
    }
    ...
    function createProposal(uint256 proposalId, bytes[] memory params)
        ...
        returns (bytes memory)
    {
        InvestmentProposalParams memory parsedParams = parseProposalParams(params);
        InvestmentProposal memory proposal = InvestmentProposal(proposalId, parsedParams.assetType, parsedParams.tokenName,
            parsedParams.tokenIdOrAmountETH, parsedParams.tokenAddress, parsedParams.side, parsedParams.price, parsedParams.maxDelta, parsedParams.osLink,
            parsedParams.projectLink);
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        InvestmentProposalParams memory params = parseProposalParams(proposalBytes);
        bytes[] memory prop = new bytes[](nparams);
        prop[0] = abi.encode(params.assetType);
        prop[1] = abi.encode(params.tokenName);
        prop[2] = abi.encode(params.tokenIdOrAmountETH);
        prop[3] = abi.encode(params.tokenAddress);
        prop[4] = abi.encode(params.side);
        prop[5] = abi.encode(params.price);
        prop[6] = abi.encode(params.maxDelta);
        prop[7] = abi.encode(params.osLink);
        prop[8] = abi.encode(params.projectLink);
        return prop;
    }
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            InvestmentProposalParams memory params = parseProposalParams(proposalBytesArr[idx]);
            props[idx] = [abi.encode(params.assetType), abi.encode(params.tokenName), abi.encode(params.tokenIdOrAmountETH), abi.encode(params.tokenAddress),
                abi.encode(params.side), abi.encode(params.price), abi.encode(params.maxDelta), abi.encode(params.osLink), abi.encode(params.projectLink)];
        }
    }
}
...
// @title INyxInvestmentProposal for Nyx DAO
// @author TheV
...
interface INyxInvestmentProposal {
    // Enums
    ////////////////////
    ...
    function parseProposalParams(bytes[] memory params) external pure returns (InvestmentProposalParams memory);
    function parseProposalParams(bytes memory params) external pure returns (InvestmentProposalParams memory);
    function proposalToBytes(InvestmentProposal memory proposal) external pure returns (bytes memory);
