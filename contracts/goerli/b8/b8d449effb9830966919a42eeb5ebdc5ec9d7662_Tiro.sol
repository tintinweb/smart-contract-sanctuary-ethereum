...
contract Tiro is ERC721Enumerable, Ownable {
    ...
    function mint(uint quantity, uint256[] memory mintPassTokenIds) external payable {
        ...
        uint256[] memory ownerTokenIds = walletOfOwner(msg.sender);
        uint256 additionalQuantity = quantity;
        ...
        if (mintPassTokenIds.length > 0) {
            ...
            additionalQuantity = quantity - freemintQuantity;
        }
        require(additionalQuantity >= 0, "Extra mint quantity cannot be negative");
        // if minting more than the number of mint pass owned
        if (additionalQuantity > 0) {
            if (isMintPassActive) {
                // guard off anyone who tries to mint more than the number of mint pass during the pre-WL time
                require(additionalQuantity == 0, "No ordinary mint during Mint Pass hour");
            }
            ...
            require(ownerTokenIds.length + additionalQuantity <= maxMintQuantity, "Cannot mint more than maximum quantity per wallet");
            require(msg.value >= price * additionalQuantity, "ether send is under price");
        } 
        else {
