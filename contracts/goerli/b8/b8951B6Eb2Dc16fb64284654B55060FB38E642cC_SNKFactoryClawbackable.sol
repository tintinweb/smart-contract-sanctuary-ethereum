...
import "./SNKVestingWalletClawbackable.sol";

contract SNKFactoryClawbackable is SNKFactoryBase {
    constructor(
    ...
    function _deployVestingWallet() internal override returns (address) {
        SNKVestingWalletClawbackable newVestingWallet = new SNKVestingWalletClawbackable(
                msg.sender,
                address(snackToken),
    ...
    function _changeVWBeneficiary(address newAddress) internal override {
        SNKVestingWalletClawbackable vestingWallet = SNKVestingWalletClawbackable(
...
contract SNKVestingWalletClawbackable is SNKVestingWallet {
    using SafeERC20 for IERC20;
     ...
     */
    function clawback(address treasury) external virtual onlyOwner {
        require(
            isClawbacked() == false,
        ...
        uint256 clawbackAmount = snackToken.balanceOf(address(this));

        snackToken.safeTransfer(treasury, clawbackAmount);

        _isClawbacked = true;

        emit ClawBackExecuted(treasury, clawbackAmount);
    }
}
