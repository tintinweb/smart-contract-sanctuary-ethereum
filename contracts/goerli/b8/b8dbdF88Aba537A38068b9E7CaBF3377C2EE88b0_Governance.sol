...
import "sacred-token/contracts/SACRED.sol";
...
contract Governance is Initializable, Configuration, Delegation {
  ...
  constructor() initializer {
    sacred = SACRED(0x000000000000000000000000000000000000dEaD);
    _initializeConfiguration();
  }

  function initialize(address _sacred) public initializer {
    sacred = SACRED(_sacred);
    // Create a dummy proposal so that indexes start from 1
    Proposal storage newProposal = proposals.push();
    newProposal.proposer = address(this);
    newProposal.target = 0x000000000000000000000000000000000000dEaD;
    newProposal.startTime = 0;
    newProposal.endTime = 0;
  ...
  function lock(
    ...
    sacred.permit(owner, address(this), amount, deadline, v, r, s);
  ...
  function unlock(uint256 amount) public virtual {
    ...
    require(sacred.transfer(msg.sender, amount), "SACRED: transfer failed");
  }
  ...
  function _transferTokens(address owner, uint256 amount) internal virtual {
    require(sacred.transferFrom(owner, address(this), amount), "SACRED: transferFrom failed");
...
contract Configuration {
  ...
  function _initializeConfiguration() internal {
    ...
    QUORUM_VOTES = 25000e18; // 0.25% of SACRED
    PROPOSAL_THRESHOLD = 1000e18; // 0.01% of SACRED
