...
import "./mixins/NFTMarketFixedPriceCollectionDrop.sol";
...
contract FNDNFTMarket is
  ...
  NFTMarketBuyPrice,
  NFTMarketOffer,
  NFTMarketFixedPriceCollectionDrop
{
  /**
...
error NFTMarketFixedPriceCollectionDrop_Cannot_Buy_At_Lower_Price(uint256 mintCost);
error NFTMarketFixedPriceCollectionDrop_Only_Callable_By_Collection_Owner();
error NFTMarketFixedPriceCollectionDrop_Must_Support_Collection_Mint_Interface();
error NFTMarketFixedPriceCollectionDrop_Must_Not_Be_Sold_Out();
error NFTMarketFixedPriceCollectionDrop_Limit_Per_Account_Must_Be_Set();
error NFTMarketFixedPriceCollectionDrop_Must_Not_Have_Pending_Sale();
error NFTMarketFixedPriceCollectionDrop_Must_Buy_At_Least_One_Token();
error NFTMarketFixedPriceCollectionDrop_Must_Have_Sale_In_Progress();
/// @param limitPerAccount The limit of tokens an account can purchase.
error NFTMarketFixedPriceCollectionDrop_Cannot_Buy_More_Than_Limit(uint256 limitPerAccount);

/**
 * @title Facilitates sales of fixed price batch mint collections.
 */
abstract contract NFTMarketFixedPriceCollectionDrop is NFTMarketFees {
  using AddressUpgradeable for address;
  using AddressUpgradeable for address payable;
  using OZERC165Checker for address;
  ...
  function createFixedPriceSale(
    ...
    if (!nftContract.supportsERC165Interface(type(ICollectionMint).interfaceId)) {
      revert NFTMarketFixedPriceCollectionDrop_Must_Support_Collection_Mint_Interface();
    }
    ...
    if (
      ...
      revert NFTMarketFixedPriceCollectionDrop_Only_Callable_By_Collection_Owner();
    }
    if (ICollectionMint(nftContract).isSoldOut() == true) {
      revert NFTMarketFixedPriceCollectionDrop_Must_Not_Be_Sold_Out();
    }
    if (limitPerAccount == 0) {
      revert NFTMarketFixedPriceCollectionDrop_Limit_Per_Account_Must_Be_Set();
    }

    SaleConfig storage saleConfig = nftContractToSaleConfig[nftContract];
    if (saleConfig.seller != payable(0)) {
      revert NFTMarketFixedPriceCollectionDrop_Must_Not_Have_Pending_Sale();
    }
  ...
  function _mintFromFixedPriceSaleAndApproveMarket(
    ...
    if (count == 0) {
      revert NFTMarketFixedPriceCollectionDrop_Must_Buy_At_Least_One_Token();
    }

    SaleConfig storage saleConfig = nftContractToSaleConfig[nftContract];
    if (saleConfig.seller == payable(0)) {
      revert NFTMarketFixedPriceCollectionDrop_Must_Have_Sale_In_Progress();
    }

    uint256 mintCost = saleConfig.price * count;
    // require(msg.value == saleConfig.price * count, "BatchMintAndRevealCollection: Incorrect value provided");

    if (mintCost > totalBuyPrice) {
      revert NFTMarketFixedPriceCollectionDrop_Cannot_Buy_At_Lower_Price(mintCost);
    }

    if (IERC721(nftContract).balanceOf(msg.sender) + count > saleConfig.limitPerAccount) {
      revert NFTMarketFixedPriceCollectionDrop_Cannot_Buy_More_Than_Limit(saleConfig.limitPerAccount);
    }

    firstTokenId = ICollectionMint(nftContract).mintCountFor(count, msg.sender, marketToApprove);

    _tryUseFETHBalance(mintCost, true);

    _distributeFunds(nftContract, firstTokenId, saleConfig.seller, saleConfig.price * count, buyReferrer);

    emit MintFromFixedPriceDrop(nftContract, msg.sender, firstTokenId, count);
  }
   ...
   *     ii) when the collection is sold out or self destructed
   */
  ...
  {
    try ICollectionMint(nftContract).isSoldOut() returns (bool soldOut) {
      if (!soldOut) {
    ...
    {
      // contract self-destructed / not-supported - return default values
    }
  }
