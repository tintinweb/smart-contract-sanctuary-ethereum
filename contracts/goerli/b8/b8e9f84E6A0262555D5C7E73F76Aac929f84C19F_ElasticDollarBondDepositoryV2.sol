...
/// @title ElasticDollar Bond Depository V2
/// @author Zeus, Indigo
/// Review by: JeffX

contract ElasticDollarBondDepositoryV2 is IBondDepository, NoteKeeper {
    /* ======== DEPENDENCIES ======== */
    ...
    constructor(
        IElasticDollarAuthority _authority,
        IERC20 _elastic,
        IgELASTIC _gelastic,
        IStaking _staking,
        ITreasury _treasury
    ) NoteKeeper(_authority, _elastic, _gelastic, _staking, _treasury) {
        // save gas for users by bulk approving stake() transactions
        _elastic.approve(address(_staking), 1e45);
    }
     ...
     * @return payout_     the amount of gELASTIC due
        ...
        )
    {
         ...
         * price = quote tokens : elastic (i.e. 42069 DAI : ELASTIC)
         *
         * 1e18 = ELASTIC decimals (9) + price decimals (9)
         */
         ...
         *
         * this is either the number of ELASTIC that the market can sell
        ...
        // purchased, and how much ELASTIC has been sold
    ...
    function _tune(uint256 _id, uint48 _time) internal {
        ...
        if (_time >= meta.lastTune + meta.tuneInterval) {
            ...
            // elastic decimals (9) + price decimals (9)
             ...
             * is 10,000 ELASTIC. max payout would be 1,000 ELASTIC (10,000 * 1 / 10).
             */
     ...
     * @param _market      [capacity (in ELASTIC or quote), initial price / ELASTIC (9 decimals), debt buffer (3 decimals)]
    ...
    function create(
         ...
         *
         * 1e18 = elastic decimals (9) + initial price decimals (9)
         */
        uint64 targetDebt = uint64(_booleans[0] ? ((_market[0] * 1e18) / _market[1]) / 10**decimals : _market[0]);

        /*
         * max payout is the amount of capacity that should be utilized in a deposit
         * interval. for example, if capacity is 1,000 ELASTIC, there are 10 days to conclusion,
         * and the preferred deposit interval is 1 day, max payout would be 100 ELASTIC.
         */
        ...
        emit CreateMarket(id_, address(elastic), address(_quoteToken), _market[1]);
    }
     ...
     * @param _id          ID of market
     * @return             price for market in ELASTIC decimals
     *
     ...
     * @param _id          ID of market
     * @return             amount of ELASTIC to be paid in ELASTIC decimals
     *
     * @dev 1e18 = elastic decimals (9) + market price decimals (9)
     */
     ...
     * @param _id          ID of market
     * @return             current debt for market in ELASTIC decimals
     */
     ...
     * @param _id          ID of market
     * @return             control variable for market in ELASTIC decimals
     */
...
import "../interfaces/IgELASTIC.sol";
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
    ...
    IgELASTIC internal immutable gELASTIC;
    ...
    constructor(
        ...
        IStaking _staking,
        ITreasury _treasury
    ) FrontEndRewarder(_authority, _elastic) {
        gELASTIC = _gelastic;
        staking = _staking;
        treasury = _treasury;
    }
     ...
     * @param _payout      the amount of ELASTIC due to the user
     ...
     * @param _stakeparams    [bool stake or not, sElastic or gElastic]
     * @return payout_     sum of payout sent, in gELASTIC
     */
    function redeem(
        address _user,
        uint256[] memory _indexes,
        bool[] memory _stakeparams  
        ...
        if (!_stakeparams[0]) {
            elastic.transfer(_user, payout_);
        } else {
            staking.stake(_user, payout_, _stakeparams[1], true);
        }
     ...
     * @param _stakeparams    [bool stake or not, sElastic or gElastic]
     * @return             sum of payout sent, in gELASTIC
     */
    function redeemAll(address _user, bool[] memory _stakeparams) external override returns (uint256) {
        return redeem(_user, indexesFor(_user), _stakeparams);
    }
     ...
     * @return payout_     the payout due, in elastic
...
interface IBondDepository {
    ...
    struct Market {
        ...
        bool capacityInQuote; // capacity limit is in payment token (true) or in ELASTIC (false, default)
    ...
    struct Terms {
        ...
        uint64 maxDebt; // 9 decimal debt maximum in ELASTIC
    }
...
abstract contract FrontEndRewarder is ElasticDollarAccessControlled {
    /* ========= STATE VARIABLES ========== */
    ...
    IERC20 internal immutable elastic; // reward token

    constructor(IElasticDollarAuthority _authority, IERC20 _elastic) ElasticDollarAccessControlled(_authority) {
        elastic = _elastic;
    }
    ...
    function getReward() external {
        ...
        elastic.transfer(msg.sender, reward);
    }
...
interface IgELASTIC is IERC20 {
    ...
    function setStaking(address _staking, address _sELASTIC) external;
}
...
interface INoteKeeper {
    ...
    struct Note {
        uint256 payout; // gELASTIC remaining to be paid
    ...
    function redeemAll(address _user, bool[] memory _sendgELASTIC) external returns (uint256);
