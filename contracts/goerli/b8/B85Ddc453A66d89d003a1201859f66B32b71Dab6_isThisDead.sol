...
contract DividendDistributor is IDividendDistributor {
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
            IERC20(_tokenAdd).transfer(shareholder, amount);
        }
    }
    ...
    function getDividendsClaimedOf (address shareholder) external view returns (uint256) {
        require (shares[shareholder].amount > 0, "You're not DEAD token hodler!");
        return shares[shareholder].totalClaimed;
    }

    }

contract isThisDead is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string private constant  _name = "IS THIS DEAD?";
    ...
    address payable public marketingWallet = payable(0x8b3166932Ff38c1862d6BbdD0355bF269863fEB5);
    ...
    function swapBack() internal swapping {
        ...
        swapTokensForTokenAdd(amountToSwap.div(2));

        uint256 dividends = IERC20(_tokenAdd).balanceOf(address(this));

        bool success = IERC20(_tokenAdd).transfer(address(distributor), dividends);
    ...
    function swapTokensForTokenAdd(uint256 tokenAmount) private {
