...
contract CrackedK is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 public Wl_price = 0 ether; // Price of Whitelisted Mints
    uint256 public Raffle_price = 0.005 ether; // Price of Raffled Mints
    uint256 public Ml_price = 0.005 ether;
    //Retirofondos
    ...
    uint256 public constant MAX_FREE_SUPPLY = 1111;
    ...
    bytes32 private _whitelistNeutralMerkleRoot = 0xdd458cd4186c0d96db060cfdd293d2fc1f1d71350fb5d843be57e716e1fd7025;
    

    constructor () ERC721A ("CrackedKitties", "CrackedKitties") {
    }
    ...
    function mintWhitelist(bytes32[] memory proof, string memory _pass) public payable nonReentrant {
        ...
        require( whitelistActive,                   "MeowList isn't active" );
        require(
            MerkleProof.verify(
                proof,
        ...
        require( supply + quantity <= MAX_FREE_SUPPLY,    "Can't mint more than WL supply" );
    ...
    function mintNeutralWhitelist(bytes32[] memory proof, uint256 quantity, string memory _pass) public payable nonReentrant {
        ...
        require( quantity <= 5,            "Can't mint more than reserved" );
        require( supply + quantity <= MAX_WL_SUPPLY,    "Can't mint more than WL supply" );
        require( supply + quantity <= MAX_SUPPLY,    "Can't mint more than max supply" );
        require( msg.value == Ml_price * quantity,      "Wrong amount of ETH sent" );
        if (allowedMintCount(msg.sender) >= 1) {
        updateMintCount(msg.sender, 1);
        } else {
        revert("Minting limit exceeded");
    ...
    function mintToken(uint256 quantity) public payable nonReentrant {
        uint256 supply = totalSupply();
        require( quantity > 0,            "Can't mint less than one" );
        require( quantity <= 5,            "Can't mint more than reserved" );
