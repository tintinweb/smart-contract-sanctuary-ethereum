...
interface IHeterosis {
    function mint(uint256 tokenId) external returns (Breed memory);
}
...
contract Minter is Pausable, AccessControl {
    ...
    function _mintBreeds(uint256 _startTokenId, uint256 _amount) internal {
        for (uint256 i = 0; i < _amount; ++i) {
            ...
            Breed memory breed = heterosisContract.mint(tokenId);
            emit BreedMinted(tokenId, breed.idUniq, breed.mainDna, breed.shadowDna);
        }
    }
}