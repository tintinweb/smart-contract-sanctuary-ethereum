...
contract Market is NFTBondMarketBase {
    ...
    uint private constant TAKER_FEE = 25;
    uint private constant MAKER_FEE = 25;
...
abstract contract NFTBondMarketBase is ManagedMarketBase {    
    ...
    function initialize(
        ...
        __Market_init(_token, _btoken,_config, _feed);
    }
    ...
    function close(uint id) public virtual override {
        ...
        // unlock BUMP tokens
        if (bumpAmount > 0) {
            bond.unlock(msg.sender, _token, bumpAmount);
        }
        // Burn position NFT token
        IPositionNFT(config.getNFTTaker(_token)).safeBurn(id);
        // close position
        super.close(id);
    }
    ...
    function withdraw(uint id) public virtual override {
        ...
        // burn position NFT token
        IPositionNFT(config.getNFTMaker(_token)).safeBurn(id);
...
abstract contract ManagedMarketBase is TransferableMarketBase, IMarketManagement {
    ...
    function setBond(address _bond) external virtual override onlyGovernance {
        bond = IBond(_bond);
    }
...
interface IMarketManagement {
    ...
    function setBond(address _bond) external;
    ...
    IMarket
{
    ...
    uint internal DIVIDER;
    uint constant internal DIVIDER_ORACLE = 10**8;
    uint constant internal DIVIDER_STABLE = 10**6;
    ...
    function __Market_init(
        ...
        token = IERC20(_token);
        stable = IERC20(config.getStable());
        bump = IERC20(config.getBump());
    ...
    function close(uint id) public virtual override takerTermEnds(id) {
        ...
        emit Closed(address(this), msg.sender, id, amountToReturn, posPremium);
    }
    ...
    function claim(uint id) external virtual override takerTermEnds(id) {
        ...
        uint claimSizeInStable = ((pos.assetAmount - p) * uint(_price)) /
            DIVIDER;
        CP -= claimSizeInStable;

        stable.safeTransfer(pos.owner, claimSizeInStable);
    ...
    function cancel(uint id) public virtual override takerTermEnds(id) {
        ...
        emit Canceled(address(this), msg.sender, id, amountToReturn, _premium);
    }
