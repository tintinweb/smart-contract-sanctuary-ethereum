...
contract HunterMemeCards is DefaultOperatorFilterer, Ownable, ERC721A, ReentrancyGuard {

    using Address for address;
    using Strings for uint256;

    string private _contractUri = "ipfs://QmfJhjtJUmnpVUHkGzGtdK5vya9mcxsyZYatigL4bvxMXz";

    //allows for team reserves to be dev minted
    uint256 public MAX_TEAM_RESERVES_PRESALE = 0;
    //sets immutable value for lifetime total collection of 4200
    uint256 public immutable MAX_TOKENS;
    //toggle supply total for sale depending on wave
    uint256 public MINT_SUPPLY_TOGGLE = 4200;
    //toggle supply total for sale depending on wave
    uint256 public WL_SUPPLY_TOGGLE = 4200;
    //sets limit of mints per wallet during allowlist sale
    uint256 public MAX_WL_TOKENS_PER_ADDRESS = 1;
    //sets limit of mints per wallet during public sale
    uint256 public MAX_PUBLIC_TOKENS_PER_ADDRESS = 2;
    ...
    uint256 collectionSize_,
    uint256 maxTOKENS_
  ) ERC721A("Hunter Meme Cards","HunterMemes", maxBatchSize_,collectionSize_) {
...
//minting function for the WL allowlist sale
    function WLsaleMint(uint256 quantity, bytes32[] calldata WLsaleMerkleProof) external payable callerIsUser nonReentrant {
        ...
        require(totalSupply() + quantity <= MINT_SUPPLY_TOGGLE, "minting this many would exceed supply for WL round");
        require(totalSupply() + quantity <= WL_SUPPLY_TOGGLE, "minting this many would exceed supply for WL round");
...
//public mint function
    function mint(uint256 quantity) external payable callerIsUser nonReentrant {
        ...
        require(totalSupply() + quantity <= MINT_SUPPLY_TOGGLE, "minting this many would exceed supply for this phase");
    ...
    function _checkMintRequirements(uint256 quantity) internal {
        ...
        require(quantity > 0 && quantity <= MAX_PUBLIC_TOKENS_PER_ADDRESS, "invalid quantity: zero or greater than mint allowance");
        ...
        require(PublicNumMinted[msg.sender] <= MAX_PUBLIC_TOKENS_PER_ADDRESS, "Cannot mint more than your allotment in this phase");
    }
...
//admin minting function for team tokens
    ...
    function setMINT_SUPPLY_TOGGLE(uint256 _MINT_SUPPLY_TOGGLE) public onlyOwner() {
    MINT_SUPPLY_TOGGLE = _MINT_SUPPLY_TOGGLE;
    }

    function setWL_SUPPLY_TOGGLE(uint256 _WL_SUPPLY_TOGGLE) public onlyOwner() {
    WL_SUPPLY_TOGGLE = _WL_SUPPLY_TOGGLE;
    }
    ...
    function setMAX_PUBLIC_TOKENS_PER_ADDRESS(uint256 _MAX_PUBLIC_TOKENS_PER_ADDRESS) public onlyOwner() {
    MAX_PUBLIC_TOKENS_PER_ADDRESS = _MAX_PUBLIC_TOKENS_PER_ADDRESS;
    }
    ...
    function setMAX_TEAM_RESERVES_PRESALE(uint256 _MAX_TEAM_RESERVES_PRESALE) public onlyOwner() {
    MAX_TEAM_RESERVES_PRESALE = _MAX_TEAM_RESERVES_PRESALE;
    }
