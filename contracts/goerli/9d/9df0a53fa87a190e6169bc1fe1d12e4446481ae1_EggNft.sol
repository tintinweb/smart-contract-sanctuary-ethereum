...
contract EggNft is ERC721, Ownable {
   ...
   function setMintType(uint256 nftMintType) external onlyOwner {
    ...
    function getUserWhiteListed() external view returns (uint){
        uint256 value;
        if(mintType==0)
        {
    ...
    function mint(uint256 _mintAmount, uint8 _nftType) external payable {
        ...
        if(mintType==0)
        {
            require(_ogList[msg.sender] == true,"Not registry at OG list");
            require(
        ...
        {
            require(_wlList[msg.sender] == true,"Not registry at WL list");
                require(
