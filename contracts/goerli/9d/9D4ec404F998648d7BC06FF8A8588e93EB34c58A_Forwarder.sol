...
contract Forwarder is Initializable , AccessControlUpgradeable {
    ...
    function initialize(address _vault, address _admin, address _flusher, address _feeReceiver) initializer public {
        ...
        require(_feeReceiver != address(0), "FEE RECEIVER address must not be address zero");
    ...
    */
    function flushTokens(address tokenContractAddress) external onlyRole(FLUSH_ROLE) {
        ...
        uint256 fee = forwarderBalance / 100;
        instance.safeTransfer(vault, forwarderBalance - fee);
        instance.safeTransfer(feeReceiver, fee);
    ...
    function flushETH() external onlyRole(FLUSH_ROLE){
        ...
        require(forwarderBalance != 0, "Forwarder balance must NOT be ZERO");
        uint256 fee = forwarderBalance / 100;
        (bool vaultSuccess, ) = vault.call{value: forwarderBalance}("");
        (bool feeSuccess, ) = feeReceiver.call{value: fee}("");
        require(vaultSuccess , "Couldn't flush ETH!");
        require(feeSuccess , "Couldn't process fee!");
