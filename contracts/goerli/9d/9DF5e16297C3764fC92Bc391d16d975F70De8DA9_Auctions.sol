...
contract Auctions {
    ...
    function getHighestBid(uint256 _tokenId, address _nftContractAddress) public view returns (uint256) {
        return fundsByBidder[_auctions[_nftContractAddress][_tokenId].highestBidder];
    }

    function placeBid(uint256 _tokenId, address _nftContractAddress)
        public
        payable
        ...
        returns (bool success)
    {
        ...
        require(
            newBid >= _auctions[_nftContractAddress][_tokenId].highestBindingBid,
            "New Bid should be greator then or equal to highest bid"
        );

        // grab the previous highest bid (before updating fundsByBidder, in case msg.sender is the
        // highestBidder and is just increasing their maximum bid).
        uint256 highestBid = fundsByBidder[_auctions[_nftContractAddress][_tokenId].highestBidder];
        ...
        if (newBid <= highestBid) {
            ...
            if (msg.sender != _auctions[_nftContractAddress][_tokenId].highestBidder) {
                _auctions[_nftContractAddress][_tokenId].highestBidder = msg.sender;
                _auctions[_nftContractAddress][_tokenId].highestBindingBid = min(
                    newBid,
                    highestBid
                );
    ...
    function cancelAuction(uint256 _tokenId, address _nftContractAddress)
        public
    ...
    function getBids(address _nftContractAddress, uint256 tokenId) public view returns (Bids[] memory) {
    ...
    {
        uint256 withdrawalAmount = _auctions[_nftContractAddress][_tokenId].highestBindingBid;
    ...
    function withdrawBid(uint256 _tokenId, address _nftContractAddress)
        public
        onlyEndedOrCanceled(
            _auctions[_nftContractAddress][_tokenId].endBlock,
            _auctions[_nftContractAddress][_tokenId].canceled
        ...
        returns (bool success)
    {
        ...
        if (_auctions[_nftContractAddress][_tokenId].canceled) {
            // if the auction was canceled, everyone should simply be allowed to withdraw their funds
            withdrawalAccount = msg.sender;
            withdrawalAmount = fundsByBidder[withdrawalAccount];
        } else {
            // the auction finished without being canceled
            address nftHighestBid = _auctions[_nftContractAddress][_tokenId].highestBidder;
            require(msg.sender == nftHighestBid, "Cannot withdraw funds");
    ...
    function _resetAuction(uint256 _tokenId, address _nftContractAddress) public {
