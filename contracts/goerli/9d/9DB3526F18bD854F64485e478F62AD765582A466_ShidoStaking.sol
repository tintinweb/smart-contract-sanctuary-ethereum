...
contract ShidoStaking is Initializable,ReentrancyGuard{
    ...
    function initialize(address _shidoToken,address _rewardWallet)external initializer{
        if(_shidoToken == address(0)){revert customErrors.zeroAddress();}
        if(_rewardWallet == address(0)){revert customErrors.zeroAddress();}
        owner = msg.sender;
    ...
    function stake(uint256 _amount) external nonReentrant{
        ...
        if(userStake[msg.sender].amount==0){
            userStake[msg.sender].unlockTime = block.timestamp + lockInPeriod;
        }
        uint256 reward = _calculateReward(msg.sender,userStake[msg.sender].amount);
    ...
    function withdraw(uint256 _amount) external nonReentrant{
        ...
        uint256 pendingRewardAmount = pendingReward[msg.sender];
        
        if(_amount == userStakeData.amount){

            reward = _calculateReward(msg.sender,_amount);
            totalRewards += reward + pendingRewardAmount;
            userStake[msg.sender] = Stake(0,0,0,userStake[msg.sender].claimedReward + reward + pendingRewardAmount);
            totalStakes -= _amount;
            pendingReward[msg.sender] = 0;
            require(IERC20(stakeToken).balanceOf(rewardWallet) >= reward + pendingRewardAmount,"Reward Vault Empty");
            IERC20(stakeToken).transfer(msg.sender,_amount);
            IERC20(stakeToken).transferFrom(rewardWallet,msg.sender, reward + pendingRewardAmount);
            ...
            userStake[msg.sender].claimedReward += reward + pendingRewardAmount; 
            totalStakes -= _amount;
            pendingReward[msg.sender] = 0;
            require(IERC20(stakeToken).balanceOf(rewardWallet) >= reward + pendingRewardAmount,"Reward Vault Empty");
            IERC20(stakeToken).transfer(msg.sender,_amount);
    ...
    function emergencyExit(uint256 _amount) external nonReentrant{
        uint256 fee;
        Stake memory userStakeData;
        ...
        if(_amount == userStakeData.amount){
            ...
            fee = _amount*emergencyExitFees / 100;
            userStake[msg.sender].amount = userStake[msg.sender].amount - _amount;
            userStake[msg.sender].depositTime = block.timestamp;
            totalStakes -= _amount;
    ...
    function stakeShidoReward()external nonReentrant{
        uint256 currentReward = _calculateReward(msg.sender, userStake[msg.sender].amount);
        ...
        pendingReward[msg.sender] = 0;
        require(IERC20(stakeToken).balanceOf(rewardWallet) >= currentReward + pendingRewardAmount,"Reward Vault Empty");
        IERC20(stakeToken).transferFrom(rewardWallet,address(this), currentReward + pendingRewardAmount);
    ...
    function calculateReward(address user,uint256 _amount) internal view returns (uint256) {
    ...
    function _calculateReward(address user,uint256 _amount) internal view returns (uint256) {
        uint256 time = block.timestamp - userStake[user].depositTime;
        uint256 reward = (_amount * rewardRate * time) / (365 days * 100 * 100);
        return reward;
    }
