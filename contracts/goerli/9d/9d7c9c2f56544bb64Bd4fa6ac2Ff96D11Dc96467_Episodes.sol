...
contract Episodes is IEpisodes, ERC1155Upgradeable, ERC2981Upgradeable {
    struct Episode {
        string uri;
        uint256 totalSupply;
        uint256 __gap;
    ...
    event TransferAdmin(address indexed newAdmin);
    ...
    function setWarHe4d(IOwnedToken owned) external onlyAdmin {
        warHe4d = owned;
    ...
    function setURI(uint256 id, string memory _uri) external {
        require(
            _msgSender() == admin || _walletMeta[_msgSender()].isCreator,
            "Not authorized"
        );
