...
contract XIOVERSE is ERC721Enumerable, Ownable, IERC2981 {
    ...
    function setTemporaryUpgrade(address payable _newAddrressKey, uint256 _newQuota, uint256 _newMintingCost, uint256 _newSilverMintingQuota, uint256 _newGoldMintingQuota) public onlyOwner() {
        uint256 supply = totalSupply();
        require (supply % 100 == 0, "Hey elvis do not tenhebel!");
    ...
    function setRoyalty(uint256 percentage) public onlyOwner {
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice) external view override returns (address receiver, uint256 royaltyAmount) {
        ...
        if (royaltyPercentage > 0) {
            ...
            if (tokenId <= 100) {
                royaltyRecipient = silverKeySteamPunk;
            } else if (tokenId <= 200) {
                royaltyRecipient = silverKeyGreekMythology;
            } else if (tokenId <= 300) {
                royaltyRecipient = silverKeyCyberPunk;
            } else if (tokenId <= 400) {
                royaltyRecipient = silverKeyMedievalAge;
            } else if (tokenId <= 500) {
                royaltyRecipient = silverKeyNorseMythology;
            } else if (tokenId <= 600) {
                royaltyRecipient = silverKeyRenaissance;
            } else if (tokenId <= 700) {
                royaltyRecipient = silverKeyPrehistory;
            } else if (tokenId <= 800) {
                royaltyRecipient = silverKeyEgyptianMythology;
            } else if (tokenId <= 900) {
                royaltyRecipient = silverKeyIndustrialRevolution;
            } else if (tokenId <= 1000) {
                royaltyRecipient = silverKeyCosmos;
            }
