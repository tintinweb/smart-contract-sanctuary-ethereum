...
contract NiftyOrderbook is INiftyOrderbook, ReentrancyGuard, Validator, Ownable, GelatoRelayContext {
    ...
    function matchOrders(
        ...
        if (_isGelatoRelay(msg.sender)) {
            ...
            totalValue = totalValue.add(_getFee());
        }
        ...
        if (_isGelatoRelay(msg.sender)) {
            _transferRelayFeeCapped(order.maxFee.mul(numOfItems));
        }
        // execute trades
        _trade(tradeDetails);
        // return remaining ETH (if any)
        ...
        // emit match event
        emit Match(orderHash, tradeDetails.length);
    }
    ...
    function validateOrderItems(
        ...
        for (uint256 i = 0; i < tradeDetails.length; i++) {
            ...
            // total order value
            totalValue = totalValue.add(tradeDetails[i].value);
        }
...
contract Validator is INiftyOrderbook, EIP712 {
    ...
    function signatureValid(bytes32 orderHash, address maker, bytes memory signature) internal view returns (bool isValid) {
...
interface INiftyOrderbook {
    ...
    event Fill (bytes indexed hash, address indexed maker, uint filled);
}
