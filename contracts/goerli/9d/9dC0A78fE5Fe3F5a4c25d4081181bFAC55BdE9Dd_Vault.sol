...
contract Vault is IVault, Ownable {
    ...
    function deposit(
        ...
        require(
            usdcToken.balanceOf(msg.sender) < amountToDeposit,
        ...
        if (compareStrings(curCoin, "usdc")) {
            uint256 totalAmountUsd = (totalVaultUSDC * usdcPrice) /
                (10 ** priceDecimal);
            estimatedLpTokens =
                (amountToMint * lpToken.totalSupply()) /
                totalAmountUsd;
            totalVaultUSDC = totalVaultUSDC + amountToDeposit;
        ...
        usdcToken.approve(msg.sender, amountToDeposit);
        usdcToken.transferFrom(msg.sender, address(this), amountToDeposit);
    ...
    function initializeVault(
        ...
        usdcToken.approve(msg.sender, usdcAmount);
    ...
    function withdraw(uint256 repaymentAmount) external override {
        ...
        uint256 usdcTokens = (repaymentAmount * (10 ** priceDecimal)) /
            usdcPrice;
        if (compareStrings(curCoin, "usdc")) {
            ...
            usdcToken.approve(address(this), usdcTokens);
...
interface IVault {
     ...
     */
    function deposit(address depositTokenAddress, uint256 amountToDeposit) payable external ;
...
contract PriceConsumerV3 {
    ...
    constructor() {
        ...
        usdcPriceFeed = AggregatorV3Interface(0x8fFfFfd4AfB6115b954Bd326cbe7B4BA576818f6);
    }
