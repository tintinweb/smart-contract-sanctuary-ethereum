...
contract Locker is ILocker {
    ...
    event Lock(address indexed to, uint256 requested, uint256 lockedAfterTax);
    ...
    constructor(
        ...
        require(_beli != address(0), "invalid address");
        BELI = _beli;
    ...
    function lock(address _account, uint256 _amount) external override {
        ...
        uint256 preBalance = IERC20(BELI).balanceOf(address(this));
        IERC20(BELI).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 postBalance = IERC20(BELI).balanceOf(address(this));
        uint256 lockedAmount = postBalance.sub(preBalance);

        _locks[_account] = _locks[_account].add(lockedAmount);
        _totalLock = _totalLock.add(lockedAmount);

        emit Lock(_account, _amount, lockedAmount);
    }
    ...
    function unlock() external override {
        ...
        IERC20(BELI).safeTransfer(msg.sender, _amount);
