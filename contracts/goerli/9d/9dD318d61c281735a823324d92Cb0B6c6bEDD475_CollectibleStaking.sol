...
contract CollectibleStaking is Pausable, Controllable, IERC721Receiver {
  ...
  uint256 public num_staked_tokens;
  ...
  function stake(uint256 tokenId) public payable {
    require(
      ...
      'CollectibleStaking: not the token owner'
    );
    require(
      token.isApprovedForAll(_msgSender(), address(this)),
      'CollectibleStaking: Not approved'
    );
    require(
      token_indices[tokenId] == 0,
      'CollectibleStaking: token already staked'
    );
    token.safeTransferFrom(_msgSender(), address(this), tokenId);
    staked_tokens.push(tokenId);
    token_indices[tokenId] = staked_tokens.length;
    token_owners[tokenId] = _msgSender();
    num_staked_tokens++;
    emit CollectibleStaked(tokenId, _msgSender());
  }

  function unstake(uint256 tokenId) public payable {
    address owner = token_owners[tokenId];
    require(
      _msgSender() == owner || isController(_msgSender()),
      'CollectibleStaking: Cannot unstake; not the owner or controller'
    );
    token.safeTransferFrom(address(this), owner, tokenId);

    delete staked_tokens[token_indices[tokenId] - 1];
    delete token_indices[tokenId];
    delete token_owners[tokenId];
    num_staked_tokens--;
    emit CollectibleUnstaked(tokenId, owner);
  }
