...
contract Staker {
    ...
    function sharePrice() public view returns (uint256 price) {
        ...
        price = ((totalAmount * phiPrecision + (phiPrecision - phi) * getUnstakedRewards()) * 10**18) / (totalShares * phiPrecision);
    ...
    function compoundRewards() public {
        ...
        uint256 amountRestaked = getUnstakedRewards();

        // to keep share price constant when rewards are staked, new shares need to be minted
        uint256 shareIncrease = ((totalAmount + amountRestaked) * 10**18) / sharePrice() - totalShares;
...
Store whitelist address
Whitelist interface

isUserWhitelisted when a user joins

think about blocking withdrawals?

frontend: direct users to getting a quad passport on eth mainnet / goerli

redeploy the whitelist with the deployment scripts (master branch in trufin repo)
modify deployment scripts to deploy on goerli
