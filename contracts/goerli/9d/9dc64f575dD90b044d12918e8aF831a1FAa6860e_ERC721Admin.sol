...
contract ERC721Admin is ERC721Enumerable, AccessControl {
    ...
    constructor(
        address initDefaultAdmin,
        address[] memory admins,
        string memory initBaseURI
    ) ERC721("MOJOY", "MOJOY") {
        uint256 size = admins.length;
        _setupRole(DEFAULT_ADMIN_ROLE, initDefaultAdmin);
        _setupRole(ADMIN, initDefaultAdmin);
    ...
    function adminBatchBurn(uint256[] calldata tokenIds)
        external
        onlyRole(ADMIN)
    {
    ...
    /// @dev Transfer default admin role
    function transferDefaultAdmin(address newDefaultAdmin)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        grantRole(DEFAULT_ADMIN_ROLE, newDefaultAdmin);
    ...
    /// @notice Override ERC721.tokenURI
