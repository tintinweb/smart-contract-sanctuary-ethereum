...
contract LockAssets is Ownable, ERC721Holder{
    ...
    function stake721(uint256 _tokenId, uint256 _stakeFor, address _add, string memory _imageLink) public {
        IERC721 nft = IERC721(_add);
        nft.safeTransferFrom(msg.sender, address(this), _tokenId);
        staked721[msg.sender].push(Asset721(_tokenId, block.timestamp + _stakeFor, nft, _imageLink));
        counter ++;
    }
    ...
    function withdraw721(uint256 _tokenId, IERC721 _add) public {
        ...
        for (uint i = 0; i < assetsStaked721.length; i++) {
            ...
            if(assetsStaked721[i].tokenId == _tokenId && assetsStaked721[i].add == _add){

            //verify time has been met
            require(block.timestamp > assetsStaked721[i].withdrawDate, "Time Not Up");

             //get address
             IERC721 add = assetsStaked721[i].add;

             //transfer token
             add.safeTransferFrom(address(this), msg.sender, assetsStaked721[i].tokenId, "0x00");
    ...
    function getLockedAssetsForAddress(address addr) public view returns (Asset721[] memory) {
