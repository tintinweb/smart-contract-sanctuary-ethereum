...
contract LifeLikeTheRichClub is ERC721A, Ownable {
    ...
    uint256 public USDprice = 100 ether;
    SaleStatus public saleStatus = SaleStatus.PUBLIC;
    string private _baseURL = "ipfs://";
    
    mapping(address => uint) private _mintedCount;

    constructor() ERC721A("LifeLikeTheRichClub VIP Tickets", "LLTRC"){
    ...
    function setPublicMintPrice(uint256 _USDprice) external onlyOwner {
        USDprice = _USDprice;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        payable(msg.sender).transfer((balance * 10000)/10000);
    ...
    function mint(uint count) external payable {
        int256 currentEthPrice = getCurrentEthPrice();
        uint256 pricePerNFT = (USDprice / (uint256(currentEthPrice)*10**10))*10**18;
        ...
        require(msg.value >= count * pricePerNFT, " Ether value sent is not sufficient");
