    ...
    IOrderBook
{
    ...
    modifier onlyNFTOwner(address dropAddress, uint256 nftId) {
        require(
            wrapper.isValidNFT(dropAddress, nftId),
            "OB: invalid drop address"
        );
        require(
            IERC721(dropAddress).ownerOf(nftId) == msg.sender,
            "OB: invalid owner"
        );
        _;
    }
    ...
    function initialize(IEndStateNFTWrapper _wrapper, IEscrow _escrow)
        external
        initializer
    {
    ...
    function list(
        ...
        Quality quality,
        uint256 expiry
    ) external override onlyNFTOwner(drop, nftId) {
        require(price >= MIN_LIST_PRICE, "OB: listing price too low");

        IERC721 drop_ = IERC721(drop);

        require(
            drop_.getApproved(nftId) == address(escrow),
            "OB: not approved for escrow"
        );

        escrow.lock(drop, nftId, msg.sender);
        ...
        emit OrderCreated(orderCount - 1, msg.sender, drop, nftId, price);
    }
    ...
    function delist(uint256 orderId) external override validOrder(orderId) {
        ...
        escrow.releaseNFT(msg.sender, order.drop, order.nftId);
    ...
    function fulfill(uint256 orderId) external override validOrder(orderId) {
        ...
        require(order.to == msg.sender, "OB: order is not yours");

        escrow.releaseNFT(order.to, order.drop, order.nftId);
    ...
    function resolveDispute(uint256 orderId)
        external
        validOrder(orderId)
        onlyOwner
    {
