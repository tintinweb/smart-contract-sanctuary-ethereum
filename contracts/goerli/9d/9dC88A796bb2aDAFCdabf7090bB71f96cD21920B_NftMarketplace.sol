...
error NftMarketplace__NotAuthorized();
...
contract NftMarketplace is ReentrancyGuard {
    ...
    modifier onlyLazyNft() {
        if (msg.sender != i_lazyNft) {
            revert NftMarketplace__NotAuthorized();
        }
    ...
    // contructor  //
    //////////////////////
    constructor(address lazyNftAddress) {
        i_lazyNft = lazyNftAddress;
    }
    ...
    function nftVoucherBought(
        address nftAddress,
        uint256 tokenId,
        uint256 price,
        address seller
    ) external payable onlyLazyNft {
        if (msg.value < price) {
