...
contract Bag is IBag, ERC721, MinterManaged {
    ...
    constructor(address manager) MinterManaged(manager) ERC721("TNL Bag", "TNLB") {}
...
contract MinterManaged is IMinterManaged, AccessControl, Ownable, Errors {
    ...
    address public constant ASM_WALLET = 0x5F3F357194C32f9c2f1176CC39B9d6c214A1fDCA;

    constructor(address manager) {
        if (manager == address(0)) revert InvalidInput(INVALID_MANAGER);
        _grantRole(MANAGER_ROLE, manager);
        transferOwnership(ASM_WALLET); // OpenSea checks the ownership of the contract
        _manager = manager;
    }
