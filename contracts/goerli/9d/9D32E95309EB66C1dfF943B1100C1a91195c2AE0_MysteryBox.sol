...
contract MysteryBox is ERC721, Ownable, IMysteryBox {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    IPotions private _potion;
    uint256 _commonLimit;
    uint256 _rareLimit;
    uint256 private _commonPrice;
    uint256 private _rarePrice;
    mapping(address => uint256) _commonLimits;
    mapping(address => uint256) _rareLimits;
    mapping(uint256 => bool) _rare;

    /**
@notice Constructor
@param name_ The name
@param symbol_ The symbol
@param potionContract_ The address of the potion contract
...
*/
    constructor(
        ...
        address potionContract_,
        uint256 commonLimit_,
        uint256 rareLimit_,
        uint256 commonPrice_,
        uint256 rarePrice_
    ) ERC721(name_, symbol_) {
        _potion = IPotions(potionContract_);
        ...
        emit PotionDefined(potionContract_);
    }
...
*/
    function setCommonLimit(uint256 value_) external override onlyOwner {
        _commonLimit = value_;
...
*/
    function setCommonPrice(uint256 value_) external override onlyOwner {
        _commonPrice = value_;
...
@param value_ New amount
*/
    function setRareLimit(uint256 value_) external override onlyOwner {
        _rareLimit = value_;
...
@param value_ New amount
*/
    function setRarePrice(uint256 value_) external override onlyOwner {
        _rarePrice = value_;
        emit RarePriceDefined(value_);
    }

    /**
@notice Set the potion contract address
@param value_ New address
*/
    function setPotion(address value_) external override onlyOwner {
        _potion = IPotions(value_);
        emit PotionDefined(value_);
    }
...
*/
    ...
    function getLimits(address account_)
        external
        view
        override
    ...
    {
        return (
            _commonLimit - _commonLimits[account_],
            _rareLimit - _rareLimits[account_]
        );
    }
...
*/
    function create(address target_, bool rare_)
        ...
        returns (uint256)
    {
        require(_potion.decreaseAmount(rare_), "MysteryBox: sold out");
        return _create(target_, rare_);
    }
...
*/
    function rarity(uint256 tokenId_) external view override returns (bool) {
...
*/
    function withdraw(address payable target, uint256 amount)
        ...
        onlyOwner
    {
        require(target.send(amount), "MysteryBox: Failed to send Ether");
    }
...
*/
    receive() external payable {
        if (_rarePrice <= msg.value) {
            if (msg.value > _rarePrice) {
                require(
                    payable(_msgSender()).send(msg.value - _rarePrice),
                    "MysteryBox: Failed to return Ether"
                );
            }
            require(
                _rareLimit > _rareLimits[_msgSender()],
                "MysteryBox: no more rare tokens allowed for user"
            );
            require(
                _potion.decreaseAmount(true),
                "MysteryBox: sold out rare tokens"
            );
            _create(_msgSender(), true);
            return;
        }
        if (_rarePrice > msg.value && _commonPrice <= msg.value) {
            if (msg.value > _commonPrice) {
                require(
                    payable(_msgSender()).send(msg.value - _commonPrice),
                    "MysteryBox: Failed to return Ether"
                );
            }
            require(
                _commonLimit > _commonLimits[_msgSender()],
            ...
            _create(_msgSender(), false);
            return;
        }
        require(false, "MysteryBox: too less Ether");
    }

    function _create(address account_, bool rare_) private returns (uint256) {
        if (rare_) {
            _rareLimits[account_] = _rareLimits[account_] + 1;
        } else {
            _commonLimits[account_] = _commonLimits[account_] + 1;
        }
        uint256 newId = _tokenIds.current();
        _rare[newId] = rare_;
        _mint(account_, newId);
        _tokenIds.increment();
        emit Created(account_, newId, rare_);
        return newId;
    }
...
*/
    function open(uint256 id_) external override returns (uint256) {
        require(ownerOf(id_) == _msgSender(), "MysteryBox: wrong owner");
        uint256 newId = _potion.create(_msgSender(), _rare[id_]);
        delete _rare[id_];
        _burn(id_);
...
interface IMysteryBox is IERC721 {
    event Created(address indexed owner, uint256 id, bool rare);
    event Opened(address indexed owner, uint256 id);
    ...
    event PotionDefined(address indexed potionContract);
...
@param value_ New address
*/
    function setPotion(address value_) external;
