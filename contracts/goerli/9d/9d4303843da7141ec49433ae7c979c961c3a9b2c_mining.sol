...
contract mining{
    ...
    ERC20 token = ERC20(0x6615867811117B835BE9F06f4456138362b5970D);
    ERC721 my_nft = ERC721(0x1Df6f31aEc0BfeFc8d1271Fb9d453c52FF188D8c);

    mapping( uint256=> uint256 ) last_block;
    // 각자의 데이터를 맵핑한다(쌍으로 묶인걸 저장)
    
    //70번  NFT -> 812523 블록으로 저장됨
    //91번  NFT -> 831154 블록
    //100번 NFT -> 922861 블록
    ...
    function do_mine(uint256 nft_id) public {
        ...
        if(my_nft.ownerOf( nft_id ) == msg.sender) {
            ...
            if(last_block[nft_id] == 0){
              // 실행할 때 NFT 넘버를 []에 입력
              // NFT에 없는 넘버를 []에 넣으면 실행 안됨
              // 아래 라스트블록코드에도 [nft_id]추가
                last_block[nft_id] = block.number;
       
            }else{

                uint256 amount = block.number - last_block[nft_id];
                // uint256 amount = 2*(block.number - last_block[nft_id]);
                // => 한 블록마다 2개씩 채굴할 때 "숫자 *" 추가
                last_block[nft_id] = block.number;
