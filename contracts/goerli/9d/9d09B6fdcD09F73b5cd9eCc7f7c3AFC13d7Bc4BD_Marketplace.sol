...
contract Marketplace is Ownable, ReentrancyGuard {
        ...
        collectionExist(collectionOwner, collectionIndex)
    {
        ...
        bool isListed = listedItemStatus[_collection][tokenId];

        if (!isListed) {
            revert ItemNotListed(tokenId);
        }
        if (
            listedItemIndex >= listedItems.length ||
            listedItems[listedItemIndex].collectionAddress !=
            address(_collection) ||
            listedItems[listedItemIndex].tokenId != tokenId
        ) {
            revert IndexDoesNotExist();
        }

        uint256 itemPrice = listedItems[listedItemIndex].price;
        ...
        listedItemStatus[_collection][tokenId] = false;
        listedItems[listedItemIndex] = listedItems[listedItems.length - 1];
        listedItems.pop();
