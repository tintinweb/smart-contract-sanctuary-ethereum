    ...
    AccessControlEnumerable
{
    ...
    function addUsername(string memory _username) private {
        require(isValidUsername(_username));
        require(!isUnique(_username));
        string memory username = toLowerCase(_username);
        usernames[username] = true;
        nfts[_nextTokenId()] = username; 
    }

    function isUnique(string memory _username) public view returns (bool) {
        require(isValidUsername(_username));
        string memory username = toLowerCase(_username);
        return usernames[username];
    }

    function getUsernameFromId(uint256 id) public view returns (string memory) {
        return nfts[id];
    }
