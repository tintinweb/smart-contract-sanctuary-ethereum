...
contract CounterNFT is ERC721URIStorage, Ownable {
    ...
    Counters.Counter private _tokenIds; // 外からトークンIDを受け取らない
     ...
     * - nftMint関数の実行アドレスにtokenIdを紐付け _msgSender()
     * - mintの際にURIを設定する _setTokenURI()
     * - EVENT発火 emit TokenURIChanged
     **/
    function nftMint() public onlyOwner {
        _tokenIds.increment();  // インクリメントする。デフォは0なので1始まるとなる。
        uint256 newTokenId = _tokenIds.current();   // 現在の値を取得

        _mint(_msgSender(), newTokenId);

        string memory jsonFile = string(abi.encodePacked('metadata', Strings.toString(newTokenId), '.json'));   // tokenIdに紐づくファイル名をセット
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://bafybeigyod7ldrnytkzrw45gw2tjksdct6qaxnsc7jdihegpnk2kskpt7a/";   // NFT.STORAGEからコピペ
    }
}
