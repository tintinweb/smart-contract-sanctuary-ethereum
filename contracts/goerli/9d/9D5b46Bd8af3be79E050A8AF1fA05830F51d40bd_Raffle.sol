...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    event RaffleEnter(address indexed player, uint256 timestamp);
    event WinnerPicked(address indexed player);
    event CheckUpkeepCall(address indexed keeper, uint timestamp, bytes upkeepData, bool upkeepNeeded);
    event TestEvent(address indexed player, bytes inputData);
    ...
    function enterRaffle() public payable {
        ...
        if (s_raffleBank > 0 && (address(this).balance + i_entranceFee) > s_raffleBank) {
            s_raffleState = RaffleState.DRAW_PENDING;
        }

        emit RaffleEnter(msg.sender, block.timestamp);
    }
    ...
    )
    {
        ...
        if (upkeepData.length > 0) {
            ...
            if (keccak256(upkeepData) == keccak256(s_raffleSecret)) {
                emit TestEvent(msg.sender, upkeepData);
                requestFromScheduler = true;
            }
        }

        upkeepNeeded = (isOpen && hasBalance && hasPlayers && (timePassed || bankCollected || protectionTimeReached || requestFromScheduler));
        emit CheckUpkeepCall(msg.sender, block.timestamp, upkeepData, upkeepNeeded);
...
//        console.log("Raffle.performUpkeep(..)");
        ...
        emit RequestRandomWords(i_gasLane, i_subscriptionId, REQUEST_CONFIRMATIONS, i_callbackGasLimit, NUM_WORDS);
    ...
    /** Setter Functions **/
    // TODO set check for owner
    function sendTestEvent(bytes memory inputData) public {
        emit TestEvent(msg.sender, inputData);
    }

    // TODO set check for owner and only at RaffleState.MAINTENANCE
    function setRaffleBank(uint256 bank) public {
    ...
    // TODO set check for owner and only at RaffleState.MAINTENANCE
    function setRaffleInterval(uint256 interval) public {
    ...
    // TODO set check for owner and only at RaffleState.MAINTENANCE
    function setRaffleSecret(bytes calldata inputData) public {
    ...
    // TODO set check for owner and only at RaffleState.MAINTENANCE
    function setPrizePct(uint8 prizePct) public {
        s_prizePct = prizePct;
    }

    // TODO set check for owner and only at RaffleState.MAINTENANCE
    function setProtectionInterval(uint256 interval) public {
