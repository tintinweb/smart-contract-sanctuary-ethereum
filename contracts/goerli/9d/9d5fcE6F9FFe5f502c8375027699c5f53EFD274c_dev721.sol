...
contract dev721 is ERC721A, Ownable {  
    ...
    constructor( ) ERC721A("dev721", "d721") {}

    function publicMint(uint256 quantity) external payable {
        if (!saleActive) revert SaleIsPaused();
        if (totalSupply() + quantity > MAX_SUPPLY) revert NotEnoughTokens();
        if (_userMintedPublic[msg.sender] + quantity <= maxMint) revert ExceedMaxMint();
    ...
    function presaleMint(uint256 quantity, bytes32[] calldata _proof) external payable {
        if (!preSaleActive) revert SaleIsPaused();
        ...
        if (totalSupply() + quantity > MAX_SUPPLY) revert NotEnoughTokens();
        if (_userMintedPresale[msg.sender] + quantity <= maxMint) revert ExceedMaxMint();
        if (msg.value != price * quantity) revert WrongValueSent();
