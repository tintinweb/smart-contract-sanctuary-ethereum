...
@external
def __init__(_name: String[64], _symbol: String[32], _main_pool: address, _main_deposit: address, _main_pool_coin_count: uint8, _main_lp_token: address):
    self.name = _name
    self.symbol = _symbol
...
def _swap(pool: address, i: int128, j: int128, from_token: address, to_token: address, is_underlying: bool, from_amount: uint256, min_amount: uint256, is_crypto_pool: bool) -> uint256:
    ...
    if is_crypto_pool:
        if CrvPool(pool).coins(convert(i, uint256)) == VETH:
...
@internal
def _withdraw(lp_token: address, _main_pool: address, i: int128, out_amount: uint256) -> uint256:
    _main_deposit: address = self.main_deposit
    if _main_deposit == IS_A_POOL_IN_DEPOSIT:
        self.safe_approve(lp_token, _main_pool, out_amount)
        return CrvAPool(_main_pool).remove_liquidity_one_coin(out_amount, i, 1, True)
    elif _main_deposit == ZERO_ADDRESS:
        self.safe_approve(lp_token, _main_pool, out_amount)
        return CrvPool(_main_pool).remove_liquidity_one_coin(out_amount, i, 1)
    elif _main_deposit == self.zap_deposit:
        self.safe_approve(lp_token, _main_deposit, out_amount)
        return CrvZapDeposit(_main_deposit).remove_liquidity_one_coin(_main_pool, out_amount, i, 1)
    else:
        self.safe_approve(lp_token, _main_deposit, out_amount)
        return CrvPool(_main_deposit).remove_liquidity_one_coin(out_amount, i, 1)
...
def withdraw(token_address: address, amount: uint256, i: int128, swap_route: DynArray[SwapRoute, MAX_SWAP]):
    ...
    out_amount = self._withdraw(lp_token, _main_pool, i, out_amount)
    for route in swap_route:
        if route.swap_pool != ZERO_ADDRESS:
...
@external
def update_pool(_out_token: address, old_i: int128, swap_route: DynArray[SwapRoute, MAX_SWAP], new_pool: address, new_deposit: address, new_i: int128, new_pool_type: uint8, new_pool_coin_count: uint8, new_lp_token: address):
    ...
    _main_pool: address = self.main_pool
    out_amount = self._withdraw(lp_token, _main_pool, old_i, out_amount)
    for route in swap_route:
        if route.swap_pool != ZERO_ADDRESS:
            out_amount = self._swap(route.swap_pool, route.i, route.j, out_token, route.j_token, route.is_underlying, out_amount, route.min_amount, route.is_crypto_pool)
            out_token = route.j_token
