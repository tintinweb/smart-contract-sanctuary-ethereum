...
contract Testies is ERC721A, Ownable, ReentrancyGuard {
    ...
    //Mint limits
    uint public FREE_MAX_MINTS = 1;
    uint public WL_MAX_MINTS = 2;
    uint public PUBLIC_MAX_MINTS = 10;

    mapping(address => uint256) freeMintsOnAddress;
    ...
    constructor() ERC721A("Testies", "Test") {
    }
     ...
     * Free mint
     */
    function freeMint(uint256 numberOfMints, bytes32[] calldata _merkleProof) external payable {
        
        require(WLSaleActive, "Whitelist sale must be active to mint"); 
        require(MerkleProof.verify(_merkleProof, freeMerkleRoot, keccak256(abi.encodePacked(msg.sender))), "Invalid proof - Caller not whitelisted");
        require(numberOfMints > 0, "Sender is trying to mint none");
        require(freeMintsOnAddress[msg.sender] + numberOfMints <= FREE_MAX_MINTS, "Sender is trying to mint more than allocated tokens");
        require(totalSupply() + numberOfMints <= maxSupply, "Mint would exceed max supply of mints");
        require(mintedFreeSupply + numberOfMints <= freeSupply, "Mint would exceed max free supply");
        require(msg.value >= numberOfMints * price, "Amount of ether is not enough");

        WLMintsOnAddress[msg.sender] += numberOfMints;
        mintedFreeSupply += numberOfMints;
     ...
     * Whitelist mint
     */
    function WLMint(uint256 numberOfMints, bytes32[] calldata _merkleProof) external payable {
        ...
        require(numberOfMints > 0, "Sender is trying to mint none");
        require(WLMintsOnAddress[msg.sender] + numberOfMints <= WL_MAX_MINTS, "Sender is trying to mint more than allocated tokens");
        require(totalSupply() + freeSupply + numberOfMints <= maxSupply, "Mint would exceed max supply of mints");
        require(msg.value >= numberOfMints * price, "Amount of ether is not enough");
    ...
    function setFreeMaxMints(uint256 newAddressMaxMints) external onlyOwner {
        FREE_MAX_MINTS = newAddressMaxMints;
    }

    function setWLMaxMints(uint256 newAddressMaxMints) external onlyOwner {
        WL_MAX_MINTS = newAddressMaxMints;
    }
    ...
    function setFreeMerkleRoot(bytes32 newMerkleRoot) external onlyOwner {
        freeMerkleRoot = newMerkleRoot;
    }
