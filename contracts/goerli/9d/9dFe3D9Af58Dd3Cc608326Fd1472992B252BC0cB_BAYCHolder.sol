...
contract BAYCHolder is AccessControl {
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");
    mapping(address => uint256) private holders;
    mapping(uint256 => address) private tokens;
    address public baycNFTAddress;
    bool public activated = false;

    constructor(address adminAddress, address operatorAddress, address baycAddress) {
        baycNFTAddress = baycAddress;
    ...
    function requestToClaim(uint256[] calldata tokenIds) external {
        ...
        for(uint i=0; i<tokenIds.length; i++) {
            ...
            require(IERC721(baycNFTAddress).ownerOf(tokenId) == _msgSender(), "Sender is not the owner of the token");
