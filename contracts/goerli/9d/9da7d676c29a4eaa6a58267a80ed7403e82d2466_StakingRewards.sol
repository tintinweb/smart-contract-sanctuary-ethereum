...
contract StakingRewards is Initializable {
  ...
  function stake(uint256 _amount) external {
    ...
    if (useBalRewardPool()) {
      stakeDepositTokenToBalRewardPool(_amount);
      emit BALRewardPool_Staked(msg.sender, _amount);
    }
  }

  /**
   * @dev Transfer previously staked balances to BAL_REWARD_POOL if applicable
   */
  function transferStakedBalanceToBalRewardPool() public {
    require(useBalRewardPool(), 'INVALID_BAL_REWARD_POOL');
    require(balances[msg.sender] > 0, 'INVALID_USER_BALANCE_TO_TRANSFER');

    uint256 transferAmount = balances[msg.sender].sub(IBALRewardPool(BAL_REWARD_POOL).balanceOf(msg.sender));
    require(transferAmount > 0, 'INVALID_AMOUNT');

    stakeDepositTokenToBalRewardPool(transferAmount);

    emit TransferredStakedBalanceToBalRewardPool(msg.sender, transferAmount);
  }
  ...
  function withdraw(uint256 _amount, bool _isClaimAllRewards) external returns (bool) {
    ...
    if (useBalRewardPool()) {
      require(DEPOSIT_TOKEN_ADDR != address(0), 'INVALID_DEPOSIT_TOKEN_ADDR');
      if(balances[msg.sender] != IBALRewardPool(BAL_REWARD_POOL).balanceOf(msg.sender)) {
    ...
    if (useBalRewardPool()) {
      withdrawDepositTokenToBalRewardPool(amountToWithdraw);
   ...
   * @param _amount amount to stake
   */
  function stakeDepositTokenToBalRewardPool(uint256 _amount) internal {
    require(useBalRewardPool(), 'INVALID_BAL_REWARD_POOL');
    require(DEPOSIT_TOKEN_ADDR != address(0), 'INVALID_DEPOSIT_TOKEN_ADDR');
   ...
   * @param _amount amount to withdraw
   */
  function withdrawDepositTokenToBalRewardPool(uint256 _amount) internal {
    require(useBalRewardPool(), 'INVALID_BAL_REWARD_POOL');
    require(DEPOSIT_TOKEN_ADDR != address(0), 'INVALID_DEPOSIT_TOKEN_ADDR');
