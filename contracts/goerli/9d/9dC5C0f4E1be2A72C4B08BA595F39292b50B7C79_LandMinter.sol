...
contract LandMinter is Minter {
    ...
    constructor(
        ...
        LandParcels land_,
        PioneerMinter pioneerMinter_,
        address payable walletAddr_
    ) Minter(contractControlList_, walletAddr_) {
        land = land_;
        pioneerMinter = pioneerMinter_;
    }
    ...
    function mintLand(
        ...
        walletAddr.transfer(nativePrice_);
    }
}
...
contract Minter {
    ...
    function setWallet(address payable wallet) external onlyAdmin {
        walletAddr = wallet;
    }
}
...
contract PioneerMinter is Minter {
    ...
    constructor(
        ContractControlList contractControlList,
        IPioneer activePioneer_,
        IPioneer founder_,
        uint16 maxMintPerWallet_,
        address payable walletAddr_
    ) Minter(contractControlList, walletAddr_) {
        activePioneer = activePioneer_;
        founder = founder_;
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            keccak256(
                                "mintActivePioneer(address to_, uint256 nativePrice_, uint256 pioneerId_, uint256 signatureDeadline_, bytes memory signature_)"
                            ),
                            to_,
                            nativePrice_,
                            pioneerId_,
    ...
    function beforeMintActivePioneer_(address to_, uint256 nativePrice_)
        internal
    {
        require(
