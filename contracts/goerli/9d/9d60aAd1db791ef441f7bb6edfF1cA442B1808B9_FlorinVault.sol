...
contract FlorinVault is ERC4626Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    using MathUpgradeable for uint256;

    IERC20MetadataUpgradeable public mdcToken;
    ...
    function initialize(FlorinToken florinToken_, IERC20MetadataUpgradeable mdcToken_) external initializer {
        __ERC20_init_unchained("Staked Florin", "stFLR");
        __Context_init_unchained();
        __Ownable_init_unchained();
        __Pausable_init_unchained();
        __ERC4626_init_unchained(IERC20MetadataUpgradeable(address(florinToken_)));
        _pause();
        mdcToken = mdcToken_;
    }
    ...
    //do not allow people to deposit manually. only via stake/unstake
    function maxDeposit(address) public view virtual override returns (uint256) {
        return 0;
    }

    /** @dev See {IERC4262-deposit}. */
    // do not allow asset owner to deposit their assets by using "deposit" => only through stake

    function deposit(uint256, address) public view virtual override returns (uint256) {
        return 0;
    }

    /** @dev See {IERC4262-withdraw}. */
    // do not allow share owner to withdraw assets by using "withdraw" => only through unstake
    function withdraw(
        uint256,
        address,
        address
    ...
    // do not allow share owner to redeem their shares against assets by using "redeem" => only through unstake
    function redeem(
        uint256,
        address,
        address
    ...
    // normal maxWithdraw checks the balance of the owner which in our case is the contract and NOT the staker!
    // Hence normal maxWithdraw returns always zero for the staker and overwriting it guarantees correct values
    function maxWithdraw(address stakingWallet) public view virtual override returns (uint256) {
        return previewRedeem(walletStakingStates[stakingWallet].stakedShares);
        //return _convertToAssets(walletStakingStates[stakingWallet].stakedShares, MathUpgradeable.Rounding.Down);
    }

    /** @dev See {IERC4262-maxRedeem}. */
    function maxRedeem(address stakingWallet) public view virtual override returns (uint256) {
        return walletStakingStates[stakingWallet].stakedShares;
    }

    function setMDCperFLRperBlock(uint256 _MDCperFLRperBlock) external onlyOwner {
        MDCperFLRperBlock = _MDCperFLRperBlock;
    }

    function stake(uint256 assets) public {
        require(assets > 0, "Assets must be greater zero");

        WalletStakingState storage walletStakingState = walletStakingStates[_msgSender()];
        updateOutstandingRewards(walletStakingState);

        uint256 shares = previewDeposit(assets);
        walletStakingState.stakedShares += shares;
        walletStakingState.stakedAssets += assets;

        _deposit(_msgSender(), address(this), assets, shares);
    }

    function unstake(uint256 requestedAssets) public {
        WalletStakingState storage walletStakingState = walletStakingStates[_msgSender()];

        uint256 requestedShares = previewWithdraw(requestedAssets);

        uint256 shares = MathUpgradeable.min(requestedShares, walletStakingState.stakedShares);

        require(shares > 0, "Shares must be greater zero");
        ...
        _withdraw(address(this), _msgSender(), address(this), assets, shares);
    }

    function updateOutstandingRewards(WalletStakingState storage walletStakingState) internal {
        if (walletStakingState.lastUpdate != 0) {
            walletStakingState.outstandingRewards = _calculateRewards(walletStakingState);
        }
        walletStakingState.lastUpdate = block.number;
    }

    function _calculateRewards(WalletStakingState memory walletStakingState) internal view returns (uint256) {
        if (walletStakingState.lastUpdate != 0) {
            return
                walletStakingState.outstandingRewards +
                walletStakingState.stakedAssets.mulDiv(MDCperFLRperBlock * (block.number - walletStakingState.lastUpdate), 10**mdcToken.decimals(), MathUpgradeable.Rounding.Down);
        }
        return 0;
    }
    ...
    function claim() public {
        WalletStakingState storage walletStakingState = walletStakingStates[_msgSender()];
        updateOutstandingRewards(walletStakingState);
        uint256 outstandingRewards = walletStakingState.outstandingRewards;
        require(outstandingRewards > 0, "No rewards to be claimed");
        walletStakingState.outstandingRewards = 0;
        SafeERC20Upgradeable.safeTransfer(mdcToken, _msgSender(), outstandingRewards);
    }
}
