...
contract Settlement {    
    ...
    function _settle(uint256 settlementUUID) private {
        ...
        for (uint index = 0; index < settlement.tokens.length; index++) {
            ...
            (operationOverflow , lockedCustodianBalances[settlement.debtor][settlement.tokens[index]]) = SafeMath.trySub(
                lockedCustodianBalances[settlement.debtor][settlement.tokens[index]], settlement.amounts[index]);
    ...
    function _lockFunds(uint256 settlementUUID) private {
        ...
        for (uint index = 0; index < settlement.tokens.length; index++) {
            bool operationOverflow = false;            
            (operationOverflow , custodianBalances[settlement.debtor][settlement.tokens[index]]) = SafeMath.trySub(
                custodianBalances[settlement.debtor][settlement.tokens[index]], settlement.amounts[index]);
            overflow = operationOverflow && overflow;
            (operationOverflow, lockedCustodianBalances[settlement.debtor][settlement.tokens[index]]) = SafeMath.tryAdd(
                lockedCustodianBalances[settlement.debtor][settlement.tokens[index]], settlement.amounts[index]);
            overflow = operationOverflow && overflow;
        }
    ...
    function getAuthorizationStatus(uint256 settlementId) public view returns (bool authorized){
        if (custodianSettlements[settlementId].authorized == true) return true;
        return false;
    }

    function getExistStatus(uint256 settlementId) public view returns (bool exists){
        if (custodianSettlements[settlementId].exists == true) return true;
        return false;
    }
