...
contract CheckPass is Ownable {
    ...
    address public CheckBirdsNft=0x8F2C8DC8a34f5a40eE7781A8d75F1cb0CA0AF083;
    address public CheckBirdsburn=0x0Ef85A33cd14F013C0392D15a45AC47409A43012;
    ...
    function burn(uint256[] memory tokenids) public payable callerIsUser{
        require(PublicMintStatus,"Error: Burn stage closed");
        require(tokenids.length > 0 && tokenids.length % 3 == 0, "Error: Wrong quantity");
        ...
        uint256  amount = tokenids.length / 3;
        burninfos[msg.sender] += amount;
    }

   function rareburn(uint256[] memory tokenids) public payable callerIsUser{
        require(PublicMintStatus,"Error: Burn stage closed");
        require(tokenids.length > 0, "Error: Wrong quantity");
        for (uint i = 0; i < tokenids.length; i++) {
        ...
        rareburninfos[msg.sender] += amount;
    }
