...
contract CrazyZooNFT is ERC721, ERC721Burnable, AccessControl {
    ...
    uint256 public mintFeeTokenPrice;
    ...
    event NewFeeTokenPrice(uint256 indexed newMintTokenPrice);
    event NewCID(uint256 indexed index, string indexed newCid);

    constructor() ERC721("Crazy Zoo NFT", "CZN") {
        ...
        if (hasRole(MINTER_ROLE, _msgSender())) {
            ...
            if (directMintEnabled) {
                if (chargeFeeOnMint) {
                    ...
                    uint256 feeAmount = (fee * mintFeeTokenPrice) / 10000;
                    transferFee(_msgSender(), feeCollector, feeAmount);
                    _safeMint(to, tokenId);
    ...
    function setMintFeeTokenPrice(uint256 _newPrice)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_newPrice != 0, "Price Can Not Be Zero");
        mintFeeTokenPrice = _newPrice;

        emit NewFeeTokenPrice(_newPrice);
    }

    function checkIdRange(uint256 _tokenId) internal view {
        require((_tokenId >= lemurMinId && _tokenId <= lemurMaxId) 
        || (_tokenId >= rhinoMinId && _tokenId <= rhinoMaxId) 
        || (_tokenId >= gorillaMinId && _tokenId <= gorillaMaxId), "Id Out Of Range");
    }
    ...
    function setCid(uint256 index, string memory _cid) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        returns (string memory)
    {
        ...
        return
            (bytes(baseURI).length > 0 && bytes(tokenCid).length > 0)
                ? string(
                    abi.encodePacked(baseURI_, tokenCid, "/", _tokenId.toString())
                )
                : "";
    }
