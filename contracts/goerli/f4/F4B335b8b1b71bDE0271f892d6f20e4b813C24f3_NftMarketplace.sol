...
contract NftMarketplace {
    ...
    modifier isOwner(address nftAddress, uint256 tokenId) {
        ...
        if (owner != msg.sender) revert NftMarketplace__NotOwner();
        _;
    }
    ...
    function listItem(
        ...
        uint256 _tokenId,
        uint256 _price
    ) external notListed(_nftAddress, _tokenId, msg.sender) isOwner(_nftAddress, _tokenId) {
        if (_price <= 0) revert NftMarketplace__PriceMustBeAboveZero();

        IERC721 nft = IERC721(_nftAddress);
        if (nft.getApproved(_tokenId) != address(this)) revert NftMarketplace__NotApprovedForMarketplace();

        s_listings[_nftAddress][_tokenId] = Listing(_price, msg.sender);
        emit ItemListed(msg.sender, _nftAddress, _tokenId, _price);
    }

    function buyItem(address _nftAddress, uint256 _tokenId) external payable isListed(_nftAddress, _tokenId) {
        Listing memory listedItem = s_listings[_nftAddress][_tokenId];
        if (msg.value < listedItem.price) revert NftMarketplace__PriceNotMet(_nftAddress, _tokenId, listedItem.price);
    ...
    function updateListing(
        ...
        console.log(
            "NftMarketplace__Contract: updateListing() called with params: _nftAddress: %s, _tokenId: %s, _newPrice: %s",
            _nftAddress,
            _tokenId,
            _newPrice
        );
