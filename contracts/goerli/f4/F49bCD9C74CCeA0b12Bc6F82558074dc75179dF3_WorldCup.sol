...
// import "@thirdweb-dev/contracts/base/ERC1155Base.sol"; // maybe one day
import "@openzeppelin/contracts/utils/Strings.sol";
// import "hardhat/console.sol";

import "./Serializer.sol";

contract WorldCup is ERC721Base {
    string baseURI;
    // tokenId -> bracket
    // 1        -> [ 1,2,3,4,...]
    ...
    // function mint(uint[] memory bracket) public payable {
    ...
    //     require(prices[bracketType] <= msg.value, "Ether value sent is not correct");
    ...
    function mint(uint[] memory bracket, uint count) public payable {
        require(saleIsActive, "Sale not active");
    ...
    function setResults(uint[] memory _results) public onlyOwner {
        // require(results.length == 31, "results are not correct length");
        results = _results;
    }

    // can't do 0 because it's valid.
    // can't do -1 because its uint
    ...
    //     // Check if the bracket matches the results
    ...
    // returns the amount in wei or whatever that you'd receive if you payout now 
    function miniPrize(uint[] memory bracket) public view returns(uint) {
        // need to check that the round type makes sense too 
        // like if you just do 0's in group prize you'll be elible
    ...
    function payout(uint tokenId) public {
        // require(msg.sender == ownerOf(tokenId), "Only token owner can call this method");

        uint[] memory bracket = brackets[tokenId];
        RoundType bracketType = inferType(bracket);

        require(currentRound() > bracketType, "We don't have any results yet");
        require(redeemed[tokenId] == false, "Already withdrew winnings");

        uint pool = prizePools[bracketType];
        uint winners = counts[Serializer.toStr(bracket)];

        uint amt = 0;
        bool deductFromPool = false;

        // Grand prize is shared across all winners in the pool
        if(grandPrize(bracket)) {
            amt = pool / winners;
            // do not deduct from pool
        } 
        // "Only group brackets are eligible for mini prizes"
        else if(bracketType == RoundType.GROUP) {
            // deduct multiples payout from the pool
            amt = miniPrize(bracket);
            deductFromPool = true;
        }
   
        require(amt > 0, "This bracket is not a winning bracket");
        // check that balance of contract is more than the expected payout 

        payable(ownerOf(tokenId)).transfer(amt); // todo: safe math? 
        if(deductFromPool) {
