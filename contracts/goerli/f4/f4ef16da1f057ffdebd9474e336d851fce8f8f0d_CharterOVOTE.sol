...
contract CharterOVOTE {
	...
	event EventVote(address publisher, uint256 id, uint256 nullifier,
			uint256 votevalue, uint64 weight);
	...
	function vote(uint256 id,
		...
		// NOTE: uncoment the next line before deploying, it is
		// commented just for the tests
		require(verifier.verifyProof(a, b, c, inputs), "zkProof vote could not be verified");

		// check that votevalue is 0 or 1 only
		require(votevalue==0 || votevalue==1, "invalid votevalue");

		process.result += votevalue * weight;

		emit EventVote(msg.sender, id, nullifier, votevalue, weight);
	}
}