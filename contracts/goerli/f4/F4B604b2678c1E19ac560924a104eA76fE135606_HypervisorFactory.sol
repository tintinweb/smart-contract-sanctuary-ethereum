    ...
    ReentrancyGuard
{
    ...
    function checkPrice() external view returns (uint256, uint256) {
        uint160 sqrtPrice = TickMath.getSqrtRatioAtTick(currentTick());
        uint256 price = FullMath.mulDiv(
            uint256(sqrtPrice).mul(uint256(sqrtPrice)),
            PRECISION,
        ...
        return (uint256(sqrtPrice), price);
    }
    ...
    function withdraw(
        ...
        // WithdrawVariable memory wVar;
        // wVar.sqrtPrice = TickMath.getSqrtRatioAtTick(currentTick());
        // wVar.price = FullMath.mulDiv(uint256(wVar.sqrtPrice).mul(uint256(wVar.sqrtPrice)), PRECISION, 2**(96 * 2));

        // wVar.data = abi.encode(to);
        // if(amount1 > amount0 ){
        //     // need to swap token1 to token0
        //     swapCalled = true;
        //     wVar.swapAmount = wVar.price * (amount1 - amount0 ) / (wVar.price + PRECISION );
        //     test(wVar.swapAmount, wVar.price);
        //     (wVar.amount0_swap, wVar.amount1_swap) = pool.swap(address(this), false, wVar.swapAmount.toInt256(), TickMath.MAX_SQRT_RATIO - 1, wVar.data);
        //     amount0 += uint256(-wVar.amount0_swap);
        //     amount1 -= uint256(wVar.amount1_swap);
        // }else if(amount1 < amount0){
        //     // need to swap token0 to token1
        //     swapCalled = true;
        //     wVar.swapAmount = PRECISION *(amount0 - amount1 ) / (wVar.price  + PRECISION );
        //     test(wVar.swapAmount, wVar.price);
        //     (wVar.amount0_swap, wVar.amount1_swap) = pool.swap(address(this), true, wVar.swapAmount.toInt256(), TickMath.MIN_SQRT_RATIO + 1, wVar.data);
        //     amount0 -= uint256(wVar.amount0_swap);
        //     amount1 += uint256(wVar.amount1_swap);
        // }
        // int256 swapAmount0;
        // int256 swapAmount1;
        if (amount0 != amount1) {
            (int256 swapAmount0, int256 swapAmount1) = _swapWhenImbalance(
                amount0,
                amount1
            );
            int256 currentAmount0 = int256(amount0) - swapAmount0;
            int256 currentAmount1 = int256(amount1) - swapAmount1;
            amount0 = currentAmount0 > 0 ? uint256(currentAmount0) : 0;
            amount1 = currentAmount1 > 0 ? uint256(currentAmount1) : 0;
        }
    ...
    function _swapWhenImbalance(
        uint256 amount0,
        uint256 amount1
    ) internal returns (int256 swapAmount0, int256 swapAmount1) {
        uint256 decimal0 = IERC20Metadata(address(token0)).decimals();
        uint256 decimal1 = IERC20Metadata(address(token1)).decimals();
        uint256 maxDecimal = decimal0 > decimal1 ? decimal0 : decimal1;
        uint160 sqrtPrice = TickMath.getSqrtRatioAtTick(currentTick());
        uint256 price = FullMath.mulDiv(
            uint256(sqrtPrice).mul(uint256(sqrtPrice)),
            PRECISION,
        ...
        if (decimal0 != decimal1) {
            amount0 = amount0 * 10 ** (maxDecimal - decimal0);
            amount1 = amount1 * 10 ** (maxDecimal - decimal1);
            // if (decimal0 > decimal1) {
        ...
        if (amount0 < amount1) {
            zeroForOne = false;
            swapAmount = (price * (amount1 - amount0)) / (price + PRECISION);
            swapAmount /= 10 ** (maxDecimal - decimal1);
        } else {
            zeroForOne = true;
            swapAmount =
                (PRECISION * (amount0 - amount1)) /
                (price + PRECISION);
            swapAmount /= 10 ** (maxDecimal - decimal0);
        }
        swapCalled = true;
    ...
    function currentPrice() public view returns (uint256) {
        ...
        return (uint256(sqrtPriceX96) ** 2 * PRECISION) / 2 ** 192;
    }
