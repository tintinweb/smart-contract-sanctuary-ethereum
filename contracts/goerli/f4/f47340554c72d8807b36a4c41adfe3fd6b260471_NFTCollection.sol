...
// File: interfaces/ISparkbloxContract.sol
...
// File: interfaces/INFTSalesPhase.sol
...
// File: interfaces/INFTCollection.sol
...
// File: prebuilts/NFTCollection.sol
    ...
    ERC721EnumerableUpgradeable
{
    ...
    function claim(
        ...
        uint256 activeConditionId = getActiveSalesPhaseId();
        ...
        bool toVerifyMaxQuantityPerTransaction = _proofMaxQuantityPerTransaction == 0 ||
            salesPhase.phases[activeConditionId].merkleRoot == bytes32(0);
        verifyClaim(
            activeConditionId,
            _msgSender(),
            _quantity,
        ...
        if (validMerkleProof && _proofMaxQuantityPerTransaction > 0) {
             ...
             */
            salesPhase.limitMerkleProofClaim[activeConditionId].set(merkleProofIndex);
        }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if (_resetClaimEligibility) {
            for (uint256 i = existingStartIndex; i < newStartIndex; i++) {
                ...
                delete salesPhase.limitMerkleProofClaim[i];
            }
        } else {
            if (existingPhaseCount > _phases.length) {
                for (uint256 i = _phases.length; i < existingPhaseCount; i++) {
                    ...
                    delete salesPhase.limitMerkleProofClaim[newStartIndex + i];
                }
            }
        }
    ...
    function transferClaimedTokens(
        ...
        salesPhase.phases[_conditionId].supplyClaimed += _quantityBeingClaimed;

        // if transfer claimed tokens is called when `to != msg.sender`, it'd use msg.sender's limits.
        // behavior would be similar to `msg.sender` mint for itself, then transfer to `_to`.
        salesPhase.limitLastClaimTimestamp[_conditionId][_msgSender()] = block.timestamp;
    ...
    function verifyClaim(
        ...
        SalesPhase memory currentClaimPhase = salesPhase.phases[_conditionId];

        require(_pricePerToken == currentClaimPhase.pricePerToken, "invalid currency or price.");
    ...
    function verifyClaimMerkleProof(
        ...
        if (currentClaimPhase.merkleRoot != bytes32(0)) {
            ...
            require(
                !salesPhase.limitMerkleProofClaim[_conditionId].get(uint256(uint160(_claimer))),
                "proof claimed."
            );
            require(
    ...
    function getActiveSalesPhaseId() public view returns (uint256) {
    ...
    {
        lastClaimTimestamp = salesPhase.limitLastClaimTimestamp[_conditionId][_claimer];

        unchecked {
            nextValidClaimTimestamp =
                lastClaimTimestamp +
                salesPhase.phases[_conditionId].waitTimeInSecondsBetweenClaims;
    ...
    function getSalesPhaseById(uint256 _conditionId) external view returns (SalesPhase memory condition) {
        condition = salesPhase.phases[_conditionId];
    }
