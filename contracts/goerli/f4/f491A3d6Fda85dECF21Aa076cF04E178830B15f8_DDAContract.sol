    ...
    */
    function donate(uint256 _to, address _currency, uint256 _amount) external notBlackRole payable {
        ...
        if (_currency == ETH_COMPARE_ADDRESS) {
            ...
            IERC20(OKAPI_ADDRESS).approve(SWAP_ROUTER_ADDRESS, buyAmount);
            path[0] = _currency;
            path[1] = OKAPI_ADDRESS;
            IUniswapV2Router02(SWAP_ROUTER_ADDRESS).swapExactTokensForTokens(buyAmount, 0, path, msg.sender, block.timestamp + 60);
        }
