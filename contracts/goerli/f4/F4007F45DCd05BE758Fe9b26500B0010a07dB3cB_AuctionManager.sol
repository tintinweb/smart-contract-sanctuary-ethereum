    ...
    ERC1155Holder
{
    ...
    function fulfillAuction(bytes32 auctionId) external override auctionIsLiveOnChain(auctionId) nonReentrant {
        ...
        require(auction.state == AuctionState.LIVE_ON_CHAIN, "Auction in incorrect state");
        ...
        {} catch {
            // if recipient isn't valid, send bid back to bidder and send nft to auction ownerxs
            _processLastHighestBid(highestBidderData, auction.currency);
    ...
    {
        IAuctionManager.EnglishAuction memory auction = _auctions[auctionId];
        require(auction.endTime == 0, "Reserve price met already");

        // send nft back to auction owner, out of escrow
        IERC721(auction.collection).safeTransferFrom(address(this), auction.owner, auction.tokenId);
     ...
     */
    function updateEndTime(bytes32 auctionId, uint256 newEndTime) external onlyAuctionOwner(auctionId) {
