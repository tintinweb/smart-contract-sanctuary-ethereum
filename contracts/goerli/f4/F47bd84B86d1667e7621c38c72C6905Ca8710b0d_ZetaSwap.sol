...
contract ZetaSwap  {
    ...
    constructor(address _zeta_mpi_address, address _zeta_token_address) {
        ZETA_MPI_ADDRESS = _zeta_mpi_address; 
        ZETA_TOKEN = _zeta_token_address;
    }

    function xcSwap(address _tokenIn, address _tokenOut, uint _amountIn, uint _amountOutMin, address _toAddress, string calldata _toChain, address _destContract) external {
        ...
        uint GAS_LIMIT = 200000;
        // bytes memory dest = abi.encodePacked(_destContract);
        // console.log("zeta output:%s", amounts[1]); 
        ...
        ZetaMPI(ZETA_MPI_ADDRESS).zetaMessageSend(msg.sender, _toChain, "", amounts[1], GAS_LIMIT, message, "");
