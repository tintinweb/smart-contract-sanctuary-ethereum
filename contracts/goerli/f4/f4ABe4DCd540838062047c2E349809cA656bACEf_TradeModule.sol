...
contract TradeModule is ModuleBase, ReentrancyGuard {
    ...
    function estimateTrade(
        ...
        public view  
         returns(TradeInfo memory,uint[] memory,uint,uint,address){
        ...
        //  uint256 protocolFeeTotal = getModuleFee(TRADE_MODULE_PROTOCOL_FEE_INDEX, result[1]);
         uint256 feePercentage = controller.getModuleFee(address(this), TRADE_MODULE_PROTOCOL_FEE_INDEX);
         uint256 protocolFeeTotal = result[1].preciseMul(feePercentage);
         result[1]-=protocolFeeTotal;
      
         return (tradeInfo,result,protocolFeeTotal,feePercentage,address(this));
    }
        ...
        internal
    {
        ...
        // 取得授权
        _tradeInfo.setToken.invokeApprove(_tradeInfo.sendToken, _tradeInfo.exchangeAdapter.getSpender(), _tradeInfo.totalSendQuantity );
        //    
        (address targetExchange, uint256 callValue,bytes memory methodData ) = _tradeInfo.exchangeAdapter.getTradeCalldata(
