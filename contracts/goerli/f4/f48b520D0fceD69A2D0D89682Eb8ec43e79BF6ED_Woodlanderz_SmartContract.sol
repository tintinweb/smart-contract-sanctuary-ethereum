...
// File contracts/Woodlanderz_SmartContract_3.sol


pragma solidity 0.8.13;
contract Woodlanderz_SmartContract is ERC721A, ERC721AQueryable, Ownable, ReentrancyGuard, ERC2981, DefaultOperatorFilterer {
    ...
    constructor() ERC721A("Test Market 6", "TM6") {
		
		_setDefaultRoyalty(owner(), 1000); //10%
	}
    ...
    function presaleMint(uint256 quantity, bytes memory signature) external payable callerIsUser {
        
		require(!isPaused, "Not Yet Active.");
		require(isPresaled, "Presaled mint is disabled.");
		require(recoverSigner(signature) == signerAddress, "Address is not allowlisted");
		require(((_numberMinted(msg.sender) + quantity) <= settings.presaleMaxCount), "Illegal count NFTs");
		require((totalSupply() + quantity) <= max_supply, "Total supply exceeded!");
		require(msg.value >= (settings.presalePrice * quantity), "Payment is below the price");
	...
	function setSettings(uint256 _presalePrice, 
						 uint256 _presaleMaxCount,  
						 uint256 _publicPrice, 
						 uint256 _publicMaxCount, 
						 uint256 _max_supply
						 ) external onlyOwner {
	
		settings.presalePrice = _presalePrice;
		settings.presaleMaxCount = _presaleMaxCount;
		settings.publicPrice = _publicPrice;
		settings.publicMaxCount = _publicMaxCount;
		max_supply = _max_supply;
	}
	
	function setSigners(address _signerAddress) external onlyOwner {
		
		signerAddress = _signerAddress;
	}

	function adminMint(uint256 quantity, address receiver) external onlyOwner {
