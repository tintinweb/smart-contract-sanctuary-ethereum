...
contract MultipleFrameFixed is Ownable, ERC1155Holder {
    ...
    event MultiOfferAccepted(address sender, uint256 lazyTokenId, Offer offer);
    ...
    function onAcceptOffer(address _collectionId, uint256 _tokenId, uint256 lazyTokenId, uint256 _offerId, uint256 _offerAmount, uint256 _pairId) public {
        require(_offerId <= offers[_collectionId][_tokenId].length && offers[_collectionId][_tokenId][_offerId].offerId == _offerId, "Could not find offer");
        Offer memory offer = offers[_collectionId][_tokenId][_offerId];
        require(offer.active, "Not Exist");
        ...
        require(governanceToken.balanceOf(offer.owner) >= offer.offerPrice.mul(_offerAmount), "The buyer's balance is insufficient.");
        ...
        emit MultiOfferAccepted(msg.sender, lazyTokenId, offer);
    }

    function onLazyAcceptOffer(LazyItem memory lazyItem, Offer[] memory _offers, uint256 _offerId, uint256 acceptAmount) external returns (uint256){
        ...
        onAcceptOffer(lazyItem.collection, tokenId, lazyItem.lazyTokenId, _offerId, acceptAmount, 1e8);
