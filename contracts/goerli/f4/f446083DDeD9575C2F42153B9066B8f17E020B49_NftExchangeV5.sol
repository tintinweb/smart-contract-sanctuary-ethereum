...
// File: contracts\NftExchangeV5.sol

// solium-disable-next-line linebreak-style
pragma solidity >=0.4.22 <0.9.0;
contract NftExchangeV5 is Initializable, ERC1155SupplyUpgradeable, EIP712Upgradeable {
    address owner;
     ...
     * @param isBuyerReferred is true if someone referred the buyer to make
     * this purchase. This param is only used for a primary market sale on a
     * fixed price item, and otherwise can be set to false.
     * @param referrerWalletAddress is the public address that will be credited
     * with any referral fees.
    ...
    function placeOrderV2(
        ...
        address seller,
        OrderType orderType,
        BuyerVoucherV2 calldata buyerVoucher,
        SellerVoucherV2 calldata sellerVoucher,
        ...
        if (
            ...
            _acceptOfferForSale(
                ...
                AcceptOfferForSaleParams(
                    ...
                    sellerVoucher.voucherId,
                    sellerVoucher.tokenId,
                    /* sellVoucherTotalTokenSupply= */
                    1,
                    /* sellVoucherQuantityForSale= */
                    1,
     ...
     * default values.
     * @param sellerVoucher is a voucher signed by the seller.
     * @param purchaseTokenQuantity is the quantity of NFT tokens to purchase.
     * This param is only used when orderType is OrderType.BUY_NOW, since a
     * buyer voucher is not provided to determine purchase quantity. For other
     * order types, this param can be set to a default value.
     * @param isBuyerReferred is true if someone referred the buyer to make
     * this purchase. This param is only used for a primary market sale on a
     * fixed price item, and otherwise can be set to false.
     * @param referrerWalletAddress is the public address that will be credited
    ...
    /**
     * @dev Verifies the signature for a given BuyerVoucherV2, returning the
     * address of the signer.
     *
     * Will revert if the signature is invalid.
     *
     * @param voucher A BuyerVoucherV2 describing an offer made by a buyer.
     */
    function _verify(BuyerVoucherV2 calldata voucher) internal view returns (address) {
    ...
    /**
     * @dev Verifies the signature for a given SellerVoucherV2, returning the
     * address of the signer.
     *
     * Will revert if the signature is invalid.
     *
     * @param voucher A SellerVoucherV2 describing an NFT for sale.
     */
    function _verify(SellerVoucherV2 calldata voucher) internal view returns (address) {
        bytes32 digest = _hash(voucher);
        return ECDSAUpgradeable.recover(digest, voucher.signature);
    }
    ...
    /**
     * @dev Returns a hash of the given BuyerVoucherV2, prepared using
     * EIP712 typed data hashing rules.
     *
     * @param voucher A BuyerVoucherV2 to hash.
     */
    function _hash(BuyerVoucherV2 calldata voucher) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            'BuyerVoucherV2(uint8 buyVoucherType,uint256 voucherId,uint256 offerVoucherId,uint256 tokenId,uint256 offerWei,address wethSmartContractAddress,bool hasEndDate,uint256 endDate)'
                        ),
    ...
    /**
     * @dev Returns a hash of the given SellerVoucherV2, prepared using
     * EIP712 typed data hashing rules.
     *
     * @param voucher A SellerVoucherV2 to hash.
     */
    function _hash(SellerVoucherV2 calldata voucher) internal view returns (bytes32) {
        bytes memory a = abi.encode(
            keccak256(
                'SellerVoucherV2(uint8 sellVoucherType,uint8 saleMarketType,uint256 voucherId,uint256 tokenId,bool hasStartDate,uint256 startDate,bool hasEndDate,uint256 endDate,address wethSmartContractAddress,uint256 minPriceWei,string uri,uint8 creatorFee,address creatorFeePayoutWalletAddress,bool isReferralEnabledByCreator,uint8 referralFee)'
            ),
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            'BuyerVoucherV3(uint8 buyVoucherType,uint256 voucherId,uint256 tokenId,uint256 quantityToPurchase,uint256 unitPriceWei,address wethSmartContractAddress,bool hasEndDate,uint256 endDate)'
                        ),
                        voucher.buyVoucherType,
                        voucher.voucherId,
                        voucher.tokenId,
    ...
    function _redeemVoucherForNftFixedPrice(
        ...
        if (totalSupply(params.tokenId) == 0) {
        ...
        bool isTokenMinted = totalSupply(params.tokenId) > 0;
        ...
        // Make sure that seller has enough tokens to satisfy purchase
        // quantity.
        require(
        ...
        uint256 weiToSeller = totalPriceWei - weiToExchange - weiToCreator - weiToReferrer;
        if (weiToSeller > 0) {
    ...
    function _acceptOfferForSale(
        ...
        // If the total token supply is 0, the token has not been minted yet.
        // Ensure that the token will be minted with a total supply > 0.
        if (totalSupply(params.buyVoucherTokenId) == 0) {
            require(saleMarketType == SaleMarketType.PRIMARY, MUST_BE_SALE_MARKET_TYPE_PRIMARY);
            require(
        ...
        bool isTokenMinted = totalSupply(params.buyVoucherTokenId) > 0;
        if (saleMarketType == SaleMarketType.PRIMARY && !isTokenMinted) {
        ...
        uint256 weiToSeller = totalPriceWei - weiToExchange - weiToCreator;
        if (weiToSeller > 0) {
     ...
     * BuyerVoucherV2 is applicable for OrderType.ACCEPT_BUY_NOW_OFFER when the
     * buyer makes an offer on a fixed price item. BuyVoucherV2 is also
     * applicable for OrderType.CONCLUDE_AUCTION when the buyer places a bid
     * on an NFT that is for sale via an auction.
     */
    struct BuyerVoucherV2 {
