...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///     the CAKE rewards to all the cakefi products.
    ...
    ///  @notice 0.4 cakes per block
    uint256 public constant MASTERCHEF_CAKE_PER_BLOCK = 4 * 1e17;
    ...
    /// only for CAKE distributions within CakeFi products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
    ...
    function pendingCake(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (getBlockNumber() > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getBlockNumber().sub(pool.lastRewardBlock);
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (_pid == 0) {
                totalcakeStaked = totalcakeStaked + _amount;
            }
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant {
        ...
        if (_pid == 0) {
            totalcakeStaked = totalcakeStaked - amount;
        }
    ...
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = CAKE.balanceOf(address(this)) - totalcakeStaked;
            if (balance < _amount) {
                _amount = balance;
            }
