...
contract KudasaiMinter is Ownable {
    mapping(address => mapping(uint256 => uint256)) public _kudasaiCounter;
    ...
    uint256 private _kantsuCounter;
    uint256 public proofRound;
    bytes32 public kudasaiListMerkleRoot;
    uint256 public mintCost = 0.01 ether;

    constructor(uint256 maxWalletPerToken_, address onchainKudasai_, address kudasaiNFT_, address hidden_, bytes32 kudasaiListMerkleRoot_) {
    ...
    modifier onlyKudasaiHolder(uint256[] memory _ids) {
        for (uint256 i = 0; i < _ids.length; i++) {
            require(claimed[_ids[i]] == false, "Already claimed");
    ...
    function kantsu(uint256 _quantity, bytes32 _code) external payable isNotContract {
        ...
        require(_kudasaiCounter[msg.sender][proofRound] + _quantity <= _maxWalletPerToken && _kantsuCounter < 10, "No More Kudasai");

        _kudasaiCounter[msg.sender][proofRound] += _quantity;
        _kantsuCounter++;
        Kudasai(_kudasaiNFT).mintKudasai(msg.sender, _quantity);
    }

    function holderClaim(uint256[] memory _ids, bool _burn) public payable onlyKudasaiHolder(_ids) isNotContract {
        require((!_burn && msg.value == mintCost * _ids.length) || (_burn && msg.value == 0), "Mint cost is insufficient");
    ...
    function kudasai(uint256 _quantity, bytes32[] calldata _proof) public payable validateKudasaiAddress(_proof) isNotContract {
        require(msg.value == mintCost * _quantity, "Mint cost is insufficient");
        require(_kudasaiCounter[msg.sender][proofRound] + _quantity <= _maxWalletPerToken, "No More Kudasai");

        _kudasaiCounter[msg.sender][proofRound] += _quantity;
        Kudasai(_kudasaiNFT).mintKudasai(msg.sender, _quantity);
    }

    function holderClaimAndKudasai(uint256 _quantity, bytes32[] calldata _proof, uint256[] memory _ids, bool _burn) public payable validateKudasaiAddress(_proof) isNotContract {
        require((!_burn && msg.value == mintCost * (_ids.length + _quantity)) || (_burn && msg.value == mintCost * _quantity), "Mint cost is insufficient");
        require(_kudasaiCounter[msg.sender][proofRound] + _quantity <= _maxWalletPerToken, "No More Kudasai");
