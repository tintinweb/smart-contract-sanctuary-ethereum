...
contract PLSDSock is ERC721A, Ownable, ReentrancyGuard {
  ...
  string private baseUri = ""; // baseUri MUST end in a slash

  // Metadata bits
  string private description =
    "Phase 1 Genesis PulseDoge sockDAO"
    " - "
    "post pulsechain launch & full mint will commence phase 2.  "
    "In the meantime, You can Trade your NFT, "
    "[redeem your sock](https://sockclaim.pulsedoge.exchange) or "
    "[burn for a pair](https://sockburn.pulsedoge.exchange). "
    "Visit [pulsedoge.exchange](https://pulsedoge.exchange) for active updates.";
  ...
  */
  function tokenURI(uint256 tokenId) public view virtual override returns(string memory) {
    ...
    return string(abi.encodePacked('data:application/json;base64,', Base64.encode(abi.encodePacked(
      '{',
        '"name": "', _name, ' #', Strings.toString(tokenId), '", ',
  ...
  */
  ...
  function setTokenUriDetails(
    ...
    bytes(_description).length != 0 ? description = _description : '';
  ...
  */
  ...
  function withdrawErc20s(address[] calldata _tokens) public onlyOwner {
    for (uint j = 0; j < _tokens.length; j++) {

      IERC20(_tokens[j]).transferFrom(
        address(this),
        owner(),
        IERC20(_tokens[j]).balanceOf(address(this))
      );

      require(IERC20(_tokens[j]).balanceOf(address(this)) == 0x0, "Transfer failed");
    }
  }

  function withdrawErc721s(address _token, uint256[] calldata _tokenIds) public onlyOwner {
    for (uint j = 0; j < _tokenIds.length; j++) {

      IERC721A(_token).transferFrom(
        address(this),
        owner(),
        _tokenIds[j]
      );

      require(IERC721A(_token).ownerOf(_tokenIds[j]) == owner(), "Transfer failed");
