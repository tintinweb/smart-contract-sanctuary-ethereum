...
// File: github/abdullahtilalcodup/nftContracts/FinalizedContract.sol
...
contract NFTA is ERC721A, Ownable {
    ...
    uint8 private nftPerAddressLimitFreeO =2; //Max Free mint for WL
    uint8 private nftPerAddressLimitO =7; //Max Limit per Wallet for WL
    uint8 private nftPerAddressLimitFreePublic =1; //Max Free mint for Public
    uint8 private nftPerAddressLimitPublic =5; //Max Limit per Wallet for Public
        ...
        returns (uint256)
    {
        ...
        if (maxFreeMintAmount > currentMintAmount) {
            ...
            if(((isWhitelist&&maxWhitelistFreeLimit+freeMintAvailable<=10)||(!isWhitelist&&maxFreeMintAmount+freeMintAvailable<=20))){
