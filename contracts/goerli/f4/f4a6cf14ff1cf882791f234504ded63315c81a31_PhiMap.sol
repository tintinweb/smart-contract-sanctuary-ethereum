...
contract PhiMap is AccessControlUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    function checkAllDepositStatus(string memory name) public view returns (DepositInfo[] memory) {
        ...
        for (uint256 i = 0; i < userObjectDeposit[name].length; i++) {
            ...
            DepositInfo memory tempItem = depositInfo[name][depositObjectInfo.contractAddress][depositObjectInfo.tokenId];
            deposits[i] = tempItem;
        }
        return deposits;
    }
    ...
    function writeLinkToObject(
        ...
        emit WriteLink(name, userObject[name][object_index].contractAddress,userObject[name][object_index].tokenId, link.title, link.url);
    }
...
interface IPhiRegistry {
    function ownerOfPhiland(string calldata name) external view returns (address);
}
