...
contract PresaleIDO is AccessControl, ReentrancyGuard {
    ...
    function finishCampaign(address _recipient) external {
        ...
        if (balanceUSDC > 0) {
            IERC20Metadata(USDC).safeTransfer(_recipient, balanceUSDC);
        }
        if (balanceUSDT > 0) {
            IERC20Metadata(USDT).safeTransfer(_recipient, balanceUSDT);
        }
        ...
        emit CampaignFinished(_recipient, totalBalance, actualCrepeRemaining);
    } 
}
