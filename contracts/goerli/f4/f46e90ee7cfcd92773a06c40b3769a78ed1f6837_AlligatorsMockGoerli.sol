...
contract Alligators is
   ...
   struct NFT_Anatomy {
    ...
    // =============================================================
    //COLLECTION SIZES
    uint constant public LIMITED_COMMON_SUPPLY = 10000;
    uint constant public LIMITED_JOKER_SUPPLY = 1000;

    uint16 public WL_LIMIT = 999;

    uint8 private PER_PUBLIC = 5;
    uint8 private PER_TX = 3;
    uint8 private PER_WL = 20;

    uint256 constant private MAX_CHANCE_VALUE = 1000;
    uint256 private MIN_CHANCE_VALUE = 888;
    ...
    mapping(uint256 => NFT_Anatomy) private _anatomy;
    mapping(uint256 => NFT_LEVEL) private _lvl;
   ...
   /**
    ...
    function whitelistMint(uint256 _quantity) public {  
        ...
        if (isWhitelisted[msg.sender] == 0) revert callErr();
    ...
    function alligatorMint(address to,uint256 _quantity, uint256 _requestId) private {
        ...
        if (AlligatorIds.current() > LIMITED_COMMON_SUPPLY) revert callErr();
    ...
    function jokerMint(address to, uint256 _requestId) private {
        ...
        if (JokerIds.current() > LIMITED_COMMON_SUPPLY) revert callErr();
    ...
    */
    ...
    function setMergeHub(address mergeHub_) public onlyOwner {
        if (mergeHub_ == address(0)) revert callErr();
        mergeHub = mergeHub_;
        emit mergeIsSet(mergeHub_);
    }

    function setCollectionSizeWL(uint16 size) public onlyOwner {
        if (size > 1200) revert callErr();
        WL_LIMIT = size;
        emit mintLimitIsSet(size);
    }

    function setPerPersonLimitWL(uint8 size) public onlyOwner {
        if (size > 50) revert callErr();
        PER_WL = size;
        emit mintLimitIsSet(size);
    }

    function setChanceArray(uint256 _min) public onlyOwner {
        if (_min > MAX_CHANCE_VALUE) revert callErr();
        MIN_CHANCE_VALUE = _min;
        emit chanceIsSet(_min);
    }

    function addWLAddress(address[] memory _whitelisted) public onlyOwner {
    ...
    function sort(uint8[7] memory data) internal returns(uint8[7] memory) {
        quickSort(data, int(0), int(data.length - 1));
        return data;
    }
    
    function quickSort(uint8[7] memory arr, int left, int right) internal{
        int i = left;
        int j = right;
        if(i==j) return;
    ...
    function JokerLevel(uint256 _tokenId) external view override returns (uint8) {
        return Jokerlvl[_tokenId];
    }

    function AlligatorLevel(uint256 _tokenId) external override returns (uint8) {
        if (_tokenId >= _currentIndex) revert callErr();
        if (tokenIdType[_tokenId] == 3) revert callErr();
        uint8[7] memory sorted = sort(_traitLevels(_tokenId));
        return sorted[6];
    }


    function traitLevels (uint256 _tokenId) external view override returns (uint8[7] memory) {
        return _traitLevels(_tokenId);
    }

    function _traitLevels(uint256 _tokenId) internal view returns (uint8[7] memory _traits) {
        if (_tokenId >= _currentIndex) revert callErr();
...
interface IAlligators is IERC721, IERC721Metadata {
    ...
    function JokerLevel(uint256 _tokenId) external view returns (uint8);
