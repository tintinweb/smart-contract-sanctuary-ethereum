...
contract Djin is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier onlyRuler() {
        require(msg.sender == ruler, "Not allowed to use this function");
        _;
    }
    ...
    function mint(uint256 amount_, bytes32[] calldata proof_) external payable nonReentrant {
        ...
        require(publicMints[msg.sender] + amount_ <= maxPerWallet, "You have minted max amount per wallet");
    ...
    function setMerkleRoot(bytes32 newRoot_) external onlyRuler {
        merkleRoot = newRoot_;
    }

    function togglePublic() external onlyRuler {
        publicPhase = !publicPhase;
    }

    function toggleWhitelist() external onlyRuler {
        whitelistPhase = !whitelistPhase;
    }

    function setBaseURI(string calldata newURI_) external onlyRuler {
        baseURI = newURI_;
    }

    function changeMintPrice(uint256 newPrice_) external onlyRuler {
        mintPrice = newPrice_;
    }

    function setRuler(address newRuler_) external onlyOwner {
        ruler = newRuler_;
    }
    ...
    function withdraw() external onlyRuler nonReentrant {
