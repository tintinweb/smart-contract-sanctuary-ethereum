    ...
    VRFV2WrapperConsumerBase
{
    ...
    mapping(uint16 buyback => mapping(uint16 domain => Buyback)) internal buybacks;
    ...
    function setTier(
        ...
        uint16 multiplier,      // scaling factor to increase the probability a ticket is selected in a random buyback
    ...
    function _requestRandomness(
        ...
        internal
    {
        uint256 price = getRequestPrice(gasLimit);
        if (LINK_TOKEN.balanceOf(address(this)) < price)
            revert LinkTokenBalanceTooLow();
        lastRequest = Request({
        ...
        internal
    {
        ...
        buybacks[lastBuyback][domain] = Buyback(lastBuyback, timestamp, request, random, size, index, ticket, records[ticket].holder, reward);
    ...
    {
        return (lastRequest.id, lastRequest.timestamp, lastRequest.random, lastRequest.price, lastRequest.status);
    }

    function getBuyback(uint16 domain)
        external view returns(
            uint16 id,
            uint32 timestamp,
            uint256 request,
    ...
    function getBuybacks(uint16 domain)
        external view returns(Buyback[] memory list, uint256 size)
    {
        list = new Buyback[](lastBuyback);
        for (uint16 i = 0; i < lastBuyback; i++) {
            list[i] = buybacks[i + 1][domain];
        }
        return (list, lastBuyback);
    }

    function getRequestPrice(uint32 gasLimit)
        public view returns(uint256)
    {
        return VRF_V2_WRAPPER.calculateRequestPrice(gasLimit);
    }
    ...
    ExenoCollectionHub
{
    ...
    {
        lastRequest = Request({
            id: 0, timestamp: 0, goal: goal, data: data, random: new uint256[](0), price: 0, status: false
        });
