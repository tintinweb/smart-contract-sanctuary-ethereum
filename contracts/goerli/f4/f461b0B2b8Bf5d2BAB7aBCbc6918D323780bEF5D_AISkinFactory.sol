...
contract AISkinFactory is AccessControl, EIP712, IAISkinFactory {
    ...
    bytes32 private constant _TOKENID_PERMIT_TYPEHASH =
        keccak256(
            "AISkin(address creator,uint256 tokenId,address tokenPayment,uint256 deadline)"
        );
    ...
    // aiskins share per item sale based on 10_000 points (250 == 2.5%)
    uint256 private _defaultShare = 250;
    ...
    function createCollection(
        ...
        if (_tokenPayment == address(0) && msg.value != _createCollectionFee) {
    ...
    function publishItem(
        ...
        if (
            collection != masterCollection && !_collections.contains(collection)
        ) {
        ...
        if (
            !_isFreePublish(tokenIdSigner) &&
            (_tokenPayment == address(0) && msg.value != _publishItemFee)
        ) {
            revert InsufficientPaymentBalance();
        }

        if (!_isFreePublish(tokenIdSigner) && _tokenPayment != address(0)) {
            require(
                IERC20(_tokenPayment).transferFrom(
                    _msgSender(),
                    _treasury,
        ...
        } else if (!_isFreePublish(tokenIdSigner)) {
            Address.sendValue(payable(_treasury), _publishItemFee);
        }

        if (collection == masterCollection) {
            IAISkinMasterCollection(masterCollection).createItem(
                tokenId,
                _msgSender(),
                cap,
                tokenPayment,
    ...
    function setMasterCollection(
        ...
        emit SetMasterCollection(masterCollection, _masterCollection);
    ...
    function updateDefaultShare(
        uint256 defaultShare
    ) external override onlyManager {
        emit UpdateDefaultShare(_defaultShare, defaultShare);
        _defaultShare = defaultShare;
    }
    ...
    function getDefaultShareFee() external view override returns (uint256) {
        return _defaultShare;
    }

    /**
     * @dev Create creator cloned collection
     */
    function _createCollection(
        ...
        collection.initialize(address(this), creator_, name_, symbol_);
    ...
    /**
     * @dev Calculate fee on collection amount sales
     */
    function _calculateCommission(
        ...
        uint256 share = _shareCollectionOverride[collection].isOverride
            ? _shareCollectionOverride[collection].share
            : _defaultShare;
