...
contract MiamiNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    event DurationSet(uint256 start);
    ...
    constructor(
        ...
        uint256[1] memory _pricesInWei,
        uint256[2] memory _limits,
        uint256 _start
    ) ERC721A(name, symbol) {

        _baseTokenURI = baseTokenURI;
        priceInWeiPublicSale = _pricesInWei[0];
    ...
    function setDurationValues(uint256 _start) external onlyOwner {
        start = _start;
        emit DurationSet(start);
    }
    ...
    // Private Functions
    function buyTokens(uint8 _amount) private nonReentrant {
