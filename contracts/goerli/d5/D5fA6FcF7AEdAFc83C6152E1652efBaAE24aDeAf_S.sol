...
contract S is Ownable, ERC721A, ERC721ABurnable, ReentrancyGuard {
    ...
    */
    function mint(bytes32[] memory _merkleProof, address to) external payable callerIsUser {
        require(mintActive, "Minting is not active");
        require(totalMinted[to] < 1, "You have already minted a token to this address");
        require(msg.value >= mintPrice, "Below mint price");

        bytes32 node = keccak256(abi.encodePacked(to));
        require(MerkleProof.verify(_merkleProof, merkleRoot, node), "You are not whitelisted");
    ...
    function mintDev(address to, uint256 amount) external onlyOwner {
        require(amount <= 200, "You can only mint 200 tokens at a time");
        require(totalMinted[to] + amount <= 200, "You are trying to mint more than 200 tokens to this address");
    ...
    // Returns the tokenURI for a specific token
    ...
    */
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(!isRevealed){
            return bytes(placeholderTokenUri).length > 0 ? string(abi.encodePacked(placeholderTokenUri, trueId.toString())) : "";
        }
