...
* @author Niftydude
*/
contract PunksComic4 is ERC721Enumerable, AccessControl {
    ...
    IPunksComic comic2Contract;
    IPunksComic comic3Contract;
    ...
    constructor(
        ...
        address _comic2Contract,
        address _comic3Contract
        ...
        comic2Contract = IPunksComic(_comic2Contract);
        comic3Contract = IPunksComic(_comic3Contract);
    ...
    function mint(
        uint256[] calldata tokenIds2,
        uint256[] calldata tokenIds3
    ) external {
        if(tokenIds2.length != tokenIds3.length) revert SameLengthRequired();
        if(block.timestamp < windowOpen || block.timestamp > windowClose) revert WindowClosed();

        for(uint256 i; i < tokenIds2.length;) {
            if(comic2Contract.ownerOf(tokenIds2[i]) != msg.sender ||
               comic3Contract.ownerOf(tokenIds3[i]) != msg.sender
            ) revert SenderNotOwner();

            comic2Contract.burn(tokenIds2[i]);
            comic3Contract.burn(tokenIds3[i]);
    ...
    */
    function ownerMint (address[] calldata _to, uint256[] calldata _amount) external onlyRole(USER_ROLE) {
    ...
    function irrevocablyCloseMinting() external onlyRole(USER_ROLE) {
