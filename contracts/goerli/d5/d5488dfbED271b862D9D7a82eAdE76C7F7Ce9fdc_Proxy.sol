...
contract Proxy is AccessControl, ReentrancyGuard, Structs {
    ...
    event RevealTimeSet(uint256 _revealTime);
    event TimestampSet(uint256 _timestamp);
    event CyrochambersSet(address _cryochambers);
    event ExoriansSet(address _exorians);
    ...
    constructor(
        ...
        require(_mintpassAddress != address(0) && _cryochambersAddress != address(0)  && _exoriansAddress != address(0),"Addresses cannot be address zero");
        require(_revealTime > 0 ,"Reveal time must be greater than zero");
    ...
    function addSalePhase(
        ...
        require(_phase.endTime > _phase.startTime, "Invalid time range");
        require(_phase.startPrice > 0, "Invalid start price");
    ...
    function setRevealTime(uint _newRevealTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit RevealTimeSet(_newRevealTime);
    }
    ...
    function withdrawFunds(address payable _wallet, uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(_wallet != address(0),"Cannot withdraw to zero address");
