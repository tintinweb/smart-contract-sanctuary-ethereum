...
contract NFTGiftClaim is Ownable, AccessControl {
  ...
  bytes32 public CONTRIBUTION_TYPEHASH =
    keccak256("Contribute(address user,uint256 deadline)");
  ...
  function claim(
    ...
    bytes32 digest = keccak256(
      abi.encodePacked(
        "\x19\x01",
        DOMAIN_SEPARATOR,
        keccak256(abi.encode(CONTRIBUTION_TYPEHASH, msg.sender, deadline))
      )
    );
    ...
    require(
      ...
      "NFTVillageWhitelist: Invalid Signature"
    );

    nft.safeTransferFrom(nftHolder, msg.sender, NFT_ID, 1, "");
  ...
  function setNFT(IERC1155 token) external onlyOwner {
    nft = token;
  }
