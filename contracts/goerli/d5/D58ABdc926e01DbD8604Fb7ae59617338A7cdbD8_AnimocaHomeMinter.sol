...
contract AnimocaHomeMinter is Context, Ownable {

  /// @dev Link to HIGHSTREET HOME ERC721 instance
  ...
  function mintBatch(uint256 startId, uint256 amount ) external onlyOwner {

    AnimocaHome home = AnimocaHome(NFT);
    require(home.minters(address(this)) == true, "not minter role");

    for (uint256 idx = 0; idx < amount; idx++) {
      home.safeMint(msg.sender, startId + idx);
    }
  }
}
