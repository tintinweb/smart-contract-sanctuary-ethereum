...
contract DeathClockDesciptor is Ownable, MetadataGenerator {
    ...
    string private PREVIEW_URL = '';
    uint256 TRANSFERABLE_CLOCKS;
    //TODO private
    mapping(uint8 => bytes32) public colorNames;
    mapping(uint8 => bytes32) public imageNames;
    ...
    constructor(
        string memory _WEBAPP_URL,
        string memory _PREVIEW_URL,
        uint256 _TRANSFERABLE_CLOCKS
    ) {
        WEBAPP_URL = _WEBAPP_URL;
        PREVIEW_URL = _PREVIEW_URL;
        TRANSFERABLE_CLOCKS = _TRANSFERABLE_CLOCKS;
    }
        ...
        returns (string memory)
    {
        ...
        bool isRemnant = tokenId >= TRANSFERABLE_CLOCKS;
        if (isRemnant) {
            previewId = 9999; //renmant
        }
        console.log('!!', tokenId, TRANSFERABLE_CLOCKS, isRemnant);
        return
...
contract MetadataGenerator {
    ...
    function _generateAttributes(
        ...
        if (payload.isRemnant) {
            _addStringAttribute(writer, 'Type', 'Remnant');
        } else {
            _addStringAttribute(writer, 'Type', 'Death Clock');
