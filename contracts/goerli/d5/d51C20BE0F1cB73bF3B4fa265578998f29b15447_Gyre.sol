...
contract Gyre is ERC721, ERC721Enumerable, ERC721URIStorage {
  using Counters for Counters.Counter;
  using Strings for uint256;
  using Strings for uint16;

  Counters.Counter private _tokenIdCounter;
  DTime public datetime;
  uint16 public currentYear;
  uint16 public currentSeconds;
  uint public newVar2;
  address ericcorrielstudios = 0x74c54593741Ae6c297341f03e5A54E31435D91BA;

  constructor(address _datetimeContractAddress) ERC721("Gyre", "GYR") {
    datetime = DTime(_datetimeContractAddress);
    currentYear = datetime.getYear(block.timestamp);
    currentSeconds = datetime.getSecond(block.timestamp);
    string memory blazz = "adff";
    newVar2 = 1;
  }
  ...
  function setFakeYear(uint16 _fakeYear) public {
    currentYear = _fakeYear;
  }

  function setActualYear() public {
    currentYear = datetime.getYear(block.timestamp);
  }
  ...
  function getTokenURI(uint256 tokenId) public view returns (string memory){
    ...
    bytes memory dataURI = abi.encodePacked(
      '{',
      '"name": "Gyre #', tokenIdString, '",',
      '"description": "For the entirety of 2020, Eric Corriel kept every piece of single-use plastic he consumed for a large-scale, physical sculpture, called Gyre. Each piece is documented as an NFT and will live on the blockchain as it slowly disintegrates over centuries",',
      '"image": "', _baseURI(), tokenIdString, '/', currentYear.toString(), '.png",',
      '"external_url": "https://gyre.xyz/items/', tokenIdString, '"'
    '}'
    );
  ...
  function getLastMintedTokenId() external returns (uint256){
  ...
  //    (bool sent, bytes memory data) = ericcorrielstudios.call{value : msg.value}("");
...
//Main contract address 0x1a6184CD4C5Bea62B0116de7962EE7315B7bcBce
