...
contract GachaDraw is Ownable {
        ...
        validActivity(_activityId)
    {
        ...
        if (activities[_activityId].remainingSupply < _ticketAmount) {
            revert InsufficientReward();
        }

        /// @dev Burns the tickets
        TicketContract.burn(activities[_activityId].activityType, msg.sender, _ticketAmount);
        totalCompleteDraw++;

        /// @dev For each tickets burns the tickets and mint a random reward
        for (uint256 i = 0; i < _ticketAmount; i++/*_uncheckedInc(i)*/) {
            uint256 randomIndex = getRandomNumber(activities[_activityId].remainingSupply);
            ...
            for (uint256 j = 0; j < activities[_activityId].rewardTokenIds.length; j++) {
                ...
                if (remaining > 0 && indexCount >= randomIndex) {
                    ...
                    unchecked { activities[_activityId].totalRewardSupply[j]++; }
                    break;
                }
            }
