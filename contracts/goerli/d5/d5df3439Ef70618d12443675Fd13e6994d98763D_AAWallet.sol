...
contract AAWallet {
    ...
    function executionFromEntryPoint(bytes memory callData) external view onlyEntryPoint returns(bytes32 destination, uint amount) {
        // unpacking callData.
        (destination, amount) = abi.decode(callData, (bytes32, uint));
