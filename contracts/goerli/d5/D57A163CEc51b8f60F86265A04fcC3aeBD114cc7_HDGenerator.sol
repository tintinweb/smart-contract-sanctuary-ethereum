...
contract HDGenerator is ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function setMaxSupply(uint96 _updatedSupply) external onlyOwner {
        maxSupply = _updatedSupply;
    }
    ...
    function airdropNerds(address[] calldata _receivers) external onlyOwner {
        uint96 updatedSupply = uint96(_receivers.length) + totalSupply;

        if (updatedSupply > maxSupply) revert NotEnoughTokensAvailable();

        uint96 tokenId = totalSupply + 1;
        for (uint96 i = 0; i < _receivers.length; i++) {
