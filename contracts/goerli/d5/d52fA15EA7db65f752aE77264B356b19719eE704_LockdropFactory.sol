...
contract AtlendisLockdrop is BaseLockdrop {
    ...
    function rescueRewards() internal pure override {
        require(true);
    }
...
abstract contract BaseLockdrop is Ownable {
    ...
    {
        return (currentLocks[tokenId].owner, currentLocks[tokenId].endDate);
    }
    ...
    function quit(uint256 tokenId) external isLockOwner(tokenId) {
        ...
        (uint256 endDate, ) = _withdraw(tokenId);
    ...
    function claim(uint256 tokenId) external isLockOwner(tokenId) {
        ...
        (uint256 endDate, uint256[] memory rewards) = _withdraw(tokenId);
        for (uint256 i = 0; i < tokens.length; i++) {
    ...
    function _withdraw(uint256 tokenId)
        internal
        returns (uint256 endDate, uint256[] memory rewards)
    {
     ...
     **/
    function rescueRewards() internal virtual;
