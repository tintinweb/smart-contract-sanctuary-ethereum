...
contract MarketplaceAuction is ERC721Holder, Ownable, Pausable {
    ...
    event MakeOrder(uint256 id, bytes32 indexed hash, address seller, uint256 startPrice, uint256 endTime);
    event CancelOrder(uint256 id, bytes32 indexed hash, address seller);
    event Bid(uint256 id, bytes32 indexed hash, address bidder, uint256 bidPrice);
    ...
    function addAuction(
        ...
        _makeOrder(_tokenId, _startPrice, _endTime);
      }
  ...
  function bid(bytes32 _order) external payable {
    ...
    require(o.seller != msg.sender, "Can not bid to your nft");

    if (lastBidPrice != 0) {
      require(msg.value > lastBidPrice, "low price bid than before");
    } else {
    ...
    orderInfo[_order].lastBidPrice = msg.value;

    if (lastBidPrice != 0) {
      (bool sentSuccess, ) = payable(lastBidder).call{value: lastBidPrice}("");
      require(sentSuccess, "failed to send last bidder");
      emit Refund(lastBidder, lastBidPrice);
    }
    emit Bid(o.tokenId, _order, msg.sender, msg.value);
  }
  ...
  function claim(bytes32 _order) external {
    ...
    (bool sentPrice, ) = payable(seller).call{value: lastBidPrice - royaltiesAmount}("");
    require(sentPrice, "failed to send price to seller");

    (bool sentroyal, ) = payable(royaltiesReceiver).call{value: royaltiesAmount}("");
    require(sentroyal, "failed to send royalty");
