...
contract FaucetERC721 is Ownable, ERC721URIStorage, ERC721Burnable {
    ...
    function mintWithURI(string calldata uri) external {
...
contract Vault is Ownable {
    ...
    event Deposit(address asset, address from, uint256 tokenId);
    event Mint(address asset, address to, uint256 tokenId);
    event Burn(address asset, address from, uint256 tokenId);
    event Unlock(address asset, address to, uint256 tokenId);
    ...
    function deposit(address nftAddress, uint256 tokenId) external {
        ...
        emit Deposit(nftAddress, _msgSender(), tokenId);
    }

// CHAIN 2 FUNCS START
    // @note called by this contract to mint on chain2 for the user
    function mint(address syntheticNFT, address user, string calldata uri) external onlyOwner {
        ...
        emit Mint(syntheticNFT, user, tokenIndex);
    }

    // @note called by operator ON chain2 once user submits request
    function burn(address syntheticNFT, uint256 tokenId) external onlyOwner {
        ...
        emit Burn(syntheticNFT, from, tokenId);
    }
// CHAIN 2 FUNCS END

    // @note called by operator to transfer the NFT back to the user ON chain1
    function unlock(address nftAddress, address to, uint256 tokenId) external onlyOwner {
        FaucetERC721(nftAddress).transferFrom(address(this), to, tokenId);
        emit Unlock(nftAddress, to, tokenId);
    }
