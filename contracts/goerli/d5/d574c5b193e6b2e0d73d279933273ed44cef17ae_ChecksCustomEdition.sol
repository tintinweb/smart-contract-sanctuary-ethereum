...
contract ChecksCustomEdition is
    ERC721A,
    Ownable,
    Pausable,
    DefaultOperatorFilterer
{
    ...
    constructor() ERC721A("Checks - Custom Edition", "CCE") {
        _pause();
    }
    ...
    function mint(uint256 qty, string calldata colors)
        external
        payable
        whenNotPaused
    {
        if (msg.value < price) revert InvalidPrice();
        if (qty > maxPerMint) revert InvalidQuantity();
        require(balanceOf(msg.sender) + qty <= maxPerWallet, "MAX_PER_WALLET");

        _safeMint(msg.sender, qty);

        uint256 nextTokenId = _nextTokenId();
        for (uint256 i; i < qty; ++i) {
            tokenColor[nextTokenId] = colors;
        }
    }
