...
import "./Standards/PaymentSplitter.sol";

/**
 * @title Base721A
 * @notice Base 721 contract
 **/
contract Base721A is ERC721A, Pausable, PaymentSplitter {
    ...
    modifier notLocked() {
        require(!locked, "Base721A: Metadata URIs are locked");
        _;
    }

    /// @notice Art not revealed modifier
    modifier notRevealed() {
        require(!revealed, "Base721A: Art is already revealed");
        _;
    }

    /// @notice Art revealed modifier
    modifier Revealed() {
        require(revealed, "Base721A: Art is not revealed");
        _;
    }

    /// @notice only Owner modifier
    modifier onlyOwner() {
        require(msg.sender == owner, "Base721A: only owner");
        _;
    }
     ...
     * @param data data for 721AProxy clone encoded
     * @param owner_ address of 721AProxy owner
     **/
        ...
        initializer
    {
        require(
            isBase == false,
            "Base721A: this is the base contract,cannot initialize"
        );

        require(owner == address(0), "Base721A: contract already initialized");

        require(owner_ != address(0), "Base721A: Owner address cannot be 0");
    ...
    receive() external payable override {
        revert("Base721A: Please use Mint or Admin calls");
    }
    ...
    function adminMint(address to, uint256 quantity)
        ...
        onlyOwner
    {
        require(to != address(0), "Base721A: Address cannot be 0");
    ...
    function mint(
        ...
        require(
            ...
            "Base721A: Ether sent mismatch with mint price"
        );
        ...
        require(
            ...
            "Base721A: max NFT mints per address exceeded"
        );
    ...
    function setPhase(
        ...
        require(mintCost > 0, "Base721A: rate is 0");
    ...
    function changeMintCost(uint256 mintCost) public onlyOwner {
        require(
            ...
            "Base721A: mint Cost cannot be same as previous"
        );
        ...
        notLocked
    {
        require(
            ...
            "Base721A: Base URI cannot be same as previous"
        );
    ...
    function changeMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        require(
            ...
            "Base721A: Merkle root cannot be same as previous"
        );
    ...
    function limitNotExceeded(uint256 quantity) public view returns (bool) {
        require(
            ...
            "Base721A: max NFT limit exceeded"
        );
        return true;
    }
        ...
        returns (bool)
    {
        if (_phase == Phase.Presale) {
            require(
                ...
                "Base721A: Caller is not whitelisted for Presale"
            );
        }
        return true;
    }
    ...
    function _beforeTokenTransfers(
        ...
        require(!paused(), "Base721A: token transfer while paused");
    }
}
...
contract PaymentSplitter is Context, Initializable {
     ...
     */
    function getPayees() view public returns(address[] memory) {
        return _payees;
    }
