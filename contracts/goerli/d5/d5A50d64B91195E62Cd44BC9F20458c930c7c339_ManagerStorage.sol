...
contract Manager is Pausable, NonReentrant, ManagerAutoProxy, IManager {
    ...
    function rewardPerTokenPerSecondApplicable(address farmingProxy) public view returns (uint256) {
        ...
        uint256 numerator = farmingStorage.getAllocPoints();
        uint256 denominator;
        uint256 length = _storage.getAllFarmingProxiesCount();
        IFarmingStorage farmingStorageIter;
        for (uint256 index = 0; index < length; ++index) {
            farmingStorageIter = IFarmingStorage(_storage.getFarmingStorageByIndex(index));
            denominator +=
                farmingStorageIter.getStakedTokenAmount() *
                farmingStorageIter.getAllocPoints();
        }
    ...
    function stakeBatch(address[] calldata farmingProxies, uint256[] calldata amounts) external {
        require(farmingProxies.length == amounts.length, 'Error in lengths of arrays');
        for (uint256 i = 0; i < farmingProxies.length; i++) {
            _stake(farmingProxies[i], amounts[i], msg.sender);
        }
    }

    // This function can be used to check how many LP tokens can be already withdrawn from a specific `farmingProxy` by
    // a specific staker `account`.
    // If checkIfUnlocked is set to true the `availableToWithdraw` function will only count unlocked parcels while when
    // it is set to `false` the function will count any parcel (locked or unlocked).
    // `checkIfUnlocked` is always `true` when a user calls this function via `exit`/`withdraw` but it can be set to
    // `false` when the owner of the contract wants to expel a staker from the pool via the `returnLpTokens` function.
    // `offset` should be set to `0` when you want to start counting from the `oldestLockingParcel` that isn't yet withdrawn
    // `limit` is the maximum number of locking parcels that should be checked
    // The `for` loop will break either if the `limit` number of locking parcel has been checked, or if the specified
    // `amount` available to withdraw has been reached
    // Set `amount` and `limit` to the maximum `uint256` value to get the maximum available amount of LP tokens that can
    // be withdrawn.
    // If the staker has a significant number of locking parcels, use `offset` and `limit` for pagination.
    // The function returns the reason why it breaks:
    // 0 = The requested amount of unlocked LP tokens is reached.
    // 1 = Limit is reached (the max amount of loops is reached in the while loop).
    // 2 = LockingParcels have not been created yet.
    // 3 = LockingParcels are still locked.
        ...
        )
    {
        ...
        while (true) {
            ...
            if (farmingStorage.getLockingParcelLockTime(account, index) == 0) {
                breakReason = 2;
                break;
            }
            ...
            if (
                ...
                farmingStorage.getLockingParcelLockTime(account, index) > lockTime
            ) {
                breakReason = 3;
                break;
            }
        ...
        return (availableAmount, index, breakReason);
    }
    ...
    function _withdraw(
        ...
        (
            ...
            uint256 breakReason
        ) = availableToWithdraw(farmingProxy, amount, account, checkIfUnlocked, 0, limit);

        if (breakReason == 1) {
            revert('Limit too low');
        }
        if (breakReason == 2) {
            revert('Amount too high');
        }
        if (breakReason == 3) {
            revert('LP tokens are still locked');
        }
    ...
    function withdrawBatch(
        ...
        require(
            farmingProxies.length == amounts.length && farmingProxies.length == limits.length,
            'Error in lengths of arrays'
        );
        for (uint256 i = 0; i < farmingProxies.length; i++) {
            _withdraw(farmingProxies[i], amounts[i], msg.sender, true, limits[i]);
        }
    }
    ...
    // This batch function can be called by a smart contract as well as
    // an externally owned address.
    function exitBatch(address[] calldata farmingProxies, uint256[] calldata limits) external {
        require(farmingProxies.length == limits.length, 'Error in lengths of arrays');
        for (uint256 i = 0; i < farmingProxies.length; i++) {
            _exit(farmingProxies[i], msg.sender, limits[i]);
        }
    }
    ...
    function getTokens(address farmingProxy) external view returns (address, address) {
        IFarmingStorage farmingStorage = IFarmingStorage(_storage.getFarmingStorage(farmingProxy));
        return (
            IFarmingStorage(farmingStorage).getToken0(),
            IFarmingStorage(farmingStorage).getToken1()
        );
    }
