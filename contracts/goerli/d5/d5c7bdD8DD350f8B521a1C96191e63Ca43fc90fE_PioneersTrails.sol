...
contract PioneersTrails is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    function getTrailIdByStage(uint8 _baseTrailId, bool debug)
        public
        view
        returns (uint8 _updatedTrailId)
    {
        if (debug) {
            return _baseTrailId;
        } else {
    ...
    function getEncodedInconveniencesByTrail(uint8 _baseTrailId)
        public
        view
        returns (bytes12)
    {
    ...
    function getDecodedInconveniencesByTrail(uint8 _baseTrailId)
        public
        view
    ...
    function getRandomNumber(bool _isDebug)
        public
        view
        returns (uint256 _rand)
    {
        if (!_isDebug) {
    ...
    function removeByteFromBytes16(bytes16 data, uint256 index)
        public
        pure
        returns (bytes16)
    {
    ...
    function currentInconveniences(uint8 _baseTrailId)
        public
        view
        returns (
        ...
        )
    {
        ...
        string memory pickedLetter = string(
            abi.encodePacked(getAvailableLettersFromWagons()[rand])
        );
    ...
    function getAvailableLettersFromWagons()
        public
        view
        returns (bytes memory letters)
    {
        bytes memory currentLetters = new bytes(25);
        uint256 length = IPioneers(pioneers).getTokenCount();
        for (uint256 i = 0; i < length; i++) {
            bool found = false;
            bytes1 letter = IPioneers(pioneers).wagonToLetter(uint16(i));
            if (letter != 0x00) {
                for (uint256 j = 0; j < i; j++)
                    if (letter == currentLetters[j]) {
                        found = true;
                        break;
                    }
                if (!found) {
    ...
    {
        require(
            IPioneers(pioneers).stage() != IPioneers.Stage.Initial &&
    ...
    function generateRandomNumbers() public {
