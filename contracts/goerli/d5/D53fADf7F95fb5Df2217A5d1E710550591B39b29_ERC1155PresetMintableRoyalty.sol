...
contract ERC1155PresetMintableRoyalty is ERC1155MintableRoyalty {
    ...
    constructor(
        ...
        address signer,
        uint96 feeInBeeps
    ) ERC1155(_uri) ERC1155MintableRoyalty(_name, _symbol, _version) {
        uri_ = _uri;
    ...
    function setUri(string memory _uri) external onlySigner {
        uri_ = _uri;
    }
    ...
    ERC1155Signature
{
    ...
    function mint(
        ...
        require(
            hasRole(
                SIGNER_ERC1155_ROLE,
                _getSigner(supply, fee, nonce, msg.sender, v, r, s)
            ),
        ...
        emit Mint(msg.sender, id, fee, supply);
    }
...
contract ERC1155Signature {
    ...
    bytes32 constant SIGNDATA_TYPEHASH =
        keccak256("SignData(uint256 supply,uint256 nonce,address creator)");

    bytes32 constant SIGNDATAWITHFEE_TYPEHASH =
        keccak256(
            "SignDataWithFee(uint256 supply,uint256 nonce,uint96 fee,address creator)"
        );
    ...
    function _getSigner(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                _hash(
                    SignData({supply: supply, nonce: nonce, creator: creator})
                )
            )
        );
