...
contract Wisdomatic is ERC721PsiAddressData, ERC2981Base, AccessControl {
  constructor(
  ...
  uint256 public _maxSupply = 2; // Maximum number of NFTs available for sale
  uint256 public _maxMintPerTx = 2; // Maximum number of mints per transaction
  ...
  function presaleMint(
    ...
    uint256 quantity
  ) external noContractCaller isNotPaused mintSupplyCompliant(quantity) {
  ...
  function mintOnBehalf(
    uint256 quantity,
    address beneficiary
  ) public mintSupplyCompliant(quantity) onlyAdmin {
  ...
  // Private functions
  function numberMinted(address user) private view returns (uint256) {
