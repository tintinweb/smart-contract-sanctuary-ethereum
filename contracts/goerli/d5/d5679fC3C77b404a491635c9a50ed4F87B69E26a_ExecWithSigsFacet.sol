...
contract ExecWithSigsFacet is ExecWithSigsBase, BFacetOwner {
        ...
        returns (uint256 estimatedGasUsed)
    {
        ...
        _requireSignerDeadline(
            _call.msg.deadline,
            "ExecWithSigsFacet.execWithSigsChecks:"
        );
        ...
        returns (uint256 estimatedGasUsed, uint256 observedFee)
    {
        ...
        _requireSignerDeadline(
            _call.msg.deadline,
            "ExecWithSigsFacet.execWithSigsFeeCollectorChecks:"
        );
        ...
        returns (uint256 estimatedGasUsed, uint256 observedFee)
    {
        ...
        _requireSignerDeadline(
            _call.msg.deadline,
            "ExecWithSigsFacet.execWithSigsRelayContextChecks:"
        );
    ...
    function simulateExecWithSigsRelayContext(
        ...
        (bool success, ) = _service.call(
            _encodeGelatoRelayContext(_data, feeCollector, _feeToken, _fee)
        );
...
import {_wasSignatureUsedAlready} from "../storage/ExecWithSigsStorage.sol";
import {
    _isExecutorSigner,
    _isCheckerSigner
...
abstract contract ExecWithSigsBase {
    ...
    function _requireExecutorSignerSignature(
        ...
        string memory _errorTrace
    ) internal view returns (address executorSigner) {
        require(
            !_wasSignatureUsedAlready(_digest),
    ...
    function _requireCheckerSignerSignature(
        ...
        string memory _errorTrace
    ) internal view returns (address checkerSigner) {
        require(
            !_wasSignatureUsedAlready(_digest),
            _errorTrace.suffix("replay")
        );
...
function _wasSignatureUsedAlready(bytes32 _digest) view returns (bool) {
    return _execWithSigsStorage().wasSignatureUsedAlready[_digest];
}

function _setWasSignatureUsedAlready(bytes32 _digest) {
    _execWithSigsStorage().wasSignatureUsedAlready[_digest] = true;
}
