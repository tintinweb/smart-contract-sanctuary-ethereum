...
contract UTPhysicalRFactory is Ownable, BlackHolePrevention {
    ...
    function addTrait(
        ...
        require(crAddr != address(0), "UTPhysicalRFactory: Invalid community ID");
        ...
        require(msg.sender == communityAdmin,"UTPhysicalRFactory: Only community admin may add traits");
        ...
        require(address(traitRegistry) != address(0), "UTPhysicalRFactory: Trait Registry not installed");
        ...
        // NFT must exist!
        require(ERC721 != address(0), "UTPhysicalRFactory: Token not installed");

        // Deploy _controller_ ONLY if not yet deployed for this traitType
        UTPhysicalRedeemController controllerCtr;
        if(traitRegistry.getDefaultTraitControllerByType(traitType) == address(0)) {
            ...
            controllerCtr = UTPhysicalRedeemController(traitRegistry.getDefaultTraitControllerByType(traitType));
        }
        ...
        traits[0] = ECRegistryV3c.traitStruct(traitId, traitType, _inputTrait.start, _inputTrait.end, address(storageCtr), _inputTrait.ipfsHash, _inputTrait.name);
...
contract UTPhysicalRedeemStorage {
    ...
    function _init(address _registry, uint16 _traitId) internal {
        require( !initialised, "UTPhysicalRedeemStorage: Already initialized");
        initialised = true;
    ...
    modifier onlyAllowed() {
        require(ECRegistry.addressCanModifyTrait(msg.sender, traitId), "UTPhysicalRedeemStorage: Not Authorised" );
        _;
    }
}
 ...
 *
 *    This contract is used for changing trait values in a PhysicalRedeemStorage contract
 *
 */
...
contract UTPhysicalRedeemController {
    ...
    constructor (address _erc721, address _registry, uint256 _startTime, uint256 _endTime) {
        _init(_erc721, _registry, _startTime, _endTime);
    }

    
    function init(address _erc721, address _registry, uint256 _startTime, uint256 _endTime) external {
        _init(_erc721, _registry, _startTime, _endTime);
    }

    function _init(address _erc721, address _registry, uint256 _startTime, uint256 _endTime) internal {
    ...
    function setValue(uint16[] memory tokenId, uint8 value, address traitStorage) public {
        require(!locked, "Physical Redeem Controller: contract locked.");
        require(tokenId.length > 0, "Physical Redeem Controller: at least 1 token.");
        require(getTimestamp() > startTime, "Physical Redeem Controller: before start time.");
        require(getTimestamp() < endTime, "Physical Redeem Controller: after end time.");
        ...
        for(uint8 i = 0; i < tokenId.length; i++) {
            ...
            if(currentValue == 1 && value == 2) {
                ...
                require(erc721.ownerOf(currentTokenId) == msg.sender, "Physical Redeem Controller: not owner of token.");
    ...
    */
    ...
    function toggleLock() public {
        require(registry.getContractControllerContains(msg.sender), "PhysicalRedeemController: Not authorised");
        locked = !locked;
    }
}
...
// ---- Change end ----
    ...
    */
    ...
    function _init(uint32  _communityId, address _owner) internal {
