...
contract NFTcontract is Ownable, ERC721A, ERC721AQueryable, ReentrancyGuard {
    ...
    constructor(
        ...
        uint _MAX_SUPPLY,
        string memory _theUnRevealedURI,
        uint _MAX_FREE,
        ...
        baseURI = _theBaseURI;
        UnRevealedURI = _theUnRevealedURI;
        Name = _name;
        Symbol = _Symbol;
    ...
    function UpdateMAX_WhiteList(uint _MAX_WhiteList) external onlyOwner {
        MAX_WhiteList = _MAX_WhiteList;
    }

    function UpdatepublicSalePrice(uint _publicSalePrice) external onlyOwner {
        publicSalePrice = _publicSalePrice;
    }

    function UpdateMAX_FREE(uint _MAX_FREE) external onlyOwner {
        MAX_FREE = _MAX_FREE;
    }

    function UpdateMAX_FREE_PERWALLET(
        uint _MAX_FREE_PERWALLET
    ) external onlyOwner {
        MAX_FREE_PERWALLET = _MAX_FREE_PERWALLET;
    }
    ...
    function freeMint(
        ...
        require(
            amountNFTperWalletFreeMint[msg.sender] + _quantity <=
                MAX_FREE_PERWALLET,
            "You can't get more NFTs on the FreeMint"
        );
        require(
            totalSupply() + _quantity <= MAX_FREE,
            "There's no more supply for the Free Mint"
        );
