    ...
    SignatureVerifier
{
    ...
    function claimCampaign(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            require(
                claimed[msg.sender] + amounts[i] <= maxAllowce,
                "ShinikiCampaign: can not claimed greater than allowce"
            );
            claimed[msg.sender] += amounts[i];
            for (uint256 k = 0; k < amounts[i]; k++) {
                currentIndex[tokens[i]] += k;
                if (
                    ...
                    IERC721Upgradeable(tokens[i]).safeTransferFrom(
                        address(this),
                        msg.sender,
                        collectionDetail[tokens[i]][currentIndex[tokens[i]]],
                        ""
                    );
                }
                if (
        ...
        emit Claimed(msg.sender, tokens, amounts, maxAllowce, nonce);
    }
    ...
    PausableUpgradeable
{
        ...
        returns (bool)
    {
        address signer = keccak256(
            abi.encode(receiver, tokens, amounts, maxAllowce, nonce)
        ).ethSignedMessage().recover(signature);
        require(signer == TRUSTED_PARTY, "Shiniki: Invalid signature claim ETH");
