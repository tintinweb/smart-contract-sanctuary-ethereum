...
contract MembershipTiers {
    ...
    /// @notice Pending renewal fees NFT has
    mapping(address => uint256) public renewalFees;
    ...
    /// @notice  Withdraws renewal fees
    function withdraw() external {
        NFTInfo memory _nftInfo = nftInfo[msg.sender];
        if (address(0) == _nftInfo.spendAddress) revert NotOwner();
        uint256 _renewalFees = renewalFees[msg.sender];
        renewalFees[msg.sender] = 0;
        payable(msg.sender).transfer(_renewalFees);
    }
