...
contract FeeSharingSystem is ReentrancyGuard, Ownable {
    ...
    IERC20 public immutable hunterGovernanceToken;
    ...
    constructor(
        ...
        hunterGovernanceToken = IERC20(_looksRareToken);
    ...
    function deposit(uint256 amount, bool claimRewardToken) external nonReentrant {
        ...
        hunterGovernanceToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function _checkAndAdjustLOOKSTokenAllowanceIfRequired(uint256 _amount, address _to) internal {
        if (hunterGovernanceToken.allowance(address(this), _to) < _amount) {
            hunterGovernanceToken.approve(_to, type(uint256).max);
        }
    }
    ...
    function _withdraw(uint256 shares, bool claimRewardToken) internal {
        ...
        hunterGovernanceToken.safeTransfer(msg.sender, currentAmount);
...
import {IHunterGovernanceToken} from "../interfaces/IHunterGovernanceToken.sol";
...
contract TokenDistributor is ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeERC20 for IHunterGovernanceToken;
    ...
    IHunterGovernanceToken public immutable looksRareToken;
    ...
    constructor(
        ...
        // now we have how many tokens to supply
        uint256 nonCirculatingSupply = IHunterGovernanceToken(_looksRareToken).SUPPLY_CAP() -
            IHunterGovernanceToken(_looksRareToken).totalSupply();
        ...
        // 2. Store values
        looksRareToken = IHunterGovernanceToken(_looksRareToken);
...
interface IHunterGovernanceToken is IERC20 {
