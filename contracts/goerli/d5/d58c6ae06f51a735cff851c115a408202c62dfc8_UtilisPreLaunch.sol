...
uint128 constant USDT_PRICE = 0.01 * 10 ** 18; // price of 1 UTILIS in USDT in 18 decimals (1 utilis = 0.01 USDT)
uint128 constant MAX_BUY_IN_UTILIS = 1000000 * 10 ** 18; // max buy per user in UTILIS
uint128 constant MIN_BUY_IN_UTILIS = 100 * 10 ** 18; // minimum buy in utilis token
uint128 constant START_TIME_UNIX_EPOCH = 0; // set the starting time (you can change it later as long as the ICO did not start)
uint128 constant END_TIME_UNIX_EPOCH = 0; // set the starting time (you can change it later as long as the ICO did not end)
...
// File contracts/UtilisPreLaunch.sol
...
contract UtilisPreLaunch is Ownable {
    using SafeERC20 for IERC20;

    IERC20 public immutable utilisToken;
    ...
    uint128 public maxBuy = MAX_BUY_IN_UTILIS;
    uint128 public minBuy = MIN_BUY_IN_UTILIS;
    ...
    modifier onlyWhenSaleIsOn() {
        require(
            ...
            "UtilisPreLaunch: Sale is not on"
        );
        _;
    }

    function buy(uint256 amount) external onlyWhenSaleIsOn {
        // transfer this usdt here
        usdt.safeTransferFrom(msg.sender, address(this), amount);
        // calculate the amount of utilis token to send
        uint256 utilisAmount = (amount * 10 ** 18) / usdtPrice;
        // send the utilis token to the user
        utilisToken.safeTransfer(msg.sender, utilisAmount);
        // update the bought amount
        _buy(_msgSender(), 1, utilisAmount, amount);
    }
    ...
    function _buyWithBNB() internal {
        ...
        // calculate the amount of utilis token to send
        uint256 utilisAmount = (usdtValue * 10 ** 18) / usdtPrice;
        // send the utilis token to the user
        utilisToken.safeTransfer(msg.sender, utilisAmount);
        // update the bought amount
        _buy(_msgSender(), 0, utilisAmount, usdtValue);
    }
    ...
    function _buy(
        ...
        require(
            amount >= minBuy,
            "UtilisPreLaunch: Amount is less than min buy"
        );
        boughtAmount[user] += amount;
        require(
            boughtAmount[user] <= maxBuy,
            "UtilisPreLaunch: Exceed max buy"
        );
        totalSold += amount;
        require(
            totalSold <= totalPresale,
            "UtilisPreLaunch: Exceed total presale"
        );
    ...
    function setUsdtPrice(uint128 _price) external onlyOwner {
        require(_price > 0, "UtilisPreLaunch: Price must be greater than 0");
        usdtPrice = _price;
    ...
    function setEndTime(uint64 _endTime) external onlyOwner {
        require(
            ...
            "UtilisPreLaunch: End time must be in the future"
        );
    ...
    function setStartTime(uint64 _startTime) external onlyOwner {
        require(
            ...
            "UtilisPreLaunch: Start time can't be changes after sale started"
        );
    ...
    function withdrawPresaleToken(uint256 amount) external onlyOwner {
        utilisToken.safeTransfer(msg.sender, amount);
        totalPresale -= amount;
    }

    function depositPresaleToken(uint256 amount) external onlyOwner {
        utilisToken.safeTransferFrom(msg.sender, address(this), amount);
        totalPresale += amount;
    }
}