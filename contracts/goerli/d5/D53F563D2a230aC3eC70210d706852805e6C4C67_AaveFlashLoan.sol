...
contract AaveFlashLoan is
    ...
    constructor(
        IPoolAddressesProvider _addressProvider,address _sRouter, address _uRouter
    ...
    function flashLoan(
        address asset,
        uint amount,
        // IClearingHouse.OpenPositionParams memory _openPosition,
        // IClearingHouse.ClosePositionParams memory _closePosition
       bool startOnUniswap,
            address token1,
        ...
        bytes memory params = abi.encode(startOnUniswap,token1,balanceBefore);
    ...
    function executeOperation(
        ...
        uint amountOwing = flashAmount.add(premiums[0]);
    
IERC20(token0).approve(address(POOL), amountOwing);
    ...
    function _withdrawFunds(
        ...
        if (_asset == address(0)) {
            require(address(this).balance > _amount, "Failed to send");
    ...
    function WithdrawNative() public onlyOwner {
        _withdrawFunds(address(0), address(this).balance, msg.sender);
    }

    function WithdrawTokens(address _asset) public onlyOwner {
        uint256 _amount = IERC20(_asset).balanceOf(address(this));
        _withdrawFunds(_asset, _amount, msg.sender);
    }
}