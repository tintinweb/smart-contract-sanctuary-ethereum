...
contract DaftSwapPair is ReentrancyGuard, NoDelegateCall {
	...
	function createLimitOrder(uint256 tokenXAmount, int24 tick, address maker) external nonReentrant noDelegateCall onlyRouter returns (uint256 orderId) {
		...
		//update lowestTick
		if (lowestTick > tick || ticks[lowestTick].currTokenXAmount == 0) lowestTick = tick;
	}
...
library Tick {
    ...
    function simulateExecute(
        ...
        uint256 prevTokenXAmount = self[tick].currTokenXAmount;
        if (prevTokenXAmount == 0) return (0, 0);

        if (tokenXDelta > prevTokenXAmount) {
            tokenXDelta = prevTokenXAmount;
        }

        requiredTokenYAmount = TickMath.getOutputTokenYAmount(tick, tickMode, tokenXDelta);
        usedTokenXAmount = tokenXDelta;
    }
