...
/*
 * @title Whitelist contract to allow user to claim their old tolen share
 *
 * @author @Pedrojok01
 */
...
contract ERC20Whitelist is Ownable, ReentrancyGuard {
    ...
    uint256 public amountLeftToClaim;
    ...
    function _whitelistSingleWallet(address wallet, uint256 _amount) private onlyOwner {
        require(!isWhitelisted[wallet], "Address already added");

        isWhitelisted[wallet] = true;
        amountClaimable[wallet] = _amount;
    }

    function whitelistMultiWallets(address[] memory wallets, uint256[] memory amounts) external onlyOwner {
        uint256 numOfwallets = 0;
        uint256 totalAmount = 0;

        for (uint256 i = 0; i < wallets.length; i++) {
            _whitelistSingleWallet(wallets[i], amounts[i]);
            numOfwallets++;
            totalAmount += amounts[i];
        }
        walletCount = numOfwallets;
        amountLeftToClaim = totalAmount;
    }

    // if merkle tree...
    // function safeMint(address to, bytes32[] calldata _merkleProof) public payable {
    //     bytes32 leaf = keccak256(abi.encodePacked(to));
    //     require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof.");
    // }

    function claim() external nonReentrant {
        require(!isWhitelisted[msg.sender], "Nothing to claim");

        uint256 amount = this.checkClaimableAmount(msg.sender);
    ...
    function checkClaimableAmount(address wallet) external view returns (uint256) {
    ...
    function vestingTokenBalance() external view returns (uint256) {
