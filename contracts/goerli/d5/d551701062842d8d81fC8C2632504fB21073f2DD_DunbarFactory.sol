...
import { IDunbarFactory } from "./IDunbarFactory.sol";

contract DunbarFactory is IDunbarFactory {
    ///                                                          ///
    ///                          IMMUTABLES                      ///
    ///                                                          ///
    ...
    constructor(address walletImpl) payable {
        WALLET_IMPL = walletImpl;
        WALLET_CREATION_CODE_HASH =
            keccak256(abi.encodePacked(type(ERC1967Proxy).creationCode, abi.encode(walletImpl, "")));
    ...
    /// @notice If a nonce has been previously used by a pool creator.
    /// @dev Account => Nonce => Used
    ...
    function computeWalletAddress(address[] calldata owners, uint256 nonce) external view returns (address) {
        return address(
            uint160(
                uint256(
                    keccak256(
                        abi.encodePacked(
                            bytes1(0xff),
                            address(this),
                            keccak256(abi.encodePacked(owners, nonce)),
                            WALLET_CREATION_CODE_HASH
                        )
                    )
                )
    ...
    event WalletCreated(address indexed wallet, address indexed owner, uint256 threshold);

    function createWallet(address[] calldata owners, address[] calldata drivers, uint256 threshold, uint256 nonce)
        public
        payable
        returns (address wallet)
    {
        if (usedNonces[msg.sender][nonce]) revert NONCE_USED();

        usedNonces[msg.sender][nonce] = true;

        bytes32 salt = keccak256(abi.encodePacked(owners, nonce));

        bytes memory createData = abi.encodePacked(type(ERC1967Proxy).creationCode, abi.encode(WALLET_IMPL, ""));

        bytes memory initData =
            abi.encodeWithSignature("initialize(address[],address[],uint256)", owners, drivers, threshold);

        assembly {
            wallet := create2(callvalue(), add(createData, 0x20), mload(createData), salt)

            if iszero(call(gas(), wallet, 0, add(initData, 0x20), mload(initData), 0, 0)) { revert(0x0, 0x0) }
        }

        unchecked {
            uint256 numOwners = owners.length;
            for (uint256 i; i < numOwners; ++i) {
                emit WalletCreated(wallet, owners[i], threshold);
            }
        }
    }
}
...
import { DunbarFactoryTypes } from "./types/DunbarFactoryTypes.sol";

interface IDunbarFactory is DunbarFactoryTypes {
    ///                                                          ///
    ///                            EVENTS                        ///
    ///                                                          ///

    // event WalletCreated(address indexed wallet, address[] owners, address[] drivers, uint256 threshold);

    event ContributionCreated(bytes32 indexed poolId, address indexed account, uint256 amount);

    // TODO ContributionIncluded
    event ContributionAdded(bytes32 indexed poolId, address indexed account, uint256 amount);

    event ContributionRemoved(bytes32 indexed poolId, address indexed account, uint256 amount);

    event PoolExecuted(bytes32 indexed poolId, address indexed wallet);

    event NonceCanceled(address indexed account, uint256 nonce);
...
interface DunbarFactoryTypes is WalletTypes {
    struct PoolType {
        address target;
        uint256 value;
        bytes data;
        ...
        address[] walletDrivers;
        uint256 walletThreshold;
        uint256 nonce;
        uint256 relayFee;
    }
}
