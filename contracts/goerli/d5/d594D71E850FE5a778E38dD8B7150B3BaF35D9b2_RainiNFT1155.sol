...
contract RainiNFT1155 is ERC1155, AccessControl, ReentrancyGuard {
    ...
    function setEtherValues(
        uint256 _unicornToEth,
        uint256 _rainbowToEth,
        uint256 _minPointsPercentToMint
    ) external onlyOwner {
    ...
    function merge(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                _burnAmounts[i] <= balanceOf(_msgSender(), _tokenIds[i]),
                "not enough balance"
            );
            ...
            require(
                ...
                "can only merge into higher levels"
            );
            CardLevel memory _tempCardLevel = cardLevels[_tempTokenVars.cardId][
                _tempTokenVars.level
            ];
        ...
        if (
            ...
            _level > 0 &&
            cardLevels[_cardId][_level - 1].tokenId == 0
        ) {
            INftStakingPool nftStakingPool = INftStakingPool(
                nftStakingPoolAddress
            );
            uint256 nextTokenId = maxTokenId;
            uint256[] memory mergedTokensIds = new uint256[](
    ...
    function mintWithPoints(
        ...
        for (uint256 i = 0; i < _cardId.length; i++) {
            ...
            require(
                block.timestamp >= card.mintTimeStart,
                "Card not yet mintable"
            );
            require(
                cardLevel.numberMinted + _amount[i] <= card.allocation,
                "Not enough tokens in supply"
            );
            require(
                numberMintedByAddress[_msgSender()][_cardId[i]] + _amount[i] <=
                    card.maxMintsPerAddress,
        ...
        for (uint256 n = 0; n < 2; n++) {
            ...
            if (totalPrice > 0) {
                ...
                if (remainingPrice > 0) {
                    uint256 minPoints = (totalPrice * minPointsPercentToMint) /
                        100;
                    require(
                        totalPrice - remainingPrice >= minPoints,
                        "not enough balance"
                    );
                    uint256 pointsToEth = loopTypeUnicorns
                        ? unicornToEth
                        : rainbowToEth;
                    require(
        ...
        (bool success, ) = _msgSender().call{
            value: msg.value - _amountEthToWithdraw
        }(""); // refund excess Eth
        require(success, "transfer failed");
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    baseUri,
                    cards[_tokenVars.cardId].pathUri,
                    "/",
                    _tokenVars.mintedContractChar,
                    "l",
