...
contract NFTExample is ERC721, Ownable {
    ...
    uint public constant max_purchase = 8;
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply) ERC721(name, symbol) {
        max_number = maxNftSupply;
    }

    function withdrawAll() public onlyOwner withSeraph {
        uint balance = address(this).balance;
        msg.sender.transfer(balance);
    }

    function setRevealTimestamp(uint256 revealTimeStamp) public onlyOwner withSeraph{
        reveal_timestamp = revealTimeStamp;
    } 

    function setProvenanceHash(string memory provenanceHash) public onlyOwner withSeraph {
        provenance_hash = provenanceHash;
    }

    function setBaseURI(string memory baseURI) public onlyOwner withSeraph {
        _setBaseURI(baseURI);
    }
    ...
    function mintNFT(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= max_purchase, "Can only mint 8 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= max_number, "Purchase would exceed max supply");
        require(price.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < max_number) {
