...
import "telepathy-contracts/src/lightclient/interfaces/ILightClient.sol";
...
import {ITelepathyHandler, ITelepathyBroadcaster} from "telepathy-contracts/src/amb/interfaces/ITelepathy.sol";

contract SourceIssuer is Ownable, ITelepathyHandler {
    uint16 public sourceChainId;
    uint16 public foreignChainId;

    address public constant tokenAddress = address(0);

    uint256 public fees = 0 ether;

    ITelepathyBroadcaster public broadcaster;
    address public receiver;
    address public foreignWithdraw;
    bool public foreignSet;

    event DepositEvent(
        address indexed from, address indexed recipient, uint256 amount, address tokenAddress, uint16 chainId
    );
    event WithdrawEvent(
    ...
    constructor(address _broadcaster, address _receiver, uint16 _sourceChainId, uint16 _foreignChainId) {
    ...
    function setWithdraw(address _foreignWithdraw) external onlyOwner {
        require(foreignSet == false, "address may only be set once");
        foreignSet = true;
    ...
    function depositIntoContract() external payable virtual {
        return;
    }

    function handleTelepathy(uint16 _foreignChainId, address _senderAddress, bytes calldata _data) external {
        require(msg.sender == address(receiver), "Only Telepathy Source receiver may call");
        require(_foreignChainId == foreignChainId, "Foreign Chain Ids don't match");
        require(_senderAddress == foreignWithdraw, "Only deposit address can trigger a message call to this contract.");
        (address recipient, uint256 amount) = abi.decode(_data, (address, uint256));
        address payable telepathyRecipient = payable(recipient);
        telepathyRecipient.transfer(amount);
        emit WithdrawEvent(msg.sender, recipient, amount, tokenAddress, tokenAddress);
    }

    function _deposit(address recipient, uint256 amount) internal {
        broadcaster.send(foreignChainId, foreignWithdraw, abi.encode(recipient, amount));
        emit DepositEvent(msg.sender, recipient, amount, tokenAddress, foreignChainId);
    }
}