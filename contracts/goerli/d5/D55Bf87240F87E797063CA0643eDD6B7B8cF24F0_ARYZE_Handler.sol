...
/// @notice ARYZE Handler used to execute an order
contract ARYZE_Handler is IHandler, Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    using SafeMath for uint256;

    IWETH public WNT;
    address public FACTORY;
    bytes32 public PAIR_CODE_HASH;
    ...
    function initialize() public initializer {
        ...
        // WNT = IWNT(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd); // bsc test //_wnt;
        setFactory(
            0x0Ce853Bcb17179Ef56eAC6a4D78AF71eAc13278F, //_factory;
            0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6, // goerli //_wnt;
            hex"42cf62e1532656d6022c54c7da0c6c0da57e5e588a5bbadc2d69498ecfaecccd" //_codeHash; ARYZE_Pair
        );
    }
    ...
    function setFactory(
        ...
        require(factory != address(0), "Zero factory address!");
        require(wrappedNativeToken != address(0), "Zero WNT address!");
        FACTORY = factory;
        WNT = IWETH(wrappedNativeToken);
    ...
    receive() external payable override {
        require(msg.sender != tx.origin, "ARYZE_Handler#receive: NO_SEND_ETH_PLEASE");
    }

    /**
     * @notice Handle an order execution V2
    ...
    function handle(
        ...
        address wnt = address(WNT);

        // Decode extra data
        (, address relayer, uint256 fee) = abi.decode(_data, (address, address, uint256));

        if (fee == 0) {
            if (inputToken == wnt || inputToken == PineUtils.ETH_ADDRESS) {
                // Convert from ETH to WNT if necessary
            ...
            } else if (outputToken == wnt || outputToken == PineUtils.ETH_ADDRESS) {
                // Swap inputToken -> WNT
                bought = _swap(inputToken, wnt, amount, address(this));

                // Convert from WNT to ETH if necessary
                if (outputToken == PineUtils.ETH_ADDRESS) {
                    WNT.withdraw(bought);
                }
                // Transfer amount to sender
                PineUtils.transfer((outputToken), msg.sender, bought);
            } else {
            ...
            if (inputToken == wnt || inputToken == PineUtils.ETH_ADDRESS) {
                ...
                // Swap inputToken -> WNT -> outputToken
                //  - inputToken -> WNT
                bought = _swap(inputToken, wnt, amount, address(this));
                ...
                // - WNT -> outputToken
                bought = _swap(wnt, outputToken, bought.sub(fee), msg.sender);
            }
        }
    ...
    /**
     * @notice Check whether can handle an order execution  V2
     * @param _inputToken - Address of the input token
     * @param _outputToken - Address of the output token
    ...
    function canHandle(
        ...
        if (fee == 0) {
            if (inputToken == wnt || inputToken == PineUtils.ETH_ADDRESS) {
                return _estimate(wnt, outputToken, _inputAmount) >= _minReturn;
            } else if (outputToken == wnt || outputToken == PineUtils.ETH_ADDRESS) {
                return _estimate(inputToken, wnt, _inputAmount) >= _minReturn;
            } else {
                return _estimate(inputToken, outputToken, _inputAmount) >= _minReturn;
            }
        } else {
            if (inputToken == wnt || inputToken == PineUtils.ETH_ADDRESS) {
                if (_inputAmount <= fee) {
                ...
                return _estimate(wnt, outputToken, _inputAmount.sub(fee)) >= _minReturn;
            } else if (outputToken == wnt || outputToken == PineUtils.ETH_ADDRESS) {
                uint256 bought = _estimate(inputToken, wnt, _inputAmount);
                ...
                return _estimate(wnt, outputToken, bought.sub(fee)) >= _minReturn;
            }
        }
    }

    /**
     * @notice Simulate an order execution  V2
     * @param _inputToken - Address of the input token
     * @param _outputToken - Address of the output token
     * @param _inputAmount - uint256 of the input token amount
     * @param _minReturn - uint256 of the min return amount of output token
    ...
    function simulate(
        ...
        if (fee == 0) {
            if (inputToken == wnt || inputToken == PineUtils.ETH_ADDRESS) {
                bought = _estimate(wnt, outputToken, _inputAmount.sub(fee));
            } else if (outputToken == wnt || outputToken == PineUtils.ETH_ADDRESS) {
                bought = _estimate(inputToken, wnt, _inputAmount);
            } else {
                bought = _estimate(inputToken, outputToken, _inputAmount);
            }
        } else {
            if (inputToken == wnt || inputToken == PineUtils.ETH_ADDRESS) {
                if (_inputAmount <= fee) {
                ...
                bought = _estimate(wnt, outputToken, bought.sub(fee));
            }
        }
    ...
    function _estimate(
        ...
        IUniswapV2Pair pair = IUniswapV2Pair(UniswapUtils.pairForSorted(FACTORY, token0, token1, PAIR_CODE_HASH));
    ...
    function _swap(
        ...
        if (_inputToken != address(WNT)) {
            prevPairBalance = PineUtils.balanceOf((_inputToken), address(pair));
        }

        // Send tokens to uniswap pair
        require(
            SafeERC20.transfer(_inputToken, address(pair), inputAmount),
            "UniswapV2Handler#_swap: ERROR_SENDING_TOKENS"
        );

        if (_inputToken != address(WNT)) {
            inputAmount = PineUtils.balanceOf(_inputToken, address(pair)).sub(prevPairBalance);
        }
...
library SafeERC20 {
    function transfer(
        ...
        (bool success, bytes memory data) = address(_token).call(
            abi.encodeWithSelector(IERC20(_token).transfer.selector, _to, _val)
        );
