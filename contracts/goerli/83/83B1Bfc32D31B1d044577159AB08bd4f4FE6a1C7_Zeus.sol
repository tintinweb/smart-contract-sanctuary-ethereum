...
contract Zeus is Context, IERC20, Ownable {
  ...
  address payable public teamWallet = payable(0x9103f89DDadF5E56854EC91eD29c5029d1483ba0);
  ...
  uint256 private _previousTeamFee = teamFee;
  ...
  function _transfer(
    ...
    if (
      ...
      to != address(uniswapV2Router)
        ...
        require(_tradingOpen, 'Trading not enabled.');
    ...
    if (
      ...
      if (_contractTokenBalance > 0) {
        if (
          _contractTokenBalance > balanceOf(uniswapV2Pair).mul(liquidityFee).div(100)
        ) {
          _contractTokenBalance = balanceOf(uniswapV2Pair).mul(liquidityFee).div(
            100
          );
        }
  ...
  function _swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
    ...
    uint256 teamETHBalance = ethBalanceUpdate.mul(teamFee).div(10**2);
    if (teamETHBalance > 0) {
      _sendETHToTeam(teamETHBalance);
    }
  }

  function _sendETHToTeam(uint256 amount) private {
    teamWallet.call{ value: amount }('');
  }
  ...
  function _calculateTeamFee(uint256 _amount)
    private
    view
    returns (uint256)
  {
    return _amount.mul(teamFee).div(10**2);
  }

  function _calculateBurnFee(uint256 _amount)
    private
    view
    returns (uint256)
  {
  ...
  function setTeamFeePercent(uint256 _newFee) external onlyOwner {
    require(_newFee <= 100, 'fee cannot exceed 100%');
    teamFee = _newFee;
  }
