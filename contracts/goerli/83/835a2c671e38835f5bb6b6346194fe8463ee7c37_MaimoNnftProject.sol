...
// File: contracts/Butterflies.sol
...
contract MaimoNnftProject is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Butterflies", "BTF") {       
        baseURI = "ipfs://QmWi3VdvCWmkgiSLdLMyCnGrqd5b6YoP53DaVarE8ejHZC/";
    }
    bool public saleStarted = true;
    uint256 public constant monsterPrice = 5;
    uint256 public constant maxSupply = 50;
    uint8 public constant maxSupplyPurchase = 3;
    ...
    function mint(bytes32[] memory proof, bytes32 leaf, uint256 numberOfTokens) public payable returns(uint256,uint256){
        ...
        require(numberOfTokens <= maxSupplyPurchase, "Can only mint 3 tokens at a time");        
        (cond, res) = totalSupply().tryAdd(numberOfTokens);
        require(cond, "Overflow error");
        require(res <= maxSupply, "Purchase would exceed max supply of Monsters");        
    ...
    function mintGiveAway(bytes32[] memory _proof, bytes32 _leaf, address _receiver) public onlyOwner returns(uint256){
       
        if (merkleEnabled) {
            // merkle tree
            require( keccak256(abi.encodePacked(_receiver)) == _leaf, "no equal" );    
            require( _proof.verify(merkleRoot, _leaf), "You are not in the list" );
        }
        
        require(saleStarted == true, "The sale is paused");   
        require(totalSupply() < maxSupply, "Max mint maximum exceeded");                             
