...
contract StreetJeetFightersFactory is Ownable {
    ...
    // price data
    uint256 public lastPrice = 777e13;
    uint256 constant e1 = 1000425986000000000;
    uint256 constant e2 = 1000700000000000000;
    uint256 constant d = 4985e18;
    // ethereum endpoints
    address public feeAddress;
    address public poolAddress;
    uint256 public constant feeAddressPercent = 10;

    constructor(address nftContractAddress_) {
        nftContract = IStreetJeetFighters(nftContractAddress_);
    ...
    function mint(address to, uint256 count) external payable {
        // mint
        uint256 ethConsumed;
        for (uint256 i = 0; i < count; ++i) ethConsumed += _mint(to);
    ...
    function _mint(address to) internal returns (uint256) {
        require(
            nftContract.balanceOf(to) < 8,
            "maximum is 8 nft count per account"
        );
        lastPrice = this.getPrice(1);
        //console.log("p[", nftContract.mintedCount() + 1, "]=", lastPrice);
        nftContract.mint(to, lastPrice);
        return lastPrice;
    }
    ...
    function getPrice(uint256 count) external view returns (uint256 price) {
        ...
        uint256 mintNumber = nftContract.mintedCount() + 1;
