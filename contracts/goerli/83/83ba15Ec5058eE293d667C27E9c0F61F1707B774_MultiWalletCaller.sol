...
contract MultiWalletCaller is Ownable, IERC721Receiver {
    ...
    modifier checkId(uint256 _startId, uint256 _endId) {
        require(_startId <= _endId && _startId < childCount[msg.sender] && _endId < childCount[msg.sender], "MultiWalletCaller: Invalid ID");
        _;
    }
    ...
    function createWallets(uint256 _quantity) public onlyHolder {
    ...
    function sendETH(uint256 _startId, uint256 _endId) public payable onlyHolder checkId(_startId, _endId) {
        ...
        for (uint256 i = _startId; i <= _endId; i++) {
            (bool success,) = address(child[msg.sender][i]).call{value: value}(new bytes(0));
    ...
    function run(uint256 _startId, uint256 _endId, address _callContract, bytes calldata _callData, uint256 _value) public onlyHolder checkId(_startId, _endId) {
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(child[msg.sender][i])).run(_callContract, _callData, _value);
        }
    }
    ...
    function withdrawERC20(uint256 _startId, uint256 _endId, address _contract) public checkId(_startId, _endId) {
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(child[msg.sender][i])).withdrawERC20(_contract, msg.sender);
            idx++;
        }
    }

    function withdrawERC721(uint256 _startId, uint256 _endId, address _contract, uint256[] calldata _tokenIds) public checkId(_startId, _endId) {
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(child[msg.sender][i])).withdrawERC721(_contract, _tokenIds[idx], msg.sender);
            idx++;
        }
    }

    function withdrawERC1155(uint256 _startId, uint256 _endId, address _contract, uint256 _tokenId) public checkId(_startId, _endId) {
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(child[msg.sender][i])).withdrawERC1155(_contract, _tokenId, msg.sender);
            idx++;
        }
    }
     ...
     * @dev Only Owner
     */
    function recoverERC721(address _contract, uint256 _tokenId) public onlyOwner {
     ...
     * @dev Only Owner
     */
    function recoverERC1155(address _contract, uint256 _tokenId, uint256 _amount, bytes memory _data) public onlyOwner {
     ...
     * @dev Only Owner
     */
    function setContract(address _kudasai) public onlyOwner {
        kudasai = _kudasai;
    }
}
...
contract MinterChaild is IERC721Receiver {
    ...
    modifier onlyOwner() {
        require(owner == msg.sender, "MinterChaild: No Call");
        _;
    }
    ...
    function withdrawERC20(address _contract, address _to) public onlyOwner {
    ...
    function withdrawERC721(address _contract, uint256 _tokenId, address _to) public onlyOwner {
        IERC721(_contract).safeTransferFrom(address(this), _to, _tokenId);
    }

    function withdrawERC1155(address _contract, uint256 _tokenId, address _to) public onlyOwner {
        bytes memory data;
    ...
    function withdrawETH() public onlyOwner {
        (bool success,) = address(owner).call{value: address(this).balance}(new bytes(0));
        require(success, "MinterChaild: Transfer Failed");
    }

    function run(address _callContract, bytes calldata _callData, uint256 _value) public onlyOwner {
        (bool success,) = address(_callContract).call{value: _value}(_callData);
