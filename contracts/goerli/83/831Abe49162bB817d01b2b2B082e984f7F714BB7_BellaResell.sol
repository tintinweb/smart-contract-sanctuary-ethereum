...
/// @author BELLA
/// @title Bella Resell
/// @notice Smart contract to buy/sell NFT ERC721/ERC1155
contract BellaResell is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    mapping(address => RoyaltyWithdraw) public pendingWithdrawalsRoyalty;
        ...
        whenNotPaused
    {
        ...
        pendingWithdrawals[backendAddress] += fees;
        ...
        if(receiptReceiver != address(0) 
            ...
            && royaltyAmount > 0) 
        {
            pendingWithdrawalsRoyalty[receiptReceiver] = RoyaltyWithdraw(
    ...
    /// Withdraw Royalty
    /// @param receiptReceiver address to withdraw for royalty
    /// @notice to withdraw sales revenue
    function withdrawRoyalty(address payable receiptReceiver) 
        external 
        nonReentrant
        onlyRole(BACKEND_ROLE)
    {
        if(block.timestamp < pendingWithdrawalsRoyalty[receiptReceiver].timestamp) {
            revert("ERR-8-BR");
        }
        if(pendingWithdrawalsRoyalty[receiptReceiver].royaltyAmount == 0) {
            revert("ERR-7-BR");
        }
        uint amount = pendingWithdrawalsRoyalty[receiptReceiver].royaltyAmount;
        emit WithdrawPerformed(receiptReceiver, pendingWithdrawalsRoyalty[receiptReceiver].royaltyAmount);

        pendingWithdrawalsRoyalty[receiptReceiver].royaltyAmount = 0;
        pendingWithdrawalsRoyalty[receiptReceiver].timestamp = 0;
        receiptReceiver.transfer(amount);
    }
...
contract Utils is IERC165, Ownable {
