...
contract PalMinter is Ownable, Pausable {
    ...
    uint256 private treasurePercentage;

    // enum Rarity {Economy, Common, Uncommon, Rare, Epic, Legendary}
    ...
    event MintingPriceUpdated(PalToken.Rarity rarity, uint256 price);
    event CooldownTimeUpdated(PalToken.Rarity rarity, uint256 time);
    event TreasureUpdated(address treasure);
    event BuybackAndBurnUpdated(address buybackAndBurn);
    event TreasurePercentageUpdated(uint256 treasurePercentage);

    constructor(address _genesisToken, address _palToken, address _treasure, address _buybackAndBurn) {
        genesisToken = IERC721(_genesisToken);
        palToken = PalToken(_palToken);
        treasure = _treasure;
        buybackAndBurn = _buybackAndBurn;
        treasurePercentage = 1000;
        rarityCooldownTimes = [0 hours, 0 hours, 0 hours, 0 hours, 0 hours, 0 hours];
        rarityPrices = [0.001 ether, 0.002 ether, 0.003 ether, 0.004 ether, 0.005 ether, 0.006 ether];
    }
    ...
    function mintPal(uint256 _genesis, PalToken.Rarity _rarity) external payable whenNotPaused {
        ...
        uint256 treasureAmount = (msg.value.mul(getTreasurePercentage())).div(10000);
        uint256 buybackAndBurnAmount = msg.value.sub(treasureAmount);
        payable(treasure).transfer(treasureAmount);
        payable(buybackAndBurn).transfer(buybackAndBurnAmount);
    ...
    function updateMintingPrice(PalToken.Rarity _rarity, uint256 _price) external onlyOwner {
        ...
        emit MintingPriceUpdated(_rarity, _price);
    }
    ...
    function updateCooldownTime(PalToken.Rarity _rarity, uint256 _cooldownTime) external onlyOwner {
        ...
        emit CooldownTimeUpdated(_rarity, _cooldownTime);
    }

    function setTreasure(address _treasure) external onlyOwner {
        treasure = _treasure;
        emit TreasureUpdated(_treasure);
    }

    function setBuybackAndBurn(address _buybackAndBurn) external onlyOwner {
        buybackAndBurn = _buybackAndBurn;
        emit BuybackAndBurnUpdated(_buybackAndBurn);
    }

    function setTreasurePercentage(uint256 _treasurePercentage) external onlyOwner {
        require(_treasurePercentage <= 10000, "PalMinter: Invalid treasure percentage");
        treasurePercentage = _treasurePercentage;
        emit TreasurePercentageUpdated(_treasurePercentage);
    }

    function getTreasurePercentage() public view returns (uint256) {
    ...
    ERC721Pausable
{
    ...
    mapping(uint256 => Rarity) private _mintRarities;

    // /**
    //  * @dev Initializes the contract by granting `DEFAULT_ADMIN_ROLE`, `MINTER_ROLE` and `PAUSER_ROLE`
    //  * to the deploying account and setting the base token URI for token metadata.
    //  *
    //  * @param baseTokenURI The base URI for the token metadata, which will be used to generate token URIs.
    //  * See {ERC721PresetMinterPauserAutoId}.
    //  */
    // constructor(
    //     string memory _name,
    ...
    //  * and burning tokens, as well as transferring them.
    //  *
     ...
     */
    function mint(address to, Rarity rarity) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have minter role to mint");

        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
        _mint(to, _tokenIdTracker.current());
        _mintRarities[_tokenIdTracker.current()] = rarity;
