...
contract FundU is ReentrancyGuard, FundUtils {
    ...
    function pause(uint256 id) external onlyStreamOwner(id) {
        ...
        emit PauseStream(id, stream.beneficiary);
    }
     ...
     * @dev It reverts if the stream is not active
     */
    function resumeStream(uint256 id, bool paid) external onlyStreamOwner(id) {
        // Get the Stream
        StreamData storage stream = s_streamById[id];
...
contract FundUtils {
    ...
    event PauseStream(uint256 indexed id, address indexed beneficiary);
