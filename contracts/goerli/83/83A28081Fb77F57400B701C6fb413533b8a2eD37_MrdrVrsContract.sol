...
contract MrdrVrsContract is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer, ReentrancyGuard, Referable, Pausable  {
    ...
    function getGuessers(uint256 _tokenId) public view returns (address[] memory, uint256[] memory) {
    ...
    function getGuesses(address _address) public view returns (uint256[] memory) {
    ...
    function burn(uint256 _burnId, uint256 _guessId, uint256 _userId) public whenNotPaused {
        ...
        require(!guessers[_guessId].contains(msg.sender), "Invalid guesser");
        require(!guesses[msg.sender].contains(_guessId), "Invalid guess");
