...
contract DevCitiBuilding is DevCitiNFT {
    ...
    function initialize(address admin) initializer public {
        __DevCitiNFT_init("CitiBuilding", "CTB", admin);
    }
}
 ...
 */
contract DevCitiNFT is BaseCitiNFT, ERC721URIStorageUpgradeable {
    /**
     * @dev Initializes the `name` and `symbol` of the contract.
     */
    function __DevCitiNFT_init(string memory _name, string memory _symbol, address admin) internal onlyInitializing {
        require(!Utils.isMainnet(), "mainnet not allowed");
        __BaseCitiNFT_init(_name, _symbol, admin);
        __ERC721URIStorage_init();
    }

    /**
     * @dev See {BaseCitiNFT}, {ERC721Upgradeable}
     */
    function _baseURI() internal view override(BaseCitiNFT, ERC721Upgradeable) returns (string memory) {
        return BaseCitiNFT._baseURI();
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId)
        internal
        whenNotPaused
        override(BaseCitiNFT, ERC721Upgradeable)
    {
     ...
     * See {ERC721URIStorageUpgradeable}, {ERC721Upgradeable}
     */
 ...
 */
contract BaseCitiNFT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, Versioned {
    ...
    /**
     * @dev Initializes the `name` and `symbol` of the contract.
     * `admin` receives {DEFAULT_ADMIN_ROLE}, {MINTER_ROLE} and {PAUSER_ROLE}, assumes msg.sender if not specified.
     */
    function __BaseCitiNFT_init(string memory _name, string memory _symbol, address admin) internal onlyInitializing {
     ...
     */
    function _baseURI() internal virtual view override(ERC721Upgradeable) returns (string memory) {
     ...
     */
    function _doSafeMint(address to) internal returns (uint256) {
     ...
     */
    function mintWithDetails(address to, string calldata details, uint256 numTokensToBurn,
...
contract BaseToken is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, Versioned {
     ...
     * `admin` receives {DEFAULT_ADMIN_ROLE}, {MINTER_ROLE} and {PAUSER_ROLE}, assumes msg.sender if not specified.
     */
    function __BaseToken_init(string memory _name, string memory _symbol, address admin) internal onlyInitializing {
        require(Utils.isKnownNetwork(), "unknown network");
...
contract Versioned {
    string public constant version = "239b5d722504fc799b218ef5d823ffd8029e2132";
}
