...
contract Grail is ERC721Tradable {
    ...
    // Rare
    mapping(uint256 => bool) private curated;
    mapping(uint256 => bytes32) private curatedHashes;
    ...
    constructor(
        ...
        daoPercentage = 7;
        baseTokenURI = "https://grailer-new.vercel.app/api/grail/1/meta/";
    }

    function contractURI() public pure returns (string memory) {
        return "https://grailer-new.vercel.app/api/grail/1/contract";
    }
    ...
    function _generateRandomHash() private view returns (bytes32) {
        return keccak256(abi.encodePacked(block.number, blockhash(block.number - 1), msg.sender));
    }

    function _randomNumber() private view returns (uint) { 
        return uint(_generateRandomHash()) % 100;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        require(balance > 0);
        _withdraw(daoAddress, balance * daoPercentage / 100);
        _withdraw(artistAddress, balance);
    }
    ...
    */
    ...
    function updateCurated(uint256[] memory _indexes, bool[] memory _curated, bytes32[] memory _curatedHashes)
        external
        onlyOwner
    {
        require(
        ...
        for (uint256 i = 0; i < _curatedHashes.length; i++) {
            _curated[_indexes[i]] = _curated[i];
