...
contract DssCdpManager {
    ...
    modifier cdpAllowed(uint256 cdp) {
        require(
            msg.sender == owns[cdp] || cdpCan[owns[cdp]][cdp][msg.sender] == 1,
            "cdp-not-allowed"
        );
        _;
    }
    ...
    modifier urnAllowed(address urn) {
        require(
            msg.sender == urn || urnCan[urn][msg.sender] == 1,
            "urn-not-allowed"
        );
        _;
    }
    ...
    // Frob the cdp keeping the generated IRDT or collateral freed in the cdp urn address.
    function frob(
        uint256 cdp,
        int256 dink,
        int256 dart
    ...
    // Transfer wad amount of IRDT from the cdp address to a dst address.
    function move(
        uint256 cdp,
        address dst,
        uint256 rad
        ...
        cdpAllowed(cdpDst)
    {
        ...
        (uint256 ink, uint256 art) = VatLike(vat).urns(
            ilks[cdpSrc],
            urns[cdpSrc]
        );
        VatLike(vat).fork(
