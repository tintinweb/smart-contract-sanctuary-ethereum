...
contract STARS is ERC721URIStorage, ERC2981, Ownable {
    ...
    mapping(uint256 => uint96) private royaltyFeesInBips;
    ...
    function setIsAddressAllowlisted(address _newAddress, bool _value)
        public
        onlyOwner
    {
    ...
    function setTokenRoyalty(
        ...
        require(
            ...
            "Access denied. Only minter may set token royalty."
        );
    ...
    function getRoyaltyFeeByTokenId(uint256 tokenId)
        public
        view
        returns (uint96)
    {
        return royaltyFeesInBips[tokenId];
    }
    ...
    function mintToken(string memory tokenURI, uint96 _royaltyFeesInBips)
        public
        returns (uint256)
    {
        _tokenIds.increment();
        ...
        royaltyFeesInBips[newItemId] = _royaltyFeesInBips;
        _setTokenURI(newItemId, tokenURI);
        setTokenRoyalty(newItemId, msg.sender, _royaltyFeesInBips);
        // Give the marketplace approval to transact NFTs between users
        setApprovalForAll(marketplaceAddress, true);

        emit TokenMinted(newItemId, marketplaceAddress, msg.sender);
        return newItemId;
    }
