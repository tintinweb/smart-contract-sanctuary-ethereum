...
import "./Types.sol";
import "./Venture.sol";
import "./SignatureStore/ERC20FixedPriceAllocatorSignatureStore.sol";
/* TODO:  */
import "hardhat/console.sol";

/* TODO: UPDATE LAA INVESTMENTS TO PURCHASE */
contract ERC20FixedPriceAllocator is Ownable, Initializable {
    ...
    /// @notice Escrow is active
    bool public escrowActive = false;

    /// @notice Has the escrow amount been claimed
    bool public isEscrowClaimed = false;

    /// @notice Token price
    uint256 public tokenPrice;

    /// @notice Hard cap on allocator
    uint256 public tokensForAllocation;

    /// @notice Hurdle on allocator
    ...
    /// @notice when the vesting starts
    uint256 public claimScheduleStartTimeStamp;

    /// @notice vesting cliff duration, kicks in after the token
    /// is created and set (ie, after setVentureToken is called)
    uint256 public claimLockDuration;

    /// @notice when the vesting starts
    uint256 public claimDuration;

    /// @notice what token are we accepting in this allocator
    IERC20 public purchaseToken;

    /// @notice what token are we issuing as per vesting conditions
    ...
    ERC20FixedPriceAllocatorSignatureStore public signatureStore;

    /// @notice Allocated tokens to a specific address
    mapping(address => uint256) public allocation;

    /// @notice Total tokens allocated in this allocator
    uint256 public totalAllocated;

    /// @notice Purchased tokens to a specific address
    mapping(address => uint256) public paidAmounts;

    /// @notice Total tokens purchased in this allocator
    uint256 public totalPaidAmount;

    /// @notice Claimed tokens to a specific address
    mapping(address => uint256) public claimed;

    /// @notice Total Claimed in this allocator
    ...
    /// @param allocated The amount that was allocated
    event Purchase(address account, uint256 investment, uint256 allocated);
    ...
    */
    ...
    function initialize(
        ...
        address _fixedPriceSignatureStoreImpl,
        address _founder,
        ...
        name = _config.name;
        Venture newVenture = Venture(_config.venture);
        require(newVenture.tokenSupply() > tokensForAllocation, "FixedPriceAllocator: Venture has insufficient token supply");
        venture = _config.venture;
        tokenPrice = _config.signatureStoreConfig.tokenPrice;
        jubiFee = tokenPrice * _jubiFeePercent.num / _jubiFeePercent.den;
        ventureFunds = tokenPrice - jubiFee;
        tokensForAllocation = _config.tokensForAllocation;
        hurdle = _config.signatureStoreConfig.hurdle;
        claimScheduleStartTimeStamp = _config.signatureStoreConfig.claimScheduleStartTimeStamp;
        claimLockDuration = _config.signatureStoreConfig.claimLockDuration;
        claimDuration = _config.signatureStoreConfig.claimDuration;
        inviteCodesMerkleRoot = _config.inviteCodesMerkleRoot;
        purchaseToken = _config.venture.treasuryToken();

        isOpen = true;

        signatureStore = ERC20FixedPriceAllocatorSignatureStore(Clones.clone(_fixedPriceSignatureStoreImpl));
        signatureStore.initialize(_config.signatureStoreConfig, name, _founder);
    }

    /**
    * @param ventureTokenPurchaseAmount  amount of venture tokens to purchase
    * @param minVentureTokenPurchase min amount of venture tokens
    * @param maxVentureTokenPurchase max amount of venture tokens
    * @param inviteCode TODO
    * @param merkleProof TODO
    ...
    */
    function purchase(
        ...
        require(MerkleProof.verify(merkleProof, inviteCodesMerkleRoot, keccak256(abi.encode(inviteCode, minVentureTokenPurchase, maxVentureTokenPurchase))), "FixedPriceAllocator: Invalid invite code");
        // We only check minVentureTokenPurchase as, if amount exceeds maxVentureTokenPurchase it will be scale down.
        require(allocation[account] >= minVentureTokenPurchase || ventureTokenPurchaseAmount >= minVentureTokenPurchase, "FixedPriceAllocator: Can not invest less than minimum amount");
        require(claimedInvites[inviteCode] == account || claimedInvites[inviteCode] == address(0), "FixedPriceAllocator: You can only invest with one wallet per invite code");

        Types.ERC20FixedPriceAllocatorSignatureConfig memory config = Types.ERC20FixedPriceAllocatorSignatureConfig({
            signature: signature,
            termsUrl: signatureStore.termsUrl(),
            termsHash: signatureStore.termsHash(),
            numTokens: ventureTokenPurchaseAmount,
            tokenPrice: tokenPrice,
            hurdle: hurdle,
            claimScheduleStartTimeStamp: claimScheduleStartTimeStamp,
            claimLockDuration: claimLockDuration,
        ...
        // 10_000
        uint256 remainingInvestment = maxVentureTokenPurchase - allocation[account];
        if (remainingInvestment < ventureTokenPurchaseAmount) {
            ventureTokenPurchaseAmount = remainingInvestment;
        }

        uint256 remainingTokenAllocation = tokensForAllocation - totalAllocated;
        uint256 purchasedAllocation = ventureTokenPurchaseAmount;

        if (remainingTokenAllocation < purchasedAllocation) {
            purchasedAllocation = remainingTokenAllocation;
            ventureTokenPurchaseAmount = remainingTokenAllocation;
        }


        // TODO: charge JUBI FEE
        allocation[account] += purchasedAllocation;
        totalAllocated += purchasedAllocation;
        claimedInvites[inviteCode] = account;


        if (totalAllocated == tokensForAllocation) {
            _close();
        }

        // the purchase tokens to transfer
        uint256 purchaseTokenAmount = allocationToPurchaseToken(ventureTokenPurchaseAmount);
        paidAmounts[account] += purchaseTokenAmount;
        totalPaidAmount += purchaseTokenAmount;

        if (totalAllocated >= hurdle) {
            SafeERC20.safeTransferFrom(venture.treasuryToken(), msg.sender, venture.fundsAddress(), ventureFunds * purchaseTokenAmount);
            SafeERC20.safeTransferFrom(venture.treasuryToken(), msg.sender, jubiFundsAddress, jubiFee * purchaseTokenAmount);
        } else {
            // escrow
            SafeERC20.safeTransferFrom(purchaseToken, msg.sender, address(this), purchaseTokenAmount);
        }


        emit Purchase(account, purchaseTokenAmount, purchasedAllocation);
    }

    function claim() external {
        /* TODO: remove use inline */
        address account = msg.sender;
        require(!isOpen, "FixedPriceAllocator: Can only claim once allocator is closed");

        uint256 claimableAmount = calculateClaimableNow(account);
        require(claimableAmount > 0, "FixedPriceAllocator: Nothing to claim");
        claimed[account] += claimableAmount;
        totalClaimed += claimableAmount;

        if (claimToken == purchaseToken) {
            SafeERC20.safeTransfer(claimToken, account, allocationToPurchaseToken(claimableAmount));
            emit Claimed(account, allocationToPurchaseToken(claimableAmount));
        } else {
            SafeERC20.safeTransfer(claimToken, account, claimableAmount);
    ...
    */
    function claimEscrow() external {
        require(!isEscrowClaimed, "FixedPriceAllocator: Escrow has already been claimed");
        require(venture.isAdminOrAllocatorManager(msg.sender), "FixedPriceAllocator: only allocator manager can claim escrow funds");
        require(totalAllocated >= hurdle, "FixedPriceAllocator: Sale has not reached hurdle");

        uint256 escrowAmount = purchaseToken.balanceOf(address(this));
        SafeERC20.safeTransferFrom(venture.treasuryToken(), address(this), venture.fundsAddress(), ventureFunds * escrowAmount);
        SafeERC20.safeTransferFrom(venture.treasuryToken(), address(this), jubiFundsAddress, jubiFee * escrowAmount);
        isEscrowClaimed = true;
    }

    function close() external {
        require(venture.isAdminOrAllocatorManager(msg.sender), "FixedPriceAllocator: only allocator manager can close allocator");
        _close();
    }
    ...
    function _close() internal {
        ...
        if (totalAllocated < hurdle) {
            claimToken = purchaseToken;
            // This will make so allocation is claimable right away
            claimScheduleStartTimeStamp = block.timestamp - claimDuration - claimLockDuration;
        } else {
            // If escrow amount is still held in contract, transfer to venture
            if (!isEscrowClaimed) {
                uint256 escrowAmount = purchaseToken.balanceOf(address(this));
                SafeERC20.safeTransferFrom(venture.treasuryToken(), msg.sender, venture.fundsAddress(), ventureFunds * escrowAmount);
                SafeERC20.safeTransferFrom(venture.treasuryToken(), msg.sender, jubiFundsAddress, jubiFee * escrowAmount);
                isEscrowClaimed = true;
            }
        }
    }

    function calculateClaimable(address account, uint256 timestamp) public view returns (uint256 claimableAmount)
    {
        if (claimScheduleStartTimeStamp == 0){
            return 0;
        }
        ...
        uint256 vestingStartTimestamp = claimScheduleStartTimeStamp + claimLockDuration;
        if (timestamp < vestingStartTimestamp) {
            return 0;
        }

        uint256 currentClaimDuration = timestamp - vestingStartTimestamp;
        // minimum claimDuration to 1 sec
        uint256 _claimDuration = 1;
        if (claimDuration > 0) {
            _claimDuration = claimDuration;
        }

        if (currentClaimDuration > _claimDuration) {
            currentClaimDuration = _claimDuration;
        }

        claimableAmount = allocation[account] * currentClaimDuration / _claimDuration;
    }

    function setClaimToken(IERC20 _claimToken, bool _setClaimableScheduleStart) external {
        require(venture.isAdminOrAllocatorManager(msg.sender), "FixedPriceAllocator: only allocator manager can set the token");
        require(address(claimToken) == address(0), "FixedPriceAllocator: Claim token already set");
        require(address(_claimToken) != address(purchaseToken), "FixedPriceAllocator: Can NOT set the claim token to purchase token");

        SafeERC20.safeTransfer(_claimToken, address(this), totalAllocated);

        claimToken = _claimToken;
        if (_setClaimableScheduleStart) {
            claimScheduleStartTimeStamp = block.timestamp;
        }
    }
    ...
    */
    function migrateVenture(address _newVenture) external {
        require(_newVenture != address(0), "FixedPriceAllocator: invalid new venture");
        Venture newVenture = Venture(_newVenture);
        require(newVenture.tokenSupply() > tokensForAllocation, "FixedPriceAllocator: Venture has insufficient token supply");
        require(newVenture != venture, "FixedPriceAllocator: Can not update to same venture");
        require(venture.isAdmin(msg.sender), "FixedPriceAllocator: only venture admin can update allocator");
        require(newVenture.isAdmin(msg.sender), "FixedPriceAllocator: only venture admin can update allocator");
        address oldVenture = address(venture);
        venture = newVenture;
        emit VentureMigrated(oldVenture, address(newVenture), msg.sender);
    }

    function calculateClaimableNow(address account) public view returns (uint256) {
        return calculateClaimable(account, block.timestamp);
    }

    function allocationToPurchaseToken(uint256 _allocation) internal view returns (uint256) {
        return _allocation / 1e18 * tokenPrice;
    }

    function investmentToAllocation(uint256 purchaseTokens) internal view returns (uint256) {
        return purchaseTokens * 1e18 / tokenPrice ;
    }
}
...
library Types {
    ...
    struct ERC20FixedPriceAllocatorConfig {
        ...
        bytes32 inviteCodesMerkleRoot;
        string merkleTreeId;
        ERC20FixedPriceAllocatorSignatureConfig signatureStoreConfig;
    }
    ...
    struct ERC20FixedPriceAllocatorSignatureConfig {
        ...
        uint256 tokenPrice;
        uint256 hurdle;
        uint256 claimScheduleStartTimeStamp;
...
contract ERC20FixedPriceAllocatorSignatureStore is Ownable, Initializable {
    ...
    struct SignatureData {
        ...
        uint256 hurdle;
        /// @notice the date at which the token is expected to be minted and the claim scheduled starts
        uint256 claimScheduleStartTimeStamp;
        /// @notice period that tokens are locked before they become claimable
        uint256 claimLockDuration;
        /// @notice period that tokens over which token claims are spread after `claimLockDuration`
        uint256 claimDuration;
    ...
    bytes32 constant SIGNATURE_TYPEHASH = keccak256(
        "SignatureData(address signer,string termsUrl,bytes32 termsHash,uint256 numTokens,uint256 tokenPrice,uint256 hurdle,uint256 claimScheduleStartTimeStamp,uint256 claimLockDuration,uint256 claimDuration,bytes32 inviteCode)"
    );
    ...
    function initialize(Types.ERC20FixedPriceAllocatorSignatureConfig memory _config, string memory name, address founder) external initializer {
                _transferOwnership(msg.sender);
        termsHash = _config.termsHash;
        ...
        verifyingContract : founder
                                                                                                                                                                        ...
                                                                                                                                                                        Signature memory signature = _verifySignature(founder, _config, FOUNDER_DOMAIN_SEPARATOR);
    ...
    ///
    function saveSignature(Types.ERC20FixedPriceAllocatorSignatureConfig memory config, address signer) public onlyOwner {
        Signature memory storedSignature = _verifySignature(signer, config, DOMAIN_SEPARATOR);
        signatures[signer].push(storedSignature);
    }

    function _verifySignature(address signer, Types.ERC20FixedPriceAllocatorSignatureConfig memory config, bytes32 domain) private view returns (Signature memory) {
        ...
        claimScheduleStartTimeStamp : config.claimScheduleStartTimeStamp,
        claimLockDuration : config.claimLockDuration,
