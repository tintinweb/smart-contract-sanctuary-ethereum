...
contract Marketplace is AccessControlled, IMarketplace, ReentrancyGuard {
    ...
    mapping(string => uint256) private soldNftsByDataType;

    uint256 private constant _INITIAL_BE_PRICE = 1 ether;

    IMintableNFT private _nftContract;
    ...
    constructor(
        ...
        _nftContract = IMintableNFT(nftContract);
        _tokenContract = ISaleToken(beTokenAddress);
        _usdtContract = IERC20(usdtTokenAddress);
    }

    function setKangaPOSAddress(address kangaPOS) external onlyManager {
        _kangaPOS = kangaPOS;

        emit KangaPOSAddressUpdated(_kangaPOS, kangaPOS);
    }

    function availableTokenSupply() external view returns (uint256) {
        return _tokenContract.balanceOf(address(this));
    }

    function buyToken(uint256 amount) external nonReentrant {
        ...
        _usdtContract.transferFrom(caller, _beneficiary, exchangedAmount);
    ...
    function upsertNFTSaleTypeData(
        ...
        nftsSaleDataByType[membershipType] = NFTTypeSaleData({
            ...
            isEnabled: isEnabled,
            price: price,
            withWatermarkedMintDate: withWatermarkedMintDate
        });

        emit NFTSTypeForSaleDataUpdated(membershipType, quantity, isEnabled, price, withWatermarkedMintDate);
    }

    function _availableNFTTypeSupply(string memory membershipType) internal view returns (uint256) {
        return nftsSaleDataByType[membershipType].quantity - soldNftsByDataType[membershipType];
    }

    function availableNFTTypeSupply(string memory membershipType) external view returns (uint256) {
        if (nftsSaleDataByType[membershipType].isEnabled) {
            return _availableNFTTypeSupply(membershipType);
        } else {
            return 0;
        }
    }
    ...
    function buyNFT(string memory membershipType) external nonReentrant {
        ...
        require(
            _availableNFTTypeSupply(membershipType) > 0,
            "Marketplace: no NFT supply for specified membership type"
        );

        address caller = _msgSender();
        uint256 nftPrice = nftsSaleDataByType[membershipType].price;
        uint256 burnableAmount = SafeMath.div(SafeMath.mul(nftPrice, 19), 20);
        uint256 posReward = nftPrice - burnableAmount;
        uint256 newMembershipTypeSerialId = soldNftsByDataType[membershipType] + 1;

        /**
         * First burn the 95% of the amount and transfer rest to contract address
         */
        _tokenContract.bridgeBurn(caller, burnableAmount);
        _tokenContract.transferFrom(caller, address(this), posReward);

        /**
         * Transfer reward with allowance to kangaPOS
         */
        _tokenContract.transfer(_kangaPOS, posReward);

        /**
         * Mint a NFT to an address and store the change
         */
        uint256 newNFTId = _nftContract.mintNFT(caller, membershipType);

        /*
         * Increase the number of NFTS sold
         */
        soldNftsByDataType[membershipType] = newMembershipTypeSerialId;

        emit NFTSold(
            caller,
            membershipType,
            newNFTId,
            nftsSaleDataByType[membershipType].withWatermarkedMintDate,
            newMembershipTypeSerialId,
            block.timestamp
        );
    }
}
