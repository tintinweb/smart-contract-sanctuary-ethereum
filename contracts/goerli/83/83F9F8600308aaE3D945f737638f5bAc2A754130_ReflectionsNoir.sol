...
contract ReflectionsNoir is ERC721AQueryable, Ownable {
    ...
    error NullString();
    error NullMint();
    error PrivateNotAvailable();
    error PublicNotAvailable();
    error WrongValue();
    error FailedTransfer();
    error EmptyRoot();
    error MintLimit();

    uint256 private constant maxSupply = 999;
    uint256 private constant maxPerWallet = 3;

    uint256 public userCost;
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedURI, bytes32 _root) ERC721A("Reflections Noir", "RNR") {
        if(
            bytes(_initBaseURI).length == 0 || bytes(_initNotRevealedURI).length == 0
        ) revert NullString();
    ...
    modifier maxWalletCheck(uint8 quantity) {
        if (balanceOf(msg.sender) + quantity > maxPerWallet
                    ||
            numberMinted[msg.sender] + quantity > maxPerWallet) revert MaxPerWallet();
        _;
    }

    // ===== Check Supply =====
    modifier supplyCheck(uint8 quantity) {
        if(totalSupply() + quantity > maxSupply) revert TotalSupply();
        if(quantity == 0) revert NullMint();
        _;
    }
    ...
    function privateMint(bytes32[] memory proof, uint8 quantity) external payable maxWalletCheck(quantity) supplyCheck(quantity) callerIsUser {
        if(publicMintStarted || !privateMintStarted) revert PrivateNotAvailable();

        if (!proof.verify(presaleMerkleRoot, keccak256(abi.encodePacked(msg.sender)))) revert NotOnWhitelist();

        if(numberMinted[msg.sender] == 0) {
            if(msg.value < (cost * (quantity - 1))) revert WrongValue();
        } else {
            if(msg.value < cost * quantity ) revert WrongValue();
        }
    ...
    function mint(uint8 quantity) external payable maxWalletCheck(quantity) supplyCheck(quantity) callerIsUser {
        if (!publicMintStarted || privateMintStarted) revert PublicNotAvailable();
        if (msg.value < (cost * quantity)) revert WrongValue();
        ...
        if(totalSupply() + quantity == maxSupply) revealedState = true;
    }
    ...
    // ===== Change Base URI =====
    function setBaseURI(string memory _newBaseURI) external onlyOwner NotNull(_newBaseURI){
        baseURI = _newBaseURI;
    }

    // ===== Change Not Revealed URI =====
    function setNotRevealedURI(string memory _newNotRevealedURI) external onlyOwner NotNull(_newNotRevealedURI) {
    ...
    function sendFunds(uint256 _totalMsgValue) internal {
        ...
        if (!s1 || !s2) revert FailedTransfer();
    }
