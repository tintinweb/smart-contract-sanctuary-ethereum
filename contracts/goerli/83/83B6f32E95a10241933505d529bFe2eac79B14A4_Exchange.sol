...
abstract contract AssetMatcher is Initializable {
    ...
    function _matchAssetOneSide(
        ...
        if (classLeft == 0xaaaebeba) {
            if (classRight == 0xaaaebeba) {
                return leftAssetType;
            }
            if (msg.sender == order.maker && classRight == 0x8ae85d84) {
                return rightAssetType;
            }
            return LibAsset.AssetType(0, "");
        }
        if (classLeft == 0x8ae85d84) {
            if (classRight == 0x8ae85d84) {
                return _simpleMatch(leftAssetType, rightAssetType);
            }
            return LibAsset.AssetType(0, "");
        }
        if (classLeft == 0x73ad2146) {
            if (classRight == 0x73ad2146) {
                return _simpleMatch(leftAssetType, rightAssetType);
            }
            return LibAsset.AssetType(0, "");
        }
        if (classLeft == 0xd8f960c1) {
            if (classRight == 0xd8f960c1) {
                return _simpleMatch(leftAssetType, rightAssetType);
            }
            return LibAsset.AssetType(0, "");
        }
    ...
    PausableUpgradeable
{
    ...
    bytes32 public constant a = keccak256("Input(address maker,Asset makeAsset,address taker,Asset[] takeAssets,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data,uint64 typeMint)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)");
    bytes32 public constant b = keccak256("Order(address maker,Asset makeAsset,address taker,Asset takeAsset,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data,uint64 typeMint)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)");
    bytes32 public constant c = keccak256("AssetType(bytes4 assetClass,bytes data)");
    bytes32 public constant d = keccak256("Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)");
    bytes32 public constant e = keccak256("Part(address account,uint96 value)");
    ...
    function matchOrderExtension(
        ...
        require(input.dataType == 0x4c234266, "Invalid match order");
    ...
    */
    ...
    function matchOrders(
        ...
        require(orderLeft.dataType == 0x4c234266, "Invalid match order");
...
library LibOrder {
    ...
    // keccak256("Input(address maker,Asset makeAsset,address taker,Asset[] takeAssets,uint256 salt,uint256 start,uint256 end,bytes4 dataType,bytes data,uint64 typeMint)Asset(AssetType assetType,uint256 value,address token,uint256 tokenId)AssetType(bytes4 assetClass,bytes data)")
    bytes32 constant INPUT_TYPEHASH =
        0x7035a7a371c23e854ce3f388463162a3246a4b162a8839cefdea1bfd3ae98359;
...
abstract contract TransferExecutor {
    ...
    function _transferToken(
        ...
        if (asset.assetType.assetClass == 0xaaaebeba) {
            to._transferEth(asset.value);
        } else if (asset.assetType.assetClass == 0x8ae85d84) {
    ...
    function _transferNFT(
        ...
        if (asset.assetType.assetClass == 0x73ad2146) {
        ...
        } else if (asset.assetType.assetClass == 0xd8f960c1) {
            require(asset.value == 1, "erc721 value error");
...
abstract contract TransferManager is TransferExecutor {
    ...
    function _getFeeSide(bytes4 make, bytes4 take)
        ...
        returns (FeeSide)
    {
        if (make == 0x8ae85d84 || make == 0xaaaebeba) {
            return FeeSide.MAKE;
        }
        if (take == 0x8ae85d84 || take == 0xaaaebeba) {
            return FeeSide.TAKE;
        }
        return FeeSide.NONE;
    }
