...
import { HDReveal } from "./extensions/HDReveal.sol";
import { HDRoyalty } from "./extensions/HDRoyalty.sol";
import { HDFiat } from "./extensions/HDFiat.sol";
import { HDSecurityUpgradeable } from "./extensions/HDSecurity.sol";
    ...
    HDFiat
{
    ...
    function initialize(
        ...
        __HDRoyalty_init(_royaltyReceiver, _royaltyFeesInBips);
        __HDFiat_init(_priceFeedContract);
     ...
     */
    function revealCollection(string calldata _newUri)
        external
     ...
     */
    function setMintPrice(uint96 _newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
        mintPrice = _newPrice;
     ...
     */
    function mintForAddress(address _receiver, uint256 _mintQuantity) external payable {
     ...
     */
    function adminMint(address _receiver, uint256 _mintQuantity) external onlyModerator {
        mint(_receiver, _mintQuantity);
    }
     ...
     */
    function totalSupply() public view override(ERC721AUpgradeable) returns (uint256) {
        return _totalMinted();
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC721AUpgradeable, HDRoyalty, HDSecurityUpgradeable)
        returns (bool)
    {
...
contract HDReveal {
    ...
    function reveal(string calldata _newUri) internal {
...
import "./HDSecurity.sol";
import "@openzeppelin/contracts-upgradeable/token/common/ERC2981Upgradeable.sol";

abstract contract HDRoyalty is ERC2981Upgradeable, HDSecurityUpgradeable {
    string public contractURI;

    event RoyaltyInfoSet(address receiver, uint256 feesInBips);

    /**********************
     * Initializer Function *
     **********************/
    function __HDRoyalty_init(address _receiver, uint96 _feesInBips)
        internal
        onlyInitializing
    {
        __ERC2981_init();
        setRoyaltyInfo(_receiver, _feesInBips);
    }
     ...
     */
    function setRoyaltyInfo(address _receiver, uint96 _feesInBips)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _setDefaultRoyalty(_receiver, _feesInBips);
        emit RoyaltyInfoSet(_receiver, _feesInBips);
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC2981Upgradeable, HDSecurityUpgradeable)
        returns (bool)
    {
        return super.supportsInterface(interfaceId);
    }
...
import { HDSecurityUpgradeable } from "./HDSecurity.sol";
import "./IHDFiat.sol";

import "@chainlink/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol";

abstract contract HDFiat is HDSecurityUpgradeable, IHDFiat {
    AggregatorV3Interface internal priceFeedInstance;
    address public priceFeedContract;

    function __HDFiat_init(address _priceFeedContract) internal onlyInitializing {
        setFeedContract(_priceFeedContract);
    }

    function setFeedContract(address _priceFeedContract)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        priceFeedContract = _priceFeedContract;
        priceFeedInstance = AggregatorV3Interface(_priceFeedContract);
    }
     ...
     */
    function getConversionRate(uint256 _mintPriceInUSD) public view returns (uint256) {
        (, int256 price, , , ) = priceFeedInstance.latestRoundData();
        uint256 ethAmountInWei = ((1 * 10**26) * _mintPriceInUSD) / uint256(price);
...
interface IHDFiat {
     ...
     * To get latest price of ether in wei to mint an NFT
     */
    function getConversionRate(uint256 _mintPriceInUSD) external view returns (uint256);
}
