...
// File: contracts/nihilium/NihiliumVendor.sol
...
contract NihiliumVendor is Ownable {

  // Our Token Contract
  Nihilium nihilium;

  // token price for ETH
  uint256 public tokensPerEth = 666;

  // Event that log buy operation
  event BuyTokens(address buyer, uint256 amountOfETH, uint256 amountOfTokens);

  constructor(address tokenAddress) {
    nihilium = Nihilium(tokenAddress);
  }
  ...
  */
  function buyTokens() public payable returns (uint256 tokenAmount) {
    ...
    uint256 vendorBalance = nihilium.balanceOf(address(this));
    require(vendorBalance >= amountToBuy, "Vendor contract has not enough tokens in its balance");

    // Transfer token to the msg.sender
    (bool sent) = nihilium.transfer(msg.sender, amountToBuy);
