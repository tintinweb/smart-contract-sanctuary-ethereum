// @author: @gizmolab_
    ...
    contract DoriStaking is Ownable {
        ...
        address public doriGen2Contract;
        ...
        function burnNfts(uint256[] calldata tokenIds) external {
            ...
            require(doriGen2Contract != address(0), "DoriGen2 Contract not set");
            IERC721 nftContract = IERC721(doriGen2Contract);
            ...
            for (uint256 i; i < tokenIds.length; i++) {
                ...
                for (
                    uint256 j;
                    j < userStakeTokens[msg.sender][doriGen2Contract].length;
                    j++
                ) {
                    if (
                        userStakeTokens[msg.sender][doriGen2Contract][j] ==
                        tokenIds[i]
                    ) {
                        isTokenOwner = true;
                        tokenIndex = j;
                ...
                uint256 reward = _calculateReward(msg.sender, doriGen2Contract);
                burnClaimed[msg.sender] += reward;

                delete vault[doriGen2Contract][tokenIds[i]];
                totalStaked--;

                userBurnTokens[msg.sender][doriGen2Contract].push(tokenIds[i]);
                userStakeTokens[msg.sender][doriGen2Contract][
                    tokenIndex
                ] = userStakeTokens[msg.sender][doriGen2Contract][
                    userStakeTokens[msg.sender][doriGen2Contract].length - 1
                ];
                userStakeTokens[msg.sender][doriGen2Contract].pop();
            }
        }
