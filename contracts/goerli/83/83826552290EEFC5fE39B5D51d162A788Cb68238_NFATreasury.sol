...
contract NFATreasury is ReentrancyGuard {
   ...
   function buyTokenWithEth(address _tokenToBuy) public payable onlyOwner {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = _tokenToBuy;

        router.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: msg.value }(5, path, address(this), block.timestamp + 15);
   }

   function sellAllTokensForETH(address _tokenToSell) public onlyOwner {
