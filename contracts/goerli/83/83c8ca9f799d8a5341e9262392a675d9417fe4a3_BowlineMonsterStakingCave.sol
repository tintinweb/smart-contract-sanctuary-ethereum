    ...
    ERC1820Implementer
{
    ...
    uint256 public rewardPeriodInSeconds = 3600;

    // Custom Bonus Reward per Epoch
    mapping(uint256 => mapping(address => mapping(uint256 => bool)))
        public epochBonusRewardsForToken;
    uint256 public currentEpoch = 0;
    uint256 public bonusRewardPercentage = 20;
    ...
    event StakedNFT(StakedToken stakedToken, address staker);
    ...
    // TODO should also send back all tokens?
    ...
    function setAdditionalRewardsForTokens(
        ...
        uint256 tokenCounter = _tokenIds.length;
    ...
    function setBonusTokensForEpoch(
        IERC721 _nftContract,
        uint256[] memory _tokenIds,
        uint256 _epoch
    ) external onlyOwner {
        ...
        require(
            _epoch > currentEpoch,
            "MonsterStakingCave: Epoch must be in the future"
        );

        uint256 tokenCounter = _tokenIds.length;
        for (uint256 i; i < tokenCounter; ++i) {
            epochBonusRewardsForToken[_epoch][address(_nftContract)][
                _tokenIds[i]
            ] = true;
        }
    }
    ...
    function estimateRewardsForToken(
        ...
        return rewardsForTokenInEpoch(nftContractAddress, tokenId, epoch);
    }
    ...
    function estimateRewardsForToken(
        ...
        return
            rewardsForTokenInEpoch(nftContractAddress, tokenId, currentEpoch);
    }

    function rewardsForTokenInEpoch(
        address nftContractAddress,
        uint256 tokenId,
        uint256 epoch
    ) internal view returns (uint256) {
        uint256 rewardsForToken = rewardsForContract[nftContractAddress];
        rewardsForToken += additionalRewardsForToken[nftContractAddress][
            tokenId
        ];
        if (epochBonusRewardsForToken[epoch][nftContractAddress][tokenId]) {
            rewardsForToken =
                (rewardsForToken * (100 + bonusRewardPercentage)) /
                100; // 20% Bonus
        }
        // rewards from community wallet maybe stand alone calculation
        ...
        )
    {
        ...
        uint256 unaccountedRewards = calculateUnaccountedRewards(
            _stakerAddress
        );
        return (
            staker.totalTokensStaked,
            staker.unclaimedRewards + unaccountedRewards,
            staker.lifetimeRewards + unaccountedRewards
        );
    }
        ...
        returns (uint256 _rewards)
    {
        ...
        for (uint256 i; i < stakedTokensCount; ++i) {
            uint256 tokenRewards = (((block.timestamp -
                stakedTokens[i].lastRewardedAt) *
                rewardsForTokenInEpoch(
                    stakedTokens[i].contractAddress,
                    stakedTokens[i].tokenId,
                    currentEpoch
    ...
    function rewardAllStakers() internal {
        uint256 currentStakerCount = currentStakers.length;
        for (uint256 i; i < currentStakerCount; ++i) {
            rewardStaker(currentStakers[i]);
        }
    }
    ...
    function withdrawalAllTokens() external nonReentrant {
        rewardStaker(msg.sender);
        StakedToken[] memory stakedTokens = stakedTokensForAddress[msg.sender];
        uint256 stakedTokensCount = stakedTokens.length;
        require(stakedTokensCount > 0, "MonsterStakingCave: No Tokens found");
        for (uint256 i; i < stakedTokensCount; ++i) {
            ejectToken(
                stakedTokens[i].contractAddress,
                stakedTokens[i].tokenId
    ...
    function withdrawalToken(address nftContractAddress, uint256 tokenId)
        external
        nonReentrant
    {
        require(
        ...
        rewardStaker(msg.sender);
        ejectToken(nftContractAddress, tokenId);
    }

    function ejectToken(address nftContractAddress, uint256 tokenId) internal {
        require(
            stakedTokenOwner[nftContractAddress][tokenId] == msg.sender,
        ...
        stakers[msg.sender].totalTokensStaked--;
        if (stakers[msg.sender].totalTokensStaked == 0) {
            for (uint256 i; i < currentStakers.length; ++i) {
        ...
        stakedTokenOwner[msg.sender][tokenId] = address(0);

        for (uint256 i; i < stakedTokensForAddress[msg.sender].length; ++i) {
            if (
                stakedTokensForAddress[msg.sender][i].contractAddress ==
                nftContractAddress &&
                stakedTokensForAddress[msg.sender][i].tokenId == tokenId
            ) {
    ...
    function claimRewards() external nonReentrant {
        rewardStaker(msg.sender);
        require(stakers[msg.sender].unclaimedRewards > 0, "MonsterStakingCave: Nothing to claim");
    ...
    function onERC721Received(
        ...
        require(
            nftContracts[msg.sender] == IERC721(msg.sender),
            "MonsterStakingCave: Unkown Contract"
        );

        if (stakers[tokenOwnerAddress].totalTokensStaked > 0) {
            rewardStaker(tokenOwnerAddress);
            stakers[tokenOwnerAddress].totalTokensStaked++;
        } else {
            stakers[tokenOwnerAddress] = Staker(
                stakers[tokenOwnerAddress].unclaimedRewards,
                stakers[tokenOwnerAddress].lifetimeRewards,
                1,
                block.timestamp
            );
            currentStakers.push(tokenOwnerAddress);
        }

        stakedTokenOwner[msg.sender][tokenIdentifier] = tokenOwnerAddress;
