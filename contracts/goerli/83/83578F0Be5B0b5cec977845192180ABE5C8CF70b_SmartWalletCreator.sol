...
contract SmartWalletCreator{
	...
	function createSmartWallet(address owner, address manager, SmartWallet implementation) public returns (address) {
		Proxy sWallet = new Proxy(owner, manager, implementation);
        emit SmartWalletCreate(sWallet._contractAddr());
        (bool success,) = sWallet._contractAddr().call{value: 0}(abi.encodeWithSelector(implementation.initialize.selector, owner, manager));
		require(success);
