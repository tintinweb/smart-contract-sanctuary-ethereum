...
contract MarketPlaceV3 is Ownable2Step {

    using SafeMath for  uint256;

    address payable public commissionRecipient;
    uint256 public commissionFactor;
    uint256 public commissionMaxLimit = 1000;
    //uint256 public maxRoyalityFactor;

    constructor (address payable _commisionRecipient, uint256 _commissionFactor) {
        require (_commissionFactor <= commissionMaxLimit, "Cannot change commiccion more than 10%");
        commissionRecipient = _commisionRecipient;
        commissionFactor = _commissionFactor;
    }
    ...
    virtual external{
        ...
        //(address royaltyRecipient, uint256 royaltyAmount) = IERC2981(tokenAddress).royaltyInfo(tokenId, saleItem.price); // Add Royality to market Items
    ...
    */
    ...
    function buyItem(uint256 tokenId, address tokenAddress) payable external
    ItemExists(tokenAddress, tokenId)
    HasTransferApproval(tokenAddress, tokenId) {
        ...
        (address royaltyRecipient, uint256 royaltyAmount) = IERC2981(tokenAddress).royaltyInfo(tokenId, saleItem.price);
        ...
        IERC721(tokenAddress).safeTransferFrom(saleItem.seller, msg.sender, saleItem.tokenId);
    ...
    function withdrawItem(uint256 tokenId, address tokenAddress) external
    ItemExists(tokenAddress, tokenId)
    OnlyTokenOwner(tokenAddress, tokenId) {
        MarketItem memory saleItem = openSales[tokenAddress][tokenId];
        delete openSales[tokenAddress][tokenId];
    ...
    */
    ...
    function updateItem(uint256 tokenId, address tokenAddress, uint256 newPrice, address newEligibleAddress) external
    ItemExists(tokenAddress, tokenId)
    OnlyTokenOwner(tokenAddress, tokenId) {
        MarketItem memory saleItem = openSales[tokenAddress][tokenId];
    ...
    /**
     * @dev Royalaty Factor Update.
     * @param tokenAddress - Address of the NFT contract
     * @param tokenId - ID of the NFT
     * @param newRoyalityFactor - New Royality Factor for NFT
     */

    // function setRoyalityFactor (address tokenAddress, uint256 tokenId, uint256 newRoyalityFactor) external
    // ItemExists(tokenAddress, tokenId)
    // OnlyTokenOwner(tokenAddress, tokenId) {

    //     MarketItem memory saleItem = openSales[tokenAddress][tokenId];

    //     //require (newRoyalityFactor <= maxRoyalityFactor);
    //     (address royaltyRecipient, uint256 royaltyAmount) = IERC2981(tokenAddress).royaltyInfo(tokenId, saleItem.price);
    //     IERC2981(tokenAddress)._setTokenRoyalty(tokenId, royaltyRecipient, newRoyalityFactor);
