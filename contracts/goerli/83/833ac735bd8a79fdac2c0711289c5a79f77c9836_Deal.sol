...
contract Deal is IDeal, DealChores {
    ...
    function exitRoom(bytes32 roomId) external whenNotPaused {
        delete rooms[roomId].offers[_msgSender()];
        emit RoomExited(roomId, _msgSender());
    }

    function closeRoom(bytes32 roomId) external whenNotPaused hostOnly(roomId) {
        _closeRoom(roomId);
    }
    ...
    function _swapTokens(
        ...
        for (uint256 i = 0; i < offer.erc20Tokens.length; i++) {
            offer.erc20Tokens[i].transferFrom(
                from,
                address(this),
                offer.erc20TokenAmounts[i]
            );
            offer.erc20Tokens[i].transfer(to, offer.erc20TokenAmounts[i]);
        }
        ...
        returns (bool)
    {
        ...
        return
            keccak256(abi.encode(offer)) == keccak256(abi.encode(idealOffer));
    }
...
interface IDeal {
    ...
    error ERoomExists();
    error ERoomDoesNotExist();
    error ERoomNotJoined();
    error ERoomAlreadyJoined();
    error EOfferEmpty();
    error EOfferExpired();
    error EOfferInvalid();
    error EActionUnauthorized();
}
