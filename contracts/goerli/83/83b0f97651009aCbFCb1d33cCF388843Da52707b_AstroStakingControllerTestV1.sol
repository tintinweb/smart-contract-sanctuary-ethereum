...
import "../AstroStakingControllerV1.sol";

contract AstroStakingControllerTestV1 is AstroStakingControllerV1 {

  uint256 public secondsPerDay;

  function initialize(address _tinyAstro, address _astroToken, address _emissionRateManager) public virtual override initializer {
    secondsPerDay = 5 minutes;
    AstroStakingControllerV1.initialize(_tinyAstro, _astroToken, _emissionRateManager);
  }

  function overrideSecondsPerDay(uint256 value) external onlyOwner {
    secondsPerDay = value;
  }

  function _secondsPerDay() internal view override returns (uint256) {
...
contract AstroStakingControllerV1 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
  ...
  function _unstake(uint256 tokenId) internal returns (uint256 amountToMint) {
    ...
    emit Unstaked(msg.sender, tokenId, amountToMint);
  }
