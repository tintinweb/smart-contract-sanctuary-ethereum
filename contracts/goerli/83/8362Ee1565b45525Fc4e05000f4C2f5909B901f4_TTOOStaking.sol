...
import {ERC721Holder} from "@openzeppelin/[email protected]/token/ERC721/utils/ERC721Holder.sol";
...
contract TTOOStaking is Ownable, ERC721Holder {
     ...
     */
    function stakeMultiple(StakeMultipleInputs[] memory inputs) external {
     ...
     */
    function removeMultipleLockTimes(uint256[] calldata times)
        external
        onlyOwner
    {
    ...
    function _stake(uint256 tokenId, uint256 lockTime) internal {
        ...
        address staker = nft.ownerOf(tokenId);

        // Check caller is owner
        if (staker != msg.sender) revert TTOOStaking__NotNFTOwner();

        // Check the NFT is not currently being staked
        if (lockInfos[tokenId].owner != address(0))
            revert TTOOStaking__AlreadyStaked();
        ...
        lockInfos[tokenId].owner = staker;
        lockInfos[tokenId].unlockAt = _unlockAt;

        // Update staker staked tokenIds
        stakedTokenIds[staker].add(tokenId);
        ...
        emit Stake(staker, tokenId, _unlockAt);
    }
     ...
     */
    function getUserStakedCount(address staker)
        external
        view
        returns (uint256)
    {
     ...
     * @param   staker     Address of the staker to return info for
     */
    function getUserStakingInfo(address staker)
        external
        view
    ...
    /// @notice Emitted when user tries to stake a tokenId already staked
    error TTOOStaking__AlreadyStaked();
    ...
    /// @notice Emitted when owner tries to remove all lockTimes from the set
