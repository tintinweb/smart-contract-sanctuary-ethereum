...
interface IDAO {
    function registerToken(uint256 tokenId) external payable;
}

contract MemberToken is ERC721, ERC721Enumerable, Pausable, Ownable, EIP712, ERC721Votes {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    uint public constant MAXSUPPLY = 100;
    uint public constant COST = 0.1 ether;

    IDAO public daoContract;

    constructor() ERC721("MemberToken", "MT") EIP712("MemberToken", "1") {}
    ...
    function updateDaoAddress(address _daoAddress) onlyOwner external {
        require(address(daoContract) == address(0), "Address already set");
        daoContract = IDAO(_daoAddress);
    }

    function safeMint(address to) public payable whenNotPaused {
        require(address(daoContract) != address(0), "Not ready to be minted");
        ...
        daoContract.registerToken{value: msg.value}(tokenId);
    }
