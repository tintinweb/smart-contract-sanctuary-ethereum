...
contract MockFaucet2 is Initializable {
    ...
    function initialize(address _underlying) public initializer {
        underlying = MockUnderlying(_underlying);
    }

    function mint() external {
        underlying.faucetMint(msg.sender, 100e18);
    }
}
...
import "./MockFaucet2.sol";

error MockUnderlyingError(string reason);
...
contract MockUnderlying is ERC20Upgradeable, Ownable2StepUpgradeable {
    ...
    /**
     * @notice Mints amount of tokens to receiver. Only faucet can call this function.
     */
    function faucetMint(address receiver, uint256 amount) external {
        if (msg.sender != faucet) {
            revert MockUnderlyingError("Only faucet can call this function");
        }
        _mint(receiver, amount);
    }
    ...
    /**
     * @notice Gets the faucet address.
     */
