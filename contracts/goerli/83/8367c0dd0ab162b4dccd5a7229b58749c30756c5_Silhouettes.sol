...
contract Silhouettes is ERC721Enumerable, Ownable 
{
    ...
    uint public perAddressLimitSL = 2;
    uint public perAddressLimitWL = 2;
    uint public perAddressLimitPublic = 2;
    ...
    function mintSL(uint256 amount, bytes32[] memory proof) external payable
    {
        ...
        require(addressMintedBalance[msg.sender] + amount <= perAddressLimitSL, "Max NFT per address exceeded");
...
function mintWL(uint256 amount, bytes32[] memory proof) external payable
    {
        require(!whitelist || verify(proof), "Address not whitelisted");
        require(msg.sender == tx.origin, "No transaction from smart contracts!");
        require(preSaleIsActive || saleIsActive, "Sale must be active to mint");
        ...
        require(totalSupply() + amount <= MAX_TOKENS - (NUMBER_RESERVED_TOKENS - reservedTokensMinted), "Purchase would exceed max supply");
        require(msg.value >= PRICE * amount, "Not enough ETH for transaction");
        require(addressMintedBalance[msg.sender] + amount <= perAddressLimitWL, "Max NFT per address exceeded");
    ...
    function mintPublic(uint256 amount) external payable
    {
        ...
        require(totalSupply() + amount <= MAX_TOKENS - (NUMBER_RESERVED_TOKENS - reservedTokensMinted), "Purchase would exceed max supply");
        require(msg.value >= PRICE * amount, "Not enough ETH for transaction");
        require(addressMintedBalance[msg.sender] + amount <= perAddressLimitPublic, "Max NFT per address exceeded");
