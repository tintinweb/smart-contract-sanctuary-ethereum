...
interface ILiquidityPool {
    /// @notice Function to stake native tokens on the Voyager
    ...
    /// @param isNative if the token being unstaked is native token for the chain
    function unstake(
        address tokenAddress,
        uint256 amount,
        bool isNative
...
contract VoyagerUtils is ContextUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, ILiquidityPool, IVoyagerUtils {
    ...
    event Unstake(address unstaker, address token, uint256 amount);
    ...
    function isContractWhitelisted(address tokenAddress) external view override returns (bool) {
    ...
    function isBurnable(address tokenAddress) external view override returns (bool) {
        return _burnList[tokenAddress];
    }

    /// @notice Function to stake native tokens on the Voyager
    function stakeNative() external payable override {
        uint256 _amount = msg.value;
        address _weth = _WETH;
        require(msg.value > 0, "stake: insufficient native tokens provided");
        IWETH(_weth).deposit{ value: _amount }();
        require(IWETH(_weth).transfer(address(_reserve), _amount));
        _reserve.stakeNative(msg.sender, _weth, _amount);

        emit Stake(msg.sender, _weth, _amount);
    }

    /// @notice Function to stake tokens on the Voyager
    /// @param tokenAddress address of the token for which lp needs to be added
    /// @param amount amount of the token
    function stake(address tokenAddress, uint256 amount) public virtual override {
        _reserve.stake(msg.sender, tokenAddress, amount);

        emit Stake(msg.sender, tokenAddress, amount);
    }
    ...
    function unstake(
        ...
        if (isNative) {
            _reserve.unstakeNative(msg.sender, tokenAddress, amount, _WETH);
            return;
        }
        _reserve.unstake(msg.sender, tokenAddress, amount);

        emit Unstake(msg.sender, tokenAddress, amount);
    }
    ...
    /// @dev Can only be called by the RESOURCE_SETTER
    /// @param reserveAddress Address of reserve handler.
    function setReserve(address reserveAddress) public virtual override onlyRole(RESOURCE_SETTER) {
        _reserve = IHandlerReserve(reserveAddress);
    }

    /// @notice Function to whitelist a token for swaps
    /// @dev Can only be called by the RESOURCE_SETTER
    /// @param contractAddress Address of contract to be called when a deposit is made and a deposited is executed.
    function setResource(address contractAddress, bool isResource) public virtual override onlyRole(RESOURCE_SETTER) {
    ...
    /// @param contractAddress Address of contract to be used when making or executing deposits.
    /// @param status Boolean flag to change burnable status.
    function setBurnable(address contractAddress, bool status) public virtual override onlyRole(RESOURCE_SETTER) {
        _setBurnable(contractAddress, status);
    }

    function setLiquidityPool(address contractAddress, address lpAddress)
        public
        virtual
        override
    ...
    function setLiquidityPoolOwner(
        ...
        address tokenAddress,
        address lpAddress
    ) public virtual override onlyRole(RESOURCE_SETTER) {
    ...
    function _setResource(address contractAddress, bool isResource) internal virtual {
