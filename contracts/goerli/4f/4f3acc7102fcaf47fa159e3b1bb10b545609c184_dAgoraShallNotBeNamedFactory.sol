...
import { ShallNotBeNamed } from "../dAgoraWizard/ShallNotBeNamed.sol";
import '../IdAgoraMembership.sol';

contract dAgoraShallNotBeNamedFactory is Ownable, ReentrancyGuard {
    ShallNotBeNamed noName;

    struct Deploys {
        address[] ContractAddress;
        address Owner;
        uint256 shallNotBeNamedIndex;
    }

    address public dAgoraMembership;
    uint256 public shallNotBeNamedCount = 0;
    bool public paused;

    mapping(address => Deploys) private _deployedContracts;
    mapping(address => uint256) public _addressDeployCount;

    event ShallNotBeNamedCreated(
        address shallNotBeNamedContract, 
        address owner, 
        uint256 shallNotBeNamedIndex,
        uint256 shallNotBeNamedCount
    );
    ...
    function createShallNotBeNamed(
        string memory _name,
        ...
        nonReentrant
    {
        ...
        shallNotBeNamedCount++;
        _addressDeployCount[msg.sender]++;
        _deployedContracts[msg.sender].ContractAddress.push(address(noName));
        _deployedContracts[msg.sender].Owner = msg.sender;
        _deployedContracts[msg.sender].shallNotBeNamedIndex = shallNotBeNamedCount;
        
        emit ShallNotBeNamedCreated(
            address(noName),
            msg.sender, 
            _addressDeployCount[msg.sender],
            shallNotBeNamedCount
...
contract ShallNotBeNamed is ERC721, Ownable {
    ...
    {
        require(
            _amount + presaleMintBalance[msg.sender] <= maxWhiteListAmount,
    ...
    function setMaxWhiteListAmount(uint256 _newMaxWhiteListAmount) public onlyOwner {
        maxWhiteListAmount = _newMaxWhiteListAmount;
    }
