...
contract LimitlessCollection is ERC721, ERC721URIStorage, Pausable, Ownable {
    ...
    ) ERC721("LimitlessCollection", "NFT") {
        baseURI = _baseURI;
    ...
    modifier whenDistributionEnded() {
        require(distributionPhase == DistributionPhase.ended, 'Distribution not ended');
        _;
    }
    ...
    function buyNFT(uint256 quantity, bytes32[] calldata _merkleProof) public payable whenNotPaused {
        ...
        if (distributionPhase == DistributionPhase.preSale) {
            require(balanceOf(msg.sender) == 0 && quantity == 1, 'On pre sale you can only buy one token');
            _requireWhitelisted(_merkleProof);
        }
    ...
    function activateNextDistributionPhase() public onlyOwner whenNotPaused {
    ...
    function reveal(string memory _baseURI) public onlyOwner whenNotPaused whenDistributionEnded {
        require(!isReveald, 'NFTs already reveald');
        baseURI = _baseURI;
        isReveald = true;
    }

    function _requireWhitelisted(bytes32[] calldata _merkleProof) private view {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        bool isWhitelisted = MerkleProof.verify(_merkleProof, whitelistMerkleTreeRoot, leaf);
        require(
            isWhitelisted,
            "Not whitelisted"
        );
    ...
    function withdraw(address to) public onlyOwner whenDistributionEnded {
        require(aeFeeClaimed, 'AE fee must be claimed first');
        payable(to).transfer(address(this).balance);
    }

    function withdrawAEFee(address to) public onlyOwner whenDistributionEnded {
