...
contract Stakable is Ownable {
    ...
    function _stake(
        ...
        uint256 index = _stakes[_msgSender()];
        ...
        //     uint256 sinceBlock;
        // }
        _stakeholders[index].userStakes.push(
            Stake(
                _msgSender(),
                amountLBSKR,
                amountBSKR,
     ...
     */
    // function _withdrawStake(uint256 user_index, Stake memory current_stake, uint256 amount, // TODO may be this should be percentage
    // uint256 index)
    //     internal
    //     // returns (uint256)
    // {
    ...
    //     // Stake memory current_stake = _stakeholders[user_index].userStakes[index];
    ...
    //     require(
    //         current_stake.amountLBSKR >= amount,
    ...
    //     current_stake.amountLBSKR = current_stake.amountLBSKR - amount;
    //     // If stake is empty, 0, then remove it from the array of stakes
    //     if (current_stake.amountLBSKR == 0) {
    //         delete _stakeholders[user_index].userStakes[index];
    //     } else {
    //         // If not empty then replace the value of it
    //         _stakeholders[user_index].userStakes[index].amountLBSKR = current_stake
    //             .amountLBSKR;
    //         // Reset timer of stake
    //         _stakeholders[user_index].userStakes[index].sinceBlock = block
    //             .timestamp;
    //     }
    ...
    //         0,
    //         _stakeholders[_stakes[_staker]].userStakes
    //     );
...
// Stake // function _creditInflation(uint256 inflation, uint256 nowTS) private {
// Stake function _calcInflation(uint256 nowTS) - ok
// Stake function _creditInflation() private { - ok
// Stake function _startInflation() private { - ok
// Stake function _swapTokensForTokens(uint256 tokenAmount) private { - ok
// Stake function createStake(uint256 stakeAmount) - ok
// Stake function getBSKRAddress() external view returns (address) { - ok
// Stake function getBSKRPerShare() - ok
// Stake function getCurrentRewards() - ok
// Stake function getTotalShares() external view returns (uint256) { - ok
// Stake function getTotalStakeholders() external view returns (uint256) { - ok
// Stake function getTotalStakes() external view returns (uint256) { - ok
...
// Stake function removeStake(uint256 stakeAmount) external whenNotPaused { - ok
// Stake function rewardForBSKR(address stakeholder, uint256 bskrAmount)
// Stake function rewardOf(
// Stake function setBSKRAddress(address newBSKRAddr) external onlyManager { - ok
// Stake function setInitialRatio(address newBSKRAddr, uint256 stakeAmount) - ok
// Stake function sharesOf(address stakeholder) external view returns (uint256) {
// Stake function stakeOf(address stakeholder) external view returns (uint256) {
// Stake function swapTokensForTokensExt(uint256 tokenAmount) external onlyManager {
// Stake function testCalcInflation(uint256 nowTS)
contract LBSKR is BaseBSKR, DSMath, Stakable {
     ...
     */
    function createStake(uint256 amountLBSKR)
        external
        whenNotPaused
        isInitialRatioSet
    {
     ...
     */
    // TODO sort
    function getBSKRPerShare() external view isInitialRatioSet returns (uint256) {
        // require(_BSKRAddr != address(0), "LBSKR: BSKR address not set");
        ...
        )
    {
        ...
        return (balanceOf(address(this)) + inflation) / _totalLBSKRShares; // TODO do we subtract _totalLBSKRStakes from numerator?
    }
     ...
     */
    function removeStake(uint256 stakeAmount, uint256 index)
        external
        isInitialRatioSet
        whenNotPaused
    {
        ...
        uint256 user_index = _stakes[_msgSender()];
        Stake memory current_stake = _stakeholders[user_index].userStakes[
            index
        ];
        ...
        emit Debug05L(
            "before",
            _msgSender(),
            current_stake.amountLBSKR,
            current_stake.amountBSKR,
            current_stake.sharesLBSKR,
            current_stake.sharesBSKR,
            current_stake.sinceBlock
        );

        // require(
        //     current_stake.amountLBSKR >= stakeAmount,
        //     "Staking: Cannot withdraw more than you have staked"
        // );

        // // uint256 lbskrStkRatio = (current_stake.amountLBSKR) /
        // //     current_stake.sharesLBSKR;
        // // uint256 bskrStkRatio = (current_stake.amountBSKR) /
        // //     current_stake.sharesBSKR;
        ...
        // uint256 lbskrShares2Deduct = (stakeAmount * current_stake.sharesLBSKR) /
        //     current_stake.amountLBSKR;
        // uint256 bskrAmount2Deduct = (stakeAmount * current_stake.amountBSKR) /
        //     current_stake.amountLBSKR;
        // uint256 bskrShares2Deduct = (stakeAmount * current_stake.sharesBSKR) /
        //     current_stake.amountLBSKR;

        // // If stake is empty, 0, then remove it from the array of stakes
        // if ((current_stake.amountLBSKR - stakeAmount) == 0) {
        //     delete _stakeholders[user_index].userStakes[index];
        // } else {
        //     // If not empty then replace the value of it
        //     _stakeholders[user_index]
        //         .userStakes[index]
        ...
        //         "after",
        //         _msgSender(),
        //         _stakeholders[user_index].userStakes[index].amountLBSKR,
        //         _stakeholders[user_index].userStakes[index].amountBSKR,
        //         _stakeholders[user_index].userStakes[index].sharesLBSKR,
        //         _stakeholders[user_index].userStakes[index].sharesBSKR,
        //         _stakeholders[user_index].userStakes[index].sinceBlock
        //     );
        // }
        ...
        //         (block.timestamp - current_stake.sinceBlock) /
        //             _SECS_IN_FOUR_WEEKS
        //     );
        ...
        //     lbskrShares2Deduct -
        //     stakeAmount) * penaltyFactor) / _BIPS; // stakeAmount never existed - it's a zombie
     ...
     */
    // function rewardOf(
    //     address stakeholder // view TODO enable later
