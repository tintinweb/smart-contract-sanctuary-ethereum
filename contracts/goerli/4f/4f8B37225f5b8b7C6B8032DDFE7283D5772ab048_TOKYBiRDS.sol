...
contract TOKYBiRDS is ERC721A, Ownable, CantBeEvil, ReentrancyGuard {
  ...
  constructor() ERC721A("Testbird", "TESTB") CantBeEvil(LicenseVersion.PERSONAL) {}
  
  mapping(address => bool) private allowList;
  mapping(address => bool) private internalList;
  mapping(address => uint256) private mintCountMap;
  uint256 private constant MINT_LIMIT_PER_WALLET = 9;
  uint256 private constant MINT_LIMIT_PER_TX = 9;

  uint256 public constant MAX_SUPPLY = 3000;  
  bool public saleIsActive = true;
  bool public saleIsAllowlist = false;
  bool public saleIsPublic = false;
  uint256 private price = 100000000000000; // 0,0001eth
  string private customBaseURI = "https://neet-to-setup.dev/";

  /** functions **/
  function mint(uint256 count) public payable nonReentrant {
    require(saleIsActive, "Sale not active");
    require(totalSupply() < MAX_SUPPLY, "Exceeds max supply");
    require(count <= MINT_LIMIT_PER_TX, "max number of mint per transaction are 3");
    require(msg.value <= ( price * count ), "Insufficient payment, 0.01 ETH per item * your count of token");
    
    if(
        internalList[msg.sender] && !saleIsPublic || 
        allowList[msg.sender] && saleIsAllowlist || 
        saleIsPublic) {
  ...
  function withdraw() public nonReentrant onlyOwner {
    ...
    Address.sendValue(payable(owner()), balance * 800 / 1000);
