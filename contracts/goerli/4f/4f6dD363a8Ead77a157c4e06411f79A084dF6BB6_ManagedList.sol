...
contract ManagedList is ReentrancyGuard, Ownable {
    ...
    function createBatch(string memory _externalId, string memory _name, uint256 _price, uint256 _maxSpots, uint256 _spotsPerWallet, address _paymentAddress, bytes memory _signature) public payable isValidMessage(_signature,  _externalId, "create") ensureUniqueBatchExternalId(_externalId) onlyOwner nonReentrant {
        ...
        require(msg.value >= priceToCreateBatch, "Not enough ETH sent");
    ...
    function updatePaymentAddress(uint256 _batchId, address _paymentAddress) public {
        require(_paymentAddress != address(0), "Payment address cannot be 0");
        require(batchesList[_batchId].creator == msg.sender || msg.sender == owner(), "You must be the creator of this batch to add a payment address");
        batchesList[_batchId].paymentAddress = _paymentAddress;
    }
    ...
    function reserveSpot(uint256 _batchId, uint8 _requestedSpotQuantity, bytes memory _signature) public payable isValidMessage(_signature, "reserve", "reserve") nonReentrant {
        uint256 ownedAndRequested = whitelist[_batchId][msg.sender] + _requestedSpotQuantity;
        ...
        whitelist[_batchId][msg.sender] += _requestedSpotQuantity;
        batchesList[_batchId].spotsTaken += _requestedSpotQuantity;
//        address payable _to = payable(batchesList[_batchId].paymentAddress);
//        uint256 amount = msg.value * (100 - batchesList[_batchId].fee) / 100;
        (bool success, ) = payable(batchesList[_batchId].paymentAddress).call{value: (msg.value * (100 - batchesList[_batchId].fee) / 100)}("");
        ...
        emit SpotReserved(Spot({
            ...
            spotsTakenByWallet: whitelist[_batchId][msg.sender],
    ...
    function checkMyWhitelistSpots(uint256 _batchId) public view returns (uint256) {
        return whitelist[_batchId][msg.sender];
    }

    function checkWhitelistSpotsForAddress(uint256 _batchId, address _address) public view returns (uint256) {
        return whitelist[_batchId][_address];
    }

    function updateWalletAddress(uint256 _batchId, address _newAddress) public nonReentrant {
        require(whitelist[_batchId][msg.sender] > 0, "You must own at least 1 spot to update your address");
        whitelist[_batchId][_newAddress] = whitelist[_batchId][msg.sender];
        delete whitelist[_batchId][msg.sender];
    }
