...
contract FxStateRootTunnel is FxBaseRootTunnel, Const {
    ...
    event Error1(string msg, uint8 no);
    event Error2(bytes msg, uint8 no);
    event Succuess(uint8 no);

    constructor(address _checkpointManager, address _fxRoot, address _bridge) FxBaseRootTunnel(_checkpointManager, _fxRoot) {
    ...
    function _processMessageFromChild(bytes memory message, bytes memory inputData) internal override {
        ...
        try manager.exit(inputData) {
            ...
            emit Error1(reason, 2);
        }  catch (bytes memory reason) {
            emit Error2(reason, 2);
        }

        try manager.exit(message) {
            emit Succuess(1);
        } catch Error(string memory reason) {
            emit Error1(reason, 1);
        }  catch (bytes memory reason) {
            emit Error2(reason, 1);
        }
    }
...
abstract contract FxBaseRootTunnel {
    ...
    function receiveMessage(bytes memory inputData) public virtual {
        ...
        _processMessageFromChild(message, inputData);
    }
     ...
     */
    function _processMessageFromChild(bytes memory message, bytes memory inputData) internal virtual;
}
