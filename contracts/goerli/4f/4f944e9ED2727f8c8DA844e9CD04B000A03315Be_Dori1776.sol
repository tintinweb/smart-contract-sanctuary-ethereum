    ...
    DefaultOperatorFilterer
{
    ...
    */
    ...
    function burn(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            require(
                ownerOf(i) == msg.sender || owner() == msg.sender,
                "You must own a token in order to burn it."
            );
            if (lockStatus[i]) {
                _unlockToken(i);
            }

            userBurnTokens[msg.sender].push(i);
            burnData[i] = block.timestamp;
            transferFrom(msg.sender, burnAddress, i);
            emit BurnToken(i, block.timestamp, msg.sender);
        }
    }
    ...
    */
    ...
    function _unlockToken(uint256 tokenId) internal {
        ...
        lockData[tokenId] = 0;
        // remove token from userLockedTokens
    ...
    {
        uint256 reward = 0;
        for (uint256 i; i < userLockedTokens[_user].length; i++) {
            uint256 token = userLockedTokens[_user][i];
            uint256 timeSinceStake = block.timestamp - lockData[token];
            reward += timeSinceStake * lockReward * 1e18;
        }
        return reward / 86400;
    }
