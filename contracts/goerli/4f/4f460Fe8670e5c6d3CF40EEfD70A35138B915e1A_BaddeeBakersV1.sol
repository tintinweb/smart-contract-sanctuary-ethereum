...
contract BaddeeBakersV1 is ERC721A, Ownable {
    ...
    constructor() ERC721A("BaddeeBakers", "BaddeeBakers"){
        ...
        freeStartTime = block.timestamp;
        freeEndTime = block.timestamp + 72 * 60 * 60;
        freePrice = 0 ether;
        freePerUserMint = 1;
        freeMaxCount = 88;

        wlStartTime = block.timestamp;
        wlEndTime = block.timestamp + 72 * 60 * 60;
        wlPrice = 0.01 ether;
        wlPerUserMint= 1;
        wlMaxCount = 5000;
    ...
    function setPublicMint(uint256 _publicStartTime,uint256 _publicPrice) external onlyOwner {
    ...
    function withdrawTo(address beneficiary) external onlyOwner {
    ...
    function wlMint( bytes32 root,bytes32[] calldata proof,uint256 quantity) external payable callerIsUser {
    ...
    function totalMinted() public view callerIsUser returns(uint256) {
        return _totalMinted();
    }
    ...
    returns (
        ...
        return MintSettings(freeStartTime,freeEndTime,freePrice,freePerUserMint,freeMaxCount,wlStartTime,wlEndTime,wlPrice,wlPerUserMint,wlMaxCount,publicStartTime,publicPrice);
    }
