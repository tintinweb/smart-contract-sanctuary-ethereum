...
contract Hypervisor is IUniswapV3MintCallback, ERC20Permit, ReentrancyGuard {
   ...
   event Deposit(
    ...
    function checkPrice() view external returns(uint256, uint256) {
        uint160 sqrtPrice = TickMath.getSqrtRatioAtTick(currentTick());
        uint256 price = FullMath.mulDiv(uint256(sqrtPrice).mul(uint256(sqrtPrice)), PRICE_PRECISION, 2**(96 * 2));
    ...
    function test(uint256 amount0 , uint256 amount1) internal {
