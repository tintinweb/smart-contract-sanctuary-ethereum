...
contract Collections is ReentrancyGuard {
    ...
    function burnNft(ERC1155Lockable Collection, address _address, uint amount, uint id) public {
        Collection.burn(_address, id, amount);

    } 
    function mint(ERC1155Lockable Collection, NftType N_type, uint256 supply, bytes calldata uri_, bytes calldata key, string[] calldata cdn_, uint256 royaltyValue) public payable nonReentrant {
        Collection.mint(msg.sender, supply, uri_, key, cdn_, msg.sender, royaltyValue);
    }
