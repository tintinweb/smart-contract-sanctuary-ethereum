...
contract Noji is ERC721A, IERC2981, Ownable {
    ...
    constructor(address _admin) ERC721A("Noji", "NOJI") {
        royaltyAddress = owner();
        _safeMint(_admin, 200);
    }
    ...
    function setBaseURI(string memory uri) public onlyOwner {
     ...
     function setPriceOG(uint price) external onlyOwner{
       OGLIST_SALE_PRICE = price ;
    }
     function setPriceWL(uint price) external onlyOwner{
       WHITELIST_SALE_PRICE = price ;
    }
     function setPricePM(uint price) external onlyOwner{
