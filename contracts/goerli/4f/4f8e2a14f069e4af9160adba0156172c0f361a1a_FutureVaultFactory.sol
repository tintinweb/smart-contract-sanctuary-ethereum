...
import "../interfaces/IFutureVaultFactory.sol";
...
contract FutureVaultFactory is
    IFutureVaultFactory,
    Ownable2Step,
    NoDelegateCall
{
    ...
    address private immutable futureVaultBeacon;
    address private immutable ytBeacon;
    address public curveAddressProvider;
    mapping(address => address) public futureIbtPoolMap;

    /* Events */
    event FutureVaultDeployed(
        address indexed _futureVault,
    ...
    /**
    @notice The constructor of the FutureVaultFactory contract.
    Sets the value of futureVault and yt beacon.
     */
    constructor(address _futureVaultBeacon, address _ytBeacon) Ownable() {
        require(
            _futureVaultBeacon != address(0),
            "FutureVaultFactory: futureVault beacon address should not be zero"
        );
        require(
            _ytBeacon != address(0),
            "FutureVaultFactory: yt beacon address should not be zero"
        );
        futureVaultBeacon = _futureVaultBeacon;
        ytBeacon = _ytBeacon;
    }
    ...
    function deployFutureVault(
        ...
        uint256 max_fees_,
        address registry_,
        address futureVaultAdmin
    )
        external
        override
        noDelegateCall
        returns (address futureVault, address curvePoolAddress)
    {
        require(
            address(futureVaultBeacon) != address(0),
            "FutureVaultFactory: FutureVault Beacon is not set"
        );
        ...
        futureVault = address(
            new BeaconProxy(
                ...
                abi.encodeWithSelector(
                    FutureVault(address(0)).initialize.selector,
                    name_,
                    symbol_,
                    ibt_,
                    asset_,
        ...
        emit FutureVaultDeployed(futureVault, msg.sender);

        // changing admin from self(futureVaultFactory) to futureVaultAdmin
        Ownable2StepUpgradeable(futureVault).transferOwnership(
            futureVaultAdmin
        );

        // deploying curve pool for the deployed PT and its IBT
        _deployCurvePool(futureVault);
        curvePoolAddress = futureIbtPoolMap[futureVault];
    }
    ...
    * @param curveAddressProvider_ The address of the curveAddressProvider.
    ...
    function setCurveAddressProvider(
        ...
        require(
            ...
            "FutureVaultFactory: Cannot set curve address provider to zero address"
        );
    ...
    * @param futureVault The address of the futureVault against which to
     ...
     */
    function getCurvePoolForFuture(
        address futureVault
    ) external view override returns (address) {
        return futureIbtPoolMap[futureVault];
    }
    ...
    function setRegistry(address _newRegistry) public {
        ...
        require(
            ...
            "FutureVaultFactory: Cannot set registry to zero address"
        );
     ...
     */
    function _deployCurvePool(address futureVault) internal {
        address[2] memory coins;
        coins[0] = IFutureVault(futureVault).getIBT();
        coins[1] = futureVault;
        uint256 ASSET_UNIT = 10 **
            IERC20MetadataUpgradeable(coins[0]).decimals();
        uint256 intitalPrice = IFutureVault(futureVault).convertToShares(
            ASSET_UNIT
        );
        require(
            curveFactory != address(0),
            "FutureVaultFactory: curve factory address is not set"
        );
        ...
        _setCurvePoolAddress(responseData, futureVault);
        emit CurvePoolDeployed(
            futureIbtPoolMap[futureVault],
            coins[0],
            futureVault
        );
    }
    ...
    /**
     * @notice Function for setting the address of the curve pool.
     * @param data ABI encoded data containing the pool address.
     * @param pt The address of the pt token.
     */
    function _setCurvePoolAddress(bytes memory data, address pt) internal {
        address addr;
        assembly {
            addr := mload(add(add(data, 12), 20))
        }
        futureIbtPoolMap[pt] = addr;
    }
    ...
    * @notice Function which sets the curve factory address used in
    ...
    function _setCurveFactory() internal {
        ...
        require(
            success,
            "FutureVaultFactory: Could not fetch curve factory address from the address provider"
        );
...
interface IFutureVaultFactory {
    ...
    function deployFutureVault(
        ...
        uint256 max_fees_,
        address registry_,
        address futureVaultAdmin_
    ) external returns (address futureVault, address curvePoolAddress);
