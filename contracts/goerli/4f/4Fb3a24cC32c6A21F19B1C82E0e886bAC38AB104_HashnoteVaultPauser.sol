...
contract HashnoteVaultPauser is Ownable, IVaultPauser {
    ...
    function pausePosition(address _account, uint256 _shares) external override {
        ...
        if (pausedPosition.round > 0 && pausedPosition.round != currentRound) {
            revert VP_PositionPaused();
        }
    ...
    function withdrawCollaterals(address _vault, address _destination) external override {
        ...
        if (vaultWhitelistAddr != address(0)) {
            ...
            if (!vaultWhitelist.isCustomer(msg.sender)) {
                revert VP_CustomerNotPermissioned();
            }
    ...
    function _withdrawCollaterals(IHashnoteOptionsVault vault, PauseReceipt memory pauseReceipt, address _destination) internal {
        ...
        for (uint256 i; i < collaterals.length;) {
            ...
            if (amounts[i] > 0) {
                IERC20(collaterals[i].addr).safeTransfer(_destination, amounts[i]);
            }
...
contract HashnoteOptionsVault is HashnoteVault, HashnoteOptionsVaultStorage, IBatchAuctionSeller {
    ...
    function stageStructure(
        ...
        emit StagedOptions(options, strikes, maxStructures, premium, msg.sender);
    }
 ...
 */
contract HashnoteOptionsVaultProxy is ERC1967Proxy {
    /**
