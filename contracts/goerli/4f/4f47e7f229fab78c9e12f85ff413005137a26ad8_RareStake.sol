...
interface IRareStake {
  ...
  event RewardClaimed(address indexed _claimer, address indexed _currencyAddress, uint256 _amount);
...
contract RareStake is IRareStake, ERC20SnapshotUpgradeable, ReentrancyGuard {
  ...
  function unstake(uint256 _amount) external nonReentrant updatePeriod {
    ...
    rare.transferFrom(address(this), msg.sender, amountDue);
    amountStakedByUser[msg.sender] -= _amount;
    stakingRegistry.decreaseAmountStaked(msg.sender, userStakedTo, _amount);
  ...
  function claimReward() external nonReentrant updatePeriod claimWindowActive {
    if (stakerClaimedRound[claimRound][msg.sender]) revert IRareStake.RewardAlreadyClaimed();

    uint256 totalSRareSupply = totalSupplyAt(claimRound);
    uint256 senderBalance = balanceOfAt(msg.sender, claimRound);

    uint256 percentOfTotal = (senderBalance * 1_000) / totalSRareSupply;

    uint256 amountDue = (percentOfTotal * totalRewards) / 1_000;

    rare.transferFrom(address(this), _msgSender(), amountDue);

    stakerClaimedRound[claimRound][msg.sender] = true;

    emit IRareStake.RewardClaimed(msg.sender, address(rare), amountDue);
  }
