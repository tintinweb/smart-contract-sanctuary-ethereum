...
contract Referral is IReferral, Ownable {
    ...
    function recordReferralCommission(address _referrer, uint256 _commission) public override onlyOperator {
        uint256 prevCommissionTotal = totalReferralCommissions[_referrer];
        if (_referrer != address(0) && _commission > 0) {
            totalReferralCommissions[_referrer] = prevCommissionTotal.add(_commission);
    ...
    function drainERC0Token(IERC20 _token, uint256 _amount, address _to) external onlyOwner {
