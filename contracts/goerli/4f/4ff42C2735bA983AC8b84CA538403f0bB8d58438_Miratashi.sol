...
contract Miratashi is ERC721M, Ownable, ERC2981, ReentrancyGuard {
    ...
    address public constant TEAM_TREASURY_ADDRESS =
        0x8544222A05C1Ffe61D36AA6c51B2797821383E41;
    ...
    uint256 public constant MAX_PUBLIC_MINT_TXN_SIZE = 2;

    // =============================================================
    // |                         Storage                           |
    // =============================================================

    // This smart contract has three phases: 0 = None, 1 = OG, 2 = Whitelist and 3 = Public.
    ...
    function mintOG(uint8 _quantity) external payable callerIsUser {
        // Check if OG phase (1 = OG)
        require(phase == 1, "Miratashi: You are not in OG phase.");
    ...
    function mintWhitelist() external payable callerIsUser {
        // Check if Whitelist phase (2 = Whitelist)
        require(phase == 2, "Miratashi: You are not in Whitelist phase.");
        ...
        if (
            ...
            block.timestamp <= (whitelistStartingTime + whitelistDurationGuarantee)
        ) {
            revert(
    ...
    function mintDutchAuction(uint8 _quantity) external payable callerIsUser {
        // Check if Public phase (3 = Public)
        require(phase == 3, "Miratashi: You are not in Public phase.");
        ...
        if (auctionMinted + _quantity == auctionCapacity || totalSupply() + _quantity == MINT_CAPACITY) {
     ...
     */
    function setBaseURI(string memory _baseURI, string memory _extension) external onlyOwner {
    ...
    function setupWhitelist(
        ...
        require(
            (TEAM_TREASURY_SUPPLY + auctionCapacity + totalSupply() + _capacity) <= MINT_CAPACITY,
    ...
    function auctionCurrentPrice() public view returns (uint256) {
        ...
        if (
            (block.timestamp < auctionStartingTime || auctionFinalPrice > 0)
