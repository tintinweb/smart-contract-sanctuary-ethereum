...
contract DiceGame is Initializable, RandomNumber {
    ...
    IPegSwap public linkPegSwap;
    address public erc20LinkToken;
    ...
    struct LinkExchange {
        ...
        address erc20LinkToken;
        address linkToNativeTokenPriceFeed;
        address[] nativeToLinkPath;
    }
    ...
    function initialize(
        ...
        LinkExchange memory linkExchange
    ) public virtual initializer {
        __RandomNumber_init(
            linkExchange.linkToken,
            linkExchange.wrapperAddress
        );
        require(
        ...
        require(
            linkExchange.sushiswap != address(0),
            "DiceGame: Invalid sushiswap"
        );
        sushiswap = IUniswapV2Router02(linkExchange.sushiswap);
        linkPegSwap = IPegSwap(linkExchange.linkPegSwap);
        erc20LinkToken = linkExchange.erc20LinkToken;
        linkToNativeTokenPriceFeed = linkExchange.linkToNativeTokenPriceFeed;
        nativeToLinkPath = linkExchange.nativeToLinkPath;
        require(
    ...
    function playGame(
        ...
        exchangeNativeToLinkFee(nativePrice, linkPrice);
    ...
    function editErc20Link(address erc20LinkToken_) external onlyOwner {
        erc20LinkToken = erc20LinkToken_;
    }

    function editLinkPegSwap(IPegSwap linkPegSwap_) external onlyOwner {
        linkPegSwap = linkPegSwap_;
    }
    ...
    function exchangeNativeToLinkFee(uint256 nativePrice, uint256 linkPrice)
        private
    {
        ...
        if (address(linkPegSwap) != address(0)) {
            // Exchange of ERC20 Link to ERC677
            bool wasApproved = IERC20Upgradeable(erc20LinkToken).approve(
                address(linkPegSwap),
                amounts[amounts.length - 1]
            );
            require(wasApproved, "DiceGame: error in link approval");
        ...
        returns (uint256 nativePrice, uint256 linkPrice)
    {
        ...
        uint256 requiredLink;
        if (currentGasPrice == 0) {
            requiredLink = VRF_V2_WRAPPER.calculateRequestPrice(
                CALLBACK_GAS_LIMIT * uint32(amountOfBets)
            );
        } else {
            requiredLink = VRF_V2_WRAPPER.estimateRequestPrice(
                CALLBACK_GAS_LIMIT * uint32(amountOfBets),
                currentGasPrice
            );
        }
...
import "./Interfaces/Chainlink/IPegSwap.sol";
import "./Interfaces/ICalculations.sol";
import "./chainlink/VRFV2WrapperConsumerBase.sol"; // Custom implementation
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";

contract RandomNumber is OwnableUpgradeable, VRFV2WrapperConsumerBase {
    ...
    function __RandomNumber_init(address linkToken, address wrapperAddress)
        internal
    {
        require(linkToken != address(0), "DiceGame: invalid linkToken");
        require(
            wrapperAddress != address(0),
            "DiceGame: invalid wrapperAddress"
        );
        __Ownable_init();
        __VRFV2WrapperConsumerBase_init(linkToken, wrapperAddress);
    }
    ...
    function requestRandomWords(
        ...
        uint32 callbackGasLimit = CALLBACK_GAS_LIMIT *
 ...
 */
abstract contract VRFV2WrapperConsumerBase is Initializable {
