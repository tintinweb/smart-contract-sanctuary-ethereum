...
//FIXME: name of contract
contract Sample is ERC1155Supply, Ownable {
    ...
    uint256 public constant PRE_SALE_DISCOUNT_PRICE = 1000000000000000;
    uint256 public constant PUBLIC_SALE_PRICE = 2000000000000000;

    //NOTE: Wed Oct 26 2022 21:00:00 GMT+0000
    uint256 public preSaleWindowOpens = 1666818000;
    //NOTE: Sat Oct 28 2022 21:00:00 GMT+0000
    uint256 public publicSaleWindowOpens = 1666990800;
    //NOTE: Sat Nov 1 2022 21:00:00 GMT+0000
    uint256 public publicSaleWindowCloses = 1667336400;
    ...
    address payable private _withdrawalWallet =
        payable(0xF33CFb981Af9a324F8Ec01367BE904807c2B0cCc);
    ...
    function checkPreSaleClaimEligibility(
        ...
        if (
            block.timestamp < preSaleWindowOpens ||
            block.timestamp > publicSaleWindowCloses
        ) {
        ...
        if (
            isFree &&
            (mintedFreeTokens[toAddress] + amount > MAX_PER_WALLET_FREE)
        ) {
        ...
        returns (string memory)
    {
        ...
        if (
            block.timestamp < publicSaleWindowOpens ||
            block.timestamp > publicSaleWindowCloses
        ) {
    ...
    function preSaleMint(
        ...
        payable
        onlyPaper(
            keccak256(abi.encode(toAddress, amount, isFree)),
            _nonce,
            _signature
        )
    {
        ...
        require(
            (isFree && msg.value == 0) ||
                (!isFree && msg.value == PRE_SALE_DISCOUNT_PRICE * amount),
    ...
    function publicSaleMint(address toAddress, uint256 amount)
        external
        payable
    {
        //NOTE: "block.timestamp" can be influenced by miners to a certain degree.
        //We use timestamp because strict time control is not necessary.
        require(
            block.timestamp >= publicSaleWindowOpens &&
                block.timestamp <= publicSaleWindowCloses,
    ...
    function editWindows(
        ...
        uint256 _preSaleWindowOpens
    ) external onlyOwner {
        require(
            _publicSaleWindowOpens > _preSaleWindowOpens &&
    ...
    //NOTE: 100% randomness is definitely impossible on smart contracts.
    //There is a risk that a user could intentionally mint specify token, user basically a mint with paper, it is not a problem for this contract.
    function _tokenId(address toAddress) private returns (uint256) {
        randNonce++;
        uint256 number = uint256(
            keccak256(abi.encodePacked(block.timestamp, toAddress, randNonce))
        ) % 200;
        if (number == 0) {
            return 1;
