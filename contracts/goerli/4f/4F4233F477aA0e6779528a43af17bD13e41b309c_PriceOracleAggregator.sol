...
interface IPriceOracleAggregator {
    ...
    function assetToOracle(IERC20) external view returns (IOracle);
...
contract PriceOracleAggregator is IPriceOracleAggregator {
    ...
    mapping(IERC20 => IOracle) public override assetToOracle;
    ...
    constructor(address _owner) {
        require(_owner != address(0), "INVALID_ADMIN");
        owner = _owner;
    }
    ...
    function setOracleForAsset(IERC20[] calldata _asset, IOracle[] calldata _oracle) external override onlyOwner {
    ...
    function getPriceInUSDMultiple(IERC20[] calldata _tokens) external view override returns (uint256[] memory prices) {
        uint256 size = _tokens.length;
