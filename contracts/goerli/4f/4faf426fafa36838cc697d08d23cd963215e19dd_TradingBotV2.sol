...
contract TradingBotV2 is ReentrancyGuard, ChainlinkClient {
    ...
    event StakedTokenSymbol(string indexed symbol);
    event TradedTokenSymbol(string indexed symbol);
    event ConcatenatedURL(string indexed url);

    // -- ERRORS --
    error UpkeepNotNeeded(uint256 currentTimeStamp, uint256 lastTimeStamp);
    error AutoApproveDisabled();
    error BadSymbolsResponse(bytes url);
    ...
    function getStakedTokenSymbol() public view returns (string memory) {
        return WETH(i_stakedToken).symbol();
    }

    function requestVolumeData() public returns (bytes32 requestId) {
        Chainlink.Request memory req = buildChainlinkRequest(i_getPriceJobId, address(this), this.fulfill.selector);

        string memory fsyms = WETH(i_stakedToken).symbol();
        emit StakedTokenSymbol(fsyms);
        string memory tsyms = ERC20(i_tradedToken).symbol();
        emit TradedTokenSymbol(tsyms);
        string memory url = string(
            abi.encodePacked('https://min-api.cryptocompare.com/data/pricemultifull?fsyms=', fsyms, '&tsyms=', tsyms)
        );
        emit ConcatenatedURL(url);

        bytes memory urlBytes = bytes(url); // Uses memory
        if (urlBytes.length == 0) {
            revert BadSymbolsResponse(urlBytes);
        }
