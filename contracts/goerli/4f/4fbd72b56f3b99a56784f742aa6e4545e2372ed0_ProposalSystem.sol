...
// File: diamond_tribunal/ProposalSystem.sol
...
contract ProposalSystem is Modifiers, Utils{
    ...
    function newProposal(uint _dealId, uint256 _proposalType, string memory _infOrTitle, uint256 _timeInDays, uint256 _subOrAddToken)
                        ...
                        isPartTaker(_dealId) wasStopped isWorking external   {
    ...
    function proposalChoose(uint _dealID,uint8 _choose) openDeal(_dealID) isPartTaker(_dealID) isWorking external{
    ...
    function addMoreGoods(uint256 _dealID)external openDeal(_dealID) isPartTaker(_dealID) { 
        ...
        uint256 _totalValidation = DS.getVar().deals[_dealID].goods + additionalTokens;

        if (_totalValidation == DS.getVar().deals[_dealID].amount){
