...
contract SpaceRouter {
    ...
    function addLiquidity(uint256 spc) public payable {
        if (msg.value == 0 || spc == 0) {
            revert("SpaceRouter: zero deposit");
        }
        uint256 requiredSPC;
        uint256 requiredETH;

        if (spaceLP.K() != 0) {
            uint256 expectedToken = spaceLP.quoteDeposit(spc, msg.value);
            (requiredSPC, requiredETH) = spaceLP.quoteWithdrawal(expectedToken);
            uint256 ethDiff = msg.value - requiredETH;
            if (ethDiff > 0) {
                (bool success, ) = payable(msg.sender).call{value: ethDiff}("");
                if (!success) {
                    revert("SpaceRouter: refund ETH failed");
                }
            }
        } else {
            requiredSPC = spc;
        ...
        spaceLP.depositSPCForLiquidity(msg.sender, requiredSPC);
        spaceLP.depositETHForLiquidity{value: requiredETH}();
    ...
    function removeLiquidity(uint256 lpToken) public {
        if (lpToken == 0) {
            revert("SpaceRouter: zero liquidity token balance");
        }
        spaceLP.depositLPToWithdraw(msg.sender, lpToken);
    ...
    function swapETHForSPC(uint256 spcOutMin) public payable {
        if (msg.value == 0) {
            revert("SpaceRouter: zero ETH input");
        }

        (uint256 spcOut, ) = spaceLP.quoteSwap(0, msg.value);
        if (spcOut < spcOutMin) {
            revert("SpaceRouter: lower than desired SPC output");
        }

        spaceLP.depositEthToSwap{value: msg.value}();
    ...
    function swapSPCForETH(uint256 spcIn, uint256 ethOutMin) public {
        if (spcIn == 0) {
            revert("SpaceRouter: zero SPC input");
        }

        (, uint256 ethOut) = spaceLP.quoteSwap(spcIn, 0);
        if (ethOut < ethOutMin) {
            revert("SpaceRouter: lower than desired ETH output");
        }

        spaceLP.depositSpcToSwap(msg.sender, spcIn);
        spaceLP.swap(msg.sender);
    }
}
