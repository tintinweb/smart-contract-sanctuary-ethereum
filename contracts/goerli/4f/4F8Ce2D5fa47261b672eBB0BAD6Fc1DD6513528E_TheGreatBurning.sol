...
contract TheGreatBurning is Ownable, VRFConsumerBaseV2, ReentrancyGuard {
    ...
    function createPrize(uint256[] memory _tokenIds, address _nftAddress)
        external
        onlyOwner
    {
    ...
    // DRAWING WINNERS
    // ===========================
    function selectWinners() public onlyOwner returns (uint256) {
        return (
    ...
    function disperseWinnings() external onlyOwner {
        for (uint256 i = 0; i < prizes.length; i++) {
            ...
            for (uint256 n = 0; n < prize.tokenIds.length; n++) {
                nft.transferFrom(address(this), winners[i], prize.tokenIds[n]);
            }
        }
    }
