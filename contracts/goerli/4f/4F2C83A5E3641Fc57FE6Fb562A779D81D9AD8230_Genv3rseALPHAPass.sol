...
contract Genv3rseALPHAPass is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(address => bool) alphaList;
    ...
    modifier isAlphaListed() {
        require(alphaList[msg.sender], "You need to be ALPHA listed to mint.");
        _;
    }
    ...
    function removeFromAlphaList(address _address) external onlyOwner {
        alphaList[_address] = false;
    }
    ...
    function alphaListMint(uint256 amount) external nonReentrant isAlphaListed {
        _walletMint(amount);
    }

    function publicMint(uint256 amount) external nonReentrant whenNotPaused {
        _walletMint(amount);
    }
