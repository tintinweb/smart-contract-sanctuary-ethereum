...
contract Staking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, IStaking {
    ...
    function deposit(uint256 amount) external override nonReentrant {
        ...
        uint256 unlockTimeStamp = nowTimeStamp + lockinBlocks;
        deposits[depositCounter] = Deposit(amount, nowTimeStamp, unlockTimeStamp, nowTimeStamp, msg.sender, DepositState.DEPOSITED, 0, 0);
        tvl = tvl + (amount);

        emit Deposited(depositCounter, msg.sender, amount, unlockTimeStamp);
