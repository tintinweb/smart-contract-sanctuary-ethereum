...
contract SimiDAO {
    ...
    // EVENTS
    // ***************
    event SubmitDonationProposal(string name, string media, uint256 duration, uint goal);
    ...
    event ProcessedProposal(uint256 paymentRequested, string category, bool[6] flags, uint256 proposalId, address indexed senderAddress, address indexed beneficiary, string beneficiaryName, string beneficiarySocial);
    ...
    mapping (uint256 => ApprovedDonation) public donation;
    mapping (uint256 => Beneficiary) public donationBeneficiary;
    ApprovedDonation[] public approvedDonations;
    ...
    constructor(address[] memory approvedMembers, address _ydt, uint256 _minFund, uint256 _minVote) {
    ...
    function _approveDonation(uint _proposalId) internal returns (bool success) {
        ...
        ApprovedDonation memory newDonation = ApprovedDonation(prop.name, prop.duration, prop.media, prop.proposer, prop.paymentRequested, 0, prop.startingTime, prop.category, 0, true);
        donation[_proposalId] = newDonation;
        approvedDonations.push(newDonation);
        validDonation[_proposalId] = true;
        return true;

    }
    function isValidForDonation(uint _proposalId) public view returns (bool) {
        ApprovedDonation memory donate = donation[_proposalId];
    ...
    function receivedDonation(uint _proposalId, uint256 _amount) external {
        require(isValidForDonation(_proposalId), "Invalid");
        donation[_proposalId].amountRaised += _amount;
        donation[_proposalId].donors += 1;
    }

    function getApprovedDonation(uint _proposalId) public view returns (ApprovedDonation memory) {
        return donation[_proposalId];
    }
    ...
    function _submitDonationProposal(
        ...
        emit SubmitDonationProposal(_name, _media, _duration, _goal);
        proposalCount += 1;
    }
    ...
    function processProposal(uint256 proposalId) public onlyMember returns (bool _status) {
        ...
        Beneficiary storage ben = donationBeneficiary[proposalId];
        ...
        if(prop.flags[4] == false && prop.flags[5] == false) {
            if(prop.yesVotes > prop.noVotes && prop.yesVotes >= minVote) {
                prop.flags[1] = true;
                prop.flags[2] = true;
                _approveDonation(proposalId);
            }
            else{
                prop.flags[1] = true;
                _cancelProposal(proposalId);
            }
            emit ProcessedProposal(prop.paymentRequested, prop.category, prop.flags, proposalId, prop.proposer, ben.beneficiary, ben.beneficiaryName, ben.beneficiarySocial);
            return true;
        }
    }
