...
error LastStateReached(uint256 tokenId);
error MaximumTotalSupplyReached(uint256 maximum);
error BurningIsNotEnabled();
error BurningIsNotApproved();

// solhint-disable no-empty-blocks
contract StateNFT is IStateNFT, ERC721Upgradeable, PaymentSplitterUpgradeable, BaseContract, StateNFTStorage {
    ...
    event TokenStateSet(uint256 tokenId, TokenState state);
    event StatesChanged(string[] states);
    ...
    function mint(address to) external payable returns (uint256 tokenId) {
        if (msg.value < _mintValue) revert InvalidMintValue(msg.value);
        if (_tokenIdCounter.current() == _maxTokenSupply + 1) revert MaximumTotalSupplyReached(_maxTokenSupply);
        tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    }

    function burn(uint256 tokenId) external override whenBurnEnabled {
        _requireMinted(tokenId);
        if (!_isApprovedOrOwner(_msgSender(), tokenId)) revert BurningIsNotApproved();
        _burn(tokenId);
    }

    function nextState(uint256 tokenId) external onlyOperator {
        _requireMinted(tokenId);
        if (_tokenStates[tokenId] == TokenState.Last) revert LastStateReached(tokenId);
        _tokenStates[tokenId] = TokenState(uint256(_tokenStates[tokenId]) + 1);
        emit TokenStateSet(tokenId, _tokenStates[tokenId]);
    }
    ...
    function getMintValue() external view returns (uint256) {
        return _mintValue;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721Upgradeable) returns (string memory) {
        ...
        string memory state = Strings.toString(uint256(_tokenStates[tokenId]));
        return string(abi.encodePacked(baseURI, Strings.toString(tokenId), "/", state));
    }
...
abstract contract StateNFTStorage {
    // Update based on how many states the NFT should have before deploying
    enum TokenState {
        First,
        // Second,
        // Third,
...
interface IStateNFT is IERC721Upgradeable {
    function mint(address to) external payable returns (uint256 tokenId);

    function burn(uint256 tokenId) external;

    function nextState(uint256 tokenId) external;
