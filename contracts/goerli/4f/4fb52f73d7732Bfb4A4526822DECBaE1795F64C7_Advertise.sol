...
contract Advertise is ERC721("Advertise", "ADV"), ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	event update(address indexed from, uint256 timestamp, uint256 tokenId, string url, string name, uint256 price);
	...
	function mint(string memory url, string memory name) public payable nonReentrant {
		...
		nameByTokenId[tokenId] = name;

		_safeMint(_msgSender(), tokenId);

		emit update(msg.sender, block.timestamp, tokenId, url, name, msg.value);
	}
	...
	function getNameById(uint256 tokenId) public view returns (string memory) {
		return nameByTokenId[tokenId];
	}
