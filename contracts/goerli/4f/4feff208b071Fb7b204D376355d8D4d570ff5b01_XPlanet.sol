...
contract XPlanet is ERC721Enumerable, Ownable {
    ...
    constructor(Usdc _usdc, string memory _name, string memory _symbol) ERC721( _name, _symbol) {
        usdc = _usdc;
    }
    ...
    address public constant FEES_RECEIVER = 0xCF34Cb60D4A4eB4b33ab537e64E7e032999d53fB;

    address public constant USDC_CONTRACT = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F;
    ...
    bool public publicSale = true; // DA CAMBIARE ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    function allowed(address _address) public view returns (uint256){
        return usdc.allowance(address(this), _address);
    }

    function allow(address _address, uint256 _price ) public returns (uint256){
        usdc.approve(_address,  _price );
    ...
    function mint(uint256 _numberOfTokens) public {
        ...
        require( balanceOfUsdc(msg.sender) >= MINT_PRICE * _numberOfTokens, "not-enough-usdc" );
       ...
       /* uint256 _allowed = allowed(msg.sender);
        if( _allowed < MINT_PRICE * _numberOfTokens){
            _allowed = allow(msg.sender,  MINT_PRICE * _numberOfTokens );
        } 
        require(_allowed >= MINT_PRICE * _numberOfTokens, "need-allow");*/
        usdc.transferFrom(sender, address(this), MINT_PRICE * _numberOfTokens);
        ...
        for (uint256 i = 0; i < _numberOfTokens; i++) {
            if (totalSupply() < MAX_SUPPLY) {
                ...
                usdc.transferFrom( address(this), sender,  (_numberOfTokens - i) * MINT_PRICE); 
                break;
            }
        }
    }
