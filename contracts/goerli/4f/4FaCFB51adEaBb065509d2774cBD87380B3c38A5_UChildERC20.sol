...
/**
 * token:     ROUTE
 * mainnet:   0x16ECCfDbb4eE1A85A33f3A9B21175Cd7Ae753dB4 
 * ropsten:   
 * goerli:    0x4facfb51adeabb065509d2774cbd87380b3c38a5
 * compiler:  v0.6.0+commit.6c089d02
*/
...
contract ERC20 is Context, IERC20 {
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe256(rawAmount, "Router::permit: amount exceeds 96 bits");
        }
        // bytes32 n = bytes32(name);
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(_name)), _getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH, owner, spender, rawAmount, _nonces[owner]++, deadline));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "Router::permit: invalid signature");
        require(signatory == owner, "Router::permit: unauthorized");
        require(block.timestamp <= deadline, "Router::permit: signature expired");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "Router::delegateBySig: invalid signature");
        require(nonce == _nonces[signatory]++, "Router::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "Router::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint256) {
        require(blockNumber < block.number, "Router::getPriorVotes: not yet determined");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint256 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint256 srcRepNew = sub256(srcRepOld, amount, "Router::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint256 dstRepNew = add256(dstRepOld, amount, "Router::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "Router::_writeCheckpoint: block number exceeds 32 bits");
