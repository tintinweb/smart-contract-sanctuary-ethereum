...
contract SBTEST4 is Ownable, ERC721A, PaymentSplitter {
    ...
    uint public transactionLimit = 10;      // Test amt.
        ...
        PaymentSplitter(distro, distro_shares)
    {
        ...
        accounts[teamclaim[0]] = Account( 25, 0, 1 ); // tbd
        accounts[teamclaim[1]] = Account( 25, 0, 1 ); // tbd
        accounts[teamclaim[2]] = Account( 25, 0, 1 ); // tbd
        accounts[teamclaim[3]] = Account( 25, 0, 1 ); // tbd
        accounts[teamclaim[4]] = Account( 25, 0, 1 ); // tbd
    ...
    function mint(uint _amount, bool isAffiliate, string memory affiliateRef) external payable noReentrant {
        ...
        if(isAffiliate) {
        	...
        	require(isActive, "Error: Affiliate account disabled.");
    ...
    function distributeSharesAffilsAndPayees() external minAdmin2 noReentrant {
        ...
        // B. Then pay normal shareholders:
    ...
    // (present as a backup distrubute mechanism).
    ...
    // (^_^) Affiliate functions (^_^) 
    // New functionality for the NFT space, by GenerativeNFTs.io (GenNFTs) :-)
