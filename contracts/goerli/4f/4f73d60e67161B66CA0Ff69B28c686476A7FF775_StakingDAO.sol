...
interface IStakingDAO {
  ...
  function getRewardRate(address stakeholder) external view returns (uint256);
...
contract StakingDAO is OwnableUpgradeable, ReentrancyGuardUpgradeable, IStakingDAO {
  ...
  function __StakingDAO_init_unchained(
    ...
    uint256 initalAmount = msg.value;
  ...
  function getInfo() external view override returns (Info[] memory results) {
    uint i = 0;
    uint length = 11;
  ...
  function getStakeholderInfo(address stakeholder) external view override returns (Info[] memory results) {
    ...
    results[i++] = Info({_type: "uint256", _name: "sharesRate", _value: abi.encode(_getSharesRate(stakeholder))});
  ...
  function getStakedAmount(address stakeholder) external view override returns (uint256) {
  ...
  function getRewards(address stakeholder) external view virtual override returns (uint256) {}

  function getRewardRate(address stakeholder) external view virtual override returns (uint256) {
    return _getSharesRate(stakeholder);
  }
  ...
  function _setMinStakeAmount(uint256 amount) internal virtual {
    require(amount > 0 && amount <= VALIDATOR_AMOUNT, "StakingDAO: the amount is not in range.");
    require(VALIDATOR_AMOUNT % amount == 0, "StakingDAO: The VALIDATOR_AMOUNT must be divisible by the minimum stake amount.");
  ...
  function _stake(address stakeholder, address payable recipient, uint32[] calldata operatorIds) internal virtual {
    ...
    require(amount <= VALIDATOR_AMOUNT, "StakingDAO: stake amount over threshold.");
  ...
  function _unstakePending(uint256 amount) internal virtual {
    ...
    require(amount >= _minStakeAmountRequired, "StakingDAO: unstake amount under threshold.");
    require(amount <= VALIDATOR_AMOUNT, "StakingDAO: unstake amount over threshold.");
    if (_minStakeAmountRequired > 0) {
      require(amount % _minStakeAmountRequired == 0, "StakingDAO: The unstake amount must be divisible by the minimum stake amount.");
    }
