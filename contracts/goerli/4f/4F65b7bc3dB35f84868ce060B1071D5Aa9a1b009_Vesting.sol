...
contract Vesting is Ownable, VestingEvents, ReentrancyGuard {
    ...
    mapping(uint8 => uint256[]) public percentageArray; // Percentage Array for Different Vesting Categories.
    ...
    mapping(address => bool) internal blackListUsers; // admin can black list the user from claiming the tokens.
    ...
    constructor(address _tokenAddress) {
        ...
        vestingStartDate = uint32(block.timestamp + 2 minutes); // Vesting Starts After ICO Ends 60 days
        tgeStartDate = vestingStartDate + 2 minutes; // TGE Start Date is Changable
        percentageArray[0] = [
            700,
            700,
            700,
        ...
        setVestingCategory(
            ...
            uint32(1 minutes), //30 days for main net and 1 minutes for the test
            0
        );
        ...
        setVestingCategory(
            (65_000_000 * (10**token.decimals())),
            uint32(12 minutes), //365 days for main net and 12 minutes for the test
            4
        );
        ...
        setVestingCategory(
            ...
            uint32(5 minutes), //150 days for main net and 5 minutes for the test
            7
        );
    }
    ...
    function claimTokens() external nonReentrant {
        require(!blackListUsers[msg.sender], "you are blacklisted");
        require(
        ...
        returns (uint256 _toBeTransfer, uint8 _claimCount)
    {
        ...
        _claimCount = uint8((_time / 1 minutes) + 1); // Claim in Ever Month 30 days for main net and 1 minutes for the test
    ...
    function changeUserVestingCategory(address userAddress, Category _choice)
        external
        onlyOwner
    {
        UserData storage user = userMapping[userAddress];
        uint8 userCategory = uint8(user.choice);
        console.log(block.timestamp);
        console.log(vestingStartDate + vestingPhaseMapping[userCategory].time);
        require(
            block.timestamp <
                (vestingStartDate + vestingPhaseMapping[userCategory].time),
            "User vesting is started, You can't change now"
        );
        uint8 newUserCategory = uint8(_choice);
        require(
            block.timestamp <
                (vestingStartDate + vestingPhaseMapping[newUserCategory].time),
            "Given vesting category is started, You can't change now"
        );
        user.choice = _choice;
    }

    function changeUserAddressForVesting(
        address userAddress,
        address newUserAddress
    ) external onlyOwner {
        UserData storage user = userMapping[userAddress];

        UserData storage newUser = userMapping[newUserAddress];

        newUser.totalAmount = user.totalAmount;
        newUser.claimedAmount = user.claimedAmount;
        newUser.choice = user.choice;
        newUser.claims = user.claims;
    ...
    function changeVestingPercentagePerMonth(
        Category _choice,
        uint256[] memory array
    ) external onlyOwner {
        uint8 userCategory = uint8(_choice);
        require(
            block.timestamp <
                (vestingStartDate + vestingPhaseMapping[userCategory].time),
    ...
    function blackListUser(address _address) external onlyOwner {
        blackListUsers[_address] = true;
        UserData memory user = userMapping[msg.sender];
        uint8 userCategory = uint8(user.choice);
        if (userCategory == 0) {
            VestingPhase storage phase = vestingPhaseMapping[userCategory];
            phase.tokenAllot -= user.totalAmount - user.claimedAmount;
        } else {
            VestingPhase storage phase = vestingPhaseMapping[userCategory];
            phase.totalAmount += user.totalAmount - user.claimedAmount;
        }
    }
    ...
    function sendLeftoverTokens() external onlyOwner {
        ...
        TransferHelper.safeTransfer(address(token), owner(), _balance);
    }
    ...
    function removeBlackListUser(address _address) external onlyOwner {
        blackListUsers[_address] = false;
    }
}
