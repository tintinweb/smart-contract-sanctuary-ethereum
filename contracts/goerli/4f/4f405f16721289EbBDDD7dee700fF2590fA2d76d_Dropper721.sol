    ...
    ERC165
{
    ...
    modifier isAdminHolder() {
        require(
            IERC1155(adminNFTAddress).balanceOf(msg.sender, adminNFTId) > 0,
            "Caller is not an admin"
        );
        _;
    }

    constructor(address _AdminTokenContract, uint256 _AdminTokenId) Ownable() {
    ...
    function returnAllNfts() external isAdminHolder {
        for (uint256 i = 0; i < count; i++) {
    ...
    function getDrops() external view isAdminHolder returns (Drop[] memory) {
        Drop[] memory curDrops = new Drop[](count);
     ...
     */
    function pause() external isAdminHolder {
        _pause();
    }

    function unpause() external isAdminHolder {
        _unpause();
    }

    function setAdminToken(address _address, uint256 _id) external onlyOwner {
    ...
    function isAdmin() internal view returns (bool) {
        if (adminNFTAddress == address(0)) {
            return true;
        }

        if (
            IERC721(adminNFTAddress).balanceOf(msg.sender) > 0 ||
            IERC1155(adminNFTAddress).balanceOf(msg.sender, adminNFTId) > 0
        ) {
            return true;
        }
