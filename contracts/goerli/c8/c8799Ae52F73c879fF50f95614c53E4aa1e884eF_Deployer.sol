...
contract Nation is AccessControlEnumerable {
  ...
  // function getRoleMemberCount(bytes32 role) public view virtual override(AccessControlEnumerable) returns (uint256) {
  //   uint256 theoreticalMemberCount = super.getRoleMemberCount(role);
  //   uint256 actualMemberCount = 0;
  //   for (uint256 i; i < theoreticalMemberCount; i++) {
  //     address citizen = getRoleMember(role, i);
  //     if (CitizenAlpha(_citizenAlpha).isCitizen(citizen)) {
  //       actualMemberCount++;
  //     }
  //   }
