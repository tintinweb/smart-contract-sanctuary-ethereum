...
contract TestnetIsERC1155 is ERC1155, Ownable, ReentrancyGuard  {
    ...
    uint public maxReservationNumber = 10; // Max number of reservation allowed
    ...
    address public testnet; // testnet's address
    ...
    constructor(address _testnet)
    ERC1155("https://game.example/api/item/")
    {
        testnet = _testnet;
    }
    ...
    */
    function mint(uint idToMint, bytes32[] calldata _proof) public payable nonReentrant {
        ...
        if (currentStep == Step.FreeMint) {
            require(isOnFreeMintList(msg.sender, _proof), "Not on free mint list");
            ...
            require(isOnFirstWhitelist(msg.sender, _proof), "Not on first whitelist");
            ...
            require(isOnSecondWhitelist(msg.sender, _proof), "Not on second whitelist");
            require(msg.value >= whitelistPrice, "Not enought ether");
    ...
    */
    ...
    /*
    * @notice update testnet's address
    * @param address : new testnet's address
    */
    function updatetestnet(address _testnet) external {
        require(msg.sender == testnet || msg.sender == owner(), "Only testnet or owner can update testnet");
        testnet = _testnet;
    }
    ...
    */
    function addBTC(uint satoshis) external {
        require(msg.sender == testnet, "Only testnet can add BTC");
    ...
    */
    function subBTC(uint satoshis) external {
        require(msg.sender == testnet, "Only testnet can sub BTC");
        balanceOfSatoshis = balanceOfSatoshis - satoshis;
    }
    ...
    */
    function burnAndRedeem(uint[] memory idsToRedeem, uint[] memory amountForId, string memory btcAddress) public nonReentrant {
        require(idsToRedeem.length == amountForId.length, "idsToRedeem and amountForId must be the same length");
        for (uint i = 0; i < idsToRedeem.length; i++) {
            require(idsToRedeem[i] >= 1, "Nonexistent id");
            require(idsToRedeem[i] <= maxToken, "Nonexistent id");
            require(amountForId[i] > 0, "Amount must be greater than 0");
            require(amountForId[i] <= balanceOf(msg.sender, idsToRedeem[i]), "You don't have enough testnet of this ID to redeem");
            _burn(msg.sender, idsToRedeem[i], amountForId[i]);
            uint satoshisToRedeem = redeemableByIdAndAmount(idsToRedeem[i], amountForId[i]);
            balanceOfSatoshis = balanceOfSatoshis - satoshisToRedeem;
            supplyByID[idsToRedeem[i]] = supplyByID[idsToRedeem[i]] -amountForId[i];
            emit newRedeemRequest(msg.sender, idsToRedeem[i], amountForId[i], btcAddress, satoshisToRedeem);
        }
    }
    ...
    */
    function reservationForWhitelist() external payable nonReentrant {
        ...
        require(currentReservationNumber + 1 <= maxReservationNumber, "Max pre-whitelist reached");
    ...
    *
    */
    function redeemableByIdAndAmount(uint _id, uint _amount) public view returns(uint) {
        uint divBy = 0;
        uint redeemable = 0;
        ...
        require(divBy > 0, "Can't div by 0");
        require(balanceOfSatoshis > 0, "Balance of satoshis is 0");
        require(supplyByID[_id] > 0, "No supply left for this id");
        require(_amount <= supplyByID[_id], "Amount can't be greater than id's supply");

        redeemable = balanceOfSatoshis / divBy / supplyByID[_id] * _amount;

        return redeemable;
    }

    /*
    * @notice get redeemable amount of satoshis for user
    * @param _user : user's address
    */
    function userTotalBalanceOfSatoshis(address _user) public view returns (uint) {
        uint divBy = 0;
        uint returnBalance = 0;
        ...
        for (uint i = 1; i <= maxToken; i++) {
            if (supplyByID[i] > 0) {
                uint balanceByID = balanceOf(_user, i);
                if (balanceByID > 0) {
                    returnBalance = returnBalance + balanceOfSatoshis / divBy / supplyByID[i] * balanceByID;
                }
            }
        }
    ...
    */
    ...
    /*
    * @notice update max reservation number
    * @param _maxReservationNumber : new max reservation number
    */
    function updateMaxReservationNumber(uint _maxReservationNumber) external onlyOwner {
        maxReservationNumber = _maxReservationNumber;
    }
    ...
    */
    ...
    /*
    * @notice know if user is on free mint list
    * @param _account address of user
    * @param proof Merkle proof
    */
    function isOnFreeMintList(address _account, bytes32[] calldata proof) public view returns(bool) {
        return _verify(_leaf(_account), proof, freeMintMerkleRoot);
    }


    /*
    * @notice know if user is on first whitelist
    * @param _account address of user
    * @param proof Merkle proof
    */
    function isOnFirstWhitelist(address _account, bytes32[] calldata proof) public view returns(bool) {
        return _verify(_leaf(_account), proof, firstMerkleRoot);
    }

    /*
    * @notice know if user is on second whitelist
    * @param _account address of user
    * @param proof Merkle proof
    */
    function isOnSecondWhitelist(address _account, bytes32[] calldata proof) public view returns(bool) {
        return _verify(_leaf(_account), proof, secondMerkleRoot);
    }
