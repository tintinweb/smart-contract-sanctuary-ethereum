...
contract WAGMI_NFT is Ownable, ERC721AEnumerable {
    ...
    bytes32 public constant WAGMI_PROVENANCE =
        bytes32(uint256(0x940fbc599d65734a74d447da78603c9092b502)); /// @dev Will be entered after all NFTs are genrated

    constructor(
        string memory baseTokenURI,
        bytes32 _rootMembers
    ) ERC721A("WAGMI NFTs", "WNFT") {
    ...
    function newPhase(uint _newMax) external onlyOwner {
        maxSupply = _newMax;
    }
    ...
    function setNewLimit(uint _mintLimit) external onlyOwner {
    ...
    function mint(bytes32[] calldata _merkleProof) external {
        if (msg.sender != owner()) {
            ...
            require(userMinted[msg.sender] < mintLimit, "Already minted all!");
        }
        require(totalMinted < maxSupply, "All NFTs for this phase are sold");
 ...
 * account.
 */
abstract contract ERC721AEnumerable is ERC721A {
    ...
    bytes4 public constant IID_IERC721AENUMARABLE =
