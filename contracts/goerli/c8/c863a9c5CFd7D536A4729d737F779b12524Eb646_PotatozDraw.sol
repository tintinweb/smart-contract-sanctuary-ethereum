...
contract PotatozDraw is VRFConsumerBaseV2 {
    ...
    uint256 public drawID = 0;
    mapping(uint256 => uint256[]) public drawRandomWords; // drawID => random uint256 array returned/set by Chainlink VRF
    mapping(uint256 => mapping(uint256 => address)) public drawETOAddresses; // drawID => ending ticket ordinals => draw winner
    mapping(uint256 => uint256[]) public drawEndTicketOrdinals; // drawID => array of ending ticket ordinals.

    event DrawFulfilled(uint256 indexed drawID, uint256 numWinners);
    event NextDraw(uint256 indexed drawID);
    ...
    function nextDraw() external onlyOwner {
        drawID++;
    }
    ...
    function setupDrawData(
        ...
        for (uint256 i = 0; i < etos.length; i++) {
            uint256 eto = etos[i];
            require(eto >= 0 && (etoArray.length == 0 || eto > etoArray[etoArray.length - 1]));
            etoArray.push(eto);
            etoAddresses[eto] = addresses[i];
        }
    }
 
    function doDraw(uint32 numWinners) external onlyOwner {
        uint256[] memory exist = drawRandomWords[drawID];
        require(exist.length == 0, "Already drawed. Need call nextDraw()");
    ...
    function testFulfillRandomWords(uint256[] memory randomWords)
        external
        onlyOwner
    {
        uint256[] memory exist = drawRandomWords[drawID];
    ...
    */
    function getWinners(uint256 _drawID)
        external
        view
