...
contract MinterRole is Ownable {
    ...
    modifier onlyMinter() {
        require(isMinter[msg.sender], "ShibBridgeERC20: FORBIDDEN");
        _;
    }
    ...
    function addMinter(address _minter) public onlyOwner {
        require(_minter != address(0), "ShibBridgeERC20: Minter address can't be zero.");
        isMinter[_minter] = true;
    }

    function removeMinter(address _minter) public onlyOwner {
        require(isMinter[_minter], "ShibBridgeERC20: The address is not minter.");
        isMinter[_minter] = false;
    }
}

contract ShibBridgeERC20 is ERC20, Ownable, MinterRole {
    using SafeERC20 for IERC20;

    uint8 private immutable decimal;
    address public orgToken;
    
    event LogSBTokenSwapOut(uint uID, address indexed account, uint amount, uint form);
    ...
    function changeOrgToken(address _orgToken) external onlyMinter returns (bool) {
        orgToken = _orgToken;
        return true;
    }
    ...
    function swapOut(uint uID, address to, uint256 amount, uint form) external onlyOwner returns (bool) {
        if(orgToken != address(0)) {
            if (IERC20(orgToken).balanceOf(address(this)) >= amount) {
                if(form == 0) {
                    ...
                    emit LogSBTokenSwapOut(uID, to, amount, 0);
                } else {
                    IERC20(orgToken).safeTransfer(to, amount);

                    emit LogSBTokenSwapOut(uID, to, amount, 1);
                }
        ...
        emit LogSBTokenSwapOut(uID, to, amount, 2);
