...
contract AppWorks is ERC721, Ownable {
  ...
  function mint(uint256 _mintAmount) public payable mintedLimit(_mintAmount) {
    //Please make sure you check the following things:
    //Current state is available for Public Mint
    require(mintActive == true, "Public Mint haven't started yet");
    //Check how many NFTs are available to be minted
    require((_nextTokenId._value + _mintAmount -1 ) <= maxSupply, "Exceed maxSupply");
    //Check user has sufficient funds
    require(msg.value >= price * _mintAmount, "No enough ETH");
    addressMintedBalance[msg.sender] += _mintAmount; // reentrancy attack risk if below for loop
    totalSupply += _mintAmount;

    // mint NFT
    uint256 currentTokenId = _nextTokenId._value;
    for (uint256 i = currentTokenId; i < currentTokenId+_mintAmount; i++) {
  ...
  function withdrawBalance(uint256 _amount) external onlyOwner returns(bool, uint256) {
  ...
  function setPrice(uint256 _price) external returns(uint256) {
      price = _price;
      return price;
  }
 
  // Implement toggleMint() Function to toggle the public mint available or not - week 8
  function toggleMint() external returns(bool) {
      return mintActive = !mintActive ;
  }

  // Set mint per user limit to 10 and owner limit to 20 - Week 8
  modifier mintedLimit(uint256 _mintAmount) {
      if (owner() == msg.sender) {
        require(addressMintedBalance[msg.sender] + _mintAmount <= 20, "Exceed personal minted limit");
      } else {
        require(addressMintedBalance[msg.sender] + _mintAmount <= 10, "Exceed personal minted limit");
      }
      _;
  }
  ...
  function setBlindURI(string calldata _newBlindURI) external {
  ...
  function tokenURI(uint256 tokenId) public view override returns(string memory) {
    ...
    if (revealed == true) {
      ...
      return bytes(blindURI).length > 0 ? string(blindURI) : "";
    }
  }

  // Early mint function for people on the whitelist - week 9
  function earlyMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable mintedLimit(_mintAmount) {
    //Please make sure you check the following things:
    //Current state is available for Early Mint
    require(earlyMintActive == true, "Early mint haven't started");
    //Check how many NFTs are available to be minted
    require((_nextTokenId._value + _mintAmount -1 ) <= maxSupply, "Exceed maxSupply");
    //Check user is in the whitelist - use merkle tree to validate
    require(MerkleProof.verify(_merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "Not in merkle proof whitelist");
    //Check user has sufficient funds
    require(msg.value >= price * _mintAmount, "No enough ETH");
    addressMintedBalance[msg.sender] += _mintAmount; // reentrancy attack risk if below for loop
    totalSupply += _mintAmount;
  ...
  function toggleEarlyMint() external returns(bool) {
    earlyMintActive = !earlyMintActive;
    return earlyMintActive;
  }
