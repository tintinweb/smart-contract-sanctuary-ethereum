...
contract GiveAForB {
    ...
    function skim() external {
        uint256 balance_a = IERC20(address_a).balanceOf(address(this));
        if (balance_a > reserve_a) {
            IERC20(address_a).transfer(msg.sender, balance_a - reserve_a);
        }

        uint256 balance_b = IERC20(address_b).balanceOf(address(this));
        if (balance_b > reserve_b) {
            IERC20(address_b).transfer(msg.sender, balance_b - reserve_b);
        }
    }
