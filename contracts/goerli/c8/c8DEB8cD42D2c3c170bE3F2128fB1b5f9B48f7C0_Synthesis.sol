...
contract Synthesis is IERC1155Receiver, CreatorExtension, Ownable {
    ...
    Redemption[] _redemptions;

    function setCreatorCore(address creatorCore) external onlyOwner {
        _creatorCore = creatorCore;
    }

    function clearRedemptions() external onlyOwner {
        delete _redemptions;
    }

    function addRedemption(
        uint256[] calldata burn_token_ids,
        uint256[] calldata burn_token_amounts,
        uint256[] calldata redeem_token_ids,
        uint256[] calldata redeem_token_amounts
    ) external onlyOwner {
        _redemptions.push(
            Redemption(
                burn_token_ids,
                burn_token_amounts,
                redeem_token_ids,
                redeem_token_amounts
            )
        );
    }

    function supportsInterface(bytes4 interfaceId) override (CreatorExtension, IERC165) public view returns (bool) {
    ...
    function onERC1155BatchReceived(
        ...
        for (uint8 r = 0; r < _redemptions.length; ++r) {
            if (
                _redemptions[r].burn_token_ids.length != ids.length ||
                _redemptions[r].burn_token_amounts.length != values.length
            ) continue;

            bool redeemable = true;
            for (uint8 i = 0; i < _redemptions[r].burn_token_ids.length; ++i) {
                if (
                    _redemptions[r].burn_token_ids[i] != ids[i] ||
                    _redemptions[r].burn_token_amounts[i] != values[i]
                ) {
                    redeemable = false;
                    break;
                }
            ...
            IERC1155CreatorCore(_creatorCore).mintExtensionExisting(
                to,
                _redemptions[r].redeem_token_ids,
                _redemptions[r].redeem_token_amounts
            );
