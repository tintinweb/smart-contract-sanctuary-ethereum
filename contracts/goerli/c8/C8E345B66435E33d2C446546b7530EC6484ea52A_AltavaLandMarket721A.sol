...
contract AltavaLandMarket721A is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    event MerkleProofEvent(bool result);
    ...
    function initialize(address landContract) initializer public {
        ...
        _merkleRoot = 0x3d7772e439932f84e6acc70344c11b0b0663bfddb833f970545245e133ca0805;
    ...
    function verifyMerkleProof(bytes32[] calldata proof) public returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        emit MerkleProofEvent(MerkleProof.verify(proof, _merkleRoot, leaf));
    }

    function verifyMerkleProof2(bytes32[] calldata proof) public returns (bool) {
     ...
     */
    function buyByETH(uint256 tokenId) public virtual payable nonReentrant whenNotPaused {
        require(getLandListCount() > 0, "landList is empty");

        // Public(0), etc(1), ...
        if(_salesType > 0) {
            require(tokenId >= _salesBeginId && tokenId <= _salesEndId, "not for sale");
        }
     ...
     */
    function buyByCoin(uint256 tokenId, uint256 price) public virtual nonReentrant whenNotPaused {
        require(getLandListCount() > 0, "landList is empty");
    ...
    // PausableUpgradeable
    function unpause(uint8 salesType, uint256 salesBeginId, uint256 salesEndId) public virtual onlyOwner {
