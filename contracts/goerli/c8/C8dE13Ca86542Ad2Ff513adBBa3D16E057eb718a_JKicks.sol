...
contract JKicks is ERC721A, Ownable {
  ...
  function mint(uint256 _count, address _address) public payable saleIsOpen {
    ...
    if (_address != owner()) {
      ...
      if (balanceOf(_address) < freeMintsPerWallet) {
        require(msg.value >= mintPrice * (_count - freeMintsPerWallet) , "Insufficient ETH amount sent.");
      } else {
