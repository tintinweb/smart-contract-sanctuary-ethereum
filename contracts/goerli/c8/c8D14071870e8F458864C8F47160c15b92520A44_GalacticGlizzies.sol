    ...
    ERC2981
{
    ...
    uint256 constant AUXSLOTSIZE = 8;

    uint256 public constant MAX_MINTS_PER_STAGE = 10;
    uint256 public constant MAX_SUPPLY = 6666;

    uint256 public privatePrice = 0.005 ether;
    uint256 public publicPrice = 0.01666 ether;
    ...
    modifier verifyAmount(uint256 price, uint256 amount) {
    ...
    modifier verifyWalletSupply(uint256 start, uint256 amount) {
        if (
            getBits(_getAux(msg.sender), start, AUXSLOTSIZE) + amount >
            MAX_MINTS_PER_STAGE
    ...
    function setPrivatePrice(uint256 _priPrice) external onlyOwner {
        privatePrice = _priPrice;
    }
    ...
    function mintPrivate(bytes32[] calldata _merkleProof, uint256 _amount)
        external
        payable
        verifySaleStage(SaleStage.Private)
        verifyProof(_merkleProof)
        verifyAmount(privatePrice, _amount)
        verifyWalletSupply(0, _amount)
        verifyAvailableSupply(_amount)
    {
    ...
    function mintPublic(uint256 _amount)
        ...
        verifyAmount(publicPrice, _amount)
        verifyWalletSupply(AUXSLOTSIZE, _amount)
        verifyAvailableSupply(_amount)
    {
        _setAux(
            msg.sender,
            _getAux(msg.sender) + uint64(_amount << AUXSLOTSIZE)
        );
        _mint(msg.sender, _amount);
    }
