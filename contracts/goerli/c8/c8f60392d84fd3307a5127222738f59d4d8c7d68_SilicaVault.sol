...
abstract contract AbstractSilicaVault is ERC20, ISilicaVault, SVStorage, SVGetters, Ownable, ReentrancyGuard {
    uint32 public constant SUBSCRIPTION_DAYS = 14;
    uint32 public constant EPOCH_DAYS = 30;
    ...
    uint16 public constant SILICA_TYPE = 0;
    ...
    function swap(
        ...
        (int24 amt, uint256 estimatePaymentTraded) = _estimateAmountOut(rewardAmount, 3000, 0);
    ...
    function _estimateAmountOut(
        ...
        uint24 fee,
        uint32 secondsAgo
    ) internal view returns (int24 tick, uint256 amountOut) {
    ...
    function startNextRound() external override onlyOwner nonReentrant returns (uint256 mintedShares) {
        SVTypes.State memory stateCopy = state;
        ...
        uint256 paymentHeld = totalPaymentHeld();
        uint256 amountToDeposit = pendingDeposits;
        uint256 shareSupply = totalShares();

        mintedShares = ShareMath.convertToShares(amountToDeposit, paymentHeld, shareSupply);
        ...
        roundSize[nextRound] = paymentHeld + amountToDeposit - mgmtFees;
        // Store share balance
        ...
        state.currentRoundEndDay = oracleDay + EPOCH_DAYS;
        state.lastDayForSettlements = oracleDay + EPOCH_DAYS + SETTLEMENT_DAYS;
    ...
    function purchaseSilica(address silicaAddress, uint256 amount) external override nonReentrant returns (uint256 silicaMinted) {
        ...
        require(silica.getCommodityType() == SILICA_TYPE, "Wrong silica type");
    ...
    function settleDefaultedSilica(address silicaAddress)
        external
        override
        onlyOwner
        nonReentrant
    ...
    function getOracleDay() internal view returns (uint32) {
        Oracle oracle = Oracle(oracleRegistry.getOracleAddress(address(rewardToken), SILICA_TYPE));
    ...
    function _previewSyncShares(address depositor, uint256 currentRound)
        internal
        view
        returns (uint256 unredeemedShares, uint256 newlySyncedShares)
    {
...
contract SilicaVault is AbstractSilicaVault {
    uint24 public constant poolFee = 3000; // PoolFee set for uniswap v3
    ...
    constructor(
        ...
        state.currentRoundEndDay = getOracleDay() + SUBSCRIPTION_DAYS - SETTLEMENT_DAYS;
    }
}
...
interface ISilicaVault {
    ...
    function startNextRound() external returns (uint256 mintedShares);
