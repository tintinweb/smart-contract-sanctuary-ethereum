...
import "./interface/IFoxPay.sol";
import "./library/Blocklistable.sol";
import "./library/ERC3525Upgradeable.sol";
import "./library/BokkyPooBahsDateTimeLibrary.sol";
import "./FoxPayMetadataDescriptor.sol";
    ...
    IFoxPay
{
    ...
    function initialize(address metadataDescriptor_) public initializer {
        __ERC3525_init("Fox Pay", "FOX", 18);
        __Context_init();
    ...
    function contractURI()
        public
        view
        override(ERC3525Upgradeable, IERC3525Metadata)
        returns (string memory)
    {
        return metadataDescriptor.constructContractURI();
    }
    ...
    function slotURI(
        ...
        returns (string memory)
    {
        return metadataDescriptor.constructSlotURI(slot_);
    }
    ...
    function tokenURI(
        ...
        public
        view
        override(ERC3525Upgradeable, IERC721Metadata)
        returns (string memory)
    {
    ...
    // To simplify implementation, add 9 hours JST time difference once and subtract it at the end
    function startDate(uint256 tokenId_) public view returns (uint256) {
        return _startDate(slotOf(tokenId_));
    }

    function expireDate(uint256 tokenId_) public view returns (uint256) {
        return _expireDate(slotOf(tokenId_));
    }

    function _startDate(uint256 slot_) internal pure returns (uint256) {
        // ex)
        //  expire datetime (GMT): 2023-05-31 15:00
        //  -> expire datetime (JST-like): 2023-06-01 00:00
        //  -> start datetime (JST-like): 2022-12-01 00:00
        //  -> start datetime (GMT): 2022-11-30 15:00
        (
            uint256 expireYear_,
            uint256 expireMonth_,

        ) = BokkyPooBahsDateTimeLibrary.timestampToDate(
                _expireDate(slot_).addHours(9)
            );
        uint256 startYear = (expireMonth_ >= 7) ? expireYear_ : expireYear_ - 1;
        uint256 startMonth = (expireMonth_ >= 7)
            ? expireMonth_ - 6
            : expireMonth_ + 6;
        return
            BokkyPooBahsDateTimeLibrary
                .timestampFromDate(startYear, startMonth, 1)
                .subHours(9);
    }

    function _expireDate(uint256 slot_) internal pure returns (uint256) {
        uint256 monthDay = slot_ % 10000;
        uint256 year = (slot_ - monthDay) / 10000;
        uint256 day = monthDay % 100;
        uint256 month = (monthDay - day) / 100;
        // ex) 2023-06-01 00:00:00 JST -> 2022-05-31 15:00:00 GMT
        return
    ...
    function mint(
        ...
        ERC3525Upgradeable._mint(to_, slot_, value_);
    }
    ...
    function supportsInterface(
        ...
        public
        view
        override(IERC165, ERC3525Upgradeable, AccessControlUpgradeable)
        returns (bool)
    {
        return ERC3525Upgradeable.supportsInterface(interfaceId);
    }
}
...
contract FoxPayMetadataDescriptor is IERC3525MetadataDescriptor {
    ...
    function constructTokenURI(
        ...
        string memory image = _generateSVG(
            tokenId_,
            balance,
            foxPay.startDate(tokenId_),
            foxPay.expireDate(tokenId_),
            erc3525.message(tokenId_)
        );
        return
            string(
                abi.encodePacked(
    ...
    function _generateSVGFilters() internal pure returns (string memory) {
    ...
    function _generateSVGGradientFirst() internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    /* solhint-disable */
    ...
    function _generateSVGGradientSecond()
        internal
        pure
        returns (string memory)
    {
    ...
    function _generateSVGTexts(
        ...
        uint256 tokenIdLength = Math.log10(tokenId_) + 1;
        string memory tokenIdStr = tokenId_.toString();
        for (uint i; i < 8 - tokenIdLength; i++) {
            tokenIdStr = string.concat("0", tokenIdStr);
        }
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    bytes(value_.toString()).addThousandsSeparator(),
                    "</text>",
                    '<text fill="#FF980E" xml:space="preserve" style="white-space: pre" font-family="Arial" font-size="32" letter-spacing="0em" x="138" y="814.094">#',
                    tokenIdStr,
                    "</text>",
                    '<text fill="#FF980E" xml:space="preserve" style="white-space: pre" font-family="Arial" font-size="56" letter-spacing="0em" x="567.938" y="180.914">',
                    startDate_.addHours(9 + 12).dateToString(), // ex) 2024-02-29 15:00 -> "2024-03-01" by adding 9 and buffer 12
                    " ~ ",
                    expireDate_.dateToString(), // ex) 2024-08-31 15:00 -> "2024-08-31" by ignoring hours
                    "</text>"
                    /* solhint-enable */
                )
            );
    ...
    function _generateSVGmessage(
        string[] memory messageAarray_
    ) internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    /* solhint-disable */
                    _generateSVGmessageOneLine(
                        messageAarray_.length >= 1 ? messageAarray_[0] : "",
                        '"600"'
                    ),
                    _generateSVGmessageOneLine(
                        messageAarray_.length >= 2 ? messageAarray_[1] : "",
                        '"668"'
                    ),
                    _generateSVGmessageOneLine(
                        messageAarray_.length >= 3 ? messageAarray_[2] : "",
                        '"736"'
                    )
                    /* solhint-enable */
                )
    ...
    function _generateSVGmessageOneLine(
        string memory partMessage_,
        string memory y
    ) internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    // solhint-disable-next-line
                    '<text fill="#777777" xml:space="preserve" style="white-space: pre" font-family="Arial" font-size="48" letter-spacing="0em" x="138" y=',
                    y,
                    ">",
                    partMessage_,
                    "</text>"
    ...
    function _generateSVGGroups() internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    /* solhint-disable */
...
interface IERC3525Metadata is IERC3525, IERC721Metadata {
    ...
    function message(uint256 tokenId_) external view returns (string[] memory);
}
    ...
    ERC165Upgradeable
{
    ...
    // add for Blocklistable
    modifier exceptRole(bytes32 role) {
        _checkUnRole(role, _msgSender());
        _;
    }
    ...
    // add for Blocklistable
    function _checkUnRole(bytes32 role, address account) internal view virtual {
        if (hasRole(role, account)) {
            revert(
                string(
                    abi.encodePacked(
    ...
    AccessControlUpgradeable
{
    ...
    function addBlocklisted(
        address _account
    ) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(BLOCKED_ROLE, _account);
    }

    function removeBlocklisted(
        address _account
    ) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        _revokeRole(BLOCKED_ROLE, _account);
    }

    modifier exceptBlocked(address _account) {
        _checkUnRole(BLOCKED_ROLE, _account);
        _;
    }
}
    ...
    IERC721Enumerable
{
    ...
    function message(
        ...
        return _allTokens[_allTokensIndex[tokenId_]].messages;
    }
    ...
    function setMessage(
        ...
        uint256 newLen = message_.length;
        uint256 nowLen = _allTokens[_allTokensIndex[tokenId_]].messages.length;
        for (uint i = 0; i < newLen; i++) {
            if (i < nowLen) {
                _allTokens[_allTokensIndex[tokenId_]].messages[i] = message_[i];
            } else {
                _allTokens[_allTokensIndex[tokenId_]].messages.push(
                    message_[i]
                );
            }

            if (nowLen > newLen) {
                for (uint j = 0; j < nowLen - newLen; j++) {
                    _allTokens[_allTokensIndex[tokenId_]].messages.pop();
                }
            }
        }
    }
...
library StringConvertor {
    ...
    function addThousandsSeparator(
        ...
        newString = new bytes(self.length + (self.length - 1) / 3);
        uint256 newIndex = 0;
        for (uint256 i = 0; i < self.length; i++) {
            if (i >= 1 && (i % 3 == self.length % 3)) {
                newString[newIndex++] = ",";
            }
            newString[newIndex++] = self[i];
        }
    }
