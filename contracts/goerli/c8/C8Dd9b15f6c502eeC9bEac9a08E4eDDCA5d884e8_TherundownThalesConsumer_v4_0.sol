...
contract TherundownThalesConsumer_v4_0 is ConfirmedOwner, ChainlinkClient {
    ...
    mapping(bytes32 => bytes[]) private s_requestIdGames;
    ...
    function fulfillGames(bytes32 _requestId, bytes[] memory _games) external recordChainlinkFulfillment(_requestId) {
        s_requestIdGames[_requestId] = _games;
    }

    function fulfillOdds(bytes32 _requestId, bytes[] memory _odds) external recordChainlinkFulfillment(_requestId) {
     ...
     * @param _hasScoresByPeriod the flag to return an array of scores by period.
     */
    ...
    function requestGamesResolveWithFilters(
        ...
        req.addStringArray("gameIds", _bytes32ArrayToString(_gameIds));
        _addUintArray(req, "statusIds", _statusIds);
        req.addBytes("hasScoresByPeriod", abi.encodePacked(_hasScoresByPeriod));
    ...
    function requestOddsWithFilter(
        bytes32 _specId,
        uint256 _payment,
        uint256 _sportId,
        uint256 _date,
        ...
        Chainlink.Request memory req = buildOperatorRequest(_specId, this.fulfillOdds.selector);
    ...
    function getGamesCreated(bytes32 _requestId, uint256 _idx) external view returns (GameCreate memory) {
        GameCreate memory game = abi.decode(s_requestIdGames[_requestId][_idx], (GameCreate));
        return game;
    }

    function getGamesResolved(bytes32 _requestId, uint256 _idx) external view returns (GameResolve memory) {
        GameResolve memory game = abi.decode(s_requestIdGames[_requestId][_idx], (GameResolve));
        return game;
    }
        ...
        returns (GameResolveWithScoreByPeriod memory)
    {
        GameResolveWithScoreByPeriod memory game = abi.decode(
            s_requestIdGames[_requestId][_idx],
