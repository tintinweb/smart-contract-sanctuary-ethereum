...
contract ADVERTNFT is Ownable,ERC721Full,ReentrancyGuard,ISetup {
    ...
    mapping(address => uint256) private claimMap;
...
;
    ...
    function startMint() external onlyCLevel{
    ...
    nonReentrant external payable
    {
        ...
        require(pt> 1 seconds,'must white list mint');
    ...
    nonReentrant external payable
    {
        ...
        uint256 pt=blockTimestamp()-nftMintStartTime;
        require(pt< 4 weeks,'not white list mint');
    ...
    internal
    {
        ...
        mintMap[msg.sender]+=singleAddressLimit;
    }
    ...
    function claim(uint256 tokenId)nonReentrant public {
        ...
        msg.sender.send(amount-claimMap[NFTs[tokenId].owner]);

        claimMap[NFTs[tokenId].owner]=amount;

    }

    function claimAddress(uint256 max)nonReentrant external {
    ...
    function editmetadata(uint256 tokenId,string calldata _image,string calldata  _name,string calldata  _tags,string calldata  _exurl) external{
        require(_exists(tokenId),"not correct tokenid");
