...
contract Standard is ERC721A, ReentrancyGuard, Delegated {
    ...
    // ======== PRICE ========
    uint256 public phase_1_Price = 0.04 ether;
    uint256 public phase_2_Price = 0.12 ether;
    uint256 public phase_3_Price = 0.06 ether;
    ...
    // ======== CONSTRUCTOR ========
    constructor() ERC721A("Standard", "STD") {}

    // ======== MINTING ========
    function mintBatch1(uint quantity)
        external
        payable
        callerIsUser
    {
        ...
        require(accessList[msg.sender] > 0, "Not whitelisted");
        require(_numberMinted(msg.sender) <= accessList[msg.sender], "Exceeds Max Claim");
        _safeMint(msg.sender, quantity);
    }

    function mintBatch2(uint quantity)
        external
        payable
        callerIsUser
    {
        ...
        require(msg.value >= phase_2_Price * (quantity / 2), "Incorrect ether sent");
        require(totalSupply() < MAX_SUPPLY, "Max supply reached");
        // require(_numberMinted(msg.sender) < 1, "Already minted");
        _safeMint(msg.sender, quantity * 2);
    }

    function mintBatch3(uint quantity) external payable callerIsUser {
