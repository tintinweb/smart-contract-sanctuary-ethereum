    ...
    OwnableUpgradeable
{
    ...
    uint256 public constant override GENESIS_BASED_SALARY = 100;
    ...
    function registerMetaCellForScientist(
        ...
        scientistIdToMetaCellId[scientistId] = [metaCellId1, metaCellId2];
    }

    function registerNanoCellForMetaCell(
        uint256 metaCellId,
        uint256 nanoCellId
    ) external override isOperator {
    ...
    function getBasedSalary(
        uint256 scientistId
    ) external view override returns (uint256 basedSalary) {
        basedSalary =
            GENESIS_BASED_SALARY +
            _additionalMultiplierBasedSalary(scientistId);
    }

    function getNanocells(
        uint256 metaCellId
    ) external view returns (uint256[] memory nanoCellIds) {
    ...
    function _additionalMultiplierBasedSalary(
        uint256 _scientistId
    ) internal view returns (uint256 _basedAdditional) {
        uint256 _metaCellId1 = scientistIdToMetaCellId[_scientistId][0];
        uint256 _stage = _stageOfMetaCell(_metaCellId1);
        uint256 _nanoCellCount = metaCellIdToNanoCellIds[_metaCellId1].length();
    ...
    function _stageOfMetaCell(
        uint256 _metaCellId
    ) internal view returns (uint256 _stage) {
