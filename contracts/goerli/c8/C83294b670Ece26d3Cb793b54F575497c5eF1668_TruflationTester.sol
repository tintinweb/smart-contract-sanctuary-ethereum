...
contract TruflationTester is ChainlinkClient, ConfirmedOwner {
    ...
    constructor(address oracleId_, string memory jobId_, uint256 fee_) ConfirmedOwner(msg.sender) {
    ...
    function requestInflationWei() public returns (bytes32 requestId) {
        Chainlink.Request memory req =
            buildChainlinkRequest(bytes32(bytes(jobId)), address(this), this.fulfillInflationWei.selector);
        req.add("service", "truflation/current");
        req.add("keypath", "yearOverYearInflation");
        req.add("abi", "int256");
