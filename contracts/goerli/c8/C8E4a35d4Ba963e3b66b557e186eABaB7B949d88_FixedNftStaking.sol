...
// File: contracts/FixedNftStaking.sol
    ...
    IERC721Receiver
{
    ...
    struct UserInfo {
        ...
        uint256 godCount; // "Candy God" nft staked count
        uint256 kingCount; // "Candy King" nft staked count
        uint256 princeCount; // "Candy Prince" nft staked count
        uint256 ministerCount; // "Candy Minister" nft staked count
        uint256 citizenCount; // "Candy Citizen" nft staked count
        uint256 unpaid;
    ...
    /// @dev Merkle root for identifying token id & rarity
    bytes32 public merkleRoot;

    /// @dev Reward amount per day, different per rarity
    mapping(Rarity => uint256) public emissions;
    // Info of each user that stakes LP tokens.
    mapping(address => UserInfo) private userInfo;

    event MaxLimitUpdated(uint256 limit);
    event EmissionUpdated(Rarity rarity, uint256 emission);
    event Staked(address indexed account, uint256[] ids);
    event Withdrawn(address indexed account, uint256[] ids);
    ...
    /// @dev Update emission value for the rarity
    function updateEmission(Rarity _rarity, uint256 _emission)
        external
        onlyOwner
    {
        emit EmissionUpdated(_rarity, _emission);
        emissions[_rarity] = _emission;
    }
    ...
    function viewUserInfo(address _account)
        ...
        returns (
            ...
            uint256, // God nft staked count
            uint256, // King nft staked count
            uint256, // Prince nft staked count
            uint256, // Minister nft staked count
            uint256 // Citizen nft staked count
        )
    {
    ...
    function isStaked(address _account, uint256 _id)
        public
        view
        returns (bool)
    {
        ...
        if (user.lastRewardTime < block.timestamp) {
            pending =
                ((block.timestamp - user.lastRewardTime) *
                    (user.godCount *
                        emissions[Rarity.GOD] +
                        user.kingCount *
                        emissions[Rarity.KING] +
                        user.princeCount *
                        emissions[Rarity.PRINCE] +
                        user.ministerCount *
                        emissions[Rarity.MINISTER] +
                        user.citizenCount *
                        emissions[Rarity.CITIZEN])) /
                1 days;
        }
    ...
    function stake(
        ...
        uint256[5] memory tempCounts; // There are 5 rarities
        for (uint256 i = 0; i < countToStake; i++) {
            require(
                MerkleProof.verify(
                    _proofs[i],
                    merkleRoot,
                    keccak256(abi.encodePacked(_ids[i], _rarities[i]))
                ),
                "Verify failed"
            );
            stakingNft.safeTransferFrom(_msgSender(), address(this), _ids[i]);
            tempCounts[_rarities[i]]++;
            user.stakedNfts.add(_ids[i]);
        }

        user.godCount += tempCounts[uint256(Rarity.GOD)];
        user.kingCount += tempCounts[uint256(Rarity.KING)];
        user.princeCount += tempCounts[uint256(Rarity.PRINCE)];
        user.ministerCount += tempCounts[uint256(Rarity.MINISTER)];
        user.citizenCount += tempCounts[uint256(Rarity.CITIZEN)];
        user.lastRewardTime = block.timestamp;

        emit Staked(_msgSender(), _ids);
    }
