...
contract SubdomainENS {
    ...
    function domainRegist(string calldata origin,bytes32 secret) external payable {

        domainNode = keccak256(abi.encodePacked(TLD_NODE, keccak256(bytes(origin))));        
        ensr.registerWithConfig(origin,msg.sender,315569520,secret,goerliResolver,msg.sender);
    }
