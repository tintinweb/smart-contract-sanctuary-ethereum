...
library InvestmentLibrary {
    ...
    function getContractTotalBalance(heldToken[] memory _tokensHeld, IUniswapV2Router02 uniswapV2Router) public view returns (uint256) {
        uint256 contractValue = 0;
        if(_tokensHeld.length != 0){
            for (uint i = 0; i < _tokensHeld.length; i++){
                if(_tokensHeld[i].tokenAddress == uniswapV2Router.WETH()){
                    contractValue += IERC20(_tokensHeld[i].tokenAddress).balanceOf(address(this));
                } else {
                    if(_tokensHeld[i].isV3 == false){
                        (uint reserveA, uint reserveB) = UniswapV2Library.getReserves(uniswapV2Factory, _tokensHeld[i].tokenAddress, uniswapV2Router.WETH());
                        contractValue += UniswapV2Library.quote( IERC20(_tokensHeld[i].tokenAddress).balanceOf(address(this)), reserveA, reserveB);
                    } else {
                        (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity) = OracleLibrary.consult(_tokensHeld[i].poolAddress, 10);
                        contractValue += OracleLibrary.getQuoteAtTick(arithmeticMeanTick, uint128(IERC20(_tokensHeld[i].tokenAddress).balanceOf(address(this))), _tokensHeld[i].tokenAddress, uniswapV2Router.WETH());
                    }
                }
            }
        }
    ...
    function getPoolTokensToMint(uint256 messageValue, heldToken[] memory _tokensHeld, IUniswapV2Router02 uniswapV2Router, uint256 totalSupply) public view returns (uint256){
        if(totalSupply != 0) {   
            uint256 poolValue = getContractTotalBalance(_tokensHeld, uniswapV2Router) - messageValue;   
...
contract Investment is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 public _factoryFee = 25000000000000000;
    address public immutable _feeHandlerAddress;
    address private _stableCoin = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F; //USDC GOERLI
    ...
    bool private _ownerIsSwapping = false;
    
    constructor(string memory name_, string memory symbol_, string memory risk_, uint256 poolFee_, address feeHandlerAddress_) ERC20(name_, symbol_) {
        _name = name_;
        _symbol = symbol_;
        _riskLevel = risk_;
        _poolFee = poolFee_;
        _feeHandlerAddress = feeHandlerAddress_;
    ...
    function getContractTotalBalance() public view virtual returns (uint256){
        return InvestmentLibrary.getContractTotalBalance(_tokensHeld, uniswapV2Router);
    }
    ...
    function crashProtect(InvestmentLibrary.crashProtectParams[] memory tokensToSwap) public onlyOwner {
        _ownerIsSwapping = true;
        for (uint i = 0; i < tokensToSwap.length; i++){
            _swapTokens(tokensToSwap[i].v2Path, tokensToSwap[i].v3Path, tokensToSwap[i].amountIn, tokensToSwap[i].minimumOut);
        }
        _ownerIsSwapping = false;
    }
    ...
    function addToPool(uint256[] memory minimumAmountsOut) public payable {
        ...
        IERC20(uniswapV2Router.WETH()).transfer(owner(), poolOwnerFeeAmount);
        processFee(uniswapV2Router.WETH(), factoryFeeAmount, 0);
        ...
        if(InvestmentLibrary.isTokenHeld(_tokensHeld, uniswapV2Router.WETH()) == false && IERC20(uniswapV2Router.WETH()).balanceOf(address(this)) > 0){
            
            _tokensHeld.push(InvestmentLibrary.heldToken(uniswapV2Router.WETH(), address(0), false));
        }
    ...
    function removeFromPool(uint256 amountToRemove, InvestmentLibrary.exitSwapParams[] memory exitSwapParams) public {
        ...
        require(_ownerIsSwapping == false, "The owner of this pool is currently swapping some tokens held by the pool. Please wait a few seconds");
        ...
        for (uint i = 0; i < tokensHeldSnapshot.length; i++){
            ...
            if(tokensHeldSnapshot[i].tokenAddress == uniswapV2Router.WETH()){
                wethPayout += tokenContract.balanceOf(address(this)) * userShare / 10**18;
            } else {
                if(exitSwapParams[i].fee == 0){
                    ...
                    uint256 wethRecieved = _swapTokens(
                        path,
                        abi.encode(tokensHeldSnapshot[i].tokenAddress, exitSwapParams[i].fee, uniswapV2Router.WETH()),
                        tokenContract.balanceOf(address(this)) * userShare / 10**18,
                        exitSwapParams[i].minimumOut
                    );
        ...
        if(InvestmentLibrary.isTokenHeld(_tokensHeld, uniswapV2Router.WETH()) == true && IERC20(uniswapV2Router.WETH()).balanceOf(address(this)) == 0){
            _removeAddressFromTokensHeld(uniswapV2Router.WETH());
        }
    ...
    function processFee(address tokenIn, uint256 amount, uint24 uniPoolfee) internal {
        ...
        if(tokenIn != uniswapV2Router.WETH()){
            if(uniPoolfee == 0){
                bytes memory emptyBytes;
                address[] memory path;
                path[0] = tokenIn;
                path[1] = uniswapV2Router.WETH();
                feeAmount = _swapTokens(path, emptyBytes, amount, 0);
            } else {
                address[] memory path;
                feeAmount = _swapTokens(path, abi.encode(tokenIn, uniPoolfee, uniswapV2Router.WETH()), amount, 0);
            }
            
        }

        IERC20(uniswapV2Router.WETH()).approve(_feeHandlerAddress, feeAmount);
        IFeeHandler(_feeHandlerAddress).collectFees(feeAmount);
    }

    function swapTokens(address[] memory v2Path, bytes memory v3Path, uint256 amountIn, uint256 amountOutMinimum) public onlyOwner {
        ...
        _swapTokens(v2Path, v3Path, amountIn - feeAmount, amountOutMinimum);
    }

    function _swapTokens(address[] memory v2Path, bytes memory v3Path, uint256 amountIn, uint256 amountOutMinimum) internal returns (uint256) {
        _adjustTokensHeld(v2Path, v3Path, amountIn);
        if(v2Path.length != 0){
            IERC20(v2Path[0]).approve(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, amountIn);
            ...
            return amountsOut[v2Path.length - 1];
        } else {
            ...
            if(v3Path.length == 96){
                ...
                IERC20(decodedTokenIn).approve(0xE592427A0AEce92De3Edee1F18E0157C05861564, amountIn);
            } else {
    ...
    function _adjustTokensHeld(address[] memory v2Path, bytes memory v3Path, uint256 amountIn) internal {
        address tokenIn;
        address tokenOut;
        address poolAddress;
        uint24 fee;
        ...
        if(IERC20(tokenOut).balanceOf(address(this)) == 0 && InvestmentLibrary.isTokenHeld(_tokensHeld, tokenOut) == false){
            if(tokenOut == uniswapV2Router.WETH()){
                poolAddress = address(0);
            } else {
                if(isV3 == false){
                poolAddress = UniswapV2Library.pairFor(uniswapV2Factory, uniswapV2Router.WETH(), tokenOut);
                } else {
                    poolAddress = IUniswapV3Factory(uniswapV3Factory).getPool(uniswapV2Router.WETH(), tokenOut, fee);
                }
            }   
            _tokensHeld.push(InvestmentLibrary.heldToken(tokenOut, poolAddress, isV3));
        }
