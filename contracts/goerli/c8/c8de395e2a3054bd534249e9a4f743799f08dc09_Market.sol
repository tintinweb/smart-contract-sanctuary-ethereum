...
contract Market is ERC721, ReentrancyGuard, OperatorAccessControl {
    ...
    //用户是否购买此商品
    mapping(address => mapping(uint256 => bool)) userProductMap;
    ...
    //构造方法
    constructor() ERC721("MARKET", "MARKET") Ownable() {}

    //设置系统收款地址
    function setIncomeAddress(address systemIncomeAddress_)
        external
        isOperatorOrOwner
    {
    ...
    //获取合作商信息
    function getPartnerInfo(uint256 partnerId)
        public
        view
        returns (uint256, address)
    {
        return (partners[partnerId].ratio, partners[partnerId].incomeAddress);
    }
    ...
    function addProduct(
        ...
        products[productId].payAmount = payAmount;
    }
    ...
    {
        return (
            products[productId].partnerId,
            products[productId].productId,
            products[productId].payContractAddress,
            products[productId].payAmount
        );
    }
    ...
    // partnerId 合作商id
    // productId 商品id
    // payContractAddress 支付合约地址 ETH_ADDRESS主链币
    // payAmount 支付数量
    // buyUserAddress 购买用户地址
    // partnerIncomeAddress 合作商收币地址
    // systemIncomeAddress 系统收币地址
    // ratio 分成比例
    event BuyProduct(
        uint256 partnerId,
        uint256 productId,
        ...
        address partnerIncomeAddress,
        address systemIncomeAddress,
        uint256 ratio
    );

    //购买
    function buy(uint256 productId) public payable nonReentrant {
        require(productId > 0, "productId must be greater than 0");
        ...
        require(
            userProductMap[buyAddress][productId] == false,
            "User has purchased"
        );
        ...
        //支付数量
        uint256 payAmount = products[productId].payAmount;
        //合作商id
        ...
        //合作商获取数量
        uint256 partnerGetAmount = (payAmount * ratio) / 10000;
        //系统获取数量
        ...
        if (payContractAddress == ETH_ADDRESS) {
            ...
            //记录用户已经购买
            userProductMap[buyAddress][productId] = true;
            //用户购买商品列表
            userProducts[buyAddress].push(productId);
        } else {
            ...
            require(
                _erc20.allowance(buyAddress, _this) >= payAmount,
                "erc20 payment allowance is insufficient"
            );

            //给合作商转账
            _erc20.transferFrom(buyAddress, incomeAddress, partnerGetAmount);
            //给系统账户转账
            _erc20.transferFrom(
                buyAddress,
                systemIncomeAddress,
    ...
    //获取用户购买产品列表
    function getUserByList(address userAddress)
        public
        view
        returns (uint256[] memory)
    {
        return userProducts[userAddress];
    }
}