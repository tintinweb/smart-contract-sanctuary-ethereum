    ...
    OwnableUpgradeable
{
    ...
    event MMTAddressUpdated(address oldAddress, address newAddress);

    function initialize(string memory __baseURI, uint256 __mintCost, address payable __payments, address __MMTAddress) public initializer {
        __ERC721_init("Cointelegraph News Clippings", "CNC");
        __Ownable_init();

        baseURI = __baseURI;
        mintCost = __mintCost;
        MMTAddress = __MMTAddress;
        payments = __payments;
    }
    ...
    function setMMT(address _newMMT) external onlyOwner {
        emit MMTAddressUpdated(MMTAddress, _newMMT);
        MMTAddress = _newMMT;
    }
