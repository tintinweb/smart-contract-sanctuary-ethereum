...
contract NFThing is ERC721Enumerable, Ownable, RandomlyAssigned{
    ...
    uint256 public MAX_NFTHINGS_PER_WALLET = 5;
    uint256 public cost = 0.002 ether;
    uint256 public maxNfthings = 100;
    ...
    modifier maxNfthingsPerWallet(uint256 numberOfTokens){
        require(balanceOf(msg.sender) + numberOfTokens <= MAX_NFTHINGS_PER_WALLET, "Max Nfthings Limit Reached");
        _;
    }
    ...
    modifier isWhiteListed(address _user){
        require(whiteList[_user], "You're not whitelisted");
        _;
    }
    ...
    RandomlyAssigned(maxNfthings, 1)
    ...
    isMintingPaused
    isAddress
    maxNfthingsPerWallet(_mintAmount)
    ...
    function mintForWhiteListed(uint256 _mintAmount)
    public
    isAddress
    isMintingPaused
    maxNfthingsPerWallet(_mintAmount)
    ...
    //no payment splitter for mint to right now test : [on ramp]
    ...
    function setMaxMintAmountPerTx(uint256 _maxMintAmountPerTx)
        public
        onlyOwner{
        MAX_NFTHINGS_PER_WALLET = _maxMintAmountPerTx;
    }
    ...
    /// @notice function to add user to allowlist. Need to be owner
    /// @param _address address that has to be added.

    function addAllowListUser(address _address) 
    public
    onlyOwner
    {
    ...
    /// @notice function to remove user to allowlist. Need to be owner
    /// @param _address address that has to be removed.
    ...
    function addWhiteListUser(address _address) 
    public
    onlyOwner
    {
    ...
    function removeWhiteListUser(address _address) 
    public
    onlyOwner
    {
