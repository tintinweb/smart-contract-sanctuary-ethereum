...
contract AAWallet {
    ...
    function validateUserOp(UserOperation calldata userOp) external onlyEntryPoint {
        ...
        // bytes32 signatureHash = keccak256(abi.encode(userOp.signature));
        bytes memory signatureStore = userOp.signature;
        // console.logBytes(signatureTest);

        require(bytes32(signatureStore) == requiredHash, "Signature Validation did not pass! Better luck next time ;)");
