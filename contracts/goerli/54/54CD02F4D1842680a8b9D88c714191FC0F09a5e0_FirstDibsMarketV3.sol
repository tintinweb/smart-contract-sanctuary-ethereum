    ...
    FirstDibsFeeSupport
{
    ...
    /// @dev The listing admin may cancel listings
    bytes32 public constant LISTING_ADMIN_ROLE = keccak256('LISTING_ADMIN_ROLE');
    ...
    /**
     * @notice This event is emitted when a Listing price is updated
     * @param listingId the ID of the listing (used for lookups)
     * @param fundsRecipient the address of the funds recipient of the token sale
     * @param buyer the address of the buyer of the token (optional; used for private sales)
     * @param quantity the number of tokens for sale
    ...
    function initialize(
        ...
        AccessControlUpgradeable._setupRole(LISTING_ADMIN_ROLE, ContextUpgradeable._msgSender());
     ...
     * @param _buyer The private buyer of the token (optional)
     * @param _quantity The quantity of tokens (optional for ERC721)
    ...
    function createListing(
        ...
        address validSeller = _getValidSeller(_tokenContract, _tokenId, _quantity, assetClass);
        ...
        _listings[_listingIdCounter].startTime = startTime;
    ...
    function updateListing(
        ...
        if (ContextUpgradeable._msgSender() != listing.seller) {
            revert UnauthorizedCaller({ listingId: _listingId });
        }
        listing.fundsRecipient = _getValidRecipient(_fundsRecipient);
        // it's valid for buyer to be zero address because that only
        // means that the listing is not private
    ...
    function cancelListing(uint256 _listingId) external nonReentrant listingExists(_listingId) {
        ...
        if (
            !AccessControlUpgradeable.hasRole(LISTING_ADMIN_ROLE, ContextUpgradeable._msgSender())
        ) {
            if (listing.assetClass == _ERC721_INTERFACE_ID) {
                // Ensure the caller is the seller or a new token owner for ERC-721
                address tokenOwner = IERC721(listing.tokenContract).ownerOf(listing.tokenId);
                if (
                    ContextUpgradeable._msgSender() != seller &&
                    ContextUpgradeable._msgSender() != tokenOwner
                ) {
                    revert UnauthorizedCaller({ listingId: _listingId });
                }
            } else if (ContextUpgradeable._msgSender() != seller) {
                revert UnauthorizedCaller({ listingId: _listingId });
            }
        }
        ...
        _removeSellerListing(seller, _listingId);
    }
    ...
    function purchaseFromListing(
        ...
        if (listing.assetClass == _ERC721_INTERFACE_ID) {
            ...
            // Reverts if the seller did not approve the FirstDibsTokenManager or no longer owns the token
            firstDibsTokenTransferHelper.erc721TransferFrom(
                listing.tokenContract,
                listing.seller,
                _recipient,
                listing.tokenId
            ...
            // Reverts if the seller did not approve the FirstDibsTokenManager or
            // no longer owns enough tokens to transfer
    ...
    function _getValidSeller(
        ...
        } else if (_assetClass == _ERC1155_INTERFACE_ID) {
            if (_quantity <= 0) {
                revert InvalidSupply({ listingId: 0 });
            }
            if (
                IERC1155(_tokenContract).balanceOf(ContextUpgradeable._msgSender(), _tokenId) <
                _quantity
            ) {
                revert InvalidSupply({ listingId: 0 });
            }
    ...
    function _getValidQuantity(
        ...
        uint256 balanceOfSender = IERC1155(_tokenContract).balanceOf(
            ContextUpgradeable._msgSender(),
            _tokenId
        );
        if (balanceOfSender < _quantity) {
            revert InvalidSupply({ listingId: _listingId });
        }
...
contract FirstDibsFeeSupport is Initializable, AccessControlUpgradeable, PullPaymentUpgradeable {
        ...
        onlyInitializing
    {
        ...
        AccessControlUpgradeable._setupRole(DEFAULT_ADMIN_ROLE, _msgSender()); // deployer of the contract gets admin permissions
