...
contract Holitra is ERC721, Ownable, ERC721Enumerable, ReentrancyGuard {

    using Strings for uint256;
	
    uint256 public MAX_NFT = 250;
	uint256 public MAX_BY_MINT_PER_WALLET = 2;
    ...
    constructor() ERC721("Holitra", "HLTR") {}
	...
	function mintPreSaleNFT(uint256 _count, bytes32[] calldata merkleProof) external payable nonReentrant{
		...
		require(
			users[msg.sender].salemint + _count <= MAX_BY_MINT_PER_WALLET,
	...
	function updateMintLimitPerWallet(uint256 newLimit) external onlyOwner {
        ...
        MAX_BY_MINT_PER_WALLET = newLimit;
    }
	...
	function updateSaleSupply(uint256 newSupply) external onlyOwner {
	    require(newSupply >= NFT_MINTED, "Incorrect value");
        MAX_NFT = newSupply;
    }
}