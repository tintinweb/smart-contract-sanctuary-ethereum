...
contract SBTPen is ERC1155, ReentrancyGuard, AccessControl, ISBTPen {
    ...
    constructor(address usdc_) ERC1155("https://{id}.json") {
        usdc = usdc_;
    ...
    function mint(uint256 id_) external override callerIsUser nonReentrant {
    ...
    function upgrade(uint256 next_) external override callerIsUser nonReentrant {
        require(next_ == MIDDLE || next_ == HIGH, "not support level");
        require(balanceOf(msg.sender, next_) == 0, "already have this level");

        uint256 id_ = getId(msg.sender);
        uint256 price = levelPrice[next_] - levelPrice[id_];
        require(id_ < next_, "not support upgrade");
