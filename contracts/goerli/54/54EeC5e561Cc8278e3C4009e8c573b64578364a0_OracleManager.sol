...
contract OracleManager is Ownable, IOracleManager {
    ...
    function createNewOracle(
        ...
        IOracle _newOracle = new Oracle();

        _addOracle(pairName, _newOracle);
        
        emit NewOracleCreated(address(_newOracle), pairName);
    }

    function _addOracle(string memory pairName, IOracle oracle) private {
        oracles[pairName] = oracle;
        pairNames.push(pairName);
    }

    function setWriter(address oracle, address writer, bool writable) external onlyOwner {
        Oracle(oracle).setWriter(writer, writable);
    }
}
...
interface IOracleManager {
    ...
    function setWriter(address oracle, address writer, bool writable) external;
}