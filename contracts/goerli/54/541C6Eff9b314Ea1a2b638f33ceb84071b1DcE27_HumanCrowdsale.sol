...
contract HumanCrowdsale is AccessControl, PullPayment, Ownable, Signer {
    ...
    event WhitePriceChanged(uint256 price);
    ...
    function setWhitePrice(uint256 _whitePrice) external onlyRole(CROWD_ROLE) {
        salePrice = _whitePrice;
        emit WhitePriceChanged(_whitePrice);
    }
    ...
    function canMint(address addr) external view returns (uint256) {
        if(HumanState.ALLOW == state || HumanState.PUBLIC == state ){
            uint256 tAmount = Teaser(teaser).balanceOf(msg.sender);
            if(tAmount >=5){
                return oLimit.sub(mintNum[addr]);
            }
    ...
    NativeMetaTransaction
{
    ...
    constructor() ERC721A("NeymarJR", "NRJ") {
...
contract Signer is Ownable {
    ...
    keccak256("allowMint(address wallet)");
    ...
    returns (bool)
    {
        return signingKey == recoverSigner(sender, TYPEHASH, signature);
    }
