...
contract ChaiLabsPrivateSale is Ownable {
    ...
    bytes32 public constant SALE_WHITELIST_ROLE =
        keccak256("SaleWhitelist(uint256 saleId,uint256 saleRoundId,address admin,address user,uint256 deadline)");
    ...
    constructor() {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("ChaiLabsPrivateSale")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
    ...
    */
    ...
    function claimBackContribution(uint256 saleId) external validateSale(saleId) {
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            ...
            if (rounds[i].startTime + rounds[i].saleDuration < block.timestamp && _softCapFilled(saleId, i)) {
