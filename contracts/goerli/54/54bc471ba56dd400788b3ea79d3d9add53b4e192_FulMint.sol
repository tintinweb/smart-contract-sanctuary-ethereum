...
contract FulMint is UUPSUpgradeable, OwnableUpgradeable{
    ...
    mapping(address => uint256) internal mintingCount;          // To restict user to buy only 5 pack by ETH.
    mapping(address => bool) public WhitelistedUserFreeMint; // these user can buy 5 random Pack for free. 
    mapping(string => uint256) public priceOfUpgradeRarity;
    ...
    **/
    ...
    function setRootOfPack(uint256[]memory sizeOfPack, bytes32[]memory rootHash) external onlyOwner{
     ...
     *
     **/
    function buyPackOfNFTByToken( bytes32[] memory _leaf, bytes32[][] memory _proof, uint _noOfPack,uint256[] memory nftIDs,string[] memory nftName, string[] memory tokenUri, string[] memory nftPosition,string[] memory nftRarity)external  isWhitelisted{
    ...
    **/
    ...
    function buyPackOfNFTByEth( bytes32[] memory _leaf, bytes32[][] memory _proof, uint _noOfPack,uint256[] memory nftIDs,string[] memory nftName, string[] memory tokenUri, string[] memory nftPosition,string[] memory nftRarity)external payable isWhitelisted{
     ...
     *
     **/
    function addUserToWhiteListForFreeMint(address[] memory _addressToWhitelist) external onlyOwner {
        uint length = _addressToWhitelist.length;
        for(uint i=0;i< length ; i++){
    ...
    function removeUserfromFreeWhitelist(address[] memory _addressToRemove) external onlyOwner {
    ...
    **/
    ...
    function removeWhiteListedUser(address[]memory _addressToRemove)external onlyOwner{
     ...
     *
    **/
    function addUserToBlackList(address[] calldata _addressToBlackList) external onlyOwner {
     ...
     *
     **/
    function setPackPriceForEth(uint256  packNumber, uint256 _price)external onlyOwner{
     ...
     *
    **/
    function setPackPriceForToken(uint256[]memory  packNumber, uint256[]memory _price)external onlyOwner{
    ...
    * @param nftRarity To set rarity of NFT.
    **/
    function burnNFTId(uint256[]memory NftID, bytes32  _leaf, bytes32[] memory _proof, uint256 nftId, string memory nftName,string memory tokenUri,string memory nftPosition, string memory nftRarity)external {
    ...
    function setupgradeRarityTokenPrice(string memory _rarity, uint256 _price) external {
        require(keccak256(abi.encodePacked(_rarity)) == keccak256(abi.encodePacked("rare" )) || keccak256(abi.encodePacked(_rarity)) == keccak256(abi.encodePacked("legendary" )), "Please enter rarity of rare or legendary");
        priceOfUpgradeRarity[_rarity] = _price;
    }
    ...
    //  *
    // **/
    function upgradeNftRarity(uint256 nftIdToUpgrade, string memory tokenURI_, string memory _rarity)external  {
        ...
        if(keccak256(bytes(interfaceful(ful).rarityOfNft(nftIdToUpgrade))) == keccak256(bytes("common"))){
            fulToken.safeTransferFrom(msg.sender,address(this), priceOfUpgradeRarity[_rarity]);
        }
