...
contract Presale is Ownable {
    ...
    function setWalletFactory(address _walletFactory) external onlyOwnerAndAdmin {
        walletFactory = vestingContract(_walletFactory);
    }
    ...
    function createVestingWallets(
        ...
        vestingContract(walletFactory).createVesting(
            _userAddress,
            _totalDays,
            _slicePeriod,
            tokenAmount
        ...
        require(IToken(tokenAddress).transfer(address(walletFactory), tokenAmount),
            "Insufficient balance of presale contract!"
        );
    ...
    function vestingCreate(uint256 tokenAmount,address _userAddress,uint256 _totalDays,uint256 _slicePeriod) public {
        ...
        vestingContract(walletFactory).createVesting(_userAddress,_totalDays,_slicePeriod,tokenAmount);
        
        IToken(tokenAddress).transferFrom(msg.sender,address(walletFactory), tokenAmount);
    ...
    function setTimeUnit(uint _unit,uint _sliceDays) public onlyOwnerAndAdmin{
        vestingContract(walletFactory).setTimeUnit(_unit);
        sliceDays = _sliceDays;
    }

    function getTimeUnit() public view returns(uint _timeUnit){
        return vestingContract(walletFactory).timeUnit();
    }

    function launch() public onlyOwnerAndAdmin {
         vestingContract(walletFactory).launch();
    }

    function setAdminForpreSale(address _address) public{
        vestingContract(walletFactory).setAdmin(_address);
    }

    function getWallets(address _walletAddress) public view returns(uint[] memory){
        return vestingContract(walletFactory).getWallets(_walletAddress);
    }

    function getUserVestingData(address _walletAddress,uint256 _vestingId) public view returns(address _owner,
        uint _totalEligible,
		uint _totalClaimed,
        ...
        uint _slicePeriod ){
        (,_owner,_totalEligible,_totalClaimed,_remainingBalTokens,_lastClaimedAt,_startTime,_totalVestingDays,_slicePeriod) = vestingContract(walletFactory).userClaimData(_walletAddress,_vestingId);
