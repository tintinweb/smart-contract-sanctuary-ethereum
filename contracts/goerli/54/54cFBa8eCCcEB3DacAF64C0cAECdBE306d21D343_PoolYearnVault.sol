...
contract PoolYearnVault is Pool {
    ...
    function _wrapPreview(
        ...
        uint256 lockedFundsRatio = (block.timestamp - yvToken.lastReport()) *
            yvToken.lockedProfitDegradation();

        if (lockedFundsRatio < DEGRADATION_COEFFICIENT) {
            freeFunds -=
                yvToken.lockedProfit() -
                ((lockedFundsRatio * yvToken.lockedProfit()) /
                    DEGRADATION_COEFFICIENT);
        }

        shares = (base_ * IERC20(sharesToken).totalSupply()) / freeFunds;
    }
