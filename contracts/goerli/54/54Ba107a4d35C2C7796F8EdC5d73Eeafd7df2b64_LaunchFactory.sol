...
contract LaunchFactory is Ownable, Pausable {
    ...
    address public immutable launchNFTLogic;
    ...
    constructor(address _launchSettings) {
        ...
        launchNFTLogic = address(new LaunchNft(_launchSettings));
    }
    ...
    function createVault(
        ...
        if(vaultType == VaultType.ERC20){
       address newVault = address(new LaunchInitProxy(launchLogic, _initializeData));
        emit Mint(nft, nftId, priceOfNft, newVault, _vaultTracker.current(), vaultType);
        IERC721(nft).safeTransferFrom(msg.sender, newVault, nftId);
        vaults[_vaultTracker.current()] = newVault;
        _vaultTracker.increment();
        }
        else if(vaultType == VaultType.ERC721){
        address newVault = address(new LaunchInitProxy(launchNFTLogic,_initializeData));
         emit Mint(nft, nftId, priceOfNft, newVault, _vaultTracker.current(), vaultType);
        IERC721(nft).safeTransferFrom(msg.sender, newVault, nftId);
        vaults[_vaultTracker.current()] = newVault;
        _vaultTracker.increment();
    ...
    ERC1155HolderUpgradeable
{
    ...
    function _claimFees() internal {
        ...
        if(fee > ILaunchSettings(launchSetting).maxCuratorFee() ){
        currentFee = ILaunchSettings(launchSetting).maxCuratorFee();
        }


        // get how much in fees the curator would make in a year
        uint256 currentAnnualFee = (currentFee * totalSupply()) / 1000;
    ...
    ERC1155HolderUpgradeable
{
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721AUpgradeable, ERC1155ReceiverUpgradeable)
        returns (bool)
    {
    ...
    function updateAuctionLength(uint256 _length) external {
        require(msg.sender == curator, "update:not curator");
        require(
            _length >= ILaunchSettings(launchSetting).minAuctionLengthForNFT() &&
                _length <= ILaunchSettings(launchSetting).maxAuctionLengthForNFT(),
            "update:invalid auction length"
        );
    ...
    function _claimFees() internal {
        ...
        if(fee > ILaunchSettings(launchSetting).maxCuratorFeeForNFT() ){
        currentFee = ILaunchSettings(launchSetting).maxCuratorFeeForNFT() ;
        }
        ...
        // now lets do the same for governance
        address govAddress = ILaunchSettings(launchSetting).feeReceiverForNFT();
        uint256 govFee = ILaunchSettings(launchSetting).governanceFeeForNFT();
        currentAnnualFee = (govFee * totalSupply()) / 1000;
        feePerSecond = currentAnnualFee / 31536000;
        uint256 govMint = sinceLastClaim * feePerSecond;
        ...
        if (curator != address(0)) {
            uint curatorRoyalty = totalSupply() + curatorMint;
            _mint(curator, curatorRoyalty);
            emit FeeClaimed(curatorMint);
        }
        if (govAddress != address(0)) {
            uint govRoyalty = totalSupply() + govMint;
            _mint(govAddress, govRoyalty);
            emit FeeClaimed(govMint);
        }
    }
    ...
    function updateUserPrice(uint256 _new) external {
        ...
        // previously they were not voting
            ...
            uint256 reservePriceMin = (averageReserve *
                ILaunchSettings(launchSetting).minReserveFactorForNFT()) / 1000;
            require(_new >= reservePriceMin, "update:reserve price too low");
            uint256 reservePriceMax = (averageReserve *
                ILaunchSettings(launchSetting).maxReserveFactorForNFT()) / 1000;
            require(_new <= reservePriceMax, "update:reserve price too high");
    ...
    function bid() external payable {
        require(auctionState == State.live, "bid:auction is not live");
        uint256 increase = ILaunchSettings(launchSetting).minBidIncreaseForNFT() +
            1000;
        require(msg.value * 1000 >= livePrice * increase, "bid:too low bid");
        require(block.timestamp < auctionEnd, "bid:auction ended");
    ...
    function redeem() external {
        ...
        bool allBurned = burnToken(totalSupply());
    ...
    function cash(uint[] memory tokenList) external {
        require(auctionState == State.ended, "cash:vault not closed yet");
        uint256 bal = balanceOf(msg.sender);
        require(bal > 0, "cash:no tokens to cash out");
    ...
    function burnToken(uint _balance) internal returns(bool tokensBurned) {
