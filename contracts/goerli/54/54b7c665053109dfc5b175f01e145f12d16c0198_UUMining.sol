...
contract UUMining is Ownable {
    ...
    address minerNftAddr = 0x1171F104F5aB234B775CE33d5587716E22b2430e; 
    address lootNftAddr = 0x9F0163D0f996724933E43DCC900Ee16E1F0594eB; 
    address uuTokenAddr = 0x9E8Ba1038e93824f9A382Ff1759E2F750d041166;
    address usdtAddr = 0xB15620234784E06C15eA55de90D5d3292017b840; 

    uint public constant CONST_UURATE = 1000; // num of days to finish distributing. 0.1% per day

    uint256 mintedMinerIndex = 0; 
    uint256 mintedLootIndex = 0; 

    uint256 treasureMapOdds = 5;
    uint256 lootBoxOdds = 5;
    uint256 treasureMapPrize = 0;
    uint256 lootBoxPrize = 0;
    ...
    function mint100(uint256 _amt, address _referrer) external {
        _mint(_amt, _referrer, 100);
    }

    function mint10000(uint256 _amt, address _referrer) external {
    ...
    function _mint(uint256 _amt, address _referrer, uint256 _usdtPrice) internal onlyValidUsers {
        ...
        usdt.transferFrom(msg.sender, address(this), requiredUsdt); // collect usdt 
    }
    ...
    function claimUU(uint _id) external onlyValidUsers {
        ...
        require (UUtoken.balanceOf(address(this)) >= available2ClaimUU, 'Insuffient UU for claiming');

        // do claiming and update
        UUtoken.transferFrom(address(this), msg.sender, available2ClaimUU);
        n.claimedUU += available2ClaimUU;

        emit ClaimUU(_id, available2ClaimUU, n.claimedUU);
    }

    function multiClaimUU(uint[] memory _ids) external onlyValidUsers {
        for (uint i = 0; i < _ids.length; i++) {
            this.claimUU(_ids[i]);
        }
    }

    function splitNftInto2(uint _id, uint _uuAmt) external onlyValidUsers {
        // check if caller owns NFT
        IERC721 _thisNFT = IERC721(minerNftAddr);
        require (msg.sender == _thisNFT.ownerOf(_id), "not NFT owner.");
    ...
    function obtainTreasures() external onlyValidUsers {
    ...
    function openTreasures(uint256 _id) external onlyValidUsers {
    ...
    function _transferUUToken (address _target, uint256 _amt) internal {
        ...
        require (erc20token.balanceOf(address(this)) >= _amt, 'insufficent token for transfer');
        erc20token.transferFrom(address(this), _target, _amt);
    }

    function withdrawErc20Token (address _tokenAddr, uint256 _withdrawAmt) external onlyOwner {
