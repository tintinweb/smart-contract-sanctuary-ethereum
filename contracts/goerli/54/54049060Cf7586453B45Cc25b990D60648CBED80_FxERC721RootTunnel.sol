  ...
  Ownable
{
  ...
  function deposit(
    ...
    for (uint256 i = 0; i < tokenIds_.length; ++i) {
      ...
      rootToken.safeTransferFrom(msg.sender, address(this), tokenId, data_);
    }

    bytes memory message = abi.encode(childToken, msg.sender, tokenIds_, data_);
  ...
  function _withdraw(bytes memory data_) internal {
    (address _rootToken, address user, uint256[] memory tokenIds, bytes memory data) = abi
      .decode(data_, (address, address, uint256[], bytes));

    IERC721 rootToken = IERC721(_rootToken);
    for (uint256 i = 0; i < tokenIds.length; ++i) {
      uint256 tokenId = tokenIds[i];
      rootToken.safeTransferFrom(address(this), user, tokenId, data);
    }
