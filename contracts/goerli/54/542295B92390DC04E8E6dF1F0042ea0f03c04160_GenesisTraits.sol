...
contract GenesisTraits is ERC721Enumerable, Ownable, ReentrancyGuard, IGenesisCollection {
    ...
    modifier traitClaim(uint256 tokenId) {
        require(IRegensZero(regensZero).tokenIdDNA(tokenId) == traitVault, "GenesisTraits: this tokenId does not have genesisDNA");
        require(
    ...
    constructor(uint256[] memory cap, uint256[] memory percentage) ERC721("GenesisTraits", "GT") {
    ...
    function setTraitsVault(address _traitsVault) public onlyOwner {
        require(traitVault == address(0), "GenesisTraits: traitVault can only be set once");
        traitVault = _traitsVault;
    }

    function setRegensZero(address _regensZero) public onlyOwner {
        require(address(regensZero) == address(0), "GenesisTraits: regensZero can only be set once");
        ...
        returns (string memory uri)
    {
        ...
        return (generalSalt> 0) ? string(abi.encodePacked(baseURI, tokenMetadata.toString(),".json")) : preRevealUri;
    }

    function tokenImage(uint256 tokenId) public view override returns (string memory) {
        uint256 imageId = distribution(tokenId,tokenId/1000000);

        return (generalSalt> 0 && tokenIdSalt(tokenId) > 0) ? string(abi.encodePacked(imageUri, imageId.toString(),".png")) : "";
    }
    ...
    function traitName(uint256 tokenId) public view override returns (string memory){
        require(tokenIdSalt(tokenId) > 0,"GenesisTraits: tokenId does not exist");
    ...
    function tokenIdSalt(uint256 tokenId) public view returns(uint256){
        return (tokenId % 1000000) <= 10000 ? IGenesisDNAVault(traitVault).getTokenIdSalt(tokenId % 1000000) : traitIdSalt[tokenId];
    }

    function distribution(uint256 tokenId,uint256 layer) public view returns(uint256){
    ...
    function mintTraits(uint256 tokenId) public override onlyTraitsVault nonReentrant {
        for (uint256 i = 0; i < INITIAL_TRAITS_AMOUNT; i++) {
