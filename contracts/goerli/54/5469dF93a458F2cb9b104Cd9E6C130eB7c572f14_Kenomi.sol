...
contract Kenomi is Ownable, Utils {
    ...
    event ItemAdded(WhiteListItem item);
    event ItemUpdated(uint256 itemIndex, WhiteListItem item);
    event ItemBuyed(WhiteListItem item, address owner);
    ...
    function addItem(bytes memory signature, WhiteListItem memory whiteListItem)
        external
        onlyOwnerOrAdmin
        requireSignature(signature)
    {
        whiteListItems.push(whiteListItem);
        emit ItemAdded(whiteListItem);
    }

    function updateItem(bytes memory signature, uint256 itemIndex, WhiteListItem memory whiteListItem)
        external
        onlyOwnerOrAdmin
        requireSignature(signature)
    {
        whiteListItems[itemIndex] = whiteListItem;
        emit ItemUpdated(itemIndex, whiteListItem);
    }

    function buyItem(bytes memory signature, uint256 itemIndex, uint256 amount)
        external
        payable
        requireSignature(signature)
    {
        ...
        emit ItemBuyed(item, msg.sender);
    }
    ...
    function getOwnedItems(address _address)
        external
        view
