...
contract Stakable is Ownable {
        ...
        )
    {
        ...
        require(
            currStake.amountLBSKR >= unstakeAmount && currStake.sharesLBSKR > 0,
        ...
        if ((currStake.amountLBSKR - unstakeAmount) == 0) {
            ...
            uint256 userStakesLen = _stakeholders[userIndex].userStakes.length;
            _stakeholders[userIndex].userStakes[stakeIndex] = _stakeholders[
                userIndex
            ].userStakes[userStakesLen - 1]; // Move last item to the spot to be deleted
            _stakeholders[userIndex].userStakes.pop(); // Remove the last element
        } else {
...
contract LBSKR is BaseBSKR, DSMath, Stakable {
    ...
    uint256 public constant _INF_RATE_HRLY = 0x33B2FEE4E1DEE6BDD000000; // ray (10E27 precision) value for 1.000008 (1 + 0.0008%)
    uint16 public constant _SECS_IN_AN_HOUR = 3600;
    uint32 public constant _SECS_IN_FOUR_WEEKS = 2419200; // 3600 * 24 * 7 * 4

    IBSKR public _BSKR;
    address public immutable _inflationAddress;
    address public _BSKRAddr;
    bool public _initialRatioFlag;
    uint16 public _burnFee = 10; // 0.1% burn fee
    uint16 public _growthFee = 10; // 0.1% growth fee

    uint256 public _lastDistTS;
        ...
        )
    {
        ...
        if ((balanceOf(address(this)) + inflation) > 0 && currStake.sharesLBSKR > 0) {
            lbskrRewards =
                (((((balanceOf(address(this)) + inflation + _totalLBSKRStakes) *
                    currStake.sharesLBSKR) / _totalLBSKRShares) -
                    currStake.amountLBSKR) * eligibleBasis) /
                _BIPS; // stakeAmount never existed - it's notional
        }

        if (_BSKR.balanceOf(_inflationAddress) > 0 && currStake.sharesBSKR > 0) {
            bskrRewards =
