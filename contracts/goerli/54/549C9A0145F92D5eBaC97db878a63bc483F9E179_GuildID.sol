...
import "@gnus.ai/contracts-upgradeable-diamond/contracts/token/ERC721/ERC721Upgradeable.sol";
import {Base64Upgradeable} from "@gnus.ai/contracts-upgradeable-diamond/contracts/utils/Base64Upgradeable.sol";
import "@gnus.ai/contracts-upgradeable-diamond/contracts/token/ERC20/IERC20Upgradeable.sol";

//  ==========  INTERNAL IMPORTS    ==========

import {Modifiers} from "../guild/Guild.sol";
import {GuildIDStorage} from "./GuildIDStorage.sol";
...
contract GuildID is Modifiers, ERC721Upgradeable {
    using GuildIDStorage for GuildIDStorage.Layout;
    using StringsUpgradeable for uint256;
    ...
    function mint(string calldata name, uint256 avatarID) external {
        if (balanceOf(msg.sender) > 0) revert AlreadyOwner();
        if (bytes(name).length == 0) revert InvalidName();

        GuildIDStorage.Layout storage s = GuildIDStorage.layout();
        ++s.counter;
        uint256 tokenID = s.counter;
        s.addressToID[msg.sender] = tokenID;
        s.data[tokenID] = GuildIDData(name, 0, avatarID, block.timestamp);
        ++s.membersInRank[0];

        gs.members[msg.sender] = true;
        emit GuildIDMinted(tokenID, msg.sender, avatarID, name);
        _mint(msg.sender, tokenID);
        _payGuildFee(msg.sender, 0);
    }

    function upgradeName(string calldata newName) external onlyMembers {
        uint256 tokenID = getID(msg.sender);
        if (bytes(newName).length == 0) revert InvalidName();
        emit GuildIDNameUpgraded(tokenID, msg.sender, newName);
        GuildIDStorage.layout().data[tokenID].name = newName;
        _payGuildFee(msg.sender, 1);
    }

    function upgradeRank() external onlyMembers {
        GuildIDStorage.Layout storage s = GuildIDStorage.layout();

        uint256 tokenID = getID(msg.sender);
        uint256 currentRank = s.data[tokenID].rank;
        if (currentRank >= gs.maxRank) revert MaxRank();

        --s.membersInRank[currentRank];
        ++s.membersInRank[currentRank + 1];

        uint256 xpToBurn = gs.rankUpBasePriceXP * (currentRank + 1);
        if (xpToBurn > gs.balances[msg.sender]) revert NotEnoughXP();
        gs.balances[msg.sender] -= xpToBurn;

        emit GuildIDRankUpgraded(tokenID, msg.sender, currentRank + 1);
        ++s.data[tokenID].rank;
        _payGuildFee(msg.sender, 2);
    }

    function upgradeAvatar(uint256 newAvatarID) external onlyMembers {
        uint256 tokenID = getID(msg.sender);
        emit GuildIDAvatarUpgraded(tokenID, msg.sender, newAvatarID);
        GuildIDStorage.layout().data[tokenID].avatarID = newAvatarID;
    ...
    function setImageURL(string memory imageUrl) external onlyManagers {
        gs.imageUrl = imageUrl;
    }

    function setMinRankForTransfers(uint256 minRankForTransfers) external onlyManagers {
        gs.minRankForTransfers = minRankForTransfers;
    }

    function setMaxRank(uint256 maxRank) external onlyManagers {
        gs.maxRank = maxRank;
    }
    ...
    function getName(uint256 tokenID) external view returns (string memory) {
        ...
        return GuildIDStorage.layout().data[tokenID].name;
    }

    function getRank(uint256 tokenID) external view returns (uint256) {
        if (!_exists(tokenID)) revert DoesNotExist();
        return GuildIDStorage.layout().data[tokenID].rank;
    }

    function getMintDate(uint256 tokenID) external view returns (uint256) {
        if (!_exists(tokenID)) revert DoesNotExist();
        return GuildIDStorage.layout().data[tokenID].mintDate;
    }

    function getAvatarID(uint256 tokenID) external view returns (uint256) {
        if (!_exists(tokenID)) revert DoesNotExist();
        return GuildIDStorage.layout().data[tokenID].avatarID;
    }
    ...
    function getMinRankForTransfers() external view returns (uint256) {
        return gs.minRankForTransfers;
    }
    ...
    function getID(address member) public view returns (uint256) {
        if (balanceOf(member) < 1) revert DoesNotExist();
        return GuildIDStorage.layout().addressToID[member];
    }

    function getMembersInRank(uint256 rank) external view returns (uint256) {
        return GuildIDStorage.layout().membersInRank[rank];
    }

    function totalSupply() external view returns (uint256) {
        return GuildIDStorage.layout().counter;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0) || _transferrable(tokenId)) {
            if (balanceOf(to) > 0) revert AlreadyOwner();
            else super._beforeTokenTransfer(from, to, tokenId, 1);
        } else {
    ...
    function _afterTokenTransfer(address from, address to, uint256 tokenId, uint256 /* batchSize */) internal override {
        if (from != address(0)) {
            GuildIDStorage.layout().addressToID[from] = 0;
            GuildIDStorage.layout().addressToID[to] = tokenId;
        }
    ...
    function approve(address to, uint256 tokenId) public override {
        if (_transferrable(tokenId)) super.approve(to, tokenId);
    ...
    function _transferrable(uint256 _tokenId) private view returns (bool) {
        if (!_exists(_tokenId)) revert DoesNotExist();
        return GuildIDStorage.layout().data[_tokenId].rank >= gs.minRankForTransfers;
    }

    function _constructTokenURI(uint256 _tokenID) private view returns (string memory) {
        GuildIDStorage.Layout storage s = GuildIDStorage.layout();

        GuildIDData memory data = s.data[_tokenID];
        string memory tokenIDStr = _tokenID.toString();
        string memory fullImageUrl = string.concat(gs.imageUrl, "/", tokenIDStr);

        // prettier-ignore
        string memory mJson = string.concat(
            '{',
                '"description": "', name(), " - Your On-Chain Membership Pass!", '",',
                '"image": "', fullImageUrl, '",',
                '"name": "', name(), " #", tokenIDStr, '",',
                '"attributes": [', 
                    '{"trait_type":"Name","value":"', data.name, '"}',
                    ',',
                    '{"trait_type":"Avatar","value":"', data.avatarID.toString(), '"}',
                    ',',
                    '{"trait_type":"Rank","value":', data.rank.toString(), ',"max_value": ', gs.maxRank.toString() , '}',
                    ',',
                    '{"display_type": "date","trait_type":"Minted On","value":', data.mintDate.toString(), '}',
                ']',
            '}'            
        );

        return string.concat("data:application/json;base64,", Base64Upgradeable.encode(bytes(mJson)));
    }
    ...
    function _payGuildFee(address user, uint256 actionID) private {
        ...
        address guildToken = gs.guildToken;

        if (actionID == 0) {
            // Mint
            ethFee = gs.mintETH;
            tokenFixedFee = gs.mintGuildToken;
        } else if (actionID == 1) {
            // Name
            ethFee = gs.nameETH;
            tokenFixedFee = gs.nameGuildToken;
        } else if (actionID == 2) {
            // Rank
            ethFee = gs.rankETH;
            tokenFixedFee = gs.rankGuildToken;
        } else if (actionID == 3) {
            // Avatar
            ethFee = gs.avatarETH;
            tokenFixedFee = gs.avatarGuildToken;
        }

        if (msg.value != ethFee) revert InvalidETHValue();

        if (guildToken != address(0)) {
            IERC20Upgradeable(guildToken).transferFrom(user, gs.guildTreasury, tokenFixedFee);
        }
    }
}
...
import {GuildID} from "./GuildID.sol";

library GuildIDStorage {
    struct Layout {
        mapping(address => uint256) addressToID;
        mapping(uint256 => uint256) membersInRank;
        mapping(uint256 => GuildID.GuildIDData) data;
        uint256 counter;
    }

    bytes32 internal constant STORAGE_SLOT = keccak256("guild.contracts.storage.guildid");
...
import "./GuildStorage.sol";
import {GuildIDStorage} from "../guild-id/GuildIDStorage.sol";

// import "./IGuild.sol";
// import "../interfaces/IGuildOracle.sol";
// import "../interfaces/IGuildXP.sol";
// import "../interfaces/IGuildID.sol";
// import "../interfaces/IGuildBank.sol";
// import "../interfaces/IGuildRental.sol";
...
contract Guild is Modifiers {
    ...
    // ) {
    //     if (bytes(name).length == 0 || bytes(symbol).length == 0 || guildTreasury == address(0))
    ...
    // function guildBank_addCollection(address collection) external onlyRole(MANAGER_ROLE) {
    //     IGuildBank(s.contracts.guildBank).addCollection(collection);

    //     emit CollectionAdded(collection);
    // }

    // function guildBank_removeCollection(address collection) external onlyRole(MANAGER_ROLE) {
    //     IGuildBank(s.contracts.guildBank).removeCollection(collection);
    ...
    function setGuildTreasury(address guildTreasury) external onlyAdmin {
        if (guildTreasury == address(0)) revert InvalidParameters();
        gs.guildTreasury = guildTreasury;
    }

    function setGuildToken(address guildToken) external onlyAdmin {
        gs.guildToken = guildToken;
    }

    function setRankUpBasePriceXP(uint256 rankUpBasePriceXP) external onlyAdmin {
        gs.rankUpBasePriceXP = rankUpBasePriceXP;
    }

    function setRankMultiplierPercent(uint256 rankMultiplierPercent) external onlyAdmin {
        gs.rankMultiplierPercent = rankMultiplierPercent;
    }

    function setTotalXPEarnedPerRank(uint256[] calldata totalXPEarnedPerRank) external onlyAdmin {
        if (totalXPEarnedPerRank.length != gs.maxRank + 1) revert InvalidArrayLength();
        gs.totalXPEarnedPerRank = totalXPEarnedPerRank;
    }
    ...
    function setFees(
        ...
        gs.nameETH = nameETH;
        gs.rankETH = rankETH;
        gs.avatarETH = avatarETH;
        gs.mintGuildToken = mintGuildToken;
        gs.nameGuildToken = nameGuildToken;
        gs.rankGuildToken = rankGuildToken;
        gs.avatarGuildToken = avatarGuildToken;
    }

    function withdrawETH() external onlyAdmin {
        payable(gs.guildTreasury).transfer(address(this).balance);
    }
    ...
    function getRankMultiplier(uint256 rank) public view returns (uint256) {
        return 100 + (gs.rankMultiplierPercent * rank);
    }

    function getRankUpCostXP(address user) public view returns (uint256) {
        uint256 tokenID = GuildIDStorage.layout().addressToID[user];
        uint256 rank = GuildIDStorage.layout().data[tokenID].rank;
        return gs.rankUpBasePriceXP * (rank + 1);
    }
    ...
    function _getActualUtilizationRate(uint256 rentalId) private view returns (uint256) {
        // IGuildBank guildBank = IGuildBank(gs.contracts.guildBank);
        // IGuildRental guildRental = IGuildRental(gs.contracts.guildRental);
    ...
    function _getXPCost(address renter, uint256 actualUtilizatonRate) private view returns (uint256 xpCost_) {
        // IGuildRental guildRental = IGuildRental(gs.contracts.guildRental);
        // IGuildID guildID = IGuildID(gs.contracts.guildID);
...
library GuildStorage {
    ...
    bytes32 internal constant STORAGE_SLOT = keccak256("guild.contracts.storage.guild");
...
contract Modifiers {
    GuildStorage.Layout internal gs;

    modifier onlyAdmin() {
        require(msg.sender == gs.admin, "!Admin");
        _;
    }

    modifier onlyManagers() {
        require(gs.managers[msg.sender] == true, "!Manager");
        _;
    }

    modifier onlyMembers() {
        require(gs.members[msg.sender] == true, "!Member");
        _;
    }
}