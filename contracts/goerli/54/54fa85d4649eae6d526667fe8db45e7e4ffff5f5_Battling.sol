...
contract Battling is Ownable, SignerVerifiable {
    ...
    uint256 public num_discounted_battles = 3;
    ...
    modifier callerVerified(uint256 _amount, string memory _message, string memory _battle_id, uint256 _deadline, address _erc20_token, address _player_referral_address, bytes memory _signature) {
    ...
    function initiateBattleERC20(FuncParams memory fp) external frozen callerVerified(fp._amount, fp._message, fp._battle_id, fp._deadline, fp._erc20_token, fp._player_referral_address, fp._signature) {
    ...
    function initiateBattleETH(FuncParams memory fp) external payable frozen callerVerified(msg.value, fp._message, fp._battle_id, fp._deadline, address(0), fp._player_referral_address, fp._signature) {
    ...
    function claimWinnings(FuncParams memory fp) external frozen callerVerified(battle_contestants[fp._battle_id].value, fp._message, fp._battle_id, fp._deadline, battle_contestants[fp._battle_id].token_address, fp._player_referral_address, fp._signature) {
        ...
        // Use the reduced fee if the user has done few enough battles and there is a referral code present, otherwise use the normal fee
        uint fee_to_use = (num_battles_per_user[msg.sender] < num_discounted_battles && ref != address(0) ? reduced_battle_fee_percent : battle_fee_precent);
        ...
        if (token_address == address(0)) {
            ...
            unchecked {
                if (token_to_aggregator_address[token_address] != address(0)) {
                    total_wagered_under_account_code[ref] += int(battle_value * (100 - fee_to_use) / 100) * getLatestPrice(token_address) / _pow(18);
                }
            }
            ...
            unchecked {
                if (token_to_aggregator_address[token_address] != address(0)) {
                    total_wagered_under_account_code[ref] += int(battle_value * (100 - fee_to_use) / 100) * getLatestPrice(token_address) / _pow(ERC20(token_address).decimals());
                }
            }
        }
    }

    function cancelBattle(FuncParams memory fp) external frozen callerVerified(battle_contestants[fp._battle_id].value, fp._message, fp._battle_id, fp._deadline, battle_contestants[fp._battle_id].token_address, fp._player_referral_address, fp._signature) {
    ...
    function _calculateRefOwedAmount(address _ref_address, uint _fee_amount) internal view returns(uint) {
        ...
        uint[9] memory REFERRAL_TIERS_AMT_REQUIRED = [uint(2_000), uint(5_000), uint(10_000), uint(25_000), uint(50_000), uint(100_000), uint(250_000), uint(500_000), uint(1_000_000)];
    ...
    function updateNumDiscountedBattles(uint256 _new_num_discounted_battles) external onlyOwner {
        num_discounted_battles = _new_num_discounted_battles;
    }
