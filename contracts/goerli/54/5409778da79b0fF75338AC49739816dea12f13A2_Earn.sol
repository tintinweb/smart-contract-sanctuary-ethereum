...
contract Earn is AccessControl {
    ...
    IsHYDT public sHYDT;
    /// @notice The address of the collector wallet.
    ...
    /// @notice User data within each corresponding farm pool.
    mapping (uint256 => mapping (address => mapping (uint256 => UserInfo))) public userInfo;

    /* ========== EVENTS ========== */

    event AddPool(uint256 indexed stakeType, uint256 allocPoint);
    event UpdateAllocation(uint256 indexed stakeType, uint256 newAllocPoint, uint256 oldAllocPoint);
    event Stake(
    ...
    function initialize(address hydt_, address hygt_, address shydt_, address treasury_, uint256 initialMintStartTime_) external {
        ...
        depositFee = 15;
        _dailyYields = [
            0.000448888888888888 * 1e18,
            0.000554444444444444 * 1e18,
            0.000820273972602739 * 1e18
        ];
        dailyPayouts = [
            1.156 * 1e18,
            0.611 * 1e18,
    ...
    function _addPool(uint8 stakeType, uint256 allocPoint, uint256 lastRewardBlock) internal {
        ...
        emit AddPool(stakeType, allocPoint);
    }
     ...
     * @param allocPoint New allocation point value.
     * @param withUpdate Whether or not to update reward variables for all pools. High gas spending.
     */
    function updateAllocation(uint8 stakeType, uint256 allocPoint, bool withUpdate) public onlyRole(GOVERNOR_ROLE) {
        if (withUpdate) {
            massUpdatePools();
        }
        ...
        emit UpdateAllocation(stakeType, allocPoint, oldAllocPoint);
    }

    /**
     * @notice Updates reward variables for all pools. High gas spending.
     */
    ...
    // function claimPending(uint8 stakeType) external {
    //     PoolInfo storage pool = poolInfo[stakeType];
    //     UserInfo storage user = userInfo[stakeType][_msgSender()];
    //     updatePool(stakeType);
    //     uint256 pending = ((user.amount * pool.accHYGTPerShare) / 1e12) - user.rewardDebt;

    //     if(pending > 0) {
    //         HYGT.mint(_msgSender(), pending);
    //     }
    //     user.rewardDebt = (user.amount * pool.accHYGTPerShare) / 1e12;
    // }

    /**
     * @notice Gets 
     */
    function getPendingBatch(address account) external view returns (uint256) {
        uint256 totalPending;

        for (uint256 i = 0 ; i < getStakings[account].length ; i++) {
            totalPending += getPending(getStakings[account][i].stakeType, account, i);
        }
        return totalPending;
    }

    function getPendingType(uint8 stakeType, address account) external view returns (uint256) {
        uint256 totalPending;

        for (uint256 i = 0 ; i < getStakings[account].length ; i++) {
            if (getStakings[account][i].stakeType == stakeType) {
                totalPending += getPending(stakeType, account, i);
            }
        }
        return totalPending;
    }
    ...
    function getPending(uint8 stakeType, address account, uint256 index) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[stakeType];
        UserInfo storage user = userInfo[stakeType][account][index];
        ...
        return ((user.amount * accHYGTPerShare) / 1e12) - user.rewardDebt;
    }

    function getDailyPayoutBatch(address account) external view returns (uint256) {
        uint256 totalDailyPayout;

        for (uint256 i = 0 ; i < getStakings[account].length ; i++) {
            totalDailyPayout += getDailyPayout(account, i);
        }
        return totalDailyPayout;
    }

    function getDailyPayout(address account, uint256 index) public view returns (uint256) {
        uint256 dailyPayout;
        Staking memory staking = getStakings[account][index];
    ...
    function getYieldBatch(address account) external view returns (uint256) {
        uint256 totalYield;

        for (uint256 i = 0 ; i < getStakings[account].length ; i++) {
            totalYield += getYield(account, i);
        }
        return totalYield;
    }

    function getYieldType(address account, uint8 stakeType) external view returns (uint256) {
        uint256 totalYield;
    ...
    function getYield(address account, uint256 index) public view returns (uint256) {
        uint256 yield;
        Staking memory staking = getStakings[account][index];
    ...
    // Deposit LP tokens to Earn for HYGT allocation.
    function _deposit(uint8 stakeType, uint256 index, uint256 amount) internal {
        PoolInfo storage pool = poolInfo[stakeType];
        UserInfo storage user = userInfo[stakeType][_msgSender()][index];
        updatePool(stakeType);
        // uint256 pending = ((user.amount * pool.accHYGTPerShare) / 1e12) - user.rewardDebt;
    ...
    // Withdraw LP tokens from Earn.
    function _withdraw(uint8 stakeType, uint256 index, uint256 amount) internal {
        PoolInfo storage pool = poolInfo[stakeType];
        UserInfo storage user = userInfo[stakeType][_msgSender()][index];

        require(user.amount >= amount, "Earn: not good");
        updatePool(stakeType);
        uint256 pending = ((user.amount * pool.accHYGTPerShare) / 1e12) - user.rewardDebt;
    ...
    function stake(uint256 amount, uint8 stakeType) external {
        ...
        _deposit(stakeType, getStakings[_msgSender()].length, amountWithFee);

        Staking memory staking;
        staking.index = getStakings[_msgSender()].length;
        staking.status = true;
        ...
        getStakings[_msgSender()].push(staking);
    ...
    function unstake(uint256 index) public {
        ...
        uint256 yield = getYield(_msgSender(), index);
        uint256 amountMintHYDT = staking.amount + yield;
        HYDT.mint(_msgSender(), amountMintHYDT);
        sHYDT.burn(staking.amount);
        _withdraw(staking.stakeType, staking.index, staking.amount);
