...
contract WorldCupMemoryNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    // tokenId -> teamId
    mapping(uint256 => uint256) public tokenAtTeam;
    mapping(uint256 => uint256) public tokenTeamMinted;
    mapping(uint256 => uint256) public tokenTeamBurned;
        ...
        nonReentrant
    {
        ...
        tokenTeamMinted[_teamId] += 1;
        uint supply = teamTotalSupply(_teamId);
        uint cost = BASE_COST * supply + CREATOR_FEE + REPURCHASE_POOL_FEE;
        require(msg.value >= cost, "cost is not enough");
        uint256 tokenId = totalCreated++;
        ...
        tokenAtTeam[tokenId] = _teamId;
        teamMintId[tokenId] = supply;
    ...
    function publicMint(uint256 _teamId) public payable validTeamId(_teamId) {
        require(publicSaleOpen, "Public-sale is not open");
        uint tokenId = totalCreated + 1;
        uint supply = teamTotalSupply(_teamId);
        uint cost = BASE_COST * supply + CREATOR_FEE + REPURCHASE_POOL_FEE;
        require(msg.value >= cost, "cost is not enough");
        _safeMint(msg.sender, tokenId);
    ...
    function harvest(uint tokenId) public {
        ...
        uint growth = (tokenTeamMinted[tokenId] - teamMintId[tokenId]);
        uint produce = growth * BASE_COST;
        tokenTeamBurned[tokenId] += 1;
        _burn(tokenId);
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    base,
                    tokenAtTeam[tokenID].toString(),
                    BASE_EXTENSION
                )
            );
    }

    function teamTotalSupply(uint256 tokenId) public view returns (uint256) {
        return tokenTeamMinted[tokenId] - tokenTeamBurned[tokenId];
    }
    ...
    function withdrawAll() public onlyOwner {
        ...
        require(ownerWithdraw, "Failed to withdraw Ether");
    }
}
