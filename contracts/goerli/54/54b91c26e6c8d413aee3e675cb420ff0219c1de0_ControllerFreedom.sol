...
contract ControllerFreedom is Initializable, UUPSUpgradeable, OwnableUpgradeable {
	...
	event EvtCreateGroup( uint256[] groupIds );
    ...
    */
    ...
    /**
     * @notice create group of community and return array of id of the group
     * external function to create group from the proposal
     */
    function createCommunityGroupFn(uint256 daoId, Group[] memory group) external onlyProxy returns(bool) { 
        uint256[] memory groupIdsArr = createCommunityGroup(daoId, group);
        emit EvtCreateGroup(groupIdsArr);
        return true;     
    }
     ...
     */
    function transferBounty(uint _amount, address _targetAcc, uint256 commId) external onlyProxy returns (bool){
        require(assertVotingContractAddr() == true, "You are unauthorized user.");
        Community storage comm = fetchAssertCommunity(commId, "Can't change community data, bcz it doesn't exist.");
        require(comm.balance >= _amount, "Community does'nt have enough funds.");
...
interface COMMUNITY{
	...
	// proposed functions
	function transferBounty(string memory amount, string memory account, uint256 commId) external returns (bool);
