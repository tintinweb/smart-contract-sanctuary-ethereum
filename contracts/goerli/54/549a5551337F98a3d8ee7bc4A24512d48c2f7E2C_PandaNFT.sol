...
contract PandaNFT is VRFConsumerBaseV2, ERC721A, Ownable {
    uint256 public constant MAX_SUPPLY = 100;
    uint256 public MINT_FEE = 0.001 ether;
    uint256 public constant MINT_FEE_RANDOM = 0.001 ether;
    ...
    mapping(uint256 => RequestStatus) public requestResult;
    ...
    constructor(
        address _owner
    ) VRFConsumerBaseV2(vrfCoordinatorAddress) ERC721A("PandaNFT", "Panda") {
        require(_owner != address(0), "owner address cannot be 0");
        //require(_subscriptionId != 0, "subscriptionId cannot be 0");
        //require(
        //    _vrfCoordinatorAddress != address(0),
        //    "vrfCoordinator address cannot be 0"
        //);
        //require(_keyHash != 0, "keyHash cannot be 0");
    ...
    function randomMint() external payable {
        ...
        requestResult[requestId] = RequestStatus({
    ...
    function fulfillRandomWords(
        ...
        require(requestResult[_requestId].exists, "request not found");
        require(_randomWords.length == 2, "Two random numbers are expected");
        require(_randomWords[0] > 0, "random number 0 not valid");
        require(_randomWords[1] > 0, "random number 1 not valid");
        requestResult[_requestId].fulfilled = true;
        requestResult[_requestId].randomWords = _randomWords;
        emit RequestFulfilled(_requestId, _randomWords);
    }

    function claim() external {
        uint256 requestId = addressToRequestId[msg.sender];
        require(requestId != 0, "requestId cannot be 0");
        require(requestResult[requestId].exists, "request not found");
        require(requestResult[requestId].fulfilled, "request not fulfilled");
        require(!requestResult[requestId].claimed, "request claimed");

        if (
            requestResult[requestId].randomWords[0] >
            requestResult[requestId].randomWords[1]
        ) {
            _safeMint(msg.sender, 1);
            emit NewMint(msg.sender, 1);
        }

        requestResult[requestId].claimed = true;
    }
}
