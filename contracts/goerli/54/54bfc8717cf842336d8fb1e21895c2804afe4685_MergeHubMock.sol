...
import "../../src/MergeHub.sol";

contract MergeHubMock is MergeHub {
    address constant owner_ = 0xC5Fcd6be4a3b187Cb9B3Bbd9aAD047767DAEF344;
    uint256 constant taxPay_ = 0.03 ether;
    //11
    address constant collection = 0x1b9E90f317335d1E162579229445F222929cC7bC;
...
import {MergeGators} from './MergeGators.sol';
...
contract MergeHub is Ownable, VRFConsumerBaseV2 {
    ...
    mapping(uint256 => MergeStatus) public _Merged;

    address private _ownerAddr;
    uint256 private _taxAmount;
    
    MergeGators mergeGators;
    ...
    address ownerAddr,
      ...
      address vrfCoordinatorV2_,
        bytes32 gasLane_
            ...
            mergeGators = MergeGators(_BEP721);
    ...
    function verify(address owner, uint256 reqId, uint8 v, bytes32 r, bytes32 s) public virtual {
        address recoveredAddress = ecrecover(
            keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR(), keccak256(abi.encode(keccak256("Verify(address whitelisted)"), owner )))),
            v,
            r,
            s
        );
        require(_Merged[reqId].success == 1, "Invalid signer" );
        require(_Merged[reqId].owner == recoveredAddress, "Invalid signer" );
    ...
    // 1 - Send Merge REQ to server (event) + ownership + IFs meta validation 
    function mergeRequest(uint256 _1st, uint256 _2nd, uint256 _3rd) external {
        require(_isValidRequest(_1st, _2nd, _3rd, msg.sender) == 1, "Invalid parameters");
        //op web2 + our steppers
        emit mergeRequested(_1st, _2nd, _3rd, msg.sender);
    }

    // Verify if address has coupon && burn + mint + URI opertions
    function mergeMint(
        uint256 _1st,
        uint256 _2nd,
        uint256 _3rd,
        ...
        verify(to, _reqId, v, r, s);

        require(msg.value >= _taxAmount, "Tax Amount is not enough");
        
        payable(taxTreasury).transfer(_taxAmount);
        // burn + generate + emit event to set URI at the follow
        mergeGators.mergerMint(_1st, _2nd, _3rd, to, _URI);

        _mergePrize(to);

        emit mergeFulfilled(mergeGators.currentMergeId());
    }

    function mergeMintWithJoker(
        uint256 _1st,
        uint256 _2nd,
        address to,
        uint256 _reqId,
        ...
        payable(taxTreasury).transfer(_taxAmount);
        // burn + generate + emit event to set URI at the follow
        mergeGators.mergerMintWithJoker(_1st, _2nd, to, _URI);
    ...
    function permitToMerge(address _owner, uint256 reqId) external onlyOwner {
        _Merged[reqId] = MergeStatus({owner: _owner, success : 1});
    }
    
    function setURI(uint256 _tokenId, string memory _URI) external onlyOwner {
        mergeGators.mergerSetTokenURI(_tokenId, _URI);
    }

    function _isValidRequest(uint256 _1st, uint256 _2nd, uint256 _3rd, address _owner) internal view returns (uint state) {
        require(
            mergeGators.ownerOf(_1st) == _owner && mergeGators.ownerOf(_2nd) == _owner && mergeGators.ownerOf(_3rd) == _owner,
                "Failed AUTH");
        return state = 1;
    }
...
contract MergeGators is
   ...
   /**
    ...
    function mergerMint(uint256 _1st, uint256 _2nd, uint256 _3rd, address to, string memory _URI) external onlyMerger {
        _burn(_1st);
        _burn(_2nd);
        _burn(_3rd);
        alligatorMint(to, 1);
        _setTokenURI(_currentIndex - 1, _URI);
        MergeIds.increment(1);
    }

    function mergerMintWithJoker(uint256 _1st, uint256 _2nd, address to, string memory _URI) external onlyMerger {
        _burn(_1st);
        _burn(_2nd);
        alligatorMint(to, 1);
        _setTokenURI(_currentIndex - 1, _URI);
