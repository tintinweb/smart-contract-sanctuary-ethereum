...
contract Emitter is Initializable, AccessControl {
    ...
    function changeImplementationAddress(address _newImp, bool _isERC721)
        external
        payable
        onlyOwner
    {
        emit ChangeImplementationAddress(msg.sender, _newImp, _isERC721);
    }
    ...
    function whitelistAddress(address _nftAddress, address _address)
        external
        onlyRole(EMITTER)
    {
    ...
    function removeWhitelistAddress(address _nftAddress, address _address)
        external
        onlyRole(EMITTER)
    {
...
contract FactoryCloneContract is Ownable, Helper {
    address public ERC20Implementation;
    address public ERC721Implementation;
    address public USDCAddress;
    address public emitterAddress;

    //Mapping to get details of a particular erc20 template dao
    mapping(address => ERC20DAOdetails) internal erc20DAOdetails;

    //Mapping to get details of a particular erc721 template dao
    mapping(address => ERC721DAOdetails) internal erc721DAOdetails;

    //Mapping to get details of token gating for a particular dao
    mapping(address => MultiTokensObject) internal tokenGatingDetails;

    /// @dev Initial Deployment of Token and Governor contract and storing their addresses for proxies creation
    constructor(address _USDC) {
        if (_USDC == address(0)) revert AddressInvalid("_USDC", _USDC);
    ...
    function defineTokenContracts(
        ...
        //Deploying ERC20 implementation contract for the reference
        ERC20Implementation = ERC20ImplementationAddress;

        //Deploying ERC721 implementation contract for the reference
    ...
    /// @dev This function returns details of a particular erc721 template dao
    /// @param _tokenAddress address of token contract
    function getERC721DAOdetails(
        address _tokenAddress
    ) external view returns (ERC721DAOdetails memory) {
        return erc721DAOdetails[_tokenAddress];
    }

    /// @dev This function returns details of a particular erc20 template dao
    /// @param _tokenAddress address of token contract
    function getERC20DAOdetails(
        address _tokenAddress
    ) external view returns (ERC20DAOdetails memory) {
        return erc20DAOdetails[_tokenAddress];
    }

    /// @dev This function returns tokens list of a particular token gated dao
    /// @param _tokenAddress address of token contract
    function getGatingTokensOfDaoToken(
        address _tokenAddress
    ) public view returns (address[] memory) {
        return tokenGatingDetails[_tokenAddress].tokenList;
    }

    /// @dev This function returns token operations of a particular token gated dao
    /// @param _tokenAddress address of token contract
    /// @param _index index of a particular token used for token gating
    function getTokenOperations(
        address _tokenAddress,
        uint256 _index
    ) public view returns (string memory) {
        return tokenGatingDetails[_tokenAddress].tokenOperations[_index];
    }

    /// @dev This function returns minimum tokens required for a particular token gated dao
    /// @param _tokenAddress address of token contract
    /// @param _token address of a particular token used for token gating
    function getMinGatedTokenReqToBuyDaoToken(
        address _tokenAddress,
        address _token
    ) public view returns (uint256) {
        return tokenGatingDetails[_tokenAddress].minTokenRequired[_token];
    }

    /// @dev Function to change merkle root of particular token contract
    /// @param _tokenAddress address token contract
    function changeMerkleRoot(
        address _tokenAddress,
        bytes32 _newMerkleRoot
    ) external payable onlyAdmins(_tokenAddress) {
        if (!erc721DAOdetails[_tokenAddress].isDeployedByFactory)
            revert AddressInvalid("_tokenAddress", _tokenAddress);
        erc721DAOdetails[_tokenAddress].merkleRoot = _newMerkleRoot;
    }

    /// @dev Function to change base Governor contract address
    /// @param _newImplementationAddress address of new Governor contract
    /// @param _isERC721 if implementation erc721 template implementation
    function changeDAOImplementation(
        ...
        if (_isERC721) {
            ERC721Implementation = _newImplementationAddress;
        } else {
            ERC20Implementation = _newImplementationAddress;
        }
    ...
    function createDAO(
        ...
        if (_quorum > 100) revert AmountInvalid("_quorum", _quorum);

        if (_threshold == 0) revert AmountInvalid("_threshold", _threshold);
        if (_threshold > 100) revert AmountInvalid("_threshold", _threshold);

        if (_depositTime == 0)
            revert AmountInvalid("_depositFunctioningDays", _depositTime);

        if (!(_ownerFeePerDeposit < 100))
        ...
        if (_totalRaiseAmount < _maxDepositPerUser)
            revert RaiseAmountInvalid(_totalRaiseAmount, _maxDepositPerUser);

        bytes memory data = abi.encodeWithSignature(
            "initializeERC20(string,string,address,address,address[])",
            _DaoName,
            _DaoSymbol,
            address(this),
            emitterAddress,
        ...
        erc20DAOdetails[_tokenAddress] = ERC20DAOdetails(
            _DaoName,
            _DaoSymbol,
            _totalRaiseAmount,
            _minDepositPerUser,
    ...
    function createDaoERC721(
        ...
        if (_totalSupplyOfToken == 0)
            revert AmountInvalid("_totalSupplyOfToken", _totalSupplyOfToken);

        if (_maxTokensPerUser == 0)
            revert AmountInvalid("_maxTokensPerUser", _maxTokensPerUser);

        bytes memory data = abi.encodeWithSignature(
            "initializeERC721(string,string,address[],address,address)",
            _DaoName,
            _DaoSymbol,
            _daoAdmins,
            address(this),
        ...
        erc721DAOdetails[_tokenAddress] = ERC721DAOdetails(
            _DaoName,
            _DaoSymbol,
            _ownerFeePerDeposit,
            block.timestamp + _depositTime,
    ...
    function changeUSDCAddress(
        address _newUSDCAddress
    ) external payable onlyOwner {
        if (_newUSDCAddress == address(0))
            revert AddressInvalid("_newUSDCAddress", _newUSDCAddress);

        USDCAddress = _newUSDCAddress;
        Emitter(emitterAddress).changedUSDCAddress(_newUSDCAddress);
    }

    //ERC721 implementation functions

    /// @dev Function to set whitelist to true for a particular token contract
    /// @param _tokenAddress address of the token contract
    function toggleOnlyAllowWhitelist(
        address _tokenAddress
    ) external payable onlyAdmins(_tokenAddress) {
        if (!erc721DAOdetails[_tokenAddress].isDeployedByFactory)
            revert AddressInvalid("_tokenAddress", _tokenAddress);

        erc721DAOdetails[_tokenAddress].onlyAllowWhitelist = !erc721DAOdetails[
            _tokenAddress
        ].onlyAllowWhitelist;
    }

    /// @dev Function to update max tokens per user for a particular token contract
    /// @param _maxTokensPerUser number of max tokens per user
    /// @param _tokenAddress address of the token contract
    function updateMaxTokensPerUser(
        uint256 _maxTokensPerUser,
        address _tokenAddress
        ...
        erc721DAOdetails[_tokenAddress].maxTokensPerUser = _maxTokensPerUser;
    ...
    /// @dev Function to update total supply of tokens for a particular token contract
    /// @param _totalSupplyOfToken New total supply of tokens
    /// @param _tokenAddress address of the token contract
    function updateTotalSupplyOfToken(
        uint256 _totalSupplyOfToken,
        address _tokenAddress
        ...
        if (_totalSupplyOfToken == 0)
            revert AmountInvalid("_maxTokensPerUser", _totalSupplyOfToken);

        if (!erc721DAOdetails[_tokenAddress].isDeployedByFactory)
            revert AddressInvalid("_tokenAddress", _tokenAddress);

        erc721DAOdetails[_tokenAddress]
            .totalSupplyOfToken = _totalSupplyOfToken;

        Emitter(emitterAddress).updateTotalSupplyOfToken(
            _tokenAddress,
            _totalSupplyOfToken
        );
    }
    ...
    function updateNftTransferability(
        ...
        erc721DAOdetails[_tokenAddress].isNftTransferable = _isNftTransferable;

        Emitter(emitterAddress).updateNftTransferability(
            _tokenAddress,
            _isNftTransferable
        );
    }

    /// @dev Function to set minting to open for a particular token contract
    /// @param _tokenAddress address of the token contract
    function toggleMinting(
        ...
        erc721DAOdetails[_tokenAddress].isMintingOpen = !erc721DAOdetails[
            _tokenAddress
        ].isMintingOpen;
    }
    ...
    function updateGovernanceActive(
        ...
        if (!erc20DAOdetails[_tokenAddress].isDeployedByFactory)
            revert AddressInvalid("_tokenAddress", _tokenAddress);

        erc20DAOdetails[_tokenAddress].isGovernanceActive = _isGovernanceActive;
    }

    /// @dev Function to update raise amount for a particular token contract
    /// @param _totalRaiseAmount New total amount to raise
    /// @param _tokenAddress address of the token contract
    function updateRaiseAmount(
        address _tokenAddress,
        uint256 _totalRaiseAmount
    ) external payable onlyTokenContract(_tokenAddress) {
        if (!erc20DAOdetails[_tokenAddress].isDeployedByFactory)
            revert AddressInvalid("_tokenAddress", _tokenAddress);

        erc20DAOdetails[_tokenAddress].totalRaiseAmount = _totalRaiseAmount;
    }

    /// @dev Function to update Minimum and Maximum deposits allowed by DAO members
    /// @param _quorum New quorum
    /// @param _threshold New threshold
    /// @param _isERC721 New maximum deposit limit amount in wei
    /// @param _tokenAddress address of the token contract
    function updateGovernanceSettings(
        address _tokenAddress,
        uint256 _quorum,
        ...
        if (_isERC721) {
            if (!erc721DAOdetails[_tokenAddress].isDeployedByFactory)
                revert AddressInvalid("_tokenAddress", _tokenAddress);
            erc721DAOdetails[_tokenAddress].quorum = _quorum;
            erc721DAOdetails[_tokenAddress].threshold = _threshold;
        } else {
            if (!erc20DAOdetails[_tokenAddress].isDeployedByFactory)
                revert AddressInvalid("_tokenAddress", _tokenAddress);
            erc20DAOdetails[_tokenAddress].quorum = _quorum;
            erc20DAOdetails[_tokenAddress].threshold = _threshold;
        }
    }
    ...
    function updateMinMaxDeposit(
        ...
        if (erc20DAOdetails[_tokenAddress].minDepositPerUser == 0)
            revert AmountInvalid("_minDepositPerUser", _minDepositPerUser);

        if (_minDepositPerUser < _maxDepositPerUser)
            revert DepositAmountInvalid(_maxDepositPerUser, _minDepositPerUser);

        erc20DAOdetails[_tokenAddress].minDepositPerUser = _minDepositPerUser;
        erc20DAOdetails[_tokenAddress].maxDepositPerUser = _maxDepositPerUser;
    ...
    function updateOwnerFee(
        ...
        if (!(_ownerFeePerDeposit < 100))
            revert AmountInvalid("_ownerFeePerDeposit", _ownerFeePerDeposit);
        erc20DAOdetails[_tokenAddress].ownerFeePerDeposit = _ownerFeePerDeposit;
    ...
    // /// @param _tokenAddress address of the token contract
    // function closeDeposit(address _tokenAddress)
    //     external
    //     payable
    //     onlyAdmins(_tokenAddress)
    // {
    //     if (erc20DAOdetails[_tokenAddress].depositCloseTime < block.timestamp)
    //         revert DepositClosed();

    //     erc20DAOdetails[_tokenAddress].depositCloseTime = block.timestamp;
    //     Emitter(emitterAddress).closeDeposit(_tokenAddress, block.timestamp);
    // }
    ...
    function startDeposit(
        ...
        if (erc20DAOdetails[_tokenAddress].depositCloseTime > block.timestamp)
            revert DepositStarted();

        erc20DAOdetails[_tokenAddress].depositCloseTime =
            block.timestamp +
            _depositTime;
    ...
    function setuptokenGating(
        ...
        if (_tokenList.length > 4) revert Max4TokensAllowed(_tokenList.length);

        if (_tokenList.length != _minTokenRequired.length)
            revert ArrayLengthMismatch(
                _tokenList.length,
                _minTokenRequired.length
            );

        if (_tokenList.length != _tokenOperations.length + 1)
            revert ArrayLengthMismatch(
                _tokenList.length,
                _tokenOperations.length + 1
            );

        tokenGatingDetails[_tokenAddress].tokenList = _tokenList;

        uint256 leng = _tokenList.length;
        for (uint8 i; i < leng; ) {
            tokenGatingDetails[_tokenAddress].minTokenRequired[
                _tokenList[i]
            ] = _minTokenRequired[i];
            tokenGatingDetails[_tokenAddress].tokenOperations[
                i
            ] = _tokenOperations[i];
        ...
        erc20DAOdetails[_tokenAddress].isTokenGatingApplied = true;
    }
    ...
    function disabletokenGating(
        ...
        erc20DAOdetails[_tokenAddress].isTokenGatingApplied = false;
    }
}
...
contract Helper {
    ...
    /// @dev onlyOwner modifier to allow only Owner access to functions
    modifier onlyGnosis(address _factory) {
        ERC20DAOdetails memory _details = FactoryCloneContract(_factory)
            .getERC20DAOdetails(address(this));
        require(_details.gnosisAddress == msg.sender, "Only Owner");
        _;
    }
    ...
    modifier onlyAdmins(address _proxy) {
        require(
            IAccessControl(_proxy).hasRole(ADMIN, msg.sender),
            "Only Admin"
        );
        _;
    }

    modifier onlyTokenContract(address _tokenAddress) {
    ...
    Helper
{
    ...
    /// @dev function to deposit USDC tokens and receive dao tokens in return
    /// @param _usdcTokenAddress address of USDC token
    /// @param _totalAmount amount of tokens to deposit
    function buyGovernanceToken(address _usdcTokenAddress, uint256 _totalAmount)
        external
    {
        ERC20DAOdetails memory _details = FactoryCloneContract(factoryAddress)
            .getERC20DAOdetails(address(this));

        if (_details.depositCloseTime < block.timestamp) revert DepositClosed();

        if (
            _usdcTokenAddress !=
            FactoryCloneContract(factoryAddress).USDCAddress()
        ) revert AddressInvalid("usdcAddress", _usdcTokenAddress);

        if (!(_totalAmount >= _details.minDepositPerUser))
            revert AmountInvalid("_totalAmount", _totalAmount);

        if (!(_totalAmount <= _details.maxDepositPerUser))
            revert AmountInvalid("_totalAmount", _totalAmount);

        bool finalCheck = true;

        if (_details.isTokenGatingApplied) {
            finalCheck = ifTokenGatingApplied();
        }

        require(finalCheck, "Token Gating Checks Failed");

        uint256 daoBalance = IERC20Extented(_usdcTokenAddress).balanceOf(
            address(this)
        );

        daoBalance += _totalAmount;

        if (daoBalance > _details.totalRaiseAmount)
            revert AmountInvalid("daoBalance", daoBalance);

        uint256 adminShareUsdc = (_totalAmount *
            (_details.ownerFeePerDeposit)) / (FLOAT_HANDLER_TEN_4);
        uint256 userShareUsdc = _totalAmount - adminShareUsdc;

        uint256 customTokenDecimals = IERC20Extented(_usdcTokenAddress)
            .decimals();
        uint256 customTokenConversion = 1 * 10**customTokenDecimals;

        uint256 totalGtTokens = (_totalAmount * (EIGHTEEN_DECIMALS)) /
            (customTokenConversion);
        uint256 userShareGt = (userShareUsdc * (EIGHTEEN_DECIMALS)) /
            (customTokenConversion);

        checkFeeUSDCAndMintTokens(
            _usdcTokenAddress,
            userShareUsdc,
            adminShareUsdc,
            totalGtTokens,
    ...
    function ifTokenGatingApplied() internal view returns (bool) {
        bool finalCheck;
        address[] memory _tokenList = FactoryCloneContract(factoryAddress)
            .getGatingTokensOfDaoToken(address(this));

        uint256 tokenBalance1 = IERC20Extented(_tokenList[0]).balanceOf(
            msg.sender
        );

        bool check1 = tokenBalance1 >=
            FactoryCloneContract(factoryAddress)
                .getMinGatedTokenReqToBuyDaoToken(address(this), _tokenList[0]);

        uint256 leng = _tokenList.length - 1;
        for (uint8 i = 1; i < leng; ) {
            address tokenAddress2 = _tokenList[i + 1];
            uint256 tokenBalance2 = IERC20Extented(tokenAddress2).balanceOf(
                msg.sender
            );
            bool check2 = tokenBalance2 >=
                FactoryCloneContract(factoryAddress)
                    .getMinGatedTokenReqToBuyDaoToken(
                        address(this),
                        tokenAddress2
                    );
            ...
            bytes32 tokenOperation = keccak256(
                abi.encodePacked(
                    FactoryCloneContract(factoryAddress).getTokenOperations(
                        address(this),
                        i - 1
                    )
                )
    ...
    /// @dev This internal function checks if FeeUSDC is true and mints tokens
    function checkFeeUSDCAndMintTokens(
        address _usdcTokenAddress,
        uint256 userShareUsdc,
        uint256 adminShareUsdc,
        uint256 totalGtTokens,
        uint256 _totalAmount,
        uint256 userShareGt
    ) internal {
        ERC20DAOdetails memory _details = FactoryCloneContract(factoryAddress)
            .getERC20DAOdetails(address(this));
        if (_details.ownerFeePerDeposit > 0) {
            IERC20Extented(_usdcTokenAddress).transferFrom(
                msg.sender,
                address(this),
                userShareUsdc
            );
            ...
            mintToken(_details.deployerAddress, (totalGtTokens - userShareGt));
    ...
    function updateProposalAndExecution(bytes memory _data)
        external
        onlyGnosis(factoryAddress)
        nonReentrant
    {
    ...
    /// @dev Function to make calls to external contracts
    ...
    /// @dev Function to add DAO admins
    /// @param _daoAdminAddresses array of addresses to add as admins
    function addDaoAdmin(address[] memory _daoAdminAddresses)
        external
        onlyCurrentContract
    {
        if (_daoAdminAddresses.length == 0)
            revert AmountInvalid(
                "_daoAdminAddresses.length",
                _daoAdminAddresses.length
            );

        uint256 leng = _daoAdminAddresses.length;
        for (uint256 i; i < leng; ) {
    ...
    function sendCustomToken(
        ...
        if (_amountArray.length != _addresses.length)
            revert ArrayLengthMismatch(_amountArray.length, _addresses.length);

        uint256 _minimumRequired = 0;
        uint256 leng = _amountArray.length;
        for (uint256 j; j < leng; ) {
            _minimumRequired = _minimumRequired + _amountArray[j];
            unchecked {
                ++j;
            }
        }
        ...
        if (
            !(_minimumRequired <=
                IERC20Extented(_customTokenAddress).balanceOf(address(this)))
        ) revert InsufficientFunds();

        for (uint256 i; i < leng; ) {
            IERC20Extented(_customTokenAddress).transfer(
                _addresses[i],
                _amountArray[i]
            );
            unchecked {
        ...
        onlyCurrentContract
    {
        ...
        FactoryCloneContract(factoryAddress).updateGovernanceSettings(
            address(this),
            _quorum,
            _threshold,
            false
    ...
    function airDropToken(
        ...
        ERC20DAOdetails memory _details = FactoryCloneContract(factoryAddress)
            .getERC20DAOdetails(address(this));
        if (!(_ownersAirdropFees < 100))
            revert AmountInvalid("_ownershipAirdropFees", _ownersAirdropFees);

        if (!(_ownersAirdropFees >= 0))
            revert AmountInvalid("_ownershipAirdropFees", _ownersAirdropFees);

        uint256 _holdings = IERC20Extented(_airdropTokenAddress).balanceOf(
            address(this)
        );

        if (!(_holdings >= _airdropAmount)) revert InsufficientFunds();

        uint256 _updatedAmount;
        if (_ownersAirdropFees == 0) {
            _updatedAmount = _airdropAmount;
        } else {
            uint256 _deployerShare = ((_airdropAmount) * (_ownersAirdropFees)) /
                (FLOAT_HANDLER_TEN_4);
            IERC20Extented(_airdropTokenAddress).transfer(
                _details.deployerAddress,
                _deployerShare
            );
            _updatedAmount = _airdropAmount - _deployerShare;
        }

        uint256 leng = _members.length;
        for (uint256 i = 0; i < leng; ) {
            uint256 _dropAmount;
            _dropAmount =
                (balanceOf(_members[i]) * (_updatedAmount)) /
                (totalSupply());
            IERC20Extented(_airdropTokenAddress).transfer(
                _members[i],
                _dropAmount
    ...
    function updateRaiseAmount(uint256 _totalRaiseAmount)
        external
        onlyCurrentContract
    {
        if (_totalRaiseAmount == 0)
            revert AmountInvalid("_totalRaiseAmount", _totalRaiseAmount);
        FactoryCloneContract(factoryAddress).updateRaiseAmount(
            address(this),
            _totalRaiseAmount
        );
        ...
        returns (bool)
    {
        ERC20DAOdetails memory _details = FactoryCloneContract(factoryAddress)
            .getERC20DAOdetails(address(this));
        require(_details.isTransferable, "Token Non Transferable");
...
contract Nft is ERC721Upgradeable, AccessControl, Ownable, Helper {
    ...
    /// @param _daoAdmins admins of DAO
    ...
    /// @param _numOfNftsToMint amount of nfts to mint
    function buyGovernanceToken(
        address _usdcTokenAddress,
        uint256 _totalAmount,
        string memory _tokenURI,
        ...
        ERC721DAOdetails memory _details = FactoryCloneContract(factoryAddress)
            .getERC721DAOdetails(address(this));
        if (_details.depositCloseTime < block.timestamp) revert DepositClosed();
        ...
        if (!(_numOfNftsToMint <= 0))
            revert AmountInvalid("_numOfNftsToMint", _numOfNftsToMint);
        if (_details.priceOfNft * (_numOfNftsToMint) != _totalAmount)
            revert AmountInvalid("_totalAmount", _totalAmount);
        ...
        uint256 daoBalance = IERC20(_usdcTokenAddress).balanceOf(address(this));
        ...
        for (uint256 i = 0; i < _numOfNftsToMint; i++) {
            mintNft(msg.sender, _tokenURI, _merkleProof);
        }
    ...
    function ifTokenGatingApplied() internal view returns (bool) {
        ...
        address[] memory _tokenList = FactoryCloneContract(factoryAddress)
            .getGatingTokensOfDaoToken(address(this));
        uint256 tokenBalance1 = IERC721(_tokenList[0]).balanceOf(msg.sender);
        ...
        for (uint8 i = 1; i < leng; ) {
            address tokenAddress2 = _tokenList[i + 1];
            uint256 tokenBalance2 = IERC721(tokenAddress2).balanceOf(
                msg.sender
            );
            bool check2 = tokenBalance2 >=
                FactoryCloneContract(factoryAddress)
    ...
    function mintNft(
        ...
        if (!_details.isMintingOpen) revert MintingNotOpen();

        if (balanceOf(_to) == _details.maxTokensPerUser)
            revert MaxTokensMintedForUser(_to);

        if (!_details.isNftTotalSupplyUnlimited) {
            if (_tokenIdTracker == _details.totalSupplyOfToken)
                revert MaxTokensMinted();
        }

        if (_details.onlyAllowWhitelist) {
            require(
                MerkleProof.verify(
                    _merkleProof,
                    _details.merkleRoot,
    ...
    function updateProposalAndExecution(
        bytes memory _data
    ) external onlyGnosis(factoryAddress) {
        (bool success, ) = address(this).call(_data);
        require(success);
    }
    ...
    function airDropToken(
        ...
        uint256 _holdings = IERC20(_airdropTokenAddress).balanceOf(
            address(this)
        );
        ...
        if (_ownersAirdropFees == 0) {
            ...
            uint256 _deployerShare = ((_airdropAmount) * (_ownersAirdropFees)) /
                (FLOAT_HANDLER_TEN_4);
            IERC20(_airdropTokenAddress).transfer(
                _details.deployerAddress,
                _deployerShare
            );
            _updatedAmount = _airdropAmount - _deployerShare;
        ...
        for (uint256 i = 0; i < leng; ) {
            ...
            IERC20(_airdropTokenAddress).transfer(_members[i], _dropAmount);
        ...
        Emitter(emitterAddress).airDropToken(
            address(this),
            _airdropTokenAddress,
            _airdropAmount,
            _ownersAirdropFees
    ...
    function addDaoAdmin(
        address[] memory _daoAdminAddresses
    ) external onlyCurrentContract {
        if (_daoAdminAddresses.length == 0)
            revert AmountInvalid(
                "_daoAdminAddresses.length",
        ...
        Emitter(emitterAddress).daoAdminsAdded(
            address(this),
            _daoAdminAddresses,
            FactoryCloneContract(factoryAddress).USDCAddress()
        );
    ...
    function sendCustomToken(
        ...
        if (
            !(_minimumRequired <=
                IERC20(_customTokenAddress).balanceOf(address(this)))
        ) revert InsufficientFunds();

        for (uint256 i; i < leng; ) {
            IERC20(_customTokenAddress).transfer(
                _addresses[i],
                _amountArray[i]
            );
            unchecked {
        ...
        Emitter(emitterAddress).sendCustomToken(
            address(this),
            _customTokenAddress,
            _amountArray,
            _addresses
