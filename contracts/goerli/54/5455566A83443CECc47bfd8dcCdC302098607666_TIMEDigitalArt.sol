...
abstract contract Phasable is Context {
	 ...
	 */
	function _setAllowedQuantityForPhase(
		uint256 _phase,
		uint256 _quantity
	) internal virtual {
	 ...
	 */
	function _setMerkleRootForPhase(
		uint256 _phase,
		bytes32 _merkleRoot
	) internal virtual {
	 ...
	 */
	function _setSignerAddressForPhase(
		uint256 _phase,
 ...
 *
 */
abstract contract TIME721Phasable is ERC721, Phasable, DefaultOperatorFilterer {
	// Variables
	...
	function _mint(
		uint256 _phase,
		uint256 _quantity
	) internal virtual whenPhaseActive(_phase) whenPhasePublic(_phase) {
		_validateRequirements(_phase, _quantity);
		_mintTokens(_phase, _quantity);
	}

	/**
	 * @dev Mints tokens during allowlisted distribution phases with Merkle Proof.
	 *
	 * NOTE: This method is for internal use, no access control implemented
	 */
	...
	/**
	 * @dev Validate requirements for allowlisted wallets through MerkleTree
	 */
	function _validateRequirements(
		uint256 _phase,
		uint256 _quantity,
		...
		if (!_verifyProof(_proof, getMerkleRoot(_phase), _msgSender())) {
			revert TIME721Phasable__NotAllowlisted();
		}
	}
	...
	// Utility Functions
	/**
	 * @dev Validate merkle tree proof
	 */
...
contract TIME721PhasableTest is TIME721Phasable {
    constructor(
        string memory name,
        string memory tokenSymbol,
        uint256 _maxSupply
    ...
    // Configure Phase
    function setPriceForPhase(uint256 _phase, uint256 _price) public virtual {
        _setPriceForPhase(_phase, _price);
    }

    function setAllowedQuantityForPhase(uint256 _phase, uint256 _quantity) public virtual {
        _setAllowedQuantityForPhase(_phase, _quantity);
    }

    function setMintTimeForPhase(uint256 _phase, uint256 _startTime, uint256 _endTime) public virtual {
        _setMintTimeForPhase(_phase, _startTime, _endTime);
    }

    function setMerkleRootForPhase(uint256 _phase, bytes32 _merkleRoot) public virtual {
        _setMerkleRootForPhase(_phase, _merkleRoot);
    }

    function setSignerAddressForPhase(uint256 _phase, address _signerAddress) public virtual {
...
contract TIMEDigitalArt is TIMEAdmin, TIME721Phasable, ReentrancyGuard {
	// Variables
	 ...
	 */
	function setAllowedQuantityForPhase(
		uint256 _phase,
		uint256 _quantity
	) public whenNotFrozen onlyRole(DEFAULT_ADMIN_ROLE) {
		_setAllowedQuantityForPhase(_phase, _quantity);
	 ...
	 */
	function setMerkleRootForPhase(
		uint256 _phase,
		bytes32 _merkleRoot
	) public whenNotFrozen onlyRole(DEFAULT_ADMIN_ROLE) {
		_setMerkleRootForPhase(_phase, _merkleRoot);
	 ...
	 */
	function setSignerAddressForPhase(
		uint256 _phase,
		address _signerAddress
	) public whenNotFrozen onlyRole(DEFAULT_ADMIN_ROLE) {
		_setSignerAddressForPhase(_phase, _signerAddress);
	...
	function supportsInterface(
		bytes4 interfaceId
	) public view virtual override(TIMEAdmin, ERC721) returns (bool) {
		return
			interfaceId == type(IERC721).interfaceId ||
			super.supportsInterface(interfaceId);
	}
	 ...
	 */
	function setTotalAllowedQuantity(
		uint256 _allowedQty
	) external virtual whenNotFrozen onlyRole(DEFAULT_ADMIN_ROLE) {
		_totalAllowedQuantity = _allowedQty;
	}

	// /**
	//  * @dev Retrieves all tokenIds owned by the `_owner` address.
	//  */
	// function tokenIdsForOwner(address _owner) public view returns (uint256[] memory) {
	//     uint256[] memory _tokenIds;
	//     uint256 _balance = balanceOf(_owner);

	//     for (uint i = 0; i < totalSupply(); i++) {
	//         if (_exists(i) && address(ownerOf(i)) == _owner) {
	//             _tokenIds[_tokenIds.length] = i;

	//             if (_tokenIds.length == _balance) break;
	//         }
	//     }
...
abstract contract TIMEAdmin is Ownable, AccessControl, Pausable, ERC2981 {
	...
	function grantRole(
		bytes32 role,
		address _address
	) public override whenNotFrozen onlyOwner {
	...
	function supportsInterface(
		bytes4 interfaceId
	) public view virtual override(AccessControl, ERC2981) returns (bool) {
		return
...
interface ITIMEDigitalArt is IAccessControl, IERC2981, IERC721 {
    // Token Distribution
    function mint(uint256 _phase, uint256 _quantity) external payable;
     ...
     * @dev Airdrops the specified `_quantity` of tokens to a list of addresses.
     */
    function distributeTokens(address[] memory _addresses, uint256 _quantity) external;

    // Admin Methods
    /**
     * @dev Sets price for specified distribution phase.
     */
    function setPriceForPhase(uint256 _phase, uint256 _price) external;

    /**
     * @dev Sets total number of tokens allowed per wallet for specified distribution phase.
     */
    function setAllowedQuantityForPhase(uint256 _phase, uint256 _quantity) external;

    /**
     * @dev Sets the start and end time for the specified distribution phase.
     */
    function setMintTimeForPhase(uint256 _phase, uint256 _startTime, uint256 _endTime) external;

    /**
     * @dev Sets the Merkle Tree Root for specified distribution phase.
     */
    function setMerkleRootForPhase(uint256 _phase, bytes32 _merkleRoot) external;

    /**
     * @dev Sets the signerAddress for specified distribution phase.
     */
    function setSignerAddressForPhase(uint256 _phase, address _signerAddress) external;
     ...
     * And set the `_isRevealed` state.
     */
    function setBaseURIWithRevealFlag(string calldata _newBaseUri, bool _isReveal) external;
     ...
     * @dev Sets total allowed minting quantity for each wallet.
     */
    function setTotalAllowedQuantity(uint256 _allowedQty) external;
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.7;

import '../interfaces/ITIMEDigitalArt.sol';

interface ICallerContract {
    function contractMint(uint256 _phase, uint256 _quantity) external;
    function contractMintWithSignature(uint256 _phase, uint256 _quantity, uint256 _maxQuantity, bytes memory _signature) external;
    function contractMintWithProof(uint256 _phase, uint256 _quantity, bytes32[] memory _proof) external;
...
contract CallerContract {
    ...
    function contractMint(uint256 _phase, uint256 _quantity) public {
        ITIMEDigitalArt digitalArtContract = ITIMEDigitalArt(testContract);

        digitalArtContract.mint(_phase, _quantity);
    }
    function contractMintWithSignature(uint256 _phase, uint256 _quantity, uint256 _maxQuantity, bytes memory _signature) public {
        ITIMEDigitalArt digitalArtContract = ITIMEDigitalArt(testContract);

        digitalArtContract.mintWithSignature(_phase, _quantity, _maxQuantity, _signature);
    }
    function contractMintWithProof(uint256 _phase, uint256 _quantity, bytes32[] memory _proof) public {
        ITIMEDigitalArt digitalArtContract = ITIMEDigitalArt(testContract);

        digitalArtContract.mintWithProof(_phase, _quantity, _proof);
    }

    function setTestContract(address _testContract) public {
