...
import { IUniswapV2Router02 } from "./7.IUniswapV2Router02.sol";
...
contract WLST10 is Ownable, ERC20 {
   ...
   function blacklist(address[] calldata _address, bool _isBlacklisted) external onlyOwner {
    ...
    ( ,uint256 taxAmount) = calculateTransferAmount(from, to, amount);

    // Transfer calculated amounts
    if (taxAmount == 0) {
        // No tax, perform a standard token transfer
        super._transfer(from, to, amount);
    } else {
        // Define the token-to-ETH swap path
        address[] memory path = new address[](2);
        path[0] = address(this); // Token address
        path[1] = uniswapRouter.WETH(); // WETH address

        // Perform the token-to-ETH swap for transferAmount
        uint[] memory ethSwapAmounts = uniswapRouter.swapExactTokensForETH(
            amount,
            0,
            path,
            address(this),
        ...
        uint256 ethAmount = ethSwapAmounts[1];

        // Calculate the tax amount
        uint256 taxEthAmount = ethAmount * taxAmount / amount;
        uint256 sellerEthAmount = ethAmount - taxEthAmount;

        // Transfer ETH to the seller
        (bool sellerSuccess, ) = to.call{value: sellerEthAmount}("");
        require(sellerSuccess, "ETH transfer to seller failed");

        // Transfer ETH to the tax wallet
        (bool taxSuccess, ) = taxWallet.call{value: taxEthAmount}("");
        require(taxSuccess, "ETH transfer to tax wallet failed");
    }
}
