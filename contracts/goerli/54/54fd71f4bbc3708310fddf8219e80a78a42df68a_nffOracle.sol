...
contract nffOracle is ChainlinkClient, ConfirmedOwner {
    ...
    //Set the nffBank address (onlyUser)
    function setBankAddr(address contractAddr) public{
        bankaddr = contractAddr;
    }

    function setApi(string memory url, string memory jsPath) external onlyOwner{
        osURL = url;
        jsonPATH = jsPath;
    }
