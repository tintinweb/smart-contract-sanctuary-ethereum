...
contract Incentives is Boosting, Coordination, IIncentives {
        ...
        returns (uint)
    {
        ...
        return boost[token].taker * amount / (IState(state).B() + amount) * uint(_price) / DIVIDER_ORACLE;
    }
        ...
        returns (uint)
    {
        ...
        return calculateCoordination() * amount / uint(DIVIDER) * uint(_price) / DIVIDER_ORACLE;        
    }
    ...
    function coordinationForMaker(address token, uint amount)
        ...
        returns (uint)
    {
        return calculateCoordination() * amount / uint(DIVIDER);
    }
}
...
abstract contract Coordination is IncentivesBase {
    int internal constant a = (DIVIDER * 3) / 100;
    int internal constant b = (DIVIDER * 80) / 100;
    int internal constant c = (DIVIDER * 90) / 100;
    int internal constant d = DIVIDER * 1;
    int internal constant f = (DIVIDER * 26) / 100;
    int internal constant g = DIVIDER * 1;
    int internal constant j = (DIVIDER * 30) / 100;
    int internal constant h = (DIVIDER * 74) / 100;
    ...
    function setIncentiveConfig(uint _scale, uint _inflection, uint _tvlMin, uint _tvlMax)
        external
        onlyGovernance
    {
        thetaScale = _scale;
    ...
    function calculateCoordination() public view returns (uint incentive) {
        ...
        if (tvlStable < tvlMin || tvlStable > tvlMax) 
            incentive = 0;
        else 
    ...
    function _incentive(uint tvlStable) internal view returns (uint value) {
        int x = (DIVIDER * int(tvlStable)) / int(thetaInflection);
        if (tvlStable < thetaInflection)
            value = uint((a * x) / ((b * x) / DIVIDER - c) + d);
        else 
        {
            int p1 = (f * x) / ((g * x) / DIVIDER) - h;
            value = p1 > j ? uint(p1 - j) : 0;
        }
...
abstract contract Boosting is IncentivesBase {
    ...
    uint public tLambdaBoostScale;
    uint public mLambdaBoostScale;
    struct BoostingStruct {
        uint taker;
        uint maker;
    }

    mapping(address => BoostingStruct) public boost;

    function setBoostConfig(uint _boostMax, uint _thetaBoost, uint _tLambdaScale, uint _mLambdaScale)
        external
        onlyGovernance
    {
        boostMax = _boostMax;
        thetaBoost = _thetaBoost;
        tLambdaBoostScale = _tLambdaScale;
        mLambdaBoostScale = _mLambdaScale;
    }
        ...
        returns (uint, uint)
    {
        int boostMaker = (int(thetaBoost) *
            SignedMath.min(
                int(boostMax),
                int(mLambdaBoostScale) * _calcBoost(b.w11-b.lrr11,b.pbl11) / DIVIDER
            )) / DIVIDER;
            
        int boostTaker = (int(thetaBoost) *
            SignedMath.min(
                int(boostMax), 
                int(tLambdaBoostScale) * _calcBoost(b.w1 - b.lrr1, b.pbl1 ) / DIVIDER
            )) / DIVIDER;

        return (uint(boostTaker), uint(boostMaker));
    }

    function _calcBoost(int d, int pbl) internal pure returns (int) {
        return DIVIDER * d / pbl * d / pbl * d / pbl * d / pbl; 
    }
}
