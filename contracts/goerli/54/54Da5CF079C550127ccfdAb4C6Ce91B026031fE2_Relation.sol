...
contract Relation is ReentrancyGuard, Ownable {
    ...
    function bind(address _account, address _referrer) external {
        if (shareData[_account].active) return;
    ...
    function getUser(address _user) external view returns (bool, address) {
        return (shareData[_user].active, shareData[_user].superior);
    }
