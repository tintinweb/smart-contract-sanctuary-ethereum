...
error CannotMintMoreThanMaxLimit();
...
contract KenomiNFT is Ownable, ERC721 {
    ...
    modifier callerIsUser() {
        if (tx.origin != _msgSender()) revert CallNotAllowed();
        _;
    }

    modifier activeWhiteListSale() {
        if (mintFreeze) revert MintFreeze();
        if (publicSaleIsActive) revert WhiteListSaleNotActive();
        _;
    }
    ...
    function validMint(uint256 quantity, uint256 maxMintQuantity, uint256 price, uint256 value) internal view returns(bool) {
        ...
        if (quantity > maxMintQuantity) revert CannotMintMoreThanMaxLimit();
        if (value != quantity * price) revert NotEnoughBalance();
        if (tokenId + quantity > maxSupply) revert NotEnoughSupply();
        return true;
    }
    ...
    function whiteListMint(uint256 quantity, bytes32[] calldata merkleProof) external payable callerIsUser activeWhiteListSale {
        if(whiteListMinted[_msgSender()]) { revert AddressAlreadyMinted(); }
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        if(!MerkleProof.verify(merkleProof, merkleRoot, leaf)) { revert InvalidMerkleProof(); }
        if (!validMint(quantity, maxWhiteListMint, whiteListPrice, msg.value)) {
            revert NotValidMint();
        }
    ...
    //   <-   PUBLIC MINT => PRICE = 0.008, MAX MINT = 2   ->   //
    ...
    modifier activePublicSale() {
        if (mintFreeze) revert MintFreeze();
        if (!publicSaleIsActive) revert PublicSaleNotActive();
        _;
    }

    function publicMint(uint256 quantity) external payable callerIsUser activePublicSale {
        if(publicMinted[_msgSender()]) { revert AddressAlreadyMinted(); }
        if (!validMint(quantity, maxPublicMint, publicPrice, msg.value)) {
            revert NotValidMint();
        }
