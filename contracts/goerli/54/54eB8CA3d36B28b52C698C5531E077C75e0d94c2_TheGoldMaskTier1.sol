...
contract TheGoldMaskTier1 is ERC1155, Ownable {
    ...
    uint8 public mintPerRound = 10; // how many NFSs are minted per round
    
    
    
    constructor() ERC1155("") {}
    
    function mintBatch(uint256[] memory ids, uint256[] memory values) external onlyOwner {
        require(ids.length == mintPerRound, "Exactly mintPerRound tier 1 NFTs must be minted in one round!");
        require(values.length == mintPerRound, "Exactly mintPerRound tier 1 NFTs must be minted in one round!");
                
        for (uint256 i = 0; i < ids.length; ++i) {
            require(ids[i]<=tokenCount, "ID must be lower than or equal to tokenCount!");
            require(values[i] == 1, "It is not permitted to mint more than one NFT for the same ID!");
