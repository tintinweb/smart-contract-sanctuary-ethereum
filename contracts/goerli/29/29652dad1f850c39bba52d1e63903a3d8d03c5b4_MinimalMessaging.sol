...
import {Message} from '../../lib/monorepo/packages/deployments/contracts/contracts/messaging/libraries/Message.sol';
import {TypedMemView} from
    '../../lib/monorepo/packages/deployments/contracts/contracts/shared/libraries/TypedMemView.sol';
...
contract MinimalMessaging {
    using TypedMemView for bytes;
    using TypedMemView for bytes29;
    using Message for bytes29;

    address public immutable STARKNET;
    uint256 public immutable L2_SPOKE_CONNECTOR;
    uint256 public immutable L2_FN_SELECTOR;

    event MessageSent(uint256[]);

    constructor(address _STARKNET, uint256 _L2_SPOKE_CONNECTOR, uint256 _L2_FN_SELECTOR) {
        STARKNET = _STARKNET;
        L2_SPOKE_CONNECTOR = _L2_SPOKE_CONNECTOR;
        L2_FN_SELECTOR = _L2_FN_SELECTOR;
    }

    function sendToStarknet(bytes memory _message) external payable {
        uint256[] memory _payload = computePayload(_message);
        IStarknetMessaging(STARKNET).sendMessageToL2{value: msg.value}(L2_SPOKE_CONNECTOR, L2_FN_SELECTOR, _payload);
        emit MessageSent(_payload);
    }

    function computePayload(bytes memory _message) internal returns (uint256[] memory _payload) {
        bytes29 _m = _message.ref(0);
        _payload = new uint256[](4);
        _payload[0] = uint256(uint160(_m.recipientAddress()));
        _payload[1] = uint256(_m.origin());
        _payload[2] = uint256(_m.nonce());
        _payload[3] = uint256(_m.sender());
        _payload[4] = uint256(uint232(_m.body()));
    }
}