...
contract DaoHallRegistrar is Ownable{
  ...
  function buySubnodeWhitelisted(string calldata label, bytes32 parentHash, address parentOwner, uint256 data) external payable {
    uint32 duration = uint32(data);
    uint64 ttl = uint64(data >> 32);
    uint32 fuses = uint32(data >> 96);
    uint64 expiry = uint64(data >> 128);
    ...
    //   controller.setSubnodeRecord(parentNode, label, address(this), resolver, ttl, fuses, expiry);
    //   Resolver(resolver).setAddr(node, msg.sender);
    // }
    controller.setSubnodeRecord(parentNode, label, msg.sender, address(0), ttl, fuses, expiry);    
  }
  ...
  function _checkBuyWhitelisted(string memory label, bytes32 parentHash, uint32 duration, uint32 limit) internal {
    NodeRecord memory record = daoHallRegistry.getRecord(_l2Node(parentHash), 0x178BC6e45Cab61E886cBd665F77Dbd9615b0D70C);
    // NodeRecord memory record = getRecord(parentHash);
