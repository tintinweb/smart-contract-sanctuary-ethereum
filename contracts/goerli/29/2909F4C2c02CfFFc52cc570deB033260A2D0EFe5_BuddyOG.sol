...
contract BuddyBase is ERC721SeaDrop {
  /**
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return
      bytes(baseURI).length != 0
        ? string(abi.encodePacked(baseURI, _toString(tokenId), ".json?alt=media"))
        : "";
  }
}
...
contract BuddyOG is BuddyBase {
  constructor() BuddyBase("Buddy OG", "BOG", 5000) {}
}
...
import {ReentrancyGuard} from "../utility-contracts/ReentrancyGuard.sol";

import {IERC165} from "@openzeppelin/contracts/utils/introspection/IERC165.sol";

import {DefaultOperatorFilterer} from "../utility-contracts/DefaultOperatorFilterer.sol";
  ...
  DefaultOperatorFilterer
{
   ...
   */
  constructor(string memory name, string memory symbol) ERC721ContractMetadata(name, symbol) {}
  ...
  function supportsInterface(
    bytes4 interfaceId
  ) public view virtual override(IERC165, ERC721ContractMetadata) returns (bool) {
    return
