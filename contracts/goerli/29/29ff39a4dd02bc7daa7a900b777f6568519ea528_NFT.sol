...
// File: contracts/nftercsample.sol
...
// NftRewards private _rewards;
 ...
 mapping(uint256 => TokenInfo) public AllowedCrypto;
    uint256 private addedTokens =0;

     //Add ERC20 Payment Tokens
 function addCurrency(string memory _name,address _add,uint256 _price,bool _enabled) public onlyOwner {
     
    AllowedCrypto[addedTokens]=TokenInfo(_name,_add,_price,_enabled);
    addedTokens++;
 }
    // Disable Payment Tokens

    function disableToken(uint256 _id, bool _toggle)public onlyOwner{
 ...
 function PayMint(uint256 _quantity,uint _id) public {
     require(saleEnable, "Sale is not Enabled");
     require(
     ...
     // ERC20 Payment
     require(AllowedCrypto[_id].enabled,"This Erc20 Token is disabled");
        address payaddress = AllowedCrypto[_id].addr;
        uint256 _price = AllowedCrypto[_id].price;
        
        IERC20 tokenAddress = IERC20(payaddress);
...
//MINT 
 function mint(uint256 quantity) external payable {
    ...
    constructor() ERC721A("ercnft", "ercnft") {}
    ...
    function mintbyref(
        address _toAddress,
        uint256 _quantity,
        address _refAddress // bytes memory _signature
    ) external payable {
        require(saleEnable, "Sale is not Enabled");
        // _safeMint's second argument now takes in a quantity, not a tokenId.
        require(
        ...
        require(
            _refAddress != _toAddress && _toAddress == msg.sender,
            "Don't cheat"
        );

        // require(
        //     getSigner(_toAddress, _quantity, _refAddress, _signature) ==
        //         signerAddress,
        //     "Don't cheat"
        // );
        ...
        payable(_refAddress).transfer((msg.value * 20) / 100);
        emit MintedNft(_quantity, _msgSender());
    }
