...
contract NFortress is OwnableUpgradeable {
    ...
    function initialize() public initializer {
        ...
        discountBulk = 5 + 10*100 + 15*10000; // 3y% + 5y%*100 + 10y%*10000
    }
    ...
    function setPromotion(address _nftAddress, uint256 _feeDiscount) external onlyOwner {
        discountData.push(DiscountData(_nftAddress, _feeDiscount*100)); // percentage*100 to reduce fee
    }

    // format 3y% + 5y%*100 + 10y%*10000
    ...
    function getFee(address addr, uint256 year) public view returns (uint256) {
        ...
        if(year > 2 && year < 5 ) _fee = _fee - (_fee*(discb % 100)*100)/10000;
        if(year > 4 && year < 10) _fee = _fee - (_fee*((discb / 100) % 100)*100)/10000;
        if(year > 9) _fee = _fee - (_fee*(discb / 10000)*100)/10000;
        return _fee - (_fee*discount)/10000;
    }
