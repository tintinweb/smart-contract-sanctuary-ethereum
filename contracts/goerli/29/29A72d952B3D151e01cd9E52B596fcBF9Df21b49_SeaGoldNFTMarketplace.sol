...
contract SeaGoldNFTMarketplace is Ownable {
    ...
    bool public StakingRewardsEnabled;
    ...
    constructor(
        ...
        StakingRewardsEnabled = _stakingRewardsEnabled;
    ...
    function updateStakingRewardsEnabled(bool _stakingRewardsEnabled) public onlyOwner {
        StakingRewardsEnabled = _stakingRewardsEnabled;
    }
    ...
    function acceptBId(
        ...
        if (SeaGoldLaunched) {
        (sellerRewards, buyerRewards, stakingRewards) = rewardCalc.getRewardsAL(salestruct.amount);
        }
        else {
            (sellerRewards, buyerRewards, stakingRewards) = rewardCalc.getRewardsBL(salestruct.amount);
        }
        uint256 seaGoldBalance = seagoldtoken.balanceOf(address(this));
        if (seaGoldBalance > (sellerRewards + buyerRewards + stakingRewards)) {
            seagoldtoken.transfer(salestruct.seller, sellerRewards);
        seagoldtoken.transfer(salestruct.user, buyerRewards);
        if (StakingRewardsEnabled) {
            seagoldtoken.transfer(StakingAddress, stakingRewards);
        }
        }
