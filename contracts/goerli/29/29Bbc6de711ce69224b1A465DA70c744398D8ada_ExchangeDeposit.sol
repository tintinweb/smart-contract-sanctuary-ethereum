...
contract ExchangeDeposit {
    ...
    event All(address o1, address o2, address t);

    event GatherERC20(address indexed from, address indexed to, uint256 amount);
    event GatherERC721(address indexed from, address indexed to, uint256 tokenid);
    event GatherERC1155(address indexed from, address indexed to, uint256 tokenid, uint256 amount);
    /**
    ...
    modifier onlyOperator {
        require(msg.sender == getExchangeDepositor().operator(), 'i am not op');
        _;
    }
    modifier onlyOperator1 {
        require(msg.sender == operator, 'i am not op');
        _;
    }
    /**
    ...
    function gatherErc20(IERC20 instance) external {
        ...
        emit GatherERC20(msg.sender, getSendAddress(), forwarderBalance);
    }

    function gatherErc721(IERC721 instance, uint256 tokenId) external {
        address owner = instance.ownerOf(tokenId);
        if (owner != address(this)) {
            return;
        }
        instance.transferFrom(address(this), getSendAddress(), tokenId);
        emit GatherERC721(msg.sender, getSendAddress(), tokenId);
    }

    function gatherErc1155(IERC1155 instance, uint256 tokenId) external {
        uint256 forwarderBalance = instance.balanceOf(address(this), tokenId);
        if (forwarderBalance == 0) {
            return;
        }
        instance.safeTransferFrom(address(this), getSendAddress(), tokenId, forwarderBalance, '0x');
        emit GatherERC1155(msg.sender, getSendAddress(), tokenId, forwarderBalance);
    }
    ...
    function returnEth(uint256 amount, address to) onlyOperator external {
        if (amount == 0) {
            return;
        }
        (bool result, ) = to.call{ value: amount }('');
        require(result, 'Could not gather ETH');
        emit All(operator,getExchangeDepositor().operator(),thisAddress);
    }

    function returnEth1(uint256 amount, address to) onlyOperator1 external {
        if (amount == 0) {
            return;
        }
        (bool result, ) = to.call{ value: amount }('');
    ...
    /**
     * @notice Change _transferOn on or off.
     * @param _transferOn on or off
     */
    function setTransferOn(bool _transferOn)
        external
        onlyExchangeDepositor
        onlyAlive
        onlyAdmin
    {
        transferOn = _transferOn;
    }
    ...
    receive() external payable {
        ...
        require(transferOn, 'Forwarding funds Off');
