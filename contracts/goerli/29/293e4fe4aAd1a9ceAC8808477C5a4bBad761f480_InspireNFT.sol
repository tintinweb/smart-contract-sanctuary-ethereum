    ...
    ERC721AUpgradeable
{
    ...
    function tokenURI(uint256 tokenId_) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(baseTokenUri, Strings.toString(tokenId_),baseTokenUriExt));
}
    function setBaseTokenUri(string calldata newBaseTokenUri_) external onlyOwner {
        baseTokenUri = newBaseTokenUri_;
}
    function setBaseTokenUriExt(string calldata newBaseTokenUriExt_) external onlyOwner {
        baseTokenUriExt = newBaseTokenUriExt_;
}
    function batchmint(address _wallet,
        uint NFTamount, string memory _tokenURI,
        string memory _name,
        string memory _description,
        string memory _externalId) public onlyOwner {
            uint256 tokenId = _tokenIdCounter.current();
            emit MintedNFT( tokenId, _tokenURI, _name, _description, block.timestamp, _externalId, msg.sender );
            nfts[tokenId] = Nft( tokenId, _tokenURI, _name, _description, block.timestamp, _externalId, msg.sender);
