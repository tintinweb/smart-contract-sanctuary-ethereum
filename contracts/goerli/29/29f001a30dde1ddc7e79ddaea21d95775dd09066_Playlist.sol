...
contract Playlist is ERC721A, ReentrancyGuard, IERC5192, Ownable  {
    ...
    constructor(string memory _name, string memory _symbol, Track[] memory _tracks) 
    ERC721A(_name, _symbol)
    {
        MAX_MINT = _tracks.length;
        batchMint(_tracks.length);
        for (uint i = 0; i < _tracks.length; i++) {
            tokenURIs[i] = _tracks[i].tokenUri;
    ...
    // function updateTokenDetails(uint256 quantity) private {
    //    assembly {
