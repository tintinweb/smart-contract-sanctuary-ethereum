...
contract PolygonPool_R2 is PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IGlobalPool_R1 {
    ...
    function _stake(address staker, uint256 amount) internal nonReentrant {
    ...
    function unstake(uint256 amount, uint256 fee, uint256 useBeforeBlock, bytes memory signature) override external nonReentrant {
        _unstake(msg.sender, amount, fee, useBeforeBlock, signature);
    }
    ...
    function _unstake(address staker, uint256 amount, uint256 fee, uint256 useBeforeBlock, bytes memory signature) internal {
...
interface IinternetBond_R1 {
    ...
    function unlockShares(uint256 bonds) external;
