...
contract LearnETHPartners is ERC721, ReentrancyGuard, Ownable {
    ...
    function initiatePartnerNetwork() external onlyOwner {
        ...
        nodeDetails[tokenId].nodeType = 4; //Special Type for parent node
        nodeDetails[tokenId].level = 1;
        nodeDetails[tokenId].position = 1;
        partnerNodeTokenMapping[1][1] = tokenId;
    }

    function registerAsPartner(uint256 _referrerTokenId, uint256 _level, uint256 _position) public nonReentrant payable {
        require(_referrerTokenId < totalSupply() && nodeDetails[_referrerTokenId].nodeType == 2,"Incorrect Referrer Token Id");
        ...
        nodeDetails[tokenId].father1 = partnerNodeTokenMapping[_level - 1][f_position];
    ...
    function registerInstructor(uint256 _referrerTokenId) public {
        require(_exists(_referrerTokenId) && nodeDetails[_referrerTokenId].nodeType == 2, "Only a partner can register an instructor");
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);
        nodeDetails[tokenId].nodeType = 1;
        nodeDetails[tokenId].personalReferrer = _referrerTokenId;
        nodeDetails[tokenId].father1 = _referrerTokenId;
    ...
    function addCourse(uint256 _tokenId, string memory _courseName, uint256 _courseFee) public {
        ...
        courseDetails[courseIndex].courseType = 1;
        ...
        instructorCourses[_tokenId] = courseIndex;
        courseIndex+=1;
    }

    function enrolForCourse(uint256 _tokenId, uint256 _courseId) public payable {
        require(msg.sender == ownerOf(_tokenId), "Not the owner of tokenId");
        require(_courseId < courseIndex, "Incorrect Course Id");
        require(msg.value == courseDetails[_courseId].courseFee, "Incorrect fee");

        enrolledCourses[_tokenId] = _courseId;
        //distributeFee();

    }

    function distributeFee(uint256 _studentTokenId, uint256 _instructorTokenId, uint256 _fee) internal {
        //
    }
