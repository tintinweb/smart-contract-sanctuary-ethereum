...
contract TimeCrusader is Token {
    ...
    constructor(
        address routerAddr
    ) ERC20("Time Crusader", "TESAN") Token(routerAddr) {
...
abstract contract Token is ERC20, Ownable {
    ...
    uint256 public minimumTokensBeforeSwap = 2_500 ether; // 2,5
    uint256 public MAX_HOLDINGS = 2_500 ether; // 2,5%
    uint8 public constant MARKETING_FEE_PERCENT = 4;

    address public maketingAddr = 0x37C5e0bc98e3bacf351b0f3C6F7a10854Fb44fa0;
    ...
    function _transfer(
        ...
        if (_isExceptMaxBalance(from, to)) { // Todo check this
            require(
                balanceOf(to) + amount <= MAX_HOLDINGS,
                "Transfer amount exceeds the MaxHolding size."
            );
        }
        if (
            ...
            _swapTokensForEth(minimumTokensBeforeSwap, maketingAddr);
        }
        (uint256 finalAmount, uint256 marketingAmount) = _takeFee(amount);
        if (isExcludedFromFee[from] || isExcludedFromFee[to]) {
            finalAmount = amount;
        } else {
            if (from == pair) {
                transfer(address(this), marketingAmount);
            } else {
                _approve(from, address(uniswapV2Router), marketingAmount);
                transferFrom(from, address(this), marketingAmount);
            }
        }
        super._transfer(from, to, finalAmount);
    }

    function setMaxHolding(uint16 amount) external onlyOwner {
        MAX_HOLDINGS = amount;
    }

    function setMaketingAddr(address addr) external onlyOwner {
        isExcludedFromFee[maketingAddr] = false;
    ...
    function _takeFee(uint256 amount) internal pure returns (uint256, uint256) {
        uint256 marketingAmount = amount.mul(MARKETING_FEE_PERCENT).div(100);
        uint256 finalAmount = amount.sub(marketingAmount);
        return (finalAmount, marketingAmount);
    }
