...
contract Futbots is Ownable, IERC2981, ERC721 {
    ...
    mapping (address => uint8) private minted;
    ...
    function mint(uint amount) external payable {
        ...
        require(totalSupply + amount < maxTotalSupply, "Request exceeds max supply!");
        require(amount + minted[msg.sender] <= maxAmount, "Request exceeds max per wallet!");

        unchecked { minted[msg.sender] += uint8(amount); }
    ...
    function setMaxMintAmount(uint8 amount) external onlyOwner {
        maxAmount = amount;
    }

    /**
     * @notice toggles the public sale
     * @dev enables/disables public sale functions 
     */
