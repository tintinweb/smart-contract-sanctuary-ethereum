    ...
    SignerManager
{
    ...
    struct MintConfig {
        uint32 mintAllowlistStartTime;
        uint32 mintPublicStartTime;
    }
    ...
    function mintAllowlist(
        ...
        uint256 _saleStartTime = uint256(mintConfig.mintAllowlistStartTime);
        require(
    ...
    function mintPublic(uint256 requested) external payable {
        uint256 _saleStartTime = uint256(mintConfig.mintPublicStartTime);
        require(
            _saleStartTime != 0 && block.timestamp >= _saleStartTime,
            "sale has not started yet"
        );
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 idx;
        for (uint256 i = 0; i < ERC721A.totalSupply(); i++) {
    ...
    */
    ...
    /**
    @notice Returns the length of time, in seconds, that the Uncode Genesis has
    been staked.
    @dev Nesting is tied to a specific Uncode Genesis, not to the owner, so it doesn't
    reset upon sale.
    @return staking Whether the Uncode Genesis is currently staking. MAY be true with
    ...
    @return total Total period of time for which the Uncode Genesis has been staked across
    ...
    */
    function _beforeTokenTransfers(
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            require(stakingStarted[tokenId] == 0, "Uncode Genesis: staking");
        }
    }

    /**
    @dev Emitted when a Uncode Genesis begins staking.
    */
    event Staked(uint256 indexed tokenId);

    /**
    @dev Emitted when a Uncode Genesis stops staking; either through standard means or
    by expulsion.
    */
    event Unstaked(uint256 indexed tokenId);

    /**
    @dev Emitted when a Uncode Genesis is expelled from the staking.
    */
    ...
    */
    ...
    /**
    @notice Changes the Avatars's staking status.
    */
        ...
        onlyApprovedOrOwner(tokenId)
    {
        uint256 start = stakingStarted[tokenId];
        if (start == 0) {
            require(stakingOpen, "Avatars: staking closed");
    ...
    /**
    @notice Changes the Uncode Genesis' staking statuss (what's the plural of status?
    statii? statuses? status? The plural of sheep is sheep; maybe it's also the
    plural of status).
    @dev Changes the Uncode Genesis' staking sheep (see @notice).
    */
    ...
    /**
    @notice Admin-only ability to unstake an Uncode Genesis Avatar.
    ...
    */
    function unstake(uint256 tokenId) external onlyOwner {
        require(stakingStarted[tokenId] != 0, "Uncode Genesis: not staked");
