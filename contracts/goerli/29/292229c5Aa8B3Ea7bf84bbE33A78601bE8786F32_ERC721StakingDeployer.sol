...
abstract contract ERC721StakingVault {
    function setRewardContract(address _address) external virtual;
    function transferOwnership(address newOwner) external virtual;
}
...
contract ERC721StakingDeployer is Ownable, ReentrancyGuard, PaymentSplitter {
    ...
    function deploy(
        ...
        if (_price > 0) {
            ...
            usdc.safeTransferFrom(msg.sender, address(this), _price);
        }

        // deploy contracts
        address _erc20Contract = ERC20Factory(erc20Deployer).deploy(_tokenParams.name, _tokenParams.symbol, _tokenParams.maxSupply, msg.sender);
        address _vaultContract = ERC721VaultFactory(vaultDeployer).deploy(_vaultParams.softStaking, _vaultParams.hardStaking);
        address _rewardContract = ERC721RewardsFactory(rewardDeployer).deploy(_erc20Contract, _vaultContract);
        ...
        ERC721StakingVault(_vaultContract).setRewardContract(_rewardContract);
        ...
        // transfer ownership
        ERC721StakingVault(_vaultContract).transferOwnership(msg.sender);
        ERC721StakingRewards(_rewardContract).transferOwnership(msg.sender);
