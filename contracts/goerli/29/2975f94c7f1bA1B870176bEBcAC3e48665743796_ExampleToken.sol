...
contract ExampleToken is ERC20, Ownable {
    ...
    constructor() ERC20("Example Token", "EXMP") {
        ...
        buyDevFee = 2;
        buyBurnFee = 0;
        buyTotalFees = buyProtocolFee + buyLiquidityFee + buyDevFee + buyBurnFee;

        sellProtocolFee = 3;
        sellLiquidityFee = 0;
        sellDevFee = 2;
        sellBurnFee = 0;
        sellTotalFees = sellProtocolFee + sellLiquidityFee + sellDevFee + sellBurnFee;

        protocolAddress = payable(0x9B86f9035B21aeFBd3344903ee276D6Dd3513E0D);
        devAddress = payable(0xCd4f3C9E000cdB3baC72719CE67d46C488D924EF);

        _excludeFromMaxTransaction(contractOwner, true);
        _excludeFromMaxTransaction(address(this), true);
        _excludeFromMaxTransaction(address(0xdead), true);
        _excludeFromMaxTransaction(protocolAddress, true);
        _excludeFromMaxTransaction(devAddress, true);

        excludeFromFees(contractOwner, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(0xdead), true);
        excludeFromFees(protocolAddress, true);
    ...
    function updateBuyFees(uint256 _protocolFee, uint256 _liquidityFee, uint256 _devFee, uint256 _burnFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _protocolFee, uint256 _liquidityFee, uint256 _devFee, uint256 _burnFee) external onlyOwner {
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForProtocol + tokensForDev;
        ...
        uint256 ethForOperations = ethBalance * tokensForProtocol / (totalTokensToSwap - (tokensForLiquidity/2));
    ...
    function updateProtocolAddress(address _protocolAddress) external onlyOwner {
