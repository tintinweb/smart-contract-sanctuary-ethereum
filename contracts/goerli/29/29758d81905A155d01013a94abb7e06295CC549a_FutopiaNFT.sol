...
contract FutopiaNFT is ERC721A, Ownable {
	...
	uint256 private PriceToPayForWl;
	uint256 private PriceToPayForPub;

	uint256 private MintTimeForWL;
	uint256 private MintTimeForPUB;

	uint256 private MaxNftCreationsPerWlUser;
	uint256 private MaxNftCreationsPerPublicUser;
	...
	bytes32 private MerkleThingy;

	constructor() ERC721A("Futopia", "FT") {
		MetaUri = "ipfs://QmaPeFBeebLb9aXsSnvsdqgK4Bc6LAbTtLScpTixotLqkc/";
		EndUri = ".json";
		...
		STOP = true;
		MerkleThingy = 0x00000000000000000000000;
	...
	function makeNftPublicly(uint256 _amount) external payable CheckCheckDoubleCheck {
	...
	function makeNftWl(uint256 _amount, bytes32[] calldata _proof) external payable CheckCheckDoubleCheck {
		require(block.timestamp >= MintTimeForWL, "it's not time yet to shine...");
        require(msg.value == PriceToPayForWl * _amount, "WRONG_ETH_AMOUNT");
