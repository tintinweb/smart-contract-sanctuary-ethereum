...
contract StakingDomiXcAlpha is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    uint256 public maxStakingAmountCap;
    ...
    uint256 public constant maxLockPeriod = 31536000; // 1 year in seconds
    ...
    constructor(
        ...
        bool _doMint,
        uint256 _maxStakingAmountCap
    ) {
        rewardToken = _rewardToken;
        doMint = _doMint;
        maxStakingAmountCap = _maxStakingAmountCap;
    }

    function setMaxStakingCap(uint256 _maxStakingAmountCap) external onlyOwner {
        maxStakingAmountCap = _maxStakingAmountCap;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        //updatingPool
        updatePool(_pid);
        //runs only if the user already exist and have amount staked
        ...
        if (_amount > 0) {
            require(
                block.number < pool.toBlock && !pool.isStopped,
                "Staking period passed or stopped!"
            );
            require(
                user.amount + _amount <= maxStakingAmountCap,
                "max cap hit"
            );
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        for (uint256 index = 0; index < user.stakeInfo.length; index++) {
            ...
            //if stake amount is zero of any deposit instance then skip it.
            if (user.stakeInfo[index].amount == 0) {
            ...
            //if totalselected + stakedInfo amount is greater than requested withdrawl amount
            ...
            if (
                ...
                user.stakeInfo[index].rewardDebt = user
                    ...
                    .rewardDebt
                    .sub(deductionAmount.mul(rewardPerDebtShare));
                //rewardsForTransfers = rewardsForTransfers + deductionAmount * accTokensPerShare(pool) / 1e12
                // - deductionAmount * rewardPerDebtShare
            } else {
                rewardForTransfers = rewardForTransfers.add(
        ...
        //if rewards for transfer greater then zero then do safeStakingTokensTransfer
        ...
        )
    {
        ...
        return (
            user.stakeInfo[index].amount,
            user.stakeInfo[index].rewardDebt,
            user.stakeInfo[index].rewardPerDebtShare,
            user.stakeInfo[index].timeStamp
        );
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (doMint) {
            rewardToken.mint(address(this), stakingReward);
        }
    ...
    function safeStakingTokensTransfer(address _to, uint256 _amount) internal {
        //checking the totalBalance of this contract on stakingToken
        uint256 stakingTokenBalanceOnChef = rewardToken.balanceOf(
            address(this)
        );
        // if amount greater than the stakingBlanceOnCheif transfer whole otherwise
        //transfer the amount.
        if (_amount > stakingTokenBalanceOnChef) {
            rewardToken.transfer(_to, stakingTokenBalanceOnChef);
        } else {
