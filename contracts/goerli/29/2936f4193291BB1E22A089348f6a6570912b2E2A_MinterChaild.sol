...
interface ChildStorage {
    ...
    function operater(address _address) external view returns (bool);
    function childCount(address _address) external view returns (uint256);
    function child(address _address, uint256 _index) external view returns (address);
}

contract MultiWalletCallerV3 is Ownable, IERC721Receiver {
    // address public kudasai = 0xECD0CBBdbFB07986E22981C8D78e17a952605854;
    address public kudasai = 0x254095353B790d2c3061e301490de06631D1Fd9D;
    MinterChaild private immutable _childContract;
    CloneFactory private immutable _cloneContract;
    ChildStorage private immutable _childStorage = ChildStorage(0x52F67E32a29345f1C21617Bd943C6d948E91737C);

    constructor () {
        _childContract = new MinterChaild(address(_childStorage));
        _cloneContract = new CloneFactory();
    }
    receive() external payable {}

    modifier onlyHolder() {
        require(IERC721(kudasai).balanceOf(msg.sender) >= 1, "MultiWalletCaller: No Call");
        _;
    }

    modifier checkId(uint256 _startId, uint256 _endId) {
        require(_startId <= _endId && _startId < ChildStorage(_childStorage).childCount(msg.sender) && _endId < ChildStorage(_childStorage).childCount(msg.sender), "MultiWalletCaller: Invalid ID");
        _;
    }
    ...
    function createWallets(uint256 _quantity) external onlyHolder {
        for (uint256 i = 0; i < _quantity; i++) {
            address newChild = CloneFactory(_cloneContract).createClone(address(_childContract));
            ChildStorage(_childStorage).addChildAddress(msg.sender, newChild);
            MinterChaild(payable(newChild)).initialize(msg.sender);
        }
    }

    function sendETH(uint256 _startId, uint256 _endId) external payable onlyHolder checkId(_startId, _endId) {
        uint256 beforeBalance = address(this).balance;
        uint256 value = msg.value / (_endId - _startId + 1);
        for (uint256 i = _startId; i <= _endId; i++) {
            (bool success,) = ChildStorage(_childStorage).child(msg.sender, i).call{value: value}(new bytes(0));
    ...
    function sendERC20(uint256 _startId, uint256 _endId, address _token, uint256 _amount) external onlyHolder checkId(_startId, _endId) {
        ...
        for (uint256 i = _startId; i <= _endId; i++) {
            IERC20(_token).transfer(ChildStorage(_childStorage).child(msg.sender, i), _amount);
        }
        // refund
        if (beforeBalance < IERC20(_token).balanceOf(address(this))) {
            IERC20(_token).transfer(msg.sender, IERC20(_token).balanceOf(address(this)) - beforeBalance);
        }
    }

    function run(uint256 _startId, uint256 _endId, address _callContract, bytes calldata _callData, uint256 _value) external onlyHolder checkId(_startId, _endId) {
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(ChildStorage(_childStorage).child(msg.sender, i))).run(_callContract, _callData, _value);
        }
    }

    function runWithSelector(uint256 _startId, uint256 _endId, address _callContract, string calldata _signature, uint256 _value) external onlyHolder checkId(_startId, _endId) {
        bytes memory callData = abi.encodeWithSignature(_signature);
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(ChildStorage(_childStorage).child(msg.sender, i))).run(_callContract, callData, _value);
        }
    }
    ...
    function withdrawETH(uint256 _startId, uint256 _endId) external checkId(_startId, _endId) {
        uint256 beforeBalance = address(this).balance;
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(ChildStorage(_childStorage).child(msg.sender, i))).withdrawETH(msg.sender);
        }
        payable(msg.sender).transfer(address(this).balance - beforeBalance);
    }
    ...
    function withdrawERC20(uint256 _startId, uint256 _endId, address _contract) external checkId(_startId, _endId) {
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(ChildStorage(_childStorage).child(msg.sender, i))).withdrawERC20(_contract, msg.sender);
            idx++;
        }
    }
    ...
    function withdrawERC721(uint256 _startId, uint256 _endId, address _contract, uint256[] calldata _tokenIds) external checkId(_startId, _endId) {
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(ChildStorage(_childStorage).child(msg.sender, i))).withdrawERC721(_contract, _tokenIds[idx], msg.sender);
            idx++;
        }
    }
    ...
    function withdrawERC1155(uint256 _startId, uint256 _endId, address _contract, uint256 _tokenId) external checkId(_startId, _endId) {
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            MinterChaild(payable(ChildStorage(_childStorage).child(msg.sender, i))).withdrawERC1155(_contract, _tokenId, msg.sender);
            idx++;
        }
    }
     ...
     * @dev Only Owner
     */
    function setContract(address _kudasai) external onlyOwner {
        kudasai = _kudasai;
    }
}
...
contract MinterChaild is IERC721Receiver {
    ...
    address private immutable _childStorage;
    constructor(address _storage) {
        _childStorage = _storage;
    }
    receive() external payable {}

    modifier onlyOwner() {
        require(deployer == tx.origin && ChildStorage(_childStorage).operater(msg.sender), "MinterChaild: No Call");
        _;
    }

    function initialize(address _deployer) external {
        require(deployer == address(0), "MinterChaild: Already initialized.");
        deployer = _deployer;
    }
    ...
    function withdrawERC721(address _contract, uint256 _tokenId, address _to) external onlyOwner {
        IERC721(_contract).safeTransferFrom(address(this), _to, _tokenId);
    }

    function withdrawERC1155(address _contract, uint256 _tokenId, address _to) external onlyOwner {
        bytes memory data;
    ...
    function withdrawETH(address _to) external onlyOwner {
        ...
        require(success, "MinterChaild: Transfer Failed");
    }

    function run(address _callContract, bytes calldata _callData, uint256 _value) external onlyOwner {
        (bool success,) = _callContract.call{value: _value}(_callData);
        require(success, "MinterChaild: Transaction Failed");
    }
}
