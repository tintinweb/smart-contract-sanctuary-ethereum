...
// File contracts/interface/IStakingDistributor.sol
...
contract Staking is AccessControl {
    ...
    struct Claim {
        ...
        bool lock; // prevent malicious delays for claim
    }
    ...
    function stake(
        ...
        if (_claim && warmupPeriod == 0) {
            ...
            warmupInfo[_to] = Claim({
                ...
                gons: info.gons.add(stakingToken.gonsForBalance(_amount)),
                expiry: epoch.number.add(warmupPeriod),
                lock: info.lock
            });

            gonsInWarmup = gonsInWarmup.add(stakingToken.gonsForBalance(_amount));
    ...
    function claim(address _to, bool _rebasing) public returns (uint256) {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            return _send(_to, stakingToken.balanceForGons(info.gons), _rebasing);
        }
        return 0;
    }
    ...
    function unstake(address _to, uint256 _amount, bool _trigger, bool _rebasing) external returns (uint256 amount_) {
        ...
        if (_rebasing) {
            ...
            governanceToken.burn(msg.sender, _amount); // amount was given in Governance Token terms
            amount_ = governanceToken.balanceFrom(amount_).add(bounty); // convert amount to Main TOken terms and add bounty
        }

        require(amount_ <= mainToken.balanceOf(address(this)), "Insufficient main token balance in contract");
        mainToken.safeTransfer(_to, amount_);
    }

    function wrap(address _to, uint256 _amount) external returns (uint256 gBalance_) {
        stakingToken.safeTransferFrom(msg.sender, address(this), _amount);
        gBalance_ = governanceToken.balanceTo(_amount);
        governanceToken.mint(_to, gBalance_);
    }

    function unwrap(address _to, uint256 _amount) external returns (uint256 sBalance_) {
        governanceToken.burn(msg.sender, _amount);
        sBalance_ = governanceToken.balanceFrom(_amount);
        stakingToken.safeTransfer(_to, sBalance_);
    }
    ...
    function rebase() public returns (uint256) {
        ...
        if (epoch.end <= block.timestamp) {
            ...
            uint256 staked = stakingToken.circulatingSupply();
    ...
    function _send(
        ...
        if (_rebasing) {
            stakingToken.safeTransfer(_to, _amount); // send as StakingToken (equal unit as MainToken)
            return _amount;
        } else {
            governanceToken.mint(_to, governanceToken.balanceTo(_amount)); // send as GovernanceToken (convert units from MainToken)
            return governanceToken.balanceTo(_amount);
        }
    }
    ...
    function supplyInWarmup() public view returns (uint256) {
        return stakingToken.balanceForGons(gonsInWarmup);
    }
