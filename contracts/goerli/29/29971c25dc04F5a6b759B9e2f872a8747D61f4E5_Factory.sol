    ...
    UUPSUpgradeable
{
    ...
    //Event for creation of the index fund
    ...
    //Event for updation of  index fund
    ...
    // It will initialize the pausable,ownable and uupsupgradable feature
    ...
    function getIndexInfo(uint256 id)
        ...
        view
        idcheck(id)
        returns (address[] memory tokens, uint16[] memory percentage)
    {
        (tokens, percentage) = IndexInterface(indexAddressArray[id])
            .getCurrentTokensInfo();
    }
    ...
    function updateIndexOwner(address newOwner, uint256 id)
        external
        onlyOwner
        idcheck(id)
    {
    ...
    function deposit(uint256 id, uint256 amount) external payable idcheck(id) {
        require(
            !(msg.value == 0 && amount == 0) && !(msg.value > 0 && amount > 0),
            "one needs to be zero"
        );
        console.log(
            !(msg.value == 0 && amount == 0) && !(msg.value > 0 && amount > 0)
        );
        ...
        if (ptoken == WETH && (msg.value > 0)) {
            ...
            console.log("B");
            console.log(IERC(ptoken).allowance(msg.sender, address(this)));
    ...
    function addRewardToken(uint256 id, address token) external idcheck(id) {
    ...
    function distributeamount(uint256 id) external onlyOwner idcheck(id) {
