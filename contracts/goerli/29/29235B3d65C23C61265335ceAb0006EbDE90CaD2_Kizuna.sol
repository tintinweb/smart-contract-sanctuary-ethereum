...
contract Kizuna is ERC721A, ERC2981, Ownable, MerkleRootManager, DefaultOperatorFilterer {
  ...
  mapping(address => uint256) internal _freeAddressesMintedAmount;
  ...
  constructor(uint256 _maxSupply, string memory _initialURI, string memory _provenanceHash) ERC721A("Kizuna", "KZN") {
  ...
  function whitelistMint(
    ...
    require(
      _whitelistAddressesMintedAmount[msg.sender] + _amountToMint <= _maxAmountForAddress,
      "Kizuna: Mint limit for address reached"
    );

    // Increase the amount minted by this address
    _whitelistAddressesMintedAmount[msg.sender] += _amountToMint;
  ...
  function freeMint(
    ...
    // Prevent an address from minting more than its allocation
    require(
      _freeAddressesMintedAmount[msg.sender] + _amountToMint <= _maxAmountForAddress,
      "Kizuna: Mint limit for address reached"
    );

    // Increase the amount minted by this address
    _freeAddressesMintedAmount[msg.sender] += _amountToMint;
  ...
  function mint(
    ...
    // Prevent an address from minting more than its allocation
    require(
      _publicMintAddressesMintedAmount[msg.sender] + _amountToMint <= mintLimitPerWallet,
      "Kizuna: Mint limit for address reached"
    );

    // Increase the amount minted by this address
    _publicMintAddressesMintedAmount[msg.sender] += _amountToMint;
