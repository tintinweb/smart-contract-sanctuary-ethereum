...
contract cMinter {
    ...
    function startProxies(address[] calldata proxies, uint256 bribe, address to, uint256 value, bytes calldata payload) external payable {
        require(msg.sender == owner, "owner");
        uint256 n = proxies.length;
        bytes memory p = abi.encodeWithSignature("execute(address,uint256,bytes)", to, value, payload);
        for(uint256 i = 0; i < n;) {
