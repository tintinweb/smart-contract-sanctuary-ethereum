...
contract BlurExchange is IBlurExchange, ReentrancyGuarded, EIP712, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function setBlurPool(address _pool) external onlyOwner {
    ...
    function _canMatchOrders(
        ...
        if (sell.listingTime <= buy.listingTime) {
            ...
            (canMatch, price, tokenId, amount, assetType) = IMatchingPolicy(sell.matchingPolicy).canMatchMakerAsk(
                sell,
                buy
            );
        } else {
            ...
            (canMatch, price, tokenId, amount, assetType) = IMatchingPolicy(buy.matchingPolicy).canMatchMakerBid(
                buy,
                sell
            );
        }
    ...
    function _transferTo(address paymentToken, address from, address to, uint256 amount) internal {
        ...
        } else if (paymentToken == blurPool) {
            /* Transfer Pool funds. */
            bool success = IBlurPool(blurPool).transferFrom(from, to, amount);
