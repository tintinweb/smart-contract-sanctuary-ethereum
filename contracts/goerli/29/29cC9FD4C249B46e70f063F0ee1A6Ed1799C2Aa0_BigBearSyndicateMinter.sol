	...
	ReentrancyGuardUpgradeable
{
	...
	mapping(address => bool) public addressToFreeMintClaim;

	/// @notice Tracks the number of tokens an address can still mint through the paid mint
	...
	*/
	function freeMint()
		...
		whenSupplyRemains(1)
	{
		if (!addressToFreeMintClaim[msg.sender]) {
			revert MintAllowanceExceeded(msg.sender, 1);
		}

		addressToFreeMintClaim[msg.sender] = false;

		_mint(msg.sender, 1);
	}

	/**
	 * @notice Sets the address to be able to claim a free mint. This does not check if addresses has already claimed a free mint before.
	 * @param users address[] of the users
	 */
	function setFreeMintClaims(address[] calldata users)
		external
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
		for (uint256 i = 0; i < users.length; i++) {
			addressToFreeMintClaim[users[i]] = true;
		}
	}
	...
	/**
	@notice Mint a BigBearSyndicate in a paid tier
	@param numBears uint256 of the number of BigBearSyndicates to mint
	*/
	function paidMint(uint256 numBears)
		external
		payable
		nonReentrant
		whenNotPaused
	...
	{
		uint256 mints = addressToPaidMints[msg.sender];
		...
		addressToPaidMints[msg.sender] -= numBears;
	...
	*/
	function allowListMint(uint256 numBears, bytes32[] calldata merkleProof)
		...
		withinMintLimit(numBears)
	{
		if (addressToAllowListMints[msg.sender] + numBears > allowListMints) {
