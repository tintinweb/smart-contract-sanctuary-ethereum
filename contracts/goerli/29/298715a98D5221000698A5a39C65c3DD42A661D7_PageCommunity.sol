    ...
    IPageCommunity
{
    ...
    mapping(uint256 => mapping(address => uint256)) private postAccess;
    ...
    function readPost(uint256 postId) external view override onlyCommunityActiveByPostId(postId) returns(
        ...
        if(isPrivacyAccess(_msgSender(), getCommunityIdByPostId(postId))) {
            ...
            isEncrypted = readed.isEncrypted;
        }
    }

    function buyPostAccess(uint256 postId) external override {
        // 1 day
        postAccess[postId][_msgSender()] = block.timestamp + 86400;
    }

    function hasAccessToPost(uint postId, address user) external view override returns(bool) {
        uint256 timeaccess = postAccess[postId][user];
        if (timeaccess != 0 && timeaccess > block.timestamp) {
            return true;
        } else {
            return false;
        }
     ...
     * @param postId ID of post
     */
    function createPost(uint256 postId, bool isEncrypted) private {
        Post storage newPost = post[postId];
        newPost.creator = _msgSender();
        newPost.isEncrypted = isEncrypted;
        newPost.isView = true;
    }
...
interface IPageCommunity {
    ...
    function buyPostAccess(uint256 postId) external;

    function hasAccessToPost(uint postId, address user) external view returns(bool);
