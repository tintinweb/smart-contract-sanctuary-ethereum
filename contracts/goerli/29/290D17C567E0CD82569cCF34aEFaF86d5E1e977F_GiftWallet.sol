...
contract GiftWallet {
    ...
    // returns 1256570613950000000000
    ...
    // actually returns ETH to USD
    function ethToUsdInWei(uint256 _weiAmount) public view returns (uint256) {
        uint256 ethPrice = getPrice();
        uint256 ethAmountInUsd = (ethPrice * _weiAmount) / 1e18;
        return ethAmountInUsd;
    }

    // actually return USD in ETH
    function usdToEth(uint256 _usdAmount) public view returns (uint256) {
        uint256 ethPrice = getPrice();
        uint256 weiAmount = _usdAmount * ethPrice;
        return weiAmount;
    }

    function getGiftInWei(uint256 value) public view returns (uint256) {
        uint256 serviceCostWei = usdToEth(SERVICE_COST_USD);
