...
contract dangar is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "dangar";
    string private _symbol = "dangar";
    ...
    uint256 public _maxTxAmount =  _totalSupply.mul(10).div(denominator);     //0.5%
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if((block.timestamp > launchTime + 15 minutes) && !triggered && (launchTime != 0) ){
            _maxTxAmount =  _totalSupply.mul(20).div(denominator);     //1.5%
            _walletMax = _totalSupply.mul(20).div(denominator);    //1.5%
            triggered = true;
