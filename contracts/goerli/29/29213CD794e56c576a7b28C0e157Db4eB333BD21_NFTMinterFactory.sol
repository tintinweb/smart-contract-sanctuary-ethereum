...
import "../market_nft_minter/MarketNFT.sol";
...
contract NFTMinterFactory {
    bytes32 public constant MINTING_CONTRACT_HASH = keccak256(abi.encodePacked(type(MarketNFT).creationCode));

    address[] public allContracts;

    event NFTMinterContractCreated(
    ...
    function createContract(
        ...
        contractCreated = address(new MarketNFT());

        INFT(contractCreated).initialize(treasury, market, name, symbol, baseURI);
        INFT(contractCreated).transferOwnership(client);

        allContracts.push(contractCreated);

        emit NFTMinterContractCreated(contractCreated, treasury, market, name, symbol, baseURI);
    }
}
