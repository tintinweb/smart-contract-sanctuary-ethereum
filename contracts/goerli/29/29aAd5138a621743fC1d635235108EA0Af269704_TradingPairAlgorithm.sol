...
contract TradingPairAlgorithm is PositionAlgorithm, ITradingPairAlgorithm {
    ...
    function createAlgorithm(
        ...
        ItemRef memory feeAsset1 = positionsController
            .getAssetReference(positionId, 1)
            .clone(address(this));
        ItemRef memory feeAsset2 = positionsController
    ...
    ITradingPairFeeDistributer
{
    ...
    function claimRewards() external {
        ...
        require(feeTokenLocks[msg.sender] > 0, 'has no fee token lock');
        _claimRewards(msg.sender);
        _tryNextFeeRound();
    }
    ...
    function _claimRewards(address account) internal {
        ...
        uint256 ownerCount = (ownerAssetToDistribute * feeTokensCount) /
            distributeRoundTotalFeeTokensLock;
        uint256 outputCount = (outputAssetToDistribute * feeTokensCount) /
            distributeRoundTotalFeeTokensLock;
        ownerAssetDistributedTotal += ownerCount;
        outputAssetDistributedTotal += outputCount;
        if (ownerCount > 0) _asset1.withdraw(account, ownerCount);
        if (outputCount > 0) _asset2.withdraw(account, outputCount);
