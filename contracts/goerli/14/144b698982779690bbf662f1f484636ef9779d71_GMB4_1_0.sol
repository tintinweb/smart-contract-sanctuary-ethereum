...
// File @a16z/contracts/licenses/[email protected]
...
// File contracts/GenesisMint/GMB-4.1.0.sol

// NOTES: GMB4 is beta deploy of the Genesis Mint
// it obfuscates the name and token
// it has first attempt with access controls
// It is not intended to be the final CursedEmoji contract
...
// TODO: remove this after dev import "hardhat/console.sol";

string constant CollectionName = "GM Beta4.1.0";
string constant CollectionSymbol = "GMBeta410";


contract GMB4_1_0 is ERC721, ERC721URIStorage, Ownable, CantBeEvil {
    using Counters for Counters.Counter;
    using EnumerableSet for EnumerableSet.AddressSet;

    Counters.Counter private _tokenIdCounter;
    Counters.Counter private _totalRemixes;
    EnumerableSet.AddressSet private cursedSet;

    mapping(uint256 => uint256) private _remixes;
    uint256 private _remixLimit;

    event GenesisMint(address indexed from, string uri);
    event Remix(address indexed from, uint256 tokenId, string uri);
    ...
    function curse(address account) public onlyOwner {
        EnumerableSet.add(cursedSet, account);
    }

    function unCurse(address account) public onlyOwner {
        EnumerableSet.remove(cursedSet, account);
    }

    function isCursed(address account) public view returns (bool) {
        return EnumerableSet.contains(cursedSet, account);
    }

    function getAllCursed() public onlyOwner view returns (address[] memory) {
        return EnumerableSet.values(cursedSet);
    }
    ...
    function genesisMint() public onlyCursed() {
        EnumerableSet.remove(cursedSet, msg.sender);
    ...
    function _checkCursed() internal view virtual {
        _checkCursed(_msgSender());
    }

    function _checkCursed(address account) internal view virtual {
        if (!EnumerableSet.contains(cursedSet, account)) {
            revert(
            string(
                abi.encodePacked(
                    "Oh no! account ",
    ...
    function setRemixLimit(uint256 limit) public onlyOwner {
        _remixLimit = limit;
    }
