...
contract Vault {
    ...
    function changeFee(uint256 newFee) public {
        require(
            newFee >= feeInterval[0] && newFee <= feeInterval[1],
            "Invalid fee value"
        );
        fee = newFee;
    }
    ...
    function deposit(uint256 amount) public {
        ...
        require(
            IERC20(tokenAddress).balanceOf(msg.sender) >= amount + feeAmount,
            "Insufficient token balance"
        );
        require(
            IERC20(tokenAddress).approve(address(this), amount + feeAmount)
        );
        require(
    ...
    function withdraw(uint256 amount) public returns (uint256, address) {
        ...
        bool isSuccessTransferFrom = IERC20(tokenAddress).transferFrom(
            address(this),
            msg.sender,
            amount
        );
