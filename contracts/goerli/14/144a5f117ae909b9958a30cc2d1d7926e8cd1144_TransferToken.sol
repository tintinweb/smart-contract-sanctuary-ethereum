...
// ----------------------------------------------------------------------------
contract TransferToken is ERC20Interface, Owned, Oracled, Verify {
    ...
    event Transfer(address indexed from, string to, uint tokens, uint chainId);
    ...
    function transfer(string memory to, uint tokens, uint chainid) public returns (bool success) {
        balances[msg.sender] = balances[msg.sender].sub(tokens);
        balances[address(0)] = balances[address(0)].add(tokens);

        emit Transfer(msg.sender, address(0), tokens);
        emit Transfer(msg.sender, to, tokens, chainid);
    ...
    function verifySigData(bytes memory sigData) private returns (TransferData memory) {
        ...
        require(block.timestamp < SafeMath.add(td.ts, (60 * 60 * 24 * 30)), "Transfer has expired");
