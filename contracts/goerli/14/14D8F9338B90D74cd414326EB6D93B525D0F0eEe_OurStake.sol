...
contract OurStake is Ownable, ReentrancyGuard {
    ...
    modifier onlyStakeholder() {
        require(isStakeholder(msg.sender), "OurStake: caller is not the stakeholder");
        _;
    }

    modifier validRewardPlanIndex(uint256 _index) {
        require(_index < rewardPlans.length, "OurStake: reward plan does not exist");
        _;
    }

    modifier validStakeIndex(address _stakeholder, uint256 _index) {
        Stake[] memory _stakes = stakeholders[_stakeholder].stakes;
        require(_index < _stakes.length, "OurStake: stake does not exist");
        _;
    }
    ...
    {
        require(_amount > 0, "OurStake: amount cannot be zero");
        RewardPlan memory _rewardPlan = rewardPlans[_rewardPlanIndex];
        require(_rewardPlan.deletedAt == 0, "OurStake: reward plan does not exist");
        ...
        require(_reward <= rewardPool.available, "OurStake: insufficient reward");
        totalStaked += _amount;
        ...
        validStakeIndex(msg.sender, _stakeIndex)
    {
        ...
        require(_amount > 0, "OurStake: stake does not exist");
        RewardPlan memory _rewardPlan = rewardPlans[_stake.rewardPlanIndex];
        require(block.timestamp - _stake.createdAt > _rewardPlan.duration, "OurStake: stake is still locked");
    ...
    function createRewardPlan(string memory _name, uint256 _duration, uint256 _rate)
        ...
        onlyOwner
    {
        require(_duration > 0, "OurStake: duration cannot be zero");
        require(_rate > 0, "OurStake: rate cannot be zero");
    ...
    function removeRewardPlan(uint256 _index)
        ...
        validRewardPlanIndex(_index)
    {
        require(rewardPlans[_index].deletedAt == 0, "OurStake: reward plan does not exist");
