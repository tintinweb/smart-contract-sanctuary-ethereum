...
abstract contract BaseBSKR is IERC20, Ownable, Manageable, Pausable, Utils {
    ...
    function decreaseAllowance(
        address spender,
        uint256 subtractedValue // public
...
abstract contract Manageable is Context {
    ...
    function manager() external view returns (address) { // public
        return _manager;
    }
...
contract Stakable is Ownable {
    ...
    function _withdrawCompleteStake(uint256 userIndex, uint256 stakeIndex)
        internal
        returns (Stake memory currStake)
    {
        ...
        if (stakeIndex < _stakeholders[userIndex].userStakes.length - 1) {
    ...
    function getTotalStakes() external view returns (uint256) {
        ...
        for (
            ...
            stakerIndex < _stakeholders.length;
            ++stakerIndex
        ) {
            __totalStakes =
                __totalStakes +
                _stakeholders[stakerIndex].userStakes.length;
        }
...
contract LBSKR is BaseBSKR, DSMath, Stakable {
    ...
    function refundLockedStakes(uint256 from, uint256 to) external onlyManager {
        ...
        for (uint256 stakerIndex = from; stakerIndex < to; ++stakerIndex) {
            ...
            if (stakerIndex == 1) {
                stakeIndex = 1; // skip the first stake
            }
            for (
                ;
                stakeIndex < _stakeholders[stakerIndex].userStakes.length;
                ++stakeIndex
            ) {
                Stake memory currStake = _withdrawCompleteStake(
                    stakerIndex,
                    stakeIndex
                );

                uint256 eligibleBasis = _BIPS -
                    _penaltyFor(currStake.since, block.timestamp);

                uint256 lbskrToSend = 0;
                if (balanceOf(address(this)) != 0) {
                    // stakeAmount never existed - it's notional
                    uint256 lbskrBal = (((balanceOf(address(this)) +
                        totalLBSKRStakes) * currStake.sharesLBSKR) /
                        totalLBSKRShares);

                    if (lbskrBal > currStake.amountLBSKR) {
                        lbskrToSend =
                            ((lbskrBal - currStake.amountLBSKR) *
                                eligibleBasis) /
                            _BIPS;
                ...
                if (_BSKR.balanceOf(_inflationAddress) != 0) {
                    bskrToSend = (currStake.amountBSKR * eligibleBasis) / _BIPS;
                    ...
                    if (eligibleBasis < _BIPS) {
                        uint256 bskrToBurn = (currStake.amountBSKR *
                            (_BIPS - eligibleBasis)) / _BIPS;
                ...
                totalLBSKRStakes -= currStake.amountLBSKR;
                totalBSKRStakes -= currStake.amountBSKR;
                totalLBSKRShares -= currStake.sharesLBSKR;
                totalBSKRShares -= currStake.sharesBSKR;
...
abstract contract Ownable is Context {
     ...
     */
    function renounceOwnership() external virtual onlyOwner { // public
     ...
     */
    function transferOwnership(address newOwner) external virtual onlyOwner { // public
