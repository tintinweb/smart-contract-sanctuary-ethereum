...
contract P2PLoanFactory{
    ...
    function LFPayLoan(uint256 _index, address _loanee) public payable {
        P2PLoan p2pLoanContract = getLoanContract(_index, _loanee);
        p2pLoanContract.payLoan(msg.value, address(p2pLoanContract), msg.sender);
    }


    function interest(uint256 _index, address _loanee) public view returns(uint256, uint256, uint256){
        P2PLoan p2pLoanContract = getLoanContract(_index, _loanee);

        (uint256 x, uint256 y, uint256 z)=p2pLoanContract.getInterest();
        return (x, y, z);
    } 
...
contract P2PLoan{
    ...
    function payLoan(uint256 _value, address _from, address _sender) public payable {
        interestInValidPeriod();
        require(_sender == borrower, "Only borrower can call this function.");
        ...
        if (block.timestamp >= loanDetail.durationOfLoan) {
            ...
            payable(lender).transfer(_value);
            NFTContract.transferFrom(_from, borrower, NFTToTokenID[NFTContract]);
        }
    ...
    function getInterest() public view returns(uint256 interestToBePaid, uint256 validStart, uint256 loanI) {
        ...
        if (block.timestamp < loanDetail.durationOfLoan) {
            ...
            return(interestToBePaid, start, block.timestamp);
        }else {
            uint256 newInterest = loanDetail.interest;
            interestToBePaid = (loanDetail.loanAmount * newInterest)/ 1 ether;
            
            return(interestToBePaid, validStart,loanDetail.interest);
        }
        // assume that the timestamp is in seconds
        // it's in wei
    }
    event Valid(uint256 _valid);
    function interestInValidPeriod() public view returns(uint256){
        (uint256 interestFromGetinterest, uint256 validStart,) = getInterest();
        uint256 interestNow;
        if (block.timestamp < validStart){
            interestNow = interestFromGetinterest;
            return interestNow;
        }else{
            (uint256 interestFromGetinterestAgain,,) = getInterest();
            interestNow = interestFromGetinterestAgain;
            return interestNow;
        }
    }
}
