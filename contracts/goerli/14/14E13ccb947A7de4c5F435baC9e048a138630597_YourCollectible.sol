...
interface IERC721JFreeClaims is IERC721J {
...
import "./IERC721J.sol"; //Interface Id: 0x7b77f4b5
import "./IERC721JFull.sol"; //Interface Id: 0xe74d3825
import "./IERC721JFreeClaims.sol"; //Interface Id: 0x4ad0d27c
...
//Added recycle to burn 2 songs to mint 1. Recycling has an opt out option.
    ...
    IERC2981
{
    ...
    event RoyaltySet(uint256 indexed royaltyBPS);
        ...
        returns (bool)
    {
        return
            ...
            interfaceId == type(IERC721JFreeClaims).interfaceId ||
    ...
    //Changes an array of songURIs
    function setArraySongURIs(
        uint256 songId,
        uint[] memory songGenerations,
        string[] memory songURIs
        ) public virtual onlyOwner {

        uint length = songGenerations.length;
        if (songId > _songIds) revert URIQueryForNonexistentSong();
    ...
    function togglePublicMint(uint256 tokenId) public virtual override {
    ...
    function rarityMultiplier(uint256 generation) public view virtual returns (uint256 multiplierPercent) {
        return _rarityMultipliers[generation];
    }
    //Returns the adjusted mint price after the multiplier
    function rarityMintPrice(uint256 generation) public view virtual returns (uint256 price) {
    ...
    function mintOriginal1Split(string memory songURI1, uint256 maxEditions, address payable splitAddress)
        public
        onlyOwner
    {
        //Updates the count of total tokenids and songids
    ...
    function amountOfSongs() public view virtual override returns (uint256) {
        return _songIds;
    }
    ...
    //Address to SongIn to SongOut. For YourCollectible
    ...
    //
    //For YourCollectible. Checks if a contract address, with song and rarity, is whitelisted
    function claimCheckSpecific(
        address _contract,
        uint256 songIdIn,
        uint256 generationIn
    ) public view virtual override returns (uint256 _songIdOut, uint256 _generationOut) {
        uint256 _songIn = (songIdIn * (10**18)) + generationIn;
    ...
    //
    //Checks if a token from a contract has been claimed through the YourCollectible specific whitelist.
    function giftClaimCheckSpecific(address _contract, uint256 tokenId)
        public
        view
        virtual
    ...
    function freeClaim(address _contract, uint256 tokenId) public override {
        //Sets contract address as external nft
        YourCollectible _claimContract = YourCollectible(_contract);
    ...
    //
    //Free claim specific is for YourCollectible tokens
    function freeClaimSpecific(address _contract, uint256 tokenId) public override {
        //Sets contract address as external nft
        YourCollectible _claimContract = YourCollectible(_contract);
        //Checks if the token has been claimed
    ...
    //
    //Free claim song reward is for YourCollectible tokens to claim a song specific reward
    ...
    //
    //Free claim rarity reward is for YourCollectible tokens to claim a rarity specific reward
