...
contract Marketplace is Ownable, Pausable, IERC1155Receiver, ERC165 {
    ...
    mapping(uint256 => uint256[]) public marketIDsbyTokenID;
    ...
    function removeMarketID(
        ...
        uint256[] storage marketIDs = marketIDsbyTokenID[tokenID];
        ...
        for (uint256 i = 0; i < marketIDs.length; i++) {
            if (marketIDs[i] == marketIDToRemove) {
                ...
                require(
                    marketItem.quantity == quantityToRemove,
                    "Quantity does not match"
                );
                // If the quantities match, delete the item from both mappings
                delete marketIDs[i];
    ...
    function extDeleteMarketItem(uint256 horseId) external {
        ...
        if (marketIDsbyTokenID[horseId].length != 0) {
            _deleteMarketItem(marketIDsbyTokenID[horseId][0], 1);
        }
    }
    ...
    function createMarketItem(
        ...
        if (is1155[nftContract] == true) {
            ...
            if (marketIDsbyTokenID[tokenID].length != 0) {
                ...
                for (
                    uint256 i = 0;
                    i < marketIDsbyTokenID[tokenID].length;
                    i++
                ) {
                    sharesForSale += marketItems[marketIDsbyTokenID[tokenID][i]]
                        .quantity;
                    if (
                        marketItems[marketIDsbyTokenID[tokenID][i]].seller ==
                        msg.sender
                    ) {
                        sharesByUser += marketItems[
                            marketIDsbyTokenID[tokenID][i]
                        ].quantity;
                    }
                }
                require(
            ...
            require(
                marketIDsbyTokenID[tokenID].length == 0,
                "CAN'T LIST ITEM THAT IS ALREADY LISTED"
            );
        }
        ...
        marketIDsbyTokenID[tokenID].push(itemID);
    ...
    function makeOffer(
        ...
        offers[nftContract][tokenID].push(
            Offer(msg.sender, (msg.value / quantity), quantity, exp)
        );
        emit OfferMade(
            indexID,
            nftContract,
    ...
    function acceptOffer(
        ...
        require(newOwner != address(0), "OFFER EXPIRED OR DOES NOT EXIST");
    ...
    function getAllOffers(
        ...
        uint256 len = offers[nftContract][tokenID].length;
        Offer[] memory items = new Offer[](len);
        uint256 j = 0;
        for (uint256 i = 0; i < len; i++) {
            if (offers[nftContract][tokenID][i].exp > block.timestamp) {
                items[j] = offers[nftContract][tokenID][i];
                j++;
            }
        }
    ...
    function fetchMarketItemsbyAddress(
        ...
        for (uint256 l = 0; l <= j; l++) {
            if (
                items[l].nftContract == tokenAddress &&
                items[l].exp > block.timestamp
            ) {
                items2[k] = (items[l]);
                k++;
