...
contract StableSwap3PoolHelper {
    ...
    function verifyAccount(
        ...
        RLPReader.RLPItem memory storageProof
    )public returns (PoolStorage memory poolStorage) {

        RLPReader.RLPItem[] memory accProofList = accountProof.toList();
        emit LogStep(accProofList.length);

        RLPReader.RLPItem[] memory storageProofList = storageProof.toList();
