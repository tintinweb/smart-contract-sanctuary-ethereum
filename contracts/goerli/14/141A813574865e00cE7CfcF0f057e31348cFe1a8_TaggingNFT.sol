...
contract TaggingNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    // ------ SET SALE AS PAUSED
    SaleStatus public saleStatus = SaleStatus.DEV;
    Counters.Counter private _tokenIdCounter;

    string private preRevealURI;
    string private postRevealBaseURI;

    uint256 public constant MAX_TAGGING = 2000;
    uint256 public constant PRICE_TAGGING = 0.02 ether;
    bytes32 public devMerkleRoot;
    bytes32 public taggingMerkleRoot;
    ...
    function verifyMerkleProof(
        ...
        return _proof.verify(devMerkleRoot, _leaf);
    }
    ...
    function devMintNFT(
        ...
        require(msg.value == PRICE_TAGGING, "Incorrect ether amount");
        require(
            saleStatus == SaleStatus.DEV ||
                saleStatus == SaleStatus.TAGGING ||
    ...
    function allowlistMintNFT(
        ...
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        require(saleStatus == SaleStatus.ALLOWLIST, "Sale is not open");
        require(msg.value == PRICE_TAGGING, "Incorrect ether amount");
        require(!claimed[tokenId], "NFT has been claimed");
        require(
            MerkleProof.verify(
    ...
    function setMerkleRoots(
        bytes32 _devMerkleRoot,
        bytes32 _taggingMerkleRoot,
        bytes32 _allowlistMerkleRoot
    ) external onlyOwner {
        devMerkleRoot = _devMerkleRoot;
        taggingMerkleRoot = _taggingMerkleRoot;
    ...
    function _transfer(
        ...
        emit TaggingTransfer(tokenId, from, to);
    }
    ...
    function tokenURI(
        ...
        return
            string(
                abi.encodePacked(postRevealBaseURI, Strings.toString(_tokenId))
            );
    }
