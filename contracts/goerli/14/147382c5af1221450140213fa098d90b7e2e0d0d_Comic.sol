...
contract Comic is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 constant MINT_PER_WALLET = 3;

    // TODO: is the merkle proof upgradable?
    // is there a limit per allowlist member?
    // is there a limit mint per wallet?
    // how are people going to redeem the comic?
    // what will be in the metadata? example tokenURI: https://ipfs.io/ipfs/QmRacb7Cd4LEqTyFah8g6SVNy5Ddv8TPFcebhLRyb1ERWC/1
    // {
    // "name": "2000AI Comic",
    // "description": "2000AI Comic, Issue #1, November 2022",
    // "image": "https://ipfs.io/ipfs/QmSJKkfvXun72s5kYADjEwSVRhMVNCFoEvuC9N6GrGwDvF/"
    // "attributes":[{"trait_type":"Comic","value":"# 1"},{"trait_type":"Name","value":"Insectoid Slurper"}] ?
    // }
    // Reveal token?
    // Merkle Root: 0x2ad235a2db7b5827ee998381e30c9c9fe0d3e163ad8a1639ee7163f9396ec779
    ...
    // todo: also add # of mintable per allowlist member
    ...
    function mint(uint256 amount) external payable {
        if (mintingPhase != Phase.PUBLIC) {
            revert IncorrectMintPhase();
        }
        if (amount > MINT_PER_WALLET) {
            revert MaxMintExceeded();
        }
        if (amount > COMIC_SUPPLY) {
            revert ComicSupplyExceeded();
        }
        _safeMint(msg.sender, amount);
    }

    // @notice Allows the owner to start the Allowlist minting phase.
    function startAllowlistPhase() external onlyOwner {
        mintingPhase = Phase.AL;
    ...
    // @notice Allows user to check if wallet has been allowlisted
