    ...
    EIP712Upgradeable
{
    ...
    bytes32 internal constant _FACTORY_ADMIN_ROLE =
    ...
    function getCreationFee(uint256 id) public view returns (uint256) {
        ...
        INash21Guarantee guaranteeInterface = INash21Guarantee(
            INash21Manager(manager).get(_GUARANTEE)
        );
        return
    ...
    function split(
        ...
        require(
            INash21Manager(manager).get(_GUARANTEE) == msg.sender,
            "N21F007"
        );
        ...
        whenNotPaused
    {
        ...
        if ((from != address(0)) && (to != address(0))) {
            ...
            if (feeRequired > 0) {
                ...
                address vault = INash21Manager(manager).get(keccak256("VAULT"));
                require(vault != address(0), "N21F002");
                tokenInterface.transferFrom(from, vault, feeRequired);
            }
        }
