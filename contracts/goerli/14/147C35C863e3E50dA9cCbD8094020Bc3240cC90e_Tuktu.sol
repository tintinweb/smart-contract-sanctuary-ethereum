...
abstract contract TAccount is XProgram {
	...
	event Registration(uint32 indexed _AccountID, uint32 _SponsorID, uint32 _UplineIDOnBINARY, uint32 _UplineIDOnTERNARY);
	...
	function _register(
		...
		_InitAccountForMaxtrixes(newid, _SponsorID, _UplineIDOnBINARY, _UplineIDOnTERNARY); // Initialize for Matrixes
		// _InitAccountForXProgram(newid); // Initialization and Activation for each xprogram

		emit Registration(newid, _SponsorID, _UplineIDOnBINARY, _UplineIDOnTERNARY);
	}
	...
	function _AccountOfAffiliate(bytes16 _Affiliate) internal view returns (uint32 _AccountID) {
		return _AccountID = Affiliates[_Affiliate];
	}

	function _AffiliateOfAccount(uint32 _AccountID) internal view returns (bytes16 _Affiliate) {
		return _Affiliate = Accounts[_AccountID].Affiliate;
	}

	function _AddressOfAccount(uint32 _AccountID) internal view returns (address _Address) {
		return Accounts[_AccountID].Address;
	}
	...
	function _isAccountExisted(uint32 _AccountID) internal view returns (bool _isExist) {
		return _AccountID != 0 && Accounts[_AccountID].AccountID == _AccountID;
	}
	...
	function InfoOfAccount(uint32 _AccountID) public view virtual returns (AccountInfoOf memory _AccountInfoOf) {
		return
			_AccountInfoOf = AccountInfoOf({
				...
				RegistrationTime: Accounts[_AccountID].RegTime,
				AutoLevelingup: _isAutoLevelUp(_AccountID)
			});
	}
}
...
contract Tuktu is TAccount {
	...
	// _LevelOn
	function Register(uint32 _SponsorID, uint32 _UplineIDOnBINARY, uint32 _UplineIDOnTERNARY) public {
		...
		_register(msg.sender, _SponsorID, _UplineIDOnBINARY, _UplineIDOnTERNARY);
	}
	...
	function Register(
		...
		_register(_NewAccountAddress, _SponsorID, _UplineIDOnBINARY, _UplineIDOnTERNARY);
	}
