...
error MaidsMarketPlace_OverSupplyError();
error MaidsMarketPlace_insufficientAllowanceError();
error MaidsMarketPlace_NotOperator();
...
contract MaidsMarketPlace is Initializable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  modifier onlyOperator() {
    if (!_operators[msg.sender]) revert MaidsMarketPlace_NotOperator();
    _;
  }
  ...
  function createMarketItem (
    ...
    string memory tokenURI
      ...
      nft.setURI(itemId, tokenURI);
  }

  function buyItem (uint256 itemId, uint256 amount) external whenNotPaused {
    uint256 supply = _idToMarketItem[itemId].supply;
    if (supply < amount) revert MaidsMarketPlace_OverSupplyError();
    _idToMarketItem[itemId].supply -= amount;

    uint256 price = _idToMarketItem[itemId].price;
    uint256 allowance = token.allowance(msg.sender, address(this));
    if (allowance < price * amount) revert MaidsMarketPlace_insufficientAllowanceError();
    token.transferFrom(msg.sender, address(this), price * amount);

    nft.mint(msg.sender, itemId, amount);

    emit BuyItem(msg.sender, itemId, amount);
  } 
