...
contract Vesting is PausableUpgradeable, BumperAccessControl {
    ...
    struct VestingInfo {
        uint start; // timestamp of start vesting
        uint end; // timestamp of end vesting
        uint cliff; // timestamp of cliff vesting
        uint vestingPerSec; // token release rate
        uint totalAmount; // total vested amount in current schedule
        uint onStartAmount; // amount is available immediately after start
        uint previousAmount; // amount from previous shedule
        uint claimedV1; // claimed amount from first version vesting
    }
    ...
    mapping(address => uint) public recipientClaimed;
    ...
    function initialize(
        ...
        if (_staking != address(0)) {
            staking = _staking;
            IERC20(tokenBUMP).safeApprove(staking, type(uint).max);
        }
    ...
    function claim(
        ...
        VestingInfo memory vestingInfo,
        bytes32[] memory merkleProof
    ) external whenNotPaused verifyProofs(account, vestingInfo, merkleProof) {
        _restoreClaimedV1(account, vestingInfo);

        uint amount = getClaimableAmountFor(account, vestingInfo);
    ...
    function stakeVestedTokens(
        ...
        require(amount >= claimAmount, "Wrong claimAmount");

        require(claimAmount >= stakingAmount, "Wrong stakingAmount");

        recipientClaimed[account] += claimAmount;

        StakeRewards(staking).stakeFor(
            stakingAmount,
            stakingOption,
            account,
            autorenew
    ...
    /// @param account Investor address to which funds will be transferred
    /// @param vestingInfo Vesting schedule info
    function _restoreClaimedV1(address account, VestingInfo memory vestingInfo)
        internal
    {
    ...
    function getClaimableAmountFor(
        ...
        uint totalClaimed = recipientClaimed[account];
        if (vestingInfo.claimedV1 > totalClaimed) {
            totalClaimed = vestingInfo.claimedV1;
        }

        uint locked = totalLockedOf(vestingInfo);

        if (
            vestingInfo.totalAmount + vestingInfo.previousAmount >
            locked + totalClaimed
        ) {
            return
                vestingInfo.totalAmount +
        ...
        returns (uint)
    {
        ...
        if (block.timestamp >= vestingInfo.end) {
            ...
            return 0;
        } else if (block.timestamp >= vestingInfo.start + vestingInfo.cliff) {
            // If the period has not yet passed block diff, OnStartVesting not blocked
            return
                (vestingInfo.end - block.timestamp) * vestingInfo.vestingPerSec;
        } else {
...
abstract contract BumperAccessControl is
    Initializable,
    ContextUpgradeable,
    OwnableUpgradeable
{
...
contract StakeRewards is Initializable, BumperAccessControl {
    ...
    // total emission per second for all options
    uint public totalEmissionPerSecond;
    ...
    event Claimed(address sender, uint amount, uint timestamp, uint16 option);
    ...
    event ChangeEmission(uint oldEmission, uint newEmission);
    ...
    function initialize(
        ...
        // default emission of 5000 BUMPs per day
        _setEmissionPerSecond((5000 * 10**18) / uint(24) / uint(60) / uint(60));
    }
    ...
    function freeAmount() public view returns (uint) {
        ...
        return IERC20(stakeToken).balanceOf(address(this)) - total;
    }
    ...
    function switchAutorenew(uint16 stakeIndex) external unlocked {
        ...
        if (s.autorenew) {
            uint64 numOfperiods = uint64(block.timestamp - s.start) /
                periods()[s.option] +
                1;
    ...
    function calcRewards(StakeInfo memory s, StakeOption memory opt)
        public
        view
        returns (
            uint rewards,
            bool claimable,
            bool withdrawable,
            uint endOfLastPeriod
        )
    {
        rewards = ((opt.index - s.lastCI) * s.amount) / 10**18;

        if (periods()[s.option] == 0) {
            // flexible staking
            endOfLastPeriod = block.timestamp;
            claimable = rewards > 0;
            withdrawable =
                block.timestamp > (s.requestedAt + cooldownPeriod) &&
                block.timestamp <
                (s.requestedAt + cooldownPeriod + withdrawWindow);
        } else if (s.autorenew) {
            uint numOfPeriods = (block.timestamp - s.start) /
                periods()[s.option];
            endOfLastPeriod = s.start + (numOfPeriods * periods()[s.option]);
            withdrawable =
                block.timestamp > endOfLastPeriod &&
                block.timestamp < endOfLastPeriod + withdrawWindow;
        } else {
            // no autorenew and option with lockup period
            endOfLastPeriod = s.end;

            if (block.timestamp > s.end) {
                uint extraTime = block.timestamp - s.end;
                uint extraRewards = (rewards * extraTime) /
                    (s.end - s.start + extraTime);
    ...
    function calcRewardsByIndex(uint16 stakeIndex)
        public
        view
        returns (
            uint rewards,
        ...
        )
    {
        ...
        StakeOption memory opt = stakeOptions[s.option];
        opt.index = calculateCumulativeIndex(s.option);
        (rewards, claimable, withdrawable, endOfLastPeriod) = calcRewards(
            s,
            opt
        );
    }
    ...
    function setEmissionPerSecond(uint newEmissionPerSecond)
        external
        onlyGovernance
    {
    ...
    function totalAmount()
        public
        view
        returns (uint amount, uint weithedAmountSum)
    {
    ...
    function _updateEmissions() internal {
        ...
        for (uint16 i = 0; i < stakeOptions.length; i++) {
            ...
            if (weigthedAmountSum > 0) {
                option.emission = uint64(
                    (totalEmissionPerSecond * (option.total * m[i])) /
                        weigthedAmountSum
                );
            } else option.emission = 0;
        }
    }
    ...
    function _setEmissionPerSecond(uint newEmissionPerSecond) internal {
        totalEmissionPerSecond = newEmissionPerSecond;
        _updateIndexes();
        _updateEmissions();
    }
    ...
    function calculateCumulativeIndex(uint16 option)
        public
        view
        returns (uint index)
    {
        ...
        if (opt.total > 0) {
            index =
                opt.index +
                ((block.timestamp - lastIndexTimestamp) *
                    opt.emission *
                    10**18) /
                opt.total;
        } else {
    ...
    function updateUnlockTimestamp(uint64 _timestamp)
        external
        onlyGovernanceOrOwner
    {
    ...
    function withdrawExtraTokens(address token, uint amount)
        external
        onlyGovernanceOrOwner
    {
        if (token == stakeToken) {
