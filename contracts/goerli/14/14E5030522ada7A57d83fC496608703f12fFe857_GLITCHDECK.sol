...
/**  FINAL GOERLI TEST: 1025pm, 2-11-23...
    .------..------..------..------..------..------.             
    |G.--. ||L.--. ||I.--. ||T.--. ||C.--. ||H.--. |             
    | :/\: || :/\: || (\/) || :/\: || :/\: || :/\: |             
    | :\/: || (__) || :\/: || (__) || :\/: || (__) |             
...
GLITCH DECK NFT is a pioneering glitch-art project by Jim Dee of GenerativeNFTs.io (@SwiggaJuice on Twitter). It consists of 8,100 playing-card-themed, ANIMATED glitch-art NFTs with regular and dynamic traits (meaning, even when two NFTs have the same trait, they may appear differently on each piece, giving even more uniqueness to each token than normal generative sets). Of the 8,100 tokens: 2,699 are available free (plus gas) for any Deck of Degeneracy (DoD) NFT holder (each DoD token may claim one Glitch Deck token until Feb 14, 2024), 100 will be held back in the project wallet, and 5,301 are available as public mints. DoD holders have one year to claim their free token(s). After that, any unclaimed reserves will be reverted to the public mint pool and sold.

As a bonus for DoD holders:  40% of the primary mint proceeds will be stored in this wallet:  0xa7CfE42834468cedbD986D58950700E84AEE59A0. The funds that accrue there, provided they meet levels discussed at GlitchDeck.xyz, will be sent out equally among all holders of DoD as of a snapshot taken at 9:51pm PST on February 11, 2023. Detailed info on how that works is available on the web site, but the goal is to sell out this (awesome) set, get cool glitch-art collectibles into DoD hands, and possibly distribute approximately $100k in ETH (current value) equally among the 512 current holders in the snapshot. 
...
contract GLITCHDECK is Ownable, ERC721A, PaymentSplitter, DefaultOperatorFilterer {
    ...
    uint public promoPrice = 0.035 ether; // initial promo price if offered via affiliates
    string public provenanceHash;  // Gonna set this post-launch, as I have 90GB of NFTs still generating!
    uint public reservedNFTs;
    bool public saleIsLive = true;
    bool public claimPeriodOpen = true;
    ...
    // Deployer wallet reserves the 2,699 free-claimable NFTs for one year.
    // After Feb 14, 2024, any left will be returned to the main supply
    // and made available for public mint.
    address public claimHoldingWallet = 0x1489Aa1854b9920529A275BF9e2615366a5488Df; // GD DEPLOYER!

    // DoD ERC721Enumerable contract address:
    // 0x6128aC22b13C03B98d150c2AdC775ACf137f57fE; // GOERLI   
    // 0x8b3c69abc8c588e3eceb2eedf3bccf8fea950241; // MAINNET
    address public ERC721_contract = 0x6128aC22b13C03B98d150c2AdC775ACf137f57fE; // GOERLI
        ...
        PaymentSplitter(distro, distro_shares)
    {
        ...
        accounts[teamclaim[0]] = Account(2699, 0, 3); // Deployer (DoD) -- holds funds / claimables until 2-14-24.
        accounts[teamclaim[1]] = Account(  50, 0, 1); // Deployer wallet (team).
        accounts[teamclaim[2]] = Account(  25, 0, 1); // Founder (J) hold-backs for future use.
        accounts[teamclaim[3]] = Account(  25, 0, 1); // Founder (W) hold-backs for future use.
   ...
   // FREE CLAIM FUNCTIONALITY:

    function claimPeriodActivate() external minAdmin2 {
        claimPeriodOpen = true;
    }

    function claimPeriodDeactivate() external minAdmin2 {
    ...
    // Users can check this on the contract to see if a Gen1 ID is claimed.
    ...
    function claimFreeNFTs() external payable noReentrant {
        ...
        // I'm going to manually close the claim period on 2-14-24, as by then anyone
        // from DoD who wants one of these should have minted by then.
        require(claimPeriodOpen, "Error: Claim period closed Feb 14, 2024. Via claimFreeNFTs().");
        ...
        uint numMapped = 0;
        uint claimLimit = 125;  // limit # free mints to 125/tx, just for good measure.
        ...
        // Iterate through the list of owned tokens to arrive
        // at amount to mint and also to map claimed ones:
        for (uint i = 0; i < numTokens; i++) {
            ...
            if ( claimedIDList(tokenArray[i]) ) {
                ...
                // If the token is mapped already, then reduce availableClaims by one:
                availableClaims -= 1; 
        ...
        if ( availableClaims > 0 ) {
            ...
            // subject to tx limit
            if ( availableClaims > claimLimit ) { availableClaims == claimLimit; }

            // just in case...
            require(totalSupply() + availableClaims <= maxSupply, "Error: Claim would exceed the max supply.");
            ...
            // Reduce claimable quantity. (We are holding claimable
            // reserves in teamclaim[0], the deploy wallet.)
