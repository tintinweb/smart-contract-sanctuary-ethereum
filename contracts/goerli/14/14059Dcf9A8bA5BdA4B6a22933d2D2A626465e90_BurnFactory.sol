...
/// @title BurnFactory
/// @notice Hume Burn NFT Factory with additional ownership access
...
contract BurnFactory is Factory, Ownable {
    /// @inheritdoc Factory
        ...
        returns (address child_)
    {
        ...
        BurnNFTConfig memory config_ = abi.decode(
            data_,
            (BurnNFTConfig)
        );
        child_ = address(new BurnNFT(config_));
    }

    /// Typed wrapper around IFactory.createChild.
    function createChildTyped(BurnNFTConfig calldata config_)
        external
        onlyOwner
        returns (BurnNFT child_)
    {
        child_ = BurnNFT(
