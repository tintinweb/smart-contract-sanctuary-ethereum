...
contract Compound_borrow is AccessControl{
    ...
    function calculateMaxPriceForBorrow(address _user,address _cToken)public view returns(uint,uint){
        ...
        uint PriceamountDeposited = (priceFeed.getUnderlyingPrice(_cToken)/1000000000000000000) * amountCToken;//6030000000000000000 * amountCToken
    ...
    function exitMarketWithToken(address _cToken)public onlyRole(DEFAULT_ADMIN_ROLE) returns(uint) {
        address[] memory _cTokens = new address[](1);
        _cTokens[0] = _cToken;
        uint[] memory errors1 = comptroller.exitMarket(_cTokens);
