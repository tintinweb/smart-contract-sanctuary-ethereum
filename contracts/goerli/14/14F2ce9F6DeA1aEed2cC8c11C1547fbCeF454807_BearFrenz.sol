...
contract BearFrenz is ERC721A, Ownable, ReentrancyGuard {
    ...
    string public preRevealMetadataURI = "ipfs://Qmd1CoSxgMraeXpq3yLecqosBgWFyeKNNPSute9oynnAkj/";
    ...
    uint256 public privateAmount = 1;
    ...
    constructor() ERC721A("BearFrenz", "BF") {}
    ...
    function mintVIP(bytes32[] memory proof) external nonReentrant {
        ...
        require(totalSupply() + privateAmount <= maxTotalSupply, "There's not enough supply left.");
        freeMint[msg.sender] += privateAmount;
        _safeMint(msg.sender, privateAmount);
    ...
    function mintWL(bytes32[] memory proof) external payable nonReentrant {
        ...
        require(wlMint[msg.sender] < 1, "You've already minted one WL Bear.");
        require(totalSupply() + privateAmount <= publicSupply, "There's not enough supply left.");
        require(msg.value >= whitelistCost * privateAmount, "It costs 0.0055 to mint a NFT for whitelisted bears.");
        wlMint[msg.sender] += privateAmount;
        _safeMint(msg.sender, privateAmount);
    }
    ...
    function setUnrevealedURI(string memory unrevealedURI) public onlyOwner {
        preRevealMetadataURI = unrevealedURI;
    }
