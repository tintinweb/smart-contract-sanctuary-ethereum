...
contract B {
    ...
    struct user {
        ...
        mapping(string => uint) my_voted;
    }

    mapping(address => user) users;

    function setPoll(string memory _title, string memory _contents) public {
        Polls[_title] = poll(index++, _title, _contents, msg.sender, 0,0, Status.voting);
    ...
    function vote(string memory _title, bool _b) public {
        ...
        require(users[msg.sender].my_voted[_title] == 0);
        if(keccak256(bytes(Polls[_title].title)) == keccak256(bytes(_title))) { //title과 _title 비교, string은 직접 비교가 아닌 hash값으로
            ...
            if(_b == true) {
                ...
                users[msg.sender].my_voted[_title]++;
                if(Polls[_title].agree+Polls[_title].disag >= 10 && Polls[_title].agree > 7) {
                    Polls[_title].status = Status.pass;
                } else {
                    Polls[_title].status = Status.fail;
                } // voted는 string과 bool로 구성(key -value), _title을 key값으로 넣고 _b를 value 값으로 넣음.
            } else {
                Polls[_title].disag++;
