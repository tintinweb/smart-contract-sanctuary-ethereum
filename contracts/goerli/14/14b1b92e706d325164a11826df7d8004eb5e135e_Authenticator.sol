...
contract Authenticator is ERC721A, Ownable {
    ...
    event Verification(address wallet, string instagram, string linkedin, uint256 idToken);
    event Analyze(address wallet, string instagram, string linkedin);

    constructor() ERC721A("Authenticator", "AUTH") {
    ...
    function verifiedProfile(string memory uri, address payable endUser, string memory instagram, string memory linkedin) public onlyOwner payable {
        ...
        uris[newIdToken] = uri;
        emit Verification(endUser, instagram, linkedin, newIdToken);
        tokenURI(newIdToken);
    ...
    function analyzeProfile(address payable endUser, string memory instagram, string memory linkedin) public onlyOwner payable {
        uint256 _mintAmount = 1;
        uint256 supply = totalSupply();
        ...
        require(_mintAmount + balanceOf(endUser) <= maxMintAmount, "Quantidade limite de verificacao por wallet excedida");
        require(supply + _mintAmount <= maxSupply, "Quantidade limite de verificacao excedida");
        emit Analyze(endUser, instagram, linkedin);
    }
