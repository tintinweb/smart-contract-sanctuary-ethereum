...
/**
    ...
    @author boredGenius and 0xmons
 */
contract RSPairEnumerableETH is RSPairEnumerable, RSPairETH {
    /**
        @notice Returns the RSPair type
     */
    function pairVariant()
        ...
        returns (IRSPairFactoryLike.PairVariant)
    {
        return IRSPairFactoryLike.PairVariant.ENUMERABLE_ETH;
    }
}
...
import {ICurve} from "./bonding-curves/ICurve.sol";
import {RSPairERC20} from "./RSPairERC20.sol";
import {RSPairCloner} from "./lib/RSPairCloner.sol";
import {IRSPairFactoryLike} from "./IRSPairFactoryLike.sol";
import {RSPairEnumerableETH} from "./RSPairEnumerableETH.sol";
import {RSPairEnumerableERC20} from "./RSPairEnumerableERC20.sol";
import {RSPairMissingEnumerableETH} from "./RSPairMissingEnumerableETH.sol";
import {RSPairMissingEnumerableERC20} from "./RSPairMissingEnumerableERC20.sol";

contract RSPairFactory is Ownable, IRSPairFactoryLike {
    using RSPairCloner for address;
    using SafeTransferLib for address payable;
    using SafeTransferLib for ERC20;
    ...
    RSPairEnumerableETH public immutable enumerableETHTemplate;
    RSPairMissingEnumerableETH public immutable missingEnumerableETHTemplate;
    RSPairEnumerableERC20 public immutable enumerableERC20Template;
    RSPairMissingEnumerableERC20
    ...
    mapping(RSRouter => RouterStatus) public override routerStatus;
    ...
    event RouterStatusUpdate(RSRouter router, bool isAllowed);

    constructor(
        RSPairEnumerableETH _enumerableETHTemplate,
        RSPairMissingEnumerableETH _missingEnumerableETHTemplate,
        RSPairEnumerableERC20 _enumerableERC20Template,
        RSPairMissingEnumerableERC20 _missingEnumerableERC20Template,
    ...
    function createPairETH(
        ...
        ICurve _bondingCurve,
        address payable _assetRecipient,
        RSPair.PoolType _poolType,
        uint128 _delta,
        uint96 _fee,
        uint128 _spotPrice,
        uint256[] calldata _initialNFTIDs
    ) external payable returns (RSPairETH pair) {
        require(
    ...
    /**
        @notice Checks if an address is a RSPair. Uses the fact that the pairs are EIP-1167 minimal proxies.
        ...
        onlyOwner
    {
        ...
        if (isAllowed) {
            require(
                !routerStatus[RSRouter(target)].wasEverAllowed,
                "Can't call router"
            );
        }
    ...
    /**
        ...
        @param isAllowed True to whitelist, false to remove from whitelist
     */
    function setRouterAllowed(RSRouter _router, bool isAllowed)
        external
        onlyOwner
    {
    ...
    */
    ...
    function depositERC20(
        ...
        if (
            ...
            if (token == RSPairERC20(recipient).token()) {
...
contract RSPairMissingEnumerableERC20 is RSPairMissingEnumerable, RSPairERC20 {
    function pairVariant()
        public
        pure
        override
        returns (IRSPairFactoryLike.PairVariant)
    {
        return IRSPairFactoryLike.PairVariant.MISSING_ENUMERABLE_ERC20;
    }
}
...
import {RSPairCloner} from "../lib/RSPairCloner.sol";
import {RSPairERC20} from "../RSPairERC20.sol";
import {IRSPairFactoryLike} from "../RSPairFactory.sol";
...
library RSPairCloner {
    ...
    /**
     * @notice Checks if a contract is a clone of a RSPairERC20.
     * @dev Only checks the runtime bytecode, does not check the extra data.
     * @param implementation the RSPairERC20 implementation contract
     * @param query the contract to check
     * @return result True if the contract is a clone, false otherwise
     */
