...
abstract contract ERC721BurnableWithFinish is Ownable, ERC721Burnable {
...
contract Frozenlist is Issuable {
   ...
   * @param _operator address
   */
  modifier onlyIfNotFrozen(address _operator) virtual {
   ...
   * @param _operator address
   */
  function addAddressToFrozenlist(address _operator) public virtual onlyIssuer {
   ...
   * @param _operator address
   */
  function removeAddressFromFrozenlist(address _operator) public virtual onlyIssuer {
   ...
   * @param _operators addresses
   */
  function removeAddressesFromFrozenlist(address[] memory _operators) public virtual onlyIssuer {
    for (uint256 i = 0; i < _operators.length; i++) {
...
/**
* @dev The NokuCustomERC721LiteToken contract is a custom ERC721-compliant token available in the Noku Service Platform (NSP).
...
*/
contract NokuCustomERC721Lite is NokuCustomTokenLite, ERC721BurnableWithFinish, Frozenlist, Whitelist {
    using SafeMath for uint256;
    using Address for address;
    ...
    event LogNokuCustomERC721LiteCreated(
    ...
    */
    ...
    modifier onlyIfNotFrozen (address _operator) override {
        if (isAdvanced) {
            require(!hasRole(FROZENLISTED_ROLE, _operator), "Account frozen");
        }
        _;
    ...
    Whitelist(_enableWhitelist)
    {
        ...
        emit LogNokuCustomERC721LiteCreated(
            _msgSender(),
            _name,
            _symbol,
            _tokenBaseURI,
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        if(getStorageType() == StorageTypes.IPFS) {
            ...
            string memory partialPath = string(abi.encodePacked(tokenBaseURIValue, addressToString(address(this))));
            return string(abi.encodePacked(string(abi.encodePacked(partialPath, "/")), Strings.toString(_tokenId)));
        } else {
            return string(abi.encodePacked(tokenBaseURIValue, Strings.toString(_tokenId)));
        }
    }
    ...
    function updateTokenMetadataURI(uint256 _tokenId, string memory _tokenMetadataURI) public onlyOwner {
        require(_exists(_tokenId), "tokenId does not exist");
    ...
    function mint(uint256 _tokenId, address _to, string memory _tokenMetadataURI) public onlyOwner {
        _mint(_tokenId, _to, _tokenMetadataURI);
    }

    function multipleMint(uint256[] memory _tokensIds, address _to) public onlyOwner {
        _multipleMint(_tokensIds, _to, "");
    }

    function multipleMint(uint256[] memory _tokensIds, address _to, string memory _tokenMetadataURI) public onlyOwner {
    ...
    function safeMint(uint256 _tokenId, address _to, string memory _tokenMetadataURI) public onlyOwner {
        _safeMint(_tokenId, _to, _tokenMetadataURI, "");
    }

    function safeMint(uint256 _tokenId, address _to, string memory _tokenMetadataURI, bytes memory _data) public onlyOwner {
        _safeMint(_tokenId, _to, _tokenMetadataURI, _data);
    }

    function multipleSafeMint(uint256[] memory _tokensIds, address _to) public onlyOwner {
        _multipleSafeMint(_tokensIds, _to, "", "");
    }

    function multipleSafeMint(uint256[] memory _tokensIds, address _to, bytes memory _data) public onlyOwner {
        _multipleSafeMint(_tokensIds, _to, "", _data);
    }

    function multipleSafeMint(uint256[] memory _tokensIds, address _to, string memory _tokenMetadataURI) public onlyOwner {
        _multipleSafeMint(_tokensIds, _to, _tokenMetadataURI, "");
    }

    function multipleSafeMint(uint256[] memory _tokensIds, address _to, string memory _tokenMetadataURI, bytes memory _data) public onlyOwner {
        _multipleSafeMint(_tokensIds, _to, _tokenMetadataURI, _data);
    }

    function burn(uint256 _tokenId) public override canBurn canBurnToken(_tokenId) onlyIfNotFrozen(_msgSender()) {
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) public override onlyIfNotFrozen(_from) onlyIfNotFrozen(_to) onlyIfWhitelisted(_to) canTransfer(_tokenId) {
    ...
    function revoke(address _from, uint256 _tokenId) public onlyIssuer isERC721Advanced {
        _transfer(_from, _msgSender(), _tokenId);
    ...
    function addAddressToFrozenlist(address _operator) public override isERC721Advanced {
        super.addAddressToFrozenlist(_operator);
    }

    function removeAddressFromFrozenlist(address _operator) public override isERC721Advanced {
        super.removeAddressFromFrozenlist(_operator);
    }

    function removeAddressesFromFrozenlist(address[] memory _operators) public override isERC721Advanced {
        super.removeAddressesFromFrozenlist(_operators);
    }

    //Internal functions

    function _mint(uint256 _tokenId, address _to, string memory _tokenMetadataURI) canMint onlyIfNotFrozen(_to) onlyIfWhitelisted(_to) internal returns (uint256) {
    ...
    function _multipleMint(uint256[] memory _tokensIds, address _to, string memory _tokenMetadataURI) internal {
        for(uint256 i = 0; i < _tokensIds.length; i++) {
            _mint(_tokensIds[i], _to, _tokenMetadataURI);
        }
    }

    function _safeMint(uint256 _tokenId, address _to, string memory _tokenMetadataURI, bytes memory _data) internal returns (uint256) {
        _mint(_tokenId, _to, _tokenMetadataURI);
        require(
            _checkOnERC721ReceivedCustom(address(0), _to, _tokenId, _data),
            "ERC721: transfer to non ERC721Receiver implementer"
        );
        return _tokenId;
    }

    function _multipleSafeMint(uint256[] memory _tokensIds, address _to, string memory _tokenMetadataURI, bytes memory _data) internal {
        for(uint256 i = 0; i < _tokensIds.length; i++) {
            _safeMint(_tokensIds[i], _to, _tokenMetadataURI, _data);
        }
    }

    function _setTokenMetadataURI(uint256 _tokenId, string memory _tokenMetadataURI) internal {
    ...
    function addressToString(address _addr) internal pure returns (string memory) {
        return Strings.toHexString(uint256(uint160(_addr)));
    }

    /**
    * @dev implement supportsInterface(bytes4) using a lookup table
    */
    function supportsInterface(bytes4 _interfaceId) public view virtual override(ERC721, SupportsInterfaceWithLookup) returns (bool) {
     ...
     * @return bool whether the call correctly returned the expected magic value
     */
    function _checkOnERC721ReceivedCustom(
        address from,
        address to,
        uint256 tokenId,
        bytes memory _data
