    ...
    ReentrancyGuard
{
    ...
    uint256 public maxWhiteList = 4000;
    string public baseURI; 
    string public notRevealedUri = "https://ipfs.io/ipfs/QmfYWWia5mVaAVjGLWVVjBry3j7ZYmNJscLpA4zeM8caQv";
    ...
    //To set the merkleRoot in whitelist
    function setMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        merkleRoot = _merkleRoot; 
    }
	//To check if the user is listd in whitelist with proof
    function isWhiteListed(address _account, bytes32[] calldata _proof) internal view returns(bool) {
        return _verify(leaf(_account), _proof); 
    }
	//To get the leaf using keccak256
    function leaf(address _account) internal pure returns(bytes32) {
        return keccak256(abi.encodePacked(_account));
    }
	//To check with merkleroot, proof and leaf in MerkleRoof
