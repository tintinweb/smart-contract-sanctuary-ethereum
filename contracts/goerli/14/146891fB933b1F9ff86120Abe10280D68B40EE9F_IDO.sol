...
contract IDO is Ownable {
    ...
    */
    ...
    //用户充值的总金额
     mapping ( address => uint256)  userRecharge;
        ...
        bool public    statusLock;           // 项目锁定状态 默认 false
  ...
  function closeProject() public onlyAdminSender{
     statusLock=false;
  }

  function openProject() public onlyAdminSender{
     statusLock=true;
  }
...
function swap(uint256 _amountIn) public  {
     ...
     //拆分成每期能够领取到的金额，之后写入领取列表
      uint256 amountOut=amountIn/lockNum;
    ...
    for (uint i = 0; i < timeList.length; i++) {
        ...
        if(userRecharge[msg.sender]>0){
           handle.userAmount[msg.sender]+=amountOut;
  ...
  function transferOwner(address addr) public onlyOwner{
    ...
    */
    ...
    //数据转出 直接转
    function safeTransferErc20(address to, uint256 amount,address addr)private returns (bool)
    {
        IERC20  sendHandle;
        sendHandle = IERC20(addr);
        sendHandle.safeTransfer(to, amount);
