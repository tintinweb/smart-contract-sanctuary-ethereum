...
contract ZingArt_NFT721 is Base_ZingNFT_721 {
    using Strings for uint256;
    // mapping for toke dBIds
    mapping(uint256 => string) private _tokenDbIds;
    ...
    function _internalMint(
        ...
        _updateRoyalty(_newTokenId, _data.royaltyFee, _data.creator);
        return _newTokenId;
    }
    ...
    function _setTokenDbId(
        ...
        require(
            bytes(_tokenDbIds[tokenId]).length == 0,
            "ZingNFT_721: Token already minted"
        );

        _tokenDbIds[tokenId] = _dbId;
    }
    ...
    function tokenByDbId(uint256 _tokenId) public view returns (string memory) {
        return _tokenDbIds[_tokenId];
    }
