    ...
    AccessControl
{
    ...
    function stableMintSign(
        ...
        bytes32 r,
        bytes32 s
    ) external payable featureRequired(Feature.USDC_WITH_SIGN) lock {
        if (!_verify(signer(), _hash(msg.sender, maxAmount), signature))
            revert Errors.InvalidSignature();
    ...
    function stableMint(
        ...
        bytes32 r,
        bytes32 s
    ) external payable featureRequired(Feature.USDC_PUBLIC) lock {
        if (minted[msg.sender] + amount > mintsPerAccountOnPublicSale)
            revert Errors.AccountAlreadyMintedMax();
    ...
    function withdrawalUSDC(address to) external onlyOwner {
    ...
    function setStableMintPrice(uint256 stableMintPrice_)
        external
        onlyOwner
    ...
    function setPriceAggregator(AggregatorV3Interface priceAggregator_)
        external
        onlyOwner
    {
    ...
    function setPriceAggregatorStep(uint256 priceAggregatorStep_)
        external
        onlyOwner
    {
    ...
    function enabledFeatures() external view returns (Feature[] memory) {
        Feature[] memory features = new Feature[](5);
    ...
    function isAcceptableEthValue(uint256 value, uint256 amount)
        public
        view
        returns (bool)
    {
        ...
        while (true) {
            ...
            uint256 ethPrice = (1 ether * stableMintPrice * 100) / ethToUSDPrice;
