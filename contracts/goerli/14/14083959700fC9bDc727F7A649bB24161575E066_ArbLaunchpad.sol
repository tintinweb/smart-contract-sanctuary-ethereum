...
// File: contracts/ArbLaunchpad.sol
...
// ArbLaunchpad v1.0
contract ArbLaunchpad is Ownable, ReentrancyGuard {
  ...
  ) external payable onlyInProgress nonReentrant returns (bool) {
  ...
  function _releaseAmount(address forAddress, uint256 _amount) private returns (bool) {
    uint256 claimedAmount = claims[forAddress];
    require(claimedAmount > 0, "Nothing to release!");
    require(claimedAmount >= _amount, "Not enought claimedAmount!");
    ERC20(launchedToken).safeTransfer(forAddress, _amount);
    claims[forAddress] -= _amount;
    return true;
  }

  // Releases claim for a single address
  function releaseAmount(address forAddress, uint256 _amount) external onlyEnded onlyOwner nonReentrant returns (bool) {
    require(_releaseAmount(forAddress, _amount));
    return true;
  }

  // Releases claim for multiple addresses
  function multiReleaseAmount(
    address[] memory forAddresses,
    uint256[] memory amounts
  ) external onlyEnded onlyOwner nonReentrant returns (bool) {
