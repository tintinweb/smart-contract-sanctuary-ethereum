...
contract NFT is ERC721ABurnable, Ownable {
	...
	// || MINT LIMITS ||
	uint public PUBLIC_MAX_MINTS = 6;
	uint public WL_MAX_MINTS = 2;

	// mappings of batch - address - number minted
	mapping (uint256 => mapping (address => uint256)) public wlMintsOnAddress;	
	mapping (uint256 => mapping (address => uint256)) public publicMintsOnAddress;
	...
	event Upgraded(uint256 newToken, uint256 parent1, uint256 parent2);
	uint256 baseUpgradeCost = 1800 ether;
	/**
	 * @dev Keeps track of gen of each NFT because breeding won't be sequential
	 * 0 - Gen1
	 * 1 - Gen2
	 * 2 - Gen3
	 * 3 - Gen4
	 */
	mapping(uint256 => uint256) public nftGeneration;

	// || META CONFIG ||
	string private baseURI;
	string private contURI;

	constructor(address _whitelistAdmin) ERC721A("NFT_TITLE", "NFT_SYMBOL") { 
	...
	modifier nftOwner(uint256 tokenId) {
		require(ownerOf(tokenId) == msg.sender, "Cannot interact with a NFTs you do not own");
		_;
	}

	function _isvalidsig(bytes32 data, bytes memory signature, address signerAddress) private pure returns (bool) {
		return data.toEthSignedMessageHash().recover(signature) == signerAddress;
	}
	...
	// used to mint NFT #1 to allow Opensea setup before WL mints, and for airdrop if necessary
	function adminMint (address _address, uint256 _count) external onlyOwner {
		require(_totalMinted() + _count <= maxGenesis,"Purchase would exceed max supply of Genesis NFTs");
		require(_totalMinted() + _count <= maxBatch*mintingPhase,"Purchase would exceed max supply of NFTs in this phase.");
		_safeMint(_address, _count);
	}

	function privateMint(
		address _contractAddress, // address of this contract
		uint256 _count, //how many to mint
		...
		require(WLPrice * _count == msg.value,"Ether value sent is not correct");
		require(wlMintsOnAddress[mintingPhase][msg.sender] + _count <= WL_MAX_MINTS , "Already minted your max WL amount");
		wlMintsOnAddress[mintingPhase][msg.sender] += _count;
		require(_totalMinted() + _count <= maxGenesis,"Purchase would exceed max supply of Genesis NFTs");
		require(_totalMinted() + _count <= maxBatch*mintingPhase,"Purchase would exceed max supply of NFTs in this phase.");	
		
		require(_isvalidsig(keccak256(abi.encodePacked(_contractAddress, msg.sender)), _signature, whitelistAdmin), "Signature was not valid");

		if (_mintToStake){
			// mint directly to the staking contract and assign token to actual owner
			_safeMint(address(stakingContract),_count);
			stakingContract.stakeMint(_currentIndex-_count, msg.sender, _count);
		} else {
	...
	function publicMint(uint256 _count, bool _mintToStake, uint256 callerPublicSaleKey) external payable {
		require(isContract(msg.sender) == false, "Cannot mint from contract");
		...
		require(_totalMinted() + _count <= maxGenesis,"Purchase would exceed max supply of Genesis NFTs");
		require(_totalMinted() + _count <= maxBatch*mintingPhase,"Purchase would exceed max supply of NFTs in this phase.");	
		require(publicMintsOnAddress[mintingPhase][msg.sender] + wlMintsOnAddress[mintingPhase][msg.sender] + _count <= PUBLIC_MAX_MINTS, "Amount to be minted would exceed maximum per address.");
		require(PublicPrice * _count == msg.value,"Eth amount sent is not enough.");
		publicMintsOnAddress[mintingPhase][msg.sender] += _count;
		if (_mintToStake){
			_safeMint(address(stakingContract),_count);
			stakingContract.stakeMint(_currentIndex-_count, msg.sender, _count);
		} else {
	...
	// WARNING: CALLING THIS FUNCTION WILL PERMANENTLY DESTROY TWO NFTs
	function Upgrade(uint256 token1, uint256 token2) external nftOwner(token1) nftOwner(token2) {
		require(upgradesActive, "Upgrading is not currently active");
		require(nftGeneration[token1] == nftGeneration[token2], "Two NFTs of the same generation required to upgrade");
		require(nftGeneration[token1] < 3, "4th generation NFT cannot be upgraded.");
		uint burnprice = baseUpgradeCost;
		if (nftGeneration[token1] == 1) {
			burnprice = burnprice*12/5;
		}
		if (nftGeneration[token1] == 2) {
			burnprice = burnprice*6;
		}
		coinContract.burn(msg.sender,burnprice);
		_burn(token1);
		_burn(token2);

		// record the generation of the baby, required for staking and breeding 
		nftGeneration[_currentIndex] = nftGeneration[token1]+1;
		_safeMint(msg.sender, 1);
		emit Upgraded(_currentIndex-1, token1, token2);
	}
	...
	function togglePublicSale(bool state) external onlyOwner {
		publicSaleActive = state;
	}

	function setPublicSaleKey(uint256 saleKey) external onlyOwner {
	...
	function setPublicMaxMints(uint256 newPublicMaxMints) external onlyOwner {
		PUBLIC_MAX_MINTS = newPublicMaxMints;
	}

	function setWLMaxMints(uint256 newWLMaxMints) external onlyOwner {
		WL_MAX_MINTS = newWLMaxMints;
	}
	...
	// Set $COIN contract
	function setCoinContract(address address_) external onlyOwner {
		coinContract = ICoin(address_);
	}	
