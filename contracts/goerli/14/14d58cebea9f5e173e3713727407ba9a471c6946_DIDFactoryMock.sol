...
abstract contract OwnableAndMasterableUpgradeable {
    uint256 private _initializer;
    address private _owner;
...
import {Msg} from "../utils/Msg.sol";
import {OwnableAndMasterableUpgradeable} from "../access/OwnableAndMasterableUpgradeable.sol";

abstract contract DID is Initializable, OwnableAndMasterableUpgradeable, UUPSUpgradeable {
    modifier forwardCheckOwner(address destination, bytes memory data, uint256 value) {
    ...
    function forwardOwner(address destination, bytes memory data, uint256 value, uint256 gas)
        external
        virtual
        onlyOwner
        forwardCheckOwner(destination, data, value)
        returns (bytes memory)
    {
        return _forward(destination, data, value, gas);
    }

    function forwardMaster(address destination, bytes memory data, uint256 value, uint256 gas)
        external
        virtual
        onlyMaster
        returns (bytes memory)
    ...
    function _forward(address destination, bytes memory data, uint256 value, uint256 gas)
        internal
        returns (bytes memory)
    {
        // solhint-disable-next-line avoid-low-level-calls
    ...
    function __DID_init(
        ...
        (address initialOwner, address initialMaster) =
            __DID_init_unchained(gnosisFactory, singleton, fallbackHandler, saltNonce, initialOwners, initialMasters);
    ...
    function __DID_init_unchained(
        ...
        msOwner =
            address(IGnosisSafeProxyFactory(gnosisFactory).createProxyWithNonce(singleton, ownerCalldata, saltNonce));
        msMaster =
            address(IGnosisSafeProxyFactory(gnosisFactory).createProxyWithNonce(singleton, masterCalldata, saltNonce));
    }
}
...
import {DIDMock} from "./DIDMock.sol";

contract DIDFactoryMock is Ownable2Step {
    event NewDID(address indexed did, address[] owners, address[] masters, uint256 saltNonce);

    address public implementation;
    mapping(address => bool) public isDID;

    constructor() {
        implementation = address(new DIDMock());
    }
    ...
    function createDID(
        ...
        return _createDID(_owners, _masters, gnosisProxyFactory, singleton, fallbackHandler, saltNonce);
    }
    ...
    function _createDID(
        ...
        address fallbackHandler,
        uint256 saltNonce
    ) private returns (address did) {
        bytes memory initializeCalldata = abi.encodeWithSelector(
            DIDMock.initialize.selector, gnosisProxyFactory, singleton, fallbackHandler, saltNonce, _owners, _masters
        );
        did = _createProxy(implementation, initializeCalldata);
        isDID[did] = true;
        emit NewDID(did, _owners, _masters, saltNonce);
        return did;
    }
...
import {DID} from "src/contracts-upgradeable/dids/DID.sol";

contract DIDMock is DID {
    ...
    function initialize(
        ...
        __DIDMock_init(gnosisFactory, singleton, fallbackHandler, saltNonce, initialOwners, initialMasters);
    }

    // solhint-disable-next-line
    function __DIDMock_init(
        address gnosisFactory,
        address singleton,
        address fallbackHandler,
        uint256 saltNonce,
