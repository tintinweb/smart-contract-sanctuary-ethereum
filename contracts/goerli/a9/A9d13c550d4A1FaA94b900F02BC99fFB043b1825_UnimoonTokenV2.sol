...
// File contracts/UnimoonTokenV2.sol
...
contract UnimoonTokenV2 is ERC20Upgradeable, OwnableUpgradeable {
    ...
    /** @dev one-time used method instead of constructor
     * @notice available initializer only
     * @param _holders an array of swap contract, airdrop wallet, team wallet, owner wallet addresses
     * @param _amounts  an array of swap contract, airdrop wallet, team wallet, owner wallet mint amounts
     * @param _factory UniswapV2 factory address
     * @param _threshold to swap Umoon earned fees automatically
     * @param _usdc token address
     * @param _antibot initial configuration
     */
    function initialize(
        address[4] memory _holders,
        ...
        __ERC20_init("Unimoon", "Umoon");
        __Ownable_init();

        require(
            _factory != address(0) && _usdc != address(0),
        ...
        uint256 sum;
        isExcludedFromFee[_holders[3]] = true;
        sum += INITIAL_SUPPLY / 2;
        _mint(_holders[3], sum);
        _burn(_holders[3], sum);
        for (uint8 i; i < 4; i++) {
            sum += _amounts[i];
            _mint(_holders[i], _amounts[i]);
        }
        require(sum == INITIAL_SUPPLY, "UnimoonToken: 2 billion");
    }
    ...
    /** @dev Function to burn Unimoon tokens
