...
contract Notarizer is Ownable {
    ...
    function notarizeBatch(
        Content[5] memory contentTest,
        string memory _tag
    ) public onlyWhitelistedNotaries {
        for (uint8 i = 0; i < (contentTest.length); i++) {
            notarizeCID(
                contentTest[i].contentId,
                contentTest[i].ipfsHash,
                _tag
            );
        }
        countBatches++;
        emit e_notarizeBatch(msg.sender, countBatches, _tag);
    }
