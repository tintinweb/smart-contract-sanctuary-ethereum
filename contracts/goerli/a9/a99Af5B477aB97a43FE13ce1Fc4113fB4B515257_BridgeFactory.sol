...
contract BridgeFactory {
    ...
    function updateProjectWithoutApproving(
        ...
        Project storage projectStruct = projects[_projectId];
        require(projectStruct.status == PROJECTSTATUS.PENDING, "Not in pending phase");
        projectStruct.projectAddress = _projectAddress;
        projectStruct.detailsHash = _detailsHash;

        emit NewProjectAdded(_projectId, _projectAddress, _detailsHash);
    }
    ...
    function updateProjectHash(uint256 _projectId, string memory _newHash) public {
        ...
        require(msg.sender == companies[companyID].admin || msg.sender == companies[companyID].wallet, "Incorrect permissions");
