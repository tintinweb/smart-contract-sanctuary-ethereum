...
contract EntityCoreFacet is EntityFacetBase, IEntityCoreFacet, IDiamondFacet {
    ...
    ) external override assertPolicyCreationEnabled {
        require(IAccessControl(_unitAndTreasuryAndStakeholders[3]).aclContext() == aclContext(), "underwriter ACL context must match");

        Policy f = new Policy(_id, address(settings()), msg.sender, _typeAndDatesAndCommissionsBP, _unitAndTreasuryAndStakeholders);
    ...
    function payTranchePremium(
        ...
        uint256 _amount
    ) external override assertCanPayTranchePremiums(_policy) {
    ...
    function updateAllowPolicy(bool _allow) external override assertIsSystemManager(msg.sender) {
