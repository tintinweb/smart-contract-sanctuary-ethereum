...
/**
 * @title ABakery
 * @dev Purchase freshly baked Pastries
...
contract ABakery {
    ...
    uint public constant NUMBER_OF_FLAVORS = 4;
    ...
    function _flavorExists(string calldata _flavor) private view returns(bool) {
        for (uint i = 1; i < flavors.length; i++) {
    ...
    /**
     * @dev Get the flavors of the day
     * @return The flavors to choose from
     */
    function getFlavors() public view returns (string memory) {
        return flavors[0];
    }
    ...
    function pickupOrderedPastry() public view returns (string memory) {
        ...
        // A view function can not change state, so (a feature) the customer can receive the ordered pastry indefinitely
    ...
    function orderPastry(string calldata flavor) public payable {
        ...
        oven[msg.sender] = pastry_of_the_day;
    }
}