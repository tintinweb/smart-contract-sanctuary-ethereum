...
contract IncomeRouter is Ownable {
    ...
    /// tracks which NFR should be paid next
    mapping(uint256 => uint256) private s_waterfall;
    ...
    function share(
        ...
        if (s_nfrToBePaid == 0) {
            ...
            while (
                s_waterfall[currNFRId] > 0 &&
                s_nfrs[s_waterfall[currNFRId]].maturityDate <= maturityDate
            ) {
                currNFRId = s_waterfall[currNFRId];
            }

            // If at the end of the waterfall
            if (s_waterfall[currNFRId] == 0) {
                s_waterfall[currNFRId] = nfrId;
            } else {
                // Reorder waterfall as the new NFR being minted needs to be
                // inserted in the middle.
                s_waterfall[nfrId] = s_waterfall[currNFRId];
                s_waterfall[currNFRId] = nfrId;
            }
        }
    ...
    function withdrawFromLatestNFR() external {
        ...
        uint256 withdrawableAmount = getNFRWithdrawableAmount(latestNFRId);
        ...
        if (nfrData.balance == 0) {
            s_nfrToBePaid = s_waterfall[latestNFRId];
        }
    ...
    function withdrawIncome(
        ...
        IERC20(i_token).transfer(feeRecipient, feeRepaymentAmount);
        IERC20(i_token).transfer(
            recipient,
    ...
    function getOwnerWithdrawableAmount() public view returns (uint256) {
        uint256 currNFRId = s_nfrToBePaid;
        uint256 owedBalance = getNFRWithdrawableAmount(currNFRId);
        while (
            s_waterfall[currNFRId] > 0 &&
            s_nfrs[s_waterfall[currNFRId]].maturityDate <= block.timestamp
        ) {
            currNFRId = s_waterfall[currNFRId];
    ...
    function getNFRPriority(uint256 nfrId) external view returns (uint256) {
        uint256 currId = s_nfrToBePaid;
        uint256 priority;
        while (nfrId != currId) {
            currId = s_waterfall[currId];
            priority++;
        }
        return priority;
    }
...
library NFRLib {
    ...
    function _getImage(
        ...
        string memory start = '<svg width="3000" height="3000" viewBox="0 0 3000 3000">'
        "<defs>"
        "<style>"
...
contract Pool is IPool, ERC721, Ownable2Step {
    ...
    function withdrawFromIncomeRouter(address routerAddr) external override {
        if (routerAddr == address(0)) revert ZeroAddress();
        if (s_ownedRouters[msg.sender] == address(0)) revert RouterNotFound();
