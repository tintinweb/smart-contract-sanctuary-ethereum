...
import '../core/interfaces/ITangleswapPool.sol';
import '../core/interfaces/callback/ITangleswapSwapCallback.sol';
...
contract QuoterV2 is IQuoterV2, ITangleswapSwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using PoolTicksCounter for ITangleswapPool;
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (ITangleswapPool) {
        return ITangleswapPool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }

    /// @inheritdoc ITangleswapSwapCallback
    function tangleswapSwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
        bytes memory path
        ...
        ITangleswapPool pool = getPool(tokenIn, tokenOut, fee);
        ...
        )
    {
        ...
        ITangleswapPool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
...
/// @title Callback for ITangleswapPoolActions#swap
/// @notice Any contract that calls ITangleswapPoolActions#swap must implement this interface
interface ITangleswapSwapCallback {
    /// @notice Called to `msg.sender` after executing a swap via ITangleswapPool#swap.
    ...
    /// @param data Any data passed through by the caller via the ITangleswapPoolActions#swap call
    function tangleswapSwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
        bytes calldata data
...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH = 0xf329477416900778e2b67f380ba1bca69ff83acd3304d76d4cbb7105711f5c96;
    ...
    /// @param factory The Tangleswap factory contract address
...
/// @notice Provides validation for callbacks from Tangleswap Pools
library CallbackValidation {
    /// @notice Returns the address of a valid Tangleswap Pool
    /// @param factory The contract address of the Tangleswap factory
    ...
    function verifyCallback(
        ...
        address tokenB,
        uint24 fee
    ) internal view returns (ITangleswapPool pool) {
    ...
    function verifyCallback(address factory, PoolAddress.PoolKey memory poolKey)
        internal
        view
        returns (ITangleswapPool pool)
    {
        pool = ITangleswapPool(PoolAddress.computeAddress(factory, poolKey));
