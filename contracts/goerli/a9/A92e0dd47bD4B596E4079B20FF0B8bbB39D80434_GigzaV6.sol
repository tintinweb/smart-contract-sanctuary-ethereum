...
contract GigzaV6 is Initializable, OwnableUpgradeable {
    ...
    function submitProposal(
       ...
       for (uint256 i = 0; i < proposals.length; i++ ){
        ...
        emit ProposalSubmitted(jobId, description, job.client, bid.freelancer);
    } 
    ...
    function sendContract(uint256 jobId, address user) external  returns (Bid[] memory) {
       ...
       emit ContractSent(jobId, job.client, job.freelancer);
       return proposals;
    }
    ...
    function acceptContract(uint256 jobId) external  returns (Bid memory) {
       ...
       for (uint256 i = 0; i < proposals.length; i++ ){
            if(msg.sender == proposals[i].freelancer){
                ...
                emit ContractAccepted(jobId, job.client, job.freelancer);
                return proposals[i];
            }
       }
       revert();  
    ...
    function submitJob(uint256 jobId, string memory message, string memory url) external  returns (Job memory) {
       ...
       emit JobCompleted(jobId, job.message, job.client, job.freelancer);
       return job;
    }   
    ...
    function releasePayment(uint256 jobId) external  returns (Job memory) {
       ...
       for (uint256 i = 0; i < proposals.length; i++ ){
            if(job.freelancer == proposals[i].freelancer){
                ...
                balance -= job.amount;
                emit PayementReleased (jobId, amount, job.client, job.freelancer);
            }
       }
       return job;
    }   
     ...
     function withdrawOffer(uint256 jobId) external  returns (Bid[] memory) {
       ...
       emit OfferWithDrawn(jobId, job.amount, job.client, job.freelancer);
       return proposals;
    }  
    ...
    function cancelJob(uint256 jobId) external  returns (Job[] memory) {
       ...
       balance -= job.amount;  
       emit JobCancelled(jobId, job.amount, job.title, job.client, job.freelancer); 
       return jobs;
    } 
    ...
    function declineOffer(uint256 jobId) external  returns (Job memory) {
       ...
       emit OfferDeclined(jobId, job.amount, job.title, job.client, job.freelancer); 
       return job;
    }
    ...
    function openDispute(uint256 jobId) external  returns (Job memory) {
       ...
       emit DisputeOpened(jobId, job.amount, job.title, job.client, job.freelancer);
       return job;
    } 
    
    // Manually Done on Dispute Resolution
    ...
    function createProfile (
        ...
        emit ProfileCreated(msg.sender, user.name, user.mainSkill, block.timestamp);
    }
    ...
    function editProfile (
        ...
        emit ProfileEdited(msg.sender, block.timestamp);
    } 
    ...
    function createReview(
        ...
        emit ReviewCreated(msg.sender, review, rating, block.timestamp);
    }
    ...
    **/
    ...
    event ProposalSubmitted(uint256 jobId, string description, address indexed client, address indexed freelancer);
    event ContractSent(uint256 jobId, address indexed client, address indexed freelancer);
    event ContractAccepted(uint256 jobId, address indexed client, address indexed freelancer);
    event JobCompleted(uint256 jobId, string message, address indexed client, address indexed freelancer);
    event PayementReleased(uint256 jobId, uint256 amount, address indexed client, address indexed freelancer);
    event OfferWithDrawn(uint256 jobId, uint256 amount, address indexed client, address indexed freelancer);
    event JobCancelled(uint256 jobId, uint256 amount, string title, address indexed client, address indexed freelancer);
    event OfferDeclined(uint256 jobId, uint256 amount, string title, address indexed client, address indexed freelancer);
    event DisputeOpened(uint256 jobId, uint256 amount, string title, address indexed client, address indexed freelancer);
    event ProfileCreated(address indexed user, string name, string skill, uint256 timestamp);
    event ProfileEdited(address indexed user, uint256 timestamp);
    event ReviewCreated(address indexed client, string meassge, uint256 rating, uint256 timestamp);
