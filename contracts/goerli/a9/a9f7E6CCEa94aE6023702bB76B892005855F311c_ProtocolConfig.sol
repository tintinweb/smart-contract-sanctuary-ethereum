...
contract ProtocolConfig is IProtocolConfig, Initializable, BACUpgradable {
    uint16 public version; // protocol version
    ...
    mapping(address => MarketConfig) public marketConfigs; // protocol settings for every token
    mapping(address => address) public bTokens; // bToken addresses mapping
    mapping(address => address) public bNFTMaker; // bToken addresses mapping
    mapping(address => address) public bNFTTaker; // bToken addresses mapping

    mapping(address => address) public markets; // IMarket, IMarketPositionTransfer implementation for each market

    /// @notice Initialize using upgradable pattern
    function initialize(address _bac) external initializer {
        _setGlobalAccessController(_bac);
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    /// @notice Get global access controller address
    function getGAC() public view override returns (address) {
        return address(bac);
    }
    ...
    {
        return marketConfigs[token];
    }

    function getPeriodIndex(uint16 period) public pure returns (uint16) {
        // 30,60,90,150,240
        if (period == 30) return 0;
        if (period == 60) return 1;
        if (period == 90) return 2;
        if (period == 150) return 3;
        if (period == 240) return 4;
        return 10;
    }

    function getRiskIndex(uint16 risk) public pure returns (uint16) {
        // (5,95), (10,90), (15,85), (20,80), (30,70)
        if (risk == 500 || risk == 9500) return 0;
        if (risk == 1000 || risk == 9000) return 1;
        if (risk == 1500 || risk == 8500) return 2;
        if (risk == 2000 || risk == 8000) return 3;
        if (risk == 3000 || risk == 8500) return 4;
        return 10;
    }

    /// @notice Set market configuration
    function setConfig(address token, MarketConfig memory config)
        external
        onlyGovernance
    {
    ...
    function setU_Lambda(address token, int128[4] calldata lambda)
        external
        onlyGovernance
    {
    ...
    /// @notice Set market configuration parameter
    function setU_Ref(address token, int128[4] calldata ref)
        external
        onlyGovernance
    {
    ...
    /// @notice Set market configuration parameter
    function setW_Lambda(address token, int128[6] calldata lambda)
        external
        onlyGovernance
    {
    ...
    /// @notice Set market configuration parameter
    function setLambdaGamma(address token, int128 lambda)
        external
        onlyGovernance
    {
    ...
    /// @notice Set market configuration parameter
    function setLambdaDelta(address token, int128 lambda)
        external
        onlyGovernance
    {
    ...
    /// @notice Set network fee
    function setFee(
        address token,
        int128 takerFee,
        int128 makerFee
    ) external onlyGovernance {
        marketConfigs[token].takerFee = takerFee;
        marketConfigs[token].makerFee = makerFee;
    }

    /// @notice Set market configuration parameter
    function setYieldMultipliers(address token, int128[5][5] calldata mul)
        external
        onlyGovernance
    {
        marketConfigs[token].Yield_Mul = mul;
    }
    ...
    /// @notice Set market NFT tokens
    function setNFT(address token, address taker, address maker) external onlyGovernance {
        bNFTMaker[token] = maker;
        bNFTTaker[token] = taker;
    }
    ...
    /// @notice Get address of B-token for given token
    function getNFTMaker(address token) public view override returns (address){
        return bNFTMaker[token];
    }

        /// @notice Get address of B-token for given token
    function getNFTTaker(address token) public view override returns (address){
        return bNFTTaker[token];
    }
    /// @notice Set market bToken for token
