...
interface IExtendedResolver {
    function resolve(bytes memory name, bytes memory data) external view returns(bytes memory, bytes memory);
}
    ...
    IExtendedResolver
{
    ...
    function resolve(bytes calldata name, bytes calldata data) external override view returns(bytes memory, bytes memory) {
