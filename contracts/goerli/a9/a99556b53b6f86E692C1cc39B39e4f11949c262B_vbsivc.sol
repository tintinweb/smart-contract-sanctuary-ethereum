...
contract vbsivc is ERC721A, Ownable, ReentrancyGuard, Minter {

    address private tresureyaddy;
    bool public fuseEnabled;
    uint256 public numberOfToken;
    uint256 public maxMint = 63;
    uint256 public price = 630 ether;
    uint256 public maxToken = 6300;
    ...
    constructor () ERC721A ("VICEca","VG", maxMint, maxToken) {
        numberOfToken = 0;
    }

    function ownerMint(uint256 _amount, address _address) public onlyOwner { 
        require(_amount + numberOfToken <= maxToken, "No more NFTs");
        numberOfToken += _amount;
        _safeMint(_address, _amount);
    }

    function fusion(uint256 _nsmo, uint256 _nsmt, uint256 _amount) public payable onlySender {
        uint256 totaltran = price * _amount;
        require(_amount + numberOfToken <= maxToken, "No more NFTs");
        require(fuseEnabled, "Fuse Paused");
        ...
        require(YieldToken.balanceOf(msg.sender) >= totaltran, "No enough token");
    ...
    function setFuse(bool bool_) external onlyOwner {
        fuseEnabled = bool_;
    }
