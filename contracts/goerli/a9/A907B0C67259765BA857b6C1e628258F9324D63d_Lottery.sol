...
contract Lottery is VRFConsumerBaseV2 {
    ...
    uint16 requestConfirmations = 3; //節點確認數，越高，花的時間越久
    uint32 numWords =  1; //請求一個隨機數

    uint256 private _lotterNumber;

    uint256 public _mintBet = 3000 wei; // 投一注需要的金額
    uint256 public _minPick = 6000 wei; // 開獎需達成的金額，至少兩個人的意思
    uint256 public _expired = 4 minutes;

    uint256 public lotteryRound = 0; //第幾期的樂透
    address _owner;
    bool isLocked = false;
    ...
    */
    ...
    // 向預言機請求，取得隨機數
    function requestRandomWords(uint256 _lotteryId) external onlyOwner potChecker(_lotteryId)  notExpired(_lotteryId)
    {
        LotteryData storage lottery = lotterys[_lotteryId];
        // 驗證是否已經產生隨機數
