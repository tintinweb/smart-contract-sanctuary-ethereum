...
contract BatchClaimXEN {
    ...
    function queryClaimMintInfo(address query) external view returns (MintInfo[] memory mintInfos){
        uint N = countClaimRank[query];
        MintInfo[] memory mints = new MintInfo[](N);
        for (uint i = 0; i < N; i++) {
            address proxy = proxyFor(msg.sender, i);
            MintInfo memory _mif = BatchClaimXEN(proxy).userMint(proxy);
            mints[i] = _mif;
        }
        return mints;
    }
