...
contract LBSKR is IERC20, Ownable, Manageable, Pausable, DSMath {
    ...
    modifier isInitialRatioNotSet() {
        require(!initialRatioFlag, "IRS");
        _;
    }

    modifier isInitialRatioSet() {
        require(initialRatioFlag, "IRNS");
        _;
    }
    ...
    function callAndParseInt24Return(address token, bytes4 selector)
        private
        view
        returns (int24)
    {
    ...
    function isBSKRLBSKRV3Pool(address target) private returns (bool) {
        ...
        int24 targetTickSpacing = callAndParseInt24Return(
            target,
            hex"d0c93a7c"
        );
    ...
    function isUniswapV3Pool(address target) private returns (bool) {
        // TODO can be set to view
        address token0;
        address token1;
    ...
    function refundLockedBSKR(uint256 from, uint256 to) external onlyManager {
        // IT - Invalid `to` param
        require(to <= stakeholders.length(), "IR");
        creditInflation();
        uint256 s;
    ...
    function removeStake(uint256 stakeAmount) external whenNotPaused {
        ...
        require(stakeholderStake >= stakeAmount, "SL");
        ...
        returns (uint256)
    {
        ...
        require(stakeholderStake >= bskrAmount, "SL");
