...
contract InfinityBitTokenV2 is IERC20, Ownable {
    ...
    address payable private _marketingWallet = payable(0x6dE6cb16D59fD75Dc06b1E0a0d09c1Fd5bBA2c7c);
    address payable private _legacyMarketingWallet = payable(0xb9B23Cc434596ABa6f83d5C6662DD3Cf4B1e099C);

    // Developer Tax
    uint8 private _devTax = 18; // 1.8%
    address payable private _devWallet = payable(0x4Af89d72497F1F0a6e9D4779b28B499c6cD8d3D2);
    address payable private _legacyDevWallet = payable(0xe83842e599b9BC6951A265dC75c3745bA9B88201);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!isMaxWalletExempt(to) && _maxWalletEnabled)
        {
        ...
        // todo: only check this is _autoSwapTokens== true and isSell()==true
        uint256 swapThresholdAmountTokens = (getLiquidityIBIT() * _swapThreshold)/1000;

        if(_autoSwapTokens && isSell(from, to) && autoswapTotalTokensAvailable(amount) >= swapThresholdAmountTokens)
        {
            uint256 lp_tokens = _calcLpTaxAmount(amount)/2;
            uint256 autoswap_amount = lp_tokens + _calcDevTaxAmount(amount) + _calcMarketingTaxAmount(amount);

            if(autoswap_amount < getMaxAutoswap())
            {
                // Take tokens from marketing and dev wallets
                uint256 max_extra_autoswap = getMaxAutoswap()-autoswap_amount;
                autoswap_amount += _takeTokensFromMarketingAndDevWallets(max_extra_autoswap);
            }

            if(autoswap_amount > getMaxAutoswap())
            {
                autoswap_amount = getMaxAutoswap();
            }
            ...
            if(to == _uniswapV2PairAddress && from != _uniswapV2RouterAddress && from != _uniswapUniversalRouter && lp_tokens > 0)
            {
