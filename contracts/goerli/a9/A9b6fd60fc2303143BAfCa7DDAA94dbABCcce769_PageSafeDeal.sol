...
import "ICryptoPageSafeDeal.sol";
    ...
    IPageSafeDeal
{
    ...
    event SetGuarantorFee(uint256 amount);
    ...
    modifier notAddress0(address _address) {
        require(_address != address(0), "SafeDeal: Address 0 is not valid");
        _;
    }
     ...
     */
    function initialize(address _admin, address _calcUserRate, address _oracle) external initializer {
        __Ownable_init();
     ...
     */
    function setToken(address newToken) external override notAddress0(newToken) onlyOwner {
    ...
    function changeDescription(uint256 dealId, string memory desc) external override onlyDealUser(dealId) {
        require(!isStartApprovedByAnyone(dealId), "SafeDeal: the deal has already started");
    ...
    function makeEndApprove(uint256 dealId) external override onlyDealUser(dealId) {
        ...
        require(!isFinished(dealId) && !isIssue(dealId), "SafeDeal: no suitable conditions");
    ...
    function addMessage(uint256 dealId, string memory message) public override onlyDealUser(dealId) {
        require(!isFinished(dealId), "SafeDeal: already finished");
        address sender = _msgSender();
        DataTypes.SafeDeal storage deal = deals[dealId];
    ...
    function finishDeal(uint256 dealId) external override onlyGuarantorOrBuyer(dealId) {
        ...
        require(
            ...
            !isIssue(dealId) &&
            !isFinished(dealId),
            "SafeDeal: no suitable conditions"
        );
        uint256 amount = deal.amount;
        deal.isFinished = true;
    ...
    /**
     * @dev Sets GUARANTOR_FEE value.
     *
     * @param amount new GUARANTOR_FEE value
     */
    function setGuarantorFee(uint256 amount) external override onlyOwner {
        require(amount != GUARANTOR_FEE, "Invalid value for GUARANTOR_FEE");
        GUARANTOR_FEE = amount;
        emit SetGuarantorFee(amount);
    }
    ...
    /**
     * @dev Check if deal start is approved by both parties.
     *
     * @param dealId Deal ID
     */
    ...
    /**
     * @dev Check if deal start is approved by any of the  parties.
     *
     * @param dealId Deal ID
     */
    function isStartApprovedByAnyone(uint256 dealId) public view override returns(bool) {
        DataTypes.SafeDeal memory deal = deals[dealId];
        return deal.startSellerApprove || deal.startBuyerApprove;
    }
...
interface IPageSafeDeal {
    ...
    function setGuarantorFee(uint256 amount) external;
    ...
    function isStartApprovedByAnyone(uint256 dealId) external view returns(bool);
