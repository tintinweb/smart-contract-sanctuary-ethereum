...
/// @notice This implements the core swap logic from NFT (Urbit Star) to TOKEN
/// @dev The alterations for Urbitex include replacing generic NFT contract input with Urbit's Ecliptic contract.
///      Eclptic's address must be determined by checking Azimuth's owner, otherwise this would risk trapping
///      stars inside of a pair contract when an upgrade to Ecliptic occurs (changing its contract address).
///      Checks are also added to ensure that only stars may be transferred into a pair contract, and that those
///      stars do not have planets spawned. Since we cannot check if there are layer-2 planets spawned, we must
///      check that no spawn proxy exists for layer-2.
///      Not all functions are used at the time of deployment, however the changes are applied everywhere
///      in order to prevent execution by a malicious actor outside of the Urbitex UI.
    ...
    ERC1155Holder
{
    ...
    function swapTokenForAnyNFTs(
        ...
        // Input validation
        {
            ...
            require(
                (numNFTs > 0) && (numNFTs <= _ecliptic.balanceOf(address(this))),
        ...
        _sendAnyNFTsToRecipient(_ecliptic, nftRecipient, numNFTs);
    ...
    function swapTokenForSpecificNFTs(
        ...
        _sendSpecificNFTsToRecipient(ecliptic(), nftRecipient, nftIds);
    ...
    function swapNFTsForToken(
        ...
        _takeNFTsFromSender(ecliptic(), nftIds, _factory, isRouter, routerCaller);
    ...
    /**
        @notice // Returns the Azimuth contract from calldata
     */
    function azimuth() public pure returns (IAzimuth _azimuth) {
        uint256 paramsLength = _immutableParamsLength();        
        assembly {
            _azimuth := shr(
                0x60,
    ...
    /**
        @notice // Returns the Ecliptic contract by calling Azimuth's owner
     */
    function ecliptic() public view returns (IERC721 _ecliptic) {
        uint256 paramsLength = _immutableParamsLength();
        IAzimuth _azimuth;
        assembly {
            _azimuth := shr(
        ...
        _ecliptic = IERC721(_azimuth.owner());
    }   
    ...
    /**
        ...
        @return inputAmount The total amount of tokens to be sent
     */
    ...
    /**
        ...
        @param protocolFee The percentage of protocol fee to be taken, as a percentage
        @return protocolFee The amount of tokens to send as protocol fee
        @return outputAmount The total amount of tokens to receive
     */
