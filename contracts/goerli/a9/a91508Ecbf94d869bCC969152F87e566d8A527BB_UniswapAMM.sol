...
// ====================== UniswapAMM ========================
// ====================================================================
// Creates Uni v3 positions between Sweep and USDC
...
import "../Common/ERC20/SafeERC20.sol";
import "../Utils/Uniswap/V2/TransferHelper.sol";
import "../Common/Owned.sol";

import "../Utils/Uniswap/V3/IUniswapV3Factory.sol";
import "../Utils/Uniswap/V3/libraries/TickMath.sol";
import "../Utils/Uniswap/V3/libraries/LiquidityAmounts.sol";
import "../Utils/Uniswap/V3/periphery/interfaces/INonfungiblePositionManager.sol";
...
contract UniswapAMM is Owned {
    ...
    address uniswap_factory_address = 0x1F98431c8aD98523631AE4a59f267346ea31F984;
    address uniswap_position_address = 0xC36442b4a4522E871399CD717aBDD847Ab11FE88;
    ...
    // Array of all Uni v3 NFT positions held by this contract
    ...
    constructor(
        ...
        univ3_factory = IUniswapV3Factory(uniswap_factory_address);
        univ3_positions = INonfungiblePositionManager(uniswap_position_address);
        univ3_router = ISwapRouter(uniswap_router_address);

        // Get the treasury and timelock addresses from the minter
    ...
    modifier onlyByOwnGovCust() {
        require(msg.sender == timelock_address || msg.sender == owner || msg.sender == treasury_address, "Not owner, tlck, or treasury");
        _;
    }
    ...
    // Returns this contract's liquidity in a specific [SWEEP]-[USDC] uni v3 pool
    function liquidityInPool(address _collateral_address, int24 _tickLower, int24 _tickUpper, uint24 _fee) public view returns (uint128) {
        IUniswapV3Pool get_pool = IUniswapV3Pool(univ3_factory.getPool(address(SWEEP), _collateral_address, _fee));
    ...
    function addLiquidity(address _tokenA, address _tokenB, int24 _tickLower, int24 _tickUpper, uint24 _fee, uint256 _amount0Desired, uint256 _amount1Desired, uint256 _amount0Min, uint256 _amount1Min) public onlyByOwnGov {
        ...
        require(_tokenA == giveback_collateral_address || _tokenA == address(SWEEP), "TokenA not allowed");
        require(_tokenB == giveback_collateral_address || _tokenB == address(SWEEP), "TokenB not allowed");
        ...
        Position memory pos = Position(
            tokenId,
            _tokenA == address(SWEEP) ? _tokenB : _tokenA,
            amountLiquidity,
            _tickLower,
            _tickUpper,
            _fee
    ...
    */
    ...
    // Uni V3 only
    function swapExactInput(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 _amountIn, uint256 _amountOutMinimum) public returns (uint256 amountOut) {
        // Approval
    ...
    function swapExactOutput(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 _amountOut, uint256 _amountInMaximum) public returns(uint256 amountIn) {
        TransferHelper.safeTransferFrom(_tokenA, msg.sender, address(this), _amountInMaximum);
        TransferHelper.safeApprove(_tokenA, address(univ3_router), _amountInMaximum);
        ...
        amountIn = univ3_router.exactOutputSingle(params);

        if (amountIn < _amountInMaximum) {
            TransferHelper.safeApprove(_tokenA, address(univ3_router), 0);
            TransferHelper.safeTransfer(_tokenA, msg.sender, _amountInMaximum - amountIn);
        }
    }
...
library SafeMath {
    ...
    function _rmul(uint256 x, uint256 y, uint256 base) internal pure returns (uint256 z) {
        z = _mul(x, y) / base;
    }
