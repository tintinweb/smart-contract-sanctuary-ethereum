  ...
  ERC721BurnableUpgradeable
{
  ...
  modifier onlyOperator() {
    require(collectionFactory.rolesContract().isOperator(msg.sender), "CC: Caller is not an operator");
    _;
  }
  ...
  constructor(address _collectionFactory) {
    require(_collectionFactory.isContract(), "CC: collectionFactory is not a contract");
  ...
  function initialize(
    ...
    require(msg.sender == address(collectionFactory), "CC: Collection must be created via the factory");
  ...
  function updateMaxTokenId(uint256 _maxTokenId) external onlyOwner {
    require(_maxTokenId > 0, "CC: Max token ID may not be cleared");
    require(maxTokenId == 0 || _maxTokenId < maxTokenId, "CC: Max token ID may not increase");
    require(latestTokenId + 1 <= _maxTokenId, "CC: Max token ID must be greater than last mint");
  ...
  function selfDestruct() external onlyOwner {
    require(totalSupply() == 0, "CC: Any NFTs minted must be burned first");
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    require(_exists(tokenId), "CC: URI query for nonexistent token");
  ...
  function _mint(string memory tokenCID) private onlyOwner returns (uint256 tokenId) {
    require(bytes(tokenCID).length > 0, "CC: tokenCID is required");
    require(!cidToMinted[tokenCID], "CC: NFT was already minted");
    unchecked {
      tokenId = ++latestTokenId;
      require(maxTokenId == 0 || tokenId <= maxTokenId, "CC: Max token count has already been minted");
