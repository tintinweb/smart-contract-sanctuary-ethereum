...
import "../../interfaces/IZonePriceOracle.sol";
import "../../libs/Const.sol";
import "../constant/EthConstantTest.sol";
import "./PriceOracle.sol";

contract EthPriceOracleTest is PriceOracle {

    IZonePriceOracle constant zonePriceOracle = IZonePriceOracle(0xDcF7C8664c2191589C5170bF73c6faC5c7cC7bd6);

    function initialize(address _owner) public override initializer {
        __Ownable_init();

        address[] memory assets = new address[](1);
        assets[0] = EthConstantTest.WETH;
        address[] memory sources = new address[](1);
        sources[0] = 0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e;
    ...
    function getAssetPrice(address asset) public virtual override view returns (uint price, uint8 decimals) {
        ...
        } else if (asset == EthConstantTest.ZONE) {
            return getZONEPrice();
        }
        return super.getAssetPrice(asset);
    }

    function getZONEPrice() public view returns (uint price, uint8 decimals) {
        uint ethPriceInZONE = zonePriceOracle.mintPriceInZone(1e18);
        (uint ethPriceInUSD, uint8 priceDecimals) = super.getAssetPrice(EthConstantTest.WETH);
        return (ethPriceInUSD * 1e18 / ethPriceInZONE, priceDecimals);
    }
...
interface IZonePriceOracle {
    function zoneToken() external view returns(address);
    function lpZoneEth() external view returns(IUniPair);
