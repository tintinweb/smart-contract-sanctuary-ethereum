...
contract DEXLimitOrder is ReentrancyGuard {
	...
	*/
    ...
    address wethAddress = 0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6; //Goerli
    // address wethAddress = 0xc778417E063141139Fce010982780140Aa0cD5Ab; //Ropsten
    ...
    * @dev List limit order for token
    ...
    {
        require(inAmt > 0 && outAmt > 0, 'Enter valid input and output amt');
        _safeTransferFrom(inTokenAddr, msg.sender, address(this), inAmt);
        timeStamp = orderType == 1 ? timeStamp : orderType == 2 ? maxTime : block.timestamp + 1 days;
    ...
    * @dev List limit order for eth swap
    ...
    nonReentrant payable 
    {
        require(msg.value > 0 && outAmt > 0, 'Enter valid amt');
        timeStamp = orderType == 1 ? timeStamp : orderType == 2 ? maxTime : block.timestamp + 1 days;
        LimitOrder memory limitOrder = LimitOrder(
            wethAddress,
            outTokenAddr,
    ...
    * @dev able to cancel listing if swap not settled
    ...
    nonReentrant payable 
	{
        ...
        if (limitOrder.isInEth == true) {
            (bool sent,) = limitOrder.owner.call{value: amt}("");
            require(sent, "Failed to send ether");
        }
        else SafeERC20.safeTransfer(IERC20(limitOrder.inTokenAddr), msg.sender, amt);
    ...
    * @dev Execute swap for the called ids
    ...
    nonReentrant payable 
    {
        for (uint i = 0; i < limitOrderIds.length; i++) {
            ...
            require(limitOrder.limitOrderStatus == false, 'Already Executed!');
            require(limitOrder.outTokenAddr != address(0) 
                && limitOrder.inTokenAddr != address(0), 'Order does not exist!');
            if(limitOrder.timeStamp > block.timestamp) {
                if(limitOrder.inBoundToken == true) {
                    ...
                    if(amounts[amounts.length - 1] >= limitOrder.boundToken) {
                        ...
                        else if(limitOrder.isOutEth) {
                        SafeERC20.safeApprove(IERC20(limitOrder.inTokenAddr), _addr, limitOrder.tokenAmt);
                    ...
                    if(amounts[0] <= limitOrder.boundToken) {
                        if(limitOrder.isInEth) {
                            ...
                            block.timestamp+600)
                                ...
                                if(!success) emit LimitOrderNotExecuted(limitOrderIds[i]);
                                    ...
                                    uint256 amountRemaining = limitOrder.boundToken - amountUsed[0];
                            ...
                            block.timestamp+600)
                                ...
                                if(!success) emit LimitOrderNotExecuted(limitOrderIds[i]);
                                    ...
                                    (uint256[] memory amountUsed) = abi.decode(data, (uint[]));
                                    uint256 amountRemaining = limitOrder.boundToken - amountUsed[0];
                                    if(amountRemaining > 0) IERC20(limitOrder.inTokenAddr).safeTransfer(limitOrder.owner, amountRemaining);
                                    limitOrder.limitOrderStatus = true;
                                    emit LimitOrderExecuted(limitOrderIds[i]);
                                }
                        }
