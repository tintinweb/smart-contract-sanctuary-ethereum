    ...
    ERC721AUpgradeable
{
    ...
    function batchmint(address _wallet, uint NFTamount, string memory _tokenURI) public onlyOwner {
    ...
    function batchmint(address _wallet, uint NFTamount, string memory _tokenURI, uint collectionID) public onlyOwner {
            uint256 tokenId = _tokenIdCounter.current();
            emit MintedNFT( tokenId, block.timestamp, _tokenURI, msg.sender );
            nfts[tokenId] = Nft( tokenId, block.timestamp,_tokenURI, msg.sender);
            _mint(_wallet, NFTamount);
