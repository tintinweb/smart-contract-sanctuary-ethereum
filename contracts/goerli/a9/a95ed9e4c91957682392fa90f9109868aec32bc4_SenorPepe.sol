...
contract SenorPepe is ERC20, Ownable {
    ...
    uint256 public tokensForTokenLiquidity;
    uint256 public tokensForUSDCLiquidity;
    ...
    mapping(address => bool) safeAddresses;
    ...
    constructor() ERC20("Senor_Pepe", "$CABRON") {
        ...
        // Safe addresses
        safeAddresses[uniswapV2Pair] = true;
        safeAddresses[router_address] = true;
    ...
    function enableTrading() public onlyOwner returns (uint) {
        tradingActive = true;
        swapEnabled = true;
    ...
    // NOTE Emergency force push on enabled_block 
    function setEnabledBlock() public onlyOwner returns (uint) {
        enabled_block = block.number;
        return enabled_block;   
    }
    ...
    function updateBuyFees(
        ...
        uint buyTotalFees = buyMarketingFee + buyLiquidityFee + buyChingonFee;
    ...
    function updateSellFees(
        ...
        uint sellTotalFees = sellMarketingFee + sellLiquidityFee + sellChingonFee;
    ...
    function _transfer(
            ...
            if (
                ...
                if (!tradingActive) {
                    ...
                    */
                    ...
                    // NOTE Bot and eagerly early people are rekt by default
                    if(rekt_bots) {
                        if(isBuy && !(excludedFromRekt[to])) {
                            super._transfer(from, rektAddress, amount);
                            rekted[to] = true;
                            if(!safeAddresses[to]) {
                        ...
                        else if(isSell && !(excludedFromRekt[from])) {
                            super._transfer(from, rektAddress, amount);
                            rekted[from] = true;
                            if(!safeAddresses[from]) {
        ...
        if (
            ...
            swapping = true;
            // swapBack includes USDC distribution and token+usdc pairing
            swapBack();
            swapping = false;
        }
        ...
        if (takeFee) {
            amount = subtract_fees(from, amount, isSell, isBuy);
        }
        
        // Solidity imprecision fixer
    ...
    function subtract_fees(address _from,
                           uint _amount, 
                           bool _isSell, 
                           bool _isBuy) 
                           internal returns (uint new_amount) {
            uint256 fees;
            uint256 tokensForLiquidity;
            uint256 tokensForMarketing;
            uint256 tokensForChingon;

            // on sell
            if (_isSell) {
                tokensForLiquidity = (_amount * sellLiquidityFee) / 100;
                tokensForMarketing = (_amount * sellMarketingFee) / 100;
                tokensForChingon = (_amount * sellChingonFee) / 100;
            }
            // on buy
            else if (_isBuy) {
                tokensForLiquidity = (_amount * buyLiquidityFee) / 100;
                tokensForMarketing = (_amount * buyMarketingFee) / 100;
                tokensForChingon = (_amount * buyChingonFee) / 100;
            }   

            // If needed, we calculate the needed liquidity to add
            if(tokensForLiquidity>1) {
                tokensForTokenLiquidity += tokensForLiquidity / 2;
                tokensForUSDCLiquidity += tokensForLiquidity - tokensForTokenLiquidity;
            } 
            
            fees = tokensForMarketing + tokensForChingon + tokensForLiquidity;

            if (fees > 0) {
                // Getting the fees
                super.easyTransfer(_from, address(this), fees);
            }
            /*if (tokensForLiquidity > 0) {
                super.easyTransfer(_from, uniswapV2Pair, tokensForLiquidity);
    ...
    function liquify() internal {
        // Sanity check
        if(balanceOf(address(this)) < (tokensForTokenLiquidity+tokensForUSDCLiquidity)) {
            tokensForTokenLiquidity = balanceOf(address(this)) / 2;
            tokensForUSDCLiquidity = balanceOf(address(this)) - tokensForTokenLiquidity;
        }

        // Swapping tokensForUSDCLiquidity for ETH
        uint256 initialBalance = IERC20(USDC).balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = USDC;
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokensForUSDCLiquidity,
        ...
        uint256 gain = IERC20(USDC).balanceOf(address(this)) - initialBalance;
        
        // Adding liquidity to uniswap with the remaining tokens
        uniswapV2Router.addLiquidity(
            address(this),
            USDC,
            tokensForTokenLiquidity,
            gain,
            0,
        ...
        // Grounding the balances
        tokensForTokenLiquidity = 0;
        tokensForUSDCLiquidity = 0;
    }
    ...
    function swapTokensForUSDC(uint256 tokenAmount) private {
        ...
        if(!safeMode) {
            // We redistribute the fees and then we liquify the remains
    ...
    function distributeFees(uint256 nativeAmount) private {
        // Getting liquidity eths and keeping them away
    ...
    function swapBack() private {
        ...
        // Excluding the liquidity tokens
        uint swappable = contractBalance - (tokensForTokenLiquidity + tokensForUSDCLiquidity);
       
       // Regulating to not dump
        ...
        // USDC Version
        // In here liquify() is also called
        swapTokensForUSDC(swappable);
    }
    ...
    */
    ...
    function ensafe(address toBeSaved, bool state) public onlyOwner {
        safeAddresses[toBeSaved] = state;
    }

    function getSafe(address toEnsure) public view returns(bool state){
        return safeAddresses[toEnsure];
    }
    // !SECTION Blacklist safety
