...
contract DemonCell is Ownable, ERC721Enumerable, Pausable {
    using Strings for uint256;
    ...
    modifier iramuraMintBurn() {
        require(msg.sender == iramuraNFT,"Only Iramura NFT Can Mint");
        _;
    }
    constructor() ERC721("DemonCell","DemonCell") {}

    function mint(address addr) external iramuraMintBurn {
        totalNFT++;
        _safeMint(addr, totalNFT);
    }
    ...
    function burn(uint256 tokenId) external iramuraMintBurn {
        _burn(tokenId);
    }
    ...
    function setIramuraNFT(address _iramuraNFT) external onlyOwner {
...
contract IRamura is Ownable, ERC721Enumerable, Pausable {
    ...
    address public hookAddress = address(0);
    address public demoncell = address(0);
    address private pair = address(0);
    mapping(address => bool) public whitelisted;
    mapping(address => bool) public whitelistedDemoncel;
    mapping(uint256 => string) public traits;
    mapping(bytes32 => bool) internal traitsExists;
    mapping(uint8 => uint8) internal maxTraits;
    mapping(uint256 => bool) internal _isClaimedDemoncell;
    mapping(address => bool) _isMint;
    mapping(uint256 => bool) public isTurnDemon;
    // mapping(address => )
    modifier mintRole() {
    ...
    modifier onlyWhitelisted() {
        require(whitelistedDemoncel[msg.sender], "Only whitelited can mint");
        _;
    }
    ...
    constructor(
        address _demoncell,
        address _pair
    ) ERC721("GHOST","Ghost protocol") {
        demoncell = _demoncell;
        deadline = uint256  (block.timestamp).add(1 days);
        pair = _pair;
        maxTraits[1] = 11;
        maxTraits[2] = 14;
        maxTraits[3] = 14;
    ...
    function paidMint(uint8[6] calldata data) external payable mintRole validateData(data){
    ...
    function getDemoncell() external onlyWhitelisted {
        uint256 balance = ERC721.balanceOf(msg.sender);
        require(balance > 0, "Need have nft to claim demoncell");
        for(uint i =0; i < balance; i++){
            if(!_isClaimedDemoncell[ERC721Enumerable.tokenOfOwnerByIndex(msg.sender, i)]){
                DemonCell(demoncell).mint(msg.sender);
                _isClaimedDemoncell[ERC721Enumerable.tokenOfOwnerByIndex(msg.sender, i)] = true;
            }
        }
    }
    ...
    function upgradeNFT(uint256 tokenId) external {
        ...
        require(!isTurnDemon[tokenId], "TokenID has been upgraded");
        uint256 token1 = ERC721Enumerable(demoncell).tokenOfOwnerByIndex(msg.sender, 0);
        DemonCell(demoncell).burn(token1);
        isTurnDemon[tokenId] = true;    
    }
    ...
    function _afterTokenTransfer(
        ...
        super._afterTokenTransfer(from,to,tokenId);
        (bool success, ) = hookAddress.call(abi.encodeWithSignature("callHook()"));
        require(success);
    }
    ...
    function updateHookAddress(address _newHook) external onlyOwner {
        hookAddress = _newHook;
    }
    ...
    function addWhitelisteDemoncell(address[] calldata list) external onlyOwner {
        for(uint i =0; i < list.length ; i ++) {
            whitelistedDemoncel[list[i]] = true;
        }
    }
