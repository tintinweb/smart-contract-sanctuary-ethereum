...
contract SynthrIssuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function collateralisationRatioAndAnyRatesInvalid(address _issuer)
        ...
        returns (uint256 cratio, bool anyRateIsInvalid)
    {
        return _collateralisationRatioCrossChain(_issuer);
    }
    ...
    function getSendBurnGasFee(
        ...
        (uint256 existingDebt, , ) = _debtBalanceOfAndTotalDebt(synthetixDebtShare().balanceOfCrossChain(account), sUSD);
        // max issuable synth in USD
        uint256 maxIssuableSynthsForAccount = _maxIssuableSynthsCrossChain(account);
        if (burnToTarget) {
        ...
        uint256 synthAmount = existingSynth < amount ? existingDebt : amount;
