...
contract DefiOTCExchange is ReentrancyGuard {
    ...
    /// Fullfill order by id
    function fulfillOrderById(uint256 orderId, uint256 tokenYAmount) external nonReentrant {
        Order storage order = orderIdMapping[orderId];
        address tokenY = order.tokenY;
        address tokenX = order.tokenX;

        if (order.maker == address(0) || tokenX == address(0) || tokenY == address(0) || order.tokenXAmount == 0 || order.desiredTokenYAmount == 0) revert OrderDoesNotExist();
        if (order.desiredTokenYAmount < tokenYAmount) revert InsufficientMarketDepth();
        if (!IERC20(tokenY).transferFrom(msg.sender, address(this), tokenYAmount)) revert TransferFailed();
        ...
        if (order.desiredTokenYAmount == tokenYAmount) delete orderIdMapping[orderId];
        else {
            order.desiredTokenYAmount -= tokenYAmount;
            order.tokenXAmount -= tokenXAmount;
        }
    ...
    function getMinimumOrderForPair(address tokenX, address tokenY) external view returns (Order memory) {
        uint256[] memory orderIdsForPair = orderBooks[tokenX][tokenY];
        return orderIdMapping[getMinimumValidOrderId(orderIdsForPair)];
    }
