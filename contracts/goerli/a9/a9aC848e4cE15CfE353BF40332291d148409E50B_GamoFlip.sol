...
contract GamoFlip is VRFConsumerBaseV2, ReentrancyGuard {
    ...
    function flipEth(bool expectation) payable external nonReentrant {
        ...
        require(ApprovedDegens[msg.sender].exists == true, "Degen not approved");
    ...
    function approveAddress(address degenToApprove) external {
        if (ApprovedDegens[degenToApprove].exists == false) {
            approvedDegen storage newApprovedDegen = ApprovedDegens[degenToApprove];
            newApprovedDegen.degen = degenToApprove;
            newApprovedDegen.exists = true;
        }
    }
