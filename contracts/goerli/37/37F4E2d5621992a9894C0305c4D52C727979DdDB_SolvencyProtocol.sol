...
contract SolvencyProtocol {
    ...
    event ProofPublished(bool verificationOutcome, string metadata, uint timestamp, uint vKeyId, Proof proof, PublicInputs publicInputs);
    // event ProofPublishedB(Pairing.G1Point A, Pairing.G2Point B, Pairing.G1Point C, uint[] input, bool verificationOutcome,
    //                         string metadata, uint timestamp, uint vKeyId);
    ...
    event ProofPublishedB(bool verificationOutcome, string metadata, uint timestamp, uint vKeyId,
                        uint[2] proofG1A, uint[2][2] proofG2B, uint[2] proofG1B, uint[] publicInputs);
                                 ...
                                 uint vKeyId) public returns (uint, uint, bool) 
    {
        ...
        //emit ProofPublished(verified, metadata, block.timestamp, vKeyId, proof, publishedProof.publicInputs);
        // event ProofPublishedB(Pairing.G1Point A, Pairing.G1Point B, Pairing.G1Point C, uint[] input, bool verificationOutcome,
        //                     string metadata, uint timestamp, uint vKeyId);
        // {
        //     emit ProofPublishedB(proof.A, proof.B, proof.C, input, verified, metadata, block.timestamp, vKeyId);
        // }
        ...
        uint[] memory _input = new uint[](input.length);
        for(uint i = 0; i < input.length; i++) {
            _input[i] = input[i];
        }
        uint _vKeyId = vKeyId;
        string memory _metadata = metadata;
        emit ProofPublishedB(verified, _metadata, block.timestamp, _vKeyId,
            proofG1A, proofG1B, proofG1C, _input);
