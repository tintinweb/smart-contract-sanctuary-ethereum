...
contract TokenPrice {
	...
	function getTokenSupplies(address[] memory _tokens) public view returns (uint256[] memory) {
	...
	function getTokensBurned(address[] memory _tokens) public view returns (uint256[] memory) {
	...
	function getTokensBurned(address[] memory _tokens, address[][] memory _burnaddresses) public view returns (uint256[] memory) {
	...
	function _calculateTokenMC(address _token, address[] memory _burnaddresses) private view returns (uint256) {
		uint256 price = 0;
		...
		if(_burnaddresses.length > 0) {
			...
			return price * (erc20.totalSupply() - burn);
		}
		return price * erc20.totalSupply();
	}
	...
	function getTokenDilutedMarketCapsInEth(address[] memory _tokens) public view returns (uint256[] memory) {
		...
		for(uint256 i = 0; i < _tokens.length; i++) {
			marketcaps[i] = _calculateTokenMC(_tokens[i], _burnaddresses);
		}
		return marketcaps;
    }
	...
	function getTokenCirculatingMarketCapsInEth(address[] memory _tokens) public view returns (uint256[] memory) {
		uint256[] memory marketcaps = new uint256[](_tokens.length);
		for(uint256 i = 0; i < _tokens.length; i++) {
			marketcaps[i] = _calculateTokenMC(_tokens[i], _burns);
		}
		return marketcaps;
    }
	...
	function getTokenCirculatingMarketCapsInEth(address[] memory _tokens, address[][] memory _burnaddresses) public view returns (uint256[] memory) {
		...
		uint256[] memory marketcaps = new uint256[](_tokens.length);
		for(uint256 i = 0; i < _tokens.length; i++) {
			marketcaps[i] = _calculateTokenMC(_tokens[i], _burnaddresses[i]);
		}
		return marketcaps;
    }
