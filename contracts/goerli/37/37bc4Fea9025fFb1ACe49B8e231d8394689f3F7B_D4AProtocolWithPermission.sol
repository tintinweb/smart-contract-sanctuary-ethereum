...
contract D4AProtocolWithPermission is D4AProtocol, EIP712Upgradeable {
    ...
    function createCanvas(
        ...
        bytes32[] calldata _blacklistProof,
        bytes32[] calldata _whitelistProof
    ) external payable nonReentrant returns (bytes32) {
        require(
            !settings.permission_control().inCanvasCreatorBlacklist(_DAO_id, msg.sender, _blacklistProof),
            "Account is blacklisted"
        );
        require(
            settings.permission_control().inCanvasCreatorWhitelist(_DAO_id, msg.sender, _whitelistProof),
            "Not in whitelist"
        );
    ...
    function mintNFT(
        ...
        require(
            !settings.permission_control().inMinterBlacklist(_DAO_id, msg.sender, _blacklistProof),
            "Account is blacklisted"
        );
        require(
            settings.permission_control().inMinterWhitelist(_DAO_id, msg.sender, _whitelistProof), "Not in whitelist"
        );
