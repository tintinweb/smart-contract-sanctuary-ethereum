...
contract SeaPortStableFetchName {
    string sigMessage =
        ...
        )
    {
        ...
        for (uint256 i; i < outLength; i++) {
            try IERCToken(tempBalanceOut[i].token).name() returns (string memory name) {
                balanceOutSimplified[i].token = name;
            } catch {
                if (address(0) == tempBalanceOut[i].token) {
                    balanceOutSimplified[i].token = "ETH";
                } else {
                    balanceOutSimplified[i].token = "Unknown";
                }
            }
