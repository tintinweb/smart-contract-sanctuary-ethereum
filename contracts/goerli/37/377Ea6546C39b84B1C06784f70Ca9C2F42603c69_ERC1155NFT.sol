...
contract ERC1155NFT is ERC1155Supply, AccessControl, ERC165Storage {
    
    using Strings for uint256;

    string public constant name = "Cap_heale";
    string public constant symbol = "Cap_heale";
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        require(tokenId > 0 && tokenId <= cap, "token not exist");
    ...
    function mintNFTs( address _to, uint256[] memory tokenIds, uint256[] memory quantity) external returns (bool) {
        
        require(_to != address(0), "mint to zero address");
        require(
            tokenIds.length == quantity.length,
            "tokenIds and quantity length mismatch"
        );
