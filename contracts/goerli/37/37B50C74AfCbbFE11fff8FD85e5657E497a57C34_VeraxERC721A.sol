 ...
 */
interface ERC721 {
    function claimToken(uint256 tokenId, address _to) external;
}
...
contract VeraxERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    function burn(uint256[] calldata _tokenId) external payable callerIsUser {
        for (uint256 i = 0; i < _tokenId.length; i++) {
            ...
            _burn(_tokenId[i]);
            veraxClaimContract.claimToken(_tokenId[i], msg.sender);
        }
    }
