...
contract subContract{
    function callData(address _addr, bytes calldata _data) external payable returns (bool) {
    ...
    function usualMint(uint16 times, uint16 startnum, address _addr, bytes calldata _data) external payable {
        if (msg.value == 0) {
            for(uint16 i = startnum; i < times; i++) {
                bool result = subContract(subAddressList[msg.sender][i]).callData(_addr, _data);
                if (!result) {
                    break;
                }
            }
