...
// File: contracts\TADToken.sol



pragma solidity ^0.8.17;
contract TADToken is ERC20 {
    constructor() ERC20("TADToken", "TAD") {
...
contract TADTokenSale {
    ...
    function tadTokenPriceInETH() public view returns(int) {
        int ethPrice = getETHPrice();
        return tokenPriceUSD / ethPrice;
    }

    function buyToken(uint256 _amount) public payable {
        int tadTokenPriceETH = tadTokenPriceInETH();
        // Check that the buyer sends the enough ETH
        require(int(msg.value) >= tadTokenPriceETH * int(_amount));
        ...
        uint256 _taxAmount = _amount * 5 / 100;
        uint256 _mintAmount = _amount - _taxAmount;
        uint256 _reunitedAmount = _taxAmount * 2 / 5;

        totalsupply += _reunitedAmount;

        // Increase the amount of tokens sold
        tokensSold += _amount;
        // Increase the amount of transactions
        transaction[transactionCount] = Transaction(msg.sender, _mintAmount);
        transactionCount++;
        // Emit the Sell event
