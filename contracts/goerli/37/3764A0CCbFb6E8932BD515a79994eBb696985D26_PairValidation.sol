...
contract PairValidation is ERC721, Ownable {
    ...
    constructor(string memory imageURI) ERC721("PairTesting", "PTS") {
        s_tokenCounter = 0;
        s_mappingSize = 0;
    ...
    function acceptNft() external hasRequestToAccept(msg.sender) {
        ...
        safeMint(requestor, s_tokenCounter, requestorURI);
        s_tokenCounter = s_tokenCounter + 1;
        string memory senderURI = createTokenURI(
            s_activeRequests[msg.sender].name1,
            s_activeRequests[msg.sender].name2
        );
        safeMint(msg.sender, s_tokenCounter, senderURI);
        s_tokenCounter = s_tokenCounter + 1;
    ...
    function createTokenURI(string memory name1, string memory name2)
        public
        view
        virtual
        returns (string memory)
    {
        string memory image = s_imageURI;
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '", "description":"',
                                names,
                                '"attributes": [{"trait_type": "Working together", "value": 404}], "image":"',
                                image,
                                '"}'
                            )
                        )
    ...
    function tokenURI(uint256 tokenId, string memory uri)
        public
        view
        virtual
        returns (string memory)
