...
contract NFTCollection is ERC721A, ERC2981, Ownable {
    ...
    address public currency; //Set the address of currency
    address public launchpad; //Set the address of launchpad
    INFTLaunchPad instance = INFTLaunchPad(launchpad);
    IERC20 objectERC20 = IERC20(currency);
    ...
    */
     ...
     *@param launchpad: address of Launchpad.
     */
    function _getBrokerage(address _currency, address launchpad)
        private
        view
        returns (int256)
    {
        int256 _brokerage = instance.getBrokerage(_currency);
     ...
     *@param launchpad: address of launchpad
     */
    function _getPublicKey(address launchpad) external view returns (address) {
    ...
    function verifyWhiteListUser(
        ...
        if (_isWhiteListed) {
            require(
                verifiedUser == platformPublicKey,
    ...
    function mint(
        ...
        int256 brokerage = _getBrokerage(currency, launchpad);
        ...
        if (currency == address(0)) {
            ...
            require(
                objectERC20.allowance(msg.sender, address(this)) >= _amount,
                "NFTCollection: Insufficient fund allowance"
            );
            uint256 brokerageAmount = (_amount * uint256(brokerage)) /
                (100 * DECIMAL_PRECISION);
            objectERC20.transferFrom(
                msg.sender,
                brokerAddress,
                brokerageAmount
            );
     ...
     */
    function withdrawERC20Token(address _erc20Token, address _receiver)
        external
        onlyOwner
    {
        require(
            objectERC20.balanceOf(address(this)) > 0,
            "NFTCollection: Insufficient fund"
        );
        objectERC20.transfer(_receiver, objectERC20.balanceOf(address(this)));
    }
