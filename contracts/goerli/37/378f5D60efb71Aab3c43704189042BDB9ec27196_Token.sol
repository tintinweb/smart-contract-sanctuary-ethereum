...
contract Token is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 public initialTotalSupply = 1000000000 * (10**18);

    uint256 public swapTokensAtAmount = initialTotalSupply.mul(100).div(denominator);

    uint256 public maxTxAmount = initialTotalSupply.mul(200).div(denominator); // 2% of the supply
    uint256 public maxWalletAmount = initialTotalSupply.mul(200).div(denominator); // 2% of the supply

    bool public swapEnabled = false;
    bool public tradingEnabled = false;

    address public devWallet = address(0); //  DEV WALLET
    ...
    function addLiquidity(uint256 liquiditySupply) external payable onlyOwner() {
        _approve(address(this), address(uniswapV2Router), liquiditySupply);
        
        _transfer(msg.sender, address(this), liquiditySupply);
    ...
    function blacklistMultipleAccounts(address[] calldata accounts) public onlyOwner
    {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function setBuyFees(uint16 fee) external onlyOwner {
        buyFee = fee;
        require (buyFee <= 1500, "max fees should be less than or equal to 15%");
    }

     function setSellFees(uint16 fee) external onlyOwner {
        sellFee = fee;
        require (sellFee <= 1500, "max fees should be less than or equal to 15%");
    }
    ...
    function _transfer(
        ...
        if (!tradingEnabled) {
            // require(!automatedMarketMakerPairs[from], "Token: trading isn't enabled yet");
            require(from != owner() || from != address(this), "Token: trading isn't enabled yet");
            super._transfer(from, to, amount);
            return;
        }

        if (from != owner() && to != owner()) {
            require(!_isBlacklisted[from], "Token: account is blacklisted");
            require(!_isBlacklisted[to], "Token: account is blacklisted");
        }
        ...
        if (
            ...
            uint256 devTokens = swapTokensAtAmount;
    ...
    function manualSendToDev() external {
        require(_msgSender() == devWallet);
        uint256 balance = address(this).balance;
        (bool mw, ) = payable(devWallet).call{value: balance}("");
        require(mw);
    }

    function removeLimits() external onlyOwner {
        maxWalletAmount = totalSupply() * 10**18;
        maxTxAmount = totalSupply() * 10**18;
    }
