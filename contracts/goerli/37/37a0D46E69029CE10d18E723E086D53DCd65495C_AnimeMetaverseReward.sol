    ...
    IAnimeMetaverseReward
{
    ...
    modifier validMintingAmount(uint256 amount) {
        if (amount > maxMintLimit || amount == 0) {
    ...
    /// @notice Check whether value is zero
    ...
    function setTokenInfo(
        ...
        totalTokenSupply += _maximumSupply;
        totalToken++;
    ...
    function setMaxMintLimit(uint256 _mintLimit) external onlyOwner {
        ...
        require(_mintLimit <= 5550, "Can not set mintLimit more than 5550.");
    ...
    /// @param _flag 'true' to activate claim merchandise event, otherwise 'false'
    ...
    function mintBatch(
        ...
        for (uint256 j = 0; j < _ids.length; j = _uncheckedInc(j)) {
            uint256 _id = _ids[j];
            if (_amounts[j] == 0) {
                revert ValueCanNotBeZero();
            }
    ...
    {
        if (tokenCollection[_id].tokenType == DIGITAL_COLLECTIBLE_TOKEN_TYPE) {
            revert InvalidMintableTokenId();
        }
        if (
    ...
    /// @notice Mint digital merchandise
    /// @param _to Receiver of the digital merchandise
    ...
    /// @notice Claim merchandise with digital merchandise
    /// @param _account Owner of the merchandise
