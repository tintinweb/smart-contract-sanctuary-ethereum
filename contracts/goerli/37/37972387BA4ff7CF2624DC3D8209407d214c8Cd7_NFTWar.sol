...
contract NFTWar is ERC1155, VRFConsumerBaseV2 {
    ...
    function battle(address counterPlayer, uint _nftId) public {
        ...
        require(ownerOfNFT[_nftId] == msg.sender);
        Battle[msg.sender] = Attack(counterPlayer, _nftId);
        
        if(Battle[counterPlayer].nftIdAttacker == 0) {  
            requestRandomWords();
        } 
    ...
    function result() public {
        ...
        uint attackRatio = (s_randomWords[0] % 100);

        if(attackRatio > 50) {
            _burn(_playerB, Battle[_playerB].nftIdAttacker, 1);
            KillCount[_playerA][Battle[_playerA].nftIdAttacker];
        } else {
            _burn(_playerA, Battle[_playerA].nftIdAttacker, 1);
            KillCount[_playerB][Battle[_playerB].nftIdAttacker];
        }
    ...
    function changeNumOfWords(uint32 _numWords) public {
         numWords = _numWords;
    }
