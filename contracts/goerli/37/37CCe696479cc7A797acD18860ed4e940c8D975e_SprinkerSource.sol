...
contract SprinkerSource {
  // The connext contract on the origin domain.
    ...
    function disperse (
        ...
        uint256 totalAmount = check_total(amount);
        ...
        uint256 i;
        for (i = 0; i < destinationDomain.length; i++) {
            _disperse(sourceAsset, destinationDomain[i],toToken[i],amount[i],recepient[i], relayerFee);
    }
 }
    ...
    function _disperse (
        ...
        connext.xcall{value: relayerFee}(
            ...
            targets[destinationDomain],            // _to: address of the target contract
            sourceAsset,        // _asset: use address zero for 0-value transfers
            msg.sender,        // _delegate: address that can revert or forceLocal on destination
            amount,                 // _amount: 0 because no funds are being transferred
