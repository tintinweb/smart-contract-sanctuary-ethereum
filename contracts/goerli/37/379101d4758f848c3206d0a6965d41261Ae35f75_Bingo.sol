...
contract Bingo is IBingo, VRFConsumerBaseV2 {
    // precision basis is used to calculated shares
    ...
    // numbers picked on a game (gameId => numbers[])
    ...
    // cards of a player in a game(gameId => playerAddress => cardIds[])
    ...
    function batchAddCards(uint256[] memory newCards) external {
        if (!admins[msg.sender]) {
            revert Unauthorized();
        }

        for (uint256 i; i < newCards.length; i++) {
            cards.push(newCards[i]);
        }
        emit CardsAdded(newCards.length, cards.length);
    }

    function batchUpdateCards(uint256[] memory indexes, uint256[] memory newCards) external {
        if (!admins[msg.sender]) {
            revert Unauthorized();
        }
    ...
    function createGame(
        ...
        uint256 firstFiveShare,
        uint256 secondFiveShare,
        uint256 tomboShare,
        uint256 houseShare
    ) external onlyHost(0) {
        ...
        // all shares combined should be equal to basis
        if (firstFiveShare + secondFiveShare + tomboShare + houseShare != PRECISION_BASIS) {
            revert InputValidation();
        }
        ...
        game.firstFiveShare = firstFiveShare;
        game.secondFiveShare = secondFiveShare;
        game.tomboShare = tomboShare;
    ...
    function joinGame(uint256 gameId, uint256 cardsCount) external payable inStatus(gameId, GameStatus.CREATED) {
        ...
        // This is a precaution to prevent to many re-rolls on duplicate cards
        ...
        requestRandomWords(uint32(cardsCount), gameId, msg.sender);
    ...
    function requestRandomWords(
        ...
        randomRequests[requestId] = RandomRequest({
            randomWords: new uint256[](numberOfWords),
            gameId: gameId,
            player: player,
            exists: true,
            fulfilled: false
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        if (req.player != address(0) && playerCards[req.gameId][req.player][0] == 1) {
            for (uint256 i; i < _randomWords.length; i++) {
                uint256 seed = _randomWords[i];

                uint256 card = cards[(seed & 0xFFFF) % cards.length];
                while (revealedCards[req.gameId][card]) {
                    seed = seed >> 16;
        ...
        emit RequestFulfilled(_requestId, _randomWords.length);
    }
    ...
    function claimRefund(uint256 gameId) external inStatus(gameId, GameStatus.CANCELLED) {
    ...
    // TODO: Delete this MOCK function this is only for testing purposes
    // TODO: FIND A WAY TO TEST THIS WITHOUT USING AN EXTERNAL FN
    function mockFulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) public {
...
interface IBingo {
    struct Game {
        ...
        uint256 firstFiveShare; // PRECISION_BASIS = 10_000
        uint256 secondFiveShare; // PRECISION_BASIS = 10_000
        uint256 tomboShare; // PRECISION_BASIS = 10_000
        uint256 houseShare; // PRECISION_BASIS = 10_000
        address firstFiveWinner;
        address secondFiveWinner;
        address tomboWinner;
        bool cancelled;
    }
    ...
    event RequestFulfilled(uint256 requestId, uint256 numberOfWords);
