...
contract Metaseum is ERC721Psi, Pausable, Ownable, ReentrancyGuard {
    //--------------------------------------------------
    // Constants
    //--------------------------------------------------
    uint256 public constant MAX_TOKENS = 1111;
    uint256 public constant MAX_TOKENS_PER_WALLET = 500;
    uint256 public constant RESERVED_AMOUNT = 111;

    //--------------------------------------------------
    // Variables
    //--------------------------------------------------
    uint256 public maxTotalMintsThisStage = 1111;
    uint256 public maxMintBatchAmount = 144;
    uint256 public pricePerToken = 0.14 ether;
    uint256 public reservedTokensMinted = 0;
    address public byontGnosisSafe;
    ...
    event SetByontGnosisSafe(address newByontGnosisSafe);
    ...
    // Constructor
    //--------------------------------------------------
    constructor(address _byontGnosisSafe) ERC721Psi('Metaseum', 'MUSEA-LP') {
        byontGnosisSafe = _byontGnosisSafe;
    }
        ...
        nonReentrant
    {
        ...
        // Royality split.
        (bool success, ) = payable(byontGnosisSafe).call{
            value: (msg.value * 10) / 100
        }('');
    ...
    //--------------------------------------------------
    // Royalty split related
    //--------------------------------------------------
    function setByontGnosisSafe(address newByontGnosisSafe) external onlyOwner {
        require(
            newByontGnosisSafe != byontGnosisSafe,
            'Cannot set the same Byont address'
        );
        byontGnosisSafe = newByontGnosisSafe;
        emit SetByontGnosisSafe(byontGnosisSafe);
    }
