...
contract UpOnly is ERC721, Ownable, ReentrancyGuard {
    ...
    function mint(
        ...
        require(artistAddress != address(0x0), "Artist address must not be 0x0");
        uint256 tokenId = supply;
        
        _safeMint(artistAddress, tokenId);
        tokens[tokenId] = UpOnlyToken(artistAddress, artistName, tokenName, tokenDescription, imageCID, videoCID, 0, 0, 0);
        ++supply;

        emit Mint(artistAddress, tokenId);
    }
    ...
    function buy(uint256 tokenId) external nonReentrant payable {
        ...
        address artistAddress = tokens[tokenId].artistAddress;
        ...
        tokens[tokenId].saleCount += 1;
        delete tokens[tokenId].listPrice;

        _transfer(seller, msg.sender, tokenId);

        (bool success, ) = artistAddress.call{value: msg.value * ARTIST_FEE / 100 }("");
    ...
    function transferFrom(address, address, uint256) public virtual override {
        require(false, "This NFT cannot be transferred. It must be sold for a higher price.");
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        require(tokens[tokenId].artistAddress != address(0x0), "Token does not exist");

        string memory image; 
        string memory video;

        if (bytes(tokens[tokenId].imageCID).length > 0) {
            image = string(abi.encodePacked('"image": "ipfs://', tokens[tokenId].imageCID, '",'));
        }

        if (bytes(tokens[tokenId].videoCID).length > 0) {
            video = string(abi.encodePacked('"animation_url": "ipfs://', tokens[tokenId].videoCID, '",'));
        }

        return string(abi.encodePacked(
            'data:application/json;utf8,{',
            '"name": "', tokens[tokenId].tokenName,'",',
            '"description": "', tokens[tokenId].tokenDescription, '",',
            image,
            video,
            '"attributes": [',
            '  {"trait_type": "Artist", "value": "', tokens[tokenId].artistName, '"}, ',
            '  {"trait_type": "Sale Count", "value": "', tokens[tokenId].saleCount, '"} ',
            ']}'
        ));
    }
