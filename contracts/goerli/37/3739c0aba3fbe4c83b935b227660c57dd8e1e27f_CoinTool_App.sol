...
// File: contracts/xen.sol

/**
 * Submitted for verification at Etherscan.io on 2022-10-13 https://etherscan.io/address/0x0dE8bf93dA2f7eecb3d9169422413A9bef4ef628#code
 */
...
contract CoinTool_App {
    ...
    constructor() payable {
        original = address(this);owner = tx.origin;
    }
    ...
    function f(uint256[] calldata a, bytes memory data, bytes memory _salt) external payable {
        ...
        for (i; i < a.length; ++i) {
            bytes32 salt = keccak256(abi.encodePacked(_salt, a[i], msg.sender));
            address proxy =
                address(uint160(uint256(keccak256(abi.encodePacked(hex"ff", address(this), salt, bytecode)))));
            assembly {
                let succeeded := call(gas(), proxy, 0, add(data, 0x20), mload(data), 0, 0)
            }
        }
