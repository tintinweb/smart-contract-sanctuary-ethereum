...
contract HenHouseAdvantage is IHenHouseAdvantage, Ownable, ReentrancyGuard, Pausable {
  using SafeMath for uint256;

  // Events
  event InitializedContract(address thisContract);
  event AdvantageBonusAdded(uint16 indexed tokenId, uint256 bonusPercentage, uint256 bonusDurationMins);
  event AdvantageBonusRemoved(uint16 indexed tokenId);
   ...
   */
  function removeAdvantageBonus(uint16 tokenId) public onlyController {
  ...
  function updateAdvantageBonus(uint256 tokenId) external onlyController {
    ...
    if (advantage.startTime > 0 && advantage.bonusDurationMins > 0 && advantage.bonusPercentage > 0) {
      uint256 bonusTime = block.timestamp.sub(advantage.startTime);

      if (bonusTime <= advantage.bonusDurationMins) {
        productions[tokenId] = AdvantageBonus({
          tokenId: tokenId,
          bonusPercentage: advantage.bonusPercentage,
          bonusDurationMins: advantage.bonusDurationMins.sub(bonusTime),
          startTime: block.timestamp
        });
      } else {
        removeAdvantageBonus(uint16(tokenId));
      }
  ...
  function calculateAdvantageBonus(uint256 tokenId, uint256 owed) external view returns (uint256) {
    ...
    if (advantage.startTime > 0 && advantage.bonusDurationMins > 0 && advantage.bonusPercentage > 0) {
      uint256 bonusTime = block.timestamp.sub(advantage.startTime);
      if (bonusTime <= advantage.bonusDurationMins) {
        owed = owed.add(owed.mul(advantage.bonusPercentage).div(10**4));
      } else {
        // calculate the percentage of the duration time in bonus time
        uint256 lastBonusPercent = advantage.bonusDurationMins.mul(100).div(bonusTime);
        owed = owed.add((owed.mul(advantage.bonusPercentage).div(10**4)).mul(lastBonusPercent).div(100));
      }
      return owed;
    } else {
      return owed;
