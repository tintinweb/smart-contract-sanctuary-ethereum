...
contract Raffle is VRFConsumerBaseV2, KeeperCompatibleInterface {
  ...
  uint16 private constant REQUEST_CONFIRMATIONS = 6;
  uint32 private immutable i_callbackGasLimit;
  uint32 private constant NUM_WORDS = 1;

  //local var
  address private s_recentWinner;
  RaffleState private s_openState;
  ...
  function enterRaffle() public payable {
    ...
    if (s_openState != RaffleState.OPEN) {
  ...
  {
    bool open = (s_openState == RaffleState.OPEN);
    bool hasEth = (address(this).balance > 0);
    bool hasPlayer = (s_players.length > 0);
    bool timepassed = ((block.timestamp - s_lastTimestamp) > i_interval);
    upkeepNeeded = (open && hasEth && hasPlayer && timepassed);
  }
  ...
  function performUpkeep(
    ...
    s_openState = RaffleState.CALCULATING;
  ...
  function fulfillRandomWords(
    ...
    uint256 winnerIndex = s_players.length % _randomWords[0];
