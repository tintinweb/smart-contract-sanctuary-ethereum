...
contract MasterContractManager is BoringOwnable, BoringFactory {
    event LogPurpleListMasterContract(address indexed masterContract, bool approved);
    ...
    /// @notice masterContract to purplelisted state for approval without signed message
    mapping(address => bool) public purplelistedMasterContracts;
    ...
    function _calculateDomainSeparator(uint256 chainId) private view returns (bytes32) {
        return
            keccak256(abi.encode(DOMAIN_SEPARATOR_SIGNATURE_HASH, keccak256("NeutronStar V1"), chainId, address(this)));
    }
    ...
    /// @notice Other contracts need to register with this master contract so that users can approve them for the NeutronStar.
    ...
    function purplelistMasterContract(address masterContract, bool approved) public onlyOwner {
        // Checks
        require(masterContract != address(0), "MasterCMgr: Cannot approve 0");

        // Effects
        purplelistedMasterContracts[masterContract] = approved;
        emit LogPurpleListMasterContract(masterContract, approved);
    }
    ...
    function setMasterContractApproval(
        ...
        if (r == 0 && s == 0 && v == 0) {
            ...
            require(purplelistedMasterContracts[masterContract], "MasterCMgr: not purplelisted");
        } else {
                ...
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            ...
                            approved
                                ? keccak256("Give FULL access to funds in (and approved to) NeutronStar?")
                                : keccak256("Revoke access to NeutronStar?"),
                            user,
                            masterContract,
                            approved,
                            nonces[user]++
...
// File contracts/NeutronStar.sol
// License-Identifier: UNLICENSED

/// @title NeutronStar
/// @author BoringCrypto, Keno
/// @notice The NeutronStar is a vault for tokens. The stored tokens can be flash loaned and used in strategies.
/// Yield from this will go to the token depositors.
/// Rebasing tokens ARE NOT supported and WILL cause loss of funds.
/// Any funds transfered directly onto the NeutronStar will be lost, use the deposit function instead.
contract NeutronStarV1 is MasterContractManager, BoringBatchable {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    struct StrategyData {
        ...
        uint128 balance; // the balance of the strategy that NeutronStar thinks is in there
    }
    ...
    /// If 'from' is the NeutronStar itself, it's allowed. Any ETH, token balances (above the known balances) or NeutronStar balances
    ...
    modifier allowed(address from) {
        if (from != msg.sender && from != address(this)) {
            ...
            require(masterContract != address(0), "NeutronStar: no masterContract");
            require(masterContractApproved[masterContract][from], "NeutronStar: Transfer not approved");
        }
        _;
    }
    ...
    function deposit(
        ...
        // Checks
        require(to != address(0), "NeutronStar: to not set"); // To avoid a bad UI from burning funds
        ...
        require(total.elastic != 0 || token.totalSupply() > 0, "NeutronStar: No tokens");
        if (share == 0) {
    ...
    function withdraw(
        ...
        require(total.base >= MINIMUM_SHARE_BALANCE || total.base == 0, "NeutronStar: cannot empty");
        totals[token] = total;
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            require(success, "NeutronStar: ETH transfer failed");
        } else {
    ...
    function transferMultiple(
        ...
        // Checks
        require(tos[0] != address(0), "NeutronStar: to[0] not set"); // To avoid a bad UI from burning funds
    ...
    function flashLoan(
        ...
        require(_tokenBalanceOf(token) >= totals[token].addElastic(fee.to128()), "NeutronStar: Wrong amount");
    ...
    function batchFlashLoan(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            require(_tokenBalanceOf(token) >= totals[token].addElastic(fees[i].to128()), "NeutronStar: Wrong amount");
    ...
    // F5 - Checks-Effects-Interactions pattern followed? (SWC-107)
    // F5: Total amount is updated AFTER interaction. But strategy is under our control.
    // F5: Not followed to prevent reentrancy issues with flashloans and NeutronStar skims?
    function harvest(IERC20 token, bool balance, uint256 maxChangeAmount) public {
        ...
        if (balanceChange > 0) {
            ...
            // But tokens with balances that large are not supported by the NeutronStar.
            uint256 sub = uint256(-balanceChange);
