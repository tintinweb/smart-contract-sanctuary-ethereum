    ...
    ERC721AUpgradeable
{
    ...
    function initialize(
        ...
        address[] memory _trustedForwarders,            // 可信任的转发者
        address _saleRecipient,                         // 销售接收者
        address _royaltyRecipient,                      // 版税接收者
        uint128 _royaltyBps,                            // 版税BPS
        ...
        _setupDefaultRoyaltyInfo(_royaltyRecipient, _royaltyBps);           // 设置默认的版税信息
 ...
 *  @notice  dmx's `ContractMetadata` is a contract extension for any base contracts. It lets you set a metadata URI
 *           for you contract.
 ...
 *  @notice  dmx's `DelayedReveal` is a contract extension for base NFT contracts. It lets you create batches of
 *           'delayed-reveal' NFTs. You can learn more about the usage of delayed reveal NFTs here - https://blog.dmx.com/delayed-reveal-nfts
 */
...
/**
 *  The interface `IClaimCondition` is written for dmx's 'Drop' contracts, which are distribution mechanisms for tokens.
 *
...
/**
 *  dmx's `DelayedReveal` is a contract extension for base NFT contracts. It lets you create batches of
 *  'delayed-reveal' NFTs. You can learn more about the usage of delayed reveal NFTs here - https://blog.dmx.com/delayed-reveal-nfts
 */
...
/**
 *  dmx's `LazyMint` is a contract extension for any base NFT contract. It lets you 'lazy mint' any number of NFTs
...
/**
 *  dmx's `Ownable` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
/**
 *  dmx's `PlatformFee` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
/**
 *  dmx's `Primary` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
/**
 *  dmx's `Royalty` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
/**
 *  The 'signature minting' mechanism used in dmx Token smart contracts is a way for a contract admin to authorize an external party's
...
interface ISignatureMintERC721 {
    ...
    struct MintInput {
        ...
        uint256 quantity;               // 铸造数量
        uint256 pricePerToken;          // 支付价格
        address payementToken;          // 支付代币
        uint128 validityStartTimestamp; // 有效期开始时间
        uint128 validityEndTimestamp;   // 有效期开始时间
        bytes32 salt;                    // 唯一标识
    }
     ...
     *       验证铸造请求是否由持有 MINTER_ROLE 的帐户签名（在函数调用时）
...
/**
 *  @title   Ownable
 *  @notice  dmx's `Ownable` is a contract extension to be used with any base contract. It exposes functions for setting and reading
 *           who the 'owner' of the inheriting smart contract is, and lets the inheriting contract perform conditional logic that uses
 *           information about who the contract's owner is.
 */
...
contract Permissions is IPermissions {
    ...
    /// @dev 默认的管理员角色，只有拥有该角色的账户才能授予/撤销其他角色。
...
/**
 *  @title   Primary Sale
 *  @notice  dmx's `PrimarySale` is a contract extension to be used with any base contract. It exposes functions for setting and reading
 *           the recipient of primary sales, and lets the inheriting contract perform conditional logic that uses information about
 *           primary sales, if desired.
 */
...
/**
 *  @title   Royalty
 *  @notice  dmx's `Royalty` is a contract extension to be used with any base contract. It exposes functions for setting and reading
 *           the recipient of royalty fee and the royalty fee basis points, and lets the inheriting contract perform conditional logic
 *           that uses information about royalty fees, if desired.
 *
