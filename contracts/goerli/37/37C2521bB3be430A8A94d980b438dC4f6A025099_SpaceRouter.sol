...
contract SpaceLP is ERC20 {
    ...
    event LiquidityAdded(address indexed user, uint256 amountEth, uint256 amountSpc, uint256 amountEthSpcLp);
    event LiquidityRemoved(address indexed user, uint256 amountEth, uint256 amountSpc, uint256 amountEthSpcLp);
    event EthForSpcSwap(address indexed user, uint256 amountEth, uint256 amountSpc, uint256 feeEth);
    event SpcForEthSwap(address indexed user, uint256 amountSpc, uint256 amountEth, uint256 feeSpc);

    constructor(SpaceCoin _spaceCoin) ERC20("Ether_SpaceCoin_LP", "ETH_SPC_LP") {
    ...
    function deposit(address to) external payable {
        ...
        if (totalSupply == 0) {
            ...
            if (differenceEth > 0 && differenceSpc > 0) { //Prevent divide-by-zero
                lpAmount = min((differenceEth * totalSupply) / reserveEth, (differenceSpc * totalSupply) / reserveSpc);
            }
        }
    ...
    function swapEthToSpc(address to) private {
        ...
        uint256 dueSpc = (ethInLessFee * reserveSpc) / (reserveEth + ethInLessFee);
    ...
    function swapSpcToEth(address to) private {
        ...
        uint256 dueEth = (spcInLessFee * reserveEth) / (reserveSpc + spcInLessFee);
    ...
    function sendEth(address to, uint256 amount) private {
        ...
        if (!success) { revert EthSendFailed(to, amount ); }
    }
...
contract SpaceRouter {
    ...
    modifier nonZeroEth() {
        if (msg.value == 0) { revert ZeroAmount("ETH"); }
        _;
    }

    modifier nonZeroSpc(uint256 amount) {
        if (amount == 0) { revert ZeroAmount("SPC"); }
        _;
    }

    modifier nonZeroEthSpcLp(uint256 amount) {
        if (amount == 0) { revert ZeroAmount("ETH_SPC_LP"); }
        _;
    }
     ...
     */
    function addLiquidity(uint256 spc) external payable nonZeroEth nonZeroSpc(spc) {
        ...
        if (reserveEth == 0 && reserveSpc == 0) {
            ...
            uint256 amountSpcOptimal = amountEthOffered * reserveSpc / reserveEth;
            if (amountSpcOptimal <= amountSpcOffered) {
                amountEthToAdd = amountEthOffered;
                amountSpcToAdd = amountSpcOptimal;
            }
            else {
                uint256 amountEthOptimal = amountSpcOffered * reserveEth / reserveSpc;
        ...
        if (spaceCoin.isTaxed()) {
            uint256 ethDeduction = amountEthToAdd * spaceCoin.TAX_RATE_BP() / spaceCoin.BP_DIVISOR();
     ...
     */
    function removeLiquidity(uint256 lpToken) external nonZeroEthSpcLp(lpToken) {
     ...
     */
    function swapETHForSPC(uint256 spcOutMin) external payable nonZeroEth liquidityExists {
     ...
     */
    function swapSPCForETH(uint256 spcIn, uint256 ethOutMin) external nonZeroSpc(spcIn) liquidityExists {
    ...
    function refund(address to, uint256 amount) private {
        (bool success, ) = to.call{value: amount}("");
        if (!success) { revert RefundFailed(to, amount ); }
    }
