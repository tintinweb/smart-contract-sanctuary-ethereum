...
contract ProtectionPlanFactory is ProtectionPlanFactoryStorage, IProtectionPlanFactory, Ownable {
    ...
    constructor(address _protocolDirectoryAddr, address signer) {
    ...
    function createNewProtectionPlan(uint256 _nonce, uint256 _deadline, bytes memory _signature) external {
        ...
        if(!SignatureChecker.isValidSignatureNow(_signer, messageHash, _signature)) revert InvalidSignature();
        if(_nonces[_nonce]) revert NonceAlreadyUsed();
