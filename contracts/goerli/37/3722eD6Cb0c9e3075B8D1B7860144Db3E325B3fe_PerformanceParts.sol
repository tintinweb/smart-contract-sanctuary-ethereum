...
import "./lib/ERC1155Burnable.sol";
import "./lib/ERC1155Supply.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

/// @author NitroLeague.
contract PerformanceParts is
    ERC1155,
    Mintable,
    ClaimContext,
    Allowlists,
    ...
    ERC1155Supply
{
    ...
    function mintPerformanceParts(uint256 _quantity)
        public
        nonReentrant
        whenNotPaused
        allowedToMint(_quantity)
    {
        ...
        uint[] memory amounts = new uint[](_quantity);

        uint _totalTokens = getTotalTokens();
        uint amount = allowlists[currentList].amount;
        for (uint i = 0; i < _quantity; i++) {
            ids[i] = randomNumber(i) % _totalTokens;
            amounts[i] = amount;
        }
    ...
    function getIsMetaLocked() external view returns (bool isMetaLocked) {
 ...
 */
contract ERC1155 is ERC2771Context, ERC165, IERC1155, IERC1155MetadataURI {
     ...
     * @dev See {_setURI}.
     */
    constructor(string memory uri_, address _forwarder)
...
abstract contract Allowlists is Mintable {
    struct Allowlist {
        string listName;
        uint allowed;
        uint amount;
    ...
    constructor() {
        allowlists.push(Allowlist("public", 1, 1));
        listIndices["public"] = 1;
    }

    function getRemainingMints(address account, uint listIndex)
        public
        view
        returns (uint)
    {
        ...
        returns (uint listIndex)
    {
        ...
        require(_listIndex > 0, "Invalid List");
        return _listIndex - 1;
    }
    ...
    modifier allowedToMint(uint quantity) {
        ...
        require(
            getRemainingMints(_msgSender(), currentList) >= quantity,
            "Quantity > Allowed"
        );
        _;
    }
    ...
    function createAllowlists(
        ...
        require(amounts.length == allowed.length, "Array Lengths Mismatch");

        for (uint i = 0; i < listNames.length; i++) {
            allowlists.push(Allowlist(listNames[i], allowed[i], amounts[i]));
            listIndices[listNames[i]] = allowlists.length;
        }
    }

    function setCurrentList(uint listIndex) public onlyOwner {
        require(listIndex < allowlists.length, "Invalid List Index");
        ...
        validListIndex(listIndex)
    {
        ...
        for (uint i = 0; i < accounts.length; i++) {
            ...
            /* require(mintCount[listIndex][account] == 0, "Duplicate Entry"); */
    ...
    function removeFromAllowlist(uint listIndex, address account)
        ...
        validListIndex(listIndex)
    {
        mintCount[listIndex][account] = allowlists[currentList].allowed;
    }

    modifier validListIndex(uint listIndex) {
        /**Check list index is neither Public nor invalid */
        require(
    ...
    function setMaxMints(
        ...
        uint allowed,
        uint amount
    ) external onlyOwner validListIndex(listIndex) {
        allowlists[listIndex].allowed = allowed;
        allowlists[listIndex].amount = amount;
    }
 ...
 * the minter.
 */
abstract contract Mintable is MetaOwnable, Pausable {
...
abstract contract ERC1155Supply is ERC1155 {
     ...
     * @dev Total amount of tokens in with a given id.
     */
    function getTotalTokens() public view virtual returns (uint256) {
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                if (!ERC1155Supply.exists(ids[i])) _tokenCounter.increment();
        ...
        if (to == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                ...
                if (_totalSupply[id] == 0) _tokenCounter.decrement();
            }
        }
    }
}
 ...
 * the owner.
 */
abstract contract MetaOwnable is ERC2771Context {
