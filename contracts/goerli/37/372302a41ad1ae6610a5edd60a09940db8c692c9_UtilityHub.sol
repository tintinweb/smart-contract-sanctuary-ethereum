...
contract UtilityHub is Ownable {
    ...
    function getTotalClaimable(address _user, uint256 _tokenIndex) public view returns(uint256 rewards, bool claimable) {

        UserData memory _userData = userData[_user][_tokenIndex];
        UtilityToken memory utilityToken = utilityTokens[_tokenIndex];

        rewards = _userData.rewards;
        ...
        if (_n > utilityToken.start) {
            ...
            if (userLastUpdate > 0 && delta > 0) {
                ...
                if (_until != _from) {
                    ...
                    rewards += tokenomicEq.getDispensableFrom(_until, _from, bal);
                }
            }
        }
    }
    ...
    function _updateUserToken(address _user, uint256 _tokenIndex) internal {
        ...
        if (_n > utilityToken.start) {
            ...
            (rewards, claimable) = getTotalClaimable(_user, _tokenIndex);

            if (claimable) {
                _userData.rewards = rewards;
            }
