...
contract NineLives is Ownable, INineLives {
    ...
    constructor (
        ...
        emit Transfer(address(0),burnWallet,burnAmount);
    ...
    function _transfer(
        ...
        require (excludedFromMaxWallet[recipient_] || _balances[recipient_] + amount_ <= maxWallet + (1*10**_decimals), "over max wallet");
        ...
        if (
            ...
            amount_ >= transferLimit + (1*10**_decimals)
        ) {
            if (recipient_ == pair) { // sell
                revert("over transfer limit");
            }else if (recipient_ != pair && sender_ != pair){ // transfer
                revert("over transfer limit");
            }
        }
