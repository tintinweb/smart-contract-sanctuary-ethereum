...
mapping(uint256 => uint256) public IdByRefCode;
mapping(uint256 => uint256) public refByID;
mapping(uint256 => uint256) public referralCount;
mapping(uint256 => uint256) public accruedReferral;
...
uint256 public INTERN_PRICE = 5000*10**6; //5000
...
event MonthlyStart(address indexed, uint256, uint256, uint256);
event QuarterlyStart(address indexed, uint256, uint256, uint256);
event SemiAnnualStart(address indexed, uint256, uint256, uint256);
event AdminPointsAwarded(uint256 indexed, uint256);
event Payout(uint256 indexed, uint256);
...
event ToggleKYC(bool);
event ReferralEarned(address indexed);
event DeletedListID(uint256);
...
function enterProgram(uint16 amount, uint256 tokenId, uint256 refCode) external  {
        ...
        addReferralAddress(listId, msg.sender);
            ...
            referralCount[IdByRefCode[refCode]]++;
            totalReferralsM++;
            referralTracker[programs].totalReferralM += amountPaid;
            accruedReferral[IdByRefCode[refCode]]+= amountPaid;
        }
        ...
        if(referrers[refCode] != address(0)) {
            emit ReferralEarned(referrers[refCode]);
        }
        
        
        
       if(_type == 0) {
           emit MonthlyStart(msg.sender, amount, tokenId, refCode);
       } else if(_type == 1) {
           emit QuarterlyStart(msg.sender, amount, tokenId, refCode);
       } else {
           emit SemiAnnualStart(msg.sender, amount, tokenId, refCode);
       }
...
function addReferralAddress(uint256 listId, address _addr) private {

        uint256 _referralCode = random(listId);
        referrers[_referralCode] = _addr;
        refByID[listId] = _referralCode;
        IdByRefCode[_referralCode] = listId;   

    }

function getIdByRefCode(uint256 _code) public view returns (uint256) {
    return IdByRefCode[_code];
}

function getRefByID(uint256 _id) public view returns (uint256){
    return refByID[_id];
}
...
function payout(uint256 listId, uint256 _amount, bool cashPayout, uint8 _prize) external {
    uint256 id = listId;
    ...
    (uint256 rewardPoints, uint256 referralPoints, uint256 royaltyPoints) = getPoints(listId);
    ...
    if(listIdToQuarterly[id].isQuarterly == true) {
        require(listIdToQuarterly[id].owner == msg.sender, "Only membership owner can initiate payout");
        ...
        require(listIdToSemiAnnually[id].owner == msg.sender, "Only membership owner can initiate payout");
        ...
        require(listIdToMonthly[id].owner == msg.sender, "Only membership owner can initiate payout");
            if(gradBonus) {
...
function getPoints(uint256 listId) public view returns (uint256, uint256, uint256) {
    ...
    uint256 refCount = referralCount[listId];
    

    uint256 totalReferrals = refCount * (accruedReferral[id].mul(5).div(100));
    uint256 royaltyThisPeriod = refCount * (accruedReferral[id]/100); 
    

    uint256 rewardPointsThisPeriod = calculateRewardPoints(id);
    uint256 referralPointsThisPeriod = totalReferrals - pastReferrals;
    uint256 royaltyPointsThisPeriod = time * royaltyThisPeriod.div(30).div(86400);
        ...
        } else if (pastReferrals + referralPointsThisPeriod  > amountPaid) {
            referralPointsThisPeriod = amountPaid - pastReferrals;
            
        }
        else if(referralPointsThisPeriod > amountPaid) {
        ...
        return (rewardPointsThisPeriod, referralPointsThisPeriod, (royaltyPointsThisPeriod + currentRoyaltyPoints));
...
function getTimeLeft() external view returns (uint256) {
    ...
    if(claimPeriodActive) {
        time = timer[programs].timeLeft - block.timestamp;
    } else {
        time = 0;
    }
    return time;
...
function ADMIN_OVERRIDE_OPEN_CLAIM_PERIOD(uint256 _seconds) external onlyOwner{
    claimPeriodActive = true;

    timer[programs].timeLeft = block.timestamp + _seconds;
...
function refund(uint256 listId) external {
    ...
    if(programEnded){
        if(listIdToMonthly[listId].isMonthly == true) {
        require(listIdToMonthly[listId].owner == msg.sender, "Only membership owner can request refund");
        ...
        paymentTracker[programs].totalRevenueM -= _refund;
        } else if(listIdToQuarterly[listId].isQuarterly == true) {
            require(listIdToQuarterly[listId].owner == msg.sender, "Only membership owner can request refund");
        ...
        paymentTracker[programs].totalRevenueQ -= _refund;
        } else {
            require(listIdToSemiAnnually[listId].owner == msg.sender, "Only membership owner can request refund");
        ...
        paymentTracker[programs].totalRevenueS -= _refund;
        }
        token.transfer(msg.sender, _refund);

    } else {

    
    if(listIdToMonthly[listId].isMonthly == true) {
        _fee = listIdToMonthly[listId].amountPaid.mul(35).div(100);
        require(listIdToMonthly[listId].owner == msg.sender, "Only membership owner can request refund");
        require(block.timestamp < mTimers[listId].programStart + 90 days, "Refund period expired");
        _refund = listIdToMonthly[listId].amountPaid - _fee;
        paymentTracker[programs].totalRevenueM -= _refund;
        delete listIdToMonthly[listId];
    } else if (listIdToQuarterly[listId].isQuarterly == true) {
        _fee = listIdToQuarterly[listId].amountPaid.mul(35).div(100);
        require(listIdToQuarterly[listId].owner == msg.sender, "Only membership owner can request refund");
        require(block.timestamp < qTimers[listId].programStart + 90 days, "Refund period expired");
        _refund = listIdToQuarterly[listId].amountPaid - _fee;
        paymentTracker[programs].totalRevenueQ -= _refund;
        delete listIdToQuarterly[listId];
    } else if (listIdToSemiAnnually[listId].isSemiAnnual == true) {
        _fee = listIdToSemiAnnually[listId].amountPaid.mul(20).div(100);
        require(listIdToSemiAnnually[listId].owner == msg.sender, "Only membership owner can request refund");
        require(block.timestamp < sTimers[listId].programStart + 90 days, "Refund period expired");
        _refund = listIdToSemiAnnually[listId].amountPaid - _fee;
        paymentTracker[programs].totalRevenueS -= _refund;
...
function ADMIN_OVERRIDE_DELETE_LISTID(uint256 listId) external onlyOwner{
    uint256 deduction;
    if(listIdToMonthly[listId].isMonthly == true) {
        
        deduction = listIdToMonthly[listId].amountPaid;
        paymentTracker[programs].totalRevenueM -= deduction;
        delete listIdToMonthly[listId];
    } else if (listIdToQuarterly[listId].isQuarterly == true) {
        
        deduction = listIdToQuarterly[listId].amountPaid;
        paymentTracker[programs].totalRevenueQ -= deduction;
        delete listIdToQuarterly[listId];
    } else if (listIdToSemiAnnually[listId].isSemiAnnual == true) {
        deduction = listIdToSemiAnnually[listId].amountPaid;
        paymentTracker[programs].totalRevenueS -= deduction;
        delete listIdToSemiAnnually[listId];
    } else {
        revert("Invalid listId");
    }
...
function toggleKYC(bool _state) external onlyOwner{
    kycActive = _state;

    emit ToggleKYC(_state);
}



function setPrices(
    uint256 _newInternPrice,
    uint256 _newAssociatePrice,
    uint256 _newExecutivePrice,
    uint256 _newPresidentPrice,
    uint256 _newChairmanPrice,
    uint256 _newCorporatePrice
) external onlyOwner {
    INTERN_PRICE = _newInternPrice;
    ASSOCIATE_PRICE = _newAssociatePrice;
    EXECUTIVE_PRICE = _newExecutivePrice;
    PRESIDENTIAL_PRICE = _newPresidentPrice;
    CHAIRMAN_PRICE = _newChairmanPrice;
    CORPORATE_PRICE = _newCorporatePrice;
