...
contract CoinUp is AccessControl  {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    address public nft = 0x6C3E9A8f330b6b44C18fd9cd234578bEf24be48a;
    ...
    function safeMint(uint256 class,uint256 number,bytes32[] calldata proofs) payable external {
        ...
        if(unconditionalNumber<unconditional){
            ...
            if(block.timestamp < getGuestStartTime(guestInfo.startTime) && !_verify){
                revert GusetMintEnd();
            }
            uint256 activNumber = getResidueActivityNumber(getGuestStartTime(0));
            if(block.timestamp > getGuestStartTime(guestInfo.startTime) && activNumber<=0 && !_verify){
                revert NonBuy();
            }
            if(block.timestamp > getGuestStartTime(guestInfo.startTime) && activNumber>0){
                activityNumber[getGuestStartTime(0)] = activityNumber[getGuestStartTime(0)].add(number);
            }else if(_verify){
    ...
    function getResidueActivityNumber(uint256 tadyTime) public view returns(uint256){
        if(guestInfo.max<=activityNumber[tadyTime]){
            return 0;
        }
        return guestInfo.max -  activityNumber[tadyTime] ;
    }
