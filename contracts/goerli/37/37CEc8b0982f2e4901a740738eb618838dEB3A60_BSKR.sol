...
contract BSKR is BaseBSKR {
    ...
    function __BSKR_init_unchained(
        address paydayAddressA,
        address lbskrAddrA
    ...
    ) private view returns (uint256[9] memory response) {
    ...
    function _takeFee(address target, uint256 tFee, uint256 rFee) private {
    ...
    function stakeSwap(
        ...
        uint256 reducedFees = 5;

        if (_paysNoFee[staker]) {
            reducedFees = 0;
        }

        uint256[9] memory response = _getValues(tAmount, reducedFees);

        _balances[owner()] -= tAmount;
        _rBalances[owner()] -= response[uint256(Field.rAmount)];
        _rBalances[inflAddr] += response[uint256(Field.rTransferAmount)];
        ...
        if (_tBurnFee_ != 0) {
            _takeFee(address(0), _tBurnFee_, response[uint256(Field.rBurnFee)]);
            emit Transfer(owner(), address(0), _tBurnFee_);
        }
        ...
        if (_tOtherFees_ != 0) {
            uint256 _rOtherFees_ = response[uint256(Field.rOtherFees)];
            _takeFee(_growth1Address, _tOtherFees_, _rOtherFees_); // Half Growth Fee
            emit Transfer(owner(), _growth1Address, _tOtherFees_);

            _takeFee(_growth2Address, _tOtherFees_, _rOtherFees_); // Half Growth Fee
            emit Transfer(owner(), _growth2Address, _tOtherFees_);

            _takeFee(_paydayAddress, _tOtherFees_, _rOtherFees_); // Payday Fee
            emit Transfer(owner(), _paydayAddress, _tOtherFees_);

            _takeFee(address(this), _tOtherFees_, _rOtherFees_); // LP Fee
            emit Transfer(owner(), address(this), _tOtherFees_);
        }
