...
import {LUSD3CRVValidator, IStableSwapPriceHelper, IMetaPoolPriceHelper} from "../validators/LUSD3CRVValidator.sol";
...
contract LUSD3CRVValidatorScript is Script {
    function run() external {
        ...
        address stableSwapPriceHelperAddress = address(
            0xE04DCa24622c5E0beaB778785F66c2a61f847A73
        );
        address metaPoolPriceHelperAddress = address(
            0xB155A0F3a6394b8a34cd42a00A43E7670B5bDbb7
        );
        ...
        IStableSwapPriceHelper stableSwapPriceHelper = IStableSwapPriceHelper(
            stableSwapPriceHelperAddress
        );
        IMetaPoolPriceHelper metaPoolPriceHelper = IMetaPoolPriceHelper(
            metaPoolPriceHelperAddress
        );

        LUSD3CRVValidator validator = new LUSD3CRVValidator(
            helper,
...
interface IMetaPoolPriceHelper {
    function get_virtual_price(
        uint256[2] memory balancer,
...
interface IStableSwapPriceHelper {
    function get_virtual_price(
...
contract LUSD3CRVValidator is IProofValidator, IValidator {
    ...
    /// ======== Custom Errors ======== ///
    error ChainlinkValidator__validate_invalidBlockNumber();
    error ChainlinkValidator__validate_invalidNonce();
    error ChainlinkValidator__getChainlinkPrice_invalidTimestamp();
    error ChainlinkValidator__verifyAndExtractTokenStorage_invalidAccount();
    error ChainlinkValidator__verifyAndExtractTokenStorage_invalidTotalSupply();

    /// ======== Events ======== ///
    event Validate(uint256 proposedValue, uint256 validatorValue);
    /// ======== Storage ======== ///

    /// @notice Blocks until a proposed value becomes outdated
    uint256 public constant proposeWindow = 50;
    /// @notice Blocks until a proposed value can not be disputed anymore [seconds]
    uint256 public constant disputeBlockWindow = 256;

    /// @notice Seconds until a chainlink feed value becomes outdated [seconds]
    uint256 public constant chainlinkTimeWindow = 24 * 3600;

    /// @notice Block header proof and chainlink round ids
    ...
    /// @notice Account and Storage proofs
    struct ProofWrapper {
        bytes curveAccount;
        bytes metaAccount;
        bytes tokenAccount;
    ...
    /// @notice Helper contract that handles price calculations
    LUSD3CRVHelper public immutable helper;
    IStableSwapPriceHelper public immutable stableSwapPriceHelper;
    IMetaPoolPriceHelper public immutable metaPoolPriceHelper;

    /// @notice Address of the 3Curve Stable Pool
    address public constant CURVE3POOL_ADDRESS =
        0x753B50733fD886bFF6C6790642592a82eb80fb88;
    /// @notice Hash of the 3Curve Stable Pool address
    bytes32 public constant CURVE3POOL_ADDRESS_HASH =
        keccak256(abi.encodePacked(CURVE3POOL_ADDRESS));

    /// @notice Address of the 3Curve LUSD Meta Pool
    address public constant LUSD3POOL_ADDRESS =
        0x68F1651d0911cc155026970E1F096cc1A48a4a0B;
    /// @notice Hash of the 3Curve LUSD Meta Pool address
    bytes32 public constant LUSD3POOL_ADDRESS_HASH =
        keccak256(abi.encodePacked(LUSD3POOL_ADDRESS));

    /// @notice Address of the 3Crv Token
    address public constant TOKEN_ADDRESS =
        0xFf634b8bdB5F207768A6fC9fe51a6c940Af9668D;
    /// @notice Hash of the 3Crv Token address
    bytes32 public constant TOKEN_ADDRESS_HASH =
        keccak256(abi.encodePacked(TOKEN_ADDRESS));

    /// @notice Storage location for totalSupply
    bytes32 public constant TOKEN_TOTAL_SUPPLY_POS = bytes32(uint256(5));
    /// @notice Hash of the totalSupply storage location
    bytes32 public constant TOKEN_TOTAL_SUPPLY_POS_HASH =
        keccak256(abi.encodePacked(TOKEN_TOTAL_SUPPLY_POS));

    constructor(
        LUSD3CRVHelper helper_,
        IStableSwapPriceHelper stableSwapPriceHelper_,
        IMetaPoolPriceHelper metaPoolPriceHelper_
    ) {
        helper = helper_;
        stableSwapPriceHelper = stableSwapPriceHelper_;
        metaPoolPriceHelper = metaPoolPriceHelper_;
    }

    /// ======== Proof Generation Helper Functions ======== ///

    /// @notice Returns the contract address and the storage positions for the 3Curve Pool
    ...
    /// @notice Returns the contract address and the storage positions for the LUSD Meta Pool
    ...
    /// @notice Returns the contract address and the storage for the 3crv Token
    ...
    /// @notice Retrieves the latest spot price for the LUSD3Curve Pool
    /// @return value_ Spot price [wad]
    /// @return nonce Encoded block.number and the hash of every chainlink feed roundId  [(block.number, roundIdHash)]
    /// @dev Check LUSD3CRVHelper.value() for more info on how the nonce is computed
    function value()
        external
        override(IProofValidator)
        returns (uint256 value_, bytes32 nonce)
    {
        /// Retrieve the spot price and the value from the helper contract
    ...
    /// @param prevNonce Nonce of the previous proposal [(block.number, roundIdHash)]
    /// @param nonce Nonce of the current proposal [(block.number, roundIdHash)]
    ...
    /// @notice Validates a `proposedValue` for given `nonce` by verifying needed storage
    /// and recomputing the value onchain
    /// @param proposedValue Value to be validated [wad]
    /// @param nonce Nonce of the `value_` [(block.number, roundIdHash)]
    /// @param encodedHeader Encoded HeaderWrapper
    /// @param encodedProofs Encoded ProofWrapper
    /// @return valid True if `value_` is equal to computed value
    /// @return computedValue Value that was computed from the verified storage
    function validate(
        ...
        // Decode ProofWrapper
        ProofWrapper memory proofs = abi.decode(encodedProofs, (ProofWrapper));

        // Verify the block header from the provided header proof
        StateProofVerifier.BlockHeader memory blockHeader = StateProofVerifier
            .verifyBlockHeader(header.blockHeaderRlpBytes);

        // Validate that the block number matches the nonce
        if (blockHeader.number != helper.decodeBlockNumber(nonce)) {
            revert ChainlinkValidator__validate_invalidBlockNumber();
        }

        // Recompute the roundId hash
        ...
        // Check that the chainlink round ids and the block number match the nonce
        if (
            helper.encodeNonce(
                uint64(blockHeader.number),
                uint192(roundIdHash)
            ) != nonce
        ) {
        ...
        // Verify and extract the stored values for the 3Curve pool
        StableSwap3PoolHelper.PoolStorage
            memory curve3PoolStorage = StableSwap3PoolHelper
                .verifyAndExtractStorage(
        ...
        // Verify and extract the stored values for the LUSD 3Curve meta pool
        Meta3PoolHelper.PoolStorage memory metaPoolStorage = Meta3PoolHelper
            .verifyAndExtractStorage(
        ...
        // Verify and extract the 3crv token totalBalance
        ...
        // Recompute the spot price from the validated values
    ...
    /// @notice Calculate the spot price from the validated storage
    /// @param curve3PoolStorage 3Curve pool storage
    /// @param metaPoolStorage 3Curve LUSD meta pool storage
    /// @param tokenTotalSupply 3crv token totalSupply
    /// @param timestamp Block timestamp used to compute the virtual prices
    /// @param usdcRoundId USDC Chainlink feed round Id
    /// @param daiRoundId DAI Chainlink feed round Id
    /// @param usdtRoundId USDT Chainlink feed round Id
    /// @param lusdRoundId LUSD Chainlink feed round Id
    /// @return value_ Spot price[wad]
    function _computeValue(
        ...
        // Validate and retrieve the price from the Chainlink feed
        ...
        uint256 curve3VirtualPrice = _getStableSwapVirtualPrice(
            curve3PoolStorage,
            tokenTotalSupply,
            timestamp
        );
        uint256 metapoolVirtualPrice = _getMetaPoolVirtualPrice(
            metaPoolStorage,
            curve3VirtualPrice,
            timestamp
        );
    ...
    function _getChainlinkPrice(
        ...
        //temp disable for testing purposes
        //todo uncomment this
        //if (blockTimestamp - timestamp > chainlinkTimeWindow)
        //revert ChainlinkValidator__getChainlinkPrice_invalidTimestamp();
    ...
    function _verifyAndExtractTokenStorage(
        ...
        if (!tokenAccount.exists) {
            revert ChainlinkValidator__verifyAndExtractTokenStorage_invalidAccount();
        }
        ...
        if (!totalSupply.exists) {
            revert ChainlinkValidator__verifyAndExtractTokenStorage_invalidTotalSupply();
        }

        totalSupply_ = totalSupply.value;
    }

    function _getStableSwapVirtualPrice(
        StableSwap3PoolHelper.PoolStorage memory poolStorage,
        uint256 tokenSupply,
        uint256 timestamp
    ) internal view returns (uint256) {
        return
            stableSwapPriceHelper.get_virtual_price(
                poolStorage.balances,
                tokenSupply,
                timestamp,
    ...
    function _getMetaPoolVirtualPrice(
        Meta3PoolHelper.PoolStorage memory poolStorage,
        uint256 baseVirtualPrice,
        uint256 timestamp
    ) internal view returns (uint256) {
        return
            metaPoolPriceHelper.get_virtual_price(
                poolStorage.balances,
                baseVirtualPrice,
                poolStorage.totalSupply,
                timestamp,
                poolStorage.initialA,
                poolStorage.futureA,
                poolStorage.initialATime,
