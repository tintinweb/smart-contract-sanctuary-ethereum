...
/// @notice Extends ERC721, mints NFT for 32 ETH
contract StakefishNFTManager is IStakefishNFTManager, ERC721Enumerable {
    ...
    function _mintOne() internal {
        ...
        emit StakefishMintedWithContract(tokenId, validatorAddr);
    }

    function mint(uint256 validatorCount) external override payable {
        require(msg.value == validatorCount * 32 ether, "nft: wrong send value");
    ...
    function burn(uint256 tokenId) external override isAuthorizedForToken(tokenId) {
        _burn(tokenId);
        _validatorToToken[_tokenToValidator[tokenId]] = 0;
        _tokenToValidator[tokenId] = address(0);
    }

    function claim(address, uint256) external override virtual {
...
interface IStakefishNFTManager is IERC721Metadata, IERC721Enumerable {
    ...
    event StakefishMintedWithContract(uint256 tokenId, address validatorContract);
