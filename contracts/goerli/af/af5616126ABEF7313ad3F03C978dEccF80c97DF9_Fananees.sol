...
import "./library/erc721A/extentions/ERC721AVestable.sol";
...
error Fananees__TokenIsLockedInVesting();
 ...
 */
contract Fananees is Neutron, ERC721AVestable, ERC721AQueryable {
	address private immutable i_payoutWallet;
	...
	constructor(
		...
		if (payoutWallet == address(0x0)) revert Fananees__ZeroAddressProhibited();
		i_payoutWallet = payoutWallet;
	}
	 ...
	 */
	function setStakingContract(IStaking stakingContract) external onlyOwner {
	...
	function withdrawMoney(address payable to) external {
		if (address(this).balance == 0) revert Fananees__NothingToWithdraw();
		if (msg.sender != i_payoutWallet) revert Fananees__NotAuthorized();

		to.transfer(address(this).balance);
	}

	// Override to prevent tokens from being moved when they are being staked or vested.
	/// @inheritdoc ERC721A
	function _beforeTokenTransfers(
		...
		for (uint256 tokenId = startTokenId; tokenId < startTokenId + quantity; tokenId++) {
			if (isTokenVested(tokenId)) revert Fananees__TokenIsLockedInVesting();

			if (!isStakingContractSet) continue;
...
import "./IERC721AVestable.sol";
import "../ERC721A.sol";

error ERC721AVestable__InvalidVestingConfiguration();

/**
 * @title ERC721AVestable
 * @author DeployLabs.io
 *
 * @dev This {ERC721} extension allows for vesting of tokens.
 * This means locking them from being transfered for a set period of time.
 */
abstract contract ERC721AVestable is ERC721A, IERC721AVestable {
	/// @dev Range, in which tokens are vested (locked from being transfered).
	VestingRange private s_vestingRange;

	/// @inheritdoc IERC721AVestable
	function setVestingRange(VestingRange memory vestingRange) external virtual override {
		if (vestingRange.fromTokenId > vestingRange.toTokenId)
			revert ERC721AVestable__InvalidVestingConfiguration();

		s_vestingRange = vestingRange;
	}

	/// @inheritdoc IERC721AVestable
	function clearVesting() external virtual override {
		delete s_vestingRange;
	}

	/// @inheritdoc IERC721AVestable
	function isTokenVested(uint256 tokenId) public view override returns (bool) {
		VestingRange memory vestingRange = s_vestingRange;

		if (tokenId < vestingRange.fromTokenId || tokenId > vestingRange.toTokenId) return false;
		if (block.timestamp > vestingRange.vestedTillUnixTime) return false;

		return true;
	}

	/// @inheritdoc IERC721AVestable
	function getCurrentVestingRange() public view override returns (VestingRange memory) {
		return s_vestingRange;
	}
}
...
interface IERC721AVestable is IERC721A {
	struct VestingRange {
		uint256 fromTokenId;
		uint256 toTokenId;
		uint32 vestedTillUnixTime;
	}

	/// @dev Set the range, in which tokens will be vested.
	function setVestingRange(VestingRange memory vestingRange) external;

	/// @dev Remove vesting from all tokens.
	function clearVesting() external;

	/// @dev Check, whether a token is vested or not.
	function isTokenVested(uint256 tokenId) external view returns (bool);

	/// @dev Get current range, in which tokens are vested (locked from being transfered).
	function getCurrentVestingRange() external view returns (VestingRange memory);
}
 ...
 * Version 1.1.0
 */
abstract contract Neutron is ERC721A, Ownable {
