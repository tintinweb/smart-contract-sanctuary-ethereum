...
contract RebasePair_Allowance is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    {
        (amountOut, amountInMax, path, profitUSD, netProfitUSD) = _getRebaseAmount(_pair);
        // check does enough allowance
        uint256 balance = IERC20(path[0]).allowance(_treasury, address(this));
        possible = path[0] != address(0) && balance >= amountInMax;
    }
        ...
        )
    {
        ...
        uint256 rebaseFeeUSD = ((340000 * tx.gasprice * fx.fxNT * 13) / 10) / 10**_oracleNTDecimal; //  13/10 - 30% buffer for extra fee
        ...
        (uint256 dif0, uint256 dif1) = (reserves.reserve0USD > averageUSD)
            ? (
                ...
                ((averageUSD - reserves.reserve1USD) * 1001500000000000000) / 1000000000000000000
            )
            : (
                ((averageUSD - reserves.reserve0USD) * 998500000000000000) / 1000000000000000000,
                ((reserves.reserve1USD - averageUSD) * 1001500000000000000) / 1000000000000000000
            );
        ...
        if (dif0 > dif1 && profitUSD > rebaseFeeUSD) {
            ...
            // amountInMax = (((reserves.reserve1 * amountOut * 1000) / reserves.reserve0) * amountOut * 997).add(1);
        ...
        if (dif1 > dif0 && profitUSD > rebaseFeeUSD) {
            ...
            // amountInMax = (((reserves.reserve0 * amountOut * 1000) / reserves.reserve1) * amountOut * 997).add(1);
