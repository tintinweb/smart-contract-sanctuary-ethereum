...
contract LKMembership is ERC1155Supply, Ownable, ReentrancyGuard, IERC2981 {
    ...
    string public symbol;              // LKM
    string public name;                 // LK Membership
    ...
    mapping(uint8 => Tier) public membershipTiers;
    ...
    event OGMembershipMinted(address indexed owner, uint256 _amount, address[] indexed contractsAddresses, uint256[] tokenIds);
    event MembershipMinted(address indexed owner, uint8 indexed membershipTier, uint256 _amount);
    ...
    function setTiers(
            ...
            uint32[] memory _saleStart, 
            uint32[] memory _saleEnd, 
            uint16[] memory _supply,
            uint256[] memory _price
        ) public onlyOwner {
        for (uint8 i = 0; i < _ids.length; i++) {
            membershipTiers[_ids[i]].saleStart = _saleStart[i];
            membershipTiers[_ids[i]].saleEnd = _saleEnd[i];
            membershipTiers[_ids[i]].tierName = _names[i];
            membershipTiers[_ids[i]].supply = _supply[i];
            membershipTiers[_ids[i]].mintPrice = _price[i];
        }
    }
    ...
    /// @param _tokenIds list of contract addresses
    /// Emits a {MembershipMinted} event.
    function redeemOG(address[] memory _addresses, uint256[] memory _tokenIds) external {
        require(mintActive,'mint disabled');
        require(_addresses.length == _tokenIds.length && _tokenIds.length > 0,'Ids addresses donot match');
        require(uint256(membershipTiers[1].supply) > super.totalSupply(1),'Exceeded max supply');
        uint256 time = (block.timestamp);
        require(time > uint256(membershipTiers[1].saleStart) && time < uint256(membershipTiers[1].saleEnd), 'Redeem not available');
        emit OGMembershipMinted(msg.sender, _tokenIds.length, _addresses, _tokenIds);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(collections[_addresses[i]], 'collection unsupported');
            IERC721 nftContract = IERC721(_addresses[i]);
            require(nftContract.ownerOf(_tokenIds[i]) == msg.sender,'unauthorized');
            bytes32 encodedToken = keccak256(abi.encode(_addresses[i],_tokenIds[i]));
            require(!usedTokens[encodedToken],'Token has been used');
            usedTokens[encodedToken] = true;
        }
        _mint(msg.sender,1,_tokenIds.length,"");
    }

    /// @notice Mints Freemiun cards
    /// @param _amount the amount of Freemiun cards to be minted, can't exceed 5 in a single transaction
    /// Emits a {MembershipMinted} event.
    function mintFreemium(uint256 _amount) external {
        require(mintActive,'mint disabled');
        require(_amount > 0 && _amount <= uint256(mintBatchLimit), 'Exceeded Batch limit');
        emit MembershipMinted(msg.sender, 2, _amount);
        _mint(msg.sender,2,_amount,"");
    }

    /// @notice Mints Premium cards, payable function
    /// @param _amount the amount of premium cards to be minted, can't exceed 5 in a single transaction
    /// Emits a {MembershipMinted} event.
    function mintPremium(uint256 _amount) external payable {
        require(mintActive,'mint disabled');
        require(_amount > 0 && _amount <= uint256(mintBatchLimit), 'Exceeded Batch limit');
        require(uint256(membershipTiers[3].supply) > super.totalSupply(3),'Exceeded max supply');
        require(msg.value >= membershipTiers[3].mintPrice * _amount, 'Insufficient ETH');
        uint256 time = (block.timestamp);
        require(time > uint256(membershipTiers[3].saleStart) && time < uint256(membershipTiers[3].saleEnd), 'Sale not live');
        emit MembershipMinted(msg.sender, 3, _amount);
