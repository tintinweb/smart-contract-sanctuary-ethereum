...
import {GoerliCompoundCTokenData, CErc20Interface} from "../data-holder/goerli/GoerliCompoundCTokenData.sol";

// solhint-disable max-line-length

/// @title Facet for Uniswap callbacks
contract UniswapCallbackFacetGoerli is IUniswapV3SwapCallback, WithStorage, UniswapData, GoerliCompoundCTokenData {
    ...
    constructor(address _factory, address _WETH9, address _router) UniswapData(_factory, _WETH9, _router) GoerliCompoundCTokenData() {}
    ...
    function uniswapV3SwapCallback(
        ...
        CErc20Interface cTokenIn = cToken(tokenIn);
        CErc20Interface cTokenOut = cToken(tokenOut);
        ...
        if (tradeType == MarginSwapTradeType.UNISWAP_EXACT_OUT_WITHDRAW) {
            ...
            if (data.path.hasMultiplePools()) {
                ...
                // withraw and send funds to the pool
                cTokenOut.redeemUnderlying(amountToPay);
        ...
        if (tradeType == MarginSwapTradeType.OPEN_MARGIN_SINGLE) {
            ...
            cTokenOut.mint(amountToSupply + data.providedAmount);
            cTokenIn.borrow(amountToBorrow);
            TransferHelper.safeTransfer(tokenIn, msg.sender, amountToBorrow);
        ...
        if (tradeType == MarginSwapTradeType.OPEN_MARGIN_MULTI_EXACT_IN) {
            ...
            cToken(data.path.getLastToken()).mint(amountToSupply + data.providedAmount);
        ...
        if (tradeType == MarginSwapTradeType.OPEN_MARGIN_MULTI_EXACT_OUT) {
            ...
            // we supply the amount received directly - together with user provided amount
             cToken(tokenIn).mint(amountToSupply + data.providedAmount);
        ...
        if (tradeType == MarginSwapTradeType.SWAP_BORROW_MULTI_EXACT_IN) {
            ...
            cToken(data.path.getLastToken()).repayBorrow(amountToRepay);
            cTokenIn.borrow(amountToBorrow);
        ...
        if (tradeType == MarginSwapTradeType.SWAP_BORROW_MULTI_EXACT_OUT) {
            ...
            cTokenIn.repayBorrow(amountToSupply);
        ...
        if (tradeType == MarginSwapTradeType.UNISWAP_EXACT_OUT_BORROW) {
            ...
            if (data.path.hasMultiplePools()) {
                ...
                tokenIn = tokenOut; // swap in/out because exact output swaps are reversed
                cToken(tokenIn).borrow(amountToPay);
        ...
        if (tradeType == MarginSwapTradeType.TRIM_MARGIN_MULTI_EXACT_OUT) {
            ...
            // withraw from cToken
            cTokenIn.repayBorrow(amountToRepay);
