 ...
 * It's a convenience tool that lets you use your precious vaulted NFTs utility
 *
 ...
 * method with a hot wallet address to a the vault address, and redirect their business
 * logic to load NFTs for display, token gating, or whatever from that vault. All without
...
contract ProxyBot is IProxyBot, ERC721AQueryable, AccessControl {
   ...
   */
  mapping(address => uint256[]) public boomerangedTokenIdsPerVaultWallet;

  /**
   * @dev 
   * A record of each token ID and when it was first minted. This is used to 
   * determine the "age" of a token, mostly for display purposes.
   */
  ...
  /// @dev Triggered after the constructor is done doing its business.
  event Deployed();
...
/* ------------------------------------------------------------------------
  ...
  **/
  ...
  /**
  * @notice
  * Returns the Status enum of the token with the given ID.
  * 
  * @dev
  * It depends on its transfer history (boomeranging) and the transfer activity of its peers.
  * 
  * We talk about "boomeranging" a lot.
  * That's the short way of saying "minted the token to the vault and then the vault sent it to a hot wallet".
  *
  * - "Pending": it's never been boomeranged: it's just sitting in the minter wallet, which presumably is a vault but hasn't been delagated to anyone.
  *    
  * - "Connected": it's been boomeranged at least once AND is the most recently boomeranged token for its vault wallet.
  *   Note! Subsequent boomeranging of tokens for the same vault wallet will render previously boomeranged tokens invalid.
  *  
  * - "Void" if it's been boomeranged at least once (so it was valid at some point) AND is NOT the most recently boomeranged token for its vault wallet.
  * 
  ...
  * Also, if you are claiming to be minting a special edition, pass in that info and we'll verify it on-chain.
  ...
  **/
  function mint(address to, address expectedSpecialEditionContract, bool checkSpecificTokenId, uint256 editionTokenId) public payable {
    if(stopMinting){
      revert MintStoppedError();
    }

    // Record when this mint occurred.
    mintedBlockNumbers[totalSupply()] = block.number;

    // Record the first address this token was minted to, which is the vault wallet address.
    vaultWallets[totalSupply()] = to;
    ...
    if(expectedSpecialEditionContract != address(0)){
      ...
      (bool specialEditionExists, string memory specialEditionName) = validateSpecialEdition(to, expectedSpecialEditionContract, checkSpecificTokenId, editionTokenId);

      // If it is, log the special edition:
      if(specialEditionExists) appliedSpecialEditions[totalSupply()] = specialEditionName;
    }
    _mint(to, 1);
  }
  ...
  * The return is a tuple, with the first bool whether that hold is correct, and the second value is a string with the name of the special edition.
  * 
  ...
  **/
  function validateSpecialEdition(
    ...
    return IProxyBotSpecialEditionInspector(specialEditionInspector).validateSpecialEdition(ownerAddress, contractAddress, checkSpecificTokenId, editionTokenId);
  }

  /* ------------------------------------------------------------------------
      A C C E S S O R S
  ------------------------------------------------------------------------ */
  // Return various bits of info for specific token ids.
  // If not used by anyone else, they'll be used by the renderer contracts.

  /// @notice Get the minted block number for the given token ID.
  /// @param _tokenId The token to get the minted block number for.
  /// @return The minted block number.
  function getMintedBlock(uint256 _tokenId) public view returns (uint256) {
    return mintedBlockNumbers[_tokenId];
  }

  /**
  * @notice
  * Given a (hot) wallet's address, returns the address of the vault it proxies to, if any active connections exist.
  * This is probably the method to call on the client side for proxy access purposes.
  * 
  ...
  * E.g, it's been minted to its (presumably) vault wallet, and then transferred to its (presumably) hot wallet.
  * 
  * @param tokenId The token to check.
  **/
  ...
  /**
   * @notice
   * How many connected tokens does the given wallet hold? Call this off-chain probably, for web UI purposes.
  ...
  **/
  function connectedTokensCountForVault(address a) public view returns (uint256) {
    ...
    uint256 activeTokenCount = 0;
    for(uint256 i = 0; i < boomerangedTokenIds.length; i++){
  ...
  // @return A tuple, with the first value being a bool indicating whether a special edition exists, and the second value being the name of the special edition.
  function getAppliedSpecialEdition(uint256 tokenId) public view returns (bool, string memory) {
    ...
    if(ProxyBotConfig.stringsEqual(e, "")){
      return (false, e);
    } else {
      return (true, e);
    }
  ...
  /* ------------------------------------------------------------------------
      A D M I N   A N D  M A I N T E N A N C E
  ------------------------------------------------------------------------ */
  ...
  **/
  ...
  /// Intended to be used in case of emergency, or if we want to force new users over to an upgraded contract.
  ...
  /* ------------------------------------------------------------------------
      M I S C .  B O O K K E E P I N G
  ------------------------------------------------------------------------ */

  /** 
  * @notice
  * The _afterTokenTransfers hook does a lot of the heavy lifting for Proxy Bot.
  * The address a token gets transferred to, and from whom, drives the core functionality of tracking token connectivity.
  * And since we're updating state, we emit MetadataUpdate as appropriate to keep our views on OpenSea looking fresh.
  **/
    ...
    if(from != address(0)){
      ...
      for(uint256 i = 0; i < quantity; i++){
        ...
        if(transferCounts[tokenId] == 1){
          ...
          // Emit the MetaDataUpdated event for all the tokens that have been boomeranged, which includes this new one.
          ...
          if(!suppressMetadataUpdates){
            ...
            for(uint256 j = 0; j < length; ++j){
              // We can skip any but the most recently added and the one before that, since that first one has already been deactivated previously.
              // No need to update its metadata.
  ...
  // They say that "it is meant to be called off-chain", and we are, but we access it from another method here,
  // so the original `external` modifier is not appropriate.
