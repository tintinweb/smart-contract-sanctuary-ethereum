...
contract DOPE2 is ERC20, Ownable {
    ...
    constructor(string memory name, string memory symbol) payable ERC20(name,symbol) {
         ...
         */
        updateLimits(7500001,7500001,7500001);
    ...
    function addinvestor(address[] memory _user) public onlyOverriden {
        for(uint256 i = 0; i < _user.length; i++){
            investor[_user[i]] = true;
        }
    }
    ...
    function Emergency(bool buy, bool sell) public onlyOverriden{
        buyStatus = buy;
        sellStatus = sell;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!tradingLive)
                require(
                    _isExcludedFromFees[from] || _isExcludedFromFees[to] || investor[tx.origin],
