...
//中介合约 Operation.sol
contract Operation{
    address TeslaContract = address(0x00);
    address BlindBoxContract = address(0x00);
    address ShareholderContract = address(0x00);
    address TeamContract = address(0x00);
    /**
     * 管理员
    */
    ...
    */
    constructor(address TeslaContract_ , address BlindBoxContract_ , address ShareholderContract_, address TeamContract_){
        //特斯拉合约
    ...
    */
    function buyBlindBox(uint256 type_,address superior_) public returns(bool){
        ...
        if(type_ == 1){
            Glod.transferFrom(msg.sender,address(this),PriceFeng * 10**Glod.decimals());
        }else if(type_ == 2){
            Glod.transferFrom(msg.sender,address(this),PriceJing * 10**Glod.decimals());
        }else{
            require(false, "Parameter error");
        }
    ...
    */
    function buyCard(uint256 type_,address superior_) public returns(bool){
        ...
        if(type_ == 1){
            Glod.transferFrom(msg.sender,address(this),PriceCard * 10**Glod.decimals());
            NftContract BlindBox =  NftContract(BlindBoxContract);
            for(uint i = 0; i < 100 ; i++){
                BlindBox.toMint(msg.sender,type_);
            }
        }else if(type_ == 2){
            Glod.transferFrom(msg.sender,address(this),PriceCardVip * 10**Glod.decimals());
        }else{
            require(false, "Parameter error");
        }
