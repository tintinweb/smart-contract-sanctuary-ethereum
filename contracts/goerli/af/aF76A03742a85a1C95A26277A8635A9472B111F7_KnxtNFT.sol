...
contract KnxtNFTAdministrable is AccessControlEnumerable {
  ...
  function getAdministrators() external view onlyRole(ADMINISTRATOR_ROLE) returns (address[] memory) {
      uint256 administratorsCount = getRoleMemberCount(ADMINISTRATOR_ROLE);
      address[] memory administratorAddresses = new address[](administratorsCount);

      for (uint256 i = 0; i < administratorsCount; ++i) {
        administratorAddresses[i] = getRoleMember(ADMINISTRATOR_ROLE, i);
      }

      return administratorAddresses;
  }

  function addAdministrator(address newAdministratorAddress) external onlyRole(ADMINISTRATOR_ROLE) {
    _grantRole(ADMINISTRATOR_ROLE, newAdministratorAddress);
  }
