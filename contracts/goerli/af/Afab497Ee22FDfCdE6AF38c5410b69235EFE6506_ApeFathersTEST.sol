...
contract ApeFathersTEST is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981 {
    ...
    address public burnToMintContractAddress = 
        0x3F1D26522fA8e19F1ce578f2A422325c3c451ce7;
    ...
    function _directApproveMsgSender(uint256 tokenId) internal {
        assembly {
            mstore(0x00, tokenId)
    ...
    function burnERC721ToMint(uint256[] calldata _tokensToBurn)
        external
        nonReentrant
        originalUser
    {
        require(isBurnClaimActive, "CLAIM_NOT_ACTIVE");
        uint256 numberOfTokens = _tokensToBurn.length;
        require(numberOfTokens > 0, "NO_TOKEN_IDS_PROVIDED");
        require(
            totalSupply() + numberOfTokens <= MAX_SUPPLY,
            "MAX_SUPPLY_EXCEEDED"
        );
        ERC721A ExternalERC721BurnContract = ERC721A(address(burnToMintContractAddress));
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            require(
                ExternalERC721BurnContract.ownerOf(_tokensToBurn[i]) == msg.sender,
