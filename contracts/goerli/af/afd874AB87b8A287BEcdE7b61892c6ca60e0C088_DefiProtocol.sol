...
contract DefiProtocol is IERC721ReceiverUpgradeable, Initializable {
    ...
    function createCard(uint256 amount) external returns(uint256) {
        ...
        _tokenIds.increment();
        _card.mintCard(_tokenIds.current(), msg.sender, amount);
...
contract DefiCard is ERC721, Ownable, VRFConsumerBaseV2 {
    using SafeMath for uint256;
    ...
    function mintCard(uint256 cardId, address userAddress, uint256 _initialPower) external onlyOwner returns(uint256) {
        ...
        _requestIdToCardInfo[requestId] = CardInfo(cardId, userAddress, _initialPower);
        emit RequestSent(requestId, numWords);
        return requestId;
    }

    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        CardInfo memory requestCardInfo = _requestIdToCardInfo[_requestId];
