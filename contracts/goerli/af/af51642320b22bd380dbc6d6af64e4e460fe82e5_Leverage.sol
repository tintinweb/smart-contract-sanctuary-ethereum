...
contract Leverage is Ownable {
     ...
     * @param _reserve reserve asset to deposit
    ...
    function _deposit(
        ...
        if (_isETH) {
            ...
            IWETH9(WETH9).approve(POOL, msg.value);
            IPool(POOL).supply(WETH9, msg.value, _onBehalfOf, referral);
            ...
            IERC20(_reserve).approve(POOL, _reserveAmount);
            IPool(POOL).supply(_reserve, _reserveAmount, _onBehalfOf, referral);
    ...
    function _updateReserveBalances(
        ...
        if (_isAdd) {
            reserveBalance[msg.sender][_asset] += _amount;
        } else {
            require(
                reserveBalance[msg.sender][_asset] > _amount,
    ...
    function _updateLoanBalances(
        ...
        if (_isAdd) {
            loanBalance[msg.sender][_asset] += _amount;
        } else {
            require(
                loanBalance[msg.sender][_asset] > _amount,
    ...
    function _removeUserBalances() internal {
        for (uint i = 0; i < reserveList.length; i++) {
            ...
            reserveBalance[msg.sender][asset] = 0;
        }
    }

    /**
     * @notice add new asset to reserveList
     * @param _asset asset address
     **/
    ...
    function _calcAvailalbleLoanAmount(
        ...
        require(reservePrice > 0, "Can't get asset price correctly!");
        loanPrice = PRICE_ORACLE_GETTER.getAssetPrice(_loan);
        require(loanPrice > 0, "Can't get asset price correctly!");
        (, , , , ltv, ) = IPool(POOL).getUserAccountData(address(this));
        require(ltv > 0, "Can't get LTV correctly!");
        _loanAmount =
     ...
     * @param _reserve reserve asset to deposit
     * @param _reserveAmount reserve amount to deposit
     * @param _isETH flag whether the reserve asset is ETH
    ...
    function depositLeverage(
        ...
        _updateReserveBalances(_reserve, secondSwapAmount, true);
     ...
     * @param _loan loan asset to withdraw
     **/
    function getAvailableLoanAmount(
        ...
        for (uint i = 0; i < reserveList.length; i++) {
            address reservedAsset = reserveList[i];
            uint256 balance = reserveBalance[_user][reservedAsset];
            if (balance > 0) {
                availableLoanAmount += _calcAvailalbleLoanAmount(
                    reservedAsset,
                    _loan,
    ...
    function testDeposit(
        ...
        uint256 _reserveAmount,
        bool _isETH
    ) external returns (uint256 availableLoanAmount) {
        // first deposit
        (_reserve, _reserveAmount) = _deposit(
            _reserve,
            _reserveAmount,
