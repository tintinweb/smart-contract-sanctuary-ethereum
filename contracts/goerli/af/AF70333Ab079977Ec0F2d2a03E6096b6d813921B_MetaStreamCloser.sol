...
// Prototype courtesy stream closer
contract MetaStreamCloser {

	// GOERLI
	// forwarder 0x3075b4dc7085C48A14A5A39BBa68F58B19545971
	...
	uint256 constant GAS = 3_000_000;

	uint256 internal _batchId = 0;
	...
	/// @notice Generates message to sign with sender's account
	/// @param token Super token being streamed
	/// @param sender Stream sender
	/// @param receiver Stream receiver
	/// @return messageHash keccak256 hash of message
	/// @dev Signature should be handled with personal_sign, `0x` included
	function generateMessage(
		address token,
		address sender,
		address receiver
	) public view returns (bytes32 messageHash) {
		IForwarder.ERC20ForwardRequest memory req = _generateRequest(
			token,
			sender,
			receiver
		);
		return
			keccak256(
				abi.encodePacked(
					...
					req.tokenGasPrice,
					req.batchId,
					_forwarder.getNonce(req.from, req.batchId),
					req.deadline,
					keccak256(req.data)
				)
			);
	...
	/// @notice Closes a stream on a user's behalf without insolvency
	/// @param token Super token being streamed
	/// @param sender Stream sender
	/// @param receiver Stream receiver
	/// @param signature SECP256K1 signature of `generateMessage` output
	function closeStreamBeforeInsolvent(
		address token,
		address sender,
		address receiver,
		bytes memory signature
	) public {
		(bool success, ) = _forwarder.executePersonalSign(
			_generateRequest(token, sender, receiver),
			signature
		);
		require(success, "transaction failed");
	}

	/// @dev Generates request data to be encoded
	/// @param token Super token being streamed
	/// @param sender Stream sender
	/// @param receiver Stream receiver
	/// @return req generated request
	function _generateRequest(
		address token,
		address sender,
		address receiver
	) internal view returns (IForwarder.ERC20ForwardRequest memory req) {
		// initialize ops array _extactly_ like this for host's calldata to be
		// encoded properly.
		...
		return
			IForwarder.ERC20ForwardRequest({
				from: sender,
				to: address(_host),
				token: token,
				txGas: GAS,
				tokenGasPrice: 0,
				batchId: _batchId,
				batchNonce: _forwarder.getNonce(sender, _batchId),
				deadline: 0,
				data: abi.encodeWithSelector(
					_host.forwardBatchCall.selector,
					ops
				)
			});
	}
