...
/**
 * @title BaseCrowdfunding
 */

contract BaseCrowdfunding is Initializable {
    ...
    /// @notice project id to be created
    uint64 _projectIdTracker = 0;
    ...
    /// @notice mapping of all crowdfunding projects
    ...
    /// @notice protocol fee on percentage basis

    uint8 public protocolFee;

    /// @notice Event emitted when project is created
    event ProjectCreated(
        uint256 indexed projectId,
        string indexed projectTitle,
    ...
    modifier contributors(uint256 indexProject) {
        require(
            ...
            "BaseCrowdfunding: Owner cannot send funds to the project"
        );
        _;
    }
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "Base1155: only owner");
        _;
    }
     ...
     * @param data data for CrowdfundingProxy clone encoded
     * @param owner_ address of CrowdfundingProxy owner
     **/
        ...
        initializer
    {
        require(
            isBase == false,
            "BaseCrowdfunding: this is the base contract,cannot initialize"
        );

        require(
            owner == address(0),
            "BaseCrowdfunding: contract already initialized"
        );

        require(
            owner_ != address(0),
            "BaseCrowdfunding: Owner address cannot be 0"
        );

        (
            string memory name_,
            string memory symbol_,
            address payable protocolAddress_,
            uint8 protocolFee_
        ) = abi.decode(data, (string, string, address, uint8));

        require(
            protocolAddress_ != address(0),
            "BaseCrowdfunding: Protocol address cannot be 0"
        );

        require(
            protocolFee_ <= 10,
            "BaseCrowdfunding: Protocol fee cannot be more than 10%"
        );
        owner = owner_;
        name = name_;
        symbol = symbol_;
    ...
    /**
     * @notice changing the protocol address
     * @param protocolAddress_ new protocol address
     **/
    function changeProtocolAddress(address payable protocolAddress_)
        public
        onlyOwner
    {
        require(
        ...
        require(
            ...
            "BaseCrowdfunding: Cannot be same as previous"
        );
        protocolAddress = protocolAddress_;
    }

    /**
     * @notice changing the protocol fee
     * @param protocolFee_ new protocol fee
     **/
    function changeProtocolFee(uint8 protocolFee_) public onlyOwner {
        require(
    ...
    function createProject(
        ...
        projects[_projectIdTracker] = project;
        _projectIdTracker++;

        emit ProjectCreated(project.projectID, _projectTitle, msg.sender);
    }
        ...
        contributors(_projectID)
    {
        ...
        require(
            received >= projects[_projectID].projectParticipationAmount,
            "BaseCrowdfunding: Contribution is less than minimal project participation amount"
        );

        uint256 fee = (received * protocolFee) / 100;

        (bool success, ) = projects[_projectID].author.call{
            value: (received - fee)
        }("");
        require(success, "BaseCrowdfunding: Failed to forward funds");
        protocolAddress.call{value: fee};
