...
import {WisdomERC20} from "../erc20/WisdomCurrency.sol";

struct WisdomRendererConfig {
    string baseURI;
    string baseContractURI;
    address wisdomCurrencyContract;
}

contract WisdomRenderer is IMetadataRenderer {

    string public baseURI;
    string public baseContractURI;
    address public wisdomCurrencyContract;

    uint256 private MAX_GUI_LEVEL = 1000000000000000000;

    constructor(WisdomRendererConfig memory config) {
        baseURI = config.baseURI;
        baseContractURI = config.baseContractURI;
        wisdomCurrencyContract = config.wisdomCurrencyContract;
    }
    ...
    /// @notice A contract URI for the Wisdom Vessels contract
    /// @dev reverts if a contract uri is not provided
    /// @return contract uri for the contract metadata
    function contractURI() external view override returns (string memory) {
        if (bytes(baseContractURI).length == 0) revert();
        return baseContractURI;
    }

    /// @notice Converting the total harvestable Wisdom currency into 1-10 levels
    /// @return A level 1-10
    function calculateHarvestLevel(uint256 harvestable) internal view returns (uint8) {
        if (harvestable >= MAX_GUI_LEVEL) {
            return 10;
        }
        uint256 level = (harvestable * 9) / MAX_GUI_LEVEL + 1;
        return uint8(level);
    }

    /// @notice A token URI for the Wisdom Vessels contract
    /// @dev reverts if a base uri is not provided
    ...
    {
        if (bytes(baseURI).length == 0) revert();

        uint256 harvestable = WisdomERC20(wisdomCurrencyContract).previewHarvest(tokenId);
        uint8 level = calculateHarvestLevel(harvestable);
        return
            string(
                abi.encodePacked(
                    baseURI,
                    ...
                    "/",
                    StringsUpgradeable.toString(level)
                )
            );
    }
}
...
contract WisdomERC20 is ERC20 {
    ...
    function _calculateHarvest (uint256 tokenId) internal view returns (uint256) {
        ...
        // Protecting against underflow
        if (harvested >= totalHarvestable) {
            return 0;
        } else {
