...
contract OptimisticOracleScript is Script {
    ...
    function deploy(address collybusAddress_, IERC20 token_) public returns(OptimisticOracle){
        OptimisticOracle oracle = new OptimisticOracle(collybusAddress_,token_,1e18);
...
///@notice The Optimistic Oracle manages asset pricing for the FiatDAO protocol by
///pushing prices to Collybus. Staked proposers can submit price updates which
///can be disputed and validated with the help of onchain Validators.
///Proposers are rewarded by the FIAT protocol off-chain.
contract OptimisticOracle is Guarded, BondManager {
    ...
    error OptimisticOracle__shift_invalidParams();

    error OptimisticOracle_dispute_proposalNotDisputable();

    error OptimisticOracle__updateCollybus_invalidRelayerType(
        RateType relayerType_
    );
    ...
    /// ======== Storage ======== ///
    /// @notice Map of proposals for all the rate Ids
    /// @dev The ProposalHash is computed by encoding the rateId, proposed value and an extra auxiliary data parameter.
    /// @dev ProposalHash => ProposalState
    mapping(bytes32 => ProposalState) public proposals;

    /// @notice Rate config parameters
    /// @dev The rateId is an simple encoding depending on rateType because of how Collybus stores
    /// discount rates(uint256) or spot prices(address)
    /// @dev RateId => RateConfig
    mapping(bytes32 => RateConfig) public rates;

    /// @notice Address of the deployed Collybus contract
    address public immutable collybus;

    /// ======== Code ======== ///
    /// @param collybus_ Collybus address where the Oracle needs to be whitelisted to be able
    /// to call update sport/discount
    /// @param token_ The ERC20 token used by the Bond Management system for staking
    /// @param bondSize_ The value that needs to be paid by an actor to become a proposer for a rate
    /// @dev The token_ and the bondSize_ are used by the Bond Manager
    constructor(
        address collybus_,
        IERC20 token_,
        uint256 bondSize_
    ) BondManager(token_, bondSize_) {
        collybus = collybus_;
    }
    ...
    function setParam(
        ...
        if (param_ == "validator") {
            rates[rateId_].validator = uint160(value_);
    ...
    function registerRate(
        ...
        if (rates[rateId_].validator != uint160(0)) {
            revert OptimisticOracle__registerRate_rateAlreadyRegistered(
                rateId_
            );
        }

        rates[rateId_] = RateConfig(uint160(validator_), uint96(rateType_));
        ...
        proposals[computeProposalId(rateId_, 0, 0)] = ProposalState.Active;
    }
    ...
    function computeProposalId(
        ...
        unchecked {
            return keccak256(abi.encode(rateId_, value_, nonce_));
        }
    }
    ...
    /// needs to be validated and pushed.
    ...
    function shift(
        ...
        RateConfig memory rate = rates[rateId_];
        if (
            !IValidator(address(rate.validator)).canShift(
                rateId_,
                prevAuxData_,
                auxData_
            )
        ) {
            revert OptimisticOracle__shift_invalidParams();
        }

        // If the proposal is active then we need to push it
        // We skip this check if the prevAuxData is 0 because
        // we are in the first shift and we do not need push
        if (prevAuxData_ != bytes32(0)) {
            if (state == ProposalState.Active) {
                _push(RateType(rate.rateType), rateId_, prevValue_);
            }
        ...
        emit Shift(rateId_, msg.sender, value_, auxData_);
    }
    ...
    /// @param blockHeaderRlpBytes_ The RLP encoded block header which we will validate against
    /// onchain block hash historical data
    /// and use for storage proof and data extraction
    /// @param proofRlpBytes_ The RLP encoded storage proof we will use to validate and extract
    /// storage data needed by the Validator
    ...
    function dispute(
        ...
        uint256 value_,
        bytes32 auxData_,
        bytes calldata blockHeaderRlpBytes_,
        bytes calldata proofRlpBytes_
        ...
        if (
            !disputable(
                IValidator(address(rate.validator)),
                rateId_,
                proposalId,
                auxData_
            )
        ) {
            revert OptimisticOracle_dispute_proposalNotDisputable();
        }
        ...
        // We push the proposal by updating Collybus with the verified value
        proposals[proposalId] = ProposalState.Pushed;
        _push(RateType(rate.rateType), rateId_, verifiedValue);
    ...
    function disputable(
        ...
        // Check that the dispute is valid
        return (proposals[proposalId_] == ProposalState.Active &&
            address(validator_) != address(0) &&
            validator_.canDispute(rateId_, auxData_));
    }

    /// @notice Helper function that pushes a proposal by updating Collybus
    ...
    function _push(
        ...
        if (type_ == RateType.Discount) {
            ...
            revert OptimisticOracle__updateCollybus_invalidRelayerType(type_);
        }
    }
}
...
contract BondManager {
    ...
    /// ======== Errors ======== ///
    error BondManager__markProposer_invalidProposer(
        address proposer,
        bytes32 rateId
    );
    ...
    error BondManager__transferBond_invalidProposerId(bytes32 proposerId);
    error BondManager__registerProposer_alreadyRegisteredForRate(
        bytes32 rateId,
        bytes32 proposerId
    );
    error BondManager__unregisterProposer_isActive(bytes32 rateId);
    error BondManager__unregisterProposer_invalidProposerId(bytes32 rateId);

    /// ======== Events ======== ///
    event ProposerAdded(address proposer, bytes32[] proposerIds);
    ...
    /// @param bondSize_ Bond value in tokens
    constructor(IERC20 token_, uint256 bondSize_) {
        token = token_;
        bondSize = bondSize_;
    }
        ...
        returns (bytes32[] memory createdIds_)
    {
        ...
        uint256 paymentAmount = len * bondSize;
        ...
        for (uint256 idx = 0; idx < len; ++idx) {
            ...
            bytes32 proposerId = _computeProposerId(proposer_, rateIds_[idx]);
            ...
            proposerIds[proposerId] = ProposerState.Inactive;
    ...
    /// @dev Reverts if the proposerId was activated via markProposer and it's still in use
        ...
        public
    {
        uint256 len = rateIds_.length;
        for (uint256 idx = 0; idx < len; ++idx) {
            bytes32 proposerId = _computeProposerId(msg.sender, rateIds_[idx]);
        ...
        uint256 totalBondSize = len * bondSize;
        token.safeTransfer(receiver_, totalBondSize);
    ...
    function isProposer(address user_, bytes32 rateId_)
        public
        view
        returns (bool)
    {
        return
            proposerIds[_computeProposerId(user_, rateId_)] !=
            ProposerState.Unregistered;
    }
    ...
    {
        return proposerIds[_computeProposerId(user_, rateId_)];
    }

    /// @notice Returns the proposer hash for a rateId
    /// @param proposer_ The Address of the proposer
    /// @param rateId_ The rate id
    function _computeProposerId(address proposer_, bytes32 rateId_)
        internal
        pure
        returns (bytes32)
    {
        return keccak256(abi.encode(proposer_, rateId_));
    }
    ...
    function _clearProposer(address proposer_, bytes32 rateId_) internal {
        // Compute the proposerId hash based on the proposer address and the rateId
        bytes32 proposerId = _computeProposerId(proposer_, rateId_);
    ...
    function _transferBond(bytes32 proposerId_, address receiver_) internal {
        ...
        token.safeTransfer(receiver_, bondSize);
    ...
    function _transferBond(
        ...
        // Compute the hash and forward the call
        _transferBond(_computeProposerId(proposer_, rateId_), receiver_);
    }
}
