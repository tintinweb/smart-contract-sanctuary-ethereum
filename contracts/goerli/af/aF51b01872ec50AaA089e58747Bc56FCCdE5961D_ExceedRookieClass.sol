...
contract ExceedRookieClass is ERC721A, Ownable {
    ...
    string uncommonTokenUri;
    string rareTokenUri;
    string legendaryTokenUri;

    bool public isPublicSale = false;

    uint256 public MAX_SUPPLY = 3333;
    uint256 public UNCOMMON = 2000;
    uint256 public RARE = 1000;
    uint256 public LEGENDARY = 333;
    ...
    constructor(string memory _uncommonTokenUri, string memory _rareTokenUri, string memory _legendaryTokenUri) ERC721A("Exceed Rookie Class", "EXC") {
        uncommonTokenUri = _uncommonTokenUri;
        rareTokenUri = _rareTokenUri;
        legendaryTokenUri = _legendaryTokenUri;
    }
    ...
    function mint(uint256 _count, uint nftType) public payable saleIsOpen {
        ...
        if (msg.sender != owner()) {
            ...
            if (nftType == 1) {
                setBaseURI(uncommonTokenUri);
                require(mintIndex + _count <= UNCOMMON, "Total uncommon supply exceeded.");
            } else if (nftType == 2) {
                setBaseURI(rareTokenUri);
                require(mintIndex + _count <= RARE, "Total rare supply exceeded.");
            } else if (nftType == 3) {
                setBaseURI(legendaryTokenUri);
                require(mintIndex + _count <= LEGENDARY, "Total legendary supply exceeded.");
            }
        }
