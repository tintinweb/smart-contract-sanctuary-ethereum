...
contract CNFT is ERC721EnumerableUpgradeable, ICNFT {
  ...
  function mint(address to, uint256 tokenId) external override {
    require(AddressUpgradeable.isContract(_msgSender()), "CNFT: caller is not contract");
    require(!_exists(tokenId), "CNFT: exist token");
    require(IERC721Upgradeable(_underlyingAsset).ownerOf(tokenId) == _msgSender(), "CNFT: caller is not owner");
  ...
  function burn(uint256 tokenId) external override {
    ...
    require(_minters[tokenId] == _msgSender(), "CNFT: caller is not minter");
  ...
  function flashLoan(
    ...
    for (i = 0; i < nftTokenIds.length; i++) {
      require(ownerOf(nftTokenIds[i]) == _msgSender(), "CNFT: caller is not owner");
    }
  ...
  function minterOf(uint256 tokenId) external view override returns (address) {
    ...
    require(minter != address(0), "CNFT: minter query for nonexistent token");
    return minter;
  }
