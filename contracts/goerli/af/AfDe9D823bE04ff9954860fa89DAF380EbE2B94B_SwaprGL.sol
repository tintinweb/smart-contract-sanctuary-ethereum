...
contract SwaprGL is BaseGovernanceUpgradeable {
    ...
    */
    ...
    function buyNowOrder(uint buyerPurchasedAmount, bytes memory data, uint split) external onlyMarketplace(data) {
        ...
        require(swaprWallet.isNFTLocked(order.lock, order.nftId), "ORDER_DOES_NOT_EXIST");
    ...
    */
    function buyNowAuction(bytes memory data) external onlyMarketplace(data) {
        ...
        Auction memory auction = _extractAuctionInfo(listingData);
        require(swaprWallet.isNFTLocked(auction.lock, auction.nftId), "AUCTION_DOES_NOT_EXIST");
    ...
    */
    function claim(bytes calldata data) external onlyMarketplace(data) {
        ...
        // if(_verify(claimant, listingData) || _verify(claimant, lastBid)){
        //     //for seller
        //     PayNow memory payOps = PayNow(auction.toEOA, auction.acceptedToken, bid.bidder, claimant, swaprWallet.getBalance(bid.bidder,auction.acceptedToken), bid.offerPrice);
        //     require(payNow(payOps), "ERROR: PAYMENT_FAILED");
        //     //for buyer
    ...
    */
    function withdrawFunds(bytes calldata data) external onlyMarketplace(data) {
        (bytes memory info,) = abi.decode(data, (bytes, bytes));
        (address token, uint withdrawable) = abi.decode(info, (address, uint));
        _withdraw(token, withdrawable, _msgSender());
    }
    ...
    function _withdraw(address token, uint withdrawable, address claimant) internal {
        if(withdrawable > 0){
            if(token == address(0)){
                swaprWallet.releaseNative(claimant, withdrawable);
            }else{
                swaprWallet.releaseERC(token, claimant, withdrawable);
            }
        }
    }
...
contract SwaprWallet is BaseGovernanceUpgradeable, ERC721HolderUpgradeable {
    ...
    /// @dev this function is experimental and dev is advised to perform splitting directly through lock proxy
    /// @param lock lock proxy address
    /// @param nftId tokenId within the lock
