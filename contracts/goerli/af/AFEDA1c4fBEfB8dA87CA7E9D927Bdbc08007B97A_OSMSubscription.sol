...
abstract contract TokenReceiver is ERC721, IERC721Receiver {
   ...
   /**
     ...
     */
    function _removeTokenFromReceivedEnumeration(address contracted, uint256 tokenId) private {
        // To prevent a gap in from's tokens array, we store the last token in the index of the token to delete, and
        // then delete the last slot (swap and pop).

        uint256 lastTokenIndex = balanceOfWallet(msg.sender,contracted) - 1;
        uint256 tokenIndex = _receivedTokensIndex[contracted][tokenId];

        // When the token to delete is the last token, the swap operation is unnecessary
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = _receivedTokens[msg.sender][contracted][lastTokenIndex];

            _receivedTokens[msg.sender][contracted][tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
        ...
        delete _receivedTokens[msg.sender][contracted][lastTokenIndex];
    }    
    ...
    function onERC721Received(address, address from, uint256 tokenId, bytes memory) public virtual override returns (bytes4) {
        _addTokenToReceivedEnumeration(msg.sender, from, tokenId);
