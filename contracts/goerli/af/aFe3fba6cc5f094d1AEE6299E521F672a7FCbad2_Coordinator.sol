...
contract Coordinator is AccessControl, ICoordinator {
    IHedgehogToken public immutable HEDGEHOG_TOKEN;

    mapping(bytes32 => Types.DrawingInfo) internal DRAWINGS;
    bytes32[] internal DRAWING_IDS;

    address[] internal ADMINS;

    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN");

    constructor(IHedgehogToken _hedgehogToken)
    {
    ...
    function getDrawing(bytes32 drawingId)
        external
        view
    ...
    function getDrawing(string calldata drawingName)
        external
        view
        returns (Types.DrawingInfo memory)
    {
        bytes32 _drawingId = _drawingIdFromName(drawingName);
        return _getDrawing(_drawingId);
    }

    function _getDrawing(bytes32 drawingId)
        internal
        view
        returns (Types.DrawingInfo memory)
    {
        ...
        return IDrawing(payable(DRAWINGS[drawingId]._address)).drawingInfo();
    }
    ...
    function _getDrawings()
        internal
        view
        returns (Types.DrawingInfo[] memory)
    {
        Types.DrawingInfo[] memory _drawings = new Types.DrawingInfo[](DRAWING_IDS.length);
        for (uint i = 0; i < DRAWING_IDS.length; i++) {
            _drawings[i] = _getDrawing(DRAWING_IDS[i]);
        }
    ...
    function newDrawing(
        ...
        returns (Types.DrawingInfo memory)
    {
        bytes32 _drawingId = _drawingIdFromName(_drawingName);

        require(
            DRAWINGS[_drawingId]._address == address(0),
            "drawing with provided name already exists"
        );
        ...
        if (_minimumPrize > 0) {
            HEDGEHOG_TOKEN.mint(_thisAddr, _minimumPrize);
            require(
                _hhog.balanceOf(_thisAddr) >= _minimumPrize,
                "not enough HHOG minted"
            );
        }

        address payable _address = _deployDrawing(
            _drawingId,
            _drawingName,
            _drawingDate,
            _ticketPrice,
        ...
        if (_minimumPrize > 0) {
            require(
                _hhog.transferFrom(_thisAddr, _address, _minimumPrize),
        ...
        DRAWINGS[_drawingId] = IDrawing(_address).drawingInfo();

        DRAWING_IDS.push(_drawingId);

        return DRAWINGS[_drawingId];
    }

    function destroyDrawing(bytes32 drawingId)
        external
        onlyAdmin
    {
        Types.DrawingInfo storage _drawing = DRAWINGS[drawingId];

        require(
            _drawing._address != address(0),
            "unknown drawingId"
        );

        bytes32[] memory _drawingIds = new bytes32[](DRAWING_IDS.length - 1);

        uint idx = 0;
        for (uint i = 0; i < DRAWING_IDS.length; i++) {
            if (DRAWING_IDS[i] != drawingId) {
                _drawingIds[idx] = DRAWING_IDS[i];
                idx++;
            }
        }

        address _drawingAddress = _drawing._address;

        IDrawing _d = IDrawing(payable(_drawingAddress));
        _d.destroy();

        delete DRAWINGS[drawingId];
        DRAWING_IDS = _drawingIds;

        emit DrawingDestroyed(
            _drawingAddress,
            drawingId
        );
    }
        ...
        returns (address payable)
    {
        ...
        return payable(address(_newDrawing));
    }

    function _drawingIdFromName(string memory _drawingName)
        internal
        pure
        returns (bytes32)
    {
        return keccak256(bytes(_drawingName));
    }
}
...
interface ICoordinator {
    /**
     * Emitted when a new drawing is created by a Coordinator contract.
     */
    event DrawingCreated(
        address indexed _address,
        bytes32 indexed _drawingId,
        string  indexed _drawingName,
        uint256 _drawingDate,
        uint256 _ticketPrice,
        uint256 _prizePercentage,
        uint256 _minimumPrize
    ...
    /**
     * Emitted when a Drawing contract is destroyed
     * by way of an admin calling destroyDrawing().
     */
    event DrawingDestroyed(
        address _address,
        bytes32 _drawingId
    );
    ...
    /**
     * Returns information about a Drawing with the passed ID.
     * @param drawingId ID of the drawing to retrieve information about
     */
    function getDrawing(bytes32 drawingId) external view returns (Types.DrawingInfo memory);

    /**
     * Returns information about a Drawing with the passed name.
     * @param drawingName name of the drawing to retrieve information about
     */
    function getDrawing(string calldata drawingName) external view returns (Types.DrawingInfo memory);

    function getDrawings() external view returns (Types.DrawingInfo[] memory);
    ...
    function newDrawing(
        ...
        uint256 _prizePercentage,
        uint256 _minimumPrize
    ) external returns (Types.DrawingInfo memory);

    /**
     * Destroys a Drawing contract controlled by a Coordinator.
     * @param drawingId ID of the Drawing contract to destroy.
     */
    function destroyDrawing(bytes32 drawingId) external;
