...
contract SendExample {
    ...
    function sendViaCall(address payable _to, string calldata _msg) public payable {
        (bool sent, bytes memory data) = _to.call{value: msg.value,gas:10000}(abi.encodeWithSignature("greet(string)", _msg));
        emit LogSend(msg.sender, data);

        require(sent, "Failed to send Ether(Call)");
    }

    function testFallback(address payable _to) public payable {
