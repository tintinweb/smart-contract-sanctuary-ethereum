...
contract TnmtToken is ERC2981, ERC721, Ownable {
    ...
    event TnmtUpdated(uint256 indexed tokenId);
    ...
    // Tnmt Id tracker
    Counters.Counter private _currentTnmtId;
    ...
    modifier onlyDataPusher() {
        require(msg.sender == dataPusher, "The Sender is not the dataPusher");
        _;
    }
     ...
     */
    constructor(address _svgGen, address _dataPusher, address _minter, uint256 _colorDif) ERC721("The NFT Monkey Theorem", "TNMT")
    {
    ...
    function setDataPusher(address _dataPusher) external onlyOwner {
        require(!isDataPusherLocked,"Minter is locked");
        dataPusher = _dataPusher;
    ...
    function lockDataPusher() external onlyOwner {
        isDataPusherLocked = true;
        emit DataPusherLocked(dataPusher);
    }  
    ...
    function setDefaultRoyalty(uint96 value) external onlyOwner returns(bool) {
        _setDefaultRoyalty(owner(), value);
        return true;
    }  
    ...
    /**
     * Checks if a tnmtId has already been minted, returns Bool
     */
    function exists(uint256 tnmtId) public view returns(bool) {
        return _exists(tnmtId);
    }
    ...
    function mint(
        ...
        require(_monkeyId < 2000,"Max monkey Id: 1999");
        require(_rotations < 4,"Max rotations: 4");
        require(_manyEdits < 4,"Max edits: 4");
        _currentTnmtId.increment();
        uint curr = _currentTnmtId.current();
        _safeMint(_to, curr);
        tnmtData[curr].auctionId = _auctionId;
        tnmtData[curr].monkeyId = _monkeyId;
        tnmtData[curr].rotations = _rotations;

        if(_editor != address(0)) {
            edits[curr].auctionId = _auctionId;
            edits[curr].monkeyId = _monkeyId;
            edits[curr].manyEdits = _manyEdits;
            edits[curr].editor = _editor;

            for(uint i = 0; i < _manyEdits; i++) {
                edits[curr].colors[i] = editColors[i];
            }
        }

        emit TnmtMinted(curr);
        return curr;
    }
    ...
    function updateTnmtData(
        uint256 _tnmtId,
        uint8 _noColors,
        ITnmtLibrary.ColorDec[] memory _colors,
        uint256 _monkeyId,
        bool _hFlip,
        bool _vFlip,
        ...
        require(_monkeyId < 2000, "MonkeyId: 0 ... 1999");
        require( _noColors > 0 && _noColors < 11, "Colors: 1 ... 10");

        if(edits[_tnmtId].editor != address(0)) {
            if(IMonkeySvgGen(svgGen).editColorsAreValid(minColorDifValue, edits[_tnmtId].manyEdits, edits[_tnmtId].colors, _colors)) {
                IMonkeySplitter(splitter).approveEditorPay(tnmtData[_tnmtId].auctionId,edits[_tnmtId].editor);
            } else {
                IMonkeySplitter(splitter).denyEditorPay(tnmtData[_tnmtId].auctionId,edits[_tnmtId].editor);
            }
        }

        for (uint8 i = 0; i < _noColors; i++) {
            require(_colors[i].colorId < 11, "Color Id: 0-9");
                tnmtData[_tnmtId].colors[i].color = IMonkeySvgGen(svgGen).ColorDecToColorString(_colors[i]);
        }
    ...
    function updateTnmtRots(
        ...
        require(_exists(_tnmtId), "Token does not exists");
        require(ownerOf(_tnmtId) == msg.sender, "Sender is not token owner");
        ...
        returns (string memory)
    {
        ...
        ITnmtLibrary.Tnmt storage _tnmt = tnmtData[_tokenId];
        ITnmtLibrary.Attributes memory attrbts;
        attrbts.auctionId = _tnmt.auctionId;
        attrbts.monkeyId = _tnmt.monkeyId;
        attrbts.noColors = _tnmt.noColors;
        attrbts.rotations = _tnmt.rotations;
        attrbts.hFlip = _tnmt.hFlip;
        attrbts.vFlip = _tnmt.vFlip;
        uint8 noColors = _tnmt.noColors;

        ITnmtLibrary.ColorStr[] memory tokenColors = new ITnmtLibrary.ColorStr[](noColors);

        for (uint8 i = 0; i < noColors; i++) {
            tokenColors[i] = _tnmt.colors[i];
        }

        uint8[1024] memory pixls = pixelsData[ _tokenId];
        ITnmtLibrary.Edit memory edit = edits[_tokenId];



        bytes memory uriString = IMonkeySvgGen(svgGen).svgCode(
        attrbts,
        tokenColors,
        pixls,
        edit);
