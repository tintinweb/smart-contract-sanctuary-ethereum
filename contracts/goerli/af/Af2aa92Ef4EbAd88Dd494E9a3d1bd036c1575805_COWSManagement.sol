...
contract COWSManagement {
  ...
  function getContractInfo(
    bytes32 _cowID,
    string memory _contractName
  ) public view returns (int32, address) {
    return (
...
contract CustodyWallet {
  ...
  address public constant BOATS = 0x5719EA9E02DA174d3bcEa374996382d12fED7769;
  ...
  modifier onlyBoats() {
    require(tx.origin == BOATS, "tx origin not BOATS addr");
    _;
  }

  constructor(bytes32 _cowId, address _managementAddress, address _stateAddress, address _keyAddress) onlyBoats() {
    cowId = _cowId;
    cowsManagement = COWSManagement(_managementAddress);
    globalState = GlobalState(_stateAddress);
    keyManagement = KeyManagement(_keyAddress);
  ...
  function getDasCheckRequest(
    bytes32 requestId
  ) public view returns (DASCheckRequest memory checkReq) {
...
contract GlobalState {
  ...
  constructor(COWSState _globalState, address _cowsManagement) onlyBoats() {
  ...
  function getActiveNormalRecords(
    bytes32 requestId
  ) public view returns (SetActiveNormalRec memory record) {
  ...
  function setFlaggedState(
    FlaggedState flaggedState,
    bool value
  ) public returns (bool isFlaggedStateSet) {
...
contract KeyManagement {
  ...
  function getAddApprovalRequest(
    bytes32 requestId
  ) public view returns (AddApprovalRequest memory request) {
    return approvalKeyRequests[requestId];
  }

  function getEmergencyKeyReplacements(
    bytes32 requestId
  ) public view returns (EmergencyReplacement memory request) {
    return emergencyKeyReplacements[requestId];
  }
...
library StringUtils {
  ...
  function memcpy(uint256 dest, uint256 src, uint256 wordLength) private pure {
    // Copy word-length chunks while possible
...
library VerifySignatureUtil {
  ...
  function convertToStruct(
    string[] memory extractedMsg
  ) external pure returns (ExtractedSigningMessage memory) {
...
library VerifyUtil {
  ...
  function isValidApprovalKey(
    KeyManagement.Key[] memory approvalKeys,
    address recoverSigner
  ) public pure returns (bool) {
  ...
  function isDuplicateKey(
    bytes32[5] memory recoverSigners,
    uint256 numOfSigner
  ) public pure returns (bool) {
