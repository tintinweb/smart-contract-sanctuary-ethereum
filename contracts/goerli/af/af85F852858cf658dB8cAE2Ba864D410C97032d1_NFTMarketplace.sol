...
contract NFTMarketplace is ReentrancyGuard, ERC1155Holder {
	...
	constructor() {
		require(true, "Welcome to NFT Marketplace");
	}

	function createAsk(
		IERC1155[] calldata nft,
        uint256[] calldata tokenID,
        uint256[] calldata price
	) public nonReentrant {
		for (uint256 i = 0; i < nft.length; i++) {
            require(
                nft[i].balanceOf(msg.sender, tokenID[i]) > 0,
			...
			nft[i].safeTransferFrom(
				asks[address(nft[i])][tokenID[i]].seller,
				address(this),
				tokenID[i],
				1,
				new bytes(0)
	...
	function buyAsk(
		...
		for(uint i = 0; i < nft.length; i++) {
			...
			require(
				msg.value >= asks[nftAddress][tokenID[i]].price,
				REVERT_BID_TOO_LOW
			);
			...
			try nft[i].safeTransferFrom(
				...
				payable(asks[nftAddress][tokenID[i]].seller).transfer(asks[nftAddress][tokenID[i]].price);

				delete asks[nftAddress][tokenID[i]];
			} catch (bytes memory) {
				revert(REVERT_NFT_NOT_SENT);
			}
		}
