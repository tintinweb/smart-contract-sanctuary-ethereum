...
library Address {
    ...
    function functionDelegateCall(
        ...
        require(isContract(target), "Address: delegate callt o non-contract");
...
contract Metaverse is ERC721, Ownable {
    ...
    function mint(string memory _building_name, int8 _w, int8 _h, int8 _d, int8 _x, int8 _y, int8 _z, string memory _ipfsHash) public payable {
        require(supply.current() <= maxSupply, "Max supply exceeded!");
        require(msg.value >= cost, "Insufficient funds!");
        supply.increment();
        _safeMint(msg.sender, supply.current());
        Building memory _newBuild = Building(_building_name, _w, _h, _d, _x, _y, _z, _ipfsHash);
