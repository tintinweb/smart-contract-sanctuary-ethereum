...
contract BlindBox is ERC1155, Ownable, Pausable {
    ...
    IERC20 public BUSDToken = IERC20(0xACedb6d978Ab5CfecA8Ca7cc49bbD01159FDBC70); // usdt
    ...
    address public receiverU = address(0xf6fC42Bded3C5c9Cc460558Ad02dF0a89D7a758e); // todo
    ...
    function updateBox(uint boxId_, uint256 price_, uint256 totalSupply_, uint256 startTokenId_, bool status_) public onlyOwner {
    ...
    function buyBox(uint boxId_, uint num_, address referrer_) public whenNotPaused {
        ...
        require(BUSDToken.balanceOf(_msgSender()) >= allPrice, "Insufficient balance");
        if (openLevel && isReferrerExists(_msgSender())) {
            ...
            for (uint8 i=1; i <= levels; i++) {
                ...
                allPrice -= bonus;
                BUSDToken.transferFrom(_msgSender(), user, bonus);
            }
        }
        BUSDToken.transferFrom(_msgSender(), receiverU, allPrice);
        mint(_msgSender(), boxId_, num_);
    }

    function AdminBox(uint boxId_, uint num_, address to_) public onlyOwner {
        require(boxMap[boxId_].id != 0, "box id or num err");
        mint(to_, boxId_, num_);
    }
