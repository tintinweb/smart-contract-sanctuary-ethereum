...
contract Dev3 is ERC721A, Ownable {
    ...
    constructor() ERC721A("Dev3", "Dev3") {
        setHiddenMetadataUri(
            "ipfs://QmPCH6n9nHmuEiqE3TTzmoL5E8jfCB4pLkcBpr1Df7oVcK/hidden.json"
        );
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        require(stage > 0 && stage <= 3, "Stage not set!");
        ...
        if (stage < 3) {
            require(
                ...
                "This address need to be whitelisted"
            );
        }

        require(_mintAmount <= maxMintPerTransaction, "Max 5 per transaction");
        ...
        require(
            totalPerWallet <= maxPerWallet,
            "Max Procrastinators per wallet"
        );
    ...
    function addUsers(address[] memory _addressesToWhitelist) public onlyOwner {
        uint256 length = _addressesToWhitelist.length;
        for (uint256 i = 0; i < length; i++) {
    ...
    function startStage3() public onlyOwner {
        require(stage == 2, "Stage is not 2!");
        paused = true;
        uint256 totalMinted = totalSupply();
        maxSupply = totalMinted + 40;
        mintCost = 0.0015 ether;
        stage = 3;
    }
}