...
contract ERC20StakingModule is IStakingModule {
    ...
    function stake(
        ...
        shares[sender] += minted;

        // add newly minted shares to global total
        totalShares += minted;

        emit Staked(sender, address(_token), amount, minted);

        return (bytes32(uint256(uint160(sender))), minted);
    }
    ...
    function unstake(
        ...
        uint256 burned = _shares(sender, amount);

        // burn shares
        totalShares -= burned;
        shares[sender] -= burned;

        // unstake
        _token.safeTransfer(sender, amount);

        emit Unstaked(sender, address(_token), amount, burned);

        return (bytes32(uint256(uint160(sender))), sender, burned);
    }
    ...
    {
        uint256 s = _shares(sender, amount);
        emit Claimed(sender, address(_token), amount, s);
        return (bytes32(uint256(uint160(sender))), sender, s);
    }
