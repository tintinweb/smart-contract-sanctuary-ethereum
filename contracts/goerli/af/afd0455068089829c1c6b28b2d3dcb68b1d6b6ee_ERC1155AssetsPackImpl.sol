  ...
  ERC1155BurnableUpgradeable
{
  ...
  function createPack(
    ...
    require(_tokens.length > 0, "nothing to pack");

    packId = _nextTokenIndex;
    _nextTokenIndex += 1;

    Pack storage info = _packs[packId];
    ...
    for (uint256 i = 0; i < _tokens.length; i++) {
      ...
      _transferToken(msg.sender, address(this), _item, _item.remaining);
    }
  ...
  function getPack(uint256 _tokenId) public view returns (Pack memory) {
    require(exists(_tokenId), "Pack does not exist");
  ...
  function _transferTokenBatch(
    ...
    for (uint256 i = 0; i < _indexes.length; i += 1) {
      _transferToken(_from, _to, _tokens[_indexes[i]], 1);
    }
  }
  ...
  function _transferToken(
    address _from,
    address _to,
    PackItem memory _token,
    uint256 _quantity
  ) internal {
    if (_token.tokenType == TokenType.ERC721) {
      IERC721Upgradeable(_token.tokenAddress).safeTransferFrom(
        _from,
        _to,
        _token.tokenId
      );
  ...
  function _burnToken(PackItem memory _token) internal {
    if (_token.tokenType == TokenType.ERC721) {
      ...
      (bool _success, bytes memory _owner) = _token.tokenAddress.call(
        signature
      );
      require(_success);
    } else if (_token.tokenType == TokenType.ERC1155) {
  ...
  SignatureMintableUpgradeable
{
  ...
  function tokensOfOwner(address _address)
    ...
    returns (uint256[] memory)
  {
    require(exists(0), "No packs created");
  ...
  function _beforeTokenTransfer(
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      ...
      if (from != address(0)) {
        require(!pack.nonTradeable, "Can't be traded");
      }
      ...
      if (from == address(0)) {
        ...
        require(
          minted[to][ids[i]] <= pack.maxPerWallet,
