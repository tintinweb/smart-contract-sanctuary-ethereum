...
/**
    Conditions:
        ...
        - Public no limits, cannot mint through contract, at price .15
 */
...
contract ZooverseGen2Controller is BaseMint {
    ...
    uint256 private gensisLimit = 2;
    uint256 public discountedPrice = 0.12 ether;
    uint256 private _lastSaleType;

    constructor() {
        limits[1] = gensisLimit;
        limits[2] = gensisLimit + 1;
        limits[3] = gensisLimit;
        limits[4] = 1;
        _lastSaleType = 4;
    }

    modifier correctMintConditions(uint256 saleType, uint256 quantity, bytes32[] calldata proof) {
        require(currentStage == 1, "Not Live");
        require(nft.numberMinted(msg.sender) + quantity <= limits[saleType]);
    ...
    function mint(uint256 quantity, bytes32[] calldata proof, uint256 saleType) 
        external 
        payable 
        enoughPrice(quantity, discountedPrice) 
    ...
    function _verify(uint256 saleType, address account, bytes32[] calldata proof) internal view returns (bool) {
        return MerkleProof.verify(proof, roots[saleType], keccak256(abi.encodePacked(account)));
    }
    ...
    function availableToMint(address account, bytes32[] calldata proof) public view returns (uint256) {
        if(currentStage == 1) {
            ...
            return limits[saleType] - balance;
        }
        if(currentStage == 2) return maxPerTx;
        return 0;
    }

    function updateLimit(uint256 saleType, uint256 limit) external adminOnly {
        require(saleType <= _lastSaleType || _lastSaleType + 1 == saleType, "Can only increment by one sale stage");        
        limits[saleType] = limit;
    ...
    function updateRoot(uint256 saleType, bytes32 _root) external adminOnly {
        roots[saleType] = _root;
    }

    function addToWhitelist(uint256 saleStage, address[] calldata to, bool[] calldata value) external adminOnly {
        uint256 total = to.length;
        for(uint256 i = 0; i < total;) {
            _whitelist[saleStage][to[i]] = value[i];
            unchecked {
                ++i;
            }
        }
...
import "../../../access/AdminController.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "../../erc721/IERC721Token.sol";
...
contract BaseMint is AdminController {
    ...
    function mint(uint256 quantity) payable external callerIsUser enoughPrice(quantity, price) {
        require(currentStage == 2, "Not Live");
        require(quantity <= maxPerTx, "Exceeds Limit");
        _mint(quantity, msg.sender);
    }

    function _mint(uint256 quantity, address to) internal {
        require(nft.totalSupply() + quantity <= nft.supply(), "Exceeds supply");
...
abstract contract AdminController is IAdminController, Ownable, Pausable {
...
// SPDX-License-Identifier: MIT
pragma solidity 0.8.13;
import "../../access/IAdminController.sol";
