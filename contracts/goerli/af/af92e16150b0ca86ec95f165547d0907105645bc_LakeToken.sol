...
contract LakeToken is ERC20, Ownable, BlacklistManager {
    ...
    function setVestingAddress(address _address) external onlyOwner {
        require(_address != address(0x0), 'LakeToken: address cannot be a zero address');
        vesting = _address;
...
abstract contract BlacklistManager is Ownable {
     ...
     */
    function addToBlacklist(address[] calldata _accounts) external onlyBlacklistManager returns (bool) {
     ...
     * @return bool true if succeed
     */
    function removeFromBlacklist(address[] calldata _accounts) external onlyBlacklistManager returns (bool) {
        for (uint256 i = 0; i < _accounts.length; i++) {
