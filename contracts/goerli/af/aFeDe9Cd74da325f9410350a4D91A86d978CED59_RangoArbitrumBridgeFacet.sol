...
contract RangoArbitrumBridgeFacet is IRango, ReentrancyGuard, IRangoArbitrum {
    ...
    function arbitrumBridge(
        ...
        (uint out, uint value) = LibSwapper.onChainSwapsPreBridge(request, calls, bridgeRequest.cost);
    ...
    /// @param amount Amount that should be bridged
    function arbitrumBridge(
        IRangoArbitrum.ArbitrumBridgeRequest memory bridgeRequest,
        address token,
        uint256 amount
        ...
        if (token == LibSwapper.ETH) {
            require(msg.value >= amount + bridgeRequest.cost, "Insufficient ETH sent for bridging");
        } else {
            require(msg.value >= bridgeRequest.cost, "Insufficient ETH sent for fee payment");
            SafeERC20.safeTransferFrom(IERC20(token), msg.sender, address(this), amount);
        }

        doArbitrumBridge(bridgeRequest, token, amount);
    }
