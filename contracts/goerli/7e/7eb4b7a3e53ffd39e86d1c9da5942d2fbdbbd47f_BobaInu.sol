...
contract BobaInu is ERC721A, Ownable, ReentrancyGuard {
    ...
    bytes32 public fmMerkleRoot;
    bytes32 public communityWlMerkleRoot;
    bytes32 public collabWlMerkleRoot;

    uint256 public constant MAX_MINT_FM = 1;
    uint256 public constant MAX_MINT_WL = 2;
    uint256 public constant MAX_MINT_PUBLIC = 3;

    uint256 public currentPhase = 0;

    uint256 constant FM_SPOTS = 300;
    uint256 constant COLLAB_WL_SPOTS = 1500;
    uint256 constant COMMUNITY_WL_SPOTS = 2000;

    uint256 totalFmMints = 0;
    uint256 totalCommunityMints = 0;
    uint256 totalCollabMints = 0;
    uint256 totalPublicMints = 0;

    mapping(address => uint256) private fmMints;
    ...
    function flipSalePhase(uint256 _phase) external onlyOwner {
        require(_phase >= 0 && _phase <= 4, "Invalid state");
        currentPhase = _phase;
    }
    ...
    function withdrawAll(address _address) external payable onlyOwner {
        uint256 balance = address(this).balance;
        uint256 balanceOne = (balance * 100) / 100;
        (bool transferOne, ) = payable(_address).call{value: balanceOne}("");
        require(transferOne, "Transfer failed.");
    }

    function setFmMerkleRoot(bytes32 _fmMerkleRoot) external onlyOwner {
        ...
        returns (string memory)
    {
        ...
        if (!isRevealed) {
            return "ipfs://QmTsxoWX4qcDNCZ4qh6ApFp779jrWNiJNo3AVsQGrAWJsQ";
        }
    ...
    function mintFm(uint256 quantity, bytes32[] calldata merkleProof)
        external
        payable
        nonReentrant
    {
        require(currentPhase == 1, "Not currently free mint phase");
        require(quantity > 0, "Must mint at least one Boba Inu");
        require(quantity <= MAX_MINT_FM, "Minting too many tokens as once");
        require(
            MerkleProof.verify(
                merkleProof,
                fmMerkleRoot,
        ...
        require(totalFmMints + quantity <= FM_SPOTS, "Free mint sold out");
        require(
            fmMints[msg.sender] + quantity <= MAX_MINT_FM,
            "Free mint for this wallet reached"
        );

        totalFmMints += quantity;
        fmMints[msg.sender] += quantity;
    ...
    function mintCollabWl(uint256 quantity, bytes32[] calldata merkleProof)
        ...
        nonReentrant
    {
        require(currentPhase == 2, "Not currently collab WL phase");
        require(quantity > 0, "Must mint at least one Boba Inu");
        ...
        require(msg.value <= quantity * wlPrice, "Insufficient funds provided");
    ...
    function mintCommunityWl(uint256 quantity, bytes32[] calldata merkleProof)
        ...
        nonReentrant
    {
        require(currentPhase == 3, "Not currently community WL phase");
        require(quantity > 0, "Must mint at least one Boba Inu");
        require(quantity <= MAX_MINT_WL, "Minting too many tokens as once");
        require(
            MerkleProof.verify(
    ...
    function mintPublic(uint256 quantity) external payable nonReentrant {
        require(currentPhase == 4, "Not currently public phase");
        require(quantity > 0, "Must mint at least one Boba Inu");
    ...
    function freeMintListed(address _wallet, bytes32[] calldata _proof)
        public
        view
        returns (bool)
    {
    ...
    function isPhaseSoldOut(uint256 _phase) public view returns (bool) {
        require(_phase >= 1 && _phase <= 4, "Not a valid phase");
        if (_phase == 1) {
            return totalFmMints == FM_SPOTS;
        } else if (_phase == 2) {
    ...
    function quantityMinted(uint256 _phase) public view returns (uint256) {
        require(_phase >= 1 && _phase <= 4, "Not a valid phase");
        if (_phase == 1) {
            return fmMints[msg.sender];
        } else if (_phase == 2 || _phase == 3) {
            return wlMints[msg.sender];
        } else if (_phase == 4) {
