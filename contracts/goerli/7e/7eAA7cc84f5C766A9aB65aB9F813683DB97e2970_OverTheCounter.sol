...
contract OverTheCounter is IERC721Receiver, Ownable {
  ...
  uint256 public serviceFeeETH = 5 ether / 100; // 0.05 ETH
  ...
  function poolCreate(
    ...
    _sendETHOrERC20(
      ...
      address(this),
      _token0,
      _token0 == address(0) ? _amount0 + serviceFeeETH : _amount0
    );
    require(
    ...
    if (serviceFeeETH > 0) {
      _sendETHOrERC20(address(this), _getTreasury(), address(0), serviceFeeETH);
    }
  ...
  function packageCreate(
    ...
    require(msg.value >= _nativeCheck + serviceFeeETH, 'PKGCR: cost + service');
    if (serviceFeeETH > 0) {
      _sendETHOrERC20(address(this), _getTreasury(), address(0), serviceFeeETH);
    }
  ...
  function buyItNow(uint256 _tokenId) external payable {
    ...
    if (_pkg.buyItNowAsset == address(0)) {
      require(msg.value >= _pkg.buyItNowAmount, 'BIN: not enough ETH');
      uint256 _balBefore = address(this).balance;
      (bool _success, ) = payable(otcNFT.ownerOf(_tokenId)).call{
        value: _pkg.buyItNowAmount
      }('');
      require(
        _success && address(this).balance >= _balBefore - _pkg.buyItNowAmount,
        'BIN: too much ETH taken'
      );
    } else {
      IERC20(_pkg.buyItNowAsset).safeTransferFrom(
        msg.sender,
        otcNFT.ownerOf(_tokenId),
        _pkg.buyItNowAmount
      );
    }

    otcNFT.safeTransferFrom(otcNFT.ownerOf(_tokenId), msg.sender, _tokenId);
  }
