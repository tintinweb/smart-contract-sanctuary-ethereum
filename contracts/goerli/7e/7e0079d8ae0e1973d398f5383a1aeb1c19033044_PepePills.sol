...
contract PepePills is IERC721A { 
    ...
    uint256 public constant MAX_SUPPLY = 789;
    uint256 public MAX_FREE = 555;
    uint256 public MAX_FREE_PER_WALLET = 1;
    uint256 public COST = 0.001 ether;
    bool public paused = true;

    string private constant _name = "PepePills";
    string private constant _symbol = "PEPEPILL";
    string private _baseURI = "QmS3forsjLXE88bDyTj3qmwLM3Ab2oKVNnMC8YaE9onVLa";
    ...
    modifier mintPriceCompliance(uint256 amount) {
        ...
        if(MAX_FREE_PER_WALLET > 0 && _numberMinted(_caller) < MAX_FREE_PER_WALLET) {
        if(amount <= MAX_FREE_PER_WALLET - _numberMinted(_caller)) {
            require(msg.value >= COST * 0, 'Insufficient funds!');
        } else {
            require(msg.value >= COST * (amount-(MAX_FREE_PER_WALLET - _numberMinted(_caller))), 'Insufficient funds!');
        }
        _;
        } else {
    ...
    function mint(uint256 amount) external payable mintPriceCompliance(amount){
        address _caller = _msgSenderERC721A();
