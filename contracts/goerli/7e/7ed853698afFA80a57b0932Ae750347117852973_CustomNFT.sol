...
// File: contracts/final_version_struct.sol
...
contract CustomNFT is ERC721, Ownable {
    ...
    constructor() ERC721('NFT Name', 'SYMBOL') {
        baseTokenURI = 'https://ipfs.io/ipfs/QmfHwRDwwsXLNcJoDaydG9Y1EXueZreXCHhQ2u4uyCfFk1/';
    }

    function mintBatch(uint256 batchSize) external onlyOwner {
        require(currentTokenId <= MAX_TOKENS, 'All NFTs have been minted');
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        if (tokenMetadata[_tokenId].useCustomMetadata) {
            string memory json = encode(
                bytes(string(abi.encodePacked(tokenMetadata[_tokenId].metadata, ', "image_data": "', getSvg(_tokenId), '"}')))
            );
            return string(abi.encodePacked('data:application/json;base64,', json));
        }

        string memory tokenIdString = uint256ToString(_tokenId);
        return string(abi.encodePacked(baseTokenURI, tokenIdString, '.json'));
    }
    ...
    function getTokenHash(uint256 _tokenId) public view returns (string memory) {
        return tokenMetadata[_tokenId].hash;
    }
    ...
    function setCustomSVG(uint256 _tokenId, string memory _svg) public {
        require(ownerOf(_tokenId) == msg.sender, 'Caller is not the owner of the token');
        tokenMetadata[_tokenId].customSVG = _svg;
    }
    ...
    function setTokenHash(uint256 _tokenId, string memory _hash) public {
        require(ownerOf(_tokenId) == msg.sender, 'Caller is not the owner of the token');
        tokenMetadata[_tokenId].hash = _hash;
    }
    ...
    function toggleCustomMetadata(uint256 _tokenId) public {
        ...
        if (tm.useCustomMetadata) {
            ...
            require(
                bytes(tm.customSVG).length > 0,
                string(abi.encodePacked('Please load custom metadata first for token ', uint256ToString(_tokenId), '.'))
            );
