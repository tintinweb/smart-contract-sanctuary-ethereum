...
contract HelpMeDistributeReward is Ownable, ReentrancyGuard {
    ...
    event RoyaltyRewardDistributed(uint256 value, address winner);
    ...
    function distributeHmpeRewardNoRecord(address payable[] calldata _addresses, uint256[] calldata _values, uint256 _icycle) 
        external
        isWhiteListed
        payable {
        for (uint256 i =0; i < _addresses.length; i++) {
            (bool sent, ) = _addresses[i].call{value: _values[i]}("");
            require(sent, "Failed to distribute. Need !debog");
        }
    }
