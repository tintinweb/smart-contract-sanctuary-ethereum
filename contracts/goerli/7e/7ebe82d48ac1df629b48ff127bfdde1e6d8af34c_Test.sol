...
contract Test is Ownable, Pausable, ERC721Holder, ERC1155Holder {

    uint64 public _swapsCounter;

    mapping (uint64 => Swap) public _swaps;
    ...
    event SwapProposed(
        ...
        uint256[] nftIds,
        uint256[] nftAmounts,
        address[] nftAddressesWant,
        uint256[] nftIdsWant,
        uint256[] nftAmountsWant
    );
    event SwapInitiated(
        address indexed from,
        address indexed to,
    ...
    modifier onlySecond(uint64 swapId) {
        require(msg.sender == _swaps[swapId].secondUser,
            "Caller is not swap Second User");
        _;
    }
    ...
    function proposeSwap(
        ...
        swap.initiatorNftAddressesWant = nftAddressesWant;
        swap.initiatorNftIdsWant = nftIdsWant;
        swap.initiatorNftAmountsWant = nftAmountsWant;
    ...
    function acceptSwap(uint64 swapId) external onlySecond(swapId) {
    ...
    // transfer NFTs from  second user to initiator
        safeMultipleTransfersFrom(
            _swaps[swapId].secondUser,
            _swaps[swapId].initiator,
            _swaps[swapId].initiatorNftAddressesWant,
            _swaps[swapId].initiatorNftIdsWant,
            _swaps[swapId].initiatorNftAmountsWant
        );
    ...
    function cancelSwap(uint64 swapId) external onlyInitiator(swapId){
        safeMultipleTransfersFrom(
            address(this),
            _swaps[swapId].initiator,
            _swaps[swapId].initiatorNftAddresses,
