...
contract PVMIdentityVerifier is AdminControl, IIdentityVerifier {

    uint40 _meaningListingID;
    uint40 _powerListingID;

    mapping(address=>uint40) public meaningOffers;
    mapping(address=>uint40) public powerOffers;

    function setListingIDs(uint40 meaningListingID, uint40 powerListingID) public adminRequired{
         _meaningListingID = meaningListingID;
        _powerListingID = powerListingID;
    }

    function verify(uint40 listingId, address identity, address, uint256, uint24, uint256, address, bytes calldata) public override returns (bool){
        require(listingId == _meaningListingID || listingId == _powerListingID);
        
        if(listingId == _meaningListingID){
            require(powerOffers[identity] != _powerListingID);
            meaningOffers[identity] = _meaningListingID;
        } else if (listingId == _powerListingID){
            require(meaningOffers[identity] != _meaningListingID);
            powerOffers[identity] = _powerListingID;
        }
