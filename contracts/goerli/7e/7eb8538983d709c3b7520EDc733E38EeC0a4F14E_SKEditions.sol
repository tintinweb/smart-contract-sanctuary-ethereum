...
contract SKEditions is ERC1155, OperatorFilterer, ERC2981, Ownable, CantBeEvil {
    ...
    constructor(string memory baseURI) ERC1155(baseURI) CantBeEvil(LicenseVersion.PERSONAL) {
        _baseURI = baseURI;
    ...
    function mint(uint16 tokenId, uint16 qty) external payable {
        // Is the sale active
        ...
        // Get token
        Token storage token = getToken(tokenId);

        // No token
        if (!token.created || !token.active) revert TokenNotAvailable();

        // Is there enough supply to mint
        if (token.qty > 0 && token.minted + qty > token.qty) revert MaxSaleReached();

        // Can sender afford mint
        if (msg.value < qty * token.cost) revert InsufficientBalance();
    ...
    function toggleToken(uint16 tokenId) external onlyOwner {
        Token storage token = getToken(tokenId);
        token.active = !token.active;
    }

    function getToken(uint16 tokenId) internal view returns (Token storage) {
        if (!tokens[tokenId].created) revert TokenNotAvailable();
        return tokens[tokenId];
    }

    function getRemaining(uint16 tokenId) external view returns (uint16 qty, uint16 minted) {
