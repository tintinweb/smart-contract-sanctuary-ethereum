...
interface INodeValidatorRegistry {
    struct Validator {
        bytes publicKey;
        bytes signature;
    ...
    function addValidatorToPool(
        bytes memory _publicKey,
        bytes memory _withdrawalCredentials,
        bytes memory _signature,
        bytes32 _depositDataRoot,
        uint32[] memory _operatorIds,
        bytes[] memory _sharePublicKeys,
        bytes[] memory _shareEncrypted
    ...
    function setValidatorInUse(uint256 _index) external;
}
...
import "./interfaces/INodeValidatorRegistry.sol";
import "./interfaces/INodeLiquidETH.sol";

/// @custom:security-contact [email protected]
contract NodeStakingPool is
    Initializable,
    ...
    PausableUpgradeable
{
    ...
    address public nLETH; // NodeLiquidETH contract
    address internal NVR; // NodeValidatorRegistry
    address internal SSVNetwork; // SSVNetwork contract
    address internal SSVToken; // SSVToken contract
    address internal BeaconContract; // Beacon contract address

    uint256 public totalValidatorsBalance;
    uint256 public totalETHStaked;

    uint256[20] __gap;

    event ValidatorRegisteredToSSVNetwork(bytes publicKey);
    event Stake(address indexed staker, uint256 shares);
    ...
    function initialize(
        address _nLETH,
        address _nodeValidatorRegistry,
        address _ssvNetwork,
        address _ssvToken,
        ...
        nLETH = _nLETH;
        NVR = _nodeValidatorRegistry;
        SSVNetwork = _ssvNetwork;
        SSVToken = _ssvToken;
        BeaconContract = _beaconContract;

        totalValidatorsBalance = 0;
        totalETHStaked = 0;

        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    // Oracle can update stats
    function oracleUpdate(
        uint256 _validatorsETHBalance
    ) public onlyRole(ORACLE_ROLE) {
        totalValidatorsBalance = _validatorsETHBalance;
    }

    function stake() public payable whenNotPaused {
        require(msg.value >= MIN_DEPOSIT_AMOUNT, "Stake amount too low");

        INodeLiquidETH(nLETH).mint(msg.sender, msg.value);

        emit Stake(msg.sender, msg.value);
    }

    function totalShares() public view returns (uint256) {
        return INodeLiquidETH(nLETH).totalSupply();
    }

    function userShares(address _user) public view returns (uint256) {
        return INodeLiquidETH(nLETH).balanceOf(_user);
    }

    function userAssets(address _user) public view returns (uint256) {
        uint256 shares = userShares(_user);
        return (shares * totalValidatorsBalance) / totalShares();
    }

    function pendingETHToStake() public view returns (uint256) {
        return address(this).balance;
    }

    function totalPoolRewards() public view returns (uint256) {
        return totalValidatorsBalance - totalETHStaked;
    }
    ...
    function registerValidatorToSSVNetwork(
        ...
        require(
            ...
            "Not enough ETH in NodePoolManager contract"
        );
        // Require that contract owns enought SSV tokens as amount
        require(
            IERC20(SSVToken).balanceOf(address(this)) >= _amount,
            "Not enough SSV tokens in NodePoolManager contract"
        );

        // get validator details by index
        INodeValidatorRegistry.Validator
            memory validator = INodeValidatorRegistry(NVR).getValidatorByIndex(
                _index
            );

        // Require that validator is in the pool
        require(validator.publicKey.length > 0, "Validator is not in the pool");

        // Deposit to Beacon Chain Contract
        IDepositContract(BeaconContract).deposit{value: 32 ether}(
            validator.publicKey,
        ...
        ISSVNetwork(SSVNetwork).registerValidator(
            ...
            validator.sharePublicKeys,
            validator.shareEncrypted,
            _amount
        );

        // set validator in use
        INodeValidatorRegistry(NVR).setValidatorInUse(_index);

        emit ValidatorRegisteredToSSVNetwork(validator.publicKey);
    ...
    function withdrawSSVTokens(
        ...
        IERC20(SSVToken).transfer(_to, _amount);
    }
