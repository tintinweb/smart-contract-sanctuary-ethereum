...

{
    ...
    function initialize() initializer public {
        ...
        // feeAccount = payable(0x2265EcB63c1a949Bf71e754b4d7448389badCA2A);
    ...
    function ListForSale(IERC1155 _nft, uint _tokenId, uint _price, uint _amount, bytes calldata _bytes, address _royaltyReceipent, uint256 _royalityPercentage) external nonReentrant returns(uint){
        ...
        // _nft.safeTransferFrom(address(msg.sender),address(this), _tokenId, _amount, _bytes);
    ...
    function PurchaseItem(uint _itemId) external payable nonReentrant{
        ...
        item.sold = true;
        item.nft.safeTransferFrom(address(item.Seller), msg.sender, item.tokenId, item.qunatity,"0x00");
        if(bid.bidPrice > 0){
    ...
    function SaleBidding(uint _itemId) external payable nonReentrant
    {
        ...
        if(msg.value == item.price){
            ...
            emit TransferSingle(address(item.Seller), msg.sender, item.tokenId, item.qunatity);
        }
        bid.Bidder.transfer(bid.bidPrice);
    ...
    function SaleAcceptBid(uint _itemId) external payable nonReentrant{
        ...
        item.tokenId, item.qunatity,"");
        emit TransferSingle(address(item.Seller), bid.Bidder, item.tokenId, item.qunatity);
    }
    ...
    function CreateAuction(IERC1155 _nft, uint _tokenId, uint _amount, uint _minPrice, uint _endDate, bytes calldata _bytes) external nonReentrant returns(uint){
        ...
        emit AuctionEvent(msg.sender, auctionItem);
        return auctionItem;
    }
...
//change the logic from here
    function AuctionDisbursal(uint _auctionId) external payable nonReentrant{
        ...
        auction.sold = true;
        emit TransferSingle(address(this), bid.Bidder, auction.tokenId, auction.qunatity);
    }
    ...
    // function ActiveListing() public view returns(uint){
    //     uint[] ActiveList;
    //     for (uint i = 0; i < itemCount; ) {  //for loop example
