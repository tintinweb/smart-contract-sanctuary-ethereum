    ...
    ReentrancyGuardUpgradeable
{
    ...
    function lock(
        ...
        uint256 beforeLock = IERC20(_tokenAddr).balanceOf(thisadd);

        IERC20(_tokenAddr).transferFrom(msg.sender, thisadd, _amount);

        uint256 afterLock = IERC20(_tokenAddr).balanceOf(thisadd);
    ...
    function unlock(
        ...
        require(
            IERC20(_tokenAddr).balanceOf(thisadd) >= _amount,
            "Insufficient ERC20 balance."
        );

        unlockFunds(_tokenAddr, _recipient, _amount, _interchainTX);
    }
    ...
    function refund(
        ...
        require(block.timestamp < _timeEnd, "singature expired");
        require(
            _checkRefundSig(_signature, _tokenAddr, _recipient, _amount, _nonce, _timeEnd),
            "Invalid signature"
        );

        require(
            refundCompleted[_nonce].tokenAddr == _tokenAddr,
            "Invalid token"
        );
        ...
        refunds(_tokenAddr, _recipient, _amount, _nonce);
    }
    ...
    function lockNft(
        ...
        for (uint256 i; i < _tokenId.length; i++) {
            IMintNFTToon(_nftAddr).transferFrom(
                msg.sender,
                address(this),
                _tokenId[i]
            );
        }
        lockNftFunds(_nftAddr, msg.sender, _tokenId, _recipient, _targetChainId);
    }
    ...
    function claimNft(
        ...
        require(
            _tokenIdSource.length == (_tokenIdDest.length + _numberMint),
        ...
        if (_tokenIdDest.length > 0) {
            ...
            for (uint256 i; i < _tokenIdDest.length; i++) {
                require(
                    IMintNFTToon(_nftAddr).ownerOf(_tokenIdDest[i]) ==
                        address(this),
    ...
    function refundNft(
        ...
        require(
            refundNftCompleted[_nonce].nftAddr == _nftAddr,
            "Invalid NFT"
        );
        require(
        ...
        for (uint256 i; i < _tokenId.length; i++) {
            require(
                IMintNFTToon(_nftAddr).ownerOf(_tokenId[i]) == address(this),
                "Nft does not lock in contract"
            );
        ...
        refundsNft(_nftAddr, _recipient, _tokenId, _nonce);
    }
...
contract BaseBridge {
    ...
    function unlockFunds(
        ...
        IERC20(_tooAddr).safeTransfer(_recipient, _amount);
        ...
        emit LogUnlock(_tooAddr, _recipient, _amount, _interchainTX);
    }
    ...
    function unlockNftFunds(
        ...
        for (uint256 i; i < _tokenIdDest.length; i++) {
            IMintNFTToon(_tooNft).safeTransferFrom(
                address(this),
                _recipient,
                _tokenIdDest[i]
            );
    ...
    function mintNftFunds(
        ...
        uint256[] memory arrNewTokenId = new uint256[](_numberMint);
        uint256 newTokenId = IMintNFTToon(_tooNft).nextTokenId();
        IMintNFTToon(_tooNft).mint(_recipient, _numberMint);
        for (uint256 i; i < _numberMint; i++) {
            arrNewTokenId[i] = newTokenId + i;
        }
    ...
    function refunds(
        ...
        emit LogRefund(_tooAddr, _recipient, _amount, _nonce);
    }
    ...
    function refundsNft(
        ...
        emit LogRefundNft(_tooNft, _recipient, _tokenId, _nonce);
    }
    ...
    {
        return (
            refundCompleted[_nonce].tokenAddr,
    ...
    {
        return (
            refundNftCompleted[_nonce].nftAddr,
...
contract Validator is EIP712Upgradeable {
    // keccak256("UnlockToken(address _tokenAddr,address _recipient,uint256 _amount,bytes32 _interchainTX,uint256 _chainId,uint256 _timeEnd)");
    bytes32 public constant UNLOCK_TOKEN_TYPEHASH =
        0x0a05fac30908cd79eca3c1fa75eaef9b36fc2012dca41a2f772678ff0c4df6e0;
    // keccak256("RefundToken(address _tokenAddr,address _recipient,uint256 _amount,uint256 _nonce,uint256 _timeEnd)");
    bytes32 public constant REFUND_TOKEN_TYPEHASH =
        0x77c025b02c4efff459aaa48dbea261fb98c79d8cf243ef96b508ce5d8ab732b4;
    // keccak256("RefuntNft(address _nftAddr,address _recipient,uint256 _amount,uint256 _nonce,uint256 _timeEnd)");
    bytes32 public constant REFUND_NFT_TYPEHASH =
        0x5143dead4601912b34753fd6c0e4e66e69a327da15b04beb53ac5136c9efa6dc;
    ...
    // keccak256("ClaimNft(address _recipient,uint256[] _arrTokenIdDest,uint256[] _arrTokenIdSource,bytes32 _interchainTX,uint256 _chainIdDest,uint256 _chainIdSource,uint256 _numberMint,uint256 _timeEnd)");
    bytes32 public constant CLAIM_NFT_TYPEHASH =
        0x026eb4c2646942e0c364e6434a2dccec92bcaeeef9ce9f7c083f6456292710c1;
