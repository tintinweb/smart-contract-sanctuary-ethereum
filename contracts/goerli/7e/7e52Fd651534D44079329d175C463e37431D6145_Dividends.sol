...
contract Dividends is IDividends, EasyAccessControl, ReentrancyGuard {
    ...
    /**
     * @dev Transfer the remaining unclaimed ERC-20 token dividends from a given snapshot into the current snapshot
     * @param token ERC-20 token address
     * @param snapshotId Snapshot ID
     */
    function transferRemains(address token, uint256 snapshotId)
        public
        override
        onlyContractAdmin
        nonReentrant
    {
        ...
        uint256 _currentSnapshotId = restrictedLockupToken
            .getCurrentSnapshotId();

        require(
            snapshotId > 0 && snapshotId < _currentSnapshotId,
            "BAD SNAPSHOT ID"
        );
        ...
        tokensFunded[_currentSnapshotId][token].unused += amount;

        emit TransferRemains(token, amount, snapshotId, _currentSnapshotId);
    }
     ...
     */
    function tokensAt(address token, uint256 snapshotId)
        public
        view
        override
        returns (uint256)
     ...
     *  @dev Proxy function for restricted token
     */
    function balanceOfAt(address sender, uint256 snapshotId)
        external
        view
        returns (uint256)
    {
     ...
     * @return amount of ERC-20 tokens
     */
    function fundsAt(address token, uint256 snapshotId)
        public
        view
        override
        returns (uint256)
    ...
    function totalAwardedBalanceAt(
        ...
        uint256 secTokenBalance = restrictedLockupToken.balanceOfAt(
            receiver,
            snapshotId
        );
        ...
        return
            (tokensFunded[snapshotId][token].total * share) /
    ...
    function unclaimedBalanceAt(
        ...
        return
            totalAwardedBalanceAt(token, receiver, snapshotId) -
...
interface IDividends {
    ...
    /// @dev Transfer remains of unused ERC-20 tokens at snapshot
    /// @param token ERC-20 token address
    /// @param snapshotId Snapshot ID
    function transferRemains(address token, uint256 snapshotId) external;
    ...
    event TransferRemains(
        address indexed token,
        uint256 amount,
        uint256 indexed fromSnapshotId,
        uint256 indexed toSnapshotId
    );
}
