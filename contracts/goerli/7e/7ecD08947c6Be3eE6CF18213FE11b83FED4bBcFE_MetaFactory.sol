...
interface ICurve {
    ...
    function getBuyInfo( uint128 _delta, uint128 _spotPrice, uint _numItems ) external  pure 
        returns ( 
    ...
    function getSellInfo( uint128 _delta, uint128 _spotPrice, uint _numItems ) external  pure 
        returns ( 
            CurveErrors.Error error, 
            uint128 newSpotPrice, 
            uint128 newDelta, 
...
contract MetaFactory is Ownable, PoolTypes {
    ...
    function _creteContract( PoolType _poolType, address _nft ) private returns ( address ) {
        ...
        MSPairNFTEnumerable hola = new MSPairNFTEnumerable();

        MSPairNFTEnumerable hola1 = MSPairNFTEnumerable( hola );

        // if( isEnumerable ) MSPairNFTEnumerable() ;

        // if( isBasic ) return MSPairNFTBasic();

    }

    function createPair( address _nft ) public payable {

        _creteContract( PoolType.Trade, _nft);
...
abstract contract MSPairBasic is ReentrancyGuard, Ownable, CurveErrors, PoolTypes {
    ...
    function initialize( uint128 _delta, uint128 _spotPrice, address _rewardsRecipent, address _owner, uint128 _fee, ICurve _curve, address _NFT, PoolType _poolType ) public payable {
