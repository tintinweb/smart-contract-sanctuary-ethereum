...
contract BacchTest is ERC721, ERC721Pausable, Ownable {
     ...
     * @param _dropper Wallet address to be used to drop tokens that were bought with fiat
     * @param _genesisCollection Address of the Genesis NFT Collection
     */
    constructor(address _usdByEthFeedAddress, address _signer, address _dropper, address _genesisCollection)
    ...
    function unrevealedMint(
     ...
     /**
     * @dev Function to be called by the contract owner or the dropper address for users that paid in fiat
     * @param to Address that will receive the NFTs
     * @param tokenIds List of token IDs to be minted
     */
    function drop(address to, uint256[] memory tokenIds) public {
        require(isPublicMint == true, "Public mint not enabled");
        require(msg.sender == dropper || msg.sender == owner(), "Not allowed");
         require(
            currentSupply + tokenIds.length <= MAX_SUPPLY,
            "Max supply has been reached"
        );
        //Does not check payment as it was paid in fiat and it's the team's responsability to mint the tokens
        for (uint256 index = 0; index < tokenIds.length; index++) {
            // It is mandatory that the wallet which is receiving the Bacchus NFTs is a holder
            // of the same IDs of the Genesis NFT Collection
            require(
     ...
     */
    function hashMessage(address to, uint256[] memory tokenIds) internal pure returns (bytes32) {
    ...
    function setDropper(address _newDropper) external onlyOwner {
        dropper = _newDropper;
    }
