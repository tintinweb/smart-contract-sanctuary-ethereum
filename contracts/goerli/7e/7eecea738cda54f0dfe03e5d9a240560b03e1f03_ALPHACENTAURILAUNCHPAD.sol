...
contract ALPHACENTAURILAUNCHPAD is IERC20, Auth {
    ...
    address payable public marketingWallet = payable(0xE6925606A3F098D4b5243921597cD3809aD0A2B2);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uint256 deadBalance = amountETH.mul(burnFeeSell).div(totalFeeSell);
    ...
    function manualSendEth() external onlyOwner {
        ...
        uint256 deadBalanceETH = contractETHBalance.mul(burnFeeSell).div(totalFeeSell);
        if(contractETHBalance > 0){          
            payable(deadWallet).transfer(deadBalanceETH); 
