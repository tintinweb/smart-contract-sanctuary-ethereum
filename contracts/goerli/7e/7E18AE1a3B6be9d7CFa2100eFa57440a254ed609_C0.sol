...
contract C0 is Initializable, ERC721Upgradeable, OwnableUpgradeable, EIP712Upgradeable {
  ...
  //
  function initialize(string calldata name, string calldata symbol, address _signer) initializer external {
  ...
  function token(Body[] calldata bodies, Input[] calldata inputs) external payable {
    ...
    for(uint i=0; i<bodies.length;) {
      ...
      if (body.relations.length == 0) {
        ...
        require(_hashTypedDataV4(bodyhash).recover(body.signature) == signer, "2");    // check script signature
      } else {
...
* with one exception:
*
* _HASHED_NAME is "internal" types instead of "private"
*   => so the inheriting contract can dynamically update it
*
***************************************************************************************************/
