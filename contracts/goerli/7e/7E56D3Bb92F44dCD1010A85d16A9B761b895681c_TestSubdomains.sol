  ...
  UUPSUpgradeable
{
  ...
  function initialize() public initializer {
    __ERC721_init("DOJI Subdomains", "DOJI_IDs");
    __Ownable_init();
  ...
  function _getMessageHash(address to, string memory label, bytes32 baseNode, address _contract, uint256 serial)
    internal
    pure
    returns (bytes32)
  {
    return
      keccak256(
        abi.encodePacked(
          ...
          _getMessage(to, label, baseNode, _contract, serial)
        )
      );
  }

  function _getMessage(address to, string memory label, bytes32 baseNode, address _contract, uint256 serial)
    internal
    pure
    returns (bytes32)
  {
    return keccak256(abi.encodePacked(to, label, baseNode, _contract, serial));
  }
  ...
  function recover(
    ...
    bytes32 hash = _getMessageHash(to, label, baseNode, address(this), serial);
  ...
  function reclaimDomain(string memory name) external onlyOwner {
    bytes32 nodehash = computeNamehash(name);
    require(ens.owner(nodehash) == address(this));
    ens.setOwner(nodehash, owner());
    if ( baseNodes[nodehash].enable ) {
      baseNodes[nodehash].enable = false;
    }
  }

  function enableDomain(string memory name) external onlyOwner {
    bytes32 nodehash = computeNamehash(name);
    require(ens.owner(nodehash) == address(this));
  ...
  function disableDomain(string memory  name) external onlyOwner {
    bytes32 nodehash = computeNamehash(name);    
    baseNodes[nodehash].enable = false;
  }
  ...
  function setImageURL(string memory _url) external onlyOwner {
    imageURL = _url;
  }

  // Set the resolver for the TLD this registrar manages.
  function setResolver(bytes32 _baseNode, address _resolver) external onlyOwner {
    ens.setResolver(_baseNode, _resolver);
    resolver = _resolver;
  }
  ...
  function register(address owner, bytes memory data) external {
    (string memory label, bytes32 baseNode, uint256 serial, bytes memory signature) = abi.decode(
      data,
      (string, bytes32, uint256, bytes)
    );

    require(baseNodes[baseNode].enable, "Invalid domain");
    require(recover(msg.sender, label, baseNode, serial, signature), "wrong signature");
    bytes32 labelhash = keccak256(abi.encodePacked(baseNode, bytes(label)));
    ...
    ensToTokenData[uint256(labelhash)].domain = baseNodes[baseNode].name;
    _register(uint256(labelhash), labelhash, baseNode, owner, true);
    _totalSupply++;

    emit TokenPayment(msg.sender, serial, uint256(labelhash), label, baseNodes[baseNode].name);
  }
    ...
    returns (string memory)
  {
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        ...
        ' is a subdomain.","external_url":"https://dojicrew.com/","name":"',
        ensToTokenData[ensID].label,
