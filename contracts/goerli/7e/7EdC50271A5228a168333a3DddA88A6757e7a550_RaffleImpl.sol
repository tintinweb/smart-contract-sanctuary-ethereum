...
contract RaffleImpl is Ownable, Raffle {
    /* Raffle Variables */
    Ticket public ticketNft;
    uint256 public entranceFee = 0.059 ether;
    uint256 public ticketsLeft;
    uint256[] private amounts;
    uint256[] public sums;
    uint256[20] private finalGameSums;
    bool public isOpen;
    ...
    constructor(
        ...
        uint256[20] memory _finalGameSums,
        address _priceFeed,
        address _ticketNft
    ) {
        require(_amounts.length == _sums.length && _amounts.length != 0, "RaffleImpl: invalid arguments");

        priceFeed = AggregatorV3Interface(_priceFeed);
        sums = _sums;
        finalGameSums = _finalGameSums;
    ...
    function finalGame() external override onlyOwner {
        uint256 randomWord = uint256(keccak256(abi.encode(msg.sender, nonce, blockhash(block.number - 1))));

        address[20] memory players = _mixTop20Players(randomWord);
        for (uint256 i = 0; i < 20; i++) {
            if (players[i] != address(0)) {
                ticketNft.safeMint(players[i], finalGameSums[i]);
            }
        }
        return;
    }

    function pay(address target, uint256 amountInUsdt) external override onlyOwner {
    ...
    function enterRaffle() external payable opened {
        ...
        uint256 randomWord = uint256(keccak256(abi.encode(msg.sender, nonce, blockhash(block.number - 1))));
        // uint256 randomWord = 10500;

        uint256 openPositionsLeft = ticketsLeft;
        uint256 randomNumber = (randomWord % openPositionsLeft) + 1;

        uint256 winningIndex = _chooseWinTicket(randomNumber);

        uint256 sumOfTicket = sums[winningIndex];
        uint256 tokenId = ticketNft.safeMint(msg.sender, sumOfTicket);
        ticketsLeft -= 1;
        emit WinnerChosen(msg.sender, sumOfTicket, tokenId);
    }
    ...
    function _mixTop20Players(uint256 randomValue) internal view returns (address[20] memory players) {
        uint256 currIndex = randomValue % 20;
        uint256 counter;
        while (counter != 20) {
            players[currIndex] = ticketNft.getTopPlayer(counter);
            currIndex = (currIndex + 1) % 20;
            ++counter;
        }
    }
    ...
    function getFinalGameSums() external view returns (uint256[20] memory) {
        return finalGameSums;
    }
...
contract Ticket is ERC721, Ownable {
    ...
    string baseUri = "ipfs://QmfGCpXXEmcENpXfUUVe3ztMMDToKN4sorsT6evF7xLv3m/";

    constructor() ERC721("VERITTY Ticket", "VRT") {}

    function safeMint(address _to, uint256 _sum) public onlyOwner returns (uint256) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        _requireMinted(tokenId);
        uint256 sum = sums[tokenId];
        return string(abi.encodePacked(baseUri, Strings.toString(sum), ".json"));
    }

    function _updateStatistics(address from, address winner, uint256 sum) internal {
    ...
    function _sortTopPlayers() internal {
        ...
        for (uint256 i = 0; i < 20; i++) {
            for (uint256 j = 1; j < (20-i); j++) {
                if (amountOfTickets[tp[j-1]] < amountOfTickets[tp[j]]) {
                    address tmp = tp[j];
                    tp[j] = tp[j-1];
                    tp[j-1] = tmp;
                }
    ...
    function _afterTokenTransfer(
        ...
        _updateStatistics(from, to, sums[firstTokenId]);
    }
