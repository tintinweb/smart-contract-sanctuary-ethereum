...
contract Staking is Ownable {
    ...
    function getStakeCreator(uint256 _poolId, uint256 _stakeNum) external view returns(StakeInPool memory) {
        return stakeCreatorInPool[_poolId][_stakeNum];
    }

    function getStakeCreators(uint256 _poolId) external view returns(StakeInPool[] memory) {
        StakeInPool[] memory stakersInPool = new StakeInPool[](totalStakesCreatedInPool[_poolId]);
        for(uint256 i=0; i<totalStakesCreatedInPool[_poolId]; ++i) {
            stakersInPool[i] = stakeCreatorInPool[_poolId][i+1];
        }
        return stakersInPool;
    }

    function getAllPoolStakeDetails(uint256 _poolId) external view returns(StakeInfoResponse[] memory) {
        uint256 totalStakes = totalStakesCreatedInPool[_poolId];
        StakeInfoResponse[] memory stakeDetail = new StakeInfoResponse[](totalStakes);
        for(uint256 i=0; i<totalStakes; ++i) {
            StakeInPool memory stake = stakeCreatorInPool[_poolId][i+1];
    ...
    function createStakeInfoResponse(address _address, uint256 _poolId, uint256 _stakeId) private view returns (StakeInfoResponse memory) {
        ...
        if(stakeInfo.amount == 0) {
            ...
            reward = (
                (stakeInfo.amount * poolInfo.apyPercent * poolInfo.duration) /
                (poolInfo.apyDivisor * 365 * 86400)
            );
        }
