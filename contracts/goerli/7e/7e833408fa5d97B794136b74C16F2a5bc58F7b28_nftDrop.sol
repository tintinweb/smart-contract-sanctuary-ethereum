...
contract nftDrop is Ownable{
    struct Drop {
        ...
        address whitelist; // whitelist address, if public, set to zero address
    }
    ...
    function PresaleClaim() public payable {
        ...
        for (uint i=0; i<drops.length; i++) {
            ...
            if (drop.ethPrice > msg.value) {
                revert("Ether is not correct");
            } else {
                activeDropIndex = i;
                break;
            }
            if (drop.limitNFT > maxMint[msg.sender]) {
                revert("You cannot mint NFT anymore");
            } else {
                activeDropIndex = i;
                break;
            }
