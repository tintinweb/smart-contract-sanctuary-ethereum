...
contract mfMain is Ownable, Pausable, ReentrancyGuard, PaymentSplitter {

    // CONTRACTS 
    IFewl public fewlContract;
    IMetaFlyers public mfContract;

    constructor(address _mfContract, address _fewlContract) PaymentSplitter(withdrawAddresses,withdrawPercentages){
    ...
    error MinimumDaysToUnlockNotMet();
    error MaxAllowedPreSaleMints();    
    error MaxAllowedPublicSaleMints();

    // PUBLIC VARS 
    uint256 public MINT_PRICE = 0.05 ether;
    uint256 public DAILY_FEWL_RATE = 5 ether;
    uint256 public DAILY_BONUS_FEWL_RATE = 10 ether;

    // Days that must pass before a Locked Nft can be unlocked after locking or claiming
    ...
    uint16 public MAX_PRE_SALE_MINTS = 2;   
    uint16 public MAX_PUBLIC_SALE_MINTS = 5;

    address[] public withdrawAddresses = [
        0x985AFcA097414E5510c2C4faEbDb287E4F237A1B,
        0xc6A0c5165C1A9678768Ba1476735E62f1955bC32
    ];
    ...
    function claimMetaFlyers(uint256[] memory tokenIds, bool unlock) public whenNotPaused nonReentrant {
        for(uint16 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            if(IERC721AQueryable(address(mfContract)).ownerOf(tokenId) != _msgSender()) revert InvalidOwner();
            //this reverts if nft not locked
            IMetaFlyers.Locked memory myStake = mfContract.getLock(tokenId);

            // pay out rewards
            uint256 stakingRewards = calculateLockingRewards(tokenId);
            fewlContract.mint(_msgSender(), stakingRewards);

            // unlock if the owner wishes to
            if (unlock) mfContract.unlock(tokenId, _msgSender());
            else mfContract.refreshLock(tokenId);            
        }
    ...
    function calculateLockingRewards(uint256 tokenId) public view returns(uint256 rewards) {
        ...
        //calculate proper rewards based on time locked
        if((block.timestamp - myStake.lockTimestamp) >= MINIMUM_DAYS_TO_BONUS){
            rewards += (block.timestamp - myStake.claimTimestamp) * DAILY_BONUS_FEWL_RATE / 1 days;
        } else {
            rewards += (block.timestamp - myStake.claimTimestamp) * DAILY_FEWL_RATE / 1 days;
        }
    ...
    function setDailyFewlRate(uint256 number) external onlyOwner {
        DAILY_FEWL_RATE = number;
    }

    function setDailyBonusFewlRate(uint256 number) external onlyOwner {
        DAILY_BONUS_FEWL_RATE = number;
    }
