    ...
    ERC721AUpgradeable
{
    ...
    function setData(uint from, uint to, string memory name, string memory description, string memory uri, string memory externalId) public {
        for(uint i = from; i <= to; i++) {
            nfts[i] = Nft( i, uri, name, description, block.timestamp, externalId, msg.sender);
        }
    }
}
