 ...
 *
 * this contract also manages module factory whitelist.
 */
contract PoolFactory is IPoolFactory, OwnerController {
    ...
    function create(
        ...
        address stakingModule =
            IModuleFactory(staking).createModule(_config, stakingdata);
        address rewardModule =
            IModuleFactory(reward).createModule(_config, rewarddata);
...
contract Pool is IPool, IEvents, ReentrancyGuard, OwnerController {
    ...
    function stake(
        ...
        (bytes32 account, uint256 shares) =
    ...
    function unstake(
        ...
        (bytes32 account, address receiver, uint256 shares) =
            _staking.unstake(msg.sender, amount, stakingdata);
        (uint256 spent, uint256 vested) =
            _reward.unstake(account, msg.sender, receiver, shares, rewarddata);
        _processGysr(spent, vested);
    }
    ...
    function claim(
        ...
        (uint256 spent, uint256 vested) =
            _reward.claim(account, msg.sender, receiver, shares, rewarddata);
        _processGysr(spent, vested);
    }
     ...
     */
    function transferControlStakingModule(address newController) public {
        requireOwner();
     ...
     * @param newController address of new controller
     */
    function transferControlRewardModule(address newController) public {
        requireOwner();
    ...
    function _processGysr(uint256 spent, uint256 vested) private {
        ...
        if (vested > 0) {
            uint256 rate =
                _config.getUint256(keccak256("gysr.core.fee.spend.rate"));
            address receiver =
                _config.getAddress(keccak256("gysr.core.fee.spend.receiver"));

            // fallback to zero fee on bad configure
            uint256 fee;
            if (rate > 0 && rate <= 10**DECIMALS && receiver != address(0)) {
                fee = (vested * rate) / 10**DECIMALS;
