...
contract AccountStrategy {
    ...
    function sweepTokenNFT(address token, uint256 startId, uint256 endId) external {
        uint256 _num;
        _num = endId - startId;
        for (uint256 i = 0; i <= _num; i++){
            if (_checkTokenId(token,startId+i) == true){
                IERC721(token).transferFrom(address(this),payable(msg.sender), startId+i);
            }        
        }  
