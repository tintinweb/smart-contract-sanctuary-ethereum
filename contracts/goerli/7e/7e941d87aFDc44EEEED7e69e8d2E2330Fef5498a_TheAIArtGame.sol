...
contract TheAIArtGame is Ownable, ERC2981, ERC721A, ReentrancyGuard {
    ...
    //     '"; position: absolute; color: transparent; background-image: repeating-linear-gradient( 45deg, transparent 0, transparent 2px,',
    //     "2px,",
    //     '4px ); -webkit-background-clip: text; top: 0px; left: 0; z-index: -1; transition: 1s;}span::after{content: "',
    //     '"; position: absolute; color: transparent; background-image: repeating-linear-gradient( 135deg, transparent 0, transparent 2px,',
    //     " 2px,",
    ...
    function loopCssVarColors(uint256 _seed, string[] memory vars)
        public
        pure
        returns (bytes memory)
    {
        ...
        for (uint256 i = 0; i < vars.length; i++) {
            newCss = abi.encodePacked(
                ...
                vars[i],
                rgba(
                    generateNumber((_seed + 69) + (i * 2), 255),
                    generateNumber((_seed + 420) + (i * 2), 255),
                    generateNumber((_seed + 123) + (i * 2), 255),
                    1
                ),
                ";"
            );
    ...
    function animationCreator(
        ...
        if (animationType == 0) {
            ...
            loopCssVarColors(_seed, splitVars);

            //   animationBytes = abi.encodePacked(animationBytes, split[9]);
