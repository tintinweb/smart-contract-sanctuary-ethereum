...
contract HordeCreaturesA is HordeMintableA {  
    ...
    string tokenName = "Creatures of the Undead";
    string version = "1";
    string tokenSymbol = "UNDEAD2";
...
contract EIP712Allowlisting is EIP712Listable {
    ...
    function recoverAddress(bytes calldata signature, address recipient) public view returns (recovered memory) {
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOM_SEP,
                keccak256(abi.encode(MINT_TYPE, recipient))
            )
        );        
        address recoveredAddress = digest.recover(signature);
        
        return recovered(recipient, signature, recoveredAddress, sigKey);
    }
    modifier requiresSig(bytes calldata sig, address recip) {
        ...
        require(recovery == msg.sender, "invalid signature");
        _;
    }
}
...
abstract contract HordeMintableA is EIP712Allowlisting, LockableA, FlexibleMetadata {  
    ...
    address payable private HORDE_AI_WALLET;

    uint256 private MAX_TOKENS = 10;
    uint256 private MAX_HORDE = 5;
    uint256 private MAX_ALLOW = 2;
    uint256 private MAX_PUBLIC = 2;

    uint256 private PUBLIC_FEE = 0.02 ether;

    bool private HORDE_LIVE = false;
    ...
    function mint(
        ...
        require(quant <= MAX_PUBLIC, INVALID_QUANT);                         
        require(askSize <= MAX_TOKENS, ALLOC_OVER);         
        require(isPublic ? PUBLIC_LIVE : HORDE_LIVE, INACTIVE_MINT); 
        require(askSize <= (isPublic ? MAX_TOKENS : MAX_HORDE), ALLOC_OVER);
        require(
            (isPublic ? public_minted[msg.sender] : allow_minted[msg.sender])+quant 
            <= (isPublic ? MAX_PUBLIC : MAX_ALLOW), ALLOC_OVER);
