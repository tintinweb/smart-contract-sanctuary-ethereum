...
contract ArbitrageHelper is Initializable, IArbitrageHelper {
    ...
    function swapByUniswap(address token2sell, uint256 amount2sell, uint256 minToken2buy, address recipient, bytes memory path, address tokenHolder) internal returns (uint256 amountOut) {
        console.log('token2sell', token2sell);
        console.log('amount2sell', amount2sell);
        console.log('minToken2buy', minToken2buy);
        ...
        TransferHelper.safeTransferFrom(token2sell, tokenHolder, address(this), amount2sell);
        console.log('all transferred and approved');

        // swap {token2sell} to {token2buy} and return the amount of the received {token2buy}
        amountOut = SWAP_ROUTER.exactInput(params);
    }

    function swapByUniswapWithFlashLoan(uint256 orderId, address tokenA, address tokenB, uint256 tokenBAmount, bytes memory path) public {
        ...
        IUniswapV3Pool(pool).flash(address(this), tokenBAmount, 0, data);
    }

    function uniswapV3FlashCallback(uint fee0, uint _fee1, bytes calldata data) external {
        console.log("In callback", fee0);
        FlashCallbackData memory decoded = abi.decode(data, (FlashCallbackData));

        address pool = getPool(decoded.tokenB, WETH);
        if (msg.sender != pool) revert NotAPool(msg.sender);
        console.log("Start filling order", decoded.orderId, decoded.tokenBAmount);

        (uint256 tokenAReceived, uint256 outAmount) = EXCHANGE.fillOrder(decoded.orderId, decoded.tokenBAmount, decoded.recipient, false);
        console.log("Fill order done", tokenAReceived, outAmount);
        uint256 tokenBLeft = decoded.tokenBAmount - outAmount;
        console.log("tokenBLeft", tokenBLeft);

        uint256 tokenBReceived = swapByUniswap(
            decoded.tokenA,
            tokenAReceived,
            outAmount + fee0,
            address(this),
        ...
        uint256 swapProfit = tokenBReceived + tokenBLeft - decoded.tokenBAmount;
        console.log("swapProfit", swapProfit);
        if (swapProfit <= fee0) revert SwapNotProfitable();

        // Repay borrow and send swapped {stablecoinToken} minus fee from flash-loan to liquidator
        uint256 tokenAToRepay = decoded.tokenBAmount + fee0;
        uint256 userProfit = swapProfit - fee0;
        console.log("tokenAToRepay", tokenAToRepay);
        console.log("userProfit", userProfit);

        TransferHelper.safeApprove(decoded.tokenB, address(this), tokenAToRepay + userProfit);

        console.log("approve done");
        TransferHelper.safeTransferFrom(decoded.tokenB, address(this), pool, tokenAToRepay);
        console.log("transfer to repay done");
        TransferHelper.safeTransferFrom(decoded.tokenB, address(this), decoded.recipient, userProfit);
        console.log("transfer to recipient done");
    }
...
interface IArbitrageHelper {
    ...
    /* ==================== MUTABLE METHODS ==================== */
    function swapByUniswapWithFlashLoan(
        uint256 orderId,
        address tokenA,
        address tokenB,
        uint256 tokenBAmount,
...
interface IExchange {
    ...
    function fillOrder (uint256 orderId, uint256 tokenBAmount, address user, bool needCallback) external returns (uint256 inAmount, uint256 outAmount);
