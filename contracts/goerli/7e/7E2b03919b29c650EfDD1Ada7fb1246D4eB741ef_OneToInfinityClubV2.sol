...
contract OneToInfinityClubV2 is ERC721Enumerable, Ownable, Airdrop {
  ...
  constructor(string memory baseURI) ERC721("Hustler Big Boys", "HBB") {
    setBaseURI(baseURI);
  }
  ...
  function mintNft(address addr) private {
    ...
    require(currentTokenId < maxSupply, "NFT run out of supply!");
  ...
  function distributeAirdrop() public onlyOwner {
    for (uint256 i = 0; i < addressesForAirdrop.length; i++) {
      ...
      if (addressToAllowedAirdrop[addr] && !addressHasReceiveAirdrop[addr]) {
        addressHasReceiveAirdrop[addr] = true;
        mintNft(addr);
      }
    }
  }
  ...
  function claimAirdrop() public {
    ...
    require(!addressHasReceiveAirdrop[msg.sender], "airdrop has been claimed");

    addressHasReceiveAirdrop[msg.sender] = true;
    mintNft(msg.sender);
  }
...
contract Airdrop is Ownable {
  ...
  mapping(address => bool) public addressHasReceiveAirdrop;
  ...
  function addAddressesForAirdop(address[] memory _addresses) public onlyOwner {
    for (uint256 i = 0; i < _addresses.length; i++) {
      if (!addressToAllowedAirdrop[_addresses[i]]) {
        addressToAllowedAirdrop[_addresses[i]] = true;
        addressesForAirdrop.push(_addresses[i]);
      }
    }
  }

  function airdropAddressCount() public view returns (uint256) {
    return addressesForAirdrop.length;
  }
}
