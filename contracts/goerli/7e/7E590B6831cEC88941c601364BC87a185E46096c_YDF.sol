...
contract YDF is IYDF, ERC20, Ownable {
  ...
  uint256 public constant TAX_DEV_PERC = 100; // 1%
  uint256 public constant TAX_EARLY_PERC = 1400; // 14%
  ...
  constructor() ERC20('Yieldification', 'YDF') {
    ...
    _stakedYDF.setPaymentAddress(owner());
    _stakedYDF.setRoyaltyAddress(owner());
    _stakedYDF.transferOwnership(owner());
    ...
    _stakedYDFLP.setPaymentAddress(owner());
    _stakedYDFLP.setRoyaltyAddress(owner());
    _stakedYDFLP.transferOwnership(owner());
  ...
  function _transfer(
    ...
    if (_isBuy) {
      ...
      lastBuyAmount[recipient] = amount;
    ...
    bool _overMinimum = contractBalance >= _swapAmount;
    ...
    uint256 tax = 0;
    if (_isSell) {
      uint256 _taxEarlyPerc = getSellEarlyTax(sender, amount, TAX_EARLY_PERC);
      uint256 _totalTax = TAX_DEV_PERC + _taxEarlyPerc;
      tax = (amount * _totalTax) / 10000;
      if (tax > 0) {
        uint256 _taxAmountDev = (tax * TAX_DEV_PERC) / 10000;
        super._transfer(sender, address(this), _taxAmountDev);
        if (_taxEarlyPerc > 0) {
          _burn(sender, tax - _taxAmountDev);
        }
      }
    }
  ...
  function getSellEarlyTax(
    ...
    if (lastBuyAmount[_sender] < _sellAmount) {
      return _tax;
    }
...
contract sYDF is YDFStake {
    ...
    )
  {
    ...
    _addAprLockOption(15000, 60 days);
...
contract slYDF is YDFStake {
  ...
  {
    _addAprLockOption(15000, 0);
    _addAprLockOption(25000, 60 days);
    _addAprLockOption(35000, 120 days);
  }
}
...
contract YDFStake is ERC721Enumerable, Ownable {
  ...
  AprLock[] private _aprLockOptions;
  ...
  function stake(uint256 _amount, uint256 _lockOptIndex) external {
    require(_lockOptIndex < _aprLockOptions.length, 'invalid lock option');
    address _user = msg.sender;
    _amount = _amount == 0 ? stakeToken.balanceOf(_user) : _amount;
    require(_amount > 0, 'must stake at least some tokens');
  ...
  function unstake(uint256 _tokenId) external {
    ...
    require(
      block.timestamp > _stake.created + _stake.lockTime,
      'must wait for lock period to expire'
    );

    uint256 _secondsStaked = block.timestamp - _stake.created;
    uint256 _earnedAmount = (_stake.amountStaked *
      _stake.apr *
      _secondsStaked) /
      PERCENT_DENOMENATOR /
      ONE_YEAR;
    ...
    stakeToken.transfer(_user, _stake.amountStaked);
