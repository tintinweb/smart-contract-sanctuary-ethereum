...
contract SoundBound is ERC721Enumerable, Ownable {
    ...
    event LogNFTCreated(uint256 indexed curTokenId, address indexed ownerWalletAddress, string domainSuffix, uint32 genre);
    ...
    function domainToNode(string memory domainSuffix) internal pure returns (bytes32) {
        bytes32 label = keccak256(bytes(domainSuffix));
        return label;
    }
    ...
    // Purchase music NFT
    function purchaseNFT(string memory domainSuffix, uint32 genreId) external payable {
        ...
        bytes32 node = domainToNode(domainSuffix);
        ...
        emit LogNFTCreated(newItemId, msg.sender, domainSuffix, genreId);
    }
