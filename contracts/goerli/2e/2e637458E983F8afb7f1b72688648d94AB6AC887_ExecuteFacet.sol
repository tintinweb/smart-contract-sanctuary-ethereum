...
contract ExecuteFacet is IOperations, ReentrancyGuard {
    ...
    function executeBatch(bytes[] calldata _data) external nonReentrant {
        ...
        for (uint256 i = 0; i < _data.length; ++i) {
            ...
            if (requestType == 0) {
                ...
                Withdraw memory withdraw = LibOperations._readWithdrawRequest(pubData, 1);
                require(s.tokenType[withdraw.listingId] == 1, "EF: Withdrawn token is not ERC20 token");
                _withdrawOrStoreCryptoCurrency(withdraw.to, withdraw.listingId, withdraw.amount);
                // Case 4: Execute withdraw ERC721 token request
            } else if (requestType == 4) {
                Withdraw memory withdraw = LibOperations._readWithdrawRequest(pubData, 1);
                require(s.tokenType[withdraw.listingId] == 2, "EF: Withdrawn token is not ERC721 token");
                _releaseOrMintERC721(withdraw.to, withdraw.listingId, withdraw.tokenId);
                // Case 5: Execute withdraw ERC1155 token request
            } else if (requestType == 5) {
                Withdraw memory withdraw = LibOperations._readWithdrawRequest(pubData, 1);
                require(s.tokenType[withdraw.listingId] == 3, "EF: Withdrawn token is not ERC1155 token");
                ...
                require(s.tokenType[withdraw1155C.listingId] == 4, "EF: Withdrawn token is not ERC1155C token");
                ...
                require(s.tokenType[withdrawBatch.listingId] == 3, "EF:not a ERC1155 token withdrawal");
    ...
    function transferERC20(
        ...
        require(balanceAfter >= _liquidityMin, "EF:Minimum liquidity limit is reached");
