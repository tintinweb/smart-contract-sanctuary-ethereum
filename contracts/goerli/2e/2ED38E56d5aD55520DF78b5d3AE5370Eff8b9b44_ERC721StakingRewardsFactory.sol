...
contract ERC721StakingRewardsFactory is Ownable {
    ...
    ERC721StakingRewards[] private contracts;
    ...
    function getDeployments() external view returns (ERC721StakingRewards[] memory) {
        return contracts;
    }
...
contract ERC721StakingRewards is Ownable, ReentrancyGuard {
    ...
    mapping(uint8 => mapping(address => uint72)) public collections;
    ...
    function addCollection(address _contract, uint72 _softRate, uint72 _hardRate) public onlyOwner {
        ...
        for (uint256 i = 0; i < contracts.length; i++) {
            if (_contract == contracts[i]) exists = true;
        }
        require(!exists, "Bad collection");
        contracts.push(_contract);
        collections[HARD_STAKED][_contract] = _hardRate;
        collections[SOFT_STAKED][_contract] = _softRate;
    }
    ...
    function removeCollection(address _contract) public onlyOwner {
        ...
        delete collections[HARD_STAKED][_contract];
        delete collections[SOFT_STAKED][_contract];
        for (uint256 i = 0; i < contracts.length; i++) {
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < contracts.length; i++) {
            ...
            uint256 _rate = collections[_type][_contract];
