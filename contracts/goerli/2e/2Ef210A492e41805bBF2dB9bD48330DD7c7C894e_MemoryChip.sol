...
contract MemoryChip is ERC721AQueryable, Owned, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) private _peasantsMinted;
    ...
    constructor(address peasantContract_) ERC721A("MemoryChip", "MC") Owned(msg.sender) {
    ...
    function implant(uint256 cardId_) external nonReentrant() {
        ...
        if (!_canImplant) revert YouCantImplantNow();
        // Saving this for easier access.
        address tokenOwner = ownerOf(cardId_);
        // If you are not owner, and you re not even the contract owner, you can't make a deal.
        if (_msgSenderERC721A() != tokenOwner) revert NotYourMemoryChip();
    ...
    function _isWithinSupply(uint256 quantity_) private view {
        ...
        if (_supplyCap < totalSupply() - _peasantsMintCounter + quantity_) revert SupplyExceeded();
    }

    function _isWithinWalletLimit(uint256 quantity_) private view {
        // Did the user already exceed the allowed limit?
        if (_numberMinted(msg.sender) + quantity_ > _maxMintPerAddress) revert AddressQuantityExceeded();
    }
    ...
    function mintFilthy(uint256 peasantId_) external {
        ...
        // Are you filthy?
        if (FilthyPeasantsContract(_peasantContract).ownerOf(peasantId_) != msg.sender) revert CantMintFilthy();
        // Has the peasant been redeemed?
        if (_peasantsMinted[peasantId_] == 1) revert PeasantAlreadyMinted();
        // If not, it is redeemed now
        _peasantsMinted[peasantId_] = 1;
    ...
    function peasantMinted(uint256 peasantId_) external view returns (bool) {
        return _peasantsMinted[peasantId_] == 1;
    }
    ...
    function tokenURI(uint256 tokenId_) public view virtual override(ERC721A, IERC721A) returns (string memory) {
        ...
        return bytes(currentBaseURI).length > 0
            ? string(abi.encodePacked(currentBaseURI, LibString.toString(tokenId_)))
            : '';
    }
