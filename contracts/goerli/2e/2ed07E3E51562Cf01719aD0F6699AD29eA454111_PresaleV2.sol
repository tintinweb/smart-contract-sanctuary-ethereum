    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        __Ownable_init_unchained();
        __ReentrancyGuard_init_unchained();
        salePrice = 0.000000015 * (10**18); //0.11 USD
        totalTokens = 150_000_000; // 150 Million
        inSale = totalTokens;
        baseDecimals = (10**18);
        ...
        returns (uint256 totalValue)
    {
        ...
        if (totalSold + _amount <= 40_000_000) {
            ...
            if (totalSold + _amount >= 40_000_000 || totalSold + _amount <= 60_000_000) {
                _salePrice = 0.0000000155 * (10**18);
                return(_amount * _salePrice);
       
            }else if(totalSold + _amount >= 60_000_000 || totalSold + _amount <= 75_000_000){
                _salePrice = 0.000000016 * (10**18);
                return(_amount * _salePrice);

            }else if(totalSold + _amount >= 75_000_000 || totalSold + _amount <= 90_000_000){
                _salePrice = 0.0000000165 * (10**18);
                return(_amount * _salePrice);
            }else if(totalSold + _amount >= 90_000_000 || totalSold + _amount <= 105_000_000){
                _salePrice = 0.000000017 * (10**18);
                return(_amount * _salePrice);
            }else if(totalSold + _amount >= 105_000_000 || totalSold + _amount <= 120_000_000){
                _salePrice = 0.000000018 * (10**18);
                return(_amount * _salePrice);
            }else if(totalSold + _amount >= 120_000_000 || totalSold + _amount <= 135_000_000){
                _salePrice = 0.000000019 * (10**18);
                return(_amount * _salePrice);
            }else if( totalSold > 135_000_000 ) {
                _salePrice = 0.000000020 *  (10**18);
                return(_amount * _salePrice);           
            }
        }
    }
