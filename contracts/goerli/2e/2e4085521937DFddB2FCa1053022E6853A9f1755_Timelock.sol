...
contract Timelock is Permissions {
  ...
  function setGracePeriod(uint256 _gracePeriod)
    external
    onlyRoleYsd(GOVERNOR_ROLE, "Must have timelock role")
  {
    require(
      _gracePeriod > delay,
  ...
  function queueTransaction(
    ...
    external
    onlyRoleYsd(GOVERNOR_ROLE, "Timelock::queueTransaction: Call must come from governor.")
    returns (bytes32)
  {
    require(
  ...
  function cancelTransaction(
    ...
    external
    onlyRoleYsd(GOVERNOR_ROLE, "Timelock::cancelTransaction: Call must come from governor.")
  {
    bytes32 txHash = keccak256(
      abi.encode(target, value, signature, data, eta)
    );
  ...
  function executeTransaction(
    ...
    external
    payable
    onlyRoleYsd(GOVERNOR_ROLE, "Timelock::executeTransaction: Call must come from governor.")
