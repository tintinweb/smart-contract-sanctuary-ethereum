...
import "./interfaces/imports/IUniswapV2Pair.sol";
...
contract CompoundPool is ICompoundPool, OwnableUpgradeable, AbstractDependant {
    using SafeMath for uint256;

    uint256 public constant USD_DECIMALS = 10**8;

    IStaking public deinStaking;
    IStaking public liquidityMiningStakingETH;
    IPolicyBookRegistry public policyBookRegistry;
    IPriceFeed public priceFeed;
    IUniswapV2Pair public DEINLPToken;
    IERC20Metadata public stblToken;

    address public claimingRegistryAddress;
    address public deinTokenAddress;

    uint256 public compoundNativeUsage; // percentage of compound native liq
    uint256 public compoundLPUsage; // percentage of compound lp liq
    uint256 public compoundPoolShare; // share percentage for policy book

    uint256 public totalUsage; // total used in buying policy
    mapping(address => uint256) public override poolsUsage; // policybook => used compound liquidity

    uint256 public stblDecimals;

    event DeployedLiquidity(
        address indexed policyBook,
        uint256 deployedAmount,
        uint256 totalUsage
    );

    event UndeployedLiquidity(
        address indexed policyBook,
        uint256 undeployedAmount,
        uint256 totalUsage
    );
    event CompoundPoolLiquidated(
        address indexed claimer,
        address indexed policyBook,
        uint256 liquidationAmount,
        uint256 lpLiquidationAmount,
        uint256 nativeLiquidationAmount
    );
    ...
    modifier onlyPolicyBook() {
        require(
            ...
            "COMPool: Not a PolicyBook"
        );
        _;
    }

    modifier onlyClaimingRegistry() {
        require(_msgSender() == claimingRegistryAddress, "COMPool: Not a Claiming Registry");
        _;
    }

    function __CompoundPool_init() external initializer {
        __Ownable_init();
        compoundNativeUsage = 40 * PRECISION; // 40%
        compoundLPUsage = 10 * PRECISION; // 10%
        compoundPoolShare = 10 * PRECISION; // 10%
    }
        ...
        onlyInjectorOrZero
    {
        ...
        liquidityMiningStakingETH = IStaking(
            _contractsRegistry.getLiquidityMiningStakingETHContract()
        );

        DEINLPToken = IUniswapV2Pair(_contractsRegistry.getAMMDEINToETHPairContract());
        stblToken = IERC20Metadata(_contractsRegistry.getUSDTContract());

        claimingRegistryAddress = _contractsRegistry.getClaimingRegistryContract();
        policyBookRegistry = IPolicyBookRegistry(
            _contractsRegistry.getPolicyBookRegistryContract()
        );
        priceFeed = IPriceFeed(_contractsRegistry.getPriceFeedContract());
        deinTokenAddress = _contractsRegistry.getDEINContract();
        stblDecimals = stblToken.decimals();
    }

    ///@notice passed in 10**25 PRECSION
    function setupCompoundPool(
        uint256 _compoundNativeUsage,
        uint256 _compoundLPUsage,
        uint256 _compoundPoolShare
    ) external onlyOwner {
        require(
            _compoundNativeUsage > 0 &&
                _compoundNativeUsage <= PERCENTAGE_100 &&
                _compoundLPUsage > 0 &&
                _compoundLPUsage <= PERCENTAGE_100 &&
                _compoundPoolShare > 0 &&
                _compoundPoolShare <= PERCENTAGE_100,
            "COMPool: invalid inputs"
        );

        compoundNativeUsage = _compoundNativeUsage;
        compoundLPUsage = _compoundLPUsage;
        compoundPoolShare = _compoundPoolShare;
    }

    ///@dev get all compounded liquidty of native and lp staking in usd multiplied (10**18) based on
    /// usage percentage
    function getCompoundLiquidity() public view override returns (uint256) {
        return _getCompoundLiquidityForNative().add(_getCompoundLiquidityForLP());
    }

    ////@dev get compounded liquidity in usd based on compoundLPUsage
    function _getCompoundLiquidityForLP() internal view returns (uint256) {
        return _getLPLiquidity().mul(compoundLPUsage).uncheckedDiv(PERCENTAGE_100);
    }

    ////@dev get compounded liquidity in usd based on compoundNativeUsage
    function _getCompoundLiquidityForNative() internal view returns (uint256) {
        return _getNativeLiquidity().mul(compoundNativeUsage).uncheckedDiv(PERCENTAGE_100);
    }

    ///@dev get total native liquidity in usdt
    function _getNativeLiquidity() internal view returns (uint256) {
        uint256 nativeLiquidity = deinStaking.getPoolLiquidity();

        return _calcTokenPrice(nativeLiquidity, 0);
    }

    ///@dev get total lp liquidity in usdt
    function _getLPLiquidity() internal view returns (uint256) {
        uint256 lpLiquidity = liquidityMiningStakingETH.getPoolLiquidity();

        (uint256 _deinAmount, uint256 _ethAmount) = _convertLPToReserveTokens(lpLiquidity);

        return _calcTokenPrice(_deinAmount, _ethAmount);
    }

    ///@dev return available compounded liquidty for policy book pool in in usd multiplied (10**18)
    // based on compound pool share
    ...
    {
        if (IPolicyBook(_policyBookAddress).whitelisted()) {
            // get total compound lq
            uint256 _compoundLiquidity = getCompoundLiquidity();
            // get share of the pool
            uint256 _poolCompoundLiquidity =
                _compoundLiquidity.mul(compoundPoolShare).uncheckedDiv(PERCENTAGE_100);
            // get available compound lq for the pool
            uint256 _poolNetCompoundLiquidity =
                _poolCompoundLiquidity.trySub(poolsUsage[_policyBookAddress]);
            // return avaiable compound lq
            _availableLiquidity = Math.min(
                _poolNetCompoundLiquidity,
                _compoundLiquidity.trySub(totalUsage)
            );
        }
    }

    ///@notice _amount in 18 decimals
    function deployLiquidity(address _policyBook, uint256 _amount)
        external
        override
        onlyPolicyBook
    {
        poolsUsage[_policyBook] = poolsUsage[_policyBook].add(_amount);
        totalUsage = totalUsage.add(_amount);

        emit DeployedLiquidity(_policyBook, _amount, totalUsage);
    }

    ///@notice _amount in 18 decimals
    function undeployLiquidity(address _policyBook, uint256 _amount)
        public
        override
        onlyPolicyBook
    {
        _undeployLiquidity(_policyBook, _amount);
    }

    function _undeployLiquidity(address _policyBook, uint256 _amount) internal {
        poolsUsage[_policyBook] = poolsUsage[_policyBook].sub(_amount);
        totalUsage = totalUsage.sub(_amount);

        emit UndeployedLiquidity(_policyBook, _amount, totalUsage);
    }
    ...
    function liquidate(
        ...
        address claimer,
        uint256 liquidationAmount
    ) external override onlyClaimingRegistry {
        // release compounded liquidation from the pool
        _undeployLiquidity(_policyBook, liquidationAmount);

        //liquidate lp first
        uint256 liquidationAmountFromLP =
            liquidationAmount <= _getCompoundLiquidityForLP()
                ? liquidationAmount
                : _getCompoundLiquidityForLP();

        // liquidate native
        uint256 liquidationAmountFromNative =
            liquidationAmountFromLP < liquidationAmount
                ? liquidationAmount.sub(liquidationAmountFromLP)
                : 0;

        liquidationAmountFromNative = liquidationAmountFromNative <=
            _getCompoundLiquidityForNative()
            ? liquidationAmountFromNative
            : _getCompoundLiquidityForNative();

        uint256 leftAmount =
            liquidationAmount.trySub(liquidationAmountFromLP.add(liquidationAmountFromNative));

        // second liquidation
        if (leftAmount > 0) {
            uint256 liquidationAmountFromLP2 =
                Math.min(_getLPLiquidity().sub(liquidationAmountFromLP), leftAmount);
            uint256 liquidationAmountFromNative2 =
                Math.min(
                    _getNativeLiquidity().sub(liquidationAmountFromNative),
                    leftAmount.sub(liquidationAmountFromLP2)
                );

            liquidationAmountFromLP = liquidationAmountFromLP.add(liquidationAmountFromLP2);
            liquidationAmountFromNative = liquidationAmountFromNative.add(
                liquidationAmountFromNative2
            );
        }

        if (liquidationAmount == liquidationAmountFromLP.add(liquidationAmountFromNative)) {
            // liqudiate from lp
            if (liquidationAmountFromLP > 0) {
                // calc lp token price
                uint256 _deinLPTokenPrice = _calcDEINLPTokenPrice();
                // calc how many lp token in usdt amount
                liquidationAmountFromLP = liquidationAmountFromLP.mul(DECIMALS18).tryDiv(
                    _deinLPTokenPrice
                );

                (uint256 _amountDEIN, uint256 _amountETH) =
                    _convertLPToReserveTokens(liquidationAmountFromLP);

                uint256 _minAmountDEIN =
                    _amountDEIN.mul(99).mul(PRECISION).uncheckedDiv(PERCENTAGE_100);

                uint256 _minAmountETH =
                    _amountETH.mul(99).mul(PRECISION).uncheckedDiv(PERCENTAGE_100);

                liquidityMiningStakingETH.liquidate(
                    liquidationAmountFromLP,
                    _minAmountDEIN,
                    _minAmountETH,
                    claimer
                );
            ...
            // liqudate from native
            if (liquidationAmountFromNative > 0) {
                // calc how many dein token in usdt amount
                liquidationAmountFromNative = priceFeed.howManyDEINsInUSDT(
                    DecimalsConverter.convertFrom18(liquidationAmountFromNative, stblDecimals)
                );

                deinStaking.liquidate(liquidationAmountFromNative, 0, 0, claimer);
            }

            emit CompoundPoolLiquidated(
                claimer,
                _policyBook,
                liquidationAmount,
                liquidationAmountFromLP,
    ...
    ///@dev can't withdraw of purchased policies for Compounded liquidity >= funds in the Native and LP staking
    function canWithdraw(uint256 _withdrawAmount, bool isLPToken)
        external
        view
        override
        returns (bool)
    {
        uint256 _withdrawAmountDEIN = _withdrawAmount;
        uint256 _withdrawAmountETH;
        if (isLPToken)
            (_withdrawAmountDEIN, _withdrawAmountETH) = _convertLPToReserveTokens(_withdrawAmount);

        uint256 _withdrawPrice = _calcTokenPrice(_withdrawAmountDEIN, _withdrawAmountETH);

        return _getNativeLiquidity().add(_getLPLiquidity()).trySub(_withdrawPrice) >= totalUsage;
    }

    function _calcTokenPrice(uint256 _deinAmount, uint256 _ethAmount)
        internal
        view
        returns (uint256)
    {
        return
            DecimalsConverter.convertTo18(
                priceFeed.howManyUSDTsInDEIN(_deinAmount).add(
                    priceFeed.howManyUSDTsInETH(_ethAmount)
                ),
                stblDecimals
            );
    }

    function _convertLPToReserveTokens(uint256 _lpAmount)
        internal
        view
        returns (uint256 _amountDEIN, uint256 _amountETH)
    {
        (uint256 totalSupply, uint256 reserveDEIN, uint256 reserveETH) = _getLPDEINPoolInfo();

        _amountDEIN = _lpAmount.mul(reserveDEIN).tryDiv(totalSupply);

        _amountETH = _lpAmount.mul(reserveETH).tryDiv(totalSupply);
    }

    function _calcDEINLPTokenPrice() internal view returns (uint256) {
        (uint256 totalSupply, uint256 reserveDEIN, uint256 reserveETH) = _getLPDEINPoolInfo();

        uint256 _totalValue = _calcTokenPrice(reserveDEIN, reserveETH);

        return _totalValue.mul(DECIMALS18).tryDiv(totalSupply);
    }

    function _getLPDEINPoolInfo()
        internal
        view
        returns (
            uint256 totalSupply,
    ...
    {
        totalSupply = DEINLPToken.totalSupply();

        if (DEINLPToken.token0() == deinTokenAddress)
            (reserveDEIN, reserveETH, ) = DEINLPToken.getReserves();
        else (reserveETH, reserveDEIN, ) = DEINLPToken.getReserves();
    }
}
