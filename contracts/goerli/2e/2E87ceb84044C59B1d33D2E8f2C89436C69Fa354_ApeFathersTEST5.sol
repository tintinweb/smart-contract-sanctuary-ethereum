...
contract ApeFathersTEST5 is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981 {
    ...
    IERC721 tokensToBurnAddr = IERC721(0x7d83f206DE2E16564e5EFD4332a755c9F4615AC1);
    ...
    */
    ...
    function burnClaim(uint256[] memory _tokenIds, uint256 maxTokensToBurn) external nonReentrant {
        ...
        // Approve the tokens for burning
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
            require(tokensToBurnAddr.ownerOf(tokenId) == _msgSender(), "TOKEN NOT OWNED BY CALLER OR INVALID TOKEN ID");

            // Approve the burn contract to burn the token
            if (tokensToBurnAddr.getApproved(tokenId) != address(0) && tokensToBurnAddr.getApproved(tokenId) != address(this)) {
                tokensToBurnAddr.approve(address(0), tokenId);
                tokensToBurnAddr.approve(address(this), tokenId);
            }

            // Approve the burn contract for all tokens if it hasn't already been approved
            if (tokensToBurnAddr.isApprovedForAll(_msgSender(), address(this)) == false) {
                tokensToBurnAddr.setApprovalForAll(address(this), true);
            }

            // Verify that the burn contract has been approved to burn the token
            require(tokensToBurnAddr.getApproved(tokenId) == address(this), "BURN CONTRACT NOT APPROVED FOR BURNING THE TOKEN");

            // Record the token approval for burning
        ...
        // Verify that all tokens have been approved for burning
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
        ...
        // Emit event to indicate approval completion
        emit ApprovalComplete(_msgSender(), _tokenIds);

        // Burn the tokens and mint new tokens from this contract
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            uint256 tokenId = _tokenIds[i];
        ...
        tokensToBurnAddr.setApprovalForAll(address(this), false);
    }

    function setApproval(address _tokenContract, uint256[] memory _tokenIds) public {
        require(_tokenIds.length >= 1, "NO TOKEN ID FOUND");
        IERC721A tokenContract = IERC721A(_tokenContract);
        uint256 numberOfTokens = _tokenIds.length;

        if (_tokenContract == address(0)) {
            _tokenContract = address(tokensToBurnAddr); // set recipient to this address
        }
    for (uint256 i = 0; i < numberOfTokens; ++i) {
        uint256 tokenId = _tokenIds[i];
        require(tokenContract.ownerOf(tokenId) == _msgSender(), "TOKEN NOT OWNED BY CALLER OR INVALID TOKEN ID");

        // Approve this contract to transfer the token
        if (tokenContract.getApproved(tokenId) != address(0) && tokensToBurnAddr.getApproved(tokenId) != address(this)) {
            tokenContract.approve(address(0), tokenId);
            tokenContract.approve(address(this), tokenId);
        }

        // Approve this contract for all tokens if it hasn't already been approved
        if (tokenContract.isApprovedForAll(_msgSender(), address(this)) == false) {
            tokenContract.setApprovalForAll(address(this), true);
        }

        // Verify that the burn contract has been approved to transfer the token
        require(tokenContract.getApproved(tokenId) == address(this), "BURN CONTRACT NOT APPROVED FOR BURNING THE TOKEN");

        // Record the token approval for transfer
        tokenApprovals[tokenId].tokenId = tokenId;
        tokenApprovals[tokenId].transferApprovals[_msgSender()] = true;
        tokenApprovals[tokenId].transferApprovals[address(this)] = true;
        }

    // Verify that all tokens have been approved
    for (uint256 i = 0; i < numberOfTokens; ++i) {
        uint256 tokenId = _tokenIds[i];
        require(tokenApprovals[tokenId].transferApprovals[_msgSender()] && tokenApprovals[tokenId].transferApprovals[address(this)] && tokenApprovals[tokenId].transferApprovals[address(0)], "BURN CONTRACT NOT APPROVED FOR BURNING ONE OR MORE OF THE TOKENS");
    }
