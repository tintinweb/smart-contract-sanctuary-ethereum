...
//    uint256 private whitelistPurchasePrice = 28500000000000000 wei;
    ...
    event LogNFTCreated(uint256 indexed tokenId, address indexed ownerWalletAddress, bytes32 node, uint32 genre);

    // contract paused
    bool public paused = false;

    // Mapping from node to its purchased status
    mapping(bytes32 => bool) private purchaseStatus;

    // Mapping from node to a token id
    ...
    // Purchase music NFT
    function purchaseNFT(bytes32 node, uint32 genreId) external payable {
        ...
        require(purchaseStatus[node] == false, "This NFT has been purchased.");
...
//        require(msg.sender == ens.ownerOf(node),  "You are not the owner of this node.");
        ...
        purchaseStatus[node] = true;
        tokenId.increment();
        uint256 newItemId = tokenId.current();
        _safeMint(msg.sender, newItemId);
        nodeToTokenId[node] = newItemId;
        emit LogNFTCreated(newItemId, msg.sender, node, genreId);
    }
    ...
    function getTokenIdByNode(bytes32 node) external view returns (uint256) {
        require(purchaseStatus[node] == true,  "The domain associated with this node hasn't been purchased yet.");
        return nodeToTokenId[node];
    }
