...
contract BaseRegistrarImplementation is ERC721, BaseRegistrar {
    ...
    modifier live() {
        require(ens.owner(baseNode) == address(this), "Registrar is not live");
        _;
    }
    ...
    function _register(
        ...
        require(block.timestamp + duration + GRACE_PERIOD > block.timestamp + GRACE_PERIOD, "overflow"); // Prevent future overflow
...
contract ETHRegistrarController is Ownable {
    ...
    constructor(
        ...
        require(_maxCommitmentAge > _minCommitmentAge, "max must be greater than min");
        base = _base;
        prices = _prices;
    ...
    function makeCommitmentWithConfig(
        ...
        require(resolver != address(0), "resolver must be set");
    ...
    function commit(bytes32 commitment) public {
        ...
        require(commitments[commitment] + maxCommitmentAge < block.timestamp, "commitment not yet valid");
        // 设置commitment时间戳
    ...
    function registerWithConfig(
        ...
        if (resolver != address(0)) {
            ...
            require(addr == address(0), "addr must be zero");
    ...
    function _consumeCommitment(
        ...
        require(commitments[commitment] + minCommitmentAge <= block.timestamp, "commitment must be older than minCommitmentAge");
        // commitment需要在24小时内消费
        require(commitments[commitment] + maxCommitmentAge > block.timestamp, "commitment must be younger than maxCommitmentAge");
        // 域名有效性校验:长度大于等于3、未注册或者超出90天保留期
        require(available(name), "name must be available");
        // 删除该commitment
        delete (commitments[commitment]);
        // 域名价格=域名单价*域名时长+附加费用(0)
        uint256 cost = rentPrice(name, duration);
        // 域名时长至少28天
        require(duration >= MIN_REGISTRATION_DURATION, "duration must be at least 28 days");
        // msg.value大于cost
        require(msg.value >= cost, "msg.value must be at least the cost of the name");
...
abstract contract ResolverBase is SupportsInterface {
    ...
    modifier authorised(bytes32 node) {
        require(isAuthorised(node), "Resolver: Unauthorized");
        _;
    }
}
