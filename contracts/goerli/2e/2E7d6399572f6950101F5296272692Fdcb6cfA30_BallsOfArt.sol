...
contract BallsOfArt is ERC721, Ownable {
    ...
    function createBallStruct(
        ...
        return
            Ball({
                ...
                width: width,
                height: height,
                fill: ballColors(randomBase % 33), // Choose random color from bColors array
    ...
    function ballSvg(Ball memory ball) internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '<rect x="',
        ...
        returns (string memory)
    {
        ...
        uint ballSize1 = drawBallSize(3, randomBase);
        console.log("Ball size 1: ", ballSize1);

        // Ball size 1x? Paint 1x at slot 1
        if (ballSize1 == 1) {
            Ball memory ball1 = createBallStruct(150, y, 300, 300, randomBase);
            lineSvg = string.concat(lineSvg, ballSvg(ball1));

            // Slot 2
            // Size of ball at slot 2
            uint ballSize2 = drawBallSize(2, randomBase >> 2);
    ...
    function generateFinalSvg() internal view returns (string memory) {
