...
contract PortfolioScoreOracle is PortfolioScore, ChainlinkClient
{
	using Chainlink for Chainlink.Request;

	event DataRequested(string url, bytes32 requestId);
	event RequestFullFilled(bytes32 requestId, uint256 apy, uint256 riskScore, uint256 portfolioScore);
	...
	mapping (bytes32 => address) public vaultForRequestId;

	constructor (address _oracle, bytes32 _jobId, uint256 _fee, address _link, string memory _uri)
	{
		if (_link == address(0))
		{
	...
	function requestVaultData(address vaultAddress) public returns (bytes32 requestId) 
	{
		Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
		string memory vault = toAsciiString(vaultAddress);
		string memory url = string.concat(uri, vault);
		...
		vaultForRequestId[requestId] = vaultAddress;

		emit DataRequested(url, requestId);
	}

	function fulfill(bytes32 requestId,
		uint256 s0, uint256 s1,
		uint256 s2, uint256 s3,
		uint256 s4, uint256 s5,
		uint256 profitScore, uint256 apy) public recordChainlinkFulfillment(requestId)
	{
		address vault = vaultForRequestId[requestId];
		...
		scoreData.profitScore = profitScore;
		scoreData.apyInPpm = apy;
		
		emit ScoreDataUpdated(vault);
		emit RequestFullFilled(requestId, apy, profitScore, getPortfolioScore(vault));
	}
