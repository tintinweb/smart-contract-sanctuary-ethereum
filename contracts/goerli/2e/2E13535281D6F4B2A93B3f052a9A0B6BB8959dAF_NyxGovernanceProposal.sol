...
contract NyxGovernanceProposal is Ownable, converter {
    ...
    {
        require(params.length == 4, "wrong format");
    ...
    function getProposal(bytes memory proposalBytes)
        public pure
        returns (bytes[4] memory prop)
    {
        GovernanceProposalParams memory params = parseProposalParams2(proposalBytes);
        prop = [abi.encode(params.ambassadorAddress), abi.encode(params.action), abi.encode(params.socialNetworkLink), abi.encode(params.ambassadorDescription)];
    }
    
    function getProposals(bytes[] memory proposalBytesArr)
        public pure
        returns (bytes[4][] memory props)
    {
        props = new bytes[4][](proposalBytesArr.length);
