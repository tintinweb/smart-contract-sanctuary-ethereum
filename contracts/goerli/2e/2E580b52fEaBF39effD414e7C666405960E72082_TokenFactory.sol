...
interface IImplementationHub {
    ...
    function getImplementationGivenTemplate(bytes32 _templateKey, uint256 _implIndex) external view returns (address);
    function getImplementation(bytes32 _key, bytes32 _version) external view returns (address);
    function getImplementationsForTemplate(bytes32 _templateKey) external view returns (address[] memory);
    ...
    function createTemplate(bytes32 _templateKey, bytes32[] calldata _keys, bytes32[] calldata _versions) external;
    function storeImplementations(bytes32[] calldata _keys, address[] calldata _impls) external;
    function storeImplementation(bytes32 _key, address _impl) external;
    function getImplByKey(bytes32 _key, bytes32 _version) external view returns(address);
    function getTemplateCache(address _operatingContract) external view returns(bytes32[] memory);
...
contract FactoryStorage {
    ...
    bytes32 internal _templateKey;
    address internal _implementationHub;
...
import 'contracts/interface/IImplementationHub.sol';
...
contract TokenFactory is AccessControlUpgradeable, OwnableUpgradeable, FactoryStorage{
    ...
    function init(address _implHub) public initializer {
        _setupRole(Operator_Role, msg.sender);
        __Ownable_init_unchained();
        setImplementationHub(_implHub);
    }
    ...
    function setImplementationHub(address _implHub) public onlyRole(Operator_Role) {
        require(_implHub != address(0));
        _implementationHub = _implHub;
    }

            // --------------------------Contract creating functions-------------------------------------

    function _deployAndBindImpl(Implementations _impl) internal returns(address) {
        require(_implementationHub != address(0), "ImplementationHub not set");
        uint256 typeIndex = uint256(_impl);
        ProxyV1 deployed = new ProxyV1(); 
        deployed.upgradeTo(IImplementationHub(_implementationHub).getImplementationGivenCacheIndex(0, typeIndex)); 
        return address(deployed);
    }

            // --------------------------Creating new Token-------------------------------------

    function createToken(string memory _mappingValue, bytes calldata _initData) external onlyRole(Operator_Role){
    ...
    function _initProxyAndCompliance(address _identityRegistry, string memory _mappingValue, bytes calldata _initData) internal{
        ...
        emit TokenCreated(proxy, IImplementationHub(_implementationHub).getImplementationGivenCacheIndex(0, uint256(Implementations.TOKEN)) , _identityRegistry, _mappingValue, block.timestamp);
    }
    ...
    function addSubAdminOnCompliance(address _userAddress, address _compliance) external onlyRole(Operator_Role){
        ...
        compliance.addSubAdminOnCompliance(_compliance);
    }
    ...
    function adminCallsUnrestricted(address[] calldata _to, uint256[] calldata _value, bytes[] calldata _data) external onlyRole(Operator_Role) returns(bool success) {
