    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize() external initializer {
        ...
        tokenFee = 35 * 10**14;
        totalSupply = 5555;
        _salesStartTime = 1674651770;  // 6.33
        _blackListStartTime = 1674652850;  //6.52
        _salesEndTime = 1674653450; //7.5
    }
     ...
     */
    function tokenMint(uint256 count, bytes32[] calldata _tree) public payable userCount(count) {
        ...
        if(
            MerkleProofUpgradeable.verify(_tree, root, leaf)){
                require(
                    _salesStartTime <= block.timestamp &&
            ...
            if (countOfUser[msg.sender] == 1) {
                ...
                require(msg.value == tokenFee, "Amount should be 0.0035");
                safeMint(msg.sender);
                emit NFTInfo(_totalMint - 1);
            }
        }
    ...
    function updateTime(
        ...
        _salesEndTime = salesEndTime;
        _blackListStartTime = blackListStartTime;
    }
    ...
    * @param _tree
    ...
    function whiteListAccount(bytes32[] calldata _tree) public view returns(bool) {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProofUpgradeable.verify(_tree, root, leaf), "Incorrect proof");
