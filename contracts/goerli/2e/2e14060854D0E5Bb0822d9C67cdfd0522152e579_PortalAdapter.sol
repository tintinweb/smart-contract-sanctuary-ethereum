...
contract PortalAdapter is ILiquidityLayerAdapter, Router {
    ...
    /// @notice transferId => whether processed or not
    mapping(bytes32 => bool) public portalTransfersProcessed;
    ...
    function sendTokens(
        ...
        bytes memory payload = adapterData(localDomain, nonce, address(_token));
    ...
    function receiveTokens(
        ...
        // // The origin Wormhole domain
        uint16 _originWormholeDomain = hyperlaneDomainToWormholeDomain[
            _originDomain
        ];

        // Get the origin information from the adapterData
        (
            uint32 _originDomainInPayload,
            uint224 _nonce,
            bytes32 originTokenAddress
        ) = abi.decode(_adapterData, (uint32, uint224, bytes32));

        require(_originDomain == _originDomainInPayload, "!originDomain");

        // Determine the local token address from which we should have received the funds from Portal
        // TODO: Make this more robust
        IERC20 token = IERC20(
            portalTokenBridge.wrappedAsset(
                _originWormholeDomain,
                originTokenAddress
            )
        );

        require(
            portalTransfersProcessed[transferId(_originDomain, _nonce)],
    ...
    function completeTransfer(bytes memory encodedVm) public {
        ...
        IPortalTokenBridge.TransferWithPayload memory parsedPayload = portalTokenBridge
            .parseTransferWithPayload(transferTokenBridgePayload);

        (uint32 _originDomain, uint224 _nonce) = abi.decode(
            parsedPayload.payload,
            (uint32, uint224)
        );
        portalTransfersProcessed[transferId(_originDomain, _nonce)] = true;
    }
...
contract MockPortalBridge is IPortalTokenBridge {
        ...
        returns (bytes memory)
    {
        ...
        return adapterData(_originDomain, _nonce, address(token));
    }
    ...
    function parseTransferWithPayload(bytes memory encoded)
        ...
        returns (TransferWithPayload memory transfer)
    {
        // Just return the encoded bytes as the transfer payload
        transfer.payload = encoded;
    }
