    ...
    IERC721ReceiverUpgradeable
{
    ...
    function createMintAuction(
        ...
        require(
            !salesHandler.listingExists(collection, itemID),
            "sale listing exists"
        );
        createAuction(collection, itemID, duration, reservePrice, true);
    }
    ...
    function createSecondaryAuction(
        ...
        createAuction(collection, itemID, duration, reservePrice, false);
    }
    ...
    function createAuction(
        ...
        uint256 auctionID = getAuctionID(collection, itemID);
        ...
        emit AuctionCreated(collection, itemID, seller, duration, reservePrice);
    ...
    function placeBid(
        ...
        uint256 lastBid = auction.highestBid;
        address lastBidder = auction.highestBidder;
        ...
        if (lastBid > 0) {
            uint256 locked = lockedFunds[lastBidder];

            require(locked >= lastBid, "insufficient locked funds");
            if (locked == lastBid) {
                delete lockedFunds[lastBidder];
            } else {
                lockedFunds[lastBidder] -= lastBid;
            }
        }
        ...
        if (block.timestamp > extendTimestamp) {
            ...
            emit AuctionExtended(collection, itemID, newEndTimestamp);
        }

        emit AuctionBid(collection, itemID, bidder, newBid);

        require(
            bidToken.transferFrom(bidder, address(this), newBid),
            "transferFrom failed"
        );

        if (lastBid > 0) {
            require(bidToken.transfer(lastBidder, lastBid), "transfer failed");
        }
    }
    ...
    function endAuction(ICollection collection, uint256 itemID) external {
        ...
        require(
            bidToken.transfer(auction.seller, sellerTransfer),
            "seller transfer failed"
        );

        if (fee > 0) {
            require(bidToken.transfer(owner(), fee), "fee transfer failed");
        }

        if (auction.mint) {
            auction.collection.mintItem(auction.itemID, auction.highestBidder);
        } else {
    ...
    function cancelAuction(ICollection collection, uint256 itemID) external {
        ...
        delete auctions[auctionID];
        emit AuctionCancelled(collection, itemID, auction.seller);
    ...
    function getAuctionID(ICollection collection, uint256 itemID)
        internal
        pure
        returns (uint256)
    {
