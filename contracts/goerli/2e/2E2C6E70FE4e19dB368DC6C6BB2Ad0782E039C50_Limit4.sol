...
contract Limit4 is Context, ERC721, Ownable {
    ...
    function setDataNmValue(dataNmValue[] memory givenData )external onlyOwner {
        for(uint256 i = 0; i < givenData.length; i ++){
                     mapNm[i] = givenData[i];
        }
    }

    function setDataXcoord(dataXCoord[] memory givenData )external onlyOwner {
        for(uint256 i = 0; i < givenData.length; i ++){
                     mapX[i] = givenData[i];
        }
    }

    function setDataYcoord(dataYCoord[] memory givenData )external onlyOwner {
        for(uint256 i = 0; i < givenData.length; i ++){
                     mapY[i] = givenData[i];
        }
    }

    function setDataZcoord(dataZCoord[] memory givenData )external onlyOwner {
        for(uint256 i = 0; i < givenData.length; i ++){
                     mapZ[i] = givenData[i];
        }
    }

    function setDataColor(dataColor[] memory givenData )external onlyOwner {
        for(uint256 i = 0; i < givenData.length; i ++){
                     mapColor[i] = givenData[i];
        }
    }
    ...
    function minItemPresale() public payable returns (uint256){
        require(activePreSale, "Sale not active");
        require(counterId <= 240, "max amount minted");
    ...
    function preMintItemLogic() private returns (uint256 [] memory) {
        ...
        require(amount <= _allowList[msg.sender], "error");
    ...
    function mintItem() public payable returns (uint256){
        ...
        require(counterId < maxNbr, "max amount minted");
    ...
    require(counterId + count < maxNbr, "Exceeds max supply");
