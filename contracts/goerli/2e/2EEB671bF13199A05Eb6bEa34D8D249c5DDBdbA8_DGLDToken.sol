    ...
    AccessControlUpgradeable
{
    ...
    /// @dev only owner can call this function. Emits CustodyFeeUpdated event
    ...
    /// @dev only owner can call this function. Emits WhiteListed event
    ...
    /// @dev only owner can call this function. Emits RemovedFromWhiteList event
    /// @param user user address
    ...
    /// @dev only multi signature address can call this function. Emits FeeAddressUpdated event
    ...
    /// @dev only multi signature address can call this function. Emits TransferFeeUpdated event
    /// @param value the percentage value (max 10%)
    ...
    /// @dev only multi signature address can call this function. Emits Mint and Transfer event
    ...
    function _canBurn(
        ...
        return balanceOf(from) >= barWeight + custodyFeesAmount + redemptionFee;
    }
    ...
    function burn(
        ...
        if (redemptionFee > 0) _transfer(from, feeAddress, redemptionFee);
    }

    /// @notice Set the T&C url
    /// @dev only owner can call this function. Emits TCUrlUpdated event
    /// @param url the T&C url
    function setTcURL(string memory url) public onlyOwner {
