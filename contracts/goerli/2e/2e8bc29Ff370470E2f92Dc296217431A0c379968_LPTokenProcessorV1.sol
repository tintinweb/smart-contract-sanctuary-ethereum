...
contract LPTokenProcessorV1 is ILPTokenProcessor, KeeperCompatible, AccessControlEnumerable {
    ...
    mapping(address => bool) public _V2Routers;
    ...
    mapping(address => ReferrerShare[]) private _referrerShareByToken;

    uint256 public constant burnBasisPoints = 2500; // 25%
    uint256 public slippageBasisPoints = 50; // 0.5%
    mapping(address => uint256) public perTokenSlippage;
    bool public requirePrice;
    ...
    constructor(
        ...
        if (routerAddressForFloki != address(0)) {
            routerByFactory[IUniswapV2Router01(routerAddressForFloki).factory()] = routerAddressForFloki;
            _V2Routers[routerAddressForFloki] = true;
        }
        FLOKI = flokiAddress;
        USDT = usdtAddress;
    ...
    function setRequirePrice(bool requires) external onlyRole(DEFAULT_ADMIN_ROLE) {
        requirePrice = requires;
    }
    ...
    function addTokenForSwapping(
    ...
    ) external override onlyRole(LP_ADMIN_ROLE) {
        ...
        if (_tokens.add(tokenAddress)) {
            _isV2LPToken[tokenAddress] = isV2;
            _tokenFactories[tokenAddress] = routerFactory;
            emit TokenAdded(tokenAddress);
        }
        if (referrer != address(0)) {
            _referrerShareByToken[tokenAddress].push(ReferrerShare(referrer, referrerShare));
        }
    }

    function addRouter(address routerAddress, bool isV2) external onlyRole(LP_ADMIN_ROLE) {
    ...
    function isV2LiquidityPoolToken(address tokenAddress) external view override returns (bool) {
    ...
    function isV3LiquidityPoolToken(address tokenAddress, uint256 tokenId) external view override returns (bool) {
        (address token0, address token1, ) = _getV3Position(tokenAddress, tokenId);
    ...
    {
        return _getV3Position(tokenAddress, tokenId);
    }

    function _getV3Position(address tokenAddress, uint256 tokenId)
        private
        view
        returns (
            address,
    ...
    function checkUpkeep(
        ...
        for (uint256 i = 0; i < loopLimit; i++) {
            ...
            if ((_lastAdded[token] + _sellDelay) < block.timestamp) {
                address routerFactory = _tokenFactories[token];
    ...
    function performUpkeep(
        ...
        for (uint256 i = 0; i < tokensLength; i++) {
            ...
            if ((_lastAdded[token] + _sellDelay) < block.timestamp) {
                ...
                if (_isV2LPToken[token]) {
                    // V2 LP Tokens
                    _swapV2tokenByUSDT(token, tokenBalance, routerAddress);
                } else {
                    // Regular ERC20 tokens
                    console.log("Swapping ERC20 token %s", token);
                    bool success = _swapTokensV2(token, tokenBalance, USDT, address(this), routerAddress);
                ...
                _burnFloki(usdtAmount);
                _processFees(token, tokenBalance, usdtAmount);
                _tokens.remove(token);
                emit TokenCleared(token);
                break; // only process one token per tx
            }
        }
    }

    /**
     * @dev Unpairs the liquidity pool token and swap the unpaired tokens by USDT.
     */
    function _swapV2tokenByUSDT(
        ...
        bool success = _swapTokensV2(token0, token0Amount, USDT, address(this), routerAddress);
        require(success, "tokenProcessorV1::_swapV2tokenByUSDT: Failed to swap token0 to USDT.");
        success = _swapTokensV2(token1, IERC20(token1).balanceOf(address(this)), USDT, address(this), routerAddress);
        require(success, "tokenProcessorV1::_swapV2tokenByUSDT: Failed to swap token1 to USDT.");

        return lpBalance;
    }

    function _burnFloki(uint256 usdtAmount) private {
        // Burn FLOKI
        if (FLOKI != address(0)) {
    ...
    function _processFees(
        ...
        // Pay referrers
        uint256 treasuryShare = usdtBalance;
        ReferrerShare[] storage referrers = _referrerShareByToken[token];
        for (uint256 i = referrers.length; i > 0; i--) {
            // Calculate the referrer share in USDT proportionally to the total token balance
            uint256 referrerShare = (usdtBalance * referrers[i - 1].share) / tokenBalance;
            treasuryShare -= referrerShare;
            IERC20(USDT).safeTransfer(referrers[i - 1].referrer, referrerShare);
            referrers.pop();
        }
    ...
    function _swapTokensV2(
        ...
        uint256 amount = _getPriceWithSlippage(sourceToken, destinationToken);
        try router.swapExactTokensForTokens(sourceAmount, amount, path, receiver, block.timestamp) returns (
    ...
    function _getPriceWithSlippage(address sourceToken, address destinationToken) private view returns (uint256) {
        uint256 price = priceOracle.getPriceInUSD(sourceToken, destinationToken);
        require(price > 0 || !requirePrice, "tokenProcessorV1::_getPriceWithSlippage: Price is zero.");
        uint256 slippage = perTokenSlippage[sourceToken];
        if (slippage == 0) {
