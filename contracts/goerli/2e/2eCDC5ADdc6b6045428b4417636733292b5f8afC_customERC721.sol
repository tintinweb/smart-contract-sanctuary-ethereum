...
contract customERC721 is ERC721, Ownable, ERC2981, ReentrancyGuard, Pausable, Blacklistable {
    ...
    function _beforeTokenTransfers(address from, address to, uint256 firstTokenId, uint256 quantity) internal whenNotPaused override {
        require(!isBlackListed(to), "Token transfer refused. Receiver is on blacklist");
        super._beforeTokenTransfers(from, to, firstTokenId, quantity);
    }
}
