...
// File: contracts/NeoNomads.sol
    ...
    contract NeoNomads is ERC721, Ownable, ERC721Enumerable, ERC2981 {
      using SafeMath for uint256;
      using Strings for uint256;
    
      uint256 public constant tokenPrice = 4700000000000000 ; // 0.0047 ETH 
      ...
      event DemoMinted(uint256 tokenId, address owner);
    
      constructor() ERC721("Neo-Nomads", "NND") {
      ...
      function mintNeo(uint256 numberOfTokens) external payable {
    uint256 MaxPerWallet = balanceOf(msg.sender).add(numberOfTokens);
    require(
    ...
    require(MaxPerWallet <= 3, "Purchase exceeds max allowed");
    
    for (uint256 i = 0; i < numberOfTokens; i++) {
        uint256 id = totalSupply().add(1);
        if (totalSupply() < MAX_TOKENS) {
            _safeMint(msg.sender, id);
            emit DemoMinted(id, msg.sender);
        }
    }
}
