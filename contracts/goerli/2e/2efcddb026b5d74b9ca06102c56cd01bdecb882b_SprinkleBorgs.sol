...
contract SprinkleBorgs is ERC721A, Ownable {
    ...
    uint256 public constant maxSupply = 5050;
    ...
    uint256 public maxRange = 3;


    uint256 public defaultRange = 3;
    ...
    modifier mintChecker(uint256 _mintAmount) {
    ...
    function goGoMint(uint256 _mintAmount) public payable mintChecker(_mintAmount) {
        _safeMint(msg.sender, _mintAmount);
    }


    function testMinting(uint256 _mintAmount) public payable {
        require(!paused, "MINT IS PAUSED! BOP!");
        require(_mintAmount > 0);
        require(totalSupply() + _mintAmount <= maxSupply, "Mint Exceed Max Supply");
        require(_mintAmount <= maxMintAmountPerTx, "Mint Exceed Max Per Tx");
