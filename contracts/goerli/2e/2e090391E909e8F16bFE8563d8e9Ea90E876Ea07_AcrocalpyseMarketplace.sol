...
contract AcrocalpyseMarketplace is Ownable, Pausable {
    ...
    event ItemBought(address purchaser, uint256 itemId, uint256 timestamp);
    ...
    function buy(
        ...
        bytes32 hash = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(_msgSender(), itemId, paperTokensCost, ethCost))
        );
        ...
        emit ItemBought(_msgSender(), itemId, block.timestamp);
    }
    ...
    function bid(
        ...
        require(bidAmount >= currentHighestBid, "Bid less than highest bid");
        ...
        emit AuctionBid(_msgSender(), itemId, bidAmount, block.timestamp);
    }
    ...
    function reclaimBids(bytes memory signature, uint256[] memory itemIds) external whenNotPaused callerIsUser {
        ...
        emit ReclaimedBids(_msgSender(), itemIds, block.timestamp);
    }
