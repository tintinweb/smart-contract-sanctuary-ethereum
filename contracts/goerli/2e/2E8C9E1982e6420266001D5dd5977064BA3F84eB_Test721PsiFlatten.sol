...
contract Test721PsiFlatten is ERC721Psi {
    event PublicMint(
        address indexed minter,
        uint256 indexed tokenId,
        uint256 quantity
    );
    event SetTokenBaseURI(string previous, string current);
    event SetDefaultBaseURI(string previous, string current);
    event SetIsRevealed(bool previous, bool current);
    ...
    ) ERC721Psi(title, symbolValue) {
        _maxSupply = maxSupplyValue;
        _defaultURI = baseURIValue;
        _tokenBaseURI = tokenURIValue;
    }
    ...
    function setTokenBaseURI(string memory newTokenBasURI) external {
        string memory previous = _tokenBaseURI;
        _tokenBaseURI = newTokenBasURI;
        emit SetTokenBaseURI(previous, _tokenBaseURI);
    }

    function setDefaultBaseURI(string memory newDefaultTokenBaseURI) external {
        string memory previous = _defaultURI;
        _defaultURI = newDefaultTokenBaseURI;
        emit SetDefaultBaseURI(previous, _defaultURI);
    }
    ...
    function setIsRevealed(bool reveal) public {
        bool previous = _isRevealed;
        _isRevealed = reveal;
        emit SetIsRevealed(previous, _isRevealed);
    }
        ...
        returns (string memory)
    {
        ...
        return isRevealed() ? _tokenBaseURI : _defaultURI;
    }
