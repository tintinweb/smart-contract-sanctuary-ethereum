...
contract ClaimNFT is IClaimNFT, PublicClaimNFT, WhitelistClaimNFT, DiscountClaimNFT {
    ...
    function discountWhitelistClaim(
        ...
        if (!_isWhitelistClaimAllowed(whitelistProof, tokens)) revert WhitelistClaimingNotAllowed();
        tokenId = _discountClaim(edition, size, discountProof, claimValue);
        _whitelistClaimedTokens[msg.sender] += 1;
    }
}
...
// solhint-disable func-name-mixedcase
// solhint-disable ordering
contract BaseNFT is IBaseNFT, ERC721Upgradeable, BaseContract, BaseNFTStorage {
...
interface IStateNFT is IBaseNFT {}
...
abstract contract BaseNFTStorage {
...
abstract contract RedeemNFTStorage is StateNFTStorage {}
...
contract WhitelistClaimNFT is IWhitelistClaimNFT, BaseClaimNFT, WhitelistClaimNFTStorage {
    ...
    function isWhitelistClaimAllowed(bytes32[] calldata whitelistproof, uint256 tokens) external view returns (bool) {
        return _isWhitelistClaimAllowed(whitelistproof, tokens);
    }

    function _isWhitelistClaimAllowed(bytes32[] calldata whitelistproof, uint256 tokens) internal view returns (bool) {
        if (_whitelistClaimedTokens[msg.sender] == tokens) return false;

        bytes32 leaf = _getWhitelistMerkleLeaf(tokens);

        return whitelistproof.verify(_whitelistMerkleRoot, leaf);
    }

    function _getWhitelistMerkleLeaf(uint256 tokens) internal view returns (bytes32) {
        return keccak256(abi.encodePacked(block.chainid, address(this), msg.sender, tokens));
    }
}
...
abstract contract WhitelistClaimNFTStorage is BaseClaimNFTStorage {
    ...
    mapping(address => uint256) internal _whitelistClaimedTokens;
}
