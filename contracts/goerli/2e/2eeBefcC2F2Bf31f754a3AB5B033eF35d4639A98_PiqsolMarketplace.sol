...
// File: contracts/marketplace_piqsol.sol
...
contract PiqsolMarketplace is  Initializable , ERC721HolderUpgradeable ,OwnableUpgradeable,UUPSUpgradeable {
    //** Tax Listing **//
    uint256 listingTax;
    uint256 buyitemTax;
    uint256 biditemTax;
   ...
   function initialize(
    ...
    struct AuctionListItem {
        ...
        uint256 percentageBasisPoints;
          address minterRoyaltyRecipientAddress;
        uint256 minterPercentageBasisPoints;
    ...
    struct FixedPriceListItem {
        ...
        address royaltiesRecipientAddress;
        uint256 percentageBasisPoints;
          address minterRoyaltiesRecipientAddress;
        uint256 minterPercentageBasisPoints;
    ...
    function listFixedItemToMarket(
        ...
        uint256 _percentageBasisPoints,
          address _minterRoyaltiesRecipientAddress,
        uint256 _minterPercentageBasisPoints
    )
        external
        HasTransferApproval(tokenAddress, tokenId)
        returns (uint256)
    {
       require(IERC721Upgradeable(tokenAddress).ownerOf(tokenId) == msg.sender,"only owner can call this method");
        require(
        ...
        FixedItemsForSale.push(
            FixedPriceListItem(
                ...
                _percentageBasisPoints,
                 _minterRoyaltiesRecipientAddress,
         _minterPercentageBasisPoints,
                payable(msg.sender),
                askingPrice,
                true,
                false
    ...
    function canclelistOfFixedItem(uint256 id)
        external
        FixedItemExists(id)
    {
        require(
        ...
        FixedPriceActiveItems[FixedItemsForSale[id].tokenAddress][
         ...
         IERC721Upgradeable(FixedItemsForSale[id].tokenAddress).safeTransferFrom(address(this), msg.sender, FixedItemsForSale[id].tokenId);
        ...
        FixedIsForSale(id)
    {
        ...
        uint256 finalAmount;
    uint256  minterRoyaltyfee;
        uint256 creatorRoyaltyfee;
        address _royaltiesRecipientAddress = FixedItemsForSale[id]
            .royaltiesRecipientAddress;
        address _minterRoyaltiesRecipientAddress = FixedItemsForSale[id].minterRoyaltiesRecipientAddress;
        FixedItemsForSale[id].isSold = true;
        FixedPriceActiveItems[FixedItemsForSale[id].tokenAddress][
            FixedItemsForSale[id].tokenId] = false;
            if(_royaltiesRecipientAddress != address(0)){
                 creatorRoyaltyfee = calculateRoyaltyFee(
                msg.value,FixedItemsForSale[id].percentageBasisPoints
            );
            payable(FixedItemsForSale[id].royaltiesRecipientAddress).transfer(creatorRoyaltyfee);
            }
           if(_minterRoyaltiesRecipientAddress != address(0)){
                      minterRoyaltyfee = calculateRoyaltyFee(
                msg.value,FixedItemsForSale[id].minterPercentageBasisPoints
            );
              payable(FixedItemsForSale[id].minterRoyaltiesRecipientAddress).transfer(minterRoyaltyfee);
            }
             finalAmount= msg.value.sub(minterRoyaltyfee+creatorRoyaltyfee);
            if(minterRoyaltyfee !=0 || creatorRoyaltyfee !=0){
    ...
    function listAuctionItemToMarket(
        ...
        address _royaltiesRecipientAddress,
        uint256 _percentageBasisPoints,
         address _minterRoyaltyRecipientAddress,
        uint256 _minterPercentageBasisPoints
    ) external returns (uint256) {
        require(
            IERC721Upgradeable(tokenAddress).getApproved(tokenId) == address(this),
            "Piqsol:approvel"
        );
        require(
        ...
        require(
            IERC721Upgradeable(tokenAddress).ownerOf(tokenId) == msg.sender,
        ...
        AuctionItemsForSale.push(
            AuctionListItem(
                ...
                _royaltiesRecipientAddress,
                _percentageBasisPoints,
                _minterRoyaltyRecipientAddress,
                 _minterPercentageBasisPoints,
                payable(msg.sender),
                _minPrice,
                true,
     ...
     function canclelistOfAuctionItem(uint256 id)
        external
        AuctionItemExists(id)
      
    {
     ...
     IERC721Upgradeable(AuctionItemsForSale[id].tokenAddress).safeTransferFrom(address(this), msg.sender, AuctionItemsForSale[id].tokenId);
    }
    ...
    function auctionEnd(uint256 id) public {
        ...
        address _royaltiesRecipientAddress = AuctionItemsForSale[id]
            .royaltiesRecipientAddress;
         address _minterRoyaltyRecipientAddress = AuctionItemsForSale[id]
            .minterRoyaltyRecipientAddress;
        AuctionItemsForSale[id].isSold = true;
        AuctionActiveItems[AuctionItemsForSale[id].tokenAddress][AuctionItemsForSale[id].tokenId] = false;
         if(_royaltiesRecipientAddress != address(0)){
                 creatorRoyaltyfee = calculateRoyaltyFee(
                 AuctionItemsForSale[id].highestBid,
                   AuctionItemsForSale[id].percentageBasisPoints
            );
            payable(_royaltiesRecipientAddress).transfer(creatorRoyaltyfee);
            }
            if(_minterRoyaltyRecipientAddress != address(0)){
                      minterRoyaltyfee = calculateRoyaltyFee(
               AuctionItemsForSale[id].highestBid,
                   AuctionItemsForSale[id].minterPercentageBasisPoints
            );
              payable(_minterRoyaltyRecipientAddress).transfer(minterRoyaltyfee);
            }
            finalAmount= AuctionItemsForSale[id].highestBid.sub(minterRoyaltyfee+creatorRoyaltyfee);
            if(creatorRoyaltyfee !=0 || minterRoyaltyfee !=0){
            AuctionItemsForSale[id].beneficiary.transfer(finalAmount);
             ...
             IERC721Upgradeable(AuctionItemsForSale[id].tokenAddress).safeTransferFrom(address(this),AuctionItemsForSale[id].highestBidder, AuctionItemsForSale[id].tokenId);
            emit AuctionEnded(
    ...
    function calculateRoyaltyFee(
        ...
        uint256 royalityfee = _salePrice.mul(_percentageBasisPoints).div(10000);
        return royalityfee;
    }
