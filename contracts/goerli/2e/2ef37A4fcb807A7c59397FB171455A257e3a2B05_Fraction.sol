...
contract Fraction is Ownable, ChainlinkClient, ActivePools {
  ...
  bytes32 internal chainlinkJobId;
  string internal buyUrl;
  string internal finalizeUrl;
  ...
  function withdraw(uint256 poolId) external {
    ...
    // I presume that piece cost is always > 0
    require(pool.pieceCost > 0,'Pool does not exist');
    require(pool.unavailable != true, 'Pool is unavailable');
    require(usersPieces[msg.sender][poolId] > 0, 'You do not have any pieces');
  ...
  function buyAsset(uint256 poolId) internal {
    ...
    req.add('get', string(abi.encodePacked(buyUrl, Strings.toHexString(pool.assetAddress), '/', Strings.toString(pool.assetId), '/', Strings.toHexString(pool.assetOwner))));
    req.add('path', 'result');
