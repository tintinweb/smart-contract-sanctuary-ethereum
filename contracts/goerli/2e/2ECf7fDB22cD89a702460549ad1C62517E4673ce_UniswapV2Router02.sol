...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    event Sync(address indexed pair, string id, string name);
     ...
     */
    function swapAndMintStarkWare(address tokenA, address tokenB, uint[] memory path,
     uint[] memory quantisedAmounts, uint quantisedAmount, uint tokenAAmount,
      uint tokenBAmount, uint nonce, address exchangeAddress, string calldata id) external returns(bool) {
      address pairAddress = UniswapV2Library.pairFor(factory, tokenA, tokenB);
      IUniswapV2Pair pair = IUniswapV2Pair(pairAddress);
      emit Sync(pairAddress, id, 'swapAndMintStarkWare');
      return pair.swapAndMintStarkWare(path[0], path[1], quantisedAmounts[0], quantisedAmounts[1], quantisedAmount, tokenAAmount, tokenBAmount, nonce, exchangeAddress);
    }

    function swapAndBurnStarkWare(address tokenA, address tokenB, uint[] memory path,
     uint[] memory quantisedAmounts, uint quantisedAmount, uint minTokenAAmount, uint minTokenBAmount,
     uint nonce, address exchangeAddress, string calldata id) external returns(bool) {
      address pairAddress = UniswapV2Library.pairFor(factory, tokenA, tokenB);
      IUniswapV2Pair pair = IUniswapV2Pair(pairAddress);
      emit Sync(pairAddress, id, 'swapAndBurnStarkWare');
      return pair.swapAndBurnStarkWare(path[0], path[1], quantisedAmounts[0], quantisedAmounts[1], quantisedAmount, minTokenAAmount, minTokenBAmount, nonce, exchangeAddress);
    }

    function swapStarkWare(address tokenA, address tokenB, uint[] memory path,
     uint[] memory quantisedAmounts, uint nonce, address exchangeAddress, string calldata id) external returns(bool) {
      address pairAddress = UniswapV2Library.pairFor(factory, tokenA, tokenB);
      IUniswapV2Pair pair = IUniswapV2Pair(pairAddress);
      emit Sync(pairAddress, id, 'swapStarkWare');
      return pair.swapStarkWare(path[0], path[1], quantisedAmounts[0], quantisedAmounts[1], nonce, exchangeAddress);
    }

    function settleStarkWare(address tokenA, address tokenB, string memory id) external returns(bool) {
      address pairAddress = UniswapV2Library.pairFor(factory, tokenA, tokenB);
      IUniswapV2Pair pair = IUniswapV2Pair(pairAddress);
      emit Sync(pairAddress, id, 'settleStarkWare');
      return pair.settleStarkWare();
    }

    function withdrawAndClearStarkWare(address tokenA, address tokenB, uint assetId) external {
      IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(factory, tokenA, tokenB));
      return pair.withdrawAndClearStarkWare(assetId);
    }
     ...
     */
    function createPair(address tokenA, address tokenB, uint pairAssetId, uint tokenAAssetId,
     uint tokenBAssetId, address expectedPairAddress) public returns (address pair) {
    ...
    /**
     * @dev Create a pair, deposit funds and switch to Layer 2
     */
    function createAndDeposit(address tokenA, address tokenB, uint pairAssetId, uint tokenAAssetId,
     uint tokenBAssetId, address expectedPairAddress, uint amountA, uint amountB, address to) public returns (address pair) {
       pair = createPair(tokenA, tokenB, pairAssetId, tokenAAssetId, tokenBAssetId, expectedPairAddress);
     ...
     * @dev Create a pair, deposit funds and switch to Layer 2
     */
    function createAndDepositToL2(address tokenA, address tokenB, uint pairAssetId, uint tokenAAssetId,
     uint tokenBAssetId, address expectedPairAddress, uint amountA, uint amountB, address to) external returns (address pair) {
       pair = createPair(tokenA, tokenB, pairAssetId, tokenAAssetId, tokenBAssetId, expectedPairAddress);
       addLiquidity(tokenA, tokenB, amountA, amountB, 1, 1, to, type(uint).max);
...
interface IUniswapV2Pair {
    ...
    function withdrawAndClearStarkWare(uint assetId) external;

    function clearLoans() external;

    function toggleLayer2(bool _isLayer2Live) external;
    function setupStarkware(uint assetId, uint tokenAAssetId, uint tokenBAssetId) external;
    function burnStarkWare(uint liquidity, address exchangeAddress) external returns(bool);
    function swapAndMintStarkWare(uint pathFrom, uint PathTo, uint SwapPathFrom, uint swapPathTo, uint quantisedAmount, uint tokenAAmount, uint tokenBAmount, uint nonce, address exchangeAddress) external returns(bool);
    function swapAndBurnStarkWare(uint pathFrom, uint PathTo, uint SwapPathFrom, uint swapPathTo, uint quantisedAmount, uint tokenAAmount, uint tokenBAmount, uint nonce, address exchangeAddress) external returns(bool);
    function swapStarkWare( uint swapPathFrom, uint swapPathTo, uint swapAmountFrom, uint swapAmountTo, uint nonce, address exchangeAddress) external returns(bool);
}
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                keccak256(abi.encodePacked(token0, token1)),
								hex'8598b005e9cf7d23ced31731db474e3ca69ba184c43588fd6e16c2e3177c0d4b'//PAIR_HASH
            )))));
    }
    ...
    function truncate(uint quantum, uint amount) private pure returns (uint) {
      require(amount > quantum, 'UniswapV2Library: AMOUNT_LOWER_THAN_QUANTUM');
