...
library CrossTalkUtils {
    ...
    function getTxStatusForAtomicCall(bool[] calldata execFlags)
        internal
        pure
        returns (bool)
    {
    ...
    function getTheIndexOfCallFailure(bool[] calldata execFlags)
        internal
        pure
        returns (uint8)
    {
    ...
    function getCrossTalkFees(address gatewayContract)
        internal
        view
        returns (uint256)
    {
 ...
 * @title Lottery
 * @author  Router Protocol
 * @notice this contract will be use to draw lottery for riders on which this is deployed and this will store all ticket data which is submitted to this contract from Ticket contract
 */
...
{
    /**
     * this will be emited when ticket is create on this chain which was sent from some supported chain
     * @param sender address of user who submmited ticket
     * @param riderId token id for rider nft
     * @param lowerBound this is lower bound for the lucky number range
     * @param upperBound this is upper bound for the lucky number range
     * @param srcChainId chainid of chain from where ticket is submitted
     * @param srcChainType chaintype of chain from where ticket is submitted
     */
    event TicketCreated(
        uint64 lotteryId,
        address sender,
    ...
    /**
     * this will be emitted when lottry is drawn and lucky number is declared
     * @param lotteryId lottery id for which lottery is drawn
     * @param luckyNumber lucky number for this lottery
     */
    event LuckyNumberDrawn(uint64 lotteryId, uint256 luckyNumber);
    ...
    // TODO: add lottery amount [can be changed to multiple token]
    event LotteryClaimed(uint64 lotteryId, address winner, uint256 riderId);

    /**
     * @notice Ticket metadata to store related information
     */
    ...
    /**
     * @notice struct to store winner data when lottery is drawn
     */
    struct WinnerData {
        bool isClaimed;
        address winner;
    ...
    /**
     * @notice mapping from riderId => lotteryId => TicketMetadata
     */
    mapping(uint256 => mapping(uint256 => TicketMetadata))
        public ticketMetadata;
    /**
     * @notice mapping lotteryId -> WinnerData
     */
    mapping(uint64 => WinnerData) public winnerData;

    /**
     * @notice this will indicate if lottery is started or not
     */
    bool public isStarted;
    /**
     * @notice this is the time when lottery started in sec
     */
    uint256 public lotteryStartTime;
    /**
     * @notice this is the time for lottery will last in sec
     */
    uint256 public lotteryDuration;
    /**
     * @notice this is use to decide lowerBound and upperBound for a ticket
     */
    uint256 ticketRanger;
    /**
     * @notice this is the current lotteryId starting from one
     */
    uint64 public lotteryId;
    /**
     * @notice gateway contract address for this chain
     */
    address public gatewayContract;
    /**
     * @notice chain id of current chain on which this lottery is deployed
     */
    string public chainId;
    /**
     * @notice chain type of current chain on which this lottery is deployed
     */
    uint64 public chainType;
    mapping(uint64 => mapping(string => bytes)) public ourTicketContract;

    /**
     * initializer for this contract for this chain
     * @param gatewayAddress gateway contract
    ...
    /**
     * starts the new lottery if no lottery is going on
     * @param _lotteryDuration lottery duration fro which new lottery will lasts
     */
    function startLottery(uint256 _lotteryDuration) public onlyOwner {
        require(!isStarted, "lottery already started!!");
        isStarted = true;
        lotteryStartTime = block.timestamp;
        lotteryId += 1;
        lotteryDuration = _lotteryDuration;
        emit LotteryStarted(lotteryId, lotteryStartTime, lotteryDuration);
    }

    /**
     * this will enroll ticket contract of chain X, which will be trusted contract
     * @param _chainType chain type of chain X
     * @param _chainId  chain id of Chain X
     * @param _contractAddress contract address of ticket on Chain x
     */
    function enrollTicketContract(
        uint64 _chainType,
        string memory _chainId,
        address _contractAddress
    ) public onlyOwner {
        ourTicketContract[_chainType][_chainId] = toBytes(_contractAddress);
    }

    /**
     * this function will be used to draw lucky number of current lottery when lottery is over and can only be called by owner of this contract
     */
    function drawLuckyNumber() public onlyOwner {
        require(isStarted, "winner already drawn");
        require(
            lotteryStartTime + lotteryDuration < block.timestamp,
            "lottery still going on"
        );

        if (ticketRanger == 0) {
            WinnerData storage _winnerData = winnerData[lotteryId];
            _winnerData.luckyNumber = 0;

            isStarted = false;

            emit LuckyNumberDrawn(lotteryId, 0);
            return;
        }
        ...
        WinnerData storage _winnerData = winnerData[lotteryId];
        _winnerData.luckyNumber = luckyNumber;

        isStarted = false;

        emit LuckyNumberDrawn(lotteryId, luckyNumber);
    }

    /**
     *  this function can be called by user who have won the lottery with lottery id and have ticket with rider as _riderID
     * @param _lotterId lottery id
     * @param _riderId token id of rider nft
     */
    function claimLottery(uint64 _lotterId, uint256 _riderId) public {
        TicketMetadata memory _ticketMetadata = ticketMetadata[_riderId][
            lotteryId
        ];
        WinnerData storage _winnerData = winnerData[_lotterId];

        require(
            _ticketMetadata.participant == msg.sender,
            "oops! you are not winner"
        );
        require(
            _ticketMetadata.lowerBound <= _winnerData.luckyNumber &&
                _winnerData.luckyNumber <= _ticketMetadata.upperBound,
            "not winner"
        );
        require(!_winnerData.isClaimed, "already claimed");

        _winnerData.winner = msg.sender;
        _winnerData.isClaimed = true;

        //TODO: now process winning amount here

        emit LotteryClaimed(_lotterId, msg.sender, _riderId);
    }
    ...
    /**
     * this function will create ticket and will event saying ticket is created
     * @param riderId token id of rider nft
     * @param sender address of user
     * @param score score of rider nft
     * @return lowerBound for this ticket
     * @return upperBound for this ticket
     */
    function _processTicket(
        uint256 riderId,
        address sender,
        uint256 score,
        string memory _chainId,
        ...
        require(isStarted, "lottery not started yet");
        require(
            uint256(lotteryStartTime + lotteryDuration) >=
                uint256(block.timestamp),
            "lottery over"
        );
        require(score > 0, "score is zero");

        TicketMetadata storage _ticketMetadata = ticketMetadata[riderId][
            lotteryId
        ];
        require(
            _ticketMetadata.score == 0,
            "already participated for curr lottery"
        );

        _ticketMetadata.participant = sender;
        _ticketMetadata.riderId = riderId;
        _ticketMetadata.lowerBound = ticketRanger;
        _ticketMetadata.upperBound = ticketRanger + score;
        ticketRanger += (score + 1);
        ...
        emit TicketCreated(
            ...
            riderId,
            ticketRanger,
            _ticketMetadata.upperBound,
            score,
            _chainId,
            _chainType
        );

        return (ticketRanger, _ticketMetadata.upperBound);
    }

    /**
     *  this function will only be called from ticket contract on which this contract is deployed
     * @param riderId token id of rider nft
     * @param sender address of user who is submitting ticket
     * @param score score of rider nft
     */
    function processTicket(
        ...
        //TODO: https://goerli.etherscan.io/tx/0x888321f18400a1155823bafc9815eb44b06d464d309e986981dea840584afb7a
        require(
            keccak256(toBytes(msg.sender)) ==
                keccak256(ourTicketContract[chainType][chainId]),
            "invalid caller"
        );

        _processTicket(riderId, sender, score, chainId, chainType);
        return lotteryId;
    }

    /**
     *  this will process ticket that have been sent from ticket contract on src chain on which user is submitting ticket
     * @param srcContractAddress contract address of ticker contract on src chain
     * @param payload payload that have been sent from submitTicket in Ticket contract
    ...
    function handleRequestFromSource(
        ...
        require(
            keccak256(srcContractAddress) ==
                keccak256(ourTicketContract[srcChainType][srcChainId]),
            "Not called from enrolled contract"
        );

        (uint256 riderId, address sender, uint256 score) = abi.decode(
            payload,
            (uint256, address, uint256)
        );

        (uint256 lowerBound, uint256 upperBound) = _processTicket(
            riderId,
            sender,
            score,
            srcChainId,
    ...
    /**
     * util function to convert address to bytes
     * @param a address to be converted into bytes
     * @return b bytes of address a
     */
