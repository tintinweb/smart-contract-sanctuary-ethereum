...
contract REAKTOR is Context, IERC20, Ownable {
  ...
  function _transfer(
    ...
    if(contractTokenBalance >= _maxTxAmount) // CB >= 150
    {
      contractTokenBalance = _maxTxAmount;   // CB = 150
    }

    bool overMinTokenBalance = contractTokenBalance >= numTokensSellToAddToLiquidity;   //  150 >= 5000
    if (
      overMinTokenBalance &&
      !inSwapAndLiquify &&
      from != uniswapV2Pair &&
