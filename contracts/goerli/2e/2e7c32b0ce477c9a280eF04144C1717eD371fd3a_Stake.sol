...
contract Stake is IStake, Ownable {
  ...
  /// @notice Safe CAPY transfer function, just in case if rounding error causes pool to not have enough CAPYs.
  /// @param _to The address to transfer CAPY to
  /// @param _amount The amount to transfer to
  function safeCapyTransfer(address _to, uint256 _amount) external override onlyOwner {
    uint256 capyBal = capy.balanceOf(address(this));
    if (_amount > capyBal) {
      capy.transfer(_to, capyBal);
    } else {
      capy.transfer(_to, _amount);
    }
  }
}