 ...
 * -#%[email protected]#-  :@#@@+%[email protected]*@*:=%+..%%#=      *@  *@++##.    =%@%@%%#-  =#%[email protected]#-   :*+**+=: %%++%*
 *
 * @title: [EIP1822/1967] Minter Implementation
...
import "./modules/factory/IMinter.sol";
import "./modules/access/IRoles.sol";
import "./lib/Roles.sol";

contract ERC1155Minter is Initializable
                        , IRoles
                        , UUPSUpgradeable {
  ...
  event Paused();
  event Unpaused();
  event LimitsSet(uint256 low, uint256 high);
  event SetMinter(address indexed newAddress);
  ...
  , uint256 _end
    ...
    public {
      __FactoryMinter_init(_backend, _dev, _admin, _start, _end);
  ...
  , uint256 _end
    ...
    backend = _backend;
    emit SetMinter(backend);
    startId = _start;
    endId = _end;
    emit LimitsSet(startId, endId);
  ...
  , uint256 _amount
    ...
    notPaused() {
    if (startId > _id || _id > endId) {
      revert MaxSplaining({
        reason: "Max1155FM:1"
      });
    } 
    IMinter(backend).FactoryMint(msg.sender, _id, _amount, abi.encodePacked(address(this)));
  }
  ...
  , uint256[] memory _amounts
    ...
    for (uint256 x = 0; x < len;) {
      if (startId > _ids[x] || _ids[x] > endId) {
        revert MaxSplaining({
          reason: "Max1155FM:1"
        });
      }
    ...
    IMinter(backend).FactoryMintBatch(msg.sender, _ids, _amounts, abi.encodePacked(address(this)));
  }
  ...
  , uint256 _amount
    ...
    IMinter(backend).FactoryMint(_to, _id, _amount, abi.encodePacked(address(this)));
  }
  ...
  , uint256[] memory _amounts
    ...
    IMinter(backend).FactoryMintBatch(_to, _ids, _amounts, abi.encodePacked(address(this)));
  }
 ...
 * -#%[email protected]#-  :@#@@+%[email protected]*@*:=%+..%%#=      *@  *@++##.    =%@%@%%#-  =#%[email protected]#-   :*+**+=: %%++%*
 *
 * @title: [EIP1155] Factory implementation for minter of 1155, Interface
 * @author: Max Flow O2 -> @MaxFlowO2 on bird app/GitHub
 * @notice Max-1155/2023 Gen 2 Demo
 */
