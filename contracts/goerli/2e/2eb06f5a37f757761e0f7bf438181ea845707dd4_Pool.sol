...
contract Pool is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 shares;
        int256 accRewards; // used to avoid rewards aggregating while a withdraw is pending
        int256 debt; // debt could be negative, just like rewardsPerShares
    }
    ...
    function deposit(uint256 amount) external {
        ...
        totalShares += amount;
        u.shares += amount;
        u.debt += int256(amount) * accRewardsPerShares;
    ...
    function withdraw(uint256 amount, address recipient) external {
        UserInfo memory u = users[recipient];
        require(amount != 0, "pool/zero-amount");
        require(amount < u.shares, "pool/amount-too-high");
        if ((block.timestamp - u.withdrawRequest) > (door + hold)) {
            // there was a previous request but is now expired
            u.withdrawRequest = block.timestamp;
            // freeze rewards accumulation
            u.accRewards =
                (int256(u.shares) * accRewardsPerShares) /
                int256(PRECISION);
        } else if ((block.timestamp - u.withdrawRequest) > door) {
            int256 pnl = u.accRewards - u.debt;
            ...
            u.debt =
                u.accRewards -
                ((int256(amount) * accRewardsPerShares) / int256(PRECISION));
            u.shares = u.shares - amount;
            u.withdrawRequest = 0;
            totalShares -= amount;
    ...
    // TODO: add auth [only Engine can access it]
