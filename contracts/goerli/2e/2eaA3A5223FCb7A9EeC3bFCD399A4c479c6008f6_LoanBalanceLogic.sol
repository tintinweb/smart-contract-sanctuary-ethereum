...
library LoanBalanceLogic {
  ...
  function getUpdatedLoanBalances(Loan memory _loan, ICopraGlobal _copraGlobal) public view returns (Loan memory) {
    ...
    if (block.timestamp >= _loan.timeActivated.add(_loan.loanTenorSeconds).add(_loan.gracePeriodSeconds)) {
  ...
  function calculateLoanValue(Loan memory _loan, ICopraGlobal _copraGlobal) public view returns (uint256) {
    ...
    uint256 loanMaturityDateWithGracePeriod = _loan.loanTenorSeconds.add(_loan.timeActivated).add(
      _loan.gracePeriodSeconds
    );
    uint256 loanLateDays = block.timestamp < loanMaturityDateWithGracePeriod
      ? 0
      : (((block.timestamp - loanMaturityDateWithGracePeriod) / 1 days) + 1).toWAD();
