...
contract Boardies is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI) ERC721("Boardies", "Boardies") {
        setBaseURI(baseURI);
    }

    // function freeMint(bytes32[] calldata merkleProof) public {
    //     uint256 freeMintQuantity = 1;
    //     uint totalMinted = _tokenIds.current();
    //     bytes32 node = keccak256(abi.encodePacked(msg.sender, freeMintQuantity));

    //     require(!_paused, "Sale paused");
    //     require(wave == 1, "Free mints only allowed in Wave 1");
    //     require(!freeMintClaimed[msg.sender], "Free mint claimed");
    //     require(MerkleProof.verify(merkleProof, merkleRoot, node), 'Invalid proof');
    //     require(totalMinted + freeMintQuantity <= maxSupply, "Not enough remaining!");
    ...
    function mint(uint _quantity, bytes32[] calldata merkleProof) public payable {

        uint totalMinted = _tokenIds.current();

        // Waves 1-3 = presale | Wave 4 = public
        // if (wave < 4) {
        ...
        require(!_paused, "Sale paused");
        require(totalMinted + _quantity <= maxSupply, "Not enough remaining!");
