...
contract Proxcity is ERC721A, Ownable, ReentrancyGuard{
	...
	uint256 public P1_COUNT = 0;
	uint256 public P2_COUNT = 0;
	uint256 public P3_COUNT = 0;
	uint256 public P4_COUNT = 0;
	uint256 private P2_COST = .07 ether;
	uint256 private P4_COST = .1 ether;

	bool public P1_ACTIVE = false;
	bool public P1_LIMTED = true;
	bool public P2_ACTIVE = false;
	bool public P3_ACTIVE = false;
	bool public P4_ACTIVE = false;
	...
	function p1Mint(uint256 _quantity) external nonReentrant{
		require(P1_ACTIVE == true, "Phase not active");
		require(_quantity > 0, "Mint amount must be greater then 0");
		require(P1_COUNT + _quantity <= 1000, "Phase1 mint complete");
	...
	function p2Mint(uint256 _quantity) external payable nonReentrant{
		require(P2_ACTIVE == true, "Phase not active");
		require(_quantity > 0 && _quantity <= 10, "Mint amount must be greater then 0 and less than or equal to 10");
		require(P2_COUNT + _quantity <= 1000, "Phase2 mint complete");
		require(_quantity * P2_COST == msg.value, "Not enough eth");
		P2_COUNT += _quantity;
		_safeMint(msg.sender, _quantity);
	}

	function p3Mint(uint256 _quantity) external nonReentrant{
		require(P3_ACTIVE == true, "Phase not active");
		require(_quantity > 0 && _quantity <= 5, "Mint amount must be greater then 0 and less than or equal to 5");
		require(p3Mints[msg.sender] + _quantity <= 5, "Address has reached the max free mints");
		require(P3_COUNT + _quantity <= 1000, "Phase3 mint complete");
		P3_COUNT += _quantity;
		p3Mints[msg.sender] += _quantity;
		_safeMint(msg.sender, _quantity);
	}

	function p4Mint(uint256 _quantity) external payable nonReentrant{
		require(P4_ACTIVE == true, "Phase not active");
		require(_quantity > 0 && _quantity <= 10, "Mint amount must be greater then 0 and less than or equal to 10");
		require(P4_COUNT + _quantity <= 1000, "Phase4 mint complete");
		require(_quantity * P4_COST == msg.value, "Not enough eth");
		P4_COUNT += _quantity;
		_safeMint(msg.sender, _quantity);
	}

	function stake(uint256 _token_id) external{
		require(_token_id >= 0 && _token_id <= 4000, "Invalid Token");
	...
	function toggleP1Limited(bool _active) external{
		P1_LIMTED = _active;
	}
	...
	function togglePhase(uint256 _phase, bool _active) external onlyOwner{
		if(_phase == 1){
			P1_ACTIVE = _active;
		} else if (_phase == 2){
			P2_ACTIVE = _active;
