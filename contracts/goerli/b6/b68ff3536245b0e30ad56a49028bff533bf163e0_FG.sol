...
contract FG is ERC721A, Ownable, ERC721ABurnable, ReentrancyGuard, PaymentSplitter {
    ...
    ) ERC721A("FG Test", "FG") PaymentSplitter(_payees, _shares) {
        baseURI = _base;
    ...
    function mint(uint16 _quantity) public payable nonReentrant {
        require(getPrice() * _quantity <= msg.value, "Insufficient ETH");

        // Retrieve in memory the token data for maximums.
        uint16 _totalSupply = _mintData.supplyTotal;
        uint16 _maxPerWallet = _mintData.maxPerWallet;
        uint16 _maxPerTx = _mintData.maxPerTx;

        // Ensure the user has not exceeded the maximums.
        require(_quantity <= _maxPerTx, "Exceeds max per transaction");
        require(totalSupply() + _quantity <= _totalSupply, "Exceeds total supply");

        // Retrieve the current sale status.
        bool _saleActive = _mintData.saleActive;
        bool _preSaleActive = _mintData.preSaleActive;
        ...
        require(hasMinted[msg.sender] + _quantity <= _maxPerWallet, "Exceeds max per wallet");
    ...
    function reserve(address _address, uint16 _quantity) public nonReentrant onlyOwner {
        require(totalSupply() + _quantity <= _mintData.supplyTotal, "Exceeds total supply");
    ...
    function readData() public view returns (MintData memory) {
        return _mintData;
    }

    function getPrice() public view returns (uint72) {
        if (_mintData.preSaleActive) {
            return _mintData.preSalePrice;
        } else {
            return _mintData.salePrice;
        }
    }
    ...
    function updateData(uint16 _supplyTotal, uint16 _maxPerWallet, uint16 _maxPerTx, uint72 _preSalePrice, uint72 _salePrice) public onlyOwner {
        require(_supplyTotal >= 0, "Error: Supply less than 0");
        require(_maxPerWallet >= 0, "Error: Max per wallet less than 0");
        require(_maxPerTx >= 0, "Error: Max per transaction less than 0");
        require(_preSalePrice >= 0, "Error: Pre-sale price less than 0");
        require(_salePrice >= 0, "Error: Sale price less than 0");

        require(_supplyTotal >= totalSupply(), "Error: Supply less than total supply");

        _mintData.supplyTotal = _supplyTotal;
        _mintData.maxPerWallet = _maxPerWallet;
        _mintData.maxPerTx = _maxPerTx;
        _mintData.preSalePrice = _preSalePrice;
        _mintData.salePrice = _salePrice;
    }

    // @dev Change the mint state.
    function setMintState(MintState _mint, bool _presale, bool _sale) public onlyOwner {
        if (_presale) {
            require(!_sale, "Error: Only one sale can be active at a time");
        } else if (_sale) {
            require(!_presale, "Error: Only one sale can be active at a time");
        }

        _mintState = _mint;
        _mintData.preSaleActive = _presale;
        _mintData.saleActive = _sale;
    }
}