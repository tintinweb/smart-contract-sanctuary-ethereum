...
contract MarketPlace is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function multiListNFT(
        NFTToBeListed[] memory _NFTsToBeListed,
        uint256 _price,
        address _nftContract
    ) public nonReentrant onlyOwner {
        if (0 != _NFTsToBeListed.length) {
            for (uint256 index = 0; index < _NFTsToBeListed.length; index++) {
                uint256 marketItemId = tokenCounter;
                tokenCounter = tokenCounter + 1;
                _idToNFT[marketItemId] = NFT(
                    marketItemId,
                    address(_nftContract),
                    _NFTsToBeListed[index]._tokenID,
                    msg.sender,
                    address(this),
                    _price,
                    true,
                    ...
                    block.timestamp,
                    block.timestamp,
                    _NFTsToBeListed[index]._tokenURI,
                    _NFTsToBeListed[index]._imageURI
                );
                _nftCount.increment();
            }
        }
        ...
        onlyOwner
    {
        ...
        emit NFTSold(marketItemId, nft.tokenId, buyer);
        _nftsSold.increment();
    }
