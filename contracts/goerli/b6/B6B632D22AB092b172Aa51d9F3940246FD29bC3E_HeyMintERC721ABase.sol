...
contract HeyMintERC721ABase is HeyMintERC721AUpgradeable, IERC2981Upgradeable {
    ...
    function initialize(
        ...
        if (_config.fundingDuration > 0) {
            HeyMintStorage.state().data.contractDeployTime = block.timestamp;
        }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return
            bytes(HeyMintStorage.state().cfg.uriBase).length == 0
                ? string(
                    abi.encodePacked(
                        "https://api.heymint.xyz/tokens/",
                        _toString(HeyMintStorage.state().cfg.projectId),
                        "/"
                    )
                )
                : HeyMintStorage.state().cfg.uriBase;
    }
        ...
        returns (string memory)
    {
        ...
        string memory uri = HeyMintStorage.state().data.tokenURIs[tokenId];
    ...
    function publicPriceInWei() public view returns (uint256) {
        return HeyMintStorage.state().cfg.publicPrice * 10 ** 13;
    }
    ...
    function refundGuaranteeActive() public view returns (bool) {
        ...
        return (block.timestamp <
            state.data.contractDeployTime +
                (state.advCfg.refundDuration * 86400));
    }
}
