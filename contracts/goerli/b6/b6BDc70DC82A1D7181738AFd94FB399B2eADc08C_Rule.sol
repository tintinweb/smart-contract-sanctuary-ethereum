...
contract Rule is IRule, Ownable {
    ...
    function setLocalList(CMTAT localList_) external onlyOwner {
    ...
    function detectTransferRestriction(
        ...
        if (globalList.isBlacklisted(_to)) return GLOBALLY_BLACKLISTED_CODE;
        if (localList.isBlacklistedLocally(_to))
            return LOCALLY_BLACKLISTED_CODE;
        if (
            !globalList.isWhitelisted(_to) &&
            !localList.isWhitelistedLocally(_to)
