...
import "./cryptography/NativeMetaTransaction.sol";
import "./interfaces/IMintableERC1155.sol";
import "./interfaces/OpenSea.sol";
import "./meta/ContextMixin.sol";
import "./metadata/ContractURI.sol";
import "./utils/HBArrayUtils.sol";
import "@openzeppelin/contracts/token/ERC1155/extensions/ERC1155Supply.sol";
import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";

contract HBEthereumERC1155 is
    ERC1155Supply,
    ContractURI,
    IMintableERC1155,
    AccessControlMixin,
    ...
    ContextMixin
{
    ...
    address internal immutable OS_PROXY_REGISTRY_ADDRESS; // solhint-disable-line var-name-mixedcase
    address internal immutable MINTABLE_ASSET_PROXY; // solhint-disable-line var-name-mixedcase
    ...
    ) ERC1155(uri_) ContractURI(contractURI_) NativeMetaTransaction(name) {
        OS_PROXY_REGISTRY_ADDRESS = osProxyRegistryAddress;
        MINTABLE_ASSET_PROXY = mintableAssetProxy;
        ...
        returns (bool)
    {
        ...
        if (
            OS_PROXY_REGISTRY_ADDRESS != address(0) &&
            address(proxyRegistry.proxies(account)) == operator
        ) {
            return true;
        }

        if (operator == MINTABLE_ASSET_PROXY) {
            return true;
        }
    ...
    function tokenIdsOf(address owner) public view returns (uint256[] memory) {
        return _ownedTokenIds[owner].values();
    }
    ...
    function _beforeTokenTransfer(
        ...
        (
            uint256 dedupedLength,
            uint256[] memory dedupedIds,
            uint256[] memory dedupedAmounts
        ) = HBArrayUtils.dedupe(ids, amounts);

        for (uint256 index = 0; index < dedupedLength; ++index) {
            uint256 id = dedupedIds[index];
            uint256 amount = dedupedAmounts[index];

            if (from != address(0)) {
                if (balanceOf(from, id) == amount) {
                    _ownedTokenIds[from].remove(id);
                }
            }

            if (to != address(0)) {
                _ownedTokenIds[to].add(id);
            }
        }
    }
...
contract ContractURI is IContractURI {
    ...
    function _setContractURI(string calldata contractURI_) internal {
...
library HBArrayUtils {
    function dedupe(uint256[] memory ids, uint256[] memory amounts)
        internal
        pure
        returns (
            uint256,
    ...
    {
        uint256[] memory dedupedIds = ids;
        uint256[] memory dedupedAmounts = amounts;

        uint256 dedupedLength = 1;
        if (ids.length > 1) {
            dedupedIds = new uint256[](ids.length);
            dedupedAmounts = new uint256[](amounts.length);
            dedupedIds[0] = ids[0];
            dedupedAmounts[0] = amounts[0];

            for (uint256 index = 1; index < ids.length; ++index) {
                uint256 id = ids[index];
                for (
                    uint256 dedupedIndex = 0;
                    dedupedIndex < dedupedLength;
                    ++dedupedIndex
                ) {
                    uint256 dedupedId = dedupedIds[dedupedIndex];
                    if (dedupedId == id) {
                        dedupedAmounts[dedupedIndex] += amounts[index];
                        break;
                    } else if (dedupedIndex == dedupedLength - 1) {
                        dedupedIds[dedupedLength] = id;
                        dedupedAmounts[dedupedLength] = amounts[index];
                        dedupedLength += 1;
                        break;
                    }
                }
        ...
        return (dedupedLength, dedupedIds, dedupedAmounts);
    }
}