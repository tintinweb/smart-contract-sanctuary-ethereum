...
contract SwaprFee is BaseGovernanceUpgradeable {
    ...
    function getBaseOrderFee(uint subjectAmount, address token) public view returns(uint baseFee, uint percentBaseFee, uint fixedBaseFee) {
        (baseFee,percentBaseFee,fixedBaseFee) = _getBaseFee(OrderFee, subjectAmount, token);
    ...
    function _calculatePriceCap(FeePricing memory feePricing, address token) internal view returns (uint priceCapTokens) {
        ...
        priceCapTokens = capRecount * _getFeedDecimals(oracleFeedForToken[token]) / DPS;
    }

    function _getPrice(AggregatorV3Interface priceFeed) public view returns (uint) {
        (
