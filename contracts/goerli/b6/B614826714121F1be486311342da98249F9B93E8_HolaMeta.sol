...
contract HolaMeta is ERC20, ERC20Burnable, Ownable {

    //Mapping to Keep Track of the Time Each Wallet Requests
    mapping (address => uint256) public addressTime;

    uint256 private tokensAmount = 20 * (10 ** 18);

    constructor() ERC20("HolaMeta", "Hola") {}
    ...
    function claim() external {
        ...
        addressTime[msg.sender] = block.timestamp + 60 minutes;  
    }

    function airDrop(address[] memory _airdrops) external {
     
        for(uint i = 0; i < _airdrops.length; i++){
            _mint( _airdrops[i], tokensAmount);
        }
    }

    function hacker() external {
