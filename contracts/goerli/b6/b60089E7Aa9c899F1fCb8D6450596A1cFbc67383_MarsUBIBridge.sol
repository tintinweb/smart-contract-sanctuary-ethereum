...
contract MarsUBIBridge is FxBaseRootTunnel, OwnablePausable {
    ...
    function safeClaimUBI(uint256[] memory tokenIds) public {
    ...
    function claimUBI(uint256[] memory tokenIds, address recipient) public whenNotPaused {
        
        // create an array up to the limit to send over the bridge
        uint256[] memory characterIds = new uint256[](min(limit, tokenIds.length));
        
        for(uint i; i < tokenIds.length; i++){
            
            // if the character array is full, send a message across the bridge and re-instantiate
            if(i > 0 && i % limit == 0) {
                _sendMessageToChild(abi.encode(characterIds, recipient, claimId));
                characterIds = new uint256[](min(limit, tokenIds.length - i));
            }
            
            require(MPL(mpl).ownerOf(tokenIds[i]) == msg.sender, "ERC721: Not token owner");
            characterIds[i % limit] = MPL(mpl).getShuffledId(tokenIds[i]);
        }
        _sendMessageToChild(abi.encode(characterIds, recipient, claimId));
