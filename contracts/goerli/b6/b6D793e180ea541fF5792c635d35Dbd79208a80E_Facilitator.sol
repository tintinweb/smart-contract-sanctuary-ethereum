...
contract Facilitator is IFacilitator, Ownable, Pausable, ReentrancyGuard {
    ...
    function purchaseNFT(address nft, address receiver, string memory tokenURI, bytes memory signature) external payable nonReentrant whenNotPaused {
        if (!listedNFTs[nft]) {
            revert NFTNotListed();
        }
        ...
        {
            nonce = nonce + 1;
            bytes32 messageHash = keccak256(abi.encode(nft, receiver, tokenURI, nonce, address(this)));
            bytes32 hash = ECDSA.toEthSignedMessageHash(messageHash);
            address recoveredAddress = ECDSA.recover(hash, signature);
            if (recoveredAddress == address(0) || recoveredAddress != authorisedPurchaseSigner) {
        ...
        */
        ...
        (address rRecv, uint256 rAmt) = IERC2981(nft).royaltyInfo(tokenId, price);
