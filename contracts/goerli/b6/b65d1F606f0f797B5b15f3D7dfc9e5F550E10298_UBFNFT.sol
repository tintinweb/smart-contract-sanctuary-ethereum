...
contract UBFNFT is ERC721 {
    UBFToken ubtoken;
    uint256 public tokenFee;
    uint256 public tokenCounter;
    uint public maxLogs=5;
    ...
    constructor(address _ubtokenAddress) public ERC721("FINAL EXAM NFT", "UBNFTF") {
        ubtoken=UBFToken(_ubtokenAddress);
        uint8 tokenDecimals = ubtoken.decimals();
        tokenFee = 1000 * (10**tokenDecimals);
        tokenCounter=0;
    }
    function checkTokenBalance() public view returns(uint256){
        return ubtoken.balanceOf(address(this));
    }
    function createLogoNFT(string memory _tokenURI) public returns(bytes32){
        require(tokenCounter<=maxLogs,"maximun quantity of logs emitted");
        require(tokenFee <= ubtoken.balanceOf(address(this)));
