...
contract EggNft is ERC721, Ownable, VRFConsumerBase {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        randomResult = (randomness % (MAX_SUPPLY / MAX_GOLDEN_COUNT)) + 1;
    }
    ...
    function mint(
        ...
        bool isGolden = false;
        if (_goldenNftMintedCount < MAX_GOLDEN_COUNT) {
            if (
                (MAX_GOLDEN_COUNT - _goldenNftMintedCount ==
                    MAX_SUPPLY - _nftMintedCount) ||
                randomResult <= MAX_GOLDEN_COUNT
            ) {
                isGolden = true;
                emit EggType("GoldenEgg", msg.sender);
                _nftMintedCount++;
                _goldenNftMintedCount++;
                _uriToTokenId[_nftMintedCount] = _goldenNftMintedCount;
