...
contract VirtualVandals is ERC721, ERC721Enumerable, Ownable {
    ...
    bool public isDropActive = false;
    ...
    function setDropIsActive(bool active) external onlyOwner {
        isDropActive = active;
    }
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external payable {
        require(isDropActive);
        ...
        require(pricePerToken * amount <= msg.value, "Ether value sent is not correct"); // ensure enough money paid for token

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        require(
            MerkleProof.verify(merkleProof, dropMerkleRoot, node),
