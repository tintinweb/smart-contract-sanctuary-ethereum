...
/// @title StakingManager for veAUXO
/// @author alexintosh
/// @notice Tokens are staker in perpetuity, no coming back
/// @dev Make sure you understand how the Timelock works
contract StakingManager {
    ...
    modifier onlyOperator() {
        require(operators[msg.sender] == true, "not authorized");
        _;
    }
    ...
    /**
     *
     * @notice Governor need to be added as operator post deployment
     */
    ...
    function approveAuxo(uint256 amount) external onlyGovernance {
        IERC20(AUXO).approve(address(tokenLocker), amount);
        emit AuxoApproval(address(tokenLocker), amount);
    }

    /**
     * @notice veAUXO holder elect the representative
     */
    function delegateTo(address representative) external onlyGovernance {
        IVotes(veAUXO).delegate(representative);
        emit RepresentativeChanged(representative);
    }
    ...
    function stake() external {
        ...
        tokenLocker.depositByMonths(uint192(balance), MONTHS, address(this));
    }
    ...
    function claim(bytes32[] calldata proof) external onlyOperator {
        //TODO
    }

    // Anybody can call boostToMax
    function boostToMax() external {
        ITokenLocker(tokenLocker).boostToMax();
    }
}