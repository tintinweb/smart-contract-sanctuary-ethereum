...
contract SBIFactory is IProductFactory {
    ...
    function deploy(
        ...
        instance = address(new SingleBondIssuance(governance, custodian, feeConfigs, parametersConfig, name, symbol));
...
contract RewardsManager is IRewardsManager, ERC721, Ownable {
    ...
    function _stake(uint256 positionId, uint256 lockingDuration) private {
        ...
        if (withLockRewards && (lockingDuration < minLockingDuration || lockingDuration > maxLockingDuration))
...
abstract contract RCLBorrowers is IRCLBorrowers, RCLOrderBook {
            ...
            if (tick.baseEpochsAmounts.borrowed > 0) {
                ...
                if (lastBorrowedEpoch.available > 0) {
                    tick.newEpochsAmounts.signalledEarnings += lastBorrowedEpoch
                        ...
                        .mul(lastBorrowedEpoch.endOfLoanEarnings);
                }
                tick.accrueYield(currentInterestRate);
                registerRepayTimeDeltas();
                if (tick.newEpochsAmounts.signalledDeposits > 0) {
    ...
    function registerRepayTimeDeltas() internal {
        if (block.timestamp >= currentMaturity) {
            loans[currentLoanId].repayTimeDelta = block.timestamp - currentMaturity;
        } else {
            loans[currentLoanId].repayTimeDelta = currentMaturity - block.timestamp;
            loans[currentLoanId].earlyRepay = true;
        }
    }

    function adjustForExits(DataTypes.Tick storage tick, uint256 rate) internal {
        uint256 deltaToMaturityAdjustment = tick.newEpochsAmounts.borrowed.mul(loans[currentLoanId].repayTimeDelta).mul(
            rate
        );
        if (loans[currentLoanId].earlyRepay) {
...
library TickLogic {
        ...
        )
    {
        ...
        if (tick.baseEpochsAmounts.available + tick.newEpochsAmounts.available > 0) {
            ...
            uint256 accuralsAllocated = args.totalAccrualsToSwap - remainingAccruals;
            penalty = ((accuralsAllocated * (args.currentMaturity - block.timestamp)) / 365 days).mul(args.rate);
        }
    }
    ...
    function accrueYield(DataTypes.Tick storage tick, uint256 rate) internal {
        uint256 accrualRate = (rate * (block.timestamp - tick.lastAccrualTimeStamp)) / 365 days;
...
abstract contract RCLLenders is IRCLLenders, RCLOrderBook, ERC721 {
    ...
    function deposit(
        ...
        // TODO positionId = currentPositionId - 1 is ugly and redundant
    ...
    function withdraw(uint256 positionId) external onlyInPhase(DataTypes.OrderBookPhase.OPEN) {
        ...
        // TODO this is hard to read - i think this is correct but i'm not sure
        // we should make it very explicit that a withdraw can ONLY happen
        // if the epoch is not borrowed
        ...
        if (tick.currentEpochId == position.epochId) {
            ...
            if (tick.baseEpochsAmounts.borrowed > 0) {
                ...
                tick.epochs[ticks[position.rate].currentEpochId].available -= withdrawableAmount;
            } else {
    ...
    function signal(uint256 positionId) external onlyInPhase(DataTypes.OrderBookPhase.OPEN) {
        ...
        if (position.epochId <= tick.loanStartEpochId) {
            tick.baseEpochsAmounts.signalledAdjusted += position.originalDeposit.div(
                getEquivalentRatio(epoch, tick, position.rate)
            );
        } else {
            tick.newEpochsAmounts.signalledDeposits += position.originalDeposit;
            epoch.signalled += position.originalDeposit;
            tick.newEpochsAmounts.signalledEarnings += position.originalDeposit.div(epoch.deposited).mul(
                epoch.endOfLoanEarnings
            );
        }
    ...
    // TODO: right now accruals are not being removed from available balances of buying ticks
    function exit(uint256 positionId) external onlyInPhase(DataTypes.OrderBookPhase.OPEN) {
        ...
        if (position.epochId <= tick.loanStartEpochId) {
            uint256 adjustedAmount = getAdjustedAmount(epoch, tick, position);
            ...
            uint256 shareOfBaseEpoch = adjustedAmount.div(tick.adjustedDeposits);
            tick.baseEpochsAmounts.borrowed -= shareOfBaseEpoch.mul(tick.baseEpochsAmounts.borrowed);
            tick.adjustedDeposits -= adjustedAmount;
            availablePortionToExit = shareOfBaseEpoch.mul(tick.baseEpochsAmounts.available);
            ...
            uint256 startOfLoanValue = position.originalDeposit;
            uint256 shareOfEpoch = startOfLoanValue.div(epoch.deposited);

            uint256 yield = shareOfEpoch.mul(epoch.endOfLoanEarnings);
            endOfLoanBorrowEquivalent = startOfLoanValue + yield;

            uint256 positionBorrowedAmount = shareOfEpoch.mul(epoch.borrowed);
            tick.newEpochsAmounts.borrowed -= positionBorrowedAmount;

            tick.newEpochsAmounts.available -= shareOfEpoch.mul(epoch.deposited - epoch.borrowed);
            epoch.borrowed -= positionBorrowedAmount;
            epoch.deposited -= startOfLoanValue;
            epoch.available -= startOfLoanValue;
        ...
        while (endOfLoanBorrowEquivalent > 10 && remainingAccrualsToExit > 10 && rate <= MAX_RATE) {
            ...
            (uint256 tickBorrowed, uint256 accuralsRemaining, uint256 penalty) = iterationTick.borrow(
                DataTypes.BorrowInput({
            ...
            penaltyToSeller += penalty;
            iterationTick.loanAccruedInterests += penalty;
            ...
            remainingAccrualsToExit = accuralsRemaining;
            rate += RATE_SPACING;
        }
        ...
        CUSTODIAN.withdraw(totalBorrowed + availablePortionToExit + accrualsToExit - penaltyToSeller, msg.sender);
    }
    ...
    function getAdjustedAmount(
        ...
        positionValue = position.originalDeposit.div(getEquivalentRatio(epoch, tick, position.rate));
    }

    // TODO this is only true for base epochs
    ...
    function computePositionValue(
        ...
        positionValue = position.originalDeposit.mul(tick.yieldFactor).div(
            getEquivalentRatio(epoch, tick, position.rate)
        );
    }
    ...
    function getEquivalentRatio(
        ...
        DataTypes.Tick storage tick,
        uint256 rate
    ) internal view returns (uint256 equivalentRatio) {
        if (epoch.isBaseEpoch) {
            // TODO epoch.loanId - 1 is not necessarily the right loan id
            // eg when the tick is matched for a loan, not for the one after, and again for the one after
            // in that case the right loan id is epoch.loanId - 2
            equivalentRatio = tick.endOfLoanYieldFactors[epoch.loanId - 1];
        } else {
            uint256 deltaToMaturityAccruals = ((epoch.borrowed * loans[epoch.loanId].repayTimeDelta) / SECONDS_PER_YEAR)
                .mul(rate);

            if (loans[epoch.loanId].earlyRepay) {
                equivalentRatio = tick.endOfLoanYieldFactors[epoch.loanId].mul(epoch.deposited).div(
                    epoch.endOfLoanEarnings + epoch.deposited - deltaToMaturityAccruals
                );
            } else {
                equivalentRatio = tick.endOfLoanYieldFactors[epoch.loanId].mul(epoch.deposited).div(
                    epoch.endOfLoanEarnings + epoch.deposited + deltaToMaturityAccruals
