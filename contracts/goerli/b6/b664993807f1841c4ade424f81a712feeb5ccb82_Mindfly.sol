...
abstract contract AccessControl is Context, IAccessControl, ERC165 {
    ...
    function grantRole(bytes32 role, address account) public virtual override onlyRole(getRoleAdmin(role)) {
        require(role != keccak256("MINT_ROLE"), "MINT_ROLE can't be added");
    ...
    ERC721ACommon
{
    ...
    bytes32 public constant MINT_TYPEHASH =
        keccak256("Mint(address account,uint256 amount,uint256 nonce)");

    bytes32 public constant SAFE_TRANSFER =
        keccak256("SafeTransferWhileCitizenCheckIn(address from,address to, unit256 tokenId,uint256 nonce)");
    ...
    /**
    @notice mintLock when it true, no more mint
     */
    ...
    @dev when mint finish, call finishMintAndBurnRest().
     */
    ...
    constructor(
        ...
        FixedPriceSeller(
            ...
            Seller.SellerConfig({
                ...
                maxPerTx: 10,
                freeQuota: uint248(reserveRemaining),
    ...
    */
    ...
    function _handlePurchase(
        ...
        _safeMint(to, n);
        assert(totalSupply() <= totalInventory);
    }
    ...
    function _mintSignCheck(
        ...
        bytes32 structHash = keccak256(
            abi.encode(MINT_TYPEHASH, address(msg.sender), _amount, nonces[msg.sender]++)
        );
    ...
    function _signTransferWhileCitizenCheck(
        address to,
        uint256 tokenId,
        uint8 v,
        bytes32 r,
        ...
        bytes32 domainSeparator = keccak256(
            abi.encode(
                DOMAIN_TYPEHASH,
                keccak256(bytes("SafeTransferWhileCitizenCheckIn")),
                keccak256(bytes("1")),
                getChainId(),
                address(this)
            )
        ...
        bytes32 structHash = keccak256(
            abi.encode(SAFE_TRANSFER, address(msg.sender), to, tokenId, nonces[msg.sender]++)
        );
        bytes32 digest = keccak256(
            abi.encodePacked("\x19\x01", domainSeparator, structHash)
        );
    ...
    function mintUnclaimed(address to, uint256 n)
        ...
        reduceReservePool(n)
    {
        require(mintLock != true, "Mint already finish! No more mint!");
    ...
    function safeTransferWhileCitizenCheckIn(
        ...
        _signTransferWhileCitizenCheck(to, tokenId, v, r, s);
        citizenTransfer = 2;
        safeTransferFrom(from, to, tokenId);
        citizenTransfer = 1;
    }
    ...
    */
    ...
    /**
    @notice Changes the Mindfly' citizening status
     */
