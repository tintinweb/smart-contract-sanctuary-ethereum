...
contract VCXNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    /// @notice This mapping checks if an address has already minted during platinum presale
    mapping(address => bool) public addressPlatinumPresaleMinted;
    ...
    function crossMint(
        ...
        if (salePhase == 3 || salePhase == 4) {
            require(
                !addressPlatinumPresaleMinted[to] &&
                    !addressGoldPresaleMinted[to],
        ...
        if (salePhase == 3) {
            addressPlatinumPresaleMinted[to] = true;
        } else if (salePhase == 4) {
            addressGoldPresaleMinted[to] = true;
        }
    }
    ...
    function mintPlatinum(bytes32[] calldata merkleProof)
        public
        payable
        nonReentrant
    {
        ...
        require(
            addressMintCount[msg.sender] + 1 <= MAX_TOKENS_PER_WALLET,
            "Exceeds max tokens per wallet"
        );
        require(totalSupply() + 1 <= MAX_TOKENS, "Exceeds max tokens");
        require(msg.value >= price, "Not enough ETH sent");
        require(
            verifyPlatinumWhitelist(merkleProof, msg.sender),
            "Invalid merkle proof"
        );
        require(!addressPlatinumPresaleMinted[msg.sender], "Already minted");

        _mint(msg.sender, 1);
        addressPlatinumPresaleMinted[msg.sender] = true;
    }

    /// @notice This mapping checks if an address has already minted during gold presale
    mapping(address => bool) public addressGoldPresaleMinted;

    /// @notice Mint function for gold group presale
    /// @param merkleProof Merkle proof
    /// @dev Only addresses on the platinum and gold presale whitelists can mint
    function mintGold(bytes32[] calldata merkleProof)
        public
        payable
        nonReentrant
    {
        ...
        if (verifyPlatinumWhitelist(merkleProof, msg.sender)) {
            require(
                !addressPlatinumPresaleMinted[msg.sender],
                "Already minted during platinum presale"
            );
        } else {
            require(
            ...
            require(
                !addressGoldPresaleMinted[msg.sender],
                "Already minted during gold presale"
            );
        }

        _mint(msg.sender, 1);
        addressGoldPresaleMinted[msg.sender] = true;
    }
