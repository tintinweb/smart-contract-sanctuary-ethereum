...
contract KittyBreeding is KittyOwnership {
    ...
    {
        // Checks for payment.
        require(msg.value >= autoBirthFee,"value");

        // Caller must own the matron.
        require(_owns(msg.sender, _matronId),"owner eror");
        ...
        require(_isSiringPermitted(_sireId, _matronId),"permission eror");

        // Grab a reference to the potential matron
        Kitty storage matron = kitties[_matronId];

        // Make sure matron isn't pregnant, or in the middle of a siring cooldown
        require(_isReadyToBreed(matron),"not ready to breed matron");

        // Grab a reference to the potential sire
        Kitty storage sire = kitties[_sireId];

        // Make sure sire isn't pregnant, or in the middle of a siring cooldown
        require(_isReadyToBreed(sire),"not ready to breed sire");
...
contract ClockAuction is Pausable, ClockAuctionBase {
    ...
    function ClockAuction(address _nftAddress, uint256 _cut) public {
        require(_cut <= 10000,"cut eror");
        ownerCut = _cut;

        ERC721 candidateContract = ERC721(_nftAddress);
        require(candidateContract.supportsInterface(InterfaceSignature_ERC721),"contract is not erc 721");
