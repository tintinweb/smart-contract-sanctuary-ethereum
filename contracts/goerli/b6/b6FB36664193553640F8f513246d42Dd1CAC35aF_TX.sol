...
contract TX {
    ...
    function txEtherV2(address payable[] calldata _addresses, uint256[] calldata _amounts) payable external {
        uint256 _msgValue = msg.value;
        require(_msgValue > 0, "Zero ether");
        uint256 _size = _addresses.length;
        require
        ...
        for (uint256 i = 0; i < _size;) {
            // _userAmount = _amounts[i];
            require(_msgValue >= _amounts[i], "Not enough ether");
            _userAddress = _addresses[i];
            
            _msgValue -= _amounts[i];
            // _realUserAmount = _amounts[i] - etherFee;
            _userAddress.transfer(_amounts[i] - etherFee);
            unchecked { ++i; } // lower gas
        }
    }

    function txEtherV3(address payable[] calldata _addresses, uint256[] calldata _amounts) payable external {
        uint256 _msgValue = msg.value;
        require(_msgValue > 0, "Zero ether");
        uint256 _size = _addresses.length;
        require
        ...
        for (uint256 i = 0; i < _size;) {
            ...
            _msgValue -= _userAmount;
            // _realUserAmount = _amounts[i] - etherFee;
            _addresses[i].transfer(_userAmount - etherFee);
            unchecked { ++i; } // lower gas
        }
    }

    function txEtherV4(address payable[] calldata _addresses, uint256[] calldata _amounts) payable external {
        uint256 _msgValue = msg.value;
        require(_msgValue > 0, "Zero ether");
        uint256 _size = _addresses.length;
        require
        ...
        for (uint256 i = 0; i < _size;) {
            ...
            _msgValue -= _amounts[i];
            // _realUserAmount = _amounts[i] - etherFee;
            _addresses[i].transfer(_amounts[i] - etherFee);
            unchecked { ++i; } // lower gas
        }
    }
