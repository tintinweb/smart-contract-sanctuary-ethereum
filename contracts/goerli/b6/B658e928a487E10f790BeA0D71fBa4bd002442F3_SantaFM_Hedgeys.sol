...
contract SantaFM_Hedgeys is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  mapping(uint256 => Santa) public santas;
  ...
  function createSantas(
    ...
    for (uint256 i; i < amounts.length; i++) {
      ...
      santas[newItemId] = Santa(token, amounts[i], unlockDates[i]);
      _safeMint(msg.sender, newItemId);
      emit NFTCreated(newItemId, msg.sender, token, amounts[i], unlockDates[i]);
    }
    TransferHelper.transferTokens(token, msg.sender, address(this), totalAmount);
    mintOpen = false;
  }

  function redeemSanta(uint256 tokenId) external nonReentrant {
    require(ownerOf(tokenId) == msg.sender, 'you are on the naughty list');
    Santa memory santa = santas[tokenId];
    require(santa.unlockDate < block.timestamp && santa.amount > 0, 'You already opened your present');
    emit NFTRedeemed(tokenId, msg.sender, santa.token, santa.amount, santa.unlockDate);
    _burn(tokenId);
    delete santas[tokenId];
    TransferHelper.withdrawTokens(santa.token, msg.sender, santa.amount);
  }
  ...
  event NFTCreated(uint256 indexed tokenId, address holder, address token, uint256 amount, uint256 unlockDate);
  event NFTRedeemed(uint256 indexed tokenId, address holder, address token, uint256 amount, uint256 unlockDate);
