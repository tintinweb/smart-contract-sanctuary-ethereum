...
contract Exchange is SuAuthenticated, IExchange {
    ...
    function fillOrder(uint256 orderId, uint256 tokenBAmount) external returns (uint256, uint256) {
        ...
        console.log('all is calculated');
        // DUST_THRESHHOLD is x * 1e18
        if (order.amountLeft > 0 && orderPrice * order.amountLeft / 1e18 <= DUST_THRESHHOLD) {
            console.log('amountLeft', order.amountLeft);
            console.log('dust', orderPrice * order.amountLeft / 1e18);
            revert SmallAmountLeft();
        }

        console.log('start transfer 1', address(this), msg.sender, order.beneficiary);
