...
contract Ballbiaga is Ownable {
    ...
    function claimPack() public {
        address addr = msg.sender;
        require(inviteeListMapping[addr].length / packRefCnt - claimedPkgMapping[addr] >= 1, 'there is no pack to claim');
   ...
   function getInviteeList(address _address) view public returns (Invitee[] memory inviteeList) {
    ...
    //     return address(nftCard);
    // }
    //  function getFundPoolAddress() view public returns (address) {
