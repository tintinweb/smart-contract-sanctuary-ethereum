...
// @TODO: Use tokensOfUser or tokensOfUserIn based on the stakingToken.totalSupply() > x (x = max safe number of tokenIds that can be queried without reversion)
...
contract MajrStaking is Ownable, AccessControl, ReentrancyGuard, Pausable {
  ...
  */
  ...
  /**
   * @notice Returns the IDs of the NFTs that a particular user has staked
   * @param _user address
  ...
  /**
   * @notice Transfer staking token (NFTs) to the staking contract
   * @param _amount uint
   * @dev Updates rewards on call
   */
  function stake(uint256 _amount) external nonReentrant whenNotPaused updateReward(msg.sender)  {
    require(_amount > 0, "MajrStaking: Cannot stake 0 tokens.");

    _totalSupply = _totalSupply.add(_amount);
    _balances[msg.sender] = _balances[msg.sender].add(_amount);
  
    uint256[] memory tokenIds = new uint256[](stakingToken.balanceOf(msg.sender));

    uint256 totalSupplyOfIDs = stakingToken.totalSupply();

    uint256 k = 0;

    if (totalSupplyOfIDs >= 2000) {
      for (uint256 i = 0; i < totalSupplyOfIDs; i += 2000) {
        uint256[] memory tokenIdsBatch = stakingToken.tokensOfUserIn(msg.sender, i, i + 2000);
        
        for (uint256 j = 0; j < tokenIdsBatch.length; j++) {
          tokenIds[k] = tokenIdsBatch[j];
          k++;
        }
      }
    } else {
      tokenIds = stakingToken.tokensOfUser(msg.sender);
    }
  ...
  function withdraw(uint256 _amount) public nonReentrant updateReward(msg.sender) {
    ...
    for (uint256 i = 0; i < _amount; i++) {
      stakingToken.transferFrom(address(this), msg.sender, userStakedTokenIds[msg.sender][userStakedTokenIds[msg.sender].length - 1]);
