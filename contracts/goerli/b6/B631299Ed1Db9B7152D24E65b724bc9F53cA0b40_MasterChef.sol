...
contract UniToken is ERC20('UniSwap Token', 'Uni') {
...
contract SyrupBar is ERC20('SyrupBar Token', 'SYRUP') {
    ...
    // Safe uni transfer function, just in case if rounding error causes pool to not have enough UNIs.
    function safeUniTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorChef {
    // Perform LP token migration from legacy UniSwap to UniSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniSwap LP tokens.
    // UniSwap must mint EXACTLY the same amount of UniSwap LP tokens or
    // else something bad will happen. Traditional UniSwap does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MasterChef is the master of Uni. He can make Uni and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    function pendingUni(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 uniReward = multiplier.mul(uniPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accUniPerShare = accUniPerShare.add(uniReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accUniPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 uniReward = multiplier.mul(uniPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uni.mint(devaddr, uniReward.div(10));
        uni.mint(address(syrup), uniReward);
        pool.accUniPerShare = pool.accUniPerShare.add(uniReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for UNI allocation.
    function deposit(uint256 _pid, uint256 _amount) public {

        require (_pid != 0, 'deposit UNI by staking');

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accUniPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeUniTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accUniPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw UNI by unstaking');
    ...
    // Stake UNI tokens to MasterChef
    ...
    // Withdraw UNI tokens from STAKING.
    ...
    // Safe uni transfer function, just in case if rounding error causes pool to not have enough UNIs.
    function safeUniTransfer(address _to, uint256 _amount) internal {
        syrup.safeUniTransfer(_to, _amount);
    }
