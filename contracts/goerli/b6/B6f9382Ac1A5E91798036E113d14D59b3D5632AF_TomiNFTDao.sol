...
contract TomiStates {
    ...
    struct ProposalSendNFTData {
        /// @notice Unique id for looking up a proposal
        uint256 id;
        /// @notice NFTs to send
        address[] executionTargets;
        /// @notice Targets for sending
        uint256[] executionNFTIds;
    }

    struct ProposalBlockWalletData {
        /// @notice Unique id for looking up a proposal
        uint256 id;
    ...
    mapping (uint256 => ProposalSendNFTData) internal proposalsSendNFTData;
    mapping (uint256 => ProposalBlockWalletData) internal proposalsBlockWalletData;
...
contract TomiNFTDao is TomiEvents, TomiStates, Ownable {
    using SafeMath for uint256;

    constructor(ITomiMainDao _tomiMainDao, ITreasury _treasury) {
    ...
    function getProposalsSendNFTData(uint256 proposalId) external view returns (ProposalSendNFTData memory) {
        return proposalsSendNFTData[proposalId];
    }

    function getProposalsBlockWalletData(uint256 proposalId) external view returns (ProposalBlockWalletData memory) {
        return proposalsBlockWalletData[proposalId];
    }
    ...
    function proposeSendNFT(
        string memory description,
        uint256[] memory nftIds,
        string memory title,
        uint8 functionId,
        address[] memory executionTargets,
        uint256[] memory executionNFTIds,
        uint256 paymentType
        ...
        require(functionId == 5, "incorrect function id");
        ...
        ProposalSendNFTData storage newProposalData = proposalsSendNFTData[tomiMainDao.proposalCount()];
        newProposalData.id = tomiMainDao.proposalCount();
        newProposalData.executionTargets = executionTargets;
        newProposalData.executionNFTIds = executionNFTIds;
    }

    function proposeBlockNFTWallet(
        string memory description,
        uint256[] memory nftIds,
        string memory title,
        uint8 functionId,
        ...
        require(nftIds.length == tomiMainDao.amountNFTRequiredToPropose(), "insufficient NFT amount for proposal");
        require(functionId == 6, "incorrect function id");
        tomiMainDao.updateProposalCount(tomiMainDao.proposalCount() + 1);
        ...
        ProposalBlockWalletData storage newProposalData = proposalsBlockWalletData[tomiMainDao.proposalCount()];
        newProposalData.id = tomiMainDao.proposalCount();
    ...
    function processEthPayments() internal {
        ...
        (bool sent, ) = address(treasury).call{value: amountFeeInEth}("");
    ...
    function executeSendNFT(uint256 proposalId) external {
        Proposal storage proposal = proposals[proposalId];

        require(proposal.functionId == 5, "incorrect function id");
        ...
        if (state(proposalId) == ProposalState.Succeeded) {
            ...
            ProposalSendNFTData memory proposalData = proposalsSendNFTData[proposal.id];
            for (uint256 i = 0 ; i < proposalData.executionTargets.length ; i++) {
                treasury.withdrawNFT(tomiNFT, proposalData.executionTargets[i], proposalData.executionNFTIds[i]);
                // tomiNFT.transferFrom(address(treasury), proposalData.executionTargets[i], proposalData.executionNFTIds[i]);
            }
        }
    ...
    function executeBlockNFTWallet(uint256 proposalId) external {
        Proposal storage proposal = proposals[proposalId];

        require(proposal.functionId == 6, "incorrect function id");
        require(state(proposalId) == ProposalState.Succeeded || state(proposalId) == ProposalState.Defeated,
            "TomiDAO::execute: proposal can only be executed if it has succeeded or failed");
        require(block.timestamp >= proposal.endTime, "proposal is currently active");
        ...
        if (state(proposalId) == ProposalState.Succeeded) {
            ...
            // TODO
            // address wallet = proposalsBlockWalletData[proposalId].wallet;
            // tomiNFT.blockWallet(wallet);
        }
...
interface ITreasury {
    ...
    // TODO (?)
    function deposit(uint256 _amount, address _token, uint256 _profit) external returns (uint256 send);
