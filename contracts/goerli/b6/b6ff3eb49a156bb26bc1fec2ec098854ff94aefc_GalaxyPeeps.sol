    ...
    DefaultOperatorFilterer
{
    ...
    modifier merkleProven(bytes32[] calldata _merkleProof, MintPass mintPass) {
        if (mintPass == MintPass.FREE_SALE) {
            revert("Invalid mint pass");
        }
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, mintPass));
        require(
    ...
    modifier merkleProvenFree(bytes32[] calldata _merkleProof, MintPass mintPass, uint256 maxQty) {
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, mintPass, maxQty));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, leaf),
            "Address is not whitelisted for free sale @ this quantity"
        );
        _;
    }
