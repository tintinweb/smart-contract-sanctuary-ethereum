...
contract Multiple_Collection is ERC1155{
    ...
    _;
     ...
     function mint(string memory _uri, uint256 amount, uint royalty) ifHashExist(_uri) public
    {
        tokenId.increment();
        hash[_uri]=true;
        ...
        nftOwner[id].nft_owner = msg.sender;
        nftOwner[id].royalty = royalty;

        currentOwner[id] = msg.sender;
    ...
    function transfer(address _from, address _to, uint _id, uint _amount) public{
    ...
    function getRoyaltyOfNFT(uint _tokenId) public view returns(uint){
        return nftOwner[_tokenId].royalty;
    }
