...
import "./EverestConsumer.sol";

contract HelloWorldKyc {
    string private text;

    EverestConsumer private everestConsumer;

    constructor(EverestConsumer _everestConsumer) {
        text = "Hello World";
        everestConsumer = _everestConsumer;
    }
    ...
    modifier onlyUniqueHuman() {
        // Retrieve the latest fulfilled request for the caller
        EverestConsumer.Request memory request = everestConsumer.getLatestFulfilledRequest(
            msg.sender
        );

        // Check if the caller is a unique human
        require(request.isHumanAndUnique, "Caller must be a unique human");

        _;
    }

    function setText(string memory newText) public onlyUniqueHuman {
        text = newText;
    }
}
