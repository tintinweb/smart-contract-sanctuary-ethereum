...
contract GP is Initializable, IGP, GPState {
        ...
        returns (bool)
    {
        ...
        require(
            isAdmin(_msgSender()) || lastWrite[sender] < block.number,
            "hmmmm what doing?"
        );
    ...
    modifier disallowIfStateIsChanging() {
        ...
        require(
            isAdmin(_msgSender()) || lastWrite[tx.origin] < block.number,
            "hmmmm what doing?"
        );
        _;
    }

    function updateOriginAccess() external override onlyAdminOrOwner {
        ...
        returns (uint256)
    {
        ...
        require(
            isAdmin(_msgSender()) || lastWrite[account] < block.number,
            "hmmmm what doing?"
        );
        ...
        returns (bool)
    {
        // NICE TRY MOUSE DRAGON
        require(
            isAdmin(_msgSender()) || lastWrite[_msgSender()] < block.number,
            "hmmmm what doing?"
        );
    ...
    ERC20Upgradeable
{
    ...
    mapping(address => uint256) internal lastWrite;
