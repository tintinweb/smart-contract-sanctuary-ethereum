...
contract TestDrop is ERC721A, Ownable {
  using Strings for uint256;

  bool public isTransferPaused = false;
  bool public isListingPaused = true;
  bool private isRevealed = true;
  string private uriPrefix;
  string private hiddenMetadataURI;

  constructor() ERC721A("TestDrop", "TDP") {}

  function setApprovalForAll(address operator, bool approved) public virtual override {
    if (isListingPaused) {
      revert("Listing paused");
    }
  ...
  function toggleTransfer(bool _state) external onlyOwner {
    isTransferPaused = _state;
  }

  function toggleListing(bool _state) external onlyOwner {
    isListingPaused = _state;
  }
  ...
  function updateURIPrefix(string calldata _uriPrefix) external onlyOwner {
