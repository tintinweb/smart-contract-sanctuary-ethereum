...
contract Oracle is Ownable {
    ...
    mapping(bytes32 => bool) public hashConditions;
    ...
    function listCondition(
        uint128 _scopeId,
        uint64[2] calldata _odds,
        uint64[2] calldata _outcomes,
        uint64 _timestamp,
        bytes32 _ipfsHash
    ) external {
        require(isReporter[msg.sender], "Oracle:unauthorized");
        require(!hashConditions[_ipfsHash], "Oracle: hash is exists");
        ...
        hashConditions[_ipfsHash] = true;
        conditions[lastOracleConditionId] = _condition;
        emit ConditionListed(lastOracleConditionId, _scopeId, _odds, _outcomes, _timestamp, _ipfsHash);
    }
    ...
    // =========== Events ===========
    event ConditionListed(uint256 conditionId, uint128 scopeId,
        uint64[2] odds,
        uint64[2] outcomes,
        uint64 timestamp,
        bytes32 ipfsHash);
