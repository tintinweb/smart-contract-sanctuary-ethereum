...
contract RiggedRoll is Ownable {
    event RiggedRoll_Receive_Value(address indexed sender,uint256 indexed sendValue);
    ...
    function withdraw(address _addr, uint256 _amount) external onlyOwner(){
        ...
        require(success,"transcation invalid");
    }
    ...
    function riggedRoll() public {
        ...
        uint256 diceNonce = diceGame.nonce();
        address  diceAddress = address(diceGame);
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash,diceAddress, diceNonce));
        uint256 roll = uint256(hash) % 16;
    ...
    receive() external payable{
        emit RiggedRoll_Receive_Value(msg.sender,msg.value);
    }
