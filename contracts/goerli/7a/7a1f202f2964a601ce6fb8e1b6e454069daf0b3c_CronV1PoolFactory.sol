...
contract CronV1PoolFactory is BasePoolFactory {
  // This contract deploys Cron V1 pools
  ...
  event CronV1PoolCreated(address indexed pool, address indexed token0, address indexed token1, uint256 poolType);
  ...
  function create(
    ...
    address pool = address(
      new CronV1Pool(IERC20(_token0), IERC20(_token1), getVault(), _name, _symbol, _poolType, address(this), _pauser)
    );
    ...
    emit CronV1PoolCreated(pool, _token0, _token1, _poolType);
    return pool;
  }

  /// @notice Sets a new factory administrator
