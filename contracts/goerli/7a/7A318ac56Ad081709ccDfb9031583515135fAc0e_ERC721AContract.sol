...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer, ReentrancyGuard, Referable {
    ...
    function isEligible(
        ...
        if(token.preSaleIsActive) {
            ...
            _isEligiblePreSale = _hasSupply &&
                ...
                _quantity <= (_maxMint - _hasMinted);
        }
        if (token.pubSaleIsActive) {
            _isEligiblePubSale = _hasSupply && _quantity <= token.maxPerTransaction;
        }
    ...
    function _mintTo(
        ...
        (bool _isEligiblePreSale, bool _isEligiblePubSale) = isEligible(_address, uint16(_quantity), _maxMint, _proof);
    ...
    function crossmint(address _address, uint256 _quantity, uint16 _maxMint, bytes32[] memory _proof, address payable _referrer) external payable nonReentrant onlyCrossmint {
        _mintTo(_address, _quantity, _maxMint, _proof, _referrer, msg.value);
    }
