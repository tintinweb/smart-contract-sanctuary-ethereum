...
contract TokenRewards is ITokenRewards, TokenRecoveryUpgradeable {
     ...
     */
    function initialize(address _rewardToken, uint256 _startTime, uint256 _endTime, bytes32 _merkleRoot)
        external
        initializer
    {
    ...
    modifier onlySeizeRole() {
        ...
        require(
            hasRole(LOCAL_SEIZE_ROLE, sender) || hasRole(GLOBAL_SEIZE_ROLE, sender), "TokenRewards: UNAUTHORIZED SEIZE"
        );
        _;
    }
    ...
    modifier onlyAdminRole() {
        address sender = _msgSender();
        require(
            hasRole(LOCAL_ADMIN_ROLE, sender) || hasRole(GLOBAL_ADMIN_ROLE, sender), "TokenRewards: UNAUTHORIZED ADMIN"
        );
        _;
    }
     ...
     * @inheritdoc ITokenRewards
     */
    function claim(bytes32[] calldata _proof, uint256 _amount) external whenNotPaused returns (bool) {
...
interface ITokenRewards {
     ...
     */
    function claim(bytes32[] calldata _proof, uint256 _amount) external returns (bool);
