 ...
 */
contract WWNFT is ERC721Enumerable, ERC2981, Ownable {
    ...
    constructor(string memory _name, string memory _symbol, string memory _hiddenMetadataUri) ERC721(_name, _symbol) {
        ...
        _setDefaultRoyalty(address(this), 10);

        // _mintLoop(msg.sender, 69);
    }
    ...
    function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
        ...
        uint256 mintAmountWithOneFree = _mintAmount + 1;

        require(!paused, "The contract is paused!");
        require(presale || publicSale, "Sale has not started!");
        if(presale && !publicSale) {
            checkWhitelist(_merkleProof);
        }
        require(totalSupply() + mintAmountWithOneFree <= maxSupply, "Max supply exceeded!");
        require(mintAmountWithOneFree <= maxMintAmountPerTx, "Max amount per transaction exceeded!");
        require(balanceOf(msg.sender) + mintAmountWithOneFree <= maxPerWallet, "Max amount per wallet exceeded!");
        require(msg.value >= cost * _mintAmount, "Insufficient funds!");

        _mintLoop(msg.sender, mintAmountWithOneFree);
    }
    ...
    function checkWhitelist(bytes32[] calldata _merkleProof) public view {
