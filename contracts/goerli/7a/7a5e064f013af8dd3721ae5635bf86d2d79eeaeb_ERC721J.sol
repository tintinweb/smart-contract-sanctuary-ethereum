...
interface IERC721JPromoUpgradeable is IERC721JUpgradeable {
    ...
    function promoClaim(address _contract, uint256 tokenId) external payable;

    //Mints a promo token using the setting that needs matching songId and generation to be accepted
    function promoClaimSpecific(address _contract, uint256 tokenId) external payable;
...
//Added promo / free claim system. Supports ERC721 tokens, and 721J support letting you set promos with rarity and songId.
    ...
    IERC2981Upgradeable
{
    ...
    // Define Promo Percent Multiplier 
    uint256 private _promoPercent;
    ...
    //For Promo Multiplier
    function setPromoMultiplier(uint256 multiplierPercent ) public virtual onlyOwner {
        _promoPercent = multiplierPercent;
    }
    ...
    //Returns the percent price multiplier for promos
    function promoMultiplier() public view returns (uint256 multiplierPercent) {
    ...
    function payPromo(uint256 _songIdOut) internal {

        uint256 promoPercent = _promoPercent;
        //Requires eth
        if (msg.value < (_mintPrice * promoPercent) / 100) revert NotEnoughEther();
        //Transfer eth
        if (promoPercent != 0 && address(_songSplits[_songIdOut]) != address(0))
    ...
    //Promo claim is for ERC721 tokens
    function promoClaim(address _contract, uint256 tokenId) public payable override {
    ...
    //
    //Promo claim specific is for ERC721J tokens
    function promoClaimSpecific(address _contract, uint256 tokenId)
        public
        payable
        override
    {
    ...
    //
    //Promo claim song reward is for ERC721J tokens to claim a song specific reward
    function promoClaimSongReward(address _contract, uint256 tokenId) public payable {
        //Sets contract address as external nft
        ERC721J _claimContract = ERC721J(_contract);
        //Checks if the token has been claimed
        if (_tokenClaimSpecific[_contract][tokenId] != false)
    ...
    //
    //Promo claim rarity reward is for ERC721J tokens to claim a rarity specific reward
    function promoClaimRarityReward(address _contract, uint256 tokenId) public payable {
        //Sets contract address as external nft
        ERC721J _claimContract = ERC721J(_contract);
        //Checks if the token has been claimed
        if (_tokenClaimSpecific[_contract][tokenId] != false)
