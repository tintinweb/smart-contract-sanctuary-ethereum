...
contract TheGoldMaskTier2 is ERC1155, Ownable {
    ...
    uint256 public mintPrice = 0.1 ether;  // Set how much ETH for sale in wei TODO
    uint256 public tokenCount = 2000; // To ensure the users that only 3000 NFTs will exist!
    ...
    function addToWhitelist(address[] calldata toAddAddresses) external onlyOwner {
        require(whitelistOn, "The whitelisting period is over, you can't add any more addresses!");
        require(whitelistedUserCount<=90); // Max 90 whitelist spot!
    ...
    // Minting
    // function mint(uint256[] memory ids, uint256[] memory amounts) external payable {
    //     uint256 quantity = ids.length;
    //     require(ids.length == amounts.length);        
    //     require(quantity * mintPrice <= msg.value, "Insufficient funds");
    ...
    //         require(quantity <= 2, "You can mint max 2 NFT per whitelist slot!");
    ...
    //         require(ids[i] > 200, "Tier 2, first token ID is #201!");
    ...
    function salesEndMint() external onlyOwner {
        require(salesEnd, "End the sales first!");
        require(mintedTokensAll != 2000, "You already minted all you can!");

        uint256 arrayLength;
        if (mintedTokensAll<=1990) {
            arrayLength = 10;
        } else {
            arrayLength = tokenCount - mintedTokensAll;
        }
        ...
        for (uint256 i = 0; i < arrayLength; ++i) {
            mintedTokensAll += 1;
            ids[i] = mintedTokensAll;
            amounts[i] = 1;
            mintedTokens[i] = true;
        }     

        walletMints[msg.sender] += ids.length;
        _mintBatch(msg.sender, ids, amounts, "");  
    }
    ...
    function setMintPrice(uint256 newMintPrice) external onlyOwner {
        require(newMintPrice!=mintPrice, "Do not set to the same");
        require(newMintPrice > 0, "Can't be zero!");
