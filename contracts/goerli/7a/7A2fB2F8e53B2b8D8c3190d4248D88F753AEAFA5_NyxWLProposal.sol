...
contract NyxWLProposal is Ownable, converter {
    string public constant name = "Nyx Proposal - WL";
    uint256 public constant nparams = 4;

    // Enums
    ////////////////////
    enum WLAddressAction{ADD, REMOVE}
    ...
    {
        uint256 id;
        WLAddressAction action;
        address addr;
        string sn_link;
        string reason;
    }

    struct WLProposalParams
    {
        WLAddressAction action;
        address addr;
        string sn_link;
    ...
    function parseProposalParams(bytes[] memory params)
        public pure
        returns (WLProposalParams memory)
    {
        require(params.length == nparams, "wrong format");

        return WLProposalParams(WLAddressAction(converter.bytesToUint(params[0])),
            converter.bytesToAddress(params[1]),
        ...
        returns (WLProposalParams memory)
    {
        ...
        return WLProposalParams(WLAddressAction(converter.bytesToUint(param1)),
            converter.bytesToAddress(param2),
            converter.bytesToString(param3),
        ...
        returns (WLProposalParams memory)
    {
        ...
        {
            (uint256 param1, address param2, string memory param3, string memory param4) = abi.decode(params, (uint256, address, string, string));
            return WLProposalParams(WLAddressAction(param1),
                param2,
                param3,
                param4);
        }
        ...
        {
            WLProposalParams memory nullProposal;
            return nullProposal;
        }
    }

    function proposalToBytes(WLProposal memory proposal)
        public pure
        returns (bytes memory)
    {
        return abi.encode(proposal.action, proposal.addr,
            proposal.sn_link, proposal.reason);
    }
    ...
    function createProposal(uint256 proposalId, bytes[] memory params)
        ...
        returns (bytes memory)
    {
        WLProposalParams memory parsedParams = parseProposalParams(params);
        WLProposal memory proposal = WLProposal(proposalId, parsedParams.action, parsedParams.addr,
            parsedParams.sn_link, parsedParams.reason);
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        WLProposalParams memory params = parseProposalParams2(proposalBytes);
        bytes[] memory prop = new bytes[](nparams);
        prop[0] = abi.encode(params.action);
        prop[1] = abi.encode(params.addr);
        prop[2] = abi.encode(params.sn_link);
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            WLProposalParams memory params = parseProposalParams2(proposalBytesArr[idx]);
            props[idx] = [abi.encode(params.action), abi.encode(params.addr), abi.encode(params.sn_link),
                abi.encode(params.reason)];
        }
    }
}
