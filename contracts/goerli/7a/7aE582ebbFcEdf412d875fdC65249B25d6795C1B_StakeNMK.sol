...
contract StakeNMK is Pausable, Ownable, ReentrancyGuard {
    Token nmkToken;

    //30 days
    uint256 public planDuration = 30 * 24 * 60 * 60;
    //180 days
    uint256 public _planExpired = 180 * 24 * 60 * 60;
    ...
    constructor(Token _tokenAddress) {
        ...
        nmkToken = _tokenAddress;
        planExpired = block.timestamp + _planExpired;
        totalStaker = 0;
    }

    //transfer token from owner to a address which stake this token
    function transferToken(address to, uint256 amount) external onlyOwner {
        require(nmkToken.transfer(to, amount), "Token tranfer failed");
    }

    //start staking by pass amount of token as parameter
    function Stake(uint256 stakeAmount) external payable whenNotPaused {
        ...
        require(
            nmkToken.balanceOf(_msgSender()) > stakeAmount,
            "Token in your wallet is not enough to stake"
        );

        nmkToken.transferFrom(
            address(_msgSender()),
            address(this),
            stakeAmount
        );
    ...
    function claimReward() external payable returns (bool) {
        ...
        nmkToken.transfer(_msgSender(), totalAmountRecieved);
