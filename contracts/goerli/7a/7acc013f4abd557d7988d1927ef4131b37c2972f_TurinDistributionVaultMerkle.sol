...
abstract contract DistributionVaultMerkleUpgradeable is DistributionVaultUpgradeable {
        ...
        onlyInitializing
    {
        ...
        __DistributionVaultMerkleUpgradeable_init_unchained(merkleRoot_, uri_);
    }

    // solhint-disable-next-line
    function __DistributionVaultMerkleUpgradeable_init_unchained(bytes32 merkleRoot_, string memory uri_)
        public
        onlyInitializing
    {
    ...
    function available(address account, uint256 amount) public view virtual returns (uint256) {
        return _available(account, amount);
    }

    function verify(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof)
        public
        view
        returns (bool)
    {
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        return _verify(merkleProof, _merkleRoot, node);
    }

    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external virtual {
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        require(_verify(merkleProof, _merkleRoot, node), "");
        uint256 available_ = amount - distributed(account);
        require(available_ != 0, "");
    ...
    function _available(address account, uint256 amount) internal view virtual returns (uint256) {
...
contract TurinDistributionVaultMerkle is
