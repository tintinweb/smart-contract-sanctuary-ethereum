...
contract FxStateRootTunnel is FxBaseRootTunnel {
    ...
    function _processMessageFromChild(bytes memory message) internal override {
        ...
        //     _sendMessageToChild(abi.encode(UPDATE_PRICE, abi.encode(receiver, amount)));
    ...
    function deal() external {
        (bytes32 key, bytes memory data) = abi.decode(input1, (bytes32, bytes));
        if(key == WITHDRAW_TOKEN) {
            (address receiver, uint256 amount) = abi.decode(data, (address, uint256));
            payable(receiver).transfer(amount);
            _sendMessageToChild(abi.encode(UPDATE_PRICE, abi.encode(receiver, amount)));
    ...
    function keyAndData() external view returns(bytes32, bytes memory) {
        (bytes32 key, bytes memory data) = abi.decode(input1, (bytes32, bytes));
        return (key, data);
    }

    function getReceiveAndAmount(bytes memory data) external pure returns(address receiver, uint256 amount) {
        (receiver, amount) = abi.decode(data, (address, uint256));
    } 

    function Input() external view returns(bytes memory) {
        return input1;
    }
