...
import "./LibShare.sol";
import "./ExpressERC2771Context.sol";

contract PNDC_ERC721 is ERC721, ERC721Enumerable, ERC721URIStorage, ExpressERC2771Context {
    ...
    constructor(string memory name, string memory symbol, address tfGelato, address tfBiconomy)
        ERC721(name, symbol) ExpressERC2771Context(tfGelato, tfBiconomy)
    {}
    ...
    function safeMintLazyGelato(
        address to,
        string memory uri,
        LibShare.Share[] memory royalties
    ) external onlyTrustedForwarder returns(uint256){
        uint256 tokenId = _tokenIdCounter.current();
        _setRoyaltiesByTokenId(tokenId, royalties);
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    ...
    function _msgSender()
        internal
        view
        override(Context, ExpressERC2771Context)
        returns (address sender)
    {
        sender = ExpressERC2771Context._msgSender();
    }
    ...
    {
        return ExpressERC2771Context._msgData();
    }
...
contract ExpressERC2771Context is Ownable {
    mapping(address => bool) public trustedForwarders;

    constructor(address tfGelato, address tfBiconomy) {
        trustedForwarders[tfGelato] = true;
        trustedForwarders[tfBiconomy] = true;
    }
