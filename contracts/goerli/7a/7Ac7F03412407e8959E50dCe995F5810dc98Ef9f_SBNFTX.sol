...
contract SBNFTX is ERC721, Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        require(numberOfTokens <= mintsPerTx, "Amount exceeds max tokens minted per transaction");
        require(_tokenSupply.current() + numberOfTokens <= maxSupply, "Purchase would exceed max number of tokens available");
    ...
    function mintToAddress(address _to, uint256 _mintAmount) external onlyOwner {
        ...
        require(_tokenSupply.current() + _mintAmount <= maxSupply, "Can't mint more than the max supply");
