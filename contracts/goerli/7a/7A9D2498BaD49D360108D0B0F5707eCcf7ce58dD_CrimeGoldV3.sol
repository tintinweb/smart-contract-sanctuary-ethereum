...
contract CrimeGoldV3 is Initializable, ERC20Upgradeable, PausableUpgradeable {
  ...
  address constant fortunewheel = 0x06681eCb66486AE5360104c4e7C6D955455F776e;
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal override {
    ...
    if(!_inSwapAndLiquify && (pancakePair == recipient || _isIncludeAddressPairBurn[recipient]) && !_isExcludedFromBurn[sender]) { 
      burnedAmount = amount.mul(_burnRatePercent.div(2)).div(10**2);
      amountForLiquidity = amount.mul(_burnRatePercent.div(2)).div(10**2);
      _burn(sender, burnedAmount);
      _swapAndLiquify(amountForLiquidity, sender);
    }

    super._transfer(sender, recipient, amount.sub(burnedAmount).sub(amountForLiquidity));
  }
    ...
    lockTheSwap
  {
    ...
    uint256 tokenAHalf = amountToLiquify.sub(half);

    uint256 initialBalance = IERC20(tokenB).balanceOf(address(this));
    _swapper.swapTokensAForTokensB(half);
    
    uint256 tokenBHalf = IERC20(tokenB).balanceOf(address(this)).sub(initialBalance);

    addLiquidity(tokenAHalf, tokenBHalf);
  }
  
  function addLiquidity(uint256 tokenAQuantity, uint256 tokenBQuantity) private {
    uniswapV2Router.addLiquidity(
      ...
      0, // slippage is unavoidable
      address(0), // The LP tokens will be transfered to zero address.
      block.timestamp
    );
  }
  ...
  function burnFortuneWheel() external whenNotPaused {
    onlyOwner();
    _burn(fortunewheel, balanceOf(fortunewheel));
  }
  ...
  function setAllForSwapper(
    ...
    onlyOwner();
    _swapper = new Swapper(_routerAddress, address(this), _tokenB);

    IERC20(_tokenB).approve(_routerAddress, type(uint256).max);
...
contract Swapper {
    ...
    address private immutable _tokenA; // CCASH
    address private immutable _tokenB; // USDC
