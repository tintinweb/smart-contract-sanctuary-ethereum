...
contract SkeleKeys is ERC721Keys, Ownable {
    ...
    address public skRepo1;
    address public skRepo2;
    ...
    mapping(uint256 => uint256) public _keysToMintOrders;
...
//    function initRepos() private {
//        SkeleKeysRepo skRepo1Contract = new SkeleKeysRepo(address(this));
//        SkeleKeysRepo skRepo2Contract = new SkeleKeysRepo(address(this));
//
//        skRepo1 = address(skRepo1Contract);
//        skRepo2 = address(skRepo2Contract);
//
//        setRelatedApprovalForAll(skRepo1);
//        setRelatedApprovalForAll(skRepo2);
//    }
...
//        initRepos();
    ...
    function hasUnallocatedKeys() public view returns (bool) {
        return ((balanceOf(skRepo1) + balanceOf(skRepo2)) > 0);
    }
...
//        uint256 numRemaining = balanceOf(skRepo1) + balanceOf(skRepo2);
...
//            if (ownerOf(i) == skRepo1 || ownerOf(i) == skRepo2) {
                if (curIndex == spotToAllocate) {
//                    return i;
                    toReturn = i;
                }
...
//    function allocateRandomKey(address to) external onlyAuthorized {
    ...
    function _arkInternal(address to) private {
        ...
        //        address curKeyOwner = ownerOf(keyToAllocate)
        //        transferFrom(address(this), to, keyToAllocate);

//        transferFrom(ownerOf(keyToAllocate), to, keyToAllocate);
...
//            allocateRandomKey(msg.sender);
    ...
    function tryToAllocateRandomKey(address to) public onlyAuthorized {
        ...
        address curKeyRepo = ownerOf(keyToAllocate);
        address otherRepo;

        if (curKeyRepo == skRepo1) {
            otherRepo = skRepo2;
        }
        else {
            otherRepo = skRepo1;
        ...
        if ((probRandom%2 == 0) && (numAllocated < totalSupply())) {
            ...
            allocateTo = otherRepo;
        }

        allocateKeyTo(allocateTo, keyToAllocate);
    }
    */
...
contract SkeleKeysRepo is Ownable {
    address public keysContract;

    constructor(
        address keysContract_
    ) {
        keysContract = keysContract_;
    }
}
