    ...
    ReentrancyGuard
{
    ...
    function upgradeKey(uint256[] calldata _assetIds) external nonReentrant whenNotPaused {
        ...
        require(ERC721(_v1Contract).isApprovedForAll(_msgSender(), msg.sender), "Real Agency: must grant approval for upgrade");

        // two loops because execution is expensive, but validation is not too bad.
        ...
        for (uint256 i = 0; i < _assetIds.length; i++) {
            // msg.sender being called from another contract, will return this CA, _msgSender returns end user
            ERC721(_v1Contract).safeTransferFrom(_msgSender(), _v1VaultAddress, _assetIds[i]);
