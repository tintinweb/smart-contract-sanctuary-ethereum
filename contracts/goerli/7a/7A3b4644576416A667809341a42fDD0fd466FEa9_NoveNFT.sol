...
contract NoveNFT is ERC721A, Delegated {
    using Strings for uint256;
    using ECDSA for bytes32;

    // ======== SUPPLY ========
    uint256 public constant MAX_SUPPLY = 8888;
    uint public maxMintLimit = 2;

    // ======== PRICE ========
    uint256 public OGMintPrice = 0.1 ether;
    uint256 public seedListMintPrice = 0.12 ether;
    ...
    bytes32 constant HASH_1 = keccak256("OG_MINT_BATCH_1");
    bytes32 constant HASH_2 = keccak256("SEEDLIST_MINT_BATCH_2");
    address private SIGNER = 0x94614c1055A6cD11212ec7349E536000d4165385;

    // ======== CONSTRUCTOR ========
    constructor() ERC721A("Nove v.2", "NVE") {}

    // ======== MINTING OGMint Batch 1 ========
    function OGMint(bytes memory _signature, uint256 _quantity)
        external
        payable
        mintBatchCheck(1)
        ...
        maxMintLimitCheck
        callerIsUser
        priceEthCheck(OGMintPrice, _quantity)
    {
        _safeMint(msg.sender, _quantity);
    }

    // ======== MINTING seedlistMint Batch 2 ========
    function seedlistMint(bytes memory _signature, uint256 _quantity)
        external
        payable
        mintBatchCheck(2)
        ...
        maxMintLimitCheck
        callerIsUser
        priceEthCheck(seedListMintPrice, _quantity)
    {
        _safeMint(msg.sender, _quantity);
    }

    // ======== MINTING public Mint Batch 3 ========
    ...
    function setOGMintPrice(uint256 _price)
        external 
        onlyDelegates 
    {
        OGMintPrice = _price;
    ...
    function setSeedMintPrice(uint256 _price) 
        external 
        onlyDelegates 
    {
        seedListMintPrice = _price;
    }
    ...
    modifier mintBatchCheck(uint8 _mintBatch) {
        require(currentMintBatch == _mintBatch,"Nove: Incorrect mint batch");
        _;
    }

    modifier maxMintLimitCheck() {
        require(_numberMinted(msg.sender) < maxMintLimit, "Nove: Reached max limit mint");
        _;
    }
    ...
    modifier callerIsUser() {
        require(tx.origin == msg.sender, "Nove: The caller is another contract");
        _;
    }
}
