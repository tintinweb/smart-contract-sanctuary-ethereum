...
contract OcarioFeature is IFeature, IOcarioFeature, FixinCommon,RouterTokenHelper {
  ...
  function OcarioSwapCallback(
    ...
    require(deltaQty0 > 0 || deltaQty1 > 0, "OcarioFeature: invalid delta qties");
    ...
    require(
      ...
      "OcarioFeature: invalid callback sender"
    );
  ...
  function _swapExactInputInternal(
    ...
    if (recipient == address(0)) recipient = address(this);
    databytes = data.path.getFirstPool();
    // (address tokenIn, address tokenOut, uint24 fee) = data.path.decodeFirstPool();
    // ocarioPool = _getPool(tokenIn, tokenOut, fee);
  ...
  function _getPool(
    ...
    return IOcarioPool(PoolAddress.computeAddress(factory, tokenA, tokenB, fee, IFactory(factory).poolInitHash()));
  }
