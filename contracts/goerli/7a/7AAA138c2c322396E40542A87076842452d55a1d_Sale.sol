...
contract Sale {
    ...
    function purchase(
        ...
        ethValue=(msg.value)*(10**18);
        require(ethValue == _totalPrice - _vatToken, "Total Amount Mismatch");
...
contract eInvoice {
    ...
    function purchase(
        ...
        uint256 invoiceId = uint256(
            keccak256(abi.encodePacked(block.timestamp, association))
        );
        ...
        invoice.paid = true;
        if (netVatAmount != 0) {
            vat.mint(association, invoice.netVatAmount);
        }
        emit InvoiceCreated(
            msg.sender,
            invoiceId,
