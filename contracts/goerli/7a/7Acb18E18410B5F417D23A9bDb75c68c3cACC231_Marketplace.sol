...
contract Marketplace is Ownable, Pausable {
    ...
    uint256 timeIncrement = (1 minutes); // 1 hour
    uint256 defaultOfferExp = 5 * timeIncrement; // 72 hours
    ...
    function setDefaultOfferExp(uint256 exp) public onlyOwner {
        defaultOfferExp = exp * timeIncrement;
    }
    ...
    function createMarketSale(uint256 itemID) public payable whenNotPaused {
        require(
    ...
    function makeOffer(address nftContract, uint256 tokenID)
        public
        payable
        whenNotPaused
    {
        ...
        offers[nftContract][tokenID].push(
            Offer(msg.sender, msg.value, block.timestamp + defaultOfferExp)
        );
    }
    ...
    function acceptOffer(
        ...
        payable(IERC721(nftContract).ownerOf(tokenID)).transfer(
            price - (((price * fees[Index.getName(nftContract)]) / 100))
        );
    ...
    function deleteOffer(
        ...
        require(
            ...
            "MUST BE OWNER OF ASSET OR OFFERER TO DELETE OFFER"
        );
        delete offers[nftContract][tokenID][index];
        payable(offers[nftContract][tokenID][index].user).transfer(
            offers[nftContract][tokenID][index].price
