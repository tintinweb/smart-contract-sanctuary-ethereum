...
// File: contracts/ComptrollerExInterface.sol
...
contract ComptrollerExInterface {
    function liquidateCalculateSeizeTokensEx(
        ...
        uint repayAmount) external returns (uint, uint, uint);
...
/**
 * @title Onyx OTokenEx Contract
 * @author Onyx
 */
contract OTokenEx is OTokenInterface, Exponential, TokenErrorReporter {
    /**
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        (mathErr, sroTokensNew) = subUInt(accountTokens[src], 1);
        if (mathErr != MathError.NO_ERROR) {
        ...
        /////////////////////////
        // EFFEOTS & INTERAOTIONS
        // (No safe failures beyond this point)

        accountTokens[src] = sroTokensNew;
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemToken The number of oTokens to redeem into underlying
    ...
    function redeemInternal(uint redeemToken) internal nonReentrant returns (uint) {
        ...
        return redeemFreshOToken(msg.sender, redeemToken);
    }

    /**
     * @notice Sender redeems oTokens in exchange for a specified amount of underlying asset
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemToken The amount of underlying to receive from redeeming oTokens
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
     ...
     * @param redeemTokenIn The number of oTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFreshOToken(address payable redeemer, uint redeemTokenIn) internal returns (uint) {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, OTokenInterface oTokenCollateral) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
        ...
        return liquidateBorrowFresh(msg.sender, borrower, repayAmount, oTokenCollateral);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, OTokenInterface oTokenCollateral) internal returns (uint, uint) {
        /* Fail if liquidate not allowed */
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(oTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        /* Verify oTokenCollateral market's block number equals current block number */
        if (oTokenCollateral.accrualBlockNumber() != getBlockNumber()) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(oTokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");

        /* Revert if borrower collateral token balance < seizeTokens */
        require(oTokenCollateral.balanceOf(borrower) >= seizeTokens, "LIQUIDATE_SEIZE_TOO_MUCH");
        ...
        if (address(oTokenCollateral) == address(this)) {
            ...
            seizeError = oTokenCollateral.seize(liquidator, borrower, seizeTokens);
        }
        ...
        emit LiquidateBorrow(liquidator, borrower, actualRepayAmount, address(oTokenCollateral), seizeTokens);
...
// File: contracts/OErc721Delegate.sol

pragma solidity ^0.5.16;

/**
 * @title Onyx's OErc721Delegate Contract
 * @notice OTokens which wrap an EIP-721 underlying and are delegated to
 * @author Onyx
 */
contract OErc721Delegate is OErc721, ODelegateInterface {
    /**
