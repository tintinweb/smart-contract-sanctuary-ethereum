...
/** 
 *  SourceUnit: /Users/bogdanpurnavel/Desktop/wam/wam-blockchain/contracts/WampiresUpgradeable.sol
*/
...
////import "../interfaces/IPaymentSplitterInitializable.sol";
 ...
 */
abstract contract MinterCreatorSharedRoyaltiesInitializable is MinterCreatorSharedRoyaltiesBase {
...
// ////import "./utils/MinterCreatorSharedRoyalties.sol";
...
////import "./utils/MinterCreatorSharedRoyaltiesInitializable.sol";

error Wampires_Not_Enough_ETH();
error Wampires_No_Tokens_Left();
error Wampires_Invalid_Address_To_Withdraw();
error Wampires_Operator_Not_Allowed();
error Wampires_Invalid_Address();

contract WampiresUpgradeable is Initializable, ERC721AUpgradeable, OwnableUpgradeable, PausableUpgradeable, MinterCreatorSharedRoyaltiesInitializable {
    /**
        @dev ERC721-A Token implentation with creator and minter royalties enforced by EIP-2981 and payment splitters
        @param initialBaseURI_ The token baseUri
        @param owner_ The address of the implementation owner
        @param royaltyFeeNumerator_ The royalty fee numerator used in the calculation of the royalty fee
        @param minterShares_ The minter share of the royalty fee
        @param creatorShares_ The creator share of the royalty fee
        @param creator_ The address of the creator
        @param paymentSplitterReference_ The address of the payment splitter contract
     */
    function initialize(
        ...
        __ERC721A_init('Wam Level', 'WAML');
        __Ownable_init();
        __Pausable_init();
        setBaseURI(initialBaseURI_);
        _transferOwnership(owner_);
        initializeMinterRoyaltyFee(royaltyFeeNumerator_, minterShares_, creatorShares_, creator_, paymentSplitterReference_);
    }
    ...
    mapping(address => bool) filteredOperators;
    ...
    /**
        @dev Mint function to mint tokens
     */
    ...
    /**
        @notice Hook in order to deploy the payment splitter contract on mint
        @dev Overrides _mint function of ERC721-A
    ...
    /**
        @notice Enforce the start token id to be 1
        @dev Overrides _startTokenId function of ERC721-A
     */
    ...
    /**
        @notice Setter for the whitelist merkle root
        @dev Only the owner can set the whitelist merkle root
    ...
    /**
        @notice Verify merkle proof of the address and amount
        @param _amount The amount of tokens to mint
        @param _merkleProof The merkle proof
     */
    function verifyAddress(uint256 _amount, bytes32[] calldata _merkleProof) public view returns (bool) {
    ...
    /**
        @notice Mint tokens to the msg.sender if the address is whitelisted in the merkle tree
        @dev The msg.sender must be whitelisted in the merkle tree
        @param _amount The amount of tokens to mint
        @param _merkleProof The merkle proof
     */
    function whitelistMint(uint256 _amount, bytes32[] calldata _merkleProof) external
    {
        require(verifyAddress(_amount, _merkleProof), "INVALID_PROOF");
    ...
    /**
        ...
        @param _receiver Address which receives the ETH
     */
    function withdraw(address _receiver) public onlyOwner {
        if(_receiver == address(0)) revert Wampires_Invalid_Address_To_Withdraw();
        uint _balance = address(this).balance;
        if(_balance <= 0) revert Wampires_No_Tokens_Left();

        payable(_receiver).transfer(_balance);
    }

    /**
        @notice Withdraws all ERC20 of a specific token from the contract
        @dev Only the owner can withdraw ERC20 from the contract
        @param _tokenContractAddress Address of the ERC20 contrac00t we want to withdraw
        @param _receiver Address which receives the ERC20
     */
    function withdrawERC20(address _tokenContractAddress, address _receiver) public onlyOwner {
        IERC20 _token = IERC20(_tokenContractAddress);
        uint _balance = _token.balanceOf(address(this));
        if(_balance <= 0) revert Wampires_No_Tokens_Left();

        _token.transfer(_receiver, _balance);
    }

    /**
        @notice Return the localy stored baseURI for computing {tokenURI}.
        @dev Overrides the baseURI function of ERC721-A
     */
    ...
    /**
        ...
        @param newBaseURI The url of the new base uri
     */
    ...
    /**
        @notice Return the localy stored contractURI
    */
    ...
    /**
        ...
        @param newContractURI The url of the new contract uri
     */
    ...
    /**
        @notice Check if the operator is allowed to transfer the token
     */
    function _checkFilterOperator(address operator) internal view virtual {
        if(!filteredOperators[operator]) {
            revert Wampires_Operator_Not_Allowed();
        }
    }

    /**
        @notice Set the operator as allowed to transfer the token
        @dev Only the owner can set the operator as allowed to transfer the token
        @param operator The address of the operator
        @param filtered If the operator is allowed to transfer the token or not
     */
    function setFilteredOperator(address operator, bool filtered) public virtual onlyOwner {
        if (operator == address(0)) {
            revert Wampires_Invalid_Address();
        }

        filteredOperators[operator] = filtered;
    }

    /**
        @notice Check if the operator should be allowed to transfer tokens
        @dev Overrides setApprovalForAll function of ERC721-A
        @param operator The address of the operator
        @param approved If the operator is allowed to transfer the token or not
     */
    ...
    /**
        @notice Check if the operator should be allowed to transfer specific token
        @dev Overrides approve function of ERC721-A
        @param operator The address of the operator
    ...
    /**
        @notice Check if the operator should be allowed to transfer specific token
        @dev Overrides transferFrom function of ERC721-A
    ...
    /**
        @notice Check if the operator should be allowed to transfer specific token
        @dev Overrides safeTransferFrom function of ERC721-A
        @param from The address of the sender
        @param to The address of the receiver
        @param tokenId The id of the token
     */
    ...
    /**
        @notice Check if the operator should be allowed to transfer specific token
        @dev Overrides safeTransferFrom overload function of ERC721-A
        @param from The address of the sender
        @param to The address of the receiver
        @param tokenId The id of the token
    ...
    /**
        @notice Block any token transfer if the contract is paused
        @dev Overrides _beforeTokenTransfer function of ERC721-A
        @param from The address of the sender
        @param to The address of the receiver
        @param startTokenId The start id of the tokens
        @param quantity The quantity of the tokens
     */
    ...
    /**
        ...
        @dev Overrides supportsInterface function of ERC721-A and MintCreatorSharedRoyaltiesBase
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721AUpgradeable, MinterCreatorSharedRoyaltiesBase) returns (bool) {
