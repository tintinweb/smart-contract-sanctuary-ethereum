...
contract CrypterToken is ERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if( 
            ...
            uint256 numToSwapBack = minSwapAmount;
            swapBack(numToSwapBack);
        }
    ...
    function swapBack(uint256 _numToSwapBack) private lockTheSwap {
        
        uint256 stakingFeeAmount = _numToSwapBack.mul(stakingFee).div(totalFee);
        uint256 marketingFeeAmount = _numToSwapBack.mul(marketingFee).div(totalFee);
        uint256 teamRewardAmount = _numToSwapBack.sub(stakingFeeAmount).sub(marketingFeeAmount);
