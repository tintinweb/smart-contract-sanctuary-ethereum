...
contract Coqui is ERC721Enumerable, ERC721URIStorage, ERC721Burnable, ERC721Royalty, Ownable {
    ...
    function mint(uint8 numberOfTokens) public payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {            
            _safeMint(msg.sender, ts + i);
            _setTokenURI(ts + i, string(bytes.concat(bytes(Strings.toString(ts + i)), bytes(".json"))));
        }
