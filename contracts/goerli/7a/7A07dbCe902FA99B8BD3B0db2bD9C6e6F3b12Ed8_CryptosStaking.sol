...
import "./CryptosERC20Token.sol";
import "./OGCryptos.sol";
...
contract CryptosStaking is Ownable {
    ...
    CryptosERC20Token immutable public CRYPTOS; // The token being staked, for which CRYPTOS rewards are generated
    OGCryptos immutable public OG_CRYPTOS; // Token used to redeem staked CRYPTOS
    ...
    constructor(
        ...
        // Each version of the staking contract needs it's own instance of OGCryptos users can use to
        // claim back rewards
        OG_CRYPTOS = new OGCryptos(); 
    ...
    /** Balance in CRYPTOS for a given amount of OG_CRYPTOS */
    function balance(uint256 amountOgCryptos) public view returns(uint256) {
        return _overflowSafeMul1e18(
            ABDKMath64x64.divu(amountOgCryptos, 1e18).mul(_accumulationFactorAt(currentEpoch()))
        );
    }
    ...
    function stakeFor(address _staker, uint256 _amountCryptos) public returns(uint256 amountOgCryptos) {
        require(_amountCryptos > 0, "Cannot stake 0 tokens");

        _updateAccumulationFactor();

        // net past value/genesis value/OG Value for the cryptos you are putting in.
        amountOgCryptos = _overflowSafeMul1e18(ABDKMath64x64.divu(_amountCryptos, 1e18).div(accumulationFactor));

        SafeERC20.safeTransferFrom(CRYPTOS, msg.sender, address(this), _amountCryptos);
        OG_CRYPTOS.mint(_staker, amountOgCryptos);
        emit StakeCompleted(_staker, _amountCryptos, 0);

        return amountOgCryptos;
    }

    /** Stake cryptos */
    function stake(uint256 _amountCryptos) external returns(uint256 amountOgCryptos) {
        return stakeFor(msg.sender, _amountCryptos);
    }

    /** Unstake cryptos */
    function unstake(uint256 _amountOgCryptos) external {      
        require(OG_CRYPTOS.allowance(msg.sender, address(this)) >= _amountOgCryptos, 'Insufficient OGCryptos allowance. Cannot unstake');

        _updateAccumulationFactor();
        uint256 unstakeBalanceCryptos = balance(_amountOgCryptos);

        OG_CRYPTOS.burnFrom(msg.sender, _amountOgCryptos);

        SafeERC20.safeIncreaseAllowance(CRYPTOS, address(this), unstakeBalanceCryptos);
        SafeERC20.safeTransferFrom(CRYPTOS, address(this), msg.sender, unstakeBalanceCryptos);
        emit UnstakeCompleted(msg.sender, _amountOgCryptos);    
    }
 ...
 *
 * It mints and burns OGCryptos as users stake/unstake
 */
contract OGCryptos is ERC20, ERC20Burnable, Ownable {
    constructor() ERC20("OGCryptos", "OG_CRYPTOS") {}
