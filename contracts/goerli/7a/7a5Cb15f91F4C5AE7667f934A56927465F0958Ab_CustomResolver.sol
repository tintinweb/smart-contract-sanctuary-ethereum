...
import {IOwnedResolver} from "./interfaces/IOwnedResolver.sol";
...
contract CustomResolver is ICustomResolver {
    ...
    function resolverAddress() external view returns (address) {
        ...
        IOwnedResolver ownedResolver = IOwnedResolver(
            ens.resolver(ethNameHash)
        );
        return
            ownedResolver.interfaceImplementer(
                ifragNameHash,
                type(ICustomResolver).interfaceId
            );
    }
}
