...
contract Etherwriter is Context, ERC721Burnable {
	...
	constructor() ERC721("Etherwriter", "EWR", "Four lines, fourty characters each of censorship-resistant microblogging.") {
	...
	function _determineColor(uint256 _tokenId) internal pure returns(string memory) {
		string memory color = "#888";
		if(_tokenId == 0) { // Unique color, bright red.
			color = "#FF1900";
		}
		else if(_tokenId > 0 && _tokenId < 5001) {//1 to 5000, light gray.
			color = "#888";
		}
		else if(_tokenId > 5000 && _tokenId < 9901) {// 5001 to 9900, amber.
			color = "#FFBF00";
		}
		else if(_tokenId > 5000 && _tokenId < 10000) {// 9901 to 9999, phosphor green.
			color = "#19FF00";
		}
	...
	/// @dev Mint a new token with the specified lines of text.
	...
	/// @dev Pauses the contract, preventing minting.
	///      Note: Only contract's owner can execute this.
