...
contract GainERC20Tokenv3test019 is ERC20Upgradeable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeMathInt for int256;
    ...
    *
    * @notice - The Current Ratio, Target, and Stretch are scaled up to the thousands for viewing and decimal conversion purposes and then scaled down
    * for the rebase calculation. Example - If the Current Ratio is publicly displayed as 2000, Target as 1100, and Stretch as 1000, this means that  
    ...
    */
    ...
    modifier onlyAuthorised() {
        require(msg.sender == LiqStakingContract || msg.sender == owner() , "GainTokenv3: not allowed to use this method");
        _;
    }
    ...
    function initialize() public initializer {
        __ERC20_init("Gainv3t019","GAINv3t019");
        __Ownable_init();
    ...
    require(enableRebaseWhitelist, "GainTokenv3: Whitelist is not enabled.");
    // Check if msg.sender is the owner or is whitelisted
    require(msg.sender == owner() || whitelisted[msg.sender], "GainTokenv3: You are not authorized to execute this function.");
    require(
        rebaseLockStatus &&
            (lastRebaseTimestampSec + intervalRebasePeriod) <=
            block.timestamp,
        "GainTokenv3: You can not rebase"
    );
        ...
        if (scaleDown(CurrentRatio()) > scaleDown(Target)) {
            uint256 percentage = (((scaleDown(CurrentRatio()) - scaleDown(Target)) / (scaleDown(Target))) * 10**20);
            ...
            _supplyDelta = int256((_data / scaleDown(Stretch)) / 10**20);
            if (_supplyDelta != 0) {
                _totalSupply = _totalSupply.add(uint256(_supplyDelta));
            }
        }
        if (scaleDown(CurrentRatio()) < scaleDown(Target)) {
            uint256 percentage = (((scaleDown(Target) - scaleDown(CurrentRatio())) / (scaleDown(Target))) * 10**20);
    ...
    function safeTransfer(address from, address to, uint256 amount) external virtual onlyAuthorised {
        require(_allowances[from][to] >= amount, "GainTokenv3: transfer amount exceeds allowance");
        unchecked {
