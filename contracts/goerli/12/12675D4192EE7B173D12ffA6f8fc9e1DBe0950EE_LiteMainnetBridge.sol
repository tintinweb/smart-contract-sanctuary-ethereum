...
contract AdminModule is VariablesV1, Events {
    ...
    function toggleRootToChildVaultMap(
        ...
        // TODO: naming not proper rootToChildVault -> rootToChildVault
        if (add) {
            require(rootToChildVault[rootVault] == address(0), "LBM:[toggleRootToChainMap]:: Root to Child Mapping already added");
            rootToChildVault[rootVault] = childVault;
        } else {
            require(rootToChildVault[rootVault] != address(0), "LBM:[toggleRootToChainMap]:: Root to Child Mapping not added");
            delete rootToChildVault[rootVault];
        }
...
contract LiteMainnetBridge is AdminModule {
    ...
    function processFromPolygon(
        ...
        depositToVault(vault, token, amount, oneInchSwapCalldata);
    ...
    function depositToVault(
        ...
        if (token == NATIVE_TOKEN) {
            ...
            if (uint256(bytes32(oneInchSwapCalldata)) > 0) {
                uint256 stEthBalance_ = stETH.balanceOf(address(this));

                // MOCK MOCK MOCK MOCK MOCK
                Address.functionCallWithValue(oneInchAddress, abi.encodeWithSignature("swapFrom(uint256,address)", amount_, vault), amount_, "LBM:[withdraw]:: steth-1inch-swap-failed");
                // MOCK MOCK MOCK MOCK MOCK
                    
                Address.functionCallWithValue(oneInchAddress, oneInchSwapCalldata, amount_, "LBM:[depositToVault]:: steth-1inch-swap-failed");

                uint256 stEthBalanceAfterSwap_ = stETH.balanceOf(address(this));
                uint256 stEthAmountFromSwap_ = stEthBalanceAfterSwap_ - stEthBalance_;

                require(stEthAmountFromSwap_ - 100_000 > amount_, "eth-steth-high-depeg"); // 100k wei difference is allowed

                amount_ = stEthBalanceAfterSwap_;

                stETH.safeApprove(vault, amount_);
                iTokenAmount_ = IiTokenVault(vault).supply(address(stETH), amount_, address(this));
            } else {
                iTokenAmount_ = IiTokenVault(vault).supplyEth{value: amount_}(address(this));
            }
        } else {
            IERC20(token).safeApprove(vault, amount_);
            iTokenAmount_ = IiTokenVault(vault).supply(token, amount_, address(this));
        }
    ...
    function updateExchangePrice(
        ...
        require(rootToChildVault[rootVault] == childVault, "LBM:[updateExchangeRate]:: root to child are not same");
    ...
    function batchUpdateExchangePrice(
    ...
    function withdrawToPolygon(
        ...
        require(rootToChildVault[rootVault] == childVault, "LBM:[withdraw]:: root to child are not same");
        ...
        if(token == NATIVE_TOKEN) {
            ...
            if (stETHBalance_ > 0) {
                ...
                // MOCK MOCK MOCK MOCK MOCK
                Address.functionCall(oneInchAddress, abi.encodeWithSignature("swapTo(uint256,address)", stETHBalance_, rootVault), "LBM:[withdraw]:: steth-1inch-swap-failed");
                // MOCK MOCK MOCK MOCK MOCK
...
contract VariablesV1 is Common, Initializable, OwnableUpgradeable {
    ...
    */
    ...
    mapping (address => address) public rootToChildVault;
