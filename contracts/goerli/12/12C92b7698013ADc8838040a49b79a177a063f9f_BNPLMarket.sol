    ...
    IBNPLMarket
{
    ...
    uint256 public immutable UPGRADE_VERSION = 3;
    ...
    uint256 public version; //used for the onUpgrade method alongside UPGRADE_VERSION
    ...
    event CreatedDiscreteOrder(uint256 bidId, address indexed borrower);
    ...
    */
    function onUpgrade(
        ...
        require(version != UPGRADE_VERSION, "already upgraded");
    ...
    function submitDiscreteOrder(
        ...
        uint256 _quantity
    ) public payable returns (uint256 bidId) {
        require(
            msg.value == _submitBidArgs.downPayment,
            "Insufficient downpayment."
        );

        bidId = _submitTellerV2Bid(_submitBidArgs, wethAddress);

        discreteOrders[bidId] = DiscreteOrder({
            borrower: _msgSender(),
            totalPurchasePrice: _submitBidArgs.totalPurchasePrice,
    ...
    function acceptDiscreteOrder(
        ...
        DiscreteOrder storage order = discreteOrders[_bidId];
        ...
        require(order.lender == address(0), "Order already accepted");
        order.lender = _msgSender();

        _acceptBidAndPurchaseNFT(
            _basicOrderParameters,
            _bidId,
            order.totalPurchasePrice,
            order.lender
        );
    }
     ...
     * @param _purchasePrice The price of the NFT needed to purchase.
    ...
    function _acceptBidAndPurchaseNFT(
        ...
        _acceptTellerV2Bid(_bidId, _lender);
        _purchaseNFT(_basicOrderParameters, _purchasePrice);
