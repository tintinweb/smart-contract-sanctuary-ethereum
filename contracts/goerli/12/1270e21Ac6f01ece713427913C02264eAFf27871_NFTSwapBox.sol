    ...
    ERC1155Holder
{
    ...
    constructor(
        ...
        NFTSwapBoxAssets = boxAssets;
        NFTSwapBoxHistory = boxHistory;
    ...
    function _transferSwapFees(
        ...
        INFTSwapBoxAssets(NFTSwapBoxAssets)._transferERC20Fee(swapBoxes[boxID].erc20Fees, NFTSwapBoxAssets, to, false);
        if(!swapped) {
            for(uint256 i = 0; i < swapBoxes[boxID].boxRoyaltyFee.length; i++){
                payable(to).transfer(swapBoxes[boxID].boxRoyaltyFee[i].feeAmount);
            }
        } else {
            for(uint256 i = 0; i < swapBoxes[boxID].boxRoyaltyFee.length; i++){
    ...
    function _caculateSwapFees(
        ...
        swapBoxes[boxID].nftSwapFee = INFTSwapBoxFees(NFTSwapBoxFees)._checkNFTFee(
        ...
        swapBoxes[boxID].gasTokenFee = INFTSwapBoxFees(NFTSwapBoxFees)._checkGasTokenFee(
    ...
    function createBox(
        ...
        uint256 createFees = INFTSwapBoxFees(NFTSwapBoxFees)._checkCreatingBoxFees(
            swapConstantFees[0],
            erc721Details,
            erc20Details,
            erc1155Details,
    ...
    function toWaitingForOffers(uint256 boxID, address[] memory offerAddress, bool whiteListOffer)
        public
        payable
        isOpenForSwap
        nonReentrant
    {
        ...
        require(
            msg.value == swapConstantFees[1] + swapBoxes[boxID].nftSwapFee + swapBoxes[boxID].gasTokenFee + boxroyaltyFees, "Insufficient Listing Fee");
        ...
        if(whiteListOffer == true)
        {
            INFTSwapBoxAssets(NFTSwapBoxAssets)._setOfferAdddress(boxID, offerAddress);
            swapBoxes[boxID].whiteListOffer = true;
        }
        ...
        nonReentrant
    {
        ...
        if(swapBoxes[listBoxID].whiteListOffer == true)
            require(INFTSwapBoxAssets(NFTSwapBoxAssets)._checkAvailableOffer(listBoxID, msg.sender) == true, "Not listed Offer Address");
        ...
        nonReentrant
    {
        ...
        INFTSwapBoxAssets(NFTSwapBoxAssets)._deleteOfferAddress(listBoxID);
        ...
        swapBoxes[listBoxID].whiteListOffer = false;
