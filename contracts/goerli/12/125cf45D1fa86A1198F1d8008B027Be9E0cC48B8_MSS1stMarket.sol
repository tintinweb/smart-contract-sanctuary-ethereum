...
abstract contract MarketCore is Constants {
  ...
  mapping(address => mapping(uint256 => bool)) private _nftContractToTokenIDToFirstSaleCompleted; // Track if there has been a sale for the NFT in this market previously.
...
****************************************************************************************************/
  function _getMinIncrement(uint256 currentAmount) internal pure returns (uint256) {
    uint256 minIncrement = currentAmount * MIN_PERCENT_INCREMENT_IN_POINTS;
    unchecked {
...
****************************************************************************************************/
  struct AuctionData {
    ...
    uint256 startTime; // 경매 시작 시간 Unix timestamp 형태로 저장. 0이면 사용 안함
    uint256 duration; // 첫 비딩 이후 경매 기간 설정 값
    uint256 endTime; // The time at which this auction will not accept any new bids. This is `0` until the first bid is placed.
    uint16 marketFee; // 마켓 수수료
  ...
  uint256 private nextAuctionID; // A global id for auctions of any type.

  mapping(address => mapping(uint256 => uint256)) private nftContractTokenIDToAuctionID; // The auction configuration for a specific auction id.
  mapping(uint256 => AuctionData) private auctionIDToAuction; // The auction id for a specific NFT. This is deleted when an auction is finalized or canceled.
...
* @dev 이 동작을 하기 전 NFT 소유주는 마켓이여야 합니다.
...
****************************************************************************************************/
  function createAuction(address nftContract, uint256 tokenID, address seller, uint256 auctionPrice, uint256 startUnixTime, uint256 duration, uint16 marketFee) external onlyAdmin {
    ...
    emit AuctionCreated(seller, nftContract, tokenID, duration, EXTENSION_DURATION, auctionPrice, auctionID); // tokenCreator 확인 필요 인터페이스 없으면 address(0)됨
  }
...
****************************************************************************************************/
  struct FixedPrice {
    ...
    uint96 price;           // The current buy price set for this NFT.
    uint16 marketFee;       // 마켓 수수료
  }
...
abstract contract Constants {
  ...
  uint16 internal constant MIN_PERCENT_INCREMENT_IN_POINTS = 1000;
