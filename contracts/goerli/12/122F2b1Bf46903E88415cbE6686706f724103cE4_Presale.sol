...
contract Presale is Ownable {
    ...
    // uint256[] public rLockinPeriod = [0,90,180,270,360];
    uint256[] public rLockinPeriod = [0,9,18,27,36];

    uint256[] public priceBrackets = [5000,50000,200000];
    uint256[] public pricePerToken = [8,7,5];
    ...
    uint256 public minEthLimit = 3000000000000000000;
    ...
    function getEthPriceInUsd() public view returns(int256) {
        return 100000;//(priceFeed.latestAnswer()/1e8);
    ...
    function setLaunchDate(uint256 _launchDate) public onlyOwnerAndAdmin{
    ...
    function setTokenRatePerEth(uint256 rate) external onlyOwnerAndAdmin {
    ...
    function buyToken(uint256 _amountInUSD) public payable returns (address) {
        require(isPresaleOpen, "Presale is not open.");
        require(getLaunchedAt() != 0,"Not yet launched");

        // require(usersInvestments[msg.sender].add(msg.value) <= maxEthLimit &&
        //         usersInvestments[msg.sender].add(msg.value) >= minEthLimit,"Installment Invalid.");
    ...
    function getTokens(uint256 _amountInUSD) public view returns(uint256 _pricePerToken, uint256 tokenAmount) {
        ...
        return (per, per*_amountInUSD*10**(tokenDecimals-rateDecimals));
        ...
        returns (uint256 range,uint256 _slicePeriod)
    {
      ...
      _slicePeriod=0;
        ...
        else if(amount <= priceBrackets[4]){
