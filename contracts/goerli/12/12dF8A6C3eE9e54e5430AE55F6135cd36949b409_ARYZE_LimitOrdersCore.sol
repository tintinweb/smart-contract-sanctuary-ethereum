...
// Upgraded
contract ARYZE_LimitOrdersCore is Order, Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    receive() external payable {
        require(msg.sender != tx.origin, "ARYZE#receive: NO_SEND_ETH_PLEASE");
    }
    ...
    function depositEth(bytes calldata _data) external payable {
        require(msg.value > 0, "ARYZE#depositEth: VALUE_IS_0");
        ...
        require(inputToken == ETH_ADDRESS, "ARYZE#depositEth: WRONG_INPUT_TOKEN");

        bytes32 key = keyOf(module, inputToken, owner, witness, data);
    ...
    function cancelOrder(
        ...
        require(msg.sender == _owner, "ARYZE#cancelOrder: INVALID_OWNER");
        ...
        emit OrderCancelled(key, _inputToken, _owner, _witness, _data, amount);
    }
    ...
    function encodeTokenOrder(
        ...
        return
            abi.encodeWithSelector(
                IERC20(_inputToken).transfer.selector,
    ...
    function executeOrder(
        ...
        if (address(_inputToken) == ETH_ADDRESS) {
            ...
            ethDeposits[key] = 0;
            (bool success, ) = _module.call{value: amount}("");
            require(success, "_pullOrder: PULL_ETHER_FAILED");
        } else {
            amount = key.executeVault(_inputToken, _module);
        }

        // Pull amount
        //  uint256 amount = _pullOrder(_inputToken, key, payable(address(_module)));
        require(amount > 0, "ARYZE#executeOrder: INVALID_ORDER");

        uint256 bought = IModule(_module).execute(_inputToken, amount, _owner, _data, _auxData);

        emit OrderExecuted(key, _inputToken, _owner, witness, _data, _auxData, amount, bought);
    }
    ...
    function existOrder(
        ...
        if (address(_inputToken) == ETH_ADDRESS) {
            ...
            return IERC20(_inputToken).balanceOf(key.getVault()) != 0;
        }
    }
    ...
    function canExecuteOrder(
        ...
        bytes32 key = keyOf(_module, _inputToken, _owner, _witness, _data);
        address vault = keyOf(_module, _inputToken, _owner, _witness, _data).getVault();
        ...
        if (address(_inputToken) == ETH_ADDRESS) {
            ...
            amount = IERC20(_inputToken).balanceOf(vault);
        }

        return IModule(_module).canExecute(_inputToken, amount, _data, _auxData);
    }
...
library Fabric {
    ...
    */
    ...
    function executeVault(
        ...
        value = IERC20(_token).balanceOf(addr);
        /* solium-disable-next-line */
        (bool success, ) = addr.call(
            abi.encodePacked(abi.encodeWithSelector(IERC20(_token).transfer.selector, _to, value), address(_token))
        );
