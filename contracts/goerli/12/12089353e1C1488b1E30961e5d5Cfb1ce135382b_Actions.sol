...
abstract contract AbstractPool is IERC20, DesynOwnable {
    ...
    function joinPool(uint poolAmountOut, uint[] calldata maxAmountsIn, address kol) external virtual;
...
abstract contract RebalaceAdapter {
    ...
    function isRouterApproved(address) external virtual returns (bool);
}
...
abstract contract IConfigurableRightsPool is AbstractPool {
    ...
    function createPool(uint initialSupply, uint collectPeriod, Period period, PoolTokenRange memory tokenRange) external virtual;
    ...
    function vaultAddress() external view virtual returns (address);
...
contract Actions {
    ...
    function joinPool(LiquidityPoolActions pool, uint poolAmountOut, uint[] calldata maxAmountsIn) external {
        address[] memory tokens = pool.getFinalTokens();
        _join(pool, tokens, poolAmountOut, maxAmountsIn, msg.sender);
    }

    function joinSmartPool(IConfigurableRightsPool pool, uint poolAmountOut, uint[] calldata maxAmountsIn, address kol) external {
        address[] memory tokens = pool.bPool().getCurrentTokens();
        _join(pool, tokens, poolAmountOut, maxAmountsIn, kol);
    }

    function exitPool(IConfigurableRightsPool pool, uint poolAmountIn, uint[] memory minAmountsOut) external {
        address[] memory tokens = pool.bPool().getCurrentTokens();
    ...
    function autoJoinSmartPool(
        ...
        _join(pool, poolTokens, poolAmountOut, maxAmountsIn, kol);
    }

    /// @notice Internal function to execute swap
    /// @param swapInfo Key information to perform rebalance
    function _makeSwap(RebalaceAdapter rebalaceAdapter, IAggregator.SwapInfo calldata swapInfo) public returns (uint256 postSwap) {
        require(rebalaceAdapter.isRouterApproved(swapInfo.aggregator), "ROUTER_NOT_APPROVED");

        IERC20(swapInfo.token0).approve(swapInfo.aggregator, swapInfo.quantity);

        uint256 preSwap = IERC20(swapInfo.token1).balanceOf(address(this));

        if (swapInfo.swapType == IAggregator.SwapType.UNISWAPV3) {
            (uint256 minReturn, uint256[] memory pools) = abi.decode(swapInfo.data, (uint256, uint256[]));
            IAggregator(swapInfo.aggregator).uniswapV3Swap(swapInfo.quantity, minReturn, pools);
        } else if (swapInfo.swapType == IAggregator.SwapType.UNISWAPV2) {
            (uint256 minReturn, address[] memory paths) = abi.decode(swapInfo.data, (uint256, address[]));
            IAggregator(swapInfo.aggregator).swapExactTokensForTokens(swapInfo.quantity, minReturn, paths, address(this), SafeMath.add(block.timestamp, 1800));
        } else {
            Address.functionCallWithValue(swapInfo.aggregator, swapInfo.data, 0);

            // _validateData(swapInfo.quantity, swapInfo.data, address(this));
        }

        postSwap = SafeMath.sub(IERC20(swapInfo.token1).balanceOf(address(this)), preSwap);
    }

    // --- Private pool management ---

    function setTokens(LiquidityPoolActions pool, address[] calldata tokens, uint[] calldata balances, uint[] calldata denorms) external {
        require(tokens.length == balances.length, "ERR_LENGTH_MISMATCH");
    ...
    function _join(AbstractPool pool, address[] memory tokens, uint poolAmountOut, uint[] memory maxAmountsIn, address kol) internal {
    ...
    function _exit(AbstractPool pool, uint poolAmountIn, uint[] memory minAmountsOut, address[] memory tokens) internal {
