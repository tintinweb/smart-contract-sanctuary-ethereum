...
contract TPTokenFactory is Ownable2Step {
    ...
    constructor(address newOwner) {
        implementation = address(new TPToken());
        _transferOwnership(newOwner);
    }
        ...
        returns (address)
    {
        bytes memory initializeCalldata =
            abi.encodeWithSelector(TPToken.initialize.selector, manager, owner, vault, maxSupply, name, symbol);
...
interface ITPManager {
    ...
    function didFactoryWhitelist(address account) external;
}