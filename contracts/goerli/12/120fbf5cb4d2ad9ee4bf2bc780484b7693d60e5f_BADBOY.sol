...
contract BADBOY is FlashLoanSimpleReceiverBase {
    ...
    address private constant DAI = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619; ////// weth MAINNET
    // address private constant DAI = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
    // address private constant WETH = 0x2e3A2fb8473316A02b8A297B982498E661E1f6f5;  // GOERIL
    ...
    function executeOperation(
        ...
        IERC20(DAI).approve(address(UNISWAP_ROUTER), MAX_INT);
        IERC20(DAI).approve(address(SUSHISWAP_ROUTER), MAX_INT);
        IERC20(DAI).approve(address(QUICKSWAP_ROUTER), MAX_INT);
        IERC20(DAI).approve(address(UNISWAP_FACTORY), MAX_INT);
        IERC20(DAI).approve(address(SUSHISWAP_FACTORY), MAX_INT);
        IERC20(DAI).approve(address(QUICKSWAP_FACTORY), MAX_INT);
