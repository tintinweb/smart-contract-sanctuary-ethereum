...
contract UniversalRegistrarControllerV3 is Ownable, BaseUniversalRegistrarControllerV2, Access {
    ...
    // Whether a commitment is required for the TLD
    mapping(bytes32 => bool) public commitmentOnly;
    ...
    function setCommitmentOnly(bytes32 node, bool commitmentOnly_) public nodeOperator(node) {
        commitmentOnly[node] = commitmentOnly_;
    }

    function registerNow(bytes32 node, string memory name, address owner,
        uint duration, address resolver, address addr) public payable {
        require(!commitmentOnly[node], "this node requires commitments");
        bytes32 label = keccak256(bytes(name));
        ...
        // Set this contract as the (temporary) owner, giving it
        // permission to set up the resolver.
        uint expires = base.register(node, label, address(this), duration);
...
//        Resolver(resolver).setAddr(nodehash, addr);
        ...
        // TODO: figure out how to deal with extra payments in case msg.sender is not owner
//        if (msg.value > cost) {
//            payable(msg.sender).transfer(msg.value - cost);
//        }
    }
