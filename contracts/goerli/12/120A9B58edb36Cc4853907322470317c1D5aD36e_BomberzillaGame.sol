...
contract BomberzillaGame is AccessControl, ERC721Holder, ERC1155Holder, Ownable {
    ...
    uint256 public minValidSignersCount = 3;
    ...
    mapping(address => mapping(TokenStandard => mapping(uint256 => uint256))) public balances; // user address => tokenStandard => tokenid => balance
    ...
    event BatchClaimed(address indexed account, uint256[] nonces);
    ...
    modifier notBlacklisted() {
        require(!blacklist[msg.sender], "GameSystem: blacklisted sender");
        _;
    }

    constructor(uint256 _claimDelay, uint256 _minSigner) {
        claimDelay = _claimDelay;
        minValidSignersCount = _minSigner;
        uint256 chainId;
        assembly {
            chainId := chainid()
        }
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("Game Token System")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
        ...
        grantRole(CLAIM_SIGNER_ROLE, msg.sender);
    }
    ...
    function cancelNonce(uint256 nonce) public notBlacklisted {
        require(!claimed[msg.sender][nonce], "GameSystem: Nonce already claimed by Sender");
    ...
    function claim(TokenClaimData calldata _claimData) public notBlacklisted {
        require(!emergencyLock, "GameSystem: Contract is locked");
        require(_claimData.signatureTimestamp + claimDelay <= block.timestamp, "GameSystem: Claim too soon!");
        require(_claimData.deadline >= block.timestamp, "GameSystem: Signature is Expired");
        require(
            balances[msg.sender][_claimData.tokenStandard][_claimData.tokenId] >= _claimData.amount,
            "GameSystem: Not enough balance"
        );
        require(!cancelled[msg.sender][_claimData.nonce], "GameSystem: Nonce Cancelled by Sender");
        require(!claimed[msg.sender][_claimData.nonce], "GameSystem: Nonce Claimed by Sender");
        ...
        cancelled[msg.sender][_claimData.nonce] = true;
        balances[msg.sender][_claimData.tokenStandard][_claimData.tokenId] -= _claimData.amount;
        emit TokenClaimed(
            _claimData.token,
            msg.sender,
    ...
    function validateSignatures(bytes32 digest, Signature[] calldata _signature) private view {
        ...
        require(validSignerCount != 0, "GameSystem: Invalid Signatures");
        require(minValidSignersCount <= validSignerCount, "GameSystem: Not Enough Valid Signatures");
    }
    ...
    function deposit(TokenDepositData calldata _depositData) public notBlacklisted {
        if (TokenStandard(_depositData.tokenStandard) == TokenStandard.ERC20) {
            ...
            // token.approve(msg.sender, _depositData.amount);
        ...
        balances[msg.sender][_depositData.tokenStandard][_depositData.tokenId] += _depositData.amount;
        emit TokenDeposited(
            _depositData.token,
            msg.sender,
    ...
    function grantRoleBatch(bytes32 role, address[] calldata users) public onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
            grantRole(role, users[i]);
        }
    }

    function revokeRoleBatch(bytes32 role, address[] calldata users) public onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
    ...
    function updateEmergencyLock(bool status) public onlyOwner {
