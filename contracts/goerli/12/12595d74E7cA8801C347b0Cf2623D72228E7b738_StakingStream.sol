...
contract StakingStream is ERC20, Ownable, ReentrancyGuard {
    ...
    function withdrawProfit(WithdrawProfitData memory _withdrawProfitData) external nonReentrant onlyOwner {
        ...
        stakingToken.safeTransfer(_withdrawProfitData.user, _withdrawProfitData.amount);
    ...
    function calculateAvailableProfit(uint256 user) public view returns(uint256) {
        ...
        for(uint256 i= 0; i<userCurrentstakings.length; i++) {
            ...
            if(!stakeRef.unstaked && stakeRef.stakedTimestamp + uint48(1) * 86400 > getCurrentTime()) {
                availableRewards += stakeRef.dailyProfit;
            }
        }
