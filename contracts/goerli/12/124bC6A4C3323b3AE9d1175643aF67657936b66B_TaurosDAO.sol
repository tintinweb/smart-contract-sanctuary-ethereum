...
contract TaurosDAO is ERC721Enumerable, ERC721Burnable, Ownable {
  ...
  function isPublicSale(bool _public) public onlyOwner {
    isPublic = _public;
  }

  function isPrivateSale(bool _private) public onlyOwner {
    isPrivate = _private;
  }
    ...
    function claimTauros(uint256 _count) public payable {
        ...
        require(_count > 0, "Please mint 1 NFT.,.");
        require(_count <= maxMintAmount, "Cannot exceed this mint amount,..");
        require(supply + _count <= maxSupply, "NFT limit exceeded..,");

        if (msg.sender != owner()) {
            if(onlyWhitelisted == true) {
                // require(isWhitelisted(msg.sender), "You are not whitelisted... Please purchase a TaurosDAO");
        ...
        for (uint256 i = 1; i <= _count; i++) {
            // PRICE must increment every 500 minted - testing this logic - can be modifier/or function
            // 5 mints set for testing purposes
            uint256 currentCount = _tokenIds.current();
            if (currentCount >= 1000 && currentCount <= 2000 ) PRICE = 0.08 ether;
            _mintSingleNFT();
        }
    }
    ...
    function whitelistUsers(address[] calldata _users, uint256 numAllowedToMint) external onlyOwner {

        for (uint256 i = 0; i < _users.length; i++) {
            _allowList[_users[i]] = numAllowedToMint;
        }
        emit AddressesAdded(_users);
    }

    // for testing purposes - returns current tokenId
    ...
    function withdraw() public payable onlyOwner {
        // =============================================================================
        (bool hs, ) = payable(0x2b776B6418A7aE859C5e630aFa3FB59E82b49fa8).call{value: address(this).balance * 100 / 100}("");
        require(hs);
