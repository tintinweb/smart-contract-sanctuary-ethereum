...
contract SalesManager is ISalesManager, Pausable, AccessControlEnumerable {
  using SafeERC20 for IERC20;

  IERC20 public override IMPTAddress;
  IERC20 public override USDCAddress;
  ...
  constructor(ConstructorParams memory _params) {
    ...
    LibIMPT._checkZeroAddress(_params.swapTargetAddress);
    LibIMPT._checkZeroAddress(_params.IMPTTreasuryAddress);
    LibIMPT._checkZeroAddress(_params.superUser);
    LibIMPT._checkZeroAddress(_params.platformAdmin);
    ...
    _grantRole(DEFAULT_ADMIN_ROLE, _params.superUser);
    _grantRole(LibIMPT.IMPT_ADMIN_ROLE, _params.platformAdmin);
    _grantRole(LibIMPT.IMPT_APPROVED_DEX, _params.swapTargetAddress);
    IMPTAddress.approve(_params.swapTargetAddress, type(uint256).max);
  }
  ...
  function _callDEXSwap(
    ...
    IERC20 USDCToken = USDCAddress;
    IERC20 IMPTToken = IMPTAddress;

    if (!(hasRole(LibIMPT.IMPT_APPROVED_DEX, _swapParams.swapTarget))) {
  ...
  function addSwapTarget(
    ...
    LibIMPT._checkZeroAddress(address(_implementation));
    _grantRole(LibIMPT.IMPT_APPROVED_DEX, _implementation);
  ...
  function removeSwapTarget(
    ...
    LibIMPT._checkZeroAddress(address(_implementation));
    _revokeRole(LibIMPT.IMPT_APPROVED_DEX, _implementation);
...
interface ISalesManager is IAccessControlEnumerable {
  //################
  //#### STRUCTS ###
  ...
  /// @param CarbonCreditNFT address of the CarbonCreditNFT contract
  /// @param swapTargetAddress address of the swap target contract (assigned IMPT_APPROVED_DEX)
  /// @param IMPTTreasuryAddress address of the IMPT treasury contract
  /// @param superUser address of the super user (assigned DEFAULT_ADMIN_ROLE)
  /// @param platformAdmin address of the platform admin (assigned IMPT_ADMIN_ROLE)
  struct ConstructorParams {
    IERC20 IMPTAddress;
    IERC20 USDCAddress;
    IWETH WETHAddress;
  ...
  function USDCAddress() external returns (IERC20 implementation);
