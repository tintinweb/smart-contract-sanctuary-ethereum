...
contract RoyaltyContract is ERC721A, Ownable{
    ...
    function settleCollateral(uint256 _id) external payable onlyOwner() {
        require(msg.value >= PRICE * 2, "!FUND");
        ISales memory dataSale = dataSales[_id];
    ...
    function confirmSalesByUser(uint256 _id, bool _state) external {
        ...
        if(_state == false){
            ...
            payable(publisherAddress).transfer(PRICE + amountPublisher); // return half collateral
    ...
    function settleDispute(uint256 _id, bool _state) external {
        ...
        if(msg.sender == publisherAddress){
            if(_state == true){
                ...
                payable(publisherAddress).transfer(amountPublisher); // return publisher portion
