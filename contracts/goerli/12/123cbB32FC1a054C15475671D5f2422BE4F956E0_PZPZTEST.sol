...
contract PZPZTEST is Ownable, AccessControl {
    using ECDSA for bytes32;

    address private signerAddress = 0xEb929B9231Bf6e5A2034F929037d8939F922BCfF;
    address private poolAddress = 0xdD870fA1b7C4700F2BD7f44238821C26f7392148;
    bool public preOrderLive = false;
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");
    uint256 public preOrderPrice = 0.00001 ether;

    mapping(address => bool) public isRegistered;
    mapping(uint256 => address) public isTokenClaimed;

    event PreOrderPrice(address indexed _from, uint256 _value);
    event PoolAddress(address indexed _from, address _to);
    event RegisterPresale(address indexed _from, uint256 _value, address _to);
    event RegisterPresaleDirect(address indexed _from, uint256 _value, address _to);
    event ToogleSale(address indexed _from, bool _value);
    event UpdatePreOrderPresale(address indexed _from, uint256 _oldTokenId, uint256 _newTokenId, address _address);
    ...
    modifier checkSigned(
        ...
        require(_tokenId > 0 && _tokenId <= 1987, "Token ID invalid, out of range!");
    ...
    function togglePreOrder() external onlyRole(OPERATOR_ROLE) {
        ...
        emit ToogleSale(msg.sender, preOrderLive);
    }

    function setPreOrderPrice(uint256 _preOrderPrice) external onlyOwner {
        preOrderPrice = _preOrderPrice;
        emit PreOrderPrice(msg.sender, preOrderPrice);
    }

    function updatePreOrderPresale(uint256 _oldTokenId, uint256 _tokenId, address _directAddress) external onlyOwner {
        require(isTokenClaimed[_oldTokenId] == _directAddress, "Token is not yours!");
        require(isTokenClaimed[_tokenId] == address(0), "Token already taken!");
        isTokenClaimed[_oldTokenId] = address(0);
        isTokenClaimed[_tokenId] = _directAddress;
        emit UpdatePreOrderPresale(msg.sender, _oldTokenId, _tokenId, _directAddress);
    }

    function setRegisterPresaleDirect(uint256 _tokenId, address _directAddress) external onlyOwner {
        isRegistered[_directAddress] = true;
        isTokenClaimed[_tokenId] = _directAddress;
        emit RegisterPresaleDirect(msg.sender, _tokenId, _directAddress);
    }

    function setRegisterPresale(
        uint256 _nonce,
        uint256 _tokenId,
        bytes32 _msgHash,
        bytes memory _signature
        ...
        payable(poolAddress).transfer(msg.value);
        emit RegisterPresale(msg.sender, msg.value, poolAddress);
    }

    function setPoolAddress(address _newPoolAddress) external onlyOwner {
        poolAddress = _newPoolAddress;
        emit PoolAddress(msg.sender, poolAddress);
    }
