...
contract Bond is BondBase {
    ...
    function takerBond(
        ...
        uint16 ,
        uint16 
    ) public view override returns (uint256 bondAmount) {
...
abstract contract BondBase is Initializable, BACUpgradable, IBond {
    ...
    /// @param market - address of market token
    ...
    function setBondConfig(
        ...
        bondConfig[market].bumpPerAsset = bumpPerAsset;
        bondConfig[market].bumpPerStable = bumpPerStable;
        bondConfig[market].assetPerBump = assetPerBump;
        bondConfig[market].stablePerBump = stablePerBump;
    }
    ...
    {
        toLock = takerBond(market, amount, risk, term);
        ...
        if (bondBalance >= toLock) {
            ...
            toReduce = takerToSwap(market, toLock);
        }
    }
    ...
    {
        toLock = makerBond(market, amount, risk, term);
        uint bondBalance = depositedBalances[recipient];
        ...
        if (bondBalance >= toLock) {
            ...
            toReduce = makerToSwap(market, toLock);
        }
    }
    ...
    function lock(
        ...
        address addr,
        uint amount
    ) external override onlyMarket(market) {
        _lock(addr, amount);
    }

    /// @notice transfer BUMP tokens between accounts (needed for position transfering)
    ...
    /// @inheritdoc IBond
    function takerToSwap(address market, uint bondAmount)
        public
        view
        virtual
        override
    ...
    /// @inheritdoc IBond
    function makerToSwap(address market, uint bondAmount)
        public
        view
        virtual
        override
