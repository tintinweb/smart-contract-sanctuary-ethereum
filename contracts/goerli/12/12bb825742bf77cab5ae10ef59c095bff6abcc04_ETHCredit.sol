...
contract ETHCredit is ERC721("Ethereum CREDIT", "eCREDIT"), Ownable {
    ...
    {
        uint256 credit = creditValue[_tokenId].depositValue = amount;
        creditValue[_tokenId].creationTime = block.number;
        _mint(customer, _tokenId);
        uint256 local = _tokenId;
        unchecked {
            ++_tokenId;
        }
        return (local, credit);
    ...
    function creditDecision() external onlyOwner {
        //if (amount > creditValue[tokenId].depositValue) revert
        //if (amount == creditValue[tokenId].depositValue) delete
        //if (amount < creditValue[tokenId].depositValue) subtract
    }
    ...
    */
    ...
    //helps prevent with flash loan attacks
