...
contract SVGRenderer is ISVGRenderer {
     ...
     */
    function generateSVGPart(Part calldata part, uint32 color)
        external
        pure
        override
        returns (string memory partialSVG)
    {
        ...
        return _generateSVGRects(SVGParams({ parts: parts, background: '', colors: color }));
    }

    /**
     * @notice Given RLE image data and color palette pointers, merge to generate a partial SVG image.
     */
    function generateSVGParts(Part[] calldata parts, uint32 colors)
        external
        pure
        override
        returns (string memory partialSVG)
    {
        return _generateSVGRects(SVGParams({ parts: parts, background: '', colors: colors }));
    }
    ...
    function replaceColorIndex(uint8 oldIndex) internal returns (uint8 newIndex) {}
