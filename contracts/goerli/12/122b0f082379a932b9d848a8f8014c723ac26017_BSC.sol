...
contract BSC is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping(address => MintedAmount) public minteds;
    mapping(uint256 => bool) ticketsUsed;
    ...
    struct MintedAmount {
        uint256 pubMintedAmount;
        uint256 wlMintedAmount;
    }
    ...
    event ClaimBonusEvent(address to,uint256[] tokenIds,ClaimStatus claimStatus);

    constructor(string memory _baseTokenUri)
        ERC721A("Bored Sports Club", "BSC")
    {
    ...
    function mint(address to, uint256 amount) external payable {
        ...
        require(
            minteds[to].pubMintedAmount + amount <= PUB_SALE_LIMIT,
            "Limit exceeded"
        );
        require(msg.value >= amount * PUB_PRICE, "Not paying enough fees");
        unchecked {
            minteds[to].pubMintedAmount += amount;
        }
        _mint(to, amount);
    }
    ...
    function wlMint(
        ...
        require(
            keccak256(abi.encodePacked(to, amount,tickets))
                .toEthSignedMessageHash()
                .recover(_singature) == mintSignAddress,
            "You're not on the whitelist"
        );
        require(
            minteds[to].wlMintedAmount + amount <= WL_LIMIT,
            "Limit exceeded"
        );
        require(!ticketsUsed[tickets],"Tickets is used!");
        ticketsUsed[tickets] = true;
        unchecked {
    ...
    function mintToBcn(
        ...
        address to = IERC721A(bcn).ownerOf(bcnTokenId);
        require(to != address(0), "BcnTokenId not exists!");
        require(bcnTokenIdUsedLimit[bcn][bcnTokenId] +amount <= BCN_LIMIT, "BcnTokenId is used limit");
        require(this.containsBcn(bcn),"Not support this bcn");
        require(
            minteds[to].wlMintedAmount + amount <= BCN_LIMIT,
            "Limit exceeded"
        );
        require(totalSupply() + amount <= MAX_SUPPLY, "Sold out!");
        require(msg.value >= amount * WL_PRICE, "Not paying enough fees");
        unchecked {
            minteds[to].wlMintedAmount += amount;
            bcnTokenIdUsedLimit[bcn][bcnTokenId] += amount;
        }
        _mint(to, amount);
    }
    ...
    function addBcn(address[] calldata bcns) external onlyOwner {
        for (uint256 i = 0; i < bcns.length; i++) {
            if (!this.containsBcn(bcns[i])) {
                ...
                supportedBcns._index[bcns[i]] = supportedBcns._bcnAddress.length;
            }
        }
    }
    ...
    function claimBonus(
        ...
        uint256 bonusValue;
        if (_claimStatus == ClaimStatus.ChampionBonus)
            bonusValue = bonus.championBonus / championTokenIdAmount;
        if (_claimStatus == ClaimStatus.TopEightBonus)
            bonusValue = bonus.topEightBonus / topEightRightTeams / 8;
        if (_claimStatus == ClaimStatus.ConsolationPrize)
            bonusValue = bonus.consolationPrize / 24 / 10;
        sendValue(payable(msg.sender), bonusValue * tokenIds.length);
        emit ClaimBonusEvent(msg.sender,tokenIds,_claimStatus);
    }

    function setBonus() external onlyOwner {
        bonus = Bonus({
    ...
    function withdraw() external onlyOwner {
        (bool success, ) = msg.sender.call{value: bonus.ownerBonus}("");
