...
// [ ] Change earlyMintStartTime.
// [ ] Change waitingListMintStartTime.
// [ ] Change mintStartTime.
// [ ] Change craftStartTime.
// [ ] Change MINT_PROVENANCE.
// [ ] Change CRAFT_PROVENANCE.
// [ ] Check MAX_SUPPLY.
// [ ] Check MINT_PER_ADDRESS_LIMIT.
...
contract MyTokenCollection is ERC721Enumerable, Ownable {
  ...
  uint256 public constant MINT_PER_ADDRESS_LIMIT = 1;

  bool public isRevealed = false;
  bool public earlyMintActive = false;
  bool public mintActive = false;
  bool public craftActive = false;

  uint256 public mintStartTime;
  uint256 public earlyMintStartTime;
  ...
  // The counter for each address has minted.
  ...
  // Function to return the merkleRoot for earlyMint according to whitelist or waiting list
  function _getMerkleRoot() internal view returns (bytes32) {
    return block.timestamp >= waitingListMintStartTime ? waitingListMerkleRoot : whitelistMerkleRoot;
  }
  ...
  // Function to verify the earlyMint proof
  function verifyEarlyMint(bytes32[] calldata _merkleProof) external view returns (bool) {
    require(earlyMintActive, "the early mint is paused");
    require(_maxAvailableTokens > 0, "all NFTs are minted!");

    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    return MerkleProof.verify(_merkleProof, _getMerkleRoot(), leaf);
  }

  // Early mint function for people on the whitelist
  function earlyMint(bytes32[] calldata _merkleProof) external {
    require(earlyMintActive, "the early mint is paused");
    require(block.timestamp >= earlyMintStartTime, "the early mint is not started yet");
    require(_maxAvailableTokens > 0, "all NFTs are minted!");

    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, _getMerkleRoot(), leaf), "invalid proof, you're not on the list.");
    require(addressMintedBalance[msg.sender] + 1 <= MINT_PER_ADDRESS_LIMIT, "max NFTs per address exceeded");
  ...
  function craft(uint256 leftId, uint256 rightId) external returns (uint256) {
    require(craftActive, "the craft is paused");
  ...
  // Function to set earlyMint start time
  function setEarlyMintStartTime(uint256 _ts) external onlyOwner {
      earlyMintStartTime = _ts;
  }

  // Function to set mint start time
  function setMintStartTime(uint256 _ts) external onlyOwner {
      mintStartTime = _ts;
  }

  // Function to set craft start time
  function setCraftStartTime(uint256 _ts) external onlyOwner {
      craftStartTime = _ts;
  }
  ...
  function toggleCraft() external onlyOwner {
