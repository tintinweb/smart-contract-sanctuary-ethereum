        ...
        return string.concat(
            ...
            string.concat(
                '"animation_url":"',
                    ...
                    "<script>setTimeout(()=>{const%20e=window.innerWidth,n=window.innerHeight,t=450;let%20i,o,a,r=0,c=window.innerHeight/2;function%20d(){c=window.innerHeight/2,i.aspect=window.innerWidth/window.innerHeight,i.updateProjectionMatrix(),a.setSize(window.innerWidth,window.innerHeight)}function%20s(e){!1!==e.isPrimary&&(r=e.clientY-c)}!function(){(i=new%20THREE.PerspectiveCamera(80,e/n,1,3e3)).position.z=1e3,o=new%20THREE.Scene;const%20r=[[.25,16742144,1],[.5,16750848,1],[.75,16755200,.75],[1,16755200,.5],[1.25,2099,.8],[3,11184810,.75],[3.5,16777215,.5],[4.5,16777215,.25],[5.5,16777215,.125]],c=function(){const%20e=new%20THREE.BufferGeometry,n=[],i=new%20THREE.Vector3;for(let%20e=0;e<1500;e++)i.x=2*Math.random()-1,i.y=2*Math.random()-1,i.z=2*Math.random()-1,i.normalize(),i.multiplyScalar(t),n.push(i.x,i.y,i.z),i.multiplyScalar(.09*Math.random()+1),n.push(i.x,i.y,i.z);return%20e.setAttribute('position',new%20THREE.Float32BufferAttribute(n,3)),e}();for(let%20e=0;e<r.length;++e){const%20n=r[e],t=new%20THREE.LineBasicMaterial({color:n[1],opacity:n[2]}),i=new%20THREE.LineSegments(c,t);i.scale.x=i.scale.y=i.scale.z=n[0],i.userData.originalScale=n[0],i.rotation.y=Math.random()*Math.PI,i.updateMatrix(),o.add(i)}(a=new%20THREE.WebGLRenderer({antialias:!0})).setPixelRatio(window.devicePixelRatio),a.setSize(e,n),document.body.appendChild(a.domElement),document.body.style.touchAction='none',document.body.addEventListener('pointermove',s),window.addEventListener('resize',d)}(),function%20e(){requestAnimationFrame(e);!function(){i.position.y+=.05*(200-r-i.position.y),i.lookAt(o.position),a.render(o,i);const%20e=1e-4*Date.now();for(let%20n=0;n<o.children.length;n++){const%20t=o.children[n];if(t.isLine&&(t.rotation.y=e*(n<4?n+1:-(n+1)),n<5)){const%20i=t.userData.originalScale*(n/5+1)*(1+.5*Math.sin(7*e));t.scale.x=t.scale.y=t.scale.z=i}}}()}()},1e3);</script>",
                '",'
            ),
