...
contract UniforgeDomains {
  ...
  mapping(address => Domain) private addressToDomain;
  ...
  constructor(uint256 _deployCost) {
    owner = payable(msg.sender);
    deployCost = _deployCost * 1 wei;
  }
  ...
  function setDeployPrice(uint256 _price) public onlyOwner {
    deployCost = _price * 1 wei;
  }
  ...
  function getAllDomains() public view returns (Domain[] memory) {
    ...
    for (uint256 i = 0; i < itemCount; i++) {
      ...
      Domain storage currentDomain = idToDomain[currentId];
  ...
  /* checks if domain already exists */
  function getDomainsByAddress(address domainOwner)
    public
    view
    returns (Domain[] memory)
  {
    ...
    for (uint256 i = 0; i < itemCount; i++) {
      uint256 currentId = i + 1;
      Domain storage currentDomain = idToDomain[currentId];
      if (currentDomain.owner == domainOwner) {
        domains[i] = currentDomain;
      }
    }
    return domains;
  ...
  function createDomain(string memory name, string memory hash) public payable {
    ...
    require(msg.value >= deployCost, "Insuficient amount");
  ...
  function transferDomain(uint256 domainId, address newOwner) public {
    Domain storage domain = idToDomain[domainId];
    require(
      domain.owner == msg.sender,
