  ...
  DefaultOperatorFilterer
{
  ...
  /**
   * @notice track tokens that have been redeemed/burned
   * @dev tokenId -> address that redeemed
   */
  mapping(uint256 => address) private s_hasRedeemed;
  ...
  ) ERC721A("Original Art Redemption", "OAredeem") {
  ...
  function redeemToken(uint256 tokenId) external {
    ...
    if (s_hasRedeemed[tokenId] != address(0)) {
      revert RedeemableTokens__AlreadyRedeemed();
    }
    _burn(tokenId);
    s_hasRedeemed[tokenId] = msg.sender;
   ...
   */
  function getRedeemed(uint256 tokenId) external view returns (address) {
    return s_hasRedeemed[tokenId];
  }
  ...
  function getPackInfo(
    uint256 packId
  ) external view returns (uint256 tokenId, address mintedTo) {
  ...
  function getTokenInfo(
    uint256 tokenId
  ) external view returns (uint256 packId, address mintedTo) {
