...
import {BatchERC721} from "./BatchERC721.sol";
import {ERC2981} from "@openzeppelin/contracts/token/common/ERC2981.sol";
import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";
import {AccessControl} from "@openzeppelin/contracts/access/AccessControl.sol";
import {MerkleProof} from "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";
import {IMortiverse} from "./IMortiverse.sol";

contract Mortiverse is BatchERC721, ERC2981, Ownable, AccessControl, IMortiverse {
    struct SaleConfig {
        uint128 allowlist;
        uint128 publicSale;
    }
    ...
    string public contractURI = "testContractURI";
    ...
    constructor(
        ...
        address manager_,
        uint256 initialQty_
    ) Ownable() AccessControl() BatchERC721("Mortiverse", "MORTI", 0) {
        _grantRole(DEFAULT_ADMIN_ROLE, owner_);
        _grantRole(MANAGER_ROLE, owner_);
        _grantRole(MANAGER_ROLE, manager_);
        _transferOwnership(owner_);
        saleConfig = SaleConfig(1665763200, 1665766800);
    ...
    function mint(uint256 quantity) external payable isMintable(quantity) {
        if (currentPhase() != Phase.ALLOWLIST) revert NotPublicPhase();
        mints[msg.sender] += quantity;
        safeMint(msg.sender, quantity);
    }
    ...
    function currentPhase() public view returns (Phase) {
        if (block.timestamp > saleConfig.publicSale + 1 days) {
            return Phase.OVER;
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(BatchERC721, AccessControl, ERC2981)
        returns (bool)
    {
        return
...
import {IBatchERC721} from "./IBatchERC721.sol";
...
contract BatchERC721 is IERC165, IERC721, IERC2309, IBatchERC721 {
    ...
    function _mint2309(address _to, uint256 _quantity) internal {
        ...
        emit ConsecutiveTransfer(startingTokenId, nextTokenId, address(0), _to);
    }
