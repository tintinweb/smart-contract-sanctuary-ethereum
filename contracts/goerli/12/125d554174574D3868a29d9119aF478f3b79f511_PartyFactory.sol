...
contract PartyGovernanceNFT is PartyGovernance, ERC721, IERC2981 {
    ...
    function burn(uint256[] memory tokenIds) public {
        ...
        // Can only burn as a user if total voting power has been set,
        // indicating the party has "started" (i.e. crowdfund has ended). In
        // otherwords, you cannot burn your membership card until the initial
        // crowdfund for the party has ended.
        //
        // Authority needs to be able to burn cards during the initial
        // crowdfund, however, to process refunds.
        bool hasPartyStarted = totalVotingPower != 0;
        if (!hasPartyStarted && !authority) revert OnlyAuthorityError();

        // Update last burn timestamp.
        lastBurnTimestamp = uint40(block.timestamp);

        // Used to update voting power state of party at the end.
        uint96 totalVotingPowerBurned;
        ...
        for (uint256 i; i < tokenIds.length; ++i) {
            ...
            if (
                ...
                // Only allow authority to burn tokens they do not own to process
                // refunds if the party has not started.
                if (hasPartyStarted || !authority) revert UnauthorizedToBurnError();
            }
        ...
        if (hasPartyStarted) {
            ...
            _governanceValues.totalVotingPower = totalVotingPower - totalVotingPowerBurned;
        }
    }
    ...
    function rageQuit(
        ...
        uint256 tokensLength = withdrawTokens.length;
        uint256[] memory withdrawAmounts = new uint256[](tokensLength);
        {
            // Get total balances of each token in party's balances.
            uint256[] memory tokenBalances = new uint256[](tokensLength);
            {
                ...
                for (uint256 i; i < tokensLength; ++i) {
                    ...
                    // Record token's balance.
                    tokenBalances[i] = address(token) == ETH_ADDRESS
            ...
            for (uint256 i; i < tokenIds.length; ++i) {
                ...
                uint256 shareOfVotingPower = getVotingPowerShareOf(tokenId);

                // Add fair share of tokens from the party to total.
                for (uint256 j; j < tokensLength; ++j) {
                    withdrawAmounts[j] += (tokenBalances[j] * shareOfVotingPower) / 1e18;
                }
            }
        }
