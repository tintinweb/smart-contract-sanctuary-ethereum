...
contract Crypto528Marketplace is ReentrancyGuard, Ownable {
    ...
    uint256[] public levelPrices = [50, 250, 500, 2500, 5000, 10000, 20000, 50000];
    ...
    function giveaway(address _to, uint256 _id, uint256 _price, 
        ...
        if (_id == 0) {
            ...
            Crypto528(nftContract).mint(newTokenId, _to, _tokenUri);
            emit Giveaway(msg.sender, _to, newTokenId);
        } else {
            if (listedMap[_id] == false) {
                Crypto528(nftContract).transferFrom(msg.sender, _to, _id);
            } else {
                require(ownerMap[_id] == msg.sender, "sender is not owner");
                Crypto528(nftContract).transferFrom(address(this), _to, _id);
                listedMap[_id] == false;
            }
            ownerMap[_id] = _to;
    ...
    function mint(
        ...
        _tokenIds.increment();
        _levelCounter[_level].increment();

        require(_levelCounter[_level]._value <= soldoutLimits[_level], "This Level is sold out");

        uint256 newTokenId = _tokenIds.current();

        paymentTokenMap[newTokenId] = paymentType;
        levelMap[newTokenId] = _level;

        if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("ETH")))) {
            price[newTokenId] = levelETHPrices[_level];
            require (msg.value >= price[newTokenId], "msg.value should be equal to the buyAmount");
            transferETHOrWETH(payable(adminRecoveryAddress), price[newTokenId].div(2));
        } else {
            price[newTokenId] = levelPrices[_level];
            ...
            require (IERC20(tokenAddressMap[paymentType]).transferFrom(msg.sender, adminRecoveryAddress, price[newTokenId]));      
        }
        
        listedMap[newTokenId] = false;
        ownerMap[newTokenId] = msg.sender;
        Crypto528(nftContract).mint(newTokenId, msg.sender, _tokenUri);
        emit Minted(msg.sender, price[newTokenId], newTokenId, _tokenUri, false);
    }
