...
contract EnergyBurning is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public maxVoidsToFill;
    uint256 public energyAmountPerVoid;
    uint256 public lpTokenAmountPerVoid;

    mapping(address => uint256) public nonces;

    event EnergyBurned(address indexed addr, uint256 voidsFilled, uint256 energyAmountBurned);
    ...
    constructor(
        ...
        address _lpToken,
        uint256 _maxVoidsToFill,
        uint256 _energyAmountPerVoid,
        uint256 _lpTokenAmountPerVoid
    ) {
        _signer = signer;
        energyConverter = IConverterLogic(_energyConverter);
        lpToken = IERC20(_lpToken);
        maxVoidsToFill = _maxVoidsToFill;
        energyAmountPerVoid = _energyAmountPerVoid;
        lpTokenAmountPerVoid = _lpTokenAmountPerVoid;
        _pause();
    }
     ...
     * @param _nonce The nonce id of the wallet address
     * @param _voidsToFill The number of voids to fill in
     * @return Encoded hash
     */
    function _getHash(
        address _walletAddress,
        uint256 _nonce,
        uint256 _voidsToFill
    ) internal pure returns (bytes32) {
        return keccak256(abi.encode(_walletAddress, _nonce, _voidsToFill));
    }
     ...
     * @param _walletAddress The wallet address that is validating signature
     * @param _nonce The nonce id of the wallet address
     * @param _voidsToFill The number of voids to fill in
     * @param _signature The signature passed from the caller
    ...
    function validateSignature(
        ...
        return
            _verify(_getHash(_walletAddress, _nonce, _voidsToFill), _signature);
    }

    /**
     * @dev Burn asto energy to fill voids and transfer LP token to the user as a reward
    ...
    function burn(
        ...
        require(maxVoidsToFill >= _voidsToFill, "Exceeded max voids limit");

        uint256 periodId = energyConverter.getCurrentPeriodId();
        uint256 availableEnergy = energyConverter.getEnergy(
            msg.sender,
            periodId
        );
        uint256 energyToBurn = _voidsToFill * energyAmountPerVoid;
        uint256 rewardToSend = _voidsToFill * lpTokenAmountPerVoid;
        require(availableEnergy >= energyToBurn, "Insufficient asto energy");
        require(
            lpToken.balanceOf(address(this)) >= rewardToSend,
        ...
        emit EnergyBurned(msg.sender, _voidsToFill, energyToBurn);
    }
     ...
     */
    function setLpToken(address _addr) external onlyOwner whenPaused {
     ...
     */
    function setSigner(address signer) external onlyOwner whenPaused {
        _signer = signer;
    }
     ...
     */
    function setMaxVoidsToFill(uint256 _amount) external onlyOwner whenPaused {
        maxVoidsToFill = _amount;
    }
     ...
     */
    function setAstoEnergyAmountPerVoid(uint256 _amount) external onlyOwner whenPaused {
        energyAmountPerVoid = _amount;
    }
     ...
     */
    function setLpTokenAmountPerVoid(uint256 _amount) external onlyOwner whenPaused {
        lpTokenAmountPerVoid = _amount;
    }
