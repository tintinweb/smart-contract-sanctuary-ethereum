...
contract DriversLimitedEditions is ERC721Base, Withdraw {
    ...
    event MintedOneOfOne(uint256 indexed id);
    event MintedEdition(
    ...
    constructor(
        ...
        address metadata_,
        address fountCard_
    ) ERC721Base(owner_, admin_, royaltiesAmount_, metadata_, fountCard_) {}
     ...
     */
    function collectOneOfOne(uint256 baseId, address to) external payable {
    ...
    function _collectOneOfOne(
        ...
        if (tokenData.size != 1) revert NotOneOfOne();
        if (!tokenData.freeMint && tokenData.price == 0) revert NotForSale();
     ...
     */
    function placeBid(uint256 baseId) external payable {
        TokenData memory tokenData = _baseIdToTokenData[baseId];
        if (tokenData.requiresSig) revert RequiresSignature();
     ...
     * @param signature The off-chain signature that permits a mint
     */
    function placeBid(uint256 baseId, bytes calldata signature) external payable {
        TokenData memory tokenData = _baseIdToTokenData[baseId];
    ...
    function settleAuction(uint256 baseId) external {
        ...
        // Note: this will already revert if it's been sold with `collectOneOfOne`
        _mint(auction.highestBidder, baseId);
     ...
     */
    function collectEdition(uint256 baseId, address to) external payable {
        TokenData memory tokenData = _baseIdToTokenData[baseId];
        if (tokenData.requiresSig) revert RequiresSignature();
    ...
    /**
     * @notice Internal function to mint the next edition with some conditions
    ...
    function _collectEdition(
        ...
        // Check to see if the next edition is mintable and the price is correct
        if (tokenData.size < 2) revert NotEdition();
        if (tokenData.minted + 1 > tokenData.size) revert EditionSoldOut();
        if (!tokenData.freeMint && tokenData.price == 0) revert NotForSale();
        if (tokenData.price != msg.value) revert IncorrectPaymentAmount();
        ...
        uint256 editionNumber = tokenData.minted + 1;
        ...
        // Mint the NFT to the `to` address
        _mint(to, tokenId);
        emit MintedEdition(baseId, editionNumber, tokenId);
    }
    ...
    function addTokenForSale(
        ...
        if (editionSize > MAX_EDITION_SIZE) revert EditionSizeExceedsMaxValue();

        TokenData memory tokenData = _baseIdToTokenData[baseId];

        // Check the token data is empty before adding
        if (tokenData.size != 0) revert TokenDataAlreadyExists();

        // Set the new token data
        tokenData.price = price;
        tokenData.size = editionSize;
        tokenData.fountExclusive = fountExclusive;
        tokenData.requiresSig = requiresSig;
        tokenData.freeMint = freeMint;
        _baseIdToTokenData[baseId] = tokenData;
    }
    ...
    function setTokenSalePrice(
        ...
        if (tokenData.size == 0) revert TokenDataDoesNotExist();
     ...
     * @dev Used if an admin wants to change the edition size. Token data must be
     * deleted and created again. Will also cancel any auctions for 1/1s.
     *
     * Reverts if:
     *  - the token data does not exist
     *  - if the token data already has mints recorded
     *  - if an auction for the token exists and has already ended (may have not settled)
     *
     * @param baseId The base NFT id
     */
    function deleteTokenData(uint256 baseId) external onlyOwnerOrAdmin {
        TokenData memory tokenData = _baseIdToTokenData[baseId];
        if (tokenData.size == 0) revert TokenDataDoesNotExist();
        ...
        // Cancel any auctions for the token which also refunds any bids
        if (tokenData.size == 1) {
            AuctionData memory auction = auctions[baseId];
        ...
        delete _baseIdToTokenData[baseId];
    }
    ...
    function tokenIsOneOfOne(uint256 baseId) external view returns (bool) {
        return _baseIdToTokenData[baseId].size == 1;
    }
    ...
    function tokenEditionSize(uint256 baseId) external view returns (uint256) {
        return _baseIdToTokenData[baseId].size;
    }

    function tokenMintedCount(uint256 baseId) external view returns (uint256) {
        return _baseIdToTokenData[baseId].minted;
    }
    ...
    function tokenRequiresOffChainSignatureToMint(uint256 baseId) external view returns (bool) {
        return _baseIdToTokenData[baseId].requiresSig;
    }

    function tokenIsFreeMint(uint256 baseId) external view returns (bool) {
        TokenData memory tokenData = _baseIdToTokenData[baseId];
        return tokenData.price == 0 && tokenData.freeMint;
    }
