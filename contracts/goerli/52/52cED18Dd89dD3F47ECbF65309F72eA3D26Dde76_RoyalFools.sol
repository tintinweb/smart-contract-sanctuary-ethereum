...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function burn(uint tokenId) public virtual {
        TokenOwnership memory currentOwnership = ownershipOf(tokenId);

        bool isApprovedOrOwner = (_msgSender() == currentOwnership.addr ||
            getApproved(tokenId) == _msgSender() ||
            isApprovedForAll(currentOwnership.addr, _msgSender()));
    ...
    function _burn(uint tokenId) internal {
        ...
        _beforeTokenTransfers(_msgSender(), address(0), tokenId, 1);

        delete _tokenApprovals[tokenId];
        _addressData[_msgSender()].balance -= 1;
        delete _ownerships[tokenId];

        emit Transfer(_msgSender(), address(0), tokenId);

        _afterTokenTransfers(_msgSender(), address(0), tokenId, 1);
    }
...
contract RoyalFools is ERC721A, Ownable {
  string public _originFoolsURI;

  bool public foolsInMarket = false;

  uint public constant FOOLSONWALLET = 1;
  uint public constant TOSSACOIN = 0 ether;
  string constant SECRETPHRASE = "13477431";

  uint public royalCourt = 7431;
  uint private inPrison = 741;
  ...
  constructor(
      ...
      uint _royalCourt,
      uint _inPrison) ERC721A("Royal Fools", "RF") {
  ...
  function flipSale() public wholsTheKingHere {
    foolsInMarket = !foolsInMarket;
  }

  function theDungeonMaster(string memory foolURI) public wholsTheKingHere {
    _originFoolsURI = foolURI;
  }
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    return _originFoolsURI;
  }

  function homeBoy() public wholsTheKingHere {
      require(foolsInMarket, "Sale is not active currently");
