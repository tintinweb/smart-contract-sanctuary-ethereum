...
*/
contract GigaMartProxyRegistryTest is ProxyRegistryTest {
...
*/
contract ProxyRegistryTest is IProxyRegistry, Ownable {
  ...
  */
  ...
  /**
    ...
    registry for the unauthenticated address `unauthorized`.

    @param unauthorized The new address to grant access to the registry.
  */
  function authenticateCaller(address unauthorized) external onlyOwner {
    if(authorizedCallers[unauthorized])
        revert AlreadyAuthorized();
    authorizedCallers[unauthorized] = true;
  }
