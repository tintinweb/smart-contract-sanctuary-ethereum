...
contract BrandExtenderSuitUp is BrandExtender {

    using Counters for Counters.Counter;

    mapping(uint => address) private minter;

    constructor(address _rr, address _newUriChanger) BrandExtender( _rr, _newUriChanger) {}
    ...
    function mintDerived(mintRequestData[] calldata) external override payable {
        revert("Deprecated");
    }
    ...
    function mintDerivedByService(address erc721, uint256 tokenId, address receiver, string calldata tmpMetadataURL) external onlyUriChanger payable {
        uint mintedID;
        mintedID = _mintDerivedMulti(erc721, tokenId, receiver);
        minter[mintedID] = receiver;
        emit MintedDerived(erc721, tokenId, mintedID, tmpMetadataURL, tokenOfOriginCounter(erc721, tokenId) - 1 );
    }
