...
contract SLnotifyStaking is ReentrancyGuard {
    ...
    constructor(IERC721 _nftCollection, IERC20 _rewardsToken, address _stakingVaultAddress, address _BurnAddress) {
    ...
    function claimRewards() public nonReentrant {
        ...
        require(stakers[msg.sender].unclaimedRewards > 0, "You don't have any unclaimed rewards!");

        // Burn 10% of the rewards
        uint256 burnAmount = stakers[msg.sender].unclaimedRewards / 10;
        rewardsToken.safeTransfer(BurnAddress, burnAmount);

        // Transfer 10% of the rewards to the staking vault
        uint256 stakingVaultAmount = stakers[msg.sender].unclaimedRewards / 10;
        rewardsToken.safeTransfer(stakingVaultAddress, stakingVaultAmount);

        // Transfer the remaining 80% of the rewards to the user
        uint256 userAmount = calculateRewards(msg.sender) +
            stakers[msg.sender].unclaimedRewards - burnAmount - stakingVaultAmount;
            rewardsToken.safeTransfer(msg.sender, userAmount);

        // Reset the unclaimed rewards for the user
        stakers[msg.sender].timeOfLastUpdate = block.timestamp;
