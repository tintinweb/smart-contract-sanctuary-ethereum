...
contract SwapperEth is Ownable {
    ...
    /**
     * Approves "_amount" of ERC20 token "_tokenName" for token storage contract to make exchanges
     */
    function approveErc20(string memory _tokenName, uint _amount) external returns(bool){
        address tokenAddress = tokenNameToInfo[_tokenName].storageInstance.tokenAddress();
        (bool success,) = tokenAddress.delegatecall(
            abi.encodeWithSignature("approve(address spender, uint256 amount)", address(tokenNameToInfo[_tokenName].storageInstance), _amount)
        );
        return success;
    }
    ...
    function isExchangePossible(string memory _tokenName, uint _tokenAmount, uint _ethAmount, address _initiator) external view returns(bool){
        ...
        uint decimals = storageInstance_.decimals();
