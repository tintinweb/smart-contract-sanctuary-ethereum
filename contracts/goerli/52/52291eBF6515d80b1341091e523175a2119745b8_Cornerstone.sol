...
contract Cornerstone is ERC721A, Ownable, ReentrancyGuard, Modifiers, Splitter {
    ...
    function setTokenURI(uint256 id, string memory newURL)
        public
        onlyOwner
        noEmptyURI(newURL)
    {
    ...
    {
        require(newSupply > _totalMinted(), "Invalid amount");
        maxSupply = newSupply;
    }
    ...
    function airdrop(address receiver, uint256 amount)
        ...
        noZeroValue(amount)
    {
        require(amount <= mintMax, "Minting limit reached");
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            require(receivers[i] != address(0), "Receivers have at least one empty address");
        }

        for (uint256 i = 0; i < amounts.length; i++) {
            require(amounts[i] != 0, "Amounts have at least one empty value");
            require(amounts[i] <= mintMax, "Minting limit reached");
            totalAmounts += amounts[i];
        }

        require(maxSupply >= totalAmounts + _totalMinted(), "Supply limit reached");
        ...
        noZeroValue(quantity)
    {
        ...
        if (whitelisted) {
            require(
                Signature.verify(signer, signature, quantity),
    ...
    function distribute() public payable nonReentrant onlyOwner {
        require(address(this).balance >= 0, "Insuficient balance");
        require(payeeCount() > 0, "Distribution values have not been set");
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            release(payable(payee(i)), amount);
        }
    }
...
abstract contract Splitter is Context {
    address[] public _payees;
    mapping(address => uint256) public _shares;

    function setPayees(address[] memory payees, uint256[] memory shares_)
        public
        virtual
    {
    ...
    function getAmount(uint256 balance, uint256 index) public view returns (uint256) {
        return (balance * _shares[payee(index)]) / 10000;
    }

    function release(address payable account, uint256 payment) public virtual payable {
        require(account != address(0), "Splitter: account is the zero address");
        require(payment > 0, "Splitter: payment are 0");
 ...
 */
library Signature {
    function verify(address signer, bytes memory signature, uint256 tokenAmount)
        internal
        view
        returns (bool)
    {
        bytes32 payload = keccak256(abi.encode(msg.sender, tokenAmount));
    ...
    function _verifyHashSignature(address signer, bytes32 message, bytes memory signature)
        internal
        pure
        returns (bool)
    {
...
abstract contract Modifiers {
    ...
    /**
     * @dev Requirement for non-zero addresses.
     */
    modifier noZeroAddresses(address _from, address _to) {
        require(_from != address(0) || _to != address(0), "Address cannot be zero");
        _;
    }
    ...
    /**
     * @dev Requirement for non-empty uint256 values.
     */
