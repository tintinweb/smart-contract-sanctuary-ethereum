...
contract Tomatosoup is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  uint256 public immutable maxMintOneTime;
  ...
  constructor(
    ...
    maxMintOneTime = _maxMintOneTime;
    price = _price;
    proAddr = _proAddr;
    preMintOpen = true;
    root = _root;
  ...
  function preMint(uint256 _quantity, bytes32[] calldata proof) external payable nonReentrant callerIsUser {
    ...
    bool minted = isMinted[minter];
    require(preMintOpen, 'PreMint close now.');
    require(MerkleProof.verify(proof, root, _leaf(minter)), 'Address not in whitelist.');
    require(!minted, 'Address already minted');
    _minNFT(minter, _quantity);
  }
  ...
  function pubMint(uint256 _quantity) external payable callerIsUser nonReentrant {
    require(pubMintOpen, 'PubMint close now.');
    _minNFT(_msgSender(), _quantity);
  }

  function _minNFT(address _addr, uint256 _quantity) internal {
    uint256 totalPrice_ = getCaculatePrice(_addr, _quantity);
    require(totalPrice_ <= msg.value, 'Value is Not Right.');
    require(totalSupply() + _quantity <= maxSupply, 'Exceed To MaxSupply.');
    if (_addr != proAddr) require(_quantity <= maxMintOneTime, 'Exceed To MaxMint of one time.');
    if (!isMinted[_addr]) isMinted[_addr] = true;
  ...
  function getProAddr() public view onlyOwner returns (address) {
    return proAddr;
  }

  function setProAddr(address _newProAddr) public onlyOwner {
    proAddr = _newProAddr;
  }

  function setPubMint() public onlyOwner {
    preMintOpen = false;
    pubMintOpen = true;
  }
