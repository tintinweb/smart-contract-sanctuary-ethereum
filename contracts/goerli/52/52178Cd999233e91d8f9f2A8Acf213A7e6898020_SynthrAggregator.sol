...
contract DebtShareAggregator {
    ...
    function sharePercent(address account) external view returns (uint) {
        uint balance = _debtShareBalanceOf(account);
        ...
        // if (debtShareTotalSupply();)

        return 10**18 * balance / totalBal;
    }

    function sharePercentOnPeriod(address account, uint periodId) internal view returns (uint) {
...
contract LiquidatorRewardsAggregator {
    ...
    function earned(
        ...
        //         .add(entry.claimable);
        return (debtShare * (accumulatedRewardsPerShare - uint256(entry.entryAccumulatedRewards))) / (10**27) + entry.claimable;
    }
}
...
contract RewardEscrowV2Aggregator {
    ...
    function _vest(
        ...
        uint _accountBalance = _totalEscrowedAccountBalance[currencyKey][account];
        _totalEscrowedAccountBalance[currencyKey][account] = _accountBalance > amount ? _accountBalance - amount : 0; 
        
        uint _totalBal =  _totalEscrowedBalance[currencyKey];
        _totalEscrowedBalance[currencyKey] = _totalBal > amount ? _totalBal - amount : 0;
...
contract SynthrAggregator is Ownable, CollateralAggregator, DebtShareAggregator, LiquidatorRewardsAggregator, RewardEscrowV2Aggregator, SynthAggregator {
    ...
    function mintSynth(
        ...
        if (_collateralKey != bytes32(0) && _collateralAmount != 0) {
            console.log("this address", _collateralAmount);
    ...
    function liquidate(
        ...
        _updateEntry(currentDebtShareBalance, account);

        _burnDebtShare(account, debtShare);

        if (currentDebtShareBalance > debtShare) {
            _updateAccumulatedShare(notifyAmount / (currentDebtShareBalance - debtShare));
        }
