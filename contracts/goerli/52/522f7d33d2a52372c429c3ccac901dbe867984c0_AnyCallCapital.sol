...
contract AnyCallBase is Initializable {
  ...
  event Fallback(address to, bytes data);
  event MyLog1(bytes4 extractedSelector, bytes4 staticSelector);
  event MyLog2(bytes);
  ...
  function setFallback(address fallback_) onlyOwner public {
    _fallback = fallback_;
  }
  ...
  function anyExecuteOld(bytes calldata data) 
    external returns (bool success, bytes memory result) 
  {
    ...
    else { // branch executed by periphery
    ...
    external returns (bool success, bytes memory result) 
  {
    ...
    emit MyLog1(selector, this.anyExecute.selector);
    emit MyLog2(data);
    if (selector == this.anyExecute.selector) {
      ...
      (
        ...
        uint requestCode,
        bytes memory payload // either request payload or payload of response to the request
      ) = abi.decode(data[4:], (address, uint, uint, uint, bytes));
      bytes memory responsePayload = new bytes(1);
      responsePayload[0] = bytes1(uint8(0x1));
      _processRequest(
        caller,
    ...
    else if (selector == this.anyFallback.selector) { // executed by Capital
