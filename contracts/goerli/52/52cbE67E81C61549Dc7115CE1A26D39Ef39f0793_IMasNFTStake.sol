...
import { IMasStaking1155Upgradeable } from "./IMasStaking1155Upgradeable.sol";
import "./IMasStake.sol";




contract IMasNFTStake is
    Initializable,
    ContractMetadata,
    PermissionsEnumerable,
    ERC2771ContextUpgradeable,
    MulticallUpgradeable,
    IMasStaking1155Upgradeable,
    ERC165Upgradeable,
    IERC1155ReceiverUpgradeable,
    IMasStake
{
    bytes32 private constant MODULE_TYPE = bytes32("IMasNFTStake");
    ...
    address public iMasRewardToken;
    address public iMasXRewardToken;

    /// @dev Total amount of reward tokens in the contract.
    uint256 private iMasRewardTokenBalance;
    uint256 private iMasXRewardTokenBalance;
    ...
    function initialize(
        ...
        address _iMasRewardToken,
        address _iMasXRewardToken,
        address _stakingToken,
        uint256 _defaultTimeUnit,
        uint256 _defaultIMasRewardsPerUnitTime,
        uint256 _defaultIMasXRewardsPerUnitTime
    ) external initializer {
        __ERC2771Context_init_unchained(_trustedForwarders);

        iMasRewardToken = _iMasRewardToken;
        iMasXRewardToken = _iMasXRewardToken;
        __Staking1155_init(_stakingToken);
        _setDefaultStakingCondition(_defaultTimeUnit, _defaultIMasRewardsPerUnitTime, _defaultIMasXRewardsPerUnitTime);
    ...
    function depositIMasRewardTokens(uint256 _amount) external payable nonReentrant {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Not authorized");

        address _iMasRewardToken = iMasRewardToken == CurrencyTransferLib.NATIVE_TOKEN ? nativeTokenWrapper : iMasRewardToken;

        uint256 balanceBefore = IERC20(_iMasRewardToken).balanceOf(address(this));
        ...
        uint256 actualAmount = IERC20(_iMasRewardToken).balanceOf(address(this)) - balanceBefore;

        iMasRewardTokenBalance += actualAmount;

        emit IMasRewardTokensDepositedByAdmin(actualAmount);
    }

    /// @dev Admin can withdraw excess reward tokens.
    function withdrawIMasRewardTokens(uint256 _amount) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Not authorized");

        // to prevent locking of direct-transferred tokens
        iMasRewardTokenBalance = _amount > iMasRewardTokenBalance ? 0 : iMasRewardTokenBalance - _amount;
        ...
        emit IMasRewardTokensWithdrawnByAdmin(_amount);
    }

    /// @notice View total rewards available in the staking contract.
    function getIMasRewardTokenBalance() external view override returns (uint256 _iMasRewardsAvailableInContract) {
        return iMasRewardTokenBalance;
    }

    function getIMasXRewardTokenBalance() external view override returns (uint256 _iMasXRewardsAvailableInContract) {
        return iMasXRewardTokenBalance;
    }
    ...
    function _mintIMasRewards(address _staker, uint256 _rewards) internal override {
        require(_rewards <= iMasRewardTokenBalance, "Not enough IMas reward tokens");
        iMasRewardTokenBalance -= _rewards;
        CurrencyTransferLib.transferCurrencyWithWrapper(
            iMasRewardToken,
            address(this),
            _staker,
    ...
    function _mintIMasXRewards(address _staker, uint256 _rewards) internal override {
        require(_rewards <= iMasXRewardTokenBalance, "Not enough IMasX reward tokens");
        iMasXRewardTokenBalance -= _rewards;
        CurrencyTransferLib.transferCurrencyWithWrapper(
            iMasXRewardToken,
            address(this),
            _staker,
...
interface IMasStake {
    ...
    event IMasRewardTokensWithdrawnByAdmin(uint256 _amount);

    /// @dev Emitted when contract admin deposits reward tokens.
    event IMasRewardTokensDepositedByAdmin(uint256 _amount);
     ...
     */
    function depositIMasRewardTokens(uint256 _amount) external payable;
...
interface IMasStaking1155 {
    ...
    event IMasRewardsClaimed(address indexed staker, uint256 rewardAmount);
    event IMasXRewardsClaimed(address indexed staker, uint256 rewardAmount);
    ...
    /// @dev Emitted when contract admin updates IMasRewardsPerUnitTime.
    event UpdatedIMasRewardsPerUnitTime(
        uint256 indexed _tokenId,
        uint256 oldIMasRewardsPerUnitTime,
        uint256 newIMasRewardsPerUnitTime
    );

    /// @dev Emitted when contract admin updates IMasXewardsPerUnitTime.
    event UpdatedIMasXRewardsPerUnitTime(
        uint256 indexed _tokenId,
        uint256 oldIMasXRewardsPerUnitTime,
        uint256 newIMasXRewardsPerUnitTime
    );
    ...
    event UpdatedDefaultIMasRewardsPerUnitTime(uint256 oldRewardsPerUnitTime, uint256 newRewardsPerUnitTime);
    event UpdatedDefaultIMasXRewardsPerUnitTime(uint256 oldRewardsPerUnitTime, uint256 newRewardsPerUnitTime);
    ...
    struct Staker {
        ...
        uint256 unclaimedIMasRewards;
        uint256 unclaimedIMasXRewards;
    ...
    struct StakingCondition {
        uint256 timeUnit;
        uint256 iMasRewardsPerUnitTime;
        uint256 iMasXRewardsPerUnitTime;
     ...
     */
    function claimIMasRewards(uint256 tokenId) external;
     ...
     *  @param tokenId   Staked token Id.
     */
    function claimIMasXRewards(uint256 tokenId) external;    
    ...
    function getStakeInfoForToken(uint256 tokenId, address staker)
        external
        view
        returns (uint256 _tokensStaked, uint256 _iMasRewards, uint256 _iMasXRewards);
    ...
    function getStakeInfo(address staker)
        ...
        returns (
            ...
            uint256 _totalIMasRewards,
            uint256 _totalIMasXRewards
        );
}
...
import "./IMasStaking1155.sol";

abstract contract IMasStaking1155Upgradeable is ReentrancyGuardUpgradeable, IMasStaking1155 {
    /*///////////////////////////////////////////////////////////////
     ...
     *  @notice    Claim accumulated rewards.
     *
     *  @dev       See {_claimIMasRewards}. Override that to implement custom logic.
     *             See {_calculateRewards} for reward-calculation logic.
     *
     *  @param _tokenId   Staked token Id.
     */
    function claimIMasRewards(uint256 _tokenId) external nonReentrant {
        _claimIMasRewards(_tokenId);
    }

    /**
     *  @notice    Claim accumulated rewards.
     *
     *  @dev       See {_claimIMasXRewards}. Override that to implement custom logic.
     *             See {_calculateRewards} for reward-calculation logic.
     *
     *  @param _tokenId   Staked token Id.
     */
    function claimIMasXRewards(uint256 _tokenId) external nonReentrant {
        _claimIMasXRewards(_tokenId);
    }    
     ...
     *
     *  @param _tokenId                   ERC1155 token Id.
     *  @param _iMasRewardsPerUnitTime    New rewards per unit time.
     */
    function setRewardsPerUnitTime(uint256 _tokenId, uint256 _iMasRewardsPerUnitTime, uint256 _iMasXRewardsPerUnitTime) external virtual {
        if (!_canSetStakeConditions()) {
            revert("Not authorized");
        }
        ...
        StakingCondition memory condition = _nextConditionId == 0
            ? defaultCondition[nextDefaultConditionId - 1]
            : stakingConditions[_tokenId][_nextConditionId - 1];
        require(_iMasRewardsPerUnitTime != condition.iMasRewardsPerUnitTime, "Reward unchanged.");

        _setStakingCondition(_tokenId, condition.timeUnit, _iMasRewardsPerUnitTime, _iMasXRewardsPerUnitTime);

        emit UpdatedIMasRewardsPerUnitTime(_tokenId, condition.iMasRewardsPerUnitTime, _iMasRewardsPerUnitTime);
        emit UpdatedIMasXRewardsPerUnitTime(_tokenId, condition.iMasXRewardsPerUnitTime, _iMasXRewardsPerUnitTime);
    }
    ...
    function setDefaultTimeUnit(uint256 _defaultTimeUnit) external virtual {
        ...
        _setDefaultStakingCondition(_defaultTimeUnit, _defaultCondition.iMasRewardsPerUnitTime, _defaultCondition.iMasXRewardsPerUnitTime);
     ...
     *  @dev     Only admin/authorized-account can call it.
     *
     *  @param _defaultIMasRewardsPerUnitTime     New IMas rewards per unit time.
     *  @param _defaultIMasXRewardsPerUnitTime    New IMasX rewards per unit time.
     */
    function setDefaultRewardsPerUnitTime(uint256 _defaultIMasRewardsPerUnitTime, uint256 _defaultIMasXRewardsPerUnitTime) external virtual {
        if (!_canSetStakeConditions()) {
            revert("Not authorized");
        }

        StakingCondition memory _defaultCondition = defaultCondition[nextDefaultConditionId - 1];
        require(_defaultIMasRewardsPerUnitTime != _defaultCondition.iMasRewardsPerUnitTime, "Default reward unchanged.");
        require(_defaultIMasXRewardsPerUnitTime != _defaultCondition.iMasXRewardsPerUnitTime, "Default reward unchanged.");

        _setDefaultStakingCondition(_defaultCondition.timeUnit, _defaultIMasRewardsPerUnitTime, _defaultIMasXRewardsPerUnitTime);

        emit UpdatedDefaultIMasRewardsPerUnitTime(_defaultCondition.iMasRewardsPerUnitTime, _defaultIMasRewardsPerUnitTime);
        emit UpdatedDefaultIMasXRewardsPerUnitTime(_defaultCondition.iMasXRewardsPerUnitTime, _defaultIMasXRewardsPerUnitTime);
    }
     ...
     *  @return _iMasRewards    Available IMas reward amount.
     *  @return _iMasXRewards   Available IMasX reward amount.
     */
        ...
        returns (uint256 _tokensStaked, uint256 _iMasRewards, uint256 _iMasXRewards)
    {
        ...
        _iMasRewards = _availableIMasRewards(_tokenId, _staker);
        _iMasXRewards = _availableIMasXRewards(_tokenId, _staker);
    }
     ...
     *  @return _totalIMasRewards    Total IMas rewards available.
     *  @return _totalIMasXRewards   Total IMasX rewards available.
     */
        ...
        )
    {
        ...
        for (uint256 i = 0; i < indexedTokenCount; i++) {
            if (_stakedAmounts[i] > 0) {
                ...
                _totalIMasRewards += _availableIMasRewards(_indexedTokens[i], _staker);
                _totalIMasXRewards += _availableIMasXRewards(_indexedTokens[i], _staker);
                count += 1;
            }
        }
    }
    ...
    function getIMasRewardsPerUnitTime(uint256 _tokenId) public view returns (uint256 _iMasRewardsPerUnitTime) {
        uint256 _nextConditionId = nextConditionId[_tokenId];
        require(_nextConditionId != 0, "Rewards not set. Check default rewards.");
        _iMasRewardsPerUnitTime = stakingConditions[_tokenId][_nextConditionId - 1].iMasRewardsPerUnitTime;
    }

    function getIMasXRewardsPerUnitTime(uint256 _tokenId) public view returns (uint256 _iMasXRewardsPerUnitTime) {
        uint256 _nextConditionId = nextConditionId[_tokenId];
        require(_nextConditionId != 0, "Rewards not set. Check default rewards.");
        _iMasXRewardsPerUnitTime = stakingConditions[_tokenId][_nextConditionId - 1].iMasXRewardsPerUnitTime;
    }
    ...
    function getDefaultIMasRewardsPerUnitTime() public view returns (uint256 _iMasRewardsPerUnitTime) {
        _iMasRewardsPerUnitTime = defaultCondition[nextDefaultConditionId - 1].iMasRewardsPerUnitTime;
    }

    function getDefaultIMasXRewardsPerUnitTime() public view returns (uint256 _iMasXRewardsPerUnitTime) {
        _iMasXRewardsPerUnitTime = defaultCondition[nextDefaultConditionId - 1].iMasXRewardsPerUnitTime;
    }
    ...
    function _stake(uint256 _tokenId, uint256 _amount) internal virtual {
        ...
        if (stakers[_tokenId][_stakeMsgSender()].amountStaked > 0) {
            _updateUnclaimedIMasRewardsForStaker(_tokenId, _stakeMsgSender());
            _updateUnclaimedIMasXRewardsForStaker(_tokenId, _stakeMsgSender());
        } else {
    ...
    /// @dev Logic for claiming IMas rewards. Override to add custom logic.
    function _claimIMasRewards(uint256 _tokenId) internal virtual {
        uint256 iMasRewards = stakers[_tokenId][_stakeMsgSender()].unclaimedIMasRewards +
            _calculateIMasRewards(_tokenId, _stakeMsgSender());

        require(iMasRewards != 0, "No IMas rewards");

        stakers[_tokenId][_stakeMsgSender()].timeOfLastUpdate = block.timestamp;
        stakers[_tokenId][_stakeMsgSender()].unclaimedIMasRewards = 0;
        ...
        _mintIMasRewards(_stakeMsgSender(), iMasRewards);

        emit IMasRewardsClaimed(_stakeMsgSender(), iMasRewards);
    }

    function _claimIMasXRewards(uint256 _tokenId) internal virtual {
        uint256 iMasXRewards = stakers[_tokenId][_stakeMsgSender()].unclaimedIMasXRewards +
            _calculateIMasXRewards(_tokenId, _stakeMsgSender());

        require(iMasXRewards != 0, "No IMasX rewards");

        stakers[_tokenId][_stakeMsgSender()].timeOfLastUpdate = block.timestamp;
        stakers[_tokenId][_stakeMsgSender()].unclaimedIMasXRewards = 0;
        ...
        _mintIMasXRewards(_stakeMsgSender(), iMasXRewards);

        emit IMasXRewardsClaimed(_stakeMsgSender(), iMasXRewards);
    }    

    /// @dev View available rewards for a user.
    function _availableIMasRewards(uint256 _tokenId, address _user) internal view virtual returns (uint256 _rewards) {
        if (stakers[_tokenId][_user].amountStaked == 0) {
            _rewards = stakers[_tokenId][_user].unclaimedIMasRewards;
        } else {
            _rewards = stakers[_tokenId][_user].unclaimedIMasRewards + _calculateIMasRewards(_tokenId, _user);
        }
    }

    function _availableIMasXRewards(uint256 _tokenId, address _user) internal view virtual returns (uint256 _rewards) {
        if (stakers[_tokenId][_user].amountStaked == 0) {
            _rewards = stakers[_tokenId][_user].unclaimedIMasXRewards;
        } else {
            _rewards = stakers[_tokenId][_user].unclaimedIMasXRewards + _calculateIMasXRewards(_tokenId, _user);
        }
    }


    /// @dev Update unclaimed rewards for a users. Called for every state change for a user.
    function _updateUnclaimedIMasRewardsForStaker(uint256 _tokenId, address _staker) internal virtual {
        uint256 rewards = _calculateIMasRewards(_tokenId, _staker);
        stakers[_tokenId][_staker].unclaimedIMasRewards += rewards;
    ...
    function _updateUnclaimedIMasXRewardsForStaker(uint256 _tokenId, address _staker) internal virtual {
        uint256 rewards = _calculateIMasXRewards(_tokenId, _staker);
        stakers[_tokenId][_staker].unclaimedIMasXRewards += rewards;
        stakers[_tokenId][_staker].timeOfLastUpdate = block.timestamp;
    ...
    function _setStakingCondition(
        uint256 _tokenId,
        uint256 _timeUnit,
        uint256 _iMasRewardsPerUnitTime,
        uint256 _iMasXRewardsPerUnitTime
    ) internal virtual {
        ...
        if (conditionId == 0) {
            ...
            for (; conditionId < _nextDefaultConditionId; conditionId += 1) {
                ...
                stakingConditions[_tokenId][conditionId] = StakingCondition({
                    ...
                    iMasRewardsPerUnitTime: _defaultCondition.iMasRewardsPerUnitTime,
                    iMasXRewardsPerUnitTime: _defaultCondition.iMasXRewardsPerUnitTime,
        ...
        stakingConditions[_tokenId][conditionId] = StakingCondition({
            timeUnit: _timeUnit,
            iMasRewardsPerUnitTime: _iMasRewardsPerUnitTime,
            iMasXRewardsPerUnitTime: _iMasXRewardsPerUnitTime,
    ...
    function _setDefaultStakingCondition(uint256 _timeUnit, uint256 _iMasRewardsPerUnitTime, uint256 _iMasXRewardsPerUnitTime) internal virtual {
        require(_timeUnit != 0, "time-unit can't be 0");
    ...
    function _calculateIMasRewards(uint256 _tokenId, address _staker) internal view virtual returns (uint256 _rewards) {
        ...
        if (_nextConditionId == 0) {
            ...
            for (uint256 i = _stakerConditionId; i < _nextConditionId; i += 1) {
                ...
                (bool noOverflowProduct, uint256 rewardsProduct) = SafeMath.tryMul(
                    ...
                    condition.iMasRewardsPerUnitTime
                );
    ...
    function _calculateIMasXRewards(uint256 _tokenId, address _staker) internal view virtual returns (uint256 _rewards) {
        Staker memory staker = stakers[_tokenId][_staker];
        uint256 _stakerConditionId = staker.conditionIdOflastUpdate;
        uint256 _nextConditionId = nextConditionId[_tokenId];
        ...
        if (_nextConditionId == 0) {
            ...
            for (uint256 i = _stakerConditionId; i < _nextConditionId; i += 1) {
                ...
                (bool noOverflowProduct, uint256 rewardsProduct) = SafeMath.tryMul(
                    (endTime - startTime) * staker.amountStaked,
                    condition.iMasXRewardsPerUnitTime
                );
                (bool noOverflowSum, uint256 rewardsSum) = SafeMath.tryAdd(
                    _rewards,
                    rewardsProduct / condition.timeUnit
    ...
    /**
     *  @notice View total IMas rewards available in the staking contract.
     *
     */
    function getIMasRewardTokenBalance() external view virtual returns (uint256 _iMasRewardsAvailableInContract);

    /**
     *  @notice View total IMas rewards available in the staking contract.
     *
     */
    function getIMasXRewardTokenBalance() external view virtual returns (uint256 _iMasXrewardsAvailableInContract);
     ...
     * ```
     */
    function _mintIMasRewards(address _staker, uint256 _rewards) internal virtual;
     ...
     * ```
     */
    function _mintIMasXRewards(address _staker, uint256 _rewards) internal virtual;
