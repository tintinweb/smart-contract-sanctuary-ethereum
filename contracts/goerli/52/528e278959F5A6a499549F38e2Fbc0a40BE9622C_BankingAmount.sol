...
contract BankingAmount is AutomationCompatibleInterface {
    ...
    uint256 private BankTotalMoney;
    ...
    function withDraw() public {
        ...
        (bool sent, ) = payable(withdraw).call{value: address(this).balance}(
            ""
        );
    ...
    ) public override returns (bool upkeepNeeded, bytes memory /*0performData*/) {
        if(status == loan.ON) revert NotProvdingLoan();
        upkeepNeeded = (block.timestamp - lastTimeStamp) > i_interval;
    }

    function performUpkeep(bytes calldata performData) external override {
        if(peopleLoan[peopleLoan.length-1].loanStatus!=true){
