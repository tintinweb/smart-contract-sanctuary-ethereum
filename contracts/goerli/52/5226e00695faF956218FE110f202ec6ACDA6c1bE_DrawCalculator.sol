...
*/
contract DrawCalculator is Initializable, IDrawCalculator {
    ...
    function initialize(
        ...
        __DrawCalculator_init_unchained(_ticket, _drawBuffer, _prizeDistributionBuffer);
    }

    /// @notice Initialize for DrawCalculator
    ...
    function calculate(
        ...
        uint64[][] memory pickIndices = abi.decode(
            _pickIndicesForDraws,
            (uint64[][])
        );
        require(
            pickIndices.length == _drawIds.length,
            "DrawCalc/invalid-pick-indices-length"
        );
    ...
    function _calculatePrizesAwardable(
        ...
        internal
        view
        returns (uint256[] memory prizesAwardable, bytes memory prizeCounts)
    {
        uint256[] memory _prizesAwardable = new uint256[](
            _normalizedUserBalances.length
        );
        ...
        for (uint32 drawIndex = 0; drawIndex < _draws.length; drawIndex++) {
            require(
                timeNow <
                    _draws[drawIndex].timestamp +
                        _prizeDistributions[drawIndex].expiryDuration,
                "DrawCalc/draw-expired"
            );
    ...
    function _calculate(
        ...
        uint256[] memory _prizeCounts = new uint256[](
            _prizeDistribution.tiers.length
        );
        ...
        for (uint32 index = 0; index < picksLength; index++) {
            require(
                _picks[index] < _totalUserPicks,
                "DrawCalc/insufficient-user-picks"
            );

            if (index > 0) {
                require(
                    _picks[index] > _picks[index - 1],
                    "DrawCalc/picks-ascending"
                );
            }
    ...
    function _calculateTierIndex(
        ...
        for (uint8 matchIndex = 0; matchIndex < masksLength; matchIndex++) {
            ...
            if (
                (_randomNumberThisPick & mask) != (_winningRandomNumber & mask)
            ) {
    ...
    function _createBitMasks(
        ...
        uint256[] memory masks = new uint256[](
            _prizeDistribution.matchCardinality
        );
        masks[0] = (2 ** _prizeDistribution.bitRangeSize) - 1;

        for (
            uint8 maskIndex = 1;
            maskIndex < _prizeDistribution.matchCardinality;
            maskIndex++
        ) {
    ...
    function _numberOfPrizesForIndex(
        ...
        if (_prizeTierIndex > 0) {
            return
                (1 << (_bitRangeSize * _prizeTierIndex)) -
                (1 << (_bitRangeSize * (_prizeTierIndex - 1)));
        } else {
            return 1;
        }
    }
...
interface IPrizeDistributor {
     ...
     */
    function setDrawCalculator(IDrawCalculator newCalculator)
        external
