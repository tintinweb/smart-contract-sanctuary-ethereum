...
contract ZKBridge is Governance {
    using RLPReader for RLPReader.RLPItem;
    using RLPReader for bytes;

    event LogMessagePublished(address indexed sender, uint16 indexed dstChainId, address dstContractAddress, uint64 sequence, bytes payload);

    bytes32 public constant MESSAGE_TOPIC = 0xc6de59b31998f775aa9d0995620ff4ac76172e89fa686f6b2420d49e78db9133;
    ...
    function send(uint16 dstChainId, address dstContractAddress, bytes memory payload) public payable returns (uint64 sequence) {
        sequence = useSequence(msg.sender);
        emit LogMessagePublished(msg.sender, dstChainId, dstContractAddress, sequence, payload);
    }

    function validateTransactionProof(uint16 srcChainId, bytes32 srcBlockHash, bytes memory mptProof) public view returns (Structs.VM memory, bool, string memory) {
        Structs.VM memory vm;
        IZkMptVerifier.Receipt memory receipt = zkMptVerifier(srcChainId).validateZkMPT(mptProof);
        if (receipt.state != 1) {
            return (vm, false, "transaction failure");
        }
        require(blockUpdater(srcChainId).checkBlock(srcBlockHash, receipt.receiptHash), "block header unsynchronized");
        (bool exist, bytes32 srcAddress, uint16 dstChainId, address dstAddress,uint64 sequence, bytes memory payload) = parseLog(receipt.logs, srcChainId);
        if (!exist) {
            return (vm, false, "permission denied");
        }
        require(dstChainId == chainId(), "invalid dstChainId");
        vm.hash = keccak256(abi.encode(srcChainId, srcBlockHash,srcAddress,sequence));
        vm.srcChainId = srcChainId;
        vm.srcAddress = srcAddress;
        vm.dstChainId = dstChainId;
        vm.dstAddress = dstAddress;
    ...
    function useSequence(address emitter) internal returns (uint64 sequence) {
        ...
        _setNextSequence(emitter, sequence + 1);
    }

    function parseLog(bytes memory logsByte, uint16 srcChainId) internal view returns (bool exist,
        bytes32 sender,
        uint16 dstChainId,
        address dstAddress,
        uint64 sequence,
        bytes memory payload){
        uint16 chainId = srcChainId;
        RLPReader.RLPItem[] memory logs = logsByte.toRlpItem().toList();
        (exist, sender, dstChainId, dstAddress, sequence, payload) = verifyLog(logs, chainId);
        if (!exist) {
            for (uint256 i = 3; i < logs.length; i++) {
                RLPReader.RLPItem[] memory logs2 = logs[i].toRlpBytes().toRlpItem().toList();
                (exist, sender, dstChainId, dstAddress, sequence, payload) = verifyLog(logs2, chainId);
                if (exist) {
                    break;
                }
            }
    ...
    function verifyLog(RLPReader.RLPItem[] memory logs, uint16 srcChainId) internal view returns (
        bool exist,
        bytes32 sender,
        uint16 dstChainId,
        address dstAddress,
        ...
        if (topic == MESSAGE_TOPIC && zkBridgeContracts(srcChainId) == contract_address) {
            exist = true;
            sender = abi.decode(topicItem[1].toBytes(), (bytes32));
            dstChainId = abi.decode(topicItem[2].toBytes(), (uint16));
            (dstAddress, sequence, payload) = abi.decode(logs[2].toBytes(), (address, uint64, bytes));
        }
    }
...
abstract contract Governance is Setters, Getters, ERC1967Upgrade {
    ...
    function setZkMptVerifier(uint16 chainId, address zkMptVerifier) public onlyOwner {
        _setZkMptVerifier(chainId, zkMptVerifier);
    }
...
contract Setters is State {
    ...
    function _setNextSequence(address emitter, uint64 sequence) internal {
        _state.sequences[emitter] = sequence;
    }

    function _setZkMptVerifier(uint16 chainId,address zkMptVerifier) internal {
        _state.zkMptVerifiers[chainId] = zkMptVerifier;
    }
...
import "./State.sol";
import "./interfaces/IZkMptVerifier.sol";
...
contract Getters is State {
    ...
    function zkMptVerifier(uint16 chainId) public view returns (IZkMptVerifier) {
        return IZkMptVerifier(_state.zkMptVerifiers[chainId]);
    }
...
contract Storage {
    struct ZKBridgeState {
        ...
        mapping(uint16 => address) zkMptVerifiers;
