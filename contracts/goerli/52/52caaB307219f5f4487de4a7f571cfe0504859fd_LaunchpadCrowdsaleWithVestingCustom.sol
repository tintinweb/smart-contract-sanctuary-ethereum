...
import "./VestedCustomCrowdsale.sol";
import "./interfaces/ILaunchpadCrowdsaleWithVestingCustom.sol";

/**
 * @title LaunchpadCrowdsaleWithVestingCustom
 * @author Enjinstarter
 * @dev Launchpad crowdsale where there is no delivery of tokens in each purchase.
 */
contract LaunchpadCrowdsaleWithVestingCustom is
    VestedCustomCrowdsale,
    CappedTokenSoldCrowdsaleHelper,
    TimedCrowdsaleHelper,
    LaunchpadWhitelistCrowdsaleHelper,
    Pausable,
    ILaunchpadCrowdsaleWithVestingCustom
{
    ...
    constructor(
        ...
        VestedCustomCrowdsale(crowdsaleInfo.vestingContract)
    ...
    modifier onlyBy(address account) {
        require(
            msg.sender == account,
            "LaunchpadCrowdsaleWithVestingCustom: sender unauthorized"
        );
        _;
    }
    ...
    ) external override onlyBy(crowdsaleAdmin) {
        require(
            startTimestamps.length > 0,
            "LaunchpadCrowdsaleWithVestingCustom: empty"
        );
        require(
            startTimestamps[0] > closingTime(),
            "LaunchpadCrowdsaleWithVestingCustom: must be after closing time"
        );
        _setVestingSchedule(startTimestamps, percentReleases, durationsDays);
    }
        ...
        onlyBy(governanceAccount)
    {
        require(
            account != address(0),
            "LaunchpadCrowdsaleWithVestingCustom: zero account"
        );
        ...
        isWhitelisted(beneficiary)
    {
        ...
        if (
            ...
            revert(
                "LaunchpadCrowdsaleWithVestingCustom: beneficiary cap exceeded"
            );
        }
...
abstract contract VestedCustomCrowdsale is Crowdsale {
    ...
    function _setVestingSchedule(
        ...
        IVestingCustom(vestingContract).setVestingSchedule(
            startTimestamps,
            percentReleases,
            durationsDays
        );
    ...
    function _deliverTokens(address beneficiary, uint256 tokenAmount)
        ...
        override
    {
        IVestingCustom(vestingContract).addVestingGrant(
            beneficiary,
            tokenAmount,
            true
        );
...
/**
 * @title ILaunchpadCrowdsaleWithVestingCustom
 * @author Enjinstarter
 */
interface ILaunchpadCrowdsaleWithVestingCustom is ICrowdsale {
