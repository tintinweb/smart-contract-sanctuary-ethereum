    ...
    OwnableUpgradeable
{
    ...
    uint16 public stageCanSplit;
    uint256 public amountCanBoost;

    function setIsOpenFeatureSplitNanoCell(bool value) external onlyTimelock {
        isOpenFeatureSplitNanoCell = value;
    }

    function setAmountCanBoostCell(uint256 value) external onlyTimelock {
        amountCanBoost = value;
    }
    ...
    function setStageCanSplit(uint16 value) external onlyTimelock {
        stageCanSplit = value;
    }
    ...
    function setMetaCell(address newMetaCell) external onlyTimelock {
        metaCell = newMetaCell;
    }
    ...
    function _evolve(
        ...
        uint256 currentStage = cell.stage;
        ...
        } else if (currentStage <= stageCanSplit) {
            cell.class = CellData.Class.COMMON;
        }
        else {
        ...
        isAllowedToBoost(_tokenID, msg.sender)
    {
        ...
        if (amountCanBoost != 0) {
            require(_amount <= amountCanBoost, "Boost cell over limit");
        }
        CellData.Cell memory cell = ICellRepository(metaCell).getMetaCell(
            _tokenID
        );
        ...
        returns (uint256)
    {
        ...
        uint256 amount = (cell.nextEvolutionBlock - block.number) * getBoostPerBlockPrice();

        if (amountCanBoost == 0) {
            return amount;
        }
        return amount < amountCanBoost ? amount : amountCanBoost;
    }
