...
contract Tiro is ERC721Enumerable, Ownable {
    ...
    Counters.Counter private _tokenIdCounter;
    string private _baseUri;
    address private _mintPassContract;
    
    uint256 public constant MAX_SUPPLY = 20;
    uint256 public constant GOLD_MINT_PASS_MAX = 3;
    ...
    bool public isSalesActive = true;
    bool public isUnreveal = true;

    mapping(address => bool) private _wlAccess;
    mapping(uint256 => uint256) private _tiroState;
    mapping(uint256 => string) private _stateUri;
    mapping(uint256 => string) private _stateTokenSuffix;
    mapping(uint256 => address) private _usedMintPass;

    constructor(string memory baseUri, address mintPassContract) ERC721("Time Is Running Out (TIRO)", "TIRO") {
        _baseUri = baseUri;
    ...
    function mint(uint quantity, uint256[] memory mintPassTokenIds) external payable {
        ...
        if (mintPassTokenIds.length > 0) {
            // get mint pass tokens owned by msg.sender
            uint256[] memory ownerMintPassTokenIds = walletOfMintPassOwner(msg.sender);
            require(ownerMintPassTokenIds.length > 0, "Sender does not own any mint pass");
            require(ownerMintPassTokenIds.length >= mintPassTokenIds.length, "Sender cannot own less mint pass than what being submitted");

            for(uint256 i=0; i < mintPassTokenIds.length; i++) {
                bool ownMintPass = false;
                for (uint256 j=0; j < ownerMintPassTokenIds.length; j++) {
                    if (ownerMintPassTokenIds[j] == mintPassTokenIds[i]) {
                        ownMintPass = true;
                        // if mint pass is owned by msg.sender and not being used yet
                        require(usedMintPassByTokenId(mintPassTokenIds[i]) == address(0), "At least one of the Mint Passes had already been used");
                        break;
                    }
                }
                require(ownMintPass == true, "Sender is not a rightful owner of the submitted Mint Passes");
            }
    ...
    function _buildTokenURI(uint256 tokenId) internal view returns (string memory) {
        uint256 tokenCurrentState = _tiroState[tokenId];
        string memory stateBaseUri = _stateUri[tokenCurrentState];
        string memory stateSuffix = _stateTokenSuffix[tokenCurrentState];
            ...
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        ...
                        "<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' version='1.1' viewBox='0 0 350 350' preserveAspectRatio='xMidYMid meet'>",
                        "<image width='350' height='350' xlink:href='", stateBaseUri, Strings.toString(tokenId), stateSuffix, "' />",
                        "</svg>"
                    )
                )
            )
            ...
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                "{'name':'TIRO #", Strings.toString(tokenId), "', 'image':'", image, unicode"', 'animation_url':'", image, unicode"', 'description': 'TIRO'}"
                            )
                        )
                    )
                )
    ...
    // return all mint pass token IDs minted by _owner
    function walletOfMintPassOwner(address _owner) public view returns(uint256[] memory) {
        return ProjectMaybeMintPass(_mintPassContract).walletOfOwner(_owner);
    }
    ...
    function tokenState(uint256 tokenId) external view returns (uint256) {
        return _tiroState[tokenId];
    }

    function setTokenState(uint256 tokenId, uint256 state) external onlyOwner {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        _tiroState[tokenId] = state;
    }
    ...
    function stateTokenUri(uint256 state) external view returns (string memory) {
        return _stateTokenSuffix[state];
    }

    function setStateTokenUri(uint256 state, string memory newStateTokenUri) external onlyOwner {
        _stateTokenSuffix[state] = newStateTokenUri;
    }

    function stateUri(uint256 state) external view returns (string memory) {
        return _stateUri[state];
    }

    function setStateUri(uint256 state, string memory newStateUri) external onlyOwner {
        _stateUri[state] = newStateUri;
    }

    function toggleSales(bool mintpass, bool wl, bool sales, bool unreveal, uint256 newPrice) external onlyOwner {
