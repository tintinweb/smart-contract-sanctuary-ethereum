...
contract ERC20Reward is ExtendedERC20, IRewardToken {
    ...
    */
    ...
    function getRewardData(address account)
        external
        view
        returns (
            uint256 futureAcc,
    ...
    {
        (futureAcc, futureStaked, lastUpdate) = _getFutureAcc(block.number + 1);
        rangeReward = _getRangeReward(lastUpdate, block.number + 1);
        if (futureStaked > 0) {
            futureAcc += (rangeReward * ACC_ADJUST) / futureStaked;
        }
        lockupPeriodForUser = _lookupLockupPeriodForUser(account, block.number);
        return (futureAcc, futureStaked, lastUpdate, block.number + 1, lockupPeriodForUser, rangeReward);
    }
