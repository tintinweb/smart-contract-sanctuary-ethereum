...
interface IE7LAutoSync {
    function syncMany(uint256[] memory tokenIds) external;
}

contract E7LAutoSync is ERC721Linkable, IE7LAutoSync {
    constructor(
        string memory _name,
    ...
    function isSynced(uint256 tokenId) public view returns (bool) {
        return super.ownerOf(tokenId) ==
            parentContract.ownerOf(_tokensInfo[tokenId].parentTokenId);
    }

    function isLinked(uint256 tokenId) public view returns (bool) {
        return _tokensInfo[tokenId].linked;
    }

    function syncMany (uint256[] memory tokenIds) public override{
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];

            super.syncToken(tokenId);
        }
    }
    function checker() external view returns (bool canExec, bytes memory execPayload) {
        uint256[] memory tokensToSync = new uint256[](totalSupply);
        uint256 counter = 0;

        unchecked {
            for (uint256 tokenId = 1; tokenId <= totalSupply; tokenId++) {
                if (!isSynced(tokenId) && isLinked(tokenId)) {
                    tokensToSync[counter++] = tokenId;
                }
            }
        }

        if (counter == 0) {
            return (false, "No tokens to sync");
        }

        canExec = true;
        execPayload = abi.encodeCall(IE7LAutoSync.syncMany, tokensToSync);
    }
...
abstract contract ERC721Linkable is ERC721, IERC721Linkable {
    ...
    mapping(uint256 => LinkableToken) internal _tokensInfo;
