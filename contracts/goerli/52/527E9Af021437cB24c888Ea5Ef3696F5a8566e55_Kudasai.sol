...
contract Kudasai is ERC721Enumerable, ERC721Royalty, Ownable {
    enum Parts {
        back,
        body,
        hair,
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        string memory _kudasaiOG = "";
        if (tokenId < _reserve) {
            _kudasaiOG = '"},{ "trait_type": "Special", "value": "KudasaiOG"';
        }
        string memory attributes = string(
            abi.encodePacked(
                '[{ "trait_type": "Background", "value": "',
                Storage(_imageStorage).getImageName(uint256(Parts.back), back),
                '"},{ "trait_type": "Body", "value": "',
                Storage(_imageStorage).getImageName(uint256(Parts.body), body),
                '"},{ "trait_type": "Hair", "value": "',
                Storage(_imageStorage).getImageName(uint256(Parts.hair), hair),
                '"},{ "trait_type": "Eyewear", "value": "',
                Storage(_imageStorage).getImageName(uint256(Parts.eyewear), eyewear),
                '"},{ "trait_type": "Face", "value": "',
                Storage(_imageStorage).getImageName(uint256(Parts.face), face),
                _kudasaiOG,
                '"}]'
            )
        );
