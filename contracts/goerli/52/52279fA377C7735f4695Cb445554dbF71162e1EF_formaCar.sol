...
contract formaCar is ERC721 {
    ...
    function mintWL(uint256 _amount, bytes32[] calldata _merkleProof) public {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(wlMint == true, "Wl mint is close");
        require(wlMinted + _amount <= maxToWlMint, "Wl size was reached");
        require(
            addresMintedWl[msg.sender] + _amount <= 2,
    ...
    function mintPublic(uint256 _amount) public payable {
        require(msg.value >= publicMintPrice * _amount, "Not enough ETH");
        require(publicMint == true, "Public mint didn't open");
        require(
            addresMintedPublic[msg.sender] + _amount <= 2,
