...
contract Defi is Ownable {
    ...
    function getAccountDividends(address account, address token) external view returns(uint256) {
        return _accountMap[account].dividends[token];
    }

    function getAccountDividendsWithdraw(address account, address token) external view returns(uint256) {
        return _accountMap[account].dividendsWithdraw[token];
    }

    function getAccountAwards(address account, address token) external view returns(uint256) {
        return _accountMap[account].awards[token];
    }

    function getDividendsToken() external view returns(address[] memory tokens) {
        tokens = new address[](_dividends.length());
        for (uint256 i = 0; i < _dividends.length(); i++) {
            tokens[i] = _dividends.at(0);
        }
    }
