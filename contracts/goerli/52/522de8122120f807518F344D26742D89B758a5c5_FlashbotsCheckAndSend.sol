...
contract FlashbotsCheckAndSend {
    ...
    // ======== view ========
    function checkBytes(address _target, bytes memory _payload) external view returns (bytes32){
        (, bytes memory _response) = _target.staticcall(_payload);
        return keccak256(_response);
    }
}