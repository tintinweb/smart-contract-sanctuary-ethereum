...
contract DaoStaking is Ownable, ReentrancyGuard {
    ...
    constructor(address _stakingToken, address _rewardsAdsToken, address _rewardsMeToken) {
    ...
    modifier checkTokenAddress(address token) {
        require(token == address(rewardsAdsToken) || token == address(rewardsMeToken), "Invalid Reward Token Address");
        _;
    }
    ...
    function addRewardType(address _rewardToken, uint256 _rewardDuration, uint256 _rewardAmount, uint256 _stakeAmount) external onlyOwner checkTokenAddress(_rewardToken){
    ...
    function updateRewardType(uint256 _id, uint256 _rewardAmount, uint256 _rewardDuration) external onlyOwner {
    ...
    function updateWithdrawFee(uint256 _id, uint256 _feeAmount) external onlyOwner {
        WithdrawFee storage withdrawFee = withdawFees[_id];
        withdrawFee.withdrawFeeAmount = _feeAmount;
    }

    function stake(uint256 _stakeAmount, uint256 _rewardType) external checkZeroAddress{
    ...
    function withdraw(uint256 _id) external checkZeroAddress{
        ...
        uint256 diffTime = block.timestamp - stakerReward.stakeTime;
        RewardType storage rewardType = rewardsTypes[stakerReward.rewardType];
        require(diffTime >= rewardType.rewardDuration, "Stake is not finished yet");
        ...
        uint256 withdrawAmount = stakerReward.stakeAmount - (stakerReward.stakeAmount / rewardType.stakeAmount) * feeAmount;
        stakingToken.transfer(msg.sender, withdrawAmount);

        uint256 rewardAmount = (stakerReward.stakeAmount / rewardType.stakeAmount) * rewardType.rewardAmount;
        if(rewardType.rewardToken == address(rewardsAdsToken)) {
            rewardsAdsToken.transfer(msg.sender, rewardAmount);
        } else if(rewardType.rewardToken == address(rewardsMeToken)) {
