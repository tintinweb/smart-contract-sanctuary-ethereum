...
error PokemonMarketplace__TokenNotApprovedForMarketplace();
error PokemonMarketplace__TokenAlreadyListed();
error PokemonMarketplace__InsufficientPrice();
error PokemonMarketplace__ItemNotListed();
error PokemonMarketplace__NotOwner();
error PokemonMarketplace__SellerNotOwner();
error PokemonMarketplace__PriceNotMet();
error PokemonMarketplace__NoProceeds();
error PokemonMarketplace__WithdrawFailed();
error PokemonMarketplace__NullishAddress();

/**
 * @title NFT marketplace of AI-generated pokemons
...
contract PokemonMarketplace is ReentrancyGuard, Ownable {
    ...
    event ItemRemoved(uint256 indexed tokenId, address indexed owner);
    ...
    constructor(address nftAddress) {
        ...
        if (nftAddress == address(0)) {
            revert PokemonMarketplace__NullishAddress();
        }
     ...
     * @dev Only server is able to list new items on request of the user
     * @param seller Whose behalf the server is going
     * to list an item on
    ...
    function listItem(
        ...
        if (nftContract.getApproved(tokenId) != address(this)) {
            revert PokemonMarketplace__TokenNotApprovedForMarketplace();
        }

        if (nftContract.ownerOf(tokenId) != seller) {
            revert PokemonMarketplace__SellerNotOwner();
        }

        if (s_listings[tokenId].price != 0) {
            revert PokemonMarketplace__TokenAlreadyListed();
        }

        if (price <= 0) {
            revert PokemonMarketplace__InsufficientPrice();
        }

        emit ItemListed(tokenId, seller, price);

        s_listings[tokenId] = Listing(seller, price);
    }

    /**
     * @notice Buys an item and transfers ownership to the buyer
     * @dev This action can be performed by anyone
     * @param tokenId Token id derived from the nft contract
     */
    function buyItem(uint256 tokenId) public payable nonReentrant {
        ...
        if (item.price == 0) {
            revert PokemonMarketplace__ItemNotListed();
        }

        if (msg.value < item.price) {
            revert PokemonMarketplace__PriceNotMet();
        }
    ...
    function withdrawProceeds() public nonReentrant {
        if (s_proceeds[msg.sender] == 0) {
            revert PokemonMarketplace__NoProceeds();
        }
        ...
        if (!success) {
            revert PokemonMarketplace__WithdrawFailed();
        }
    }
    ...
    function removeItem(uint256 tokenId) public nonReentrant {
        ...
        if (msg.sender != item.seller) {
            revert PokemonMarketplace__NotOwner();
        }
    ...
    /**
     * @dev Renounce is disabled for due to the absence of need
     */
