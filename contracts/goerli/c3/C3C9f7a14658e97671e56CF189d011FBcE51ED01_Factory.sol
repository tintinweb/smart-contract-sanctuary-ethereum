...
library Accountant {
  ...
  function calculatePrincipalAccrued(ICreditLine cl, uint256 balance, uint256 timestamp) public view returns (uint256) {
  ...
  function lateFeeApplicable(CreditLine cl, uint256 timestamp, uint256 gracePeriodInDays) public view returns (bool) {
...
contract Factory is BaseUpgradeablePausable {
  Config public config;
  ...
  function initialize(address owner, Config _config) public initializer {
...
contract Go is IGo, BaseUpgradeablePausable {
  ...
  Config public legacyGoList;
  uint256[11] public allIdTypes;
  event ConfigUpdated(address indexed who, address configAddress);

  function initialize(address owner, Config _config, address _uniqueIdentity) public initializer {
    require(
   ...
   * to the null address.
   */
  function setLegacyGoList(Config _legacyGoList) external onlyAdmin {
   ...
   * account's status on the legacy go-list maintained on Config.
   * @param account The account whose go status to obtain
  ...
  function goOnlyIdTypes(address account, uint256[] memory onlyIdTypes) public view override returns (bool) {
    ...
    Config goListSource = _getLegacyGoList();
  ...
  function _getLegacyGoList() internal view returns (Config) {
...
contract Zapper is BaseUpgradeablePausable {
  ...
  function zapStakeToCurve(uint256 tokenId, uint256 fiduAmount, uint256 usdcAmount) public whenNotPaused nonReentrant {
...
contract Borrower is BaseUpgradeablePausable, BaseRelayRecipient, IBorrower {
   ...
   */
  function drawdown(address poolAddress, uint256 amount, address addressToSendTo) external onlyAdmin {
...
interface IOneSplit {
  ...
  ) external view returns (uint256 returnAmount, uint256 estimateGasAmount, uint256[] memory distribution);
...
import { Config } from "../protocol/core/Config.sol";
  ...
  ICurveLP
{
  ...
  constructor(uint256 initialSupply, uint8 decimals, Config _config) public {
    _setupDecimals(decimals);
    _mint(msg.sender, initialSupply);
    config = _config;
  }
  ...
  /// @dev Left unimplemented because we're only using this in mainnet forking tests
  function exchange(uint256 i, uint256 j, uint256 dx, uint256 min_dy) public override returns (uint256) {
    return 0;
  }
...
import "../protocol/core/Config.sol";

contract TestConfig is Config {
...
contract TestTheConfig {
  ...
  address public factoryAddress = 0x0afFE1972479c386A2Ab21a27a7f835361B6C0e9;
  ...
  address public configAddress = address(8);
  ...
  function validateTheEnums(address _configAddress) public {
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.TransactionLimit), 1);
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.TotalFundsLimit), 2);
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.MaxUnderwriterLimit), 3);
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.ReserveDenominator), 4);
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.WithdrawFeeDenominator), 5);
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.LatenessGracePeriodInDays), 6);
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.LatenessMaxDays), 7);
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.DrawdownPeriodInSeconds), 8);
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.TransferRestrictionPeriodInDays), 9);
    Config(_configAddress).setNumber(uint256(ConfigOptions.Numbers.LeverageRatio), 10);

    Config(_configAddress).setAddress(uint256(ConfigOptions.Addresses.Fidu), fiduAddress);
    Config(_configAddress).setAddress(uint256(ConfigOptions.Addresses.Pool), poolAddress);
    Config(_configAddress).setAddress(uint256(ConfigOptions.Addresses.CreditDesk), creditDeskAddress);
    Config(_configAddress).setAddress(uint256(ConfigOptions.Addresses.Factory), factoryAddress);
    Config(_configAddress).setAddress(uint256(ConfigOptions.Addresses.TrustedForwarder), trustedForwarderAddress);
    Config(_configAddress).setAddress(uint256(ConfigOptions.Addresses.CUSDCContract), cUSDCAddress);
    Config(_configAddress).setAddress(uint256(ConfigOptions.Addresses.Config), configAddress);
    Config(_configAddress).setAddress(uint256(ConfigOptions.Addresses.FiduUSDCCurveLP), fiduUSDCCurveLPAddress);
    Config(_configAddress).setAddress(
      uint256(ConfigOptions.Addresses.TranchedPoolImplementationRepository),
      tranchedPoolImplementationRepositoryAddress
    );

    Config(_configAddress).setTreasuryReserve(treasuryReserveAddress);
  }
}
