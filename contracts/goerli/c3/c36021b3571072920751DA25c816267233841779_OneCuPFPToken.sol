...
contract OneCuPFPToken is ERC721, ContractURI {
    ...
    */
    ...
    uint256 private constant burnSupply = 1111;
    ...
    */
    ...
    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        ...
        revealed = false;
        transferLock = true;
        notRevealedUri = "https://live---metadata-5covpqijaa-uc.a.run.app/metadata/7473";
    ...
    */
    ...
    function reveal(bool _revealed) external {
        revealed = _revealed;
    }
    ...
    */
    ...
    function setPioneerPass(address PioneerPassAddress) external {
        pioneerPass = PioneerPassAddress;
    }

    function setBurnActive(bool burnIt) external {
        burnActive = burnIt;
    }

    function setEarlyBirdsEvent(bool crankIt) external {
    ...
    */
    ...
    function setWhitelist(bytes32 root) external {
        whitelistRoot = root;
    }
    ...
    function setActiveSales(bool preMint, bool mint) external {
        mintActive = mint;
        preMintActive = preMint;
    }


    function setMaxMint(uint256 amount) external {
        maxMint = amount;
    }

    function lock(bool lockTransfers) external {
    ...
    */
    ...
    /**
    *   When the contract receives a token id 1 from PioneerPass, it automatically burns it then mint a new token in this collection
    */
    ...
    function onERC1155Received(
        ...
        if (burnActive) {
            ...
            require(_userBurned[from] + value <= 10, "Max 10 burns per wallet");
            require((totalBurned + value) <= burnSupply, "Burn amount exceeds the available burnable supply");
            //burn the token

            totalBurned += value;

            _userBurned[from] += value;

            (bool burned,) = address(pioneerPass).call(
                abi.encodeWithSignature("burn(address,uint256,uint256)", address(this), burnedId, value)
            );
            require(burned, "Burn Failed");
            //2x1 the first 24 hrs
            if (earlyBirdsDiscount) {
                value += value;
            }
            //mints a token in this contract if the burn was successful
            for (uint bar = 0; bar < value; bar++) {
                totalMinted += 1;
                _safeMint(from, (totalMinted - 1));
            }
            //required return for safe transfer
    ...
    */
    ...
    function ValidateMint(
        ...
        if (_whitelisted) {
            ...
            require(mintActive, "No public sale active");
 ...
 * {ERC721Enumerable}.
 */
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC1155Receiver {
