...
contract EVEOTCStakes is Owner, ReentrancyGuard {
    ...
    struct Plan {
        ...
        uint8 smart_discount; // smart trades percentage discount to apply, from 0, to 100
        uint8 option_discount; // option trades percentage discount to apply, from 0, to 100
     ...
     * @param _smart_discount; smart trades percentage discount to apply, from 0, to 100
     * @param _option_discount; option trades percentage discount to apply, from 0, to 100
     ...
     *
     */ 
    function addPlan(uint256 _token_amount, uint8 _smart_discount, uint8 _option_discount, uint256 _minimum_time, uint256 _discount_duration) external isOwner {
        require(_token_amount > 0, "Minimum amount has to be greater than 0");
        require(_smart_discount <= 100, "Discount has to be equal or lower than 100");
        require(_option_discount <= 100, "Discount has to be equal or lower than 100");
        plans.push(Plan(_token_amount, _smart_discount, _option_discount, _minimum_time, _discount_duration, true));
    }
     ...
     * @param user user account to be queried
     */
    function getOptionDiscount(address user) external view returns (uint8 _discount ) {
        if (nft_option_discount > 0) {
            for (uint256 index = 0; index < nfts.length; index++) {
                if (nfts[index].balanceOf(user) > 0) {
                    return nft_option_discount;
                }
            }
        } else {
            if (ledger[user].active) {
                if (expiring_discount[user] > block.timestamp) {
                    return plans[ledger[user].plan].option_discount;
                } else {
                    return 0;
                }
            } else {
