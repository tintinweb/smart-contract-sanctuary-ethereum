...
contract TestRFI is Context, IERC20, Ownable {
    ...
    uint256 public maxTransactionAmount = 10000 * 10**18; //1% of total supply
    ...
    constructor (address _marketingWallet, address _opsWallet, address _farmContract) {
    ...
    function _transfer(
        ...
        if (
            ...
            if (to != owner() && to != address(uniswapV2Router) && to != address(uniswapV2Pair)){
                require(_holderLastTransferTimestamp[tx.origin] < block.number, "Only one purchase per wait time allowed.");
        ...
        if (
            ...
            swapAndSend(tokensForOps + tokensForMarketing + tokensForFarmRewards);
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (tRemainingFees > 0) {
                ...
                _rOwned[address(this)] += rRemainingFees;
                if(_isExcluded[address(this)])
                    _tOwned[address(this)] += tRemainingFees;

                emit Transfer(from, address(this), tRemainingFees);
    ...
    function swapAndSend(uint256 tokens) private {
        ...
        // fees distributed
        uint256 marketing = usdc.balanceOf(address(this)) * tokensForMarketing / tokens;
        uint256 ops = usdc.balanceOf(address(this)) * tokensForOps / tokens;
        uint256 farm = usdc.balanceOf(address(this)) - marketing - ops; // in case of rounding error give leftover as dividends
        ...
        usdc.transfer(farmContract, farm);
    ...
    function swapAndLiquify(uint256 liquidityTokens) private {
        ...
        uint256 newBalance = usdc.balanceOf(address(this)) - initialBalance;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 usdcAmount) private {
        ...
        uniswapV2Router.addLiquidity(
            ...
            0, // slippage is unavoidable
            owner(), // TODO: I THINK WE NEED TO CHANGE THIS
            block.timestamp
        );
    ...
    function updateFarmContract(address newWallet) external onlyOwner {
        farmContract = newWallet;
    }
