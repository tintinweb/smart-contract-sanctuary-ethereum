...
// File: contracts/MyContractCentralizedLowGas.sol
...
contract CentralizedMultiPrice is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public totalRaise = 60000;
    ...
    constructor() payable ERC721("__TOKEN_NAME__", "__TOKEN_SYMBOL__") {
    ...
    // Use getTierMintPrice(typeId) to replace this function, so that the contract can be under the size limit.
    function getMintPrice() public view returns (uint256) {
        return uint256(uint(unitRaise * 1e26 * 1129 / 1000) / uint(getLatestPrice()));
    }
    // If there's a tier requirement for the project, then this function should have a typeId input.
    // If typeId > 3, then use different formulas to calculate the price.
    ...
    function mintBatch(
        ...
        // One mint can have upto __MINT_LIMIT__ NFTs
        ...
        // typeId
        // 1: Normal
        // 2: Whitelist (we no longer have whitelist now)
        // 3: Purchased (paid by credit card, preordered, is a free mint member, etc.)
        // If we don't want to use typeId anymore, then we might use the number greater than 3 to represent the tiers.
        // So that 1 and 2 would be tier 1, and 4, 5, 6, etc. would be the other greater tiers.
        // Then, if the typeId is greater than 3, we should apply different price calculation function for each of the tiers.
        // And we also need to update the price calculating function to match the tiers.
        ...
        if (msg.value != 0) {
            payable(0x769ab204f1817C3B7338980AA93117767ca45170).transfer(msg.value);
        }
