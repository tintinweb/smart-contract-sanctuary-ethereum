...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => address[]) private buyersWaitingToTokens;
    ...
    function buy(uint256 _saleId, bytes32[] memory _merkleProof, uint256 _amount) external payable nonReentrant saleValid(_saleId) {
    ...
    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256 _amount, uint256 _price) private {
        ...
        if (_project.minSales == 0 && _project.isInstantPayment) {
            ...
            if (billInfo.account != _msgSender()) {
                project.setTotalBuyersWaitingToToken(_project.id, project.getTotalBuyersWaitingToToken(_project.id) + 1);
                buyersWaitingToTokens[_sale.id].push(_msgSender());
            }
    ...
    function close(uint256 _closeLimit, ProjectInfo memory _project, SaleInfo memory _sale, uint256 _totalBuyersWaitingClose, uint256 _totalCloseSale, bool _isGive) external onlyProject nonReentrant returns (uint256, uint256) {
        address[] memory buyersWaiting = getBuyersWaitingToTokens(_sale.id);
        for (uint256 i; i < buyersWaiting.length; i++) {
            _totalBuyersWaitingClose++;
            ...
            buyersWaitingToTokens[_sale.id].pop();
            if (getBuyersWaitingToTokens(_sale.id).length == 0) {
                _totalCloseSale++;
                sales[_sale.id].isClose = true;
            }
            if (_totalBuyersWaitingClose == _closeLimit) break;
        }

        return (_totalBuyersWaitingClose, _totalCloseSale);
    }
    ...
    function getBuyersWaitingToTokens(uint256 _saleId) public view returns (address[] memory) {
        return buyersWaitingToTokens[_saleId];       
    }
...
interface IProject {
    ...
    function setTotalBuyersWaitingToToken(uint256 _projectId, uint256 _total) external;
    function getTotalBuyersWaitingToToken(uint256 _projectId) external view returns (uint256);
