...
contract GtnStorage {
  ...
  event LeafAdded(uint256 indexed leafNumber, Leaf leaf);

  // The address authorized to add leaf
  address admin;
  ...
  /**
   * Hash the two values passed in param using the keccak hash function.
  ...
  function hashLeftRight(uint256 _left, uint256 _right) internal pure returns (uint256) {
    return uint256(keccak256(abi.encodePacked(_left, _right)));
  }
  ...
  function addLeaf(
    ...
    //todo: check if i am admin
    require(msg.sender == admin, 'Only the GTN admin can add a new leaf');
    ...
    Leaf memory leaf = Leaf(_ipfsCid, _leaf, _startDate, _endDate);

    leaves[nextLeafIndex] = leaf;
    emit LeafAdded(nextLeafIndex, leaf);
   ...
   * @return ipfsCid - the cid of the ipfs resource linked to the leaf
   * @return leafHash - the leaf value
   * @return startDate - the start time of the time window for this leaf
   * @return endDate - the end time of the time window for this leaf
   */
  function leafInfo(uint256 index)
    public
    view
    returns (
