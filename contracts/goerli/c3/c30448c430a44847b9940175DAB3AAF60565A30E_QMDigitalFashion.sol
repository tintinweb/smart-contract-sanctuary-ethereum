...
contract QMDigitalFashion is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, ERC1155URIStorageUpgradeable {
    ...
    /**
     * The default price of a mint.
     */
    uint256 public defaultPriceInUsd;
    /**
     * Overridden prices of a token in USD.
     */
    mapping(uint256 => uint256) public pricesInUsd;
    /**
     * Default supply per NFT.
     */
    uint256 public defaultMaxSupply;
    /**
     * Overridden supply per token id.
     */
    mapping(uint256 => uint256) public tokenMaxSupply;
    /**
     * Is NFT claimable by the holder of QM NFT.
     */
    ...
    /**
     * Sets the price in USD to a specific token.
     */
    function setPriceInUsd(uint256 tokenId, uint256 priceInUsd) public onlyOwner {
        pricesInUsd[tokenId] = priceInUsd;
    }
    ...
    function adminMint(
        ...
        uint256 priceInUsd,
        bool claimableByHolder
    ) public onlyOwner {
        createAndMintInternal(receiver, tokenId, 1, supply, metadataUrl, priceInUsd, claimableByHolder);
    }
    ...
    function createAndMint(
        ...
        validateStableCoins(stableCoinAddress, stableCoinAmount, tokenId);
    ...
    function validateStableCoins(
        ...
        require(stableCoinAmount == getPriceInTokens(tokenId), "Not enough tokens sent to mint");
     ...
     */
    function getPriceInTokens(uint256 tokenId) public view returns (uint256) {
        return getPriceInUsd(tokenId) * 10 ** 6;
    }

    /**
     * Returns the price of a token in USD.
     */
    function getPriceInUsd(uint256 tokenId) public view returns (uint256) {
        if (pricesInUsd[tokenId] > 0) {
            return pricesInUsd[tokenId];
        }
        return defaultPriceInUsd;
    }
    ...
    function createAndMintInternal(
        ...
        tokenMaxSupply[id] = supply;
        pricesInUsd[id] = priceInUsd;
        _setURI(id, metadataUrl);
        claimable[id] = claimableByHolder;
    }
    ...
    /**
     * Sets the max supply for the token id.
     */
