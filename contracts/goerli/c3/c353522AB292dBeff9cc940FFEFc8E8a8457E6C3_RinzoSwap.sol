...
contract RinzoSwap is SpecialTransferHelper, Ownable, ReentrancyGuard {
    ...
    function _trade(
        ...
        for (uint256 i = 0; i < _tradeDetails.length; i++) {
            // get market details
            (address _market, bool _isLib, bool _isActive) = marketRegistry.markets(_tradeDetails[i].marketId);
            // market should be active
            require(_isActive, "_trade: InActive Market");
            // execute trade
            // if (_market == PROJECT_WYVERN_EXCHANGE || _market == WYVERN_EXCHANGE) {
            //     // Project Wyvern Exchange or Wyvern Exchange Contract
            //     _market.call{value:_tradeDetails[i].value}(_tradeDetails[i].tradeData);
            // } else {
            //     (bool success, ) = _isLib
            ...
            if (_market == PROJECT_WYVERN_EXCHANGE || _market == WYVERN_EXCHANGE) {
                // Project Wyvern Exchange or Wyvern Exchange Contract
                RinzoProxy(payable(proxy)).trade{value:_tradeDetails[i].value}(_market, false, _tradeDetails[i].tradeData, _tradeDetails[i].value);
            } else {
                (bool success, ) = _isLib
                    ? RinzoProxy(payable(proxy)).trade(_market, true, _tradeDetails[i].tradeData, _tradeDetails[i].value)
                    : RinzoProxy(payable(proxy)).trade{value:_tradeDetails[i].value}(_market, false, _tradeDetails[i].tradeData, _tradeDetails[i].value);
                // check if the call passed successfully
                _checkCallResult(success);
            }
...
contract RinzoProxy is Ownable, ReentrancyGuard {
    address private _mainContract;

    constructor(address mainContract_) {
      _mainContract = mainContract_;
    }

    function setMainContract(address mainContract_) external onlyOwner {
      _mainContract = mainContract_;
    }

    function trade(address market, bool isLib, bytes memory tradeData, uint256 value) payable external nonReentrant returns (bool, bytes memory)
    {
        require(msg.sender == _mainContract, "Only can be called from rinzo");

        (bool success, bytes memory returnedData) = isLib
          ? market.delegatecall(tradeData)
          : market.call{value: value}(tradeData);
