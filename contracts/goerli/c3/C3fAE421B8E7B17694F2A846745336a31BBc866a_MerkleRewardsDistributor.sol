...
library MerkleUtils {
    function containedInTree(bytes32 merkleRoot, bytes memory data, bytes32[] memory nodes, uint256 index) internal pure returns(bool) {
...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => bool)) public claimedIndex;
    ...
    function claim(
        ...
        if(claimedIndex[currentEpoch][leafIndex]) revert RewardAlreadyClaimed();
        /// @dev make sure the merkle proof validates the claim
        if(!isInState(msg.sender, amountToClaim, merkleProof, leafIndex))
        ...
        claimedIndex[currentEpoch][leafIndex] = true;

        tokenContract.transfer(msg.sender, tokensToDistribute);

        emit Claimed(leafIndex, msg.sender, tokensToDistribute);
    }
    ...
    /**
     *  @notice Asks the tree whether or not a user was included with a specific
     *          index and amount of tokens that he is able to claim
     */
    function isInState(
        address userAddress,
        uint256 amountToClaim,
        ...
        return MerkleUtils.containedInTree(
            merkleRoot,
            node(userAddress, amountToClaim),
            merkleProof,
            leafIndex
        );
    }
     ...
     */
    function node(address account, uint256 amountToClaim)
        internal
        pure
        returns (bytes memory) {
        return
            abi.encodePacked(
                /// @dev This converts checksummed addresses to non-checksummed!
    ...
    function swapMultiple(
        ...
        for (uint256 i = 0; i < tokensAttributed.length;) {
            ...
            if(currentTokenToSwap == address(tokenContract)) {
                /// @dev we still need to apply fee and emit our event
