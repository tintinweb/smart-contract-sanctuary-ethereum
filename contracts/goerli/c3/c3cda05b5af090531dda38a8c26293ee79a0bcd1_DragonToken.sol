...
contract DragonToken is ERC721, Ownable{
    ...
    function LevelUP_MSP(address MSPContract, uint amount, uint tokenId) public returns(bool){
        ERC20 MSPToken = ERC20(MSPContract);
        require(MSPToken.balanceOf(msg.sender) > 1000, "NEED 1000 MSP! ");
        
        MSPToken.approve(msg.sender, 0);
        MSPToken.increaseAllowance(address(this), amount);
        MSPToken.transfer(address(this), amount);
        
        MSPToken.transferFrom(address(this), owner(), amount); 
