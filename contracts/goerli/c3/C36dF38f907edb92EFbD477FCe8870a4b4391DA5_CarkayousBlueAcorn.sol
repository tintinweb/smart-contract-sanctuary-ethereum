...
contract CarkayousBlueAcorn is LushLinkCronosUnlimitedDrop {
    ...
    constructor(address _ownerAddress) LushLinkCronosUnlimitedDrop(
        "Carkayous Blue Acorn",
        "BLUEACORN",
        "https://www.lushlink.io/collection/a0cyaFpXSDhxK0VYK0dLSUdzMEROQT09/nft",
        _ownerAddress
    ){
...
contract LushLinkCronosUnlimitedDrop is ERC721Enumerable, ERC2981, Hierarchy {
    ...
    mapping(address => uint256) allowListMinted;
    ...
    function allowListMint() public payable {
        require(isAllowListAddress(_msgSender()), "Caller is not allowed to mint");
        if (allowListMinted[_msgSender()] >= 1) {
            revert("Caller already minted");
        }

        uint256 totalCost = mintCost(_msgSender());

        require(totalCost <= msg.value, "Insufficient Funds");

        allowListMinted[_msgSender()] = 1;
        _mintNextId(_msgSender());
    }
    ...
    function addAllowListAddressBatch(address[] memory _addresses) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not allowed to add to allow list");
        for (uint i=0; i<_addresses.length; i++) {
            _mintNextId(_addresses[i]);
        }
    }

    function removeAllowListAddress(address _address) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not allowed to remove from allow list");
        if (allowListedAddresses[_address]) {
            delete allowListedAddresses[_address];
        }
    }

    function isAllowListAddress(address _address) public view returns(bool) {
        return allowListedAddresses[_address];
    }
