...
contract Sale is ReentrancyGuard, Ownable {
    ...
    function TokenToSell(address _token) public onlyOwner{
        SaleToken = _token;
    }

    function MinTokenToBuy(uint256 _amount) public onlyOwner{
    ...
    //     address prevValue = presale[presaleId].saleToken;
    //     presale[presaleId].saleToken = _newAddress;
    ...
    function setExcludeFromMinToken(address _user, bool _status)
        external
        onlyOwner
    {
    ...
    function setAggregator(address _add) external onlyOwner {
        aggregatorInterface = Aggregator(_add);
    }
    ...
    modifier checkSaleState(uint256 _id, uint256 amount) {
        require(
            block.timestamp >= presale[_id].startTime && presale[_id].Active ==true,
        ...
        returns (bool)
    {
        ...
        if (block.timestamp >= _presale.endTime) {
            ...
            bool status = IERC20(_presale.saleToken).transfer(
                msg.sender,
                amountToSend
            );
