    ...
    AccessControl
{
    ...
    uint256 public maxLunchboxes;
    uint256 public maxFreeLunchboxes;
    ...
    constructor(
        address _importantAddress,
        string memory _baseURI,
        uint256 _numFree,
        uint256 _maxMint
    ) ERC1155(_baseURI) Signing(_importantAddress) {
        importantAddress = _importantAddress;
        maxLunchboxes = _maxMint;
        maxFreeLunchboxes = _numFree;

        _grantRole(ADMIN_ROLE, msg.sender);
        _grantRole(ADMIN_ROLE, importantAddress);
    }
    ...
    function setMaxLunchboxes(uint256 _maxMint) public onlyRole(ADMIN_ROLE) {
        maxLunchboxes = _maxMint;
    }

    function setMaxFreeLunchboxes(uint256 _numSale)
        public
        onlyRole(ADMIN_ROLE)
    {
        maxFreeLunchboxes = _numSale;
    }

    function setSoldLunchboxes(uint256 _numSold) public onlyRole(ADMIN_ROLE) {
        currentSaleCount = _numSold;
    }

    function setCurrentLunchboxes(uint256 _numMint)
        public
        onlyRole(ADMIN_ROLE)
    {
        currentLunchboxes = _numMint;
    }
    ...
    function getCurrCost(uint256 count) public view returns (uint256) {
        ...
        if (!allowListEvent) {
            for (uint256 i = 0; i < count; i++) {
                if (currentSaleCount + i > maxFreeLunchboxes) {
                    //5000
                    total += .1 ether;
                }
            }
    ...
    function _mintSoul(
        ...
        require(
            currentLunchboxes + 1 <= maxLunchboxes, //MAX_LUNCHBOXES
