...
contract Metazoku is ERC721A, Ownable {
     ...
     * Transfers message value to contract owner
     */
    function mintPresale(uint256 amount, bytes32 hash, bytes calldata signature) external payable {
        require(_checkSignature(hash, signature), "Metazoku: invalid signature");
        require(!_isCollectionFreeze, "Metazoku: collection is freeze");
    ...
    /*
     * Allows to stake given NFT `id`. Staked token can not be transfer.
     *
     * Requirements:
     ...
     * @param `id` - token ID
     *
     * Emits `TokenStaked` event
     */
    function stakeToken(uint256 id) external {
        require(ownerOf(id) == msg.sender, "Metazoku: you are not the token owner");
        require(!_isTokenStaked(id), "Metazoku: token already staked");

        _stakedTokens.push(id);
        emit TokenStaked(id, msg.sender);
    }

    /*
     * Allows to unstake given NFT `id`.
     *
     * Requirements:
     * - caller should be token owner
     * - token should not be staked
     ...
     * @param `id` - token ID
     *
     * Emits `TokenUnstaked` event
     */
    function unstakeToken(uint256 id) external {
        require(ownerOf(id) == msg.sender, "Metazoku: you are not the token owner");
        require(_isTokenStaked(id), "Metazoku: token is not staked");

        for (uint256 i = 0; i < _stakedTokens.length; i++) {
            if (_stakedTokens[i] == id) {
                _stakedTokens[i] = _stakedTokens[_stakedTokens.length - 1];
                _stakedTokens.pop();
            }
        }

        emit TokenUnstaked(id, msg.sender);
    }
