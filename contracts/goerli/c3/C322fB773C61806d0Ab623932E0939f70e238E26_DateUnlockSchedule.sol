...
contract DateUnlockSchedule is
    ...
    IUnlockSchedule
{
    ...
    function __DateUnlockSchedule_init(ILock _lock, bytes calldata _data)
        internal
        onlyInitializing
    {
    ...
    function __DateUnlockSchedule_init_unchained(
        ILock _lock,
        ...
        require(
            _unlockTimestamps.length >= 2,
            "ERROR: Need more unlock timestamps"
        );
        require(
            _unlockTimestamps.length == _normalizedAmounts.length,
            "ERROR: Lenght of KeyPoints and amounts must be equal"
        );
        require(
            _checkAmounts(_normalizedAmounts),
            "ERROR: Amount are not correctly normalized"
        );
        require(
            _checkUnlockTimes(_unlockTimestamps),
            "ERROR: Key Points are not correctly created"
        );
        lockContract = _lock;
     ...
     * @return The unlocked tokens
     */
    function unlockedAmount(uint256 initialAmount)
        external
        view
        override
        returns (uint256)
    ...
    function nextRelease() external view returns (uint256 timestamp) {
        for (uint256 i; i < unlockTimestamps.length; ) {
            if (unlockTimestamps[i] > block.timestamp)
                return timestamp = unlockTimestamps[i];
            unchecked {
                i++;
            }
        }
    ...
    function _checkAmounts(uint256[] memory _amounts)
        internal
        pure
        returns (bool)
    {
        ...
        for (uint256 i; i < _amounts.length; ) {
            total = SafeMath.add(total, _amounts[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function _checkUnlockTimes(uint256[] memory _unlockTimes)
        internal
        view
        returns (bool)
    {
        for (uint256 i; i < _unlockTimes.length - 1; ) {
            if (
                _unlockTimes[i] < block.timestamp ||
                _unlockTimes[i] > _unlockTimes[i + 1]
            ) {
                return false;
            }
            unchecked {
