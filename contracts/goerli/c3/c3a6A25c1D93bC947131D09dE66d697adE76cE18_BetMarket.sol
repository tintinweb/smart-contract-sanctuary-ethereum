...
contract BetMarket {
    ...
    function addYesBet(uint256 _marketId, uint256 _value) public payable {
        require(_value <= ERC20(betToken).allowance(msg.sender, address(this)), "Not allowed to spend this amount.");
        Markets storage market = markets[_marketId];
        ERC20(betToken).transferFrom(msg.sender, address(this), _value);
        ...
        payable
    {
        ...
        if (eventOutcome) {
            ...
            for (uint256 i = 0; i < winningAddresses.length; i++) {
                ...
                ERC20(betToken).transfer(_address, winningAmount[_address]);
