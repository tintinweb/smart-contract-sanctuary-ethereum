...
contract SimpleTokenSwap {
    ...
    */
    ...
    function CheckERC20Balance(IERC20 token)
        view
        external
        returns (uint256)
    {
    ...
    function checkCondition(IERC20 token)
        external
        view
        returns (bool)
    {   
    ...
    */
        ...
        payable
    {
        ...
        for (uint i = 0; i < sellToken.length; i++){
            ...
            if(sellToken[i] == WETH && balance >= amount[i]){
                balance -= amount[i];
            }
            else{
