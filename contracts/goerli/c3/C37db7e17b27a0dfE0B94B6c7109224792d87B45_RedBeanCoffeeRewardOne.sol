...
contract RedBeanCoffeeRewardOne is ERC721A, Ownable, ReentrancyGuard {
    ...
    // ===== Quantity and Supply Check ======
    modifier checkSupplyAndQuantity(uint256 quantity) {
    ...
    // ===== Dev Mint =====
    function devMint(uint256 quantity) external onlyOwner checkSupplyAndQuantity(quantity) nonReentrant callerIsUser {
    ...
    // ===== Change Base URI =====
    function setBaseURI(string memory newBaseURI) external onlyOwner notNull(newBaseURI) nonReentrant callerIsUser {
        baseURI = newBaseURI;
    }
