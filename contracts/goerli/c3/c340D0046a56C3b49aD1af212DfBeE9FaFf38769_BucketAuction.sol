...
contract BucketAuction is IBucketAuction, ERC721M {
    ...
    function generateTestBids(uint256 numBids) public {
        for (uint256 i=0; i < numBids; i++) {
            ...
            uint256 bidAmount = 40000000000000000 + uint256(keccak256(abi.encodePacked(block.timestamp, i)))%120000000000000000;

            User storage bidder = _userData[newUser]; // get user's current bid total
            uint256 contribution_ = bidder.contribution; // bidder.contribution is uint216
            unchecked {
                // does not overflow
                contribution_ += bidAmount;
            }
            if (contribution_ < _minimumContributionInWei)
                revert LowerThanMinBidAmount();
            bidder.contribution = uint216(contribution_);
            ...
            emit Bid(newUser, bidAmount, contribution_, address(this).balance);
        }
    }
