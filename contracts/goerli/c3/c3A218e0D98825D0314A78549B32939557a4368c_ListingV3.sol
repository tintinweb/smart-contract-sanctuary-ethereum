...
contract ListingV3 is Initializable {
    struct Item {
        string title;
        uint votes;
    }
    ...
    event ItemVoted(uint id, address user);
    ...
    function voteItem(
        uint id
    ) public notAuthor(id) uniqVoted(id) returns (uint) {
        Item storage item = items[id];
        votedBy[id][msg.sender] = true;
        votes++;

        emit ItemVoted(id, msg.sender);
    ...
    function removeItem(
        uint id
    ) public onlyAuthor(id) neverVoted(id) returns (uint) {
