 ...
 */
contract SafetyModule is Initializable, SM1Slashing, SM1Operators, SM1Admin, SM1Getters {
    // ============ Constants ============

    string public constant EIP712_DOMAIN_NAME = "VOLX Safety Module";
 ...
 *    at once.
 */
abstract contract SM1Operators is SM1Staking, SM1Roles {
    // ============ Events ============
 ...
 */
abstract contract SM1Slashing is SM1Staking, SM1Roles {
 ...
 *   is equal to one.
 */
abstract contract SM1Staking is SM1StakedBalances, SM1ERC20 {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    IERC20MetadataUpgradeable
{
    ...
    function balanceOf(address account)
        public
        view
        override(IERC20Upgradeable, SM1GovernancePowerDelegation)
        returns (uint256)
    {
        return
     ...
     * In this initial implementation with no VOLX minting, simply returns the current supply
     * A snapshots mapping will need to be added in case a mint function is added to the VOLX token in the future
     **/
    function totalSupplyAt(uint256 blockNumber)
        external
        view
        override(IGovernancePowerDelegationToken)
        returns (uint256)
    {
        return totalSupply();
    }
    ...
    IGovernancePowerDelegationToken
{
    ...
    function delegateByTypeBySig(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked("\x19\x01", _DOMAIN_SEPARATOR_, structHash)
        );
    ...
    function getPowerAtBlock(
        ...
        (
            mapping(address => mapping(uint256 => SM1Types.Snapshot))
                storage snapshots,
            mapping(address => uint256) storage snapshotCounts, // unused: delegates
        ...
        return
            underlyingAmountFromStakeAmountWithExchangeRate(
                stakeAmount,
                exchangeRate
            );
    }
    ...
    function _moveDelegatesForTransfer(
        ...
        address votingPowerFromDelegatee = _getDelegatee(
            from,
            _VOTING_DELEGATES_
        );
        ...
        address propositionPowerFromDelegatee = _getDelegatee(
            from,
            _PROPOSITION_DELEGATES_
        );
        address propositionPowerToDelegatee = _getDelegatee(
            to,
            _PROPOSITION_DELEGATES_
        );
    ...
    function _moveDelegatesByType(
        ...
        if (from != address(0)) {
            mapping(uint256 => SM1Types.Snapshot)
                storage fromSnapshots = snapshots[from];
        ...
        if (to != address(0)) {
            mapping(uint256 => SM1Types.Snapshot)
                storage toSnapshots = snapshots[to];
