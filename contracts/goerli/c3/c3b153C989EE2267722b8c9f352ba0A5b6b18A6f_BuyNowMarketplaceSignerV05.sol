...
contract BuyNowMarketplaceSignerV05 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function initialize(uint _feePercent, uint _initialFeePercent, address payable _feeAccount, address _hpRolesContractAddress, string memory name, string memory version) initializer public {
    ...
    function recoverSigner(SignerItem calldata item) public view returns (address) {
        ...
        (address signer, ECDSAUpgradeable.RecoverError error) = ECDSAUpgradeable.tryRecover(digest, item.signature);
        return signer;
    }

    function recoverMintSigner(MintSignerItem calldata item) public view returns (address) {
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("MintSignerItem(string saleId,address royaltyAddress,uint96 feeNumerator,string uri,string trackId,uint price,string currency,address seller,address nft,address purchaser,uint expiry)"),
            keccak256(bytes(item.saleId)),
            item.royaltyAddress,
            item.feeNumerator,
    ...
    function purchaseMintSignerItem(MintSignerItem calldata mintSignerData) public payable nonReentrant {
        ...
        address buyer = mintSignerData.purchaser;
    ...
    // function purchaseSignerItemForTest(address nft, address seller, address purchaser, uint256 tokenId) public payable nonReentrant {

    //     IERC721Upgradeable hpMarketplaceNft = IERC721Upgradeable(nft);
    ...
    function purchaseSignerItemForTest(SignerItem calldata signerData) public payable nonReentrant {
