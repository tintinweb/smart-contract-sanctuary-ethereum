...
contract Coupon is EIP712 {
    ...
    modifier onlyWithCoupon(
        ...
        require(receipts[id] == 0, "Coupon: already used coupon");
    ...
    VRFConsumerBaseV2
{
    ...
    event ProvenanceHashConfigured(string hash, uint256 timestamp);
    ...
    /// @dev initialization
    /// @param name -> the name of the collection {ERC721}
    /// @param symbol -> the symbol of the collection {ERC721}
    ...
    constructor(
        ...
        Coupon(couponSigner, name, couponSignatureVersion)
    {
    ...
    /// @dev functionality for setting the provenance hash
    /// @param newProvenanceHash -> the provenance hash
    /// @return successful -> confirmation of activity
        ...
        returns (bool successful)
    {
        ...
        emit ProvenanceHashConfigured(newProvenanceHash, block.timestamp);
        successful = true;
    }

    /// @dev functionality for minting tokens
    /// @return tokenId -> the token id that has been minted
    function mint(
        uint256 id,
        uint256 expiresAt,
        bytes memory signature
        ...
        external
        onlyWithCoupon(id, expiresAt, signature)
        nonReentrant
        returns (uint256)
    {
    ...
    /// @param ids -> the ids of tokens
    /// @return successful -> confirmation of activity
    function airdrop(uint256[] calldata ids)
        external
        onlyAuthorized
        returns (bool successful)
    {
    ...
    function setBaseURI(string calldata newBaseURI) external onlyAuthorized {
    ...
    function updateTermsAndConditions(string calldata link) external onlyAuthorized {
    ...
    function _transfer(
        ...
        // have approved the transfer of the token by {msg.sender} and not {from}
    ...
    /// @dev fetch whether ANS is configured or not
    /// @return successful -> confirmation of activity
    function _isANSConfigured() private view returns (bool successful) {
        successful = ANS != address(0);
    }
