...
contract USB is ERC20, Ownable {

    mapping(address => bool) public isMinter;
    event SetMinter(address minter);
    event RevokeMinter(address minter);
    event Mint(address indexed acount, uint amount);
    event Burn(address indexed acount, uint amount);

    constructor() ERC20("USB Stablecoin", "USB") {}

    modifier onlyMinter() {
        require(isMinter[msg.sender], "USB: caller is not the owner");
        _;
    }

    function setMinter(address minter) public onlyOwner {
        require(minter != address(0) && !isMinter[minter], "USB: invalid address");
