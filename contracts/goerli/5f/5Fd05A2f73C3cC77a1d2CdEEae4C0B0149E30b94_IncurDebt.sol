...
contract IncurDebt is OlympusAccessControlledV2, IIncurDebt {
    ...
    /// @notice Encodes the necessary parameters to pass as _strategyParams in the createLP function for depositing to a Balancer pool
    ...
    /// @param _fromInternalBalance This should always be false as IncurDebt sends in ERC20 tokens
    function encodeBalancerCreateParams(
        ...
        encodedParams = abi.encode(_poolId, _assets, _maxAmountsIn, _minimumBPT, _fromInternalBalance);
    }

    /// @notice Encodes the necessary parameters to pass as _strategyParams in the removeLP function for removing from a Balancer pool
    /// @param _poolId The ID for the pool you wish to deploy assets into
    ...
    ///                       These amounts need to match by index to the assets in _assets
    /// @param _toInternalBalance This should always be false as IncurDebt expects the withdrawal of ERC20 tokens
    function encodeBalancerRemoveParams(
        ...
        encodedParams = abi.encode(_poolId, _assets, _minAmountsOut, _toInternalBalance);
    }

    /// @notice Encodes the necessary parameters to pass as _strategyParams in the createLP function for depositing to a Curve pool
    ...
    /// @notice Encodes the necessary parameters to pass as _strategyParams in the removeLP function for a Curve pool
    ...
    /// @notice Encodes the necessary parameters to pass as _strategyParams in the createLP function for a Uniswap or Sushiswap pool
    ...
    /// @notice Encodes the necessary parameters to pass as _strategyParams in the removeLP function for a Uniswap or Sushiswap pool
    /// @param _tokenA The first token in the pool
    /// @param _tokenB The second token in the pool
