...
contract HddenToken is Ownable, ERC721Checkpointable {
    using Strings for uint256;
    address public hiddenDao;
    ...
    modifier onlyHddenDao() {
        require(msg.sender == hiddenDao, "Sender is NOt DAO");
        _;
    }
    modifier onlyMinter() {
    ...
    constructor(address _hiddendao, address _minter, IProxyRegistry _proxyRegistry) ERC721("HDDENTOKEN", "HD") {
        minter = _minter;
        hiddenDao = _hiddendao;
    ...
    function mint() public  onlyMinter returns (uint256) {
        ...
        _mintTo(minter, _currentTokenId++);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "Hdden Token: URI query for nonexistent token");
        return bytes(_contractURIHash).length > 0 ? string(abi.encodePacked(_contractURIHash, tokenId.toString())) : "";

    }
    function setHddenDao(address _hddenDao) external  onlyHddenDao {
        hiddenDao = _hddenDao;
        // emit dao change
    }
