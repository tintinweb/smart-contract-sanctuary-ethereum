...
contract TokenDistribute {
    ...
    */
        ...
        external
    {
        ...
        console.log("distribute projectToken %s", projectToken);
        console.log("distribute sender %s", msg.sender);
        uint256 allowance = IERC20(projectToken).allowance(msg.sender, address(this));
        console.log("distribute address(this) %s %s", address(this), allowance);
        ...
        console.log("distribute sum %s", sum);
        console.log("distribute totalDistributeAmount %s", totalDistributeAmount);
        require (sum == totalDistributeAmount, 'E4');
        IERC20(projectToken).safeTransferFrom(msg.sender, address(this), totalDistributeAmount);

        console.log("done safeTransferFrom");
        for (uint256 i = 0; i < len; i++){
            DistributeInfo memory info = tokens[i];
            console.log("info %s %s", info.to, info.amount);
