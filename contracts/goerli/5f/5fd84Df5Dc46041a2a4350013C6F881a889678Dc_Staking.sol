...
contract Staking is Initializable{
    ...
    function initialize(address _shidoToken)external initializer{
        ...
        lockInPeriod = 600; //30days for mainnet
...
function stake(uint256 _amount) external{
        ...
        emit events.Staked(msg.sender, _amount, userStake[msg.sender].unlockTime ,currentTime);
    }
    ...
    function claimReward()external{
        ...
        if(reward <= 0){revert customErrors.lessThanZeroAmount(reward);}
        totalRewards += reward;
