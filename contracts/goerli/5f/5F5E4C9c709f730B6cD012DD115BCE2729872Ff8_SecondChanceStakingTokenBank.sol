...
contract SecondChanceStakingTokenBank is ReentrancyGuard, Ownable {
    event Staked(uint _id, uint _timestamp, uint _endDate, address _staker, address _token, uint _amount, uint _decimals);
    ...
    //base 10% penalty 20%
    constructor(uint endDate, uint baseFee, uint penaltyFee){
        EndDate = endDate;
        BaseFee = baseFee;
    ...
    /**
    * @dev function to set the end date
    * @param endDate the end date to set
    */ 
    function setEndDate(uint endDate) external onlyOwner{
        EndDate = endDate;
    }
    ...
    */ 
    function stake(address stakingTokenAddress) external nonReentrant{
        ...
        StakeEntries[LastId].PeriodFinish = EndDate;
        ...
        emit Staked(LastId, block.timestamp, EndDate, _msgSender(), stakingTokenAddress, actualTransfer, IERC20Extended(stakingTokenAddress).decimals());
    }
