...
abstract contract PolemosPoolBase is IPoolBase, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function unstakeFromPoolTokemak(uint256 _depositId, uint256 _amount) external override {
    // delegate call to an internal function
  ...
  function depositPLMS(uint256 _amount) external {
    ...
    require(_amount <= MAX_DEPOSIT_AMOUNT, 'too much');
  ...
  function setMaxPlmsDepositAmount(uint256 _amount) external onlyOwner {
