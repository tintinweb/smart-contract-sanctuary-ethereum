...
contract D3MM is D3Funding, D3Liquidation, D3Trading, D3Maker {
    function init(
        ...
        bytes calldata mixData //uint256 IM, uint256 MM, address maintainer, address feeModel     
    ) external {
        {
        require(
            epochDuration >= 1 days && epochDuration <= 30 days,
            Errors.WRONG_EPOCH_DURATION
        );
        ...
        (state._INITIAL_MARGIN_RATIO_, state._MAINTENANCE_MARGIN_RATIO_, state._MAINTAINER_, state._MT_FEE_RATE_MODEL_) = abi.decode(mixData, (uint256, uint256, address, address)); // IM, MM, maintainer,feeModel
...
import "./D3Library.sol";

/**
 * @author  DODO
 * @title   D3Funding
 * @dev     To save contract size, most of the function implements are moved to D3Library.
...
contract D3Funding is D3Storage {
    ...
    modifier poolOngoing() {
        require(
            state._POOL_STATUS_ == Types.PoolStatus.Ongoing,
            Errors.POOL_NOT_ONGOING
        );
        _;
    }
    ...
    modifier tokenExist(address token) {
        require(
            state.assetInfo[token].d3Token != address(0),
            Errors.TOKEN_NOT_EXIST
        );
        _;
    }
    ...
    function getTotalAssetsValue() public view returns (uint256 totalValue) {
        totalValue = D3Library.getTotalAssetsValue(state);
    }
    ...
    function getTotalDebtValue() public view returns (uint256 totalDebt) {
        totalDebt = D3Library.getTotalDebtValue(state);
    }
    ...
    function lpDeposit(address lp, address token)
        public
        nonReentrant
        poolOngoing
        tokenExist(token)
    {
        uint256 amount = D3Library.lpDeposit(lp, token, state);
    ...
    function lpRequestWithdraw(address token, uint256 d3TokenAmount)
        ...
        tokenExist(token)
    {
        D3Library.lpRequestWithdraw(msg.sender, token, d3TokenAmount, state);
        emit LpWithdrawRequest(msg.sender, token, d3TokenAmount);
    }
    ...
    function lpWithdrawAfterPoolEnd(address token)
        public
        nonReentrant
        poolEnd
        tokenExist(token)
    {
        uint256 originBalance = D3Library.lpWithdrawAfterPoolEnd(msg.sender, token, state);
        emit Refund(msg.sender, token, originBalance);
    }
    ...
    function addNewToken(
        ...
        D3Library.addNewToken(token, interestRate, maxDepositAmount, state);
    }

    /// @notice Owner set max deposit amount for a token
    /// @param token the token address
    /// @param maxDepositAmount the max deposit amount for the token
    function setMaxDeposit(address token, uint256 maxDepositAmount)
        external
        onlyOwner
        poolOngoing
        tokenExist(token)
    ...
    function setNextEpoch(
        ...
        D3Library.setNextEpoch(tokenList, interestRates, state);
    }
    ...
    function executeEpochUpdate() external poolOngoing {
        D3Library.executeEpochUpdate(state);
    }
    ...
    function refund(uint256 index) public onlyOwner nonReentrant poolOngoing {
        (address user, address token, uint256 amount) = D3Library.refund(index, state);
        emit Refund(user, token, amount);
    }

    /// @notice Owner deposit token
    /// @param token the token going to be deposited
    function ownerDeposit(address token)
        external
        poolOngoing
        tokenExist(token)
    {
    ...
    function ownerWithdraw(
        ...
        D3Library.ownerWithdraw(to, token, amount, state);
    ...
    function ownerWithdrawAfterPoolEnd(
        ...
        D3Library.ownerWithdrawAfterPoolEnd(to, token, amount, state);
        emit OwnerWithdraw(to, token, amount);
    }
    ...
    function ownerClosePool() external onlyOwner poolOngoing {
        D3Library.ownerClosePool(state);
    }
    ...
    function accrueInterests() public {
        D3Library.accrueInterests(state);
    }
...
contract D3Liquidation is D3Funding {
    ...
    */
    ...
    function startLiquidation() external onlyLiquidator poolOngoing {
        D3Library.startLiquidation(state);
    }
    ...
    function liquidate(
        ...
        D3Library.liquidate(order, routeData, router, state);
    }
    ...
    function finishLiquidation() external onlyLiquidator poolLiquidating {
        D3Library.finishLiquidation(state);
    }
    ...
    function forceRefund(
        ...
        D3Library.forceRefund(index, order, routeData, router, state);
    }
}
...
contract D3Trading is D3Funding {
        ...
        )
    {
        ...
        cumulativeAsk = state.tokenMMInfoMap[token].cumulativeAsk;
        cumulativeBid = state.tokenMMInfoMap[token].cumulativeBid;
    }
...
// todo add judge token whitelist or not?
