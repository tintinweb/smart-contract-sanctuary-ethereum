...
contract LuckyContract is ReentrancyGuard, Ownable {
    ...
    event winner(uint256 indexed round, address winner, uint256 winAmount);
    ...
    function bet(uint256 _amount) external nonReentrant {
        require(_amount > 10**tokenDecimals, "bet: Invalid amount");
        ...
        uint256 steps = _amount.div(10**tokenDecimals);
    ...
    function clacWinner() external onlyOwner {
        LuckyInfo storage curInfo = Infos[round];
        require(
            curInfo.participationCounts > 0,
            "clacWinner: Insufficient participants"
        );
        uint256 number = block.number + block.timestamp;
        address winnerAddress = curInfo.luckyList[
            number.mod(curInfo.participationCounts)
        ];
        uint256 winAmount = curInfo.totalAmount.mul(92).div(100);
        // user get 92%
        LuckyToken.safeTransfer(winnerAddress, winAmount);
        // free 8%
        LuckyToken.safeTransfer(
            msg.sender,
            curInfo.totalAmount.mul(8).div(100)
        );
        emit winner(round, winnerAddress, winAmount);
        round++;
    }
