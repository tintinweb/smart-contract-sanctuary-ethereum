...
contract HIKARI is ERC721, Pausable, Ownable {
    ...
    error HIKARI_PublicMintNotStart();
    error HIKARI_ExceedMaxSupply();
    error HIKARI_MintNotAuthorized_1();
    error HIKARI_MintNotAuthorized_2();
    error HIKARI_MintPriceNotMet();
    error HIKARI_MintAmountNotAllowed();
    ...
    function delContract() external onlyOwner {
    ...
    constructor(address signer, uint256 publicTimestamp)
        ERC721("HIKARI: TEST 10-28", "HIKARI")
    {
        verifySigner = signer;
        ...
        notContract
    {
        if (block.timestamp <= publicMintStartTime) {
            revert HIKARI_PublicMintNotStart();
        }

        if (tokenIdCounter.current() + count > totalSupply) {
            revert HIKARI_ExceedMaxSupply();
        }
        ...
        if (
            ...
            revert HIKARI_MintAmountNotAllowed();
        }
        if (msg.value < mintPrice[uint(MintTypes.PUBLIC_SALE)] * count) {
            revert HIKARI_MintPriceNotMet();
        }
    ...
    function allowlistMint(
        ...
        uint256 mintedNum = mintedAddress[to];
        if (
            count <= 0 || (mintedNum + count) > maxMintLimit[uint256(mintType)]
        ) {
            revert HIKARI_MintAmountNotAllowed();
        }
        if (msg.value < mintPrice[uint256(mintType)] * count) {
            revert HIKARI_MintPriceNotMet();
        }
        if (!verify(to, mintType, signature)) {
            revert HIKARI_MintNotAuthorized_2();
        }
