...
struct BalanceIn {
    ...
    function evalEIP712Buffer(OrderComponents memory order) public view returns(BalanceOut[] memory balanceOut, BalanceIn[] memory balanceIn, uint256 expirationTime) { 
        ...
        for (uint i; i < order.offer.length; i++) {   
            if (outTokenAddressToPosition[order.offer[i].token] > 0 ) {
                balanceOut[outTokenAddressToPosition[order.offer[i].token] - 1].amount += order.offer[i].startAmount;
            }
            else {
        ...
        for (uint i; i < order.consideration.length; i++) {   
            if (order.offerer == order.consideration[i].recipient) {
                if (inTokenAddressToPosition[order.consideration[i].token] > 0 ) {
                    balanceIn[inTokenAddressToPosition[order.consideration[i].token] - 1].amount += order.consideration[i].startAmount;
                }
                else {
