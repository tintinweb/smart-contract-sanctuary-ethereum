...
// File: contracts/lib/ERC721URIStorage.sol
...
contract SoulBoundToken is ERC721, ERC721URIStorage, Ownable {
    ...
    bool public currentStatus;
    address public minter;

    uint256 public totalSupply;
    mapping(uint256 => RecoveryApproval) RecoveryApprovalData;
    // email to TokenID
    mapping(string => uint256) public claimedEmail;
    ...
    modifier hasMintingRole() {
        require(
            owner() == _msgSender() || _msgSender() == minter,
            "caller is not the owner or has Minting Role"
        );
        _;
    }

    function setCurrentState(bool _currentStatus) external onlyOwner {
        require(currentStatus != _currentStatus, "Status Already set!");
    ...
    function safeMint(
    ...
    ) public hasMintingRole {
        require(currentStatus == true, "Minting is Paused");
        ...
        require(
            claimedEmail[email] == 0,
            "This email has already claimed its Token."
        );
        uint256 tokenId = ++totalSupply;

        claimedEmail[email] = tokenId;
    ...
    function validRecoveryAccounts(
        uint256 tokenId
    ...
    function recoverAppealOrganistion(
        uint256 tokenId,
        address _recipient
    ) external onlyOwner {
    ...
    function _afterTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            emit Recovered(from, to, firstTokenId);
        }
