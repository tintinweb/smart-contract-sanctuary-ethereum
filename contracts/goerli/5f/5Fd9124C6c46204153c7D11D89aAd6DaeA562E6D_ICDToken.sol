...
contract ICDToken  is StandardToken{
  ...
  constructor(address student, string tokenName, string tokenSymbol, uint totalSupply, uint adminFeePrctg) public {
    icdAdmin = msg.sender; // setting icd admin to the deployer address
    owner = student; // setting the student as passed by the admin
    name = tokenName;
    symbol = tokenSymbol; 
    adminFeePercentage = adminFeePrctg; // setting the fee percentage that will go to icdAdmin on each payment
