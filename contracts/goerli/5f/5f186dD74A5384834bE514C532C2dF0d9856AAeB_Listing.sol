...
interface IListing {
    event NodeAdded(uint256 tokenId, string name);

    function getName(uint256 tokenId) external view returns (string memory);

    function nodeCount() external view returns (uint256);

    function addRootNode(address to, string memory name) external;

    function batchAddRootNodes(address to, string[] memory names) external;
...
interface IPosting {
    ...
    event ItemUpdated(uint256 itemId, string newTitle, string newDescription);
    ...
    function erc6150() external view returns (address);

    function erc5018() external view returns (address);

    function postedCount() external returns (uint256);
...
import "./interfaces/IPosting.sol";
    ...
    Ownable
{
    ...
    constructor() ERC6150("Listing", "LIST") {}
