...
/**
 * Interfaces burger contract
 */
interface IBURGER {
  function burnBurger(address account, uint256 quantity) external;
}
 ...
 * they are using to claim; meaning the account must own each astrobull, and each astrobull can
 * only be used once. The contract will store which astrobull traits to give each metabull.
 * Users will burn burgers for each metabull they mint.
 */
contract MetaBull is ERC721A, Ownable {
  using Strings for uint256;
  /// contract instances ///
  IBURGER internal immutable Burger;
  ISUPER1155 internal constant Astro =
   ...
   */
  constructor(string memory _URI, address burgerAddr)
    ERC721A("METABULLS", "MBULL")
  {
    URI = _URI;
    Burger = IBURGER(burgerAddr);
  }
  ...
  function claimBull(uint256[] memory astrobullIds) public {
    ...
    for (uint256 i = 0; i < astrobullIds.length; ++i) {
      if (Astro.balanceOf(msg.sender, astrobullIds[i]) == 0) {
    ...
    Burger.burnBurger(msg.sender, toBurn);
    /// sets contract state ///
    totalBurns += toBurn;
