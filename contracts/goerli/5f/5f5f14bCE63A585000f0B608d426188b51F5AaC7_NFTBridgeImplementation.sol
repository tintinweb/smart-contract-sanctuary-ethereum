...
contract NFTBridge is NFTBridgeGovernance {
    ...
    function completeTransfer(bytes32 blockHash, bytes memory proofBlob, bytes32[] memory merkleProof) public {
        _completeTransfer(blockHash,proofBlob,merkleProof);
    }

    // Execute a Transfer message
    function _completeTransfer(bytes32 blockHash, bytes memory proofBlob, bytes32[] memory merkleProof) internal {
        (IZKHole.VM memory vm, bool valid, string memory reason) = zkhole().parseAndVerifyProof(blockHash,proofBlob,merkleProof);
...
interface IZKHole is Structs {
    ...
    function parseAndVerifyProof(bytes32 blockHash, bytes memory proofBlob,bytes32[] memory merkleProof) external view returns (Structs.VM memory vm, bool valid, string memory reason);
