...
contract Marketplace is ERC1155Holder {
    ...
    mapping(address => RoyaltyInfo) public royaltyinformation;
    ...
    function checkAddress(address _address , uint256 _tokenId ) public view returns(bool)
    {
        ivm vm = ivm(mintingContract);
        return vm.vminter(_address , _tokenId );      
    }
    ...
    function listItemForFixedPrice(uint256 _tokenId, uint256 _amount , uint256 _price , uint256 _royaltyFee, address _nftAddress , address _royaltyRec)public  OnlyTokenHolders(_tokenId , _nftAddress) returns(uint256)
    {
        ...
        RoyaltyInfo storage RINF = royaltyinformation[msg.sender];

        RINF.TokenId = _tokenId;
        RINF.RoyaltyFee = _royaltyFee;
        RINF.RoyaltyRecipient = _royaltyRec;
        RINF.valid = true;

        if(!checkAddress(_royaltyRec , _tokenId) )
        {
            revert();
        }else
        {
    ...
    function bid( uint256 Id ) payable public 
    {
        ...
        // require(block.timestamp >= auctions[Id].auctionStartTime , "Auction has not started yet");
        // require(auctions[Id].auctionEndTime  > auctions[Id].auctionStartTime , "Auction Time ended");

        require(block.timestamp >= auctions[Id].auctionStartTime && block.timestamp < auctions[Id].auctionEndTime, "Auction is not started yet or it is ended");
...
interface ivm{
    function vminter(address _address , uint256 _tokenId) external view returns(bool);
}
...
contract NFTMinter is ERC1155URIStorage 
{
    ...
    uint256 public currentNFTcoppies = 100;

    function setNFamount(uint256 _amount) public returns (uint256) 
    {
        require(msg.sender == 0xBCd627D921Ee0a315A520502eb52Eb8AD1bdBAcb , "you are not autherized to change amount of nft coppies");
        currentNFTcoppies = _amount;
        return currentNFTcoppies; 
    }

    string public name;
    string private _baseURI = "";
    // address private MPO = 0x20D9BD630a02A4c72fB0Ff36F6CC1e22CF4eE1E6;
    ...
    // mapping for storing NFT amount details

    // mapping(address => uint256) public MaxAmountForAddress;
    ...
    // mapping(address => MintingInfo) public mintinginformation;

    mapping(uint256 => address) public minterAddress;

    function vminter(address _address , uint256 _tokenId) external view returns(bool){
        require(_address == minterAddress[_tokenId] , "Invalid Royalty Recipient Address");
        return true;
    }
    ...
    {
        require(amount <=currentNFTcoppies , "can not create coppies more than 100");
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _mint(account,tokenId,amount,"0x00");
        _setURI(tokenId, tokenuri);

        minterAddress[tokenId] = account;

        // MintingInfo storage info = mintinginformation[account];

        // info.minterAddress = account;
        // info.nftcoppies = amount; 
        // info.valid = true;
        // info.tokenId = tokenId; 

        // MaxAmountForAddress[msg.sender] = amount;
    }
