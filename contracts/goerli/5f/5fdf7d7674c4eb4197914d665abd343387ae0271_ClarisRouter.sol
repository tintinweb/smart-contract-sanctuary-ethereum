...
contract ClarisRouter  {
    ...
    function addLiquidity(address tokenA,address tokenB,uint amountADesired,uint amountBDesired,address to
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure virtual returns (uint amountIn) {
        require(amountOut > 0, "claraLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "claraLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        require(tokenA != tokenB, "claraLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "claraLibrary: ZERO_ADDRESS");
    }
    ...
    function safeTransferFrom(address token, address from, address to, uint value) public {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "claraHelper: TRANSFER_FROM_FAILED");
    }

    function safeTransfer(address token, address to, uint value) internal {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0xa9059cbb, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), "claraHelper: TRANSFER_FAILED");
    }

    function safeTransferETH(address to, uint value) internal {
        (bool success,) = to.call{value:value}(new bytes(0));
        require(success, "claraHelper: ETH_TRANSFER_FAILED");
    }
