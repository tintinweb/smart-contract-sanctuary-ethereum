    ...
    ERC721HolderUpgradeable
{
    ...
    address public apeCoinStakingAddress;
    ...
    function initialize(
        ...
        address _apeCoinStakingAddress,
        string memory _name,
        ...
        apeCoinStakingAddress = _apeCoinStakingAddress;
    ...
    function apeCoinStaking(uint256 tokenId, uint256 amount) external {
        console.log("token:%s, amount:%s", tokenId, amount);
        require(_exists(tokenId), "Wrapped token does not exist");
        require(
            ownerOf(tokenId) == msg.sender,
            "Only wrapped token owner can call this function"
        );
        IERC20Upgradeable apeCoin = IERC20Upgradeable(apeCoinAddress);
        console.log("address:%s, amount:%s", apeCoinAddress, amount);
        require(
            apeCoin.allowance(msg.sender, address(this)) >= amount,
            "ApeCoin allowance not enough"
        );
        apeCoin.safeTransferFrom(msg.sender, address(this), amount);
        apeCoin.approve(apeCoinStakingAddress, amount);

        IApeCoinStaking apeCoinStaking = IApeCoinStaking(apeCoinStakingAddress);
    ...
    function apeCoinWithdraw(uint256 tokenId, uint256 amount) external {
        require(_exists(tokenId), "Wrapped token does not exist");
        require(
            ownerOf(tokenId) == msg.sender,
            "Only wrapped token owner can call this function"
