...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    function _submitClaimAndInitializeVoting(
        ...
        if (!appeal) {
            ...
            uint256 claimIndex = claimingRegistry.claimIndex(claimProvenance);
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 _reinsurancePrice =
            _totalPrice.mul(PROTOCOL_PERCENTAGE).uncheckedDiv(PERCENTAGE_100).uncheckedSub(
                _distributorFeeAmount
            );

        uint256 _price = _totalPrice.uncheckedSub(_distributorFeeAmount);
