    ...
    Ownable
{
    ...
    function createTrade(
        ...
        require(nftContractToTokenIdToOfferId[nftContract][tokenId] == 0, "Token already offered on another trade");
        require(nftContractToTokenIdToListingId[nftContract][tokenId] == 0, "Token already listed for sale");
        require(nftContractToTokenIdToTradeId[nftContract][tokenId] == 0, "Token already listed for trade");

        uint256 tradeId = _getNextListingAndIncrementId();
        nftContractToTokenIdToTradeId[nftContract][tokenId] = tradeId;

        tradeIdToTrade[tradeId] = Trade(nftContract, tokenId, payable(msg.sender), endOfferTime, endTime);
    ...
    function placeOffer(
        ...
        require(nftContractToTokenIdToTradeId[nftContract][tokenId] == 0, "Token already listed for trade");
        require(nftContractToTokenIdToListingId[nftContract][tokenId] == 0, "Token already listed for sale");
        require(nftContractToTokenIdToOfferId[nftContract][tokenId] == 0, "Token already offered");
    ...
    function cancelListing(uint256 listingId) public nonReentrant {
        ...
        require(listing.seller == msg.sender, "WTDNFTMarket: Not your listing");
        require(listing.bidder == address(0), "WTDNFTMarket: Can not cancel listing in progress");
