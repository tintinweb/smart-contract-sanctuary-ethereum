...
contract Staking is AccessControl {
    ...
    function initiliaze(
        ...
        __Staking_init_unchained(_mainToken, _stakingToken, _epochLength, _firstEpochNumber, _firstEpochTime);
    }
    ...
    */
    ...
    function _send(
        ...
        if (_rebasing) {
            stakingToken.safeTransfer(_to, _amount); // send as sOHM (equal unit as OHM)
            return _amount;
        } else {
            governanceToken.mint(_to, governanceToken.balanceTo(_amount)); // send as gOHM (convert units from OHM)
