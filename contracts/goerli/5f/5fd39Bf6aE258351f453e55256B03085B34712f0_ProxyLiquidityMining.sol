...
contract ProxyLiquidityMining is LiquidityMiningLogic, ReentrancyGuard {
    ...
    constructor(
        ...
        //approve(_DFI, _secondToken, _pair);
        IERC20(_DFI).approve(router_, MAX_INT);
        IERC20(_secondToken).approve(router_, MAX_INT);
        IERC20(_pair).approve(router_, MAX_INT);
    }

    // function approve(address _tokenA, address _tokenB, address _pair ) public {
    //      IERC20(_tokenA).approve(address(router), MAX_INT);
    //     IERC20(_tokenB).approve(address(router), MAX_INT);
    //     IERC20(_pair).approve(address(router), MAX_INT);
    // } 
