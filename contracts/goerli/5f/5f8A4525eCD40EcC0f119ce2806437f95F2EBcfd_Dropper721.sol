    ...
    Pausable
{
    ...
    string public baseURI = "http://localhost:3001/";
    ...
    constructor(address _AdminTokenContract, uint256 _AdminTokenId) {
        adminNFTAddress = _AdminTokenContract;
        adminNFTId = _AdminTokenId;
    ...
    function isAdminHolder() internal view returns (bool) {
        return IERC1155(adminNFTAddress).balanceOf(msg.sender, adminNFTId) > 0;
    }

    function claim(address _owner, uint256 _dropId) external whenNotPaused {
    ...
    function returnNfts() external {
        require(isAdminHolder(), "Not admin");
    ...
    function getCUrrentLinks() external view returns (string[] memory) {
        require(isAdminHolder(), "Not admin");

        string[] memory links = new string[](count);

        for (uint256 i = 0; i < count; i++) {
            if (!drops[i].claimed) {
                links[i] = buildLink(drops[i].tokenId);
            }
        }
    ...
    function setAdminToken(address _address, uint256 _id)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        adminNFTAddress = _address;
        adminNFTId = _id;
    }
}
