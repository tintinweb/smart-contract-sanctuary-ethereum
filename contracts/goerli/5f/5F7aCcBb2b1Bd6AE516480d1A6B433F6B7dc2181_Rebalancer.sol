...
contract Rebalancer is BACUpgradeable, PausableUpgradeable, IRebalancer {
    ...
    modifier onlyArbitrager() {
        require(bac.userHasRole(ARBITRAGER_ROLE, msg.sender), "R-not-arbitrager");
        _;
    }
    ...
    function initialize(address protocolConfig, address _bst) external initializer {
    ...
    {
        console.log(_market);
        console.log(IMarket(_market).getRiskCalc());
        RebalanceAmount memory deltaAmounts = IRebalanceCalculation(IMarket(_market).getRiskCalc())
                .rebalanceAmount();

        return
            deltaAmounts.deltaAP != 0 ||
            deltaAmounts.deltaAR != 0 ||
            deltaAmounts.deltaCP != 0 ||
            deltaAmounts.deltaCR != 0 ||
            deltaAmounts.sellToken != 0 ||
            deltaAmounts.sellStable != 0;
    }
        ...
        whenNotPaused
    {
        ...
        _rebalance(deltaAmounts, _market);
    }

    /// @dev internal rebalance impl for prevent stack is too deep error
    function _rebalance(RebalanceAmount memory deltaAmounts, address _market) internal {

        console.log("_rebalance", deltaAmounts.sellToken, deltaAmounts.sellStable);
        if (deltaAmounts.sellToken > 0) {
            IERC20 token = IERC20(
                address(IMarket(_market).ASSET())
            );
            ...
            token.safeTransferFrom(_market, address(this), deltaAmounts.sellToken);

            _approveToMax(token, swap.swapAdapter, deltaAmounts.sellToken);

            uint amountOut = ISwap(swap.swapAdapter).swapExactTokensForTokensMult(
                    swap.path,
                    deltaAmounts.sellToken,
                    0,
                    address(bst)
                );

            bst.mint(amountOut * (10**18) / (10**IERC20Metadata(stableToken).decimals()), _market);
            deltaAmounts.deltaCP += int(amountOut);

        }
        if (deltaAmounts.sellStable > 0) {
            ...
            bst.withdraw(stableToken, deltaAmounts.sellStable, _market, address(this) );
            _approveToMax(IERC20(stableToken), swap.swapAdapter, deltaAmounts.sellStable);

            deltaAmounts.deltaAP += int(
                ISwap(swap.swapAdapter).swapExactTokensForTokensMult(
                    swap.path,
                    deltaAmounts.sellStable,
                    0,
                    _market
                )
            );
        ...
        IMarket(_market).onAfterRebalance(deltaAmounts.deltaAP, deltaAmounts.deltaAR, deltaAmounts.deltaCP, deltaAmounts.deltaCR);

        emit Rebalance(
            _market,
            deltaAmounts.deltaAP,
            deltaAmounts.deltaAR,
            deltaAmounts.deltaCP,
            deltaAmounts.deltaCR,
            deltaAmounts.sellToken,
            deltaAmounts.sellStable
        );
    }
    ...
    function setSwap(
        ...
        swapInfo[_token][_stable] = SwapInfo({
            swapAdapter: _swap,
            path: _path
        });
    }

    function setStableForMarket(address market, address token) external virtual onlyGovernance {
    ...
    function _approveToMax(IERC20 token, address spender, uint amount)
        internal
    {
