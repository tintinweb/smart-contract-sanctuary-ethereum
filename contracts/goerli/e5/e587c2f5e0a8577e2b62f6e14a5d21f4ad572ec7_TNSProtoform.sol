...
contract TNSProtoform is ITNSProtoform, MerkleBase, Protoform, TNSReverse {
    ...
    /// @notice Minimum time to live of ENS record
    uint256 public constant TTL = 365 days;
    ...
    function _transfer(
        ...
        uint256 threshold = block.timestamp + TTL + _duration;
        // Reverts if TTL of ENS node is less than threshold period
        uint64 ttl = IENS(ens).ttl(_node);
        /* if (ttl < threshold) revert InvalidExpiration(ttl, threshold); */
