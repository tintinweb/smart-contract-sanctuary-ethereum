...
contract PanaBondingCalculator is IBondingCalculator {
    ...
    function getTotalValue(address _pair) public view returns (uint256 _value) {
        ...
        _value = getKValue(_pair).div(100).sqrrt().mul(2);
    ...
    function valuation(address _pair, uint256 amount_) external view override returns (uint256 _value) {
...

        {
            ...
            */    
            ...
            numerator = numerator.mul(10**(2+decimals));
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(PANA)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(PANA), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(PANA)).decimals())).div(getTotalValue(_pair));
    }
}
