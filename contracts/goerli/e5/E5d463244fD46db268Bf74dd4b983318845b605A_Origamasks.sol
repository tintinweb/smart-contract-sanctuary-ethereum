    ...
    VRFV2WrapperConsumerBase
{
    ...
    mapping(uint256 => address) public tokensMinted; // get owner from specific tokenId

    uint256 public collectionSize = 5000; //DEV

    uint256 public totalSupply = 1; // already reserved 1 for collection configuration in marketplace
    uint256 public totalAirdrop; //tracker

    uint256 public vipSupply;
    uint256 public mintPrice = 0.1 ether;
    uint256 public publicMintPrice;
    uint256 public constant VIP_LIMIT_PER_WALLET = 1;
    uint256 public constant BUDDY_LIST_LIMIT_PER_WALLET = 1;
    uint256 public waitListLimitPerWallet = 2;
    uint256 public publicLimitPerWallet = 2;
    uint256 public numberReserved; //DEV
    ...
    constructor(
        ...
        address payable treasuryJarAddress_,
        ...
        VRFV2WrapperConsumerBase(linkAddress_, wrapperAddress_)
    {
        ...
        setTreasuryJarAddress(treasuryJarAddress_);
        ...
        _mint(origamasksAddress_, 1); // reserve for initial collection setup
    }

    /**
     * @dev handle VIP minting
     * @param signature_ validate
     * @param tokenId_ chosen token id
     */
        ...
        isSaleState(SaleState.VIP)
    {
        ...
        if (tokensMinted[tokenId_] != address(0x0)) revert TokenNotAvailable();
        ...
        if (buddyListMints[msg.sender] >= VIP_LIMIT_PER_WALLET)
        ...
        tokensMinted[tokenId_] = msg.sender; // secure spot for this tokenid
        ...
        isSaleState(SaleState.BuddyList)
    {
        ...
        // Specific per sale state validation
        if (!verifySignature(signature_, "BuddyList"))
            revert InvalidSignature();
        if (totalSupply + 1 > maxSupply()) revert SoldOut();
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (buddyListMints[msg.sender] >= BUDDY_LIST_LIMIT_PER_WALLET)
            revert LimitPerWalletExceeded();
        ...
        isSaleState(SaleState.WaitList)
    {
        ...
        if (totalSupply + 1 > maxSupply()) revert SoldOut();
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (waitListMints[msg.sender] >= waitListLimitPerWallet)
            revert LimitPerWalletExceeded();
        ...
        isSaleState(SaleState.Public)
    {
        ...
        if (publicMints[msg.sender] >= publicLimitPerWallet)
            revert LimitPerWalletExceeded();
    ...
    {
        if (totalSupply + tokenIds.length > collectionSize)
            revert ExceedReservedSpots();
    ...
    /**
     * @dev batch mints for every holders after sale finished
     * @param tos_ addresses of holders
     */
    function batchMints(address[] memory tos_) external onlyOwner nonReentrant {
        // Loop through owners' addresses
        for (uint256 i = 0; i < tos_.length; i++) {
            address _receiver = tos_[i];
            uint256[] memory _tokenIds = mintedTokenIds[_receiver]; //get minted token ids for each owner

            // Loop through every token ids
            for (uint256 j = 0; j < _tokenIds.length; j++) {
                _mint(_receiver, _tokenIds[j]);
                totalAirdrop++;
            }
        }
    }
    ...
    */
    ...
    // Token URI for Dynamic Visual based on Level from FieldTrip
    ...
    /* tokenId to fieldTrip start time in epoch (0 = not fieldTrip) */
    ...
    */
    ...
    /* Force dismiss - with help manually from DISMISS role */
    ...
    /// @notice BLOCK TRANSFER WHEN LISTING : _beforeTokenTransfer
    function _beforeTokenTransfer(
        address,
        address,
        uint256 tokenId,
    ...
    /**
    @dev Emitted when stops fieldTrip; either through standard means or
    by expulsion.
     */
    ...
    // REWARD based on Level of the token
    mapping(uint256 => bool) public rewardAlreadyClaimed;
    mapping(uint256 => address) public rewardContractAddress;
    ...
    /**
     * @dev sets the base token uri for {_baseURI}
     */
    ...
    // Sets Origamasks Address for withdraw() and reserved tokens
    ...
    // Sets Treasury Jar Address for ERC2981 royaltyInfo
    address payable public treasuryJarAddress;

    /**
     * @dev Update the royalty wallet address
     */
    function setTreasuryJarAddress(address payable treasuryJarAddress_)
        public
        onlyOwner
    {
        if (treasuryJarAddress_ == address(0)) revert ZeroAddress();
        treasuryJarAddress = treasuryJarAddress_;
    }
    ...
    function setRoyaltyInfo(uint96 royaltyPercentage_) public onlyOwner {
        if (treasuryJarAddress == address(0)) revert ZeroAddress();
        _setDefaultRoyalty(treasuryJarAddress, royaltyPercentage_);
    }
    ...
    function withdraw() external onlyOwner {
        (bool success, ) = origamasksAddress.call{value: address(this).balance}(
            ""
        );
    ...
    /**
     * @dev Withdraw for Airdrop.
     */
    function withdrawForAirdrop(uint256 amount_) external onlyOwner {
    ...
    function setReservedToken(uint256 numberReserved_) external onlyOwner {
