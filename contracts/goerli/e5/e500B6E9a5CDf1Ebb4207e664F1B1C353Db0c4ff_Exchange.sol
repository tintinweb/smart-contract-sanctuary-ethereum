  ...
  UUPSUpgradeable
{
   ...
   * Emits a {PlatformRegistryUpdated} event
   */
  function updatePlatformRegistry(IPlatformRegistry _newPlatformRegistry)
    public
    onlyAdmin
  {
   ...
   * Emits a {TransferProxyUpdated} event
   */
  function updateTransferProxy(ITransferProxy _newTransferProxy)
    public
    onlyAdmin
  {
   ...
   * Emits a {RoyaltyManagerUpdated} event
   */
  function updateRoyaltyRegistry(IRoyaltyRegistry _newRoyaltyRegistry)
    public
    onlyAdmin
  {
   ...
   */
  function _isSold(OrderDomain.SaleOrder calldata _saleOrder)
    private
    view
    returns (bool)
  {
  ...
  function _simulatePartnerFeeTransfer(
    ...
    if (OrderDomain._isPartnerOrigin(_asset.originKind)) {
      address payable partnerFeeReceiver = platformRegistry
        .getPartnerFeeReceiver(_asset.token);
      // Make sure platform fee recever is set
      // EM: platform fee recever is not set
  ...
  function exchange(
    ...
    OrderDomain._checkParameterForExchange(_saleOrder, _buyOrder);
    ...
    if (OrderDomain._isOnchainPaymentMode(paymentMode)) {
      ...
      _resetPayoutTemp();
      // Sale Price = Platform Fee + Partner Fee(If partner) + Saler(Secondary onwards) + Royalty
      uint256 totalSalePrice = OrderDomain._findTotalSalePrice(
        _saleOrder,
        _buyOrder
      );

      // Make sure ETH recieved is enough for sale
      // if we get more ETH/ERC20 then needed, it will be stored in this contract
      if (paymentMode == OrderDomain.NATIVE_PAYMENT_MODE) {
        // EM: not enough ETH received for sale
        require(msg.value >= totalSalePrice, "E:E:NETH");
      } else if (paymentMode == OrderDomain.ERC20_PAYMENT_MODE) {
        ...
        require(allowanceCount >= totalSalePrice, "E:E:NERC20");
      ...
      for (uint256 idx = 0; idx < assetList.length; idx++) {
        ...
        uint256 pricePerAssetAfterPfAndPartnerFee = pricePerAsset -
          pfFeePerAsset -
          partnerFeePerAsset;
        // Simulate sending royalty cut
        uint256 totalRoyaltyFee = _simulateRoyaltyTransfer(
          asset,
          pricePerAssetAfterPfAndPartnerFee
        );
        ...
        if (OrderDomain._isSecondarySale(asset.isSecondarySale)) {
          _registerPayout(
            paymentReceiver,
            (pricePerAssetAfterPfAndPartnerFee - totalRoyaltyFee)
          );
        }
      }
   ...
   */
  function withdrawTip(address payable _receiver, address _erc20Token)
    public
    onlyAdmin
  {
