...
contract testNFTWrapper is Initializable, ItestNFTWrapper, IERC165Upgradeable, OwnableUpgradeable, UUPSUpgradeable{
    ...
    function initialize() initializer public {
        baseURI = "";
        __ERC721_init("testNFTWrapper","tNW");
        __Ownable_init();
	...
	function ownerOf(uint256 recordId) public view returns(address) {
		...
		return IERC721Upgradeable(Wrapper.characterNFT).ownerOf(Wrapper.characterId);
	}
    ...
    function mint(
		...
		require(IERC165Upgradeable(characterNFT).supportsInterface(type(IERC721Upgradeable).interfaceId), "characterNFT is not ERC721");

		require(!exists(recordId), "iNFT already exists");

		require(characterWrappers[characterNFT][characterId] == 0, "NFT is already bound");

        address owner = IERC721Upgradeable(characterNFT).ownerOf(characterId);

        require(owner != address(0));

        if(KDValue > 0){
            require(KDBalance + KDValue <= IERC20Upgradeable(KDContract).balanceOf(address(this)), "KD tokens not yet transferred");
            KDBalance += KDValue;
        }
    ...
    function mintbatch(
		...
		for(uint96 i = 0; i < n; i++) {
            ...
            address owner = IERC721Upgradeable(characterNFT).ownerOf(characterId + i);
        ...
        if(_KDValue > 0){
            require(KDBalance + _KDValue <= IERC20Upgradeable(KDContract).balanceOf(address(this)), "KD tokens not yet transferred");
            KDBalance += _KDValue;
        }
    ...
    function burn(uint256 recordId) public onlyOwner{
        ...
        address owner = IERC721Upgradeable(Wrapper.characterNFT).ownerOf(Wrapper.characterId);

        require(owner != address(0), "no such NFT");

        if(Wrapper.KDValue > 0){
            KDBalance -= Wrapper.KDValue;
            IERC20Upgradeable(KDContract).transfer(owner, Wrapper.KDValue);
        }
    ...
    function increaseKD(uint256 recordId, uint96 KDDelta) public onlyOwner{
		...
		require(KDBalance + KDDelta <= IERC20Upgradeable(KDContract).balanceOf(address(this)), "KD tokens not yet transferred");
    ...
    function decreaseKD(uint256 recordId, uint96 KDDelta, address recipient) public onlyOwner{
		...
		IERC20Upgradeable(KDContract).transfer(recipient, KDDelta);
    ...
    function reserveAnswer1(uint256 recordId, string memory answer) public onlyOwner {
		require(exists(recordId), "wrappedNFT doesn't exist");

        Wrappers[recordId].Answer1 = answer;

    }

    function reserveAnswer2(uint256 recordId, string memory answer) public onlyOwner {
		require(exists(recordId), "wrappedNFT doesn't exist");

        Wrappers[recordId].Answer2 = answer;

    }

    function reserveAnswer3(uint256 recordId, string memory answer) public onlyOwner {
		require(exists(recordId), "wrappedNFT doesn't exist");

        Wrappers[recordId].Answer3 = answer;
