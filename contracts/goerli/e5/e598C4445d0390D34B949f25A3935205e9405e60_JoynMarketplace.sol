...
error MarketplaceFeeMustBeBelow10000();
    ...
    OwnableUpgradeable
{
        ...
        isOwner(nftAddress, tokenId, _msgSender())
    {
        ...
        if (marketplaceFee > PERCENTAGE_DIVIDER) {
            revert MarketplaceFeeMustBeBelow10000();
        }

        address collectionAddress = nftAddress;
        uint256 listingTokenId = tokenId;
        if (
            IERC721Upgradeable(collectionAddress).getApproved(listingTokenId) !=
            address(this)
        ) {
        ...
        nonReentrant
    {
        ...
        delete (s_listings[nftAddress][tokenId]);
        uint256 marketplaceFeeAmount = (listedItem.price *
            listedItem.marketplaceFee) / PERCENTAGE_DIVIDER;
        uint256 amountDueToSeller = listedItem.price - marketplaceFeeAmount;
        ...
        isOwner(nftAddress, tokenId, _msgSender())
    {
        ...
        emit ItemListed(
            ...
            tokenId,
            newPrice,
            listing.marketplaceFee
        );
    }

    function updateListingMarketplaceFee(
        address nftAddress,
        uint256 tokenId,
        uint256 marketplaceFee
    )
    ...
    function withdrawFee(address receiver, bool isEmergencyExit)
        external
        onlyOwner
    {
        uint256 amount;
