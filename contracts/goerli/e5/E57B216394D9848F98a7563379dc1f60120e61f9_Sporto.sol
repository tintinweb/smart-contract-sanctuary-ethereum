...
contract Sporto is AccessControl {
    ...
    function makeBet(uint256 id, Result choice, uint256 ticketCount) external {
        ...
        require(bet.startsAt <= block.timestamp || block.timestamp <= bet.endsAt, "Not open");
    ...
    function closeBet(
        ...
        Bet storage bet = bets[id];
        require(bet.result == Result.NONE, "Already closed");
        bet.result = result;
    ...
    function finalizeBet(uint256 id, uint256 fromIdx, uint256 limit) external onlyRole(OPERATOR_ROLE) {
        Bet storage bet = bets[id];
        require(bet.result != Result.NONE, "Not closed");
