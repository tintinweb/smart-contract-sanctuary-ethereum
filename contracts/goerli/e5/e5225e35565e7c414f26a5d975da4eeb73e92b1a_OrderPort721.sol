...
/**
 * @title OrderPortInvoke
 * @author BaseLabs
 */
contract OrderPortInvoke is Ownable {
    ...
    modifier onlyOrderPort() {
        if (getOrderPortAddress() != _msgSender()) revert ErrCallerIsNotAllowed();
        _;
    }
}
 ...
 * @author BaseLabs
 */
contract OrderPort721 is ERC721A, OrderPortInvoke, Pausable, ReentrancyGuard, Sealable, ERC2981 {
    ...
    constructor(string memory name_, string memory symbol_, 
        address orderPortAddress_, Config memory config_) ERC721A(name_, symbol_) OrderPortInvoke(orderPortAddress_) {
        _config = config_;
    }
     ...
     */
    function airdrop(address[] calldata addresses_, uint64[] calldata nums_) external onlyOwner nonReentrant {
        if (addresses_.length != nums_.length) revert ErrInvalidArguments("addresses_ and nums_");
        if (addresses_.length == 0) revert ErrInvalidArguments("addresses_");
        unchecked {
            for (uint256 i = 0; i < addresses_.length; i++) {
                _safeMint(addresses_[i], nums_[i]);
            }
        }
    }
     ...
     */
    function whitelistSale(address address_, uint64 num_) external payable onlyOrderPort nonReentrant {
        if(!_checkTimestamp(_config.whitelistSale.startTime, _config.whitelistSale.endTime)) revert ErrNotStarted();
        uint64 whitelistMinted = _getAux(address_) + num_;
        if (whitelistMinted > _config.whitelistSale.quota) revert ErrQuotaExceeded();
        _sale(address_, num_);
        _setAux(address_, whitelistMinted);
    }
     ...
     * @param num_ number of tokens
     */
    function publicSale(address address_, uint64 num_) external payable onlyOrderPort nonReentrant {
        if(!_checkTimestamp(_config.publicSale.startTime, _config.publicSale.endTime)) revert ErrNotStarted();
        _sale(address_, num_);
    }
     ...
     * @param num_ number of tokens
     */
    function _sale(address address_, uint64 num_) internal {
        if (num_ == 0) revert ErrNoEffect();
        if (totalMinted() + num_ > _config.maxToken) revert ErrMaxSupplyExceeded();
     ...
     */
    function setWhitelistConfig(WhitelistSaleConfig memory config_) external onlyOwner {
     ...
     */
    function setPublicSaleConfig(PublicSaleConfig memory config_) external onlyOwner {
     ...
     */
    function _checkTimestamp(uint256 startTime_, uint256 endTime_) internal view returns (bool) {
        if (startTime_ == 0 || startTime_ > block.timestamp) return false;
        if (endTime_ > 0 && 
            endTime_ < block.timestamp) return false;
        return true;
    }
