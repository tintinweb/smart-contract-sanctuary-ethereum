...
contract Pool is IModule, IYieldFarmingV1Pool, Initializable, AccessControlUpgradeable {
    ...
    function enforcePrerequisites(address registryAddr, bytes32 orgId) external view {
        ...
        require(registry.hasModule(orgId, Modules.YIELDFARMING_V1_STAKING), "Prerequisites: yield_farming_v1_staking is required");
        require(address(staking) == registry.getModuleValue(orgId, Modules.YIELDFARMING_V1_STAKING, 0), "Prerequisites: staking must match registry");

        require(registry.hasModule(orgId, Modules.REWARDS_ESCROW_V1), "Prerequisites: rewards_escrow_v1 is required");
        require(rewardsEscrow == registry.getModuleValue(orgId, Modules.REWARDS_ESCROW_V1, 0), "Prerequisites: rewardsEscrow must match registry");
    }
