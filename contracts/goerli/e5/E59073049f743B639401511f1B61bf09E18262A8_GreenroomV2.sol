...
contract GreenroomV2 is ERC1155, Ownable, EIP712 {
    ...
    constructor() ERC1155("") EIP712("GreenRoom", "1.0.0") {
        fee = 25;

        signerAddress = 0xa2bcd5Cc70E1e568Dd31C475FcB19E8c367B2202;

        showTokenAddr = 0x1dD4d0D2ADB23546a33D87ec50d1Ea61d243d0eC;

        baseUri = "https://api.gmistudios-marketplace.io/collection/";

        contractUri = "https://deadheads.mypinata.cloud/ipfs/QmXuvXS2r3QZLXSncutoqbwfjkdhngzqw6roghwJN2k6CW";
    }
    ...
    function mint(
        ...
        if (item.purchaseType == ItemPurchaseType.ETH) {
            require(total == msg.value, "Wrong price amount");

            item.owner.transfer(total - ((total * fee) / 1000));
        } else {
            require(total == _price * _quantity, "Wrong price amount");

            // price amount of $SHOW token should be approved before

            IERC20(showTokenAddr).safeTransferFrom(
                msg.sender,
                item.owner,
                _price * _quantity
            );
    ...
    function createItem(
        ...
        if (items[_itemId].maxSupply > 0) {
            mint(_itemId, _quantity, _price);
        } else {
            require(_quantity <= _maxSupply, "Quantity exceeds the supply.");

            uint256 total = _price * _quantity;

            if (_purchaseType == ItemPurchaseType.ETH) {
                require(total == msg.value, "Wrong price amount");

                _owner.transfer(total - ((total * fee) / 1000));
            } else {
                // price amount of $SHOW token should be approved before
    ...
    function updateItemActive(uint256 _itemId, bool _active) public {
        require(items[_itemId].maxSupply > 0, "item does not exist");
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "CREATE(address owner,uint256 maxSupply,uint256 price,uint256 purchaseType,uint256 id,address caller)"
                        ),
                        _owner,
                        _maxSupply,
                        _price,
    ...
    function recoverAddress(
        ...
        return
            ECDSA.recover(
                _hash(_owner, _maxSupply, _price, _purchaseType, _id, _account),
                signature
            );
    }
