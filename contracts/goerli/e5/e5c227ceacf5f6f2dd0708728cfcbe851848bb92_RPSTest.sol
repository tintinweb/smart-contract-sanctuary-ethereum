...
contract Generator is IGenerator {
    ...
    function generateSVG(
        ...
        svg = string.concat(svg, _generateStroke(_gameId));
        if (
    ...
    function _generateRoot() internal pure returns (string memory) {
        return "<svg version='1.1' viewBox='0 0 1200 1200' xmlns='http://www.w3.org/2000/svg'>";
    }

    function _generateFill(uint256 _gameId) internal pure returns (string memory) {
        string memory fill = (_gameId % 2 == 0) ? BLACK : WHITE;
    ...
    function _generateStroke(uint256 _gameId) internal pure returns (string memory) {
        string memory stroke = (_gameId % 2 == 0) ? WHITE : BLACK;
        string[3] memory g;
        g[0] = "<g stroke='";
        g[1] = stroke;
        g[2] = "'>";

        return string(abi.encodePacked(g[0], g[1], g[2]));
    }
...
contract RPSTest is IRoshambo, ERC721, ERC721Holder, Ownable {
    using Strings for uint160;
    using Strings for uint256;
    /// @dev Interface identifier for royalty standard
    bytes4 constant _INTERFACE_ID_ERC2981 = 0x2a55205a;
    /// @dev Denominator amount used for calculating payouts
    ...
    /// @notice Mapping of game ID to lobby index
    mapping(uint256 => uint256) public indices;
    /// @notice Mapping of player to record info
    mapping(address => Record) public records;
    /// @notice Mapping of player to payout balances
    mapping(address => uint256) public payouts;

    constructor() payable ERC721("RPSTest", "TEST") {
        beneficiary = owner();
    ...
    function newGame() external payable {
        ...
        Player memory player = Player(msg.sender, Choice.NONE, bytes32(0));
        game.p1 = player;
        game.pot += msg.value;
        game.state = State.PENDING;

        lobby.push(totalSupply);
        indices[totalSupply] = lobby.length - 1;
        records[msg.sender].gameIds.push(totalSupply);

        emit NewGame(totalSupply, msg.sender, msg.value);
    }
    ...
    function joinGame(uint256 _gameId) external payable {
        ...
        uint256 last = lobby.length - 1;
        (lobby[index], lobby[last]) = (lobby[last], lobby[index]);
        lobby.pop();
        records[msg.sender].gameIds.push(_gameId);
    ...
    function settle(uint256 _gameId) external {
        ...
        if (
            ...
            ++r1.rank;
            ++r2.losses;
            if (r2.rank > 1) --r2.rank;
            payouts[player1] += (pot - fee);
            payouts[beneficiary] += fee;
            _burn(_gameId);
            if (c2 != Choice.NONE) _safeMint(player1, _gameId);
        } else if (
            ...
            ++r2.rank;
            ++r1.losses;
            if (r1.rank > 1) --r1.rank;
            payouts[player2] += (pot - fee);
            payouts[beneficiary] += fee;
            _burn(_gameId);
            if (c1 != Choice.NONE) _safeMint(player2, _gameId);
        } else {
    ...
    function cancel(uint256 _gameId) external {
        if (_gameId > totalSupply) revert InvalidGame();
        Game memory game = games[_gameId];
        uint256 pot = game.pot;
        if (game.state != State.PENDING) revert InvalidState();
        if (game.p1.player != msg.sender) revert InvalidPlayer();
...
import {Game, Player} from "src/interfaces/IRoshambo.sol";
...
interface IRoshambo {
    ...
    function indices(uint256) external view returns (uint256);
