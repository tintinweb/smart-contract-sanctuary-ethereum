...
contract EarlyMintersRoyalties is ContractRestricted, IEarlyMintersRoyalties {
    ...
    function withdraw() public override payable{
     ...
     * @dev 
     */
    function safeOwnerWithdraw() public override payable onlyOwner {
        require(
            lastWithrawPeriod[msg.sender].current() < currentPeriod.current(),
            "Withdraw is up to date"
        );
        ...
        lastWithrawPeriod[msg.sender].increment();
        (bool hs, ) = payable(owner()).call{value: amount}("");
        require(hs);
    }
