...
contract LuckyDraw is Ownable {
    ...
    event TransferEvent(uint256 _round ,address indexed _from, address indexed _to, uint256 _value);
    ...
    function register(string memory accountName) public payable {
        require(msg.value == 1 ether, "Register sizes are restricted to 0.001 ether");
        ...
        onlyOwner 
    {
        ...
        emit TransferEvent(currentRound, address(this), toAccount, reward);
        payable(owner()).transfer(fee);
        emit TransferEvent(currentRound, address(this), owner(), fee);
        adminReset();
    }
