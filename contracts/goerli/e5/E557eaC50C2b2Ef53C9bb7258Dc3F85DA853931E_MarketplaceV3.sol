...
contract MarketplaceV3 is Initializable,ReentrancyGuardUpgradeable {
    ...
    function _takeFeeValueDistribution (bool isToken, uint256 feeValue) internal returns (uint256 etherFeeTaken, uint256 cashbackAmount) { 
        ...
        if (isToken) {
            ...
            payable(platform).sendValue(feeValue);//we cant transfer feeValue because in case if we are calling

            return (feeValue, cashbackAmount);
        }
    }

    function getContractBalanceV3()public view returns(uint){
