...
contract eth_grd is ERC721Enumerable, AccessControl {
    ...
    constructor(string memory _uri) ERC721("TEST_GRD", "TESTGRD") {
    ...
    function mint(address user, uint256[] calldata tokenIds) external onlyRole(MINTER_ROLE) {
        for (uint256 j = 0; j < tokenIds.length; j++) {
            _mint(user, tokenIds[j]);
        }
    }
