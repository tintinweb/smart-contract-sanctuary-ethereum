 ...
 */
contract Alienators is Ownable, AccessControlEnumerable, ERC721Enumerable, ReentrancyGuard {
    using Counters for Counters.Counter;

    uint256 public MAX_SUPPLY = 5000;
    uint256 public MAX_SUPPLY_PUBLIC_MINT = 1000;
    ...
    // WARNING: Can only be called before reveal
    function updateMintPrice(uint256 _mintCost) external onlyOwner {
        require(!revealed);
        MINT_COST = _mintCost;
    }

    // WARNING: Can only be called before reveal
    function updateMintVault(address _mintVault) external onlyOwner {
        require(!revealed);
        MINT_VAULT = payable(_mintVault);
    }
    ...
    // WARNING: Can only be called before reveal
    function updateMaxSupplyPublicMint(uint256 _maxSupplyPublicMint) external onlyOwner {
        require(!revealed);
        MAX_SUPPLY_PUBLIC_MINT = _maxSupplyPublicMint;
    }
    ...
    function _mint(address to, uint8 quantity, bytes32[] memory merkleProof) private {
        require(openMint, 'Minter: Minting is not open');
        require(quantity > 0, 'Minter: Quantity must be > 0');
        require(msg.value >= MINT_COST * quantity, 'Minter: Not enough ether');
        require(openPublicMint || totalSupply() < (MAX_SUPPLY - MAX_SUPPLY_PUBLIC_MINT), 'Minter: Whitelisted minting limit reached');
        ...
        if (!openPublicMint) {
            ...
            require(openPublicMint || MerkleProof.verify(merkleProof, merkleRoot, node), 'Minter: Invalid proof');
        }
    ...
    // Opens minting for non-whitelisted addresses
    function publicMintOpen() external onlyOwner {
        openPublicMint = true;
    }

    // Closes minting for non-whitelisted addresses
    function publicMintClose() external onlyOwner {
        openPublicMint = false;
    }
}
