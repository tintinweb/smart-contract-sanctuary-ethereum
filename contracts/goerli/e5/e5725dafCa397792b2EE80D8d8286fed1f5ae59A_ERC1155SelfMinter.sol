...
contract ERC1155SelfMinter is ERC1155, ReentrancyGuard, DefaultOperatorFilterer, Ownable {
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < tiersNumbers.length; i++) {
            tiers[tiersNumbers[i]] = Tier(starts[i], ends[i], merkleRoots[i], limitPerWalletPerTier[i]);
        }
    ...
    function tierMint(
        ...
        if(address(token) == address(0)){
            ...
            ERC20(token).transferFrom(msg.sender, treasuryWallet, mintFee);
        }
    ...
    function partnerTierMint(
        ...
        if (address(token) == address(0)) {
            ...
            _partnerAmount = mintFee * partners[partnerWallet].sharePct / 100;

            ERC20(token).transferFrom(msg.sender, treasuryWallet, mintFee - _partnerAmount);
    ...
    function _checks(
        ...
        if(!MerkleProof.verify(_merkleProof, tiers[tierNo].merkleRoot, keccak256(abi.encodePacked(mintAddress, tierNo)))) revert InvalidProof();
        ...
        if(mintedPerAddress[mintAddress] + amount > tiers[tierNo].limitPerWalletPerTier) revert TierLimitReached();
        ...
        if (address(token) == address(0)) {
            ...
            require(
                ERC20(token).balanceOf(msg.sender) >= mintFee * amount,
                "doesn't have enough tokens to mint the NFT"
            );
        }
    }
