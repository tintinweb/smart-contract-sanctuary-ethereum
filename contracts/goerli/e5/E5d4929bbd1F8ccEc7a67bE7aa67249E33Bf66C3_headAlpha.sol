...
contract headAlpha is KeyInterface, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) accessMinted;
    uint256[] public accessIDs;
    uint256 nextID = 0;
    IERC20 public erc20Token;


    constructor() ERC1155("https://api.beargame.io/api/buckets/") {
        ...
        _addmonth(10000, 1000, true);


    }

    function mintAccess(uint256 _amount, uint256 _type) external {
        address msgsender = _msgSender();
        
        AccessInfo memory _info = accessInfo[_type];
        accessMinted[_type] += _amount;

        require(tx.origin == msgsender, "Only EOA");
        require(_type < accessIDs.length, "Access ID Doesnt Exist");
        require(_info.saleLive,"Sale for this type is not active");
        require(accessMinted[_type] < _info.limit,"Sale for this type is sold out");
    ...
    function addNewMonth(uint256 _cost, uint256 _limit, bool _live) external onlyOwner {
        _addmonth(_cost,_limit,_live);
    }

    function addCustomAccess(uint256 _id, uint256 _cost, uint256 _limit, bool _live) external onlyOwner {
    ...
    function setAccessPrice(uint256 _id, uint256 _cost) external onlyOwner {
        AccessInfo memory _info = accessInfo[_id];
        accessInfo[_id]  = AccessInfo(_cost * 10 ** 18, _info.limit, _info.saleLive);

    }

    function setAccessLimit(uint256 _id, uint256 _limit) external onlyOwner {
        AccessInfo memory _info = accessInfo[_id];
        accessInfo[_id]  = AccessInfo(_info.cost, _limit, _info.saleLive);

    }
    
    function _addmonth(uint256 _cost, uint256 _limit, bool _live) internal {
    ...
    function setHeadAddress(address _head) external onlyOwner {
