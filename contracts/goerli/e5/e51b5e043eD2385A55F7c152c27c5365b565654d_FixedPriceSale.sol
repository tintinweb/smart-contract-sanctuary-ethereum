...
import "./FixedPriceSaleStorage.sol";
    ...
    UUPSUpgradeable
{
    ...
    event FixedPriceDropRePublished(
        uint256 indexed dropId,
        uint256[] itemIds,
        uint256 price,
        uint256 start
    );
    event FixedPriceDropCancelled(uint256 indexed dropId);
    ...
    function initialize(address admin, address minter)
        ...
        initializer
    {
        __FixedPriceSale_init(admin, minter);
    }

    function __FixedPriceSale_init(address admin, address minter)
        internal
        onlyInitializing
    {
        __Ownable_init();
        ...
        __FixedPriceSale_init_unchained(admin, minter);
    }

    function __FixedPriceSale_init_unchained(address admin, address minter)
        internal
        onlyInitializing
    {
        FixedPriceSaleStorage.Layout storage layout = FixedPriceSaleStorage
            .layout();
    ...
    function setMinter(address minter) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        FixedPriceSaleStorage.layout().minter = minter;
    }
    ...
    {
        FixedPriceDrop memory fpDrop = FixedPriceSaleStorage.layout().drops[
            dropId
        ];
    ...
    function isItemInDrop(uint256 dropId, uint256 itemId)
        ...
        returns (bool)
    {
        uint256[] memory itemIds = FixedPriceSaleStorage
            .layout()
            .drops[dropId]
            .itemIds;
    ...
    function validateVoucher(
        ...
        if (FixedPriceSaleStorage.layout().vouchersUsed[voucherId])
            revert VoucherUsed();
    ...
    modifier validateMintAuth(MintAuth calldata mintAuth) {
        if (mintAuth.dropType != DropType.FixedPrice) revert InvalidDropType();
    ...
    function publishDrop(
        ...
        FixedPriceSaleStorage.Layout storage fp = FixedPriceSaleStorage
            .layout();
        ...
        fp.drops[dropId] = FixedPriceDrop(
            collection,
            itemIds,
            price,
            start,
        ...
        emit FixedPriceDropPublished(dropId, collection, itemIds, price, start);
    }
    ...
    function rePublishDrop(
        ...
        FixedPriceDrop memory fpDrop = layout.drops[dropId];

        if (fpDrop.cancelled) revert DropCancelled(dropId);
        if (fpDrop.itemIds.length == 0) revert DropNotFound(dropId);

        emit FixedPriceDropRePublished(dropId, itemIds, price, start);
        ...
        if (block.timestamp < fpDrop.start) {
            ...
            emit FixedPriceDropRePublished(
                dropId,
                fpDrop.itemIds,
                price,
                fpDrop.start
        ...
        notPaused
    {
        ...
        emit FixedPriceDropCancelled(dropId);
    }
    ...
    function _mint(
        ...
        FixedPriceDrop memory fpDrop = fp.drops[dropId];
        ...
        IMinterceptor minterceptor = IMinterceptor(fp.minter);

        fp.drops[dropId].numMinted++;

        // transfer bid funds to Minterceptor contract & mint item
        minterceptor.mintByUri{value: msg.value}(
            fpDrop.collection,
            to,
            uri,
            data
    ...
    function authorizedMint(MintAuth calldata mintAuth)
        public
        payable
        validateMintAuth(mintAuth)
    {
        // expire the nonce immediately to avoid re-entrancy
        FixedPriceSaleStorage.layout().vouchersUsed[mintAuth.id] = true;
...
interface IFixedPriceSale {
    ...
    function authorizedMint(MintAuth calldata mintAuth) external payable;
}
...
error ItemMinted(uint256 itemId);
...
library FixedPriceSaleStorage {
    struct Layout {
        address minter;
        mapping(uint256 => bool) vouchersUsed;
        mapping(uint256 => FixedPriceDrop) drops;
    }

    bytes32 internal constant STORAGE_SLOT =
        keccak256("quantum.contracts.storage.fixedpricesale.v1");
