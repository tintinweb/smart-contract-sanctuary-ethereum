...
/**
    @title Global Fit Collective Origin Pass
    @notice NFT will reside on ETH mainnet, while the utility token to which it connects will be deployed on Polygon
    @author [email protected] (curion#0001)
    +thanks to api3, hyperlane, and openzeppelin for the goods
*/   
...
contract GFC_ORIGIN is ERC1155, AccessControl, RrpRequesterV0 {
    ...
    // [ ] will setGoldIds work? what will gas be? (Est: X Gas)
    //----------------------
    ...
    address public withdrawToAddress; //payment splitter for GFC treasury/team

    address[] public partnerNftAddresses; //array of partner addresses (NTS1, NTS2, DP, others to come)
    ...
    uint256 public goldSpacing; //totalSupply must be divisible by goldSupply
    ...
    MessageLog[] public pendingMessagesForUtilityToken;
    MessageLog[] public sentMessagesForUtilityToken;
        ...
        RrpRequesterV0(_airnodeRrp)
    {
        ...
        mintSupply = 10000;
        goldSupply = 500;
        goldSpacing = Math.mulDiv(totalSupply, 1, goldSupply);
        maxMintsPerTxn = 20;
    ...
    function requestMint(
        ...
        if( msg.value < (mintPrice * _mintQuantity) + apiCallForwardingValue + hyperlaneForwardingValue ) { revert InsufficientFunds(); }
        ...
        sendQrngRequest(_minter, _mintQuantity, hyperlaneForwardedGasFee(_offChainDeterminedGasFee));
    }
    ...
    function requestWhitelistMint(
        ...
        if( !isValidMerkle(_proof, _leaf, _minter) && !hasRole(ADMIN_ROLE, msg.sender) ) { revert InvalidMerkleOrWrongSender(); }
        if( whitelistMintIsPaused || !hasRole(ADMIN_ROLE, msg.sender) ) { revert MintPaused(); }
        if( mintSupply == 0 ) { revert MaxSupplyReached(); }
        if( _mintQuantity == 0 ) { revert ZeroMintQuantity(); }
        if( _mintQuantity > maxMintsPerTxn ) { revert ExceededMaxMintsPerTxn(); }
        if( msg.value < (whitelistMintPrice * _mintQuantity) + apiCallForwardingValue + hyperlaneForwardingValue ) { revert InsufficientFunds(); }
        
        //check that this is required beyond msg.value (i.e. both will not result in more funds being requested than intended)
        (bool fwd, ) = sponsorWallet.call{value: apiCallForwardingValue }(""); 
        if(!fwd){ revert ForwardFailed(); }

        sendQrngRequest(_minter, _mintQuantity, _offChainDeterminedGasFee);
    }
        ...
        onlyAirnodeRrp
    {
        ...
        for (uint256 i = 0; i < mintQuantity; i++) {
            ...
            amountMinted[mintTo] += mintQuantity; //add to amount minted before calling mint function
            
            if(utilityTokenIsDeployed){
                sendOwnerInfo(transferSignalToChild, requestIdToMintCostCalculatedOffChain[_requestId]);
                sentMessagesForUtilityToken.push(MessageLog(mintTo, block.timestamp, transferSignalToChild));
            } else {
                pendingMessagesForUtilityToken.push(MessageLog(mintTo, block.timestamp, transferSignalToChild));
    ...
    /// @dev Returns the tier of the token to be minted, and manages both gold tier and total supplies
    function getTierFromQrnAndManageSupply(uint256 _QRN) private returns (uint256) {
        ...
        uint256 thisRandomGoldIdIndex = (_QRN % goldSupply) + 1; //for removing gold ids in random order, should one be selected, to preserve odds
        uint256 thisTier;
        if (isGoldId(thisRandomId)) {
            thisTier = 2;
            goldSupply--;
            removeGoldId(thisRandomGoldIdIndex);
        } else {
            thisTier = 1;
        }
        mintSupply--;
    ...
    /// @dev checks if current id is that of a gold membership pass
    function isGoldId(uint256 _tokenId) private view returns (bool) {
        for (uint i = 0; i < goldIds.length; i++) {
            if (goldIds[i] == _tokenId) {
                return true;
            }
        }
        return false;
    ...
    function isValidMerkle(bytes32[] memory _proof, bytes32 _leaf, address _mintingAddress) private view returns (bool) {
        if (_leaf == keccak256(abi.encodePacked(_mintingAddress)) ) { //redundant?
    ...
    function updatePartnerNFTBalancesOnUtilityToken(address _requester) public {
        ...
        sentMessagesForUtilityToken.push(MessageLog(_requester, block.timestamp, transferSignalToChild));
    ...
    function sendPendingMessages(uint256 _offChainDeterminedGasFee) public onlyRole(ADMIN_ROLE) {
        ...
        for(uint256 i=0; i<pendingMessagesForUtilityToken.length; i++) {
            sendOwnerInfo(pendingMessagesForUtilityToken[i].message, hyperlaneForwardedGasFee(_offChainDeterminedGasFee));
            sentMessagesForUtilityToken.push(MessageLog(msg.sender, block.timestamp, pendingMessagesForUtilityToken[i].message));
        }
    }
    
    function sendMessageManually(bytes memory _message, address _userAddress, uint256 _offChainDeterminedGasFee) public onlyRole(ADMIN_ROLE) {
        sendOwnerInfo(_message, hyperlaneForwardedGasFee(_offChainDeterminedGasFee));
        sentMessagesForUtilityToken.push(MessageLog(_userAddress, block.timestamp, _message));
    }
    ...
    /// @dev called once upon deployment to spread goldIds across the supply so random chance yields ~same odds throughout mint of a gold pass
    function setGoldIds() public onlyRole(ADMIN_ROLE) {
        uint256 goldId = goldSpacing;
        goldIds.push(1);
    ...
    function adminMintTo(
        ...
        for(uint256 i=0; i<_tierArray.length; i++){
            ...
            if(_tierArray[i]==1) {
                _mint(_recipient, _tierArray[i], 1, ""); // _mint(account, id, amount, data);
            } else {
                removeGoldId(goldIds[goldIds.length-1]);
                goldSupply--;
                _mint(_recipient, _tierArray[i], 1, ""); // _mint(account, id, amount, data);
            }
            ...
            if(utilityTokenIsDeployed){
                ...
                pendingMessagesForUtilityToken.push(MessageLog(msg.sender, block.timestamp, transferSignalToChild));
    ...
    function removeGoldId(uint256 _index) private {
        if(_index >= goldIds.length) {revert InvalidIndex(); }
        for (uint256 i = _index; i<goldIds.length-1; i++){
            goldIds[i] = goldIds[i+1];
        }
        goldIds.pop();
    }
    ...
    function safeTransferFrom(
        ...
        for(uint256 i=0; i<amount; i++) {
            ...
            if (utilityTokenIsDeployed) {
                ...
                pendingMessagesForUtilityToken.push(MessageLog(from, block.timestamp, transferSignalToChild));
