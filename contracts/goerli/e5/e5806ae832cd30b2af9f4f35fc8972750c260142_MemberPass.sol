...
contract ERC5643 is ERC721Enumerable, IERC5643, Ownable {
    ...
    /**
     * @dev extend subscription, can only be called by the owner.
     * Enables extending of subscription without any payment.
     */
    function _extendSubscriptionFor(uint256 tokenId, uint64 duration)
        internal
        virtual
        onlyOwner
    {
    ...
    function hasValidSubscription(address user)
        external
        view
        virtual
        returns (bool isValid)
    {
        // TODO: Handle scenarios where user may have more than one subscription NFT. Perhaps block it during the minting function.
        uint256 tokenId = tokenOfOwnerByIndex(user, 0);
        return _expirations[tokenId] > block.timestamp;

        // try  returns (uint256 tokenId) {
...
interface IERC5643 {
    ...
    /// @notice Checks if a given user has a valid subscription NFT
    /// @param user The user to check the subscription for
    /// @return The status of whether the user has a valid subscription 
...
contract MemberPass is ERC5643 {
    ...
    function mintForWithSubscription(
        address to,
        uint256 tokenId,
        uint64 duration
    ) public onlyOwner {
        _mint(to, tokenId);
        _extendSubscriptionFor(tokenId, duration);
    }
