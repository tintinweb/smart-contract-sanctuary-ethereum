...
contract Mock1155 is AbstractERC1155Factory {
    ...
    uint256 public earlyAccessWindowOpens = 1639760400;
    uint256 public purchaseWindowOpens = 1639760400;
    uint256 public purchaseWindowCloses = 4101134400;

    uint256 public burnWindowOpens = 1639760400;
    uint256 public burnWindowCloses = 4101134400;
    ...
    function purchase(uint256 amount) external payable whenNotPaused {
        require(
            block.timestamp >= purchaseWindowOpens &&
                block.timestamp <= purchaseWindowCloses,
     ...
     */
    function redeemCardForOther(
        uint256 cardIdToRedeem,
        uint256 amount
    ) external whenNotPaused {
        require(
            balanceOf(msg.sender, cardIdToRedeem) >= amount && amount > 0,
            "BurnCardForOther: amount not allowed"
        );
        require(
            block.timestamp >= burnWindowOpens &&
                block.timestamp <= burnWindowCloses,
            "BurnCardForOther: window closed"
        );
        require(
            cardIdToRedeem < cardIdToMint,
            "BurnCardForOther: card cannot be burned"
        );
