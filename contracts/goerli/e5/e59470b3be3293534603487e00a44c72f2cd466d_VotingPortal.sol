...
contract VotingPortal is IVotingPortal {
  ...
  address public immutable GOVERNANCE_CORE;
  ...
  function receiveCrossChainMessage(
    ...
    IGovernanceCore(GOVERNANCE_CORE).queueProposal(
      proposalId,
      forVotes,
      againstVotes
    );
...
interface IGovernanceCore {
   ...
   * @param executionPortal address of the executionPortal used for communication with the execution chain
...
interface IVotingPortal is IBaseReceiverPortal {
  ...
  /// @dev gets the address of the connected governance core
  function GOVERNANCE_CORE() external view returns (address);
