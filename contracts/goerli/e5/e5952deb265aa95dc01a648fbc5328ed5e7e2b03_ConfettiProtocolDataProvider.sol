...
contract ConfettiProtocolDataProvider {
        ...
        returns (ReserveTokenData[] memory)
    {
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            reservesTokens[i] = ReserveTokenData({
                ...
                cTokenSymbol: IERC20Detailed(reserveData.cTokenAddress).symbol(),
    ...
    function getReserveTokenData(address asset)
        external
        view
        returns (ReserveTokenData memory)
    {
        ...
        returns (NftTokenData[] memory)
    {
        ...
        for (uint256 i = 0; i < nfts.length; i++) {
            ...
            nftTokens[i] = NftTokenData({
                ...
                nftAddress: nfts[i],
                cNFTSymbol: IERC721Detailed(nftData.cNFTAddress).symbol(),
                cNFTAddress: nftData.cNFTAddress
            });
        }
        return nftTokens;
    }

    function getNftTokenData(address nftAsset)
        external
        view
        returns (NftTokenData memory)
    {
        ...
        return
            NftTokenData({
                ...
                nftAddress: nftAsset,
                cNFTSymbol: IERC20Detailed(nftData.cNFTAddress).symbol(),
                cNFTAddress: nftData.cNFTAddress
            });
    }
        ...
        returns (NftConfigurationData memory configData)
    {
        ...
        (
            ...
            configData.buyItNowDuration,
            configData.redeemFine,
            configData.redeemThreshold
        ) = configuration.getBuyItNowParamsMemory();

        (configData.isActive, configData.isFrozen) = configuration
            .getFlagsMemory();

        (configData.minBuyItNowFine) = configuration.getMinBuyItNowFineMemory();
    }
        ...
        )
    {
        ...
        return (
            IERC20Detailed(asset).balanceOf(reserve.cTokenAddress),
        ...
        )
    {
        ...
        currentCTokenBalance = IERC20Detailed(reserve.cTokenAddress).balanceOf(
            user
        );
    ...
    function _fillLoanData(
        ...
        loanData.buyItNowStartTimestamp = loan.buyItNowStartTimestamp;
        loanData.buyItNowBuyerAddress = loan.buyItNowBuyerAddress;
        loanData.buyItNowPrice = loan.buyItNowPrice;
        loanData.buyItNowBorrowAmount = loan.buyItNowBorrowAmount;
    }
}
