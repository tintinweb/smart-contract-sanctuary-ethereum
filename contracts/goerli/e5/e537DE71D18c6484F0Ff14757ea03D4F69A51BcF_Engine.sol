...
library FeesMath {
    ...
    function premium(
        ...
        premium_ = o.point.premium.bps(notional);
...
library JoinLib {
    ...
    function normalize(Match memory m, uint amount)
        public
        pure
...
library LineMath {
    ...
    /// @notice pointA is (minPremium, maxFR), pointB is (maxPremium, minFR)
    function validate(
        Point calldata pointA,
        Point calldata pointB,
        IConfig config
    ) external view {
        console.log(
            'config.minFRPerYear: ',
        ...
        require(pointB.fundingRate > 0, 'IL');
        require(uint24(pointB.fundingRate) >= config.minFRPerYear(), 'IL');
        require(pointA.premium <= pointB.premium, 'IL');
        require(pointA.fundingRate >= pointB.fundingRate, 'IL');
    }

    /// @notice deduct the slope of the line describe by the 2 points
    function getSlope(Point memory pointA, Point memory pointB)
        internal
        pure
        returns (int slope)
    {
        slope = int24(pointB.fundingRate - pointA.fundingRate).div(
            pointB.premium - pointA.premium
        );
    }

    function validatePoint(Match memory m, Point calldata point)
        external
        pure
        returns (bool)
    {
        require(
            point.premium >= m.pointA.premium &&
                point.premium <= m.pointB.premium,
            'IP'
        );
        require(
            point.fundingRate >= m.pointB.fundingRate &&
                point.fundingRate <= m.pointA.fundingRate,
            'IP'
        );
        // check if the point is at one extreme of the line
        if (
            (point.premium == m.pointA.premium &&
                point.fundingRate == m.pointA.fundingRate) ||
            (point.premium == m.pointB.premium &&
        ...
        int lineSlope = getSlope(m.pointA, m.pointB);
        int pointSlope = getSlope(m.pointA, point);
...
library PnLMath {
        ...
        )
    {
        ...
        console.log(m.point.fundingRate);
    ...
    function makerFRfees(uint makerFRFee, uint fundingRate)
        internal
        pure
        returns (uint)
    {
...
struct Match {
    ...
    Point pointA; // For line offers, pointA is (minPremium, maxFR)
    Point pointB; // For line offers, pointB is (maxPremium, minFR)
    Point point; // The actual point chosen by the trader
...
contract Engine is IEngineEvents {
    ...
    function getOrder(uint idx) external view returns (Order memory) {
        return opens[idx];
    }
    ...
    function open(
        ...
        LineMath.validate(pointA, pointB, config);
        ...
        if (
            matches[matchId].isOverLeveraged(
                oracle.getPrice(),
                config,
                pointB.fundingRate, // use the worst funding rate for the leverage check
                true,
                tokenDecimals
            )
        ...
        if (msg.sender == vault.ownerOf(matches[matchId].maker)) {
            ...
            if (
                matches[matchId].isOverLeveraged(
                    priceData,
                    config,
                    matches[matchId].point.fundingRate,
                    true,
                    tokenDecimals
                )
