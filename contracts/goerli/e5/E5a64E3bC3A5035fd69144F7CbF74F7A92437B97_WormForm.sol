...
contract WormForm is ERC721Upgradeable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, IWormForm {
    ...
    /**
     * @dev Function for minting a token with a given `_id` to on of the `_addresses`.
     * @param _addresses Array of addresses to whome `_id`s will be minted.
     * @param _ids IDs of token to mint one of the `_addresses`.
     */
    function mintToAddresses(address[] memory _addresses, uint256[] memory _ids) external onlyOwner {
        require(_addresses.length != 0, "At lease one entry must be provided.");
        require(_addresses.length < 26, "Only 25 addresses maximum per call.");
        require(_addresses.length == _ids.length, "Arrays must be the same length.");
        
        uint256 i;
        for (i; i < _addresses.length; i++) {
            if (_idIsClaimable(_ids[i])) {
                tokenIdClaimable[_ids[i]] == false;
            }
...
interface IWormForm {
    ...
    function mintToAddresses(address[] memory _addresses, uint256[] memory _ids) external;
    ...
    function changeOldNFTContract(address _new) external;
