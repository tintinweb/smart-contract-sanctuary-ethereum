    ...
    function publicMint(
        ...
        if (code > 0) {
            ...
            if (proCode) {
                ...
                require(
                    IBrokerDefiPartner(BROKER_DEFI_PARTNER).partnerCodesVerification(code),
                    "Wrong code"
                );
                partnerCodesCount[code] += 1;
                commission = (msg.value * PARTNER_COMMISSION) / 100;
                discount = (msg.value * PARTNER_DISCOUNT) / 100;
                uint tokenId = IBrokerDefiPartner(BROKER_DEFI_PARTNER).codeOwners(code);
                recruiter = payable(IBrokerDefiPartner(BROKER_DEFI_PARTNER).ownerOf(tokenId));
            }
