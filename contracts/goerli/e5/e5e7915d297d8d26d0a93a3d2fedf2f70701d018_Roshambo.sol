...
contract Roshambo is IRoshambo, ERC721, ERC721Holder, Ownable {
    ...
    Generator public immutable generator;
    /// @notice Address of RecordBook contract
    RecordBook public immutable record;
    ...
    function newGame() external payable returns (uint256) {
        ...
        if (record.balanceOf(msg.sender) == 0) record.mint(msg.sender);
    ...
    function reveal(uint256 _gameId, Choice _choice, string memory _secret) external {
        ...
        if (p1.player == msg.sender) {
            if (p1.choice != Choice.NONE) revert AlreadyRevealed();
            _verifySecret(_gameId, msg.sender, _choice, _secret, p1.commitment);
            p1.choice = _choice;
        } else if (p2.player == msg.sender) {
            if (p2.choice != Choice.NONE) revert AlreadyRevealed();
            _verifySecret(_gameId, msg.sender, _choice, _secret, p2.commitment);
            p2.choice = _choice;
        } else {
            revert InvalidPlayer();
        }
    ...
    function settle(uint256 _gameId) external {
        ...
        if (
            ...
            _setRecord(player1, _gameId, true, false);
            _setRecord(player2, _gameId, false, true);
            if (c2 != Choice.NONE) _transferGame(_gameId, player1);
        } else if (
            ...
            _setRecord(player2, _gameId, true, false);
            _setRecord(player1, _gameId, false, true);
            if (c1 != Choice.NONE) _transferGame(_gameId, player2);
        } else {
            game.state = State.DRAW;
            payouts[player1] += pot / 2;
            payouts[player2] += pot / 2;
            _setRecord(player1, _gameId, false, false);
            _setRecord(player2, _gameId, false, false);
        }
    ...
    function generateCommit(
        ...
        return keccak256(abi.encodePacked(_gameId, _player, _choice, _secret));
    }
    ...
    function _setRecord(address _player, uint256 _gameId, bool _win, bool _loss) internal {
        record.setRecord(_player, _gameId, _win, _loss);
    }

    function _transferGame(uint256 _gameId, address _player) internal {
        _burn(_gameId);
    ...
    function _verifySecret(
        ...
        if (keccak256(abi.encodePacked(_gameId, _player, _choice, _secret)) != _commitment)
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        string memory image = Base64.encode(
            abi.encodePacked(generator.generateSVG(_tokenId, p1, p2))
        );
