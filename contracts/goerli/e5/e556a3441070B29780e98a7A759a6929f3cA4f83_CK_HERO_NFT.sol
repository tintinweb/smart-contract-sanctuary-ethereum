 ...
 * @author CK_HERO_NFT
 */
contract CK_HERO_NFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    // Mapping owner address to token count
    mapping(address => uint256) private _balances;

    // contract STEPNNFT is E, Ownable {
    // base uri for nfts
    string private _buri;

    bool private isAllowListActive;

    mapping(address => uint8) private _allowList;

    constructor() ERC721("CK_HERO_NFT", "CK_HERO_NFT") {}
    ...
    function freeMint(uint256 tokenId) external nonReentrant {
        ...
        require(1 <= _allowList[msg.sender], "not in white list");
        _allowList[msg.sender] -= 1;
        _safeMint(msg.sender, tokenId);
    }

    function removeAllowList(address addr) external onlyOwner {
