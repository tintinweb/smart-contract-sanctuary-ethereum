...
contract PixelmonTrainerAdventure is WinnerSelectionManager, Utils, ReentrancyGuard {
    ...
    function claimTreasure(bytes32[] calldata proof, uint256 _weekNumber) external nonReentrant noContracts {
    ...
    function removeTreasureFromPoolExternal(uint256[] memory _poolIndexs, uint256 _weekNumber)
        external
    ...
    function resetTreasurePool(uint256 _weekNumber) public onlyAdmin(msg.sender) validPoolUpdationPeriod(_weekNumber) {
...
contract WinnerSelectionManager is Ownable, VRFConsumerBaseV2 {
    ...
    function setMerkleRoot(bytes32 merkle, uint256 _weekNumber)
        external
        onlyModerator(msg.sender)
        validWeekNumber(_weekNumber)
        validWinnerUpdationPeriod(_weekNumber)
    ...
    function updateMerkleRoot(bytes32 merkle, uint256 _weekNumber) external onlyModerator(msg.sender) validWeekNumber(_weekNumber) {
        if (merkle[0] == 0) {
            revert InvalidMerkleRoot();
        }
        weekInfos[_weekNumber].winnersMerkleRoot = merkle;
