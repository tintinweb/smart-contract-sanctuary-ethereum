...
contract SoulAether is NotablesERC721A {
    ...
    uint256 constant discount = 0.00005 ether;
    ...
    constructor() ERC721A("SoulAether", "SAE") {
        waveTimes = [block.timestamp, block.timestamp + 12 hours, block.timestamp + 2 hours, block.timestamp + 10 hours];
    }

    function privateMint(bytes32[] calldata _proof) external payable {
        uint8 wave = getWave();

        require(wave != 3, "Private mint is not available");
        require(msg.value >= getCost(wave), "Ether value sent is below the price");
        require(totalSupply() < maxSupply, "Sold out");
        require(!addressClaimed[msg.sender], "Address has already claimed");
    ...
    function airdrop(address[50] calldata _addresses) external onlyOwner {
        require(getWave() == 1, "Airdrop is only available during Wave 1");
    ...
    function getCost(uint8 _wave) private pure returns (uint256) {
        if(_wave == 1) {
            return discount;
        }
        return cost;
    ...
    function getAddressClaimed(address _address) external view returns (bool) {
