...
import "./mixins/mintMarket/MintMarketCore.sol";
import "./mixins/mintMarket/MintMarketFixedPriceDrop.sol";

/**
 * @title A market for minting NFTs with Foundation.
 */
contract FNDMintMarket is
  Initializable,
...
abstract contract MarketFees is FoundationTreasuryNode, MarketSharedCore, SendValueWithFallbackWithdraw {
  ...
  {
    seller = _getSellerFor(nftContract, tokenId);
 ...
 * @dev This also leaves a gap which can be used to add a new mixin to the top of the inheritance tree.
 */
abstract contract MintMarketCore {
  /**
   * @notice This empty reserved space is put in place to allow future versions to add new
   * variables without shifting down storage in the inheritance chain.
   * See https://docs.openzeppelin.com/contracts/4.x/upgradeable#storage_gaps
...
error MintMarketFixedPriceCollectionDrop_Cannot_Buy_At_Lower_Price(uint256 mintCost);
error MintMarketFixedPriceCollectionDrop_Only_Callable_By_Collection_Owner();
error MintMarketFixedPriceCollectionDrop_Must_Support_Collection_Mint_Interface();
error MintMarketFixedPriceCollectionDrop_Must_Not_Be_Sold_Out();
error MintMarketFixedPriceCollectionDrop_Limit_Per_Account_Must_Be_Set();
error MintMarketFixedPriceCollectionDrop_Must_Not_Have_Pending_Sale();
error MintMarketFixedPriceCollectionDrop_Must_Buy_At_Least_One_Token();
error MintMarketFixedPriceCollectionDrop_Must_Have_Sale_In_Progress();
/// @param limitPerAccount The limit of tokens an account can purchase.
error MintMarketFixedPriceCollectionDrop_Cannot_Buy_More_Than_Limit(uint256 limitPerAccount);

/**
 * @title Allows creators to list a collection for sale at a fixed price point.
 */
abstract contract MintMarketFixedPriceDrop is MarketFees {
  using AddressUpgradeable for address;
  using AddressUpgradeable for address payable;
  using ERC165Checker for address;
  ...
  function createFixedPriceSale(
    ...
    if (!nftContract.supportsInterface(type(ICollectionMint).interfaceId)) {
      revert MintMarketFixedPriceCollectionDrop_Must_Support_Collection_Mint_Interface();
    }
    ...
    if (
      ...
      revert MintMarketFixedPriceCollectionDrop_Only_Callable_By_Collection_Owner();
    }
    if (ICollectionMint(nftContract).isSoldOut() == true) {
      revert MintMarketFixedPriceCollectionDrop_Must_Not_Be_Sold_Out();
    }
    if (limitPerAccount == 0) {
      revert MintMarketFixedPriceCollectionDrop_Limit_Per_Account_Must_Be_Set();
    }

    SaleConfig storage saleConfig = nftContractToSaleConfig[nftContract];
    if (saleConfig.seller != payable(0)) {
      revert MintMarketFixedPriceCollectionDrop_Must_Not_Have_Pending_Sale();
    }
  ...
  function _mintFromFixedPriceSaleAndApproveMarket(
    ...
    if (count == 0) {
      revert MintMarketFixedPriceCollectionDrop_Must_Buy_At_Least_One_Token();
    }

    SaleConfig storage saleConfig = nftContractToSaleConfig[nftContract];
    if (saleConfig.seller == payable(0)) {
      revert MintMarketFixedPriceCollectionDrop_Must_Have_Sale_In_Progress();
    }
    ...
    if (mintCost > totalBuyPrice) {
      revert MintMarketFixedPriceCollectionDrop_Cannot_Buy_At_Lower_Price(mintCost);
    }

    if (IERC721(nftContract).balanceOf(msg.sender) + count > saleConfig.limitPerAccount) {
      revert MintMarketFixedPriceCollectionDrop_Cannot_Buy_More_Than_Limit(saleConfig.limitPerAccount);
    }
