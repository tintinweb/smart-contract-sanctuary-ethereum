...
contract Sando is ERC721A, ERC721ABurnable, Ownable, ReentrancyGuard {
        ...
        payable
    {
        ...
        // we can skip the arithmetic overflow/underflow checks here to save some gas wasted on those as we don't expect an overflow or underflow
        unchecked {
            do {
                // assign the lenght once so it's not called repetitively on every loop
                uint256 ingredientLength = ingredients[i].length;
                // throw if the minimum ingredients is too low
                require(
        ...
        uint256 costMinusCredit = ingredientCost > credits
            ...
            : 0;
        uint256 baseWithDiscount = discounts[msg.sender]
            ? price - DISCOUNT
            : price;
        ...
        // use _mint as we don't allow contract minting anyways
    ...
    function addToDiscountList(address[] calldata entries) external onlyOwner {
        ...
        unchecked {
            do {
                ...
                if (discounts[entry]) continue;

                discounts[entry] = true;
    ...
    {
        uint256 i;
        // assign the length once so it's not called repetitively on every loop
