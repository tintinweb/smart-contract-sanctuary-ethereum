...
contract TimePieces is Satoshigoat {
	...
	NFT721 constant public plugNft = NFT721(0x2Bb501A0374ff3Af41f2009509E9D6a36D56A6c0);//mainnet
	//testing purposes...
	NFT721 constant public sugoiNft = NFT721(0x8088f4612eaDB9d60D5C8Abf4a9D0FDfC3dF2f1E);//mainnet
		...
		notContract(to)
	{
		...
		uint256 tid = _totalMinted() - 1;//should be current token id
		...
		purchaseArgsOK(to, msg.value, 1)
	{
		...
		if (plugBalance == 0) {
			...
			uint i;
			for (i = 0; i < 2; i++) {
				if (timeNft.ownerOf(_timeNftIds[i]) == to) {
					found = true;
					break;
				}
			}
		...
		//if execution reaches here, `to` either owns a Plug or a TIMEPieces x LOGIK piece
		_safeMint(to, 1);
		uint256 tid = _totalMinted() - 1;//should be current token id
		if (good) _gudOrEvol[tid] = true;
		else _gudOrEvol[tid] = false;
