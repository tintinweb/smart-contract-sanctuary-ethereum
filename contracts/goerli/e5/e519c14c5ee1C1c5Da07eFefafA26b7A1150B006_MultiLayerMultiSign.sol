...
function SetupTrxPool (TrxStr[] memory _Trxpool) public
{
    uint256 PoolSummary;
    require(_Trxpool.length<=10 && CheckKey(msg.sender,Layer1OPKeys)&&TrxPoolStatus<=1&&Layer2OPKeys.length>0,"Setup Transaction Pool Failed");
    //insert fail event

    delete TrxPool;
     for(uint8 i=0; i<_Trxpool.length; i++){
            TrxPool.push(_Trxpool[i]);
            PoolSummary+=_Trxpool[i].TrxAmount;
        }
    ...
    TrxPoolStatus=2;
    TrxPoolHash=keccak256(abi.encode(_Trxpool));
...
function ApproveStage(TrxStr[] memory _Trxpool) public {
    bytes32 _Trxpoolhash;
    require(_Trxpool.length<=10 && CheckKey(msg.sender,Layer2OPKeys) &&TrxPoolStatus>1,"Approve Failed");
