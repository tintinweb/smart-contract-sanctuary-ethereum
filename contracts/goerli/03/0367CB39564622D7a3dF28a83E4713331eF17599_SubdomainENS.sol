...
contract SubdomainENS {
    ...
    function domainCommit(string calldata origin) external payable {
    
        require(ens.owner(domainNode)==address(0),'already minted');
        domainNode = keccak256(abi.encodePacked(TLD_NODE, keccak256(bytes(origin))));
        cm = keccak256(abi.encodePacked(keccak256(bytes(origin)), msg.sender, goerliResolver, msg.sender, domainNode));
        ensr.commit(cm);
