...
contract NFTStore is Ownable, SignerRole, StoreBase {
    ...
    function initialize(string memory _name,string memory _symbol,address owner,address signer) external {
        name = _name;
        symbol = _symbol;
...
contract NFTFactory is Ownable {
    event NewNFTtokenContract(address indexed smartChef, uint256 indexed NFTstoreCount);
    uint256 public NFTdeployCount;
    mapping(uint256 => address)public NFTstoreAddress;

    address public wallet;
    uint256 public feeAmount;

    event SetFeeAmount(address indexed owner, uint256 feeAmount);
    event SetWalletAddres(address indexed owner, address indexed newWallet);

    constructor(uint256 _feeAmount,address _wallet) public {
    ...
    function UpdateFee(uint256 _feeAmount) external onlyOwner {
        feeAmount = _feeAmount;
        emit SetFeeAmount(msg.sender, _feeAmount);
    }

    function UpdateWallet(address _wallet) external onlyOwner {
        wallet = _wallet;
        emit SetWalletAddres(msg.sender, _wallet);
    }


    function deployNFT(string memory _name,string memory _symbol,address signer) external payable {
        require(feeAmount <= msg.value || msg.sender == owner(),"bad fee amount");
        NFTdeployCount++;
        bytes memory bytecode = type(NFTStore).creationCode;
        bytes32 salt = keccak256(abi.encodePacked( _name,_symbol,signer));
        address NFTAddress;
        ...
        NFTStore(NFTAddress).initialize(
           _name,
           _symbol,
           msg.sender,
           signer
        );
        NFTstoreAddress[NFTdeployCount] = NFTAddress;
        require(payable(wallet).send(msg.value),"transaction failed");

        emit NewNFTtokenContract(NFTAddress, NFTdeployCount);
    }
}