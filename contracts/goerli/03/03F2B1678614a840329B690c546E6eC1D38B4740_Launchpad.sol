...
{   
    ...
    function InitArtBlocksProject (
        ...
        uint MintPassID = IMintPass(Params._FactoryMintPass).__InitMintPass(ParamsMintPass); // done
        ParamsSale._ProjectIDMintPass = MintPassID;
        IMarketplace(Params._Marketplace).__StartSale(ParamsSale, ParamsSaleInternal, RootsPriority, RootsAmounts, DiscountAmounts);
        // ILiveMint(Params._LiveMint).__InitLiveMint(ILiveMint.Params(ArtBlocksProjectID, ParamsMintPass._MaxSupply));
    ...
    function InitProject (
        ...
        require(Params._Active, "Launchpad: Not Active");
        ParamsMintPass._ArtBlocksProjectID = type(uint).max;
        ILiveMint(Params._LiveMint).__InitLiveMint(ParamsLiveMint);                     
        ...
        ILaunchpadRegistry(Params._LaunchpadRegistry).__NewProject(ILaunchpadRegistry.Project (
            ...
            true,                             // _Active
            false,                            // _ArtBlocks
            type(uint).max,                   // _ArtBlocksProjectID note: (not used for non AB projects)
            ParamsMintPass._MaxSupply,        // _MaxSupply
            ParamsMintPass._MetadataURI,      // _MetadataMintPass
...
interface ILiveMint
{
    ...
    function __InitLiveMint(Params memory) external;
}
