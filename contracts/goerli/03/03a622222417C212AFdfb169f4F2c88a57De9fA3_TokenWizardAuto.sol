                    ...
                    uint40 mostRecentDueDate;
                        for (
                            ...
                            if (
                                ...
                                mostRecentDueDate = twContract
                                    .financialTerms
                                    .paymentDates[i];
                            }
                        }
                    if(lastPenalizedAt < mostRecentDueDate) {
                        if (twContract.financialTerms.paymentDates[0] > 0) {
                        // this for loop gets the totalAmount that should have been paid already (as of current block.timestamp)
                        uint256 outstandingBalance;
                        uint40 recentDueDate;
...
library PriceConverter {
    ...
    {
        uint256 ethPrice = getPrice(priceFeed);
        uint256 multipliedAmount = amount * 1e10; 
        uint256 xAmountInEth = (multipliedAmount / ethPrice) * 1e18;
        return xAmountInEth;
    }
