...
// File: undefined/IERC721Receiver.sol
...
//██║░╚═╝░██║██║░░██║██║░╚███║██████╔╝╚█████╔╝██╔╝╚██╗  ░░╚██╔╝░░███████╗
//╚═╝░░░░░╚═╝╚═╝░░╚═╝╚═╝░░╚══╝╚═════╝░░╚════╝░╚═╝░░╚═╝  ░░░╚═╝░░░╚══════╝
// Not a MEME! - A Movement
// Twitter: twitter.com/OfficialMandox
// Website: www.officialmandox.com - Mandox.io
// Discord: discord.gg/MANDOX
// Telegram: T.me/Officialmandox

// Discover The Mandox - #MilitiaWeStand 
...
contract MandoxV2Token is Context, IERC20, Ownable {
...
//    using Address for address;
    ...
    uint256 private _tTotal = 50000000000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    address public marketingWallet;

    string private _name = "Mandox";
    string private _symbol = "MANDOX";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 2;
    uint256 private _previousTaxFee = _taxFee;

    uint256 public marketingFeePercent = 45;
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee < 15, "Liquidity fee cannot be more than 15%");
        _liquidityFee = liquidityFee;
    }
   
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > 10000, "Max Tx Amount cannot be less than 10 Thousand");
        _maxTxAmount = maxTxAmount * 10**9;
    }
    
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        require(SwapThresholdAmount > 10000, "Swap Threshold Amount cannot be less than 10 Thousand");
...
contract EvaStake is Ownable, IERC721Receiver, Pausable {
    ...
    // reference to the EVA NFT contract
    EvaTribe eva;
    // reference to the $MANDOX contract for earnings
    ...
    uint256 public rewardPerDay30 = 650000; // Values in $MANDOX tokens for Payout
    uint256 public rewardPerDay60 = 1425000;  
    uint256 public rewardPerDay90 = 3000000;
    ...
    constructor(address _eva, address payable _mandox, address _rewardingWallet) {
        eva = EvaTribe(_eva);
    ...
    function addManyToRegister(uint256[] calldata tokenIds, uint16 tier) external {
        for (uint i = 0; i < tokenIds.length; i++) {
            require(eva.ownerOf(tokenIds[i]) == _msgSender(), "MANDOX: IS NOT OWNER");
            eva.transferFrom(_msgSender(), address(this), tokenIds[i]);
    ...
    */
    function _claimNftFromRegister(uint256 tokenId, bool unstake) internal returns (uint256 rewards) {
        ...
        if (stake.tier == 30) {
            rewards = uint256((block.timestamp - stake.stakeTime) / 1 minutes * ( rewardPerDay30 * (10 ** 9)));
        ...
        if (unstake == true) {
            eva.safeTransferFrom(address(this), _msgSender(), tokenId); // send back NFT
    ...
    */
    function rescue(uint256[] calldata tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            require(stake.owner == _msgSender(), "MANDOX: SHOULD BE OWNER");
            eva.safeTransferFrom(address(this), _msgSender(), tokenId); // send back eva
            delete register[tokenId];
