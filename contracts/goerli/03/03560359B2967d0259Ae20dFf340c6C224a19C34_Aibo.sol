...
error MaxLightMints();
//@0xSimon
contract Aibo is ERC721AQueryable, ERC2981,Ownable,DefaultOperatorFilterer {
    ...
    uint public publicRafflePrice = .0001 ether;
    string public baseURI;
    string public darkNotRevealedUri = "ipfs://";
    string public lightNotRevealedUri = "ipfs://QmexJhweLAquZANgwhR4d3EpKbSciKUwfgNieSwjYazerU";
    ...
    function teamMint(address[] calldata accounts,Alleigance[] calldata alleigances) external onlyOwner {
        ...
        for(uint i; i<accounts.length;){
            ...
            assembly{
                //++lightMintCounter
                sstore(lightMintCounter.slot,add(sload(lightMintCounter.slot),1))
            }
        }
        else{
    ...
    /*///////////////////////////////////////////////////////////////
                          MINT FUNCTIONS
    //////////////////////////////////////////////////////////////*/
    // function mintSupplyCheck(Alleigance alleigance) internal {
    ...
    //     }
    // }
    function whitelistMint(bytes memory signature,Alleigance alleigance) external payable {
        if(saleStatus != SaleStatus.WHITELIST) revert SaleNotStarted();
        uint nextTokenId = _nextTokenId();
        if(nextTokenId + 1 > MAX_WHITELIST_SUPPLY) revert SoldOut();
        ...
        if(_numberMinted(_msgSender()) + 1 > maxTotalMints) revert MaxMints();
        if(msg.value < presalePrice ) revert Underpriced();
    ...
    function publicRaffle( bytes memory signature,Alleigance alleigance) external payable {
        if(saleStatus != SaleStatus.PUBLIC_RAFFLE) revert SaleNotStarted();
        uint nextTokenId = _nextTokenId();

        if(nextTokenId + 1 > MAX_PUBLIC_RAFFLE_SUPPLY) revert SoldOut();
        bytes32 hash = keccak256(abi.encodePacked("ABUP",_msgSender()));
        if(hash.toEthSignedMessageHash().recover(signature)!=signer) revert NotWhitelisted();
        if(_numberMinted(_msgSender()) + 1 > maxTotalMints) revert MaxMints();
        if(msg.value < publicRafflePrice) revert Underpriced();
          if(alleigance == Alleigance.LIGHT) {
            uint numLightMints = getNumLightMints();
            //Safety Check To Make Sure No Side Can Have More Than 4500
            if(numLightMints + 1 > MAX_PER_ALLEIGANCE) revert MaxLightMints();
    ...
    function publicMint(Alleigance alleigance) external payable {
        if(saleStatus != SaleStatus.PUBLIC) revert SaleNotStarted();
        uint nextTokenId = _nextTokenId();
        if(nextTokenId + 1 > MAX_SUPPLY) revert SoldOut();
        
        uint numMinted = _numberMinted(_msgSender());
        if(numMinted + 1 > maxTotalMints) revert MaxMints();
        //Impossible To Overflow Since Supply < type(uint64).max
        if(msg.value < publicPrice) revert Underpriced();
          if(alleigance == Alleigance.LIGHT) {
            uint numLightMints = getNumLightMints();
            //Safety Check To Make Sure No Side Can Have More Than 4500
            if(numLightMints + 1 > MAX_PER_ALLEIGANCE) revert MaxLightMints();
    ...
    function getNumDarkMints() public view returns(uint){
        return totalSupply() - lightMintCounter;
    }
    ...
    function getAlleigance(uint tokenId) external view returns(Alleigance) {
    ...
    function getBatchAlleigances(uint[] calldata tokenIds) external view returns(Alleigance[] memory) {
        ...
        for(uint i; i<tokenIds.length;++i){
            ...
            alleigances[i] = tokenIdToAlleigance[tokenId];
        }
        return alleigances;
    }
    /*///////////////////////////////////////////////////////////////
        ...
        returns (string memory)
    {
        if (revealed == false) {
            ...
            if(tokenIdToAlleigance[tokenId] == Alleigance.DARK){
