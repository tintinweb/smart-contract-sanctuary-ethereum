...
contract Sale is ReentrancyGuard, Ownable {
    ...
    constructor(address _oracle, address _usdt,address _SaleTokenAdress,uint256 _minTokenToBuy) {
        ...
        onlyOwner
    {
      ...
      function setExcludeFromMinFee(address _user, bool _status) external onlyOwner {
        isExcludeMinFee[_user] = _status;
    }
   // @dev enabel Claim amount
    function enableClaim(uint256 _id,bool _status) public
      checkPresaleId(_id) 
      onlyOwner
      {
    ...
    modifier checkSaleState(uint256 _id, uint256 amount) {
        require(
            block.timestamp >= presale[_id].startTime && !(presale[_id].isStartSale),
        ...
        returns (bool)
    {
     ...
     function removeContractTokens(address _token, uint256 amount) external onlyOwner {
    ...
    function claimMultiple(uint256[] calldata _id)
        external
        returns (bool)
    {
