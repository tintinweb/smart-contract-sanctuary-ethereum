    ...
    UUPSUpgradeable
{
    ...
    mapping(bytes32 => mapping(bytes32 => bool)) public cars;
    ...
    function initialize(
        ...
        numbers = [
            ...
            keccak256(abi.encodePacked("6")),
            keccak256(abi.encodePacked("7")),
            keccak256(abi.encodePacked("8")),
            keccak256(abi.encodePacked("9")),
            keccak256(abi.encodePacked("10")),
            keccak256(abi.encodePacked("11"))
        ];
    ...
    modifier carAvailable(string memory color, string memory number) {
        bytes32 carColor = keccak256(abi.encodePacked(color));
        bytes32 carNumber = keccak256(abi.encodePacked(number));

        require(has(colors, carColor), "AvicarERC721: invalid color supplied");

        require(
            has(numbers, carNumber),
            "AvicarERC721: invalid number supplied"
        );

        require(
            !cars[carColor][carNumber],
            "AvicarERC721: car has already been claimed"
        );

        cars[carColor][carNumber] = true;
    ...
    modifier publicMinter(
        ...
        require(
            ...
            "AvicarERC721: signature validation failed"
        );

        require(
            expires > block.timestamp,
            "AvicarERC721: validation has expired"
        );

        _;
    }

    function has(bytes32[] memory arr, bytes32 searchFor)
        internal
        returns (bool)
    {
    ...
    function adminSetOwner(address admin) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(this.owner() == address(0), "AvicarERC721: Owner already set");
        ...
        pure
    {
        ...
        require(
            total == 10000,
            "AvicarERC721: Beneficiary allocation must equal 100%"
        );
    }
    ...
    function mint(Mint memory context)
        public
        virtual
        carAvailable(context.attributes.color, context.attributes.number)
    {
        require(
            hasRole(MINTER_ROLE, _msgSender()),
            "AvicarERC721: must have minter role to mint"
        );
    ...
    function doMint(
        ...
        require(
            ...
            "AvicarERC721: TokenUri can not be empty"
        );
    ...
    function pause() external {
        require(
            ...
            "AvicarERC721: must have pauser role to pause"
        );
        _pause();
    }
    ...
    function unpause() external {
        require(
            hasRole(PAUSER_ROLE, _msgSender()),
            "AvicarERC721: must have pauser role to unpause"
        );
        _unpause();
    }
