...
/** Something to make this a little easier, Swap direction should always be swapping in,
because we won't have a bridgeable asset for the launching token
*/
...
/** Questions:
1. does this need to be payable?
2. What is with the return type used in NftSwapProxy.sol and also Vault.swap where a variable is specified? My compiler was complaining until I removed that.
*/
    function singleSwapWithLbp(
        SingleSwap memory singleSwap,
        FundManagement memory funds,
        uint256 limit,
        uint256 deadline,
        ...
        require(lbpContractAddress != address(0), "lbpContractAddress must be defined");
        require(singleSwap.kind == SwapKind.GIVEN_IN, "can only swap into pool with bridged assets");
        ...
        uint256 swappedAmount = Vault(vaultAddress).swap(singleSwap, funds, limit, deadline);
        TransferHelper.safeApprove(singleSwap.assetOut, lbpContractAddress, swappedAmount);
    ...
    ) external returns (bytes memory) { // return type warning, but I think we need to ignore
...
//        address _conxLbpSwapSourceContract, // oh this might be the same as originSender
//        uint256 _relayerFee,
//        SingleSwap memory singleSwap,
//        FundManagement memory funds,
//        uint256 limit,
//        uint256 deadline,
//        address lbpContractAddress
//        ) = abi.decode(_callData, (address, address, uint256, SingleSwap, FundManagement, uint256, uint256, address));
//
//        // The nested xcall
//        // Make sure to use received amount which will likely be slightly less than sent amount because of relayFees
//        singleSwap.amount = _amount;
//        // swappedAmount is the amount of tokens the bridged was swapped for
//        uint256 swappedAmount = singleSwapWithLbp(singleSwap, funds, limit, deadline, lbpContractAddress);
//
//        // send Funds to End User
////        transferSwapOutput(_swapRecipient, swappedAmount, swappedAsset)
//        // TODO: maybe we can skip this step if the Fund Object specifies the swapRecipient address?
//        TransferHelper.safeTransfer(_swapRecipient, _swapRecipient, swappedAmount);
