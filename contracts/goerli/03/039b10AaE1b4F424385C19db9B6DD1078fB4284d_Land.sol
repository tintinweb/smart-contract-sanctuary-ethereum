...
contract Land is Ownable, ERC721ABurnable, Pausable {
    ...
    function mintTransfer(address to, uint256 amount_req)
        ...
        whenNotPaused
    {
        skyFallsPyramidAddress = Index.getAddress("SkyFalls");
        require(
    ...
    function extBurn(uint256 tokenID) public {
        require(
    ...
    function farmConditionVerifier(
        ...
        FarmContract = Farmer(FarmAddress);
        require(msg.sender == FarmAddress, "MUST BE CALLED FROM FARM CONTRACT");
        uint256 minTerm = nums[0];
        uint256 maxTerm = nums[1];
        uint256 destablingFee = nums[2];
        uint256 ownerFee = nums[3];
        uint256 stableID = nums[4];
        uint256 idsLen = ids.length;
        if (idsLen != 1) {
            for (uint256 i = 0; i < idsLen; i++) {
                if (i < idsLen - 1) {
                    uint256 _id = ids[i];
                    uint256 _id1 = ids[i + 1];
                    require(
                        keccak256(abi.encodePacked(landTypes[_id])) ==
                            keccak256(abi.encodePacked(landTypes[_id1])),
                        "LAND TYPES MUST BE THE SAME"
                    );
                }
                require(
                    ownerOf(ids[i]) == to,
            ...
            require(ownerOf(ids[0]) == to, "MUST OWN ALL SELECTED LAND TOKENS");
        }
        require(
            ids.length >= FarmContract.minFarmSizeByType(landTypes[ids[0]]),
            "FARM MUST BE >= REQUIRED SIZE FOR YOUR LAND TYPE"
        );
        require(
            ids.length <= FarmContract.maxFarmSize(),
            "FARM MUST BE LESS THAN or = to MAX SIZE"
        );
        require(
            minTerm >= FarmContract.minStableTerm(),
            "MIN TERM MUST BE >= GLOBAL MIN STABLE TERM"
        );
        require(
            maxTerm <= FarmContract.maxStableTerm(),
            "MAX TERM MUST BE <= GLOBAL MAX STABLE TERM"
        );
        require(minTerm <= maxTerm, "MIN TERM MUST BE <= MAX TERM");
        require(
            destablingFee >= FarmContract.minDestablingFee(),
            "DESTABLING FEE MUST BE HIGHER THAN THE MINIMUM"
        );
        if (ids.length > 10) {
            require(
                ids.length % 10 == 0,
                "IF Farm > 10 MUST BE IN MULTIPLES OF 10"
            );
        }
        require(
            ownerFee <= FarmContract.maxOwnerFee(),
            "OWNER FEE MUST BE LESS THAN THE MAXIMUM"
        );
        if (stableID == 0) {
            require(FreeStable[ids[0]], "LAND MUST HAVE STABLE FUNCTIONALITY");
            return false;
        } else {
            Stabler StableContract = Stabler(Index.getAddress("Stable"));
            require(
                StableContract.ownerOf(stableID) == to,
