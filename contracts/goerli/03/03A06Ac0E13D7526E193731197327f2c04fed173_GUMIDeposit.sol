...
contract GUMIDeposit is Ownable, Pausable {
    ...
    /**
     * @dev Emitted when `value` tokens are burned from one account (`from`) with
     * Yay user id (`userId`).
     *
     * Note that `value` may be zero.
     */
    event Deposit(address indexed from, uint256 value, string userId);
    ...
    function transferSigner(address newSigner) public virtual onlyOwner {
        require(newSigner != address(0), "GUMIDeposit: new signer is the zero address");
    ...
    function hash(uint256 amount, string calldata userId, uint expiryDateTime) public pure returns (bytes32) {
        bytes32 _hash = keccak256(abi.encodePacked(amount, userId, expiryDateTime));
     ...
     * total supply.
     *
     * Emits a {Deposit} event with Yay user id `userId`.
     *
     * Requirements:
     *
     ...
     */
    function deposit(uint256 amount, string calldata userId, uint expiryDateTime, bytes calldata signature) public {
        require(block.timestamp < expiryDateTime, "GUMIDeposit: the signature is already expired");
        require(isSignatureUsed[signature] == false, "GUMIDeposit: the signature already redeemed");
        require(SignatureChecker.isValidSignatureNow(_signer, hash(amount, userId, expiryDateTime), signature), "GUMIDeposit: the signature is invalid");

        isSignatureUsed[signature] = true;
        gumiContract.burnFrom(_msgSender(), amount);
        emit Deposit(_msgSender(), amount, userId);
    }
