...
contract FujiToken is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, ERC721Burnable, VRFConsumerBaseV2, ConfirmedOwner {
    // Counters.counter型に対してCountersを使用することを宣言
    ...
    struct RequestStatus {
        ...
        uint256[] randomWords; // ここに変数がリスト形式で格納される
    }
    ...
    function safeMint(address to) public onlyOwner {
        ...
        string memory uri = _getRamdomTokenUri(tokenId);
    ...
    function _getImage(uint colorNum1, uint colorNum2, uint colorNum3, uint colorNum4) internal view returns(string memory) {
    ...
    function _getRamdomTokenUri(uint256 tokenId) internal view returns(string memory) {

        uint256 colorNum1 = s_requests[lastRequestId].randomWords[0] % colors.length;
        uint256 colorNum2 = s_requests[lastRequestId].randomWords[1] % colors.length;
        uint256 colorNum3 = s_requests[lastRequestId].randomWords[2] % colors.length;
        uint256 colorNum4 = s_requests[lastRequestId].randomWords[3] % colors.length;
        ...
        bytes memory metaData = abi.encodePacked(
            '{"name": "',
            'Colorful Cordinate Squere # ',
            Strings.toString(tokenId),
            '", "description": "Ramdomly colored the Squere.",',
            '"image":"data:image/svg+xml;base64,',
            Base64.encode(bytes(_getImage(colorNum1, colorNum2, colorNum3, colorNum4))),
