...
contract CabinetOfCuriosities is ERC1155, Ownable {
    ...
    mapping(uint256 => address) tokenIndexToAddress;

    address public mirrorAddress = address(0);
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < _currentTokenID; ++i) {
            if (operator == tokenIndexToAddress[i]) {
                return true;
            }
        }
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            if (
                tokenIndexToAddress[i] != address(0) &&
                tokenIndexToAddress[i] != to &&
                super.balanceOf(to, i) == 0
            ) {
                GameBalance(tokenIndexToAddress[i]).updateOwners(to);
            }
        }
    }

    function setAddressForTokenIndex(address _address, uint256 _index)
        public
        onlyOwner
    {
        tokenIndexToAddress[_index] = _address;
    }
}
...
/**
 * @dev Implementation based on OpenZeppelin, _balances access changed to internal.
