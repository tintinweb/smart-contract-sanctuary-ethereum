...
contract PermissionedPortfolioFactory is BasePortfolioFactory {
    address defaultControllerAdmin;

    function setDefaultControllerAdmin(address _defaultControllerAdmin) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _defaultControllerAdmin != address(0),
            "PermissionedPortfolioFactory: Default controller admin cannot be address zero"
        );
        require(
            _defaultControllerAdmin != defaultControllerAdmin,
            "PermissionedPortfolioFactory: Default controller admin cannot be set to its current value"
        );
        defaultControllerAdmin = _defaultControllerAdmin;
    }
