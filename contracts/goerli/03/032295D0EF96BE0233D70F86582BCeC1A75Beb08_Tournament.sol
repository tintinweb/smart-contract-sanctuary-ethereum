...
contract Tournament is Initializable {
    ...
    function initialize(
        ...
        ISoldierANT(contracts.contractSoldier).initialize(_epochDuration);
    ...
    function enterTournament() public payable {
        require(msg.value == entranceFee, "Pay to enter.");
        bool access = false;
        for (uint256 i = 0; i < participants.length; i++) {
            access = participants[i] == msg.sender ? true : false;
        }
...
interface ISoldierANT is IERC721 {
    ...
    function battle(uint256 attackerSoldierCount, uint256 targetSoldierCount)
        external
...
interface IQueenANT is IERC721 {
    ...
    function layingEggs(address _user, uint256 _index)
        external
        view
        returns (uint256 deservedEggs);
    ...
    function idToFertility(uint256) external view returns (bool);
