...
contract Assets is Ownable {
    ...
    function getPoints(address addr) public view returns (uint256 points){
        ...
        points += items[addr].cashBag*9;
        points += items[addr].cash*8;
        points += items[addr].chipStacks*7;
        points += items[addr].chips*6;
        points += items[addr].gangLeaders*5;
        points += items[addr].streetThugs*4;
        points += items[addr].superCars*3;
        points += items[addr].sportsCars*2;
