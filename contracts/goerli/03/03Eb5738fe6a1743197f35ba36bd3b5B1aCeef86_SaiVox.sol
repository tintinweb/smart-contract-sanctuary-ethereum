...
contract SaiVox is DSThing {
    ...
    function mold(bytes32 param, uint256 val) public note auth {
...
contract SaiTub is DSThing, SaiTubEvents {
    ...
    function era() public constant returns (uint256) {
        return block.timestamp;
    }
    ...
    function per() public view returns (uint256 ray) {
        return skr.totalSupply() == 0 ? RAY : rdiv(pie(), skr.totalSupply());
    }

    // Join price (gem per skr)
    function ask(uint256 wad) public view returns (uint256) {
    ...
    function exit(uint256 wad) public note {
        require(!off || out);
    ...
    function tag() public view returns (uint256 wad) {
        return off ? fit : wmul(per(), uint256(pip.read()));
    }
    ...
    function lock(bytes32 cup, uint256 wad) public note {
        require(!off);
    ...
    function free(bytes32 cup, uint256 wad) public note {
        require(msg.sender == cups[cup].lad);
    ...
    function draw(bytes32 cup, uint256 wad) public note {
        require(!off);
        require(msg.sender == cups[cup].lad);
    ...
    function wipe(bytes32 cup, uint256 wad) public note {
        require(!off);
        ...
        var (val, ok) = pep.peek();
        if (ok && val != 0) gov.move(msg.sender, pit, wdiv(owe, uint256(val)));
    }
    ...
    function cage(uint256 fit_, uint256 jam) public note auth {
        require(!off && fit_ != 0);
        off = true;
        axe = RAY;
        gap = WAD;
