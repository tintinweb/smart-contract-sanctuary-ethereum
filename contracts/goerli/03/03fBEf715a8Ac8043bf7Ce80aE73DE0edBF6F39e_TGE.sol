    ...
    ITGE
{
        ...
        whenPoolNotPaused
    {
        ...
        uint256 lockedAmount = (amount * info.vestingPercent + 99) / 100;
        if (amount - lockedAmount > 0) {
            _token.mint(msg.sender, amount - lockedAmount);
        }
        _token.mint(address(this), lockedAmount);
        lockedBalanceOf[msg.sender] += lockedAmount;
        _totalLocked += lockedAmount;
        _token.increaseTotalTGELockedTokens(lockedAmount);
        ...
        whenPoolNotPaused
    {
        ...
        uint256 vesting = lockedBalanceOf[msg.sender];
        ...
        if (vesting > 0) {
            ...
            _totalLocked -= vesting;
            refundAmount += vesting;
            token.decreaseTotalTGELockedTokens(vesting);
    ...
    function getTotalLockedValue() public view returns (uint256) {
        return (_totalLocked * info.price) / 10**18;
    }
