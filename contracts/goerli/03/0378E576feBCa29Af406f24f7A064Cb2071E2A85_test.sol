    ...
    ERC721AOpensea
{
    ...
    /**
     * @notice set the minimum clearing price for bidding
     * @dev set this price in wei, not eth!
     * @param minimumClearingPriceInWei new price, set in wei
     */
    function setMinimumClearingPrice(uint256 minimumClearingPriceInWei)
        external
        onlyRole(SUPPORT_ROLE)
    {
        _setMinimumClearingPrice(minimumClearingPriceInWei);
    }
    ...
    function _sendTokensAndRefund(ReceiverData calldata receiver) internal {
        ...
        uint256 refundValue;
        unchecked {
            refundValue = userContribution - (price_ * numberOfTokens);
        }
     ...
     * @param approvalCheck check to see whether msg.sender is approved to burn
     *   the token
     */
...
contract SteppedDutchAuction {
    ...
    uint256 public minimumClearingPrice;
    ...
    /**
     * @dev sets a minimum clearing price for the auction.
     * @param price the minimum price in wei
     */
    function _setMinimumClearingPrice(uint256 price) internal virtual {
        minimumClearingPrice = price;
    }
     ...
     *  the start price. Returns the calculated auction price or minimum
     *  clearing price, whichever is greatest
     */
    function getAuctionPrice() public view returns (uint256) {
        ...
        if (elapsed >= duration) {
            return
                finalPrice > minimumClearingPrice
                    ? finalPrice
        ...
        return
            auctionPrice > minimumClearingPrice
                ? auctionPrice
                : minimumClearingPrice;
    }
}
