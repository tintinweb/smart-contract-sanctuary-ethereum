    ...
    UUPSUpgradeable
{
    ...
    function isVaultDeprecated(uint256 index)
        external
        view
        override
        returns (bool)
    ...
    function getBatchBudgetPerDeposit()
        external
        view
        override
        returns (uint256)
    {
        return _totalFees / _vaultDepositsCount;
    }
        ...
        returns (bool)
    {
        ...
        if (!_isVaultDeprecated[vaultIndex]) {
            return false;
        }
        ...
        returns (uint256 balance)
    {
        ...
        for (uint256 i = 1; i < subvaultNfts.length; i++) {
            ...
            balance += _token.balanceOf(address(marginEngine));
        }
    }
    ...
    function submitBatchForFee(uint256 index, uint256 batchSize)
        external
        override
    {
        ...
        _token.safeTransfer(msg.sender, allocatedFee);
    }
    ...
    function triggerAutoRollover(uint256 vaultIndex) external override {
        require(_isVaultDeprecated[vaultIndex], "Vault still active");
...
interface IMellowMultiVaultRouter {
    ...
    /// @notice Returns fee accumulated per vault deposit
    /// @dev This is not equal to _fee
    function getBatchBudgetPerDeposit() external view returns (uint256);
    ...
    /// @notice Push the batched funds to Mellow and transfer deserving fee to sender
    function submitBatchForFee(uint256 index, uint256 batchSize) external;
...
contract MellowMultiVaultRouterStorage is MellowMultiVaultRouterStorageV1 {
    ...
    uint256[58] private __gap; // total storage = 100 slots, including structs
}