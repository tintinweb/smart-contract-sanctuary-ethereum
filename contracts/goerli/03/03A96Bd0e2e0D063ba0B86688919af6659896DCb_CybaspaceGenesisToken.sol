...
contract CybaspaceGenesisToken is ERC721, AccessControl, CustomErrors, ICybaspaceGenesisToken {
    ...
    function mint(
        address _to,
        uint256 _tokenId
    ) external onlyRole(MINTER_ROLE) whenMintingIsNotFrozen {
        //if (mintingFrozen) revert MintingIsFrozen();
        _mint(_to, _tokenId);
    }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '", "description": "',
                        'Our Cybaspace Starter Pack Founders Edition is the first of a series of tokens that will be ',
                        'at the core of association with Cybaspace. The starter pack will include a CYBAsuit, ',
                        'a weapon, an artifact and an upgrade. The founders edition will offer unique and extremely ',
                        'rare assets, early alpha access, and long term opportunities - unlocking massive rewards for ',
                        'those taking full advantage of our collection."',
                        ', "image": "',
                        _baseImageURI,
                        '"',
     ...
     * MintingIsFrozen
     */
    function freezeMinting() external onlyRole(DEFAULT_ADMIN_ROLE) {
        mintingFrozen = true;
    }
