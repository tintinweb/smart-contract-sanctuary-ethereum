...
contract AsteroidTokenBridge is Ownable, Manageable {
  ...
  // l2BridgeSelector
  // ex: 421276901562941159060571287445547166253423171725132730820182335590594297755 Asteroid_bridgeFromL1
  ...
  // setters
  function setL1TokenContract(address _l1TokenContract) external onlyManagers {
    l1TokenContract = IAsteroidToken(_l1TokenContract);
  }
  ...
  function getL1TokenContract() public view returns (address) {
    return address(l1TokenContract);
  }
  ...
  function bridgeToStarknet(uint256[] calldata tokenIds, uint256 l2AccountAddress) external {
    ...
    // check ownership and burn
    for (uint256 i = 0; i < tokenIds.length; i++) {
      address owner = l1TokenContract.ownerOf(tokenIds[i]);
      require(owner == msg.sender, 'Bridge/sender-not-token-owner');
      l1TokenContract.burn(tokenIds[i]);

      payload[2 + i] = tokenIds[i];

      emit BridgeToStarknet(address(l1TokenContract), msg.sender, l2AccountAddress, tokenIds[i]);
    }
  ...
  function bridgeFromStarknet(uint256[] calldata tokenIds, uint256 l2AccountAddress) external {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      l1TokenContract.mint(msg.sender, tokenIds[i]);
      payload[4 + i] = tokenIds[i];

      emit BridgeFromStarknet(l2AccountAddress, address(l1TokenContract), msg.sender, tokenIds[i]);
    }
