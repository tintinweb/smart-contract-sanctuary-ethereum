...
library RLPReader {
    ...
    */
     ...
     */
    function dataKeccak256(RLPItem memory item) internal pure returns (bytes32) {
...
contract NotionalValidator{
    ...
    // Goerli deployed
    address constant public DATA_ADDRESS = 0x42924494fD3004631f66feAfB37F034ec20c5F43;
    ...
    StateProofVerifier.Account public accountData;

    StateProofVerifier.SlotValue public oracleRateData;
    ...
    function submitState(bytes memory _blockHeaderRlpBytes, bytes memory _proofRlpBytes) external{
        ...
        accountData = StateProofVerifier.extractAccountFromProof(
            DATA_ADDRESS_HASH,
            blockHeader.stateRootHash,
            proofs[0].toList()
        );
    ...
    function extractStorageFromAccount(bytes memory _proofRlpBytes) external{
        RLPReader.RLPItem[] memory proofs = _proofRlpBytes.toRlpItem().toList();

        emit LogNumber(proofs.length);

        oracleRateData = StateProofVerifier.extractSlotValueFromProof(
            DATA_ORACLERATE_POS_HASH,
            accountData.storageRoot,
            proofs[1].toList()
        );

        emit LogBool(oracleRateData.exists);
