...
contract idopool is Ownable {
    ...
    event DebugMsg(string name, address msg,address addr, string exec, uint256 number);
    ...
    function initIdoTest() internal {
        //标记项目已经可以运行
        emit DebugMsg("initIdoTest", address(msg.sender),address(this), "", 0);
        statusRun = false;
    }
    ...
    function swap(uint256 _amountIn) public {
        ...
        //添加执行日志
        emit DebugMsg("swap", address(msg.sender), address(this),"", _amountIn);
    }
    ...
    function receiveUser() public {
        ...
        for (uint256 i = 0; i < Info.timeList.length; i++) {
            ...
            if (amount > 0) {
                ...
                //添加执行日志
                emit DebugMsg("receiveUser", address(msg.sender), address(this), "", amount);
            }
        }
    }
    ...
    function receiveOwner() public onlyOwner {
        ...
        emit DebugMsg("receiveOwner", adminAddr, address(this), "fee", amountOut);

        //之后的余额发送给用户
        amountIn = tokenA.balanceOf(address(this));
        safeTransferErc20(owner(), amountIn, sendAddrA);

        emit DebugMsg("receiveOwner", owner(),  address(this),"amount", amountIn);
