...
contract colorfullyxyzc is ERC721A, ERC2981, Ownable, ReentrancyGuard, PaymentSplitter {
    ...
    constructor(address[] memory _payees, uint256[] memory _shares) ERC721A('colorfullyxyzc', 'colorfullyxyzc') PaymentSplitter(_payees, _shares) {
    payees = _payees;
    }

    function PublicMint(uint amount) external payable {
        require(totalSupply() < maxSupply, "sold out");
        require(salesStarted, "PublicMint is not open");
    ...
    function PreSale(bytes32[] calldata _merkleProof, uint256 _amount) external payable {
        require(salesStarted, "PreSale period is not open");
        require(_amount > 0, "invalid amount");
        require(totalSupply() < maxSupply, "sold out");
        require(msg.value >= _amount * mintPrice, "invalid mint price");
        require(_amount <= maxMintPerTx, "max tokens per transaction reached");
