...
contract WarmUpsNFT is IERC721, Ownable, ERC721 {
  ...
  require(minters[_msgSender()] <= MAX_MINT_PER_WALLET + amount, "WarmUpsNFT: cannot exceed max mint amount");
  ...
  function teamMint(uint8 amount, address recipient) public onlyOwner {
    ...
    /// @notice checks amount already minted
     require(minters[recipient] <= MAX_MINT_PER_WALLET + amount, "WarmUpsNFT: already minted max amount");
