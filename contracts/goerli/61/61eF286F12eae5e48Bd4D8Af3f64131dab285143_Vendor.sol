...
contract Vendor is Ownable {
  ...
  event SellTokens(address, uint256, address);
  ...
  function buyTokens() external payable {
    ...
    require(yourToken.balanceOf(address(this)) >= amountOfTokens);
  ...
  //   require(totalBalance > 0, 'Vendor no have founds');
  //   (bool success, ) = msg.sender.call{value: totalBalance}('');
  //   require(success, 'RIP; withdrawal failed :( ');
  ...
  function sellTokens(uint256 _amount) public {
    ...
    require(success, 'Transfer failed');
    (success, ) = msg.sender.call{value: amountOfTokens}('');
    require(success, 'Payment failed');
    emit SellTokens(msg.sender, amountOfTokens, address(this));
  }

  function getContractBalance() external onlyOwner returns (uint256 contractBalance) {
