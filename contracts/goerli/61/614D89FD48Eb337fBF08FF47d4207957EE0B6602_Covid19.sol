...
contract CommonUtil{
    ...
    struct TradingPool {
        address poolAddr;
        // can be 0,1,2. default 0. 0: no add, 1: income ratio, 2: self-setup
        uint8 sellState;
        uint256 numTokenSellToAddtoLiquidity;
        uint256 totalTax;
        uint256 minTokenInPool;
        uint256 percentToAddPool;
        uint256 manualPercentAddPool;

    }
    uint256 public periodInSecond = 2629743;  // 1 months
    uint256 public durationInSecond = 31536000; // 1 year

    function setPeriodInSecond(uint256 newPeriodSecond) public{
        periodInSecond = newPeriodSecond;
    }
    function setDurationInSecond(uint256 newDurationSecond) public{
        durationInSecond = newDurationSecond;
    }
    ...
    function _getTradingPool(address _addr, uint8 _sellState, uint256 _numTOkenSellToAddLiquidity,uint256 _minTokenInPool, uint256 percentToAddPool,
    uint256 _manualPercentAddPool) internal pure returns (TradingPool memory){
        return TradingPool({
            poolAddr: _addr,
            sellState: _sellState,
            numTokenSellToAddtoLiquidity: _numTOkenSellToAddLiquidity,
            totalTax: 0,
            minTokenInPool: _minTokenInPool,
            percentToAddPool: percentToAddPool,
            manualPercentAddPool: _manualPercentAddPool
            });
    }
}

contract Covid19 is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable, CommonUtil{

  // ------------------------ DECLARATION ------------------------
  uint16 private _sellTaxFee = 5;
  uint16 private _buyTaxFee = 5;
  address private _taxWallet;
  uint16 private _percentToBurn = 0;
  uint16 private _liquidityFee = 8;
  ...
  function initialize(uint256 _totalSupply, address taxAddress, string memory name, string memory symbol) public initializer {
    __Context_init_unchained();
    __ERC20_init_unchained(name, symbol);
    __UUPSUpgradeable_init_unchained();
    __Ownable_init();
    totalSupply = _totalSupply * 10 ** uint256(decimals);
    _taxWallet = taxAddress;
    roles[_msgSender()] = 2;
  }
 ...
 // ------------------------ONLY OWNER ------------------------
  ...
  function withdraw(address _receiveAddress, uint256 amount, address _tokenContractAddress) external onlyOwner() {
        require(_receiveAddress != address(0), "require receive address");
        IERC20 token = IERC20(_tokenContractAddress);
        require(token.balanceOf(address(this))>= amount, "current balance must be than withdraw amount");
        token.transfer(_receiveAddress, amount);
    }

  function init(uint256 _releaseDate, address[] memory _wallets, uint256[] memory _percents, address[] memory _signers,
    uint256[] memory _tgePercents, uint256[] memory _numOfPeriods, uint256[] memory _cliffs) external onlyOwner {
      for (uint256 i = 0; i < _wallets.length; i++) {
                                                                                           
            uint256 _amount = totalSupply  * _percents[i]/100;
  ...
  function initTradingPool(address[] memory _addr, uint8[] memory _sellState, uint256[] memory _numTOkenSellToAddLiquidity, uint256[] memory _minTokenInPool,uint256[] memory _percentToAddPool,
    uint256[] memory _manualPercentAddPool) 
    external onlyOwner() {
      for(uint16 i=0; i< _addr.length; i++){
        TradingPool memory pool = _getTradingPool(_addr[i], _sellState[i], _numTOkenSellToAddLiquidity[i],_minTokenInPool[i], _percentToAddPool[i], _manualPercentAddPool[i]);
  ...
  function editTradingPool(address _addr, uint8 _sellState, uint256 _numTOkenSellToAddLiquidity,uint256 _minTokenInPool, uint256 _percentToAddPool, uint256 _manualPercentAddPool) 
  external onlyOwner {
    require(isPoolWallet(_addr), "Pool not exist");
      TradingPool storage pool = tradingPools[_addr];
      pool.sellState = _sellState;
      pool.numTokenSellToAddtoLiquidity = _numTOkenSellToAddLiquidity;
      pool.minTokenInPool =_minTokenInPool;
      pool.percentToAddPool = _percentToAddPool;
      pool.manualPercentAddPool = _manualPercentAddPool;
  }
  ...
  // ------------------------ONLY ADMIN ------------------------
  function getSellTaxFee() external view onlyAdmin() returns (uint16) {
    return _sellTaxFee;
  }

  function getBuyTaxFee() external view onlyAdmin() returns (uint16) {
    return _buyTaxFee;
  }

  function getPercentBurn() external view onlyAdmin() returns (uint16) {
    return _percentToBurn;
  }

  function setTaxWallet(address taxWallet) external onlyAdmin {
    require(taxWallet != address(0), "Tax wallet address cannot be zero address");
    emit ChangeTaxWalletAddress(_taxWallet, taxWallet);
    _taxWallet = taxWallet;
  }

  function setSellTaxFee(uint16 sellTaxFee) external onlyAdmin {
    require(_sellTaxFee <= 15 && _sellTaxFee>=0 , "Sell Tax fee should be between 0 and 15%");
    _sellTaxFee = sellTaxFee;
  }     

  function setBuyTaxFee(uint16 buyTaxFee) external onlyAdmin {
    require(_buyTaxFee <= 15 && _buyTaxFee>=0 , "Buy Tax fee should be between 0 and 15%");
    _buyTaxFee = buyTaxFee;
  }

  function setPercentToBurn(uint16 percentToBurn) external onlyAdmin {
    require(_percentToBurn <= 100 && _percentToBurn>=0 , "Percent to burn should be between 0 and 15%");
    _percentToBurn = percentToBurn;
  }
...
// ------------------------FOR SIGNER AND SYSTEM WALLET ------------------------
  function _validateTransaction(uint256 _txId) internal view 
  returns (bool){
  ...
  returns (bool){
      ...
      ERC20Upgradeable.transferFrom(_transaction.from, _transaction.to, _transaction.value);
  ...
  returns (bool) {
      return _doTransfer(_msgSender(), _msgSender(), _receiver, _amount * 10**decimals);
	}
  ...
  returns (bool) {
    ...
    _spendAllowance(_from, spender, _amount);
      return _doTransfer(_msgSender(), _from, _receiver, _amount * 10**decimals);
  }
  ...
  requiredTransfer(_sender, _from, _receiver, _amount) returns (bool){
      ...
      if(isPoolWallet(_sender)){
          fee = _amount * _sellTaxFee/100;
          tradingPools[_sender].totalTax += fee;
      }else {
        fee = _amount * _buyTaxFee/100;
        tradingPools[_receiver].totalTax += fee;
      }
      _transfer(_from,_receiver, _amount-fee);
      _transfer(_from,_taxWallet, fee);
