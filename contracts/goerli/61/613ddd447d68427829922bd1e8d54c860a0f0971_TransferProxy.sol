...
contract TransferProxy is Ownable {
  ...
  function sendETH(uint256 amount, address payable recipient, string memory id) public payable {
    require(msg.value == amount, "Eth not equal amount");

    uint256 amountMinusFee = amount.sub(feeFor(amount));

    (bool sent, ) = recipient.call{value: amountMinusFee}("");
    require(sent, "Failed to send Ether");

    emit Payment(msg.sender, recipient, address(0), msg.value, amountMinusFee, id);
  }

  function sendERC20(address tokenAddress, uint256 amount, address recipient, string memory id) public {
    ...
    emit Payment(msg.sender, recipient, tokenAddress, amount, amountMinusFee, id);
  }
