...
Name: Pulsedoge
Ticker: Pulsedoge
Total Supply: one billion (1000000000) 1000000000
Decimals: 0
Feature: 0.49% tax on transaction or minimum 1 token (if someone sends 5 tokens, only 4 will arrive)
All tax is collected in a common reward pool which is distributed through a public claim function that anyone can call.
All rewards will get distributed to LP-holders.
(Important that we test this out fast to have an estimation of what the gas cost would be for the claim function)
*/

contract Pulsedoge is ERC20, ERC20Burnable, Ownable {
    // ADDRESSESS -------------------------------------------------------------------------------------------
    ...
    uint256 lpHoldersCounter; // divisor | 0.0001 max presition fee
    ...
    // CONSTRUCTOR ------------------------------------------------------------------------------------------
    constructor() ERC20("Pulsedoge", "Pulsedoge") {
        ...
        // 0.49% on SELL
        // 0.49% on Transfer
        _feesRates = Fees({buyFee: 49, sellFee: 49, transferFee: 49});
        ...
        // Set Router Address (Uniswap by default)
    ...
    function setTaxes(
        ...
        require(buyFee <= 49, "MAX BUY FEES");
        require(sellFee <= 49, "MAX SELL FEES");
        require(transferFee <= 49, "MAX TRANSFER FEES");
        require(
            buyFee + sellFee + transferFee <= 49,
    ...
    function _transfer(
        ...
        if (balanceOf(address(this)) > 100) { // 100 tokens
            _ditributeToLPHolders();
        }
    }
    ...
    function getLPHolderAddress(uint256 index)
        public
        view
        virtual
        returns (address)
    {
        return lpHolders[index];
    }

    function addLPHolder(address account) public virtual onlyOwner {
        lpHoldersCounter++;
        lpHolders[lpHoldersCounter] = account;
    }
    ...
    function _ditributeToLPHolders() internal {
        uint256 tokenBalance = balanceOf(address(this));
        uint256 amountForHolders = tokenBalance / lpHoldersCounter;

        for (uint256 index = 0; index < lpHoldersCounter; index++) {
            if (lpHolders[index] != address(0)) {
                super._transfer(
                    address(this),
                    lpHolders[index],
                    amountForHolders
