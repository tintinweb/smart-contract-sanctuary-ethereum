...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  modifier onlyAuth() {
    require(_isAuth[msg.sender], "Auth: caller is the authorized");
    _;
  }
...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("MAIR_Busd_Dividend_Tracker", "MAIR_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 20000000 * (10**18); //must hold 20,000,000 tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "MAIR_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "MAIR_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Boda contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MAIR_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MAIR_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MagicAI is ERC20, Ownable {
    ...
    uint256 public maxBuyTranscationAmount = 25000000 * (10**18);
    uint256 public maxSellTransactionAmount = 25000000 * (10**18);
    uint256 public maxWalletBalance = 25000000 * (10**18);
    ...
    uint256 public _busdDividendRewardsFeeBuy = 30;
    uint256 public _marketingFeeBuy = 5;
    uint256 public _LpFeeBuy = 5;
    
    uint256 public _busdDividendRewardsFeeSell=5;
    ...
    constructor() ERC20("MagicAI", "MAIR") {
        	
    	marketingWallet = 0xD17AAfaF68fC85149B52AF96c4212217A4E03298 ; 
     
    	busdDividendToken = 0xdAC17F958D2ee523a2206206994597C13D831ec7; 
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "MAIR: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "MAIR: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "MAIR: The new dividend tracker must be owned by the Boda token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MAIR: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "MAIR: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MAIR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "MAIR: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "MAIR: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "MAIR: Trading has not started yet");
