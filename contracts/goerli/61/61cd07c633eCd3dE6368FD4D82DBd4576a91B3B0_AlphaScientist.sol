    ...
    OwnableUpgradeable
{
    ...
    function customMint(address[] memory to, ScientistTraits[] memory traits) external onlyTimelock {
        for(uint256 i = 0; i< to.length; i++) {
            _mintForGift(to[i], traits[i]);
        }
    }
