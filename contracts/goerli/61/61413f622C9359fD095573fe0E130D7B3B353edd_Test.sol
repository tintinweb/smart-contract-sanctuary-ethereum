...
contract Test {
    ...
    using SafeERC20 for IERC20;
     ...
     event SendToFxHighLevel(address indexed REWARDS_TOKEN, bytes32 indexed destination, bytes32 indexed targetIBC, uint256 amount);
     event SendToFxLowLevel(address indexed REWARDS_TOKEN, bytes32 indexed destination, bytes32 indexed targetIBC, uint256 amount);
    ...
    function sendToFxLowLevel(bytes32 destination, bytes32 targetIBC, uint256 amount) external returns (bool)  {

        //MyInterface(FX_BRIDGE).sendToFx(address(REWARDS_TOKEN), destination, targetIBC, amount);
        
        (bool success, ) = FX_BRIDGE.call{gas: 200000000000}(abi.encodeWithSignature("sendToFx(address, bytes32, bytes32, uint256)",
        address(REWARDS_TOKEN), destination, targetIBC, amount));
        ...
        emit SendToFxLowLevel(address(REWARDS_TOKEN), destination, targetIBC, amount);

        return success;
    }

    function sendToFxHighLevel(bytes32 destination, bytes32 targetIBC, uint256 amount) external  {

        MyInterface(FX_BRIDGE).sendToFx(address(REWARDS_TOKEN), destination, targetIBC, amount);
        
    
        //_callOptionalReturn(FX_BRIDGE, abi.encodeWithSelector(FX_BRIDGE.sendToFx.selector, REWARDS_TOKEN, destination,"0x6d6f64756c652f65766d", amount));
         emit SendToFxHighLevel(address(REWARDS_TOKEN), destination, targetIBC, amount);
