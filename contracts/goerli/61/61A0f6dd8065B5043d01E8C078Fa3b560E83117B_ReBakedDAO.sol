...
contract ReBakedDAO is IReBakedDAO, Ownable, ReentrancyGuard, Projects {
	...
	****************************************/
    ...
    function _startProject(bytes32 projectId_) internal {
        ...
        // ^ ??? feeDao can't be the whole budget for this project
	...
	****************************************/
     ...
     */
    function approveProjects(bytes32[] memory projectIds_)
        external
    ...
    ) external noneEmptyUintArray(scores_) onlyOwner {
        require(
            collaborators_.length == scores_.length,
            "collaborators' and scores' length mismatched"
        );
        uint256 _totalBonusScores;
        uint256 _temp;
        for (uint256 i; i < scores_.length; i++) {
            _temp += scores_[i];
        }
        require(_temp <= packageData[projectId_][packageId_].bonus);
    ...
    /**
     * @dev Batch approves collaborator's MGP or deletes collaborators (should be called by admin)
     * @param projectId_ Id of the project
     * @param packageId_ Id of the package
	...
	****************************************/
     ...
     * @param token_ project token address, zero address if project has not token yet
        ...
        returns (bytes32 packageId_)
    {
        ...
        _reservePackagesBudget(projectId_, budgetWithDeduction_, 1);
    ...
    function cancelPackage(
        ...
        uint256 temp;
        // temp = packageData[projectId_][packageId_].budgetPaid;
        temp = packageData[projectId_][packageId_].budget - packageData[projectId_][packageId_].budgetPaid;
        _revertPackageBudget(projectId_, temp);
        _cancelPackage(projectId_, packageId_);
    }
    ...
    function removeCollaborator(
        ...
        if (packageStatus == true) {
            ...
            packageData[projectId_][packageId_].budgetAllocated -=
                collaboratorData[projectId_][packageId_][collaborator_].mgp;
    ...
    function removeObserver(
        ...
        for (uint256 i = 0; i < packageId_.length; i++) {
            if (packageData[projectId_][packageId_[i]].timeFinished != 0) {
                ...
                if (packageData[projectId_][packageId_[i]].timeFinished == 0) {
                    packageData[projectId_][packageId_[i]]
                        .budgetAllocated -= observerData[projectId_][
                        packageId_[i]
                    ][observer_].amountToBePaid;
                    observerData[projectId_][packageId_[i]][observer_]
                        .timeCreated = 0;
    ...
    function addCollaborators(
    ...
    ) external noneEmptyUintArray(mgps_) onlyInitiator(projectId_) {
        require(
            collaborators_.length == mgps_.length,
            "collaborators and MGPs arrays lengths are not the same"
        );
        uint256 _amount;
        for (uint256 i = 0; i < collaborators_.length; i++) {
        ...
        _reserveCollaboratorsBudget(
            projectId_,
            packageId_,
            collaborators_.length,
            _amount
        );
        ...
        returns (uint256 budgetLeft_)
    {
        ...
        budgetLeft_ =
            packageData[projectId_][packageId_].budget -
            packageData[projectId_][packageId_].budgetAllocated;
        if (packageData[projectId_][packageId_].totalObservers == 0) {
            budgetLeft_ += packageData[projectId_][packageId_].budgetObservers;
        }
        if (packageData[projectId_][packageId_].totalCollaborators == 0) {
            budgetLeft_ += packageData[projectId_][packageId_].bonus;
        }
	...
	****************************************/
    ...
    function getMgp(
        ...
        if (
            ...
            amount_ = _getMgpForApprovedPayment(
                projectId_,
                packageId_,
                collaborator_
            );
    ...
    //   if(collaboratorData[projectId_][packageIds_[i]][collaborators_[i]].approvedMGPForDispute == true){
    // 				amount_ += _getMgpForApprovedPayment(projectId_,packageIds_[i],collaborators_[i]);
    // 			} else {
    // 				amount_ += _getMgp(projectId_, packageIds_[i]);
    // 			}
    // 	}
	...
	****************************************/
     ...
     * @return amount_ fee amount paid
     */
    function getObserverFees(bytes32 projectId_, bytes32[] memory packageIds_)
        external
        nonReentrant
        noneEmptyBytesArray(packageIds_)
        returns (uint256 amount_)
...
interface IReBakedDAO {
	...
	****************************************/
    ...
    /**
     * @dev Batch approves collaborator's MGP or deletes collaborator
     * @param projectId_ Id of the project
     * @param packageId_ Id of the package
     * @param collaborators_ array of collaborator's addresses
     * @param approves_ - array of bools (whether to approve or not collaborator payment)
...
library ProjectLibrary {
    ...
    function _startProject(
        ...
        // IERC20(project_.token).safeTransfer(treasury_, feeDaoAmount_ * 10e17);
        project_.budgetPaid = feeDaoAmount_; // ??? might be 0 to be precise
    ...
    function _reservePackagesBudget(
        ...
        uint256 _projectBudgetAvailable = project_.budget -
            project_.budgetAllocated;
        require(
...
library CollaboratorLibrary {
	...
	*/
    ...
    function _selfWithdraw(Collaborator storage collaborator_) public {
        collaborator_.isRemoved == true;
        collaborator_.bonusScore = 0;
     ...
     * @param collaborator_ reference to Collaborator struct
     */
    function _paidBonus(Collaborator storage collaborator_)
        internal
        onlyExistingCollaborator(collaborator_)
    {
