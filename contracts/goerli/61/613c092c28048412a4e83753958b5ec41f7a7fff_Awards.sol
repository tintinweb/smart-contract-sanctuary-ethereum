...
contract Awards is Ownable { //ERC721
    ...
    mapping(uint256 => NFTStructure) _allNfts;
    ...
    function mint(address _owner, uint256 _proposalId, uint256 _ordinalVote) public onlyOwner {
        ...
        NFTStructure storage nft = _allNfts[_tokenId];
    ...
    function safeTransferFrom(
        ...
        if (_to.code.length > 0) {
            require(ERC721TokenReceiver(_to).onERC721Received(
                msg.sender, 
                _from, 
                _tokenId, 
                data) == bytes4(keccak256("onERC721Received(address,address,uint256,bytes)")),
             "ERC721Receiver not implemented");
            }
    }
    ...
    function transferFrom(
        ...
        bool isOwner = (msg.sender == _tokenOwners[_tokenId]);
        bool isApprovedOperator = (_operatorApprovals[_tokenOwners[_tokenId]][msg.sender]);
        bool isApprovedToken = (msg.sender == _tokenApprovals[_tokenId]);

        require(isOwner || isApprovedOperator || isApprovedToken, "You are not authorized");
        require(_from == _tokenOwners[_tokenId], "The sender does not own this NFT");
        require(_to != address(0), "This is not a valid address");
        require(_tokenOwners[_tokenId] != address(0), "This is not a valid NFT");
    ...
    function ownerOf(uint256 _tokenId) public view returns (address) {
        require(_tokenOwners[_tokenId] != address(0), "Invalid NFT");
