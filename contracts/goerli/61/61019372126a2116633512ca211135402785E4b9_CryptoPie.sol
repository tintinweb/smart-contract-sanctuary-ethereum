    ...
    MinimalForwarder
{
    ...
    function getTokenConfigNumeratorFee(address token_address)
        external
        view
        override
        returns (uint256)
    ...
    function getTokenConfigDenumeratorFee(address token_address)
        external
        view
        override
        returns (uint256)
    ...
    function getTokenConfigMinimumFee(address token_address)
        external
        view
        override
        returns (uint256)
    ...
    function getTokenConfigMinimumPrice(address token_address)
        external
        view
        override
        returns (uint256)
    ...
    function getTokenConfigMinimumDelay(address token_address)
        external
        view
        override
        returns (uint256)
    ...
    function getSubscriptionPrice(uint256 subscription_id)
        external
        view
        override
        returns (uint256)
    ...
    function getSubscriptionDelay(uint256 subscription_id)
        external
        view
        override
        returns (uint256)
    ...
    function getSubscriptionTokenAddress(uint256 subscription_id)
        external
        view
        override
        returns (address)
    ...
    function getSubscriptionFundsReceiver(uint256 subscription_id)
        external
        view
        override
        returns (address)
    ...
    function getUserSubscriptionsEvents(address user)
        external
        view
    ...
    function getSubscriptionUserLastPayment(
    ...
    function getSubscriptionSpecialMinimumFee(uint256 subscription_id)
        external
        view
        override
        returns (uint256)
    ...
    function getSubscriptionSpecialNumeratorFee(uint256 subscription_id)
        external
        view
        override
        returns (uint256)
    ...
    function getSubscriptionSpecialDenumeratorFee(uint256 subscription_id)
        external
        view
        override
        returns (uint256)
    ...
    function getOneTimePaymentSpecialMinimumFee(address funds_receiver)
        external
        view
        override
        returns (uint256)
    ...
    function getOneTimePaymentSpecialNumeratorFee(address funds_receiver)
        external
        view
        override
        returns (uint256)
    ...
    function getOneTimePaymentSpecialDenumeratorFee(address funds_receiver)
        external
        view
        override
        returns (uint256)
    ...
    function pause() external override onlyRole(MODERATOR_ROLE) {
        _pause();
    }

    function unpause() external override onlyRole(MODERATOR_ROLE) {
        _unpause();
    }
    ...
    function add_token(
        ...
        require(
            numerator_fee <= denumerator_fee,
            "Denumerator fee < numerator fee"
        );
    ...
    function add_special_fees_subscription(
        uint256 subscription_id,
        uint256 minimum_fee,
        uint256 numerator_fee,
        uint256 denumerator_fee
        ...
        _special_fees_subscription[subscription_id]
            .numerator_fee = numerator_fee;
        _special_fees_subscription[subscription_id]
            .denumerator_fee = denumerator_fee;
        _special_fees_subscription[subscription_id].minimum_fee = minimum_fee;

        emit AddSpecialFeesSubscription(
            _msgSender(),
            subscription_id,
            minimum_fee,
            numerator_fee,
    ...
    function add_special_fees_one_time_payment(
        ...
        _special_fees_one_time_payment[funds_receiver]
            .numerator_fee = numerator_fee;
        _special_fees_one_time_payment[funds_receiver]
            .denumerator_fee = denumerator_fee;
        _special_fees_one_time_payment[funds_receiver]
        ...
        emit AddSpecialFeesOneTimePayment(
            _msgSender(),
            funds_receiver,
            minimum_fee,
            numerator_fee,
    ...
    function withdrawal(address withdrawal_address, address[] memory tokens)
        external
        override
    ...
    function _make_transfer(
        ...
        if (token.allowance(from, address(this)) >= amount_decimals) {
            ...
            uint256 funds_fee = (amount_decimals / denumerator_fee) *
                numerator_fee;
    ...
    function _process(
        ...
        TokenConfig memory token_config
    ) internal returns (bool) {
        ...
        if (_special_fees_subscription[subscription_id].denumerator_fee > 0) {
            subscription_minimumfee = _special_fees_subscription[
                subscription_id
            ].minimum_fee;
            subscription_numerator_fee = _special_fees_subscription[
                subscription_id
            ].numerator_fee;
            subscription_denumerator_fee = _special_fees_subscription[
                subscription_id
            ].denumerator_fee;
        }
        ...
        if (
            user_last_payment > 0 &&
            block.timestamp >= user_last_payment + subscription.delay_seconds
        ) {
    ...
    function process(uint256 subscription_id, address[] memory users)
        external
        override
        onlyRole(ASSISTANT_ROLE)
        whenNotPaused
    {
        ...
        TokenConfig memory token_config = _token_config[
            subscription.token_address
        ];
        ...
        for (uint256 i = 0; i < users.length; i++) {
            if (
                _process(subscription_id, users[i], subscription, token_config)
            ) {
                successful_claims++;
            } else {
                failed_claims++;
    ...
    function create_subscription(
        ...
        require(
            _token_config[token_address].minimum_price > 0,
            "Token is not authorized"
        );
        require(
            _token_config[token_address].minimum_price <= price_decimals,
            "Price is low"
        );
        require(
            _token_config[token_address].minimum_delay <= delay_seconds,
            "Delay is low"
        );
    ...
    function subscribe(
        ...
        require(subscription.price_decimals > 0, "Subscription not found");
        require(
            _user_last_payment[subscription_id][_msgSender()] == 0,
            "User is already subscribed"
        );
        require(
            subscription_price_decimals == subscription.price_decimals,
            "Wrong subscription price in signature"
        );
        require(
            subscription_delay_seconds == subscription.delay_seconds,
            "Wrong subscription delay in signature"
        );
        require(
            subscription_token_address == subscription.token_address,
            "Wrong subscription token in signature"
        );
        require(
            subscription_funds_receiver == subscription.funds_receiver,
            "Wrong subscription receiver in signature"
        );
        ...
        require(
            _process(subscription_id, _msgSender(), subscription, token_config),
            "Payment fail"
        );
    ...
    function single_payment(
        ...
        if (_special_fees_one_time_payment[funds_receiver].denumerator_fee == 0) {
            require(
                _make_transfer(
                    _msgSender(),
                    funds_receiver,
...
interface ICryptoPie {
    ...
    event AddSpecialFeesSubscription(
        address indexed caller,
    ...
    event AddSpecialFeesOneTimePayment(
        address indexed caller,
        address indexed funds_receiver,
        uint256 minimum_fee,
        uint256 numerator_fee,
    ...
    event Process(
        address indexed caller,
        uint256 indexed subscription_id,
        uint256 successful_claims,
        uint256 failed_claims
    );
    ...
    event Withdrawal(
        address indexed caller,
        address indexed withdrawal_address,
        address indexed token_address,
        uint256 amount
    );
