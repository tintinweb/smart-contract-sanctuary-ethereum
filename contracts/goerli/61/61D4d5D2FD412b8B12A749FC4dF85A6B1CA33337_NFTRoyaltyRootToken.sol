...
// File: contracts/NFTRoyaltyToken.sol
...
contract NFTRoyaltyToken is ERC721Enumerable, ERC721URIStorage, Ownable{
    ...
    function setContractUri(string memory contractUri_) external onlyOwner{
        require(bytes(contractUri_).length > 0 ,"The _contractUri length should be greater than 0");
...
// File: contracts/NFTRoyaltyRootToken.sol


pragma solidity 0.8.7;


contract NFTRoyaltyRootToken is NFTRoyaltyToken{

    address private immutable _mintableERC721Proxy;
    
    constructor (string memory name, string memory symbol,address mintableERC721Proxy) NFTRoyaltyToken(name,symbol) {
      _mintableERC721Proxy = mintableERC721Proxy;  
    }
 ...
 function setTokenMetadata(uint256 tokenId, bytes calldata data) external onlyMintableProxy{
        string memory uri = abi.decode(data, (string));
        _setTokenURI(tokenId, uri);
    }

   modifier onlyMintableProxy() {
      require(_msgSender() == _mintableERC721Proxy,"The address needs to be mintable erc721 proxy");
       _;
    }
