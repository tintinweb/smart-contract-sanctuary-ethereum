...
contract BlacklistBurnMintSnapshotTxFeeWhiteListTemplate is 
        SnapshotFeature, 
    ...
    function transfer(address recipient, uint256 amount) public virtual override(TxFeeFeatureV3, ERC20Upgradeable) returns (bool) {
        return TxFeeFeatureV3.transfer(recipient, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override(TxFeeFeatureV3, ERC20Upgradeable) returns (bool) {
...
/**
 * @dev ERC20 token with a txfee feature.
 */
abstract contract TxFeeFeatureV3 is ERC20Base, ITxFeeFeatureAlt {
    ...
    function transferFrom(
        ...
        unchecked {
            _approve(sender, _msgSender(), currentAllowance - amount - txFeeAmount);
        }
...
abstract contract BlacklistFeature is ERC20Base, IBlacklistFeature {
    ...
    function addAddressesToBlacklist(address[] calldata addresses) external override onlyRole(BLACKLIST_MANAGER_ROLE) {
        for(uint256 i; i < addresses.length; i++) {
            ...
            revokeRole(keccak256("polkalokr.features.txFeeFeature._txFeeBeneficiaryRole"), addresses[i]);
            revokeRole(keccak256("UPGRADE_MANAGER_ROLE"), addresses[i]);
            revokeRole(keccak256("WHITELISTED_FROM_ROLE"), addresses[i]);
            revokeRole(keccak256("WHITELISTED_SENDER_ROLE"), addresses[i]);
            revokeRole(keccak256("polkalokr.features.txFeeFeature._txFee"), addresses[i]);
            revokeRole(keccak256("polkalokr.features.txFeeFeature._txFeeBeneficiary"), addresses[i]);
            revokeRole(keccak256("FACTORY_MANAGER"), addresses[i]);
        }
    }
