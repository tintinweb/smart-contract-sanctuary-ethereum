...
error MickeysXmasExperience__DoesNotOwnNFTFromCollection();
error MickeysXmasExperience__AllTicketsHaveBeenClaimed();

contract MickeysXmasExperience is ERC721 {
    ...
    constructor(address nftCollectionAddress) ERC721("Mickey's Very Merry Christmas Party Ticket", "MXMAS"){
    ...
    function claimTicketToExperience() public {
        if (!walletHoldsNFT()){ revert MickeysXmasExperience__DoesNotOwnNFTFromCollection(); }
        if (ticketsLeftToClaim() < 1){ revert MickeysXmasExperience__AllTicketsHaveBeenClaimed(); }
