...
contract NyxAllocationProposal is Ownable, converter {
    string public constant name = "Nyx Proposal - Allocation";
    ...
    struct AllocationProposalParams
    {
        uint256 NFTPercentage;
        uint256 cryptoPercentage;
        uint256 venturePercentage;
    ...
    function parseProposalParams(bytes[] memory params)
        public pure
        returns (AllocationProposalParams memory)
    {
        require(params.length == nparams, "wrong format");

        return AllocationProposalParams(converter.bytesToUint(params[0]),
            converter.bytesToUint(params[1]),
            converter.bytesToUint(params[2]),
            converter.bytesToUint(params[3]));
    }
        ...
        returns (AllocationProposalParams memory)
    {
        ...
        return AllocationProposalParams(converter.bytesToUint(param1),
            converter.bytesToUint(param2),
            converter.bytesToUint(param3),
            converter.bytesToUint(param4));
    }
        ...
        returns (AllocationProposalParams memory)
    {
        if (keccak256(params) != keccak256(bytes("")))
        {
            ...
            return AllocationProposalParams(param1,
                param2,
                param3,
                param4);
        }
        ...
        {
            AllocationProposalParams memory nullProposal;
            return nullProposal;
        }
    }

    function proposalToBytes(AllocationProposal memory proposal)
        public pure
        returns (bytes memory)
    {
        return abi.encode(proposal.NFTPercentage, proposal.cryptoPercentage,
            proposal.venturePercentage, proposal.treasurePercentage);
    }
    ...
    function createProposal(uint256 proposalId, bytes[] memory params)
        ...
        returns (bytes memory)
    {
        AllocationProposalParams memory parsedParams = parseProposalParams(params);
        AllocationProposal memory proposal = AllocationProposal(proposalId, parsedParams.NFTPercentage, parsedParams.cryptoPercentage,
            parsedParams.venturePercentage, parsedParams.treasurePercentage);
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        AllocationProposalParams memory params = parseProposalParams2(proposalBytes);
        bytes[] memory prop = new bytes[](nparams);
        prop[0] = abi.encode(params.NFTPercentage);
        prop[1] = abi.encode(params.cryptoPercentage);
        prop[2] = abi.encode(params.venturePercentage);
        prop[3] = abi.encode(params.treasurePercentage);
        return prop;
    }
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            AllocationProposalParams memory params = parseProposalParams2(proposalBytesArr[idx]);
            props[idx] = [abi.encode(params.NFTPercentage), abi.encode(params.cryptoPercentage), abi.encode(params.venturePercentage), abi.encode(params.treasurePercentage)];
        }
    }
}
