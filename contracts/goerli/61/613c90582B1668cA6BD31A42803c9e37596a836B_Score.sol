...
contract Score
{
	...
	function addScoreSeal(bytes memory proof) external	// proof of seal, seal must be less then 1 hour old
	{
		 (, uint256 timestamp, uint256 permalink, uint128 version,) = listhash.getSeal(proof);
    	 require( version != 1, "S7 claim is revoked");
    	 require( (block.timestamp - timestamp) < 3600, "S8 seal is more than one hour old");
