...
contract ARCANA is Ownable {
    ...
    mapping(uint256 => uint256) public standbyStarted;
        ...
        onlyChimeraOwner(_contractIndex, _tokenId)
    {
        ...
        uint256 start = standbyStarted[_tokenId];
        if (start == 0) {
            standbyStarted[_tokenId] = block.timestamp;
        } else {
    ...
    {
        standbyTotal[_address] = standbyTotal[_address] - _size;
    }
}
