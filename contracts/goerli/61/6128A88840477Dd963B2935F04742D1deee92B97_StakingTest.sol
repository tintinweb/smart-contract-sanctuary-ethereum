...
contract StakingTest is Ownable, ReentrancyGuard {
    ...
    function unstake() external nonReentrant {
        require(block.timestamp <= end + 15 minutes, "Stake is lost");
    ...
    function claimRestTyz() external onlyOwner nonReentrant {
        require(
            block.timestamp > end + 15 minutes ||
        ...
        )
    {
        if (start > 0 && block.timestamp >= start) {
            staking = block.timestamp < end - 5 minutes;
            withdraw = block.timestamp <= end + 15 minutes;
