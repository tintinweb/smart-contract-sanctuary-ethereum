...
contract WPCD is ERC721A, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    mapping(address => bool) public mintedPool;
     ...
     */
    event NFTMintEvent(uint256 token, address to, uint256 quantity);
    ...
    event CommonTokenURIEvent(string _commonTokenURI);

    constructor(
        address _executorAddress,
        string memory baseUri,
        string memory _commonTokenURI,
        uint256 _maxTotalSupply
    ) ERC721A("WPCD", "WPCD"){
        executorAddress = _executorAddress;
        baseURI = baseUri;
        commonTokenURI = _commonTokenURI;
        maxTotalSupply = _maxTotalSupply;
    }

    modifier nftIsOpen {
        require(!PAUSE, "nft has not open");
        _;
    }

    modifier onlyExecutor {
        require(executorAddress == _msgSender() || owner() == _msgSender(), "caller is not the executor or ower");
        _;
    }

    function setBaseURI(string memory _baseURI) public onlyExecutor {
        require(bytes(_baseURI).length > 0, "URI length not null");
        baseURI = _baseURI;
        emit BaseURIEvent(_baseURI);
    }

    function setCommonTokenURI(string memory _commonTokenURI) public onlyExecutor {
        require(bytes(_commonTokenURI).length > 0, "URI length not null");
        commonTokenURI = _commonTokenURI;
        emit CommonTokenURIEvent(_commonTokenURI);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(tokenURI).length > 0) {
            uri = string(abi.encodePacked(uri, tokenURI));
        } else {
            uri = string(abi.encodePacked(uri, commonTokenURI));
            uri = string(abi.encodePacked(uri, uint2str(tokenId)));
        }
        return uri;
    }
    ...
    function setMaxTotalSupply(uint256 _maxTotalSupply) public onlyExecutor {
        maxTotalSupply = _maxTotalSupply;
        emit MaxTotalSupplyEvent(_maxTotalSupply);
    }
    ...
    function executorMint(address to, uint256 quantity) public onlyExecutor nftIsOpen {
        ...
        _mint(to, quantity);
        emit NFTMintEvent(tokenId, to, quantity);
    }

    function mint(address to) public nftIsOpen {
        require(msg.sender == tx.origin, "only origin");
        require(to == msg.sender, "The address of to cannot be the address of the caller");

        require(maxTotalSupply == 0 || totalSupply() <= maxTotalSupply, "Total limit");

        bool minted = mintedPool[to];
        require(!minted, "This address has mint");
    ...
    function isBlackMarketplaces(address operator) public view returns (bool){
