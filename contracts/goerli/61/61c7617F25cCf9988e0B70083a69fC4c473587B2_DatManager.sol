...
import "./interfaces/IDatManager.sol";
...
contract DatManager is ReentrancyGuard, Governable, IDatManager {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant PRICE_PRECISION = 10 ** 30;
    uint256 public constant USDG_DECIMALS = 18;
    uint256 public constant DAT_PRECISION = 10 ** 18;
    ...
    constructor(address _vault, address _usdg, address _dat, address _shortsTracker, uint256 _cooldownDuration) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdg = _usdg;
        dat = _dat;
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external override onlyGov {
        require(shortsTrackerAveragePriceWeight <= BASIS_POINTS_DIVISOR, "DatManager: invalid weight");
    ...
    function setCooldownDuration(uint256 _cooldownDuration) external override onlyGov {
        require(_cooldownDuration <= MAX_COOLDOWN_DURATION, "DatManager: invalid _cooldownDuration");
        cooldownDuration = _cooldownDuration;
    }
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minDat) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("DatManager: action not enabled"); }
        return _addLiquidity(msg.sender, msg.sender, _token, _amount, _minUsdg, _minDat);
    }

    function addLiquidityForAccount(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minDat) external override nonReentrant returns (uint256) {
        _validateHandler();
        return _addLiquidity(_fundingAccount, _account, _token, _amount, _minUsdg, _minDat);
    }

    function removeLiquidity(address _tokenOut, uint256 _datAmount, uint256 _minOut, address _receiver) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("DatManager: action not enabled"); }
        return _removeLiquidity(msg.sender, _tokenOut, _datAmount, _minOut, _receiver);
    }

    function removeLiquidityForAccount(address _account, address _tokenOut, uint256 _datAmount, uint256 _minOut, address _receiver) external override nonReentrant returns (uint256) {
        _validateHandler();
        return _removeLiquidity(_account, _tokenOut, _datAmount, _minOut, _receiver);
    }

    function getPrice(bool _maximise) external view returns (uint256) {
        uint256 aum = getAum(_maximise);
        uint256 supply = IERC20(dat).totalSupply();
        return aum.mul(DAT_PRECISION).div(supply);
    }
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minDat) private returns (uint256) {
        require(_amount > 0, "DatManager: invalid _amount");

        // calculate aum before buyUSDG
        uint256 aumInUsdg = getAumInUsdg(true);
        uint256 datSupply = IERC20(dat).totalSupply();

        IERC20(_token).safeTransferFrom(_fundingAccount, address(vault), _amount);
        uint256 usdgAmount = vault.buyUSDG(_token, address(this));
        require(usdgAmount >= _minUsdg, "DatManager: insufficient USDG output");

        uint256 mintAmount = aumInUsdg == 0 ? usdgAmount : usdgAmount.mul(datSupply).div(aumInUsdg);
        require(mintAmount >= _minDat, "DatManager: insufficient DAT output");

        IMintable(dat).mint(_account, mintAmount);

        lastAddedAt[_account] = block.timestamp;

        emit AddLiquidity(_account, _token, _amount, aumInUsdg, datSupply, usdgAmount, mintAmount);

        return mintAmount;
    }

    function _removeLiquidity(address _account, address _tokenOut, uint256 _datAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        require(_datAmount > 0, "DatManager: invalid _datAmount");
        require(lastAddedAt[_account].add(cooldownDuration) <= block.timestamp, "DatManager: cooldown duration not yet passed");

        // calculate aum before sellUSDG
        uint256 aumInUsdg = getAumInUsdg(false);
        uint256 datSupply = IERC20(dat).totalSupply();

        uint256 usdgAmount = _datAmount.mul(aumInUsdg).div(datSupply);
        ...
        IMintable(dat).burn(_account, _datAmount);

        IERC20(usdg).transfer(address(vault), usdgAmount);
        uint256 amountOut = vault.sellUSDG(_tokenOut, _receiver);
        require(amountOut >= _minOut, "DatManager: insufficient output");

        emit RemoveLiquidity(_account, _tokenOut, _datAmount, aumInUsdg, datSupply, usdgAmount, amountOut);

        return amountOut;
    }

    function _validateHandler() private view {
        require(isHandler[msg.sender], "DatManager: forbidden");
    }
}
