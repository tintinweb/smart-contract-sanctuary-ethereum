...
import "./interfaces/IScientistCharacteristic.sol";
    ...
    OwnableUpgradeable
{
    ...
    // Constants
    uint256 public constant LENGTH_OF_SCIENTIFIC_FIELDS = 7;
    ...
    uint256 public constant LENGTH_OF_VICES = 7;
    ...
    mapping(bytes32 => bool) public isClaimedByLeaf;
    address private proxyRegistryAddress;
    uint96 public feeNumerator;

    // Events
    event SetGiftMerkleRootAndPrice(
        bytes32 merkleTree,
        uint256 price,
        uint256 timestamp
    );
    ...
    event SetMintableAmount(uint256 amount, uint256 timestamp);
    ...
    modifier isValidMerkleProof(bytes32[] calldata merkleProof, bytes32 root) {
        ...
        bytes32 validateData = getKeccakScientist(msg.sender);
        bytes32 leaf = keccak256(abi.encodePacked(validateData));
        require(
            claimedTimes[msg.sender] < maxClaimed && !isClaimedByLeaf[leaf],
            "Scientist is already claimed by this wallet"
        );
        claimedTimes[msg.sender]++;
        isClaimedByLeaf[leaf] = true;
        require(
    ...
    function initialize(
        ...
        baseTokenURI = "http_create(account);s://madscientists.org/";
        contractURI = "https://madscientists.org/";
        maxClaimed = 5;
    }
    ...
    function setMintableAmount(uint256 newAmount) external onlyTimelock {
        ...
        emit SetMintableAmount(newAmount, block.timestamp);
    }
    ...
    function setGiftMerkleRootAndPrice(bytes32 merkleRoot, uint256 newPrice)
        external
        onlyTimelock
    {
        giftMerkleRoot = merkleRoot;
        price = newPrice;
        emit SetGiftMerkleRootAndPrice(merkleRoot, newPrice, block.timestamp);
    }
    ...
    function _create(address _account) internal {
        ...
        _setTokenRoyalty(tokenIdCount.current(), msg.sender, feeNumerator);
    }
...
abstract contract AScientistRepository is IScientistRepository, Multicall {
    ...
    function _removeScientist(address _user, uint256 _tokenId) internal {
        ...
        require(_scientist.user != address(0), "Token not exists");
