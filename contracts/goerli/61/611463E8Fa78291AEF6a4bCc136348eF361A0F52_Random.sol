...
contract Random is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    {
        bool hasNumbers = randomNumbers.length == 0;
        upkeepNeeded = (hasNumbers);
    ...
    function fulfillRandomWords(
        ...
        randomNumbers = new uint256[](NUM_WORDS);
        randomTimestamp = new uint256[](NUM_WORDS);
        for (uint256 i = 0; i < NUM_WORDS; i++) {
            randomNumbers[i] = randomWords[i];
            randomTimestamp[i] = randomWords[i] % s_seconds;
        }
        emit NumbersPicked(randomNumbers, randomTimestamp);
    }
    ...
    function getSeconds() public view returns (uint32) {
        return s_seconds;
    }
}