    ...
    UUPSUpgradeable 
{
   ...
   // Initializes initial contract state
    ...
    function fractionalizeNft(
       ...
       require(paymentToken[_paymentIndx] != address(0), "no payment token on this index");
        ...
        require(_supply >= denominator," supply too low");
        uint256 amountLiquidityWallet = ((_supply * liquidityShare) / denominator);
        uint256 availableForSale = (_supply - amountLiquidityWallet); 
       ...
       address uniSwapPair = IUniswapV2Factory(router.factory()).createPair(
        ...
        Shares(__token).setPairAddress(uniSwapPair); 
    ...
    function buyShare(
        ...
        require(finalfee >= denominator, "Insufficient payment amount");
    ...
    function calculateShare(uint256 _amount) public view returns(uint256, uint256, uint256) {
        (uint16 companyRatio, uint16 creatorRatio, uint16 liquidityRatio) = getShareRatio();
         uint256 companyA =((_amount * companyRatio)/ denominator) ;
         uint256 creatorA = ((_amount * creatorRatio )/ denominator);
         uint256 liquidityA = ((_amount * liquidityRatio)/ denominator) ;
    ...
    // Allow only owner to remove a whitelisted a address to fractionalize 
    function setHoldoutPeriod(uint256 _period) external onlyRole(DEFAULT_ADMIN_ROLE) {
        holdoutPeriod = _period;
    ...
    function setShareRatio(uint16 _companyShare, uint16 _creatorShare, uint16 _liquidityShare) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_companyShare + _creatorShare + _liquidityShare == denominator, "Sum must be equal to 10000 pt");
...
contract Shares is Context, IERC20, IERC20Metadata, Ownable {
 ...
 constructor(  
        ...
        _mint(_liquidityWallet, _amountLiquidityWallet);
        _mint(msg.sender, _PreSaleAmount);
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        checkAndTransfer(msgSender,  recipient,  amount);
        return true;
    }
    ...
    function transferFrom(address from, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        checkAndTransfer( from,  recipient,  amount);

        return true;
    }

    //calculates the fee for the given amount    
    function calculateAndTransfer(address from, address recipient, uint256 amount) internal { 
        require(amount >= denominator," amount too low");
        require(block.timestamp > preSaleEndTime, "Shares: cannot swap before sale ends");

        uint256 _ownerFee = (amount * creatorFee) / denominator;
        uint256 _platformFee = (amount * platformFee) / denominator;
    ...
    function checkAndTransfer(address from, address recipient, uint256 amount) internal{

        if (from == platformAddress || from == liquidityWallet || from == ownerAddress) {
            _transfer(from, recipient, amount);
        }  else if (from == uniswapV2Pair || recipient == uniswapV2Pair) {
            calculateAndTransfer(from, recipient, amount);
        } else { 
