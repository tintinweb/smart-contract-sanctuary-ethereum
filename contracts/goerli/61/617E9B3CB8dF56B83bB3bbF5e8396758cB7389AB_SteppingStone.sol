...
// ----------------------------
// Mint 1 of two versions of NFT, each will be burned after 30 days
// depending on the msg.value sent it will mint the corresponding nft
// ~$50 for current price in eth, ~$300 for current price in eth
// 1st will be burned after 1 usage
// 2nd will be burned after 30 days

// Core Functions:
// --------------------
//  1. Minting Function
//  2. Burning Function
//  3. Automatic burn with chainlink keepers

// What can currently be done?

// mints two diff versions of nft, each with unique metadata and expiration dates
// only need to test that it will burn and update mapping as expected
// lastly check for security issues , aka only owner should be able to burn or valid addresses
...
error UpKeepNotNeeded(uint256 timeStamp);
error TransferFailed();
error NothingToWithdraw();
error NotOwner();

contract SteppingStone is ERC721URIStorage, Ownable, KeeperCompatibleInterface {
    // Type Declarations
    ...
    event NftMinted(address minter, address nftContract, Target token);
    ...
    function handleExpiredTokens() external {
        ...
        for (uint256 i = 0; i < amountOfTokens; i++) {
            ...
            if (!target.burned) {
                if ((target.expires - lastTimeStamp) > interval) {
                    redeemNft(target.tokenId);
                }
            }
            // if time has expired, should be for 1 month in seconds.
            // currently just getting creation of contract
        }
    ...
    // for local
    function LOCAL_handleExpiredTokens() public {
        // currently x amount of nfts
        uint256 amountOfTokens = s_tokenCounter;
        ...
        for (uint256 i = 0; i < amountOfTokens; i++) {
            // target = s_targets[tokensToUpdate[i]];
            target = s_targets[i];

            // if time has expired, should be for 1 month in seconds.
            // currently just getting creation of contract
            // console.log("target.expires", target.expires);
            // console.log("target.timeStamp", target.timeStamp);
            // console.log("lastTimeStamp", lastTimeStamp);
            // console.log(target.burned);
            if (!target.burned) {
                if ((target.expires - lastTimeStamp) > interval) {
                    // redeemNft(target.tokenId);
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        console.log("inside performUpkeep!!!");
        if (!upKeepNeeded) {
            revert UpKeepNotNeeded(block.timestamp);
        }

        LOCAL_handleExpiredTokens();

        emit PerformUpKeepCompleted();
    }

    // may need to update this to be 1 month from mint, block.timestamp +
    //    /**
    //    * @notice Mints a one time use nft that will burn after x amount of seconds or until redeemed
    //    * @emits NftMinted Event
    //    */

    function mintOneTimeUseNft() public payable returns (uint256) {
        mintFee = getMintFeeForOneTimeUse();
        ...
        s_targets[newTokenId] = Target({
            ...
            expires: (block.timestamp + 30),
            timeStamp: block.timestamp,
            tokenId: newTokenId,
            burned: false
        });

        emit NftMinted(msg.sender, address(this), s_targets[newTokenId]);
    }
    ...
    function mintUnlimitedUseNft() public payable returns (uint256) {
        mintFee = getMintFeeUnlimitedUse();
        if (msg.value != mintFee) {
            revert IncorrectAmountOfEthSent();
        }
    ...
    function getCreationTimeStamp(uint256 tokenId)
        public
        view
        returns (uint256)
    {
        return s_targets[tokenId].timeStamp;
    }
