    ...
    UUPSUpgradeable
{
    ...
    function _AccessControlledAndUpgradeable_init(address initialAdmin)
        internal
        onlyInitializing
    {
...
contract WhitelistedFunctionExecutor is AccessControlledAndUpgradeable {
    address public safe;

    // mapping targetAddress => functionSignature => bool
    mapping(address => mapping(bytes4 => bool)) public whitelisted;
    ...
    function setUp(address _owner, address _safe) public initializer {
        _AccessControlledAndUpgradeable_init(_owner);
        safe = _safe;
    }

    receive() external payable {}

    // Same as the fallback, but here for convenience
    function deposit() external payable {}

    function withdrawGas(uint256 amount, address recipient)
        public
        payable
        adminOnly
    {
    ...
    function whitelistFunction(address target, bytes4 functionSignature)
        public
        adminOnly
    {
        whitelisted[target][functionSignature] = true;
    }

    function unwhitelistFunction(address target, bytes4 functionSignature)
        public
        adminOnly
    {
        whitelisted[target][functionSignature] = false;
    }

    function isWhitelisted(address target, bytes4 functionSignature)
        public
        view
        returns (bool)
    {
        return whitelisted[target][functionSignature];
    }
    ...
    function exec(
        ...
        require(
            isWhitelisted(target, functionSignature),
            "Function not whitelisted"
        );
        // (bool success, bytes memory response) = target.call(
        //     abi.encodePacked(functionSignature, data)
        // );
        // if (!success) {
        //     assembly {
        //         revert(add(response, 0x20), mload(response))
        //     }
        // }
        MockSafe(payable(safe)).execTransactionFromModule(
            payable(target),
            0,
            abi.encodePacked(functionSignature, data),
            0
        ...
        // This is the gas used by the call and transfer outside of checking the gas with a proxy and gnosis safe. Value might change based on data size etc.
        uint256 callAndTransferGas = 34596;

        // NOTE: this should pay a little bit more for the cost of transfer and function call
        payable(msg.sender).transfer(
            (gasUsed + callAndTransferGas) * tx.gasprice
        );
    }
}