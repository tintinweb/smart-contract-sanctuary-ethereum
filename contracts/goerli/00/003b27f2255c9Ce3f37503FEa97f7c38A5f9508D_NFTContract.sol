...
contract NFTContract is IERC721 {
  ...
  event msgSender(string _name, address msgSender);
  ...
  function getMetadata(uint256 _token_id) external payable onlyNFTOwner(_token_id) returns (string memory _name, string memory _description, string memory _imageURI, uint256 _mintDate) {
    emit msgSender('metadata', msg.sender);
  ...
  modifier onlyNFTOwner(uint256 _token_id) {
    ...
    emit msgSender('onlyNFTOwner', msg.sender);
