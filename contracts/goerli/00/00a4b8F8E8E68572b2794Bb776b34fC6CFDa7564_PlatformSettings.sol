...
contract PlatformSettings is IPlatformSettings {
    ...
    function setConsts(address newConstsAddress) external onlyConfigurator(msg.sender) {
        require(newConstsAddress.isContract(), "NEW_CONSTS_MUST_BE_CONTRACT");

        address oldConsts = consts;

        consts = newConstsAddress;

        emit ConstsUpdated(msg.sender, oldConsts, newConstsAddress);
    }
