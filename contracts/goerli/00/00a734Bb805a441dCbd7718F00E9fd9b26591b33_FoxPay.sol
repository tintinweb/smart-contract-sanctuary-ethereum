...
contract FoxPayMetadataDescriptor is IERC3525MetadataDescriptor {
    ...
    function _generateSVGGiftMessage(
        ...
        (string memory former, string memory latter) = _splitString(giftMessage_, 5);
        return
            string(
                abi.encodePacked(
                    /* solhint-disable */
    ...
    function _splitString(string memory str_, uint256 length_) internal pure returns (string memory, string memory) {
        uint256 totalLength = bytes(str_).length;

        if (totalLength == 0) {
            return ("", "");
        } else if (totalLength <= length_) {
            return (str_, "");
        }
