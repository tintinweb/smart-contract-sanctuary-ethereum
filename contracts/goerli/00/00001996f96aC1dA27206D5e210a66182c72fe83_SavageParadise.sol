...
contract SavageParadise is ERC721A, ERC2981, Ownable {

  bool public isPublicMintingOpened;

  uint16 public constant COLLECTIONSIZE = 10000;

  uint256 public publicPrice = 0.2 ether;
  uint256 public immutable NFBTPERIOD;

  string public baseURI;
  address public proxyRegistryAddress;
  address public treasureAddress;
  address public nfbtAddress;
  ...
  event NFBTAddressSet(address nfbt);
  ...
  constructor(
    ...
    address _proxy,
    uint256 _period,
    address _nfbtAddress)
    ERC721A("Savage Paradise","SAVAGEPARADISE")
    {
      require(owner != address(0), "Please provide a valid owner");
      baseURI = _uri;
      proxyRegistryAddress = _proxy;
      setDefaultRoyalty(owner, 700); // 7% fees
      setNFBTAddress(_nfbtAddress);
      NFBTPERIOD =_period;
  ...
  /// @param feeNumerator 10% => feeNumerator = 1000
  function setDefaultRoyalty(address receiver, uint96 feeNumerator) public onlyOwner{
    _setDefaultRoyalty(receiver, feeNumerator);
    emit DefaultRoyaltySet(receiver, feeNumerator);
  }

  /// @notice OpenSea specifics
  ...
  function setNFBTAddress(address nfbt) public onlyOwner {
    nfbtAddress = nfbt;
    emit NFBTAddressSet(nfbt);
  }
  ...
  function claimNFBT(uint256 id) external {
    // ownership.startTimestamp is updated uppon mint, transfer or burn
    TokenOwnership memory ownership = _ownershipOf(id);
    uint256 start = (ownership.startTimestamp > _lastClaimedAt[id]) ? ownership.startTimestamp : _lastClaimedAt[id];

    // Check at least one period has passed
    require(block.timestamp > start + NFBTPERIOD, "Not Holding Enough");

    // Calculate the number of NFBT that can be claimed
    uint256 _count = (block.timestamp - start) / NFBTPERIOD;

    // Update the counter for next claim
    _lastClaimedAt[id] = start + NFBTPERIOD * _count;

    // mint the claimed NFBT
    NFBT(nfbtAddress).mint(ownerOf(id), uint96(id), _count);
  }

  // Allow to update the treasure by the treasure
