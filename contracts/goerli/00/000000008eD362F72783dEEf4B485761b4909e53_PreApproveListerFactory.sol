...
contract PreApproveListerFactory {
     ...
     * @return lister The address of the deployed lister.
     */
    function deployDeterministic(address initialOwner, bytes32 salt) external payable returns (address lister) {
    ...
    function predictDeterministicAddress(bytes32 salt) external view returns (address lister) {
        lister = LibClone.predictDeterministicAddress(PRE_APPROVE_LISTER_IMPLMENTATION, salt, address(this));
    }
