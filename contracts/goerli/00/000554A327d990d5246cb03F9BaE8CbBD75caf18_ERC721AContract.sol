...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    mapping(address => uint16) public referrals;
    ...
    function checkClaimEligibility(address _address, uint256 _quantity) public view returns (string memory) {
        ...
        if (token.preSaleIsActive || !token.pubSaleIsActive) return "NOT_LIVE";
        if (!token.preSaleIsActive && token.pubSaleIsActive) return "";
    }
    ...
    function setReferralFee(uint256 _fee) public onlyOwner {
        referralFee = _fee;
    }
    ...
    function updateSaleState(
        ...
        if (_preSaleIsActive && _preSalePhase == 1) require(mintpassAddress != address(0), "MintPass undefined.");
    ...
    function mint(
        ...
        uint256 _preSalePrice = token.preSalePrice;
        ...
        if(_preSaleIsActive) {
            require(_preSalePrice * _quantity <= msg.value, "ETH incorrect");
            if (token.preSalePhase == 1) {
                isAllowed = mintpass.balanceOf(msg.sender, mintpassId) >= _quantity;
                if (isAllowed) mintpass.burnForAddress(mintpassId, _quantity, msg.sender);
            }
            if (token.preSalePhase == 2) {
                ...
                isAllowed = MerkleProof.verify(_proof, saleMerkleRoot, leaf);
                if (isAllowed) hasMinted[msg.sender] = mintedAmount;
            }
        }
        require(isAllowed || _pubSaleIsActive, "Not allowlisted");
        if (!isAllowed) {
            require(_pubSaleIsActive, "Public sale inactive");
            require(_pubSalePrice * _quantity <= msg.value, "ETH incorrect");
            require(_quantity <= _maxPerTransaction, "Invalid quantity");
        }
        _safeMint(msg.sender, _quantity);
        _payReferral(msg.sender, _referrer, _quantity);
    }

    function _payReferral(address _recipient, address payable _referrer, uint256 _quantity) internal {
        if (_referrer != address(0) && _referrer != _recipient) {
            emit Referral(_referrer, _quantity, referralFee * _quantity);
            referrals[_referrer] += uint16(_quantity);
            payable(_referrer).transfer(referralFee * _quantity);
        }
    }

    function fiatMint(address _address, uint256 _quantity, address payable _referrer, uint256 _value) internal {
        require(_address != _referrer, "Referrer same as sender");
        require(token.pubSaleIsActive, "Presale is not active.");
        ...
        _payReferral(_address, _referrer, _quantity);
    }

    function crossmint(address _address, uint256 _quantity, address payable _referrer) public payable nonReentrant onlyCrossmint {
        fiatMint(_address, _quantity, _referrer, msg.value);
    }

    function paper(address _address, uint256 _quantity, address payable _referrer, bytes32 _nonce, bytes calldata _signature) public payable 
        nonReentrant onlyPaper(keccak256(abi.encode(_address, _quantity, _referrer)), _nonce, _signature)
    {
        fiatMint(_address, _quantity, _referrer, msg.value);
    }
