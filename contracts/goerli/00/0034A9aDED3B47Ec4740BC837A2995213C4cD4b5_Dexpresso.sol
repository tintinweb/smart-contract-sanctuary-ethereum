...
import './interface/IDexpresso.sol';
import './interface/IAccessControler.sol';
import '@openzeppelin/contracts/token/ERC20/IERC20.sol';
import '@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol';
import '@openzeppelin/contracts/security/Pausable.sol';
import '@openzeppelin/contracts/utils/cryptography/SignatureChecker.sol';

/// @title A semi-decentralized exchange
/// @author Dexpresso team
/// @notice Only a group of preApproved addresses are allowed to swap assets directly from the contract
/*
...
contract Dexpresso is IDexpresso, Pausable {
     ...
     * set MaxFeeRatio, unpause.
     */
    address private daoMultiSig;
    address private candidatedDaoMultiSig;

    uint16 public maxFeeRatio;

    // status codes
    // SUCCESSFULL SWAP(S) 200 (OK)
    ...
    // Fee Comparison ERROR (FC) 416 (Range Not Satisfiable)
    ...
    uint256 private constant FEE_RATIO_DENOMINATOR = 1000;

    /// @dev brokersAdresses are the only addresses that are allowed to call the executeSwap function
    mapping(address => bool) public brokersAdresses;
    ...
    /// @dev Emitted when the timeToGo function is called
    event transferredAssets(address[]);

    // Modifiers

    modifier isBroker() {
        require(brokersAdresses[msg.sender], 'ERROR: unauthorized caller');
        _;
    }
    ...
    /// @dev isDaoMember, using the IAccessControler checks to see if the caller is one the listed DAOmember of the DAOMultiSig
    /// @dev approvers are the only addresses that are allowed to call the following functions: addBroker, removeBroker, pause
    modifier isDaoMember() {
        require(IAccessControler(daoMultiSig).IsDaoMember(msg.sender), 'ERROR: unauthorized caller');
        _;
    }
    ...
    /**
     * @dev Sets the values for {MaxFeeRatio} and {DAOMultiSig} and {brokersAdresses} mapping.
     *
     *
     */
    constructor(uint16 feeRatio, address _daoMultiSig, address[] memory _brokers) {
        maxFeeRatio = feeRatio;
        daoMultiSig = _daoMultiSig;

        for (uint256 i = 0; i < _brokers.length; ) {
            brokersAdresses[_brokers[i]] = true;
     ...
     * @dev SwapExecuted event is emited with the batchExecuteStatus array in the executeSwap function,
    ...
    function executeSwap(MatchedOrders[] calldata matchedOrders) external virtual whenNotPaused isBroker {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            uint256 takerFee = (matchedOrder.makerTotalSellAmount * matchedOrder.takerFeeRatio) / FEE_RATIO_DENOMINATOR;
            uint256 makerFee = (matchedOrder.takerTotalSellAmount * matchedOrder.makerFeeRatio) / FEE_RATIO_DENOMINATOR;

            bool isMakerTransactionFeasible = _transferFeasibility(
                matchedOrder.makerUserAddress,
                matchedOrder.makerSellTokenAddress,
                matchedOrder.makerTotalSellAmount
            );
            bool isTakerTransactionFeasible = _transferFeasibility(
                matchedOrder.takerUserAddress,
                matchedOrder.takerSellTokenAddress,
                matchedOrder.takerTotalSellAmount
            );

            if (!(isMakerTransactionFeasible && isTakerTransactionFeasible)) {
     ...
     *
     * @param _newFeeRatio uint256 is the new fee to be set.
     *
     */
    ...
    /**
     * @notice proposeToUpdateDaoMultiSig function handle the update of the daoMultiSig address,
     *
     * @dev the new daoMultiSig cannot be the same as the last one,
     * @dev msg.sender must be the previous daoMultiSig contract,
     *
     * @param _newDaoMultiSig address is the new candidate of being daoMultiSig address.
     *
     */
    function proposeToUpdateDaoMultiSig(address payable _newDaoMultiSig) external whenNotPaused isDaoMultiSig {
    ...
    function updateDaoMultiSig() external whenNotPaused {
        require(candidatedDaoMultiSig != address(0), 'ERROR: no proposal');
    ...
    /**
     * @notice addBroker function sets an address to True in brokersAdresses mapping
     *
     * @dev msg.sender must be a DAO member,
     *
     * @param _broker address is the latest broker in the brokersAdresses mapping which will be mapped to True.
     *
     */
    function addBroker(address _broker) external whenNotPaused isDaoMember {
        brokersAdresses[_broker] = true;
    }

    /**
     * @notice removeBroker function sets an address to False in brokersAdresses mapping
     *
     * @dev msg.sender must be a DAO member,
     *
     * @param _broker address is the  broker in the brokersAdresses mapping which will be mapped to False.
     *
     */
    function removeBroker(address _broker) external whenNotPaused isDaoMember {
        brokersAdresses[_broker] = false;
    }

    // puase and unpause functions
    /**
     * @notice puase function, transfers all the given tokens balances to the DAOMultiSig contract and triggers stopped state,
     *
     * @dev Paused event is emitted with the list of tokens,
     * @dev msg.sender must be the DAOMultiSig member,
     *
     * @param tokenAddresses is the token list to be transfered to the DaoMultisig contract,
     *
     */
    function puase(address[] memory tokenAddresses) external whenNotPaused isDaoMember {
    ...
    /**
     * @notice unpause function, Returns the contract to normal state,
     *
     * @dev msg.sender must be the DAOMultiSig,
     */
    ...
    /**
     * @dev ValidateTransaction function compares the users balance and allowance against the amounts to be transfered,
     * to see if the transaction is possible.
     *
     ...
     *
     *@return A boolean dictating if the transaction is possible or not.
     */
    ...
    /**
     * @dev _createMessageHash function hashes the data that user signed to be later used in validateUserSignature function,
     *
     * @param _messageParameters(from MessageParameters struct) contains the data that a user signed while placing on order.
     *
     */
     ...
     *
     * @param _userAddress is the address of the user whos signature is being validated,
