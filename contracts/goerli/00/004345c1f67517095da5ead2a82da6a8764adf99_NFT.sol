...
//--------------------- Set & Change anythings
    ...
    function setPBRoot (bytes32 newPBRoot) public onlyOwner {
        PBroot = newPBRoot ;
    }
...
//-------------------- END WhitelistMint
//-------------------- PublicMint
    function mintPublic(uint256 _mintAmount ,bytes32[] memory _Proof,bytes32 _Leaf) external payable callerIsUser {

        require(mintPhase == 5, "public sale hasn't begun yet");
        // require(pvseed == getpvseed, "public sale hasn't begun yet (wrong seed).");
        
        require(
            MerkleProof.verify(_Proof, PBroot, _Leaf),
