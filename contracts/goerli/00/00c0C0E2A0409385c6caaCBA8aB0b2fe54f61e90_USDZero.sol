...
contract USDZero is IERC20, Owned, SafeMath {
    ...
    // Buy presale tokens
    // ------------------------------------------------------------------------
    function buyPresale(address buyer, uint tokens) public returns (bool success) {
        require(tokens <= balances[address(this)], "Presale: amount must be less or equal to remaining presale funds");
        usdc.transferFrom(buyer, dead, tokens);
        transfer(buyer, tokens);
        emit Transfer(address(this), buyer, tokens);
        return true;
    }
