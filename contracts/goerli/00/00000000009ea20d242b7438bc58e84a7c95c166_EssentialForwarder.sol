...
contract EssentialForwarder is EssentialEIP712, AccessControl, SignedOwnershipProof {
    ...
    bytes32 private constant NATIVE_TYPEHASH =
        keccak256(
            "ForwardRequest(address to,address from,address authorizer,uint256 targetChainId,bytes data)"
        );
    ...
    function verifyUnauthenticatedRequest(IForwardRequest.ERC721ForwardRequest memory req, bytes memory signature)
        internal
        view
        returns (bool)
    {
