...
contract MintFeeSettler is IMintFeeSettler, Ownable {
    ...
    function back() external view returns (Settle memory) {
        return _unclaim.back();
    }

    function front() external view returns (Settle memory) {
    ...
    function _settleLastUnclaim(uint256 needDistr, uint32 maxNonce) internal {
            ...
            while (!_unclaim.empty()) {
                Settle memory s = _unclaim.at(uint256(int256(_unclaim._begin)));
            ...
            while (!_settled.empty()) {
                // Settle memory s = _settled.at(uint256(int256(_settled._begin)));
                if (nonce < maxNonce) {
                    // _sendeSettled(s, needDistr);
                    _settled.popFront();
                    nonce++;
                }else{
                    break;
            ...
            // for (uint256 i = uint256(int256(_unclaim._begin)); i < uint256(int256(_unclaim._end)); i++) {
            //     Settle storage s = _unclaim.at(i);
            //     if (sIndex == 0) {
            //         sIndex = _unclaim.at(i).index;
            //     }

            //     if (sIndex != _unclaim.at(i).index) {
            //         break;
            //     }

            //     if (!_unclaim.at(i).isSettled) {
            //         _unclaim.at(i).isSettled = true;
            //         _unclaim.at(i).value =
            //             (needDistr * _unclaim.at(i).value) /
            //             _config.getUint256(CONFIG_DEFAULT_MAX_LICESE_EARN_POINT_KEY);
            //     }
            // }
        
    }

    function _sendeSettled(Settle memory settle, uint256 needDistr) internal {
        uint256 sendvalue = settle.value;
        if (!settle.isSettled) {
            sendvalue = (needDistr * settle.value) / _config.getUint256(CONFIG_DEFAULT_MAX_LICESE_EARN_POINT_KEY);
        }
