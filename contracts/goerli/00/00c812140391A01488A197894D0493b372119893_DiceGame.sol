...
contract P2PGame is Ownable, BaseGame {
    ...
    function endGame(
        ...
        if (winner == address(0x0)) {
            emit GameOver(gameId, 2, address(0x0), game.results);
            return;
        }
        ...
        emit GameOver(gameId, 1, winner, game.results);
    }
    ...
    function emergencyEndGame(bytes32 gameId) external virtual override onlyOwner {
        ...
        emit GameOver(gameId, 0, address(0x0), game.results);
    }
...
contract BaseGame is Ownable, IGame {
    ...
    event GameOver(bytes32 indexed gameId, uint256 status, address winner, uint256[] results);
