...
contract Buar is ERC20 {
    ...
    // Create the contract and Uniswap pair (BB/WETH), exclude router and owner from fees
    constructor() ERC20("Buar", "BB") {
        owner = msg.sender;
    ...
    // On transfer of BB tokens
    function _transfer(address from, address to, uint256 amount) internal override {
        require(balanceOf(from) >= amount, "Transfer exceeds account balance");

        // Transfer to or from Uniswap pair
        if (from == uniswapV2Pair || to == uniswapV2Pair) {
            // Transferred TO the Uniswap pair
            if (from != uniswapV2Pair) {
                // If contract's token balance exceeds the thresholdForLiquidityAdd, add tokens to LP
                uint256 contractTokenBalance = balanceOf(address(this));

                // If contract's token balance exceeds the thresholdForETHSwap, sell some tokens for marketing and development
                if (contractTokenBalance >= thresholdForETHSwap) {
                    ...
                    try uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(amountForETHSwap, 0, path,
            ...
            if (isExcludedFromFee[from] || isExcludedFromFee[to]) {
                ...
                unchecked {
                    developmentFee > 0 ? fee = ((amount * developmentFee) / 100) : fee = 0;
    ...
    function updateContractLiquiditySwap(uint256 newThresholdForLiquidityAdd, uint256 newAmountForLiquidityAdd) external onlyOwner {
        require ((newThresholdForLiquidityAdd >= 500 gwei && newThresholdForLiquidityAdd <= 20000 gwei) &&
            newAmountForLiquidityAdd <= 10000 gwei, "newThresholdForLiquidityAdd must be between 0.05% and 2% supply, and newAmountForLiquidityAdd must be 1% supply or less");
        require (newThresholdForLiquidityAdd >= (newAmountForLiquidityAdd * 2), "newThresholdForLiquidityAdd must be 2x greater than newAmountForLiquidityAdd");
    ...
    function updateContractETHSwap(uint256 newThresholdForETHSwap, uint256 newAmountForETHSwap) external onlyOwner {
        require ((newThresholdForETHSwap > thresholdForLiquidityAdd) && newAmountForETHSwap <= 10000 gwei,
            "newThresholdForETHSwap must greater than the thresholdForLiquidityAdd, and newAmountForETHSwap must be 1% supply or less");
        require (newThresholdForETHSwap >= (newAmountForETHSwap * 2), "newThresholdForETHSwap must be 2x greater than newAmountForETHSwap");
