...
contract ReviverArtEditionSale is Ownable, ReentrancyGuard {
    uint256 private constant YinTokenID = 3;
    uint256 private constant YangTokenID = 4;

    uint256 public constant YinPrice = 0.00001 ether;
    uint256 public constant YangPrice = 0.00001 ether;
    ...
    // set time
    uint64 public immutable ALStartTime = 1576563200; // 2023-02-17 00:00:00 GMT+8
    uint64 public immutable ALEndTime = 1976692800; // 2023-02-18 12:00:00 GMT+8
    uint64 public immutable WLStartTime = 1576736000; // 2023-02-19 00:00:00 GMT+8
    uint64 public immutable WLEndTime = 1976822400; // 2023-02-20 00:00:00 GMT+8
        ...
        nonReentrant
    {
        require(
            YinALMinted[msg.sender] + amount <= maxAmount &&
                YinEditionMinted + amount <= MaxMintAmount,
            "exceed max amount"
        );
        ...
        nonReentrant
    {
        require(
            YangALMinted[msg.sender] + amount <= maxAmount &&
                YangEditionMinted + amount <= MaxMintAmount,
            "exceed max amount"
        );
        address[] memory addr = new address[](1);
        uint256[] memory tokenID = new uint256[](1);
    ...
    function adminMintYinEdition(uint256 n) public onlyOwner nonReentrant {
        ...
        require(n + YinEditionMinted <= MaxMintAmount, "exceed max amount");
        address[] memory addr = new address[](1);
        uint256[] memory tokenID = new uint256[](1);
        uint256[] memory mintAmount = new uint256[](1);
        addr[0] = msg.sender;
    ...
    function adminMintYangEdition(uint256 n) public onlyOwner nonReentrant {
        ...
        require(n + YangEditionMinted <= MaxMintAmount, "exceed max amount");
        address[] memory addr = new address[](1);
        uint256[] memory tokenID = new uint256[](1);
        uint256[] memory mintAmount = new uint256[](1);
        addr[0] = msg.sender;
    ...
    function adminMintYinEditionTest(uint256 n) public onlyOwner nonReentrant {
        address[] memory addr = new address[](1);
        uint256[] memory tokenID = new uint256[](1);
        uint256[] memory mintAmount = new uint256[](1);
        addr[0] = msg.sender;
