    ...
    ReentrancyGuardUpgradeable
{
    ...
    function atomicMatchAndDeposit(
        ...
        // 1. Conduct the atomic match
        atomicMatch(transactionType, _order, seller, _sellerMetadata, sellerSig, buyer, _buyerMetadata, buyerSig);

        // 2. Transfer to the corresponding manager's address
        (address tokenAddress, uint256 tokenId) = _getTokenInfo(_order);
        address managerAddress = IGateway(gateway).nftManager(tokenAddress);
        address buyerRecipient = _getBuyerRecipient(_buyerMetadata);
        IERC721(tokenAddress).safeTransferFrom(buyerRecipient, managerAddress, tokenId);
    }
    ...
    function _getTokenInfo(bytes memory _order)
        internal
        pure
        returns (address, uint256)
    {
        Order memory order = decodeOrder(_order);
        return (order.targetTokenAddress, order.targetTokenId);
    }

    function _getBuyerRecipient(bytes memory _metadata)
        internal
        pure
        returns (address)
    {
        OrderMetadata memory buyerMetadata = decodeOrderMetadata(_metadata);
        return buyerMetadata.recipient;
    }
}