...
contract Confer is Ownable {
    ...
    enum State { Inactive, Created, Locked, Release, Finished}
    State public state;
    ...
    error StudentOnly();
     ...
     /* Add more of these to get more specific errors */
    //// This function cannot be called in the current transaction state. */
    error InvalidState();


    /// The amount deposited must be an even number.
    error ValueNotEven();
   ...
   function getBalance() external view returns (uint256) {
    ...
    function confirmReceived()external onlyStudent inState(State.Locked) {
        state = State.Release;
        student.transfer(rate);
    }

    function paySeller() external onlyOwner inState(State.Release) {
