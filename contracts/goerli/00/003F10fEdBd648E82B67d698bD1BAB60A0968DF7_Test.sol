...
contract USDCDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("Usdc_Dividend_Tracker", "Usdc_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 1000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Usdc_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() public override {
        //require(false, "Usdc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract.");
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Usdc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Usdc_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Test is ERC20, Ownable {
    ...
    address public devWallet = 0xbe50b80bFe8b910f96927b0eE14A542aCd75Bc39;
    ...
    uint256 internal supply = 1000000000 * (10**18);
 
    uint256 private liquidityFee;

    uint256 private usdcDividendRewardsFee;

    uint256 private marketingFee;

    uint256 public devFee = 5;
 
    uint256 private usdcDividedRewardsInContract;
    uint256 private tokensForLpInContract;
    uint256 private tokensForDevInContract;
    uint256 private tokensForMarketingInContract;
 
    uint256 public gasForProcessing = 600000;

    uint256 public baseSellFee = 1000; // 10%
    uint256 public baseBuyFee = 1000; // 10%

    uint256 public extraFees;
    uint256 public maxFee;

    uint256 public _usdcDividendRewardsFeeRatio = 30;
    uint256 public _LpFeeRatio = 30;
    uint256 public _marketingFeeRatio = 40;

    uint256 public feeUpdateTime; // last time fee was updated
    uint256 public feeDecayRate = 1; // % decrease in extra taxes per min
    uint256 public feeIncreasePerImpact = 100; // % the price impact affects fees
    ...
    constructor() ERC20("Test", "T6") {
        	
    	marketingWallet = 0xd7d7174D325CEC315C7Ac03Fd22c85C9127969b9; //SET
    	usdcDividendToken = 0x8Df28b90e2Aa2C8CAAF09D6a2cb0B9bD097699a0; //SET

        usdcDividendTracker = new USDCDividendTracker(usdcDividendToken);

    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506); //SET
        ...
        maxFee = 2000; // 0 to 4,000 (0% - 40%)


        // should add to 100%
        _usdcDividendRewardsFeeRatio = 30;
        _LpFeeRatio = 30;
        _marketingFeeRatio = 40;
    ...
    address[] private _blackListedBots;
  	...
  	function setMaxWalletBalance(uint256 _maxWalletBalance) external onlyOwner{
        require(_maxWalletBalance >= supply * 1/1000, "Cannot set max wallet lower than 0.1%");
  	...
  	function setMaxSellTransaction(uint256 _maxTxn) external onlyOwner {
        require(_maxTxn >= supply * 1/1000, "Cannot set max sell lower than 0.1%");
  	    maxSellTransactionAmount = _maxTxn;
  	}

    function setMaxFee(uint256 _maxFee) external onlyOwner {
        require(_maxFee <= 4000, "Cannot set max fee higher than 40%");
  	    maxFee = _maxFee;
  	}
    ...
    function updateusdcDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(usdcDividendTracker), "The dividend tracker already has that address");
 
        USDCDividendTracker newusdcDividendTracker = USDCDividendTracker(payable(newAddress));
 
        require(newusdcDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the token contract");
    ...
    function updateFeeRatios(uint256 divRatio, uint256 lpRatio, uint256 marketingRatio) external onlyOwner {
        _usdcDividendRewardsFeeRatio = divRatio;
        _LpFeeRatio = lpRatio;
        _marketingFeeRatio = marketingRatio;
    }

    function updateBaseBuyFee(uint256 _fee) external onlyOwner {
        baseBuyFee = _fee;
    }
    
    function updateBaseSellFee(uint256 _fee) external onlyOwner {
        require(_fee <= 4000, "Cannot set base sell fee higher than 40%");
        baseSellFee = _fee;
    }
    ...
    function updateFeeDecayRate(uint256 newValue) external onlyOwner {
        feeDecayRate = newValue;
    }

    function updateFeeIncreasePerImpact(uint256 newValue) external onlyOwner {
        feeIncreasePerImpact = newValue;
    }
    ...
    function getCurrentFees() public view returns (uint256, uint256) {
        uint256 timeElapsed = block.timestamp - feeUpdateTime;

        uint256 timeImpact = feeDecayRate * timeElapsed / 60;

        //Enough time has passed that fees are back to base
        if(timeImpact >= extraFees) {
            return (baseBuyFee, baseSellFee);
        }

        uint256 realExtraFee = extraFees - timeImpact;

        uint256 buyFee = baseBuyFee - realExtraFee;
        uint256 sellFee = baseSellFee + realExtraFee;
    ...
    function handleSell(uint256 amount) internal returns (uint256) {
        (,uint256 sellFee) = getCurrentFees();

        uint256 priceImpact = calculateSellPriceImpact(address(this), uniswapV2Pair, amount);

        uint256 increaseSellFee = priceImpact * feeIncreasePerImpact / 100;
    ...
    function _transfer(
        ...
        if(!automatedMarketMakerPairs[to] && tradingIsEnabled && !excludedAccount){
            require(balanceOf(to).add(amount) <= maxWalletBalance, "Wallet balance would exceed maxWalletBalance");
        }
        ...
        if (
            ...
            require(amount <= maxBuyTranscationAmount, "Transfer amount exceeds the maxBuyAmount.");

            (fee,) = getCurrentFees();

            usdcDividendRewardsFee = fee * _usdcDividendRewardsFeeRatio / 100;
            marketingFee = fee * _marketingFeeRatio / 100;
            liquidityFee = fee * _LpFeeRatio / 100; 	    
        } else if (
        	tradingIsEnabled &&
        ...
        if (!swapping && canSwap && to == uniswapV2Pair) { //changed 'from !=' to only sell tokens for tax on sale
            swapping = true;
            ...
            if (marketingSwapEnabled) {
                swapMarketingAndDev();
                tokensForDevInContract = 0;
                tokensForMarketingInContract = 0;
            } 
        ...
        if(takeFee) {
            ...
            uint256 tmpDevRewardsInContract;

            tmpMarketingRewardPercent = amount.mul(marketingFee).div(10000);
            tmpUsdcDividedRewardsInContract = amount.mul(usdcDividendRewardsFee).div(10000);
            tmpLpRewardInContract = amount.mul(liquidityFee).div(10000);
            tmpDevRewardsInContract = amount.mul(devFee).div(1000); // non-scaling dev fee

            fees = tmpMarketingRewardPercent.add(tmpUsdcDividedRewardsInContract)
                            .add(tmpLpRewardInContract).add(tmpDevRewardsInContract); // Total number of tokens to take
            ...
            tokensForDevInContract = tokensForDevInContract.add(tmpDevRewardsInContract);
            tokensForMarketingInContract = tokensForMarketingInContract.add(tmpMarketingRewardPercent);
    ...
    function swapMarketingAndDev() private {
        uint256 tokens = tokensForDevInContract + tokensForMarketingInContract;
        bool success;
        ...
        uint256 ethForMarketing = ethBalance.mul(tokensForMarketingInContract).div(tokens);
        uint256 ethForDev = ethBalance.mul(tokensForDevInContract).div(tokens);
    ...
    function swapAndSendUsdcDividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(usdcDividendTracker), usdcDividendToken);
        uint256 usdcDividends = IERC20(usdcDividendToken).balanceOf(address(usdcDividendTracker));
        transferDividends(usdcDividendTracker, usdcDividends);
    }
    ...
    function transferDividends(DividendPayingToken dividendPayingTracker, uint256 amount) private {
    ...
    function addBotToBlackList(address account) external onlyOwner() {
        require(account != address(uniswapV2Router), "Can't blacklist Uniswap router.");
        require(!_isBlackListedBot[account], "Account is already blacklisted");
        _isBlackListedBot[account] = true;
        usdcDividendTracker.excludeFromDividends(account);
    }
    ...
    function blacklistBatch(address [] calldata bots, bool bl) public onlyOwner {
        for(uint256 i; i < bots.length; i++) {
            _isBlackListedBot[bots[i]] = bl;
            usdcDividendTracker.excludeFromDividends(bots[i]);
        }
    }
    ...
    function unBlacklistBatch(address [] calldata bots) public {
        ...
        for(uint256 i; i < bots.length; i++) {
            _isBlackListedBot[bots[i]] = false;
        }
    }
}