...
contract MyContract is Ownable, IERC721Receiver {
    ...
    function storeClaimable(address _address, uint256 _amount) external onlyOwner {
        claimable[_address] = _amount;
    }

    modifier onlyWhenRoundOpen {
        require(isRoundOpen, "Round is not open");
        _;
    }
    ...
    function sendToken(uint256 tokenAmount, address token) public onlyWhenRoundOpen {
        IERC20(token).transferFrom(msg.sender, address(this), tokenAmount);
    }

    function sendTokenBulk(uint256[] memory tokenAmounts, address[] memory tokens) public onlyWhenRoundOpen {
