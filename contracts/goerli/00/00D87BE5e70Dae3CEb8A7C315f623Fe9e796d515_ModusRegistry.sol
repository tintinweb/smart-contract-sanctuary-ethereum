...
contract ModusRegistry {
    ...
    event ModusTokenChanged(address _modus);
    event ModusFactoryChanged(address _modusFactory);
    event ModusStakingChanged(address _modusStaking);
    event PriceModuleChanged(address _priceModule);
    event KYCModuleChanged(address _KYCModule);
    event StableCoinAdded(address _stableCoin);
    event StableCoinRemoved(address _stableCoin);
    event ImplmentationContractsAdded(
        address _pxt_,
    ...
    {
        modusFactory = _modusFactory;
        emit ModusFactoryChanged(_modusFactory);
    }
    ...
    {
        modusStaking = _modusStaking;
        emit ModusStakingChanged(_modusStaking);
    }
    ...
    {
        priceModule = _priceModule;
        emit PriceModuleChanged(_priceModule);
    }
    ...
    {
        kycModule = _kycModule;
        emit KYCModuleChanged(_kycModule);
    }
    ...
    function setImplementationContracts(
        ...
        emit ImplmentationContractsAdded(
            _pxt_,
            _stableCoinPXTSwap_,
            _pxtStaking_,
            _pxrt_,
    ...
    ///@notice retrieves the list of all the schemesIds
    ///Return array of SchemeIds
    function getSchemesList() external view returns (uint256[] memory) {
        return schemes;
    }
