...
contract MentalBros is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI, string memory hiddenURI) ERC721("MentalBros", "Mental") {
        setBaseURI(baseURI);
    ...
    function payToMintNFT() public payable {
        uint totalMinted = _tokenIds.current();

        require(totalMinted <= MAX_SUPPLY, "Not enough NFTs left!");
        require(balanceOf(msg.sender) == 0, 'Each address may only own one Mental Bros NFT!');
    ...
    function verifyIfUserAddressIsWhitelisted (address _address) public view returns(bool) {
      IWhitelist whitelist = IWhitelist(whitelistAddress);

      return whitelist.isWhitelisted(_address);
    }

    function getNumberOfWhitelistedAdresses() public view returns(uint256) {
      IWhitelist whitelist = IWhitelist(whitelistAddress);

      return whitelist.getNumberOfWhitelistedAddresses();
    }

    function getMaxNumberOfWhitelistedAddresses() public view returns(uint256) {
      IWhitelist whitelist = IWhitelist(whitelistAddress);

      return whitelist.getMaxNumberOfWhitelistedAddresses();
    }

    function getWhitelistContractOwner() public view returns(address) {
      IWhitelist whitelist = IWhitelist(whitelistAddress);

      return whitelist.getOwner();
    }
...
interface IWhitelist {
    function verifyUserAddress(address _whitelistedAddress) external view returns (bool);
    function isWhitelisted(address _whitelistedAddress) external view returns (bool);
    function getNumberOfWhitelistedAddresses() external view returns (uint256);
    function getMaxNumberOfWhitelistedAddresses() external view returns (uint256);
