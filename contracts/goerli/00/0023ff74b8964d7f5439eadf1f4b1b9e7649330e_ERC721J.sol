...
interface IERC721JUpgradeable {
    ...
    function mintCopy(uint256 tokenId, address to) external payable;
    ...
    //Mints an original using 3 pieces of metadata
    function mintOriginal(
        string memory songURI1,
        string memory songURI2,
    ...
    OwnableUpgradeable
{
    ...
    function mintCopy(uint256 tokenId, address to) public payable {
        //requires eth
        if (msg.value < _mintPrice) revert NotEnoughEther();
        //requires the sender to have the tokenId in their wallet
        if (ownerOf(tokenId) != msg.sender)
    ...
    function mintCopyToken(uint256 tokenId, address token, address to) public {
        //requires token
        if (ERC20Upgradeable(token).balanceOf(msg.sender) < _tokenPrice[token])
            revert NotEnoughEther();
        //checks if contract is approved
