...
contract TherundownThalesConsumer_v3_2 is ChainlinkClient {
    ...
    mapping(bytes32 => bytes[]) public requestIdOdds;
    ...
    function fulfillOdds(bytes32 _requestId, bytes[] memory _odds) external recordChainlinkFulfillment(_requestId) {
        requestIdOdds[_requestId] = _odds;
    }
    ...
    function requestGamesResolveWithFilters(
        ...
        req.addBytes("hasScoresByPeriod", abi.encode(_hasScoresByPeriod));
    ...
    function getOdds(bytes32 _requestId, uint256 _idx) external view returns (GameOdds memory) {
        GameOdds memory odds = abi.decode(requestIdOdds[_requestId][_idx], (GameOdds));
        return odds;
    }
