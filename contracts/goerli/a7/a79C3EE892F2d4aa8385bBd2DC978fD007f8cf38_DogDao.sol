...
contract DogDao is ERC20, Ownable {
    ...
    uint256 public swapAtAmount = _tTotal.mul(10).div(100000);       // 0.10% of total supply
    uint256 public maxTxLimit = _tTotal.mul(50).div(100000);         // 0.50% of total supply
    uint256 public maxWalletLimit = _tTotal.mul(100).div(100000);    // 1.00% of total supply
    ...
    constructor(address _dev) public ERC20("DoggyDao", "DoggyDao") {
        ...
        ExcludeFromDoggyFees(owner(), true);
        ExcludeFromDoggyFees(address(this), true);
    ...
    function ExcludeFromDoggyFees(address account, bool value) public onlyOwner() {
        excludeFromFees(account, value);
        excludeFromTxLimit(account, value);
        excludeFromWalletLimit(account, value);
    }

    function DoggyEntryFee(uint256 liquidityFee, uint256 txFee) external onlyOwner() {
        buyLiquidityFee = liquidityFee;
        buyTxFee = txFee;
    }

    function DoggyDues(uint256 liquidityFee, uint256 txFee) external onlyOwner() {
        sellLiquidityFee = liquidityFee;
        sellTxFee = txFee;
    }

    function MaxTxnLimit(uint256 newLimit) external onlyOwner() {
        maxTxLimit = newLimit * (10**9);
    }

    function MaxWalletLimit(uint256 newLimit) external onlyOwner() {
    ...
    function NewDevWallet(address newWallet) external onlyOwner() {
        dev = newWallet;
    }
    ...
    function LetTehDogsOut() external onlyOwner() {
