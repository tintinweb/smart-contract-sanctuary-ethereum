...
import {Cat} from "./system/cat.sol";
import {Dog} from "./system/dog.sol";
import {SimJoin} from "./system/join.sol";
import {Flapper} from "./system/flap.sol";
import {Flopper} from "./system/flop.sol";
import {Flipper} from "./system/flip.sol";
...
import {Sim} from "./system/sim.sol";
...
contract SimFab {
    function newSim(address owner, uint256 chainId)
    public
    returns (Sim sim)
    {
        sim = new Sim(chainId);
...
contract SimJoinFab {
    function newSimJoin(address vat, address sim)
    public
    returns (SimJoin simJoin)
    {
        simJoin = new SimJoin(vat, sim);
    }
}
...
contract PauseFab {
    function newPause(
        ...
        address owner,
        address authority
    ) public returns (Pause pause) {
        pause = new Pause(delay, owner, authority);
    }
}
...
contract Deployment {
    ...
    SimFab public simFab;
    SimJoinFab public simJoinFab;
    ...
    function deploySim(uint256 chainId) public {
        require(address(vat) != address(0), "Missing previous step");

        sim = simFab.newSim(address(this), chainId);
        simJoin = simJoinFab.newSimJoin(address(vat), address(sim));
        sim.rely(address(simJoin));
    }
    ...
    function deployPause(uint256 delay, address authority) public {
        require(address(sim) != address(0), "Missing previous step");
...
contract Pause is Auth {
    ...
    function addd(uint256 x, uint256 y) internal pure returns (uint256 z) {
        z = x + y;
        require(z >= x, "pause-addition-overflow");
    }
...
/*
    ...
    adapters here are provided as working examples:
      ...
      - `SimJoin`: For connecting internal sim balances to an external
...
contract SimJoin {
    ...
    VatLike public vat; // CDP Engine
    TokenLike public sim; // Stablecoin Token
    uint256 public live; // Active Flag
    ...
    constructor(address vat_, address sim_) public {
        wards[msg.sender] = 1;
        live = 1;
        vat = VatLike(vat_);
        sim = TokenLike(sim_);
    ...
    function join(address usr, uint256 wad) external {
        ...
        sim.burn(msg.sender, wad);
        emit Join(usr, wad);
    }

    function exit(address usr, uint256 wad) external {
        require(live == 1, "SimJoin/not-live");
...
/// sim.sol -- IRDT Stablecoin ERC-20 Token
...
contract Sim {
    ...
    // --- ERC20 Data ---
    string public constant name = "Sim Stablecoin";
    ...
    function transferFrom(
        ...
        require(balanceOf[src] >= wad, "Sim/insufficient-balance");
        if (src != msg.sender && allowance[src][msg.sender] != uint256(-1)) {
            require(
                allowance[src][msg.sender] >= wad,
                "Sim/insufficient-allowance"
            );
    ...
    function burn(address usr, uint256 wad) external {
        require(balanceOf[usr] >= wad, "Sim/insufficient-balance");
    ...
    function permit(
        ...
        require(holder != address(0), "Sim/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "Sim/invalid-permit");
        require(expiry == 0 || now <= expiry, "Sim/permit-expired");
        require(nonce == nonces[holder]++, "Sim/invalid-nonce");
