...
interface IMetriaAddressRegistry {
    function metria() external view returns (address);
...
interface IMetriaBundleMarketplace {
 ...
 */
contract MetriaAuction is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /// @notice Event emitted only on construction. To be used by indexers
    event MetriaAuctionContractDeployed();
    ...
    function initialize(address payable _platformFeeRecipient) public {
        require(
            _platformFeeRecipient != address(0),
            "MetriaAuction: Invalid Platform Fee Recipient"
        );

        platformFeeRecipient = _platformFeeRecipient;
        emit MetriaAuctionContractDeployed();
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IMetriaTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
 ...
 */
        ...
        nonReentrant
    {
        ...
        IMetriaMarketplace marketplace = IMetriaMarketplace(
        ...
        IMetriaBundleMarketplace(addressRegistry.bundleMarketplace())
