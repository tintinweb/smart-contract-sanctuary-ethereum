...
contract HyewonNft is IHyewonNft, ERC721A, Ownable, ReentrancyGuard, Pausable {
    ...
    /**
     *  Record whitelist for each round
     */
    ...
    modifier onlyOwnerOrAdmin(uint16 roundNumber) {
        ...
        if (_msgSender() != owner() && (admin == address(0) || _msgSender() != admin)) {
        ...
        whenNotPaused
    {
        ...
        MintingType mintingType = currentRound.mintingType;

        if (mintingType == MintingType.WHITELISTED_ONLY) {
            if (!whitelists[currentRoundNumber][_msgSender()]) {
                revert AddressNotWhitelisted(_msgSender());
            }
        } else if (mintingType == MintingType.ADMIN_ONLY) {
            if (_msgSender() != owner() && _msgSender() != currentRound.admin) {
                revert NotOwnerNorAdmin(_msgSender());
            }
        }
        ...
        if (mintingType == MintingType.WHITELISTED_ONLY) {
           whitelists[currentRoundNumber][_msgSender()] = false;
        }
    ...
    function changeMintingType(uint16 roundNumber, MintingType mintingType) 
        external onlyOwnerOrAdmin(roundNumber) {
    ...
    /**
     *  Add addresses to whitelist of round
     */
    function addToWhitelist(uint16 roundNumber, address[] calldata addressesToAdd) external onlyOwnerOrAdmin(roundNumber) {
        for (uint i = 0; i < addressesToAdd.length; i++) {
            whitelists[roundNumber][addressesToAdd[i]] = true;
        }
    }

    /**
     *  Remove addresses from whitelist of round
     */
    function removeFromWhitelist(uint16 roundNumber, address[] calldata addressesToRemove) external onlyOwnerOrAdmin(roundNumber) {
        for (uint i = 0; i < addressesToRemove.length; i++) {
            delete whitelists[roundNumber][addressesToRemove[i]];
        }
    }
        ...
        private
    {
        ...
        if (
            currentRound.maxMintingQuantity != 0 &&
            currentRound.maxMintingQuantity < quantity
        ) {
        ...
        // check if proper fee is received (except AdminOnly round)
