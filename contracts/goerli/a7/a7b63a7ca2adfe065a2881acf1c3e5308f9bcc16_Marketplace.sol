    ...
    ReentrancyGuardUpgradeable
{
    ...
    function cancel(LibOrder.Order memory order) external nonReentrant {
        require(order.maker != address(0), "not address 0");
    ...
    function matchBatchOrders(LibOrder.BatchOrder[] memory batchOrder)
        ...
        nonReentrant
    {
        for (uint8 i = 0; i < batchOrder.length; i++) {
            require(
                batchOrder[i].orderLeft.dataType == bytes4(keccak256("V1")),
                "Invalid match order"
            );
            validateFull(batchOrder[i].orderLeft, batchOrder[i].signatureLeft);
            validateFull(
                batchOrder[i].orderRight,
                batchOrder[i].signatureRight
            );
            ...
            // matchAndTransfer(batchOrder[i].orderLeft, batchOrder[i].orderRight);
        }
    }
...
import "./lib/LibERC721Data.sol";
import "./lib/LibERC1155Data.sol";
import "./lib/LibFill.sol";
...
abstract contract TransferManager is OwnableUpgradeable, ITransferManager {
    ...
    function encode721Lazy(
        address token,
        LibERC721Data.ERC721Data memory data
    ...
    function encode1155Lazy(
        address token,
        LibERC1155Data.ERC1155Data memory data
    ) external pure returns (bytes memory) {
        return abi.encode(token, data);
    }
        ...
        returns (LibPart.Part[] memory)
    {
        if (
            ...
            ( , LibERC1155Data.ERC1155Data memory data) = abi.decode(
                matchNft.data,
                (address, LibERC1155Data.ERC1155Data)
            );
            ...
            ( , LibERC721Data.ERC721Data memory data) = abi.decode(
                matchNft.data,
                (address, LibERC721Data.ERC721Data)
            );
...
library LibOrderData {
    ...
    {
        if (order.dataType == LibOrderData.V1 || order.dataType == LibOrderData.V2) {
...
import "./transfer-proxy/interfaces/INftTransferProxy.sol";
import "./transfer-proxy/interfaces/IERC20TransferProxy.sol";
    ...
    ITransferExecutor
{
    ...
    function transfer(
        ...
        if (asset.assetType.assetClass == LibAsset.ETH_ASSET_CLASS) {
            ...
            (address token, LibERC721Data.ERC721Data memory data) = abi
                .decode(
            ...
            (address token, LibERC1155Data.ERC1155Data memory data) = abi
                .decode(
                    asset.assetType.data,
                    (address, LibERC1155Data.ERC1155Data)
                );
...
import "../../../marketplace/lib/LibAsset.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC721/IERC721Upgradeable.sol";
...
library LibERC721Data {
    ...
    bytes32 public constant TRANSFER_ERC721_TYPEHASH =
        keccak256(
            "TransferERC721(uint256 tokenId,Part[] royalties)Part(address account,uint96 value)"
        );

    function hash(ERC721Data memory data) internal pure returns (bytes32) {
...
library LibERC1155Data {
    ...
    bytes32 public constant TRANSFER_ERC1155_TYPEHASH =
        keccak256(
            "TransferERC1155(uint256 tokenId,uint256 supply,Part[] royalties)Part(address account,uint96 value)"
        );

    function hash(ERC1155Data memory data) internal pure returns (bytes32) {
        bytes32[] memory royaltiesBytes = new bytes32[](data.royalties.length);
        for (uint256 i = 0; i < data.royalties.length; i++) {
            royaltiesBytes[i] = LibPart.hash(data.royalties[i]);
        }
