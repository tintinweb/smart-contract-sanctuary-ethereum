...
contract TEST is ERC721A("TEST", "TEST"), Ownable {
    ...
    function mint(address to, bytes32[] calldata _merkleProof) public payable {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    ERC721A.tokenURI(getLevel(ownerOf(tokenId))),
                    baseExtension
                )
            );
    }
    ...
    function getLevel(address _owner) public view returns (uint256) {
        uint256 balance = BalanceOfInterface(MIA_CONTRACT_ADDRESS).balanceOf(
            _owner
        );
