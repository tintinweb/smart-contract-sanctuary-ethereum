...
//    mapping(address => mapping(uint256 => uint256)) public mintCount;
    ...
    function publicMint(
        ...
        if (phase == SalePhase.ALSale) {
            require(
                balanceOf(msg.sender, id) + _mintAmount <= _alAllocated,
                "Exceeds your allocation"
            );
        } else if (phase == SalePhase.FCFSSale) {
            require(
                balanceOf(msg.sender, id) + _mintAmount <= _alAllocated + maxMintForFCFS,
                "Exceeds your allocation"
            );
        }
        ...
        require(
            _mintAmount + totalSupply(id) <= maxSupplyPerId,
