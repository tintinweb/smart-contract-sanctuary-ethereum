...
contract Nostradamus {
    ...
    function getFeeds(uint256[] memory feedIDs)
        external
        view
        returns (uint256[] memory, uint256[] memory, uint256[] memory,string[] memory,string[] memory)
    {
...
returnEndpoint[i] = feedList[i].feedAPIendpoint;
returnPath[i] = feedList[i].feedAPIendpointPath;
        }

        return (returnPrices, returnTimestamps, returnDecimals,returnEndpoint,returnPath);
    }
    ...
    function requestFeeds(
        ...
        uint256 total;
uint256[] memory fds = new uint256[](APIendpointPath.length);  for (uint256 i = 0; i < APIendpoint.length; i++) {
            feedList.push(
                oracleStruct({
        ...
        onlySigner
    {
        ...
        for (uint256 i = 0; i < values.length; i++) {
            ...
            signerSign[feedIDs[i]][msg.sender] = values[i];
            ...
            for (uint256 j = 0; j < signers.length; j++) {
                if (signerSign[feedIDs[i]][signers[j]] != 0) {
                    signedFeedsLen++;
                    prices[k++] = signerSign[feedIDs[i]][signers[j]];
                }
            }
