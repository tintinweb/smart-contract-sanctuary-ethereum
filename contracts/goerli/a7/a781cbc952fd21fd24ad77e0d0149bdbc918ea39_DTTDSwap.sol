...
contract DTTDSwap is Ownable, ReentrancyGuard, Pausable, IDTTDSwap {

    constructor(address _authority) {
        if(_authority == address(0)) { revert DTTDAuthorityError__SetInvalidAuthority(); }
    ...
    modifier ensureUnusedSwapNonce(uint256 _swapNonce) {
        if (executedSwapNonce[_swapNonce]) { revert DTTDSwapError__SwapExecuted(_swapNonce); }
      _;
   }
    ...
    modifier ensureNotExpired(uint256 _expiry) {
        if (block.timestamp > _expiry) { revert DTTDSwapError__SwapExpired(); }
      _;
   }
      ...
      ensureNotExpired(_swap.endTime)
    {
        ...
        if (totalOffers < 1) { revert DTTDSwapError__InsufficientOffers(); }

        if (_signature.length != totalOffers + 1) { revert DTTDSwapError__OfferSignatureLengthMismatch(); }
        ...
        if (blockedSwapHash[swapHash]) {
            revert DTTDSwapError__SwapBlocked(swapHash);
        }
        ...
        if(!validAuthority) { revert DTTDSignatureError__InvalidAuthority(); }
        ...
        for (uint256 i = 0; i < totalOffers; i++) {
            ...
            if (from != msg.sender) {
                ...
                if(!validSignature) { revert DTTDSignatureError__InvalidSignature(); }
            }
            ...
            for (uint256 j = 0; j < totalOfferItems; j++) {
                ...
                if (offerItem.offerItemType > 3) { revert DTTDSwapError__UnknownOfferItemType(); }
                OfferItemType itemType = OfferItemType(offerItem.offerItemType);

                address to = offerItem.to;
                if (to == address(0x0)) { revert DTTDSwapError__InvalidOfferItemTo(); }
                ...
                if (itemType == OfferItemType.ERC721) {
                    ...
                    // disallow 0 amount
                    if (amount == 0) { revert DTTDSwapError__InvalidOfferItemAmount(); }
                    ...
                    if (!ok || data.length != 32) {
                        revert DTTDSwapError__ERC20TransferFailed(token, from, to, amount);
                    }
                    ...
                    if (from != msg.sender) { revert DTTDSwapError__NativeTokenTransferNotSender(); }
                    ...
                    if (!sent) { revert DTTDSwapError__NativeTokenTransferFailed(from, to, amount); }
                }
            }
        }
        ...
        if (remainingFunds > 0) {
            ...
            if (!sent) {
                revert DTTDSwapError__NativeTokenTransferFailed(address(this), msg.sender, remainingFunds);
            }
        }
    ...
    function blockSwap(
        Swap calldata _swap
    ) external nonReentrant ensureUnusedSwapNonce(_swap.swapNonce) ensureNotExpired(_swap.endTime) {
        ...
        revert DTTDSwapError__NotAnOfferer(msg.sender);
    }
      ...
      returns (bool[] memory, uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < _swap.offer.length; i++) {
            ...
            for (uint256 j = 0; j < offer.offerItem.length; j++) {
                ...
                // Check if the offer item value can be converted into the enum
                if (item.offerItemType > 3) { revert DTTDSwapError__UnknownOfferItemType(); }
