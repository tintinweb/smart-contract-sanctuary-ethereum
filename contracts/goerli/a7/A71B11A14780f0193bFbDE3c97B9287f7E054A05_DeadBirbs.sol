...
contract DeadBirbs is ERC721A, Seller, AttributeStore, ReentrancyGuard {
    ...
    constructor(
        ...
        setAttributeSigner(0xB9987a20a6F835A63Eeb5750eB1356b584c8b606);
    }
...
contract AttributeStore is Ownable {
    ...
    function verifyAttributeHash(string memory attributeHash, bytes memory attributeSignature) internal virtual view {
        checkSignature(attributeSigner, attributeHash, attributeSignature);
        require(firstMinters[attributeHash] == address(0), "hash has been minted already");
    }

    function checkSignature(address signer, string memory attributeHash, bytes memory signature) private pure {
        bytes32 messageHash = keccak256(abi.encodePacked(attributeHash));
        require(signer == messageHash.toEthSignedMessageHash().recover(signature), string(abi.encodePacked(messageHash)));
    }

    function sotreAttributeHash(string memory attributeHash, address minter, uint256 tokenId) internal {
