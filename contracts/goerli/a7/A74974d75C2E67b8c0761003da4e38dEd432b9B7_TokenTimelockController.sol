...
contract TokenTimelockController is Ownable, EmergencyWithdrawable {
    ...
    /**
     * @dev Emitted when multiple {TokenTimelock} has claimed.
     */
    event TokenTimelocksClaimed(TokenTimelock[] tokenTimelock, address[] beneficiary, uint256 claimedTime, uint256[] amount);
    ...
    /**
     * @dev Emitted when multiple {TokenTimelock} has detroyed.
     */
    event TokenTimelocksDestroyed(TokenTimelock[] tokenTimelock, address[] beneficiary, uint256 destroyedTime, uint256[] amount);
     ...
     */
    function _claim(address tokenTimelock_) private returns (TokenTimelock, address, uint256) {
        TokenTimelock tokenTimelock = TokenTimelock(tokenTimelock_);
        ...
        return (tokenTimelock, beneficary, balance);
    }
    ...
    function claim(address tokenTimelock_) public {
        (TokenTimelock tokenTimelock, address beneficiary, uint256 amount) = _claim(tokenTimelock_);
    ...
    function claim(address beneficiary_, uint256 index_) public {
        address _tokenTimelock = _tokenTimelocks[beneficiary_].at(index_);
        (TokenTimelock tokenTimelock, address beneficiary, uint256 amount) = _claim(_tokenTimelock);
    ...
    function claimAll() public {
        ...
        address[] memory addresses = new address[](length);
        TokenTimelock[] memory tokenTimelocks = new TokenTimelock[](length);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (block.timestamp >= tokenTimelock.releaseTime()) {
                (, address beneficiary, uint256 amount) = _claim(address(tokenTimelock));

                amounts[i] = amount;
                addresses[i] = beneficiary;
                tokenTimelocks[i] = tokenTimelock;
            }
        }

        if (tokenTimelocks.length > 0) {
            emit TokenTimelocksClaimed(tokenTimelocks, addresses, block.timestamp, amounts);
        }
    }
