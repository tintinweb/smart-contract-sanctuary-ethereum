...
contract SubCharacter is Initializable, ERC721EnumerableUpgradeable {
    ...
    function initialize(string memory name_) initializer public {
        __ERC721_init_unchained(name_, "SUBC");
        __ERC721Enumerable_init_unchained();
        grades = ["Bronze", "Silver", "Gold"];
        backgrounds = ["peru", "silver", "gold"];
    }
    ...
    function getBadges(uint tokenId_) internal view returns (string memory) {
        ...
        for (uint i = 0; i < registeredNfts.length; i++) {
            if (IERC721Upgradeable(registeredNfts[i]).balanceOf(owner) > 0) {
    ...
    function tokenURI(uint256 tokenId_) public view override returns (string memory) {
        ...
        string memory characterId = tokenId_.toString();
        string memory name = string(abi.encodePacked(name(), ' ', characterId));
        string memory description = string(abi.encodePacked('Character ', characterId, ' is a sub-character.'));
    ...
    returns (string memory)
    {
        string memory image = generateSVGImage(
            SVGParams({ characterId: params.characterId, grade: params.grade, background: params.background, badges: params.badges })
        );
    ...
    {
        return Base64Upgradeable.encode(bytes(generateSVG(params)));
    }
    ...
    function generateSVG(SVGParams memory params) internal pure returns (string memory svg) {
        ...
        return string(
            abi.encodePacked(
                ...
                '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle"> ', params.characterId, ' </text>',
                _SVG_END_TAG
            )
        );
    }
...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;
import "./SubCharacter.sol";

contract SubCharacterFactory {
    function createSubCharacter(string memory name_) public returns (address subCharacter){
        bytes memory bytecode = type(SubCharacter).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(name_));
        assembly {
            subCharacter := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        SubCharacter(subCharacter).initialize(name_);
