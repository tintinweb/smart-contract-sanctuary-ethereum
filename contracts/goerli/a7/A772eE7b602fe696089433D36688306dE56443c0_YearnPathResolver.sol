...
import { IPathFinder, PathFinderComponent } from "../interfaces/IPathFinder.sol";
...
contract YearnPathResolver is IPathResolver, ResolverConfigurator {
    ...
    PathFinderComponent public override getComponentId =
        PathFinderComponent.YEARN_PATH_RESOLVER;
    ...
    function _updatePathFinderComponents() internal override {
        ...
        yearnDepositor = IDepositor(
            pathFinder.componentAddressById(PathFinderComponent.YEARN_DEPOSITOR)
        );

        yearnWithdrawer = IWithdrawer(
            pathFinder.componentAddressById(
                PathFinderComponent.YEARN_WITHDRAWER
            )
        );
    ...
    function findOneTokenPath(
    ...
    ) public returns (StrategyPathTask memory) {
        if (
            typeIn == TokenType.NORMAL_TOKEN &&
            task.targetType == TokenType.YEARN_ON_NORMAL_TOKEN
        ) {
            return normal2YearnOnNormalToken(tokenIn, amount, task);
        }

        if (
            typeIn == TokenType.YEARN_ON_NORMAL_TOKEN &&
            task.targetType == TokenType.NORMAL_TOKEN
        ) {
            return yearnOnNormalToken2Normal(tokenIn, amount, task);
        }

        if (
            typeIn == TokenType.NORMAL_TOKEN &&
            task.targetType == TokenType.YEARN_ON_CURVE_TOKEN
        ) {
            return normal2YearnOnCurveToken(tokenIn, amount, task);
        }

        if (
            typeIn == TokenType.YEARN_ON_CURVE_TOKEN &&
            task.targetType == TokenType.NORMAL_TOKEN
        ) {
            return yearnOnCurveToken2Normal(tokenIn, amount, task);
        }
        ...
        if (
            ...
            return curve2YearnOnCurveToken(tokenIn, amount, task);
        }
        ...
        if (
            ...
            return yearnOnCurveToken2Curve(tokenIn, amount, task);
        }
    ...
    {
        address yearnUnderlying;

        (yearnUnderlying, task) = yearnDepositor.getUnderlying(
            task.target,
            task
        );
        ...
        if (
            pathFinder.tokenTypes(yearnUnderlying) == TokenType.CURVE_LP_TOKEN
        ) {
            ...
            for (uint256 i; i < len; ++i) {
                ...
                (, currentTask) = yearnDepositor.depositAll(
                    currentTask.target,
                    currentTask
                );
        ...
        task = swapAggregator.swapAllNormalTokens(task, yearnUnderlying);
        (, task) = yearnDepositor.depositAll(task.target, task);

        return task;
    }

    /// @dev NORMAL TOKEN -> YEARN ON NORMAL TOKEN
    function normal2YearnOnNormalToken(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public returns (StrategyPathTask memory) {
        ...
        bool isAllUnderlying = task.balances.getBalance(yearnUnderlying) < 2;

        if (yearnUnderlying != tokenIn) {
            (amount, task) = swapAggregator.findBestSwapOrRevert(
                tokenIn,
                amount,
                yearnUnderlying,
                task
        ...
        if (isAllUnderlying) {
            ...
            (, task) = yearnDepositor.deposit(amount, task.target, task);
        }

        return task;
    }

    /// @dev YEARN ON NORMAL TOKEN ->  NORMAL TOKEN
    function yearnOnNormalToken2Normal(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public returns (StrategyPathTask memory) {
        ...
        (yearnUnderlying, task) = yearnDepositor.getUnderlying(tokenIn, task);

        bool isAllUnderlying = task.balances.getBalance(yearnUnderlying) < 2;

        (amount, task) = yearnWithdrawer.withdraw(amount, tokenIn, task);

        if (yearnUnderlying != task.target) {
            if (isAllUnderlying) {
                (, task) = swapAggregator.findBestAllInputSwapOrRevert(
                    yearnUnderlying,
                    task.target,
                    task
                );
            } else {
                (, task) = swapAggregator.findBestSwapOrRevert(
                    yearnUnderlying,
                    amount,
                    task.target,
                    task
    ...
    /// @dev NORMAL TOKEN -> YEARN ON CURVE TOKEN
    function normal2YearnOnCurveToken(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public returns (StrategyPathTask memory) {
        ...
        (inputs, task) = curveDepositor.getUnderlyings(
            TokenType.NORMAL_TOKEN,
            yearnUnderlying,
            task
        );
        ...
        StrategyPathTask memory bestTask = task.clone();
        bool pathFound = false;

        bool isAllCurve = task.balances.getBalance(yearnUnderlying) < 2;

        unchecked {
            for (uint256 i; i < len; ++i) {
                StrategyPathTask memory currentTask = task.clone();

                bool isAllMiddle = task.balances.getBalance(inputs[i]) < 2;
                ...
                if (isAllCurve) {
                    (amountOut, currentTask) = yearnDepositor.depositAll(
                        task.target,
                        currentTask
                    );
                } else {
                    (amountOut, currentTask) = yearnDepositor.deposit(
                        amountOut,
                        task.target,
                        currentTask
                    );
    ...
    /// @dev YEARN ON CURVE TOKEN -> NORMAL TOKEN
    function yearnOnCurveToken2Normal(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public returns (StrategyPathTask memory) {
        ...
        unchecked {
            for (uint256 i; i < len; ++i) {
                ...
                bool isAllMiddle = task.balances.getBalance(
                    curveUnderlyings[i]
                ) < 2;
    ...
    /// @dev CURVE_LP TOKEN -> YEARN ON CURVE TOKEN
    function curve2YearnOnCurveToken(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
        ...
        if (yearnUnderlying != tokenIn) {
            (amount, task) = curveDepositor.deposit(
    ...
    /// @dev  YEARN ON CURVE TOKEN -> CURVE_LP TOKEN
    function yearnOnCurveToken2Curve(
        address tokenIn,
        uint256 amount,
        StrategyPathTask memory task
    ) public view returns (StrategyPathTask memory) {
        ...
        if (yearnUnderlying != task.target) {
            if (isAllUnderlying) {
                (, task) = curveWithdrawer.withdrawAll(
                    yearnUnderlying,
                    TokenType.CURVE_LP_TOKEN,
                    task.target,
                    task
