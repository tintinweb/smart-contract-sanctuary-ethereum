...
contract MyToken is ERC1155, Ownable {
    uint16 [] public supplies =[50,100,150];
    uint16 [] public minted =[0,0,0];
    uint64 []  rates =[0.05 ether,0.1 ether,0.255 ether];
    mapping(address => bool) public whitelist;



    constructor() ERC1155("https://api.blabla.com/{id}") {}

    function mint(uint16 _tokenId, uint16 _amount)
        public
        payable
    {   uint16 index = _tokenId-1;
        require (_tokenId<=supplies.length && _tokenId>=0,"Token doesn't exist");
    ...
    function claimToken(uint16 _tokenId, uint16 _amount) public payable {
        require(whitelist[msg.sender], "User must be on the whitelist to claim tokens.");
        mint(_tokenId, _amount);
