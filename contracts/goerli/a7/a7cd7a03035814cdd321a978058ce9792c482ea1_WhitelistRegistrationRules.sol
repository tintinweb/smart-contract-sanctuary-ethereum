...
contract WhitelistRegistrationRules is IRegister {
    ...
    function canRegister(uint256 _tokenId, string memory _label, address _addr, uint256 _priceInWei, bytes32[] calldata _proofs) public view returns(bool){
        bytes32 leaf = keccak256(abi.encodePacked(_addr, _label));
        return MerkleProof.verify(_proofs, merkleRoots[_tokenId], leaf) && _priceInWei == mintPrices[_tokenId];
    }

    function mintPrice(uint256 _tokenId, string calldata _label, address _addr, bytes32[] calldata _proofs) external view returns(uint256){
        require(canRegister(_tokenId, _label, _addr, 0, _proofs), "not authorised");
