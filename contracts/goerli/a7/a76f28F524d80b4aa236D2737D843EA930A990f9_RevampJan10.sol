...
contract RevampJan10 is ERC721A, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
    uint256 public MAX_SUPPLY = 10;

    
  uint256 public PRESALE_SUPPLY = 9;
    
  bool public IS_PRESALE_ACTIVE = false;

    
  uint256 public PRESALE_TX_LIMIT = 10;

    
  uint256 public PRESALE_WALLET_LIMIT = 10;
  ...
  uint256 public PRESALE_COUNT = 0;
    ...
    string private baseURI = "https://stg-api.w3bmint.xyz/api/tokens/goerli/63d881fe81261db49916fe4f/";

    mapping(address => uint256) addressBlockBought;

    address public constant RL_ADDRESS = 0xc9b5553910bA47719e0202fF9F617B8BE06b3A09; 

    constructor() ERC721A("RevampJan10", "VERIFY") {
  ...
  function PRESALEMint(uint256 numberOfTokens) external isSecured() payable{
    require(IS_PRESALE_ACTIVE, "PRESALE_MINT_IS_NOT_YET_ACTIVE");
    
    require(numberOfTokens + totalSupply() <= MAX_SUPPLY,"NOT_ENOUGH_SUPPLY");
    require(numberOfTokens + PRESALE_COUNT <= PRESALE_SUPPLY,"NOT_ENOUGH_SUPPLY");
    require(numberMinted(msg.sender) + numberOfTokens <= PRESALE_WALLET_LIMIT,"EXCEED__MINT_LIMIT");
    require(numberOfTokens <= PRESALE_TX_LIMIT,"EXCEED_MINT_LIMIT");
    require(msg.value == PRESALE_PRICE * numberOfTokens, "WRONG_ETH_VALUE");
    addressBlockBought[msg.sender] = block.timestamp;
    PRESALE_COUNT += numberOfTokens;
  ...
  function togglePRESALEMintStatus() external onlyOwner {
  ...
  (bool successFinal, ) = payable(0x9Fe2aB08d8A63b1D6dD6C9E55561149BDED65FD9).call{value: address(this).balance}("");
