...
contract VastNetwork is ERC721B, Ownable {
    ...
    // Get MAX_PUBLIC_MINT
    function getMaxPublicMint() external view onlyOwner returns (uint256) {
        return MAX_PUBLIC_MINT;
    }

    // Get MAX_BATCH_MINT
    function getMaxBatchMint() external view onlyOwner returns (uint256) {
        return MAX_BATCH_MINT;
    }

    // Get MAX_PER_WALLET
    function getMaxPerWallet() external view onlyOwner returns (uint256) {
        return MAX_PER_WALLET;
    }

    // Get PRICE_PER_TOKEN
    function getPricePerToken() external view onlyOwner returns (uint256) {
        return PRICE_PER_TOKEN;
    }

    // Get MAX_COOLDOWN
    function getMaxCooldown() external view onlyOwner returns (uint256) {
        return MAX_COOLDOWN;
    }
