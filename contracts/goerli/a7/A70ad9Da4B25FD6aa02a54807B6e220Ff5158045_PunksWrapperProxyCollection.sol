...
contract PunksWrapperProxyCollection is Ownable, ERC721Enumerable, ReentrancyGuard, RendererLockable {
    ...
    function wrap(uint _punkId) nonReentrant external payable {
        ...
        PunksWrapperProxy(payable(proxyForPunksOwner[msg.sender])).acquire(_punkId, msg.sender);
    ...
    function unwrap(uint256 _punkId) nonReentrant external {
        ...
        PunksWrapperProxy(payable(proxy)).giveBack(_punkId, msg.sender);
    }
    ...
    function tokenURI(uint256 itemId) public view override returns (string memory) {
        return IPunksWrapperProxyCollectionRenderer(renderer).getTokenURI(itemId, isEmptyWrapper(itemId));
    }
...
contract PunksWrapperProxy {
    ...
    function giveBack(uint256 _punkId, address unwrapCaller) external onlyController {
