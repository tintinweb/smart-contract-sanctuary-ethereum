...
contract ERC721Auction is IERC721Auction, ERC721, Ownable {
    ...
    function bid(uint256 tokenId) external payable {
        ...
        require(tokenId < maxSupply, "All tokens claimed");
        require(tokenIdCounter.current() == tokenId, "Bid for another token");
        ...
        require(msg.value >= startingPrice, "Bid lower than startingPrice"); // Can be the same custom error as below
        require(msg.value >= bidAmount + ((bidAmount * minimumPercentageIncreasex100) / 10000), "Bid too low");
        ...
        // Note: condition is mostly false - creating a local variable for timeBuffer wouldn't save gas for most users.
    ...
    function settleAuction() external {
        ...
        // Mint the token to the winner (if possible).
        if (bidder == address(0)) {
            // TODO: handle when there are no bids
        } else {
            ...
            emit AuctionSettled(tokenId, bidder, bidAmount);
        }
    }
    ...
    // Create a new auction if and emit an event.
    ...
    // TODO: check how these returns look like on etherscan, consider adding back tokenId
    function getAuctionState() external view returns (Auction memory) {
        return auction;
    }
