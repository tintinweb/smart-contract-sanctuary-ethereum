...
contract CAVStaking is AccessControlEnumerable, ReentrancyGuard, Pausable {
    ...
    ILaunchpad public launchpad; // Launchpad address
    ...
    constructor(
        ...
        launchpad = ILaunchpad(launchpadAddress);
    ...
    function withdrawReward(uint256 tokenId) public nonReentrant {
        ...
        if (reward > 0) {
            ...
            tokenInfo[tokenId].lastRewardTime = uint32(block.timestamp);
            rewardBalance -= reward;
    ...
    function deposit(uint256 tokenId, uint32 term)
        external
        nonReentrant
        whenNotPaused
    {
        ...
        ILaunchpad.TokenData memory token = launchpad.getFreezingBalance(
            tokenId
        );
        require(
        ...
        if (token.notFrstTimeStaked) {
            launchpad.reciveFeeFromStakingContract(msg.sender);
        }
    ...
    function addBalance(uint256 amount) external onlyOwner {
    ...
    function setNewLaunchpad(address newLaunchpad) external onlyOwner {
        launchpad = ILaunchpad(newLaunchpad);
    }
    ...
    function _calculatePercentage(uint16 percentage)
        internal
        view
        returns (uint256)
    {
        return (rewardRate / 100000) * percentage;
    }

    function calcRewardWithDistr(uint256 distributionLvl)
        internal
        view
        returns (uint256)
    {
    ...
    function pendingReward(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 quantityRewardPercentage = (token.quantity * uint256(10**18)) /
            totalTokenQuantity;
        uint256 quantityReward = uint256(
            (blockRewardPercentage * quantityRewardPercentage) / uint256(10**18)
        );
...
interface ILaunchpad {
    ...
    function reciveFeeFromStakingContract(address sender) external;
}
