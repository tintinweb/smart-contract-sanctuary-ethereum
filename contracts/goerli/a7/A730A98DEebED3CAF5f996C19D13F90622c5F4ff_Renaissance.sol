...
contract Renaissance is ERC721A, Ownable, ReentrancyGuard, ERC2981 {
    ...
    uint256 WL_SUPPLY;
    uint256 PUBLIC_SUPPLY; 
    uint256 FREEMINT_SUPPLY;

    uint256 publicMintPrice = 1 ether;
    ...
    constructor () ERC721A("Renaissance", "RNSC") {
    }
    ...
    function mintWhitelist () external payable {
        ...
        if (_totalMinted() + 1 > WL_SUPPLY) revert MaxMintExceeded();

        require(whitelisted[msg.sender] >  0, "Message sender not whitelisted");
    ...
    function mintPublic () external payable {
        ...
        if (tx.origin != msg.sender) revert NoContractMints();
        if (_totalMinted() + 1 > PUBLIC_SUPPLY) revert MaxMintExceeded();
    ...
    function setPublicSupply(uint256 publicSupply) external onlyOwner {
    ...
    function giveFreeMint () external onlyOwner{

    }

    function setWhitelisted (address walletAddress) external onlyOwner {
        require(whitelisted[walletAddress] < 1);
        whitelisted[walletAddress] = 1;
    }
