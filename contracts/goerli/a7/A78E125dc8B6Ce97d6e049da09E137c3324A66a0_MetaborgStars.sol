...
contract MetaborgStars is ERC721Upgradeable {
    ...
    */
    ...
    function initialize(uint _elements, address _ERC1155Address) initializer public {
        ...
        for(uint index = uint(0); index < _elements; index++){
            availablePagesArray.push(uint8(index.add(6)));
        }
    }
    ...
    */
    ...
    function tokenURI(uint256 _tokenId) public override pure returns (string memory) {
        return string(abi.encodePacked("https://poseidondao.mypinata.cloud/ipfs/QmP9urnKMSDCAkzNyRJzmpJjbhmYuQbLPaYdunQdthYWAh/",_tokenId.toString(),".json"));
    }
