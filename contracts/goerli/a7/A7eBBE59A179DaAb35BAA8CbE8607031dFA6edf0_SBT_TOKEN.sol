...
contract SBT_TOKEN is ERC4973 ,Ownable{
    ...
    */
    ...
    struct UNIVERSITY {
        address university_wallet_address;
        string uni_name;
    ...
    */
    ...
    //  HASHMAP TO STORE USER CERTIFICATES
    mapping(address => CERTIFICATE[]) public userCertifacte;
    
    event UNIVERSITY_ADDED(address indexed _from, uint indexed _id, string NAME);
    event CERTIFICATE_GENERATED(address indexed _from, uint indexed _id, string NAME);
    ...
    function CREATE_UNIVERCITY(uint ID, string memory _NAME,address UNI_ADDRESS) private{
        universities[ID].university_wallet_address = UNI_ADDRESS;
    ...
    function GET_WHITELISTED_ACCOUNTS(uint id) public view returns(address[] memory){
        require (universities[id].university_wallet_address == msg.sender , "Only UNIVERSITY Can check the whitelisted Accounts");
        return universities[id].whitelisted_accounts;
    }

    function WHITELIST_ADDRESS_FOR_UNIVERSITY(uint id,address whitelist_account) public{
        require (universities[id].university_wallet_address == msg.sender , "Only UNIVERSITY owner can whitelist accounts");
    ...
    function GENERATE_CERTIFICATE(string memory _student_name,
        string memory _program,
        string memory _issue_date,
        ...
        require(_isWhiteListedAccount(msg.sender,_uni_id ),"Only whitelisted Accounts can generate token");
       
       uint256 cert_id = _certificate_Counter.current();
       _certificate_Counter.increment();
       CERTIFICATE memory certificate;

        certificate.student_name = _student_name;
        certificate.program = _program;
        certificate.issue_date = _issue_date; 
        certificate.department = _department;
        certificate.contact_number = _contact_number;
        certificate.cgpa = _cgpa;
        certificate.certificate_uri = _certificate_uri;
        certificate.tenure = _tenure;

        userCertifacte[_student_address].push(certificate);
    ...
    function _isWhiteListedAccount(address account, uint id) private returns (bool){
    ...
    function GET_USER_CERTIFICATES(address user_wallet_address) public view returns(CERTIFICATE[] memory){

        require(userCertifacte[user_wallet_address].length > 0, " No Certificates Found");
        return userCertifacte[user_wallet_address];
    }

    function GET_MY_CERTIFICATES() public view returns(CERTIFICATE[] memory){
        return GET_USER_CERTIFICATES(msg.sender);
    }
