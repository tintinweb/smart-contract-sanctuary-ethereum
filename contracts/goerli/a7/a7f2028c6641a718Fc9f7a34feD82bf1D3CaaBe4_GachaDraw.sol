...
contract GachaDraw is Ownable {
        ...
        validActivity(_activityId)
    {
        ...
        uint256[] memory selectedTokenAmount = new uint256[](6);
        ...
        for (uint256 i = 0; i < _ticketAmount; i++) {
            ...
            selectedTokenAmount[selectedTokenType]++;
        ...
        for (uint256 i = 0; i < REWARD_TOKEN_TYPE_LIST.length; i++) {
            uint256 tokenType = REWARD_TOKEN_TYPE_LIST[i];
            if(selectedTokenAmount[tokenType] > 0) {
                ///@dev Mints one randomly choosen reward ticket
                RewardContract.mintBatch(
                    _activityId,
                    msg.sender,
                    tokenType,
                    selectedTokenAmount[tokenType],
                    ""
                );
            }
        }
