 ...
 */
 ...
 contract RhinoChef is Ownable, ReentrancyGuard {
     ...
     // RHINOS
     IRhinosToken public rhinos;
     // RHINOS LP address
     IUniswapPair public rhinosLp;
     // RHINOS tokens reward per block.
     ...
     // The block number when RHINOS mining starts.
     ...
     constructor(
         ...
         startTime = _startTime < block.timestamp ? block.timestamp : _startTime;
         lastBlock = _startTime < block.timestamp ? block.timestamp : _startTime;
     }
     ...
     // Update the given pool's RHINOS allocation point. Can only be called by the owner.
     function set(
         uint256 _pid,
         uint256 _allocPoint,
         bool _withUpdate
         ...
         returns (uint256)
     {
         ...
         if (address(pool.lpToken) == address(rhinos)) {
             lpSupply = rhinos.balanceOfUnderlying(address(this));
         }
         if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
             uint256 rhinosReward = ((block.timestamp - pool.lastRewardTime) *
                 rewardPerSecond *
                 pool.allocPoint) / totalAllocPoint;
             accRewardPerShare += (rhinosReward * 1e12) / lpSupply;
         }
         return
     ...
     function updatePool(uint256 _pid) public {
         ...
         uint256 rhinosReward = ((block.timestamp - pool.lastRewardTime) *
             rewardPerSecond *
             pool.allocPoint) / totalAllocPoint;
         pool.accRewardPerShare += (rhinosReward * 1e12) / lpSupply;
         pool.lastRewardTime = block.timestamp;
     ...
     // Deposit tokens to RhinoChef for RHINOS allocation.
     function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
         ...
         if (address(pool.lpToken) == address(rhinos)) {
             _amount = rhinos.fragmentToEggs(_amount);
         }
     ...
     // Withdraw tokens from RHINOChef.
     function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
         ...
         user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
         if (address(pool.lpToken) == address(rhinos)) {
             _amount = rhinos.eggsToFragment(_amount);
         }
     ...
     function claim(uint256 _pid, address _account) external nonReentrant {
         ...
         rhinos.mint(_account, pending);
         if (lastBlock != block.timestamp) {
             rhinos.rebase(
                 block.timestamp,
                 compound(1e18, compoundRatio, block.timestamp - lastBlock) - 1e18,
                 false
             );
