...
error StructERC721_InsufficientPermissions();
    ...
    OperatorFilterer
{
    ...
    IERC721 private immutable _STRUCT_PASS;
    IRandomnessManager private immutable _RANDOMNESS_MANAGER;
    bytes32 private immutable _TOKEN_OFFSET_SLOT;
    address payable private immutable _FEE_RECEIVER;
    ...
    constructor(
        ...
        _FEE_RECEIVER = payable(feeReceiver);
        _STRUCT_PASS = structPass;
        _RANDOMNESS_MANAGER = randomnessManager;
        _TOKEN_OFFSET_SLOT = randomnessManager.TOKEN_OFFSET_SLOT();
        VERSION = version;
    }
    ...
    function initialize(
        ...
        public
    {
        if (config.maxSupply > 10_000) revert StructERC721_InvalidConfiguration();
        if (config.revealed && config.offsetEnabled) revert StructERC721_InvalidConfiguration();
     ...
     */
    function initializeMintSettings(
        ...
        public
    {
        ...
        if (_STRUCT_PASS.balanceOf(msg.sender) == 0) {
            if (_mintSettings.privatePrice < 0.0025 ether || _mintSettings.privatePrice < 0.0025 ether) {
                AddressUpgradeable.sendValue(_FEE_RECEIVER, 0.00005 ether * uint256(config.maxSupply));
            } else {
                if (totalShares() != 10_000 || shares(_FEE_RECEIVER) != 500)
    ...
    /**
     * @notice Mint reserves to a specified address
     * @dev This should only be called in the initializer
     * @param to Receiving address of the minted reserve tokens
     */
    function _mintReserve(address to) internal {
        if (config.reserveAmount > config.maxSupply) revert StructERC721_InsufficientSupply();
        if (config.reserveAmount > 0) _mint(to, config.reserveAmount);
    }
     ...
     * @param _mintSettings New mint settings
     */
        ...
        onlyOwner
    {
        ...
        if (
            ...
            _mintSettings.privatePrice < 0.0025 ether || _mintSettings.privatePrice < 0.0025 ether
        ) {
            AddressUpgradeable.sendValue(_FEE_RECEIVER, 0.00005 ether * uint256(config.maxSupply));
        }
     ...
     */
    function setOperatorFilter(bool enabled) public payable onlyOwner {
     ...
     */
    function setProvenanceHash(string calldata _provenanceHash) public payable lockable onlyOwner {
        if (config.offsetEnabled && _RANDOMNESS_MANAGER.randomness(address(this), _TOKEN_OFFSET_SLOT) != 0)
     ...
     * @param _uri Placeholder or base token URI depending on revealed state
     */
    function setRevealed(bool _revealed, string calldata _uri) public payable lockable onlyOwner {
     ...
     */
    function commitMetadata() public onlyOwner {
     ...
     */
    function privateMint(uint256 amount, bytes memory signature) public payable {
        if (msg.value != mintSettings.privatePrice * amount) revert StructERC721_InvalidTransactionValue();
    ...
    function publicMint(uint256 amount) public payable {
        if (msg.value != mintSettings.publicPrice * amount) revert StructERC721_InvalidTransactionValue();
    ...
    function _mint(uint256 amount) internal {
        ...
        if (_numberMinted(msg.sender) + amount > mintSettings.maxPerAddress)
