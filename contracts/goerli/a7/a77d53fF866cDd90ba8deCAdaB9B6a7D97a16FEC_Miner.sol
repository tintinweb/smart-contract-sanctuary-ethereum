...
contract Miner is Ownable, ReentrancyGuard {
    ...
    mapping (address => uint) userOrderLen;
    ...
    /**
     * @dev deposit
     * @param _depositAmount minimum amount is 50 USDC
     * @param _invitation Address of superior inviter
     */
    function deposit(uint _depositAmount, address _invitation) public {
        require(_depositAmount >= 50e18, 'Minimum amount is 50 USDC');
        IERC20(USDC).transferFrom(_msgSender(), address(this), _depositAmount);

        userOrders[_msgSender()][userOrderLen[_msgSender()]] = Order(_depositAmount, block.timestamp + lockTime, 1, 1);
        userOrderLen[_msgSender()]++;  

        if(_invitation != address(0)) referralBonus[_invitation] += _depositAmount * molecular / denominator;   // 邀请人奖励

        emit Deposit(_msgSender(), _depositAmount);
    }


    /**
     * @dev Withdrawal of principal
     * @param _amount Amount of deposit
     */
    function withdraw(uint _amount) public nonReentrant {
        uint len = userOrderLen[_msgSender()];
        for(uint i = 0; i < len; i++) {
            Order storage targetOrder = userOrders[_msgSender()][i];
        ...
        IERC20(USDC).transfer(_msgSender(), actualReceiveAmount);

        emit Withdraw(_msgSender(), actualReceiveAmount);
    }

    /**
     * @dev View rebate rewards
     * @param account users address
     */
    ...
    function claimReferralBonus() external nonReentrant {
        uint referral_bonus = checkReferralBonus(_msgSender());
        require(referral_bonus > 0, 'You have no referral bonus');
        delete referralBonus[_msgSender()];
        IERC20(USDC).transfer(_msgSender(), referral_bonus);

        emit ClaimReferralBonus(_msgSender(), referral_bonus);
    }

    /**
     * @dev Administrator Settings
     * @param _totalDevices Total number of mining machines in the mine
     * @param _totalProfit_30days Total 30-day earnings from the mine
     */ 
    ...
    function redeemFees() external onlyOwner {
        ...
        delete fees;
        IERC20(USDC).transfer(_msgSender(), fee_amount);

        emit RedeemFees(_msgSender(), fee_amount);
    }
