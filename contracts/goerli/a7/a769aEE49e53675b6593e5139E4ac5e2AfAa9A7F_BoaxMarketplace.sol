...
contract BoaxMarketplace is AccessControlEnumerable, ReentrancyGuard {
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        BoaxMarketplaceState.setTokenInfo(_tokenId, msg.sender, _artistFee);
    }
     ...
     * @param _royalityFee is the new royaltiesFee for token
     */
    function modifyTokenRoyalities(uint256 _tokenId, uint24 _royalityFee)
        external
    {
        require(BoaxERC721.exists(_tokenId), "nonexistent token");

        require(BoaxERC721.ownerOf(_tokenId) == msg.sender, "not owner");
        BoaxMarketplaceState.modifyTokenRoyalities(_tokenId, _royalityFee);
    }
...
interface IBoaxMarketplaceState {
     ...
     * @dev updates the royaltiesFee for given token
     */
    function modifyTokenRoyalities(uint256, uint24) external;
