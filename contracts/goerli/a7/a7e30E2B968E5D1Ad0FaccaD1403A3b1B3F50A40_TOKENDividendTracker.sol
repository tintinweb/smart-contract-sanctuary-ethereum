...
contract $TESTD is ERC20, Ownable {
    ...
    constructor() ERC20("Test D", "TESTD") {
    ...
    function setMaxTx (uint256 amount) external onlyOwner {
        require(amount >= (totalSupply().mul(1).div(100)),"No rug pull!");
        maxTxAmount = amount;
    }

    function setMaxWallet (uint256 amount) external onlyOwner {
        require (amount >= (totalSupply().mul(1).div(100)));
        require (amount <= (totalSupply().mul(5).div(100)));
        maxWallet = amount ;
    }
...
contract TOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(address rewardToken)
        DividendPayingToken("Test Dividend Tracker", "TESTDT", rewardToken)
    {
        claimWait = 3600;
