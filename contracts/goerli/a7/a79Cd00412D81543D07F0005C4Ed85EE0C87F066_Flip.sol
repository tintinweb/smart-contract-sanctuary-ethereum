...
contract Flip is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    event FlipResult(uint256 requestId, bool result);
    ...
    address constant coordinatorAddress = 0x2Ca8E0C643bDe4C2E08ab1fA0da3401AdAD7734D;

    bytes32 keyHash = 0x79d3d8832d904592c0bf9818b621522c988bb8b0c05cdc3b15aea1b6e8db0c15;

    uint128 constant entryFee = 0.001 ether;
   ...
   function FlipCoin(FlipSelection choice) external payable returns (uint256) {
       require(msg.value == entryFee, "You should pay the fees");
   ...
   function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
       require(statuses[requestId].exists, "Request doesn't exist!");

       statuses[requestId].fulfilled = true;
       statuses[requestId].randomWord = randomWords[0];

       FlipSelection result = FlipSelection.HEAD;
       if (randomWords[0] % 2 == 0) {
           result = FlipSelection.TAIL;
       }

       if (statuses[requestId].choice == result) {
           statuses[requestId].result = true;
           payable(statuses[requestId].player).transfer(entryFee * 2);
       }

       emit FlipResult(requestId, statuses[requestId].result);
   }
