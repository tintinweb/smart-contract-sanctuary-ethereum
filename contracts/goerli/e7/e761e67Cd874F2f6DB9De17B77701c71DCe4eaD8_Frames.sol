...
contract Frames is AdminControl, ICreatorExtensionTokenURI, ILazyDelivery, ILazyDeliveryMetadata {
    ...
    // Initialize minimum bid to the max value of uint256
    uint256 private _minBid = 2**255 - 1;
    ...
    function deliver(uint40 listingId, address to, uint256, uint24, uint256 payableAmount, address, uint256) external override returns(uint256) {
        require(msg.sender == _marketplace && listingId == _listingId, "Invalid call data");

        // Set minimum bid if uninitialized
        if (_minBid == 2**255 - 1) {
            Bid[] memory bids = IMarketplaceCore(_marketplace).getBids(_listingId);
            for (uint256 i = bids.length-1; i >= 0; i--) {
                if (bids[i].amount < _minBid) {
                    _minBid = bids[i].amount;
                }
            }
        }
