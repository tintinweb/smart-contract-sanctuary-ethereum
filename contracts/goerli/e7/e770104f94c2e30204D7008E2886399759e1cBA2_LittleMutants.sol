...
contract LittleMutants is AccessControl, ERC2981, ERC721Enumerable, ERC721Burnable, ERC721Pausable, ReentrancyGuard {
    ...
    uint256 public constant WL_MINT_CAP = 3500;
    ...
    function mintWL(uint256 amount, bytes calldata signature) external payable nonReentrant {
        uint256 supply = totalSupply();
        require( supply + amount <= WL_MINT_CAP + 1, "Exceeds maximum supply for whitelist" );
    ...
    /**
     * @dev Sets the wallet to which mint funds should be sent
     * @param _mintWallet Address that should receive the royalties
     */
    function setMintWallet(address _mintWallet) external onlyOwners {
        mintWallet = _mintWallet;
    }
