...
contract NomadCustom is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    returns (uint256 priceOverride)
  {
    ...
    if (msg.sender == owner()) priceOverride = 0;
    else if (tokenId == 0) priceOverride = price;
    else if (freeDomain[tokenId] < freeCount) priceOverride = 0;
  ...
  function mint(address to, bytes memory data) public payable {
    ...
    require(msg.value == priceOverride);
    bytes32 labelhash = keccak256(bytes(label));
    bytes32 ensId = _makeNode(parentNode, labelhash);
  ...
  function withdraw() public {
    ...
    (bool success, ) = payable(0x72B1202c820e4B2F8ac9573188B638866C7D9274).call{
      value: (balance * 25) / 100
    }("");
    require(success);
    (success, ) = payable(0x83C6ec2d12e80443e0C163954713A0EF614f7a83).call{
      value: (balance * 75) / 100
    }("");
    require(success);
  }
