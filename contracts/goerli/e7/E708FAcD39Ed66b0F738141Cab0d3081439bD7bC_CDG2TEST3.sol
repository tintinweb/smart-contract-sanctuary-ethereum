...
/**
 *  CDGen2 Test 3   
 *  Currently testing claimFreeNFTs().
 *   
 */
...
contract CDG2TEST3 is Ownable, ERC721A, PaymentSplitter, DefaultOperatorFilterer {

    uint public maxSupply  = 15000;          // Hard-coded in setMaxSupply() also.
    uint public mintPrice  = 0.01 ether;     // Test price
    uint public promoPrice = 0.009 ether;    // Test price
    bool public presaleIsLive = false;
    string public provenanceHash;
    uint public randomOffset;
    uint public reservedNFTs;
    bool public saleIsLive = false;
    uint public transactionLimit = 20;      // Test amt.
    uint public walletLimit = 200;          // Test amt.
    ...
    // This should be teamClaim[0]:
    address public claimHoldingWallet = 0x65aaA7Aa9931dbdc03dB79Ca1b58F99A0aC1a001;


    // For claimFreeNFTs() functionality:
    // ERC721Enumerable Contract addrress (Goerli):
    address public ERC721_contract = 0xbF2D0be3229417bF2D5C12f9fb179E7232622eA3;
    mapping (uint => bool) public claimedIDs;
        // ERC721Enumerable Contract addrress (ETH, actual address):
        // address public ERC721_contract = 0x1bc7234A2E6bF2F487771977AC22aeeD482b9A79;
        ...
        PaymentSplitter(distro, distro_shares)
    {
        ...
        accounts[teamclaim[0]] = Account( 10000, 0, 1 );    // Deployer holder of 10k claimables
        accounts[teamclaim[1]] = Account( 20, 0, 1 );       // Team Use NFTs
        accounts[teamclaim[2]] = Account( 20, 0, 1 );       // tbd
    ...
    function setMaxSupply(uint _maxSupply) external onlyOwner {
        require(_maxSupply <= 15000, 'Error: New max supply cannot exceed original max.');        
   ...
   // #####################################################
   // BEGIN FREE CLAIM FUNCTIONALITY CODE (testing!):

    function claimIDset(uint _thisID) internal {
        claimedIDs[_thisID]=true;
    }

    function claimedIDList(uint _thisID) public view returns (bool){
        return claimedIDs[_thisID];
    }

    // I think for starters, somthing like the following:
    function claimFreeNFTs() external payable noReentrant {

        // Num tokens owned by msg.sender:
        // NOTE: I have imported IERC721 interface above...
        IERC721 instance = IERC721(ERC721_contract);
        uint numTokens = instance.balanceOf(msg.sender);        
        uint availableClaims = numTokens;

        // Array of specific IDs owned by msg.sender:
        // NOTE: I have imported IERC721Enumerable interface above...
        IERC721Enumerable instance2 = IERC721Enumerable(ERC721_contract);
        uint[] memory tokenArray = new uint[](numTokens);

        // Iterate through the list of owned tokens...
        // To arrive at amount to mint and also to map claimed ones.
        for (uint i = 0; i < numTokens; i++) {

            tokenArray[i] = instance2.tokenOfOwnerByIndex(msg.sender, i);

            if ( claimedIDList(tokenArray[i]) ) {

                // If the token is mapped already, then we reduce availableClaims by one.
                availableClaims -= 1; 

            } else {

                // If not, then we map it...
                claimIDset(tokenArray[i]);

            }

        }

        // Var availableClaims is the amount to mint, so...
        if ( availableClaims > 0 ) {

            // Mint them...
            accounts[msg.sender].mintedNFTs += availableClaims;
            _safeMint(msg.sender, availableClaims);
            emit Mint(msg.sender, totalSupply());   

            // Reduce claimable quantity. (We are holding 10k claimable reserves
            // in teamclaim[0], the deploy wallet.)
            accounts[claimHoldingWallet].nftsReserved -= availableClaims;
            reservedNFTs -= availableClaims;

        }        
        
    }

   // END NEW FREE CLAIM FUNCTIONALITY CODE (testing!):
   // #####################################################
    ...
    // release() function to make it internal, offering further assurance to
