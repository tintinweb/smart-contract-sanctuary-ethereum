...
contract MeanFlight is ERC721A, ReentrancyGuard, Ownable {
    ...
    mapping(address => address) public sponsorFor;

    string public uriPrefix = "ipfs://";
    string public uriSuffix = ".json";
    string public uriClaimedPrefix = "ipfs://";
    ...
    uint256 public preMintTxLimit = 20;
    uint256 public publicMintTxLimit = 20;
    uint256 public maxPreMintAmount = 20;
    uint256 public maxPublicMintAmount = 20;
    uint256 public internalMintAmount = 100;
    ...
    constructor(address[] memory _whiteListAddresses) ERC721A("MeanFlight", "MNFLT") {
    ...
    function preMint( uint256  _mintAmount) public payable preMintCompliance(_mintAmount) nonReentrant {
        if(sponsorFor[msg.sender] != 0x0000000000000000000000000000000000000000){
            (bool payment, ) = payable(sponsorFor[msg.sender]).call{value: msg.value * 5 / 100}("");
            require(payment);
        }else{
    ...
    function createSponsorship(address[] calldata _users, address _sponsor) public onlyOwner{
        for(uint256 i = 0; i < _users.length; i++){
            sponsorFor[_users[i]] = _sponsor;
        }
    }
    ...
    function withdraw() public onlyOwner nonReentrant{
        ...
        (bool dev, ) = payable(0xDC9eeF462bD661D5a14146dFA05f5cdB6Db5Fecf).call{value: withdrawAmount}("");
        require(dev);
        (bool artist, ) = payable(0xDC9eeF462bD661D5a14146dFA05f5cdB6Db5Fecf).call{value: withdrawAmount}("");
        require(artist);
        (bool marketing, ) = payable(0xDC9eeF462bD661D5a14146dFA05f5cdB6Db5Fecf).call{value: address(this).balance}("");
        require(marketing);
    }
}
