...
contract TestZooNFT is ERC721, ERC721Burnable, AccessControl {
    using Strings for uint256;
    ...
    event NewMintFeeToken(address indexed feeAddress);
    event FeeStatusUpdated(bool indexed newStatus);
    event ToggleDirectMinting(bool indexed enabled);
    ...
    constructor(
        uint256 _lemurMintFee,
        uint256 _rhinoMintFee,
        uint256 _gorillaMintFee,
        ...
        string memory _cid2,
        string memory _cid3
    ) ERC721("Test Zoo NFT", "TestTZN") {
        lemurMintFee = _lemurMintFee;
        rhinoMintFee = _rhinoMintFee;
        gorillaMintFee = _gorillaMintFee;
    ...
    function mintLemur(address to) public {
        ...
        while (_exists(lemurIdCounter)) {
            lemurIdCounter = lemurIdCounter + 1;
        }

        require(
            lemurIdCounter >= lemurMinId && lemurIdCounter <= lemurMaxId,
            "Lemur Id Out Of Range"
        );

        safeMint(to, lemurIdCounter);
        lemurIdCounter = lemurIdCounter + 1;
    }
    ...
    function mintRhino(address to) public {
        require(to != address(0), "Can Not Mint To Zero Address");
        while (_exists(rhinoIdCounter)) {
            rhinoIdCounter = rhinoIdCounter + 1;
        }
        require(
            rhinoIdCounter >= rhinoMinId && rhinoIdCounter <= rhinoMaxId,
            "Rhino Id Out Of Range"
        );

        safeMint(to, rhinoIdCounter);
        rhinoIdCounter = rhinoIdCounter + 1;
    }

    // Mint only Gorilla NFT
    function mintGorilla(address to) public {
        require(to != address(0), "Can Not Mint To Zero Address");
        while (_exists(gorillaIdCounter)) {
    ...
    function transferFee(
        ...
        require(
            IMintFeeToken(mintFeeToken).allowance(from, address(this)) >=
                amount,
            "Approve Contract For Payment"
        );
        IMintFeeToken(mintFeeToken).transferFrom(from, to, amount);
    }

    function setMintFeeToken(address newToken)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(newToken != address(0), "Address Can Not Be Zero Address");
        mintFeeToken = newToken;
        emit NewMintFeeToken(newToken);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit ToggleDirectMinting(setValue);
    }
    ...
    function setFees(
        ...
        lemurMintFee = _lemurFee;
        rhinoMintFee = _rhinoFee;
        gorillaMintFee = _gorillaFee;
