...
abstract contract ReentrancyGuard is Initializable {
    ...
    function __reentrencyGuard_initialize() internal initializer {
...
contract FarmBoosterProxy is ReentrancyGuard {
    ...
    address public FARM_BOOSTER_PROXY_FACTORY;
     ...
     * @param _bullToken: the address of the bull token
     */
    function initialize(
        address _admin,
    ...
    function harvestbull() internal {
        uint256 bullBalance = bullToken.balanceOf(address(this));
        if (bullBalance > 0) {
            bullToken.safeTransfer(msg.sender, bullBalance);
        }
    }
}

contract FarmBoosterProxyFactory is Initializable {
