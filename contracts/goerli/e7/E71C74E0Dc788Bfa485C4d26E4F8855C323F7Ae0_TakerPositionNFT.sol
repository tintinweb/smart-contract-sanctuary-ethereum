...
contract TakerPositionNFT is BumperCoreNFT {
    function initialize(
        string memory name,
    ...
    function _exists(uint tokenId)
        ...
        returns (bool)
    {
        return IMarketPositionManagement(market).existsTakerPosition(tokenId);
    }

    function ownerOf(uint tokenId) public view override returns (address) {
        return IMarketPositionManagement(market).takerPositionOwner(tokenId);
    }
    ...
    function _transfer(
        ...
        IMarketPositionManagement(market).safeTransferTakerPosition(
            from,
            to,
            tokenId
        );
    ...
    function balanceOf(address owner) public view override returns (uint) {
        return IMarket(market).getTakerPositions(owner).length;
    }
}
