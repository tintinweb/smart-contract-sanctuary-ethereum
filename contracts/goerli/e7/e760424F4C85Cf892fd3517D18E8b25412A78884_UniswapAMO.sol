...
import "../Uniswap/V3/IUniswapV3Factory.sol";
import "../Uniswap/V3/libraries/TickMath.sol";
import "../Uniswap/V3/libraries/LiquidityAmounts.sol";
...
contract UniswapAMO is Owned {
    ...
    modifier onlyStabilizers() {
       require(stabilizers[msg.sender] == true, "Only stabilizers");
        _;
    }
    ...
    function showAllocations() public view returns (uint256[4] memory allocations) {
        // All numbers given are in Sweep unless otherwise stated

        // Unallocated SWEEP
        allocations[0] = SWEEP.balanceOf(address(this));

        // Unallocated Collateral Dollar Value (E18)
        allocations[1] = freeColDolVal();

        // Sum of Uni v3 Positions liquidity, if it was all in SWEEP
        allocations[2] = TotalLiquiditySweep();
    ...
    // Needed for the Sweep contract to function
    function collatDollarBalance() public view returns (uint256) {
        // Get the allocations
        uint256[4] memory allocations = showAllocations();

        // Get the collateral and SWEEP portions
        uint256 collat_portion = allocations[1];
        uint256 sweep_portion = (allocations[0]).add(allocations[2]);

        return (collat_portion).add(sweep_portion);
    }

    function dollarBalances() public view returns (uint256 sweep_val_e18, uint256 collat_val_e18) {
        sweep_val_e18 = showAllocations()[3];
        collat_val_e18 = collatDollarBalance();
    }

    function TotalLiquiditySweep() public view returns (uint256) {
        uint256 sweep_tally = 0;
        ...
        for (uint256 i = 0; i < positions_array.length; i++) {
            ...
            if (this_liq > 0){
                ...
                if (thisPosition.collateral_address > 0x853d955aCEf822Db058eb8505911ED77F175b99e){ // if address(SWEEP) < collateral_address, then SWEEP is token0
                    sweep_tally = sweep_tally.add(LiquidityAmounts.getAmount0ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, this_liq));
                }
                else {
                    sweep_tally = sweep_tally.add(LiquidityAmounts.getAmount1ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, this_liq));
                }
            }
        }

        // Return the sum of all the positions' balances of SWEEP, if the price fell off the range towards that side
        return sweep_tally;
    }
    ...
    function mintedBalance() public view returns (int256) {
        return amo_minter.sweep_mint_balances(address(this));
    }
    ...
    function addLiquidity(address _tokenA, address _tokenB, int24 _tickLower, int24 _tickUpper, uint24 _fee, uint256 _amount0Desired, uint256 _amount1Desired, uint256 _amount0Min, uint256 _amount1Min) public onlyByOwnGov {
        ...
        require(allowed_collaterals[_tokenA] || _tokenA == address(SWEEP), "TokenA not allowed");
        require(allowed_collaterals[_tokenB] || _tokenB == address(SWEEP), "TokenB not allowed");
    ...
    */
    ...
    function refreshTargetPrice(uint _price) external onlyStabilizers {
        amo_minter.refreshTargetPrice(_price);
    }

    // Swap tokenA into tokenB using univ3_router.ExactInputSingle()
    // Uni V3 only
    function swapExactInput(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 _amountAtoB)  public onlyStabilizers returns (uint256 amountOut) {
        // Make sure the collateral is allowed
        // require(allowed_collaterals[_tokenA] || _tokenA == address(SWEEP), "TokenA not allowed");
        // require(allowed_collaterals[_tokenB] || _tokenB == address(SWEEP), "TokenB not allowed");
    ...
    function swapExactOutput(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 amountOut, uint256 amountInMaximum) public onlyStabilizers returns(uint256 amountIn) {
    ...
    function giveCollatBack(uint256 collat_amount) external {
        giveback_collateral.approve(address(amo_minter), collat_amount);
        //amo_minter.receiveCollatFromAMO(collat_amount);
    }

    // Burn unneeded or excess SWEEP. Goes through the minter
    function burnSweep(uint256 sweep_amount) public onlyByOwnGovCust {
        SWEEP.approve(address(amo_minter), sweep_amount);
        amo_minter.burnSweepFromAMO(address(this), sweep_amount);
    }
    ...
    function addCollateral(address collat_addr) public {
        require(collat_addr != address(0), "Zero address detected");
        require(collat_addr != address(SWEEP), "SWEEP is not collateral");
    ...
    function addOracle(address collat_addr, address oracle) public {
        require(collat_addr != address(0), "Zero address detected");
        require(collat_addr != address(SWEEP), "SWEEP is not collateral");
