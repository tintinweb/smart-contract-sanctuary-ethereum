...
// Human interaction creates and renews the unique experience of harmony in disorder.
...
contract Primes is ERC721r, Ownable {
    ...
    uint256 private constant MAX_SUPPLY = 1575 + 1; // ERC721r start at 0
    ...
    function mint(uint256 _quantity) external payable isUser {
        require(!paused, "Primes: contract is paused");
        // require(
    ...
    function sendGift(address _address, uint256 _quantity) public onlyOwner {
        uint256 supply = totalSupply();
        require(
            supply > MAX_SUPPLY - gifts,
            "Primes: after the end of the sale"
        );
    ...
    function tokenURI(
        ...
        uint primesCount = getNumberOfPrimes(_tokenId);
        ...
        for (y = 0; y <= 800; y += 100) {
            for (x = 0; x <= 800; x += 100) {
                ...
                if (isPrime(uint256(count))) {
                    ...
                    string memory start_or_end = setAnimationEvent(
                        index,
                        _tokenId
                    );
                    paths = concatenate(
