...
contract SweepersCompetitionVault is ReentrancyGuard, Ownable, IERC721Receiver, IERC1155Receiver, OpsReady {
    ...
    ) external payable onlySweepersTreasury nonReentrant returns (uint256) {
    ...
    function enrollReferrer(string calldata referralCode) external nonReentrant {
        require(referrerId[msg.sender].length == 0, 'User already enrolled');
        bytes32 bytesCode = keccak256(abi.encode(referralCode));
        require(referrer[bytesCode].referralCount == 0 && !referrer[bytesCode].isValidReferrer, 'referralCode already exists');
