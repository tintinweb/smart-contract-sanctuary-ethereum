...
contract ERC721 is ERC165, IERC721, IERC721Metadata, Ownable, DefaultOperatorFilterer {
     ...
     * Emits a {Transfer} event.
     */
    function _mintBatch(uint256 amount, address to, uint tokenId) internal {
...
// File: PADBriefcase.sol

contract PADBriefcase is Ownable, IERC2981, ERC721 {
    ...
    function mintFromReserve(uint amount, address to) external onlyOwner {
        ...
        require(amount + tokenId <= maxTotalSupply);
        _mintBatch(amount, to, tokenId);
    }
    ...
    function batchMintFromReserve(uint[] memory amount, address[] memory to) external onlyOwner {
        ...
        require(tokenId <= maxTotalSupply, "Exceeds max supply!");
    ...
    function mint() external payable {
        ...
        require(msg.value == mintPrice, "ETH Amount is not correct!");
        require(totalSupply < maxTotalSupply, "Request exceeds max supply!");
    ...
    function whitelistMint(bytes calldata sig) external {
        ...
        uint tokenId = totalSupply;
        require(tokenId < maxTotalSupply, "Request exceeds max supply!");
        require(!minted[msg.sender], "Request exceeds max per wallet!");
    ...
    function withdraw() onlyOwner external {
        (bool success, ) = payable(msg.sender).call{value: address(this).balance, gas: 3000}("");
        require(success, "Transfer to contract owner failed!");
    }

    /**
     * @notice toggles whitelist mint
     * @dev enables the whitelist functions.
     */
    ...
    function setMaxTotalSupply(uint24 _maxTotalSupply) external onlyOwner {
        maxTotalSupply = _maxTotalSupply;
    }
