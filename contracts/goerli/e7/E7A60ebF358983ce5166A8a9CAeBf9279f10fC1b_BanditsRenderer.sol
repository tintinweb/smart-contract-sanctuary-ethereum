...
contract BanditsRenderer is Ownable, ReentrancyGuard {
        ...
        returns (string memory)
    {
        ...
        Color memory bodyColor = getColor(
            HelperLib.parseInt(_hash) % 20
        );
        ...
        for (uint i; i < 6; i++) {
            ...
            if (i < layerNames.length - 1) {
                svgBytes.appendSafe(
                    abi.encodePacked(
                        ...
                        positions[i],
                        '" x="160" fill="',
                        traitIndex > 0 && traitIndex < 5 ? bodyColor.value : "#964b00",
                        '">',
                        values[i][traitIndex],
                        '</tspan>'
                    )
            ...
            if (i == layerNames.length) {
                uint maxHealth = HelperLib.parseInt(values[i][traitIndex]);
