...
contract MyEpicNFT is ERC721URIStorage {
  ...
  string[] backgroundColor = ["black", "red", "blue", "green", "purple", "orange"];
  ...
  function pickRandomBackgroundColor(uint256 tokenId) public view returns (string memory) {
    // pickRandomThirdWord 関数のシードとなる rand を作成します。
    uint256 rand = random(string(abi.encodePacked("BACKGROUND_COLOR", Strings.toString(tokenId))));
  ...
  function makeAnEpicNFT() public {
    ...
    require(newItemId < MAX_SUPPLY, "Sold Out!");

    // 3つの配列からそれぞれ1つの単語をランダムに取り出します。
    string memory first = pickRandomFirstWord(newItemId);
    string memory second = pickRandomSecondWord(newItemId);
    string memory third = pickRandomThirdWord(newItemId);
    string memory background = pickRandomBackgroundColor(newItemId);

	  // 3つの単語を連携して格納する変数 combinedWord を定義します。
    string memory combinedWord = string(abi.encodePacked(first, second, third));

    // 3つの単語を連結して、<text>タグと<svg>タグで閉じます。
    string memory finalSvg = string(abi.encodePacked(baseSvg1, background, baseSvg2, combinedWord, "</text></svg>"));
