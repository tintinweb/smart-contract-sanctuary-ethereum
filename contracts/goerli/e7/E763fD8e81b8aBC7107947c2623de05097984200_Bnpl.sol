...
contract Bnpl is NftReceiver, Ownable2Step, ReentrancyGuard {
    ...
    /**
     * @notice A mapping that states that a set of loanContract and loanCoordinator addresses are allowed to be used
     * by the BNPL.
     */
    mapping(bytes32 => bool) private allowedNFTfi;
    ...
    event NFTfiAllowance(address indexed loanContract, address indexed loanCoordinator, bool allowed);
    ...
    error Bnpl__setNFTfiAllowance_invalidLoanContractAddress();
    error Bnpl__setNFTfiAllowance_invalidLoanCoordinatorAddress();

    error Bnpl__setLoanCapBps_invalidLoanCapBps();

    error Bnpl__claimERC20_noTokens();

    error Bnpl__claimERC721_notOwner();

    error Bnpl__claimERC1155_noTokens();

    error Bnpl__execute_invalidAssetType();
    error Bnpl__execute_invalidModule();
    error Bnpl__execute_invalidNFTfi();
    ...
    /**
     * @notice This function can be called by admins to change the allowance status of set of NFTfi loanContract and
     * loanCoordinator contracts.
     * This includes both adding a the contracts to the allowance list and removing them.
     *
     * @param _loanContract - The address of the MarketModule contract whose allowance list status changed.
     * @param _loanCoordinator - The address of the MarketModule contract whose allowance list status changed.
     * @param _allowed - The new status of whether the module is allowed or not.
     */
    function setNFTfiAllowance(address _loanContract, address _loanCoordinator, bool _allowed) external onlyOwner {
        if (!_loanContract.isContract()) revert Bnpl__setNFTfiAllowance_invalidLoanContractAddress();
        if (!_loanCoordinator.isContract()) revert Bnpl__setNFTfiAllowance_invalidLoanCoordinatorAddress();

        allowedNFTfi[_nftfiKey(_loanContract, _loanCoordinator)] = _allowed;
        emit NFTfiAllowance(_loanContract, _loanCoordinator, _allowed);
    }
    ...
    /**
     * @notice This function can be called by admins claim ERC20 tokens miss deposited in this contract.
     * @param _token - address of the token contract for the token to be sent out
     * @param _receiver - receiver of the token
     */
    function claimERC20(address _token, address _receiver) external onlyOwner {
        IERC20 token = IERC20(_token);
        uint256 amount = token.balanceOf(address(this));
        if (amount == 0) revert Bnpl__claimERC20_noTokens();
        token.safeTransfer(_receiver, amount);
    }

    /**
     * @notice This function can be called by admins claim ERC721 tokens miss deposited in this contract.
     * @param _token - address of the token contract for the token to be sent out
     * @param _id - id token to be sent out
     * @param _receiver - receiver of the token
     */
    function claimERC721(
        ...
        if (token.ownerOf(_id) != address(this)) revert Bnpl__claimERC721_notOwner();
        token.safeTransferFrom(address(this), _receiver, _id);
    }

    /**
     * @notice This function can be called by admins claim ERC1155 tokens miss deposited in this contract.
     * @param _token - address of the token contract for the token to be sent out
     * @param _id - id token to be sent out
     * @param _receiver - receiver of the token
     */
    function claimERC1155(
        ...
        uint256 amount = token.balanceOf(address(this), _id);
        if (amount == 0) revert Bnpl__claimERC1155_noTokens();
        token.safeTransferFrom(address(this), _receiver, _id, amount, "");
    }
    ...
    function execute(Execution calldata _params) external nonReentrant returns (bool) {
        ...
        if (!isNFTfiAllowed(_params.loanContract, _params.loanCoordinator)) revert Bnpl__execute_invalidNFTfi();
    ...
    function isNFTfiAllowed(address _loanContract, address _loanCoordinator) public view returns (bool) {
        return allowedNFTfi[_nftfiKey(_loanContract, _loanCoordinator)];
    }
    ...
    function _nftfiKey(address _loanContract, address _loanCoordinator) internal pure returns(bytes32) {
        return keccak256(abi.encodePacked(_loanContract, _loanCoordinator));
    }
}
