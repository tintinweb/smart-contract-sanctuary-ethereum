...
contract RewardPool is IRewardPool, OwnableUpgradeable, AbstractDependant {
    ...
    address public deinNativeStakingAddress;
    address public deinLPStakingAddress;
    address public deinCoverStakingAddress;

    uint256 public stblDecimals;

    uint256 public lastUpdateTime; // end of period rebalanced
    uint256 private _dailyReward;
    mapping(address => uint256) private _stakingRatio; // staking contract -> percent of reward

    event DailyRewardSet(uint256 monthlyReward, uint256 timestamp);
    event StakingRatioSet(uint256 nativeRatio, uint256 LPRatio, uint256 XCoverRatio);
    event Refilled(uint256 amountRefill);
    event Rebalanced(address indexed stakingAddress, uint256 rewardStakingEpoch);

    function __RewardPool_init() external initializer {
        __Ownable_init();
    ...
    function setDependencies(IContractsRegistry _contractsRegistry)
        ...
        onlyInjectorOrZero
    {
        dein = IERC20(_contractsRegistry.getDEINContract());
        deinNativeStakingAddress = _contractsRegistry.getDEINStakingContract();
        deinLPStakingAddress = _contractsRegistry.getLiquidityMiningStakingETHContract();
        // TODO DEINXCOVER STAKING
        // deinCoverStakingAddress = _contractsRegistry.getDEINXCoverStakingContract();
    ...
    modifier onlyMaintainer() {
        require(_msgSender() == maintainer, "RewardPool: Not maintainer");
        _;
    }

    modifier onlyClaimingRegistry() {
        require(_msgSender() == address(claimingRegistry), "RewardPool: Not CR");
        _;
    }

    function setMaintainer(address maintainerAddress) external onlyOwner {
        maintainer = maintainerAddress;
    }

    /// @param dailyReward passed with precision 10**25
    function setDailyReward(uint256 dailyReward) external onlyOwner {
        require(
            dailyReward > 0 && dailyReward < 100 * PRECISION,
            "RewardPool: Wrong distribution"
        );
        _dailyReward = dailyReward;

        emit DailyRewardSet(dailyReward, block.timestamp);
    }

    /// @param nativeRatio passed with precision 10**25
    /// @param LPRatio passed with precision 10**25
    /// @param XCoverRatio passed with precision 10**25
    function setPercentPerStaking(
        uint256 nativeRatio,
        uint256 LPRatio,
        uint256 XCoverRatio
    ) external onlyOwner {
        require(
            nativeRatio.add(LPRatio).add(XCoverRatio) == 100 * PRECISION,
            "RewardPool: Wrong distribution"
        );
        _stakingRatio[deinNativeStakingAddress] = nativeRatio;
        _stakingRatio[deinLPStakingAddress] = LPRatio;
        _stakingRatio[deinCoverStakingAddress] = XCoverRatio;

        emit StakingRatioSet(nativeRatio, LPRatio, XCoverRatio);
    }

    function rebalanceStakingPools() external override onlyMaintainer {
        uint256 elapsedDays = _calculateElapsedDays();
        lastUpdateTime = block.timestamp;
        ...
        _sendToStaking(deinNativeStakingAddress, rewardEpoch, elapsedDays);
        _sendToStaking(deinLPStakingAddress, rewardEpoch, elapsedDays);
        // TODO DEINXCOVER STAKING
        // _sendToStaking(deinCoverStakingAddress, rewardEpoch, elapsedDays);
    }

    function _calculateElapsedDays() internal view returns (uint256 elapsedDays) {
        uint256 elapsedTime = block.timestamp.sub(lastUpdateTime).mul(PRECISION);
        elapsedDays = elapsedTime.div(1 days);
    }

    function _sendToStaking(
        address stakingAddress,
        uint256 rewardEpoch,
        uint256 elapsedDays
    ) internal {
        uint256 rewardStakingEpoch =
            rewardEpoch.mul(_stakingRatio[stakingAddress]).div(PERCENTAGE_100);

        dein.safeTransfer(stakingAddress, rewardStakingEpoch);
        IStaking(stakingAddress).setRewards(rewardStakingEpoch, elapsedDays);

        emit Rebalanced(stakingAddress, rewardStakingEpoch);
    }

    function rebalanceVoteReward(uint256 claimIndex)
        external
        override
        onlyClaimingRegistry
        returns (uint256 rewardAmount)
    {
        IClaimingRegistry.ClaimStatus claimStatus = claimingRegistry.getClaimStatus(claimIndex);

        IClaimingRegistry.ClaimProvenance memory claimProvenance =
            claimingRegistry.getClaimProvenance(claimIndex);

        uint256 cover;
        uint256 protocolFeeUSDT;
        if (claimProvenance.provenance == IClaimingRegistry.Provenance.POLICY) {
            (cover, , , , protocolFeeUSDT) = IPolicyBook(claimProvenance.bookAddress)
                .policyHolders(claimProvenance.claimer);
        } else if (claimProvenance.provenance == IClaimingRegistry.Provenance.DEMAND) {
            (, , , , protocolFeeUSDT, ) = IDemandBook(claimProvenance.bookAddress)
                .getDemandAmountInfo(claimProvenance.demandIndex);
        }

        /// @dev got from calculation because already modified in storage at rewarding calculator
        (, uint256 lockedAmount, ) = claimingRegistry.getClaimAmounts(claimIndex);
        uint256 protocolFeeDEIN =
            IPriceFeed(priceFeed).howManyDEINsInUSDT(
                DecimalsConverter.convertFrom18(protocolFeeUSDT, stblDecimals)
            );

        rewardAmount = Math.min(protocolFeeDEIN, lockedAmount);

        if (claimStatus == IClaimingRegistry.ClaimStatus.ACCEPTED) {
            dein.transfer(address(claimingRegistry), rewardAmount);
        }
    }
}
...
interface IStaking {
    ...
    function lockingPeriods(uint256 index) external view returns (uint256);
    ...
    function getTotalStakedAmountByStaker(address _user)
        external
        view
        returns (uint256 totalStaked, uint256 totalStakedByLiquidation);
    ...
    function getStakerMultiplier(uint256 _stakingIndex) external view returns (uint256);

    function getStakerLockingDuration(uint256 _stakingIndex) external view returns (uint256);
