...
contract LLCollection is ERC721Enumerable, Ownable, RoyaltiesV2Impl, Pausable {
    ...
    constructor(
        ...
        address _escrow,
        address _secondary
    ) ERC721("Legendary Looks", "LELO") {
        require(
            _escrow != address(0),
            "Legendary Looks: escrow is the zero address"
        );
        require(
            _secondary != address(0),
            "Legendary Looks: secondary is the zero address"
        );
        blindURI = _blindURI;
    ...
    function updateEscrow(address _newEscrow) external onlyOwner {
        require(
            ...
            "Legendary Looks: new escrow is the zero address"
        );
    ...
    function updateSecondary(address _newSecondary) external onlyOwner {
        require(
            ...
            "Legendary Looks: new secondary is the zero address"
        );
    ...
    function claim() external onlyOwner {
        ...
        require(sent, "Legendary Looks: Failed to send Ether");
    }
    ...
    function mintNFT(uint256 _numOfTokens) public payable whenNotPaused {
        require(
            msg.value >= NFT_PRICE * _numOfTokens,
            "Legendary Looks: Ether value sent is not correct"
        );
        ...
        returns (string memory)
    {
        require(
            _exists(_tokenId),
            "Legendary Looks: URI query for nonexistent token"
        );
        if (!reveal) {
            return blindURI;
        } else {
