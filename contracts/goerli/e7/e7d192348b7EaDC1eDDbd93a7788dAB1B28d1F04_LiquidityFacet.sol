...
contract LiquidityFacet is ILiquidity, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        // Check that `msg.sender` owns the corresponding `_amount` of short and
        // long position tokens. In particular, this check will revert when a
        // user tries to remove an amount that exceeds the overall position token
        // supply which is the maximum amount that a user can own
        if (
            shortToken.balanceOf(msg.sender) < _amount ||
            longToken.balanceOf(msg.sender) < _amount
        ...
        if (_fees.protocolFee > 0) {
            ...
            _protocolFee = LibDIVA._calcFee(
                _fees.protocolFee,
                _amount,
                IERC20Metadata(_pool.collateralToken).decimals()
            );
            // User to increase `_amount` if fee is 0
        ...
        if (_fees.settlementFee > 0) {
            ...
            _settlementFee = LibDIVA._calcFee(
                _fees.settlementFee,
                _amount,
                IERC20Metadata(_pool.collateralToken).decimals()
            );
        ...
        shortToken.burn(msg.sender, _amount);
        longToken.burn(msg.sender, _amount);
        ...
        // `collateralBalance` is reduced inside `_returnCollateral`.
        LibDIVA._returnCollateral(
            _poolId,
            _pool.collateralToken,
            msg.sender,
            _amount - _protocolFee - _settlementFee
        );
...
interface ILiquidity {
    // Thrown if return collateral is paused
    error ReturnCollateralPaused();

    // Thrown if status of `finalReferenceValue` is already "Confirmed"
    error FinalValueAlreadyConfirmed();

    // Thrown if a user's short or long position token balance is smaller
    // than the indicated amount in `removeLiquidity`
...
interface IPositionTokenFactory {
     ...
     * @param _name Name string of the position token.
     * @param _symbol Symbol string of position token.
