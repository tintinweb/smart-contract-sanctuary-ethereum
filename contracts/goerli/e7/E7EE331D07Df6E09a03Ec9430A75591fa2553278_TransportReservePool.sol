...
contract TransportReservePool is Ownable, ReentrancyGuard {
    // Staking Contracts
    address public transportStaking;
    ...
    constructor(
        address _transportStakingAddress,
        address _bonusPackStakingAddress,
        IERC20 _rewardToken
    ) {
        transportStaking = _transportStakingAddress;
    ...
    // Get total transport weight of user
    ...
    // Get total transport weight of user
    function getTotalTransportWeightByUser(address user)
        external
        view
        returns (uint256)
    {
        return _getTotalTransportWeightByUser(user);
    }
    ...
    function getTransportWeight(uint256 tokenId)
        external
        pure
        returns (uint256)
    {
        return _getTransportWeight(tokenId);
    }
    ...
    function _getClaimableReward(address user) internal view returns (uint256) {
        // Get reward amount
        uint256 totalTransportWeightOfUser = _getTotalWeightByUser(user);

        // No reward if total transport weight is smaller than user reward
        if (
            totalTransportWeightOfUser >=
    ...
    function _getTotalWeightByUser(address user)
        ...
        returns (uint256)
    {
        uint256 totalTransportWeight = _getTotalTransportWeightByUser(user);
        uint256 bpm = _getBonusMultiplierByUser(user);
        uint256 totalWeight = totalTransportWeight * bpm;
        return totalWeight;
    }

    // Get total transport weight by user
    function _getTotalTransportWeightByUser(address user)
        internal
        view
        returns (uint256)
    {
        uint256 balance = IERC721Enumerable(transportStaking).balanceOf(user);
        uint256 totalWeight = 0;
        for (uint256 idx = 0; idx < balance; idx++) {
            uint256 token = IERC721Enumerable(transportStaking)
                .tokenOfOwnerByIndex(user, idx);
            uint256 stakedFrom = INFTStaking(transportStaking)
            ...
            if (monthlyRewards[currentRewardIndex].snapshot > stakedFrom) {
                ...
                uint256 weight = _getTransportWeight(token);
    ...
    // Get Transport Weight by transport token ID
    function _getTransportWeight(uint256 tokenId)
        internal
        pure
        returns (uint256)
    {
        ...
        if (tokenId <= 400) {
            ...
            weight = 500000;
        } else if (tokenId <= 1800) {
            weight = 100000;
        } else if (tokenId <= 3350) {
            weight = 50000;
        } else if (tokenId <= 7350) {
            weight = 10000;
        }
        return weight;
    }
