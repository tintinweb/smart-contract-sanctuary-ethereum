    ...
    UUPSUpgradeable
{
    ...
    event VoteCreated(
        ...
        VoteSchedule schedule,
        Governance.VotingCriteria criteria
    );
    ...
    /**
     * Call for a vote and inform {Token} contract to create a checkpointId
    ...
    function createVote(
        uint256 voteId,
        VoteSchedule calldata schedule,
        Governance.VotingCriteria calldata criteria
     ...
     */
    function modifyVote(uint256 voteId, VoteSchedule calldata schedule)
        public
        onlyRole(GOVERNOR_ROLE)
    {
        _votes[voteId].schedule = schedule;

        emit VoteModified(voteId, _msgSender(), schedule);
    }
    ...
    function getVote(uint256 voteId)
        ...
        returns (
            ...
            VoteCount memory,
            uint256 totoalVotingWeight
        )
    {
        return (
            _votes[voteId].checkpointId,
            _votes[voteId].schedule,
            _votes[voteId].criteria,
            _votes[voteId].count,
            token.checkpointedTotalVotingWeight(_votes[voteId].checkpointId)
        );
    }

    function getVotingWeight(address voter, uint256 voteId)
        public
        view
        returns (uint256)
    {
        return
            token.checkpointedVotingWeight(voter, _votes[voteId].checkpointId);
    }

    // Obtain the vote choice of an account for a vote
    function hasVoted(uint256 voteId, address account)
        public
        view
        returns (VoteChoice)
    {
        return _votes[voteId].hasVoted[account];
    }

    // An account can make a transaction to cast a vote
    function castVote(uint256 voteId, VoteChoice choice)
        public
        returns (uint256)
    {
    ...
    // Generate a vote certificate which can be signed by any voter and cast by a different account
    function generateVoteCertificate(
        uint256 voteId,
        uint8 choice,
        uint256 expiry
    ...
    function castVoteBySig(
        ...
        return _castVote(voter, voteId, _msgSender(), VoteChoice(choice));
    }
    ...
    function _castVote(
        ...
        Vote storage vote = _votes[voteId];
        require(
            block.timestamp >= vote.schedule.start &&
                block.timestamp <= vote.schedule.end,
    ...
    function _countVote(
        ...
        require(
            vote.hasVoted[voter] == VoteChoice.NotVoted,
            "Voting: vote already cast"
        );
        vote.hasVoted[voter] = choice;

        if (choice == VoteChoice.Nay) {
            vote.count.nay += weight;
        } else if (choice == VoteChoice.Yea) {
            vote.count.yea += weight;
        } else if (choice == VoteChoice.Abstain) {
            vote.count.abstain += weight;
        } else {
    ...
    /**
     * Delegate the voting weight to another account
     */
    function delegate(address delegatee) public {
        token.delegate(_msgSender(), delegatee);
    }
    ...
    function delegateBySig(
        ...
        address signer = ECDSAUpgradeable.recover(
            _hashTypedDataV4(
                keccak256(
                    abi.encode(DELEGATION_TYPEHASH, delegatee, expiry, nonce)
                )
            ),
            v,
            r,
        ...
        token.delegate(signer, delegatee);
    }
     ...
     */
    function getVoteCountThresholds(uint256 voteId)
        public
        view
        returns (
            uint256 quorum,
            uint256 approvalYea,
            uint256 earlyApprovalYea,
            uint256 earlyRejectionNay
        )
    {
        Vote storage vote = _votes[voteId];
        uint256 totalVotingWeight = token.checkpointedTotalVotingWeight(
            vote.checkpointId
        );
        return (
            _percentage(totalVotingWeight, vote.criteria.quorumNumerator),
            _percentage(
                vote.count.nay + vote.count.yea,
                vote.criteria.approvalRateNumerator
            ),
            _percentage(
                totalVotingWeight - vote.count.abstain,
                vote.criteria.approvalRateNumerator
            ),
            _percentage(
                totalVotingWeight - vote.count.abstain,
                DENOMINATOR - vote.criteria.approvalRateNumerator
            )
        );
    }
     ...
     *
     * Quorum is satisfied when ((#Yea + #Nay + #Abstain) / #Total) >= quorum (inclusive)
     * Approval rate is satified when (#Yea / (#Yea + #Nay)) >= approval rate (inclusive)
     * If "strictly greater than" is desired, the approaval rate can be calculated as {math.floor((#Yea + 1) / (#Yea + #Nay))}
     * A draw is considered to be an approved vote where (#Yea / (#Yea + # No)) == approval rate
     */
    function state(uint256 voteId) public view returns (VoteState) {
        Vote storage vote = _votes[voteId];
        if (block.timestamp < vote.schedule.start) {
        ...
        if (
            block.timestamp > vote.schedule.end &&
            (vote.count.nay + vote.count.yea + vote.count.abstain) >=
            _percentage(totalVotingWeight, vote.criteria.quorumNumerator) &&
            vote.count.yea >=
            _percentage(
                vote.count.nay + vote.count.yea,
    ...
    UUPSUpgradeable
{
    ...
    /**
     * Owner is defined as either the address that holds all the tokens of a token ID or {address(0)}
     */
    ...
    function _tokenTransfer(
        ...
        require(amount > 0, "Token: transfer amount must be greater than 0");
        if (
            from != address(0) &&
            from != _msgSender() &&
        ...
        if (_accountBalances[to][tokenId] == _totalSupplies[tokenId]) {
            _owners[tokenId] = to;
    ...
    UUPSUpgradeable
{
    ...
    bytes32 public constant PROPOSER_ROLE = keccak256("PROPOSER_ROLE");
    // {EXECUTOR_ROLE} can execute a approved proposal if an execution is attached
    bytes32 public constant EXECUTOR_ROLE = keccak256("EXECUTOR_ROLE");
    ...
    /**
     * Emitted when a proposal is updated.
     */
    ...
    /**
     * Indicates the change of the voting criteria
     */
     ...
     *
     * We refer to the meta state {"Approved"} as being either {Queued} or {Executable}
     *
    ...
    /**
     * The value of the {state} of ta proposal can not be trusted unless that state can only be explicitly set and
     * the following state can also only be explicitly set (or there must not be a following state).
     *
     * {executionTime} is the time the proposal becomes executable (if approved and not cancelled)
     *
     * {nonce} is used to ensure the {executionHash} of any proposal is unique
     *
     * The eventual state of a proposal is set when {Canceled} or {Executed} and can be trusted as it is,
     * while the states {Pending}, {Active}, {Rejected}, and {"Approved"} ({Executable} or {Queued}) can only be inferred.
     *
     * {Nonexistent} is also trusted since it are explicitly set and
     * the following state is explicitly set as well.
     */
    struct Proposal {
        address proposer;
        uint256 executionTime;
     ...
     * {vales} are the ether amounts to be transferred to the targets
     * {calldatas} are the calldata of the message call
     * {descriptionHash} is the hash of the description of the proposal
     * Ideally the description is in JSON format and include the date and time (as salt)
     */
    struct Execution {
        address[] targets;
        uint256[] values;
    ...
    /**
     * A vote starts at {voteStart} and ends at {voteEnd} inclusively and voters
     * can still cast their ballot at time {voteEnd}
     * {executionTime} is the time when the proposal becomes executable if the vote
     * succeeds and the proposal is not cancelled
     *
     */
    struct ProposalSchedule {
        uint256 voteStart;
        uint256 voteEnd;
        uint256 executionTime;
    }
    ...
    /**
     * quorum = (Yea + Nay + Abstain) / total number of voters
     * approval rate = Yea / (Yea + Nay)
     */
    ...
    string private _uri;
    // {_nonce} is a counter to ensure each proposal is unique
    ...
    /**
     * The sender is required to have the {role} or have over threshold number of governance tokens
     ...
     */
    function _groupAccessControlWithThreshold(bytes32 role) private view {
        require(
            (accessControl.hasRole(role, ANYONE_GROUP_ADDRESS) &&
                token.balanceOf(_msgSender(), 0) >= _proposalThreshold) ||
                accessControl.hasRole(role, _msgSender()),
    ...
    /**
     * Obtain the current nonce of the governance contract, the nonce can be used to
     * calculate the proposal ID by calling {hashProposal}
     */
     ...
     */
    function hashProposal(Execution calldata execution, uint256 nonce)
        public
        pure
        returns (bytes32)
    {
     ...
     * An {Approved} vote might be an {executable} or {Queued} proposal
     */
    ...
    /**
     * Create a proposal with the provided {votingCriteria}
     */
    function dictate(
        Execution calldata execution,
        ProposalSchedule calldata proposalSchedule,
        VotingCriteria calldata votingCriteria
    ) public onlyRole(GOVERNOR_ROLE) returns (uint256) {
        return _createProposal(execution, proposalSchedule, votingCriteria);
    }

    /**
     * Create a proposal with the set {votingCriteria} on the governance contract
     */
    function propose(
        Execution calldata execution,
        ProposalSchedule calldata proposalSchedule
    ) public returns (uint256) {
        _groupAccessControlWithThreshold(PROPOSER_ROLE);
        return
            _createProposal(execution, proposalSchedule, getVotingCriteria());
    }
    ...
    function _createProposal(
        ...
        bytes32 executionHash = hashProposal(execution, _nonce);
        ...
        voting.createVote(
            proposalId,
            Voting.VoteSchedule(
                proposalSchedule.voteStart,
                proposalSchedule.voteEnd
            ),
            votingCriteria
        );
        proposal.proposer = _msgSender();
        proposal.executionTime = proposalSchedule.executionTime;
        proposal.executionHash = executionHash;
        // Increase the nonce counter afterwards
        proposal.nonce = _nonce++;

        emit ProposalCreated(
            proposalId,
            _msgSender(),
            proposalSchedule.executionTime,
            execution
        );

        if (block.timestamp < proposalSchedule.voteStart) {
    ...
    /**
     * Original proposer of a proposal can amend the proposal provided the proposal is still {Pending}
     */
    function amend(
        uint256 proposalId,
        Execution calldata execution,
        ...
        // To amend a proposal, the original proposer must satify the the requirements to propose
        _groupAccessControlWithThreshold(PROPOSER_ROLE);
        require(
            state(proposalId) == ProposalState.Pending,
            "Governance: only pending proposals can be amended"
        );
        require(
            _msgSender() == _proposals[proposalId].proposer,
            "Governance: original proposer only"
        );
        _modifyProposal(proposalId, execution, proposalSchedule);
    }

    /**
     * {GOVERNOR_ROLE} can revise a proposal if it is not {Executed} or {Canceled}
     * (or equivalently if the proposal is {Pending}, {Active}, {Rejected}, {Executable} or {Queued})
     */
    function revise(
        uint256 proposalId,
        Execution calldata execution,
        ...
        require(
            ...
            "Governance: the proposal has eventuated"
        );
        _modifyProposal(proposalId, execution, proposalSchedule);
    }

    function _modifyProposal(
        uint256 proposalId,
        Execution calldata execution,
        ProposalSchedule calldata proposalSchedule
    ) private {
        ...
        _proposals[proposalId].executionHash = hashProposal(execution, _nonce);
        _proposals[proposalId].nonce = _nonce++;
        _proposals[proposalId].executionTime = proposalSchedule.executionTime;
    ...
    /**
     * {Approved} and {Queued} proposals can become {Executable}, which can be executed
     *
        ...
        returns (uint256)
    {
        ...
        Proposal storage proposal = _proposals[proposalId];
        require(
            proposal.executionHash == hashProposal(execution, proposal.nonce),
