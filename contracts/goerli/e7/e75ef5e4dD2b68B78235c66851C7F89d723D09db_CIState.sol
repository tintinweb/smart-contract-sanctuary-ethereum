...
contract CIState is State {
        ...
        )
    {
        uint papsInAsset;
        uint papsInStable;
        (papsInAsset, papsInStable, _lastPrice, _lastTimestamp, _lastRoundId, ) = _calcNewPaps();

        if(RWAP > 0){
            cia = ciTakerAsset + papsInAsset * DIVIDER_ASSET / RWAP;
            cis = ciTakerStable + papsInStable * DIVIDER_ASSET / RWAP;
        } else {
            cia = ciTakerAsset;
            cis = ciTakerStable;
        }
    ...
    {
        (int _price, uint currentTimestamp, uint80 currentId) = price();

        _lastPrice = lastPrice;
        _lastTimestamp = lastTimestamp;

        if (currentId <= lastRoundId || currentTimestamp <= lastTimestamp)
            return (0, 0, _lastPrice, _lastTimestamp, lastRoundId, probabilityOfClaim);

        if (RWAP == 0) return (0, 0, _price, currentTimestamp, currentId, probabilityOfClaim);

        uint pap;

        for (uint80 i = lastRoundId + 1; i <= currentId; i++) {
            (int iPrice, uint iTimestamp) = priceAt(i);
            if (iTimestamp > _lastTimestamp) {
                (pap, _probabilityOfClaim) = calc.PAP(iPrice, _lastPrice, iTimestamp, _lastTimestamp);
                papsInAsset += (iTimestamp - _lastTimestamp) * pap / 1 days;
                papsInStable += uint(iPrice) * papsInAsset * DIVIDER_STABLE / DIVIDER_ORACLE;

                _lastTimestamp = iTimestamp;
                _lastPrice = iPrice;
            }
        }

        return (papsInAsset, papsInStable, _lastPrice, _lastTimestamp, currentId, _probabilityOfClaim);
    }

    /// @inheritdoc IState
    function update() public override onlyMarket {

        uint papsInAsset;
        uint papsInStable;
        (papsInAsset, papsInStable, lastPrice, lastTimestamp, lastRoundId, probabilityOfClaim) = _calcNewPaps();

        if(RWAP > 0){
            ciTakerAsset += papsInAsset * DIVIDER_ASSET / RWAP;
            ciTakerStable += papsInStable * DIVIDER_ASSET / RWAP;

            AP -= papsInAsset;
            AR += papsInAsset;

            B -= papsInAsset;
            L -= RWL * papsInAsset / RWAP;
        }

        Y += D * calc.YIELD_EPSILON() / UDIVIDER;
    }
    ...
    function newTakerPosition(
        ...
        AP += assetAmount;
        RWAP += (assetAmount * rate) / UDIVIDER;
    ...
    function closeTakerPosition(
        ...
        if (AP > assetAmount) {
            AP -= assetAmount - premium;
            if (assetAmount - premium > payout.assetAmount){
                AR += assetAmount - premium - payout.assetAmount;
            }
        } else {
            AR = AR + AP - payout.assetAmount;
            AP = 0;
        }

        if (payout.stableAmount > 0) CP -= payout.stableAmount;

        uint rate = calc.getTakerRate(risk, term);

        RWAP -= (assetAmount * rate) / DIVIDER_ASSET;
        RWL -= (floor * assetAmount * rate) / UDIVIDER / DIVIDER_ASSET;
    ...
    function newMakerPosition(
        ...
        RWCPp += stableAmount * positiveRate / DIVIDER_STABLE;
        RWCPn += stableAmount * negativeRate / DIVIDER_STABLE;
    }
    ...
    function closeMakerPosition(
        ...
        uint partOfDebt = uint(int(stableAmount) + yield) * DIVIDER_STABLE / D;

        CP -= payout.stableAmount;
        D -= stableAmount;
        if (payout.assetAmount > 0) {
            if (AP > payout.assetAmount) {
                AP -= payout.assetAmount;
            } else {
                AR = AR + AP - payout.assetAmount;
                AP = 0;
            }
    ...
    IState
{
    ...
    /// @inheritdoc IState
    uint public override RWCPp;

    /// @inheritdoc IState
    uint public override RWCPn;
        ...
        returns (MarketState memory)
    {
        return
            MarketState({
                ...
                Y: (DIVIDER * int(Y)) / int(DIVIDER_STABLE),
                RWAP: (DIVIDER * int(RWAP)) / int(DIVIDER_ASSET)
            });
    }
...
interface IState {
    ...
    function RWCPp() external view returns (uint);

    /// @return current risk weighted capital value with negative rate
    function RWCPn() external view returns (uint);
    ...
    /// @notice updates state variables by a provided alghoritm
    ...
    /// @return lastPrice last price from the price feed
    /// @return lastTimestamp last update timestamp from the price feed
    /// @return lastRoundId last update roundId from the price feed
    function calcCI()
        external
        view
        returns (
...
interface IRiskCalculation is ITakerPositionRate, IMakerPositionRate, IYieldCalculation {
    function PAP(
        int currentPrice,
        int previousPrice,
        uint curetnTime,
        uint previousTime
    ) external view returns (uint _PAP, uint _probabilityOfClaim);
    ...
    function boostParameters(
        int currentPrice,
        uint currentTimeStamp,
        int previousPrice,
        uint previousTimeStamp
    ) external returns (BoostingParameters memory);
}
...
abstract contract TakerPositionRate is ITakerPositionRate {
    uint public constant M_30_80 = 0.035718046987284806 * (10**18);
    uint public constant M_60_80 = 0.07342024995668095 * (10**18);
    uint public constant M_90_80 = 0.10463304916186243 * (10**18);

    uint public constant M_30_90 = 0.07351808097304989 * (10**18);
    uint public constant M_60_90 = 0.12069098118177554 * (10**18);
    uint public constant M_90_90 = 0.1571040164882781 * (10**18);

    uint public constant M_30_95 = 0.09875150395875265 * (10**18);
    uint public constant M_60_95 = 0.1489907700572694 * (10**18);
    uint public constant M_90_95 = 0.1871733012350463 * (10**18);
    ...
    function getTakerRate(uint16 risk, uint16 term)
        ...
        returns (uint)
    {
        if (risk == 8000 || risk == 2000) {
            if (term == 30) return M_30_80;
            if (term == 60) return M_60_80;
            if (term == 90) return M_90_80;
        } else if (risk == 9000 || risk == 1000) {
            if (term == 30) return M_30_90;
            if (term == 60) return M_60_90;
            if (term == 90) return M_90_90;
        } else if (risk == 9500 || risk == 500) {
            if (term == 30) return M_30_95;
            if (term == 60) return M_60_95;
            if (term == 90) return M_90_95;
        }
...
struct MarketState {
    ...
    int Y; // Yield target value with DIVIDER precision (can be negative)
    int RWAP; // Risk weighted asset pool
}
