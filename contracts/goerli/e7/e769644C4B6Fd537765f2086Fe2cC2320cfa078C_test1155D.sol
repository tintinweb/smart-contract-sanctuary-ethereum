...
contract test1155D is ERC1155Upgradeable, IERC2981 { /*TODO: 1155D*/

    bytes32 public merkleSeed;
    ...
    uint256 public constant maxNFTWhitelist = 5;

    uint256 public constant maxNFTPublic = 5;
...
function initialize(string memory _metadata, bytes32 _seed, string memory _contractURI) virtual public initializer {
    admin = msg.sender;
    id = 1;
    paused = true;
...
function mint(uint256 _amount) public payable notPaused {
    require(id < totalSupply, "Error: max supply reached");
    require((id + _amount) <= totalSupply, "Error: more than max supply will be reached");
    require(_amount <= maxNFTPublic, "Error: you can't mint more than max public supply");
...
function mintWhitelist(bytes32[] calldata _merkleProof, uint256 _amount) public payable notPaused {
    require(whitelistPaused == false, "Error: whitelist mint is paused");
    require(WLClaimed[msg.sender] == false, "Error: you have already claimed your whitelist");
    require(id < totalSupply, "Error: max supply reached");
    require((id + _amount) <= totalSupply, "Error: more than max supply will be reached");
    require(_amount <= maxNFTWhitelist, "Error: you can't mint more than max whitelist supply");
    require(msg.value >= whitelistPrice * _amount, "Error: insufficient funds");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProofUpgradeable.verify(_merkleProof, merkleSeed, leaf), "Error: you are not on the whitelist");
...
function checkWhitelisted(bytes32[] calldata _merkleProof) virtual public view returns (bool) {
    return (MerkleProofUpgradeable.verify(_merkleProof, merkleSeed, keccak256(abi.encodePacked(msg.sender))));
}

function setSeed(bytes32 _seed)virtual public onlyAdmin {
    require(merkleSeed != _seed, "Error: seed already set");
    merkleSeed = _seed;
}
...
function setPaused(bool _paused) virtual public onlyAdmin {
    require(paused != _paused, "Error: paused already setted");
    paused = _paused;
}
...
receive() external payable { /**TODO: splitter? */
...
//import "openzeppelin-contracts-upgradeable/contracts/utils/introspection/IERC165Upgradeable.sol";
