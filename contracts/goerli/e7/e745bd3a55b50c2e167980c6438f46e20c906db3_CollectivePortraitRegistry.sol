    ...
    Pausable
{
    ...
    */
    ...
    function mintPortrait(
        ...
        tokenHoldersByTokenId[portraitId].push(msg.sender);
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (
                ...
                for (uint256 j = 0; j < tokenHoldersByTokenId[id].length; j++) {
                    address tokenHolder = tokenHoldersByTokenId[id][j];
                    if (tokenHolder != from) {
                        if (
                ...
                if (balanceOf(from, id) == 0) {
                    for (
                        ...
                        if (tokenHoldersByTokenId[id][j] == from) {
                            delete tokenHoldersByTokenId[id][j];
                            break;
                        }
                    }
                }
    ...
    */
    ...
    /*
    Function to do a multiplier stock split for a token ID, only if the user has the minimumControlShares
    ...
    */
    function stockSplit(uint256 _id, uint256 _multiplier) public whenNotPaused {
        // Require the user to have the minimumControlShares
        require(
            records[getNameByTokenId(_id)].minimumControlShares <=
                balanceOf(msg.sender, _id),
        ...
        for (uint256 i = 0; i < tokenHoldersByTokenId[_id].length; i++) {
            uint256 addedShares = _multiplier *
                balanceOf(tokenHoldersByTokenId[_id][i], _id) -
                balanceOf(tokenHoldersByTokenId[_id][i], _id);

            // Mint the new shares
            _mint(tokenHoldersByTokenId[_id][i], _id, addedShares, "");
        }
