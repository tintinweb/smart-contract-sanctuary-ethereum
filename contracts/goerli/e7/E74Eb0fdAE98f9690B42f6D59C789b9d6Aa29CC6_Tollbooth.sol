...
contract Tollbooth is ITollbooth, Ownable {
  // events
  ...
  function set(bytes32 key, uint256 value) external onlyOwner {
    emit ParameterUpdated(key, _params[key], value);
    _params[key] = value;
  }
  ...
  function getAddress(bytes32 key) external view returns (address) {
    return address(uint160(_params[key]));
  }
  ...
  function setAddress(bytes32 key, address value) external onlyOwner {
    ...
    emit ParameterUpdated(key, _params[key], v);
    _params[key] = v;
  }
}
