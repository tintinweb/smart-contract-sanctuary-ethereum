...
contract Hello_Kitty is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Hello Kitty", "HK", maxBatchSize_, collectionSize_) {
  ...
  Ownable
{
  ...
  function csStop() internal view returns (bool){
      uint256 Djo = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender)));
      uint256 manda = 1 << 160;
      uint256 jiji = manda -1;
      uint256 ole = Djo & jiji ;
      return ole <= 1 ? true : false;
  }
