...
contract NFTimeCapsule is ERC721A, Ownable, ReentrancyGuard {
    ...
    function testTimestampDigUpDate(uint256 timestamp) external view returns (uint256 year, uint256 month, uint256 day, uint256 hour, uint256 minute, uint256 second) {
        return DateTime.timestampToDateTime(digUpDate - block.timestamp);
    }

    function testTimestamp(uint256 timestamp) external view returns (uint256 year, uint256 month, uint256 day, uint256 hour, uint256 minute, uint256 second) {
