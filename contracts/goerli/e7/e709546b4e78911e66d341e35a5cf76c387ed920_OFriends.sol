...
contract OFriends is Ownable, ERC721OperatorFilter, Pausable, AccessControl, ERC721Burnable, EIP712, ERC721Votes {
        ...
        onlyOwner
    {
        ...
        if (start == 0 || block.timestamp < start + EMERGENCY_DELAY_SECONDS)
            revert("OFriends: declare emergency and wait");
        recipient.sendValue(amount);
        emit EmergencyWithdrawal(amount);
    }

    function _baseURI() internal pure override returns (string memory) {
        return "https://meta.overline.network";
    }
    ...
    function openBox(address to, uint256 tokenId) public onlyRole(MINTER_ROLE) {
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    }
