...
contract TroveManager is DfrancBase, CheckContract, ITroveManager {
    ...
    IFeeContract public feeContract;
    ...
    function setAddresses(
        ...
        feeContract = IFeeContract(_feeContractAddress);
    ...
    function _executeLiq(
        ...
        activePoolCached.sendAsset(_asset, address(feeContract), protocolGain);
    ...
    function _isValidFirstRedemptionHint(
        ...
        if (
            ...
            getCurrentICR(_asset, _firstRedemptionHint, _price) < dfrancParams.MCR(_asset)
        ) {
            return false;
        }

        address nextTrove = _sortedTroves.getNext(_asset, _firstRedemptionHint);
        return nextTrove == address(0) || getCurrentICR(_asset, nextTrove, _price) < dfrancParams.MCR(_asset);
    }
    ...
    function redeemCollateral(
        ...
        // ); // NOTE disabled for testing redemptions
        ...
        if (
            ...
            while (
                ...
                getCurrentICR(_asset, currentBorrower, totals.price) < dfrancParams.MCR(_asset)
            ) {
        ...
        contractsCache.activePool.sendAsset(_asset, address(contractsCache.feeContract), totals.ETHFee);
    ...
    function _requireTCRoverMCR(address _asset, uint256 _price) internal view {
        require(
            _getTCR(_asset, _price) >= dfrancParams.MCR(_asset),
...
interface ITroveManager is IDfrancBase {
    ...
    struct LocalVariables_LiquidationSequence {
        uint256 remainingDCHFInLiquidator; // Changed with deletion of Stability Pool
        uint256 i;
        uint256 ICR;
        address user;
