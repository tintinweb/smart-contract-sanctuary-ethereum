...
contract GrassHouse is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  modifier onlyXAlpaca() {
    require(msg.sender == xALPACA, "only xALPACA");
    _;
  }
  ...
  function _calculateReward(
    ...
    _maxUserEpoch = IxALPACA(xALPACA).userPointEpoch(_user);
  ...
  function claim(address _user) external nonReentrant onlyLive onlyXAlpaca returns (uint256) {
    if (block.timestamp >= weekCursor) _checkpointTotalSupply();
    uint256 _lastTokenTimestamp = lastTokenTimestamp;
