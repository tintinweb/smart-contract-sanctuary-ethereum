...
contract GigzaV4 is Initializable, OwnableUpgradeable {
    ...
    //To be removed
    mapping(uint256 => uint256) public reviewTimestamp;

    function initialize(address _currency) public payable initializer {
    ...
    function createProfile (
        ...
        require(profile[msg.sender].userAddress == address(0), "Profile already exists");
    ...
    function editProfile (
        ...
        //Check required if profile doesn't exists
        require(profile[msg.sender].userAddress == msg.sender, "FORBIDDEN");
    ...
    function createReview(
        ...
        reviewTimestamp[jobId] = block.timestamp;
    ...
    function getReviewTimeStamp(uint256 jobId) external view returns (uint256){
        return reviewTimestamp[jobId];
    }

     function getReviewsTimeStamps(uint256[] memory jobIds) external view returns (uint256[] memory){
        uint256[] memory timestamps = new uint256[](jobIds.length);
        
        for (uint256 i = 0; i < jobIds.length; i++ ){
            uint256 timestamp = reviewTimestamp[i];
    ...
    **/
    ...
    event ProfileEdited(address indexed user);
