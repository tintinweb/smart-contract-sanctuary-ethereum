...
update tier two so that starts accumulating token from when it is first minted. x 

add withdraw method to the ticket contract for token. x 

add quantity into merkle tree for pcc-land == 

check logic for max mint vs current max mint on pcc-land

mint first 3 tier 2 ids on deployment.
