...
contract FantomFoxes is ERC721AQueryable, ReentrancyGuard, ERC2981, Ownable {

    uint public maxSupply       = 5555;
    uint public freePerWallet   = 1;
    uint public maxPerTx        = 9;
    uint public maxPerWallet    = 9;
    uint public costAfterFree = 0.003 ether;

    bool public mintAllowed = false;

    string private _baseTokenURI;

    constructor() ERC721A("Fantom Foxes", "FF") {}
    ...
    function mint(uint amount)
      ...
      callerIsUser
      nonReentrant {
        uint price = getPrice(amount);

        if (!mintAllowed) revert MintDisabled();
        if (amount > maxPerTx + 1) revert ExceedsMaxPerTransaction();
        if (_numberMinted(msg.sender) + amount > maxPerWallet + 1) revert ExceedsMaxPerWallet();
        if (_totalMinted() + amount > maxSupply + 1) revert ExceedsAllowedMints();
    ...
    function toggleMintAllowed() external onlyOwner {
