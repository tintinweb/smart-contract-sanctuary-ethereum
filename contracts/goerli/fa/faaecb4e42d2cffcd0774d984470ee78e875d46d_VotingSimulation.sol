...
contract VotingSimulation {
    ...
    function createVoters() public {
        ...
        uint256 prevNumber = uint(keccak256(abi.encodePacked(block.timestamp + block.difficulty))) % 10;
        ...
        for (uint256 i;i<5;i++){
         ...
         for (uint256 i; i< 100; i++){
            s_votesArray[i] = 0;
            randomNumber= uint(keccak256(abi.encodePacked(prevNumber + i)))%10;
    ...
    function startVoting() public{
        ...
        uint256 prevNumber = uint(keccak256(abi.encodePacked(block.timestamp + block.difficulty))) % 5;
        uint256 randomNumber;

       for (uint256 i; i<10; i++){
        for (uint256 j; j<10; j++){
        randomNumber= uint(keccak256(abi.encodePacked(prevNumber*(i+1) + j))) % 5;
