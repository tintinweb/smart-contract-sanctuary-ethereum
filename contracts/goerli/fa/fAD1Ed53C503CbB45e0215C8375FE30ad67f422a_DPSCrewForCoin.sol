...
contract DPSCrewForCoin is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function redeemDPS(uint256 _tokenId) public nonReentrant {
        ...
        // should never skip this in case the LFlagship it's in our contract or at the borrower or lender
        if (
            lDPS.ownerOf(_tokenId) == address(this) ||
            lDPS.ownerOf(_tokenId) == asset.borrower ||
            lDPS.ownerOf(_tokenId) == asset.lender
        ) {
            burned = true;
            lDPS.burn(_tokenId);
        }
    ...
    function redeemFlagship(uint256 _tokenId) public nonReentrant {
        ...
        // should never skip this in case the LFlagship it's in our contract or at the borrower or lender
        if (
            lFlagship.ownerOf(_tokenId) == address(this) ||
            lFlagship.ownerOf(_tokenId) == asset.borrower ||
            lFlagship.ownerOf(_tokenId) == asset.lender
        ) {
            burned = true;
