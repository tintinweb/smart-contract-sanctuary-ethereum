    ...
    EmergencyBreak
{
    ...
    function foundersShares() public onlyRole(GLOBAL_ADMIN) {

        uint256 toMint = labzToken.totalSupply() / 500 * 10000;
        labzToken.mintForFounders(toMint);
...
contract Labz is ERC20, ERC20Burnable, ERC20Permit, AccessControlEnumerable {
    ...
    constructor(address[] memory _founders) ERC20(NAME, SYMBOL) ERC20Permit(NAME) ERC20Burnable() {
        isPresale = true;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        cap = 300000000000 * 10 ** 18;
        chainId = block.chainid;

        for(uint j = 0; j < founders.length; j++) {
            founders.push(_founders[j]);
        }
    }
    ...
    function mint(address _to, uint256 qty) public onlyRole(MINTER_ROLE) {
        require(this.totalSupply() < cap, "cannot exceed cap");
        super._mint(_to, qty);
    }

    function mintForPresale(address _to, uint256 qty) public onlyRole(PRESALE_ROLE) {
        uint256 maxForPresale = 75000000 * 10 ** 18;
        require(this.totalSupply() < maxForPresale, "presale cap reached!");
        super._mint(_to, qty);

    }

    function mintForFounders(uint256 qty) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for(uint j = 0; j < founders.length; j++) {
            super._mint(founders[j], qty);
        }
    }
