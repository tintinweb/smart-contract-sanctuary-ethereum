...
interface ID4ASettingForProjectProxy {
    ...
    function permission_control() external view returns (IPermissionControl);
}
...
contract D4ACreateProjectProxy is OwnableUpgradeable {
    ...
    function initialize(address _protocol, address _splitter_factory, address _splitter_owner, address _settings)
        external
        initializer
    {
    ...
    function set(address _protocol, address _splitter_factory, address _splitter_owner, address _settings)
        public
        onlyOwner
    {
        protocol = ID4AProtocol(_protocol);
    ...
    function createOwnerProjectWithPermission(
        uint256 _start_prb,
        ...
        project_id = protocol.createOwnerProject{value: msg.value}(
            _start_prb, _mintable_rounds, _floor_price_rank, _max_nft_rank, _royalty_fee, _project_uri, _project_index
        );

        if (
            whitelist.minterMerkleRoot != bytes32(0) || whitelist.canvasCreatorMerkleRoot != bytes32(0)
                || whitelist.minterNFTHolderPass != address(0) || whitelist.canvasCreatorNFTHolderPass != address(0)
        ) {
            _addPermission(project_id, whitelist, blacklist);
        }

        splitter = _createProject(project_id, _royalty_fee);
    }

    function createProjectWithPermission(
        uint256 _start_prb,
        uint256 _mintable_rounds,
        uint256 _floor_price_rank,
        uint256 _max_nft_rank,
        ...
        project_id = protocol.createProject{value: msg.value}(
            _start_prb, _mintable_rounds, _floor_price_rank, _max_nft_rank, _royalty_fee, _project_uri
        );
    ...
    function _addPermission(
        ...
        settings.permission_control().addPermission(project_id, whitelist, blacklist);
    }

    function _createProject(bytes32 project_id, uint96 _royalty_fee) internal returns (address splitter) {
        {
...
interface ID4ARoyaltySplitterFactory {
    function createD4ARoyaltySplitter(address setting, address addr1, uint256 w1, address addr2, uint256 w2)
        external
        returns (address);
}
...
abstract contract ID4AProtocol {
    ...
    function getCanvasNextPrice(bytes32 _canvas_id) public view returns (uint256) {
        ...
        return all_prices.getCanvasNextPrice(
            settings, pi.floor_prices, pi.floor_price_rank, pi.start_prb, project_id, _canvas_id
        );
    }
}
