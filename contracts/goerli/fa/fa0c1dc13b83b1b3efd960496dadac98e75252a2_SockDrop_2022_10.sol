...
contract SockDrop_2022_10 is ERC721A, Ownable, ReentrancyGuard {

  mapping(address => bool) public whitelistClaimedList;
  ...
  */
    ...
    returns(uint256)
  {
    ...
    if(verified) {
      if(whitelistClaimedList[_msgSender()]) {
        return 0;
      } else {
        return 1;
      }
  ...
  function _assertCanClaim(
    ...
    returns(bool)
  {
    require(whitelistClaimedList[_msgSender()] == false, "You've already claimed");
  ...
  ******
  */
  function whitelistMint(bytes32[] calldata proof) public payable mintCompliance() nonReentrant {
    _assertCanClaim(proof);

    whitelistClaimedList[_msgSender()] = true;
    _safeMint(_msgSender(), 1);
  }

  function mint() public payable mintCompliance() nonReentrant {
    require(publicMintEnabled == true, "Public mint not available");
    require(claimedList[_msgSender()] == false, "You've already claimed");
  ...
  function mintForAddress(address receiver, uint256 mintAmount) public mintCompliance() onlyOwner {
