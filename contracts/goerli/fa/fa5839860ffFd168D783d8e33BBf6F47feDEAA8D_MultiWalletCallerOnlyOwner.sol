...
interface IChildStorage {
    ...
    function addChildAddress_VtF(address _user, address _newChild) external;
...
contract MultiWalletCallerOnlyOwner is Ownable, IERC721Receiver {
    ...
    function createWallets(uint256 _quantity) external onlyOwner {
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            IChildStorage(_childStorage).addChildAddress_VtF(msg.sender, newChild);
            IMinterChaild(payable(newChild)).initialize_puB(msg.sender);
        }
    }
    ...
    function withdrawETH(uint256 _startId, uint256 _endId) external checkId(_startId, _endId) {
        ...
        for (uint256 i = _startId; i <= _endId; i++) {
            IMinterChaild(payable(IChildStorage(_childStorage).child(msg.sender, i))).withdrawETH_RBf(msg.sender);
        }
    ...
    function withdrawERC20(uint256 _startId, uint256 _endId, address _contract) external checkId(_startId, _endId) {
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            IMinterChaild(payable(IChildStorage(_childStorage).child(msg.sender, i))).withdrawERC20_ATR(_contract, msg.sender);
            idx++;
        }
    }
    ...
    function withdrawERC721(uint256 _startId, uint256 _endId, address _contract, uint256[] calldata _tokenIds) external checkId(_startId, _endId) {
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            IMinterChaild(payable(IChildStorage(_childStorage).child(msg.sender, i))).withdrawERC721_VKo(_contract, _tokenIds[idx], msg.sender);
            idx++;
        }
    }
    ...
    function withdrawERC1155(uint256 _startId, uint256 _endId, address _contract, uint256 _tokenId) external checkId(_startId, _endId) {
        uint256 idx;
        for (uint256 i = _startId; i <= _endId; i++) {
            IMinterChaild(payable(IChildStorage(_childStorage).child(msg.sender, i))).withdrawERC1155_wcC(_contract, _tokenId, msg.sender);
            idx++;
        }
    }
