...
contract CruExchange is ReentrancyGuard, CollateralWrapper {
    using SafeERC20 for IERC20;
    uint256 private fxRate = 500000; //cru equivalent per USDT
    ...
    constructor(address _cruToken, address _collateral) CollateralWrapper(_collateral) {
    ...
    function swap(uint256 cruAmount) external nonReentrant {
        ...
        _collateralTransfer(msg.sender, collateralToSend); //send the USDT to the sender
        
        emit Exchange(msg.sender, cruAmount, collateralToSend, fxRate);
    }
    ...
    function changeFxRate(uint256 amount) external onlyOwner {
