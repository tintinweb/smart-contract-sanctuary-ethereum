...
contract LandNFT_V1 is Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    event TransferTokensEvent(string tokenURI);
    event PurchasedNftPriceEvent(uint256 price);
     ...
     */
    function isAddressDiscountable(
        address _ethereumAddress,
        ...
        returns (bool)
    {
        ...
        bytes32 leaf = keccak256(abi.encodePacked(_ethereumAddress));
        if (MerkleProofUpgradeable.verify(_merkleProof, discount2PerMerkleRoot, leaf)) {
            isDiscountable = true;
        } else if (MerkleProofUpgradeable.verify(_merkleProof, discount3PerMerkleRoot, leaf)) {
            isDiscountable = true;
        } else if (MerkleProofUpgradeable.verify(_merkleProof, discount5PerMerkleRoot, leaf)) {
            isDiscountable = true;
        } else {
        ...
        returns(uint256)
    {
        ...
        } else if (MerkleProofUpgradeable.verify(_discount3PerMerkleProof, discount3PerMerkleRoot, leaf)) {
            tokenPrice = MINT_PRICE_USDC.sub(MINT_PRICE_USDC.mul(3).div(100));
        } else if (MerkleProofUpgradeable.verify(_discount5PerMerkleProof, discount5PerMerkleRoot, leaf)) {
    ...
    function transferTokens(uint256 _mintNum, uint256 _tokenPrice) internal {
        for (uint256 i; i < _mintNum; i++) {
            for (uint256 index; index < MAX_SUPPLY_MINT; index++) {
                ...
                if (!isNonMintTokenIds[tokenId]) {
                    ...
                    emit TransferTokensEvent(tokenURI(tokenId));
                    break;
                } else {
                    tokenIdListIndex++;
                }
        ...
        emit PurchasedNftPriceEvent(_tokenPrice.mul(_mintNum));
    }
}