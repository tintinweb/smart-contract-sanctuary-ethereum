...
contract BlackErc20 is ERC20, Ownable {
    ...
    uint256 private _devPro = 0;
    uint256 private _deplyPro = 0;
    uint256 private _vitalikPro = 0;
    uint256 private _maxPro = 0;
    uint256 private _berc20EthPro = 0;
    uint256 private _burnPer = 0;
    ...
    function setContractAuth(uint256[] memory params, address[] memory authContracts) external onlyOwner {
        require(params.length >= 5, "Invalid params length");
...
contract BlackErcFactory is Ownable {
    ...
    function createToken(
        ...
        require(params[3] <= 3, "Invalid Per");
        require(params[4] <= 3, "Invalid Per");

        // Create an ERC20 token contract
        BlackErc20 token = new BlackErc20(name, symbol, totalSupply,
            maxMintCount, maxMintPerAddress,mintPrice,address(this),devAddress,msg.sender,params);
