...
contract RwaLiquidationOracle {
    ...
    address     public vow;
    uint256     public mat; //collateral ratio limit
    uint256     public currRatio; // current collat ratio
    uint256     public adjustedValue;
    ...
    struct Ilk {
        ...
        address urn; // urn of ilk
        uint48 ratio; // collateral ratio
    }
    ...
    function init(bytes32 ilk, uint256 val, string calldata doc, uint48 tau, address urn, uint256 mat_) external auth {
        ...
        // ilks[ilk].ratio = div(val,20000000000000000000000000);
        ...
        currRatio = div(val,art);

        adjustedValue = div(val,mat);
    ...
    function dip(bytes32 ilk, uint256 val) external auth {
        ...
        //val cannot be lower than line (DC)
        (,,,uint256 line,) = vat.ilks(ilk);
        require(val>=line, "RwaOracle/line-breached");
        pip.poke(bytes32(val));
    ...
    function good(bytes32 ilk) external view returns (bool) {
        require(ilks[ilk].pip != address(0), "RwaOracle/unknown-ilk");
        // address urn = ilks[ilk].urn;
        // (uint256 art,,,,) = vat.ilks(ilk);
        // uint val = uint256(DSValue(ilks[ilk].pip).read());
        //if debt is 0 then dont check ratio
        // if(art==0){
        //     return true;
        // }
        //else get the current collat ratio

        (,uint256 rate,,,) = vat.ilks(ilk);
        (, uint256 art) = vat.urns(ilk, ilks[ilk].urn);

        
        
        if(adjustedValue > mul(art, rate)){
            return true;
        }
        else{
            return false;
