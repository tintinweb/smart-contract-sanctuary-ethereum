...
contract ArtTest is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer {
  using Strings for uint256;
  ...
  uint256 private constant TotalCollectionSize_ = 1500; // total number of nfts
  uint256 private constant MaxMintPerBatch_ = 20; //max mint per transaction
  uint256 public PhaseCollectionSize_ = 1500; // total number of nfts in Phase 1
  ...
  constructor() ERC721A("ArtTest","ATS") {
  ...
  function airdrop(address sendTo, uint quantity)public onlyOwner{
    ...
    uint256 private TotalWLavailable = 1500; // total number of nfts
    ...
    function purchaseWhiteListTokens(uint256 _howMany) external payable {
