...
contract BatchTokenAssignment is AccessControl, ReentrancyGuard {
  ...
  constructor() {
    ...
    assignment.setProxyState(address(this), true);
  ...
  function _depositAndVestingPlanMB1(uint256 nftID, uint256 amount, address token, uint256 _cliff, uint16[] calldata _percentages, uint256[] calldata _unlockDuration, uint256 _startTime) internal {
    depositTokenForMB1(nftID, token, amount);
  ...
  function _depositAndVestingPlanGG(uint256 nftID, uint256 amount, address token, uint256 _cliff, uint16[] calldata _percentages, uint256[] calldata _unlockDuration, uint256 _startTime) internal {
    depositTokenForGG(nftID, token, amount);
  ...
  function depositTokenForMB1(uint256 nftID, address token, uint256 amount) public onlyRole(DEPOSIT_ROLE) nonReentrant {
    ...
    if(assignment.MB1isVested(nftID, token)){ 
      ...
      emit MB1TokenBalanceChange(nftID, token, balance, claimable, balance - claimable);
    } else {
      emit MB1TokenBalanceChange(nftID, token, balance, balance, 0);
    }
  }

  function withdrawForMB1(uint256 withdrawRequestID, uint256 nftID, address token, uint256 amount, address user, address recipient) public onlyRole(WITHDRAW_ROLE) nonReentrant {
  ...
  function depositTokenForGG(uint256 nftID, address token, uint256 amount) public onlyRole(DEPOSIT_ROLE) nonReentrant {
    ...
    if(assignment.GGisVested(nftID, token)){ 
      ...
      emit GGTokenBalanceChange(nftID, token, balance, claimable, balance - claimable);
    } else {
      emit GGTokenBalanceChange(nftID, token, balance, balance, 0);
    }
  }

  function withdrawForGG(uint256 withdrawRequestID, uint256 nftID, address token, uint256 amount, address user, address recipient) public onlyRole(WITHDRAW_ROLE) nonReentrant {
  ...
  event GGTokenBalanceChange(uint256 indexed nftID, address indexed token, uint256 balance, uint256 claimable, uint256 locked);
  event MB1TokenBalanceChange(uint256 indexed nftID, address indexed token, uint256 balance, uint256 claimable, uint256 locked);
}
