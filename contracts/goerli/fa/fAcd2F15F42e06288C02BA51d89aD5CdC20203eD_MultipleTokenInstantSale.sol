...
contract MultipleTokenInstantSale is Pausable, ReentrancyGuard {
    ...
    function buy(uint256 uid, uint256 amount) public payable notPaused nonReentrant {
        ...
        if (_royalty_receivers[item.token_address]) {
            ...
            payable(creator).send((item.price * amount * royalty) / 1000);
            summ += royalty;
        }
        payable(_owner_of).send((item.price * amount * 25) / 1000);
        summ += 25;
        payable(item.owner_of).send(msg.value - ((item.price * amount * summ) / 1000));
