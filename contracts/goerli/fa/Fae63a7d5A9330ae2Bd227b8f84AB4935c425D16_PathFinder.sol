...
contract PathFinder {
    ...
    function bestExactInputPath(
        ...
        path = _bestV3Path(1, tokenIn, tokenOut, amount, tokens);
    }
    ...
    function bestExactOutputPath(
        ...
        path = _bestV3Path(2, tokenIn, tokenOut, amount, tokens);
    }
    ...
    function _bestV3Path(
        ...
        for (uint256 i = 0; i < fees.length; i++) {
            ...
            uint256 amountOut = _quote(tradeType, path, amount);
    ...
    function _quote(
        ...
        if (tradeType == 1) {
            try quoter.quoteExactInput(path, amount) {} catch {
                return 0;
            }
        } else {
            try quoter.quoteExactOutput(path, amount) {} catch {
                return 0;
            }
        }
    }
