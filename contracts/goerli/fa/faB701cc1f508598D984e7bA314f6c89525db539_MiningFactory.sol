...
contract AtlendisMining is Ownable {
    ////////////
    // EVENTS //
    ////////////

    event Stake(uint256 tokenId, address from);
    event Withdraw(uint256 tokenId, address from);
    event Claim(address from, uint256[] rewards);
    event TokenAdded(address token, uint256 rate);
    ...
    constructor(address _nft, bytes32 _poolHash) {
    ...
    function getUserForToken(address token, address user) external view returns(User memory) {
        return rewardTokens[token].users[user];
    }

    function getRewardForUser(address _token, address _user) external view returns(uint256) {
        RewardToken storage token = rewardTokens[_token];
        User storage user = token.users[_user];
        uint256 userDeposits = deposits[_user];
        uint256 lastLiquidityRatio = user.lastLiquidityRatio == 0 ? 1e18 : user.lastLiquidityRatio;

        uint256 newRewards = (block.timestamp - token.lastUpdate) * token.rate;
        uint256 contractTokenBalance = ERC20(_token).balanceOf(address(this));
        if(contractTokenBalance < newRewards) {
            newRewards = contractTokenBalance;
        }
        uint256 newLiquidityRatio = token.liquidityRatio + newRewards * 1e18 / token.totalAdjustedDeposits;

        return user.earned + userDeposits * newLiquidityRatio / lastLiquidityRatio - userDeposits;
    }
    ...
    modifier updateRewards(address from) {
        require(tokens.length > 0, "No token is registered");

        for (uint256 i = 0; i < tokens.length; i++) {
            // update global data
            RewardToken storage token = rewardTokens[tokens[i]];
            if (totalDeposits > 0) {
                uint256 newRewards = (block.timestamp - token.lastUpdate) * token.rate;
                uint256 contractTokenBalance = ERC20(tokens[i]).balanceOf(address(this));
                if(contractTokenBalance < newRewards) {
                    newRewards = contractTokenBalance;
                }
                token.pendingRewards += newRewards;
                token.liquidityRatio += newRewards * 1e18 / token.totalAdjustedDeposits;
            }
            token.lastUpdate = block.timestamp;

            // update user individual earnings
            User storage user = token.users[from];
            uint256 userDeposits = deposits[from];
            uint256 lastLiquidityRatio = user.lastLiquidityRatio == 0 ? 1e18 : user.lastLiquidityRatio;
            user.earned += userDeposits * token.liquidityRatio / lastLiquidityRatio - userDeposits;

            // register user earnings impact over whole distribution
            uint256 oldAdjustedBalance = userDeposits * 1e18 / lastLiquidityRatio;
            uint256 newAdjustedBalance = userDeposits * 1e18 / token.liquidityRatio;
            token.totalAdjustedDeposits -=  oldAdjustedBalance - newAdjustedBalance;
            user.lastLiquidityRatio = token.liquidityRatio;            
        }
        _;
    }
    ...
    function addToken(address token, uint256 rate) external onlyOwner {
        tokens.push(token);
        rewardTokens[token].rate = rate;
        rewardTokens[token].liquidityRatio = 1e18;
        rewardTokens[token].lastUpdate = block.timestamp;
        rewardTokens[token].totalAdjustedDeposits = totalDeposits;
    ...
    function stake(uint256 tokenId) external updateRewards(msg.sender) {
        ...
        uint128 amount = bondsQuantity + normalizedDepositedAmount;

        totalDeposits += amount;
        deposits[msg.sender] += amount;
        stakedNFTs[tokenId] = StakedNFT({deposit: amount, owner: msg.sender});

        for (uint256 i = 0; i < tokens.length; i++) {
            RewardToken storage token = rewardTokens[tokens[i]];
            token.totalAdjustedDeposits += amount * 1e18 / token.liquidityRatio;
        }
    ...
    function withdraw(uint256 tokenId) external updateRewards(msg.sender) {
        require(msg.sender == stakedNFTs[tokenId].owner, "Caller is not the owner of the target token");

        totalDeposits -= stakedNFTs[tokenId].deposit;
        deposits[msg.sender] -= stakedNFTs[tokenId].deposit;

        for (uint256 i = 0; i < tokens.length; i++) {
            RewardToken storage token = rewardTokens[tokens[i]];
            if(totalDeposits == 0) {
                token.totalAdjustedDeposits = 0;
            } else {
                token.totalAdjustedDeposits -= stakedNFTs[tokenId].deposit * 1e18 / token.liquidityRatio;
            }
        }
    ...
    function claim() external updateRewards(msg.sender) {
        uint256[] memory toClaim;
        for (uint256 i = 0; i < tokens.length; i++) {
            RewardToken storage token = rewardTokens[tokens[i]];
            User storage user = token.users[msg.sender];

            toClaim[i] = user.earned;
            token.pendingRewards -= user.earned;
            user.earned = 0;
        }

        for (uint256 i = 0; i < tokens.length; i++) {
            ERC20(tokens[i]).transfer(msg.sender, toClaim[i]);
        }
...
import "./AtlendisMining.sol";
import "../token/AtlendisToken.sol";

/**
 * @title MiningFactory
 * @notice Deploys and configure new minings
 **/
contract MiningFactory {
    ////////////
    // EVENTS //
    ////////////
    ...
    mapping(bytes32 => address) private minings;
    ...
    function getMining(bytes32 id) public view returns (address) {
        return minings[id];
    }
    ...
    /**
     * @notice Deploys a new mining contract
     * Grants the priviledged transfer role to allow for lock claims to work even when transfers are paused
     * Mints new tokens to be used by the mining contract
     **/
    function deployNewMining(
        bytes32 id,
        uint256 amount,
        bytes32 poolHash,
        uint256 rate
        ...
        require(
            minings[id] == address(0),
            "Mining with target id already exists"
        );

        // deploy new vesting wallet with vesting starting now
        AtlendisMining deployedMining = new AtlendisMining(
            positionManager,
            poolHash
        );

        // register contract
        minings[id] = address(deployedMining);

        // add atlendis token
        deployedMining.addToken(address(token), rate);

        // give ownership back to creator
        deployedMining.transferOwnership(msg.sender);

        // mint new tokens to lockdrop contract
        token.mint(address(deployedMining), amount);

        // allow vesting wallet to transfer tokens otherwise claim would fail
        token.grantRole(
            token.PRIVILEDGED_TRANSFER_ROLE(),
            address(deployedMining)
        );
 ...
 * Token supply is capped, cap defined at deployment
 * Transfers can be paused, but addresses with priviledged role can still transfer
 * THe priviledged transfer role is meant to allow utility for the token before it has liquidity
 **/
    ...
    ERC20PausableUpgradeable
{
    ...
    function initialize(
        ...
        // the association is an accounting management unit only allowed to mint tokens
        _setupRole(MINTER_ROLE, association);
    }
    ...
    /**
     * @notice Override mint function
     * Tokens can only be minted up to the maximum supply set at deployment
     **/
    ...
    /**
     * @notice Override pause function
     * Tokens can still be transfered by addresses with the priviledged transfer role
     **/
    function paused() public view override returns (bool) {
        if (hasRole(PRIVILEDGED_TRANSFER_ROLE, msg.sender)) return false;
