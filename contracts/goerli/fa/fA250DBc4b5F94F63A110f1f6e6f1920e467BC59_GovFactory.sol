...
contract GovFactory{
    ...
    function createSpace(address _tokenaddr ,bool _snap, uint256 min) public returns(address space){

        require(spaces[_tokenaddr]==address(0x0),"Space already exists for token");
        
        PYEGov newGov = new PYEGov(_tokenaddr, _snap, min);
        
        spaces[_tokenaddr] = address(newGov);
        
        emit SpaceCreated(_tokenaddr, space);

    }

    function getSpace(address _tokenaddr)external view returns(address){
        
        return spaces[_tokenaddr];
...
interface IGovernor {
    ...
    function initprop(string memory , uint256, uint256 ) external;
