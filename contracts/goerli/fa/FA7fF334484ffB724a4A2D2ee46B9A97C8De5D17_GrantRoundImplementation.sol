...
contract GrantRoundImplementation is AccessControl, Initializable {
  ...
  function initialize(
    ...
    for (uint256 i = 0; i < _roundOperators.length; ++i) {
      _grantRole(DEFAULT_ADMIN_ROLE,  _roundOperators[i]);
      // _grantRole(ROUND_OPERATOR_ROLE, _roundOperators[i]);
    }
  }

  // @notice Update metaPtr (only by DEFAULT_ADMIN_ROLE)
  /// @param _newMetaPtr new metaPtr
  function updateMetaPtr(MetaPtr memory _newMetaPtr) public onlyRole(DEFAULT_ADMIN_ROLE) {
    emit MetadataUpdated(metaPtr, _newMetaPtr);
    metaPtr = _newMetaPtr;
  }

  /// @notice Update roundStartTime (only by DEFAULT_ADMIN_ROLE)
  /// @param _newRoundStartTime new roundStartTime
  function updateRoundStartTime(uint256 _newRoundStartTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  /// @notice Update roundEndTime (only by DEFAULT_ADMIN_ROLE)
  /// @param _newRoundEndTime new roundEndTime
  function updateRoundEndTime(uint256 _newRoundEndTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  /// @notice Update grantApplicationsStartTime (only by DEFAULT_ADMIN_ROLE)
  /// @param _newGrantApplicationsStartTime new grantApplicationsStartTime
  function updateGrantApplicationsStartTime(uint256 _newGrantApplicationsStartTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
