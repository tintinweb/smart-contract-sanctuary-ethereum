...
contract Huralya is ERC721, ERC721URIStorage, ReentrancyGuard {
  using SafeMath for uint256;
  using Strings for uint256;
  using Counters for Counters.Counter;
  Counters.Counter private _idCounter;
  string public baseURI = 'https://nft.huralya.com/sdude/';
  address public lyaToken = 0x654affdFBC0C032810A58f258a8b308190D1a460;
  address public owner;
  uint256 public mintPrice = 50000000000000000;
  uint256 public totalSupply = 50000;
  uint256 public mintPriceIncrement = 50000000000000000;
  uint256 public currentSupply = 0;
  uint256 private tokensForEachMintPeriod = 3;
  ...
  mapping(uint256 => bool) private lockedStakedTokens;
  mapping(address => uint256[]) private stakedTokensByAddress;
  mapping(address => uint256[]) private lockedTokensByAddress;
  mapping(address => mapping(uint256 => uint256)) private stakedTokens;
  mapping(address => mapping(uint256 => uint256)) private unstakedTokens;
  mapping(uint256 => bool) private tokenIds;
  mapping(uint256 => bool) staked;
  uint256 private stakeRewardPeriod = 2 minutes;
  uint256 public totalStakePeriod = 1 hours;
  uint256 private rewardPercent = 1000000000000000; // 0.001
  ...
  mapping(uint => StakingHistory) public stakingHistory;
  ...
  function mint(uint256 _quantity) external nonReentrant {
    ...
    require(
      ...
      "Recipient's account is locked and cannot receive tokens"
    );
  ...
  function lockAccount(address _account) public onlyOwner {
  ...
  function lockToken(uint256 _tokenId) public onlyOwner {
    lockedTokensByAddress[msg.sender].push(_tokenId);
    lockedTokens[_tokenId] = true;
  }

  function unlockToken(uint256 _tokenId) public onlyOwner {
    uint256 index;
    for (uint256 i = 0; i < lockedTokensByAddress[msg.sender].length; i++) {
      if (lockedTokensByAddress[msg.sender][i] == _tokenId) {
        index = i;
        break;
      }
    }
    delete lockedTokensByAddress[msg.sender][index];
  ...
  function transferFrom(
    ...
    require(
      msg.sender == owner || !lockedAccounts[_from],
      "Sender's account is locked and cannot transfer tokens"
    );
    require(
      msg.sender == owner || !lockedAccounts[_to],
      "Recipient's account is locked and cannot receive tokens"
    );
    require(
      msg.sender == owner || !lockedTokens[_tokenId],
      'Token is locked and cannot be transferred'
    );

    require(!isStaked(_tokenId), 'Token already staked');
  ...
  function stake2(uint256 _tokenId) public {
    ...
    require(!lockedAccounts[msg.sender], 'Your account is locked');
    require(!lockedTokens[_tokenId], 'This token is locked');
    ...
    stakingHistory[_tokenId].allowedStakingDays = remainingStakingDays(
      _tokenId
    );
    ...
    if (history.totalStakedDays >= 0) {} else {
      stakingHistory[_tokenId].totalStakedDays = 0;
    }
    */
  }
  ...
  function unstake(uint256 _tokenId) public {
    ...
    require(!lockedAccounts[msg.sender], 'Your account is locked');
    require(!lockedTokens[_tokenId], 'This token is locked');
    require(isStaked(_tokenId), 'Token not staked');
    /*
    uint256 profit = calculateReward(_tokenId);
    if (profit > 0) {
      require(
        ERC20(lyaToken).transferFrom(msg.sender, ownerOf(_tokenId), profit),
        'LYA Transfer failed'
      );
    }
    */
  ...
  function historyTotalStake(uint256 _tokenId) public view returns (uint256) {
    require(exists(_tokenId), 'Token ID does not exist');
  ...
  function passedStakingDays(uint256 _tokenId) public view returns (uint256) {
    require(exists(_tokenId), 'Token ID does not exist');
    ...
    return totalDaysStaked + history.totalStakedDays;
  }
  ...
  function remainingStakingDays(
    ...
    totalDaysStaked = totalDaysStaked + history.totalStakedDays;

    return totalStakePeriod - totalDaysStaked;
  }
  ...
  function calculateReward(uint256 _tokenId) public view returns (uint256) {
    ...
    if (remainingTime <= 0) {
      reward = tokenMintPrice[_tokenId].mul(100000).div(1000000000000000000);
      return reward.mul(allowedStakingDays(_tokenId));
    }

    reward = tokenMintPrice[_tokenId].mul(100000).div(1000000000000000000);
    return reward.mul(passedStakingDays(_tokenId));
    /*
    require(exists(_tokenId), 'Token ID does not exist');
    require(isStaked(_tokenId), 'Token not staked');
    ...
    if (remainingTime <= 0) {
      return (tokenMintPrice[_tokenId].mul(rewardPercent)) * allowedStakingDays(_tokenId);
    }

    return (tokenMintPrice[_tokenId] * 1) * passedStakingDays(_tokenId); // TODO ZARB
    */
  }

  function allowedStakingDays(uint256 _tokenId) private view returns (uint256) {
    require(exists(_tokenId), 'Token ID does not exist');

    StakingHistory storage history = stakingHistory[_tokenId];

    return history.allowedStakingDays;
  }
  ...
  function getStakedTokensByAddress(
    ...
    return stakedTokensByAddress[_user];
  }

  function getLockedTokensByAddress(
    address _user
  ) public view returns (uint256[] memory) {
    return stakedTokensByAddress[_user];
  }
