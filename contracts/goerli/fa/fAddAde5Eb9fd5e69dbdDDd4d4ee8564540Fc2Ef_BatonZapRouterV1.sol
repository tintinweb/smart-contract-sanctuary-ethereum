...
contract BatonZapRouterV1 {
    ...
    function zapInETH(address payable farm, CaviarZapRouter.BuyParams calldata buyParams, CaviarZapRouter.AddParams calldata addParams) public payable {
        ...
        if (pair.lpToken().allowance(address(this), address(farm)) < lpTokenAmount) {
            pair.lpToken().approve(address(farm), type(uint256).max);
        }

        pair.lpToken().transferFrom(address(this), msg.sender, lpTokenAmount);
    }
}