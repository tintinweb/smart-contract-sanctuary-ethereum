...
import "./IBadToken.sol";
import "./IGoodToken.sol";

contract InvoiceFactory is Invoice, VerifySignature, AdminOnly {
    ...
    IBadToken public badTokenContract;
    IGoodToken public goodTokenContract;
    mapping(uint256 => InvoiceStruct) public invoices;
    mapping(uint256 => Request) public requests;
    address derogatoryAddress;
    ...
    event InvoicerSigned(uint256 idInvoice, bytes signature);
    event ValidatorSigned(uint256 idInvoice, bytes signature);
    event PayerSigned(uint256 idInvoice, bytes signature);
    event TokenIssued(uint256 idInvoice, uint256 date);
    event TokenRevoke(uint256 idInvoice, uint256 date);
    event CertificateIssued(uint256 idInvoice, uint256 date);

    constructor() {}

    function derogatoryContractUpdate(address newAddress) public onlyAdmin {
        badTokenContract = IBadToken(newAddress);
        derogatoryAddress = newAddress;
    }

    function certifateContractUpdate(address newAddress) public onlyAdmin {
        goodTokenContract = IGoodToken(newAddress);
        certificateAddress = newAddress;
    }
    ...
    modifier OnlyValidator(uint256 _id) {
        require(
            ...
            "Only validator can sign the invoice"
        );
        _;
    }
    ...
    function issueDerogatoryMark(uint _id) public onlyInvoicer(_id) {
        require(
            block.timestamp >= invoices[_id].dueDate,
            "Cannot issue now, whait for due date"
        );
        badTokenContract.safeMint(
            invoices[_id].payer,
            _id,
            "This is a Derogatory Mark"
        );
        badTokenContract.freezeToken(_id);
        emit TokenIssued(_id, block.timestamp);
    }

    function revokeDerogatoryMark(uint _id) public onlyInvoicer(_id) {
        badTokenContract.burn(_id);
        emit TokenRevoke(_id, block.timestamp);
    }

    function updateValidatorContract(
        IValidatorNFT _validatorContract
    ) public onlyAdmin {
    ...
    function createInvoice(
        ...
        isTheFuture(_dueDate)
        onlyWhitelisted(_validator)
        returns (uint256)
    {
        require(msg.sender != _payer, "Payer can't be the same as invoicer");
        require(msg.sender != _validator, "Creator can't be Validator");
        require(_payer != _validator, "Payer can't be Validator");
        invoiceCount++;
        invoices[invoiceCount] = InvoiceStruct(
            invoiceCount,
            _fee,
            _amount,
            block.timestamp,
    ...
    function signInvoiceInvoicer(
        uint256 _id,
        bytes memory _invoicerSignature
    ) public validID(_id) onlyInvoicer(_id) {
        InvoiceStruct storage invoice = invoices[_id];
        require(
    ...
    function signInvoicePayer(
        uint256 _id,
        bytes memory _payerSignature
    ) public validID(_id) onlyPayer(_id) {
        InvoiceStruct storage invoice = invoices[_id];
        require(
            block.timestamp <= invoice.creationDate + 86400,
            "Time to sign has passed"
    ...
    function signInvoiceValidator(
        uint256 _id,
        bytes memory _validatorSignature
    ) public validID(_id) OnlyValidator(_id) onlyWhitelisted(msg.sender) {
        InvoiceStruct storage invoice = invoices[_id];
        require(
            block.timestamp <= invoice.creationDate + 86400,
            "Time to sign has passed"
    ...
    function verifySignatures(
        uint256 _id
    ) public view validID(_id) returns (bool) {
    ...
    function payInvoice(
        uint256 _id
    ) public payable validID(_id) paid(_id) onlyPayer(_id) {
        ...
        revokeDerogatoryMark(_id);
        goodTokenContract.mintSoulboundNFT(
            _id,
            invoices[_id].payer,
    ...
    function acceptRenegotiation(
        uint256 _id,
        bytes memory _invoicerSignature
    ) public validID(_id) onlyInvoicer(_id) payerSigned(_id) {
...
interface IGoodToken {
    ...
    function getNFTInfo(uint256 tokenId) external view returns (address owner, string memory tokenURI, address approvedAddress, bool isApprovedForAll, uint256 balance, bool soulbound, address payer);
...
contract Invoice is ERC721, ERC721Enumerable, ERC721URIStorage {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("KryptoKreditInvoice", "KKI") {}
