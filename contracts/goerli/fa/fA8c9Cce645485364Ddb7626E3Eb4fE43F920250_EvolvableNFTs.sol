...
contract EvolvableNFTs is ERC1155LazyMint {
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
    ...
    function verifyClaim(
        ...
        require(_tokenId == 0, "EvolvableNFT: tokenId must be 0");
    }

    function evolve(uint256 _tokenId) external {
        require(_tokenId == 0 || _tokenId == 1, "Invalid tokenId");

        uint256 newTokenId = _tokenId + 1;

        _burn(msg.sender, _tokenId, 1);

        transferTokensOnClaim(msg.sender, newTokenId, 1);

        emit Evolved(_tokenId, newTokenId);
    }

    event Evolved(uint256 indexed _tokenId, uint256 indexed _newTokenId);
}