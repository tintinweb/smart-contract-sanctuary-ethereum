...
contract NFT1 is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function teamClaim(uint256 _num) external onlyOwner {
        // claim
        _safeMint(msg.sender, _num);
    }

    function boardClaim(address[] calldata boardAddresses, uint256 _num) external onlyOwner {
        // claim
        for (uint i = 0; i < boardAddresses.length; i++) {
            _safeMint(boardAddresses[i], _num);
