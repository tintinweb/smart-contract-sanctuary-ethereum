...
contract ZeroEggs is EggsERC721, LogisticVRGDA, Owned, ERC1155TokenReceiver {
    ...
    event teamAndCommunityMinted(address indexed user, uint256 lastMintedEggId, uint256 numEggsEach);
   ...
   function eomBalance(address user) public view returns (uint256) {
    ...
    function privateMint(
        ...
        uint256 amount, 
        uint256 maxAmount
    ) internal onlyOwner returns (uint256 lastMintedEggId) {
        if (alreadyAmount + amount <= maxAmount) {
            teamAndCommunityMintedAmount[sender] += amount;
            lastMintedEggId = _batchMint(sender, amount, currentNonLegendaryId);
            emit teamAndCommunityMinted(msg.sender, lastMintedEggId, amount);
        } else {
            teamAndCommunityMintedAmount[sender] = maxAmount;
            lastMintedEggId = _batchMint(sender, maxAmount - alreadyAmount, currentNonLegendaryId);
            emit teamAndCommunityMinted(msg.sender, lastMintedEggId, maxAmount - alreadyAmount);
        }
    }
...
contract EggReserve is Owned {
    ZeroEggs public immutable zeroEggs;

    constructor(ZeroEggs _zeroEggs, address _owner) Owned(_owner) {
        zeroEggs = _zeroEggs;
    }
    ...
    function withdraw(address to, uint256[] calldata ids) external onlyOwner {
        unchecked {
            for (uint256 i = 0; i < ids.length; ++i) {
                zeroEggs.transferFrom(address(this), to, ids[i]);
            }
        }
    }
}
