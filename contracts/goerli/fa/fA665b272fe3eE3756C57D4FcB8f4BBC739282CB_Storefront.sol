...
contract Storefront is Ownable2Step {
    using SafeERC20 for IERC20;

    error ParamLengthMissMatch();
    error InvalidAssetsForPhysicalProduct();
    ...
    event PhysicalProductPurchased(
    ...
    function sweepERC20(IERC20 _token) external onlyOwner {
    ...
    /// Called by prime token contract to handle NFT transfer after payment
    function handleInvokeEchelon(
        ...
        ) = abi.decode(_data, (uint256, uint256, uint256, uint256, bytes));

        if (
            productInfo[productId].priceEth * amount != _ethValue &&
            productInfo[productId].pricePrime * amount != _primeValue
        ) {
            revert InvalidAmountPaid();
        }

        if (nonceId - nonces[msg.sender] != 1) {
            revert InvalidNonce();
        }
        if (block.timestamp > timestamp + signatureWindow) {
        ...
        nonces[msg.sender] = nonceId;

        if (productInfo[productId].productType == ProductType.Digital) {
            for (
                uint256 i = 0;
                i < productInfo[productId].tokenAddresses.length;
                i++
            ) {
                IERC1155(productInfo[productId].tokenAddresses[i])
                    .safeBatchTransferFrom(
                        pullFromAddress,
                        _from,
                        productInfo[productId].tokenIds[i],
                        productInfo[productId].amounts[i],
                        bytes("")
                    );
            }
            ...
            emit DigitalProductPurchased(
                ...
                nonceId,
                timestamp,
                productInfo[productId].tokenAddresses,
                productInfo[productId].tokenIds,
                productInfo[productId].amounts,
                signature
            );
        } else {
            emit PhysicalProductPurchased(
                _from,
                productId,
                amount,
                nonceId,
    ...
    function _validateProductInfo(
        ...
        if (productType == ProductType.Physical && tokenAddresses.length > 0) {
            revert InvalidAssetsForPhysicalProduct();
        }

        if (priceEth == 0 || pricePrime == 0) {
            revert InvalidProductPrice();
        }
    }
}