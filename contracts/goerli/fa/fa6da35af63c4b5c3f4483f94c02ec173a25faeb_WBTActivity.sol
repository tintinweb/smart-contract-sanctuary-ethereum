...
contract WBTActivity is Ownable {
    ...
    mapping(uint => Activity) private wbtActivities;
    ...
    function activityById(uint activityId) public view returns(Activity memory) {
        return wbtActivities[activityId];
    }
    ...
    function AddActivityMember(address member, uint amount) external {
        ...
        require(wbtActivities[currentWBTActivityId].finishActivity > block.timestamp, 'Can not add while end activity not reach');
    ...
    function createActivity(
        ...
        if (wbtActivities[currentWBTActivityId].isRewardDistributed == false && currentWBTActivityId !=0) {
    ...
    function pickRandomWinners() internal view returns(uint[] memory) {
