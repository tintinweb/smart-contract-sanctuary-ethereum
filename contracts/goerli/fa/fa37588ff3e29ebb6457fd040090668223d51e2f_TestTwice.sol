...
contract TestTwice is ERC20, Ownable {
    ...
    address public marketingWallet = address(0xE06dAa176a624be741340e2d02F242eb1f950596);
    ...
    constructor(uint256[][] memory _taxes) ERC20("WOOF", unicode"0.F²", 18) {
        _createPair();
        uint256 totalSupply = 250000000 * (10**decimals());

        maxTransactionAmount = totalSupply.mul(15).div(1000);
        maxWallet = totalSupply.mul(25).div(1000);
        swapTokensAtAmount = totalSupply.mul(5).div(10000);

        buyTax = Taxes(_taxes[0][0], _taxes[0][1], _taxes[0][2], _taxes[0][0] + _taxes[0][1] + _taxes[0][2]);
        sellTax = Taxes(_taxes[1][0], _taxes[1][1], _taxes[1][2], _taxes[1][0] + _taxes[1][1] + _taxes[1][2]);
    ...
    function startTrading() external onlyOwner {
        ...
        swapEnabled = tradingActive = true;
    }
    ...
    function updateSwapTokensAtAmount(uint256 _swapTokensAtAmount) external onlyOwner {
        _swapTokensAtAmount = totalSupply().mul(_swapTokensAtAmount).div(100000);
    ...
    function updateMaxWalletAndTxnAmount(uint256 _maxTransactionAmount, uint256 _maxWallet) external onlyOwner {
        _maxTransactionAmount = totalSupply().mul(_maxTransactionAmount).div(1000);
        _maxWallet = totalSupply().mul(_maxWallet).div(1000);
    ...
    function changeBuyTaxes(uint256 marketing, uint256 liquidity, uint256 burn) external onlyOwner {
        buyTax = Taxes(marketing, liquidity, burn, marketing + liquidity + burn);
        require(buyTax.total <= 5, "Must keep fees at 5% or less.");
    }

    function changeSellTaxes(uint256 marketing, uint256 liquidity, uint256 burn) external onlyOwner {
        sellTax = Taxes(marketing, liquidity, burn, marketing + liquidity + burn);
        require(sellTax.total <= 10, "Must keep fees at 10% or less.");
    }

    function changeMarketingWallet(address _marketingWallet) external onlyOwner {
        require(_marketingWallet != zeroAddress, "Marketing Address cannot be zero address");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (takeFee) {
            if (automatedMarketMakerPairs[to] && sellTax.total > 0) {
                ...
                tokensForBurn += (to == marketingWallet ? fees : (fees * buyTax.burn) / buyTax.total);
            }
