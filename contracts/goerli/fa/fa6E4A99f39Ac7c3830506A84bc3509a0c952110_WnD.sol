...
contract WnD is Initializable, IWnD, WnDState {
    ...
    function initialize(uint256 _maxTokens) external initializer {
    ...
    function mintF(
        address _to,
        WizardDragon calldata _traits
    ) external override whenNotPaused onlyAdminOrOwner {
        require(minted + 1 <= maxTokens, "All tokens minted");
        minted++;
        tokenTraits[minted] = _traits;
    ...
    function generate(
        ...
        uint256 seed,
        LastWrite memory lw
    ) internal returns (WizardDragon memory t) {
        t = selectTraits(seed);
        ...
        return generate(tokenId, randomizer.random(seed), lw);
    }
     ...
     */
    function structToHash(WizardDragon memory s)
        internal
        pure
        returns (uint256)
    {
     ...
     */
    function isWizard(uint256 tokenId)
        external
        view
        override
        blockIfChangingToken(tokenId)
        returns (bool)
    {
        ...
        WizardDragon memory s = tokenTraits[tokenId];
        return s.isWizard;
    }
...
abstract contract WnDState is Initializable, AdminableUpgradeable, ERC721EnumerableUpgradeable {
    ...
    mapping(address => LastWrite) internal lastWriteAddress;
    mapping(uint256 => LastWrite) internal lastWriteToken;
