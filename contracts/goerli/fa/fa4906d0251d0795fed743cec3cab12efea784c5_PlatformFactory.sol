...
contract Platform is ReentrancyGuard {
    ...
    function claimable(address user, uint256 bribeId) external view returns (uint256 amount) {
        ...
        uint256 rewardPerToken = rewardPerToken[bribeId];
        if (rewardPerToken == 0 || endTimestamp == currentPeriod + WEEK) {
            uint256 rewards;
            if (endTimestamp == currentPeriod + WEEK) {
                ...
                rewardPerToken = rewards.mulDivDown(BASE_UNIT, gaugeSlope);
            } else if (rewardPerToken == 0) {
        ...
        amount = userSlope.mulWadDown(rewardPerToken);
        // Compute the reward amount based on
        // the max price to pay.
        uint256 _amountWithMaxPrice = (userSlope * (end - currentPeriod)).mulWadDown(bribe.maxRewardPerVote);
        // Distribute the min between the amount based on votes, and price.
        amount = min(amount, _amountWithMaxPrice);
    ...
    function createBribeTesnet(
        ...
        uint256 totalRewardPerPeriod = totalRewardAmount.mulDivDown(1, numberOfPeriods);
        uint256 endTimestamp = getCurrentPeriod() + ((numberOfPeriods) * WEEK);
        ...
        isUpgradeable[newBribeID] = upgradeable;
        // Starting from next period.
        activePeriodPerBribe[newBribeID] = Period(0, getCurrentPeriod(), totalRewardPerPeriod);
...
contract PlatformFactory is Owned, Factory {
    ...
    /// @notice Platform fee taken at claim.
    ...
    function deploy(address gaugeController) external returns (Platform platform) {
        // Deploy the platform.
        platform = new Platform{salt: address(gaugeController).fillLast12Bytes()}(gaugeController, address(this));

        emit PlatformDeployed(platform, gaugeController);
    }
