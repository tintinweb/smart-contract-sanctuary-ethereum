...
contract Conatus is iConatus, ERC721, ReentrancyGuard, Ownable {
  ...
  uint256[] private _prices = [1 ether, 0.5 ether, 0.5 ether, 0.01 ether];
  uint256[] private _totalAmounts = [3, 3, 5, 50];
  ...
  function mint(uint256 _type)
    ...
    checkMintable(_type)
  {
    require(msg.value == _prices[_type - 1], "Conatus: Invalid value");
  ...
  function mintForVendingMachine() internal {
    uint256 _type = 4;
    uint256 totalAmount = _totalAmounts[_type - 1];
    require(
      _mintedAmounts[_type - 1] < totalAmount,
      "Conatus: Already minted all"
    );
