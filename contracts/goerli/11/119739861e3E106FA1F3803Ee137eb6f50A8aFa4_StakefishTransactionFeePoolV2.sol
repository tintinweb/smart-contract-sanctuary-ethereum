    ...
    ReentrancyGuard
{
    ...
    // @dev: Updates contract uptime (idempotent)
    ...
    // @param validatorPubKey: The validator's public key
    // @param joinTime: The timestamp as of which the validator stopped accruing payable uptime
    function partPool(
        bytes calldata validatorPubKey,
        uint256 leaveTime
    ) external override nonReentrant operatorOnly {
    ...
    // @param depositorAddresses: The depositor addresses to associate with the validators. If length is 1, then
    //   the same depositor address is used for all validators. Otherwise the array must have length equal to
    //   validatorPubKeys.length/48.
   ...
   function pendingReward(address depositorAddress) external override view returns (uint256, uint256) {
    ...
    // @param amount: The amount that will be credited to the `beneficiary`. If 0, collect all fees.
    ...
    // @notice A summary of a depositor's activity in the pool
    // @dev A summary of a depositor's activity in the pool
