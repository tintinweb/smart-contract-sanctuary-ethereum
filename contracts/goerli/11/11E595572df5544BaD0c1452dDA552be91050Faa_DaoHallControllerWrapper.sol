...
// File contracts/registrar/DaoHallControllerWrapper.sol



pragma solidity ^0.8.4;
contract DaoHallControllerWrapper is IDaoHallController, Operable {
    ...
    constructor(NameWrapper _wrapper) {
        wrapper = _wrapper;
    }
    ...
    function setSubnodeRecord(
        ...
        wrapper.setSubnodeRecord(node, label, owner, resolver, ttl, fuses, expiry);
    }

    function setSubnodeExpiry(bytes32 node, string calldata label, uint64 expiry) external override onlyOperator{
        bytes32 labelhash = keccak256(bytes(label));
        wrapper.setChildFuses(node, labelhash, 0, expiry);
    }
    
    function getNameData(bytes32 labelhash) external view returns (address owner, uint64 expiry) {
        bytes32 node = keccak256(abi.encodePacked(ETH_NODE, labelhash));
        (owner,, expiry) = wrapper.getData(uint256(node));
    }

    function getNodeData(bytes32 node) external view returns (address owner, uint64 expiry) {
        (owner,, expiry) = wrapper.getData(uint256(node));
    }
}