    ...
    EIP712("Watch Chain Limit Order", "1")
{
    ...
    uint256 private constant _FUNDING_PERIOD = 10 days;
    ...
    function addWatch(
        ...
        require(shares > 0, "price is too small");
        require(
            kycInfo[watchOwner].verification == true || watchOwner == wcnFundAddress,
    ...
    function watchStatus(uint256 watchId) public view returns (Status) {
        if (watchId < watches.length) {
            ...
            if (watch.shares > 0) {
                ...
                if (
                    (block.timestamp - watch.fundingStartTime) > _FUNDING_PERIOD
                ) {
