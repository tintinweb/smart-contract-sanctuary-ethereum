...
contract TPToken is ERC20Snapshot, ERC20Burnable, TPWhitelistAndBlacklist, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant TPTOKEN_ADMIN_ROLE = keccak256("TPTOKEN_ADMIN_ROLE");
    ...
    constructor(
        ...
        address signer_
    ) ERC20(name_, symbol_) TPWhitelistAndBlacklist(signer_) {
    ...
    function setPair(address pair, bool isPair_) public onlyRole(TPTOKEN_ADMIN_ROLE) {
        require(pair != address(0), "TPE003");
        _pairs[pair] = isPair_;
    }

    function setSwaper(address swaper, bool isSwaper_) public onlyRole(TPTOKEN_ADMIN_ROLE) {
        require(swaper != address(0), "TPE004");
        _swapers[swaper] = isSwaper_;
    }

    function snapshot() public onlyRole(TPTOKEN_ADMIN_ROLE) {
        _snapshot();
    }

    function blacklist(address account) public override onlyRole(TPTOKEN_ADMIN_ROLE) {
        _blacklist(account);
    }

    function whitelist(address account) public override onlyRole(TPTOKEN_ADMIN_ROLE) {
        _whitelist(account);
    }

    function updateMerkleRoot(bytes32 merkleRoot_, string memory uri_) public override onlyRole(TPTOKEN_ADMIN_ROLE) {
        _updateMerkleRoot(merkleRoot_, uri_);
    }

    function setSigner(address newSigner) public override onlyRole(TPTOKEN_ADMIN_ROLE) {
    ...
    function _beforeTokenTransfer(
        ...
        require(whitelisted(from) || from == address(0), "AAAAA"); //TBD: could be needed if blacklisted with balance > 0?
        require(whitelisted(to) || to == address(0), "BBBBB");
