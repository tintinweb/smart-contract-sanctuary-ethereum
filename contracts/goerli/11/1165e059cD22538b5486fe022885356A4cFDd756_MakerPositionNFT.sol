...
contract MakerPositionNFT is BumperCoreNFT {
    function initialize(
        string memory name,
        ...
        __BumperCoreNFT__init(name, symbol, _token, _config);
    }
    ...
    function _exists(uint tokenId)
        ...
        returns (bool)
    {
        return IMarketPositionManagement(market).existsMakerPosition(tokenId);
    }

    // Terrible name. Should be getOwnerById
    function ownerOf(uint tokenId) public view override returns (address) {
        return IMarketPositionManagement(market).makerPositionOwner(tokenId);
    }
    ...
    function _transfer(
        ...
        IMarketPositionManagement(market).safeTransferMakerPosition(
            from,
            to,
            tokenId
        );
    ...
    function balanceOf(address owner) public view override returns (uint) {
        return IMarket(market).getMakerPositions(owner).length;
    }
}
...
/// @notice core for bumper NFT positions
abstract contract BumperCoreNFT is
    IERC721Upgradeable,
    ...
    ModuleAccessControllerUpgradeable
{
    ...
    modifier onlyPermittedAdapter() {
        require(
            hasRole(ADAPTER_ROLE, msg.sender),
            "CNFT: adapter-not-permitted"
        );
        _;
    }

    /// @notice requires market to be not paused
    modifier whenMarketNotPaused() {
        require(
            PausableUpgradeable(market).paused() == false,
            "CNFT: market-is-paused"
        );
        _;
    }

    /// @dev initializer of a upgradeable pattern
    /// @param name_ token name
    /// @param name_ token symbol
    /// @param token_ asset token address
    /// @param config_ protocol config address
    function __BumperCoreNFT__init(
        string memory name_,
        ...
        _name = name_;
        _symbol = symbol_;
        config = IProtocolConfig(config_);
        market = config.getMarket(token_);
    ...
    function _transfer(
        ...
        address to,
        uint tokenId
    ) internal virtual whenMarketNotPaused {
        require(
            ownerOf(tokenId) == from,
            "ERC721: transfer must be called by owner"
        );
        require(to != address(0), "ERC721: must not transfer to zero address");
