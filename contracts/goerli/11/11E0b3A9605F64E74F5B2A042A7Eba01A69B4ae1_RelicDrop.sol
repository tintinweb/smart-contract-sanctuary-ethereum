...
contract RelicDrop is ERC1155Receiver, AllowList, Administration {
    ...
    uint256 private immutable _STARTING_INDEX = 1500;
    ...
    struct Sale {
        // The starting token id for the sale.
        uint256 startIndex;
        // The ending token id for the sale.
        uint256 endIndex;
        // The token index of the next token to be minted.
        uint256 nextIndex;
    ...
    function hasParticipatedInPresale(bytes32 nonce)
        public
        view
    ...
    function getAllowList(uint256 id)
        public
        view
        returns (AllowListData memory allowlist)
    {
        allowlist = _returnAllowList(bytes32(id));
    }

    function getTicket(address ticketAddress)
        public
        view
    ...
    function maxSupply(uint256 id) public view returns (uint256 supply) {
        Sale memory sale = _sales[id];
        unchecked {
            supply = _maxSupply(sale.startIndex, sale.endIndex);
        }
    }
    ...
    function totalMinted(uint256 id) public view returns (uint256 minted) {
        Sale memory sale = _sales[id];
        unchecked {
            minted = _totalMinted(sale.startIndex, sale.nextIndex);
        }
    }
    ...
    function createDrop(
        ...
        // TODO: review the indexing logic
        Sale storage _sale = _sales[id];

        if (_sale.startTime != 0) {
            revert DropExists();
        }

        if (startTime > endTime) {
            revert InvalidDropConfig();
        }

        if (_sale.nextIndex != 0) {
            revert InvalidDropConfig();
        }

        unchecked {
            uint256 startIndex = _highestIndex;
            uint256 endIndex = startIndex + quantity - 1;

            // Increment the highest index
            _highestIndex = endIndex;

            _sale.activeFrom = activeFrom;
            _sale.startIndex = startIndex;
            _sale.endIndex = endIndex;
            _sale.startTime = startTime;
            _sale.endTime = endTime;
            _sale.price = price;
            _sale.nextIndex = startIndex;
        }
    }

    function updateDropAllowList(
        uint256 id,
        ...
        if (_maxSupply(_sale.startIndex, _sale.endIndex) == 1) {
            revert InvalidDropConfig();
        }
    ...
    function updateDropTicket(
        ...
        uint256 supply = _maxSupply(_sale.startIndex, _sale.endIndex);
    ...
    function presaleMint(
        ...
        // verify the drop exists
        if (_maxSupply(sale.startIndex, sale.endIndex) == 1) {
            revert InvalidDropConfig();
        }
        ...
        unchecked {
            sale.nextIndex = nextIndex + quantity;
        }

        // Mint
        _mint(_msgSender(), _relicsContract, nextIndex, quantity);
    }

    function publicMint(uint256 id, uint256 quantity)
        external
        payable
        whenNotPaused
    {
    ...
    function onERC721Received(
        ...
        _verifyQuantity(
            ...
            sale.startIndex,
            sale.endIndex,
            // Set the tickets to zero to ensure the pre-sold ticket can be redeemed
            0
        );
        ...
        // Add mint quantity to drop next index
        unchecked {
            sale.nextIndex = nextIndex + 1;
        }

        // Mint the relics
        // Only 1 relic can be minted when an ERC721 is received
        _mint(from, _relicsContract, nextIndex, 1);
    ...
    function onERC1155Received(
        ...
        if (id != ticket.tokenId) {
            revert InvalidTicket(id, ticket.tokenId);
        }
        ...
        // Mint the relics
        _mint(from, _relicsContract, nextIndex, value);
    ...
    function _hashPresaleData(Presale memory presale)
        internal
        pure
    ...
    function _verifyPresale(uint256 quantity, Presale calldata presale)
        internal
    {
        // Verify quantity is not more than allowed
        if (quantity > presale.limit) {
            revert InvalidQuantity(quantity, presale.limit);
        }

        // Verify nonce has not been used
        if (_participated[presale.nonce] == true) {
    ...
    function _verifyTicketsRedeemed(Sale storage sale, uint24 value) internal {
        if (sale.redeemed + value > sale.tickets) {
            revert TicketQuantityExceeded(sale.tickets);
        }
    ...
    function _verifyQuantity(
        ...
        unchecked {
            uint256 maxSupply_ = _maxSupply(startIndex, endIndex);
            uint256 totalMinted_ = _totalMinted(startIndex, nextIndex);
            uint256 newIndex = (nextIndex + quantity + tickets) - 1;
            ...
            // console.log("maxSupply_", maxSupply_);
            // console.log("totalMinted_", totalMinted_);
            // console.log("newIndex > endIndex", newIndex > endIndex);

            if (newIndex > endIndex) {
                revert RequestExceedsMaxSupply(
                    quantity,
                    totalMinted_,
                    maxSupply_
                );
    ...
    function _mint(
        ...
        for (uint256 i = 0; i < quantity; i++) {
            IERC721(contractAddress).mint(to, nextIndex);
            nextIndex++;
        }
    }

    function _totalMinted(uint256 startIndex, uint256 nextIndex)
        internal
        pure
        returns (uint256)
    {
    ...
    function _maxSupply(uint256 startIndex, uint256 endIndex)
        internal
        pure
        returns (uint256)
    {
...
contract AllowList {
    ...
    function verifyProof(
        ...
        return
            MerkleProof.verify(
                merkleProof,
                d.merkleRoot,
                _formatLeaf(from, data)
            );
    }

    function _formatLeaf(address from, bytes memory data)
        internal
        pure
        returns (bytes32)
    {
    ...
    function _returnAllowList(bytes32 list)
        internal
        view
        returns (AllowListData memory)
    {
        return _allowListMerkleRoots[list];
    }

    function _updateAllowList(bytes32 id, AllowListData calldata allowListData)
        internal
        virtual
    {
