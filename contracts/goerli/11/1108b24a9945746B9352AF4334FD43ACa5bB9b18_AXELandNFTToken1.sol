...
contract AXELandNFTToken1 is ERC721, Ownable {
    ...
    function setBaseURI(string calldata baseURIItem) external onlyOwner {
        baseURI = baseURIItem;
    }
    ...
    function getTokenOfAddress(address owner) public view returns (uint256[] memory) {
        uint256 balanceOfTheAddress = balanceOf(owner);
        uint256[] memory result = new uint256[](balanceOfTheAddress);
        unchecked {
            uint256 operationIndex = 0;
            for (uint256 index = 0; index < totalSupply; index++) {
                uint256 tokenId = index + 1;
                if (_ownerOf[tokenId] == owner) {
                    result[operationIndex] = tokenId;
                    operationIndex += 1;
                }
            }
        }
