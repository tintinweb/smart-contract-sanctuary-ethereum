...
contract ERC20Vault {
    ...
    function validateAction(
        ...
        if (_actions[actionId].validatorCnt == 0) {
            _actions[actionId] = ActionInfo(action, actionData, 1, 1);
        } else {
            require(
            ...
            require(_actions[actionId].action == action, "Action Mismatch");
            require(
        ...
        if (_actions[actionId].readCnt == _validators.length()) {
            ...
            if (_actions[actionId].validatorCnt < _threshold) {
                emit ActionFailure(actionId); // Quorum Failed, manual intervention required
            }
        }
    ...
    function validateAddValidator(uint128 actionId, address newValidator)
        external
    {
        require(!_validators.contains(newValidator), "Already a validator");
    ...
    function validateRemoveValidator(uint128 actionId, address oldValidator)
        external
    {
        require(_threshold <= _validators.length() - 1, "Threshold too high");
        require(
            _validators.contains(oldValidator),
        ...
        if (res == ValidationRes.Execute) {
            _validators.remove(oldValidator);
        }
    }
    ...
    modifier checkLockUSDC(uint256 amount) {
        require(amount > 1000000000000000, "Sending amount is too small!");
