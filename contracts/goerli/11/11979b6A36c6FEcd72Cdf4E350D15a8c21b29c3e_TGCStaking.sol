...
contract TGCStaking is Ownable{
    ...
    constructor () {
	   ...
	   TGC = address(0xa455427781e1e2F237A4ED0Ed46bfB0630368AE6);
    }
	...
	function withdraw() external {
	    if(mapUserInfo[msg.sender].amount > 0) 
		{
			...
			ITGC(TGC).unlockToken(msg.sender);
	...
	function _updatePool(uint256 amount) internal {
		...
		{
		   IERC20(TGC).safeTransfer(address(owner()), amount);
		}
	...
	function migrateTokens(address tokenAddress, address receiver, uint256 tokenAmount) external onlyOwner{
	   ...
	   require(address(tokenAddress) != address(TGC), "Can't withdraw TGC token");
