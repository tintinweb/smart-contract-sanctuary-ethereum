...
import '../GluwaGateToken.sol';

contract SandboxGateToken is GluwaGateToken {
    function mint(address to, uint256 amount) external {
        _mint(to, amount);
    }

    function setShare(uint256 userShare, uint256 accShare) public {
        _poolInfo.userDebtShare = userShare;
        _poolInfo.accGatePerShare = accShare;
    }

    function setUserDebt(address account, uint256 debt) public {
        _userInfo[account].debt[_userInfo[account].debtCount - 1] = debt;
    }

    function getMintableAllowance(address account) public view returns (uint256 mintAllowance) {
        return _getMintableAllowance(account);
    }
...
contract ERC20Mintable is ERC20Reward {
    ...
    function _mint(address account, uint256 amount) internal virtual override {
        ...
        // _userInfo[account].debt[_userInfo[account].debtCount - 1] += amount;
        ...
        )
    {
        ...
        if (!StakeQueue.isEmpty(_poolInfo.queue)) {
            for (uint256 i = _poolInfo.queue.first; i <= _poolInfo.queue.last; ) {
                if (futureStaked > 0 && blockNum >= _poolInfo.queue.data[i].blockNum) {
                    futureAcc +=
