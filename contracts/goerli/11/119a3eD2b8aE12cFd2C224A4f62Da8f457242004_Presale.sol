    ...
    PausableUpgradeable
{
    ...
    function buyWithETHWert(address user, uint256 amount) external payable whenNotPaused returns(bool) {
        // if(whitelistClaimOnly){
        require(isWhitelisted[_msgSender()], "User not whitelisted for this tx" );
        // }
        uint256 usdPrice = calculatePrice(amount);
        uint256 ethAmount = (usdPrice * baseDecimals) / getLatestPrice();
        require(msg.value >= ethAmount, "Not enough amount");
        uint256 excess = msg.value - ethAmount;
        if (totalTokensSold > rounds[0][currentStep]) currentStep += 1; // addeed this
