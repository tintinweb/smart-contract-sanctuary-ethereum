...
// File contracts/pools/HPoolManager.sol
 ...
 * Github: madjarevicn
 */
contract HPoolManager is ERC1155HolderUpgradeable, HordUpgradable, ReentrancyGuardUpgradeable {
    using SafeMath for *;
    ...
    // Mapping is champion withdraw deposit
    mapping(uint256 => bool) public isChampionDepositWithdrawn;
    ...
    event PlatformFeesWithdrawn(address beneficiary, uint256 amount);
    ...
    /**
         ...
         * @param           _hordTicketFactory is the address of the HordTicketFactory contract
         ...
         * @param           _chainlinkOracle is the address of the ChainlinkOracle contract
         * @param           _hordConfiguration is the address of the HordConfiguration contract
     */
    function initialize(
        address _hordCongress,
        address _maintainersRegistry,
    ...
    isPoolIdValid(poolId)
    {
        ...
        require(msg.sender == hp.poolContractAddress, "Caller is not HPool contract");
    ...
    /**
         * @notice          Function to start private subscription phase. Can be started only if current
         *                  state of the hPool is TICKET_SALE.
         * @param           poolId is the ID of the pool contract.
     */
    ...
    nonReentrant
    {
        ...
        require(
            block.timestamp <= hp.privateSubscriptionEndTime,
            "The time has elapsed for the private subscription phase."
        );
    ...
    /**
        * @notice          Function to start public subscription phase. Can be started only if current
        *                  state of the hPool is PRIVATE_SUBSCRIPTION.
        * @param           poolId is the ID of the pool contract.
    */
    ...
    isPoolIdValid(poolId)
    {
        ...
        require(
            block.timestamp > hp.privateSubscriptionEndTime &&
            hp.followersEthDeposit < convertUSDValueToETH(hordConfiguration.maxFollowerUSDStake()),
            "Previous Phase isn't over yet, or pool reached maximal size."
        );
    ...
    isPoolIdValid(poolId)
    {
        ...
        require(
            block.timestamp <= hp.publicSubscriptionEndTime,
            "The time has elapsed for the public subscription phase."
        );
        ...
        uint256 feeAmount = msg.value
            .mul(hordConfiguration.percentBurntFromPublicSubscription())
    ...
    isPoolIdValid(poolId)
    {
        hPool storage hp = hPools[poolId];
        require(
            (hp.poolState == PoolState.PUBLIC_SUBSCRIPTION && hp.publicSubscriptionEndTime < block.timestamp) ||
            (hp.poolState == PoolState.PRIVATE_SUBSCRIPTION && hp.privateSubscriptionEndTime < block.timestamp &&
            hp.followersEthDeposit >= convertUSDValueToETH(hordConfiguration.maxFollowerUSDStake())),
            "Conditions for init hPool are not met."
        );
        require(
        ...
        poolContract.mintHPoolToken(name, symbol, hordConfiguration.totalSupplyHPoolTokens(), address(hordConfiguration), address(matchingMarket));
        // Add hPoolContract on white list
        isWhitelisted[address(poolContract)] = true;
        // Store addresses of all HPoolTokens
        isHPoolToken[address(poolContract)] = true;
        // Set the deployed address of hPool
        hp.poolContractAddress = address(poolContract);
        ...
        uint256 totalDeposit = hp.followersEthDeposit.add(hp.championEthDeposit);
        uint256 amountEth = totalDeposit.sub(treasuryFeeETH);
        uint256 amountUSD = uint256(getLatestETH2USDPrice()).mul(amountEth).div(getDecimalsReturnPrecision());

        poolContract.depositBudget{
            value: amountEth
    ...
    /**
         * @notice          Function to withdraw deposit to champion. It can be called whenever after subscription phase.
         * @param           poolId is the ID of the pool for which user is withdrawing.
     */
    function withdrawDepositToChampion(
        uint256 poolId
    )
    external
    isPoolIdValid(poolId)
    nonReentrant
    {
        ...
        require(
            !isChampionDepositWithdrawn[poolId],
            "Deposit already withdrawn"
        );

        // Mark that champion withdrawn his subscription.
        isChampionDepositWithdrawn[poolId] = true;
        // Transfer subscription back to champion
        safeTransferETH(msg.sender, hp.championEthDeposit);
        // Fire ChampionWithdrawn event
        emit ChampionWithdrawn(
            poolId,
            msg.sender,
    ...
    nonReentrant
    {
        ...
        emit PlatformFeesWithdrawn(address(hordTreasury), amount);
    }
    ...
    nonReentrant
    {
        ...
        address[] memory path = IHPoolHelper(hPoolFactory.hPoolHelper()).getBestSellRoute(hordToken);
        ...
        emit BuyAndBurn(amounts[0], amounts[targetTokenIdx]);
    }

    /**
        * @notice          Function to add more protocol fees in ETH
     */
    function addProtocolFeeETH()
    payable
    external
    {
     ...
     */
    function getLatestETH2USDPrice()
    public
    view
    returns (int256)
    {
        ...
        require(price > 0, "Price can not be negative integer.");
        uint256 decimalsDiff = 18 - uint256(chainLinkOracle.decimals());
        if(decimalsDiff > 0) {
            return price * int256(10**decimalsDiff);
        }
        return price;
    }
    ...
    returns (uint256)
    {
        uint256 returnPrecision = 10 ** 18;
        return returnPrecision;
    }

    function convertUSDValueToETH(
        uint256 amount
    )
    public
    view
     ...
     */
    function getRequiredNumberOfTicketsToUse(
    ...
    */
    function getNumberOfTicketsUsed(
        uint256 poolId
    )
    external
    view
        ...
        returns (uint256, uint256, bool)
    {
        ...
        return (subscription.amountEth, subscription.numberOfTickets, subscription.isSubscriptionWithdrawnPoolTerminated);
    }
     ...
     * @notice          Function to set chainlink oracle
     */
    function setChainLinkOracleInternal(
        address _chainLinkOracle
    )
    internal
    {
