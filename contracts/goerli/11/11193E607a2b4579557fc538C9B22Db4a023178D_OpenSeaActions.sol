...
/**
 * @title OpenSeaActions
 * @dev Library for creating Seaport orders.
 */
contract OpenSeaActions is IBuyEvents, IListEvents {
    error NFTNotAllowed();
    ...
    modifier onlyGovernorOrCaptain() {
        ShipModule shipModule = ShipModule(payable(address(msg.sender)));
        address caller = shipModule.caller();
        if (shipModule.governor() != caller && shipModule.captain() != caller) {
            revert NotGovernorOrCaptain();
        }
        _;
    }

    function buy(
        address sailHelperAddress,
        Order memory seaportOrder
    ) public onlyCaptain {
        ShipModule shipModule = ShipModule(payable(msg.sender));
        SailHelper sailHelper = SailHelper(sailHelperAddress);
        ...
        // Revert if the NFT is not allowed
        shipModule.verifyBuyRestrictions(nftContract);

        shipModule.setBought();
        ...
        ConsiderationInterface(sailHelper.seaport()).fulfillOrder{value: value}(
            seaportOrder,
            bytes32(0)
        );
     ...
     * @param sailHelperAddress address of the sail helper
     * @param lp parameters needed to publish a listing
    ...
    function list(
        address sailHelperAddress,
        ListParams memory lp
    ) public virtual onlyCaptain {
        if (ShipModule(payable(msg.sender)).isRaiseOpen()) {
            revert ListNotAllowed();
        }

        if (lp.royaltyRecipients.length != lp.royaltyAmounts.length) {
        ...
        // Check transfer
        if (IERC721(lp.nftContract).ownerOf(lp.tokenID) != address(this)) {
            revert OrderFailed();
        }

        if (sailHelper.royaltyEngine() != address(0)) {
            (
...

            {
                ...
                unchecked {
                    for (
                        uint256 i = 0;
                        i < declaredRoyaltyRecipients.length;
                        ++i
                    ) {
                        for (
                            uint256 j = 0;
                            j < lp.royaltyRecipients.length;
                            ++j
                        ) {
                            // Ensure recipient the same and the min amount provided
                            if (
                                declaredRoyaltyRecipients[i] ==
                                lp.royaltyRecipients[j] &&
                                lp.royaltyAmounts[j] >=
                                declaredRoyaltyAmounts[i]
                            ) {
                                ++verifiedRoyalties;
                                break;
                            }
        ...
        {
            uint256 endTime = block.timestamp + lp.duration;

            Order memory seaportOrder = sailHelper.buildOrderFor(
                address(this),
                payable(address(this)),
                lp.royaltyRecipients,
                lp.nftContract,
                lp.tokenID,
                lp.amount,
                lp.royaltyAmounts,
    ...
    ) public virtual onlyGovernorOrCaptain {
        ShipModule shipModule = ShipModule(payable(msg.sender));
        if (
            !ConsiderationInterface(shipModule.sailHelper().seaport()).cancel(
                orders
            )
        ) {
            revert CancelFail();
     ...
     */
    function _list(SailHelper sailHelper, Order memory seaportOrder) internal {
