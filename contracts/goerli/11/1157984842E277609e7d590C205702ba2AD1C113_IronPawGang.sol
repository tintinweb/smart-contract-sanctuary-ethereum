...
contract IronPawGang is ERC721A, ERC2981, Ownable, ReentrancyGuard {
    ...
    constructor(string memory initialBaseURI, address payable royaltiesReceiver) ERC721A("Iron Paw Gang", "IPG") {
    ...
    function swapDrivers(uint256 a, uint256 b) public nonReentrant {
    ...
    function toggleListActive(string calldata list, bool active) external onlyOwner {
        _lists.active[list] = active;
    }
        ...
        nonReentrant
    {
        ...
        uint256 alreadyUsed = _lists.usageCount(list, _msgSender());
        if (amount > maxAmount - alreadyUsed) revert MaxAmountExceededError();

        _lists.incrementUsageCount(list, _msgSender(), amount);
