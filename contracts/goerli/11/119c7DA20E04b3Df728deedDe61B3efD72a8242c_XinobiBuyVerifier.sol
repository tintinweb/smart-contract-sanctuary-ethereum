...
import "./interfaces/IXinobiBuyVerifier.sol";
import "../Utils/AccessCotrolled.sol";
import "../Utils/Charge.sol";
import "../Utils/Accounts.sol";

contract XinobiBuyVerifier is IXinobiBuyVerifier, AccessCotrolled {
    using ECDSA for bytes32;
    // TODO: ERC1155のトークンIDバッティングに対応する
    // contract -> token -> nonce
    mapping(address => mapping(uint256 => List)) private _nonce;
    address private _signer;

    constructor(address accessControl, address signer)
        AccessCotrolled(accessControl)
    {
        _signer = signer;
    }
    ...
    modifier onlySigner() {
        require(
            msg.sender == _signer,
            "no authenticated. you are not signer."
        );
        _;
    }
    ...
    {
        List memory _list = _nonce[collectionAddress][tokenId];
        return (_list.seller, _list.nonce);
    }

    function list(address collectionAddress, uint256 tokenId, address seller)
        external
        onlySigner
    {
        ...
        _nonce[collectionAddress][tokenId] = List(seller, rand);
    }
    ...
    function resetNonce(address collectionAddress, uint256 tokenId)
        external
        onlyBroker
    {
        _nonce[collectionAddress][tokenId] = List(address(0), 0);
    }
    ...
    ) external view onlyBroker returns (bool) {
        List memory _list = _nonce[collectionAddress][tokenId];
        return
            _list.nonce != 0 &&
            _list.seller == accounts.seller &&
            keccak256(
                abi.encodePacked(
                    collectionAddress,
                    tokenId,
                    ...
                    buyer,
                    accounts.seller,
                    accounts.royalityReceievr,
                    accounts.affiliater,
                    charge.royaltyPercentage,
                    charge.affiliatePercentage,
                    charge.operatingPercentage,
                    _list.nonce
                )
            ).toEthSignedMessageHash().recover(signature) == _signer;
    }
    ...
    function verify(
        ...
        return
            ...
            ).toEthSignedMessageHash().recover(signature) == seller;
    }
}
