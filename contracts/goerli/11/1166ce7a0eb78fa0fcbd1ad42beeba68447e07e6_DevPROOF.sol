...
import "./PROOFCollective.sol";

contract DevPROOF is PROOFCollective {
    constructor(string memory baseURI)
        PROOFCollective("DevPROOF", "DP", payable(msg.sender))
    {
        baseTokenURI = baseURI;
    }

    function devmint(address to, uint256 num) public bypassSellerConfig {
        _purchase(to, num);
    }

    function devmint(uint256 num) external {
        devmint(msg.sender, num);
    }

    modifier bypassSellerConfig() {
        DutchAuctionConfig memory dutchSave = dutchAuctionConfig;
        SellerConfig memory sellerSave = sellerConfig;

        dutchAuctionConfig.startPoint = block.timestamp;
        dutchAuctionConfig.startPrice = 0;
        dutchAuctionConfig.decreaseSize = 0;
        ...
        dutchAuctionConfig = dutchSave;
        sellerConfig = sellerSave;
    }
}
