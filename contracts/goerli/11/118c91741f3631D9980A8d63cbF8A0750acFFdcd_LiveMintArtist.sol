...
{   
    ...
    event ArtistContractAddressChanged(uint[] ArtistIDs, address[] NewContractAddresses);
    ...
    { 
        ...
        Artists[15]._MintPass = 0xA3B2031DE045db639936d4A4cc0f3618F2cf0209; // goerli
        Artists[15]._ProjectID = 2; // goerli
        Artists[15]._Minter = 0x9F19DC02Fe3FdF5C9242812c355226186Cf3b71B; // goerli
        Artists[15]._ERC20 = 0x9B067ff0bE00e3c9a7F5511D58C4b564D7793784; // goerli
     ...
     * @dev Changes Name
     */
    function __ChangeName(uint ArtistID, string calldata Name) external onlyOwner { Artists[ArtistID]._Name = Name; }

    /**
     * @dev Modifies The Current ArtBlocks ProjectID
     **/
    function __ChangeProjectID(uint ArtistID, uint ArtBlocksProjectID) external onlyOwner 
    { 
     ...
     */
    function __ChangeArtistMinterAddress(uint[] calldata ArtistIDs, address[] calldata NewAddresses) external onlyOwner
    {
        ...
        emit ArtistContractAddressChanged(ArtistIDs, NewAddresses);
    }
    ...
    {
        for(uint x; x < ArtistIDs.length; x++) { Artists[ArtistIDs[x]]._Root = NewRoots[x]; }
        emit MerkleRootChanged(ArtistIDs, NewRoots);
    }
    ...
    function readUnmintedTicketIDs(uint[] calldata ArtistIDs, uint Range) public view returns(uint[][] memory TokenIDs)
    {
        ...
        for(uint ArtistID; ArtistID < ArtistIDs.length; ArtistID++)
        {
            ...
            for(uint TokenID; TokenID < Range; TokenID++)
            {
                try IERC721(Artists[ArtistIDs[ArtistID]]._MintPass).ownerOf(TokenID)
                {
                    if(
