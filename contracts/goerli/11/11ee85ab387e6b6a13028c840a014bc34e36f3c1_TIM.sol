...
contract TIM is ERC1155, ReentrancyGuard, Ownable {
    ...
    function mintTo(string[] memory _stringLines, uint256 _quantity, address[] memory _to, bool asAirdrop) public payable {
        ...
        require(_stringLines.length <= maxLines, "Need <18 array items of text.");
        
        for(uint256 i = 0; i < _stringLines.length; i++) {
            require(bytes(_stringLines[i]).length <= maxLineLength, "Line too long.");
        }

        if(_stringLines.length < maxLines) {
            for(uint256 i = _stringLines.length; i < maxLines; i++) {
                _stringLines[i] = "";
            }
        }
    ...
    function getTextLinesOneThroughSix(uint256 startY, string[] memory stringLine) private pure returns (string memory) {
        return string(
            abi.encodePacked(
    ...
    function getTextLinesSevenThroughTwelve(uint256 startY, string[] memory stringLine) private pure returns (string memory) {
        return string(
            abi.encodePacked(
                getSVGTextLine(startY, stringLine[6]),
                getSVGTextLine(startY + 20, stringLine[7]),
                getSVGTextLine(startY + 40, stringLine[8]),
                getSVGTextLine(startY + 60, stringLine[9]),
                getSVGTextLine(startY + 80, stringLine[10]),
                getSVGTextLine(startY + 100, stringLine[11])
            )
        );
    }

    function getTextLinesThirteenThroughEighteen(uint256 startY, string[] memory stringLine) private pure returns (string memory) {
        return string(
            abi.encodePacked(
                getSVGTextLine(startY, stringLine[12]),
                getSVGTextLine(startY + 20, stringLine[13]),
                getSVGTextLine(startY + 40, stringLine[14]),
                getSVGTextLine(startY + 60, stringLine[15]),
                getSVGTextLine(startY + 80, stringLine[16]),
                getSVGTextLine(startY + 100, stringLine[17])
            )
        );
    }
        ...
        return
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        header,
                        getTextLinesOneThroughSix(0, stringLine),
                        getTextLinesSevenThroughTwelve(120, stringLine),
                        getTextLinesThirteenThroughEighteen(240, stringLine),
                        '</text>',
                        footer,
                        '</svg>'
                    )
