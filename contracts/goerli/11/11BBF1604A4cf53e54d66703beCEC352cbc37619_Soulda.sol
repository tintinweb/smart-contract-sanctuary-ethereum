...
contract Soulda is Ownable, ERC721A, ReentrancyGuard {
    ...
    function whitelistMint(address _account, uint256 _quantity, bytes32[] calldata _proof, string memory _soulType) external payable callerIsUser {
        ...
        require(totalMinted[msg.sender] + _quantity <= 3, "You can only mint 3 NFT on the Soulda Whitelist Sale");
        // require(totalSupply() + _quantity <= MAX_WHITELIST, "Max supply exceeded");
        soulaType[totalSupply() +1] = _soulType;
    ...
    function publicSaleMint(address _account, uint256 _quantity, string memory _soulType) external payable callerIsUser {
        require(currentTime() > saleStartTime + 60 minutes, "Soulda public mint is not open yet");
        require(totalMinted[msg.sender] + _quantity <= 3, "You can only mint up to 3 Soulda NFTs");
