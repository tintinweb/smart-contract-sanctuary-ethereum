...
contract Launchpad {
    ...
    constructor (address _admin, address _offeringToken) {
        admin = _admin;
    ...
    function initialize(
        ...
        uint256 _tier1MaxBNB,
        uint256 _tier2MaxBNB
    ) public {
        if(msg.sender != admin) revert NotPermitted();
        if (isInitialized) revert NotInitialized();
        ...
        if (_raisingAmount == 0) revert InvalidAmount();
        if (_offeringAmount == 0) revert InvalidAmount();
        if (_minimumRequirementForTier2 == 0) revert InvalidAmount();
        ...
        emit init(_startBlock, _endBlock, _wkdCommit, _raisingAmount, _offeringAmount);
    }

     function deposit() public payable {
        if (isInitialized) revert NotInitialized();
        if (block.number < StartBlock) revert NotStarted();
        if (block.number > EndBlock) revert HasEnded();
    ...
    function claimToken() public {
        if (isInitialized) revert NotInitialized();
        if (block.number < StartBlock) revert NotStarted();
        if (block.number < EndBlock) revert StillActive();
