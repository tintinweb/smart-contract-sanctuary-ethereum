...
contract BDCContract is  ERC721A, ThirdwebContract {
    ...
    modifier onlyOwner() {
        require(_canSetOwner(), "Ownable: caller is not owner");
        _;
    }
    modifier canMintBDCs(uint256 numberOfTokens) {
        require(
            _totalMinted() + numberOfTokens <=
                maxDogs - whitelistMaxSupply,
            "Not enough BDC remaining to mint"
        );
        _;
    }
    ...
    constructor(string memory customBaseURI_,
                 uint256 _maxDogs,
                 uint256 _whitelistMaxSupply) ERC721A("Bad Dogs Company", "BDC") {
    ...
    function mint(address to, uint256 quantity, bytes32[] calldata merkleProof) 
        external 
        payable
