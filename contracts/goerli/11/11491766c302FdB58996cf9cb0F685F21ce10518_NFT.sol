...
contract NFT is ReentrancyGuard, ERC721, ERC721Enumerable, ERC2981, ERC721URIStorage, Ownable, INFT, Multicall {
	...
	modifier onlyOwnerorManagers() {
		require(managers[msg.sender] || owner() == msg.sender, 'manager-access-denied');
		_;
	}
	...
	///@dev anyone can mint
	function mint(string calldata _tokenURI, address _to) public override nonReentrant returns (uint256) {
	...
	///@dev ERC2981 standard function to set artist royalty and only token owner can call
	function setArtistRoyalty(
		uint256 _tokenId,
		address _receiver,
		uint96 _feeNumerator
	) public override onlyManagers {
	...
	function addManagers(address _newManager) public override onlyOwnerorManagers {
