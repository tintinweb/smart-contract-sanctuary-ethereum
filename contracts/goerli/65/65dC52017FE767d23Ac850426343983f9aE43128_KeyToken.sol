...
//Errors
error KeyToken__MaximumSupplyMinted();
...
contract KeyToken is ERC20, ERC20Burnable, Pausable, AccessControl {
    ...
    uint256 private immutable _maxSupply = 1111111 * 10**decimals();

    // access control roles
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    /** @dev Initiallise the contract by
     *	granting access control only to the
     *  contract deployer
     */
    ...
    function mint(address to, uint256 amount) public onlyRole(MINTER_ROLE) {
        // Reverts if maxSupply of token has been minted
        if (totalSupply().add(amount) > _maxSupply) {
            revert KeyToken__MaximumSupplyMinted();
        }
        _mint(to, amount);
    }
