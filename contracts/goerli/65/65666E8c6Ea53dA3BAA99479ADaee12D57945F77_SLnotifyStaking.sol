...
contract SLnotifyStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    // value of now.
    // This function is used to stake an ERC721 token.
    // Stake tokens
    function stake(uint256 _tokenId) external nonReentrant {
    // Get the staker from the mapping of stakers
    Staker storage staker = stakers[msg.sender];
    // Require that the sender is the owner of the NFT
    require(nftCollection.ownerOf(_tokenId) == msg.sender, "You don't own this token!");
    // Require that the token is not locked or the lock time has passed
    require(staker.lockTime == 0 || block.timestamp >= staker.lockTime, "Token is still locked");
    // If the staker already has staked tokens, calculate and add any unclaimed rewards
    ...
    // Add the staked token to the staker's list of staked tokens
    staker.stakedTokens.push(StakedToken(msg.sender, _tokenId));
    // Increment the amount of tokens staked
    staker.amountStaked++;
    // Increment the total amount of tokens staked
    totalStaked++;
    // Set the staker's address for this token
    stakerAddress[_tokenId] = msg.sender;
    // Set the lock time for this token
    staker.lockTime = block.timestamp + LOCK_TIME; // set lock time
    // Set the time of the last update for this staker
    staker.timeOfLastUpdate = block.timestamp;
}


function withdraw(uint256 _tokenId) external nonReentrant {
    // Get the staker's data from the stakers mapping
    Staker storage staker = stakers[msg.sender];

    // Make sure the staker has at least one token staked
    require(staker.amountStaked > 0, "You have no tokens staked");

    // Make sure the staker owns the token they are trying to withdraw
    require(stakerAddress[_tokenId] == msg.sender, "You don't own this token!");

    // Calculate the staker's rewards before they withdraw
    uint256 rewards = calculateRewards(msg.sender);
    staker.unclaimedRewards += rewards;

    // Find the index of the token in the staker's list of staked tokens
    uint256 index;
    StakedToken[] storage tokens = staker.stakedTokens;
    for (uint256 i = 0; i < tokens.length; i++) {
        if (tokens[i].tokenId == _tokenId && tokens[i].staker != address(0)) {
            index = i;
            break;
        }
    }

    // Remove the staked token from the staker's list of staked tokens
    if (index < tokens.length - 1) {
        tokens[index] = tokens[tokens.length - 1];
    }
    tokens.pop();

    // Set the staker's address for this token to address 0 to mark it as no longer staked
    stakerAddress[_tokenId] = address(0);

    // Decrement the amount of tokens staked by this staker
    staker.amountStaked--;

    // Transfer the token back to the staker's wallet
    nftCollection.transferFrom(address(this), msg.sender, _tokenId);

    // Update the time of last update for the staker
    staker.timeOfLastUpdate = block.timestamp;
}
...
function claimRewards() public nonReentrant whenNotPaused {
    ...
    if (burnAmount > 0) {
        ...
        totalRewards -= burnAmount; // reduce total rewards by the burn tax amount
    }
    ...
    if (stakingVaultAmount > 0) {
        ...
        totalRewards -= stakingVaultAmount; // reduce total rewards by the vault tax amount
    }
