...
contract PioneerPass is PioneerPassToken, ReentrancyGuard {
    string public name = "Pioneer Pass v0.0.3";
    string public symbol = "Pioneer v0.0.3";

    constructor(string memory baseURI, string memory _contractURI, address royaltyAddress) PioneerPassToken(baseURI, _contractURI, royaltyAddress){}

    function mint(bytes32[] calldata _merkleProof, uint256 _passId, uint16 _amount, bool partner) external payable nonReentrant callerIsUser {
        ...
        if (partner) {
            require(!PioneerPassStorage.partnersMinted[msg.sender][_passId], "Already minted");
            require(Library.ValidatePresaleMint(pass, _amount, true, _merkleProof));
            PioneerPassStorage.partnersMinted[msg.sender][_passId] = true;
        } else if (pass.preSale) {
            require(!PioneerPassStorage.presaleMinted[msg.sender][_passId], "Already minted");
            for (uint bar = 0; bar <= passIdIndex.length; bar++) {
                require(balanceOf(msg.sender, passIdIndex[bar]) >= 1, "Does not own previous tokens");
            }
            ...
            uint16 _hodls;
            if (balanceOf(msg.sender, passIdIndex[0]) >= 1 && _passId != 1) {
                for (uint bar = 0; bar <= passIdIndex.length; bar++) {
                    if (balanceOf(msg.sender, passIdIndex[bar]) >= 1) {
                        _hodls = _hodls + 1;
                    }
                }
            }
...
contract PioneerPassToken is ERC1155, Ownable, PioneerPassStorage, ContractURI, Stacking, PioneerPassUtils {
    ...
    constructor(string memory _baseURI, string memory _contractURI, address _royaltyAddress){
    ...
    */
    ...
    function _beforeTokenTransfer(
        ...
        for (uint bar = 0; bar < ids.length; bar++) {
            require(!transfersLock[ids[bar]], "transfers paused");
        }
...
contract PioneerPassStorage is Ownable {
    ...
    mapping(address => mapping(uint256 => bool)) internal partnersMinted;
    ...
    */
    ...
    function getPresaleMinted(uint256 _passId, address _account) external view returns (bool) {
        return presaleMinted[_account][_passId];
    }

    function getPartnersMinted(uint256 _passId, address _account) external view returns (bool) {
        return partnersMinted[_account][_passId];
    }
...
abstract contract ERC1155 is Context, ERC165, ERC1155URIStorage, Pausable {
