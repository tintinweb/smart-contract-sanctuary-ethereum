...
import "./VikuUser1155Token.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract Factory1155 is Ownable {

    error MintingFeeInvalid(uint256 mintingFee, uint256 valueSent);
    ...
    function deploy(
        ...
        //require(msg.value == _mintingFee, "Error: minting fee");
        if(!(msg.value == _mintingFee)) revert MintingFeeInvalid(_mintingFee, msg.value);
        addr = address(
            new VikuUser1155Token{salt: _salt}(name, symbol, tokenURIPrefix)
        );

        payable(treasury).transfer(msg.value);
        VikuUser1155Token token = VikuUser1155Token(address(addr));
        token.transferOwnership(msg.sender);
        emit Deployed(msg.sender, addr);
    }

    function updateTreasury(address _newTreasury) external onlyOwner returns(address){
