...
contract Dochain is AccessControl, Pausable, ERC721URIStorage, KeeperCompatibleInterface {
    ...
    constructor(
        address adminlawyer,
        address lawyerVisitor,
        uint256 interval,
        string memory ipfsUri
    ) ERC721("Briefcase", "BC") {
        // Role assignment
        _grantRole(ROL_ADMIN, _msgSender());
        _grantRole(ROL_ADMIN, adminlawyer);
        _grantRole(ROL_VISITOR, lawyerVisitor);
        i_interval = interval;
        ...
        )
    {
        ...
        bool chageHash = _compareStrings(s_ipfsUri, s_newHash);
    ...
    function performUpkeep(
        ...
        if ((block.timestamp - s_lastTimeStamp) > i_interval) {
            ...
            s_ipfsUri = s_newHash;
            _setTokenURI(tokenId, s_ipfsUri);
            emit TriggerHash(tokenId, s_ipfsUri);
        }
    }
    ...
    function safeMint(address to) public whenNotPaused onlyRole(ROL_VISITOR) {
        ...
        _setTokenURI(tokenId, s_ipfsUri);
        emit MintSuccess(tokenId, s_ipfsUri);
    }
    ...
    function addRolevisitor(address newlawyerVisitor) public onlyRole(ROL_ADMIN) whenNotPaused {
        _grantRole(ROL_VISITOR, newlawyerVisitor);
        emit AddRole(newlawyerVisitor);
    }
