...
contract AISkinFactory is AccessControl, EIP712, IAISkinFactory {
    ...
    function createCollection(
        uint96 typeId,
        address royaltyReceiver, // breederdao as royalty receiver
        uint96 royaltyFee
    ) external payable {
    ...
    function publishItem(
        ...
        // @todo logic optimize ~ check if publisher -> get
        if (!_isValidSigner(tokenIdSigner)) {
            revert Unauthorized(PUBLISHER_SIGNER, tokenIdSigner);
        } else if (
            _isValidSigner(tokenIdSigner) && !_isFreePublish(tokenIdSigner)
        ) {
        ...
        if (collection == masterCollection) {
            ...
            if (IDropCollection(collection).creator() != _msgSender()) {
