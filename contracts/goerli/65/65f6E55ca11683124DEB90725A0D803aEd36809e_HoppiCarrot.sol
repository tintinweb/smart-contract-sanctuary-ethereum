...
abstract contract HoppiFactory {
    function feed(address to) external payable virtual returns (uint256);

    // function feed(address to, uint256[] memory tokenIds)
    //     public
    //     virtual
    ...
    ERC2981Upgradeable
{
    ...
    function initialize(
        ...
        mintInfo[0].maxSupplyAmounts = 1000000;
    ...
    */
    ...
    function feed(address to, uint256 typeId)
        external
        payable
        nonReentrant
        returns (uint256)
    {
        ...
        HoppiFactory factory = HoppiFactory(hoppiContract);

        _burn(to, typeId, 1);

        (bool success, ) = address(hoppiContract).call{
            value: 33000000000000000
        }("");

        uint256 blackdustTokenId = factory.feed(to);

        // emit Awakened(to, takrutId);
        return blackdustTokenId;
    }

    function burnCarrotForAddress(
        uint256 typeId,
        address burnerAddress,
        uint256 amount
    ) external {
        require(msg.sender == hoppiContract, "Invalid Hoppi Contract Address");
        _burn(burnerAddress, typeId, amount);
    }

    function burnManyCarrots(
