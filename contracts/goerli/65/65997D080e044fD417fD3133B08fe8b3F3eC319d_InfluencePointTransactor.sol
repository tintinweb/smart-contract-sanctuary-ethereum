 ...
 */
contract InfluencePointTransactor is AccessControlEnumerable, Signable, Pausable, ReentrancyGuard {
    ...
    function buyTokens(GenericERC20BuyLib.GenericERC20Buy calldata buy)
        public
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        if (buy.paymentToken == ETH_CONSTANT) {
            ...
            IERC20(buy.paymentToken).safeTransfer(
                multisig,
                IERC20(buy.paymentToken).balanceOf(address(this))
            );
        }
    ...
    function setMinimumAmount(uint256 newMinimunAmount)
        external
