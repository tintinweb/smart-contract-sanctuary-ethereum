...
contract IT02MintingModule is AccessControl, ERC721Holder {
    ...
    function preMint(uint256 mintCount) external onlyRole(DEFAULT_ADMIN_ROLE) {        
        for (uint256 i = 0; i < mintCount; i++) {
            uint256 id = passport.mintPassport(address(this));
            passport.burn(id);
        }
    }
    ...
    function mint(uint256[] memory a1TokenIds, Size[] calldata sizes) external payable returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < sizes.length; i++) {
            ...
            // mint token
            passportTokenIds[i] = passport.mintPassport(msg.sender);
