...
// File contracts/LimitOrdersStorage.sol
    ...
    LimitOrdersStorage
{
    ...
    function initialize(
        ...
        whiteListFactory = ITFactory(_whiteListFactory);
        ...
        createDelegator = _createDelegator;
        editDelegator = _editDelegator;
        executeDelegator = _executeDelegator;
        cancellDelegator = _cancellDelegator;
        repayerDelegator = _repayerDelegator;
        __Pausable_init();
        __AccessControl_init();
        __UUPSUpgradeable_init();
    }

    function setNewWhiteListFactory(address _whiteListFactory)
        external
    ...
    {
        positionRouter = IPositionRouter(_positionRouter);
        emit SetNewPositionRouter(_positionRouter);
    }
    ...
    function setCreateDelegator(address _createDelegator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        createDelegator = _createDelegator;
        emit SetNewCreateDelegator(_createDelegator);
    }

    function setEditDelegator(address _editDelegator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        editDelegator = _editDelegator;
        emit SetNewEditDelegator(_editDelegator);
    }

    function setExecuteDelegator(address _executeDelegator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        executeDelegator = _executeDelegator;
        emit SetNewExecuteDelegator(_executeDelegator);
    }

    function setCancelDelegator(address _cancellDelegator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        cancellDelegator = _cancellDelegator;
        emit SetNewCancelDelegator(_cancellDelegator);
    }

    function setRepayDelegator(address _repayerDelegator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        repayerDelegator = _repayerDelegator;
        emit SetNewRepayDelegator(_repayerDelegator);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        require(_newSlippageAccuracy % 10 == 0, "STSL2");
        SLIPPAGE = _newSlippage;
        SLIPPAGE_ACCURACY = _newSlippageAccuracy;
        emit SetNewSlippageValue(_newSlippage, _newSlippageAccuracy);
    }

    function setGAS(uint256 _newGAS, uint256 _newGASAccuracy)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_newGASAccuracy > _newGAS, "STG1");
        GAS_SLIPPAGE = _newGAS;
        GAS_SLIPPAGE_ACCURACY = _newGASAccuracy;
        emit SetNewGASValue(_newGAS, _newGASAccuracy);
    }
    ...
    function setPriceLimit(
        ...
        if (_path[0] == swapRouter.WETH9()) {
            pathTokenToWETH[_token] = PoolRequirement({
                path: "",
            ...
            require(
                _path.length >= 2 &&
                    _pathFee.length >= 1 &&
                    _path.length - 2 == _pathFee.length - 1,
                "Wrong path length"
            );
            require(_path[0] == _token, "Wrong first token");
            require(
                _path[_path.length - 1] == swapRouter.WETH9(),
        ...
        returns (uint256)
    {
        ...
        bytes memory returnData = delegateTransaction(createDelegator, call);
    ...
    {
        bytes memory call = abi.encodeWithSelector(
            this.editOrder.selector,
            _editParams
        );
        bytes memory returnData = delegateTransaction(editDelegator, call);
        (newAmountOfToken0, newAmountOfToken1) = abi.decode(
            returnData,
            (uint256, uint256)
        );
    }
    ...
    {
        bytes memory call = abi.encodeWithSelector(
            this.cancelOrder.selector,
            _poolAddress,
            _tokenId
        );
        delegateTransaction(cancellDelegator, call);
    }
    ...
    /// @param _tokenId tokenId of uniswapV3 nft
    /// @param _poolAddress address of uniswapV3 pool
    function executeOrder(address _poolAddress, uint256 _tokenId)
        public
        allowedOrder(_tokenId, _poolAddress)
    {
        bytes memory call = abi.encodeWithSelector(
        ...
        delegateTransaction(executeDelegator, call);
    }
