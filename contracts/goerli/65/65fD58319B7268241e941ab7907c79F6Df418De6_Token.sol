...
contract Token is Context, IERC20, Ownable {
    ...
    address USDT = 0xC5294F25b848D45274b6a39F0e58625D1db28d86;
    ...
    function totalSellFeePerTx(uint256 amount) public view returns (uint256) {
        uint256 fee = amount.mul(TaxFeeOnSelling.add(liquidityFeeOnSelling)).div(
            percentDivider
        );
        return fee;
    }
    ...
    function distributeAndLiquify(address from, address to) private {
        ...
        if (
            ...
            !(from == address(this) && dexPair[to] 
            && contractTokenBalance>0) // swap 1 time
        ) {
            // approve contract
            ...
            // IERC20(address(this)).approve(address(dexRouter),tokenAmountToBeSwapped);
            ...
            IERC20(USDT).approve(address(dexRouter),USDTToBeAddedToLiquidity);
            ...
            uint256 USDTForLp = IERC20(USDT).balanceOf(address(this));
...
library Utils {
    ...
    function swapTokensForTokens(address routerAddress, uint256 tokenAmount, address USDT)
        internal
    {
