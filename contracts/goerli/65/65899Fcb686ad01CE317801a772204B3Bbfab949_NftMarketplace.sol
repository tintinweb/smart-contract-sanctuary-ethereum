...
abstract contract TransferExecutor is Initializable, OwnableUpgradeable, ITransferExecutor {
    ...
    function transferEth(
        ...
        // ETH Fee
        uint256 fee = ((value - royalty) * (hasNft(gkContract, from) ? 
            gkFee :
            hasNft(nftProfile, from) ?
                profileFee :
                protocolFee
            )
        ) / 10000;
    ...
    function transfer(
        ...
        if (params.asset.assetType.assetClass == LibAsset.ETH_ASSET_CLASS) {
            transferEth(params.from, params.to, value, params.validRoyalty, params.optionalNftAssets);
