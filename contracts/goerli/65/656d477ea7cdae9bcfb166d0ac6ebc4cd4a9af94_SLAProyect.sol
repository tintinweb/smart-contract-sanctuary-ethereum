    ...
    function activatedByCron() public returns (string memory){
        ...
        for (uint i = 0; circuitsArray.length > i; i++) {  //for by every circuit
            ...
            if (lastEmailSendMap[circuitIdentifier].length > 0){
                ...
                monthStart = IDatetime(datetimeAddress).toTimestamp(last_year, last_month, 0);
                lastEmailSendMap[circuitIdentifier].push(lastSendedEmail({date:monthStart, hasChanged:false}));
