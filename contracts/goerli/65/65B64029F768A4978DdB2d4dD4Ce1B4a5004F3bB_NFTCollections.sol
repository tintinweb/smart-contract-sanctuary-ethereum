...
contract NFTCollections is ERC721Upgradeable, OwnableUpgradeable {
     ...
     * 
     */
    function internalMintFromCollection(uint256 collectionId, uint256 tokenIdInCollection, address mintTo, bool fromAdmin, bool checkPrice, uint256 proxyType) internal {
        // revert if collection id is outside bounds
        require(collectionId < collections.length, "Bad collection ID");
        
        if (checkPrice) {
            uint256 expectedPrice;
            if (vipPassContract.balanceOf(mintTo) > 0) {
                expectedPrice = collections[collectionId].discountPrice;
            } else {
                expectedPrice = collections[collectionId].price;
            }
            // revert if amount of ETH sent is not correct
     ...
     */
    function mintFromCollection(uint256 collectionId, uint256 tokenIdInCollection) external payable {
        internalMintFromCollection(collectionId, tokenIdInCollection, msg.sender, false, true, 0);
    }

    /**
     * @dev Mint multiple tokens from collections
     */
    function mintFromCollectionMultiple(uint256[] calldata collectionIds, uint256[] calldata tokenIdsInCollection, uint256[] calldata proxyTypes) external payable {
        ...
        for (uint256 i=0; i < collectionIds.length;) {
            internalMintFromCollection(collectionIds[i], tokenIdsInCollection[i], msg.sender, false, false, proxyTypes[i]);
            unchecked{i++;}
        }
    }
    ...
    function runAirdrop(uint256[] calldata collectionIds, uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < collectionIds.length;) {
            ...
            internalMintFromCollection(collectionIds[i], tokenIds[i], wallet, true, false, 0);
            unchecked{i++;}
        }
    }
    ...
    function runAirdropFreeForAll(uint256[] calldata collectionIds, uint256[] calldata tokenIds, address[] calldata owners) external {
        ...
        for (uint256 i = 0; i < tokenIds.length;) {
            ...
            internalMintFromCollection(collectionIds[i], tokenIds[i], owners[i], true, false, 0);
            unchecked{i++;}
        }
    }
