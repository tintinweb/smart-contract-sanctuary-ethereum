...
import {LendingRateOracle} from "../oracle/LendingRateOracle.sol";
...
contract StableAndVariableTokensHelper is Ownable {
    ...
    function initDeployment(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            emit deployedContracts(
                address(new StableDebtToken()),
                address(new VariableDebtToken())
            );
        }
    }
    ...
    function setOracleOwnership(address oracle, address admin)
        external
        onlyOwner
    {
...
import {IPhiatIncentivesController} from "../../interfaces/IPhiatIncentivesController.sol";
...
contract StableDebtToken is IStableDebtToken, DebtTokenBase {
        ...
        onlyLendingPool
    {
        ...
        if (previousSupply <= amount) {
            ...
            // exclude the rare case where first ever borrowing is stable and get 0 rate
            // if no one borrows in between, repay may trigger _totalSupply = 0 unintentionally
            if (_avgStableRate > 0 || userStableRate > 0) {
    ...
    {
        return
            IPhiatIncentivesController(
                _pool.getAddressesProvider().getIncentivesController()
            );
    }
    ...
    function _mint(
        ...
        IPhiatIncentivesController incentivesController = _getIncentivesController();
