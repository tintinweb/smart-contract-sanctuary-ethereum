...
contract NFT is ERC721, PullPayment, Ownable {
  ...
  function whitelist_mintTo(address recipient, uint256 quantity, bytes32[] calldata merkleProof) public payable returns (uint256) {
    bytes32 node = keccak256(abi.encodePacked(msg.sender, quantity));
    require(MerkleProof.verify(merkleProof, merkleRoot, node) == true, "invalid merkle proof");
