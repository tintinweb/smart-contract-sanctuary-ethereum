...
contract Trunk1 is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply, ReentrancyGuard {
  ...
  function airdrop(
    ...
    if (receivers.length != numTokens.length || receivers.length == 0) revert MismatchedArrays();
    if (processedChunksForAirdrop[chunkNum] || balanceOf(receivers[0], TRUNK_1) > 0) revert ChunkAlreadyProcessed();
