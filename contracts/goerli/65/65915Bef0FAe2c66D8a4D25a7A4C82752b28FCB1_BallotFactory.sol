...
contract BallotFactory is AccessControl {
    ...
    function vote(uint256 ballotID, uint8 option) public {
        address ballotAddress = ballots[ballotID];

        Ballot(ballotAddress).vote(option, msg.sender);
    ...
    // This function closes and defines winner option of a ballot
...
contract Ballot is AccessControl {
    ...
    function vote(uint8 option, address from) public isBallotValid(true) {
        uint256 amount = IERC20(token).balanceOf(from);

        require(amount > 0, "You must vote for at least 1 token");

        require(_voted[from] == false, "You have already voted");
        _voted[from] = true;
    ...
    function closeBallot() external onlyRole(TEAM_MEMBER) isBallotValid(true) {
        isClosed = true;

        _defineWinner();

        for (uint256 i = 0; i < voters.length; i++) {
            Token(token).removeTokensUsedBallot(
                voters[i],
                tokensUsed[voters[i]]
            );
        }

        emit BallotClosed(isClosed);
    }
    ...
    function _defineWinner() internal {
        optionWinner = _getWinner();
        emit WinnerDefined(optionWinner);
    }

    function _getWinner() internal view returns (uint8) {
        uint8 winner = 0;
        uint256 maxVotes = 0;
    ...
    modifier isBallotValid(bool _isSealed) {
        require(isSealed == _isSealed, "Invalid: Function not valid now");

        if (isSealed) {
            uint256 dateNow = block.timestamp;
            require(dateNow > dateToEnd, "Invalid: Ballot did not end yet");
        }
...
contract Token is ERC20, AccessControl {
    ...
    // Burn tokens from the caller of function.
    function burnMyTokens(uint256 amount) public onlyRole(TEAM_MEMBER) {
    ...
    function _beforeTokenTransfer(
        ...
        uint256 tokensLeftLocked = balance - tokensLocked;
        ...
        if (
            ...
            require(tokensLeftLocked > 0, "Token: No tokens left");
    ...
    uint256 public constant TIME_TO_GET_REWARDS = 2 days;
    ...
    function getReward() public {
        ...
        require(tokensLocked > 0, "Token: No tokens locked");

        uint256 timePassed = block.timestamp - time[msg.sender];
        time[msg.sender] = block.timestamp;

        require(timePassed >= TIME_TO_GET_REWARDS, "Not enough time");

        uint256 percentage = (tokensLocked * 100) / totalLockedTokens;
        uint256 rewardAmount = (percentage * reward) / 100;
