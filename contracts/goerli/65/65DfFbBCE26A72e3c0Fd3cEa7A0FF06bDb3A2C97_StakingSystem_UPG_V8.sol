...
contract StakingSystem_UPG_V8 is Initializable, OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    struct Staker {
        ...
        mapping(uint256 => uint256) tokenStakingStart;
        uint256 balance;
        uint256 rewardsReleased;
    }


    function initialize(IERC721 _nft, IRewardToken _rewardsToken) public initializer  {
        stakingTime = 300 seconds;
		token = 100e18;
        nft = _nft;
    ...
    function stakeFromContract(uint256 tokenId, address _user) public {
        _stake(_user, tokenId);
    }
    ...
    function _stake(address _user, uint256 _tokenId) internal {
        ...
        staker.tokenStakingStart[_tokenId] = block.timestamp;
    ...
    function unstake(uint256 _tokenId) public {
        ...
        if(block.timestamp > uint(staker.tokenStakingCoolDown[_tokenId])){
            stakedDays = ((block.timestamp - uint(staker.tokenStakingCoolDown[_tokenId]))) / stakingTime;
            partialTime = ((block.timestamp - uint(staker.tokenStakingCoolDown[_tokenId]))) % stakingTime;
        } else {
            stakedDays = 0;
            partialTime = 0;
        }
    ...
    function _unstake(address _user, uint256 _tokenId) internal {
        ...
        staker.tokenIds[indexToken] = staker.tokenIds[staker.tokenIds.length - 1];
        ...
        staker.tokenStakingStart[_tokenId] = 0;
    ...
    function claimReward() public {
        ...
        IERC20(rewardsToken).transferFrom(address(this), msg.sender, stakers[msg.sender].balance);
        emit RewardPaid(msg.sender, stakers[msg.sender].balance);

        stakers[msg.sender].rewardsReleased += stakers[msg.sender].balance;
    ...
    function updateReward(address _user) public {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (
                ...
                if(block.timestamp > uint(staker.tokenStakingCoolDown[ids[i]])){
                    stakedDays = ((block.timestamp - uint(staker.tokenStakingCoolDown[ids[i]]))) / stakingTime;
                    partialTime = ((block.timestamp - uint(staker.tokenStakingCoolDown[ids[i]]))) % stakingTime;
                } else {
                    stakedDays = 0;
                    partialTime = 0;
                }
    ...
    function getStakerInfo(address _user) external view returns (
        ...
        for (uint256 i = 0; i < ids.length; i++) {  
            ...
            if(block.timestamp > uint(staker.tokenStakingCoolDown[ids[i]])){
                stakedDays = ((block.timestamp - uint256(staker.tokenStakingCoolDown[ids[i]]))) / stakingTime;
                partialTime = ((block.timestamp - uint256(staker.tokenStakingCoolDown[ids[i]]))) % stakingTime;
            } else {
                stakedDays = 0;
                stakedDays = 0;
            }
    
            newBalance +=  (token * stakedDays);   

        }

        newBalance += stakers[_user].balance;

        total = newBalance + stakers[_user].rewardsReleased; 
    ...
    function getstakingCoolDown(address _user, uint256 _tokenId) external view returns (uint256){
        Staker storage staker = stakers[_user];
        return staker.tokenStakingCoolDown[_tokenId];
    }

    function getstakingStart(address _user, uint256 _tokenId) external view returns (uint256){
        Staker storage staker = stakers[_user];
        return staker.tokenStakingStart[_tokenId];
    }
    ...
    function deposit(uint _amount) public {
        IERC20(rewardsToken).approve(address(this), _amount);
    ...
    function withdrawDT(uint256 _amount) external onlyOwner {
        IERC20(rewardsToken).approve(address(this), _amount);
        IERC20(rewardsToken).transferFrom(address(this), owner(), _amount);
    }
