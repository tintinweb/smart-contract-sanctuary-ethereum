...
/// @author Huy Tran
contract AGC is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
    ...
    function performRebasement(uint256 dividend, uint256 divisor, uint256 fromIndex, uint256 toIndex) external onlyOwner whenNotPaused {
        require(dividend > 0 && divisor > 0 && dividend != divisor, "rebasement: dividend and divisor must not be zero and is equal");
    ...
    function mint(address _userAddress, uint256 _amount) public onlyOwner whenNotPaused updateUserAddress(_userAddress) {
    ...
    function transfer(address _toUser, uint256 _amount) public override onlyOwner whenNotPaused updateUserAddress(_toUser) returns (bool) {
        require(_toUser != address(0) && _toUser != companyAddress, "transfer: user address is zero or equal company address");
        require(_amount > 0 && _amount <= balanceOf(companyAddress), "transfer: insufficient company balance");
    ...
    /// @dev Transfers AGC from a user to another. 
    function transferFrom(address from, address to, uint256 amount) public onlyOwner whenNotPaused override returns (bool) {
    ...
    function burnFrom(address _userAddress, uint256 _amount) public override onlyOwner whenNotPaused {
        require(_userAddress != address(0) && _userAddress != companyAddress, "burnFrom: user address is zero or equal company address");
        require(_amount > 0 && _amount <= balanceOf(_userAddress), "burnFrom: cannot burn zero token");
    ...
    function burn(uint256 _amount) public override onlyOwner whenNotPaused {
        require(_amount > 0, "burn: cannot burn zero amount");
        require(_amount <= balanceOf(companyAddress), "burn: insufficient company balance");
