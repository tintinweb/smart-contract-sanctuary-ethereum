...
contract QMMarketplace {
    ...
    function buy(address _contract, uint256 _tokenId) external payable {
        uint256 price = HDGenerator(_contract).getTokenPriceInWei(_tokenId);
        require(price > 0, 'This token is not for sale');
        uint256 priceInWei = HDGenerator(_contract).getTokenPriceInWei(_tokenId);
        require(msg.value >= priceInWei, 'Incorrect value');
        
        address seller = HDGenerator(_contract).ownerOf(_tokenId);
        HDGenerator(_contract).transferFrom(seller, msg.sender, _tokenId);
    ...
    Stateable
{
    ...
    mapping (uint256 => uint96) public tokenIdToPrice;
    ...
    /**
     * Puts on sale a token with @param _tokenId and price in USD @param _price.
     */
    function allowBuy(uint256 _tokenId, uint96 _priceInUsd) external {
        require(msg.sender == ownerOf(_tokenId), 'Not owner of this token');
        require(_priceInUsd > 0, 'Price zero');
        tokenIdToPrice[_tokenId] = _priceInUsd;
    ...
    function buy(uint256 _tokenId) external payable {
        ...
        require(price > 0, 'This token is not for sale');
        uint256 priceInWei = getTokenPriceInWei(_tokenId);
        require(msg.value >= priceInWei, 'Incorrect value');
    ...
    /*
     * To get the token price in wei.
     */
    function getTokenPriceInWei(uint256 _tokenId) public view returns (uint256) {
        uint96 priceInUsd = tokenIdToPrice[_tokenId];
