...
contract PbxToken is ERC20, ERC20Capped, ERC20Snapshot, AccessControl, Pausable {
    ...
    error AccountBlacklistedError(address caller, string errorMessage);

    bytes32 public constant SNAPSHOT_ROLE = keccak256("SNAPSHOT_ROLE");
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant BLACKLISTER_ROLE = keccak256("BLACKLISTER_ROLE");
    uint256 private constant CAP = 1000000000 * 10**18;
    string private constant NAME = "Probinex Token Test";
    string private constant SYMBOL = "tPBX";

    mapping(address => bool) private blacklisted;

    modifier notBlacklisted(address _account) {
        if (blacklisted[_account]) {
            revert AccountBlacklistedError(_account, "Blacklistable: account is blacklisted");
        }
        _;
    }
