...
contract FarmingRewardsFactory is Ownable {
    ...
    uint256 public minEpochDuration = 604800;
    ...
    event FarmingRewardsCreated(address farmingRewards);
    ...
    function deploy(
        ...
        emit FarmingRewardsCreated(address(farmingProxy));
    }
}
...
contract FarmingRewards is ReentrancyGuard, IFarmingRewards {
    ...
    uint256 public dateOfFirstFarmInvestment;
    ...
    modifier earlyStakingExit(address account) {
        uint unstakeAvailableDate = accountStartFarm[account] + 7 days;
        require(block.timestamp > unstakeAvailableDate, 'Unavailable unstake earlier then 7 date from stake');
        _;
    }
    ...
    function farm(uint256 amount) earlyStakingExit(msg.sender) nonReentrant updateReward(msg.sender) external {
