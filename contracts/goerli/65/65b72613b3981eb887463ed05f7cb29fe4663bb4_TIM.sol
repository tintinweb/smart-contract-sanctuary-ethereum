...
contract TIM is ERC1155, ReentrancyGuard, Ownable {
    ...
    function mintTo(bytes memory _userTextLines, uint256 _quantity, address[] memory _to, bool asAirdrop) public payable {
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            MessagesToTokenId[totalSupply + 1] = newMessage; //Add Message to mapping @tokenId
            if (asAirdrop) { 
    ...
    function mintToTest() public {
        ...
        bytes memory testBytes = stringToBytes(testString);
        address[] memory testAddress = new address[](1);
        testAddress[0] = msg.sender;
        mintTo(testBytes, 1, testAddress, false);
    }
    ...
    function getTextBlock0(bytes memory _bytesMessage) private pure returns (string memory) {
        ...
        return abi.decode(_bytesMessage, (string));
    }
        ...
        return
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        header,
                        getSVGTextLine(0, "hellllllo"),
                        getSVGTextLine(20, "helllllllllllllllo"),
                        getSVGTextLine(40, "helllllllllllllllllllllllo"),
