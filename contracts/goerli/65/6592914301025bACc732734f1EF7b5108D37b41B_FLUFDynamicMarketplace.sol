...
contract FLUFDynamicMarketplace is ERC1155Holder, ReentrancyGuard, Ownable {
  ...
  struct Sales {
    ...
    uint[] ids; // The tokenIds
    uint[] amounts; // The quantity of each token to transfer
    uint price;
  }
  
  mapping(uint256 => Sales) public availableSales;
  uint public saleNonce = 0; // The nonce of the current sale, starts with 0;
  mapping(uint => mapping(bytes => bool)) public usedToken; // Whether the SALT Token has been consumed
  mapping(uint => mapping(address => bool)) public addressMinted; // Whether this address has already minted during this sale
  ...
  function updateSaleNonce(uint _nonce) public onlyOwner {
    saleNonce = _nonce;
  }
  ...
  function grantTokens(address _to) internal {
    uint[] memory ids = availableSales[saleNonce].ids;
    uint[] memory amounts = availableSales[saleNonce].amounts;
  ...
  function mint(
    ...
    uint price = availableSales[saleNonce].price;
    require(
      msg.value >= price,
    ...
    require(!addressMinted[saleNonce][msg.sender], "This address has already minted during private sale.");
    }
  ...
  function emergencyWithdrawTokens(address _to, uint[] memory ids, uint[] memory amounts, address contractAddress) public onlyOwner {
