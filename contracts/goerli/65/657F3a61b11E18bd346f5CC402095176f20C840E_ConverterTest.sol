...
contract ConverterTest is Converter {
    ...
    uint256 public defaultEnergy = 1000 * 10**36;
    ...
    constructor(
        ...
        Period[] memory _periods,
        uint256 time
    ) Converter(owner, lba, _periods, time) {}
    ...
    function getEnergy(address addr, uint256 periodId) public view virtual override returns (uint256) {
        return
            defaultEnergy +
            calculateEnergy(addr, periodId) -
            getConsumedEnergy(addr) +
            getRemainingLBAEnergy(addr, periodId);
    }

    function setDefaultEnergy(uint256 energy) external {
