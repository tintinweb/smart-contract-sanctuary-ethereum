...
contract FairLaunch is Ownable, ReentrancyGuard {
    ...
    uint256 lockedToken;     // total token locked
    ...
    mapping(address => UserBuy) private _buyed;   //user -> poolid -> UserBuyed
    ...
    event TokensPurchased(
        ...
        uint256 value,
        uint256 raised,  //buyed
        uint256 price  //etimate
    ...
    function getPool() public view returns (string memory, uint256, uint256, bool, uint256, uint256, uint256, uint256) {
        return (name, lockedToken, raised, reached, minVal, maxVal, openingTime, closingTime);
    }
    ...
    function buyToken(address refaddr, uint ln) public payable {
        ...
        emit TokensPurchased(beneficiary, weiAmount, raised,  sellToken.balanceOf(address(this)).div(raised));
    }

    function getBuyInfo(address account) public view returns (UserBuy memory) {
    ...
    function fairLaunchCalculate() external onlyOwner{
        uint256 totaltoken = sellToken.balanceOf(address(this));
        //
        price = totaltoken.div(raised);
        for (uint i = 0; i < _lookup.length; i++) {
            _buyed[_lookup[i]].locked = _buyed[_lookup[i]].raised.mul(price);
        }
    }
