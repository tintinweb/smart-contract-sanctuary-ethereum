...
import "cojodi/contracts/sell/LinearDutchAuction.sol";

import "./TheSquidsBase.sol";

contract TheSquids is TheSquidsBase, MerkleWhitelist, LinearDutchAuction {
  ...
  constructor(address dev_, address projectOwner_)
    TheSquidsBase()
    LinearDutchAuction(1.2 ether, 0.1 ether, 0.1 ether, 60 * 60)
    MerkleWhitelist("TODO ROOT")
  {
    dev = dev_;
    projectOwner = projectOwner_;
  }

  function mintWhitelist(bytes32[] calldata merkleProof_) external payable isWhitelisted(merkleProof_) isPaymentOk(1) {
    _safeMint(msg.sender);
  }

  function mintPublic() external payable isPublic isPaymentOk(1) {
    _safeMint(msg.sender);
  }
  ...
  function withdraw() external {
    ...
    payable(dev).transfer((balance * 6) / 100);
...
contract LinearDutchAuction is DutchAuction {
  using SafeMath for uint256;

  uint256 public deductionPrice;

  constructor(
    uint256 startPrice_,
    uint256 endPrice_,
    uint256 deductionPrice_,
    uint256 updateEvery_ // in seconds
  ) DutchAuction(startPrice_, endPrice_, updateEvery_) {
    deductionPrice = deductionPrice_;
  }

  function _currentPrice() internal view virtual override returns (uint256) {
    uint256 timePassed = block.timestamp - timer;
    uint256 deduction = timePassed.div(updateEvery).mul(deductionPrice);

    if (deduction.add(endPrice) >= startPrice) return endPrice;

    return startPrice.sub(deduction);
  }

  function setDeductionPrice(uint256 deductionPrice_) external onlyOwner {
    deductionPrice = deductionPrice_;
  }
}
...
abstract contract DutchAuction is Ownable {
  ...
  constructor(
    ...
    endPrice = endPrice_;
    updateEvery = updateEvery_;
  }

  function _currentPrice() internal view virtual returns (uint256);

  modifier isPaymentOk(uint256 amount_) {
    require(auctionEnabled, "auction not enabled");
    require(msg.value == _currentPrice().mul(amount_), "wrong amount paid");
    _;
  }

  function toggleAuction() external onlyOwner {
    if (!auctionEnabled) {
      _setTimer();
    }
  ...
  function setStartPrice(uint256 startPrice_) external onlyOwner {
    startPrice = startPrice_;
  }

  function setEndPrice(uint256 endPrice_) external onlyOwner {
    endPrice = endPrice_;
  }

  function setUpdateEvery(uint256 updateEvery_) external onlyOwner {
    updateEvery = updateEvery_;
  }
}
