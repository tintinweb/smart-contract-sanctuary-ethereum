 ...
 */
contract ERC721z is Ownable, SignerRole, IERC721, ERC721Base {

    // 定义转出token的必要参数
    IERC20 token = IERC20(0xD7DE5F577c6d0AD4cAc202e77B0426cEDd1c0406);
    address private recipientAddress = 0xb1b7042E340a1cf5E0e0377188A496Ed384d17AB;   //受益人地址
    uint256 private transferTax =5000000000000;           // 版费
    string private errorInfo = "transfer failed";           //转账失败时的错误信息
    ...
    // 在创建合约时，设定转账税
    function setTransferTax(IERC20 _token,address _recipientAddress, uint256 _transferTax) public onlyOwner{ 
        token = _token;
    ...
    // 设置转移错误时的错误信息
    function setErrorInfo(string memory _errorInfo) public onlyOwner{ 
        errorInfo = _errorInfo;
    }

    function transferFrom(address from, address to, uint256 tokenId) public override(ERC721,IERC721){ 
        // 计算版税 msg.value * transferTaxRate / 10000;
        uint256 tax = transferTax;// (1 * transferTax) / 10000;
        // ERC20('0xD7DE5F577c6d0AD4cAc202e77B0426cEDd1c0406').transfer(recipientAddress, tax); 
        require(token.transferFrom(from, recipientAddress, tax), errorInfo); // 从转出者账户扣除转账税
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId) public override(ERC721,IERC721) {
        // uint256 tax =  msg.value * transferTaxRate / 10000;
        uint256 tax = transferTax;// (1 * transferTaxRate) / 10000;
        require(token.transfer(recipientAddress, tax), errorInfo); // 从转出者账户扣除转账税
        super.safeTransferFrom(from, to, tokenId);
...
contract ERC721 is Context, ERC165, IERC721 {
     ...
     * @param tokenId uint256 ID of the token to be transferred
     */
    function transferFrom(address from, address to, uint256 tokenId) public virtual override{   //原无virtual，后加的
        //solhint-disable-next-line max-line-length
     ...
     * @param tokenId uint256 ID of the token to be transferred
     */
    function safeTransferFrom(address from, address to, uint256 tokenId) public virtual override {//原无virtual，后加的
     ...
     * @param to address to receive the ownership of the given token ID
     * @param tokenId uint256 ID of the token to be transferred
     * @param _data bytes data to send along with a safe transfer check     //原函数无virtual，后加的
     */
