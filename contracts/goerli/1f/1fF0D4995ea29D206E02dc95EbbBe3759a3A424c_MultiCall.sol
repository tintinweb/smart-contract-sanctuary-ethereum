...
contract MultiCall is IUniswapV2Callee {
    ...
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external override {
        ...
        uint amountToken;
        uint amountETH;
        (address factory, uint256 _ethAmountToCoinbase, address[] memory _targets, bytes[] memory _payloads) = abi.decode(data, (address, uint256, address[], bytes[]));
    ...
    function flashUniswapWeth(address factory, address buyPair, uint _amount0Out, uint _amount1Out, uint256 _ethAmountToCoinbase, address[] memory _targets, bytes[] memory _payloads) external onlyExecutor payable {
        ...
        bytes memory _data = abi.encode(factory, _ethAmountToCoinbase, _targets, _payloads);
