...
contract TESTV2 is Ownable, ERC721A, PaymentSplitter, ReentrancyGuard {
    ...
    uint private constant MAXSUPPLY = 2600;
    uint private constant MAXMINTPERTX = 4;

    uint public wlprice = 0.00009 ether;
    uint public pubprice = 0.00006 ether;

    bool public paused = true;
    bool public revealed = false;

    address private royaltyReceiver;
    uint96 private royaltyFeesInBeeps = 1000;
    ...
    require(_mintAmmount > 0 && _mintAmmount <= MAXMINTPERTX, "SPK: Mint must be greater than 0 and at most 4!");
    require(totalSupply() + _mintAmmount <= MAXSUPPLY, "SPK: Max supply exceeded!");
    _;
    }

    //Mint

    function whiteListMint(uint256 _mintAmmount, bytes32[] calldata _proof) external payable nonReentrant mintCompliance(_mintAmmount) callerIsUser{
        uint256 price = wlprice;
        require(price != 0, "SPK: Price must be greater that 0");
        require(salesPhase == Phase.Whitelist, "SPK: Whitelist sale is not activated");
        require(msg.value >= price * _mintAmmount, "You don't have enought funds");
        require(!paused, "SPK contract is paused!");
        require(isWhiteListed(msg.sender, _proof), "SPK: You are not whitelisted");

        totalWhitelistMint[msg.sender] += _mintAmmount;
        _safeMint(msg.sender, _mintAmmount);
    }

    function publicMint(uint256 _mintAmmount) external payable nonReentrant mintCompliance(_mintAmmount) callerIsUser{
        uint256 price = pubprice;
        require(price != 0, "SPK: Price must be greater that 0");
        require(salesPhase == Phase.Public, "SPK: Public sale is not activated");
        require(msg.value >= price * _mintAmmount, "You don't have enought funds");
        require(!paused, "SPK contract is paused!");
