...
import { LidoV1_Calls, LidoV1_Multicaller } from "../../multicall/adapters/lido/LidoV1_Calls.sol";
...
contract LidoSwapper is ISwapper, GasEstimator {
    ...
    PathFinderComponent public override getComponentId =
        PathFinderComponent.LIDO_SWAPPER;
    ...
    {
        bool found = swapTask.tokenIn == ILidoV1Adapter(adapter).weth() &&
            swapTask.tokenOut == ILidoV1Adapter(adapter).stETH();

        if (found) {
            quote = SwapQuote({
                found: true,
                amount: swapTask.amount,
                multiCall: getMulticall(swapTask, adapter),
    ...
    function getMulticall(SwapTask memory swapTask, address adapter)
        internal
        pure
        returns (MultiCall memory)
    {
        if (
            ...
            swapTask.swapOperation == SwapOperation.EXACT_OUTPUT
        ) {
            return LidoV1_Multicaller(adapter).submit(swapTask.amount);
        } else if (swapTask.swapOperation == SwapOperation.EXACT_INPUT_ALL) {
            return LidoV1_Multicaller(adapter).submitAll();
        }
