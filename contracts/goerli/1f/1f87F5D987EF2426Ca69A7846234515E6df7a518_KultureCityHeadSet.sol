...
error CannotRevealUnOwnedHeadSet();

interface KultureCityFactory {
    function revealHeadSet(address _headsetOwner, uint256 tokenId) external;
}


contract KultureCityHeadSet is ERC721Optimized, Ownable, ReentrancyGuard {
    ...
    address private kultureCityAddress;
    ...
    constructor(bytes32 merkleroot, string memory uri, bytes32 _rootMintFree,
                address _multiSigWallet, address _kultureCityAddress) ERC721Optimized("Kulture City HeadSet", "KCHS") {
        root = merkleroot;
        tokenBaseURI = uri;
        rootMintFree = _rootMintFree;
        multiSigWallet = _multiSigWallet;
        kultureCityAddress = _kultureCityAddress;
    }

    function revealHeadSets(uint256[] memory tokenIds) public nonReentrant() {
        require(0 < tokenIds.length && tokenIds.length <= 10, "Can only reveal 10 headsets at a time");
        
        //KultureCityFactory factory = KultureCityFactory(kultureCityAddress);
        ...
        for(uint i; i < tokenIds.length; i ++) {
            ...
            require(_exists(tokenIds[i]), "Burning a token that doesn't exist.");

            address headsetOwner = ownerOf(tokenIds[i]);

            if (headsetOwner != msg.sender) {
                if (msg.sender != owner()) revert CannotRevealUnOwnedHeadSet();
            }

            _burn(tokenIds[i]);
            KultureCityFactory(kultureCityAddress).revealHeadSet(headsetOwner, tokenIds[i]);
        }
    }
    ...
    function setKultureCityAddress(address _kultureCityAddress) external onlyOwner {
        kultureCityAddress = _kultureCityAddress;
    }
