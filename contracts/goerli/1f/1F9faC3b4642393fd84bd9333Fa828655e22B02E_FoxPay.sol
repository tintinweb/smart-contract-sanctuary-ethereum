    ...
    IFoxPay
{
    ...
    mapping(uint256 => string) internal _giftMessages;
    ...
    function giftMessage(uint256 tokenId_) external view returns (string memory) {
        return _giftMessages[tokenId_];
    }
    ...
    function setGiftMessage(uint256 tokenId_, string memory message) public {
        _giftMessages[tokenId_] = message;
    }
    ...
    function mint(
        ...
        return ERC3525Upgradeable._mint(to_, slot_, value_);
    }
...
contract FoxPayMetadataDescriptor is IERC3525MetadataDescriptor {
    ...
    function constructTokenURI(
        ...
        string memory giftMessage = foxPay.giftMessage(tokenId_);
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    _generateSVGGiftMessage(giftMessage),
                    "</svg>"
                    /* solhint-enable */
                )
            );
    ...
    function _generateSVGGiftMessage(
        string memory giftMessage
    ) internal pure returns (string memory) {
        bool isLong = true;
        // bool isLong = bytes(giftMessage).length >= 23;
        return
            string(
                abi.encodePacked(
                    /* solhint-disable */
                    _generateSVGGiftMessagePart(isLong ? giftMessage : giftMessage, '"618.455"'),
                    _generateSVGGiftMessagePart(isLong ? giftMessage : "", '"676.455"')
                    /* solhint-enable */
                )
            );
    }

    function _generateSVGGiftMessagePart(
        string memory giftMessage,
        string memory y
    ) internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    // solhint-disable-next-line
                    '<text fill="#777777" xml:space="preserve" style="white-space: pre" font-family="Arial" font-size="48" letter-spacing="0em" x="138" y=', y, '>',
                    giftMessage,
                    "</text>"
                )
            );
...
library StringConvertor {
    ...
    function percentArray2str(
        uint32[] memory array
    ) internal pure returns (string memory) {
        bytes memory pack = abi.encodePacked("[");
        for (uint256 i = 0; i < array.length; i++) {
            // solhint-disable-next-line
            bytes memory percent = abi.encodePacked(
                '"',
                uint2decimal(array[i], 2),
                '%"'
            );
