...
contract Token is StartTokenIdHelper, ERC721ABurnable, Ownable, ReentrancyGuard {
    ...
    event SaleStart(uint256 indexed _saleStartTime, uint256 _price, uint16 _transactionLimit);
    ...
    function startSale( uint256 price_, uint8 transactionLimit_ ) external onlyOwner {
        ...
        saleActive = true;
        emit SaleStart(saleStartTime, price, transactionLimit);
    }
    ...
    function _pauseSale() internal {
        saleActive = false;
        emit SalePaused(block.timestamp, totalSupply());
    }
