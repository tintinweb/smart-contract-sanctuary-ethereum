...
contract BeijingDuckLotteryDaily is ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => mapping(address => EnumerableSet.UintSet)) private userTictkets; //tickets[lotteryID][address] = [ uint32, uint32, ...]
    ...
    event BuyTickets(address indexed user, uint256 indexed lotteryID, uint32[] numberTickets);
    ...
    function buyTickets(uint256 _lotteryID, uint32[] calldata _ticketNumbers) external notContract nonReentrant {
        ...
        require(_ticketNumbers.length + userTictkets[_lotteryID][msg.sender].length() <= maxNumberTicketsPersonBuy, "Over quantity many tickets can buy");
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            userTictkets[_lotteryID][msg.sender].add(thisTicketNumber);
            allNumberTickets[_lotteryID].add(thisTicketNumber);
        }

        emit BuyTickets(msg.sender, _lotteryID, _ticketNumbers);
    }
    ...
    function _calculateTotalPriceForBuyTickets(uint256 _priceTicket, uint256 _numberTickets) internal pure returns (uint256) {
        ...
        if (_numberTickets >= 2 && _numberTickets < 50) {
            _discountDivisor = 5; // 0.05%
        } else if (_numberTickets >= 50 && _numberTickets < 100) {
            _discountDivisor = 245; // 2.45%
        } else if (_numberTickets > 100) {
            _discountDivisor = 495; // 4.95%
        }
        return (_priceTicket * _numberTickets * (10000 - _discountDivisor)) / 10000;
    }
    ...
    function getUserTictkets(uint256 _lotteryID, address _user) public view returns (uint256[] memory) {
        return userTictkets[_lotteryID][_user].values();
    }
