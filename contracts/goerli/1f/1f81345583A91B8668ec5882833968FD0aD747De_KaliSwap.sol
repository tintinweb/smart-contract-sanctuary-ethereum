...
contract KaliSwap is KaliOwnable, Multicall, ReentrancyGuard {
    /// -----------------------------------------------------------------------
    /// Library Usage
    /// -----------------------------------------------------------------------
    ...
    function setExtension(bytes calldata extensionData) external payable {
        (
            ...
            string memory details
                ...
                extensionData,
                (uint256, uint256, address, bool, uint32, uint96, uint96, string)
            );
    ...
    function callExtension(address dao, uint256 amount, bool swapIn)
        external
        payable
        nonReentrant
        returns (uint256 amountOut)
    {
        ...
        uint256 personalPurchased = sale.personalPurchased[msg.sender];
        uint256 purchaseTotal = sale.purchaseTotal;
        uint256 payment;

        if (sale.bidirection && swapIn) {
            if (
                personalPurchased < amount || 
                purchaseTotal < amount
            ) revert InvalidAmount(); 

            IKaliShareManager(dao).burnShares(msg.sender, amount);

            sale.purchaseTotal -= amount;
            sale.personalPurchased[msg.sender] -= amount;

            uint256 refund;
            refund = amount / sale.purchaseMultiplier;

            if (sale.purchaseAsset == address(0)) {
                // send ETH to msg.sender
                msg.sender._safeTransferETH(refund);
            } else if (sale.purchaseAsset == address(0xDead)) {
                // send ETH to wETH
                wETH._safeTransferETH(refund);
                // send wETH to msg.sender
                wETH._safeTransfer(msg.sender, refund);
            } else {
                // send tokens to msg.sender
                sale.purchaseAsset._safeTransferFrom(dao, msg.sender, refund);
            }
        } else {
            uint256 total;
            ...
            if (purchaseTotal + amountOut > sale.purchaseLimit)
                revert PurchaseLimit();
            if (personalPurchased + amountOut > sale.personalLimit)
