...
contract BulkRegister is Ownable {
    ...
    function verifyRegisterParams(RegisterRequest[] calldata requests) external view returns (string memory) {
        for (uint i = 0; i < requests.length; i++) {
            if (requests[i].tldNameHash == FOREVER_NAMEHASH) {
                string memory foreverError = _verifyForever(requests[i]);
                if (bytes(foreverError).length > 0) {
                    return foreverError;
                }
            } else if (requests[i].tldNameHash == ETH_NAMEHASH) {
                string memory ensError = _verifyENS(requests[i]);
                if (bytes(ensError).length > 0) {
                    return ensError;
                }
            } else {
                string memory imperviousError = _verifyImpervious(requests[i]);
    ...
    function _verifyForever(RegisterRequest calldata request) internal view returns (string memory) {
        ...
        if (cost != request.targetCost && request.targetCost + ((request.targetCost * 500) / 10000) > cost) {
            return "Prices have changed please try again";
        }

        return "";
    }

    function _verifyImpervious(RegisterRequest calldata request) internal view returns (string memory) {
        if (!registrar.controllers(request.tldNameHash, request.controller)) {
            return "Controllers have been updated please try again";
        }

        if (request.secret == 0 && IImperviousRegisterNowController(request.controller).commitmentOnly(request.tldNameHash)) {
            return "Some names require a commitment";
        }
    ...
    function _verifyENS(RegisterRequest calldata request) internal view returns (string memory) {
