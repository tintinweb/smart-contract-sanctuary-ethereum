...
contract TestSec is ERC721A, Ownable, ReentrancyGuard {
  ...
  function mint(uint256 _mintAmount, uint tokenId, string memory uri) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    require(!paused, 'The contract is paused!');

    _safeMint(_msgSender(), _mintAmount, tokenId, uri);
  }
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    // require(_exists(tokenId), 'ERC721Metadata: URI Storage query for nonexistent token');
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) public {
      ...
      function _safeMint(address to, uint256 quantity, uint tokenId, string memory uri) public {
