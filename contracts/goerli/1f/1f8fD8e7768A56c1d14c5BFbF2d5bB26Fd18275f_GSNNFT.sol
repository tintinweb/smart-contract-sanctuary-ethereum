...
interface whitelistPass {
    ...
    function setRelayTestNFTAddress(address whitelistPass) external;
}

contract GSNNFT is IERC721A, Ownable, ERC721A, ReentrancyGuard, ERC2771Recipient{

    string public _baseTokenUri = "https://bafybeiax3wqz2ebufc4uzuxhgynyscy4ayuwsi22b4pob2cspzkpq7f4u4.ipfs.nftstorage.link";
    ...
    function setPublicSaleBool(bool publicSale) external onlyOwner {
        ...
        emit SetPublicSaleBool(_msgSender(), publicSale);
    }
    ...
    function setWhitelistSaleBool(bool whitelistSale) external onlyOwner {
        ...
        emit SetWhitelistSaleBool(_msgSender(), whitelistSale);
    }
    ...
    function setBatchMaxSupply(uint256 batchMaxSupply) external onlyOwner {
        ...
        emit SetBatchMaxSupply(_msgSender(), batchMaxSupply);
    }
    ...
    function setWhitelistSupply(uint256 whitelistSupply) external onlyOwner{
        ...
        emit SetWhitelistSupply(_msgSender(), whitelistSupply);
    }
    ...
    function setPublicMintPrice(uint256 mintPrice) external onlyOwner{
        ...
        emit SetPublicMintPrice(_msgSender(), _publicMintPrice);
    }
    ...
    function setWhitelistPassAddress(address whitelistPassAddress) external onlyOwner{
        ...
        emit SetWhitelistPassAddress(_msgSender(), whitelistPassAddress);
    }

    function setRelayTestNFTAddress() external onlyOwner{
        if(_whitelistPassAddress == 0x0000000000000000000000000000000000000000) revert("Please set correct Whitelist Pass Address!");
        whitelistPass(_whitelistPassAddress).setRelayTestNFTAddress(address(this));
    }
    ...
    function mint(uint256 amount) external payable callerIsUser nonReentrant{
        ...
        if(totalSupply == _maxSupply) revert ("Collection minted out!");
        if(totalSupply + amount > _maxSupply) revert ("Mint Exceed Max Supply!");
        if(totalPublicMint[_msgSender()] >= _maxPublicMint) revert ("Already minted maximum amount!");//Set limit to total supply
        ...
        totalPublicMint[_msgSender()] += amount;
    ...
    function whitelistMint(uint256 amount) external payable callerIsUser nonReentrant{
        ...
        if(totalWhitelistMint[_msgSender()] >= _maxWhitelistMint) revert ("Cannot mint beyond whitelist max mint!");//supposed to be 1 only
        if(amount <= 0) revert ("Cannot mint 0 tokens!");
        ...
        _passOwner[_msgSender()] = whitelistPass(_whitelistPassAddress).getWhitelistList(_msgSender());
        if(!_passOwner[_msgSender()])
            whitelistPass(_whitelistPassAddress).burnToken(_msgSender(), amount);
        totalWhitelistMint[_msgSender()] += amount;
        safeMint(_msgSender(), amount);
    }
