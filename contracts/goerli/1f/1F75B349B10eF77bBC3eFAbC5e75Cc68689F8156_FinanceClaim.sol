...
contract FinanceClaim is IFinanceClaim, Ownable {
    ...
    IFixedSwap public fixedSwapContract;
    ...
    modifier isValidPoolIndex(uint256 poolIndex) {
        ...
        if (poolIndex >= poolsCount) {
            revert ClaimErrors.FinanceClaim__PoolIndexNonExistent(
                poolIndex,
                poolsCount
            );
        }
    ...
    constructor(address fixedSwapContractAddress) {
        require(
            fixedSwapContractAddress != address(0),
            "FinanceClaim: FixedSwap contract address is 0x0"
        );
        finOpsWhitelisted[msg.sender] = true;
        fixedSwapContract = IFixedSwap(fixedSwapContractAddress);
    }
    ...
    function depositFunds(
        uint256 poolIndex,
        uint256 amount
    ) external onlyFinOps isValidPoolIndex(poolIndex) {
        if (amount == 0) {
        ...
        IERC20(pools[poolIndex].reward).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        );
    ...
    function depositEther(
        uint256 poolIndex
    ) external payable onlyFinOps isValidPoolIndex(poolIndex) {
        if (!fixedSwapContract.isEtherPool(poolIndex)) {
            revert ClaimErrors.FinanceCLaim_PoolIsNotEther(poolIndex);
        }
        if (msg.value == 0) {
            revert ClaimErrors.FinanceClaim__ZeroDepositFunds();
        }
        pools[poolIndex].fundsDeposited += msg.value;
        emit FundsDeposited(poolIndex, msg.sender, msg.value);
    }

    /// @notice List rewards to their corresponding pool indices
    /// @dev Only triggerable by FinOps
    /// @dev Purposefully does not check whether the pool indices are ether pools
    ...
    function addPoolRewards(
        uint256[] calldata poolsIdxs,
        address[] calldata rewards
    ) external onlyFinOps {
        if (poolsIdxs.length != rewards.length) {
            revert ClaimErrors.FinanceClaim__ArrayLengthsMismatch(
                poolsIdxs.length,
                rewards.length
            );
        }
        for (uint256 i = 0; i < poolsIdxs.length; i++) {
            pools[poolsIdxs[i]].reward = rewards[i];
    ...
    function whitelistFinOpsAddresses(
        address[] calldata finOpsAddresses
    ) external onlyOwner {
        for (uint256 i = 0; i < finOpsAddresses.length; i++) {
    ...
    function removeWhitelistedFinOpsAddresses(
        address[] calldata finOpsAddresses
    ) external onlyOwner {
        for (uint256 i = 0; i < finOpsAddresses.length; i++) {
    ...
    /// @dev Can only be called by the owner
    /// @param poolIndex Index of the pool in FixedSwap contract
    /// @param users Array of addresses to set funds to
    /// @param amounts Array of amounts respectively corresponding to the addresses
    function setFundsClaimable(
        uint256 poolIndex,
        address[] calldata users,
        uint256[] calldata amounts
    ) external onlyOwner isValidPoolIndex(poolIndex) {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            if (users[i] == address(0)) {
                revert ClaimErrors.FinanceClaim__InvalidClaimableUser(users[i]);
            }
            pools[poolIndex].userClaimableFunds[users[i]] = amounts[i];
            pools[poolIndex].validUserClaims[users[i]] = keccak256(
                abi.encodePacked(
                    users[i],
                    poolIndex,
                    pools[poolIndex].latestWithdrawalTs
                )
            );
        }
    ...
    function claimFunds(
        uint256 poolIndex
    ) external isValidPoolIndex(poolIndex) {
        uint256 claimAmount = pools[poolIndex].userClaimableFunds[msg.sender];
        if (claimAmount == 0) {
            revert ClaimErrors.FinanceClaim__NoFundsToClaim(
                poolIndex,
                msg.sender
            );
        }
        bool isEtherPool = fixedSwapContract.isEtherPool(poolIndex);
        if (!isEtherPool && pools[poolIndex].reward == address(0)) {
            revert ClaimErrors.FinanceClaim__RewardNotInitialized(poolIndex);
        }
        bytes32 userClaimHash = keccak256(
            abi.encodePacked(
                msg.sender,
                poolIndex,
                pools[poolIndex].latestWithdrawalTs
        ...
        if (userClaimHash != pools[poolIndex].validUserClaims[msg.sender]) {
            revert ClaimErrors.FinanceClaim__InvalidClaimHash(
                msg.sender,
                poolIndex
            );
        }
        pools[poolIndex].userClaimableFunds[msg.sender] = 0;
        pools[poolIndex].fundsDeposited -= claimAmount;
        if (isEtherPool) {
            (bool sent, ) = msg.sender.call{value: claimAmount}("");
            require(sent, "Failed to send Ether");
        } else {
            IERC20(pools[poolIndex].reward).safeTransfer(
                msg.sender,
                claimAmount
            );
        }
    ...
    /// @dev only triggerable by FinOps
    /// @param poolIndex Index of the pool
    function withDrawPoolUnclaimedFunds(
        uint256 poolIndex
    ) external onlyFinOps isValidPoolIndex(poolIndex) {
        bool isEthPool = fixedSwapContract.isEtherPool(poolIndex);
        uint256 amount = pools[poolIndex].fundsDeposited;
        pools[poolIndex].fundsDeposited = 0;
        if (pools[poolIndex].reward == address(0)) {
            revert ClaimErrors.FinanceClaim__RewardNotInitialized(poolIndex);
        }
        pools[poolIndex].latestWithdrawalTs = block.timestamp; // Reset the timestamp to the current block timestamp so
        // that all users trying to claim after this will fail
        if (isEthPool) {
            (bool sent, ) = msg.sender.call{value: amount}("");
            require(sent, "Failed to send Ether");
        } else {
            IERC20(pools[poolIndex].reward).safeTransfer(msg.sender, amount);
        }
        emit UnClaimedFundsWithdrawn(poolIndex, msg.sender, amount);
    }

    /// @notice Changes the address of the FixedSwap contract instance in case of a wrongly set address upon deployment
    /// @dev only triggerable by the owner
    /// @param newFixedSwapAddress Address of the FixedSwapContract
    function changeFixedSwapAddress(
        address newFixedSwapAddress
    ) external onlyOwner {
        require(
            newFixedSwapAddress != address(0),
            "FinanceClaim: FixedSwap contract address is 0x0"
        );
        fixedSwapContract = IFixedSwap(newFixedSwapAddress);
        emit FixedSwapAddressChanged(msg.sender, newFixedSwapAddress);
    }

    /// @notice Returns the total deposited funds in a pool (both ETH and non-ETH pools)
    /// @param poolIndex Index of the pool
    function getPoolFundsDeposited(
        uint256 poolIndex
    ...
    function getUserClaimableFunds(
        uint256 poolIndex,
        address user
    ) external view returns (uint256) {
...
library ClaimErrors {
    ...
    error FinanceCLaim_PoolIsEther(uint256 poolIndex);
    error FinanceCLaim_PoolIsNotEther(uint256 poolIndex);
}
...
interface IFinanceClaim {
    ...
    function setFundsClaimable(uint256 poolIndex, address[] memory users, uint256[] memory amounts) external;
    ...
    function changeFixedSwapAddress(address newFixedSwapAddress) external;
    ...
    event FundsClaimed(uint256 poolIndex, address user, uint256 amount);
...
interface IFixedSwap {
    ...
    function isEtherPool(uint256 poolIndex) external view returns (bool);
}
