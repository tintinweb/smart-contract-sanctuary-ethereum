...
interface IOldCoolPets {
    function _currentPetId() external returns (uint256);
    function _reserved() external returns (uint256);
    ...
    OwnableUpgradeable
{
    ...
    // Token mint values
    uint256 public MAX_PET_ID;
    // Track minting outside of Cool Cat ids.
    uint256 public _currentPetId;
    ...
    bool public _claimsPaused;
    bool public _merkleLocked;
    ...
    function initialize(
        ...
        __ERC721_init("Cool Pets", "PETS");
        __Ownable_init();

        _w1 = 0x1fFa3371A45C22B1284fE5a251eD64F40580a1E3;
        burnAddr = 0x000000000000000000000000000000000000dEaD;
        _systemAddress = address(0);
        MAX_PET_ID = 19999;
        // Track minting outside of Cool Cat ids.
        _currentPetId = IOldCoolPets(oldContract)._currentPetId();
        // for giveaways, staff and partnerships
        _reserved = IOldCoolPets(oldContract)._reserved();
        _merkleRoot = 0x0;
        _price = 0.5 ether;
        _claimsPaused = true;
        _merkleLocked = false;
        ...
        _setDefaultRoyalty(_royaltyReceiver, 0); // set 0% royalty
    }

    function migrate(uint256[] memory tokenIds) external {
        for (uint256 i; i < tokenIds.length; i++) {
            IOldCoolPets(_oldContract).transferFrom(msg.sender, burnAddr, tokenIds[i]);
    ...
    function publicAdopt(
        ...
        require(
            _publicMintStatus == EPublicMintStatus.OPEN,
            "CP: Minting closed"
        );
    ...
    function allowListAdopt(
        ...
        require(msg.sender == tx.origin, "CP: We like real users");
        require(
            _publicMintStatus == EPublicMintStatus.ALLOW_LIST,
            "CP: Allow list closed"
        );
        require(msg.value == _price, "CP: Invalid Eth sent");
    ...
    function _claimTicket(uint256 ticketNumber) internal {
        require(
            ticketNumber < _allowListTicketSlots.length * 256,
            "CP: Invalid ticket"
        );
    ...
    function reservedMint() external onlyOwner {
        ...
        require(
            _publicMintStatus == EPublicMintStatus.RESERVED_MINT,
            "CP: Reserved mint closed"
        );
    ...
    function setMintSlotLength(uint256 num) external onlyOwner {
        ...
        require(
            _currentPetId + num <= MAX_PET_ID,
            "CP: More tickets than pets"
        );
    ...
    function changeRoyalty(address _newReceiver, uint96 _newPc)
        external
        onlyOwner
    {
        _setDefaultRoyalty(_newReceiver, _newPc);
    }
