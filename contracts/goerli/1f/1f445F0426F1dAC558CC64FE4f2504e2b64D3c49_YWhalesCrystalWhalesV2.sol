...
contract YWhalesCrystalWhalesV2 is Initializable, ERC721Upgradeable, ERC721BurnableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://tomato-capitalist-mollusk-258.mypinata.cloud/ipfs/QmQgpWodeeeYQaae5rtCv7MfasCeZXF5vcwUtpiJsCt7Gy/";
    }
    ...
    function mint(address[] memory _token0Addresses, address[] memory _token1Addresses, address[] memory _token2Addresses, address[] memory _token3Addresses, address[] memory _token4Addresses) public onlyRole(DEFAULT_ADMIN_ROLE){
        for(uint i = 0; i < _token0Addresses.length; i++) {
            _safeMint(msg.sender, 0);
        }

        for(uint i = 0; i < _token1Addresses.length; i++) {
            _safeMint(msg.sender, 1);
        }

        for(uint i = 0; i < _token2Addresses.length; i++) {
            _safeMint(msg.sender, 2);
        }

        for(uint i = 0; i < _token3Addresses.length; i++) {
            _safeMint(msg.sender, 3);
        }

        for(uint i = 0; i < _token4Addresses.length; i++) {
    ...
    function withdraw(address _to) external payable onlyRole(DEFAULT_ADMIN_ROLE) {
