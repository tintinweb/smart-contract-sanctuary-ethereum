...
contract Nemesis is ERC721A {
  ...
  function mint(uint256 price, uint256 nonce, bytes memory signature) external payable {
      ...
      require(isValidPriceSignature(price, nonce, signature), "Invalid signature");
  ...
  function renew(uint256 price, uint256 nonce, bytes memory signature, uint256 _tokenId) public payable {
      require(ownerOf(_tokenId) == msg.sender, "You don't own that token");
      require(_tokenExpiry[_tokenId] + _gracePeriod >= block.timestamp, "Token can't be renewed");
      require(msg.value >= price, "Not enough ether sent");
      require(!isNonceUsed(nonce), "Nonce already used");
  ...
  function isValidPriceSignature(uint256 price, uint256 nonce,  bytes memory _signature)  public
        view
        returns (bool) {
