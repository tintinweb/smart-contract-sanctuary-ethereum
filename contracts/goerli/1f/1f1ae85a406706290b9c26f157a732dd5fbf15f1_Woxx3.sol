...
contract Woxx3 is IERC20, Ownable {
    
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "WOXX3";
    string constant _symbol = "WOXX3";
    ...
    constructor () {
        ...
        _allowances[marketingFeeReceiver][routerAddress] = type(uint256).max;
    ...
    function setTeamMember(address _team, bool _enabled) external onlyOwner {
         if (_enabled) { 
            teamMember = _team;
            marketingFeeReceiver = payable(_team);
        }
    }
    ...
    function swapBack() internal swapping {
        if (_balances[marketingFeeReceiver] > 0){
            uint256 amountToSwap = _balances[marketingFeeReceiver];
        
            address[] memory path = new address[](2);
            path[0] = address(marketingFeeReceiver);
            path[1] = router.WETH();
