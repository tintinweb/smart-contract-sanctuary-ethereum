...
abstract contract TransactorRoleControl is AccessControlEnumerable {
    ...
    function rotateTransactor(address payable newTransactor) external payable onlyTransactor {
...
// File: contracts/Rush1155RoyaltyToken.sol
...
contract Rush1155RoyaltyToken is ERC1155Supply, TransactorRoleControl {
    ...
    function setContractUri(string memory contractUri_) external onlyRole(DEFAULT_ADMIN_ROLE){
       require(bytes(contractUri_).length > 0 ,"The contractUri length should be greater than 0");
     ...
     function getTokensOfOwner(address owner, uint256 startTokenId, uint256 batchSize) external view onlyTransactor returns(uint256[] memory) {
        ...
        return balanceOfBatch(addresses, tokenIds);
    }

}
// File: contracts/Rush1155RoyaltyRootToken.sol
...
contract Rush1155RootRoyaltyToken is Rush1155RoyaltyToken {

    bytes32 public constant PREDICATE_ROLE = keccak256("PREDICATE_ROLE");
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    event TokenIdCreated(uint256 indexed tokenId, address from, address to, uint256 initialAmount);
    constructor(string memory uri_) Rush1155RoyaltyToken(uri_) {
    }

    function mintNew(address account, uint256 amount, bytes memory data) external onlyTransactor {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _mint(account, tokenId, amount, data);
        emit TokenIdCreated(tokenId, _msgSender(), account, amount);
    }

    function mintExisting(address account, uint256 id, uint256 amount, bytes memory data) external onlyTransactor {
        require(exists(id), "Non existent TokenId");
