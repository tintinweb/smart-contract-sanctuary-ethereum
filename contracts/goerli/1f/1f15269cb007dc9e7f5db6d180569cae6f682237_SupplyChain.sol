...
contract SupplyChain {
 ...
 // Function to add a MedicalCompany to the supply chain system
  ...
  function addDrug(string memory _name, uint _id,string memory _serialNumber) public {
  ...
  function createDrugPackage(string memory _name,address _hospitaladdress, uint256 _id, uint256 _drugId, uint256 _quantity,string memory _expirationDate) public {
    if(medicalCompanies[msg.sender].etheruemAddress != msg.sender)
    {
        revert AccessDenied__OnlyMedicalCompany();
    }
...
/*    hospital patiatent patient     */
// Function to allow patients to send a request for a drug
...
function getMyPackages()external view returns(DrugPackage[] memory){

    if(msg.sender == suppliers[msg.sender].etheruemAddress)
    {
    DrugPackage[] memory data = new DrugPackage[](suppliers[msg.sender].packageIds.length);
    for (uint i = 0; i < suppliers[msg.sender].packageIds.length; i++) {
       if(drugPackages[suppliers[msg.sender].packageIds[i]].currentHolder == msg.sender) 
        data[i] = drugPackages[suppliers[msg.sender].packageIds[i]];
    }
    return data;
    }
    if(msg.sender == hospitals[msg.sender].etheruemAddress)
    {
    DrugPackage[] memory data = new DrugPackage[](hospitals[msg.sender].packageIds.length);
    for (uint i = 0; i < hospitals[msg.sender].packageIds.length; i++) {
       if(drugPackages[hospitals[msg.sender].packageIds[i]].currentHolder == msg.sender) 
        data[i] = drugPackages[hospitals[msg.sender].packageIds[i]];
    }
    return data;
    }
    if(msg.sender == medicalCompanies[msg.sender].etheruemAddress)
    {
    DrugPackage[] memory data = new DrugPackage[](medicalCompanies[msg.sender].packageIds.length);
    for (uint i = 0; i < medicalCompanies[msg.sender].packageIds.length; i++) {
       if(drugPackages[medicalCompanies[msg.sender].packageIds[i]].currentHolder == msg.sender) 
        data[i] = drugPackages[medicalCompanies[msg.sender].packageIds[i]];
    }
    return data;
    }
    else revert AccessDenied__OnlyHospital();
}
function getRemainingQuantiy(uint _drugPackageId)external view returns (uint numberOfDrugsInPackage){
  if(msg.sender != hospitals[msg.sender].etheruemAddress )
  revert AccessDenied__OnlyHospital();
    if(drugPackages[_drugPackageId].currentHolder != msg.sender )
  revert AccessDenied__OnlyCurrentHolder();
  return drugPackages[_drugPackageId].numberOfDrugsInPackage;
}
