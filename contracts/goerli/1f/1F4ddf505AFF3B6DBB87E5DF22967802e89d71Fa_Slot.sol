...
contract Slot is VRFConsumerBaseV2, Ownable {
    ...
    // constants
    uint32 private constant STARTING_STORED_WORDS = 500;
    uint32 private constant REFRESH_STORED_WORDS = 250;
    ...
    function requestMoreSpins() public returns (uint256 requestId) {
        ...
        if (randomWordsStored > REFRESH_STORED_WORDS) {
            revert Slot__HaveEnoughSpins(randomWordsStored, REFRESH_STORED_WORDS);
        }
    ...
    function spin() public payable {
        ...
        if (s_storedRandomWords.length <= REFRESH_STORED_WORDS) {
            requestMoreSpins();
        }
    }
