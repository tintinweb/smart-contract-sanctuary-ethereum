...
contract ISOY is ERC721A, Ownable {
    ...
    uint256 public FREE_SUPPLY = 999;
    uint256 public MAX_SUPPLY = 999;

    mapping(address=>uint256) public walletFreeMints;

    constructor() ERC721A("ISOY.wtf", "ISOY", MAX_TXN) {
        saleEnabled = true;
        price = 0.002 ether;
    }
    ...
    function freeMint(uint256 numOfTokens) external payable {
        ...
        require(walletFreeMints[msg.sender] + numOfTokens <= MAX_TXN_FREE, 'Exceed max per wallet');

        walletFreeMints[msg.sender]=numOfTokens;
        _safeMint(msg.sender, numOfTokens);
    }
