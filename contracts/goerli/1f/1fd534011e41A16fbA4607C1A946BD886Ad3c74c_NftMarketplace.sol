...
contract NftMarketplace is ReentrancyGuard {
  ...
  // 3. Cancel item
  // 4. Update listing
  // 5. Withdraw payments for my bought NFTs
  ...
  // Nft variables
  mapping (address => mapping (uint256 => Listing)) private s_listings; // collection -> token -> listing
  ...
  */
  ...
  function listItem(
    ...
    // Owners will own their NFTs and the Marketplace will have the approval to sell the NFTs.
    // nft address is like collection
    if (price <= 0) {
    ...
    isListed(nftAddress, tokenId) 
  {
    ...
    // Sending them to the user -> no
    // have them withdraw the money -> yes
    s_proceeds[listedItem.seller] = s_proceeds[listedItem.seller] + msg.value;
    delete (s_listings[nftAddress][tokenId]);
    IERC721(nftAddress).safeTransferFrom(listedItem.seller, msg.sender, tokenId);
    // check to make sure transfer
    emit ItemBought(msg.sender, nftAddress, tokenId, listedItem.price);
    // safe from reentrancy attacks --> always reset the balance first call last.
    // mutex modifier for security --> mutex lock
  }
