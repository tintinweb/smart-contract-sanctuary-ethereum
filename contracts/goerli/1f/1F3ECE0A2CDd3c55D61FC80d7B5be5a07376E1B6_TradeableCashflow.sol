...
contract TradeableCashflow is ERC721, RedirectAll {
    ...
    // Function to set image data for a token ID
    ...
    // Function to get image data for a token ID
    function getTokenImage(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId), "ERC721: token does not exist");
        return _tokenImages[tokenId];
    }

    // Override _baseURI to return the base URI for your token metadata
    function _baseURI() internal view virtual override returns (string memory) {
        return "https://gateway.pinata.cloud/ipfs/QmQ5iodMqYjnuMZwuVpoK6XHZ3LxLeq2gewFFuksZShdcc";
    }
}