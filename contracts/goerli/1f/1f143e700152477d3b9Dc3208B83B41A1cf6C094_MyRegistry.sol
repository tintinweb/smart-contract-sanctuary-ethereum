...
contract MyRegistry is Context, Ownable {
  ...
  constructor (string memory name_, string memory symbol_, address[] memory validators_) {
    ...
    /// @param version_ current version of the snap
    /// @param location location of the snap
    /// @param checksum checksum of the snap
  function create(address owner, string memory snapName, string memory uri, string memory version_, string memory location, string memory checksum) external {

    Snap storage newSnap = snaps[totalSnaps.current()];
    Version storage newVersion = snaps[totalSnaps.current()].versions[version_];
  ...
  function snap(uint256 snapId, string memory version) public view returns(string memory, address, Status, string memory) {
    return (snaps[snapId].name, snaps[snapId].owner, snaps[snapId].versions[version].status, snaps[snapId].uri);
  }

  /// Set snap uri
  function setURI(uint256 snapId, string memory uri) external {
    if (!_isOwner(_msgSender(), snapId)) revert OnlySnapOwner(snaps[snapId].owner, _msgSender());
    snaps[snapId].uri = uri;
  }

  /// Change snap owner
  function setOwner(uint256 snapId, address owner) external {
    if (!_isOwner(_msgSender(), snapId)) revert OnlySnapOwner(snaps[snapId].owner, _msgSender());
    snaps[snapId].owner = owner;
  }
  ...
  function assessement(uint256 snapId, string memory version, Risk risk, bytes memory signature) external {
    ...
    bytes32 structHash = 
      keccak256(abi.encode(
        ...
        snapId, 
        keccak256(bytes(snaps[snapId].name)), 
        keccak256(bytes(snaps[snapId].versions[version].location)), 
        keccak256(bytes(snaps[snapId].versions[version].checksum)),
        uint256(risk)));
    ...
    snaps[snapId].versions[version].risk = risk;
  }
  ...
  function validate(uint256 snapId, string memory version, Status status_, bytes memory signature) external {
    ...
    address signingAddress = ECDSA.recover(digest, signature);
    if(!_isValidator(signingAddress)) revert OnlyValidators(signingAddress);
    snaps[snapId].versions[version].status = status_;
  }
  ...
  function change(uint256 snapId, string memory version, string memory location, string memory checksum) external {
    if (!_isOwner(_msgSender(), snapId)) revert OnlySnapOwner(snaps[snapId].owner, _msgSender());
    snaps[snapId].versions[version].status = Status.UNVERIFIED;
    snaps[snapId].versions[version].location = location;
    snaps[snapId].versions[version].location = checksum;
  }

  function _isOwner(address requester, uint256 snapId) internal view returns (bool) {
    if(requester == snaps[snapId].owner) {
      return true; 
    } else {
      return false;
    }
  ...
  function block_(uint256 snapId, string memory version) external {
    if (!_isOwner(_msgSender(), snapId)) revert OnlySnapOwner(snaps[snapId].owner, _msgSender());
    snaps[snapId].versions[version].status = Status.BLOCKED;
  }
  ...
  function isValidated(uint256 snapId, string memory version) public view returns(bool) {
    return snaps[snapId].versions[version].status == Status.VERIFIED;
  }
