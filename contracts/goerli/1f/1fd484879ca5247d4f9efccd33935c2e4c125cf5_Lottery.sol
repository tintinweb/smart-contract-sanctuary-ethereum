...
// File: contracts/用 chainlink VRF 產生隨機數來實作樂透.sol
...
contract Lottery is VRFConsumerBaseV2 {
    ...
    uint32 numWords =  1;  //請求1個隨機數

    uint256 private _lotterNumber;
    uint256 public _mintBet = 0.0001 ether; //投一柱需要的金額
    uint256 public _minPick = 0.0005 ether; //開講需達成的金額 
    uint256 public _expired = 10 minutes; //進階提 設定時間
    ...
    modifier isActive(uint256 _lotteryId) {
        ...
        require(lottery.isActived , "Lottery not actived"); //lottery.isActived == true
        _;
    }
    //確認樂透是否超過期限
    ...
    //投注樂透,檢查狀態是否啟動,檢查是否過期
    function entry(uint256 _lotteryId) payable public isActive(_lotteryId) notExpired(_lotteryId){
        require(msg.value >= _mintBet, "You have not enough money."); //驗證金額是否大於最小投注金額
        LotteryData storage lottery = lotterys[_lotteryId];
    ...
    function openLottery() external onlyOwner{
        ...
        lottery.isActived = true;                           //1.把 isActive狀態打開
        lottery.expired = block.timestamp + _expired ;      //2.設定lottery expired    
        lotteryRound++;                                     //3.lotteryRound 增加
    }

    //挑選出幸運得主
    function pickWinner(uint256 _lotteryId) external onlyOwner isActive(_lotteryId) potChecker(_lotteryId) lock{
        LotteryData storage lottery =  lotterys[lotteryRound];
