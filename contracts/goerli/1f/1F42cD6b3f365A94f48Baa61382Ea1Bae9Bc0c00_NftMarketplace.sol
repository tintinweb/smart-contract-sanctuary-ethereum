...
contract NftMarketplace is ReentrancyGuard {
  //    a. `listItem`: ListNFTs on the marketplace
  //    b. `buyItem`: Buy the NFTs
  //    c. `cancelItem`: Cancel a listing
  //    d. `updateListing`: Update the price
  ...
  // NFT contract address -> NFT TokenID -> NFT Listing
    ...
    isOwner(nftAddress, tokenId, msg.sender)
  {
    ...
    // 1. Send the NFT to the contract. Transfer -> Contract "hold" the NFT (less preferred)
    // 2. Owners can still hold their NFT, and give the marketplace approval to sell the NFT for them
    // ^ The marketplace contract has the approval to swap the NFT with the buyer once the price is met
  }
    ...
    nonReentrant
  {
    ...
    // 2. Transfer the NFT to the buyer
    IERC721 nft = IERC721(nftAddress);
