...
import {IGovCoreDataHelper} from "./interfaces/IGovCoreDataHelper.sol";
import {IGovernanceCore} from "../../interfaces/IGovernanceCore.sol";

contract GovCoreDataHelper is IGovCoreDataHelper {
  function getProposalsData(IGovernanceCore govCore, uint256[] calldata proposalIds)
    external
    view
    returns (Proposal[] memory)
    {
      ...
      for (uint256 i = 0; i < proposalIds.length; i++) {
        ...
        proposals[i] = Proposal({
          ...
          executionPortal: proposalData.executionPortal,
  ...
  function getConstants(IGovernanceCore govCore)
    external
    view
  ...
  function getVotingConfigs(IGovernanceCore govCore, CrossChainMandateUtils.AccessControl[] calldata accessLevels)
    external
    view
    returns (VotingConfig[] memory)
    {
      VotingConfig[] memory votingConfigs = new VotingConfig[](accessLevels.length);
      ...
      for (uint256 i = 0; i < accessLevels.length; i++) {
        ...
        votingConfigs[i] = VotingConfig({
          ...
          quorum: votingConfig.quorum,
          differential: votingConfig.differential,
          minPropositionPower: votingConfig.minPropositionPower
        });
      }
