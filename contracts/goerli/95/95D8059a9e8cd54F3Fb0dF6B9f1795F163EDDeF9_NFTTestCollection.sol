...
import "./templates/NFTTest.sol";

contract NFTTestCollection is NFTTest {
    constructor(
...
Initializable
{
    ...
    struct DeploymentConfig {
        ...
        // Use `transferOwnership()` to update the contract owner.
        address owner;
        // Maximum supply of tokens that may be minted
        uint256 maxSupply;
    ...
    struct RuntimeConfig {
        ...
        uint256 mintStart;
        // Root of the Merkle tree approved minting.
        bytes32 approvalMerkleRoot;
    ...
    function mint(string memory uri, bytes32 approvalLeaf, bytes32[] calldata proof)
    external
    payable
    paymentProvided(_runtimeConfig.mintPrice)
    {
        require(mintingActive(), "Minting has not started yet");
        require(_tokenIdCounter.current() < _deploymentConfig.maxSupply, "The maximum supply has been reached.");
        require(isApproved(approvalLeaf, proof), "Not approved for minting.");

        _approvalMinted[approvalLeaf] = true;
    ...
    function mintingActive() public view returns (bool) {
        ...
        return block.timestamp > _runtimeConfig.mintStart;
    }

    /// Check if a valid approval leaf has been provided.
    function isApproved(bytes32 approvalLeaf, bytes32[] calldata proof)
    public
    view
    returns (bool)
    {
        require(!_approvalMinted[approvalLeaf], "Already minted");

        bytes32 leaf = keccak256(abi.encodePacked(approvalLeaf));

        return
        MerkleProof.verify(proof, _runtimeConfig.approvalMerkleRoot, leaf);
    }
    ...
    function withdrawFees() external onlyRole(ADMIN_ROLE) {
        _deploymentConfig.payoutAddress.sendValue(address(this).balance);
    }
    ...
    /// Mapping for tracking of approved mints
    mapping(bytes32 => bool) internal _approvalMinted;
    ...
    function contractURI() public pure returns (string memory) {
        return "https://cdn.sunriseserver.com/smart-contracts/metadata/NFTTestCollection.json";
    }
    ...
    function mintStart() public view returns (uint256) {
        return _runtimeConfig.mintStart;
    }

    function payoutAddress() public view returns (address) {
        return _deploymentConfig.payoutAddress;
    }
