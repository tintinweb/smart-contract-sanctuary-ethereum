...
contract ShibaDogeArmyV5 is ERC1155SupplyUpgradeable, Ownable2StepUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    using AddressUpgradeable for address;
    ...
    function updateArmyAddresses(address doge, address shiba) external onlyOwner {
    ...
    event crateDeposited(uint256 crateID, address sender);
    event crateContentsWithdrawn(uint256 crateID, address sender, uint256 contentsID, uint256 randomSeed);
    event crateBurned(uint256 crateID, address sender, uint256 withdrawBlockNumber);
    event crateInitializationComplete(uint256 crateID, uint256[] items, uint256[] supplies, uint256 totalCrates);
    event cratePauseToggled(bool paused);

    // crate id => user address => depositedBlockNumber;
    mapping(uint256 => mapping(address => uint256)) public crateAddressDepositBlock;

    function initializeCrate(uint256 crateID, uint256[] calldata items, uint256[] calldata supplies, uint256 totalCrates) external onlyOwner {
    ...
    function withdrawUnlockedCrateContents(uint256 crateID, bytes calldata data) external nonReentrant {
        require(!cratePaused[crateID]);
        ...
        if(withdrawBlockHash == 0){ // only the last 256 blockhashes are stored onchain, otherwise it's 0
        ...
        // get randomseed
        uint256 randomSeed = uint256(keccak256(abi.encodePacked(withdrawBlockHash, msg.sender, withdrawBlock)));

        // Get the item in the crate and its index for subtraction
        (uint256 crateContentsId, uint256 crateContentsIndex) = getCrateContents(crateID, randomSeed);
        ...
        emit crateContentsWithdrawn(crateID, msg.sender, crateContentsId, randomSeed);
    }

    function getCrateContents(uint256 crateID, uint256 randomseed) public view returns (uint256 crateContentsEftId, uint256 crateContentsIndex) {
