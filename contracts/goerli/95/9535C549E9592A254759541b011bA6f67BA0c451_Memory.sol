...
contract Memory is ERC721, ERC721Royalty, Ownable {
    ...
    function mintFromMinter(
        ...
        tokenHashesToTokenIds[tokenHash] = tokenId;
        tokenIdsToTokenHashes[tokenId] = tokenHash;
        tokenIdsToComponents[tokenId] = components;

        emit Mint(recipient, tokenHash, tokenId, components);
...
contract Minter is Ownable {
    ...
    function mintMemory(
        ...
        bytes32 componentsHash = keccak256(abi.encodePacked(components));
        require(
            isValidVerifySignature(componentsHash, signature),
            "Invalid verify signature provided"
        );
        require(msg.value > 0 && msg.value == PRICE, "Incorrect amount sent");

        mem.mintFromMinter(msg.sender, componentsHash, components);
    ...
    function allowlistMintMemory(
        ...
        bytes32 componentsHash = keccak256(abi.encodePacked(components));
        require(
            isValidVerifySignature(componentsHash, verifySignature),
            "Invalid verify signature provided"
        );
        require(msg.value > 0 && msg.value == PRICE, "Incorrect amount sent");
