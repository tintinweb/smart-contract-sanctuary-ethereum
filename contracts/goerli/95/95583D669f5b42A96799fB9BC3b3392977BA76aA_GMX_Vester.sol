...
contract GMX_Vester is GMX_IVester, GMX_IERC20, GMX_ReentrancyGuard, GMX_Governable {
    ...
    function withdraw() external nonReentrant {
        ...
        if (hasPairToken()) {
            ...
            GMX_IERC20(pairToken).safeTransfer(_receiver, pairAmount);
        }

        GMX_IERC20(esToken).safeTransfer(_receiver, balance);
    ...
    function transferStakeValues(address _sender, address _receiver) external override nonReentrant {
        ...
        uint256 cumulativeReward = GMX_IRewardTracker(rewardTracker).cumulativeRewards(_sender);
    ...
    function getMaxVestableAmount(address _account) public override view returns (uint256) {
        ...
        uint256 cumulativeReward = GMX_IRewardTracker(rewardTracker).cumulativeRewards(_account);
    ...
    function getCombinedAverageStakedAmount(address _account) public override view returns (uint256) {
        ...
        uint256 averageStakedAmount = GMX_IRewardTracker(rewardTracker).averageStakedAmounts(_account);
    ...
    function _deposit(address _account, uint256 _amount) private {
        ...
        GMX_IERC20(esToken).safeTransferFrom(_account, address(this), _amount);
        ...
        if (hasPairToken()) {
            ...
            if (nextPairAmount > pairAmount) {
                ...
                GMX_IERC20(pairToken).safeTransferFrom(_account, address(this), pairAmountDiff);
    ...
    function _updateVesting(address _account) private {
        ...
        GMX_IMintable(esToken).burn(address(this), amount);
    }
    ...
    function _claim(address _account, address _receiver) private returns (uint256) {
        ...
        GMX_IERC20(claimableToken).safeTransfer(_receiver, amount);
