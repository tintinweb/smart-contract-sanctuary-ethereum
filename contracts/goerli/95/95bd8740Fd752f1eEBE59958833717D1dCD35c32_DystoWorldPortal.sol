...
contract DystoWorldPortal is ERC721AUpgradeable, OwnableUpgradeable {  
    ...
    function initialize(
        ...
        __ERC721A_init("DystoWorldPortal", "DYSTO");
        __Ownable_init();
        ...
        discounts.push(Discount(0, 0));
        discounts.push(Discount(5, 5));
        discounts.push(Discount(25, 8));
        discounts.push(Discount(100, 12));
        discounts.push(Discount(250, 20));
    }

    function getPriceAfterDiscount(uint256 _numTokens) private view returns (uint256 priceAfterDiscount) {
        uint256 i;
        for (i = 1; i < discounts.length; i++) {
            if(_numTokens < discounts[i].threshold) {
                return price * _numTokens * (100 - discounts[i-1].percentage) / 100;
            }
        }
    }
    ...
    function mint(
        ...
        require(transactions[_transactionId] == false, "Transaction id was already used.");
        require(this.totalSupply() + _numTokens <= maxSupply, "Minted tokens would exceed supply.");

        uint256 priceAfterDiscount = getPriceAfterDiscount(_numTokens);
        if(paymentMethod == address(0)) {
            require(msg.value >= priceAfterDiscount, "Not enough ether to pay.");
        } else {
            IERC20(paymentMethod).transferFrom(msg.sender, address(this), priceAfterDiscount);
        }

        bytes32 _messageHash = keccak256(abi.encodePacked(msg.sender, _issued, _expiration, _numTokens, _transactionId, _chainId));
        require(
       ...
       emit Purchased(msg.sender, this.totalSupply() - _numTokens, _numTokens, _transactionId);
    }

    function mintTeam(address _receiver, uint256 _numTokens, uint256 _transactionId) external onlyOwner {
       require(transactions[_transactionId] == false, "Transaction id was already used.");
       require(this.totalSupply() + _numTokens <= maxSupply, "Minted tokens would exceed the max supply.");

       transactions[_transactionId] = true;
       _mint(_receiver, _numTokens);
       emit Purchased(_receiver, this.totalSupply() - _numTokens, _numTokens, _transactionId);
    }
    ...
    function updatePaymentMethod(address _paymentMethod) public onlyOwner {
        emit PaymentMethodUpdated(paymentMethod, _paymentMethod);
        paymentMethod = _paymentMethod;
    }

    function updateExpirationTimestamp(uint256 _expirationTimestamp) external onlyOwner {
    ...
    function addDiscount(uint256 _threshold, uint256 _percentage) external onlyOwner {
        require(_threshold >= 0, "threshold should be positive.");
        require(_percentage >= 0 && _percentage < 100, "Percentage out of bounds.");

        discounts.push(Discount(_threshold, _percentage));
    }

    function updateDiscount(uint256 _index, uint256 _threshold, uint256 _percentage) external onlyOwner {
        require(_index >= 0 && _index < discounts.length, "Index out of bounds.");
        require(_threshold >= 0, "threshold should be positive.");
        require(_percentage >= 0 && _percentage < 100, "Percentage out of bounds.");

        discounts[_index].threshold = _threshold;
        discounts[_index].percentage = _percentage;
    }

    function deleteLastDiscount() external onlyOwner {
        delete discounts[discounts.length - 1];
    }
