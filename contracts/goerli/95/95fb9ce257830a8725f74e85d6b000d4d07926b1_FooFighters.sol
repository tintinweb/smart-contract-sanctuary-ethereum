...
contract FooFighters is ERC721, Owned {
    ...
    /// @notice tokenId -> kills stats
    mapping(uint256 => uint256) public kills;
    ...
    function FOOFIGHTER_mint() external payable {
        require(!mintingClosed, "minting closed");
        require(msg.sender == tx.origin, "try a little harder");
        // require(msg.value == 0.01 ether, "send 0.01 ether to mint");
        unshiftedPower[totalSupply] = unsafeRandom(MAX_INITIAL_POWER, totalSupply);
    ...
    function finalizeMinting() external onlyOwner {
        mintingClosed = true;
        if (shift == 0) {
            shift = unsafeRandom(MAX_INITIAL_POWER, totalSupply) + 1;
        }
    }
    ...
    function acquireAllTheETH() external onlyOwner {
    ...
    function power(uint256 tokenId) public view returns (int256) {
        ...
        return int256((unshiftedPower[tokenId] + shift) % MAX_INITIAL_POWER) + powerDelta[tokenId] + 1;
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[5] = string.concat('kills: ', tokenKills.toString());
        parts[6] = '</text></svg>';
        ...
        string memory attributes = string.concat(
            '"attributes":[{"trait_type": "Power","value": ', tokenPower.toString(), '}, {"trait_type": "Health","value": ', tokenHealth.toString(), '}, {"trait_type": "Kills","value": ', tokenKills.toString(), '}]'
        );
