...
contract ProductPool is Ownable, ReentrancyGuard {
    ...
    //DataTypes.ProductInfo[] productPool;

    event AddProduct(address indexed owner, uint256 indexed productId);
    ...
    function _s_publishProduct(DataTypes.ProductInfo memory product)
        external
        onlyOwner
        returns (bool)
    {
        productPool[product.productId] = product;
        emit AddProduct(msg.sender, product.productId);
        return true;
    }

    function updateIsProductSatisfied(uint256 productId, bool isSatisfied)
        external
        onlyAdmin
        returns (bool)
    {
        productPool[productId].isSatisfied = isSatisfied;
        return true;
    }
    ...
    function _s_retireProductAndUpdateInfo(
        ...
        productPool[productId].totalCustomerReward = totalCustomerReward;
        return true;
    }
