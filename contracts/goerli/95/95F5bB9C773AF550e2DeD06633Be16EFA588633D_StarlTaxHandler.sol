...
contract StarlTaxHandler is ITaxHandler, Ownable {
     ...
     */
    function getTaxBasisPoints(address user) internal view returns (uint256) {
        uint256 userLevel = 0;
    ...
    function setTaxRates(
        ...
        require(thresholds.length == rates.length, "TaxHandler: Invalid level points");
        require(_basisTaxRate > 0, "TaxHandler: Invalid base rate");
    ...
    function _addNFTs(address[] memory contracts, uint8[] memory levels) private {
    ...
    function pauseTax() external onlyOwner {
        require(taxDisabled, "TaxHandler: already disabled");
        taxDisabled = true;
    }
    ...
    function resumeTax() external onlyOwner {
        require(!taxDisabled, "TaxHandler: not disabled");
        taxDisabled = false;
    }
}