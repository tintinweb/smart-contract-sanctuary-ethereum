...
contract ComptrollerV5Storage is ComptrollerV4Storage {
    /// @notice The portion of XCN that each contributor receives per block
    mapping(address => uint) public xcnContributorSpeeds;

    /// @notice Last block at which a contributor's XCN rewards have been allocated
...
contract Comptroller is ComptrollerV6Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when a new XCN speed is set for a contributor
    event ContributorXcnSpeedUpdated(address indexed contributor, uint newSpeed);
    ...
    /**
     * @notice Calculate additional accrued XCN for a contributor since last accrual
     * @param contributor The address to calculate contributor rewards for
     */
    function updateContributorRewards(address contributor) public {
        uint xcnSpeed = xcnContributorSpeeds[contributor];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, lastContributorBlock[contributor]);
        if (deltaBlocks > 0 && xcnSpeed > 0) {
            uint newAccrued = mul_(deltaBlocks, xcnSpeed);
            uint contributorAccrued = add_(xcnAccrued[contributor], newAccrued);

            xcnAccrued[contributor] = contributorAccrued;
    ...
    /**
     * @notice Set XCN speed for a single contributor
     * @param contributor The contributor whose XCN speed to update
     * @param xcnSpeed New XCN speed for contributor
     */
    function _setContributorXcnSpeed(address contributor, uint xcnSpeed) public {
        require(adminOrInitializing(), "only admin can set xcn speed");

        // note that XCN speed could be set to 0 to halt liquidity rewards for a contributor
        ...
        xcnContributorSpeeds[contributor] = xcnSpeed;

        emit ContributorXcnSpeedUpdated(contributor, xcnSpeed);
    }
