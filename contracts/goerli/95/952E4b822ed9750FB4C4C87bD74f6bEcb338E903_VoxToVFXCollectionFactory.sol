 ...
 */
contract VoxToVFXCollectionFactory is ICollectionFactory {
  ...
  modifier onlyAdmin() {
    require(rolesContract.isAdmin(msg.sender), "VoxToVFXCollectionFactory: Caller does not have the Admin role");
    _;
  }
  ...
  /**
   * @notice Allows VoxToVFX to change the collection implementation used for future collections.
  ...
  /**
   * @notice Allows VoxToVFX to change the proxy call contract address.
  ...
  /**
   * @notice Allows VoxToVFX to change the admin role contract address.
  ...
  function createCollection(
    ...
    require(bytes(symbol).length != 0, "VoxToVFXCollectionFactory: Symbol is required");
  ...
  function _updateRolesContract(address _rolesContract) private {
    require(_rolesContract.isContract(), "VoxToVFXCollectionFactory: RolesContract is not a contract");
  ...
  function _updateImplementation(address _implementation) private {
    require(_implementation.isContract(), "VoxToVFXCollectionFactory: Implementation is not a contract");
    ...
    ICollectionContractInitializer(_implementation).initialize(
      ...
      string(abi.encodePacked("VoxToVFX Collection Template v", version.toString())),
  ...
  function _updateProxyCallContract(address _proxyCallContract) private {
    require(_proxyCallContract.isContract(), "VoxToVFXCollectionFactory: Proxy call address is not a contract");
