...
contract BlackDAOBondDepositoryV3 is IBondDepository, NoteKeeper {
    /* ======== DEPENDENCIES ======== */
    ...
    /**
        @notice set adjustment info
        @param _id uint256
        @param _change uint64
        @param _timeToAdjusted uint48
     */
    function setAdjustment(
        uint256 _id,
        uint64 _change,
        uint48 _timeToAdjusted
        
    ) external onlyPolicy {
        require(isLive(_id), "Only can set adjustment in live market");
        uint48 _lastAdujstment = uint48(block.timestamp);

        adjustments[_id] = Adjustment({change: _change, lastAdjustment: _lastAdujstment, timeToAdjusted: _timeToAdjusted, active: true});
    } 
}
