...
import "./interfaces/IDiamondTickets.sol";
...
contract DiamondTicketsStorage {
    uint256 public nextTokenId;
    uint256 public tokenSupply;
    uint256 public constant maxTicketAmount = 9547;
    ...
    /// Set with #setMaxTicketTierAmount
    uint256 public maxTicketTierAmount;
...
contract DiamondTickets is 
    IDiamondTickets, 
    DiamondTicketsStorage,
    ReentrancyGuard,
    Ownable,
    ERC1155Metadata,
    ERC1155Enumerable,
    ...
    modifier cutTicketActive() {
        // If it's not yet or after the redeem time
    ...
    modifier saleInfoCheck(uint256 _startTime, uint256 _endTime) {
        // If we set the start time before end time
        if (_startTime > _endTime) {
            revert InvalidInput();
        }

        // If we haven't set the tier amount, mint time shouldn't be set
        if (maxTicketTierAmount == 0) {
            revert MintInfoUnset();
        }
        _;
    }
        ...
        mintWhitelistActive
    {
        bytes32 hash = ECDSA.toEthSignedMessageHash(
            keccak256(
                abi.encodePacked(msg.sender, _maxMintableAmount)
            )
        );

        // If this signature is from a valid signer
        if (ECDSA.recover(hash, _signature) != signer) {
            revert InvalidSignature();
        }

        whitelistMintedAmount[msg.sender] += _amount;

        // If the address has already minted
        if (whitelistMintedAmount[msg.sender] > _maxMintableAmount) {
            revert ExceedAmount();
        }
     ...
     * @param _amount Amount of tickets
     */
    function _mint(
        address _to,
        uint256 _amount
    ...
    {
        tokenSupply += _amount;
        // If the mint amount exceeds the maximum supply
        if (tokenSupply > maxTicketTierAmount) {
            revert ExceedAmount();
        }

        for (uint256 i; i < _amount; ) {
            _mint(_to, nextTokenId, 1, "");
    ...
    function setWhitelistMintPhase(
        ...
        override
        onlyOwner
        saleInfoCheck(_startTime, _endTime)
    {
        // If public mint time overlaps with whitelist mint time
        if ((_startTime >= mintPublicStartTime &&
            _startTime <= mintPublicEndTime) ||
            (_endTime >= mintPublicStartTime &&
            _endTime <= mintPublicEndTime) ||
            (_startTime <= mintPublicStartTime &&
            _endTime >= mintPublicEndTime)) {
            revert InvalidInput();
        }
    ...
    function setPublicMintPhase(
        ...
        saleInfoCheck(_startTime, _endTime)
    {
        // If whitelist mint time overlaps with public mint time
        if ((_startTime >= mintWhitelistStartTime &&
            _startTime <= mintWhitelistEndTime) ||
            (_endTime >= mintWhitelistStartTime &&
            _endTime <= mintWhitelistEndTime) ||
            (_startTime <= mintWhitelistStartTime &&
            _endTime >= mintWhitelistEndTime)) {
            revert InvalidInput();
        }
        ...
        onlyOwner
    {
        ...
        emit NumberSet(_price, "Price");
    }

    /**
     * @dev Set the maximum mintable tier amount in each phase
     * @param _amount New maximum tier amount
     */
    function setMaxTicketTierAmount(uint256 _amount)
        external
        override
        onlyOwner
        notDuringSales
    {
        ...
        if (_amount > maxTicketAmount || _amount < nextTokenId) { 
            revert InvalidInput();
        }

        maxTicketTierAmount = _amount;

        emit NumberSet(_amount, "Tier");
    }
    ...
    /** 
     * @dev Set the URI for tokenURI, which returns the metadata of the tokens
     * @param _baseURI URI that caller wants to set as tokenURI
     */
...
interface IDiamondTickets {
    ...
    function mintWhitelistTicket(uint256 _amount, uint256 _maxMintableAmount, bytes calldata _signature) external;
    // User can mint tickets with any address.
    function mintPublicTicket() external;
    ...
    function setMaxTicketTierAmount(uint256 _amount) external;
