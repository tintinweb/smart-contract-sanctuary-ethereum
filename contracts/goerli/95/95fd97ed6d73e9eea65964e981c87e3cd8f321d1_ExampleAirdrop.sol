...
contract ExampleAirdrop is ERC721AQueryable, ERC721ABurnable, EIP712, Ownable {
    ...
    constructor() ERC721A("ExampleAirdrop", "EXF") EIP712("Example", "1.0.0") {
    }
    ...
    function _hash(address account, uint amount) private view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("Example(address account,uint256 amount)"),
                    account,
                    amount
                )
            )
