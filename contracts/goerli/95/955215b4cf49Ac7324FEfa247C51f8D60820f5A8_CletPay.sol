...
contract CletPay is Ownable {
    ...
    mapping(string => uint256) private listedName_ID;
        ...
        isNameOwner(name_ToOwner[_name.toLower()])
    {
        if (nameForSale[_name] == true) {
            nameForSale[_name] = false;
            name_ToPrice[_name] = 0;
            deleteListedName(listedName_ID[_name]);
    ...
    function deleteListedName(uint256 _index) private {
        for (uint i = _index; i < listedNames.length - 1; i++) {
