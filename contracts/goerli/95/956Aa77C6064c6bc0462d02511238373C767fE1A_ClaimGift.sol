...
contract ClaimGift is Ownable, IERC1155Receiver, ReentrancyGuard, IERC721Receiver, GiftIndexManager {
    using SafeMath for uint256;

    ///@dev Event for transfering a gift to the user wallet address
    ...
    event ClaimGiftRequestComplete(uint256 requestId, uint256 requestAmount, address userwaller);
    ...
    ///@notice Only whitelisted wallet will be able to add claimable gift
    mapping(address => bool) public whiteListedWallets;

    ///@dev Wallet mapping with requestId
    mapping(uint256 => address) public giftClaimer;

    ///@dev Claimed gift amount mapping with requestId
    mapping(uint256 => uint256) public claimedAmount;

    ///@dev Claimed gift request status
    ...
    ///@dev Modifier for checking user claim amount
    ///@dev User will not be able to claim more than his gift of soul token amount
    modifier hasValidGiftBoxTokenBalance(uint256 _giftAmount) {
        ERC1155 RewardContract = ERC1155(rewardContractAddress);
        if (RewardContract.balanceOf(msg.sender, giftBoxTokenId) < _giftAmount) {
    ...
    function addERC1155GiftTokensToSmartContract(
        ...
        for (uint256 index = 0; index < _amount; index = _uncheckedInc(index)) {
            ...
            addIndexNumber(totalGiftAmount);
        }
    ...
    function rescueGiftToken(uint256 _giftIndex, address _transferTo) external nonReentrant onlyOwner {
        if (giftTokens[_giftIndex].giftCollectionContractType == uint256(GiftContractType.ERC_1155)) {
            ...
            if (!giftTokens[_giftIndex].isGivenAway && !giftTokens[_giftIndex].isRescued) {
                ...
                giftContract.safeTransferFrom(address(this), _transferTo, _giftIndex, 1, "");
                uint256 giftIndex = getIndexofNumber(_giftIndex);
                removeIndexNumber(giftIndex);
            } else {
    ...
    function claimGift(uint256 _claimGiftAmount) external nonReentrant noContracts hasValidGiftBoxTokenBalance(_claimGiftAmount) {
        if (!giftClaimActive) {
        ...
        giftClaimer[totalClaimedGift] = msg.sender;
        claimedAmount[totalClaimedGift] = _claimGiftAmount;
    ...
    function sendGift(uint256 _claimRequestId, uint256 _randomSeed) noContracts public {
        ...
        for (uint256 index = 0; index < claimedAmount[_claimRequestId]; index = _uncheckedInc(index)) {
            ...
            removeIndexNumber(selectedIndex);
...
/// @author LiquidX
/// @notice You could use function in this smart contract as a helper to do menial task

pragma solidity ^0.8.16;

error InvalidIndex();

contract GiftIndexManager {
    uint256 public giftIndexCount = 0;

    uint256[1000] public giftIndexs;

    function initNumbers(uint256 size) internal {
        for (uint256 index = 1; index <= size; index++) {
            giftIndexs[index] = index;
        }
        giftIndexCount = size;
    }

    function removeIndexNumber(uint256 index) internal {
        giftIndexs[index] = giftIndexs[giftIndexCount];
        giftIndexCount--;
    }

    function addIndexNumber(uint256 value) internal {
        giftIndexCount++;
        giftIndexs[giftIndexCount] = value;
    }

    function getIndexofNumber(uint256 number) public view returns (uint256) {
        uint256 selectedIndex = 0;
        for (uint256 index = 1; index <= giftIndexCount; index++) {
            if (giftIndexs[index] == number) {
                selectedIndex = index;
                break;
            }
        }
    ...
    function updateIndexNumber(uint256 index, uint256 value) internal {
        if (index < 1 || index > giftIndexCount) {
            revert InvalidIndex();
        }
        giftIndexs[index] = value;
    }
}
