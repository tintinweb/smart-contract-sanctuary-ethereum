...
contract KiKiStaking is IKiKiStaking, KiKiStakingToken, IsContract, Ownable, Initializable
{
    ...
    bytes32 internal constant LiQUIDITY_POOLED_POSITION = keccak256("KKS.KKS.liquidityPooledEther");
    ...
    /// @dev Total liquidity fee
    bytes32 internal constant TOTAL_LIQUIDITY_POOL_FEE_POSITION = keccak256("KKS.KKS.totalLiquidityPoolFee");
    
    /// @dev liquidity pool percent, based on 10000 point
    bytes32 internal constant LIQUIDITY_POOL_CAP_PERCENT_POSITION = keccak256("KKS.KKS.liquidityPoolCapPercent");
    /// @dev liquidity pool min fee, based on 10000 point
    bytes32 internal constant LIQUIDITY_POOL_MIN_FEE_POSITION = keccak256("KKS.KKS.liquidityPoolMinFee");
    /// @dev liquidity pool max fee, based on 10000 point
    bytes32 internal constant LIQUIDITY_POOL_MAX_FEE_POSITION = keccak256("KKS.KKS.liquidityPoolMaxFee");
    ...
    */
    ...
    function setLiquidityPoolPercent(uint16 points) external onlyOwner {        
        if (_readBPValue(LIQUIDITY_POOL_CAP_PERCENT_POSITION) != points) {
            ...
            if (liquidityPooled > liquidityPoolCap)
            {
                _updateLiquidityPool(0);
            }
        }
    }

    /**
      * @notice Set liquidity pool fee percent to min `minPoints`, max `maxPoints`, need manager role
    ...
    */
    function getLiquidityPoolFeePointsByEtherAmount(uint256 etherAmount) public view returns (uint16) {
        ...
        uint256 liquidityPoolCap = _getLiquidityPoolCap();
        if (0 == liquidityPoolCap) {
                return 0;
        }
        if (maxFee == minFee) {
            return minFee;
        } else {   
            uint256 _liquidityPooled = _getLiquidityPooledEther();
            // k = (maxFee - minFee) / liquidityPoolCap;
            // feePoints = (liquidityPoolCap - _liquidityPooled + outEth) * k + minFee;
            uint256 feePoints = liquidityPoolCap.sub(_liquidityPooled).add(outEth).mul(uint256(maxFee).sub(minFee)).div(liquidityPoolCap).add(minFee);
    ...
    function getLiquidityPoolNewStatus(uint256 _value) internal view returns (uint256 newLiquidityPooled, uint256 bufferedEther) {
        uint256 liquidityPoolCap = _getLiquidityPoolCap();
        ...
        if (_value.add(oldLiquidityPooled) <= liquidityPoolCap) {
            ...
            uint256 total = depositedAndBufferedEthers.add(oldLiquidityPooled).add(_value);
            // liquidity == liquidity pool size
            newLiquidityPooled = total.mul(liquidityPoolCapPercentPoints).div(10000);
            uint256 etherToBuffered = total.sub(newLiquidityPooled).sub(depositedAndBufferedEthers);
    ...
    function _setLastTotalLiquidityPoolFee(uint256 etherAmount) internal {
        TOTAL_LIQUIDITY_POOL_FEE_POSITION.setStorageUint256(etherAmount);
    }


    function _getLastTotalLiquidityPoolFee() internal view returns (uint256) {
        return TOTAL_LIQUIDITY_POOL_FEE_POSITION.getStorageUint256();
    }
    ...
    */
    ...
    /**
      * @notice Gets total liquidity pool fee
      */
    function getTotalLiquidityPoolFee() external view returns (uint256) {
        return TOTAL_LIQUIDITY_POOL_FEE_POSITION.getStorageUint256();
    }
    ...
    */
    ...
    function _getLiquidityPooledEther() internal view returns (uint256) {
        ...
        require(address(this).balance >= buffered, "KiKiStaking::_getLiquidityPooledEther: liquidity pool ether error");
        return buffered;
    }
    ...
    */
    ...
    function _getLiquidityPoolCap() internal view returns (uint256) {
        ...
        // todo liquidityPoolCapPercentPoints == 10000?
        return depositedAndBufferedEthers.mul(10000).div(uint256(10000).sub(liquidityPoolCapPercentPoints)).sub(depositedAndBufferedEthers);
    }

    function _updateLiquidityPool(uint256 _value) internal {
        (uint256 liquidityPooled, uint256 bufferedEther) = getLiquidityPoolNewStatus(_value);
        if (_getBufferedEther() != bufferedEther) {
            BUFFERED_ETHER_POSITION.setStorageUint256(bufferedEther);
        }
        LiQUIDITY_POOLED_POSITION.setStorageUint256(liquidityPooled);
    }
    ...
    function _swap(address from, address recipient, uint256 ethAmount, uint256 kETHAmount) internal whenNotStopped returns (uint256) {
        uint256 ethFeeAmount = getLiquidityPoolFeeByEtherAmount(ethAmount);
        LiQUIDITY_POOLED_POSITION.setStorageUint256(_getLiquidityPooledEther().sub(ethAmount).add(ethFeeAmount));
        _burnShares(from, kETHAmount);
        distributeRewards(ethFeeAmount);
        saveLiquidityPoolFee(ethFeeAmount);
    ...
    function saveLiquidityPoolFee(uint256 feeETH) internal {
        TOTAL_LIQUIDITY_POOL_FEE_POSITION.setStorageUint256(TOTAL_LIQUIDITY_POOL_FEE_POSITION.getStorageUint256().add(feeETH));
    }
