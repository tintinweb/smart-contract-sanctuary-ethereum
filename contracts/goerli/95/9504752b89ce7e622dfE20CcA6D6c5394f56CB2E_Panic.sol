...
contract Panic is Ownable, Initializable {
    ...
    function removeUniswapV3Liquidity(
        dataParams memory data
    ) external {
        data._nonfungiblePositionManager.safeTransferFrom(
            msg.sender,
            address(this),
            data._tokenId
        );
        ...
        (uint256 amount0, uint256 amount1) = data._nonfungiblePositionManager.decreaseLiquidity(params);

        // Get
        IERC20(data.token0).transfer(msg.sender, amount0);
        IERC20(data.token1).transfer(msg.sender, amount1);
    }
}
