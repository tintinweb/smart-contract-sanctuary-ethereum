...
contract Counter {
    ...
    /// @notice Function to set the address of the owner.
    ...
    function setFeePerTx(uint256 _feePerTx) external onlyOwner {
        require((_feePerTx != 0) && (_feePerTx < 10**16), "fee should be between 0 and 0.01 Ether");
        feePerTx = _feePerTx;
    }
    ...
    function send() external payable {
        ...
        uint256 _feePerTx = feePerTx;

        require(_counterpartOnOtherChain != address(0), "counterpart address on other chain not set");
        require(_feePerTx != 0, "fee per tx is not set");
        require(msg.value >= _feePerTx, "passed value is lesser than required");
