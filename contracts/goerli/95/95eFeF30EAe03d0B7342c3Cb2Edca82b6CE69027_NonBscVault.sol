...
contract NonBscVault is Ownable, Pausable {
    ...
    address public CROSS_FARM_ETH_CONTRACT;
    ...
    event UpdateCrossFarmingParams(address indexed sender, address crossFarming, uint8 chainId);
    event Pause();
    event Unpause();

    /**
     * @notice Constructor
     * @param _crossFarming: the Cross Farming contract
     * @param _chainId: the target chain id (on BSC)
     * @param _admin: address of the admin
     */
    constructor(
        address _crossFarming,
        uint8 _chainId,
        address _admin
    ) {
        CROSS_FARM_ETH_CONTRACT = _crossFarming;
    ...
    function deposit(uint256 _pid, uint256 _amount) external whenNotPaused notContract {
        ...
        // send message
        CrossFarming(CROSS_FARM_ETH_CONTRACT).sendFarmMessage(
            CROSS_FARM_ETH_CONTRACT,
            msg.sender,
            _pid,
            _amount,
     ...
     */
    function updateCrossFarmingParams(address _crossFarming, uint8 _chainId) external onlyAdmin {
        require(address(_crossFarming) != address(0), "no cross farming");
        CROSS_FARM_ETH_CONTRACT = _crossFarming;
        BSC_CHAIN_ID = _chainId;

        emit UpdateCrossFarmingParams(msg.sender, _crossFarming, _chainId);
    }
