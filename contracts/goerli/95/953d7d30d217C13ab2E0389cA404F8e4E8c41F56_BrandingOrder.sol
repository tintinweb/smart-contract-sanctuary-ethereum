...
contract BrandingOrder is Adminer {
  ...
  constructor(address payable platformAddress, uint256 platformFee) {
  ...
  function fulfillOrder(Order calldata order, uint256 amount, address to) external payable {
    ...
    if (info.orderType == 0) {
      ...
      uint256 payOwnerVal = (totalPrice / 100) * (100 - _platformFee);

      (bool platformReceiveSuccess, ) = _platformAddress.call{ value: totalPrice - payOwnerVal }("");
      require(platformReceiveSuccess, "Owner failed to receive eth");

      (bool ownerReceiveSuccess, ) = payable(owner).call{ value: payOwnerVal }("");
      ...
      weth.transferFrom(owner, _platformAddress, platformReceiveBalance);
  ...
  function setPlatformAddress(address payable platformAddress) public onlyAdminer {
    require(platformAddress != address(0), "Platform Address is error");
  ...
  function withdraw() external onlyAdminer {
    ...
    if (wethBalance > 0) {
      weth.transferFrom(address(this), _platformAddress, wethBalance);
    }
    
    (bool success, ) = _platformAddress.call{ value: address(this).balance }("");
