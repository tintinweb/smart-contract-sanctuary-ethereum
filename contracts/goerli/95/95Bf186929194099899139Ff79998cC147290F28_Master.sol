...
import "./libraries/ArrayManagement.sol";
...
contract Master is IMaster, Ownable, ReentrancyGuard {
    ...
    uint256 internal gasCheckpoint;
    ...
    constructor(
        ...
        if (_jolt == address(0)) revert ZeroAddressNativeToken();
        if (
            _unbondingBlocks == 0 ||
    ...
    function bond(uint256 _amount) public override {
        _bond(_amount);
    }
    ...
    function _bond(uint256 _amount) internal {
        ...
        IERC20(jolt).safeTransferFrom(msg.sender, address(this), _amount);
    }

    function consolidateBond() public override {
        Worker storage _worker = workers.byAddress[msg.sender];
        if (_worker.disallowed) revert Disallowed();
    ...
    function cancelBonding() public override {
        Worker storage _worker = workers.byAddress[msg.sender];
        if (_worker.disallowed) revert Disallowed();
        if (_worker.bonding == 0) revert NotBonding();
        uint256 _refundedAmount = _worker.bonding;
        _worker.bonding = 0;
        IERC20(jolt).safeTransfer(msg.sender, _refundedAmount);
    }
    ...
    function consolidateUnbonding() public override {
        Worker storage _worker = workers.byAddress[msg.sender];
        if (_worker.disallowed) revert Disallowed();
        ...
        IERC20(jolt).safeTransfer(msg.sender, _unbonding);
    }

    function cancelUnbonding() public override {
        Worker storage _worker = workers.byAddress[msg.sender];
        if (_worker.disallowed) revert Disallowed();
    ...
    function disallow(address _address) public override {
        if (msg.sender != owner()) revert Forbidden();
    ...
    function removeJob(uint256 _id) external override nonReentrant {
        ...
        Credit memory _credit = _job.credit; // gas savings
        if (_credit.amount > 0)
            IERC20(jolt).safeTransfer(_jobOwner, _credit.amount);
    ...
    function addCredit(uint256 _id, uint256 _amount) external override {
        _addCredit(_id, _amount);
    }
    ...
    function _addCredit(uint256 _id, uint256 _amount) internal {
        Job storage _job = jobs.byId[_id];
        address _jobOwner = _job.owner;
        if (_jobOwner == address(0)) revert NonExistentJob();
        uint256 _fee = (_amount * fee) / BASE;
        uint256 _creditMinusFee = _amount - _fee;
        _job.credit.add(_creditMinusFee, false);
        address _jolt = jolt; // gas savings
        IERC20(_jolt).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20(_jolt).safeTransfer(feeReceiver, _fee);
    }
    ...
    function addLiquidityCredit(
        ...
        address _jobOwner = _job.owner;
        if (_jobOwner == address(0)) revert NonExistentJob();
        uint256 _liquidityJoltValue = ILiquidityWeightCalculator(
            _liquidityWeightCalculator
        ).calculate(_liquidity, _amount);
        uint256 _credit = (_liquidityJoltValue *
            (BASE + liquidityTokenPremium)) / BASE;
        _job.credit.add(_credit, true);
        IERC20(jolt).safeTransferFrom(owner(), address(this), _credit);
    ...
    function removeCredit(uint256 _id, uint256 _amount) external {
        ...
        _job.credit.remove(_amount, false);
        IERC20(jolt).safeTransfer(msg.sender, _amount);
    }
        ...
        returns (bool, bytes memory)
    {
        ...
        if (workers.byAddress[_worker].bonded == 0) revert NotAWorker();
        (
            bool _assignedTurn,
            uint256 _turnIndex,
            uint256 _firstBlockInTurn
    ...
    function work(uint256 _id, bytes calldata _data) external override {
        gasCheckpoint = gasleft(); // even turn validation should be paid out to workers
        Job storage _job = jobs.byId[_id];
        Job memory _memoryJob = _job; // gas savings
        if (_memoryJob.owner == address(0)) revert NonExistentJob();
        ...
        uint256 _usedCredit = IWorkEvaluator(workEvaluator).evaluateCost(
            msg.sender,
            gasCheckpoint - gasleft()
        );
        _job.credit.remove(_usedCredit, true);
    ...
    function bonded(address _address) external view override returns (uint256) {
        return workers.byAddress[_address].bonded;
    }

    function earned(address _address) external view override returns (uint256) {
        return workers.byAddress[_address].earned;
    }
    ...
    function disallowed(address _address)
        ...
        returns (bool)
    {
        return workers.byAddress[_address].disallowed;
    }
    ...
    function credit(uint256 _id)
        external
        view
        override
        returns (Credit memory)
    {
        return jobs.byId[_id].credit;
    }
    ...
    function setJolt(address _jolt) external override {
        if (msg.sender != owner()) revert Forbidden();
        if (_jolt == address(0)) revert ZeroAddressJolt();
        jolt = _jolt;
    }
}
