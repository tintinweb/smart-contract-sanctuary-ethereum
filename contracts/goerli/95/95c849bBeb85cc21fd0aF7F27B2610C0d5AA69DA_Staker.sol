...
contract Staker {
    ...
    mapping(uint256 => bool) withdrawIDClaimable; // tracks what nonces the global claim fn has been called on
    ...
    constructor(
        ...
        // Once upgradeable, this will be removed
    ...
    function initialize(
        ...
        emit Initialized(_stakingTokenAddress, _stakeManagerContractAddress, _validatorShareContractAddress, _treasuryAddress, _phi, _cap);
    }

    // --- Events ---

    event Initialized(address _stakingTokenAddress, address _stakeManagerContractAddress, address _validatorShareContractAddress, address _treasuryAddress, uint256 _phi, uint256 _cap);
    ...
    function setPhi(uint256 _phi) public {
        // owner function
        require(
            msg.sender == ownerAddress,
            "Staker: message sender must be admin"
    ...
    function withdrawRequest(uint256 _amount) public {
    ...
    // TODO: code claim function!
    function claimWithdrawal(uint256 _withdrawID) public {
        require(
            withdrawIDClaimable[_withdrawID],
            "Staker: unbonding not complete or claim has not been called on withdrawal"
        );

        IERC20(stakingTokenAddress).transfer(msg.sender, userPendingWithdrawals[_withdrawID][msg.sender]);

        emit WithdrawalClaimed(msg.sender, userPendingWithdrawals[_withdrawID][msg.sender]);
    }
    ...
    function claimUnbond(uint256 _unbondNonce) public {
    ...
    function artificialDepositNotice(uint256 _amount) public {
        require(
            totalShares == 0,
    ...
    function transferStake(address _receiver) public {
...
// note perhaps set min amounts on functions with _amount to avoid rounding issues?
