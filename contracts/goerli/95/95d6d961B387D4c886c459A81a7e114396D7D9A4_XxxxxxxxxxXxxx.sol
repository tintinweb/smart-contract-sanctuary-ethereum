    ...
    ReentrancyGuard
{
    ...
    //
    uint256 private minRentalPeriod = 24 hours;
    uint256 private maxRentalPeriod = 7 days;
    ...
    function setTreasury(address _treasury) external onlyOwner {
        ...
        treasury = _treasury;
        // TODO: update defaultRoyaltyInfo as well
    }
    ...
    function mint(
        ...
        if (!_signatureValid(_permitted, _deadline, _signature))
            revert F_SignatureInvalid();
    ...
    function setUser(
        ...
        if (owner == address(0)) revert F_BadInput();

        if (msg.sender != owner)
            if (!isApprovedForAll[owner][msg.sender])
                if (getApproved[_tokenId] != msg.sender) revert F_Forbidden();

        if (_unpackedRentable(rentables[_tokenId]).expires > block.timestamp)
            revert F_TokenInUse();

        rentables[_tokenId] = _packRentable(_user, _expires, 0);
     ...
     */
    function rateOf(uint256 _tokenId) external view returns (uint256) {
        if (_ownerOf[_tokenId] == address(0)) revert F_BadInput();
     ...
     * @notice See {ERC4907Rentable}
     */
    function rent(uint256 _tokenId, uint64 _expires)
        external
        payable
        nonReentrant
    {
        address owner = _ownerOf[_tokenId];
        if (owner == address(0)) revert F_BadInput();
        if (_expires < block.timestamp) revert F_BadInput();
        ...
        uint256 _rent = rentable.rate * (_expires - block.timestamp);
        if (msg.value < _rent) revert F_Underpaid();

        rentables[_tokenId] = _packRentable(
            msg.sender,
            _expires,
            rentable.rate
        );
    ...
    /**
     * @notice See {ERC4907Rentable}
     * @dev The `rate` will be reset to zero upon token transfer.
     */
    function setRate(uint256 _tokenId, uint256 _rate) external {
        address owner = _ownerOf[_tokenId];
        if (owner == address(0)) revert F_BadInput();
        ...
        rentables[_tokenId] = _packRentable(address(0), 0, uint64(_rate));
        ...
        returns (string memory)
    {
        ...
        string memory _id = _tokenId.toString();
        uint256 _since = lastTransferred[_tokenId];
        string memory _level = (block.timestamp - _since) < initiation
            ? "Silver"
            : "Black";
        Rentable memory rentable = _unpackedRentable(rentables[_tokenId]);
        ...
        string memory encoded = Base64.encode(bytes(string(abi.encodePacked(
            /* solhint-disable */
            '{"name":"', name, ' #', _id, '","image":"', baseURI, _level, '.png",'
            '"animation_url":"', baseURI, _level, '.mp4",'
            '"attributes":[{"trait_type":"Level","value":"', _level, '"},',
            rentable.rate > 0 && rentable.expires < block.timestamp
                ? '{"value":"Rentable"},'
                : '',
            '{"display_type": "date","trait_type":"Since","value":',
            _since.toString(), '}]}'
            /* solhint-enable */
        ))));
     ...
     */
    function _unpackedRentable(uint256 _packed)
        internal
        pure
        returns (Rentable memory rentable)
    {
    ...
    function _packRentable(
        ...
        assembly {
            ...
            result := or(
                _user,
                or(shl(BITPOS_EXPIRES, _expires), shl(BITPOS_RATE, _rate))
            )
        }
    }
    ...
    function _signatureValid(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(msg.sender, _permitted, _deadline)
        );
     ...
     */
    function _signatureUsed(bytes32 _signatureHash)
        internal
        view
        returns (bool)
    {
    ...
    /**
     * @notice The hook function after token transfer
     */
...
interface IERC4907 {
    /**
     * @notice Emits when the `user` of an NFT or the `expires` of the `user` is changed
     */
    event UpdateUser(
...
interface IERC4907Rentable is IERC4907 {
     ...
     *  When a {Transfer} event emits, this also indicates that the rental rate
     *  of that NFT is reset to none.
     */
    event UpdateRate(uint256 indexed tokenId, uint256 rate);

    /**
     * @notice Rent an NFT with a specific end time
     * @dev Emit the {Rental} event. This function is called by the renter,
     *  while {setUser} is called by the owner or approved operators.
     * @param tokenId The identifier for an NFT
     * @param expires The end time of the rental
     */
    function rent(uint256 tokenId, uint64 expires) external payable;
    ...
    /**
     * @notice Query the rental price per second for an NFT
