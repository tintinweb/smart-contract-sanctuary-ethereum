...
contract UnifierProtocolDAO is Governor, GovernorSettings, GovernorCountingSimple, GovernorVotes, GovernorTimelockControl {
    ...
    event NewReviewerGroup(string indexed name, address indexed DAOaddress, address DAOtoken,
                           address DAOtimelock, address dataCollection);

    address public ReviewerGroupTemplate;
    address public ReviewerVotesTemplate;
    address public ReviewerTimeLockTemplate;


    constructor(IVotes _token, TimelockController _timelock,
                address _ReviewerGroupTemplate, address _ReviewerVotesTemplate, address _ReviewerTimeLockTemplate)
        Governor("UnifierProtocolDAO")
        GovernorSettings(1 /* 1 block */, 25 /* 250s */, 0)
        GovernorVotes(_token)
        GovernorTimelockControl(_timelock){
            ReviewerGroupTemplate = _ReviewerGroupTemplate;
            ReviewerVotesTemplate = _ReviewerVotesTemplate;
            ReviewerTimeLockTemplate = _ReviewerTimeLockTemplate;
    }

    function newReviewerGroup(string memory name, address DataCollectionTemplate) public {
        ReviewerVotes _token =  ReviewerVotes(Clones.clone(ReviewerVotesTemplate));
        _token.initialize(name);
        _token.transferOwnership(timelock());
        ReviewerTimeLock _timelock = ReviewerTimeLock(payable(Clones.clone(ReviewerTimeLockTemplate)));
        ReviewerGroup reviwerGroup = ReviewerGroup(payable(Clones.clone(ReviewerGroupTemplate)));
        _timelock.initialize(address(reviwerGroup));
        reviwerGroup.initialize(name, _token, _timelock, DataCollectionTemplate);
        address dataCollection = reviwerGroup.addDataCollection();
        emit NewReviewerGroup(name, address(reviwerGroup), address(_token), address(_timelock), dataCollection);

    }

    function setTemplate(address _ReviewerGroupTemplate,
                         address _ReviewerVotesTemplate,
                         address _ReviewerTimeLockTemplate) public onlyGovernance {
        ReviewerGroupTemplate = _ReviewerGroupTemplate;
        ReviewerVotesTemplate = _ReviewerVotesTemplate;
        ReviewerTimeLockTemplate = _ReviewerTimeLockTemplate;
    }
...
contract ReviewerGroup is Initializable, GovernorUpgradeable, GovernorSettingsUpgradeable, GovernorCountingSimpleUpgradeable, GovernorVotesUpgradeable, GovernorTimelockControlUpgradeable {
        ...
        initializer public
    {
        ...
        __GovernorSettings_init(1 /* 1 block */, 25 /* 25 blocks */, 0);
    ...
    function addDataCollection() public returns (address){
...
contract ReviewerVotes is Initializable, ERC20Upgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
    ...
    function addNewReviewer(address newReviewer) public onlyOwner {
        _mint(newReviewer, 1);
    }
