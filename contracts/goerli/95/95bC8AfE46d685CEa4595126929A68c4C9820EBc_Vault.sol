...
contract Vault is IVault, Ownable, ReentrancyGuard {
        ...
        unchecked {
            ...
            for (i = 0; i < _assets.paymentTokens.length; i++) {
                if (ftTypes[i] == AssetType.ERC_20) {
                    IERC20(_assets.paymentTokens[i]).safeTransferFrom(
                        _from,
                        _to,
                        _assets.amounts[i]
                    );
                    ...
                    if (_allowEth) {
                        (bool success, ) = _to.call{value: _assets.amounts[i]}(
                            ""
                        );
                        if (success == false)
                            revert VaultError(
    ...
    /// @notice Inherit from IVault
    function receiveAssets(Assets calldata _assets, address _from)
        external
        override
        onlyApproved
    {
    ...
    /// @notice Inherit from IVault
    function sendAssets(Assets calldata _assets, address _to)
        external
        override
        onlyApproved
    {
        ...
        unchecked {
            ...
            for (i = 0; i < _assets.paymentTokens.length; i++) {
                if (ftTypes[i] == AssetType.ERC_20) {
                    IERC20(_assets.paymentTokens[i]).safeTransfer(
                        _to,
                        _assets.amounts[i]
                    );
                } else {
...
interface IVault {
    ...
    /// @param assets Assets to be transfered
    /// @param from Sender address
    function receiveAssets(Assets calldata assets, address from) external;
