...
import './TangleswapPoolDeployer.sol';
import './NoDelegateCall.sol';

import './TangleswapPool.sol';

/// @title Canonical Tangleswap factory
/// @notice Deploys Tangleswap pools and manages ownership and control over pool protocol fees
contract TangleswapFactory is ITangleswapFactory, TangleswapPoolDeployer, NoDelegateCall {
    /// @inheritdoc ITangleswapFactory
    address public override owner;

    /// @inheritdoc ITangleswapFactory
    address public override burnWallet;

    /// @inheritdoc ITangleswapFactory
    address public override nonfungiblePositionManager;
    ...
    /// @param _burnWallet Burn wallet(used for buyback-and-burn of VOID)
    constructor(address _burnWallet) {
        owner = msg.sender;
    ...
    /// @inheritdoc ITangleswapFactory
    function modifyBurnWallet(address _burnWallet) external override {
        require(msg.sender == owner);
        burnWallet = _burnWallet;
    }

    /// @inheritdoc ITangleswapFactory
    function initNonfungiblePositionManager(address _nonfungiblePositionManager) external override {
        require(msg.sender == owner);
...
contract TangleswapPoolDeployer is ITangleswapPoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @inheritdoc ITangleswapPoolDeployer
    ...
    function deploy(
        ...
        pool = address(new TangleswapPool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
