    ...
    AccessControlUpgradeable
{
        ...
        isOwner(tokenAddress, tokenId, msg.sender)
    {
        ...
        // if (IDooplication(dooplicationAddress).tokenDooplicated(tokenId, tokenAddress)) {
    ...
    /**
     * @notice Get all the listings of a token contract on a dooplication contract.
     * Order is not guaranteed.
     * If any of the listings exist but the seller is not the owner of the token,
     * it is marked as not dooplicable.
     * This could happen if the seller transfer the token after the listing is created,
     * but before the dooplication is done.
     * Take this in consideration when iterating over the array,
     * as the dooplicable listings are the only ones valid to dooplicate.
     * @param dooplicationAddress Address of dooplication contract
     * @param tokenAddress Address of token contract
    ...
    function getActiveListings(
        ...
        ListingView[] memory activeListings = new ListingView[](length);
