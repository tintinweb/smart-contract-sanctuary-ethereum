...
contract Emily is Ownable,ERC721ACommon {
    ...
    uint private mintCostPresale = 0.1 ether;
    uint private mintCostPublicSale = 0.1 ether;
    ...
    address payable public daoWallet = payable(0x4864A4DAb5072541d47C6fc27Bb56d84c4EfBcdA);
    address payable public incubationWallet = payable(0xC7086517dEAd1157cBa2133288d68ABA7D5FD8Cc);
    address payable public marketingWallet = payable(0xB653f6Bbc877F81195FEE9e94Dc52E3804DA38ab);
    ...
    //mint a @param number of NFTs in presale
    function presaleMint(uint256 number, bytes32 hash, bytes memory signature) public payable {
        State saleState_ = saleState();
        require(saleState_ == State.Presale, "Not in Presale!");
        require(numberOfTotalTokens + number <= maxTokensPresale, "Not enough NFTs left to mint..");
        require(mintsPerAddress[msg.sender] + number - maxMintGenesis <= maxMintPresale, "Maximum 3 Mints per Address allowed!");
        require(recoverSigner(hash, signature) == signatureVerifier, "Address is not allowlisted");
