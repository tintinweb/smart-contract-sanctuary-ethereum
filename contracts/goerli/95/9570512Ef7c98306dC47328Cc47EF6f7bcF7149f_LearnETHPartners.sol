...
contract LearnETHPartners is ERC721, ReentrancyGuard, Ownable {
    ...
    uint256 monthDuration1 = 60;
	uint256 monthDuration = 600;
    ...
    event paidForCourse(uint256 paymentId, uint256 paidBy, uint256 paidTo, uint256 paidFor, uint256 paidAmount, uint256 paidAt);
    ...
    function registerAsPartner(uint256 _referrerTokenId) public nonReentrant payable {
        ...
        require(nodeDetails[_referrerTokenId].nodeType >= 2,"Incorrect Referrer Token Id");
    ...
    function enrolForCourse(uint256 _tokenId, uint256 _courseId) public payable {
        ...
        enrolledCourses[_tokenId].push() = _courseId;
        nodeDetails[nodeDetails[_tokenId].personalReferrer].lastSaleAt = block.timestamp;
        paymentDetails[paymentIndex].paidBy = _tokenId;
        paymentDetails[paymentIndex].paidTo = courseDetails[_courseId].instructorTokenId;
        paymentDetails[paymentIndex].paidFor = _courseId;
        ...
        emit paidForCourse(paymentIndex, _tokenId, courseDetails[_courseId].instructorTokenId, _courseId, msg.value, block.timestamp);
        paymentIndex+=1;
    }
    ...
    function enrolForPartnerCourse(uint256 _tokenId, uint256 _months) public nonReentrant payable {
        ...
        for(uint i=0; i< _months; i++) {
            nodeDetails[_tokenId].subscribedTill+=monthDuration;
    ...
    function unlockPayment(uint256 _paymentId) external {
		...
		console.log("Distributing Student upline");
        distributeUplineFee(instructorTokenId, instructorUplineFee);
		console.log("Distributing Instructor upline");
   ...
   function distributeUplineFee(uint256 _tokenId, uint256 _amount) public {
        ...
        for(uint i=1; i<=9; i++) {
			console.log("under for loop");
            if(_father1 == 1) {break;}
            else {
                 if(isEligibleForDistribution(_father1,i)) {
                    nodeDetails[_father1].balance+=_share;
					console.log("Adding balance %d to node %d",_share,_father1);
                    _amount-=_share;
                }
            }  
            _father1 = nodeDetails[_father1].father1;      
        }
		console.log("Amount is %d",_amount);
        nodeDetails[1].balance += _amount;
    } 
	
	/*function distributeUplineFee(uint256 tokenId, uint256 _amount) public {
        //uint256 _father1 = nodeDetails[_tokenId].father1;
    ...
    function isEligibleForDistribution(uint256 _tokenId, uint _depth) internal view returns(bool) {
		...
		{ 
	      console.log("Inside for node type 0");
	      return false;
	    } //unlikely scenario
		
        else if(nodeType > 2) 
        
        {  	console.log("Inside for node type greater than 2");
			return true;
		} //exempted from partner requirements
            ...
            if(subscribedTill < block.timestamp) 
			{
				console.log("Inside first if and returning false");
				return false;
			}
            else if(totalRecruits == 0 || totalRecruits <= 2 && _depth > 3 || totalRecruits <=4 && _depth > 6)
			
			{
				console.log("Inside else if and returning false");
				return false;
			}
    ...
    function updateCurrentPartnetCourseId() external onlyOwner {
