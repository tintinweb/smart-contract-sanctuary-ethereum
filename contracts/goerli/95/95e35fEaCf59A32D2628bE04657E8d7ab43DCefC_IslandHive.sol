...
contract IslandHive is ERC1155, Ownable {

    using Strings for uint256;
    uint256 public passPrice;
    uint256 private immutable passLimit;
    mapping(uint256 => address) internal creator;
    mapping(address => mapping(uint256 => uint256)) internal numberMinted;
    
    constructor(string memory name_, string memory symbol_, uint _passLimit, uint256 _passPrice, string memory baseURI_ ) ERC1155(baseURI_) {
        name_           =   name_;
        symbol_         =   symbol_;
    ...
    function _mintIsland(uint256[] memory ids, uint256[] memory amounts) external onlyOwner{
    ...
    function MindIsalndPass(uint _id) payable external{
        
        require(msg.sender != address(0), "Zero address");
        require(msg.value == passPrice, "Please enter correct price for minting pass");
        require(numberMinted[msg.sender][_id] + 1 <= passLimit, "You can not buy more pass");
        _safeTransferFrom(owner(), msg.sender, _id, 1, "");
        numberMinted[msg.sender][_id]++;
    }
