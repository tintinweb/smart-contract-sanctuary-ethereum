...
contract PermissionControl is IPermissionControl, Initializable, EIP712Upgradeable {
    ...
    mapping(bytes32 => mapping(address => uint256)) private blacklisted;
    ...
    function _addPermission(bytes32 daoId, Whitelist calldata whitelist, Blacklist calldata blacklist) internal {
        ...
        for (uint256 i = 0; i < length;) {
            blacklisted[daoId][blacklist.minterAccounts[i]] |= 0x11;
        ...
        for (uint256 i = 0; i < length;) {
            blacklisted[daoId][blacklist.canvasCreatorAccounts[i]] |= 0x101;
    ...
    function modifyPermission(
        ...
        uint256 length = blacklist.minterAccounts.length;
        for (uint256 i = 0; i < length;) {
            blacklisted[daoId][blacklist.minterAccounts[i]] |= 0x10;
        ...
        length = blacklist.canvasCreatorAccounts.length;
        for (uint256 i = 0; i < length;) {
            blacklisted[daoId][blacklist.canvasCreatorAccounts[i]] |= 0x100;
        ...
        for (uint256 i = 0; i < length;) {
            blacklisted[daoId][unblacklist.minterAccounts[i]] &= 0x01;
        ...
        for (uint256 i = 0; i < length;) {
            blacklisted[daoId][unblacklist.canvasCreatorAccounts[i]] &= 0x001;
    ...
    function isMinterBlacklisted(bytes32 daoId, address _account) external view returns (bool) {
        if ((blacklisted[daoId][_account] & 0x10) != 0) return true;
        return false;
    }
    ...
    function isCanvasCreatorBlacklisted(bytes32 daoId, address _account) external view returns (bool) {
        if ((blacklisted[daoId][_account] & 0x100) != 0) return true;
        return false;
    }
        ...
        returns (bool)
    {
        ...
        if (
            MerkleProofUpgradeable.verifyCalldata(
                _proof, whitelist.minterMerkleRoot, keccak256(abi.encodePacked(_account))
            )
        ) {
            return true;
        }
        ...
        returns (bool)
    {
        ...
        if (
            MerkleProofUpgradeable.verifyCalldata(
                _proof, whitelist.canvasCreatorMerkleRoot, keccak256(abi.encodePacked(_account))
            )
        ) {
            return true;
        }
