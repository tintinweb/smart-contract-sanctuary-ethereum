...
contract DAOxxx is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(uint256 maxBatchSize_, uint256 collectionSize_) ERC721A("xxxDAO", "xxxDAO", maxBatchSize_, collectionSize_) {
    maxPerAddressMint = maxBatchSize_;
    saleConfig.publicPrice = 0 ether;
    saleConfig.publicSaleKey = 0;
  }

  function endAuctionAndSetupNonAuctionSaleInfo(uint64 publicPriceWei, uint32 publicSaleKey) external onlyOwner {
  ...
  function publicSaleMint(uint256 quantity, uint256 callerPublicSaleKey) external payable callerIsUser {
    ...
    require(isPublicSaleOn(publicSaleKey),"public sale has not begun yet");
  ...
  function isQualified(bytes32[] memory proof, bytes32 root, address account) virtual public view returns (bool) {
  ...
  function isPublicSaleOn(uint256 publicSaleKey) public pure returns (bool) {
    return publicSaleKey != 0;
  }
