...
contract PlutoPawn is Ownable, ReentrancyGuard {
    ...
    /// @dev KerberosVault Contract address
    address private kerberosContract;
    ...
    /// @notice Delacre Offer Created Event
    event OfferCreated(
        uint256 listingId,
        uint256 offerId,
        address creator
    ...
    /// @notice Declare Listing Cancelled
    event CancelledListing(
        uint256 id
    );

    /// @notice Declare Offer Cancelled
    ...
    /// @param kerberos Kerberos Contract Address
    constructor(address kerberos) {
        if (kerberos == address(0)) { revert ZeroAddress(); }
        kerberosContract = kerberos;
        maxLoanPeriod = 366;
        pawnOpen = true;        
    }
    ...
    function acceptListing(uint256 listingId) external nonReentrant {
        ...
        if(IERC20(listing.tokenAddress).balanceOf(_msgSender()) < listing.loanAmount){ revert InvalidAmount(); }
        ...
        IKerberosVault(kerberosContract).createLoan(
            listing.creator,
            _msgSender(),
    ...
    function acceptOffer(uint256 listingId, uint256 offerId) external nonReentrant {
        ...
        Listing memory listing = listings[listingId];
        if(!listing.isActive){ revert InactiveListing(); }
        if(listing.signer != address(0)) { revert ZeroAddress(); }        
        if(_hasExpired(listingId, listing.listDate, listing.creator, listing.tokenAddress)){ revert ListingExpired();}
        Offer memory offer = offers[listingId][offerId];

        listings[listingId].isActive = false;
        if(!_verifyOwnership(listing.creator, listing.nftAddress, listing.nftId)){ revert InvalidCollateral(); }
        if(IERC20(listing.tokenAddress).balanceOf(offer.Lender) < listing.loanAmount){ revert InvalidAmount(); }
    ...
    function cancelOffer(uint256 listingId, uint256 offerId) external nonReentrant {
        ...
        emit CancelledOffer(listingId, offerId);
    }
    ...
    function counterOffer(
        ...
        Listing memory listing = listings[listingId];
        if(listing.signer != address(0))
            revert NotAuthorized();
        if(!listing.isActive)
            revert InactiveListing();
        if(_hasExpired(listingId, listing.listDate, listing.creator, listing.tokenAddress))
            revert ListingExpired();
        if(IERC20(token).balanceOf(_msgSender()) < loanAmount)
        ...
        emit OfferCreated(listingId,  listing.offerCount,  _msgSender());
    }
    ...
    /// @notice Upgrade the Kerberos Vault Contract
    /// @param kerberos New Kereros Contract Address
    function upgradeContracts(address kerberos) external onlyOwner {
        if (kerberos == address(0)) { revert ZeroAddress(); }
        kerberosContract = kerberos;
    }
    ...
    function _hasExpired(
        ...
        if (block.timestamp > creationDate + gracePeriod) {
            ...
            emit ListingDeleted(id, creator, nft);
        }
    }
