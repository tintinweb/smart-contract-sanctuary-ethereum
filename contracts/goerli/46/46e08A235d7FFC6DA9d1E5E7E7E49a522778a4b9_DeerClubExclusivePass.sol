...
import { IERC721AQueryable } from "lib/ERC721A/contracts/extensions/IERC721AQueryable.sol";
...
contract LasmStaking is Ownable,ReentrancyGuard {
    ...
    IERC721AQueryable public nftCollection;
    uint public constant MIN_LOCKED_PERIOD = 365 days;

    mapping(address=>mapping(uint=>uint[])) public userLockedNftTokenIds;
    mapping(address=>uint) public userLockedNftCount;

    event AddRewardToken(address account, uint amount);
    event StakeAdded(
    ...
    event RewardTokenRewardAdded(address indexed _from, uint _amount);
    event RewardTokenRewardRemoved(address indexed _to, uint _amount);
    ...
    event LockNft(address account, uint nftCount);
    event UnLockNft(address account, uint nftCount);
    ...
    function setRewardToken(IERC20 token) external onlyOwner {
        require(address(token) != address(0), "Wrong token address");
        rewardToken = token;
    }
    ...
    function withdrawRewardToken(uint amount) external onlyOwner {
        require(amount <= rewardToken.balanceOf(address(this)), "Insufficient balance");
        totalRewardTokenAmount -= amount;

        rewardToken.safeTransfer(msg.sender, amount);

        emit WithdrawRewardToken(msg.sender, amount);
    }

    function setNftCollection(IERC721AQueryable _nftCollection) external onlyOwner {
    ...
    function stake(uint amount, uint8 durationCode, uint lockNftCount) external nonReentrant {
        require(enableStaking,"Execution enableStaking");
        require(durationCode < 4,"Invalid duration with lock nft count");

        if (lockNftCount > 0) {
            require(nftCollection.balanceOf(msg.sender) >= lockNftCount, "Invalid lock nft count");
            require(durations[durationCode] >= MIN_LOCKED_PERIOD, "Invalid duration");
        }
        
        userId[msg.sender]++;

        uint bonus = calculateBonusRate(durationCode, lockNftCount);

        userStaked[msg.sender][userId[msg.sender]] = Info(amount, block.timestamp, block.timestamp,
                                                            durationCode, stakedIds[msg.sender].length, 
                                                            0, bonus, 0);
        ...
        _lockNft(lockNftCount, userId[msg.sender]);

        totalStakedToken += diff;
        userTotalStaked[msg.sender] += diff;
    ...
    function _lockNft(uint nftCount, uint stakedId) private {
        if (nftCount == 0)
            return;

        uint[] memory tokenIds = nftCollection.tokensOfOwner(msg.sender);
        
        require(nftCount <= tokenIds.length, "Lock NFT count wrong");
        ...
        for (uint i=0; i<nftCount; i++) {
            ...
            userLockedNftTokenIds[msg.sender][stakedId].push(tokenId);
            userLockedNftCount[msg.sender] ++;
        }

        emit LockNft(msg.sender, nftCount);
    }

    function calculateBonusRate(uint durationCode, uint nftCount) public view returns (uint result) {
        if (durations[durationCode] < MIN_LOCKED_PERIOD)
            return 0;
        ...
        if (nftCount <= 10) {
            result = bonusRates[0] + (nftCount - 1) * bonusRates[1];     // additional 0.75% for each item until 10 pieces
        }
        else if (nftCount > 10 && nftCount <= 15) {
            result = bonusRates[0] + 9 * bonusRates[1] + (nftCount - 10) * bonusRates[2];   // additional 0.5% for each item over 10 pieces
        }
        else {
            result = bonusRates[0] + 9 * bonusRates[1] + 5 * bonusRates[2];
        }
    }

    function getReward(address user, uint id) public view returns(uint reward) {
        Info memory userInfo = userStaked[user][id];
        require(durations[userInfo.durationCode] > 0, "Wrong duration value");
    ...
    function getAllReward(address user) public view returns(uint amount) {
        uint length = stakedIds[user].length;
        for(uint i=0; i<length; i++){
            uint amountOfIndex = getReward(user, stakedIds[user][i]);
            amount += amountOfIndex;
        }
    }

    function getStakedInfo(address user) public view 
        ...
        for(uint i=0; i<length; i++){
            Info storage userInfo = userStaked[user][stakedIds[user][i]];
            infors[i] = userInfo;
            pending[i] = getReward(user, stakedIds[user][i]);
            claimable[i] = claimableReward(user, stakedIds[user][i]);
        }
    }

    function claimableReward(address user, uint id) public view returns(uint) {
        Info storage userInfo = userStaked[user][id];

        if (block.timestamp - userInfo.stakeTime < durations[userInfo.durationCode])
            return 0;

        return getReward(user, id);
    }

    function claimableAllReward(address user) public view returns(uint) {
        uint amount;
        uint length = stakedIds[user].length;
        for(uint i=0; i<length; i++){
            Info storage userInfo = userStaked[user][stakedIds[user][i]];
            ...
            uint amountIndex = getReward(user, stakedIds[user][i]);
            amount += amountIndex;
        }
    ...
    function _claim(uint id) private {
        require(userStaked[msg.sender][id].amount != 0, "Invalid ID");

        uint amount = getReward(msg.sender, id);
        require(
        ...
        Info storage userInfo = userStaked[msg.sender][id];
        userInfo.lastClaim = block.timestamp;
        userInfo.earned += diff;

        userTotalEarnedReward[msg.sender] += diff;
        totalRewardTokenAmount -= amount;
    }
    ...
    function claimReward(uint id) public nonReentrant {
        ...
        emit ClaimReward(msg.sender, block.timestamp, id);
    }
    ...
    function claimAllReward() public nonReentrant {
        ...
        emit ClaimRewardAll(msg.sender, block.timestamp, diff);
    }

    function unstake(uint amount, uint id, bool unlockNft) external nonReentrant{
        _claim(id);

        Info storage userInfo = userStaked[msg.sender][id];
        require(userInfo.amount != 0 && amount <= userInfo.amount ,"Invalid ID");
        ...
        if (amount == userInfo.amount) {
            ...
            delete userStaked[msg.sender][id];
        }
        else {
            if (unlockNft)
                unLockNft(id);
    ...
    function popSlot(uint id) internal {
        uint length = stakedIds[msg.sender].length;
        bool replace = false;
        for (uint i=0; i<length; i++) {
            if (stakedIds[msg.sender][i] == id)
                replace = true;
    ...
    function unLockNft(uint userStakeId) private {
        Info storage userInfo = userStaked[msg.sender][userStakeId];

        uint lockedNftCount = userLockedNftTokenIds[msg.sender][userStakeId].length;
        if (lockedNftCount == 0)
            return;

        uint tokenId;
        for (uint i = 0; i < lockedNftCount; i++) {
            tokenId = userLockedNftTokenIds[msg.sender][userStakeId][i];
        ...
        delete userLockedNftTokenIds[msg.sender][userStakeId];

        userInfo.unLockedNftTime = block.timestamp;

        emit UnLockNft(msg.sender, lockedNftCount);
    }

    function getTokenIdsOfOwner(address account) public view returns(uint[] memory) {
        uint length = stakedIds[account].length;

        uint[] memory tokenIds = new uint[](userLockedNftCount[account]);
        uint index;
        uint stakedId;
        for(uint i=0; i<length; i++) {
            stakedId = stakedIds[account][i];
            for (uint j=0; j<userLockedNftTokenIds[account][stakedId].length; j++) {
                tokenIds[index] = userLockedNftTokenIds[account][stakedId][j];
...
contract Yield is Ownable, ReentrancyGuard {
    ...
    function addRewardToken(uint amount) external onlyOwner {
        ...
        emit AddRewardToken(msg.sender, diff);
    }
    ...
    function getRewardOfTokenId(uint tokenId) public view returns(uint) {
        ...
        uint64 mintedTime = IDeerClubExclusivePass(address(nftCollection))
                        .getTokenMintInfo(tokenId).mintedTimestamp;
        ...
        require(yieldPeriod > 0, "Wrong yield period");
    ...
    function getTotalReward(address account) public view returns(uint) {
        ...
        for (uint i=0; i<tokenIds.length; i++) {
            reward += getRewardOfTokenId(tokenIds[i]);
        }
