...
// File: contracts/royalty_nft.sol
...
contract Royalty_GAT is ERC721A, Ownable, ReentrancyGuard {
  ...
  function airdrop(address[] calldata wAddresses) external returns (address[] calldata)  {
  ...
  function mint(uint256 _mintAmount, string calldata _salt, bytes calldata _token) external payable nonReentrant {
    require(!paused, "Paused");
    if(usedMessages[_salt] == true)  
      revert saltAlreadyUsed();      
    if (!verifyTokenForAddress(_salt, _token, msg.sender, _signer))
