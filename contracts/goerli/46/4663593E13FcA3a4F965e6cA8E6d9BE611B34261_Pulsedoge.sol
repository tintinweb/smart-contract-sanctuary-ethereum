...
contract Pulsedoge is ERC20, ERC20Burnable {
    ...
    function whitelistAccount(address account) external onlyOwner {
        require(
            ...
            "Address is not contract address. Whitelisting is only on contracts"
        );
    ...
    function unWhitelistAccount(address account) external onlyOwner {
        require(
            isContract(account) == true,
            "Address is not contract address. UnWhitelisting is only on contracts"
        );
        delete _whitelisted[account];
    }

    /**
     * returns true if an account is whitelisted
     */
