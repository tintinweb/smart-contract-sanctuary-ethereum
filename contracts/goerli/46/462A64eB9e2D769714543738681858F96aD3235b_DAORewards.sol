...
contract DAORewards is IModule, IDAOV1Rewards, Initializable, AccessControlUpgradeable {
    ...
    function enforcePrerequisites(address registryAddr, bytes32 orgId) external view {
        ...
        require(
            address(daoStaking) == registry.getModuleValue(orgId, Modules.DAO_V1_STAKING, 0),
            "Prerequisites: daoStaking address must match registry"
        );
    }
    ...
    function setupPullToken(address source, uint256 startTs, uint256 endTs, uint256 amount) public {
        require(hasRole(MANAGER_ROLE, msg.sender), "DAORewards: caller is not the owner");
