...
contract RTKStaking is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    struct Stake {
        address owner;
        uint256 amount;
        uint256 time;
    ...
    function initializeStakingContract(
        ...
        require(_oneMonthTime > 0, "Time should be greater than zero");
        setMinimumStakingAmount(_amount);
        setRewardParameters(_oneMonth, _threeMonth, _sixMonth);
        initialized = true;
    ...
    function setRewardParameters(
        ...
        require(
            _oneMonth > 0 && _threeMonth > 0 && _sixMonth > 0,
    ...
    function claimRewards(bytes32 stakeId) public {
        ...
        require(
            s.daysToReward > 0,
            "You have already claimed all the daily rewards in this stake"
        );
        ...
        token.transferFrom(
            rewardsWallet,
            s.owner,
            daysUnclaimed * s.dailyReward
        );
        stakes[stakeId].lastRewardClaimAt =
            s.lastRewardClaimAt +
            daysUnclaimed *
            (oneMonth / 30);
    ...
    function stake(uint256 amount, uint256 _time) public {
        require(
            _time == oneMonth || _time == threeMonths || _time == sixMonths,
