...
interface ICityParks {
    function mint(uint256 numberOfMints) external payable;
}

contract CityParksClaim is IERC165, IERC721LazyPayableClaim, ICreatorExtensionTokenURI, ReentrancyGuard {
    using Strings for uint256;

    address public cityParksContract;
    ...
    modifier creatorAdminRequired(address creatorContractAddress) {
        Ownable creatorCoreContract = Ownable(creatorContractAddress);
        require(creatorCoreContract.owner() == msg.sender, "Wallet is not an owner for contract");
        _;
    }
    ...
    function initializeClaim(
        ...
        uint256,
        ClaimParameters calldata
    ) external override creatorAdminRequired(creatorContractAddress) {
        cityParksContract = creatorContractAddress;
    }
    ...
    function getClaim(address, uint256) external override view returns(Claim memory claim) {
        return Claim({
            total: uint32(ERC721Enumerable(cityParksContract).totalSupply()),
            totalMax: 1700,
            walletMax: 0,
            startDate: 0,
            endDate: 0,
            ...
            paymentReceiver: payable(0x6140F00e4Ff3936702E68744f2b5978885464cbB)
        });
    }

    function checkMintIndex(address, uint256, uint32) public override pure returns(bool) {
        return true;
    }

    function checkMintIndices(address, uint256, uint32[] calldata mintIndices) external override pure returns(bool[] memory minted) {
    ...
    function getTotalMints(address, address, uint256) external override view returns(uint32) {
        return uint32(ERC721Enumerable(cityParksContract).totalSupply());
    }

    /**
     * See {IERC721LazyClaim-mint}.
     */
    function mint(address, uint256, uint32, bytes32[] calldata, address) external payable override {
        ICityParks(cityParksContract).mint{value: msg.value}(1);
    }

    /**
     * See {IERC721LazyClaim-mintBatch}.
     */
    function mintBatch(address, uint256, uint16 mintCount, uint32[] calldata, bytes32[][] calldata, address) external payable override {
        ICityParks(cityParksContract).mint{value: msg.value}(mintCount);
    }

    function airdrop(address, uint256, address[] calldata,
            uint16[] calldata) external override {
        // Not implemented
    }

    function tokenURI(address, uint256 tokenId) external override view returns(string memory uri) {
        return ERC721Enumerable(cityParksContract).tokenURI(tokenId);
    }
}
