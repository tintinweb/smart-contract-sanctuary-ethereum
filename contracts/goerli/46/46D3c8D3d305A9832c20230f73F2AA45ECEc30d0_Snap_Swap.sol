    ...
    ReentrancyGuard
{
    ...
    */
     ...
     */
    function swap_by_USDT(uint256 _amount, bytes32 _referral)
        public
        nonReentrant
    {
        address _USDT = USDT;
        ...
        IERC20(token).transfer(treasury_wallet, percentage);
    ...
    */
    ...
    @param _referral code 
     */
    function swap_by_USDC(uint256 _amount, bytes32 _referral)
        public
        nonReentrant
    {
        address _USDC = USDC;
    ...
    * NOTE: false is USDC, true is USDT
     ...
     */
    function updateAggr(address _newPriceFeed, bool _isUSDT) public AdminOnly {
     ...
     * Note: only owner can perform this function
     */
     ...
     */
    function updateLimit(uint256 _new_Limit, bool _isDaily) public AdminOnly {
        if (_isDaily) {
     ...
     */
    function getRefPercentage(bytes32 _ref_code) public view returns (uint256) {
        return REF_DB.referrals_mapping(_ref_code, 0);
    }
     ...
     */
    function setBasePercentage(uint256 _new_base) public AdminOnly {
        base_fee = _new_base;
...
contract ReferralDB is Context, AccessControlEnumerable, Pausable {
     ...
     * @param _info in uint256 to place in the index of the referral array
     * @param _referral_code of the user - string
    ...
    /**
     * @dev adds description for a specific index
     * @param _index of the array to add description for
     * @param _desc is the string description of the index
     * Note: only Admin can perform this action
     */
    function updateDesc(uint256 _index, bytes32 _desc) public {
        if (!hasRole(DEFAULT_ADMIN_ROLE, _msgSender())) {
            revert NotUpdater();
        }
        if (_index > 999 || _index < 0) {
        ...
        index_desc[_index] = _desc;
        emit desc_updated(_index, _desc);
    }
}