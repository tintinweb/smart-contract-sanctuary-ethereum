...
contract Staking is Pausable, Ownable {
  ...
  event SetExpiresAfterToPercent(uint256[] expiresAfter, uint256[] percent);
  ...
  mapping(address => uint256) private _accountsTotalPositionsCount;
  ...
  constructor(address tokenAddress) {
    ...
    setExpiresAfterToPercent(_availableExpiresAfter, _availablePercents);
  }

  function _incrementAccountsTotalPositionsCount(address account)
    internal
    virtual
    returns (uint256)
  {
    uint256 currentTotalPositionsCount = _accountsTotalPositionsCount[account];
    _accountsTotalPositionsCount[account] = _accountsTotalPositionsCount[
      account
    ].add(1);
    ...
    returns (bool)
  {
    ...
    bytes32 id = keccak256(
      abi.encode(_incrementAccountsTotalPositionsCount(msg.sender))
    );
    ...
    if (_accountsPositionsTail[msg.sender] != 0) {
      _positions[msg.sender][_accountsPositionsTail[msg.sender]].nextId = id;
    }

    _positions[msg.sender][id] = position;
  ...
  function closePosition(bytes32 id) public {
    require(_positions[msg.sender][id].deposit > 0, "Position does not exist");
    require(
      _positions[msg.sender][id].expiresAt < block.timestamp,
      "Position is not expired"
    );

    uint256 amount = _positions[msg.sender][id].deposit;
    uint256 interest = _positions[msg.sender][id].interest;
    ...
    if (_positions[msg.sender][id].previousId != 0) {
      _positions[msg.sender][_positions[msg.sender][id].previousId]
        .nextId = _positions[msg.sender][id].nextId;
    }

    // # if position is not the last one in the list (has next position)
    if (_positions[msg.sender][id].nextId != 0) {
      _positions[msg.sender][_positions[msg.sender][id].nextId]
        .previousId = _positions[msg.sender][id].previousId;
    }

    // # if position is the last one in the list (has no next position)
    if (_accountsPositionsTail[msg.sender] == id) {
      _accountsPositionsTail[msg.sender] = _positions[msg.sender][id]
        .previousId;
    }
    // # save position to emit event
    Position memory position = _positions[msg.sender][id];
  ...
  function getExpiresAfterToPercent()
    public
    view
    virtual
    returns (
  ...
  function setExpiresAfterToPercent(
    ...
    emit SetExpiresAfterToPercent(expiresAfter, percents);
  }
    ...
    returns (Position[] memory)
  {
    ...
    for (uint256 i = _accountsActivePositionsCount[account]; i > 0; i--) {
      if (i == _accountsActivePositionsCount[account]) {
        positions[activePositionsIndex.sub(1)] = _positions[account][
        ...
        positions[activePositionsIndex.sub(1)] = _positions[account][prevId];
        prevId = positions[activePositionsIndex.sub(1)].previousId;
        activePositionsIndex--;
      }
    }
  ...
  function getLastPosition(address account)
    ...
    returns (Position memory)
  {
    return _positions[account][_accountsPositionsTail[account]];
  }
  ...
  function getPosition(address account, bytes32 id)
    public
    view
    virtual
    returns (Position memory)
  {
    return _positions[account][id];
  }
  ...
  function getAccountsTotalPositionsCount(address account)
    public
    view
    virtual
    returns (uint256)
  {
    return _accountsTotalPositionsCount[account];
  }
