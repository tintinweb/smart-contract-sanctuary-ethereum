    ...
    VRFConsumerBaseV2
{
    ...
    mapping(address => uint256[]) private allUserTokenIds;
    ...
    mapping(address => uint256[]) private allUsersBurnedTokenIds;
    ...
    function requestCreateWord(string memory _word) public payable {
        ...
        require(
            nordleWordBank.exists(_word),
            "Word must exist in the word bank"
        );
    ...
    function fulfillRandomWords(
        ...
        _createWord(
            words[randomIndex],
            wordRequesters[_vrfRequestId],
            new uint256[](0)
        ); // last arg: no tokens to combine
    ...
    function _submitChainlinkApiRequest(
        ...
        // https://nordle-server-ltu9g.ondigitalocean.app
        string memory drawUrl = string.concat(
            "https://c050-4-71-27-132.ngrok.io/draw?phrase=",
            phrase
        );
        ...
        internal
    {
        ...
        allUserTokenIds[owner].push(tokenIdCount);
        userTokenIdIndex[owner][tokenIdCount] =
            allUserTokenIds[owner].length -
            1;
    ...
    function _burnWord(uint256 tokenId) internal {
        ...
        uint256 index = userTokenIdIndex[owner][tokenId];

        _burn(tokenId);
        allUsersBurnedTokenIds[owner].push(tokenId);

        allUserTokenIds[owner][index] = allUserTokenIds[owner][
            allUserTokenIds[owner].length - 1
        ];
        userTokenIdIndex[owner][allUserTokenIds[owner][index]] = index;

        delete allUserTokenIds[owner][allUserTokenIds[owner].length - 1];
        delete userTokenIdIndex[owner][tokenId];
    }

    function getAllUserTokenIds(address user)
        public
        view
        returns (
            uint256[] memory userTokenIds,
            uint256[] memory userBurnedTokenIds
        )
    {
        userTokenIds = allUserTokenIds[user];
        userBurnedTokenIds = allUsersBurnedTokenIds[user];
    }
