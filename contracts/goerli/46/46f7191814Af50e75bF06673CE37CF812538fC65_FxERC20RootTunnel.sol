...
contract FxERC20RootTunnel is FxBaseRootTunnel, Create2 {
    ...
    event Withdraw(uint txHash);

    mapping(address => address) public rootToChildTokens;
    bytes32 public childTokenTemplateCodeHash;
    address public bridge; // bridge's wallet
    mapping (uint => bool) public txsCompleted;

    constructor(
        address _fxRoot,
        address _fxERC20Token
    ) FxBaseRootTunnel(_fxRoot) {
    ...
    function withdraw(address rootToken, uint amount, uint txId, bytes memory _signature) public {
        require(verify(amount, txId, _signature), "Invalid signature");
	...
	function getMessageHash(address _receiver, uint _amount, uint _txId) public pure returns(bytes32) {
		return keccak256(abi.encodePacked(_receiver, _amount, _txId));
    }

    function verify(uint _amount, uint _txId, bytes memory _signature) public view returns(bool) {
        bytes32 messageHash = getMessageHash(msg.sender, _amount, _txId);
