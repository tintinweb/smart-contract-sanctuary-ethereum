...
contract ProtocolRegistry is Registry, Ownable {
    ...
    function addPartnership(string memory partnerName_, address admin_) external payable onlyOwner {
    ...
    function registerRentContract(
        ...
        _setRecord(nodeName_, owner_, address(wrapperContractAddr_));
...
//        UnfreezeWrapper wrapper = new UnfreezeWrapper(name_, symbol_, owner_, originalNftAddr_);
//        _setRecord(nodeName_, owner_, address(wrapper));
//        originalNftAddr[IERC4907(wrapper)] = originalNftAddr_;
//        wrapperNftAddr[originalNftAddr_] = IERC4907(wrapper);
//        wrapperAdminRole[wrapperContractAddr_] = _createPartnerRole(partnerName_);
    }
...
abstract contract Registry is IContractRegistry {
    ...
    modifier onlyRecordOwner(string memory nodeName_) {
        if (records[keccak256(abi.encode(nodeName_))].owner != msg.sender) {
