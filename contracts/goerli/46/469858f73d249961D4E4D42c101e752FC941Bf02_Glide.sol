...
contract Glide is Ownable {
    ...
    uint256 public rerollEndTimestamp;
    ...
    function setRerollCosts(uint256 one, uint256 three, uint256 five) external onlyOwner {
        rerollOneCost = one;
        rerollThreeCost = three;
        rerollFiveCost = five;
    }

    function setRerollEndTimestamp(uint256 timestamp) external onlyOwner {
        rerollEndTimestamp = timestamp;
    }
    ...
    function _rerollUsed(uint256 tokenId, uint256 day) internal view returns (bool) {
        return _dna[tokenId] & (_BITMASK_FREE_REROLL_START << day) != 0;
    }

    function _setRerollUsed(uint256 tokenId, uint256 day) internal {
        _dna[tokenId] = _dna[tokenId] | (_BITMASK_FREE_REROLL_START << day);
    }
    ...
    function freeReroll(uint256 tokenId, uint256 traitId) external {
        if (rerollEndTimestamp == 0 || block.timestamp > rerollEndTimestamp) revert RerollEnded();
        // 1 day = 24 * 60 * 60 = 86400 seconds;
        uint256 day = block.timestamp / 86400;
        if (freeRerollStartDay == 0 || day < freeRerollStartDay) revert FreeRerollNotStarted();
        day = day - freeRerollStartDay;
        if (day >= FREE_REROLLS) revert FreeRerollEnded();

        if (_rerollUsed(tokenId, day)) revert FreeRerollUsed();
        _setRerollUsed(tokenId, day);
        _rerollTrait(tokenId, traitId, 3);
    }
    ...
    function rerollOne(
        ...
        _transferETH(msg.sender, msg.value - rerollOneCost);
    }
    ...
    function rerollThree(
        ...
        _transferETH(msg.sender, msg.value - rerollThreeCost);
    }
    ...
    function rerollFive(
        ...
        _transferETH(msg.sender, msg.value - rerollFiveCost);
    }
    ...
    function _rerollTrait(
        ...
        if (rerollTraitId > 3 || rerollTraitId == 0) revert TraitNotRerollable();
    ...
    error FreeRerollEnded();
    error FreeRerollNotStarted();
    error FreeRerollUsed();
