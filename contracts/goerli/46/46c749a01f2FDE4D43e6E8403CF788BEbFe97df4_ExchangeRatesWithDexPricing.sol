...
contract ExchangeRates is Owned, MixinSystemSettings {
    ...
    function _getRateAndTimestampAtRound(
        bytes32 currencyKey,
        uint256 roundId
    ) internal view returns (uint256 rate, uint256 time) {
        // short circuit sUSD
        if (currencyKey == sUSD) {
    ...
    function _effectiveValueAndRates(
        ...
        uint256 sourceAmount,
        bytes32 destinationCurrencyKey
    ) internal view returns (uint256 value, uint256 sourceRate, uint256 destinationRate) {
    ...
    function _rateIsStaleAtRound(bytes32 currencyKey, uint256 roundId, uint256 _rateStalePeriod) internal view returns (bool) {
        // sUSD is a special case and is never stale (check before an SLOAD of getRateAndUpdatedTime)
        if (currencyKey == sUSD) {
            return false;
        }
...
contract ExchangeRatesWithDexPricing is ExchangeRates {
    ...
    function effectiveAtomicValueAndRates(
        ...
        external
        view
        returns (uint256 value, uint256 systemValue, uint256 systemSourceRate, uint256 systemDestinationRate)
    {
