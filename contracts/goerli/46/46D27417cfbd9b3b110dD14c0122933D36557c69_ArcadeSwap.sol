...
contract ArcadeSwap is AccessControl, ReentrancyGuard {
    ...
    /**
     * @notice The swap fee rate between xArcade <> Arcade
     */
    uint256 public SWAP_FEE_RATE;
     ...
     */
    function swapArcadeToXArcade(uint256 _amount) public payable nonReentrant {
        uint256 arcadeSupply = ARCADE_CONTRACT.totalSupply() - _amount;
        uint256 xArcadeSupply = X_ARCADE_CONTRACT.totalSupply() + _amount;
        require(
            arcadeSupply + xArcadeSupply <= MAX_SUPPLY,
            "Swap cannot exceed Max Supply of the tokens"
        );
        require(
            ARCADE_CONTRACT.allowance(msg.sender, address(this)) >= _amount,
            "Approve tokens first!"
        );

        uint256 swapFee = (_amount * SWAP_FEE_RATE) / FEE_BASIS_POINT;

        uint256 taxedSwap = _amount - swapFee;

        // Transfer tokens to be burned to swap contract
        ARCADE_CONTRACT.safeTransferFrom(msg.sender, address(this), taxedSwap);
        ...
        X_ARCADE_CONTRACT.transferFrom(address(this), msg.sender, taxedSwap);

        // Forwards swap fee to swap fee receiver
        forwardSwapFees(address(ARCADE_CONTRACT), swapFee);
     ...
     * @param _amount The amount of Arcade tokens to swap for xArcade
     */
    function swapXArcadeToArcade(uint256 _amount) public payable nonReentrant {
        uint256 arcadeSupply = ARCADE_CONTRACT.totalSupply() + _amount;
        uint256 xArcadeSupply = X_ARCADE_CONTRACT.totalSupply() - _amount;
        require(
            arcadeSupply + xArcadeSupply <= MAX_SUPPLY,
        ...
        require(
            X_ARCADE_CONTRACT.allowance(msg.sender, address(this)) >= _amount,
            "Approve tokens first!"
        );
        ...
        // Transfer tokens to be burned to swap contract
        X_ARCADE_CONTRACT.safeTransferFrom(msg.sender, address(this), taxedSwap);
        ...
        // Transfer tokens minted to user from swap contract
        ARCADE_CONTRACT.transferFrom(address(this), msg.sender, taxedSwap);

        // Forwards swap fee to swap fee receiver
        forwardSwapFees(address(X_ARCADE_CONTRACT), swapFee);

        emit SwapXArcadeToArcade(msg.sender, taxedSwap, swapFee);
    }

    /**
     * @notice Forward swap fees to swap fee receiver
     *      Called by swapArcadeToXArcade() and swapXArcadeToArcade()
     *
     * @param _feeToken address of the token to transfer
     * @param _fee amount of fees to forward
     */
    function forwardSwapFees(address _feeToken, uint256 _fee) internal {
        if (_feeToken == address(ARCADE_CONTRACT)) {
            ARCADE_CONTRACT.safeTransferFrom(
                msg.sender,
                SWAP_FEE_RECEIVER,
                _fee
            );
        ...
        if (_feeToken == address(X_ARCADE_CONTRACT)) {
            X_ARCADE_CONTRACT.safeTransferFrom(
                msg.sender, 
                SWAP_FEE_RECEIVER, 
                _fee
            );
    ...
    /**
     * @notice Update the swap fee rate for the contract
     *
     * @dev Throws on the following restriction errors:
     ...
     */
    function updateSwapFee(uint256 _newFee)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        SWAP_FEE_RATE = _newFee;
     ...
     */
    function updateFeeReceiver(address _newReceiver)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     */
    function updateArcadeContract(address _arcadeToken)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ARCADE_CONTRACT = IERC20(_arcadeToken);
     ...
     */
    function updateXArcadeContract(address _xArcadeToken)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        X_ARCADE_CONTRACT = IERC20(_xArcadeToken);
