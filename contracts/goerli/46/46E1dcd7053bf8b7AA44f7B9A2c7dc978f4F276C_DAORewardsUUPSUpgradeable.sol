...
import "./DAORewards.sol";
import "./interfaces/IDAOV1RewardsUUPSUpgradeable.sol";

contract DAORewardsUUPSUpgradeable is IDAOV1RewardsUUPSUpgradeable, DAORewards, UUPSUpgradeable {
    bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");

    function initializeUUPS(
        RewardsConfig memory cfg,
        address roleAdmin,
        address upgrader
    ) public initializer {
        __Rewards_init(cfg, roleAdmin);
    ...
    function _authorizeUpgrade(address) internal view override {
        require(hasRole(UPGRADER_ROLE, msg.sender), "Rewards: caller does not have upgrader role");
    }
}
...
contract DAORewards is IDAOV1Rewards, Initializable, AccessControlUpgradeable {
    ...
    IDAOV1Staking public daoStaking;
    ...
    function initialize(RewardsConfig memory cfg, address roleAdmin) public initializer {
        __Rewards_init(cfg, roleAdmin);
    }

    function __Rewards_init(RewardsConfig memory cfg, address roleAdmin) internal onlyInitializing {
        __Rewards_init_unchained(cfg, roleAdmin);
    }

    function __Rewards_init_unchained(RewardsConfig memory cfg, address roleAdmin) internal onlyInitializing {
        require(cfg.token != address(0), "DAORewards: reward token must not be 0x0");
        require(cfg.daoStaking != address(0), "DAORewards: daoStaking address must not be 0x0");

        _grantRole(DEFAULT_ADMIN_ROLE, roleAdmin);

        rewardToken = IERC20Upgradeable(cfg.token);
        daoStaking = IDAOV1Staking(cfg.daoStaking);
    }
    ...
    function registerUserAction(address user) public {
        require(msg.sender == address(daoStaking), "DAORewards: only callable by daoStaking");
    ...
    function claim() public returns (uint256){
        ...
        require(amount > 0, "DAORewards: nothing to claim");
    ...
    function setupPullToken(address source, uint256 startTs, uint256 endTs, uint256 amount) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "DAORewards: caller is not the owner");
        require(!disabled, "DAORewards: contract is disabled");

        if (pullFeature.source != address(0)) {
            require(source == address(0), "DAORewards: contract is already set up, source must be 0x0");
            disabled = true;
        } else {
            require(source != address(0), "DAORewards: contract is not setup, source must be != 0x0");
        }

        if (source == address(0)) {
            require(startTs == 0, "DAORewards: disable contract: startTs must be 0");
            require(endTs == 0, "DAORewards: disable contract: endTs must be 0");
            require(amount == 0, "DAORewards: disable contract: amount must be 0");
        } else {
            require(endTs > startTs, "DAORewards: setup contract: endTs must be greater than startTs");
            require(amount > 0, "DAORewards: setup contract: amount must be greater than 0");
        }
    ...
    function setDAOStaking(address _daoStaking) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "DAORewards: caller is not the owner");
        require(_daoStaking != address(0), 'DAORewards: daoStaking address must not be 0x0');

        daoStaking = IDAOV1Staking(_daoStaking);
    }
