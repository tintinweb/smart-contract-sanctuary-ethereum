...
contract UnisTest is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private constant _name = "UnisTest";
    string private constant _symbol = unicode"UnisTest";
    ...
    uint256 private constant _tTotal = 14022023 * 10**9;
    ...
    address payable private _developmentAddress = payable(0x1699A84995b22ae3C98e3506B2961A7bc4c4E1e8);
    address payable private _marketingAddress = payable(0x1699A84995b22ae3C98e3506B2961A7bc4c4E1e8);
    ...
    uint256 public _maxTxAmount = 284440 * 10**9;
    uint256 public _maxWalletSize = 284440 * 10**9;
    ...
    mapping(address => uint256) internal profitPerStakePrevious;
    event Staked(address indexed user, uint256 amount, uint256 timestamp);
    modifier Mstacked(address _address) {
        require(tradingOpen == true, "STAKE: This account cannot stake until trading is enabled");
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            // check tax in contract without include poolstake
            uint256 contractTokenBalance = balanceOf(address(this)) - poolStake;
    ...
    function manualswap() external {
        ...
        uint256 contractBalance = balanceOf(address(this)) - poolStake;
    ...
    function depositPoolStake(uint256 _amount) external onlyOwner {
        bool isSuccess = transfer(address(this), _amount);
        if(isSuccess) {
            poolStake+=_amount;
        }
    }
    ...
    function stake(uint256 _amount) public {
      ...
      require(_amount <= balanceOf(_msgSender()), "STAKE: Cannot stake more than you own");
        _stake(_amount, _msgSender());
        _tokenTransfer(_msgSender(), address(this), _amount, false);
    }
    ...
    function _calculateProfit (address _address) private view returns (uint256) {
        ...
        return profit + profitPerStakePrevious[_address];
    }

    // havest profit
    function havestStake() external Mstacked(_msgSender()) {
         uint256 profit = _calculateProfit(_msgSender());
        stakeHolders[holderIndex[_msgSender()]].time = block.timestamp;
        bool isSuccess = token.transfer(_msgSender(), profit);
        if(isSuccess) {
            poolStake-=profit;
        }
    }
    ...
    function profitStakePerUser(address _address) external view returns(uint256) {
        return _calculateProfit(_address);
    }
}