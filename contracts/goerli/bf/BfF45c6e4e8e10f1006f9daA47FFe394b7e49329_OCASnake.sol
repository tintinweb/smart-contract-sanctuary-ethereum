...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    mapping(uint256 => bytes32) public snakenames;
    mapping(bytes32 => bool) public isTaken;

    bool[65535] public tokenSnakeIndexes;
    ...
    function recursiveNameFind(
        ...
        uint256 seed,
        uint256 lastSeed,
        uint256[2] memory snakeNameIndexes
    ) public view returns (string memory) {
        if (
            (snakeNameIndexes[0] == 0 && snakeNameIndexes[1] == 0) ||
            seed != lastSeed
        ) {
            uint256 snakeNameIndex = (generateAppleSeed(tokenId, seed) %
                snakes.length);
            uint256 adjectiveIndex = (generateAppleSeed(tokenId, seed) %
        ...
        uint256 indexCompare = snakeNameIndexes[0] * snakeNameIndexes[1];

        uint256 snakeNameIndexCompare = (generateAppleSeed(index, seed) %
            snakes.length);
        uint256 adjectiveIndexCompare = (generateAppleSeed(index, seed) %
            wackyAdjectives.length);

        uint256 addCompare = snakeNameIndexCompare * adjectiveIndexCompare;

        if (index == 0) {
            return
                this.recursiveNameFind(
                    tokenId,
                    index + 1,
                    seed,
                    seed,
        ...
        //  else if (indexCompare == addCompare && tokenId < index) {
        //     return
        //         this.recursiveNameFind(
        //             tokenId,
        //             index,
        //             seed + randomNumber(0, 1) == 0 ? 420 : 69,
        //             seed,
        //             snakeNameIndexes
        //         );
        // } else if (indexCompare == addCompare && tokenId > index) {
    ...
    function isSet(bytes32 name) public view returns (bool) {
        return (isTaken[name]);
    }

    function isSetTwo(uint256 index) public view returns (bool) {
        return (tokenSnakeIndexes[index]);
    }
    ...
    function getName(uint256 tokenId) private view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    snakenames[generateAppleSeed(tokenId, 0) % currentIndex]
                )
            );
    }

    function registerName(uint256 index) internal {
        //    snakenames[tokenId] = name;
        //    isTaken[name] = true;

        tokenSnakeIndexes[index] = true;
    }
    ...
    function snakeNameSave(uint256 tokenId, bool extraName) private {
        uint256 snakeNameIndex = (generateAppleSeed(tokenId, 0) %
            snakes.length);
        uint256 adjectiveIndex = (generateAppleSeed(tokenId, 69) %
            wackyAdjectives.length);

        // uint256 combineIndex = snakeNameIndex *
        //     adjectiveIndex +
        //     snakeNameIndexes[snakeNameIndex * adjectiveIndex].snakeName ==
        //     0
        //     ? 0
        //     : supremeIndex;

        // snakeNameIndexes[combineIndex] = NameInformation(
        //     snakeNameIndex,
        //     adjectiveIndex,
        //     supremeIndex
        // );
        ...
        if (extraName) {
            uint256 supremeIndex = generateAppleSeed(tokenId, 420) %
                supremeAdjectives.length;
            supreme = supremeAdjectives[supremeIndex];
        }
        ...
        if (isSetTwo(snakeNameIndex * adjectiveIndex))
            return snakeNameSave(tokenId + block.timestamp, true);
        else {
            registerName(snakeNameIndex * adjectiveIndex);
        }
    }
    ...
    function startTheMatrix(
        ...
        if (sale == 1 && !OCASnakeArt.turnOnThePower) {
            ...
            apple = _newapple;
            extraLives = limit;
        } else OCASnakeArt.turnOnThePower = true;
    }
...
contract OCASnakeArt is Ownable, ReentrancyGuard {
