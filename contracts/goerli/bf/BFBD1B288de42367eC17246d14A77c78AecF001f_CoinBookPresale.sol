...
contract CoinBookPresale is ICoinBookPresale, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    PresaleInfo public whitelistSaleInfo;
    PresaleInfo public publicSaleInfo;
    ...
    modifier claimable() {
        require(_presaleStatus() == 5, "Too early too claim");
        require(
    ...
    function initialize(
        ...
        whitelistSaleInfo.maxSpend = _wlMaxSpend;
        ...
        publicSaleInfo.maxSpend = _psMaxSpend;
    ...
    // TO DELETE
    function setSaleAmounts(uint256 _wl, uint256 _ps) external onlyOwner {
        whitelistSaleInfo.saleAmount = _wl;
        publicSaleInfo.saleAmount = _ps;
    }
    // END TO DELETE
    ...
    function contributeInWhitelist() external payable override nonReentrant {
    ...
    function contributeInPublic() external payable override nonReentrant {
    ...
    function claimBook() external override nonReentrant claimable {
        ...
        uint256 refundOwed = wlRefund + psRefund;
        ...
        if (refundOwed > 0) {
            _safeTransferETHWithFallback(msg.sender, refundOwed);
        }
    ...
    function finalizePresales() external override nonReentrant onlyOwner {
        require(_presaleStatus() >= 4, "Public Sale has not ended");
        require(!presaleFinalized, "Presale already finalized");        

        uint256 collectableCORE;
        if (whitelistSaleInfo.raisedAmount > whitelistSaleInfo.target) {
            collectableCORE += whitelistSaleInfo.target;
        } else {
            collectableCORE += whitelistSaleInfo.raisedAmount;
        }
        if (publicSaleInfo.raisedAmount > publicSaleInfo.target) {
            collectableCORE += publicSaleInfo.target;
        } else {
            collectableCORE += publicSaleInfo.raisedAmount;
        }
        _safeTransferETHWithFallback(owner(), collectableCORE);
        ...
        emit PresaleFinalized(t, w, p, collectableCORE, excessBook, block.timestamp);
    }
    ...
    function _getClaimableAmounts(
        ...
        if (user.wlContributed > 0) {
            ...
            if (whitelistSaleInfo.raisedAmount > whitelistSaleInfo.target) {
                ...
                wlRefund = user.wlContributed - ((refundRateWL * user.wlContributed) / A_FACTOR);
            } else {
...
interface ICoinBookPresale {
    ...
    function contributeInWhitelist() external payable;

    function contributeInPublic() external payable;

    function claimBook() external;

    function fundContract() external;

    function finalizePresales() external;
