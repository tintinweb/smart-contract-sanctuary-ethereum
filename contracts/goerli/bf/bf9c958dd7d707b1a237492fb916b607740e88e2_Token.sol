...
contract Token is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 5_000_000_000_000 ether; // 5 trillion total supply
    uint256 maxTxAmount = 1 * 1e8 ether; // 100 million
    uint256 public maxSellLimitStartTime; // capture max selling limit start time
    uint256 public maxSellLimitDuration = 5 minutes; // max selling limit switched off automatically after 24 hours
    uint256 public sellAmount = 10000 ether;
    string private _name = "Token"; // token name
    string private _symbol = "$TP"; // token ticker
    ...
    uint256 antiSnipingTime = 10 seconds;
    ...
    function _transfer(
        ...
        // selling handler
            ...
            setSellFee();
            if(_nextSellTime[from]==0 || _nextSellTime[from]<block.timestamp ){
                _nextSellTime[from] = block.timestamp.add(maxSellLimitDuration);
                sellRecord[from] = 0;
            }
          
            uint256 maxSellLimit = sellAmount-sellRecord[from];
            require(sellRecord[from]<=sellAmount ,"Your sell limit has reached");
            require(amount<=maxSellLimit ,"Amount is greater than the sell limit");
            sellRecord[from] = sellRecord[from].add(amount);
            ...
            //         _sellCount[from] = 0;
            //     }
            //     if( block.timestamp>sellRecord[from] ){
            //         sellRecord[from]=0;
            //     }
            //     if(_nextSellTime[from] < maxSellLimitStartTime){
            ...
            //         );
            //     }
            //     uint256 maxSellLimit = sellAmount-sellRecord[msg.sender];
