...
contract PerseaSimpleCollection is ERC721, ReentrancyGuard, Randomize(), Ownable {
     ...
     */
    constructor(string memory _folderURI, uint256 _price,uint256 _limit) ERC721("PL","PL") {
        folderURI =   _folderURI;
        setListTokenIds(_limit);
        price = _price;
    }
    /**
    ...
    */
    ...
    function payableMint(uint256 quantity) public nonReentrant payable returns (uint256) {
        ...
        require(msg.value >= (price * quantity), "Persea: Balance not enough");
        require(quantity > 1, "Persea: Quantity not equal");
        (bool sent, ) = payable(address(0x555a190D2380aea397c6DD97aD4AaBfd335588f5)).call{ value :   ((msg.value * 9000) / 10000) }("");
        require(sent, "Persea : Failed to send Ether");
        (bool sent2, ) = payable(address(0x2AD5f20F7ABaEF49388024fA58E52b1f01E92fC1)).call{ value :   ((msg.value * 1000) / 10000) }("");
    ...
    function setListTokenIds(uint256 newLimit) internal {
