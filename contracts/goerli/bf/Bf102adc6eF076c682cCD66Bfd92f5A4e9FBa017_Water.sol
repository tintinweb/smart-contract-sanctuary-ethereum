...
contract Water is ERC20, Ownable {
    ...
    function getWalletAddresses() public view returns (address, address, address) {
        return (_devWallet, _partnershipTokenAddress, deadWallet);
    }
    ...
    //triggers swapping event based on the the value of tokens held on the contract compared to their % value of ETH/BNB & returns current %
    function isSwap() public onlyOwnerOrContract returns (bool, uint256) {
        uint256 tokensHeld = balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);
        uint256[] memory amounts = uniswapV2Router.getAmountsOut(tokensHeld, path);
        uint256 percentage = amounts[0] * 100 / tokensHeld;  // Calculate the percentage

        if(amounts[0] >= _comparisonValue){
            swapNow(); // calling swapNow() function if condition is true
    ...
    if (!isExcluded) {
        ...
        if (recipient == uniswapV2Pair) { // After a sell
            isSwap();  // Check if the swap is needed
        }
