...
contract GlobalVerisartManifoldExtension is ICreatorExtensionRoyalties  {
    ...
    event Granted(address indexed account, address indexed creatorCore);
    event Revoked(address indexed account, address indexed creatorCore);

    mapping(address => mapping(address => bool)) private _permissions;

    mapping(address => mapping(uint256 => address payable[])) private _tokenRoyaltyAddresses;
    mapping(address => mapping(uint256 => uint256[])) private _tokenRoyaltyBps;

    mapping(address => address payable[]) private _defaultRoyaltyAddresses;
    mapping(address => uint256[]) private _defaultRoyaltyBps;
    ...
    function grantMinting(address creatorContract, address creator) public {
        require(ERC721Creator(creatorContract).isAdmin(msg.sender), "Must be admin");
        _permissions[creatorContract][creator] = true;
        emit Granted(creator, creatorContract);
    }

    function revokeMinting(address creatorContract, address creator) public {
        require(ERC721Creator(creatorContract).isAdmin(msg.sender), "Must be admin");
        _permissions[creatorContract][creator] = false;
        emit Revoked(creator, creatorContract);
    }

    function mint(address creatorContract, address to, string calldata uri, address payable[] memory royaltyAddresses, uint256[] memory royaltyBps) public {
        ERC721Creator creatorCon = _checkIsGranted(creatorContract);
        uint256 tokenId = creatorCon.mintExtension(to, uri);

        _setTokenRoyalties(creatorContract, tokenId, royaltyAddresses, royaltyBps);
    }

    function mintBatch(address creatorContract, address to, string[] calldata uris, address payable[][] memory royaltyAddressesPerToken, uint256[][] memory royaltyBpsPerToken) public {
        require(royaltyAddressesPerToken.length == royaltyBpsPerToken.length, "Mismatch in array lengths");
        require(royaltyAddressesPerToken.length == 0 || royaltyAddressesPerToken.length == uris.length, "Incorrect royalty array length");
        ERC721Creator creatorCon = _checkIsGranted(creatorContract);
        uint256[] memory tokenIds = creatorCon.mintExtensionBatch(to, uris);

        if (royaltyAddressesPerToken.length != 0) {
            for (uint256 i = 0; i < tokenIds.length; i++) {
                _setTokenRoyalties(creatorContract, tokenIds[i], royaltyAddressesPerToken[i], royaltyBpsPerToken[i]);
            }
        }
    }

    function setTokenRoyalties(address creatorContract, uint256 tokenId, address payable[] memory royaltyAddresses, uint256[] memory royaltyBps) external {
        _checkIsGranted(creatorContract);
        _setTokenRoyalties(creatorContract, tokenId, royaltyAddresses, royaltyBps);
    }

    function setDefaultRoyalties(address creatorContract, address payable[] memory royaltyAddresses, uint256[] memory royaltyBps) external {
        _checkIsGranted(creatorContract);
        _defaultRoyaltyAddresses[creatorContract] = royaltyAddresses;
        _defaultRoyaltyBps[creatorContract] = royaltyBps;
    }
    ...
    function getRoyalties(address creator, uint256 tokenId) external virtual view override returns (address payable[] memory, uint256[] memory) {
        address payable[] memory tokenAddresses = _tokenRoyaltyAddresses[creator][tokenId];
        if (tokenAddresses.length != 0) {
            return (tokenAddresses, _tokenRoyaltyBps[creator][tokenId]);
        }

        return (_defaultRoyaltyAddresses[creator], _defaultRoyaltyBps[creator]);
    }

    function _setTokenRoyalties(address creatorContract, uint256 tokenId, address payable[] memory royaltyAddresses, uint256[] memory royaltyBps) private {
        require(royaltyAddresses.length == royaltyBps.length, "Royalty arrays not same length");
        if (royaltyAddresses.length != 0) {
            _tokenRoyaltyAddresses[creatorContract][tokenId] = royaltyAddresses;
            _tokenRoyaltyBps[creatorContract][tokenId] = royaltyBps;
        }
    }
