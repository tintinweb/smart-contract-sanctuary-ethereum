...
contract HumanCrowdsale is AccessControl, PullPayment, Ownable{
    ...
    uint256 public whitePrice = 0.25 ether;
    uint256 public allowPrice = 0.22 ether;
    ...
    function allowMint(uint256 _amount)
        external
        payable
    ...
    function whiteMint(uint256 _amount) external payable isState(HumanState.WHITE){
        ...
        require(
            msg.value == _amount.mul(whitePrice),
            "HUMANCROWDSALE:Payment declined"
        );
        mAmount = mAmount.add(_amount);
        require(
    ...
    function saleState() external  view returns (HumanState,uint256) {
        if(state == HumanState.ALLOW){
            return (state,allowPrice);
        }else if(state == HumanState.WHITE){
            return (state,whitePrice);
        }
        return (state,0);
    }
