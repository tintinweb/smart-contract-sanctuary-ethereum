...
error AddressHasTier(address account, uint8 tier);
error AddressHasNoTier(address account, uint8 tier);

error AddressSuspended(address _address);
error AddressNotSuspended(address _address);

error AddressRejected(address _address);
error AddressNotRejected(address _address);

error AddressHasItem(address _address, uint256 _itemId);
error AddressHasNoItem(address _address, uint256 _itemId);
...
import "../customErrors/PermissionManagerErrors.sol";
...
contract PermissionManagerV2 is Initializable, AccessControlUpgradeable, PermissionManagerStorage {
     ...
     */
    function assignTier1For(address _account) public zeroAddressCheck(_account) onlyPermissionsAdmin {
        if (hasTier1(_account)) {
            revert AddressHasTier(_account, 1);
        }
     ...
     */
    function revokeTier1For(address _account) public onlyPermissionsAdmin {
        if (!hasTier1(_account)) {
            revert AddressHasNoTier(_account, 1);
        }
     ...
     */
    function assignTier2For(UserProxy calldata _userProxy)
        public
        zeroAddressCheck(_userProxy.user)
        onlyPermissionsAdmin
    {
        if (hasTier2(_userProxy.user)) {
            revert AddressHasTier(_userProxy.user, 2);
        }
        if (hasTier2(_userProxy.proxy)) {
            revert AddressHasTier(_userProxy.proxy, 2);
        }
    ...
    function assignTiers2(UserProxy[] calldata _usersProxies) external {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            assignTier2For(_usersProxies[i]);
        }
    }
     ...
     * @param _userProxy The address of user and proxy.
     */
    function revokeTier2For(UserProxy calldata _userProxy) public onlyPermissionsAdmin {
        if (!hasTier2(_userProxy.user)) {
            revert AddressHasNoTier(_userProxy.user, 2);
        }
        if (!hasTier2(_userProxy.proxy)) {
            revert AddressHasNoTier(_userProxy.proxy, 2);
        }
    ...
    function revokeTiers2(UserProxy[] calldata _usersProxies) external {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            revokeTier2For(_usersProxies[i]);
        }
    }
     ...
     * @param _userProxy The address of user and proxy.
     */
    function suspendUserFor(UserProxy calldata _userProxy)
        public
        zeroAddressCheck(_userProxy.user)
        onlyPermissionsAdmin
    {
        if (isSuspended(_userProxy.user)) {
            revert AddressSuspended(_userProxy.user);
        }
        permissionItems.mint(_userProxy.user, SUSPENDED_ID, 1, "");

        if (_userProxy.proxy != address(0)) {
            if (isSuspended(_userProxy.proxy)) {
                revert AddressSuspended(_userProxy.proxy);
            }
    ...
    function suspendUsers(UserProxy[] calldata _usersProxies) external {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            suspendUserFor(_usersProxies[i]);
        }
    }
     ...
     * @param _userProxy The address of user and proxy.
     */
    function unsuspendUserFor(UserProxy calldata _userProxy) public onlyPermissionsAdmin {
        if (!isSuspended(_userProxy.user)) {
            revert AddressNotSuspended(_userProxy.user);
        }
        permissionItems.burn(_userProxy.user, SUSPENDED_ID, 1);

        if (_userProxy.proxy != address(0)) {
            if (!isSuspended(_userProxy.proxy)) {
                revert AddressNotSuspended(_userProxy.proxy);
            }
    ...
    function unsuspendUsers(UserProxy[] calldata _usersProxies) external onlyPermissionsAdmin {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            unsuspendUserFor(_usersProxies[i]);
        }
    }
     ...
     * @param _userProxy The address of user and proxy.
     */
    function rejectUserFor(UserProxy calldata _userProxy)
        public
        zeroAddressCheck(_userProxy.user)
        onlyPermissionsAdmin
    {
        if (isRejected(_userProxy.user)) {
            revert AddressRejected(_userProxy.user);
        }
        permissionItems.mint(_userProxy.user, REJECTED_ID, 1, "");

        if (_userProxy.proxy != address(0)) {
            if (isRejected(_userProxy.proxy)) {
                revert AddressRejected(_userProxy.proxy);
            }
     ...
     * @param _userProxy The address of user and proxy.
     */
    function unrejectUserFor(UserProxy calldata _userProxy) public onlyPermissionsAdmin {
        if (!isRejected(_userProxy.user)) {
            revert AddressNotRejected(_userProxy.user);
        }
        permissionItems.burn(_userProxy.user, REJECTED_ID, 1);

        if (_userProxy.proxy != address(0)) {
            if (!isRejected(_userProxy.proxy)) {
                revert AddressNotRejected(_userProxy.proxy);
            }
    ...
    function unrejectUsers(UserProxy[] calldata _usersProxies) external {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            unrejectUserFor(_usersProxies[i]);
        }
    }
     ...
     * @param _account The address to assign Tier1.
     */
    function assignItemFor(address _account, uint256 _itemId) public zeroAddressCheck(_account) onlyPermissionsAdmin {
        if (_hasItem(_account, _itemId)) {
            revert AddressHasItem(_account, _itemId);
        }
    ...
    function assignItems(uint256 _itemId, address[] calldata _accounts) external {
        for (uint256 i = 0; i < _accounts.length; i++) {
            assignItemFor(_accounts[i], _itemId);
        }
    }
     ...
     * @param _account The address to assign Tier1.
     */
    function removeItemFor(address _account, uint256 _itemId) public onlyPermissionsAdmin {
        if (!_hasItem(_account, _itemId)) {
            revert AddressHasNoItem(_account, _itemId);
        }
    ...
    function removeItems(uint256 _itemId, address[] calldata _accounts) external {
        for (uint256 i = 0; i < _accounts.length; i++) {
            removeItemFor(_accounts[i], _itemId);
        }
    }
