...
/**
 * @dev this contract is similar to the original OmniHorse contract, with changes to make it
 * interact properly with the new MasterPriceController contract
 */
contract OmnihorsePCT is ERC721A, Ownable, ERC721ABurnable, ERC721AQueryable {
    string private _name;
    string private _symbol;
     ...
     */
    function getPrice(uint256 amount_) internal returns(uint256) {
        require(amount_ > 0, "Cannot mint zero");
        return priceController.getPrice(amount_);
    }
    ...
    //     return priceController.getPrice(user_, 1);
    // }
    ...
    function mint(uint256 amount_) external payable {
        require(amount_ > 0, "Cannot mint zero"); // Should handle single and multiple mints in one function
        require(maxSupply >= totalSupply() + amount_, "Insufficent supply");
        uint256 price = defaultSale ? getDefaultPrice(amount_) : getPrice(amount_);
        require(msg.value >= price, "Insufficent input amount");
        //currentSupply += amount_; // unnecessary, already have totalSupply(). can save gas by not writing 
     ...
     */
    // function mint() external payable forSale {
    //     require(maxSupply >= totalSupply() + 1, "Insufficent supply");
    //     uint256 price = defaultSale ? getDefaultPrice(1) : getPrice(1);
    //     require(msg.value >= price, "Insufficent input amount");
    //     //++currentSupply;
