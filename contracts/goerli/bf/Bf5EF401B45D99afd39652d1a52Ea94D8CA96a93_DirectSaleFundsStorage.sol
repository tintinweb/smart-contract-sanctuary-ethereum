...
contract DirectSaleFundsStorage is IDirectSaleFundsStorage, ActionContext {
    event FundsReceived(address from, uint256 amount);
    event FundsWithdrawn(address to, uint256 amount);
    event ERC20FundsWithdrawn(address to, IERC20 erc20, uint256 amount);
    ...
    function balanceERC20(IERC20 erc20) external view override returns(uint256) {
        return erc20.balanceOf(address(this));
    }

    function withdraw(address payable to_) external override onlyAdminRole {
        require(to_ != address(0), "DirectSaleFundsStorage: Zero address");

        emit FundsWithdrawn(_msgSender(), address(this).balance);
        to_.transfer(address(this).balance);
    }

    function withdrawERC20(address to_, IERC20 erc20_) external override onlyAdminRole {
        require(to_ != address(0), "DirectSaleFundsStorage: Zero address");

        uint256 balance = erc20_.balanceOf(address(this));

        require(erc20_.transfer(to_, balance), "DirectSaleFundsStorage: transfer failed");
        emit ERC20FundsWithdrawn(to_, erc20_, balance);
    }
}
