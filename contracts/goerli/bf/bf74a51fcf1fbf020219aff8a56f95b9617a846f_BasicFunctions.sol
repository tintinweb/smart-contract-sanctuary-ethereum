...
contract BasicFunctions {
    ...
    // pure => anything put in the function will be put and returned into the function and will
    //         not interact with any of the state variables
    ...
    // return a string (use view which interacts with state variable 'name')
    ...
    // function that uses for loops and multiplies params and string comparison
    ...
    function addCoin(string memory _name, string memory _symbol, uint _supply) external {
        // use global 'msg.sender' address
        myCoins[msg.sender] = Coin(_name, _symbol, _supply);
    }

    // function get coin from myCoin mapping
    function getMyCoin() public view returns (Coin memory) { // dealing with struct similar to dealing with a string
