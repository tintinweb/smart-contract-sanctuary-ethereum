...
contract ICO is Token {
    ...
    modifier preSaleActive() {
        require(block.timestamp < startTime + 10000, "PreSale is ended");
        _;
    }
    ...
    /**
     * @dev invest function  adds the lets the investor invest his funds in the ICO
    ...
    function invest() public payable preSaleActive { 
        ...
        uint invested= msg.value;

        for (uint256 i = 0; i < whitelistInvesters.length; i++) {
            if (whitelistInvesters[i] == msg.sender) {
               profit=  invested * foundersProfitRate;
            } else {
                 profit=  invested * crowdSaleProfitRate;
            }
        }
        ...
        emit fundsRecieved(msg.sender, profit);
    }
    ...
    function collectProfit() public {
        ...
        mint(msg.sender, transferableTokens);
