...
contract SXCStake {
    ...
    function stake(uint256 amount1, uint256 timeperiod) public {
        ...
        uint256 SXCVAL = getPriceinUSD();
        uint256 amount = amount1.sub((amount1.mul(totalFee)).div(percentDivider));
        uint256 rewardtokenPrice = (amount.mul(SXCVAL)).div(1e9);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address SXC_WBNB = 0x3C60D4434B09C4267d39b7836475f97cd762eb9c; // SXC_WBNB pancake pool address
        IERC20 SXCTOKEN = IERC20(0x3C60D4434B09C4267d39b7836475f97cd762eb9c); // SXC Token address 
        uint256 WBNBSUPPLYINSXC_WBNB =(WBNBTOKEN.balanceOf(SXC_WBNB));
        uint256 SXCSUPPLYINSXC_WBNB = (SXCTOKEN.balanceOf(SXC_WBNB));
        uint256 SXCUSDVAL = (((WBNBSUPPLYINSXC_WBNB.mul(1e9)).div((SXCSUPPLYINSXC_WBNB))).mul(BNBPRICE)).div(1e18);
        return SXCUSDVAL;
    }
