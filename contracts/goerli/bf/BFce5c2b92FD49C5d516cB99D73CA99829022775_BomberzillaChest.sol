...
contract BomberzillaChest is Ownable {
    ...
    constructor(
        ...
        address _paymentRecipient,  // Any one like us
        IERC20 _paymentToken,   // BUSD
        uint256 _characterChestPrice,  // in BUSD
        uint256 _skinsChestPrice,   // in BUSD
        uint256 _mixChestPrice  // in BUSD
    ) {
        characterNFT = _characterNFT;
        assetsNFT = _assetsNFT;
        paymentRecipient = _paymentRecipient;
        paymentToken = _paymentToken;
        characterChestPrice = _characterChestPrice;
        skinsChestPrice = _skinsChestPrice;
        mixChestPrice = _mixChestPrice;
    }

    function buyCharacterChest(uint256 quantity) external {
        _buyCharacterChest(quantity, msg.sender);
    }

    function buySkinsChest(uint256 quantity) external {
        _buySkinsChest(quantity, msg.sender);
    }

    function buyMixChest(uint256 quantity) external {
        for (uint256 i = 0; i < quantity; i++) {
            if (getRandomNumber(0, 1) == 0) {
                _buyCharacterChest(1, msg.sender);
            } else {
                _buySkinsChest(1, msg.sender);
            }
        }
        emit MixChestBought(msg.sender, quantity);
    }

    function _buyCharacterChest(uint256 quantity, address user) internal {
        paymentToken.safeTransferFrom(user, paymentRecipient, characterChestPrice * quantity);
        characterNFT.mint(user);

        emit CharacterChestBought(user, quantity);
    }

    function _buySkinsChest(uint256 quantity, address user) internal {
        paymentToken.safeTransferFrom(user, paymentRecipient, skinsChestPrice * quantity);
        uint256[] memory tokensIds = new uint256[](quantity);
        uint256[] memory amounts = new uint256[](quantity);
        for (uint256 i = 0; i < quantity; i++) {
            tokensIds[i] = getRandomNumber(0, 9);
            amounts[i] = 1;
        }
        assetsNFT.mint(user, tokensIds, amounts, "");
    ...
    function setCharacterChestPrice(uint256 _price) external onlyOwner {
        emit CharacterChestPriceUpdated(characterChestPrice, _price);
        characterChestPrice = _price;
    }

    function setSkinsChestPrice(uint256 _price) external onlyOwner {
        emit CharacterChestPriceUpdated(skinsChestPrice, _price);
        skinsChestPrice = _price;
    }

    function setMixChestPrice(uint256 _price) external onlyOwner {
    ...
    function getRandomNumber(uint256 min, uint256 max) internal returns (uint256) {
        if (max == 0) return 0;
        return
            (uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender, nonce++))) % max) + min;
    }
}
