...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    modifier isCorrectPayment(uint256 stage, uint256 numberOfTokens) {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            if (stage == 0) {
                payAmount += PRE_MINT_PRICES[i];
            } else if (stage == 1) {
                payAmount += PUBLIC_MINT_PRICES[i];
            }
        }
        require(
            payAmount == msg.value,
        ...
        nonReentrant
    {
        ...
        require(stageTime[stage] <= block.timestamp, "Not available yet. Please wait for more...");
    ...
    function updateScheduledTime(uint256[] calldata _stageTimes) onlyOwner external {
        require(_stageTimes[0] < _stageTimes[1], "Please input the stageTimes in valid format");
        require(_stageTimes.length == 2, "Please input two stageTimes, pre-mint & mint stage.");
    ...
    function updateFixedModelStagesInfo(uint256 _premintPrice, uint256 _publicMintPrice) onlyOwner external {
        for (uint256 i = 1; i <= maxPublicMint; i++) {
    ...
    function updateDynamicModelStagesInfo(uint256[] calldata _premintPrices, uint256[] calldata _publicMintPrices) onlyOwner external {
        for (uint256 i = 1; i <= _premintPrices.length; i++) {
            PRE_MINT_PRICES[i] = _premintPrices[i];
        }
        for (uint256 i = 1; i <= _publicMintPrices.length; i++) {
            PUBLIC_MINT_PRICES[i] = _publicMintPrices[i];
        }
    }
}
