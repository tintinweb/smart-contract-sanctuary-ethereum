...
contract MGLTH is ERC721A, Owned {
    ...
    mapping (address => bool) public addressPayer;
    mapping (address => uint) public payedAmount;

    event Broadcast(address indexed injector, uint indexed token, string indexed media);
    event Vandalize(address indexed injector, uint indexed token, string indexed media);
    ...
    function pray() external {
        ...
        require(addressPayer[msg.sender], "You have not sacraficed for this prayer");
        _mint(msg.sender, 1);
        addressPayer[msg.sender] = false;
    }
    ...
    function postPrayer() external onlyOwner {
        uint quantity = totalSupply() / 10;
        _mint(msg.sender, quantity);
    }

    function number() external view returns (uint testNumber){
        uint quantity = testNumber / 10;
        return quantity;
    }
