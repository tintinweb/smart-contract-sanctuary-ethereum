...
/**
 * @title The required interface for collections to support the NFTDropMarket.
...
abstract contract NFTDropMarketExhibition is Context {
  ...
  function _addCollectionToExhibition(address nftContract, uint256 exhibitionId) internal {
    if (exhibitionId != 0) {
      ...
      emit CollectionAddedToExhibition({ nftContract: nftContract, exhibitionId: exhibitionId });
    }
  }
  ...
  NFTDropMarketExhibition
{
  ...
  function _mintFromFixedPriceSale(
    ...
    firstTokenId = INFTDropCollectionMint(nftContract).mintCountTo(uint16(count), sender);
