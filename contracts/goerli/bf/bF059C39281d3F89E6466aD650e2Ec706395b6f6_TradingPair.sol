...
contract TradingPair is PositionLockerBase {
        ...
        positionLocked(positionId)
    {
        ...
        // берем тотал саплай токенов ликвидности
        ...
        uint256 liquidityTokensToMint = (liquidityTokens.totalSupply() *
            assets.ownerAsset.count()) / ownerAsset.count();
    ...
    function _swap(
        ...
        data.k = data.inputlastCount * output.count();
        data.newOutput = data.k / input.count();
        data.buyCount = output.count() - data.newOutput;
