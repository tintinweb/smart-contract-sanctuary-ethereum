...
import {IOptimisticRelayer} from "./interfaces/IOptimisticRelayer.sol";
import {IValidator} from "./interfaces/IValidator.sol";

import {IERC20} from "@openzeppelin/contracts/interfaces/IERC20.sol";

contract OptimisticRelayer is Guarded, IOptimisticRelayer {
    ...
    event DisputeValue(address disputer_, bytes32 encodedId_, uint256 verifiedValue_,uint256 proposedValue_, bool disputeIsValid_);
    ...
    uint256 public constant disputeBlockWindow = 255;

    mapping(bytes32 => Proposal) public proposals;
    mapping(bytes32 => Rate) public rates;

    address public immutable collybus;
    Type public immutable relayerType;

    constructor(address collybus_, Type relayerType_) {
        collybus = collybus_;
        relayerType = relayerType_;
    }
    ...
    function setParam(
        ...
        if (param_ == "deviation") {
            rates[encodedId_].deviation = uint256(value_);
        } else if (param_ == "validator") {
            rates[encodedId_].validator = address(uint160(uint256(value_)));
        } else
    ...
    function register(
        ...
        address validator_,
        uint256 deviation_
    ) external override(IOptimisticRelayer) checkCaller {
        ...
        rates[encodedId_].validator = validator_;
        rates[encodedId_].deviation = deviation_;
    }
        ...
        checkCaller
    {
        ...
        rates[encodedId_].validator = address(0);
    }

    function rateParams(bytes32 encodedId_) external view returns (address) {
        return (rates[encodedId_].validator);
    }
    ...
    function dispute(
        ...
        updateCollybus(encodedId_, rates[encodedId_].deviation, verifiedValue);
        emit DisputeValue(msg.sender, encodedId_, verifiedValue, proposedValue, !isValid );

        return !isValid;
    }

    function getProposal(bytes32 encodedId_)
        external
        view
        override(IOptimisticRelayer)
        returns (
            ...
            uint256 blockNumber_,
            bool inDisputeWindow_,
            bool disputed_
        )
    {
        if (rates[encodedId_].validator == address(0)) {
            revert OptimisticRelayer__proposal_rateNotRegistered(encodedId_);
        }

        uint256 proposalBlockNumber = proposals[encodedId_].blockNumber;
        return (
            proposals[encodedId_].proposedValue,
            proposalBlockNumber,
            proposalBlockNumber + disputeBlockWindow >= block.number,
            proposals[encodedId_].disputed
        );
    }
        ...
        )
    {
        ...
        IValidator validator = IValidator(rates[encodedId_].validator);
        (value_, blockNumber_) = validator.value();

        updatesCollybus_ = checkDeviation(
            getCollybusValue(encodedId_),
            value_,
            rates[encodedId_].deviation
        );
    }
    ...
    function updateCollybus(
        ...
        if (relayerType == IOptimisticRelayer.Type.DiscountRate) {
            return updateDiscountRate(uint256(encodedId_), deviation_, value_);
        } else if (relayerType == IOptimisticRelayer.Type.SpotPrice) {
            return
                updateSpot(
    ...
    function getCollybusValue(bytes32 encodedId_)
        private
        view
        returns (uint256)
    {
...
interface IOptimisticRelayer {
    ...
    function unregister(bytes32 encodedId_) external;
    ...
    function getValidator(bytes32 encodedId_) external returns (address);
}
