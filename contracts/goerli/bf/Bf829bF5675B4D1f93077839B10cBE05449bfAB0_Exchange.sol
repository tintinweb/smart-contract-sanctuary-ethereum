...
contract MixinProtocolSub is Ownable, IProtocolSub {
    ...
    function handleInvokeEchelon(
        ...
        if (primeValue != package.cost * amount) {
            LibRichErrors.rrevert(
    ...
    function _handlePurchase(
        ...
        if (subscription.endTimestamp < block.timestamp) {
            endTimestamp = (package.period * amount) + block.timestamp;
        } else {
            endTimestamp =
    ...
    function _onlyActiveSubscription(address makerAddress, address takerAddress)
        internal
        view
    {
    ...
    MixinTransactions
{
    ...
    // /// @dev The current function will be called in the context of this address.
    // ///      If calling a fill function, this address will represent the taker.
    // ///      If calling a cancel function, this address will represent the maker.
    // /// @return `msg.sender` if entry point is any other function.
...
library LibProtocolSubRichErrors {
    ...
    function InvalidInvokerError(address msgSender, address prime)
        internal
        pure
        returns (bytes memory)
    {
    ...
    function InvalidPrimeAmountError(uint256 givenAmount, uint256 desiredAmount)
        internal
        pure
        returns (bytes memory)
    {
    ...
    function MakerMissingSubError(address makerAddress)
        internal
        pure
        returns (bytes memory)
    {
    ...
    function TakerMissingSubError(address takerAddress)
        internal
        pure
        returns (bytes memory)
    {
