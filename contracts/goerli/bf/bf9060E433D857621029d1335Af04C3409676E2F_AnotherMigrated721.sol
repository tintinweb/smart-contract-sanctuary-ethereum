...
//
/**
 * @title AnotherMigrated721
 * @author Anotherblock Technical Team
 * @notice Anotherblock NFT contract meant to replace the original NFT contract for Drop #1, #2, #3
 **/
...
contract AnotherMigrated721 is ERC721ABv2, ERC721ABErrors {
    //     _____ __        __
    ...
    ///@dev ABMigrator Contract Address
    address private abMigrator;
     ...
     */
    function mint(address _to, uint256 _quantity) external onlyMigrator {
        _mint(_to, _quantity);
    }

    function setABMigrator(address _abMigrator) external onlyOwner {
        abMigrator = _abMigrator;
    }
    ...
    /**
     * @notice
     *  Ensure that the caller is ABMigrator contract
     */
    modifier onlyMigrator() {
        if (msg.sender != abMigrator) revert Forbidden();
        _;
    }
}
