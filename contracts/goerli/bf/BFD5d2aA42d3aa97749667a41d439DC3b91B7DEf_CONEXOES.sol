...
contract CONEXOES is Ownable, ERC721A, Royalty, ERC721AQueryable {
    using Strings for uint256;
    using ECDSA for bytes32;
    // Addresses
    address public sender = 0xA65aae78EdEF916d4102BA7b5672068C0D35fbff;
    address public crossmintAddress = 0x12A80DAEaf8E7D646c4adfc4B107A2f1414E2002;
    address[] private addressWithdraw;
    // Sales configs
    uint256 public maxSupply = 1107380;

    uint256[] public maxSupplyEach = [581625,581625,581625,140750,140750,140750,140750,25000,25000,25000,1500,1500,581625,581625,581625,140750,140750,140750,140750,140750,25000,25000,500,500,581625,581625,581625,140750,140750,140750,140750,25000,25000,25000,25000,500,581600,581600,581600,581600,140750,140750,140750,140750,25000,25000,25000,5000,581625,581625,581625,140750,140750,140750,140750,25000,25000,25000,1500,1500,581625,581625,581625,140750,140750,140750,140750,25000,25000,25000,1500,1500,581625,581625,581625,140750,140750,140750,140750,140750,25000,25000,25000,5000,581625,581625,581625,140750,140750,140750,140750,140750,25000,25000,25000,500,581625,581625,581625,140750,140750,140750,140750,140750,25000,25000,25000,5000,581625,581625,581625,140750,140750,140750,140750,140750,25000,25000,25000,5000,581625,581625,581625,140750,140750,140750,140750,140750,25000,25000,25000,5000];
    ...
    // Metadata uris
    string public baseURI = "https://core-items-images.s3.amazonaws.com/conexao/metadatas/";
    // Amount minted at phases
    mapping(uint256 => uint256) public tokenIdsUris;
    mapping(uint256 => uint256) public currentSupply;
    
    constructor() ERC721A("MUSIGREEN", "MUSIGREEN") {
        _setupDefaultRoyaltyInfo(0xA65aae78EdEF916d4102BA7b5672068C0D35fbff, 200);
    ...
    function mint(address to, uint256 quantity, uint256[] calldata id) private {
       for (uint256 index = 0; index < quantity; index++) {
            require(currentSupply[index] + quantity <= maxSupplyEach[id[index]], "This amount of tokens would surpass the max supply.");
            tokenIdsUris[totalSupply() + index] = id[index];
        }

        _mint(to, quantity);
    }

    function crossmint(address to, address user, uint256 quantity, uint256[] calldata id, bytes calldata signature) external payable  {
    ...
    function giveaway(address to, uint256[] calldata id, uint256 quantity) external onlySender {
        mint(to, quantity, id);
    }
    ...
    function setMaxSupply( uint256[] calldata _maxSupply) external onlyOwner {
        maxSupplyEach = _maxSupply;
    }
    ...
    // Functions to change contract settings
    function withdrawAddress(address[] memory _addresses) external onlyOwner {
        addressWithdraw = _addresses;
    }
    ...
    function checkTokenCurrentSupply() external view returns (uint256[] memory) {
        uint256[] memory totalSupplyEach = new uint[](maxSupplyEach.length);

        for (uint8 i = 0; i < maxSupplyEach.length; i++) 
        {
            totalSupplyEach[i] = currentSupply[i];
        }

        return totalSupplyEach;
    }


    function getSupplyArray() external view returns (uint256[] memory) {
        return maxSupplyEach;
    }

    function withdrawMoney() external onlyOwner {
        uint256 amount1 = address(this).balance / 100 * 5;
