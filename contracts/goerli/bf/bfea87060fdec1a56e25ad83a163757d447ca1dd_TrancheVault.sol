...
contract TrancheVault is ITrancheVault, ERC20Upgradeable, UUPSUpgradeable {
    ...
    function deposit(uint256 amount, address receiver) external returns (uint256) {
        ...
        if (portfolio.status() == Status.Open) {
            token.safeTransferFrom(msg.sender, address(this), amount);
        } else {
            portfolio.updateCheckpoint(waterfallIndex, SafeCast.toInt256(amount));
    ...
    function mint(uint256 shares, address receiver) external returns (uint256) {
        uint256 assetAmount = previewMint(shares);
        require(assetAmount <= maxDeposit(msg.sender), "TrancheVault: Exceeds max mint amount");
    ...
    function maxWithdraw(address owner) public view returns (uint256) {
        ...
        if (portfolio.status() != Status.Closed || _totalAssets == 0) {
            return 0;
        }

        uint256 trancheMaxWithdraw = _convertToAssets(balanceOf(owner), _totalAssets);
    ...
    function maxRedeem(address owner) public view returns (uint256) {
        ...
        uint256 strategyMaxWithdraw = withdrawStrategy.maxWithdraw(owner);
        return _min(trancheMaxRedeem, _convertToShares(strategyMaxWithdraw, _totalAssets));
    }
    ...
    function _requireTrancheStrategyOwner() internal view {
        require(portfolio.hasRole(portfolio.TRANCHE_STRATEGY_OWNER_ROLE(), msg.sender), "TrancheVault: Not a tranche strategy owner");
    }
