...
contract AlphaBet is ERC721, ERC721URIStorage, Ownable, ERC2981 {
    ...
    mapping(string => WordData) mintedWord;
    ...
    function cost(string memory _word) public view returns (uint256) {
        uint256 numberMinted = getNumbersMinted(_word);
        if (numberMinted == 0) return 0;
        return getCurrentCost(_word) + initialCost();
    }
    ...
    function setInitialCost(uint256 _cost) public onlyOwner {
    ...
    function mint(
        string memory _word,
        string memory uri
    ) public payable mintCompliance(_word) {
        uint256 tokenId = _tokenIdCounter.current();
        uint256 mintedCount = getNumbersMinted(_word);
        if (mintedCount == 51) revert ALPHABET_MaxMintLimitReached();
        uint256 actualCost = cost(_word);
        if (msg.sender != owner()) {
        ...
        mintedWord[_word].currentCost = actualCost;
