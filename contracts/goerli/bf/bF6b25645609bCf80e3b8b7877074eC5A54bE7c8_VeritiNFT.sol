...
contract VeritiNFT is ERC721Enumerable, Ownable, Pausable, Initializable {
  ...
  mapping(bytes32 => bool) private allowedKeyIndex;
  ...
  function setAllowedKeys(AllowedKey[] memory _allowedKeys) external onlyVeritiNFTBridge {
    for (uint256 i = 0; i < _allowedKeys.length; i++) {
      if(allowedKeyIndex[_allowedKeys[i].name] == false) {
        allowedKeys.push(_allowedKeys[i]);
        allowedKeyIndex[_allowedKeys[i].name] = true;
      }
    }
  }
  ...
  function tokenURI(uint256 tokenId) public override view returns (string memory) {
    ...
    for (uint256 i = 0; i < arrLength; i++) {
      dataURI = abi.encodePacked(dataURI,
        '"', bytes32ToString(allowedKeys[i].name), '": "', metadataContents[tokenId][allowedKeys[i].name], '",'
      );
    }
...
contract VeritiNFTBridge is Ownable, Initializable {
  ...
  function updateAllowedKeys(bytes calldata _message) private {
    AllowedKey[] memory allowedKeys = abi.decode(_message, (AllowedKey[]));
