...
contract NFT is ERC721A, Ownable {
    ...
    // @dev Wallet association address
    //address payable public _walletAssociation;

    // @dev Association shares
    //uint public associationShares = 20;
    ...
    constructor(address payable team) ERC721A("NFT", "NFT") {
        _team = team;
        //_walletAssociation = association;
        _mint(team, 1);
    }

    function mint(uint256 quantity, bytes32[] calldata merkleProof) external payable isWhitelisted(merkleProof) {
        // `_mint`'s second argument now takes in a `quantity`, not a `tokenId`.
        require(!paused, "Sale is paused !");
        require (quantity + balanceOf(msg.sender) <= maxByWallet, "Request too much for one wallet !");
        require (quantity + totalSupply() <= maxSupply, "Max supply reached !");
        require(msg.value >= price * quantity, "Not enough ETH !");

        //(bool success, ) = _walletAssociation.call{value: msg.value * associationShares / 100 }("");
