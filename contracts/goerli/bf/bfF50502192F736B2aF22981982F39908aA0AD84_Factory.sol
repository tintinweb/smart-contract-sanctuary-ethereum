...
contract Factory {
     ...
     */
    function createAccount(address owner,uint256 salt) public returns (SmartWallet ret) {
        address addr = getAddress(owner, salt);
        uint codeSize = addr.code.length;
        if (codeSize > 0) {
            return SmartWallet(payable(addr));
        }
        ret = SmartWallet(payable(new ERC1967Proxy{salt : bytes32(salt)}(
 ...
 */
abstract contract Base is IAccount {
...
/**
  ...
  */
contract SmartWallet is Base, TokenCallbackHandler, UUPSUpgradeable, Initializable {
