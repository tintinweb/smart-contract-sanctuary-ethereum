...
contract TrashIsekaiBattleItems is ReentrancyGuard, Context {
    ...
    function trashItems(uint256[] calldata weaponIds, uint256[] calldata weaponValues, uint256[] calldata armorIds, uint256[] calldata armorValues) public virtual nonReentrant {
...
contract IsekaiBattleStakeUpgraded is
    Initializable,
    ...
    AccessControlEnumerableUpgradeable
{
    ...
    event StakeDefence(address indexed user, uint256[] tokenId);
    event Unstake(address indexed user, uint64 regionId, uint256[] tokenId);
    event UnstakeDefence(address indexed user, uint256[] tokenId);
    ...
    event ClaimFragment(address indexed user, uint64 regionId, uint256 tokenId);
    ...
    struct RegionInfo {
        uint256 WeaponCountBase;
        uint256 ArmorCountBase;
        uint256 SeedPerBase;
    ...
    IsekaiBattle public ISB;
    IIsekaiBattleSeeds public SED;
    IsekaiBattleArmor public AMR;
    IsekaiBattleWeapon public WPN;
    IsekaiBattleBag public BAG;
    Random public RDM;
    Staking public STK;

    uint256 public maxStakingCharacters;
    uint256 public minGetSeedCharacters;
    uint256 public weaponInfosLength;
    uint256 public armorInfosLength;
    uint256 public statusMastersLength;

    uint256 public upgradeableTest;
    ...
    function initialize(
        ...
        RDM = _RDM;
        STK = _STK;
        maxStakingCharacters = 15;
        minGetSeedCharacters = 3;
        weaponInfosLength = 15;
        armorInfosLength = 9;
        statusMastersLength = 3;
    }

    function setUpgradeableTest(uint256 value) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        upgradeableTest = value;
    }

    function setISB(IsekaiBattle addr) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        ISB = addr;
    }

    function setSED(IIsekaiBattleSeeds addr) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        SED = addr;
    }

    function setAMR(IsekaiBattleArmor addr) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        AMR = addr;
    }

    function setWPN(IsekaiBattleWeapon addr) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        WPN = addr;
    }

    function setBAG(IsekaiBattleBag addr) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        BAG = addr;
    }

    function setRDM(Random addr) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        RDM = addr;
    }

    function setSTK(Staking addr) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        STK = addr;
    }
    ...
    function setWeaponInfosLength(uint256 value) public virtual onlyRole(INFO_SETTER_ROLE) {
        weaponInfosLength = value;
    }

    function setArmorInfosLength(uint256 value) public virtual onlyRole(INFO_SETTER_ROLE) {
        armorInfosLength = value;
    }

    function setStatusMastersLength(uint256 value) public virtual onlyRole(INFO_SETTER_ROLE) {
        statusMastersLength = value;
    }
    ...
    function stake(StakingRequest[] memory _stakingInfo) public virtual {
        ...
        if (stakingInfos[_msgSender()].stakingCharactorCounts > 0) {
            claim(true, true, true);
            for (uint256 i = 0; i < stakingInfos[_msgSender()].stakingRegionIds.length; i++) {
                STK.unstake(stakingTokenIds[_msgSender()][stakingInfos[_msgSender()].stakingRegionIds[i]]);
                emit Unstake(
                    _msgSender(),
        ...
        for (uint256 i = 0; i < _stakingInfo.length; i++) {
            ...
            STK.stake(_msgSender(), _stakingInfo[i].tokenIds);
    ...
    function claim(bool weapon, bool armor, bool seed) public virtual nonReentrant {
        ...
        for (uint256 i = 0; i < stakingInfos[_msgSender()].stakingRegionIds.length; i++) {
            ...
            uint256 ratioIndex = MathUpgradeable.min(tokenIds.length, 5);
            uint256 weaponCount = 0;
            uint256 armorCount = 0;
            if (weapon) {
                (weaponCount, armorCount, , , , ) = getEstimate();
                if (weaponCount > 0) {
                    uint256[] memory detailsCount = new uint256[](weaponInfosLength);
                    for (uint256 j = 0; j < weaponCount; j++) {
                        uint256 randomNumber = RDM.getRandomNumber() % 1000000;
                        uint256 ratioCount = 0;
                        for (uint256 k = 0; k < weaponInfosLength; k++) {
                            uint256 level = k % 3;
                            if (level == 0) {
                                ratioCount += (targetRegionInfo.WeaponDetails[level].initRatio -
                                    (targetRegionInfo.WeaponDetails[level].unitRatio * ratioIndex));
                            } else {
                                ratioCount += (targetRegionInfo.WeaponDetails[level].initRatio +
                                    (targetRegionInfo.WeaponDetails[level].unitRatio * ratioIndex));
                            }
                    ...
                    for (uint256 j = 0; j < detailsCount.length; j++) {
                        if (detailsCount[j] > 0) {
                            WPN.mint(_msgSender(), j, detailsCount[j], '');
                        }
                    }
                    if (origin.wpnClaim == actionCounts[_msgSender()].wpnClaim) {
                        actionCounts[_msgSender()].wpnClaim++;
                    }
                    if (origin.claimPlus == actionCounts[_msgSender()].claimPlus) {
            ...
            if (armor) {
                if (!weapon) {
                    (, armorCount, , , , ) = getEstimate();
                }
                if (armorCount > 0) {
                    uint256[] memory detailsCount = new uint256[](armorInfosLength);
                    for (uint256 j = 0; j < armorCount; j++) {
                        uint256 randomNumber = RDM.getRandomNumber() % 1000000;
                        uint256 ratioCount = 0;
                        for (uint256 k = 0; k < armorInfosLength; k++) {
                            uint256 level = k % 3;
                            if (level == 0) {
                                ratioCount += (targetRegionInfo.ArmorDetails[level].initRatio -
                                    (targetRegionInfo.ArmorDetails[level].unitRatio * ratioIndex));
                            } else {
                                ratioCount += (targetRegionInfo.ArmorDetails[level].initRatio +
                                    (targetRegionInfo.ArmorDetails[level].unitRatio * ratioIndex));
                            }
                    ...
                    for (uint256 j = 0; j < detailsCount.length; j++) {
                        if (detailsCount[j] > 0) {
                            AMR.mint(_msgSender(), j, detailsCount[j], '');
                        }
                    }
            ...
            if (seed) {
                uint256 seedPer = 0;
                if (ratioIndex >= minGetSeedCharacters) {
                    uint256 factor = ratioIndex - minGetSeedCharacters + 1;
                    seedPer = (elapsed > targetRegionInfo.SeedInterval) ? targetRegionInfo.SeedPerBase * factor : 0;
                }
                if (ratioIndex >= minGetSeedCharacters && seedPer > 0) {
                    for (uint256 j = 0; j < tokenIds.length; j++) {
                        uint256 random = RDM.getRandomNumber() % 1000000;
                        if (random < seedPer) {
                            SED.mint(_msgSender(), (random % statusMastersLength) * 10, 1, '');
                            ...
                            emit ClaimSeed(_msgSender(), regionId, (random % statusMastersLength) * 10);
                        }
                    }
                }
            }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < stakingInfos[_msgSender()].stakingRegionIds.length; i++) {
            ...
            if (!targetRegionInfo.active) continue;
            uint256 weaponRatio = targetRegionInfo.WeaponCountBase + (250 * stakingCharactorCount);
            weaponCount += (elapsed * weaponRatio) / targetRegionInfo.WeaponInterval / 10000;
            uint256 armorRatio = targetRegionInfo.ArmorCountBase + (250 * stakingCharactorCount);
            armorCount += (elapsed * armorRatio) / targetRegionInfo.ArmorInterval / 10000;
            if (
...
contract Staking is IERC721Receiver, Context, AccessControlEnumerable {
    ...
    function stake(address user, uint256[] memory tokenIds) public virtual {
...
contract IsekaiBattleStakeUpgradeable is
    Initializable,
    ReentrancyGuardUpgradeable,
    ContextUpgradeable,
    AccessControlEnumerableUpgradeable
