...
contract testMap is ERC721A, ReentrancyGuard, Ownable, DefaultOperatorFilterer {
    ...
    mapping(address => mapping(uint => uint)) public mintedClasses;
    uint[] public itemClasses;
    uint[] public classCounter;
    uint classMIN = 0;
    uint classMAX = 3;
    ...
    function addItems( address _to, uint _count, uint256 _start, uint _class ) internal {
	uint _a = mintedClasses[_to][_class];
	mintedClasses[_to][_class] = _a + _count;
	uint _b = classCounter[_class];
	classCounter[_class] = _b + _count;
	for (uint256 i = _start; i < _start + _count; i++) {
	    itemClasses[i] = _class;
	}
    }
