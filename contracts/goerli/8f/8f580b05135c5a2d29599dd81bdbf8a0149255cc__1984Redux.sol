...
contract _1984Redux is ERC721A, Ownable {
    ...
    uint256 public constant MINT_PRICE = 0.001 ether; // In production: 0.1984
    ...
    function setProvenanceHash(string calldata _provenanceHash) external lockable onlyOwner {
        if (bytes(PROVENANCE_HASH).length != 0) revert();

        PROVENANCE_HASH = _provenanceHash;
    }

    function setOffset() external lockable onlyOwner {
        if (bytes(PROVENANCE_HASH).length == 0) revert();
        if (_offset != 0) revert();

        // In production: Use Chainlink VRF
        _offset = uint256(keccak256(
