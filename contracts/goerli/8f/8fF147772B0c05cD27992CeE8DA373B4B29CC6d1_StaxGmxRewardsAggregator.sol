...
// STAX (investments/gmx/StaxGmxRewardsAggregator.sol)
...
/// @title STAX GMX/GLP Rewards Aggregator
/// @notice Manages the collation and selection of GMX.io rewards sources to the correct STAX investment vault.
/// ie the STAX GMX vault and the STAX GLP vault
/// @dev This implements the IStaxInvestmentManager interface -- the STAX GMX/GLP Rewards Distributor 
/// calls to harvest aggregated rewards.
contract StaxGmxRewardsAggregator is IStaxInvestmentManager, Ownable {
    using SafeERC20 for IERC20;

    /// @notice The STAX contract managing the holdings of staked GMX derived rewards
    ...
    /// @notice The STAX contract managing the holdings of staked GLP derived rewards
    ...
    /// @notice If true, this investment manager only harvests the rewards sourced from staked GLP.
    /// If false, this investment manager only harvests the rewards sourced from staked GMX, esGMX, multiplier points
    bool public immutable forStakedGlpRewards;
    ...
    constructor(address _gmxManager, address _glpManager, bool _forStakedGlpRewards) {       
        gmxManager = IStaxGmxManager(_gmxManager);
        glpManager = IStaxGmxManager(_glpManager);
        forStakedGlpRewards = _forStakedGlpRewards;
        rewardTokens = _forStakedGlpRewards 
    ...
    function harvestableRewards() external view override returns (uint256[] memory amounts) {
        ...
        // Add any balances which have already been harvested and sitting in this aggregator
    ...
    /// @notice Harvest any STAX claimable rewards distributable to users from the glpManager and gmxManager.
    ...
    function harvestRewards() external override returns (uint256[] memory amounts) {
        ...
        // The GLP Vault harvests rewards for staked GLP (and derived GMX/esGMX/mult points) which are sent to the GLP and GMX 
        // rewards aggregators accordingly
        // The GMX Vault harvests for staked GMX and derived esGMX/mult points.
        if (forStakedGlpRewards) {
          glpManager.harvestRewards();  
        } else {
          gmxManager.harvestRewards();
        }

        // Given rewards may be harvested async and sent to this contract,
        // transfer any accrued balance of the reward token.
        IERC20 rewardToken;
        uint256 amount;
