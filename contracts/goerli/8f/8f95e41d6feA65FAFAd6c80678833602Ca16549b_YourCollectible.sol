...
contract YourCollectible is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
  ...
  // Set a `name` and `symbol` that your collectible will use via OpenZeppelin's ERC721.sol
  ...
  function mintItem(address to, string memory tokenURI) public returns (uint256) {
    ...
    // _mint and _setTokenURI come from ERC721
    ...
    // used to store an item’s metadata
    // tokenURI is a string (hash) that points to a JSON file with the item’s metadata in ipfs
    _setTokenURI(id, tokenURI);

    return id;
  }

  // ### Metadata extension functions (implemented in ERC721) ###
  // If set, the resulting URI for each token will be the concatenation of the baseURI and the tokenId
  // should return an HTTP or IPFS URL.
  ...
  // and override supportsInterface to check for the additional interface id that will be supported.
  // See https://docs.openzeppelin.com/contracts/4.x/api/utils#ERC165
