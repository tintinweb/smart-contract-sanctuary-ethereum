    ...
    HatsAccessControl
{
    ...
    function initialize(
        ...
        dao = IBaal(initData.membershipCriteria);

        // reference sharesToken of Baal
        sharesToken = Token(dao.sharesToken());
        ...
        if (
            initData.topHatId != 0 &&
            HATS.isWearerOfHat(address(dao), initData.topHatId)
        ) {
            topHat = initData.topHatId;
            bytes memory data2 = _encodeCreateHatProposal();
    ...
    /**
     * @dev Encoding functions for Baal proposal submission
     */
    ...
    function _encodeMintHatProposal(address _deployer)
        internal
        view
        returns (bytes memory)
    {
    ...
    function _submitBaalProposal(bytes memory multiSendMetaTx) internal {
        uint256 proposalOffering = dao.proposalOffering();
        require(msg.value == proposalOffering, "Missing tribute");

        dao.submitProposal{value: proposalOffering}(
            multiSendMetaTx,
            0,
            0,
