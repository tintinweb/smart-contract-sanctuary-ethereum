...
contract RoyaltyVaultForPassHolder is AccessControl, Pausable, ReentrancyGuard{
  ...
  // Review: Done
  function batchWithdraw(address[] memory addressList) external payable onlyRole(WITHDRAWER_ROLE) nonReentrant whenNotPaused{
  ...
  // Review: Done
  error InvalidWallet();
  function sweep(address[] memory addressList) external payable onlyRole(WITHDRAWER_ROLE) nonReentrant whenPaused{
