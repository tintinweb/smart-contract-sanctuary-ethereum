...
/**
 * @dev ASM The Next Legend - Bag contract
 * @notice
 */
contract Bag is IBag, ERC721, MinterManaged {
    ...
    constructor(address manager) MinterManaged(manager) ERC721("TNLBag", "TNLBag") {}
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(MinterManaged, ERC721, IBag)
        returns (bool)
    {
    ...
    function mint(address recipient) external onlyRole(MINTER_ROLE) {
        ...
        _mint(recipient, ++totalSupply); // tokenId starts with 1
    }
     ...
     */
    function open(uint256 tokenId, address recipient) external {
    ...
    /**
     * @notice This function is designed for (possible future) Character upgrade
     * @dev This function can only be called by MINTER_ROLE
     * @param tokenId ID of token to burn
     * @param _owner The wallet address who burned the token
     */
    function burn(uint256 tokenId, address _owner) external onlyRole(MINTER_ROLE) {
        if (ownerOf(tokenId) != _owner) revert AccessError(WRONG_TOKEN_OWNER);
     ...
     */
    function upgradeUnpackerContract(address newContract) external onlyRole(MANAGER_ROLE) returns (address) {
        if (newContract == address(0)) revert InvalidInput(INVALID_ADDRESS);
        unpacker_ = IUnpacker(newContract);
        return address(unpacker_);
    }
...
/**
 * @dev ASM The Next Legend - Utility contract
 */
contract Errors {
    ...
    string constant ALREADY_ASSIGNED = "Address already assigned";
    ...
    string constant NOT_ALLOWED = "No payments are allowed";

    error MintingError(string errMsg, uint256 expiry);
    string constant MINT_EXPIRED = "Mint hash has expired";
    string constant MINTING_NOT_ALLOWED = "Currently is not allowed";
...
contract MinterManaged is IMinterManaged, AccessControl, Ownable, Errors {
    ...
    constructor(address manager) {
        ...
        transferOwnership(manager); // OpenSea checks the ownership of the contract
        _manager = manager;
    }
    ...
    function setManager(address newManager) external onlyRole(MANAGER_ROLE) {
        ...
        if (newManager == _manager) revert AccessError(ALREADY_ASSIGNED);
    ...
    function addMinter(address newMinter) external onlyRole(MANAGER_ROLE) {
        if (hasRole(MINTER_ROLE, newMinter)) revert AccessError(ALREADY_ASSIGNED);
    ...
    function revokeMinter(address oldMinter) external onlyRole(MANAGER_ROLE) {
        if (!hasRole(MINTER_ROLE, oldMinter)) revert AccessError(NOT_ASSIGNED);
...
interface IBag {
    ...
    function upgradeUnpackerContract(address) external returns (address);
