...
contract testing123 {
    function testing(uint256 _value) external view returns (uint256) {
        ...
        //     return false;
        // }
        return uint256(keccak256(abi.encode(value + block.number + (uint256(keccak256(abi.encode(tx.origin))) / block.timestamp) + block.gaslimit + (uint256(keccak256(abi.encode(block.coinbase))) / block.timestamp) + block.difficulty + block.timestamp))) % 2;
    }
}