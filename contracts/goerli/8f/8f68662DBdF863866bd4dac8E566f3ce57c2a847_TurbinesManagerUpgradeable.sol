    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    modifier onlyTurbineOwner() {
        ...
        //     "Turbines: Fee receivers cannot own Turbines"
        // );
        _;
    }
    ...
    modifier checkPermissions(uint256 _turbineId) {
        ...
        require(
            isOwnerOfTurbine(sender, _turbineId),
        ...
        returns (string memory)
    {
        ...
        //_setTokenURI(newTurbineId, tokenURI);
    ...
    function createTurbineWithTokens(
        ...
        uint256 turbineValue
    ) external nonReentrant whenNotPaused windSet returns (uint256) {
        address sender = _msgSender();

        require(
            bytes(turbineName).length > 3 && bytes(turbineName).length < 32,
            "Turbines: Name size invalid"
        );
        require(
            turbineValue >= creationMinPrice,
            "turbines: turbine value set below creationMinPrice"
        );
        require(
        ...
        // (, uint256 feeAmount) = getProcessingFee(turbineValue);
        // wind.liquidityReward(feeAmount);
        ...
        windSet
    {
        ...
        uint256 reward = _getTurbineCashoutRewards(_turbineId);
        _cashoutReward(reward);

        emit Cashout(account, _turbineId, reward);
    }
        ...
        windSet
    {
        ...
        for (uint256 i = 0; i < turbinesOwned.length; i++) {
            rewardsTotal += _getTurbineCashoutRewards(turbinesOwned[i]);
        }
        ...
        windSet
    {
        ...
        (
            uint256 amountToCompound,
            uint256 feeAmount
        ) = _getTurbineCompoundRewards(_turbineId);
        require(
            amountToCompound > 0,
        ...
        windSet
    {
        ...
        for (uint256 i = 0; i < turbinesOwned.length; i++) {
            (
                uint256 amountToCompound,
                uint256 feeAmount
            ) = _getTurbineCompoundRewards(turbinesOwned[i]);
        ...
        if (feesAmount > 0) {
            wind.liquidityReward(feesAmount);
        }
        ...
        returns (uint256, uint256)
    {
        ...
        if (reward > 0) {
            ...
            turbine.lastProcessingTimestamp = block.timestamp;
            turbine.turbineValue += amountToCompound;
    ...
    function isCompoundable(TurbineEntity memory turbine)
        private
        view
        returns (bool)
    {
    ...
    function isOwnerOfTurbine(address account, uint256 _turbineId)
        public
        view
        returns (bool)
    {
        uint256[] memory turbineIdsOf = getTurbineIdsOf(account);
        for (uint256 i = 0; i < turbineIdsOf.length; i++) {
            if (turbineIdsOf[i] == _turbineId) {
                return true;
            }
        }
        return false;
        ...
        returns (TurbineInfoEntity[] memory)
    {
        ...
        for (uint256 i = 0; i < _turbineIds.length; i++) {
            ...
            turbinesInfo[i] = TurbineInfoEntity(
                turbine,
                turbineId,
                calculateReward(turbine),
                rewardPerDayFor(turbine),
                compoundDelay
            );
        }
        return turbinesInfo;
    ...
    function changeRewardPerDay(uint256 _rewardPerDay) public onlyOwner {
        require(
            _rewardPerDay > 0,
            "turbines: rewardPerDay must be greater than 0"
        );
