...
contract Vault is Initializable, UUPSUpgradeable {
  ...
  modifier clearQueue() {
    if (block.timestamp < RoundExpiry(round) + ROUND_PERIOD)
      require(AllowInteraction == true, "Disabled during auction.");
  ...
  function _withdraw() internal returns (uint256) {
    ...
    if (
      ...
      removeFromUserDeposit(msg.sender);
      vaultTotalClaimableWithdrawals -= withdrawAmt;
  ...
  function settlementComplete() external {
    ...
    assetScaleX1e36[round] =
      (assetScaleX1e36[round] * _currentAssets) /
      totalAsset[round];
    vaultTotalClaimableWithdrawals += totalWithdraw[round + 1];
