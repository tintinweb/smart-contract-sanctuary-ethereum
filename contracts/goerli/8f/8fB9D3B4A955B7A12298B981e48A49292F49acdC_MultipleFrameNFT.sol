...
contract MultipleFrameNFT is ERC1155, AccessControl {
    ...
    address public serverAddress = 0x842C0236236BbDaC052CC2FCC943C6cb9A0e167a; //Server Wallet Address Finalize Auction.
	 ...
	 */
    function addItem( uint256 supply, uint256 royalty, string memory _uri, address sender, uint256 lazyTokenId, address marketAddress) public returns (uint256) {
        require(hasRole(MINTER_ROLE, sender), "Only minter can add item");
        ...
        _mint(sender, currentID, supply, "Mint");
        if (lazyTokenId > 0)_setApprovalWithCustom(sender, marketAddress, true);
        Items[currentID] = Item(currentID, sender, _uri, supply, royalty);
        emit MultiItemCreated(currentID, _uri, supply, sender, royalty);
        return currentID;
    }
