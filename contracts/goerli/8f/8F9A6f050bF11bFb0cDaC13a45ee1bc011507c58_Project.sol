    ...
    ERC2771ContextUpgradeable
{
    ...
    function updateProjectHash(
        ...
        if (_data.nonce != hashChangeNonce++) {
    ...
    function lendToProject(uint256 _cost) external nonReentrant {
        ...
        if (!senderIsBuilder && _sender != homeFi.communityContract()) {
            revert NeitherBuilderNorCommunity();
        }

        // Revert if try to lend 0
        if (_cost == 0) {
            revert IncorrectCost();
        }

        // Revert if try to lend more than project cost
        uint256 _newTotalLent = totalLent + _cost;
        if (projectCost < _newTotalLent) {
            revert TotalLentGreaterThanCost();
        }
        ...
        // Allocate funds to tasks and mark them as allocated
        allocateFunds(50);
    }
        ...
        external
    {
        ...
        if (_sender != builder && _sender != contractor) {
            revert NeitherBuilderNorContractor();
        }

        // Revert if taskList array length not equal to scList array length.
        uint256 _length = _taskList.length;
        if (_length != _scList.length) {
            revert LengthMismatch();
        }
    ...
    function recoverTokens(address _tokenAddress) external {
        ...
        if (_tokenAddress == address(currency)) {
            ...
            for (uint256 _taskID = 1; _taskID <= _length; ) {
                if (tasks[_taskID].getState() != TaskStatus.Complete) {
                    revert NotComplete();
                }
    ...
    function changeOrder(
        ...
        // Revert if task is already completed
        if (task.state == TaskStatus.Complete) {
            revert Inactive();
        }

        // Revert if decoded changeOrderNonce is incorrect. This indicates wrong data.
        if (_data.changeOrderNonce != task.changeOrderNonce++) {
            revert ChangeOrderNonceIncorrect();
        }

        // ensure change order makes changes
        if (task.cost == _data.newCost && task.subcontractor == _data.newSC) {
            revert Unchanged();
        }
        ...
        if (_data.newCost != _taskCost) {
            ...
            if (getAlerts(_data.taskID)[uint256(Lifecycle.TaskAllocated)]) {
                ...
                if (_data.newCost < _taskCost) {
                    ...
                    totalAllocated = totalAllocated - _withdrawDifference;
                    ...
                    totalLent = totalLent - _withdrawDifference;
                ...
                // If new cost is more than task cost and totalLent is not enough.
                    ...
                    totalAllocated = totalAllocated - _taskCost;
        ...
        if (_data.newSC != task.subcontractor) {
            ...
            if (_data.newSC != address(0)) {
                ...
                _inviteSC(_data.taskID, _data.newSC);
                emit SingleSCInvited(_data.taskID, _data.newSC);
            }
    ...
    function raiseDispute(
        ...
        // validate actionType
        if (_data.actionType == 0) {
            revert InvalidDispute();
        }
        if (_data.actionType >= 4) {
            revert InvalidDispute();
        }
        ...
        if (_data.taskID == 0) {
            ...
            if (_signer != builder && _signer != contractor) {
                revert NeitherBuilderNorContractor();
            }
        } else {
            ...
            if (
                ...
                revert NeitherBuilderNorContractorNorSubContractor();
            }
            ...
            if (_signer == tasks[_data.taskID].subcontractor) {
                ...
                if (
                    ...
                    revert NotConfirmedBySubContractor();
                }
            }
        }
    ...
    {
        cost = tasks[_taskId].cost;
        subcontractor = tasks[_taskId].subcontractor;
        state = tasks[_taskId].state;
        changeOrderNonce = tasks[_taskId].changeOrderNonce;
    }
    ...
    function allocateFunds(uint256 _maxLoop) public {
        ...
        uint256[] memory _tasksAllocated = new uint256[](
            _taskCount - j + _changeOrderedTaskLength - i
        );
        ...
        if (_changeOrderedTaskLength != 0) {
            ...
            for (; i < _changeOrderedTaskLength; ) {
                // Local instance. To save gas.
                uint256 _changeOrderedTaskI = _changeOrderedTask[i];
                uint256 _taskCost = tasks[_changeOrderedTaskI].cost;
                ...
                // If there are enough funds to allocate this task
                if (_allocationBudget >= _taskCost) {
                    // Reduce task cost from _allocationBudget
                    unchecked {
                        _allocationBudget -= _taskCost;
                    }

                    // Mark the task as allocated
                    tasks[_changeOrderedTaskI].fundTask();

                    // Add task to _tasksAllocated array
                    _tasksAllocated[_loopCount] = _changeOrderedTaskI;
        ...
        totalAllocated = _totalLent - _allocationBudget;
    }
     ...
     */
    function _inviteSC(uint256 _taskID, address _sc) internal {
        ...
        internal
    {
        ...
        // When there is a contractor
            ...
            if (contractorDelegated) {
                ...
                _checkSignatureValidity(_contractor, _hash, _signature, 0);
            }
            // When builder has not delegated rights to contractor
                ...
                if (_builder != _contractor) {
                    _checkSignatureValidity(_contractor, _hash, _signature, 1);
                }
            }
        }
    }
    ...
    function _checkSignatureTask(
        ...
        // When there is a contractor
            ...
            // When builder has not delegated rights to contractor
                ...
                if (_builder != _sc && _contractor != _sc) {
                    // if hash is approved by the builder and builder is not SC
    ...
    function _checkValidAmount(uint256 _amount) internal pure {
        ...
        if (((_amount / PPT_DIVISOR) * PPT_DIVISOR) != _amount) {
            revert IncorrectPrecision();
        }
    }
}
