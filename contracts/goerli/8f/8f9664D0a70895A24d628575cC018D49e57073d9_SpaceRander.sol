...
contract SpaceRander is DesiderNft {
    ...
    constructor(address _owner) ERC721("Space Render", "SR")
    {
    ...
    function freeMint(uint256 tokenQuantity, bytes32[] calldata merkleProof) external onlyOwner {
        require(
            totalSupply() + tokenQuantity <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        ...
        _mintWithProof(tokenQuantity, merkleProof, freeMerkleRoot);
    }


    function payMint(uint256 tokenQuantity, bytes32[] calldata merkleProof) external payable {
        require(
            totalSupply() + tokenQuantity <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
