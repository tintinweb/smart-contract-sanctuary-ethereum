...
contract DssProxyActions is Common {
    ...
    function chargeFeeBorrow(Info memory info, address jug, uint repayAmount, address USBJoin) internal returns (uint feeBorrow) {
        (, , uint debt) = VatLike(info.vat).urns(info.ilk, info.urn);
        uint fee = JugLike(jug).getFeeBorrow(info.ilk);
        if(debt > 0 && fee > 0) {
            uint debtAmount = repayAmount <= debt ? repayAmount : debt;
            feeBorrow = debtAmount * fee / decimal;
            address treasury = JugLike(jug).treasury();
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, treasury, feeBorrow);
        }
    }
    ...
    function wipe(
        ...
        if (own == address(this) || ManagerLike(manager).cdpCan(own, cdp, address(this)) == 1) {
            ...
            USBJoin_join(info.ilk, USBJoin, info.urn, wad);
            // Paybacks debt to the CDP
            frob(manager, cdp, 0, _getWipeDart(info.vat, VatLike(info.vat).USB(info.urn), info.urn, info.ilk));
        } else {
            // Joins USB amount into the vat
            USBJoin_join(info.ilk, USBJoin, address(this), wad);
            // Paybacks debt to the CDP
            VatLike(info.vat).frob(
                ...
                address(this),
                0,
                _getWipeDart(info.vat, wad * RAY, info.urn, info.ilk)
            );
        }
        uint feeBorrow = chargeFeeBorrow (info, jug, wad, USBJoin);
        VatLike(info.vat).subDebt(info.ilk, info.urn, wad);

        emit Repay(msg.sender, cdp, wad, feeBorrow);
    }
    ...
    function wipeAll(
        ...
        if (own == address(this) || ManagerLike(manager).cdpCan(own, cdp, address(this)) == 1) {
            // Joins USB amount into the vat
            USBJoin_join(info.ilk, USBJoin, info.urn, _getWipeAllWad(info.vat, info.urn, info.urn, info.ilk));
            // Paybacks debt to the CDP
            frob(manager, cdp, 0, -int(art));
        } else {
            // Joins USB amount into the vat
            USBJoin_join(info.ilk, USBJoin, address(this), _getWipeAllWad(info.vat, address(this), info.urn, info.ilk));
            // Paybacks debt to the CDP
            VatLike(info.vat).frob(
                info.ilk,
                info.urn,
        ...
        uint feeBorrow = chargeFeeBorrow (info, jug, debt, USBJoin);
        VatLike(info.vat).subDebt(info.ilk, info.urn, debt);

        emit Repay(msg.sender, cdp, art, feeBorrow); 
    }
    ...
    function wipeAndFreeETH(
        ...
        Info memory info = getInfo(manager, cdp);
        // Joins USB amount into the vat
        USBJoin_join(info.ilk, USBJoin, info.urn, wadD);
        ...
        uint feeBorrow = chargeFeeBorrow (info, jug, wadD, USBJoin); 
        VatLike(info.vat).subDebt(info.ilk, info.urn, wadD);
        ...
        emit Repay(msg.sender, cdp, wadD, feeBorrow);
