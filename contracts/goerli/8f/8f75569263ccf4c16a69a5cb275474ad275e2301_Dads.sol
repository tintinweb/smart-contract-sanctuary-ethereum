...
contract Dads {
  ...
  //events
  event BannerMinted(uint256 id, address owner, bytes32 companyName, bytes32 websiteUrl, uint256 subscriptionFee, uint256 subscriptionPeriod);
  event BannerUpdated(uint256 id, uint256 subscriptionFee, uint256 subscriptionPeriod, bytes32 subscriberImageUrl);
  event SubscriptionCreated(uint256 bannerId, address subscriber, bytes32 subscriberImageUrl, uint256 subscriptionTimestamp);
  ...
  function mintBanner(bytes32 companyName, bytes32 websiteUrl, uint256 subscriptionFee, uint256 subscriptionPeriod) payable external {
    ...
    banners[id] = Banner(id, msg.sender, address(0x0), bytes32(0x0), companyName, websiteUrl, subscriptionFee,subscriptionPeriod, block.timestamp);
    owners[msg.sender] = id;
    //emit event
    emit BannerMinted(id, msg.sender, companyName, websiteUrl, subscriptionFee, subscriptionPeriod);
  }

  function updateSubscriberImageUrl(uint256 id, bytes32 subscriberImageUrl) external{
  ...
  function updateSubscriptionFee(uint256 id, uint256 subscriptionFee) external {
    ...
    //emit event
    emit BannerUpdated(id, subscriptionFee, 0, bytes32(0x0));
  }
  ...
  function updateSubscriptionPeriod(uint256 id, uint256 subscriptionPeriod) external {
    ...
    //emit event
    emit BannerUpdated(id, 0, subscriptionPeriod, bytes32(0x0));
  }

  function subscribe(uint256 id, bytes32 subscriberImageUrl) payable external {
  ...
  function cancelSubscription(uint256 id) external {
    ...
    banners[id].subscriberImageUrl=bytes32(0x0);
