...
contract DssSpellAction is DssAction {
    ...
    function actions() public override {
        ...
        vat.rely(LINE_MOM);
        AuthLike_2(AUTOLINE).rely(LINE_MOM);

        // 2. Owner is Pause Proxy - just a sanity check
        require(LineMomLike_1(LINE_MOM).owner() == PAUSE_PROXY);

        // 3. File the AutoLine
        LineMomLike_1(LINE_MOM).file("autoLine", AUTOLINE);

        // 4. Authority is Chief
        LineMomLike_1(LINE_MOM).setAuthority(CHIEF);

        // 5. Add PSM ilks
        LineMomLike_1(LINE_MOM).addIlk("PSM-USDC-A");
        LineMomLike_1(LINE_MOM).addIlk("PSM-PAX-A");
        LineMomLike_1(LINE_MOM).addIlk("PSM-GUSD-A");
