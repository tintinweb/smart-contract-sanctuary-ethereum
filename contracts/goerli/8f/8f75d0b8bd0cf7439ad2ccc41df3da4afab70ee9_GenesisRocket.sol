...
contract GenesisRocket is ERC721A, ERC721AQueryable, IERC721ABurnable,ERC2771Context, OwnableRoles, Pausable, ReentrancyGuard {
    ...
    function setNewTokenURI2(uint256 typeOfURI, uint256 tokenId, string calldata newURI) public onlyAuthorized  {
        ...
        function setNewTokenURI3 (uint256 typeOfURI, uint256 tokenId, string calldata newURI) external onlyRolesOrOwner(ADMIN_ROLE){
         // Avoid unnecessary approvals for the authorized contract
        if(typeOfURI == 0) 
            normalUri = newURI;
        else 
   ...
   function tokenURI(uint256 tokenId) public view virtual override(ERC721A, IERC721A) returns (string memory) {
    ...
    function ownerMint(address to, uint256 quantity) external onlyMinter {
        ...
        function Mintermint2(address to, uint256 quantity) external onlyMinter {
        require(totalSupply() + quantity <= _maxSupply, "Quantity exceeds supply");
        _safeMint(to, quantity);
    }
