...
contract VEXT is ERC20, Pausable, ERC20Permit, ERC20Votes, IVEXT {
    /* ========== STATE VARIABLES ========== */
    ...
    /**
     @dev this modifier calls the Access contract. Access will revert
    ...
    constructor(address _accessAddress, address _systemPauseAddress)
        ERC20("Veloce", "VEXT")
        ERC20Permit("VeloceGovernanceToken")
    {
    ...
    function mint(address to, uint256 amount)
        ...
        onlyExecutive
    {
        if (_notZero(amount)) _increaseTotalAccounts(to);
        ...
        onlyExecutive
    {
        ...
        if (_notZero(amount)) _decreaseTotalAccounts(from);
    }

    /**
     * @dev function to pause contract only callable by admin
     */
    function pauseContract() external virtual override onlyAdmin {
        _pause();
    }
     ...
     */
    function unpauseContract() external virtual override onlyAdmin {
        _unpause();
    }
        ...
        returns (bool)
    {
        ...
        if (_notZero(amount)) _decreaseTotalAccounts(msg.sender);
        return true;
    }
    ...
    function _decreaseTotalAccounts(address _account) internal {
        if (_account != address(0) && balanceOf(_account) == uint256(0)) {
            require(totalAccounts > 0, "VEXT: totalAccounts is 0");
            unchecked {
                --totalAccounts;
            }
        }
    ...
    /**
    @dev internal function which checks that the amount is not Zero
    @param _amount. The amount to check 
    Returns true if the amount is not equal to zero
     */

    function _notZero(uint256 _amount) internal pure returns (bool notZero) {
        return (_amount != 0);
    }
