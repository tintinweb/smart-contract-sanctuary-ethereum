...
contract SomaStarter is ReentrancyGuard, TokenRecoveryUpgradeable {
    ...
    modifier onlyWhitelist() {
        address sender = _msgSender();
        require(
            hasRole(GLOBAL_WHITELIST_ROLE, sender) || hasRole(LOCAL_WHITELIST_ROLE, sender),
            "SomaStarter: ONLY_WHITELIST"
        );
        _;
    }
    ...
    /**
     * @notice Emitted when pool privileges are updated.
     * @param poolIndex The index of the pool.
     * @param newRequiredPrivileges The new required privileges of the pool.
     */
    event PoolPrivilegesChanged(uint256 poolIndex, bytes32 newRequiredPrivileges);
     ...
     */
    function createSimplePool(Props memory props, uint256 paymentLimit, bytes32 requiredPrivileges, address owner_)
        external
        onlyAdmin
        returns (bool success, uint256 poolIndex)
    {
        poolIndex = _createSimplePool(props, paymentLimit, owner_, Type.SIMPLE).index;
        _updateRequiredPrivileges(poolIndex, requiredPrivileges);
    ...
    function createIntervalPool(
        ...
        _updateRequiredPrivileges(pool.index, requiredPrivileges);
        ...
        for (uint256 i = 0; i < intervalsCount; i++) {
            ...
            require(
                startingTimestamp > lastIntervalStartingTimestamp, "SomaStarter: INVALID_INTERVAL_STARTING_TIMESTAMP"
            );
     ...
     * @custom:requirement The message sender's account must have the required privileges of the pool.
     ...
     * @return success Boolean indicating if the transaction was successful.
     */
    function changeLimit(uint256 poolIndex, uint256 limitIndex, uint256 newLimit)
        external
        onlyWhitelist
     ...
     * @return success Boolean indicating if the transaction was successful.
     */
    function setAccountsLimit(uint256 poolIndex, uint256 limitIndex, address[] memory accounts)
        external
        onlyWhitelist
        returns (bool success)
    {
    ...
    /**
     * @notice Updates the required privileges of the pool.
     * @param poolIndex The index of the pool.
     * @param newRequiredPrivileges The new required privileges of the pool.
     * @custom:emits PoolPrivilegesChanged
     * @custom:requirement The function caller must have the GLOBAL_ADMIN_ROLE or LOCAL_ADMIN_ROLE.
     * @return success Boolean indicating if the transaction was successful.
     */
    function updateRequiredPrivileges(uint256 poolIndex, bytes32 newRequiredPrivileges)
        external
        onlyAdmin
        returns (bool success)
    {
        _updateRequiredPrivileges(poolIndex, newRequiredPrivileges);
        return true;
    }
    ...
    function _calculateSwapAmounts(Pool storage pool, uint256 requestedPaymentAmount, address account)
        private
        view
        returns (uint256 paymentAmount, uint256 issuanceAmount)
    {
    ...
    function _createSimplePool(Props memory props, uint256 paymentLimit, address owner_, Type type_)
        private
    ...
    function _updateRequiredPrivileges(uint256 poolIndex, bytes32 newRequiredPrivileges) private {
        emit PoolPrivilegesChanged(poolIndex, newRequiredPrivileges);

        Pool storage pool = _getPool(poolIndex);
        pool.state.requiredPrivileges = newRequiredPrivileges;
    }
}
