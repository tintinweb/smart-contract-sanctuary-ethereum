...
contract Meta is ERC721A, Ownable {
    ...
    function publicMint(uint256 quantity) 
        ...
        payable 
    {
        require(totalSupply() + quantity <= maxSupply, "No more NFTs");
        uint256 cost = getPrice();

        uint256 maxPerWalletLocal = maxPerWallet;
        if (cost == 0) {
            maxPerWalletLocal = 2;
        }
        require(
            _numberMinted(msg.sender) + quantity <= maxPerWalletLocal,
            "Max per wallet"
        );
