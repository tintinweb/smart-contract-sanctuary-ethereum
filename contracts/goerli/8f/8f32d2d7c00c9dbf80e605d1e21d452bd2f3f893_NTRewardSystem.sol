...
contract NTRewardSystem is Ownable {
    using SafeMath for uint256;

    //errors
    error RecipientIsNotACitizen();
    error P2PDisabled();
    error VaultIsNotActive();
    ...
    uint256 public vaultTax = 5;

    // mapps the specific amounts to the appropriate Reward
    ...
    // directly to the receipient | peer to peer
    function rewardBytesPeerToPeer(address _recipient, uint256 _amount)
        external
    {
        if (vaultActive) revert P2PDisabled();
        if (
    ...
    function emitReward(address _recipient, uint256 _amount) private {
        if (
    ...
    function rewardBytes(address _recipient, uint256 _amount) external {
        if (!vaultActive) revert VaultIsNotActive();
        if (
            citizenContract.balanceOf(_recipient) == 0 &&
            outerCitizenContract.balanceOf(_recipient) == 0
        ) revert RecipientIsNotACitizen();
        ...
        uint256 executedVaultTax = balance.mul(vaultTax).div(100);
    ...
    function whichReward(uint256 _amount) external view returns(string memory){
                if (
            _calculateReward[1] - 1 < _amount && _amount < _calculateReward[2]
        ) {
            return "Bronze";
