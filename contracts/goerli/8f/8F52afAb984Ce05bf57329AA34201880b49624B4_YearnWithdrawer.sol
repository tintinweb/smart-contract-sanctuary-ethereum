...
import { IWithdrawer } from "../interfaces/IWithdrawer.sol";
...
contract YearnWithdrawer is YearnHelper, IWithdrawer {
    ...
    RouterComponent public override getComponentId =
        RouterComponent.YEARN_WITHDRAWER;
    ...
    {
        return _withdraw(amountIn, lpToken, false, task);
    }
    ...
    {
        return
            _withdraw(task.balances.getBalance(lpToken), lpToken, true, task);
    }
    ...
    function _withdraw(
        ...
        address adapter = findYearnAdapter(lpToken, task);
        address underlying = IYVault(adapter).token();

        amountOut = estimateWithdraw(adapter, amountIn);
        amountOut = task.amountOutWithSlippage(amountOut);

        task.balances.subBalance(lpToken, amountIn);
        task.balances.addBalance(underlying, amountOut);

        MultiCall memory call = isAll
            ? YearnV2_Multicaller(adapter).withdraw()
            : YearnV2_Multicaller(adapter).withdraw(amountIn);

        task.calls = task.calls.append(call);
        task.gasUsage += gasUsageByAdapterAndTokens(
            adapter,
            lpToken,
            underlying
        );
    ...
    /// @dev Withdraws all YEARN LP tokens to underlying one
    ...
    {
        address[] memory yearnAdapters = filterYearnAdapters(task.adapters); // F:[YPF-3]
        uint256 len = yearnAdapters.length; // F:[YPF-3]
        MultiCall[] memory calls = new MultiCall[](len); // F:[YPF-3]
        uint256 callsFound;

        for (uint256 i; i < len; ) {
            address adapter = yearnAdapters[i]; // F:[YPF-3]
            address yVault = IAdapter(adapter).targetContract(); // F:[YPF-3]

            uint256 yIndex = task.balances.getIndex(yVault); // F:[YPF-3]
            uint256 balance = task.balances[yIndex].balance; // F:[YPF-3]

            if (balance > 1) {
                task.balances[yIndex].balance = 1; // F:[YPF-3]
                address token = IYVault(yVault).token();

                task.balances.addBalance(
                    token,
                    task.amountOutWithSlippage(
                        estimateWithdraw(yVault, balance)
                    )
                ); // F:[YPF-3]

                calls[callsFound] = MultiCall({
                    target: adapter,
        ...
        task.calls = task.calls.concat(calls.copy(callsFound));

        return task;
    }

    function estimateWithdraw(address yVault, uint256 amount)
        public
        view
        returns (uint256)
    {
        return
            (amount * IYVault(yVault).pricePerShare()) /
            (10**IYVault(yVault).decimals()); // F:[YPF-2]
    }
}
