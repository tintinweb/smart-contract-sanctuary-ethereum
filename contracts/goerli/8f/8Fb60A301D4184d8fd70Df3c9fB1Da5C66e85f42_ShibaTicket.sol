...
contract ShibaTicket is VRFV2WrapperConsumerBase, ReentrancyGuard, Ownable, Pausable {
    ...
    uint public freeplaycount;
    uint256 internal balance;
    uint internal ticketprice = 132052;
    ...
    function resetContract() public whenPaused onlyOwner {
        ticketcount = 0;
        lastRequestId = 0;
        lotteryId = 1;
        freeplaycount = 0;
    ...
    function priceupdate(uint256 _ticketprice) public onlyOwner {
        ticketprice = _ticketprice;
    }

    // When called it returns the current ticket price
    function showTicketPrice() public view returns (uint256) {
    ...
    function winnerByLottery(uint256 lottery)
        public
        view
    ...
    function pickWinner() public whenPaused onlyOwner {
        requestRandomWords();
    }

    // Only the owner can call this function which will send remaining ERC20
    // balance to the owner
    function finalizeFunds(IERC20 token) public whenPaused onlyOwner {
    ...
    function payWinner(IERC20 token) public whenPaused onlyOwner {
        // Calculates 10 percent of the pot to be given to the winner
        require(
            lastRequestId > 0,
            "Must have a source of randomness before paying a winner"
        );
