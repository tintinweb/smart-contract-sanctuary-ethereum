...
contract Bank is ReentrancyGuard, Ownable {
    ...
    function claim(
        ...
        uint256 phaseId = _phaseId.current();
        require(
            _validateClaim(phaseId, tokenId_, fraction_, r, s, v),
            "Indicia Bank: Invalid fraction"
        );
        Pool memory pool = pools[phaseId];
    ...
    function _validateClaim(
        ...
        bytes32 typeHash = keccak256(
            "Fraction(uint256 phaseId,uint256 tokenId,uint96 fraction)"
        );
        bytes32 digest = keccak256(
            abi.encode(typeHash, phaseId_, tokenId_, fraction_)
        );
        return
