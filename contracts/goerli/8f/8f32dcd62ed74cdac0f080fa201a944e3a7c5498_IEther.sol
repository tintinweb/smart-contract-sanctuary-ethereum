...
interface IrontrollerInterface {
    /**
     * @notice Marker function used for light validation when updating the irontroller of a market
     * @dev Implementations should simply return true.
     * @return true
     */
    function isIrontroller() external view returns (bool);

    /*** Assets You Are In ***/

    function enterMarkets(address[] calldata cTokens) external returns (uint[] memory);
    function exitMarket(address iToken) external returns (uint);

    /*** Policy Hooks ***/

    function mintAllowed(address iToken, address minter, uint mintAmount) external returns (uint);
    function mintVerify(address iToken, address minter, uint mintAmount, uint mintTokens) external;

    function redeemAllowed(address iToken, address redeemer, uint redeemTokens) external returns (uint);
    function redeemVerify(address iToken, address redeemer, uint redeemAmount, uint redeemTokens) external;

    function borrowAllowed(address iToken, address borrower, uint borrowAmount) external returns (uint);
    function borrowVerify(address iToken, address borrower, uint borrowAmount) external;
    ...
    function transferAllowed(address iToken, address src, address dst, uint transferTokens) external returns (uint);
    function transferVerify(address iToken, address src, address dst, uint transferTokens) external;
...
contract IrontrollerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        IRONTROLLER_MISMATCH,
...
contract TokenErrorReporter {
    enum Error {
        ...
        UNAUTHORIZED,
        BAD_INPUT,
        IRONTROLLER_REJECTION,
        IRONTROLLER_CALCULATION_ERROR,
    ...
    enum FailureInfo {
        ...
        BORROW_IRONTROLLER_REJECTION,
        LIQUIDATE_ACCRUE_BORROW_INTEREST_FAILED,
        LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED,
        LIQUIDATE_COLLATERAL_FRESHNESS_CHECK,
        LIQUIDATE_IRONTROLLER_REJECTION,
        LIQUIDATE_IRONTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED,
        ...
        LIQUIDATE_SEIZE_IRONTROLLER_REJECTION,
        LIQUIDATE_SEIZE_LIQUIDATOR_IS_BORROWER,
        LIQUIDATE_SEIZE_TOO_MUCH,
        MINT_ACCRUE_INTEREST_FAILED,
        MINT_IRONTROLLER_REJECTION,
        ...
        REPAY_BORROW_IRONTROLLER_REJECTION,
        ...
        TRANSFER_IRONTROLLER_REJECTION,
...
interface InterestRateModel {
    /**
      ...
      * @param cash The total cash of the underlying asset in the IToken
      * @param borrows The total borrows of the underlying asset in the IToken
      * @param reserves The total reserves of the underlying asset in the IToken
...
contract IToken is EIP20Interface, Exponential, TokenErrorReporter, ReentrancyGuard {
    /**
     * @notice Indicator that this is a IToken contract (for inspection)
     */
    bool public constant isIToken = true;
    ...
    /**
     * @notice Contract which oversees inter-IToken operations
     */
    IrontrollerInterface public irontroller;
    ...
    /**
     * @notice Event emitted when irontroller is changed
     */
    event NewIrontroller(IrontrollerInterface oldIrontroller, IrontrollerInterface newIrontroller);
     ...
     * @param irontroller_ The address of the Irontroller
     ...
     */
    constructor(IrontrollerInterface irontroller_,
        ...
        // Set the irontroller
        uint err = _setIrontroller(irontroller_);
        require(err == uint(Error.NO_ERROR), "Setting irontroller failed");

        // Initialize block number and borrow index (block number mocks depend on irontroller being set)
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint allowed = irontroller.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
            return failOpaque(Error.IRONTROLLER_REJECTION, FailureInfo.TRANSFER_IRONTROLLER_REJECTION, allowed);
        }
        ...
        irontroller.transferVerify(address(this), src, dst, tokens);
     ...
     * @dev This is used by irontroller to more efficiently perform liquidity checks.
    ...
    /**
     * @notice Returns the current per-block borrow interest rate for this IToken
    ...
    /**
     * @notice Returns the current per-block supply interest rate for this IToken
    ...
    /**
     * @notice Calculates the exchange rate from the underlying to the IToken
    ...
    /**
     * @notice Get cash balance of this IToken in the underlying asset
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint) {
        ...
        uint allowed = irontroller.mintAllowed(address(this), minter, mintAmount);
        if (allowed != 0) {
            return failOpaque(Error.IRONTROLLER_REJECTION, FailureInfo.MINT_IRONTROLLER_REJECTION, allowed);
        }
         ...
         *  Note: The IToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the IToken holds an additional mintAmount of cash.
        ...
        irontroller.mintVerify(address(this), minter, mintAmount, vars.mintTokens);
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        uint allowed = irontroller.redeemAllowed(address(this), redeemer, vars.redeemTokens);
        if (allowed != 0) {
            return failOpaque(Error.IRONTROLLER_REJECTION, FailureInfo.REDEEM_IRONTROLLER_REJECTION, allowed);
        }
         ...
         *  Note: The IToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the IToken has redeemAmount less of cash.
        ...
        /* We call the defense hook */
        irontroller.redeemVerify(address(this), redeemer, vars.redeemAmount, vars.redeemTokens);
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        uint allowed = irontroller.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
            return failOpaque(Error.IRONTROLLER_REJECTION, FailureInfo.BORROW_IRONTROLLER_REJECTION, allowed);
        }
         ...
         *  Note: The iToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the iToken borrowAmount less of cash.
         *  If doTransferOut fails despite the fact we checked pre-conditions,
         *   we revert because we can't be sure if side effects occurred.
         */
        ...
        /* We call the defense hook */
        irontroller.borrowVerify(address(this), borrower, borrowAmount);
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint) {
        ...
        uint allowed = irontroller.repayBorrowAllowed(address(this), payer, borrower, repayAmount);
        if (allowed != 0) {
            return failOpaque(Error.IRONTROLLER_REJECTION, FailureInfo.REPAY_BORROW_IRONTROLLER_REJECTION, allowed);
        }
         ...
         *  Note: The iToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the iToken holds an additional repayAmount of cash.
         *  If doTransferIn fails despite the fact we checked pre-conditions,
         *   we revert because we can't be sure if side effects occurred.
         */
        ...
        /* We call the defense hook */
        irontroller.repayBorrowVerify(address(this), payer, borrower, vars.repayAmount, vars.borrowerIndex);
     ...
     * @param borrower The borrower of this iToken to be liquidated
     * @param cTokenCollateral The market in which to seize collateral from the borrower
     * @param repayAmount The amount of the underlying borrowed asset to repay
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, IToken cTokenCollateral) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, IToken cTokenCollateral) internal returns (uint) {
        /* Fail if liquidate not allowed */
        uint allowed = irontroller.liquidateBorrowAllowed(address(this), address(cTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
            return failOpaque(Error.IRONTROLLER_REJECTION, FailureInfo.LIQUIDATE_IRONTROLLER_REJECTION, allowed);
        }
        ...
        (uint amountSeizeError, uint seizeTokens) = irontroller.liquidateCalculateSeizeTokens(address(this), address(cTokenCollateral), repayAmount);
        if (amountSeizeError != 0) {
            return failOpaque(Error.IRONTROLLER_CALCULATION_ERROR, FailureInfo.LIQUIDATE_IRONTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED, amountSeizeError);
        }
        ...
        /* We call the defense hook */
        irontroller.liquidateBorrowVerify(address(this), address(cTokenCollateral), liquidator, borrower, repayAmount, seizeTokens);
     ...
     * @dev Will fail unless called by another iToken during the process of liquidation.
     *  Its absolutely critical to use msg.sender as the borrowed iToken and not a parameter.
    ...
    function seize(address liquidator, address borrower, uint seizeTokens) external nonReentrant returns (uint) {
        ...
        uint allowed = irontroller.seizeAllowed(address(this), msg.sender, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
            return failOpaque(Error.IRONTROLLER_REJECTION, FailureInfo.LIQUIDATE_SEIZE_IRONTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        irontroller.seizeVerify(address(this), msg.sender, liquidator, borrower, seizeTokens);
    ...
    /**
      * @notice Sets a new irontroller for the market
      * @dev Admin function to set a new irontroller
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setIrontroller(IrontrollerInterface newIrontroller) public returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_IRONTROLLER_OWNER_CHECK);
        }

        IrontrollerInterface oldIrontroller = irontroller;
        // Ensure invoke irontroller.isIrontroller() returns true
        require(newIrontroller.isIrontroller(), "marker method returned false");

        // Set market's irontroller to newIrontroller
        irontroller = newIrontroller;

        // Emit NewIrontroller(oldIrontroller, newIrontroller)
        emit NewIrontroller(oldIrontroller, newIrontroller);
...
contract IEther is IToken {
    /**
     * @notice Construct a new IEther money market
     * @param irontroller_ The address of the Irontroller
     * @param interestRateModel_ The address of the interest rate model
     * @param initialExchangeRateMantissa_ The initial exchange rate, scaled by 1e18
    ...
    IToken(irontroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_) {}
     ...
     * @param cTokenCollateral The market in which to seize collateral from the borrower
     */
    function liquidateBorrow(address borrower, IToken cTokenCollateral) external payable {
        requireNoError(liquidateBorrowInternal(borrower, msg.value, cTokenCollateral), "liquidateBorrow failed");
    }

    /**
     * @notice Send Ether to IEther to mint
     */
