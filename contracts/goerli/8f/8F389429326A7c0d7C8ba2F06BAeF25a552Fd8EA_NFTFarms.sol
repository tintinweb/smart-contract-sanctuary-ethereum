...
contract NFTFarms is ReentrancyGuard, Ownable, Pausable, IERC721Receiver {
  ...
  event CreateFarm(uint256 indexed farmId,
    bool active,
    address stakedNFTCollection,
    uint256 rewardsDistributionRate,
    uint256 rewardsPerStakedNFT,
    uint256 lastUpdatedAt,
    uint256 endTime
  ...
  */
  function rewardsEarned(uint256 _id, address _user) public view returns (uint256) {
    ...
    return ((position.stakedAmount
             * SAFE_MULTIPLIER // multiply by 1e18 as stakedAmount is absolute number
  ...
  function _updateFarm(uint256 _id) internal {
    ...
    if (farm.totalStaked > 0 && farm.lastUpdatedAt < farm.endTime) {
      ...
      farm.rewardsPerStakedNFT = (time - farm.lastUpdatedAt)
        ...
        * SAFE_MULTIPLIER
        / farm.totalStaked
        / SAFE_MULTIPLIER // since totalStaked is not in 1e18
        + farm.rewardsPerStakedNFT;
    }
  ...
  */
  function _updateUserRewardsDebt(uint256 _id, address _user, uint256 _amount) private {
    ...
    position.rewardsDebt = position.rewardsDebt
                          + (_amount
                            * SAFE_MULTIPLIER // multiply by 1e18 as _amount is an absolute number
                            * farms[_id].rewardsPerStakedNFT
                            / SAFE_MULTIPLIER);
  }
  ...
  function claim(uint256 _id) public whenNotPaused {
    ...
    if (rewards > 0) {
      ...
      position.rewardsDebt = position.stakedAmount
                             * SAFE_MULTIPLIER // multiply by 1e18 as stakedAmount is in absolute number
                             * currentRewardPerStakedToken(_id)
                             / SAFE_MULTIPLIER;
  ...
  */
  function createFarm(
    ...
    emit CreateFarm(
      farm.id,
      farm.active,
      address(farm.stakedNFTCollection),
      farm.rewardsDistributionRate,
      farm.rewardsPerStakedNFT,
      farm.lastUpdatedAt,
      farm.endTime
  ...
  */
  ...
  /**
  * Emergency recover ERC20 tokens
  * @param _token Address of ERC20 token
  * @param _amount Anmount of token to recover
  */
  ...
  /**
  * Emergency recover native ETH
  * @param _amount Amount of native ETH to recover
  */
