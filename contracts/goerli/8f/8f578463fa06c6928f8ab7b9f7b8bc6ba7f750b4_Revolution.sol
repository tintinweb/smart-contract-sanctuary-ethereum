...
contract Revolution is Ownable {
    ...
    uint16 public constant SPLIT_BPS = 5; //50%
    ...
    modifier hasVotes {
      require(daoToken.getVotes(msg.sender) >= MIN_VOTES);
      _;
    }

    function dosomething() external returns (uint64) {
        return 42;
    }

    function deployRevolutionDrop(
        string memory name,
