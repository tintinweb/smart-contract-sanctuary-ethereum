...
import "../MarketStableDef_goerli.sol";

contract WETHMarket_goerli is MarketStableDef_goerli {
...
abstract contract MarketStableDef_goerli is Market {
    ...
    function BOND_ADDRESS() public view virtual override returns (address) {
        return bond; // address(0xd8B895a12557DbB346FB5d660A670B39DaD96a97);
    }
}
    ...
    IMarket
{
    ...
    modifier takerTermEnds(uint id) {
        ...
        require(
            ...
            block.timestamp >
                (pos.start + 
                 pos.term * pos.flags.getAutorenewTerms() *
                    1 days),
            "MC: taker-position-not-ended"
        );
        _;
    }

    /// @notice Check whether the given Maker position has ended
    ...
    modifier makerTermEnds(uint id) {
        ...
        require(
            ...
            block.timestamp >
                (pos.start + 
                 pos.term * pos.flags.getAutorenewTerms() * 1 days),
        ...
        returns (uint id, uint floor)
    {
        ...
        floor = (((uint(price) * risk) / 10000) * DIVIDER_STABLE()) /
            DIVIDER_ORACLE;
        ...
        if (hook != address(0))
            IMarketCallback(hook).onProtected(id);
    }
    ...
    function close(
        ...
        onlyPermittedAdapter
        whenNotPaused
        returns (uint premium, Payout memory payout)
    {
        TakerPosition storage pos = allTakerPositions[id];
        require(account == pos.owner, "MC: not-position-owner");

        premium = premiumOnClose(id);
    ...
    virtual override onlyPermittedAdapter whenNotPaused 
    returns (uint premium, Payout memory payout)
    {
        TakerPosition storage pos = allTakerPositions[id];
        require(account == pos.owner, "MC: not-position-owner");
        ...
        premium = premiumOnCancel(id);
        uint amount = pos.assetAmount - premium;
        payout = IRebalancer(rebalancer)
            .createTakerPayoutOnCancel(address(this), amount);
        IState(state).closeTakerPosition(
            pos.assetAmount,
            pos.risk,
            pos.term,
        ...
        returns (uint id)
    {
        ...
        IState(state).newMakerPosition(pos.stableAmount, pos.tier, pos.term);
        ...
        if (hook != address(0))
            IMarketCallback(hook).onDeposited(id);
    }
        ...
        returns (int yield, Payout memory payout)
    {
        ...
        payout = IRebalancer(rebalancer)
            .createMakerPayoutOnWithdraw(address(this), amount);
    ...
    returns (int yield, Payout memory payout)    
    {
        ...
        uint amount = uint(int(pos.stableAmount) + yield);
        payout = IRebalancer(rebalancer)
            .createMakerPayoutOnAbandon(address(this), amount);
    ...
    /// @inheritdoc IMarket
    function getTakerPositionBond(uint id)
        public
        view
        virtual
        override
        returns (uint)
    {
        return allTakerPositions[id].bumpAmount;
    }
    ...
    /// @inheritdoc IMarket
    function getMakerPositionBond(uint id)
        public
        view
        virtual
        override
        returns (uint)
    {
        return allMakerPositions[id].bumpAmount;
    }
