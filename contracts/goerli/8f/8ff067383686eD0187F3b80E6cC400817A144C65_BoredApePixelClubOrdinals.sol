...
contract BoredApePixelClubOrdinals is ERC721A, Ownable, ReentrancyGuard {
    ...
    // Public mint 
    function mint(uint256 quantity, address to) external payable nonReentrant callerIsUser {
        ...
        // Math.floor of totalSupply / nextPause
    ...
    // Giveaway mint
    function giveaway(uint256 quantity, address to) external payable onlyOwner nonReentrant  {
        uint256 totalMinted = totalSupply() + quantity;
        require(totalMinted <= maxSupply, "Not enough supply left for this mint quantity");
