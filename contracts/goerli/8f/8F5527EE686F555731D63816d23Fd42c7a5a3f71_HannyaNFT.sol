...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9; //0x8F5527EE686F555731D63816d23Fd42c7a5a3f71
...
contract HannyaNFT is ERC1155, Ownable { 
   
    string public name;
    string public symbol;
    uint256 public numberOfNFTs = 2;// 2 for test but there are 8(7) in total 
    uint256 public saleState;
    address payable public withdrawWallet;


    //struct and mapping to block looping
    struct MintedWallet {
        uint256 tokenMinted; 
        uint256 tokenMintedAmount;
        uint256 publicMint; //if minted in public they cannot mint again
        uint256 wlMint;
    }

    mapping(address => MintedWallet) public mintedwallet;

    //mapping because multiple tokens can have different price, supply etc..
...
//=====================================================================================================//
    ...
    function mintWLOnly(uint256 _id, uint256 _amount, bytes32[] calldata merkleProof) external payable {
        ...
        uint256 index = _id - 1;
        require(mintAmount[index] + _amount <= maxSupply[index], "Not enough supplies");
        require(saleState == 1, "WL Minting is not live");
        require(msg.value == mintPrice[index] * _amount, "Insufficient funds");
        require(_amount <= maxPerTokenId[index], "Exceed max mint");
        require(index < numberOfNFTs, "This NFT does not exist");
        ...
        mintAmount[index] += _amount;
        mintedwallet[msg.sender].tokenMinted = index; 
        mintedwallet[msg.sender].tokenMintedAmount = mintAmount[index]; 
        mintedwallet[msg.sender].wlMint = 1;
        _mint(msg.sender, index, _amount, '');
    ...
    function mintPublic(uint256 _id, uint256 _amount) external payable {
        ...
        //reason for minus 1 is because solidity starts with 0 but front end will put 1
        uint256 index = _id - 1;
        require(mintAmount[index] + _amount <= maxSupply[index], "Not enough supplies");
        ...
        require(_amount <= maxPerTokenId[index], "Exceed max mint");
        require(index < numberOfNFTs, "This NFT does not exist");
        require(mintedwallet[msg.sender].publicMint == 0, "You have already minted");
        
        mintAmount[index] += _amount;
        mintedwallet[msg.sender].tokenMinted = index; //which token was minted
        mintedwallet[msg.sender].tokenMintedAmount = mintAmount[index]; //how many mints
        mintedwallet[msg.sender].publicMint = 1; //have minted
        _mint(msg.sender, index, _amount, '');
