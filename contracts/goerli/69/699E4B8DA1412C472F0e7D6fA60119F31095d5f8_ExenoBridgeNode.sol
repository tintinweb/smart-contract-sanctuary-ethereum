...
contract ExenoBridgeNode is 
    ...
    ERC1363Payable
{
    // Encoded name of the blockchain where this contract is deployed
    bytes32 public immutable blockchain;
    ...
    // Addresses of the private keys used for signing playloads
    ...
    // Indicates a withdrawal has been made - only for informational purposes, not intended to be acted upon by the cloud infrastructure
    event Withdraw(
        bytes32 indexed id,
        address indexed platform,
        bytes32[2] network,
    ...
    // Indicates that a new router has been added
    event RouterAdded(
        address router
    );

    // Indicates that an existing router has been removed
    event RouterRemoved(
        address router
    );
    ...
    /**
        fulfilled: flag that indicates a deposit when set to `false` or a withdrawal when set to `true`
        platform: an address associated with a UI, used for allocating fees
        fee: first value is denominated in cash (i.e. native currency), while the second value is denominated in core token (i.e. EXN token)
     */
    struct Meta {
        bool fulfilled;
        uint256[2] fee;
    ...
    {
        Data memory data = Data(id, platform, network, beneficiary, token, amount, timeout, operation);
        Meta memory meta = Meta(fulfilled, fee);
        _verifySignature(data, meta, signature);
        _validateDeposit(data, meta);
        _processDeposit(data);
        if (_isImmediate(data)) {
    ...
    function deposit(
        ...
        external payable nonReentrant whenNotPaused
    {
        (Data memory data, Meta memory meta, bytes memory signature) = _unpackSignedData(encodedData);
        _verifySignature(data, meta, signature);
        _validateDeposit(data, meta);
        _processDeposit(data);
        if (_isImmediate(data)) {
        ...
        external view returns(bool)
    {
        (Data memory data, Meta memory meta, bytes memory signature) = _unpackSignedData(encodedData);
        _verifySignature(data, meta, signature);
        require(!meta.fulfilled,
            "ExenoBridgeNode: this payload is not a deposit");
        require(!depositIds[data.id],
            "ExenoBridgeNode: deposit already processed");
        require(data.network[0] == blockchain,
            "ExenoBridgeNode: blockchain mismatch");
        if (data.operation[0] == bytes32("burn")) {
            require(_isMintable(data.token[0]),
                "ExenoBridgeNode: burn operation cannot be done on a non-mintable token");
        ...
        external view returns(bool)
    {
        ...
        require(data.network[1] == blockchain,
            "ExenoBridgeNode: blockchain mismatch");
        if (data.operation[1] == bytes32("mint")) {
            require(_isMintable(data.token[1]),
                "ExenoBridgeNode: mint operation cannot be done on a non-mintable token");
    ...
    function payout(
        ...
        external onlyRouter
    {
        _processWithdraw(Data(id, platform, network, beneficiary, token, amount, timeout, operation));
    }
        ...
        internal override nonReentrant whenNotPaused
    {
        (Data memory data, Meta memory meta, bytes memory signature) = _unpackSignedData(encodedData);
        _verifySignature(data, meta, signature);
        if (!meta.fulfilled) { // in case it's a deposit we treat tokens sent to this contract as a deposit and/or fee payment    
            _validateDeposit(data, meta, sender, amount);
            _processDeposit(data);
            if (_isImmediate(data)) {
                _processWithdraw(data);
            }
        } else { // in case it's a withdrawal we treat tokens sent to this contract as a fee payment
            _validateWithdraw(data, meta, amount);
            _processWithdraw(data);
        }
    }
    ...
    function _verifySignature(
        ...
        internal view
    {
        bytes memory payload = abi.encodePacked(d.id, d.platform, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.operation, m.fulfilled, m.fee);
        ...
        internal
    {
        ...
        if (_isNative(d.token[0])) {
            require(msg.value >= d.amount[0] + m.fee[0],
                "ExenoBridgeNode: not enough cash attached to cover amount and fee");
            earnedFee[d.platform][0] += m.fee[0];
            extraFee[d.platform][0] += msg.value - d.amount[0] - m.fee[0];
        } else {
            require(msg.value >= m.fee[0],
            ...
            earnedFee[d.platform][0] += m.fee[0];
            extraFee[d.platform][0] += msg.value - m.fee[0];
        }
    }
        ...
        internal
    {
        ...
        if (_isCore(d.token[0])) {
            require(amount >= d.amount[0] + m.fee[1],
                "ExenoBridgeNode: not enough core tokens transferred to cover amount and fee");
            earnedFee[d.platform][1] += m.fee[1];
            extraFee[d.platform][1] += amount - d.amount[0] - m.fee[1];
        } else {
            require(amount >= m.fee[1],
            ...
            require(IERC20(d.token[0]).transferFrom(sender, address(this), d.amount[0]),
                "ExenoBridgeNode: ERC20 transferFrom failed");
            earnedFee[d.platform][1] += m.fee[1];
            extraFee[d.platform][1] += amount - m.fee[1];
        }
    }
        ...
        internal
    {
        ...
        emit Deposit(d.id, d.platform, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.operation);
    }

    /**
     * When fee is paid in cash (i.e. native currency), validate the following aspects of a withdrawal:
        ...
        internal
    {
        ...
        if (d.operation[1] == bytes32("mint")) {
            ...
            require(ownedTokens(d.token[1]) >= d.amount[1],
                "ExenoBridgeNode: cannot unfreeze tokens as there are not enough owned tokens");
            require(frozenTokens >= d.amount[1],
                "ExenoBridgeNode: cannot unfreeze tokens as there are not enough frozen tokens");
            ...
            if (_isNative(d.token[1])) {
                require(availableCash() >= d.amount[1],
                    "ExenoBridgeNode: cannot release cash as there is not enough available cash");
                Address.sendValue(payable(d.beneficiary[1]), d.amount[1]);
            } else {
                require(availableTokens(d.token[1]) >= d.amount[1],
                    "ExenoBridgeNode: cannot release tokens as there are not enough available tokens");
                require(IERC20(d.token[1]).transfer(d.beneficiary[1], d.amount[1]),
                    "ExenoBridgeNode: ERC20 transfer failed");
            }
        }
        emit Withdraw(d.id, d.platform, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.operation);
    }
        ...
        internal pure returns(Data memory)
    {
        ...
        ) = abi.decode(encodedData, (bytes32, address, bytes32[2], address[2], address[2],  uint256[2], uint256[2], bytes32[2]));
        return Data(id, platform, network, beneficiary, token, amount, timeout, operation);
    }
    ...
    function _unpackSignedData(bytes memory encodedData)
        internal pure returns(Data memory, Meta memory, bytes memory)
    {
        (
            bytes32 id,
            address platform,
            ...
            bytes memory signature
        ) = abi.decode(encodedData, (bytes32, address, bytes32[2], address[2], address[2], uint256[2], uint256[2], bytes32[2], bool, uint256[2], bytes));
        return (Data(id, platform, network, beneficiary, token, amount, timeout, operation), Meta(fulfilled, fee), signature);
    }
    ...
    /**
     * Checks whether deposit's payload meets the conditions for immediate withdraw
     */
    function _isImmediate(Data memory d)
        internal pure returns(bool)
    {
        return d.network[1] == d.network[0] && d.timeout[1] == 0;
    }
        ...
        external onlyOwner
    {
        require(earnedFee[platform][0] > 0 || earnedFee[platform][1] > 0,
            "ExenoBridgeNode: there is no fee to reset");
        ...
        external nonReentrant
    {
        require(extraFee[platform][0] > 0 || extraFee[platform][1] > 0,
            "ExenoBridgeNode: there is no fee to release");
    ...
    function addRouter(address router)
        external onlyOwner validAddress(router)
    {
        require(!routers[router],
            "ExenoBridgeNode: router already added");
        routers[router] = true;
        emit RouterAdded(router);
    }

    /**
     * Remove an existing router
     */
    function removeRouter(address router)
        external onlyOwner validAddress(router)
    {
        require(routers[router],
            "ExenoBridgeNode: router already removed");
    ...
    {
        require(IERC20(_coreToken()).allowance(account, address(this)) >= amount,
            "ExenoBridgeNode: before freezing account must approve amount");
        require(IERC20(_coreToken()).transferFrom(account, address(this), amount),
            "ExenoBridgeNode: ERC20 transferFrom failed");
        frozenTokens += amount;
