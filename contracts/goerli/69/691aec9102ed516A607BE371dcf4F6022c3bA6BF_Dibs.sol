...
contract Dibs is AccessControl {
    ...
    mapping(uint32 => uint256) public roundToRandomSeed;
    ...
    error LotteryRoundRandomSeedExists();
    ...
    function claim(
        ...
        _claim(token, msg.sender, amount, to);
    }
    ...
    event RoundRandomSeed(uint32 _round, uint256 _randomSeed);

    function setRoundRandomSeed(uint32 roundId, uint256 randomSeed) external {
        if (roundToRandomSeed[roundId] != 0) {
            revert LotteryRoundRandomSeedExists();
        }

        roundToRandomSeed[roundId] = randomSeed;

        emit RoundRandomSeed(roundId, randomSeed);
    }
    ...
    function claimDIBS(
        ...
        _claim(token, codeToAddress[DIBS], amount, to);
    }
    ...
    function _claim(
        ...
        // get ramaining balance todo: get muon signature
