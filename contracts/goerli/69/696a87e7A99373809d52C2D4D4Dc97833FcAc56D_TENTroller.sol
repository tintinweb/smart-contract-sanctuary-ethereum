...
// File: tenlendContracts/EIP20Interface.sol
...
// File: tenlendContracts/ExponentialNoError.sol
...
// File: tenlendContracts/CarefulMath.sol
...
// File: tenlendContracts/Exponential.sol
...
// File: tenlendContracts/ErrorReporter.sol

pragma solidity ^0.5.16;

contract TENTrollerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        TENTROLLER_MISMATCH,
...
contract TokenErrorReporter {
    enum Error {
        ...
        UNAUTHORIZED,
        BAD_INPUT,
        TENTROLLER_REJECTION,
        TENTROLLER_CALCULATION_ERROR,
    ...
    enum FailureInfo {
        ...
        BORROW_TENTROLLER_REJECTION,
        LIQUIDATE_ACCRUE_BORROW_INTEREST_FAILED,
        LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED,
        LIQUIDATE_COLLATERAL_FRESHNESS_CHECK,
        LIQUIDATE_TENTROLLER_REJECTION,
        LIQUIDATE_TENTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED,
        ...
        LIQUIDATE_SEIZE_TENTROLLER_REJECTION,
        LIQUIDATE_SEIZE_LIQUIDATOR_IS_BORROWER,
        LIQUIDATE_SEIZE_TOO_MUCH,
        MINT_ACCRUE_INTEREST_FAILED,
        MINT_TENTROLLER_REJECTION,
        ...
        REPAY_BORROW_TENTROLLER_REJECTION,
...
// File: tenlendContracts/EIP20NonStandardInterface.sol
...
// File: tenlendContracts/InterestRateModel.sol

pragma solidity ^0.5.16;

/**
  * @title TENLend's InterestRateModel Interface
  * @author TENLend
  */
...
// File: tenlendContracts/TENTrollerInterface.sol
...
// File: tenlendContracts/TTokenInterfaces.sol
...
contract TTokenStorage {
     ...
     */
    TENTrollerInterface public tentroller;
...
contract TTokenInterface is TTokenStorage {
    ...
    /**
     * @notice Event emitted when tentroller is changed
     */
    event NewTENTroller(TENTrollerInterface oldTENTroller, TENTrollerInterface newTENTroller);
    ...
    function _setTENTroller(TENTrollerInterface newTENTroller) public returns (uint);
...
// File: tenlendContracts/TToken.sol
...
/**
 * @title TENLend's TToken Contract
...
contract TToken is TTokenInterface, Exponential, TokenErrorReporter {
     ...
     */
    function initialize(TENTrollerInterface tentroller_,
        ...
        // Set the tentroller
        uint err = _setTENTroller(tentroller_);
        require(err == uint(Error.NO_ERROR), "setting tentroller failed");

        // Initialize block number and borrow index (block number mocks depend on tentroller being set)
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint allowed = tentroller.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
            return failOpaque(Error.TENTROLLER_REJECTION, FailureInfo.TRANSFER_TENTROLLER_REJECTION, allowed);
        }
        ...
        // unused function
        // tentroller.transferVerify(address(this), src, dst, tokens);
     ...
     * @dev This is used by tentroller to more efficiently perform liquidity checks.
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        uint allowed = tentroller.mintAllowed(address(this), minter, mintAmount);
        if (allowed != 0) {
            return (failOpaque(Error.TENTROLLER_REJECTION, FailureInfo.MINT_TENTROLLER_REJECTION, allowed), 0);
        }
        ...
        // unused function
        // tentroller.mintVerify(address(this), minter, vars.actualMintAmount, vars.mintTokens);
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        uint allowed = tentroller.redeemAllowed(address(this), redeemer, vars.redeemTokens);
        if (allowed != 0) {
            return failOpaque(Error.TENTROLLER_REJECTION, FailureInfo.REDEEM_TENTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        tentroller.redeemVerify(address(this), redeemer, vars.redeemAmount, vars.redeemTokens);
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        uint allowed = tentroller.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
            return failOpaque(Error.TENTROLLER_REJECTION, FailureInfo.BORROW_TENTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        // unused function
        // tentroller.borrowVerify(address(this), borrower, borrowAmount);
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        uint allowed = tentroller.repayBorrowAllowed(address(this), payer, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.TENTROLLER_REJECTION, FailureInfo.REPAY_BORROW_TENTROLLER_REJECTION, allowed), 0);
        }
        ...
        /* We call the defense hook */
        // unused function
        // tentroller.repayBorrowVerify(address(this), payer, borrower, vars.actualRepayAmount, vars.borrowerIndex);
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, TTokenInterface tTokenCollateral) internal returns (uint, uint) {
        ...
        uint allowed = tentroller.liquidateBorrowAllowed(address(this), address(tTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.TENTROLLER_REJECTION, FailureInfo.LIQUIDATE_TENTROLLER_REJECTION, allowed), 0);
        }
        ...
        (uint amountSeizeError, uint seizeTokens) = tentroller.liquidateCalculateSeizeTokens(address(this), address(tTokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_TENTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");
        ...
        /* We call the defense hook */
        // unused function
        // tentroller.liquidateBorrowVerify(address(this), address(tTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        uint allowed = tentroller.seizeAllowed(address(this), seizerToken, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
            return failOpaque(Error.TENTROLLER_REJECTION, FailureInfo.LIQUIDATE_SEIZE_TENTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        // unused function
        // tentroller.seizeVerify(address(this), seizerToken, liquidator, borrower, seizeTokens);
    ...
    /**
      * @notice Sets a new tentroller for the market
      * @dev Admin function to set a new tentroller
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setTENTroller(TENTrollerInterface newTENTroller) public returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_TENTROLLER_OWNER_CHECK);
        }

        TENTrollerInterface oldTENTroller = tentroller;
        // Ensure invoke tentroller.isTENTroller() returns true
        require(newTENTroller.isTENTroller(), "marker method returned false");

        // Set market's tentroller to newTENTroller
        tentroller = newTENTroller;

        // Emit NewTENTroller(oldTENTroller, newTENTroller)
        emit NewTENTroller(oldTENTroller, newTENTroller);
...
// File: tenlendContracts/PriceOracle.sol
...
// File: tenlendContracts/TENTrollerStorage.sol
...
contract UnitrollerAdminStorage {
    ...
    */
    address public pendingTENTrollerImplementation;
}

contract TENTrollerV1Storage is UnitrollerAdminStorage {
...
contract TENTrollerV2Storage is TENTrollerV1Storage {
    struct Market {
...
contract Unitroller is UnitrollerAdminStorage, TENTrollerErrorReporter {
    ...
    /**
      * @notice Emitted when pendingTENTrollerImplementation is changed
      */
    event NewPendingImplementation(address oldPendingImplementation, address newPendingImplementation);

    /**
      * @notice Emitted when pendingTENTrollerImplementation is accepted, which means tentroller implementation is updated
      */
    event NewImplementation(address oldImplementation, address newImplementation);
    ...
    function _setPendingImplementation(address newPendingImplementation) public returns (uint) {
        ...
        pendingTENTrollerImplementation = newPendingImplementation;
    ...
    /**
    * @notice Accepts new implementation of tentroller. msg.sender must be pendingImplementation
    * @dev Admin function for new implementation to accept it's role as implementation
    * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
    */
    function _acceptImplementation() public returns (uint) {
        // Check caller is pendingImplementation and pendingImplementation ≠ address(0)
        if (msg.sender != pendingTENTrollerImplementation || pendingTENTrollerImplementation == address(0)) {
...
// File: tenlendContracts/tentroller.sol
 ...
 * @author TENLend
 */
contract TENTroller is TENTrollerV5Storage, TENTrollerInterface, TENTrollerErrorReporter, ExponentialNoError {
    ...
    function seizeAllowed(
        ...
        if (TToken(tTokenCollateral).tentroller() != TToken(tTokenBorrowed).tentroller()) {
            return uint(Error.TENTROLLER_MISMATCH);
        }
    ...
    function _setPriceOracle(PriceOracle newOracle) public returns (uint) {
        ...
        // Track the old oracle for the tentroller
        PriceOracle oldOracle = oracle;

        // Set tentroller's oracle to newOracle
        oracle = newOracle;
    ...
    function getLendAddress() public view returns (address) {
        return 0x4DdbD048eE2d9cBB44a833E74356A2C55ECEC2Ef;
    }
}