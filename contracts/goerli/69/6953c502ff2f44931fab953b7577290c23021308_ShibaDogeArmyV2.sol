...
// File contracts/ShibaDogeArmyV2.sol

 
pragma solidity 0.8.17;
contract ShibaDogeArmyV2 is ERC1155SupplyUpgradeable, Ownable2StepUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    string public name;
    ...
    // mint function for NFTs
    // needs param for initial equipped items
    ...
    function mintNFT(
        ...
        emit ShibaDogeArmyMinted( NFTSupply + 1, dogeParentID, shibaParentID, traits);
    ...
    function lockNFTEquipmentForSale(uint256 tokenID, uint256 lockTime)
        external whenNotPaused
    {
    ...
    function applyMintedTraits(uint256 tokenID, uint24[15] calldata traits)
        internal
    {
    ...
    function modifySlotsCanBeEmpty(bool[15] calldata GobbledygoolBobbledyBools) external onlyOwner {
    ...
    function mintEFT(uint256 id, uint256 amount, bytes calldata data) external onlyOwner {
        if(balanceOf(ALL_TRAITS_INVENTORY, id) == 0){
            _mintEFT(ALL_TRAITS_INVENTORY, id, 1, data);
        }
