  ...
  AccessControlUpgradeable
{
  ...
  mapping(uint => uint) public TOKEN_TO_ISSUE;
  ...
  function launchIssue(
    ...
    ISSUES[_issue].saleSize = _saleSize;
  ...
  function mint(
    ...
    returns (uint[] memory)
  {
    require(_amount <= 5, 'EXCEEDING_AMOUNT');
    ...
    for (uint i = 0; i < _amount; i++) {
      uint tokenId = (_issue * (10 ** 6)) + (ISSUES[_issue].totalSupply++);
  ...
  function issueList() public view returns (uint[] memory) {
    return ISSUE_IDS;
  }

  function tokenToIssue(uint _tokenId) public view override returns (uint) {
  ...
  function timeToIssue(
    ...
    if (ISSUES[CURRENT_ISSUE].timestamp <= _timestamp)
      ...
      for (uint i = CURRENT_ISSUE; i > 0; i--) {
        if (!isIssueLaunched(i)) break;
        if (
          ISSUES[i].timestamp <= _timestamp &&
          ISSUES[i + 1].timestamp > _timestamp
        ) _issue = ISSUES[i];
      }
    }
