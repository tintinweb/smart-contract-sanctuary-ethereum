...
contract NoteOptimize is Ownable, INote {
    ...
    //states
    // [  note1 |  note2 |  note3 |  note4 | available ]
    // [ 60 bit | 60 bit | 60 bit | 60 bit |  16 bits  ]
    // [ MSB                                       LSB ]
    bytes32 private _noteState;
    uint256 private constant _NOTE_1_OFFSET = 196;
    uint256 private constant _NOTE_2_OFFSET = 136;
    uint256 private constant _NOTE_3_OFFSET = 76;
    uint256 private constant _NOTE_4_OFFSET = 16;
    uint256 private constant _NOTE_BIT_LENGTH = 60;

    function note1() external view returns (uint256) {
        return _noteState.decodeUint(_NOTE_1_OFFSET, _NOTE_BIT_LENGTH);
    }

    function note2() external view returns (uint256) {
        return _noteState.decodeUint(_NOTE_2_OFFSET, _NOTE_BIT_LENGTH);
    }

    function note3() external view returns (uint256) {
        return _noteState.decodeUint(_NOTE_3_OFFSET, _NOTE_BIT_LENGTH);
    }

    function note4() external view returns (uint256) {
        return _noteState.decodeUint(_NOTE_4_OFFSET, _NOTE_BIT_LENGTH);
    }
    ...
    function setAllNote(
        ...
        _noteState = _noteState.insertUint(
            _note1,
            _NOTE_1_OFFSET,
            _NOTE_BIT_LENGTH
        );
