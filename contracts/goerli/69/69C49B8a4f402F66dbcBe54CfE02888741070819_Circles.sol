...
contract Circles is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    address private company = 0x0c8e5995f252510b31BeF534941a605B8E313C58;
    ...
    function mint(uint256 _mintAmount) external payable nonReentrant {
        ...
        require(!isContract(msg.sender), "Invalid");
        require(_mintAmount > 0 && _mintAmount <= maxMint, "0");
        uint256 s = totalSupply();
        require(s + _mintAmount <= maxSupply, "Max");
        if (s >= free) require(msg.value >= cost * _mintAmount, "ETH");
    ...
    function mintForAddress(address[] calldata receiver) external onlyOwner {
        uint256 s = totalSupply();
