...
contract RegistryV2 is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    struct Organisation {
        ...
        mapping(bytes32 => string) assetsIdentifiers;
    ...
    function deployModule(bytes32 orgId, bytes32 module, bytes memory initData, bool upgradeable) public payable returns (address){
    ...
    function addOwnTokenV1(bytes32 orgId, address tokenAddr) public {
        Organisation storage org = _getOrganisationWithOwnerOrRevert(orgId, msg.sender);

        require(org.modules[Modules.TOKEN_V1].length == 0, "Prerequisites: module already exists");

        uint256 contractSize;
        assembly {
            contractSize := extcodesize(tokenAddr)
        }
        require(contractSize > 0, "Prerequisites: tokenAddr is not a contract");

        org.importedToken = true;
        org.modules[Modules.TOKEN_V1].push(tokenAddr);
        org.installedModules.push(Modules.TOKEN_V1);

        emit OrganisationAddedModule(orgId, Modules.TOKEN_V1, false, tokenAddr);
    }

    function setAssetIdentifier(bytes32 orgId, bytes32 asset, string memory identifier) public {
        Organisation storage org = _getOrganisationWithOwnerOrRevert(orgId, msg.sender);

        org.assetsIdentifiers[asset] = identifier;
    }
    ...
    function getAssetIdentifier(bytes32 orgId, bytes32 assetId) public view returns (string memory) {
        Organisation storage org = _getOrganisationOrRevert(orgId);

        return org.assetsIdentifiers[assetId];
    }
