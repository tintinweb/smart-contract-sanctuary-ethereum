...
contract MintDeployer {
  ...
  function DeployBasicMint(uint256 mintPrice, address payable contractOwner) external payable {
    ...
    userToMint[contractOwner].push(mint);
  }

  function getNumberOfContractsForUser(address contractOwner) public view returns (uint)
  {
      uint length = userToMint[contractOwner].length;
      return length;
  }
