...
import { ClosedSaleParams, VoucherParams } from "./StructsV5.sol";
import { ABI_MULTISIG_REVEAL_SALE, ABI_REVEAL_SALE, ABI_VOUCHER_CLOSED_SALE_MINT, ABI_CLOSED_SALE_MINT } from "./FunctionAbiV5.sol";
import "./Erc721SaleV5.sol";

/**
    @notice This contract manages the Closed Sale for the NFTs.
 */
contract Erc721ClosedSaleV5 is Erc721SaleV5 {
  ...
  /**
    ...
    */
  function revealSale(string calldata baseURI_, MultiSigParams calldata params_)
    external
    ...
    whenNotPaused
  {
    ...
    _verifyMultisig(
      abi.encode(ABI_MULTISIG_REVEAL_SALE, keccak256(bytes(baseURI_)), params_.nonce, ABI_REVEAL_SALE),
      params_,
      CONTRACT_ADMIN_ROLE
    );
  ...
  /**
    ...
    */
  function mintTokens(uint256 tokenCount_, VoucherParams calldata params_) external payable whenNotPaused {
    require(isSaleActive, "15");
    require(totalSupply + tokenCount_ <= maxSupply, "17");
    require(!msg.sender.isContract(), "16");
    require(balanceOf(msg.sender) + tokenCount_ <= maxTokensPerWallet, "19");
    ...
    _verifyVoucher(
      abi.encode(
        ABI_VOUCHER_CLOSED_SALE_MINT,
        msg.sender,
        params_.issueTime,
        params_.expirationDuration,
        params_.pricePerToken,
        params_.nonce,
        ABI_CLOSED_SALE_MINT
      ),
      params_,
      VOUCHER_SIGNER_ROLE
    );
  ...
  function tokenURI(uint256 tokenId_) public view override(ERC721) returns (string memory) {
    ...
    if (isRevealed) {
      ...
      if (offsetTokenId >= maxSupply) offsetTokenId -= maxSupply;
