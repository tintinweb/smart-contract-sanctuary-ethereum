...
//SPDX-IDENTIFIER-LICENSE:MIT LICENSE
...
contract NFTStaking is Ownable, IERC721Receiver {
        ...
        returns (uint256[1] memory info)
    {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            //require(staked.owner == account, "not an owner");
            uint256 stakedAt = staked.timestamp;
            rewardmath = (100 ether * (1665791384 - stakedAt)) / 86400;
            earned = rewardmath / 100;
        }
        if (earned > 0) {
            return [earned];
    ...
    function getOwner(uint256[] calldata tokenIds)public view returns (address){
