...
contract PreSale is OwnerWithdrawable, Pausable {
    ...
    // Amount of token that owner want to sell at this round
    uint256[3] public DISTRIBUTION_TOKEN = [
        2700000 ether,
        3600000 ether,
        4500000 ether
    ];

    // Rate to swap from 1$ to XOX, unit is base on BASE_DENOMINATOR. E.g: 0.5$ = 1 XOX => _exchangeRate = 5000 (5000 / 10_000 = 0.5)
    uint256[3] public EXCHANGE_RATE = [440, 450, 460];

    // Percentage of bonus XOXS that user will receive on investment. Current base is 100. E.g: 8% => _bonusPercentage = 8 (8 / 100 = 8%)
    uint256[3] public BONUS_PERCENTAGE = [8, 6, 4];

    // Buy boundary
    uint256 public constant MAXIMUM_BUY = 5000000000;
    uint256 public constant MINIMUM_BUY = 50000000;

    // Current round to check status scheducle sale
    ...
    mapping(address => mapping(uint256 => uint256)) public userInvestedAmount;

    // Amount each user has invested USD in each round
    mapping(address => mapping(uint256 => uint256))
        private userInvestedUSDAmount;
     ...
     * @param _merkleRoot Root of merkle tree, used to whitelist address on pre sale
    ...
    /**
     * @dev Update timeline for pre-sale schedule
     */
    function updateTimeline(
        ...
        saleRound[_round].startDate = _startDate;
        saleRound[_round].endDate = _endDate;
    }

    /**
     * @dev Call this function to start a new round of private sale
     * @param _startDate Time to start this round in timestamp unit
     * @param _endDate Time to end this round in timestamp unit
     * @notice Each parameters should be set carefully since it's not modifiable for each round
     */
    function setUpNewSaleRound(
        uint256 _startDate,
        uint256 _endDate
        ...
        require(
            (currentRound == 0) ||
                (saleRound[currentRound].endDate < timestampCurrent),
            "Not time to start new private-sale round"
        );
        require(_startDate < _endDate, "Invalid sale time");
        saleRound[currentRound.add(1)] = SaleRound({
            totalDistribution: DISTRIBUTION_TOKEN[currentRound],
            startDate: _startDate,
            endDate: _endDate,
            exchangeRate: EXCHANGE_RATE[currentRound],
            bonusPercentage: BONUS_PERCENTAGE[currentRound]
        });
        currentRound++;
    }
    ...
    function invest(
        ...
        require(currentRound > 0, "PreSale: Not start any round");
        uint256 timestampCurrent = block.timestamp;
        SaleRound storage currentSaleRound = saleRound[currentRound];
        require(
        ...
        require(
            userInvestedUSDAmount[msg.sender][currentRound].add(cash) <=
                MAXIMUM_BUY,
            "PreSale: Exchange amount exceed limit!"
        );
        uint256 amounXOXToInvest = cash.mul(BASE_DENOMINATOR).mul(1e12).div(
        ...
        userInvestedUSDAmount[msg.sender][currentRound] = userInvestedUSDAmount[
            msg.sender
        ][currentRound].add(cash);
        userInvestedAmount[msg.sender][currentRound] = userInvestedAmount[
            msg.sender
        ][currentRound].add(amounXOXToInvest);
    ...
    function whiteListedInvest(
        ...
        SaleRound storage currentSaleRound = saleRound[currentRound];
        require(
            timestampCurrent >= currentSaleRound.startDate.sub(1 hours),
            "PreSale: Not sold yet"
        );
        require(
            timestampCurrent <= currentSaleRound.endDate,
     ...
     */
    function pendingXOXInvest(
        address account,
        uint256 round
    ) public view returns (uint256) {
        if (currentRound == 0 || round == 0 || round > 3) return 0;
        if (block.timestamp < saleRound[round].endDate) return 0;
        uint256 userInvestedBalance = userInvestedAmount[account][round];
        uint256 unlockedPercent = _caculateUnlockedPercent(round);
        uint256 unlockedAmount = userInvestedBalance.mul(unlockedPercent).div(
            100
        );
        return unlockedAmount.sub(userClaimedAmount[account][round]);
    }
    ...
    /**
     * @dev View function, get amount of value USD that user has invested
     */
    function getInvestedUSDAmount(
        address _user,
        uint256 _saleRound
    ) public view returns (uint256) {
        return userInvestedUSDAmount[_user][_saleRound];
    }
    ...
    function _caculateUnlockedPercent(
        ...
        uint256 monthsPass = (block.timestamp.sub(saleRound[round].endDate))
            .div(ONE_MONTH);
