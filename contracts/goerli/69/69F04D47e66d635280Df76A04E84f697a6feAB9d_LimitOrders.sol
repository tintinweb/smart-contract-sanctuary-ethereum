...
library LimitOrders {
    ...
    event BuyLimitOrderEnqueued(uint256 indexed orderId, uint256 price);
    event SellLimitOrderEnqueued(uint256 indexed orderId, uint256 price);
    ...
    struct StoredOrders {
        Orders.BuyParams buyParams;
        Orders.SellParams sellParams;
    ...
    struct Data {
        ...
        mapping(uint256 => StoredOrders) orders;
        mapping(uint256 => uint256) delayOrderIdMapping;
        address delay;
        address factory;
        uint256 maxGasLimit;
    ...
    function buyLimitOrder(
        ...
        uint256 gasPrice = getGasPrice(data);
        uint256 tokenTransferCost = ITwapDelay(data.delay).getTransferGasCost(buyParams.tokenIn);
        ...
        checkOrderParams(
            ...
            ORDER_BASE_COST.add(tokenTransferCost * 2)
        );
        ...
        allocateGasRefund(data, value, buyParams.gasLimit, gasPrice);
        uint256 share = tokenShares.amountToShares(buyParams.tokenIn, buyParams.amountInMax, buyParams.wrapUnwrap);
        StoredOrders storage buyOrder = data.orders[data.newestOrderId + 1];
        buyOrder.buyParams.tokenIn = buyParams.tokenIn;
        buyOrder.buyParams.tokenOut = buyParams.tokenOut;
        buyOrder.buyParams.amountOut = buyParams.amountOut;
        buyOrder.buyParams.amountInMax = buyParams.amountInMax;
        buyOrder.buyParams.wrapUnwrap = buyParams.wrapUnwrap;
        buyOrder.buyParams.to = buyParams.to;
        buyOrder.buyParams.gasLimit = buyParams.gasLimit;
        buyOrder.buyParams.submitDeadline = buyParams.submitDeadline;
        ...
        buyOrder.gasPrice = gasPrice;
        buyOrder.to = buyParams.to;
        buyOrder.submitter = msg.sender;
        buyOrder.gasLimit = buyParams.gasLimit;
        buyOrder.twapInterval = twapInterval;
        enqueueOrder(data, buyOrder);
        emit SellLimitOrderEnqueued(data.newestOrderId, price);
        return data.newestOrderId;
    }
    ...
    function sellLimitOrder(
        ...
        uint256 gasPrice = getGasPrice(data);
        uint256 tokenTransferCost = ITwapDelay(data.delay).getTransferGasCost(sellParams.tokenIn);
        require(tokenTransferCost != 0, 'OS0F');
        checkOrderParams(
            data,
            sellParams.to,
        ...
        allocateGasRefund(data, value, sellParams.gasLimit, gasPrice);
        uint256 share = tokenShares.amountToShares(sellParams.tokenIn, sellParams.amountIn, sellParams.wrapUnwrap);
        StoredOrders storage sellOrder = data.orders[data.newestOrderId + 1];
        sellOrder.sellParams.tokenIn = sellParams.tokenIn;
        sellOrder.sellParams.tokenOut = sellParams.tokenOut;
        sellOrder.sellParams.amountIn = sellParams.amountIn;
        sellOrder.sellParams.amountOutMin = sellParams.amountOutMin;
        sellOrder.sellParams.wrapUnwrap = sellParams.wrapUnwrap;
        sellOrder.sellParams.to = sellParams.to;
        sellOrder.sellParams.gasLimit = sellParams.gasLimit;
        sellOrder.sellParams.submitDeadline = sellParams.submitDeadline;
        sellOrder.orderType = LimitOrderType.Sell;
        sellOrder.status = LimitOrderStatus.Waiting;
        sellOrder.price = price;
        sellOrder.gasPrice = gasPrice;
        ...
        sellOrder.gasLimit = sellParams.gasLimit;
        sellOrder.twapInterval = twapInterval;
        enqueueOrder(data, sellOrder);
        emit SellLimitOrderEnqueued(data.newestOrderId, price);
        return data.newestOrderId;
    }
    ...
    function allocateGasRefund(
        ...
        futureFee = gasPrice.mul(gasLimit).mul(2);
        require(value >= futureFee, 'OS1E');
        if (value > futureFee) {
            TransferHelper.safeTransferETH(
                msg.sender,
                value.sub(futureFee),
                ITwapDelay(data.delay).getTransferGasCost(address(0))
            );
        }
    }

    function markOrderFail(Data storage data, uint256 orderId) external {
        data.orders[orderId].status = LimitOrderStatus.Fail;
    }
    ...
    function executeOrder(
        ...
        StoredOrders storage order = data.orders[orderId];
        require(order.orderType != LimitOrderType.Empty, 'order not existent');
        require(order.status == LimitOrderStatus.Waiting, 'order status is not waiting');
        uint256 gasPrice = getGasPrice(data);
        uint256 delayOrderId;
        if (order.orderType == LimitOrderType.Buy) {
            ITwapERC20(order.buyParams.tokenIn).increaseAllowance(data.delay, order.buyParams.amountInMax);
            uint256 amountInMax = tokenShares.sharesToAmount(order.buyParams.tokenIn, order.share);
            order.buyParams.amountInMax = amountInMax;
            delayOrderId = ITwapDelay(data.delay).buy{ value: gasPrice.mul(order.buyParams.gasLimit) }(order.buyParams);
            emit BuyLimitOrderSubmitted(orderId, order.price);
        } else {
            ITwapERC20(order.sellParams.tokenIn).increaseAllowance(data.delay, order.sellParams.amountIn);
            uint256 amountIn = tokenShares.sharesToAmount(order.sellParams.tokenIn, order.share);
            order.sellParams.amountIn = amountIn;
            delayOrderId = ITwapDelay(data.delay).sell{ value: gasPrice.mul(order.sellParams.gasLimit) }(
                order.sellParams
            );
            emit SellLimitOrderSubmitted(orderId, order.price);
        }
        data.delayOrderIdMapping[orderId] = delayOrderId;
        data.orders[orderId].status = LimitOrderStatus.Submitted;
    }

    function getOrder(Data storage data, uint256 orderId) external view returns (StoredOrders memory) {
        return data.orders[orderId];
    }

    function getOrders(Data storage data, address user) external view returns (uint256[] memory) {
        return data.userOrders[user];
    }

    function enqueueOrder(Data storage data, StoredOrders memory order) internal {
        ++data.newestOrderId;
        data.orders[data.newestOrderId] = order;
        data.userOrders[order.to].push(data.newestOrderId);
    }

    function getOrderStatus(Data storage data, uint256 orderId) external view returns (LimitOrderStatus) {
        if (data.orders[orderId].status == LimitOrderStatus.Submitted) {
            Orders.OrderStatus status = ITwapDelay(data.delay).getOrderStatus(data.delayOrderIdMapping[orderId]);
            return LimitOrderStatus(uint256(status));
        }
        return data.orders[orderId].status;
    }
    ...
    function getGasPrice(Data storage data) internal view returns (uint256) {
        return ITwapDelay(data.delay).gasPrice();
    }
}
