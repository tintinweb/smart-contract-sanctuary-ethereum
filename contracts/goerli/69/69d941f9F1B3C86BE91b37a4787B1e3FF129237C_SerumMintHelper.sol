...
contract SerumMintHelper is Ownable, ReentrancyGuard, ERC1155Holder {
  ...
  constructor(address _dogc, address _serum) {
    dogc = Dogc(_dogc);
  ...
  uint256 public holdersNumber = 0;
  bool public populated = false;
  ...
  function countHolder() external onlyOwner {
    for (uint16 i = 0; i < dogc.totalSupply(); i++) {
      address holder = dogc.ownerOf(i);
      if (holderIndex[holder] == 0) {
        holderIndex[holder] = dogc.balanceOf(holder);
        holdersNumber++;
      }
    }
  ...
  function mintM1BatchV2() external payable nonReentrant {
    require(m1MintStateV2, "Exclusive mint haven't start");
    require(populated, "Haven't mint enough serums to helper");
    require(balance(serum.M1_SERUM()) > 0, "Insufficient remains");

    uint256 count = holderIndex[msg.sender];
    require(count > 0, "Caller don't own any dogc");

    require(msg.value >= serum.price() * count, "Insufficient payment");

    serum.safeTransferFrom(
      address(this),
      msg.sender,
      serum.M1_SERUM(),
      holderIndex[msg.sender],
      ""
    );
  }
  ...
  function mintM2BatchV2() external payable nonReentrant {
    require(m2MintStateV2, "Exclusive mint haven't start");
    require(populated, "Haven't mint enough serum to helper");
