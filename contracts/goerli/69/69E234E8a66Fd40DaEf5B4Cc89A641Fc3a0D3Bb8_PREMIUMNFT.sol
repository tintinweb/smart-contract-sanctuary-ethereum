...
contract PREMIUMNFT is ERC721Enumerable, ERC721Royalty, AccessControl{
    ...
    constructor(address _graveaddress, address _listaddress) ERC721('PREMIUMNFT', 'PN') {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setRoleAdmin(CHANGE_ROLE, DEFAULT_ADMIN_ROLE);
        _setDefaultRoyalty(msg.sender, 1000);
        mintTermDatas[++mintTerm] = mintTermData(10, 1676600854, 1677292054, 1677292054, 0);
        graveAddress = _graveaddress;
        listAddress = _listaddress;
    }

    function setMintTerm(uint256 _supply, uint256 _startTime, uint256 _endALTime, uint256 _endFreemintTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
        mintTermDatas[++mintTerm] = mintTermData(_supply, _startTime, _endALTime, _endFreemintTime, 0);
    }
    ...
    function changeMintData(uint256 _supply, uint256 _startTime, uint256 _endALTime, uint256 _endFreemintTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        mintTermDatas[mintTerm].allowListEndTime = _endALTime;
        mintTermDatas[mintTerm].freeMintEndTime = _endFreemintTime;
    }
