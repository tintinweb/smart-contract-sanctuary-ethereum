...
contract Streamer is Ownable {
    ...
    function withdrawEarnings(Voucher calldata voucher) public {
        ...
        Signature memory _sig = voucher.sig;

        address signerAddress = ecrecover(prefixedHashed, _sig.v, _sig.r, _sig.s);

        require(balances[signerAddress] > voucher.updatedBalance,"greater than balance");
        //0.5eth - 0.2eth = 0.3eth
        balances[signerAddress] = balances[signerAddress] - voucher.updatedBalance;
        (bool result,) = owner().call{value:balances[signerAddress]}("");
        require(result,"transaction failed");
        emit Withdrawn(signerAddress,balances[signerAddress]);
        /*
    ...
    */
    function challengeChannel() public {
        require(balances[msg.sender] != 0,"the channel is not open!");
    ...
    */
    function defundChannel() public {
        require(canCloseAt[msg.sender]!=0,"channel already closed");
        require(canCloseAt[msg.sender] < block.timestamp,"the current time must later than the closing time");
        uint256 sendValue = balances[msg.sender];
