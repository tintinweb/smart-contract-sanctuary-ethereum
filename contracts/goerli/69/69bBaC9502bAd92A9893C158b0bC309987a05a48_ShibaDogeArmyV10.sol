    ...
    PausableUpgradeable
{
    ...
    function depositLockedCrate(uint256 crateID) external {
        ...
        require(balanceOf(msg.sender, crateID) > 0);
    ...
    function withdrawUnlockedCrateContents(
        ...
        //If more than 256 blocks have passed, crate is lost
        if (withdrawBlockHash == 0) {
            // only the last 256 blockhashes are stored onchain, otherwise it's 0
            crateAddressDepositBlock[crateID][msg.sender] = 0; // remove the crate so that a new one can be deposited by the user
            emit crateBurned(crateID, msg.sender, withdrawBlock);
        ...
        uint256 randomSeed = uint256(
            keccak256(
                abi.encodePacked(withdrawBlockHash, msg.sender, withdrawBlock)
            )
        );
        ...
        ) = getCrateContents(crateID, randomSeed);

        // ensure that the crateContentsId is valid
        require(crateContentsId != 0);

        // remove that item from the crate supply and create a new
        crateItemSupplies[crateID][crateContentsIndex] -= 1;
        crateAddressDepositBlock[crateID][msg.sender] = 0;
        _mintEFT(msg.sender, crateID, 1, data);
        ...
        returns (uint256 crateContentsEftId, uint256 crateContentsIndex)
    {
        ...
        for (i = 0; i < crateItemSupplies[crateID].length; i++) {
            // 0 case gets skipped so we don't need this
