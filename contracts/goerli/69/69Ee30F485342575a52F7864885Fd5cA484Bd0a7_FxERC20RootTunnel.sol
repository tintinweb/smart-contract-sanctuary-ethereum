...
contract FxERC20RootTunnel is FxBaseRootTunnel, Create2, Const {
    ...
    mapping(address => mapping(address => uint256)) public userFunds; // userFunds[rootToken][msg.sender] = amount
    ...
    function _processMessageFromChild(bytes memory data) internal override {
        ...
        IERC20(rootToken).safeTransfer(address(this), amount);
        userFunds[rootToken][to] += amount;
        emit FxWithdrawERC20(rootToken, childToken, to, amount);
    }

    function claim(address token) external {
        require(userFunds[token][msg.sender] > 0, "FxERC20RootTunnel: NO_FUNDS");
        uint256 amount = userFunds[token][msg.sender];
        userFunds[token][msg.sender] = 0;
        IERC20(token).safeTransfer(msg.sender, amount);
        bytes memory message = abi.encode(UPDATE_PRICE, abi.encode(token, msg.sender, amount));
        _sendMessageToChild(message);
    }
}
