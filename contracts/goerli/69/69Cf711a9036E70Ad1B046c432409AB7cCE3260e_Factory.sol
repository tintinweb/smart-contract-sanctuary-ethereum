...
contract Factory {
    event ContractDeployed(address _securitiesAddr, address _accountAddr);

    function deploy(address _adminAddr, string memory _uri, uint256 _share) external {
        Securities newSecurities = new Securities(_uri);
        Account newAccount = new Account(_adminAddr, address(newSecurities), _share);

        // transfer ownership
        newSecurities.transferOwnership(msg.sender);
        newAccount.transferOwnership(msg.sender);

        emit ContractDeployed(address(newSecurities), address(newAccount));
    }
}
