...
abstract contract WhitelistUpgradeable is Initializable, ContextUpgradeable {
    ...
    function __Whitelist_init(uint256 limitCount_) internal onlyInitializing {
        __Whitelist_init_unchained(limitCount_);
    }

    function __Whitelist_init_unchained(uint256 limitCount_) internal onlyInitializing {
        _setLimitCount(limitCount_);
    }
    ...
    function _addWhitelist(address addr) internal virtual {
        ...
        require(_limitCount >= _whitelist.length() && _limitCount > 0, "Whitelist: limit reached");
    ...
    /**
     * @dev set a whitelist limit count
     * @param limitCount_ a whitelist limit count
     */
    function _setLimitCount(uint256 limitCount_) internal virtual {
    ...
    UUPSUpgradeable
{
    ...
    function setWhitelistLimitCount(uint256 limitCount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setLimitCount(limitCount);
    }

    function resetWhitelist() external onlyRole(DEFAULT_ADMIN_ROLE) {
        _removeWhitelistBatch(allWhitelist());
    }

    function addWhitelist(address addr) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _addWhitelist(addr);
    }

    function removeWhitelist(address addr) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _removeWhitelist(addr);
    }

    function burn(address from, uint256 id) external {
        require(
            from == _msgSender() || isApprovedForAll(from, _msgSender()) || hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
    ...
    function mintPriceByAccount(address account) public view returns (uint256) {
        return isWhiteList(account) ? salesInfo().mintPrice.mul(100 - discountRate).div(100) : salesInfo().mintPrice;
    }
