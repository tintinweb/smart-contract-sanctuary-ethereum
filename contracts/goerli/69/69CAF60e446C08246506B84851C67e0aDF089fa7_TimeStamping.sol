...
contract TimeStamping {
    ...
    EnumerableMap.UintToUintMap private _history;

    function createStamp(uint256 hash_) external {
        require(_history.set(hash_, block.timestamp), "Hash collision");
    }

    function getHashStamp(uint256 hash_) external view returns (uint256) {
        bool succes_;
        uint256 result_;
