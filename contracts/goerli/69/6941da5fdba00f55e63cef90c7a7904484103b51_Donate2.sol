...
contract Donate2 is Ownable {
    ...
    function buy(uint256 purchaseIndex, uint256 code) public {
        ...
        if (code != 0) {
            // add bonus
            _bonus[_code2Addr[code]] = _bonus[_code2Addr[code]].add(_maxAmount.mul(100).div(1000));
        }
    ...
    function myReward(address account, uint256 t) public view returns(uint256) {
        if(_balancesTimestamp[account] == 0) return 0;
        uint256 endTime = _balancesTimestamp[account].add(60 days);
        uint256 daysDiff = 0;
        if(t > endTime){
            daysDiff = t.sub(endTime).div(86400);
        }else{
            daysDiff = endTime.sub(t).div(86400);
        }
        if (daysDiff == 0) {
            return 0;
        }
