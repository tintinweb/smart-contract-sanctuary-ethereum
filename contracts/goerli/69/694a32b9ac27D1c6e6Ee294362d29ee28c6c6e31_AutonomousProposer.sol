...
contract AutonomousProposer is
    IAutonomousProposer,
    CommonChecks,
    ReentrancyGuard
{
    ...
    uint96 public immutable stakeAmount;

    /// @notice The proposal authors and stakedAmounts
    ...
    constructor(
        ...
        IVotes(token).delegate(address(this)); // activating voting power
    }
    ...
    function create(
        ...
        uint govProposalId = Governor.propose(
            _targets,
            _values,
            _calldatas,
            _description
        ...
        UserInfo storage user = users[govProposalId];
     ...
     */
    function cancel(uint _proposalId) external nonReentrant {
        UserInfo storage user = users[_proposalId];
        require(
            msg.sender == user.account,
            "AutonomousProposer: INVALID_AUTHOR"
        );

        Governor.cancelProposal(_proposalId);

        // Transfer staked EGO tokens back to the author
        IERC20(token).safeTransfer(user.account, stakeAmount);

        user.stakedAmount -= stakeAmount;

        emit AutonomousProposalCancelled(_proposalId, user.account);
    }
    ...
    function withdraw(uint _proposalId) external {
        ...
        require(
            status != IEgoGovernorV1.ProposalState.Cancelled,
            "AutonomousProposer: CANCELLED"
        );

        // Transfer staked EGO tokens back to the author
        IERC20(token).safeTransfer(user.account, stakeAmount);

        user.stakedAmount -= stakeAmount; // reverts on underflow

        emit Withdraw(_proposalId, user.account, stakeAmount);
    }
}
...
interface IAutonomousProposer {
    struct UserInfo {
        address account; // 160
        uint96 stakedAmount; // 160 + 96 = 256
    }
...
interface IEgoGovernorV1 {
    ...
    struct Proposal {
        ...
        uint32 voteEnd; // 64
        uint32 extendedDeadline; // 128 (its end of voting if voteStart + preventLateQuorum > voteEnd)
        bool rejected; // 136
        bool cancelled; // 144
