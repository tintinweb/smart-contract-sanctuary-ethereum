...
contract WVRHController is Ownable {
    ...
    function addExtraReward() external onlyOperator{
        uint256 balance = IERC20(wvrhAddress).balanceOf(address(this));
        if(balance > 0){
            IERC20(wvrhAddress).transfer(poolAddress, balance);
            WVRHStakePool(poolAddress).notifyRewardAmount(balance);
        }
    }
