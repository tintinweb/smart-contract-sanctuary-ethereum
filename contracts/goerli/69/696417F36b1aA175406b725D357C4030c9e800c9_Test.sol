...
contract Test {
    ...
    function retrieve() public view returns (uint256){
        uint256 bid = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
        MarketplaceLib.Bid[] memory bids = IMarketplaceCore(0x554FA73BE2f122374e148b35de3eD6c34602Dbf6).getBids(156);
        for (uint256 i = bids.length - 1; i >= 0; i--) {
            if (bids[i].amount < bid) {
                bid = bids[i].amount;
            }
        }
        return bid;
...
library MarketplaceLib {
    ...
    function modifyListing(uint40 listingId, Listing storage listing, uint256 initialAmount, uint48 startTime, uint48 endTime) public {
        ...
        require(listing.flags & MarketplaceLib.FLAG_MASK_FINALIZED == 0 && (
