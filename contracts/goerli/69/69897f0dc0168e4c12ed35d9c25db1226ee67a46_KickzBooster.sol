...
// File: contracts/KickzBooster.sol
...
contract KickzBooster is ERC721A, Ownable {
    enum ContractStatus {
        Sale,
        Paused
    }
    ...
    // Other contracts
    address public kickzMetaContract;
    ...
    constructor(bytes32 _merkleRoot, string memory contractBaseURI) ERC721A ("KICKZ BOOSTER", "KICKZ BOOSTER") {
    ...
    function setKickzMetaContractAddress(address kickzMetaContractAddress) external onlyOwner {
        kickzMetaContract = kickzMetaContractAddress;
    }

    function mint(uint256 quantity, uint256 allowedQuantity, bytes32[] calldata proof) external callerIsUser {
        require(contractStatus == ContractStatus.Sale, "Sale not available");
        require(_totalMinted() + quantity <= maxSupply, "Not enough supply");
        require(isWalletOnWhitelist(msg.sender, allowedQuantity, proof), "Wallet verification failed");
    ...
    function burnBooster(uint256 tokenId) external {
        require(msg.sender == kickzMetaContract, "Invalid burner address");
        _burn(tokenId);
    }

    function isWalletOnWhitelist(address account, uint256 allowedQuantity, bytes32[] calldata proof) public view returns (bool) {
    ...
    function withdraw() external onlyOwner {
        ...
        address kickz = payable(0x0a02A6e9ef13a47DdD43c287171fd4ea7858AE31);

        bool success;

        (success,) = kickz.call{value : (amount)}("");
