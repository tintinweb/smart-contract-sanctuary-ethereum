    ...
    PausableUpgradeable
{
    ...
    function matchOrderExtend(
        ...
        require(orderRight.dataMint.length != 0, "dataMint invlaid");

        LibOrder.Order memory orderLeft;

        LibLazyMint.LazyMint memory dataLazyMint = LibLazyMint._decode(
            orderRight.dataMint
        );
        ...
        } else if (dataLazyMint.typeMint == LibLazyMint.MINT_SIGNED) {
            orderLeft = LibOrder.Order(
                input.maker,
    ...
    */
    function _matchAndTransfer(
        ...
        LibLazyMint.LazyMint memory dataLazyMintLeft;
        LibLazyMint.LazyMint memory dataLazyMintRight;

        if (orderLeft.dataMint.length == 0) {} else {
            dataLazyMintLeft = LibLazyMint._decode(orderLeft.dataMint);
            dataLazyMintRight = LibLazyMint._decode(orderRight.dataMint);
            dataLazyMintLeft.typeMint = dataLazyMintRight.typeMint;
        }
    ...
    */
    ...
    function encodeLazyMint(LibLazyMint.LazyMint memory lazyMint)
        public
        pure
        returns (bytes memory)
    {
        return abi.encode(lazyMint);
    }
