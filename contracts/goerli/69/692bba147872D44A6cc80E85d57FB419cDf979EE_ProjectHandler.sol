...
contract ProjectHandler is BaseStructs, IProjectHandler, Ownable {
    ...
    constructor(address _nftVillageChief, address _owner, uint256 _projectFee, uint256 _poolFee, uint256 _rewardFee, address _projectFeeReceiver, address _poolFeeReceiver, address _rewardFeeReceiver) {
        require(_nftVillageChief != address(0) 
    ...
    function setCardHandler(ICardHandler _cardHandler) override external onlyNFTVillageChief {
    ...
    function addProject(address _admin, uint256 _adminReward, uint256 _referralFee, uint256 _startBlock, INFTVillageCards _poolCards) override external payable onlyOwner {
    ...
    function setProject(uint256 projectId, address _admin, uint256 _adminReward, uint256 _referralFee, INFTVillageCards _poolCards) override external {
        ProjectInfo storage project = projectInfo[projectId];
    ...
    function setPoolShares(uint256 projectId, uint256 poolId, uint256 shares) override external onlyNFTVillageChief{
        projectInfo[projectId].pools[poolId].totalShares = shares;
    }

    function setStakedAmount(uint256 projectId, uint256 poolId, uint256 amount) override external onlyNFTVillageChief{
        projectInfo[projectId].pools[poolId].stakedAmount = amount;
    }

    function setRewardPerBlock(uint256 projectId, uint256 poolId, uint256 rewardId, uint256 rewardPerBlock) override external onlyNFTVillageChief{
        rewardInfo[projectId][poolId][rewardId].rewardPerBlock = rewardPerBlock;
    }

    function setLastRewardBlock(uint256 projectId, uint256 poolId, uint256 rewardId, uint256 lastRewardBlock) override external onlyNFTVillageChief{
        rewardInfo[projectId][poolId][rewardId].lastRewardBlock = lastRewardBlock;
    }

    function setRewardPerShare(uint256 projectId, uint256 poolId, uint256 rewardId, uint256 accRewardPerShare) override external onlyNFTVillageChief{
        rewardInfo[projectId][poolId][rewardId].accRewardPerShare = accRewardPerShare;
    }

    function setRewardSupply(uint256 projectId, uint256 poolId, uint256 rewardId, uint256 supply) override external onlyNFTVillageChief{
    ...
    function pendingRewards(uint256 projectId, uint256 poolId, UserInfo calldata user, UserRewardInfo[] calldata userReward) onlyNFTVillageChief override external view returns (uint256[] memory) {
        ProjectInfo storage project = projectInfo[projectId];
        PoolInfo storage pool = project.pools[poolId];
