...
contract Sale is ReentrancyGuard, Ownable {
        ...
        returns (bool)
    {
        ...
        require(tokens >= presale[_id].minTokenToBuy, "Insufficient amount!");

        if (userVesting[_msgSender()][_id].totalAmount > 0) {
            userVesting[_msgSender()][_id].totalAmount += tokens;
        } else {
        ...
        returns (bool)
    {
        ...
        uint256 tokens = ethToTokens(_id, msg.value);
        require(tokens >= presale[_id].minTokenToBuy, "Insufficient amount!");
        // uint256 usdPrice = (amount * presale[_id].price) / (10**18);
        ...
        if (userVesting[_msgSender()][_id].totalAmount > 0) {
            ...
            userVesting[_msgSender()][_id] = Vesting(
                0, // Last claimed at
                false, // is TGE Claimed
                _presale.vestingStartTime, // claimStart
