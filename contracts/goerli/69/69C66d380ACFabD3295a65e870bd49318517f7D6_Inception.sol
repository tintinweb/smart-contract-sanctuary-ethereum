...
contract Inception is ERC721, ERC2981, BoolMap {
    ...
    constructor(address _signer, address ownerAddress) ERC721("INCEPTION", "icNFT") {
    ...
    function insiderMint(uint16 index, bytes calldata signature) external payable notBot {
        require(saleState == 1, "insiderMint is not active");
    ...
    function fellowMint(uint16 index, bytes calldata signature) external payable notBot {
        require(saleState == 2, "fellowmint is not active");
...
Goal is storing 255 boolean in one slot. First slot cannot be changed by users.
*/
