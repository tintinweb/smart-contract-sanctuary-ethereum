...
contract EscrowProducer is Utilities, Ownable, AssetType{
    ...
    function findMatchedAssetsArray(uint _escrowID, string[] memory _collectedAssets) internal returns(string[] memory){   
    ...
    function findMatchedAssets(uint _escrowID, string[] memory _collectedAssets) internal returns(string memory,address, string[] memory,uint,uint){
        string[] memory matchedAssets = findMatchedAssetsArray(_escrowID,_collectedAssets);
      
        uint totalRedeemedAmount = matchedAssets.length*producerArrayMap[_escrowID].amount/producerArrayMap[_escrowID].postedAssets.length;
        ...
        return (producerArrayMap[_escrowID].escrowName,producerArrayMap[_escrowID].producer, matchedAssets,producerRedeemedAmount,providerRedeemedAmount);
...
contract EscrowProvider is Utilities, Ownable{
    ...
    function addNewProvider(uint _escrowID, string memory _escrowName, address _producer,address _provider,string[] memory _collectedAssets, uint _redeemedAmount, uint _rebateReceivedAmount) internal returns(ProviderStruct memory){
    ...
    function getProviderInfoArray() public view returns(ProviderInfoStruct[] memory){
        ...
        for (uint i = 0; i < currentProviderCount; i++)
        {
            ...
            providerInfo.quantity = providerArrayMap[i].collectedAssets.length;
    ...
    function getProviderInfoByID(uint _providerID) public view returns(ProviderInfoStruct memory){
        ...
        providerInfo.quantity = providerArrayMap[_providerID].collectedAssets.length;
...
contract REscrow_v1_4 is EscrowProducer, EscrowProvider {
    ...
    function addProviderUSDT(string[] memory _collectedAssets) public returns(string memory){
        ...
        {
            (string memory producerEscrowName,address producerAddress, string[] memory matchedAssets, uint rebateReceivedAmount,uint redeemedAmount) = EscrowProducer.findMatchedAssets(i, _collectedAssets);
            ProviderStruct memory newProvider;
            newProvider = EscrowProvider.addNewProvider(i, producerEscrowName, producerAddress, msg.sender,_collectedAssets,redeemedAmount,rebateReceivedAmount);
