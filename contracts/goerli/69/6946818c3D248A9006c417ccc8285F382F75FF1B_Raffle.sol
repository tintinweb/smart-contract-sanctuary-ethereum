  ...
  Ticket
{
  ...
  uint256 public constant WAIT_PERIOD_FOR_RNG = 1 days;
  //VRF randomness request variables and states
  ...
  //raffle variables and states
  RaffleItem public raffleItem;
  IERC20Upgradeable public rafflePaymentToken;
  State public state;
  uint256 public ticketPrice;
  uint256 public startTime;
  uint256 public endTime;
  uint256 public cancelTimeStamp;
  uint16 public ticketsSold;
  uint16 public ticketsGiveAway;
  mapping(address => uint16) public purchaseCount;
  WinnerInfo public winner;
  DrawInfo public drawInfo;
  ICoCreateProject public coCreateProject;
  ...
  function initialize(
    ...
    address owner,
    address _coCreateProject,
    uint64 _subscriptionId,
    bytes32 _keyHash,
    uint16 _totalTickets,
    RaffleItem memory _raffleItem,
    address _rafflePaymentToken,
    ...
    __Ticket_init(_totalTickets);
    transferOwnership(owner);
    subscriptionId = _subscriptionId;
    keyHash = _keyHash;
    raffleItem = _raffleItem;
    ticketPrice = _ticketPrice;
    startTime = _startTime;
    endTime = _startTime + _duration;
    state = State.WaitingForRaffleItem;
    callbackGasLimit = 100000;
    requestConfirmations = 20;
    numWords = 1;
    rafflePaymentToken = IERC20Upgradeable(_rafflePaymentToken);
    coCreateProject = ICoCreateProject(_coCreateProject);
    emit RaffleCreated(
      _name,
      _description,
      owner,
    ...
    emit StateChanged(State.WaitingForRaffleItem);
  }

  function verifyRaffleItem() external {
    require(state == State.WaitingForRaffleItem, "Must be in WaitingForRaffleItem");
    RaffleItem memory item = raffleItem;
    if (item.rewardType == RaffleRewardType.ERC721) {
      if (IERC721(item.rewardContract).ownerOf(item.tokenId) == address(this)) {
        state = State.WaitingForStart;
        emit StateChanged(State.WaitingForStart);
      }
    } else if (item.rewardType == RaffleRewardType.ERC1155) {
      if (IERC1155(item.rewardContract).balanceOf(address(this), item.tokenId) == item.amount) {
        state = State.WaitingForStart;
        emit StateChanged(State.WaitingForStart);
      }
    } else if (item.rewardType == RaffleRewardType.ERC20) {
      if (IERC20Upgradeable(item.rewardContract).balanceOf(address(this)) >= item.amount) {
        state = State.WaitingForStart;
        emit StateChanged(State.WaitingForStart);
      }
    }
  ...
  function giveawayTickets(GiveAwayInfo[] memory receivers) external onlyOwner {
    require(
      state == State.WaitingForStart || state == State.SellingTickets,
      "Must be in WaitingForRaffleItem or SellingTickets"
    );
    if (state == State.WaitingForStart) {
      if (block.timestamp > startTime && block.timestamp < endTime) {
        state = State.SellingTickets;
        emit StateChanged(State.SellingTickets);
      }
    }
    require(block.timestamp < endTime, "Raffle ended");

    for (uint256 i = 0; i < receivers.length; i++) {
      GiveAwayInfo memory item = receivers[i];

      assignTickets(item.giveAwayReceiver, item.numTickets);
      ticketsGiveAway += item.numTickets;
      require(totalTickets == ticketsLeft + ticketsSold + ticketsGiveAway, "invalid count");
    }
    if (ticketsLeft == 0) {
      performDraw();
    }
  ...
  function purchaseTickets(uint16 count) external {
    if (state == State.WaitingForStart) {
      if (block.timestamp > startTime && block.timestamp < endTime) {
        state = State.SellingTickets;
        emit StateChanged(State.SellingTickets);
    ...
    require(state == State.SellingTickets, "Raffle not in SellingTickets state");
    require(block.timestamp < endTime, "Raffle sale has ended");
    require(count > 0, "Ticket count invalid");
    //transfer and check amount paid
    uint256 totalPrice = count * ticketPrice;
    uint256 balancePrevious = rafflePaymentToken.balanceOf(address(this));
    rafflePaymentToken.safeTransferFrom(msg.sender, address(this), totalPrice);
    uint256 balanceAfterTransfer = rafflePaymentToken.balanceOf(address(this));
    require(balanceAfterTransfer == (balancePrevious + totalPrice), "Invalid amount paid for raffle purchase");
    //assign tickets
    assignTickets(msg.sender, count);
    ticketsSold += count;
    require(totalTickets == ticketsSold + ticketsGiveAway + ticketsLeft, "Invalid count detected");
  ...
  function performDraw() public {
    require(ticketsLeft == 0, "Not all tickets sold");
    require(state == State.SellingTickets, "Invalid state");
    require(!drawInfo.performed, "Draw already performed");
    drawInfo = DrawInfo(true, block.timestamp, msg.sender);
    state = State.WaitingForRNG;
    emit StateChanged(State.WaitingForRNG);
    requestRandomWords();
  }

  function cancelBeforeStart() external onlyOwner {
    require(
      state == State.WaitingForRaffleItem || state == State.WaitingForStart,
      "Must be in WaitingForRaffleItem or WaitingForStart"
    );
  ...
  function cancel() external {
    require(
      state == State.SellingTickets || state == State.WaitingForStart || state == State.WaitingForRNG,
      "Invalid state"
    );
    require(block.timestamp > endTime, "Raffle hasn't ended");
    if (state == State.WaitingForRNG) {
      require(
        block.timestamp > drawInfo.timestamp + WAIT_PERIOD_FOR_RNG,
        "minimum period for wait for RNG has not passed"
      );
    } else {
      require(ticketsLeft > 0, "No unsold tickets");
    }
    state = State.Cancelled;
    emit StateChanged(State.Cancelled);
    cancelTimeStamp = block.timestamp;
  ...
  function transferRaffleItemToWinnerIfCompleted() external {
    require(state == State.Completed, "Must be in Completed");
    RaffleItem memory item = raffleItem;
    if (item.rewardType == RaffleRewardType.ERC20) {
      IERC20Upgradeable(item.rewardContract).safeTransfer(winner.winnerAddress, item.amount);
    } else if (item.rewardType == RaffleRewardType.ERC721) {
      IERC721(item.rewardContract).safeTransferFrom(address(this), winner.winnerAddress, item.tokenId);
    } else if (item.rewardType == RaffleRewardType.ERC1155) {
      IERC1155(item.rewardContract).safeTransferFrom(
        address(this),
  ...
  function transferRaffleItemToOwnerIfCancelled() external {
    require(state == State.Cancelled, "Must be in Cancelled state");
    RaffleItem memory item = raffleItem;
    if (item.rewardType == RaffleRewardType.ERC20) {
      IERC20Upgradeable(item.rewardContract).safeTransfer(owner(), item.amount);
    } else if (item.rewardType == RaffleRewardType.ERC721) {
      IERC721(item.rewardContract).safeTransferFrom(address(this), owner(), item.tokenId);
    } else if (item.rewardType == RaffleRewardType.ERC1155) {
      IERC1155(item.rewardContract).safeTransferFrom(address(this), owner(), item.tokenId, item.amount, "");
    }
  }

  function transferRefundIfCancelled() external {
    require(state == State.Cancelled, "Must be in Cancelled State");
    uint256 refundAmount = purchaseCount[msg.sender] * ticketPrice;
    if (refundAmount > 0) {
      purchaseCount[msg.sender] = 0;
      rafflePaymentToken.safeTransfer(msg.sender, refundAmount);
    }
  }

  function transferPaymentTokenToOwnerIfCompleted() external {
    require(state == State.Completed, "Must be in Completed State");
    uint256 balance = rafflePaymentToken.balanceOf(address(this));
    rafflePaymentToken.safeTransfer(owner(), balance);
  }

  function requestRandomWords() internal returns (uint256 requestId) {
    require(state == State.WaitingForRNG, "Invalid state");
    ...
    requests[requestId] = RequestStatus({ randomWords: new uint256[](0), fulfilled: false });
  ...
  function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
    ...
    uint16 winningTicket = uint16(requests[_requestId].randomWords[0] % totalTickets);
    winner.ticketNumber = winningTicket;
    winner.timestamp = block.timestamp;
    winner.winnerAddress = findOwnerOfTicketNumber(winningTicket);
    state = State.Completed;
    emit StateChanged(State.Completed);
    require(winner.winnerAddress != address(0), "Invalid winner");
  ...
  function setChainLinkRequestConfigs(
...
abstract contract Ticket is Initializable {
  ...
  uint16 internal totalTickets;
  uint16 internal ticketsLeft;

  TicketNumberRange[] private ticketNumberRanges;
  mapping(address => uint16) private addressToAssignedCountMap;
  mapping(address => uint16[]) private addressToAssignedTicketNumberRangesMap;

  /* solhint-disable func-name-mixedcase */
  function __Ticket_init(uint16 _totalTickets) internal onlyInitializing {
    /* solhint-disable func-name-mixedcase */
    __Ticket_init_unchained(_totalTickets);
  }

  function __Ticket_init_unchained(uint16 _totalTickets) internal onlyInitializing {
    totalTickets = _totalTickets;
    ticketsLeft = _totalTickets;
  }

  function assignTickets(address owner, uint16 count) internal {
    require(totalTickets > 0, "All tickets are assigned");
    require(ticketsLeft >= count, "Not enough tickets left");

    uint16 from = totalTickets - ticketsLeft;
    ticketsLeft -= count;
    TicketNumberRange memory ticketNumberRange = TicketNumberRange({ owner: owner, from: from, to: from + count - 1 });
    ticketNumberRanges.push(ticketNumberRange);
    addressToAssignedCountMap[owner] += count;
    addressToAssignedTicketNumberRangesMap[owner].push(uint16(ticketNumberRanges.length - 1));
    assert(ticketNumberRanges[ticketNumberRanges.length - 1].to == totalTickets - ticketsLeft - 1);

    emit TicketsAssigned(ticketNumberRange, ticketsLeft);
  }

  function findOwnerOfTicketNumber(uint16 ticketNumber) public view returns (address) {
    require(ticketNumber < totalTickets, "Ticket number does not exist");
    require(ticketsLeft == 0, "Not all tickets are assigned");

    uint16 ticketNumberRangesLength = uint16(ticketNumberRanges.length);
    assert(ticketNumberRanges[0].from == 0);
    assert(ticketNumberRanges[ticketNumberRangesLength - 1].to == totalTickets - 1);
    ...
    while (ownerAddress == address(0)) {
      pivot = (left + right) / 2;
      TicketNumberRange memory ticketNumberRange = ticketNumberRanges[pivot];
...
abstract contract ERC165Upgradeable is Initializable, IERC165Upgradeable {
    /**
     * @notice This function is used to initialize the ERC165 interface.
     * @dev This function should only be called during the initialization phase.
     */
    function __ERC165_init() internal onlyInitializing {
    }

    /**
     * @notice This function is used to initialize the ERC165 contract.
     * @dev This function should only be called during the initialization of the contract.
     */
...
interface ICoCreateLaunch {
  ...
  function getCoCreateProjects() external view returns (address[] memory);
...
abstract contract ERC1967UpgradeUpgradeable is Initializable {
    /**
     * @notice This function is used to initialize the ERC1967Upgrade contract.
     * @dev This function should only be called during the initialization of the contract.
     */
