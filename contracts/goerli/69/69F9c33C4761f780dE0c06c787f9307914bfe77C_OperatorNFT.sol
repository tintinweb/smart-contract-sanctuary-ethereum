    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function levels(uint256 version)
        external
        view
     ...
     */
    function levelOfToken(uint256 tokenId)
        public
        view
        returns (ERC721AStorageCustom.Level memory)
    {
     ...
     */
    function tokenListOfAll(uint256 page, uint256 pageSize)
        external
        view
        returns (ERC721AStorageCustom.TokenWithLevel[] memory)
    {
        uint256 startIndex = (page - 1) * pageSize;
        uint256 endIndex = page * pageSize;
     ...
     */
    function setTokenSupply(uint256 level, uint256 newLevelTokenSupply)
        external
        onlyRole(WORKER_ROLE)
    {
     ...
     * @notice - Only admin can call this function while not in period.
     */
    function startPeriod(uint256 newPeriodTokenSupply)
        external
        onlyRole(WORKER_ROLE)
        whenNotInPeriod
    {
    ...
    function _mint(
        ...
        if (cur >= data._sumOfPeriodTokenSupply) {
        ...
        IERC20Upgradeable(ERC721AStorageCustom.layout()._paymentContract)
            .safeTransferFrom(
                _msgSender(),
                address(this),
                data._levels[data._levelVersion][level - 1].price
            );
    }
     ...
     * @param newLevel - new level
     */
    function _setLevel(ERC721AStorageCustom.Level[] calldata newLevel)
        internal
    {
        ERC721AStorageCustom.Layout storage data = ERC721AStorageCustom
            .layout();
     ...
     * @param newLevelTokenSupply - new token supply of the level
     */
    function _setTokenSupply(uint256 level, uint256 newLevelTokenSupply)
        internal
    {
        ERC721AStorageCustom.Layout storage data = ERC721AStorageCustom
            .layout();
    ...
    AccessControlUpgradeable
{
     ...
     * @notice - Only admin can call this function.
     */
    function batchGrantWorker(address[] memory newWorkers)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _batchGrantWorker(newWorkers);
