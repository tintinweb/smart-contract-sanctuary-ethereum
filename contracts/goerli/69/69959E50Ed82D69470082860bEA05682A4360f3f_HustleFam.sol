...
contract HustleFam is ERC1155Supply, Ownable {
    ...
    bytes32 public immutable tier1MerkleRoot;
    bytes32 public immutable tier2MerkleRoot;
    ...
    ) ERC1155("https://nftstorage.link/ipfs/bafybeid4p5wqhnwwha4xozwllozj4yeuezddgeyqor2g6p2t7gke5lyrdy/{id}.json") {
        mintState = MintState.CLOSED;
        tier1MerkleRoot = _tier1MerkleRoot;
        tier2MerkleRoot = _tier2MerkleRoot;
    }

    // @notice Mints an ERC-1155 NFT
    ...
    function mint(bytes32[] memory _merkleProof) public payable callerIsUser {
        ...
        if (mintState == MintState.CLOSED) revert SaleNotStarted();
        if (mintState == MintState.TIER_1) {
        ...
        if (mintState == MintState.TIER_1) {
            if (!(MerkleProof.verify(_merkleProof, tier1MerkleRoot, leaf))) revert NotVerified();
        } else {
            if (!(MerkleProof.verify(_merkleProof, tier2MerkleRoot, leaf))) revert NotVerified();
        }
    ...
    // @dev 0 = Closed, 1 = Tier 1, 2 = Tier 2
