...
contract BridgeFactory {
    ...
    event ProjectCreated(address proj, Company owner, uint256 projectId);
    ...
    event AddedNftSold(address buyer, uint256 companyId, uint256 projectId, uint256 quantity);
    event AddedCompanyRevenue(uint256 companyId, uint256 price, uint256 projectId);
    ...
    function createProject(
        ...
        emit ProjectCreated(address(project), owner, projectNumber);
    }
    ...
    function createCompany(address _wallet, address _admin) public isBridgeAdmin {
        ...
        companies[companyNumber] = Company({
            wallet: _wallet,
            admin: _admin,
            companyProjects: projectInstances,
            totalNFTsSold: 0,
            totalRevenue: 0
        });
        companyNumber++;
    ...
    function addCompanyNFTsSold(
        ...
        emit AddedNftSold(_buyer, _companyID, _projectId, _quantity);
    }
    ...
    function addTotalRevueToCompany(
        ...
        emit AddedCompanyRevenue(_companyID, _price, _projectId);
    }
    ...
    function getProjectNumber() public view returns (uint256) {
...
contract Bridge is ERC721A, Ownable {
    ...
    event AdjustedDistributionRecipient(address newRecipient, uint256 recipientPercentage, address[] recipients, uint256[] percentages);
    ...
    event TokensMinted(address owner, uint256 price, uint256 quantity);
    ...
    function addAddressToMintingProceeds(
        ...
        emit AdjustedDistributionRecipient(_newRecipient, _newPercentage, mintingDistributionRecipients, _newRecipientPercentages);
    }
    ...
    function _mint(
        ...
        factory.addCompanyNFTsSold(msg.sender, _companyID, _quantity, _projectId);
        factory.addTotalRevueToCompany(_companyID, _price, _projectId);

        emit TokensMinted(msg.sender, _price, _quantity);
    }
}
