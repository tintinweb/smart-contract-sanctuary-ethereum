...
contract ERC1155 {
    ...
    event ApprovalsForAll(address indexed _owner, address indexed _operator, bool _approved);
    ...
    public view returns(uint256[] memory){
        require(accounts.length == ids.length, "Accounts and ids are not the same lenght");
    ...
    function setApprovalForAll(address operator, bool approved) public {
        ...
        emit ApprovalsForAll(msg.sender, operator, approved);
    }
    ...
    function safeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes calldata _data) external {
        require(msg.sender == from || isApprovedForAll(from, msg.sender), "msg.sender is not the operator");
