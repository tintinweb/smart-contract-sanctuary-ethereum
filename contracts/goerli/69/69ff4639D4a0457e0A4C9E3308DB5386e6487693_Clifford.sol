...
contract Metadata is Ownable {
    ...
    function getIndexes(uint256 _tokenId) public view returns (uint[] memory indexes) {
        uint[] memory shell = getShell(_tokenId);
        uint[] memory rightWall = getRWPositions(_tokenId);
        uint[] memory leftWall = getLWPositions(_tokenId);
        uint[] memory floor = getFPositions(_tokenId);
        uint[] memory allIndexes = combineUintArrays(shell, combineUintArrays(rightWall, combineUintArrays(leftWall, floor)));
        return allIndexes;
    }
