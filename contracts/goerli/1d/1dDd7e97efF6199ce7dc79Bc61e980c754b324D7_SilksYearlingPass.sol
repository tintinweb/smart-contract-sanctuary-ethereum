...
contract SilksYearlingPass is ERC721Redeemable, CollaborationPricing, WhitelistPricing {
    ...
    constructor(
        ...
        collaborations[address(0)] = Collaboration(_publicSaleInfo[0], false, _publicSaleInfo[1], _publicSaleInfo[2], true);
    }
    ...
    function whitelistGatedPurchase(string calldata _id, bytes32[] calldata merkleProof) public payable whenNotPaused {
    ...
    function airDrop(address _to, uint256 _amount) public onlyOwner whenNotPaused {
        _mintPasses(_to, _amount);
    }
    
    function genesisAvatarGatedOwnerMint(address _to, uint256[] calldata _tokenIds) external payable onlyOwner {
    ...
    function genesisAvatarsGatedPurchase(uint256[] calldata _tokenIds) public payable whenNotPaused {
        tokenGatedPurchase(avatarsAddress, _tokenIds);
    }
    
    function tokenGatedPurchase(address _contractAddress, uint256[] calldata _tokenIds) public payable whenNotPaused {
    ...
    function _checkAndMintPasses(address _contractAddress, address _to, uint256[] calldata _tokenIds) internal {
    ...
    function checkPurchaseConditions(address _to, uint256 price, uint256 maxPerTx, uint256 maxPerWallet) internal {
        ...
        require(maxPerWallet == 0 || (balanceOf(_to) + (msg.value / price)) <= maxPerWallet, "PER_WALLET_ERROR");
    }
    ...
    function checkIfTokenUsedBatch(address _contractAddress, uint256[] calldata _tokenIds) view public returns (bool[] memory) {
...
contract CollaborationPricing is Ownable {
    ...
    function getCollaboration(address _contractAddress) public view returns (uint price, bool paused, uint maxPerTx, uint maxPerWallet, bool valid) {
        return (collaborations[_contractAddress].price, collaborations[_contractAddress].paused, collaborations[_contractAddress].maxPerTx, collaborations[_contractAddress].maxPerWallet, collaborations[_contractAddress].valid);
    }
    
    function setCollaboration(address _contractAddress, uint _price, bool _paused, uint _maxPerTx, uint _maxPerWallet) external onlyOwner {
        collaborations[_contractAddress] = Collaboration(_price, _paused, _maxPerTx, _maxPerWallet, true);
    }
}
