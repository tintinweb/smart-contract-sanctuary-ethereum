...
contract NyxMintProposal is Ownable, converter {
    string public constant name = "Nyx Proposal - Mint";

    // Enums
    ////////////////////
    enum MintAction{START, STOP}
    ...
    function parseProposalParams(bytes[] memory params)
        public pure
        returns (MintProposalParams memory)
    {
        require(params.length == 2, "wrong format");

        return MintProposalParams(MintAction(converter.bytesToUint(params[0])),
            converter.bytesToString(params[1]));
    }
    ...
    function parseProposalParams(bytes memory params)
        ...
        returns (MintProposalParams memory)
    {
        (bytes memory param1, bytes memory param2) = abi.decode(params, (bytes, bytes));
        return MintProposalParams(MintAction(converter.bytesToUint(param1)),
            converter.bytesToString(param2));
    }
    ...
    function parseProposalParams2(bytes memory params)
        ...
        returns (MintProposalParams memory)
    {
        (uint256 param1, string memory param2) = abi.decode(params, (uint256, string));
        return MintProposalParams(MintAction(param1),
            param2);
    }

    function proposalToBytes(MintProposal memory proposal)
        public pure
        returns (bytes memory)
    {
        return abi.encode(proposal.action, proposal.reason);
    }
    ...
    function createProposal(uint256 proposalId, bytes[] memory params)
        ...
        returns (bytes memory)
    {
        MintProposalParams memory parsedParams = parseProposalParams(params);
        MintProposal memory proposal = MintProposal(proposalId, parsedParams.action, parsedParams.reason);
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        MintProposalParams memory params = parseProposalParams2(proposalBytes);
        bytes[] memory prop = new bytes[](2);
        prop[0] = abi.encode(params.action);
        prop[1] = abi.encode(params.reason);
        return prop;
    }

    function getProposals(bytes[] memory proposalBytesArr)
        public pure
        returns (bytes[2][] memory props)
    {
        props = new bytes[2][](proposalBytesArr.length);
        for (uint256 idx = 0; idx < proposalBytesArr.length; idx++)
        {
            MintProposalParams memory params = parseProposalParams2(proposalBytesArr[idx]);
            props[idx] = [abi.encode(params.action), abi.encode(params.reason)];
        }
    }
}
