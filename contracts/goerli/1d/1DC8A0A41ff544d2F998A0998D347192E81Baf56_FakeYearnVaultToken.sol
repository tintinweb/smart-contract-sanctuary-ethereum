...
contract FakeYearnVaultToken is ERC20 {
    ...
    // @param n name
    // @param s symbol
    // note that yVaults match their underlying asset's decimals
    constructor(address u, string memory n, string memory s) ERC20(n, s, 6) {
        underlyingDecimals = IERC20(u).decimals();
        underlyingAsset = u;
    }

    /// @notice Returns the price for a single Yearn Vault share.
    /// @dev Returns amount of underlying needed to acquire 1 YearnVault share
    /// @dev total vault assets (underlying) / total token supply (shares) (calculated not cached)
    function pricePerShare() external view returns (uint256) {
        // Default to a price of 1 if there are no shares
        if (_totalSupply == 0) {
            return 1000000;
        }

        // Return underlying / shares available
        return
            (1e6 * IERC20(underlyingAsset).balanceOf(address(this))) /
            _totalSupply;
    }

    function internalPricePerShare() internal view returns (uint256) {
        // Default to a price of 1 if there are no shares
        if (_totalSupply == 0) {
            return 1000000;
        }
    ...
    function deposit(
        ...
        // Calculate how many shares are owed
        uint256 shares = (_amount * 1e6) / internalPricePerShare();

        // Receive the underlying from the user
    ...
    function mint(address _recipient, uint256 _shares) external {
        ...
        // Calculate amount of underlying needed to mint _shares tokens
        uint256 amount = (internalPricePerShare() * _shares) / 1e6;

        // Receive the underlying from the user
        IERC20(underlyingAsset).transferFrom(_recipient, address(this), amount);
    ...
    function withdraw(
        ...
        // Calculate how many shares are needed to withdraw _amount
        uint256 shares = (_amount * 1e6) / internalPricePerShare();

        // Transfer owed underlying to user
