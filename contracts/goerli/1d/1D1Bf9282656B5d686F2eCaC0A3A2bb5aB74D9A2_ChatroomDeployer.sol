...
contract ChatroomDeployer {
    address[] public chatrooms;
    uint256 public cost = 0.01 ether;

    function deploy() public payable {
        require(cost == msg.value, "Cost to deploy chatroom not met.");
        Chatroom chatroom = new Chatroom(msg.sender);
        chatrooms.push(address(chatroom));
    }

    function getChatrooms() public view returns (address[] memory) {
        return chatrooms;
    }
}
...
contract Chatroom is Ownable {
    ...
    constructor(address __owner) {
        ...
        participantList[__owner] = ParticipantStruct(
            __owner,
            "owner",
            block.timestamp
        );
