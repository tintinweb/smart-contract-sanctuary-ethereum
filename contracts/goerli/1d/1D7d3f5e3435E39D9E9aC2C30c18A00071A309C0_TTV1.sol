    ...
    UUPSUpgradeable
{
    ...
    function allocationMinting(address[] calldata accountList) external virtual onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view override virtual returns (string memory) {
        return _reveal ? string(abi.encodePacked(_baseURI(), tokenId.toString(), ".json")) : string(abi.encodePacked(_baseURI(), "0.json"));
    }
...
abstract contract MintingSalesUpgradeable is Initializable, ContextUpgradeable {
    ...
    struct SalesInfo {
        ...
        uint256 whitelistPrice;      // Whitelist minting price
    ...
    modifier salesValidator(uint8 requestedCount, bool isWhitelist) {
        ...
        require(msg.value == (isWhitelist ? _salesInfo.whitelistPrice.mul(requestedCount) : _salesInfo.mintPrice.mul(requestedCount))
            , "Sales: incorrect ETH for payment");
        _;
    }
     ...
     * @param whitelistPrice Whitelist minting price.
