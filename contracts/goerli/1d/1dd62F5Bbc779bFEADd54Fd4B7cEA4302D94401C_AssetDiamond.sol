...
contract AssetDiamond is SolidStateDiamond {
    using ERC165Storage for ERC165Storage.Layout;
    using ERC721MetadataStorage for ERC721MetadataStorage.Layout;
    using AssetStorage for AssetStorage.Layout;

    constructor(string memory name, string memory symbol, address spaceportAddress) {
        ERC165Storage.Layout storage erc165Storage = ERC165Storage.layout();
        erc165Storage.setSupportedInterface(type(IERC721).interfaceId, true);
        erc165Storage.setSupportedInterface(type(IERC165).interfaceId, true);

        ERC721MetadataStorage.Layout storage erc721Storage = ERC721MetadataStorage.layout();
        erc721Storage.name = name;
        erc721Storage.symbol = symbol;

        AssetStorage.Layout storage spaceportAssetStorage = AssetStorage.layout();
        spaceportAssetStorage.spaceportAddress = spaceportAddress;
    }
}
...
library AssetStorage {
    bytes32 internal constant STORAGE_SLOT = keccak256("spaceport.contracts.asset");
    ...
    struct Layout {
        ...
        uint256 currentTokenId; // default: 0
        mapping(uint256 => Asset) tokens;
        address spaceportAddress;
    }
    ...
    function getCurrentTokenId(Layout storage l) internal view returns (uint256) {
        return l.currentTokenId;
    }

    function incrementCurrentTokenId(Layout storage l) internal {
        unchecked {
            l.currentTokenId += 1;
        }
    }

    function decrementCurrentTokenId(Layout storage l) internal {
        require(l.currentTokenId > 0, "decrementCurrentTokenId: overflow");
        unchecked {
            l.currentTokenId -= 1;
        }
    }

    function resetCurrentTokenId(Layout storage l) internal {
        l.currentTokenId = 0;
    }
}