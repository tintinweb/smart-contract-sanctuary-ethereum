 ...
 * @author @inetdave
 * @notice Bidding contract developed with mechanisms for airdrops and refunds
 * @dev All function calls have been tested.
 * v.00.90.00
 */
contract InetBidMint is InetBid, AccessControl, ERC2981 {
    ...
    // ERC165
    /**
     * @dev See {InetBase-_setContractState} for implementation.
     */
    ...
    /**
     * @dev See {InetBase-_setPrices} for implementation.
     */
    function setPrices(
        uint256 _price,
    ...
    /**
     * @dev set the max bid supploy on contract InetBase
     */
     ...
     */
    // function getNumberOfBidEntries() external view {
    //     return bidderData.length()
    //     //TODO
    // }
     ...
     * @param _tos address array to mint tokens to.
    ...
    ) external onlyRole(SUPPORT_ROLE) finalPriceSet {
        require(contractState != ContractState.PAUSED, "Contract paused");
        require(_tos.length == _numbers.length, "Mismatched arrays");
        for (uint256 i; i < _tos.length; ) {
            _airdropMint(_tos[i], _numbers[i]);
            unchecked {
                i += 1;
            }
        }
...
* @dev v.00.90.00
*/
contract InetBid is InetMint{

    uint256 public constant MIN_BID = 0.04 ether;
    // uint256 public constant MAX_BID = 0.2 ether;
    ...
    event Refunded(address indexed bidder, uint256 refundAmount);
    event Minted(address indexed bidder, uint256 mintAmount);
    
    struct Bidder {
        /**
        * @dev  cumulative sum of ETH bids. this is useful incase bidders are increasing their bid
        */
        uint232 contribution; 
        /**
    ...
    */
    ...
    function bid() external payable {
        ...
        // require(contribution <= MAX_BID, "Bid high");
        bidder.contribution = uint232(contribution);
     ...
     */
    function _processBidder(address _address) internal finalPriceSet{
        ...
        emit Refunded(_address, refundValue);
 ...
 * @dev v.00.90.00
 */
contract InetMint is InetBase, ERC721A, OperatorFilterer {
    constructor() ERC721A("InetMint", "INET") {
        _registerForOperatorFiltering();
...
* @dev v.00.90.00
*/
contract InetBase is ReentrancyGuard, Ownable {
    ...
    uint256 public maxBidSupply = 3000;
    ...
    address internal constant VOID_ZERO = 0x85bc99582D5821a8cF08d135e85C8f57a426d4A9;
    address internal constant OWNER = 0x98e253c16C61DE5F1BA182A87bA1346418D5c7b1;
