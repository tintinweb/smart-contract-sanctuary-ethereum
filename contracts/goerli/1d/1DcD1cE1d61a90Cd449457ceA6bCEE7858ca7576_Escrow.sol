...
contract Escrow is Ownable, ReentrancyGuard {
     ...
     * @param signature Signer signature
     */
    struct MintLicense {
        address collection;
        uint256 tokenId;
    ...
    /// @dev License token
    ILicense private license;

    /// @dev Weth token
    IWETH private immutable weth;

    /// @dev To make fractional part
    uint256 private constant DENOMINATOR = 10000; // 100.00%

    /// @dev Max fee
    uint256 private constant MAX_FEE = 2000; // 20.00%
     ...
     * @param _signer Server address that signs messages
    ...
    function _checkCollectionSignature(MintLicense memory _params) internal {
        ...
        _checkSignature(_hash, _params.signature);
    }
    ...
    /**
     * @dev Is signer sign this transaction?
     * @param _hash Hash from untrusted source
     * @param _signature Signed trusted hash
     */
    function _checkSignature(bytes memory _hash, bytes memory _signature)
        internal
    {
        bytes32 _encoded = _hash.toEthSignedMessageHash();

        require(!alreadyExecuted[_encoded], "hash already used");
        require(_encoded.recover(_signature) == signer, "bad signature");

        alreadyExecuted[_encoded] = true;
    }
    ...
    /**
     * @dev Resale license NFT
     * @param _params Set of params for license resale
     */
    function _resaleLicense(SellLicense memory _params) internal {
        license.safeTransferFrom(
            _params.seller,
            _params.buyer,
            _params.tokenId
        );
        emit Resale(_params.tokenId, _params.seller, _params.buyer);
    }
