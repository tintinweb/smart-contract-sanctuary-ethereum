...
contract VestingUUPSUpgradeable is IVestingUUPSUpgradeable, Vesting, UUPSUpgradeable {
    ...
    function _authorizeUpgrade(address) internal view override {
        require(hasRole(UPGRADER_ROLE, msg.sender), "Vesting: caller does not have upgrader role");
    }
...
contract Vesting is IVesting, Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(VestingConfig memory cfg, address roleAdmin) public initializer {
        __Vesting_init(cfg, roleAdmin);
    }

    function __Vesting_init(VestingConfig memory cfg, address roleAdmin) internal onlyInitializing {
        __ReentrancyGuard_init();
        __AccessControl_init();
        __Vesting_init_unchained(cfg, roleAdmin);
    }

    function __Vesting_init_unchained(VestingConfig memory cfg, address roleAdmin) internal onlyInitializing {
    ...
    function changeClaimant(address newClaimant) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Vesting: only admin can change claimant");
