...
contract Refr is Ownable, ERC20("Refr", "REFR") {
    ...
    // uint public maxWallet = 1e25;
    // uint public maxTransfer = 2e24;

    uint public marketingAmt;
    uint public marketingLimit = 1e21;

    // *************  this is only for testing ******************
    uint public maxWallet = 1e27;
    uint public maxTransfer = 1e27;
    ...
    function _distributeTax() private {
        if(marketingAmt >= marketingLimit) {
            require(balanceOf(address(this)) >= marketingAmt, "err here");
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[_from] && !_isExcludedFromFee[_to]) {
            ...
            require(balanceOf(_to) + _amount <= maxWallet, "Over max balance");
