...
/// @title Mutniks User Generative Collection
/// @author 0xSimon_
contract Mutniks is ERC721AQueryable, Ownable,OperatorFilterer,ERC2981 {
    using ECDSA for bytes32;
    uint256 public maxSupply = 5000;
    uint256 constant private BITSIZE_OF_TRAIT = 16; //16 bits
    uint256 constant private DATA_ENTRY = (1 << 16) - 1;
    uint256 public maxPublicMints = 5000;
    uint256 public whitelistMintPrice;
    uint256 public publicMintPrice;
    address private signer = 0x235AAE9ed08Edfe150fd71592d51b1Af44C57034;
    string private baseURI = "https://smuphotobucket.s3.amazonaws.com/mutniks/json/";
    ...
    */
    ...
    bool public holderClaimOn;
    bool public whitelistMintOn;
    bool public publicMintOn;
   

    constructor() ERC721A("Mutniks", "MTNKS") {
        // setBaseURI("");
    ...
    */
    ...
    function packTraitArray(uint[11] calldata traitArray) public pure returns(uint){
        uint packedVal;
    ...
    function unpackPackedTraitArrayValue(uint packedVal) public pure returns(uint[11] memory traitArray) {
        for(uint i; i< traitArray.length;){
            uint unpackedVal = (packedVal >> (i*BITSIZE_OF_TRAIT)) &  DATA_ENTRY;
            traitArray[i] = unpackedVal;
            unchecked{++i;}
        }

    }
    function unpackPackedTokenTraitArray(uint tokenId) external view returns(uint[11] memory) {
    ...
    */
    ...
    function mutnikMint(uint[11] calldata traitArray,bytes memory signature) external payable {
        ...
        if(_numberMinted(_msgSender()) +1 > maxPublicMints) revert MaxMints();
        bytes32 hash = keccak256(abi.encodePacked(traitArray));
        if(hash.toEthSignedMessageHash().recover(signature) != signer) revert CombinationNotApproved();
    ...
    function mutnikMintWhitelist(uint[11] calldata traitArray,uint max,bytes memory signature) external payable {
        uint packedVal = packTraitArray(traitArray);
        uint nextTokenId = _nextTokenId();
        if(isBitpackedComboUsed[packedVal]) revert  CombinationAlreadyUsed();
        ...
        if(getNumMintedPublicOrWhitelist(_msgSender()) + 1 > max) revert MaxMints();
        bytes32 hash = keccak256(abi.encodePacked(_msgSender(),max,traitArray,"WHITELIST"));
        if(hash.toEthSignedMessageHash().recover(signature) != signer) revert CombinationNotApproved();
        isBitpackedComboUsed[packedVal] = true;
        bitpackedTokenTraitArray[nextTokenId] = packedVal;
        emit MutnikCreated(nextTokenId,traitArray);
    ...
    function holderClaimMint(uint[11] calldata traitArray,uint max,bytes memory signature) external  {
        uint packedVal = packTraitArray(traitArray);
        uint nextTokenId = _nextTokenId();
        if(isBitpackedComboUsed[packedVal]) revert  CombinationAlreadyUsed();
        if(nextTokenId + 1 > maxSupply) revert SoldOut();
        ...
        bytes32 hash = keccak256(abi.encodePacked(_msgSender(),max,traitArray));
        if(hash.toEthSignedMessageHash().recover(signature) != signer) revert CombinationNotApproved();
        isBitpackedComboUsed[packedVal] = true;
        bitpackedTokenTraitArray[nextTokenId] = packedVal;
        incrementNumClaimed(_msgSender(),1);
        emit MutnikCreated(nextTokenId,traitArray);
        _mint(_msgSender(),1);
    }
    ...
    *                     Getters
        ...............~~~~~~~~~~~~~~~...............
    */
       function isTraitArrayUsed(uint[11] calldata traitArray) external view returns(bool) {
        uint packedVal =  packTraitArray(traitArray);
        return isBitpackedComboUsed[packedVal];
    }

    function getNumMintedPublicOrWhitelist(address account)
        public
        view
        returns (uint256)
    {
    ...
    function incrementNumClaimed(address account,uint amount) private {
