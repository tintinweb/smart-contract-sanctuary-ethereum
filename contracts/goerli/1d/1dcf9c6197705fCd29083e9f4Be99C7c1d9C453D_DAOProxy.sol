...
/// @title The Worldician DAO proxy contract

// LICENSE
// DAOProxy.sol is a modified version of Compound Lab's GovernorBravoDelegator.sol:
...
//
//
// DAOProxy.sol uses parts of Open Zeppelin's Proxy.sol:
...
import "../interface/IDAOInterfaces.sol";

contract DAOProxy is DAOProxyStorage, DAOEvents {
    constructor(
        address timelock_,
        address worldician_,
        address house_,
     ...
     * @param implementation_ The  of the new implementation for delegation
     */
    function _setImplementation(address implementation_) public {
        require(
            msg.sender == admin,
            "DAOProxy::_setImplementation: admin only"
        );
        require(
            implementation_ != address(0),
            "DAOProxy::_setImplementation: invalid implementation address"
        );
...
/// @title  DAO Logic interfaces and events

// LICENSE
// DAOInterfaces.sol is a modified version of Compound Lab's GovernorBravoInterfaces.sol:
 ...
 * DAOStorageVX.
 */
contract DAOStorageV1 is DAOProxyStorage {
    ...
    /// @notice The address of the  DAO Executor DAOExecutor
    IDAOExecutor public timelock;

    /// @notice The address of the  tokens
    WorldicianTokenLike public worldician;

    HouseTokenLike public house;
...
interface WorldicianTokenLike {
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint96);

    function totalSupply() external view returns (uint96);
}

interface HouseTokenLike {
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint96);
