...
interface IUniversalEquipmentManager {
    ...
    function universalBindOrUnbind(bytes4 actionName, Object calldata from, Object calldata to, bytes4 nftType) external;
}
    ...
    ERC5050State
{
    ...
    ///@dev Direct call to bind or unbind
    function universalBindOrUnbind(bytes4 actionName, Object calldata from, Object calldata to, bytes4 nftType) 
    external onlyRole(EXTERNAL_BIND)
    {
        if (actionName == EQUIP_SELECTOR) {
            _equip(
                from,
                to,
                nftType
            );
        } else if (actionName == UNEQUIP_SELECTOR) {
           _unequip(
                from,
                to,
                nftType
    ...
    function _equip(
        ...
        // TODO: perhaps allow this? equip a power stone to a sword, equip the sword to a backpack, etc.
