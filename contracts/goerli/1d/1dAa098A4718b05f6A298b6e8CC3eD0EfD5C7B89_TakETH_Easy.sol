...
interface IDividendDistributor {
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution,uint256 _minRequiredToHold) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minRequiredToHold        = 100 * (10**9); // 100 tokens for rewards
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _minRequiredToHold) external override onlyToken {
        minPeriod           = _minPeriod;
        minDistribution     = _minDistribution;
        minRequiredToHold          = _minRequiredToHold * (10**9);
        emit DistributionCriteriaUpdated(minPeriod, minDistribution, minRequiredToHold);
    }
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        if(amount >= minRequiredToHold && shares[shareholder].amount == 0){
            addShareholder(shareholder);
        }else if(amount < minRequiredToHold && shares[shareholder].amount > 0){
            removeShareholder(shareholder);
        }
    
        if(amount < minRequiredToHold) amount = 1;
    ...
    event DistributionCriteriaUpdated(uint256 minPeriod, uint256 minDistribution, uint256 minRequiredToHold);
...
contract TakETH_Easy is IERC20, Context, ReentrancyGuard {
    ...
    uint256 public constant _initialSupply  = 1000000;
...
// = amount.mul(10**_decimals);
    ...
    // Goerli router    : 0x374F843fF0b4E1Ec9B0a14e400360034124de959
    ...
    constructor() {
        ...
        TransactionSettings.maxTxAmount = _totalSupply / 50;        // 2%
        TransactionSettings.maxWalletAmount = _totalSupply / 50;    // 2%
        LiquiditySettings.numTokensToSwap = _totalSupply / 400;     // 0.25%
    ...
    function clearStuckBalance(uint256 amountPercentage) external onlyOwner {
        ...
        payable(ecosystemAddress).transfer(
    ...
    function setTxLimit(uint256 _amount) external authorized { // percent, uint256 divisor
        // require(percent >= 1 && divisor <= 1000);
        TransactionSettings.maxTxAmount = _amount / (10**_decimals);
        emit TxLimitUpdated(TransactionSettings.maxTxAmount);
    }

    function setMaxWallet(uint256 _amount) external authorized { // percent, uint256 divisor
        // require(percent >= 1 && divisor <= 100);
        TransactionSettings.maxWalletAmount = _amount / (10**_decimals);
    ...
    function setBuyFees(uint16 _liquidityFee, uint16 _rewardFee, uint16 _ecosystemFee) external authorized {
        require(_liquidityFee + _ecosystemFee + _rewardFee <= MaxFees.totalFee);
        BuyFees = IFees({
    ...
    function setTransferFees(uint16 _liquidityFee, uint16 _rewardFee, uint16 _ecosystemFee) external authorized {
        require(_liquidityFee + _ecosystemFee + _rewardFee <= MaxFees.totalFee);
        TransferFees = IFees({
            liquidityFee: _liquidityFee,
            ecosystemFee: _ecosystemFee,
    ...
    function setSellFees(uint16 _liquidityFee, uint16 _rewardFee, uint16 _ecosystemFee) external authorized {
        require(_liquidityFee + _ecosystemFee + _rewardFee <= MaxFees.totalFee);
        SellFees = IFees({
            liquidityFee: _liquidityFee,
            ecosystemFee: _ecosystemFee,
    ...
    function setFeeReceivers(address _liquidityAddress, address _ecosystemAddress) public onlyOwner {
        liquidityAddress = _liquidityAddress;
        ecosystemAddress = _ecosystemAddress;
        emit FeeReceiversUpdated(_liquidityAddress, _ecosystemAddress);
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount) external authorized{
        ...
        LiquiditySettings.numTokensToSwap = _amount / (10**_decimals);
   ...
   function setAutoLiquifyEnabled(bool _enabled) public authorized {
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _minRequiredToHold) external authorized {
    ...
    function swapBack() internal swapping {
        ...
        if (LiquiditySettings.liquidityFeeAccumulator >= LiquiditySettings.numTokensToSwap && LiquiditySettings.autoLiquifyEnabled) {
            ...
            uint256 amountEthReward = (amountEth *
                (BuyFees.rewardFee + SellFees.rewardFee)) /
                (BuyFees.totalFee + SellFees.totalFee);

            if(fundRewards) {
                try distributor.deposit{value: amountEthReward}() {} catch {}
                (bool ecosystem, ) = payable(ecosystemAddress).call{ value: amountEthEcosystem, gas: 30000}("");
    ...
    function getAccumulatedFees() external view returns (uint256 collectedFees, uint256 currentBalance) {
        ...
        currentBalance = ecosystemAddress.balance;
    }
    ...
    event FeeReceiversUpdated(address liquidityAddress, address ecosystemAddress);
    event Launched();
