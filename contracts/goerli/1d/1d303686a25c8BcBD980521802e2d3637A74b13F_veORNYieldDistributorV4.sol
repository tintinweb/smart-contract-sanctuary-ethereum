...
// |                                                                  |
// ====================================================================
// ======================veORNYieldDistributorV4=======================
// ====================================================================
// Distributes Frax protocol yield based on the claimer's veORN balance
// V3: Yield will now not accrue for unlocked veORN
...
// Originally inspired by Synthetix.io, but heavily modified by the Frax team (veORN portion)
...
contract veORNYieldDistributorV4 is Owned, ReentrancyGuard {
    ...
    // Yield tracking
    uint256 public yieldPerVeORNStored = 0;
    mapping(address => uint256) public userYieldPerTokenPaid;
    mapping(address => uint256) public yields;

    // veORN tracking
    uint256 public totalVeORNParticipating = 0;
    uint256 public totalVeORNSupplyStored = 0;
    mapping(address => bool) public userIsInitialized;
    mapping(address => uint256) public userVeORNCheckpointed;
    mapping(address => uint256) public userVeORNEndpointCheckpointed;
    ...
    function fractionParticipating() external view returns (uint256) {
        return totalVeORNParticipating.mul(PRICE_PRECISION).div(totalVeORNSupplyStored);
    }

    // Only positions with locked veORN can accrue yield. Otherwise, expired-locked veORN
    // is de-facto rewards for ORN.
    function eligibleCurrentVeORN(address account) public view returns (uint256 eligible_veorn_bal, uint256 stored_ending_timestamp) {
        uint256 curr_veorn_bal = veORN.balanceOf(account);
        
        // Stored is used to prevent abuse
        stored_ending_timestamp = userVeORNEndpointCheckpointed[account];

        // Only unexpired veORN should be eligible
        if (stored_ending_timestamp != 0 && (block.timestamp >= stored_ending_timestamp)){
            eligible_veorn_bal = 0;
        }
        else if (block.timestamp >= stored_ending_timestamp){
            eligible_veorn_bal = 0;
        }
        else {
            eligible_veorn_bal = curr_veorn_bal;
        }
    }
    ...
    function yieldPerVeORN() public view returns (uint256) {
        if (totalVeORNSupplyStored == 0) {
            return yieldPerVeORNStored;
        } else {
            return (
                yieldPerVeORNStored.add(
                    lastTimeYieldApplicable()
                        .sub(lastUpdateTime)
                        .mul(yieldRate)
                        .mul(1e18)
                        .div(totalVeORNSupplyStored)
                )
            );
        }
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        // Get eligible veORN balances
        (uint256 eligible_current_veorn, uint256 ending_timestamp) = eligibleCurrentVeORN(account);

        // If your veORN is unlocked
        uint256 eligible_time_fraction = PRICE_PRECISION;
        if (eligible_current_veorn == 0){
        ...
        // If the amount of veORN increased, only pay off based on the old balance
        // Otherwise, take the midpoint
        uint256 veorn_balance_to_use;
        {
            uint256 old_veorn_balance = userVeORNCheckpointed[account];
            if (eligible_current_veorn > old_veorn_balance){
                veorn_balance_to_use = old_veorn_balance;
            }
            else {
                veorn_balance_to_use = ((eligible_current_veorn).add(old_veorn_balance)).div(2); 
            }
        }

        return (
            veorn_balance_to_use
                .mul(yieldPerVeORN().sub(userYieldPerTokenPaid[account]))
    ...
    function _checkpointUser(address account) internal {
        ...
        // Get the old and the new veORN balances
        uint256 old_veorn_balance = userVeORNCheckpointed[account];
        uint256 new_veorn_balance = veORN.balanceOf(account);

        // Update the user's stored veORN balance
        userVeORNCheckpointed[account] = new_veorn_balance;

        // Update the user's stored ending timestamp
        IveORN.LockedBalance memory curr_locked_bal_pack = veORN.locked(account);
        userVeORNEndpointCheckpointed[account] = curr_locked_bal_pack.end;

        // Update the total amount participating
        if (new_veorn_balance >= old_veorn_balance) {
            uint256 weight_diff = new_veorn_balance.sub(old_veorn_balance);
            totalVeORNParticipating = totalVeORNParticipating.add(weight_diff);
        } else {
            uint256 weight_diff = old_veorn_balance.sub(new_veorn_balance);
            totalVeORNParticipating = totalVeORNParticipating.sub(weight_diff);
        }
    ...
    function _syncEarned(address account) internal {
        if (account != address(0)) {
            ...
            userYieldPerTokenPaid[account] = yieldPerVeORNStored;
        }
    }
    ...
    function sync() public {
        // Update the total veORN supply
        yieldPerVeORNStored = yieldPerVeORN();
        totalVeORNSupplyStored = veORN.totalSupply();
