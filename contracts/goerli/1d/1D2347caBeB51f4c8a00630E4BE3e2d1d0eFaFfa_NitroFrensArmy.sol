...
contract NitroFrensArmy is ERC20, Ownable {
    ...
    event frenWalletUpdated(address indexed newWallet, address indexed oldWallet);

    constructor(address _nfa_erc721) ERC20("Nitro Frens Armmy", "NFA") {
        NFA_ERC721 = _nfa_erc721;
    ...
    function enableTrading() external returns (bool) {
        require(msg.sender == owner() || msg.sender == NFA_ERC721, "Not Fren Controller");
        _openTrade = true;
        swapEnabled = true;
    ...
    function setNFAddress(address _nfa_erc721) external onlyOwner {
        NFA_ERC721 = _nfa_erc721;
    }
    ...
    function updateFrenWallet(address newFrenWallet) external onlyOwner {
        emit frenWalletUpdated(newFrenWallet, frenWallet);
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || IToken(NFA_ERC721).balanceOf(tx.origin) > 0) {
            takeFee = false;
        }
