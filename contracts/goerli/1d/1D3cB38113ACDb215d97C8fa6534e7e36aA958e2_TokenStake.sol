...
//        uint balance = IERC20(token).balanceOf(address(this));
    ...
    function withdraw(address to, uint amount) public override {
        require(amount <= reserve, "insufficient funds");
        _safeTransfer(token, to, amount);
        reserve -= amount;
    }
}

contract TokenStake is Initializable, OwnableUpgradeable, PausableUpgradeable {
    using AddressUpgradeable for address;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable public arsw;

    modifier onlyEOA() {
        require(_msgSender() == tx.origin, "TokenStake: not eoa");
        _;
    }
    ...
    struct UserLPInfo {
        uint amount;
        uint rewardDebt;
        uint firstDepositedTime;
        uint averageDepositedTime;
    }
    ...
    uint public rewardPerBlock; //for arsw reward
    ...
    function initialize(IERC20Upgradeable arsw_,
        uint rewardPerBlock_, uint startBlock_) external virtual initializer {
        __ArswFarming_init(arsw_, rewardPerBlock_, startBlock_);
    }

    function __ArswFarming_init(IERC20Upgradeable arsw_,
        uint rewardPerBlock_, uint startBlock_) internal initializer {
        __Ownable_init();
        __Pausable_init_unchained();
        require(address(arsw_) != address(0),"arsw_ address is null");
    ...
    function depositLP(uint _pid, uint _amount) external virtual onlyEOA whenNotPaused {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                arsw.safeTransferFrom(address(this),_msgSender(), pending);
            }
        }
        if (_amount > 0) {
    ...
    function updateLPPool(uint _pid) public whenNotPaused {
        ...
        uint arswReward = multiplier * rewardPerBlock * lpPool.allocPoint / totalLPAllocPoint;
        lpPool.accRewardPerShare += arswReward * 1e12 / lpSupply;
        lpPool.lastRewardBlock = block.number;
    }
    ...
    function pendingLPReward(uint _pid, address _user) public view returns (uint[] memory pending) {
        ...
        if (block.number > lpPool.lastRewardBlock && lpSupply != 0) {
            uint multiplier = block.number - lpPool.lastRewardBlock;
            uint arswReward = multiplier * rewardPerBlock * lpPool.allocPoint / totalLPAllocPoint;
            accRewardPerShare = accRewardPerShare + (arswReward * 1e12 / lpSupply);
        }
        pending = new uint[](1);
    ...
    function setArswAddress(IERC20Upgradeable token_) onlyOwner external {
