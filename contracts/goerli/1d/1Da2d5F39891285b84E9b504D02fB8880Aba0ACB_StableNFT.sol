...
contract BlacklistUpgradeable is Initializable, ContextUpgradeable {
    ...
    modifier notBlacklisted() {
        require(!isBlacklisted(_msgSender()), " Blacklist: account is blacklisted");
        _;
    }
    ...
    function _toBlacklist(address _account) internal {
...
import "../contracts/BlacklistUpgradeable.sol";

contract StableNFT is
    Initializable,
    ERC1155Upgradeable,
    ...
    ReentrancyGuardUpgradeable
{
     ...
     * See https://docs.openzeppelin.com/contracts/4.x/upgradeable#storage_gaps
     */
    //need to review before production
    ...
    function mintByOwnerBatch(
        ...
        require(
            to.length == ids.length && ids.length == amounts.length && amounts.length == uris.length,
    ...
    function _beforeTokenTransfer(
        ...
        uint256[] memory amounts,
        bytes memory data
    ) internal override(ERC1155Upgradeable, ERC1155SupplyUpgradeable) whenNotPaused notBlacklisted {
        super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
    }
    ...
    function updateTokenUri(
        ...
        require(_isFreezeTokenUri || (bytes(_newUri).length != 0), "Either _newUri or _isFreezeTokenUri=true required");

        if (bytes(_newUri).length != 0) {
            require(
                keccak256(bytes(_tokenURIs[_tokenId])) != keccak256(bytes(string(abi.encodePacked(_newUri)))),
