...
contract LSDLIDOVault is LSDBase, ILSDLIDOVault {
        ...
        onlyLSDContract("lsdDepositPool", msg.sender)
    {
        ...
        if (path.length <= 1) {
            path.push(getContractAddress("lido"));
            path.push(getContractAddress("weth"));
        }
        uint256[] memory amounts = uniswapRouter.getAmountsIn(_ethAmount, path);
        uniswapRouter.swapTokensForExactETH(_ethAmount, amounts[0], path, address(this), block.timestamp + 30); 
    ...
    function withdrawStEth() public {
        ILido lido = ILido(getContractAddress("lido"));
        lido.transfer(msg.sender, lido.sharesOf(address(this)));
    }
}