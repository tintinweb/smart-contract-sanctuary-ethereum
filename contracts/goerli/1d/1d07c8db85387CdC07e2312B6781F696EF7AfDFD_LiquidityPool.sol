...
contract LiquidityPool is AccessHandler, ILiquidityPool {
     ...
     */
    function withDrawAllTokens(address token) public override {
        address owner = msg.sender;
     ...
     */
    function withDrawTokens(address token, uint256 amount) public override {
        address owner = msg.sender;
        if (amount == 0) {
            emit WithdrawZero(owner, token);
            return;
    ...
    /**
     * @notice Burn an amopunt of LP token.
