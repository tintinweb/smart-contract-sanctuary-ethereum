 ...
 */
  ...
  function unLockGas(uint256[] memory tokenIds) payable external {
    ...
    emit UnLockGasPaid(tokenIds,msg.sender,depositAmount);
  }

  function lockGas(uint256[] memory tokenIds) payable external {
    uint256 depositAmount = msg.value;
    (bool success,) = payable(gasCollector).call{value: depositAmount}("");
    if(!success) revert("unable to receive eth");
 ...
 /**
  ...
  event UnLockGasPaid(uint256[] tokenIds, address from, uint256 price);
