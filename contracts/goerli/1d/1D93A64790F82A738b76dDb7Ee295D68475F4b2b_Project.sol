...
contract Project is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Publish(uint256 indexed projectId, bool indexed isCreatedByAdmin, bool isSaleWithPacks, address indexed token, string name, string symbol, uint256[] saleIds);
        ...
        for (uint256 i; i < saleInfos.length; i++) {
            ...
            if (accountBuyers.length == 0) {
                if (!_project.isSaleWithPacks) {
                    ...
                    for (uint256 j; j < saleInfo.packTokenIds.length; j++) {
                        _project.isSingle ? IOSB721(_project.token).safeTransferFrom(address(sale), _project.manager, saleInfo.packTokenIds[j]) :
                        IOSB1155(_project.token).safeTransferFrom(address(sale), _project.manager, saleInfo.packTokenIds[j], sale.getAmountMultiTokenOfPacks(saleInfo.id, saleInfo.packTokenIds[j]), "");
                    }
                }
                continue;
            }
            if (saleInfo.amount > 0) {
                if (!_project.isSaleWithPacks && !_project.isSingle) IOSB1155(_project.token).safeTransferFrom(address(sale), _project.manager, saleInfo.tokenId, saleInfo.amount, "");
                if (_project.isSaleWithPacks) {
                    if (_project.isSingle) {
                        for (uint256 j; j < saleInfo.warePackTokenIds.length; j++) {
                            IOSB721(_project.token).safeTransferFrom(address(sale), _project.manager, saleInfo.warePackTokenIds[j]);
                        }
                    } else {
                        for (uint256 j; j < saleInfo.warePackTokenIds.length; j++) {
                            IOSB1155(_project.token).safeTransferFrom(address(sale), _project.manager, saleInfo.warePackTokenIds[j], sale.getAmountMultiTokenOfPacks(saleInfo.id, saleInfo.warePackTokenIds[j]), "");
                        }
                    }
                }
            }
    ...
    function publish(ProjectInput memory _projectInput, SaleInput[] memory _saleInputs) external payable {
        ...
        project.isSaleWithPacks  = _projectInput.isSaleWithPacks;
        ...
        project.amount           = sale.creates(_msgSender(), isCreateNewToken, _projectInput.isSaleWithPacks, project, _saleInputs);
        ...
        emit Publish(project.id, isCreatedByAdmin, _projectInput.isSaleWithPacks, project.token, _projectInput.tokenName, _projectInput.tokenSymbol, sale.getSaleIdsOfProject(project.id));
    }
