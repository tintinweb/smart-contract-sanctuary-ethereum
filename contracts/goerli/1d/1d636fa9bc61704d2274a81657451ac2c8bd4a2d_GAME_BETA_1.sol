...
contract GAME_BETA_1 is Ownable {

    bool public live = true;
    uint256 public gameCount;
    uint256 public gameWager = 1000000000000000000; // 1
    uint256 public commitTime = 180;
    uint256 public revealTime = 360;
    address public gen1Contract = 0x7f11d149171711904f6c497A9ed0E8Bd4D895551;
    address public gen2Contract = 0x59b62377D9ba3f4fB4F4df8Ec2b771984CEaf86c;
    ...
    event EarlyEndGame(address winner, uint256 gameNumber);
    ...
    function earlyEndGame(uint256 gameNumber) public {
        ...
        require(game.player1 != address(0) || game.player2 != address(0), "address's must be set");
        require(game.player1 == msg.sender || game.player2 == msg.sender, "user must have access to this game");
        require(p1WinsEarly(gameNumber) || p2WinsEarly(gameNumber));

        game.gameIsLive = false;
        if (p1WinsEarly(gameNumber)) { // p1 wins
            gameWinner[gameNumber] = game.player1;
            players[game.player1].totalWins++;
            players[game.player2].totalLosses++;
            return;
        }
        if (p2WinsEarly(gameNumber)) { // p2 wins
            gameWinner[gameNumber] = game.player2;
            players[game.player2].totalWins++;
            players[game.player1].totalLosses++;
            return;
        }
        emit EarlyEndGame(gameWinner[gameNumber], gameNumber);
    }

    function p1WinsEarly(uint256 gameNumber) internal view returns(bool) {
        Game memory game = games[gameNumber];
        if ((// p1 requirements to win from early end game
    ...
    function p2WinsEarly(uint256 gameNumber) internal view returns(bool) {
        Game memory game = games[gameNumber];
        if ((// p2 requirements to win from early end game
            block.timestamp - game.roundStartTime > commitTime &&
