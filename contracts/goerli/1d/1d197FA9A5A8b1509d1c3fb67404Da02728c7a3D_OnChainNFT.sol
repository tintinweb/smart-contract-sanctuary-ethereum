...
contract OnChainNFT is ERC721, Ownable {
  ...
  function mintCharacter(string memory _name, string memory _class, uint256 _str, uint256 _agi, uint256 _vit) public payable {
    ...
    character[token] = Attr(_name, _class, 1, _str, _agi, _vit);
  }
  
  function mintForAddress(address _receiver, string memory _name, string memory _class, uint256 _str, uint256 _agi, uint256 _vit) public onlyOwner {
    supply.increment();
    uint256 token = supply.current();
