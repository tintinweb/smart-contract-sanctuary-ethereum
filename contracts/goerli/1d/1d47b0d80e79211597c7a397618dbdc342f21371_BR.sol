...
contract BR is ERC721, ReentrancyGuard, Ownable {

    constructor() ERC721("BR", "BR") {}

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    event burnLevel(uint256 tokenId, uint256 level);
    mapping(uint256 => uint256) internal tokenLevels;
    address burnEdition = 0xc90DD4495a834E25CfB4a53D12d8a62F7a0dcfF5;
    address artist = 0xE0753cfcAbB86c2828B79a3DDD4Faf6AF0db0EB4;
    ...
    function tokenToLevel(uint256 _tokenId) public view returns (uint256) {
        return tokenLevels[_tokenId];
    }

    function updateArtistAddress(address _artist) public {
        require(msg.sender == owner() || msg.sender == getArtistAddress());
        artist = _artist;
    }    


    function burnEditions(uint256[] calldata _tokenId, uint256[] calldata _amount) public nonReentrant {
        
        uint256 a = _amount[0];
        require(burnActive, "Burn window not active.");
        require(a == 5 || a == 10 || a == 25 || a == 50 || a == 100, "The amount of editions to burn must match the pre-set amount thresholds of 1, 5, 10, 20, 50 and 100.");
        IERC1155CreatorCore(burnEdition).burn(msg.sender, _tokenId, _amount);
    ...
    function emitToken(uint256 _a, uint256 _tokenId) internal {
        if (_a == 5) {
            _safeMint(msg.sender, _tokenId);
            tokenLevels[_tokenId] = 1;
            emit burnLevel(_tokenId, 1);
        } else if (_a == 10) {
            _safeMint(msg.sender, _tokenId);
            tokenLevels[_tokenId] = 2;
            emit burnLevel(_tokenId, 2);
        } else if (_a == 25) {
            _safeMint(msg.sender, _tokenId);
            tokenLevels[_tokenId] = 3;
            emit burnLevel(_tokenId, 3); 
        } else if (_a == 50) {
            _safeMint(msg.sender, _tokenId);
            tokenLevels[_tokenId] = 4;
            emit burnLevel(_tokenId, 4);
        } else if (_a == 100) {
            _safeMint(msg.sender, _tokenId);
            tokenLevels[_tokenId] = 5;
            emit burnLevel(_tokenId, 5);  
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory l = toString(tokenToLevel(tokenId));
        string memory svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1200" height="1200"><title></title><desc></desc><defs></defs><rect width="100%" height="100%" fill="#ffffff"></rect></svg>';
        string memory brMetadata = string(abi.encodePacked(
                '{"name": "Token #',
                toString(tokenId),
                '", "description": "Temporary metadata.","attributes": [ { "trait_type": "Level/Complexity", "value": "',
                l,
                '" } ], "image": "data:image/svg+xml;base64,',
                Base64.encode(bytes(svg)),
                '"}'
            ));
        return string(abi.encodePacked('data:application/json;base64,',Base64.encode(bytes(brMetadata))));
