...
contract RecentHistory is ERC721A, Ownable {
    ...
    // List of addresses allowed to participate in private sale
    ...
    // Triggered when users are added to `allowList`
    event UsersAddedToAllowList(address[] users);
    ...
    /*
     * Allows to add user addresses to allow list
     *
     * Requirements:
     * - caller should be a contract owner
     ...
     * - collection should not be freeze
     * - user should not be in allow list already
     * - user should not be zero address
     *
     * @param `users` - array of user addresses
     *
     * Emits `UsersAddedToAllowList` event
     */
    function addToAllowList(address[] memory users) external onlyOwner {
        require(!_isCollectionFreeze, "Recent History: collection is freeze");
        ...
        for(uint256 i = 0; i < users.length; i++) {
            require(!_isAddressInAllowList(users[i]), "Recent History: address is already in allow list");
            require(users[i] != address(0), "Recent History: address is a zero address");

            _allowList.push(users[i]);
        }

        emit UsersAddedToAllowList(users);
    }

    // Returns true if given address in allow list and false if not
    function isAddressInAllowList(address user) external view returns(bool) {
        return _isAddressInAllowList(user);
    }
    ...
    /*
     * Allows to mint one new NFT to caller
     *
     * Requirements:
     * - collection should not be freeze
     * - token price should be more than 0
     * - private or public sale should be active
     * - message value should be equal to token price
     * - if private sale active, caller should be in allow list
     *
    ...
    function mint() external payable {
        ...
        require(_isPrivateSaleActive || _isPublicSaleActive, "Recent History: sale is not active");
        require(msg.value == _tokenPrice, "Recent History: invalid funds amount");

        if (_isPrivateSaleActive) {
            require(_isAddressInAllowList(msg.sender), "Recent History: you can not participate in private sale");
        }
    ...
    // Returns true if given user in `_allowList` and false if not
    function _isAddressInAllowList(address user) internal view returns(bool) {
