...
contract ChookyRoyaltySharing is ReentrancyGuard, Ownable {
    ...
    function _stakeChookyForRoyalty(uint256 _amount) external nonReentrant {
        ...
        require(ChookyInu.balanceOf(msg.sender) >= _amount, "Not enough Balance");
        require(holder.userStakedBalance == 0, "Only new holders can lock using this option.");
        ...
        else if(holder.userStakedBalance < ChookySupply / 100 && holder.userStakedBalance >= ChookySupply / 125) {
            holder.tier = 1;
            tierIIHolders ++;
        }
        else if(holder.userStakedBalance < ChookySupply / 125 && holder.userStakedBalance >= ChookySupply / 200) {
            holder.tier = 2;
            tierIIIHolders ++;
        }
        else if(holder.userStakedBalance < ChookySupply / 200 && holder.userStakedBalance >= ChookySupply / 400) {
            holder.tier = 3;
            tierIVHolders ++;
        }
    ...
    function _unstakeChooky(uint256 _amount) external nonReentrant {
        Holder storage holder = chookyHolders[msg.sender];
        require(ChookyInu.balanceOf(address(this)) >= holder.userStakedBalance, "Not enough Balance");
        require(_amount <= holder.userStakedBalance, "Can't withdraw more than what you locked!");
        ...
        if(_amount == holder.userStakedBalance) {
            holder.userStakedBalance -= _amount;
            holder.tier = -1;
            holder.stakeTime = 0;
        } else {
    ...
    function _claimRoyalty() external nonReentrant{
        Holder storage holder = chookyHolders[msg.sender];
        require(holder.userStakedBalance > 0, "You have not staked your Chooky Inu!");
        require(holder.tier >= 0, "Hodler doesn't belong to a staked tier");
        require(block.timestamp >= holder.lastClaimTime + claimWaitTime, "Can't claim before two weeks of last claim!");

        if (holder.tier == 0) {
            require(tierIHolders > 0, "Not enough holders!");
            uint256 claimAmountShare = address(this).balance * royalty.tierI /100;
            ...
            uint256 claimAmountShare = address(this).balance * royalty.tierII /100;
            ...
            uint256 claimAmountShare = address(this).balance * royalty.tierIII /100;
            ...
            uint256 claimAmountShare = address(this).balance * royalty.tierIV /100;
    ...
    function  _tierBalances() external view returns(uint256, uint256, uint256, uint256) {
        uint256 tierIBalance = address(this).balance * royalty.tierI /100;
        uint256 tierIIBalance = address(this).balance * royalty.tierII /100;
        uint256 tierIIIBalance = address(this).balance * royalty.tierIII /100;
    ...
    function _updateRoyaltyShare(
        ...
        uint256 _tierIV
            ...
            require((_tierI + _tierII + _tierIII + _tierIV)==100, "Must be equal to 100");
    }
    ...
    function _withdrawOtherTokens(address _token, uint256 amount) public onlyOwner {

        if(IERC20(_token) == IERC20(ChookyInu)) {
        require(ChookyInu.balanceOf(address(this)) > TVL, "Not enought Balance");
