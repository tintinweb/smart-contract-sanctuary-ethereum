...
// - `withdrawProceeds`: Withdraw payment for my bought
contract NftMarketplace is ReentrancyGuard{
    ...
    // e.g. Multiple NFT contracts to multiple tokens towners (like BasicNft -> Token 0 -> Listing(price:10, seller: msg.sender))
     ...
     * @dev Technically, we could have the contract be the escrow for the NFTs but this way people can still hold their NFT when listed.
     */
    function listItem(
        address nftAddress,
        uint256 tokenId,
        ...
        external
        // Challange: Have this contract accept payment in a subset of token as well
        // Hint: Use the Chainlink Price Feed to convert the price of the tokens
        ...
        isOwner(nftAddress, tokenId, msg.sender)
    {
        ...
        IERC721 nft = IERC721(nftAddress);
        // getApproved method become available because BasicNft ERC721 token. The token has to be minted and approved prior to listing
    ...
    function buyItem(address nftAddress, uint256 tokenId)
        external
        payable
        nonReentrant // Available from ReentrancyGuard.sol
        isListed(nftAddress, tokenId)
    {
        ...
        // SENDING the money to the user should be AVOIDED, instead, have them WITHDRAW the money
        s_proceeds[listedItem.seller] = s_proceeds[listedItem.seller] + msg.value;
        delete (s_listing[nftAddress][tokenId]);

        // Addressing Re-Entrancy attack and Oracke attach volnerability
        // Always execute external call at the end
        // Check to make sure the NFT was transformed (no return for ERC721 safeTransferFrom)
        IERC721(nftAddress).safeTransferFrom(listedItem.seller, msg.sender, tokenId);

        // After everything is complete, emit the event
