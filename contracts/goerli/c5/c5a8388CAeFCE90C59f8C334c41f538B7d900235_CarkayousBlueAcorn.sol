...
contract LushLinkCronosUnlimitedDrop is ERC721Enumerable, ERC2981, Hierarchy {
    ...
    error NotOnAllowList(address who);
    ...
    function allowListMint() public payable {
        ...
        if (isAllowListAddress(_msgSender()) == false) {
            revert NotOnAllowList({
                who: _msgSender()
            });
        }
        if (allowListMinted[msg.sender]) revert AlreadyClaimed();
    ...
    // Allow list.
    function addAllowListAddressBatch(address[] memory _addresses) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not allowed to add to allow list");
        for (uint i=0; i < _addresses.length; i++) {
            allowListAddresses[_addresses[i]] = true;
        }
    }
    ...
    function removeAllowListAddress(address _address) public {
        ...
        if (allowListAddresses[_address]) {
            delete allowListAddresses[_address];
        }
    }

    function isAllowListAddress(address _address) public view returns(bool) {
        return allowListAddresses[_address];
    }
