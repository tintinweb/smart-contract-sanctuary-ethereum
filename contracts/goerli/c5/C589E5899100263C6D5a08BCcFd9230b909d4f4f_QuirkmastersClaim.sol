...
contract QuirkmastersClaim is OwnableInternal {
    ...
    function burnByERC721B(
        uint256 burnOwnedTokenIdB, // Token ID in ERC721 collection.
        uint256 burnClaimTokenIdB, // Token ID in ERC1155 to be burn.
        uint256 burnCountB // How many of this ERC1155 token to burn?
    ) external {
        Layoutb storage lb = layoutb();

        require(IERC721(lb.targetERC721ContractAddressB).ownerOf(burnOwnedTokenIdB) == _msgSender(), "MAX_ALLOWANCE");

        lb.tokensToMintedAmountB[burnOwnedTokenIdB] -= burnCountB;

        require(lb.tokensToMintedAmountB[burnOwnedTokenIdB] <= lb.maxAllowancePerERC721B, "MAX_ALLOWANCE");

        IERC1155MintableExtension(address(this)).mintByFacet(_msgSender(), burnClaimTokenIdB, burnCountB, "");
    }
}