...
contract testing is ERC721AQueryable, Ownable {
  using Strings for uint256;

  uint256 public TOTAL_MAX_SUPPLY = 50;
  uint256 public totalFreeMints = 50;
  ...
  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    require(balanceOf(msg.sender) < maxPublicMintPerWallet, "testing: Mint limit exceeded");
    require(saleStarted, "testing: Sale has not started");
    if (_totalMinted() < (TOTAL_MAX_SUPPLY)) {
      if (freeMintCount >= totalFreeMints) {
        require(msg.value >= _quantity * publicTokenPrice, "testing: More ETH required");
        ...
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(
            ...
            "testing: Not enough ETH"
          );
        }
        _mint(msg.sender, _quantity);
      } else {
        require(msg.value >= (_quantity * publicTokenPrice), "testing: Not enough ETH");
        _mint(msg.sender, _quantity);
      }
    }
  }
  ...
  function withdrawFunds() external onlyOwner {
    ...
    require(success, "testing: Transfer failed");
  }
