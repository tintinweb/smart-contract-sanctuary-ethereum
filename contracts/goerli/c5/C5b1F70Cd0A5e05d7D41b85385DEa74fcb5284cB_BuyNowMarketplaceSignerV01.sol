...
contract BuyNowMarketplaceSignerV01 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function purchaseMintSignerItem(MintSignerItem calldata mintSignerData) public payable nonReentrant {
        ...
        uint fee = calculateFee(mintSignerData.price, initialFeePercent);
        address buyer = msg.sender;
        if (compare(mintSignerData.currency, "usd") == 0) {
            buyer = address(mintSignerData.purchaser);
        }

        uint256 sellerTransferAmount = mintSignerData.price - fee;
