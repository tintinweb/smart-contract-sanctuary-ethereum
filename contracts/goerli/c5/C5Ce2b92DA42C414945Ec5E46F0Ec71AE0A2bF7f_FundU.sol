...
contract FundU is ReentrancyGuard, FundUtils {
    ...
    address private immutable i_CNET;
    ...
    /*** CONSTRUCTOR ***/
    constructor(address _CNET, address _USDC, address _USDT) {
        s_streamId = 0;
     ...
     * @param user The one who deposit
     ...
     */
    function depositOnWallet(address user, uint256 deposit, address tokenAddress) external {
        ...
        s_userBalanceByToken[user][tokenAddress] += deposit;

        IERC20 token = IERC20(tokenAddress);
        token.safeTransferFrom(user, address(this), deposit);

        emit WalletDeposit(deposit, tokenAddress, user);
    }

    function withdrawFromWallet(uint amount, address tokenAddress) external nonReentrant {
     ...
     * @return the newly created instant payment´s id
    ...
    function instantPayments(
        ...
        require(
            ...
            "Instant_Payment: Invalid beneficiary address"
        );
        require(amountToDeposit != 0, "Instant_Payment: Zero amount");
        require(
            s_userBalanceByToken[msg.sender][tokenAddress] >= amountToDeposit,
            "Instant_Payment: Not enough balance"
        );
        ...
        // Transfer the tokens directly to the beneficiary protocol´s wallet
        s_userBalanceByToken[msg.sender][tokenAddress] -= amountToDeposit;
        s_userBalanceByToken[beneficiary][tokenAddress] += amountToDeposit;
    ...
    function newStream(
        ...
        require(amountToDeposit != 0, "Stream: Zero amount");
        require(
            s_userBalanceByToken[msg.sender][tokenAddress] >= amountToDeposit,
        ...
        require(stop > startTime, "Stream: Invalid stop time");

        uint256 duration = stop - startTime;

        // This check is to ensure a rate per second, bigger than 0
        require(amountToDeposit > duration, "Stream: Deposit smaller than time left");
        ...
        // Transfer the tokens, in this case to the contract
        // TODO: Just take this away? The balance is already on the contract
     ...
     * @param id Stream´s id
     * @param paid true if it is a paid paise, false if not
     * @dev It fails if the caller is not the owner
     * @dev It fails if the stream is not active
     */
    ...
    function cancelStream(uint256 id) external nonReentrant onlyStreamOwner(id) {
        ...
        if (beneficiaryRemainingBalance > 0) {
            ...
            */
            ...
            s_userBalanceByToken[stream.owner][stream.tokenAddress] -= beneficiaryRemainingBalance;
        }
        /*
    ...
    function withdraw(uint256 id) public nonReentrant onlyStreamBeneficiary(id) {
        ...
        s_userBalanceByToken[stream.owner][stream.tokenAddress] -= balance;
    ...
    function _withdrawPauseAndResume(uint256 _id, address _who) private {
        ...
        if (_balance > 0 && _who == stream.beneficiary) {
            //IERC20(stream.tokenAddress).safeTransfer(_who, _balance);

            s_userBalanceByToken[stream.beneficiary][stream.tokenAddress] += _balance;
            s_userBalanceByToken[stream.owner][stream.tokenAddress] -= _balance;
