...
contract InfinityBitTokenV2 is IERC20, Ownable {
    ...
    uint8 private _marketingTax = 50; // 3%
    address payable private _marketingWallet = payable(0xbcb8aba8bf2aB0D8F8462e712E9E229Cd0D49B89);

    // Developer Tax
    uint8 private _devTax = 0; // 2%
    address payable private _devWallet = payable(0x912E159BFF3fBCe9a6DF2FEAdab2332166034C89);
    ...
    uint8 private immutable _autoSwapDeadlineSeconds = 10;
    ...
    uint8 public _swapThreshold = 50; // 5%
    uint8 public immutable _swapThresholdMax = 80; // 8%
    ...
    function isSell(address recipient) private view returns (bool) {
        return recipient == _uniswapV2PairAddress || recipient == address(_uniswapV2Router);
    }
    function isBuy(address sender) private view returns (bool) {
        return isSell(sender);
    }
    ...
    // Limit the maximum autoswap based on _swapThreshold percent
    function getMaxAutoswap() public view returns (uint256 max_autoswap) {
        return (_swapThreshold * getLiquidityIBIT()) / 1000;
    }

    // Returns the autoswap threshold percent with one decimal, i.e. 50 = 5%
    ...
    function setAutoswapThreshold(uint8 swapThreshold) public onlyOwner {
        require(swapThreshold < _swapThresholdMax, "IBIT: swapThreshold exceeds limit");
        _swapThreshold = swapThreshold;
    }

    event AutoLiquidityFailed(uint256 token_amount, uint256 eth_amount);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(tax_amount == 0 && _calcLpTaxAmount(amount) == 0)
        {
            _taxlessTransfer(from, to, amount);
            return;
        }

        // Take taxes and LpTax
        _takeTaxes(from, tax_amount+_calcLpTaxAmount(amount));

        // AutoSwap Check
        if(_autoSwapTokens && isSell(to))
        {
            uint256 lp_tax_tokens = _calcLpTaxAmount(amount)/2;
            uint256 autoswap_amount = tax_amount + lp_tax_tokens;
            ...
            if(to != _uniswapUniversalRouter) // Don't AutoLp with UniversalRouter transfers
            {
                uint256 lp_tax_eth = _calcTaxDistribution(ethCollected, _lpTax)/2;
                if(!addLiquidity(lp_tax_tokens, lp_tax_eth)) {
                    emit AutoLiquidityFailed(lp_tax_tokens, lp_tax_eth);
                }
            }
        ...
        else
        {
            ...
            _balances[address(owner())] += _calcLpTaxAmount(amount);
        }
    ...
    function _calcTaxes(uint256 amount) public view returns (uint256 tax_to_collect) {
        return _calcDevTaxAmount(amount) + _calcMarketingTaxAmount(amount);
    }
