...
contract HaruStaking {
    ...
    uint public amountOfTokenPerEth = 1302000000000000000000;
    ...
    // Helper Functions
     ...
     function calculateAnnualInterest(uint tokenQuantity, uint apy) public pure returns(uint) {
          return (tokenQuantity / 100) * apy;
     }
    ...
    function stakeTokens(string calldata symbol, uint tokenQuantity, uint numDays) external {
          ...
          uint annualInterest = calculateAnnualInterest(tokenQuantity, apy);
          uint monthlyInterest = annualInterest / 12;
     ...
     function closePosition(uint positionId) external {
          ...
          uint ethInterest = (positions[positionId].profitsToReclaim) / amountOfTokenPerEth;

          IERC20(tokens[positions[positionId].symbol].tokenAddress).transfer(msg.sender, positions[positionId].tokenQuantity);

          payable(msg.sender).call{value: ethInterest}("");
     }
