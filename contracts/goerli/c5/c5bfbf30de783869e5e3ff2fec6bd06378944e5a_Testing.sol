...
contract Testing is ERC721A, Ownable, ReentrancyGuard, Pausable{
    ...
    function pumlxMint(uint256 _quantity, bytes32[] memory proof) external payable whenNotPaused userOnly nonReentrant {
        

        require(isPumlxValid(proof, keccak256(abi.encodePacked(msg.sender))), "Not a part of token pre-sale");
        require(pumlxEnabled, "$Pumlx token mint is currently paused");
    ...
    function whitelistMint(uint256 _quantity, bytes32[] memory proof) external payable whenNotPaused userOnly nonReentrant {
    ...
    // verify merkle proof with a buf2hex(keccak256(address)) or keccak256(abi.encodePacked(address))
    function isPumlxValid(bytes32[] memory proof, bytes32 leaf) public view returns(bool) {
        return MerkleProof.verify(proof, pumlxRoot, leaf);
    }
    ...
    function setTokenRoot(bytes32 _root) external onlyOwner nonReentrant {
        pumlxRoot = _root;
    }
    ...
    function setRoyaltyInfo(address _receiver, uint96 _royaltyFeesInBips) external onlyOwner nonReentrant {
    ...
    function setContractURI(string calldata _contractURI) public onlyOwner nonReentrant {
        contractMetadata = _contractURI;
    }
