...
contract Marketplace is OpenStore {
    struct EIP712Domain {
        string  name;
        string  version;
        uint256 chainId;
    ...
    function acceptOffer(Offer calldata _offer, uint8 v, bytes32 r, bytes32 s) external {
      require(
        verify(_offer, owner(), v, r, s),
        "wrong signature"
       );
      require(block.timestamp <= _offer.timestamp + EXPIRED_TIME, "Deadline reached");
      require(
            _offer.seller == _msgSender(),
            "ONLY_SELLER_ALLOWED"
        );
      uint256 tokenAllowance = IERC20(_offer.tokenAddress).allowance(_offer.buyer, address(this));
      require(
           tokenAllowance >= _offer.tokenAmount, "ERC20: insufficient allowance"
        );
      IERC20(_offer.tokenAddress).transferFrom(_offer.buyer,_offer.seller, _offer.tokenAmount);
      safeTransferFrom(_offer.seller, _offer.buyer, _offer.nftId, _offer.nftAmount, "0x");
    }
    ...
    function acceptERC1155Offer(Offer calldata _offer, uint8 v, bytes32 r, bytes32 s) external {
      ...
      require(block.timestamp <= _offer.timestamp + EXPIRED_TIME, "Deadline reached");
      require(_offer.seller == _msgSender(), "ONLY_SELLER_ALLOWED");
      ...
      IERC20(_offer.tokenAddress).transferFrom(_offer.buyer,_offer.seller, _offer.tokenAmount);
      IERC1155(_offer.collectionAddress).safeTransferFrom(_offer.seller, _offer.buyer,  _offer.nftId,_offer.nftAmount, "0x");
    }
    ...
    function hashOffer(Offer memory offer) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            keccak256("Offer(uint256 nftId,address collectionAddress,uint256 nftAmount,address tokenAddress,uint256 tokenAmount,address buyer,address seller,uint256 timestamp)"),
            offer.nftId,
