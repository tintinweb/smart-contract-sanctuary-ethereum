...
contract EVEOTCTokens is Owner, ReentrancyGuard {
    ...
    // the index of the token in the tokens array
    // the if the value is 0 means it does not exists, if the value is > 0 then the index is token_indexes_plus1[address] - 1
    // this is useful to avoid looping the user tokens array to find the index of an existing token
    ...
    function addUserToken(address _token, uint256 _price) external {
        ...
        require(_price > 0, "OTC: cannot add token without a price");
...
contract EVEOTC is EVEOTCTokens {
    ...
    struct Purchase {
        ...
        uint256 timestamp;
        address buyer;
        uint32  commission_buy;   // buying commission at the moment of Purchase creation (only used on option trades)
    ...
    function surplus(uint256 _available, bool _smart_trade) public view returns (uint256 _surplus, uint16 _commission_sell) {
        ...
        if (_smart_trade) {
            ...
            (stake_discount_seller,) = stakes.getOptionDiscount(msg.sender);
        }
     ...
     * Add a system token
     */
    function addToken(address _token, uint256 _cmc_index, address _aggregator, uint256 _price) override public {
    ...
    function addOffer(bool _smart_trade, address _from, address[] memory _to, uint256 _available, uint256 _price, uint256 _discount, 
        ...
        trades.push(Offer(_smart_trade, msg.sender, _from, _to, _available, 0, _surplus, 0, _price, _discount, _premium, _time, true));
    ...
    function buySmartTrade(uint256 _index, address _to, uint256 _amount) external nonReentrant isNotPaused {
        ...
        smart_trades_purchases.push(Purchase(_index, trades[_index].from, _to, _amount, 0, 0, block.timestamp, msg.sender, 0, true));
    ...
    function validateOptionTrade(uint256 _purchase_index) internal view returns (uint256 offer_index) {
        ...
        Purchase memory _purchase = option_trades_purchases[_purchase_index];

        // Requires that the sender is the buyer
        require(_purchase.buyer == msg.sender, "OTC: you are not the buyer");

        // Requires that the purchase is not already filled
        require(!_purchase.paid, "OTC: the order is already paid");

        uint256 _offer_index = _purchase.offer_index;
    ...
    function payOptionTrade(uint256 _purchase_index) external nonReentrant isNotPaused {
        ...
        uint256 _surplus = option_trades_purchases[_purchase_index].amount * trades_commission_sell[_offer_index] / 10000;
        
        if (_surplus > 0) {
            require(IERC20(trades[_offer_index].from).transfer(owner_wallet, _surplus), "OTC: error paying commissions to owner");
