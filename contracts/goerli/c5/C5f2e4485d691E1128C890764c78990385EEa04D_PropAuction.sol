...
contract PropAuction is Ownable,ReentrancyGuard {
    ...
    struct order{
        ...
        uint256     number;             //挂卖数量
        uint256     startingpPrice;     //起拍价(注意小数位)
    ...
    /**************事件****************/
    /**发布拍卖*/
    event _releaseAuction(uint256 _orderId,address from,address gameContract_,address glodContract_,uint256 tokenId_,uint256 number_,uint256 startingpPrice_,uint256 startTime_);
    /**取消拍卖*/
    event _cancelTheAuction(uint256 _orderId); 
    /**流拍*/
    event _streamBeat(uint256 _orderId); 
    /**拍卖成功*/
    ...
    */
    function accountAddTheOrder(
        ...
        returns(bool)
        {
            return toAddTheOrder(gameContract_,glodContract_,tokenId_,number_,startingpPrice_,startTime_);
        }
    /** 发布
     *  Role        私有的                   
    */
    function toAddTheOrder(
        ...
        returns(bool)
        {
            ...
            orders[orderId] = order(msg.sender,gameContract_,glodContract_,tokenId_,number_,startingpPrice_,startTime_,address(0x00),0,1);
            _transferToken(gameContract_,msg.sender,address(this),tokenId_,number_);//将装备从卖家处转移到本合约地址
            emit _releaseAuction(orderId,msg.sender,gameContract_,glodContract_,tokenId_,number_,startingpPrice_,startTime_);
            return true;
        }
    ...
    */   
    function theOrderDetailsIm(uint256 orderId_)
        ...
        {
            return (orders[orderId_].account,orders[orderId_].gameContract,orders[orderId_].glodContract,orders[orderId_].tokenId,orders[orderId_].number,orders[orderId_].startingpPrice,orders[orderId_].startTime);
        }    
    /** 查看订单状态
     *  Role        ALL
     *  parameter   uint256     orderId_            订单号
    ...
    */
    function toCancel(uint256 orderId_)
        ...
        returns(bool)
        {
            ...
            require (myOrder.startTime > block.timestamp,'The auction has begun');                  //判断拍卖是不是还没开始
    ...
    */
    function streamBeat(uint256 orderId_)
        nonReentrant
        public
        onlyOwner()
        returns(bool)
        {
            ...
            require (myOrder.state == 1,'Abnormal order status');                   //判断订单状态是不是拍卖中
            require (myOrder.startTime < block.timestamp,'The auction has begun');              //判断拍卖是不是还没开始
            myOrder.state = 4;                                                      //修改订单状态为流拍
            _transferToken(myOrder.gameContract,address(this),msg.sender,myOrder.tokenId,myOrder.number);//将装备退给卖家
            emit _streamBeat(orderId_);
            return true;
        }
     ...
     *  parameter   uint256     transactionPrice_   成交价
     *  returns     bool                        
    */    
    function successfulAuction(uint256 orderId_,address to_,uint256 transactionPrice_)
        nonReentrant
        public
        payable
        onlyOwner()
        returns(bool)
        {
            ...
            myOrder.state = 2;
            myOrder.to = to_;
            myOrder.transactionPrice = transactionPrice_;                                                             //将订单状态改为已交易
            if(myOrder.glodContract != address(0x00)){
                //代币交易
                //初始化币种合约地址
                IERC20 coinContract = IERC20(myOrder.glodContract);                          //买家付款
                coinContract.transferFrom(msg.sender,myOrder.account,transactionPrice_);
            }else{
                //链币交易
                require (msg.value >= transactionPrice_,'Insufficient transaction amount');                //买家付款
                payable(myOrder.account).transfer(transactionPrice_);
            }
