...
contract SquiggleRae is ERC721, Ownable {
    ...
    function setTokenHash(uint256 _tokenId, bytes32 _hash) external onlyOwner {
        seeds[_tokenId] = _hash;
    }
    ...
    function tokenURI(
        ...
        WrappedScriptRequest[] memory requests = new WrappedScriptRequest[](6);
        ...
        bytes memory divContent = abi.encodePacked(
            '<div id="top"></div><div id="squiggle"></div><div id="bottom"></div>'
        );

        requests[5].wrapType = 4;
        requests[5].scriptContent = divContent;

        uint256 finalBufferSize = bufferSize + bytes('<script>').length + bytes('</script>').length + scriptContent.length + divContent.length;
        ...
        string memory name = string.concat("Chromie Squiggle #", tokenIdStr);
        string memory description = string.concat("This Vault contains Chromie Squiggle ", tokenIdStr, " created by Art Blocks.");
