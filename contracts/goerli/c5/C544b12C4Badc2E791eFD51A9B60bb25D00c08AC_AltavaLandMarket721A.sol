...
contract AltavaLandMarket721A is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    event MerkleProofEvent(address sender, bytes32[] proof, bytes32 _merkleRoot, bool result);
    ...
    function verifyMerkleProof(bytes32[] calldata proof) public {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        emit MerkleProofEvent(msg.sender, proof, _merkleRoot, MerkleProof.verify(proof, _merkleRoot, leaf));
    }

    function verifyMerkleProof2(bytes32[] calldata proof) public {
