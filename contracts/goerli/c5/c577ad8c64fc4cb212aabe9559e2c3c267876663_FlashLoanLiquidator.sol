 ...
 *
 * `collateralToFlashLoan` is used to swap to `flashLoanAsset` and repay flash loan, from
 * asset received by `liquidationCall()`.
 *
 * `flashLoanAsset` and `flashLoanAmount` are used to swap to debtAsset via `collateralToFlashLoan`.
 */
...
contract FlashLoanLiquidator is LiquidatorBase, IFlashLoanReceiver {
  ...
  function executeOperation(
    ...
    if (params.debtAsset != params.flashLoanAsset) {
      LiquidatorLib._swapTokensForExactTokens(
        params.flashLoanAsset,
        amounts[0],
        params.debtToCover,
        params.flashLoanToDebt
      );
    }
    ...
    uint256 flashBalBefore = IERC20(params.flashLoanAsset).balanceOf(
      address(this)
    );
    ...
    if (flashBalBefore < repayAmount) {
      if (params.collateralAsset != params.flashLoanAsset) {
        ...
        LiquidatorLib._swapTokensForExactTokens(
          ...
          amountNeeded,
          params.collateralToFlashLoan
        );
      }
    }
    uint256 flashBal = IERC20(params.flashLoanAsset).balanceOf(address(this));
    if (flashBal < repayAmount) {
      revert CannotRepayFlashLoan(repayAmount, flashBal);
    }

    if (params.debtAsset != params.flashLoanAsset) {
      _transferToAdmin(params.debtAsset);
    }
    if (params.collateralAsset != params.flashLoanAsset) {
      _transferToAdmin(params.collateralAsset);
    }
    if (flashBal > repayAmount) {
      _transferToAdmin(params.flashLoanAsset, flashBal - repayAmount);
    }
   ...
   */
  function unstakeAndRepay(UnstakeAndRepayParams[] memory params)
    public
    onlyExecutor
  {
    for (uint256 i = 0; i < params.length; i++) {
      CONFIG.pool().unstakeApePositionAndRepay(
        params[i].nftAsset,
        params[i].tokenId
      );
    }
