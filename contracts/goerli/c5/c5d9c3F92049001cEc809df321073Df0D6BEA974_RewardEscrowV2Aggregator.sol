...
contract RewardEscrowV2Aggregator is BaseAggregator {
    ...
    uint private _gasToAppend = 200000;
    uint private _gasToVest = 200000;

    event Append(uint16 indexed srcChainId, address indexed account, uint amount);
    event Vest(uint16 indexed srcChainId, address indexed account, uint amount);
    ...
    function sendAppend(
        ...
        bytes memory lzPayload = abi.encode(PT_APPEND, currencyKey, abi.encodePacked(account), amount);

        _broadcast(lzPayload, PT_APPEND);
    }
    ...
    function sendVest(
        ...
        bytes memory lzPayload = abi.encode(PT_VEST, currencyKey, abi.encodePacked(account), amount);

        _broadcast(lzPayload, PT_VEST);
    }

    /** Gas for LZ fee */
    function gasToAppend() external view returns (uint) {
        return _gasToAppend;
    }

    function gasToVest() external view returns (uint) {
        return _gasToVest;
    }

    function setGasToAppend(uint newGas) external onlyOwner {
        _gasToAppend = newGas;
    }

    function setGasToVest(uint newGas) external onlyOwner {
        _gasToVest = newGas;
    }
    ...
    function _lzAdapterParam(uint16 packetType) internal view override returns (bytes memory) {
        uint16 version = 1;
        if (packetType == PT_APPEND) {
            return abi.encodePacked(version, _gasToAppend);
        }
        return abi.encodePacked(version, _gasToVest);
    }
}
