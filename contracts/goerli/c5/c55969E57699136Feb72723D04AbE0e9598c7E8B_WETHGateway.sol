...
contract WETHGateway is IWETHGateway, ERC721HolderUpgradeable, EmergencyTokenRecoveryUpgradeable {
  ...
  mapping(address => mapping(uint256 => uint256)) public userLoanDebt;
  ...
  function withdrawETH(uint256 amount, address to) external override nonReentrant {
    ...
    ILendPool cachedPool = _getLendPool();
    IUDToken bWETH = IUDToken(cachedPool.getReserveData(address(WETH)).UDTokenAddress);
    ...
    //test
    if (userDeposited[_msgSender()] >= amount) {
      userDeposited[_msgSender()] -= amount;
    } else {
      userDeposited[_msgSender()] = 0;
    }
  ...
  function borrowETH(
    ...
    _checkValidCallerAndOnBehalfOf(onBehalfOf);
    //test
    userBorrowed[_msgSender()] += amount;
    ILendPool cachedPool = _getLendPool();
    ...
    //test
    uint256 loanIds = cachedPoolLoan.getCollateralLoanId(nftAsset, nftTokenId);
    userLoanDebt[_msgSender()][loanIds] += amount;
  ...
  function repayETH(
    ...
    ILendPoolLoan cachedPoolLoan = _getLendPoolLoan();
    uint256 loanIds = cachedPoolLoan.getCollateralLoanId(nftAsset, nftTokenId);
    uint256 lastDebt = userLoanDebt[_msgSender()][loanIds];
    if (lastDebt >= amount) {
      userLoanDebt[_msgSender()][loanIds] -= amount;
    } else {
      userLoanDebt[_msgSender()][loanIds] = 0;
    }
    if (userBorrowed[_msgSender()] >= amount) {
      userBorrowed[_msgSender()] -= amount;
    } else {
      userBorrowed[_msgSender()] -= lastDebt;
    }
  ...
  function redeemETH(
    ...
    require(loanId > 0, "collateral loan id not exist");
    //test
    if (userLoanDebt[_msgSender()][loanId] >= amount) {
      userLoanDebt[_msgSender()][loanId] -= amount;
    }
    if (userBorrowed[_msgSender()] >= amount) {
      userBorrowed[_msgSender()] -= amount;
    }
  ...
  function liquidateETH(address nftAsset, uint256 nftTokenId) external payable override nonReentrant returns (uint256) {
    ...
    require(loan.reserveAsset == address(WETH), "loan reserve not WETH");
    //test
    userBorrowed[loan.borrower] -= userLoanDebt[loan.borrower][loanId];
    userLoanDebt[loan.borrower][loanId] = 0;
    if (msg.value > 0) {
      WETH.deposit{value: msg.value}();
    }
  ...
  //test
  function getInfo(address user) external view returns (uint256 deposited, uint256 borrowed) {
    return (userDeposited[user], userBorrowed[user]);
  }
}
...
interface IWETHGateway {
  /**
   * @dev deposits WETH into the reserve, using native ETH. A corresponding amount of the overlying asset (UDTokens)
   * is minted.
   * @param onBehalfOf address of the user who will receive the UDTokens representing the deposit
  ...
  /**
   * @dev borrow WETH, unwraps to ETH and send both the ETH and UBTokens to msg.sender, via `approveDelegation` and onBehalf argument in `LendPool.borrow`.
...
interface ILendPool {
   ...
   * @param onBehalfOf The beneficiary of the deposit, receiving the UDTokens
   ...
   * @param user The address initiating the withdrawal, owner of UDTokens
  ...
  /**
   * @dev Deposits an `amount` of underlying asset into the reserve, receiving in return overlying UDTokens.
   * - E.g. User deposits 100 USDC and gets in return 100 bUSDC
   * @param reserve The address of the underlying asset to deposit
   * @param amount The amount to be deposited
   * @param onBehalfOf The address that will receive the UDTokens, same as msg.sender if the user
   *   wants to receive them on his own wallet, or a different address if the beneficiary of UDTokens
  ...
  /**
   * @dev Withdraws an `amount` of underlying asset from the reserve, burning the equivalent UDTokens owned
   ...
   *   - Send the value type(uint256).max in order to withdraw the whole UDToken balance
  ...
  /**
   * @dev Validates and finalizes an UDToken transfer
   * - Only callable by the overlying UDToken of the `asset`
   * @param asset The address of the underlying asset of the UDToken
   * @param from The user from which the UDTokens are transferred
   * @param to The user receiving the UDTokens
   * @param amount The amount being transferred/withdrawn
   * @param balanceFromBefore The UDToken balance of the `from` user before the transfer
   * @param balanceToBefore The UDToken balance of the `to` user before the transfer
   */
