...
contract GuideDAOAccessToken is IERC721Metadata {
    ...
    uint32 public currentIdToMint = 1;
    uint32 public currentCollectionId;
    Collection[] public collections;
    mapping(address => uint32) private _ids;
    mapping(uint32 => address) private _owners;
    mapping(address => bool) public admins;

    modifier isMinted(uint32 tokenId) {
    ...
    function burn(address from, uint32 tokenId) public isAdmin {
    ...
    function setCollectionURI(uint32 collectionId, string memory newURI)
        public
        isAdmin
    ...
    function getAddressNFTId(address user) public view returns (uint32) {
        return _ids[user];
    }

    function approve(address to, uint32 tokenId) external override {}
    ...
    function getApproved(uint32 tokenId)
        external
        view
        override
