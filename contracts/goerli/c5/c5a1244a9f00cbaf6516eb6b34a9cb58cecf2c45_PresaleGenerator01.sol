...
contract Presale01 is ReentrancyGuard, Proxiable{
    ...
    // apply for goerli testnet
    function initialize(address _presaleGenerator) external onlyNotInitialized {
    ...
    function ownerWithdrawTokens() external onlyPresaleOwner {
        ...
        if(statusWithDraw) {
            require(
                REFUND_INFO.refundTime + 5 hours < block.timestamp, 
    ...
    function finalize() external {
        require(MONOPOLY_DEV == msg.sender, "ONLY NOMOPOPY DEV.");
    ...
    function updateCode(address _presaleAddress) external {
...
contract PresaleGenerator01 is Ownable {
    ...
    address[] public presaleTampAddress;
    ...
    struct PresaleParams {
        ...
        uint256 listingRate; // sale token listing price on uniswaps
        uint256 startTime;
        uint256 endTime;
        uint256 lockPeriod;
    }
    ...
    function createPresale (
      ...
      uint256[] memory _refundInfo
        ...
        Presale01 newPresale = (new Presale01)();
        presaleTampAddress.push(address(newPresale));
        NomopolyProxy newPresaleProxy = (new NomopolyProxy){value: msg.value}(address(newPresale));
        TransferHelper.safeTransferFrom(address(_presaleToken), address(msg.sender), address(newPresaleProxy), tokensRequiredForPresale);
        Presale01(address(newPresaleProxy)).initialize(address(this));
        Presale01(address(newPresaleProxy)).init1(_presaleOwner, params.amount, params.tokenPrice, params.maxSpendPerBuyer, params.hardcap, params.softcap, 
        params.liquidityPercent, params.listingRate, params.startTime, params.endTime, params.lockPeriod);
        Presale01(address(newPresaleProxy)).init2(_baseToken, _presaleToken, PRESALE_SETTINGS.getBaseFee(), PRESALE_SETTINGS.getTokenFee(), PRESALE_SETTINGS.getEthAddress(), PRESALE_SETTINGS.getTokenAddress());
        require(_refundInfo[0] >= 0 && _refundInfo[0] <= 1000, "INVALID REFUND FEE."); 
        Presale01(address(newPresaleProxy)).init3(is_white_list, _caller, percentFee, _distributionTime, _unlockRate, _isRefund, _refundInfo);
        PRESALE_FACTORY.registerPresale(address(newPresaleProxy));
    }
    ...
    function calculateAmountRequired (uint256 _amount, uint256 _tokenPrice, uint256 _listingRate, uint256 _liquidityPercent, uint256 _tokenFee) internal pure returns (uint256) {
