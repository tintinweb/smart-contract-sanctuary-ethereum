...
contract MarketingVesting is KeeperCompatibleInterface, ReentrancyGuard {
  /*==================================================== State Variables ====================================================*/
  Vesting vesting;
  ...
  function checkUpkeep(
    ...
    uint256 startTime = vesting.globalTime();
    address[] memory _users = vesting.getUsers(1);
    bool outBreak = false;
    for (uint256 i = 0; i < _users.length; i++) {
      uint256[] memory _indexes = vesting.getInvestorIndexes(_users[i], 1);
      for (uint256 k = 0; k < _indexes.length; k++) {
        uint256 _last = vesting.getInvestorInfo(_users[i], 1, _indexes[k]).lastReceivedTime;
        if (upkeepNeeded = (block.timestamp > startTime) && ((block.timestamp - _last) > 30 days)) {
          performData = abi.encode(_users[i], _last, 1, _indexes[k]);
          break;
        }
      }
      if (outBreak) break;
  ...
  function performUpkeep(bytes calldata performData) external override nonReentrant {
    (address _user, uint256 _last, uint256 _vCat, uint256 _index) = abi.decode(performData, (address, uint256, uint256, uint256));
    require((block.timestamp - _last) > 30 days, "You Shall Not Pass!");
    vesting.withdrawTokensAutomatically(_user, _vCat, _index);
  }
}
...
contract Vesting is ReentrancyGuard {
  ...
  mapping(uint256 => VestingCategory) private intToEnum;
  ...
  constructor(address _admin, uint256 _globalTime) {
    ...
    intToEnum[5] = VestingCategory.PUBLIC_LOCKED;
    intToEnum[6] = VestingCategory.PUBLIC_UNLOCKED;
    ...
    enumToInt[VestingCategory.PUBLIC_LOCKED] = 5;
    enumToInt[VestingCategory.PUBLIC_UNLOCKED] = 6;
    //============================================================================
  ...
  */
  ...
  function calculateUnlockedTokens(
    ...
    uint256 cliffTimestamp = startTimeStamp + (investor.schedules.cliffDays * 1 days);

    uint256 vestingTimestamp = cliffTimestamp + (investor.schedules.recurrence * period * 1 days);
    ...
    if (currentTimeStamp > startTimeStamp) {
      if (currentTimeStamp <= cliffTimestamp) {
        ...
        uint256 diffDays = ((currentTimeStamp - cliffTimestamp) / (period * 1 days));
  ...
  */
  function withdrawTokensAutomatically(
    ...
    VestingCategory _vest = intToEnum[_vCat];
    Investor storage investor = investorsInfo[_user][_vest][_index];

    uint256 tokensAvailable = withdrawableTokens(_user, _vest, _index);
    ...
    investorsInfo[_user][_vest][_index].lastReceivedTime = block.timestamp;
  ...
  */
  ...
  function _computeSoftCap(uint256 _soldLocked, uint256 _soldUnlocked) internal returns (bool _status, uint256 _value) {
    ...
    _status ? globalTime = block.timestamp : globalTime;
  }
}
