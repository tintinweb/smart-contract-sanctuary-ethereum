...
abstract contract DeIDOffering is ERC777, AccessControl, ReentrancyGuard {
    ...
    function investorSell(uint256 tokensToSell)
        public
    ...
    function contributorPurchase() public payable onlyRole(CONTRIBUTORS_ROLE) {
        require(
            currentState != State.pauseAll || currentState != State.pauseBuy
        );
        uint256 amountWei = msg.value;
    ...
    function contributorSell(uint256 tokensToSell)
        public
