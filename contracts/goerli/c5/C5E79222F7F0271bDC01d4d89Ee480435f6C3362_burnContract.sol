...
contract burnContract is ERC721AQueryable, Ownable, RevokableDefaultOperatorFilterer, ReentrancyGuard {

    address private OGContract = 0x69CE25fdBe295e6F78308E977e660D1Cdad4ecF3;
    address private signer = 0x2f2A13462f6d4aF64954ee84641D265932849b64;
    string public _metadata = "metadata.com/";

    uint256 public regularMinted = 0;
    uint256 public SOTYClaimed = 0;
    uint256 private maxBurnPerWallet = 10;

    uint256 private MAX_REGULAR_MINT = 200;
    uint256 private MAX_SOTY_MINT = 25;
    uint256 private MAX_SUPPLY = MAX_REGULAR_MINT + MAX_SOTY_MINT;

    mapping(address => uint256) public totalMintedRegular;
    mapping(address => bool) public claimedSOTY;

    constructor() ERC721A("90sJunk", "JNKIES") {
        for(uint i = 0; i < MAX_SOTY_MINT; i++)
    ...
    function burnToken(uint256[] memory tokenIds) public nonReentrant {
        ...
        require(minted + amount <= MAX_SUPPLY, "Minted out");
...
function claimSOTY(address wallet, bytes calldata voucher, bool delegate) public nonReentrant {
    ...
    require(!claimedSOTY[wallet], "Already minted");

    require(SOTYClaimed + 1 <= MAX_SOTY_MINT, "Sold out");

    claimedSOTY[wallet] = true;

    // Approve the contract to spend the SOTY token on behalf of the owner
    approve(address(this), SOTYClaimed);

    // Transfer the SOTY token from the contract to the person claiming it
    transferFrom(address(this), msg.sender, SOTYClaimed);
    ...
    function tokenURI(uint256 tokenId) public view virtual override(IERC721A, ERC721A) returns (string memory) {
        ...
        return string(abi.encodePacked(_metadata, Strings.toString(tokenId)));
	}
