...
/// @title An interface for a contract that is capable of deploying Lottery contracts
/// @notice A contract that constructs a Lottery must implement this to pass arguments to the Lottery
/// @dev This is used to avoid having constructor arguments in the Lottery contract, which results in the init code hash
/// of the Lottery being constant allowing the CREATE2 address of the Lottery to be cheaply computed on-chain
interface ILotteryDeployer {
    /// @notice Get the parameters to be used in constructing the Lottery, set transiently during Lottery creation.
    /// @dev Called by the Lottery constructor to fetch the parameters of the Lottery
    /// Returns factory The factory address
    /// Returns ticketPrice The chainlink VRF coordinator address
    /// Returns durationMinutes The chainlink VRF key hash
    function parameters()
        external
        view
        returns (address factory, uint256 ticketPrice, uint256 durationMinutes);
}
...
interface ILotteryFactory {
    ...
    /// @param ticketPrice The first token of the pool by address sort order
    /// @param durationMinutes The second token of the pool by address sort order
    /// @param lottery The address of the created pool
    event LotteryCreated(
        uint256 indexed ticketPrice,
        uint256 indexed durationMinutes,
        address lottery
    );
    ...
    /// @notice Returns the lottery address for a given pair of tokens and a fee, or address 0 if it does not exist
    /// @dev tokenA and tokenB may be passed in either token0/token1 or token1/token0 order
    /// @param ticketPrice The contract address of either token0 or token1
    /// @param durationMinutes The contract address of the other token
    /// @return lottery The lottery address
    function getLottery(
        uint256 ticketPrice,
        uint256 durationMinutes
    ) external view returns (address lottery);

    /// @notice Creates a pool for the given two tokens and fee
    /// @param ticketPrice One of the two tokens in the desired pool
    /// @param durationMinutes The other of the two tokens in the desired pool
    /// @dev tokenA and tokenB may be passed in either order: token0/token1 or token1/token0. tickSpacing is retrieved
    /// from the fee. The call will revert if the pool already exists, the fee is invalid, or the token arguments
    /// are invalid.
    /// @return lottery The address of the newly created pool
...
import "./interfaces/ILotteryDeployer.sol";
...
error JackpotNotZero();
error PayoutsIncomplete();
error InvalidDurationMinutes();
error InvalidPlayer();
error InvalidPayout();
error EmptyBalance();
error InvalidCall();
    ...
    NoDelegateCall
{
    ...
    constructor(
        ...
        vrfCoordinatorV2Interface = VRFCoordinatorV2Interface(
    ...
    function vrfSettings(
        ...
        if (vrfCoordinatorAddress_ == address(0))
    ...
    function initialize(
        ...
        if (feeDestination_ == address(0))
            revert InvalidFeeDestinationAddress();
        if (feePercentage_ == 0) revert InvalidFeePercentage();
        if (winnerPayoutPercentage_ == 0)
            revert InvalidWinnerPayoutPercentage();
        if (charityPercentage_ > 0 && charityDestination_ == address(0))
            revert InvalidCharityDestinationAddress();
        if (charityPercentage_ == 0 && charityDestination_ != address(0))
            revert InvalidCharityDestinationAddress();
    ...
    function _isReadyForDraw() internal view returns (bool) {
        ...
        bool isExpired = (block.timestamp >=
            (openedAt + durationMinutes * 1 minutes));

        return (isOpen && (maxPlayersReached || isExpired));
    }
    ...
    function _isReadyToAcceptPlayer(
        address player_
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        if (_isReadyForPayouts()) {
            ...
            ) = _computePayouts(payoutsExecuted);
            performData = abi.encode(
                LotteryState.ReadyForPayouts,
                _addresses,
                _balances
            );
            upkeepNeeded = true;
    ...
    function performUpkeepManual(
        ...
        (
            ...
            address[] memory _addresses,
            uint256[] memory _balances
        ) = abi.decode(performData, (LotteryState, address[], uint256[]));
    ...
    function _performUpkeep(
        ...
        if (nextState == LotteryState.ReadyForPayouts)
            return _executePayouts(_addresses, _balances);
    }
    ...
    function _computePayouts(
        ...
        uint256 currentBlockPayouts = payoutsLeft > payoutsPerBlock
            ? payoutsPerBlock
            : payoutsLeft;
        ...
        for (
            ...
            i < (startIndex + currentBlockPayouts);
            i = unsafeInc(i)
        ) {
    ...
    function _executePayouts(
        ...
        for (
            ...
            index = unsafeInc(index)
        ) {
            // Ensure that
            // 1. the payout is not executed twice for the same player
            if (!playerHasEntered[_addresses[index]]) revert InvalidPlayer();
            ...
            if (_addresses[index] == winner && _payouts[index] != winnerPayout)
                revert InvalidPayout();
            else if (
                _addresses[index] != winner &&
                _payouts[index] != leftOverPayoutPerPlayer
            ) revert InvalidPayout();
...
contract LotteryDeployer is ILotteryDeployer {
    struct Parameters {
        address factory;
        uint256 ticketPrice;
        uint256 durationMinutes;
    ...
    function getCreationBytecode(
        ...
        return
            abi.encodePacked(
                bytecode,
                abi.encode(ticketPrice, durationMinutes)
            );
    }

    /// @dev Deploys a lottery contract with the given parameters by transiently setting the parameters storage slot and then
    /// clearing it after deploying the lottery.
    /// @param ticketPrice The first token of the pool by address sort order
    /// @param durationMinutes The second token of the pool by address sort order
    function deploy(
        address vrfCoordinatorAddress,
        uint256 ticketPrice,
        uint256 durationMinutes
    ) internal returns (address lottery) {
        address addr;
...
import "./LotteryDeployer.sol";
import "./NoDelegateCall.sol";

import {Lottery} from "./Lottery.sol";

/// @title Canonical Uniswap V3 factory
/// @notice Deploys Uniswap V3 lottery and manages ownership and control over pool protocol fees
contract LotteryFactory is ILotteryFactory, LotteryDeployer, NoDelegateCall {
    /// @inheritdoc ILotteryFactory
    address public override owner;

    /// @inheritdoc ILotteryFactory
    mapping(uint256 => mapping(uint256 => address)) public override getLottery;
    ...
    function createLottery(
        ...
        uint256 ticketPrice,
        uint256 durationMinutes
    ) external override noDelegateCall returns (address lottery) {
        // TODO: ADD CHECKS FOR TICKET PRICE AND DURATION MINUTES
        lottery = deploy(vrfCoordinatorAddress, ticketPrice, durationMinutes);

        getLottery[ticketPrice][durationMinutes] = lottery;
        getLottery[durationMinutes][ticketPrice] = lottery;

        emit LotteryCreated(ticketPrice, durationMinutes, lottery);
    }
