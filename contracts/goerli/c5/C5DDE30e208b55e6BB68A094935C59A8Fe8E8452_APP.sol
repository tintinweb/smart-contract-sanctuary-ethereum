...
import "./interface/IsafeContruct.sol";
...
abstract contract APPcore is  Ownable{
    ...
    IsafeContruct public safecontruct;

    address public constant WITHDRAW_ADDRESS = 0x6277A815CB471ec4FDf84C599D8293DB7BbFF437;
    uint256 public maxSupply = 10000;
    uint256 public maxBurnMint = 2000;
    uint256 public limitGroup = 0;      //0 start
    uint256 public limitRelease = 1;    //LimitReleaseON>2
    uint256 public limitReleaseMaxAmount = 1;
    uint256 public cost = 0.001 ether;
    string public baseURI = "https://nft.aopanda.ainy-llc.com/app/metadata/";
    string public baseExtension = ".json";
    bytes32 public merkleRoot = 0;
    uint256 public wlcount = 0;         // max:65535 Always raiseOrder
    uint256 public bmcount = 0;         // max:65535 Always raiseOrder
    Phase public phase = Phase.BeforeMint;
    address public royaltyAddress = 0x6277A815CB471ec4FDf84C599D8293DB7BbFF437;
...
abstract contract APPadmin is APPcore,AccessControl,ERC721AQueryable,ERC2981{
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override(IERC721A,ERC721A) {
        // if(address(safecontruct) != address(0)){
        //     require(safecontruct.isSafe(operator) == true,"address no list");
        // }
...
contract APP is APPadmin {
    ...
    //constructor() ERC721A('AopandaParty', 'APP') {
    constructor() ERC721A('Test Panda 1004', 'TST') {
        // _safeMint(0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482, 2000);
        // _safeMint(0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482, 100);
        // _safeMint(0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482, 100);
        // _safeMint(0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482, 100);
        // _safeMint(0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482, 100);
        // _safeMint(0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482, 60);
        // _safeMint(0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482, 60);
        
        //_safeMint(0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266, 5);
        _safeMint(0x6277A815CB471ec4FDf84C599D8293DB7BbFF437, 10);
    ...
    function _beforeTokenTransfers(
        ...
        // RewardLockUp
        address[6] memory rewardaddress = [0x6277A815CB471ec4FDf84C599D8293DB7BbFF437,
                                           0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482,
                                           0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482,
                                           0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482,
                                           0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482,
                                           0x6A1Ebf8f64aA793b4113E9D76864ea2264A5d482];
        for(uint256 i = 0; i < rewardaddress.length; i++){
            if(from == rewardaddress[i]){
                ...
                // UnixTime 1685577600:UTC 2023.06.01
                //require(block.timestamp > 1685577600,"Transfer is not possible during lockup.");

                //test 10/4 20:00
                require(block.timestamp > 1664881200,"Transfer is not possible during lockup.");
            }
        }
    }
    ...
    function _checkWLCount(address _owner) internal{
        ...
        if(_auxval.get16_forAux(0) < wlcount){
            // console.log("_auxval.set16_forAux(0,uint64(wlcount)).set16_forAux(1,0):%s",_auxval.set16_forAux(0,uint64(wlcount)).set16_forAux(1,0));
            _setAux(_owner,_auxval.set16_forAux(0,uint64(wlcount)).set16_forAux(1,0));  // CountUp + Clear
        }
    }

    function _checkBMCount(address _owner) internal{
        uint64 _auxval = _getAux(_owner);
        if(_auxval.get16_forAux(2) < bmcount){
            // console.log("_auxval.set16_forAux(2,uint64(bmcount)).set16_forAux(3,0):%s",_auxval.set16_forAux(2,uint64(bmcount)).set16_forAux(3,0));
    ...
    function _getAuxforWLAmount(address _owner) internal returns (uint64){
        ...
        // console.log("_getAux(_owner).get16_forAux(1): '%s'",_getAux(_owner).get16_forAux(1));
        return _getAux(_owner).get16_forAux(1);
    }

    function _getAuxforBMAmount(address _owner) internal returns (uint64){
         _checkBMCount(_owner);
        // console.log("_getAux(_owner).get16_forAux(3): '%s'",_getAux(_owner).get16_forAux(3));
        return _getAux(_owner).get16_forAux(3);
    }

    function _setAuxforWL(address _owner, uint64 _aux) internal {
        _checkWLCount(_owner);
        // console.log("_getAux(_owner).set16_forAux(1,_aux): '%s'", _getAux(_owner).set16_forAux(1,_aux));
        _setAux(_owner,_getAux(_owner).set16_forAux(1,_aux));
    }

    function _setWLmintedCount(address _owner,uint256 _mintAmount) internal{
        //require(_auxval < 65535,"minted count is over");
        unchecked {
            // console.log("_getAuxforWLAmount(_owner) + uint64(_mintAmount): '%s'", _getAuxforWLAmount(_owner) + uint64(_mintAmount));
    ...
    function _setAuxforBM(address _owner, uint64 _aux) internal{
        _checkBMCount(_owner);
        // console.log("_owner,_getAux(_owner).set16_forAux(3,_aux): '%s'",_owner,_getAux(_owner).set16_forAux(3,_aux));
        ...
        if(phase == Phase.WLMint){
            if(getWLExit(_address,_wlAmountMax,_wlGroup,_merkleProof) == true){
                if(limitRelease == 1){
                    ...
                    // console.log("_Amount:'%s' _getAux(_address).get16_forAux(1):'%s'", _Amount ,_getAux(_address).get16_forAux(1));
                }else{
                    _Amount = limitReleaseMaxAmount - _getAux(_address).get16_forAux(1);    // After limit release
                }
            } 
        }
        // console.log("return _Amount:'%s'", _Amount );
        return _Amount;
    }
    ...
    function getBMRemain(address _address,uint256 _wlAmountMax,uint256 _wlGroup,bytes32[] calldata _merkleProof
        ...
        if(phase == Phase.BurnMint){
            if(getWLExit(_address,_wlAmountMax,_wlGroup,_merkleProof) == true){
                ...
                // console.log("_Amount:'%s' _getAux(_address).get16_forAux(3):'%s'", _Amount ,_getAux(_address).get16_forAux(3));
            } 
        }
        return _Amount;
    }
    ...
    function getWLExit(address _address,uint256 _wlAmountMax,uint256 _wlGroup,bytes32[] calldata _merkleProof
        ...
        //console.log("_address %s _wlAmountMax %s _wlGroup %s", _address, _wlAmountMax,_wlGroup);
    ...
    // external
    // function checkWLCount(address _ower)external{
    //     _checkWLCount(_ower);
    // }
    // function checkBMCount(address _ower)external{
    //     _checkBMCount(_ower);
    // }
    ...
    function mint(uint256 _mintAmount,uint256 _wlAmountMax,uint256 _wlGroup,bytes32[] calldata _merkleProof)
        ...
        _checkWLCount(msg.sender);  // Always check before getWLRemain
    ...
    function burnMint(uint256[] memory _burnTokenIds,uint256 _wlAmountMax,uint256 _wlGroup,bytes32[] calldata _merkleProof) 
        ...
        _checkBMCount(msg.sender);  // Always check before getBMRemain
...
interface IsafeContruct{
    function isSafe(address _address) external view returns(bool result);
}
