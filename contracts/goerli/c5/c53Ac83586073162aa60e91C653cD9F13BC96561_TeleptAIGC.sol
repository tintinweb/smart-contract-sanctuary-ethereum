...
contract TeleptAIGC is ERC721URIStorage, Ownable {
    ...
    bool public requireInvitation = false;

    string baseURI = "https://aigc.telept.xyz/aigcnft/";

    mapping(address => uint256) mintCount;
    mapping(address => bool) invitationList;

    constructor() ERC721("TeleptAIGC", "TeleptAIGC") {}

    function mintNFT(string calldata aigcId) public payable returns (uint256) {
        ...
        return mintInternal(msg.sender, aigcId);
    }

    function mintInternal(
        address to,
        string calldata aigcId
        ...
        require(tokenId < totalSupply, "no more tokens");
        ...
        _mint(to, tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked(tokenId, "-", baseURI, aigcId)));
    ...
    function airdrop(
        ...
        return mintInternal(wAddresses, aigcId);
    }
    ...
    function setRequireInvitation(
        bool requireInvitationUpdate
    ) public onlyOwner {
        requireInvitation = requireInvitationUpdate;
    }

    function addToInvitationList(
