...
contract OrderManager is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function placeOrder(
        ...
        require(_pool.validateToken(_indexToken, _side, _collateralToken), "OrderManager: invalid tokens");
        if (_updateType == UpdatePositionType.INCREASE) {
    ...
    function placeSwapOrder(
        ...
        require(executionFee >= minExecutionFee, "executionFeeTooLow");
    ...
    function executeOrder(uint256 _orderId, address payable _feeTo) external nonReentrant {
        ...
        require(order.owner != address(0), "OrderManager: order not exists");
        require(isPool[address(order.pool)], "OrderManager: invalid or paused pool");
        require(block.number > order.submissionBlock, "OrderManager: block not pass");
    ...
    function cancelOrder(uint256 _orderId) external nonReentrant {
        Order memory order = orders[_orderId];
        require(order.owner == msg.sender, "OrderManager: unauthorized cancellation");
        UpdatePositionRequest memory request = requests[_orderId];
    ...
    function _createDecreasePositionOrder(
        ...
        if (_orderType == OrderType.MARKET) {
            (order.price, order.payToken, request.sizeChange, request.collateral) = abi.decode(
                _data,
                (uint256, address, uint256, uint256)
            );
            order.triggerAboveThreshold = _side == Side.LONG;
        } else {
            (order.price, order.triggerAboveThreshold, order.payToken, request.sizeChange, request.collateral) = abi
                .decode(_data, (uint256, bool, address, uint256, uint256));
        }
        order.pool = _pool;
        ...
        require(order.executionFee >= minExecutionFee, "OrderManager: insufficient execution fee");
    ...
    function _createIncreasePositionOrder(
        ...
        address purchaseToken;
        uint256 purchaseAmount;
        (order.price, purchaseToken, purchaseAmount, request.sizeChange, request.collateral) = abi.decode(
            _data,
            (uint256, address, uint256, uint256, uint256)
        );

        require(purchaseAmount > 0, "OrderManager: invalid purchase amount");
        require(purchaseToken != address(0), "OrderManager: invalid purchase token");
    ...
    function setOracle(address _oracle) external onlyOwner {
        require(_oracle != address(0), "OrderManager: invalid oracle addres");
        oracle = IOracle(_oracle);
        emit OracleChanged(_oracle);
    }

    function addPool(address _pool) external onlyOwner {
        require(!isPool[_pool], "OrderManager: pool already added");
        require(_pool != address(0), "OrderManager: invalid pool addres");
        isPool[_pool] = true;
        allPools.push(_pool);
    ...
    function removePool(address _pool) external onlyOwner {
        require(isPool[_pool], "OrderManager: pool not added");
        require(_pool != address(0), "OrderManager: invalid pool addres");
        isPool[_pool] = false;
