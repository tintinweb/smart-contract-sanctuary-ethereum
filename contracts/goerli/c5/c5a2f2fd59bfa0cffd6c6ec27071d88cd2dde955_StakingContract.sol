...
contract StakingContract {
    ...
    event areenaTokenSold(address sellerAddress, uint256 lowerMileStone, uint256 upperMileStone, uint256 LowerBusdLimit);
    ...
    function BuyAreenaBoster() external {
        ...
        emit areenaBooster(msg.sender, areenaBosterPrice, player.walletLimit);
    }
    ...
    function sell(uint256 amount) external {
        ...
        if(!onceGreater){
            require(busdToken.balanceOf(busdWallet) >= (102000*1e18) ,"Selling of Areena token will start when BusdTreasury wallet reaches 102000.");
            onceGreater = true;
        }
        ...
        require(busdToken.balanceOf(busdWallet) > lowerMileStone,"Areena selling Start when busdTreasury will be greater then lower milestone.");
        require(busdToken.balanceOf(busdWallet) <= uppermileStone,"Areena selling Start when busdTreasury will be less or equal to uppermilestone.");
        ...
        if(busdToken.balanceOf(busdWallet) > lowerMileStone && busdToken.balanceOf(busdWallet) <= uppermileStone){
            ...
            if(busdToken.balanceOf(busdWallet) > (200000*1e18)){
                ...
                emit areenaTokenSold(msg.sender, sendAmount, lowerMileStone, lowerMileStone);
            }
            else{
    ...
    function setAreenaBosterPrice() public returns(uint256){

        uint256 ABV = ((totalAreena - areenaInCirculation).div(1e18)).mul(10);
        uint256 findValue = (ABV.div(10000)). mul(25);
        uint256 bosterPercentage = calculateBosterPercentage(findValue);
        areenaBosterPrice = bosterPercentage;
    ...
    function getAreenaBosterPrice() external returns(uint256){  
