...
contract Test is VRFConsumerBaseV2 {
    ...
    bytes32 private s_keyHash = 0x79d3d8832d904592c0bf9818b621522c988bb8b0c05cdc3b15aea1b6e8db0c15;
    uint16 private requestConfirmations = 3;

    uint256 private _lotteryNumber;
    uint256 public _mintBet = 3000 wei; // 投一注需要的金額
    uint256 public _minPick = 3000 wei; // 開獎需達成的金額
    uint256 public _expired = 4 minutes;
    ...
    modifier notExpired(uint256 _lotteryId) {
        LotteryData storage lottery = lotterys[_lotteryId];
        require(block.timestamp < lottery.expiredAt, "lottery is expired");
        _;
    }
    ...
    event ExceedTimeAndRefund(uint256 _lotteryid);

    modifier potChecker(uint256 _lotteryId) {
        LotteryData storage lottery = lotterys[_lotteryId];
        require(lottery.pot >= _minPick, "not enough pot");
        require(block.timestamp < lottery.expiredAt, "expired already");
        _;
    }
    ...
    event Test(uint256 _randomNumber, uint256 _totalParticipants);


    // 投注
    function entry(uint256 _lotteryId) public payable isActive(_lotteryId) notExpired(_lotteryNumber) {
        require(msg.value >= _mintBet, "not enougt bet value");
    ...
    */
    function openLottery() external onlyOwner {
        ...
        lottery.expiredAt = block.timestamp + _expired;
        lotteryRound++;
    }
        ...
        lock
    {
        ...
        */
        if (block.timestamp > lottery.expiredAt) {
            for (uint256 i; i < lottery.totalParticipants; i++) {
                ...
                emit ExceedTimeAndRefund(_lotteryId);
            }
        } else {
            // 確認已產生隨機數，確保公平
    ...
    // 向預言機請求，取得隨機數
    function getRandomNumber(uint256 _lotteryId) 
        external
        onlyOwner
        potChecker(_lotteryId)
    ...
    function fulfillRandomWords(
        ...
        randomNumber = randomWords[0] % 50;
        emit Test(randomWords[0], lottery.totalParticipants);
    }
