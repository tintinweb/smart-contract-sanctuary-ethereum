...
contract SynthrIssuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function burnForRedemption(address deprecatedSynthProxy, address account, uint256 balance) external onlySynthRedeemer {
    ...
    function liquidateAccount(
        ...
        bytes32 _collateralKey,
        bool isSelfLiquidation
    ) external onlySynthetix returns (uint256 totalRedeemed, uint256 amountToLiquidate, uint256 sharesToRemove) {
    ...
    function destIssue(address _account, bytes32 _synthKey, uint256 _synthAmount, uint256 _debtShare) external onlySynthrBridge {
        // ISynthetixDebtShare sds = synthetixDebtShare();
    ...
    function destBurn(address _account, bytes32 _synthKey, uint256 _synthAmount, uint256 debtShare) external onlySynthrBridge {
    ...
    function _burnSynths(
        ...
        _removeCoveredDebtShare(debtAccount, amountBurntInUSD);
    ...
    function _removeFromDebtRegister(address from, uint256 debtToRemove, uint256 existingDebt) internal returns (uint256) {
        // important: this has to happen before any updates to user's debt shares
        liquidatorRewardsManager().updateEntry(from);
    ...
    function _removeCoveredDebtShare(address from, uint256 amount) internal returns (uint256) {
