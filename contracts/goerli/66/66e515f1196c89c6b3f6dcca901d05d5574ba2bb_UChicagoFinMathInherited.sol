...
// add function addOwner : Takes one address as an argument and adds to the list of owners.  Any owner can use this function.
// first only the original owner of the contract can add an owner, then any owner can add an owner
    function addOwner(address _member) public isMember { //runs the is owner code everytime you check this function
        {members[_member] = true;}
    }


    // add function deleteOwner: Remove an owner from the list of owners.  Only the original owner (i.e., the one who launched the contract) may call this function.
    // make sure only original owner of the contract can edit this list
