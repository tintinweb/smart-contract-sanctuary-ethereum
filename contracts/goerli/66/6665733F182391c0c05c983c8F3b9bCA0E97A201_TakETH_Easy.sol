...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minPeriod        = 24*60 minutes; 
    uint256 public minHoldReq       = 100 * (10**9);
...
contract TakETH_Easy is IERC20, Context {
    ...
    constructor() {
        ...
        TransactionSettings.maxTxAmount = _totalSupply / 50;
        ...
        LiquiditySettings.numTokensToSwap = (_totalSupply * 10) / (4000);
    ...
    function clearStuckBalance(uint256 amountPercentage) external onlyOwner {
        ...
        payable(ecosystemFeeReceiver).transfer(
            (amountEth * amountPercentage) / 100
        );
        ecosystemFees += amountEth * amountPercentage;
    }
    ...
    function setBuyFees(uint16 _liquidityFee, uint16 _reflectionFee, uint16 _ecosystemFee) external authorized {
        require(_liquidityFee + _ecosystemFee + _reflectionFee <= MaxFees.totalFee);
        BuyFees = IFees({
            liquidityFee: _liquidityFee,
            ecosystemFee: _ecosystemFee,
            reflectionFee: _reflectionFee,
            totalFee: _liquidityFee + _ecosystemFee
        });
    }
    
    function setTransferFees(uint16 _liquidityFee, uint16 _reflectionFee, uint16 _ecosystemFee) external authorized {
        require(_liquidityFee + _ecosystemFee + _reflectionFee <= MaxFees.totalFee);
        TransferFees = IFees({
            liquidityFee: _liquidityFee,
            ecosystemFee: _ecosystemFee,
    ...
    function setSellFees(uint16 _liquidityFee, uint16 _reflectionFee, uint16 _ecosystemFee) external authorized {
        require(_liquidityFee + _ecosystemFee + _reflectionFee <= MaxFees.totalFee);
        SellFees = IFees({
            liquidityFee: _liquidityFee,
            ecosystemFee: _ecosystemFee,
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _ecosystemFeeReceiver) public onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        ecosystemFeeReceiver = _ecosystemFeeReceiver;
        emit FeeReceiversUpdated(_autoLiquidityReceiver, _ecosystemFeeReceiver);
    }
   ...
   function setAutoLiquifyEnabled(bool _enabled) public authorized {
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(block.timestamp >= Launch.launchedAt + 1 hours && Launch.earlySellFee){
            SellFees = IFees({
                liquidityFee: 3,
                ecosystemFee: 0,
                reflectionFee: 4,
    ...
    function swapBack() internal swapping {
        ...
        if (LiquiditySettings.liquidityFeeAccumulator >= LiquiditySettings.numTokensToSwap && LiquiditySettings.autoLiquifyEnabled) {
            ...
            uint256 amountEthEcosystem = (amountEth *
                (BuyFees.ecosystemFee + SellFees.ecosystemFee)) /
            ...
            if(fundRewards) {
                ...
                (bool ecosystem, ) = payable(ecosystemFeeReceiver).call{ value: amountEthEcosystem, gas: 30000}("");
                if(ecosystem) ecosystemFees += amountEthEcosystem;
            } else {
                (bool ecosystem, ) = payable(ecosystemFeeReceiver).call{ value: amountEthEcosystem, gas: 30000}("");
                if(ecosystem) ecosystemFees += amountEthEcosystem;
            }
    ...
    function getAccumulatedFees() external view returns (uint256 collectedFees, uint256 currentBalance) {
        collectedFees = ecosystemFees;
        currentBalance = ecosystemFeeReceiver.balance;
    }
    ...
    event FeeReceiversUpdated(address autoLiquidityReceiver, address ecosystemFeeReceiver);
    event Launched();
