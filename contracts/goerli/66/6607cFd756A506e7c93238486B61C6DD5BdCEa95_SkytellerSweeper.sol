...
/**
 * @title BaseOwnable
 * @notice Common functionality for Skyteller owned contracts
 */
abstract contract BaseOwnable is Ownable2Step, Pausable {
...
error Skyteller_NegativePrice(address feed);

/// @dev Price feed returned a zero price,
///      isn't valid in our token use case
error Skyteller_ZeroPrice(address feed);
...
/// @dev Unable to derive a price from the given feeds
error Skyteller_NoPriceFeedDerivation(address feedIn, address feedOut);
...
/// @dev Unable to find token price feed
error Skyteller_TokenPriceFeedNotFound(address token);
...
import {ISkytellerSweepDelegate} from "./interfaces/ISkytellerSweepDelegate.sol";
import {IWETH9} from "./interfaces/IWETH9.sol";
import {TokenPricing} from "./lib/TokenPricing.sol";
import {
    Skyteller_InvalidFee,
    Skyteller_InvalidPercent,
    Skyteller_NoSwapRoute,
    Skyteller_NoPriceDerivation
} from "./SkytellerErrors.sol";

/**
 * @title SkytellerSweeper
 * @notice Version 2 of the sweeper delegate for Skyteller
 */

contract SkytellerSweeper is ISkytellerSweepDelegate, BaseOwnable, Initializable, Multicall {
    using SafeERC20 for IERC20;
    ...
    event SetSkytellerSwapFee(uint16 skytellerSwapFee);

    /// @dev Owner has set the minimum percent out
    event SetMinimumOutPercent(uint16 minimumOutPercent);
    ...
    uint16 internal constant DEFAULT_MINIMUM_SWAP_PERCENT_OUT_BIPS = 9850;
    uint16 internal constant DEFAULT_SKYTELLER_SWAP_FEE_BIPS = 75;
    uint16 internal constant BIPS_PRECISION = 10000;

    /// @notice Uniswap uses precision 1 BIP = 100
    uint24 internal constant UNISWAP_POOL_FEE_PRECISION = 1000000;
    ...
    /// @notice Fee for conducting swaps (1 bip = 100)
    uint16 public skytellerSwapFee = DEFAULT_SKYTELLER_SWAP_FEE_BIPS;

    /// @notice Minimum amount out for conducting swaps (1 bip = 100)
    uint16 public minimumOutPercent = DEFAULT_MINIMUM_SWAP_PERCENT_OUT_BIPS;

    /// @dev The set of tokens that can be swept
    EnumerableSet.AddressSet internal _tokens;

    /// @notice Get the token USD price feed
    mapping(address => address) public priceFeedUsd;

    /// @notice Get the token ETH price feed
    mapping(address => address) public priceFeedEth;
     ...
     */
    function tokenPriceUsd(address _token) external view returns (uint256 price, uint8 decimals) {
        return TokenPricing._tokenPrice(priceFeedUsd[_token]);
    }
     ...
     * @return decimals The number of decimals in the price
     */
    function tokenPriceEth(address _token) external view returns (uint256 price, uint8 decimals) {
        return TokenPricing._tokenPrice(priceFeedEth[_token]);
    }
     ...
     * @return The price of tokenIn in terms of tokenOut
     */
    function currentPrice(address tokenIn, address tokenOut, uint256 decimals)
        external
        view
        returns (uint256)
    {
        ...
        (address feedIn, address feedOut) = _pricingFeeds(tokenIn, tokenOut);
        return TokenPricing._derivePriceFromFeeds(feedIn, feedOut, decimals);
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        fee = skytellerSwapFee * swapOut / BIPS_PRECISION;
    ...
    /// @dev The fee is denominated in BIPs, e.g. 1% = 100
    /// @param _fee The fee for sweeping
    function setSkytellerSwapFee(uint16 _fee) public onlyOwner {
        if (_fee > BIPS_PRECISION) {
    ...
    /// @param _minimumOutPercent The minimum percent out for swaps
    function setMinimumOutPercent(uint16 _minimumOutPercent) external onlyOwner {
        if (_minimumOutPercent > BIPS_PRECISION) {
            revert Skyteller_InvalidPercent();
        }
        minimumOutPercent = _minimumOutPercent;
        emit SetMinimumOutPercent(minimumOutPercent);
    }
    ...
    function setPool(address token0, address token1, uint24 fee) external onlyOwner {
        if (fee > UNISWAP_POOL_FEE_PRECISION) {
            revert Skyteller_InvalidFee();
        }
        if (token0 > token1) {
    ...
    /// @notice Add a token to the list of supported tokens with a price feed
    /// @param _token The token to add
    /// @param usdFeed The address of the USD price feed for the token
    function setTokenPriceFeedUsd(address _token, address usdFeed) external onlyOwner {
        _tokens.add(_token);
        priceFeedUsd[_token] = usdFeed;
    }

    /// @notice Add a token to the list of supported tokens with a price feed
    /// @param _token The token to add
    /// @param ethFeed The address of the USD price feed for the token
    function setTokenPriceFeedEth(address _token, address ethFeed) external onlyOwner {
        _tokens.add(_token);
        priceFeedEth[_token] = ethFeed;
    }

    /// @notice Remove a token from the list of supported tokens with price feeds
    function removeToken(address _token) external onlyOwner {
        _tokens.remove(_token);
        delete priceFeedUsd[_token];
        delete priceFeedEth[_token];
    }
    ...
    function _routerPath(address tokenIn, address tokenOut)
        internal
        view
        returns (bytes memory path)
    {
        ...
        if (poolFee1 == 0) {
            poolFee1 = poolFee(tokenIn, weth);
            poolFee2 = poolFee(weth, tokenOut);
            if (poolFee1 == 0 || poolFee2 == 0) {
                revert Skyteller_NoSwapRoute(tokenIn, tokenOut);
            }
            return abi.encodePacked(tokenIn, poolFee1, weth, poolFee2, tokenOut);
        } else {
            return abi.encodePacked(tokenIn, poolFee1, tokenOut);
        }
    }

    function _pricingFeeds(address tokenIn, address tokenOut)
        internal
        view
        returns (address feedIn, address feedOut)
    {
        feedIn = priceFeedUsd[tokenIn];
        feedOut = priceFeedUsd[tokenOut];
        if (feedIn == address(0) || feedOut == address(0)) {
            feedIn = priceFeedEth[tokenIn];
            feedOut = priceFeedEth[tokenOut];
            if (feedIn == address(0) || feedOut == address(0)) {
    ...
    {
        bytes memory path = _routerPath(tokenIn, tokenOut);
...
import {Skyteller_NegativePrice} from "../SkytellerErrors.sol";
import {Skyteller_ZeroPrice} from "../SkytellerErrors.sol";

library TokenPricing {
    /**
     * @notice Derives the price of tokenIn in terms of tokenOut
     * @dev
     * @param feedIn The token to derive the price of
     * @param feedOut The token to derive the price in terms of
     * @param decimals The number of decimals in the price
     * @return price The price of tokenIn in terms of tokenOut
     */
    function _derivePriceFromFeeds(address feedIn, address feedOut, uint256 decimals)
        internal
        view
        returns (uint256 price)
    {
        ...
        (priceIn, decimalsIn) = _tokenPrice(feedIn);
        (priceOut, decimalsOut) = _tokenPrice(feedOut);
        if (priceIn == 0) {
            revert Skyteller_ZeroPrice(feedIn);
        }
        if (priceOut == 0) {
            revert Skyteller_ZeroPrice(feedOut);
        }
        ...
        return Math.mulDiv(priceIn, 10 ** decimals, priceOut);
    }

    /**
     * @notice Calculate the amount of tokenOut to receive
     *         based on current price feed data
     * @dev The  amount is calculated as follows:
     *      amountOut = derivedPrice * amountIn.
     *
     * @param amountIn The token to swap from
     * @param amountInDecimals The token to swap to
     * @param amountOutDecimals The amount of tokenIn to swap
     * @param feedIn The token to derive the price of
     * @param feedOut The token to derive the price in terms of
    ...
    function _amountOut(
        ...
        uint256 amountOutDecimals,
        address feedIn,
        address feedOut
    ) internal view returns (uint256) {
        return Math.mulDiv(
            _derivePriceFromFeeds(feedIn, feedOut, amountOutDecimals),
            amountIn,
            10 ** amountInDecimals
        );
    }
     ...
     * @param feed The token to fetch the price of
     * @return price The price of the token
     * @return decimals The number of decimals in the price
     */
    function _tokenPrice(address feed) internal view returns (uint256 price, uint8 decimals) {
        (, int256 answer,,,) = AggregatorV3Interface(feed).latestRoundData();
        if (answer < 0) {
            revert Skyteller_NegativePrice(feed);
        }
