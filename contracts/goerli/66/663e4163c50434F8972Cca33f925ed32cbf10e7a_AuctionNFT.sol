...
contract AuctionNFT is Ownable, ERC1155Holder {
    mapping(address => bool) public signers;
    using SafeERC20 for IERC20;
    using ECDSA for bytes32;

    event buy(address indexed user, uint itemId, uint amount);
    event userGetNFT(address indexed user, uint itemId, uint amount);

    address public paymentKeeper;
    address immutable paymentToken;
    IERC1155 immutable paymentNFT;
    ...
    function buyItem(
        ...
        bytes32 _msgForSign = keccak256(
            abi.encode(payType, amount, itemId, nftId, msg.sender)
    ...
    function getNFT(
        ...
        require(!items[itemId], "other user has got this item");
        items[itemId] = true;
        emit userGetNFT(msg.sender, itemId, amount);

        bytes32 _msgForSign = keccak256(
            abi.encode(amount, itemId, nftId, msg.sender)
        ).toEthSignedMessageHash();
    ...
    function transferAllNFTToOwner(uint itemId) external onlyOwner {
        paymentNFT.safeTransferFrom(
            address(this),
            msg.sender,
            itemId,
            paymentNFT.balanceOf(address(this), itemId),
            ""
        );
    }
}
