...
contract TheCircle is ERC1155, Ownable {
    ...
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _amount, bytes memory _data) public override {
        require(authorizedReceivers[_id][_to].isAuthorized, "Receiver not authorized");
    ...
    function safeBatchTransferFrom(address _from, address _to, uint256[] memory _ids, uint256[] memory _amounts, bytes memory _data) public override {
        for (uint i = 0; i < _ids.length; i++) {
            require(authorizedReceivers[_ids[i]][_to].isAuthorized, "Receiver not authorized");
        }
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        bytes memory svg = abi.encodePacked(
            ...
            '<circle cx="50" cy="50" r="40" fill="', getColor(timestamp, blockNumber), '"/>',
            '</svg>'
        );
        string memory dataURI = string(abi.encodePacked('data:image/svg+xml;base64,', Base64.encode(svg)));
        return string(abi.encodePacked('data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
            '{"name": "The Circle #', uint256ToString(tokenId), '", "description": "A randomly generated circle with a random color based on timestamp and block number.", "image": "', dataURI, '"}'
        )))));
    }

    function getColor(uint256 timestamp, uint256 blockNumber) internal pure returns (string memory) {
    ...
    function toHex(uint8 value) internal pure returns (bytes1) {
        if (value < 10) {
