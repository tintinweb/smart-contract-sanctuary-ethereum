...
interface ILiqdLending {
    event ContractApprovementUpdated(
        address _contract,
        bool _status,
        uint256 _timestamp
    );

    function isAvailableCurrency(address _currency) external returns (bool);

    function createLoanExternal(DataTypes.LoanPayload memory _payload)
        external
        payable
        returns (uint256);
}
...
contract LendingCore is Ownable, ILendingCore {
    mapping(address => bool) public contractApprovement;
...
library DataTypes {
    ...
    struct Offer {
        address nftAddress; // address of nft
        address currency; // address of loans" currency
        address lender; // address of offer creator
        uint256 amount; // amount of currency
        uint256 apr; // APR of loan
        uint256 maxLoanPeriod; // max period of loan
        uint256 loanId; // if 0, then not accepted yet
        uint8 nftTokenType;
    }
}
...
import "./libraries/DataTypes.sol";
import "./interfaces/ILiqdLending.sol";
    ...
    ILiqdLending
{
    ...
    bytes32 public constant EXTERNAL_CALLER_ROLE =
        keccak256("EXTERNAL_CALLER_ROLE");
    ...
    mapping(bytes => bool) internal cancelledSignatures;
    ...
    /// @notice display if currency is available or not
    /// @param _currency address of currency
    function isAvailableCurrency(address _currency)
        external
        view
        override
        returns (bool)
    {
        return availableCurrencies[_currency];
    }

    /// @notice internal function to create loan
    /// @param _payload structure LoanPayload
    /// @return currentId the id of loan
    function _createLoan(DataTypes.LoanPayload memory _payload)
        internal
        returns (uint256 currentId)
    {
        ...
        if (
            ...
            require(toSuccess, "TX_FAILED");
            if (platformFee > 0) {
                (bool daoSuccess, ) = DAO.call{value: platformFee}("");
                require(daoSuccess, "TX_FAILED_TO_DAO");
            }
        }
        emit LoanCreated(
            _payload.lender,
    ...
    function createLoanExternal(DataTypes.LoanPayload memory _payload)
        ...
        override
        nonReentrant
        onlyRole(EXTERNAL_CALLER_ROLE)
        returns (uint256 loanId)
    {
        loanId = _createLoan(_payload);
    }
    ...
    /// @return loanId the id of loans
    function createLoan(
        DataTypes.LoanPayload memory _payload,
        bytes memory _sig
    ) external payable nonReentrant returns (uint256) {
        require(!usedSignatures[_sig], "ALREADY_USED_SIGNATURE");
        require(!cancelledSignatures[_sig], "INVALID_SIGNATURE");
        ...
        uint256 currentId = _createLoan(_payload);
    ...
    function createCOffer(
        ...
        require(!usedSignatures[_sig], "ALREADY_USED_SIGNATURE");
        require(!cancelledSignatures[_sig], "INVALID_SIGNATURE");
        require(msg.sender == _payload.borrower, "CALLER_SHOULD_BE_BORROWER");
        ...
        nonReentrant
    {
        ...
        if (
            ...
            require(msg.value >= _amountDue, "NOT_ENOUGH_ETH");
            (bool toSuccess, ) = loan.lender.call{value: _amountDue}("");
            require(toSuccess, "TX_FAILED");
            if (msg.value > _amountDue) {
                (bool toSuccess, ) = payable(msg.sender).call{
