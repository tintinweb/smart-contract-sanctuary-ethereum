...
contract TIM is ERC1155, ReentrancyGuard, Ownable {
    ...
    function getTextLinesOneThroughSeven(uint256 startY, string[] memory stringLine) private pure returns (string memory) {
        return string(
            abi.encodePacked(
                ...
                getSVGTextLine(startY + 140, stringLine[7])
            )
        );
    }

    function getTextLinesEightThroughFourteen(uint256 startY, string[] memory stringLine) private pure returns (string memory) {
        return string(
            abi.encodePacked(
                getSVGTextLine(startY, stringLine[8]),
                getSVGTextLine(startY + 20, stringLine[9]),
                getSVGTextLine(startY + 40, stringLine[10]),
                getSVGTextLine(startY + 60, stringLine[11]),
                getSVGTextLine(startY + 80, stringLine[12]),
                getSVGTextLine(startY + 100, stringLine[13]),
                getSVGTextLine(startY + 120, stringLine[14])
            )
        );
    }
        ...
        return
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        header,
                        getTextLinesOneThroughSeven(20, stringLine),
                        getTextLinesEightThroughFourteen(180, stringLine),
                        '</text>',
                        footer,
                        '</svg>'
                    )
