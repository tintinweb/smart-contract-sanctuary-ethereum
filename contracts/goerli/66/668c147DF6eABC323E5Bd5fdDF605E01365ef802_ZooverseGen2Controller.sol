...
/**
    Conditions:
        ...
        - Public no sales, cannot mint through contract, at price .15
 */
...
contract ZooverseGen2Controller is BaseMint {
    ...
    constructor() {
        sales[1] = Sale(genesisLimit, 1, "");
        sales[2] = Sale(genesisLimit + 1, 1, "");
        sales[3] = Sale(genesisLimit, 1, "");
        sales[4] = Sale(1, 1, ""); 
        sales[5] = Sale(1, 2, "");
        _lastSaleType = 5;
    }
    ...
    modifier correctMintConditions(uint256 saleType, uint256 quantity, bytes32[] calldata proof) {
        ...
        nft.setAux(msg.sender, uint64(quantity));
        _;
    }

    function claimMint(uint256 quantity, bytes32[] calldata proof) 
        external 
        payable 
        enoughPrice(quantity, discountedPrice) 
        correctMintConditions(1, quantity, proof) 
    {        
