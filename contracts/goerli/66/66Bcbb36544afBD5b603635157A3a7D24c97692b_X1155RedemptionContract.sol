...
abstract contract Closeable is Teams {
  ...
  abstract contract ERC1155Redemption is ERC1155, Teams {
      ...
      struct RedemptionDetail {
          ...
          mapping(address => bool) redemptionContracts;
      ...
      function bulkSetRedemptionDetails(
          ...
          details.redemptionContracts[_redemptionContract] = true;
      }

      function redemptionStatsForContract(uint256 tokenId, address _contractAddress) public view returns (uint256, bool, bool, bool) {
          RedemptionDetail storage details = redemptionDetail[tokenId];
          return (
              details.redemptionSurcharge[_contractAddress],
              details.redemptionModeEnabled,
              details.verifiedClaimModeEnabled,
              details.redemptionContracts[_contractAddress]
          );
      }
      ...
      function setRedeemableContract(uint256 tokenId, address _contractAddress, bool _status) public onlyTeamOrOwner {
        redemptionDetail[tokenId].redemptionContracts[_contractAddress] = _status;
      }

      // @dev Allow owner/team to determine if contract is accepting redemption mints
      function setRedemptionMode(uint256 tokenId, bool _newStatus) public onlyTeamOrOwner {
        redemptionDetail[tokenId].redemptionModeEnabled = _newStatus;
      }

      // @dev Allow owner/team to determine if contract is accepting verified claim mints
      function setVerifiedClaimMode(uint256 tokenId, bool _newStatus) public onlyTeamOrOwner {
        redemptionDetail[tokenId].verifiedClaimModeEnabled = _newStatus;
      }

      // // @dev Set the fee that it would cost a minter to be able to burn/validtion mint a token on this contract.
      function setRedemptionSurcharge(uint256 tokenId, address _contractAddress, uint256 _newSurchargeInWei) public onlyTeamOrOwner {
        redemptionDetail[tokenId].redemptionSurcharge[_contractAddress] = _newSurchargeInWei;
      }

      // @dev Set the redemption address where redeemed NFTs will be transferred when "burned".
      // @notice Must be a wallet address or implement IERC1155Receiver.
      function setRedemptionAddress(uint256 tokenId, address _newRedemptionAddress) public onlyTeamOrOwner {
        redemptionDetail[tokenId].redemptionAddress = _newRedemptionAddress;
      }
  }
  ...
  function mintToMultiple(address _to, uint256 _id, uint256 _qty) public payable whenNotPaused {
    ...
    **/
    function redeem(uint256 tokenId, address redemptionContract, uint256 ftokenId) public payable {
        ...
        if(!details.redemptionContracts[redemptionContract]) revert IneligibleRedemptionContract();
