...
contract PoorPlebOnce is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(bytes32 _merkleRoot) ERC20("PoorPleb", "PP") {
        _mint(msg.sender, 67000000000000000000000000000);
        merkleRoot = _merkleRoot;

        claimAmount = 1420369000000000000000000;
        PAAmountPerClaim = 420369000000000000000000;
        totalClaimedAmount = 0;
        allowClaimAllDate = block.timestamp + 7 days; // todo change to 69 days on real version
    ...
    function claim(bytes32[] calldata merkleProof) external {
        ...
        if (claimedCounter[msg.sender] >= 1) {
            require(
        ...
        returns (bool)
    {
        ...
        if (claimedCounter[claimer] == 0) {
            _canClaim = true;
        } else {
            _canClaim = !claimed[claimer] &&
            MerkleProof.verify(
                merkleProof,
                merkleRoot,
