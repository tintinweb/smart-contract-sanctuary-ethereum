...
contract QuestMinter is Ownable {
    ...
    uint256 public startTokenId = 10000;
    ...
    event Claimed(uint256 indexed tokenId, address indexed user);
    ...
    function createQuest(
        ...
        emit QuestCreated(startTokenId, msg.sender, startTs, endTs, uri);
        startTokenId += 1;
    }
    ...
    function setCustomURI(
        ...
        require(quest.creator == msg.sender, "Not creator");
    ...
    function claim(uint256 tokenId, bytes calldata signature) external {
        ...
        Quest storage quest = quests[tokenId];
        if (quest.supply > 0)
            require(badge.tokenSupply(tokenId) < quest.supply, "Over limit");

        require(block.timestamp > quest.startTs, "Not in time");
        if (quest.endTs > 0)
            require(block.timestamp <= quest.endTs, "Not in time");
