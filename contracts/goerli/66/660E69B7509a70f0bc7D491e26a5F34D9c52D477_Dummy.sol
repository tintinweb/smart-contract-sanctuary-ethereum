...
contract Dummy is ERC721A, EIP712, ReentrancyGuard, Ownable {
    ...
    string public baseURI = "http://preview/";
    bytes32 public constant MINTER_TYPEHASH =
        keccak256("Minter(address recipient)");
    address public signer;
    Stage public stage;

    constructor() ERC721A("Dummy", "Dummy") EIP712("Dummy", "1.0.0") {
        _mint(msg.sender, 1);
        signer = msg.sender;
        stage = Stage.Pause;
    }

    function signatureMint(uint256 _quantity, bytes calldata _signature)
        external
        payable
    {
        require(stage == Stage.Sale, "STAGE_NMATCH");
        require(signer == _verify(msg.sender, _signature), "INVALID_SIGNATURE");
        require(msg.value >= (price * _quantity), "INSUFFICIENT_FUND");

        _mint(msg.sender, _quantity);
    }

    function signatureFreeMint(uint256 _quantity, bytes calldata _signature)
        external
    {
        require(stage == Stage.Sale, "STAGE_NMATCH");
        require(signer == _verify(msg.sender, _signature), "INVALID_SIGNATURE");
    ...
    function _verify(address _recipient, bytes calldata _sign)
        internal
        view
        returns (address)
    {
        bytes32 digest = _hashTypedDataV4(
            keccak256(abi.encode(MINTER_TYPEHASH, _recipient))
        );
    ...
    function withdrawAll() external onlyOwner {
        ...
        sendValue(payable(signer), walletBalance);
    }
