...
//@title Adventures of Blackman - Episode 1
//@author Satoshigoat (gh:@jcksber)
contract AdventuresOfBlackman is Satoshigoat {

	struct TokenInfo {
		uint256 timesTraded;//number of times token has been traded
		uint8 degredationPeriod;//number of trades to cause a degredation - between 5 and 50?
	}

	//token id -> associated token info
	mapping(uint256 => TokenInfo) internal _tokens;

	//the six degredation phases
	string [6] internal _hashes = ["","","","","",""];
	//NOTE: the nft never fully deteriorates rn...

	// -----
	// Core
	// -----

	constructor() Satoshigoat("Adventures of Blackman - Episode 1", "", "ipfs://") 
	{
	...
	{	
		...
		return string(abi.encodePacked(_baseURI(), _getIpfsHash(tid)));
	}

	//@dev Determine the hash to be used by the number of trades & degredation period ([0,5])
	function _getIpfsHash(uint256 tid) internal view returns (string memory)
	{	
		//NOTE: the film never fully deteriorates rn
		uint state = _tokens[tid].timesTraded / _tokens[tid].degredationPeriod;
		state = state > 5 ? 5 : state;//state is in [0,5]
		 
		return _hashes[state];
	}
	...
	{	
		// If this isn't a mint transaction, increment `timesTraded` for the token
		if (from != address(0)) {
			uint i;
			uint nextIdx = startTokenId + quantity;
			for (i = startTokenId; i < nextIdx; i++) {
				//make sure this works.....
				_tokens[i].timesTraded += 1;
			}
		}
	}

	//@dev Allows owners to mint for free 
	function mint(address to, uint256 qty) external isSquad nonReentrant enoughSupply(qty)
	{
		_mintInternal(to, qty);
	}

	//@dev Purchase function - max 3 per call
	function purchase(address to, uint256 qty) external nonReentrant enoughSupply(qty)
	{	
		if (_isContract(to)) 
			revert DataError("silly rabbit :P");
		if (qty > 3)
			revert DataError("can only mint 3 at a time");
		
		_mintInternal(to, qty);
	}

	//@dev Helper function for minting
	function _mintInternal(address to, uint256 qty) internal
	{
		_safeMint(to, qty);
		// Give the tokens a random number for the degredation cycle
		uint i;
		uint currentIdx = _totalMinted();
		uint nextIdx = currentIdx + qty;//bits are good for the family 
		for (i = currentIdx; i < nextIdx; i++) {
			uint8 rnd = uint8(_random(i) % 50 + 5);//5 to 55
			// Create a new token and assign values before adding to `_tokens`
			TokenInfo memory newToken;
			newToken.timesTraded = 0;
			newToken.degredationPeriod = rnd;
			_tokens[i] = newToken;
		}
	}
	...
	//@dev [BACKUP FUNCTION] Set a hash in `_hashes`
	function setHashAtIndex(string memory newHash, uint256 idx) external isSquad
	{
		if (0 > idx || idx > 5)
			revert DataError("idx OOB");
		if (_stringsEqual(newHash, _hashes[idx]))
			revert DataError("same hash");
...
contract Satoshigoat is ERC721A, SquadOwnable, ReentrancyGuard {
	...
	//@dev Sale Data
	uint256 public constant MAX_NUM_TOKENS = 77;
	...
	//@dev _startTokenId should be paid attention to if using this mod lol
	modifier onlyValidTokenId(uint256 tid) {
		if (tid < _startTokenId() || tid > MAX_NUM_TOKENS + _startTokenId() - 1)
    	...
    	ERC721A(name_, symbol_)
    {
    	...
    	_contractURI = "ipfs://add address";
    	_owner = address(0x6b8C6E15818C74895c31A1C91390b3d42B336799);//change
    }
