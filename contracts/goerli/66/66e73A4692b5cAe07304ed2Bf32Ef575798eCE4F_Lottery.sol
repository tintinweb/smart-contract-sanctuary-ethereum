 ...
 * // note --> Sending ETH to pay the Lottery:  0.001 ETH = "lotteryFee"
 * // note --> + Transfering USDC to start lending: 10.0 USDC = "lotteryTicketPrice"
 */
...
contract Lottery is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    /* State Variables */
    // Lottery Variables
    uint256 private immutable i_lotteryFee; // in ETH 18 decimals
    uint256 private immutable i_lotteryTicketPrice; // in USDC 6 decimals
    uint256 private immutable i_interval; // Lottery & ChainLink Keepers
    uint256 private s_lastTimeStamp;
    uint256 private s_newPrize;
    uint256 private s_totalNumTickets;
    ...
    bool public isFirstPlayer = true;
     ...
     * 1. Lottery is in "OPEN_TO_LEND" state ("closed" when waiting for a random number from Chainlink VRF)
    ...
    function fulfillRandomWords(
        ...
        // set LotteryState
        //* note: TODO set LotteryState to OPEN_TO_WITHDRAW and Set a KEEPERS WITH TIME AUTOMATION
     ...
     * need 1. user transfer USDC to "this" and then 2. _approveAndSupplyCompound() called by Lottery contract
     * note: /!\ to implement from FRONT END with instance of USDC : firstly REQUIRES USER DIRECT INTERACTION w/ USDC ERC20 CONTRACT for USER TO TRANSFER i_lotteryTicketPrice USDC TO "THIS"
     * note IF called by ADMIN: no need for 1. All current Lottery USDC balance will supply Compound.
     * note:  "THIS" accumulates USDC from users: total lotteryUSDCBalance = _getlotteryUSDCBalance()
     * note: after user transfer to this, then Lottery contract calls Compound to:
     * Approves Compound to use Lottery's USDC
     * Supplies Compound with USDC (amount = f( isFirstPlayer, Admin ))
     */
    ...
    /**
   * @notice function called by Player to get USDC
   * Player calls LotteryToken: transfers its Tokens to Lottery
   * Reset Player mapping toNumTickets
   * Updates totalNumTickets
   * Lottery calls USDC: Transfer player's ALL USDC from Lottery
   ...
   */
    ...
    */
    ...
    /**
    * @notice Getter
    * returns the Lottery USDC balance
    */
    ...
    */
    function _getPlayerRatio(address _player)
        internal
        returns (
          uint256 userRatio
        )
