...
/*
* This contract is used to collect DIVx stacking dividends from fee (like swap, deposit on pools or farms)
*/
contract DivMoneyPot is Ownable, IMoneyPot {
    ...
    mapping(address => mapping (address => UserInfo)) public DIVxHoldersRewardsInfo;
    // user => LastDIVxBalanceSaved
    mapping (address => uint256) public DIVxHoldersInfo;
    ...
    // At least, it will include the masterChef address as masterChef minting continuously DIVx for rewards on DIVs pair pool.
    // We can add later LP contract if someone initialized DIVx LP
    // Those contracts are included as holders on DIVx
    ...
    uint256 public lastDIVxSupply; // Cache the last totalSupply of DIVx

    constructor (IBEP20 _DIVx, address _feeManager, address _masterDiv, uint256 _startBlock, uint256 _initialUpdateMoneyPotPeriodNbBlocks) public{
        updateMoneyPotPeriodNbBlocks = _initialUpdateMoneyPotPeriodNbBlocks;
        startBlock = _startBlock;
        lastUpdateMoneyPotBlocks = _startBlock;
        DIVx = _DIVx;
        addressWithoutReward[_masterDiv] = true;
    ...
    function _updateTokenPot(address _token) internal {
        ...
        if (block.number >= tokenPot.lastUpdateTokenPotBlocks.add(updateMoneyPotPeriodNbBlocks)){
            if(tokenPot.tokenAmount > 0){
                ...
                tokenPot.accTokenPerShare = tokenPot.accTokenPerShare.add(tokenRewardsPerBlock.mul(multiplier).mul(1e12).div(lastDIVxSupply));
            }
    ...
    */
    function pendingTokenRewardsAmount(address _token, address _user) external view returns (uint256){
        ...
        if (block.number > lastRewardBlock && lastDIVxSupply != 0 && tokenReward > 0) {
            accTokenPerShare = accTokenPerShare.add(
                    tokenReward.mul(getMultiplier(lastRewardBlock, block.number)
                ).mul(1e12).div(lastDIVxSupply));
        }
        return (DIVx.balanceOf(_user).mul(accTokenPerShare).div(1e12).sub(DIVxHoldersRewardsInfo[_token][_user].rewardDept))
                    .add(DIVxHoldersRewardsInfo[_token][_user].pending);
    }


    /*
    Update tokenPot, user's DIVx balance (cache) and pending dividends
    */
    function updateDIVxHolder(address _DIVxHolder) external virtual override {
        uint256 holderPreviousDIVxAmount = DIVxHoldersInfo[_DIVxHolder];
        uint256 holderBalance = DIVx.balanceOf(_DIVxHolder);
        uint256 length = registeredToken.length;
        for (uint256 index = 0; index < length; ++index) {
            _updateTokenPot(registeredToken[index]);
            TokenPot storage tokenPot = _distributedMoneyPot[registeredToken[index]];
            if(holderPreviousDIVxAmount > 0 && tokenPot.accTokenPerShare > 0){
                uint256 pending = holderPreviousDIVxAmount.mul(tokenPot.accTokenPerShare).div(1e12).sub(DIVxHoldersRewardsInfo[registeredToken[index]][_DIVxHolder].rewardDept);
                if(pending > 0) {
                    if (addressWithoutReward[_DIVxHolder]) {
                        if(DIVxHoldersRewardsInfo[registeredToken[index]][_DIVxHolder].pending > 0){
                            pending = pending.add(DIVxHoldersRewardsInfo[registeredToken[index]][_DIVxHolder].pending);
                            DIVxHoldersRewardsInfo[registeredToken[index]][_DIVxHolder].pending = 0;
                        }
                        reserveTokenAmount[registeredToken[index]] = reserveTokenAmount[registeredToken[index]].add(pending);
                    }
                    else {
                        DIVxHoldersRewardsInfo[registeredToken[index]][_DIVxHolder].pending = DIVxHoldersRewardsInfo[registeredToken[index]][_DIVxHolder].pending.add(pending);
                    }
                }
            }
            DIVxHoldersRewardsInfo[registeredToken[index]][_DIVxHolder].rewardDept = holderBalance.mul(tokenPot.accTokenPerShare).div(1e12);
        }
        if (holderPreviousDIVxAmount > 0){
            lastDIVxSupply = lastDIVxSupply.sub(holderPreviousDIVxAmount);
        }
        lastDIVxSupply = lastDIVxSupply.add(holderBalance);
        DIVxHoldersInfo[_DIVxHolder] = holderBalance;
    }

    function harvestRewards(address _DIVxHolder) external {
        uint256 length = registeredToken.length;

        for (uint256 index = 0; index < length; ++index) {
            harvestReward(_DIVxHolder, registeredToken[index]);
        }
    }
    ...
    */
    function harvestReward(address _DIVxHolder, address _token) public {
        uint256 holderBalance = DIVxHoldersInfo[_DIVxHolder];
        _updateTokenPot(_token);
        TokenPot storage tokenPot = _distributedMoneyPot[_token];
        if(holderBalance > 0 && tokenPot.accTokenPerShare > 0){
            uint256 pending = holderBalance.mul(tokenPot.accTokenPerShare).div(1e12).sub(DIVxHoldersRewardsInfo[_token][_DIVxHolder].rewardDept);
            if(pending > 0) {
                if (addressWithoutReward[_DIVxHolder]) {
                        if(DIVxHoldersRewardsInfo[_token][_DIVxHolder].pending > 0){
                            pending = pending.add(DIVxHoldersRewardsInfo[_token][_DIVxHolder].pending);
                            DIVxHoldersRewardsInfo[_token][_DIVxHolder].pending = 0;
                        }
                        reserveTokenAmount[_token] = reserveTokenAmount[_token].add(pending);
                }
                else {
                    DIVxHoldersRewardsInfo[_token][_DIVxHolder].pending = DIVxHoldersRewardsInfo[_token][_DIVxHolder].pending.add(pending);
                }
            }
        }
        if ( DIVxHoldersRewardsInfo[_token][_DIVxHolder].pending > 0 ){
            safeTokenTransfer(_token, _DIVxHolder, DIVxHoldersRewardsInfo[_token][_DIVxHolder].pending);
            DIVxHoldersRewardsInfo[_token][_DIVxHolder].pending = 0;
        }
        DIVxHoldersRewardsInfo[_token][_DIVxHolder].rewardDept = holderBalance.mul(tokenPot.accTokenPerShare).div(1e12);
    }
    ...
    */
    ...
    /*
    * Allow token address to be distributed as dividends to DIVx holder
    */
    ...
    /*
    Remove token address to be distributed as dividends to DIVx holder
    */
