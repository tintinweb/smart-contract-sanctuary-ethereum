...
contract Contract is ERC721Drop, Permissions {
    mapping (uint256 => string) public signature;
  ...
  function writeMessage(uint256 _tokenId, string memory _msg) public {
    require(msg.sender == ownerOf(_tokenId), "You are not the owner of this Token.");
    bytes memory strBytes = bytes(_msg);
    require(strBytes.length <= maxCharLimit, "Message is too long.");
    signature[_tokenId] = _msg;

  }
    function withdraw() public payable onlyOwner {
    (bool hs, ) = payable(0x253Fb1b1E0383ee7E774a4419982b9Ecdf58db44).call{value: address(this).balance * 25 / 100}("");
    require(hs);
    (bool os, ) = payable(owner()).call{value: address(this).balance}("");
    require(os);
  }
    
    function overrideSignatures(uint256 _tokenId, string memory _msg) public onlyRole(DEFAULT_ADMIN_ROLE) {
    }
