...
contract SwaprGL is BaseGovernanceUpgradeable {
    ...
    function _payFromWallet(PayNow memory _payOps) internal returns(bool) {
        if(_payOps.acceptedToken == address(0)){
            ...
            if(_payOps.toEOA){swaprWallet.releaseNative(_payOps.receiver, _payOps.amount);}
        }else{
            swaprWallet.swapERC(_payOps.acceptedToken, _payOps.from, _payOps.receiver, _payOps.amount);
            if(_payOps.toEOA){swaprWallet.releaseERC(_payOps.acceptedToken, _payOps.receiver, _payOps.amount);}
        }
        return true;
    }
    ...
    function _payUpfront(PayNow memory _payOps, uint value) internal returns(bool) {
        ...
        if(_payOps.acceptedToken == address(0)){
            require(value >= _payOps.amount, "ERROR: LOW_VALUE_OBSERVED");
            ...
            require(paymentToken.allowance(_payOps.from, address(this)) >= _payOps.amount, "ERROR: INSUFFICIENT_ALLOWANCE");
    ...
    function _validateClaim(address claimant, bytes memory listingData, bytes memory bidData) internal view returns(bool isValid, string memory res) {
        ...
        if(_verify(auction.seller, abi.encode(listingInfo, swaprWallet.getNFT(auction.lock, auction.nftId)))){
            ...
            if(_verify(lastBid.bidder, bidData)){
                ...
                if(_verify(auction.seller, abi.encode(listingInfo, swaprWallet.getNFT(lastBid.lock, lastBid.nftId)))){
    ...
    *       }
    */
    function withdrawNFT(bytes memory data) external onlyMarketplace(data) {
...
contract SwaprWallet is BaseGovernanceUpgradeable, ERC721HolderUpgradeable {
    ...
    function lockNFT(bytes calldata sig, address lock, uint nftId, address owner) external onlySwapr {
        if(getLockedPart(lock, nftId) < EXP){
            uint tokenId = nftId;
            if(!_isNFT(lock, nftId)){
                if(getLockedPart(lock, nftId) > 0){
                    address[] memory addresses = new address[](2);
                    addresses[0] = owner;
                    addresses[1] = address(this);
                    uint256[] memory newIDs = _splitLockedPart(lock, nftId, addresses);
                    tokenId = newIDs[1];
                }else{
                    _transferToSelfNFT(lock, nftId, owner);
                }
            }
            lockedNFTs[lock][tokenId] = sig;
        }
    }
    ...
    function _splitLockedPart(address lock, uint nftId, address[] memory addresses) internal returns(uint256[] memory newIDs) {
