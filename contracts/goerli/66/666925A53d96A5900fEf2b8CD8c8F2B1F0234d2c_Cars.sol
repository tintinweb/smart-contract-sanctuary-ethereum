...
function createCar(string memory _color,string memory _name,uint _year, bool reg) public payable {
    require (msg.value == 0.01 ether);
    carowner[id]=msg.sender;
    cars.push(Car(_color,_name,_year,reg));
    (cars[id].color, cars[id].name, cars[id].year, cars[id].registered)=(_color,_name,_year, reg);
    id++;
}
