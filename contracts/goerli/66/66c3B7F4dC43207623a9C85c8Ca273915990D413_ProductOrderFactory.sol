...
contract ProductOrder is KeeperCompatibleInterface {
    ...
    function performUpkeep(bytes calldata) external override {
        ...
        if (s_state == POState.SENT || s_state == POState.ACCEPTED) {
            ...
            payable(i_purchaserAddress).transfer(s_amountOfMoney - s_tokenWorthShipped);
            payable(i_vendorAddress).transfer(s_tokenWorthShipped);
            s_amountOfMoney = 0;
        }
    }
