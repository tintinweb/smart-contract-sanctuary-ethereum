...
abstract contract AccessManaged {
    ...
    error AccessManagerInsufficientPermissions();

    /// @dev Checks with the AccessMananger wheter the function can be called by the msg.sender
    modifier canCallFunction() {
        bytes4 functionSig = msg.sig;
        if (!_accessManager.canCall(msg.sender, address(this), functionSig)) {
            revert AccessManagerInsufficientPermissions();
        }
        _;
    }

    constructor(IAccessManager accessManager) {
...
contract DynamicERC20 is ERC20, ERC20Burnable, Pausable, AccessManaged {
    IAccessManager private _accessManager;

    constructor(
        IAccessManager accessManager
    ) ERC20("DynamicERC20", "DR") AccessManaged(accessManager) {}

    function pause() public canCallFunction {
        _pause();
    }

    function unpause() public canCallFunction {
        _unpause();
    }

    function mint(address to, uint256 amount) public canCallFunction {
        _mint(to, amount);
    }
