...
contract FreaksNGuilds is Controllable, Ownable, ERC721A("Freaks N' Guilds", "FnG") {
  ...
  uint256 public constant FNG_PRICE_ETH = 0.1 ether;
  ...
  constructor(
    ...
    isFBXSaleLive = _isFBXSaleLive;
    isETHSaleLive = _isETHSaleLive;
    fbx = IFBX(_fbx);
    ckey = ICKEY(_ckey);
  ...
  /// @notice Toggle `isETHSaleLive`.
  function toggleETHSaleLive() external onlyOwner {
    isETHSaleLive = !isETHSaleLive;
  }

  /// @notice Toggle `isFBXSaleLive`.
  function toggleFBXSaleLive() external onlyOwner {
    isFBXSaleLive = !isFBXSaleLive;
  }

  /// @dev Call the `metadaHandler` to retrieve the tokenURI for each character.
  function tokenURI(uint256 id) public view override returns (string memory) {
    if (celestials[id].powMod != 0) {
      // Celestial
      Celestial memory celestial = celestials[id];
      return
        metadaHandler.getCelestialTokenURI(id, celestial.healthMod, celestial.powMod, celestial.cPP, celestial.cLevel);
    } else if (freaks[id].species != 0) {
      // Freak
      Freak memory freak = freaks[id];
      return
        metadaHandler.getFreakTokenURI(
          id,
          freak.species,
          freak.body,
          freak.mainHand,
          ...
          freak.power,
          freak.health,
          freak.criticalStrikeMod
        );
    } else {
      return ""; // placeholder for compile
      // return baseMetadata; // need to create baseMetadata for name, description, and pre-reveal image
    }
  }
  ...
  function mintWithETH(uint256 amount) external payable noCheaters {
    uint256 supply = _currentIndex;
    require(supply + amount <= maxSupply, "maximum supply reached");
    if (msg.sender != owner()) {
      require(isETHSaleLive == true, "mint not live ser");
      require(msg.value >= amount * FNG_PRICE_ETH, "invalid ether amount");
    }
    uint256 rand = _rand();
  ...
  function mintWithETHHoldersOnly(uint256[] memory ckeyIds) external payable noCheaters {
    uint256 supply = _currentIndex;
    ...
    uint256 rand = _rand();
    for (uint256 i = 0; i < amount; i++) {
      require(msg.sender == ckey.ownerOf(ckeyIds[i]), "invalid token ID");
  ...
  function mintWithETHWhitelist(uint256 amount, bytes32[] memory proof) external payable {
    uint256 supply = _currentIndex;
    require(supply + amount <= maxSupply, "maximum supply reached");
  ...
  function mintWithFBX(uint256 amount) external noCheaters {
    uint256 supply = _currentIndex;
    require(supply + amount <= maxSupply, "maximum supply reached");
    if (msg.sender != owner()) {
      require(isFBXSaleLive == true, "mint not live ser");
    }
    uint256 rand = _rand();
    for (uint256 i = 0; i < amount; i++) {
      uint256 rNum = rand % 100;
  ...
  function _revealFreak(uint256 rNum, uint256 id) internal {
    uint256 _rNum = _randomize(rNum, id);
    uint8 species = _getFreaky(_rNum);
    _rNum = _randomize(_rNum, id);
    uint16 mainHand = _getEquipment(species, _rNum);
    _rNum = _randomize(_rNum, id);
    uint8 body = _getBody(species, _rNum);
    _rNum = _randomize(_rNum, id);
    uint8 power = _calcPow(species, _rNum);
    _rNum = _randomize(_rNum, id);
    uint8 health = _calcHealth(species, _rNum);
    _rNum = _randomize(_rNum, id);
    uint16 armor = _getEquipment(species, _rNum); // replace with function call
    uint8 criticalStrikeMod = 0;
    Freak memory freak = Freak(species, body, mainHand, offHand, armor, power, health, criticalStrikeMod);
    freaks[id] = freak;
    freakSupply += 1;
  }
  ...
  function _calcMod(uint256 rNum) internal view returns (uint8) {
    // uint256 _rNum = _randomize(rNum, _rand(), id);
    // might need to cast? we will see...
    return uint8((rNum % 7) + baseMod);
  }
  ...
  function _calcPow(uint8 species, uint256 rNum) internal pure returns (uint8) {
    ...
    // might need to cast? we will see...
    return uint8((rNum % 31) + basePow);
  }

  function _getFreaky(uint256 rNum) internal pure returns (uint8) {
    // might need to cast? we will see...
    return uint8((rNum % 3) + 1);
  }

  function _getBody(uint8 species, uint256 rNum) internal pure returns (uint8) {
    uint8 base = uint8((rNum % 3) + 1); // troll
    if (species == 2) {
      base += 3; // fairy
    } else if (species == 3) {
      base += 6; // ogre
  ...
  function _getEquipment(uint8 species, uint256 rNum) internal pure returns (uint16) {
    uint16 base = uint16((rNum % 3) + 1); // troll
    if (species == 2) {
      base += 1000; // fairy
    } else if (species == 3) {
      base += 2000; // ogre
