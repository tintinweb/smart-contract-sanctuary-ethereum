...
// Author: @redappleabc
contract ServicooUniswapLiquidity is IERC721Receiver, Ownable {

    uint24 public servicooFee = 300000;
    ...
    function mintNewPosition(address token0, address token1, uint256 amount0ToMint, uint256 amount1ToMint, uint24 poolFee)
        external
        returns (
            uint256 tokenId,
            uint128 liquidity,
    ...
    function _sendToOwner(
        ...
        TransferHelper.safeTransfer(token0, owner, amount0 * (1000000 - servicooFee) / 1000000);
        TransferHelper.safeTransfer(token1, owner, amount1 * (1000000 - servicooFee) / 1000000);
        TransferHelper.safeTransfer(token0, this.owner(), amount0 * servicooFee / 1000000);
        TransferHelper.safeTransfer(token1, this.owner(), amount1 * servicooFee / 1000000);
    }
    ...
    function setServicooFee(uint24 _fee) external onlyOwner {
        require(_fee >= 0, "fee must be more than 0");
        require(_fee <= 1000000, "fee must be less than  1000000(100%)");
        servicooFee = _fee;
    }
}