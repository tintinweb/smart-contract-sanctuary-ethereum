...
contract DeflationaryERC20 is ERC20Base {
    /*********************************** Structs **********************************/
    struct TokenData {
    ...
    uint256 public taxMin;
    uint256 public taxMax;

    /*********************************** Errors ***********************************/

    /**
     * Max supply reached on mint.
     *
     * @param maxSupply max token supply.
     * @param minSupply min token supply.
     * @param tokenSupply current token supply
     */
        ...
        initializer
    {
         ...
         *   _tokenData sets token data (initial supply, decimals, burn tax, min supply, max supply)
         *   _admins set the admins
         *   _minters set the minters
         *   _excluders eth the excluders
        ...
        if (
            _tokenData.taxMax > 10000 ||
            _tokenData.burnTax > _tokenData.taxMax ||
            _tokenData.burnTax < _tokenData.taxMin
        ) {
            revert TaxOutOfRange({
                tax: _tokenData.burnTax,
                taxMin: _tokenData.taxMin,
                taxMax: _tokenData.taxMax
            });
        }
        ...
        _decimals = _tokenData.decimals;
        tax = _tokenData.burnTax;
        minSupply = _tokenData.minSupply;
        taxMax = _tokenData.taxMax;
        taxMin = _tokenData.taxMin;
        ...
        if (contractOptions.mintEnabled) {
            maxSupply = _tokenData.maxSupply;
        } else {
            maxSupply = _tokenData.initialSupply;
        }

        if (
            _tokenData.initialSupply > maxSupply ||
            _tokenData.initialSupply < minSupply
        ) {
            revert SupplyOutOfRange({
                maxSupply: maxSupply,
                minSupply: minSupply,
                tokenSupply: _tokenData.initialSupply
            });
        }

        _mint(_defaultAdmin, _tokenData.initialSupply);

        _grantRole(OWNER_ROLE, _defaultAdmin);
        _grantRole(ADMIN_ROLE, _defaultAdmin);
        _setRoleAdmin(EXCLUDER_ROLE, ADMIN_ROLE);
        ...
        for (uint256 i = 0; i < _excluders.length; i++) {
            _grantRole(EXCLUDER_ROLE, _excluders[i]);
        }
    }
    ...
    function mint(address to, uint256 amount)
        ...
        mintable
    {
        uint256 newSupply = amount + totalSupply();
    ...
    /**
     * @dev Transfers specified amount from caller to recipient
    ...
    {
        address owner = _msgSender();
        uint256 transferAmount = taxTransfer(owner, amount);
    ...
    /**
     * @dev Transfers specified amount from sender to recipient
    ...
    function transferFrom(
        ...
        address spender = _msgSender();
        uint256 transferAmount = taxTransfer(sender, amount);
    ...
    /**
     * @dev Changes the burn tax
     * @param newTax New tax to change
     */
    function changeBurnTax(uint256 newTax) public adminOrOwner {
        if (!contractOptions.dynamicTaxEnabled) {
            revert DisabledOption({option: 'dynamicTax'});
        }
        if (newTax > taxMax || newTax < taxMin) {
            revert TaxOutOfRange({tax: newTax, taxMin: taxMin, taxMax: taxMax});
        }
        tax = newTax;
    ...
    /**
     * @dev Calculates and burn taxed amount on transfer
     * @param sender Sender of the transfer
     * @param amount Amount of transfer
     */
    function taxTransfer(address sender, uint256 amount)
        internal
        returns (uint256)
    {
        if (totalSupply() <= minSupply || hasRole(EXCLUDER_ROLE, sender)) {
            return amount;
        }

        uint256 taxAmount = Math.min(
            (amount * tax) / 10000,
    ...
    ERC20PermitUpgradeable
{
    ...
    /**
     * Burn Tax is out of range
     * @param tax Tax on transfer
     * @param taxMin Minimum tax on transfer
     * @param taxMax Maximum tax on transfer
     */
    error TaxOutOfRange(uint256 tax, uint256 taxMin, uint256 taxMax);
