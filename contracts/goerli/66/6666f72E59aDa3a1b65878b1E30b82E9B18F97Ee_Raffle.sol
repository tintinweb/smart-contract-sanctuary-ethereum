...
/// @author justandy.eth
/// @notice This contract is for creating a decentralized autonomous lottery
...
contract Raffle is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    function enterRaffle() external payable {
        // Check that the raffle is open
        if (raffleState != RaffleState.OPEN) {
            revert Raffle__RaffleNotOpen();
        }

        // Check that the entrance fee has been met
    ...
    {
        upkeepNeeded = getUpkeepNeeded();
    }

    function performUpkeep(bytes calldata /* performData */) external override {
        bool upkeepNeeded = getUpkeepNeeded();
    ...
    function fulfillRandomWords(
        ...
        if (!success) {
            revert Raffle__PayWinnerFailed();
        }
        emit WinnerPicked(winner);
    }

    function getUpkeepNeeded() private view returns (bool) {
