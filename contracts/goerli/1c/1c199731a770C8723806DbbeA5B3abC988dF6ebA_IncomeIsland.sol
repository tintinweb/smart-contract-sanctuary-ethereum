...
contract IncomeIsland is ERC1155 {
    using Address for address;
    ...
    function mintByProperty(
        ...
        if (isDisabled(_nftId)) {
            nftPrice = utility.getBnbTokenFromDollar(
                (nftTypeList[_nftId].priceBNB * nftTypeList[_nftId].prosBNB) /
                    100
            );
        }
        ...
        nftPrice =
            (nftTypeList[_nftId].priceBNB * (1000 + priceRisingRate)) /
            1000;
        if (isDisabled(_nftId)) {
            nftPrice = nftTypeList[_nftId].priceBNB;
        }
        updatePublicNFTType(
            nftPrice,
    ...
    // /**
    //  * @notice use this function for sending USDT
    //  */
    // function sendUSDT(address receiver, uint256 amount) external onlyOwner {
    //     require(usdt.balanceOf(address(this)) >= amount, "Balance error");
    ...
    function setConfig(
        ...
        metaverseFeePrice = _metaverseFeePrice;
    }
