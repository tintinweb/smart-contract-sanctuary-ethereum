...
contract IDOSale is AccessControl, Pausable, ReentrancyGuard, Ownable2Step {
    ...
    constructor(
        ...
        require(_purchaseMin > 0 && _purchaseCap > 0 && _purchaseCap > _purchaseMin, "IDOSale: PURCHASE_CAP_INVALID");
        require(block.timestamp <= _startTime && _startTime < _endTime && _endTime < _claimableTime, "IDOSale: TIMESTAMP_INVALID");
     ...
     */
    function purchase(uint256 amount) payable external nonReentrant whenNotPaused {
        ...
        require(amount >= purchaseMin, "IDOSale: PURCHASE_MIN_LIMIT");
        ...
        require(purchaseTokenAmount <= msg.value, "IDOSale: INSUFFICIENT_FUNDS");
    ...
    function claim() external nonReentrant whenNotPaused {
        require(claimableTime <= block.timestamp, "IDOSale: CLAIM_NOT_START");
        require(claimedAmounts[_msgSender()] == 0, "IDOSale: CLAIMED");
        uint256 amount = purchasedAmounts[_msgSender()];
    ...
    function sweep(address payable to) external onlyOwner {
        ...
        require(endTime <= block.timestamp, "IDOSale: NOT_END");
      
        uint256 ebal = address(this).balance;
        to.transfer(ebal);

        uint256 bal = ido.balanceOf(address(this));
        if (bal - totalPurchasedAmount > 0) {
            ido.safeTransfer(to, bal - totalPurchasedAmount);
        }
    ...
    function testSetTime(uint256 _startTime, uint256 _endTime, uint256 _claimableTime) external onlyOperator {
        require(block.timestamp <= _startTime && _startTime < _endTime && _endTime < _claimableTime, "IDOSale: TIMESTAMP_INVALID");
        startTime = _startTime;
        endTime = _endTime;
        claimableTime = _claimableTime;
