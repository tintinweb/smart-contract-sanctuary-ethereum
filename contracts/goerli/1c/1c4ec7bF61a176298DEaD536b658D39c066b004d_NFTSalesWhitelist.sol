...
import "../../contracts/test/ERC721Min.sol";

abstract contract $ERC721Min is ERC721Min {
    constructor(string memory name_, string memory symbol_) ERC721Min(name_, symbol_) {}

    function $_owners() external view returns (address[] memory) {
        return _owners;
    }

    function $_isStaked(uint256 arg0) external view returns (bool) {
        return _isStaked[arg0];
    }
    ...
    function $_mintMin() external {
        return super._mintMin();
    }

    function $_mintMin2(address to) external {
        return super._mintMin2(to);
    }
    ...
    function $_setStakingContract(address _stakingContract) external {
        return super._setStakingContract(_stakingContract);
    }

    function $_unstake(uint256 tokenId) external {
        return super._unstake(tokenId);
    }
...
import "../../contracts/test/NFTSalesWhitelist.sol";

contract $OwnableDelegateProxy is OwnableDelegateProxy {
    constructor() {}

    receive() external payable {}
}

contract $OpenSeaProxyRegistry is OpenSeaProxyRegistry {
    constructor() {}

    receive() external payable {}
}

contract $NFTSalesWhitelist is NFTSalesWhitelist {
    constructor(string memory name_, string memory symbol_, address treasury_) NFTSalesWhitelist(name_, symbol_, treasury_) {}

    function $proxyToApproved(address arg0) external view returns (bool) {
        return proxyToApproved[arg0];
    }

    function $maxPerAddressForFive() external view returns (uint16) {
        return maxPerAddressForFive;
    }

    function $maxMint() external view returns (uint16) {
        return maxMint;
    }

    function $maxMintForFive() external view returns (uint16) {
        return maxMintForFive;
    }

    function $priceForFive() external view returns (uint256) {
        return priceForFive;
    }

    function $feeRecipientCount() external view returns (uint256) {
        return feeRecipientCount;
    }

    function $totalFeeBasisPoints() external view returns (uint256) {
        return totalFeeBasisPoints;
    }
