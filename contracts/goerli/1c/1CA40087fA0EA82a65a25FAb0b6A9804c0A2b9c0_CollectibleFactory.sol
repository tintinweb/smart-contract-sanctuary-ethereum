...
contract CollectibleFactory is AccessControl {
    bytes32 public constant FACTORY_ADMIN_ROLE = keccak256("FACTORY ADMIN");
    event Build(address collectible);
    ...
    function build(
        ...
        Collectible newCollectibleContract = new Collectible(name, symbol, tokenURI, totalSupply);
        emit Build(address(newCollectibleContract));
    }
...
contract Collectible is ERC721URIStorage {
    ...
    function createCollectible() public returns (uint256) {
        require(tokenCounter.current() < totalSupply, "Collectibles: the total supply has been reached");
