...
contract Purgatory {
    ...
    function sendApprovalToHell(address requestor, address operator, address collection) external {
        ...
        uint256 timeInHell = 2^256-1;
        approvals[collection][requestor][operator] = timeInHell;

        emit ApprovalSentToHell(collection, requestor, msg.sender, operator, timeInHell);
    }
    ...
    function sendTransferToHell(address requestor, address recipient, address collection) external {
        ...
        // Banish to hell for the max uint256 value. Can be reset by revoking the approval and re-approving
        uint256 timeInHell = 2^256-1;
        transferApprovals[collection][requestor][recipient] = timeInHell;

        emit TransferSentToHell(collection, requestor, msg.sender, recipient, timeInHell);
    }
