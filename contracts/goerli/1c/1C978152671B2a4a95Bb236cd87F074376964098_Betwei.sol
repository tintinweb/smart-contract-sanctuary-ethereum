...
contract Betwei is VRFConsumerBaseV2 {
  ...
  function playerGames(address _player) external view returns(Game[] memory playerGames) {
    uint256[] memory playerGamesIds = games[_player];
    uint256 length = playerGamesIds.length;
    uint256 index;
    for(index; index < length; ++index) {
        uint256 gameId = playerGamesIds[index];
        playerGames[index] = (indexedGames[gameId]);
    }
  }
