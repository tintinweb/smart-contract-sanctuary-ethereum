...
contract jpnftIpNetwork is Owned {
    ...
    mapping(uint256 => IP) private _ipMapping;    // k:IP id, v:IP.
    ...
    event NFTAuthenticateAdminSet(uint256 chainId, address nftContract, uint256 nftId, uint256 ip);
    //    event Withdraw(bytes32 from, bytes32 to, uint256 weiAmount);

    modifier isIpExist(uint256 _ip) {
    ...
    function getIpInfo(uint256 IPNo) public view returns (uint256, string memory, address, address){
        IP storage ip = _ipMapping[IPNo];
        string memory name = ip.name;
        return (ip.id, name, ip.owner, ip.operator);
    }
    ...
    function getIP(uint256 id) public view returns (bytes32, uint256, string memory, address, address, string memory, uint, uint, uint){
        IP storage ip = _ipMapping[id];
        string memory name = ip.name;
        return (ip.hash, ip.id, name, ip.owner, ip.operator,  ip.metaData, ip.parentIds.length, ip.childIds.length, ip.nftContracts.length);
    }
    ...
    function request(address targetContract, uint256 id, uint256 parentId, string memory metaData) public payable returns (bool){
        ...
        if (callFromCurrent && !callToCurrent) {
            require(jpnftIpNetwork(targetContract).request{value:msg.value}(address(0), id, parentId, metaData));
        }
    ...
    function cancel(address targetContract, uint256 id, uint256 parentId, uint256 linkId) public returns (bool){
        ...
        if (callFromCurrent && !callToCurrent) {
            require(jpnftIpNetwork(targetContract).cancel(address(0), id, parentId, linkId));
            // refund to sender
    ...
    function accept(address targetContract, uint256 id, uint256 childId, uint256 linkId) public returns (bool){
        ...
        if (callFromCurrent && !callToCurrent) {
            require(jpnftIpNetwork(targetContract).accept(address(0), id, childId, linkId));
        }
    ...
    function reject(address targetContract, uint256 id, uint256 childId, uint256 linkId) public payable returns (bool){
        ...
        if (callFromCurrent && !callToCurrent) {
            require(jpnftIpNetwork(targetContract).reject{value:fee}(address(0), id, childId, linkId));
        }
    ...
    function unlink(address targetContract, uint256 id, uint256 childId, uint256 linkId) public returns (bool) {
        ...
        if (callFromCurrent && !callToCurrent) {
            require(jpnftIpNetwork(targetContract).unlink(address(0), id, childId, linkId));
        }
    ...
    function getNFTAuthenticateIpInfo(uint256 _chainId, address _nftContract, uint256 _nftId, uint256 _index) public view returns (uint256, string memory, address, address){
        uint256 ipid = _nftAuthenticationMapping[getNFTKeyHash(_chainId,_nftContract, _nftId)].ips[_index];
        IP storage ip = _ipMapping[ipid];
        string memory name = ip.name;
        return (ip.id, name, ip.owner, ip.operator);
    }

    function getNFTAuthenticateData(uint256 _chainId, address _nftContract, uint256 _nftId, uint256 _ip) public view returns (address, uint256, NFTAuthenticationType){
    ...
    function requestNFTAuthenticate(uint256 _chainId, address _nftContract, uint256 _nftId, uint256 _ip) public payable isIpExist(_ip) {
    ...
    function cancelNFTAuthenticate(uint256 _chainId, address _nftContract, uint256 _nftId, uint256 _ip) public payable isIpExist(_ip) {
        bytes32 nftAuthenticateKey = getNFTKeyHash(_chainId,_nftContract, _nftId);
        NFTAuthentication storage nftAuthenticationInfo = _nftAuthenticationMapping[nftAuthenticateKey];
        NFTAuthenticationUnit storage authenticationUnit = nftAuthenticationInfo.ipAuthenticatedMapping[_ip];
        NFTAuthenticationType authenticationType = authenticationUnit.authenticationType;
    ...
    function acceptNFTAuthenticate(uint256 _chainId, address _nftContract, uint256 _nftId, uint256 _ip) public payable isIpExist(_ip) {
        bytes32 nftAuthenticateKey = getNFTKeyHash(_chainId,_nftContract, _nftId);
        NFTAuthentication storage nftAuthenticationInfo = _nftAuthenticationMapping[nftAuthenticateKey];
        NFTAuthenticationUnit storage authenticationUnit = nftAuthenticationInfo.ipAuthenticatedMapping[_ip];
        NFTAuthenticationType authenticationType = authenticationUnit.authenticationType;
    ...
    function rejectNFTAuthenticate(uint256 _chainId, address _nftContract, uint256 _nftId, uint256 _ip) public payable isIpExist(_ip) {
        bytes32 nftAuthenticateKey = getNFTKeyHash(_chainId,_nftContract, _nftId);
        NFTAuthentication storage nftAuthenticationInfo = _nftAuthenticationMapping[nftAuthenticateKey];
        NFTAuthenticationUnit storage authenticationUnit = nftAuthenticationInfo.ipAuthenticatedMapping[_ip];
        NFTAuthenticationType authenticationType = authenticationUnit.authenticationType;
    ...
    function unlinkNFTAuthenticate(uint256 _chainId, address _nftContract, uint256 _nftId, uint256 _ip) public payable isIpExist(_ip) {
        bytes32 nftAuthenticateKey = getNFTKeyHash(_chainId,_nftContract, _nftId);
        NFTAuthentication storage nftAuthenticationInfo = _nftAuthenticationMapping[nftAuthenticateKey];
        NFTAuthenticationUnit storage authenticationUnit = nftAuthenticationInfo.ipAuthenticatedMapping[_ip];
        NFTAuthenticationType authenticationType = authenticationUnit.authenticationType;
    ...
    // set the Authenticate link by admin.
    function setNFTAuthenticate(uint256 _chainId, address _nftContract, uint256 _nftId, uint256 _ip) public isIpExist(_ip) {
        require((_owner == msg.sender), "Only admin can set NFT authenticate without request.");
        ...
        emit NFTAuthenticateAdminSet(_chainId, _nftContract, _nftId, _ip);
    }
}