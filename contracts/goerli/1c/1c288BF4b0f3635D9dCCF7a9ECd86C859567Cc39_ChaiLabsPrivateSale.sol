...
contract ChaiLabsPrivateSale is Ownable {
    ...
    function contribute(
        ...
        uint256 roundId,
        uint256 amount,
        WhitelistSignature memory signature
        ...
        _canContribute(saleId, roundId, amount, signature);
    ...
    function _canContribute(
        ...
        if (saleRound.whitelisted) {
            _validateWhitelist(saleId, roundId, msg.sender, signature);
        }
    }
    ...
    function _validateWhitelist(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                DOMAIN_SEPARATOR,
                keccak256(
                    abi.encode(SALE_WHITELIST_ROLE, saleId, saleRoundId, sales[saleId].admin, user, signature.deadline)
                )
            )
        );
        require(signature.deadline >= block.timestamp, "ChaiLabs: whitelist signature expired");
        address signer = ecrecover(digest, signature.v, signature.r, signature.s);
        require(sales[saleId].admin == signer, "ChaiLabs: invalid signature");
