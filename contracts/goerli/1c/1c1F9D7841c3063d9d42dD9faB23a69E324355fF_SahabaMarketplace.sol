    ...
    ReentrancyGuard
{
    ...
    function _buyToken(uint256 tokenId, address _payToken) private {
        ...
        uint256 feeAmount = (marketItem.price * _service_fees) / 1 ether;

        // send token's worth of ethers to the owner
        if (_payToken == address(0)) {
            (bool ownerTransferSuccess, ) = payable(marketItem.currentOwner).call{
                value: (marketItem.price - feeAmount) / 1 ether
            }("");
            ...
            IERC20(_payToken).safeTransferFrom(
                ...
                (marketItem.price - feeAmount) / 1 ether
            );
