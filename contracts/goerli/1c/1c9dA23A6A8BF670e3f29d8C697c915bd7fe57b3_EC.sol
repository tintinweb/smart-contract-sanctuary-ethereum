    ...
    IERC1363Receiver
{
        ...
        external payable nonReentrant whenNotPaused
    {
        if (!signers[msg.sender]) {
            _verifySigner(id, EC.extractSigner(abi.encodePacked(id, beneficiary, tier, currency, price, bonus), signature));
        }
...
library EC
{
    ...
    function toArray(uint256 input)
    ...
    function toArray(uint256[3] memory input)
        external pure returns(uint256[] memory output)
    {
    ...
    function isEmpty(uint256[3] memory input)
    ...
    VRFV2WrapperConsumerBase
{
    ...
    {
        if (!signers[msg.sender]) {
            _verifySigner(id, EC.extractSigner(abi.encodePacked(id, ticket, accept), signature));
        }
    ...
    ExenoCollectionHub
{
    ...
    function simulateMilestoneBuyback(
        uint256[3] memory random,
        uint256[3] memory reward
    )
        external onlyOwner
    {
        _simulate(Goal.MILESTONE, abi.encode(reward), EC.toArray(random));
    }

    function simulateFinalBuyback(
        uint256 random,
        uint256 reward
    )
        external onlyOwner
    {
        _simulate(Goal.FINAL, abi.encode(reward), EC.toArray(random));
    }

    function simulateReset(uint256 id)
        external onlyOwner
    {
        lastBuyback = id;
    }
