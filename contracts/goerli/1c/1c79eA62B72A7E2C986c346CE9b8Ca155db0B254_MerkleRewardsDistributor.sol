...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    event EthSwapped(uint256 receivedTokens, uint256 epoch);
    event TokensSwapped(uint256 receivedTokens, uint256 epoch, address token);
    ...
    function setRoot(bytes32 _merkleRoot, bytes memory _newMerkleCDI)
        public
        onlyOwner
    {
    ...
    function _swap(
        ...
        emit TokensSwapped(amounts[1], _epochsCount, _path[0]);
    }
    ...
    function swapMultiple(
        ...
        if (msg.value > 0) {
            ...
            emit EthSwapped(amounts[1], _epochsCount);
        }
