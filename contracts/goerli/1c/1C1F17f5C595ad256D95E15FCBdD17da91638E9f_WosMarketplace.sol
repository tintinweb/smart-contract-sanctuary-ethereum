...
abstract contract MarketplaceBase {
    ...
    function _deleteArtifact(address contractAddress) internal {
        artifacts[contractAddress] = Artifact(0, address(0), 0, 0);
    }
...
contract WosMarketplace is ReentrancyGuard, MarketplaceBase, Tax {
    ...
    function BuyArtifact(
        ...
        uint256 price = artifacts[contractAddress].price;
    ...
    function ModifyPriceArtifact(
        ...
        Signature.verifyMessage(
            abi.encodePacked(price, contractAddress).hash(),
            nonce,
            timestamp,
            signatures
        );

        _modifyPriceArtifact(price, contractAddress);
    }
}
