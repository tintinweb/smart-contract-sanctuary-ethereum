...
contract Gelato1BalanceDev is IGelato1Balance, Proxied {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    modifier onlyManager() {
        require(_managers.contains(msg.sender), "Gelato1Balance.onlyManager");
        _;
    }
    ...
    function requestWithdrawal(address _token, uint256 _withdrawalAmount)
        external
        override
    {
        require(
        ...
        uint256 totalDeposited = totalDepositedAmount[msg.sender][_token];
        uint256 totalWithdrawn = totalWithdrawnAmount[msg.sender][_token];
        ...
        emit LogRequestWithdrawal(root, msg.sender, _token, _withdrawalAmount);
    }
    ...
    function cancelWithdrawalRequest(
        ...
        uint256 newTotalWithdrawn = totalWithdrawnAmount[msg.sender][_token] +
            _cancelledAmount;
        uint256 oldTotalDeposited = totalDepositedAmount[msg.sender][_token];
        ...
        totalWithdrawnAmount[msg.sender][_token] = newTotalWithdrawn;

        totalDepositedAmount[msg.sender][_token] =
            oldTotalDeposited +
            _cancelledAmount;
    ...
    function withdraw(
        ...
        require(
            newTotalWithdrawn <= totalDepositedAmount[msg.sender][_token],
        ...
        _transferTo(_token, msg.sender, _amount);

        emit LogSponsorWithdrawal(rootCache, msg.sender, _token, _amount);
    }
    ...
    function settle(bytes32 _root, bytes32 _newRoot)
        external
        override
        onlyManager
    {
        ...
        emit LogSettlement(_newRoot, msg.sender);
    }
    ...
    function _collectFees(Fee[] calldata _fees) private {
        ...
        for (uint256 i; i < _fees.length; i++) {
            ...
            _transferTo(_fees[i].token, feeCollector_, _fees[i].amount);
