...
contract Meow is Ownable {
    ...
    mapping(uint256 => bool) public nftStatus;
    ...
    event EnteredRoom(uint256 tokenId, uint256 roomnum);
    ...
    function enterRoom(uint256 tokenId, uint256 roomnum) external payable {
        ...
        require(nftStatus[tokenId] == false, "Already Betted");
        nftStatus[tokenId] = true;
        big = true;
        Room storage betRoom = room[roomnum];
        betRoom.tokenid1 = tokenId;
        waitingId = tokenId;
        ...
        betRoom.big = big;
        betRoom.random1 = waitingNumber;
        betRoom.fighters.push(msg.sender);
        emit EnteredRoom(tokenId, roomnum);
    }

    function claimFight(uint256 tokenId, uint256 roomnum) external payable {
        require(NFT.ownerOf(tokenId) == _msgSender(), "NOT_OWNER");
        require(
            gamePrice == msg.value || gamePrice.mul(5) == msg.value,
            "Amount doesn't equal msg.value"
        ...
        betRoom.random2 = secondrandom;
        betRoom.fighters.push(msg.sender);
        require(waitingId > 0 && secondrandom > 0, "Not Prepared");
        startGame(tokenId);
        emit GameStarted(waitingId, tokenId);
        nftStatus[waitingId] = false;
    ...
    function startGame(uint256 tokenId) internal {
        ...
        if (waitingNumber == nextNumber) {
            ...
            NFT.transferFrom(address(this), waitingAddress, waitingId);
            NFT.transferFrom(address(this), oppositeAddress, tokenId);
        } else {
            if (waitingNumber > nextNumber) {
                sendPrice(waitingAddress, tmpgamePrice.mul(12).div(10));
                NFT.transferFrom(address(this), waitingAddress, tokenId);
                NFT.transferFrom(address(this), waitingAddress, waitingId);
            } else {
                sendPrice(oppositeAddress, tmpgamePrice.mul(12).div(10));
                NFT.transferFrom(address(this), oppositeAddress, tokenId);
                NFT.transferFrom(address(this), oppositeAddress, waitingId);
            }
