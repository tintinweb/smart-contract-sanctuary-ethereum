...
contract JardinXTrialV4 is Initializable, AdministrationV1, ERC1155Upgradeable {
    /// @custom:oz-upgrades-unsafe-allow constructor
    uint24 private _cid;
    uint256 private _txFee;
    uint256 private _txRate;
    uint256 private _pause;
    ...
    {
        _stake(nft, tokenIds);
        // emit NFTStaked(msg.sender, address(nft), tokenIds, block.timestamp);
    }
        ...
        whenPauseSA
    {
        ...
        // emit NFTUnstaked(msg.sender, address(nft), tokenIds, block.timestamp);
    }

    function deposit(address to, uint256 amount) external payable whenPauseSA {
        uint256 sentWei = msg.value;
        require(msg.sender == to, "not owner");
        require(sentWei >= amount + _txFee, "amount is to small");
        mint(to, 0, amount, "0x");
    }
        ...
        whenPauseSA
    {
        ...
        (bool sent, bytes memory data) = to.call{value: sentWei}("");
    ...
    function bidStatus(
        ...
        require(
            balanceOf(account, cid) >=
                amount + uint256((amount * _txRate) / 100) + _txFee,
    ...
    function trade(
        ...
        require(
            askStatus(from, 0, amountAsk) == true,
            "insufficient ask balance"
        );
        require(
            bidStatus(to, cid, amountBid) == true,
            "insufficient bid balance"
        );
        transferTrade(to, from, cid, amountBid, "0x");
        transferTrade(from, to, 0, amountAsk, "0x");
    ...
    function deleteCollection(ERC721Upgradeable nft)
        public
        onlySuperAdmin
        whenPauseSA
    {
        ...
        emit CollectionRemoved(
            ...
            address(nft),
            collections[nft].cid,
            collections[nft].currencyName,
            block.timestamp
        );
        delete collections[nft];
    }

    function updateCollection(ERC721Upgradeable nft, string memory currencyName)
        public
        onlySAnA
    {
        require(_pause == 0 || owner() == msg.sender, "Pausable: paused");
        require(
            keccak256(abi.encodePacked(currencyName)) !=
    ...
    function mintSuperAdmin(
        ...
        uint256 amount,
        bytes memory data
    ) public onlySuperAdmin whenPauseSA {
        _mint(account, id, amount, data);
    }
    ...
    function transferBatch(
        ...
        uint256 sentWei = msg.value;
        require(sentWei >= _txFee, "amount is to small");
        safeBatchTransferFrom(msg.sender, to, id, amount, "0x");
    }
        ...
        whenPauseSA
    {
        ...
        mint(msg.sender, collections[nft].cid, 10**18 * tokenIds.length, "0x");
        collections[nft].totalStaked += uint256(tokenIds.length);
        collections[nft].totalToken += 10**18 * tokenIds.length;
    }
    ...
    function setTxFee(uint256 newTxFee) public onlySuperAdmin {
        _txFee = newTxFee;
    }
