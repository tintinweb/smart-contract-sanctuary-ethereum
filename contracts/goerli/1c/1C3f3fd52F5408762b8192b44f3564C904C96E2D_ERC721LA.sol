 ...
 */
contract ERC721LA is PublicMintable, AirDropable, Burnable {
...
abstract contract AirDropable is IAirDropable, ERC721Mintable {
...
abstract contract ERC721LACore is
    LAInitializable, AccessControl, Pausable, IERC721LA
{
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
     *                               LIBRARIES
     * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
abstract contract ERC721Mintable is  ERC721LACore  {
    using BitMaps for BitMaps.BitMap;
    using ERC721State for ERC721State.ERC721LAState;
...
abstract contract AccessControl {
    ...
    /**
     * @notice Checks that msg.sender has MINTER_ROLE or the COLLECTION_ADMIN_ROLE
     * Reverts with a AccessControlNotAllowed.
     *
     */
    modifier onlyAdminOrMinter() {
        bool isMinter = hasRole(MINTER_ROLE, msg.sender);
        bool isAdmin = hasRole(COLLECTION_ADMIN_ROLE, msg.sender);
        if (!isMinter && !isAdmin) {
