...
contract YouTheRealMVP_V4 is ERC721x, DefaultOperatorFiltererUpgradeable {
    ...
    function getCaptainzInfo(uint256 captainzTokenId) external view returns (uint256 tokenId, uint256 lastBoostedAt, uint256 boosted) {
        require(address(captainzContract) != address(0), "captainzContract not set");
        uint256[] storage existCheck = questingCaptainzToMVP[captainzTokenId];
        if (existCheck.length == 0) return (0, 0, 0);
        tokenId = existCheck[0];
        if (captainzContract.ownerOf(captainzTokenId) != ownerOf(tokenId)) return (0, 0, 0);
        return (tokenId, captainzLastBoostedAt[captainzTokenId], 1);
    }
