...
contract Coinhound is ERC20, Ownable {
	...
	constructor() ERC20("Coinhound", "CND") {
	   ...
	   maxTokenPerWallet = 1500000 * (10 ** 18);
	   maxTokenPerTxn = 750000 * (10 ** 18);
	   swapTokensAtAmount = 1500 * (10 ** 18);
	   
	   marketingWallet = address(0x760c5A41b67BE0b8E208Da61c9654d5aad1e92f2);
	...
	function excludeFromFee(address account) public onlyOwner {
	   require(isExcludedFromFee[account] == false, "Account is already the value of 'true'");
	   isExcludedFromFee[account] = true;
	}
	
	function excludeFromMaxTokenPerWallet(address account) public onlyOwner {
	   require(isExcludedFromMaxTokenPerWallet[account] == false, "Account is already the value of 'true'");
	   isExcludedFromMaxTokenPerWallet[account] = true;
	}
	
	function setMaxTokenPerWallet(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "Amount cannot be over the total supply.");
		require(amount >= 1000 * (10 ** 18), "Minimum `1000` token per wallet required");
		
		maxTokenPerWallet = amount;
		emit MaxTokenPerWalletUpdated(amount);
	}
	
	function setMaxTokenPerTxn(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "Amount cannot be over the total supply.");
		require(amount >= 100 * (10 ** 18), "Minimum `100` token per txn required");
	...
	function setliquidityFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
	    require(marketingFee[0] + buy  <= 400 , "Max fee limit reached for 'BUY'");
		require(marketingFee[1] + sell <= 400 , "Max fee limit reached for 'SELL'");
		require(marketingFee[2] + p2p  <= 400 , "Max fee limit reached for 'P2P'");
	...
	function setMarketingFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
	    require(liquidityFee[0] + buy  <= 400 , "Max fee limit reached for 'BUY'");
		require(liquidityFee[1] + sell <= 400 , "Max fee limit reached for 'SELL'");
		require(liquidityFee[2] + p2p  <= 400 , "Max fee limit reached for 'P2P'");
	...
	function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20){      
		...
		if (canSwap && !swapping && isAutomatedMarketMakerPairs[recipient] && swapAndLiquifyEnabled) 
		{
			...
			uint256 tokenToSwap = tokenToLiqudity + tokenToMarketing;
			...
			if(tokenToSwap >= swapTokensAtAmount) 
			{
				...
				uint256 liqudityPart = newBalance * tokenToLiqudity / tokenToSwap;
				uint256 marketingPart = newBalance - liqudityPart;
				
				if(liqudityPart > 0)
				{
				    uint256 liqudityToken = swapTokensAtAmount * tokenToLiqudity / tokenToSwap;
				...
				if(marketingPart > 0) 
				{
					...
					marketingFeeTotal = marketingFeeTotal - (swapTokensAtAmount * tokenToMarketing / tokenToSwap);
				}
				swapping = false;
			}
        }
	...
	function collectFee(uint256 amount, bool sell, bool p2p) private returns (uint256) {
        ...
        return (newMarketingFee + newLiquidityFee);
    }
	...
	function migrateTokens(address token, address receiver, uint256 amount) external onlyOwner{
	   ...
	   {
	       require(IERC20(address(this)).balanceOf(address(this)) - liquidityFeeTotal - marketingFeeTotal >= amount, "Insufficient balance on contract");
	   }
	   else
	   {
