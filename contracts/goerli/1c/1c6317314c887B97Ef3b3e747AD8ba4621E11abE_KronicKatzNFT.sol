...
abstract contract ERC721EnumerableB is
    ERC721B,
    IERC721Batch,
    IERC721Enumerable
{
    ...
    function balanceOf(
        address owner
    ) public view virtual override(ERC721B, IERC721) returns (uint) {
        require(
            owner != address(0),
            "ERC721: balance query for the zero address"
        );
    ...
    function tokenOfOwnerByIndex(
        address owner,
        uint index
    ) external view virtual override returns (uint tokenId) {
        require(
...
contract KronicKatzNFT is Delegated, ERC721EnumerableB, PaymentSplitter {
    ...
    string private _tokenURIPrefix =
        "https://kronickatznfts.mypinata.cloud/ipfs/bafybeiamkhfq2tyzcjgz2wemhsnlst2mgjjethssuxxpwbtvfxoqqzel2a/";
    ...
    uint[] private shareList = [88, 10, 1, 1];
    ...
    function mint(uint quantity) external payable {
        ...
        if (isMainsaleActive) {
            ...
            require(
                msg.value >= MAINSALE_PRICE * quantity,
        ...
        if (
            FRELINE > 0 && balance < FRELINE && balanceOf(msg.sender) >= FRELINE
        ) _mint(msg.sender, _next());
    }

    //delegated
    function mintTo(
        uint[] calldata quantity,
    ...
    function setPrice(
        ...
        require(
            MAINSALE_PRICE != mainsalePrice || PRESALE_PRICE != presalePrice,
            "New value matches old"
        );
