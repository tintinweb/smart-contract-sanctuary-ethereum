...
contract RareStakes is Ownable {
    ...
    function stake(uint256 stakeAmount) public {
        ...
        // first staking asign same value to min and max
        if (minStakeAmount == 0 && maxStakeAmount == 0) {
            minStakeAmount = usdVal;
            maxStakeAmount = usdVal;
        } else if (usdVal < minStakeAmount) {
            minStakeAmount = usdVal;
        } else if (usdVal > maxStakeAmount) {
            maxStakeAmount = usdVal;
        }
    ...
    function releaseToCharity(address charity, uint256 amount)
        external
        onlyOwner
    {
        if (amount == 0) {
        ...
        mainToken.transfer(charity, amount);
    }
