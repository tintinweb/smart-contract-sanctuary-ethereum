    ...
    UniversalPaymentReceiver
{
     ...
     *   and if there is a next auction, then start the next auction without an overriding AuctionPack.
     */
    function settleAuction() external override nonReentrant whenNotPaused {
        _settleAuction(false);

        if (_hasNextAuction(true)) {
            _createAuction();
        }
    }
    ...
    function minimumBid() public view returns (uint256 minimum) {
        if (auction.amount > 0) {
            return
                auction.amount +
                ((auction.amount * minBidIncrementPercentage) / 100);
        } else {
            return Math.max(reservePrice, auction.auctionPack.reservePrice);
        }
    }
    ...
    function createBid(
        ...
        _acceptReserveCurrency(token, amountInReserveCurrency, amountInMaximum);
    ...
    function _hasNextAuction(bool canRestartCycle) private view returns (bool) {
...
contract UniversalPaymentReceiver is Context {
    ...
    function _acceptReserveCurrency(
        ...
        if (address(token) != address(reserveCurrency)) {
            ...
            uint256 amountIn = IV3SwapRouter(swapRouter).exactOutputSingle(
                params
            );
            ...
            if (amountIn < amountInMaximum) {
                uint256 leftover = amountInMaximum - amountIn;

                if (address(token) == address(WETH9) && msg.value > 0) {
                    IPeripheryPaymentsWithFeeExtended(swapRouter).unwrapWETH9(
                        0
                    );

                    WETH9.withdraw(leftover);
                    ...
                    token.transfer(_msgSender(), leftover);
                }
            }
        } else {
            token.transferFrom(
