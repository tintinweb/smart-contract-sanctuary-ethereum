...
abstract contract MarketPlace {
    function extDeleteOffer(uint256 horseId) external virtual;

    function extDeleteMarketItem(uint256 horseId) external virtual;
}

contract HorsePartnerships is Pausable, SilksERC1155Burnable {
    using Address for address;

    // As of right now, the horse owner is not stored as a partner.
    struct Partnership {
        address[] partners;
        uint256 maxPartnershipShares;
    ...
    uint256 public maxPartnershipShares;

    address private _royaltyReceiver;
    uint96 private _royaltyRate; // 8 = 8%

    mapping(uint256 => Partnership) internal _partnerships;
    ...
    modifier whenReconstitutionNotPaused() {
        require(!fractionalizationPaused, "RECONSTITUTION-PAUSED");
        _;
    }
    ...
    function _mintPartnershipTokenType(
        ...
        ERC721 horseContract = ERC721(_indexContract.getAddress("Horse"));

        require(horseContract.ownerOf(horseId) == account, "NOT-TOKEN-OWNER");
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (to != address(0)) {
                ...
                if (from != address(0)) {
                    ...
                    for (uint256 j = 0; j < partnerAddresses.length; j++) {
                    ...
                    ERC721 horseContract = ERC721(
                        _indexContract.getAddress("Horse")
                    );
                    address owner = horseContract.ownerOf(id);
                    ...
                    if (!addressExists) {
                        if (to != owner) {
                            // Extend the array.
                            uint256 partnerAddressesLength = partnerAddresses
                                .length;
                            address[]
            ...
            uint256 totalSupply;
            for (uint256 j = 0; j < partnerAddresses.length; j++) {
                address partnerAddress = partnerAddresses[j];
                if (partnerAddress != address(0)) {
                    totalSupply += balanceOf(partnerAddress, id);
                }
            }

            // If the number of shares is now more than the maxPartnershipShares roll back the transaction
            require(
                totalSupply <= _partnerships[id].maxPartnershipShares,
                "MAX-NUM-SHARES-EXCEEDED"
            );
    ...
    function isFractionalized(uint256 horseId) external view returns (bool) {
        return _partnerships[horseId].isFractionalized;
    }

    function fractionalize(uint256 horseId)
        external
        whenNotPaused
        whenFractionalizationNotPaused
    {
    ...
    function adminFractionalize(address account, uint256 horseId)
        external
        onlyOwner
    {
        _mintPartnershipTokenType(
    ...
    function reconstitute(uint256 horseId)
        external
        whenNotPaused
        whenReconstitutionNotPaused
    {
        _reconstituteHorsePartnership(msg.sender, msg.sender, horseId);
    }

    function adminReconstitute(address account, uint256 horseId)
        external
        onlyOwner
        whenNotPaused
        whenReconstitutionNotPaused
    {
        _reconstituteHorsePartnership(msg.sender, account, horseId);
    }

    function getPartnership(uint256 horseId)
        external
        view
        returns (address[] memory, uint256[] memory)
    {
        address[] storage partners = _partnerships[horseId].partners;
        ...
        for (uint256 i = 0; i < partners.length; i++) {
            ...
            if (partner != address(0)) {
                partnerAddresses[i] = partner;
                partnerShares[i] = balanceOf(partner, horseId);
            }
        }
    ...
    function setFractionalizationPaused(bool _fractionalizationPaused)
        external
        onlyOwner
    {
    ...
    function setReconstitutionPaused(bool _reconstitutionPaused)
        external
        onlyOwner
    {
    ...
    function setMaxPartnershipShares(uint256 num) external onlyOwner {
        maxPartnershipShares = num;
    }

    function setContractGlossary3(address ContractGlossary3Address)
        external
        onlyOwner
    {
        require(
    ...
    function royaltyInfo(uint256 horseId, uint256 salePrice)
        external
        view
    ...
    function setRoyaltyInfo(address royaltyReceiver, uint96 royaltyRate)
        external
        onlyOwner
    {
        _royaltyReceiver = royaltyReceiver;
...
abstract contract SilksERC1155Burnable is ERC1155, Ownable {
    modifier isWhitelisted(address _addr) {
        require(whitelist[_addr], "NOT-WHITELIST-TO-BURN");
        _;
    }
    ...
    function burn(
        ...
        uint256 id,
        uint256 value
    ) public virtual isWhitelisted(msg.sender) {
        require(
