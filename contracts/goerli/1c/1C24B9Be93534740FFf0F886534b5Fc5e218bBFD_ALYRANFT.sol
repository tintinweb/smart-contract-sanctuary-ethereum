...
contract ALYRANFT is ERC721A, ERC721AQueryable, Ownable, PaymentSplitter {
    ...
    /**
     *@notice Possibility to c that runs the code only if the SC is not in pause
     */
    modifier whenNotPaused() {
        require(!isPaused, "The contract is in pause");
        _;
    }

    /**
     *@notice Mint function for the whitliste sale
     *@param _account Account which will receive the NFT (not necessarely the future NFT owner)
    ...
    function whitelistMint(
        ...
        require(
            ...
            "Maximum supply exceeed"
        );
        require(
            msg.value >= PRICE_WHITHLIST_MINT * _quantity,
            "Not enough funds"
        );
        amountNFTperWalletWhitelistSale[msg.sender] += _quantity;
    ...
    function publicMint(address _account, uint256 _quantity)
        ...
        whenNotPaused
    {
        require(getStep() == Step.PublicSale, "Public Sale is not open");
        require(
    ...
    /**
     *@notice Allows to give NFTs (owner only)
     *@param  _to address of the beneficiary
     *@param _quantity Amount of NFTs the owner wants to give to the beneficiary
     */
    function gift(address _to, uint256 _quantity)
        external
        onlyOwner
        whenNotPaused
    {
    ...
    /**
     *@notice change the base URI of the NFT to reveal them after a certain time
     *
    ...
    /**
     *@notice change the merkleRoot to switch the whitelist addresses
     *
    ...
    /**
     *@notice Allow to get the step of the sale*
     */
    ...
    /**
     *@notice Verify if an address is whitelisted or not (not yet merkle root creation)
     *
     *@param _account The account is checked
     *@param _proof corresponds to the merkle proof
     *@return bool returns true if the address is whitlisted and false otherwise
     */
    ...
    /**
     *@notice how to get the token URI of a NFT by its ID
     *@param _tokenId the NFT's Id you want to have the URI of the metatdatas
     *@return URI of a NFT by its Id
     */
    ...
    // not allowing reveiving ehters without a NFT mint

    receive() external payable override {
        revert("Only if you mint a NFT");
    }
}
