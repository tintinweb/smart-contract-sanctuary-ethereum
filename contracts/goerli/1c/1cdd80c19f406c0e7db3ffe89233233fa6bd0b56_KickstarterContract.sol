...
// File: contracts/icoContract.sol
...
abstract contract Crowdsale is ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        _token = tokenAddress;
        _owner = msg.sender;
        _remainingReferalTokens = _token.totalSupply().mul(10).div(100);
        _salesTokenDistribution = _token.totalSupply().mul(70).div(100);
        referalCommision = 2000000000000000000;
    }
    ...
    function tokenRaised() public returns (uint256) {
        _totalRemainingTokens = _token.balanceOf(_owner);
        return
            _token.totalSupply().sub(
                _totalRemainingTokens,
     ...
     */
    function preIcoAllocation() public returns (uint256) {
        //20 percent from the sales token distribution
        _totalPreIcoTokens = _salesTokenDistribution.mul(30).div(100);
     ...
     */
    function postIcoAllocation() public returns (uint256) {
        //70 percent from the sales token distribution
        _totalPostIcoTokens = _salesTokenDistribution.mul(70).div(100);
     ...
     * @return the Ico tokens distribution.
     */
    function referalAllocation() public returns (uint256) {
        //10 percent from the total supply
        _totalreferalTokens = _token.totalSupply().mul(10).div(100);
        return _totalreferalTokens;
    }

    function tokensLimit(uint256 tokenAmount) private {
        if (stage == CrowdsaleStage.PreICO) {
            require(
     ...
     */
    function buyTokens(address beneficiary, address referal)
        public
        payable
        nonReentrant
    {
        tokensLimit(tokenRaised().add(msg.value));
        ...
        if (referal != address(0) && msg.value > 1000000000000000000) {
            ...
            _remainingReferalTokens = _remainingReferalTokens.sub(
                referalCommision
            );
    ...
    {
        if (stage == CrowdsaleStage.PreICO) {
            uint256 preIcoAward = weiAmount.mul(bonus()).div(
                100000000000000000000
            );
            uint256 totalTokens = weiAmount.mul(_originalRate);
            return preIcoAward.add(totalTokens);
        }

        return weiAmount.mul(_originalRate);
    }
...
abstract contract TimedCrowdsale is Crowdsale {
    ...
    function setMainICO() public onlyOwner {
        if (_openingTime >= _preIcoClosingTime) {
            emit PreSaleEnd();
...
contract KickstarterContract is CappedCrowdsale, RefundableCrowdsale {
    ...
    uint256 public investorMinCap = 5; // 0.000000000000000001 ether
    ...
    uint256 private _preIcoClosingTime = _preIcoOpeningTime + 86400 * 15; //15 days

    uint256 private _openingTime = _preIcoClosingTime;
    uint256 private _closingTime = _openingTime + 86400 * 30; //30 days

    // Hard Code values for quick Testing

    uint256 private rateToken = 1; //1 wei = 1 Token
    uint256 capToken = 60000000000000000000; //60 ether -> hardcap
    uint256 goalToken = 20000000000000000000; //20 ether -> softcap
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount)
        internal
        view
        override(CappedCrowdsale, TimedCrowdsale)
    {
        super._preValidatePurchase(beneficiary, weiAmount);
    }
}