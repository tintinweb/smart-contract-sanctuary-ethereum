...
contract AQMDeviceLicense {
    ...
    function addEarnings(address[] memory wallets, uint256[] memory amounts) external onlyOwner{
        require(wallets.length == amounts.length, "Arrays length do not match");

        for( uint i = 0; i < wallets.length; i++ ) {
            earnings[wallets[i]] += amounts[i];            
        }
    }
