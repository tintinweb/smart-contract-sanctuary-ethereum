...
contract AppContractFactory {
    ...
    function createAppContract(
        ...
        NFT.safeMint(msg.sender, appDetails);
    ...
    function getdeployedAppContractAddress(
        ...
        return address(deployedAppContract[_address][index]);
    }
}
...
contract AppContract {
    uint256 private appTokenPrice;
    uint256 private appEthPrice;
    uint256 private tokenBalance = 0;
    uint256 private scoreSum;
    uint256 private scoreCount;
    uint256 private appDownloadCount;
    ...
    mapping(address => bool) private allowDownloadAddress;
    string private appInfo;
    event successfulWithdraw(bool TransferToManager, bool TransferToOwner);
    event allowToDownload(address user, bool mintedNFT);

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(
        uint256 _appEthPrice,
        uint256 _appTokenPrice,
        address payable appOwner,
        address _ContractOptionsAddress,
        address payable _manager,
        address _DappetizeTokenAddress,
        address _DownloadNFTAddress
    ) {
        appEthPrice = _appEthPrice;
        appTokenPrice = _appTokenPrice;
    ...
    ) public OwnerRestricted returns (uint256, uint256) {
        appEthPrice = newEthPrice;
        appTokenPrice = newTokenPrice;
        return (appEthPrice, appTokenPrice);
    }

    function getAppPrice() public view returns (uint256, uint256) {
        return (appEthPrice, appTokenPrice);
    }

    function setScore(uint256 score) public returns (uint256, uint256) {
        require(
            allowDownloadAddress[msg.sender] == true &&
                1 <= score &&
                score <= 5,
    ...
    function withdraw() public payable OwnerRestricted {
        ...
        if (success) {
            ...
            manager.transfer((address(this).balance * siteFee) / 100);
    ...
    function appDownload(
        ...
        if (state == true) {
            (bool success, bytes memory data) = DappetizeTokenAddress.call(
                abi.encodeWithSignature(
                    "transferToken(address,address,uint256)",
                    msg.sender,
                    address(this),
            ...
            state = success;
            if (state) {
                tokenBalance += appTokenPrice;
            }
        }

        require(
            (allowDownloadAddress[msg.sender] == true) ||
                msg.value == appEthPrice ||
                state,
            "insufficient fund"
        );

        appDownloadCount = appDownloadCount + 1;
        if (allowDownloadAddress[msg.sender] == false) {
            (bool success1, bytes memory data1) = DownloadNFTAddress.call(
                abi.encodeWithSignature(
            ...
            allowDownloadAddress[msg.sender] = true;
            emit allowToDownload(msg.sender, success1);
        }
