...
contract EarthverseDeposit {
  ...
  function depositRPAndSendNFTLand(
    ...
    if (msg.value <= 0) revert EarthverseDeposit_InvalidDepositAmount();

    //Transfer NFTLand to the sender
    ...
    INFTDollar(nftd).mint(seller, msg.value, decimalsOfInput);

    // Swaps stablecoin for WETH
    amountOut = swapExactInputSingle(msg.value, tokenIn);
