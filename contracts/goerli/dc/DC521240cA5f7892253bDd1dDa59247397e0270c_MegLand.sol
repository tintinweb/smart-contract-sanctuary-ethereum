...
interface IMegNFT is IMintableNFT, IBurnableNFT {
 ...
 * @title Meg NFT
 *
 * @notice This contract contains base implementation of Meg NFT
 *
 * @dev This contract is inherited from MegNFT
 */
contract MegLand is MegNFT {
   ...
   */
  function __MegLand_init(string memory _uri) external initializer {
    __MegNFT_init("MegLand", "ML", _uri);
  }
}
...
//import "./interfaces/IMegCreator.sol";
import "./utils/OpenseaDelegate.sol";

/**
 * @title NFT
 * @notice This contract contains base implementation of NFT contracts
...
abstract contract MegNFT is ERC721Upgradeable, OwnableUpgradeable, AccessControlUpgradeable, IMegNFT {
   ...
   */
  function __MegNFT_init(string memory _name, string memory _symbol, string memory _tokenUri) internal initializer {
    __Ownable_init();
   ...
   */
  function setURI(string memory _uri) external onlyRole(EDITOR_ROLE) {
    uri = _uri;
   ...
   * @param _typeId Type id: 0 standard, 1 premium
   * @param _ids Token ids
   * Required Statements
   * - MegNFT:01 Invalid ids
   */
  function setTypeIds(uint256 _typeId, uint256[] memory _ids) external onlyRole(EDITOR_ROLE) {
    require(_ids.length > 0, "MegNFT:01");

    for (uint256 i = 0; i < _ids.length; i++) {
      typeIds[_ids[i]] = _typeId;
    }
  }
   ...
   * @param _id Token id
   * Required Statements
   * - MegNFT:01 Only owner of NFT or whoever has burner role can burn
   */
  function burn(uint256 _id) external override {
    require(ownerOf(_id) == _msgSender() || hasRole(BURNER_ROLE, _msgSender()), "MegNFT:01");
    _burn(_id);
  }
