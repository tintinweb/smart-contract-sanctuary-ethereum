...
contract AsciiPunks is Ownable, ERC721A {
    ...
    bytes constant tokenDescription = "Ascii Punks is a generative homage to the original CryptoPunks. Each Ascii Punk is stored on-chain, and traits are read from the CryptoPunksData contract.";
    address public baguetteImg;
    mapping(uint => mapping(PunkAttributeValue =>address)) public storedAttributes;
    ...
    function _internalMint(address toAddress, uint numTokens) private {
        if (msg.sender != YS && msg.sender != WG && msg.sender != owner()) {
    ...
    function ownerMint(address toAddress, uint numTokens) external {
    ...
    function baguette() private view returns (string memory) {
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '"name":"', title, '",'
                                '"description":"', desc, '",'
                                '"image_data":"data:image/png+xml;base64,', baguetteImg, '",'
    ...
    function tokenImage(uint16 punkId) public view returns (string memory) {
        ...
        DynamicBufferLib.DynamicBuffer memory svgBytes = DynamicBufferLib.DynamicBuffer('');
        svgBytes.append('<svg width="1200" height="1200" shape-rendering="crispEdges" xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 1200 1200"><rect x="0" y="0" style="width:100%;height:100%" fill="white" />');
        ...
        for (uint i = 0; i < attrCount; ++i) {
            ...
            address storedAttributeAddr = storedAttributes[uint(attrVal)][punk.sex];
            if (attrVal != none) {
                svgBytes.append('<image src="data:image/png;base64,');
                svgBytes.append(Base64.decode(string(SSTORE2.read(storedAttributeAddr))));
                svgBytes.append('"></image>');
            }
        }
    ...
    function setAttribute(string memory attrM, string memory attrF, string memory traitName) external onlyOwner {
        uint attrIndex = uint(extendedPunkDataContract.attrStringToEnumMapping(traitName));
        require(attrIndex != 0, "not found");
        setAttribute(attrM, attrF, attrIndex);
    }

    function setAttributeEqual(string memory attr, uint attrIndex) public onlyOwner {
        storedAttributes[attrIndex][PunkAttributeValue.MALE] = SSTORE2.write(bytes(attr));
        storedAttributes[attrIndex][PunkAttributeValue.FEMALE] = storedAttributes[attrIndex][PunkAttributeValue.MALE];
    }

    function setAttribute(string memory attrM, string memory attrF, uint attrIndex) public onlyOwner {
        storedAttributes[attrIndex][PunkAttributeValue.MALE] = SSTORE2.write(bytes(attrM));
        storedAttributes[attrIndex][PunkAttributeValue.FEMALE] = SSTORE2.write(bytes(attrF));
    }
    function setBaguette(string memory attr) public onlyOwner {
        baguetteImg = SSTORE2.write(bytes(attr));
    }

    //selector = 0, male, selector=1, female, selector=2, both

    function isAttributeSet(string memory traitName, uint selector) external view returns (bool) {
        uint attrIndex = uint(extendedPunkDataContract.attrStringToEnumMapping(traitName));
        bool res;
        if (selector != 1) {
            res = isAttributeSet(attrIndex, PunkAttributeValue.MALE);
        }
        if (selector != 0) {
            res = res && isAttributeSet(attrIndex, PunkAttributeValue.FEMALE);
        }

        return res;
    }
    function isAttributeSet(uint attrIndex, PunkAttributeValue sex) public view returns (bool) {
        return storedAttributes[attrIndex][sex] != address(0);
    }
}
