...
contract MoneyMarketFacetGoerli is WithStorage, UniswapData, GoerliCompoundCTokenData {
    ...
    ) external payable onlyOwner returns (uint256 errorCode) {
        CErc20Interface cToken = cToken(_underlying);
        errorCode = cToken.redeem(_cAmountToRedeem);
        TransferHelper.safeTransfer(_underlying, msg.sender, _cAmountToRedeem);
    }
    ...
    ) external onlyOwner returns (uint256 errorCOde) {
        errorCOde =  cToken(_underlying).borrow(_borrowAmount);
        TransferHelper.safeTransfer(_underlying, _recipient, _borrowAmount);
    }
    ...
    function swapAndSupplyExactIn(ExactInputToSelfParams memory uniswapParams) external onlyOwner {
        ...
        TransferHelper.safeTransferFrom(tokenIn, msg.sender, address(this), uniswapParams.amountIn);
    ...
    function borrowAndSwapExactIn(ExactInputParams memory uniswapParams) external onlyOwner returns (uint256 amountOut) {
        ...
        cToken(tokenIn).borrow(uniswapParams.amountIn);
        // approve minimal router
        TransferHelper.safeApprove(tokenIn, router, type(uint256).max);
