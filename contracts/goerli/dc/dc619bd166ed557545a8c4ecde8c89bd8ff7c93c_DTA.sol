...
contract DTA is ReentrancyGuard, Ownable, HellBallots{
    ...
    IMinter public minter = IMinter(0x5E5C99aE2FDF4F881e93f16a3583a39114faca82);
    //testing only
    ...
    uint256 triSumOfTokens = 1;

    mapping(uint256 => uint256) public unlock;
    mapping(address=> Data) mintInformation;
    ...
    constructor()HellBallots("Dynamic Token Allocation", 0x899BF84CeA7A2c6a335f6ce8050eBe6ae292409A, 10){
        ...
        mintInformation[address(this)] = Data (1 ether, 1000 ether, 0, 0, false); //Set For Matic or other blockchain gas
    ...
    function requestToChangeToken(uint256 id, address tokenA, bool removeFalseAddTrue,uint256 initailTokenA, 
                                    uint256 initailPrimary, bool farm)external ownsDao(id) returns(bool){
        ...
        if(set){
            ...
            setData(initailTokenA, initailPrimary, farm);
    ...
    function setData(uint256 initailTokenA, uint256 initailPrimary, bool farm)internal{
        currentVoteToAdd = Data(initailTokenA, initailPrimary, 0, 0, farm);
    }
    ...
    function removeToken(address token)internal{
        ...
        mintInformation[token] = Data(0,0,0,0,false);
    ...
    function addToken(address token)internal{
        ...
        currentVoteToAdd = Data(0,0,0,0,false);
        uint256 n = tokens.length;
        _tokenIndex[token] = n;
        tokens.push(token);
        n++;
    ...
    function ownerAddToken(address tokenA,uint256 initailTokenA, uint256 initailPrimary, bool farm)external onlyOwner{
    ...
    function unlockTime(uint256 id)public view returns(uint256){
        ...
        if(time > delayTimer){
            return time - ((delayTimer * power)/topPower);
        }else{
            return time;
        }
    }
    ...
    function uiHelper()external view returns(UIData[] memory){
        uint256 l = tokens.length;
        UIData[] memory d = new UIData[](l);
        for(uint i = 0; i < l; i++){
            address a = tokens[i];
            d[i] = UIData(a, mintInformation[a]);    
