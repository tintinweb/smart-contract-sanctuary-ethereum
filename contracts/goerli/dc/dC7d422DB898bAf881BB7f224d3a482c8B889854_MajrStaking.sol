...
contract MajrStaking is Ownable, AccessControl, ReentrancyGuard, Pausable {
  ...
  */
  ...
  function stake(uint256[] calldata _tokenIds) external nonReentrant whenNotPaused updateReward(msg.sender)  {
    ...
    _totalSupply = _totalSupply.add(_tokenIds.length);
    _balances[msg.sender] = _balances[msg.sender].add(_tokenIds.length);
  ...
  function notifyRewardAmount(uint256 _reward) external onlyOwner updateReward(address(0)) {
    ...
    require(rewardRate <= balance.div(rewardsDuration), "MajrStaking: Provided reward too high.");
