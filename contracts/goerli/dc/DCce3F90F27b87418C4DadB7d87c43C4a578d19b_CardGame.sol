...
contract CardGame is AccessControlUpgradeable {
    ...
    /**
     * @notice Returns true if a user has season pass
     * @param owner of the season pass
     */
    function hasPass(address owner) external view returns (bool) {
        return seasonPass.balanceOf(owner) != 0;
    }

    /**
     * @notice Returns the phase by number
     * @param phaseNumber the number of voting phase
     * @param tokenId Season Pass Token Id
     */
    function hasVoted(uint256 phaseNumber, uint256 tokenId) external view returns (bool) {
        return userVotes[phaseNumber][tokenId].voter != address(0);
    }

    /**
     * @notice Returns Season Pass Token Id
     * @param owner of the Season Pass Token Id
     */
    function getTokenId(address owner) external view returns (uint256 tokenId) {
        for (uint256 i = 0; seasonPass.ownerOf(i) != address(0); ) {
            try seasonPass.ownerOf(i) returns (address realOwner) {
                if (realOwner == owner) {
                    return i;
                }
