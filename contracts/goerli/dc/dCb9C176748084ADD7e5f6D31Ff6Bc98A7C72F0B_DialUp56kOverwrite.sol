...
contract DialUp56kOverwrite is OverwriteState, Ownable {
    ...
    function getOSWrites(uint16 _osId) public view returns (uint16[] memory, uint16[] memory) {
        require(operatingSystems[_osId].writes > 0, "OS not written");
    ...
    function overwrite(uint16 _osId, uint16 _diskId) external {
        require(disks[_diskId].loaded, "Disk not loaded");
        require(disks[_diskId].active, "Disk not active");
        require(disks[_diskId].overwrites > 0, "Disk not writeable");
        ...
        uint randomNumber = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, _osId, block.difficulty)));
        uint16 _traitId = uint16(randomNumber % disks[_diskId].overwrites);

        Overwrite memory newOverwrite;
        newOverwrite.diskId = _diskId;
        newOverwrite.traitId = _traitId;

        operatingSystems[_osId].overwrites[operatingSystems[_osId].writes] = newOverwrite;
        operatingSystems[_osId].writes = operatingSystems[_osId].writes + 1;
    }
    ...
    function setDiskReclaims(uint8 _reclaims) external onlyOwner {
        diskReclaims = _reclaims;
    }
...
abstract contract OverwriteState {
    ...
    struct OS {
        mapping(uint8 => Overwrite) overwrites;
        uint8 writes;
    }
