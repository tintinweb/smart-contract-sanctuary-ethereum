...
contract OniMainTest is ERC721, Ownable, DefaultOperatorFilterer {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant _TOTALSUPPLY = 6300;

    uint256 public tokenIDRed = 1;
    uint256 public tokenIDBlue = 3151;
    uint256 public _totalMinted = 0;
    uint public status = 0; //0-pause 1-CristalHolders 2-INANAMI/KENZOKU/Waitlist 3-Public
    uint256 public TOKEN_NUMBER = 3500;
    string private second_baseTokenURI;

    // ---------- Cristal Holders and Public SALE ----------- //
    uint256 public holdersTotalAvailable = 1000;
    uint256 public maxQuantity = 1; // maximum amount that user can mint/Transaction
    uint256 public MAX_PER_WALLET = 1; // maximum amount that user can mint/Wallet
    uint256 public price = 0 ether;
    bytes32 public holdersListMerkleRoot;

    mapping(address => uint256) public holdersClaimedBy;
    mapping(address => uint256) public publicClaimedBy;


    constructor() ERC721("OniMainTest", "OMT") {}
    ...
    function setTotalAvailableHolders(uint256 _holdersCollection)
        external
        onlyOwner
    {
        holdersTotalAvailable = _holdersCollection;
    }
   ...
   function getStatus()public view returns(uint){
    ...
    function GetRedORBlue(uint _quantity) public view returns(uint){
        // generate random
        uint Number;
        if((tokenIDRed + _quantity) -1 <= 3150 && (tokenIDBlue + _quantity) -1 <= 6300){
            Number = random(10);
        } else if((tokenIDRed + _quantity) -1 > 3150){
            Number = 6;
        } else {
            Number = 1;
        }
    ...
    function mint(uint256 _quantity) public payable isSaleOpen {
        require(status == 3 , "Sale is not active");
        require(totalSupply() + _quantity <= _TOTALSUPPLY,"Quantity must be lesser then MaxSupply");
        require(_quantity <= maxQuantity,"can not mint this many");
        require(msg.value >= _quantity * price, "Try to send more ETH");

        publicClaimedBy[msg.sender] += _quantity;
        require(publicClaimedBy[msg.sender] <= MAX_PER_WALLET,"Purchase exceeds max allowed");

        uint GNumber = GetRedORBlue(_quantity);

        if(GNumber <= 5){ // RED       
        require((tokenIDRed + _quantity) - 1 <= 3150,"Quantity must be lesser then MaxSupply for Red");
        ...
        require((tokenIDBlue + _quantity) -1 <= 6300,"Quantity must be lesser then MaxSupply for Blue");
        for (uint256 i = 0; i < _quantity; i++) {
    ...
    function mintHolders(uint256 _quantity, uint256 _colorCode) public payable isSaleOpen {
        require(status == 1 , "Sale is not active");
        require(totalSupply() + _quantity <= _TOTALSUPPLY,"Quantity must be lesser then MaxSupply");
        require(totalSupply() + _quantity <= holdersTotalAvailable,"Quantity must be lesser then holders Total Available");
        // require(inHolderslist(_proof, msg.sender), "You are not in presale");
        require(_quantity <= maxQuantity,"can not mint this many");
        require(msg.value >= _quantity * price, "Try to send more ETH");

        holdersClaimedBy[msg.sender] += _quantity;
        require(holdersClaimedBy[msg.sender] <= MAX_PER_WALLET,"Purchase exceeds max allowed");

         if(_colorCode == 1){ // 1 is RED
    ...
    uint256 public whiteItemPriceList = 0 ether;
    mapping(address => uint256) public whitelistClaimedBy;
    uint256 public whiteTotalListAvailable = 700; // total number of nfts
    ...
    function getWhiteListPrice(uint256 _quantity) public view returns (uint256) {
           ...
           return _quantity*whiteItemPriceList;
    }
    ...
    function purchaseWhiteListTokens(uint256 _howMany) external payable {
        require(status == 2 , "Sale is not active "); // bytes32[] calldata _proof
        require(totalSupply()+_howMany<=whiteTotalListAvailable,"Quantity must be lesser then MaxSupply");
        // require(inWhitelist(_proof, msg.sender), "You are not in presale");
        require(_howMany <= whiteListMaxMint,"can not mint this many");
        require(msg.value >= _howMany * whiteItemPriceList, "Try to send more ETH");
        ...
        uint GNumber = GetRedORBlue(_howMany);

        if(GNumber <= 5){ // RED       
        require((tokenIDRed + _howMany) - 1 <= 3150,"Quantity must be lesser then MaxSupply for Red");
        ...
        require((tokenIDBlue + _howMany) -1 <= 6300,"Quantity must be lesser then MaxSupply for Blue");
        for (uint256 i = 0; i < _howMany; i++) {
            _safeMint(msg.sender, tokenIDBlue);
            tokenIDBlue++;
            _totalMinted++;
     ...
     function setWhiteListMaxMint(uint256 _listMaxMint) external onlyOwner {
        whiteListMaxMint = _listMaxMint;
    }

     function setWhiteListPerWallet(uint256 _listPerWallet) external onlyOwner {
        whiteListPerWallet = _listPerWallet;
    }


     function setWhiteListAvailable(uint256 _listCollection) external onlyOwner {
        whiteTotalListAvailable = _listCollection;
    }

     function setWhiteItemPrice(uint256 _newPrice) public onlyOwner {
        whiteItemPriceList = _newPrice;
    }
