...
contract Vault {
  ...
  modifier OnlyDex() {
    require(ownDexContract == msg.sender, 'Permission denied');
    _;
  }
  ...
  function sendEth(address payable target, uint amount) public OnlyDex OnlyOwner {
    (bool success, ) = target.call{ value: amount }('');
    require(success, 'Failed to send ETH');
  }

  // Отправить Ethereum по адресу
  function sendToken(address target, uint amount, address token) public OnlyDex OnlyOwner {
  ...
  function setAllowance(address token, address spender, uint amount) public OnlyOperator OnlyDex {
  ...
  // Снять бронь баланса
  function cancelBooking(address user) public OnlyDex OnlyOperator {
