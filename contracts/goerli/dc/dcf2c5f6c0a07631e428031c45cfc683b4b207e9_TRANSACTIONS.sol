...
import {ERC721A} from "lib/chiru-labs/ERC721A/contracts/ERC721A.sol";
...
/**
 ...
 @title  Transactions
 @author curion, chiru, opensea, openzeppelin
 [ ] 0.05 / mint
 ...
 * 
 */
contract TRANSACTIONS is ERC721A, DefaultOperatorFilterer, Ownable {
    using Strings for uint256;

    address public beforeTokenTransferHandler;
    address public paymentSplitterAddress;
    
    bool public airdropMintComplete = false;
    ...
    constructor() ERC721A("Transactions", "TXN") {}
    ...
    function mintPublic(uint256 _amount) public payable {
        ...
        if(!airdropMintComplete) { revert MintIsClosed(); }
        if(amountMinted >= maxSupply) { revert MaxSupplyReached(); }
        if(amountMinted + _amount > maxSupply) { revert MaxSupplyWillBeReached(); }
    ...
    // admin mints ids 1-998 to airdrop to Pendings holders. can batch if desired
    function mintForAirdrop(uint256 _amount) public onlyOwner {
        _safeMint(msg.sender, _amount);
        amountMinted+=_amount;

        if(_amount + amountMinted == pendingsSupply){
            airdropMintComplete = true;
        }
    }

    // admin mints ids 1-998 to airdrop to Pendings holders
    function mintForAirdropViaContract(address[] memory _recipients, uint256[] memory _amounts) public onlyOwner {
        for(uint256 i = 0; i < _recipients.length; i++){
            _safeMint(msg.sender, _amounts[i]);
            amountMinted+=_amounts[i];
        }
        airdropMintComplete = true;
    }
