...
contract OwnerScholarshipMarketplace is OwnerMarketplace {
  ...
  function userAcceptsAvailableScholarshipRental(uint256 rentId_) external payable whenNotPaused {
    ...
    offer.rewardConfiguration.addLastRewardReceiver(msg.sender);
  ...
  function offerCreatorCancelsRental(uint256 rentId_) external {
    return _ownerCancelsRentalOffer(rentId_);
  }
}
...
interface IContractRegistry {
    ...
    function contractExists(string memory contractName_) external view returns (bool);
...
abstract contract Marketplace is MarketplaceBase, IMarketplaceSide {
  ...
  function ownerChangesPaymentOptions(uint256 rentId_, bool rentExtensionAllowed_) external {
    ...
    offer.paymentOptions = _createPaymentInterface(
      offer.paymentOptions.payAddress(),
      offer.paymentOptions.payAmount(),
      false,
  ...
  function _ownerFinishesRental(uint256 rentId_) internal {
    ...
    _finishRental(rentId_, offer.tokenAddress.ownerOf(offer.tokenId), offer);
  }

  function _finishRental(
    uint256 rentId_,
    address owner_,
    RentOffer memory offer_
  ) internal {
    if (owner_ != msg.sender) {
      revert UserIsNotTheOwner();
    }
    offer_.status = RentStatus.ENDED;
    rentOffers[rentId_] = offer_;

    IERC4907 nftInterface = IERC4907(address(offer_.tokenAddress));
    if (nftInterface.userExpires(offer_.tokenId) >= block.timestamp) {
      revert CannotCancelYet();
    }
    nftInterface.setUser(offer_.tokenId, address(0), uint64(block.timestamp + 1));

    emit OwnerFinishRental(address(offer_.tokenAddress), offer_.tokenId, rentId_);
  }
}
...
abstract contract OwnerMarketplace is Marketplace {
  function userExtendsRental(uint256 rentId_) external payable {
    RentOffer memory offer = _getValidRental(rentId_, RentStatus.RENT);
  ...
  function _ownerCancelsRentalOffer(uint256 rentId_) internal {
    RentOffer memory offer = _getValidRental(rentId_, RentStatus.CREATED);
    ...
    emit OfferCreatorCancelRental(address(offer.tokenAddress), offer.tokenId, rentId_);
  }
...
abstract contract MarketplaceBase is Ownable, Pausable, IMarketplaceBase {
  ...
  function _forwardPayment(
    IPayment paymentOptions_,
    IContractRegistry.FeeSettings[] memory fees_,
    address to_
  ) internal {
    bool upfrontPayment = paymentOptions_.upfrontPayment();
    address from_ = upfrontPayment ? address(this) : msg.sender;
    ...
    if (paymentOptions_.payAddress() == address(0)) {
      ...
      if (upfrontPayment && !payToken.transfer(to_, payAmount - feePaid)) {
        revert ERC20TransferFailed();
      } else if (!upfrontPayment && !payToken.transferFrom(from_, to_, payAmount - feePaid)) {
        revert ERC20TransferFailed();
      }
    }
  }

  function _forwardNativeFees(
    uint256 paymentAmount_,
    IContractRegistry.FeeSettings[] memory fees_,
    address from_
  ...
  function _forwardErc20Fees(
    IERC20 paymentAddress_,
    uint256 paymentAmount_,
    IContractRegistry.FeeSettings[] memory fees_,
    address from_
  ) internal returns (uint256) {
    uint256 feesLen = fees_.length;
    bool upfrontPayment = from_ == address(this);
    uint256 totalFee = 0;
    for (uint256 i = 0; i < feesLen; ) {
      uint256 feeAmount = (paymentAmount_ * fees_[i].percentage) / 100 ether;

      if (upfrontPayment && !paymentAddress_.transfer(fees_[i].addr, feeAmount)) {
        revert ERC20TransferFailed();
      } else if (!upfrontPayment && !paymentAddress_.transferFrom(from_, fees_[i].addr, feeAmount)) {
        revert ERC20TransferFailed();
      }
      unchecked {
        ++i;
  ...
  function _createPaymentInterface(
    address paymentAddress_,
    uint256 paymentAmount_,
    bool upfrontPayment_,
    bool rentExtensionAllowed_
  ) internal returns (IPayment) {
    if (paymentAddress_ == address(0)) {
      ...
      return
        LPaymentDeployer.deployERC20Payment(paymentAddress_, paymentAmount_, upfrontPayment_, rentExtensionAllowed_);
    }
  }
...
interface IMarketplaceBase {
    ...
    // Shared events
    event OfferCreatorCancelRental(address indexed rentTokenAddress, uint256 indexed rentTokenId, uint256 rentId);
    event OwnerFinishRental(address indexed rentTokenAddress, uint256 indexed rentTokenId, uint256 rentId);
