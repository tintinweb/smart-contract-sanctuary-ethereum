...
contract Swap is SwapGovernance {
    ...
    function _swapToken1Inch(
        ...
        uint256 amountArg,
        string calldata functionCode,
        address dstToken,
        address srcAddr
    ) internal returns (uint256) {
        IERC20 srcTokenFinal;
        uint256 amountFinal = 0;
        if (keccak256(abi.encodePacked(functionCode)) == keccak256(abi.encodePacked('0x12aa3caf'))) {
        ...
        } else if (keccak256(abi.encodePacked(functionCode)) == keccak256(abi.encodePacked('0xf78dc253'))) {
        ...
        if (address(srcTokenFinal) == NATIVE_ADDRESS || srcTokenFinal == IERC20(address(0))) {
            uint256 feeAmount = (msg.value).mul(FEE_PERCENT).div(FEE_PERCENT_DENOMINATOR);
            // It means we are trying to transfer with Native amount
            //            require(msg.value >= amountFinal, "Key Pair: Amount Not match with Swap Amount.");
            require(
    ...
    function swapTokens(
        ...
        if (_1inchData.length > 1) {
            return _swapToken1Inch(_1inchData, amount, functionCode, dstToken, address(0));
...
contract SwapGovernanceTest is SwapGovernance, SwapStateTest {
    ...
    function updateAggregatorRouters(address _1inchRouterAddr, address _0xRouterAddr) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not a admin");
        require(_1inchRouterAddr != address(0), "Key Pair: oneInchAggregatorRouter Invalid");
        require(_0xRouterAddr != address(0), "Key Pair: OxAggregatorRouter Invalid");

        oneInchAggregatorRouter = _1inchRouterAddr;
        OxAggregatorRouter = _0xRouterAddr;
    }
    ...
    function swapTokens(
        ...
        if (_1inchData.length > 1) {
            if(keccak256(abi.encodePacked(functionCode)) == keccak256(abi.encodePacked('0x12aa3caf'))) {
                ...
                if (address(swapDescriptionObj.srcToken) != NATIVE_ADDRESS && swapDescriptionObj.srcToken != IERC20(address(0))) {
                    require(tokensMap[swapDescriptionObj.srcToken] != address(0), "Key Pair: mock does not exist for src token");
                }

                if (address(swapDescriptionObj.dstToken) != NATIVE_ADDRESS && swapDescriptionObj.dstToken != IERC20(address(0))) {
                    require(tokensMap[swapDescriptionObj.dstToken] != address(0), "Key Pair: mock does not exist for dst token");
                }

                return _swapToken1Inch(_1inchData, amount, functionCode, dstToken, tokensMap[swapDescriptionObj.srcToken]);
            } else if(keccak256(abi.encodePacked(functionCode)) == keccak256(abi.encodePacked('0xf78dc253'))) {
                (, IERC20 srcToken, uint256 amount, uint256 minReturn,) = abi.decode(_1inchData[4:], (address, IERC20, uint256, uint256, uint256[]));
                if (address(srcToken) != NATIVE_ADDRESS && srcToken != IERC20(address(0))) {
                    require(tokensMap[srcToken] != address(0), "Key Pair: mock does not exist for src token");
                }

                return _swapToken1Inch(_1inchData, amount, functionCode, dstToken, tokensMap[srcToken]);
            } else {
                revert("Key Pair: Invalid function code");
            }
        } else if (_0xData.length > 1) {
            ...
            console.log(swapDescriptionObj.inputToken);
            console.log(swapDescriptionObj.outputToken);
            if (swapDescriptionObj.inputToken != NATIVE_ADDRESS && IERC20(swapDescriptionObj.inputToken) != IERC20(address(0))) {
                require(tokensMap[IERC20(swapDescriptionObj.inputToken)] != address(0), "Key Pair: mock does not exist for src token");
            }

            if (swapDescriptionObj.outputToken != NATIVE_ADDRESS && IERC20(swapDescriptionObj.outputToken) != IERC20(address(0))) {
