...
contract TI7 is ERC721AQueryable, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  mapping(address => bool) public comWaitlistClaimed;
  ...
  string public uriPrefix = "ipfs://QmPMCmyaZjFRtxdYN8Q21yMM2inEcxKedRtBhDskMNkBuU/";
  ...
  bool public comWaitlistMintEnabled = false;
...
// ATG Main Constructor
  ...
  function comWhitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    require( reserveCount <= reserveSupply, "comWhitelist: Insufficient Spots");

    comWhitelistClaimed[_msgSender()] = true;
    reserveList[reserveCount].ids += uint16(_mintAmount + totalSupply());
    reserveList[reserveCount].addr = msg.sender;
    reserveList[reserveCount].claimed = false;
    reserveCount += uint16(_mintAmount);
  }

  function comWaitlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    // Verify whitelist requirements
    require(comWaitlistMintEnabled, "The community waitlist sale is not enabled!");
    require(!comWaitlistClaimed[_msgSender()], "Address already claimed!");
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof!");
    require( reserveCount <= reserveSupply, "comWaitlist: Insufficient Spots");

    comWaitlistClaimed[_msgSender()] = true;
    reserveList[reserveCount].ids += uint16(_mintAmount + totalSupply());
    reserveList[reserveCount].addr = msg.sender;
    reserveList[reserveCount].claimed = false;
    reserveCount += uint16(_mintAmount);
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof!");
    require( reserveCount <= reserveSupply, "Whitelist: Insufficient Spots");
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    require( reserveCount <= reserveSupply, "reserve: insufficient spots");
  ...
  function checkReserveAddr(uint256 ids) public view returns (mintReserve memory) {

    return reserveList[ids];
  }
  ...
  // Set Community Whitelist Status
  function setComWhitelistMintEnabled(bool _state) public onlyOwner {
    comWhitelistMintEnabled = _state;
  }

    // Set Community Waitlist Status
  function setComWaitlistMintEnabled(bool _state) public onlyOwner {
    comWaitlistMintEnabled = _state;
  }
