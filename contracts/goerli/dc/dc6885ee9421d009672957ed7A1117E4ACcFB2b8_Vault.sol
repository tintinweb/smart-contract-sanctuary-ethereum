...
contract Vault is ReentrancyGuard {
    ...
    // ERC20 token for interest payments
    IERC20 public CTM;
    IERC20 public USDC;

    // Wallet address for unrefundable portion of payments
    ...
    // Mappings for USDC token
    mapping(address => uint256) public usdcTokenBalance;

    // Deployer of the contract gains permission to supply interest as collateral
    address payable owner;
    address swap;

    // Set the basis points penalty for early widthdrawal 
    uint256 public penalty; // bps

    // Total amount of CTM in the contract
    uint256 public ctmTotal;
    // Total amount of USDC in the contract
    ...
    // Set the token that will be used for payments (USDC)
    function setPaymentToken(address token) public onlyOwner {
        USDC = IERC20(token);
    }

    // can withdraw entire payment token supply held in the contract / vault
    ...
    function removePaymentTokens(uint256 _amount) public onlyOwner {
        ...
        require(usdcTotal >= _amount, "There are not enought USDC tokens in the vault");
        usdcTotal -= _amount;
        emit RemovedPaymentTokens(msg.sender, usdcTotal);
    ...
    function addPaymentTokens(uint256 _amount) public onlyOwner {
        ...
        usdcTotal += _amount;
        emit AddedPaymentTokens(owner, _amount);
        USDC.safeTransferFrom(owner, address(this), _amount);
    }
    ...
    function supplyCTM (uint256 _amount) public onlyOwner {
        ...
        CTM.safeTransferFrom(msg.sender, address(this), _amount);
    }
    ...
    // Checks the balance of CTM
    function balanceOfCTM() public view returns(uint256) {
        return ctmTotal;
    }

    // Checks the balance of USDC in the contract
    ...
    function getFundsAddress() public view onlyOwner returns (address) {
        return fundsAddress;
    }

    function getSwapAddress() public view onlyOwner returns (address) {
        return swap;
    }
    ...
    // Allows only the swap contract to use this function
    modifier onlySwap() {
    ...
    function buyCTM(address _user, uint256 _usdcAmount, uint256 _ctmAmount) public onlySwap nonReentrant {
        ...
        // Penalty portion of payment sent to fundsAddress upon CTM purchase
        USDC.safeTransferFrom(_user, fundsAddress, unrefundableAmount);
        // Maximum refundable portion of payment sent to vault
        USDC.safeTransferFrom(_user, address(this), storedAmount);

        emit CtmPurchased(_user, _usdcAmount, _ctmAmount);
    }

    // Lets an account withdraw the CTM that was purchased
    /// @param _user this is the buyers address
    /// @param _amount this is the amount of CTM to send to the buyer
    function withdraw(address _user, uint256 _amount, uint256 _usdcAdjustment) public onlySwap {
        ...
        // Adjust the balances of the users account
        ctmTokenBalance[_user] -= _amount;
        usdcTokenBalance[_user] -= _usdcAdjustment;
        // Remove the CTM that was sent from the total amount in the contract
        ctmTotal -= _amount;
    ...
    /// @param _buyer this is the buyer's address
    /// @param _amount this is the amount to be refunded
    /// @param _ctmAdjustment this is the amount of CTM to be removed from the buyers account
    function refund(address _buyer, uint256 _amount, uint256 _ctmAdjustment) public onlySwap {
        ...
        // Remove the USDC that was sent from the total amount in the contract
        usdcTotal -= _amount;
