...
contract Stakable is Ownable {
    ...
    function _stake(
        ...
        stakeholders[index].userStakes.push(
            Stake(
                _msgSender(),
                amount,
                sinceBlock//,
    ...
    function _totalStakes() internal view returns (uint256) {
        ...
        for (uint256 s = 0; s < stakeholders.length; s += 1) {
            __totalStakes =
                __totalStakes +
                stakeholders[s].userStakes.length;
        }
        ...
        returns (uint256)
    {
        ...
        Stake memory current_stake = stakeholders[user_index].userStakes[
            index
        ];
        ...
        if (current_stake.amount == 0) {
            delete stakeholders[user_index].userStakes[index];
        } else {
        ...
        returns (StakingSummary memory)
    {
        ...
        StakingSummary memory summary = StakingSummary(
            0,
            stakeholders[stakes[_staker]].userStakes
        );
...
// Stake function createStake(uint256 stakeAmount)
// Stake function getBSKRAddress() external view returns (address) {
// Stake function getBSKRPerShare()
// Stake function getCurrentRewards()
// Stake function getTotalShares() external view returns (uint256) {
// Stake function getTotalStakeholders() external view returns (uint256) {
// Stake function getTotalStakes() external view returns (uint256) {
// Stake function refundLockedBSKR(uint256 from, uint256 to) external onlyManager {
// Stake function removeLockedRewards() external onlyManager {
// Stake function removeStake(uint256 stakeAmount) external whenNotPaused {
// Stake function rewardForBSKR(address stakeholder, uint256 bskrAmount)
// Stake function rewardOf(
// Stake function setBSKRAddress(address newBSKRAddr) external onlyManager {
// Stake function setInitialRatio(address newBSKRAddr, uint256 stakeAmount)
...
contract LBSKR is BaseBSKR, DSMath, Stakable {
        ...
        )
    {
        ...
        uint256 hoursElapsed = uint256(secsElapsed / _SECS_IN_AN_HOUR);
    ...
    function _creditInflation() private inflationStarted {
    ...
    function _swapTokensForTokens(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(this), tokenAmount); // allow this contract to spend its tokens
        _approve(address(this), address(_dexRouterV2), tokenAmount); // allow router to spend contract's tokens

        // uint256 balInfAddrB4 = _BSKR.balanceOf(_inflationAddress);
    ...
    //     // );
    //     // require(status, "LBSKR: TransferFrom failed");
    ...
    //     // emit Debug02L(stakeAmount, 0, 0, 0, 0);
    //     // _approve(address(this), address(this), stakeAmount); // allow this contract to spend its tokens
    //     // _approve(address(this), address(_dexRouterV2), stakeAmount); // allow router to spend contract's tokens

    //     _approve(_msgSender(), _msgSender(), stakeAmount); // allow this contract to spend its tokens
    //     _approve(_msgSender(), address(_dexRouterV2), stakeAmount); // allow router to spend contract's tokens
    ...
    //     _stakeholderToStake[_msgSender()].stakedBSKR += (balInfAddrAfter -
    //         balInfAddrBefore);
    ...
    //         balInfAddrBefore,
    //         balInfAddrAfter,
    //         (balInfAddrAfter - balInfAddrBefore),
    //         0,
    //         _stakeholderToStake[_msgSender()].stakedBSKR,
    //         _stakeholderToStake[_msgSender()].shares
    //     );
    ...
    // {
    //     if (_lastDistTS == 0) {
    //         return balanceOf(address(this)) - _totalStakes;
    //     }
    ...
    //     _creditInflation();
    //     uint256 stakeholderStake = _stakeholderToStake[_msgSender()]
    //         .stakedBSKR;
    //     uint256 stakeholderShares = _stakeholderToStake[_msgSender()].shares;

    //     require(stakeholderStake >= stakeAmount, "LBSKR: Stake low");
    ...
    function setBSKRAddress(address newBSKRAddr) external onlyManager {
     ...
     * wallet (owner) needs LBSKR allowance for itself (spender)
     */
    // function setInitialRatio(address newBSKRAddr, uint256 stakeAmount)
    //     external
    //     onlyManager
