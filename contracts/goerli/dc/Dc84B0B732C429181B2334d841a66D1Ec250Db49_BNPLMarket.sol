    ...
    IERC721Receiver
{
    ...
    uint256 public immutable CRA_SIGNATURE_VERSION = 3;
    ...
    event CreatedDiscreteOrder(uint256 bidId, address borrower);

    event AcceptedDiscreteOrder(uint256 bidId, address lender);
    ...
    function submitDiscreteOrder(
        ...
        bidId = _submitBid(_submitBidArgs, wethAddress);
    ...
    function acceptDiscreteOrder(
        ...
        DiscreteOrder memory order = discreteOrders[_bidId];

        _acceptBid(_bidId, _msgSender());

        require(
            discreteOrders[_bidId].lender == address(0),
            "Order already accepted"
        );
        discreteOrders[_bidId].lender = _msgSender();
        ...
        require(assetTokenId == order.assetTokenId, "Invalid assetTokenId");
        require(quantity == order.quantity, "Invalid quantity");
        ...
        emit AcceptedDiscreteOrder(_bidId, _msgSender());
    }
    ...
    function _hasAssetEscrowed(
        ...
        if (tokenType == TokenType.ERC721) {
            ...
            bool registerIsSet = validateAssetReceiptRegister(
                assetContractAddress,
                assetTokenId,
                quantity
            );
            resetAssetReceiptRegister();
            return registerIsSet;
        }
    }
