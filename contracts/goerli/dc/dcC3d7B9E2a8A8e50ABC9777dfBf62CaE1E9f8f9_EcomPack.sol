...
contract EcomPack is ERC1155, AccessControl {
    ...
    mapping(uint256 => uint256) public eurPrices;
    ...
    constructor(
        ...
        _addProduct(10000000, 23000); // 230€
        _addProduct(10000000, 42000); // 420€
    }
    ...
    function addProduct(
        ...
        _addProduct(_supply, _price);
    }

    function _addProduct(uint256 _supply, uint256 _price) internal {
        currentId += 1;
        supplyLeft[currentId] = _supply;
        eurPrices[currentId] = _price;
    }

    function updateSupplyLeft(
        uint256 _id,
        uint256 _supply
    ) public onlyRole(ADMIN_ROLE) {
    ...
    function getNFTPriceInETH(
        ...
        uint256 priceInEur = eurPrices[_tokenId];
    ...
    /**
     * @dev Set the price in USD (2 decimals) of a given token
     * @param _tokenId Id of the token to change the price of
     * @param _price New price in EUR (2 decimals) for the token
     */
    function setPrice(
        uint256 _tokenId,
        uint256 _price
    ) external onlyRole(ADMIN_ROLE) {
        eurPrices[_tokenId] = _price;
    }
}