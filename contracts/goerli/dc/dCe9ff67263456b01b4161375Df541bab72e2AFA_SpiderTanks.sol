...
contract SpiderTanks is IERC1155, ERC165, Pausable, CommonConstants, Ownable {    
        ...
        for (uint256 i = 0; i < _ids.length; ++i) {
            if (isNonFungible(_ids[i])) {
                ...
                balances[getNonFungibleBaseType(_ids[i])][_from] = balances[
                    getNonFungibleBaseType(_ids[i])][_from] - _values[i];
   ...
   function supportsInterface(bytes4 interfaceId) public view virtual override(ERC165, IERC165) returns (bool) {
    ...
    function setTokenURI(string calldata _uri, uint256 _id)
        ...
        creatorOnly
    {
        require(bytes(_uri).length > 0,"URI must have value");
        _tokenURIs[_id] = _uri;
        emit URI(uri(_id), _id);
    }

    function setBaseURI(string memory baseURI) external creatorOnly {
        _baseURI = baseURI;
