...
contract Artpad is Ownable {
    ...
    IERC20 _usdc;
    IERC20 _busd;
    mapping(uint => address) paymentTokens;
    IERC20 wARTR;
    IStaking staking;
    ...
    constructor(address _staking, address _wARTR, address _importToken) {
        paymentTokens[1] = 0xdAC17F958D2ee523a2206206994597C13D831ec7;//USDT
        paymentTokens[2] = 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48;//USDC
        paymentTokens[3] = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;//BUSD
    ...
    function registerUserBasic() public {
        address userAddress = _msgSender();
        uint tier = staking.getHolderTier(userAddress);
        require(whiteList[userAddress] || lottery[userAddress] || tier >= 3, "You don't have base allocation");
        ...
        if (tierCf == 0) tierCf = 10;
        if (timeCf == 0) timeCf = 10;
    ...
    function Claim() public {
        ...
        require(readyPayments > 0, "You do not have tokens to claim now");
    ...
    function buyForBNB(uint amount) public payable {
        ...
        uint allocation = getUserAllocation(account);
    ...
    function buyForToken(uint amount, uint tokenIndex) public {
        address account = _msgSender();
        IERC20 tokenContract = IERC20(paymentTokens[tokenIndex]);
        uint weiAmount = tokenContract.balanceOf(account);
        uint allocation = getUserAllocation(account);

        require(sellingIsActive, "Selling is not active");
        require(tokenIndex > 0 && tokenIndex < 4, "Invalid token index");
        require(users[account].registratedBasic, "User does not registered");
        require(weiAmount >= amount * price, "Exceeded balance");
        require(allocation >= amount, "Exceeded allocation");

        tokenContract.transferFrom(account, address(this), weiAmount);
        totalBoughtTokens += amount;
        users[account].tokenAmount = amount;
    ...
    function buyFreeSaleForBNB(uint amount) public payable {
        ...
        require(users[account].registratedBasic, "User doesn't registered");
        require(amount + totalBoughtTokens <= getTotalTokens(), "exeeded token amount");
        require(weiAmount >= amount * price, "Exceeded balance");
    ...
    function buyFreeSaleForToken(uint amount, uint tokenIndex) public {
        address account = _msgSender();
        IERC20 tokenContract = IERC20(paymentTokens[tokenIndex]);
        uint weiAmount = tokenContract.balanceOf(account);
        uint allocation = getUserAllocation(account);
    ...
    function setVesting(uint _firstVestingTime, uint _vestingTime, uint _vestingPercent) public onlyOwner {
        ...
        numOfPayments = 100 % vestingPercent > 0 ? 100/vestingPercent + 1 : 100/vestingPercent;
    }
    ...
    function isUserExists(address userAddress) public view returns (bool) {
        return (users[userAddress].registratedBasic);
    }

    function getUserAllocation(address userAddress) public view returns (uint) {
    ...
    function getUserData(address _holder) external view returns 
       (uint Cf,
        ...
        bool isPaid) {
            return 
            (users[_holder].Cf,
            users[_holder].registratedBasic,
            users[_holder].tokenAmount,
            users[_holder].gotTokens,
            users[_holder].gotPayments,
            users[_holder].isPaid);
    }
}