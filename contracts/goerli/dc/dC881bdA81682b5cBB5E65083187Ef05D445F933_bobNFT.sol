...
contract bobNFT is Ownable, PaymentSplitter, ERC721A, ReentrancyGuard {
    uint256 public immutable MAX_SUPPLY = 1000;
    uint256 public constant PRICE = 0.3 ether;
    uint256 public maxClaim = 1;
    uint256 public maxPublic = 5; //max total for public mint
    uint256 public maxPublicTx = 3; //max per tx public mint
    uint256 public immutable amountForTeam;

    bool public _MogulPresaleActive = false;
    bool public _isActive = false;

    mapping(address => bool) public allowList;
    mapping(address => uint8) public _claimListCounter;
    ...
    function mogulActive(bool isActive) external onlyOwner {
        _MogulPresaleActive = isActive;
    }

    function setMaxMogulmints(uint256 _maxPresale) external onlyOwner {
        maxClaim = _maxPresale;
    }

    function setMaxPublicMints(uint256 maxMints_) public onlyOwner {
        maxPublic = maxMints_;
    }
    ...
    // Mogul
    function Mogulpresale(uint8 quantity) external payable callerIsUser nonReentrant
    {
        require(_MogulPresaleActive, "Mogul presale is not active");
        require(_claimListCounter[msg.sender] + quantity <= maxClaim, "Exceeded max available to mint");
        require(quantity > 0, "Must mint more than 0 tokens");
        require(PRICE * quantity == msg.value, "Incorrect funds");
        require(totalSupply() + quantity <= MAX_SUPPLY, "would exceed max supply of Tokens");
        require(allowList[msg.sender] = true);
          
        _safeMint(msg.sender, quantity);
        _claimListCounter[msg.sender] =
            _claimListCounter[msg.sender] +
            quantity;
    }

    function addTheWhitelist(address[] calldata addresses)
        external
        onlyOwner
    {
    ...
    function removeTheWhitelist(address[] calldata addresses)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
    ...
    function publicSale(uint quantity) external payable nonReentrant callerIsUser
    {
        require(quantity > 0, "Must mint more than 0 tokens");
        require(_isActive, "public sale has not begun yet");
        require(PRICE * quantity == msg.value, "Incorrect funds");
        require(quantity <= maxPublicTx, "exceeds max per transactions");
