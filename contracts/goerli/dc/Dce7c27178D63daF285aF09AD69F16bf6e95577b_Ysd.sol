...
/// @notice The ERC20 token contract for Ysd V2
contract Ysd is ERC20Permit, Permissions {
  // Can mint/burn Ysd
  ...
  constructor(
    ...
    require(_timelock != address(0), "Ysd: Timelock addr(0)");
    require(initialAdmin != address(0), "Ysd: Admin addr(0)");
    require(_transferService != address(0), "Ysd: XferSvc addr(0)");
    ...
    onlyRoleYsd(ADMIN_ROLE, "Must have admin role")
  {
    ...
    require(!initialSetup, "Ysd: Already setup");
    initialSetup = true;

    for (uint256 i = 0; i < minters.length; i = i + 1) {
      require(minters[i] != address(0), "Ysd: Minter addr(0)");
      _setupRole(MONETARY_MINTER_ROLE, minters[i]);
    }
    for (uint256 i = 0; i < burners.length; i = i + 1) {
      require(burners[i] != address(0), "Ysd: Burner addr(0)");
  ...
  function mint(address to, uint256 amount)
    external
    onlyRoleYsd(MONETARY_MINTER_ROLE, "Must have monetary minter role")
  {
    _mint(to, amount);
  }
  ...
  function burn(address from, uint256 amount)
    external
    onlyRoleYsd(MONETARY_BURNER_ROLE, "Must have monetary burner role")
  {
    _burn(from, amount);
  }
...
contract ERC20Permit is ERC20, EIP712, IERC20Permit {
  ...
  constructor(string memory name, string memory ticker)
    ERC20(name, ticker) EIP712(name, "1") {}
  ...
  function permit(address target, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external override {
    ...
    require(verifyEIP712(target, hashStruct, v, r, s) || verifyPersonalSign(target, hashStruct, v, r, s), "invalide EIP712 #1");
  ...
  function transferWithPermit(address target, address to, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external returns (bool) {
    ...
    require(verifyEIP712(target, hashStruct, v, r, s) || verifyPersonalSign(target, hashStruct, v, r, s), "invalid EIP712 #2");
...
interface ITransferService {
  function verifyTransfer(address, address, uint256) external view returns (bool, string memory, address[2] memory, bytes[2] memory);
  function verifyTransferAndCall(address, address, uint256) external returns (bool, string memory);
