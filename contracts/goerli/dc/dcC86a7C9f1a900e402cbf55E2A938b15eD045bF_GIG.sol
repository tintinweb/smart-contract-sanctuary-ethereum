...
contract GIG is protected, IERC20 {
    ...
    constructor() {
        ...
        fees[normalList] = FEES(1, 1, 9, 33, 33, 34);
        fees[whitelistedList] = FEES(0, 0, 0, 0, 0, 0);
        fees[bluelistedList] = FEES(1, 1, 1, 0, 0, 100);
        fees[greenlistedList] = FEES(1, 1, 3, 33, 33, 34);
    ...
    function swapTokensForEth(uint _amount) internal returns(uint _gain) {
        ...
        if ( ! (_allowances[address(this)][address(router)] >= _amount) ) {
            approve(address(router), _amount + 1);
        }
        // Avoid swapping more than the contract balance
        if (! (_balances[address(this)] >= _amount) ) {
            _amount = _balances[address(this)];
        }
