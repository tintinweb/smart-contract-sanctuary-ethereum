...
contract WeRent is WerentSignature, IWeRent, ERC721Holder, ERC1155Receiver, ERC1155Holder, Initializable {
    ...
    function distributeClaimPayment(LendingRenting memory _lendingRenting, DiscountSignature memory _discountSignature)
        private
    {
        bool useNativeToken = _lendingRenting.lending.useNativeToken;
    ...
    function handleClaimCollateral(CallData memory _cd) private {
        for (uint256 i = _cd.left; i < _cd.right; i++) {
            ...
            distributeClaimPayment(item, _cd.discountSignatures[i]);
...
contract WerentSignature {
    function getMessageHash(
        uint256 _discount,
        uint256 _deadline,
        address _lender
    ) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_discount, _deadline, _lender));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(_discount, _deadline, _lender);
