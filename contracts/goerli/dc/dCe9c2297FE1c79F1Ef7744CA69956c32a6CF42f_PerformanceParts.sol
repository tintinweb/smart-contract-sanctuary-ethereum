    ...
    ERC1155Supply
{
        ...
        allowedToMint(_quantity)
    {
        ...
        uint amount = getListAt(currentList).amount;
        ...
        emit UserMinted(
            _msgSender(),
            getListAt(currentList).listName,
            _quantity
        );
    }
...
abstract contract Allowlists is Mintable {
    ...
    function getListAt(uint index) public view returns (Allowlist memory) {
        return allowlists[index];
    }

    function getRemainingMints(address account) public view returns (uint) {
        uint allowed = allowlists[currentList].allowed;
        uint count = mintCount[currentList][account];
    ...
    modifier allowedToMint(uint quantity) {
        ...
        require(
            quantity <= getRemainingMints(_msgSender()),
            "Quantity > Allowed"
        );
        _;
    }
