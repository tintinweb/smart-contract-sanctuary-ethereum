...
contract ClosePaPremiumPass is ERC721AQueryable ,ERC2981 ,TeamsAccess ,Pausable ,DefaultOperatorFilterer  {

    using Strings for uint256;

    uint256 public maxSupply    = 1500;
    uint96  private royaltyFee   = 1000; // 10%
    address public royaltyAddress;

    uint256 private prePrice     = 0.025 ether;//presale price.
    uint256 private publicPrice  = 0.025 ether;//publicSale price.

    enum SaleState { NON, NOT/*1*/, PRE/*2*/, PRE2/*3*/, PUB/*4*/, FIN /*5*/} // Enum
    SaleState public saleState = SaleState.NOT;

    uint256 private max_Mint_AL    = 2;
    uint256 private max_Mint_AL2nd = 3;
    ...
    payable whenNotPaused {
        ...
        if(saleState == SaleState.PRE){
            require(_MintedAL[msg.sender] + _amount <= max_Mint_AL,"Over max minted");
            _safeMint(msg.sender, _amount);
            _MintedAL[msg.sender]+=_amount;
        }else{
            require(_MintedAL2nd[msg.sender] + _amount <= max_Mint_AL2nd,"Over max minted");
            _safeMint(msg.sender, _amount);
            _MintedAL2nd[msg.sender]+=_amount;
        }
    }
    ...
    function setExtContract(address _addr) external onlyOwner{
