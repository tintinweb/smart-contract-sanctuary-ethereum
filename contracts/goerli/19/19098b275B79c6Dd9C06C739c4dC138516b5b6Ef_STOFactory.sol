    ...
    STOErrors
{
    ...
    /// @dev Event emitted when wallets are changed in the the whitelist
    ...
    /// @dev Event emitted when the owner change the stored beacons addresses for STOToken and STOEscrow
	...
	/// @dev Event emitted when the feesa are charged at each new tokenization
    /// @param issuer wallet address of the issuer
    /// @param currrency token used to pay the fee, usually BKN
    ...
    function initialize(
		...
		require(
            Address.isContract(IBeacon(beaconToken).implementation()),
            "ERC1967: beacon Token implementation is not a contract"
        );
		require(
            Address.isContract(IBeacon(beaconEscrow).implementation()),
            "ERC1967: beacon Escrow implementation is not a contract"
        );
    ...
    function changeWhitelist(address[] calldata users, bool[] calldata statuses)
        ...
        onlyOwner
    {
        if(users.length != statuses.length || users.length == 0) revert LengthsMismatch();
    ...
    /// @dev And the Beacon will be upgradeable thorugh JS/TS script (e.g. via Hardhat: `await upgrades.upgradeBeacon(beaconAddress, newImplementation);`)
    ...
    function changeBeacon(address newToken, address newEscrow)
        external
        whenNotPaused
        onlyOwner
    {
    ...
    /// @param newPriceInBKN New price in BKN
    /// @param newPriceInUSD New price in USDC
    function changeFee(uint256 newPriceInBKN, uint256 newPriceInUSD)
        external
        whenNotPaused
        onlyOwner
    {
    ...
    function newTokenization(TokenizationConfig memory config)
        external
        whenNotPaused
        onlyWhitelisted
    {
    ...
    function _chargeFee(address user) internal {
        ...
        uint256 amountToPay = priceInBKN > 0 ? priceInBKN : 0;

        /// If a fixed fee in dollar is set, it then overwrites it with it
        if (priceInUSD > 0) {
            /// Take USD price and divide priceInUSD for that price and round up to obtain amount of BKN to pay
            /// Change then amountToPay for this amount
			emit ChargeFee(user, 'USD', amountToPay);
        }
...
contract BeaconProxy is Proxy, ERC1967Upgrade {
	...
	function upgradeBeaconToAndCall(address beacon, bytes memory data) public  {
        _upgradeBeaconToAndCall(beacon, data, false);
    }
}
