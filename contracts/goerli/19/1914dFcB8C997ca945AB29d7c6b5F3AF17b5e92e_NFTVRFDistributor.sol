...
interface INFTBatchTransfer {
    function getStartId(ERC721Like nftAddress) external view returns (uint256 startId);
...
contract NFTVRFDistributor is VRFv2Consumer {
    ...
    **/
    ...
    function fulfillRandomWords(
        ...
        round.nounSupply = uint16(NOUNS_TOKEN.totalSupply());
    ...
    **/
    ...
    /// @param nftAddress address of the NFT collection being distributed
    function requestRound(address nftAddress) external onlyOwner returns (uint256 requestId) {
        uint256 availNFTs = remainingAllowance(nftAddress);
        if (availNFTs > 256) {
          revert RoundIsTooBig();
        }
    ...
    function claim(address nftAddress, uint8 index) external {
        uint8 currentRound = currentRounds[nftAddress];
        ClaimRound memory round = claimRounds[nftAddress][currentRound];
        ...
        uint256 startId = NFT_BATCH_TRANSFER.getStartId(ERC721Like(nftAddress));
    ...
    function claimMany(address nftAddress, uint8[] calldata indexes) external {
        uint8 currentRound = currentRounds[nftAddress];
        ClaimRound memory round = claimRounds[nftAddress][currentRound];
        if (round.randomness == 0) {
            revert InvalidRound();
