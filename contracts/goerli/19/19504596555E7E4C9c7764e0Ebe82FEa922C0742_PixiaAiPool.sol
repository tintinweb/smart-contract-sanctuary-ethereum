    ...
    ReentrancyGuardUpgradeable
{
    ...
    function pendingReward(address user_) public view returns (uint256) {
        ...
        // If user lock is expired, but lock was not removed yet ==> we should not calculate the lock booster from the lock ends time
        uint64 userLockEndTime = user.lockTime + user.lastDepositAt;
        ...
        uint64 userLastRewardAt = user.lastRewardAt;
        if (userLastRewardAt < user.lastDepositAt)
            userLastRewardAt = user.lastDepositAt;
        if (userLastRewardAt < _startAt) userLastRewardAt = _startAt;

        // User reward amount is calculated in normal way, but we should consider when lock expired
        uint256 userRewardAmount = (user.boostedAmount *
            adjustedTokenPerShare) /
            PRECISION_FACTOR -
            user.rewardDebt;

        // If lock expired, but not reset yet, we recalculate the amount since lock expired
        if (
            userLockEndTime > userLastRewardAt &&
            userLockEndTime < block.timestamp
        ) {
            uint64 lockedTime = userLockEndTime - userLastRewardAt;
            uint64 passedTime = uint64(block.timestamp) - userLastRewardAt;

            uint16 lockBooster = uint16(
                100 + (user.lockTime * maxLockMultiplier) / maxLockTime
            );
            uint256 rewardAmountInLocked = (userRewardAmount * lockedTime) /
                passedTime;
            uint256 rewardAmountInExpired = ((userRewardAmount *
                (passedTime - lockedTime)) /
                passedTime /
                lockBooster) * 100;
            userRewardAmount = rewardAmountInLocked + rewardAmountInExpired;
        }
    ...
    function _preUpdateUser() internal {
        ...
        if (pending > 0) {
            ...
            user.lastRewardAt = uint64(block.timestamp);
        }
    ...
    function addReward(uint256 amount_) external payable {
        ...
        if (launchPlan == ICommon.LaunchPlan.FREE) {
            uint16 commissionFee = IPixiaAiPoolFactory(poolFactory)
                .freePlanCommission();
            address payable pixiaAiTreasury = IPixiaAiPoolFactory(poolFactory)
                .treasury();
