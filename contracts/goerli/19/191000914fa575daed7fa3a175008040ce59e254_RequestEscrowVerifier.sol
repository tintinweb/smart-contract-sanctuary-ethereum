...
abstract contract BaseVerifier is IIncomeVerifier, Ownable2Step {
    /// @notice This event is emitted whenever a pool is allowed to be verified
    /// @param pool The target pool
    /// @param canVerify True if allowed to be verified
    event PoolAuthorizationSet(address pool, bool canVerify);

    /// @notice This error is thrown whenever an address without access tries
    /// to execute a transaction
    error AccessForbidden();

    /// @notice Mapping of pool to canVerify bool
    mapping(address => bool) private s_pools;

    constructor() Ownable2Step() {}

    modifier isAuthorizedPool() {
        if (!s_pools[msg.sender]) revert AccessForbidden();
        _;
    }

    /**
     * @notice Grants pool authorization to be verified
     * @param pool The address of the target pool
     * @param canVerify True if pool is authorized
     */
    function setPool(address pool, bool canVerify) external onlyOwner {
        s_pools[pool] = canVerify;
        emit PoolAuthorizationSet(pool, canVerify);
    }

    /**
     * @notice Returns pool authorization bool
     * @param pool The address of the target pool
     */
    function getIsAuthorizedPool(address pool) external view returns (bool) {
        bool canVerify = s_pools[pool];
        return canVerify;
    }
...
/**
@notice Interface contract so that the verifier can call the RequestEscrow contract.
This is defined here as Request does not expose an interface for the
Escrow contract.
*/
interface IRequestEscrow {
    struct Request {
...
/**
 * @notice This contract verifies that a borrower minting an NFR
 * has escrowed funds from an employer
 */
contract RequestEscrowVerifier is BaseVerifier {
    /// @notice Tracks the amount of funds shared
    /// from an escrow agreement
    mapping(bytes => uint256) private s_sharedEscrowAmounts;

    /// @notice A pointer to Request Finance's global escrow contract.
    IRequestEscrow private s_requestEscrow;

    constructor(address requestEscrow) BaseVerifier() {
        s_requestEscrow = IRequestEscrow(requestEscrow);
    }

    /// @inheritdoc BaseVerifier
    /// @dev Verifies that there are escrowed funds routed to
    /// an income router from an income source before minting
    /// a new NFR
    function verify(
        bytes32 verificationSource,
        address routerAddr,
        ...
        address owner,
        address token
    ) external override isAuthorizedPool {
        IRequestEscrow.Request memory request = s_requestEscrow.requestMapping(
            paymentBytes
        );

        if (
            request.payee != routerAddr ||
            request.amount <= faceValue + s_sharedEscrowAmounts[paymentBytes] ||
            (request.unlockDate != 0 && request.unlockDate > maturityDate) ||
            token != request.tokenAddress
        ) revert VerificationFailed();

        s_sharedEscrowAmounts[paymentBytes] += request.amount;

        IPool(msg.sender).mint(
            verificationSource,
            routerAddr,
            request.payer,
            maturityDate,
    ...
    /// @dev Gets the address of request escrow for testing
    /// purposes
    function getRequestEscrow() external view returns (address) {
        return address(s_requestEscrow);
    }

    /// @dev Gets the total shared escrow of the payment ref
    /// for testing purposes
    function getSharedEscrowAmount(
        bytes memory paymentBytes
    ) external view returns (uint256) {
        return s_sharedEscrowAmounts[paymentBytes];
    }
}
