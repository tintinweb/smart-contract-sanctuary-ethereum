...
contract Clu3 is Clu3Whitelist {
    ...
    function verifySigner(string memory _message, bytes memory _signature)
        public
        pure
        returns (address)
    {
    ...
    function clu3Transaction(uint256 _timestamp, bytes memory _signature)
        ...
        returns (bytes32)
    {
        if (!senderInWhitelist(i_signer) || !isWhitelistImplemented()) {
            return bytes32(0);
        }

        if (_timestamp + i_lifespan > block.timestamp) {
            revert Clu3__TimestampAlreadyPassed();
        }

        // Verify that the message: "timestamp-clu3_id-sender_address"
        if (verifySigner("abcde", _signature) != i_signer) {
        ...
        return
            bytes32(abi.encodePacked(Strings.toString(_timestamp), i_clu3Id));
    }
