...
contract RichardNFT is ERC721Enumerable, Ownable {    
    ...
    uint256 _mintTimeSWhite = 0;  // white mint start time
    uint256 _mintTimeEWhite = 0;  // white mint end time
    bytes32 _merkleRoot; // white merkleRoot

    uint256 _mintTimeS = 0;  // start mint time
    uint256 _mintTimeE = 0;  // end mint time
    mapping(address => bool) public _claimed;  // claimed address
    address private _withdrawAddress; // withdraw address
    string _myBaseUri = '';
    ...
    // 開發階段使用，正式部屬要拿掉
    function deleteClaimed(address[] calldata users) 
        external
    {
        for (uint256 i; i < users.length; i++) {
            delete _claimed[users[i]];
        }
    }
