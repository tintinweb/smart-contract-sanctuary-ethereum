...
contract Tontine is ERC721URIStorage, Ownable {
    ...
    function splitBalance() external {
        if (tokenHolders.length() >= TOTAL_WINNERS) {
            emit BalancePayout("Too many winners", address(0), 0);
            revert("Too many winners");
        }

        if (_tokenIds.current() <= TOTAL_SUPPLY - 1) {
            emit BalancePayout("Not enough minted", address(0), 0);
            revert("Not enough minted");
        }

        uint256 balance = address(this).balance;

        if (balance <= 1000 || splitPayoutDone) {
            emit BalancePayout("No payout", address(0), 0);
            revert("No payout");
        }
