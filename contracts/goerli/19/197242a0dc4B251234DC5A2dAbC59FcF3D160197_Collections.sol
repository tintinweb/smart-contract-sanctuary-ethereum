...
contract Collections is Initializable, UUPSUpgradeable {
    ...
    */
    ...
    function create_collection(string memory name, uint256 supply, bytes calldata uri_, 
        string[] calldata cdn_, uint256 royaltyValue) public {
            // require(msg.value >= 0, "Not enough amount for the Fees");
            ERC1155Lockable ERC1155 =new ERC1155Lockable(name,name, true);
            ...
            ERC1155.mint(msg.sender, supply, uri_, cdn_, msg.sender, royaltyValue);
     ...
     */
    function mint(ERC1155Lockable Collection, uint256 supply, bytes calldata uri_, bytes calldata key, string[] calldata cdn_, uint256 royaltyValue) public  {
        Collection.mint(msg.sender, supply, uri_, cdn_, msg.sender, royaltyValue);
    }
...
contract ERC1155Lockable is
    ...
    constructor(string memory _name, string memory _symbol, bool _isProtected)
        ERC1155("") {
            name=_name;
            id = 0;
            symbol = _symbol;
     ...
     * acceptance magic value.
     */
    function mint(address to, uint256 amount, bytes calldata uri_, string[] calldata cdn_, address royaltyRecipient, uint256 royaltyValue) external virtual  {
        _mint(to, id, amount, "");
        if (uri_.length > 0) {
