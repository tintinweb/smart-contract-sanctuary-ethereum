...
contract MergeNFT is ERC721, Ownable, ERC721URIStorage {
    uint256 private _tokenCounter;
    uint256 private _canvasDimension;
    address private immutable MERGE_CANVAS_CONTRACT_ADDRESS;
    mapping(uint256 => address) public tokenIdToContributor;
    mapping(address => bool) private hasMinted;
    MergeCanvas mergeCanvasContract;
    ...
    struct Square {
        uint16 x;
        uint16 y;
        MergeCanvas.RGB color;
    }

    constructor(address _merge_canvas_contract_address)
        ERC721("MergeNFT", "MTK")
    {
        _tokenCounter = 0;

        MERGE_CANVAS_CONTRACT_ADDRESS = _merge_canvas_contract_address;
        mergeCanvasContract = MergeCanvas(MERGE_CANVAS_CONTRACT_ADDRESS);
        _canvasDimension = uint256(mergeCanvasContract.CANVAS_DIMENSION());
    }
    ...
    function mint() public Contributed {
        ...
        string memory svg = generateSVG(msg.sender);
        string memory imageURI = svgToImageURI(svg);
        string memory uri = formatTokenURI(imageURI);
        _setTokenURI(tokenId, uri);
        emit MergeNFT_Minted(msg.sender, tokenId);
    }

    function mint2(address to, string calldata uri) public onlyOwner {
        // if (hasMinted[to] == true) {
        //     revert MergeNFT_MintedBefore(to);
        // }
        hasMinted[to] = true;
        uint256 tokenId = _tokenCounter;
        _tokenCounter = _tokenCounter + 1;
        ...
        _setTokenURI(tokenId, uri);
        emit MergeNFT_Minted(to, tokenId);
    }

    function mint3() public Contributed {
        // if (hasMinted[msg.sender] == true) {
        //     revert MergeNFT_MintedBefore(msg.sender);
        // }
        hasMinted[msg.sender] = true;
    ...
    modifier Contributed() {
        if (!mergeCanvasContract.Contributed(msg.sender)) {
    ...
    function tokenURI_method3(uint256 tokenId)
        public
        view
        returns (string memory)
    {
        ...
        return
            formatTokenURI(
                svgToImageURI(generateSVG(tokenIdToContributor[tokenId]))
            );
    }

    function generateSVG(address to) internal view returns (string memory) {
        return
            string.concat(
                '<svg xmlns="http://www.w3.org/2000/svg" height="',
                Strings.toString(_canvasDimension),
                '" width="',
                Strings.toString(_canvasDimension),
                '">',
                generateRects(to),
        ...
        returns (string memory _rects)
    {
            ...
            uint256[] memory pixel_coordinates = mergeCanvasContract.getAddressPixels(_to);
        ...
        for (uint16 i = 0; i < pixel_coordinates.length; i++) {
            ...
            uint16 y_coordinate = uint16(curr_pixel_coordinates_encoded % (2**16));

            MergeCanvas.RGB memory pixel_color = mergeCanvasContract.getPixelColor(x_coordinate, y_coordinate);

            //TODO: query the pixel
            Square memory sqr = Square({
                x: uint16(pixel_coordinates[i] >> 16),
                y: uint16(pixel_coordinates[i] % (2**16)),
                color: pixel_color
            });
    ...
    function op_generateRects(address _to)
        internal
        view
        returns (string memory _rects)
    {
        (
            uint256[] memory pixel_coordinates,
            MergeCanvas.RGB[] memory pixel_colors
        ) = mergeCanvasContract.getPixelsForAddress(_to);

        uint256 pixel_coordinates_encoded = pixel_coordinates[0];
        uint16 x_coordinate = uint16(pixel_coordinates_encoded >> 16);
        uint16 y_coordinate = uint16(pixel_coordinates_encoded % (2**16));

        Square memory sqr_curr = Square({
            x: x_coordinate,
            y: y_coordinate,
            color: pixel_colors[0]
        });

        Square memory sqr_next;

        _rects = string.concat(_rects, firstTag(sqr_curr));

        for (uint16 i = 0; i < pixel_coordinates.length - 1; i++) {
            pixel_coordinates_encoded = pixel_coordinates[i];
            x_coordinate = uint16(pixel_coordinates_encoded >> 16);
            y_coordinate = uint16(pixel_coordinates_encoded % (2**16));

            MergeCanvas.RGB memory pixel_color = pixel_colors[i];
            ...
            pixel_coordinates_encoded = pixel_coordinates[i + 1];
            x_coordinate = uint16(pixel_coordinates_encoded >> 16);
            y_coordinate = uint16(pixel_coordinates_encoded % (2**16));
            ...
            _rects = string.concat(_rects, middleTag(sqr_curr, sqr_next));
        }
        _rects = string.concat(_rects, lastTag(sqr_next));
        return _rects;
    }

    function firstTag(Square memory _sqr) internal pure returns (string memory) {
        return
            string.concat(
                '<path fill="rgb(',
                Strings.toString(_sqr.color.R),
                ",",
                Strings.toString(_sqr.color.G),
                ",",
                Strings.toString(_sqr.color.B),
                ')" d="M',
                Strings.toString(_sqr.x)
            );
    }

    function middleTag(Square memory _sqr_curr, Square memory _sqr_next)
        internal
        pure
        returns (string memory)
    {
        return
            string.concat(
                " ",
                Strings.toString(_sqr_curr.y),
                "h1v1H",
                Strings.toString(_sqr_curr.x),
                "zM",
                Strings.toString(_sqr_next.x)
            );
    }

    function lastTag(Square memory _sqr_next)
        internal
        pure
        returns (string memory)
    {
...
contract MergeCanvas {
    ...
    modifier SufficientBid(uint16 _x, uint16 _y) {
        uint256 pixel_price = _calculatePixelPrice();
        if (msg.value < pixel_price) {
            revert MergeCanvas_InsufficientBid(msg.value, pixel_price);
        }
        _;
    }

    //OPTIONAL: Limit the number of pixels an address can own
    modifier AddressCanChangePixels() {
        uint256 address_pixel_capacity = pixel_lookup[msg.sender].length();
        if (address_pixel_capacity >= 100) {
    ...
    modifier BeforeMerge() {
        if (merged) {
            revert MergeCanvas_AlreadyMerged("Merge has occured");
        }
        _;
    }
    ...
    function _calculatePixelPrice()
        internal
        view
        returns (uint256 pixel_price)
    {
        uint256 time_difference = block.timestamp.sub(DEPLOY_TIMESTAMP);
        // Price doubles every day
        uint256 price_power = (time_difference.div(86400));
        if (price_power == 0) {
            return 0;
        }
        return (2**price_power).mul(500000000000000); // 0.0005 ETH
    }
    ...
    function changePixelColor(
        ...
        pixel_lookup[msg.sender].remove(pixel_encoded);
    ...
    function getPixelPrice() external view returns (uint256 pixel_price) {
        pixel_price = _calculatePixelPrice();
        return pixel_price;
    }
        ...
        returns (uint256[] memory address_pixels)
    {
        ...
        return address_pixels;
    }

    // Gets the all pixels (coordinate and color) for an address
    function getPixelsForAddress(address _address)
        external
        view
        returns (uint256[] memory pixel_coordinates, RGB[] memory pixel_colors)
    {
        pixel_coordinates = pixel_lookup[_address].values();
        uint256 pixel_count = pixel_coordinates.length;
        for (uint256 i = 0; i < pixel_count; i++) {
            uint16 x = uint16(pixel_coordinates[i] >> 16);
            uint16 y = uint16(pixel_coordinates[i] % (2**16));
            bytes32 coordinates_encoded = _calculateCoordinatesHash(x, y);
            // pixel_colors[i] = pixel_color[coordinates_encoded];
        }
        return (pixel_coordinates, pixel_colors);
    }

    // Temporary function to simulate the merge happening
    function mergeNow() external OnlyOwner returns (uint256) {
        merged = true;
        return 1;
    }
