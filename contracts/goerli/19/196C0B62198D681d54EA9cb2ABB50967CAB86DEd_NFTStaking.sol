    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 private constant _BUFFER_REWARDS = 0x152D02C7E14AF6800000; // 100000 LBSKR
    ...
    function _availableRewards(address user) private view returns (uint256) {
        if (stakeMap[user].stakes.length == 0) {
            return stakeMap[user].totalUnclaimedRewards;
        }
        uint256 _rewards = _calculateRewards(user);
        return _rewards;
    }
        ...
        returns (uint256 rewards)
    {
        ...
        if (rewardsAccruing) {
            ...
            for (uint256 j; j < stakesMeta.stakes.length; ++j) {
                rewards +=
                    stakesMeta.stakes[j].unclaimedRewards +
                    ((nowTS - lastUpdateTS) * _REWARDS_PER_HOUR) /
                    countOfStakes /
                    3600;
            }
        } else {
            rewards = stakesMeta.totalUnclaimedRewards;
        }
    ...
    function _creditRewards() private {
        if (rewardsAccruing) {
            uint256 unclaimedRewardsOverall;
            uint256 nowTS = block.timestamp;
            for (uint256 i; i < stakersArray.length; ++i) {
                StakesMeta storage stakesMeta = stakeMap[stakersArray[i]];
                uint256 unclaimedRewardsSum;
                for (uint256 j; j < stakesMeta.stakes.length; ++j) {
                    stakesMeta.stakes[j].unclaimedRewards +=
                        ((nowTS - lastUpdateTS) * _REWARDS_PER_HOUR) /
                        countOfStakes /
                        3600;
                    unclaimedRewardsSum += stakesMeta
                        .stakes[j]
                        .unclaimedRewards;
                }
                stakesMeta.totalUnclaimedRewards = unclaimedRewardsSum;
                unclaimedRewardsOverall += unclaimedRewardsSum;
                // stakeMap[stakersArray[i]] = stakesMeta;
            }
            lastUpdateTS = nowTS;

            // The purpose of reward balance if for central check in the DAPP
            // DAPP would disable the stake button and display warning that rewards are depleting
            if (rewardBalance < lbskrToken.balanceOf(address(this))) {
                rewardBalance = lbskrToken.balanceOf(address(this));
            }

            if (
                (lbskrToken.balanceOf(address(this)) -
                    unclaimedRewardsOverall) < _BUFFER_REWARDS
            ) {
    ...
    function claimRewards() external {
        ...
        require(stakesMeta.stakes.length > 0, "You have no tokens staked");
        ...
        for (uint256 j; j < stakesMeta.stakes.length; ++j) {
            ...
            rewardsPen +=
                (stakesMeta.stakes[j].unclaimedRewards * eligibleBasis) /
                10000;
            rewards += stakesMeta.stakes[j].unclaimedRewards;
            stakesMeta.stakes[j].unclaimedRewards = 0;
        }
        // stakeMap[_msgSender()] = stakesMeta;

        require(rewards > 0, "You have no rewards to claim");
        stakeMap[_msgSender()].totalUnclaimedRewards -= rewards;
    ...
    function stake(uint256[] calldata tokenIds) external nonReentrant {
        ...
        for (uint256 i; i < tokenIds.length; ++i) {
            ...
            stakeMap[_msgSender()].stakes.push(
                Stake(tokenIds[i], block.timestamp, 0, 0)
            );
            ++countOfStakes;
        }
    }
