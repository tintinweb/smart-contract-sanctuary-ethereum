  ...
  UUPSUpgradeable
{
   ...
   */
  function _hashRoyaltyInfo(RoyaltyInfo calldata royaltyInfo_)
    internal
    pure
    returns (bytes32)
  {
   ...
   */
  function updateMaxRoyaltyReceiversCount(uint8 _newMaxRoyaltyReceiversCount)
    external
    onlyAdmin
  {
  ...
  function _validateRoyaltyInfo(
    ...
    // EM: RoyaltyRegistry:_validateRoyalty Overfrow royalty value limit or value 0
    require(
      _royaltyInfo.secondaryOnwardsRoyaltyPercentage > 0 &&
        _royaltyInfo.secondaryOnwardsRoyaltyPercentage <=
        (feeDenominator() / 2),
      "RR:VRI:ROFZ"
    );
  }
   ...
   * @param _token address
   */
  function deleteCollectionDefaultRoyalty(address _token)
    external
    virtual
    onlyAdmin
  {
   ...
   */
  function resetTokenRoyalty(address _token, uint256 _tokenId)
    external
    virtual
    onlyAdmin
  {
