...
import "./extensions/Payout2981Support.sol";
    ...
    ReentrancyGuard
{
    ...
    EnumerableSet.AddressSet private whitelistedTokens1155_721;
    ...
    Counters.Counter internal _proposalCounter;
    ...
    mapping(uint256 => ProposeData) public proposals;
    ...
    uint32 public constant TEN_PERCENTS = 1000;
    uint32 public fee;
    ...
    event ProposalCanceled(address creator, uint256 offerId);
    ...
    function addTokensToWhitelist(
        ...
        for (uint256 i; i < tokens.length; ++i) {
            if (!tokensType[i]) {
                ...
                whitelistedTokens1155_721.add(tokens[i]);
            }
        }
    }
    ...
    function removeTokensFromWhitelist(
        ...
        for (uint256 i; i < tokens.length; ++i) {
            if (!tokensType[i]) {
                ...
                whitelistedTokens1155_721.remove(tokens[i]);
            }
        }
    }
    ...
    {
        if (tokenType) {
            return whitelistedTokens1155_721.values();
        } else {
    ...
    function setFee(uint32 newFee) external onlyOwner {
        require(newFee <= TEN_PERCENTS, "Fee can not be more than 10%.");
        fee = newFee;
        emit SetFee(newFee);
    }
    ...
    function createOffer(
        ...
        require(
            whitelistedTokens1155_721.contains(token),
    ...
    function propose(
        ...
        require(quantity <= offers[offerId].quantity, "Quantity too big.");

        uint256 proposalId = uint256(_proposalCounter.current());

        proposals[proposalId] = ProposeData(
            msg.sender,
            offerId,
            price,
            quantity
        ...
        if (saleToken == address(0)) {
            require(price == msg.value, "Value is not equal.");
        } else {
            IERC20(offers[offerId].saleToken).safeTransferFrom(
                msg.sender,
                address(this),
                price
            );
    ...
    function acceptProposal(uint256 offerId, uint256 proposalId)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        require(isOfferActive(offerId), "Offer does not active.");
        uint256 offerQuantity = offers[offerId].quantity;
        require(
            offers[offerId].creator == msg.sender,
            "You are not owner of the offer."
        );
        require(isProposalExist(proposalId), "Proposal does not exist.");

        address token = offers[offerId].token;
        address reciever = proposals[proposalId].creator;
        address creator = offers[offerId].creator;
        address saleToken = offers[offerId].saleToken;
        uint256 tokenId = offers[offerId].tokenId;
        uint256 price = proposals[proposalId].price;
        uint256 proposalQuantity = proposals[proposalId].quantity;
        uint256 priceWithoutFee = price - calculatePart(price);
        uint256 royaltyAmount;

        if (proposalQuantity == offerQuantity) {
            offers[offerId].active = false;
        } else {
            offers[offerId].quantity = offerQuantity - proposalQuantity;
        }
        ...
        if (IERC165(token).supportsInterface(type(IERC721).interfaceId)) {
            ...
            if (IERC721(token).supportsInterface(type(IERC2981).interfaceId)) {
                address receiver;
                (receiver, royaltyAmount) = getRoyaltyInfo(
                    token,
                    tokenId,
                    price
                );
                ...
                if (saleToken != address(0)) {
                    IERC20(saleToken).safeTransfer(receiver, royaltyAmount);
                } else {
            ...
            if (IERC1155(token).supportsInterface(type(IERC2981).interfaceId)) {
                address receiver;
                (receiver, royaltyAmount) = getRoyaltyInfo(
                    token,
                    tokenId,
        ...
        if (saleToken == address(0)) {
            ethBalance += calculatePart(price);
            (bool sent, ) = payable(msg.sender).call{
                value: priceWithoutFee - royaltyAmount
            }("");
            require(sent, "Failed to send Ether");
        } else {
            tokenBalances[saleToken] += calculatePart(price);
            IERC20(saleToken).safeTransfer(
                msg.sender,
                priceWithoutFee - royaltyAmount
            );
        }
    }
        ...
        nonReentrant
    {
        require(isOfferActive(offerId), "Offer does not active.");
        uint256 offerQuantity = offers[offerId].quantity;
        require(quantity <= offerQuantity, "Quantity too big.");
        require(
            offers[offerId].creator != msg.sender,
            "You are owner of the offer."
        );

        uint256 amount = offers[offerId].price * quantity;
        address token = offers[offerId].token;
        uint256 tokenId = offers[offerId].tokenId;
        address saleToken = offers[offerId].saleToken;
        address creator = offers[offerId].creator;
        ...
        if (quantity == offerQuantity) {
            ...
            offers[offerId].quantity = offerQuantity - quantity;
        }

        if (IERC1155(token).supportsInterface(type(IERC2981).interfaceId)) {
            (, royaltyAmount) = getRoyaltyInfo(token, tokenId, amount);
            _repayRoyalty(msg.sender, saleToken, token, tokenId, amount);
        }

        if (saleToken == address(0)) {
            require(amount + royaltyAmount == msg.value, "Value is not equal.");
            ethBalance += calculatePart(amount);

            payable(creator).transfer(
                msg.value - calculatePart(amount) - royaltyAmount
            );
        } else {
            tokenBalances[saleToken] += calculatePart(amount);
    ...
    function cancel(uint256 id, bool action)
        external
        whenNotPaused
        nonReentrant
    {
        if (action) {
            ...
            require(isProposalExist(id), "Proposal does not exist.");
            require(
                proposals[id].creator == msg.sender,
                "You are not owner of the proposal."
            );
            uint256 offerId = proposals[id].offerId;
            uint256 price = proposals[id].price;
            address saleToken = offers[offerId].saleToken;
            ...
            if (saleToken == address(0)) {
                ...
                IERC20(saleToken).safeTransfer(msg.sender, price);
            }

            emit ProposalCanceled(msg.sender, id);
        }
    }
    ...
    function isProposalExist(uint256 proposalId) public view returns (bool) {
        if (proposals[proposalId].price != 0) return true;
        else return false;
    }

    function calculatePart(uint256 value) internal view returns (uint256) {
        return (value / 10000) * fee;
    }
