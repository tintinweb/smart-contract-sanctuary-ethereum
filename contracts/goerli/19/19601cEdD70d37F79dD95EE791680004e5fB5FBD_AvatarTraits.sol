...
contract AvatarTraits is ERC1155Supply, AccessControl, ReentrancyGuard {
    ...
    constructor(address defaultAdmin, bool _isOpen)
        ERC1155("http://base.uri/token?id=")
    {
        isOpen = _isOpen;
    ...
    function createItem(
        ...
        emit ItemCreated(items[_tokenIds.current()]);
        _tokenIds.increment();
    }
    ...
    function toggleItemSale(uint256 id) external onlyRole(MANAGER_ROLE) {
        ...
        if (items[id].active)
            emit StartItemSale(
                ...
                items[id].maxSupply,
                items[id].unitPrice,
                items[id].maxPerWallet,
                items[id].transfersEnabled
            );
        else
            emit StopItemSale(
                items[id].id,
    ...
    function toggleTransfers(uint256 id, bool enabled)
        external
        onlyRole(MANAGER_ROLE)
    {
        items[id].transfersEnabled = enabled;
    }
    ...
    function mint(uint256 id) external nonReentrant {
        nonces[msg.sender].increment();
        _mint(msg.sender, id, 1, "");
    }

    function mintBatch(address owner, uint256[] memory ids)
        external
        nonReentrant
        onlyRole(MANAGER_ROLE)
    {
