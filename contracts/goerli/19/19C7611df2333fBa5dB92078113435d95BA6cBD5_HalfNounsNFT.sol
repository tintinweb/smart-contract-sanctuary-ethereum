...
/// @title The HalfNouns ERC-721 token
...
contract HalfNounsNFT is IHalfNounsNFT, Ownable, ERC721Checkpointable {
    // The Nouns DAO address
    address public nounsDAO;

    // The halfNounders DAO address (creators org)
    address public halfNoundersDAO;

    // An address who has permissions to mint HalfNouns
    address public minter;

    // The HalfNouns token URI descriptor
    IHalfNounsDescriptor public descriptor;

    // The HalfNouns token seeder
    IHalfNounsSeeder public seeder;
    ...
    // The halfNoun seeds
    mapping(uint256 => IHalfNounsSeeder.Seed) public seeds;

    // The internal halfNoun ID tracker
    uint256 private _currentHalfNounId;
    ...
    /**
     * @notice Require that the sender is the halfNounders DAO.
     */
    modifier onlyHalfNoundersDAO() {
        require(msg.sender == halfNoundersDAO, 'Sender is not the halfNounders DAO');
        _;
    }
    ...
    constructor(
        address _nounsDAO,
        address _halfNoundersDAO,
        address _minter,
        IHalfNounsDescriptor _descriptor,
        IHalfNounsSeeder _seeder
    )
        // IProxyRegistry _proxyRegistry
        ERC721('Half Nouns NFT', 'HALF NOUN')
    {
        nounsDAO = _nounsDAO;
        halfNoundersDAO = _halfNoundersDAO;
        minter = _minter;
    ...
    /**
     * @notice Mint a HalfNoun to the minter, along with a possible halfNounders reward
     * HalfNoun. HalfNounders reward HalfNouns are minted every 10 HalfNouns, starting at 0,
     * until 183 halfNounder HalfNouns have been minted (5 years w/ 24 hour auctions).
     * @dev Call _mintTo with the to address(es).
     */
    function mint() public override onlyMinter returns (uint256) {
        if (_currentHalfNounId <= 3650 && _currentHalfNounId % 20 == 0) {
            _mintTo(halfNoundersDAO, _currentHalfNounId++);
        }
        if (_currentHalfNounId <= 3650 && _currentHalfNounId % 20 == 1) {
            _mintTo(nounsDAO, _currentHalfNounId++);
        }
        return _mintTo(minter, _currentHalfNounId++);
    }

    /**
     * @notice Burn a halfNoun.
     */
    function burn(uint256 halfNounId) public override onlyMinter {
        _burn(halfNounId);
        emit HalfNounBurned(halfNounId);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        // require(_exists(tokenId), 'HalfNounsNFT: URI query for nonexistent token');
    ...
    /**
     * @notice Set the halfNounders DAO.
     * @dev Only callable by the halfNounders DAO when not locked.
     */
    function setHalfNoundersDAO(address _halfNoundersDAO) external override onlyHalfNoundersDAO {
        halfNoundersDAO = _halfNoundersDAO;

        emit HalfNoundersDAOUpdated(_halfNoundersDAO);
    }
     ...
     * @dev Only callable by the owner when not locked.
     */
    function setDescriptor(IHalfNounsDescriptor _descriptor) external override onlyOwner whenDescriptorNotLocked {
        descriptor = _descriptor;
     ...
     * @dev Only callable by the owner when not locked.
     */
    function setSeeder(IHalfNounsSeeder _seeder) external override onlyOwner whenSeederNotLocked {
        seeder = _seeder;
    ...
    /**
     * @notice Mint a HalfNoun with `halfNounId` to the provided `to` address.
     */
    function _mintTo(address to, uint256 halfNounId) internal returns (uint256) {
        IHalfNounsSeeder.Seed memory seed = seeds[halfNounId] = seeder.generateSeed(halfNounId, descriptor);

        _mint(owner(), to, halfNounId);
        emit HalfNounCreated(halfNounId, seed);
