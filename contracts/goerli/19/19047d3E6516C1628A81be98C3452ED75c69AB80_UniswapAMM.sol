...
contract UniswapAMM is Owned {
    ...
    ISweepAMOMinter private amo_minter;
    ...
    constructor(
        ...
        amo_minter = ISweepAMOMinter(_amo_minter_address);

        // Get the treasury and timelock addresses from the minter
        treasury_address = amo_minter.treasury_address();
    ...
    function refreshTargetPrice(uint _price) external onlyByOwnStabilizers {
        amo_minter.refreshSweepTargetPrice();
    }
...
interface ISweepAMOMinter {
  ...
  function burnSweepFromAMO(uint256 sweep_amount) external;
  function refreshSweepTargetPrice() external;
  ...
  function setTreasury(address _treasury_address) external;
