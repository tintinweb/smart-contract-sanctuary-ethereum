...
contract MevRewardsSenseiStake is Ownable, ReentrancyGuard {
    ...
    event NFTOwnerUpdated(address indexed owner);
    ...
    function setNFTOwner(uint256 _tokenId, address _owner) external onlyOwner {
        if (_owner == address(0)) {
            revert InvalidAddress();
        }
        nftOwner[_tokenId] = _owner;
        emit NFTOwnerUpdated(_owner);
    }
}