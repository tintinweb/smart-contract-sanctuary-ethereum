...
contract FlashLoanArbitrage is FlashLoanSimpleReceiverBase {
    ...
    function swapExactInputSingle(
        ...
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
            ...
            amountIn: amountIn,
            amountOutMinimum: normal ? amountIn / getLatestPrice() : amountIn * getLatestPrice(), // CHANGE MIN TO AVOID LOSS          change exchange rate here?
            sqrtPriceLimitX96: 0
        });
    ...
    function swapOnCurve(
        ...
        if (normal) {
            ICurvePool(curveRouter).exchange(0, 2, _amount, _amount * getLatestPrice(), false); // CHANGE MIN TO AVOID LOSS
        } else if (!normal) {
            ICurvePool(curveRouter).exchange(0, 2, _amount, _amount / getLatestPrice(), false);
        } else {
            revert("transaction not possible");
        }
    ...
    function changeToken0(address _tokenAddress) external {
        token0 = _tokenAddress;
    }

    function changeToken1(address _tokenAddress) external {
