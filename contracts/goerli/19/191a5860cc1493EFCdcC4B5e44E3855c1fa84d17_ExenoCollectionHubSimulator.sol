    ...
    IERC1363Receiver
{
    ...
    function onTransferReceived(
        ...
        ) = abi.decode(data, (bytes32, address, uint16, address, uint256, uint256, bytes));
        if (!signers[sender]) {
            _verifySigner(id, _extractSigner(abi.encodePacked(id, beneficiary, tier, currency, price, bonus), signature));
        }
        ...
        if (amount < price)
            revert UndeliveredTokenPayment();
        uint256 ticket = _issueTicket(beneficiary, tier);
        emit Purchase(id, beneficiary, tier, currency, price, bonus, ticket);
    ...
    VRFV2WrapperConsumerBase
{
    ...
    uint16 internal lastDraw;

    Request internal lastRequest;

    mapping(uint16 domain => uint256 count) internal sales;
    ...
    mapping(uint16 draw => mapping(uint16 domain => Draw)) internal draws;
    ...
    /// Not enough tickets have been minted to execute a draw
        ...
        VRFV2WrapperConsumerBase(linkToken, linkWrapper)
    {
        ...
        setTier(2, 4, 3000);
        setTier(3, 10, 2000);
        setEndpoint(["https://dataphora.com/nft/meta/", ".json"]);
    }
    ...
    function setTier(
        uint16 id,              // tier id, i.e. 1 | 2 | 3
        uint16 multiplier,      // scaling factor to increase the probability a ticket is selected in a random draw
    ...
    {
        lastRequest = Request({
            id: requestRandomness(gasLimit, NUM_OF_CONFIRMATIONS, size),
            goal: goal,
            data: data,
            random: new uint256[](0),
            payment: VRF_V2_WRAPPER.calculateRequestPrice(gasLimit),
            status: false
        });
        emit RequestSent(lastRequest.id);
    }
        ...
        internal override
    {
        ...
        if (lastRequest.goal == Goal.MILESTONE) {
            ...
            for (uint16 i = 0; i < 3; i++) {
                if (reward[i] > 0) {
                    _draw(i + 1, request, random[i], reward[i]);
                }
            }
        } else if (lastRequest.goal == Goal.FINAL) {
            lastDraw++;
            _draw(0, request, random[0], abi.decode(lastRequest.data, (uint256)));
        }
    }
        ...
        internal
    {
        ...
        assert (ticket > 0);
        uint16 tier = records[ticket].tier;
        assert (domain == 0 || tier == domain);
        rewards[ticket].amount = reward;
        records[ticket].recalled = true;
        uint32 timestamp = uint32(block.timestamp);
        draws[lastDraw][domain] = Draw(lastDraw, timestamp, request, random, size, index, ticket, records[ticket].holder, reward);
        emit DrawResult(lastDraw, domain, tier, ticket, records[ticket].holder, reward);
    }
    ...
    /**
     */
    function getSales(uint16 domain)
    ...
    function getTicket(uint256 ticket)
        external view returns(uint32 timestamp, uint16 tier, address holder, uint256 reward, bool recalled, bool paid)
    {
    ...
    function getSpace(uint16 domain)
        ...
        if (domain > 0) {
            ...
            for (uint16 tier = 1; tier <= 3; tier++) {
                for (uint256 i = 1; i <= sales[tier]; i++) {
                    uint256 ticket = tier * TIER_RANGE + i;
                    if (!_isDisqualified(ticket)) {
                        uint16 multiplier = TIERS[records[ticket].tier - 1].multiplier;
                        size += multiplier;
                    }
                }
            }
    ...
    {
        return (lastRequest.id, lastRequest.random, lastRequest.payment, lastRequest.status);
    }

    function getDraw(uint16 domain)
        external view returns(
            uint16 id,
            uint32 timestamp,
            uint256 request,
    ...
    {
        Draw memory d = draws[lastDraw][domain];
        return (d.id, d.timestamp, d.request, d.random, d.size, d.index, d.ticket, d.holder, d.reward);
    }

    function getDraws(uint16 domain)
        external view returns(Draw[] memory list, uint256 size)
    {
        list = new Draw[](lastDraw);
        for (uint16 i = 0; i < lastDraw; i++) {
            list[i] = draws[i + 1][domain];
        }
        return (list, lastDraw);
    }
...
// File: contracts/ExenoCollectionHubSimulator.sol


pragma solidity 0.8.18;


contract ExenoCollectionHubSimulator is
    ExenoCollectionHub
{
    constructor(
        address coreToken,
        ...
        ExenoCollectionHub(coreToken, payoutToken, linkToken, linkWrapper)
    {}

    function simulateMilestoneDraw(
        uint256[3] memory random,
        uint256[3] memory reward
    )
        external onlyOwner
    {
        _simulate(Goal.MILESTONE, abi.encode(reward), _toArray(random));
    }

    function simulateFinalDraw(
        uint256 random,
        uint256 reward
    )
        external onlyOwner
    {
        _simulate(Goal.FINAL, abi.encode(reward), _toArray(random));
    }
    ...
    {
        lastRequest = Request({
            id: 0, goal: goal, data: data, random: new uint256[](0), payment: 0, status: false
        });
