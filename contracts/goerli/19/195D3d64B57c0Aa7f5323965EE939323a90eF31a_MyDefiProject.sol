...
error MyDefiProject__MintFailed();
error MyDefiProject__RedeemFailed();
error MyDefiProject__EnterMarketFailed();
error MyDefiProject__BorrowFailed();
error MyDefiProject__RepayBorrowFailed();
error MyDefiProject__GetAccountLiquidityFailed();
error MyDefiProject__AccountUnderwater();
error MyDefiProject__NotEnoughCollateral();
...
contract MyDefiProject {
    ...
    function supply(address cTokenAddress, uint underlyingAmount) public {
        ...
        if (result != 0) {
            revert MyDefiProject__MintFailed();
        }
    }
    ...
    function redeem(address cTokenAddress, uint cTokenAmount) external {
        ...
        if (result != 0) {
            revert MyDefiProject__RedeemFailed();
        }
    }
    ...
    function enterMarket(address cTokenAddress) external {
        ...
        if (results[0] != 0) {
            revert MyDefiProject__EnterMarketFailed();
        }
    }
    ...
    function borrow(address cTokenAddress, uint256 borrowAmount) external {
        ...
        if (result != 0) {
            revert MyDefiProject__BorrowFailed();
        }
    }
        ...
        external
    {
        ...
        if (result != 0) {
            revert MyDefiProject__RepayBorrowFailed();
        }
    }

    /// @notice Calculate the max amount a user is allowed to borrow
    /// @dev A Helper function to determine how much a user is allowed to borrow
    /// @param cTokenAddress The address of the compound pair
    /// @return liquidity divided by underlyingPrice
    function maxBorrow(address cTokenAddress) external view returns (uint) {
