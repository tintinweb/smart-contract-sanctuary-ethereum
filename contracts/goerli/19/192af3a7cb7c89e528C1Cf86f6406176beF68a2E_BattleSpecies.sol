...
abstract contract Withdrawable {
    ...
    function withdraw(address[3] memory _team, uint256 amount) internal {
        if (amount > address(this).balance)
            amount = address(this).balance;
        uint256 share = (amount * 3333333) / 10000000;
        _widthdraw(_team[2], share);
        _widthdraw(_team[1], share);
        _widthdraw(_team[0], amount - (share * 2));
    }
...
contract BattleSpecies is ERC721A, OperatorFilterer, Biographical, Withdrawable, Ownable  {
    ...
    constructor(address _hyperwarpManager, address _utilityHub, bytes32 _wlRoot) ERC721A("Battlespecies", "BATTLE") {
    ...
    function inWhitelist(bytes32[] calldata _wlProof) public view returns (bool isValid){
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        return MerkleProofLib.verify(_wlProof, wlRoot, leaf);
    }
...
contract Biographical {
    ...
    function reserveName(string memory newName, string memory oldName) internal {
        ...
        bool isDifferent;
        assembly {
            isDifferent := eq(keccak256(add(newName, 0x20), mload(newName)), keccak256(add(oldName, 0x20), mload(oldName)))
        }
        require(isDifferent, "NameMustBeDifferent"); 
