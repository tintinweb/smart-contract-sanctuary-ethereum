...
contract ERC721PresetWhitelist is ERC721, ERC721Enumerable, Ownable, Pausable, ReentrancyGuard, Whitelist {
    ...
    constructor (
        ...
        uint _mintingFee
    ) ERC721("ERC721PresetBase", "ERC721PB") {
    ...
    function mint(uint amount) external payable onlyWhitelisted whenNotPaused nonReentrant {
        ...
        require(supply + amount < MAX_SUPPLY + 1, "exceed max supply");

        for(uint i; i < amount; i++){
            _safeMint(msg.sender, supply+1+i); // +1 because metadata starts with 1
        }
    }
    ...
    function setRequireWhitelist(bool value) public onlyOwner {
        _setRequireWhitelist(value);
    }
...
contract Whitelist {
    ...
    modifier onlyWhitelisted() {
        if(requireWhitelist) {
            require(isWhitelisted(msg.sender), "Whitelist: not found");
        }
        _;
    }
