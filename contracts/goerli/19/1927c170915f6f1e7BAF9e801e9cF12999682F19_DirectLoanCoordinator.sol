...
contract DirectLoanCoordinator is IDirectLoanCoordinator, Ownable {
        ...
        returns (uint32)
    {
        ...
        require(
            getContractFromType(_loanType) == loanContract,
            "Caller must be registered for loan type"
        );

        // (loanIds start at 1)
        totalNumLoans += 1;

        uint64 smartNftId = uint64(
        ...
        SmartNft(promissoryNoteToken).mint(
            _lender,
            smartNftId,
        ...
        onlyInitialized
    {
        ...
        require(
            getTypeFromContract(loanContract) != bytes32(0),
            "Caller must a be registered loan type"
        );
        ...
        // nedded?
        require(
            SmartNft(promissoryNoteToken).exists(smartNftId),
            "Promissory note should exist"
        );
        // nedded?
        require(
            !SmartNft(obligationReceiptToken).exists(smartNftId),
            "Obligation r shouldn't exist"
        );
        ...
        SmartNft(obligationReceiptToken).mint(
            _borrower,
            smartNftId,
            abi.encode(_loanId)
        );
    ...
    function resolveLoan(uint32 _loanId) external override onlyInitialized {
        ...
        require(
            loan.loanContract == msg.sender,
            "Not the same Contract that registered Loan"
        );
     ...
     */
    function registerLoanType(string memory _loanType, address _loanContract)
        external
        onlyOwner
    {
     ...
     */
    function getContractFromType(bytes32 _loanType)
        public
        view
        returns (address)
    {
     ...
     */
    function getTypeFromContract(address _loanContract)
        public
        view
        returns (bytes32)
    {
     ...
     * @param _loanContract - The address of the wrapper contract that implements the loan type's behaviour.
     */
    function _registerLoanType(string memory _loanType, address _loanContract)
        internal
    {
     ...
     * @param _loanContracts - The addresses of each wrapper contract that implements the loan type's behaviour.
     */
    function _registerLoanTypes(
        string[] memory _loanTypes,
        address[] memory _loanContracts
    ) internal {
        require(
            _loanTypes.length == _loanContracts.length,
