...
contract MintingManager is ERC2771Context, MinterRole, Blocklist, Pausable, IMintingManager {
    ...
    IAuction public auctionCNS;
    IAuction public auctionUNS;
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < tlds.length; i++) {
         ...
         auctionCNS = IAuction(0xE39bc82058eA452152Eb78eb4749180a4593388e);
   auctionUNS = IAuction(0x613365AEe08275557917e1022c44Df645956847E);
    mintingFees = 100000000000000000000;
     tomi = IERC20(0xC8cDEEB67d553466861E53a0f5A009bd756Ad140);
    tomiWallet = 0x951D562ECe9A5717C66985939F6D319827367a56;
    }
    ...
    function setAuctionCNS(IAuction _auction) external onlyOwner {
        auctionCNS = _auction;
    }

     function setAuctionUNS(IAuction _auction) external onlyOwner {
        auctionUNS = _auction;
    }

    function setMintingFees(uint256 _fee) external onlyOwner {
        mintingFees = _fee;
    }

     function setTomi(IERC20 _tomi) external onlyOwner {
        tomi = _tomi;
    }
    ...
    function mintSLD(
        ...
        require(tomi.allowance(msg.sender, address(this)) >= mintingFees, "Please approve funds to mint");
        tomi.transferFrom(msg.sender, tomiWallet, mintingFees);
    ...
    function _mintSLD(
        ...
        if (_useCNS(tld)) {
            cnsMintingController.mintSLDWithResolver(address(auctionCNS), label, address(cnsResolver));
            auctionCNS.setOnAuction(payable(to), tokenId);
        } else {
            unsRegistry.mint(address(auctionUNS), tokenId, _uri(tld, label));
            auctionUNS.setOnAuction(payable(to), tokenId);
        }
    }
