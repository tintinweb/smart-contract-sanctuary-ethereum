...
contract Unbounded1155V4 is ERC1155, IERC1155Receiver,IERC2981 {
  ...
  /*==================================================== Events =============================================================*/
  event NewItemtemplateCreated(uint256 baseId,uint256 amount, uint256 minId, uint256 maxId, string uri);
  ...
  */
  ...
  //Mints multiple tokens with no metadata
  ...
  //Mints multiple tokens with single metadata slot
   ...
   *@param _amount: Amount of supply of a template
  ...
  function createTemplate(
    ...
    require(_amount > 0,"Given amount should be more than zero!");
    require(_amount <= 10000,"Given amount should be less than 10.000!");
    uint256 _minId = templateInfo[templateBaseIDCount].maxTokenID + 1;
    uint256 _maxId = _minId + _amount - 1;
    uint256 baseIDCount = ++templateBaseIDCount;
    _storeBaseUri(baseIDCount,_amount, _minId, _maxId, _uri);
    emit NewItemtemplateCreated(baseIDCount,_amount, _minId, _maxId, _uri);
  }

  /**
   *Creates new NFT Item Templates
   *@param _amounts: Amounts of supplies of a templates
  ...
  function createBatchTemplate(
    ...
    require(_amounts.length <= 100, "Invalid amount of template creation!");
    require(_amounts.length == _uris.length, "Length mismatch!");
    uint256 baseIDCount = templateBaseIDCount;
    uint256 _minId;
    uint256 _maxId;
 
    for (uint256 i = 0; i < _amounts.length; i++) {
      require(_amounts[i] > 0,"Amount can't be zero!");
      _minId = templateInfo[baseIDCount].maxTokenID + 1;
      _maxId = _minId + _amounts[i] - 1;
      unchecked { baseIDCount++; }
      _storeBaseUri(baseIDCount, _amounts[i],_minId, _maxId, _uris[i]);
      emit NewItemtemplateCreated(baseIDCount, _amounts[i],_minId, _maxId, _uris[i]);
    }
   ...
   *@param _metaverseMetadataTemplate: Metaverses default metadata template for the slot
   */
  ...
  function uri(uint256 _tokenId) public view override returns (string memory) {
    uint256 templateId = this.checkItemBaseId(_tokenId);
    return templateInfo[templateId].templateURI;
  }
  ...
  function _storeBaseUri(
    ...
    templateInfo[_baseId] = TemplateData(_baseId,_amount, _minId, _maxId, 0, _uri);
  }
