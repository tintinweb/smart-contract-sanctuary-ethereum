...
contract Pool is IPool, NonReentrancy, WeekManaged, Ownable {
    ...
    ClaimRequest[] claimRequestArray;
    ...
    function buy(
        ...
        require(toWeek_.sub(fromWeek_) <= policyWeek,
            "Too many weeks");
        require(fromWeek_ > getNow().div(1 weeks), "Buy next week");

        Policy storage policy = policyArray[policyIndex_];
        uint256 premium = amount_.mul(policy.weeklyPremium).div(1e4);
        ...
        uint256 maximumToCover = poolInfo.amountPerShare.mul(
            poolInfo.totalShare).sub(poolInfo.pendingWithdrawAmount).mul(
                1e4).div(policy.collateralRatio);
    ...
    function getTidalAmount(address who_) external view returns(uint256) {
        UserInfo storage userInfo = userInfoMap[who_];
