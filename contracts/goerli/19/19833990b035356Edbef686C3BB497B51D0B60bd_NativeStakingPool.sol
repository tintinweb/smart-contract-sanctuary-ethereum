...
abstract contract FeeCollectionGuard
{
	using Address for address payable;
	...
	{
		require(msg.sender == feeRecipient, "access denied");
		_;
	}
	...
	function setFeeRecipient(address payable _feeRecipient) external onlyFeeManager
	{
		feeRecipient = _feeRecipient;
		emit UpdateFeeRecipient(_feeRecipient);
	}

	function setFixedValueFee(bytes4[] calldata _selectors, uint256 _fixedValueFee) external onlyFeeManager
	{
	...
	event UpdateFeeManager(address _feeManager);
