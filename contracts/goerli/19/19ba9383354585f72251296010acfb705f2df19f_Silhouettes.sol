...
contract Silhouettes is ERC721Enumerable, Ownable 
{
    ...
    uint256 public SLPRICE = 30000000000000000; //0.03 eth
    uint256 public WLPRICE = 30000000000000000; //0.03 eth
    uint256 public PUBLICPRICE = 40000000000000000; //0.04 eth
    uint public SLperAddressLimit = 2;
    uint public WLperAddressLimit = 1;
    uint public PublicperAddressLimit = 2;
    ...
    function SLmint(uint256 amount, bytes32[] memory proof) external payable
    {
        ...
        require(msg.value >= SLPRICE * amount, "Not enough ETH for transaction");
        require(addressMintedBalance[msg.sender] + amount <= SLperAddressLimit, "Max NFT per address exceeded");
    ...
    function WLmint(uint256 amount, bytes32[] memory proof) external payable
    {
        ...
        require(totalSupply() + amount <= MAX_TOKENS - (NUMBER_RESERVED_TOKENS - reservedTokensMinted), "Purchase would exceed max supply");
        require(msg.value >= SLPRICE * amount, "Not enough ETH for transaction");
        require(addressMintedBalance[msg.sender] + amount <= WLperAddressLimit, "Max NFT per address exceeded");
