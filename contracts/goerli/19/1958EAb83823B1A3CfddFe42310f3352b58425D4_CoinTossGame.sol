...
contract CoinTossGame {
    ...
    enum GameStatus {Pending, Win, Lose} // Renamed from GameResult to GameStatus

    struct Game {
        address player;
        CoinSide chosenSide;
        GameStatus status; // Renamed from result to status
    }
    ...
    event GameStatusChanged(uint256 indexed gameId, GameStatus status); // Renamed from GameResult to GameStatus
    ...
    constructor(address _erc20Token) {
        owner = msg.sender;
        erc20Token = _erc20Token; // Assign the provided ERC20 token address to the contract variable
    }
    ...
    function startGame(CoinSide _chosenSide, uint256 _betAmount) external {
        ...
        games.push(Game(msg.sender, _chosenSide, GameStatus.Pending));
        emit GameStarted(gameId, msg.sender);

        // Perform the coin toss and determine the result
        CoinSide coinSide = randomCoinSide();
        GameStatus status = (coinSide == _chosenSide) ? GameStatus.Win : GameStatus.Lose;
    ...
    function withdrawHouseBalance(address _token) external onlyOwner {
        ...
        emit HouseBalanceDeposited(amount);
    }
}
