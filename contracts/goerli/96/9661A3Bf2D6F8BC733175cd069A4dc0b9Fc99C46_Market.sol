...
contract Market is ERC721, ERC721Burnable, Ownable, ReentrancyGuard, Pausable, IMarket {
  ...
  function _deleteLockId(
    ...
    uint256[] storage _lockIds = _map[_target];
    _replaceElement(_lockIds, _lockId);
  }

  function _replaceElement(uint256[] storage _lockIds, uint256 _lockId) internal {
    unchecked {
      for (uint256 i = 0; i < _lockIds.length; i++) {
        if (_lockIds[i] == _lockId) {
  ...
  function setProtocolAdminFeeRatio(uint256 _protocolAdminFeeRatio) external onlyProtocolAdmin {
    ...
    if (vaults.length == 0) require(!(_protocolAdminFeeRatio > 100 * 1000), 'totalFeeIsOver100%');
    require(!_exceeds, 'totalFeeIsOver100%');
    protocolAdminFeeRatio = _protocolAdminFeeRatio;
  }
  */
  function setProtocolAdminFeeRatio(uint256 _protocolAdminFeeRatio) external onlyProtocolAdmin {
    require(_protocolAdminFeeRatio <= 10 * 1000, 'protocolAdminFee>10%');
    protocolAdminFeeRatio = _protocolAdminFeeRatio;
  }
  ...
  function rent(
    ...
    _calcFee(_lockId, _lend, lendRent[_lockId], _rentalDurationByDay, _amount);
  ...
  function _calcFee(
    ...
    lenderBenefit[_lockId][_paymentToken] += _lenderBenefit;
    collectionOwnerBenefit[_lend.vault][_paymentToken] += _collectionOwnerBenefit;
    protocolAdminBenefit[_paymentToken] += _protocolAdminBenefit;
  }
  ...
  function checkAvailability(uint256 _lockId) external view returns (bool) {
  ...
  function _mintONft(uint256 _lockId) internal {
    ...
    if (_originalNftAddress.is721()) {
      ...
      require(msg.sender == IERC721(_originalNftAddress).ownerOf(_tokenId), 'notOwnerAddr');
    }

    if (_originalNftAddress.is1155()) {
      uint256 _balance = IERC1155(_originalNftAddress).balanceOf(msg.sender, _tokenId);
      require(_balance >= _amount, 'notEnoughTokens');
    }
...
library Calculator {
  ...
  function fee(
    address _vaultAddress,
    IMarket.LendRent memory _lendRent,
    ...
    )
  {
     ...
     */
    IMarket.Lend memory _lend = _lendRent.lend;

    uint256 _rentalFee = _lend.dailyRentalPrice * _amount * _rentalDurationByDay; // dailyRentalPrice per Unit * Rental Amount * rentalDuration(days)
    uint256 _adminFee = (_rentalFee * _adminFeeRatio) / E5;
    uint256 _collectionOwnerFee = (_rentalFee * IVault(_vaultAddress).collectionOwnerFeeRatio()) /
      E5;
    _paymentToken = _lend.paymentToken;
    if (_lend.lockStartTime < _lend.lockExpireTime)
      // If the lockDuration is Zero, fee is sent to lender this time
      _lenderBenefit = _rentalFee - _adminFee - _collectionOwnerFee;
    _collectionOwnerBenefit = _collectionOwnerFee;
    _protocolAdminBenefit = _adminFee;
