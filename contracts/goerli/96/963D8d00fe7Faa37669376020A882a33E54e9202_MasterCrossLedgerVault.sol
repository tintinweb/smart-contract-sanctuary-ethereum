...
import "ECDSA.sol";
import "CrossLedgerPortfolioScore.sol";
...
contract MasterCrossLedgerVault is CrossLedgerVault, ERC20Burnable {
    ...
    CrossLedgerPortfolioScore oracle;

    mapping(uint256 => uint256) public chainIdToGasLimit;
    mapping(address => mapping(uint256 => address)) public tokensAddresses;
    ...
    constructor(
        ...
        IRootVaultZap _zap,
        address _lzEndpoint,
        CrossLedgerPortfolioScore _oracle,
        string memory _name,
    ...
    function _setTransferAsCompleted(bytes32 transferId) internal {
    ...
    {
        _setTransferAsCompleted(transferId);
    }
    ...
    function updateTokenAddress(
        ...
        tokensAddresses[asset][chainId] = dstAsset;
    }
    ...
    function startNextOperation() public payable {
		require(operationsQueue.front < operationsQueue.ids.length, "no operations to start");
		require(!operationsQueue.isBusy, "there is another task in progress");
        uint256 opId = operationsQueue.ids[operationsQueue.front];
		...
		operationsQueue.isBusy = true;
    }

    function _completeOperation(uint256 opId) internal {
        Operation storage operation = operations[opId];
        if (operation.opType == OperationType.REDEEM) {
            // no need to complete, funds already routed to user
        } else if (operation.opType == OperationType.DEPOSIT) {
            (, , address beneficiary) = abi.decode(
                operation.params,
                (address, uint256, address)
            );
            _completeDeposit(opId, beneficiary);
        }
		emit OperationCompleted(opId);
        operationsQueue.front++;
		operationsQueue.isBusy = false;
    }
    ...
    function _addOperationToQueue(
        ...
        operationsQueue.ids.push(opId);
        bool canProcessNow = (operationsQueue.front + 1) ==
            operationsQueue.ids.length;
        if (canProcessNow) {
            startNextOperation();
        }
    }
    ...
    function totalPortfolioScore() public view returns (uint256 total) {
        total = oracle.portfolioScore(block.chainid);
        for (uint256 i = 0; i < chains.length(); i++) {
            uint256 chainId = chains.at(i);
            total += oracle.portfolioScore(chainId);
        }
    }
    ...
    function _startDeposit(
        ...
        totalAssetsBeforeDeposit[opId] += rootVault.convertToAssets(rootVault.balanceOf(address(this)));
        uint256 totalScore = totalPortfolioScore();
        rootVaultZap.deposit(
            asset,
            (value * oracle.portfolioScore(block.chainid)) / totalScore
        );
        totalAssetsAfterDeposit[opId] += rootVault.convertToAssets(rootVault.balanceOf(address(this)));
		uint sentTransfers;
        for (uint256 i = 0; i < chains.length(); i++) {
            uint256 chainId = chains.at(i);
            ...
            bytes32 transferId = adapter.sendAssetsToRootVault(
                amountToSend,
                asset
            );
            transferIdToOpId[transferId] = opId;
    ...
    function _completeDeposit(uint256 opId, address beneficiary) internal {
		...
		if (totalSupply() == 0) {
			...
			pricePerToken = totalAssetsBefore * (10 ** decimals()) / totalSupply();
		}
    ...
    function _requestRedeem(
        ...
        uint256 shares,
        uint256 totalSupply,
        address receiver, // address(0) in case of redeemToRootVault
        address asset
    ) internal {
        uint16 lzChainId = chainIdToLzChainId[chainId];
        address dstAsset = tokensAddresses[asset][chainId];
    ...
    function _startRedeem(
		...
		uint256 amountToRedeem = shares * rootVault.balanceOf(address(this)) / _totalSupply;
		if (amountToRedeem > 0) {
			uint256 assets = rootVaultZap.redeem(
				asset,
				amountToRedeem
			);
...
contract CrossLedgerPortfolioScore is Ownable
{
	event ScoreDataUpdated(uint256 indexed chainId, uint256 score);

	mapping (uint256 => uint256) public portfolioScore;

	function setPortfolioScore(uint256 chainId, uint256 score) public onlyOwner
	{
		portfolioScore[chainId] = score;
		emit ScoreDataUpdated(chainId, score);
	}

	function setPortfolioScoreBatch(uint256[] memory chains, uint256[] memory scores) public onlyOwner
	{
		require(chains.length == scores.length, "chains and scores length mismatch");
		for (uint i = 0; i < chains.length; i++) {
			portfolioScore[chains[i]] = scores[i];
			emit ScoreDataUpdated(chains[i], scores[i]);
		}
	}
