...
contract MojoTrolls is ERC721A, IERC2981, IERC4494, ReentrancyGuard, Ownable {
    using Strings for uint256;

    // declare all variables that are not constant/immutable as private
    // and set a getter/(setter) for them makes the contract safer,
    // since state variables could be shadowed in the constructor of inheriting contract
    // which potentially leads to misuse. Modifiers (override) in functions help us in that
    ...
    bool private _publicFlag;
    bool private _presaleFlag;
    bool private _freeMintFlag;
    uint256 private _publicPrice;
    uint256 private _presalePrice;
    uint32 private _publicMaxPerAddress;
    uint32 private _presaleMaxPerAddress;
    uint32 private _freeMintMaxPerAddress;
    uint32 private _maxPreSaleSupply;
    uint32 private _maxFreeMintSupply;
    uint32 private _royaltyRate;
    uint32 private _toReveal;
    ...
    event RevealNumberUpdated(uint32 amount);
    ...
    function publicFlag() external view returns (bool) {
        return _publicFlag;
    }

    function presaleFlag() external view returns (bool) {
        return _presaleFlag;
    }

    function freeMintFlag() external view returns (bool) {
        return _freeMintFlag;
    }
    ...
    function publicMaxPerAddress() external view returns (uint256) {
        return _publicMaxPerAddress;
    }

    function presaleMaxPerAddress() external view returns (uint256) {
        return _presaleMaxPerAddress;
    }

    function freeMintMaxPerAddress() external view returns (uint256) {
        return _freeMintMaxPerAddress;
    }
    ...
    function switchPublicFlag() public onlyOwner {
        bool publicFlag_ = _publicFlag;

        _publicFlag = !publicFlag_;

        emit FlagSwitched(publicFlag_);
    }

    function switchPresaleFlag() public onlyOwner {
        bool presaleFlag_ = _presaleFlag;

        _presaleFlag = !presaleFlag_;

        emit FlagSwitched(presaleFlag_);
    }

    function switchFreemintFlag() public onlyOwner {
        bool freeMintFlag_ = _freeMintFlag;

        _freeMintFlag = !freeMintFlag_;
    ...
    function setRevealNumber(uint32 amount) public onlyOwner {
        _toReveal = amount;
    ...
    {
        require(_publicFlag, "Public sale not active");
        require(msg.value >= _publicPrice * quantity, "Mint: Insufficient ETH");

        if (_publicMaxPerAddress > 0) {
            require(
                quantity + publicMinted[msg.sender] <= _publicMaxPerAddress,
    ...
    function presaleMint(
    ...
    ) public payable nonReentrant isSufficientSupply(quantity) {
        address sender = msg.sender;

        require(_presaleFlag, "Presale mint is not active");
        require(
            msg.value >= _presalePrice * quantity,
        ...
        if (_presaleMaxPerAddress > 0) {
            require(
                quantity + presaleMinted[sender] <= _presaleMaxPerAddress,
                "Presale mint: Amount exceeds max per address"
            );
        }

        presaleMinted[sender] += quantity;
        _safeMint(to, quantity, "");
    ...
    function freeMint(
        ...
        require(_freeMintFlag, "Free mint is not active");
        require(
            MerkleProof.verify(
                proof,
                _freeMintMerkleRoot,
        ...
        if (_freeMintMaxPerAddress > 0) {
            require(
                quantity + freeMintMinted[sender] <= _freeMintMaxPerAddress,
                "Free mint: Amount exceeds max per address"
            );
        }

        freeMintMinted[sender] += quantity;
        _safeMint(to, quantity, "");
    }
