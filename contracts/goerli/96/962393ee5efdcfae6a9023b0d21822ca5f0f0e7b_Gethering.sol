...
{    
    uint public constant COLLECION_SIZE = 256;
    ...
    function metadataJson(uint token) public pure returns(string memory metadata)
    {
        ...
        metadata = Base64.encode(
            abi.encodePacked(
                "{\"image\": \"", image, "\", \"name\":\"Piece #", Strings.toString(token) ,"\", \"description\":\"Piece of Gethering.\",\"attributes\":",attributesJson,"}"));
    }
    ...
    function getContentBytes(uint token) public pure returns(bytes memory content, string[] memory attributes)
    {
        ...
        setBackground(content, 0xff, 0xff);

        uint number = 1 << (token - 1);
        uint x = 0;
        uint y = 0;

        while (number > 1 << 16) {number = number >> 16; x++;}
        while (number > 1) {number = number >> 1; y++;}

        addPixel(content, x, y, 0xef, 0x79);

        attributes = new string[](2);
        attributes[0] = string(abi.encodePacked("X: ", x + 1));
        attributes[1] = string(abi.encodePacked("Y: ", y + 1));
    }
    ...
    function addPixel(bytes memory content, uint x, uint y, bytes1 c1, bytes1 c2) private pure
    {
