...
contract MetaFlyers is IMetaFlyers, ERC721AQueryable, IERC2981, Ownable, Pausable, ReentrancyGuard {
    ...
    function lock(uint256 tokenId, address user) external onlyAdmin {
        ...
        _lockedByTokenId[tokenId] = IMetaFlyers.Locked({
            ...
            lockTimestamp: uint128(block.timestamp)
        });
    ...
    function refreshLock(uint256 tokenId) external onlyAdmin {
        ...
        myLock.lockTimestamp = uint128(block.timestamp);
    ...
    function getAllLockedTokens(address owner) external view returns (uint256[] memory) {
