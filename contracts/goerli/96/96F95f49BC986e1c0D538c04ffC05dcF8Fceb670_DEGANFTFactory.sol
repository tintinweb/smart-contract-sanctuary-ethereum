...
contract DEGANFTContract is ERC721, Ownable {
    ...
    mapping(uint256 => address) public tokenByNFT;
    mapping(uint256 => address) public NFTCollectionByNFT;
    ...
    constructor(
        ...
        onChainMetadata = _onChainMetadata;
    ...
    function mint(
        ...
        if (mintLimitEnabled) {
            require(_tokenIds.current() <= maxMintId, "max mint limit reached");
        }
        ...
        if (_erc20Token != address(0)) {
            tokenByNFT[newItemId] = _erc20Token;
        }
        if (_nftCollection != address(0)) {
            NFTCollectionByNFT[newItemId] = _nftCollection;
        }

        emit Minted(newItemId, _tokenURI, _to);
        return newItemId;
    }
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "./DEGANFTContract.sol";

import "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import "./interfaces/IDEGANFTContract.sol";
...
contract DEGANFTFactory is Ownable {
    ...
    function createNFTContract(
        ...
        DEGANFTContract nft = new DEGANFTContract(
            _name,
            _symbol,
            _mintLimitEnabled,
            _maxMintId,
        ...
        collectionsByOwner[_owner].push(address(nft));

        emit ContractCreated(_msgSender(), address(nft));
        return address(nft);
    }

    function getTokenURI(address _collection, uint256 tokenId)
        public
        view
        returns (string memory)
    {
        return IDEGANFTContract(_collection).tokenURI(tokenId);
    }
}
...
interface IDEGANFTContract is IERC721Metadata {
