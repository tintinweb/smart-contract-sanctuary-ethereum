    ...
    ISuperGalatic
{
     ...
     * @param nftId address of the pool
     */
    event updateBodyPart(uint256 nftId, uint256 bodyType, uint256 bodyLevel, uint256 bodyRarity);
     ...
     */
    function updateBody(uint256 bodyType, uint256 tokenId) external override onlyFactory {
        BodyInfo storage info = nftInfos[tokenId][bodyType];
        require(_ableToUpdate(info.level, info.rarity), Errors.CANNOT_UPDATE);
        info.level = info.level + 1;
        emit updateBodyPart(tokenId, bodyType, info.level, uint256(info.rarity));
    }
    ...
    function _ableToUpdate(uint256 originalLevel, Rarity rarity) private pure returns (bool) {
        if (rarity == Rarity.Purple || rarity == Rarity.Blue) {
