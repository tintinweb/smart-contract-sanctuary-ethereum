...
contract UpdatedERC721Staking is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    mapping(address => uint256) public noOfStakedTokens;
    mapping(uint256 => address) public stakedAssetOwner;
    ...
    function stake(uint256[] memory tokenIds) external nonReentrant whenNotPaused updateReward(msg.sender) {
        require(rewardRate != 0, "RewardRate is 0, first set rate by calling setRewardAmount");
        ...
        for (uint256 i = 0; i < tokenIds.length; i += 1) {
            ...
            stakedAssetOwner[tokenIds[i]] = msg.sender;
        }
        _stake(amount);
    ...
    function withdraw(uint256[] memory tokenIds) internal nonReentrant updateReward(msg.sender) {
        require(tokenIds.length != 0, "Staking: No tokenIds provided");
        ...
        for (uint256 i = 0; i < tokenIds.length; i += 1) {
            ...
            require(
                stakedAssetOwner[tokenIds[i]] == msg.sender,
                "Staking: Not the staker of the token or the token is not staked"
            );
            ...
            amount += 1;
            // Cleanup stakedAssetOwner for the current tokenId
            stakedAssetOwner[tokenIds[i]] = address(0);
        }
        _withdraw(amount);
    ...
    function unstake(uint256[] memory tokenIds) external {
        require(tokenIds.length != 0, "input tokenId cannot be 0");
        withdraw(tokenIds);
        getReward();
    }
    ...
    function _stake(uint256 _amount) internal {
        totalStaked += _amount;
        noOfStakedTokens[msg.sender] += _amount;
    }
    ...
    function setRewardAmount(uint256 reward) external onlyOwner updateReward(address(0)) {
        require(reward != 0, "input reward amount cannot be 0");
        require(rewardsDuration > 0, "rewardsDuration not set, first set rewardsDuration");
        if (block.timestamp >= periodFinish) {
            require(reward >= rewardsDuration, "Reward amount should be greater than or equal to rewardsDuration");
        ...
        require(rewardRate <= balance / rewardsDuration, "Staking: Provided reward too high. Less reward tokens available");
    ...
    function setRewardsDuration(uint256 _rewardsDuration) external onlyOwner {
        require(_rewardsDuration != 0, "input _rewardsDuration cannot be 0");
        require(
