...
contract Token is Ownable, ERC721AQueryable, ReentrancyGuard, PaymentSplitter {
     ...
     * Airdrop Mint
     */
    function airdropMint(uint256 quantityPerAddress, address[] memory recipients) external onlyOwner {
        require(
        ...
        require(
            totalSupply() + (quantityPerAddress * recipients.length) <= _totalAmount,
            "Exceeds maximum supply"
        );

        for(uint i = 0; i < recipients.length; i++) {
            if(recipients[i] != address(0)) {
                _safeMint(recipients[i], quantityPerAddress);
            }
        }
    }
