...
contract Auction is
    ...
    ERC721Holder,
    ERC1155Holder,
    EIP712("InstantLiquidityVaults", "1")
{
    ...
    bytes32 private constant BID_MESSAGE =
        keccak256(
            "BidMessage(address bidder,uint256 price,uint256 saleId,uint256 nonce)"
        );
    ...
    struct AuctionSaleInfo {
        ...
        address currency; // Purchase Token
        uint256 borrowedId;
    }
    ...
    function requestSale(
        ...
        require(_tokenId > 0, "Sale: INVALID_TOKENS");
        require(_endTime > block.timestamp, "Sale: INVALID_TIME");
        require(_currency != address(0), "Sale: INVALID_CURRENCY");
        require(_borrowedId != 0, "Sale: INVALID_BORROWED_ID");
     ...
     */
    function updateSale(uint256 _saleId, uint256 _endTime) external onlyOwner {
        require(_endTime > block.timestamp, "Sale: INVALID_TIME");
    ...
    function cancelSale(uint256 _saleId) external onlyOwner {
        ...
        if (
            IERC165(_saleInfo.nftToken).supportsInterface(INTERFACE_ID_ERC1155)
        ) {
            IERC1155(_saleInfo.nftToken).safeTransferFrom(
                address(this),
                msg.sender,
                _saleInfo.tokenId,
                1,
    ...
    function endAuction(
        ...
        bytes32 structHash = keccak256(
            abi.encode(BID_MESSAGE, buyer, price, saleId, nonce)
        );
        ...
        require(price >= _saleInfo.minPrice, "Sale: INVALID_PRICE");

        IERC20(_saleInfo.currency).safeTransferFrom(_buyer, vault, _price);
        ...
        IInstantLiquidityVaults(vault).repayWithAuction(
            _saleInfo.borrowedId,
            price
        );
        emit Purchased(_buyer, _saleId, _price);
        delete saleInfo[_saleId];
    }
