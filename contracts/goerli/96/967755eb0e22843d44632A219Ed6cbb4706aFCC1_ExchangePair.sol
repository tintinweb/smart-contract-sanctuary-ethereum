                ...
                if(!isCancelled(__hash)) {
                    if(!isCompleted(__hash)) { 
                        ...
                        if (tAmt_[3] > tAmt_[0]) { 
                            ...
                            _transfer(tokens[1], orderByHash[__hash].maker, tAmt_[1] - fees[1]);
                            updateOrder(__hash, hash_, tAmt_[1], tAmt_[0], fees[1], LibOrder.Status.PARTIALCOMPLETED);

                            _transfer(tokens[0], orderByHash[hash_].maker, tAmt_[0] - fees[0]);
                            ...
                            if( _tAmounts[0]!=0) {
                                ...
                                _transfer( tokens[0], orderByHash[hash_].maker, tAmt_[3] - fees[0] );
                                updateOrder(hash_,__hash,tAmt_[3],tAmt_[2],fees[0],LibOrder.Status.PARTIALCOMPLETED);

                                _transfer( tokens[1],orderByHash[__hash].maker,tAmt_[2] - fees[1]);
