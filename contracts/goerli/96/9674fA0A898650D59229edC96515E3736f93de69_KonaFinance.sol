...
contract KonaFinance is KonaOracle, AccessControl {
    address brzToken = 0x6Ea73313Fa986A35FbC898997F3f3Cf0DefB4FEC;
    ...
    event ProviderEnabled(uint256 indexed providerID, string name, string apiURL);
    ...
    event ProviderURLSet(uint256 indexed providerID, string apiURL);
    ...
    function invest(uint256 _providerID, uint256 _loanID, address _payToContract) external {
        ...
        string memory _url = string(abi.encodePacked(string.concat(providers[_providerID].apiURL, Strings.toString(_loanID), providers[_providerID].apiSuffix)));
    ...
    function loanChecked(uint256 _providerID, uint256 _loanID, uint256 _result) public {
        if (_result == 1) {
    ...
    function enableProvider(uint256 _providerID, string calldata _name, string calldata _apiURL, string calldata _apiSuffix, uint256[] calldata _feeAmounts, address[] calldata _feeWallets) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit ProviderEnabled(_providerID, _name, _apiURL);
    }
    ...
    function updateProvider(uint256 _providerID, string memory _apiURL, string memory _apiSuffix, bool _autoConfirm, uint256 _maxLoanAmount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit ProviderURLSet(_providerID, _apiURL);
    }
...
contract KonaOracle is ChainlinkClient, ConfirmedOwner {
    ...
    address linkToken = 0xe74FC81dC8299223c6E25766F95300248472c3C4; // 0xb0897686c545045aFc77CF20eC7A532E3120E0F1; 
