...
//import "./zodiac/Roles.sol";

contract GuildSafe {

    uint256 nonce;
    address public safeMaster;
    address public rolesMaster;

    constructor(
        address _safeMaster,
        address _rolesMaster
    ) {
        safeMaster = _safeMaster;
        rolesMaster = _rolesMaster;
    }

    function getSaltNonce() public returns (uint256) {
        uint256 salt = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty,  msg.sender, nonce)));
        nonce++;
        return salt;
    }

    function createSafe() public returns (address safe) {
        // Deploy a new Gnosis Safe using GnosisSafeProxyFactory contract
        ...
        GnosisSafeProxy safeInstance = factory.createProxy(safeMaster, data);
        //GnosisSafeProxy safeInstance = factory.createProxy(address(0xd9Db270c1B5E3Bd161E8c8503c55cEABeE709552), "");

        // Set the safe variable to the address of the new Safe instance
        return address(safeInstance);
    }
    ...
    returns (
        ...
        // Deploy a new Zodiac Role modifier
        ...
        address moduleAddress = factory.deployModule(rolesMaster, initData, salt);
