...
/// @notice This most of the functions in this contract are public callable.
    ...
    INF3Market
{
    ...
    {
        // Call core contract.
        ISwap(swapAddress).cancelListing(_listing, _signature, msg.sender);
    }
    ...
    {
        // Call core contract.
        ISwap(swapAddress).cancelSwapOffer(_offer, _signature, msg.sender);
    }
    ...
    function cancelCollectionOffer(
        ...
        // Call core contract.
        ISwap(swapAddress).cancelCollectionOffer(
            _offer,
            _signature,
            msg.sender
        );
    ...
    function acceptReserveOffer(
        ...
        // Call core contract.
        IReserve(reserveAddress).acceptReserveOffer(
            _listing,
            _listingSignature,
            _offer,
            _offerSignature,
...
interface IReserve {
    ...
    /// @param reserves Reservation info
    ...
    /// @dev Emits when the seller has accepted reservation offer.
    /// @param listing Listing info
    /// @param offer Reservaton offer
    /// @param user Listing owner
    event ReserveOfferAccepted(
        Listing listing,
        ReserveOffer offer,
        address user
    );
    ...
    /// @param positionTokenId Position token id
    /// @param user Seller address
    event Claimed(Listing listing, uint256 positionTokenId, address user);
    ...
    /// @param listing Listing info
    /// @param listingSignature Signature of listing info
    /// @param reserveInfo Reserve details
    /// @param positionTokenId Position token id
    /// @param user Buyer address
    ...
    function payRemains(
        Listing calldata listing,
        bytes memory listingSignature,
        ReserveInfo calldata reserveInfo,
        uint256 positionTokenId,
        address user,
        uint256 value
    ) external;
