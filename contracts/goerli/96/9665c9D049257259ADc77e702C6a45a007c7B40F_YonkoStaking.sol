...
contract YonkoStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    function updateConfig(
        ...
        yonkoNFT = _yonkoNFT;
        configChanges[lastConfigBlock].nextBlock = block.number;
        lastConfigBlock=block.number;
        configChanges[block.number].rewardForRarity = _rewardForRarity;
        rewardToken = _rewardToken;
        feed = _feed;
    ...
    function updateReward(uint256[4] memory _rewardForRarity) external onlyOwner {
        configChanges[lastConfigBlock].nextBlock = block.number;
        lastConfigBlock=block.number;
        configChanges[block.number].rewardForRarity = _rewardForRarity;
    }
    ...
    function _pendingRewards(address user, uint256 tokenId) private view returns(uint256 totalRewards) {
        ...
        do {
            ...
            uint rewardPerPeriod = changes.rewardForRarity[rarity[tokenId - 1] -1];
            uint stakeBlocks = changes.nextBlock != 0? changes.nextBlock - info.staked[tokenId]: block.number - info.staked[tokenId];
            totalRewards += stakeBlocks*rewardPerPeriod/rewardPeriod;
            lastBlock = changes.nextBlock;
        }
        while(lastBlock != 0);
    }
