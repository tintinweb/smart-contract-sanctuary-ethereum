...
interface ISqrl721Core is IERC721 {
  ...
  function injectToken(address _sender, uint _tokenId, string calldata _seed, string calldata _traits) external;
}
...
contract SqrlMarketplace is IERC721Receiver, Ownable {
  ...
  uint public listInjectionFee = 2000000000000000;
  ...
  event ArtistSharePaid(uint tokenId, uint salePrice, uint royaltyAmount);
  ...
  function fillListing(uint _projectId, uint _tokenId) external payable {
    ...
    _payPipeline(_projectId, _tokenId, _listing.listingPrice, _listing.nftOwner);
    ...
    _tokenContract.updateTokensOwned(_listing.nftOwner, _listing.nftOwner, msg.sender, _tokenId, 0);
  ...
  function acceptOffer(uint _projectId, uint _tokenId, address _buyer) external {
    ...
    if (_listing.nftOwner != msg.sender) {
      ...
      require(_listing.nftOwner == msg.sender, "Only the owner can accept the offer");
    }

    _payPipeline(_projectId, _tokenId, _offer.price, owner);
    ...
    _tokenContract.updateTokensOwned(owner, msg.sender, _buyer, _tokenId, 1);
  ...
  function listInjection(uint _price, string calldata _seed) external payable {
    ...
    payable(commissionAddress).transfer(_price);

    emit ListInjectionReturnSeed(_seed);
  }

  function buyInjection(uint _projectId, address _address, uint _price, uint _tokenId, string calldata _seed, string calldata _traits) external payable {
    Project memory _project = _tokenContract.getProject(_projectId);

    uint sqrlAmount = _price.mul(3).div(100);

    payable(commissionAddress).transfer(sqrlAmount);

    uint artistAmount = _price.mul(_project.royaltyPercentage).div(100);

    /*
    for (uint i = 0;i < _project.royaltySalesAddresses.length;i++) {
      payable(_project.royaltySalesAddresses[i]).transfer(artistAmount.mul(_project.royaltySalesShares[i]).div(1000));
    }
    */
    payable(_project.royaltySalesAddress).transfer(artistAmount);

    uint remainingAfterSqrl = _price.sub(sqrlAmount);
    uint remainingAfterRoyalties = remainingAfterSqrl.sub(artistAmount);

    payable(_address).transfer(remainingAfterRoyalties);

    emit BuyInjectionReturnTokenId(_tokenId);

    _tokenContract.injectToken(msg.sender, _tokenId, _seed, _traits);
  }
  ...
  returns (uint[6] memory splits) {
    ...
    splits = [
      sqrlRoyalty[_tokenTurnover],
      collector1Royalty[_tokenTurnover],
      collector2Royalty[_tokenTurnover],
      collector3Royalty[_tokenTurnover],
      collector4Royalty[_tokenTurnover],
      collector5Royalty[_tokenTurnover]
    ];
    return splits;
  }

  function _payPipeline(uint _projectId, uint _tokenId, uint _paymentAmount, address _currentOwner) internal {
    Project memory _project = _tokenContract.getProject(_projectId);

    uint[6] memory splits = getRoyaltySplit(_tokenId);

    uint _artistAmount = _paymentAmount.mul(_project.royaltyPercentage).div(100);

    //pay royalty to creator/artist
    if (_artistAmount > 0) {
      /*
      for (uint i = 0;i < _project.royaltySalesAddresses.length;i++) {
        payable(_project.royaltySalesAddresses[i]).transfer(_artistAmount.mul(_project.royaltySalesShares[i]).div(1000));
      }
      */
      payable(_project.royaltySalesAddress).transfer(_artistAmount);

      emit ArtistSharePaid(_tokenId, _paymentAmount, _artistAmount);
    }
    ...
    //pay fee to Sqrl
    if (splits[0] > 0) {
      _amount = _paymentAmount.mul(splits[0]).div(100_000);
    ...
    if (splits[1] > 0) {
      _amount = _paymentAmount.mul(splits[1]).div(100_000);
      if (tokenTurnover[_tokenId].length < 6) {
        payable(tokenTurnover[_tokenId][0]).transfer(_amount);
      } else {
        payable(tokenTurnover[_tokenId][tokenTurnover[_tokenId].length - 5]).transfer(_amount);
      }
    ...
    if (splits[2] > 0) {
      _amount = _paymentAmount.mul(splits[2]).div(100_000);
      if (tokenTurnover[_tokenId].length < 6) {
        payable(tokenTurnover[_tokenId][1]).transfer(_amount);
      } else {
        payable(tokenTurnover[_tokenId][tokenTurnover[_tokenId].length - 4]).transfer(_amount);
      }
    ...
    if (splits[3] > 0) {
      _amount = _paymentAmount.mul(splits[3]).div(100_000);
      if (tokenTurnover[_tokenId].length < 6) {
        payable(tokenTurnover[_tokenId][2]).transfer(_amount);
      } else {
        payable(tokenTurnover[_tokenId][tokenTurnover[_tokenId].length - 3]).transfer(_amount);
      }
    ...
    if (splits[4] > 0) {
      _amount = _paymentAmount.mul(splits[4]).div(100_000);
      if (tokenTurnover[_tokenId].length < 6) {
        payable(tokenTurnover[_tokenId][3]).transfer(_amount);
      } else {
        payable(tokenTurnover[_tokenId][tokenTurnover[_tokenId].length - 2]).transfer(_amount);
      }
    ...
    if (splits[5] > 0) {
      _amount = _paymentAmount.mul(splits[5]).div(100_000);
      if (tokenTurnover[_tokenId].length < 6) {
        payable(tokenTurnover[_tokenId][4]).transfer(_amount);
      } else {
        payable(tokenTurnover[_tokenId][tokenTurnover[_tokenId].length - 1]).transfer(_amount);
      }
  ...
  function updateRoyaltyAddress(address _commissionAddress) external onlyOwner {
    commissionAddress = _commissionAddress;
  }

  function updateListInjectionFee(uint _listInjectionFee) external onlyOwner {
