...
contract supplychain {
    ...
    function getMedWithId(uint256 _id)
        public
        view
        returns (
    ...
    function setMed(
        ...
        require(manufacturAccessMapping[msg.sender] == true, "Sender is not manufacturer");

        require(distributerAccessMapping[_to] == true, "Reciever is not valid Distributer");
    ...
    function acceptdist(
        ...
        require(manufacturAccessMapping[_from] == true, "Sender is not manufacturer");
        require(distributerAccessMapping[msg.sender] == true, "Reciever is distributer");

        for (uint256 i = sid; i <= lid; i++) {
            require(checkEx(i) == true, "medicine expired");
            require(_from == last_accessMapping[i], "medicine came from someone else");
            require(
    ...
    function setDistDetails(
        ...
        require(distributerAccessMapping[msg.sender] == true, "NOt a distributer");
    ...
    function setRetailDetails(
        ...
        for (uint256 i = sid; i <= lid; i++) {
            require(checkEx(i) == true, "medicine expired");
            require(_dist == last_accessMapping[i], "medicine came from someone else");
            require(soldMapping[i] == false, "medicine already sold");
            require(
                msg.sender == next_accessMapping[i],
                "Unauthorized to access this medicine"
    ...
    function sellWithId(uint256 _id) public {
