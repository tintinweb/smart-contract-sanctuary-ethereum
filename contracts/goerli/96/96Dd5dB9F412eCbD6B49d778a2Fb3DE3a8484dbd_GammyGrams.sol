...
contract GammyGrams is ERC721A, MultisigOwnable {
    using Strings for uint256;

    uint constant public TOKEN_LIMIT = 9;// 10e3;
    ...
    function mintFromSale(uint gammiesToMint) public payable {
        ...
        require(gammiesToMint <= 5, "Only up to 5 gammies can be minted at once");
        uint cost;
        unchecked {
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        if (onIPFS) {
            return string(abi.encodePacked(baseURI, (id+1).toString(), ".json"));
        } else {
            return string(abi.encodePacked(baseURI, (id+1).toString()));
        }
    }
}
