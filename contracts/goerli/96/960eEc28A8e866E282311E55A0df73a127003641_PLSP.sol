...
contract PLSP is ERC20, Ownable {
    ...
    function performAirdrop() external returns (uint256) {
        ...
        if (!isAirdropping) {
            // Start a new airdrop
            totalTokenHolding = totalUserBalance();
            lastAirdropTime = block.timestamp;
    ...
    function _determineLotteryWinner(uint256 minimum) internal view returns (address) {
        ...
        int256 winnerValue = int256(randomNumber % totalUserBalance());
        address winner;

        for (uint256 i = 0; i < tokenHolders.length; i++) {
            uint256 balance = balanceOf(tokenHolders[i]);
            winnerValue -= int256(balance);
    ...
    function testWinnerPick() external view returns (uint256, address[] memory) {
        uint256 total;
        address[] memory users = new address[](100000);
