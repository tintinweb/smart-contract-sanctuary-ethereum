...
// The ProxyBot renderer is used to render the ProxyBot token, swappable as future requirements dictate.
// The metadata for a token is all on-chain so it can immedaitly reflect the state of a token,
// but the images are off-chain. No need to get tricky with that here.
contract ProxyBotRenderer {

  string public baseUri;
  address proxyBotContractAddress;

  constructor(address _proxyBotContractAddress, string memory _baseUri) {
    baseUri = _baseUri;
  ...
  function tokenURI(uint256 _tokenId) external view returns (string memory) {
    ...
    if(!hasSpecialEdition){
      editionForJson = 'Standard V1';
    }

    string memory json = Base64.encode(
      bytes(
        string.concat('{"name": "Turf Proxy Bot", "description": "Turf Proxy Bot Description", "image": "',
  ...
  function imageForTokenID(uint256 _tokenId, ProxyBotConfig.Status _status, bool _hasSpecialEdition, string memory _editionLabel) public view returns (string memory) {
    ...
    if(_hasSpecialEdition){
      ...
      baseImageName = string(abi.encodePacked(baseImageName, '-', specialEdition));
    }
