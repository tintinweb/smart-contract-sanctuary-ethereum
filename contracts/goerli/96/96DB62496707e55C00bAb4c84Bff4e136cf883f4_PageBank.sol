    ...
    IPageBank
{
    ...
    uint256 public FOR_MINT_GAS_AMOUNT;
    uint256 public FOR_BURN_GAS_AMOUNT;
    uint256 public MAX_POST_CREATE_FEE;
    uint256 public MAX_COMMENT_CREATE_FEE;
    ...
    uint256 constant ALL_PERCENT = 10000;
    ...
    uint64 public defaultCreatePostOwnerFee;
    uint64 public defaultCreatePostCreatorFee;
    uint64 public defaultRemovePostOwnerFee;
    uint64 public defaultRemovePostCreatorFee;

    uint64 public defaultCreateCommentOwnerFee;
    uint64 public defaultCreateCommentCreatorFee;
    uint64 public defaultRemoveCommentOwnerFee;
    uint64 public defaultRemoveCommentCreatorFee;
    ...
    event SetMaxPostCreateFee(uint256 oldValue, uint256 newValue);
    event SetMaxCommentCreateFee(uint256 oldValue, uint256 newValue);
    ...
    modifier notAddress0(address _address) {
        require(_address != address(0), "PageBank: Address 0 is not valid");
        _;
    }
        ...
        initializer
    {
        ...
        FOR_MINT_GAS_AMOUNT = 145000;
        FOR_BURN_GAS_AMOUNT = 95000;
        MAX_POST_CREATE_FEE = 9000;
        MAX_COMMENT_CREATE_FEE = 9000;
    ...
    function updatePostFee(
        ...
        require(newCreatePostOwnerFee + newCreatePostCreatorFee <=
                MAX_POST_CREATE_FEE, "The total amount of fees exceeded the maximum");
    ...
    function updateCommentFee(
        ...
        require(newCreateCommentOwnerFee + newCreateCommentCreatorFee <=
                MAX_COMMENT_CREATE_FEE, "The total amount of fees exceeded the maximum");
     ...
     * @param gas Gas used
     * @param mintOwnerTokensToCommunityBalance If tokens needs to be assigned to community balance
     */
    function mintTokenForNewPost(
        ...
        require(_owner != address(0), "PageBank: wrong address");
        require(creator != address(0), "PageBank: wrong address");
        ...
        if (mintOwnerTokensToCommunityBalance) {
            mintCommunityPageToken(communityId, amount, communityFee[communityId].createPostOwnerFee);
        } else {
            mintUserPageToken(_owner, amount, communityFee[communityId].createPostOwnerFee);
        }
        mintUserPageToken(creator, amount, communityFee[communityId].createPostCreatorFee);
        mintTreasuryPageToken(amount);

        emit MintForPost(communityId, _owner, creator, amount);
    }
    ...
    function mintTokenForNewComment(
        ...
        mintUserPageToken(_owner, amount, communityFee[communityId].createCommentOwnerFee);
        mintUserPageToken(creator, amount, communityFee[communityId].createCommentCreatorFee);
        mintTreasuryPageToken(amount);

        emit MintForComment(communityId, _owner, creator, amount);
    }
    ...
    function addUpDownActivity(
        ...
        require(postCreator != address(0), "PageBank: wrong address");
        if (isUp) {
    ...
    function burnTokenForPost(
        ...
        burnUserPageToken(_owner, amount, communityFee[communityId].removePostOwnerFee);
        burnUserPageToken(creator, amount, communityFee[communityId].removePostCreatorFee);
        mintTreasuryPageToken(amount);

        emit BurnForPost(communityId, _owner, creator, amount);
    }
    ...
    function burnTokenForComment(
        ...
        burnUserPageToken(_owner, amount, communityFee[communityId].removeCommentOwnerFee);
        burnUserPageToken(creator, amount, communityFee[communityId].removeCommentCreatorFee);
        mintTreasuryPageToken(amount);

        emit BurnForComment(communityId, _owner, creator, amount);
    }
    ...
    {
        require(wallet != address(0), "PageBank: wrong address");
    ...
    function payForPrivacyAccess(uint256 amount, uint256 communityId) external override {
        ...
        if (endPrivacyTime[sender][communityId] > block.timestamp) {
            endPrivacyTime[sender][communityId] += daysCount * 1 days;
        } else {
    ...
    function setDefaultFee(uint256 index, uint64 newValue) external override onlyOwner {
        if (index == 0) {
            require(defaultCreatePostOwnerFee + defaultCreatePostCreatorFee <=
                MAX_POST_CREATE_FEE, "The total amount of fees exceeded the maximum");
        ...
        if (index == 4) {
            require(defaultCreateCommentOwnerFee + defaultCreateCommentCreatorFee <=
                MAX_COMMENT_CREATE_FEE, "The total amount of fees exceeded the maximum");
    ...
    /**
     * @dev Changes the value of the FOR_BURN_GAS_AMOUNT.
     *
     * @param newValue New value for FOR_BURN_GAS_AMOUNT
     */
    ...
    /**
     * @dev Changes the value of the MAX_POST_CREATE_FEE.
     *
     * @param newValue New value for MAX_POST_CREATE_FEE
     */
    function setMaxPostCreateFee(uint256 newValue) external override onlyOwner {
        require(MAX_POST_CREATE_FEE != newValue, "PageBank: wrong value for MAX_POST_CREATE_FEE");
        emit SetMaxPostCreateFee(MAX_POST_CREATE_FEE, newValue);
        MAX_POST_CREATE_FEE = newValue;
    }

    /**
     * @dev Changes the value of the MAX_COMMENT_CREATE_FEE.
     *
     * @param newValue New value for MAX_COMMENT_CREATE_FEE
     */
    function setMaxCommentCreateFee(uint256 newValue) external override onlyOwner {
        require(MAX_COMMENT_CREATE_FEE != newValue, "PageBank: wrong value for MAX_COMMENT_CREATE_FEE");
        emit SetMaxCommentCreateFee(MAX_COMMENT_CREATE_FEE, newValue);
        MAX_COMMENT_CREATE_FEE = newValue;
    }
     ...
     */
    function setToken(address newToken) external override onlyOwner notAddress0(newToken){
    ...
    /**
     * @dev Mints PAGE tokens for community.
     *
     * @param communityId Id of community
     * @param amount Amount of tokens
     * @param ownerFee Fee for operation
     */
    function mintCommunityPageToken(uint256 communityId, uint256 amount, uint256 ownerFee) private {
        uint256 communityAmount = amount * ownerFee / ALL_PERCENT;
        token.mint(address(this), communityAmount);
        communityBalance[communityId] += communityAmount;
    }
