...
contract Vendor is Ownable {
    ...
    function buyTokens() public payable returns (uint256 tokenAmount) {
        ...
        require(
            ...
            "Vendor contract has not enough token"
        );

        (bool sent) = yourToken.transfer(msg.sender, amountToBuy);
        require(sent, "failed to tranfer token to user!!!");
    ...
    function sellTokens(uint256 tokenAmountToSell) public {
        require(tokenAmountToSell > 0, "token less than zero ");
        uint256 userBalance = yourToken.balanceOf(msg.sender);
        require(
            userBalance >= tokenAmountToSell,
            "your balance is less than amt of token to sell"
        );
        ...
        require(
            ...
            "vendor has not enough funds to accept sell request!"
        );
