...
contract FxStateRootTunnel is FxBaseRootTunnel {
    ...
    function getKeyAndData() external view returns(bytes32 key, bytes memory data) {
        (key, data) = abi.decode(input1, (bytes32, bytes));
    }

    function getAccountAndAmount(bytes memory data) external pure returns(address account, uint256 amount) {
