...
contract EveraiMemoryCoreBurner is Ownable {
    IEveraiMemoryCore public everaiMemoryCore;
    IEveraiOriginArchive public everaiOriginArchive;
    address archiveAddress;

    uint256 public minBurnMemoryCores;

    event Link(uint256 _archiveId);
    event BatchBurn(uint256[] _memoryCoreIds);

    constructor(
        address everaiMemoryCoreAddress,
        address everaiOriginArchiveAddress,
        uint256 minBurnMemoryCores_
    ) {
        everaiMemoryCore = IEveraiMemoryCore(everaiMemoryCoreAddress);
        archiveAddress = everaiOriginArchiveAddress;
        everaiOriginArchive = IEveraiOriginArchive(archiveAddress);
        minBurnMemoryCores = minBurnMemoryCores_;
    }
    ...
    function mint(address addr, uint256 numTokensClaimed) external {
        require(archiveAddress == msg.sender, "Authorization required");
        everaiMemoryCore.mint(addr, numTokensClaimed);
    }

    function batchBurnMemoryCore(uint256[] memory ids) external onlyOwner {
        for (uint256 i = 0; i < ids.length; i++) {
            everaiMemoryCore.burn(ids[i]);
        }
        emit BatchBurn(ids);
    }

    function setMinBurnMemoryCores(uint256 minBurnMemoryCores_)
        external
        onlyOwner
    {
        minBurnMemoryCores = minBurnMemoryCores_;
