...
contract GeweemERC721 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable,ERC721RoyaltyUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable,NativeMetaTransactionUpgradeable,ContextMixin {
    ...
    function initialize(string memory name, string memory symbol,uint256 mintPrice, uint saleStartsIn ,string memory baseURI,uint initReleaseCount,uint maxSupply,uint getFreeBuyCount,uint getFree,uint promoBefore,address royaltyReceiver,uint96 royaltyFeeBasePoint) initializer public {
        ...
        for (uint i=0; i<initReleaseCount; i++) {
            safeMint(_msgSender()); 
        }
    }

    function contractURI() public view virtual returns (string memory) {
        return string(abi.encodePacked(_updatedContractURI , "contractMetadata.json"));
    }

    function setContractURI(string memory baseContractURI_) internal onlyRole(MINTER_ROLE) {
        _updatedContractURI = baseContractURI_;
    }
    
    function setBaseURI(string memory baseURI_) internal onlyRole(MINTER_ROLE) {
    ...
    function withdraw() public whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) {
...
import {INativeMetaTransaction} from "./INativeMetaTransaction.sol";


abstract
contract NativeMetaTransactionUpgradeable is EIP712Base,INativeMetaTransaction {
    ...
    function getNonce(address user) public view override returns (uint256 nonce) {
        nonce = nonces[user];
    }
...
contract EIP712Base is Initializable
{
    ...
    function __EIP712_init(string memory name) internal onlyInitializing {
