...
contract CBGZ is ERC721A, ReentrancyGuard, Delegated, DefaultOperatorFilterer {
    using Strings for uint256;
    mapping(address => uint256) public accessList;
    mapping(address => uint256) public phase_2_num_minted;
    mapping(address => uint256) public phase_3_num_minted;
    ...
    // ======== MINTING ========
    function mintBatch1(uint256 quantity) external payable callerIsUser {
        ...
        require(
            _numberMinted(msg.sender) <= accessList[msg.sender],
            "Exceeds Max Claim"
        );
        _safeMint(msg.sender, quantity * 2);
    }

    function mintBatch2(uint256 quantity) external payable callerIsUser {
        ...
        require(totalSupply() < MAX_SUPPLY, "Max supply reached");
        require(
            phase_2_num_minted[msg.sender] <= 15 - (quantity * 2),
            "Exceeds Max Claim"
        );
     ...
     */
    function setWhitelistAddress(address[] calldata accounts, uint256 allowed)
        external
        onlyDelegates
    {
