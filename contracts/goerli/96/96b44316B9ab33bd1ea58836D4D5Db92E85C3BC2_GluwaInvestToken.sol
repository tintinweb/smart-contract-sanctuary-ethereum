    ...
    EthlessTransfer
{
    ...
    function balanceOf(address account)
        ...
        view
        virtual
        override(ERC20Upgradeable, EthlessReservable)
        returns (uint256)
    {
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !isBlacklisted(from),
            "GluwaInvestToken: sender is blacklisted."
        );
        require(
            !isBlacklisted(to),
            "GluwaInvestToken: receiver is blacklisted."
        );
...
contract EthlessReservable is ERC20Upgradeable, SignerNonce {
     ...
     */
    function reservedOf(address account)
        public
        view
        virtual
    ...
    function _reserve(
        ...
        require(
            _reserved[sender][gluwaNonce]._expiryBlockNum == 0,
            "EthlessReservable: the sender used the nonce already"
        );
        require(amount >= 0, "EthlessReservable: invalid reserve amount");
        uint256 total = amount + fee;
        require(
            _unreservedBalance(sender) >= total,
            "EthlessReservable: insufficient unreserved balance"
        );

        _reserved[sender][gluwaNonce] = Reservation(
            amount,
            fee,
            recipient,
            executor,
     ...
     */
    function execute(address sender, uint256 gluwaNonce)
        external
        virtual
        returns (bool success)
    {
        unchecked {
            ...
            require(
                ...
                "Reserveable: reservation does not exist"
            );
            require(
                reservation._executor == _msgSender() || sender == _msgSender(),
                "Reserveable: this address is not authorized to execute this reservation"
            );
            require(
                reservation._expiryBlockNum > block.number,
                "Reserveable: reservation has expired and cannot be executed"
            );
            require(
                reservation._status == ReservationStatus.Active,
                "Reserveable: invalid reservation status to execute"
            );
     ...
     */
    function reclaim(address sender, uint256 gluwaNonce)
        external
        returns (bool success)
    {
        unchecked {
            ...
            require(
                reservation._status == ReservationStatus.Active,
                "Reserveable: invalid reservation status to reclaim"
            );
            if (_msgSender() != reservation._executor) {
                require(
                    _msgSender() == sender,
                    "Reserveable: only the sender or the executor can reclaim the reservation back to the sender"
                );
                require(
                    reservation._expiryBlockNum <= block.number,
     ...
     */
    function _pastReservedOf(address account, uint256 blockNumber)
        internal
        view
        virtual
     ...
     */
    function _unreservedBalance(address account)
        internal
        view
        virtual
        returns (uint256 amount)
    {
        unchecked {
            amount = ERC20Upgradeable.balanceOf(account) - _totalReserved[account];
        }
    }
     ...
     */
    function _checkUnreservedBalance(address from, uint256 amount)
        internal
        view
        virtual
    {
        if (from != address(0)) {
            require(
                ...
                "Reserveable: transfer amount exceeds unreserved balance"
            );
        }
    }
