    ...
    TypeAndVersionInterface
{
    ...
    uint256 private constant MAX_NUM_WORDS = 1000;

    function getMaxNumWords() external pure returns (uint256) {
    ...
    function processVRFOutputs(
        ...
        uint16 numOutputs;
        OutputServed[] memory outputsServed = new OutputServed[](
            vrfOutputs.length
        );
        ...
        assembly {
            // truncate heights
            mstore(outputsServed, numOutputs)
        }
        emit OutputsServed(
            blockHeight,
            msg.sender,
    ...
    function processVRFOutput(
        ...
        if (output.callbacks.length > 0) {
            assembly {
                // truncate dynamic array
                mstore(errorData, errorCount)
            }
    ...
    uint256 private immutable i_beaconPeriodBlocks;
    ...
    uint256 private immutable i_startSlot;

    function getBeaconPeriodBlocks() external view returns (uint256) {
        return i_beaconPeriodBlocks;
    }

    function getStartSlot() external view returns (uint256) {
        return i_startSlot;
    }
        ...
        )
    {
        ...
        uint256 nextBeaconOutputHeight = blockNumber +
            i_beaconPeriodBlocks -
            (blockNumber % i_beaconPeriodBlocks); // period offset
        ...
        uint24 confDelay = ConfirmationDelay.unwrap(confirmationDelayArg);
        uint256 numConfDelays = rp.confirmationDelays.length;
        uint256 i;
        for (i = 0; i < numConfDelays; i++) {
            if (ConfirmationDelay.unwrap(rp.confirmationDelays[i]) == confDelay)
                break;
        }
        if (i >= numConfDelays) {
        ...
        BeaconRequest memory r = BeaconRequest({
            slotNumber: SlotNumber.wrap(uint32(slotNumberBig)),
            confirmationDelay: confirmationDelayArg,
            numWords: numWords,
            requester: msg.sender
        });
    ...
    function finalOutput(
        ...
        uint16 numWords = r.numWords;
        if (numWords > MAX_NUM_WORDS) {
            // Could happen if corrupted quorum submits
            revert TooManyWords(numWords, MAX_NUM_WORDS); // fake callback
        }
    ...
    error IncorrectNumberOfPaymentsInJuels();
    ...
    function batchTransferLink(
        ...
        if (numRecipients == 0 || numRecipients > MAX_NUM_ORACLES)
            revert IncorrectNumberOfRecipients();
        if (numRecipients != paymentsInJuels.length)
            revert IncorrectNumberOfPaymentsInJuels();
        for (
...
{
    LinkTokenInterface internal immutable i_link; // Address of LINK token contract
    AggregatorV3Interface internal immutable i_link_eth_feed; // Address of LINK/ETH feed
    ...
    function getLinkEthFeed() external view returns (AggregatorV3Interface) {
        return i_link_eth_feed;
    }

    function getMaxConsumers() external pure returns (uint16) {
    ...
    function getFeedData() internal view returns (uint256) {
        ...
        (, weiPerUnitLink, , timestamp, ) = i_link_eth_feed.latestRoundData();
