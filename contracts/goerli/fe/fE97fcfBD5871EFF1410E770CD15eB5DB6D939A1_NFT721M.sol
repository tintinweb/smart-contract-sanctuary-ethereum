...
contract MemberAccessControl {
    mapping (address=>bool) memberAccessControl;
    event AccessControlMember(address indexed owner, address indexed operator, bool approved);
    function _setMemberAccessControl(address _address,bool accessed) internal virtual {
        memberAccessControl[_address] = accessed;
        emit AccessControlMember(_address,msg.sender,accessed);
    }

    function getMemberAccessControl(address _address) public virtual view returns(bool) {
       return memberAccessControl[_address];
    }

    modifier isMemberAccessControl(){
        require(memberAccessControl[msg.sender], "access denied");
        _;
    }
}
...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
contract MemberPoints {
    mapping(address => uint256) internal points;
    event MemberPointsEvent(address indexed owner, int256 indexed amount,address indexed operator);
    function _modifyMemberPoints(address _address ,int256 amount) internal virtual {
         require(
            int256(points[_address]) + amount >= 0,
            "amount verify failed"
        );
        points[_address] = uint256(int256(points[_address]) + amount);
        emit MemberPointsEvent(_address,amount,msg.sender);
    }
    function getMemberPoints(address _address) public view returns (uint256) {
...
import {MemberAccessControl} from "./Member/AccessControl.sol";
import {MemberPoints} from "./Member/MemberPoints.sol";


contract NFT721M is ERC721A, DefaultOperatorFilterer,MemberAccessControl,MemberPoints, Ownable {
    ...
    function setMemberAccessControl(address _address,bool accessed) public onlyOwner {
        _setMemberAccessControl(_address, accessed);
    }

    function adminModifyMemberPoints(address _address ,int256 amount) public isMemberAccessControl {
        _modifyMemberPoints(_address,amount);
    }
    function modifyMemberPoints(
        int256 amount,  bytes32 hash,
        bytes memory signature,
        uint256 blockHeight,
        string memory nonce) public  verifyBlock(blockHeight) verifyNonce(nonce)  verifySigner(hash,signature) {
        require(modifyMemberPointsHash(amount, blockHeight, nonce, "foundo_member_points") == hash, "Invalid hash!");
        _modifyMemberPoints(msg.sender,amount);
        nonceMap[nonce] = true;
    }
    ...
    //******Tool******
    function modifyMemberPointsHash (int256 amount, uint256 blockHeight, string memory nonce, string memory code)
    private
    view
    returns (bytes32)
    {
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encodePacked(msg.sender, amount, blockHeight, nonce,code)
                )
            )
        );
        return hash;
