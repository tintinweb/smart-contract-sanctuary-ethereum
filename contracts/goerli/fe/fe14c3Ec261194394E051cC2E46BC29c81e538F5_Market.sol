...
contract Market is ManagedMarketBase, TransferableMarketBase
{    
    using SafeERC20 for IERC20;

    IMarketStates public state;
    IRiskCalculation public calc;
    IBond public bond;
    ...
    function setFees(uint _takerFee, uint _makerFee) external onlyGovernance {
        takerFee = _takerFee;
        makerFee = _makerFee;
    }
    ...
    function premium(uint id) public view override returns (uint) {
        TakerPosition memory pos = allTakerPositions[id];
        uint p = calc.premium( address(token), pos );    
        (int price,) = price();
        return DIVIDER * p * pos.assetAmount / AP / uint(price);
    }

    /// @notice calculate position yield
    function yield(uint id) public view override returns (int) {
        MakerPosition memory pos = allMakerPositions[id];
        return
            (calc.yield(address(token), pos) *
                int(uint(pos.term)) *
                int(pos.stableAmount)) / int(CP);
    }
    ...
    function protect(uint amount, uint16 risk, uint16 term, bool autorenew) 
        public virtual override returns (uint id) {
        // lock BUMP tokens using Bond contract
        uint bumpAmount = bond.takerLock(msg.sender, address(token), amount, risk, term );            
    ...
    function protectNative(
        ...
        // lock BUMP tokens using Bond contract
        uint bumpAmount = bond.takerLock(msg.sender, address(token), msg.value, risk, term );            
        // open position and transfer tokens
        uint id = super.protectNative( risk, term, autorenew );
        // update bumpAmount for position
        allTakerPositions[id].bumpAmount = bumpAmount;
        // mint token for position
        IPositionNFT( config.getNFTTaker(address(token)) ).safeMint(msg.sender,id);     
    ...
    function deposit(uint amount, uint16 risk, uint16 term) public virtual override returns (uint id) {
        // lock BUMP tokens using Bond contract
        uint bumpAmount = bond.makerLock(msg.sender, address(token), amount, risk, term );
        // deposit tokens and open position
        uint id = super.deposit(amount,risk,term);
        // set BUMP tokens for this position
        allMakerPositions[id].bumpAmount = bumpAmount;
        // mint position NFT token
        IPositionNFT( config.getNFTMaker(address(token)) ).safeMint(msg.sender,id);   
...
abstract contract ManagedMarketBase is IMarketManagement, MarketBase {
    using SafeERC20 for IERC20;
    ...
    function rebalance() external virtual onlyArbitrager  override {     
        (int quantity, uint discount)  = IRebalancer(rebalancer).calculate( address(token) );

        if (quantity > 0) {
            ISwap(swap).swapExactTokensForTokens(address(token), uint(quantity), config.getStable(), 0, 0);
        }
        else {
            ISwap(swap).swapExactTokensForTokens(config.getStable(), uint(-quantity), address(token), 0, 0);
        }

        emit Rebalanced();
    }

    /// @notice set rebalancer
    ...
    /// @notice Set rebalancer smart contract
    function setSwap(address _swap) external {
        swap = _swap;

        stable.approve(swap, 0 );
        stable.approve(swap, type(uint256).max );
        token.approve(swap, 0 );
        token.approve(swap, type(uint256).max );        
    }
...
abstract contract TransferableMarketBase is IMarketPositionTransfer, MarketBase {
    ...
    function safeTransferTakerPosition(address from, address to, uint id) external override {
        ...
        console.log("new owner of taker position is %s", to);
...
interface IMarket {
    function price() external view returns (int _price, uint _updatedAt);
...
interface IMarketStates {
    /// @notice get last calculated PAP
    function lastPAP(address token) external view returns (int128);
...
interface IProtocolConfig {
    ...
    /// @notice Get premium multiplier
    function getPremiumMultiplier(
        address token,
        uint16 risk,
        uint16 term
    ) external view returns (int128 multiplier);
...
interface IRiskCalculation {
    function PAP(address token) external view returns (int128 _PAP);

    function epsilon(address token) external view returns (int128);

    function premium(address token, TakerPosition memory pos)
        external
        view
        returns (uint);

    function yield(address token, MakerPosition memory pos)
        external
        view
        returns (int);
}
...
interface IBond {
    ...
    /// @notice locks amount of token in contract
    /// @param _owner - owner of the position
    /// @param marketToken - address of market's token
    ...
    /// @param marketToken - asset token address
    ...
    BACUpgradable
{
    ...
    event Claimed(address account, uint id, uint amount, uint floor);
    event Closed(
        address account,
        uint id,
        uint amount,
    ...
    event Withdrawn(address account, uint id, uint amount, int yield);
    ...
    function protectWithPermit(
        ...
        bytes32 r,
        bytes32 s
    ) external virtual override returns (uint id) {
...
//        AR += posPremium;
        ...
        console.log("Return to taker %s", amountToReturn);
        token.safeTransfer(pos.owner, amountToReturn);

        // calc
        (int _price, ) = price();
        uint claimInStableTokens = 0;
        if (pos.floor > uint(_price)) {
            claimInStableTokens = DIVIDER_STABLE * (pos.floor - uint(_price)) / DIVIDER_ORACLE * pos.assetAmount / DIVIDER;
            stable.safeTransfer(pos.owner, claimInStableTokens );
        }

        delete allTakerPositions[id];

        emit Closed(msg.sender, id, amountToReturn, posPremium, claimInStableTokens);
    }
    ...
    function claim(uint id) external virtual override takerTermEnds(id) {
        ...
        // calc 
        (int _price, ) = price();
        uint posSizeInStable = pos.assetAmount / DIVIDER * uint(_price);
        uint claimAmount = pos.floor > posSizeInStable ? (pos.floor - posSizeInStable) : 0;
        if (claimAmount > 0) {
            pos.floor =  uint(_price);            
            L -= claimAmount;
            stable.safeTransfer(pos.owner, DIVIDER_STABLE * claimAmount / DIVIDER_ORACLE  );
        }

        emit Claimed(msg.sender, id, claimAmount, pos.floor);
    }
    ...
    function deposit(
        ...
        uint amountInOraclePrecision = (amount / DIVIDER_STABLE) *
            DIVIDER_ORACLE;

        CP += amountInOraclePrecision;
        //CR += (fee / DIVIDER_STABLE) * DIVIDER_ORACLE;
        D += amountInOraclePrecision;

        MakerPosition memory pos = MakerPosition({
            owner: msg.sender,
            stableAmount: amountInOraclePrecision,
        ...
        emit Deposited(msg.sender, makerPosIndex, amount, risk, term );
    ...
    function withdraw(uint id) public virtual override {
        ...
        emit Withdrawn(msg.sender, id, pos.stableAmount, _yield);
    ...
    {
        (int _price, ) = price();
        uint AR = token.balanceOf(address(this)) - AP;
        uint CR = stable.balanceOf(address(this)) * DIVIDER_ORACLE / DIVIDER_STABLE - CP;
        return (
            AP,
            (AP * uint(_price)) / DIVIDER,
            (AR * uint(_price)) / DIVIDER,
            CP,
            CR,
            (B * uint(_price)) / DIVIDER,
            L,
            D
        );
    }
...
interface IRebalancer {
    function calculate(address token)
        external
        view
...
contract BACUpgradable is AccessControlUpgradeable {
...
contract MarketStorage {
    ...
    uint public AP; // Asset pool (in tokens with DIVIDER precision)
    //uint public AR; // Asset reserve (in tokens with DIVIDER precision)
    uint public L; // Liability
    ...
    // MAKER DATA
    uint makerPosIndex;
    uint public CP; // Capital pool
    //uint public CR; // Capital reserve
    uint public B; // Book
    uint public D; // Debt
...
struct MarketConfig {
    ...
    int128[5][5] Premium_Mul; // multiplier for conversion premium against the pool to individual taker premium using risk and term
...
/// @title BondConfig.
/// @param x - param for taker position.
/// @param y - param for maker position.
struct BondConfig {
    uint256 x;
    uint256 y;
}