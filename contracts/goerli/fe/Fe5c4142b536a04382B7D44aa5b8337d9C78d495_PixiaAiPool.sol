    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        _earlyFeeTime = early_period_; // Early fee time is 3 days in default
    }
    ...
    function _initializeNumbers(uint256 numbers_) internal {
        _earlyWithdrawFee = uint16(numbers_);
        maxLockMultiplier = uint32(numbers_ >> 16);
        nftMultiplier = uint16(numbers_ >> 48);
        maxLockTime = uint64(numbers_ >> 64);
        uint64 endAt_ = uint64(numbers_ >> 128);
        uint64 startAt_ = uint64(numbers_ >> 192);
