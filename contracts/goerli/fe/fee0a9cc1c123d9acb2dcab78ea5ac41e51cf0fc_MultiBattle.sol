...
contract MultiBattle is ReentrancyGuard, Ownable, ERC721Holder {
    ...
    function changeJudge(address _newJudge) external onlyOwner {
        if (_newJudge == address(0)) revert ZeroAddressNotAllowed();
        if (_newJudge == withdrawJudge) revert UniqueJudge();
        judge = _newJudge;
    }

    function changeWithdrawJudge(address _newWithdrawJudge) external onlyOwner {
        if (_newWithdrawJudge == address(0)) revert ZeroAddressNotAllowed();
        if (_newWithdrawJudge == judge) revert UniqueJudge();
        withdrawJudge = _newWithdrawJudge;
    }
    ...
    function emergencyWithdrawERC20(address payable payee, address token)
        external
        onlyOwner
        nonReentrant
    {
        ...
        SafeTransferLib.safeApprove(tokenContract, payee, balance);
        SafeTransferLib.safeTransfer(tokenContract, payee, balance);
    }
        ...
        )
    {
        ...
        return (
            btl.one.user,
            uint256(btl.one.asset),
            btl.one.contractAddr,
            btl.one.amtOrTokenId,
            btl.one.isWinner,
            btl.one.hasWithdrawn,
            btl.two.user,
            uint256(btl.two.asset),
            btl.two.contractAddr,
            btl.two.amtOrTokenId,
            btl.two.isWinner,
    ...
    function initBattle(
        ...
        AssetType _asset = AssetType.NULL;
        uint256 _amtOrTokenId;
        if (
            msg.value > 0 &&
            ...
            _asset = AssetType.ETH;
            _amtOrTokenId = msg.value;
        } else if (contractAddr != address(0) && amtOrTokenId != 0) {
            if (nft) {
                _asset = AssetType.NFT;
            } else {
                _asset = AssetType.TOKEN;
            }
            _amtOrTokenId = amtOrTokenId;
        }
        ...
        if (_asset == AssetType.ETH) {
            ...
            nftContract.approve(escrowAddr, amtOrTokenId);
            escrow.deposit(
                msg.sender,
                battleId,
                true,
        ...
        } else if (_asset == AssetType.TOKEN) {
            ERC20 stakeTokenC = ERC20(contractAddr);
            ...
            SafeTransferLib.safeApprove(stakeTokenC, escrowAddr, amtOrTokenId);
            escrow.deposit(
                msg.sender,
                battleId,
                false,
        ...
        } else if (_asset == AssetType.NULL) {
            escrow.deposit(msg.sender, battleId, false, address(0), 0);
        }
    ...
    function joinBattle(
        ...
        if (selectedBattle.matched) {
            ...
            selectedBattle.two.asset = _asset;
            selectedBattle.two.contractAddr = contractAddr;
            selectedBattle.two.amtOrTokenId = _amtOrTokenId;
            battles[battleId] = selectedBattle;
    ...
    ) external payable battleExists(battleId) {
 ...
 */
contract MultiEscrow is Ownable, ERC721Holder {
    ...
    mapping(address => mapping(uint256 => MultiStake)) private _deposits;
    ...
    {
        MultiStake memory stake = _deposits[payee][battleId];
        return (uint256(stake.asset), stake.contractAddr, stake.amtOrTokenId);
    }
    ...
    function deposit(
        ...
        MultiStake memory _deposit = MultiStake({
            asset: AssetType.NULL,
            contractAddr: address(0),
            amtOrTokenId: 0
        });
        if (
            ...
            _deposit = MultiStake({
                asset: AssetType.NFT,
                contractAddr: contractAddr,
                amtOrTokenId: amtOrTokenId
            });
        } else {
            // token
            ERC20 tokenContract = ERC20(contractAddr);
            SafeTransferLib.safeTransferFrom(
                tokenContract,
                msg.sender,
                address(this),
        ...
        _deposits[payee][battleId] = _deposit;
        emit Deposited(
            payee,
            battleId,
            uint256(_deposit.asset),
            contractAddr,
            amtOrTokenId
        );
    }
    ...
    function withdraw(
        ...
        MultiStake memory _stake = _deposits[payee][battleId];

        _deposits[payee][battleId] = MultiStake({
            asset: AssetType.NULL,
            contractAddr: address(0),
            amtOrTokenId: 0
        });

        if (_stake.asset == AssetType.ETH) {
            receiver.sendValue(_stake.amtOrTokenId);
        } else if (_stake.asset == AssetType.NFT) {
            ERC721 nftContract = ERC721(_stake.contractAddr);
            nftContract.approve(receiver, _stake.amtOrTokenId);
            nftContract.safeTransferFrom(
                address(this),
                receiver,
                _stake.amtOrTokenId
            );
        } else if (_stake.asset == AssetType.TOKEN) {
            ERC20 tokenContract = ERC20(_stake.contractAddr);
            SafeTransferLib.safeApprove(
                tokenContract,
                receiver, // to
                _stake.amtOrTokenId // amount
            );
