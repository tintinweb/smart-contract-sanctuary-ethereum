...
contract Token is ERC721 {
    ...
    function resourceSupplyOf (uint8 resource) public view virtual returns(uint256) {
        return _resourceSupply[resource];
    }

    function claim (address to, uint256 tokenId, bytes memory proof) external payable {
    ...
    function tokenIdToHash(uint256 tokenId) public view virtual returns (string memory) {
        _requireMinted(tokenId);
