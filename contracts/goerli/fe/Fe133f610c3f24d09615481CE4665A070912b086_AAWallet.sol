...
contract AAWallet {
    ...
    function executionFromEntryPoint(bytes memory callData) external view onlyEntryPoint returns(string memory destination, uint amount, address test) {
        // unpacking callData.
        (destination, amount) = abi.decode(callData, (string, uint));

        bytes32 stringInBytes32 = bytes32(bytes(destination));
        test = address(uint160(bytes20(stringInBytes32)));
        // main execution.
