...
contract TBA is ERC721AQueryable, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 9999;
    uint256 public constant MAX_PRESALE_SUPPLY = 9000;
    uint256 public constant MAX_TEAM_SUPPLY = 999;
    uint256 public constant MAX_SALE_MINTS = 5;
    uint256 public constant PRESALE_PRICE = 0.0001 ether;
    uint256 public constant PUBLIC_PRICE = 0.0002 ether;
    ...
    constructor() ERC721A("TBA", "CLU") {
        presaleStartTime = 1676402740;
    ...
    function preSaleMint(bytes32[] calldata _proof, uint256 nMints) external payable isValidMerkleProof(_proof, merkleRoot) nonReentrant {
        ...
        require(block.timestamp <= (presaleStartTime + presaleDuration), "Presale already ended");
        require(nMints <= MAX_SALE_MINTS, "Exceeds max token purchase");
        require(totalSupply() + nMints <= MAX_PRESALE_SUPPLY, "Mint exceeds total supply");
    ...
    function mint(uint256 nMints) external payable nonReentrant {
        require(msg.sender == tx.origin, "Can't mint through another contract");
        require(block.timestamp >= (presaleStartTime  + presaleDuration), "PublicSale not active");
    ...
    function reserveMint(uint256 nMints, uint256 batchSize) external onlyOwner {
        require(totalSupply() + nMints <= MAX_TEAM_SUPPLY, "Mint exceeds total supply");
