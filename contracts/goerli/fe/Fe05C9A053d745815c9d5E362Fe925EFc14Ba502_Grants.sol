...
contract Grants is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IERC721ReceiverUpgradeable {
    ...
    */
        ...
        returns (uint256)
    {
        ...
        if (_grants.startTime > _grants.endTime) revert IncorrectTimesGiven();
        if (_grants.topDonatedAmount > 0) revert InitialAmountHasToBeZero();
    ...
    */
    function cancelGrant(uint256 grantsID) public onlyRole(CURATOR_ROLE) {
        ...
        ) revert GrantHasEnded(); // check this logic
    ...
    */
    function donate(uint256 _grantsId, uint256 _amount)
        ...
        nonReentrant
    {
        if (_amount != msg.value) revert AmountsNotEqual();
        ...
        if (
            ...
            grants[_grantsId]
                .topDonatedAmount = totalDonationPerAddressPerCycle[_grantsId][
                msg.sender
            ];
        }
    ...
    */
    function sendRewards(uint256 grantsID) public onlyRole(CURATOR_ROLE) {
        ...
        if (grants[grantsID].cancelled == true) revert GrantCancelled();
        if (grants[grantsID].ended == true) revert GrantHasEnded();
