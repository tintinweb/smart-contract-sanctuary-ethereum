...
// import {LibAppStorageBase, SaleConfig} from "../../libraries/LibAppStorage.sol";
...
abstract contract MintMarketFixedPriceDrop is MarketFees {
  ...
  event MintFromFixedPriceDrop(
    ...
    struct Vars {
        mapping(address => SaleConfig) nftContractToSaleConfig;
    }

    function vars() internal pure returns(Vars storage ds) {
        bytes32 storagePosition = keccak256("diamond.storage.MintMarketFixedPriceDrop");
  ...
  function createFixedPriceSale(
    ...
    SaleConfig storage saleConfig = vars().nftContractToSaleConfig[nftContract];
    ...
    )
  {
    try ICollectionMint(nftContract).isSoldOut() returns (bool soldOut) {
      if (!soldOut) {
        SaleConfig memory saleConfig = vars().nftContractToSaleConfig[nftContract];
