...
import { SimpleNFTA } from "../dAgoraWizard/Simple-NFT-A.sol";
...
contract dAgoraSimpleNFTAFactory is Ownable, ReentrancyGuard {
    ...
    uint256 public nftACount = 0;
    bool public paused;

    mapping(address => Deploys) private _deployedContracts;

    event FactoryContractCreated(address nftContract, address owner, uint256 nftAIndex);
        ...
        nonReentrant
    {
        ...
        _deployedContracts[msg.sender].nftAIndex = nftACount;

        emit FactoryContractCreated(
            address(nfta), 
            msg.sender,
            nftACount
        );
...
contract SimpleNFTA is ERC721A, Ownable {  
  ...
  function togglePaused(bool _state) 
    public 
    onlyOwner
  {
    paused = _state;
