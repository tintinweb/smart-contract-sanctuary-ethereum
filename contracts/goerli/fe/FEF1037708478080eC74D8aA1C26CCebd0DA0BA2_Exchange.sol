...
address test3= address(0);
    ...
    mapping(bytes32 => uint256) private fills; // Orders fills (basis points: 10000 <=> 100%) indexed by orders hashes
    ...
    uint256 private protocolFee; // Protocol fee (basis points: 10000 <=> 100%) to be paid by seller of ERC721/ERC1155 asset
    ...
    function setProtocolFee(uint256 _newProtocolFee) public override requireOwner {
...
contract Exchange is Pausable, NonReentrant, ExchangeAutoProxy, IExchange {
    ...
    constructor(
        ...
        address _weth, // WETH token address (only ERC20 token allowed by default)
        uint256 _protocolFee
    ) ExchangeAutoProxy(_proxy, address(this)) {
    ...
    function matchOrders(
        ...
        uint256 matchLeftBeforeBlock, // Block after which matching taker order is not allowed by order-book
        ...
        uint256 matchRightBeforeBlock, // Block after which matching maker order is not allowed by order-book
        ...
        checkCounterparties(orderLeft, orderRight);
        ...
        getExchangeHelperImpl().validateOrder(orderRight, signatureRight, _callerAddress(), proxy);
        getExchangeHelperImpl().validateOrder(orderLeft, signatureLeft, _callerAddress(), proxy);
    ...
    function matchAndTransfer(
        ...
        // Process ETH and WETH conversions and transfers to proxy, and calculate totalETHAmountWithFees
        uint256 totalETHAmountWithFees;
        (totalETHAmountWithFees, makerAssetType) = processEthAndWeth(
            makerAssetType,
            takerAssetType,
            orderLeft,
            orderRight,
        ...
        // Transfer assets
        (
            uint256 totalMakeValue, // Total value sent by maker to taker
            uint256 totalTakeValue // Total value sent by taker to maker
        ...
        if (makerAssetType.assetClass == LibAssetClasses.ETH_ASSET_CLASS) {
            if (totalETHAmountWithFees > totalTakeValue) {
                IExchangeGovernedProxy(proxy).safeTransferETH(
                    _callerAddress(),
                    totalETHAmountWithFees.sub(totalTakeValue)
                );
            }
        } else if (takerAssetType.assetClass == LibAssetClasses.ETH_ASSET_CLASS) {
            if (totalETHAmountWithFees > totalMakeValue) {
                IExchangeGovernedProxy(proxy).safeTransferETH(
                    _callerAddress(),
                    totalETHAmountWithFees.sub(totalMakeValue)
                );
            }
        }
        ...
        IExchangeGovernedProxy(proxy).emitMatchAssetDetails(
            makerAssetType.assetClass,
            makerAssetType.data,
            takerAssetType.assetClass,
            takerAssetType.data
        );
    }
    ...
    function processEthAndWeth(
        ...
        internal
        returns (uint256 totalETHAmountWithFees, LibAssetTypes.AssetType memory _makerAssetType)
    {
        // totalETHAmountWithFees is the amount of ETH sent to this contract, either directly through msg.value,
        // or indirectly in WETH for which ETH is redeemed
        _makerAssetType = makerAssetType;
        // Check msg.value
        if (msg.value > 0) {
            totalETHAmountWithFees = msg.value;
            // If msg.value > 0, maker should expect to receive ETH or WETH
            require(
            ...
            if (makerAssetType.assetClass == LibAssetClasses.ETH_ASSET_CLASS) {
                ...
                // Forward msg.value to proxy (proxy holds funds)
                ...
                require(
                    ...
                    'Exchange: failed to forward msg.value to proxy'
                );
            } else {
                ...
                IERC20(weth).transfer(proxy, msg.value);
            ...
            // Taker is paying the protocol fee, we calculate the total amount to be paid including fees
            LibOrderDataV1Types.DataV1 memory leftOrderData = getExchangeHelperImpl().parse(
                orderLeft
            );
            totalETHAmountWithFees = getExchangeHelperImpl().calculateTotalAmount(
                newFill.rightOrderTakeValue,
            ...
            IExchangeGovernedProxy(proxy).safeTransferERC20From(
                ...
                orderLeft.maker,
                address(this),
                totalETHAmountWithFees
            );
            // Redeem ETH for WETH
            IWrappedCoin(weth).withdraw(totalETHAmountWithFees);
            // Forward ETH to proxy via receiveETH function (proxy holds funds)
            // selector = bytes4(keccak256(bytes('receiveETH()')))
            (bool success, bytes memory data) = proxy.call{ value: totalETHAmountWithFees }(
                abi.encodeWithSelector(0x3ecfd51e)
            );
            require(
                success && (data.length == 0 || abi.decode(data, (bool))),
            ...
            // If msg.value == 0 and taker is expecting ETH, but maker sends WETH
            // Maker is paying the protocol fee, we calculate the total amount to be paid including fees
    ...
    {
        if (order.salt == 0) {
            // order.salt can be zero for taker orders that are not registered in off-chain OrderBook
            fill = 0;
        } else {
    ...
    function doTransfers(
    ...
    ) internal returns (uint256 _totalMakeValue, uint256 _totalTakeValue) {
        ...
        _totalMakeValue = _fill.leftOrderTakeValue;
        _totalTakeValue = _fill.rightOrderTakeValue;
        ...
        if (feeSide == LibFeeSideTypes.FeeSide.MAKE) {
            ...
            _totalMakeValue = doTransfersWithFees(
            ...
            _totalTakeValue = doTransfersWithFees(
                _fill.rightOrderTakeValue, // Value to be transferred from taker to maker
                _leftOrder.maker, // Taker
                leftOrderData, // Taker order data
                rightOrderData, // Maker order data
    ...
    function doTransfersWithFees(
    ...
    ) internal returns (uint256 _totalAmount) {
        ...
        _totalAmount = getExchangeHelperImpl().calculateTotalAmount(
            _amount,
    ...
    function transferProtocolFee(
        ...
        // LibExchange.subFeeInBp function below, and subtracting it from the totalAmount value which has been calculated
        ...
        // LibExchange.subFeeInBp as _rest = _totalAmount - protocolFee
        (uint256 _rest, uint256 _protocolFee) = getExchangeHelperImpl().subFeeInBp(
            _totalAmount,
            _amount,
            _storage.getProtocolFee()
        );
    ...
    function transferFees(
        ...
        for (uint256 i = 0; i < _fees.length; i++) {
            ...
            (uint256 newRestValue, uint256 feeValue) = getExchangeHelperImpl().subFeeInBp(
                _newRest,
                _amount,
                _fees[i].value
            );
    ...
    function transferPayouts(
        ...
        for (uint256 i = 0; i < _payouts.length - 1; i++) {
            ...
            uint256 payoutAmount = getExchangeHelperImpl().bp(_amount, _payouts[i].value);
    ...
    function setProtocolFee(uint256 newProtocolFee) external override onlyOwner {
        _storage.setProtocolFee(newProtocolFee);
    }
    ...
    function getProtocolFee() external view override returns (uint256) {
        return _storage.getProtocolFee();
    }
...
interface IExchangeGovernedProxy {
    ...
    function emitMatchAssetDetails(
        bytes4 leftAssetClass,
        bytes calldata leftAssetData,
        bytes4 rightAssetClass,
        bytes calldata rightAssetData
    ) external;
