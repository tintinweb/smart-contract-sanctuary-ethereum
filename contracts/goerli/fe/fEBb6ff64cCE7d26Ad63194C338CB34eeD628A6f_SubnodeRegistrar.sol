...
// File contracts/registrar/SubnodeRegistrar.sol
...
contract SubnodeRegistrar is Ownable{
  ...
  function _setPrice(bytes32 node, uint256 startTime, uint256[5] calldata annualPrice, uint256[5] calldata maxPrice) internal authorised(node){
  ...
  function _setWhitelist(bytes32 node, uint256 startTime, uint256 param, bytes32 root) internal authorised(node){
    _nodeRecords[node].setter = msg.sender;
  ...
  function setPrice(bytes32 node, uint256 startTime, uint256[5] calldata annualPrice, uint256[5] calldata maxPrice) external authorised(node){
    _setPrice(node, startTime, annualPrice, maxPrice);
    emit NodeUpdated(node, msg.sender);
  }

  function setWhitelist(bytes32 node, uint256 startTime, uint256 param, bytes32 root) external authorised(node){
    _setWhitelist(node, startTime, param, root);
    emit NodeUpdated(node, msg.sender);
  }

  function setRecord(bytes32 node, NodeRecord calldata record) external authorised(node) {
  ...
  function buySubnodeEns(string memory label, bytes32 parentNode, address resolver, uint64 ttl, uint32 fuses, uint32 duration) external payable {
    _checkBuy(label, parentNode, duration);
    _buyEns(label, parentNode, resolver, ttl, fuses, msg.value, duration);
  }

  function buySubnodeWhitelistedEns(string memory label, bytes32 parentNode, address resolver, uint64 ttl, uint32 fuses, bytes32[] calldata proof, uint64 data) external payable {
    uint32 limit = uint32(data >> 32);
    uint32 duration = uint32(data);
    _checkBuyWhitelisted(label, parentNode, duration, proof, limit);
    _buyEns(label, parentNode, resolver, ttl, fuses, msg.value, duration);
  ...
  function extendSubnodeEns(string memory label, bytes32 parentNode, uint32 duration) external payable {
    // check for subnode
    bytes32 labelhash = keccak256(bytes(label));
    bytes32 node = _makeNode(parentNode, labelhash);
    ...
    uint256 parentExpiry = registrar.nameExpires(uint256(parentNode));
  ...
  function _buyEns(string memory label, bytes32 parentNode, address resolver, uint64 ttl, uint32, uint256 price, uint32 duration) internal{
    // check availability
    bytes32 labelhash = keccak256(bytes(label));
    bytes32 node = _makeNode(parentNode, labelhash);
    ...
    // check expiry date
    address parentOwner = ens.owner(parentNode);
    uint64 parentExpiry = uint64(registrar.nameExpires(uint256(parentNode)));
    uint64 childExpiry = parentExpiry;
    if(duration > 0){
      childExpiry = uint64(block.timestamp + (duration * 365 days));
      require(childExpiry <= parentExpiry, "Invalid duration");
