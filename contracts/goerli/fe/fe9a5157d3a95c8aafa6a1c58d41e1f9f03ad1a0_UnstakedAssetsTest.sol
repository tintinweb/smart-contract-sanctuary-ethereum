...
contract UnstakedAssetsTest {
    ...
    function getAssetNotEnnumerable(
        address account,
        address asset,
        uint256 lastTokenId
    ...
    function unstakedAssets(address account, address[] calldata assets)
        public
        view
        returns (Asset[] memory)
    {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            ERC721Enumerable c = ERC721Enumerable(assets[i]);
            bool isEnnumerable = c.supportsInterface(
                type(IERC721Enumerable).interfaceId
            );
            for (uint256 j = 0; j < balance; j++) {
                uint256 tokenId = isEnnumerable
                    ? c.tokenOfOwnerByIndex(account, j)
                    : getAssetNotEnnumerable(
                        account,
                        assets[i],
                        j == 0 ? 1 : list[outerIndex - 1].tokenId
                    );
