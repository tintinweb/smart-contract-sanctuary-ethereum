...
/**
 * @title NFT_to_be_redeemed contract
 */
...
contract ToBeRedeemedREAL3 is ERC721A, Ownable, ReentrancyGuard {
    ERC721A public accessKeysCollection3;
  string public        baseURI;
  uint public          price             = 0.001 ether;
  uint public          maxPerTx          = 10;
  uint public          maxFreePerWallet  = 3;
  ...
  constructor(
      address _accessKeysCollection3
      ) ERC721A("To Be Redeemed REAL", "TBR2") {
          accessKeysCollection3 = ERC721A(_accessKeysCollection3);
          }
  ...
  {
    uint cost = price;
    require(accessKeysCollection3.balanceOf(msg.sender) >= amt,"Rejected: Claimer does not have access key"); 
    require(msg.sender == tx.origin,"The caller is another contract");
    require(msg.value >= amt * cost,"Please send the exact amount.");
    require(totalSupply() + amt < maxSupply + 1,"Reached max supply");
    require(mintEnabled, "Minting is not live yet, hold on...");
    require( amt < maxPerTx + 1, "Max per TX reached.");
   // safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, 1);
    _safeMint(msg.sender, amt);
  }

  function ownerMint(uint256 amt) external onlyOwner
  {
  ...
  function setMaxPerWallet(uint256 maxFreePerWallet_) external onlyOwner {
