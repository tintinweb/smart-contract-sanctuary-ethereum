...
contract ProxyIDO is Ownable {
     ...
     bool public    statusRun;  //合约状态
     address public adminAddr;
     uint256 public adminPoint;

     mapping ( address => uint)  userTime;

     event createLog(string  func,address AddressMsg,address owner,address admin);
  ...
  function open() public onlyOwner{
    ...
    //function createIDO()  public returns (address,address,address) {
    function createIDO(IDO.projectInfo memory data) public returns (address,address,address) {

        require(statusRun==true, "contract lock");

        //检索上次创建时间
        uint timeVlaue=userTime[msg.sender];
        ...
        emit createLog("create", address(contractAddr),msg.sender,adminAddr);
    
        
        //写入项目结束时间
        userTime[msg.sender]= data.endTime;

        //return (address(contractAddr),msg.sender,adminAddr);
        return (address(msg.sender),msg.sender,adminAddr);
    }

    function createIDOTest() public returns (address,address,address,IDO.projectInfo memory ) {
        ...
        return (address(msg.sender),msg.sender,adminAddr,data);
    }
...
contract IDO is Ownable {
    ...
    struct projectInfo {
            ...
            uint256  maxExchange;      // 项目方允许的最大单次兑换 数字 × 1000，用户输入的最小数字 为0.1
            uint256  exchange;         // 
            uint256  decimal;          // TokenOut 精度，需要除以 1000
            uint     lockNum;          // 锁仓次数 最大36
            uint[]   timeList;         // 时序list
      }
  ...
  event DebugMsg(string  func,address AddressMsg,string exec,uint256 number);
    ...
    constructor(
       projectInfo memory data,address owner,address admin,uint256 _adminPoint
        ) {
    ...
    //设置项目基础信息 初始化的时候自动设置 内部调用
    function initIdo ()  internal  {      

      //项目方转账 //合约生成测试用 不转账版本
      SafeERC20.safeTransferFrom(tokenB,address(msg.sender),address(this), Info.inTokenCapacity);
      emit DebugMsg("refillOwner",owner(),"amount",Info.inTokenCapacity);
...
function swap(uint256 _amountIn) public  {
        ...
        //计算余下的 Token 是否足够兑换
        uint256  amountIn=(_amountIn*Info.exchange)*Info.decimal;
        
        require(Info.inTokenCapacity-amountIn>=0,"surplus amount insufficient");

        //检查用户转金额是否充足
        uint256 balance=tokenA.balanceOf(msg.sender);
        uint256 transferAmount=_amountIn*Info.decimal;
