...
contract CNPCEma is ERC721AQueryable, Ownable {
    ...
    function mint(uint256 _mintAmount, address recipient) external payable {
        ...
        if(address(cnpc)!=address(0)){
            if(isCNPCsale==true){
                if(cnpc.balanceOf(_msgSender()) >= discountBorderAmount){
    ...
    function setDiscountBorderAmount(uint256 _newBorder) external onlyOwner {
        discountBorderAmount = _newBorder;
    }

    function setCanBuyBorderAmount(uint256 _newBorder) external onlyOwner {
        canBuyBorderAmount = _newBorder;
    }
