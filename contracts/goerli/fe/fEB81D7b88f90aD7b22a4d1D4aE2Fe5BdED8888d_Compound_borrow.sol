...
contract Compound_borrow is AccessControl{
    ...
    //using CTokenInterface for Ctoken;
    ...
    address payable _cEtherAddress = payable(0x20572e4c090f15667cF7378e16FaD2eA0e2f3EfF);
    address payable Ether = payable(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);
    ...
    function addCtokenReplesentedByToken(IERC20 _token,address _CToken)public onlyRole(DEFAULT_ADMIN_ROLE){
        tokenForCtoken[address(_token)] = _CToken;
    } 

    function supplyEthToCompound(address payable _cEtherContract)public payable returns (bool){
        // Create a reference to the corresponding cToken contract
        CEth _cToken = CEth(_cEtherContract);
        ...
        _cToken.mint{ value: msg.value, gas: 250000 }();
        return true;
    }

    function supplyErc20ToCompound(address _erc20Contract,uint256 _numTokensToSupply) public returns (uint) {
        // Create a reference to the underlying asset contract, like DAI.
        address _cToken = tokenForCtoken[_erc20Contract];
        // Amount of current exchange rate from cToken to underlying
        uint256 exchangeRateMantissa = cToken.exchangeRateCurrent();
        ...
        IERC20(_erc20Contract).approve(_cToken, _numTokensToSupply);
        // Mint cTokens
        uint mintResult = CErc20(_cToken).mint(_numTokensToSupply);
        return mintResult;
    }

    function redeemCErc20Tokens(uint256 amount,bool redeemType,address token) public returns (bool) {
        // Create a reference to the corresponding cToken contract, like cDAI
        address _cToken = tokenForCtoken[token];
        uint256 redeemResult;

        if (redeemType == true) {
            // Retrieve your asset based on a cToken amount
            redeemResult = CErc20(_cToken).redeem(amount);
        } else {
            // Retrieve your asset based on an amount of the asset
            redeemResult = CErc20(_cToken).redeemUnderlying(amount);
        }
    ...
    function redeemCEth(uint256 amount,bool redeemType,address _cEtherContract) public returns (bool) {
        ...
        if (redeemType == true) {
            // Retrieve your asset based on a cToken amount
            redeemResult = _cToken.redeem(amount);
        } else {
            // Retrieve your asset based on an amount of the asset
            redeemResult = _cToken.redeemUnderlying(amount);
        }
...
function borrowErc20Example(address _cTokenAddress,uint _underlyingDecimals) public payable returns (uint256) {
    ...
    function myErc20RepayBorrow(address _erc20Address,address _cErc20Address,uint256 amount)public returns (bool) {
        //Erc20 underlying = Erc20(_erc20Address);
        address _cToken = tokenForCtoken[_erc20Address];

        IERC20(_erc20Address).approve(_cErc20Address, amount);
        uint256 error = CErc20(_cToken).repayBorrow(amount);
        require(error == 0, "CErc20.repayBorrow Error");
        return true;
    }

    function borrowEthExample(address _cTokenAddress,address _underlyingAddress,uint256 _underlyingToSupplyAsCollateral)public  returns (uint) {
        // CEth cEth = CEth(_cEtherAddress);
        // Comptroller comptroller = Comptroller(_comptrollerAddress);
        // CErc20 cToken = CErc20(_cTokenAddress);
        // Erc20 underlying = Erc20(_underlyingAddress);
        address _cToken = tokenForCtoken[_underlyingAddress];

        // Approve transfer of underlying
        IERC20(_underlyingAddress).approve(_cToken, _underlyingToSupplyAsCollateral);
        ...
        CEth(_cEtherAddress).borrow(numWeiToBorrow);

        uint256 borrows = CEth(_cEtherAddress).borrowBalanceCurrent(address(this));
        emit MyLog("Current ETH borrow amount", borrows);

        return borrows;
    }

    function myEthRepayBorrow( uint256 amount, uint256 gas)public returns (bool)
    {
        CEth(_cEtherAddress).repayBorrow{ value: amount, gas: gas }();
        return true;
    }
}
