    ...
    OwnableUpgradeable
{
    ...
    function _splitBiometa() internal {
        uint256 amount = IRandom(random).randomRateSplitBiometaToken() *
            10**IERC20Metadata(biometaToken).decimals();
    ...
    {
        require(
            IERC20(biometaToken).balanceOf(msg.sender) >= _amount,
            "Not enough funds"
        );
        CellData.Cell memory cell = ICellRepository(metaCell).getMetaCell(
            _tokenID
        ...
        if (block.number >= cell.nextEvolutionBlock - _blocksAmount) {
            cell.nextEvolutionBlock = block.number;
        } else {
            cell.nextEvolutionBlock = cell.nextEvolutionBlock - _blocksAmount;
        }
        require(
    ...
    function _getBoostedBlocks(uint256 _price) private view returns (uint256) {
        return _price / boostPerBlockPrice;
    }
        ...
        returns (uint256)
    {
        ...
        return
            (cell.nextEvolutionBlock - block.number) * getBoostPerBlockPrice();
    }
    ...
    function withdrawFee(address to) external onlyTimelock {
        IERC20(biometaToken).transfer(
            to,
            IERC20(biometaToken).balanceOf(address(this))
        );
    }
