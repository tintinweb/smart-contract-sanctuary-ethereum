...
contract NFTauction is Ownable, ReentrancyGuard {
    ...
    function getDigest(
        ...
        return keccak256(abi.encode(auctionType, tokenId, bidPrice, winner, creator, nftContract));
    }
        ...
        onlyNotCancelled(nftContract, tokenId)
    {
        ...
        address creator = auctionItems[nftContract][tokenId].creator;

        bytes32 digest = getDigest(auctionType, tokenId, bidPrice, winner, creator,nftContract);
        require(_isVerifiedCoupon(digest, coupon), "invalid-coupon");

        if (auctionType == 1) {
            uint256 realAmount = msg.value;
            uint256 servicePercentage = auctionItems[nftContract][tokenId].royaltyPercentage;
            uint256 finalPrice = realAmount - (realAmount * servicePercentage) / 10000;
            require(realAmount >= bidPrice, "invalid-amount");
            _transfer(creator, finalPrice);
