...
contract DynamicNft is AccessControl, IDynamic {
    ...
    function getCurrentSubscriptionTier() public view returns (uint256) {
        INft SubscriptionNft = INft(subsContractAddress);

        uint256 currentTotalSupply = SubscriptionNft.totalSupply();
        uint256 subsOneQuantities = subscriptionDetails.tierOneQuantities;
        uint256 subsTwoQuantities = subscriptionDetails.tierTwoQuantities;
        uint256 subsThreeQuantities = subscriptionDetails.tierThreeQuantities;

        if (currentTotalSupply < subsOneQuantities) {
            return 1;
        } else if (currentTotalSupply < (subsOneQuantities + subsTwoQuantities)) {
            return 2;
        } else if (currentTotalSupply < (subsOneQuantities + subsTwoQuantities + subsThreeQuantities)) {
            return 3;
        } else {
            return 4;
        }
