...
contract NftMinter is ERC721Enumerable, Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public maxPublicNFTCount = 1000;
    ...
    uint256 public whitelistLevelLimit = 4;
    bytes32 public root;
    ...
    event Mint(address minter, uint256 _tokenAmount);
    ...
    function airdropMint() external onlyOwner whenNotPaused {
        uint256 supply = totalSupply();
        for(uint256 i = 0; i < airdropAddresses.length; i++){
            require(airdropList[airdropAddresses[i]].add(supply) <= maxSupply, "Exceeded max supply");
            ...
            emit Mint(airdropAddresses[i], airdropList[airdropAddresses[i]]);
        }
    }

    /**
    @notice API for Public Minting. 
            Allows only one NFT per Address, with max PublicNFT count as 1000
    @param  _amount Number of NFTs to be minted
    */
    function publicMint(uint256 _amount) external nonReentrant whenNotPaused{
        require(block.timestamp > whitelistMintPeriod,"public mint not started");
        require(_amount > 0, "Value less than 0");

        uint256 supply = totalSupply();
        require(_amount.add(supply) <= maxSupply, "Exceeded max supply");

        if (msg.sender != owner()) {
            require(publicMinted[msg.sender] <= maxAmount, "Limit Exceeded");
            require(_amount <= maxAmount, "Exceeded max amount");
            require(publicNftMintCount < maxPublicNFTCount,"Public nft Minted");
        }
    ...
    function whitelistMint(bytes32[] calldata proof,uint _level, uint _amount) external nonReentrant whenNotPaused{
        ...
        require(_level <= whitelistLevelLimit, "Level Not Allowed");
    ...
    function reserveMint(uint _amount) external whenNotPaused nonReentrant {
        require(reserveList[msg.sender] > 0, "Not a part of Allowlist");
        require(_amount > 0, "Value less than 0");
        require(reservedNftMintCount.add(_amount) <= 500,"Amount Exceeds Total Reserved Supply");
