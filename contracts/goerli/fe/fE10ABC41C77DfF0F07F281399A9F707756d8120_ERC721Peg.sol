...
contract ERC721Peg is Ownable, ERC721Holder, IBridgeReceiver {
    ...
    event Deposit(address indexed _address, address[] indexed tokenAddresses, uint256[][] indexed tokenIds, address[] contractOwners, address destination);
    ...
    function deposit(address[] calldata _tokenAddresses, uint256[][] calldata _tokenIds, address _destination) payable external {
        ...
        address[] memory contractOwners = new address[](_tokenAddresses.length);
        ...
        for (uint256 i = 0; i < _tokenAddresses.length; ++i) {
            ...
            for (uint256 j = 0; j < tokenIds.length; j++) {
                IERC721(tokenAddress).safeTransferFrom(msg.sender, address(this), tokenIds[j]);
            }
            // get contract owner via staticcall
            (bool success, bytes memory data) = tokenAddress.staticcall(abi.encodeWithSignature("owner()"));
            if (success) {
                contractOwners[i] = abi.decode(data, (address));
            }
        }

        emit Deposit(msg.sender, _tokenAddresses, _tokenIds, contractOwners, _destination);

        // send message to bridge
        bytes memory message = abi.encode(_tokenAddresses, _tokenIds, contractOwners, _destination);
        bridge.sendMessage{ value: bridge.sendMessageFee() }(palletAddress, message);
    }
