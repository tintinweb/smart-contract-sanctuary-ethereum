...
contract ODmerfs is ERC721, Ownable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmbDWUAvpXt4cBETsCidvRjYJ2SodTR8N3dHkjmwM6JAE5/";
    uint256 public OD_FREE_RESERVE = 0;
    uint256 public OD_PUBLIC = 5000;
    uint256 public OD_MAX = OD_FREE_RESERVE + OD_PUBLIC;
    uint256 public OD_PRICE = 0.02
     ether;
    ...
    constructor() ERC721("ODmerfs", "OD") {}

    function gift(address[] calldata receivers) external onlyOwner {
        require(_tokensMinted.current() + receivers.length <= OD_MAX, "EXCEED_MAX");
    ...
    function founderMint(uint256 tokenQuantity) external onlyOwner {
        require(_tokensMinted.current() + tokenQuantity <= OD_MAX, "EXCEED_MAX");
    ...
    function claim(uint256 tokenQuantity) external {
        ...
        require(claimTokensMinted + tokenQuantity <= OD_FREE_RESERVE, "EXCEED_FREE_RESERVE");
        require(_tokensMinted.current() + tokenQuantity <= OD_MAX, "EXCEED_MAX");
    ...
    function mint(uint256 tokenQuantity) external payable {
        ...
        require(publicTokensMinted + tokenQuantity <= OD_PUBLIC, "EXCEED_PUBLIC");
        require(_tokensMinted.current() + tokenQuantity <= OD_MAX, "EXCEED_MAX");
        require(tokenQuantity <= 15, "EXCEED_PER_MINT");
        require(OD_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
