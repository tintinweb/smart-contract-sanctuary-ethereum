...
contract Battleship is IBattleship, Ownable {
    ...
    function Review() external view override returns(GameStatus status, IBattleship challenger, bool turn, address gameOwner){
        status = game.status;
    ...
    function cleanUp() internal returns (bool){
        delete game;
        game.status = GameStatus.FINISHED;
        firstAttack = true;
        carrier = Ship(5, 0, false);
        battleship = Ship(4, 0, false);
        cruiser = Ship(3, 0, false);
        submarine = Ship(3, 0, false);
        destroyer = Ship(2, 0, false);
        return true;
    }
    ...
    function getPendingPositions()
        ...
        view
        override
        returns (uint pendingpos)
    {
        pendingpos = (game.pendingChallengerGuess.x * 10) + game.pendingChallengerGuess.y;
    }
...
interface IBattleship {
    ...
    function Review() external view returns(GameStatus status, IBattleship challenger, bool turn, address gameOwner);
