...
contract CharityPets is ERC721URIStorage, Ownable {
    ...
    function mint(uint256 amount) public payable {
        ...
        for (uint256 i = 0; i < amount; i++) {
            uint256 newTokenId = pickRandomUniqueId();
    ...
    function pickRandomUniqueId() private returns (uint8)
    {
        uint8 randomId = uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % MAX_SUPPLY);
        if (_exists(randomId)) {
            return pickRandomUniqueId();
        }
