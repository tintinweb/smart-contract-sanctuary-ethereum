...
contract XLARSCValveFactory {
    address payable public immutable contractImplementation;
    uint256 constant version = 1;

    event RSCGCreated(address contractAddress, address owner, string name, uint256 version);
     ...
     * @param _name Name of the RSC contract
     * @param _controller Address which can set / remove recipients
     * @param _immutableController Flag indicating whether controller is immutable / mutable
    ...
    function createRSCValve(
        ...
        XLARSCValve(clone).initialize(
            msg.sender, _controller, _immutableController, _autoEthDistribution
        );
        emit RSCGCreated(clone, msg.sender, _name, version);
        return clone;
    }
}
...
contract XLARSCValve is Initializable, OwnableUpgradeable {
    ...
    /**
     * @dev Throws if sender is not controller
     */
    modifier onlyController {
        require(msg.sender == controller || msg.sender == owner(), "Sender is not controller");
        _;
    }
    ...
    function redistributeEth() external onlyDistributor {
        require(autoEthDistribution == false, "Auto distribution is enabled");
        _redistributeEth();
    }
     ...
     */
    function removeAll() public onlyController {
        if (numberOfRecipients == 0) {
            return;
        }

        require(controller != address(0), "Contract is immutable");
    ...
    function setRecipients(
        ...
        require(controller != address(0) || numberOfRecipients == 0, "Contract is immutable");
        ...
        for (uint256 i = 0; i < newRecipientsLength;) {
            _addRecipient(_newRecipients[i], _percentages[i], _names[i]);
            unchecked{i++;}
        }
    ...
    function setDistributor(address _distributor) external onlyOwner {
        require(_distributor != distributor, "Distributor already configured");
    ...
    function setController(address _controller) external onlyOwner {
        require(controller != address(0), "Contract is immutable");
        require(immutableController == false || _controller == address(0), "Controller address cannot be change");
