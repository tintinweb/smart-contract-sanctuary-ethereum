...
contract NFTLPWrapper is Ownable, ReentrancyGuard {
    ...
    bool _minting;
    NFTLP _targetContract;

    constructor(address targetContract) {
        _targetContract = NFTLP(targetContract);
    }
    ...
    function setTargetContract(address targetContract) external onlyOwner {
        _targetContract = NFTLP(targetContract);
    }
    ...
    function mint(bytes32[] calldata) public payable nonReentrant {
        ...
        require(
            msg.value >= _targetContract._pricePublic(),
        ...
        _targetContract.mintPrivate(receivers);
    }

    // Proxy read
    function leftToTake() public view returns (uint256) {
        return _targetContract.leftToTake();
    }

    function uri(uint256 tokenId) public view virtual returns (string memory) {
        return _targetContract.uri(tokenId);
    }

    function _totalSupply() public view returns (uint256) {
        return _targetContract._totalSupply();
    }
}
