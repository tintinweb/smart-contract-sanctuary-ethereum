...
import { OnChainMonkey } from "./OnChainMonkey.sol";
import { Moonbirds } from "./Moonbirds.sol";
import { PudgyPenguins } from "./PudgyPenguins.sol";

contract Faucet {
    ICollection public boredApeYachtClub;
    ICollection public onChainMonkey;
    ICollection public azuki;
    ICollection public moonbirds;
    ICollection public pudgyPenguins;

    mapping(address => uint256) public lastMintTimestamp;

    modifier checkTimestamp(address minter) {
        require(lastMintTimestamp[minter] + 3600 < block.timestamp, "Need to wait 1 hour");
        _;
    }

    constructor() {
        // deploy nft collections
        boredApeYachtClub = new BoredApeYachtClub();
        onChainMonkey = new OnChainMonkey();
        azuki = new Azuki();
        moonbirds = new Moonbirds();
        pudgyPenguins = new PudgyPenguins();
    }

    function mint() external checkTimestamp(msg.sender) {
        boredApeYachtClub.mint(msg.sender);
        onChainMonkey.mint(msg.sender);
        azuki.mint(msg.sender);
        moonbirds.mint(msg.sender);
        pudgyPenguins.mint(msg.sender);
...
contract Azuki is ERC721Enumerable, Ownable, ICollection {
    uint256 public constant MAX_AZUKIS = 10000;
    ...
    function mint(address recipient) external override onlyOwner {
        require(totalSupply() < MAX_AZUKIS, "Exceeds max mint");
...
contract BoredApeYachtClub is ERC721Enumerable, Ownable, ICollection {
    uint256 public constant MAX_APES = 10000;
    string public constant BASE_TOKEN_URI = "ipfs://QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/";

    constructor() ERC721("BoredApeYachtClub", "BAYC") {}

    function mint(address recipient) external override onlyOwner {
        require(totalSupply() < MAX_APES, "Exceeds max mint");
        _safeMint(recipient, totalSupply());
    }
...
contract OnChainMonkey is ERC721Enumerable, ReentrancyGuard, Ownable, ICollection {
    using Strings for uint256;
    ...
    string[] private z = [
        ...
        '"/><rect width="300" height="120" x="99" y="400" style="fill:#',
        ...
        '"/><circle cx="200" cy="220" r="7" style="fill:#000"/><circle cx="300" cy="220" r="7" style="fill:#000"/>',
    ...
    struct Ape {
        // a nod to BAYC, "ape" was shorter to type than monkey
        uint8 bg;
        uint8 fur;
        uint8 eyes;
        uint8 mouth;
    ...
    function getTraits(Ape memory ape) internal view returns (string memory) {
        string memory o = string(
            abi.encodePacked(
                tr1,
                uint256(ape.bg).toString(),
                tr2,
                uint256(ape.fur).toString(),
                tr3,
                uint256(ape.earring).toString()
            )
        );
        return
            string(
                abi.encodePacked(
                    o,
                    tr4,
                    uint256(ape.hat).toString(),
                    tr5,
                    uint256(ape.eyes).toString(),
                    tr6,
                    uint256(ape.clothes).toString(),
                    tr7,
                    uint256(ape.mouth).toString(),
                    tr8
                )
            );
    }
    ...
    function genEye(
        ...
        string memory out = "";
        if (h > 4) {
            out = string(abi.encodePacked(sl1, a, sl2, a, zz));
        }
        if (h > 10) {
            out = string(abi.encodePacked(out, ey1, b, ey2, b, zz));
        }
        if (h > 16) {
            out = string(abi.encodePacked(out, ey3, a, zz));
        }
        return out;
    }
    ...
    function genMouth(uint8 h) internal view returns (string memory) {
        ...
        if ((h > 24) || ((h > 8) && (h < 16))) {
            for (i = 0; i < 7; i++) {
                out = string(
                    abi.encodePacked(out, mo1, (175 + i * 25).toString(), mo2, (175 + i * 25).toString(), mo3)
                );
            }
            for (i = 0; i < 6; i++) {
                out = string(
                    abi.encodePacked(out, mo1, (187 + i * 25).toString(), mo4, (187 + i * 25).toString(), mo5)
                );
            }
        }
        if (h > 15) {
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    tokenId.toString(),
                                    getTraits(ape),
                                    Base64.encode(bytes(genSVG(ape))),
                                    rl3
                                )
                            )
                        )
    ...
    function mint(address recipient) external override onlyOwner {
        require(numClaimed >= 0 && numClaimed < 9500, "invalid claim");
        _safeMint(recipient, numClaimed + 1);
        numClaimed += 1;
    }
...
contract Moonbirds is ERC721Enumerable, Ownable, ICollection {
    uint256 public constant MAX_BIRDS = 10000;
    string public constant BASE_TOKEN_URI = "https://live---metadata-5covpqijaa-uc.a.run.app/metadata/";

    constructor() ERC721("Moonbirds", "MOONBIRD") {}

    function mint(address recipient) external override onlyOwner {
        require(totalSupply() < MAX_BIRDS, "Exceeds max mint");
        _safeMint(recipient, totalSupply());
    }
...
contract PudgyPenguins is ERC721Enumerable, Ownable, ICollection {
    uint256 public constant MAX_PENGUINS = 8888;
    string public constant BASE_TOKEN_URI = "https://ipfs.io/ipfs/QmWXJXRdExse2YHRY21Wvh4pjRxNRQcWVhcKw4DLVnqGqs/";

    constructor() ERC721("Pudgy Penguins", "PPG") {}

    function mint(address recipient) external override onlyOwner {
        require(totalSupply() < MAX_PENGUINS, "Exceeds max mint");
        _safeMint(recipient, totalSupply());
    }
