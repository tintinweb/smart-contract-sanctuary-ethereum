...
contract CollectionFactory is ICollectionFactory, Ownable {
    ...
    function config(
        address agent_,
        address collectionProxy_,
        address accounting_
    ) external onlyOwner {
        if (
            agent_ == address(0) ||
            collectionProxy_ == address(0) ||
        ...
        collectionProxy = collectionProxy_;
    ...
    function deploy(
        ...
        string calldata contractURIPath_
        ...
        if (
            ...
            bytes(contractURIPath_).length == 0
        ) {
        ...
        ICollectionProxy(collectionProxy).addCollection(
            creator_,
            address(collection)
        );
        collection.transferOwnership(
            IAccounting(accounting).getCollectionOwner()
        );

        _collectionsForCreators[creator_].push(address(collection));
        _collections.push(address(collection));

        emit Deploy(creator_, address(collection));
    ...
    function getCollections(uint256 pageNum_, uint256 pageSize_)
        external
        view
        returns (uint256 total, address[] memory collections)
    {
        if (pageNum_ == 0 || pageSize_ == 0) {
            revert ZeroPageParam();
        }

        total = _collections.length;
        ...
        collections = new address[](end - start);
        for (uint256 i = start; i < end; ++i) {
            collections[i - start] = _collections[i];
        }
    }
    ...
    function getCollectionsByCreator(
        ...
        uint256 pageNum_,
        uint256 pageSize_
    ) external view returns (uint256 total, address[] memory collections) {
        if (pageNum_ == 0 || pageSize_ == 0) {
            revert ZeroPageParam();
        }

        address[] memory all = _collectionsForCreators[creator_];
...
interface ICollectionProxy {
    error ZeroPrice(); // 0x4dfba023
    error RefundFailed(); // 0xf0c49d44
    error ZeroAddress(); // 0xd92e233d
    error CallerNotAllowed(); // 0x2af07d20
    error DuplicateCollections(); // 0x454f6936
    error NonexistentCollection(); // 0xfbacaa17
    // error InsufficientValue(); // 0x11011294
    error TokenNotExists(); // 0x01bdc21c

    event PriceChanged(address[] collections, uint256[][] tokenIds);
    ...
    function addCollection(address creator, address collection) external;
    ...
    ) external payable returns (uint256[] memory collectionFees);
...
interface ICollectionFactory {
    ...
    event Deploy(address indexed creator, address indexed collection);
    
    function deploy(
        address creator,
        string calldata name,
        string calldata symbol,
        string calldata contractURIPath
...
interface ICollection {
    error CallerNotAllowed();   // 0x2af07d20
    error ZeroMaxSupply();      // 0x31c93643
    error TokenNotExists();     // 0x01bdc21c
    error MaxSupplyExceeded();  // 0x8a164f63
    error ApprovalToAgent();    // 0xb32b04ed

    event ItemsAdded(uint256[] ids, string[] hashes);
    ...
    function addItems(uint256[] calldata maxSupplies, string[] calldata hashes)
        external
    ...
    function totalItem() external view returns (uint256);

    function metadatasOf(uint256[] calldata ids)
        external
        view
...
import {ProxyRegistry} from "../common/ProxyRegistry.sol";

/**
 * @dev A collection for NFT items. 
 * NOTICE: This collection is compatible with OpenSea.
 */
contract Collection is ERC1155, Ownable, ICollection, IERC2981 {
    using Counters for Counters.Counter;

    address public immutable agent;
    address public immutable proxyRegistryAddress;
    address public immutable accounting;
    address public immutable collectionProxy;
    ...
    constructor(
        ...
        string memory contractURIPath_
    ) ERC1155("") {
        ...
        name = name_;
        symbol = symbol_;
        contractURIPath = contractURIPath_;
    }
    ...
    {
        return string(abi.encodePacked(IAccounting(accounting).getBaseTokenURI(), Strings.toHexString(address(this)), "/", Strings.toString(id_)));
    }

    function totalSupply(uint256 id_)
        external
        view
        mustExist(id_)
        returns (uint256)
    {
        return metadatas[id_].totalSupply;
    }

    function addItems(uint256[] calldata maxSupplies_, string[] calldata hashes_)
        external
        override 
        returns (uint256[] memory tokenIds)
    {
        if (msg.sender != collectionProxy) {
            revert CallerNotAllowed();
        }

        tokenIds = new uint256[](maxSupplies_.length);
        for (uint256 i = 0; i < maxSupplies_.length;) {
            if (maxSupplies_[i] == 0) {
                revert ZeroMaxSupply();
            }

            tokenIds[i] = _nextTokenId.current();
            _nextTokenId.increment();

            Metadata storage metadata = metadatas[tokenIds[i]];
            metadata.maxSupply = maxSupplies_[i];
        ...
        emit ItemsAdded(tokenIds, hashes_);
    }
    ...
    {
        if (IAgent(agent).isApproved(_owner, _operator)) {
            return true;
        }

        // Whitelist OpenSea proxy contract for easy trading. Approve 
        // OpenSea offical proxy contracts.
    ...
    function mint(
        ...
        for (uint256 i = 0; i < ids_.length; ) {
            Metadata storage metadata = metadatas[ids_[i]];
            if (metadata.maxSupply == 0) {
                revert TokenNotExists();
            }
            metadata.totalSupply += amounts_[i];
            if (metadata.totalSupply > metadata.maxSupply) {
    ...
    function totalItem() external override view returns (uint256) {
        return _nextTokenId.current() - 1;
    }

    function metadatasOf(uint256[] calldata ids_)
        external
        override 
        view
        returns (Metadata[] memory result)
    {
        result = new Metadata[](ids_.length);
        for (uint256 i = 0; i < ids_.length; ++i) {
            result[i] = metadatas[ids_[i]];
            if (result[i].maxSupply == 0) {
                revert TokenNotExists();
            }
        }
    }
    ...
    function royaltyInfo(uint256 tokenId_, uint256 salePrice_)
        ...
        if (metadatas[tokenId_].maxSupply == 0) {
            revert TokenNotExists();
        }
