...
contract RewardBadge is ERC721Time, Ownable, AccessControl, ReentrancyGuard {
    using Counters for Counters.Counter;
    using ECDSA for bytes32;
    using Strings for uint256;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER");
    string public constant NAME = "Metaintro Reward Badge";
    string public constant SYMBOL = "MRB";
    ...
    modifier onlyAllowed() {
        if (!_onlyAllowedOperator(_msgSender())) {
    ...
    function mint(
        ...
        if (minted[_timestamp]) {
            revert MintedAlready();
        }

        if (!_verify(_to, _badgeId, _timestamp, _signature)) {
            revert InvalidSignature();
        }

        uint256 tokenId = tokenIdCounter.current();

        minted[_timestamp] = true;
        badgeType[tokenId] = _badgeId;
    ...
    function _onlyAllowedOperator(address account) internal view returns (bool) {
        return account == owner() || !hasRole(MINTER_ROLE, account);
    }
    ...
    function _verify(
        ...
        bytes32 signedHash = keccak256(abi.encodePacked(_to, _bageId, _timestamp));
        bytes32 messageHash = signedHash.toEthSignedMessageHash();
        address signer = messageHash.recover(_signature);

        return _onlyAllowedOperator(signer);
    }

    /**
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view override(AccessControl, ERC721Time) returns (bool) {
        return interfaceId == type(IAccessControl).interfaceId || super.supportsInterface(interfaceId);
    }

    /**
     * @dev See {ERC721DateTime-tokenURI}.
     */
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        uint256 badgeId = badgeType[tokenId];

        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, badgeId.toString())) : "";
    }
...
abstract contract ERC721Time is Context, ERC165, IERC721Time, IERC721Metadata {
     ...
     */
    function __ERC721_Init(string memory __name, string memory __symbol) internal {
        _name = __name;
        _symbol = __symbol;
    }
