...
contract MulticallTickerLens {
    ...
    function concatTest(uint[] memory Accounts, uint[] memory Accounts2) internal pure returns(uint[] memory) {
        uint[] memory returnArr = new uint[](Accounts.length + Accounts2.length);
