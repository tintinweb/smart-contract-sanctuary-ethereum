...
contract ERC20Peg is Ownable, IBridgeReceiver, ReentrancyGuard {
    ...
    address public palletAddress = address(55_555);
    ...
    event Deposit(address indexed _address, address indexed tokenAddress, uint256 indexed amount, address destination);
    event Withdraw(address indexed _address, address indexed tokenAddress, uint256 indexed amount);
    ...
    function deposit(address _tokenAddress, uint256 _amount, address _destination) payable external {
        ...
        if (_tokenAddress == ETH_RESERVED_TOKEN_ADDRESS) {
            ...
            require(msg.value == 0, "incorrect token address");
        ...
        bytes memory message = abi.encode(msg.sender, _tokenAddress, _amount, _destination);
        bridge.sendMessage(_destination, message);
    }
    ...
    function onMessageReceived(address _source, bytes calldata _message) external override {
        ...
        require(_source == palletAddress, "source must be peg pallet address");

        (address tokenAddress, uint256 amount, address recipient) = abi.decode(_message, (address, uint256, address));
    ...
    function _withdraw(address _tokenAddress, uint256 _amount, address _recipient) internal nonReentrant {
