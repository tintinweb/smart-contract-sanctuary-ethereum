...
{ 
    ...
    event UnBlocklisted(address []   _accounts);
    ...
    function initialize(string calldata name, string calldata symbol) public initializer {
        ...
        _grantRole(BLOCKLISTER_ROLE, msg.sender);

        _cap = 50_000_000_000 * 10**8;
    }
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            _totalMinted = _totalMinted + amounts[i];
        }
        return true;
    }
    ...
    function blocklist(address[] calldata _accounts)
        external
        onlyRole(BLOCKLISTER_ROLE)
    {
        for (uint256 i = 0; i < _accounts.length; i++) 
        {
             blocklisted[_accounts[i]] = true;    
        }        
        
        emit Blocklisted(_accounts);
    }

    function unBlocklist(address[] calldata _accounts) external onlyRole(BLOCKLISTER_ROLE) {         
        for (uint256 i = 0; i < _accounts.length; i++) 
        {
             blocklisted[_accounts[i]] = false;    
        }
        
         emit UnBlocklisted(_accounts);
    }
