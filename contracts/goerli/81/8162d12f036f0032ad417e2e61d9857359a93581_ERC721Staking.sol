...
contract ERC721Staking is Ownable, ReentrancyGuard {
    ...
    // address-akvs eseti field {"address":[{"amountofStaked":'1'}, {"timeoflastupdate":'2'}, {"unclaimedrewards":"3"}] == stakers
    ...
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        if (stakers[msg.sender].amountStaked > 0) {
            ...
            stakers[msg.sender].unclaimedRewards += rewards;   // Amovigot stakers dictidan addressis mixedvit
        } else {
    ...
    function calculateRewards(address _staker) internal view returns (uint256 _rewards) {
        ...
        } else if (stakers[_staker].amountStaked == 2) { // 110%
            Staker memory staker = stakers[_staker];
            return (((((block.timestamp - staker.timeOfLastUpdate) * staker.amountStaked)) * rewardsPerDay) / 86400*110/100);
        } else if (stakers[_staker].amountStaked == 3) { // 120%
            Staker memory staker = stakers[_staker];
            return (((
            ((block.timestamp - staker.timeOfLastUpdate) * staker.amountStaked)
        ) * rewardsPerDay) / 86400*120/100);
        } else if (stakers[_staker].amountStaked == 4) { // 130%
            Staker memory staker = stakers[_staker];
            return (((
            ((block.timestamp - staker.timeOfLastUpdate) * staker.amountStaked)
        ) * rewardsPerDay) / 86400*130/100);
        } else if (stakers[_staker].amountStaked >= 5) { // 140%
            Staker memory staker = stakers[_staker];
            return (((
            ((block.timestamp - staker.timeOfLastUpdate) * staker.amountStaked)
        ) * rewardsPerDay) / 86400*140/100);
        }
