...
/// @title Staking contract with APY that can be changed by the owner
contract Staking is Ownable {
    ...
    ///@dev _rateValues[i] is the rate (APY) after _rateTimestamps[i] and before _rateTimestamps[i+1]
    uint256[] private _rateValues;

    ///Mappings to safe the current staked amount of each user 
    ///and the timestamp where the first stake / last compound occured
    ...
    function fillContract(uint256 amount) public {
        ...
        _totalSupplyReward += amount;
    }
    
    /**
     * @dev Sends `amount` tokens from the caller to this contract, deducting from the caller's
     * allowance. Updates User Array.
     *
     * Requirements:
     *
     * - the contract must have allowance for the tokens from the caller of at least `amount`.
     */
    function stakeTokens(uint256 amount) public {
        require(amount > 0, 'Amount cant be zero');
        require(_token.allowance(_msgSender(), address(this)) >= amount, 'Insufficient allowance');
        _token.transferFrom(_msgSender(), address(this), amount);
        _totalStakedTokens +=_tokensReceived(amount);

        //we compound and credit the tokens
        uint256 timestampStake = block.timestamp;
        _compound(timestampStake);
        _stakedAmount[_msgSender()] += _tokensReceived(amount);
        _timestampStake[_msgSender()] = timestampStake;

    }

    function compound() public {
        require(_stakedAmount[_msgSender()] > 0, 'Caller stakes no tokens');
        require(_timestampStake[_msgSender()] - block.timestamp > 10 minutes, 'Compounding only every 10 minutes');
    ...
    function withdrawTokens() public {
        ...
        _compound(block.timestamp);
        _token.transfer(_msgSender(), _stakedAmount[_msgSender()]);
    ...
    function _compound(uint256 timestampCompound) private {
        ...
        //The first if statement covers the case, where
        //the timestamp of the last compound/stake is greater
        //than the timestamp of the last time the APY got adjusted
        if (_timestampStake[_msgSender()] > _rateTimestamps[_rateTimestamps.length - 1]){
        ...
        } else { /// @dev this else part implies _rateTimestamps.length > 2
            
            //the reward for the last interval
            reward += _computeReward(
                _stakedAmount[_msgSender()],    //stakedAmount
            ...
            bool startIntervalIsComputed = false;
            for (uint256 i = 1; i < _rateTimestamps.length - 1; i++) { 

                //Compute the start interval
                if (_timestampStake[_msgSender()] < _rateTimestamps[i] && !startIntervalIsComputed) {
                    reward += _computeReward(
                        _stakedAmount[_msgSender()],    //stakedAmount
                        _timestampStake[_msgSender()],  //intervalStart
                    ...
                    startIntervalIsComputed = true;
                }

                //Compute remaining intervals
                if (startIntervalIsComputed) {
                    reward += _computeReward(
                        _stakedAmount[_msgSender()],    //stakedAmount
    ...
    function _computeReward(
        ...
        //reward = stake * intervalDuration (in seconds) * (rewardRate/10**18) / (seconds in a year)
        return stakedAmount * (intervalEnd - intervalStart) * rate / (10**18 * 31536000);
    }

    /*
     * Compute the DIV tokens that the recepient of a transaction gets
     */
    function _tokensReceived(uint256 amount) private view returns (uint256) {
    // function _tokensReceived(uint256 amount) private returns (uint256) {
        uint256 tBurn = 0;
        ...
        return amount - ((amount * _token.foundationRate()) / 10**4) - ((amount * _token.communityRate()) / 10**4) - tBurn;
    }
}
