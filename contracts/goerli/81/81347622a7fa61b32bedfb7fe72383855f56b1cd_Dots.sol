...
contract Dots is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint256 private _salePrice = 0.00041 ether;
 ...
 constructor(
  ...
  function ReservedMint(uint quantity)public onlyOwner{
      uint256 ownerTokenCount = balanceOf(msg.sender);
       if (totalSupply() + quantity > MAX_SUPPLY)  revert AmountExceedsSupply();
       if (quantity > MAX_RESERVED)            revert AmountExceedsTransactionLimit();
       if (ownerTokenCount+quantity > MAX_RESERVED) revert AmountExceedsTransactionLimit();
...
//   return base token uri or Notrevealed in constructor
