...
contract SealsNFT is ERC721Enumerable, Ownable
{
    ...
    // IPFS hash of the 100x100 grid of the SealsNFT
    // You can use this hash to verify the image file containing all the seals
    string public NTP_PROVENANCE_SHA256 = "6f6c9f62838dc13db2c72379863d1fe364b168039041cbea7a01ccbbb49c4328";

    constructor(string memory baseURI, address _frstAddress) ERC721("SealsNFT", "SLS")  {
        setBaseURI(baseURI);
        frstAddress = IERC20(_frstAddress);
    }
    ...
    function mint(uint chosenAmount) public isSaleOpen{
        uint256 finalPrice = price.mul(chosenAmount);
        ...
        // number of minted considered for free
        if (totalSupply()>=2)
        {

        uint256 balanceOf = frstAddress.balanceOf(msg.sender);
        require(finalPrice <= balanceOf, "token balance is not enough");
        frstAddress.transferFrom(msg.sender, address(this), finalPrice);
    ...
    function withdrawToken() public onlyOwner {
        frstAddress.transfer(msg.sender, frstAddress.balanceOf(address(this)));
    }
}