...
contract XBeyond is ERC721, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 collectionId, uint256 tokenId, bool isBend, bytes32[] calldata proof, address to) payable public nonReentrant{
        ...
        require(to == nft.ownerOf(tokenId), 'not owner');
        _safeMint(to, collectionId  + tokenId);
    }


    function mintAndRedeem(uint256 collectionId, uint256 tokenId, bool isBend, bytes32[] calldata proof) payable external{
        mint(collectionId, tokenId, isBend, proof, msg.sender);
        redeem(collectionId, tokenId, proof, 1);
    }
    ...
    function redeem(uint256 collectionId, uint256 tokenId, bytes32[] calldata proof, uint num) public {
        Config memory _config = checkWl(collectionId, tokenId, proof) ? configs[1] : configs[0];
        ...
        if (_config.redeemPrice > 0) {
            IERC20(paymentToken).transferFrom(msg.sender, owner(), _config.redeemPrice * num);
        }
        redeemed[collectionId + tokenId] += num;
    }

    function airdrop(uint[] calldata ids, address[] calldata tos) external onlyOwner  {
