...
error CHIZURequestMaker_Currency_Not_Allowed();
error CHIZURequestMaker_Not_Refund_Due_Yet();
error CHIZURequestMaker_Request_Overdue();
error CHIZURequestMaker_Escrow_Is_Not_NFT(uint8 escrowType);
error CHIZURequestMaker_Escrow_Is_Not_Currency(uint8 escrowType);
error CHIZURequestMaker_Signature_Expired();

contract CHIZURequestMaker is Initializable, RequestValidator {
    uint40 internal constant TIMELIMIT = 10 seconds;
    ...
    event Registered(
        ...
        address indexed requesterAccount,
        OrderInfo orderInfo,
    ...
    function registerWithCurrency(
        ...
        if (
            !ICurrencyManager(currencyManager).isSupportedCurrency(
                currencyInfo.currencyAddress
            )
        ) {
            revert CHIZURequestMaker_Currency_Not_Allowed();
        }
        ...
        _requestInfo.status = ACCEPTED;
        _requestInfo.endTime = uint40(block.timestamp) + TIMELIMIT;
        _requestInfo.escrowType = CURRENCY;
        _requestInfo.creatorAccount = orderInfo.creatorAccount;
        _requestInfo.requesterAccount = orderInfo.requesterAccount;
        _requestInfo.contractAddress = currencyInfo.currencyAddress;
        _requestInfo.value = currencyInfo.price;
    ...
    function registerWithNFT(
        ...
        _requestInfo.status = ACCEPTED;
        _requestInfo.endTime = uint40(block.timestamp) + TIMELIMIT;
        _requestInfo.escrowType = escrowType;
        _requestInfo.creatorAccount = orderInfo.creatorAccount;
        _requestInfo.requesterAccount = orderInfo.requesterAccount;
        _requestInfo.contractAddress = nftInfo.collectionAddress;
        _requestInfo.value = nftInfo.tokenId;
        _requestInfo.amount = nftInfo.amount;
    ...
    function deliverWithCurrency(
        ...
        if (_requestInfo.escrowType != CURRENCY) {
            revert CHIZURequestMaker_Escrow_Is_Not_NFT(_requestInfo.escrowType);
        }
    ...
    function refundCurrency(bytes32 requestHash) public payable {
        ...
        if (_requestInfo.escrowType != CURRENCY) {
            revert CHIZURequestMaker_Escrow_Is_Not_Currency(
        ...
        returns (bytes32 _hash)
    {
        _hash = keccak256(
            abi.encodePacked(
                orderInfo.creatorSalt,
                orderInfo.requesterSalt,
                orderInfo.creatorAccount,
                orderInfo.requesterAccount
            )
        );
...
abstract contract RequestValidator is RequestCore {
    ...
    function _validateRegisterWithCurrencySignature(
        ...
        bytes32 creatorOrigin = creatorHash.toEthSignedMessageHash();
        bytes32 requesterOrigin = requesterHash.toEthSignedMessageHash();
        ...
        if (creatorHash != userValidate.creatorHash) {
            return (false, "ReqeustValidator : creator hash does not match");
        }
        if (creatorSigner != orderInfo.creatorAccount) {
            return (false, "ReqeustValidator : creator signer does not match");
        }
        if (requesterHash != userValidate.requesterHash) {
            return (false, "ReqeustValidator : requester hash does not match");
        }
...
error RequestCore_Only_Admin_Available();
error RequestCore_Only_Node_Available();
error RequestCore_Is_Not_Owner();

abstract contract RequestCore is Initializable {
    ...
    modifier onlyAdmin() {
        if (!IAdminRole(core).isAdmin(msg.sender)) {
            revert RequestCore_Only_Admin_Available();
        }
        _;
    }
    ...
    modifier onlyNode() {
        if (!INodeRole(core).isNode(msg.sender)) {
            revert RequestCore_Only_Node_Available();
        }
        _;
    }
    ...
    struct RequestInfo {
        // initial(default) : 0, canceled(refunded):1, processing(accepted) : 2, finished: 3(delivered)
        uint8 status;
        // end time
        uint40 endTime;
    ...
    /**
     * @dev  It's internal function to transfer nft
     */
    function _transferNftFrom(
        ...
        if (amount == ERC721_RESERVED_AMOUNT) {
            ...
            if (fromAddress != IERC721(contractAddress).ownerOf(tokenId)) {
                return (false, "RequesteCore : is no owner");
            }
            ...
            if (
                ...
                return (false, "RequesteCore : is not approved");
            }
