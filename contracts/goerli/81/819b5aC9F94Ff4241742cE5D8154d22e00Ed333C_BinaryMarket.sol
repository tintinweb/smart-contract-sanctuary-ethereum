    ...
    AccessControl
{
    ...
    // @dev timeframeId => block number => round. Rounds that will be closed at certain block.
    mapping(uint8 => mapping(uint256 => Round)) public roundsEndAt;

    /// @dev
    mapping(uint8 => uint256) public currentEpochs;
    mapping(uint8 => uint256) public currenetLockedEpochs;
    ...
    */
    ...
    function genesisLockRound(uint8 timeframeId) external onlyOperator whenNotPaused {
        ...
        currenetLockedEpochs[timeframeId] = currentEpochs[timeframeId];
    ...
    function executeRound(
        ...
        for (uint8 i = 0; i < timeframeIds.length; i = i + 1) {
            ...
            if(genesisLockOnces[timeframeId]) {
                ...
                _safeEndRound(
                    timeframeId,
                    currenetLockedEpochs[timeframeId],
                    currentTimestamp,
                    currentPrice
                );
                ...
                currentClosedEpochs[timeframeId] = currenetLockedEpochs[timeframeId];
                currenetLockedEpochs[timeframeId] = currentEpochs[timeframeId];
                currentEpochs[timeframeId] = currentEpoch;
            }
    ...
    function _safeStartRound(uint8 timeframeId, uint256 epoch) internal {
        ...
        require(
            block.number >= rounds[timeframeId][currenetLockedEpochs[timeframeId]].closeBlock,
    ...
    function _startRound(uint8 timeframeId, uint256 epoch) internal {
        ...
        roundsEndAt[timeframeId][round.closeBlock] = round;
    ...
    */
    function getCurrentBettableAmount(uint8 timeframeId, uint256 epoch) public view returns (uint256) {
        ...
        console.log("maxHourlyExposure", maxHourlyExposure);
        if (maxMinuteExposure == 0) {
            return 0;
        }

        if (rounds[timeframeId][epoch].startBlock != 0) {
            // Current epoch
            uint256 totalRisk;
            for (uint256 i; i < timeframes.length; i ++) {
                uint8 _timeframeId = timeframes[i].id;
                Round memory round = roundsEndAt[_timeframeId][rounds[timeframeId][epoch].closeBlock];
                uint256 _risk;
            ...
            // Future epoch
            (uint256 blockNumber, uint256 timestamp) = getBlockForEpoch(timeframeId, epoch);
            uint256 closeBlock = blockNumber + timeframes[timeframeId].intervalBlocks * 2;
            uint256 totalRisk;

            for (uint256 i; i < timeframes.length; i ++) {
                uint256 startBlock = closeBlock - timeframes[i].intervalBlocks * 2;
                (uint256 _epoch, ) = getRoundIdAt(timeframes[i].id, timestamp, startBlock);
        ...
        returns (bool)
    {
        ...
        if (rounds[timeframeId][epoch].startBlock != 0) {
            ...
            return timestamp <= block.timestamp + futureBettingTimeUpTo &&  block.number <= blockNumber && block.timestamp <= timestamp;
        }
    ...
    */
    ...
    function isNecessaryToExecute(uint8 timeframeId) public view returns(bool) {
        ...
        Round memory currentLockedRound = rounds[timeframeId][currenetLockedEpochs[timeframeId]];

        bool lockable = currentActiveRound.startBlock != 0 && currentActiveRound.lockPrice == 0 && block.number >= currentActiveRound.lockBlock;
        bool closable = currentLockedRound.lockBlock !=0 && currentLockedRound.closePrice == 0 && !currentLockedRound.oracleCalled && block.number >= currentLockedRound.closeBlock;

        return (lockable && closable && (currentLockedRound.totalAmount > 0 || currentActiveRound.totalAmount > 0)) || (round.totalAmount > 0);
    }
    ...
    */
    ...
    {
        return getRoundIdAt(timeframeId, block.timestamp, block.number);
    }
    ...
    */
    function getRoundIdAt(uint8 timeframeId, uint256 timestamp, uint256 blockNumber)
        public
        view
        returns(uint256 roundFromBlockNumber, uint256 roundFromBlockTime)
    {
        roundFromBlockNumber = (blockNumber - genesisStartBlockNumber) / timeframes[timeframeId].intervalBlocks;
    ...
    */
    function getBlockForEpoch(uint8 timeframeId, uint256 epoch) public view returns(uint256 blockNumber, uint256 timestamp) {
        timestamp = genesisStartBlockTimestamp + epoch * timeframes[timeframeId].interval;
        blockNumber = genesisStartBlockNumber + epoch * timeframes[timeframeId].intervalBlocks;
    }
