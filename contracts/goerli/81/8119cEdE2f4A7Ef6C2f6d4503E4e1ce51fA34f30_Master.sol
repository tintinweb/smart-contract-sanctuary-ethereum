...
contract Master is AccessControl, ReentrancyGuard, Pausable {
  ...
  mapping(uint => bytes32) private roomsIndex;
  mapping(bytes32 => Room) private rooms;
  uint _totalRoom;

  /// @notice Emitted when reserves is deposited.
  event RoomUpdated(bytes32 indexed roomId, address indexed contractAddress, uint256 betLimitPerDay, uint256 maxBetPerPlayer, uint32 apiPermission);
  ...
  modifier onlyAdmin() {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "ADMIN_PERMISSION_DENIED");
    _;
  }
  ...
  function createRoom(bytes32 roomId, address creator, bytes32 network, uint32 chainID) external nonReentrant whenNotPaused {
  ...
  function setRoomLimit(uint256 betLimitPerDay, uint256 maxBetPerPlayer, uint32 apiPermission, bytes32 roomId) external nonReentrant whenNotPaused onlyRoomManager {
    ...
    emit RoomUpdated(roomId, msg.sender, betLimitPerDay, maxBetPerPlayer, apiPermission);
  }

  function getTotalRoom() public view returns(uint) {
    return _totalRoom;
  }

  function getRoomByIndex(uint index) public view returns(Room memory) {
