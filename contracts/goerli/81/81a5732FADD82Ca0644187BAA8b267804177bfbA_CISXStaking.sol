...
contract CISXStaking is Ownable2Step {
  ...
  address public carbonToken = 0x141E855b7Bf6F2135420cc77b2a745825Bc63Df6; // TODO: change to mainnet address
  ...
  function unstake(uint256 _idx) external {
    ...
    uint256[3] memory lockDuration = [10 minutes, 15 minutes, uint256(30 minutes)];
    uint8[3] memory apys = [5, 12, 15];
    require(block.timestamp - item.start >= lockDuration[item.lock_type], "Lock period not passed");

    uint256 total = ((item.amount * apys[item.lock_type]) / 100) + item.amount; // rewards + amount
