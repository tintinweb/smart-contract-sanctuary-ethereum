    ...
    DefaultOperatorFilterer
{
    ...
    uint256 public constant PRESALE_MAX_TX = 2;
    ...
    bytes32 public constant PRESALE_TYPEHASH =
        keccak256("Presale(address minter)");
    ...
    ) ERC721A("Spropen", "SPROPEN") EIP712("Spropen", "1.0.0") {
    ...
    function mintPresale(
        ...
        require(saleState == SaleState.Presale, "State doesn't match");
        require(
            signerAddress == _verifyPresaleSignature(msg.sender, _signature),
            "Invalid signature"
        );
        require(_quantity <= PRESALE_MAX_TX, "Maximum tx exceeded");
        require(
            presaleCount[msg.sender] + _quantity <= PRESALE_MAX_MINT,
    ...
    function mintPublicSale(
        ...
        require(
            publicSaleCount[msg.sender] + _quantity <= PUBLIC_SALE_MAX_MINT,
            "Maximum mint exceeded"
        );
        require(
            totalSupply() + _quantity <= MAX_SUPPLY,
        ...
        publicSaleCount[msg.sender] += _quantity;
        _mint(msg.sender, _quantity);
    }
    ...
    function _verifyPresaleSignature(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(abi.encode(PRESALE_TYPEHASH, _minter))
        );
