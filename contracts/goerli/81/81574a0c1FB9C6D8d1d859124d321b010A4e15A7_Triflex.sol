    ...
    ReentrancyGuard
{
    ...
    //@dev CHANGE BEFORE DEPLOYMENT
    // address public immutable developmentWallet = payable(0x7a143450589f0C2086D2FAb16B56275707f77C53);
    address public immutable developmentWallet = payable(0xE8e70c7ea8354E150453f99254309736617bde13); // goerli gnosis
    ...
    constructor (address payable _marketingWallet) ERC20("Triflex", "TF") ERC20Permit("Triflex") {
    ...
    function setMaxWalletAmount(uint256 amount) public onlyOwner {
        require(amount > (TOTAL_SUPPLY * 15) / 10000000, "TF: too small");  //.00015% of TOTAL_SUPPLY
    ...
    function initializeRewardTokens() public onlyOwner {
        ...
        addRewardAddress(address(this), false); // TRFX
    }
...
contract Ham is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    bytes32 public constant TRIFLEX_ROLE = keccak256("TRIFLEX_ROLE");
    ...
    function initialize(address _triflex, address payable _onRye)
        public
        initializer
    {
        ...
        _grantRole(TRIFLEX_ROLE, _triflex);
        claimWait = 3600; // 1 Hour
        onRye = IOnRye(_onRye);
        autoProcessAccount = true;
    }

    function setAutoProcessAccount(bool state) external onlyRole(DEFAULT_ADMIN_ROLE){
        require(state != autoProcessAccount, "TRT: current state");
        autoProcessAccount = state;
    }

    function eFR(address account)
        external
        onlyRole(TRIFLEX_ROLE)
        nonReentrant
    {
    ...
    function uCW(uint256 newClaimWait)
        external
    ...
    function sb(address payable account, uint256 newBalance)
        external
        onlyRole(TRIFLEX_ROLE)
    {
    ...
    function pA(address payable account, bool automatic)
        external
        onlyRole(TRIFLEX_ROLE)
        returns (bool)
    {
    ...
    function setTotalPendingDividends(uint256 newDividends)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
...
interface IOnRye {
    ...
    event TriflexSet(address _triflex);
    event TriflexTokenRateSet(uint256 _triflexTokenRate);
    event PromoTokenRateSet(uint256 _promoTokenRate);
    event TriflexPairSet(address _triflexPair);
    ...
    function setTriflex(address _triflex) external;
    function setTriflexPair(address _triflexPair) external;
    ...
    function setTriflexTokenRate(uint256 _triflexTokenRate) external;
