...
contract Swap is AccessControl, ReentrancyGuard {
    ...
    event IncludeToken(address token, address feed);
    ...
    uint256 public SLIPPAGE = 5 * 1e5; // 0.5%
    ...
    mapping(address => AggregatorV3Interface) public mapTokenFeed; // token address=> feed address
    ...
    constructor(
        ...
        address CRUSWallet,
        address agregatorETH,
        address agregatorUSDC
    ) {
        ...
        mapTokenDecimals[ETH] = 18;

        mapTokenFeed[ETH] = AggregatorV3Interface(agregatorETH);
        mapTokenFeed[usdc] = AggregatorV3Interface(agregatorUSDC);
    ...
    function includeToken(address token, address feed)
        external
        onlyRole(ADMIN_ROLE)
    {
        mapTokenDecimals[token] = IERC20Metadata(token).decimals();
        mapTokenFeed[token] = AggregatorV3Interface(feed);

        emit IncludeToken(token, feed);
    }

    function excludeToken(address token) external onlyRole(ADMIN_ROLE) {
        delete (mapTokenDecimals[token]);
        delete (mapTokenFeed[token]);
    ...
    function mint(address acceptedToken, uint256 amount) external {
        ...
        (uint256 amountWithoutTax, uint256 tax) = _subtractPercentage(
            amount,
            _feeMint
        );
        ...
        uint256 amountCRUS = swapTokensForCRUS(amountWithoutTax, acceptedToken);
        require(amountCRUS >= MIN_AMOUNT, "Minimum 1 USD");

        IERC20MintBurn(CRUS).mint(msg.sender, amountCRUS); // transfer CRUS to user

        emit Mint(acceptedToken, amount, amountCRUS, tax);
    }
    ...
    function mintETH(uint256 minAmountCRUS) external payable nonReentrant {
        ...
        // transfer tax
        _operationWallet.call{value: tax}("");
        // transfer USD
        _masterWallet.call{value: amountWithoutTax}("");

        uint256 amountCRUS = swapTokensForCRUS(amountWithoutTax, ETH);
        require(amountCRUS >= MIN_AMOUNT, "Minimum 1 USD");
        require(minAmountCRUS >= amountCRUS, "slippage error");

        IERC20MintBurn(CRUS).mint(msg.sender, amountCRUS); // transfer CRUS to user

        emit Mint(ETH, amount, amountCRUS, tax);
    }

    function redeem(uint256 amountCRUS) external {
        // burn CRUS
        IERC20MintBurn(CRUS).burnFrom(msg.sender, amountCRUS);

        uint256 amountUSDC = swapUSDCForCRUS(amountCRUS);
    ...
    function swapTokensForCRUS(uint256 amount, address token)
        public
        view
        returns (uint256 amountCRUS)
    {
        uint256 price = getPrice(token);

        return (amount * price) / (10**mapTokenDecimals[token]);
    }
    ...
    function getPrice(address token) public view returns (uint256) {
        (, int256 price, , , ) = mapTokenFeed[token].latestRoundData();
        uint256 factor = 18 - mapTokenFeed[token].decimals();
        return uint256(price) * (10**factor);
    }

    function _calcFactor(address token) internal view returns (uint256 factor) {
        return mapTokenDecimals[CRUS] - mapTokenDecimals[token];
    }

    function swapUSDCForCRUS(uint256 amountCRUS)
        public
        view
        returns (uint256 amountUSDC)
    {
        uint256 price = getPrice(USDC);
        return (amountCRUS * (10**mapTokenDecimals[USDC])) / price;
    }

    function _subtractPercentage(uint256 amount, uint256 percent)
        internal
        pure
        returns (uint256 remains, uint256 share)
    {
    ...
    function CRUSTransferOwnership(address newOwner)
        external
        onlyRole(ADMIN_ROLE)
    {
