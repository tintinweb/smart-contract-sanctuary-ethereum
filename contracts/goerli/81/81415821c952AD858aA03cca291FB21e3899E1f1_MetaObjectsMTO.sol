...
// File: contracts/MetaObjectsMTO.sol
...
/** @title MTO
 * MetaObjects Reward Token
 * http://metaobjects.art
 */

contract MetaObjectsMTO is Ownable, ERC20, IMintableERC20 {
    ...
    modifier onlyMinter() {
        require(_minters[_msgSender()], "MetaObjects: Not minter");
        _;
    }

    modifier whenMintingOpened() {
        require(!isMintComplete, "MetaObjects: Mint is complete");
        _;
    }

    constructor()
        ERC20("MetaObjects Token", "MTO") {
    }
    ...
    function mint(address destination, uint256 amount) external override onlyMinter whenMintingOpened {
        require(destination != address(0), "MetaObjects: Mint to address(0)");
    ...
    function addMinter(address account) external onlyOwner {
        require(!_minters[account], "MetaObjects: Already minter");
    ...
    function removeMinter(address account) external onlyOwner {
        require(_minters[account], "MetaObjects: Not minter");
    ...
    function disableMinting() external onlyOwner {
        require(!isMintComplete, "MetaObjects: Mint already complete");
        isMintComplete = true;
        emit MintDisable();
    }
}