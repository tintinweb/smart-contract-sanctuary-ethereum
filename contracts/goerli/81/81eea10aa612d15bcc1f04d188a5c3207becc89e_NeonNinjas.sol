...
//set new state for contract
  function setSaleState(State _state) external onlyOwner {
    saleState = _state;
}

// modifier - checking mintamount
  modifier mintCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0 && _mintAmount <= maxAmountPerWallet, 'Invalid mint amount!');
    require(totalSupply() + _mintAmount <= maxSupply, 'Max supply exceeded!');
    _;
  }

// modifier - checking funds
...
//MINT Functions
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    require(saleState == State.whitelistsale, 'The whitelist sale is not enabled!');
  ...
  function waitlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    // Verify whitelist requirements
    require(saleState == State.waitlistsale, 'The waitlist sale is not enabled!');
  ...
  function publicMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    require(saleState == State.publicsale, 'The publicSale is not enabled');
...
//Price function in case of pricechanges
  function setPrice(uint256 _price) public onlyOwner {
    price = _price;
  }


// Set merkelroot for whitelist
...
// Art - Metadata
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return bytes(metadataURI).length > 0 ? string(abi.encodePacked(metadataURI, tokenId.toString(), ".json")) : "";
  }
