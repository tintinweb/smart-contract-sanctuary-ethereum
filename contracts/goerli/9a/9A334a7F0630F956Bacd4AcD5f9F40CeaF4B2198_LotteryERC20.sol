...
contract LotteryERC20 is LotteryBase {
    ...
    constructor(
        ...
        console.log("tokenAdress %s", tokenAddress_);
...
abstract contract LotteryBase is Ownable, Pausable, ReentrancyGuard, VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    event PlayerJoined(uint256 id, address player, uint256 ticketPrice);
    ...
    function closeAndPause() external nonReentrant onlyOwner {
        _close();
        _pause();
    }
    ...
    function _close() internal whenNotPaused {
        if (!_isReadyToClose()) revert InvalidState();
    ...
    function initialize(
        ...
        if (ticketPrice_ == 0) revert InvalidTicketPrice();
        ...
        if (durationMinutes_ == 0) revert InvalidDurationMinutes();
    ...
    function getLottery()
        ...
        returns (
            uint256 id_,
            uint256 openedAt_,
            LotteryState lotteryState_,
            uint256 jackpot_,
            uint256 playersCount_,
            address winner_,
            uint256 winnerPayout_,
            uint256 leftOverPayoutPerPlayer_,
            uint256 vrfRequestId_,
            uint256 vrfRandomNumber_,
            uint256 payoutsExecuted_
        )
    {
        return (
            id,
    ...
    function getVrfSettings()
        external
        view
        returns (
            uint64 vrfSubscriptionId_,
            uint32 vrfCoordinatorCallbackGasLimit_,
            uint256 vrfRequestConfirmations_,
            bytes32 vrfKeyHash_
        )
    {
        return (vrfSubscriptionId, vrfCoordinatorCallbackGasLimit, vrfRequestConfirmations, vrfKeyHash);
    }
    ...
    function _open() internal {
        if (!_isReadyToOpen()) revert InvalidState();
    ...
    function _enter(address player_) internal {
        ...
        emit PlayerJoined(id, player_, ticketPrice);
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override nonReentrant {
        ...
        lotteryState = LotteryState.ReadyToComputePayouts;
    ...
    function _isReadyToDraw() internal view returns (bool) {
        ...
        return (isOpen && (maxPlayersReached || (areEnoughPlayers && isExpired)));
    }

    function _isReadyToCompuePayouts() internal view returns (bool) {
        return lotteryState == LotteryState.ReadyToComputePayouts;
    }
    ...
    function _isReadyToClose() internal view returns (bool) {
    ...
    function draw() external nonReentrant onlyOwner {
        _draw();
    }

    function _draw() private {
        if (!(_isReadyToDraw())) revert InvalidState();
    ...
    function checkUpkeep(
        ...
        if (_isReadyToOpen()) {
            performData = abi.encode(LotteryState.Open, new address[](0), new uint256[](0));
            upkeepNeeded = true;
        ...
        if (_isReadyToCompuePayouts()) {
            (address[] memory _addresses, uint256[] memory _balances) = _computePayouts(payoutsExecuted);
            performData = abi.encode(LotteryState.ReadyToComputePayouts, _addresses, _balances);
            upkeepNeeded = true;
            return (upkeepNeeded, performData);
        }
    ...
    function _performUpkeep(
        ...
        if (nextState == LotteryState.Open) return _open();
        if (nextState == LotteryState.Draw) return _draw();
        if (nextState == LotteryState.ReadyToComputePayouts) return _performPayouts(_addresses, _balances);
    }
    ...
    function _performPayouts(address[] memory _addresses, uint256[] memory _payouts) internal {
        if (!(_isReadyToCompuePayouts())) revert InvalidState();
