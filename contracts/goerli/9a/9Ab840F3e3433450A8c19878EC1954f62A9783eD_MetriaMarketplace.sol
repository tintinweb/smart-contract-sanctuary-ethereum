...
interface IMetriaPriceFeed {
    function wETH() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract MetriaMarketplace is Ownable, ReentrancyGuard {
    ...
    function createOffer(
        ...
        IMetriaAuction auction = IMetriaAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isMetriaNFT(_nftAddress), "invalid nft address");
    ...
    function _isMetriaNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.metria() == _nftAddress ||
            IMetriaNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IMetriaNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IMetriaNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IMetriaNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IMetriaPriceFeed priceFeed = IMetriaPriceFeed(
