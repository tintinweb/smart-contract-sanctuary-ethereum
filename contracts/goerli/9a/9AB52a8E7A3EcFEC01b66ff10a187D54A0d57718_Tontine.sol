...
contract Tontine is ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Twontine", "TNT") {}
    ...
    function mint() external payable {
        ...
        if (tokenByAddress[msg.sender] == 0 && _balances[msg.sender] == 0) {
    ...
    function safeTransferFrom(
        ...
        if (_balances[to] > 0 || tokenByAddress[to] != 0) {
            _burn(tokenId);
        } else {
        ...
        // a transfer destroys the from token & holdings
        _balances[from] = 0;
        removeAddress(from);
        tokenByAddress[from] = 0;

        // Create or replace the existing tokenid uri
        string memory tokenUri = _buildTokenURI(tokenId, _balances[to]);
    ...
    function splitBalance() private {
        ...
        if (balance <= 1000) {
            emit BalancePayout(address(0), 0);
            return;
        }
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '", "image":"',
                                image,
                                unicode'", "description": "The Tontine. Three Winners. Do you have the resolve to win?"',
                                '"tokenID":',
                                uintToString(id),
                                ",",
                                '"attributes": [',
                                ...
                                '"trait_type": "Owner",',
                                '"value": "',
                                addressToString(ownerOf(id)),
                                '"',
                                "},",
                                "{",
                                '"trait_type": "METL Rewards",',
                                '"value": "0.25 per day"',
                                "}",
                                "}"
                            )
