...
contract LP_UF_V5 is Ownable {
    ...
    function exchangeLiquidityPoolTokens(
        address _liquidityToken,
        address _tokenToSwap
    ) external payable {
        uint256 deadline = block.timestamp + 300; // using 'now' for convenience, for mainnet pass deadline from frontend!
        uint256 balance = IERC20(_liquidityToken).balanceOf(msg.sender);
        IERC20(_liquidityToken).transferFrom(
            msg.sender,
            address(this),
            balance
        );
        IERC20(_liquidityToken).approve(address(uniswapRouter), balance);
        address[] memory path = new address[](2);
        path[0] = _liquidityToken;
        path[1] = WETH;
        ...
        IERC20(_tokenToSwap).transfer(
            0x0000000000000000000000000000000000000000,
            IERC20(_tokenToSwap).balanceOf(address(this))
        );
