...
contract UnleashedMantleX12 is 
    ERC721, 
    Ownable, 
    ReentrancyGuard 
{
    ...
    uint256 public ownerPerc = 5; // Owner percentage
    ...
    uint256 _price = 27000000000000000; // 0.027 ETH

     Counters.Counter private _tokenIds;

     // Add team member addresses and their respective percentage of the total balance
    address[] public teamMembers = [
    address(uint160(owner())), // Owner address
        0x67D3a454aE98102950b4d2360436cEd8827b349F, // Team member 1 address 5%
        0xa4e82498AE98a1E2D535524e3260c5ae6a051450, // Team member 2 address 15%
        0x6Ba6F2Cad343BaC7eDBbf3887BF5c34f1f680D3f,  // Charity Donation 15%
        0xd8F14206C8Cd715030f77Fb7f162d31d54004361, // Team member 3 address 20%
        0x08861def471f6fC97B03616EA4309d197EdE003b  // Company Reserve 45%
    ];
    uint256[] private teamMemberPerc = [5, 15, 15, 20, 45]; // Percentage of the total balance to pay to each team member
    
    function setTeamMembers(address[] memory _teamMembers, uint256[] memory _teamMemberPerc) external onlyOwner {
    require(_teamMembers.length > 0, "Team members array should not be empty");
    require(_teamMembers.length <= 10, "Team members array and percentage array length should match");

    uint256 totalPerc = ownerPerc;

    for (uint256 i = 0; i < _teamMembers.length; i++) {
        teamMembers.push(payable(_teamMembers[i]));
        totalPerc += _teamMemberPerc[i];
    }
    require(totalPerc == 100, "The sum of ownerPerc and team member percentages should be 100");
    teamMemberPerc = _teamMemberPerc;
    }
