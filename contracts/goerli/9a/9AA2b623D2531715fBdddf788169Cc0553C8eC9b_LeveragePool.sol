...
contract LeveragePool is ILeveragePool, AbstractDependant, ERC20PermitUpgradeable {
    ...
    uint256 public constant override READY_TO_WITHDRAW_PERIOD = 1 days;
    ...
    uint256 public override a1_ProtocolConstant;
    ...
    function __UserLeveragePool_init(
        ...
        string memory fullSymbol = string(abi.encodePacked("dein", _projectSymbol, "Cover"));
        ...
        unchecked {
            ...
            a1_ProtocolConstant = 100 * PRECISION;
    ...
    /// @notice deploy lStable from user leverage pool using 2 formulas: access by policybook.
    ...
    function setRebalancingThreshold(uint256 _threshold) external override onlyPolicyBookAdmin {
        require(_threshold > 0, "LP: threshold can not be 0");
    ...
    function setProtocolConstant(
        ...
        a1_ProtocolConstant = _a1_ProtocolConstant;
        ...
        returns (address[] memory _coveragePools)
    {
        ...
        _coveragePools = new address[](to.uncheckedSub(offset));
    ...
    /// calculate the net mpl for the other LP pools
    function _deployLeverageStableToCoveragePools(address policyBookAddress)
        internal
        returns (uint256 deployedAmount)
    {
        ...
        uint256 _netMPL =
            totalLiquidity.mul(_policyBookFacade.userleveragedMPL()).uncheckedDiv(PERCENTAGE_100);

        uint256 _netMPLn = leveragePortfolioView.calcNetMPLn(address(_policyBookFacade));
        ...
        poolsLDeployedAmount[policyBookAddress] = deployedAmount;
    ...
    function _reevaluateProvidedLeverageStable(uint256 newAmount) internal {
        uint256 _newAmountPercentage = newAmount.mul(PERCENTAGE_100).tryDiv(totalLiquidity);

        if (
            (totalLiquidity > 0 && _newAmountPercentage > rebalancingThreshold) ||
            totalLiquidity == 0
        ) {
            _rebalanceInvestedPools();

            emit ProvidedLeverageReevaluated();
        }
    }

    /// @notice rebalance all pools provided by the leverage stable
    function _rebalanceInvestedPools() internal {
        for (uint256 i = 0; i < leveragedCoveragePools.length(); i = uncheckedInc(i)) {
            address policyBookAddress = leveragedCoveragePools.at(i);

            IPolicyBookFacade _policyBookFacade =
                leveragePortfolioView.getPolicyBookFacade(policyBookAddress);

            uint256 deployedAmount = _deployLeverageStableToCoveragePools(policyBookAddress);

            _policyBookFacade.deployLeverageFundsAfterRebalance(deployedAmount);
        }
    }

    function getEpoch(uint256 time) public view override returns (uint256) {
        return time.sub(epochStartTime).uncheckedDiv(EPOCH_DURATION).tryAdd(1);
    }

    function _getPremiumDistributionEpoch() internal view returns (uint256) {
        return block.timestamp.uncheckedDiv(PREMIUM_DISTRIBUTION_EPOCH);
    }
    ...
    function _getSTBLToBMIXRatio(uint256 currentLiquidity) internal view returns (uint256) {
        ...
        return currentLiquidity.mul(PERCENTAGE_100).uncheckedDiv(_currentTotalSupply);
    }
    ...
    function convertBMIXToSTBL(uint256 _amount) public view override returns (uint256) {
        ...
        return _amount.mul(_getSTBLToBMIXRatio(currentLiquidity)).uncheckedDiv(PERCENTAGE_100);
    }

    function convertSTBLToBMIX(uint256 _amount) public view override returns (uint256) {
        (, uint256 currentLiquidity, ) = getNewCoverAndLiquidity();

        return _amount.mul(PERCENTAGE_100).tryDiv(_getSTBLToBMIXRatio(currentLiquidity));
    }
        ...
        )
    {
        ...
        distributionEpoch = Math.min(
            currentEpoch,
            lastEpoch.tryAdd(MAX_PREMIUM_DISTRIBUTION_EPOCHS).tryAdd(1)
        );

        for (uint256 i = lastEpoch.tryAdd(1); i <= distributionEpoch; i = uncheckedInc(i)) {
    ...
    function forceUpdateBMICoverStakingRewardMultiplier() public override {
        ...
        for (uint256 i = 0; i < leveragedCoveragePools.length(); i = uncheckedInc(i)) {
            ...
            _leverageProvided = poolsLDeployedAmount[policyBookAddress].mul(PRECISION).tryDiv(
                totalLiquidity
            );

            _poolUR = _coveragepool.totalCoverTokens().mul(PERCENTAGE_100).tryDiv(
        ...
        rewardsGenerator.updatePolicyBookShare(
            _totalBmiMultiplier.uncheckedDiv(10**22),
            address(this)
    ...
    function addPremium(uint256 epochsNumber, uint256 premiumAmount)
        external
        override
        ...
        onlyCapitalPool
    {
        ...
        uint256 _totalSeconds =
            secondsToEndCurrentEpoch().tryAdd(
                (epochsNumber.uncheckedSub(1)).tryMul(EPOCH_DURATION)
            );

        _addPolicyPremiumToDistributions(
            _totalSeconds.tryAdd(VIRTUAL_EPOCHS.tryMul(EPOCH_DURATION)),
            premiumAmount
        );
    ...
    function _addPolicyPremiumToDistributions(uint256 _totalSeconds, uint256 _distributedAmount)
        internal
    {
        uint256 distributionEpochs =
            _totalSeconds.tryAdd(1).uncheckedDiv(PREMIUM_DISTRIBUTION_EPOCH).max(1);

        int256 distributedPerEpoch = int256(_distributedAmount.tryDiv(distributionEpochs));
        uint256 nextEpoch = _getPremiumDistributionEpoch().tryAdd(1);

        premiumDistributionDeltas[nextEpoch] += distributedPerEpoch;
        premiumDistributionDeltas[nextEpoch.tryAdd(distributionEpochs)] -= distributedPerEpoch;
    }
    ...
    function updateEpochsInfo() public override {
        ...
        uint256 _newDistributionEpoch =
            Math.min(getEpoch(block.timestamp), _lastDistributionEpoch.tryAdd(MAXIMUM_EPOCHS));
    ...
    function secondsToEndCurrentEpoch() public view override returns (uint256) {
        uint256 epochNumber =
            block.timestamp.trySub(epochStartTime).uncheckedDiv(EPOCH_DURATION).tryAdd(1);

        return epochNumber.tryMul(EPOCH_DURATION).trySub(block.timestamp.trySub(epochStartTime));
    }
        ...
        returns (WithdrawalStatus)
    {
        ...
        if (
            block.timestamp >= readyToWithdrawDate.tryAdd(READY_TO_WITHDRAW_PERIOD) &&
        ...
        withPremiumsDistribution
    {
        ...
        require(
            !claimingRegistry.hasProcedureOngoing(address(this), 0),
        ...
        require(
            capitalPool.canRequestWithdraw(_stblTokensToWithdraw),
            "LP: Not enough withdraw liquidity, please contact us"
        );
    ...
    function _requestWithdrawal(uint256 _tokensToWithdraw, address _user) internal {
        uint256 _readyToWithdrawDate = block.timestamp.tryAdd(capitalPool.getWithdrawPeriod());
    ...
    function _lockTokens(address _userAddr, uint256 _neededTokensToLock) internal {
        ...
        if (_currentLockedTokens > _neededTokensToLock) {
            this.transfer(_userAddr, _currentLockedTokens.uncheckedSub(_neededTokensToLock));
        } else if (_currentLockedTokens < _neededTokensToLock) {
            this.transferFrom(
                _userAddr,
                address(this),
                _neededTokensToLock.uncheckedSub(_currentLockedTokens)
            );
        }
    }
    ...
    function updateLiquidity(uint256 _lostLiquidity) external override onlyCapitalPool {
        ...
        _reevaluateProvidedLeverageStable(_lostLiquidity);
    ...
    function getAPY() public view override returns (uint256) {
        ...
        premiumDistributionAmount += premiumDistributionDeltas[currentEpoch.tryAdd(1)];
        return
            uint256(premiumDistributionAmount).mul(365).mul(10**7).uncheckedDiv(
...
interface ILeveragePortfolioView {
    ...
    function calcBMIMultiplier(ILeveragePool.BMIMultiplierFactors memory factors)
        external
        view
        returns (uint256);
    ...
    EIP712Upgradeable
{
     ...
     */
    function __ERC20Permit_init(string memory _name) internal onlyInitializing {
    ...
    function name() public view virtual override returns (string memory) {
        string memory exName = super.name();
        string memory newName =
            LibString.replace(exName, "Bridge Mutual", "Decentralized Insurance");

        return newName;
    }

    function symbol() public view virtual override returns (string memory) {
        string memory exSymbol = super.symbol();
        string memory newSymbol = LibString.replace(exSymbol, "bmiV2", "dein");
...
/// ref : https://gist.github.com/Vectorized/56ac210117f9baa15ac74a9ae779cd1f
