...
// File: contracts/marginTrading/interfaces/IDODOApprove.sol
...
interface IMarginTradingFactory {
    ...
    function isAllowedProxy(address _marginTradingAddress, address _proxy)
        external
        view
        returns (bool);
    ...
    function addFlashLoanProxy(address _marginTradingAddress, address _proxy)
        external;

    function removeFlashLoanProxy(
        address _marginTradingAddress,
        address _oldProxy
    ) external;
}
...
// File: contracts/marginTrading/MarginTradingFactory.sol
...
contract MarginTradingFactory is Ownable, IMarginTradingFactory {
    ...
    address immutable MARGIN_TRADING_TEMPLATE;
    ...
    //全仓
    //(userAddress => MarginTradingAddress)
    ...
    function getCreateMarginTradingAddress(
        ...
        _ad = Clones.predictDeterministicAddress(
            MARGIN_TRADING_TEMPLATE,
            keccak256(abi.encodePacked(_user, _num, _flag))
        );
    }
    ...
    function addFlashLoanProxy(address _marginTradingAddress,address _proxy) external {
        require(
            IMarginTrading(_marginTradingAddress).user() == msg.sender,
            "MarginTradingFactory: caller is not the user"
        );
    ...
    function createMarginTrading(
        ...
        if (_flag == 1) {
            marginTrading = Clones.cloneDeterministic(
                ...
                keccak256(
                    abi.encodePacked(
                        msg.sender,
                        crossMarginTrading[msg.sender].length,
                        _flag
                    )
                )
            );
            crossMarginTrading[msg.sender].push(marginTrading);
            emit MarginTradingCreated(
                msg.sender,
                marginTrading,
                crossMarginTrading[msg.sender].length,
                _flag
        ...
        if (_flag == 2) {
            marginTrading = Clones.cloneDeterministic(
                ...
                keccak256(
                    abi.encodePacked(
                        msg.sender,
                        isolatedMarginTrading[msg.sender].length,
                        _flag
                    )
                )
            );
        ...
        IMarginTrading(marginTrading).initialize(
            LendingPool,
            address(WETH),
            msg.sender
        );
        if (depositParams.length > 0) {
            ...
            if (_depositFlag == 2) {
                depositMarginTradingETH(marginTrading, false, 1);
            }
        }
        if (executeParams.length > 0) {
            ...
            _executeMarginTradingFlashLoans(
                marginTrading,
                _assets,
                _amounts,
                _modes,
    ...
    function _executeMarginTradingFlashLoans(
        ...
        IMarginTrading(_marginTradingAddress).executeFlashLoans(
            assets,
            amounts,
            modes,
            mainToken,
    ...
    function _depositMarginTradingERC20(
        ...
        if (_margin) {
            IMarginTrading(_marginTradingAddress).lendingPoolDeposit(
                _marginAddress,
                _marginAmount,
                _flag
            );
        }
        emit DepositMarginTradingERC20(
            _marginTradingAddress,
            _marginAddress,
            _marginAmount,
            _margin,
    ...
    function sendMarginTradingETH(
        ...
        IMarginTrading(_marginTradingAddress).sendETH(_to, _amount);
        emit SendMarginTradingETH(_marginTradingAddress, _to, _amount);
    }
    ...
    function sendMarginTradingERC20(
        ...
        IMarginTrading(_marginTradingAddress).sendERC20(
            _to,
            _tokenAddress,
            _amount
        );
        emit SendMarginTradingERC20(
            _marginTradingAddress,
            _tokenAddress,
            _to,
            _amount
