...
abstract contract ERC721Tradable is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    Counters.Counter _evaTokenIdCounter;
    string public _adamTokenUri = "https://adam.xxx/metadata/";
    string public _evaTokenUri = "https://eva.xxx/metadata/";
    ...
    constructor(
        ...
        _evaTokenIdCounter.increment();
        _supplyCounterForWhitelist.increment();
    }

    function totalSupply() public view override returns (uint256) {
        return _adamTokenIdCounter.current() + _evaTokenIdCounter.current() - 2;
    }
    ...
    function tokenURI(uint256 _tokenId) public view override(ERC721, ERC721URIStorage) returns (string memory) {
        if (SafeMath.mod(_tokenId, 2) == 0) {
            return string(abi.encodePacked(_evaTokenUri, SafeMath.div(_tokenId, 2).toString()));
        } else {
...
contract AdamAndEva is ERC721Tradable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    uint256 adamMaxSupply = 4000;
    uint256 evaMaxSupply = 1000;
    uint256 whitelistSupply;

    mapping(address => bool) whitelistedAddresses;

    bool canMint = false;
    bool canMintByWhitelistUser = false;

    uint256 tokenPrice = 19000000000000000; // 0.019 ETH
    uint256 tokenPriceForWhitelistUser = 9000000000000000; // 0.009 ETH

    constructor(
        uint256 _whitelistSupply
    )   ERC721Tradable("Adam&Eva", unicode"♂❤♀") {
    ...
    function setEvaMaxSupply(uint256 _newMaxSuppply) public onlyOwner {
        evaMaxSupply = _newMaxSuppply;
    }
    ...
    function newEva(uint256 _amount) public payable returns(uint256) {
        require(canMint, "Market is not started yet");
        require(_amount > 0, "Amount must be bigger than 0");
        require(_evaTokenIdCounter.current() - 1 + _amount <= evaMaxSupply, "Sold out");
        require(msg.value >= tokenPrice.mul(_amount), "Insufficient balance");

        uint256 mintedId;
        for(uint256 i; i<_amount; i++) {
            mintedId =  SafeMath.mul(_evaTokenIdCounter.current(), 2);
            _safeMint(msg.sender, mintedId);
            _evaTokenIdCounter.increment();
        }
    ...
    function newEvaForWhitelist(uint256 _amount) public payable returns(uint256) {
        require(canMintByWhitelistUser, "Market for whitelist user is not started yet");
        require(whitelistedAddresses[msg.sender], "You need to be whitelisted");
        require(_amount > 0, "Amount must be bigger than 0");
        require(_supplyCounterForWhitelist.current() - 1 + _amount <= whitelistSupply, "Sold out for whitelist user");
    ...
    function adjustEvaUri(string memory newUri) public onlyOwner {
        _evaTokenUri = newUri;
    }
