...
contract LilBottles is Ownable, ERC721A, ERC2981, PaymentSplitter {
    ...
    constructor() ERC721A("LIL-BOTTLES", "LIL-BOTTLES") PaymentSplitter(_shareholders, _shares) {
    ...
    function verify(MintRequest memory mintRequest, bytes memory signature) public view returns (bool) {
        return keccak256(abi.encodePacked(mintRequest.to,mintRequest.amount,mintRequest.price)).toEthSignedMessageHash().recover(signature) == signer;
    }
    ...
    function mintPrivate(MintRequest calldata _mintRequest, bytes calldata _signature) external payable {
        ...
        require(totalSupply().add(_mintRequest.amount) <= maxSupply, "Out of supply");
