...
contract MetadventureGen1 is ERC721Enumerable, ERC721Royalty, VRFConsumerBaseV2, Ownable {
    ...
    function tokenURIByServices(uint256 tokenId, bool permanent, bool initial) public view virtual returns (string memory) {
        require(_exists(tokenId), "Undefined token");

        string memory serviceUri;

        serviceUri = permanent ?
         initial ?
            initialArdriveBaseUri : ardriveBaseURI
        : initial ?
            initialIpfsBaseUri : ipfsBaseURI;
    ...
    function whitelistedMint(bytes32[] memory _proof, bytes32 _leaf, uint256 _mintAmount) public payable {
        ...
        require(isWhitelisted(_proof, _leaf), "Address not whitelisted");
