...
contract STBT is Ownable, ISTBT {
    ...
    function calculateAddedMargin(uint256 _value) view external returns (uint) {
        if (_value == 0 || totalSupply == 0) {
            return _value;
        }
	...
	uint _totalSupply = totalSupply;
        ...
        uint margin = 0;
        while(margin < 10**8) { 
            uint inputValue = _value + margin;
            uint deltaShares = inputValue * _totalShares / _totalSupply;
            _totalSupply += inputValue;
	    uint amount = deltaShares * _totalSupply / _totalShares;
	    if(amount>=_value) {
		return margin;
            } else { 
                _totalSupply -= inputValue;
		margin += (_value - amount)/2 + 1;
            }
        }
	revert("MARGIN_TOO_LARGE");
    }
