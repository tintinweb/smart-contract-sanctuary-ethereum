...
contract Staking is AccessControl, Pausable {
    ...
    */
    function getRewards(
        ...
        require(stake_.closed == false, "Stake closed");
    ...
    */
    function getActiveStakesByAccount(
        ...
        uint256 offset,
        uint256 limit
    ) public view returns (StakeData[] memory stakeData, uint256 allRewardsCanBeWithdraw) {
        StakeData[] memory stakedInformation = _stakedInformation[account];
        ...
        for (uint256 i = 0; i < stakedInformation.length; i++) {
            if(stakedInformation[i].closed == false){
                ...
                allRewardsCanBeWithdraw+=reward;
                counterForPush++;
            }
        }
    ...
    */
    ...
    /**
        ...
        @return boolean value indicating whether the operation succeeded
    */
    function returnAllStakesWithRewards() external onlyRole(ADMIN_ROLE) whenPaused returns (bool) {
        for (uint256 i = 0; i < _allAccounts.length; i++) {
    ...
    */
    ...
    function claimReward(uint256 id) external whenNotPaused returns (bool) {
        _claimReward(id);
        return true;
    }

    function claimAllReward() external whenNotPaused returns (bool) {
        address caller = msg.sender;
        uint256 countStakes = getStakesCount(caller);
        for (uint256 i = 0; i < countStakes; i++) {
            StakeData storage stake_ = _stakedInformation[caller][i];
    ...
    */
    ...
    /**
        ...
        @param id the number of stake
    */
    function _getStake(address account, uint256 id) internal view returns (StakeData memory stake) {
        require(id < _stakedInformation[account].length, "Invalid stake id");
        stake = _stakedInformation[account][id];
        (uint256 reward, ,uint256 timestampNextReward) = getRewards(account, id);
        stake.rewardCanWithdrawn = reward;
        stake.timestampNextReward = timestampNextReward;
    }
