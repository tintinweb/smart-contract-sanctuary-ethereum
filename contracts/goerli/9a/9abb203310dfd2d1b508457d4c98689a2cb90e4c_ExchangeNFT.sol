...
// File: exchange/libraries/TransferHelper.sol
...
// File: exchange/libraries/ECDSA.sol
...
// File: exchange/interfaces/IStableToken.sol
...
// File: exchange/interfaces/IERC165.sol
...
// File: exchange/interfaces/IERC721.sol
...
contract ExchangeNFT is Ownable {
    ...
    modifier verifySignature(
        uint256 nonce,
        address[7] memory _tradeAddress,
    ...
    function checkFeeProductExits(
        address[7] memory _tradeAddress,
        uint256[5] memory _attributes
    ) private returns (uint256 amount, uint256 feeOwner, uint256 feeAdmin, uint256 feeTeam) {
        ...
        // Check fee for team
        if (_tradeAddress[6] != address(0)) {
            feeTeam = _attributes[0].mul(_attributes[4]).div(1000);
            totalFeeTrade += feeTeam;
        }
        amount = _attributes[0].sub(totalFeeTrade);
    }

    // Buy NFT normal by token ERC-20
    // address[7]: buyer, seller, token, fee, feeAdmin, NFT contract, teamFeeAddress
    // uint256[5]: amount, tokenId, feePercent, feePercentAdmin, teamFeePercent
    function buyNFTNormal(
        ...
        (uint256 amount, uint256 feeOwner, uint256 feeAdmin, uint256 feeTeam) = checkFeeProductExits(
        ...
        IERC721(tradeInfo.tradeAddress[5]).safeTransferFrom(
            tradeInfo.tradeAddress[1],
            msg.sender,
        ...
        verifySignature(nonce, _tradeAddress, _attributes, signature)
    {
        ...
        if (feeTeam != 0) {
            TransferHelper.safeTransferETH(tradeInfo.tradeAddress[6], feeTeam);
        }
    ...
    function auctionNFT(
        ...
        if (tradeInfo.tradeAddress[1] == admin) {
            require(
                IERC721(tradeInfo.tradeAddress[5]).isApprovedForAll(admin, address(this)),
                "tokenId do not approve for contract"
            );
        } else {
            require(
                IERC721(tradeInfo.tradeAddress[5]).getApproved(tradeInfo.attributes[1]) == address(this),
                "tokenId do not approve for contract"
            );
        }
