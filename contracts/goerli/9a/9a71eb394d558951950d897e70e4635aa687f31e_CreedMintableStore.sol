...
*/
 contract ERC1155Metadata_URI is HasTokenURI {
    ...
    function tokenUri(uint256 _id) external virtual  view returns (string memory) {
...
contract StoreBase is ERC165, Ownable, ERC1155Metadata_URI,  ERC1155Base {
    ...
    function _mint(address from, address to,uint256 _id, uint256 _supply, string memory _uri) internal {
        require(creators[_id] == address(0x0) || creators[_id] == from, "Token is already minted");
        require(_supply != 0, "Supply should be positive");
        require(bytes(_uri).length > 0, "Uri should be set");

        creators[_id] = from;
      
        _balances[_id][to] = _balances[_id][to].add(_supply);
...
contract CreedMintableStore is Ownable, SignerRole, StoreBase {
    string public name;
    ...
    event SetBlockLimit(address owner, uint256 newBlockLimit);
    //modifier
    modifier isOperator() {
    ...
    function mint(
        ...
        bytes32 msgHash = prepareHash(from, id, blockExpiry);
        require(
    ...
    function setBlockLimit(uint256 newBlockLimit) external onlyOwner{
        blockLimit = newBlockLimit;
        emit SetBlockLimit(msg.sender, newBlockLimit);
    }
    ...
    function prepareHash(address from, uint256 id, uint256 blockExpiry)
        public
        view
        returns (bytes32)
    {
        return
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encodePacked(
                            abi.encodePacked(this, id, blockExpiry, from)
                        )
                    )
                )
            );
