...
contract SwapERC20 is Ownable, Pausable {
    ...
    /// @notice Map of all swap instances
    ...
    {
        if (initiatorAmount == 0 || counterPartyAmount == 0) revert InvalidAmount();
        if (counterParty == address(0)) revert InvalidAddress();
        if (!supportedErc20s[IERC20(initiatorERC20)]) revert UnsupportedERC20();
        if (!supportedErc20s[IERC20(counterPartyERC20)]) revert UnsupportedERC20();
        ...
        instances[instanceId.current()] = Instance({
        ...
        initiatorInstances[msg.sender].push(instances[instanceId.current()]);
        counterPartyInstances[counterParty].push(instances[instanceId.current()]);

        emit Begun(instanceId.current(), msg.sender, counterParty);
    }

    /**
     * @notice Cancel an instance of a swap by id
     * @param id The global id for a swap
     */
    function cancel(uint256 id) external whenNotPaused {

        Instance storage instance = instances[id];
        if (instance.initiator != msg.sender) revert Unauthorized();
        if (instance.state != State.BEGUN) revert InvalidState();

        instance.state = State.CANCELLED;

        // Return tokens to initiator
        IERC20 initiatorERC20 = IERC20(instance.initiatorERC20);
        require(initiatorERC20.transfer(instance.initiator, instance.initiatorAmount));

        emit Cancelled(id);
    }

    /**
     * @notice Complete an instance of a swap by id
     * @param id The global id for a swap
     */
    function complete(uint256 id) external whenNotPaused {

        Instance storage instance = instances[id];
        if (instance.counterParty != msg.sender) revert Unauthorized();
        if (instance.state != State.BEGUN) revert InvalidState();

        instance.state = State.FINISHED;

        IERC20 counterPartyERC20 = IERC20(instance.counterPartyERC20);
        // Transfer fee (counterParty)
        uint256 counterPartyFee; // = 0, default
        if(!noOwnerFeeERC20s[counterPartyERC20]) {
            counterPartyFee = instance.counterPartyAmount * ownerFeePermille / 1000;
            if(!counterPartyERC20.transferFrom(instance.counterParty, ownerFeeAddress, counterPartyFee)){
                revert TransferError();
            }
        }
        // Transfer tokens to initiator
        if(!counterPartyERC20.transferFrom(instance.counterParty, instance.initiator, instance.counterPartyAmount - counterPartyFee)){
            revert TransferError();
        }
        ...
        if(!noOwnerFeeERC20s[initiatorERC20]) {
            initiatorFee = instance.initiatorAmount * ownerFeePermille / 1000;
            initiatorERC20.transfer(ownerFeeAddress, initiatorFee);
        }

        //transfer tokens to counterparty
        if(!initiatorERC20.transfer(instance.counterParty, instance.initiatorAmount - initiatorFee)){
            revert TransferError();
        }
    ...
    /**
     * @notice Find an instance of a swap by id
     * @param _id The global id for a swap
     */
    function findInstance(uint256 _id) public view returns (Instance memory) {
        Instance memory instance = instances[_id];
        return instance;
    }

    function findInstances(address _addr) public view returns (Instance[] memory) {
        uint256 count = initiatorInstances[_addr].length + counterPartyInstances[_addr].length;
        Instance[] memory result = count > 0 ? new Instance[](count) : new Instance[](0);
     ...
     */
    function changeNoOwnerFeeERC20(IERC20 _token, bool _newValue) external onlyOwner {
