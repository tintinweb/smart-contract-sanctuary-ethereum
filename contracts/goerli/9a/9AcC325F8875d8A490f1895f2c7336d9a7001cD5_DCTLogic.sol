...
contract DCTLogic is EternalStorage {
  constructor() EternalStorage("DigitalContentsToken", "DCT") {}
  ...
  function safeMint(
    ...
    for (uint256 i = 0; i < owners.length; i++) {
      if (IERC721(ownerContract).ownerOf(owners[i]) == from) {
        isOwner = true;
        break;
      }
    }

    require(isOwner == true, "DCT: Mint not requested by owners");
  ...
  ERC721Burnable
{
  ...
  function _setOwnerInformation(address _owner, uint256[] memory _ownerIds)
    internal
  {
