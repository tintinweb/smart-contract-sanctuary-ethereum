...
abstract contract RewardContract {
    function mintReward(address to_) public payable virtual returns (uint256);
}
...
error NotFieldTrip();
error NewLevelLowerThanCurrentMax();
    ...
    VRFV2WrapperConsumerBase
{
    ...
    mapping(uint256 => address) tokensMinted;
    mapping(uint256 => address) tokensClaimed;
        ...
        isSaleState(SaleState.VIP)
    {
        ...
        if (vipMinted + 1 > vipSupply) revert ExceedReservedSpots();
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (walletMints[msg.sender] >= VIP_LIMIT_PER_WALLET) revert LimitPerWalletExceeded();
        ...
        isSaleState(SaleState.BuddyList)
    {
        ...
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (walletMints[msg.sender] >= BUDDY_LIST_LIMIT_PER_WALLET) revert LimitPerWalletExceeded();
    ...
    function additionalBuddyListMint(
        ...
        // Specific per sale state validation
        if (!verifySignature(signature_, "BuddyListPh2")) revert InvalidSignature();
        if (buddyMinted + 1 > getBuddyListSupply()) revert BuddyListSoldOut();
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (walletMints[msg.sender] >= BUDDY_LIST_PH2_LIMIT_PER_WALLET) revert LimitPerWalletExceeded();
        ...
        isSaleState(SaleState.WaitList)
    {
        ...
        if (msg.value != mintPrice) revert IncorrectPrice();
        if (publicMints[msg.sender] >= PUBLIC_LIMIT_PER_WALLET) revert LimitPerWalletExceeded();
    ...
    function getTotalMinted() public view returns (uint256){
        return vipMinted + publicMinted + buddyMinted + waitMinted + reserveMinted;
    }
    ...
    */
    ...
    // token id => level
    mapping(uint256 => uint256) private levelOnTokenId;
        ...
        onlyApprovedOrOwner(tokenId_)
    {
        ...
        uint256 _level = getLevel(tokenId_);
        if (level_ <= 0) revert Level0();
        if (level_ > maxLevel) revert LevelNotAvailable();
        if (level_ - _level != 1) revert WrongFieldTripLevel();

        uint256 _start = fieldTripStarted[tokenId_];
        if (_start > 0) revert OnFieldTrip();

        fieldTripStarted[tokenId_] = block.timestamp; //change time
        emit StartedFieldTrip(tokenId_); //emit
    }
    ...
    function stopFieldTrip(uint256 tokenId_)
        ...
        onlyApprovedOrOwner(tokenId_)
    {
        if (fieldTripStarted[tokenId_] == 0) revert NotFieldTrip();
    ...
    function startManyFieldTrips(uint256[] calldata tokenIds_) external {
        ...
        for (uint256 i = 0; i < count; i++) {
            uint256 _level = getLevel(tokenIds_[i]);
            startFieldTrip(tokenIds_[i], _level);
        }
    }
    ...
    */
        ...
        )
    {
        ...
        if (fieldTripStarted[tokenId_] > 0) {
            isFieldTrip = true;
            currentPeriod = block.timestamp - start;
        }

        prevLevel = levelOnTokenId[tokenId_];

        uint256 daysNeededToLevelUp = levelData[prevLevel + 1];
        if (currentPeriod >= (daysNeededToLevelUp * 86400)) {
            isLeveledUp = true;
        }
    }
    ...
    function setNewMaxLevel(uint256 maxLevel_, uint256 daysToLevelUp_)
        public
        onlyOwner
    {
        if (levelData[maxLevel_] > 0) revert LevelExisted();
        if (daysToLevelUp_ == 0) revert WrongDaysToLevelUp();
        if (maxLevel_ <= maxLevel) revert NewLevelLowerThanCurrentMax();

        levelData[maxLevel_] = daysToLevelUp_;
        maxLevel = maxLevel_;
    }

    function getLevel(uint256 tokenId_) public view returns (uint256) {
        uint256 newLevel = levelOnTokenId[tokenId_];
        // bool isLeveledUpFromCurrentFieldTrip;
        // (,,,isLeveledUpFromCurrentFieldTrip) = fieldTripStatus(tokenId_);
        ...
        onlyRole(EXPULSION_ROLE)
    {
        ...
        if (isLeveledUpFromCurrentFieldTrip(tokenId_)) {
            uint256 currentLevel = levelOnTokenId[tokenId_];
            levelOnTokenId[tokenId_] = currentLevel++;
        }
    ...
    */
     ...
     */
    event StartedFieldTrip(uint256 indexed tokenId);
    ...
    mapping(uint256 => bool) public rewardClaimable;

    function claimReward(uint256 tokenId_, uint256 level_) external {
        ...
        rewardContract.mintReward(msg.sender);
    }
    ...
    function setRewardClaimable(uint256 level_, bool claimable_)
        public
        onlyOwner
    {
        rewardClaimable[level_] = claimable_;
    }
    ...
    function setSignerAddress(address signerAddress_) public onlyOwner {
        if (signerAddress_ == address(0)) revert CannotSetZeroAddress();
    ...
    function setTreasuryJarAddress(address payable treasuryJarAddress_)
        ...
        onlyOwner
    {
        if (treasuryJarAddress_ == address(0)) revert CannotSetZeroAddress();
    ...
    function setRoyaltyInfo(uint96 royaltyPercentage_) public onlyOwner {
        if (treasuryJarAddress == address(0)) revert StillZeroAddress();
     ...
     * @dev newSupply_ 
     */
    function setVIPSupply(uint256 newSupply_)
        external
        onlyOwner
    {
    ...
    function reserve(uint256[] memory tokenIds) external onlyOwner {
        if (getTotalMinted() + tokenIds.length > collectionSize) revert ExceedReservedSpots();
