...
contract Matr1x2061 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable{
    ...
    address private constant withdrawAddress = 0xD3973e234744D115e963B76A0F80A81829df9812;
    ...
    mapping(string => uint8) public _lockEventState;
    //tokenId => lock counts
    mapping(uint256 => uint8) public _lockCounts; 
    //eventId => ( tokenId=> lockState),lockState: 0-unlock, 1-lock
    mapping(string => mapping(uint256 => uint8)) private _eventTokenLock;
    ...
    function initialize() initializer public {
        ...
        yatc = YATC(0xDD4fa24998C20807F4F1F9C772454A9c96826B81);
        ...
        verifySigner = 0xD3973e234744D115e963B76A0F80A81829df9812;
    ...
    // =================v2 start==============
    function lock(string calldata _eventId,uint256[] calldata _tokenIds) public {
    ...
    function unlock(string calldata _eventId,uint256[] calldata _tokenIds) public {
        require(_tokenIds.length <= 20,"Max limit one call");
        for(uint256 i=0;i<_tokenIds.length;){
            uint256 _tokenId=_tokenIds[i];
            require(msg.sender == ownerOf(_tokenId),"Caller not owner");
    ...
    function setEventState(string calldata _eventId,uint8 _state) public onlyRole(MINTER_ROLE){
        _lockEventState[_eventId]=_state;
    }

    function queryEventLockState(string calldata _eventId,uint256[] calldata _tokenIds) public view returns (uint8[] memory){
        uint8[] memory _states=new uint8[](_tokenIds.length);
        for(uint256 i=0;i<_tokenIds.length;){
