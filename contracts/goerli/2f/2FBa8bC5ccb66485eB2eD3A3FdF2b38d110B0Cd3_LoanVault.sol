...
contract LoanVault {
  ...
  function withdrawalAmount(address _lender) public view returns (uint256) {
    if (lendersBalance[_lender] == 0 || lockedAmount == 0) {
      return 0;
    }

    uint256 participation = lendersBalance[msg.sender] / lockedAmount * 100;
  ...
  function withdraw() public {
    ...
    delete lendersBalance[msg.sender];
  ...
  function loan(
    ...
    nftFloorPriceFeed = AggregatorV3Interface(_collateralTargetAddress);
    (, int256 price, , , ) = nftFloorPriceFeed.latestRoundData();

  require(
    address(this).balance >= uint256(price),
      "Insufficient liquidity in the vault"
    );

    unchecked {
      require(uint256(price) >= 0, "Loan amount should be greater than 0");
      require((lockedAmount -= uint256(price)) >= 0, "Insufficient liquidity on vault");
      lockedAmount -= uint256(price);
    }
  ...
  function currentPaybackAmount(address _borrower) public view returns (uint256) {
    ...
    return foundLoan.amount + interest;
  }
...
//    nftFloorPriceFeed = AggregatorV3Interface(_collectionAddress);
