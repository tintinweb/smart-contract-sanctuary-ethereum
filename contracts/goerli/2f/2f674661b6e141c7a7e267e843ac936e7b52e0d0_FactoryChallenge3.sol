...
import {BoringToken} from "../tokens/boring.sol";
import {InSecureumLenderPool} from "./Challenge1.sol";
import {InsecureDexLP} from "./Challenge2.sol";

import {BorrowSystemInsecureOracle} from "./Challenge3.sol";

contract FactoryChallenge3 is IChallengeFactory {
  function deploy() external returns (address[] memory ret) {
    // create the tokens
    IERC20 token0 = IERC20(new InsecureumToken(30000 ether));
    IERC20 token1 = IERC20(new BoringToken(20000 ether));
        
    // setup dex & oracle
    InsecureDexLP oracleDex = new InsecureDexLP(address(token0),address(token1));
    ...
    address flashLoanPool = address(new InSecureumLenderPool(address(token0)));
    // send tokens to the flashloan pool
    token0.transfer(flashLoanPool, 10000 ether);
    
    // setup the target conctract
    address challenge = address(new BorrowSystemInsecureOracle(address(oracleDex), address(token0), address(token1)));

    // lets fund the borrow
    token0.transfer(challenge, 10000 ether);
    token1.transfer(challenge, 10000 ether);
    
    
    ret = new address[](5);
    ret[0] = challenge;
    ret[1] = address(token0); // tokenInsecureum
    ret[2] = address(token1); // tokenBoring
    ret[3] = address(oracleDex);
    ret[4] = flashLoanPool;
  }
