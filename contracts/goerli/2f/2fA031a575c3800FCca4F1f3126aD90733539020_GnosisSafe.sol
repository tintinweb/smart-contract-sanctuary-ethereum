    ...
    GuardManager
{
    ...
    IPayLaterLoanCore internal loanCore;
    // keccak256(
    ...
    function setup(
        ...
        IPayLaterLoanCore core
    ) external {
        // setupOwners checks if the Threshold is already set, therefore preventing that this method is called twice
        setupOwners(_owners, _threshold);
        if (fallbackHandler != address(0)) internalSetFallbackHandler(fallbackHandler);
        require(address(core) != address(0), "Invalid LoanCore");
        loanCore = core;
    ...
    function execTransaction(
        ...
        {   if(tokenId == 0 || tokenId == 1) {
                require(address(loanCore) != address(0), "Null LoanCore");
                require(loanCore.checkForNFTsPayLaterLoan(to, tokenId) != true, "Asset Under PayLaterLoan");
                require(loanCore.checkForNFTsUnderRental(to, tokenId) != true, "Asset Under PayLaterLoan");
            }
        }
    ...
    function getTransactionHash(
        ...
        return keccak256(encodeTransactionData(to, value, data, operation, safeTxGas, baseGas, gasPrice, gasToken, refundReceiver, _nonce, tokenId));
    }

    function getApprovalForDefaultedNFTTransfer(address tokenAddress, uint256 tokenId, uint256 tokenType) public payable returns(bool) {
        {
            address coreaddress = address(loanCore);
            address payable corepayableaddress = payable(coreaddress);
            // address payable paylatercore = address(uint160(core));

            require(msg.sender == coreaddress, "Unauthorized Access");
            require(tokenType == 0 || tokenType == 1, "Unsupported TokenType");
            if(tokenType == 0) {
                IERC721(tokenAddress).approve(corepayableaddress, tokenId);
            } else if(tokenType == 1) {
                IERC1155(tokenAddress).setApprovalForAll(corepayableaddress, true);
            }
            return true;
        }
    }
