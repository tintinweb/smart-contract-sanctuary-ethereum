...
contract RainshowerUniswapModule is Modifiers
{
    ...
    // data LAYOUT:
    // token tokensForBorrow - holds token information, including address and ammount
    // token swapData.baseToken - holds token information, including address and ammount
    // swapData swap
    bytes public data;
    ...
    function onFundWithTokens () external returns(bool res) {
        require (state == 0);
        state = 1;
        token memory tokensForBorrow;
        (
            tokensForBorrow,
        ) = abi.decode(data, (
            token, swapData
        ));
        res = IERC20Minimal(tokensForBorrow.tokenAddress).transferFrom(msg.sender, address(this), tokensForBorrow.amount);
        return res;
    }

    function onFundWithMarginAndOpen (uint256 _extraMargin) external returns(uint256) {
        require (state == 1);
        state = 2;
        ...
        (
            tokensForBorrow, swap
        ) = abi.decode(data, (
            token, swapData
        ));
        IV3SwapRouter swapRouter;
        swapRouter = IV3SwapRouter(swap.swapRouterAddress);
        
        uint256 price = IuniswapV3Oracle(swap.oracleAddress).getSpotPrice(swap.pool, swap.period, 10**18, swap.baseToken, tokensForBorrow.tokenAddress);
        uint256 tokenBalance = FixedPointMathLib.mulWadUp(swap.maintanenceMargin, price) + _extraMargin;

        IERC20Minimal(swap.baseToken).transferFrom(borrowTaker, address(this), tokenBalance);
        return swapExactInputSingle(tokensForBorrow.tokenAddress, swap.baseToken, tokensForBorrow.amount, swap.poolFee, swapRouter);
    }

    function onClose () external onlyAddress(borrowTaker) returns(bool) {
        require (state == 2);
        state = 3;
        ...
        IV3SwapRouter swapRouter;
        swapRouter = IV3SwapRouter(swap.swapRouterAddress);
        uint256 tokenBalance = IERC20Minimal(swap.baseToken).balanceOf(address(this));
        uint256 amountOut = swapExactOutputSingle(swap.baseToken, tokensForBorrow.tokenAddress, swap.poolFee, tokensForBorrow.amount, tokenBalance, swapRouter);

        console.log("tokenBalance: %s", tokenBalance);
        IERC20Minimal(swap.baseToken).transfer(borrowTaker, tokenBalance);
        console.log("tokenBalance: %s", amountOut);
        IERC20Minimal(tokensForBorrow.tokenAddress).transfer(borrowMaker, amountOut);
    ...
    function swapExactInputSingle(address _tokenIn, address _tokenOut, uint256 _amountIn, uint24 _poolFee, IV3SwapRouter _swapRouter) internal returns (uint256 amountOut) {
    ...
    function swapExactOutputSingle(address _tokenIn, address _tokenOut, uint24 _poolFee, uint256 amountOut, uint256 amountInMaximum, IV3SwapRouter _swapRouter) internal returns (uint256 amountIn) {
        ...
        TransferHelper.safeApprove(_tokenIn, address(_swapRouter), amountInMaximum);
