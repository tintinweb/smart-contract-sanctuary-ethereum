...
contract CHDNFTMarketplace is AccessControl {
    ...
    constructor(address _nftAddress, address _listAddress) {
    ...
    function setSaleData(
        ...
        presaleDatas[_time] = preSaleData(_startTime, _endTime, _startId, _endId, _whiteList);
    }

    function sellNFT(uint256 _offerId, uint256 _salesTime) public payable{
        require(msg.value == salesPrice, "The price for all approved NFTs is 1 ether.");
        ...
        require(presaleFinished[_offerId] == true, "NFT is already sold");
        require(whiteList(listAddress).checkWhiteListAmount(msg.sender) >= 1);
        // msg.sender.transfer(msg.value);
        presaleFinished[_offerId] = true;
        whiteList(listAddress).addWhiteListUsed(msg.sender);
...
contract whiteList is AccessControl {
    bytes32 SALE_ROLE = keccak256('SALE_ROLLE');
    constructor() {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
