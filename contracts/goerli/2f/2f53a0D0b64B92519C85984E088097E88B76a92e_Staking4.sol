...
contract Staking4 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Deposit(address indexed user, uint128 stakeAmount, uint128 stakeType, uint128 stakePeriod, uint256 time);
    event Withdraw(address indexed user, uint128 stakeAmount, uint128 stakeType, uint128 rewardAmount, uint256 time);
    event Compound(address indexed user, uint128 rewardAmount, uint128 stakeType, uint256 time);
    ...
    event EmergencyWithdrawn(address indexed user, uint128 amount, uint128 stakeType, uint256 time);
    ...
    function initialize(address _ownerWallet, address _stakedToken, 
                        ...
                        uint128 _withdrawlsFees, uint128 _rewardRate, uint128 _emergencyFees, uint128 _platformFee) 
                        external initializer {
        require(_emergencyFees > 0, "EMERGENCY_FEES_CANT_BE_ZERO");
        require(_platformFee > 0, "PLATFORM_FEE_CANT_VE_NULL");
        ...
        stakeTypesList.push(DetailsLibraryUpdated.StakeTypeData(_stakeType,_stakePeriod,_depositFees,_withdrawlsFees,_rewardRate,0));

    }
    
    function deposit(address user,uint128 _amount, uint128 _stakeType) external payable nonReentrant onlyOwner returns(uint128 emitAmount, uint128 _period) {
        ...
        uint128 _depositFees = stakeType.depositFees;
        uint128 fees;
        uint128 actualAmount = _amount;

        if(_depositFees !=0) {
            fees = _amount * _depositFees *100 / 1000000;
        ...
        if(stakes.stakeAmount == 0) {
            ...
            stakeType.totalStakedIn = uint128(stakeType.totalStakedIn.add(actualAmount));
            userTotalPerPool[user][_stakeType] +=  actualAmount;
            totalStaked += actualAmount;
            emit Deposit(user, actualAmount, _stakeType, stakeType.stakePeriod, block.timestamp);
            emitAmount = actualAmount;
            ...
            if(uint128(block.timestamp) < userStakesData[user][_stakeType].fullWithdrawlTime) {
                ...
                stakeTimeTillNow = userStakesData[user][_stakeType].fullWithdrawlTime - userStakesData[user][_stakeType].lastClaimTime;
                if(stakeTimeTillNow > 0 ) totalRewardTillNow = rewardCalculation(actualAmount, _stakeType, stakeTimeTillNow); 

            }
            userStakesData[user][_stakeType].stakeAmount += totalRewardTillNow;

            stakeType.totalStakedIn += actualAmount + totalRewardTillNow;
            userTotalPerPool[user][_stakeType] +=  actualAmount + totalRewardTillNow;
            totalStaked += actualAmount + totalRewardTillNow;
            ...
            emitAmount = totalRewardTillNow + actualAmount;
            emit Deposit(user, emitAmount, _stakeType, stakeType.stakePeriod, block.timestamp);
        ...
        if(fees > 0) 
        {
            success = stakedToken.transferFrom(user, ownerWallet, fees);
            if(!success) revert();
            
        }

         success = stakedToken.transferFrom(user, address(this), actualAmount);
        if(!success) revert();

    }

    function compound(address user, uint128 _stakeType) external payable nonReentrant onlyOwner returns(uint128, uint128) {
        require(stakeTypeExist[_stakeType], "STAKE_TYPE_DOES_NOT_EXIST");
        ...
        uint128 stakeTimeTillNow;
            ...
            stakeType.totalStakedIn +=  totalRewardTillNow;
            userTotalPerPool[user][_stakeType] +=  totalRewardTillNow;
            totalStaked += totalRewardTillNow;
            if(totalRewardTillNow > 0)
            claimReward(address(this), totalRewardTillNow);

            emit Compound(user, totalRewardTillNow, _stakeType, block.timestamp);
            return (totalRewardTillNow, stakeType.stakePeriod);


    }

    function withdraw(address user, uint128 _amount, uint128 _stakeType) external payable nonReentrant onlyOwner returns(uint128, uint128) {
        ...
        emit Withdraw(user, actualAmount, _stakeType, rewardTillNow, block.timestamp);
        return (actualAmount, stakeType.stakePeriod);
    }
    ...
    function rewardCalculation(uint128 _amount, uint128 _stakeType, uint128 _time) public view returns(uint128) {
        ...
        uint128 interest = (_amount * rate * 10 * (_time / 60) * 1 minutes) / (1000 * 365 minutes);
        return interest;
    ...
    function addStakedType(uint128 _stakeType, uint128 _stakePeriod, uint128 _depositFees, uint128 _withdrawlFees, uint128 _rewardRate) external onlyOwner {
        ...
        stakeTypesList.push(DetailsLibraryUpdated.StakeTypeData(_stakeType,_stakePeriod,_depositFees,_withdrawlFees,_rewardRate,0));

        emit AddStakeType(_stakeType, _stakePeriod, _depositFees, _withdrawlFees, _rewardRate);
    ...
    function deleteStakeType(uint128 _stakeType) external onlyOwner {
        require(stakeTypeExist[_stakeType], "STAKE_TYPE_DOES_NOT_EXIST");
    ...
    function emergencyWithdraw(address user, uint128 _stakeType) external payable onlyOwner returns(uint128, uint128){
        DetailsLibraryUpdated.StakeTypeData storage stakeType = stakeTypesList[_stakeType];
        DetailsLibraryUpdated.eachTransaction storage stakes = userStakesData[user][_stakeType];
        ...
        emit EmergencyWithdrawn(user, amount, _stakeType, block.timestamp);
        return (amount, stakeType.stakePeriod);
