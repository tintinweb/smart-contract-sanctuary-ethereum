...
contract NFC2023 is ERC721, ERC721Enumerable, ERC2981ContractWideRoyalties, Ownable {
    ...
    https://opengem.com
    */
    string public constant NFT_INFO = "Access to the Non-Fungible Conference on June 7th & 8th, 2023 from 9am-6pm on both days.";
    string public constant SHA256_PROVENANCE = "d6357cf6261516ad796bde082f473fdbeeab66b5221044e0c91cc58ef4516ba3";
    ...
    string public jsonForFetchingOnly;
    string public imgBackupTx;
    bool public salesLocked = false;

    modifier onlyPublicSales {
    ...
    constructor(address _royaltyWallet, address _paidWallet, bytes32 _merkleRoot, uint256 _basePrice, string memory _jsonForFetchingOnly, address _storageAddress)
        ERC721("Non Fungible Conference 2023", "NFCSummit2023") 
    {
        _setRoyalties(_royaltyWallet, 1000);
        paidWallet = _paidWallet;
        merkleRoot = _merkleRoot;
        basePrice = _basePrice;
        jsonForFetchingOnly = _jsonForFetchingOnly;
    ...
    function updatePhase(uint256 _phase) external onlyOwner {
        storageContract.updatePhase(_phase);
    }

    function updateJsonFetching(string calldata _jsonForFetchingOnly) external onlyOwner {
        jsonForFetchingOnly = _jsonForFetchingOnly;
    }
    ...
    function tokenURIarweave(uint256 _tokenId) external view ifExist(_tokenId) returns (string memory) {
        return "https://arweave.net/jWr16e0Yt9H__uzxCf8ACHKbmh8Q7RScNZMoFunuo98";
    }

    function tokenURIipfs(uint256 _tokenId) external view ifExist(_tokenId) returns (string memory) {
        return "https://gateway.pinata.cloud/ipfs/QmdjxKfEbY5xH2BACS9VvbWYYybLnRQRadiTevkmFKvHrc";
    }
    ...
    function append721Coupon(address[] calldata _nft_contract, uint256[] calldata _phase) external onlyOwner  {
        storageContract.append721Coupon(_nft_contract, _phase);
    }

    function discountedMint(address _nft_contract, uint256 _nft_id) external payable {
        require(!salesLocked, "Sales are closed.");
        ...
        require(msg.value >= (basePrice - (storageContract.phase() * basePrice) / 100), "Ether value sent is not correct.");
    ...
    function normalMint() external payable onlyPublicSales {
        require(!salesLocked, "Sales are closed.");
    ...
    function freeMint(bytes32[] calldata _merkle) external {
    ...
    function _approve(address _to, uint256 _tokenId) internal override(ERC721) onlyPublicSales {
        super._approve(_to, _tokenId);
    }

    function _setApprovalForAll(address _owner, address _operator, bool _approved) internal override(ERC721) onlyPublicSales {
        super._setApprovalForAll(_owner, _operator, _approved);
    }

    function _transfer(address _from, address _to, uint256 _tokenId) internal override(ERC721) onlyPublicSales {
...
interface INFCStorage {
    ...
    function append721Coupon(address[] calldata _nft_contract, uint256[] calldata _phase) external;
    ...
    function updatePhase(uint256 _phase) external;
}
