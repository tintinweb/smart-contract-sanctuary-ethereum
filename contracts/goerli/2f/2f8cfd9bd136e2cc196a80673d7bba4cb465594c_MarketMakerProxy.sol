...
contract MarketMakerProxy is LibDecoder {
    ...
    function isValidSignature(bytes32 orderHash, bytes memory signature) external view returns (bytes32) {
        require(SIGNER == ecrecoverAddress(orderHash, signature), "MarketMakerProxy: invalid signature");
        return keccak256("isValidWalletSignature(bytes32,address,bytes)");
    }

    function ecrecoverAddress(bytes32 orderHash, bytes memory signature) public view returns (address) {
