...
contract WETHGateway is IWETHGateway, Ownable {
  ...
  function withdrawETH(
    ...
    IBToken aWETH = IBToken(ILendingPool(lendingPool).getReserveData(address(WETH)).bTokenAddress);
...
interface ILendingPool {
  ...
  function withdrawValidation(address user, address asset, uint256 amount) external;
}
...
import {IBakeryIncentivesController} from './IBakeryIncentivesController.sol';
...
interface IBToken is IERC20, IScaledBalanceToken, IInitializableBToken {
  ...
  /**
   * @dev Transfers bTokens in the event of a borrow being liquidated, in case the liquidators reclaims the bToken
   ...
   **/
  function getIncentivesController() external view returns (IBakeryIncentivesController);
 ...
 * - Owned by the Bakery Governance
 * @author Bakery Finance
 **/
interface ILendingPoolAddressesProvider {
  ...
  event SafeGuardUpdated(address indexed newAddress);
  ...
  function getSafeGuard() external view returns (address);

  function setSafeGuardImpl(address safeGuard, address dusd) external;
}
...
interface IInitializableBToken {
   ...
   * @param treasury The address of the Bakery treasury, receiving the fees on this bToken
  ...
  function initialize(
    ...
    IBakeryIncentivesController incentivesController,
    uint8 bTokenDecimals,
...
interface IBakeryIncentivesController {
 ...
 * @author Bakery Finance
 * @notice Defines the error messages emitted by the different contracts of the Bakery protocol
