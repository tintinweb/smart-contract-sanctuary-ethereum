...
contract UniqueNameToken is ERC721, ERC721URIStorage, EIP712, AccessControl, ReentrancyGuard {
    ...
    function mint(bytes calldata _data) external payable nonReentrant {
        ...
        (bool _success,) = payable(ADMIN).call{value: MINTING_FEE}("");
        ...
        uint256 _refund = msg.value - MINTING_FEE;
        if (_refund > 0) {
    ...
    function updateMintingFee(uint256  _newFee) public onlyRole(DEFAULT_ADMIN_ROLE) {
        MINTING_FEE = _newFee;
        emit FEE(_newFee);
    }
    ...
    function _getSignedData(bytes calldata _data) internal view returns(address, string memory, string memory) {
        (bytes memory _signature, , string memory _username, string memory _tokenURI, uint256 _deadline) = _decode(_data);
        require(_deadline >= block.timestamp, "UniqueNameToken: Signature expired");

        bytes32 _digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("UNT(address account,string username,string tokenURI,string deadline)"), 
                    _msgSender(), 
        ...
        return (_signer, _username, _tokenURI);
    }
    ...
    function encode(bytes memory _signature, address _account, string calldata _username, string calldata _tokenURI, uint256 _deadline) external pure returns(bytes memory) {
        return abi.encode(_signature, _account, _username, _tokenURI, _deadline);
    }

    function _decode(bytes calldata _encoded) internal pure returns(bytes memory, address, string memory, string memory, uint256) {
        (bytes memory _signature, address _account,  string memory username, string memory _tokenURI, uint256 _deadline) = abi.decode(_encoded, (bytes, address, string, string, uint256));
        return (_signature, _account, username, _tokenURI, _deadline);
    }
