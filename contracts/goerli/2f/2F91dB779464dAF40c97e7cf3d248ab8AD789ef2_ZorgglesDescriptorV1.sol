...
@@@@@@@BBBBBBBBBB######RRRRRRRROOOOOOOOOOOOOOOOZZZZZZZZZZZZZZZZZZZ[email protected]@@@@@@@@
...
contract ZorgglesDescriptorV1 is Initializable, OwnableUpgradeable {
    ...
    string private zorbPathsChunk2;
    string private zorbPathsChunk3;
    string[80] private zorbPositions;
    uint256 public zorbCount;

    IERC721Drop public zoraDrop;
    IERC721AUpgradeable public tokenContract;

    event ZoraDropUpdated(IERC721Drop zoraDrop);
    ...
    event ZorbPositionsUpdated(string[80] zorbPositions);
    ...
    function initialize(address owner) public initializer {
        ...
        description = string(
            abi.encodePacked(
                "zorbs are a triple threat. they are a zorggle, a zorb, and a nog. they are the ultimate zorggle. they are the ultimate zorb. they are the ultimate nog. they are the ultimate zorggle zorb nog."
            )
        );
        name = string(abi.encodePacked("Zorb Sheet"));
    }
    ...
    function getTokenIdSvg(
        ...
        // bytes[] memory zorbColors = getZorbColors();
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    "</defs>",
                    // string(
                    // abi.encodePacked(zorbColors[0][0])),
                    // string(
                    // abi.encodePacked(zorbColors[1][1])),
                    // string(
                    // abi.encodePacked(zorbColors[2][2])),
                    '<path fill="#EBEBEB" d="M2000 0H0V2000H2000V0Z"/>',
                    '<path fill="#fff" d="M1448.89 447.852H551.108V1552.15H1448.89V447.852Z"/>',
                    zorbPathsChunk0,
                    zorbPathsChunk1,
                    zorbPathsChunk2,
                    zorbPathsChunk3,
    ...
    function getZorbColors() internal view returns (address[20] memory zorbColors) {       
        
        unchecked {
            for(uint256 n = 0; n < 20;) {
              address ownerAddress = getOwnerOf(n);
            //   bytes[5] memory colors = ColorLib.gradientForAddress(0xCB43078C32423F5348Cab5885911C3B5faE217F9);
            //   zorbColors.push(address);
            n++;
    ...
    function buildZorbs(uint256 tokenId) internal view returns (string memory) {
        console.log("buildZorbs");
        bytes memory list;
        // uint256 totalSupply = tokenContract.getTotalSupply();
        // uint256 startingTokenId = totalSupply - tokenId;
        // console.log(startingTokenId, totalSupply, tokenId);
        ...
        // tokenId: 400
        //
        // for (uint i; i < tokenId; ++i) {
        for (uint i; i < tokenId; ++i) {
            // bytes[5] memory colors = zorbSheet.getColorsForZorb(
            //     startingTokenId + i
            // );
            address ownerAddress = getOwnerOf(i);
            ...
            list = abi.encodePacked(
                ...
                string(
                    abi.encodePacked(
                        '<radialGradient gradientTransform="translate(',
                        zorbPositions[i],
                        ') scale(43.6687)" gradientUnits="userSpaceOnUse" cx="0" cy="0" r="1" id="zorb-',
                        i.toString(),
                        '">',
    ...
    */
    ...
    function setZoraDropContract(IERC721Drop _zoraDrop) external {
        require(msg.sender == _owner, "Rejected: not owner");
        zoraDrop = _zoraDrop;
        emit ZoraDropUpdated(_zoraDrop);
    }
    ...
    function setZorbPositions(string[80] memory _zorbPositions) external {
        require(msg.sender == _owner, "Rejected: not owner");
    ...
    function setZorbPaths(uint256 chunkNum, string memory _zorbPaths) external {
        ...
        if (chunkNum == 0) {
            ...
            zorbPathsChunk2 = _zorbPaths;
        } else if (chunkNum == 3) {
            zorbPathsChunk3 = _zorbPaths;
        }
