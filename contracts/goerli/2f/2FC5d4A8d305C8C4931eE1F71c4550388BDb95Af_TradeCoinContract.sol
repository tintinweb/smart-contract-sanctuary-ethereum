    ...
    ITradeCoinContract
{
    ...
    bytes32 private constant _MINT_TYPEHASH =
        keccak256(
            "Mint(string product,uint256 amount,bytes32 unit,string state,uint256 CO2Emissions,string transformation,address owner,address tokenizer,uint256 paymentInWei,bytes32[] docHashes,bytes32[] docTypes,uint256 nonce,uint256 deadline)"
        );
    ...
    modifier isLegalOwnerOrCurrentHandler(address _sender, uint256 tokenId) {
        require(
            _tradeCoin[tokenId].currentHandler == _sender ||
    ...
    modifier onlyEnoughDocuments(Documents memory documents) {
        require(
    ...
    mapping(uint256 => TradeCoin) private _tradeCoin;

    mapping(address => uint256) public nonces;
    mapping(address => uint256) public pendingWeiForWithdrawal;

    // Mappings for commercial sale
    ...
    function mintProduct(
        ...
        require(block.timestamp <= productInit.deadline, "Deadline expired");
        require(productInit.owner == msg.sender, "Not Owner");
        require(msg.value == productInit.paymentInWei, "Wrong Amount");
        require(validators.length >= 1, "Not Enough Validators");

        bytes32 hash;
        address[] memory validatorAddrs = new address[](validators.length);

        for (uint256 index; index < validators.length; index++) {
            hash = getTypedDataHash(
            ...
            require(
                hasRole(validators[index].role, validators[index].signer),
                string(
                    abi.encodePacked(
                        "Not Right Role: ",
                        Strings.toHexString(validators[index].signer)
                    )
                )
            );
            ...
            validatorAddrs[index] = validators[index].signer;
        }
        uint256 tokenId = ++_tokenIdCounter;

        _mint(productInit.owner, tokenId);
        pendingWeiForWithdrawal[validators[0].signer] += msg.value;

        TradeCoin storage _tradeCoinPointer = _tradeCoin[tokenId];
        _tradeCoinPointer.product = productInit.product;
        _tradeCoinPointer.amount = productInit.amount;
        _tradeCoinPointer.unit = productInit.unit;
        _tradeCoinPointer.state = productInit.state;
        _tradeCoinPointer.CO2Emissions = productInit.CO2Emissions;
        _tradeCoinPointer.currentHandler = currentHandler;
        _tradeCoinPointer.transformations.push(productInit.transformation);

        emit MintProduct(
            productInit.owner,
            tokenId,
            productInit.CO2Emissions,
            validatorAddrs,
            productInit.product,
            productInit.amount,
            productInit.unit,
            productInit.state,
            productInit.transformation,
            currentHandler,
            productInit.documents.docHashes,
            productInit.documents.docTypes
        );
    }

    function withdrawPayment() external override {
        uint256 withdrawnAmount = pendingWeiForWithdrawal[msg.sender];
        pendingWeiForWithdrawal[msg.sender] = 0;

        payable(msg.sender).transfer(withdrawnAmount);

        emit WithdrawPayment(msg.sender, withdrawnAmount);
    }
    ...
    function unitConversion(
        ...
        TradeCoin storage tradeCoinPointer = _tradeCoin[tokenId];

        require(newAmount > 0, "Can't be 0");
        require(previousAmountUnit != newUnit, "Invalid Conversion");
        require(
            previousAmountUnit == tradeCoinPointer.unit,
            "Invalid Match: unit"
        );

        tradeCoinPointer.amount = newAmount;
        tradeCoinPointer.unit = newUnit;
        tradeCoinPointer.CO2Emissions += CO2Emissions;
    ...
    function addTransformation(
        ...
        external
        override
        isLegalOwnerOrCurrentHandler(msg.sender, tokenId)
        onlyEnoughDocuments(documents)
    {
        TradeCoin storage _tradeCoinPointer = _tradeCoin[tokenId];
        int256 newAmount = int256(_tradeCoinPointer.amount) + weightDifference;

        if (
            keccak256(bytes(transformationCode)) ==
            keccak256(bytes("Certification"))
        ) {
            require(weightDifference == 0, "Invalid Certification");
        } else {
            require(
                weightDifference != 0 && newAmount > 0,
                "Invalid weight difference"
            );
        }

        _tradeCoinPointer.transformations.push(transformationCode);
        _tradeCoinPointer.amount = uint256(newAmount);
        _tradeCoinPointer.CO2Emissions += CO2Emissions;
        ...
        onlyEnoughDocuments(documents)
    {
        for (uint256 tokenId; tokenId < tokenIds.length; tokenId++) {
            require(ownerOf(tokenIds[tokenId]) != address(0));
    ...
    function addValidation(
        ...
        string memory result,
        Documents memory documents
    ) external override onlyEnoughDocuments(documents) {
        require(ownerOf(tokenId) != address(0));
    ...
    function changeProductHandler(
        ...
        string memory geoLocation,
        Documents memory documents
    ) external override isLegalOwner(tokenId) onlyEnoughDocuments(documents) {
        _tradeCoin[tokenId].currentHandler = newCurrentHandler;

        emit ChangeProductHandler(
            tokenId,
            msg.sender,
            newCurrentHandler,
            geoLocation,
    ...
    function changeProductState(
        ...
        onlyEnoughDocuments(documents)
    {
        _tradeCoin[tokenId].state = newState;
        _tradeCoin[tokenId].CO2Emissions += CO2Emissions;
    ...
    function splitProduct(
        ...
        external
        override
        onlyLegalOwner(msg.sender, tokenId)
        onlyEnoughDocuments(documents)
    {
        uint256[] memory splitTokenIds = new uint256[](partitions.length);
        TradeCoin storage tradeCoinPointer = _tradeCoin[tokenId];
        uint256 sumPartitions;
        uint256 splittedCO2Emission;

        uint256 cachedPartLength = partitions.length;
        for (uint256 index; index < cachedPartLength; ) {
            require(partitions[index] != 0, "Partitions can't be 0");
            sumPartitions += partitions[index];
            splittedCO2Emission =
                (tradeCoinPointer.CO2Emissions * partitions[index]) /
                tradeCoinPointer.amount +
                (CO2Emissions * partitions[index]) /
                tradeCoinPointer.amount;
            ...
            splitTokenIds[index] = _tokenIdCounter;
        ...
        require(
            tradeCoinPointer.amount == sumPartitions,
            "Incorrect sum of amount"
        );

        burn(tokenId, geoLocation, documents);
        emit SplitProduct(
            tokenId,
            msg.sender,
            CO2Emissions,
    ...
    function batchProduct(
        ...
        uint256 cummulativeCO2Emissions;
        bytes32 hashed;
        TradeCoin storage _tradeCoin0 = _tradeCoin[tokenIds[0]];

        //Note: to prevent stack too deep error
        {
            hashed = keccak256(
                abi.encode(
                    _tradeCoin0.product,
                    _tradeCoin0.state,
                    _tradeCoin0.transformations,
                    _tradeCoin0.unit
                )
            );
        }

        uint256 cachedTokenIdsLength = tokenIds.length;
        for (uint256 index; index < cachedTokenIdsLength; ) {
            require(ownerOf(tokenIds[index]) == msg.sender, "Unauthorized");
            bytes32 hashed2;

            TradeCoin storage tradeCoinPointer = _tradeCoin[tokenIds[index]];
            //Note: to prevent stack too deep error
            {
                hashed2 = keccak256(
                    abi.encode(
                        tradeCoinPointer.product,
                        tradeCoinPointer.state,
                        tradeCoinPointer.transformations,
                        tradeCoinPointer.unit
                    )
                );
            }

            require(hashed == hashed2, "Invalid PNFT");

            cummulativeAmount += tradeCoinPointer.amount;
            cummulativeCO2Emissions += tradeCoinPointer.CO2Emissions;
            burn(tokenIds[index], geoLocation, documents);
            unchecked {
                ++index;
            }
        }
    ...
    function initiateCommercialTx(
        ...
        onlyLegalOwner(msg.sender, tokenId)
    {
        require(msg.sender != newOwner, "Can't sell to self");
        if (payInFiat) {
    ...
    function servicePayment(
        ...
        bool payInFiat,
        Documents memory documents
    ) external payable override nonReentrant onlyEnoughDocuments(documents) {
        // When not paying in Fiat pay but in Eth
        if (!payInFiat) {
            require(
    ...
    {
        require(ownerOf(tokenId) != address(0));
        TradeCoin storage _tcPointer = _tradeCoin[tokenId];

        product = _tcPointer.product;
        amount = _tcPointer.amount;
        unit = _tcPointer.unit;
        state = _tcPointer.state;
        CO2Emissions = _tcPointer.CO2Emissions;
        currentHandler = _tcPointer.currentHandler;
        transformations = _tcPointer.transformations;
    }
    ...
    function mintAfterSplitOrBatch(
        uint256 amount,
        uint256 CO2Emissions,
        TradeCoin storage tradeCoinPointer,
        string memory geoLocation
    ) internal {
        require(tradeCoinPointer.amount != 0, "Insufficient Amount");
        ...
        _tradeCoin[id].product = tradeCoinPointer.product;
        _tradeCoin[id].amount = amount;
        _tradeCoin[id].unit = tradeCoinPointer.unit;
        _tradeCoin[id].state = tradeCoinPointer.state;
        _tradeCoin[id].CO2Emissions = CO2Emissions;
        _tradeCoin[id].currentHandler = tradeCoinPointer.currentHandler;
        _tradeCoin[id].transformations = tradeCoinPointer.transformations;

        emit MintAfterSplitOrBatch(id, msg.sender, geoLocation);
    }
    ...
    function getTypedDataHash(
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                _MINT_TYPEHASH,
                keccak256(bytes(productInit.product)),
                productInit.amount,
                productInit.unit,
                keccak256(bytes(productInit.state)),
                productInit.CO2Emissions,
                keccak256(bytes(productInit.transformation)),
                productInit.owner,
                tokenizer,
                productInit.paymentInWei,
                keccak256(abi.encodePacked(productInit.documents.docHashes)),
                keccak256(abi.encodePacked(productInit.documents.docTypes)),
                nonces[signer],
                productInit.deadline
            )
        );
