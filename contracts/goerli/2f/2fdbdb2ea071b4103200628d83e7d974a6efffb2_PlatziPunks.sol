...
pragma solidity ^0.8.0;
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA, Ownable {
    ...
    function safeMint(address to) public onlyOwner {
        ...
        require(tokenId < maxSupply, "No tokens");
        tokenDNA[tokenId] = deterministicPseudoRandomDNA(tokenId, to);
        _safeMint(to, tokenId);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory)
    {
        require(
            _exists(tokenId),
            "ERC721: URI query no existe token"
        );

        uint256 dna = tokenDNA[tokenId];
        string memory image = imageByDNA(dna);

        bytes memory jsonURI = 
            abi.encodePacked(
                '{"name": "PlatziPunks #',
                tokenId.toString(),
                '", "description": "Imagenes guardadas en blockchain", "image": "',
                image,
                '"}'
            );
