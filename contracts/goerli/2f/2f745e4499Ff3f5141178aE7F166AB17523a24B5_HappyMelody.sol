...
contract HappyMelody is RegisterFilter721, Pausable {
  ...
  event CliamedAirdropNFT(address, uint);
  ...
  uint256 public MAX_MINT_PER_TX = 985;
  uint256 public MAX_SUPPLY = 1000;
  uint256 public AIRDROP_NFT_COUNT = 5;
  uint256 public TEAM_NFT_COUNT = 10;
  ...
  bytes32 private _merkleRoot = 0x8fb300e3fdb6f30a4c67233b997f99fdd518b968b9a3fd65857bfe78b2600719;
  ...
  constructor(
      ...
      address artistAddress_) RegisterFilter721("Happy Melody", "HPM") {
  ...
  function mint(uint256 quantity) external payable whenNotPaused {
    ...
    // require(publicRoundMintPrice > 0, "Price is not yet set");
    ...
    uint256 cost = quantity * publicRoundMintPrice;
    ...
    emit TotalMint(
        ...
        currentPhase.amountMinted + quantity
        );
 ...
 function isClaimedAirdrop(address address_)  public  view  returns(bool) {
  ...
  function claimAirdropNFTs() external whenNotPaused {
    require(isAirdropActive, "Airdrop is inactive");
    require(whitelistAddressesAirdrop.length > 0 , "Already cliam airdrop");
    require(isAddressInWhitelistAirdrop(msg.sender), "You need to be whitelisted to mint in the airdop round");
    ...
    emit CliamedAirdropNFT(msg.sender, startTokenId);
  }

  ///////////////
  // Whitelist Airdrop
  ///////////////
  function setAddressWhitelistAirdrop(address[] calldata _addresses) external onlyOwner {
    require(_addresses.length == AIRDROP_NFT_COUNT, "Wrong airdrop NFT");

    whitelistAddressesAirdrop = _addresses;

  }

  function isAddressInWhitelistAirdrop(address _address) public view returns (bool) {
    for (uint i = 0; i < whitelistAddressesAirdrop.length; i++) {
      if (whitelistAddressesAirdrop[i] == _address) {
        return true;
      }
    }
    return false;
  ...
  function removeWhitelistAirdrop(address _address) internal {
    for (uint i = 0; i < whitelistAddressesAirdrop.length; i++) {
      if (whitelistAddressesAirdrop[i] == _address) {
        whitelistAddressesAirdrop[i] = whitelistAddressesAirdrop[whitelistAddressesAirdrop.length - 1];
        whitelistAddressesAirdrop.pop();
        break;
      }
    }
  }
