...
contract LinksPFP is ERC721ABurnable, Ownable, AccessControl, Toggleable, EIP712Common {
  ...
  function whitelistMint(uint256 _count, uint256 _allowedMints, bytes calldata _signature) external payable requiresDiscount(_signature, _allowedMints) requireActiveWhitelist noContracts {
    if (_totalMinted() + _count > MAX_SUPPLY) revert ExceedsMaxSupply();
    if (_numberMinted(msg.sender) + _count > _allowedMints +  _getAux(msg.sender)) revert ExceedsMaxPerWallet();
  ...
  function allowedWhitelistMintCount(address _minter, uint256 _allowedMints, bytes calldata _signature) external view requiresDiscount(_signature, _allowedMints) returns (uint256) {
    return _allowedMints + _getAux(_minter) - _numberMinted(_minter);
  }

  /** WHITELIST **/
  function checkWhitelist(uint256 _allowedMints, bytes calldata _signature) external view requiresDiscount(_signature, _allowedMints) returns  (bool){
    return true;
  }

  /** ADMIN **/
  function setMaxPerWallet(uint64 _maxPerWallet) external onlyOwner {
