...
contract FactoryNew is OwnableUpgradeable{
    ...
    event EmergencyWithdrawn(address indexed stakedToken,  uint128 amount, uint128 stakeType, address indexed user, uint256 blockTimestamp, uint128 period, uint128 totalStakedInPool);
    ...
    event Deployed(address indexed instance, string uri);
    event StakingInitialized(address indexed proxyInstance, address indexed stakedToken, uint128 stakePeriod, uint128 depositFees, uint128 withdrawlsFees, uint128 rewardRate, string name, string _symbol, string uri);
    ...
    function deployInstance(string memory _uri) external onlyOwner returns(address addr) {
    ...
    function initializeProxyInstance(address _proxyInstance, address _ownerWallet, address _stakedToken, 
                        ...
                        uint128 _withdrawalFees, uint128 _rewardRate, uint128 _emergencyFees, uint128 _platformFee, address _treasury, string memory _uri) external onlyOwner {
        ...
        {
        bytes memory init = returnHash(_ownerWallet, _stakedToken, _stakeType, _stakePeriod, _depositFees, _withdrawalFees, _rewardRate, _emergencyFees, _platformFee, _treasury);
        if (init.length > 0)
        ...
        emit StakingInitialized(_proxyInstance, _stakedToken, _stakePeriod, _depositFees, _withdrawalFees, _rewardRate, _name, _symbol, _uri);
        }
    }
    ...
    function returnHash(address _ownerWallet, address _stakedToken, 
                        uint128 _stakeType, uint128 _stakePeriod, uint128 _depositFees, 
                        uint128 _withdrawalFees, uint128 _rewardRate, uint128 _emergencyFees, uint128 _platformFee, address _treasury) internal pure returns(bytes memory data) {
        data = abi.encodeWithSignature("initialize(address,address,uint128,uint128,uint128,uint128,uint128,uint128,uint128,address)", _ownerWallet,_stakedToken,_stakeType,_stakePeriod,_depositFees,_withdrawalFees,_rewardRate, _emergencyFees,_platformFee,_treasury);
    }
