...
contract BrandExtender is ERC5169, RoyaltySpliterStatic, VerifySignature, ERC721URIStorage, UriChanger, DerivedERC2981Royalty, IERC721Enumerable {
    ...
    function mintDerived(mintRequestData[] calldata data) external virtual payable {
        ...
        for (i=0; i < data.length; i++) {
            ...
            emit MintedDerived(data[i].ERC721, data[i].tokenId, mintedID, data[i].tokenURI, tokenOfOriginCounter(data[i].ERC721, data[i].tokenId) - 1 );
        }
    }
    ...
    function setRoyaltyPercentage(uint value) external onlyOwner {
