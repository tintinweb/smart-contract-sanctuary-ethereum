...
contract PermittedPartners is Ownable, IPermittedPartners {
     ...
     * taken as a revenue share for a the partner.
     */
    function setPartnerRevenueShare(
        address _partner,
        uint16 _revenueShareInBasisPoints
    ) external onlyOwner {
        require(_partner != address(0), "Partner is address zero");
        require(
            _revenueShareInBasisPoints <= HUNDRED_PERCENT,
            "Revenue share too big"
        );
     ...
     */
    function getPartnerPermit(address _partner)
        external
        view
        override
        returns (uint16)
 ...
 * @author
 * @dev Interface for NFT Types Registry supported by .
 */
