...
error AllowlistMintDisabled();
error InvalidAllowlistQuantity();
    ...
    DefaultOperatorFilterer
{
    ...
    address[] private _shareholders = [
        0x734c77A2265c1d1B37Ad44Dd262c71A49BECb764
    ];

    mapping(address => uint256) public allowlostMints;

    constructor()
        ERC721A("Tickets", "Tickets")
        PaymentSplitter(_shareholders, _shares)
    {
        _pause();
    }

    function setAllowlistMintEnabled(bool _allowlistMintEnabled)
        external
        onlyOwner
    {
    ...
    function mintAllowlist(
        ...
        if (!allowlistMintEnabled) revert AllowlistMintDisabled();
        if (allowlostMints[msg.sender] + qty > maxqty) {
            revert InvalidAllowlistQuantity();
        }
