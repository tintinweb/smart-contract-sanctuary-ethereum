...
contract MutariuumStaking is IMuutariumStaking, Ownable {
    ...
    function unstake(address nft, uint256[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            (address owner, uint256 startTime) = _unpackStakingInfos(_packedStakingInfos[nft][tokenIds[i]]);
            ...
            emit Unstake(nft, tokenIds[i], block.timestamp - startTime);
        }
    }
