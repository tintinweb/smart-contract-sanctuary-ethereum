...
contract Treasury is UUPSUpgradeable, Constants, ReentrancyGuardUpgradeable, ITreasury {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IComptroller public TONPOUND_COMPTROLLER;
    uint64 private _REWARD_CLAIM_START;
    uint256 private constant MAX_RESERVE_BPS = 5e3;
    uint256 private constant DENOM_BPS = 1e4;
    ...
    function initialize(address comptroller_) external initializer {
        ...
        _REWARD_CLAIM_START = uint64(block.timestamp + REWARD_ACCUMULATING_PERIOD);
    }
    ...
    function _distributeInternal(address[] memory tokens) internal {
        ...
        for (uint256 i; i < tokens.length; ) {
            ...
            if (takeReserve) {
                reserveAmount = (amount * resFactorBPS) / DENOM_BPS;
    ...
    function _updateRewards(
        ...
        rewardPerShare[rewardToken] = curRewardPerShare + (amount * EXP_REWARD_PER_SHARE) / supply;
        return true;
    }
    ...
    function registerTokenId(uint256 tokenId, bool state) external nonReentrant {
        ...
        uint256 deltaWeight = gNft.getTokenData(tokenId).slot0.rewardWeight;
    ...
    function setReserveFactor(uint256 newReserveBPS) external onlyAdmin {
        if (newReserveBPS > MAX_RESERVE_BPS) {
    ...
    modifier whenNotPaused() {
        if (PausableUpgradeable(address(_getgNFTAddress())).paused()) {
            revert Paused();
        }
        _;
    }
    ...
    modifier onlyAdmin() {
        if (
            !IAccessControlUpgradeable(address(_getgNFTAddress())).hasRole(
                DEFAULT_ADMIN_ROLE,
                msg.sender
            )
        ) {
