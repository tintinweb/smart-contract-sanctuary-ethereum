...
contract MOPNLand is ERC721A, Ownable, DefaultOperatorFilterer {
    ...
    function ethMint(uint256 quantity) external payable {
        require(
        ...
        require(
            quantity >= 1 && msg.value == ethMintTotalPrice(quantity),
            "Invalid price"
        );
        ...
        emit EthMinted(msg.sender, msg.value, quantity);
    }

    function auctionMint(address to, uint256 amount) external {
        require(
            msg.sender == auctionAddress && msg.sender != address(0),
            "Invalid auctionAddress"
        );
        _mintLand(to, 1);
    ...
    function ethMintTotalPrice(uint256 quantity) public view returns (uint256) {
        uint256 r_pow_n = R ** quantity; // Calculate R^n
        uint256 numerator = startPrice.mul(r_pow_n.sub(1000 * (10 ** 18))); // Fix precision issue
        uint256 denominator = R.sub(1000 * (10 ** 18)); // Fix precision issue

        uint256 sum = numerator.div(denominator).div(10 ** 15); // Divide by 10^15 for correct decimal place
    ...
    function _resetStartPrice(uint256 n) internal {
        uint256 newStartPrice = startPrice.mul(R ** n).div(1000 ** n);
