...
contract ClaimGift is Ownable, IERC1155Receiver, ReentrancyGuard, IERC721Receiver, VRFConsumerBaseV2 {
    using SafeMath for uint256;
    ///@dev Event for user gift claim request
    event GiftClaimRequestSent(uint256 requestId, uint32 numWords, address userWallet, uint256 claimAmount);
    ...
    ///@dev Chain Link requestIds to store for callback usage
    uint256[] public requestIds;

    ///@dev Chain Link last requestId for random number
    uint256 public lastRequestId;

    ///@dev The gas lane to use, which specifies the maximum gas price to bump to.
    ///@dev This is a chainlink configuration to support the callback method gas.
    ///@dev a list of available gas lanes on each network,
    ///@dev https://docs.chain.link/docs/vrf/v2/subscription/supported-networks/#configurations
    ...
    ///@dev so 100,000 is a safe default for this example contract. Test and adjust
    ///@dev this limit based on the network that you select, the size of the request,
    ///@dev and the processing of the callback request in the fulfillRandomWords()
    ///@dev function.
    uint32 public callbackGasLimit = 1000000;

    ///@dev Chain Link configuration
    ...
    ///@param _vrfCoordinator Clainlink VRF contract address
    ///@param _chainLinkSubscriptionId Chainlink subscription Id to get link token for random number
    ///@param _keyHash Chainlink keyHash for random number request
    ...
    ///@param _randomNumber Random got from chain link
    ///@param _requestId Chain link request Id
    function transferGift(uint256 _randomNumber, uint256 _requestId) internal {
    ...
    ///@dev Internal method for chain link callback
    ///@param _requestId Chain link request Id
    ///@param _randomWords Chain link random number list
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        chainLinkRequests[_requestId].randomNumbers = _randomWords;
        emit RequestFulfilled(_requestId, _randomWords);

        IAnimeMetaverseReward AnimeMetaverseReward = IAnimeMetaverseReward(rewardContractAddress);

        AnimeMetaverseReward.forceBurn(chainLinkRequests[_requestId].msgSender, giftBoxTokenId, _randomWords.length);

        for (uint256 index = 0; index < _randomWords.length; index = _uncheckedInc(index)) {
            transferGift(_randomWords[index], _requestId);
        }
    }

    ///@dev External method for claiming gift
    ///@param _claimGiftAmount Gift of soul token amount for burning and claiming gif token
    function claimGift(uint32 _claimGiftAmount)
        external
        nonReentrant
        noContracts
        hasValidGiftBoxTokenBalance(_claimGiftAmount)
        // returns (uint256 requestId)
    {
        ...
        for (uint256 index = 0; index < _claimGiftAmount; index = _uncheckedInc(index)) {
            ...
            transferGift(randomNumber, requestId);
        }
        ...
        //     _claimGiftAmount * callbackGasLimit,
        //     _claimGiftAmount
        // );

        // chainLinkRequests[requestId] = RequestStatus({randomNumbers: new uint256[](0), exists: true, fulfilled: false, msgSender: msg.sender});

        // requestIds.push(requestId);
        // lastRequestId = requestId;
        // emit GiftClaimRequestSent(requestId, _claimGiftAmount, msg.sender, _claimGiftAmount);
    ...
    ///@dev Sets Chain Link subscriptionId
    ///@notice Only contract can update chain link subscription Id
    ///@param _chainLinkSubscriptionId required subscription Id to get link token for random number
    function setChainLinkSubscriptionId(uint64 _chainLinkSubscriptionId) external onlyOwner {
        chainLinkSubscriptionId = _chainLinkSubscriptionId;
    }

    ///@dev Sets Chain Link key hash
    ///@notice Only contract owner can update chain link key hash
    ///@param _keyHash required keyHash for random number request
