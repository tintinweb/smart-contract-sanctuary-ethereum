...
// File: contracts/library/DetailsLibraryUpdated.sol
...
contract Staking4 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _ownerWallet, address _stakedToken, 
        ...
        stakeTypesList.push(DetailsLibraryUpdated.StakeTypeData(_stakeType,_stakePeriod * 60,_depositFees,_withdrawlsFees,_rewardRate,0));

    }
    
    function deposit(address user,uint128 _amount, uint128 _stakeType) external nonReentrant onlyOwner returns(uint128 emitAmount, uint128 _period) {
    ...
    function compound(address user, uint128 _stakeType) external nonReentrant onlyOwner returns(uint128, uint128) {
        require(stakeTypeExist[_stakeType], "STAKE_TYPE_DOES_NOT_EXIST");
        ...
        uint128 stakeTimeTillNow;
            ...
            if(uint128(block.timestamp) < stakes.fullWithdrawlTime) {
                ...
                if(stakeType.stakePeriod == 0) stakeTimeTillNow = uint128(block.timestamp) - stakes.lastClaimTime;
                else stakeTimeTillNow = stakes.fullWithdrawlTime - stakes.lastClaimTime;
    ...
    function withdraw(address user, uint128 _amount, uint128 _stakeType) external nonReentrant onlyOwner returns(uint128, uint128) {
    ...
    function claim(address user, uint128 _stakeType) external nonReentrant onlyOwner returns(uint128, uint128) {
        require(stakeTypeExist[_stakeType], "STAKE_TYPE_DOES_NOT_EXIST");
    ...
    function addStakedType(uint128 _stakeType, uint128 _stakePeriod, uint128 _depositFees, uint128 _withdrawlFees, uint128 _rewardRate) external onlyOwner {
        ...
        stakeTypesList.push(DetailsLibraryUpdated.StakeTypeData(_stakeType,_stakePeriod * 60,_depositFees,_withdrawlFees,_rewardRate,0));
    ...
    function updateStakeType(uint128 _stakeType, uint128 _stakePeriod, uint128 _depositFees, uint128 _withdrawlFees, uint128 _rewardRate) external onlyOwner {
        ...
        stakeType.stakePeriod = _stakePeriod * 60;
