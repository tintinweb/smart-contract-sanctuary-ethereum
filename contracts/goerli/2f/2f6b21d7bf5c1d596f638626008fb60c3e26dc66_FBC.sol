...
contract FBC is ERC721A, ReentrancyGuard{ 
	using Strings for uint256;
	...
	uint256 private _maxSupply = 5_400;
	...
	function crossmint(uint256 mintQty, address receiver) external payable {
		...
		if(mintQty > 6) revert MintLimit();
        if(mintQty < 6){    
		    if(mintQty * cost != msg.value) revert EthValue();                    
        } else {
            if(cost * 5 != msg.value) revert EthValue();
        }
        unchecked{ if(mintQty == 5) {++mintQty;} }
		if(mintQty + _totalMinted() > _maxSupply) revert MaxSupply();
	...
	function withdraw() external nonReentrant {
		...
		payable(_admin).transfer(initalBalance * 15 / 100);
		// some other address
		//payable(XXX).transfer(initalBalance * 20 / 100);
