...
contract DepositNFT {
    ...
    constructor(uint8 _maxSupply, int _depositPrice, uint8 _sellGoal, uint256 _decisionTime, address _bankOwner) {
    ...
    function mint() public payable {
        ...
        require(msg.value >= uint256(depositPrice), "Deposit is less than the required deposit price");
    ...
    function burn(uint8 _tokenId) public {
        ...
        payable(msg.sender).transfer(uint256(depositPrice));
    ...
    */
    function burnIfGone() public {
        ...
        for (uint8 i = 0; i < maxSupply; i++) {
            if (tokenMinted[i].isDeposit == true) {
                ...
                payable(tokenMinted[i].owner).transfer(uint256(depositPrice));
    ...
    function sell(uint8 _tokenId, address _buyer, uint8 _price) public {
        ...
        require(_price > uint256(depositPrice), "Selling price is less than or equal to deposit price");

        payable(msg.sender).transfer(_price);
        payable(bankOwner).transfer(uint256(depositPrice));
