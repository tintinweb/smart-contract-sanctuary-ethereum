...
contract KeeperRegistry is Initializable, OwnableUpgradeable, UUPSUpgradeable, IKeeperRegistry {
    ...
    uint256 public transferDelay;   //Slash safety period--if a keeper leaves, this is the amount of time they must wait before they can withdraw their bond.
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i != joinersLength; i++) {
            ...
            registry[joiners[i]] = WorkerDetails({
                arrayIndex: _firstEmptyKeeperIndex + i,
    ...
    function queueToLeave() public {
        ...
        registry[msg.sender].leaveDate = block.timestamp + transferDelay;
    ...
    function _removeKeeper(address targetKeeper) internal   {
        ...
        uint256 targetArrayIndex = registry[targetKeeper].arrayIndex;
...
contract Orchestrator is IOrchestrator, OwnableUpgradeable, UUPSUpgradeable {
    ...
    */
    ...
    /**
     * @dev set the total reward per action to be distributed
     * @param _rewardPerAction total reward to be distributed per action--split between all runners.
     */
    ...
    function createAction(
        ...
        require(_recipients.length != 0, "_recipients length cannot be zero");
        require(
     ...
     * which is open for approval for all members of the group.
     * @dev 1 represents no, 2 represents yes.
     *
     * Emits a {vote} event.
     */
        ...
        returns (ActionState)
    {
        ...
        //Ensure action is not already executed
        require(
        ...
        if (actionApproval == 2) {
            ...
            uint256 gasAvailable = gasVault.gasAvailableForTransaction(
                thisAction.targetAddress
            );
            ...
            gasVault.reimburseGas(thisAction.targetAddress, originalGas);
...
interface IKeeperRegistry {
    ...
    struct WorkerDetails {
        uint256 arrayIndex; // Index of this keeper in the registeredKeepers array.
        uint256 bondHeld;   // bondCoin held by this keeper.
        uint256 leaveDate;  // If this keeper has queued to leave, they can withdraw their bond after this date.
    }
...
interface IOrchestrator {
    ...
    event ActionRejected(uint256 indexed _actionId, address _from);
    event ActionFailed(uint256 indexed _actionId);
    event vote(uint256 indexed _actionId, address indexed _from, uint8 _approved);
}