 ...
 */
contract VestingTokenFactory is Ownable, IFeeManager, IBeacon {
    /**
     ...
     */
    mapping(address => address[]) internal vestingTokensByUnderlyingToken;
     ...
     * - `newMerkleRootSigner` can't be address 0x0.
     ...
     * @param feeCollector          Address of fee collector.
     * @param feePercentage         Fee percentage that this  value.
     */
    constructor(address implementationAddress, address feeCollector, uint64 feePercentage) {
        require(Address.isContract(implementationAddress), "VestingTokenFactory: implementation is not a contract");
        require(
            feePercentage >= MIN_FEE && feePercentage <= MAX_FEE,
            "VestingTokenFactory: fee percentage is out of the accepted range"
        );

        implementation = implementationAddress;
        setFeeCollector(feeCollector);
        _feeData.feePercentage = feePercentage;
    }

    /**
     * @dev Creates new VestingToken contracts.
     *
     * Requirements:
     *
    ...
    function createVestingToken(
        ...
        BeaconProxy vestingToken = new BeaconProxy(address(this), "");

        IVestingToken(address(vestingToken)).initialize(name, symbol, underlyingTokenAddress, milestonesArray);

        vestingTokensByUnderlyingToken[underlyingTokenAddress].push(address(vestingToken));
        emit VestingTokenCreated(underlyingTokenAddress, address(vestingToken));
    }
    ...
    function setFeeCollector(address newFeeCollector) public onlyOwner {
        require(newFeeCollector != address(0), "VestingTokenFactory: fee collector cannot be zero address");
     ...
     *
     * @param newFeePercentage Amount of fee.
     */
    function setFeePercentage(uint64 newFeePercentage) external {
        require(_msgSender() == _feeData.feeCollector, "VestingTokenFactory: caller is not fee collector");
        require(
            newFeePercentage >= MIN_FEE && newFeePercentage <= MAX_FEE,
            "VestingTokenFactory: fee percentage is out of the accepted range"
        );
    ...
    /**
     * @notice Exposes the whole array that `vestingTokensByUnderlyingToken` maps.
     */
    function vestingTokens(address underlyingToken) external view returns (address[] memory) {
        return vestingTokensByUnderlyingToken[underlyingToken];
    }

    /**
     * @notice Exposes the `FeeData` for `VestingToken`s to consume.
     */
    function feeData() external view override returns (FeeData memory) {
        return _feeData;
    }
}