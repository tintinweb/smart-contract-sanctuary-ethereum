...
contract Maximillion {
     ...
     * @dev The provided Ether is applied towards the borrow balance, any excess is refunded
     * @param borrower The address of the borrower account to repay on behalf of
     * @param _cEther The address of the cEther contract to repay in
     */
    function repayBehalfExplicit(address borrower, CEther _cEther) public payable {
        uint received = msg.value;
        uint borrows = _cEther.borrowBalanceCurrent(borrower);
        if (received > borrows) {
            _cEther.repayBorrowBehalf{value: borrows}(borrower);
            payable(msg.sender).transfer(received - borrows);
        } else {
            _cEther.repayBorrowBehalf{value: received}(borrower);
        }
    }
}