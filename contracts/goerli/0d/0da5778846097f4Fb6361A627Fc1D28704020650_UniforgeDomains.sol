...
contract UniforgeDomains {
  ...
  event DomainUpdated(uint256 id, string name, address nftAddress);
  event NewDomainOwner(uint256 id, address newOwner);

  /* Set Contract Ownership */
  constructor(uint256 _deployPrice) {
    owner = payable(msg.sender);
    deployPrice = _deployPrice * 1 wei;
  }
  ...
  function createDomain(string memory name) public payable {
    ...
    require(msg.value <= deployPrice, "Insuficient amount");
  ...
  function updateDomainName(
    ...
    if (keccak256(bytes(domain.name)) != keccak256(bytes(name))) {
      (bool domainExists, ) = getDomainByName(name);
      require(!domainExists, "Domain already exists");
      domain.name = name;
    }

    domain.nftAddress = nftAddress;
    idToDomain[domainId] = domain;
    emit DomainUpdated(domain.id, name, nftAddress);
  }
