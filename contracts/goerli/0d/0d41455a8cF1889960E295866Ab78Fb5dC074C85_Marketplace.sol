...
contract Marketplace is ReentrancyGuard {
    ...
    event ListingPurchased(bytes32 id);
    ...
    function createListing(
        ...
        // unique order id. block timestamp and block hash guarantee that there won't be any duplicates
    ...
    function buyItem(bytes32 id) external payable nonReentrant {
        // Get listing
        Listing storage listing = listings[id];

        // Perform validations
        require(listing.seller != msg.sender, "Buyer can't be owner");
        ...
        require(sent, "Problem with sending funds");

        // Emit event
        emit ListingPurchased(id);
    }
}
