...
contract GAT is ERC721A, Ownable, ReentrancyGuard {
  ...
  function _setProjectWallet(address _PROJECT_WALLET, string calldata _salt, bytes calldata _token, uint256 _amount, address signer) external onlyOwner {
    if (!verifyTokenForAddress(_salt, _token, msg.sender, _amount, signer))
