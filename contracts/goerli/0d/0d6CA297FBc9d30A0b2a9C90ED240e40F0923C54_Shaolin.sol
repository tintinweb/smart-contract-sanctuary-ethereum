...
contract Shaolin is ERC721A, Ownable, ReentrancyGuard {
	...
	*/
	...
	function setMaxPerWalletWL(uint _max) external onlyOwner {
		maxPerWalletWL = _max;
	}
	...
	*/
    ...
    function whitelistMint(uint count, bytes32[] memory proof) public payable {
        ...
        require(_walletMintedCount[msg.sender] + count <= maxPerWalletWL,"Exceeds max per wallet for whitelist!");
    ...
    function publicMint(uint count) external payable {
		...
		require(count <= maxPerTransaction,"Exceeds NFT per transaction limit");
		require(_totalMinted() + count <= maxSupply,"Exceeds max supply");
        require(_walletMintedCount[msg.sender] + count <= maxPerWalletPublic,"Exceeds max per wallet for public!");
