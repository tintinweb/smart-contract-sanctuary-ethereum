...
contract Lottery is VRFConsumerBaseV2, ConfirmedOwner {
  ...
  function randomBalls() external view returns (uint8[5] memory) {
    uint8 ball1 = uint8(s_requests[lastRequestId].randomWords[0] % maxBall);
    uint8 ball2 = uint8(s_requests[lastRequestId].randomWords[1] % maxBall);
    uint8 ball3 = uint8(s_requests[lastRequestId].randomWords[2] % maxBall);
    uint8 ball4 = uint8(s_requests[lastRequestId].randomWords[3] % maxBall);
    uint8 ball5 = uint8(s_requests[lastRequestId].randomWords[4] % maxBall);
