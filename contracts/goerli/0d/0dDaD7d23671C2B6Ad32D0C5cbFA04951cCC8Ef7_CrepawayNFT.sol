...
/**s
 * @title ERC1155 Token Contract for Crepaway drop
 * @notice This contract is meant to serve for both Semi- and Non- fungible tokens as part of the Crepaway drop
 * @author Ghadi Mhawej
 **/
contract CrepawayNFT is
    Ownable,
    ERC1155Pausable,
    ERC1155Supply,
    ERC1155Burnable,
    PaymentSplitter
{
    ...
    uint64 public mintsPerAddressLimit;
    ...
    constructor(
        ...
        symbol = symbol_;
        maxId = maxId_;
        _initialAddTokens(maxId_, mintCostPerTokenId_, supplyPerTokenId_);
        mintsPerAddressLimit = mintsPerAddressLimit_;
    }
        ...
        onlyOwner
    {
        require(
            ...
            "CrepawayNFT: Mint Price should be different than the previous price"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "CrepawayNFT: Max Supply should be different than the previous supply"
        );
    ...
    {
        require(exists(tokenId), "CrepawayNFT: NONEXISTENT_TOKEN");
        return
            string(
                abi.encodePacked(
                    super.uri(0),
    ...
    function mint(
        ...
        require(to_ != address(0), "CrepawayNFT: Address cannot be 0");
        require(
            received == mintPrice[tokenId_] * (amount_),
            "CrepawayNFT: Ether sent is not the right amount"
        );
        require(tokenId_ <= maxId, "CrepawayNFT: Token id mismatch");
        require(
            amount_ + totalSupply(tokenId_) <= supplyPerTokenId[tokenId_],
            "CrepawayNFT: Amount mismatch"
        );

        require(
            checkLimit(_msgSender(), amount_),
            "CrepawayNFT: max NFT mints per address exceeded"
        );
    ...
    function adminMint(
        ...
        require(to != address(0), "CrepawayNFT: Address cannot be 0");

        require(id <= maxId, "CrepawayNFT: Token id mismatch");
        require(
     ...
     **/
    function checkLimit(address minter, uint256 amount)
        public
        view
        returns (bool)
    {
        return minted[minter] + (amount) <= mintsPerAddressLimit;
    }
    ...
    function _initialAddTokens(
        ...
        require(
            ...
            "CrepawayNFT: IDs/MintCost arity mismatch"
        );
        require(
            maxId_ + 1 == supplyPerTokenId_.length,
            "CrepawayNFT: IDs/Supply arity mismatch"
        );

        for (uint256 i = 0; i < maxId_ + 1; i++) {
            mintPrice[i] = mintCostPerTokenId_[i];
            supplyPerTokenId[i] = supplyPerTokenId_[i];
        }
    }
