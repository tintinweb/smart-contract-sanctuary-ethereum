    ...
    OwnableUpgradeable
{
    ...
    function setDomain(
        ...
        if (claimerToExpireDate[tokenOwner] < block.timestamp && claimerToExpireDate[tokenOwner] > 0) {
            bytes32 old_domain = claimerToHash[tokenOwner];
            _reset(tokenOwner);
            emit AddrChanged(old_domain, address(0));
            emit AddressChanged(old_domain, 60, abi.encodePacked(address(0)));
        }
    ...
    function prolongENS(address claimer) external {
        require(
            _msgSender() == owner() ||
                (_msgSender() == claimer &&
                    claimerToHash[claimer] != 0x0 &&
    ...
    function forceReset(bytes32 hash) external onlyOwner {
        hashToClaimer[hash] = address(0);
        hashToDomainMap[hash] = "";

        emit AddrChanged(hash, address(0));
        emit AddressChanged(hash, 60, abi.encodePacked(address(0)));
    }
