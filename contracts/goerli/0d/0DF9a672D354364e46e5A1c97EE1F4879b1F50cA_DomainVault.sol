...
contract DomainVault is IDomainVault, ERC721Holder, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // ballot box
    uint256 private _ballotBox;

    // ballot box(uint256) => voter(address) => whether voted(bool)
    ...
    *  @notice Buy presale fragment using ETH
    ...
    function afterTokenTransferForDomainERC20(address from, address to, uint256 amount) external {
        ...
        if (voteEndTime > _currentTime()) {
            //判断投票人 from
            if (from != address(0) && _whetherVotedMapping[_ballotBox][from]) {
                //销毁或转给抵押库 不减票
                ...
                if (fragmentBalance == 0) {
                    _whetherVotedMapping[_ballotBox][from] = false;
                    emit RemoveVoter(from);
                }
            }
            //判断投票人 to
            if (to != address(0) && _whetherVotedMapping[_ballotBox][to]) {
                votes += amount;
                afterVote();
                emit Vote(_msgSender(), votes);
            }
    ...
    function redeem() public whenNotPaused nonReentrant {
        ...
        if (bidPrice > 0 && bidder != address(0)) {
            //IERC20Metadata(priceCurrency).transfer(bidder, bidPrice);
    ...
    function vote() external whenNotPaused nonReentrant {
        ...
        bool voted_ = _whetherVotedMapping[_ballotBox][_msgSender()];

        //是否重复投票
        require(!voted_, "DomainVault: you has voted");
        ...
        //添加投票人
        if (!voted_) {
            _whetherVotedMapping[_ballotBox][_msgSender()] = true;
