...
contract Payments is Ownable {
    ...
    function buyLinkToken(
        ...
        LinkTokenInterface LINKTOKEN = LinkTokenInterface(chainLinkToken);
        address tokenToConvertFrom_ = tokenToConvertFrom;
        uint256 amountToTokenToBuy_ = amountToTokenToBuy;
        address _linkTokenAddress = chainLinkToken;
        ...
        if (chainLinkPeggedToken != address(0x0)) {
            LinkSwapper swapper = LinkSwapper(chainLinkPeggedSwapContract);
            LINKTOKEN.approve(chainLinkPeggedSwapContract, recievedCl);
            swapper.swap(recievedCl, chainLinkToken, chainLinkPeggedToken);
            LINKTOKEN = LinkTokenInterface(chainLinkPeggedToken);
        }
    ...
    function createSubscriptionAndTopUpBalance(
        ...
        // if there is a pegged version, we need to convert it to that pegged version.
        // if (chainLinkPeggedToken != address(0x0)) {
        //     LinkSwapper swapper = LinkSwapper(chainLinkPeggedSwapContract);
        //     LinkToken.approve(chainLinkPeggedSwapContract, amountOfLinkToTopUp);
        //     swapper.swap(
        //         amountOfLinkToTopUp,
        //         chainLinkToken,
        //         chainLinkPeggedToken
