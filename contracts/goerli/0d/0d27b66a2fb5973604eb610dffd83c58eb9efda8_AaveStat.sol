...
contract AaveStat {
    ...
    function calcuUserLiquidationPrice(
        ...
        address[] memory statAssets,
        address[] memory stablecoins,
        address pool,
        bool checkStable
    ) external view returns(LiquidationPrice[] memory) {
        require(users.length == statAssets.length,"Array size error");
        address[] memory reserves = ILendingPool(pool).getReservesList();
        address oracle = getOracle(pool);
        ...
        for(uint i=0; i<users.length; i++){
            CalcuParam memory cp = CalcuParam({
                user: users[i],
                userConfig: ILendingPool(pool).getUserConfiguration(users[i]),
                reserves: reserves,
                oracle: oracle,
                pool: pool
            });
            UserAssets memory userAssets = getUserAssets(cp);
            arr[i] = LiquidationPrice({
                user: users[i],
                asset: statAssets[i],
                price: calculateLiquidationPrice(userAssets, statAssets[i], stablecoins, checkStable)
            });
        }
    ...
    struct StatAssetLiquidationAmountParam {
        address statAsset;
        address[] againstAssets;
        bool flag;
        address[] userList;
        uint256[] priceListETH;
        address pool;
    }

    function statAssetLiquidationAmount(
        address statAsset,
        address[] memory againstAssets,
        bool flag,
        address[] memory userList,
        uint256[] memory priceListETH,
        address pool
    ) external view returns(LiquidationAmount[] memory) {
        return statAssetLiquidationAmount_(StatAssetLiquidationAmountParam({
            statAsset: statAsset,
            againstAssets: againstAssets,
            flag: flag,
            userList: userList,
            priceListETH: priceListETH,
            pool: pool
        }));
    }

    function statAssetLiquidationAmount_(
        StatAssetLiquidationAmountParam memory laparam
    ) internal view returns(LiquidationAmount[] memory) {
        address[] memory reserves = ILendingPool(laparam.pool).getReservesList();
        address oracle = getOracle(laparam.pool);
        LiquidationAmount[] memory list = initLiquidationArr(laparam.priceListETH);

        for(uint i=0; i<laparam.userList.length; i++) {
            address user = laparam.userList[i];
            CalcuParam memory cp = CalcuParam({
                user: user,
                userConfig: ILendingPool(laparam.pool).getUserConfiguration(user),
                reserves: reserves,
                oracle: oracle,
                pool: laparam.pool
            });
            UserAssets memory userAssets = getUserAssets(cp);
            for (uint j=0; j<laparam.priceListETH.length; j++) {
                CalculateUserAccountDataVars memory vars = calculateUserAccountData(userAssets, laparam.statAsset, laparam.priceListETH[j], laparam.againstAssets, laparam.flag);
                if (vars.healthFactor < HEALTH_FACTOR_LIQUIDATION_THRESHOLD) {
                    LiquidationAmount memory la = list[j];
                    la.price = laparam.priceListETH[j];
                    la.amount = la.amount + vars.statAssetSupplyAmount;
                }
            }
        }
    ...
    function getOracle(address pool) internal view returns(address) {
    ...
    function calculateLiquidationPrice(UserAssets memory userAssets, address statAsset,address[] memory stablecoins, bool checkStableCoin) 
        ...
        for (vars.i = 0; vars.i < userAssets.supplyList.length; ) {
    ...
    function calculateUserAccountData(UserAssets memory userAssets, address statAsset, uint256 unitPrice, address[] memory stablecoins, bool checkStableCoin) 
        ...
        for (vars.i = 0; vars.i < userAssets.supplyList.length; vars.i++) {
            ...
            if(statAsset == ad.asset.reserveAddress){
                ...
                vars.statAssetSupplyAmount = ad.amount;
            }
    ...
    function userAsets(
        address[] memory users,
        address pool
    ) external view returns(UserAssets[] memory) {
        address[] memory reserves = ILendingPool(pool).getReservesList();
        address oracle = getOracle(pool);

        UserAssets[] memory arr = new UserAssets[](users.length);
    ...
    function userparm(
        address user,
        address pool
    ) external view returns(CalcuParam memory) {
        address[] memory reserves = ILendingPool(pool).getReservesList();
        address oracle = getOracle(pool);
    ...
    function getUserAssets(CalcuParam memory param) internal view returns (UserAssets memory){
        ...
        userAssets.user = param.user;
        userAssets.supplyList = new AssetDetail[](0);
        userAssets.borrowList = new AssetDetail[](0);
        if (param.userConfig.isEmpty()) {
            return userAssets;
        }
        AssetDetail[] memory supplyArr = new AssetDetail[](param.reserves.length);
        AssetDetail[] memory borrowArr = new AssetDetail[](param.reserves.length);
        uint sl = 0;
        uint bl = 0;
        for (uint i = 0; i < param.reserves.length; i++) {
            ...
            DataTypes.ReserveData memory currentReserve = ILendingPool(param.pool).getReserveData(rinfo.reserveAddress);
        ...
        userAssets.supplyList = new AssetDetail[](sl);
        userAssets.borrowList = new AssetDetail[](bl);
        if (param.userConfig.isEmpty()) {
            return userAssets;
        }

        for(uint i=0;i<sl;i++){
            userAssets.supplyList[i] = supplyArr[i];
        }
        for(uint i=0;i<bl;i++){
            userAssets.borrowList[i] = borrowArr[i];
        }
