...
interface IUNFTRegistry {
  ...
  function batchUpgradeUNFT(address[] calldata nftAssets) external;

  function batchUpgradeAllUNFT() external;
...
contract UNFTUpgradeableProxy is TransparentUpgradeableProxy {
  constructor(
    address _logic,
    address admin_,
    bytes memory _data
...
import {UNFTUpgradeableProxy} from "../../libraries/proxy/UNFTUpgradeableProxy.sol";
...
contract UNFTRegistry is IUNFTRegistry, Initializable, OwnableUpgradeable {
  ...
  modifier onlyClaimAdmin() {
    require(claimAdmin() == _msgSender(), "UNFTR: caller is not the claim admin");
    _;
  }
   ...
   */
  function createUNFT(address nftAsset) external override nonReentrant returns (address uNftProxy) {
   ...
   */
  function setUNFTGenericImpl(address genericImpl) external override nonReentrant onlyOwner {
    require(genericImpl != address(0), "UNFTR: impl is zero address");
    uNftGenericImpl = genericImpl;
  ...
  function upgradeUNFTWithImpl(
    ...
    _upgradeUNFTWithImpl(nftAsset, uNftImpl, encodedCallData);
  }

  function _upgradeUNFTWithImpl(
    address nftAsset,
    address uNftImpl,
    bytes memory encodedCallData
  ) internal {
    ...
    UNFTUpgradeableProxy proxy = UNFTUpgradeableProxy(payable(uNftProxy));
  ...
  function batchUpgradeUNFT(address[] calldata nftAssets) external override nonReentrant onlyOwner {
    require(nftAssets.length > 0, "UNFTR: empty assets");

    for (uint256 i = 0; i < nftAssets.length; i++) {
      _upgradeUNFTWithImpl(nftAssets[i], uNftGenericImpl, new bytes(0));
    }
  }

  function batchUpgradeAllUNFT() external override nonReentrant onlyOwner {
    for (uint256 i = 0; i < uNftAssetLists.length; i++) {
      _upgradeUNFTWithImpl(uNftAssetLists[i], uNftGenericImpl, new bytes(0));
    }
  }
  ...
  function setClaimAdmin(address newAdmin) public virtual onlyOwner {
    require(newAdmin != address(0), "UNFTR: new admin is the zero address");
  ...
  function _createProxyAndInitWithImpl(address nftAsset, address uNftImpl) internal returns (address uNftProxy) {
    ...
    UNFTUpgradeableProxy proxy = new UNFTUpgradeableProxy(uNftImpl, address(this), initParams);
