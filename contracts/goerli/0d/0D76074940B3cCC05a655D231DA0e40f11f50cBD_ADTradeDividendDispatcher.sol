...
contract ADTradeDividendDispatcher is Father {
    using SafeMath for uint256;
    using Address for address;
    using SafeBEP20 for IBEP20;

    IADSwapHelperInterfae public swapHelper;
    ...
    event BuyDividendLiquidityDisptached(uint256 amount);
    ...
    event SellDividendLiquidityDisptached(uint256 amount);
    ...
    constructor(
        ...
        swapHelper = IADSwapHelperInterfae(swapHelper_);
        ADAOToken = IBEP20(adao_);

        fundAddress = fund_;
        holdingRewardAddress = holding_;
    }

    function changeSwaper(address swapHelper_) public {
        swapHelper = IADSwapHelperInterfae(swapHelper_);
    }

    function changeADAOToken(address adao) external onlyOwner {
        ADAOToken = IBEP20(adao);
    }

    function changeHoldingRewardAddress(address addr) external onlyOwner {
        holdingRewardAddress = addr;
    }
    ...
    function getBuyDividendPercents() public view returns (uint256) {
        return
            buyDividendDeadPercent
                .add(buyDividendHoldingPercent)
                .add(buyDividendLiquidityPercent)
                .add(buyDividendFundPercent);
    }

    function getSellDividendPercents() public view returns (uint256) {
        return
            sellDividendDeadPercent
                .add(sellDividendHoldingPercent)
                .add(sellDividendLiquidityPercent)
                .add(sellDividendFundPercent);
    }
    ...
    function dispatchSellDividend(address user, uint256 buyAmount)
        external
        onlyCaller
    {
        if (enableSellDispatch) {
            uint256 total = _sellDividend(buyAmount);
    ...
    function _buyDividendToDead(uint256 buyAmount)
        internal
        virtual
        returns (uint256)
    {
        uint256 realAmount = buyAmount.mul(buyDividendDeadPercent).div(100);

        ADAOToken.safeTransfer(deadAddress, realAmount);

        globalDeadTokenAmount = globalDeadTokenAmount.add(realAmount);

        emit BuyDividendDeadDisptached(deadAddress, realAmount);

        return realAmount;
    }

    function _buyDividendToFund(uint256 buyAmount)
        internal
        virtual
        returns (uint256)
    {
        uint256 realAmount = buyAmount.mul(buyDividendFundPercent).div(100);

        ADAOToken.safeTransfer(fundAddress, realAmount);

        globalFundTokenAmount = globalFundTokenAmount.add(realAmount);

        emit BuyDividendFundDisptached(fundAddress, realAmount);
        return realAmount;
    }

    function _buyDividendToHolding(uint256 buyAmount)
        internal
        virtual
        returns (uint256)
    {
        uint256 realAmount = buyAmount.mul(buyDividendHoldingPercent).div(100);

        swapHelper.swapADAOToBNB(realAmount, address(this));

        address(uint160(holdingRewardAddress)).transfer(address(this).balance);

        globalHoldingBNBAmount = globalHoldingBNBAmount.add(realAmount);

        emit BuyDividendHoldingDisptached(holdingRewardAddress, realAmount);

        return realAmount;
    }

    function _buyDividendToLiquidity(uint256 buyAmount)
        internal
        virtual
        returns (uint256)
    {
        uint256 realAmount = buyAmount.mul(buyDividendLiquidityPercent).div(
            100
        );

        uint256 realAmountHalf = realAmount.div(2);
        swapHelper.swapADAOToBNB(realAmountHalf, address(this));
        swapHelper.addADAOLiquidity(realAmountHalf, address(this).balance);
        ...
        returns (uint256)
    {
        ...
        dividendAmount = dividendAmount.add(_buyDividendToDead(buyAmount));
        dividendAmount = dividendAmount.add(_buyDividendToFund(buyAmount));
        dividendAmount = dividendAmount.add(_buyDividendToHolding(buyAmount));
        dividendAmount = dividendAmount.add(_buyDividendToLiquidity(buyAmount));

        return dividendAmount;
    }

    function _sellDividendToDead(uint256 sellAmount)
        internal
        virtual
        returns (uint256)
    {
        uint256 realAmount = sellAmount.mul(sellDividendDeadPercent).div(100);

        ADAOToken.safeTransfer(deadAddress, realAmount);

        globalDeadTokenAmount = globalDeadTokenAmount.add(realAmount);

        emit SellDividendDeadDisptached(deadAddress, realAmount);
        return realAmount;
    }

    function _sellDividendToFund(uint256 sellAmount)
        internal
        virtual
        returns (uint256)
    {
        uint256 realAmount = sellAmount.mul(sellDividendFundPercent).div(100);

        ADAOToken.safeTransfer(fundAddress, realAmount);

        globalFundTokenAmount = globalFundTokenAmount.add(realAmount);

        emit SellDividendFundDisptached(fundAddress, realAmount);
        return realAmount;
    }

    function _sellDividendToHolding(uint256 sellAmount)
        internal
        virtual
        returns (uint256)
    {
        uint256 realAmount = sellAmount.mul(sellDividendHoldingPercent).div(
            100
        );

        ADAOToken.safeTransfer(holdingRewardAddress, realAmount);

        globalHoldingTokenAmount = globalHoldingTokenAmount.add(realAmount);
        emit SellDividendHoldingDisptached(holdingRewardAddress, realAmount);
        return realAmount;
    }

    function _sellDividendToLiquidity(uint256 sellAmount)
        internal
        virtual
        returns (uint256)
    {
        uint256 realAmount = sellAmount.mul(sellDividendLiquidityPercent).div(
            100
        );
    ...
    {
        uint256 dividendAmount = 0;
        dividendAmount = dividendAmount.add(_sellDividendToDead(sellAmount));
        dividendAmount = dividendAmount.add(_sellDividendToFund(sellAmount));
        dividendAmount = dividendAmount.add(_sellDividendToHolding(sellAmount));
        // dividendAmount = dividendAmount.add(
        //     _sellDividendToLiquidity(sellAmount)
        // );
...
interface IADSwapHelperInterfae {
    ...
    function addADAOLiquidity(uint256 tokenAmount, uint256 ethAmount)
        external
        payable;
}
