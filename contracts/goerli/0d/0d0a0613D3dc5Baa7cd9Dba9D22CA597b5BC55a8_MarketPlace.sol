    ...
    EIP712Upgradeable
{
    ...
    event LogDiscount(uint256 discount, string name);
    ...
    function findFeePercent() public returns (uint256) {
        uint256 balanceInWei = weth.balanceOf(_msgSender());
        emit LogFeePercent(balanceInWei, "balanceInWei");
        uint256 balanceInEther = balanceInWei / 1 ether;
        emit LogFeePercent(balanceInEther, "balanceInEther");
        uint256 valueIs5 = 5;

        if (balanceInEther > 2) {
            uint256 discount = feePercent - 2;
            emit LogDiscount(discount, "Hurry you have a discount");
            return discount;
        } else if (balanceInEther > 1) {
            uint256 discount = feePercent - 1;
            emit LogDiscount(discount, "Hurry you have a discount");
            return discount;
        } else if (balanceInEther > (valueIs5 / 10)) {
            uint256 discount = feePercent - (valueIs5 / 10);
            emit LogDiscount(discount, "Hurry you have a discount");
            return discount;
        } else {
            emit LogDiscount(
