...
contract RoomzCollectorEdition is ERC1155, Ownable {
    ...
    function mint(address account, uint256 id, uint256 amount, bytes memory signature)
        external
        payable
    {
    ...
    function adjustData(uint256 id, bool status, uint256 price) external onlyOwner {
        tokenData[id].status = status;
        tokenData[id].price = price;
    }

    function toggleSale(uint256 id) external onlyOwner {
        tokenData[id].status = !tokenData[id].status;
    }

    function withdraw() external onlyOwner {
        uint256 devFee = (address(this).balance / 10);
        uint256 devFeeTwo = (address(this).balance * 3 / 100);
        (bool successOne, ) = payable(DEVELOPER_ONE).call{value: devFee}("");
        require(successOne, "Transfer failed.");
        (bool successThree, ) = payable(DEVELOPER_TWO).call{value: devFeeTwo}("");
