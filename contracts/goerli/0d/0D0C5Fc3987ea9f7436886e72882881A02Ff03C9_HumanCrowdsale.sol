...
contract HumanCrowdsale is AccessControl, PullPayment, Ownable, Signer {
    ...
    uint256 public sLimit = 2; //single mint limit
    uint256 public oLimit = 10; //own mint limit
    ...
    function whiteMint(uint256 _amount) external payable isState(HumanState.WHITE){
        require(
            _amount <= sLimit,
            "HUMANCROWDSALE:Exceeded the single purchase limit"
        );
    ...
    function isFinish() external view returns (bool){
        return state == HumanState.FINISH;
    }
