...
interface IMetriaAddressRegistry {
...
interface IMetriaTokenRegistry {
    function enabled(address) external returns (bool);
}

contract MetriaBundleMarketplace is Ownable, ReentrancyGuard {
    ...
    IMetriaAddressRegistry public addressRegistry;
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IMetriaTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "MetriaMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "MetriaMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IMetriaMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IMetriaMarketplace(addressRegistry.marketplace()).getPrice(
                _payToken
            ),
            price
        );
    ...
    /**
     @notice Update MetriaAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IMetriaAddressRegistry(_registry);
    }
