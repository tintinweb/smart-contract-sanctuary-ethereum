...
contract LevenToken is Initializable, ERC20Upgradeable, OwnableUpgradeable {
     ...
     */
    function preSaleBuy(uint256 amount) public payable {
        ...
        require(
            _maxPreSaleBuyLimit > amount,
            "Unable to buy token due to maximum purchase limit reached"
        );
        uint256 calculatedLevenToken = (amount / _price) * 10**decimals();
        payable(address(this)).transfer(amount);
        _transfer(address(this), _msgSender(), calculatedLevenToken);
        _soldPreSaleAmount = _soldPreSaleAmount + calculatedLevenToken;
    }
