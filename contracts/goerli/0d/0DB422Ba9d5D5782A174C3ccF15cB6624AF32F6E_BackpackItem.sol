  ...
  IBackpackItem
{
  ...
  function fulfill(address recipient) external onlyOperatorsAndOwner {
    uint256[] memory eligibleTokenIds = _eligibleTokenIds();
    uint256 eligibleTokenIdCount = eligibleTokenIds.length;

    if (eligibleTokenIdCount <= fulfillmentQuantity) {
      _mintBatch(recipient, eligibleTokenIds, _quantities(eligibleTokenIdCount), "");
    } else {
      _mintBatch(recipient, _selectTokens(eligibleTokenIds, fulfillmentQuantity), _quantities(fulfillmentQuantity), "");
    }
  }

  function _selectTokens(uint[] memory sourceArray, uint n) internal view returns (uint[] memory) {
    uint[] memory selectedItems = new uint[](n);
    ...
    while (i != n) {
      found = false;
      randomIndex = uint(uint256(keccak256(abi.encodePacked(block.timestamp, i + iterations))) % sourceLength);
      for (uint256 j = 0; j < i; ) {
        if (selectedItems[j] == sourceArray[randomIndex]) {
          found = true;
          ++iterations;
          break;
        }
  ...
  function _eligibleTokenIds() internal view returns (uint256[] memory) {
    ...
    for (uint256 i = 0; i < tokenIdCount; ) {
      fulfillmentRule memory rule = fulfillmentRules[i];
      if (rule.eligible && _hasAvailableSupplyForFulfillment(rule.maxSupply, totalSupply(i))) {
        unchecked {
          ++count;
        }
      }
  ...
  function airdropBatch(
    ...
    if (!_hasAvailableSupply(tokenID, sum(amounts))) revert ExceedsMaxSupply();
