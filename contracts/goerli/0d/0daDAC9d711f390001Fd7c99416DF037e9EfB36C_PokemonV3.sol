...
contract PokemonV3 is IERC721, Pausable, Ownable, VRFV2WrapperConsumerBase {
    ...
    //todo find more efficient way of storing each accounts pokemon ids
    //todo change ownedPokemonIds when transfering pokemons
    ...
    mapping(address => uint256[]) public ownedPokemonIds;
    mapping(address => EnumerableSet.UintSet ) ownedPokemonIds2;
    ...
    function mint() external whenNotPaused {
        ...
        ownedPokemonIds[msg.sender].push(totalSupply);
        ...
        ownedPokemonIds2[msg.sender].add(totalSupply);

        emit Transfer(address(0), msg.sender, totalSupply);
    }

    function getSet(address user) public view returns(uint256[] memory) {
        return ownedPokemonIds2[user].values();
    }
    ...
    function _randomDna() private returns (string memory) {
        ...
        uint256 minNumber = 1;
        string memory hat = Strings.toString(
            uint256(
                keccak256(
        ...
        if (hat_.length < 2) {
            hat = string(abi.encodePacked("0", hat));
        }
        randNonce++;
        string memory head = Strings.toString(
            uint256(
                keccak256(
                    abi.encodePacked(block.timestamp, msg.sender, randNonce)
                )
        ...
        if (head_.length < 2) {
            head = string(abi.encodePacked("0", head));
        }
        randNonce++;
        string memory body = Strings.toString(
            uint256(
                keccak256(
                    abi.encodePacked(block.timestamp, msg.sender, randNonce)
                )
        ...
        if (body_.length < 2) {
            body = string(abi.encodePacked("0", body));
        }
        randNonce++;
        string memory legs = Strings.toString(
            uint256(
                keccak256(
                    abi.encodePacked(block.timestamp, msg.sender, randNonce)
                )
        ...
        if (legs_.length < 2) {
            legs = string(abi.encodePacked("0", legs));
        }
