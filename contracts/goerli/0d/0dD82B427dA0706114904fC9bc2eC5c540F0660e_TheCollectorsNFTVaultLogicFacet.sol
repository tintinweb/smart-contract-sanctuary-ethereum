...
contract TheCollectorsNFTVaultLogicFacet is TheCollectorsNFTVaultBaseFacet {
    ...
    */
    ...
    /*
        @dev
        Setting the token id(s) that the vault can purchase 1 token from and the maximum buying price.
        After setting token info, participants can vote for or against buying it.
        ...
        Participants can call this method again in order to change the token info and max buying price.
        Everytime this function is called all the votes are reset and the voting starts again.
        If the vault is being kept hostage by a participant by always resetting the votes,
        the other participants can always withdraw their ETH and just open a new vault without the bad actors.
    */
    function setTokenInfoAndMaxBuyPrice(
        uint64 vaultId,
        address collection,
        uint256[] calldata authorizedTokenIdsToPurchase,
        uint128 maxBuyPrice
    ) external {
        LibDiamond.AppStorage storage _as = LibDiamond.appStorage();
        ...
        vault.authorizedTokenIdsToPurchase = authorizedTokenIdsToPurchase;
        vault.buyAnyToken = authorizedTokenIdsToPurchase.length == 0;
        ...
        for (uint256 i; i < vaultExtension.numberOfParticipants;) {
            ...
            if (participant.participant == msg.sender) {
                ...
                emit NFTTokenWasSet(vault.id, vault.collection, authorizedTokenIdsToPurchase, maxBuyPrice);
    ...
    */
    function _vote(uint64 vaultId, LibDiamond.Participant storage participant, bool yes, LibDiamond.VoteFor voteFor) internal {
        ...
        if (voteFor == LibDiamond.VoteFor.Buying) {
            emit VotedForBuy(vaultId, participant.participant, yes, vault.collection, vault.authorizedTokenIdsToPurchase);
