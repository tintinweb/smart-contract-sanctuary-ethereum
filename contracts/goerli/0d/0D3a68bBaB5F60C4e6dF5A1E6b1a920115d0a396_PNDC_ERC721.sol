...
contract PNDC_ERC721 is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, GelatoRelayContext {
    ...
    function safeMintLazy(
        address to,
        string memory uri,
        LibShare.Share[] memory royalties
    ) external onlyGelatoRelay returns(uint256){
        _transferRelayFee();
        uint256 tokenId = _tokenIdCounter.current();
        _setRoyaltiesByTokenId(tokenId, royalties);
        _tokenIdCounter.increment();
