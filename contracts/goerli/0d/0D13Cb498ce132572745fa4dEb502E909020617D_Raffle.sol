...
contract Raffle is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    constructor(
        ...
        s_lastTimeStamp = block.timestamp; //keep track of the time that the contract is deployed
        i_interval = interval;
    }

    function enterRaffle() public payable {
        //ensure user pay enough entrance fee
     ...
     * It should returns true when:
     * 1. the time interval has passed
     *    (block.timestamp - last block timestamp) > interval
     * 2. lottery should at least 1 player and have some ETH
     * 3. Subscription is funded with LINK
     * 4. lottery is 'open', a state where players can enter the raffle
     */
    function checkUpkeep(
        bytes memory /* checkdata */ //change to public from external to allow performUpkeep function to call this function
    )
        public
        override
        returns (
    ...
    function performUpkeep(
        ...
        //requestRandomWords function emits a RandomWordsRequested event
        //RandomWordsRequested event contains the requestId
        emit RequestedRaffleWinner(requestId); //can be omitted
    }
