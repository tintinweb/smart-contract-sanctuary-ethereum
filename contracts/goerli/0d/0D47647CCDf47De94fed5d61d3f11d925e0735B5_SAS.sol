...
contract SAS is ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 public rewardsDuration = 3 hours;
    ...
    uint256 public constant minDuration = 1 hours;
    uint256 public constant maxDuration = 3 hours;
    ...
    function batchWithdraw(uint256[] calldata lockIds) external nonReentrant {
        for(uint i = 0; i < lockIds.length; i++) {
            require(i < locks[msg.sender].length, "Lock id does not exist");
            require(!isWithdrawn(msg.sender, i), "Lock already withdrawn");
            require(isUnlocked(msg.sender, i), "Cant withdraw before unlock time");

            updateReward(msg.sender, i);

            LockedBalance memory lock = locks[msg.sender][i];
            ...
            stakingToken.safeTransfer(msg.sender, lock.balance);
            emit Withdrawn(msg.sender, lock.balance, i);
    ...
    function batchWithdrawEarly(uint256[] calldata lockIds) external nonReentrant {
        for(uint i = 0; i < lockIds.length; i++) {
            require(i < locks[msg.sender].length, "Lock id does not exist");
            require(!isWithdrawn(msg.sender, i), "Lock already withdrawn");
            require(!isUnlocked(msg.sender, i), "Unlock time has passed, use standard withdraw instead");
