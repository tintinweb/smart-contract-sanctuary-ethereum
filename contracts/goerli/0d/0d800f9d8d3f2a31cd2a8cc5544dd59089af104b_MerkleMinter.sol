...
// Non-reusable dumb implementation of merkle minter for the interface particle #1
...
contract MerkleMinter is Ownable, ReentrancyGuard {
  ...
  uint256 public constant mintStart = 15430000;
  ...
  ) public payable enough(mintMerklePrice) started nonReentrant {
    if (!allowListed(msg.sender, _proof)) revert Errors.NotAllowListed();
    _mintLogic(tokenId, editions);
  }

  function mintPublic(uint256 tokenId, uint256 editions)
    public
    payable
    enough(mintPrice)
    started
  ...
  function _mintLogic(uint256 tokenId, uint256 editions) private {
