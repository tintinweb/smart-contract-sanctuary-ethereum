...
contract Stencils is IStencils, ERC721Enumerable {
    ...
    uint256 public LAUNCH_TIMESTAMP;
    uint256 public AUCTION_END_TIMESTAMP;
    ...
    function purchase(address destination_, uint128 quantity_, uint128 minQuantity_) external payable onlyAfterLaunch returns (uint256[] memory tokenIds_) {
        if (quantity_ == 7) {
            AUCTION_END_TIMESTAMP = block.timestamp + 120 + (AUCTION_END_TIMESTAMP - LAUNCH_TIMESTAMP);
            LAUNCH_TIMESTAMP = block.timestamp + 120;
