    ...
    ReentrancyGuard
{
    ...
    function initialize(address operatorAddress_, address adminAddress_, address devAddress_) initializer external {
        ...
        validatorCount = 0;
        stakefishCommissionRateBasisPoints = 1000;
    ...
    function updateComputationCache() internal {
        ...
        uint256 balanceDiffSinceLastUpdate =
            address(this).balance - cache.totalUncollectedCommission - cache.totalUncollectedUserBalance - cache.totalUnsentUserRewards;
    ...
    function _joinPool(
        ...
        require(
            !validatorsInPool[validatorPubKey],
        ...
        validatorsInPool[validatorPubKey] = true;
    ...
    function partPool(
        ...
        _partPool(validatorPubKey, depositor, leaveTime);
    ...
    function _partPool(
        ...
        users[depositor].validatorCount -= 1;
        validatorsInPool[validatorPubKey] = false;
    ...
    function bulkJoinPool(
        ...
        address[] calldata depositorAddresses, uint256 ts
    ) external override nonReentrant operatorOnly {
        require(48 * depositorAddresses.length == validatorPubkeyArray.length, "validatorPubkeyArr has incorrect length");

        for(uint32 i = 0; i < depositorAddresses.length; i++) {
            _joinPool(validatorPubkeyArray[i*48:(i+1)*48], depositorAddresses[i], ts);
        }
    ...
    function bulkPartPool(
        ...
        require(48 * depositorAddresses.length == validatorPubkeyArray.length, "validatorPubkeyArray has incorrect length");

        for(uint32 i = 0; i < depositorAddresses.length; i++) {
            _partPool(validatorPubkeyArray[i*48:(i+1)*48], depositorAddresses[i], ts);
        }
    ...
    function pendingReward(address depositorAddress) external override view returns (uint256, uint256) {
        ...
        if (users[depositorAddress].validatorCount > 0) {
            uint256 balanceDiffSinceLastUpdate =
                address(this).balance - cache.totalUncollectedCommission - cache.totalUncollectedUserBalance;
            uint256 commission = balanceDiffSinceLastUpdate * stakefishCommissionRateBasisPoints / 10000;
    ...
    function _collectReward(
        ...
        if (users[depositorAddress].validatorCount > 0) {
            // refresh total uptime in the pool
            updateComputationCache();
            uint256 payoutUptime =
                block.timestamp * users[depositorAddress].validatorCount - users[depositorAddress].totalStartTimestamps;
            uint256 payoutAmount = computePayout(payoutUptime);
            ...
            cache.totalUncollectedUserBalance -= payoutAmount;
            cache.totalUnsentUserRewards += payoutAmount;
            users[depositorAddress].totalStartTimestamps = block.timestamp * users[depositorAddress].validatorCount;
    ...
    function collectPoolComission(address payable beneficiary, uint256 amountRequested)
        external
        override
        nonReentrant
        adminOnly
    {
        ...
        if (amountRequested == 0 || cache.totalUncollectedCommission < amountRequested) {
          ...
          beneficiary.sendValue(cache.totalUncollectedCommission);
        } else {
    ...
    function emergencyWithdraw (address depositorAddress, address beneficiary, uint256 maxAmount) 
        external
        override
        nonReentrant
        adminOnly 
    {
        ...
        _collectReward(depositorAddress, payable(beneficiary), maxAmount);
    }
...
interface IStakefishTransactionFeePool {
    ...
    function partPool(bytes calldata validatorPubkey, address depositorAddress, uint256 ts) external;
    function bulkJoinPool(bytes calldata validatorPubkeyArray, address[] calldata depositorAddress, uint256 ts) external;
    function bulkPartPool(bytes calldata validatorPubkeyArray, address[] calldata depositorAddress, uint256 ts) external;
    ...
    // @beneficiary is the address to send the reward to; defaults to msg.sender.
    ...
    function collectPoolComission(address payable beneficiary, uint256 amountRequested) external;
    ...
    // @depositorAddress is the address of the delegator in the pool for which to withdraw rewards from.
    // @beneficiary is the address to send the reward to; defaults to msg.sender, which is the admin.
    // @amountRequested is the maximum amount of tokens to withdraw. If set to 0, then all available rewards are withdrawn.
    function emergencyWithdraw(address depositorAddr, address beneficiary, uint256 amountRequested) external;
...
contract StakefishTransactionStorage {
    ...
    mapping(bytes => bool) internal validatorsInPool;
