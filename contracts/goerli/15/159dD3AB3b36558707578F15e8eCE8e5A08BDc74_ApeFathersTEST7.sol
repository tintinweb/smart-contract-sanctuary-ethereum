...
contract ApeFathersTEST7 is ERC721A, ERC721AQueryable, Ownable, Pausable, ReentrancyGuard, ERC2981, ERC4907A {
        using SafeMath for uint256;

    IERC721A tokensToBurnAddr = IERC721A(0x7d83f206DE2E16564e5EFD4332a755c9F4615AC1);
    ...
    event BurnClaimComplete(address indexed user, uint256[] newTokenIds, uint256[] tokenIds);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal {
        ApeFathersTEST7(_msgSender()).beforeTokenTransfer(from, to, tokenId);
    }
    ...
    */
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC2981, ERC721A, IERC721A, ERC4907A)
        returns (bool)
    {
    ...
    */
    ...
    function approveTokensForBurn(uint256[] calldata _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(IERC721A(tokensToBurnAddr).ownerOf(tokenId) == _msgSender(), "TOKEN NOT OWNED BY CALLER OR INVALID TOKEN ID");
            IERC721A(tokensToBurnAddr).approve(address(this), tokenId);
        }
    ...
    function burnClaim(uint256[] calldata _tokenIds, uint256 maxTokensToBurn) external nonReentrant {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            IERC721A(tokensToBurnAddr).transferFrom(_msgSender(), burnAddress, tokenId);
        ...
        emit BurnClaimComplete(_msgSender(), newTokenIds, _tokenIds);
    }
