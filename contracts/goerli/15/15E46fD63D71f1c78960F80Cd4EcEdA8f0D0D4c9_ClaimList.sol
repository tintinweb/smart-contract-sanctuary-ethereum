...
contract ClaimList is BaseMintModuleCloneable, IMintingModule {
    ...
    /// @param _minter The address of the wallet or contract that can call canMint
    /// @param _claimlistRoot merkel tree root for claimlist
    /// @param _mintPrice The price per mint
    function initialize(
        address _admin,
        address _minter,
    ...
    function decode(bytes memory data) public pure returns (uint256 maxAmount, uint256 claimAmount) {
    ...
    function canMint(
        ...
        (uint256 maxAmount, uint256 claimAmount) = decode(data);
...
contract BaseMintModule is AccessControl {
    address public minterAddress;

    uint256 public maxClaim; // number of tokens that can be minted per unqiue id
    uint256 public mintPrice; // cost to mint a single token
    bool public isActive; // if the mint is active and can be used
    ...
    /// @notice Constructor function for contract creation
    /// @param _admin The address of the admin
    /// @param _minter The address of the wallet or contract that can call canMint
    /// @param _mintPrice The price per mint
    constructor(
