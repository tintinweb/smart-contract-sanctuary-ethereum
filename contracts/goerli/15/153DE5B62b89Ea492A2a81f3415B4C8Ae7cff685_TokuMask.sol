...
contract TokuMask is ERC721Enumerable, Pausable, Ownable, ERC721Royalty, ReentrancyGuard {
    ...
    uint64 constant MAX_MINT_AMOUNT = 5;

    uint256 public cost = 0.1 ether;

    uint96 private immutable royalty = 400; // 4%

    IERC20 token; // reward token on mint
    uint256 rewardAmount = 100 ether;

    bool public revealed = false;
    string public nonRevealedURI;

    constructor(string memory _initURI, address _rewardTokenAddress) ERC721("TokuMask", "MASK") {
        setNonRevealedURI(_initURI);
    ...
    /**
     * Mint upto max 5 NFTs to the recipient
     * should be under supply and have enough ETH for each NFT price
     */
    function mint(
        uint256 mintAmount,
        address _recipient
    ) external payable nonReentrant whenNotPaused beforeMint(mintAmount) {
        if (_msgSender() != owner()) {
        ...
        token.transfer(_recipient, mintAmount * rewardAmount);
    }
     ...
     */
    function reserveTokuNFT(
        uint256 _mintAmount
    ) public onlyOwner beforeMint(_mintAmount) whenNotPaused {
