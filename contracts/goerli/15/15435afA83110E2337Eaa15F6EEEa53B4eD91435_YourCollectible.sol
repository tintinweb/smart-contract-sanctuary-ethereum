...
contract YourCollectible is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
  ...
  function mintPublicSale(uint256 _mintAmount, string memory tokenURI) external payable {
    require(publicSaleLive, 'the public sale is not open yet');
    require(_mintAmount > 0, 'amount to mint should be a positive number');
    require(numberMinted[msg.sender] <= 4, 'you have already minted more than the amount you are trying to mint');

    address minter = _msgSender();
    require(tx.origin == minter, 'contracts are not allowed to mint');

    require(MINT_PRICE * _mintAmount <= msg.value, 'insufficient funds');
  ...
  function mintItem(address to, string memory tokenURI) public returns (uint256) {
    require(publicSaleLive, 'Public sale has not started yet');
  ...
  function setPublicSaleLive(bool live) public {
    publicSaleLive = live;
  }
}