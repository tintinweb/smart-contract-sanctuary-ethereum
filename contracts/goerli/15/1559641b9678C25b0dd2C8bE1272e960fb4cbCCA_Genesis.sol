    ...
    IGenesis
{
    ...
    event SetRevealUri(string oldValue, string newValue);
    event SetRevealAdmin(address indexed oldValue, address indexed newValue);
    event SetReveal(bool oldValue, bool newValue);
    ...
    function setRevealUri(string memory _revealUri) external onlyRevealAdminOrOwner {
        ...
        emit SetRevealUri(_oldValue, revealUri);
    }
     ...
     */
    function setRevealAdmin(address _revealAdmin) external onlyOwner {
        address _oldValue = revealAdmin;
        revealAdmin = _revealAdmin;
        emit SetRevealAdmin(_oldValue, revealAdmin);
    }
    ...
    function setReveal(bool _isReveal) external onlyRevealAdminOrOwner {
        ...
        isReveal = _isReveal;
        emit SetReveal(_oldValue, isReveal);
    }
     ...
     */
    function mintBatch(address receiver, uint256 times)
        external
        onlyAdmin
