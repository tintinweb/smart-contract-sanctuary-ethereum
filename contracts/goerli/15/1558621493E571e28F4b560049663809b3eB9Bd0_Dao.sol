...
library Constants {
    ...
    address private constant USDC =
        address(0x8Caa28b5E98f31E96E2ACF725216e89b91992348);  //test network
    ...
    uint256 private constant EPOCH_START = 1671803660;  //test
...
contract Bonding is Setters, Permission {
    ...
    */
    ...
    function depositCoupons(uint256[] calldata couponIds) internal {
    ...
    function withdraw(uint256 value) internal {
        ...
        require(
            balanceOfStaged(msg.sender) >= balanceOfCouponStaged(msg.sender),
            "Bonding: insufficient dollar staged balance"
        );
        dollar().transfer(msg.sender, value);
    }

    function withdrawCoupons(uint256[] calldata couponIds) internal {
        require(coupon().ownerOf(couponIds) == msg.sender, "not the owner");
        uint256 value = coupon().getCouponsValue(couponIds);
    ...
    function unbond(uint256 value) internal {
    ...
    function unbondUnderlying(uint256 value) internal {
        uint256 balance = value.mul(totalNet()).div(totalBonded());
        incrementBalanceOfStaged(msg.sender, value);
    ...
    */
    ...
    function unstakeCoupons(uint256[] calldata couponIds) external {
        uint256 value = coupon().getCouponsValue(couponIds);
        unbondUnderlying(value);
...
contract Regulator is Comptroller {
    ...
    function growSupply(Decimal.D256 memory price) private {
        ...
        uint256 newSupply = delta.mul(totalSupply()).asUint256();
    ...
    AccessControlEnumerableUpgradeable
{
    ...
    function initialize(
        ...
        // initial lp contributor rewards:
        // mintToAccount(0x4f714880A5847D335606bb37848CcAcbcD6d5836, 1000e18); // 1000 pina
    }
    ...
    function advance() external incentivized onlyRole(ADVANCE_ROLE) {
        Bonding.bondingStep();
    ...
    modifier incentivized() {
        ...
        // ).latestRoundData();
        // (, int256 fastGasPrice, , , ) = AggregatorV3Interface(
        ...
        // });
        // Decimal.D256 memory usdCost = ethSpent.mul(
        //     Decimal.D256({
        //         value: uint256(ethPrice).mul(1e10) // chainlink ETH price has 8 decimals
        //     })
        // );
        _;
        Decimal.D256 memory usdCost = Decimal.zero();
        Decimal.D256 memory incentive = (usdCost.add(Constants.getAdvanceIncentive())).div(_state.price);
