...
contract P2pDefi is Ownable {
    ...
    function createLendOrder(
        ...
        orderLendMap[_lendingToken][msg.sender][0] = OrderLendData(
            0,
            _lendingToken,
            _termPeriodInDays,
            _lendingAmount,
    ...
    function createBorrowOrder(
        ...
        Status status,
        bool isActive
    ) public returns (OrderBorrowData memory) {
    ...
    function getBorrowOrders(
        address _user,
        address _borrowingToken,
        uint256 id
    ) public view returns (OrderBorrowData memory) {
        return orderBorrowMap[_borrowingToken][_user][id];
    }
}
