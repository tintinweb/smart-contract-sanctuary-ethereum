    ...
    AccessControlUpgradeable
{
    ...
    // Mapping to store users protection contract address
    mapping(address => address) public userToProtectionContract;
    ...
    // Check if user has staked NFT and provided valid details
    modifier validDetails(UserUtility.UserDetails memory user) {
        if ((INFTStaking(stakingContract).getStakesOf(msg.sender)).length == 0)
            revert NFTNotStaked();

        if (user._beneficiary == address(0) || user._beneficiary == msg.sender)
            revert InvalidBeneficiary();

        if (
            (user._erc721Assets.length != user._tokenIds.length) ||
            (user._erc1155Assets.length != user._ids.length)
        ) revert AssetAndIdsLengthMismatch();

        for (uint256 i = 0; i < user._erc721Assets.length; i++) {
            UserUtility.checkOwner(user._erc721Assets[i], user._tokenIds[i]);
        }
        _;
    }
    ...
    // Register user- If ETH is provided converts it to WETH
    // Deploys the protection contract for user
    function register(UserUtility.UserDetails memory user)
        external
        payable
        validDetails(user)
    {
        ...
        // If Ethereum is selected
        if (msg.value > 0) {
            protectETH = true;
            swapEthToWeth(msg.sender);
        } else {
            if (
                (user._erc20Assets.length +
                    user._erc721Assets.length +
                    user._erc1155Assets.length) == 0
            ) revert NoAssetProvided();
        }

        address protectionContract = address(
            new Protection(msg.sender, user, protectETH)
        );
        Protection(protectionContract).transferOwnership(msg.sender);

        userToProtectionContract[msg.sender] = protectionContract;
        emit UserRegistered(
            msg.sender,
            protectionContract,
            user._erc20Assets,
    ...
    function transferUserAssets(address user)
        public
        onlyRole(TRANSFER_ROLE)
        returns (bool)
    {
        address protectionAddress = userToProtectionContract[user];
        if (protectionAddress == address(0)) revert UserNotRegistered();

        Protection(protectionAddress).transferAllAssets();
        address beneficiary = Protection(protectionAddress).beneficiary();
        INFTStaking(stakingContract).transferStakedNFTs(user, beneficiary);
        bool transferred = Protection(protectionAddress).assetsTransferred();
        return transferred;
    }

    function setUserHasStaked(address _user, bool _hasStaked) public {
        if (msg.sender != stakingContract) revert OnlyStakingContract();

        address protectionAddress = userToProtectionContract[_user];
        if (protectionAddress != address(0))
            Protection(protectionAddress).setHasStaked(_hasStaked);
    }
...
interface INFTStaking {
    function getStakesOf(address account)
        external
        view
        returns (uint256[] memory);

    function transferStakedNFTs(address _user, address _beneficiary) external;
}
