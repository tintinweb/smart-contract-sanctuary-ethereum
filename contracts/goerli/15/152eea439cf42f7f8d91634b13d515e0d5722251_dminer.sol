...
contract dminer is Ownable, ReentrancyGuard {
    ...
    */
    ...
    mapping(address=>mapping(uint256=>mapping(uint256=>uint256[]))) private nftTicketHolding;
  ...
  bytes32 private constant _TRANSFER_EVENT_SIGNATURE =
    ...
    function getNFTTicketHolding(address wallet, uint256 roundId, uint256 tokenId) public view returns(uint256[] memory){
        return nftTicketHolding[wallet][roundId][tokenId];
    }
  ...
  function buyTickets(  bytes calldata signature,   uint256 quantity,  uint256 tokenIdHolding,  uint256 multiplier) external payable nonReentrant {
        ...
        if(round.ticketsSold > 10000) revert TicketsSold();
        ...
        for(uint256 i= 0;i<quantity;){
            ...
            TicketKey[roundId][ticketId] = Ticket(buyer, tokenIdHolding, winningOre);
            ticketHolding[buyer][roundId].push(ticketId);
            nftTicketHolding[buyer][roundId][tokenIdHolding].push(ticketId);
    ...
    function buyTicketsML2( /* bytes calldata signature,  */ uint256 quantity,  uint256 tokenIdHolding,  uint256 multiplier) external payable nonReentrant {
        //require(msg.value == ticketPrice.mul(quantity), "DIGI: Send proper ticket fees");
        //if(msg.value != ticketPrice.mul(quantity)) revert BadFees();
    ...
    function getTicketKey(uint256 ticketId) private view returns(uint256){
        return uint(keccak256(abi.encodePacked(msg.sender, ticketId, block.difficulty, block.number))) % 10**5;
    }
    ...
    function getWinningOre(uint256 winningKey_, uint256 roundId, uint256 TicketKey_, uint256 multiplier) private view returns(Ores Ore){
       // Ores previousOre = Ores.Iron; //starting from lowest
        ...
        uint256 multiplierResult= ticketKeyFactory(TicketKey_, i).mul(winningKey_) % 10**5 ;
    ...
    function ticketKeyFactory(uint256 ticketKey, uint256 multiplier) public pure returns(uint256){
        return (ticketKey.mul(multiplier)) % 10**5;
    }
    ...
    function preClaim(uint256 roundId, uint256 ticketId) private returns(uint256) {
        ...
        if(DIGI_NFT.ownerOf(ticket.tokenId) != _msgSender()) revert NotOwner();
