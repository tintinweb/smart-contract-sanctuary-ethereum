    ...
    LimitOrdersStorage
{
        ...
        allowedOrder(_tokenId, _poolAddress)
    {
        ...
        require(order.poolData.token0 != address(0), "EXEO1");
        require(checkClosability(_poolAddress, _tokenId), "EXEO4");
        (uint256 toTransferToken0, uint256 toTransferToken1) = _closeOrder(
            _tokenId,
            order
        );
        if (order.orderType == OrderType.BUY_LIMIT) {
            require(toTransferToken0 >= order.receivedAmount, "EXEO5");
        } else if (order.orderType == OrderType.TAKE_PROFIT) {
            require(toTransferToken1 >= order.receivedAmount, "EXEO5");
        }
        if (order.orderType == OrderType.BUY_LIMIT) {
        ...
        if (address(positionRouter) != order.owner)
            _repayForTransaction(
                _poolAddress,
                _tokenId,
                startGas,
                toTransferToken0,
        ...
        uint256 balanceOfToken0After = IERC20Upgradeable(order.poolData.token0)
            .balanceOf(address(this));
        uint256 balanceOfToken1After = IERC20Upgradeable(order.poolData.token1)
            .balanceOf(address(this));
        emit ExecuteOrder(
            order.owner,
            _tokenId,
        ...
        if (toTransferToken0 > 0) {
            uint256 transferCost = gasleft();
            IERC20Upgradeable(order.poolData.token0).transfer(
                order.owner,
                balanceOfToken0After
            );
            if (transferCost - gasleft() > TRANSFER_GAS)
                TRANSFER_GAS = transferCost - gasleft();
        }
        if (toTransferToken1 > 0) {
            uint256 transferCost = gasleft();
            IERC20Upgradeable(order.poolData.token1).transfer(
                order.owner,
        ...
        if (address(positionRouter) == order.owner) {
            order.executionCost =
                ...
                (startGas - gasleft());
            positionRouter.executionComplite(_tokenId, startGas);
        }
    }
    ...
    function _repayForTransaction(
        ...
        bytes memory call = abi.encodeWithSelector(
            this.repayForTransaction.selector,
            _poolAddress,
            _tokenId,
            startGas,
            availableToken0,
        ...
        (bool success, ) = repayerDelegator.delegatecall(call);
        if (success == false) {
            assembly {
                let ptr := mload(0x40)
