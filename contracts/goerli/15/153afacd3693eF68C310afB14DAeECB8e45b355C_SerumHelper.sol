...
contract SerumHelper is Ownable, ReentrancyGuard, ERC1155Holder {
    DogcInterface public immutable dogc;
    ...
    event ClaimM1Serum(uint256[] _tokenIds);
    ...
    function mintM1BatchV2(uint256[] memory _tokenIds)
    external
    payable
    nonReentrant
    {
        ...
        require(msg.value >= serum.price() * _tokenIds.length, "Insufficient payment");

        uint256 validIdsLength = 0;
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (!isMinted[_tokenIds[i]]
                && dogc.ownerOf(_tokenIds[i]) == msg.sender
            ) {
                validIdsLength++;
        ...
        serum.safeTransferFrom(address(this), msg.sender, serum.M1_SERUM(), validIdsLength, "");
        emit ClaimM1Serum(_tokenIds);
    }
