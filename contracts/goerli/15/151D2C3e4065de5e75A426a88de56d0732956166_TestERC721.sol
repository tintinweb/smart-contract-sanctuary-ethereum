...
contract TestERC721 is ERC721 {
    ...
    function mintTo(address to) public {
        uint256 tokenToMint = nextTokenId();

        _latestTokenId = tokenToMint;

        _safeMint(to, nextTokenId(), "0x");
    }
    ...
    function batchMint(address to, uint24 size) external {
        for(uint24 i; i < size; i++) {
            mintTo(to);
        }
    }

    function nextTokenId() public view returns (uint256) {
        return _latestTokenId + 1;
    }
}
