...
contract MerklePermitter is IPermitter, OwnerTwoStep {
    ...
    bool private _merkleRootLocked;
    ...
    event MerklePermitterAdminLockedMerkleRoot();
    event MerklePermitterAdminUpdatedMerkleRootUri();
    ...
    function initialize(bytes memory initializationData) external override {
        if (_initialized) {
        ...
        (
            ...
            _merkleRootUri,
            initiallyLocked
        ) = abi.decode(initializationData, (bytes32, address, string, bool));
    ...
    function checkPermitterData(
        ...
        if (tokenIdHashes.length != merkleProofs.length) {
    ...
    /**
     * @dev Allows the admin to update the Merkle root, allowing a different set of ids to be validated.
     * @param newMerkleRoot_ The new merkle root.
     */
    function updateMerkleRoot(bytes32 newMerkleRoot_) external onlyOwner {
        if (_merkleRootLocked) {
            revert MerklePermitterMerkleRootLocked();
        }

        _merkleRoot = newMerkleRoot_;

        emit MerklePermitterAdminUpdatedMerkleRoot(newMerkleRoot_);
    }

    /**
     * @dev Allows the admin to update the Merkle root uri, allowing a different set of ids to be validated
     *   and a different set of data to be used.
     * 
    ...
    function updateMerkleRootUri(string memory newMerkleRootUri_) external onlyOwner {
        ...
        emit MerklePermitterAdminUpdatedMerkleRootUri();
    }

    /**
     * @dev Allows the admin to lock the Merkle root, indicating that the accepted ids can never change again.
     */
    ...
    // ============================================================
    /**
     * @notice Locks the Merkle root, indicating that the accepted ids can never change again.
     */
    function _lockMerkleRoot() internal {
        _merkleRootLocked = true;

        emit MerklePermitterAdminLockedMerkleRoot();
    }
    ...
    function _hash(uint256[] memory values) internal pure returns (bytes32[] memory result) {
        result = new bytes32[](values.length);
    ...
    /**
     * @notice Returns whether or not the merkle root is locked
     * @return merkleRootLocked Whether or not the merkle root is locked
     */
    function merkleRootLocked() external view returns (bool) {
        return _merkleRootLocked;
    }
