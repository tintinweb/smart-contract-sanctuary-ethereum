...
// File contracts/v1/governance/IPRTCLBaseGovernor.sol
...
// File contracts/v1/governance/PRTCLBaseGovernor.sol
 ...
 * @author Particle Collection - valdi.eth
 */
abstract contract PRTCLBaseGovernor is Context, ERC165, EIP712, IPRTCLBaseGovernor, IERC721Receiver, IERC1155Receiver, Ownable {
    using SafeCast for uint256;
    using Timers for Timers.BlockNumber;
     ...
     *
     * The governance executing address is always the Governor's own address. The executor is only able to invoke these
     * functions during the execution of the governor's {execute} function, and not under any other circumstances.
     */
    modifier onlyGovernance() {
    ...
    function _cancel(
        ...
        return _cancelProposal(proposalId);
    }

    function _cancelProposal(uint256 proposalId) internal returns (uint256) {
        _proposals[proposalId].canceled = true;
...
// File contracts/v1/governance/PRTCLGovernorCountingSimple.sol
...
// File contracts/v1/governance/PRTCLGovernorPreventLateQuorum.sol
 ...
 * Known limitation: Only one set of vote extension parameter for every collection.
 *
 * @author Particle Collection - valdi.eth
 */
...
// File contracts/v1/governance/PRTCLGovernorSettings.sol
...
// File contracts/v1/governance/IPRTCLVotes.sol
...
// File contracts/v1/governance/PRTCLGovernorVotes.sol
...
abstract contract PRTCLGovernorVotes is PRTCLBaseGovernor {
    IPRTCLVotes public immutable coreToken;

    constructor(IPRTCLVotes tokenAddress) {
        coreToken = tokenAddress;
    }
    ...
    function _getVotes(
        ...
        return coreToken.getPastVotes(account, blockNumber, collectionId);
    }
}


// File contracts/v1/governance/PRTCLGovernorVotesQuorumFraction.sol
...
/**
 * @dev Extension of {PRTCLGovernorVotes} for voting weight extraction from an {ERC20Votes} token and a quorum expressed as a
 * fraction of the total supply.
 *
 * Modified version of OpenZeppelin's GovernorVotesQuorumFraction.sol:
 * 1. Uses coreToken from PRTCLGovernorVotes
 ...
 * 
 * Known limitation: Only one `quorumNumerator` for every collection.
 *
 * @author Particle Collection - valdi.eth
 */
abstract contract PRTCLGovernorVotesQuorumFraction is PRTCLGovernorVotes {
    ...
    function quorum(uint256 blockNumber, uint256 collectionId) public view virtual override returns (uint256) {
        return (coreToken.getPastTotalSupply(blockNumber, collectionId) * quorumNumerator(blockNumber)) / quorumDenominator();
    }
...
// File contracts/v1/governance/PRTCLGovernorV1.sol
...
/// @dev Uses Particle's modified version of OpenZeppelin's Governor contract suite.
/// @custom:security-contact [email protected]
contract PRTCLGovernorV1 is PRTCLBaseGovernor, PRTCLGovernorPreventLateQuorum, PRTCLGovernorSettings, PRTCLGovernorCountingSimple, PRTCLGovernorVotes, PRTCLGovernorVotesQuorumFraction, ReentrancyGuard {
    using ECDSA for bytes32;

    /**
     * @dev Emitted when a min price is updated.
     */
    event MinPriceUpdated(uint256 collectionId, address currencyAddress, uint256 minPriceInWei);
    ...
    struct Bid {
        uint256 amountInWei;
        // Proceeds per token if sold for `amountInWei`
        uint256 proceedsPerTokenInWei;
        address currency;
        address buyer;
        bytes32 descriptionHash;
        bool withdrawn;
    ...
    // Minimum price to be paid per collection in wei
    ...
    constructor(IPRTCLVotes _tokenVotes, IPRTCLCollections721V1 _tokenData, address _signer)
        ...
        PRTCLGovernorVotesQuorumFraction(51)
    ...
    function proposeBuy(
        ...
        require(_verifyAddressSigner(signature));
        require(isCurrencyAllowed(currencyAddress), "Currency not allowed");
        require(coreTokenData.collectionCanBeSold(collectionId), "Collection cannot be sold");
        require(priceInWei >= _currencyMinBidPerCollection[currencyAddress][collectionId], "Minimum price not met");
        
        uint256 lastHighiestBidPId = _highestBidProposalIdPerCollection[collectionId];
        bool activePreviousBid = false;

        if (lastHighiestBidPId != 0) {
            Bid memory lastHighiestBid = _bids[lastHighiestBidPId];
            ProposalState lastHighiestBidState = state(lastHighiestBidPId);
            activePreviousBid = lastHighiestBidState == ProposalState.Active || lastHighiestBidState == ProposalState.Pending;

            require(!activePreviousBid || (currencyAddress == lastHighiestBid.currency && priceInWei >= (lastHighiestBid.amountInWei * 105 / 100)) , "Bid must match currently active bid currency and be at leaset 5% higher");
        }
        ...
        if (currencyAddress != address(0)) {
            ...
            IERC20(currencyAddress).transferFrom(
                msg.sender,
                address(this),
                priceInWei
            );
        ...
        if (activePreviousBid) {
            _cancelProposal(lastHighiestBidPId);
        }

        uint256 proposalId = _createBuyProposal(currencyAddress, priceInWei, collectionId, description);
     ...
     * Emits a {ProposalCreated} event.
     */
    function _createBuyProposal(address currencyAddress, uint256 priceInWei, uint256 collectionId, string memory description) private returns(uint256) {
        (address[] memory targets, uint256[] memory values, bytes[] memory calldatas) = buyProposalBuilder(collectionId);
        ...
        bid.amountInWei = priceInWei;
        bid.proceedsPerTokenInWei = coreTokenData.proceedsPerToken(collectionId, priceInWei);
     ...
     */
    function buyProposalBuilder(uint256 collectionId) private view 
        returns (
     ...
     */
    function currentHighestBid(uint256 collectionId) public view returns (uint256) {
     ...
     * @dev See {IPRTCLBaseGovernor-castVote}.
     */
    function castVote(uint256 proposalId, uint8 support, bytes memory signature) public returns (uint256) {
        require(_verifyAddressSigner(signature), "Invalid whitelist signature");
     ...
     */
    function _verifyAddressSigner(bytes memory signature) private 
    view returns (bool) {
        bytes32 messageHash = keccak256(abi.encodePacked(msg.sender));
        return whitelistSigner == messageHash.toEthSignedMessageHash().recover(signature);
    }
     ...
     */
    function setMinPrice(uint256 collectionId, address currencyAddress, uint256 priceInWei) external onlyOwner {
        _currencyMinBidPerCollection[currencyAddress][collectionId] = priceInWei;

        emit MinPriceUpdated(collectionId, currencyAddress, priceInWei);
    }
    ...
    /**
     * @notice Lets the potential buyer withdraw funds if bid failed
     */
    function withdraw(uint256 proposalId) external nonReentrant {
        ...
        require(state == ProposalState.Canceled || state == ProposalState.Defeated || state == ProposalState.Expired, "Invalid bid state for withdrawal");

        bid.withdrawn = true;

        _sendFunds(bid.buyer, bid.currency, bid.amountInWei);
    }

    /**
     * @notice Lets the holder redeem funds for a successful sale, in exchange for burning the tokens.
     */
    function redeem(uint256 proposalId) external nonReentrant {
        require(state(proposalId) == ProposalState.Succeeded, "Invalid bid state for redeeming");

        uint256 collectionId = proposalCollection(proposalId);

        // Check collection balance
        uint256 collectionBalance = coreTokenData.balanceOf(msg.sender, collectionId);
        require(collectionBalance > 0, "No tokens to redeem in collection");
        ...
        // Burn tokens
        coreTokenData.burn(msg.sender, collectionId);
        
        // Send funds (ETH or ERC20)
        Bid memory bid = _bids[proposalId];
        _sendFunds(msg.sender, bid.currency, collectionBalance * bid.proceedsPerTokenInWei);
    }
     ...
     * @param amountInWei uint256 amount of wei to be sent
     */
    function _sendFunds(address to, address currency, uint256 amountInWei) private {
        if (currency != address(0)) {
            ...
            (bool sent, ) = to.call{value: amountInWei}("");
