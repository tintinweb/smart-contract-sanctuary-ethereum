...
contract Raffle is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    uint256 private constant MIN_AMOUNT = 10000000000000000;
    ...
    function fulfillRandomWords(
        ...
        // uint256 winnerIndex = randomWords[0] % 2; // if % 2, the remainer will be limited to a range of 0-1
        uint256 winnerIndex = randomWords[0] % s_players.length;
        address payable winner = s_players[winnerIndex];
        s_winner = winner;

        // Once the winner is picked up , reset and clean up all the state even if it fails eventually,
        // Therefore the raffle can restart on a clean slate -
        // state is open , having a new timestamp to wait for passing the next interval, and all brand new players then comes in  .
        s_raffleState = RaffleState.OPEN;
        s_lastTimeStamp = block.timestamp;
