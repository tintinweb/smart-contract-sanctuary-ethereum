...
interface DeerClubExclusivePass {
    function getTokenInfo(uint _tokenId) external view returns (TokenInfo memory);
}
...
contract Yield is Ownable, ReentrancyGuard {
    ...
    constructor() {
        ratesGdc[0] = 10 * 1e18;        // 10 $LASM daily for standard rarity
        ratesGdc[1] = 20 * 1e18;        // 20 $LASM daily for rare rarity
        ratesGdc[2] = 40 * 1e18;        // 40 $LASM dailf for super rare rarity
    }
    ...
    function setGdcNFT(address _gdcNFT) external onlyOwner {
        require(_gdcNFT != address(0), "Wrong address");
        gdcNFT = _gdcNFT;
    }

    function setDceNFT(address _dceNFT) external onlyOwner {
    ...
    function setRateGdc(uint[3] memory _rates) external onlyOwner {
        ratesGdc = _rates;
    }

    function setRateDce(uint _rate) external onlyOwner {
        rateDce = _rate;
    }

    function getYieldRatesInGdcNFT() public view returns (uint[] memory) {
    ...
    function getRewardsOfTokenIdInGdc(uint _tokenId) public view returns(uint) {
        uint mintedTime = Gambdeers_Club(gdcNFT).getTokenInfo(_tokenId).mintedTime;
        uint period = (block.timestamp - mintedTime) / 1 days;
        uint rarity = Gambdeers_Club(gdcNFT).getRarity(_tokenId);
        uint[] memory rates = getYieldRatesInGdcNFT();

        return rates[rarity] * period - claimedRewardsInGdc[_tokenId];
    }

    function getTotalRewardsOfGdcNFT(address _account) public view returns(uint) {
    ...
    function claimRewardInGdcNFT() external nonReentrant {
        require(enableYield, "Yield is disable");
        uint rewards = getTotalRewardsOfGdcNFT(msg.sender);
        ...
        updateGdcNFT(msg.sender);

        emit Claimed(msg.sender, rewards);
    }

    function updateGdcNFT(address _account) internal {
        uint balance = ERC721Enumerable(gdcNFT).balanceOf(_account);
        uint tokenId;
        for (uint i=0; i<balance; i++) {
            tokenId = ERC721Enumerable(gdcNFT).tokenOfOwnerByIndex(_account, i);
    ...
    function getRewardsOfTokenIdInDce(uint _tokenId) public view returns(uint) {
        uint mintedTime = DeerClubExclusivePass(dceNFT).getTokenInfo(_tokenId).mintedTime;
        uint period = (block.timestamp - mintedTime) / 30 days;

        return rateDce * period - claimedRewardsInDce[_tokenId];
    }

    function getTotalRewardsOfDceNFT(address _account) public view returns(uint) {
    ...
    function claimRewardInDceNFT() external nonReentrant {
        require(enableYield, "Yield is disable");
        uint rewards = getTotalRewardsOfDceNFT(msg.sender);
        ...
        updateDceNFT(msg.sender);

        emit Claimed(msg.sender, rewards);
    }

    function updateDceNFT(address _account) internal {
        uint balance = ERC721Enumerable(dceNFT).balanceOf(_account);
        uint tokenId;
        for (uint i=0; i<balance; i++) {
            tokenId = ERC721Enumerable(dceNFT).tokenOfOwnerByIndex(_account, i);
