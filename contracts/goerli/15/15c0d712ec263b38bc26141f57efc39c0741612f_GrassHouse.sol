...
contract GrassHouse is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function increasePenaltyOf(address _user, uint256 _amount) external nonReentrant onlyLive onlyXAlpaca {
  ...
  /// @param _user The address to claim rewards for
  function claim(address _user, uint256 _penalty) external nonReentrant onlyLive onlyXAlpaca returns (uint256) {
    if (block.timestamp >= weekCursor) _checkpointTotalSupply();
    uint256 _lastTokenTimestamp = lastTokenTimestamp;
    ...
    if (_amount != 0) {
      uint256 _totalPenalty = penaltyOf[_user] + _penalty;
      lastTokenBalance = lastTokenBalance - _amount;
      if (_totalPenalty != 0) {
        // transfer penalty to treasury
        rewardToken.safeTransfer(treasury, _totalPenalty);
        penaltyOf[_user] = 0;
      }
      // transfer remaining back to _user
      rewardToken.safeTransfer(_user, _amount - _totalPenalty);
    }
