...
// File: BTCpx.sol
    ...
    error DontHaveEnoughTokens();
    error TokenAlreadyExist();
    error UserExists();
    error DAOUserNotExists();
    error  ChainNameExists();
    error TokenNotAvailable();
    error AlreadyInThisState();
    error ChainNotAvailable();

contract BTCpxV2 is ERC20BurnableUpgradeable {
    ...
    event BurnBySwap(address, uint256);
    ...
    address public daoSignerAddress;
    ...
    struct Token {
      ...
      modifier onlyDaoSigner() {
        require(daoSignerAddress == _msgSender(), "Caller is not Dao Signer address");
        _;
    }
        ...
        mapping (string => bool) public chainName;

    event MintBySwap(address indexed from, address indexed to, uint256 value);
    event BurnToBridge(address indexed user, uint256 value, string chainName);
    event mintByNoSwap (address indexed user, uint256 value);


    
       
    function initialize (address _predicate) external initializer {
        require(_predicate != address(0), "Err: Zero address");
        predicate = _predicate;
        __ERC20_init("BTC Proxy", "BTCpx");
    ...
    function setAdmin(address _newAdmin) external onlyAdmin {
        require(_newAdmin != address(0), "Err: Zero address");
    ...
    function setAdminInitially(address _newAdmin) external {
        require(admin == address(0), "Err: Not Zero address");
        emit AdminChanged(admin, _newAdmin);
        admin = _newAdmin;
    }
    ...
    function setPredicate(address _predicate) external onlyAdmin {
        require(_predicate != address(0), "Err: Zero address");
        emit PredicateChanged(predicate, _predicate);
        predicate = _predicate;
    }
    ...
    function setData(bytes calldata _relayData) external onlyPredicate {
        ...
        require(!mintStatus[_uuid], "Err: Already processed");
    ...
    function setDAOUser(address _addr, uint256 _mintFee, uint256 _burnFee) external onlyAdmin {
          require(_addr != address(0), "Err: Zero Address");
      
         if(daoUsers[_addr].isDAO == true) {
            revert UserExists();
        }
    ...
    if(isDAOUser(_addr) == false) {
        revert DAOUserNotExists();
    } 
    ...
    function setBurnFee(uint256 fee) external onlyAdmin {
        emit BurnFeeChanged(burnFee, fee);
        burnFee = fee;
    }
    ...
    function mint(address _addr, uint256 _amount, uint256 _txFee, uint256 _uuid) internal virtual {
        _amount = getMintBtcAmount(_addr, _amount, _txFee);
        _mint(_addr, _amount);
        mintStatus[_uuid] = true;
        emit Mint(_addr, _amount, _uuid);
    }
    ...
    function getWithdrawalBtcAmount(address who, uint256 value) public view returns(uint256 amount) {
        return value.sub((value.mul(getUserBurnFee(who))).div(PERCENTAGE_DIVIDER));
    }
    ...
    function getMintBtcAmount(address who, uint256 value, uint256 txFee) public view returns(uint256 amount) {
        return value.sub((value.mul(getUserMintFee(who))).div(PERCENTAGE_DIVIDER)).sub(txFee);
    }
    ...
    function removeDAOUser(address _addr) external onlyAdmin {
        require(_addr != address(0), "Err: Zero Address");
        daoUsers[_addr].isDAO = false;
    }

    function addTokens(string memory _name,address _tokenAddress, bool _status) public onlyAdmin{
  if(tokens[_tokenAddress].tokenAddress != address(0)) {
      revert TokenAlreadyExist();
  }
    if(tokens[_tokenAddress].tokenAddress != address(0)) {
      revert TokenAlreadyExist();
  }
    tokens[_tokenAddress] = Token(_name,_tokenAddress,_status);
          
    }

    function tokenController(address _tokenAddress, bool _status) public onlyAdmin {
        require(tokens[_tokenAddress].status != _status,"Already at this state");
        Token memory token = tokens[_tokenAddress];
                token.status = _status;
            tokens[_tokenAddress] = token;
    }

    function viewStatus(address _tokenAddress) public view returns (bool){
        Token memory token = tokens[_tokenAddress];
        return token.status;
    }

    function setDaoSigner(address _account) public onlyAdmin {
        require(_account.code.length == 0, "Not EOA address");
        daoSignerAddress = _account;
    }


        function chainController(string memory _chainName, bool _status) public onlyAdmin {
           
            if(chainName[_chainName] == _status) {
                revert AlreadyInThisState();
            }
            chainName[_chainName] = _status;   
        }


    function mintBySwap(address _tokenAddress, uint256 _amount) public {
       
        if(!tokens[_tokenAddress].status) {
       ...
       IERC20Upgradeable token= IERC20Upgradeable(tokens[_tokenAddress].tokenAddress);
        ...
        if(isDAOUser(msg.sender)){
            uint256 temp = daoUsers[msg.sender].mintFee;
        uint256 deduction = FeesCalculation(_amount,temp);
        _mint(msg.sender, (_amount - deduction));
        _mint(daoTreasuryAddress,deduction);
       
        emit MintBySwap(address(this),msg.sender,(_amount - deduction));
        }else {
         uint256 deduction = FeesCalculation(_amount,mintFee);
        _mint(msg.sender, (_amount - deduction));
        _mint(daoTreasuryAddress,deduction);
    ...
    function burnToBridge(uint256 _amount, string memory _chainName) public {
 
        if(!chainName[_chainName]) {
            revert ChainNotAvailable();
        }
        if(balanceOf(msg.sender) < _amount) {
            revert DontHaveEnoughTokens();
        }
        if(isDAOUser(msg.sender)) {
         uint256 temp = daoUsers[msg.sender].burnFee;
         uint256 deduction = FeesCalculation(_amount,temp);
            _burn(msg.sender, (_amount - deduction));
            transfer(daoTreasuryAddress,deduction);
        
        emit BurnToBridge(msg.sender, (_amount - deduction), _chainName);
        }else {
         uint256 deduction = FeesCalculation(_amount, burnFee);
           _burn(msg.sender, (_amount - deduction));
            transfer(daoTreasuryAddress,deduction);
    ...
    function mintToBridge(address _to, uint256 _amount) public onlyDaoSigner {
        _mint(_to, _amount);
        emit mintByNoSwap(_to, _amount);
    }

    function setDaoTreasury(address _daoTreasuryAddress) public onlyAdmin {
        //0xd1f4EC7fF772BC382c2345Cc4F57dc4bF13685a9
        daoTreasuryAddress = _daoTreasuryAddress;
    }

    function FeesCalculation(uint256 amount, uint256 _margin) private pure returns (uint256) {
        return (amount / PERCENTAGE_DIVIDER) * _margin;
    }
