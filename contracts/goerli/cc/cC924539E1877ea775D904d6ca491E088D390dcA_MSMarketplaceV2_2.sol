...
import { IMarketplaceV2 } from "../interfaces/marketplace/IMarketplaceV2.sol";
    ...
    IERC1155ReceiverUpgradeable
{
    ...
    uint64 private primaryMSCommunityFeeBps;

    /// @dev The % of primary sales fees collected to Monde Singulier Community.
    uint64 private secondaryMSCommunityFeeBps;
    ...
    function createListing(ListingParameters memory _params) external override {
        ...
        Listing memory newListing = Listing({
            ...
            auctionDrop: _params.auctionDrop,
            platformAuctionDropFeeBps: _params.platformAuctionDropFeeBps
        });
    ...
    function updateListing(
        ...
        listings[_listingId] = Listing({
            ...
            auctionDrop: targetListing.auctionDrop,
            platformAuctionDropFeeBps: targetListing.platformAuctionDropFeeBps
        });
    ...
    function payout(
        ...
        uint256 secondaryMSCommunityFees = (_totalPayoutAmount * secondaryMSCommunityFeeBps) / MAX_BPS;
        ...
        CurrencyTransferLib.transferCurrencyWithWrapper(
            ...
            _payer,
            _payee,
            _totalPayoutAmount - (platformFeeCut + royaltyCut + secondaryMSCommunityFees),
            _nativeTokenWrapper
        );
    }
    ...
    function getMSCommunityFeeInfo() external view returns (address, uint16, uint16) {
        return (MSCommunityFeeRecipient, uint16(primaryMSCommunityFeeBps), uint16(secondaryMSCommunityFeeBps));
    }
    ...
    /// @dev Lets a contract admin update platform fee recipient and bps.
    function setMSCommunityFeeInfo(address _MSCommunityFeeRecipient, uint256 _primaryMSCommunityFeeBps, uint256 _secondaryMSCommunityFeeBps)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_primaryMSCommunityFeeBps <= MAX_BPS && _secondaryMSCommunityFeeBps <= MAX_BPS, "bps <= 10000.");

        primaryMSCommunityFeeBps = uint64(_primaryMSCommunityFeeBps);
        secondaryMSCommunityFeeBps = uint64(_secondaryMSCommunityFeeBps);
        MSCommunityFeeRecipient = _MSCommunityFeeRecipient;

        emit MSCommunityFeeInfoUpdated(_MSCommunityFeeRecipient, _primaryMSCommunityFeeBps, _secondaryMSCommunityFeeBps);
    }
...
interface IMarketplaceV2 is IMSContract, IPlatformFee {
     ...
     *
     *  @param auctionDrop           Is an auction for Drop or no.
     *
     *  @param platformAuctionDropFeeBps Fees that marketplace takes for auction drop listing.
    **/
    struct ListingParameters {
        ...
        bool auctionDrop;
        uint256 platformAuctionDropFeeBps;
    }
...
interface IPlatformFee {
    ...
    /// @dev Emitted when fee on primary sales is updated.
    event MSCommunityFeeInfoUpdated(address indexed MSCommunityFeeRecipient, uint256 primaryMSCommunityFeeBps, uint256 secondaryMSCommunityFeeBps);
}
