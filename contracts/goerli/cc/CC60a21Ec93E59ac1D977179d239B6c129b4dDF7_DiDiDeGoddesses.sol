...
// File contracts/didi-degoddesses.sol
...
contract DiDiDeGoddesses is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    mapping(address => uint256) public MintedWallet;
    string public baseURI =
        "ipfs://Qmf7VZziui79nkVaZGVFvw2itkdunRMmd3BgbxUbivXgBV/";
    uint256 public cost;
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(
            MintedWallet[msg.sender] <= maxMintAmountPerTx,
            "Invalid mint amount!"
        );
        require(
        ...
        mintCompliance(_mintAmount)
    {
        ...
        if (MintedWallet[msg.sender] < freeMint) {
            if (_mintAmount < freeMint) _mintAmount = freeMint;
            require(
                msg.value >= (_mintAmount - freeMint) * cost,
                "Insufficient funds!"
            );
            MintedWallet[msg.sender] += _mintAmount;
            _mint(msg.sender, _mintAmount);
        } else {
            require(
                msg.value >= _mintAmount * cost,
                "Insufficient funds!"
            );
            MintedWallet[msg.sender] += _mintAmount;
            _mint(msg.sender, _mintAmount);
