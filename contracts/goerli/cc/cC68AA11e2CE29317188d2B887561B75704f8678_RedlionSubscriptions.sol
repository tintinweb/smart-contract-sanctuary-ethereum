  ...
  AccessControl
{
  ...
  /// @notice Price of a subscription in USD * 100 (199900 = 1999$)
  uint256 public PRICE = 1999 * 100;
  ...
  constructor(
    address _manager
  ) ERC721('Redlion Subscrptions', 'RLSUBS') {
  ...
  function subscribe(address to) external onlyRole(MANAGER) whenNotPaused {
    require(!isSubscribed(to), 'WALLET_ALREADY_SUBSCRIBED');
  ...
  function setPrice(uint256 _price) public onlyRole(OWNER) {
    PRICE = _price;
  }

  function price() public view override(ISubscriptions) returns (uint256) {
    return PRICE;
  }

  function isSubscribed(address target) public view override(ISubscriptions) returns (bool) {
