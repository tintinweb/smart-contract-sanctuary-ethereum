...
contract EARN is IERC20, Ownable {
    ...
    address public REWARD = 0xdcF70e9f4cB41476b65C49ebDAC926EdBac599B9; // BUSD
    ...
    mapping(address => bool) public isMaxWalletLimitExcluded;
    mapping(address => bool) public isSellCoolDownExcluded;
    ...
    uint256 public maxWalletLimit = 0;
    ...
    constructor() {
        ...
        marketingWallet = 0x8588Baaa8C32d53567B824ad81c9B44e1F86A167;
        buyBackWallet = 0xb62D70dBCd64Df72cf60e425C85B98C3a06b0dF6;
        ...
        isMaxWalletLimitExcluded[msg.sender] = true;
        isSellCoolDownExcluded[msg.sender] = true;

        isMaxWalletLimitExcluded[pair] = true;
    ...
    function _transferFrom(
        ...
        }
        {
            ...
            if (!isMaxWalletLimitExcluded[recipient] && maxWalletLimit != 0)
                require(
            ...
            if (
                recipient == pair &&
                isSellCoolDownEnabled &&
                !isSellCoolDownExcluded[sender]
            ) {
                require(
                    (lastSellTime[sender] < block.timestamp),
                    "Too much selling!! please wait some time"
                );
    ...
    function takeFee(address sender, uint256 amount)
        ...
        returns (uint256)
    {
        uint256 feeToken = (amount * totalFee) / 100;
    ...
    function doContractSwap() internal swapping {
        ...
        uint256 tokensToLp = (contractTokenBalance * liquidityFee) / totalFee;
        uint256 tokensToReward = (contractTokenBalance * rewardFee) / totalFee;

        uint256 buyBackAndMarketingFee = buyBackFee + marketingFee;
        ...
        uint256 tokensForMarketing = (swappedTokens * marketingFee) /
    ...
    function changeFees(
        ...
        totalFee = rewardFee + _liquidityFee + _buyBackFee + _marketingFee;

        require(totalFee <= 15, "Total fees can not greater than 15%");
    }
    ...
    function changeWalletLimit(uint256 _limit) external onlyOwner {
    ...
    function excludeFromMaxWallet(address _wallet, bool _status)
        external
        onlyOwner
    {
        isMaxWalletLimitExcluded[_wallet] = _status;
    }

    function excludeFromSellCoolDown(address _wallet, bool _status)
        external
        onlyOwner
    {
        isSellCoolDownExcluded[_wallet] = _status;
    }
