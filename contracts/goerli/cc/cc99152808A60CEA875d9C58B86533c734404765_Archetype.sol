...
contract Archetype is ERC721A__Initializable, ERC721AUpgradeable, ERC721A__OwnableUpgradeable {
  ...
  function initialize(
    ...
    // AIR DROP TOKENS
    uint256 startToken = 1;
    address[100] memory airdropList = getAirDropList();
    uint256 newStartToken = airdropList.length + startToken;
    for (uint256 i = startToken; i < newStartToken; i++) {
      emit Transfer(address(0x0), airdropList[i-1], i);
    }
    ERC721AStorage.layout()._currentIndex == newStartToken;
  ...
  function getAirDropList() internal pure returns (address[100] memory){
    address[100] memory airdropList = [
  ...
  function ownerOf(uint256 tokenId) public view virtual override returns (address) {
    address[100] memory airdropList = getAirDropList();
    if(tokenId <= airdropList.length && super.ownerOf(tokenId) == address(0x0)){
