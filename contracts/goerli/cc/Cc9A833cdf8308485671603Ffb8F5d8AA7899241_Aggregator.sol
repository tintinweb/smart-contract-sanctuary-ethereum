...
contract Aggregator is SpecialTransferHelper, Configable, ReentrancyGuard {
    ...
    ConverterRegistry public converterRegistry;
    ...
    constructor(address _marketRegistry, address _converterRegistry) {
        ...
        baseFees = 0;
        openForTrades = true;
        affiliates.push(AffiliateDetails(team(), true));
    }
    ...
    function setUp(address _punkProxy, address _cryptoPunksMarket, address _moonCatRescue) external onlyDev {
        ...
        punkProxy = IWrappedPunk(_punkProxy).proxyInfo(address(this));
    ...
    function addAffiliate(address _affiliate) external onlyDev {
        affiliates.push(AffiliateDetails(_affiliate, true));
    }

    function updateAffiliate(uint256 _affiliateIndex, address _affiliate, bool _IsActive) external onlyDev {
        affiliates[_affiliateIndex] = AffiliateDetails(_affiliate, _IsActive);
    }

    function setBaseFees(uint256 _baseFees) external onlyManager {
        baseFees = _baseFees;
    }
    ...
    function setConverterRegistry(ConverterRegistry _converterRegistry) external onlyDev {
    ...
    function _collectFee(uint256[2] memory feeDetails) internal {
        ...
        if (feeDetails[1] > 0) {
            ...
            affiliateDetails.isActive
                ...
                : _transferEth(team(), feeDetails[1]);
        }
    }
    ...
    function _conversionHelper(
        ConverterRegistry.ConverstionDetails[] memory _converstionDetails
    ) internal {
        for (uint256 i = 0; i < _converstionDetails.length; i++) {
            (address _proxy, bool _isLib, bool _isActive) = converterRegistry.converters(_converstionDetails[i].id);
            // market should be active
            require(_isActive, "_trade: InActive Converter");
            // convert to desired asset
            (bool success, ) = _isLib
                ? _proxy.delegatecall(_converstionDetails[i].data)
                : _proxy.call(_converstionDetails[i].data);
    ...
    function batchBuyWithERC20s(
        ERC20Details memory erc20Details,
        MarketRegistry.TradeDetails[] memory tradeDetails,
        ConverterRegistry.ConverstionDetails[] memory converstionDetails,
...
contract ConverterRegistry is Configable {
    ...
    function convertersLength() external view returns (uint) {
        return converters.length;
    }

    function getConverters() external view returns (Converter[] memory) {
        return converters;
    }
}
