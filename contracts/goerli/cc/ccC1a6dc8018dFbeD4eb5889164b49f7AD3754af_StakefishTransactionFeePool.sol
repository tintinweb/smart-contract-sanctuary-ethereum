    ...
    ReentrancyGuard
{
    ...
    // Upgradable contract.
    // @disableInitializer if true, the initializer will be disabled.
    //   When deployed as a proxy target implementation, the initializer should be disabled.
    //   For unit testing without proxies, we allow initialize() to be called.
    ...
    function initialize(address _operatorAddress, address _adminAddress, address _devAddress) initializer public {
    ...
    function updateComputationCache() internal {
        ...
        require(
            cache.totalUncollectedCommission + cache.totalUncollectedUserBalance + cache.totalUnsentUserRewards == address(this).balance,
            "Detected balance mismatch which should never happen"
        );
    ...
    function joinPool(
        ...
        // Emit events so our Oracle can keep track of a list of vaildators in the pool.
    ...
    function collectReward(address payable beneficiary, uint256 amountRequested) external nonReentrant {
        if (beneficiary == address(0)) {
            beneficiary = payable(msg.sender);
        }
        uint256 payoutAmount = _collectReward(msg.sender, beneficiary, amountRequested);
        emit ValidatorRewardCollected(msg.sender, beneficiary, payoutAmount);
    }

    /**
     * Admin Functions
     */
    function setCommissionRate(uint256 commissionRate) external nonReentrant adminOnly {
    ...
    function changeOperator(address _newOperator) external nonReentrant adminOnly {
        operatorAddress = _newOperator;
    }

    function emergencyWithdraw(address depositorAddress, uint256 maxAmount) external nonReentrant adminOnly {
        uint256 payoutAmount = _collectReward(depositorAddress, payable(msg.sender), maxAmount);
        emit EmergencyWithdraw(msg.sender, depositorAddress, payoutAmount);
    }
...
interface IStakefishTransactionFeePool {
    ...
    event ValidatorRewardCollected(address indexed depositorAddress, address beneficiary, uint256 rewardAmount);
    event EmergencyWithdraw(address caller, address indexed depositorAddress, uint256 amount);
    ...
    function emergencyWithdraw(address depositorAddr, uint256 amountRequested) external;
