...
contract UniswapV2TWAPFactory is Clone {
    ...
    OracleBeacon public beacon;

    event InstanceCreated(address _tenant, address _proxy);
    ...
    constructor(address bluePrint, address _owner) {
        owner = _owner;
        beacon = new OracleBeacon(bluePrint, _owner);
    }

    function createTWAPOracle(address tenant, address factory, address keeperAddress) external {
        ...
        instance[tenant] = true;
        tenants[tenant] = newTenant;
        tenantCounter.increment();

        emit InstanceCreated(tenant, address(proxy));
    }

    function getInstance(address _tenant) public view returns (BeaconProxy) {
        if (!instance[_tenant]) {
...
contract OracleBeacon is Ownable {
    ...
    constructor(address initBlueprint, address owner) {
