    ...
    ERC2981
{
    ...
    uint256 private FREE_RESERVED = 68;
    uint256 public TEAM_RESERVED = 150;
    uint256 private free_minted = 0;
    ...
    mapping(address => bool) private addressFreeMintClaim;

    bytes32 public PresaleMerkleRoot = 0x00;
    bytes32 public FreeMerkleRoot = 0x00;
    ...
    constructor(
        ...
        setFreeReserved(_freeReserved);
        setPresaleMerkleRoot(_presaleMerkleRoot);
        setFreeMerkleRoot(_freeMerkleRoot);
    ...
    function checkIfFreeMinted(
        address ad1
    ) public view onlyOwner returns (bool) {
        return addressFreeMintClaim[ad1];
    }
    ...
    function setFreeReserved(uint256 _freeReserved) public onlyOwner {
        FREE_RESERVED = _freeReserved;
    }
    ...
    /*  function freeMint(
        ...
        if (paused) revert ContractPausedError();
        if (!isWhitelistedForFreeMint(_merkleProof, _mintAmount))
            revert NotElegibleOrWrongAmountError();
        if (addressFreeMintClaim[msg.sender])
            revert AddressAlreadyClaimedError();

        addressFreeMintClaim[msg.sender] = true;
        free_minted += _mintAmount;
        _safeMint(msg.sender, _mintAmount);
    }*/
