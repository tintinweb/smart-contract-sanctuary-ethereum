    ...
    ReentrancyGuard
{
    ...
    function _buyToken(uint256 tokenId, address _payToken) public payable {
        address tokenOwner = ownerOf(tokenId);
        // get that token from all market items mapping and create a memory of it defined as (struct => MarketItem)
        MarketItem memory marketItem = idMarketItem[tokenId];
        ...
        // calc the seller fees
        uint256 sellerAmount = (marketItem.price - feeAmount) / 1 ether;

        // send token's worth of ethers to the owner
        if (_payToken == address(0)) {
            payable(tokenOwner).transfer(sellerAmount); // send the ETH to the seller
