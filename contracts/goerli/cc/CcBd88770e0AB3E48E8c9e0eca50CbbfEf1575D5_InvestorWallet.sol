...
/**
 * @title InvestorWallet
 *
 * Implements a vesting wallet that allows access to "chunks" of the vested amount in configurable
 * chunks.
 */
contract InvestorWallet is VestingWallet {

    /*
     * Stores the timestamps in UNIX seconds
     */
     ...
     *  contract
     * @param startTimestamp The timestamp in UNIX seconds at which the vesting starts
     * @param durationSeconds The total duration in seconds the vesting will take
     * @param checkpointTimestamps The vesting schedule in UNIX seconds. Chunks of the vested amount
     * will be available at each checkpoint. The size of a chunk is "total vested amount /
     *  checkpoints.length".
     *  The first checkpoint _must_ be the same as startTimestamp and the last checkpoint _must_ be
     *  the same as "startTimestamp + durationSeconds". The checkpoints _must_ be ordered ascending.
     */
    constructor(
        address beneficiaryAddress,
        uint64 startTimestamp,
        uint64 durationSeconds,
        uint64[] memory checkpointTimestamps
    ) VestingWallet(beneficiaryAddress, startTimestamp, durationSeconds) {
        _checkpoints = checkpointTimestamps;
        require(checkpointTimestamps[0] == startTimestamp, "First checkpoint must be equal to the start timestamp");
        require(checkpointTimestamps[checkpointTimestamps.length - 1] == startTimestamp + durationSeconds, "Last checkpoint must be equal to start time plus duration");
    ...
    function _vestingSchedule(uint256 totalAllocation, uint64 timestamp) override internal view returns (uint256) {
        if (timestamp < start()) {
            ...
            // does not matter.
            return (totalAllocation / _checkpoints.length) * currentCheckpoint;
        }
     }
}
