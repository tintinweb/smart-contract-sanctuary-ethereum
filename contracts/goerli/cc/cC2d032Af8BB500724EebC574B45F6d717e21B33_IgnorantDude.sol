...
contract IgnorantDude is ERC721Enumerable, Ownable, SalesActivation {

    uint64 public price = 0.01 ether;
    uint256 public max_sales_ignorant = 349;
    ...
    ) ERC721("IGNORANT DUDE", "IGNRT") SalesActivation(_publicSalesStartTime, _preSalesStartTime, _preSalesEndTime) {
        nftURI = _nftURI;
        claimFundAddress = _claimAddress;
    }

    function presale(uint256 _ignorantNumber) external payable isPresalesActive {
        uint256 supply = totalSupply();
        require(presaleList[msg.sender], "You are not on the Allowlist");
        require(presaleListClaimed[msg.sender] + _ignorantNumber <= presaleListMax, "Purchase exceeds max number");
        require(msg.value >= price * _ignorantNumber, "You don't send enough eth to purchase the ignorant dude");
        require(supply + _ignorantNumber <= max_sales_ignorant, "Exceeds maximum IGNORANT DUDE supply");
        require(tx.origin == msg.sender, "Contracts are not allowed to mint");
        
        for (uint i = 1; i < _ignorantNumber; i++) {
            presaleListClaimed[msg.sender] +=1;
            _safeMint(msg.sender, supply + i);
        }
        emit Presale(_ignorantNumber, msg.sender);
    }

    function mint(uint256 _ignorantNumber) external payable isPublicSalesActive{
        uint256 supply = totalSupply();
        require(supply + _ignorantNumber <= max_sales_ignorant, "Exceeds maximum ignorant dude supply");
        require(tx.origin == msg.sender, "Contracts are not allowed to mint");
        require(_ignorantNumber > 0, "You cannont mint 0 ignorant dude");
        require(_ignorantNumber <= 1, "You are not allowed to buy many ignorant dude at once");
        require(msg.value >= price * _ignorantNumber, "You don't send enough eth to purchase the ignorant dude");

        for (uint i = 1; i < _ignorantNumber; i++) {
            _safeMint(msg.sender, supply + i);
        }
        emit Mint(_ignorantNumber, msg.sender);
    }
    ...
    function removeFromPresaleList(address[] calldata _presaleList) external onlyOwner {
          for (uint256 i=0; i < _presaleList.length; i++){
            presaleList[_presaleList[i]] = false;
        }
    }
    ...
    function setTotalSales(uint256 _totalIgnorant) external onlyOwner {
        max_sales_ignorant = _totalIgnorant;
    }

    function setPresaleListMax(uint256 _maxPresaleList) external onlyOwner {
        presaleListMax = _maxPresaleList;
    }

    function setClaimFundAddress(address _newClaimFundAddress) external onlyOwner {
        claimFundAddress = _newClaimFundAddress;
    }
...
contract SalesActivation is Ownable {
    ...
    modifier isPresalesActive() {
        require(isPresalesActivated(), "Presales not started or ended");
        _;
    }
    ...
    function setPresaleTime(uint256 _startTime, uint256 _endTime) external onlyOwner {
        require(_endTime > _startTime, "End time should be later than strat time");
