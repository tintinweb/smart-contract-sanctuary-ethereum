...
contract GreenPepperDeployer is ERC721, ERC2981, Pausable, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
   ...
   // Base URI:
   string private baseMetadataURI;   // <<--------------------------Originally "private" - WHY??????
   ...
   uint96 public royaltyPercentage = 1000; // <<----- this means 10% !!!
   uint256 public ShenanigansCounter = 0;



   constructor() ERC721("GreenPeppers", "GPs") {   
      console.log("\n\n>In the 'ShenanigansMinter' constructor()!!!\n");

      // https://www.larvalabs.com/cryptopunks
      baseMetadataURI = "https://fractionalownership.io/PunkWidgets/JSON/widget";
      ...
      console.log("\n\n>Still in 'constructor()', 'defaultRoyalty.royaltyFraction' was just set to '500' - which should mean 5% !!! \n");
      // console.log("\n\n>Still in 'constructor()', 'defaultRoyalty.royaltyFraction' was just set to: %s", _defaultRoyaltyInfo.royaltyFraction);

   }   // END constructor



   // The next function - "contractURI()" - is used OpenSea.
   // It returns the URI of a JSON file which contains Metadata pertaining to the entire contract - INCLUDING ROYALTY Rate!
   ...
   // 
   //  ORIGINAL OPENSEA CODE FOR OVERRIDING THESE ERC721 FUNCTIONS:
   // 
   ...
   // FREE MINTING -  Cause this function is *NOT* "payable"!!!
   // Meaning no ETH is required to be sent to this function by the caller.
   ...
   function safeFREEMint(address to, string memory metadataURI, uint96 leRoyaltyAmount) public onlyOwner() {
      ...
      console.log("\n>div 1,000,000,000,000,000 --> 'leRoyaltyAmount' = %s", leRoyaltyAmount / 1000000000000000);
      ...
      // REMINDER: 
      // THE ERC2981 Contract's DEFAULT DENOMINATOR = 10,0000 (...I could change that if I wanted...)
      // SO passing in A NUMERATOR OF SAY 750 WILL GIVE US: 750 / 10,000 = 7.5%
      // 
      // REMINDER # 2: function _setTokenRoyalty(uint256 tokenId, address receiver, uint96 feeNumerator)
      //
      // -->If I somehow got here WITHOUT passing in ANY value, I need to just set 'leRoyaltyAmount' to a default value of 500:
      if(leRoyaltyAmount == 0) {
         _setTokenRoyalty(ShenanigansCounter, to, 500);
      }
      else {
         _setTokenRoyalty(ShenanigansCounter, to, leRoyaltyAmount / 10000000000000000);
      }

      // REMINDER: "royaltyInfo()" does NOT return the % of Royalty, it returns the actual AMOUNT of Royalty. In WEI.
      // "royaltyInfo()" --> returns(<RECEIVER> address, <RoyaltyAmount> uint256)
      // function royaltyInfo(uint256 _tokenId, uint256 _salePrice) public view virtual override returns (address, uint256)
      (, uint royaltyAmount) = royaltyInfo(ShenanigansCounter, 10 ether); // <<--------- <--------- THIS CODE IS NOT NECESSARY!!! It's just for VERIFICATION! NEED TO COMMENT OUT!
      console.log("\n\n>Back in 'safeFREEMint()' --> 'royaltyAmount' of Token # %s = %s", ShenanigansCounter, royaltyAmount);  // <<--------- <--------- THIS CODE IS NOT NECESSARY!!! It's just for VERIFICATION! NEED TO COMMENT OUT!
   ...
   //    if(payWithETH == true) {
   //       require(msg.value >= ShenaniganETHPrice, "'regularMint()' ERROR! Insufficient ETH funds!");
   //    }
   //    else {
   //       require(msg.value >= ShenaniganCIGsPrice, "'regularMint()' ERROR! Insufficient CIGs funds!");
   //    }

     
   //    _safeMint(msg.sender, ShenanigansCounter, "");
   //    ShenanigansCounter++;
   ...
   function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
      ...
      // if(tokenURIsDictionary[tokenId].length > 0) {
...
abstract contract OperatorFilterer {
   ...
   function _checkFilterOperator(address operator) internal view virtual {
      ...
      console.log("\n\n>In 'OperatorFilterer.sol'!!!!\n\n    >In the '_checkFilterOperator()' function!");
