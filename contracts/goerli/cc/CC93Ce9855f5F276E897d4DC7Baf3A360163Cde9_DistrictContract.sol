...
contract DistrictContract is ERC721A,  ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("DISTRICT 69", "DISTRICT") {}
    ...
    function mint(uint8 _amountOfDistrict, bytes32[] memory _proof) public payable mintIsOpen nonContract nonReentrant{
        ...
        if (saleState == Stage.DistrictList) {
            require(_districtListMerkleRoot != "", "Fish Claim merkle tree not set. This address is not allowed to mint");
    ...
    function _publicMint(uint8 _amountOfDistrict) internal mintIsOpen {
        ...
        require(getRedemptionsPublic() + _amountOfDistrict <= maxByWalletPerPublic, "Exceeded max available to purchase");
        incrementRedemptionsPublic(_amountOfDistrict);
        _safeMint(msg.sender, _amountOfDistrict);
    }

    function _districtListMint(uint8 _amountOfDistrict) internal mintIsOpen {
        require(saleState == Stage.DistrictList, "FishList mint is not open yet!");
        require(getRedemptionsFishList() + _amountOfDistrict <= maxByWalletPerfFishSale, "Exceeded max available to purchase");
