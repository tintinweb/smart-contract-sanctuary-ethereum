...
contract DefiContribute is Ownable {
    ...
    event NewDonee(string indexed cause, address indexed donee, uint256 indexed doneeId, string message);
    event EliminatedDonee(string indexed cause, uint256 indexed doneeId, address indexed donee);
    event Dontation(address indexed to, address indexed from, uint indexed amount);
    event RedFlag(string indexed name, uint256 indexed id, address wallet, address indexed admin);
    event ReadyToRemove(string indexed name, uint256 indexed id, address indexed wallet);
    event SummedToThePool(uint256 indexed amount);
    event Withdrawal(Donee indexed donee, uint256 indexed proceeds);
    ...
    function becomeDonee(string memory _name, string memory _message) public onlyAcceptedDonee() {
        idToDonee[doneesId] = Donee(_name, doneesId, 0, payable(msg.sender), _message);
        doneeToWithdrawalFreeze[doneesId] = block.timestamp + 4 weeks;
        emit NewDonee(_name, msg.sender, doneesId, _message);
        doneesId++;
    }
    ...
    function addRedFlag(uint256 _doneeId, address _wallet) public onlyAdmin() {
        if (_doneeId <= doneesId && idToDonee[_doneeId]. wallet == _wallet) {
            ...
            emit RedFlag(idToDonee[_doneeId].name,_doneeId, _wallet, msg.sender);
        } else {
            revert DefiContribute__NotDonee();
        }
    }
    ...
    function removeDonee(uint256 _doneeId, address payable _wallet) public onlyAdmin() doneeRemovalRequirements(_doneeId, _wallet) {
        string memory name = idToDonee[_doneeId].name;
        ...
        emit EliminatedDonee(name, _doneeId, wallet);
    }
    ...
    function donate(uint256 _doneeId, address payable _wallet) public payable {
        if ((idToDonee[_doneeId].wallet != 0x0000000000000000000000000000000000000000) && (idToDonee[_doneeId].wallet == _wallet)) {
            ...
            emit Dontation(_wallet, msg.sender, msg.value);
        } else {
            revert DefiContribute__NotTheDonee();
        }
    }
    ...
    function withdraw() public onlyDonee() {
        if (doneeToWithdrawalFreeze[addressToDoneeId[msg.sender]] <= block.timestamp) {
            ...
            emit Withdrawal(idToDonee[addressToDoneeId[msg.sender]], proceeds);
        } else {
