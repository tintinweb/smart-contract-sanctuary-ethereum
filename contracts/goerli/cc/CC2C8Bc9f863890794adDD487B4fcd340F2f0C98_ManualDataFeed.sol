...
import '../interfaces/bridges/IBridgeAdapter.sol';

import './peripherals/Governable.sol';

contract ManualDataFeed is Governable, IManualDataFeed {
  ...
  /*
    TODO: remove this 
    Note: Data receiver should have a registry, and we should bridge token0, token1 or the pool aswell so that it can map address => oracleSidechain 
    TODO: question: adapters should only be callable by this contract/job
    TODO: consideration: different bridges will have different {destination, origin} domain ids, which can lead to mistakes in the inputs by a distracted user
          for example: if bridging through connext, mainnet and polygon domains are 1, 30, respectively; 
                       if bridging through layer zero, mainnet and polygon domains are 15, 19, respectively
                       They can make the mistake of choosing connext as bridge adapter and using the {destination,origin} domains of layer zero
                       Do we care? I believe this would revert at some point due to invalid destination domains
  */

  /*
    TODO: question: the current approach saves us writing a bridgeRouter contract, but allows the user to input some data--albeit small--
                    is there a way to exploit this? Are the checks and whitelists enough to prevent shady behavior?

    Writing summarized natspec in case I end up changing this:
    @param _bridgeAdapter Address of the bridge adapter
    @param _dataReceiver Address of the receiving contract in the destination chain
  */
  ...
  function sendObservation(
    ...
    address _dataReceiver = receivers[keccak256(abi.encodePacked(_bridgeAdapter, _destinationDomainId))];
    if (_dataReceiver == address(0)) revert ReceiverNotSet();

    uint32 _blockTimestamp = uint32(block.timestamp);
    (, int24 _tick, , , , , ) = _pool.slot0();
    _bridgeAdapter.bridgeData(_dataReceiver, _destinationDomainId, _blockTimestamp, _tick);
  ...
  function whitelistAdapters(IBridgeAdapter[] calldata _bridgeAdapters, bool[] calldata _isWhitelisted) external onlyGovernance {
    ...
    uint256 i;
    unchecked {
      for (i; i < _bridgeAdapterLength; ++i) {
        whitelistedAdapters[_bridgeAdapters[i]] = _isWhitelisted[i];
        emit AdapterWhitelisted(_bridgeAdapters[i], _isWhitelisted[i]);
      }
    }
  }

  /*
    Objective: I want to give as little room to the users in charge of bridging data to pass arbitrary data as possible.
              Ideally, the users would only pass _bridgeAdapter, _destinationDomainId, and _pool to sendObservation.
              To disallow any sort of potential bad behavior I'm adding permissioned _bridgeAdapters and forcing
              receivers based on the adapter they use and their destination domains, this way the data they have to pass must
              be valid.
              I don't think there's anything I can do with the pool, but that check should be further away, in the sidechain.
              The reason why I'm kecckaking _bridgeAdapter and _destinationDomainId instead of just doing a
              destinationDomain => receiver mapping, is because the possibility of two bridges having the same destinationDomain
              for different sidechains can happen--although it's improbable
              Also, I need to test this but I believe it's cheaper to do:

                  - bytes32 => address (keccak bridge+destinationDomain => receiver)

                      over

                  - address => uint32 => address (_bridgeAdapter => destinationDomain => receiver)

              Mostly because it should save an SSTORE and an SSLOAD at the cost of a SHA3 and some bit shifting OPCODES
              I can be wrong tho--will check
  */
  function setReceiver(
    ...
    // TODO: check if abi.encode is cheaper
    bytes32 _bridgeDestinationId = keccak256(abi.encodePacked(_bridgeAdapter, _destinationDomainId));
    receivers[_bridgeDestinationId] = _dataReceiver;
    emit ReceiverSet(_bridgeDestinationId, _bridgeAdapter, _destinationDomainId, _dataReceiver);
  }

  // TODO: check if this doesn't skip [0] due to ++i
  function setReceivers(
    ...
    uint256 i;
    bytes32 _bridgeDestinationId;
    unchecked {
      for (i; i < _bridgeAdapterLength; ++i) {
        _bridgeDestinationId = keccak256(abi.encodePacked(_bridgeAdapters[i], _destinationDomainIds[i]));
        receivers[_bridgeDestinationId] = _dataReceivers[i];
        emit ReceiverSet(_bridgeDestinationId, _bridgeAdapters[i], _destinationDomainIds[i], _dataReceivers[i]);
      }
    }
  }
}
...
import {IBridgeAdapter} from './bridges/IBridgeAdapter.sol';
import {IGovernable} from './peripherals/IGovernable.sol';
import '@uniswap/v3-core/contracts/interfaces/IUniswapV3Pool.sol';

interface IManualDataFeed is IGovernable {
  // STATE VARIABLES

  function whitelistedAdapters(IBridgeAdapter _bridgeAdapter) external view returns (bool _isWhitelisted);

  function receivers(bytes32 _bridgeDestinationId) external view returns (address _dataReceiver);

  // EVENTS

  event DataSent(IBridgeAdapter _bridgeAdapter, address _dataReceiver, uint32 _destinationDomainId, uint32 _blockTimestamp, int24 _tick);

  event AdapterWhitelisted(IBridgeAdapter _bridgeAdapter, bool _isWhitelisted);

  event ReceiverSet(bytes32 _bridgeDestinationId, IBridgeAdapter _bridgeAdapter, uint32 _destinationDomainId, address _dataReceiver);
