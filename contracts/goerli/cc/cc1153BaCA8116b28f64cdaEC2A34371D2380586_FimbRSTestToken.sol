...
contract FimbRSTestToken is ERC20Snapshot, Ownable {
    ...
    uint256 public constant TOTAL_TOKENS = 10 * 1 ether;
    uint256 public constant TOTAL_INSTALLMENTS = 3;
    uint256 public constant INSTALLMENTS_VALUE = 2.79 * 1 ether;
    uint256 public constant START_PAYMENT = 3348 * 1 ether;
    uint256 public constant DAILY_RATE = 0.0033 * 1 ether;

    address public constant OWNER = 0x17d6FF647015Eb09877d96c109A4AEe1f788dfeF;

    // Name of the token
    string public constant TOKEN_NAME = "FimbRSTestToken";
    // Symbol of the token
    string public constant TOKEN_SYMBOL = "FRSTT01";
    ...
    // A flag marking if the installments has started and if the startPayments was already called
    bool private bStartedInstallments;
    ...
    /**
     * @dev Owner function to start dividends payment period, pay start installment and froze token sale
     */
    function startInstallments() public onlyOwner {
        require(
            !bStartedInstallments,
            "The dividends payment period has already started"
        );
        ...
        require(
            START_PAYMENT <= nAllowance,
            "Not enough allowance to pay dividends"
        );

        // increase the total amount paid
        nTotalDividendsPaid = nTotalDividendsPaid.add(START_PAYMENT);

        // transfer the tokens from the sender to the contract
        dividendsToken.transferFrom(_msgSender(), address(this), START_PAYMENT);

        // snapshot the tokens at the moment the ether enters
        nCurrentSnapshotId = _snapshot();
        // Add ammount to payedAmmount
        paymentsMap[nCurrentSnapshotId] = START_PAYMENT;
        // save the date
        mapPaymentDate[nCurrentSnapshotId] = block.timestamp;

        if (nCurrentSnapshotId >= TOTAL_INSTALLMENTS) {
            bCompletedPayment = true;
        }

        bStartedInstallments = true;
    }
    ...
    function payDividends() public onlyOwner {
        require(
            bStartedInstallments,
            "The dividends payment period haven't started yet"
        );
        require(!bCompletedPayment, "Dividends payment is already completed");
        ...
        // make sure we are allowed to transfer the total payment value
        require(
            INSTALLMENTS_VALUE <= nAllowance,
            "Not enough allowance to pay dividends"
        );

        // increase the total amount paid
        nTotalDividendsPaid = nTotalDividendsPaid.add(INSTALLMENTS_VALUE);
        ...
        nCurrentSnapshotId = _snapshot();
        // Add ammount to payedAmmount
        paymentsMap[nCurrentSnapshotId] = INSTALLMENTS_VALUE;
        // save the date
        mapPaymentDate[nCurrentSnapshotId] = block.timestamp;
    ...
    /**
     * @dev Gets the value of start installment
     */
    function getStartInstallmentValue() public view returns (uint256) {
        return START_PAYMENT;
    }

    /**
     * @dev Gets the value of regular installment
     */
    function getInstallmentValue() public view returns (uint256) {
        return INSTALLMENTS_VALUE;
    }

    /**
     * @dev Gets the ammount of installments
     */
    function getInstallmentAmmount() public view returns (uint256) {
        return TOTAL_INSTALLMENTS;
    }

    /**
     * @dev Gets the value of daily rate interests
     */
    ...
    /**
     * @dev Gets if installments period is already started
     */
    function getStartedInstallments() public view returns (bool) {
        return bStartedInstallments;
    }
