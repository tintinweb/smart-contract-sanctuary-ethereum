...
contract TokenSale is Ownable, ReentrancyGuard {
    ...
    event SaleStarted(uint256 tokensPerWeiPublic, uint128 startTime, uint128 publicSaleEnd, uint128 tier2ClaimTime, uint128 tier3ClaimTime);
    ...
    uint256 public tokensPerWeiPublic;
    ...
    uint256 public constant MAX_RAISE = MAX_TIER2_CUMULATIVE_CONTRIBUTIONS + 138 ether;
     ...
     * @param   _tokensPerWeiPublic       Wei of DSQ to mint per wei of Ether contributed during public sale
     * @param   _startTime                Public sale start timestamp in Unix epoch seconds
     * @param   _publicEnd                Public sale end timestamp in Unix epoch seconds
     */
    function startSale(uint256 _tokensPerWeiPublic, uint128 _startTime, uint128 _publicEnd) external onlyOwner {
        require(startTime == 0, "Started");
        require(_publicEnd > _startTime && _startTime > block.timestamp, "Dates");

        tokensPerWeiPublic = _tokensPerWeiPublic;

        startTime = _startTime;
        publicSaleEnd = _publicEnd;
        tier2ClaimTime = _publicEnd + CLAIM_PERIOD;
        tier3ClaimTime = _publicEnd + 2 * CLAIM_PERIOD;

        emit SaleStarted(_tokensPerWeiPublic, _startTime, _publicEnd, tier2ClaimTime, tier3ClaimTime);
    }
    ...
    function purchasePublic() external payable nonReentrant returns (uint256) {
        ...
        require(block.timestamp < publicSaleEnd, "PublicSale Over");
        require(msg.value > 0, "Amount");
        // Cache
        uint256 _totalContribution = totalContribution;
        require(_totalContribution < MAX_RAISE, "SaleMax");
        uint256 _tokensPerWeiPublic = tokensPerWeiPublic;

        uint256 purchaseWeiAmount = (_totalContribution + msg.value > MAX_RAISE) ? MAX_RAISE - _totalContribution : msg.value;
        uint256 tokensToPurchase = purchaseWeiAmount * _tokensPerWeiPublic;
        ...
        tier1Pending[msg.sender] += (tier1Amount * _tokensPerWeiPublic);
        tier2Pending[msg.sender] += (tier2Amount * _tokensPerWeiPublic);
        tier3Pending[msg.sender] += (tier3Amount * _tokensPerWeiPublic);
    ...
    /**
     * @notice  Claim tokens purchased during the public sale
    ...
    function claim() external nonReentrant returns (uint256) {
        ...
        if (block.timestamp > tier3ClaimTime) {
            ...
            tier1Pending[msg.sender] = 0;
            tier2Pending[msg.sender] = 0;
        } else if (block.timestamp > publicSaleEnd) {
