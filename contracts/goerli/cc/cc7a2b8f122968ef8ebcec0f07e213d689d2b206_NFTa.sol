...
contract NFTa is INFT, AccessControlEnumerableUpgradeable, ERC4907AUpgradeable {
    ...
    function changeDirName(string memory dirName) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "role error");
        _dirName = dirName;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(AccessControlEnumerableUpgradeable, ERC4907AUpgradeable)
        returns (bool)
    {
    ...
    function issueNFT(uint16 _productId, uint256[] memory numPerClass) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "sender err");
        require(numPerClass.length == classNum, "cla num err");
        for (uint8 class = 1; class <= classNum; class++) {
            issueConf[_productId][class] = numPerClass[class - 1];
        }
    }
