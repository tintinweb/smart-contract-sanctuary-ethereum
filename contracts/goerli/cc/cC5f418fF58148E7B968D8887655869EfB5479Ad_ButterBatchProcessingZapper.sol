...
contract ButterBatchProcessingZapper {
  ...
  address public constant usdc = 0x705776924f4e7e96A5778c63413eADAC3F374314;
   ...
   * @param _amounts An array of amounts in stablecoins the user wants to deposit
   * @param _min_mint_amounts The min amount of 3CRV which should be minted by the curve three-pool (slippage control)
   * @param deadline The UNIX timestamp deadline of the permit signature
  ...
  function zapIntoBatchPermit(
    ...
    for (uint8 i; i < _amounts.length; i++) {
      if (_amounts[i] > 0) {
        //Deposit Stables
        if (i == 1) {
          // index of 1 currently represents usdc
          IERC20Permit(curve3Pool.coins(i)).permit(msg.sender, address(this), _amounts[i], deadline, v, r, s);
        }
        IERC20(curve3Pool.coins(i)).safeTransferFrom(msg.sender, address(this), _amounts[i]);
      }
    }
  ...
  function permitAndSpend(
    ...
    IUSDC(token).permit(msg.sender, address(this), amount, deadline, v, r, s);

    // IUSDC(curve3Pool.coins(1)).transferFrom(msg.sender, address(this), amount);
...
interface IUSDC is IERC20 {
  ...
  function makeDomainSeparator(string memory name, string memory version) external view;
