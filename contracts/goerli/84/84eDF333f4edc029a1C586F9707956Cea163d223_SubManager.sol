...
contract SubManager is ISubManager, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public platformFeeRate; // deprecated
    ...
    function createPlan(uint256[] memory uints, address[] memory addresses, string[] memory strings) external override whenNotPaused {
        // avoid stack depth limit
        {
            ...
            require(IPlatformFeeManager(platformFeeManager).validPrice(addresses[0], uints[2]), "price invalid");
        }
    ...
    function disablePlan(uint256 merchantTokenId, uint256 planIndex) external override whenNotPaused {
        IPlanManager.Plan memory plan = IPlanManager(planManager).getPlan(
            merchantTokenId,
            planIndex
        );
    ...
    function enablePlan(uint256 merchantTokenId, uint256 planIndex) external override whenNotPaused {
        IPlanManager.Plan memory plan = IPlanManager(planManager).getPlan(
            merchantTokenId,
            planIndex
        );
    ...
    // charge
    function charge(uint256 subscriptionTokenId) external override nonReentrant whenNotPaused {
    ...
    function  _charge(uint merchantTokenId, uint planIndex, uint subscriptionTokenId, address subOwner, address payeeAddress, address paymentToken, uint pricePerBillingPeriod, uint billingTime) internal {
        ...
        uint256 platformFee = IPlatformFeeManager(platformFeeManager).getFee(
            paymentToken,
    ...
    // cancel sub
    function cancelSubscription(uint256 subscriptionTokenId) external override whenNotPaused {
