...
contract ProjectHandler is BaseStructs, IProjectHandler, Ownable {
    ...
    function addProject(address _admin, uint256 _adminReward, uint256 _referralFee, uint256 _startBlock, IElvantisPoolCards _poolCards) override external payable onlyOwner {
    ...
    function setProject(uint256 projectId, address _admin, uint256 _adminReward, uint256 _referralFee, IElvantisPoolCards _poolCards) override external {
        ProjectInfo storage project = projectInfo[projectId];
    ...
    function pendingRewards(uint256 projectId, uint256 poolId, UserInfo calldata user, UserRewardInfo[] calldata userReward) onlyElvantisSpaceMoon override external view returns (uint256[] memory) {
        ProjectInfo storage project = projectInfo[projectId];
        PoolInfo storage pool = project.pools[poolId];
        ...
        for (uint256 i = 0; i < rewardInfo[projectId][poolId].length; i++) {
            ...
            if (block.number > _rewardInfo.lastRewardBlock && pool.totalShares != 0 && (_rewardInfo.supply > 0 || _rewardInfo.mintable)) {
                ...
                if(!_rewardInfo.mintable && _rewardInfo.supply < totalRewards) {
                    rewardAmount = (FEE_DENOMINATOR * _rewardInfo.supply) / (FEE_DENOMINATOR + project.adminReward + project.rewardFee);
...
interface IProjectHandler is BaseStructs {
    ...
    function addProject(address _admin, uint256 _adminReward, uint256 _referralFee, uint256 _startBlock, IElvantisPoolCards _poolCards) external payable;
    function setProject(uint256 projectId, address _admin, uint256 _adminReward, uint256 _referralFee, IElvantisPoolCards _poolCards) external;
