...
contract Land is ERC721A, Ownable {
    ...
    constructor(uint256 maxLands) ERC721A("N", "S") {
        MAX_LANDS = maxLands;
    }
        ...
        mustMatchPrice(MINT_PRICE, numLands)
    {
        ...
        if (elapsed <= 1800 && !bonusMarketingFeeSet && totalSupply() - mintStartTokenId + 1 >= 9) {
    ...
    function _payBonusMarketingFee(uint256 elapsed) internal {
        if (marketingWallet != address(0)) {
            ...
            if (_in1h && bonusMarketingFeeSet) {
                _percent = 8;
            } else if (_in1h || bonusMarketingFeeSet) {
                _percent = 4;
            }
            if (_percent > 0) {
