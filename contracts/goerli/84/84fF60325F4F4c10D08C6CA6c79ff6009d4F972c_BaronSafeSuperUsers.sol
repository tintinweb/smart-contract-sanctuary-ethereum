...
import "./lib/SafeSuperUsers.sol";
import "./lib/BaronBase.sol";

// This contract is used to manage the super users of the Baron treasury safe.
//
// These are the users who can execute transactions against the safe without
// requiring the ordinary threshold of confirmations from the multisig.
contract BaronSafeSuperUsers is BaronBase, SafeSuperUsers, ReentrancyGuard {

    // Admin Methods

    // This allows the treasury to specify the `safe` controlled by these super users.
    function setSafe(address payable safe) external onlyOperator {
        _setSafe(safe);
    }

    // This allows the treasury to add super users.
    function addSuperUser(address superUser) external onlyOperator {
        _addSuperUser(superUser);
    }

    // This allows the treasury to remove super users.
    function removeSuperUser(address superUser) external onlyOperator {
...
// This allows the designation of super users on a gnosis safe.
//
// Implementations should call _setSafe, _addSuperUser, and _removeSuperUser
// to configure the super users on the corresponding safe.
//
// Then the super can call execTransactionAsSuperUser to execute a transaction
...
abstract contract SafeSuperUsers {
    ...
    function execTransactionAsSuperUser(
        ...
        require(safe != address(0), "safe not configured");
        return IGnosisSafe(safe).execTransactionFromModule(to, value, data, operation);
    }

    /// The implementation should use these methods to configure the super users.

    // This should be called to set the safe that super users can control.
    ...
    // This should be called to add a super user.
    function _addSuperUser(address superUser) internal {
        require(
            !_superUsers.contains(superUser),
    ...
    // This should be called to remove a super user.
...
// This is a trimmed-down version of the GnosisSafe interface.
// It is used to avoid importing the entire GnosisSafe contract.
// It contains only the methods that are needed by SafeSuperUsers.
interface IGnosisSafe {
    // See safe-contracts/.../ModuleManager.sol
...
// See safe-contracts/.../common/Enum.sol
contract Enum {
