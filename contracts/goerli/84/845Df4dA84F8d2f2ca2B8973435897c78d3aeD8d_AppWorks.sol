...
contract AppWorks is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize() public initializer {
    ...
    // _blindTokenURI = "ipfs://QmXLvhmd98RkZgTtfUx1PpbChn11YUp1gKjqKdWyLsYDtH"; // Version 1
    // _blindTokenURI = "ipfs://QmUDpRuGwcJxZBFDiRUCBETq5Zb1uujuyh45mLiaD9UdSP"; // Version 2
  }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    // 因為第 10 週作業要改成更新盲盒圖片，所以這裡就不用 revealed 判斷了，直接寫死，反正會更新合約

    // 第一版的程式碼如下
    return _baseURI(); // 未打開盲盒，直接回傳網址（這時 baseURI 要指向一張圖片）

    // 更新版的程式碼如下
    // return string.concat(_baseURI(), "/", tokenId.toString(), ".jpg"); // 已打開盲盒，回傳 NFT 圖片網址（這時 baseURI 要指向一個資料夾位置）

    // 以下是正常用法，透過 revealed 控制盲盒是否開啟
    // if (revealed) {
    //   return string.concat(_baseURI(), "/", tokenId.toString(), ".jpg"); // 已打開盲盒，回傳 NFT 圖片網址（這時 baseURI 要指向一個資料夾位置）
    // } else {
    //   return _baseURI(); // 未打開盲盒，直接回傳網址（這時 baseURI 要指向一張圖片）
