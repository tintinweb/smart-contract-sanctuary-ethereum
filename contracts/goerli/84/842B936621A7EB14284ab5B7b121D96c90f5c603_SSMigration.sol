...
contract SSMigration is  ERC721Upgradeable, OwnableUpgradeable, DefaultOperatorFiltererUpgradeable {

    using Counters for Counters.Counter;
    
    address public SNContract;
    ...
    function initialize(string memory name, string memory symbol) initializer public {
        ...
        claimLive = false;
        SNContract = 0x3c613f941b6086212448c0276F7E3AA923Fe3c92;
    ...
    function mint(uint256[] memory tokenIds) external hasSN(msg.sender){
        require(claimLive, "Claiming is paused or not started");

        for(uint i = 0; i < tokenIds.length; ++i){

            require(IERC721(SNContract).ownerOf(tokenIds[i]) == msg.sender,"You don't own at least one of the tokens you input.");

            if(!_exists(tokenIds[i])){
                _mintInternal(msg.sender, tokenIds[i]);
                IERC721(SNContract).transferFrom(msg.sender,burnAddress,tokenIds[i]);
            }
        }
    }
