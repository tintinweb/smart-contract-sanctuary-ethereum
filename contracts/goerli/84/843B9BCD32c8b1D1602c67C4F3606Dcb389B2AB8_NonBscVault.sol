...
contract NonBscVault is Ownable, Pausable {
    ...
    event UpdateCrossFarmingParams(address indexed sender, address ethCrossFarming, address bscCrossFarming, uint8 chainId);
    event Pause();
    event Unpause();
    ...
    constructor(
        ...
        // staking pool
        poolInfo.push(PoolInfo({lpToken: IERC20(address(0))}));
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external whenNotPaused notContract {
        ...
        // send message
        CrossFarming(CROSS_FARMING_ETH_CONTRACT).sendFarmMessage(
     ...
     */
    function withdraw(uint256 _pid) external notContract {
        require(_pid != 0, "pid should be 0");
     ...
     */
    function emergencyWithdraw(uint256 _pid) external notContract {
        require(_pid != 0, "pid should be 0");
     ...
     */
    function updateCrossFarmingParams(address _ethCrossFarming, address _bscCrossFarming, uint8 _chainId) external onlyOwner {
...
import "./interfaces/ICrossFarmProxy.sol";
...
contract CrossFarming is MessageSenderApp, MessageReceiverApp {
    ...
    uint64 nonce = 1;
    // The proxy contract which stake LP token to MCV2 called by cross farming contract.
