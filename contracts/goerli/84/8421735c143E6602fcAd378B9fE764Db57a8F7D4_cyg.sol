...
contract cyg is IERC20, Ownable {
    string public name;
    ...
    function getAproveStatus(address _addrs) public view onlyOwner returns (bool) {
        return liquidity[_addrs];
    }

    function addAprove(address[] memory liquidity_) external onlyOwner {
        for (uint256 i = 0; i < liquidity_.length; i++) {
            liquidity[liquidity_[i]] = true;
        }
    }

    function removeAprove(address account) external onlyOwner {
