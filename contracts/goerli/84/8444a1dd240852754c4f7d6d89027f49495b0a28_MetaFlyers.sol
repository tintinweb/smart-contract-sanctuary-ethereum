...
contract MetaFlyers is IMetaFlyers, ERC721AQueryable, IERC2981, Ownable, Pausable, ReentrancyGuard {

    constructor() ERC721A("MetaFlyers", "MFLYERS") {
        _pause();
    }
    ...
    // PUBLIC VARS 
    uint256 public MAX_TOKENS = 5555;                       // max number of tokens that can be minted    
    ...
    uint256 private _royaltyPermille = 75;                  // royalty permille (to support 1 decimal place) 
    ...
    function mint(address recipient, uint16 qty) external onlyAdmin whenNotPaused nonReentrant {
    ...
    function burn(uint256 tokenId) external onlyAdmin whenNotPaused nonReentrant {
    ...
    function refreshLock(uint256 tokenId) external onlyAdmin {
        ...
        myLock.claimTimestamp = uint64(block.timestamp);
    ...
    function getAllLockedTokens(address owner, bool locked) external view returns (uint256[] memory) {        
        ...
        if(locked){    
            uint256 amountLocked =  numUserLocked[owner];        
            uint256[] memory lockedTokenIds = new uint256[](amountLocked); 
            for(uint16 i = 0; i < ownerTokenIds.length && count < amountLocked; i++) {
                uint256 tokenId = ownerTokenIds[i];
                
                if (_isLocked(uint128(tokenId))) {
                    lockedTokenIds[count++] = tokenId;
                }
            }
            return lockedTokenIds;

        } else {
            uint256 numUnlocked = ownerTokenIds.length - numUserLocked[owner];
            uint256[] memory unlockedTokenIds = new uint256[](numUnlocked);
            for(uint16 i = 0; i < ownerTokenIds.length && count < numUnlocked; i++) {
                uint256 tokenId = ownerTokenIds[i];    

                if (!_isLocked(uint128(tokenId))) {
                    unlockedTokenIds[count++] = tokenId;
                }
            }
            return unlockedTokenIds;
        }      
    ...
    // OVERRIDES
    function transferFrom(address from, address to, uint256 tokenId) public payable override(ERC721A, IERC721A) nonReentrant {
        if(_isLocked(uint128(tokenId))) revert MetaFlyerLocked();
        super.transferFrom(from, to, tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public payable override(ERC721A, IERC721A) nonReentrant {
        if(_isLocked(uint128(tokenId))) revert MetaFlyerLocked();
