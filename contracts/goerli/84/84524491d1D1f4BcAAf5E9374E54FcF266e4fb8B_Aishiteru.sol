    ...
    DefaultOperatorFilterer
{
    ...
    uint256 public lovelistSupply = 5000;
        ...
        VRFV2WrapperConsumerBase(linkAddress_, wrapperAddress_)
    {
        ...
        setBaseLockedTokenURI(baseLockedTokenURI_);
    ...
    /* MINT */
    function lovelistMint(
        uint256 quantity_,
        bytes calldata signature_
    ) external payable isSaleState(SaleState.Lovelist) {
        if (!verifySignature(signature_, "Lovelist")) revert WrongSignature();
        if (_totalMinted() + quantity_ > lovelistSupply)
            revert LovelistSoldOut();
        if (quantity_ > lovelistLimit) revert Exceeded();
        if (_numberMinted(msg.sender) + quantity_ > lovelistLimit)
            revert LimitExceed();
        if (_numberMinted(msg.sender) >= lovelistFreeLimit) {
            ...
            if (lovelistFreeLimit >= _numberMinted(msg.sender) + quantity_) {
                ...
                if (
                    msg.value !=
                    ((_numberMinted(msg.sender) + quantity_) -
                        lovelistFreeLimit) *
                        lovelistMintPrice
                ) revert WrongPrice();
            }
    ...
    function commonTouch(uint256[] memory tokenIds_) external payable {
        ...
        if (msg.value != touchPrice * tokenIds_.length)
            revert TouchNotAllowed();
    ...
    function goldenTouch(uint256[] memory tokenIds_) external payable {
        // Validate touch price
        if (msg.value != goldenTouchPrice * tokenIds_.length)
            revert TouchNotAllowed();
    ...
    function secretMint(
        uint256[] memory tokenIds_,
        uint256 minimumLoveScore_
    ) public payable {
        if (!secretOpen) revert NotOpenYet();

        // BURN
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            if (loveScore[tokenIds_[i]] < minimumLoveScore_)
                revert NotAllowed();
            _burn(tokenIds_[i]);
        }
