...
contract AstroStakingControllerV1 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
  ...
  // Override duration of one day to speedup testing. 
  // REMOVE ON PRODUCTION
  uint256 public durationOfOneDay;
  ...
  function setDurationOfOneDay(uint256 duration) external onlyOwner {
  ...
  function _unstake(uint256 tokenId) internal returns (uint256 amount) {
    ...
    uint256 numberOfDays = (block.timestamp - stakedToken.timestamp) / durationOfOneDay;
    ...
    unchecked {
      for (uint256 i = 0; i < tokenIds.length; i++) {
        ...
        if (numberOfDays > 0) {
          ...
          stakedToken.timestamp += uint32(numberOfDays * durationOfOneDay);
        }
      }
  ...
  function rent(
    ...
    uint32 expiration = uint32(block.timestamp + duration * durationOfOneDay);
