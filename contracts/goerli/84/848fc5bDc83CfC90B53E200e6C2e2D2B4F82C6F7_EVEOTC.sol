...
contract EVEOTC is EVEOTCTokens {
    ...
    function addOffer(bool _smart_trade, address _from, address[] memory _to, uint256 _available, uint256 _price, uint8 _discount_or_premium, uint256 _time) external {
        ...
        if (_smart_trade) {
            ...
            smart_trades.push(Offer(_smart_trade, msg.sender, _from, _to, _available, 0, _surplus, 0, _price, _discount_or_premium, _time, _commission));
        ...
        // process option trades
            ...
            // add the offer to the record list
            option_trades.push(Offer(_smart_trade, msg.sender, _from, _to, _available, 0, _surplus, 0, _price, _discount_or_premium, _time, _commission));
     ...
     *
     */
    function cancelSmartTrade(uint256 _index) external nonReentrant {
    ...
    function removeSmartTradeActive(uint256 _index) internal {
        for (uint256 i = 0; i < smart_trades_active.length; i++) {
            if (smart_trades_active[i] == _index) {
                smart_trades_active[_index] = smart_trades_active[smart_trades_active.length - 1];
    ...
    function buySmartTrade(uint256 _index, address _to, uint256 _amount) external nonReentrant {
        ...
        if (smart_trades[_index].filled == smart_trades[_index].available) {
            removeSmartTradeActive(_index);
        }
