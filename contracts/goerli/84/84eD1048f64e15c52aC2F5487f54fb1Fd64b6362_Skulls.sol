...
contract Skulls is ERC721Enumerable, Ownable {
    ...
    ISkullDescriptor public descriptor;
    ...
    constructor(
        ...
        uint256 _maxFree,
        ISkullDescriptor newDescriptor
        ...
        mintCompliance(_quantity)
    {
        ...
        uint256 userMinted = balanceOf(msg.sender);
        uint256 nextTokenId = totalSupply();

        if (nextTokenId + _quantity < FREE) {
            if (userMinted == 0) {
        ...
        if (nextTokenId + _quantity <= collectionSize) {

            for (uint32 i; i < _quantity;) {
    ...
    // Set descriptor
    function setDescriptor(ISkullDescriptor newDescriptor) external onlyOwner {
        descriptor = newDescriptor;
    }
    ...
    function getSeed(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Skull does not exist.");
    ...
    function generateSeed(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 headSeed = (r % 9) + 1;
        uint256 eyeSeed = ((r / 9) % 6) + 1;
        uint256 bodySeed = ((r / (9 * 6)) % 7) + 1;
        return uint256(keccak256(abi.encodePacked(headSeed, eyeSeed, bodySeed))) % (10 ** 18);
    }
