...
contract Cells is ERC721, Ownable, ERC2981 {
    ...
    function generateHTMLandSVG(address _address) internal pure returns (string memory finalHtml, string memory finalSvg) {
        ...
        string memory HTMLfirstBlock = '<canvas id="golCanvas"></canvas><style>* {margin:0;}body { background: rgb(37,38,30);height: 100%;}</style> <script>var canvas = document.getElementById("golCanvas");canvas.width = 700;canvas.height = 700;var WIDTH = canvas.width;var HEIGHT = canvas.height;var ctx = canvas.getContext("2d");var LEN = 10;var x = Math.floor(WIDTH/LEN);var y = HEIGHT/LEN;var myGol;var golTmp;function initTmp(){for(var xVal = 0; xVal<=x+2;xVal++){golTmp[xVal] = new Array();for(var yVal = 0; yVal<=y+2; yVal++){golTmp[xVal][yVal] = 0;}}}function initMatrix(){myGol = new Array();golTmp = new Array();for(var xVal = 0; xVal<=x+2;xVal++){myGol[xVal] = new Array();golTmp[xVal] = new Array();for(var yVal = 0; yVal<=y+2; yVal++){golTmp[xVal][yVal] = 0;var randVal = Math.floor((Math.random()*2));myGol[xVal][yVal] = randVal;if (randVal == 1){draw(xVal+1,yVal+1);}}}}function draw(x,y){ctx.fillRect(LEN*(x-1),LEN*(y-1),LEN,LEN);}function nextStep(){initTmp();ctx.fillStyle = "#';
    ...
    function mint(uint8 quantity) public payable {
        ...
        for (uint8 i = 0; i < quantity; i++) {
            numMinted += 1;
            _setTokenRoyalty(numMinted, owner(), 10000);
