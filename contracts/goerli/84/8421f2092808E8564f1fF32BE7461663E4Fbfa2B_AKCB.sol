...
contract AKCB is ERC721, DefaultOperatorFilterer, Ownable {
	...
	uint32 private constant TOTAL_MALES = 450;
	uint32 private constant TOTAL_FEMALES = 50;
	...
	constructor() ERC721("a KID called BEAST", "AKCB") {
		setBaseURI("ipfs://QmcbJXK2SvbJyide1Sr2XXLNr2P76FHybXf7bSeriR4WX1/");
   ...
   function getInformations() external view returns (string memory,uint256[(MAX_TOKENS/MAX_EACH_TYPE) + 1] memory,uint256[(MAX_TOKENS/MAX_EACH_TYPE) + 1] memory, uint, uint,uint,uint, bool,bool)
   {
		...
		return (CURR_ROUND_NAME, tokenIdsMale,tokenIdsFemale, CURR_ROUND_TIME,CURR_MINT_COST,nftPerAddressLimit, totalSupply(), hasSaleStarted, onlyWhitelisted);
   }
	...
	function setNewRound(uint cost, string memory name, uint32 perAddressLimit, uint theTime, bool isOnlyWhitelisted, bool saleState) external onlyOwner {
		CURR_MINT_COST = cost;
