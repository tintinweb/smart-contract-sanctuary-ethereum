...
// These trading rules help ensure a fair game for everyone.

contract MACPRO is Ownable, ERC20 {
    ...
    uint256 public minTimeBetweenSales = 180; // seconds - Minimum time between a buy and sell to avoid tax ( Don't sell within 180 sec otherwise you will get rekt )
    uint256 public maxTimeWithoutTax = 260; // seconds - Maximum time without tax even if there are less than 5 buys ( If you sell after 260 sec you don't get taxed )
    ...
    constructor() ERC20("MACPRO", "MACPRO") {
    ...
    function setRule(
        ...
        uint256 _GTA_Mode
    ) external onlyOwner { // Cleaning CD Mode
    ...
    function setTaxReceiver(address _taxReceiver) external onlyOwner {
        require(_taxReceiver != address(0), "New tax receiver is the zero address");
        taxReceiver = _taxReceiver;
    }

    function updateBlacklist(address _address, bool _value) external onlyOwner {
        blacklists[_address] = _value;
    }

    function updateWhitelist(address _address, bool _value) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (!whitelists[to] && !whitelists[from]) {
            if (from == uniswapV2Pair) { //buy action
            ...
            if (to == uniswapV2Pair && GTA_Mode > 0) { //sell action
                ...
                if (
                    (block.timestamp - lastBuyTime[from] < minTimeBetweenSales &&
                    globalBuyCount - buyCount[from] < minBuysBetweenSales) ||
                    block.timestamp - lastBuyTime[from] > maxTimeWithoutTax
                ) {
                    uint256 taxAmount = amount * GTA_Mode / 100;
                    amount -= taxAmount;
