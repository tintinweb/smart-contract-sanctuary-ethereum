...
contract NFTcontract is ERC721Enumerable, ReentrancyGuard {
    ...
    baseURIInfo[] private baseURIs;
    // mapping(uint256 => baseURIInfo) private baseURIs;
    ...
    function updateBaseURI(uint256 _numOfToken, string calldata _baseURI) external {
        baseURIInfo memory newBaseURI = baseURIInfo(_numOfToken, _baseURI);
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < baseURIs.length; i++) {
            if (tokenId < baseURIs[i].numOfToken) {
                return  string(
                        abi.encodePacked(baseURIs[i].baseURI, Strings.toString(tokenId), ".json")
                    );
            }
        }
        return "";
