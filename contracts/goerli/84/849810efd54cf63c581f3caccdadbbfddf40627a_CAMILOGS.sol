...
contract CAMILOGS is Ownable, ReentrancyGuard, ERC721A {
using Strings for uint256;

  uint256 public maxMintAmount = 4;
  bytes32 public AdminMerkleRoot;
  uint256 Price = 0.018 ether;
  uint256 maxSupply = 3000;
  bool public paused = true;
  bool public revealed = false;
  uint256 public constant ADMIN_PRICE = 0 ether;
  string private  baseTokenUri;
  string public   placeholderTokenUri;

    struct SaleConfig {uint256 Price; uint256 AmountForAdmin;}

  SaleConfig public saleConfig;

  constructor() ERC721A("LUXURY GSCAMILO V2", "LGSCV2") {
  }
  ...
  function AdminMint(uint256 quantity, bytes32[] calldata _merkleProof)  public payable callerIsUser
    {
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, AdminMerkleRoot, leaf), "Invalid proof!");
    require(!paused, "contract paused");    
    require(totalSupply() + quantity <= maxSupply, "reached max supply"); 
    require(numberMinted(msg.sender) + quantity <= saleConfig.AmountForAdmin, "can not mint this many");
      _safeMint(msg.sender, quantity);
      refundIfOver(ADMIN_PRICE);  }
  ...
  function setAdminMerkleRoot(bytes32 _merkleRoot) public onlyOwner {
    AdminMerkleRoot = _merkleRoot;
  }   
  ...
  function InitInfoOfSale(uint256 price, uint256 amountForAdmin) external onlyOwner {
    saleConfig = SaleConfig(price, amountForAdmin);}
...
function withdrawMoney() external onlyOwner {
    (bool success, ) = 0x07e8756416ACFac918661FF7A6e5bd1A5052C240.call{value: address(this).balance}("");
