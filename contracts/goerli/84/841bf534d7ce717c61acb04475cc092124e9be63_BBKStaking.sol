...
contract BBKStaking is IERC721Receiver, Ownable, ReentrancyGuard, Pausable {
    using EnumerableSet for EnumerableSet.UintSet;

    //contract address of the nft
    address public nftAddress;
    address public erc20Address;
    uint256 public rate = 1 * 1e18; //per day
    uint256 public legendaryRate = 5 * 1e18; //per day
    ...
    function stake(uint256[] calldata tokenIds) external whenNotPaused {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            stakerToTokenIds[msg.sender].add(curTokenId);
            tokenIdToStaker[curTokenId] = msg.sender;
            tokenIdToTimestamp[curTokenId] = block.timestamp;
        }
    }
        ...
        whenNotPaused
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            tokenIdToStaker[curTokenId] = address(0);
            tokenIdToTimestamp[curTokenId] = 0;
            stakerToTokenIds[msg.sender].remove(curTokenId);
    ...
    function claimRewards() external nonReentrant whenNotPaused {
        EnumerableSet.UintSet storage tokenIds = stakerToTokenIds[msg.sender];
        int256 reward = 0;
        for (uint256 i = 0; i < tokenIds.length(); i++) {
            uint256 curTokenId = tokenIds.at(i);
            uint256 stakedSince = tokenIdToTimestamp[curTokenId];
            tokenIdToTimestamp[curTokenId] = block.timestamp;
            uint256 timePassed = block.timestamp - stakedSince;
            uint256 _rate = isLegendaryToken[curTokenId] ? legendaryRate : rate;
            reward += int256(
    ...
    function depositsOf(address addr) external view returns (uint256[] memory) {
        return stakerToTokenIds[addr].values();
    }
    ...
    */
    function calculateRewards(address account) public view returns (uint256) {
        EnumerableSet.UintSet storage tokenIds = stakerToTokenIds[account];
        int256 reward = 0;
        for (uint256 i = 0; i < tokenIds.length(); i++) {
            uint256 curTokenId = tokenIds.at(i);
            uint256 stakedSince = tokenIdToTimestamp[curTokenId];
    ...
    function balanceOf(address addr) public view returns (uint256) {
        return stakerToTokenIds[addr].length();
    }
        ...
        returns (uint256)
    {
        ...
        uint256 stakedSince = tokenIdToTimestamp[tokenId];
        uint256 timePassed = block.timestamp - stakedSince;
        uint256 _rate = isLegendaryToken[tokenId] ? legendaryRate : rate;
