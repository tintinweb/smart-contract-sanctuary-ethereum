...
error SendingWrongValue();

contract CryptoGainsTest is ERC721Enumerable, Ownable, ReentrancyGuard {
    /**
    ...
    /// @notice can be implemented for two step ownership transfer
    // address private proposedOwner;

    /// @notice address to collect 10% fees
    ...
    /**
     * // FOR TESTING: CryptoGains,CG,ipfs/QmZdCwu88xKngFXMHS6x7RZXd2EvgyMSxyaFHor4F5Etm3/,0x39706EC255653733cB0B869A528f06be73B4DabD
     ...
     * @param _feeCollector 0xA9965198a513E20A3372997915649a187f2b3Edd
     */
    constructor(
        string memory _name,
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant {
        ...
        if (balanceOf(msg.sender) + _mintAmount > maxMintAmount)
            revert MintingWrongAmount();
        // add tx.origin != msg.sender => revert
        if (msg.sender != tx.origin) revert Unavailable();
        // check if it returns the right balance
        uint256 _supply = totalSupply();
        if (_supply + _mintAmount >= maxSupply) revert ExceedsMaxSupply();
        // where is owner set ?
        uint256 _fee = price * _mintAmount;
        if (msg.sender != owner()) {
            if (msg.value != _fee) revert SendingWrongValue();
        }
        ...
        // emit minted event
        emit Minted(msg.sender, price);
    }

    /// @notice withdraws contract's balance and sends 10% fee to Fee Collector Address
    function withdraw() external onlyOwner {
        // * before / to minimize potential of rounding errors
        uint256 _balanceFee = (address(this).balance * 10) / 100;
        // send fee to collector
        (bool successFee, ) = (feeCollector).call{value: _balanceFee}("");
        require(successFee);
    ...
    // function proposeChangeOwner(address _owner) external onlyOwner {
    //     proposedOwner = _owner;
    // }

    // function acceptChangeOwner() external {
    //     if (msg.sender != proposedOwner) revert Unavailable();
    //     owner = proposedOwner;
    // }
}