    ...
    Liquidity
{
    ...
    function createToken(
        ...
        address _token = address(
            new THERUGGAME(name, symbol, supply, decimal, address(this))
        );
        ...
        )
    {
        for (uint256 i = 0; i < activeTokens.length; i++) {
            if (
                ...
                IToken(activeTokens[i]).points() >= point
            ) {
                point = IToken(activeTokens[i]).points();
    ...
    function distributeRewardsAndRugLoser() external onlyOwner {
        ...
        require(
            winnerToken != address(0) && loserToken != address(0),
            "W/L address zero"
        );
        _distributeRewards();
        _rugLoser();
    ...
    function _distributeRewards() private {
        ...
        uint256 totalActiveReward;
        for (uint256 i = 0; i < activeTokens.length; i++) {
            if (activeTokens[i] != address(0)) {
                totalActiveReward += IToken(activeTokens[i]).wethReward();
            }
        }

        uint256 totalEliminatedReward;
        for (uint256 i = 0; i < eliminatedTokens.length; i++) {
            if (eliminatedTokens[i] != address(0)) {
                totalEliminatedReward += IToken(eliminatedTokens[i])
                    .wethReward();
            }
        }

        uint256 totalReward = winnerTotalRewards[winnerToken] -
            totalEliminatedReward;
        uint256 tokensToTransfer = totalActiveReward - totalReward;

        if (totalActiveReward > totalReward) {
            IERC20Upgradeable(WETH).transfer(winnerToken, tokensToTransfer);
            winnerTotalRewards[winnerToken] += tokensToTransfer;
        }
    }

    function _rugLoser() private {
        (address loserToken, , uint256 index) = getLoser();
        (, uint256 amountB) = removeLiquidity(loserToken, WETH, address(this));
    ...
    function changeDCult(address _dCult) external {
        cult = _dCult;
    }
...
interface IToken is IERC20Upgradeable {
    ...
    function wethReward() external view returns (uint256);
}
...
contract THERUGGAME is ERC20, Liquidity {
    ...
    function _transfer(
        ...
        if (
            ...
            // console.log("feesOnContract before", feesOnContract);

            if (from != pair && feesOnContract > 0) {
                uint256 swappedCult = _buyAndBurnToken(
                    IFactory(factory).cult(),
                    feesOnContract
                );
                uint256 swappedTrg = _buyAndBurnToken(
                    IFactory(factory).trg(),
                    feesOnContract * 2
                );
                // console.log("got back trg", swappedTrg);
        ...
        returns (uint256)
    {
        ...
        // console.log("buy and burned", ERC20(_tokenOut).name(), swappedAmount);
        ...
        returns (uint256 reward)
    {
        ...
        reward =
            ((IFactory(factory).winnerTotalRewards(address(this)) *
                userBalance) / totalSupply()) -
            rewardDebt[_user];
        // console.log(
        //     "reward debt and reward",
        //     rewardDebt[_user],
        //     IFactory(factory).winnerTotalRewards(address(this))
        // );
    }
    ...
    function claimReward() external {
        ...
        IERC20(IFactory(factory).WETH()).transfer(msg.sender, userReward);
        rewardDebt[msg.sender] += userReward;
        // console.log("rewardDebt[msg.sender]", rewardDebt[msg.sender]);
    }

    function bribe(address _token, uint256 _amount) public {
        address pair = IUniswapV2Factory(FACTORY).getPair(address(this), WETH);
        bool isRugged = IERC20(pair).balanceOf(factory) == 0;

        require(!isRugged, "Token eliminated");
        require(
            _token == IFactory(factory).cult() ||
                _token == IFactory(factory).trg(),
            "Only bribe with CULT or TRG"
        );

        IERC20(_token).transferFrom(msg.sender, address(this), _amount);

        uint256 amountToBurn = _amount / 2;
        uint256 amountForHolders = _amount - amountToBurn;

        IERC20(_token).transfer(deadAddress, amountToBurn);

        if (_token == IFactory(factory).trg()) {
            IERC20(_token).transfer(_token, amountForHolders);
        } else {
            IERC20(_token).transfer(
                IFactory(factory).dCult(),
                amountForHolders
            );
        }
