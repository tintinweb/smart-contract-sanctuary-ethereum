...
contract HeyMintERC721AExtensionA is HeyMintERC721AUpgradeable {
    ...
    function updateBaseConfig(
        ...
        require(
            _baseConfig.fundingDuration == state.cfg.fundingDuration,
            "CANNOT_UPDATE_CONSTANT_VARIABLE"
        );
        require(
    ...
    function updateAdvancedConfig(
        ...
        if (state.advCfg.refundDuration > 0) {
            ...
            require(
                _advancedConfig.refundDuration >= state.advCfg.refundDuration,
                "CANNOT_UPDATE_CONSTANT_VARIABLE"
            );
        } else if (
            _advancedConfig.refundDuration > 0 ||
...
struct AdvancedConfig {
    ...
    // The address authorized to call the crossmint mint function. If not set, defaultCrossmintAddress is used.
