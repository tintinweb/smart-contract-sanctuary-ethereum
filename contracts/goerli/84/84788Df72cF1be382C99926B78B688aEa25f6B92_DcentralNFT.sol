...
import "../extensions/IDcentralMarketplaceMint.sol";
...
contract DcentralNFT is
    ...
    OwnableUpgradeable,
    NFTContractMetadataUpgradeable,
    IDcentralMarketplaceMint
{
    bool private hasInitialized;
    address public mintAdmin;
    address public eventEmitterAddress;
    ...
    event Minted(uint256 indexed tokenId, string itemId);
    ...
    function initialize(
        ...
        IDcentralEvent eventEmitter = IDcentralEvent(
            address(eventEmitterAddress)
        );
        eventEmitter.setAllowedContracts(address(this));
        eventEmitter.emitNftContractInitialized(address(this));
    }
    ...
    function mint(
        ...
        IDcentralRoles dcentralRoles = IDcentralRoles(address(rolesAddress));
        require(
            mintAdmin == msg.sender ||
                dcentralRoles.isAdmin(msg.sender) == true,
            "Admin rights required"
        );
        _mintTo(to, creatorRoyaltyAddress, feeNumerator, uri, itemId);
    }
    ...
    function marketplaceMint(
        ...
        IDcentralRoles dcentralRoles = IDcentralRoles(address(rolesAddress));
        require(
            dcentralRoles.isApprovedMarketplace(msg.sender) == true,
            "Only approved Marketplaces can call this function"
        );
        uint256 newTokenId = _mintTo(
            to,
            creatorRoyaltyAddress,
            feeNumerator,
            uri,
            itemId
        );
    ...
    {
        IDcentralRoles dcentralRoles = IDcentralRoles(address(rolesAddress));
        if (dcentralRoles.isApprovedMarketplace(operator)) {
            return true;
        }
    ...
    function setTokenRoyalty(
        ...
        require(
            mintAdmin == msg.sender ||
                owner() == msg.sender ||
                dcentralRoles.isAdmin(msg.sender) == true,
            "Admin or owner rights required"
        );
    ...
    function setRoles(address _rolesAddress) public onlyOwner {
        rolesAddress = _rolesAddress;
    }

    function getTokenIdFromItemId(string memory itemId)
        public
        view
        returns (uint256)
    {
        return _itemIdToTokenId[itemId];
    }
...
abstract contract IDcentralEvent {
    ...
    function emitNftContractInitialized(address nft) external virtual;
...
abstract contract IDcentralMarketplaceMint {
    function marketplaceMint(
        address to,
        address creatorRoyaltyAddress,
        uint96 feeNumerator,
