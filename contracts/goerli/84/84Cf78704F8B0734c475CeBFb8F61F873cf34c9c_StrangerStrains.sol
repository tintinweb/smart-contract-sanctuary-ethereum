...
contract StrangerStrains is ERC1155, Ownable, ReentrancyGuard {
  ...
  function mint(uint _id, uint _amount) public payable {
    ...
    require(_amount > 0,  'Can not mint zero tokens!');
    require(_exists(_id), 'This strain does not exist yet, check back later!');
    require(_amount + strains[_id].supply <= strains[_id].maxSupply, 'Max supply for this strain would be exceeded!');
    require(msg.value >= strains[_id].mintPrice * _amount, 'Insufficient funds!');
  ...
  function mintBatch(uint[] memory _ids, uint[] memory _amounts) public payable {
    require(!paused, 'The contract is paused!');
    require(_ids.length == _amounts.length, 'id and amount arrays must be the same lenght!');
    ...
    for (uint i = 0; i < _ids.length; i++) {
      ...
      require(amount > 0,  'Can not mint zero tokens!');
      require(_exists(id), 'This strain does not exist yet, check back later!');
      require(amount + strains[id].supply <= strains[id].maxSupply, 'Max supply for this strain would be exceeded!');
      
      uint cost = strains[id].mintPrice * amount;
      batchPrice += cost;
    }
    
    require(msg.value >= batchPrice, 'Insufficient funds!');

    for (uint i = 0; i < _ids.length; i++) {
      uint id = _ids[i];
      uint amount = _amounts[i];

      strains[id].supply += amount;
    }
