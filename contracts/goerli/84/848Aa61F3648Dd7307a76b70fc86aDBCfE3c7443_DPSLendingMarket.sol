...
contract DPSLendingMarket is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    uint256 public maxDuration = 29 days;
    ...
    function borrowDPS(uint256 _tokenId) external nonReentrant {
        ...
        asset.endTime = uint64(block.timestamp) + asset.epochs * 1 days;
    ...
    function setMaxDurations(uint256 _newMaxDurations) external onlyOwner {
        emit MaxDurationChanged(maxDuration, _newMaxDurations);
        maxDuration = _newMaxDurations;
    }
    ...
    event MaxDurationChanged(uint256 _oldDuration, uint256 _newDuration);
