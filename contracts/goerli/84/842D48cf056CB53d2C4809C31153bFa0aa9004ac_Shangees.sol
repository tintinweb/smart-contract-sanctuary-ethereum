...
// contract Shangees is ERC721Royalty, Pausable, Ownable, ReentrancyGuard {
contract Shangees is ERC721, Pausable, Ownable, ReentrancyGuard {
// contract PKWidgetNFTMinter is ERC721, Pausable, Ownable, ReentrancyGuard {
   ...
   // MAPPING of GOLD Mint-Pass Holders:
   ...
   // string public baseMetadataURI; // <---- why not "public"????
...
//  ROYALTY FEES????????????????????????????????
   ...
   // constructor() ERC721Royalty("ShangeesNFTs!", "SHANG") {
   ...
   function safeFREEMint(address to, string memory metadataURI) public onlyOwner() {
      ...
      // So "800" should give us 8%:
      // if(ShenanigansCounter % 2 == 0) {
      //    _setTokenRoyalty(ShenanigansCounter, to, 500); // 5%
      // }
      // else {
      //    _setTokenRoyalty(ShenanigansCounter, to, 800); // 8%
      // }

      // // REMINDER: "royaltyInfo" *returns* (<RECEIVER> address, <RoyaltyAmount> uint256)
      // // function royaltyInfo(uint256 _tokenId, uint256 _salePrice) public view virtual override returns (address, uint256)
      // (, uint royaltyAmount) = royaltyInfo(ShenanigansCounter, 100);
      // console.log("\n\n-->ROYALTY was just set to: %s%", royaltyAmount);

      // console.log("\n-->Hey World!");
      console.log("\n>Just minted token # %s to address: %s%", ShenanigansCounter, to);

      ShenanigansCounter++;
      console.log("Incremented 'ShenanigansCounter'! Next Token that will be minted will have an ID of: %s", ShenanigansCounter);
