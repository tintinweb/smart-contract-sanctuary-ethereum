...
contract CardinalityJob is ICardinalityJob, Keep3rJob {
   ...
   */
  uint16 internal constant _MAX_CARDINALITY = 65_535;
   ...
   */
  constructor(IPoolManagerFactory _poolManagerFactory, uint16 _minCardinalityIncrease, address _governor)
    payable
  ...
  function work(IPoolManager _poolManager, uint16 _increaseAmount) external upkeep(msg.sender) notPaused {
    ...
    if (_increaseAmount < getMinCardinalityIncreaseForPool(_poolManager)) {
      revert CardinalityJob_MinCardinality();
    }
    if (!poolManagerFactory.isChild(_poolManager)) {
      revert CardinalityJob_InvalidPoolManager();
    }
