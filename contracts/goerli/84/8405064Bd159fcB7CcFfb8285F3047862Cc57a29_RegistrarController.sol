...
contract RegistrarController is Ownable {
    ...
    bytes4 constant private INTERFACE_China_ID = bytes4(keccak256("supportsInterface(bytes4)"));
    ...
    function registerWithConfig(string memory name, address owner, uint duration, bytes32 secret, address resolver, address addr) public payable {
        ...
        if(resolver != address(0)) {
            ...
            // Set the resolver
            base.Marscard().setResolver(nodehash, resolver);
    ...
    function withdrew() public onlyOwner {
        payable(msg.sender).transfer(address(this).balance);        
    }

    function supportsInterface(bytes4 interfaceID) external pure returns (bool) {
        return interfaceID == INTERFACE_China_ID ||
...
/*     constructor(MarsCard _Marscard, bytes32 _baseNode) ERC721("","") {
        Marscard = _Marscard;
        baseNode = _baseNode;
    } */

    function initialize(MarsCard _Marscard, bytes32 _baseNode) initializer public {
        __Ownable_init();
        __ERC721_init("", "");
        Marscard = _Marscard;
        baseNode = _baseNode;
    ...
    modifier live {
        require(Marscard.owner(baseNode) == address(this));
        _;
    }
    ...
    function setResolver(address resolver) external override onlyOwner {
        Marscard.setResolver(baseNode, resolver);
    }
    ...
    function _register(uint256 id, address owner, uint duration, bool updateRegistry) internal live onlyController returns(uint) {
        ...
        _mint(owner, id);
        if(updateRegistry) {
            Marscard.setSubnodeOwner(baseNode, bytes32(id), owner);
        }
    ...
    /**
     * @dev Reclaim ownership of a name in MarsCard, if you own it in the registrar.
     */
...
interface IABIResolver {
    ...
    /**
     * Returns the ABI associated with an MetaCard node.
     * Defined in EIP205.
     * @param node The MetaCard node to query
...
interface IAddrResolver {
    ...
    /**
     * Returns the address associated with an MetaCard node.
     * @param node The MetaCard node to query.
...
interface IContentHashResolver {
    ...
    /**
     * Returns the contenthash associated with an MetaCard node.
     * @param node The MetaCard node to query.
...
interface INameResolver {
    ...
    /**
     * Returns the name associated with an MetaCard node, for reverse records.
     * Defined in EIP181.
     * @param node The MetaCard node to query.
...
interface IPubkeyResolver {
    ...
    /**
     * Returns the SECP256k1 public key associated with an MetaCard node.
     * Defined in EIP 619.
     * @param node The MetaCard node to query
...
interface ITextResolver {
    ...
    /**
     * Returns the text data associated with an MetaCard node and key.
     * @param node The MetaCard node to query.
