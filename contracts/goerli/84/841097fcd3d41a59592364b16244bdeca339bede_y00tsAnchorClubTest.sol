...
contract y00tsAnchorClubTest is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint   public price = 0.0 ether;
	...
	function mint(uint256 count) external payable {
		...
		uint256[] memory ownedTokens = getYYCBelongingToOwner(msg.sender, count);
		for (uint256 i; i < count; ++i) {
			currentIndex = ownedTokens[i];
			_safeMint(msg.sender, 1);	 		
		}
	}

	function getYYCBelongingToOwner(address _owner, uint count) public view returns (uint256[] memory) {
		uint256 numYYCs = oldContract.balanceOf(_owner);
		require(count <= numYYCs, "Max mint count exceed");
