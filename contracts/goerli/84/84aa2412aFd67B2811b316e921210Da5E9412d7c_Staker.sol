...
contract Staker {
  ...
  function stake() public payable {
    require(block.timestamp <= deadline, "Staking has already been ended");
  ...
  function execute() external {
    require(!isOpenForWithdraw && !exampleExternalContract.completed(), "Already completed");

    if (block.timestamp <= deadline) {
      revert(string(abi.encodePacked(
        "deadline has not yet become: it's ", Strings.toString(deadline - block.timestamp), " seconds to wait"
      )));
    }
