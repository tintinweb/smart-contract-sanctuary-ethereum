...
contract LandReservePool is ERC20 {
    address public land;
    ...
    mapping(address => mapping(uint256 => bool)) public rewardsCollected;
    ...
    function claimReward(address user) public {
        ...
        uint256 mintable = (monthlyRewards[currentRewardIndex].reward *
            totalLandWeightOfUser) /
            (monthlyRewards[currentRewardIndex].totalWeight);

        _mint(user, mintable);

        rewardsCollected[user][currentRewardIndex] = true;
    }
    ...
    function isRewardActive() public view returns (bool) {
        return
            monthlyRewards[currentRewardIndex].snapshot > block.timestamp &&
