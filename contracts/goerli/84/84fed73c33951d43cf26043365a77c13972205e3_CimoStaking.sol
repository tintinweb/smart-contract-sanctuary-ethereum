...
contract CimoStaking is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    // Info of each user.
    struct UserInfo {
        uint16 boostPointsBP; //NFT token multiplier
    ...
    uint256 constant ONE_DAY = 60; //86400;
    // The Cimo token
    ICimo public Cimo;
    ...
    function addLiquidityNoFeeAndStake(uint256 amountTokensIn, uint256 amountETHMin, uint256 amountTokenMin, uint256 lockTime) public payable nonReentrant {
        ...
        (, uint256 ETHUsed, uint256 numLiquidityAdded) = _uniswapV2Router
    ...
    function _deposit(address sender, uint256 _pid, uint256 _amount, uint256 lockTime) internal {
        ...
        if (_amount > 0) {
            require((lockTime >= 0 && lockTime <= 90 * ONE_DAY && user.lockedUntil <= lockTime + block.timestamp), "deposit : Lock out of range or previously locked tokens are locked longer than new desired lock");
    ...
    function updateUserWeightedBalance(uint256 _pid, address _user) public {
        ...
        if (user.lockedUntil < block.timestamp) {
            user.weightedBalance = user.amount;
