    ...
    RevokableDefaultOperatorFilterer
{
    ...
    bool _checkSanctions = true;

    // 1155 Capsule contract
    IERC1155 private _src;

    // Decaying state
    bool private _decayEnabled = true;
     ...
     */
    function setAddTokenMessage(bool enableMessages) public onlyOwner {
     ...
     */
    function setTokenScore(uint256 tokenId, uint256 value) public onlyOwner {
    ...
    /**
     * @param tokenId[] array of tokenid to have score value changed.
     * @param value[] array of score values
     */
    function setBatchTokenScore(
        ...
        if (owner() == msg.sender || _scoreAdmins[msg.sender]) {
            require(tokenId.length == value.length, "Mismatched lengths");
            uint256 count = tokenId.length;
            unchecked {
                for (uint256 i = 0; i < count; ) {
                    if (!_exists(tokenId[i]))
                        revert URIQueryForNonexistentToken();
                    _score[tokenId[i]] = value[i];
                    i++;
                }
            }
        }
    ...
    function burnAndMint(uint256 id, uint256 value) external {
        ...
        uint256 newMax = totalSupply() + value;
        ...
        onlyOwner
    {
        ...
        unchecked {
            for (uint256 i = 0; i < count; ) {
                ...
                uint256 newMax = totalSupply() + value[i];
                require(_maxSupply >= newMax, "Max Supply limit reached");
    ...
    function _decayToken(
        ...
        if (_decayEnabled) {
            uint256 _from = uint256(keccak256(abi.encodePacked(from)));
            uint256 _to = uint256(keccak256(abi.encodePacked(to)));
            uint256 probability = random(_from, _to, tokenId);
            bool shouldDecay = probability < _decayProbability;

            if (shouldDecay == true) {
                uint256 decayAmount = 1;
                if (_decay[tokenId] == 0) {
                    //Initialize value
                    _decay[tokenId] = 10;
                } else if (
                    _decay[tokenId] == _decayLimit || _decay[tokenId] == 1
                ) {
                    decayAmount = 0;
                }
                _decay[tokenId] = _decay[tokenId] - decayAmount;
                ...
                abi.encodePacked(
                    block.timestamp +
                        ...
                        ((uint256(keccak256(abi.encodePacked(msg.sender)))) /
                            (block.timestamp)) +
                        block.number +
                        uint256(keccak256(abi.encodePacked(_src)))
                )
            )
        );

        return (seed + (to - from + tokenId)) % 100;
    }
    ...
    // OpenSea Registry Validated Transfers required due to modifier onlyAllowedOperatorApproval
