...
contract TheLegendOfSamuraiApes is ERC721A, Ownable, ReentrancyGuard {
    
    uint256 public MINT_PRICE = 0.002 ether;
    uint256 public MAX_SUPPLY = 444;
    uint256 public MAX_FREE_SUPPLY = 222;
    ...
    constructor(string memory initBaseURI) ERC721A("TheLegendOfSamuraiApes", "TLOSA") {
        baseURI = initBaseURI;
    }
    ...
    function freeMint(uint256 _mintAmount) public payable nonReentrant {
        ...
        require(
            ...
            "Max TheLegendOfSamuraiApes per address exceeded"
        );
        require(_mintAmount > 0, "Cannot mint 0 TheLegendOfSamuraiApes");
        require(
            s + _mintAmount <= MAX_FREE_SUPPLY,
            "Cannot exceed TheLegendOfSamuraiApes supply"
        );
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant {
        uint256 s = totalSupply();
        require(_mintAmount > 0, "Cant mint 0 TheLegendOfSamuraiApes");
        require(
            _mintAmount <= MAX_PUBLIC_PER_WALLET,
            "Cant mint more TheLegendOfSamuraiApes"
        );
        require(s + _mintAmount <= MAX_SUPPLY, "Cant exceed TheLegendOfSamuraiApes supply");
    ...
    function alphaMint(uint256 _number) external onlyOwner {
