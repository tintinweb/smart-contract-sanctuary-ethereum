...
contract DZooStaking is ReentrancyGuard, Ownable {
    ...
    uint256 public constant fractionalizationRate = 1e18;
    ...
    // amount of tokens staked in the contract
    ...
    function rewardsSupply() external view returns (uint256) {
        return nftsAssignedtoStake.sub(nftsMintedviaStake);
    }
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        require(reward >= fractionalizationRate, "Cannot claim partial NFT");
        uint256 remainder = reward.mod(fractionalizationRate); // whole "integer" of caller
        uint256 claimable = (reward.sub(remainder)).div(fractionalizationRate);
        rewards[msg.sender] = remainder; // update remainder fractional rewards
        rewardsToken.mint(msg.sender, claimable);
        nftsMintedviaStake = nftsMintedviaStake.add(claimable);
    ...
    function notifyRewardAmount(uint256 nfts)
        external
        onlyOwner
        updateReward(address(0))
    {
        require(nfts <= rewardsToken.MAX_SUPPLY(), "Invalid NFT reward amount");
        // since reward is passed down as whole NFT, we'll multiply it by fractionalizationRate
        uint256 reward = nfts.mul(fractionalizationRate);
        ...
        nftsAssignedtoStake = nftsAssignedtoStake.add(nfts);
    ...
    // Added to support recovering LP Rewards/ tokens from other systems
