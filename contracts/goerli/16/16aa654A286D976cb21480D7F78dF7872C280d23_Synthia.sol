...
contract Synthia is Ownable, ERC721Enumerable {
  ...
  string public baseURI = "ipfs://QmUSLA9BCJrR9uDsdBnMcBj67cM9adred4haZyNYyiCqHD/";
  ...
  function whitelistMint(
    ...
    // whitelist eligiblity
    if (block.timestamp > foundersSalesEnd) {
      require(block.timestamp >= whitelistSalesStart && block.timestamp < publicSalesStart, "Whitelist sales inactive");
    } else {
      // founders case (regular)
      require(block.timestamp >= foundersSalesStart && block.timestamp <= foundersSalesEnd, "Founders sales inactive");
    }
