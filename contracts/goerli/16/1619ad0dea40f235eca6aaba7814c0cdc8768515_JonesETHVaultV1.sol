...
contract JonesETHVaultV1 is Ownable {
    ...
    function depositETH(uint256 _amount) public payable {
        ...
        jonesETHToken.mint(msg.sender, _amount.mul(jonesEthPerEth()));
        emit Deposited(msg.sender, _amount.mul(jonesEthPerEth()), _amount);
    }

    /// Burn jETH and redeem ETH from the vault.
    /// @dev Assumes both tokens have same decimal places.
    /// @param _amount Amount of jETH to burn.
    function claimETH(uint256 _amount) public payable {
        ...
        require(
            address(this).balance >= _amount.div(jonesEthPerEth()),
            "Not enough ETH in the contract."
        );
        jonesETHToken.burnFrom(msg.sender, _amount);
        payable(msg.sender).transfer(_amount.div(jonesEthPerEth()));
        emit Claimed(msg.sender, _amount.div(jonesEthPerEth()), _amount);
    }

    /// Deposit fallback function.
    /// Also used by multisig to deposit ETH into the vault.
    receive() external payable {
        address beneficiary = msg.sender;
        require(beneficiary != address(0), "invalid address");
        require(msg.value > 0, "invalid amount");
        if (beneficiary == owner()) {
            snapshotVaultBalance = msg.value;
            emit SnapshotBalance(msg.value);
        } else {
    ...
    /// Calculates how much jETH one ETH can buy.
    /// e.g 100 jETH, 200 ETH, function returns 0.5 because 1 ETH = 0.5 jETH.
    /// @return jETH per ETH.
    function jonesEthPerEth() public view returns (uint256) {
        return snapshotJonesETHSupply.div(snapshotVaultBalance);
    }

    /// Calculates claimable ETH for the current users.
    ...
    function claimableEth(address _user) public view returns (uint256) {
        ...
        if (usrBalance > 0) {
            return usrBalance.div(jonesEthPerEth());
        }
        return 0;
    }

    /// Withdraws all ETH from the vault to the aumMultisigAddr multisig address.
    function withdrawToMultisig() public payable onlyOwner {
