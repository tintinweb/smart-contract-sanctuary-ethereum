...
contract GigaSpaceLandV3 is Initializable, ERC721Upgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, ERC721URIStorageUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function privateMintERC20(address to, uint256 size, int256 x, int256 y, string memory uri, uint256 eid, bytes memory signature) external nonReentrant payable callerIsUser {
        require(_phase == SalePhase.PrivateSale, "Private phase is not active");
        mintLandERC20(to, size, scaleXY(x), scaleXY(y), uri, eid, signature);
    }    

    function publicMintERC20(address to, uint256 size, int256 x, int256 y, string memory uri, uint256 eid, bytes memory signature) external nonReentrant payable callerIsUser {
        require(_phase == SalePhase.PublicSale, "Public phase is not active");
        mintLandERC20(to, size, scaleXY(x), scaleXY(y), uri, eid, signature);
    }    
    ...
    function mintLandERC20(address to, uint256 size, uint256 x, uint256 y, string memory landUri, uint256 eid, bytes memory signature) internal {
        ...
        //console.log("erc20Token: ", erc20Info.payERC20);
    ...
    function withdrawBalERC20(uint256 id) external payable onlyRole(DEFAULT_ADMIN_ROLE) {
