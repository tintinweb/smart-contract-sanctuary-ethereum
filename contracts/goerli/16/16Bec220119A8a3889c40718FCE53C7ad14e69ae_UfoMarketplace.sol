...
contract UfoMarketplace is BaseContract, Initializable, OwnableUpgradeable, PausableUpgradeable, IERC721Receiver {
    ...
    event AuctionCanceled(address indexed _nftAddress, uint256 indexed _tokenId, uint256 blockNumber);

    event FixedItemCreated(address indexed _nftAddress, uint256 indexed _tokenId, uint256 _price, address _seller, uint256 blockNumber);
    ...
    event FixedItemCanceled(address indexed _nftAddress, uint256 indexed _tokenId, uint256 blockNumber);
    ...
    function createFixedItem(
        ...
        emit FixedItemCreated(_nftAddress, _tokenId, _price, _seller, block.number);
    }
    ...
    function createAuctionItem(
        ...
        emit AuctionItemCreated(_nftAddress, _tokenId, _startingPrice, _endingPrice, _duration, _seller, block.number);
    }
    ...
    function buyFixedSellItemByNativeToken(address _nftAddress, uint256 _tokenId) external payable whenNotPaused {
        ...
        // _transfer(_nftAddress, address(this), msg.sender, _tokenId);
        emit FixedItemSuccessful(_nftAddress, _tokenId, msg.value, msg.sender, block.number);
    }
    ...
    function buyAuctionItemByNativeToken(address _nftAddress, uint256 _tokenId) external payable whenNotPaused {
        ...
        _transfer(_nftAddress, address(this), msg.sender, _tokenId);
        payable(_item.seller).transfer(actualPrice);
        emit AuctionSuccessful(_nftAddress, _tokenId, msg.value, msg.sender, block.number);
    }
    ...
    function _cancelAuction(
        ...
        emit AuctionCanceled(_nftAddress, _tokenId, block.number);
    }
    ...
    function _cancelFixedSell(
        ...
        _removeSellItem(_nftAddress, _tokenId);
        _transfer(_nftAddress, address(this), _seller, _tokenId);
        emit FixedItemCanceled(_nftAddress, _tokenId, block.number);
    }
