...
contract ShieldMining is IShieldMining, OwnableUpgradeable, ReentrancyGuard, AbstractDependant {
        ...
        onlyInjectorOrZero
    {
        ...
        _currentNetwork = _contractsRegistry.currentNetwork();
    }
    ...
    function earned(
        ...
        if (_userLeveragePool == address(0)) {
            ...
            userLiquidity = ILeveragePool(_userLeveragePool).userLiquidity(_account);
        ...
        uint256 newlyAccumulated = userLiquidity.mul(rewardsDifference).uncheckedDiv(DECIMALS18);
        ...
        returns (uint256)
    {
        ...
        _poolUtilizationRation = _coveragePool.totalCoverTokens().mul(PERCENTAGE_100).tryDiv(
            _coverageLiq
        );
        ...
        override
    {
        ...
        shieldMiningInfo[_policyBook].decimals = IERC20Metadata(_shieldMiningToken).decimals();
    ...
    function fillShieldMining(
        ...
        uint256 _blocksAmount = _duration.tryMul(getBlocksPerDay(_currentNetwork)).uncheckedSub(1);

        uint256 _rewardPerBlock = _amount.uncheckedDiv(_blocksAmount);
    ...
    function _getRewardFromLeverage(
        ...
        ILeveragePool userLeveragePool = ILeveragePool(_userLeveragePool);
    ...
    function recoverNonLockedRewardTokens(address _policyBook) public onlyOwner {
        ...
        if (tokenBalance > _futureRewardTokens) {
            uint256 nonLockedTokens = tokenBalance.uncheckedSub(_futureRewardTokens);
        ...
        )
    {
        ...
        _tokenPerDay = _rewardPerBlock.mul(getBlocksPerDay(_currentNetwork));
        ...
        returns (uint256 _lastBlockWithReward)
    {
        ...
        _lastBlockWithReward = _startingBlock.tryAdd(_blocksAmount);
        ...
        returns (uint256 _rewardPerBlock)
    {
        ...
        for (
            uint256 i = 0;
            i < lastBlockWithRewardList[_policyBook].length();
            i = uncheckedInc(i)
        ) {
