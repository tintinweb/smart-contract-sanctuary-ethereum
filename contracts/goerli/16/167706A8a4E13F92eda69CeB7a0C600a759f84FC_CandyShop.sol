...
contract CandyShop is Ownable, Pausable {
...
// Mint
    ...
    function mintWithEth(address _to) public payable whenNotPaused {
        require(minted[_to] == false, "Already minted.");
    ...
    function claimGift(address _to) public whenNotPaused {
        require(gifts[_to] && !minted[_to], "Not on the list.");
        minted[_to] = true;
        candies.mint(_to);
        emit SaleInETH(_to,priceETH);
    }
...
// Admin
    ...
    function gift(address _wallet) public onlyOwner {
        gifts[_wallet] = true;
    }

    function withdrawReg() public onlyOwner {
    ...
    function setCandiesAddress(address addr) onlyOwner public {
        candies = CandiesNFT(addr);
    }

    function setPriceInETH(uint _val) onlyOwner public {
        priceETH = _val;
    }

    function setPriceInREG(uint _val) onlyOwner public {
        priceREG = _val;
    }

    function setAvailableETH(uint _val) onlyOwner public {
        availableETH = _val;
    }

    function setAvailableREG(uint _val) onlyOwner public {
        availableREG = _val;
    }

    function setBeneficiary(address _addr) onlyOwner public {
        beneficiary = _addr;
    }

    function withdraw(address _addr) onlyOwner public {
