...
contract Asterix is ERC721Enumerable, Ownable {
    event SeedUpdated(uint256 indexed tokenId, uint256 seed);

    mapping(uint256 => uint256) internal seeds;
    IAsterixDescriptor public descriptor;
    uint256 public maxSupply = 10000;
    bool public minting = false;
    bool public canUpdateSeed = true;

    constructor(IAsterixDescriptor newDescriptor) ERC721("Asterix", "***") {
    ...
    function setDescriptor(IAsterixDescriptor newDescriptor) external onlyOwner {
        descriptor = newDescriptor;
    }
    ...
    function getSeed(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Asterix does not exist.");
    ...
    function generateSeed(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 row1 = 100 * (r % 7 + 10) + ((r >> 48) % 20 + 10);
        uint256 row2 = 100 * ((r >> 96) % 6 + 10) + ((r >> 96) % 20 + 10);
        uint256 row3 = 100 * ((r >> 144) % 7 + 10) + ((r >> 144) % 20 + 10);
        uint256 row4 = 100 * ((r >> 192) % 2 + 10) + ((r >> 192) % 20 + 10);
        return 10000 * (10000 * (10000 * row1 + row2) + row3) + row4;
    }
