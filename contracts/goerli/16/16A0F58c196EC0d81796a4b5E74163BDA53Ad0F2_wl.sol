...
contract wl is ERC721A {
    address[] public _preSaleWhitelist;

    constructor() ERC721A('wl', 'wl') {}

    function setPreSaleWhitelist(address[] calldata whitelistAddresses) public {
        for (uint256 i = 0; i < whitelistAddresses.length; i += 1) {
            _preSaleWhitelist.push(whitelistAddresses[i]);
        }
    }

function checkAddressEligibilityForMint(address minter) public {

    
        (bool preSaleEligible, uint256 preSaleQty) = _checkUserEligibilityInPresale(minter);
    ...
    function _checkUserEligibilityInPresale(address minter) internal view returns (bool, uint256) {
            ...
            if (isInWhitelist) {
                uint256 availableToMintAmount = 5;
