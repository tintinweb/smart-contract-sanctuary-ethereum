...
function start() external payable {
        require(isParticipant[msg.sender] = false);
        require (msg.value >= price, "You must deposit at least the price");
...
// random number between 0, 1 and 2
    ...
    function claimPrice() external payable OnlyWinner {
    ...
    function getDifficulty() public view returns (string memory) {
        ...
        else if (enemy[msg.sender].difficulty == 3) {
            return "Hard";
        }
        else {
            return "-";
