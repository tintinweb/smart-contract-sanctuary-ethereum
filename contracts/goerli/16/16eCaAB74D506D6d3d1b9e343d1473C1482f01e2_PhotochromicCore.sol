...
contract PhotochromicCore is Ownable {
    ...
    function setTicketValidity(uint256 _ticketValidity) public onlyOwner {
    ...
    function lastLiveness(string calldata userId) public view returns (uint32) {
        (uint32 livenessTime, ) = resolver.getValidityInfo(userId);
        return livenessTime;
    }
    ...
    event Ticket(address indexed user, string userId, string profile, uint256 timestamp);
    ...
    constructor(
        ...
        Profile[] memory _details
    ) {
        registrar = _registrar;
        resolver = _resolver;
        ens = _ens;
        photochromicSignerAddress = _sigAddr;
        profileNames = _profiles;
        for (uint i = 0; i < _profiles.length; i++) {
            profiles[_profiles[i]] = _details[i];
        }
    }
    ...
    function decodeDomainInfo(string memory e) public pure returns (DomainInfo memory) {
        ...
        // profile
        uint8 lengthInBytes = uint8(bs[4]);
        bytes memory profile = new bytes(lengthInBytes);
        for (uint i = 0; i < lengthInBytes; i++) {
            profile[i] = bs[5 + i];
        }
        domainInfo.profile = string(profile);

        // userId
        bytes memory userId = new bytes(bs.length - lengthInBytes - 5);
        for (uint256 i = lengthInBytes + 5; i < bs.length; i++) {
            userId[i - lengthInBytes - 5] = bs[i];
        }
     ...
     */
    function getPrice(string calldata profile) external view returns (uint256) {
        return profiles[profile].price;
    }

    /**
     * Returns the amount of socials allowed to mint.
     */
    function getSocialsAmount(string calldata profile) external view returns (uint8) {
        return profiles[profile].socials;
    }

    /**
     * Sets the profile prices (in USD).
     */
    function setPrices(string[] memory _profiles, Profile[] memory _details) external onlyOwner {
        require(_profiles.length == _details.length && _profiles.length != 0);
        profileNames = _profiles;
        for (uint i = 0; i < _profiles.length; i++) {
            profiles[_profiles[i]] = _details[i];
        }
    ...
    function purchase(string calldata userId, string calldata profile) external payable {
        require(bytes(userId).length > 0);
        // Accept both the current Chainlink round as the previous.
        uint256 price = profiles[profile].price;
        require(0 < price, "unknown profile");
        ...
        addressToDomainInfo[msg.sender] = encodeDomainInfo(currentTime, profile, userId);
        emit Ticket(msg.sender, userId, profile, currentTime);
    }
    ...
    /**
     * Returns the user id for which the sender has reserved a ticket.
     */
    ...
    function burn(string memory userId) external {
        bytes32 node = registrar.getNode(userId);
        ...
        registrar.burn(registrar.tokenOfOwnerByIndex(nodeOwner, 0));
        delete userIdToAddress[userId];
        delete addressToDomainInfo[nodeOwner];
    ...
    function getUserId() public view returns (string memory userid, bool minted) {
        ...
        return (myTicketUserId(), false);
    }

    /**
     * Returns the profile corresponding to the ticket of {msg.caller} address.
     */
    function getProfile() public view returns (string memory profile) {
        DomainInfo memory domainInfo = decodeDomainInfo(addressToDomainInfo[msg.sender]);
        return domainInfo.profile;
    }
    ...
    function mint(
        ...
        require(list.length <= profiles[domainInfo.profile].socials);

        // 3. Check validity of signature.
        ...
        uint256 tokenId = registrar.register(msg.sender, address(resolver), userId, ipfsHash);
        bytes32 node = bytes32(tokenId);
        resolver.setPCRecords(node, userId, contents, msg.sender);
        resolver.setValidityInfo(
            node,
            uint32(domainInfo.purchaseTime + 365 days),
            livenessTime
        );
        resolver.update(node, list);
