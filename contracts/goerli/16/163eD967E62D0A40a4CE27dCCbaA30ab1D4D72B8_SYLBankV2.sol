...
contract SYLBank is AccessControl, Initializable {
    bytes32 public constant DISTRIBUTOR_ROLE = keccak256("DISTRIBUTOR_ROLE");

    IERC20 public erc20;
    SYLVestingWallet public vestingWallet;
    ...
    function initialize(address erc20Address, address vestingWalletAddress) external initializer {
    ...
    function claim() external {
        ...
        uint256 treasuryBalance = erc20.balanceOf(address(treasury));
        ...
        if (treasuryBalance >= claimable) {
            ...
            erc20.transferFrom(treasury, msg.sender, claimables[msg.sender]);
        } else {
            // primarily transfer amount from treasury to msg.sender
            ...
            erc20.transfer(msg.sender, claimables[msg.sender] - treasuryBalance);
        }
        emit Claim(msg.sender, claimables[msg.sender]);
...
contract SYLBankV2 is SYLBank {

    function initializeV2(address erc20Address, address vestingWalletAddress) external reinitializer(2) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(DISTRIBUTOR_ROLE, msg.sender);
        erc20 = IERC20(erc20Address);
        vestingWallet = SYLVestingWallet(vestingWalletAddress);
     ...
     * @dev Swap function for exchanging SYLToken to karma point
     */
    function swapV2(uint256 amount) external {
        erc20.transferFrom(msg.sender, address(this), amount * 11 / 10);
        emit Swap(msg.sender, amount);
    }
}
