...
contract AaveAMO is Owned, IAsset, Asset {
    ...
    /*
        MainNet Addresses
            aaveLending_Pool: 0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9
            USDC: 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48
            aaveUSDC: 0xBcca60bB61934080951369a648Fb03DF4F96263C

        Goerli Addresses
            aaveLending_Pool: 0x4bd5643ac6f66a5237E18bfA7d47cF22f1c9F210
            USDC: 0x9FD21bE27A2B059a288229361E2fA632D8D2d074
            aaveUSDC: 0x935c0F6019b05C787573B5e6176681282A3f3E05
    */
    constructor(
        ...
        Asset(_sweep_address, _usdx_address, _amm_address)
    {
    ...
    /// @param sweep_amount Amount of asset to be diposited - E18
    /// @param usdx_amount Amount of asset to be diposited - E6
    function deposit(uint sweep_amount, uint256 usdx_amount) public onlyStabilizer {
        if(usdx_amount > 0) usdx.transferFrom(msg.sender, address(this), usdx_amount);
        sweep.transferFrom(msg.sender, address(this), sweep_amount);

        sell(sweep_amount);
        usdx_amount = usdx.balanceOf(address(this));
        aaveDepositUSDX(usdx_amount);

        emit Deposit(address(usdx), usdx_amount);
    }

    /// @notice Function to withdraw USDX from AMO to Stabilizer
    /// @param sweep_amount Amount of asset to be withdrawed - E18
    function withdraw(uint256 sweep_amount) public onlyStabilizer {
        uint usdx_amount = SWEEPinUSDX(sweep_amount, sweep.current_target_price());
        aaveWithdrawUSDX(usdx_amount);

        buy(sweep_amount);
        sweep.transfer(msg.sender, sweep_amount);

        emit Withdraw(address(sweep), sweep_amount);
    }

    /// @notice Function to deposit USDX to Aave pool
    /// @param amount Amount of asset to be diposited
    function aaveDepositUSDX(uint256 amount) private {
        usdx.approve(address(aaveLending_Pool), amount);
        aaveLending_Pool.deposit(address(usdx), amount, address(this), 0);
    }

    /// @notice Function to withdraw USDX from Aave pool
    /// @param aToken_amount Amount of asset to be withdrawed
    function aaveWithdrawUSDX(uint256 aToken_amount) private {
        aaveLending_Pool.withdraw(address(usdx), aToken_amount, address(this));
    }

    /// @notice Function to Recover Erc20 token to Stablizer
    /// @param tokenAddress token address to be recovered
    /// @param tokenAmount token amount to be recovered
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyStabilizer {
        TransferHelper.safeTransfer(address(tokenAddress), msg.sender, tokenAmount);
    }

    // compliance with the IAsset.sol
    function withdrawRewards(address) pure external {}
    function updateValue(uint) pure external {}
}
...
interface IAsset {
  ...
  function deposit(uint sweep_amount, uint usdx_amount) external;
...
contract Asset {
   ...
   */
  function buy(uint sweep_amount) internal {
  ...
  /** @notice Sell Sweep
    ...
    */
  function sell(uint sweep_amount) internal {
    if(sweep_amount == 0) { revert InputIsZero(); }
    if(sweep.balanceOf(address(this)) < sweep_amount) { revert NotEnoughBalance(); }

    uint sweep_price = sweep.current_target_price();
...
import {IERC20} from "../../Common/ERC20/IERC20.sol";
