...
// File: contracts/libs/MadWorldStruct.sol
...
library MadWorldStruct {
    ...
    struct Vault {
        ...
        Booster[] booster;
        uint256 amountUmaxValue;
        uint256 topUpUmaxValue;
        uint256 startStake;
...
// File: contracts/extensions/Signature.sol
...
library Signature {
    ...
    function getSignerForPayload(
        ...
        bytes32 hashMessage = keccak256(
            abi.encodePacked(
                ...
                mergedBooster,
                payload.amountUmaxValue,
                payload.topUpUmaxValue,
                payload.APR,
                payload.valueInUMax
            )
        );
    ...
    function getSignerForUnstakePayload(
        ...
        if (payload.unstakeType == MadWorldStruct.UnStakeType.NFT) {
            ...
            payload.unstakeType == MadWorldStruct.UnStakeType.UMAXVALUE
        ) {
            hashMessage = keccak256(
                abi.encodePacked(
                    payload.unstakeType,
...
// File: contracts/MadWorldStakingV2.sol
    ...
    AccessControlUpgradeable
{
    ...
    function stake(
        ...
        poolInfo.totalStaked =
            poolInfo.totalStaked -
            userInfo.valueInUMax +
            _payload.valueInUMax;
        ...
        // transfer
        {
            ...
            poolInfo.stakingToken.transferFrom(
                msg.sender,
                address(this),
                _payload.amountUmaxValue + _payload.topUpUmaxValue
            );
        }
        ...
        // push new vault
        pushToNewVault(_payload, msg.sender);

        //update user
        {
            userState[_payload.poolId][msg.sender].APR = _payload.APR;
            userState[_payload.poolId][msg.sender].valueInUMax = _payload
                .valueInUMax;
            userState[_payload.poolId][msg.sender].latestUpdate = block
                .timestamp;
            userState[_payload.poolId][msg.sender].indexVault =
                latestVaultIndexForUser[_payload.poolId][msg.sender] -
                1;
            userState[_payload.poolId][msg.sender].rewardWaitting = 0;
        }
    ...
    function pushToNewVault(
        ...
        vaultStorage[_payload.poolId][_user][latestVaultIndex]
            .amountUmaxValue = _payload.amountUmaxValue;
        vaultStorage[_payload.poolId][_user][latestVaultIndex]
            .topUpUmaxValue = _payload.topUpUmaxValue;
        vaultStorage[_payload.poolId][_user][latestVaultIndex]
    ...
    function unstake(
        ...
        (address signer, bytes32 hashMessage) = Signature
            .getSignerForUnstakePayload(_payload);
...

        {
            ...
            userState[_payload.poolId][msg.sender].valueInUMax = _payload
                .valueInUMax;
            userState[_payload.poolId][msg.sender]
                .rewardWaitting = getPendingRewardRealTime(
                _payload.poolId,
                msg.sender
            );
            userState[_payload.poolId][msg.sender].latestUpdate = block
    ...
    function _transferToUserAndDelete(
        ...
        if (payload.unstakeType == MadWorldStruct.UnStakeType.NFT) {
            if (payload.tokenIdForPrimary.length > 0) {
                for (
                    uint256 i = payload.tokenIdForPrimary.length - 1;
                    i >= 0;

                ) {
                    require(
                        payload.tokenIdForPrimary[i] ==
                            vaultInfo.tokenIdForPrimary[
                                payload.indexForPrimary[i]
                            ],
                        "Wrong token"
                    );
                    ...
                    vaultInfo.tokenIdForPrimary[
                        ...
                        vaultInfo.tokenIdForPrimary.length - 1
                    ];

                    // delete item
                    vaultInfo.tokenIdForPrimary.pop();
                    unchecked {
                        i--;
                    }
                }
            ...
            if (payload.indexForBooster.length > 0) {
                for (uint256 i = payload.indexForBooster.length - 1; i >= 0; ) {
                    require(
                        payload.booster[i].collectionBooster ==
                            vaultInfo
                                .booster[payload.indexForBooster[i]]
                                .collectionBooster &&
                            payload.booster[i].tokenId ==
                            vaultInfo
                                .booster[payload.indexForBooster[i]]
                    ...
                    vaultInfo
                        .booster[payload.indexForBooster[i]]
                        .collectionBooster = vaultInfo
                        .booster[vaultInfo.booster.length - 1]
                        .collectionBooster;
                    vaultInfo
                        .booster[payload.indexForBooster[i]]
                        .tokenId = vaultInfo
            ...
            require(
                payload.amountUmaxValue <= vaultInfo.amountUmaxValue,
                "Out of your bound"
            );
            ...
            vaultInfo.amountUmaxValue -= payload.amountUmaxValue;
        } else if (payload.unstakeType == MadWorldStruct.UnStakeType.TOPUP) {
            require(
                payload.topUpUmaxValue <= vaultInfo.topUpUmaxValue,
                "Out of your bound"
            );
            ...
            vaultInfo.topUpUmaxValue -= payload.topUpUmaxValue;
        }
    }
    ...
    function getPendingRewardDaily(
        ...
        if (
            ...
            block.timestamp / 86400 > pools[_poolId].endJoinTime / 86400
        ) toTime = pools[_poolId].endJoinTime;
        else toTime = (block.timestamp / 86400) * 86400;

        // formular = token * APR / decimalsForAPR * (toTime - latestUpdate) / (365 * 86400 )
        ...
        return (
            (userInfo.valueInUMax * userInfo.APR * diff) /
                decimalsForAPR /
                (365 * 86400) +
    ...
    function updateLatestVault(uint256 _poolId, address _user) private {
        ...
        if (latestVaultIndex > 0) {
            vaultStorage[_poolId][_user][latestVaultIndex]
