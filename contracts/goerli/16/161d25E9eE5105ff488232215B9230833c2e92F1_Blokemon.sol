...
contract Blokemon is ERC721Enumerable, Ownable{
    ...
    function _pickRandomUniqueId() public returns (uint256 id) {
        uint256 newTokenId = uint256(keccak256(abi.encodePacked(block.timestamp, maxSupply)));
        if (tokenExists(newTokenId)){
            _pickRandomUniqueId();
        }else{
            tokens[newTokenId] = msg.sender;
            return newTokenId;
        }
    }
    ...
    function mint(uint256 quantity_) public payable {
        ...
        for(uint256 i; i < quantity_; i++){
            ...
            totalAmount++;
            uint256 tokenId = _pickRandomUniqueId();
