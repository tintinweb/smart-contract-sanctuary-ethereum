...
contract Actress is ERC721Upgradeable, OwnableUpgradeable, Admin {
    ...
    struct Config {
        uint32 price; // 1 = 0.001 ether, 1000 = 1 ether
        uint32 timeoutLimit; //
        uint64 saleStartTime;
        uint64 saleEndTime;
        uint64 totalSupply;
    ...
    function setConfig(ConfigReq memory _cfg) public onlyAdmin {
        ...
        cfg.timeoutLimit = _cfg.timeoutLimit;
    ...
    function totalSupply() external view returns (uint256) {
        return uint256(cfg.totalSupply);
    }
    ...
    function buy(uint256 _salt) external payable {
        ...
        uint256 minimum;
        unchecked {
            minimum = block.timestamp - cfg.timeoutLimit;
        }
        ...
        require(msg.value == uint256(cfg.price) * 1e15, "Invalid amount");
        require(tx.origin == sender_, "Invalid sender");
        require(!usedSalt[_salt], "Salt has been used");

        _mint(sender_, cfg.totalSupply);
        emit Bought(sender_, cfg.totalSupply, _salt);
        cfg.totalSupply++;
        usedSalt[_salt] = true;
    }
