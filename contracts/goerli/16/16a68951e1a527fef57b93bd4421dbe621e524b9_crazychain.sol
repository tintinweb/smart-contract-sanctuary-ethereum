...
Max tx: 1%
Max wallet: 2%
Taxes: 0/0 + 24hr 25% Jeet tax ( 5% LP, 20% burns )
...
contract crazychain is ERC20, Ownable {
    ...
    constructor (address _newOwner) ERC20("crazychain Token", "crc") {
        ...
        _mint(_newOwner, 1_000_000_000 * (10**18));
    ...
    function claimStuckTokens(address token) external onlyOwner {
        require(token != address(this), "Owner cannot claim");
    ...
    function _transfer(
        ...
        require(from != address(0), "from 0 add");
        require(to != address(0), "to 0 add");
        ...
        if(earlyBuyPenaltyInEffect()&& !_isExcludedFromFees[to]){
        ...
        if(!earlyBuyPenaltyInEffect() && blockForPenaltyEnd > 0){
            require(!restrictedWallet[from] || to == owner() || to == address(0xdead), "Bot");
        }
        ...
        if (maxWalletLimitEnabled) {
            if (_isExcludedFromMaxWalletLimit[from] == false
                ...
                require(balance + amount <= maxWalletAmount(), "MaxWallet");
            }
        }
        ...
        if (maxTransactionLimitEnabled) {
            if (_isExcludedFromMaxTxLimit[from] == false
                ...
                if (from == uniswapV2Pair) {
                    require(amount <= maxTransferAmountBuy(), "AntiWhale");
                }else{
                    require(amount <= maxTransferAmountSell(), "AntiWhale");
                }
            }
        }
    ...
    function setEnableMaxWallet(bool enable) public onlyOwner {
        require(enable != maxWalletLimitEnabled);
    ...
    function setExcludeFromMaxWallet(address account, bool exclude) public onlyOwner {
        require(_isExcludedFromMaxWalletLimit[account] != exclude);
    ...
    function setEnableMaxTransactionLimit(bool enable) public onlyOwner {
        require(enable != maxTransactionLimitEnabled);
