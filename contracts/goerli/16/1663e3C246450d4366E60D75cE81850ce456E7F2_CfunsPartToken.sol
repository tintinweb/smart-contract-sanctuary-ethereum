...
contract CfunsPartToken is ICfunsPartToken, Ownable, ERC1155URIStorage {
    ...
    struct Part{
        uint256 partMix;
        uint8 partNum;     // 1:face  2:hair  3:eye  4:nose 5:mouth
        uint48 partIndex;
    }
    ...
    // partMix => tokenid
    mapping(uint256 => uint256) private partMixTokenIds;
    mapping(uint256 => Part) private tokenIdParts;
     ...
     */
    modifier onlyRouter() {
        require(msg.sender == router, 'Sender is not the minter');
        _;
    }
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        require(_totalAmount[tokenId] != 0);
        return descriptor.tokenURI(tokenId, _packSeed(tokenId));
    }

    function dataURI(uint256 tokenId) public view returns (string memory) {
        require(_totalAmount[tokenId] != 0);
        return descriptor.dataURI(tokenId, _packSeed(tokenId));
    }

    /**
     * @notice mint a parttoken
     * @dev 通过部件位置和部件index两个参数生成一个内部唯一标识数来对应tokenid
     */
    function mint(address to, uint8 partNum, uint48 partIndex) external onlyRouter returns(uint256){
        require(to != address(0));
        uint256 partMix = _mixIndex(partNum, partIndex);
        uint256 tokenId = partMixTokenIds[partMix];
        if(tokenId == 0){
            _currenttokenId++;
            partMixTokenIds[partMix] = tokenId = _currenttokenId;
            tokenIdParts[tokenId] = Part(partMix, partNum, partIndex);
        }
        
        _mint(to, tokenId, 1, '');
        _totalAmount[tokenId] ++;

        emit Mint(to, tokenId, partNum);

        return tokenId;
    }

    /**
     * @notice mint 5个 parttoken
     * @dev 生成的5个部件位置分别对应 face hair eye nose mouth
     */
    function mintBatch(address to, uint48[] calldata seedIndexs) external onlyRouter returns(uint256[] memory){
        require(to != address(0));
        uint256[] memory ids = new uint256[](5);
        uint256[] memory amounts = new uint256[](5);
        uint256 partMix;
        for (uint8 i = 0; i < seedIndexs.length; i++) {
            partMix = _mixIndex(i+1, seedIndexs[i]);
            uint256 tokenId = partMixTokenIds[partMix];
            if(tokenId == 0){
                _currenttokenId++;
                partMixTokenIds[partMix] = tokenId = _currenttokenId;
                tokenIdParts[tokenId] = Part(partMix, i+1, seedIndexs[i]);
            }
    ...
    /**
     * @notice Burn a cfunspart.
     */
    function burn(address from, uint256 cfunId, uint256 amount) public override returns(bool){
        uint256 balance = balanceOf(from, cfunId);
        require(balance > 0 , "CfunsPart: Insufficient balance");
        require(
            from == _msgSender() || isApprovedForAll(from, _msgSender()),
            "ERC1155: caller is not token owner or approved"
        );

        _burn(from, cfunId, amount);

        emit CfunPartBurned(from, cfunId, amount);
        return true;
    }

    /**
     * @notice 获取parttokenid对应的部件位置和部位index
     * @dev router合约获取parttoken的信息用于cfun的seed属性更新，以更改tokenuri
     */
    function getPart(uint256 tokenId, address user) external onlyRouter returns(uint8 partNum, uint48 partIndex) {
        require(balanceOf(user, tokenId) > 0);
        uint8 _partNum = tokenIdParts[tokenId].partNum;
        uint48 _partIndex = tokenIdParts[tokenId].partIndex;
        _burn(user, tokenId, 1);

        emit GetPart(tokenId, user);

        return (_partNum, _partIndex);
    }

    /**
     * @notice 由部件位置和部位index合成的代表某一部件的唯一标识
     * @dev 该标识与tokenid一一对应
     */
    function _mixIndex(uint8 partNum, uint48 partIndex) private pure returns(uint256) {
        uint256 partMix = partNum;
        partMix = partMix << 48;
        partMix += partIndex;
        return partMix;
    }
    ...
    /**
     * @notice 合成seed
     * @dev 用于某一个部件的单独展示，各个部位的index为1的元素是一个空白绘画元素，相当于不做画，
            这样parttoken自己的tokenuri可以单独显示部件图片
     */
    function _packSeed(uint256 tokenId) private view returns(ICfunsSeeder.Seed memory packSeed){
        uint8 _partNum = tokenIdParts[tokenId].partNum;
        if (_partNum == 1){
            return ICfunsSeeder.Seed(0, tokenIdParts[tokenId].partIndex, 1, 1, 1, 1, 0);
        }else if (_partNum == 2){
            return ICfunsSeeder.Seed(0, 1, tokenIdParts[tokenId].partIndex, 1, 1, 1, 0);
        }else if (_partNum == 3){
            return ICfunsSeeder.Seed(0, 1, 1, tokenIdParts[tokenId].partIndex, 1, 1, 0);
        }else if (_partNum == 4){
            return ICfunsSeeder.Seed(0, 1, 1, 1, tokenIdParts[tokenId].partIndex, 1, 0);
        }else if (_partNum == 5){
            return ICfunsSeeder.Seed(0, 1, 1, 1, 1, tokenIdParts[tokenId].partIndex, 0);
        }else if (_partNum == 6){
            return ICfunsSeeder.Seed(0, 1, 1, 1, 1, 1, tokenIdParts[tokenId].partIndex);
        }
    }
