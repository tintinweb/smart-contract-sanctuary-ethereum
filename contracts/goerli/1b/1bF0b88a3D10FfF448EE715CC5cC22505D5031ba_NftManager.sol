...
contract NftManager {
    ...
    function getClaimableAmount(uint256 _type, address account)
        external
        view
        returns (uint256)
    {
        if (_type == 1) {
            return kittieNftType1.getClaimableAmount(account);
        } else if (_type == 2) {
            return kittieNftType2.getClaimableAmount(account);
        } else if (_type == 3) {
            return kittieNftType3.getClaimableAmount(account);
        } else {
            revert("Invalid type");
        }
    }
