...
error BasicNFT__NotEnouthEtherSent(uint256 amountSent, uint256 amountRequired);
...
contract BasicNFT is ERC721, Ownable {
    ...
    function mintNFT() external payable {
        if (msg.value < nftPrice) {
            revert BasicNFT__NotEnouthEtherSent(msg.value, nftPrice);
        }
        _safeMint(msg.sender, tokenCounter);
        emit NftMinted(msg.sender, tokenCounter, nftPrice);
        tokenCounter++;
    }
