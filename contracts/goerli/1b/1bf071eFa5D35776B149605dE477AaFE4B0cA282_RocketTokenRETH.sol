...
import "./interface/RocketNetworkBalancesInterface.sol";
import "./interface/RocketTokenRETHInterface.sol";
import "./interface/RocketDAOProtocolSettingsNetworkInterface.sol";
...
contract RocketTokenRETH is RocketBase, ERC20, RocketTokenRETHInterface {
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketBase(_rocketStorageAddress) ERC20("Myso Rocket Pool ETH - 2", "MysoRETH2") {
        // Version
        version = 1;
    ...
    function getTotalCollateral() override public view returns (uint256) {
        ...
        return rocketDepositPool.getExcessBalance() + address(this).balance;
    }
    ...
    function withdrawDepositCollateral(uint256 _ethRequired) private {
        ...
        // Withdraw
        RocketDepositPoolInterface rocketDepositPool = RocketDepositPoolInterface(getContractAddress("rocketDepositPool"));
        rocketDepositPool.withdrawExcessBalance(_ethRequired - ethBalance);
    }
    ...
    function depositExcessCollateral() external override {
        ...
        if (collateralRate > targetCollateralRate) {
            ...
            uint256 targetCollateral = address(this).balance * targetCollateralRate / collateralRate;
            // If we have excess
            if (address(this).balance > targetCollateral) {
                // Send that excess to deposit pool
                uint256 excessCollateral = address(this).balance - targetCollateral;
