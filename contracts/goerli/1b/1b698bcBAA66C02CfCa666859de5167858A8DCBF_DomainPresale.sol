...
contract DomainPresale is Ownable, Pausable, ReentrancyGuard {
    ...
    function issue(
        ...
        //2. not exist
        require(presaleInfo.endTime < block.timestamp && presaleInfo.remainAmount == 0, "DomainPresale: token presale already exists");
    ...
    /**
     * @notice Buy presale erc20
     * @param erc20 presale erc20
     * @param buyAmount buy amount
     */
    function buy(address erc20, uint256 buyAmount) external whenNotPaused nonReentrant {
        ...
        //5. purchasableAmountWithoutPower 可购买碎片数量，没有精度
        uint256 purchasableAmountWithoutPower = presaleInfo.remainAmount > buyAmount ? buyAmount : presaleInfo.remainAmount;

        //6. paymentFunds 支付资金
        uint256 paymentFunds = purchasableAmountWithoutPower.mul(presaleInfo.price);

        //7. transfer from 转移碎片
    ...
    /**
     * @notice Buy presale erc20 using ETH
     * @param erc20 presale erc20
     * @param buyAmount buy amount
     */
    function buyUsingETHAndWETH(address erc20, uint256 buyAmount) payable external whenNotPaused nonReentrant {
        ...
        //8. transfer from 转移碎片
        uint256 purchasableAmountWithPower = _toWithPower(erc20, purchasableAmountWithoutPower);
        IERC20Metadata(erc20).transferFrom(presaleInfo.curator, _msgSender(), purchasableAmountWithPower);

        //9. transfer from 支付总额 转账给策展人
        IWETH(WETH).transfer(presaleInfo.curator, paymentFunds);

        //10. subtract presale erc20 remain amount 扣减预售数量(没有精度的数额)
        presaleInfo.remainAmount = presaleInfo.remainAmount.sub(purchasableAmountWithoutPower);

        //11. sale amount 扣减预售数量(没有精度的数额)
