...
contract ResiklosTest1 is ERC721A, Ownable {
    using Strings for uint256;
    ...
    uint256 public LEGENDARY_PRICE = .1 ether;
    uint256 public RARE_PRICE = .1 ether;
    uint256 public COMMON_PRICE = .1 ether;
    ...
    function rskMint(bytes32[] memory _merkleProof, string calldata rarity, uint256 _quantity)  external payable correctState(SaleState.MINTNOW) {
        ...
        if (keccak256(abi.encodePacked(rarity)) == keccak256(abi.encodePacked("legendary"))) {
            if (MAX_LEGENDARY_MINT > 0) {
                require((totalLegendaryMint[msg.sender] + _quantity) <= MAX_LEGENDARY_MINT, "You have reached the maximum amount of mints.");   
            } 
            require(msg.value >= LEGENDARY_PRICE * _quantity, "Invalid Amount.");

            bytes32 sender = keccak256(abi.encodePacked(msg.sender));
            require(MerkleProof.verify(_merkleProof, merkleRootResiklos, sender), "You are not an OG!");        
            ...
            if (MAX_RARE_MINT > 0) {
                require((totalRareMint[msg.sender] + _quantity) <= MAX_RARE_MINT, "You have reached the maximum amount of mints.");
            }  
            ...
            if (MAX_COMMON_MINT > 0) {
                require((totalCommonMint[msg.sender] + _quantity) <= MAX_COMMON_MINT, "You have reached the maximum amount of mints.");  
            }
    ...
    function mapRarity(string memory rarity, uint256 _quantity) public onlyOwner {
    ...
    function _baseURICommon() internal view virtual  returns (string memory) {
        ...
        //based on rarityMap  tokenID and rarity
         //determine rarity to set the tokenURI for that current token index
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory)  {
        ...
        if (keccak256(abi.encodePacked(rarity)) == keccak256(abi.encodePacked("legendary"))) {
            return bytes(rarity).length > 0 ? string(abi.encodePacked(legendBaseURI, tokenId.toString(), '.json')) : '';

        } else if (keccak256(abi.encodePacked(rarity)) == keccak256(abi.encodePacked("rare"))) {
             return bytes(rarity).length > 0 ? string(abi.encodePacked(rareBaseURI, tokenId.toString(), '.json')) : '';

        } else if (keccak256(abi.encodePacked(rarity)) == keccak256(abi.encodePacked("common"))) {
            return bytes(rarity).length > 0 ? string(abi.encodePacked(commonBaseURI, tokenId.toString(), '.json')) : '';
    ...
    // ONLY OWNER
    // function rskFoundersMint(uint256 _quantity, string memory rarity) external onlyOwner {
    //     require((totalSupply() + _quantity) <= MAX_SUPPLY, "Beyond max supply.");
    ...
    function devMint(uint256 legendary, uint256 rare, uint256 common) external onlyOwner {
        // 10 common, 5 rare , 3 legendary
        // 15 common, 3 rare , 0 legendary
        // 18 common, 0 rare , 0 legendary
        ...
        if(legendary > 0) {
            if (MAX_LEGENDARY_MINT > 0) {
                require((totalLegendaryMint[msg.sender] + legendary) <= MAX_LEGENDARY_MINT, "You have reached the maximum amount of mints.");   
            } 
        ...
        if(rare > 0) {
            if (MAX_RARE_MINT > 0) {
                require((totalRareMint[msg.sender] + rare) <= MAX_RARE_MINT, "You have reached the maximum amount of mints.");
            }  
        ...
        if(common > 0) {
            if (MAX_COMMON_MINT > 0) {
                require((totalCommonMint[msg.sender] + common) <= MAX_COMMON_MINT, "You have reached the maximum amount of mints.");  
            }
