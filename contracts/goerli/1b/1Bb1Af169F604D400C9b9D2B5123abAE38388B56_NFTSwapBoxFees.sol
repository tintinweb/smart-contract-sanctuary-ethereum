...
contract NFTSwapBoxFees is INFTSwap,AccessControl {
    ...
    // mapping(address => uint256) private NFTRoyaltyFee;
    mapping(address => uint256) private ERC20SwapFee;
    // mapping(address => address) private NFTRoyaltyReceiver;
    mapping(address => RoyaltyFee) private nftRoyaltyFee;
    mapping(uint256 => mapping(uint256 => address)) private availableOfferAddress;
    ...
    function setNFTRoyaltyFee(address nftAddress, uint256 fee, address receiver) public {
        require(hasRole(ADMIN, msg.sender) || hasRole(MANAGER, msg.sender), "Not member");
        require(fee > 0, "fee must be greate than 0");
        nftRoyaltyFee[nftAddress].feeAmount = fee;
    ...
    function _checkERC20Fee(
        ...
        uint256 discount = INFTSwapFeeDiscount(nftSwapFeeDiscount).getUserDiscount(boxOwner);
        ERC20Fee[] memory erc20fees = new ERC20Fee[](erc20Details.tokenAddrs.length);
        for(uint256 i = 0 ; i < erc20Details.tokenAddrs.length ; i++) {
            uint256 fee = 0;
            if(ERC20SwapFee[erc20Details.tokenAddrs[i]] > 0)
                fee = erc20Details.amounts[i].mul(ERC20SwapFee[erc20Details.tokenAddrs[i]]);
            else
                fee = erc20Details.amounts[i].mul(defaultTokenSwapPercentage);
            erc20fees[i].tokenAddress = erc20Details.tokenAddrs[i];
            erc20fees[i].feeAmount = fee.div(10000) - fee.div(10000) * discount.div(10000);
        }
    ...
    function _checkGasTokenFee(
        ...
        fee = fee - fee.mul(discount).div(10000);
        return fee;
    }
    ...
    function _checkNFTFee(
        ...
        for(uint256 i = 0 ; i < erc721Details.length; i ++){
            uint256 nftDiscount = INFTSwapFeeDiscount(nftSwapFeeDiscount).getNFTDiscount(erc721Details[i].tokenAddr);
            ...
            if(nftDiscount > discount)
                fee -= fee.mul(nftDiscount).div(10000);
            else
                fee -= fee.mul(discount).div(10000);
        }

        for(uint256 i = 0 ; i < erc1155Details.length; i ++){
            uint256 nftDiscount = INFTSwapFeeDiscount(nftSwapFeeDiscount).getNFTDiscount(erc1155Details[i].tokenAddr);
            uint256 totalAmount = 0;
            for(uint256 j = 0 ; j < erc1155Details[i].amounts.length ; j++) {
                totalAmount += erc1155Details[i].amounts[j];
            }
            if(NFTSwapFee[erc1155Details[i].tokenAddr] > 0)
                fee += NFTSwapFee[erc1155Details[i].tokenAddr].mul(erc1155Details[i].ids.length).mul(totalAmount);
            else
                fee += defaultNFTSwapFee.mul(erc1155Details[i].ids.length).mul(totalAmount);
            if(nftDiscount > discount)
                fee -= fee.mul(nftDiscount).div(10000);
            else
                fee -= fee.mul(discount).div(10000);
    ...
    function _checkRoyaltyFee(
        ...
        for(uint256 i = 0 ; i < erc721Details.length; i ++){
            uint256 nftDiscount = INFTSwapFeeDiscount(nftSwapFeeDiscount).getNFTDiscount(erc1155Details[i].tokenAddr);
            if(nftRoyaltyFee[erc721Details[i].tokenAddr].feeAmount > 0)
            {
                royaltyFee[nftIndex].feeAmount = nftRoyaltyFee[erc721Details[i].tokenAddr].feeAmount.mul(erc721Details[i].ids.length);
                
                if(nftDiscount > discount)
                    royaltyFee[nftIndex].feeAmount -= royaltyFee[nftIndex].feeAmount.mul(nftDiscount).div(10000); 
                else
                    royaltyFee[nftIndex].feeAmount -= royaltyFee[nftIndex].feeAmount.mul(discount).div(10000); 
                
                royaltyFee[nftIndex].reciever = nftRoyaltyFee[erc721Details[i].tokenAddr].reciever;
                nftIndex++;
            }
        }
        ...
        for(uint256 i = 0 ; i < erc1155Details.length; i ++){
            ...
            if(nftRoyaltyFee[erc1155Details[i].tokenAddr].feeAmount > 0)
            {
                royaltyFee[nftIndex].feeAmount = nftRoyaltyFee[erc1155Details[i].tokenAddr].feeAmount.mul(erc1155Details[i].ids.length).mul(totalAmount);
                if(nftDiscount > discount)
                    royaltyFee[nftIndex].feeAmount -= royaltyFee[nftIndex].feeAmount.mul(nftDiscount).div(10000);
                else
                    royaltyFee[nftIndex].feeAmount -= royaltyFee[nftIndex].feeAmount.mul(discount).div(10000);
                royaltyFee[nftIndex].reciever = nftRoyaltyFee[erc1155Details[i].tokenAddr].reciever;
                nftIndex++;
            }
        }
