...
contract DigitalVehicleRegistration {
    ...
    function addVehicle(bytes32 _id, string memory _ipfsAddr) public {
    ...
    function transferTo(bytes32 _id, address _newOwner) public {
        ...
        DigitalVehicle(transferVehicle.contractAddr).changeOwner(_newOwner);
...
contract DigitalVehicle is AccessControl {
    ...
    //addresses
    address public currentReporter;
    address public currentOwner;
    address public currentMaintainer;
    ...
    event ownershipTransfered(address _newOwner, address _oldOwner);
    ...
    constructor(bytes32 _vin, string memory _ipfsAddr) {
        ...
        _setRoleAdmin(MAINTAINER_ROLE, OWNER_ROLE);
        _setRoleAdmin(REPORTER_ROLE, OWNER_ROLE);
    }
    ...
    function getCurrentRole() public view returns (string memory) {
    ...
    function assignMaintainer(address _newMaintainer)
        public
        onlyRole(OWNER_ROLE)
    {
        if (currentMaintainer != address(0)) {
            revokeRole(MAINTAINER_ROLE, currentMaintainer);
        }
        currentMaintainer = _newMaintainer;
        grantRole(MAINTAINER_ROLE, _newMaintainer);
    }

    function assignReporter(address _newRole) public onlyRole(OWNER_ROLE) {
        if (currentReporter != address(0)) {
            revokeRole(REPORTER_ROLE, currentReporter);
        }
        currentReporter = _newRole;
        grantRole(REPORTER_ROLE, _newRole);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        if (currentOwner != address(0)) {
            // revokeRole(MAINTAINER_ROLE, currentMaintainer);
            revokeRole(OWNER_ROLE, currentOwner);
        }
        // grantRole(DEFAULT_ADMIN_ROLE, _newOwner);
        grantRole(OWNER_ROLE, _newOwner);

        emit ownershipTransfered(_newOwner, currentOwner);

        currentOwner = _newOwner;
    }

    function updateOperationHours(uint256 _hours)
        public
    ...
    function getLocation() public view returns (int256[2] memory) {
        int256[2] memory location = [lat, long];
        return location;
    }

    function updateLocation(int256 _lat, int256 _long)
        public
        onlyRole(REPORTER_ROLE)
    {
        lat = _lat;
        long = _long;
        emit updatedLocation(lat, long);
    }

    function logMaintenance(string memory _parts)
        public
