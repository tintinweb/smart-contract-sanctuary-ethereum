...
import "./ERC721CollectorBound.sol";
    ...
    ERC721CollectorBound
{
     ...
     */
    function setBindingEnable(bool enableBinding) external onlyOwner {
        _setBindingEnable(enableBinding);
    }

    function kickFromBinding(uint256 tokenId) external onlyOwner {
        _exitBinding(tokenId);
    }

    function swapBindingOperator(address operator) external onlyOwner {
    ...
    /**
        ...
        Binding based approval and transfer control: The users cannot approve their token if it is bound.
    ...
    function mintEpics(address to, uint256 series, uint256 quantity) public onlyOwner {
        unchecked{
    ...
    function mint(
        ...
        for(uint256 i = batch; i < (count+batch); i++){
        // quantity check
        if (quantity[i] == 0) {
            revert ZeroQuantity();
        }
...
contract ERC721CollectorBound {
    bytes32 private constant storagePosition =
        keccak256("diamond.storage.ERC721CollectorBound");

    error AlreadyBound();
    error NotBound();
    error BindingDisabled();
    error NotAllowed();
    error NotAuthorized();

    struct ERC721CollectorBoundStorage {
    ...
    struct TokenParameter {
        uint64 bindingStartTime;
        uint64 totalBindingTime;
    }
    ...
    function _retriveERC721Storage()
        private
        pure
        returns (ERC721CollectorBoundStorage storage ds)
    {
    ...
    function isBinding(uint256 tokenId) public view returns (bool) {
        return _retriveTokenParam(tokenId).bindingStartTime > 0;
    }

    function bindingTime(uint256 tokenId) public view returns (uint256 t) {
        t = _retriveTokenParam(tokenId).totalBindingTime;
        if (isBinding(tokenId)) {
            t +=
                uint64(block.timestamp) -
                _retriveTokenParam(tokenId).bindingStartTime;
        }
    }

    function enterBinding(uint256 tokenId) external onlyTokenOwner(tokenId) {
        _enterBinding(tokenId);
    }

    function exitBinding(uint256 tokenId) external onlyOperator {
        _exitBinding(tokenId);
    }

    function enterBindingMulti(uint256[] calldata tokenId)
        external
        onlyTokensOwner(tokenId)
    {
        for (uint256 i; i < tokenId.length; i++) {
            _enterBinding(tokenId[i]);
        }
    }

    function exitBindingMulti(uint256[] calldata tokenId)
        external
        onlyOperator
    {
        for (uint256 i; i < tokenId.length; i++) {
            _exitBinding(tokenId[i]);
        }
    }

    function _enterBinding(uint256 tokenId) internal {
        if (isBinding(tokenId)) {
            revert AlreadyBound();
        }

        if (!_retriveERC721Storage().enableBinding) {
            revert BindingDisabled();
        }

        _retriveTokenParam(tokenId).bindingStartTime = uint64(
            block.timestamp
        );
    }

    function _exitBinding(uint256 tokenId) internal {
        if (!isBinding(tokenId)) {
            revert NotBound();
        }

        _retriveTokenParam(tokenId).totalBindingTime +=
            uint64(block.timestamp) -
            _retriveTokenParam(tokenId).bindingStartTime;
        _retriveTokenParam(tokenId).bindingStartTime = 0;
    }

    function _setBindingEnable(bool enableBinding) internal {
        _retriveERC721Storage().enableBinding = enableBinding;
    }
    ...
    function isBindingEnabled() public view returns (bool) {
        return _retriveERC721Storage().enableBinding;
    }
