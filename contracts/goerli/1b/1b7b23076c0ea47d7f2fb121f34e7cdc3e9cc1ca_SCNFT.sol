...
contract SCNFT is ERC721 {
    ...
    function mintMultiple(address _to, uint _amount) payable public {
        // 1) mint _amount of NFT
        for (uint256 i = 0; 1 < _amount; i++) {
            //get current tokenId
            uint256 tokenId = _tokenIds.current();
            safeMint(tokenId);
            _tokenIds.increment();
        }
        // 2) calculate excess fund
        uint256 balance = msg.value - (mintPrice * _amount);
        // 3) return excess fund
        if (balance > 0) {
    ...
    function refund(uint256 _tokenId) external {
        //destroy designated NFT id
        burn(_tokenId);
        payable(msg.sender).transfer(mintPrice);
        //TODO: add event for refund
    }

    // TODO: 項目方領錢
    function withdraw () external {
        //check if the person is owner.
        require(owner == msg.sender, "you are not conteact owner");
