...
error Raffle__NotEnouhghETHEntered();
error Raffle__TransferFailed();
error Raffle__NotCurrentlyOpen();
...
contract Raffle is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    bytes32 private immutable i_vrfKeyHarsh;
    uint32 immutable i_gas_limit;
    uint16 constant VRF_REQUEST_COMFIRMATION = 3;
    ...
    function enterRaffle() public payable {
        if (msg.value < i_entranceFee) {
            revert Raffle__NotEnouhghETHEntered();
        }
        s_players.push(payable(msg.sender));
        if (s_raffleState != RaffleState.OPEN) {
            revert Raffle__NotCurrentlyOpen();
        }
    ...
    /**
     * @dev This is the fucntion that the Chainlink nodes call
     */
