    ...
    RoleManager
{
    ...
    {
        require(
            _brokrageAmt < 100 * decimalPrecision,
        ...
        auctionNonExistant(nftContract, tokenId)
    {
        ...
        IERC721 instane = IERC721(nftContract);
        require(
            instane.isApprovedForAll(instane.ownerOf(tokenId), address(this)) ||
                instane.getApproved(tokenId) == address(this),
        ...
        auctionComplete(nftContract, tokenId)
    {
        ...
        IERC2981 RoyalityInfo = IERC2981(nftContract);
        (address creator, uint256 royality) = RoyalityInfo.royaltyInfo(
            tokenId,
            auctions[nftContract][tokenId].amount
        );
        ...
        if (creator == fundsRecipient) {
            ...
            if (royality > 0) {
                transferBNBOrWBNB(payable(creator), royality);
            }
            // Send the remainder of the amount to the funds recipient.
            transferBNBOrWBNB(
                fundsRecipient,
                amount.sub(borkerAmount).sub(royality)
            );
        }
    ...
    function recoverERC20(address _erc20) external isOperator {
        require(_erc20 != wbnbAddress, "Can't recover BNB directly");
...
contract RoleManager is Initializable , AccessControlEnumerableUpgradeable, OwnableUpgradeable  {
