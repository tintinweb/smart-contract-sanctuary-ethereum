    ...
    Ownable
{
    ...
    IAmartyaGate public AmartyaGateContract;
    ...
    constructor(
        ...
        address _amartyaGateAddress,
        uint256 _maxSupply
    ) ERC721A("Amartya", "AMARTYA") EIP712("Amartya", "1.0.0") {
        stage = Stage.Pause;
        signer = _signer;
        baseURI = _previewURI;
        AmartyaGateContract = IAmartyaGate(_amartyaGateAddress);
    ...
    /// @notice Reveal burned NFT
    function reveal(
        uint256 _tokenId,
        bytes calldata _signature
    ) external nonReentrant notContract {
        require(stage == Stage.Reveal, "STAGE_NMATCH");
        require(
            signer == _verify(msg.sender, _tokenId, _signature),
            "INVALID_SIGNATURE"
        );
        require(!tokenOwner[_tokenId], "ALREADY_CLAIMED");
        require(totalSupply() + 1 <= maxSupply, "SUPPLY_EXCEEDED");

        tokenOwner[_tokenId] = true;
        AmartyaGateContract.burn(_tokenId);
        _mint(msg.sender, 1);

        emit Revealed(_tokenId, _nextTokenId() - 1, msg.sender);
    }
    ...
    function _verify(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(abi.encode(MINTER_TYPEHASH, _recipient, _tokenId))
        );
    ...
    function setAmartyaGateContract(
        address _amartyaGateAddress
    ) external onlyOwner {
        AmartyaGateContract = IAmartyaGate(_amartyaGateAddress);
    }
