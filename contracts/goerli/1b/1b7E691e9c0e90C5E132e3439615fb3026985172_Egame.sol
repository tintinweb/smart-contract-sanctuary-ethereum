...
contract Egame is ReentrancyGuard {
  ...
  function claimScore(uint256 _score) public nonReentrant returns (bool)  {
    ...
    if ( !(validating(msg.sender, oldScore))) return false;
    ...
    // will return tru if claim score is valid
    // return false will cancell
    return true;
  }
  ...
  function calculateToken(address _player) private {
    ...
    require(_score > 1000,"Must be => 1000 to claim");
    uint256 _token = 0;
  ...
  // update token player after claim
  function updateToken(uint256 _subToken) internal {
    uint256 oldToken = mappingPlayer[msg.sender].token;
    uint256 newToken = oldToken - _subToken;
    mappingPlayer[msg.sender].token = newToken;
  }
