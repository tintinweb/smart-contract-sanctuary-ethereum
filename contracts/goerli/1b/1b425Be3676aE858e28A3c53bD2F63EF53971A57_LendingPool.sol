...
contract LendingPool is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function calculateWalletRewardsYieldPeriod(
        ...
        return PeriodStaking.calculateWalletRewardsYieldPeriod(periodStakingStorage[0], wallet, rewardPeriodId, yieldPeriod);
    }
}
...
library PeriodStaking {
    ...
    function calculateWalletRewardsYieldPeriod(
        ...
        uint256 walletStakingScore = periodStakingStorage.walletStakingScores[rewardPeriod.id][wallet];
        ...
        // 2) Deflate walletStakingScore down to current moment
        uint256 walletStakingScorePresentMoment = walletStakingScore - (rewardPeriod.end - blockNumber) * periodStakingStorage.walletStakedAmounts[wallet].stakedBalance;

        // 3) normalize the rewards from period to one block
        uint256 yieldBearingCapitalPresentMoment = walletStakingScorePresentMoment / (blockNumber - rewardPeriod.start);

        // 1000 = 4 decimals precision for yieldDecimals
        // percentage is based on the length of a period
        return ((yieldBearingCapitalPresentMoment * 10000) / yieldPeriod);
    }
