...
contract Nexus is ERC721A, ERC2981, AccessControl {
    ...
    // 権限の付与/剥奪は grantRole, revokeRole を使用
    modifier onlyMgr() {
    ...
    function mintGold(address _address, bytes32[] calldata _merkleProof) external {
        ...
        // totalSupplyで正しく動くかは要テスト
        mapTokenKind[totalSupply() - 2] = 1;
        mapTokenKind[totalSupply() - 1] = 4;
        mapTokenKind[totalSupply()]     = 5;
    }

    function mintPlatinum(address _address, bytes32[] calldata _merkleProof) external {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    ...
    function mintBlack(address _address, bytes32[] calldata _merkleProof) external {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
