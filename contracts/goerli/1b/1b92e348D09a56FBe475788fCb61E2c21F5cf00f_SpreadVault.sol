    ...
    ReentrancyGuardUpgradeable
{
    ...
    IAggrigatorWrapper public PRICE_FEED;
    ...
    /// the strike price Low for each round
    mapping(uint256 => uint256) public StrikeLowX1e6;
    /// the strike price High for each round
    mapping(uint256 => uint256) public StrikeHighX1e6;
    ...
    event NewRound(
        address indexed _from,
        uint256 indexed _round,
        uint256 _strikeLowX1e6,
        uint256 _strikeHighX1e6,
        uint256 _premium,
    ...
    function initialize(
        ...
        require(
            collateralAddress != address(0) &&
                aggrigatorWrapper != address(0) &&
                linkAggregator != address(0) &&
                vaultMaker != address(0) &&
                owner != address(0) &&
                treasury != address(0),
            "address zero"
        );
        ...
        PRICE_FEED = IAggrigatorWrapper(aggrigatorWrapper);
    ...
    function processQueue() public nonReentrant clearQueue {}
     ...
     */
    function deposit(uint256 amt) external nonReentrant clearQueue {
     ...
     */
    function initWithdraw() external nonReentrant clearQueue returns (uint256) {
    ...
    function cancelWithdraw() external nonReentrant clearQueue {
        require(
    ...
    function checkVaultParams(
        ...
        if (_spotPrice == 0) {
            (int256 price, uint80 id) = PRICE_FEED.getLastPriceX1e6(
                address(LINK_AGGREGATOR)
            );
            _spotPrice = uint256(price);
            roundID = id;
        }
        if (VAULT_TYPE == VaultTypes.PUT_SPREADS) {
            // PUT SPOT Price > Low Strike > High Strike
            require(
                _spotPrice > roundStrikeLowX1e6,
                "_spotPrice<=roundStrikeLowX1e6"
            );
            require(
                roundStrikeLowX1e6 > roundStrikeHighX1e6,
                "roundStrikeLowX1e6<=roundStrikeHighX1e6"
            );
        } else {
            // SPOT Price < Low Strike < High Strike
            require(
                _spotPrice < roundStrikeLowX1e6,
                "_spotPrice>=roundStrikeLowX1e6"
            );
     ...
     * @param roundStrikeLowX1e6:the round strike price Low
     * @param roundStrikeHighX1e6:the round strike price High
    ...
    function initNewRound(
        ...
        checkVaultParams(roundStrikeHighX1e6, roundStrikeLowX1e6, _spotPrice);

        StrikeLowX1e6[round + 1] = roundStrikeLowX1e6;
        StrikeHighX1e6[round + 1] = roundStrikeHighX1e6;
    ...
    function sendPremium(uint256 premium) external nonReentrant {
        require(msg.sender == VAULT_MAKER, "NotDesignatedMaker");
        require(
     ...
     */
    function settleStrikeByRoundExpiry(uint256 SettlementAmount) internal {
    ...
    function settlementPending(
        ...
        if (price > 0) {
            settleStrikeManual(price, SettlementAmount);
        } else {
            settleStrikeByRoundExpiry(SettlementAmount);
        }
    }
    ...
    function _settleStrike(
        ...
        if (VAULT_TYPE == VaultTypes.PUT_SPREADS) {
            _settleStrike_put(priceX1e6, SettlementAmount, oracleTime, roundID);
        } else {
            _settleStrike_call(
                priceX1e6,
                SettlementAmount,
    ...
    function _settleStrike_call(
        ...
        if (roundExpiry > 0) {
            ...
            if (settlementPrice > StrikeHighX1e6[round]) {
                require(SettlementAmount > 0, "SettlementAmount==0");
                require(
                    SettlementAmount < totalAsset[round],
                    "SettlementAmount>TVL"
                );
                settlementLoss[round] = SettlementAmount;
    ...
    function settlementComplete() external nonReentrant {
        require(msg.sender == VAULT_MAKER, "Not Maker");
     ...
     */
    function settleStrikeManual(
        uint256 priceX1e6,
        uint256 SettlementAmount
    ) internal {
        require(msg.sender == VAULT_MAKER, "Not Maker");
        _settleStrike(priceX1e6, SettlementAmount, roundExpiry, 0);
    }
    ...
    function setAggrigatorWrapper(
        IAggrigatorWrapper newAggrigatorWrapper
    ) external {
        require(msg.sender == VAULT_MAKER, "Not VAULT_MAKER");
     ...
     */
    function depositOnBehalf(address tgt, uint256 amt) external nonReentrant {
        require(msg.sender == VAULT_MAKER, "Not VAULT_MAKER");
        require(tgt != address(0), "address-zero");
