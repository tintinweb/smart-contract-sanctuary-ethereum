...
abstract contract ERC20Permit is IERC20Permit, EIP712, ERC20 {
...
abstract contract BurnPermit is EIP712, ERC20 {
    using CheckSig for address;
    ...
    bytes32 private constant BURN_TYPEHASH =
        keccak256("Burn(address applicant,uint256 amount,uint256 nonce,uint256 deadline)");

    // solhint-disable-next-line var-name-mixedcase
    bytes32 private constant BURN_FROM_TYPEHASH =
        keccak256("Burn(address applicant,address from,uint256 amount,uint256 nonce,uint256 deadline)");

    /// @dev Compute digest of the burn request.
    function burnDigest(address applicant, uint256 amount, uint256 deadline) public view returns (bytes32) {
        bytes32 structHash =
            keccak256(abi.encode(BURN_TYPEHASH, applicant, amount, _burnNonces[applicant], deadline));
        bytes32 digest =
            keccak256(abi.encodePacked("\x19\x01", _domainSeparator(), structHash));
        return digest;
    }

    /// @dev Compute digest of the burn from request.
    function burnFromDigest(address applicant, address from, uint256 amount, uint256 deadline) public view returns (bytes32) {
        bytes32 structHash =
            keccak256(abi.encode(BURN_FROM_TYPEHASH, applicant, from, amount, _burnNonces[applicant], deadline));
        bytes32 digest =
            keccak256(abi.encodePacked("\x19\x01", _domainSeparator(), structHash));
        return digest;
    }
    ...
    function burn(
        ...
        bytes32 digest = burnDigest(applicant, amount, deadline);
        _burnNonces[applicant]++;

        applicant.checkSignature(digest, v, r, s);
    ...
    function burnFrom(
        ...
        bytes32 digest = burnFromDigest(applicant, from, amount, deadline);
        _burnNonces[applicant]++;
...
abstract contract ERC20 is Ownable, IERC20 {
    ...
    // uint256 private constant BLOCKS_PER_DAY   = 3600 / 12 * 24;
    uint256 private constant BLOCKS_PER_YEAR  = 365 * BLOCKS_PER_DAY;
    ...
    event Shrink(string indexed share, uint256 amount);
    ...
    function mintToPlatform(address recipient, uint256 amount) public onlyOwner {
    ...
    function batchMintToPlatform(address[] memory recipients, uint256[] memory amounts) external onlyOwner {
    ...
    function mintToPartner(
        ...
        require(totalAmount > 0, "mint amount too less");
        require(totalAmount <= _partnerShare, "Insufficient partner share");
        ...
        uint256 deadline = block.number.add(vestingDays.mul(BLOCKS_PER_DAY));
        if (vestingAmount > 0) {
            _mintVestFund(state, Fund.FundType.Partner, vestingAmount, deadline);
        }

        if (evenVestingAmount > 0) {
            _mintEvenVestFund(
                state,
                Fund.FundType.Partner,
                evenVestingAmount,
                deadline,
                deadline.add(evenVestingDays.mul(BLOCKS_PER_DAY))
            );
        }
    ...
    function batchMintToPartners(
        ...
        require(recipients.length == amounts.length, "Unmatched array length");
        require(amounts.length == vestingAmounts.length, "Unmatched array length");
        require(vestingAmounts.length == vestingDaysArr.length, "Unmatched array length");
        require(vestingDaysArr.length == evenVestingAmounts.length, "Unmatched array length");
        require(evenVestingAmounts.length == evenVestingDaysArr.length, "Unmatched array length");
    ...
    function batchMintToTeamMembers(address[] memory recipients, uint256[] memory amounts) external onlyOwner {
        require(recipients.length == amounts.length, "Unmatched array length");
    ...
    function _burn(address applicant, uint256 amount) internal {
        ...
        require(state.available >= amount, "Burn: Insufficient balance");
...
abstract contract EIP712 {
    ...
    /// @dev Returns the domain separator used in the encoding of the signature for HashKey Token, as defined by {EIP712}.
    // solhint-disable-next-line func-name-mixedcase
...
library CheckSig {
    function checkSignature(
        ...
        if (from.code.length > 0) {
            // from is a contract
            require(IERC1271(from).isValidSignature(digest, abi.encodePacked(r, s, v)) == 0x1626ba7e, "ERC1271: Unauthorized");
        } else {
            address signer = ecrecover(digest, v, r, s);
            require(signer != address(0), "CheckSig: Invalid signature");
            require(signer == from, "CheckSig: Unauthorized");
        }
    }
}
