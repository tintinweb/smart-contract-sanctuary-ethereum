...
contract CheckDescriptor is ICheckDescriptor {
    uint base = 1000000;
    uint layers = 5;
    function tokenURI(uint256 tokenId, string memory seed) external pure returns (string memory) {
        bytes memory svg = abi.encodePacked(
            ...
            '<tspan dy="22" x="155" fill="#', substring(seed, 36, 41),'">-------</tspan>',
            '</text></svg>'
        );
        return string(
            abi.encodePacked(
...
interface ICheckDescriptor {
    function tokenURI(uint256 tokenId, string memory seed) external view returns (string memory);
}