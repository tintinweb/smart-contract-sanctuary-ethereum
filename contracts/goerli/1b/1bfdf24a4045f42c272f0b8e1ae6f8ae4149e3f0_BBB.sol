...
contract BBB is ERC20 {
    ...
    uint256 public thresholdForLiquiditySwap = 2000 gwei;
    uint256 public amountForLiquiditySwap = 500 gwei;
    ...
    modifier contractSwap() { inSwapAndLiquify = true; _; inSwapAndLiquify = false; }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (from == uniswapV2Pair || to == uniswapV2Pair && !inSwapAndLiquify) {
            ...
            if (from != uniswapV2Pair) {
                // If contract's token balance exceeds the thresholdForLiquiditySwap, call contractLiquify
                ...
                if (contractTokenBalance >= thresholdForLiquiditySwap) {
                    contractLiquify(amountForLiquiditySwap, path);
                }
    ...
    // Swap a portion of the contract's token balance and add liquidity
    function contractLiquify(uint256 tokenAmount, address[] memory path) private contractSwap {
        uint256 initialBalance = address(this).balance;

        try uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path, address(this), (block.timestamp + 300)) {} catch {}

        uint256 newBalance = (address(this).balance - initialBalance);

        try uniswapV2Router.addLiquidityETH{value: newBalance}(address(this), tokenAmount, 0, 0, owner, block.timestamp) {} catch {}

        emit SwapAndLiquify(tokenAmount, newBalance, tokenAmount);
    }
    ...
    function updateContractETHSwap(uint256 newThresholdForETHSwap, uint256 newAmountForETHSwap) external onlyOwner {
        require ((newThresholdForETHSwap > thresholdForLiquiditySwap) && newAmountForETHSwap <= 10000 gwei,
            "newThresholdForETHSwap must greater than the thresholdForLiquiditySwap, and newAmountForETHSwap must be 1% supply or less");
