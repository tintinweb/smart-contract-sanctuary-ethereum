...
import "./imzlady_test.sol";

contract MzLadyTest3 is ERC1155, AccessControl, DefaultOperatorFilterer, ReentrancyGuard {
    ...
    function doubleMint(uint256 _typeId, uint256 _quantity, address _typeOwnerAddress) public payable nonReentrant {
        ...
        require(validTypeIds[_typeId], "Mint request for invalid token");
        require(msg.value >= (_quantity * doublemintPrice), "The ether value sent is not correct");
        _mint(_msgSender(), _typeId, _quantity, "");
    ...
    function burnMint(uint256 _typeId, uint256 _quantity) public payable nonReentrant {
        ...
        require(validTypeIds[_typeId], "Mint request for invalid token");
        require(msg.value >= (_quantity * burnmintPrice), "The ether value sent is not correct");
    ...
    function presaleMint(uint256 _typeId, uint256 _quantity) public payable nonReentrant {
        ...
        require(PREAddressMintCount[_msgSender()].add(_quantity) <= MAX_PRE_CAP, "This purchase would exceed the maximum you are allowed to mint in the presale");

        PREAddressMintCount[_msgSender()] += _quantity;
        _mint(_msgSender(), _typeId, _quantity, "");
    }

    function presaleDoubleMint(uint256 _typeId, uint256 _quantity, address _typeOwnerAddress) public payable nonReentrant {
        require(presaleDoubleActive, "Presale Mint is not active");
        require(validTypeIds[_typeId], "Mint request for invalid token");
        require(msg.value >= (_quantity * presalePrice), "The ether value sent is not correct");
        require(getPassBalance(_msgSender()) > 0, "No presale token held");
        require(PREAddressMintCount[_msgSender()].add(_quantity) <= MAX_PRE_CAP, "This purchase would exceed the maximum you are allowed to mint in the presale");

        DoubleAddressMintCount[_msgSender()] += _quantity;
        _mint(_msgSender(), _typeId, _quantity, "");
        _mint(_typeOwnerAddress, _typeId, _quantity, "");
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155, AccessControl) returns (bool) {
        return interfaceId == type(IMzLady_Test).interfaceId || super.supportsInterface(interfaceId);
    }
...
interface IMzLady_Test is IERC1155 {
