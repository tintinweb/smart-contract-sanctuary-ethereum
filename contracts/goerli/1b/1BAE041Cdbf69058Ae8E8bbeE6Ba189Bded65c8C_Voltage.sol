...
contract Voltage is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {

    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter private _tokenIds;
    uint256 public constant MAX_SUPPLY = 10;//5K. 10 for test on goerli 
        ...
        returns (uint256)
    {
        ...
        require(
            ...
            "Sorry can't mint, we reached the cap"
        );
    ...
    function getHolders(uint256 startIndex, uint256 endIndex)
        public
        view
        returns (NftInfo[] memory)
    {
        uint256 totalSupply = totalSupply();
        require(startIndex < endIndex, "startIndex greater endIndex");
        require(endIndex <= totalSupply, "endIndex greater totalSupply");

        uint256 range = endIndex - startIndex;

        NftInfo[] memory results = new NftInfo[](range);

        for (uint256 resultIndex = 0; resultIndex < range; resultIndex++) {
            uint256 tokenIndex = startIndex + resultIndex;
