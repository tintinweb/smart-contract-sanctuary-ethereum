...
contract RoyaltySplitterProxyFactory is ERC2771ContextUpgradeable {
    ...
    function createProxyContract(
        ...
        GenericProxy newProxy = new GenericProxy(
            ...
            abi.encodeWithSelector(
                RoyaltySplitter(payable(address(uint160(_config.masterCopy)))).initialize.selector, 
                _config.shares,
                _config.receivers,
                _config.collection,
                _config.creator
            ),
            MinimalForwarder(_config.forwarder)
        );
        
        ERC721Collection(_config.collection).setRoyaltyAddress(address(newProxy));
...
contract RoyaltySplitter is Initializable, OwnableUpgradeable, ERC2771ContextUpgradeable
{
    ...
    ERC721URIStorageUpgradeable membershipToken;

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(MinimalForwarderUpgradeable _minimalForwarder) ERC2771ContextUpgradeable(address(_minimalForwarder)) { }

    function initialize(uint256[] memory shares_, address[] memory _receivers, address _owner)
        public initializer
    {
    ...
    //@follow-up Check with team to this requirement
    function updateMembershipToken(address membershipToken_) public onlyOwner {
...
contract CreatorDAO is Initializable, OwnableUpgradeable, ERC2771ContextUpgradeable  {
    ...
    function getSalesRevenue(address _collection) public view returns (uint256) {
        return ERC721Collection(_collection).getSalesRevenue(); 
    } 
