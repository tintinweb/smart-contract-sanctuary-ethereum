...
contract AuctionCreator{
    ...
    function CreateAuction(uint finalPrice, uint minPrice) public{
        require(minPrice <= finalPrice);
        Auction newAuction = new Auction(finalPrice, minPrice, msg.sender);
...
contract Auction{
    ...
    constructor(uint _finalPrice, uint _minPrice, address eoa){
    ...
    // bidinimo funkcija
    function placeBid(address _sender) public payable notOwner afterStart{
        require(msg.value >= bidIncrement, "Bid should be atleast 100 wei");
        require(msg.value <= finalPrice, "Bid should be not more than max Price");
        require(msg.value % bidIncrement == 0, "Bid should be divdeable by 100 without fraction");
