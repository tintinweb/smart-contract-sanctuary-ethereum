...
contract Claim is AccessControl {
    ...
    event AirdropClaimed(address _user, address _token, uint _amount);
    ...
    constructor(address _admin, ClaimSettings memory _claimSettings) {
        claimSettings = _claimSettings;
        claimSettings.startTime = _claimSettings.startTime + block.timestamp;
        claimSettings.endTime = _claimSettings.endTime + block.timestamp;
        claimSettings.cooldownDetails.cooldownPeriod =
            _claimSettings.cooldownDetails.cooldownPeriod +
            block.timestamp;
    ...
    function claim(uint _amount, bytes32[] calldata _merkleProof) external {
        ...
        uint _claimAmount = checkAmount(_amount);
        ...
        if (uint(claimSettings.permission) == 0) {
            ...
            bytes memory encodedData = encode(msg.sender, _amount);
    ...
    function checkAmount(
        uint _amount
    ) private view returns (uint _claimAmount) {
        if (claimSettings.claimAmountDetails.isMaxClaimable) {
            _claimAmount = claimSettings.claimAmountDetails.maxClaimable;
            require(_amount <= _claimAmount, "Max claim amount");
        } else {
            //pro-rata based calculations
            if (claimSettings.isNFT) {
                _claimAmount =
                    (claimSettings.claimAmountDetails.maxClaimable *
                        IERC721(claimSettings.airdropToken).balanceOf(
                            msg.sender
                        )) /
                ...
                _claimAmount =
                    (claimSettings.claimAmountDetails.maxClaimable *
                        IERC20(claimSettings.airdropToken).balanceOf(
                            msg.sender
                        )) /
                    IERC20(claimSettings.airdropToken).totalSupply();
            }
        }
    }
    ...
    function pendingClaim(address _user) external {
        if (claimSettings.cooldownDetails.hasCooldownPeriod) {
            require(
    ...
    function airdropTokens(address _user) private {
        uint _claimAmount = claimAmount[_user];
        if (claimSettings.isNFT) {
            ...
            claimAmount[_user] -= _claimAmount;
        }
    }

    function rollbackTokens() external onlyAdmin {
        if (claimSettings.isNFT) {
            for (
                uint i;
                i < claimSettings.claimAmountDetails.tokenIds.length;

            ) {
                IERC721(claimSettings.airdropToken).safeTransferFrom(
                    address(this),
                    claimSettings.rollbackAddress,
                    claimSettings.claimAmountDetails.tokenIds[i]
                );
    ...
    function toggleClaim() external onlyAdmin {
    ...
    function changeRollbackAddress(address _newAddress) external onlyAdmin {
    ...
    function encode(
        ...
        return abi.encode(_userAddress, _amountOrTokenId);
    }
}
...
struct ClaimAmountDetails {
    ...
    uint maxClaimable; //Maximum claimable amount
