 ...
 */
interface ERC721 {
    function claimToken(uint256[] calldata tokenId, address _to) external;
}
...
contract VeraxERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    uint256 public wlSalePrice = 100000000000000;
    uint256 public publicSalePrice = 100000000000000;

    bytes32 public wlMerkleRoot;
    string public baseURI;
    uint256 private teamLength;

    ERC721 public veraxClaimContract;
    ...
    ) ERC721A("Verax", "VRX") PaymentSplitter(_team, _teamShares) {
    ...
    function burn(uint256[] calldata tokenId) external payable callerIsUser {
        for (uint256 i = 0; i <= tokenId.length; ++i) {
            require(
                msg.sender == ownerOf(tokenId[i]),
                "Message sender must be the Verax token owner."
            );
            _burn(tokenId[i]);
        }
        veraxClaimContract.claimToken(tokenId, msg.sender);
    }
    ...
    function whitelistMint(
        ...
        require(
            ...
            "Max supply exceeded for WL Mint"
        );
    ...
    function setClaimContract(ERC721 _claimContract) external onlyOwner {
        veraxClaimContract = _claimContract;
    }
