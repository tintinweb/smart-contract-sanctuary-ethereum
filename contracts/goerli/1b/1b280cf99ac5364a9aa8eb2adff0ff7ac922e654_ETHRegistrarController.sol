    ...
    ERC20Recoverable
{
    ...
    function register2(
        RegisterData calldata resData
    ) public payable override{
         IPriceOracle.Price memory price = rentPrice(resData.name, resData.duration);
        if (msg.value < price.base + price.premium) {
            revert InsufficientValue();
        }

        // require(inviteCode.strlen() <= 10);
        ...
        _consumeCommitment(
            ...
            makeCommitment(
                ...
                resData.resolver,
                resData.data,
                resData.reverseRecord,
                resData.fuses,
                resData.wrapperExpiry
            )
        );
        ...
        if (resData.data.length > 0) {
            _setRecords(resData.resolver, keccak256(bytes(resData.name)), resData.data);
        }

        if (resData.reverseRecord) {
            _setReverseRecord(resData.name, resData.resolver, msg.sender);
        }

        emit NameRegistered(
            resData.name,
            keccak256(bytes(resData.name)),
            resData.owner,
            price.base,
            price.premium,
            expires,
