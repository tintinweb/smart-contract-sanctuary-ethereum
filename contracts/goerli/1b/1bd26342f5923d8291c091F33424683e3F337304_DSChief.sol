...
//add interface NFTFactory
interface NFTFactory{
    function isFromFactory(address spell) external view returns(bool);
}
...
contract DSChief is DSRoles, DSChiefApprovals {
    ...
    constructor(DSToken GOV, DSToken IOU, uint MAX_YAYS, address NFT_Factory)
    DSChiefApprovals (GOV, IOU, MAX_YAYS)
    public
    {
        authority = this;
        owner = address(0);
        NFTF = NFT_Factory; //set NFTFactory addreee
    }
    ...
    returns (bool)
    {
        //add nft spell cancall role if it from NFTFactory
        return (live && who == hat || NFTFactory(NFTF).isFromFactory(who));
    }
...
contract DSChiefFab {
    //add arg NFTFactory
    function newChief(DSToken gov, uint MAX_YAYS, address NFT_Factory) public returns (DSChief chief) {
        DSToken iou = new DSToken('IOU');
        //add arg NFTFactory
        chief = new DSChief(gov, iou, MAX_YAYS, NFT_Factory);
