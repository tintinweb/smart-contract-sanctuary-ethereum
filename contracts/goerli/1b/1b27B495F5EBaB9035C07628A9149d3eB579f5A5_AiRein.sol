...
contract AiRein is ERC721A, Ownable, ERC2981, ReentrancyGuard {
    ...
    bytes32 public merkleRoot = 0x4169f1e5801db98094c8a6209165869abb575c8c9524f5de3b6aafb8819789b3;
    ...
    function freeMint(uint256 qty, address recipient) external onlyAdmin {
        if (_totalMinted() + qty > supply) revert SoldOut();
        _safeMint(recipient, qty);
    }

    function setPayeeAddress(address _payeeAddress) external onlyAdmin {
        require(_payeeAddress != address(0), "Payee address cannot be zero");
        payeeAddress = _payeeAddress;
    }
