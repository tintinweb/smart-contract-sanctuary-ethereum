...
struct lendingAccepted {
  ...
  address borrowedToken;
  uint[] bothSides; // [0] == debtor || [1] == borrower
...
uint[] memory wallets = new uint[](2);
wallets[0] = nftsID - 2;
wallets[1] = nftsID - 1;
uint calculatedDeadLine = lendingO.timelap_O * lendingO.paymentCount_O;
uint basePayment = lendingO.amountForBorrow / lendingO.paymentCount_O;
uint paymentEachTime = ((100 + lendingO.interest_O) * basePayment) / 100;
uint totalPayment = lendingO.paymentCount_O * paymentEachTime;
...
function payDebt(uint lendingOfferID) public payable {
...
lendingByID[lendingOfferID].debt -= m_lendingAccepted.paymentPerTime;
  lendingByID[lendingOfferID].deadlineNextPayment = m_lendingAccepted.deadlineNextPayment + m_lendingAccepted.timelap ; 
lendingByID[lendingOfferID].paymentsLeft--; 

if(lendingByID[lendingOfferID].debt == 0) {
  delete lendingByID[lendingOfferID].collateral;
  (bool success, ) = msg.sender.call{value: m_lendingAccepted.collateral}("");
  if(!success) {
    revert();
  }
} 
