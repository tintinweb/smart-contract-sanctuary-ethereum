...
contract PredefinedVoting is VotingFactory {
    event PredefinedVotingCreated(
    ...
    PredefinedVoting public commissionVoting =
        PredefinedVoting(address(0), address(0), VotingResultType.IntType);

    PredefinedVoting public someStrVoting =
        PredefinedVoting(address(0), address(0), VotingResultType.StrType);

    // Predefined Commission Votings
    function createCommissionVoting(
        VotingLibrary.BaseVotingState calldata votingConfig,
        uint256[] calldata options
    ) external returns (address addr) {
        addr = createIntTypeVoting(votingConfig, options);
        commissionVoting.previous = commissionVoting.current;
        commissionVoting.current = addr;
        emit PredefinedVotingCreated(
            addr,
            msg.sender,
            "CommissionVoting",
    ...
    function getElectedCommission() external view returns (uint256) {
        return getIntTypeResult(getCurrentValidVotingAddress(commissionVoting));
    }

    // Predefined SomeStr Votings
    function createSomeStrVoting(
        VotingLibrary.BaseVotingState calldata votingConfig,
        string[] calldata options
    ) external returns (address addr) {
        addr = createStrTypeVoting(votingConfig, options);
        someStrVoting.previous = someStrVoting.current;
        someStrVoting.current = addr;
        emit PredefinedVotingCreated(
            addr,
            msg.sender,
            "SomeStrVoting",
    ...
    function getSomeStrResult() external view returns (string memory) {
        return getStrTypeResult(getCurrentValidVotingAddress(someStrVoting));
    }
}
