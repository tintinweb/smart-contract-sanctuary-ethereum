...
contract HyperMintERC721A is ERC721ABurnable, Ownable {
    ...
    function buy(uint256 _amount) external payable buyAllowed nonContract {
        if (block.timestamp < publicSaleDate) revert PublicSaleClosed();
    ...
    // ============= Ownership =============
    function transferContractOwnership() external {
        if (msg.sender != addresses.customerAddress) revert NotAuthorised();
...
abstract contract Ownable is Context {
    ...
    address internal _collectionOwner;
    ...
    /**
     * @dev Returns the Collection Owner
