    ...
    AccessControlUpgradeable
{
    ...
    function getActiveListings(
        ...
        IERC721Enumerable tokenEnumerable = IERC721Enumerable(tokenAddress);

        uint256 totalSupply = tokenEnumerable.totalSupply();
        ...
        ListingView[] memory activeListings = new ListingView[](
            totalActiveListings
        );
        uint256 counter = 0;
        for (uint256 i = 0; i <= tokenEnumerable.totalSupply(); i++) {
            Listing memory listing = _listings[dooplicationAddress][
                tokenAddress
            ][i];
            if (listing.price > 0) {
                activeListings[counter] = ListingView(i, listing);
                counter++;
            }
    ...
    function testView() external view returns (uint256) {
        return 1;
    }
}
