...
contract AuthHelper is Ownable {
    ...
    function isMetaChainpassWallet(MetaTransactionInput memory metaTXInput) public view returns (bool) {
    ...
    function isMetaOwner(MetaTransactionInput memory metaTXInput) public view returns (bool) {
    ...
    function isSignerOrCaller(address signer, MetaTransactionInput memory input) public view returns (bool) {
        return msg.sender == signer || isSigner(signer, input);
    }

    function isSigner(address signer, MetaTransactionInput memory input) public view returns (bool) {
        require(!usedNoncesBySignerAddress[signer][input.nonce], "nonce already used");

        bytes32 msgHash = keccak256(abi.encodePacked(address(this), signer, input.nonce));
        return verifySignature(msgHash, input.v, input.r, input.s) == signer;
    }

    function verifySignature(bytes32 messageHash, uint8 v, bytes32 r, bytes32 s) private pure returns (address) {
