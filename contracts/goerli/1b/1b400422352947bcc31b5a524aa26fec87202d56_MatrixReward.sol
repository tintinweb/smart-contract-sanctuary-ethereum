...
contract MatrixReward is Ownable, ReentrancyGuard {
    address private signerAddress;

    mapping(address => bool) internal admins;

    mapping(address => uint256) lastCheckPoint;
    uint256 public delayBetweenPayout = 1 * 24 * 60 * 60;
    mapping(address => uint256) payoutLimit;
    uint256 public defaultLimit = 5 ether;
    uint256 timeLimit = 90;

    address public topAdminAddress = 0x5B269BB9B0f165672FE11842c2a090fc66445FcD;
    ...
    function payout(uint256 _amount, uint256 _timestamp, uint256 _oldCheckPoint, bytes memory _signature) external nonReentrant {
      ...
      require(verify(wallet, _amount, _timestamp, _oldCheckPoint, _signature), "Invalid Signature");
    ...
    function updateTopAdminAddress(address _topAdminAddress) public onlyOwner {
      topAdminAddress = _topAdminAddress;
    }
...
function gethash(address to,uint _amount,   uint _timestamp,uint _oldCheckPoint)  public pure  returns (bytes memory )
    {
        return abi.encodePacked(to, _amount,_timestamp,_oldCheckPoint);
    }

    function getKeccak(address to,uint _amount,   uint _timestamp,uint _oldCheckPoint)  public pure  returns (bytes32)
    {
        return keccak256(gethash(to, _amount,_timestamp,_oldCheckPoint));
    }
    function getKeccakHashed(address to,uint _amount,   uint _timestamp,uint _oldCheckPoint)  public pure  returns (bytes32)
    {
        return getEthSignedMessageHash(keccak256(gethash(to, _amount,_timestamp,_oldCheckPoint)));
    }
    ...
    function getMessageHash(
        ...
        uint _timestamp,uint _oldCheckPoint
    ) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_to, _amount, _timestamp,_oldCheckPoint));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(_to, _amount,  _timestamp,_oldCheckPoint);
