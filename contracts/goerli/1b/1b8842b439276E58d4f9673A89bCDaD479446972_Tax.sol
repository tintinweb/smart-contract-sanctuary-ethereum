...
contract Tax is Context, IERC20, Ownable {
    ...
    string private constant _name = "Tax";
    string private constant _symbol = "STV1";
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1000000 * 10**9; // 1M
    uint256 private maxWalletAmount = 20000 * 10**9; 
    uint256 private maxTxAmount = 40000 * 10**9; 
    uint256 private swapAtAmount = 1000 * 10**9;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 contractTokenBalance = balanceOf(address(this));
            if (!inSwap && from != uniswapV2Pair && swapEnabled && contractTokenBalance>swapAtAmount && _reduceTaxCountdown<=_preventSwapBefore) {
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        if(from == uniswapV2Pair) {  
            ...
            if(_reduceTaxCountdown==0){
                tokens = amount * _finalBuyTax / masterTaxDivisor;
                }   
                else{
                    tokens = amount * _initialBuyTax / masterTaxDivisor; 
                }          
         ...
         //sell      
            ...
            if(_reduceTaxCountdown==0){
                tokens = amount * _finalSellTax / masterTaxDivisor; 
                }   
                else{
                    tokens = amount * _initialSellTax / masterTaxDivisor; 
                }   
        }   

        _tOwned[address(this)] += tokens;
        emit Transfer(from, address(this), tokens);

        uint256 feeAmount = tokens;
