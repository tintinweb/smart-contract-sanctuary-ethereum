...
contract VortexFoundersPacks is Ownable {
    ...
    function setVortexPackContract(VortexPacks _packContract) external onlyOwner {
        packContract = _packContract;
    }
    ...
    function getVortexPointData(address[] memory possibleMinters) public view returns (uint256[] memory actualMints) {
        actualMints = new uint256[](possibleMinters.length);
        
        for(uint i; i < possibleMinters.length; i++) {
    ...
    ERC2981
{
    ...
    uint256 public constant MINTER = 1 << 1; 
