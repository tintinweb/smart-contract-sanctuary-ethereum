...
contract ApeCoinStaking is Ownable {
    ...
    /**
     * @dev Add single time range with a given rewards per hour for a given pool
     * In practice one Time Range will represent one quarter (defined by `_startTimestamp`and `_endTimeStamp` as whole hours)
    ...
    function rewardsBy(uint256 _poolId, uint256 _from, uint256 _to) public view returns (uint256, uint256) {
        ...
        uint256 rewards;
        for(uint256 i = pool.lastRewardsRangeIndex; i < pool.timeRanges.length; ++i) {
    ...
    /**
     * @dev Returns relevant values for all the Pools for UI purposes.
     */
    ...
    function _claimPairNft(uint256 mainTypePoolId, PairNft[] calldata _pairs, address _recipient) private {
        for(uint i; i < _pairs.length; ++i) {
            ...
            require(mainToBakc[mainTypePoolId][mainTokenId].tokenId == bakcTokenId
                && bakcToMain[bakcTokenId][mainTypePoolId].tokenId == mainTokenId, "The provided Token IDs are not paired");
