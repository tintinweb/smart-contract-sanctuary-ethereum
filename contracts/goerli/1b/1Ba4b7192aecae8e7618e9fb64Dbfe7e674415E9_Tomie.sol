...
contract Tomie is ERC721, ERC721Enumerable, ERC2981, ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => string[]) private kinds;

    uint256 private regenerateTime = 5 days;
    mapping(uint256 => uint256) private birthTimes;
    mapping(uint256 => uint256) private killTimes;

    uint256 public killCost = 0.005 ether;
    uint256 public freeKill = 1;
    uint256 public killLimit = 5;
    ...
    constructor() ERC721("LE Test", "LE Test") {
        // begininng(_fascinatedPeople);
        _setDefaultRoyalty(msg.sender, royaltyFee);
    }

    function begininng(address[] memory _fascinatedPeople) public onlyOwner {
        require(
            _tokenIdCounter.current() + _fascinatedPeople.length <= firstBorn,
            "Can't birth any more"
        );
        for (uint256 i = 0; i < _fascinatedPeople.length; i++) {
            _tokenIdCounter.increment();
            uint256 tokenId = _tokenIdCounter.current();
            birthTimes[tokenId] = 1;
            _safeMint(_fascinatedPeople[i], tokenId);
        }
    }

    function setImageAndKinds(
        uint256 phase,
        string[] memory newImageURIs,
        string[] memory newKinds
    ) external onlyOwner {
        require(newImageURIs.length == newKinds.length, "Array size mismatch.");
        setImageURIs(phase, newImageURIs);
        setKinds(phase, newKinds);
    }

    function setImageURIs(uint256 phase, string[] memory newImageURIs)
        public
        onlyOwner
    {
    ...
    function setKinds(uint256 phase, string[] memory newKinds)
        public
        onlyOwner
    {
        kinds[phase] = newKinds;
    }
    ...
    function setKillLimit(uint256 newLimit) external onlyOwner {
        killLimit = newLimit;
    }
    ...
    function kill(uint256 tokenId) public payable {
        require(
        ...
        require(
            killedTokenIds[msg.sender].length < killLimit,
            "You can't kill any more."
        );
        require(
            killedTokenIds[msg.sender].length < freeKill ||
                msg.value == killCost,
            "You can't kill her without paying the price."
        );
        ...
        birthTimes[_tokenIdCounter.current()] = block.timestamp;
    }

    function getOwnTokenIds(address walletAddress)
        public
        view
        returns (uint256[] memory)
    {
        uint256 balance = balanceOf(walletAddress);
        uint256[] memory ownTokenList = new uint256[](balance);

        for (uint256 i = 0; i < balance; i++) {
            ownTokenList[i] = tokenOfOwnerByIndex(walletAddress, i);
        }

        return ownTokenList;
    }

    function getKilledTokenIds(address walletAddress)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getRegenerateLevel(uint256 tokenId) public view returns (uint256) {
        require(tokenId <= _tokenIdCounter.current(), "Not yet Birth");

        // if (tokenId <= firstBorn) {
    ...
    function _getImageIndex(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 random = uint256(
            keccak256(
                abi.encodePacked(tokenId, birthTimes[tokenId], regenerateLevel)
            )
        );
        // uint256 imageIndex = (tokenId + birthTimes[tokenId]) % imageNum;
        ...
        returns (string memory)
    {
        ...
        bytes memory attrs = abi.encodePacked(
            '"attributes":[{"trait_type":"regenerateLevel","value":"',
            level.toString(),
