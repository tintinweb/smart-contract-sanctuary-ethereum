...
// https://contract-metadata.s3.amazonaws.com/v1/NickGonzalez4__.json
// 200000000000000000
// 1663196020
// 1663196020

// 0x3E1d10CF0af0386f03faA1Db1b4e62A6752795D0 Expert Address
...
// - Modifiers
// - functions
// - constructor (not one here because of Initializer)
...
contract TwaliContract is Initializable, ReentrancyGuard {
    ...
    bytes32 public constant WORK_DETAILS_TYPEHASH = keccak256(
        "WorkDetails(address owner,string contract_sowMetaDataUrl,uint contract_payment_amount,uint contract_start_date,uint contract_end_date,uint contract_created_on,bool contract_workIsApproved,bool contract_workIsPaidOut,bool contract_workIsRefunded)"
         );

    bytes32 public constant EXPERT_TYEHASH = keccak256("WerkExpert(address contract_expert)");
    ...
    event contractSafeCreate(address, address);
    event DepoistedExpertPaynment(address, uint);
    event ownerRefunded(address, uint);
    event expertSetToContract(address, bool);
    event expertPaymentSuccess(address, uint);
    event refundTransferred(address, uint);
    ...
    function initialize(
        ...
        require(workdetails[_adminClient][0].owner == address(0), "Contract is already initialized");

        workdetails[_adminClient][0] = WorkDetails(_adminClient, _sowMetaDataUrl, _contract_payment_amount, _contract_start_date, _contract_end_date, _contract_created_on, false, false, false);
        isInitialized = true;
    ...
    */ 
    ...
    modifier onlyOwner() {
        require(
            msg.sender == workdetails[msg.sender][0].owner, 
    ...
    modifier isExpert() {
        require(msg.sender == werkexpert.contract_expert, "Not contract expert address");
        _;
    }
    ...
    function getWorkDetails(address _owner, uint256 index) public view returns (WorkDetails memory werkdetail) {
        werkdetail = workdetails[_owner][index];
        return werkdetail;
    }
...
// TODO: [] - Convert to read from WorkDetails Struct contract amount
    ...
    function addExpertDelegate(address _contract_expert) 
        ...
        // Do a check that the address expert is not set yet
        // Do a check that the address isn't the owner address.
        werkexpert.contract_expert = _contract_expert;
    }


    function transferTakeRate(GnosisSafe safe, address payable to, uint amount) external onlyOwner {
        // uint contract_amount = werkdetails[msg.sender][0].contract_payment_amount;
        // // Was giving an error for `memory` still. Trying without since it references a storage slot for the struct.
        // uint twaliTake = contract_amount * contract_takeRate;
    ...
    function transferExpertPayment(
        ...
        emit expertPaymentSuccess(to, amount);
    }
    ...
    { 
        ...
        werkexpert.expertConfirmation = true;

        emit expertSetToContract(_contract_expert, true);
    }
...
//   gnosisSafeProxyFactory: '0xa6B71E26C5e0845f74c812102Ca7114b6a896AB2',
        ...
        uint safeAmount = workdetails[msg.sender][0].contract_payment_amount;
        address[] memory safeOwner = new address[](1);
        safeOwner[0] = address(_newSafeOwner);

        // setup function outline here: https://github.com/safe-global/safe-contracts/blob/c36bcab46578a442862d043e12a83fec41143dec/contracts/GnosisSafe.sol#L75
        _safe.setup(
        ...
        emit contractSafeCreate(msg.sender, address(_safe));
    ...
    //    require(signer == werkexpert.contract_expert);
    ...
    /// 
    function approveWorkSubmitted(GnosisSafe safe,address payable to) 
        public 
        onlyOwner
        // werkNotApproved
        // werkNotPaid
        ...
        setNextStage 
    {
        ...
        transferExpertPayment(safe, to, contract_amount);           
