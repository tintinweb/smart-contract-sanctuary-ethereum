...
contract MYNT is Ownable, ERC721A {
    ...
    constructor( ) ERC721A("Bolao Mynt", "Bolao Mynt") {
    }
    ...
    function giveaway(address to, uint256[] calldata ids, uint256 quantity) external onlySender {
        require(totalSupply() + quantity <= maxSupply, "This amount of tokens would surpass the max supply.");

        for (uint256 index = 0; index < quantity; index++) {
            tokenIdsUris[totalSupply() + index] = ids[index];
        }
