...
contract ForjSoulbound is ReentrancyGuard, Ownable {
    using Strings for uint256;
    ...
    // 'from' will always be address(0). 
    // Only emitted on mint
    ...
    // max supply settings
    function changeMaxSupply(uint256 supply) public onlyOwner {
        maxSupply = supply;
    }
 
    function changeCollectionToken(address _addr) external onlyMultiSig(msg.sender) {
    ...
    function testMint() public {
        _mint(msg.sender, 1);
        mintedPerAddress[msg.sender] = mintedPerAddress[msg.sender] + 1;
        ...
        onlyStarted
    {
        ...
        require(
            ...
            "Exception: Reached the limit for each user. You can't mint any more"
        );
        require(
            mintedAmount < maxSupply,
            "Exception: Reached the minting limit for the contract. Can't mint any more"
        );
