...
abstract contract MultiStage {
    ...
    // quantity -> price
    mapping(uint256 => mapping(uint256 => uint256)) internal _discount;
    ...
    function discountAt(uint256 stageId, uint256 quantity)
        external
        view
        returns (uint256)
    {
        return _discount[stageId][quantity];
    }
    ...
    function _createSale(
        ...
        for (uint256 i = 0; i < length; i++) {
            _discount[stageId][discountQuantities[i]] = discountPrices[i];
        }
    ...
    OperatorFilterer
{
    ...
    function mint(uint256 stageId, uint64 quantity)
        external
        payable
        onlyMintable(stageId, quantity)
    {
        require(!_presaleActive[stageId], "Presale active");

        _purchaseMint(stageId, _msgSender(), quantity);
    }
    ...
    function presaleMint(
        ...
        uint256 mintQuantity = _mintCount[stageId][_msgSender()].add(quantity);
        require(_presaleActive[stageId], "Presale not active");
    ...
    function _purchaseMint(
        ...
        uint256 discountPrice = _discount[stageId][quantity];

        if (discountPrice > 0) {
            require(
                discountPrice.mul(quantity) == msg.value,
                "Discount price mismatch"
            );
        } else {
            require(
