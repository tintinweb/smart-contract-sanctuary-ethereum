...
library Counters {
    ...
    function increment(Counter storage counter, uint256 nCount) internal {
        unchecked {
            counter._value += nCount;
        }
    }

    function decrement(Counter storage counter, uint256 nCount) internal {
        uint256 value = counter._value;
        require(value > 0, "Counter: decrement overflow");
        unchecked {
            counter._value = value - nCount;
        }
    }
...
contract NOWNFTTest is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("NOWNFT", "NN") {
        presaleStartTime = 1666311573;
    ...
    function preSaleMint(bytes32[] calldata _proof, uint256 nMints) external payable isValidMerkleProof(_proof, merkleRoot) nonReentrant {
        ...
        require((presaleStartTime + presaleDuration) >= block.timestamp, "Presale not active");
    ...
    function isPreSaleActive() public view returns(bool) {
        return (block.timestamp >= presaleStartTime) && ((presaleStartTime + presaleDuration) >= block.timestamp);
    }
