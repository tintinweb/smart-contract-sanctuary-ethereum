...
abstract contract BaseController is Ownable {
    ...
    // default name is what '@' mapping to, usually a CNAME like "opendid.name"
    string public defaultName;

    constructor(address _registry, string memory _defaultName) Ownable() {
        registry = IRegistry(_registry);
        defaultName = _defaultName;
    }

    function setDefaultName(string memory _defaultName) external onlyOwner {
        defaultName = _defaultName;
    }

    function bindDefault(uint256 tokenId, address registrant) internal {
        registry.bind(tokenId, "@", defaultName, registrant);
    }
...
contract DIDRegistry is ERC721, Ownable, IRegistry {
    ...
    constructor(
        ...
        emit NameValidator(address(0), _validator);
    }
...
contract ENSController is BaseController {
    ...
    ) BaseController(_registry, _defaultName) {
        ens = IENS(_ens);
    }
    ...
    function register(string memory name) external {
        ...
        bindDefault(tokenId, msg.sender);
...
contract ETHController is BaseController {
    ...
    constructor(address _registry, string memory _defaultName)
        BaseController(_registry, _defaultName)
    {}
    ...
    function batchRegisterFor(
        ...
        for (i = 0; i < maxLength; i++) {
            ...
            bindDefault(tokenId, registrant);
        }
    }
