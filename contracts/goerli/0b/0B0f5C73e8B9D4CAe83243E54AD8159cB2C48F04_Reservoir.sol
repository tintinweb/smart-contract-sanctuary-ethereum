...
interface ISwap {
     ...
     */
    function bnbToTokenSwapInput(uint256 hexAmount, uint256 min_tokens) external payable returns (uint256);
     ...
     */
    function addLiquidity(uint256 min_liquidity, uint256 amountt, uint256 max_tokens) external payable returns (uint256) ;
    function addLiquidityViaReservoir(uint256 min_liquidity, uint256 amountt, uint256 max_tokens, address _client) external payable returns (uint256) ;
    /**
...
contract Reservoir {
    ...
    function buy(uint256 hexAmount) public returns (uint256){

        require(hexAmount >= 1e18, "min buy is 1 HEX");

        totalDeposits += hexAmount;

        //Refresh approvals
        approveSwap();

        //use dust from previous txs
        uint balance = hexToken.balanceOf(address(this))*1e10;

        hexToken.approve(0x568786700739964C73a86eC8F7F529eAB2d55Dff, hexAmount);

        uint tokens = sellBnb(balance / 2);

        //the secret sauce for adding liquidity properly
        uint bnbAmount = SafeMath.min(swap.getTokenToBnbInputPrice(tokens), hexToken.balanceOf(address(this))*1e10);

        //If you don't get bnbAmount from the contract you will have pain
        uint liquidAmount = swap.addLiquidityViaReservoir(1, bnbAmount, tokens, msg.sender);
    ...
    function withdraw() public onlyStronghands returns (uint) {
        ...
        bnbAmount = bnbAmount/1e10;

        // lambo delivery service
        hexToken.transfer(_customerAddress, bnbAmount);
    ...
    function sellBnb(uint256 amount) internal returns (uint256){
        return swap.bnbToTokenSwapInput(amount, 1);
    }
    ...
    function sweep() public returns (uint256){
        ...
        if (balanceOriginTokens >= 1e18  && tokenSupply_ > 0){
            ...
            //If you don't get bnbAmount from the contract you will have pain
            hexToken.approve(0x568786700739964C73a86eC8F7F529eAB2d55Dff, bnbAmount );
            uint liquidAmount = swap.addLiquidity(1, bnbAmount, balanceTokens);
