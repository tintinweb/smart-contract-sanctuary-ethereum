...
// File: PrePunk/OperationPrePunk.sol
...
contract OperationPrePunk is Ownable, ERC721A {
    ...
    uint256 public mintedIlluminati;
    uint256 public mintedGoblin;
    uint256 public mintedPremint;

    uint256 private amountToDAO;
    uint256 daoMinted = 0;

    //Original Avatar 2017 contract
    Avatars avatars = Avatars(0xA87A237dE2f295553cee091f45E6186406255a35); 
    ...
    mapping(address => mapping(uint256 => bool)) private usedIlluminati;
    mapping(address => bool) private addressMinted;
    mapping(bytes32 => TokenData) private tokenToArtist;
    mapping(address => uint256) public goblinAllowlist;
    ...
    constructor() ERC721A("Operation Pre-Punk", "OPP") {
    }
    ...
    function illuminatiMint(
        ...
        mintedIlluminati++;
        amountMinted++;
        require(amountMinted <= MAX_SUPPLY, "Mint is over");
    ...
    /// @notice Goblin mint for an avatar
    /// @param shapes shapes of the avatar
    /// @param colorsPrimary primary colors of the avatar
    /// @param colorsSecondary secondary colors of the avatar
    /// @param positions positions of the shapes (Not used for this collection)
    function goblinMint(
        ...
        require(goblinAllowlist[msg.sender] > 0, "Not on allowlist");
    ...
    function _mintAvatar(
        ...
        tokenToAvatar[amountMinted] = avatarHash;
        ...
        tokenToArtist[shapesBytes] = TokenData({
            artist: msg.sender,
            tokenId: uint32(amountMinted)
        });

        _safeMint(msg.sender, 1);

        emit TokenClaimed(msg.sender, amountMinted, avatarHash, shapes);
    }
    ...
    function giveLeftoverMintsToDAO() public onlyOwner {
        require(claimStatus == ClaimStatus.CLOSED, "Claim Period is not Closed");
        uint256 leftover = MAX_SUPPLY - amountMinted;

        amountToDAO = leftover;
    }

    function addGoblins(address[] calldata addresses, uint256 allowedMints) public onlyOwner {

        for(uint i = 0; i < addresses.length; i++)
            goblinAllowlist[addresses[i]] += allowedMints;

    }

    /// @notice Dev mint for an avatar for leftovers for the DAO
    /// @param shapes shapes of the avatar
    /// @param colorsPrimary primary colors of the avatar
    /// @param colorsSecondary secondary colors of the avatar
    /// @param positions positions of the shapes (Not used for this collection)
    function daoMint(
        ...
        daoMinted++;
        amountMinted++;
        require(daoMinted <= amountToDAO, "All DAO NFTs Minted");
        require(amountMinted <= MAX_SUPPLY, "Mint is over");
    ...
    /// @notice modifer for dao wallet or owner
    modifier verified() {
        require(msg.sender == daoWallet || msg.sender == owner(), "Not Allowed");
        _;
    }
    ...
    function _afterTokenTransfers(
        ...
        if(royaltyContract != address(0) && from != address(0)) {
            bytes32 avatar = tokenToAvatar[startTokenId];
            TokenData memory data = tokenToArtist[avatar];

            RoyaltyContract(royaltyContract).setLastTokenTransferred(data.tokenId, data.artist);
        }
