...
contract SxTCommunityUpgradeable is ISxTCommunityUpgradeable, SxTCommunityStorageUpgradeable, ERC1155SupplyUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function addMultipleSxtNFT(string [] memory newTokenURIs, uint256 [] memory maxNewTokenSupplies) external override onlyOwner {
        ...
        uint256 [] memory nftTokensAdded = new uint256 [](maxNewTokenSupplies.length);
        for(uint256 index = 0; index < maxNewTokenSupplies.length; index++){
            ...
            SxtNFT memory newToken = addNewSxtNFT(newTokenURIs[index], maxNewTokenSupplies[index]);
    ...
    function addSxtNFT(string memory newTokenURI, uint256 maxNewTokenSupply) external override onlyOwner {
        ...
        SxtNFT memory newToken = addNewSxtNFT(newTokenURI, maxNewTokenSupply);
        emit SxtNFTAdded(newToken.id);
    }

    function addNewSxtNFT(string memory newTokenURI, uint256 maxNewTokenSupply) internal returns(SxtNFT memory nft){
    ...
    function airdropMultipleTokens(uint256 id, address[] memory recipients, uint256[] memory quantities) external override onlyOwner whenNotPaused nonReentrant{
    ...
    function airdrop(uint256 id, address[] memory recipients) external override onlyOwner whenNotPaused nonReentrant{
        require(!compareStrings(tokenDetails[id].tokenUri, ""), "SxTCommunityUpgradeable: URI nonexistent token");
...
interface ISxTCommunityUpgradeable {
    ...
    /// @dev This is the event to notify that multiple new NFT token have been added in contract
    /// @param id Array of IDs of NFT tokens added
