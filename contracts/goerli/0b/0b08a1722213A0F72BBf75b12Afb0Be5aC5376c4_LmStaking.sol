...
contract LmStaking is Initializable, OwnableUpgradeable, IERC721ReceiverUpgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    */
    ...
    function deposit(
        ...
        (, , _token0, _token1, , , , _amount, , , , ) = poolInfo[_pid].lpToken.positions(
            _tokenId
        );
        require(lpTokensStatus[_token0][_token1], "LP token not added");
        require(lpTokensStatus[_token0][_token1], "LP token not added");
                require(
            (_token0 == poolInfo[_pid].token0 && _token1 == poolInfo[_pid].token1) ||
                (_token0 == poolInfo[_pid].token1 && _token1 == poolInfo[_pid].token0),
        ...
        _deposit(_pid, _amount, _vault, msg.sender, _tokenId);
    }
    ...
    function depositFromOtherContract(
        ...
        _deposit(_pid, _amount, _vault, _userAddress, _tokenId);
    }
    ...
    function _deposit(
        ...
        updateUserDepositDetails(_pid, _userAddress, _amount, _vault, _tokenId);
    ...
    */
    ...
    function emergencyWithdraw(uint256 _pid, uint256 _tokenId) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        pool.lpToken.safeTransferFrom(address(this) ,address(msg.sender), _tokenId);
