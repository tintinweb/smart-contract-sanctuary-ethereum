...
contract EthBcnTickets is ERC721Enumerable, Ownable {
    ...
    uint256 private airdropCounter = 1000;
    ...
    function _transfer(
        ...
        if (from != address(0) && tokenId > 1000) {
            require(
                to == address(0),
                "Aidropped tickets are only allowed to transfer to burn address"
            );
    ...
    function mintTicket(
        ...
        require(
            tokenAddressToPayWith == usdcAddress ||
                tokenAddressToPayWith == daiAddress,
            "Only USDC and DAI accepted"
        );
        require(
    ...
    function airdropTicket(
        ...
        for (uint256 i = 1; i <= _numberOfTokens; i++) {
            _safeMint(_account, ++airdropCounter);
            wave.minted++;
        }
    }
    ...
    function withdrawAll() public onlyOwner {
        _withdrawToken(usdcAddress);
        _withdrawToken(daiAddress);
    }

    function _withdrawToken(address tokenAddress) internal onlyOwner {
        ...
        for (uint256 i = 0; i < royalties.length; i++) {
            ...
            console.log("royalty bal: ", toCollect, "for", royalty.category);
            token.transfer(royalty.walletAddress, toCollect);
        }

        console.log("final bal: ", token.balanceOf(address(this)));
    }
}
