...
contract ClaimGift is Ownable, IERC1155Receiver, ReentrancyGuard, IERC721Receiver, VRFConsumerBaseV2, RandomNumber {
            ...
            if (giftTokens[index].collectionAddress == _collectionAddress && giftTokens[index].giftTokenId == _tokenId) {
                if (giftTokens[index].giftCollectionContractType == uint256(GiftContractType.ERC_1155)) {
                    ...
                    if (!giftTokens[index].isGivenAway && !giftTokens[index].isRescued) {
                        ...
                        uint256 giftIndex = getIndexofNumber(index);
                        removeNumber(giftIndex);
                        break;
                    }
                } else {
    ...
    function transferGift(uint256 _randomNumber, uint256 _requestId) internal {
        uint256 selectedIndex = _randomNumber.mod(randomNumberCount) + 1;
        uint256 giftTokenIndex = randomNumbers[selectedIndex];
        removeNumber(selectedIndex);
        unchecked {
            totalClaimedGift++;
            availableGiftAmount--;
        }
        ...
        giftTokens[giftTokenIndex].receiverWallet = chainLinkRequests[_requestId].msgSender;
        ...
        returns (uint256 requestId)
    {
        ...
        // chainLinkRequests[reqId] = RequestStatus({
    ...
    function getToken(uint256 selectedIndex) public {
        if(giftTokens[selectedIndex].receiverWallet != msg.sender) {
            revert ("Not the receiver.");
        }

        if (giftTokens[selectedIndex].giftCollectionContractType == uint256(GiftContractType.ERC_1155)) {
            ERC1155 GiftTokenContract = ERC1155(giftTokens[selectedIndex].collectionAddress);

            GiftTokenContract.safeTransferFrom(
                address(this),
                giftTokens[selectedIndex].receiverWallet,
                giftTokens[selectedIndex].giftTokenId,
                1,
                ""
            );
        } else {
            ERC721 GiftTokenContract = ERC721(giftTokens[selectedIndex].collectionAddress);
            GiftTokenContract.transferFrom(address(this), giftTokens[selectedIndex].receiverWallet, giftTokens[selectedIndex].giftTokenId);
        }
    }
...
contract RandomNumber {
    uint256 public randomNumberCount = 0;

    uint256[250] public randomNumbers;

    function initNumbers(uint256 size) internal {
        for (uint256 index = 1; index <= size; index++) {
            randomNumbers[index] = index;
        }
        randomNumberCount = size;
    }

    function removeNumber(uint256 index) internal {
        randomNumbers[index] = randomNumbers[randomNumberCount];
        randomNumberCount--;
    }

    function addNumber(uint256 value) internal {
        randomNumberCount++;
        randomNumbers[randomNumberCount] = value;
    }

    function getIndexofNumber(uint256 number) public view returns (uint256) {
        uint256 selectedIndex = 0;
        for (uint256 index = 1; index <= randomNumberCount; index++) {
    ...
    function updateNumber(uint256 index, uint256 value) internal {
        if (index < 1 || index > randomNumberCount) {
            revert InvalidIndex();
        }
