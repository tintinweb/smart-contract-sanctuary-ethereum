...
contract iMETA_NFT_Market is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, ERC1155HolderUpgradeable {    
    ...
    function createMarketItem_eth(address _nftContract, uint256 _tokenId, uint256 _count, uint256 _eth_price) public payable nonReentrant{
        ...
        idMarketItem[_tokenId] = MarketItem(marketId, _nftContract, _tokenId, _count, payable(msg.sender), payable(address(this)), _eth_price,0, false);
    ...
    function buy_with_eth(address _nftContract, uint256 _tokenId, uint256 _count, uint256 _eth_price) public payable nonReentrant{
        
        uint price = idMarketItem[_tokenId].eth_price * _count;
        ...
        idMarketItem[_tokenId].count = idMarketItem[_tokenId].count - _count;
        if(idMarketItem[_tokenId].count == 0){
    ...
    function buy_with_token(address _nftContract, uint256 _tokenId, uint256 _count, uint256 _price) public nonReentrant{
        uint price = idMarketItem[_tokenId].price * _count;
    ...
    function cancleMarketSale(address _nftContract, uint256 _tokenId) public nonReentrant{
        ...
        IERC1155Upgradeable(_nftContract).safeTransferFrom(address(this), msg.sender, _tokenId, idMarketItem[_tokenId].count, "0x");
