...
abstract contract AllocatableERC721Upgradeable is Initializable, ERC721Upgradeable {
    mapping(uint256 => bool) private _lock;
    uint256 private _allocIdx;
    uint64 private _expires;
    
    event Lock(uint256 tokenId);

    function __AllocatableERC721_init(string memory name_, string memory symbol_, uint256 allocIdx_, uint64 expires_) internal onlyInitializing {
        __AllocatableERC721_init_unchained(name_, symbol_, allocIdx_, expires_);
    }

    function __AllocatableERC721_init_unchained(string memory name_, string memory symbol_, uint256 allocIdx_, uint64 expires_) internal onlyInitializing {
        __ERC721_init(name_, symbol_);
        _allocIdx = allocIdx_;
        _expires = expires_;
    }

    function currentAllocIdx() public view virtual returns (uint256) {
        return _allocIdx;
    }

    function isLock(uint256 tokenId) public virtual returns(bool) {
        return _lock[tokenId] && block.timestamp < _expires;
    }

    function _allocationMinting(address[] calldata accountList) internal virtual {
        for (uint256 i = 0; i < accountList.length; i++) {
            uint256 tokenId = currentAllocIdx();
            _safeMint(accountList[i], tokenId);
            _lockup(tokenId);
            _increaseAllocIdx();
        }
    }

    function _increaseAllocIdx() internal virtual {
        _allocIdx++;
    }

    function _lockup(uint256 tokenId) internal virtual {
        _lock[tokenId] = true;
        emit Lock(tokenId);
    }

    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        require(!isLock(tokenId), "Token is lock");
...
import "../../lib/minting-sales/MintingSalesUpgradeable.sol";
import "../../lib/whitelist/WhitelistUpgradeable.sol";
import "../allocation/AllocatableERC721Upgradeable.sol";

contract TTV1 is
    Initializable,
    AllocatableERC721Upgradeable,
    OwnableUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    uint8 private _whitelistLimitPerOnce;
    ...
    function initialize(
        ...
        public initializer
    {
        __AllocatableERC721_init(name_, symbol_, allocIdx_, expires_);
        __MintingSales_init(collector_);
        __Ownable_init();
    ...
    function addWhitelistBatch(address[] calldata accountList, uint256[] calldata amountList) external onlyOwner {
        ...
        external onlyOwner
    {
        ...
        _whitelistLimitPerOnce = whietlistLimitPerOnce;
    }

    function airdrop(address[] calldata accountList) external onlyOwner {
        for (uint256 i = 0; i < accountList.length; i++) {
    ...
    function allocationMinting(address[] calldata accountList) external onlyOwner {
        _allocationMinting(accountList);
    }

    function publicMinting(uint256 requestedCount)
        external
        payable
        nonReentrant
        salesValidator()
    {
        require(requestedCount > 0 && requestedCount <= salesInfo().mintLimitPerOnce, "Sales: too many request or zero request");
        require(msg.value == salesInfo().mintPrice.mul(requestedCount), "Sales: incorrect ETH for payment");
    ...
    function whitelistMinting(uint256 requestedCount)
        external
        payable
        onlyWhitelist
        nonReentrant
        salesValidator
    {
        require(requestedCount > 0 && requestedCount <= _whitelistLimitPerOnce, "Sales: too many request or zero request");
        require(msg.value == _whitelistPrice.mul(requestedCount), "Sales: incorrect ETH for payment");
    ...
    function burn(address from, uint256 tokenId) external {
        require(
            from == _msgSender() || isApprovedForAll(from, _msgSender()) || owner() == _msgSender(),
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return _isReveal ? _revealURI : _baseTokenURI;
    }
...
abstract contract MintingSalesUpgradeable is Initializable, ContextUpgradeable {
    ...
    struct SalesInfo {
        uint8 antibotInterval;       // Interval block count to prevent bot attack
        uint8 mintLimitPerOnce;      // Limit per once for minting
        uint256 mintStartTimestamp;  // Start timestamp of minting
        uint256 mintEndTimestamp;    // End timestamp of minting
        uint256 mintStartIndex;      // First index for minting
        uint256 maxMintingAmount;    // Maximum volume of minting
        uint256 mintPrice;           // price per 1 NFT for minting(1 ETH = 1000000000000000000)
    }
    ...
    function __MintingSales_init(address collector_) internal onlyInitializing {
        __MintingSales_init_unchained(collector_);
    }

    function __MintingSales_init_unchained(address collector_) internal onlyInitializing {
    ...
    function _withdraw() internal virtual {
        ...
        require(amount > 0, "Minting: contract's ETH balance is zero");
...
abstract contract WhitelistUpgradeable is Initializable, ContextUpgradeable {
    ...
    /**
     * @dev Returns count of the account
     * @param account The address to check whitelist count
     */
    function whitelistCount(address account) public view virtual returns (uint256) {
    ...
    function _addWhitelist(address account, uint256 amount) internal virtual {
        ...
        _whitelist[account] = _whitelist[account].add(amount);
    ...
    function _addWhitelistBatch(address[] calldata accountList, uint256[] calldata amountList) internal virtual {
        ...
        for (uint256 i = 0; i < accountList.length ;i++) {
            ...
            _whitelist[accountList[i]] = _whitelist[accountList[i]].add(amountList[i]);
    ...
    function _removeWhitelist(address account) internal virtual {
        ...
        _whitelist[account] = _whitelist[account].sub(1, "Safe Math: incorrect calculation");
