...
contract Parts is ERC1155, ERC1155CollectionBase {
    ...
    */
    ...
    function purchase(bytes32[] calldata merkleProof) public {
        if(IS_PUBLIC_SALE_ACTIVE == false) {
            require(MerkleProof.verify(merkleProof, merkleRoot, toBytes32(msg.sender)) == true, "Public sale is not active yet and Address is not whitelisted");
        }
        _purchase();
    }
