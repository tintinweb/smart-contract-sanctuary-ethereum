...
contract MergeCanvas is IMergeCanvas {
        ...
        SufficientBid(_x, _y)
    {
        ...
        if (msg.value > 0) {
            emit PixelPriceChanged(_x, _y, old_price, msg.value);
        ...
        NotAtMaxPixelCapacity
    {
        ...
        for (uint i = 0; i < _x.length; i++) {
            ...
            emit PixelColorChanged(_x[i], _y[i], old_color, _new_color[i]);
            emit PixelOwnerChanged(_x[i], _y[i], old_owner, msg.sender);

            // Refund all new money to previous owner
            // at last for CIE pattern
            if (_prices[i] > 0) {
                emit PixelPriceChanged(_x[i], _y[i], old_price, _prices[i]);
