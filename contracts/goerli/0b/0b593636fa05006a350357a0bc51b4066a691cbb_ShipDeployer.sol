...
contract ShipDeployer is IDeployerEvents, IDeployerActions, Ownable {
    ...
    /**
     * @dev Token rate and DAO fee rate
     */
    ...
    /**
     * @dev Allows the owner to set whether a given address is allowed to use the createAndSetup() function or not.
     * @param addr The address to set the allowance for.
     * @param allowed Boolean representing whether the address is allowed or not.
    ...
    function getIsAllowed(address addr) public view returns (bool) {
        return
            bypassCheck ||
            ERC721PresetMinterPauserAutoId(CAPTAIN_PASS_NFT).balanceOf(addr) >
            0;
    }
}
