...
contract VEXHAT is Ownable, ERC721A {
    ...
    modifier onlySender() {
        require(msg.sender == sender, "Only sender can use giveaway function.");
        _;
    }

    function mint(uint256 quantity, uint256 id, uint256 cost, bytes calldata signature) external payable {
        require(block.timestamp >= windowOpens, "Purchase window closed.");
        require(verifySignature(signature, cost), "Account not eligible for claim.");
    ...
    function verifySignature(bytes calldata signature, uint256 cost) internal view returns (bool) {
        return sender == keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(this, msg.sender, cost)))).recover(signature);
    }
