...
contract FirstGoerli is ERC721A, Ownable {
    ...
    constructor(       
        ...
        ERC721A("First Goerli", "FG", _amountForTeam, _maxQtyPerTransaction)        
    {
    ...
    // Presale
    function mintPreSaleTokens(address to, uint8 quantity, bytes32[] calldata _merkleProof)
        external
        payable
        callerIsUser        
    {
        require(_presaleActive, "Whitelist mint is not active yet");
        require(_preSaleListCounter[to] + quantity <= maxQtyPerWalletAddressPreSale, "Exceeded max available to purchase at a time");
        ...
        require(MerkleProof.verify(_merkleProof, preSaleRoot, leaf) || allowList[to], "Invalid MerkleProof");

        _safeMint(to, quantity);
        _preSaleListCounter[to] = _preSaleListCounter[to] + quantity;
    }        

    // public mint
    function publicSaleMint(address to, uint256 quantity)
        external
        payable        
        callerIsUser
    {
        ...
        require(_publicCounter[to] + quantity <= maxQtyPerWalletAddressPublicSale, "Exceeds max per address");
        require(totalSupply() + quantity <= maxSupply, "No more NFTs left");
