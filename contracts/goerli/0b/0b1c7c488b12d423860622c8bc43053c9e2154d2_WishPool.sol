...
contract WishPool is Initializable, Pausable, Ownable {
    ...
    function _donate(address userAddr, address donateToken, uint256 donateAmount) private {
        if (donateToken == address(0)) {
            Address.sendValue(payable(appConf.getDonateAddr()), donateAmount);
        } else {
            IERC20Ex(donateToken).safeTransferFrom(userAddr, appConf.getDonateAddr(), donateAmount);
        }
    }

    function _incense(address userAddr, address incenseToken, uint256 incenseAmount) private {
        if (incenseToken == address(0)) {
            Address.sendValue(payable(appConf.getIncenseAddr()), incenseAmount);
        } else {
            IERC20Ex(incenseToken).safeTransferFrom(userAddr, appConf.getIncenseAddr(), incenseAmount);
        }
    }
}
