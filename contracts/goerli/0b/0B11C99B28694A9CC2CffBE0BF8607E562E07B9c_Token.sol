...
contract Token is Ownable, ERC721A, ReentrancyGuard, PaymentSplitter {
    struct SaleConfig {
        uint256 publicSaleStartTime;
    }

    uint256 public constant AUCTION_PRICE = 0.000001 ether;
    uint256 public immutable _totalAmount;
    uint256 public immutable _maximumPerUser;
    uint256 public immutable _maximumPerOwner;
    ...
    function ownerMint(uint256 quantity) external payable onlyOwner {
        ...
        require(
            _numberMinted(msg.sender) + quantity <= _maximumPerOwner,
            "Can not mint this many"
        );
        _safeMint(msg.sender, quantity);
    }
...
contract ERC721A is IERC721A {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory notRevealedURI = _notRevealedURI();
        if (!_isRevealed()) {
