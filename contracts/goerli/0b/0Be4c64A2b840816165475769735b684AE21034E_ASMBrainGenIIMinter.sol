...
contract ASMBrainGenIIMinter is IASMBrainGenIIMinter, Util, AccessControl, ReentrancyGuard {
     ...
     */
    function mint(bytes32[] calldata hashes, bytes calldata signature) external nonReentrant {
        uint256 quantity = hashes.length;
        if (quantity == 0) revert InvalidHashes(quantity, configuration.maxQuantityPerTx, 1);
        if (quantity > configuration.maxQuantityPerTx)
            revert InvalidHashes(quantity, configuration.maxQuantityPerTx, 1);

        if (!validateSignature(hashes, msg.sender, brain.numberMinted(msg.sender), signature))
            revert InvalidSignature();

        uint256 remainingEnergy = energyConverter.getEnergy(msg.sender);
        uint256 energyToUse = quantity * configuration.energyPerBrain;
        if (energyToUse > remainingEnergy) revert InsufficientEnergy(energyToUse, remainingEnergy);
        energyConverter.useEnergy(msg.sender, energyToUse);
     ...
     */
    function updateConfiguration(Config calldata config) external onlyRole(ADMIN_ROLE) {
...
interface IASMBrainGenIIMinter {
    ...
    event ConfigurationUpdated(address indexed operator, Config config);
     ...
     * @param signature The signature for verification. It should be generated from the Dapp and can only be used once
     */
    function mint(bytes32[] calldata hashes, bytes calldata signature) external;
     ...
     * @param config New config data
     */
    function updateConfiguration(Config calldata config) external;
...
interface IConverterLogic {
    function getEnergy(address addr) external view returns (uint256);
