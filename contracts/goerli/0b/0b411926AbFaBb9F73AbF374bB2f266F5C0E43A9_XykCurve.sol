...
abstract contract CollectionPoolERC20 is CollectionPool {
    ...
    function _pullTokenInputAndPayProtocolFee(
        ...
        if (isRouter) {
            ...
            // First, pull total inputAmount because user can't approve to royalty recipients a priori
            // Cache state and then call router to transfer tokens from user
            uint256 beforeBalance = _token.balanceOf(address(this));
            uint256 amountToPool = inputAmount - protocolFee;
            router.poolTransferERC20From(_token, routerCaller, address(this), amountToPool, poolVariant());

            // Verify token transfer (protect pool against malicious router)
            require(_token.balanceOf(address(this)) - beforeBalance == amountToPool, "ERC20 not transferred in");
            ...
            // Now, pay royalties from the tokens pulled
            ...
            for (uint256 i = 0; i < length;) {
                ...
                if (royaltyAmount > 0) {
                    ...
                    router.poolTransferERC20From(_token, address(this), royaltyRecipient, royaltyAmount, poolVariant());
            ...
            // Finally, now that royalties and protocol fees have been paid, we can transfer the remainder to the asset recipient
            if (_assetRecipient != address(this)) {
                uint256 assetRecipientInitialBalance = _token.balanceOf(_assetRecipient);
                uint256 amountToRecipient = amountToPool - totalRoyaltiesPaid;
                router.poolTransferERC20From(_token, address(this), _assetRecipient, amountToRecipient, poolVariant());

                // Verify token transfer (protect pool against malicious router)
                require(
                    _token.balanceOf(_assetRecipient) - assetRecipientInitialBalance == amountToRecipient,
    ...
    /**
     * Pay from the pool. Ensure funds available.
     */
    function _payRoyalties(RoyaltyDue[] memory royaltiesDue) internal returns (uint256 totalRoyaltiesPaid) {
        ...
        for (uint256 i = 0; i < length;) {
            ...
            if (royaltyAmount > 0) {
                ...
                _token.safeTransferFrom(address(this), royaltyRecipient, royaltyAmount);
            }
        ...
        override
    {
        ...
        if (outputAmount > 0) {
            ERC20 _token = token();
            require(_token.balanceOf(address(this)) >= outputAmount + tradeFee, "Too little ERC20");
