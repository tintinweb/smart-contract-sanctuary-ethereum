...
contract ArbitrageBot is UniswapFlashSwapper {
  ...
  constructor(address _uniswapRouter, address _sushiRouter)
    public
    UniswapFlashSwapper(_uniswapRouter, _sushiRouter)
  {}
...
contract UniswapFlashSwapper is IUniswapV2Callee {
  ...
  function uniswapV2Call(
    ...
    // decode data
    (SwapType _swapType, bytes memory _otherData) = abi.decode(data, (SwapType, bytes));
  ...
  function flashLoanExecute(
    ...
    uint256 amountRequired = UniswapV2Library.getAmountsIn(uniswapFactory, _amountTokenIn, _pathIn)[
      0
    ];
