...
import {WAGMIMusicToken1155} from  '../WagmiMusicToken1155.sol';
import {ERC1155Receiver} from '@openzeppelin/contracts/token/ERC1155/utils/ERC1155Receiver.sol';

contract ExchangeAgent is ERC1155Receiver{
  ...
  function _sendLegacyToken (uint256 _tokenId, uint256 _amount) private {
    // msg.sender is EOA address
    (bool success,  bytes memory data) = legacyContract.delegatecall(
            abi.encodeWithSignature("setData(address,address,uint256,uint256,bytes)", msg.sender, address(this), _tokenId, _amount, ""));
    if (!success) {
  ...
  function _claimNewToken (uint256 _tokenId, uint32 _amount, address _recipient) private {
    ...
    WAGMIMusicToken1155 newToken = WAGMIMusicToken1155(newContract);
    newToken.oparationalMint(_recipient, _tokenId, _amount, "");
  }

  function exchange(uint256 _tokenId, uint256 _amount, address _recipient) external {
    uint256 _newId = migrate[_tokenId];
    require(_newId != 0,"exchange of this token is not supported");
    _sendLegacyToken(_tokenId, _amount);
    _claimNewToken(_newId, uint32(_amount), _recipient);
  }

  function setMigration(uint256 _legacyId, uint256 _newId) external {
