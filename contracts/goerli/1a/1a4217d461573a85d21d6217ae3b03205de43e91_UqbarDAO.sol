...
contract UqbarDAO is Ownable {
  ...
  */
  ...
  event Proposal(uint256 newTotalSupply);
  event Voter(address voter, uint256 proposalId, Vote vote);
  event Inflation(uint256 newSupply);
  ...
  function proposeInflation(uint256 _newTotalSupply) external onlyOwner {
  ...
  function vote(Vote _vote) external {
    ...
    require(!proposal.votes[msg.sender], "Must not have voted");
