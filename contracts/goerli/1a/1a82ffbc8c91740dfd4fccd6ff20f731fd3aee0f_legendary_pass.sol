    ...
    Ownable
{
    ...
    uint256 public legandaryReserve = 400; // Reserve 400 legandary passes for team & community (Used in giveaways, events etc...)
    uint256 public maxMintPerPurchase = 10;
    
    constructor(string memory _initBaseURI)
        ERC721("legend_pass_ERC20", "L20PASS")
    {
    ...
    function mintPassERC20(uint256 _amount, uint256 _pid) public {
        ...
        paytoken.approve(address(this), mintPrice * _amount);
    ...
    function reservePass(address _to, uint256 _amount) public onlyOwner {
        uint256 supply = totalSupply();
        require(_amount > 0 && _amount <= legandaryReserve, "Not enough reserve left");
        require(supply + _amount <= maxSupply, "Can not mint more than max supply.");

        for (uint256 i = 0; i < _amount; i++) {
            _safeMint(_to, supply + i);
        }
        legandaryReserve = legandaryReserve - _amount;
    }
