...
contract TokenStaking is ReentrancyGuard, Ownable, Recoverable {
    ...
    uint256 public s_rewardRate = 3.154e7;
    ...
    function stake(uint256 _amount) external nonReentrant {
        ...
        require(token.balanceOf(msg.sender) >= _amount, 'balance is <= amount');
    ...
    function unstake(uint256 _amount) external nonReentrant {
        require(_amount > 0, 'amount is <= 0');
        require(staked[msg.sender] >= _amount, 'amount is > amount');
    ...
    function claim() public nonReentrant {
        require(staked[msg.sender] > 0, 'amount is > amount');
        uint256 secondsStkd = block.timestamp - stakedTimestamp[msg.sender];
        uint256 rewards = staked[msg.sender] * secondsStkd / s_rewardRate;
    ...
    function earned(address _staker) public view returns(uint256) {
        uint256 secondsStkd = block.timestamp - stakedTimestamp[_staker];
        uint256 rewards = staked[msg.sender] * secondsStkd / s_rewardRate;
        return rewards;
    }
}
