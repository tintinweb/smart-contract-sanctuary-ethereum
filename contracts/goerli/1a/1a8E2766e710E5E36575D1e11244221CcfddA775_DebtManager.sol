...
contract DebtManager is Initializable {
  ...
  /// @notice Leverages the floating position of `_msgSender` to match `targetHealthFactor` by taking a flash loan
  ...
  function leverage(Market market, uint256 principal, uint256 targetHealthFactor, bool deposit) external msgSender {
    ...
    if (deposit) asset.safeTransferFrom(sender, address(this), principal);
    ...
    calls[0] = abi.encodeCall(ERC4626.deposit, (amounts[0] + (deposit ? principal : 0), sender));
    calls[1] = abi.encodeCall(Market.borrow, (amounts[0], address(balancerVault), sender));
  ...
  function deleverage(Market market, uint256 maturity, uint256 maxAssets, uint256 percentage) external msgSender {
    uint256[] memory amounts = new uint256[](1);
    ERC20[] memory tokens = new ERC20[](1);
    bytes[] memory calls = new bytes[](2);
    ...
    if (maturity == 0) {
      ...
      calls[0] = abi.encodeCall(Market.repay, (amounts[0], sender));
    } else {
      uint256 positionAssets;
      (amounts[0], positionAssets) = repayAtMaturityAssets(market, maturity, percentage);
      calls[0] = abi.encodeCall(Market.repayAtMaturity, (maturity, positionAssets, maxAssets, sender));
    }
    calls[1] = abi.encodeCall(Market.withdraw, (amounts[0], address(balancerVault), sender));
  ...
  function rollFloatingToFixed(
    ...
    (r.principal, r.fee) = market.fixedBorrowPositions(borrowMaturity, r.sender);
    (, , uint256 floatingBorrowShares) = market.accounts(r.sender);
    ...
    for (r.i = 0; r.i < r.loopCount; ) {
      calls[r.callIndex++] = abi.encodeCall(
        Market.repay,
        (r.i == 0 ? amounts[0] : r.repayAssets / r.loopCount, r.sender)
      );
      calls[r.callIndex++] = abi.encodeCall(
    ...
    balancerVault.flashLoan(address(this), tokens, amounts, abi.encode(market, calls));
    (uint256 newPrincipal, uint256 newFee) = market.fixedBorrowPositions(borrowMaturity, r.sender);
  ...
  function rollFixedToFloating(
    ...
    for (r.i = 0; r.i < r.loopCount; ) {
      calls[r.callIndex++] = abi.encodeCall(
        ...
        (repayMaturity, positionAssets, type(uint256).max, r.sender)
      );
      calls[r.callIndex++] = abi.encodeCall(
        Market.borrow,
        (amounts[0], r.i + 1 == r.loopCount ? address(balancerVault) : address(this), r.sender)
      );
      unchecked {
        ++r.i;
      }
    ...
    balancerVault.flashLoan(address(this), tokens, amounts, abi.encode(market, calls));
    (, , floatingBorrowShares) = market.accounts(r.sender);
  ...
  function rollFixed(
    ...
    for (r.i = 0; r.i < r.loopCount; ) {
      calls[r.callIndex++] = abi.encodeCall(
        Market.repayAtMaturity,
        (repayMaturity, r.positionAssets, type(uint256).max, r.sender)
      );
      calls[r.callIndex++] = abi.encodeCall(
        Market.borrowAtMaturity,
        (
  ...
  modifier recover(
    ...
    assert(block.chainid == chainId && _msgSender == address(0));
    {
      address recoveredAddress = ecrecover(
        keccak256(
          abi.encodePacked(
            "\x19\x01",
            DOMAIN_SEPARATOR,
            keccak256(abi.encodePacked(message, abi.encode(account, nonces[account]++)))
          )
        ),
        v,
        r,
      ...
      if (recoveredAddress == address(0) || recoveredAddress != account) revert InvalidOperation();
      _msgSender = recoveredAddress;
    }
    _;
    ...
    recover(
      abi.encode(
        keccak256(
          ...
          "RollFloatingToFixed(address market,uint256 borrowMaturity,uint256 maxBorrowAssets,uint256 percentage,address account,uint256 nonce)"
        ),
        market,
        borrowMaturity,
        maxBorrowAssets,
    ...
    )
  {
    address(this).functionCall(
      abi.encodeCall(IDebtManager.rollFloatingToFixed, (market, borrowMaturity, maxBorrowAssets, percentage)),
      ""
    );
  }
  ...
  function rollFixedToFloating(
    ...
    recover(
      abi.encode(
        keccak256(
          // solhint-disable-next-line max-line-length
          "RollFixedToFloating(address market,uint256 repayMaturity,uint256 maxRepayAssets,uint256 percentage,address account,uint256 nonce)"
        ),
        market,
        repayMaturity,
        maxRepayAssets,
  ...
  {
    address(this).functionCall(
      abi.encodeCall(IDebtManager.rollFixedToFloating, (market, repayMaturity, maxRepayAssets, percentage)),
      ""
    );
  }
  ...
  function rollFixed(
    ...
    recover(
      abi.encode(
        keccak256(
          // solhint-disable-next-line max-line-length
          "RollFixedToFloating(address market,uint256 repayMaturity,uint256 borrowMaturity,uint256 maxRepayAssets,uint256 maxBorrowAssets,uint256 percentage,address account,uint256 nonce)"
        ),
        market,
        repayMaturity,
        borrowMaturity,
    ...
    )
  {
    address(this).functionCall(
      abi.encodeCall(
        ...
        (market, repayMaturity, borrowMaturity, maxRepayAssets, maxBorrowAssets, percentage)
      ),
      ""
    );
  }
