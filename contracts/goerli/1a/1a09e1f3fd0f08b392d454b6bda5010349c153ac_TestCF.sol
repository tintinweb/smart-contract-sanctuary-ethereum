...
contract TestCF is ERC721, Ownable, Functional {
    ...
    function whitelistMint(uint256 qty) external payable reentryLock {
        ...
        require(whitelistUser[_msgSender()] > 0, "Whitelist: Invalid User"); 
        require((qty + reservedTokens + tokenMinted) <= maxSupply, 
        ...
        require(tokenMintedby[_msgSender()] + qty <= whitelistUser[_msgSender()],
    ...
    */
    ...
    function setMultipleWhitelist(address [] memory _Address, uint256 [] memory qty) public onlyOwner{
        require(_Address.length == qty.length, "Whitelist Entry: length mismatch error");
        for( uint256 i = 0; i < _Address.length; i++ ){
            whitelistUser[_Address[i]] = qty[i];
        }
    }

    function setSingleWhitelist(address _Address, uint256 _qty) public onlyOwner {
        whitelistUser[ _Address ] = _qty;
    }
    ...
    function getWhitelistToken() external view returns (uint256) {
        return whitelistUser[_msgSender()];
    }
