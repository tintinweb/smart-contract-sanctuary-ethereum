...
import {IVault, Plugin, InitInfo} from "./interfaces/IVault.sol";
...
contract Vault is Clone, IVault, NFTReceiver {
    ...
    /// @param _plugins Plugin struct for plugin address and selector
    function setPlugins(Plugin[] memory _plugins) external {
        if (msg.sender != FACTORY() && msg.sender != OWNER())
            revert NotFactory(FACTORY(), msg.sender);

        bytes4 selector;
        for (uint256 i; i < _plugins.length; ) {
            selector = _plugins[i].selector;
            if (methods[selector] != address(0))
                revert InvalidSelector(selector);
            methods[selector] = _plugins[i].target;
            unchecked {
                ++i;
            }
        }
        emit UpdatedPlugins(_plugins);
    }
...
import {Vault, Plugin, InitInfo} from "./Vault.sol";
...
contract VaultFactory is IVaultFactory {
    ...
    function deploy(bytes32 _merkleRoot, Plugin[] calldata _plugins)
        external
        returns (address payable vault)
    {
        vault = deployFor(_merkleRoot, msg.sender, _plugins);
    }
    ...
    /// @param _plugins List of plugins
    /// @return vault Address of deployed vault
    function deployFor(
        bytes32 _merkleRoot,
        address _owner,
        ...
        Vault(vault).setPlugins(_plugins);
    }
