...
contract TokenLockStakeable is Ownable {
    ...
    event ParterAddressChanged(address oldAddress, address newAddress);
    event CurrentUnlockRateUpdated(uint256 currentUnlockRate);
    event RewardSet(address user, uint256 amount);
    ...
    function changePartnerAddress(address oldAddress, address newAddress) external onlyOwner {
        ...
        emit ParterAddressChanged(oldAddress, newAddress);
    }
