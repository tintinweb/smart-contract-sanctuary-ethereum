 ...
 */
contract Converter is IStaking, Util, PermissionControl, Pausable {
    ...
    constructor(
        ...
        astoMultiplier = _astoMultiplier;
    ...
    /**
     * @dev Calculate the energy for `addr` based on the staking history
     *
     * @param addr The wallet address to calculated for
     * @return energy amount
     */
    function calculateEnergy(address addr) public view returns (uint256) {
        uint256 astoEnergyAmount = calculateAstoEnergy(addr);
        uint256 lpEnergyAmount = calculateLpEnergy(addr);
     ...
     * @return energy amount
     */
    function calculateAstoEnergy(address addr) public view nonZero(addr) returns (uint256) {
        Stake[] memory astoHistory = stakingLogic_.getHistory(ASTO_TOKEN_ID, addr, currentTime());

        return _calculateEnergyForToken(astoHistory, astoMultiplier);
    }
     ...
     * @return energy amount
     */
    function calculateLpEnergy(address addr) public view nonZero(addr) returns (uint256) {
        Stake[] memory lpHistory = stakingLogic_.getHistory(LP_TOKEN_ID, addr, currentTime());

        return _calculateEnergyForToken(lpHistory, lpMultiplier);
    }
     ...
     * @return energy amount
     */
    function calculateAvailableLBAEnergy(address addr) public view returns (uint256) {
        uint256 lbaEnergyStartTime = getLBAEnergyStartTime();
        if (currentTime() < lbaEnergyStartTime) return 0;

        uint256 finishTime = currentTime() < lbaEnergyFinishTime ? currentTime() : lbaEnergyFinishTime;
        uint256 elapsedTime = finishTime - lbaEnergyStartTime;
        uint256 dailyLbaEnergyAmount = getDailyLBAEnergyProduction(addr);
     ...
     */
    function getDailyASTOEnergyProduction(address addr) public view nonZero(addr) returns (uint256) {
        Stake[] memory history = stakingLogic_.getHistory(ASTO_TOKEN_ID, addr, currentTime());
        return history.length > 0 ? history[history.length - 1].amount.mul(astoMultiplier) : 0;
    }
     ...
     * @return energy amount (per day)
     */
    function getDailyLPEnergyProduction(address addr) public view nonZero(addr) returns (uint256) {
        Stake[] memory history = stakingLogic_.getHistory(LP_TOKEN_ID, addr, currentTime());
        return history.length > 0 ? history[history.length - 1].amount.mul(lpMultiplier) : 0;
    }
     ...
     * @return energy amount (per day)
     */
    function getDailyLBAEnergyProduction(address addr) public view nonZero(addr) returns (uint256) {
        return lba_.claimableLPAmount(addr).mul(lpMultiplier);
    }
     ...
     */
    function getEnergy(address addr) public view virtual returns (uint256) {
        if (stakingLogic_.lbaMigrated(addr)) {
            uint256 generatedEnergy = calculateEnergy(addr);
            ...
            uint256 remainingEnergy = generatedEnergy > consumedEnergy ? generatedEnergy - consumedEnergy : 0;
            return remainingEnergy + getRemainingLBAEnergy(addr);
        }
    }
     ...
     */
    function getRemainingLBAEnergy(address addr) public view returns (uint256) {
        uint256 availableEnergy = calculateAvailableLBAEnergy(addr);
     ...
     */
    function useEnergy(address addr, uint256 amount) public whenNotPaused onlyRole(CONSUMER_ROLE) nonZero(addr) {
        if (amount > getEnergy(addr)) revert InvalidInput(WRONG_AMOUNT);

        uint256 remainingLBAEnergy = getRemainingLBAEnergy(addr);
    ...
    /**
     * @notice Update ASTO multiplier for ASTO Energy.
     * @param _astoMultiplier The new ASTO multiplier to be set
     */
    function setASTOMultiplier(uint256 _astoMultiplier) external onlyRole(MANAGER_ROLE) {
        astoMultiplier = _astoMultiplier;
    }

    /**
     * @notice Update LP multiplier for ASTO Energy.
     * @param _lpMultiplier The new LP multiplier to be set
     */
    function setLPMultiplier(uint256 _lpMultiplier) external onlyRole(MANAGER_ROLE) {
        lpMultiplier = _lpMultiplier;
    }
...
contract ConverterTest is Converter {
    ...
    constructor(
        ...
        uint256 time,
        uint256 finishTime
    ) Converter(owner, lba, _astoMultiplier, _lpMultiplier, time, finishTime) {}
    ...
    function getEnergy(address addr) public view virtual override returns (uint256) {
        uint256 customized = customizedEnergy[addr];

        if (customized > 0) {
            uint256 consumed = getConsumedEnergy(addr);
            return customized > consumed ? customized - consumed : 0;
        }
        return super.getEnergy(addr);
    }
