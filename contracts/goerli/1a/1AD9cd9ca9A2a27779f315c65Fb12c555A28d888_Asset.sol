...
contract Asset  is ERC721, Ownable {
    //the address of the imx contract that does the minting & transfers on L1
    address public imx;
    string public baseTokenURI;

    //event to let imx know the mint was succesfull
    ...
    constructor(
        ...
        imx = _imx;
        baseTokenURI = "https://nn-image-store.fra1.digitaloceanspaces.com/NuggetsTest/collection1/api_imx";
       
    }

    //modifier to assure IMX can call the mintFor function
    modifier onlyOwnerOrIMX() {
        require(msg.sender == imx || msg.sender == owner(), "Only allowed for owner or IMX");
        _;
    }
    ...
    //helper function for splitting blob
    ...
    // {token_id}:{blueprint}
    function splitBlob(bytes calldata blob)
        internal
        pure
    ...
    //function needed for IMX to call when assets are withdrawn to L1 for the first time
    function mintFor(
        address user,
        uint256 quantity,
        ...
        //no on-chain metadata so we only need the id, not the blueprint
        (uint256 id, bytes memory blueprint) = splitBlob(mintingBlob);
        _safeMint(user, id);
