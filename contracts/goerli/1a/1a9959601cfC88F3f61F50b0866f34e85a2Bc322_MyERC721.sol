...
contract MyERC721 is ImmutableERC721Preset {
    ...
    // Event that's emitted when a new ticket is purchased
    event TicketPurchased(address indexed buyer, uint256 ticketId);

    // Event that's emitted when a winner is picked
    event WinnerPicked(address indexed winner, uint256 prize);

    // The cost of each ticket, in wei
    uint256 public constant TICKET_PRICE = 10 wei;

    // The percentage of the prize that goes to the admin
    uint256 public constant ADMIN_FEE_PERCENTAGE = 10;

    // The current ticket ID counter
    uint256 private _currentTicketId = 0;

    // The list of ticket IDs that are currently in the pool
    uint256[] private _ticketPool;

    // The mapping of ticket IDs to the addresses that own them
    ...
    // Buy a ticket and enter the lottery pool
    function buyTicket() public payable {
        require(msg.value == TICKET_PRICE, "Lottery: incorrect ticket price");

        // Mint a new NFT for the buyer
        _currentTicketId = _currentTicketId.add(1);
        _safeMint(msg.sender, _currentTicketId);

        // Add the ticket ID to the pool and map it to the buyer's address
        _ticketPool.push(_currentTicketId);
        _ticketOwners[_currentTicketId] = msg.sender;

        emit TicketPurchased(msg.sender, _currentTicketId);
    }

    // Pick a winner and distribute the prize
    function pickWinner() public {
        require(msg.sender == super.owner(), "sender is not owner");
        require(_ticketPool.length > 0, "Lottery: no tickets in the pool");

        // Pick a random ticket ID from the pool
        uint256 winnerIndex = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % _ticketPool.length;
        uint256 winnerId = _ticketPool[winnerIndex];

        // Get the winner's address and calculate the prize amounts
        address winner = _ticketOwners[winnerId];
        uint256 adminFee = TICKET_PRICE.mul(_ticketPool.length).mul(ADMIN_FEE_PERCENTAGE).div(100);
        uint256 prize = _ticketPool.length.mul(TICKET_PRICE).sub(adminFee);

        // Transfer the prize to the winner and the admin fee to the admin
        payable(winner).transfer(prize);
        payable(super.owner()).transfer(adminFee);

        // Remove all tickets from the pool and clear the mapping
        for (uint256 i = 0; i < _ticketPool.length; i++) {
            delete _ticketOwners[_ticketPool[i]];
        }
        delete _ticketPool;
