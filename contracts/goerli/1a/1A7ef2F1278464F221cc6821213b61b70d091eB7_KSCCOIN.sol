...
contract KSCCOIN is ERC721,BlackLkisted {
	using SafeMath for uint256;
	IBEP20 dart;
	constructor() payable ERC721('KSCCOIN', 'KSCCOIN') {
		sellingFess = 15;
	}
    uint256 public sellingFess;
    uint256 public creationFessETH = 1000000000000000;
    uint256 public creationFessKSC = 2000000000000000000;
	...
	function setContractFees(uint256 _creationFessETH, uint256 _creationFessKSC,uint256 _sellingFess,uint256 _referralFees) public onlyOwner {
	    creationFessETH = _creationFessETH;
        creationFessKSC = _creationFessKSC;
	...
	function mintNFT(string memory _tokenURI,uint256 amount, uint256 tokenId, bool isToken,uint256 price,IBEP20 paymentCurrency,uint256 royalties) public payable {
        ...
        if(creationFessETH > 0 || creationFessKSC > 0) {
                if(isToken) {
                    require(amount>=creationFessKSC,"Low Creation Fees");
                    ...
                    require(msg.value>=creationFessETH,"Low Creation Fees");
    ...
    function lazyMintNFT(string memory _tokenURI,uint256 amount, uint256 tokenId, bool isToken,uint256 price,IBEP20 paymentCurrency,uint256 royalties) public payable {
		require(!isBlacklisted[msg.sender], "caller is backlisted");  
        require(checkExitscurrency(paymentCurrency),"Invalid Payment Currency");
        require(hasSaleStarted == true, "Sale hasn't started");
                if(isToken) {
        ...
        nftINFO[tokenId].price=price;
        nftINFO[tokenId].owner_address=msg.sender;
        nftINFO[tokenId].creator_address=owner();
        nftINFO[tokenId].isToken=isToken;
        nftINFO[tokenId].paymentCurrency=paymentCurrency;
        nftINFO[tokenId].sell=true;
        nftINFO[tokenId].royalty=royalties;
