...
/**
 * @dev upgradable {ERC721} token with staking functionality and cross-chain support.
 */
contract OColdBloodedCreepzV2 is
  ...
  ONFT721Upgradeable
{
  ...
  bool private canTransferWhikeStaked;
  ...
  */
  ...
  function initialize(
    ...
    //@TODO: update token names and LZ values
   ...
   */
  function calculateStakeForMultipleTokens(uint256[] calldata tokenIds)
    external
    view
    returns (uint256 totalAccumulated)
  {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
   ...
   */
  function trasnferWhileStaked(address to, uint256[] calldata tokenIds)
    external
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      require(ownerOf(tokenId) == _msgSender(), "C:!O");
      canTransferWhikeStaked = true;
      _transfer(_msgSender(), to, tokenId);
      canTransferWhikeStaked = false;
    }
  }
   ...
   */
  function ownerRemoveFromSoftStake(uint256[] calldata tokenIds)
    external
    nonReentrant
    onlyOwner
  {
   ...
   */
  function airdrop(address[] calldata recipients, uint256[] calldata tokenIds)
    external
    onlyOwner
  {
  ...
  /**
   * @dev Function to claim tokens. No limit on claim more than initial 11111, since contract is upgradable anyways... But we're not gonna use it, so it's fine. Don't sleep on Creepz!
   */
  function claim(uint256[] calldata tokenIds) external nonReentrant {
    require(!isClaimPaused);
    for (uint256 i = 0; i < tokenIds.length; i++) {
  ...
  /**
   * @dev Sets the address of creepzv1 contract.
   */
  function setCreepzAddress(address _creepzAddress) external onlyOwner {
    creepzV1Address = _creepzAddress;
  }
  ...
  */
  function _beforeTokenTransfer(
    ...
    require(stakedAtTimestamp[tokenId] == 0 || canTransferWhikeStaked, "C:TS");
    if (from != address(0)) {
  ...
  ILayerZeroUserApplicationConfigUpgradeable
{
  ...
  uint256 public constant DEFAULT_PAYLOAD_SIZE_LIMIT = 10000; // 32 bytes
