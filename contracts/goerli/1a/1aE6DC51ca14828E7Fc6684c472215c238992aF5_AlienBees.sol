...
contract AlienBees is ERC721, Ownable, ReentrancyGuard, RandomlyAssigned{
    ...
    constructor(
        ...
        for(uint32 walletLoop =0; walletLoop < _whiteListedAddresses.length; walletLoop++){
            if(whiteListedWallets[_whiteListedAddresses[walletLoop]] != true ){
                whiteListedWallets[_whiteListedAddresses[walletLoop]] = true;
            }
        }
        for(uint32 HpwalletLoop =0; HpwalletLoop < _HivePalsHolders.length; HpwalletLoop++){
            if(HpSaleWallets[_HivePalsHolders[HpwalletLoop]] != true ){
                HpSaleWallets[_HivePalsHolders[HpwalletLoop]] = true;
            }
        }
    }
    ...
    function addNewWalletsForHivePalsSale(address[] memory _HivePalsHolders) external onlyOwner{  
        for(uint32 hpwalletLoop =0; hpwalletLoop < _HivePalsHolders.length; hpwalletLoop++){
            if(HpSaleWallets[_HivePalsHolders[hpwalletLoop]] != true ){
                HpSaleWallets[_HivePalsHolders[hpwalletLoop]] = true;
            }
    ...
    function mintBee(uint256 quantity) public payable {
        ...
        require((HpSaleWallets[msg.sender] == true && HPSale == true)
        || (whiteListedWallets[msg.sender] == true && whiteListSale == true)
        || publicSale == true, "Minting is not live.");
        ...
        require( (remainingLimit + quantity) <= totalLimit, "Your mint limit has been met.");
    ...
    function mintReserved(uint256 quantity) public {
        ...
        require( (reservedSupply + quantity) <= reservedNFTLimit, "You cannot mint more reserved NFTs.");
