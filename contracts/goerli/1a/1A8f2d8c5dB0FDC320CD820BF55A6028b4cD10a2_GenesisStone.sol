    ...
    DefaultOperatorFilterer
{
    ...
    //------------------------------------------------------------//
    // The following functions are required to mint by signature. //
    //------------------------------------------------------------//
    function mint(bytes calldata _signature) public {
        if (!_verifySignature(_signature)) {
            revert InvalidSignature();
        }
        if (_whitelistMinted[msg.sender]) {
    ...
    function _hashSignature(address address_) internal view returns (bytes32) {
        return
            keccak256(abi.encodePacked(address(this), address_))
                .toEthSignedMessageHash();
    }

    function _verifySignature(bytes memory signature_)
        internal
        view
        returns (bool)
    {
        address signer = _hashSignature(msg.sender).recover(signature_);
