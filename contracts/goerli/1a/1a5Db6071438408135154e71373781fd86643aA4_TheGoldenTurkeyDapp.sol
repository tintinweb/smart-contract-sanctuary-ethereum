...
contract TheGoldenTurkeyDapp is ReentrancyGuard {
    ...
    function getUserEvents(address player) public view returns (ocurrence[] memory filteredOcurrences) {
        ocurrence[] memory ocurrencesTemp = new ocurrence[](nextOcurrence - 1);
        uint count;
        for (uint i = 0; i < (nextOcurrence - 1); i++) {
            if (Ocurrences[i].from == player || Ocurrences[i].to == player) {
                ocurrencesTemp[count] = Ocurrences[i];
                count ++;
            }
        }   

        filteredOcurrences = new ocurrence[](count);
        for (uint i = 0; i < count; i++) {
            filteredOcurrences[i] = ocurrencesTemp[i];
        }

        return filteredOcurrences;
    }
