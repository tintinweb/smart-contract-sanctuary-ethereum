...
import { IERC165 } from "openzeppelin/utils/introspection/IERC165.sol";
import { IRarementFeeRegistry } from "@core/interfaces/IRarementFeeRegistry.sol";
import { IVolumeMaxMinter, VolumeMintData, MintInfo } from "./interfaces/IVolumeMaxMinter.sol";
import { BaseMinter } from "./BaseMinter.sol";
import { IMinterModule } from "@core/interfaces/IMinterModule.sol";
import { IRarementVolumeV1, VolumeInfo } from "@core/interfaces/IRarementVolumeV1.sol";

/*
 * @title VolumeMaxMinter
 * @notice Module for unpermissioned mints of Rarement volumes.
 * @author rmnt.xyz
 */
contract VolumeMaxMinter is IVolumeMaxMinter, BaseMinter {
    // =============================================================
    //                            STORAGE
    // =============================================================
     ...
     * @inheritdoc IVolumeMaxMinter
     */
    function mintInfo(address volume, uint128 mintId) external view returns (MintInfo memory info) {
        BaseData memory baseData = _baseData[volume][mintId];
        VolumeMintData storage mintData = _volumeMintData[volume][mintId];

        VolumeInfo memory volumeInfo = IRarementVolumeV1(volume).volumeInfo();
        ...
        info.maxMintableLower = volumeInfo.volumeMaxMintableLower;
        info.maxMintableUpper = volumeInfo.volumeMaxMintableUpper;
        info.cutoffTime = volumeInfo.volumeCutoffTime;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(IERC165, BaseMinter) returns (bool) {
        return BaseMinter.supportsInterface(interfaceId) || interfaceId == type(IVolumeMaxMinter).interfaceId;
    }
    ...
    function moduleInterfaceId() public pure returns (bytes4) {
        return type(IVolumeMaxMinter).interfaceId;
    }
}
...
/**
 * @dev Data unique to a volume max mint.
 */
...
/**
 * @dev All the information about a volume max mint (combines VolumeMintData with BaseData).
 */
struct MintInfo {
    uint32 startTime;
    uint32 endTime;
...
/**
 * @title IVolumeMaxMinter
 * @dev Interface for the `VolumeMaxMinter` module.
 * @author rmnt.xyz
 */
interface IVolumeMaxMinter is IMinterModule {
    // =============================================================
    //                            EVENTS
    // =============================================================

    /**
     * @dev Emitted when a volume max is created.
    ...
    /**
     * @dev Returns {IVolumeMaxMinter.MintInfo} instance containing the full minter parameter set.
