...
import { ERC1967Proxy } from "./utils/proxy/ERC1967Proxy.sol";

import { StreamTypes } from "./types/StreamTypes.sol";

interface IFactory is StreamTypes { }

contract Factory is IFactory {
    address public immutable VAULT_IMPL;
    address public immutable WALLET_IMPL;

    constructor(address vaultImpl, address walletImpl) payable {
    ...
    event PairCreated(address indexed vault, address indexed wallet, uint256 nonce);

    error VAULT_EXISTS(address vault);

    error WALLET_EXISTS(address wallet);

    ///                                                          ///
    ///                                                          ///
    ///                                                          ///

    function create(address[] calldata owners, uint256 vaultThreshold, uint256 nonce)
        external
        returns (address vault, address wallet)
    {
        vault = createVault(owners, vaultThreshold, nonce);

        // TODO if deploying BOTH, we do not need to check if wallet exists after vault deployment -- bc its not possible
        // This check only needs to be done if deploying WALLET only
        wallet = createWallet(vault);

        emit PairCreated(vault, wallet, nonce);
    }

    function createWithStream(address[] calldata owners, uint256 vaultThreshold, uint256 nonce, Stream memory stream)
        external
        returns (address vault, address wallet)
    {
        bytes32 salt = keccak256(abi.encodePacked(owners, nonce));

        address precomputedVault = address(
            uint160(
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            address(this),
                            salt,
                            keccak256(abi.encodePacked(type(ERC1967Proxy).creationCode, abi.encode(VAULT_IMPL, "")))
                        )
                    )
                )
            )
        ...
        if (precomputedVault.code.length != 0) revert VAULT_EXISTS(precomputedVault);

        vault = address(new ERC1967Proxy{salt: salt}(VAULT_IMPL, ""));

        address precomputedWallet = address(
            uint160(
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            address(this),
                            salt,
                            keccak256(abi.encodePacked(type(ERC1967Proxy).creationCode, abi.encode(WALLET_IMPL, "")))
                        )
                    )
                )
            )
        ...
        if (precomputedWallet.code.length != 0) revert WALLET_EXISTS(precomputedWallet);

        wallet = address(new ERC1967Proxy{salt: salt}(WALLET_IMPL, ""));

        stream.recipient = wallet;

        IVault(vault).initializeWithStream(owners, vaultThreshold, stream);
        IWallet(wallet).initialize(vault);
    ...
    function createVault(address[] calldata owners, uint256 vaultThreshold, uint256 nonce)
        internal
        returns (address vault)
    {
        ...
        IVault(vault).initialize(owners, vaultThreshold);
    }

    function createWallet(address vault) internal returns (address wallet) {
...
import { StreamTypes } from "../types/StreamTypes.sol";
import { TransactionTypes } from "../types/TransactionTypes.sol";

interface IVault is TransactionTypes, StreamTypes {
    function isOwner(address account) external view returns (bool);

    function initialize(address[] calldata initOwners, uint256 initThreshold) external payable;

    function initializeWithStream(address[] calldata initOwners, uint256 initThreshold, Stream calldata stream)
        external
        payable;
}
...
interface IWallet is TransactionTypes {
    function initialize(address initVault) external payable;
}
...
interface StreamTypes {
    struct Stream {
        address recipient;
        address tokenContract; // address(0) for ETH
        uint256 tokenAmount;
        uint256 period;
    }
}
