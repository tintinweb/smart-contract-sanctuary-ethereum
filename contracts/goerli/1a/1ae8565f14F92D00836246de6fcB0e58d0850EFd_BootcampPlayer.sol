    ...
    Ownable
{
  ...
  constructor(address _playerOracle) ERC721("BOOTCAMP", "BOOTCAMP") {
  ...
  function getTeam(uint256 tokenId) public view returns(uint256) {
      require(hasStats(tokenId),"NOT_CUSTOMISED");
      return (playerStats[tokenId].kit + 1) / 3;
  }
  ...
  function customise(uint256 tokenId, Stats calldata stats) public {
    ...
    Metadata memory metadata = playerOracle.getCharacterTraits(tokenId);
    uint256 skillTotal = startingSkillTotal[metadata.skillLevel];

    if(metadata.playerType == 1) skillTotal += 4; // martian bonus
    if(metadata.playerType == 2) skillTotal += 8; // astronaut bonus
    require(_checkStruct(stats, skillTotal, metadata.skillLevel), "Invalid point assignment");
  ...
  function _checkStruct(Stats memory stats, uint max, uint skillLevel) internal pure returns(bool){
    uint addup;
    ...
    if(stats.kit == 1 || stats.boots == 1){
      require(skillLevel == 6, "Skill level not high enough for this kit");
    }
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
      ...
      string memory image = string(abi.encodePacked("https://cloudFunctionURL?id=", getPlayerId(id).toString()));
