...
import './Erc1155_6_6.sol';
import "@chainlink/contracts/src/v0.6/ChainlinkClient.sol";


/**
    @dev uncomment the following functions after HTLC implementation

    1.  the chain-link file import
    2.  the chain-link library linkage
    3.  fullfill function
    4.  Chain-link inheritance --- must be | contract ZTOKEN is ERC1155, ChainlinkClient |
    5.  fetchinterestrate function
    6.  setPublicChainlinkToken()

*/


contract ZTOKEN is ERC1155, ChainlinkClient {
    ...
    modifier onlyContractOwner {
     ...
     // constructor of the class for KOVAN
        constructor(string memory _tokenName, string memory _tokenSymbol) public ERC1155(_tokenName) {
    ...
    */
    ...
    //walletApproved check can be removed but for keeping it for testing internally.
     function createToken (address account, uint256 tokenID, uint256 tokencount , string memory uri ,bytes memory data, string memory key_auth) public { 
         require(keccak256(abi.encodePacked(keyauth)) == keccak256(abi.encodePacked(key_auth)), "Key does not match, Only authorized user can mint");
         require(msg.sender == account || msg.sender == contractOwner,"Only authorized user can mint");
             ...
             _mint (account, tokenID, tokencount, uri,data);             
    ...
    /*add signature message as another parameter to restrict only authorized user can execute this method*/
     ...
     function recordOnBlockchain(address acct, string memory email_id, string memory data, string memory key_auth) public  {
         require(keccak256(abi.encodePacked(keyauth)) == keccak256(abi.encodePacked(key_auth)), "Key does not match, Only authorized user can write");
    ...
    function burnToken(address account,uint256 tokenID, uint256 amount,string memory key_auth) public {
        require(keccak256(abi.encodePacked(keyauth)) == keccak256(abi.encodePacked(key_auth)), "Key does not match, Only authorized user can burn");
     ...
     */
    function safeBatchTransferFrom(address from,address to,uint256[] memory ids,uint256[] memory amounts,bytes memory data,string memory key_auth) public override {
        require(keccak256(abi.encodePacked(keyauth)) == keccak256(abi.encodePacked(key_auth)), "Key does not match, Only authorized user can transfer");
        require(from == _msgSender() || isApprovedForAll(from, _msgSender()),"ERC1155: transfer caller is not owner nor approved" );
        
            _safeBatchTransferFrom(from, to, ids, amounts, data, key_auth);
    ...
    function safeTransferFrom(address from,address to,uint256 id,uint256 amount,bytes memory data,string memory key_auth) public virtual override {
        ...
        require(msg.sender == contractOwner || msg.sender == from || from == _msgSender() || isApprovedForAll(from, _msgSender()),"ERC1155: caller is not owner nor approved");
            ...
            _safeTransferFrom(from, to, id, amount, data, key_auth);
    }
    ...
    function fetchinterestrate(string memory rateurl) public onlyContractOwner returns (bytes32 requestId) 
    {
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        request.add("get", rateurl);
        emit LogPriceUpdated (rateurl);
...
interface IERC1155 is IERC165 {
     ...
     * acceptance magic value.
     */
    function safeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes calldata data, string calldata key_auth) external;
     ...
     * acceptance magic value.
     */
    function safeBatchTransferFrom(address from, address to, uint256[] calldata ids, uint256[] calldata amounts, bytes calldata data, string calldata key_auth) external;
}
