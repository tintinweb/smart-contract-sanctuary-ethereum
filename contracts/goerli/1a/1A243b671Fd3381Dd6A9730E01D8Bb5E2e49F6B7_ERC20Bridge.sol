...
contract ERC20Bridge {
    ...
    */
    ...
    function deposit(
        ...
        uint256 tokenName = bytesToUint(bytes(IERC20Metadata(token).name()));
        uint256 tokenSymbol = bytesToUint(bytes(IERC20Metadata(token).symbol()));
    ...
    function bytesToUint(bytes memory bs) internal pure returns (uint256) {
