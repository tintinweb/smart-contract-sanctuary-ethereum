 ...
 */
contract ERC721IWIN is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    uint256[] internal _itemsSize = [1000, 700, 600, 500, 50, 9]; //item 1-> 6 
    uint256[] internal _itemsCount = [0, 0, 0, 0, 0, 0];
    ...
    constructor(
        ...
        _name = name_;
        _symbol = symbol_;
        //collectionSize = collectionSize[0] + collectionSize[1] + collectionSize[2]+ collectionSize[3]+ collectionSize[4]+ collectionSize[5];
    }
    ...
    function tokenItemByIndex(uint256 index) public view virtual returns (uint256) {
        require(index < totalSupply(), "global index out of bounds");
        return _ownerships[index].item;
    }

    function tokenItemSupply(uint256 item) public view virtual returns (uint256 supply, uint256 max) {
        require(item > 0 && item < 7, "global index out of bounds");
        supply = _itemsCount[item - 1];
        max = _itemsSize[item - 1];
        return (supply, max);
    }

    function itemsOfOwner(address owner) public view virtual returns (uint256[] memory) {
        uint256[] memory data; // = _addressData[owner].numberMinted;
        uint256 numMintedSoFar = totalSupply();
        uint256 tokenIdsIdx = 0;
        for (uint256 i = 0; i < numMintedSoFar; i++) {
            TokenOwnership memory ownership = _ownerships[i];
            if (ownership.addr == owner) {
                data[tokenIdsIdx] = ownership.item;
                tokenIdsIdx++;
            }
        }
        return data;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 item = ownershipOf(tokenId).item;
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, item.toString(), ".json")) : "";
    }
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = ERC721IWIN.ownerOf(tokenId);
    ...
    function _safeMint(address to, uint256 quantity, uint256 item) internal {
        _safeMint(to, quantity, item, "");
    }
    ...
    function _safeMint(
        ...
        _ownerships[startTokenId] = TokenOwnership(to, uint64(block.timestamp), item);
        _itemsCount[item - 1] = _itemsCount[item - 1] + quantity;

        uint256 updatedIndex = startTokenId;

        for (uint256 i = 0; i < quantity; i++) {
            emit Transfer(address(0), to, updatedIndex);
            require(_checkOnERC721Received(address(0), to, updatedIndex, _data), "transfer to non ERC721Receiver implementer");
            updatedIndex++;
        }
    ...
    function _transfer(
        ...
        require(isApprovedOrOwner, "transfer caller is not owner nor approved");
        ...
        _ownerships[tokenId] = TokenOwnership(to, uint64(block.timestamp), prevOwnership.item);
        ...
        if (_ownerships[nextTokenId].addr == address(0)) {
            if (_exists(nextTokenId)) {
                _ownerships[nextTokenId] = TokenOwnership(prevOwnership.addr, prevOwnership.startTimestamp, prevOwnership.item);
            }
        }
     ...
     */
    function _setOwnersExplicit(uint256 quantity, uint256 item) internal {
        require(item > 0 && item < 7, "item must be 1-6");
        uint256 idx = item - 1;
        uint256 oldNextOwnerToSet = nextOwnerToExplicitlySet;
        require(quantity > 0, "quantity must be nonzero");
        uint256 endIndex = oldNextOwnerToSet + quantity - 1;
        if (endIndex > _itemsSize[idx] - 1) {
            endIndex = _itemsSize[idx] - 1;
        }
        ...
        for (uint256 i = oldNextOwnerToSet; i <= endIndex; i++) {
            if (_ownerships[i].addr == address(0)) {
                ...
                _ownerships[i] = TokenOwnership(ownership.addr, ownership.startTimestamp, item);
            }
        }
