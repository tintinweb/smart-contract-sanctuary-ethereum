...
contract GoblinLootWTF is ERC721A, Ownable, Pausable, PullPayment {
    ...
    function getPrice(uint256 quantity) external view returns (uint256) {
        ...
        uint256 price = 0;
        uint newTotal = totalMinted.add(quantity);

        if (newTotal > freeMintAmount) {
            if (totalMinted < freeMintAmount ) {
                uint payableQuantity = newTotal - freeMintAmount;
    ...
    function mint(uint256 quantity) external payable whenNotPaused {
        uint256 price = this.getPrice(quantity);
        ...
        require(totalSupply().add(quantity) < MAX_SUPPLY, "Not dat many iz left!");
