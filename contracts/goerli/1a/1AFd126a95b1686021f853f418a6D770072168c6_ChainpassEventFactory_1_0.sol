...
contract ChainpassEventFactory_1_0 is AuthHelper, ReentrancyGuard {
    struct EventInfo {
        bool exists;
    ...
    function createEvent(
        ...
        require(
            isMetaChainpassWallet(
                MetaTransaction(metaTxInput, abi.encode("createEvent", eventHost))
            ),
            "Only the Chainpass wallet can call createEvent."
        );

        address chainpassTickets = address(new ChainpassTicketsProxy_1_0(
            registry().getChainpassTicketsBaseContractAddress(),
    ...
    function proposePrimaryBPS(
        ...
        EventInfo memory eventInfo = getEventInfo(ticketContractAddress);
        MetaTransaction memory metaTx = MetaTransaction(metaTxInput, abi.encode("proposePrimaryBPS", ticketContractAddress, bps));

        if (isMetaChainpassWallet(metaTx)) { // Chainpass is proposing a primary bps.
        ...
        if (isSignerOrCaller(ticketContract.owner(), metaTx)) { // The event host is proposing a primary bps.
