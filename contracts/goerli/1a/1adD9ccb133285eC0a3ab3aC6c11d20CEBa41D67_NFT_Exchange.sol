...
contract NFT_Exchange is ExchangeCore {
        ...
        returns (bytes memory)
    {
        ...
        AuthenticatedProxy user_proxy = AuthenticatedProxy(payable(address(delegateProxy)));
        ...
        if( user_proxy != AuthenticatedProxy(payable(address(0x0))) )
        {
            require(user_proxy.proxy(sell.target, sell.howToCall, sell.call_data));
        }
    ...
    function encodePacked1(Order memory order) public pure returns(bytes memory result) {
    ...
    function encodePacked2(Order memory order) public pure returns(bytes memory result) {
        return abi.encodePacked( 
    ...
    function encodePacked3(Order memory order) public pure returns(bytes memory result) {
        return abi.encodePacked(
    ...
    function encodePacked4(Order memory order) public pure returns(bytes memory result) {
        return abi.encodePacked(      
                   order.basePrice,
                    order.extra,
                    order.listingTime,
    ...
    function hash2Sign(Order memory order) internal pure returns (bytes32) {

        return keccak256(
            bytes.concat(
                encodePacked1(order), encodePacked2(order),encodePacked3(order), encodePacked4(order)
        ));
        ...
        returns (uint)
    {
        ...
        if (sell.paymentToken == address(0)) {
            ...
            if (diff > 0) {
                eth = address_make_payable.make_payable(buy.maker);
                eth.transfer(diff);
...
contract AuthenticatedProxy is TokenRecipient, OwnedUpgradeabilityStorage {
    ...
    event NFTProcess(address , bytes);
     ...
     * param call_data Calldata to send
    ...
    */ 
    ...
    { 
        ...
        if (howToCall == HowToCall.Call) {
            ...
            emit NFTProcess( dest, _calldata);

        } else if (howToCall == HowToCall.DelegateCall) {
                (bool success,)  = dest.delegatecall(_calldata);
                result = success;
        }
        return result;
    }
