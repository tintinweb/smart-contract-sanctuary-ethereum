...
contract LPStakingV4 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function _getReward(
        stakeDetails memory stakeDetail,
        uint256 _stake,
        uint256 _ethUSD,
        uint256 _mpwrUSD,
        uint256 _totalLpStake,
        ...
        uint256 apr = dynamic
            ...
            : stakeDetail.APR;
        reward = dynamic
            ? _stake.mul(apr).mul(_timestamp.sub(stakeDetail.lastRewardCalculated)).div(APRTime.mul(100).mul(1000)) /
                _ethUSD * PRECISION_FACTOR
            : _stake.mul(apr).mul(_timestamp.sub(stakeDetail.lastRewardCalculated)).div(APRTime.mul(100).mul(1000)) /
                _mpwrUSD * PRECISION_FACTOR;
