...
contract PawnShop is Ownable, IERC721Receiver {
    ...
    event LoanCreated(uint id, address owner, address tokenAddress, uint tokenCnt, uint price);
    event LoanUpdated(uint id, uint updatePrice);
    ...
    function adminForceClose(uint256 _id) public onlyOwner {
        for (uint256 i = 0; i < loans[_id].tokenCnt; i++) {
            ERC721(loans[_id].tokenAddress).transferFrom(
                address(this),
                msg.sender,
                loans[_id].tokenIds[i]
            );
        }
    ...
    function getSigner(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(_tokenAddress, _tokenIds, _tokenCnt, _amount)
        );
    ...
    function openALoan(
        ...
        emit LoanCreated(totalCnt, msg.sender, _tokenAddress, _tokenCnt, _amount * 10**ERC20(usdtAddress).decimals());
    ...
    function endLoan(uint256 _id) public {
        ...
        uint256 fee = (loans[_id].price * (monthlyFee + originFee)) /
            10000 +
            loans[_id].price;
        ERC20(usdtAddress).transferFrom(msg.sender, address(this), fee);
        for (uint256 i = 0; i < loans[_id].tokenCnt; i++) {
