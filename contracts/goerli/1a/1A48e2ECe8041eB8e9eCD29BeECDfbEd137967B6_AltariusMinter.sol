...
contract AltariusMetadata is AccessControl {
    ...
    event MetadataCreated(uint256 indexed id, bytes32 name);
    ...
    function createMetadata(
        ...
        uint256 id = tokensMetadataCounter.current();
        tokensMetadata[id] = metadata;
        tokensName[id] = name;
        tokensImages[id] = images;
        emit MetadataCreated(id, name);
    }
...
contract AltariusMinter is VRFConsumerBaseV2, AccessControl {
    ...
    mapping(uint256 => mapping(uint256 => mapping(uint256 => mapping(bool => uint256[]))))
        public cardIds; // type => level => rarity => holographic cardIds
    ...
    mapping(address => uint256) public freePacksMinted; // minter => freePacksMinted

    uint256 public freePacksAvailable;
    uint256 public maxFreePacksPerAddress;
    ...
    event PackMinted(address indexed minter, uint256[] ids);
    ...
    constructor(
        ...
        uint256 _freePacksAvailable,
        uint256 _maxFreePacksPerAddress,
        uint256 _packPrice
        ...
        freePacksAvailable = _freePacksAvailable;
        maxFreePacksPerAddress = _maxFreePacksPerAddress;
    ...
    function setFreePacksAvailable(
        uint256 _freeMintPacksCount
    ) external onlyRole(CONFIGURATOR_ROLE) {
        freePacksAvailable = _freeMintPacksCount;
    }
    ...
    function setCardIds(
        ...
        cardIds[cardType][level][rarity][holographic] = _cardIds;
    }
    ...
    function getCardIds(
        ...
        return cardIds[cardType][level][rarity][holographic];
    }
    ...
    function mintFreePack() external {
        require(freePacksAvailable > 0, "No free packs left");
        require(
            freePacksMinted[msg.sender] < maxFreePacksPerAddress,
            "Max free packs minted"
        );
        freePacksAvailable--;
        freePacksMinted[msg.sender]++;
    ...
    function requestPack() external payable returns (uint256 requestId) {
        require(msg.value == packPrice, "Insufficient funds");
        require(
    ...
    function mintPayablePack() external {
        require(pendingVrfRequestIds[msg.sender] != 0, "No pending request");
        uint256 requestId = pendingVrfRequestIds[msg.sender];
        require(vrfRequests[requestId].fulfilled, "Request not fulfilled");
        vrfRequests[requestId].minted = true;
        pendingVrfRequestIds[msg.sender] = 0;
        mintPack(vrfRequests[requestId].randomWord);
    }
    ...
    function mintPack(uint256 random) internal {
        ...
        emit PackMinted(msg.sender, mintableCardIds);
    }
    ...
    function getRandomCard(
        ...
        if (cardIds[cardType][level][rarity][holographic].length == 0) {
            holographic = false;
        }
        uint256[] memory randomCardIds = cardIds[cardType][level][rarity][
            holographic
        ];
    ...
    // Check https://stackoverflow.com/questions/70104391/generating-several-random-numbers-in-one-transaction-solidity
    function expand(
        uint256 randomValue,
        uint256 n
