...
error PublicClosed();
error ExceedsMaxMintsPublic(uint256 currentBalance, uint256 maxLimit);
...
contract Primordials is ERC721A, Ownable, VRFConsumerBaseV2 {
  ...
  uint256 public constant PUBLIC_SALE_PRICE = 0.0011 ether;
  uint256 public constant ALLOWLIST_SALE_PRICE = 0.001 ether;
  ...
  uint256 public revealTime; // remove once we finalize the revealtime along with setter function.
  ...
  {
    console.log('Primordials :: Contract constructor init...');
  ...
  function teamMint() public onlyOwner {
    ...
    teamMinted = true;
    _safeMint(owner(), MAX_TEAM_MINT); // TODO: split team mint into batches.
  }

  function allowlistVerify(bytes32[] memory _merkleProof)
    public
    view
    returns (bool)
  {
  ...
  function allowlistMint(bytes32[] memory _merkleProof, uint256 _quantity)
    external
    payable
    callerIsUser
    whenNotPaused
    mintQCheck(_quantity)
  {
    ...
    if (totalAllowlistMint[msg.sender] + _quantity > MAX_ALLOWLIST_MINT)
      revert ExceedsMaxMintsAllowlist(
        totalAllowlistMint[msg.sender],
        MAX_ALLOWLIST_MINT
      );
    if (msg.value < (ALLOWLIST_SALE_PRICE * _quantity))
      revert InsufficientFunds(msg.value, ALLOWLIST_SALE_PRICE * _quantity);

    require(
      allowlistVerify(_merkleProof),
    ...
    mintQCheck(_quantity)
  {
    ...
    if (msg.value < (PUBLIC_SALE_PRICE * _quantity))
      revert InsufficientFunds(msg.value, PUBLIC_SALE_PRICE * _quantity);
  ...
  // function getRevealTime() public pure returns (uint256) {
  // 	return 1653071400; // May 20, 2022 @ 6:30:00 pm
  // }
