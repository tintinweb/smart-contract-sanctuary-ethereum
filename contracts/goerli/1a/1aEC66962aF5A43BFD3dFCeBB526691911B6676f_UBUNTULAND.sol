...
contract UBUNTULAND is ERC721, Ownable, Pausable {
    ...
    */
    ...
    /**
    * @dev Mints 1 NFT to specified address for testnet use
    */
    function testnetMintToAddressSingleLandNFT(address receiverAddress) public {
        require(!_exists(numMinted), "That token ID has already been minted. Please try again.");
        _safeMint(receiverAddress, numMinted);
        _setTokenURI(numMinted);
        ++numMinted;

    }

    /**
    * @dev Mints batch of n number of NFTs for testnet use
    */
    function testnetMintToAddressBatchOfLandNFT(uint256 quantity, address receiverAddress) external {
        for (uint i=0; i < quantity; i++) {
            testnetMintToAddressSingleLandNFT(receiverAddress);
        }
    }
