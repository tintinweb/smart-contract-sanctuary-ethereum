...
// File contracts/nft-collateral/interfaces/IUNIV3NFT.sol
...
// File contracts/nft-collateral/interfaces/IPositionValue.sol
...
// File contracts/nft-collateral/interfaces/IAggregatorV3Interface.sol
...
// File contracts/nft-collateral/price/UniLPPriceHelper.sol
...
contract UniLPPriceHelper is Ownable, IPriceHelper {
    ...
    modifier validNFTIndex(address nftContract, uint256 nftIndex) {
        ...
        require(
            IUNIV3NFT(nftContract).ownerOf(nftIndex) != address(0),
            "invalid_nft"
        );
        _;
    }
    ...
    function getNFTValueUSD(address _nftContract, uint256 _nftIndex)
        ...
        returns (uint256)
    {
        (, , address token0, address token1, uint24 fee, , , , , , , ) = nft
            .positions(_nftIndex);

        address poolAddr = positionValue.computeAddress(
            nft.factory(),
            token0,
            token1,
            fee
        ...
        (uint amount0, uint amount1) = positionValue.total(
            nft,
            _nftIndex,
            sqrtRatioX96
        );
        uint256 totalValue;
        if (
            whiteList[token0] && address(tokenAggregator[token0]) != address(0)
        ) {
            totalValue += amount0 * tokenPrice(tokenAggregator[token0]);
        }
        if (
            whiteList[token1] && address(tokenAggregator[token1]) != address(0)
        ) {
    ...
    function tokenInfo(address _nftContract, uint256 _nftIndex)
        public
        view
        validNFTIndex(_nftContract, _nftIndex)
    ...
    function tokenPrice(IAggregatorV3Interface aggregator)
        public
        view
        returns (uint256)
    {
