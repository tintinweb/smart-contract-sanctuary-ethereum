...
contract Slippery_Fish_Pass is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public MAX_SlipperyFishPass_PRESALE_CAP;
  ...
  constructor(
    ...
    uint256 _maxSlipperyFishPasspresaleCap
    ...
    MAX_SlipperyFishPass_PRESALE_CAP = _maxSlipperyFishPasspresaleCap;
  }
  ...
  function setPreSaleCap(uint256 _newPresaleCap) external onlyOwner {
    MAX_SlipperyFishPass_PRESALE_CAP = _newPresaleCap;
  }
  ...
  function giftMint(uint256 _quantity, address _giftAddress) external onlyOwner {
    ...
    _giftMintSlipperyFishPass(_quantity, _giftAddress);
  }

  function _giftMintSlipperyFishPass(uint256 _quantity, address _giftAddress) internal {
    require(_quantity > 0, "You must mint at least 1 SlipperyFishPass");
    require(_totalMinted() + _quantity <= MAX_SlipperyFishPass, "This gift would exceed max supply of SlipperyFishPass");
  ...
  function presaleMint(uint256 _quantity, bytes calldata _presaleSignature) external payable nonReentrant {
    ...
    require(_quantity <= MAX_SlipperyFishPass_PRESALE_CAP, "This would exceed the maximum SlipperyFishPass you are allowed to mint in presale");
    require(msg.value >= SlipperyFishPass_PRICE * _quantity, "The ether value sent is not correct");
    require(presaleAddressMintCount[msg.sender].add(_quantity) <= MAX_SlipperyFishPass_PRESALE_CAP, "This purchase would exceed the maximum SlipperyFishPass you are allowed to mint in the presale");

    presaleAddressMintCount[msg.sender] += _quantity;
    _safeMintSlipperyFishPass(_quantity);
  }
  ...
  function _safeMintSlipperyFishPass(uint256 _quantity) internal {
    require(_quantity > 0, "You must mint at least 1 SlipperyFishPass");
    require(_quantity <= MAX_SlipperyFishPass_PER_PURCHASE, "Quantity is more than allowed per transaction.");
    require(_totalMinted() + _quantity <= MAX_SlipperyFishPass, "This purchase would exceed max supply of SlipperyFishPass");
  ...
  function withdraw() public onlyOwner {
    ...
    payable(0x9B8AF273FB41a2a8893591c88F142977b80e4E77).transfer((balance*10)/100);
    payable(0x9B8AF273FB41a2a8893591c88F142977b80e4E77).transfer((balance*90)/100);
  }
