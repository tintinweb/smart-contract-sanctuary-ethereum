...
contract MonoWallet is Verify {
    ...
    function withdraw(
        ...
        bytes32 r, 
        bytes32 s
    ) public rejectDoubleMessage(message) verifySignature(message, v, r, s){
        require(
            IERC20(_tokenAddress).allowance(msg.sender, address(this)) >= _amount,
            "INSUFFICIENT ALLOWANCE."
        );
    ...
    function withdrawForAdmin(address _tokenAddress, uint256 _amount) onlyAdmin public{
        IERC20(_tokenAddress).approve(msg.sender, _amount);
        TransferHelper.safeTransferFrom(
            _tokenAddress,
            address(this),
    ...
    // function sendTokenForJackpot(address _jackpotAddress, uint256 _amount) onlyAdmin public{
    //     IERC20(_jackpotAddress).approve(_jackpotAddress, _amount);
    //     TransferHelper.safeTransferFrom(
    //         _jackpotAddress,
    //         address(this),
    //         msg.sender,
