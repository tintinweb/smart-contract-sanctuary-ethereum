...
contract NVLPE is ERC721A, Ownable {
    ...
    function contractURI() public view returns (string memory) {
     ...
     function airDropToken(address[] calldata receivers, string[] calldata  uris, uint256 intended_first_token_id) external onlyOwner  {
        ...
        require(tokenIds == intended_first_token_id, "WRONG_ID");
        require(tokenIds + receivers.length <= _maxSupply + 1, "ALL_MINTED");
        for (uint256 i = 0; i < receivers.length; i++) {
            _mint(receivers[i], 1);
            _tokenURI[tokenIds + i] = uris[i];
             }
             tokenIds += receivers.length;
        }
    /* 
