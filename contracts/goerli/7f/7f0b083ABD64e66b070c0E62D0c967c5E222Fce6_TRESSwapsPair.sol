...
contract SwapsERC20 {
    ...
    string public constant name = "Tres Leches Swap";
...
contract TRESSwapsPair is SwapsERC20 {
        ...
        external
    {
        require(
            ...
            "TresSwapsPair: ALREADY_INITIALIZED"
        );
...
contract TRESSwapsFactory {
    ...
    constructor(
        ...
        if (_feeToSetter == ZERO_ADDRESS) {
            revert("TresSwapsFactory: INVALID_INPUT");
        }
        ...
        bytes memory bytecode = type(TRESSwapsPair).creationCode;
        ...
        returns (address pair)
    {
        ...
        require(
            token0 != ZERO_ADDRESS,
            "TresSwapsFactory: ZERO_ADDRESS"
        );

        require(
            getPair[token0][token1] == ZERO_ADDRESS,
            "TresSwapsFactory: PAIR_ALREADY_EXISTS"
        );
...
contract FactoryCodeCheck {
    ...
    {
        return keccak256(
            type(TRESSwapsFactory).creationCode
        );
    }
    ...
    {
        return keccak256(
            type(TRESSwapsPair).creationCode
        );
    }
}