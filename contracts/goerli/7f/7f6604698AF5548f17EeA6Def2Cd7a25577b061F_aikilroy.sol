...
contract aikilroy is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "KILR";
    ...
    uint256 private constant _maxTxAmountPercentage = 30; // 3%
    ...
    function _beforeTransfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (_isCooldownEnabled && !_isExcludedFromCooldown[from] && !_isExcludedFromCooldown[to]) {
            require(_lastTxBlock[from] < block.number, "You can only transfer once per block");
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        // Chekck if swap is made by the owner
    ...
    function openTrading(uint256 amount) external payable onlyOwner {
        ...
        _isExcludedFromCooldown[_uniswapV2Router.WETH()] = true;
        _isExcludedFromCooldown[address(_uniswapV2Router)] = true;
