  ...
  DefaultOperatorFilterer
{
  ...
  bytes32 private constant _TYPEHASH =
    keccak256(
      "Attribute(uint256 tokenId,string name,uint256 gender,uint256 height,uint256 weight,string bloodType,string hometown,uint256 character,string madeFrom)"
    );
  ...
  function createAttribute(
    ...
    return
      Attribute(
        attribute.tokenId,
        attribute.name,
        (attribute.gender + inheritedChildAttribute.gender) / 2,
        (attribute.height + inheritedChildAttribute.height) / 2,
        (attribute.weight + inheritedChildAttribute.weight) / 2,
        attribute.bloodType,
        attribute.hometown,
        (attribute.character + inheritedChildAttribute.character) / 2,
        attribute.madeFrom
      );
  }
