...
contract StakingFacet {
    ...
    function initDAOStaking(address _govToken, address _rewards) public {
        require(_govToken != address(0), "govToken address must not be 0x0");

        LibStakingStorage.Storage storage ds = LibStakingStorage.getPointer();

        require(!ds.initialized, "DAOStaking: already initialized");
        LibOwnership.enforceIsContractOwner();

        ds.initialized = true;

        ds.govToken = IERC20(_govToken);
    ...
    function deposit(uint256 amount) public {
        ...
        LibStakingStorage.Storage storage ds = LibStakingStorage.getPointer();
        uint256 allowance = ds.govToken.allowance(msg.sender, address(this));
        ...
        ds.govToken.transferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint256 amount) public {
        ...
        _updateUserBalance(ds.userStakeHistory[msg.sender], balance - amount);
        _updateLockedGovToken(govTokenStakedAtTs(block.timestamp) - amount);
        ...
        ds.govToken.transfer(msg.sender, amount);
    ...
    function govTokenStakedAtTs(uint256 timestamp) public view returns (uint256) {
        return _checkpointsBinarySearch(LibStakingStorage.getPointer().govTokenStakedHistory, timestamp);
    }
    ...
    function delegatedPowerAtTs(address user, uint256 timestamp) public view returns (uint256) {
        return _checkpointsBinarySearch(LibStakingStorage.getPointer().delegatedPowerHistory[user], timestamp);
    }
    ...
    function _stakeMultiplier(LibStakingStorage.Stake memory stake, uint256 timestamp) internal pure returns (uint256) {
    ...
    function _updateLockedGovToken(uint256 amount) internal {
        ...
        if (ds.govTokenStakedHistory.length == 0 || ds.govTokenStakedHistory[ds.govTokenStakedHistory.length - 1].timestamp < block.timestamp) {
            ds.govTokenStakedHistory.push(LibStakingStorage.Checkpoint(block.timestamp, amount));
        } else {
            LibStakingStorage.Checkpoint storage old = ds.govTokenStakedHistory[ds.govTokenStakedHistory.length - 1];
            old.amount = amount;
        }
    }
}
