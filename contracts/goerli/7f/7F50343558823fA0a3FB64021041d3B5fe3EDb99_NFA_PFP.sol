...
contract NFA_PFP is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer, ReentrancyGuard {
    ...
    mapping(address => mapping(uint16 => bool)) public hasClaimed;
    ...
    function setContract(address _contract, bool _active) external onlyOwner {
        require(_contract != address(0), "Bad contract");
        if (_active) {
            approvedContracts[_contract] = true;
        } else {
            delete approvedContracts[_contract];
        }
    }
    ...
    function updateSupply(uint16 _maxSupply) external onlyOwner {
    ...
    function updateClaimState(bool _claimIsActive) external onlyOwner {
        claimIsActive = _claimIsActive;
    }

    function mint(Claim[] memory _claims) public {
        uint256 _quantity;
        require(claimIsActive, "Claim inactive");
        for (uint256 i; i < _claims.length; i++) {
            require(approvedContracts[_claims[i].addr], "Invalid contract");
            for (uint256 j; j < _claims[i].ids.length; j++) {
                require(ERC721A(_claims[i].addr).ownerOf(_claims[i].ids[j]) == msg.sender, "Not owner");
                require(!hasClaimed[_claims[i].addr][_claims[i].ids[j]], "Already claimed");
                hasClaimed[_claims[i].addr][_claims[i].ids[j]] = true;
            }
            _quantity += _claims[i].ids.length;
        }
