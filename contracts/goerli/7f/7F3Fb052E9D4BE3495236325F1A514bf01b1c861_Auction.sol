...
contract Auction {
    ...
    /**
    @notice user struct in a mapping stores each user address,uint,uint
    @param amountBid, mapping takes in a auction id and returns the amount user has bid in that auction
    ///key is is Auction ID
    ///value is user bid amount in that auction
     */
    struct User {
    ...
    ///@notice Auctions, key is Auction ID, value is Auction struct
    mapping(uint=>auctionX) public Auctions;
    ///@notice bids, key is user address, value is User struct
    ...
    ///@notice resolveAuction
    ///@param _ID - id of the auction
    ///used by owner to resolve an auction/complete it
    function resolveAuction(uint _ID) external onlyOwner {
        auctionX memory auction = Auctions[_ID];
