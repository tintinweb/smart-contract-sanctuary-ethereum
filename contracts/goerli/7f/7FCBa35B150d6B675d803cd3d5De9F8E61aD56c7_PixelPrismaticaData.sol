    ...
    function getSVG_RainbowLight() private view returns (string memory) {
        ...
        for(uint256 i_y = 0; i_y < num_rect_y; i_y++) {
            for(uint256 i_x = 0; i_x < num_rect_x; i_x++) {
                uint256 rect_x = i_x * rect_width;
                uint256 rect_y = i_y * rect_height;
                ...
                content = string.concat(content, "<animate attributeName=\"fill\" values=\"", sValues, "\" dur=\"", dur, "\" repeatCount=\"indefinite\"/>", "\n");
