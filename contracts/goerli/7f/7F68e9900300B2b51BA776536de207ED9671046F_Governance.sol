...
interface IStaking {
  ...
  function mapUserInfo(address voter)
    external
    view
    returns (
      uint256,
...
contract Governance {
  ...
  constructor() {
    ...
    staking = address(0x58aA9d2399cb9b31285e8b0F36A51a3cb7a6fFf7);
    marketing = address(0x760c5A41b67BE0b8E208Da61c9654d5aad1e92f2);
  }

  function propose(uint256 fundRequest, string memory description)
    public
    returns (uint256)
  {
    ...
    require(
      IStaking(staking).checkHighestStaker(msg.sender),
      "Governor::propose: only top staker"
    );
    require(
      fundRequest <= 10000 * 10**18,
      "Governor::propose: max 10000 USDC limit"
    );
    require(fundRequest >= 1 * 10**6, "Governor::propose: min 1 USDC limit");
    require(
      IERC20(USDC).balanceOf(marketing) >= fundRequest,
      "Governor::propose: insufficient fund in marketing wallet"
    );
  ...
  function claimFund(uint256 proposalId) external {
    ...
    require(
      ...
      "Governor::claimFund: proposal is not succeeded"
    );
    require(
      proposal.claimed == false,
      "Governor::claimFund: Fund already claimed"
    );
  ...
  function votingPercent(uint256 proposalId) public view returns (uint256) {
    ...
    uint256 totalVote = proposal.forVotes +
