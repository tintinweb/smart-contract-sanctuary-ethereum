...
contract NRFFCollectible is ERC721, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    Counters.Counter private _referralCodes;

    uint256 public constant MAX_TOKENS = 7777;

    bytes32 private _merkleRoot;

    uint256 public presaleDate = 1649944146;
    uint256 public hatchDate = 1650288170;
    bool public referralsPaid = false;

    uint256 public TOKEN_PRICE = 100000000000000000; // 0.1 ETH
    uint256 public constant PRESALE_PRICE = 80000000000000000; //0.08 ETH
    uint256 public constant REFERRAL_AMOUNT = 10000000000000000; //0.01 ETH
    uint256 public constant MAX_MINTS = 6;
    uint256 public constant MAX_PRESALE_MINTS = 1;

    uint256 public constant MAX_RESERVED = 8;
    ...
    function mintReserved(uint256 numberOfTokens) external onlyOwner {
        require(
            reservedTokens + numberOfTokens <= MAX_RESERVED,
        ...
        onlyEOA
    {
        ...
        require(numberOfTokens <= MAX_MINTS, "Can only mint 6 at a time");
    ...
    function isPresaleOpen() public view returns (bool) {
        if (block.timestamp >= presaleDate && block.timestamp <= hatchDate) {
            return true;
        } else {
            return false;
        }
