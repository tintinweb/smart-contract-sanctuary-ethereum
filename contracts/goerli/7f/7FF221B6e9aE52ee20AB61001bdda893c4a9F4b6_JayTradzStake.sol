...
function payout(uint256 listId, address _addr, uint256 _amount, bool cashPayout, uint8 _prize) external {
    ...
    if(amount <= totalAdminPoints) {
        ...
        qTimers[id].termStart = block.timestamp;
            ...
            require(amount <= adminPoints[id], "Insufficient admin balance. Please wait until your next claim period");
...
function approveDashboardAccess(address _addr) external onlyOwner {
