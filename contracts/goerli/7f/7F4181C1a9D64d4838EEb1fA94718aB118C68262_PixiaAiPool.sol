    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _releaseTokenAndNfts(
        ...
        uint16 i;
        for (; i < nftIds_.length; i++) stakingInfo.nfts.add(nftIds_[i]);
    ...
    function unlockToken(
        ...
        if (newLockTime_ > 0) {
            ...
            uint64 nftCount = uint64(stakingInfo.nfts.length());
            // Calculate the boosted amount from the new lock time
            boostedAmount_ =
                (stakedAmount_ * getBooster(nftCount, newLockTime_)) /
                1e6;
            stakingInfo.boostedAmount = boostedAmount_;
            boostedSupply += boostedAmount_;
    ...
    /// @notice Withdraw staked tokens and nfts without any rewards in case of emergency
    function emergencyWithdraw(bytes32 stakingId_) external nonReentrant {
        StakingInfo storage stakingInfo = _stakingInfo[stakingId_];
        UserInfo storage userInfo = _userInfo[_msgSender()];
        ...
        if (stakingInfo.account != _msgSender()) revert Unpermitted();
        if (stakedAmount_ == 0) revert InsufficientAmount();
        // Even emergency withdraw can not be done when locked
        if (
            stakingInfo.lockTime > 0 &&
            stakingInfo.lockTime + stakingInfo.lastDepositAt > block.timestamp
        ) revert StillLocked();
        ...
        uint256[] memory nftIds = stakingInfo.nfts.values();
        uint64 nftAmount = uint64(nftIds.length);
        // Withdraw user boosting nfts
        if (nftAmount > 0) {
            uint256 i;
            IERC721Upgradeable _boostingNft = IERC721Upgradeable(boostingNft);
        ...
        )
    {
        ...
        isLocked = (lockTime + lastDepositAt) > block.timestamp;
    }
