...
contract GalleyRouter {
    function swapNFTsForToken(
        ...
        for (uint256 i; i < numSwaps; ) {
            ...
            for (uint256 j; j < numNfts; ) {
                nft.safeTransferFrom(msg.sender, address(this), swapList.nftIds[j]);
                unchecked {
                    ++j;
                }
            }
        ...
        )
    {
        ...
        for (uint256 i; i < numSwaps; ) {
            ILSSVMRouter.PairSwapSpecific memory swapList = firstParams.nftToTokenTrades[i].swapInfo;
            IERC721 nft = swapList.pair.nft();
            uint256 numNfts = swapList.nftIds.length;
            for (uint256 j; j < numNfts; ) {
                nft.safeTransferFrom(msg.sender, address(this), swapList.nftIds[j]);
        ...
        for (uint256 i; i < numSwaps2; ) {
            ILSSVMRouter.PairSwapSpecific memory swapList = secondParams.nftToTokenTrades[i].swapInfo;
            IERC721 nft = swapList.pair.nft();
            uint256 numNfts = swapList.nftIds.length;
            for (uint256 j; j < numNfts; ) {
                nft.safeTransferFrom(msg.sender, address(this), swapList.nftIds[j]);
