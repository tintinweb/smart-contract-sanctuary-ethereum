...
abstract contract BaseJumpRateModel is InterestRateModel {
     ...
     * @param _kink The utilization point at which the jump multiplier is applied
     */
    function updateJumpRateModelInternal(uint baseRatePerYear, uint multiplierPerYear, uint jumpMultiplierPerYear, uint _kink) internal {
        baseRatePerBlock = baseRatePerYear / BLOCKS_PER_YEAR;
        multiplierPerBlock = (multiplierPerYear * BASE) / (BLOCKS_PER_YEAR * _kink);
...
import {BaseJumpRateModel} from "./abstracts/BaseJumpRateModel.sol";
import {InterestRateModel} from "./abstracts/InterestRateModel.sol";
