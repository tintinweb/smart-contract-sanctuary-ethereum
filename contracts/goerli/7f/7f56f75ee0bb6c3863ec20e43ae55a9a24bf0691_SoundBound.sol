...
//    uint256 private whitelistPurchasePrice = 0 wei;
    ...
    event logNFTCreatedFromSubdomain(uint256 indexed curTokenId, address indexed ownerWalletAddress, string subdomain, uint32 genre);
    event logNFTCreatedFromOwner(uint256 indexed curTokenId, address indexed ownerWalletAddress, string subdomain, uint32 genre);
    ...
    function mintNFTFromSubdomain(string memory subdomain, uint32 genreId) external payable {
        require(!paused, "The contract has been paused");
        ...
        domainToTokenId[name] = newItemId;
        purchaseStatus[name] = true;
        emit logNFTCreatedFromSubdomain(newItemId, msg.sender, name, genreId);
    }

    function mintNFTByAdmin(string[] memory domainMap, address[] memory addressMap) external onlyMember {
        require(!paused, "The contract has been paused");
        for (uint i=0; i<domainMap.length; i++) {
            ...
            domainToTokenId[name] = newItemId;
            purchaseStatus[name] = true;
            emit logNFTCreatedFromOwner(newItemId, msg.sender, name, 0);
        }
    }
