...
contract MergeGatorsMock is MergeGators {
    ...
    string constant collection_ = "MergeGators";
    string constant symbol_ = "MGT";
...
contract MergeGators is
    ...
    //COLLECTION SIZES
    uint constant COLLECTION_SIZE = 10000;
    uint constant COLLECTION_JOKER_SIZE = 1000;
    uint public collection_size_WL = 999;
    //LIMITS
    uint constant TOKENS_PER_PUBLIC = 5;
    uint constant TOKENS_PER_TX = 3;
    uint public tokens_per_WL = 10;
    ...
    // 0 for off - 1 for on
    mapping(address => uint) public _whitelistedAddr;
    ...
    uint32 private constant NUM_WORDS = 490;
    uint16 private constant REQUEST_CONFIRMATIONS = 3;
    bytes32 private immutable gasLane;
    uint256 internal constant MAX_CHANCE_VALUE = 100;
    uint256 internal MIN_CHANCE_VALUE = 70;
    uint256 internal MIN_CHANCE_VALUE_PRIZE = 90;

    mapping(uint256 => RequestStatus) internal vrf_requests; /* requestId --> requestStatus */
    ...
    function verify(address whitelisted, uint8 v, bytes32 r, bytes32 s) internal returns (bool) {
        address recoveredAddress = ecrecover(
            keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR(), keccak256(abi.encode(keccak256("Verify(address whitelisted)"), whitelisted )))),
            v,
            r,
            s
        );

        if (_whitelistedAddr[recoveredAddress] == 1) {
            return true;
        } else {
            return false;
        }

        emit Verified(recoveredAddress);
    }
    ...
    function alligatorTotalSupply() public view virtual returns (uint256) {
   ...
   /**
    ...
    function _getAux(address owner_) internal view returns (uint64) {
        return _addressData[owner_].aux;
    }
    ...
    //permission for hub Conntract nned to be implemented
    function mergerSetTokenURI(uint256 tokenId, string memory _tokenURI) external onlyMerger {
    ...
    function publicMint(address to, uint256 _quantity) public {
        if (saleStatus == SaleStatus.PAUSED && saleStatus == SaleStatus.PRESALE) revert callErr();
        if (_quantity > TOKENS_PER_PUBLIC) revert callErr();
        if (_quantity > TOKENS_PER_TX) revert callErr();

        if (saleStatus == SaleStatus.PUBLIC) {
            VRFMint(to, _quantity);
        }
        if (saleStatus == SaleStatus.COMMON_SUPPLIED) {
            jokerMint(to);
        }
        if (saleStatus == SaleStatus.JOKER_SUPPLIED) {
            alligatorMint(to, _quantity);
        }
    }

    function VRFMint(address to, uint256 _quantity) internal {
        if (saleStatus == SaleStatus.PAUSED) revert callErr();

        uint256[] memory _randomWords = vrf_requests[lastRequestId].randomWords;

        uint innerModdedRange = block.timestamp % 490;

        uint moddedRng = _randomWords[innerModdedRange] % MAX_CHANCE_VALUE;

        uint256[2] memory chanceArracy = getChanceArray();
        uint256[2] memory chanceArrayPrize = getChanceArrayPrize();
       ...
       if (moddedRng < chanceArracy[0]) {
            ...
            jokerMint(to);
            alligatorMint(to, _quantity - 1);
        }

        if (moddedRng > chanceArrayPrize[0]) {
            alligatorMint(to, 1);        
        }

        RandomCounter.increment(1);
        if (RandomCounter.current() == 499) {
            RandomCounter.reset();
        }
    }  
    ...
    function requestRandomWords() external onlyOwner returns (uint256 requestId) {
        ...
        vrf_requests[requestId] = RequestStatus({randomWords: new uint256[](0), exists: true, fulfilled: false});
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        if (!vrf_requests[_requestId].exists) revert callErr();
        vrf_requests[_requestId].fulfilled = true;
        vrf_requests[_requestId].randomWords = _randomWords;
        emit RequestFulfilled(_requestId, _randomWords);
    }

    // function random(address to) internal returns(uint256){
    //     // maximum of 500 VRF rand words with each request which is set to lastRequestId.
    //     uint256 id = RandomCounter.current();
    //     return uint256(keccak256(abi.encode(to, _randomWords[id])));
    // }

    function getChanceArray() public view returns (uint256[2] memory) {
        return [MIN_CHANCE_VALUE, MAX_CHANCE_VALUE];
    }
    function getChanceArrayPrize() public view returns (uint256[2] memory) {
        return [MIN_CHANCE_VALUE_PRIZE, MAX_CHANCE_VALUE];
    }
    //
    function alligatorMint(address to, uint256 _quantity) internal {
        if (AlligatorIds.current() > COLLECTION_SIZE) revert callErr();
        for (uint index = 0; index < _quantity; index++) {
            _mint(to, 1);
            _setTokenURI(_currentIndex - 1, "ALLIGATOR");
            AlligatorIds.increment(1);
        }            
    }

    function jokerMint(address to) internal {
        // ONLY 1 NFT (JOKER) is allowed because alligators are supplied :)
        if (JokerIds.current() > COLLECTION_JOKER_SIZE) revert callErr();
        JokerIds.increment(1);
        _mint(to, 1);
        _setTokenURI(JokerIds.current(), "JOKER");
    }

    function mergeMint(uint256 _1st, uint256 _2nd, uint256 _3rd, address to) external onlyMerger {
        _burn(_1st);
        _burn(_2nd);
        _burn(_3rd);
        alligatorMint(to, 1);
        _setTokenURI(MergeIds.current(), "MERGE");
        MergeIds.increment(1);
    }

    function whitelistMint(address to, uint count, uint8 v, bytes32 r, bytes32 s ) external {  
        if (saleStatus == SaleStatus.PAUSED) revert callErr();
        if (count <= 0) revert callErr();
        if (_currentIndex + count >= COLLECTION_SIZE + COLLECTION_JOKER_SIZE) revert callErr();
        if (saleStatus == SaleStatus.PRESALE) {
            //check coupon
            if (verify(to, v, r, s) != true) revert invalidSigner();
            //check limits - per wallet
            if (_whitelistMintedCount[to] + count >= tokens_per_WL) revert invalidSigner();
            //check limits - WL sale allowance
            if (_currentIndex + count >= collection_size_WL) revert callErr();
    ...
    function setChancePrizeArray(uint256 _min) external onlyOwner {
        if (_min > MAX_CHANCE_VALUE) revert callErr();
        MIN_CHANCE_VALUE_PRIZE = _min;
        emit chanceIsSet(_min);
    }
     ...
     */
     function approve(address to, uint256 tokenId) public override {
        address owner_ = MergeGators.ownerOf(tokenId);
    ...
    // =============================================================
    //        Royalty + Ownership + contract URI implementation
    // =============================================================
 ...
 */
interface IMergeGators is IERC721, IERC721Metadata {
    /**
