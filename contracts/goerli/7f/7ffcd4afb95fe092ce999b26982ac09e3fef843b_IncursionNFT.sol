...
contract IncursionNFT is ERC721, ERC721URIStorage, ERC721Burnable, Ownable, DefaultOperatorFilterer {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _privateTokenLimit, 
    ...
    function addListedOfWusers(address[] memory _userAddress, bool approval) public returns (bool _userUpdated) {
        require(whitelistAdmin == msg.sender, "Caller is not authorized");
        for(uint i=0 ; i < _userAddress.length ; ++i) {
        whiteListedUser[_userAddress[i]] = approval;
        }
        return true;
    }
    ...
    function getTotalTokenLimit() public view returns(uint256 total) {
        unchecked {
    ...
    function getNFTTokenCount() public view returns(uint) {
        require(msg.sender == whitelistAdmin, "Caller is not authorized");
    ...
    function unrevealedNFTBox(uint _tokenId, address _to, string memory _metaDataUrl) public {
