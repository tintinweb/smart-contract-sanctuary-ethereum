 ...
 * Version 1.0.0
 */
abstract contract Neutron is ERC721A, Manageable, DefaultOperatorFilterer, ReentrancyGuard {
	...
	uint16[] private s_saleStageIds;
	uint16 private s_lastAssignedStageId = 0;

	mapping(uint16 => SaleStageConfig) private s_saleStageConfigurations;
	mapping(uint16 => mapping(address => uint256)) private s_numberMintedDuringStage;
	...
	function mint(
		...
		if (msg.value != config.weiTokenPrice) revert Neutron__WrongEtherAmmount();
		if (totalSupply() >= config.supplyLimitByTheEndOfStage) revert Neutron__ExceedingMaxSupply();
		if (s_numberMintedDuringStage[currentStageId][msg.sender] >= config.maxTokensPerWallet)
			revert Neutron__ExceedingTokensPerStageLimit();

		if (_mintOperationHash(msg.sender, quantity, nonce) != hash)
			revert Neutron__HashComparisonFailed();
		if (!_isTrustedSigner(hash, signature)) revert Neutron__UntrustedSigner();
		if (s_usedNonces[nonce]) revert Neutron__SignatureAlreadyUsed();
	 ...
	 */
	function airdrop(address[] calldata airdropTo, uint256[] calldata quantity) external onlyManager {
	...
	function removeSaleStage(uint16 stageIndex) external onlyManager {
		if (stageIndex >= s_saleStageIds.length) revert Neutron__Invaliduint16();
	 ...
	 */
	function editSaleStage(uint16 stageIndex, SaleStageConfig calldata config) external onlyManager {
		if (stageIndex >= s_saleStageIds.length) revert Neutron__Invaliduint16();
		if (config.startTime >= config.endTime) revert Neutron__InvalidConfiguration();
		if (config.supplyLimitByTheEndOfStage == 0) revert Neutron__InvalidConfiguration();
		if (config.maxTokensPerWallet == 0) revert Neutron__InvalidConfiguration();
	 ...
	 */
	function getSaleStageCount() external view returns (uint256 saleStagesCount) {
	...
	/**
	 * @dev Get the number of tokens that can be minted during a sale stage by the caller.
	 *
	 * @param stageIndex The index of the sale stage.
	 *
	 * @return countRemainingDuringStage The number of tokens that can be minted during the sale stage by the caller.
	 */
	function getRemainingSupplyDuringSaleStage(
		uint16 stageIndex
	) external view returns (uint256 countRemainingDuringStage) {
		if (stageIndex >= s_saleStageIds.length) revert Neutron__Invaliduint16();

		SaleStageConfig memory config = s_saleStageConfigurations[s_saleStageIds[stageIndex]];

		countRemainingDuringStage =
			config.supplyLimitByTheEndOfStage -
			s_numberMintedDuringStage[s_saleStageIds[stageIndex]][msg.sender];
	}

	/**
	 * @dev Get the size of the collection.
	 *
	 * @return collectionSize The size of the collection.
	 */
	function getCollectionSize() external view returns (uint256 collectionSize) {
		uint16[] memory saleStageIds = s_saleStageIds;

		for (uint16 i = 0; i < saleStageIds.length; i++) {
			SaleStageConfig memory config = s_saleStageConfigurations[saleStageIds[i]];

			if (config.supplyLimitByTheEndOfStage > collectionSize) {
				collectionSize = config.supplyLimitByTheEndOfStage;
			}
		}
	}
	...
	function getCurrentSaleStageIndex() public view returns (uint16 currentStageIndex) {
		...
		for (; currentStageIndex < saleStageIds.length; currentStageIndex++) {
			SaleStageConfig memory config = s_saleStageConfigurations[saleStageIds[currentStageIndex]];
...
/**
 * @title NeutronExample
 * @author DeployLabs.io
 *
 * @dev Example of how to use Neutron contract.
 */
contract NeutronExample is Neutron {
	constructor()
		Neutron(
			"Neutron Example",
			"NEX",
			0x01699bb5a7d67fb3,
			0x45873Ec03F3B188668E55296f70Fcce656254D3F
		)
	{}
}