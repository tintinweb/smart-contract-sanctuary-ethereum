...
error Lottery__NotEnoughtCash();
error Lottery_TransactionFailed();
error Lottery__NotOpen();
error Lottery__NotUpKeepNeeded(uint256 balance, uint256 playersCount, uint256 lotteryState);
...
contract Lottery is VRFConsumerBaseV2, AutomationCompatibleInterface {
    ...
    event EventLotteryEnter(address indexed player);
    event EventLotteryWinner(address indexed winner);
    ...
    function enterLottery() public payable {
        if (msg.value < i_entranceFee) {
            revert Lottery__NotEnoughtCash();
        }
        if (s_state != LotteryState.OPEN) {
            revert Lottery__NotOpen();
        }
        s_players.push(payable(msg.sender));
        emit EventLotteryEnter(msg.sender);
    }
    ...
    function fulfillRandomWords(uint256 /*requestId*/, uint256[] memory randomWords) internal override {
        ...
        emit EventLotteryWinner(s_winner);
    }
    ...
    function performUpkeep(bytes calldata /*performData*/) external override {
        ...
        if (!upkeepNeeded) {
            revert Lottery__NotUpKeepNeeded(address(this).balance, s_players.length, uint256(s_state));
        }
