...
contract DarkWarriorNFT is ERC721, Ownable {
    ...
    constructor() ERC721("DarkWarrior", "SYMBOL") {
        baseUri = "ipfs://QmYMHmFvBBTzbhmoDSVqAjZHkXv3emSTPSASbV9yNUyowu/";
    ...
    function mintForWeb(uint256 _numTokens) external payable {
        ...
        if(isAccountMintedFree()) {
            ...
            if(_numTokens <= freeSupplyWallet - mintedPerWalletFree[msg.sender]) {
                ...
                require((_numTokens - freeSupplyWallet + mintedPerWalletFree[msg.sender]) * price <= msg.value, "Insufficient funds.");
                for(uint256 i = 1; i <= _numTokens; ++i) {
                    _safeMint(msg.sender, curTotalSupply + i);
                }
    ...
    function isAccountMintedFree() view public returns (bool) {
        if(mintedPerWalletFree[msg.sender] >= freeSupplyWallet) {
            return true;
        } else {
            return false;
        }
    ...
    function getAccountMintedFree() view public returns (uint256) {
        return mintedPerWalletFree[msg.sender];
    }
