...
/**
 * @dev It needs 10 part NFTs to assemble a Mech NFT. 
 * A Mech NFT can be disassembled back to 10 parts. 
 */
contract Mech is 
    ...
    function initialize(
        ...
        uint256 priceForDisassemble) initializer public {
     ...
     * All parts nfts that are used should be verified from backend system.
     ...
     *
     * Params
     * - `blockNumberLimit`: a transaction should be ended before blockNumberLimit
     ...
     * - `signature`: signature to validate all parameters
     */
    function disassemble(uint256 blockNumberLimit, uint256[] calldata parts, uint256 tokenId, bytes calldata signature) external payable nonReentrant {
...
//        require(mechPartsData[tokenId].length > 0, "Invalid mech NFT");
        bytes32 partsKey = generateUniqueMechKey(parts);
        require(tokenId == getMechTokenId(partsKey), "Part hash value is not matched with this mech");
   ...
   function withdrawBalance(address payable recipient, uint256 amount) external only(DEFAULT_ADMIN_ROLE) nonReentrant {
     ...
     * Params
     * - `blockNumberLimit`: a transaction should be ended before blockNumberLimit
     * - `tokenOwner`: mech nft owner
     * - `tokenId`: a mech NFT tokenId.
     * - `signature`: signature to validate all parameters
     */
    ...
    /**
     * @dev Stores association information of part nft tokenIds and its mech tokenId
     * Because the same set of part nft tokenIds should be associated with a mech tokenId
     * When it disassembled and re-assembled with same part tokenIds, the mech tokenId is always same
