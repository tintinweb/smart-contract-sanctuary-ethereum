...
contract Swap is ZetaInteractor, ZetaReceiver, SwapErrors, Pausable {
            ...
            ZetaInterfaces.SendInput({
                ...
                message: abi.encode(
                    ...
                    XcMessage({
                        ...
                        fromToken: args.fromToken, // Needs to be parameterized
            ...
            ZetaInterfaces.SendInput({
                ...
                message: abi.encode(
                    ...
                    XcMessage({
                        ...
                        sourceTxOrigin: msg.sender,
                        receiverAddressEncoded: args.receiverAddress,
                        fromToken: args.buyTokenAddress,
                        fromTokenAmount: 0,
                        toToken: address(0),
                        toTokenMinAmount: 0,
                        poolFee: 0
    ...
    {
        (bytes32 messageType, XcMessage memory xcMessage) = abi.decode(
            rawZetaMessage.message,
            (bytes32, XcMessage)
        );
        ...
        if (messageType == SWAP_MESSAGE) {
        ...
        } else if (messageType == SWAP_MESSAGE_ZEROEX) {
            ...
            if (xcMessage.toToken == ETH) {
                ...
                IERC20 toToken = IERC20(xcMessage.fromToken);
                ...
                if (!toToken.transfer(receiverAddress, toToken.balanceOf(address(this)) - prevBal))
                    revert ErrorTransferringTokens(xcMessage.fromToken);
            }
        } else {
    ...
    {
        (bytes32 messageType, XcMessage memory xcMessage) = abi.decode(
            rawZetaRevert.message,
            (bytes32, XcMessage)
        );

        if (messageType != SWAP_MESSAGE && messageType != SWAP_MESSAGE_ZEROEX)
            revert InvalidMessageType();
