...
interface IResolverService {
    ...
    function _resolve(bytes calldata data) external view returns(bytes memory result, uint64 expires, bytes memory sig);
...
contract OffchainResolver is IExtendedResolver, SupportsInterface {
    ...
    function _resolve(bytes calldata data) external view returns (bytes memory) {
    ...
    function _offChainLookup(bytes calldata callData) private view returns(bytes memory) {
        ...
        revert OffchainLookup(
            address(this),
            urls,
            abi.encodeWithSelector(IResolverService._resolve.selector, callData),
