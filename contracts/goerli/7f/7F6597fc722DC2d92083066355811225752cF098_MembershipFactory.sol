...
contract MembershipFactory is IMembershipFactory, IEvents {
  /**
  ...
  function create(
    ...
    require(
      metadata != address(0),
      "MembershipFactory: metadata is zero address"
    );

    // create membership
    Membership m = new Membership(name, metadata, params);

    // transfer ownership
    m.transferOwnership(msg.sender);

    // return
    emit MembershipCreated(address(m), msg.sender);
    return address(m);
  }
}
