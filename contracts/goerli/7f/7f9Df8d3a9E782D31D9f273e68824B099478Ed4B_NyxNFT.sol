...
// @title NyxNFT for Nyx Capital Club
// @author TheV
...
contract NyxNFT is ERC1155Supply, Ownable
{
    ...
    constructor(address _vault_address, address _main_address, address _signer_address,
        string memory metadata_url, uint256 _wavePrice, uint256 _waveMaxSupply) ERC1155(metadata_url)
    {
        ...
        initWave(currentTokenId, _wavePrice, _waveMaxSupply, BASE_VOTING_POWER);
    }
    ...
    function setNyxPremintPct(uint256 _nyx_mint_pct)
        public onlyOwner
    {
        nyx_mint_pct = _nyx_mint_pct;
    }
    ...
    function computeTeamMint(uint256 supply)
        public view
        returns (uint256)
    {
        return (supply - (supply/nyx_mint_pct)) / nyx_mint_pct;
    }

    function initWave(uint256 wave, uint256 _wavePrice, uint256 _waveMaxSupply, uint256 votingPower)
        private
    {
        wavePrice[wave] = _wavePrice;
        waveCurrentSupply[wave] = 0;
        waveMaxSupply[wave] = _waveMaxSupply;
        // waveMintableSupply[wave] = (100 - nyx_mint_pct) * _waveMaxSupply / 100;
        waveMintableSupply[wave] = _waveMaxSupply - computeTeamMint(_waveMaxSupply);
        ...
        public onlyApproved
    {
        ...
        uint256 mint_amount = computeTeamMint(waveCurrentSupply[wave]);
        if (mint_amount > 0)
        {
    ...
    function startNewWave(uint256 _wavePrice, uint256 _waveMaxSupply, uint256 votingPower)
        private onlyApproved
    {
        ...
        currentTokenId += 1;
        initWave(currentTokenId, _wavePrice, _waveMaxSupply, votingPower);
        _isMinting = false;
    }

    function createNewTokenId(uint256 _wavePrice, uint256 _waveMaxSupply, uint256 votingPower)
        external onlyApproved
    {
        startNewWave(_wavePrice, _waveMaxSupply, votingPower);
    }
        ...
        external onlyOwner
    {
        ...
        (bool succes_nyx, ) = payable(main_address).call{value: allowedWithdraw}("");
        (bool succes_vault, ) = payable(vault_address).call{value: restWithdraw}("");
