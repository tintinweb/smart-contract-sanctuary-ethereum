...
contract ERCVault is ReentrancyGuard {
    ...
    function lockLiquidity(address tokenAddress, uint tokenQuantity, uint unlockDate) external payable {
        ...
        require(IERC20(tokenAddress).approve(address(this), tokenQuantity), 'Failed to approve tokens');
        require(IERC20(tokenAddress).transferFrom(msg.sender, address(this), tokenQuantity), 'Failed to transfer tokens to locker');
    ...
    function withdrawTokens(uint id) external {
        if (msg.sender != owner) {
            ...
            require(vaults[id].walletAddress == msg.sender, 'Not the owner of this Vault');
        }
        ...
        require(IERC20(tokenAddress).transfer(msg.sender, amount), 'Failed to transfer tokens');
