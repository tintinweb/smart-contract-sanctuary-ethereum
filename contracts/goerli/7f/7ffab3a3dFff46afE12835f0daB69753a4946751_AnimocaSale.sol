...
contract AnimocaSale is IAnimocaHome, Ownable, Pausable ,ReentrancyGuard {
  ...
  uint256 public constant PHASE1_PRICE = 150 * (10 ** 18);
  uint256 public constant PHASE1_SALE_AMOUNT = 300;
  uint256 public constant HOLDER_THRESHOLD = 1;

  uint256 public constant PHASE2_PRICE = 222 * (10 ** 18);
  uint256 public constant PHASE2_SALE_AMOUNT = 888;

  uint256 public constant PHASE1_PENDING_BEFORE_WHITELIST = 2 days;
  uint256 public constant TOKEN_ADDITION_MODIFIER = 1000000;
  uint256 public constant PURCHASE_LIMIT = 20;

  IERC20 high;
  AnimocaHome animoca;
  IERC1155 highstreetAssets;
  uint256 nextId;
  uint256[2] public phase1Time;
  uint256[2] public phase2Time;
  mapping (address => bool) internal whitelist;

  event Purchase(address account, stage purchaseStage, uint256 amount, uint256 startId);

  constructor(
    IERC20 high_,
    AnimocaHome animocaNft_,
    IERC1155 highstreetAssets_,
    uint256[2] memory phase1Time_,
    uint256[2] memory phase2Time_
  ) {
    high = high_;
    animoca = animocaNft_;
    highstreetAssets = highstreetAssets_;
    phase1Time = phase1Time_;
    phase2Time = phase2Time_;
  }
  ...
  function isQualifiedAssetsOwner(address account) public view returns (bool) {
    address[] memory accounts = new address[] (14);
    uint256[] memory ids = new uint256[] (14);
    for (uint256 i = 0; i < 14; ++i) {
      accounts[i] = account;
      ids[i] = TOKEN_ADDITION_MODIFIER + i;
    }
    uint256[] memory returnBalance = highstreetAssets.balanceOfBatch(accounts, ids);
    uint256 sum;
    for (uint256 i = 0; i < returnBalance.length; ++i) {
      sum += returnBalance[i];
    }
    return sum >= HOLDER_THRESHOLD;
  }

  function getTokenLeft() public view returns (uint256) {
    if (getStage() == stage.phase1) {
      return PHASE1_SALE_AMOUNT - nextId;
    }
    if (getStage() == stage.phase2) {
      return (nextId < PHASE1_SALE_AMOUNT) ? PHASE2_SALE_AMOUNT: PHASE1_SALE_AMOUNT + PHASE2_SALE_AMOUNT - nextId;
    }
    return 0;
  }

  function getStage() public view returns (stage) {
    uint256[2] memory phase1 = phase1Time;
    uint256[2] memory phase2 = phase2Time;

    if (block.timestamp > phase2[1]) {
      return stage.over;
    }
    if (block.timestamp >= phase2[0] && block.timestamp <= phase2[1]) {
      return stage.phase2;
    }
    if (block.timestamp >= (phase1[0] + PHASE1_PENDING_BEFORE_WHITELIST ) && block.timestamp <= phase1[1]) {
      return stage.phase1;
    }
    if (block.timestamp >= phase1[0]) {
      return stage.whitelist;
    }
    return stage.notStart;
  }

  function purchaseForWhitelist() external nonReentrant whenNotPaused {
    if (getStage() != stage.whitelist) {
      revert StageError();
    }
    if (whitelist[msg.sender] != true) {
      revert NotInWhitelist();
    }
    if (nextId + 1 >= PHASE1_SALE_AMOUNT) {
      revert SaleIsOver();
    }
    delete whitelist[msg.sender];
    high.safeTransferFrom(msg.sender, address(this), PHASE1_PRICE);
    animoca.safeMint(msg.sender, nextId);
    emit Purchase(msg.sender, stage.whitelist, 1, nextId);
    ++nextId;
  }
  ...
  function purchase(uint256 amount) external nonReentrant whenNotPaused {
    ...
    if (current == stage.phase1) {
      uint256 left = getTokenLeft();
      uint256 toMint = (left < amount) ? left: amount;
      if (toMint == 0) {
        revert SaleIsOver();
      }
      if (!isQualifiedAssetsOwner(msg.sender)) {
        revert NotQualifiedHolder();
      }
      high.safeTransferFrom(msg.sender, address(this), PHASE1_PRICE * toMint);
      for (uint256 i = 0; i < toMint; ++i) {
        animoca.safeMint(msg.sender, i + nextId);
      }
      emit Purchase(msg.sender, stage.phase1, toMint, nextId);
      nextId += toMint;
    } else if (current == stage.phase2) {
      if (nextId < PHASE1_SALE_AMOUNT) {
        nextId = PHASE1_SALE_AMOUNT;
      }
      uint256 left = getTokenLeft();
      uint256 toMint = (left < amount) ? left: amount;
      if (toMint == 0) {
      ...
      high.safeTransferFrom(msg.sender, address(this), PHASE2_PRICE * toMint);
      for (uint256 i = 0; i < toMint; ++i) {
        animoca.safeMint(msg.sender, i + nextId);
      }
      emit Purchase(msg.sender, stage.phase2, toMint, nextId);
      nextId += toMint;
    } else {
      revert StageError();
    }
  ...
  function setWhitelist(address[] memory toAdd_, address[] memory toRemove_) external onlyOwner {
    for (uint256 i = 0; i < toAdd_.length; i++) {
      whitelist[toAdd_[i]] = true;
    }
    for (uint256 i = 0; i < toRemove_.length; i++) {
      whitelist[toRemove_[i]] = false;
    }
  }

  function setPhase1Time(uint256[2] memory newTime_) public onlyOwner {
    if (getStage() != stage.notStart) {
      revert NotDuringSale();
    }
    phase1Time = newTime_;
  }

  function setPhase2Time(uint256[2] memory newTime_) public onlyOwner {
    if (getStage() != stage.notStart) {
      revert NotDuringSale();
    }
    phase2Time = newTime_;
...
interface IAnimocaHome {
  ...
  // whitelist
  function purchaseForWhitelist() external;
  // others sell
  function purchase(uint256 amount) external;
  function isWhitelist(address account) external view returns (bool);
  function isQualifiedAssetsOwner(address account) external view returns (bool);
