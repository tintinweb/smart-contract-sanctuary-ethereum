...
contract IrBankMonitorSingleAsset {
    ...
    function isCompareOracleDeviate(
        ...
        if (irbankPrice >= bandPrice) {
            ...
            if (bandPrice.mul(ratio).div(100000000) < priceGap) {
                return true;
            }
        } else {
    ...
    function getOracleDeviate(
        ...
        if (irbankPrice >= bandPrice) {
            ...
            priceDeviationThreshold = bandPrice.mul(ratio).div(100000000);
    ...
    function getStakeReward(address _pToken) external view returns (address)  {
    ...
    function _getStakeRewardInternal(address _pToken) internal view returns (address)  {
        address _stakeReward;

        _pToken = StakingRewardsFactoryInterface(factory).getStakingRewards(
            _pToken
        );
        return _stakeReward;
    }
    ...
    function checker(
        ...
        _stakeReward = _getStakeRewardInternal(_pToken);
