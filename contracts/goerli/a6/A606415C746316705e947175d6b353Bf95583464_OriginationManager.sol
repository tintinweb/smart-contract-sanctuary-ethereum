...
import "./interfaces/IOriginationManager.sol";
// import "./libraries/PayLaterLoanLibrary.sol";
import "hardhat/console.sol"; // remove or comment 

contract OriginationManager is Context, IOriginationManager, EIP712 {
    using SafeERC20 for IERC20;
    address public loanCore;

    constructor(address _loanCore) EIP712("OriginationManager", "1") {
        require(_loanCore != address(0), "Origination: loanCore not defined");
        loanCore = _loanCore;
    }

    function initializePayLaterRequest(
        PayLaterLoanLibrary.LoanTerms calldata loanTerms,
    ...
    ) public override returns (uint256 payLaterLoanId) {

        require(_msgSender() == borrowerPrimaryAddress , "Origination: sender not participant");

        require(tokenType == 0 || tokenType == 1, "Unsupported Token standard");

        payLaterLoanId = IPayLaterLoanCore(loanCore).createPayLaterPurchase(loanTerms, borrowerPrimaryAddress, borrowerSecondaryAddress, lenderPrimaryAddress, tokenType, tokenAddress);

        uint256 firstInstallment = IPayLaterLoanCore(loanCore).getFirstInstallmentOfPayLaterPurchase(payLaterLoanId);
        console.log(firstInstallment);
        IERC20(loanTerms.payableCurrency).safeTransferFrom(borrowerPrimaryAddress, address(this), firstInstallment);

        IERC20(loanTerms.payableCurrency).approve(loanCore, firstInstallment);

        if(tokenType == 0) {
            IERC721(tokenAddress).transferFrom(lenderPrimaryAddress, borrowerSecondaryAddress, loanTerms.collateralTokenId);
        } else if(tokenType == 1){
            IERC1155(tokenAddress).safeTransferFrom(lenderPrimaryAddress, borrowerSecondaryAddress, loanTerms.collateralTokenId, 1, "0x0");
        } 

        IPayLaterLoanCore(loanCore).startPayLaterPurchase(payLaterLoanId);
         
        return payLaterLoanId;
    }

    function getFirstInstallmentByAmount(uint256 amount, uint256 parts, uint256 interest) public override view returns (uint256 installment) {
        return IPayLaterLoanCore(loanCore).getFirstInstallmentWithoutLoan(amount, parts, interest);
    }

    function initializeRentalRequest(
        RentalLibrary.RentalTerms calldata rentalTerms,
        address borrowerPrimaryAddress,
        address borrowerSecondaryAddress,
        address lenderPrimaryAddress,
        uint256 tokenType,
        address tokenAddress,
        uint256 rentalType
    ) public override returns (uint256 rentalLoanId) {

        require(_msgSender() == borrowerPrimaryAddress , "Origination: sender not participant");

        require(tokenType == 0 || tokenType == 1, "Unsupported Token standard");

        rentalLoanId = IPayLaterLoanCore(loanCore).createRentalRequest(rentalTerms, borrowerPrimaryAddress, borrowerSecondaryAddress, lenderPrimaryAddress, tokenType, tokenAddress, rentalType);

        uint256 firstInstallment = IPayLaterLoanCore(loanCore).getFirstInstallmentOfRentalPurchase(rentalLoanId);

        IERC20(rentalTerms.payableCurrency).safeTransferFrom(borrowerPrimaryAddress, address(this), firstInstallment);

        IERC20(rentalTerms.payableCurrency).approve(loanCore, firstInstallment);

        if(tokenType == 0) {
            IERC721(tokenAddress).transferFrom(lenderPrimaryAddress, borrowerSecondaryAddress, rentalTerms.collateralTokenId);
        } else if(tokenType == 1){
            IERC1155(tokenAddress).safeTransferFrom(lenderPrimaryAddress, borrowerSecondaryAddress, rentalTerms.collateralTokenId, 1, "0x0");
        }

        IPayLaterLoanCore(loanCore).startRentalPurchase(rentalLoanId);
...
interface IOriginationManager {
    function getFirstInstallmentByAmount(
        uint256 amount,
        uint256 parts,
        uint256 interest
    ) external view returns (uint256 installment);
