...
contract ManualDataFeed is IManualDataFeed {
  ...
  function sendManualObservation(
    address _to,
    ...
    Data memory _data = Data(_blockTimestamp, _tick, _liquidity);
    ...
    bytes4 selector = bytes4(keccak256('addObservation((uint32,int24,uint128))'));
    ...
    emit DataSent(_blockTimestamp, _tick, _liquidity);
  }
}
...
interface IManualDataFeed {
  ...
  /// EVENTS
  event DataSent(uint32 blockTimestamp, int24 tick, uint128 liquidity);

  /// STATE VARIABLES
  function connext() external returns (IConnextHandler _connext);
