...
contract DEINStakingView is IDEINStakingView, AbstractDependant, Initializable {
    ...
    mapping(uint256 => uint256) public override voterMultipliers; // stake duration (months) => multiplier , for voter
    ...
    function getAPY() external view override returns (uint256) {
        ...
        return _getAPY(INITIAL_STAKED_AMOUNT, 5, true);
    }

    /// @notice returns APY% with 10**5 precision
    /// expected position APY
    function getExpectedAPY(uint256 _staked, uint256 _multiplier)
        external
        view
        override
        returns (uint256)
    ...
    function getPositionAPY(uint256 _tokenId) external view override returns (uint256) {
        ...
        return _getAPY(_staked, _multiplier, false);
    }
    ...
    function _getAPY(
        ...
        uint256 _stakedByM = _staked.mul(_multiplier);

        uint256 _totalPoolWithPower = deinStaking.totalPoolWithPower();

        if (_totalPoolWithPower == 0) {
            _totalPoolWithPower = _stakedByM;
        } else if (isExpected) {
            _totalPoolWithPower = _totalPoolWithPower.add(_stakedByM);
        }
