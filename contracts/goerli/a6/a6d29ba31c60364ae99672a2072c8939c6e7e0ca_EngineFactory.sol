...
contract EngineFactory is Ownable {
    ...
    constructor(address dataProvider, address collateralToken) {
        ...
        vault = new Vault(
            ...
            IPool.Config({
                ...
                door: 240,
                hold: 240,
                cap: 1_000_000 * 10**IERC20Metadata(collateralToken).decimals()
            })
        );
        ...
        returns (address newEngine)
    {
        ...
        IEngine(newEngine).initialize(newOracle, vault, IConfig(newConfig));
...
abstract contract Pool is IPool, Ownable {
    ...
    IPool.Config public config;

    uint256 constant PRECISION = 1e18;
    IERC20 public immutable collateralToken; // collateral token address
    mapping(address => UserInfo) public users;
    uint256 public totalShares;
    int256 public accRewardsPerShares; // rewards are not guaranteed and can potentially turns negative
    uint256 public protocolFees; // fees collected from withdraws;
    ...
    event Accumulate(int256 amount);
    ...
    // --- Init ---
    constructor(IERC20 usdc_, Config memory config_) {
        require(address(usdc_) != address(0), 'address0');
        collateralToken = usdc_;
        config = config_;
    }
    ...
    function capLimit() external view returns (uint256) {
        ...
        return IERC20(collateralToken).balanceOf(address(this)) / markets;
    }
    ...
    function stake(uint256 amount, address recipient) external {
        ...
        require(amount + balance <= config.cap, 'pool/total-cap-exceeded');
        ...
        emit Deposited(recipient, address(collateralToken), amount);
    }
    ...
    function unstake(uint256 amount, address recipient) external {
        ...
        if (u.withdrawRequest == 0) {
            ...
            (block.timestamp - u.withdrawRequest) > (config.door + config.hold)
        ) {
            ...
            // take a snapshot of the current accRewardsPerShare
            u.accRewardsSnapshotAtLock = accRewardsPerShares;
        } else if ((block.timestamp - u.withdrawRequest) > config.door) {
            ...
            uint256 fees = (amount * config.wfee) / PRECISION;
            protocolFees += fees;
    ...
    function accumulate(int256 amount) internal {
        require(totalShares != 0, 'no-shares');
        accRewardsPerShares +=
            (amount * int256(PRECISION)) /
            int256(totalShares);
        emit Accumulate(amount);
    }

    function withdrawFees() public onlyOwner {
        // reset totalFees accumulated
        collateralToken.safeTransfer(owner(), protocolFees);
        protocolFees = 0;
    }
}
...
contract Vault is ERC721, IVault, Pool {
    using FinMath for uint256;
    using FinMath for int256;
    using SafeCast for uint256;
    using SafeCast for int256;
    ...
    ) Pool(IERC20(collateralToken_), poolConfig) ERC721('TEST', 'TST') {
    ...
    function settle(
        ...
        int256 balance = pnl.add(collateral_);
        if (balance >= 0) {
    ...
    function adjust(
        uint256 collateralT_,
        int256 pnlT,
        address recipient
    ) external override onlyApproved {
        uint256 fee = liquidationConfig.fixLiquidationFee +
            collateralT_.bps(liquidationConfig.liquidationFeeBps); // 1 dollar
        int256 remaining = pnlT + int256(collateralT_);
        if (remaining > 0) {
            // we guarantee a fixed fee to the liquidator, + half the remaining
            uint256 liquidatorFees = fee + (uint256(remaining) / 2);
            IERC20(collateralToken).transferFrom(
                address(this),
                recipient,
                liquidatorFees
            );
            // the other half of the remaining minus the fixed fees is accrued to the insurance pool
            accumulate((remaining / 2) - int256(fee));
        } else {
            IERC20(collateralToken).transferFrom(address(this), recipient, fee);
            accumulate(remaining - int256(fee));
        }
    }
