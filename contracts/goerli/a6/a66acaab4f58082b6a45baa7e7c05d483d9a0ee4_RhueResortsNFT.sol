...
/** 
 *  SourceUnit: d:\GitWork\RhueResortsNFT\contracts\RhueResortsNFT.sol
*/
...
contract RhueResortsNFT is ERC721, Ownable, ERC721Holder {
    ...
    event ItemListed(uint256 id, uint256 tokenId, uint256 price, address owner);
    ...
    function mint() external payable {
        ...
        require(currentID < maxSupply, "Exceed max supply");
        if (isPublicSale == false) {
            // only whitelist user can mint
            require(
                whitelist[msg.sender],
                "only whitelisted user can mint now"
            );
        }
    ...
    /**
		Marketplace Logic
	 */
    function list(uint256 tokenId, uint256 _price) external {
    ...
    function delist(uint256 _id) external {
        ...
        _transfer(address(this), pairs[_id].owner, pairs[_id].tokenId);
    ...
    function buy(uint256 _id) external payable {
        ...
        // transfer coin to collecton owner
        if (royalty > 0) {
            payable(owner()).transfer(
                totalAmount.mul(royalty).div(PERCENTS_DIVIDER)
            );
        }

        // transfer coin to nft owner
        uint256 ownerPercent = PERCENTS_DIVIDER.sub(royalty);
        payable(pair.owner).transfer(
            totalAmount.mul(ownerPercent).div(PERCENTS_DIVIDER)
        );

        // transfer NFT token to buyer
        _transfer(address(this), msg.sender, pair.tokenId);
