...
function transferReward(address rewardgetter) public onlyOwner{
     ...
     for(uint i;i<len;i++){
          ...
          uint rewardbalance= IERC(_rewardtokens[i]).balanceOf(address(this));
    ...
    function returnstates() public view onlyOwner returns (bool[5] memory) {
        return [purchased, updated, staked, sold, soldafterupdate];
    }
    ...
    function _purchase(uint _amount,uint[] calldata _allowedslippage) internal {
    ...
    function _sell(address[] memory _tokens, uint[] calldata amounts,uint[] calldata _allowedslippage) internal {
    ...
    function purchase(uint amount, uint[] calldata allowedslippage) public onlyOwner {
        require(
        ...
        require(amount<=IERC(ptoken).balanceOf(address(this)),"Amount should be less than balance");
    ...
    function sell(uint[] calldata _amounts,uint[] calldata allowedslippage ) public onlyOwner purchasedcheck {
    ...
    function rebalancesell(uint[] calldata amounts,uint[] calldata allowedslippage) public {
        require(
            block.timestamp > depositendtime,
        ...
        if (!flag) {
            soldafterupdate = true;
        }
    }

    function rebalancepurchase(uint amount,uint[] calldata allowedslippage) public {
        require(
            block.timestamp > depositendtime,
            "time period should be between deposit and ending"
        );

        require(soldafterupdate, "all tokens not sold yet");

        _purchase(amount,allowedslippage);

        updated = false;
        soldafterupdate = false;
    }
