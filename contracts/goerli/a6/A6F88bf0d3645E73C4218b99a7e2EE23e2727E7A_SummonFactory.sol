...
contract SummonFactory {
  event SummonCreated(address summonAddress, address owner);
  mapping(address => Summon) public AddressToSummon;

  function CreateNewSummon(address _owner) public {
    require(AddressToSummon[_owner].owner() == address(0), "address already has a SummonSafe");
    Summon summon = new Summon(_owner);
    AddressToSummon[_owner] = summon;
    emit SummonCreated(address(summon), _owner);
  }
