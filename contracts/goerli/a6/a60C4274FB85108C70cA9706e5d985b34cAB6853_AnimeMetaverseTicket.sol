...
contract AnimeMetaverseTicket is Ownable, ERC1155, IAnimeMetaverseTicket {
    ...
    uint256 public constant PREMIUM_TICKET_PRICE = 0.06 ether;
    uint256 public premiumTicketPrice = PREMIUM_TICKET_PRICE;
    ...
    mapping(address => WhiteListedUser) public whiteListedUsers;
    ...
    function addToWhitelistBatch(
        ...
        require(
            _accounts.length == _ticketAmounts.length,
            "Account and ticket have different length"
        );

        for (uint256 i = 0; i < _accounts.length; i++) {
            whiteListedUsers[_accounts[i]] = WhiteListedUser({
    ...
    function mintFreeTicket(uint256 _mintAmount)
        external
        validMintAmount(_mintAmount)
    {
        ...
        require(
            isWhiteListed(msg.sender, _mintAmount),
        ...
        validMintAmount(_mintAmount)
    {
        require(
            ...
            "Ticketss are expensive, need more ETH to afford that."
        );
        require(
        ...
        whiteListedUsers[msg.sender].alreadyMinted += _mintAmount;
    ...
    function isWhiteListed(address _walletAddress, uint256 _mintAmount)
        public
        view
        returns (bool)
    {
        if (
            whiteListedUsers[_walletAddress].alreadyMinted + _mintAmount <
            whiteListedUsers[_walletAddress].maxAllowedToMint
        ) return true;
        else return false;
    }
    ...
    function resetMintPrice() external onlyOwner {
        premiumTicketPrice = PREMIUM_TICKET_PRICE;
    }
