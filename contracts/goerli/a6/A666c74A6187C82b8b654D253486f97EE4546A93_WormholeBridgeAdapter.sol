...
interface ITokenBridge
{
	function transferTokens(
		...
		bytes32 recipient,
		uint256 arbiterFee,
		uint32 nonce) external returns (uint64 sequence);
...
contract Worker is Ownable {
	...
	function init(ITokenBridge _tokenBridge, uint16 _recipientChain)
		external
	{
	...
	{
		IERC20(asset).safeIncreaseAllowance(address(tokenBridge), type(uint256).max);
		tokenBridge.transferTokens(asset, amount, recipientChain, bytes32(abi.encode(address(this))), 0, uint32(block.number));
	}
	...
	{
		tokenBridge.completeTransfer(exitData);
...
contract WormholeBridgeAdapter is IBridgeAdapter, Ownable {
	...
	function _deployWorkerProxy(bytes32 salt) internal returns (Worker) {
		...
		Worker(proxy).init(tokenBridge, uint16(dstChainId));
		return Worker(proxy);
	}
