...
contract ArbOtc is Ownable, ReentrancyGuard {
    ...
    function getArbToClaim(address wallet) public pure returns (uint256) {
        return 5000;
        /*uint256 claimable = TokenDistributor(AIRDROP_ADDRESS).claimableTokens(wallet);

        claimable -= offeredArb[wallet];
