...
contract TokenVesting is Ownable {
    ...
    constructor(
        ...
        require(_tgeTimestamp > 0, 'TokenVesting: incorrect TGE timestamp');
        require(_treasuryAddress != address(0x0), 'TokenVesting: treasury address cannot be a zero');
    ...
    function createVestingSchedule(
        ...
        require(_terms > 0, 'TokenVesting: terms must be > 0');
    ...
    function increaseVestingScheduleTotalAmount(uint256 _amount, string calldata _name) external onlyOwner {
        ...
        require(_amount > 0, 'TokenVesting: amount must be greater than zero');
        require(
            getUnusedAmount() >= _amount,
            'TokenVesting: cannot increase vesting schedule total amount because not sufficient tokens'
        );
    ...
    function decreaseVestingScheduleTotalAmount(uint256 _amount, string calldata _name) external onlyOwner {
        ...
        require(
            vestingSchedules[_name].totalAmount >= _amount,
            'TokenVesting: amount cannot be greater than vesting schedule total amount'
        );
        vestingSchedules[_name].totalAmount -= _amount;
        vestingSchedulesTotalReservedAmount -= _amount;
    }
