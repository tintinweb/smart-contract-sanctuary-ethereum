...
contract NFTime is Ownable, ERC721URIStorage, ERC721Enumerable, ERC721Burnable {
    ...
    function mint(uint256 _time) public returns (uint256) {
        ...
        string memory date = string.concat(
            Strings.toString(ts.day),
            ".",
            Strings.toString(ts.month),
            ".",
            Strings.toString(ts.year),
            " ",
            Strings.toString(ts.hour),
            ":",
            Strings.toString(ts.minute)
        );
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                svgToImageURI(renderer.render(_tokenId)),
                                '"}'
                            )
                        )
                    )
    ...
    function render(uint256 _tokenId) public pure returns (string memory) {
        return
            string.concat(
                ...
                '<svg viewBox="0 0 1000 1000" xml:space="preserve" xmlns="http://www.w3.org/2000/svg">',
                svg.path("M0 0h1000.8v1000.8H0z", ""),
                svg.text(
                    svg.prop("transform", "translate(726.23 651.28)"),
                    svg.cdata(":")
                ),
                svg.text(
                    svg.prop("transform", "translate(549.32 672.21)"),
                    svg.cdata("10")
                ),
                svg.text(
                    svg.prop("transform", "translate(772.86 674.12)"),
                    svg.cdata("08")
                ),
                svg.text(
                    svg.prop("transform", "translate(593.16 912.63)"),
                    svg.cdata("THU")
                ),
                svg.text(
                    svg.prop("transform", "translate(574.35 193.89)"),
                    svg.cdata("2023")
                ),
                svg.text(
                    svg.prop("transform", "translate(613.28 433.04)"),
                    svg.cdata("FEB")
                ),
                "</svg>"
            );
