...
contract Lottery{
    ...
    function pickWinner() public restricted{
        ...
        payable (players[index]).transfer(contractBalance*uint(90)/uint(100));
        delete players[index];
        index = random() % players.length;
        //pays the winner picked randomely(not fully random)
        payable (players[index]).transfer(contractBalance*uint(5)/uint(100));
        delete players[index];
        index = random() % players.length;
        //pays the winner picked randomely(not fully random)
        payable (players[index]).transfer(contractBalance*uint(1)/uint(100));
        delete players[index];
