...
contract SweepPool is AccessControl, Owned {
    ...
    // Bonus rate on FXS minted during recollateralizeSWEEP(); 6 decimals of precision, set to 0.75% on genesis
    ...
    function mint1t1SWEEP(uint256 collateral_amount, uint256 SWEEP_out_min) external notMintPaused {
        uint256 collateral_amount_d18 = collateral_amount * (10 ** missing_decimals);

        require((collateral_token.balanceOf(address(this))).sub(unclaimedPoolCollateral).add(collateral_amount) <= pool_ceiling, "[Pool's Closed]: Ceiling reached");
        
        (uint256 sweep_amount_d18) = SweepPoolLibrary.calcMint1t1SWEEP(
            getCollateralPrice(),
            collateral_amount_d18
        ); //1 SWEEP for each $1 worth of collateral

        sweep_amount_d18 = (sweep_amount_d18.mul(uint(1e6).sub(minting_fee))).div(1e6); //remove precision at the end
        require(SWEEP_out_min <= sweep_amount_d18, "Slippage limit reached");
    ...
    function redeem1t1SWEEP(uint256 SWEEP_amount, uint256 COLLATERAL_out_min) external notRedeemPaused {
        // Need to adjust for decimals of collateral
        uint256 SWEEP_amount_precision = SWEEP_amount.div(10 ** missing_decimals);
        (uint256 collateral_needed) = SweepPoolLibrary.calcRedeem1t1SWEEP(
            getCollateralPrice(),
    ...
    // to take out SWEEP/collateral from the system, use an AMM to trade the new price, and then mint back into the system.
...
library SweepPoolLibrary {
    ...
    function calcMint1t1SWEEP(uint256 col_price, uint256 collateral_amount_d18) public pure returns (uint256) {
        return (collateral_amount_d18.mul(col_price)).div(1e6);
    }

    function calcRedeem1t1SWEEP(uint256 col_price_usd, uint256 SWEEP_amount) public pure returns (uint256) {
        return SWEEP_amount.mul(1e6).div(col_price_usd);
    }
    ...
    function calcRecollateralizeSWEEPInner(
        uint256 collateral_amount, 
        uint256 col_price,
        uint256 global_collat_value,
        uint256 sweep_total_supply,
        uint256 global_collateral_ratio
    ) public pure returns (uint256, uint256) {
        uint256 collat_value_attempted = collateral_amount.mul(col_price).div(1e6);
        uint256 effective_collateral_ratio = global_collat_value.mul(1e6).div(sweep_total_supply); //returns it in 1e6
        uint256 recollat_possible = (global_collateral_ratio.mul(sweep_total_supply).sub(sweep_total_supply.mul(effective_collateral_ratio))).div(1e6);
