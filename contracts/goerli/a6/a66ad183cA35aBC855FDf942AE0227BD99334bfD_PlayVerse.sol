...
contract PlayVerse is Ownable, ERC721A, ReentrancyGuard {
    ...
    ) ERC721A("PlayVerse", "PV", maxPerAddress_, collectionSize_) {
        maxPerAddress = maxPerAddress_;
        totalAmount = collectionSize_;
    ...
    function whitelistMint(uint256 quantity) external payable callerIsUser {
        ...
        require(price != 0, "Whitelist sale has not started yet.");
        require(whitelist[msg.sender] > 0, "Not in whitelist.");
        require(
            quantity <= whitelist[msg.sender],
    ...
    function publicMint(uint256 quantity, uint256 callerSaleKey)
        external
        payable
        callerIsUser
    {
        ...
        require(saleKey == callerSaleKey, "Called with incorrect sale key.");

        require(
            isPublicSale(price, saleKey, publicSaleStartTime),
    ...
    function reservedMint(uint256 quantity) external onlyOwner {
        ...
        require(
            totalSupply() + quantity <= reservedAmount,
            "Maximum quantity allowed exceeded."
        );
        _safeMint(msg.sender, quantity);
    }
    ...
    function startWhitelistMint(uint64 priceWei) external onlyOwner {
        saleConfig = SaleConfig(priceWei, 0, 0, 0);
    }
    ...
    function startPublicMint(
        ...
        saleConfig = SaleConfig(0, priceWei, saleKey, publicSaleStartTime);
    }
