...
contract Staker {
  ...
  constructor(address exampleExternalContractAddress) public {
	...
	modifier notCompleted() {
		require(exampleExternalContract.completed() == false, "Contract has already been completed");
		_;
	}
  ...
  //  ( make sure to add a `Stake(address,uint256)` event and emit it for the frontend <List/> display )
	...
	function stake() public payable {
		require(openForWithdraw == false, "Contract must not be opened for withdraw");
  ...
  //  It should call `exampleExternalContract.complete{value: address(this).balance}()` to send all the value
	...
	function execute() public notCompleted {
		...
		// if (block.timestamp <= deadline) {
		// 	require(address(this).balance >= threshold, "balance must be higher than or equal to threshhold");
		// 	exampleExternalContract.complete{value: address(this).balance}();
		// } else {
		// 	openForWithdraw = true;
		// }
  ...
  // TODO: if the `threshold` was not met, allow everyone to call a `withdraw()` function
	function withdraw() public notCompleted {
		...
		require(openForWithdraw, "Contract is not opened for withdrawl");
