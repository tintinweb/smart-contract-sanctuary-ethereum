...
contract TIMEDividend is Ownable, ERC20, ERC20Burnable, Utils, Multicall {
    ...
    function _beforeTokenTransfer(
        ...
        int256 correctionDelta = int256(magnifiedDividendPerShare * amount);
        if (from != address(0)) {
    ...
    function dividendFrom(
        ...
        uint256 product = uint256(int256(magDividendPerShare * balance) + correction);
    ...
    function distributeAll(address internetMoneySwapRouter, uint256 amount) public {
        IInternetMoneySwapRouter(internetMoneySwapRouter).distributeAll(amount);
    }
}
