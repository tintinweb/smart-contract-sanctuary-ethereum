...
contract PersonaTraits is IPersona, Ownable {
    ...
    mapping(uint8 => string) public storeTraitsBaseURIs; //traits metadata baseuri
    string public mintTraitsBaseURI;
    ...
    event UpdateNFT(uint16[] oldTraits, uint16[] newTraits);
    ...
    function addMintTraits(
        ...
        for (uint i; i < _traitIDs.length; i++) {
            addTrait(_attrID, _traitIDs[i], false, _traitRarities[i]);
            emit NewTrait(_attrID, _traitIDs[i], false, 0, 0, _traitRarities[i]);
        }
    }
    ...
    function addStoreTraits(
        ...
        storeTraitsBaseURIs[_attrID] = _baseURI;
        for (uint i; i < _traitIDs.length; i++) {
            addTrait(_attrID, _traitIDs[i], true, _traitRarities[i]);
    ...
    function addTrait(
        ...
        traits[_traitID] = Trait(_traitID, _attrID, _onStore, _traitRarity);
    }

    function removeStoreTrait(
        uint8 _attrID,
        uint16 _traitID
    ) external onlyOwner {
        require(
    ...
    function setTraitPointsByRarity(
        PersonaTraitRarity[] memory _rarities,
    ...
    function useTraitsForUpdateNFT(
        ...
        emit UpdateNFT(_originTraits, _newTraits);
    }
    ...
    function getTraitURI(
        ...
        if (trait.onStore)
            return
                string(
                    abi.encodePacked(
                        storeTraitsBaseURIs[trait.attrID],
                        "/",
    ...
    function setMintBaseURI(
        string memory _mintTraitsBaseURI
    ) external onlyOwner {
        mintTraitsBaseURI = _mintTraitsBaseURI;
    }
