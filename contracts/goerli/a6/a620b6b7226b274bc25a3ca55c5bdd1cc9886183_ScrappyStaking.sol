...
contract ScrappyStaking is IERC721Receiver, ReentrancyGuard {
  ...
  function stakeNFT(uint256[] calldata _id) external { 
  ...
  function unStakeNFT(uint256[] calldata _id) external payable {
    for (uint256 j = 0; j < _id.length; j++) {
      // require(address(msg.sender) == stake[_id[j]].owner, "No stake exists");
      IERC721(scrappy).safeTransferFrom(address(this), stake[_id[j]].owner, _id[j]);
      userInfo[msg.sender].totalStaked --;
      for (uint256 i = 0; i < userInfo[msg.sender].tokenId.length; i++){
        if(userInfo[msg.sender].tokenId[i] == _id[j]){
  ...
  function claimReward(uint256[] memory _id) public payable nonReentrant{
    for (uint256 j = 0; j < _id.length; j++) {
      require(msg.sender == stake[_id[j]].owner, "No stake exists");
      uint256 timeStaked = (block.timestamp - stake[_id[j]].stakingtime)/secPerWeek;
      uint256 stakeAmount = timeStaked - stake[_id[j]].claim;
      if(stakeAmount > 0){
        require(msg.value >= claimFee * stakeAmount, "Eth amount");
        stake[_id[j]].claim = timeStaked;
        userInfo[msg.sender].rewards += stakeAmount;
        emit claimed(stakeAmount, msg.sender);
      }
    }
  }
  function readData(uint256[] calldata _id) public view returns(address){
    for (uint256 j = 0; j < _id.length; j++) {
      return stake[_id[j]].owner;
    }
  }
  function calculateReward(uint256 _id) public view returns(uint256){
    require(stake[_id].stakingtime > 0, "No stake exists");
