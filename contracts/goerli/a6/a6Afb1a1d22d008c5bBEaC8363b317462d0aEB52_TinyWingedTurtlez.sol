...
contract TinyWingedTurtlez is ERC721, Ownable {
    ...
    function setWingType(
        uint48 wingTypeIndex,
        Coordinates[3] memory coordinates
    ) private {
        for (uint8 i = 0; i < coordinates.length; i++) {
    ...
    ) ERC721("Tiny Winged Turtlez", "TWT") {
        // Start at token 1
    ...
    function weightedRarityGenerator(
        uint16 pseudoRandomNumber,
        uint8 trait
    ...
    function createTokenIdTurtle(
        uint256 tokenId
    ) public view returns (Turtle memory) {
        ...
        return
            Turtle({
                ...
                wingType: weightedRarityGenerator(
                    uint16(uint16(pseudoRandomBase >> 1) % maxSupply),
                    0
                ),
                wingColor: weightedRarityGenerator(
                    uint16(uint16(pseudoRandomBase >> 2) % maxSupply),
                    1
                ),
                bandana: weightedRarityGenerator(
                    uint16(uint16(pseudoRandomBase >> 3) % maxSupply),
                    2
                ) == 1,
                bandanaColor: uint16(uint16(pseudoRandomBase >> 4) % 5),
                boots: weightedRarityGenerator(
                    uint16(uint16(pseudoRandomBase >> 5) % maxSupply),
                    3
                ) == 1,
                bootsColor: uint16(uint16(pseudoRandomBase >> 6) % 5),
                pupil: uint16(uint16(pseudoRandomBase >> 7) % 4),
                tongue: weightedRarityGenerator(
                    uint16(uint16(pseudoRandomBase >> 8) % maxSupply),
                    4
                ) == 1,
                tail: uint16(uint16(pseudoRandomBase >> 9) % 2) == 1,
                turtleType: weightedRarityGenerator(
                    uint16(uint16(pseudoRandomBase >> 10) % maxSupply),
                    5
                )
            });
    }

    function getTurtleBase(
        Turtle memory turtle
    ) private view returns (string memory turtleBase) {
        return
            string(
                abi.encodePacked(
                    "<rect fill='",
    ...
    function getTurtleWings(
        Turtle memory turtle
    ) private view returns (string memory turtleWings) {
        turtleWings = string(
            abi.encodePacked(
                "<rect fill='",
    ...
    function getTurtlePupil(
        Turtle memory turtle
    ) private view returns (string memory turtlePupil) {
        return
            string(
                abi.encodePacked(
                    "<rect fill='",
    ...
    function getTurtleDetail(
        Turtle memory turtle
    ) private view returns (string memory turtleDetail) {
        return
            string(
                abi.encodePacked(
                    "<rect fill='",
    ...
    function getTurtleBoots(
        Turtle memory turtle
    ) private view returns (string memory turtleBoots) {
        return
            string(
                abi.encodePacked(
                    "<rect fill='",
    ...
    function getTurtleBandana(
        Turtle memory turtle
    ) private view returns (string memory turtleBandana) {
        return
            string(
                abi.encodePacked(
                    "<rect fill='",
    ...
    function getTurtleTongue()
        private
        pure
        returns (string memory turtleTongue)
    {
        return
            string(
                abi.encodePacked(
    ...
    function getTurtleTail(
        Turtle memory turtle
    ) private view returns (string memory turtleTail) {
    ...
    function getTokenIdTurtleMetadata(
        Turtle memory turtle
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    Base64.encode(
                                        bytes(getTokenIdTurtleSvg(turtle))
                                    ),
                                    '","attributes":',
    ...
    function claim(uint256 numberOfTokens) external {
        ...
        require(
            totalSupply() + numberOfTokens <= freeTurtlez,
            "Exceeds claim supply."
        );
    ...
    function mint(uint256 numberOfTokens) external payable {
        ...
        require(
            msg.value >= numberOfTokens * turtlePrice,
