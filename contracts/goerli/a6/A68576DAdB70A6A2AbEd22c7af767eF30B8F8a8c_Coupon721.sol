...
contract Coupon721 is Juicing721 {
    ...
    */
    ...
    function initialize(address _dollar) public initializer {
        ...
        c1 = Coupon(1, 1000e18, 100, 3000, 720, "", address(0)); // 1,000 pina, 1000 supply, no discount, 1 month redeemable
        c2 = Coupon(2, 10000e18, 98, 300, 2160, "", address(0)); // 10,000 pina, 100 supply, 98% discount, 3 month redeemable
        c3 = Coupon(3, 100000e18, 96, 100, 4320, "", address(0)); // 100,000 pina, 10 supply, 96% discount, 6 month redeemable
        c4 = Coupon(4, 1000000e18, 94, 10, 8640, "", address(0)); // 100,000 pina, 10 supply, 96% discount, 12 month redeemable
    }
    ...
    /*
        URI on-chain
    */
    function imageURI(uint256 _baseTokenID)
        internal
        view
        returns (string memory)
    {
        ...
        return
            bytes(currentBaseURI).length > 0
                ? string(
                    abi.encodePacked(currentBaseURI, _baseTokenID.toString())
                )
                : "";
    }
    ...
    function getTokenURI(
        ...
        bytes memory dataURI = abi.encodePacked(
            ...
            c.name,
            " #",
            _tokenId.sub(_baseTokenID).toString(),
            '",',
            '"description": "Meme Coupon colletions on chain",',
            '"level": "',
            cInfo.c.level,
            '"',
            '"value": "',
            ...
            '"',
            '"redeemableEpoch": "',
            cInfo.redeemableEpoch,
            '"',
            '"artist": "',
            cInfo.c.artist,
            '"',
            '"image": "',
            imageURI(_baseTokenID),
            '"',
            "}"
        );
        return
    ...
    */
        ...
        returns (uint256)
    {
        ...
        uint256 cPriceDiscount = cPrice.mul(c.discount).div(100);
    ...
    function getCouponValue(uint256 _tokenID) external view returns (uint256) {
        CouponInfo memory cInfo = couponInfo[_tokenID];
    ...
    */
    ...
    function purchaseCoupon(uint256 _baseTokenID) external {
        ...
        _setTokenURI(tokenID, getTokenURI(_baseTokenID, tokenID, cInfo));
