...
library bitwise_boolean {
    ...
    function check_state(uint8 boolean_array, uint8 bool_index) 
                         internal pure returns(bool bool_result) {
        return (boolean_array & bool_index) > 0;
    }

    function set_true(uint8 boolean_array, uint8 bool_index) 
                     internal pure returns(uint8 resulting_array){
        return (boolean_array | uint8(1) << bool_index);
        //return boolean_array | (((boolean_array & bool_index) > 0) ? 1 : 0);
    }

    function set_false(uint8 boolean_array, uint8 bool_index) 
                      internal pure returns(uint8 resulting_array){
        return (boolean_array & ~(uint8(1) << bool_index));
        //return boolean_array & (((boolean_array & bool_index) > 0) ? 1 : 0);
    }
...
contract modern {
    ...
    using bitwise_boolean for uint8;
    ...
    // Bit 7 is free
    mapping (address => uint8) authorizations;
    // !SECTION Bitwise Definition
...
contract controllable is modern {
    ...
    // Bit 6: token_running
    // Bit 7: free
    uint8 contract_controls;
    // !SECTION Bitwise definitions

    // SECTION Writes
    function set_swaps_open(bool state) public onlyOwner {
        if(!state) {
            contract_controls = contract_controls.set_false(1);
        } else {
            contract_controls = contract_controls.set_true(1);
        }
    }
    ...
    // SECTION Views
    function get_swaps_open() public view returns(bool) {
        return contract_controls.check_state(1);
    }
...
contract Ichiba is IERC20, controllable
{
    ...
    uint8 public BalanceMitigationFactor=50; // 2% max balance
    ...
    uint8 public liquidityFee=50; // Portion of taxes that goes to liquidity
    uint8 public developmentFee=50; // Portion of taxes that goes to development
    ...
    uint public effectiveCirculating= 888 * 10**9 * 10**_decimals;
    uint public balanceMitigation = effectiveCirculating/BalanceMitigationFactor;
    uint public txs;
    ...
    uint swapMitigation = effectiveCirculating/50; // 2%

    IUniswapRouter02 public immutable router = IUniswapRouter02(router_address);
    ...
    constructor () {
        ...
        _balances[msg.sender] = effectiveCirculating;
        emit Transfer(address(0), msg.sender, effectiveCirculating);
    ...
    function _transfer(address sender, address recipient, uint amount) private{
        enforce(sender != address(0), "Transfer from ded");
        enforce(recipient != address(0), "Transfer to ded");
        txs += 1;
    ...
    function feesDividETH(uint ETHamount) private {
        uint tokenSplit = (ETHamount * developmentFee)/100;
        tokenBalance+=tokenSplit;
    ...
    function getantispamSecondsInSeconds() public view returns(uint){
    ...
    function setOpentoken(bool booly) public onlyAuth {
        set_swaps_open(booly);
    }
    ...
    function assignWins(address winner, uint percentage) internal {
        uint availableForWinners = tokenBalance / 2;
        uint winning = (availableForWinners * percentage) / 100;
        tokenBalance = tokenBalance - winning;
        if (address(this).balance < winning) {
            winning = (address(this).balance/2);
        }
        (bool success,) = winner.call{value: (winning)}("");
        enforce(success,"assignWins failed");
    }

    function WithdrawtokenETH() public onlyAuth{
