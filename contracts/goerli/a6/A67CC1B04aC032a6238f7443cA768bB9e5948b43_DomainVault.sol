...
contract DomainVault is IDomainVault, ERC721Holder, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    *  @notice Buy presale fragment using ETH
    ...
    function redeem() public whenNotPaused nonReentrant {
        ...
        require(voteEndTime == 0 || (voteEndTime < _currentTime() && !_isVotePast()), "DomainVault: voting or voted passed, can't redeeming");
        ...
        /*if(reserveCurrency == WETH){
            IERC20Metadata(WETH).transfer(bidder, bidPrice);
        }else{
    ...
    function firstBidUsingETHAndWETH(uint256 currentBidPrice) external payable whenNotPaused nonReentrant {
        ...
        //投票未开始 或者 投票结束且投票未通过（提示：投票中或投票通过，不能发起买断）
        require(voteEndTime == 0 || (voteEndTime < _currentTime() && !_isVotePast()), "DomainVault: voting or voted passed, can't make bid");
    ...
    function firstBid(uint256 currentBidPrice) external payable whenNotPaused nonReentrant {
        ...
        //投票未开始 或者 投票结束且投票未通过（提示：投票中或投票通过，不能发起买断）
        require(voteEndTime == 0 || (voteEndTime < _currentTime() && !_isVotePast()), "DomainVault: voting or voted passed, can't make offer bid");
    ...
    function vote() external whenNotPaused nonReentrant {
        ...
        //是否已经投票通过
        require(!_isVotePast(), "DomainVault: vote passed, can't vote");
        ...
        if (!_isVotePast() && voteEndTime.sub(_currentTime()) <= 15 minutes) {
    ...
    function bidUsingETHAndWETH(uint256 currentBidPrice) external payable whenNotPaused nonReentrant {
        ...
        require(voteEndTime < _currentTime() || _isVotePast(), "DomainVault: voting or no vote passed, can't bid");
    ...
    function withdrawBidFunds() external payable whenNotPaused nonReentrant {
        ...
        require(!_isVotePast(), "DomainVault: vote passed, can't withdraw bid funds");
        ...
        /*if (reserveCurrency == WETH) {
            ...
            IERC20Metadata(reserveCurrency).transferFrom(address(this), bidder, bidPrice);
        }*/
        IERC20Metadata(reserveCurrency).transfer(bidder, bidPrice);
    ...
    function claimNFT() external whenNotPaused nonReentrant {
        ...
        //投票通过（提示：投票未通过）
        require(_isVotePast(), "DomainVault: vote no passed, can't claim NFT");
    ...
    function claimFunds(uint256 withdrawERC20Amount) public whenNotPaused nonReentrant {
        ...
        //投票通过（提示：投票未通过）
        require(_isVotePast(), "DomainVault: vote passed, can't claim funds");
        ...
        /*if (reserveCurrency == WETH) {
            IWETH(WETH).transfer(_msgSender(), fundsShareAmount);
        } else {
            IERC20Metadata(reserveCurrency).transferFrom(address(this), _msgSender(), fundsShareAmount);
        }*/
    ...
    //票数是否通过
    function _isVotePast() internal view returns (bool) {
        uint256 originalTotalSupply_ = IDomainERC20(fragment).originalTotalSupply();
        return votes.mul(uint256(10000)) >= originalTotalSupply_.mul(votePercentage);
    }
