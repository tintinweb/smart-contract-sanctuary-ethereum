...
contract AlienBees is ERC721, Ownable, ReentrancyGuard, RandomlyAssigned{
    ...
    constructor(
        ...
        for(uint32 HpwalletLoop =0; HpwalletLoop < _HivePalsHolders.length; HpwalletLoop++){
            if(HivePalsWhiteListedAddresses[_HivePalsHolders[HpwalletLoop]] != true ){
    ...
    function mintBee(uint256 quantity) public payable {
        ...
        uint256 remainingLimit = mintedTokensByAddress[msg.sender];

        uint256 totalLimit = (HivePalsWhiteListedAddresses[msg.sender] == true && HPSale == true) ?  HPLimit: 0;
