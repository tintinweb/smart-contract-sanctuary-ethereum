...
contract YouBox is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    ) ERC721A("YouBox", "YB", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "YouBox: zero address");
        require(amount > 0, "YouBox: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "YouBox: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "YouBox: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "YouBox: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "YouBox: Public sale is not active.");
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "YouBox: Need to send more ETH.");
        if (msg.value > price) {
