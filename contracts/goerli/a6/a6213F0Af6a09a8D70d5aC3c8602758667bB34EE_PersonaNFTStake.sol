...
contract PersonaNFTStake is IPersona, Ownable {
    ...
    function calcTotalCreditsByHolder(
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (block.timestamp >= lastClaimedTime[tokenId] + rewardInterval) {
                credit +=
                    (rewardPointByRarity[
                        IPersonaNFT(personaNFT).rarityOfNFT(tokenId)
    ...
    function getRemainTimes(
        uint256[] memory _tokenIds
    ) external view returns (uint256[] memory) {
        uint256[] memory remainTimes = new uint256[](_tokenIds.length);
        for (uint i; i < _tokenIds.length; i++) {
            remainTimes[i] = rewardInterval - (block.timestamp - lastClaimedTime[_tokenIds[i]]) % rewardInterval;
        }
