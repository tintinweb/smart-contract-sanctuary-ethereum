...
contract BallsTest is ERC20Interface, SafeMath {
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
        ...
        uint256 amount = safeMul(tokens, 1000000000000000000);

        uint256 finalAmount = safeMul(amount, _finalPercentage) / 100;
        uint256 feeAmount = safeMul(amount, _feePercentage) / 100;
