...
contract A {
    ...
    function getExtended(string memory s) public view returns(string memory) {
        if(keccak256(bytes(s)) == keccak256(bytes("1"))) {
            return "1111";
        } else if(keccak256(bytes(s)) == keccak256(bytes("a"))) {
            return "aaaa";
        }
    }
    ...
    function getDescended() public returns(uint[] memory) {
        
        for(uint i=0; i<nums.length; i++) {
            for(uint k=i; k<nums.length; k++) {
                if(nums[i] < nums[k]) {
                    uint temp = nums[i];
                    nums[i] = nums[k];
                    nums[k] = temp;
    ...
    // 예시 : 1850년 -> 19세기
    function getCentury(uint year) public view returns(uint) {
        return (year / 100) + 1;
    }

    // 4) 첫 수와 끝 수 그리고 총 숫자 수를 알려주었을 때 자동으로 중간 숫자들을 채워 등차수열을 완성하세요. 
    // 예시 : 1,7,3 -> 1,4,7 // 1,9,5 -> 1,3,5,7,9
    function getArithmeticSequence(uint first, uint last, uint size) public view returns (uint[] memory) {
