...
contract ControlMint is ContractRestricted, IControlMint {
    ...
    //Guarda el numero de minters que se han añadido al freemint. No puede soprepasar el limite
    Counters.Counter private implementAddressFreemintList;
    //Guarda el numero de mints que se han generado en la freemint
    Counters.Counter private mintsFreemintList;
    //Guarda el numero de minters que se han añadido a la whitelist. No puede soprepasar el limite
    Counters.Counter private implementAddressWhitelistList;
    //Guarda el numero de mints que se han generado en la whitelist
    Counters.Counter private mintsWhitelistList;

    //Limite de minters que se pueden añadir a la freemints
    uint16 private maxMintAddressFreemintList = 100;
    //Limite de minters que se pueden añadir a la whitelist
    uint16 private maxMintAddressWhitelistList = 500;

    //Un address no peude mintear mas de 5 nft de whitelist aun que se activen el evento para mintear uno mas
    uint32 private numMaxForAddress = 5; 
    // uint32 private numMaxForAddress = 10000;
    //Contiene el numero de mints que tienen permitidos hacer los minters que estan dentro de la lista de "addressWhitelistList"
    uint64 private event1MoreCount = 0;
    //Guarda a los address que han minteado en un periodo
    mapping(address => uint64) private event1MoreLastMint;

    //Inicia o desactiva el evento par aque los usuarios que ya han minteado un nft puedan mintear unos más
    bool private event1More = false;

    //Lista de minters que se han añadido a la freemint. Se guardan los datos del struct "AccessFreemint"
    mapping(address => AccessFreemint) private addressFreemintList;
    //Lista de minters que se han añadido a la whitelist. Se guardan los datos del struct "AccessWhitelist"
     ...
     * @dev Comprueba que no haya llegado al limite: "maxMintAddressFreemintList"
     ...
     */
    function addAddressFreemintList(address[] memory newMinters) public override onlyOwnerOrContract {
    ...
    function addAddressWhitelistList(address[] memory newMinters) public override onlyOwnerOrContract{
     ...
     */
    function getAddressFreemintListCount() public override virtual view onlyOwnerOrContract returns(uint256){
     ...
     */
    function getAddressWhitelistListCount() public override virtual view onlyOwnerOrContract returns(uint256){
