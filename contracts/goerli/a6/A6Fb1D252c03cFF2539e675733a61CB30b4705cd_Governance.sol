...
contract Governance is Ownable, ReentrancyGuard, ERC4907A, ERC721AQueryable {
    using ECDSA for bytes32;

    uint256 private constant totalAmount = 38400;
    uint64 private constant lockTime = 60 * 60 * 24 * 30 * 6;
    uint256 private constant contributeRewardUnit = 9;
    address private signer;
    address public friendshipToken;
    address public treasury;
    address public rewardExtractor;
    string public baseURI;

    uint256 private _contributeReward = 0;

    event SignerChanged(address operator, address from, address to);
    event TreasuryChanged(address operator, address from, address to);
    event RewardExtractorChanged(address operator, address from, address to);
    ...
    modifier checkExpired(uint256 tokenId) {
        require(userOf(tokenId) == address(0), "Token is locked");
        _;
    }

    constructor(
        address _signer,
        address _friendshipToken,
        address _treasury,
        address _rewardExtractor,
        string memory _baseURI
    ) ERC721A("FrenArtGovernance", "FAG") {
        signer = _signer;
        friendshipToken = _friendshipToken;
        treasury = _treasury;
        rewardExtractor = _rewardExtractor;
        baseURI = _baseURI;
    }

    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC4907A, IERC721A, ERC721A) returns (bool) {
        return
    ...
    function _hashBytes(
        ...
        return keccak256(abi.encode(from, to, tokenId));
    }
    ...
    function setTreasury(address _treasury) external onlyOwner {
        emit TreasuryChanged(_msgSender(), treasury, _treasury);
        treasury = _treasury;
    }

    function setRewardExtractor(address _rewardExtractor) external onlyOwner {
        emit RewardExtractorChanged(
            _msgSender(),
            rewardExtractor,
            _rewardExtractor
        );
    ...
    function extractReward(uint256 quantity) external {
        uint256 consume = contributeRewardUnit * quantity;
        require(
            _contributeReward >= consume,
            "You don't have enough contribution to mint."
        );
        address from = _msgSender();
        uint64 expires = uint64(block.timestamp) + lockTime;

        for (uint256 i = 0; i < quantity; i++) {
            uint256 returnTokenId = _nextTokenId();
            _safeMint(rewardExtractor, 1);
            setUser(returnTokenId, rewardExtractor, expires);
            emit Minted(from, rewardExtractor, returnTokenId);
        }

        _contributeReward = _contributeReward - consume;
    }
    ...
    function mint(
        ...
        address owner = IERC721A(friendshipToken).ownerOf(tokenId);
        address from = _msgSender();

        require(owner == from, "The token owner is not caller.");
        // verify signature
        require(
            _verify(_hashBytes(_msgSender(), to, tokenId), signature),
            "Invalid signature."
        );
        ...
        // transfer FB token
        IERC721A(friendshipToken).safeTransferFrom(from, treasury, tokenId);

        // mint and lock governance token
        uint256 mintTokenId = _nextTokenId();
        _safeMint(from, 1);
        setUser(mintTokenId, from, expires);
        emit Minted(from, from, mintTokenId);

        // return and lock governance token
        _contributeReward = _contributeReward + 1;
    }

    function userExpiresList(
        uint256[] calldata tokenIds
    ) external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            result[i] = userExpires(tokenIds[i]);
        }
        return result;
    }
    ...
    function approve(
        address to,
        uint256 tokenId
    ) public payable override(ERC721A, IERC721A) checkExpired(tokenId) {
    ...
    function _beforeTokenTransfers(
        ...
        uint256 quantity
    ) internal override checkExpired(startTokenId) {
