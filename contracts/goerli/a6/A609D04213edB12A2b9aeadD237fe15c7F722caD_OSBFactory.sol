...
contract OSBFactory is IOSBFactory, ContextUpgradeable {
     ...
     * @return tokenInfo Token information after creation
     */
    function create(bool _isSingle, string memory _uri, string memory _name, string memory _symbol, address _defaultReceiverRoyalty, uint96 _defaultPercentageRoyalty) public returns (TokenInfo memory) {
        lastId.increment();
        ...
        if (_isSingle) {
            ...
            _osb721.initialize(_msgSender(), address(this), _uri, _name, _symbol, _defaultReceiverRoyalty, _defaultPercentageRoyalty); 
            deployedContract = address(_osb721);
        } else {
            OSB1155 _osb1155 = OSB1155(ClonesUpgradeable.cloneDeterministic(library1155Address, salt));
            _osb1155.initialize(_msgSender(), address(this), _uri, _name, _symbol, _defaultReceiverRoyalty, _defaultPercentageRoyalty); 
     ...
     * @param _mintTo The address where the tokens will be minted to.
     ...
     */
    function createAndMintSingleToken(address _mintTo, TokenInput memory _tokenInput, string[] memory _tokenUris) external payable {
        TokenInfo memory tokenInfo = create(true, _tokenInput.uri, _tokenInput.name, _tokenInput.symbol, address(0), 0);
        OSB721(tokenInfo.token).mintBatch(_mintTo, _tokenUris);
    }
     ...
     */
    function createAndMintSingleTokenWithRoyalty(address _mintTo, TokenInput memory _tokenInput, string[] memory _tokenUris, RoyaltyInput memory _defaultRoyaltyInput, RoyaltyInput[] memory _royaltyInputPerTokenMinted) external payable {
        TokenInfo memory tokenInfo = create(true, _tokenInput.uri, _tokenInput.name, _tokenInput.symbol, _defaultRoyaltyInput.receiver, _defaultRoyaltyInput.percentage);
        OSB721(tokenInfo.token).mintBatchWithRoyalty(_mintTo, _tokenUris, _royaltyInputPerTokenMinted);
    }
    /**
     ...
     */
    function createAndMintMultiToken(address _mintTo, TokenInput memory _tokenInput, string[] memory _tokenUris, uint256[] memory _amounts) external payable {
        TokenInfo memory tokenInfo = create(false, _tokenInput.uri, _tokenInput.name, _tokenInput.symbol, address(0), 0);
        OSB1155(tokenInfo.token).mintBatch(_mintTo, _amounts, _tokenUris);
    }
     ...
     * @param _royaltyInputPerTokenMinted Custom royalty information for each token in the batch.
     */
    function createAndMintMultiTokenWithRoyalty(address _mintTo, TokenInput memory _tokenInput, string[] memory _tokenUris, uint256[] memory _amounts, RoyaltyInput memory _defaultRoyaltyInput, RoyaltyInput[] memory _royaltyInputPerTokenMinted) external payable {
        TokenInfo memory tokenInfo = create(false, _tokenInput.uri, _tokenInput.name, _tokenInput.symbol, _defaultRoyaltyInput.receiver, _defaultRoyaltyInput.percentage);
        OSB1155(tokenInfo.token).mintBatchWithRoyalty(_mintTo, _amounts, _tokenUris, _royaltyInputPerTokenMinted);
    }
}