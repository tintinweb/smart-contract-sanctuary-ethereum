...
contract SaleUtopiaNFTV2 is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    modifier onlyAllowListed(address _address, bytes32[] calldata _merkleProof) {
        PhaseInfo storage phase = phasesInfo[currentPhaseId];

        if (phase.whiteListRequired) {
            bool passMerkle = checkMerkleProof(_address, _merkleProof);
            require(passMerkle, "Not allowListed");
        }
        _;
    }

    function checkMerkleProof(address _address, bytes32[] calldata _merkleProof) public view virtual returns (bool) {
    ...
    function buy(uint256 _quantity, address _to, bytes32[] calldata _merkleProof) external payable nonReentrant onlyAllowListed(msg.sender, _merkleProof) {
    ...
    function _buy(uint256 _quantity, address _to, bool _isCreditCardPayment, bytes32[] calldata _merkleProof) internal {
        ...
        if (!_isCreditCardPayment) {
            if (checkMerkleProof(msg.sender, _merkleProof)) {
