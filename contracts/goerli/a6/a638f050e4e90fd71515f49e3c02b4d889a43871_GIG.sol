...
contract GIG is protected, IERC20 {
    ...
    string public name = "Gigidy";
    string public symbol = "GIG";
    uint8 public decimals = 18;
    uint256 public totalSupply = 100 * 10**6 * 10**decimals; // 100.000.000 + 18 decimals
    uint public swapTreshold = totalSupply / 1000; // 0.1% of total supply

    // ANCHOR Lists controls
    uint8 normalList = 0;
    uint8 whitelistedList = 1;
    uint8 bluelistedList = 2;
    uint8 greenlistedList = 3;
    ...
    constructor() {
        ...
        limits[bluelistedList] = LIMITS(2 seconds,
                                        100 * 10**6 * 10**decimals, // 100%
                                        100 * 10**6 * 10**decimals, // 100%
                                        100 * 10**6 * 10**decimals // 100%
                                        );
        limits[greenlistedList] = LIMITS(2 seconds,
                                        100 * 10**6 * 10**decimals, // 100%
                                        100 * 10**6 * 10**decimals, // 100%
                                        100 * 10**6 * 10**decimals // 100%
                                        );
        ...
        fees[bluelistedList] = FEES(0, 0, 1, 0, 0, 100);
        fees[greenlistedList] = FEES(0, 0, 3, 33, 33, 34);
    ...
    function transferFrom(address _from, address _to, uint256 _value) public override returns (bool success) {
        require(_allowances[_from][msg.sender] >= _value, "GIG: Not enough allowance");
    ...
    function approve(address _spender, uint256 _value) public override returns (bool success) {
        require(msg.sender != _spender, "GIG: Approve to yourself");
    ...
    function setCooldownTimerInterval(uint _list, uint _interval) public onlyOwner {
        require(_interval >= 1 seconds, "GIG: Interval must be at least 1 second");
        require(_interval <= 1 minutes, "GIG: Interval must be at most 1 minute");
        require(_list >= 0 && _list <= 3, "GIG: List must be between 0 and 3");
    ...
    function setMaxWallet(uint _list, uint _maxWallet) public onlyOwner {
        require(_maxWallet >= 1 * 10**4 * 10**decimals, "GIG: Max wallet must be at least 10 thousand");
        require(_maxWallet <= 2 * 10**6 * 10**decimals, "GIG: Max wallet must be at most 2 million");
        require(_list >= 0 && _list <= 3, "GIG: List must be between 0 and 3");
    ...
    function setMaxSale(uint _list, uint _maxSale) public onlyOwner {
        require(_maxSale >= 1 * 10**3 * 10**decimals, "GIG: Max sale must be at least 1 thousand");
        require(_list >= 0 && _list <= 3, "GIG: List must be between 0 and 3");
    ...
    function setBuyFee(uint8 _list, uint _buyFee) public onlyOwner {
        require(_buyFee >= 0 && _buyFee <= 49, "GIG: Buy fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "GIG: List must be between 0 and 3");
    ...
    function setTransferFee(uint8 _list, uint _transferFee) public onlyOwner {
        require(_transferFee >= 0 && _transferFee <= 49, "GIG: Transfer fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "GIG: List must be between 0 and 3");
    ...
    function setSellFee(uint8 _list, uint _sellFee) public onlyOwner {
        require(_sellFee >= 0 && _sellFee <= 49, "GIG: Sell fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "GIG: List must be between 0 and 3");
    ...
    function setLiquidityShareFee(uint8 _list, uint _liquidityShareFee) public onlyOwner {
        require(_liquidityShareFee >= 0 && _liquidityShareFee <= 100, "GIG: Liquidity share fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "GIG: List must be between 0 and 3");
    ...
    function setBurnShareFee(uint8 _list, uint _burnShareFee) public onlyOwner {
        require(_burnShareFee >= 0 && _burnShareFee <= 100, "GIG: Burn share fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "GIG: List must be between 0 and 3");
    ...
    function setRevenueShareFee(uint8 _list, uint _revenueShareFee) public onlyOwner {
        require(_revenueShareFee >= 0 && _revenueShareFee <= 100, "GIG: Marketing share fee must be between 0 and 100");
        require(_list >= 0 && _list <= 3, "GIG: List must be between 0 and 3");
    ...
    function getListNumbers() public view returns (uint8 _normalList , 
                                                   ...
                                                   uint8 _bluelist, 
                                                   uint8 _greenlist) {
        return (normalList, whitelistedList, bluelistedList, greenlistedList);
    }
    // !SECTION Lists controls
    ...
    function withdraw() public onlyOwner {
        ...
        require(success, "GIG: Transfer failed");
    }
    ...
    function swapTokensForEth(uint _amount) internal returns(uint _gain) {
        if (! (_balances[address(this)] >= _amount) ) {
            revert("GIG: Not enough tokens to swap");
        }
    ...
    function swapEthForTokens(uint _amount) internal {
        if (! (address(this).balance >= _amount) ) {
            revert("GIG: Not enough ETH to swap");
        }
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(this);
    ...
    function _transfer(address _from, address _to, uint _value, address txSender) private {
        ...
        if (blacklisted[txSender]) {
            revert("GIG: Sender is blacklisted");
        }
        // Then we check if the recipient is blacklisted
        if (blacklisted[_to]) {
            revert("GIG: Recipient is blacklisted");
        }
        ...
        if (isSell) {
            if (_value > limits[listNumber].maxSale) {
                revert("GIG: Sell limit exceeded");
            }
        } else {
            if (_value > limits[listNumber].maxTx) {
                revert("GIG: tx limit exceeded");
            }
            ...
            if (limits[listNumber].maxWallet > 0) {
                ...
                if (walletBalance + _value > limits[listNumber].maxWallet) {
                    revert("GIG: Wallet limit exceeded");
                }
            }
        }
        ...
        if (!isExcludedFromCooldown[_from]) {
            ...
            if (cooldown > 0) {
                if (lastTxTime[_from] + cooldown > block.timestamp) {
                    revert("GIG: Cooldown not expired");
                }
        ...
        if (liquidityFee > 0 || revenueFee > 0) {
            ...
            if (_balances[address(this)] > swapTreshold) {
                // Ensure we have enough ETH to swap
    ...
    function _transferTokens(address _from, address _to, uint _value) private {
        if (_balances[_from] < _value) {
            revert("GIG: Not enough tokens to transfer");
        }
    ...
    function redistributeFees() private {
        // NOTE We check if we have enough liquidity and revenue to swap
    ...
    function _burnTokens(uint _value) private {
        if (_balances[address(this)] < _value) {
            revert("GIG: Not enough tokens to burn");
        }
    ...
    function withdrawETH() external onlyOwner {
        ...
        require(success, "GIG: ETH transfer failed");
