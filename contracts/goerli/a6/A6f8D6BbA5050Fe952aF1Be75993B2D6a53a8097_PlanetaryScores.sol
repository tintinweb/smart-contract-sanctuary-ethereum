    ...
    Ownable
{
    ...
    uint256 public constant PS_RESERVED_TOKENS = 25;

    /// @dev Maximum amount of tokens that an address is allowed to mint.
    uint256 public constant PS_WALLET_MINT_MAX = 2;
        ...
        mintsPreCheck(1)
    {
        ...
        if (_getAux(msg.sender) != 0) revert EarlyDiscoveryPlanetClaimed();
        if (!verifyTokenForAddress(_salt, _token, _msgSenderERC721A()))
            revert InvalidToken();

        _setAux(_msgSenderERC721A(), 1);
        _mint(_msgSenderERC721A(), 1);
        emit TokensMinted(_msgSenderERC721A(), 1, MintStates.EARLY_DISCORVEY);
    }
        ...
        mintsPreCheck(_quantity)
    {
        ...
        emit TokensMinted(_msgSenderERC721A(), _quantity, MintStates.GENERAL);
    }
        ...
        onlyOwner
    {
        ...
        emit TokensMinted(_msgSenderERC721A(), _quantity, MintStates.OWNER);
    }
...
interface IPlanetaryScores is IERC721A {
    ...
    error InvalidToken();
    error EarlyDiscoveryPlanetClaimed();
