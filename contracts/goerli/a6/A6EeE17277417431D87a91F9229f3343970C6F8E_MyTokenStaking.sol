...
contract MyTokenStaking is ReentrancyGuard {
    using SafeERC20 for IERC20;

    // NFT max supply amount
    uint256 private constant MAX_SUPPLY = 10000;
    uint256 private constant STAKING_COOLDOWN_TIME = 86400;
    ...
    function _stake(address _user, uint256 _tokenId) private {
        // token must be craft, so tokenId >= 10000 && tokenId <=14999
        require(
            MAX_SUPPLY <= _tokenId && _tokenId < MAX_SUPPLY+(MAX_SUPPLY/2),
            "can not staking uncrafted token"
        );
    ...
    function _unstake(address _user, uint256 _tokenId) private {
        ...
        for (uint256 i = 0; i < stakers[_user].stakedTokens.length; i++) {
            ...
            require(
                stakers[_user].stakedTokens[i].tokenStakedAt + (STAKING_COOLDOWN_TIME * 1 seconds) <
                block.timestamp,
