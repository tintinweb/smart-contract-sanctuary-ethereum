...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, StakingToken }
    ...
    uint public StakingTokenQueue; // Delays change to StakingToken address
    ...
    /**
        @notice allow approved address to deposit an asset for TOKEN
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) public returns ( uint send_ ) {
        ...
        // mint TOKEN needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( TOKEN ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TOKEN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITOKENERC20( TOKEN ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( StakingToken ).balanceOf( msg.sender ); // Can only borrow against StakingToken held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TOKEN
        @param _amount uint
     */
    function repayDebtWithTOKEN( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITOKENERC20( TOKEN ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TOKEN, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TOKEN ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TOKEN ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TOKEN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( TOKEN ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.StakingToken ) { // 9
            StakingTokenQueue = block.number.add( blocksNeededForQueue );
        } else return false;
