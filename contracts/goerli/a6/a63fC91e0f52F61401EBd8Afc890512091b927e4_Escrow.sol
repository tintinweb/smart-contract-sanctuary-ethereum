...
contract Escrow is IEscrow, Initializable, ReentrancyGuard {
    ...
    /// @notice Releases eth to talent and 15% to resolver
    ...
    function _release(uint256 _amount, string memory _note) internal {
        ...
        Address.sendValue(payable(resolver), resolverShare); // 15% to resolver
        Address.sendValue(payable(talent), _amount - resolverShare); // 85% to talent
    }
