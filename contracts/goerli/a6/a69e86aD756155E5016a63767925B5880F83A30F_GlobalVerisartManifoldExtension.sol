...
// Imports added to get solc working
// {{{
 ...
 * Version: 1.0
 */
contract GlobalVerisartManifoldExtension  {
    ...
    // Disabling as the contract does not support any interfaces that outside contracts may query for
    // Only needed as the Manifold contract expects this function to exist
    ...
    function mint(address creatorContract, address to, string calldata uri) public {
        ERC721Creator creatorCon = ERC721Creator(creatorContract);
        require(hasMintingPermission(creatorContract, msg.sender) || creatorCon.isAdmin(msg.sender),"Not allowed to mint");
        creatorCon.mintExtension(to, uri);
    }

    function mintBatch(address creatorContract, address to, string[] calldata uris) public {
        ERC721Creator creatorCon = ERC721Creator(creatorContract);
        require(hasMintingPermission(creatorContract, msg.sender) || creatorCon.isAdmin(msg.sender), "Not allowed to mint");
        creatorCon.mintExtensionBatch(to, uris);
    }
}
