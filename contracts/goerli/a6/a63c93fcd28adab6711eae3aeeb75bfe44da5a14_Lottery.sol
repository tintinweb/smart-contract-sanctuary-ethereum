...
contract Lottery is VRFConsumerBaseV2, ConfirmedOwner {
  ...
  uint256 public lastDrawBalance;
  ...
  function randomPlayer(uint256 i) internal view returns (address player) {
    uint256 index = s_requests[lastRequestId].randomWords[i] % playerMap[currentDrawNumber].length;
    player = playerMap[currentDrawNumber][index];
    return player;
  }
