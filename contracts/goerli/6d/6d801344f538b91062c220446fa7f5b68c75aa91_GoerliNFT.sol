...
contract GoerliNFT is ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Goerli_NFT", "Goerli") {}

    event Withdraw(address _to, uint256 _value);

    modifier onlyOwnerOf(uint256 tokenId) {
        require(msg.sender == ERC721.ownerOf(tokenId), 'Only the owner can open the gift');
        _;
    }
    ...
    function mint(uint8 count) public payable validMintCount(count) {
        require(MAX_SUPPLY >= _tokenIds.current(), "Max number has been minted");
