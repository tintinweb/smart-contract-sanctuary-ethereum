...
contract SproutsAcademy is ERC721A, Ownable {
    ...
    modifier callerIsUser() {
        require(tx.origin == msg.sender, "Sprouts :: Cannot be called by a contract");
        _;
    }

    function mintWLCC(uint256[] memory tokenIds) external {
        require(wlccSale, "Sprouts :: Not Yet Active.");
        require((totalWlccMint[msg.sender]) <= MAX_WLCC_MINT, "Sprouts :: Already minted maximum times!");
        ...
        if(wlccCount == 1 || wlccCount == 2) {
            sproutMintCount = 1;
        }
        else if(wlccCount > 2 && wlccCount < 5) {
            sproutMintCount = 2;
        }
        else if(wlccCount > 4 && wlccCount < 8) {
            sproutMintCount = 5;
        }
        else if(wlccCount > 7 && wlccCount < 10) {
            sproutMintCount = 6;
        }
        else if(wlccCount > 9) {
            sproutMintCount = 10;
    ...
    function mint() external {
        require(publicSale, "Sprouts :: Not Yet Active.");
        require((totalSupply() + 1) <= MAX_SUPPLY, "Sprouts :: Cannot mint beyond max supply");
        require((totalPublicMint[msg.sender] + 1) <= MAX_PUBLIC_MINT, "Sprouts :: Already minted maximum times!");

        totalPublicMint[msg.sender] += 1;
        _safeMint(msg.sender, (totalSupply() + 1));
    }

    function whitelistMint(bytes32[] memory _merkleProof) external {
        require(whiteListSale, "Sprouts :: Minting is on Pause");
        require((totalSupply() + 1) <= MAX_SUPPLY, "Sprouts :: Cannot mint beyond max supply");
        require((totalWhitelistMint[msg.sender] + 1)  <= MAX_WHITELIST_MINT, "Sprouts :: Cannot mint beyond whitelist max mint!");

        //create leaf node
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, sender), "Sprouts :: You are not on the Sproutlist");
