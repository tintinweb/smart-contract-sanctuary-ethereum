...
# Originally CurveToken, changed to vaETH
interface vaETH:
...
interface Math:
    def geometric_mean(unsorted_x: uint256[N_COINS], sort: bool) -> uint256: view
    ...
    def x_zero(x_unsorted: uint256[N_COINS]) -> uint256: view
...
math: constant(address) = 0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512
token: constant(address) = 0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9
views: constant(address) = 0x40745803C2faA8E8402E2Ae935933D07cA8f355c
coins: constant(address[N_COINS]) = [
    0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0, #allETH
    0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9, #stETH
    0x5FC8d32690cc91D4c39d9d3abcBD16989F875707, #rETH
]

# NOTE MY CODE: adding collateral specific vaETH tokens
vaTokens: constant(address[N_COINS]) = [
    0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9, # shouldn't matter
    0x0165878A594ca255338adfa4d48449f69242Eb8F, #vastETH
    0xa513E6E4b8f2a923D98304ec87F64353C4D5C853 #varETH
]
...
#     1000000000000, # 12 zeroes
#     10000000000, # 10 zeroes 
#     1,
# ]
...
@internal
@view
# self.balances return without precision
# this function multiplies precision and divides precision 
def xp() -> uint256[N_COINS]:
    ...
    for i in range(1, N_COINS): # from 1, N_COINS since all prices are in terms of coins[0]
        # gettign the right p from the packed representation
...
@external
@view
def get_xp() -> uint256[N_COINS]:
    return self.xp()

# my helper function
@external
def set_balances(bal_0: uint256, bal_1: uint256, bal_2: uint256):
    self.balances[0] = bal_0
    self.balances[1] = bal_1 
    self.balances[2] = bal_2 
...
# @view
# @external
# def _A_gamma_external() -> uint256[2]:
#     self._A_gamma()
...
def get_xcp(D: uint256) -> uint256:
    ...
    for i in range(1, N_COINS):
        x[i] = D * 10**18 / (N_COINS_256 * bitwise_and(packed_prices, PRICE_MASK))  # ... * PRICE_PRECISION_MUL)
        packed_prices = shift(packed_prices, -PRICE_SIZE)

    return Math(math).geometric_mean(x, True)


@external
@view
def get_virtual_price() -> uint256:
    return 10**18 * self.get_xcp(self.D) / vaETH(token).totalSupply()
...
def _claim_admin_fees():
    ...
    if xcp_profit > xcp_profit_a:
        ...
        if fees > 0:
            ...
            claimed: uint256 = vaETH(token).mint_relative(receiver, frac)
            xcp_profit -= fees*2
            self.xcp_profit = xcp_profit
            log ClaimAdminFee(receiver, claimed)

    total_supply: uint256 = vaETH(token).totalSupply()
...
def tweak_price(A_gamma: uint256[2],
    ...
    xp[0] = D_unadjusted / N_COINS_256
    for k in range(N_COINS-1):
        # This is the D / N*p_i used in calculating X_cp 
        xp[k+1] = D_unadjusted * 10**18 / (N_COINS_256 * price_scale[k])
    xcp_profit: uint256 = 10**18
    virtual_price: uint256 = 10**18

    if old_virtual_price > 0:
        xcp: uint256 = Math(math).geometric_mean(xp, True)
    ...
    if needs_adjustment:
        ...
        if norm > adjustment_step ** 2 and old_virtual_price > 0:
            ...
            xp[0] = D / N_COINS_256
            for k in range(N_COINS-1):
                xp[k+1] = D * 10**18 / (N_COINS_256 * p_new[k])
            # We reuse old_virtual_price here but it's not old anymore
            old_virtual_price = 10**18 * Math(math).geometric_mean(xp, True) / total_supply
...
@external
@nonreentrant('lock')
# i: index of token you are trading in 
# j: index of token you are trading out 
# dx: amount of token i with precision
...
def exchange(i: uint256, j: uint256, dx: uint256, min_dy: uint256):
    ...
    assert i < N_COINS_256  # dev: coin index out of range
    assert j < N_COINS_256  # dev: coin index out of range
    ...
    if True:  # scope to reduce size of memory when making internal calls later
        ...
        for k in range(N_COINS-1):
            price_scale[k] = bitwise_and(packed_prices, PRICE_MASK)  # * PRICE_PRECISION_MUL
            packed_prices = shift(packed_prices, -PRICE_SIZE) # shifting packed_prices

        precisions: uint256[N_COINS] = PRECISIONS
        xp[0] *= PRECISIONS[0]

        for k in range(1, 3): # ORIGINALLY range(1, N_COINS-1) 
...
def _calc_token_fee(amounts: uint256[N_COINS], xp: uint256[N_COINS]) -> uint256:
    ...
    N_COINS_256: uint256 = convert(N_COINS, uint256)
    fee: uint256 = self._fee(xp) * (N_COINS_256 / (4 * N_COINS_256 -1))
    S: uint256 = 0
    for _x in amounts:
        S += _x
    avg: uint256 = S / N_COINS_256
    Sdiff: uint256 = 0
    for _x in amounts:
        if _x > avg:
            Sdiff += _x - avg
...
@external
@nonreentrant('lock')
# NOTE: MY CODE THAT DEPOSTS LSD AND MINTS ALLETH
# 1. call add_liquidity with deposit lsd amount
# 2. mint allETH amount using oracle price
# 3. call add_liquidity with the allETH amount
def deposit(amount: uint256, coin_index: uint256, min_mint_amount: uint256):
    # Add LSD liquidity 
    amounts: uint256[N_COINS] = empty(uint256[N_COINS])
    amounts[coin_index] = amount
    va_min_mint_amount: uint256 = 0
    self.add_liquidity(amounts, va_min_mint_amount) # add LSD liquidity 

    # mint allETH
    price: uint256 = self._packed_view(coin_index - 1, self.price_oracle_packed)
    mintAmount: uint256 = amount / price * PRECISION
    assert mintAmount > min_mint_amount
    ERC20(coins[0]).mint(msg.sender, mintAmount * 2) # mint allETH

    # Add allETH liquidity 
    amounts = empty(uint256[N_COINS])
    amounts[0] = amount # allETH is index 0
    va_min_mint_amount = 0
    self.add_liquidity(amounts, va_min_mint_amount) # add allETH liquidity 


@internal # NOTE: WAS ORIGIINALLY EXTERNAL
# @nonreentrant('lock')
# NOTE SHOULD ONLY BE USED FOR SINGLE SIDED LIQUIDITY 
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256):
    ...
    xp: uint256[N_COINS] = self.balances #NOTE: comes with precision 
    ...
    if True:  # Scope to avoid having extra variables in memory later
        ...
        for i in range(N_COINS):
            ...
            # NOTE MY CODE: sets which coin is being added to the pool 
            # if all three coins are above 0, se
            if (amounts[i] > 0):
                coin_index = i
            bal: uint256 = xp[i] + amounts[i]
            xp[i] = bal
            self.balances[i] = bal # NOTE sanity check, has precision 
        xx = xp
        ...
        # NOTE: this function outputs xp[i] and xp[0] divided by price scale without precision and with price         
        for i in range(1, N_COINS):
        ...
        # NOTE: this function transfers the ERC20 tokens using amounts(with precision) 
        for i in range(0, 3):
            if amounts[i] > 0:
                # assert might be needed for some tokens - removed one to save bytespace
                ...
                if ix == INF_COINS:
                    ix = i #convert(i, uint256)
                else:
                    ix = INF_COINS-1

        assert ix != INF_COINS  # dev: no coins to add

        # NOTE: when is t greater than zero? 
        t: uint256 = self.future_A_gamma_time
        if t > 0:
    ...
    # NOTE MY CODE
    token_supply: uint256 = vaETH(vaTokens[coin_index]).totalSupply()
    old_token_supply: uint256 = token_supply
    # NOTE MY CODE
    total_deposit: uint256 = xp_old[coin_index] # excluding new deposit 
    if old_D > 0:
    ...
    # NOTE: What does this function do? 
    if old_D > 0:
        d_token_fee = self._calc_token_fee(amountsp, xp) * d_token / 10**10 + 1
        d_token -= d_token_fee
        token_supply += d_token
        # print("if d_token: ", d_token, hardhat_compat=True)
        
        # NOTE: mint vastETH if stETH, rETH varETH if rETH
        # new shares = (new_deposit / total_deposit) * total_shares 
        vaETHMint: uint256 = amounts[coin_index] * old_token_supply / total_deposit
        vaETH(vaTokens[coin_index]).mint(msg.sender, vaETHMint)
        ...
        if d_token > 10**5:
            if convert(ix, int128) < N_COINS:
                ...
                for k in range(N_COINS-1):
                    ...
                    packed_prices = shift(packed_prices, -PRICE_SIZE)       
                for i in range(N_COINS):
                    if i != convert(ix, int128): # WHATS GOING ON HERE 
                        if i == 0:
                            S += xx[0] * PRECISIONS[0]
                        else: # WHY IS THIS THROWING ERRORS
                            S += xx[i] * last_prices[convert(i, uint256)-1] * precisions[i] / PRECISION
        ...
        # NOTE: mint vastETH if stETH, varETH if rETH 
        vaETH(vaTokens[coin_index]).mint(msg.sender, d_token)
...
@external
@nonreentrant('lock')
# NOTE amounts[N_COINS] comes with precision 
def seed_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256):
    ...
    token_supply: uint256 = vaETH(token).totalSupply()
    if old_D > 0:
        d_token = token_supply * D / old_D - token_supply
    else:
        d_token = self.get_xcp(D)  # making initial virtual price equal to 1
    ...
    if old_D > 0:
        ...
        # NOTE: mint vastETH if stETH, rETH varETH if rETH
        vaETH(token).mint(msg.sender, d_token) 
        ...
        # NOTE: mint vastETH if stETH, varETH if rETH 
        vaETH(token).mint(msg.sender, d_token) 
        vaETH(vaTokens[1]).mint(msg.sender, amounts[1]) 
        vaETH(vaTokens[2]).mint(msg.sender, amounts[2])

    assert d_token >= min_mint_amount, "Slippage"

    log AddLiquidity(msg.sender, amounts, d_token_fee, token_supply)
    print("-----------------------", hardhat_compat=True)

# NOTE MY CODE: to redeem a collateral specific vaToken
@external
@nonreentrant('lock')
def redeem_liquidity(_amount: uint256, i: uint256, min_amounts: uint256[N_COINS]):
    """
    This withdrawal method is very safe, does no complex math
    """    
    _coins: address[N_COINS] = coins
    total_supply: uint256 = vaETH(vaTokens[i]).totalSupply() # with precision
    vaETH(vaTokens[i]).burnFrom(msg.sender, _amount)
    balances: uint256[N_COINS] = self.balances
    amount: uint256 = _amount - 1  # Make rounding errors favoring other LPs a tiny bit

    # for i in range(0, N_COINS): # NOTE commented out since we only redeem one at a time 
    d_balance: uint256 = balances[i] * amount / total_supply
    assert d_balance >= min_amounts[i]

    # calculate amount of allETH equivalent to d_balance, then transfer from sender
    ...
    # NOTE: what's going on with the D value here? why is it being updated 
    D: uint256 = self.D
...
def remove_liquidity(_amount: uint256, min_amounts: uint256[N_COINS]):
    ...
    _coins: address[N_COINS] = coins
    total_supply: uint256 = vaETH(token).totalSupply() # with precision
    vaETH(token).burnFrom(msg.sender, _amount)
    balances: uint256[N_COINS] = self.balances
    amount: uint256 = _amount - 1  # Make rounding errors favoring other LPs a tiny bit
...
def _calc_withdraw_one_coin(A_gamma: uint256[2], token_amount: uint256, i: uint256, update_D: bool,
    ...
    assert convert(i, int128) < N_COINS  # dev: coin out of range
...
def remove_liquidity_one_coin(token_amount: uint256, i: uint256, min_amount: uint256):
    ...
    self.balances[i] -= dy
    vaETH(token).burnFrom(msg.sender, token_amount)
