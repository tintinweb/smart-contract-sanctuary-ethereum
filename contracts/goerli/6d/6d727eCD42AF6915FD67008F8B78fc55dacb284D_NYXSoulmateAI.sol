...
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        // Keep track of burned token count
        burnedSouls++;
...
contract NYXSoulmateAI is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    address payable internal constant po =
        payable(0x27059456EdF2Fa89bB82cfdbf87CaC8b6925E5e7);
    address payable internal constant dev =
        payable(0x9933Fc3Ae2188Ff5C53AEB6a24f31104733CD87F);

    uint256 public MAX_PER_WALLET = 3;
    uint256 public MINT_COST = 88000000000000000; // 0.088 ETH
    uint256 public MAX_BATCH_SUPPLY = 5000; // can never be higher than MAX_SOUL_SUPPLY
    ...
    event burnMultipleSouls(address account, uint256[] tokenIds);
    event burnSoul(address account, uint256 tokenId);
    ...
    function mint(uint256 count) external payable nonReentrant {
        ...
        if (addressToFreeMint[msg.sender]) {
            require(
                addressToMinted[msg.sender] + count <= MAX_PER_WALLET + 1,
                "Exceeds your allowance"
            );
        } else {
            require(
                addressToMinted[msg.sender] + count <= MAX_PER_WALLET,
                "Exceeds your allowance"
            );
        }
        ...
        require(supply + count <= MAX_BATCH_SUPPLY, "Max supply reached");
    ...
    function presaleMint(
        ...
        bool hasFreeMint = freeRoot && !addressToFreeMint[msg.sender];
        ...
        } else if (addressToFreeMint[msg.sender]) {
            require(
                addressToMinted[msg.sender] + count <= MAX_PER_WALLET + 1,
                "Exceeds your allowance"
            );
        ...
        uint256 payedCount = hasFreeMint ? count - 1 : count;
        require(MINT_COST * payedCount == msg.value, "Invalid funds provided");
    ...
    function burnMultiple(uint256[] calldata tokenIds) external nonReentrant {
        ...
        emit burnMultipleSouls(_msgSender(), tokenIds);
    }
    ...
    function burnSingle(uint256 tokenId) external nonReentrant {
        ...
        _burn(tokenId);
        emit burnSoul(_msgSender(), tokenId);
    }
        ...
        onlyOwner
    {
        ...
        uint256 supply = _owners.length;
        require(
            supply + totalQuantity <= MAX_BATCH_SUPPLY,
            "Max supply reached"
        );
    ...
    function setMaxSupply(uint256 maxSupply) external onlyOwner {
        ...
        require(
            maxSupply > MAX_BATCH_SUPPLY,
            "New max supply must be higher than MAX_BATCH_SUPPLY"
        );
        MAX_BATCH_SUPPLY = maxSupply;
    }
    ...
    function setMaxPerWallet(uint256 maxPerWallet) external onlyOwner {
        require(
            ...
            "New max per wallet must be higher"
        );
