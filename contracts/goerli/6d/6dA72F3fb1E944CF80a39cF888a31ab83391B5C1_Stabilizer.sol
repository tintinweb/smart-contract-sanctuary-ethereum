...
contract Stabilizer {
  ...
  modifier onlyBankerOrBalancer {
    if(msg.sender != banker && msg.sender != balancer) { revert OnlyBankerOrBalancer(); }
    _;
  }
   ...
   */
  function burn(uint _sweep_amount) public onlyBankerOrBalancer {
  ...
  function defaulted() public view returns(bool) {
    return (
      ...
      IBalancer(balancer).isDefaulted(banker)
    );
  }
