/** @notice The Factory contracts (here, Factory721) have the responceblity for deploying 721 own contracts which can be used for  Minting NFTs , transferring , burning NFTs from the wallets
    assign operators for NFTs, returning Ipfs target path(tokenURIs) of given tokenId,..etc
*/
//SPDX-License-Identifier: MIT
pragma solidity ^0.8.14;

import "./RAJNFTUser721Token.sol";

contract Factory721 {

    //@notice Deployed the event is emited at the time of  factory function invoke. 
    //@param owner address of the contract owner.
    //@param contractAddress deployed own contract address.
    event Deployed(address owner, address contractAddress);

    //@notice deploy for transferring contract treasuryHolder to new treasuryHolder address.
    //@param _salt 
    //@param name name of the ERC721 token
    //@param symbol symbol of the ERC721 token
    //@param tokenURIPrefix base URI of the ERC721 token
    //@return address of the deployed contract. 

    //@dev see{RAJNFTUser721Token}.
    // emits {Deployed} event.
    function deploy(
        bytes32 _salt,
        string memory name,
        ...
        addr = address(
            new RAJNFTUser721Token{salt: _salt}(name, symbol, tokenURIPrefix)
        );
        RAJNFTUser721Token token = RAJNFTUser721Token(address(addr));
        token.transferOwnership(msg.sender);
        emit Deployed(msg.sender, addr);
    }
}

/** @notice The RAJNFTUser721Token own contracts have the responceblity for Minting NFTs , transferring , burning NFTs from the wallets
    assign operators for NFTs, returning Ipfs target path(tokenURIs) of given tokenId,..etc
*/
...
//@title RAJNFTUser721Token having the nature of ERC721 with splitRoyalty, Lazymint.
//@dev see{ERC721Enumerable, ERC721Burnable, ERC721URIStorage, Context, Counters, AccessControl}.

contract RAJNFTUser721Token is
    Context,
    ERC721Enumerable,
    ERC721Burnable,
    ERC721URIStorage,
