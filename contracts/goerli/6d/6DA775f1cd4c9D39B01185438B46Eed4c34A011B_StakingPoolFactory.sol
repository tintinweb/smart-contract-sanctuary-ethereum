...
contract StakingPoolFactory is ERC721Enumerable, Ownable{
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
    ...
    uint decimalofDeposit = (depositForAddress % 1 ether) / 10**14;
    uint shareForAddress = stakingPool.getDistributableShare(id);
    uint decimalofShare = (shareForAddress % 1 ether) / 10**14;
    ...
    string memory description = string(abi.encodePacked(
      'this fren has a deposit of ',uint2str(depositForAddress / 1 ether),'.', uint2str(decimalofDeposit),
      ' Eth in pool ', stakingPoolAddress, ' with claimable balance of ', uint2str(shareForAddress / 1 ether), '.', uint2str(decimalofShare), 'Eth'));
                    ...
                    Base64.encode(
                        bytes(
                              abi.encodePacked(
                                  ...
                                  uint2str(depositForAddress / 1 ether), '.', uint2str(decimalofDeposit),
                                  '"},{"trait_type": "claimable", "value": "Wei: ',
                                  uint2str(shareForAddress), '.', uint2str(decimalofShare),
                                  '"}], "image": "',
