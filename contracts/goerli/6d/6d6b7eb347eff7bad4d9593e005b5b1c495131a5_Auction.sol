...
contract Auction is IERC20Metadata, Mortal {
    ...
    function mint(address account, uint256 amount, uint256 tokenId) public returns (bool) {
        ...
        walletWinner = account;
        lastBid = amount;
        emit BidAuction(account, amount, tokenId, contractAddress);
        balances[account] = balances[account] - amount;
        myTotalSupply = myTotalSupply - amount;
        emit Transfer(address(0), account, amount);
        return true;
