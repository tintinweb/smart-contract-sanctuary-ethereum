...
contract WorldCupQuiz is Ownable, ReentrancyGuard {
    ...
    function buyTicket(uint256 _amount, address _referrer) public payable {
        require(_amount > 0, "Amount must be greater than 0");
        require(msg.value == _amount * ticketPrice, "Incorrect amount of ETH sent");
        if (referrals[_msgSender()] != address(0)) {
            require(referrals[_msgSender()] == _referrer, "Referrer cannot change");
        } else {
            referrals[_msgSender()] = _referrer;
        }
