...
contract Rental is Ownable, IERC721Receiver {
   ...
   struct Lease {
    ...
    function rentalTokensOfOwner(address account) public view returns (uint256[] memory ownersTokens) {
        ...
        uint256 index = 0;
        for(uint tokenId = 1; tokenId <= supply; tokenId++) {
            if ( book[tokenId].borrowerAddress == account && book[tokenId].currentlyOnLease ) {
                tmp[index] = book[tokenId].nftId;
                index +=1;
            }
        }
