...
contract A {
    ...
    function setPoll(string memory _title, string memory _contents) public {
        Polls.push(poll(Polls.length+1, _title, _contents, msg.sender, 0, 0, Status.registered)); // Polls에 poll을 새로 추가
    ...
    // poll의 정보 받아오기
    function getPoll(uint _a) public view returns(uint, string memory, string memory, address, uint, uint, Status) {
        return (Polls[_a-1].number, Polls[_a-1].title, Polls[_a-1].contents, Polls[_a-1].by, Polls[_a-1].agree, Polls[_a-1].disag, Polls[_a-1].status);
    }
    ...
    function vote(string memory _title, bool _b) public {
        ...
        for(uint i; i<Polls.length;i++) {
            ...
            // 개인의 이전 투표여부 확인 시도했다가 fail.. --> if(keccak256(bytes(Polls[i].title)) == keccak256(bytes(_title)) && keccak256(bytes(users[msg.sender].voted[_title]) == keccak256(bytes(""))))
    ...
    function finishPoll(string memory _title) public {
        for(uint i; i<Polls.length;i++) {
            if(keccak256(bytes(Polls[i].title)) == keccak256(bytes(_title)) && (Polls[i].agree+Polls[i].disag) >= 10) {
                if(100*Polls[i].agree/(Polls[i].agree+Polls[i].disag) >=70 ){
                    Polls[i].status = Status.passed; // 성공
                } else {
                    Polls[i].status = Status.failed; // 실패 -> 찬성/전체 * 100 < 60 실패
                }
            }
        }
    }
