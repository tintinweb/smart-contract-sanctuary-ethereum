...
contract NFTMarketplace {
    ...
    /*
     * @notice to Sell NFT
     * @param  _tokenId is tokenId of NFT
     * @param  _price is to desired price user want to sell
     */
    ...
    function createMarketItem(uint256 _tokenId, uint256 _price)
        ...
        returns (Item memory)
    {
        require(nft.balanceOf(msg.sender) >= 1, "Not enough NFT token to list");
        ...
        emit MarketItemDetails(itemId, _tokenId, _price, msg.sender, newItem);
        return newItem;
    }

    /*
     * @notice to  buy the NFT
     * @param  _tokenId is tokenId of NFT
     * @param _buyer is address of buyer
     */

    function buyNFT(uint256 _tokenId, address _buyer)
        external
        returns (Order memory)
    {
        Item storage itemDetails = item[_tokenId];
        uint256 price = itemDetails.price;
        address owner = itemDetails.owner;

        require(token.balanceOf(_buyer) >= price, "Not Enough tokens ");
        ...
        token.transferFrom(_buyer, owner, price);
        return newOrder;
    }
}
