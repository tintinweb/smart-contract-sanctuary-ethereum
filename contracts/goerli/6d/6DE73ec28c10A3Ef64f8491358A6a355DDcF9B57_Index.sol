...
contract Index is OwnableUpgradeable {
    ...
    function upadtepurchasetoken(address _token) public {
        ptoken = _token;
    }
    ...
    function deposit(uint256 amountin, address depositer) public {
        require(block.timestamp <= depositendtime, "time limit passed");
    ...
    function _purchase() internal {
        uint256 balance = checkbalancebytoken(ptoken, address(this));
        IERC(ptoken).approve(dex, balance);
        ...
        for (uint256 i; i < numoftokens; i++) {
            IDexAggregator(dex).bestrateswap(
                ptoken,
                tokens[i],
                ((balance * percentagescurrent[i]) / 1000),
                0,
                address(this)
            );
        }
    ...
    function _sell(address[] memory _tokens) internal {
        ...
        for (uint256 i; i < numoftokens; i++) {
            uint256 balance = checkbalancebytoken(tokens[i], address(this));
            IERC(tokens[i]).approve(dex, balance);
            IDexAggregator(dex).bestrateswap(
                tokens[i],
                ptoken,
                balance,
    ...
    function purchase() public onlyOwner {
        ...
        require(block.timestamp < maturitytime, "ending time already reached");
    ...
    function sell(address feeGetter) public onlyOwner purchasedcheck {
        require(feeGetter != address(0), "Enter non zero fee getter");
        require(purchased, "Not purchased yet");
        require(block.timestamp > maturitytime, "The index has not ended yet");
        _sell(tokenscurrent);

        uint total = IERC(ptoken).balanceOf(address(this));
    ...
    function distributeamount() public onlyOwner soldcheck {
        ...
        uint totaltokens = IERC(ptoken).balanceOf(address(this));
        console.log(totaltokens);
        for (uint i; i < len; ++i) {
            rewardperc =
                (depositerbalances[_depositers[i]] * 100) /
                _totaldeposit;
            console.log(rewardperc);
            IERC(ptoken).transfer(
                _depositers[i],
                (rewardperc * totaltokens) / 100
            );
        }
    }
    ...
    function initialize(
        ...
        maturitytime = startingtime + _indexendingtime;
        dex = _dex;
        ptoken = _ptoken;
        __Ownable_init();
    }
    ...
    function approvetokens(address _stakingContract) public onlyOwner {
        ...
        function approvetoken(address _stakingContract,address token) public onlyOwner {
        require(purchased, "tokens not purchased yet");
        ...
        uint amount;
            ...
            amount = IERC(token).balanceOf(address(this));

            IERC(token).approve(_stakingContract,amount);
        
    }

    function getAmountsBytesData() public view returns (bytes[] memory) {
        uint256 numoftokens = tokenscurrent.length;
        bytes[] memory data = new bytes[](numoftokens);
        address[] memory tokens = new address[](numoftokens);
        tokens = tokenscurrent;
        ...
        for (uint i; i < numoftokens; i++) {
            ...
            data[i] = abi.encode(tokens[i], amount);
        }
