...
contract Lottery is VRFConsumerBaseV2 {
    ...
    mapping(address => uint256) public fundAccounting;
    ...
    modifier onlyAboveThreshold(uint256 amount) {
        require(amount>5,"Insufficient funding");
        _;
    }
    ...
    function fund(uint256 amount) public onlyAboveThreshold(amount) {
        ...
        fundAccounting[msg.sender] = fundAccounting[msg.sender] + amount;

        if(includes(msg.sender)==false){
            participants.push(msg.sender);
            emit AddedNewParticipant(msg.sender);
        }
    }

    modifier hasAppropriateFunding(uint256 amount) {
        require(fundAccounting[msg.sender] >= amount,"You do not have this amount of funds inside the lottery!");
        _;
    }

    function withdraw(uint256 amount) public hasAppropriateFunding(amount){
    ...
    function fundsOf(address participant) public view returns(uint){
        return fundAccounting[participant];
    }

    function includes(address sender) public view returns(bool) {
    ...
    function fulfillRandomWords(uint256, /*requestId*/ uint256[] memory randomWords) internal override {
        ...
        uint minFunding = totalFunds;
        for(uint i=0; i < participants.length; i++){
            address participantsAddress = participants[i];
            if (fundAccounting[participantsAddress] < minFunding) {
                minFunding = fundAccounting[participantsAddress];
            }
        }

        uint payoutAmount = minFunding * participants.length / 10 * 9;
        casinoToken.transfer(participants[indexOfWinner],payoutAmount);
        totalFunds = totalFunds - payoutAmount;
        emit WinnerIndex(indexOfWinner);
        winner = randomWords[0];

        for(uint i=0; i < participants.length; i++){
            address participantsAddress = participants[i];

            fundAccounting[participantsAddress] = fundAccounting[participantsAddress] - minFunding;
        }
    }
