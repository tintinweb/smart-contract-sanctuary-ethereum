...
contract ExmentMarketPlace is IExmentMarketPlace {
    ...
    function collectionsOfOwner(
        ...
        unchecked {
            ...
            for (uint256 i = 0; collectionIdsIdx != collectionIdsLength; ++i) {
                ...
                if (ownership.owner != address(0)) { // collection hasnot been created yet
                    currOwnershipAddr = ownership.owner;
                }
                if (currOwnershipAddr == owner) {
                    collectionIds[collectionIdsIdx++] = ownership.collection;
    ...
    function createCollection(
        ...
        // push event
        emit CollectionCreated(newCollection, _seller, _collections.length, _localId);
    }
}
...
contract ExmentCollection is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    function mint(address to) external onlyMarket {
        _mint(to, ++_tokenIds);
    }
