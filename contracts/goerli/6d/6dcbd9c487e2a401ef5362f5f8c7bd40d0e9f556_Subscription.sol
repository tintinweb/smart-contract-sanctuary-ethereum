...
interface IERC5643 {
    ...
    function renewSubscription(address _subscriber, uint256 tokenId, uint256 expiration) external;
...
contract Subscription is ERC721, IERC5643, ERC721Enumerable, ERC721URIStorage {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    event Subscriptions(address _subscriber, string uri);

    mapping(address => mapping(uint256 => uint256)) private _subscriptions;
    mapping (address => uint256) public _isApprovedOrOwner;

    constructor() ERC721("Subscription", "SBS") {}

    function subsription(string memory uri) public {
         uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(msg.sender, tokenId);
        _setTokenURI(tokenId, uri);

        emit Subscriptions(msg.sender, uri);
    }
    ...
    function renewSubscription(address _subscriber, uint256 tokenId, uint256 expiration) external {
        ...
        _subscriptions[_subscriber][tokenId] = expiration;
        emit SubscriptionUpdate(tokenId, expiration);
    }

    function cancelSubscription(uint256 tokenId) external {
        require(_isApprovedOrOwner[msg.sender] == tokenId, "Caller is not owner nor approved");
        delete _subscriptions[msg.sender][tokenId];
        emit SubscriptionUpdate(tokenId, 0);
    }

    function expiresAt(uint256 tokenId) external view returns(uint256) {
        return _subscriptions[msg.sender][tokenId];
    }

    function isExires(uint256 tokenId) external returns(bool) {
        require(_subscriptions[msg.sender][tokenId] > block.timestamp);
