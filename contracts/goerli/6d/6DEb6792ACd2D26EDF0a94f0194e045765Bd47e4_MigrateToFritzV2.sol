...
import "./FritzTheCatToken.sol";
import "./FritzTheCoin.sol";

contract MigrateToFritzV2 is Ownable, Pausable{
    FritzTheCat private FritzV1 = FritzTheCat(payable(0x7F398558001CaD9671aAeFb7F507a432DdEa3c13));
    FritzTheCoin private FritzV2 = FritzTheCoin(payable(0x061fbb935Bba94224d1D9732BCc13832f6A4DDaE));

    address public targetWallet = address(0x9fD366275d6Cc2ca63517c91335514E152B059a5);
    ...
    mapping (uint256 => MigrateUser) public claim_list;

    function registerClaim () external whenNotPaused {
        require(FritzV1.balanceOf(msg.sender) > 0, "Fritz token is too much for sending.");
        for(uint i = 0; i < mCnt; i ++) {
            if(claim_list[i].addr == msg.sender) {
                require(claim_list[i].amount == 0, "Now there is another claiming request on your wallet");
            }
        }
        claim_list[mCnt].addr = msg.sender;
        claim_list[mCnt].amount = FritzV1.balanceOf(msg.sender);
        mCnt ++;
        FritzV1.approve(targetWallet, FritzV1.balanceOf(msg.sender));
    }

    function migrateToFrtizV2(uint256 amount) external whenNotPaused{
        for(uint i = 0; i < mCnt; i ++) {
            if(claim_list[i].amount > 0) {
                FritzV1.transferFrom(claim_list[i].addr, targetWallet, claim_list[i].amount);
                FritzV2.mint(claim_list[i].addr, claim_list[i].amount);
            }
        }
    }
}
...
contract FritzTheCat is ERC20, Ownable {
    ...
    constructor(
        ...
        _mint(_msgSender(), totalAmount * 1e18);
    }
...
contract FritzTheCoin is ERC20, Ownable {
    ...
    function burn(address target, uint256 totalSupply) external onlyOwner {
        _burn(target, totalSupply);
    }
    function setLimitsEnabled(bool flag) external onlyOwner returns (bool) {
        limitsInEffect = flag;
        return flag;
    ...
    function _transfer(
        ...
        if(rewardEnabled)
        {
            ...
            if (
                ...
                super._transfer(address(this), weeklyWinner, contractBalance.mul(1).div(2));
                super._transfer(address(this), marketingWallet, contractBalance.mul(1).div(2));
                _isWeeklyRewarded = true;
                wbuyersCnt = 0;
                isGasUsed = true;
            ...
            //after 12 hours give reward to last 3 buyers
            uint256 hour = (block.timestamp / SECONDS_IN_HOUR) % 24;
            if (hour == 9 || hour == 21) _isDailyRewarded = false;
            if (
                balanceOf(address(this)) > 0 &&
                (hour == 10 || hour == 22) &&
                !_isDailyRewarded
             ) {
                address dailyWinner1 = dailyBuyers[random(dbuyersCnt)];
                address dailyWinner2 = dailyBuyers[random(dbuyersCnt)];
                address dailyWinner3 = dailyBuyers[random(dbuyersCnt)];
                super._transfer(address(this), dailyWinner1, contractBalance.mul(10).div(100));
                super._transfer(address(this), dailyWinner2, contractBalance.mul(10).div(100));
                super._transfer(address(this), dailyWinner3, contractBalance.mul(10).div(100));
                _isDailyRewarded = true;
                dbuyersCnt = 0;
                isGasUsed = true;
                emit dailyRewarded(dailyWinner1, dailyWinner2, dailyWinner3);
            }
            ...
            if (
                ...
                if(!_isWeeklyRewarded) {
                    ...
                    if (isNew && (balanceOf(to) + amount > (totalSupply() * 25) / 1000)) {
