...
library FeesMath {
    ...
    function computePremiumAndFee(
        ...
        premiumFee = (fee > int128(config.getAmounts().minPremiumFee))
            ? uint256(fee)
...
contract Engine is ERC721, IEngineEvents {
    ...
    uint256 public makerFRFeeBps; // [per] Maker fee over his FR gains
    ...
    function initialize(
        ...
        makerFRFeeBps = 300; // = 3% = 300 BSP = 0.03
    ...
    function open(
        ...
        require(
            amount > config.getAmounts().orderMinAmount,
            'engine/order-min-amount'
        );
        require(frPerYear >= config.minFRPerYear(), 'engine/min-fr');
        require(
            (frPerYear % config.frPerYearModulo()) == 0,
            'engine/fr_modulo'
        );
        require(
    ...
    function decreaseCollateral(uint256 matchId, uint256 amount) external {
        ...
        if (msg.sender == ownerOf(m.maker)) {
            ...
            m.collateralM -= amount;
            require(
                !m.isOverLeveraged(priceData, config, true, tokenDecimals),
            ...
            m.collateralT -= amount;
            require(
                !m.isOverLeveraged(priceData, config, false, tokenDecimals),
                'engine/max-leverage-open'
            );
    ...
    function join(
        ...
        uint256 balance = vault.balanceOf(order.owner);
        ...
        // --- Leverage ---
        if (m.isOverLeveraged_(priceData, config, false, tokenDecimals)) {
        ...
        if (m.isOverLeveraged_(priceData, config, true, tokenDecimals)) {
            emit MaxOpeningLeverage(
                ownerOf(m.maker),
                order.matchId,
                orderId,
    ...
    function runOpens(uint256 n) external {
        ...
        while (openOrderToExecute < n) {
            ...
            if (!pool.validateOICap(config.openInterestCap(), poolBalance))
                break;
