...
import {INativeTokenImpactVault} from "src/interfaces/INativeTokenImpactVault.sol";
 ...
 */
contract SpiralsETHImpactVault is INativeTokenImpactVault {
        ...
        nonReentrant
    {
        ...
        uint256 amountRETHReceived = stake(_amount);
        _amount = convertToAsset(amountRETHReceived);
        ...
        nonReentrant
    {
        ...
        uint256 amountRETHReceived = stake(msg.value);
        uint256 amount = convertToAsset(amountRETHReceived);
    ...
    /**
     * @dev Wrapper for depositing ETH that returns the total amount deposited
     * minus Rocket Pool fees.
     */
    function stake(uint256 _amountETH) internal virtual returns (uint256) {
        uint256 rETHBeforeDeposit = getRETH().balanceOf(address(this));
        _stake(_amountETH);
        return getRETH().balanceOf(address(this)) - rETHBeforeDeposit;
    }

    /**
     * @dev Deposit ETH into RP Deposit Pool and receive rETH.
     */
    function _stake(uint256 _amountETH) internal virtual override {
        getDepositPool().deposit{value: _amountETH}();
    }
    ...
    function _withdraw(address _receiver, uint256 _amountETH)
        ...
        override
    {
        uint256 amountRETH = convertToYieldAsset(_amountETH);
        withdrawRETH(_receiver, amountRETH);
    }

    /**
     * @dev To comply with INativeTokenImpactVault interface,
     * Implement this when unstaking from ETH2 is available!
     */
     ...
     */
    function withdrawRETH(address _receiver, uint256 _amountRETH)
        public
        virtual
    {
        // Convert ETH -> rETH for receiver
        getRETH().transfer(_receiver, _amountRETH);
    }
...
abstract contract INativeTokenImpactVault is ImpactVault {
    function depositETH(address) external payable virtual;
