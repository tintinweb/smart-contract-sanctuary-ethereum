...
/**
 * @dev ASM The Next Legend - Minter Pro contract
 * @notice
 */
contract MinterPro is IMinterPro, Errors, AccessControl, ReentrancyGuard, Pausable {
    ...
    function mintPro(
        ...
        if (!validateSignature(to, char_.walletMinted(to), expiry, signature)) revert InvalidInput(INVALID_SIGNATURE);
     ...
     * @param numberMinted Number of tokens minted by the addr; serves as a nonce
    ...
    function validateSignature(
        ...
        bytes32 messageHash = _hash(addr, numberMinted, expiry);
    ...
    function _hash(
        ...
        return keccak256(abi.encode(addr, numberMinted, expiry));
    }
...
interface ICharacter {
    ...
    function walletMinted(address addr) external returns (uint256);
