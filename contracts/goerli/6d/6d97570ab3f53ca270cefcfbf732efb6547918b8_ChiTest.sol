...
contract ChiTest is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer {
  using Strings for uint256;
  ...
  uint256 public PhaseCollectionSize_ = 1000; // total number of nfts in Phase 1
  ...
  constructor() ERC721A("ChiTest","CTS") {
  ...
  function airdrop(address sendTo, uint quantity)public onlyOwner{
    ...
    uint256 public whiteListPerWallet = 5;
    uint256 public MAX_PER_WL_TRANSACTION = 5; // maximam amount that user can mint/Wallet
    ...
    function purchaseWhiteListTokens(uint256 _howMany, bytes32[] calldata _proof, uint256 _type) external payable {
        ...
        if(_type == 1){
        require(_howMany <= 1 , "can not mint this many");

        whiteListClaimedBy[msg.sender] += _howMany;
        require(whiteListClaimedBy[msg.sender] <= 1, "Purchase exceeds max allowed");
