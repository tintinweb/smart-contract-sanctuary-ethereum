...
contract CLProxyCLF is TransparentUpgradeableProxy {
    constructor( address _CLLCLF, address _CLPADM, bytes memory _data) TransparentUpgradeableProxy( _CLLCLF, _CLPADM, _data ) {}
}
...
contract CLFactory is Initializable {
    ...
    mapping(uint256 => address) public mapCLH;
        ...
        reinitializer( __UPGRADEABLE_CLF_VERSION__ )
    {
        ...
        mapCLH[ numCLH ] = address(0);
        mapCLHId[ address(0) ] = numCLH;
        ...
        returns( address )
    {
        ...
        BeaconProxy proxyCLH = new BeaconProxy(
            address( beacon ),
        ...
        numCLH++;
        mapCLH[ numCLH ] = address( proxyCLH );
        mapCLHId[ address( proxyCLH ) ] = numCLH;

        emit evtHouseCreation( address( proxyCLH ), _houseName, numCLH );

        return address( proxyCLH );
...
contract CLProxyBeaconCLH is UpgradeableBeacon {
    constructor( address _CLLCLH ) UpgradeableBeacon( _CLLCLH ) {}
}
...
contract CLStorage {
    ...
    uint8 public numManagers;
    uint8 public govRuleApprovPercentage;
    uint8 public govRuleMaxUsers;
    uint8 public govRuleMaxManagers; // immutable
