...
//     }
  ...
  function mintPlanetX(uint256 count) public payable  {
    if (tester = true){
  ...
  //  require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    ...
    if (allowedPlanet1MintCount(_msgSender()) > 0) {
      updatePlanet1MintCount(_msgSender(), count);
    } else {
      revert("Minting limit exceeded");
    }
    for (uint256 i = 0; i < count; i++) {
...
// Random state to test if/else allowlist trick on mintPlanetX() 
  ...
  */
  ...
  // ============================================================= //
    ...
    if (allowedPlanet2MintCount(_msgSender()) > 0) {
      updatePlanet2MintCount(_msgSender(), count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");
     ...
     if (allowedPlanet3MintCount(_msgSender()) > 0) {
      updatePlanet3MintCount(_msgSender(), count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    function mintPlanet4(uint256 count) public payable  {
    require(saleIsActive, "Sale not active");
    require(totalPlanet4Supply() + count - 1 < PLANET4_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= PLANET_PRICE * count, "Insufficient payment");

         if (allowedPlanet4MintCount(_msgSender()) > 0) {
      updatePlanet4MintCount(_msgSender(), count);
    } else {
      revert("Minting limit exceeded");
    }
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(msg.sender, totalPlanet4Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY );
    }

    payable(_splitContract).transfer(msg.value);
  }

    function mintPlanet5(uint256 count) public payable  {
    require(saleIsActive, "Sale not active");
 ...
 //   require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(msg.sender, totalPlanet5Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY);
    }

    payable(_splitContract).transfer(msg.value);
  }

    function mintPlanet6(uint256 count) public payable  {
    require(saleIsActive, "Sale not active");
 ...
 //   require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    ...
    for (uint256 i = 0; i < count; i++) {
      planet5SupplyCounter.increment();
      _safeMint(msg.sender, totalPlanet6Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY);
    }

    payable(_splitContract).transfer(msg.value);
  }

    function mintPlanet7(uint256 count) public payable  {
    require(saleIsActive, "Sale not active");
 ...
 //   require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(msg.sender, totalPlanet7Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY);
    }

    payable(_splitContract).transfer(msg.value);
  }

    function mintPlanet8(uint256 count) public payable  {
    require(saleIsActive, "Sale not active");
 ...
 //   require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(msg.sender, totalPlanet8Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY + PLANET7_SUPPLY);
    }

    payable(_splitContract).transfer(msg.value);
  }

    function mintPlanet9(uint256 count) public payable  {
    require(saleIsActive, "Sale not active");
 ...
 //   require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    ...
    for (uint256 i = 0; i < count; i++) {
      planet8SupplyCounter.increment();
      _safeMint(msg.sender, totalPlanet9Supply() + PLANET1_SUPPLY + PLANET2_SUPPLY + PLANET3_SUPPLY + PLANET4_SUPPLY + PLANET5_SUPPLY + PLANET6_SUPPLY + PLANET7_SUPPLY + PLANET8_SUPPLY);
    }
