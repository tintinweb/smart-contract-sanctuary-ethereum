...
contract Slot is VRFConsumerBaseV2, Ownable, KeeperCompatibleInterface {
    ...
    uint256 private constant NUM_WORDS_PER_BUY = 50; // NUM_REELS * SPINS_PER_BUY
    ...
    uint256 private s_storedWordsIdx;
    ...
    //     uint256[] memory storedRandomWords = s_storedRandomWords;
    //     uint256 storedRandomWordsEndIdx = s_storedWordsIdx;
    //     uint256[] memory slotDigits = new uint256[](NUM_WORDS_PER_BUY);
    //     uint256 balanceChg = 0;

    //     for (
    //         uint16 i = 0;
    //         i < NUM_WORDS_PER_BUY; /*now incrementing at end of function with an unchecked function to save gas */

    //     ) {
    //         unchecked {
    //             slotDigits[i] = storedRandomWords[storedRandomWordsEndIdx] % NUM_STOPS;

    //             // run this at end of loop
    //             storedRandomWordsEndIdx -= 1;
    //             i += 1;
    //         }
    //     }

    //     s_storedWordsIdx = storedRandomWordsEndIdx; // used for next spin
    //     delete storedRandomWordsEndIdx;
    ...
    //         // for each reel - check for normal winning spin
    //         for (uint16 e = 0; e < NUM_REELS; ) {
    //             // check if we've already reached a winning spin (excluding lucky reels which we handle later)
    //             if (spinStreak >= STOPS_TO_WIN) {
    //                 break; // winner if the spin streak array reaches the length of spins to win
    //             }

    //             // get the idx of the random number stored in slotDigits array
    //             uint256 slotDigitIdx;
    //             unchecked {
    //                 slotDigitIdx = i + (e * NUM_REELS); // e.g. if this is the second spin, will be e = 1 * 5 reels so the 0th reel in that spin will be index 5 in the random numbers array
    //             }
    ...
    //         // for each reel -- check for lucky winning spin
    ...
    //         // now that we have arrays, look for a winner / winner type
    //         bool isWinner = false;
    ...
    //         playerWins[i] = isWinner;
    //         playerLuckyWins[i] = isLuckyWinner;
    //         playerWinningStops[i] = spinStreakStop;
    ...
    //         delete isWinner;
    //         delete isLuckyWinner;
    //         delete spinStreakStop;
    //         delete spinStreak;
    //         delete spinStreakLucky;
    //         delete spinStreakLuckyTemp;
    //     }
    //     s_playerBalance[playerAddress] += balanceChg;
    //     s_playerWinningStopsArray[playerAddress] = playerWinningStops;
    //     s_playerWinsArray[playerAddress] = playerWins;
    //     s_playerLuckyWinsArray[playerAddress] = playerLuckyWins;
    // }
    ...
    function buySpins() public payable {
        ...
        for (uint256 i = 0; i < (NUM_WORDS_PER_BUY); ) {
            // console.log(i);
    ...
    function fulfillRandomWords(
        ...
        unchecked {
            s_storedWordsIdx = randomWords.length - 1;
        }
