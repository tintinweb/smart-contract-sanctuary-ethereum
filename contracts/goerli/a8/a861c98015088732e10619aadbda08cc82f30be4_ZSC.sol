...
contract ZSC {
    ...
    function rollOver(bytes32 yHash) internal {
        uint256 epoch = ZSCStorage.layout().epochLength;
    ...
    // test function
    function getData(Utils.G1Point memory y)
        public
        view
        returns (
            Utils.G1Point[2][] memory accounts,
            uint256 rollover,
            uint256 blockTime,
            uint256 lastUpdate,
            uint256 e
        )
    {
        // in this function and others, i have to use public + memory (and hence, a superfluous copy from calldata)
        // only because calldata structs aren't yet supported by solidity. revisit this in the future.
        accounts = new Utils.G1Point[2][](2);
        bytes32 yHash = keccak256(abi.encode(y));
        accounts[0] = ZSCStorage.layout().acc[yHash];
        accounts[1] = ZSCStorage.layout().pending[yHash];
        rollover = ZSCStorage.layout().lastRollOver[yHash];
        blockTime = block.timestamp;
        e = block.timestamp / ZSCStorage.layout().epochLength;
        lastUpdate = ZSCStorage.layout().lastGlobalUpdate;
    }
}
