...
function _deposit() internal {
    ...
    require(token.approve(address(this), _value), "Approval of deposit amount failed");
...
function withdraw() public _withdrawalIsEnabled {
    ...
    require(nft.balanceOf(msg.sender) > 0 || isNFTHolder[msg.sender] == true, "This wallet has No Jafoo NFTs");
   ...
   // require(token.approve(address(this), withdrawAmount), "Approval withdraw failed");
    require(token.transfer(sender, withdrawAmount), "Transfer Failed");
...
function adminWithdraw() public _onlyOwner{
    ...
    require(token.approve(address(this), totalBalance), "Approval Failed");
    require(token.transferFrom(address(this), owner, totalBalance), "Transfer Failed");
