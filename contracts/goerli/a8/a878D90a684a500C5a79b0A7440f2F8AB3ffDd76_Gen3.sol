...
contract Gen3 is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("Gen3", "GEN3") {
        USDC = IERC20(0x07865c6E87B9F70255377e024ace6630C1Eaa37F); // USDC mainnet address
        feeRecipient = msg.sender;
        liquidity = 0;
        exchangeRate = 10**6;
        maxFee = 0.1 * (10**6);
        blockTimeout = 240; //blocks (around 21 days for mainnet)
    }
    ...
    // function burnG3(uint256 value) public {
    //     require(
    ...
    //     uint256 fee = value.mul(percentFee).div(10**this.decimals());
    //     value -= fee;

    //     uint256 USDCfee = fee.mul(exchangeRate).div(10**this.decimals());
    //     uint256 USDCreturn = value.mul(exchangeRate).div(10**this.decimals());

    //     lastWithdraw[msg.sender] = block.number;
    //     liquidity -= USDCfee + USDCreturn;

    //     USDC.safeTransfer(owner(), USDCfee); // why is this 9.9999
    //     USDC.safeTransfer(msg.sender, USDCreturn);
    // }

    // value = number of GEN3 to burn
    // this version uses less gas
    function burnG32(uint256 value) public {
        require(
            liquidity > value.mul(exchangeRate).div(10**this.decimals()),
            "Not enough liquidity"
        );
        ...
        USDC.safeTransfer(owner(), fee); // why is this 9.9999
