...
contract UPNFTStaking_MR is Ownable, ReentrancyGuard, ERC721Holder {
     ...
     */
    function earned(address token) public view returns (uint256 reward) {
        for (uint256 i; i < stakedNFTs[msg.sender].length; i++) {
            reward +=
                (accumulatedRewardPerNFT(token) -
                    abortedRewardPerNFT[token][stakedNFTs[msg.sender][i]]) /
                1e18;
        }
    }
        ...
        onlyRewardDistributor
    {
        ...
        if (block.timestamp < periodFinish[token]) {
            ...
            require(block.timestamp < periodFinish[token], "Staking: not reasonable duration");
            rewardRate[token] = leftover / (periodFinish[token] - block.timestamp);
        }
    }
