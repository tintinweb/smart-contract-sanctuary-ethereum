    ...
    DefaultOperatorFiltererUpgradeable
{
    ...
    mapping(address => uint64) public mintCountPerWallet;
    ...
    function initialize(
        ...
        _tokenIdCounter.increment(); // @dev start index is 1
    ...
    function mint(address to, string calldata uri) external payable {
        _checkMintConditions(
            1,
            msg.value
        );
        _checkGlobalPerWalletMax(msg.sender, 1);
    ...
    function mintBatch(
        ...
        _checkGlobalPerWalletMax(msg.sender, quantities);
        ...
        emit TokensMinted(msg.sender, quantities);
    }
    ...
    function airdrop(
        ...
        for (uint256 i = 0; i < numRecipients; ) {
            for (uint j =0; j < quantities[i]; ) {
                uint256 tokenId = _tokenIdCounter.current();

                _safeMint(recipients[i], tokenId);
                _setTokenURI(tokenId, uris[i][j]);
    ...
    function _checkMintConditions(
        ...
        if (balance != quantity * tokenPrice) {
            revert InvalidPrice();
        }
    ...
    function _checkGlobalPerWalletMax(
        ...
        if (
            maxPerWallet > 0 &&
            mintCountPerWallet[wallet] + quantity > maxPerWallet
        ) {
    ...
    function getMintBalance(address wallet) external view returns (uint256) {
        return mintCountPerWallet[wallet];
    }
