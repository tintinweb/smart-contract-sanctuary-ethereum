...
contract RedeemBonds is AutomationCompatibleInterface {
    ...
    /**
     * @notice Gets a list of bonds that can be redeemed
     * @return redeemableBonds list of bond addresses that can be redeemed
     */
    function getRedeemableBonds()
        public
        view
        returns (address[] memory redeemableBonds)
    {
        address[] memory redeemables = new address[](s_watchList.length);
        uint256 count = 0;
        ...
        for (uint i = 0; i < s_watchList.length; i++) {
            ...
            if (
                ...
                redeemables[count] = s_watchList[i];
            }
        }
    ...
    function redeemBonds(address[] memory redeemableBonds) public {
        address[] memory buyers;
        ...
        for (uint i = 0; i < redeemableBonds.length; i++) {
            buyers = s_bondListings[redeemableBonds[i]].buyers;

            for (uint j = 0; j < buyers.length; j++) {
                bool success = payable(buyers[j]).send(
                    s_bondListings[redeemableBonds[i]].profit
                );

                if (success) {
                    s_bondListings[redeemableBonds[i]]
                        .lastRedeemTimeStamp = uint56(block.timestamp);

                    emit RedeemSucceeded(redeemableBonds[i], buyers[j]);
                }
            }
        }
    }
    ...
    {
        address[] memory redeemableBonds = getRedeemableBonds();
        upkeepNeeded = redeemableBonds.length > 0;
        performData = abi.encode(redeemableBonds);

        return (upkeepNeeded, performData);
    }

    function performUpkeep(bytes calldata performData) external override {
        address[] memory redeemableBonds = abi.decode(performData, (address[]));
        redeemBonds(redeemableBonds);
    }
