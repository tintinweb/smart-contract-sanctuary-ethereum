...
// File: contracts/MultiErc20Mint.sol
...
contract Bfactor is ERC721A, Ownable, ReentrancyGuard, Pausable{
    ...
    // Struct to keep track of allowed ERC20 tokens
    struct TokenInfo {
        IERC20 paytoken;
        uint256 costvalue;
    }
    ...
    function pumlxMint(uint256 _quantity, bytes32[] memory proof, uint256 _pid) external payable whenNotPaused userOnly nonReentrant {
        TokenInfo storage tokens = AllowedTokens[_pid];
        IERC20 paytoken;
        paytoken = tokens.paytoken;
        pumlPrice = tokens.costvalue;

        require(isPumlxValid(proof, keccak256(abi.encodePacked(msg.sender))), "Not a part of token pre-sale");
        require(pumlxEnabled, "ERC20 token mint is currently paused");
        require(msg.value >= (_quantity * pumlPrice), "Not enough ether sent");
        ...
        paytoken.transferFrom(msg.sender, address(this), (_quantity * pumlPrice));
    ...
    function addToken(IERC20 _paytoken, uint256 _costValue) public onlyOwner userOnly nonReentrant { 
    ...
    function withdrawToken(uint256 _pid, address _to) external onlyOwner nonReentrant userOnly{
        TokenInfo storage tokens = AllowedTokens[_pid];
        IERC20 paytoken;
        paytoken = tokens.paytoken;
