...
contract Marketplace is ERC1155Holder 
{
    ...
    // Mappings
    // mapping(address => uint256) public pendingResturns; vanished
    mapping(uint256 => RoyaltyInfo) public royaltyinformation;
    ...
    function listItemForFixedPrice(uint256 _tokenId, uint256 _amount , uint256 _price , uint256 _royaltyFee, address _nftAddress , address _royaltyRec)public  OnlyTokenHolders(_tokenId , _nftAddress) returns(uint256)
    {
        ...
        RoyaltyInfo memory RINF = royaltyinformation[_tokenId];

        if(royaltyinformation[_tokenId].valid ==  false)
        {   
            ...
            Fixedprices.push(FixedPrice(false,true,msg.sender,address(0),_nftAddress, RINF.RoyaltyRecipient,_price,0,newItemId,_tokenId,RINF.RoyaltyFee,_amount));
        ...
        // if(!checkAddress(RINF.RoyaltyRecipient , _tokenId))
        // {
        //     revert();
        // }
        // else
    ...
    public  OnlyTokenHolders(tokenId , _nftAddress) returns(uint256)
    {
        ...
        RoyaltyInfo memory RINF = royaltyinformation[tokenId];

        if(royaltyinformation[tokenId].valid ==  false)
        {   
            RINF.TokenId = tokenId;
            RINF.RoyaltyFee = _royaltyFee;
            RINF.RoyaltyRecipient = _royaltyRecipient;
            RINF.valid = true;
        }
        ...
        // if(!checkAddress(RINF.RoyaltyRecipient , tokenId))
        // {
        //     revert();
        // }
        // else
    ...
    { 
        ...
        emit buyFixedprice(Fixedprices[Id].tokenid,Fixedprices[Id].totalcopies,Fixedprices[Id].newowner ,ServiceFee,RoyaltyFes,amountToSendSeller );
