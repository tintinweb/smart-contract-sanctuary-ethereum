...
// File: contracts\CyberKyodai1.sol

contract CyberKyodai1 is IKyodai, ERC721K1, UUPSUpgradeable {
    bool public mintActive;

    // address traitGenAddy;    --> 0
    // address descriptorAddy;  --> 1
    // address shateiAddy;      --> 2
    // address yenAddy;         --> 3
...
/_/ /_/ /_/_/_/ /_/\__/  /_/  \__,_/_/ /_/\___/\__/_/\____/_/ /_/ 
   ...
   */
    ...
    function mint() public payable noCheating {
        // check donation: msg.value to increase rare chance
        require(mintActive, "mint not active");
        require(addressToMinted[msg.sender] == false, "can not mint twice");
        require(_totalSupply + 1 <= 3333, "fully minted");

        addressToMinted[msg.sender] = true;
        mintInternal(
            msg.sender,
            // msg.value == 0 ? 0 : _increaseChance(msg.value)
            0
        );
    }

    //shatei order must be druggie, hacker, cyborg for the calldata
    function mintKyodai(uint256[] calldata shateis) public noCheating {
        IShatei shatei = IShatei(officialAddys[2]);
        require(_totalSupply + 1 <= 6666, "fully minted");
        for (uint256 i = 0; i < 3; ++i) {
            require(
                shateis[i] > (6666 + (11111 * i)) &&
                    shateis[i] <= (6666 + (11111 * (i + 1))),
                "class incompatible"
            );
            require(
                shatei.checkLevel(shateis[i]) >= currentMinShateiLevel(),
                "insufficient level"
            );
            shatei.burn(shateis[i], msg.sender);
        }
        INeoYen(officialAddys[3]).burn(msg.sender, 350 * 1 ether);
        mintInternal(msg.sender, 0);
    }

    function currentMinShateiLevel() public view returns (uint256) {
        if (_totalSupply < 3333) return 0;
        if (_totalSupply >= 3333 && _totalSupply < 4444) return 10;
        if (_totalSupply >= 4444 && _totalSupply < 5555) return 15;
        if (_totalSupply >= 5555 && _totalSupply < 6666) return 20;

        revert();
    }

    function mintInternal(address to, uint256 chanceGain) internal {
        uint256 tokenId = _totalSupply + 1;
        kyodais[tokenId] = Kyodai({
            lvlProgress: 0,
            level: 1,
            traitHash: ITraitGen(officialAddys[0]).genKyodaiHash(
                tokenId,
                to,
                0,
                chanceGain
    ...
    // TODO manually mint until 50 token max for owner
    function ownerMint() external onlyOwner {
        require(ownerMinted < 50);
        // ownerMinted++;
        for (uint256 i = 0; i < 10; ++i) {
            mintInternal(owner(), 0);
        }
        ownerMinted += 10;
    }
...
*/
        ...
        returns (uint256)
    {
        ...
        uint256 production = lvl >= 50 ? 25 : ((lvl * 2) / 5) + 5;
    ...
    //     require(
    //         activity.action == Action.TRAINING ||
    //             activity.action == Action.FARMING
    //     );
