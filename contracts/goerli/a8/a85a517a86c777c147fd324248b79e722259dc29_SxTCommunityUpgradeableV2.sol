...
contract SxTCommunityUpgradeableV2 is ERC1155SupplyUpgradeable, WhitelistUpgradeable {
    ...
    event SxTNFTMinted(uint256 id, address to);
    ...
    function mintNFT(uint256 id) external whenNotPaused nonReentrant whenWhiteListedUser(_msgSender()) {   
        require(!compareStrings(tokenDetails[id].tokenUri, ""), "SxTCommunity: URI nonexistent token");    
        require(!checkMaxSupply(id, DEFAULT_AMOUNT, totalSupply(id)), "SxTCommunity: Total supply exceeded maximum supply");
        require(!isAddressUtilized[id][_msgSender()], "SxTCommunity: Already minted this NFT once to this account");
        isAddressUtilized[id][_msgSender()] = true;
        _mint(_msgSender(), id, DEFAULT_AMOUNT, "");
        emit SxTNFTMinted(id, _msgSender());
    }
...
contract WhitelistUpgradeable is SxTCommunityStorageUpgradeable {

    // Event to trigger the addition of address to whitelist mapping
    event AddedToWhiteList(address _user);

    // Event to trigger the removal of address from whitelist mapping
    event RemovedFromWhiteList(address _user);

    function __WhiteListEnabled_init() internal onlyInitializing {
        __WhiteListEnabled_init_unchained();
    }

    function __WhiteListEnabled_init_unchained() internal onlyInitializing {
    }
    
    // This function would add an address to the whitelist mapping
    /// @param _user The account to be added to whitelist

    function _addToWhiteList(address _user) internal virtual returns (bool) {
        whiteList[_user] = true;
        emit AddedToWhiteList(_user);
        return true;
    }

    // This function would remove an address from the whitelist mapping
    /// @param _user The account to be removed from whitelist

    function _removeFromWhiteList(address _user) internal virtual returns (bool) {
        delete whiteList[_user];
        emit RemovedFromWhiteList(_user);
        return true;
    }

    // This function would check an address from the whitelist mapping
    /// @param _user The account to be checked from whitelist mapping

    function _isWhiteListedUser(address _user) internal virtual returns (bool){
        return whiteList[_user];
    }

    // Modifier to check address from the whitelist mapping
    /// @param _user The account to be checked from whitelist mapping

    modifier whenWhiteListedUser(address _user) {
        require(_isWhiteListedUser(_user), "SxTCommunity: This address is not in the whitelist. Go to the Space and Time discord and comment your wallet address in the whitelist channel before minting!");
        _;
    }

    /// @dev add an array of addresses to the whitelist

    function addToWhitelist(address[] calldata newAddresses) external onlyOwner {
        for (uint i = 0; i < newAddresses.length; i++) {
            if(address(newAddresses[i]) != ZERO_ADDRESS){
                _addToWhiteList(newAddresses[i]);
            }
        }
    }
    ...
    function removeFromWhitelist(address[] calldata existingAddresses) external onlyOwner {
        for (uint i = 0; i < existingAddresses.length; i++) {
            _removeFromWhiteList(existingAddresses[i]);
        }
    }
