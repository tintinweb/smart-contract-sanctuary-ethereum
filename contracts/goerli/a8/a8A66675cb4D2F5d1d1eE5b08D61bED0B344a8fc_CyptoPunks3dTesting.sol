...
contract CyptoPunks3dTesting is
    ERC721,
    Pausable,
    ERC721Burnable,
    AccessControl
{
    ...
    address public burnAddress = 0x000000000000000000000000000000000000dEaD; // Amulay sir's wallet
    ...
    uint256 public Discount; // upto one decimals 50 % -> 500
    uint256 public whitelistTimeBound;
    ...
    string public baseURI = "http://18.237.220.76/api/crypt3dpunks/";

    mapping(uint256 => bool) public stopTransfer;
    mapping(uint256 => address[]) public whiteListedAddress;
    ...
    constructor(
        ...
        address _d,
        uint256 amount
    ) ERC721("CyptoPunks3d", "CP3d") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);
        ExistingERC1155 = _e;
        CryptoCurrencies[0] = _a;
        CryptoCurrencies[1] = _b;
        CryptoCurrencies[2] = _c;
        CryptoCurrencies[3] = _d;

        validators[msg.sender] = true;
        owner = msg.sender;
        EtherPrice = amount; // current price of ethereum upto 2 decimals.
        unpauseTimeStamp[1] = block.timestamp;
        Discount = 50;
        whitelistTimeBound = 259200; // 60*60*24*3  - 3 days
    }

    function ADD_DEFAULT_ADMIN_ROLE(address account)
        public
        virtual
    ...
    // Caution
    function LEAVE_DEFAULT_ADMIN_ROLE() public virtual {
        renounceRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function getNFTPriceinEth(uint256 quantity) public view returns (uint256) {
        require(quantity >= 0 && quantity <= 5, " Invalid quantity");
        return (priceCardInDollar[Round][quantity] * (10 * 20)) / EtherPrice;
    }

    function updateEtherPrice(uint256 price)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        EtherPrice = price;
    ...
    function AlterPayment(uint256 token) public onlyRole(DEFAULT_ADMIN_ROLE) {
        paymentPermitted[token] = !paymentPermitted[token];
    }

    function addwhiteListBatch(uint256 _round, address[] memory _addresses)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function addwhiteListAddress(uint256 _round, address _address)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_round >= 1 && _round <= 10);
    ...
    function removeWhiteListAddress(uint256 _round, address _address)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            presentInWhitelist(_round, _address),
        ...
        whenNotPaused
    {
        ...
        uint256 localDiscount;
        uint256 amount;
        // NOTE: EtherPrice is stored upto two decimals i.e. 137890

        if (!validators[msg.sender]) {
            if (presentInWhitelist(Round, msg.sender)) {
                require(
                    block.timestamp <=
                        unpauseTimeStamp[Round] + whitelistTimeBound,
                    "Time limit exceeded to redeem"
                );
                localDiscount = 1000 - Discount;
                deleteFromWhiteList(Round, msg.sender);
            } else {
                localDiscount = 1000;
            }
            if (_quantity != 10) {
                uint256 temp = _quantity - 1;
                amount =
                    ((priceCardInDollar[Round][temp] *
                        (10**20) *
                        localDiscount) * _quantity) /
                    (EtherPrice * 1000);
            } else {
                amount =
                    ((priceCardInDollar[Round][5] * (10**20) * localDiscount) *
                        _quantity) /
                    (EtherPrice * 1000);
            }
            ...
            // } else if (_quantity == 2) {
            //     require(msg.value >= ((priceCardInDollar[Round][1]*(10**20)*localDiscount)*_quantity)/(EtherPrice*1000), "Not enough ethers");
            // } else if (_quantity == 3) {
            //     require(msg.value >= ((priceCardInDollar[Round][2]*(10**20)*localDiscount)*_quantity)/(EtherPrice*1000), "Not enough ethers");
            // } else if (_quantity == 4) {
            //     require(msg.value >= ((priceCardInDollar[Round][3]*(10**20)*localDiscount)*_quantity)/(EtherPrice*1000), "Not enough ethers");
            // } else if (_quantity == 5) {
            //     require(msg.value >= ((priceCardInDollar[Round][4]*(10**20)*localDiscount)*_quantity)/(EtherPrice*1000), "Not enough ethers");
            // } else if (_quantity == 10) {
            //     require(msg.value >= ((priceCardInDollar[Round][5]*(10**20)*localDiscount)*_quantity)/(EtherPrice*1000), "Not enough ethers");
            // }
        }
    ...
    function reveal(uint256 _round) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function unpause(
        ...
        uint256 _whitelistTimeBound
    ) public onlyRole(PAUSER_ROLE) {
        require(
            _discountPercentage <= 1000,
    ...
    function updateDiscountPercentage(uint256 _discountPercentage)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        Discount = _discountPercentage;
    ...
    function updatewhitelistTimeBound(uint256 _whitelistTimeBound)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        whitelistTimeBound = _whitelistTimeBound;
    ...
    function withdrawEthers(address to, uint256 amount)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            owner != validator,
            "Can not remove the owner of the contract as validator"
        );
        validators[validator] = false;
    }

    function presentInWhitelist(uint256 _round, address _address)
        internal
        view
        returns (bool)
    {
    ...
    function deleteFromWhiteList(uint256 _round, address _address) internal {
        uint256 index;
        address[] memory a = whiteListedAddress[_round];
        for (uint256 i = 0; i < a.length; i++) {
