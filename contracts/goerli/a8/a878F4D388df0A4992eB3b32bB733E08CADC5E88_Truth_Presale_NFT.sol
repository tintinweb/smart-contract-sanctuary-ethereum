...
contract Truth_Presale_NFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    */
    ...
    function airdrop(address recipient, uint256 numTokens) public onlyOwner {
        // Check that the recipient address does not already own an NFT
        require(_balances[recipient] == 0, "Recipient already owns an NFT");

        // Check that the recipient address is approved for airdrop
        require(_whitelist[recipient], "Recipient is not approved for airdrop");

        // Check that the recipient address is not the contract owner
        require(recipient != msg.sender, "Cannot airdrop to contract owner");
        ...
        require(numTokens > 0, "Number of tokens to be distributed must be greater than zero");

        // Check that the number of tokens to be distributed does not exceed the maximum supply
        require(numTokens + _totalSupply <= MAX_SUPPLY, "Cannot exceed maximum supply");
