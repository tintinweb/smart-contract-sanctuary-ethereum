...
contract ManagedList is ReentrancyGuard, Ownable {
    ...
    struct Batch {
        ...
        uint8 fee; // % of whitelist sale amount that gets paid to this contract
    ...
    modifier isValidCreate(bytes memory _signature, string memory _batchId, uint8 _fee) {
        bytes32 messageHash = keccak256(
            abi.encodePacked(address(this), msg.sender, _batchId, "create", _fee)
        );
    ...
    modifier isValidReserve(bytes memory _signature, uint256 _batchId) {
        bytes32 messageHash = keccak256(
            abi.encodePacked(address(this), msg.sender, _batchId, "reserve")
        );
        address signer = messageHash.toEthSignedMessageHash().recover(
            _signature
        );
    ...
    function createBatch(Batch memory _batch, bytes memory _signature) public payable isValidCreate(_signature,  _batch.externalId, _batch.fee) ensureUniqueBatchExternalId(_batch.externalId) nonReentrant {
        require(contractPaused == false, "Contract is paused");
        require(_batch.spotsPerWallet > 0, "You must allow at least 1 spot per wallet");
        require(_batch.maxSpots > 0, "You must allow at least 1 spot");
        require(_batch.paymentAddress != address(0), "You must provide a payment address");
        require(msg.value >= priceToCreateBatch, "Not enough ETH sent");
        require(_batch.fee <= 100, "Fee cannot be greater than 100");
        require(_batch.fee >= 0, "Fee cannot be less than 0");

    batchesList[batchCounter] = Batch({
            id: batchCounter,
            externalId: _batch.externalId,
            creator: msg.sender,
            paymentAddress: _batch.paymentAddress,
            paused: true,
            fee: _batch.fee,
            price: _batch.price,
            maxSpots: _batch.maxSpots,
            spotsTaken: 0,
            spotsPerWallet: _batch.spotsPerWallet
        });
    ...
    function reserveSpot(uint256 _batchId, uint8 _requestedSpotQuantity, bytes memory _signature) public payable isValidReserve(_signature, _batchId) nonReentrant {
