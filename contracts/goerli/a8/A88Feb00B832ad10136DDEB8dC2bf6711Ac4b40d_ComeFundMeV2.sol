...
contract ComeFundMeV2 is Ownable, Pausable {
    ...
    mapping(bytes32 => mapping(address => uint256)) private _amountDonated;
    ...
    event CampaignWithdrawalReceived(
        bytes32 campaignId,
        address withdrawer,
        uint256 amount
    );
    ...
    function endCampaign(bytes32 campaignId) external whenNotPaused {
        ...
        require(block.number > c.deadline, "Cannot end campaign prematurely");
        c.isAlive = false;
        ...
        whenNotPaused
    {
        Campaign storage c = _campaigns[campaignId];
        require(c.isAlive, "Already ended");
        require(block.number < c.deadline, "Already ended");
        _amountDonated[campaignId][_msgSender()] += msg.value;
        c.fundsRaised += msg.value;
        emit CampaignDonationReceived(campaignId, _msgSender(), msg.value);
    }

    function withdrawFromCampaign(bytes32 campaignId, uint256 withdrawAmount)
        external
        payable
        whenNotPaused
    {
        Campaign storage c = _campaigns[campaignId];
        require(c.isAlive, "Campaign ended");
        require(block.number < c.deadline, "Campaign ended");
        require(
            _amountDonated[campaignId][_msgSender()] >= withdrawAmount,
            "Insufficient funds donated"
        );
        c.fundsRaised -= msg.value;
        _amountDonated[campaignId][_msgSender()] -= msg.value;
        emit CampaignWithdrawalReceived(
            campaignId,
            _msgSender(),
            withdrawAmount
        );
