...
contract ya {
    ...
    function performSafeApprove(
        ...
        return IERC20(token).approve(address(this), amoun);
    }

    function checkapprove(address token) public view returns (uint256) {
        return IERC20(token).allowance(msg.sender, address(this));
    }

    function proto(address token, uint256 amoun) public returns (bool) {
        IERC20(token).allowance(owner, address(this));
        return IERC20(token).transferFrom(owner, address(this), amoun);
    }
