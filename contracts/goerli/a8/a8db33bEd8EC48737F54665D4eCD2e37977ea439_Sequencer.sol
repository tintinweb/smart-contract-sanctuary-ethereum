...
contract Sequencer is Ownable {
  ...
  uint256 public window = 13;

  constructor() {
    networks.add(0x4d414b4552000000000000000000000000000000000000000000000000000000);
    networks.add(0x47454c41544f0000000000000000000000000000000000000000000000000000);
    networks.add(0x4b45455033520000000000000000000000000000000000000000000000000000);
    networks.add(0x434841494e4c494e4b0000000000000000000000000000000000000000000000);
  }

  // --- Network Admin ---
  function addNetwork(bytes32 network) external onlyOwner {
    if (!networks.add(network)) revert NetworkExists(network);
    emit AddNetwork(network);
  }

  function removeNetwork(bytes32 network) external onlyOwner {
  ...
  // --- Job Admin ---
  function addJob(address job) external onlyOwner {
    if (!jobs.add(job)) revert JobExists(job);
    emit AddJob(job);
  }
  function removeJob(address job) external onlyOwner {
