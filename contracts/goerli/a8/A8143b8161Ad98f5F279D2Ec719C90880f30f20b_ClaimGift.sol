...
contract ClaimGift is Ownable, IERC1155Receiver, ReentrancyGuard, IERC721Receiver {
    using SafeMath for uint256;
    ///@dev Event for user gift claim request
    event GiftClaimRequestSent(uint256 requestId, address userWallet, uint256 claimAmount);
    ...
    // ///@dev Event after getting random number from Chain Link in callback method
    ...
    ///@dev Total number of claimed request
    uint256 public totalNumberOfRequest;
    ...
    modifier hasValidGiftBoxTokenBalance(uint32 _giftAmount) {
    ...
    ///@dev structure for saving data to use in the callback
    struct RequestStatus {
        bool fulfilled; // whether the request has been successfully fulfilled
        bool exists; // whether a requestId exists
        uint256[] randomNumbers; // random number array from chainlink
        address msgSender; // claimer of the gift token
    }
    ...
    ///@dev Request status map as storage for using in the callback
    ...
    ///@param _tokenId TokenId to rescue
    ///@param _collectionAddress Gift token collection address
    ...
    function rescueGiftToken(
        ...
        bool tokenRescued = false;

        for (uint256 index = 1; index <= totalGiftAmount; index = _uncheckedInc(index)) {
            if (giftTokens[index].collectionAddress == _collectionAddress && giftTokens[index].giftTokenId == _tokenId) {
                if (giftTokens[index].giftCollectionContractType == uint256(GiftContractType.ERC_1155)) {
                    ERC1155 giftContract = ERC1155(giftTokens[index].collectionAddress);

                    if (!giftTokens[index].isGivenAway && !giftTokens[index].isRescued) {
                        giftTokens[index].isRescued = true;
                        tokenRescued = true;
                        unchecked {
                            availableGiftAmount--;
                        }

                        giftContract.safeTransferFrom(address(this), _transferTo, _tokenId, 1, "");

                        break;
                    }
                } else {
                    ERC721 giftContract = ERC721(giftTokens[index].collectionAddress);

                    if (giftContract.ownerOf(_tokenId) == address(this)) {
                        giftTokens[index].isRescued = true;
                        tokenRescued = true;
                        unchecked {
                            availableGiftAmount--;
                        }
                        giftContract.safeTransferFrom(address(this), _transferTo, _tokenId);
                        break;
                    }
                }
            }
    ...
    ///@dev Internal method for transferring gift randomly
    ///@param _requestId claim request Id
    function transferGift(uint256 _requestId) internal {
        // uint256[] memory availableGiftIndex = new uint256[](totalGiftAmount);
        // uint256 availableTotalGiftAmount = 0;

        // for (uint256 index = 1; index <= totalGiftAmount; index = _uncheckedInc(index)) {
        //     if (!giftTokens[index].isGivenAway && !giftTokens[index].isRescued) {
        //         availableGiftIndex[availableTotalGiftAmount] = index;
        //         availableTotalGiftAmount++;
        //     }
        // }

        // uint256 selectedIndex = _randomNumber.mod(availableTotalGiftAmount);
        uint256 randomNumber = getRandomNumber();
        uint256 giftTokenIndex = totalGiftAmount - availableGiftAmount + 1; //availableGiftIndex[selectedIndex];//availableGiftIndex[selectedIndex];
        ...
        giftTokens[giftTokenIndex].receiverWallet = msg.sender;
    ...
    function claimGift(uint32 _claimGiftAmount) external nonReentrant noContracts hasValidGiftBoxTokenBalance(_claimGiftAmount) {
        if (!giftClaimActive) {
        ...
        emit GiftClaimRequestSent(totalNumberOfRequest, msg.sender, _claimGiftAmount);
        ...
        for (uint256 index = 0; index < _claimGiftAmount; index = _uncheckedInc(index)) {
            transferGift(totalNumberOfRequest);
        }
    }
