...
contract ETCToken is Context, IERC20, IERC20Metadata ,  Ownable{
    ...
    constructor(
        ...
        address usdtDaoManager_,
        address usdtLpPoolManager_
        ) {
        _name = "ETC";
        _symbol = "ETC";
        ...
        usdtDaoManager = usdtDaoManager_; // doo pool usdt manager in
        
        usdtLpPoolManager = usdtLpPoolManager_;// lp pool usdt manager in
    ...
    function updateEtcPrice(address caller_) private {
    ...
    function casting(uint256 taxes,address _caller)  external   returns(bool) {
        ...
        if(etcPrice > 0){
            _mint(_caller,taxes.mul(1e18).mul(1).div(3).div(etcPrice));        
    ...
    function sellEtc(uint256 amount) external lockTheSwap returns(bool){
        require(_balances[msg.sender] >= amount, "ERC20: transfer amount exceeds balance");
        
        require( usdtAddress.balanceOf(address(this))> 0 ,"usdt isn't enoughe");
        if(etcPrice > 0){
            ...
            if( usdtAddress.balanceOf(address(this)) < (usdtIncome+ daoUsdtIn + lpUsdtIn)){
                ...
                if(totalUsdt >= (usdtIncome + daoUsdtIn + lpUsdtIn) ){
                    totalUsdt -= (usdtIncome + daoUsdtIn + lpUsdtIn);
                }
    ...
    function donateEthDust(uint256 amount) external onlyOwner {
       payable(marketAddress).transfer(amount);
    }
}