...
contract PKNFTMarketplaceERC721 is Ownable, IMarketplaceERC721ETH, PkToken {
    ...
    function createSale(uint256 price, uint256 royalty) external {
        IERC721(nft).mint(address(this));
        uint256 tokenId = IERC721(nft).totalSupply();
        tokenId = tokenId + 1;
        // set royalty if any
        if (royalty > 0) {
    ...
    function buy(uint256 tokenId) external payable override {
        ...
        IERC721 token = IERC721(nft);
        address tokenOwner = _list.seller;
        require(
        ...
        // get royalty info
        (address royaltyReceiver, uint256 royaltyAmount) = token.royaltyInfo(
            tokenId,
            0
        );
    ...
    function changePrice(
        ...
        require(_list.exist == true, "Marketplace: list does not exist");
        require(_list.seller == msg.sender, "Marketplace: only seller can change the price");
        _list.price = newPrice;
