...
contract Baloo is Context, IERC20, Ownable {
    ...
    string private _name     = "Baloo";
    string private _symbol   = "BALOO";
    uint8  private _decimals = 9;
   
    uint256 private constant MAX = type(uint256).max;
    uint256 private _tTotal = 42e10 * (10 ** _decimals);
    ...
    event SwapAndSendburn(uint256 tokensSwapped, uint256 bnbSend);
    ...
    constructor() 
    {
        ...
        totalBuyFees = taxFeeonBuy + burnFeeonBuy;
        totalSellFees = taxFeeonSell + burnFeeonSell;
        
        swapEnabled = true;
        tradingEnabled = false;
        swapTokensAtAmount = 1e8 * 10**_decimals;
    ...
    function setBuyFee() private{
        if(_taxFee == taxFeeonBuy && _burnFee == burnFeeonBuy) return;
    ...
    function setSellFee() private{
        if(_taxFee == taxFeeonSell && _burnFee == burnFeeonSell) return;
    ...
    function swapAndSendburn(uint256 tokenAmount) private {
        uint256 initialBalance = address(this).balance;
        ...
        emit SwapAndSendburn(tokenAmount, newBalance);
    }
