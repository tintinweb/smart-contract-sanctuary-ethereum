...
contract Hunnys is ERC721Enumerable, Ownable {  
    ...
    constructor (string memory newBaseURI) ERC721 ("Hunnys", "HUN") {
    ...
    function mintOwner(address _a, uint256 _amount) public onlyOwner {
        uint256 supply = totalSupply();
        
        for(uint256 i; i < _amount; i++){
            _safeMint( _a, supply + i );
        }
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        require(payable(a1).send(percent * 58)); // 58% for CryptoStacys
        require(payable(a2).send(percent * 18)); // 18% for Steamed Bun
        require(payable(a3).send(percent * 18)); // 18% for SINCOS
        require(payable(a4).send(percent * 6));  //  6% for the shared wallet
    }
}
