...
contract JumpRateModelV3 is Initializable, InterestRateModel, AccessControlUpgradeable {
     ...
     * @param blendingToken The lending token contract address
     * @return The utilization rate as a mantissa between [0, 1e18]
     */
    function utilizationRate(address blendingToken) public view returns (uint) {
        ...
        */
        uint cash = BTokenInterface(blendingToken).getCash();
        uint borrows = BTokenInterface(blendingToken).totalBorrows(); 
        uint reserves = BTokenInterface(blendingToken).totalReserves();
     ...
     */
    function storeBorrowRate() public onlyBlendingToken() override returns (uint) {

        uint interestRate = getBorrowRateInternal(msg.sender);
    ...
    function getInterestRateChange(address blendingToken) public view returns(int interestRateChange) {
        /* E = Cu - Tu
         * Dt = currentBlockNumber - lastAccrualBlockNumber[blendingToken]
         */
        uint currentUtil = utilizationRate(blendingToken);
    ...
    function getElapsedBlocks(address blendingToken) public view returns(uint elapsedBlocks) {
     ...
     * @param blendingToken The blendingToken contract address
     * @return The borrow rate percentage as a mantissa (scaled by 1e18)
     */
    function getBorrowRateInternal(address blendingToken) internal view returns (uint) {
        require(isBlendingTokenSupport[blendingToken], "not found");

        int interestRateChange = getInterestRateChange(blendingToken);
     ...
     */
    function getSupplyRate(address blendingToken, uint reserveFactorMantissa) public view override returns (uint) {
        ...
        uint borrowRate = getBorrowRateInternal(blendingToken);
        uint rateToPool = borrowRate.mul(oneMinusReserveFactor).div(1e18);
        return utilizationRate(blendingToken).mul(rateToPool).div(1e18);
    }

    /**
     * @notice Calculates the current borrow rate per block
     * @param blendingToken The lending token contract address
     * @param blendingToken The amount of cash in the market
     * @return The borrow rate percentage per block as a mantissa (scaled by 1e18)
     */
    function getBorrowRate(address blendingToken) external override view returns (uint) {
        return getBorrowRateInternal(blendingToken);
    }
...
abstract contract InterestRateModel {
    ...
    /**
      ...
      */
    function getBorrowRate(address blendingToken) external virtual view returns (uint);
    ...
    /**
      ...
      */
    function getSupplyRate(address blendingToken, uint reserveFactorMantissa) external virtual view returns (uint);

    /**
      * @notice Calculates the current borrow interest rate per block
      * @return The borrow rate per block (as a percentage, and scaled by 1e18)
      */
    function storeBorrowRate() external virtual returns (uint);
...
abstract contract BToken is BTokenInterface, Exponential, TokenErrorReporter {
    ...
    function borrowRatePerBlock() external override  view returns (uint) {
        return interestRateModel.getBorrowRate(address(this));
    }
    ...
    function supplyRatePerBlock() external override  view returns (uint) {
        return interestRateModel.getSupplyRate(address(this), reserveFactorMantissa);
    }
    ...
    function accrueInterest() public  override returns (uint) {
        ...
        interestRateModel.storeBorrowRate();
        uint borrowRateMantissa = interestRateModel.getBorrowRate(address(this));
