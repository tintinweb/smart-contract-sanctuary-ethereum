...
contract HungryHamsterClub is ERC721A, ERC2981, Ownable, AccessControl, Pausable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    function privateMint(uint64 amount, uint64 maxAmount, bytes memory signature) external payable canMint(amount) whenNotPaused nonReentrant {
        require(phase == Phases.PRIVATE, 'mint is not open');

        uint64 aux = _getAux(msg.sender);
        if(Signature.verify(maxAmount, msg.sender, signature) != signer[Phases.PRIVATE]) revert InvalidSignature();
