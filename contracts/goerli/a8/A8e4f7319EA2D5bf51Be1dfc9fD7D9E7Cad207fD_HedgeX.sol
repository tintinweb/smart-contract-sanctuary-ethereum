...
contract HedgeX {
    ...
    //0.5 ETH
    uint256 initialLiqThreshold = 10000000;
    ...
    mapping(uint => bool) public makerOrderCanceled;
    ...
    event NewMakerOrder(address _maker, uint _maker_order_ID, uint256 _amt);
    event Taken(address _taker, uint _maker_order_ID, uint256 lockUpEnds);
    event Settled(address _settler, uint _taker_order_ID, uint _maker_order_ID, uint256 _settler_fee, uint256 _maker_fee, uint256 _timestamp);
    ...
    function takeOrder(uint makerOrderID, bytes32 feedID, bytes[] calldata priceUpdateData) payable public{
        ...
        require(makerOrderCanceled[makerOrderID] == false, "Maker-order has been canceled on chain.");
    ...
    function settleOrder(uint takerOrder_ID,bytes[] calldata priceUpdateData) public  {
        ...
        if(initialLiquidityProvided == true){
            if(hdgx.balanceOf(pool_address) > (initialLiqThreshold * 250)){
                uint256 amountOut = swapExactInputSingle(((maker.ethPosted + taker.ethPosted) * 1 / 100) * 50 / 100);
            }
            else{
                if(initialLiquidityPulled == false){
                    halveLiquidity();
                }
                // //If Remaining liqudiity > 1 wei
                // if(hdgx.balanceOf(pool_address) > 1000){
                //     //If amount of buyback is greater than 10% of liqudity left
                //     if ((((maker.ethPosted + taker.ethPosted) * 1 / 100) * 50 / 100) > (hdgx.balanceOf(pool_address)) * 10 / 100){
                //         //Swap for 10% of tokens in pool
                //         uint256 amountOut = swapExactInputSingle((hdgx.balanceOf(pool_address)) * 10 / 100);
                //     }
                // }
            }
        }
        emit Settled(msg.sender, takerOrder_ID, taker.makerOrder_ID, (maker.ethPosted + taker.ethPosted) * 5 / 1000, (maker.ethPosted + taker.ethPosted) * 1 / 100, block.timestamp);
    }
    ...
    function halveLiquidity() internal{
       (uint256 amount0, uint256 amount1) = hedgexLiqManager.decreaseLiquidityCurrentRange();
    ...
    //Todo
    function cancelMakerOrder(uint _makerOrderID) public{
