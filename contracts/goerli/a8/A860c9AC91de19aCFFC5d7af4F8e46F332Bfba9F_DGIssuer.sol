...
contract DGIssuer is Initializable{
    ...
    mapping(DeedGrain => address) public deedGrainAddrToClientAddr;
    ...
    function issueDG(string memory _name, string memory _symbol, string memory _baseUri, bytes memory _evidence, bool _transferable) public {
        require(_validate(keccak256(abi.encodePacked(msg.sender)), _evidence), "invalid evidence");
    ...
    /// @param _signer Did singer address
    ...
    function setTokenSupply(address DGAddr, uint tokenId, uint supply) public {
        require(msg.sender == owner || msg.sender == deedGrainAddrToClientAddr[DeedGrain(DGAddr)], "caller are not allowed to set supply");
    ...
    function mintDG(address DGAddr, uint tokenId, address[] memory addrs) public {
        require(msg.sender == dgMinter || msg.sender == deedGrainAddrToClientAddr[DeedGrain(DGAddr)], "caller are not allowed to mint");
        DeedGrain DG = DeedGrain(DGAddr);
        for(uint i=0; i<addrs.length; i++){
            DG.mint(addrs[i], tokenId, 1);
        }
    }
    ...
    function claimDG(address DGAddr, uint tokenId, bytes memory evidence) public {
        require(_validate(keccak256(abi.encodePacked(msg.sender, DGAddr, tokenId)), evidence), "invalid evidence");
        DeedGrain DG = DeedGrain(DGAddr);
...
contract DeedGrain is ERC1155Supply {
    ...
    function mint(address to, uint256 tokenId, uint256 amount) public onlyControllers {
        if (_supplies[tokenId] != 0) {
            require(totalSupply(tokenId) + amount <= _supplies[tokenId], "insufficient supply");
        }
