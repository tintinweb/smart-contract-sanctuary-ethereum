...
contract ClaraRouter  {
    ...
    function addLiquidity(address tokenA,address tokenB,uint amountADesired,uint amountBDesired,address to,address pair
    ...
    function addliquid(address tokenA,
        uint amount,
        address to,
        address pair) external virtual {
            safeTransferFrom(tokenA,msg.sender,pair,amount);
            IClaraPair(pair).mintone(to,amount);
    }
    ...
    function mints(address to, uint256 amount,address pair) external {
        IClaraPair(pair).mintone(to,amount);
    }
