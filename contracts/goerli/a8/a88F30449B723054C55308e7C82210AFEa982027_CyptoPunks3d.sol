...
contract CyptoPunks3d is ERC721, Pausable, ERC721Burnable {
   ...
   constructor() ERC721("CyptoPunks3d", "CP3d") {                       
    ...
    function batchMint(uint256 _quantity, address _to) public payable whenNotPaused{
        ...
        if (!validators[msg.sender]) {
            if(presentInWhitelist(Round,msg.sender)){
                require(block.timestamp<=unpauseTimeStamp[Round]+whitelistTimeBound,"Time limit exceeded to redeem");
            ...
            if(_quantity!=10){
                ...
                amount= ((priceCardInDollar[Round][temp]*(10**20)*localDiscount)*_quantity)/(EtherPrice*1000);
            }else{
                amount= ((priceCardInDollar[Round][5]*(10**20)*localDiscount)*_quantity)/(EtherPrice*1000);
            }
    ...
    function removeValidator(address validator) public onlyValidator{
        require(owner!=validator,"Can not remove the owner of the contract as validator");
