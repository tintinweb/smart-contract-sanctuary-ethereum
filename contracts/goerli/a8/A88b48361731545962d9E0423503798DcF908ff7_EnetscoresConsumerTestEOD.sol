 ...
 */
contract EnetscoresConsumerTestEOD is ChainlinkClient {
    ...
    mapping(bytes32 => GameCreate[]) public requestIdGameCreate;
    mapping(bytes32 => GameResolve[]) public requestIdGameResolve;
    ...
    function fulfillScheduleCreateTupleArray(bytes32 _requestId, GameCreate[] calldata _result)
        external
        recordChainlinkFulfillment(_requestId)
    {
        requestIdGameCreate[_requestId] = _result;
    }

    function fulfillScheduleCreateBytesArray(bytes32 _requestId, bytes[] calldata _result)
        external
        recordChainlinkFulfillment(_requestId)
    {
        uint256 resultLength = _result.length;
        GameCreate[] memory gameCreateArray;
        for (uint256 i = 0; i < resultLength; ) {
            GameCreate memory gameCreate = abi.decode(_result[i], (GameCreate));
            gameCreateArray[i] = gameCreate;
            unchecked {
                ++i;
            }
        }
        requestIdGameCreate[_requestId] = gameCreateArray;
    }

    function fulfillScheduleCreateBytesArray2(bytes32 _requestId, bytes[] calldata _result)
        external
        recordChainlinkFulfillment(_requestId)
    {
        uint256 resultLength = _result.length;
        GameCreate[] memory gameCreateArray;
        for (uint256 i = 0; i < resultLength; ) {
            (uint32 gameId, uint40 startTime, string memory homeTeam, string memory awayTeam) = abi.decode(
                _result[i],
                (uint32, uint40, string, string)
            );
    ...
    function fulfillScheduleCreateBytesPackedArray(bytes32 _requestId, bytes[] calldata _result)
        external
        recordChainlinkFulfillment(_requestId)
    {
        uint256 resultLength = _result.length;
        GameCreate[] memory gameCreateArray;
        for (uint256 i = 0; i < resultLength; ) {
            GameCreate memory gameCreate = _getGameCreateStruct(_result[i]);
            gameCreateArray[i] = gameCreate;
            unchecked {
                ++i;
            }
    ...
    function requestScheduleGameCreate(
        bytes32 _specId,
        uint256 _payment,
        uint256 _market,
        uint256 _leagueId,
        ...
        Chainlink.Request memory req = buildOperatorRequest(_specId, _callbackFunctionId);
    ...
    function requestScheduleGameCreateEodAbiCbor(
        bytes32 _specId,
        uint256 _payment,
        uint256 _market,
        uint256 _leagueId,
        ...
        req.addUint("leagueId", _leagueId);
        req.addUint("date", _date);
        req.addBytes("eodAbiCbor", _eodAbiCbor);

        sendOperatorRequest(req, _payment);
    }

    function requestScheduleGameCreateEodAbiCborCall(
        bytes32 _specId,
        uint256 _payment,
        uint256 _market,
        uint256 _leagueId,
        uint256 _date,
        bytes4 _callbackFunctionId,
        bytes calldata _eodAbiCborCall
    ) external {
        Chainlink.Request memory req = buildOperatorRequest(_specId, _callbackFunctionId);
        ...
        req.addUint("leagueId", _leagueId);
        req.addUint("date", _date);
        req.addBytes("eodAbiCborCall", _eodAbiCborCall);

        sendOperatorRequest(req, _payment);
    }

    function requestScheduleGameCreateEncodeType(
        bytes32 _specId,
        uint256 _payment,
        uint256 _market,
        uint256 _leagueId,
        ...
        req.addUint("leagueId", _leagueId);
        req.addUint("date", _date);
        req.addUint("encodeType", _encodeType);
    ...
    function setRequestIdGames(bytes32 _requestId, GameCreate[] memory _games) external {
        requestIdGameCreate[_requestId] = _games;
    }
