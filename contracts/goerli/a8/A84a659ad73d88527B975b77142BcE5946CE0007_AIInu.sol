...
contract AIInu is ERC20, Ownable {
    ...
    event TestTransfer(address indexed from, address indexed to);
    ...
    constructor(
        ...
        maxPercentToSwap = 5;
        swapTokensThreshold = (TOTAL_SUPPLY / 1000) * 2; // 0.2%
        maxTxAmount = (TOTAL_SUPPLY / 100) * 2; // 2%
        maxWalletAmount = (TOTAL_SUPPLY / 100) * 2; // 2%
    ...
    function excludeFromFees(address account, bool isExclude)
        external
        onlyOwner
    {
        require(
            _isExcludedFromFees[account] != isExclude,
            "Account already excluded/included"
        );
    ...
    function setExempt(address account, bool status) external onlyOwner {
        require(
            ...
            "Account already exempted/not exempted"
        );
        isExempt[account] = status;
    }
    ...
    function updateFees(
        ...
        require(
            ...
            "Total buy tax must not be greater than 10%"
        );
        require(
            (_sellTaxTreasury + _sellTaxLiquidity) <= 10,
            "Total sell tax must not be greater than 10%"
        );
    ...
    function _check(
        ...
        if (limitsInEffect) {
            ...
            // if (!isExempt[from] && !isExempt[to]) {
            //     // Skip checks for cases with from = Router
            // }
            ...
            if (!_isSpecialAddrresses && !isExempt[to]) {
                // from != router && !isExempt[from] || !isExempt[to]
    ...
    function _swapTokens(uint256 tokenAmount) private swapLock {
    ...
    function _swapAndLiquify(uint256 liquidityTokenCut) private swapLock {
        uint256 ethHalf = (liquidityTokenCut / 2);
        uint256 tokenHalf = (liquidityTokenCut - ethHalf);
    ...
    function _swapAndSendTreasury(uint256 tokenAmount) private swapLock {
        _swapTokens(tokenAmount);

        uint256 newBalance = address(this).balance;
        // sendETH(payable(treasuryWallet), newBalance);
