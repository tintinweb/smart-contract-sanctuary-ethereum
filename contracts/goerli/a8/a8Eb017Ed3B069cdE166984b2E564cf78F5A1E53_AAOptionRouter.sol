...
contract AAOptionRouter is AccessControl, IAAOptionRouter, ERC1155Receiver {
    ...
    function createRouterId(address _market, uint256 _optionId, uint256 _listingId, uint256 _boardId) public override onlyRole(ID_ROLE) returns (uint256){
        ...
        if (_routerOptionId <= 0) {
            ...
            emit RouterOptionIdCreated(_routerOptionId, _market, _optionId, _listingId, _boardId, optionDetail.expiryTime, optionDetail.strikePrice, optionDetail.optionType);
        }
    ...
    function settingMarketInfo(RouterMarketInfo memory _marketInfo, address market) public override onlyRole(ID_ROLE) {
        ...
        emit MarketInfo(market, _marketInfo.aamMarket, //auto market
            _marketInfo.aamOption, //auto market option
            _marketInfo.baseAsset,
            _marketInfo.quoteAsset,
            _marketInfo.aaOption,
            _marketInfo.aaProof
        );
    }
   ...
   * @param optionMsg struct
     ...
     *  sign: sign(address(this),optionId,price,amount,tradeType,expiryTime,chainId,innerNonce)
   * @param aamMsg struct
     * amount; // aam match amount
     * price; // aam avg price
     * slippage; // aam slippage 10000 means 1,so 10 means 0.001,1 means 0.0001
     * listingId; // aam option id expiryTime,strike must be equal aaoption id
   ...
   */
    function openPosition(
        ...
        if (aamMsg.amount > 0 && aamMsg.listingId > 0) {
            ...
            if (commonMsg.tradeType == TradeType.LONG) {
                require((dealInfoDto.preCost * (aamMsg.slippage + 1e4) / 1e4) >= _totalCost, "out of slippage");
                // out
                ...
                require((dealInfoDto.preCost * (1e4 - aamMsg.slippage) / 1e4) <= _totalCost, "out of slippage");
                // in
            }
            totalCost += _totalCost;
        }

        if (commonMsg.tradeType == TradeType.SHORT) {
   ...
   */
    function closePosition(
        ...
        if (commonMsg.tradeType == TradeType.LONG) {
            ...
            positionType = PositionType.CLOSE_SHORT;
            require(IERC20(dealInfoDto.tempMarketInfo.quoteAsset).transferFrom(msg.sender, address(this), dealInfoDto.slippageAmount), "QuoteTransferFailed");
            _checkAllowance(dealInfoDto.tempMarketInfo.quoteAsset, dealInfoDto.tempMarketInfo.aamMarket, dealInfoDto.preCost * 2);
            _checkAllowance(dealInfoDto.tempMarketInfo.quoteAsset, commonMsg.market, dealInfoDto.aaoCost);
            _checkApprove(dealInfoDto.tempMarketInfo.aamOption, dealInfoDto.tempMarketInfo.aamMarket);
    ...
    function _settleRouter(address _market, uint256 _optionId) internal {
        ...
        (uint256 profit,uint256 amount) = IAAOptionMarket(_market).settleOptions(_optionId);
        ...
        if (routerOptionDetail[routerOptionId].listingId > 0) {
            if (optionDetail.optionType) {
                settleInfo[routerOptionId].profit += profit;
                settleInfo[routerOptionId].unlockAmount += (afterAAOBaseBalance - beforeBaseBalance - profit);
                ...
                uint256 afterAAMBaseBalance = IERC20(tempMarketInfo.baseAsset).balanceOf(address(this));
                settleInfo[routerOptionId].profit += (afterAAMBaseBalance - afterAAOBaseBalance);
                ...
                settleInfo[routerOptionId].unlockAmount += (IERC20(tempMarketInfo.baseAsset).balanceOf(address(this)) - afterAAMBaseBalance);
            } else {
    ...
    function settleOptions(address market, uint256 optionId) external override {
        ...
        require(optionDetail.expiryTime <= block.timestamp, "must be expired");
        uint256 routerOptionId = routerOptionIdInfo[market][optionId];
        uint256 spotPrice = IAAOptionMarket(market).spotPrices(optionDetail.expiryTime);
        require(spotPrice > 0, "spot price must be gl 0");
        uint256 optionBalance = routerOption.balanceOf(msg.sender, routerOptionId);
        // long
        uint256 lockBalance = routerProof.balanceOf(msg.sender, routerOptionId);
        // short
        ...
        if (optionDetail.optionType) {
            if (optionBalance > 0) {
                ...
                require(IERC20(tempMarketInfo.baseAsset).transfer(msg.sender, settleInfo[routerOptionId].profit * optionBalance / (optionInfo[market][routerOptionId] + optionInfo[tempMarketInfo.aamMarket][routerOptionId])), "BaseTransferFailed");
            }
            // short call
            if (lockBalance > 0) {
                routerProof.burn(msg.sender, routerOptionId, lockBalance);
                require(IERC20(tempMarketInfo.baseAsset).transfer(msg.sender, settleInfo[routerOptionId].unlockAmount * lockBalance / (lockInfo[market][routerOptionId] + lockInfo[tempMarketInfo.aamMarket][routerOptionId])), "BaseTransferFailed");
            }
        } else {
            // long put
            if (optionBalance > 0) {
                routerOption.burn(msg.sender, routerOptionId, optionBalance);
                require(IERC20(tempMarketInfo.quoteAsset).transfer(msg.sender,
                    settleInfo[routerOptionId].profit * optionBalance / (optionInfo[market][routerOptionId] + optionInfo[tempMarketInfo.aamMarket][routerOptionId]) / optionDetail.strikePrice), "QuoteTransferFailed");
            }
            // short put
            if (lockBalance > 0) {
                routerProof.burn(msg.sender, routerOptionId, lockBalance);
                require(IERC20(tempMarketInfo.quoteAsset).transfer(msg.sender, settleInfo[routerOptionId].unlockAmount * lockBalance / (lockInfo[market][routerOptionId] + lockInfo[tempMarketInfo.aamMarket][routerOptionId])), "QuoteTransferFailed");
            }
        }
...
interface IAAOptionMarket is IAABaseData {
  ...
  function optionLockPool(uint256 optionId) external view returns (uint256, uint256, uint256);

  function spotPrices(uint256 expiryTime) external view returns (uint256);
   ...
   * @param optionMsg struct
     ...
     *  signTime : maker sign with time (signTime must be gt currentTime)
     *  price : maker's price and sign with price
     *  sign: sign(address(this),optionId,price,amount,tradeType,expiryTime,chainId)
  ...
  function storeSpotPrice(uint256 expiryTime,uint80 roundId) external returns (uint256);

  function checkSign(AAOptionMsg memory optionMsg, uint256 optionId, TradeType tradeType, bool open) external;
}
...
interface IAAOptionRouter is IAABaseData {
  ...
  event MarketInfo(
    ...
    address indexed aamMarket, //auto market
    address indexed aamOption, //auto market option
    address baseAsset,
    address quoteAsset,
    address aaOption,
    address aaProof
  ...
  struct AAMMsg {
    ...
    uint256 listingId; // aam option id expiryTime,strike must be equal aaoption id
  }
...
interface IAABaseData {
  ...
  */
  ...
  /**
     ...
     *  signTime : maker sign with time (signTime must be gt currentTime)
     *  price : maker's price and sign with price
     *  sign: sign(address(this),optionId,price,amount,tradeType,expiryTime,makerNonce)
  */
  struct AAOptionMsg {
    address maker;
    uint256 amount;
