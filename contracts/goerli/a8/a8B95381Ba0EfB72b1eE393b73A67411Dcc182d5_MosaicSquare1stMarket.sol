...
****************************************************************************************************/
  function _checkRegisteredMarket(address, uint256) internal virtual {
    return;
  }
...
* @dev 주석 주석
****************************************************************************************************/
abstract contract MarketAuction is Constants, AdminTreasury, MarketCore, SupportsInterfaces, SendValueWithFallbackWithdraw, MarketFees {
...
****************************************************************************************************/
  function createAuction(address nftContract, uint256 tokenId, address seller, uint256 auctionPrice, uint256 startUnixTime, uint256 duration, uint16 marketFee) external onlyAdmin {
    ...
    require(address(this) == _getInterfacesOwnerOf(nftContract, tokenId), "MarketAuction: NFT not owned by this contract");
...
****************************************************************************************************/
  function placeBid(address nftContract, uint256 tokenId) external payable {
    ...
    require(auction.startTime <= block.timestamp, "MarketAuction: The auction did not begin");
...
****************************************************************************************************/
  function createFixedPrice(address nftContract, uint256 tokenId, address seller, uint256 price, uint256 startUnixTime, uint16 marketFee) external onlyAdmin {
    ...
    require(address(this) == _getInterfacesOwnerOf(nftContract, tokenId), "MarketFixedPrice: NFT not owned by this contract");
...
****************************************************************************************************/
  function _buy(address nftContract, uint256 tokenId) private {
    FixedPrice memory fixedPrice = nftContractToTokenIdToFixedPrice[nftContract][tokenId];
...
****************************************************************************************************/
  function setTreasury(address payable _treasury) external onlyAdmin {
    require(!_treasury.isContract(), "AdminTreasury: The treasury address should not be a contract");
    treasury = _treasury;
  }
...
****************************************************************************************************/
  function setAdmin(address _contractAdmin) external onlyAdmin {
    require(_contractAdmin.isContract(), "AdminTreasury: The contract admin address must be a contract");
    contractAdmin = _contractAdmin;
  }
...
abstract contract SendValueWithFallbackWithdraw is ReentrancyGuardUpgradeable {
  ...
  EnumerableMapUpgradeable.AddressToUintMap private pendingWithdrawals;
...
****************************************************************************************************/
  function _sendValueWithFallbackWithdraw(address nftContract, uint256 tokenId, address payable user, uint256 amount, uint256 gasLimit) internal nonReentrant {
    if (amount == 0) {
      return;
    }
...
****************************************************************************************************/
abstract contract SupportsInterfaces is Constants { // Adding this unused mixin to help with linearization 
