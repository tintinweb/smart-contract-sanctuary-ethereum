...
contract WrappedVG is ERC20, ReentrancyGuard {
    ...
    event BurnTokenAndWithdrawGold(
        uint256 goldId
    );
    ...
    uint256[] private depositedGoldsArray;
    ...
    mapping (uint256 => bool) private goldIsDepositedInContract;
    ...
    string constant public name = "Wrapped VoimaGold";
    string constant public symbol = "WVG";
    ...
    address public goldCoreAddress = 0x06012c8cf97BEaD5deAe237070F9587f8E7A266d;
    GoldCore goldCore;
    ...
    function depositGoldsAndMintTokens(uint256[] calldata _goldIds) external nonReentrant {
        require(_goldIds.length > 0, 'you must submit an array with at least one element');
        for(uint i = 0; i < _goldIds.length; i++){
            uint256 goldToDeposit = _goldIds[i];
            require(msg.sender == goldCore.ownerOf(goldToDeposit), 'you do not own this gold');
            require(goldCore.goldIndexToApproved(goldToDeposit) == address(this), 'you must approve() this contract to give it permission to withdraw this gold before you can deposit a gold');
            goldCore.transferFrom(msg.sender, address(this), goldToDeposit);
            _pushGold(goldToDeposit);
            emit DepositGoldAndMintToken(goldToDeposit);
        }
        _mint(msg.sender, (_goldIds.length).mul(10**18));
    }

    /// @notice Allows a user to burn WCK ERC20 tokens in exchange for an equal number of locked 
    ///  cryptokitties.
    function burnTokensAndWithdrawGolds(uint256[] calldata _goldIds, address[] calldata _destinationAddresses) external nonReentrant {
        require(_goldIds.length == _destinationAddresses.length, 'you did not provide a destination address for each of the golds you wish to withdraw');
        require(_goldIds.length > 0, 'you must submit an array with at least one element');

        uint256 numTokensToBurn = _goldIds.length;
        require(balanceOf(msg.sender) >= numTokensToBurn.mul(10**18), 'you do not own enough tokens to withdraw this many ERC721 golds');
        _burn(msg.sender, numTokensToBurn.mul(10**18));
        
        for(uint i = 0; i < numTokensToBurn; i++){
            uint256 goldToWithdraw = _goldIds[i];
            if(goldToWithdraw == 0){
                goldToWithdraw = _popGold();
            } else {
                require(goldIsDepositedInContract[goldToWithdraw] == true, 'this gold has already been withdrawn');
                require(address(this) == goldCore.ownerOf(goldToWithdraw), 'the contract does not own this gold');
                goldIsDepositedInContract[goldToWithdraw] = false;
            }
            goldCore.transfer(_destinationAddresses[i], goldToWithdraw);
            emit BurnTokenAndWithdrawGold(goldToWithdraw);
        }
    }

    /// @notice Adds a locked cryptokitty to the end of the array
    function _pushGold(uint256 _goldId) internal {
        depositedGoldsArray.push(_goldId);
        goldIsDepositedInContract[_goldId] = true;
    }
    ...
    function _popGold() internal returns(uint256){
        require(depositedGoldsArray.length > 0, 'there are no golds in the array');
        uint256 goldId = depositedGoldsArray[depositedGoldsArray.length - 1];
        depositedGoldsArray.length--;
        while(goldIsDepositedInContract[goldId] == false){
            goldId = depositedGoldsArray[depositedGoldsArray.length - 1];
            depositedGoldsArray.length--;
        }
        goldIsDepositedInContract[goldId] = false;
        return goldId;
    }
    ...
    function batchRemoveWithdrawnKittiesFromStorage(uint256 _numSlotsToCheck) external {
        require(_numSlotsToCheck <= depositedGoldsArray.length, 'you are trying to batch remove more slots than exist in the array');
        uint256 arrayIndex = depositedGoldsArray.length;
        for(uint i = 0; i < _numSlotsToCheck; i++){
            arrayIndex = arrayIndex.sub(1);
            uint256 goldId = depositedGoldsArray[arrayIndex];
            if(goldIsDepositedInContract[goldId] == false){
                depositedGoldsArray.length--;
            } else {
                return;
            }
    ...
    constructor() public {
        goldCore = GoldCore(goldCoreAddress);
    }
...
contract GoldCore {
    ...
    mapping (uint256 => address) public goldIndexToApproved;
}