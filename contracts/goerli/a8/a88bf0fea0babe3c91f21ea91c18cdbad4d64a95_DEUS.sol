...
//  Current version of DEUS Staking Token has an automated deflationary mechanism where 12% of 
//  each transaction is burned instantly. Mechanism enables DEUS token to have an innate value-
...
//  Function-1: On the upcoming version of the DEUS Staking Token burn rate will drop to 6% per 
//  transaction and another 6% of each realized transaction will be distributed among the DEUS stakers. 
//  This allows DEUS Staking Token to have a deflationary feature while rewarding the stakers.
...
//  DEUS Staking Token Smart Contract
//  Token Symbol      : DEUS
//  Token Name        : DEUS Staking
...
contract DEUS {
	...
	uint256 constant private Total_DEUS = 25e19; // 250.000.000 Initial Supply
	uint256 constant private Burn_Ratio = 0;   // 12% burn rate for each transaction
	uint256 constant private Burn_Threshold = 0;     
	uint256 constant private Freeze_Size = 1e20; 
	
	string constant public name = "DEUS Staking";
	string constant public symbol = "DEUS";
	...
	constructor() public {
		...
		info.totalSupply = Total_DEUS;
		info.users[msg.sender].balance = Total_DEUS;
		emit Transfer(address(0x0), msg.sender, Total_DEUS);
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if (totalSupply() - _burnedAmount < Total_DEUS * Burn_Threshold / 100 || isWhitelisted(_from)) {
			_burnedAmount = 0;
		}
