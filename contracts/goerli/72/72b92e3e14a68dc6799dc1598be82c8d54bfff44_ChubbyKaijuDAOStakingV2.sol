...
// File: IChubbyKaijuDAOStakingV2Old.sol


pragma solidity ^0.8.10;

interface IChubbyKaijuDAOStakingV2Old {
  struct TimeStake { uint256 tokenId; uint48 time; address owner; uint16 trait; uint16 reward_type;}
  function gen2StakeByToken(uint256) external view returns (TimeStake memory);
  function totalAlienStaked() external view returns (uint32);
  function totalZombieStaked() external view returns (uint32);
  function totalUndeadStaked() external view returns (uint32);
}
// File: Strings.sol
...
contract ChubbyKaijuDAOStakingV2 is IChubbyKaijuDAOStakingV2, OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable {
  ...
  struct rewardTypeStaked {
    uint16 common; // reward_type: 0 -> 1*1*(1+Time weight)
  ...
  function _initSpeciesRate() internal {
    speciesRate[0] = 125;
    speciesRate[1] = 150;
    speciesRate[2] = 200;
  }
  ...
  function migrateFromOldContract() external onlyOwner {
    uint256 totalStaked;
    IChubbyKaijuDAOStakingV2Old.TimeStake memory token;

    totalStaked = gen2StakeOld.totalAlienStaked() + gen2StakeOld.totalZombieStaked() + gen2StakeOld.totalUndeadStaked();
    for (uint256 i = 0; i < totalStaked; i++) {
      token = gen2StakeOld.gen2StakeByToken(i);
       _stakeGEN2(token.owner, token.tokenId, token.time);
    }
  }
  ...
  modifier _updateEarnings() {
    if (totalCrunchEarned < MAXIMUM_CRUNCH) {
      uint256 time = uint256(block.timestamp);
      uint256 temp = (1*(time-lastClaimTimestamp)*CRUNCH_EARNING_RATE*stakedCount.common);
