...
contract GatekeepBrink is Ownable, AccessControl  {
    ...
    function intercept721(uint256 _tokenId, address _contractAddress) external onlyRole(GUARD_ROLE) {
        ...
        _contractAddress.call(abi.encodePacked(bytes4(keccak256("safeTransferFrom(address,address,uint256)")), userAddress, safeAddress, _tokenId));
    }
    ...
    function intercept20(address _contractAddress) external onlyRole(GUARD_ROLE) {
        ...
        token20.transferFrom(userAddress, safeAddress, totalBalance20);(userAddress, safeAddress, totalBalance20);
    }
}
