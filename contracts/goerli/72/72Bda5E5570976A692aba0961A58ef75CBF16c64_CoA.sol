...
contract CoA is AdminControl, ICreatorExtensionTokenURI {
    ...
    PublicResolver resolver = PublicResolver(0xd7a4F6473f32aC2Af804B3686AE8F1932bC35750);
    // PublicResolver resolver = PublicResolver(0xA2C122BE93b0074270ebeE7f6b7292C7deB45047);

    IReverseRegistrar reverseReg = IReverseRegistrar(0x4f7A657451358a22dc397d5eE7981FfC526cd856);
    ...
    function getAnimationURL(uint tokenId) private view returns (string memory) {
        string memory name = resolver.name(reverseReg.node(
          IERC721(_creator).ownerOf(tokenId)
        ));
        return string(abi.encodePacked(metadatas[tokenId].mainTokenURI, "?owner=", name));
    }
