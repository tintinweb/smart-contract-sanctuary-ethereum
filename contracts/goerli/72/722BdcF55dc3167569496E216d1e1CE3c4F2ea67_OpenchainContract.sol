...
contract OpenchainContract {
    ...
    function swapToETH(
        ...
        require(sellTokenContract.transferFrom(msg.sender, address(this), _amountIn), 'transferFrom failed.');
        // // approve
        // require(sellTokenContract.approve(address(UNISWAP_ROUTER_ADDRESS), _amountIn), 'approve failed.');

        // address[] memory path = new address[](2);
        // path[0] = address(_tokenIn);
        // path[1] = uniswapRouter.WETH();
        // uniswapRouter.swapExactTokensForETH(_amountIn, _amountOutMin, path, _to, _deadline);
    }
}
