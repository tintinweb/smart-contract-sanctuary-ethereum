...
import "../util/NoDelegateCall.sol";
    ...
    NoDelegateCall
{
    ...
    address private immutable principalTokenBeaconUpgradable;
    address private immutable ytBeaconUpgradable;
    ...
    /**
    @notice The constructor of the PrincipalTokenFactory contract.
    Sets the value of principalToken and yt beacon.
     */
    constructor(
        address _principalTokenBeaconUpgradable,
        address _ytBeaconUpgradable
    ) Ownable() {
        require(
            _principalTokenBeaconUpgradable != address(0),
            "PrincipalTokenFactory: principalToken beacon address should not be zero"
        );
        require(
            _ytBeaconUpgradable != address(0),
            "PrincipalTokenFactory: yt beacon address should not be zero"
        );
        principalTokenBeaconUpgradable = _principalTokenBeaconUpgradable;
        ytBeaconUpgradable = _ytBeaconUpgradable;
    }
    ...
    {
        require(
            address(principalTokenBeaconUpgradable) != address(0),
            "PrincipalTokenFactory: PrincipalToken Beacon is not set"
        );

        principalToken = address(
            new BeaconProxy(
                principalTokenBeaconUpgradable,
    ...
    * @param curveAddressProvider_ The address of the curveAddressProvider.
    ...
    function setCurveAddressProvider(
        ...
        _checkOwner();
        require(
            curveAddressProvider_ != address(0),
            "PrincipalTokenFactory: Cannot set curve address provider to zero address"
        );
    ...
    function setRegistry(address _newRegistry) public {
        ...
        require(
            ...
            "PrincipalTokenFactory: Cannot set registry to zero address"
        );
    ...
    function _deployCurvePool(
        ...
        require(
            ...
            "PrincipalTokenFactory: curve factory address is not set"
        );
    ...
    * @notice Function which sets the curve factory address used in
    ...
    function _setCurveFactory() internal {
        ...
        require(
            success,
            "PrincipalTokenFactory: Could not fetch curve factory address from the address provider"
        );
...
interface IPrincipalToken is IERC20Upgradeable, IERC4626Upgradeable {
    ...
    /** @dev Deposits amount of assets into the vault and mints atleast minShares to user.
    ...
    /** @dev Deposits amount of ibt into the vault and mints at least minShares to users
     * @param ibtAmount the amount of ibt being deposited
     * @param receiver the receiver of the shares
     * @param minShares The minimum expected shares from this deposit
     * @return shares the amount of shares minted to the receiver
     ...
     * @return assets The actual amount of assets taken by vault for minting the shares.
     */
    function mint(
        uint256 shares,
        address receiver,
     ...
     * @return assets The actual amount of assets returned by vault for burning the shares.
     */
    function redeem(
        uint256 shares,
        address receiver,
     ...
     * @param receiver the receiver of the assets
     * @param owner the owner of the shares
     * @param maxShares The maximum shares that can be burnt by the vault
     * @return shares The actual amount of shares burnt by vault for returning the assets.
     */
    function withdraw(
        uint256 assets,
        address receiver,
    ...
    /** @dev Returns max fee that can be set for the vault */
...
interface IPrincipalTokenFactory {
    ...
    * @param curveAddressProvider The address of the curveAddressProvider.
     ...
     * @param _principalToken the address of the principalToken.
     * @param poolIndex the index at which pool is stored in array.
     * @return pool The struct of type PoolData of the corresponding pool.
     */
    function getPool(
        address _principalToken,
        uint256 poolIndex
...
interface IYT is IERC20Upgradeable {
     ...
     * @param vault The address of the principalToken associated with this yt token.
     */
    function initialize(
    ...
    IPrincipalToken
{
    ...
    uint256 private ibtUnit; // equal to one unit(10^decimals) of the token (IBT) held by the vault
    ...
    address private ibt; // interest bearing token held by the vault
    ...
    modifier notExpired() {
        require(block.timestamp <= expiry, "PrincipalToken: Vault expired");
        _;
    }
    ...
    modifier afterExpiry() {
        require(
            ...
            "PrincipalToken: Called before expiry"
        );
        _;
    }
     ...
     * @param _expiry the timestamp at which the vault will expire/mature
    ...
    function initialize(
        ...
        require(
            _ytBeacon != address(0),
            "PrincipalToken: yt beacon address should not be equal to zero"
        );
        require(
            _ibt != address(0),
            "PrincipalToken: Cannot set ibt to zero address"
        );
        ...
        require(
            _max_fees <= MAX_FEES,
            "PrincipalToken: Max fees should be less than 100%"
        );
        ...
        returns (uint256 assets)
    {
        require(
            owner == msg.sender,
            "PrincipalToken: caller should be the owner of shares"
        );
        require(
            shares <= maxRedeem(owner),
            "PrincipalToken: redeem more than max"
        );
        ...
        returns (uint256 shares)
    {
        ...
        require(
            ...
            "PrincipalToken: Input amount exceeds withdraw limit"
        );
        ...
        if (assetsInIBT > yieldInIBT - feesInIBT) {
            ...
            if (yieldInIBT - feesInIBT == 0) {
                revert(
                    "PrincipalToken: Net yield is 0 whereas supposed to cover the withdraw"
                );
            }
        ...
        returns (uint256 withdrawnAssets)
    {
        ...
        yt.burnWithoutUpdate(owner, yt.balanceOf(owner));
    }
    ...
    function claimFees() external returns (uint256) {
        require(
            ...
            "PrincipalToken: Caller should be FEE_COLLECTOR"
        );
    ...
    function claimYield() external override returns (uint256) {
        return claimYieldOfAmount(yt.balanceOf(msg.sender));
    }
    ...
    function claimYieldOfAmount(
        ...
        if (yieldToTransfer != 0 && (_amount != 0 || userYTBalance == 0)) {
            ...
            require(
                ...
                "PrincipalToken: yieldToTransfer exceeds user yield"
            );
    ...
    function beforeYtTransfer(address _from, address _to) external override {
        require(
            ...
            "PrincipalToken: Caller should be a YT"
        );
        updateYield(_from);
        updateYield(_to);
    }
    ...
    function setFeeCollector(address _feeCollector) public {
        ...
        require(
            ...
            "PrincipalToken: Cannot set fee collector to zero address"
        );
        ...
        returns (uint256 _ibtRate, uint256 _ptRate)
    {
        require(
            !ratesAtExpiryStored,
            "PrincipalToken: Rates at expiry already stored"
        );
    ...
    function previewWithdraw(
        ...
        uint256 _oldibtRate = ibtRateOfUser[msg.sender];
        uint256 _oldptRate = ptRateOfUser[msg.sender];
        uint256 _yieldOfUserInIBT;
    ...
    function maxWithdraw(
        ...
        (uint256 _ptRate, uint256 _ibtRate) = _getPTandIBTRates();
        uint256 _oldIBTRateUser = ibtRateOfUser[owner];
        uint256 _oldPTRateUser = ptRateOfUser[owner];
        uint256 _yieldOfUserInIBT;
        // Check for skipping the computation of yield when the user oldIBTRate is 0.
        if (_oldIBTRateUser != 0) {
            _yieldOfUserInIBT = _computeYield(
    ...
    function previewRedeem(
        ...
        if (ratesAtExpiryStored) {
            ...
            revert("PrincipalToken: Rates at expiry not stored");
        }
    }
    ...
    function getIBTRateAtExpiry() external view override returns (uint256) {
        require(
            ratesAtExpiryStored,
            "PrincipalToken: Rates at expiry not stored yet"
        );
        return ibtRate;
    }
    ...
    function _computeYield(
        ...
        if (_oldPTRate == _ptRate && _ibtRate == _oldIBTRate) {
            ...
            if (_oldPTRate > _ptRate) {
                ...
                revert(
                    "PrincipalToken: PT rate increased or no PT depeg but IBT rate decreased!"
                );
            }
        }
