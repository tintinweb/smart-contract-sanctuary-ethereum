 ...
 * This way we ensure that Contract's state variables come first in the storage layout and that Contract has the
 * ability to change the list of contracts it inherits from in the future via upgradeability.
 */
...
contract Contract is ContractStorage, UUPSUpgradeable, OwnableUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable {
  ...
  function addOrRefillTag(
    ...
    require (perUser <= totalSupply, 'failed: perUser > totalSupply');

    bytes32 tagHash = hashUniqueTag(msg.sender, uid);

    bool isNewTag = tags[tagHash].totalSupply == 0;

    // require(false, 'hello world 3');

    if (tagType == TagType.LimitedOrOpenEdition) {
      require(false, 'hello world A');
      // Verify that either this tag has never existed before or the supply has been completely depleted
      require (isNewTag || tags[tagHash].numClaimed >= tags[tagHash].totalSupply, 'existing tag undepleted');

      // TODO: Use custom Copyable type...

    } else if (tagType == TagType.SingleUse1Of1 || tagType == TagType.Refillable1Of1) {
      // require(false, 'hello world B');
      ...
      require (isNewTag || (tagType == TagType.Refillable1Of1 && tags[tagHash].numClaimed >= 1), 'existing tag either not a Refillable1Of1 or undepleted');
      // require(false, 'hello world B.1');

      // Tag storage tag = Tag(tagType, tokenAddress, erc721TokenId, tagAuthority, ((tagType == TagType.SingleUse1Of1) ? 1 : totalSupply), perUser, 0, uid);
      Tag storage tag = tags[tagHash];
      // require(false, 'hello world B.2');
      tag.tagType = tagType;
      ...
      tag.uid = uid;
      // require(false, 'hello world B.3');
      if (isNotErc1155) {
        // require(false, 'hello world B.31');
        IERC721Upgradeable token = IERC721Upgradeable(tokenAddress);
        // require(false, 'hello world B.311');
        require(
          false,
          string.concat(
            string.concat(
            ...
            Strings.toString(erc721TokenId)
          )
        );
        token.safeTransferFrom(msg.sender, address(this), erc721TokenId);
      } else {
        // require(false, 'hello world B.32');
      ...
      require(false, 'hello world B.4');
    } else if (tagType == TagType.WalletRestrictedFungible) {
      require(false, 'hello world C');
      // Verify that either this tag has never existed before or the supply has been completely depleted
      require (isNewTag || tags[tagHash].numClaimed >= tags[tagHash].totalSupply, 'existing fungible tag undepleted');
      require (fungiblePerClaim <= perUser, 'failed: fungiblePerClaim > perUser');

      // tags[tagHash] = Tag(tagType, tokenAddress, 0, tagAuthority, totalSupply, perUser, fungiblePerClaim, uid);
      Tag storage tag = tags[tagHash];
      tag.tagType = tagType;
      tag.tokenAddress = tokenAddress;
      tag.erc721TokenId = 0;
      ...
      if (isNotErc1155) {
        ...
        token.transferFrom(msg.sender, address(this), fungiblePerClaim);
      } else {
        IERC1155Upgradeable token = IERC1155Upgradeable(tokenAddress);
        token.safeTransferFrom(msg.sender, address(this), 0, fungiblePerClaim, "0x00");
      }
    } else if (tagType == TagType.HotPotato) {
      require(false, 'hello world D');
      ...
      require(false, 'hello world E');
      // TODO: Implement
    }
    require(false, 'hello world F');
    if (isNewTag) {
      numTags += 1;
    }
  ...
  function claimTag(
    ...
    require ((tags[tagHash].tagType == TagType.HotPotato) || tags[tagHash].numClaimed < tags[tagHash].totalSupply, 'not HotPotato and depleted');
    require ((tags[tagHash].tagType == TagType.HotPotato) || (tags[tagHash].tagType == TagType.SingleUse1Of1) || (tags[tagHash].claimsMade[receiver] < tags[tagHash].perUser), 'not (HotPotato or SingleUse1Of1) and depleted');
    ...
    if (tags[tagHash].tagType == TagType.LimitedOrOpenEdition) {
      ...
      if (isNotErc1155) {
        ...
        token.transferFrom(address(this), receiver, tags[tagHash].fungiblePerClaim);
      } else {
        IERC1155Upgradeable token = IERC1155Upgradeable(tags[tagHash].tokenAddress);
