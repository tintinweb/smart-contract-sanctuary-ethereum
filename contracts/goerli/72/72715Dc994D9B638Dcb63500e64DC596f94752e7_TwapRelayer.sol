...
contract TwapRelayer is ITwapRelayer {
    ...
    function calculateAmountIn(
        ...
        amountIn = amountOut.mul(decimalsConverter).div(price);
    }
    ...
    function getPoolState(address token0, address token1)
        ...
        returns (
            ...
            uint256 fee,
            uint256[] memory limits0,
            uint256[] memory limits1
        )
    {
        (address pair, bool inverted) = getPair(token0, token1);
        fee = swapFee[pair];

        (uint256 spotPrice, uint256 averagePrice, , ) = getPricesFromOracle(pair);
        if (inverted) {
        ...
        limits0 = getLimits(token0);
        limits1 = getLimits(token1);
    }

    function getLimits(address token) internal view returns (uint256[] memory limits) {
        limits = new uint256[](2);
        limits[0] = tokenLimitMin[token];
        limits[1] = tokenLimitMax[token];
    }
