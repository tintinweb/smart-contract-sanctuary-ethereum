...
contract SilkRenewal is ERC721, Ownable {
    ...
    function verifyAllowlisted(string memory phase,uint max,address account,bytes memory signature) internal view returns(bool) {
    ...
    function allowlistMint(bytes memory signature) public payable contractGuard {
        ...
        if(!verifyAllowlisted("ALLOWLIST", 1, msg.sender, signature)) revert NotAuthorized();
