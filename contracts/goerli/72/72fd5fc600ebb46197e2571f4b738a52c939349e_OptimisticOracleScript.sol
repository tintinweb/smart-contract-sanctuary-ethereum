...
contract OptimisticOracle is Guarded {
    ...
    /// @dev Settled proposal which have not been disputed set to `Null`
    enum ProposalState {
        // Null proposal
        Null,
        // Proposal is within the dispute window
        Proposed,
        // Proposal was disputed and settled
        Disputed
    }
    ...
    /// @notice Map of ProposalStates by ProposalId
    /// @dev ProposalId => ProposalState
    ...
    event Shift(bytes32 rateId, address proposer, uint256 value, bytes32 nonce);
    event DisputeValue(address disputer, bytes32 rateId, uint256 verifiedValue);
    ...
    function setRateConfig(
        ...
        // Set `ProposalState` of the initial proposal that will be referenced during the first shift
        proposals[computeProposalId(0, 0, 0)] = ProposalState.Proposed;
    }
    ...
    function shift(
        ...
        // Verify that the previous proposal is in the `Proposed` or `Disputed` state
        ProposalState prevProposalState = proposals[prevProposalId];
        if (prevProposalState == ProposalState.Null) {
        ...
        proposals[proposalId] = ProposalState.Proposed;

        // Clear the previous `ProposalState`
        delete proposals[prevProposalId];

        emit Shift(rateId, msg.sender, value, nonce);
    }
    ...
    function _settleDispute(
        ...
        if (proposals[disputedProposalId] != ProposalState.Proposed) {
            revert OptimisticOracle__settleDispute_invalidProposalState();
        }
        // Removed the disputed proposal
        delete proposals[disputedProposalId];

        // Add the verified value as a new proposal
        bytes32 validatedProposalId = computeProposalId(
            computeProposerKey(address(validator), rateId),
            verifiedValue,
            uint256(nonce)
        );
        proposals[validatedProposalId] = ProposalState.Disputed;

        // Transfer the bond to the receiver
        _claimBond(proposerKey, receiver);

        emit DisputeValue(msg.sender, rateId, verifiedValue);
    }
    ...
    function returnProposerKey(
        ...
        if (proposals[proposalId] != ProposalState.Proposed) {
