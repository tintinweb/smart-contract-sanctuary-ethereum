...
/// @title Bond
/// @notice represents first approach to calculate bond amount
contract Bond is BondBase {
    /// @notice multipliers divider (presicion)
    uint public constant DIVIDER = 1e18;

    /// @notice initializer of this contract
    /// @param _bondToken - address of bonding token
    /// @param _protocolConfig - protocol configuration address
    function initialize(address _bondToken, address _protocolConfig)
        external
        virtual
        initializer
    {
        __Bond__init(_bondToken, _protocolConfig);
    }

    /// @notice initializer of this contract
    /// @param _bondToken -
    /// @param _protocolConfig - protocol configuration address
    function __Bond__init(address _bondToken, address _protocolConfig)
        internal
        virtual
        onlyInitializing
    {
        __BondBase__init(_bondToken, _protocolConfig);
    }

    /// @dev TODO: include risk/term into calculation
    /// @inheritdoc IBond
    function takerBond(
        address token,
        uint amount,
        uint16,
        uint16
    ) public view override returns (uint bondAmount) {
        bondAmount = (amount * bondConfig[token].bumpPerAsset) / DIVIDER;
    }
    ...
    function makerBond(
        ...
        bondAmount = (amount * bondConfig[token].bumpPerStable) / DIVIDER;
    }
    ...
    function takerToSwap(address token, uint bondAmount)
        ...
        returns (uint amount)
    {
        amount = (bondAmount * bondConfig[token].assetPerBump) / DIVIDER;
    }
    ...
    function makerToSwap(address token, uint bondAmount)
        ...
        returns (uint amount)
    {
        amount = (bondAmount * bondConfig[token].stablePerBump) / DIVIDER;
    }
}
...
/// @title BondBase
/// @notice Abstract contract that implements storage of tokens
/// @dev Need to write calcAmountToLock function in your implementation
abstract contract BondBase is Initializable, BACUpgradable, IBond {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    /// @inheritdoc IBond
    address public override bondToken;

    /// @return address of the protocol's configuration
    address public protocolConfig;

    /// @dev record that represents deposited balance of particular user
    mapping(address => uint) internal depositedBalances;

    /// @dev record that represents locked balance of particular user
    mapping(address => uint) internal lockedBalances;

    /// @notice record that represents bond config of particular market
    mapping(address => BondConfig) public bondConfig;

    /// @notice requires msg.sender to be market contract
    /// @param token - address of market token
    modifier onlyMarket(address token) {
        require(
            IProtocolConfig(protocolConfig).getMarket(token) == msg.sender,
            "!market"
        );
        _;
    }
    ...
    /// @notice initializer of this contract
    /// @param _bondToken - bonding token
    /// @param _protocolConfig - protocol configuration address
    function __BondBase__init(address _bondToken, address _protocolConfig)
        internal
        onlyInitializing
    {
        require(_bondToken != address(0), "Bad token address");

        bondToken = _bondToken;
        protocolConfig = _protocolConfig;

        _setGlobalAccessController(IProtocolConfig(protocolConfig).getGAC());
    }

    /// @notice set multipliers for bond calculations
    function setBondConfig(
        ...
        bondConfig[token].bumpPerAsset = bumpPerAsset;
        bondConfig[token].bumpPerStable = bumpPerStable;
        bondConfig[token].assetPerBump = assetPerBump;
        bondConfig[token].stablePerBump = stablePerBump;
    }

    /// @inheritdoc	IBond
    function deposit(address depositTo, uint amount) public virtual override {
        require(amount > 0, "Bad amount");
        require(depositTo != address(0), "Bad depositTo");

        depositedBalances[depositTo] += amount;

        IERC20Upgradeable(bondToken).safeTransferFrom(
            _msgSender(),
            address(this),
            amount
        );
    ...
    function withdraw(uint amount) external virtual override {
        require(
            amount > 0 && depositedBalances[_msgSender()] >= amount,
            "Bad withdraw amount"
        );

        depositedBalances[_msgSender()] -= amount;

        IERC20Upgradeable(bondToken).safeTransfer(_msgSender(), amount);
    }
    ...
    function lockForTaker(
        ...
        uint16 risk,
        uint16 term
    ) external virtual override onlyMarket(token) returns (uint bondAmount) {
        require(depositedBalances[_owner] >= amount, "Not enough balance");
        bondAmount = takerBond(token, amount, risk, term);

        depositedBalances[_owner] -= bondAmount;
        lockedBalances[_owner] += bondAmount;
    }
    ...
    function lockForMaker(
        ...
        require(depositedBalances[_owner] >= amount, "Not enough balance");
        bondAmount = makerBond(token, amount, risk, term);
    ...
    function unlock(
        ...
        address token,
        uint bondAmount
    ) external virtual override onlyMarket(token) {
        require(lockedBalances[_owner] >= bondAmount, "Bad unlock amount");
    ...
    {
        (bondAmount, toTransfer, toReduce) = calcBondSizeForTakerPosition(
            recipient,
            token,
            amount,
            risk,
        ...
        _lockBondPosition(recipient, bondAmount, toTransfer);
    }
    ...
    {
        (bondAmount, toTransfer, toReduce) = calcBondSizeForMakerPosition(
            recipient,
            token,
            amount,
            risk,
    ...
    {
        toLock = takerBond(token, amount, risk, term);
        uint bondBalance = depositedBalances[recipient];
        if (bondBalance >= toLock) {
            toReduce = 0;
            toTransfer = 0;
        } else if (
            bondBalance + IERC20Upgradeable(bondToken).balanceOf(recipient) >=
            toLock
        ) {
            toTransfer = toLock - bondBalance;
        } else {
            toReduce = takerToSwap(token, toLock);
        }
    }
    ...
    {
        toLock = makerBond(token, amount, risk, term);
        uint bondBalance = depositedBalances[recipient];
        ...
        if (bondBalance >= toLock) {
            ...
            toReduce = makerToSwap(token, toLock);
        }
    }
    ...
    function balanceOf(address addr)
        ...
        returns (uint amount)
    {
        amount = depositedBalances[addr];
    }

    /// @inheritdoc	IBond
    function lockedOf(address addr)
        external
        view
        virtual
        override
    ...
    function lock(
        ...
        address addr,
        uint amount
    ) external override onlyMarket(token) {
        _lock(addr, amount);
    }
    ...
    function takerBond(
        ...
        uint16 risk,
        uint16 term
    ) public view virtual override returns (uint bondAmount);
    ...
    /// @dev internal implementation of {lock} method
    function _lock(address addr, uint amount) internal {
        depositedBalances[addr] -= amount;
        lockedBalances[addr] += amount;
    }

    function _lockBondPosition(
        address recipient,
        uint bondAmount,
        uint toTransfer
    ) internal {
        if (toTransfer > 0) {
            depositedBalances[recipient] = 0;
            lockedBalances[recipient] += bondAmount;
            IERC20Upgradeable(bondToken).safeTransferFrom(
                recipient,
                address(this),
                toTransfer
...
interface IMarket is IMarketStorage, IMakerPosition, ITakerPosition, IRebalanceable {
