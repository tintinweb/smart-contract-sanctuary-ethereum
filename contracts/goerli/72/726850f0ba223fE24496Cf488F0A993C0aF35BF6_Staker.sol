...
error Staker__NotEnoughEth();
error Staker__LessThanThreshold();
error Staker__NotPassedTheDeadline();
error Staker__NotOpenForWithdraw();
error Staker__NotAllowedToWithdraw();
error Staker__TransactionFailed();
...
contract Staker {
  ...
  event FundTransferedToExternalContract();
  ...
  function stake(address sender, uint256 value) private notCompleted() {
    if(value <= 0) {
      revert Staker__NotEnoughEth();
    }
  ...
  function execute() external notCompleted() {
    if(block.timestamp < deadline) {
      revert Staker__NotPassedTheDeadline();
    }
    ...
    emit FundTransferedToExternalContract();
  }
  ...
  function withdraw() external notCompleted() {
    ...
    if(balanceToTransfer <= 0) {
      revert Staker__NotAllowedToWithdraw();
    }

    delete balances[msg.sender];
    (bool sent, ) = payable(msg.sender).call{ value: balanceToTransfer }("");

    if(!sent) {
      revert Staker__TransactionFailed();
    }
  }
