...
// File: Scripts/Call function of other contract.sol

//SPDX-License-Identifier: UNLICENSE

//Импорт кода контракта


pragma solidity ^0.8.3;

//Создаю интерфейс I{название контракта NFT}
interface Islm{
        //Передаю тута все функции, которые хочу вызвать из другого контракта
        function mint(uint32 _mintCount) external payable;
        function transferFrom(address from,address to,uint256 tokenId) external;
}

interface Isec{
        //Передаю тута все функции, которые хочу вызвать из другого контракта
        function Callmint(uint32 _mintCount) external;
}

contract Main {

    function MintAllContract(uint8 _mintCount, address payable [] calldata _addr) external {
        for(uint i=0; i<_addr.length; i++){
            Isec(_addr[i]).Callmint(_mintCount);
        }
    }

    //Если контракт nft поддерживает минт other контрактом, то вернёт true, в противном случае - false(функция safemint(и другие) вызывает onERC721Received)
    function onERC721Received(address, address, uint256, bytes memory) public virtual returns (bytes4) {
        return this.onERC721Received.selector;
    }
    //Создаю функцию, которая задаёт адрес смарт контракта NFT
    //function SetAddresB(address _addr) external {
    //    addressB = _addr;
    //}

    //Создаю функцию, которая вызывает и возвращает другой контракт
    //function Callmint(uint32 _mintCount) external {
    //    Islm(addressB).mint(_mintCount);
    //}

    //function CallmintMULTI(uint32 _mintCount, address payable [] memory _addrs) external {
    //    for(uint i=0; i<_addrs.length; i++){
    //        Islm(addressB).mint(_mintCount);
    //    }
    //}

    //function Transfermint(address to,uint256 tokenId) external {
    //    Islm(addressB).transferFrom(address(this), to, tokenId);
    //}
