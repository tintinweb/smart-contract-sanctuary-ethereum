...
import { Weasley } from "../dAgoraWizard/Weasley.sol";
import '../IdAgoraMembership.sol';

contract dAgoraWeasleyFactory is Ownable, ReentrancyGuard {
    Weasley weasley;

    struct Deploys {
        address[] ContractAddress;
        address Owner;
        uint256 weasleyIndex;
    }

    address public dAgoraMembership;
    uint256 public weasleyCount = 0;
    bool public paused;

    mapping(address => Deploys) private _deployedContracts;
    mapping(address => uint256) public _addressDeployCount;

    event RonaldWeasleyCreated(
        address weasleyContract, 
        address owner,
        uint256 _weasleyIndex,
        uint256 _weasleyCount
    );
        ...
        nonReentrant
    {
        ...
        _deployedContracts[msg.sender].ContractAddress.push(address(weasley));
        _deployedContracts[msg.sender].Owner = msg.sender;
        _deployedContracts[msg.sender].weasleyIndex = weasleyCount;

        emit RonaldWeasleyCreated(
            address(weasley), 
            msg.sender, 
            _addressDeployCount[msg.sender],
            weasleyCount
...
contract Weasley is ERC721, ERC721Enumerable, Ownable {
...
interface IdAgoraMembership is IERC721A {
    function checkTokenTier(uint256 _tokenId) external view returns (bool);

    function isValidMembership(uint256 _tokenId) external view returns (bool);

    function membershipExpiresIn(uint256 _tokenId) external view returns (uint256);

    function membershipIndexToOwner(address _owner) external view returns (uint256);
