...
contract MockCollybusScript is Script {
    function run() external {
        vm.startBroadcast();

        MockCollybus collybus = new MockCollybus();
...
{
    /// ======== Errors ======== ///
    error ChainlinkValidator__value_feedNotFoundForToken(address tokenAddress_);
    error ChainlinkValidator__validate_feedNotFoundForToken(
        address tokenAddress_
    );

    /// ======== Events ======== ///
    event FeedSet(address tokenAddress_, address feedAddress_);
    event FeedUnset(address tokenAddress_);
    event Validate(address tokenAddress_, uint256 value_, uint256 scaledValue);
    ...
    /// @param prevTimestamp_ Previous proposal timestamp, will be used to check the dispute window
    /// @param timestamp_ Current proposal timestamp, will be used to check the propose window
    /// @return boolean value, returns if the shift can be done
    function canShift(bytes32 prevTimestamp_, bytes32 timestamp_)
        external
        view
        override(IValidator)
        returns (bool)
    {
        ...
        if (prevTimestamp_ > 0 && canDispute(prevTimestamp_)) {
            return false;
        }

        // Check whether the new proposal is not older than the propose window
        if (!canPropose(timestamp_)) {
            return false;
        }
    ...
    /// @param timestamp_ Proposal timestamp, will be used to check the propose window
    /// @return boolean value, returns if the propose can be made
    function canPropose(bytes32 timestamp_)
        public
        view
        override(IValidator)
        returns (bool)
    ...
    /// @param timestamp_ Proposal timestamp, will be used to check the dispute window
    /// @return boolean value, returns if the dispute can be made
    function canDispute(bytes32 timestamp_)
        public
        view
        override(IValidator)
        returns (bool)
    ...
    /// @notice Function that validates a value from a roundId against data retrieved from the corresponding chainlink feed
