...
contract CasinoWarTable{
    ...
    modifier onlyOwner(){
        require(msg.sender == owner, "You are not the owner of the cotract");
        _;
    }
    ...
    // Changing dealer
    //      - to let others run the game tables for demo
    function changeDealer(address _newDealer) public onlyOwner {
        dealer = payable(_newDealer);
    }
   ...
   receive() external payable{
    ...
    function pickWinner() public payable onlyDealer{
        ...
        // Transfer the remaining contract amount to the owner
        transferMoney(owner, getTableValue());
