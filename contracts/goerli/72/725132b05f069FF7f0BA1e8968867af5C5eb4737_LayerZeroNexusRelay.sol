...
import { NonblockingLzApp } from "../interfaces/LayerZero/NonblockingLzApp.sol";

contract LayerZeroNexusRelay is NonblockingLzApp, AbstractNexusRelay {
	...
	mapping(uint16 => uint256) public lzChainId_to_chain;
	...
	constructor(
		...
		for (uint256 i = 0; i < supported_chains.length; i++) {
			...
			lzChainId_to_chain[_chainData.lzChainId] = _chainId;
		}

		_setLzEndpoint(chain_data[chainId].lzEndpoint);
	}
	...
	function _transmit(
		...
		require(
			amount == 0,
			"This provider (LayerZero) does not support asset transfers"
		);

		sendTransferMessage(
			chain_data[destinationChainId].lzChainId,
			msg.value,
			message,
			_refundAddress
		);
	...
	function setExternalNexusRelayPartner(
		...
		setTrustedRemoteAddress(
			chain_data[chainId].lzChainId,
			abi.encodePacked(partner)
		);
		_setExternalNexusRelayPartner(chainId, partner);
	}
	...
	function _nonblockingLzReceive(
		...
		_forwardMessageToNexus(
			...
			address(0),
			srcAddress,
			lzChainId_to_chain[_srcChainId],
			_payload
		);
	}

	modifier onlyGateway(address) override {
		// No check needed here as LzApp has inbuilt checks in lzReceive()
		_;
	}
}