...
contract SeggzCoin is ERC20, Pausable, Ownable {
    ...
    function _transfer(
        ...
        if (!isCEX) {
            ...
            // they have some events ? to check if they fires, but events only work if it succeeded mmh which one exaclty  cause the error 


            // this is exactly as its deployed right now...  any idea what could be the problem if we uncomment the above?
        } else {
            super._transfer(sender, recipient, amount);
        }
