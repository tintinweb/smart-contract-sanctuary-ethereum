...
// File: contracts/interfaces/IFlipKit.sol
...
// File: contracts/interfaces/IBaseCollection.sol
...
contract FlipKit is OwnableUpgradeable, IFlipKit {
    ...
    function createCollection(
        ...
        require(_deployFee <= msg.value, "Value incorrect. Please send the correct amount.");

        //get the contract we are cloning
        address implementation_ = _implementations[collectionType];

        //deploy the new contract as a proxy
        ...
        //add the contract address to our collections array
        _collections.add(deployed);

        //send an event with the data
        emit CollectionCreated(deployed, affiliate, name, symbol, collectionType);
    }


    function setDeployFee(uint256 deployFee) public onlyOwner {
    ...
    function addImplementation(address implementation, string memory name) public onlyOwner {
        _implementations[name] = implementation;
    }

    function deleteImplementation(string memory name) public onlyOwner {
        delete _implementations[name];
    }
    ...
    function _createCollection(
        ...
        IBaseCollection newCollection = IBaseCollection(deployed);
        ...
        newCollection.transferOwnership(_msgSender());
