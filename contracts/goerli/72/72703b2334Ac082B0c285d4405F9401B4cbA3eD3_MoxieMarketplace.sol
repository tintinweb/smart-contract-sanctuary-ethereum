...
interface IMoxieAddressRegistry {
    function moxie() external view returns (address);
...
interface IMoxieBundleMarketplace {
...
interface IMoxiePriceFeed {
    function wETH() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract MoxieMarketplace is Ownable, ReentrancyGuard {
    ...
    function _buyItem(
        ...
        IMoxieBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IMoxieAuction auction = IMoxieAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isMoxieNFT(_nftAddress), "invalid nft address");
    ...
    function _isMoxieNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.moxie() == _nftAddress ||
            IMoxieNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IMoxieNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IMoxieNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IMoxieNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IMoxiePriceFeed priceFeed = IMoxiePriceFeed(
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IMoxieTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
