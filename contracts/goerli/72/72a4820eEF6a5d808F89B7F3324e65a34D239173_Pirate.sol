    ...
    UUPSUpgradeable
{
    ...
    struct WeightSet {
        uint112 beard; // New slot
        uint112 femaleBody;
        uint32 gender;
        uint176 femaleHat; // New slot
        uint80 wings;
        uint96 hairColor; // New slot
        uint128 maleBody;
        uint32 unique;
        uint112 maleHairs; // New slot
        uint144 special;
        uint256 maleClothing; // New slot
        uint256 maleHat; // New slot
        uint224 maleFace; // New slot
        uint144 femaleClothing; // New slot
        uint112 beardIfSkeleton;
        uint96 femaleFace; // New slot
        uint48 femaleHairs;
        uint48 maleHairsIfCrown;
        uint96 maleHairsIfMask; // New slot
        uint144 maleFaceIfSkeleton;
        uint224 maleFaceIfCrown; // New slot
        uint192 maleFaceIfSpecial; // New slot
    }
    ...
    modifier noContract() {
        ...
        if (sender.isContract() || sender != tx.origin) {
            revert ContractAccountNotAllowed();
        }

        _;

        _nonce = uint256(keccak256(abi.encodePacked(block.timestamp, sender, _nonce)));
    }
    ...
    function initialize(uint16 maxSupply, uint256 mintPrice) public initializer {
        ...
        // Beard enum
        _weightPool.beard = 0x0384_0898_0898_0898_044c_044c_012c;
        _weightPool.beardIfSkeleton = 0x0514_0a28_0000_0a28_05dc_05dc_01f4;
        // FemaleBody enum
        _weightPool.femaleBody = 0x07d0_07d0_0384_07d0_07d0_0384_00c8;
        // FemaleClothing enum
        ...
        // HairColor enum
        _weightPool.hairColor = 0x060e_0898_0898_0898_060e_012c;
        // MaleBody enum
        _weightPool.maleBody = 0x07d0_07d0_0352_07d0_07d0_0352_0096_0096;
        // MaleClothing enum
        _weightPool.maleClothing = 0x0578_0578_0578_0578_0578_0578_0118_0118_0118_0104_0064_0064_0064_0064_0032_0032;
        // MaleFace enum
        _weightPool.maleFace = 0x0fa0_0320_0320_0320_0320_0320_0320_0113_0113_0113_0113_0022_0021_0021;
        _weightPool.maleFaceIfCrown = 0x0fa0_03c0_03c0_03c0_0000_03c0_03c0_016f_016f_0000_016f_0032_0032;
        _weightPool.maleFaceIfSkeleton = 0x1d4c_0000_0000_044c_0000_0000_0000_02bc_02bc;
        _weightPool.maleFaceIfSpecial = 0x1770_0384_0384_0384_0000_0000_0000_0190_0190_0000_0190_0064;
        // MaleHairs enum
        _weightPool.maleHairs = 0x06b8_06b8_06b8_02bc_06b8_06b8_02bc;
        _weightPool.maleHairsIfCrown = 0x0fa0_0fa0_07d0;
        _weightPool.maleHairsIfMask = 0x0708_0708_0708_03e8_0708_0708;
        // MaleHat enum
        _weightPool.maleHat = 0x03e8_03e8_03e8_03e8_03e8_03e8_03e8_03e8_0154_0154_0154_0154_0154_0064_0064_0064;
        // Special enum
        _weightPool.special = 0x24b8_007d_007d_007d_007d_001e_001e_0014_0014;
        // Unique enum
        _weightPool.unique = 0x2690_0080;
        // Wing enum
        _weightPool.wings = 0x251c_0096_0096_0096_0032;
    }
    ...
    function mintWhitelist(
        ...
        if (_mintLaunchDate >= block.timestamp + 1 days) {
            revert MintingNotLaunched();
        }
    ...
    function reclaim(uint256[] calldata tokenIds) external whenNotPaused noContract {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (sender != _stakes[tokenId].owner) {
                revert Unauthorized();
            }
    ...
    function stake(uint256[] calldata tokenIds) external whenNotPaused noContract {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _stakes[tokenId] = Stake({owner: sender, timestamp: uint64(block.timestamp)});
        }
    }
    ...
    function stakedTokenIdOfOwnerByIndex(address owner, uint256 index) external view returns (uint256) {
        uint256 count = balanceOf(address(this));
        uint256 j;
    ...
    function _randomTraits(uint256 seed) private view returns (TraitSet memory) {
        ...
        if (traits.gender == Gender.Male) {
            traits.special = Special(Random.weighted(_weightPool.special, 9, seed++));
            ...
            if (traits.special < Special.IvorySkeleton) {
                ...
                traits.wings = Wing(Random.weighted(_weightPool.wings, 5, seed++));
            }

            // If skeleton or not special
            if (traits.special == Special.None || traits.special >= Special.IvorySkeleton) {
                MaleHat hat = MaleHat(Random.weighted(_weightPool.maleHat, 16, seed++));
                MaleFace face;

                if (traits.special == Special.None) {
                    traits.clothing = uint8(Random.weighted(_weightPool.maleClothing, 16, seed++) + 1);

                    // No face and hairs if samurai helmet
                    if (hat != MaleHat.SamuraiHelmet) {
                        if (hat == MaleHat.Crown || hat == MaleHat.Headband || hat == MaleHat.RoyalCrown) {
                            face = MaleFace(Random.weighted(_weightPool.maleFaceIfCrown, 13, seed++));

                            traits.hairs = uint8(Random.weighted(_weightPool.maleHairsIfCrown, 3, seed++) + 2);
                        } else {
                            face = MaleFace(Random.weighted(_weightPool.maleFace, 14, seed++));
                            ...
                            if (
                                ...
                                traits.hairs = uint8(Random.weighted(_weightPool.maleHairsIfMask, 6, seed++) + 1);
                            } else {
                                traits.hairs = uint8(Random.weighted(_weightPool.maleHairs, 7, seed++) + 1);
                            }
                        }
                    }
                } else {
                ...
                // Only compatible faces if special knight/ninja
                traits.face = uint8(Random.weighted(_weightPool.maleFaceIfSpecial, 12, seed++));
            }
...
enum Special {
    ...
    ShadowNinja, // 1_25
    BloodNinja, // 1_25
    GildedBlackKnight, // 0_30
    GildedWhiteKnight, // 0_30
    IvorySkeleton, // 0_20
    EbonySkeleton // 0_20
}
