...
contract TokenX is ITokenX {
    ...
    uint256 constant private INITIAL_SUPPLY_AMOUNT = 10**7 * 1e18;
    uint256 constant private INITIAL_TAX_FEE = 50;  // 50%
    ...
    modifier onlyPoolOwner {
        require (
            ...
            msg.sender == owner, "no permission");
        _;
    }
    ...
    constructor (
        ...
        owner = msg.sender;
        _balances[owner] = INITIAL_SUPPLY_AMOUNT / 100;
    }
    ...
    function getLiquidAmount(address user_) external view override returns (uint256) {
    ...
    function leftDays() external view override returns (uint256) {
        ...
        return (current - launchedAt) / 1 days;
    }

    function supplyAmount(uint256 auctionId_) external pure override returns (uint256) {
        return INITIAL_SUPPLY_AMOUNT * (100 - auctionId_) / 100;
    }
