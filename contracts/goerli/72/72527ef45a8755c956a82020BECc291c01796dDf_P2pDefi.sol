...
contract P2pDefi is Ownable {
    ...
    mapping(address => mapping(address => mapping(uint256 => OrderLendData)))
        public orderLendMap;
    mapping(address => mapping(address => mapping(uint256 => TermLendData)))
        public termLendMap;
    mapping(address => mapping(address => mapping(uint256 => OrderBorrowData)))
        public orderBorrowMap;
    mapping(address => mapping(address => mapping(uint256 => TermBorrowData)))
        public termBorrowMap;
    mapping(address => mapping(address => uint256)) public orderBorrowId;
    ...
    function createLendOrder(
        ...
        // Status status,
        bool isActive
    ) public returns (OrderLendData memory) {
        ...
        uint256 i = 1;
        while (orderLendMap[_lendingToken][msg.sender][i].id != 0) {
            if (!orderLendMap[_lendingToken][msg.sender][i].isActive) {
                orderLendMap[_lendingToken][msg.sender][i] = OrderLendData(
                    i,
                    _lendingToken,
                    _termPeriodInDays,
                    _lendingAmount,
    ...
    function createBorrowOrder(
        ...
        uint256 i = 1;
        while (orderBorrowMap[_borrowingToken][msg.sender][i].id != 0) {
            if (!orderBorrowMap[_borrowingToken][msg.sender][i].isActive) {
                orderBorrowMap[_borrowingToken][msg.sender][
                    i
                ] = OrderBorrowData(
                    i,
                    _borrowingToken,
            ...
            orderBorrowId[_borrowingToken][msg.sender] = i;
        }
    }

    function getBorrowOrders(address _user, address _borrowingToken)
        public
        view
        returns (uint256)
    {
        return orderBorrowId[_borrowingToken][_user];
    }
}
