    ...
    Ownable
{
    ...
    uint256 public immutable ALLOWLIST_MINT_PRICE = 88000000000000000;
    //todo: set a public mint price
    uint256 public publicMintPrice;

    // Number of tokens reserved for public sale allowlist
    uint256 public publicSaleFreeAllowListReserve;

    // Max number of mints per tx for public mint
    // todo: set this in constructor as some param initially?
    uint256 public maxNumberMintsPerTx;
    ...
    // Track allocation remaining that is allowed to be minted by user
    mapping(address => int256) public allowlistQuantity;
    ...
    modifier canTransfer(uint256 tokenId) {
        require(!staking[tokenId].staked, "Token is currently being staked and cannot be transferred");
        _;
    }
     ...
     */
    function setMerkleTreePreSaleFreeAllowlist(bytes32 _preSaleFreeRoot) external onlyOwner {
        preSaleFreeRoot = _preSaleFreeRoot;
    }
     ...
     */
    function setMerkleTreePreSalePaidAllowlist(bytes32 _preSalePaidRoot) external onlyOwner {
        preSalePaidRoot = _preSalePaidRoot;
    }
     ...
     */
    function setMerkleTreePublicSaleFreeAllowlist(bytes32 _publicSaleFreeRoot, uint256 _publicSaleFreeAllowListReserve) external onlyOwner {
        publicSaleFreeRoot = _publicSaleFreeRoot;
        publicSaleFreeAllowListReserve = _publicSaleFreeAllowListReserve;
    }

    /**
    * @dev Function called to return if an address is allowlisted for the pre-mint free mint list.
    * @param proof Merkel tree proof.
    * @param _address Address to check.
    */
    function isPreSaleFreeAllowlisted(bytes32[] calldata proof, address _address)
        public
        view
        returns (bool)
    {
    ...
    /**
    * @dev Function called to return if an address is allowlisted for the pre-mint paid mint list.
    * @param proof Merkel tree proof.
    * @param _address Address to check.
    */
    function isPreSalePaidAllowlisted(bytes32[] calldata proof, address _address)
        public
        view
        returns (bool)
    {
    ...
    /**
    * @dev Function called to return if an address is allowlisted for the public sale free mint list.
    * @param proof Merkel tree proof.
    * @param _address Address to check.
    */
    function isPublicSaleFreeAllowlisted(bytes32[] calldata proof, address _address)
        public
        view
        returns (bool)
    {
        ...
        external
    {
        ...
        require(
            isPreSaleFreeAllowlisted(proof, msg.sender),
            "Caller not pre-sale free allowlisted"
        );
        require(
            quantity > 0,
            "Quantity must be a postitive number"
        );
        require(
            quantity <= allowlistQuantity[msg.sender] + 1,
            "Minting quantity exceeds allocation left for user"
        );
        require(
            tokenCounter < MAX_SUPPLY,
            "Token count exceeds limit"
        );
        _safeMint(msg.sender, uint256(quantity));
        allowlistQuantity[msg.sender] -= quantity;
        ...
        payable
    {
        ...
        require(
            isPreSalePaidAllowlisted(proof, msg.sender),
        ...
        require(
            quantity <= allowlistQuantity[msg.sender] + 2,
            "Minting quantity exceeds allocation left for user"
        );
        require(
            tokenCounter < MAX_SUPPLY,
        ...
        require(
            msg.value >= (ALLOWLIST_MINT_PRICE * uint256(quantity)),
     ...
     * @param quantity Quantity to mint.
     */
    function mintPublicSaleFreeAllowlist(bytes32[] calldata proof, int256 quantity)
        external
    {
        require(
            isPublicSaleActive,
        ...
        require(
            isPublicSaleFreeAllowlisted(proof, msg.sender),
        ...
        payable
    {
        ...
        //todo: set real mint price properly below
        require(
            msg.value >= (publicMintPrice * quantity),
            "Ether value sent is incorrect"
        );
        require(
            tokenCounter < (MAX_SUPPLY - publicSaleFreeAllowListReserve),
            "Token count exceeds limit"
        );
     ...
     * @dev Flips the staking state for staking.
     */
    function setMaxNumberMintsPerTx(uint256 _maxNumberMintsPerTx) external onlyOwner {
    ...
    function stake(uint256 quantity, uint256[] calldata tokenIds) external {
        ...
        require(
            ...
            "Quantity exceed numbers of tokens held by sender available for staking"
        );
    ...
    /**
     * @dev Sets the pre-sale free allowlist quantities to be set. Default maximum of 1 free mint per user.
     ...
     */
    function setPreSaleFreeAllowlist(address[] calldata addresses, int256[] calldata quantities) external onlyOwner {
        require(addresses.length == quantities.length, "Addresses and quantities lengths do not match up");
    ...
    /**
     * @dev Sets the pre-sale paid allowlist quantities to be set. Default maximum of 2 paid mints per user.
     * @param addresses The addresses to set for the allowlist.
     * @param quantities The quantities for each address to be able to mint in the allowlist.
     */
    function setPreSalePaidAllowlist(address[] calldata addresses, int256[] calldata quantities) external onlyOwner {
        require(addresses.length == quantities.length, "Addresses and quantities lengths do not match up");
        for (uint256 i=0; i < addresses.length; i++) {
    ...
    /**
     * @dev Sets the public sale free allowlist quantities to be set. Default maximum of 1 free mint per user.
     * @param addresses The addresses to set for the allowlist.
     * @param quantities The quantities for each address to be able to mint in the allowlist.
     */
    function setPublicSaleFreeAllowlist(address[] calldata addresses, int256[] calldata quantities) external onlyOwner {
        require(addresses.length == quantities.length, "Addresses and quantities lengths do not match up");
        for (uint256 i=0; i < addresses.length; i++) {
    ...
    function _beforeTokenTransfers(
        ...
        uint256 quantity
    ) internal canTransfer(startTokenId) override {}
