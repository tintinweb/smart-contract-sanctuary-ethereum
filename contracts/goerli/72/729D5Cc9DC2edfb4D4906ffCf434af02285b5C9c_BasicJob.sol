...
contract BasicJob is Keep3rMeteredFallbackJob {
  uint256 public nonce;
  uint256 nextWorkable;
  error NotWorkable();

  address internal constant DAI = 0x6B175474E89094C44Da98b954EedeAC495271d0F;
  address internal constant DAI_WETH_POOL = 0xC2e9F25Be6257c210d7Adf0D4Cd6E3E881ba25f8;

  constructor() Governable(msg.sender) Keep3rMeteredFallbackJob(DAI, DAI_WETH_POOL) {}

  function work() external upkeep {}

  function workHard(uint256 howHard) external upkeep {
    _workHard(howHard);
  }

  function workHardMetered(uint256 howHard) external upkeepMetered {
    _workHard(howHard);
  }

  function workHardFallbackMetered(uint256 howHard) external upkeepFallbackMetered {
    _workHard(howHard);
  }

  function _workHard(uint256 howHard) internal {
    if(block.timestamp < nextWorkable) revert NotWorkable();

    nextWorkable = block.timestamp + 60;
    for (uint256 i = 0; i < howHard; i++) {
      nonce++;
    }
  }
}
...
abstract contract Keep3rMeteredFallbackJob is Keep3rMeteredJob, IKeep3rMeteredFallbackJob {
  ...
  modifier upkeepFallbackMetered() {
    ...
    if (!_fallback && _initialGas - _gasAfterWork > gasMaximum) revert GasMeteredMaximum();
...
abstract contract Keep3rMeteredJob is Keep3rJob, IKeep3rMeteredJob {
  ...
  modifier upkeepMetered() {
    ...
    _;
    uint256 _gasAfterWork = gasleft();
    if (_initialGas - _gasAfterWork > gasMaximum) revert GasMeteredMaximum();
