...
contract StakingSBT is ERC721Psi, IERC4906, Ownable {
    ...
    constructor() ERC721Psi("StakingReceipt", "STKR") {}
    ...
    /// @notice called by staking contract when a user stakes or unstakess
    function emitBatchMetadataUpdate() private {
        /// @notice update all rankings as suggested by https://docs.opensea.io/docs/metadata-standards
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory svgTemplate = '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><rect width="100" height="100" style="fill:#FFFFFF;"/><text x="50" y="50" text-anchor="middle" font-size="40">#NUM#</text></svg>';
        string memory svg = bytes(svgTemplate).length > 0 ? string(abi.encodePacked(svgTemplate)) : '';

        // Replace the #NUM# placeholder with the actual number
        svg = string(abi.encodePacked(
        bytes(svg),
        bytes(
            abi.encodePacked(
            '<text x="50" y="50" text-anchor="middle" font-size="40">',
            thisRank,
            '</text>'
            )
        )
        ...
        // return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, thisRank, ".json")) : "";
    }

    function lockRanks() public onlyOwner {
        lockTimestamp = block.timestamp;
        ranksAreLocked = true;
    }

    function unlockRanks() public onlyOwner {
    ...
    function setStakingContract(address newStakingContract) public onlyOwner {
