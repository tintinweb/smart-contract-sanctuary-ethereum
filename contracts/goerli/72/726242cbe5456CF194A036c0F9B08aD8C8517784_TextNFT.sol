...
contract TextNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    constructor(string memory name, string memory symbol, string memory description) ERC721(name, symbol) Ownable() {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', name(), '# ', toString(tokenId), '", "description": "', _description, '", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function claim(uint256 amount) public nonReentrant {
        for(uint256 i; i<amount; i++) {
            require(claimCount[_msgSender()] <= claimLimit, "over limit");
            _claim(_msgSender());
        }
    }
    
    function ownerClaim(address to, uint256 amount) public nonReentrant onlyOwner {
        for(uint256 i; i<amount; i++) {
            _claim(to);
        }
    }
