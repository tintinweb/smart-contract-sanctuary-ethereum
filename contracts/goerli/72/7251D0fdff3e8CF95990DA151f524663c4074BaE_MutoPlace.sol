...
contract MutoPlace is Ownable, Verification {
    constructor() {}
    ...
    event ShopTransferred(uint256[] mintedTokens, string[] resourceIds, address user);
    ...
    function buyShop(buyShopData memory _buyData) external payable {
        require(!seenNonces[msg.sender][_buyData.nonce], "Invalid request");
        seenNonces[msg.sender][_buyData.nonce] = true;
        // require(verify(msg.sender, msg.sender, _buyData.amount, _buyData.encodeKey, _buyData.nonce, _buyData.signature), "invalid signature");
        uint256 tokenId;
        uint256 length = _buyData._assetsData.length;
        uint256[] memory mintedTokens = new uint256[](length);
        string[] memory resourceIds = new string[](length);
        for(uint8 i = 0 ; i < length; i++) {
            mint721Data memory mintData = mint721Data(
                _buyData._assetsData[i].metadata,
                payable(msg.sender),
                _buyData.nft
            );
            tokenId = mint721(mintData);
            mintedTokens[i] = tokenId;
            resourceIds[i] = _buyData._assetsData[i].resourceId;
        }
        if(_buyData.currency!=1) {
            transfer20(msg.sender, address(this), _buyData.amount, _buyData.erc20token);
        }
        emit ShopTransferred(mintedTokens, resourceIds, msg.sender);
    }
