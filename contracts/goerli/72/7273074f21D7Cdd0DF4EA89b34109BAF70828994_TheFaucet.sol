 ...
 * @author Basil Gorin
 */
contract TheFaucet is UpgradeableAccessControl {
	/**
	...
	function postConstruct() public initializer {
		...
		epochLength = 1 days;
		weiLimitPerEpoch = 10 ether;

		// emit an event
		emit EpochParamsUpdated(epochLength, weiLimitPerEpoch);
	}
	...
	function addUsers(address[] calldata users) public {
		...
		for(uint256 i = 0; i < users.length; i++) {
			...
			// update the permission bypassing the standard ACL flow, we don't want to share ROLE_ACCESS_MANAGER permission:
			// reassign the role
			userRoles[users[i]] = ROLE_FAUCET_USER;
			// fire an event
			emit RoleUpdated(msg.sender, users[i], ROLE_FAUCET_USER, ROLE_FAUCET_USER);
		}
	}
	...
	function removeUsers(address[] calldata users) public {
		...
		for(uint256 i = 0; i < users.length; i++) {
			...
			userRoles[users[i]] = 0;
			// fire an event
			emit RoleUpdated(msg.sender, users[i], 0, 0);
		}
	}
