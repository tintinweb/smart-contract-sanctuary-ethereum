...
contract Wiwimates is
    ...
    ERC2981
{
    constructor() ERC721("Wiwimates", "WIWI") {}
    ...
    MintPhase private currentPhase;
    ...
    mapping(uint256 => uint256) celstates;
    mapping(uint256 => string) BASE_URIS;
    string private BASE_EXTENSION;
    uint256 private URI_SELECTOR;

    // ------------------ External ------------------ //

    function claim(uint256 _celblock, bytes memory _signature)
        external
        payable
        nonReentrant
    {
        ...
        require(
            _recoverSigner(_celblock, _signature) == owner(),
            "Not on allowlist"
        );
    ...
    function _recoverSigner(uint256 _celblock, bytes memory signature)
        internal
        view
        returns (address)
    {
         ...
         bytes32 messageDigest = keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encodePacked(msg.sender, _celblock))
            )
        );
    ...
    function setURISelector(uint256 _selector) external onlyOwner {
        URI_SELECTOR = _selector;
    }
    ...
    function setAuthorised(address _address, bool _auth) public onlyOwner {
