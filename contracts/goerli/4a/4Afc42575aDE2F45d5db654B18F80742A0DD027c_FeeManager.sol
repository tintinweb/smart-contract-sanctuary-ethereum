...
contract FeeManager is OwnableUpgradeable {
    ...
    event CreationFeeChanged(uint256 indexed creationFee);
    event PaymentFeeChanged(uint256 indexed paymentFee);

    // INIT

    function initialize(uint256 initialCreationFee, uint256 initialPaymentFee)
        public
        initializer
    {
        __Ownable_init();
    ...
    function collectFee(IAgreement from) public onlyOwner {
        from.collectFee();
    }

    function withdrawNativeCoins(address payable to) public onlyOwner {
        to.transfer(address(this).balance);
    }

    function withdrawERC20(address payable to, IERC20Upgradeable token)
        public
        onlyOwner
    {
