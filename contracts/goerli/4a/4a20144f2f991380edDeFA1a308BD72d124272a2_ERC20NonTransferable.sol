...
contract ERC20NonTransferable is ERC20Upgradeable,AccessControl {
    ...
    /// @dev details of user
    function userDetails(address _userAddress) public view returns(bool,uint256,bool){
    ...
    function startDeposit(uint256 _days)
        external
    {
        require(hasRole(ADMIN, msg.sender), "Only Admin");
        require(
    ...
    function getUsdcDetails(address _tokenAddress) external view returns(uint256,uint256){
        return (USDCDeposited[_tokenAddress],USDCTransferedToAdmin[_tokenAddress]);
    }
...
contract Emitter is Initializable,AccessControl {
    ...
    event NewUser(address indexed _proxy, address indexed _depositor, address indexed _tokenAddress, uint256 _amount,uint256 _timeStamp);
    ...
    function newUser(address _proxy, address _depositor, address _tokenAddress, uint256 _amount,uint256 _timeStamp) public onlyRole(EMITTER) {
        emit NewUser(_proxy,_depositor,_tokenAddress,_amount,_timeStamp);
    }
