...
contract ProxySale is Ownable, ReentrancyGuard, IProxySale {
    ...
    event Stake(address indexed owner, string indexed text, uint256 indexed timestamp);
    ...
    constructor(address _signer, address _vault) public {
        signer = _signer;
        vault = _vault;
        MaxSupply = 588;
    }
    ...
    function _verify(bytes32 _hash, bytes memory _token) internal view returns (bool)
    {
        return signer == _recover(_hash, _token);
    }
    ...
    {
        preMint(_proof);
        uint256 _tokenId = erc721.balanceOf(msg.sender);
        stake(_tokenId, _stakeType);
    }
    ...
    eoaOnly
    {
        ...
        require(_verify(_hash(_salt, msg.sender), _signature), "Invalid signature");

        require(AuctionEpochSupply - erc721.totalSupply() > 1, "Max supply reached");
    ...
    eoaOnly
    {
        publicMint(_salt, _signature);
        uint256 _tokenId = erc721.balanceOf(msg.sender);
        stake(_tokenId, _stakeType);
    }


    function stake(uint256 _tokenId, uint256 _stakeType) public {
        ...
        emit Stake(msg.sender,string(abi.encodePacked(Strings.toString(_tokenId), ",", Strings.toString(_stakeType))) ,stakeList[_tokenId].stakeTimestamp);
    }

    function unStake(string calldata _salt, bytes calldata _signature,  uint256 _tokenId) external {
    ...
    function setAuctionConfig(
        ...
        AuctionEpochSupply = erc721.totalSupply() + _epochSupply;
