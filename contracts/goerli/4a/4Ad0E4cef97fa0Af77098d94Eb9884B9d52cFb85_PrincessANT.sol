...
contract PrincessANT is ERC721Upgradeable, OwnableUpgradeable {
    ...
    function getClaimable(address _user)
        public
        view
        returns (uint256 _claimable)
    {
        ...
        for (uint256 i = 0; i < playerToPrincesses[_user].length; i++) {
            uint256 _princessId = playerToPrincesses[_user][i];
            uint256 _passed = _now - idToMateTime[_princessId];
            if (_passed >= duration && idToMateTime[_princessId] != 0) {
                _claimable++;
            }
        }
    }
