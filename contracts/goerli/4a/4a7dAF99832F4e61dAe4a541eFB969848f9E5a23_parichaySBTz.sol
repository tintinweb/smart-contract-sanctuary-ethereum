...
interface IMantleSBT {
    function safeMint(uint256, address, uint256) external;
    function updateExpiry(address) external;
}

contract parichaySBTz is ERC721, ERC721Burnable, IXReceiver, Ownable {
    ...
    function xReceive(
        ...
        if (condition && !isMantle) {
            ...
            require(balanceOf(aadmi) == 0, "already present");
            safeMint(aadmi, tokenId);
            addressToExpiry[aadmi] = expiry;
        } if (!condition && !isMantle) {
            //update on any chain
            updateExpiry(aadmi);
        } if (condition && isMantle) {
            ...
            message = abi.encodeWithSelector(
                IMantleSBT.updateExpiry.selector,
                aadmi
            );
