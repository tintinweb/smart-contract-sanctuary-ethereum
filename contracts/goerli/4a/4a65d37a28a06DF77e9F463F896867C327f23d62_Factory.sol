    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _implementation,address _dex) public initializer {
        __Pausable_init();
        __Ownable_init();
    ...
    function createIndexFund(
        ...
        IndexInterface(indexAddress).initialize(_percentages,_tokens,_depositendingtime,_indexendingtime,_ptoken,_dex);
    ...
    function updateIndexFund(
        ...
        require(
            ...
            "percentage array should not be empty"
        );
    ...
    function purchase(uint id) public onlyOwner {
        IndexInterface(indexAddressArray[id]).purchase();
    }

    function sell(uint id) public onlyOwner {
        IndexInterface(indexAddressArray[id]).sell(feeGetter);
    }

    function rebalancePurchase(uint id) public onlyOwner {
        IndexInterface(indexAddressArray[id]).rebalancepurchase();
    }

    function rebalanceSell(uint id) public onlyOwner {
    ...
    function deposit(
       ...
       uint indexdepositamount= amount-fees;
        ...
        IERC(ptoken).transferFrom(msg.sender,indexAddressArray[id],indexdepositamount);

        IndexInterface(indexAddressArray[id]).deposit(indexdepositamount);
    }

    function indextokenbalance(uint id) public view onlyOwner returns (uint256[] memory){
...
interface IndexInterface {
    ...
    function sell(address feeGetter ) external;

    function rebalancesell() external;

    function rebalancepurchase() external;
