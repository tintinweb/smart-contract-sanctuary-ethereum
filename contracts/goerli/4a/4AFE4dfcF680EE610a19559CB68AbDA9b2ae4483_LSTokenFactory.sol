...
contract LSToken is ERC20 {
  ...
  function mint() external {
    ...
    uint256 projectAmount = ((projectShare * mintAmount) / TOTALSHARE) * 1e18;
...
// File contracts/token/LSTokenFactory.sol
...
contract LSTokenFactory {
  ...
  //   uint256 _rate,
  //   uint256 _proposalId
  // ) external OnlyFundraising returns (address ipToken) {
  //   bytes memory bytecode = type(LSToken).creationCode;
  //   bytes32 salt = keccak256(
  //     abi.encodePacked(keccak256(abi.encode("LeapStart IP Token")), _owner)
  //   );
  //   assembly {
  //     ipToken := create2(0, add(bytecode, 32), mload(bytecode), salt)
  //   }

  //   ILSToken(ipToken).initialize(
  //     _owner,
  //     _rate,
  //     _proposalId,
  //     address(fundraising)
  ...
  function createIPToken(
    ...
    uint256 _projectId,
    uint256 _projectShare
  ) external OnlyFund returns (address) {
    LSToken ipToken = new LSToken(_tokenOwner, _projectId, address(fundraising), _projectShare);

    emit IpTokenCreated(_tokenOwner, address(ipToken), _projectId, _projectShare);
