...
contract TRANSACTIONS is ERC1155, Ownable, RrpRequesterV0, DefaultOperatorFilterer {
    ...
    function batchMintAuto(uint256[] memory _pendingsIds) public payable {
        for(uint256 i = 0; i < _pendingsIds.length; i++){
            mintSingleAuto(_pendingsIds[i]);
        }
    }

    //if mintphase == 2, input arg doesn't matter, hide any input on site
    function mintSingleAuto(uint256 _pendingsId) public payable {
        if(mintPhase == 1) { mintSingleBlockPendingsHolderWhitelist(_pendingsId); }
        if(mintPhase == 2) { mintSingleBlockPublic(); }
    }
    ...
    function claimSinglePendingsOwner(uint256 _pendingsId) public {
        ...
        if(mintPhase != 1){ revert MintIsClosed(); }
    ...
    function mintSingleBlockPublic() public payable {
        if(paused){ revert MintIsClosed(); }
        address sender = msg.sender;

        if(mintPhase != 2){ revert MintIsClosed(); }
        if(msg.value < mintPrice + qrngGasForwarded) { revert InsufficientFunds(); }
        if(totalMinted == nominalMaxSupply){revert MaxSupplyReached(); }
    ...
    */ 
    ...
    function addUnusedFreeMintsToRemainingIds() private {
    ...
    function setMintPhase(uint256 _phase) public onlyOwner {
        ...
        if(mintPhase == 2) {
            addUnusedFreeMintsToRemainingIds();
        }
    }
