...
//It is an ABSOLUTE NEED to declare the tx and important funtions ass only usable for the contract owner. See Argent method ass example https://github.com/argentlabs/argent-contracts/blob/develop/contracts/modules/TransactionManager.sol
    ...
    uint8 private _signaturesNeeded=10;
    ...
    //Maps Guardians
    mapping (address => Guardian) internal guardians;

    // functions 

    // Guardian related functions
    function createGuardian(address _guardian, uint8 _level) public {
        require(_msgSender() == _owner, "Only owner can call function");
        guardians[_guardian] = Guardian(true,_level);
        if (_signaturesNeeded >= 10){
            _signaturesNeeded = 1;
        } else { _signaturesNeeded +=1;}
        emit GuardianCreated(_guardian,_level);
    }

    function signOwnerChange() public {
        require(guardians[msg.sender].isguardian == true, "Not a guardian");
        require(guardians[msg.sender].level > 0, "Not a guardian");
        _signaturesDone += 1;
        emit OwnerChangeSigning(msg.sender);
    }

    function changeOwner(address _newOwner) public {
        if (guardians[msg.sender].level == 1) {
            require(guardians[msg.sender].isguardian == true, "Not a guardian");
            _owner = _newOwner;
            _signaturesDone = 0;
    ...
    // Balance and receiving and sending tx realted functions
    ...
    function viewGuardian(address guardian) public view returns(bool) {
        return(guardians[guardian].isguardian);
    }
