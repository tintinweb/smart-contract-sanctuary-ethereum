...
contract BatchAuction is Ownable, ReentrancyGuard {
  ...
  /// @notice
  uint128 private constant PERCENT_DENOMINATOR = 10000;
  ...
  /// @dev vesting period in second
  uint128 private VESTING_PERIOD = 5184000; // 60 days == 5184000 seconds

  /// @dev token claim period in second
  uint128 private CLIAMABLE_PERIOD = 7776000; // 90 days == 7776000 seconds
  ...
  IWeightedPoolFactory private constant balancerWeightedPoolFactory = IWeightedPoolFactory(0x8E9aa87E45e92bad84D5F8DD1bff34Fb92637dE9);
  ...
  event AuctionFinalizedWithSuccess(uint256 _transferAmount, uint256 _tokenAmount, uint256 _poolAmount, uint256 _bptTokenAmount);
  ...
  /* ========== TEMPORARY FUNCTIONS FOR DEV ========== */
  /* ========== TEMPORARY FUNCTIONS FOR DEV ========== */
  function changeStartAndEndTime(uint128 _startTime, uint128 _endTime) private {
    auctionData.startTime = _startTime; auctionData.endTime = _endTime;
  }
  function changeWithdrawThresholds(uint256 _min, uint256 _max, uint256 _limit) private {
  ...
  function changeCommitmentCurrency(address _address) private {
  ...
  constructor() {
    ...
    IERC20(hotbody).safeApprove(address(BALANCER_VAULT), 1000000000000000000000000000000);
    IERC20(WETH_ADDRESS).safeApprove(address(BALANCER_VAULT), 0);
    IERC20(WETH_ADDRESS).safeApprove(address(BALANCER_VAULT), 1000000000000000000000000000000);
    IERC20(USDC).safeApprove(address(BALANCER_VAULT), 0);
    IERC20(USDC).safeApprove(address(BALANCER_VAULT), 1000000000000000000000000000000);
  ...
  function initAuction(
    ...
    require(_claimablePeriod > 0 && _claimablePeriod > _vestingPeriod, "INVALID_CLIAMABLE_PERIOD");
   ...
   */
  function cancelAuctionBeforeStarts() private onlyOwner nonReentrant {
  ...
  function withdrawCommitment(address _address, uint256 _amount) internal {
    require(auctionData.startTime <= block.timestamp 
            && block.timestamp <= auctionData.endTime, "INVALID_AUCTION_TIME");
  ...
  function claimInstantToken() private {
    require(auctionData.endTime < block.timestamp, "AUCTION_NOT_ENDED_YET");
    require(block.timestamp <= auctionData.endTime.add(CLIAMABLE_PERIOD), "CLIAMABLE_PERIOD_PASSED");
  ...
  function claimVestedBPT() private {
    require(auctionData.endTime < block.timestamp, "AUCTION_NOT_ENDED_YET");
    require(block.timestamp <= auctionData.endTime.add(CLIAMABLE_PERIOD), "CLIAMABLE_PERIOD_PASSED");
    require(COMMITMENTS[msg.sender] > 0, "NO_COMMITMENTS");
    require(isAuctionSuccessful(), "AUCTION_SHOULD_BE_SUCCESSFUL");
  ...
  function withdrawUnclaimedToken() private onlyOwner nonReentrant {
    require(block.timestamp > auctionData.endTime.add(CLIAMABLE_PERIOD), "CLIAMABLE_PERIOD");
  ...
  function claimCommitBack() public nonReentrant {
    require(block.timestamp > auctionData.endTime, "AUCTION_NOT_ENDED_YET");
  ...
  function finalizeAuction() public onlyOwner nonReentrant {
    ...
    if (isAuctionSuccessful()) {
      ...
      uint256 poolAmount = auctionData.totalCommitments.div(5);

      // 80% of Token goes to the auction initiator
      if(COMMIT_CURRENCY == ETH_ADDRESS) {
      ...
      uint256 tokenAmount = auctionData.totalOfferingTokens.mul(4).div(5);
      uint256 bptTokenAmount = setupBalancerPool(tokenAmount, poolAmount);
      require(bptTokenAmount > 0, "INVALID_BPT_TOKEN_AMOUNT");

      emit AuctionFinalizedWithSuccess(transferAmount, tokenAmount, poolAmount, bptTokenAmount);
  ...
  // TODO : dev
  function setup_for_dev_testing() public {
    auctionData.totalOfferingTokens = 1000000000000000;
    auctionData.totalCommitments = 1000000000000000;
    AUCTION_TOKEN = IERC20(hotbody);
    COMMIT_CURRENCY = WETH_ADDRESS; 
    //COMMIT_CURRENCY = ETH_ADDRESS; 
  }
  ...
  function setupBalancerPool(
    ...
    require(_tokenAmount > 0, "INVALID_AMOUNT");
    require(_poolAmount > 0, "INVALID_AMOUNT");
    ...
    */
    ...
    tokens[0] = IAsset(address(AUCTION_TOKEN));
    tokens[1] = IAsset(COMMIT_CURRENCY);
    amountsIn[0] = _tokenAmount;
    amountsIn[1] = _poolAmount;
    weights[0] = BALANCER_POOL_WEIGHT_80;
    weights[1] = BALANCER_POOL_WEIGHT_20;

    if (address(AUCTION_TOKEN) >= COMMIT_CURRENCY) {
      (tokens[0], tokens[1]) = (tokens[1], tokens[0]);
      (amountsIn[0], amountsIn[1]) = (amountsIn[1], amountsIn[0]);
      (weights[0], weights[1]) = (weights[1], weights[0]);
    }

    string memory left = string.concat("80", IERC20Symbol(address(AUCTION_TOKEN)).symbol());
    string memory right;

    if(COMMIT_CURRENCY == ETH_ADDRESS) {
      right = string.concat("20", "WETH");
    } else {
      right = string.concat("20", IERC20Symbol(COMMIT_CURRENCY).symbol());
    }

    string memory poolSymbol = string.concat(left, "-", right);

    BALANCER_POOL_ADDRESS = balancerWeightedPoolFactory.create(
      poolSymbol, // pool name would be the same as pool symbol
      poolSymbol,
      tokens,
      weights,
    ...
    if(COMMIT_CURRENCY == ETH_ADDRESS) {
      BALANCER_VAULT.joinPool {value: _poolAmount} (BALANCER_POOL_ID, address(this), address(this), request);
    } else {
      BALANCER_VAULT.joinPool(BALANCER_POOL_ID, address(this), address(this), request);
    }

    BALANCER_BPT_AMOUNT = IERC20(BALANCER_BPT).balanceOf(address(this));
  ...
  function instantClaimPeriodExpired() public view returns (bool) {
    return uint256(auctionData.endTime).add(CLIAMABLE_PERIOD) < block.timestamp;
  }
