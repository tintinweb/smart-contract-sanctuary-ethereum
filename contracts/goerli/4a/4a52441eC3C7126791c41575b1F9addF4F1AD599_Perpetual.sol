...
abstract contract Deposits is Storage {
    ...
    function depositERC20(
        ...
        if (
            cancellationRequests[packedKey] != 0 && ethKeys[l2Key] == msg.sender
        ) {
    ...
    function depositCancel(uint160 l2Key, uint24 accountId)
        external
        onlyKeyOwner(l2Key)
    {
        uint256 packedKey = (uint256(l2Key) << 24) | uint256(accountId);
    ...
    function depositReclaim(uint160 l2Key, uint24 accountId)
        external
        onlyKeyOwner(l2Key)
        nonReentrant
    {
...
abstract contract Withdrawals is Storage {
    ...
    function withdrawERC20(
        ...
        emit LogWithdrawalPerformed(ownerKey, amount, recipient);
    }
    ...
    function withdrawTo(uint256 ownerKey, address payable recipient)
        external
...
abstract contract UpdateState is ForcedWithdrawals, ForcedTrade, Config {
    ...
    function postProcess(
        ...
        while (offset < pubData.length) {
            ...
            uint256 packedKey = (uint256(op.l2Key) << 24) | uint256(op.accountId);
    ...
    function verifyProofCommitment(
        ...
        for (uint256 i = 0; i < _newBlocks.length; ++i) {
            ...
            require(proof_commitments[i] & INPUT_MASK == uint256(commitment) & INPUT_MASK, "ia");
...
contract Storage is Governance, ReentrancyGuard {
    ...
    function removeForcedRequest(uint160 req, bool isForcedWithdrawal)
        internal
    {
