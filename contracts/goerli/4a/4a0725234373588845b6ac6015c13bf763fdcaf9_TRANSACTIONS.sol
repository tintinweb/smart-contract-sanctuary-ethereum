...
contract TRANSACTIONS is ERC721, DefaultOperatorFilterer, Ownable {
    ...
    bool public blurAirdropComplete = false;
    ...
    // if the airdrop mint is not complete, no mint
    ...
    function mintPublic(uint256 _amount) public payable {
        if(!mintIsOpen) { revert MintIsClosed(); }
        if(!pendingsHolderAirdropMintComplete) { revert MintIsClosed(); }
        if(amountMinted >= nominalMaxSupply) { revert MaxSupplyReached(); }
        if(amountMinted + _amount > nominalMaxSupply) { revert MaxSupplyWillBeReached(); }
    ...
    function mintAllPendingsIdsToDeployer() public onlyOwner {
        if(pendingsHolderAirdropMintComplete) { revert MintIsClosed(); }
        for(uint256 i = 0; i < pendingsSupply; i++){
            _safeMint(msg.sender, amountMinted + 1);
            amountMinted++;
        }
        pendingsHolderAirdropMintComplete = true;
    }

    // admin mints ids 1-998 to airdrop to Pendings holders
    function mintAllBlurVictimsIdsToDeployer() public onlyOwner {

        if(blurAirdropComplete) { revert MaxSupplyWillBeReached(); }    

        for(uint256 i = 0; i < blurMaxSupply; i++){
            _safeMint(msg.sender, nominalMaxSupply + 1);
        }
        blurAirdropComplete = true;

    }

    function testFullPublicMintAdmin() public onlyOwner {
        for(uint256 i = amountMinted; i < nominalMaxSupply; i++){
            _safeMint(msg.sender, amountMinted + 1);
            amountMinted++;
        }
    }

    function testNonSeqMint(uint256 _id) public onlyOwner {
    ...
    function setNotRevealedURI(string memory _unrevealedBaseURI) public onlyOwner {
