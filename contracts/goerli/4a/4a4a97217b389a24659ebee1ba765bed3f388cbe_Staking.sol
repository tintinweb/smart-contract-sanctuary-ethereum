...
contract Staking is EIP712 {
    ...
    IERC721A private _mainContract = IERC721A(0x53A81b23eF192fAA864f77eFebbCccf582D25eC7);
    IERC20 golds = IERC20(0x9076993E1f930f39b3B25B0e5695d99BaA0A1727);
    ...
    function withdrawGolds(uint amount, bytes calldata signature) external {
        require(_signerAddress == _recoverAddress(msg.sender, amount, signature), "invalid signature");
    ...
    function _hash(address account, uint amount) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("Staking(address account,uint256 amount)"),
                        account,
                        amount
                    )
                )
    ...
    function _recoverAddress(address account, uint amount, bytes calldata signature) internal view returns(address) {
