...
interface Pool:
     ...
     def deposit(amount: uint256) -> uint256: payable
     def withdraw(amtDyad: uint256, recipient: address) -> uint256: nonpayable
     def mintDyad(recipient: address) -> uint256: payable
     def newETHPrice() -> int256: nonpayable
     def token() -> address: view
     def ethVault() -> address: view
     def lastEthPrice() -> uint256: view
     def lastCheckpointIndex() -> uint256: view
     def poolBalanceAtCheckpoint(arg0: uint256) -> uint256: view
     def poolDeltaAtCheckpoint(arg0: uint256) -> int256: view
     def totalXpAtCheckpoint(arg0: uint256) -> uint256: view
     def xpDeltaAtCheckpoint(arg0: uint256) -> uint256: view
...
@payable
@external
def mintDyad(tokenId: uint256):
    assert msg.sender == self.idToOwner[tokenId]

    res: Bytes[32] = raw_call(
        self.dyad_pool.address,
        method_id("mintDyad()"),
        max_outsize=32,
        value=msg.value
    )
    new_dyad: uint256 = convert(res, uint256)
