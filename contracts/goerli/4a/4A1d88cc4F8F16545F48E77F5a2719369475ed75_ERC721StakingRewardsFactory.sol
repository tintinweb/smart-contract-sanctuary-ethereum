...
contract ERC721StakingRewardsFactory is Ownable {
    ...
    function deploy(
        ...
        ERC721StakingRewards _contract = new ERC721StakingRewards(_erc20Address, _vaultAddress, _logoUrl, _backgroundUrl, _colors, msg.sender);
...
contract ERC721StakingRewards is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        for (uint8 i = 0; i < _colors.length; i++) colors[i] = _colors[i];
    ...
    function getColors(uint8 _count) external view returns (string[] memory) {
        string[] memory _colors = new string[](_count);
        for (uint8 i = 1; i <= _count; i++) {
            _colors[i] = colors[i];
        }
        return _colors;
    }

    function setColor(uint8 _index, string memory _value) external onlyOwner {
