...
*/
/// @title BlueTracker NFT contract using ERC1155.
contract BlueTrackerERC1155 is Ownable, ERC1155, PaymentSplitter {
    ...
    **/
    function publicSaleMint() external payable callerIsUser {
        ...
        require(currentPublic + 1 <= maxPublic, "Max supply exceeded");
        require(msg.value >= price * 1, "Not enough funds sent in the transaction");
    ...
    **/
    function gift(address[]  calldata  _to, uint[] calldata  _quantity) external onlyOwner {
        require(sellingStep > Step.Before, "Gift can happen only during public or soldout space");
        for(uint i = 0; i < _to.length; i++)
        {
            uint quantity = _quantity[i];
            require(currentGift + quantity <= maxGift, "Max supply for gift exceed");            
            _mint(_to[i], 1, quantity, "");
    ...
    */
    ...
    function setMaxGiftSupply(uint _maxGiftSupply) external onlyOwner {
        maxGift = _maxGiftSupply;
        maxSupply = maxPublic + maxGift;
    }
    ...
    */
    function totalSupply() external view returns(uint) {
        return currentGift + currentPublic;
    }
