...
contract GU is Ownable, ERC721A {
    string public baseURI;
    uint256 public maxSupply;
    uint256 public price;
    uint256 public maxTokenPerWallet;
    bool public salesStatus;

    event BridgeToken(address indexed burner, uint256 indexed tokenId, string btcAddress, uint256 burnTimestamp);
    ...
    function mint(uint256 _quantity) external payable {
        require(salesStatus, "NotStarted");
        require(msg.sender == tx.origin, "NotAllowed");
        require(_totalMinted() + _quantity <= maxSupply, "MaxSupply");
    ...
    function burn(uint256[] memory _tokenId, string memory _btcAddress) external {
        for (uint256 i = 0; i < _tokenId.length; i++) {
            require(ownerOf(_tokenId[i]) == msg.sender, "InvalidOwner");
            _burn(_tokenId[i]);
            emit BridgeToken(msg.sender, _tokenId[i], _btcAddress, block.timestamp);
        }
    }
    ...
    function numberBurnedByAddress(address _burner) external view returns (uint256) {
        return _numberBurned(_burner);
    }
