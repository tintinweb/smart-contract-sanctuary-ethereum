...
contract CultRewardPool is Ownable {
    ...
    /// @dev To change the coinReward variable within the range of 1-10,000 $CULT per day. Can only be called by the current owner.
    function setCoinReward(uint256 _coinReward) external onlyOwner {
        require(
            _coinReward > 1 ether && _coinReward < 10001 ether,
            "Please set reward to the range of 1-10,000 $CULT per day."
        );
        coinReward = _coinReward * 100 ether;
    }
}