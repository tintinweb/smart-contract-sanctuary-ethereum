...
contract Contract is ERC721Drop, PermissionsEnumerable {
    ...
    { _setupRole(DEFAULT_ADMIN_ROLE, msg.sender); }
        ...
        mapping(uint256 => bool) private _tokenURIsExists;
        using TWStrings for uint256;
        
    function setTokenURI(uint256 _tokenId, string memory _newURI) external onlyOwner {
    require(_exists(_tokenId), "Nonexistent token");
    _tokenURIs[_tokenId] = _newURI;
    _tokenURIsExists[_tokenId] = true;
    }


    function resetTokenURI(uint256 _tokenId) external onlyOwner {
    require(_exists(_tokenId), "Nonexistent token");
    _tokenURIsExists[_tokenId] = false;
    }
        ...
        if ( _tokenURIsExists[_tokenId])
        return string(_tokenURIs[_tokenId]);

        else if (isEncryptedBatch(batchId)) {
            return string(abi.encodePacked(batchUri, "0"));
        } else {
            return string(abi.encodePacked(batchUri, _tokenId.toString()));
        }
