...
contract TwapLimitOrder is ITwapLimitOrder {
    ...
    function _executeOrder(
        ...
        uint256 inOrderGasPrice = _uint32ToGasPrice(order.gasPrice);
        require(latestGasPrice <= inOrderGasPrice, 'TL60');
        (bool executionSuccess, bytes memory data) = address(this).call{ gas: order.gasLimit }(
    ...
    function _performRefund(
        ...
        if (shouldRefundPrepaidGas) {
            uint256 value = _uint32ToGasPrice(order.gasPrice).mul(order.gasLimit);
    ...
    function _buyLimitOrder(
        ...
        buyOrder.gasPrice = _gasPriceToUint32(_gasPrice.mul(gasMultiplier).div(GAS_PRECISION));
    ...
    function _sellLimitOrder(
        ...
        sellOrder.gasPrice = _gasPriceToUint32(_gasPrice.mul(gasMultiplier).div(GAS_PRECISION));
    ...
    function _gasPriceToUint32(uint256 _gasPrice) private pure returns (uint32 gasPrice32) {
        require((_gasPrice / 1e6) * 1e6 == _gasPrice, 'TL3C');
        gasPrice32 = (_gasPrice / 1e6).toUint32();
    }

    function _uint32ToGasPrice(uint32 gasPrice32) private pure returns (uint256 _gasPrice) {
        _gasPrice = uint256(gasPrice32) * 1e6;
    }
