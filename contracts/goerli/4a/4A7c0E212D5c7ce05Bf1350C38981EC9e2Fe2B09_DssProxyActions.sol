...
interface USBJoinLike {
    ...
    function wipe(bytes32, address, uint , uint) external payable;
}
...
interface JugLike {
    ...
    function getFeeBorrow(bytes32) external view returns (uint);
...
contract Common {
    ...
    function USBJoin_join_wipe(bytes32 ilk, address apt, address urn, uint wad, uint actualWad) public {
        // Gets USB from the user's wallet
        USBJoinLike(apt).USB().transferFrom(msg.sender, address(this), actualWad);
        // Approves adapter to take the USB amount
        USBJoinLike(apt).USB().approve(apt, actualWad);
        // Joins USB into the vat
        USBJoinLike(apt).wipe(ilk, urn, wad, actualWad);
    }
}
