...
contract GraphTokenLockWallet is GraphTokenLock {
    ...
    fallback() external payable {
        ...
        if (revocable == Revocability.Enabled) {
            ...
            require(
                usedAmount <= vestedAmount(),
                "Cannot use more tokens than vested amount"
            );
        }
    }
}
...
interface IGraphTokenLockManager {
    ...
    function setAuthFunctionCall(string calldata _signature, address _target)
        external;
    ...
    function getAuthFunctionCallTarget(bytes4 _sigHash)
        external
        view
        returns (address);
