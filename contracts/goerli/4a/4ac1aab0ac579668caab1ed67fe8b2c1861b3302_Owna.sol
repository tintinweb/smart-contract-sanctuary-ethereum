...
// File: contracts/Lender.sol
    ...
    function listedNft() public view returns( uint256[] memory) {
        ...
        function fixedLoanOffer( uint _durations ,uint _apr,uint _minLoan,uint _maxLoan,uint _nftId,address _borrower ) public  {
            ...
            require(_apr > 0, "Apr Cannot be 0");
    ...
    function flexibleLoanOffer( uint _durations,uint _apr,uint _minLoan,uint _maxLoan,uint _nftId,uint256 _acceptable_debt,address _borrower) public  {
            ...
            require(!idToNft[_nftId], "Borrower Already Accepted the offer at this Nft ID");
            require(!lenderOnNftId[msg.sender][_nftId], "Lender cannot again Offer on same nftId");
            require(_acceptable_debt <= 25,"Maximum Acceptable Debt cannot be more than 23%" );
            require(_acceptable_debt >2,"Maximum Acceptable Debt cannot be 0%" );
            require(_durations != 0,"Duration of loan zero no acceptable");
            require(_minLoan> 0,"Minimum loan should be greater 0");
            require(_maxLoan> _minLoan,"Maximum should be greater than minimum loan");
            require(((_maxLoan.mul(_acceptable_debt.sub(2).mul(100)).div(10000)).mul(365))/(_maxLoan.mul(_apr.mul(100)).div(10000)) == _durations.div(86400), 
            "Please Select a valid duration for flexible offering ");  
            require(_apr > 0, "Apr Cannot be 0");
            LoanDetail memory loanDetail = LoanDetail({apr:_apr,minLoan: _minLoan,maxLoan:_maxLoan,loan:0,acceptable_debt:_acceptable_debt-2 });  
            TimeDetail memory timeDetals = TimeDetail({durations:_durations,startTime:block.timestamp});  
            LoanOffer memory fix = LoanOffer(
                {
                offerID: requestAgainstNft[_nftId].length,
...
contract Borrower is Lender {
    ...
    function borrowFixLoan(uint256 _nftId ,uint256 _offerID ,uint256 _amount) public {
       ...
       require(_amount >= requestAgainstNft[_nftId][_offerID].loanDetail.minLoan && _amount <= requestAgainstNft[_nftId][_offerID].loanDetail.maxLoan, "Amount should be in MinLoan and MaxLoan Range" );
...
// ============================================repayFixLoan=====================
    function repayFixLoan(uint256 nftID,uint256 offerID,uint256 _selectAmount) internal {
       ...
       // calculate intrest 
        ...
        uint256 intrestAmount = last_update_days.div(oneDay).mul(fixedBorrow.loanAmount.div(100).mul(offer.loanDetail.apr).div(365)).div(24);
          // dailyInterest(fixedBorrow.loanAmount, offer.loanDetail.apr));
    ...
    //   =======> find if amount is more then just intrest af user also paying the intrest. also minus it from loan and send to the lender with 20% amount 
     ...
     // if amount only for. entery fee and interest
        ...
        IERC20(tokenAddress).transfer(secondWallet, _selectAmount.sub(eightyPercent(_selectAmount))+entryFee); // 20 % to owna => in Contract = 80% + 7000
          // trnasfer 80% of intrest 
        IERC20(tokenAddress).transfer(offer.lender,eightyPercent(_selectAmount));
 ...
 emit loanFixRepaid(offer.borrower, nftID, offerID, intrestAmount );
  ...
  // =======================repayLoan=============================>>
    ...
    function percentageCalculate ( uint256 _val ) public view returns(uint256){
      return _val.div(100).mul(adminFeeInBasisPoints)/100;
        
    } 

    // daily Fix interest
    function hourlyInterest (uint256 _val, uint256 _apr) public pure returns(uint256) {
           return _val.div(100).mul(_apr).div(365).div(24);
    } 
    ...
    function eightyPercent(uint256 _val) public pure returns (uint256){
      //  uint256 _eightyPercent = 8000; // 80 %
          return _val.mul(8000).div(10000); // 80% of cumulated interest (1467)
