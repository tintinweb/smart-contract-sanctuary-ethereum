...
contract LockBox is ILockBox, AccessHandler {
    ...
    function lockAmount(
        ...
        address token,
        uint256 amount
    ) external override onlyRole(LOCKBOX_ROLE) {
    ...
    function unlockAmount(
        ...
        address token,
        uint256 amount
    ) external override onlyRole(LOCKBOX_ROLE) returns (bool) {
        if (amount == 0)
            return false;
    ...
    function unlockAmountTo(
        ...
        console.log("Allowance (proxy):", amount.add(allowance).div(1 ether));
