...
contract Alph4SpaceDogs is ERC721, PullPayment, Ownable {
    ...
    uint256 public constant PRESALE_MINT_PRICE = 0.001 ether;
    uint256 public constant SALE_MINT_PRICE = 0.002 ether;
    ...
    mapping(address => uint256) public mintsWalletAddresses;

    constructor() ERC721("Alph4SpaceDogs", "ASPACE") {
        baseTokenURI = "";
        mintPrice = PRESALE_MINT_PRICE;
    }
    ...
    function mintTo(address recipient)
        ...
        returns (uint256)
    {
        if ( mintsWalletAddresses[msg.sender] + 1 > maxMintsPerWallet ) revert ExceededMaxPerWallet();
        ...
        mintsWalletAddresses[msg.sender] += 1;
        _safeMint(recipient, newItemId);
        return newItemId;
    }

    function mintWhitelist(uint256 _qty) external payable {
        if ( mintsWalletAddresses[msg.sender] + _qty > maxMintsPerWallet ) revert ExceededMaxPerWallet();
        ...
        mintsWalletAddresses[msg.sender] += _qty;
