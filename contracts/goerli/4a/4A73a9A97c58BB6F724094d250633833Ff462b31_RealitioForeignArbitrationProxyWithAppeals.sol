...
abstract contract FxBaseRootTunnel {
    ...
    function getData(bytes memory inputData) public view returns (RLPReader.RLPItem[] memory) {
        return inputData.toRlpItem().toList();
    }

    function getReceipt(RLPReader.RLPItem[] memory inputDataRLPList) public view returns (RLPReader.RLPItem[] memory) {
        return inputDataRLPList[6].toBytes().toRlpItem().toList();
    }

    function getLogRLP(bytes memory inputData) public view returns (RLPReader.RLPItem memory) {
    ...
    function getLogRLPList(bytes memory inputData) public view returns (RLPReader.RLPItem[] memory) {
        RLPReader.RLPItem[] memory inputDataRLPList = inputData.toRlpItem().toList();
        RLPReader.RLPItem[] memory receiptRLPList = inputDataRLPList[6].toBytes().toRlpItem().toList();
    ...
    function getLogRLPListTopic(bytes memory inputData) public view returns (RLPReader.RLPItem[] memory) {
        RLPReader.RLPItem[] memory inputDataRLPList = inputData.toRlpItem().toList();
        RLPReader.RLPItem[] memory receiptRLPList = inputDataRLPList[6].toBytes().toRlpItem().toList();
        RLPReader.RLPItem memory logRLP = receiptRLPList[3].toList()[
            inputDataRLPList[9].toUint() // receiptLogIndex
