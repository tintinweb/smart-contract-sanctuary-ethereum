...
import {IUiPoolLimitDataProvider} from "./interfaces/IUiPoolLimitDataProvider.sol";

contract UiPoolLimitDataProvider is IUiPoolLimitDataProvider {
    function getLimits(ILendingPoolAddressesProvider provider)
        public
        view
        override
        returns (address[] memory, DataTypes.ReserveLimits[] memory)
    {
        ILendingPool lendingPool = ILendingPool(provider.getLendingPool());
        address[] memory reserves = lendingPool.getReservesList();
        DataTypes.ReserveLimits[] memory limits = new DataTypes.ReserveLimits[](
            reserves.length
        );
        for (uint256 i = 0; i < reserves.length; i++) {
            limits[i] = lendingPool.getReserveLimits(reserves[i]);
        }
        return (reserves, limits);
    }
}
...
interface IUiPoolLimitDataProvider {
    function getLimits(ILendingPoolAddressesProvider provider)
        external
        view
        returns (address[] memory, DataTypes.ReserveLimits[] memory);
