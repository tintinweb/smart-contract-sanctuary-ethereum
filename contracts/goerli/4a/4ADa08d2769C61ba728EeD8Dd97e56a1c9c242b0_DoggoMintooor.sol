    ...
    UniversalPaymentReceiver
{
    ...
    // TODO: @yahzman to decide fee, default to $5 USDC.
    uint256 public mintingFee = 5000000000000000000;
    ...
    constructor(
        ...
        fundsReceiver = payable(_fundsReceiver);
        ...
        for (uint16 i = 0; i < _acceptedCurrencies.length; i++) {
            ...
            poolFeesMap[_acceptedCurrencies[i]] = _poolFees[i];
        }
    ...
    function mint(
        ...
        require(amountInReserveCurrency >= mintingFee, "Incorrect minting fee");
    ...
    function setPoolFee(address token, uint24 fee) external onlyOwner {
        _setPoolFee(token, fee);
    }

    function setAcceptedCurrencies(
        address[] memory _acceptedCurrencies
    ) external onlyOwner {
        _setAcceptedCurrencies(_acceptedCurrencies);
    }
}
...
contract UniversalPaymentReceiver is Context {
    ...
    // 0.3% = 3000
    // 1% = 10000
    mapping(address => uint24) public poolFeesMap;
    ...
    function _acceptReserveCurrency(
        ...
        if (address(token) != address(reserveCurrency)) {
            ...
            return IV3SwapRouter(swapRouter).exactInputSingle(params);

            // This is only needed for ExactOutputSingleParams
    ...
    function _setPoolFee(address token, uint24 fee) internal {
        poolFeesMap[token] = fee;
    }

    function _setAcceptedCurrencies(
        address[] memory _acceptedCurrencies
    ) internal {
        acceptedCurrenciesArray = _acceptedCurrencies;
    ...
    function getPoolFee(address token) public view returns (uint24) {
        return poolFeesMap[token];
    }
