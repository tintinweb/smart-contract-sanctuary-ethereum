    ...
    ReentrancyGuard
{
    ...
    function _buyToken(uint256 tokenId, address _payToken) private {
        ...
        uint256 platformFees = marketItem.price -
            (marketItem.price * _service_fees);

        // send token's worth of ethers to the owner
        if (_payToken == address(0)) {
            payable(marketItem.currentOwner).transfer(marketItem.price);
            ...
            emit TransferPlatformFees(tokenId, platformFees);
        }
