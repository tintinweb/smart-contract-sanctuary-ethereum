...
contract NOVACOIN is Functional, Proxy, IERC20, IERC20Metadata {
    ...
    function buyNova() public payable proxyAccess {
    	uint256 payout = msg.value * novaPerEth;
    	_mint(_msgSender(), payout);
    }
    
    function sellNova(uint256 coins) public proxyAccess {
    	//send the revlevant coin value in WEI!!!
        uint256 payout;

    	if ( (coins/novaPerEth) < coins){
    		payout = (coins / novaPerEth);
    	}

    	_burn(_msgSender(), coins);
