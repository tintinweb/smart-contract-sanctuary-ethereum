...
contract OToken is OTokenInterface, Exponential, TokenErrorReporter {
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        (mathErr, sroTokensNew) = subUInt(accountTokens[src], tokens);
        if (mathErr != MathError.NO_ERROR) {
...
contract OErc20 is OToken, OErc20Interface {
    ...
    /**
    * @notice Admin call to delegate the votes of the XCN-like underlying
    * @param xcnLikeDelegatee The address to delegate votes to
    * @dev OTokens whose underlying are not XcnLike should revert here
    */
    function _delegateXcnLikeTo(address xcnLikeDelegatee) external {
        require(msg.sender == admin, "only the admin may set the xcn-like delegate");
        XcnLike(underlying).delegate(xcnLikeDelegatee);
    }
}
