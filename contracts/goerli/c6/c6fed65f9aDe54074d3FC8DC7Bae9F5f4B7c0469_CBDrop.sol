    ...
    OperatorFiltererUpgradeable
{
    ...
    mapping(address => bool) public freeNftperWalletClaimed;
    ...
    function initialize() public initializer initializerERC721A {
        ...
        DEFAULT_OPERATOR_FILTER = address(0x3cc6CddA760b79bAfa08dF41ECFA224f810dCeB6);
        ...
        uriPrefix = "";
        uriSuffix = ".json";
        refundPeriod = 45 days;
        freeNftPerWallet = true;
        whitelistMintEnabled = false;
        setCost(0.01 ether);
        setWlCost(0.000001 ether);
        maxSupply = 3333;
    ...
    function refund(uint256[] calldata tokenIds) external nonReentrant whenNotPaused{
        require(refundGuaranteeActive(), "Refund expired");
        uint256 refundAmount = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(msg.sender == ownerOf(tokenId), "Not token owner");
            refundAmount = refundAmount + purchasePrice[tokenId];
    ...
    function whitelistMint(
        ...
        mintWlPriceCompliance(_mintAmount)
        whenNotPaused
        nonReentrant
    {
        ...
        nonReentrant
    {
        for (uint i = totalSupply(); i < (totalSupply() + _mintAmount); ++i) {
            ...
            purchasePrice[i + 1] = cost;
        }
        if (
            freeNftPerWallet &&
            !freeNftperWalletClaimed[msg.sender] &&
            _mintAmount + 1 <= maxSupply
        ) {
            _mintAmount = _mintAmount + 1;
            mintedAt[totalSupply() + _mintAmount] = block.timestamp;
            purchasePrice[totalSupply() + _mintAmount] = 0; // non refundable as it was free
            freeNftperWalletClaimed[msg.sender] = true;
        }
        _safeMint(_msgSender(), _mintAmount);
    }
