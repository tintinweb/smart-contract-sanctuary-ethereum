...
contract ApexConsumer is Initializable, ProxyOwned, ProxyPausable {
    ...
    function fulfillMatchup(
        ...
        if (raceFulfilledCreated[_eventId]) {
            ...
            if (race.startTime > block.timestamp) {
                ...
                newGameOdds.arePostQualifyingOddsFetched = race.qualifyingStartTime < block.timestamp;
    ...
    {
        return
            _isGameInPostQualifyingPhase(_gameId) && !gameOdds[_gameId].arePostQualifyingOddsFetched
                ? (0, 0, 0)
    ...
    function getNormalizedOdds(bytes32 _gameId) public view returns (uint[] memory) {
        ...
        for (uint i = 0; i < normalizedOdds.length; i++) {
            normalizedOdds[i] = _isGameInPostQualifyingPhase(_gameId) && !gameOdds[_gameId].arePostQualifyingOddsFetched
                ? 0
                : (1e18 * normalizedOdds[i]) / 1e4;
        }
    ...
    function _isGameInPostQualifyingPhase(bytes32 _gameId) internal view returns (bool) {
        GameCreate memory game = gameCreated[_gameId];
        RaceCreate memory race = raceCreated[game.raceId];

        return race.qualifyingStartTime < block.timestamp;
    }
