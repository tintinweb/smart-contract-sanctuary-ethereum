...
contract ProxyAuction is Ownable, ReentrancyGuard, IProxySale {
    ...
    mapping(uint256 => uint256) public auctions;
    uint256 public auctionFloorPrice;
    uint256 public auctionTotalRefundPrice;
    uint256 public auctionBalanceRefundPrice;
    ...
    constructor(address _signer, address _vault) public {
        signer = _signer;
        vault = _vault;
        auctionTotalRefundPrice = 0;
        auctionBalanceRefundPrice = 0;
    }
    ...
    function getAuctionRefundPrice()
    public
    nonReentrant
    eoaOnly {
        for (uint256 i = 317; i < erc721.totalSupply(); i++) {
            if(auctions[i] >0) {
               auctionBalanceRefundPrice = auctionBalanceRefundPrice + (auctions[i]- auctionFloorPrice);
            }
        }
        auctionTotalRefundPrice = auctionBalanceRefundPrice;
    }
    ...
    function _publicMint(string calldata _salt, bytes calldata _signature) internal
    {
        ...
        require(block.timestamp - auctionStartTime >= auctionPriceCurveLength + epochInterval, "Auction is Over");
        ...
        auctions[erc721.totalSupply()] = currentPrice;

        if(auctionFloorPrice > currentPrice) {
            auctionFloorPrice = currentPrice;
        }
    ...
    function refund(uint256 _tokenId)
    public
    payable
    nonReentrant
    eoaOnly
    {
        require(auctionTotalRefundPrice > 0, "Auction refund not started");

        require(auctionBalanceRefundPrice > 0, "Auction refund is over");

        require(auctions[_tokenId] > 0, "NFT did not participate in the auction");

        require(auctions[_tokenId] - auctionFloorPrice > 0, "The refund amount is less than 0");

        require(erc721.ownerOf(_tokenId) == msg.sender, "You are not the owner of the NFT");

        require(auctions[_tokenId] - auctionFloorPrice > address(this).balance, "You do not have enough ETH to refund, please notify the project team");

        payable(msg.sender).transfer(auctions[_tokenId] - auctionFloorPrice);

        auctionBalanceRefundPrice = auctionBalanceRefundPrice - (auctions[_tokenId] - auctionFloorPrice);
    ...
    /***
     * This method is implemented to prevent refund calculation errors
     * Refunds are endorsed by the project party's credit
     */
    function setAuctionRefundPrice(uint256 _auctionTotalRefundPrice, uint256 _auctionBalanceRefundPrice) external onlyOwner {
        auctionTotalRefundPrice = _auctionTotalRefundPrice;
        auctionBalanceRefundPrice = _auctionBalanceRefundPrice;
    }

    function withdraw() public onlyOwner {
        uint256 balance = address(this).balance;
        require(auctionTotalRefundPrice > 0, "Refund not yet enabled");
        payable(vault).transfer(balance - auctionBalanceRefundPrice);
    }
}
