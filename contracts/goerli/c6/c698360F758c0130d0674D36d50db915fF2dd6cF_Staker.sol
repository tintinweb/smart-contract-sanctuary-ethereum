...
contract Staker {
  ...
  if(requireDeadlinePassed){
    require(timeRemaining<=0,"Deadline not yet met");
  }else{
    require(timeRemaining>0,"Deadline already met");
  }
  _;
  }
  modifier notCompleted(){
    bool externalStatus = exampleExternalContract.completed();
    require(!externalStatus,"contract is complete");
    _;
  }
  ...
  function withdraw() public deadlinePassed(true) notCompleted {
    ...
    require(valueToSend>0,"no money to withdraw");
    balances[msg.sender] = 0;
    (bool sent,) = payable(msg.sender).call{value:valueToSend}("");
    require(sent,"something happen");
  }
