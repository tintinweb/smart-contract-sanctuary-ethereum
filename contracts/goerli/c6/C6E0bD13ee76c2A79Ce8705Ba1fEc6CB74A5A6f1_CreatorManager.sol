...
contract CreatorContract is ERC721Holder, Ownable {
    ...
    function onERC721Received(address, address, uint256, bytes memory data) public virtual override returns (bytes4) {
        ...
        if(!EnumerableSet.contains(myPoolAddresses, tokenAddress)){
...
contract CreatorManager {
    ...
    function createACreatorForSelf() public returns (address) {
        address user = msg.sender;

        require(address(creatorAddress[user]) == address(0), "Already exists");
    ...
    function getMyCreatorAddress() public view returns (address) {
        return address(creatorAddress[msg.sender]);
    }
    ...
    // function setStakingPoolManager(address _stakingPoolManager) public {
