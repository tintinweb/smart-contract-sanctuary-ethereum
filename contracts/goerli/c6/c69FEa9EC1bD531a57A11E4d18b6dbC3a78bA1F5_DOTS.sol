 ...
 * @author Jack Chuma
 */
contract DOTS is ERC721AQueryable, AccessControl, VRFConsumerBaseV2 {
    
    bytes32 constant USER_ROLE = keccak256("USER_ROLE");
    uint8 constant NUM_BACKGROUND = 9;
    uint8 constant NUM_BACK = 12;
    uint8 constant NUM_PAINTJOB = 40;
    uint8 constant NUM_EYEWEAR = 30;
    uint8 constant NUM_MOUTHGEAR = 30;
    uint8 constant NUM_HEADGEAR = 40;
    uint8 constant NUM_CLOTHING = 40;
    uint8 constant NUM_ACCESSORIES = 5;
    uint16 constant BIT_MASK = 65535;

    string uri;
    IMintPassTwo mintPassTwoContract;
    bool locked;

    mapping(uint256 => Evo) public evoData; // evo # => evo info
    mapping(uint256 => Metadata) public metadata; // tokenId => Metadata
    mapping(uint256 => bool) randomEntropies;
    ...
    constructor(
        ...
        if (_mintPassTwo == address(0)) revert ZeroAddress();
     ...
     */
    function mint(uint256 _evoNum, uint32 _numDots) external {
        ...
        uint256 lastToken;
        unchecked {
            lastToken = nextToken + _numDots;
        }

        for (uint i=nextToken; i<=lastToken; ) {

            metadata[i] = Metadata(_evoNum, 0);
        ...
        _mint(msg.sender, _numDots);

        emit DOTsMinted(_evoNum, _numDots, _info.numTokensNeeded);

        sendVRFRequests(_numDots, nextToken);
    }
    ...
    function sendVRFRequests (
        ...
        while (_tokenAmount > 0) {
            ...
            vrfRequests[s_requestId] = VRFRequest({
                minter: msg.sender,
                amount: uint128(_tokenAmount),
                fromTokenId: uint128(_fromTokenId)
            });
        }
    ...
    function fulfillRandomWords(
        ...
        for(uint256 i; i < randomWords.length;) {
            ...
            uint256 _genes = _generateTraits(randomWords[i]);
            metadata[vrfRequest.fromTokenId + i].genes = _genes;
    ...
    function upgrade(
        ...
        metadata[_primaryTokenId].evoNum = _targetEvoNum;
    ...
    /**
     * @notice Called by contract owner to update stored data for a batch of EVO stages
    ...
    /**
     * @notice Called by contract owner to set a new base URI for DOTS
     */
    function setURI(string memory _uri) external onlyRole(USER_ROLE) {
        uri = _uri;
    ...
    function supportsInterface(bytes4 interfaceId) public pure override(ERC721A, IERC721A, AccessControl) returns (bool) {
        return interfaceId == type(IAccessControl).interfaceId || 
               ...
               interfaceId == 0x80ac58cd || // EIP-165 interface ID for ERC721.
               interfaceId == 0x5b5e139f; // EIP-165 interface ID for ERC721Metadata.       
    }
    ...
    function _normalize(
        ...
        uint256 _backgroundId = _getId(_rand, uint256(16), uint256(65529), NUM_BACKGROUND);
        uint256 _backId = _getId(_rand, uint256(32), uint256(65532), NUM_BACK);
        uint256 _paintId = _getId(_rand, uint256(48), uint256(65520), NUM_PAINTJOB);
        uint256 _eyewearId = _getId(_rand, uint256(64), uint256(65520), NUM_EYEWEAR);
        uint256 _mouthgearId = _getId(_rand, uint256(80), uint256(65520), NUM_MOUTHGEAR);
        uint256 _headgearId = _getId(_rand, uint256(96), uint256(65520), NUM_HEADGEAR);
        uint256 _clothingId = _getId(_rand, uint256(112), uint256(65520), NUM_CLOTHING);
        uint256 _accessoryId = _getId(_rand, uint256(128), uint256(65535), NUM_ACCESSORIES);

        if (_traitNumSelector < 1638) {
            _mouthgearId = 0;
            _headgearId = 0;
            _clothingId = 0;
            _accessoryId = 0;
        } else if (_traitNumSelector < 39321) {
            _headgearId = 0;
            _mouthgearId = 0;
            _accessoryId = 0;
        } else if (_traitNumSelector < 57343) {
            if (_traitNumSelector < 46530) {
                _mouthgearId = 0;
                _accessoryId = 0;
            } else if (_traitNumSelector < 56442) {
                _headgearId = 0;
                _accessoryId = 0;
            } else {
                _headgearId = 0;
        ...
        } else if (_traitNumSelector < 63897) {
            if (_traitNumSelector < 62088) _accessoryId = 0;
            else if (_traitNumSelector < 63289) _mouthgearId = 0;
            else _headgearId = 0;
        }
    ...
    function _burnDots(
        ...
        for (uint i=0; i<_tokenIdsToBurn.length; ) {
            ...
            unchecked { _value += evoData[metadata[_tokenId].evoNum].numTokensNeeded; }
    ...
    function _burnDotsUpgradeMultiple(
        ...
        for (uint i=0; i<_tokenIdsToBurn.length; ) {
            ...
            unchecked { _value += evoData[_evoNum].numTokensNeeded; }
            _burn(_tokenId, false);
            delete metadata[_tokenId];
            unchecked { i++; }
        }
    ...
    function _generateTraits(uint256 _rand) private view returns (uint256 _genes) {
        uint256 _nonce;
    ...
    function _checkMintWindow(uint256 _start, uint256 _end) private view {
        if (block.timestamp < _start || block.timestamp > _end) revert MintWindowClosed();
    }
}
