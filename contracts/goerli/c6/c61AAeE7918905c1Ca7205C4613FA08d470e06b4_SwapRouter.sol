...
contract SwapRouter {
    ...
    function removeLiquidity(
        ...
        address pair = pairFor(tokenA, tokenB);
        SwapPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ...
                : _to;
            SwapPair(pairFor(input, output)).swap(amount0Out, amount1Out, to);
        }
    }
        ...
        returns (address pair)
    {
        ...
        pair = SwapFactory(factory).getPairAddress(token0, token1);
    }
    ...
    {
        (address token0, ) = sortTokens(tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1, ) = SwapPair(
    ...
    function getAmountOut(
        ...
        uint256 ao = reserveOut - (k / (reserveIn + amountIn));
        amountOut = ((ao * 997) / 1000) - 1;
    }
    ...
    function getAmountIn(
        ...
        require(reserveIn > 0 && reserveOut > 0, "INSUFFICIENT_LIQUIDITY");
        uint256 k = reserveIn * reserveOut;
        uint256 ai = (k / (reserveOut - amountOut)) - reserveIn;
        amountIn = ((ai * 1000) / 997) + 1;
    }
...
contract SwapPair is ERC20 {
    ...
    constructor() ERC20("SwapERC20", "ST") {
        factory = msg.sender;
    }
        ...
        returns (bool feeOn)
    {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint256 newRootK = uint256(Math.sqrt(_reserve0 * _reserve1));
                uint256 oldRootK = uint256(Math.sqrt(_kLast));

                if (newRootK < oldRootK) {
                    uint256 numerator = totalSupply() * (newRootK - oldRootK);
                    uint256 denominator = 5 * newRootK + oldRootK;
                    uint256 liquidity = uint256(numerator / denominator);
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            require(
                ...
                "liquidity must be at least 1000"
            );
        } else {
            liquidity = Math.min(
...
contract SwapFactory {
    ...
    function getPairAddress(address token0, address token1) external view returns (address pair) {
