...
abstract contract PolemosPoolBase is IPoolBase, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  /// @dev total yield reward generated
  uint256 private totalYieldReward;
  ...
  /**
   * @dev list of whitelisted users who can `swap` PLMS <--> tPLMS
   */
  mapping(address => bool) plmsDepositWhitelist;

  /**
   * @dev total PLMS staking amount
   */
  uint256 public totalPlmsStakingAmount;

  /**
   * @dev total swapped PLMS amount
   */
  ...
  function _unstakeFromPool(
    ...
    require(stakeDeposit.isPLMS == _isPLMS, 'type not match');
  ...
  function getTotalYieldReward() external view returns (uint256) {
    if (lastYieldDistribution >= endBlock) {
    ...
    return totalYieldReward + tPLMSReward;
  }
