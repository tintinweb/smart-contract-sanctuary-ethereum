...
contract IP3 {
    ...
    function purchaseByAmount(
        ...
        uint256 latestStartPriceByCount = authorizedNFTSubmit.authorizedByCountInfo.latestStartPrice;

        if ((nftOnRecord.contains(hashedNft)) && latestStartPriceByCount !=0 ) {
    ...
    function purchaseByDuration(
        ...
        uint256 latestStartPriceByDuration = authorizedNFTSubmit.authorizedByDurationInfo.latestStartPrice;
        if (nftOnRecord.contains(hashedNft) && latestStartPriceByDuration !=0) {
            // get the latest start price from record on chain
            currentStartPrice = getCurrentStartingPricePerNFT(hashedNft, isByDuration);
        } else {
            // new NFT to be licenced
