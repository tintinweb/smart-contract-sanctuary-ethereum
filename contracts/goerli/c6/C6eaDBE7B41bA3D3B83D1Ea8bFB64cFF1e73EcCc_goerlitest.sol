...
// File: contracts/openzeppelin-contracts/contracts/token/ERC721/extensions/ERC721URIStorage.sol
...
abstract contract NFTOasis is AccessControl, ERC721URIStorage, Ownable {
    using Strings for uint256;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory tokenid = super.tokenURI(tokenId);
        string memory filename = ".json";
        bytes memory strTokenId = bytes(tokenid);
        bytes memory strFilename = bytes(filename);
        bytes memory str = new bytes(strTokenId.length + strFilename.length);
        uint8 point = 0;
        for (uint8 j = 0; j < strTokenId.length; j++) {
            str[point] = strTokenId[j];
            point++;
        }
        for (uint8 k = 0; k < strFilename.length; k++) {
            str[point] = strFilename[k];
            point++;
        }
        return string(str);
    }
...
contract goerlitest is NFTOasis {
  constructor() NFTOasis("goerli-test", "goerli-test") {}
}