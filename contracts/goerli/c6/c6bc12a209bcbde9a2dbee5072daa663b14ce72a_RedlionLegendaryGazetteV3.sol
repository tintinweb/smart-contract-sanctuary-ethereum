...
import '../libraries/interfaces/IRedlionLegendaryGazetteV2.sol';

/**
 * @title RedlionLegendaryGazette
 * @dev This contract allows the creation and management of auctions for rare issues of Redlion Legendary Gazette.
 * @dev Bidders can place bids on auctions and the contract will automatically keep track of the current highest bid.
 * @dev The owner of the contract can force end an auction and the winning bidder will be awarded the NFT.
 */
contract RedlionLegendaryGazetteV3 is
  Initializable,
  IRedlionLegendaryGazetteV2,
  ERC721Upgradeable,
  AccessControlUpgradeable
{
  // Struct to represent a bid

  uint public MINIMUM_OUTBID; // 1000 => 10.00%
  uint public MAX_AUCTION_TIME; // seconds
  uint public MIN_AUCTION_TIME; // seconds
  uint public ENTRY_FEE;

  mapping(uint => mapping(address => BidderIndex)) biddersIndex; // Issue ID to bidders index mapping
  ...
  // Mapping from issue ID to issue data
  mapping(uint => Issue) public issues;

  uint ongoing;

  // Role required to start an auction

  /**
   * Launches a new auction for the given issue with the given name and duration
  ...
  function launchAuction(
    ...
    uint256 _reserve,
    string memory _uri
  ) public override(IRedlionLegendaryGazetteV2) onlyRole(MANAGER_ROLE) {
    require(!isAuctionLaunched(_issue), 'ALREADY_EXISTENT_AUCTION');
    // Set the start and end times for the auction
    uint startTime = block.timestamp;
    // Create a new issue with the given data and add it to the mapping
    ...
    // Emit an event to announce the new issue
    emit AuctionLaunched(_issue, startTime);
  }

  /**
   * @dev Places a bid on the given auction.
   * @param _issue ID of the auction to bid on.
   */
  function placeBid(uint _issue) external payable {
    ...
    if (!biddersIndex[_issue][msg.sender].set) {
       ...
       * in any case, we'll keep the winners fee in case we have to force claim.
       */
    ...
    // Record the bid and update the winning bid and winner if necessary
    Bid memory bid = Bid({
    ...
    if (!biddersIndex[_issue][msg.sender].set) {
      uint newIndex = issue.bids.length;
      biddersIndex[_issue][msg.sender].index = newIndex;
      biddersIndex[_issue][msg.sender].set = true;
      issue.bids.push(bid);
    } else {
      // We don't want to change the initial fee in case it was changed mid auction
      uint bIndex = biddersIndex[_issue][msg.sender].index;
      issue.bids[bIndex].value = bid.value;
      issue.bids[bIndex].time = bid.time;
    }
    if(issue.bidHistory.length == 0) {
      ongoing++;
    }
    issue.bidHistory.push(bid);
    uint timeLimit = MAX_AUCTION_TIME / (2 ** issue.bidHistory.length - 1);
    if (timeLimit < MIN_AUCTION_TIME) timeLimit = MIN_AUCTION_TIME;
    issue.endTime = bid.time + timeLimit;
  ...
  function getIssue(uint _issue) public view returns (Issue memory) {
    Issue storage issue = issues[_issue];
    return issue;
  }
  ...
  function forceClaim(uint _issue) public onlyRole(OWNER_ROLE) {
    _claim(_issue, false);
  }
...
@dev This function allows the winning bidder of an auction to claim their NFT.
@param _issue ID of the auction to claim
*/
  function claim(uint _issue) public override(IRedlionLegendaryGazetteV2) {
    // Ensure the caller is the winning bidder
    _requireWinningBidder(_issue, msg.sender);
    _claim(_issue, true);
  }

  function winningBid(uint _issue) public view returns (Bid memory) {
    uint winningValue = 0;
    uint index = 0;
    Issue storage issue = issues[_issue];
    if (issue.bids.length == 0) return Bid(0, 0, address(0), 0);
    for (uint i = 0; i < issue.bids.length; i++) {
  ...
  function isAuctionLaunched(
    uint _issue
  ) public view override(IRedlionLegendaryGazetteV2) returns (bool) {
    return getIssue(_issue).startTime != 0;
  }

  function minimumOutbid(uint _issue) public view returns (uint) {
  ...
  /*///////////////////////////////////////////////////////////////
                          INTERNAL FUNCTIONS
  ///////////////////////////////////////////////////////////////*/
  function _claim(uint _issue, bool _fee) internal {
  ...
  function _requireAuctionExists(uint _issue) internal view {
    require(isAuctionLaunched(_issue), 'NONEXISTENT_AUCTION');
  }

  function _requireAuctionNotEnded(uint _issue) internal view {
    _requireAuctionExists(_issue);
    require(
  ...
  function _requireAuctionEnded(uint _issue) internal view {
    _requireAuctionExists(_issue);
  ...
  /**
  @dev This internal function checks if the given address is the winning bidder for the given auction.
  @param _issue ID of the auction
  @param _bidder Address to check
  */
  function _requireWinningBidder(uint _issue, address _bidder) internal view {
    // Get the issue data
    Issue memory issue = issues[_issue];
    // Ensure the auction has bids
...
interface IRedlionLegendaryGazetteV2 is IERC721Upgradeable {
  /**
