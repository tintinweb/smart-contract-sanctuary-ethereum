    ...
    NativeMetaTransaction
{
    ...
    mapping(uint256 => uint256) mapDisplayToken;
    mapping(uint256 => uint256) mapRealToken;

    constructor() ERC721A("UGC-BAYC", "UGC-BAYC") {
        _initializeEIP712("UGC-BAYC");
        baseTokenURI = "https://ugc-nft.io/moment-nft/metadata/bayc/"; 
        collectionURI = "https://ugc-nft.io/moment-nft/metadata/bayc/contract.json";
    ...
    function getDisplayToken(uint256[] calldata realTokens)
        external
        view
        returns (uint256[] memory displayTokens)
    {
        displayTokens = new uint256[](realTokens.length);
        for (uint256 i = 0; i < realTokens.length; i++) {
            displayTokens[i] = mapDisplayToken[realTokens[i]];
        }

        return displayTokens;
    }

    function getRealToken(uint256[] calldata displayTokens)
        external
        view
        returns (uint256[] memory realTokens)
    {
        realTokens = new uint256[](displayTokens.length);
        for (uint256 i = 0; i < displayTokens.length; i++) {
            realTokens[i] = mapRealToken[displayTokens[i]];
        }
        ...
        return
            bytes(baseURI).length != 0
                ? string(
                    abi.encodePacked(
                        baseURI,
                        _toString(mapDisplayToken[tokenId])
                    )
                )
                : "";
    }

    function mintTo(address to, uint256 displayToken)
        public
        onlyRole(MINER_ROLE)
    {
        require(mapRealToken[displayToken] == 0, "Already minted");

        _mint(to, 1);

        uint256 realToken = _nextTokenId() - 1;
        mapDisplayToken[realToken] = displayToken;
        mapRealToken[displayToken] = realToken;
    }
