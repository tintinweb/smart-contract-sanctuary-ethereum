...
contract Csr {
    ...
    string internal constant s1 =
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 500"><rect y="0" x="0" width="500" height="500" fill="#';
    string internal constant s2 = "</svg>";
    string internal constant s3 = '"/>';

    uint public constant TOKEN_LIMIT = 512; // nfts limit
    ...
    function minMax(
        ...
        return
            uint(
                uint(
                    keccak256(
                        abi.encodePacked(block.timestamp, msg.sender, _seed)
                    )
                ) % uint(_b - _a + 1)
            ) + _a;
    }
    ...
    function createCircle(
        ...
        return
            string.concat(
                ...
                ' fill="',
                (bytes(fill).length > 0 ? string.concat("#", fill) : "none"),
                s3
            );
    }
    ...
    function createFigure(
        ...
        if (_figure >= 0 && _figure <= 35) {
            ...
            out = string.concat(
                out,
                createRectangle(_cx, _cy, _height, _width, color)
            );
            ...
            for (uint i = 0; i < 3; i++) {
                out = string.concat(
                    out,
                    createCircle(_cx, _cy, _width, "", color)
                );
                _width -= 15;
            }
        }
    ...
    function drawByKind(uint _tokenId) internal view returns (string memory) {
        ...
        for (uint i = 0; i < nfts[_tokenId].len; i++) {
            ...
            if (_generate) {
                ...
                width = (figure > 95)
                    ? minMax(cx * cy * 2, 65, 100)
                    : minMax(cx * cy * 19 * i, 5, 100);
                height = (figure > 55 && figure <= 75)
                    ? minMax(cx * cy * 512, 45, 100)
                    : width;
            }
            if (
                bytes(nfts[_tokenId].elements[i].color).length == 0 || _generate
            ) {
                color = colors[
                    minMax(block.timestamp * i, 0, colors.length - 1)
                ];
            } else {
            ...
            out = string.concat(
                out,
                createFigure(figure, cx, cy, width, height, color)
            );
        }
    ...
    function draw(uint _tokenId, uint8 _kind) internal returns (string memory) {
        ...
        string memory out;
        string memory color;
        string memory backgroundColor = background[
            minMax(block.timestamp, 0, background.length - 1)
        ];
    ...
    function _mint(address _to) internal returns (string memory) {
        ...
        // simple nfts check
        if (kind == 0 && msg.value != 30000000000000000) {
            revert();
        }
        // can generate nfts by type
        require(
            nftsQty[kind] >= nftsQtyCurrent[kind] + 1,
            "no nfts by this type."
        );
        uint id = numTokens + 1;
    ...
    {
        string memory uri = drawByKind(_tokenId);
        return
            string.concat(
                'data:application/json;ascii,{"name": "CSR #',
                uint2str(_tokenId),
                '","image":"data:image/svg+xml;base64,',
                Base64.encode(bytes(uri)),
                '","seller_fee_basis_points": 1000,"fee_recipient": "0xe4dd4725Ed4634A130977CB12De23366E6eCe1f8"}'
            );
    }
}