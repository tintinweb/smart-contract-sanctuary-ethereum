...
contract OnChainAlpha is ERC721A, ReentrancyGuard, Ownable {
    ...
    constructor() ERC721A("On-Chain Alpha", "OCA") {
        TIERS[0] = [7707,1,1,1,1,1,1,1]; // 1 of 1
        TIERS[1] = [0,7627,42,55,33,20]; //light saber
        TIERS[2] = [0,6677,215,175,166,150,144,125,75,50]; //mouth special
        TIERS[3] = [0,2822,400,375,355,344,330,320,310,300,266,250,240,230,225,220,200,190,150,100,75,50,25]; //headwear
        TIERS[4] = [0,3877,1000,700,600,500,400,350,250,100]; //eyewear
        TIERS[5] = [0,2133,800,700,600,500,475,469,440,430,420,410,300,100]; //eyes
        TIERS[6] = [0,3860,777,555,350,330,310,272,269,250,234,220,200,150]; //mouth
        TIERS[7] = [0,5560,777,667,325,227,221]; //outerwear
        TIERS[8] = [0,5097,722,690,490,410,368]; //ears
        TIERS[9] = [0,0,2877,1500,1000,900,700,600,200]; //body ring
        TIERS[10] = [0,0,2377,1500,1000,900,700,600,400,200,100]; //body
        TIERS[11] = [0,0,2377,1500,1000,900,700,600,400,200,100]; //background
        ...
        randomSeedData = entropyForExtraData();
        ...
        for (uint i = 0; i < NUM_LAYERS; i++) {
            ...
            if(_hideTraits[_tokenId][i]==true){
                ...
                if (modifiedLayers[i] == false) {
                    ...
                    uint256 _randinput;
                    if (tokenExtraData == 0) {
                        _randinput = ((_tokenId + i + traitSeed) * PRIME_NUMBERS[i]) % maxSupply;
                    } else {
                        _randinput = uint256(
                            keccak256(
                                abi.encodePacked(
            ...
            if (hash[0] > 0)
            {
                for(uint j = 1; j < hash.length - 1; ++j)
                {
                    hash[j] = 0;
                }
            }
