...
contract SignalSuscriptionModule is ModuleBase, ReentrancyGuard {
    ...
    function initialize(address _setToken)
        external
        onlyValidAndPendingSet(ISetToken(_setToken))
        onlySetManager(ISetToken(_setToken), msg.sender)
    {
        ISetToken(_setToken).initializeModule();
    }
    ...
    function subscribe(address _setToken, address target)
        external
        nonReentrant
        onlyManagerAndValidSet(ISetToken(_setToken))
    {
        require(allowed_Copytrading[target], "Unable to subscribe this fund");
        require(!isFollowing[_setToken], "Signal has been already subscribed");
        followers[target].push(_setToken);
        isFollowing[_setToken] = true;
        Signal_provider[_setToken] = target;
    }

    function udpate_allowedCopytrading(address _setToken, bool can_copy_trading)
        external
        onlyManagerAndValidSet(ISetToken(_setToken))
    {
        allowed_Copytrading[_setToken] = can_copy_trading;
    }

    function unsubscribe(address _setToken, address target)
        external
        nonReentrant
        onlyManagerAndValidSet(ISetToken(_setToken))
    {
        require(isFollowing[_setToken], "Signal has not been subscribed");
        followers[target].removeStorage(_setToken);
        isFollowing[_setToken] = false;
        delete Signal_provider[_setToken];
    }
    ...
    function get_signal_provider(address _setToken)
        external
        view
        returns (address)
    {
        return Signal_provider[_setToken];
    }
}
