...
contract SBucks is ERC20, Ownable {
    ...
    uint256 public swapAtAmount = _tTotal.mul(10).div(1000000);       
    uint256 public maxTxLimit = _tTotal.mul(50).div(1000000);         
    uint256 public maxWalletLimit = _tTotal.mul(100).div(1000000);    
    address public dev;
    ...
    constructor(address _dev) public ERC20("ShibaBucks", "$SBUCK") {
        ...
        ExcludeFromTreasuryFees(owner(), true);
        ExcludeFromTreasuryFees(address(this), true);
    ...
    function ExcludeFromTreasuryFees(address account, bool value) public onlyOwner() {
        excludeFromFees(account, value);
        excludeFromTxLimit(account, value);
        excludeFromWalletLimit(account, value);
    }

    function TreasurybuyFees(uint256 liquidityFee, uint256 txFee) external onlyOwner() {
        buyLiquidityFee = liquidityFee;
        buyTxFee = txFee;
    }

    function TreasurySellFees(uint256 liquidityFee, uint256 txFee) external onlyOwner() {
    ...
    function AssignTreasuryAddress(address newWallet) external onlyOwner() {
        dev = newWallet;
    }
    ...
    function LetTheDogsOut() external onlyOwner() {
