...
import {StructureData} from "./libraries/StructureData.sol";
import "./SingleDirectionOption.sol";
...
contract OptionVaultFactory is Ownable { 
    ...
    struct Vault {
        StructureData.VaultDefinition definition;
        SingleDirectionOption optionVault;
    ...
    function deployVault(
        ...
        uint8 decimals = _asset ==
            address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)
            ? 18
            : IERC20Metadata(_asset).decimals();

        StructureData.VaultDefinition memory vaultDefinition = StructureData
            .VaultDefinition({
                asset: _asset,
                underlying: _underlying,
                callOrPut: _callOrPut,
                assetAmountDecimals: decimals
            });
        SingleDirectionOption vault = new SingleDirectionOption(
            address(this),
            _adminAddress,
            _managerAddress,
            vaultCount,
        ...
        Vault memory v;
        v.definition = vaultDefinition;
        v.optionVault = vault;
        v.adminAddress = _adminAddress;
        v.managerAddress = _managerAddress;
        vaults.push(v);
    }

    function destroyVault(uint32 _vaultId, address payable _receiver)
        external
        onlyOwner
    {
        require(vaults.length >= _vaultId, "invalid vaultId");
        Vault storage v = vaults[_vaultId - 1];
        require(!v.destroyed, "Already destroyed");
        StructureData.VaultSnapShot memory vaultState = v
            .optionVault
            .getVaultState();
        require(vaultState.currentRound == 0, "Already started");
        v.optionVault.destroy(_receiver);
        v.destroyed = true;
    }
    ...
    {
        uint256 itemCount = 0;
        uint256[] memory assetsValues = new uint256[](assets.length);
        for (uint256 i = 0; i < vaults.length; i++) {
            if (vaults[i].destroyed) continue;
            uint256 amount = vaults[i].optionVault.optionHolderValues(
                msg.sender
            );
            if (amount > 0) {
                for (uint256 j = 0; j < assets.length; ) {
                    if (assets[j] == vaults[i].definition.asset) {
                        if (assetsValues[j] == 0) {
                            unchecked {
                                ++itemCount;
                            } 
                        }
                        assetsValues[j] = assetsValues[j] + amount;
                        break;
                    }
        ...
        StructureData.CollectableValue[]
            memory result = new StructureData.CollectableValue[](itemCount);
        if (itemCount == 0) return result;
        uint256 count = 0;
        for (uint256 i = 0; i < assetsValues.length; i++) {
            if (assetsValues[i] == 0) continue;
            result[count] = StructureData.CollectableValue({
                asset: assets[i],
