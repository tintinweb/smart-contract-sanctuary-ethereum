...
contract SubscriptionApp {
    ...
    function batchProcessPayment(uint256[] memory _orderIds, address[] memory _customers) public {
        ...
        for(uint256 i=0; i< _orderIds.length; i++){
            try SubscriptionApp(this).processPayment(_orderIds[i], _customers[i]
            ) {
                emit SuccessfulPay(_orderIds[i], _customers[i]);
            } catch Error(string memory revertReason) {
                emit PaymentFailure(revertReason, _orderIds[i], _customers[i]);
            } catch (bytes memory returnData) {
                emit PaymentFailureBytes(returnData, _orderIds[i], _customers[i]);
            }
        }
    }

    function processPayment(uint256 _orderId, address _customer) public {
        Order storage order = orders[_orderId];
        require(!order.paused, "Cannot process, this order is paused");
        ...
        require(IERC20(order.erc20).allowance(_customer, address(this)) >= howMuchERC20ToSend, "Insufficient erc20 allowance");
        require(IERC20(order.erc20).balanceOf(_customer) >= howMuchERC20ToSend, "Insufficient balance");
