...
contract VRFBeaconOCR is VRFBeaconReport {
    ...
    function setConfig(
        ...
        if (signersLength > MAX_NUM_ORACLES) revert TooManyOracles();
        if (signersLength != transmitters.length || signersLength <= 3 * f)
            revert IncorrectNumberOfSigners();
        _requirePositiveF(f);
        ...
        for (uint256 i = 0; i < signersLength; i++) {
            if (s_signers[args.signers[i]].active) revert RepeatedSigner();
            s_signers[args.signers[i]] = Signer({
                active: true,
                index: uint8(i)
            });
            if (s_transmitters[args.transmitters[i]].active)
                revert RepeatedTransmitter();
    ...
    function transmit(
        ...
        if (s_configInfo.latestConfigDigest != reportContext[0])
    ...
    function _requireExpectedMsgDataLength(
        ...
        uint256 expected = TRANSMIT_MSGDATA_CONSTANT_LENGTH_COMPONENT +
            ...
            0; // placeholder
        if (msg.data.length != expected) revert CalldataLengthMismatch();
    }
    ...
    function setPayees(
        ...
        if (transmitters.length != payees.length)
            revert IncorrectNumberOfPayees();
    ...
    function transferPayeeship(address transmitter, address proposed) external {
        if (msg.sender != s_payees[transmitter]) revert OnlyCurrentPayee();
        if (msg.sender == proposed) revert InvalidPayee();
    ...
    function _payOracles() internal {
        unchecked {
            LinkTokenInterface linkToken = i_link;
            uint32 latestAggregatorRoundId = s_hotVars.latestAggregatorRoundId;
        ...
        validateOwnerOrBillingAdmin
    {
        ...
        uint256 linkBalance = i_coordinator.getTotalLinkBalance();
        if (linkBalance < linkDue) revert InsufficientBalance();
        ...
        returns (int256 availableBalance)
    {
        ...
        int256 balance = int256(i_coordinator.getTotalLinkBalance());
    ...
    function _transmitterGasCostWei(
        ...
        // a wrong reward than risk a liveness failure due to a revert.
        unchecked {
            if (initialGas < leftGas) revert LeftGasExceedsInitialGas(); /* XXX: Failing on this line? */
...
contract VRFBeaconDKGClient is DKGClient {
    DKG private s_keyProvider;
    bytes32 private s_keyID;
    bytes32 private s_provingKeyHash;
    ...
    function getKeyID() external view returns (bytes32) {
        return s_keyID;
    }

    function getProvingKeyHash() external view returns (bytes32) {
        return s_provingKeyHash;
    }
...
contract VRFBeaconReport is VRFBeaconTypes, OwnerIsCreator {
    ...
    IVRFCoordinatorProducerAPI internal immutable i_coordinator;
    ...
    function getCoordinator()
        external
        view
        returns (IVRFCoordinatorProducerAPI)
    {
...
contract DKG is KeyDataStruct, OCR2Abstract, OwnerIsCreator {
    ...
    function _afterSetConfig(
        ...
        bytes32 keyID;
        bytes32 zero;
        if (_onchainConfig.length != 32) revert InvalidOnchainConfigLength();
        assembly {
            keyID := mload(add(_onchainConfig, 0x20))
        }
        if (keyID == zero) revert KeyCopyFailed();
    ...
    uint32 private s_epochOfLastReport; // epoch at the time of the last-reported distributed key
    ...
    function setConfig(
        ...
        if (_signers.length > MAX_NUM_ORACLES) revert TooManyOracles();
        if (
            _signers.length != _transmitters.length || _signers.length <= 3 * _f
        ) revert IncorrectNumberOfSigners();
        if (_f == 0) revert IncorrectNumberOfFaultyOracles();
    ...
    function transmit(
        ...
        assert(initialGas < MAX_UINT_32);
        _payTransmitter(uint32(initialGas), msg.sender);
    }

    error InvalidTransmitters();
    error InvalidSigner();
    ...
    function _requireValidSignatures(
...

        {
            ...
            for (uint256 i = 0; i < rs.length; i++) {
                ...
                o = s_oracles[signer];
                if (o.role != Role.Signer) revert InvalidSigner();
                if (signed[o.index]) revert NonUniqueSignature();
                signed[o.index] = true;
            }
        }
    }
