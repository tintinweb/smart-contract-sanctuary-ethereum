...
contract NFTAirdrop is Pausable {
  ...
  uint public constant INVERSE_BASIS_POINT2 = 10000;
    ...
    if(sell.feeRecipient != address(0)) {
      if(sell.feeMethod == FeeMethodNM.SplitFee) {
        if(sell.makerRelayerFee > 0) {
          makerRelayerFee = SafeMath.div(SafeMath.mul(sell.makerRelayerFee, price), INVERSE_BASIS_POINT2);
          if(sell.paymentToken == address(0)) {
            receiveAmount1 = SafeMath.sub(receiveAmount, makerRelayerFee);
          }
        }
        if(sell.takerRelayerFee > 0) {
          takerRelayerFee = SafeMath.div(SafeMath.mul(sell.takerRelayerFee, price), INVERSE_BASIS_POINT2);
          if(sell.paymentToken == address(0)) {
  ...
  function calculateFeeWithMakerBuyNM(OrderNM memory buy, OrderNM memory sell, uint256 receiveAmount, uint256 requiredAmount, uint256 price) internal pure returns (uint makerRelayerFee,uint takerRelayerFee,uint receiveAmount1,uint requiredAmount1)
  {
    ...
    if(sell.feeMethod == FeeMethodNM.SplitFee) {
      ...
      if(buy.makerRelayerFee > 0) {
        makerRelayerFee = SafeMath.div(SafeMath.mul(buy.makerRelayerFee, price), INVERSE_BASIS_POINT2);
      }
      if(buy.takerRelayerFee > 0) {
        takerRelayerFee = SafeMath.div(SafeMath.mul(buy.takerRelayerFee, price), INVERSE_BASIS_POINT2);
      }
    }
  }
