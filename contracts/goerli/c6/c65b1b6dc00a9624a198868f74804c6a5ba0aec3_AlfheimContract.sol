...
contract AlfheimContract is Ownable, ERC721A, ReentrancyGuard {
    uint256 public  TotalWhiteListMint = 800; 
    uint256 public  TotalWhiteListMintPerwallet  = 1;  
    mapping(address => uint256) public MintPerWallet;
    mapping(bytes => bool) public signatureUsed;
    bool public     bMintStart  = false;
    bool public     bWaitlistMintStart = false;   
    bool public     bWLMintStart = true;  
    uint256 public  TotalTokenNumber;
    uint256 public  WLSaleprice = 70000000000000000;  
    uint256 public  saleprice = 88000000000000000;  
    bytes32 public root;

 constructor(
    uint256 MaxMintPerTx_,
    uint256 TotalToken_
  ) ERC721A("Alfheim", "Alfheim", MaxMintPerTx_, TotalToken_) {
  ...
  function WLMint(uint256 mintamount,  bytes32[] calldata proof) external payable {
    require(bWLMintStart || bWaitlistMintStart, "WL Sale is not started");  
    require(isValid(proof, keccak256(abi.encodePacked(msg.sender))), "Not a part of Whitelist");

    require(mintamount > 0 && (mintamount + MintPerWallet[msg.sender]) <= TotalWhiteListMintPerwallet, "Can not mint more than max per wallet");
    if(bWLMintStart) //WL mint
    {
        require(msg.value >= WLSaleprice*mintamount, "Not paid enough");
        require(totalSupply() + mintamount <= TotalWhiteListMint, "Can not mint more than max token");   
    }       
    else //Waittlist mint
    {
        require(msg.value >= saleprice*mintamount, "Not paid enough");
        require(totalSupply() + mintamount <= TotalTokenNumber, "Can not mint more than max token");   
    }      
  ...
  function setTotalWhiteListMint(uint _TotalWhiteListMint) external onlyOwner {
    TotalWhiteListMint = _TotalWhiteListMint;
  }
  ...
  function SetWaitlistMintStart() public onlyOwner {
    bWaitlistMintStart = !bWaitlistMintStart;
  }
