...
contract Staking is IStaking, Ownable {
    uint public rewardPercentBP = 300;
    uint public stakeFreeze = 7 days;
    ...
    function unstake() public override {
        require(
            ...
            "Can't unstake, time is not come."
        );
        require(
            block.timestamp > dao.tokenFreeze(msg.sender),
            "Can't unstake, tokens participate in active proposal"
        );
        
        uint staked = stakers[msg.sender].balance;
    ...
    function changeStakeFreeze(uint256 _days) public override onlyOwner {
        stakeFreeze = _days;
    }
...
interface IStaking {
    ...
    function balanceOf(address _staker) external returns(uint);
...
interface IDao {
    ...
    /// Deposit is too low
    error DepositIsTooLow(uint _totalDeposit);
    ...
    function tokenFreeze(address _staker) external returns(uint);
    ...
    function changeParam (uint _percent, uint _days) external;
}
