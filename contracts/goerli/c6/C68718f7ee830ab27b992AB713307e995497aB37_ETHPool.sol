...
contract PriceOracle is IPriceOracle {
    ...
    function _getPrice(uint256 poolValue, uint256 totalSupply) internal pure returns (uint256) {
        if (poolValue == 0 || totalSupply == 0) return 1e18;
        return ((poolValue * DIVISION_GUARD) / totalSupply);
    }

    // returns price of i_tokenAddress (1 token in eth)
    function getPrice() public view override returns (uint256) {
        uint256 poolValue = i_poolAddress.balance;
        uint256 totalSupply = i_token.totalSupply();

        return _getPrice(poolValue, totalSupply);
    }

    // converts eth to token price
        ...
        returns (uint256)
    {
        ...
        return
            (ethAmount * ((1e18 * DIVISION_GUARD) / _getPrice(poolValueMinMsgValue, totalSupply))) /
            DIVISION_GUARD;
    }
        ...
        returns (uint256)
    {
        ...
        return
            (ethAmount * ((1e18 * DIVISION_GUARD) / _getPrice(poolValue, totalSupply))) /
            DIVISION_GUARD;
    }
