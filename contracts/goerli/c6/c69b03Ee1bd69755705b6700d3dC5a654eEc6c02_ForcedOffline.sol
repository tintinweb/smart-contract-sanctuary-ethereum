...
contract ForcedOffline is Initializable, UUPSUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    mapping (uint => uint) public tokenOccupation;
    ...
    function initialize() initializer public {
        ...
        occupations = ["sniper", "Aviator", "Ninja"];
        backgrounds = ["chartreuse", "darkturquoise", "lightskyblue"];
    }
    ...
    function setOccupation(uint tokenId_, uint occupation_) public {
        require(_exists(tokenId_), "occupation set of nonexistent token");
        require(occupation_ < occupations.length, "invalid occupation");
        tokenOccupation[tokenId_] = occupation_;
    }

    function mint(address to, uint background_, uint occupation_) public {
        uint256 newItemId = _tokenIds.current();
        _safeMint(to, newItemId);
        setBackground(newItemId, background_);
        setOccupation(newItemId, occupation_);
        _tokenIds.increment();
    }

    function tokenURI(uint256 tokenId_) public view override returns (string memory) {
        require(_exists(tokenId_), 'URI query for nonexistent token');
        string memory offId = tokenId_.toString();
        string memory occupation = occupations[tokenOccupation[tokenId_]];
        ...
        returns (string memory)
    {
        string memory image = generateSVGImage(
            SVGParams({ offId: params.offId, occupation: params.occupation, background: params.background })
        );
        ...
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked('{"occupation":"', params.occupation, '", "image": "', 'data:image/svg+xml;base64,', image, '"}')
                    )
                )
            )
        );
    ...
    function generateSVG(SVGParams memory params) internal pure returns (string memory svg) {
        ...
        return string(
            abi.encodePacked(
                ...
                '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle"> ', params.occupation, ' OffId ', params.offId, ' </text>',
                _SVG_END_TAG
            )
        );
    }
