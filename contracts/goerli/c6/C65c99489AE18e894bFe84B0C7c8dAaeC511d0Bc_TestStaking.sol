...
contract TestStaking is ReentrancyGuard, AccessControl {
    using SafeERC20 for IERC20;

    // Interfaces for ERC20 and ERC721
    IERC20 public immutable rewardsToken;
    IERC721 public immutable nftMainCollection;

    //MINIMUM STAKING TIME
    uint256 public minimumStakingTime = 0;

    // Constructor function to set the rewards token and the NFT collection addresses
    constructor(IERC721 _nftMainCollection, IERC20 _rewardsToken) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        nftMainCollection = _nftMainCollection;
    ...
    uint256 private rewardsPerHour = 625000000000000000;//1000000000000000000;
    ...
    function _stake(uint _tokenId) internal {
        ...
        require(
            nftMainCollection.ownerOf(_tokenId) == msg.sender,
            "You don't own this token!"
        );

        unchecked {
                // Transfer the token from the wallet to the Smart contract
            nftMainCollection.transferFrom(msg.sender, address(this), _tokenId);
    ...
    function _withdraw(uint256 _tokenId) internal {
        ...
        nftMainCollection.transferFrom(address(this), msg.sender, _tokenId);
    }
