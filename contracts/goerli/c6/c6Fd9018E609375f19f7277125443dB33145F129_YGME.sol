...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString())) : orgURI;
    }
...
contract YGME is ERC721, Ownable {
    ...
    constructor(address pay, address reward) ERC721("YGME_TEST2", "YGME_TEST2") {
    ...
    function swap(address to, address _recommender) external onlyWhiter {
    ...
    function safeMint(address _recommender) external checkRecommender(_recommender) {
        address from = _msgSender();
        address self = address(this);
        
        payCon.transferFrom(from, self, PAY);
        distribute();
    ...
    function distribute() private {
        ...
        if(receicve_address_first != zero && 0 != receicve_amount_first){
            payCon.transfer(receicve_address_first, receicve_amount_first);
        }

        if(receicve_address_second != zero && 0 != receicve_amount_second){
            payCon.transfer(receicve_address_second, receicve_amount_second);
        }
    }



    function _safeMint(address to) private isMax  {
    ...
    function _rewardMint(address to) private {
       ...
       for (uint i = 0; i <= maxLevel; i++){
           ...
           if(rewward != zero && 0 != rewardLevelAmount[i]){
               rewardCon.transfer(rewward, rewardLevelAmount[i]);
           }
    ...
    modifier isMax {
        ...
        require(MAX >= _tokenIdCounter.current(), "already minted token of max");
