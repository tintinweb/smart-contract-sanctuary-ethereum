...
contract NFTFarms is ReentrancyGuard, Ownable, Pausable, IERC721Receiver {
  ...
  * @param _nftId  Unique id of NFT in NFT collection
  */
  function stake(uint256 _farmId, uint256 _nftId) external nonReentrant whenNotPaused {
    require(_farmId < totalFarms, "Cannot stake from an unexisting farm");
    require(farms[_farmId].active, "Farm is not active");
    require(farms[_farmId].stakedNFTCollection.ownerOf(_nftId) == msg.sender, "NFT does not belong to staker");
    require(positions[_farmId][msg.sender].stakedAmount < 1, "Only 1 nft staked per farm per unique address");

    // Currently we only allow staking of 1 NFT in a NFT collection per unique wallet address
    uint256 amount = 1;

    claim(_farmId);
    _updateFarm(_farmId);
    _updateUserRewardsDebt(_farmId, msg.sender, amount);

    Position storage position = positions[_farmId][msg.sender];
    Farm storage farm = farms[_farmId];

    position.stakedAmount = position.stakedAmount + amount;
    position.nftId = _nftId;
    position.staker = msg.sender;

    farm.totalStaked = farm.totalStaked + amount;

    farm.stakedNFTCollection.safeTransferFrom(msg.sender, address(this), _nftId);

    emit Stake(_farmId, msg.sender, _nftId);
  }
  ...
  * @param _nftId  Unique id of NFT in NFT collection
  */
  function unstake(uint256 _farmId, uint256 _nftId) public nonReentrant whenNotPaused {
    require(_farmId < totalFarms, "Cannot unstake from an unexisting farm");
    require(positions[_farmId][msg.sender].staker == msg.sender, "NFT was not staked by withdrawer");
    ...
    position.stakedAmount = position.stakedAmount - amount;
    position.nftId = 0; // default to 0 for null
    position.staker = address(0); // default to 0 address for nothing staked
    ...
    farm.stakedNFTCollection.safeTransferFrom(address(this), msg.sender, _nftId);

    emit Unstake(_farmId, msg.sender, _nftId);
  }
  ...
  */
  function claim(uint256 _id) public whenNotPaused {
    ...
    if (rewards > 0) {
      ...
      emit Claim(_id, msg.sender, NONA, rewards);
    }
  }
