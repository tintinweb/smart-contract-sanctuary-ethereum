...
contract NDTrading is Ownable {
    ...
    enum TxState { Empty, Active, Finished }
    struct Transaction { uint256 price; address seller; TxState state; }

    event ListTrait(uint256 indexed transactionId);
    event CancelListing(uint256 indexed transactionId);
    event BuyTrait(uint256 indexed transactionId, address wallet);
    ...
    /// @notice safe guard from malicious royalty setting
    uint256 constant public MAX_PERCENTAGE = 1000;
    ...
    ///         (eg. 100 -> 1%) 
    constructor(address _signer, address payable _ndRoyaltyAddress, uint256 _ndRoyaltyPercentage) {
        require(_ndRoyaltyPercentage < MAX_PERCENTAGE, "ND: percentage too high");
    ...
    // ------------------------ INTERNAL ------------------------ //
    // ---------------------------------------------------------- //
    function validSignature(uint256 transactionId, bytes memory signature) internal view {
        bytes32 hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(transactionId, msg.sender))
            )
        );
        require(signer == hash.recover(signature), "ND: unauthorized");
    }

    function calcRoyalties(uint256 price) internal view returns (uint256 toSeller, uint256 toRoyaltyAddr) {
        toRoyaltyAddr = (price * NDRoyaltyPercentage) / 1e4;
        toSeller = price - toRoyaltyAddr;
    }

    function transferPayments(address payable seller, uint256 toSeller, uint256 toRoyaltyAddr) internal {
    ...
    // ------------------------ EXTERNAL ------------------------ //
    // ---------------------------------------------------------- //
    function listTrait(uint256 price, uint256 transactionId, bytes memory signature) external {
        require(price > 0, "ND: invalid price");

        Transaction storage transaction = transactions[transactionId];
        require(transaction.state == TxState.Empty, "ND: txId not empty");

        validSignature(transactionId, signature);
    ...
    function buyTrait(uint256 transactionId) payable external {
        ...
        require(txn.state == TxState.Active, "ND: listing not active");
        require(msg.value == txn.price, "ND: invalid value");
        require(msg.sender != txn.seller, "ND: invalid buyer");
    ...
    function setRoyaltyInfo(address payable _royaltyAddress, uint256 _royaltyPercentage) external onlyOwner {
    ...
    function setTransactionDetails(uint256 transactionId, Transaction calldata _txInfo) external onlyOwner {
