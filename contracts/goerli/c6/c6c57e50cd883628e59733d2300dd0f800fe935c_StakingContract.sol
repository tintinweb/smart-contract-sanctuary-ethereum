...
contract StakingContract {
    ...
    uint256 private lowerMileStone;
    uint256 private uppermileStone;
    ...
    function JoinBattle(uint256 _amount, uint256 _battleId, address _joinerReferalPerson) public {
        ...
        referalTime[_battleId][battle.creator][referalPerson[battleId].creatorReferalPerson] = creatorReferalTime;
        settingReferalInfo(creatorReferalTime,creatorReferalAmount, referalPerson[battleId].creatorReferalPerson, battle.creator);
    ...
    function settingReferalInfo(uint256 _referalTime, uint256 _referalAmount, address _referalPerson, 
        address _battlePerson) internal {	
			claimRferalAmount[_referalPerson][_battlePerson][_referalTime] = _referalAmount;
	}
    ...
    function calculateAreenaBosterPrice() public view returns(uint256){
        ...
        uint256 bosterPercentage = calculateBosterPercentage(findValue.mul(1e18));
    ...
    function claimReferalBonus(uint256 _amount) public {

        _amount = _amount.mul(1e18);

        players[msg.sender].referalAmount -= _amount;
        players[msg.sender].genAmountPlusBonus += _amount;
    }

    function claimReferalBonusNotWorking(uint256 _battleId) public {

        Battle memory battle = battles[_battleId];

        
        if(referalPerson[battleId].creatorReferalPerson == battle.creator){
            
            uint256 referallTime = referalTime[_battleId][battle.creator][referalPerson[battleId].creatorReferalPerson];
            require(block.timestamp > referallTime.add(7 minutes),"You can not claim bonus before 7 minutes."); //////////Add 7 days here /////////
            
            uint256 referallAmount = claimRferalAmount[referalPerson[battleId].creatorReferalPerson][battle.creator][referallTime];
            players[msg.sender].referalAmount -= referallAmount;
            players[msg.sender].genAmountPlusBonus += referallAmount;
        }else{

            uint256 referallTime = referalTime[_battleId][battle.joiner][referalPerson[battleId].joinerReferalPerson];
            require(block.timestamp > referallTime.add(7 minutes),"You can not claim bonus before 7 minutes."); //////////Add 7 days here /////////

            uint256 referallAmount = claimRferalAmount[referalPerson[battleId].joinerReferalPerson][battle.joiner][referallTime];
            players[msg.sender].referalAmount -= referallAmount;
            players[msg.sender].genAmountPlusBonus += referallAmount;
        }
