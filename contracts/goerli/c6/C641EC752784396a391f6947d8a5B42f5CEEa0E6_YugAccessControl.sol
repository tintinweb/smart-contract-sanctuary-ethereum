...
contract YugAccessControl {
    ...
    function addAccess(address to, string memory key, string memory url, uint64 expiry, string memory kind) public {
        require(expiry > block.timestamp, "Incorrect expiry");
        require(IYugKYC(_yugKYC).isApproved(msg.sender, kind), "Sender KYC not done");
