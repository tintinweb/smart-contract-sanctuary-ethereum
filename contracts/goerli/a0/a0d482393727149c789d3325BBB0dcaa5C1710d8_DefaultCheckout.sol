...
/**
 * @title {DefaultCheckout} is a basic implementation of {Checkout} for which the {_postCheckoutAction}
 * always succeeds without doing anything.
 */
contract DefaultCheckout is Checkout {
    ...
    /**
     * @dev a default post checkout function that does nothing. See {Checkout-_postCheckoutAction}.
     */
     function _postCheckoutAction(bytes memory) internal pure override {}
...
abstract contract Checkout is Pausable, Ownable, ReentrancyGuard {
    ...
    event CheckoutSuccess(string indexed checkoutId, address indexed signer);

    /**
     * @dev using a struct to encapsulate all the data that is used to compute a checkout signature.
     */
    ...
    /**
     * @dev helper to compute a checkout hash for both {computeErc20CheckoutHash} and {computeNativeCheckoutHash}.
    ...
    function _checkEligibility(CheckoutRequest memory request) private view {
        ...
        require(!_hasBeenProcessed(request.checkoutId), "Checkout: Checkout with checkoutId was already processed.");
    ...
    /**
     * @dev helper to check if a checkout was previously processed.
     * @param checkoutId the checkout id.
     * @return true if {checkoutId} has been processed, false otherwise.
     */
    function _hasBeenProcessed(string memory checkoutId) private view returns (bool) {
     ...
     */
    function computeErc20CheckoutRequest(
        ...
        return _encodeCheckoutRequest(checkoutId, paymentToken, paymentValue, expirationTimestamp, postCheckoutCallData);
    }
     ...
     *      - If {data} is the result of this function, checkoutOracleAccount.signMessage(ethers.utils.arrayify(data))
     *        must be called in order to properly encode the hash before signing it.
     *      - Calling this function externally is the same as calling computeErc20CheckoutRequest with address(0) as
     *        the {paymentToken}. This is not a bug, just an incidental feature.
     */
    function computeNativeCheckoutRequest(
        string calldata checkoutId,
        uint256 paymentValue,
        uint256 expirationTimestamp,
        bytes calldata postCheckoutCallData
    ) public view returns (bytes memory) {
        return _encodeCheckoutRequest(checkoutId, address(0), paymentValue, expirationTimestamp, postCheckoutCallData);
    }
    ...
    function _wrapUp(address caller, string memory checkoutId, bytes memory postCheckoutCallData) private {
        _postCheckoutAction(postCheckoutCallData);
        processed[_checkoutIdHash(checkoutId)] = true;
        emit CheckoutSuccess(checkoutId, caller);
    }
     ...
     */
     function _postCheckoutAction(bytes memory data) internal virtual;
