...
contract LeveragePortfolioView is ILeveragePortfolioView, AbstractDependant {
    ...
    {
        ILeveragePool leveragePool = ILeveragePool(leveragePoolAddress);
        uint256 _targetUR = leveragePool.targetUR();

        uint256 _PRECISION = 10**27;

        uint256 _ur = _targetUR.trySub(poolUR);
        if (_ur == 0) _ur = poolUR.trySub(_targetUR);

        _multiplier = Math.min(
            leveragePool.d_ProtocolConstant().mul(_PRECISION).tryDiv(_ur).mul(_PRECISION).tryDiv(
                leveragePool.a1_ProtocolConstant()
            ),
        ...
        returns (uint256)
    {
        ...
        uint256 _poolUR = _totalCoverTokens.mul(PERCENTAGE_100).tryDiv(_poolTotalLiquidity);

        uint256 _minUR = _getPoolMINUR(factors.policyBookAddr);

        uint256 _ur = Math.max(_poolUR, _minUR);

        uint256 _res1 = _poolUR.mul(_poolTotalLiquidity).uncheckedDiv(PERCENTAGE_100);
        uint256 _res2 = _ur.mul(_poolTotalLiquidity).tryDiv(_minUR);
        uint256 _formula2 =
            (_res2.trySub(_poolTotalLiquidity)).mul(factors.netMPL).tryDiv(
                factors.netMPL.add(factors.netMPLn)
            );

        uint256 _formula1 = _res1.trySub(factors.netMPL);
        _formula1 = factors.netMPL.mul(_poolTotalLiquidity).tryDiv(_formula1);
        ...
        returns (uint256)
    {
        return
            factors
                ...
                .mul(ILeveragePool(msg.sender).a2_ProtocolConstant())
                .uncheckedDiv(PERCENTAGE_100)
                .uncheckedDiv(10**3);
    }
    ...
    function calcNetMPLn(address _policyBookFacade)
        public
        view
        override
        returns (uint256 _netMPLn)
    {
        ...
        for (uint256 i = 0; i < _userLeverageArr.length; i = uncheckedInc(i)) {
            ...
            _netMPLn = _netMPLn.add(
                ILeveragePool(_userLeverageArr[i])
                    .totalLiquidity()
