...
abstract contract HavahMitter is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _verify(bytes memory data, bytes calldata signature) internal view {
...
contract HavahMitterFT is HavahMitter {
    ...
    function _transfer(address fromToken, string memory fromTokenStr, uint256 amount, uint256 fee) private returns (OriginInfo memory) {
        ...
        if (tokenInfo.wrappedToken != address(0)) {
            require(IERC20(fromToken).balanceOf(msg.sender) >= amount, "not enough balance");

            SafeERC20.safeTransferFrom(IERC20(fromToken), msg.sender, treasury(), fee);
            WERC20(fromToken).burnFrom(msg.sender, amount.sub(fee));
            ...
            if (fromToken == address(0)) {
                ...
                SafeERC20.safeTransferFrom(IERC20(fromToken), msg.sender, treasury(), fee);
                SafeERC20.safeTransferFrom(IERC20(fromToken), msg.sender, address(this), amount.sub(fee));
            }
