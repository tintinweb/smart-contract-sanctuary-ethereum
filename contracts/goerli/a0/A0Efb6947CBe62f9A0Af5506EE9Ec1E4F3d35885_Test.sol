...
contract Test {
  ...
  function swap(uint256 amountIn, address[] memory path) external {
    IERC20(path[0]).safeTransferFrom(msg.sender, address(this), amountIn);
    IERC20(path[0]).safeApprove(liquidityRouter, amountIn);
    bytes memory pathBytes = abi.encodePacked(path[0]);
    for (uint256 i = 1; i < path.length; i++) {
      pathBytes = bytes.concat(pathBytes, abi.encodePacked(uint24(3000), path[i]));
    }
    ...
    ISwapRouter(liquidityRouter).exactInput(params);
  }
  ...
  function burn(uint256 tokenId) external {
    ...
    (, , address token0, address token1, , , , uint128 liquidity, , , , ) = pm.positions(tokenId);
...
library StopLoss {
  ...
  function run(
    ...
    ISwapRouter.ExactInputParams memory params = ISwapRouter.ExactInputParams({
        path: abi.encodePacked(order.path[0], order.fee, order.path[1]),
        recipient: address(this),
        deadline: _deadline,
        amountIn: baseBalance,
        amountOutMinimum:order.amountOutMin 
    });
    uint256 amountOut = ISwapRouter(liquidityRouter).exactInput(params);
