...
contract AstroLama is ERC721AQueryable, Ownable {
    string public constant collectionName = "Astro Lama";
    string public constant collectionSymbol = "LAMA";
    uint256 public constant maxQuantityPerMint = 5;
    uint256 public constant maxQuantityPerGift = 5;
    uint256 public mintPrice = 0.036 ether;
    uint256 public mintWLPrice = 0.015 ether;
    uint256 public maxTotalSupply = 10_000;
    uint256 public mintStartAt = 0;
    uint256 public mintWlStartAt = 0;

    mapping(address => bool) public whitelist;
    mapping(address => uint256) public mintCount;
    mapping(address => uint256) public giftCount;
    mapping(uint256 => uint256) public randomId;
    ...
    function gift(address to, uint256 quantity) external onlyOwner {
        require(giftCount[to] + quantity <= maxQuantityPerGift, "Gift quantity limit 5 per address");
        require(totalSupply() + quantity <= maxTotalSupply, "Total supply would be exceeded");
        giftCount[to] += quantity;
        _safeMint(to, quantity);
    }
    ...
    function mint(uint256 quantity) external payable callerIsUser {
        ...
        require(quantity <= maxQuantityPerMint, "Mint quantity limit 5");
        require(msg.value >= currentPrice, "Not enough payed");
        require(totalSupply() + quantity <= maxTotalSupply, "Total supply would be exceeded");

        randomizeTokens(quantity);
        _safeMint(_msgSenderERC721A(), quantity);

        refundIfOver(currentPrice);
    }

    function mintPrivate(uint256 quantity) external payable callerIsUser {
        uint256 currentPrice = quantity * mintWLPrice;

        require(block.timestamp >= mintWlStartAt, "Mint not started");
        require(whitelist[_msgSenderERC721A()], "Not in white list");
        require(mintCount[_msgSenderERC721A()] <= maxQuantityPerMint, "Mint quantity limit 5 per address");
        require(quantity <= maxQuantityPerMint, "Mint quantity limit 5");
        require(msg.value >= currentPrice, "Not enough payed");
        require(totalSupply() + quantity <= maxTotalSupply, "Total supply would be exceeded");

        mintCount[_msgSenderERC721A()] += quantity;
        randomizeTokens(quantity);
        _safeMint(_msgSenderERC721A(), quantity);
    ...
    function changeMintStartDate(uint256 timestamp) public onlyOwner {
        mintStartAt = timestamp;
    }

    function changeMintWLStartDate(uint256 timestamp) public onlyOwner {
        mintWlStartAt = timestamp;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual returns (string memory) {
        ...
        string memory fileName = toHex(
            keccak256(
                abi.encodePacked(_toString(tokenId), '-', _toString(randomId[tokenId]))
            )
        );
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, fileName, baseExtension)) : '';
    }
    ...
    function randomizeTokens(uint256 quantity) internal {
        uint256 _next = _nextTokenId();
        for (uint256 i = _next; i < _next + quantity; i++) {
            randomId[i] = random() % 3;
        }
    }
