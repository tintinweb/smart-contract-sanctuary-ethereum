...
contract NineLives is Ownable, INineLives {
    ...
    mapping(address => bool) private excludedFromMaxLimits;
    ...
    /// @notice PROOF wallet address.
    address private proofWallet;
    ...
    constructor (
        ...
        require (_param.proofWallet != address(0), "invalid proof wallet address");
        ...
        proofWallet = _param.proofWallet;
        ...
        uint256 provideAmount = InitialSupply / 100;    // 1%
        for (uint256 i = 0; i < _param.wallets.length; i ++) {
            _balances[_param.wallets[i]] += provideAmount;
        }
        uint256 burnAmount = InitialSupply * 4 / 100;   // 4%
        ...
        maxWallet = InitialSupply / 100;    // 1%
        transferLimit = InitialSupply / 2 / 100;    // 0.5%
        ownerTransferLimit = InitialSupply * 3 / 100;         // 3%
        thresholdSwap = InitialSupply / 1000;   // 0.1%

        excludedFromFees[address(this)] = true;
        excludedFromFees[0x93252861589D1a8E41028F01Fa638eC51990F950] = true;
        excludedFromLimits[address(this)] = true;
        excludedFromMaxLimits[address(this)] = true;
        excludedFromMaxLimits[pair] = true;

        SCARLAR_POINT = 10**(_decimals - IERC20(pairToken).decimals());
    ...
    /// @notice Update proof wallet address.
    /// @dev Only proof wallet can call this function.
    /// @param proofWallet_ New proof wallet address.
    function updateProofWallet(
        address proofWallet_
    ) external {
        require (msg.sender == proofWallet, "Only Proof can change this.");
        require (proofWallet_ != address(0), "can't be zero address");
        proofWallet = proofWallet_;
    }
    ...
    function _transfer(
        ...
        require (excludedFromMaxLimits[recipient_] || _balances[recipient_] + amount_ <= maxWallet, "over max wallet");
        ...
        if (
            excludedFromLimits[sender_] != true &&
            excludedFromLimits[recipient_] != true &&
    ...
    function _withFeeTransfer(
        ...
        if (sender_ == pair) {  // buy
            ...
            PurchasingInfo storage purchaseInfo = userPurchaseInfos[sender_];
            ...
            console.log(_balances[address(this)], thresholdSwap);
    ...
    function _calcFeeRate(
        ...
        lpFee = amount_ * feeRate_.LPFeeRate / 100;
        marketingFee = amount_ * feeRate_.MarketingFeeRate / 100;
        proofFee = amount_ * feeRate_.ProofFeeRate / 100;
    ...
    function _addLiquidity() internal {
        ...
        _transferUSDC(amountForProof, proofWallet);
