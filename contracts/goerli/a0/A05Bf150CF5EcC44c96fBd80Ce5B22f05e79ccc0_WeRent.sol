...
contract WeRent is IWeRent, ERC721Holder, ERC1155Receiver, ERC1155Holder, Initializable {
    ...
    function returnIt(
        ...
        bundleCall(handleReturn, createActionCallData(_nfts, _tokenIds, _lendingIds));
    }
    ...
    function claimCollateral(
        ...
        bundleCall(handleClaimCollateral, createActionCallData(_nfts, _tokenIds, _lendingIds));
    }
    ...
    function takeFee(
        ...
        fee = _rent * rentFee;
        fee /= 10000;
        uint256 discountForLender = staking.getRentDiscountByAddress(_lenderAddress);
        fee = fee - (discountForLender * fee) / 10000;
        if (_useNativeToken) {
    ...
    function distributePayments(LendingRenting storage _lendingRenting, uint256 _secondsSinceRentStart)
        private
        returns (uint256)
    {
        ...
        uint256 takenFee = takeFee(sendLenderAmt, useNativeToken, _lendingRenting.lending.lenderAddress);
    ...
    function distributeClaimPayment(LendingRenting memory _lendingRenting) private {
        bool useNativeToken = _lendingRenting.lending.useNativeToken;
        ...
        uint256 takenFee = takeFee(maxRentPayment, useNativeToken, _lendingRenting.lending.lenderAddress);

        uint256 discountForLender = staking.getRentDiscountByAddress(_lendingRenting.lending.lenderAddress);
        takenFee = takenFee - (discountForLender * takenFee) / 10000;
    ...
    function handleReturn(CallData memory _cd) private {
        ...
        for (uint256 i = _cd.left; i < _cd.right; i++) {
            ...
            uint256 renterFee = distributePayments(item, secondsSinceRentStart);
...
interface IStaking {
    ...
    event SetLockDuration(uint128 lockDuration);
    ...
    function getLockDuration() external view returns (uint128);

    function getAcceptedTokenAddress() external view returns (address);

    function setLockDuration(uint128 _lockDuration) external;
