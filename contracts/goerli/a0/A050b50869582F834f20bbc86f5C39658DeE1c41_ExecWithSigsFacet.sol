...
import {_getBalance} from "../functions/Utils.sol";
import {
    ExecWithSigs,
...
contract ExecWithSigsFacet is ExecWithSigsBase, BFacetOwner {
    ...
    /// @return estimatedGasUsed Estimated gas used using gas metering
    /// @return fee The fee transferred to the fee collector
        ...
        returns (uint256 estimatedGasUsed, uint256 fee)
    {
        ...
        bytes32 digest = _getDigestSyncFee(_getDomainSeparator(), _call.msg);
...

        {
            ...
            (bool success, bytes memory returndata) = _call.msg.service.call(
                abi.encodePacked(_call.msg.data, feeCollector)
            );
            if (!success)
                GelatoBytes.revertWithError(
                    returndata,
            ...
            fee = postFeeTokenBalance - preFeeTokenBalance;
        }
    ...
    {
        callStatus = new bool[](_calls.length);
        gasUsed = new uint256[](_calls.length);

        for (uint256 i = 0; i < _calls.length; i++) {
            uint256 startGas = gasleft();

            require(
                _isCheckerSigner(msg.sender),
                "ExecWithSigsFacet.simulateBatchExecWithSigs: not checker"
            );

            (bool success, bytes memory returndata) = _calls[i]
                .msg
                .service
                .call(_calls[i].msg.data);

            returndata;

            callStatus[i] = success;
    ...
    function simulateBatchExecWithSigsFeeCollector(
        ...
        for (uint256 i = 0; i < _calls.length; i++) {
            ...
            require(
                _isCheckerSigner(msg.sender),
                "ExecWithSigsFacet.simulateBatchExecWithSigsFeeCollector: not checker"
            );

            (bool success, bytes memory returndata) = _calls[i]
                .msg
                .service
                .call(abi.encodePacked(_calls[i].msg.data, feeCollector));
    ...
    function simulateBatchExecWithSigsRelayContext(
        ...
        for (uint256 i = 0; i < _calls.length; i++) {
            ...
            require(
                _isCheckerSigner(msg.sender),
                "ExecWithSigsFacet.simulateBatchExecWithSigsRelayContext: not checker"
            );
