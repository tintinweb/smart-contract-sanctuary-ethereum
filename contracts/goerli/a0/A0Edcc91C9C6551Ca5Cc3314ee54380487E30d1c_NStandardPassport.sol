...
import "./n-passport-base.sol";

contract NStandardPassport is NPassportBase {
    constructor(address _withdrawAddress) NPassportBase("NStandardPassport", "NSP", _withdrawAddress) {}
}
...
contract NPassportBase is AccessControl, ERC721, Ownable, Pausable {
    bytes32 public ADMIN = "ADMIN";
    uint256 public SUBSCRIBE_TERM_SEC = 370 * 24 * 60 * 60;
    ...
    mapping(uint256 => PassportInfo) passportInfoByTokenId;
    ...
    modifier existsPassportInfo(uint256 tokenId) {
        require(passportInfoByTokenId[tokenId].expiredTimestamp > 0, "Passport Info Does Not Exist");
        _;
    }
    ...
    function getMintCostForSubscribe(uint256 tokenId) public view returns(uint256) {
        return planCosts[passportInfoByTokenId[tokenId].planNo];
    }
    function getPassportInfo(uint256 tokenId) view public returns(PassportInfo memory) {
        return passportInfoByTokenId[tokenId];
    }
    function passportIsValid(uint256 tokenId) view public returns(bool) {
        return passportInfoByTokenId[tokenId].expiredTimestamp >= block.timestamp;
    }
    ...
    function resetExpiredTimestamp(uint256 tokenId) external onlyRole(ADMIN)
        existsPassportInfo(tokenId)
        isTokenOwner(tokenId)
    {
        passportInfoByTokenId[tokenId].expiredTimestamp = block.timestamp - 1;
    }
        ...
        existsPlan(planNo)
    {
        ...
        totalSupply++;
        passportInfoByTokenId[tokenId] = PassportInfo(
            planNo,
            block.timestamp + SUBSCRIBE_TERM_SEC
        );
    }
    ...
    function subscribe(uint256 tokenId) external payable
        ...
        existsPassportInfo(tokenId)
    {
        if (passportInfoByTokenId[tokenId].expiredTimestamp > block.timestamp) {
            require(msg.value >= planCosts[passportInfoByTokenId[tokenId].planNo] , 'Not Enough Eth');
            passportInfoByTokenId[tokenId].expiredTimestamp += SUBSCRIBE_TERM_SEC;
        } else {
            require(msg.value >= planCosts[planNoForPublic] , 'Not Enough Eth');
            passportInfoByTokenId[tokenId].expiredTimestamp = block.timestamp + SUBSCRIBE_TERM_SEC;
            passportInfoByTokenId[tokenId].planNo = planNoForPublic;
        }
    }
