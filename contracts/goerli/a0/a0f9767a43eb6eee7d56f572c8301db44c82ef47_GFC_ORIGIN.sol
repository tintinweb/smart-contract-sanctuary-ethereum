...
contract GFC_ORIGIN is ERC1155, AccessControl, RrpRequesterV0 {
    ...
    function requestMintT2(
        address _minter, 
        uint256 _mintQuantity,
        uint256 _offChainDeterminedGasFee
    ) public payable {
        sendQrngRequest(_minter, _mintQuantity, 1);
    } 

    function requestMintT3(
        address _minter, 
        uint256 _mintQuantity,
        uint256 _offChainDeterminedGasFee
    ) public payable {
    ...
    function setWhitelistMerkleRoot(bytes32 _root) external onlyRole(ADMIN_ROLE) {
    ...
    /// @dev TESTING: in case above function exceeds block gas limit...?
    uint256 public lastGoldId;
    function setGoldIdsFlexible(uint256 _trial) public onlyRole(ADMIN_ROLE) {
        uint256 goldId = goldSpacing;
        ...
        for(uint256 i=lastGoldId; i<goldSupply-1; i++) {
            goldIds.push(goldId);
            goldId += goldSpacing;
        }
    ...
    function setMintIsPaused(bool _mintIsPaused) external onlyRole(ADMIN_ROLE) {
        mintIsPaused = _mintIsPaused;
    }

    function setWhitelistMintIsPaused(bool _whitelistMintIsPaused) external onlyRole(ADMIN_ROLE) {
