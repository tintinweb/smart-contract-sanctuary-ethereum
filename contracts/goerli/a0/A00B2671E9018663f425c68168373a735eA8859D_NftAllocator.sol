...
*/
contract NftAllocator is Ownable, Initializable, ERC721A__IERC721Receiver {
    ...
    mapping(bytes => TokensAllocated) public invitesAllocation;
    ...
    */
    function initialize(address _jubiFundsAddress, Types.Fraction memory _jubiFeePercent, Types.NftAllocatorConfig memory _config) external initializer {
    ...
    */
    function invest(uint256 numTokens, uint256 minAllowedTokens, uint256 maxAllowedTokens, bytes calldata inviteCode, bytes32[] calldata merkleProof) external {
        ...
        if (address(nft) != address(0) && nft.balanceOf(address(this)) >= totalUnclaimed && (tokenIdsLength - nextTokenId) >= numTokens) {
            ...
            SafeERC20.safeTransferFrom(venture.treasuryToken(), msg.sender, address(this), ventureFunds * numTokens);
    ...
    */
    function claim(uint256 numToClaim) external {
        ...
        SafeERC20.safeTransfer(venture.treasuryToken(), venture.fundsAddress(), ventureFunds * numToClaim);
...
contract Venture is Ownable, Initializable, AccessControl {
    ...
    */
    function initialize(Types.VentureConfig memory config, address _owner) external initializer {
        // Ownable
