...
contract Sale is ReentrancyGuard, Ownable {
    ...
    function startPresale() public onlyOwner {
        require(!(presale[presaleId].isCreateSale), "Sale is not created");
    ...
    function endPresale() public onlyOwner {
        ...
        presale[presaleId].isCreateSale == false;
    }
    ...
    function updatePresale(
        ...
        presale[_id].hardCapUSDT = _hardCapUSDT;
    }
    ...
    function removeStuckTokens(address _token, uint256 _amount)
        external
        onlyOwner
    {
        IERC20(_token).transfer(fundReceiver, _amount);
    }
        ...
        returns (bool)
    {
        ...
        if (isExcludeMinFee[msg.sender]) {
            require(
                tokens >= presale[presaleId].minTokenToBuy,
                "Insufficient amount!"
            );
        }
        ...
        returns (bool)
    {
        ...
        if (block.timestamp >= _presale.endTime) {
            amountToSend = userClaimData[user][_id].totalAmount;
            userClaimData[user][_id].claimAt = block.timestamp;

            userClaimData[user][_id].claimedAmount += amountToSend;
