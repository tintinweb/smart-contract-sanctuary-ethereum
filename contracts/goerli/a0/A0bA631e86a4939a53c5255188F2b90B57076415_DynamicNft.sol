...
contract DynamicNft is AccessControl, IDynamic {
    ...
    function mintGenesis(address _to, bytes32[] calldata _merkleProof) public payable {
        ...
        require(GenesisNft.balanceOf(_to) == 0, "ALREADY A TOKEN HOLDER");
        require(GenesisNft.totalSupply() < 500, "INSUFICIENT GENESIS STOCK");

        if (merkleRoot != ZEROSTATE) {
            require(
                MerkleProof.verify(_merkleProof, merkleRoot, keccak256(abi.encodePacked(_to))),
    ...
    function mintSubscription(
        ...
        if (subsStage == SubscriptionStage.TIER_ONE) {
            ...
            if (
                ...
                uint256 amountLeftOfTierTwo = (subscriptionDetails.tierTwoQuantities + subscriptionDetails.tierOneQuantities) - SubscriptionNft.totalSupply();
            ...
            if (
                ...
                uint256 amountLeftOfTierThree = (subscriptionDetails.tierTwoQuantities + subscriptionDetails.tierOneQuantities + subscriptionDetails.tierThreeQuantities) - SubscriptionNft.totalSupply();
        ...
        for (uint256 _id = SubscriptionNft.currentIndex(); _id < (_quantity + SubscriptionNft.currentIndex()); _id++) {
        ...
        if(SubscriptionNft.totalSupply() >= (subscriptionDetails.tierThreeQuantities + subscriptionDetails.tierTwoQuantities + subscriptionDetails.tierOneQuantities)){
            subsStage = SubscriptionStage.TIER_FOUR;
        }else if(SubscriptionNft.totalSupply() >= (subscriptionDetails.tierTwoQuantities + subscriptionDetails.tierOneQuantities)){
    ...
    function claimDistributedTokens() public {
        INft GenesisNft = INft(genesisContractAddress);

        require(GenesisNft.balanceOf(msg.sender) != 0, "NOT A GENESIS HOLDER");
        require(distributeFlag == true, "DISTRIBUTION NOT OPEN");

        for (uint256 _id = GenesisNft.currentIndex(); _id < (3 + GenesisNft.currentIndex()); _id++) {
            genesisHolder[_id] = Genesis({tokenId: _id, owner: msg.sender});
...
interface IDynamic {
    ...
    function mintSubscription(address to, uint256 amount, uint256 code) external payable;

    function setDistibuteGenesisTokensActive() external;

    function claimDistributedTokens() external;
