...
contract tomiStaking is OwnableUpgradeable {
    ...
    // events
    event stakedAPY  (address staker,uint amountStaked, uint stakeTime, uint lockPeriod, uint APY , StakesInfo stakesInfo );
    ...
    function stakeForAPY(uint principalAmount_ , uint type_) public {
        ...
        emit stakedAPY (msg.sender, principalAmount_, block.timestamp,stakeperiod[type_], stakeApy[type_], stakes[msg.sender][stakerindex[msg.sender]] );
    }
    ...
    function stakeForPioneer(uint amountToStake ) public {
        ...
        stakesForPioneer[msg.sender][stakerIndexPioneer[msg.sender]].endTime = block.timestamp + (stakesForPioneerPeriod * timeInSeconds); 
    ...
    function unStakeWithAPY ( uint indexUnstake ) public {
        ...
        uint256 _reward = _claim(  indexUnstake );
        ...
        emit unStakeAPY( msg.sender,  block.timestamp,indexUnstake, stakes[msg.sender][indexUnstake].stakeAmount, _reward);
    ...
    function _claim(uint index ) public view returns (uint256) {
            if ( stakes[msg.sender][index].stakeDuration == 0 ) {
                return 0;
            }

            uint256 res = ( stakes[msg.sender][index].stakeAmount * 10 ) /100 ;
            // uint oneDayAPY = res /365;
            uint oneMintAPY = res/ (365* 24 * 60);

            uint256 endTime = block.timestamp > stakes[msg.sender][index].endTime_ ? stakes[msg.sender][index].endTime_ : block.timestamp;

            uint timePassedInMinutes = ( endTime - stakes[msg.sender][index].lastClaimTime ) / 60 ;
