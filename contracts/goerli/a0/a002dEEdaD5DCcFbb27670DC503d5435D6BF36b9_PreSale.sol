...
contract PreSale is Ownable {
    ...
    uint256 public _MIN_ETH_CONTRIBUTION = 1 * 10**18;
    ...
    mapping(address => bool) public isPresaleWhitelist;

    mapping(address => uint256) public totalPurchased;
    mapping(address => bool) public withdrawn;

    constructor() {
        token = IERC20(0xBBF84f68734ac0cCF22c845b6607886247Ba99e9);
    }
    ...
    function ownerWithdrawETH() public onlyOwner{
        ...
        require(_TOTAL_DEPOSIT_ETH >= _SOFTCAP, "Presale didn't hit softcap");
    ...
    function setupPresale(uint256 start, uint256 end, uint256 release, uint256 rate, uint256 softcap, uint256 hardcap, uint256 maxPerWallet, uint256 minPerWallet) public onlyOwner{
        _RATE = rate;
        _MAX_ETH_CONTRIBUTION = maxPerWallet;
        _MIN_ETH_CONTRIBUTION = minPerWallet;
        _SOFTCAP = softcap;
        _HARDCAP = hardcap;
        _START_TIME = start;
        _END_TIME = end;
    ...
    function deposite() public payable {
        ...
        if(!isPreSaleOpen){
            require(isPresaleWhitelist[msg.sender], "This wallet is not allowed to join presale");
        }
        require(msg.value >= _MIN_ETH_CONTRIBUTION, "Please check minimum ETH contribution");
        require(totalPurchased[msg.sender].add(msg.value) <= _MAX_ETH_CONTRIBUTION, "Check max contribution per wallet");
        require(_TOTAL_DEPOSIT_ETH.add(msg.value) <= _HARDCAP, "Exceed HARD CAP");

        totalPurchased[msg.sender]=  totalPurchased[msg.sender].add(msg.value);
        _TOTAL_DEPOSIT_ETH = _TOTAL_DEPOSIT_ETH.add(msg.value);
        _TOTAL_SOLD_TOKEN = _TOTAL_DEPOSIT_ETH.mul(_RATE);
    }

    function withdraw() public {
        require(block.timestamp > _END_TIME, "Presale hasn't finished yet");
        require(!withdrawn[msg.sender], "Already withdawn!");
        if(_TOTAL_DEPOSIT_ETH < _SOFTCAP){
            if(totalPurchased[msg.sender] > 0){
                payable(msg.sender).transfer(totalPurchased[msg.sender]);
                withdrawn[msg.sender] = true;
            }
        }else {
            require(block.timestamp > _RELEASE_TOKEN_TIME, "Please check token release time");
            token.transfer(msg.sender, totalPurchased[msg.sender].mul(_RATE));
            withdrawn[msg.sender] = true;
        }
    }
}