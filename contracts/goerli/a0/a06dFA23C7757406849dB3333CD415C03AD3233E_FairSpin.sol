...
contract FairSpin is Ownable {
    ...
    event NewGame(bytes32 indexed gameId, address[] players, uint256 nonce);
    event GameOver(bytes32 indexed gameId, uint256 result, address winner);
    ...
    function newGame(
        ...
        bytes32 gameId = keccak256(abi.encode(address(this), players, ethBalance, nonce));
        require(gameId.toEthSignedMessageHash().recover(signature) == verifier, "FAIRSPIN: Invalid signature");
        require(players.length == ethBalance.length, "FAIRSPIN: Invalid input");

        usedNonce[nonce] = true;
        vault.lockFairSpin(players, ethBalance);

        Game storage game = games[gameId];
        game.players = players;
        game.ethBalance = ethBalance;
        game.fee = fee;
        game.total = total;

        emit NewGame(gameId, players, nonce);
    }
    ...
    function endGame(
        ...
        require(_hash.toEthSignedMessageHash().recover(signature) == verifier, "FAIRSPIN: Invalid signature");

        Game storage game = games[gameId];
        game.winner = winner;
        game.result = result;

        vault.unlockFairSpin(game.players, game.ethBalance);
        vault.setFairSpinResult(winner, game.players, game.ethBalance, game.total, game.fee);

        emit GameOver(gameId, result, winner);
    }

    function emergencyEndGame(bytes32 gameId) external onlyOwner {
        Game memory game = games[gameId];
        require(game.players.length != 0, "invalid game id");
    ...
    function getFee(uint256 amount) internal view returns (uint256) {
        if (vault.feeType() == IVault.FeeType.FixedFee) {
            ...
            return (amount * vault.fee()) / 10000;
        }
        return 0;
    }

    function getDetails(bytes32 gameId) public view returns (address[] memory, uint256[] memory) {
        return(
            games[gameId].players,
            games[gameId].ethBalance
        );
    }
}
