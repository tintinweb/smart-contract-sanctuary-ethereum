...
import "./interface/IStakingDistributor.sol";
import "./interface/ITreasury.sol";

contract StakingDistributor is AccessControl, IStakingDistributor {
    ...
    */
    ...
    function distribute() external override {
        ...
        // This function mints reward for the staker based on the stakers reward rate
        require(msg.sender == staking, "Only stacking");
    ...
    function adjust(uint256 _index) internal {
        ...
        // This function serves to increase or decrease reward rate of an index
    ...
    function nextRewardAt(uint256 _rate) public view override returns (uint256) {
        return mainToken.totalSupply().mul(_rate).div(rateDenominator);
    }
    ...
    function addRecipient(address _recipient, uint256 _rewardRate) external override onlyGovernor {
        ...
        require(_rewardRate <= rateDenominator, "Rate");
        info.push(Info({recipient: _recipient, rate: _rewardRate}));
    }

    function removeRecipient(uint256 _index) external override onlyGovernorGuardian {
        require(info[_index].recipient != address(0), "Recipient does not exist");
        info[_index].recipient = address(0);
        info[_index].rate = 0;
    }

    function setAdjustment(uint256 _index, bool _add, uint256 _rate, uint256 _target) external override onlyGovernorGuardian {
        require(info[_index].recipient != address(0), "Recipient does not exist");
        ...
        if (!_add) {
            require(_rate <= info[_index].rate, "Cannot decrease rate by more that it already is");
        }
