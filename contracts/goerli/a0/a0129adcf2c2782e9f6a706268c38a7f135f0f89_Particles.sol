...
contract Particles is ERC1155, Ownable, ReentrancyGuard {
  ...
  function spawn(
    ...
    if (particleExists(tokenId)) revert ParticleAlreadyExists();
    if (maxSpawn == 0) revert ParticleMaxSpawnCannotBeZero();
    ...
    for (uint16 i = 0; i < propertyIds.length; ) {
      if (!propertyValueInRange(propertyIds[i], propertyValues[i]))
        revert ParticleValueOutOfRangeOrDoesntExist();
      unchecked {
        ++i;
      }
    }
  ...
  function discover(
    ...
    if (minValue >= maxValue) revert PropertyMinCannotBeBiggerMax();
    if (maxValue == 0) revert PropertyMaxSpawnCannotBeZero();

    properties[propertyId].minValue = minValue;
    properties[propertyId].maxValue = maxValue;
    properties[propertyId].name = _name;

    emit Discovery(propertyId, minValue, maxValue, _name);
  }
  ...
  function mint(
    ...
    if (!particleExists(tokenId)) revert UnknownParticle();
