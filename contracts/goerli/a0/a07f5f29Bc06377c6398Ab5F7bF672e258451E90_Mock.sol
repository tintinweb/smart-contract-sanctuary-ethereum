...
abstract contract ERC721PsiBurnableAirdrop is ERC721PsiBurnable {
    ...
    function _airdrop() internal virtual {
        ...
        // emit Transfer
        // Emit events
        for (uint256 tokenId = _startTokenId(); tokenId < _startTokenId() + AIRDROP_AMOUNT;) {
            unchecked {
    ...
    */
    ...
    function _appendAirdropAddresses(address[] memory candidates) internal virtual {
        ...
        for (uint256 i = 0; i < candidates.length;) {
            unchecked {
                if (candidates[i] == address(0)) revert("contains address zero");
                i++;
            }
        }

        // write input
        for (uint256 i = 0; i < division;) {
            unchecked {
                if ((i + 1) == division) {
    ...
    function _decode(address pointer, uint256 index) internal view returns (address addr) {
        ...
        assembly {
            ...
            if iszero(eq(shr(248, stuckHeader), 20)) { revert(0, 0) }
