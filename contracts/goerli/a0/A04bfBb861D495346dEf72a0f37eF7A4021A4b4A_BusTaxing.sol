...
contract BusTaxing is PassengerVehichleTaxing, KeeperCompatibleInterface {
    ...
    function checkUpkeep(
        ...
        require(
            isCalculated == false,
            "error: variable must be initialized as false"
        );
...
contract PassengerVehichleTaxing {
    ...
    uint256 private s_minVehicleCapacity;
    uint256 private s_maxVehicleCapacity = 24;
    ...
    function unRegister(uint256 _passId) public virtual onlyDriver {
        ...
        for (uint256 i = 0; i < m_drivers.length; i++) {
            if (m_drivers[i].addr == msg.sender) {
                ...
                /// @notice throwing an error if caller is not a known driver
                revert PassengerVehichleTaxing__NotPermitted(
                    "caller not a known driver"
                );
            }
        }
