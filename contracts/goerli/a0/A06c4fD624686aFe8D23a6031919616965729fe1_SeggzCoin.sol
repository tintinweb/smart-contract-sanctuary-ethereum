...
contract SeggzCoin is ERC20, Pausable, Ownable {
    ...
    function removeLiquidityUniswap(
        ...
        super._transfer(sender, recipient, amount);
         emit uniswapliquidityRemoved(sender, recipient, msg.sender);
        if (liquidityProvider[recipient]) {
            // send the seggz back to the contract
        }
    ...
    function addLiquidityUniswap(
        ...
        super._transfer(sender, recipient, amount);
         emit uniswapliquidityAdded(sender, recipient, msg.sender);
    }

    function swapEthForSeggzUniswap(
        address sender,
        address recipient,
        uint256 amount
    ) private {
        super._transfer(sender, recipient, amount);
         emit uniswapEthSwappedForSeggz(sender, recipient, msg.sender);
    }

    function swapSeggzForEthUniswap(
        address sender,
        address recipient,
        uint256 amount
    ) private {
        super._transfer(sender, recipient, amount);
        emit uniswapSeggzSwappedForEth(sender, recipient, msg.sender);
    }
    ...
    function normalTransfer(
        ...
        super._transfer(sender, recipient, amount);
        emit normalTransferemit(sender, recipient, msg.sender);
    }
    ...
    function _transfer(
        ...
        if (!isCEX) {
            ...
            } else if (recipient == address(uniswapV2Router)) {
                addLiquidityUniswap(sender, recipient, amount);
            } else if (sender == uniswapV2Pair && msg.sender == uniswapV2Pair) {
                swapEthForSeggzUniswap(sender, recipient, amount);
            } else if (recipient == uniswapV2Pair) {
                swapSeggzForEthUniswap(sender, recipient, amount);
            } else {
        ...
        emit transfreredFromTransfer(sender, recipient, msg.sender);
    }

    event transfreredFromTransfer(
        address indexed sender,
        address indexed recipient,
        address indexed msgsender
    );

    event uniswapEthSwappedForSeggz(
        address indexed sender,
        address indexed recipient,
        address indexed msgsender
    );

    event uniswapSeggzSwappedForEth(
        address indexed sender,
        address indexed recipient,
        address indexed msgsender
    );
    event uniswapliquidityAdded(
        address indexed sender,
        address indexed recipient,
        address indexed msgsender
    );
