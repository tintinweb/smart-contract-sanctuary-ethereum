...
contract Meow is Ownable {
    ...
    function enterBigRoom(uint256 tokenId, uint256 roomnum) external payable {
        require(NFT.ownerOf(tokenId) == _msgSender(), "NOT_OWNER");
        require(
    ...
    function claimFight(uint256 tokenId, uint256 roomnum) external payable {
        ...
        require(
            NFT.ownerOf(betRoom.tokenId1) == address(this),
            "NFT not transfered"
        );
        startGame(roomnum, false);
        emit GameStarted(betRoom.tokenId1, tokenId);
        gameStatus[roomnum] = true;
    }

    function claimBigFight(uint256 tokenId, uint256 roomnum) external payable {
        require(NFT.ownerOf(tokenId) == _msgSender(), "NOT_OWNER");
        require(
            gamePrice.mul(5) == msg.value,
            "Amount doesn't equal msg.value"
    ...
    function claimNFT() external {
        uint256 nftCount = tokenOwner[msg.sender].length;
        while(nftCount != 0) {
            if(tokenOwner[msg.sender][nftCount - 1] == 0) break;
            NFT.transferFrom(address(this), msg.sender, tokenOwner[msg.sender][nftCount - 1]);
            tokenOwner[msg.sender][nftCount - 1] = 0;
            nftCount --;
        }
    }

    function claimMoney () external {
        sendPrice(msg.sender, claimAmount[msg.sender]);
