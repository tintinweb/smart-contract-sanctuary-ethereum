...
contract SweepDollarCoin is BaseSweep {
     ...
     */
    function is_minting_allowed(uint256 _mint_amount) external view returns (bool) {
        (uint256 sweep_amount, uint256 usdx_amount) = uniV3TWAPOracle.getPoolLiquidity();

        uint256 new_sweep_amount = sweep_amount + _mint_amount;
        uint256 usdx_sweep_amount = convertToSWEEP(usdx_amount);
        
        return new_sweep_amount < usdx_sweep_amount ? true : false;
    }

    /**
     * @notice Get Maximum Sweep Amount For Margin Invests
     ...
     */
    function convertToSWEEP(uint256 _amount) public view returns (uint256) {
