...
contract GenerativeCollectionToken is ERC721, ERC2981 {
    ...
    function mint(uint256 quantity_) external payable {
        require(_reservations[msg.sender] == 0);
        ...
        if (!(ICollectionAirdrop(_airdrop_storage_address).getOptions(address(this)) > total_supply + quantity_ && has_access_token)) {
    ...
    function reserve(uint256 quantity_) public payable {
        ...
        bool is_whitelist_member = ICollectionWhiteList(_goldlist_storage_address).isMember(address(this), msg.sender);
        ...
        require(_limit >= reserved + quantity_);
        require(_reservations[msg.sender] + quantity_  <= _limit_per_wallet);
        reserved += quantity_;
    ...
    function getReservation(address eth_address_) public view returns (uint256) {
        return _reservations[eth_address_] - _minted_reservations[msg.sender];
    }
