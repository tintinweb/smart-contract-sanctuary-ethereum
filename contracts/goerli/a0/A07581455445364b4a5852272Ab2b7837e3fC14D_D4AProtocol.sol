...
interface ID4ASettingForProtocol {
...
contract D4AProtocol is Initializable, ReentrancyGuardUpgradeable, ID4AProtocol{
  ...
  function createProject(uint256 _start_prb,
                         ...
                         uint256 _project_index) public override payable nonReentrant returns(bytes32 project_id){
    require(ID4ASettingForProtocol(address(settings)).hasRole(keccak256("OPERATION_ROLE"), msg.sender), "only admin can specify project index");
    require(!settings.d4a_pause(), "D4A Paused");
