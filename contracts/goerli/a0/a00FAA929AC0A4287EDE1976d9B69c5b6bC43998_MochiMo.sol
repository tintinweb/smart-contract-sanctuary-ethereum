...
contract MochiMo is ERC721A, Ownable, PaymentSplitter {
    ...
    // Whitelist settings
    uint256 constant public WHITELIST_SUPPLY = 3333;
    uint256 constant public MAX_MINT_WHITELIST = 2;
    uint256 public mintPriceWhitelist = 0.0077 ether;
    ...
    uint256 private _mintedViaVip = 0;
    ...
    function setMerkleRootWhitelist(bytes32 _newMerkleRootWhitelist) external onlyOwner {
        merkleRootWhitelist = _newMerkleRootWhitelist;
    }
    ...
    // VIP metadata
    function setMerkleRootVip(bytes32 _newMerkleRootVip) external onlyOwner {
        merkleRootVip = _newMerkleRootVip;
    }
    ...
    function mintPublic(uint256 amount) external payable mintCompliance(amount) {
        ...
        require(totalSupply() + amount <= MAX_SUPPLY - WHITELIST_SUPPLY - VIP_SUPPLY, "Can't mint that many!");
    ...
    function mintWhitelist(uint256 amount, bytes calldata signature) external payable mintCompliance(amount) {
        ...
        require(totalSupply() + amount <= MAX_SUPPLY - VIP_SUPPLY, "Can't mint that many via whitelist!");
    ...
    function mintVip(uint256 amount, bytes calldata signature) external mintCompliance(amount) {
        ...
        require(totalSupply() + amount <= MAX_SUPPLY, "Can't mint that many via VIP!");
