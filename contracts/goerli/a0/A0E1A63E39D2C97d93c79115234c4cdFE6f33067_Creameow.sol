...
contract Creameow is ERC721A, ERC721ABurnable, ERC721AQueryable, ERC2981, ReentrancyGuard, Pausable, Ownable {
    ...
    function whiteList(address[] calldata _addressList,uint256[] calldata countList) external onlyOwner {
        require(_addressList.length > 0, "Error: list is empty");
        require(countList.length == _addressList.length, "Error: invalid list");
        for (uint i = 0; i < _addressList.length; ++i) {
            whiteListed[_addressList[i]] = countList[i];
        }
    }
