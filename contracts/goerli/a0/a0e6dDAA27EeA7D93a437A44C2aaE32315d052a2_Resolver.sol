...
contract Resolver {
    ...
    constructor() {
        ...
        Gateways.push(Gate(
            "ccip-gateway-p1lhnfki8vld.runkit.sh", 
    ...
    function DNSDecode(bytes calldata encoded) public pure returns(string memory _name, bytes32 namehash) {
        ...
        while (j > 0) { // return ...bob.alice.eth
            _name = string.concat(string(labels[--j]), ".", _name); 
    ...
    function randomGateways(string memory _name) public view returns(string[] memory urls) {
        ...
        uint k = uint(keccak256(abi.encodePacked(block.timestamp, _name, msg.sender, blockhash(block.number - 1))));
        for (uint i; i < len;) {
     ...
     * @param digest : digest hash of signed message
