...
contract Dev is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Person #',toString(tokenId), '", "description": "Developers  Join the movement that is community owned, building the future from the bottom up.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '", "attributes": ', "[", '{"trait_type": "TextColor", "value" : "', parts[1] , '"}', "]"  ,'}'))));
