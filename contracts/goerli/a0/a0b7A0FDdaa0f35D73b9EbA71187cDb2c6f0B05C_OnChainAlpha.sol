...
//name of the NFT to the "username" field that the holder populates? 
//If empty, it will just be OC Alpha #<TokenId>
...
contract OnChainAlpha is ERC721A, ReentrancyGuard, Ownable {
    ...
    event AttributesUpdated(uint256 tokenId, string userName, string twitter, string primaryENS, string website);
        ...
        returns (string memory)
    {
        ...
        if (keccak256(abi.encodePacked(tokenValues[_tokenId].userName)) > 0)
        {
            //_toString(tokenValues[_tokenId].userName)
            jsonBytes.appendSafe(abi.encodePacked(unicode"{\"name\":\"", tokenValues[_tokenId].userName));
        } else {
            jsonBytes.appendSafe(unicode"{\"name\":\"OnChain Alpha #");
        }
