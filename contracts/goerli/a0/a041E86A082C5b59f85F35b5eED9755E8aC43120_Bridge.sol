...
contract Bridge is Pausable, AccessControl {
    ...
    mapping(uint32 => mapping(uint256 => uint256)) public usedNonces;
     ...
     */
    function deposit(uint32 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) external payable whenNotPaused {
     ...
     */
    function executeProposal(uint32 originDomainID, uint64 depositNonce, bytes calldata data, bytes32 resourceID, bytes calldata signature) public whenNotPaused {
     ...
     */
    function isProposalExecuted(uint32 domainID, uint256 depositNonce) public view returns (bool) {
      return usedNonces[domainID][depositNonce / 256] & (1 << (depositNonce % 256)) != 0;
    }

    function setMPC(address MPCAddress) external {
        require(msg.sender == _MPCAddress && MPCAddress != address(0) && MPCAddress != _MPCAddress, "bad");
...
interface IFeeHandler {
    ...
    /**
        ...
        @param feeData Additional data to be passed to the fee handler.
     */
    function collectFee(address sender, uint32 fromDomainID, uint32 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) payable external;
     ...
     */
    function calculateFee(address sender, uint32 fromDomainID, uint32 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) external view returns(uint256, address);
}
