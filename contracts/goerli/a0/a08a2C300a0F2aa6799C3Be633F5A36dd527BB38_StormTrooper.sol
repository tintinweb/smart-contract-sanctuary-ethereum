...
contract StormTrooper is ERC1155, ERC2981, AccessControl, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    // function.
    uint32 callbackGasLimit = 140000;
    ...
    // brand => collectionIds 
    mapping(uint256 => uint256[]) private brands;
    // brand => mintPriceInWei
    mapping(uint256 => uint256) private brandMintPriceInWei;
    ...
    function requestMint(uint256 brand) public payable {
        require(brandMintPriceInWei[brand] * 1 == msg.value, "Not enough eth.");
        require(getMintableCollectionIds(brand).length > 0, "Sold out.");
        ...
        accountMintRequest[msg.sender].push(requestId);
    ...
    function requestMint(address _to, uint256 brand) public onlyHasRole(MINTER_ROLE) {
        require(getMintableCollectionIds(brand).length > 0, "Sold out.");
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        uint256 _collectionId = brands[req.brand][collectionIndexToMint];
    ...
    function getMintableCollectionIds(uint256 _brandId) public view returns (uint256[] memory) {
        ...
        for (uint256 id = 0; id < ids.length; id++) {
            ...
            if (infanities[ids[id]].minted >= infanities[ids[id]].cap) {
                unmintableCount += 1;
            }
        }
    ...
    function getCollectionInfoById(uint256 _tokenId) public view returns (StormTrooperItem memory) {
        require(infanities[_tokenId].enabled, "Token does not exist.");
    ...
    /// @dev Set brand mint price
    /// @param brandId The Brand ID
    /// @param mintPriceInWei The mint price for this brand
    function setOrUpdateBrandMintPriceInWei(uint256 brandId, uint256 mintPriceInWei) public onlyHasRole(ADMIN_ROLE) {
        brandMintPriceInWei[brandId] = mintPriceInWei;
        emit SetOrUpdateMintPrice({ brand: brandId, mintPriceInWei: mintPriceInWei });
    }
    ...
    function createCollection(uint256 cap, uint256 brand) external onlyHasRole(ADMIN_ROLE) {
        ...
        require(brandMintPriceInWei[brand] > 0, "brand does not exist.");
