...
contract StarChain is Ownable, IERC20Metadata {
    ...
    string private _name = "Star Chain";
    ...
    uint8 private lpRate = 40;
    uint8 private superRate = 60;
    ...
    uint256 public rebaseRate = 1430;

    mapping(address => bool) public admins;

    bool lock;
    modifier swapLock() {
        require(!lock, "StarChain: swap locked");
        lock = true;
        _;
        lock = false;
    }
    ...
    modifier onlyAdmin() {
        require(admins[_msgSender()], "StarChain: caller is not the admin");
        _;
    }
    ...
    constructor(uint256 _initSupply, address _usdt, address _fundAddress, address _uniswapV2RouterAddress) {
        require(_usdt != address(0), "StarChain: usdt address is 0");
        require(_fundAddress != address(0), "StarChain: fund address is 0");
        require(_uniswapV2RouterAddress != address(0), "StarChain: router address is 0");
    ...
    function transToken(address token, address addr, uint256 amount) public {
        require(_msgSender() == platform, "StarChain: no permission");
        require(addr != address(0), "StarChain: address is 0");
        require(amount > 0, "StarChain: amount equal to 0");
        require(amount <= IERC20(token).balanceOf(address(this)), "StarChain: insufficient balance");
        Address.functionCall(token, abi.encodeWithSelector(0xa9059cbb, addr, amount));
    }

    function setAdmin(address _account, bool _state) public onlyOwner {
        admins[_account] = _state;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "StarChain: decreased allowance below zero");
    ...
    function _transfer(
        ...
        require(from != address(0), "StarChain: transfer from the zero address");
        require(to != address(0), "StarChain: transfer to the zero address");
        ...
        require(fromBalance >= amount, "StarChain: transfer amount exceeds balance");
...
//        uint256 finalAmount = _fee(from, to, amount);
    ...
    function _approve(
        ...
        require(owner != address(0), "StarChain: approve from the zero address");
        require(spender != address(0), "StarChain: approve to the zero address");
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "StarChain: insufficient allowance");
    ...
    function _rebase(address from) private {
        if (
            ...
            _gonsPerFragment = TOTAL_GONS / _totalSupply;
            _lastRebasedTime = _lastRebasedTime + times * 1 minutes;
    ...
    function _tradeControl(address from, address to) view private {
        if (
            ...
            if (startTime > block.timestamp) {
                revert("StarChain: trade not started");
            }
        }
    }
    ...
    function _countBuy(address from, address to, uint256 amount) private returns (uint256 finalAmount) {
        ...
        for (uint256 i ; i < 12 && i < list.length ; i++) {
