    ...
    VRFv2Consumer
{
    ...
    event ProfitSplitPercentUpdated(
        uint16 _split1BP,
        uint256 _split2BP
    );
    ...
    event BurnAndProfitPercentUpdated(
        uint16 _burnPercent,
    ...
    event profits(
        ...
        uint256 _splitWallet1Amount,
        uint256 _splitWallet2Amount
    );
    ...
    function createRaffle(
        ...
        require(
            ...
            "Provide future time value."
        );
        totalRaffles++;
        RaffleStorage.RaffleInfo storage raffleEntry = Raffle[
            totalRaffles
        ];
        ...
        emit RaffleCreated(
            ...
            _startTime,
            _endTime,
            10000 - profitPercent - burnPercent,
            _rewardToken
        );
    }

    function updateBurnAndProfitPercent(
        uint16 _burnBp,
        uint16 _profitBp
    ...
    function updateProfit1Address(address payable _profitWallet1)
        external
        onlyAdmin
    {
        profitWallet1 = _profitWallet1;
    ...
    function updateProfit2Address(address payable _profitWallet2)
        external
        onlyAdmin
    {
        profitWallet2 = _profitWallet2;
    ...
    function updateProfitSplitPercent(uint16 _bp)
        external
        onlyAdmin
    {
        profitSplit1BP = _bp;
    ...
    {
        return
            userTicketNumbersInRaffle[_raffleNo][_owner]
                .ticketsNumber;
    }
    ...
    function buyTicket(
        ...
        RaffleStorage.RaffleInfo storage raffleInfo = Raffle[
            _raffleNumber
        ];
        require(
            raffleInfo.endTime > block.timestamp,
            "Buying ticket time over!"
        );
        require(
            raffleInfo.ticketCounter + _noOfTickets <=
                raffleInfo.maxTickets,
        ...
        for (uint8 i = 1; i <= _noOfTickets; i++) {
            ...
            userTicketNumbersInRaffle[_raffleNumber][msg.sender]
                .ticketsNumber
                .push(raffleInfo.ticketCounter);

            raffleInfo.ticketOwner[
                raffleInfo.ticketCounter
            ] = msg.sender;
        }
        totalBurnAmount +=
            (msg.value * raffleInfo.burnPercent) /
            10000;
        ...
        totalRevenue +=
            (msg.value * (10000 - raffleInfo.rewardPercent)) /
            10000;
        emit BuyTicket(
            _raffleNumber,
            msg.sender,
    ...
    function checkDeclare(uint256 _raffleNumber)
        external
        view
        returns (bool)
    {
        ...
        return block.timestamp > raffleInfo.endTime;
    }
    ...
    function changeOperator(address _address)
        external
        onlyAdmin
    {
        operator = _address;
    ...
    function splitProfit(
        ...
        uint256 totalAmount = _noOfTickets *
            raffleInfo.ticketPrice;
        uint16 profitpercent = 10000 -
            raffleInfo.rewardPercent -
            raffleInfo.burnPercent;
        uint256 profitAmount = (profitpercent * totalAmount) /
            10000;
        uint256 splitWallet1Amount = (profitAmount *
            profitSplit1BP) / 10000;
        (bool sent, ) = profitWallet1.call{
            value: splitWallet1Amount
        }("");
        uint256 splitWallet2Amount = profitAmount -
            splitWallet1Amount;
        (bool success, ) = profitWallet2.call{
            value: splitWallet2Amount
        }("");
    ...
    function declareWinner(uint256 _raffleNumber)
        external
        onlyOperator
    {
        RaffleStorage.RaffleInfo storage raffleInfo = Raffle[
        ...
        require(
            raffleInfo.winner == address(0),
            "Winner Already Declared"
        );
        uint256 totalTicketsSold = raffleInfo.ticketCounter;
        requestRandomWords();
        uint256 winnerTicketNumber = (s_requestId %
            totalTicketsSold) + 1;
        raffleInfo.winningTicket = winnerTicketNumber;
        raffleInfo.winner = raffleInfo.ticketOwner[
            winnerTicketNumber
        ];
        uint256 reward = ((raffleInfo.ticketPrice *
            raffleInfo.ticketCounter) *
            raffleInfo.rewardPercent) / 10000;
        uint256 amount = swapRewardInToken(
            raffleInfo.raffleRewardToken,
            reward
        );
    ...
    function claimReward(uint256 _raffleNumber)
        external
        nonReentrant
        returns (bool)
    {
        ...
        uint256 rewardAmount = raffleInfo
            .raffleRewardTokenAmount;
        require(
            msg.sender == raffleInfo.winner,
            "You are not the winner"
        );
        bool success = IERC20(raffleInfo.raffleRewardToken)
