...
contract BlackErc20 is ERC20, Ownable {
    ...
    function mint(uint256 mintCount) external payable {
        ...
        IWETH(wethAddress).transfer(lpContract, msg.value*99/100);
        IWETH(wethAddress).transfer(devAddress, msg.value*1/100);
        ...
        IERC20(address(this)).transfer(lpContract, mintAmount); // 49.5%转给lp
        IERC20(address(this)).transfer(devAddress, devAmount); // 49.5转给交易发送者
    }
    ...
    function setDevAddress(address dev) external onlyOwner {
        require(dev == address(0), "devAddress contract already set");
        devAddress = dev;
    }

    function setFactoryContract(address factory) external onlyOwner {
        require(factory == address(0), "factoryContract contract already set");
...
contract BlackErcFactory is Ownable {
    ...
    function createToken(
        ...
        // 创建 ERC20 代币合约
        BlackErc20 token = new BlackErc20(name, symbol, totalSupply, maxMintCount, maxMintPerAddress,mintPrice);
        ...
        token.setFactoryContract(address(this));
