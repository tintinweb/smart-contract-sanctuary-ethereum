    ...
    UUPSUpgradeable
{
    ...
    event DiscountAmount(uint amount);
    ...
    function makePayment(uint _discount) public payable {
        require(balanceOf(msg.sender) >= _discount, "Insufficient coin balance for user.");
        admin.transfer(msg.value);
        _burn(msg.sender, _discount);
        emit paymentEvent(msg.sender, admin, msg.value, block.timestamp);
        emit DiscountAmount(_discount);
    }
    ...
    function adminTransferToken(address _user, uint _amount) external onlyOwner{
    ...
    function refund(address user) external payable onlyOwner {
        ...
        emit refundEvent(user, msg.sender, msg.value, block.timestamp);
    }
}
