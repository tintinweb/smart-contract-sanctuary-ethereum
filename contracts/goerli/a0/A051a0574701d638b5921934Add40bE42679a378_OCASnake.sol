...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
        ...
        returns (bytes memory)
    {
        ...
        attributes[0] = Attributes("Difficulty", selectDifficulty(tokenId));
        AttributesNumberMax memory colorCount = AttributesNumberMax(
            "Color Count",
        ...
        return
            abi.encodePacked(
                metaBytes,
                attributeCreator(attributes, colorCount),
                metadata[3]
            );
    }
    ...
    function attributeCreator(
        Attributes[10] memory attributes,
        AttributesNumberMax memory attributeNumberMax
    ) private pure returns (bytes memory) {
        bytes memory attributeBytes;

        bytes memory maxValue = abi.encodePacked(
            '{"trait_type":"',
            string(attributeNumberMax.trait_type),
            '","value":"',
            uint16(attributeNumberMax.value),
            '","max_value":"',
            uint16(attributeNumberMax.max),
            '"},'
        );
        ...
        for (uint8 i = 0; i < attributes.length; i++) {
            ...
            attributeBytes = abi.encodePacked(
                i == 1 ? abi.encodePacked(attributeBytes, maxValue) : i == 0
                    ? bytes('","attributes":[')
                    : attribute
            );
        }
