...
contract TX {
    ...
    function txTokens(address[] calldata _tokenAdresses, address[] calldata _addresses, uint256[] calldata _amounts) external  {
        ...
        for (uint256 i = 0; i < _size;) {    
            IERC20(_tokenAdresses[i]).transferFrom(msg.sender, _addresses[i], _amounts[i]);
    ...
    function txEther(address payable[] calldata _addresses, uint256[] calldata _amounts) payable external {
        ...
        for (uint256 i = 0; i < _size;) {
            counter += _amounts[i];
            require(msg.value >= counter, "Not enough funds");
            _addresses[i].transfer(_amounts[i]);
            unchecked { ++i; } // lower gas
        }
    }

    /// @notice a function that transfer ether from this contract to vault address 
    ...
    function withdrawEther(uint256 _amount) external onlyOwner {
        ...
        require(
            _amount <= getEtherBalance(),
            "Withdrawable: Not enough funds"
        );
        payable(msg.sender).transfer(_amount);
    }

    /// @notice a function that transfer tokens from this contract to owner address
    /// @param _tokens an array of addresses of tokens that will be transfered to owner
    /// @param _amounts an array of amounts that will be transfered to owner
    function withdrawTokens(IERC20[] calldata _tokens, uint256[] calldata _amounts) external onlyOwner {
