...
// Import: SignatureDrop functionality.
import "@thirdweb-dev/contracts/base/ERC721Drop.sol";

import "@thirdweb-dev/contracts/extension/SoulboundERC721A.sol";

/**
 *  The `SignatureDropRandom` smart contract uses thirdweb's audited `SignatureDrop` smart contract.
 *
 *  At the time of lazy minting a delayed-reveal batch of NFTs, we calculate and store a 'tokenID offset'
 *  for that batch. When this batch of NFTs is revealed, the metadata for an NFT of tokenId `n` is
 *  `baseURI/{n + offset}`, where `baseURI` is the unencrypted base URI for the relevant batch of NFTs.
 */

contract SignatureDropRandomNew is ERC721Drop, SoulboundERC721A {
    using TWStrings for uint256;
        ...
        returns (string memory)
    {
        ...
        // If the relevant batch of NFTs has NOT been revealed, return the placeholder URI.
        if (isEncryptedBatch(batchId)) {
            return string(abi.encodePacked(batchUri, "0"));
        } else {
            // If the relevant batch of NFTs has been revealed, get metadata for the tokenId post random offset.
            uint256 metadataId = _getMetadataId(_tokenId, batchId, index);
            return string(abi.encodePacked(batchUri, metadataId.toString()));
        }
    }

    /// @dev Lets an authorized address reveal a batch of delayed reveal NFTs.
        ...
        returns (string memory revealedURI)
    {
        ...
        revealedURI = super.reveal(_index, _key);
    }

    /// @dev Returns the metadata ID for a given tokenId.
    function _getMetadataId(
        uint256 _tokenId,
        uint256 _batchId,
        uint256 _indexOfBatchId
    ) private view returns (uint256 metadataId) {
        uint256 randomness = uint256(tokenIdOffset[_batchId]);
        ...
        metadataId = prevBatchId + ((_tokenId + offset) % batchSize);
    }
