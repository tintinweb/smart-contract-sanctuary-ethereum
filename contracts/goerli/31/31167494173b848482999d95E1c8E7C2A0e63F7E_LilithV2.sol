...
contract LilithV2 is AccessControl, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, EIP712{
  bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  string private constant SIGNING_DOMAIN = "LilithV2-Voucher";
  string private constant SIGNATURE_VERSION = "1";
  address private _tokenContractAddress;
  mapping (address => uint256) pendingWithdrawals;

  constructor(address payable admin,address tokenAddress) ERC721("LilithV2", "LLT2") EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
  ...
  struct NFTVoucher {
    ...
    // @notice expiry date.
    uint64 expiry;
    // @notice unlock minting by non-holders.
    bool unlock;
  ...
  function ownerOfV1(uint256 id) private view returns (address) {
      ILilith iansdr = ILilith(_tokenContractAddress);
  ...
  function redeem(address redeemer, NFTVoucher calldata voucher) public payable {
    ...
    // Check if request ID has already been taken
