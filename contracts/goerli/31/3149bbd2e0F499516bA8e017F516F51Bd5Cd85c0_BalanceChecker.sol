...
contract BalanceChecker {
    bytes4 public constant balanceOfSig =
        bytes4(keccak256("balanceOf(address)"));
    bytes4 public constant blockedBalanceOfSig =
        bytes4(keccak256("blockedBalanceOf(address)"));
  ...
  */
        ...
        returns (uint256)
    {
        ...
        if (tokenCode > 0 && token.call(balanceOfSig, user)) {
  ...
  */
        ...
        returns (uint256)
    {
        ...
        // is it a contract and does it implement balanceOf
        if (tokenCode > 0 && token.call(blockedBalanceOfSig, user)) {
            return Token(token).balanceOf(user);
        } else {
            return 0;
        }
  ...
  */
    ...
    user, and so on.
  */
    function blockedBalances(address[] users, address[] tokens)
        external
        view
        returns (uint256[])
    {
