...
contract AIns is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    uint256 private unlockDate = 1678831322;
    uint256 public cost_public = 5000000000000000;
    uint256 public cost_wl = 3000000000000000;
    string private domain='.ai';
    string private BASE_URI = 'https://nftmetadata.live/ai/';
    ...
    constructor() ERC721A(".AI Name Service", "NAMES") {
        tokenIDandAddress[_currentIndex]="OPEN";
        tokenAddressandID["OPEN"]=_currentIndex;
        resolveAddress["OPEN"]=msg.sender;
    ...
    function setUnlockDate(uint256 MintDate) external onlyOwner {
        unlockDate = MintDate;
    }

    function isContractLocked() public view returns( bool ) {
        if ( block.timestamp < unlockDate ) return true;
    ...
    function setPrice(uint256 setPricePublic,uint256 SetPriceWL) external onlyOwner {
        cost_public = setPricePublic;
        cost_wl = SetPriceWL;
    }
        ...
        payable
    {
        ...
        require(msg.value >= cost_public, "Insufficient funds!");
        require( !isContractLocked(), "Register is not active!" );
        tokenAddressandID[NAME]=_currentIndex;
        _safeMint(msg.sender,1);
    }


     function WhitelistRegister(string memory NAME, bytes32[] calldata _merkleProof)
        public
        payable
    {
            ...
            require(_checkName(NAME), "Invalid Name");
            require (tokenAddressandID[NAME] == 0 , "This is already taken");
            require(MerkleProof.verify(_merkleProof, merkleRoot, leaf),"Invalid proof for whitelist register!");
            require(msg.value >= cost_wl, "Insufficient funds!");
            require( !isContractLocked(), "Register is not active!" );
...
function setMerkleRoot(bytes32 _newMerkleRoot) external onlyOwner {
 ...
 function _checkName(string memory NAME) public view returns(bool){
        uint16 allowedChars=0;
    ...
    function withdraw() public onlyOwner nonReentrant {
        uint256 balance = address(this).balance;
        Address.sendValue(payable(0xc4dc1F7CDAFd1beDc1042ea07dec4E46296a6685),balance);
        }
