...
contract TokenVesting is Ownable {
     ...
     */
    function getBeneficiaryNextVestings(address _beneficiary) external view returns (BeneficiaryOverview[] memory) {
        string[] memory scheduleNames = getBeneficiaryScheduleNames(_beneficiary);
        BeneficiaryOverview[] memory beneficiaryOverviews = new BeneficiaryOverview[](scheduleNames.length);
        for (uint32 i = 0; i < scheduleNames.length; i++) {
            if (beneficiaries[_beneficiary][scheduleNames[i]].allocatedAmount > 0) {
                BeneficiaryOverview memory beneficiaryOverview = BeneficiaryOverview({
