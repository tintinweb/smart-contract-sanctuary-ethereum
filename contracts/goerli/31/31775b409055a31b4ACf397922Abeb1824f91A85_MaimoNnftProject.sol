...
contract MaimoNnftProject is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Bunnies", "BNS") {       
        baseURI = "ipfs://QmUmJv3gzEVZnV6stFG4Wba7fvzLvz6hJQNh7poCz74if9/";
    }
    bool public saleStarted = true;
    uint256 public constant bunnyPrice = 5; //0.05 ETH
    ...
    function mint(bytes32[] memory proof, bytes32 leaf, uint256 numberOfTokens) public payable returns(uint256,uint256){
        ...
        require(numberOfTokens <= maxSupplyPurchase, "Can only mint 2 tokens at a time");        
        (cond, res) = totalSupply().tryAdd(numberOfTokens);
        require(cond, "Overflow error");
        require(res <= maxSupply, "Purchase would exceed max supply of Bunnies");        
        require(msg.sender != address(0x0), "Public address is not correct");
        (cond, res) = bunnyPrice.tryMul(numberOfTokens);
        require(cond, "Overflow error");                        
