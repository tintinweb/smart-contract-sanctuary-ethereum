...
contract StaffTokensScheme {
  ...
  mapping(address => address[]) private AccountsByToken;
  ...
  function owner_changeOwner(address newOwner) external onlyOwner {
  ...
  function owner_addAdmin(address admin) external onlyOwner {
    if (!Admin[admin]) Admin[admin] = true;
  }

  function owner_removeAdmin(address admin) external onlyOwner {
    if (Admin[admin]) Admin[admin] = false;
  }
  ...
  function admin_addBeneficiary(string calldata tokenSymbol, address account, uint256 award, uint256 numPayments, Date calldata startDate) external onlyOwnerOrAdmin {
    ...
    addAccountByToken(token, account);
  ...
  function admin_removeBeneficiary(string calldata tokenSymbol, address account) external onlyOwnerOrAdmin {
    address token = getToken(tokenSymbol);
    removeAccountByToken(token, account);
  ...
  function makeAllPaymentsDue() external {
    ...
    for (uint256 i; i < tokensLength; i++) {
      token = getToken(tokenSymbols[i]);
      accounts = AccountsByToken[token];
      accountsLength = accounts.length;
      for (uint256 j; j < accountsLength; j++) {
        makePayment(token, accounts[j]);
      }
    }
  }
  ...
  function getNextPaymentAmount(string calldata tokenSymbol, address account) external view returns (uint256) {
    address token = getToken(tokenSymbol);
    bytes32 entry = getEntry(token, account);
    if (FirstPaymentAmount[entry] > 0) return FirstPaymentAmount[entry];
    else return RegularPaymentAmount[entry];

  }

  function removeAccountByToken(address token, address account) private {
    address[] memory accounts = AccountsByToken[token];
    ...
    if (found) {
      if (i < accountsLength - 1) AccountsByToken[token][i] = AccountsByToken[token][accountsLength - 1];
      AccountsByToken[token].pop();
    }
  }

  function addAccountByToken(address token, address account) private {
    address[] memory accounts = AccountsByToken[token];
    uint256 accountsLength = accounts.length;
    for (uint256 i; i < accountsLength; i++) {
      if (accounts[i] == account) return;
    }
    AccountsByToken[token].push(account);
  }
  ...
  function makePayment(address token, address account) private {
    ...
    if (NumPaymentsRemaining[entry] == 0) {
      removeAccountByToken(token, account);
      NextPaymentDue[entry] = 0;
      RegularPaymentAmount[entry] = 0;
    }
  }
