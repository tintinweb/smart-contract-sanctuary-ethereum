...
contract ArtreusTestMarketplace is ReentrancyGuard, Ownable {
    ...
    mapping(uint => MarketItem) public _items;

    mapping(uint => MarketItem) public listings;
    ...
    function createMarketSale(address nftContract, uint256 itemId) public payable nonReentrant {
        ...
        item.seller.transfer(price - royaltyAmount - feeAmount);

        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);

        _itemsSold.increment();
        if (listingPrice != 0) payable(feeAccount).transfer(listingPrice);
    ...
    function getTotalPrice(uint itemId) view public returns(uint) {
        return idToMarketItem[itemId].price*(100 + feePercent/100);
    }
