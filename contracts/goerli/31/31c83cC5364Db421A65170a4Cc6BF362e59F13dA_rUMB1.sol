...
contract rUMB1 is rUMB {
     constructor (
        ...
        address _umb
    ) rUMB(_owner, _initialHolder, _initialBalance, _maxAllowedTotalSupply, _swapDuration, _name, _symbol, _umb) {}
}
...
abstract contract rUMB is MintableToken, SwappableToken {
    ...
    SwappableToken(_umb, _swapDuration) {
        if (_initialHolder != address(0) && _initialBalance != 0) {
            everMinted = _initialBalance;
...
abstract contract SwappableToken is Owned, ERC20 {
    ...
    uint256 public immutable swapDuration;
    ...
    constructor(address _umb, uint _swapDuration) {
    ...
    function canSwapTokens(address _address) external view returns (bool) {
        return balanceOf(_address) <= totalUnlockedAmountOfToken() - swappedSoFar;
    }

    function currentLimit() external view returns (uint256) {
        return totalUnlockedAmountOfToken() - swappedSoFar;
    }

    function totalUnlockedAmountOfToken() public view returns (uint256) {
        if (block.timestamp < swapStartsOn) {
            return 0;
        } else if (block.timestamp >= swapStartsOn + swapDuration) {
            return totalSupply() + swappedSoFar;
        } else {
            return (totalSupply() + swappedSoFar) * (block.timestamp - swapStartsOn) / swapDuration;
        }
    }
}
