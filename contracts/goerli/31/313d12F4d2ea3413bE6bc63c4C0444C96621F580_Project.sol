...
contract Project is ERC721, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    constructor(string memory _placeholderTokenURI, address[] memory _admins) payable ERC721("Testing token 6", "TST") // Will be Board to Death, BTD
    {
        // Admins must be defined at the construction of the contract
        // and cannot be changed later on.
        admins = _admins;

        // Make sure tokens start from 1 rather than 0
    ...
    {
        require(whiteList[msg.sender] == true, "This function can only be called from a whitelisted account!");
        _;
    }
    ...
    /*
     * @notice Return the percentage of tokens minted.
     */
    function mintedPercent() public view returns(uint256)
    {
        return (_tokenIdCounter.current()*100) / totalSupply;
    }
    ...
    /*
     * @notice Mint the URI stored in `uri` as an NFT under this contract.
     *
     * IMPORTANT: This function gives too much power and should only 
                    here for testing. Should be removed before release
     */
    ...
    {
        // IMPORTANT: Find out if we start from 0 or 1
    ...
    /*
     * @notice Add an array of accounts to the white list..
     */
    function addWhiteListAccounts(address[] memory accounts) onlyAdmin public
    {
    ...
    /*
     * @notice Remove an array of accounts from the white list.
     */
    function delWhiteListAccounts(address[] memory accounts) onlyAdmin public
    {
        for (uint32 i = 0; i < accounts.length; i++)
        {
     ...
     */
    function whiteListMint(uint16 amount) onlyWhiteList beforeReveal public payable
    {
