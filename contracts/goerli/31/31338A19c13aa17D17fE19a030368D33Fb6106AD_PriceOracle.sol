...
contract PriceOracle {
    ...
    INonfungiblePositionManager public immutable LPToken;
    ...
    constructor(
        ...
        INonfungiblePositionManager _LPToken,
        address _weth
    ) {
        owner = msg.sender;
        factory = _factory;
    ...
    function convertToETH(address token, uint256 amount)
        public
        view
        returns (uint256)
    {
    ...
    function convertLPToken(uint256 tokenId, address token)
        external
        view
        returns (uint256)
    {}
    ...
    function poolIdToPoolKey(uint256 tokenId)
        public
        view
    ...
    {
        PoolKey memory poolKey = poolIdToPoolKey(tokenId);
        Position memory position = positions(tokenId);

        token0 = poolKey.token0;
        token1 = poolKey.token1;

        if (position.liquidity == 0) (token0, token1, 0, 0);
        tokensOwed0 = position.tokensOwed0;
        tokensOwed1 = position.tokensOwed1;

        IUniswapV3Pool pool = IUniswapV3Pool(
            factory.getPool(token0, token1, poolKey.fee)
        );
    ...
    function getArithmeticMeanTick(address pool)
        internal
        view
        returns (int24 tick)
    {
    ...
    function getLargestPool(address token0, address token1)
        internal
        view
