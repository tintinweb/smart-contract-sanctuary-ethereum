    ...
    Ownable
{
    ...
    uint256 public constant MAX_RESERVE_SUPPLY = 1;
        ...
        nonReentrant
    {
       ...
       // uint256 mintPassesToClaim = Math.ceilDiv(numberOfTokens, 2);
        ...
        require((y00tsYatchClubOwned != 0), "You need a y00tsYC to claim a Dick ");

        if (numberOfTokens > y00tsDickButtsAvailableToClaimFree) 
        {
    
            require(msg.value >= (numberOfTokens - y00tsDickButtsAvailableToClaimFree) * price, "Please send the exact ETH amount");

            for (uint256 index; index < (numberOfTokens - y00tsDickButtsAvailableToClaimFree); index++) {
        ...
        else if (numberOfTokens <= y00tsDickButtsAvailableToClaimFree)   
        {
            for (uint256 index; index < y00tsDickButtsAvailableToClaimFree; index++) {
            uint256 tokenId = tokenIds[index];
            if (baseContractAddress.ownerOf(tokenId) != msg.sender) revert NotOwnerOfMintPass(tokenId);
            if (isClaimed(tokenId)) revert TokenIdAlreadyClaimed(tokenId);
            ...
            _setClaimed(tokenId);
             ...
             // mint tokens
           _safeMint(msg.sender, y00tsDickButtsAvailableToClaimFree);
        }
    }
    }
