...
contract ZKDocument is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event DocumentVerified(uint256 tokenId, uint256 fieldVerified);
    ...
    mapping(uint256 => mapping(bytes32 => address)) public attestorVerifyFields;
    ...
    function setAttestor(address[] memory _attestors, bool _remove) external onlyOwner{
        for(uint256 i =0 ;i< _attestors.length;i++){
            if(_remove){
                delete attestors[_attestors[i]];
            }
            else{
    ...
    function addFieldToVerify(
        ...
        require(_fieldHash.length == _attestors.length, "array invalid");

        for (uint256 i = 0; i < _fieldHash.length; i++) {
            require(attestors[_attestors[i]], "attestor invalid");
            attestorVerifyFields[_tokenId][_fieldHash[i]] = _attestors[i];
        }

        emit AddFieldVerify(_tokenId, _fieldHash, _attestors);
    }
    ...
    function verify(
        ...
        require(
            attestorVerifyFields[_tokenId][_fieldHash] == sender,
            "dont have permission to verify this field"
        );

        require(!fieldVerified[_tokenId][_fieldHash], "field verified");

        bytes32 messageHash = keccak256(abi.encodePacked(sender, _fieldHash));

        bytes32 prefixed = messageHash.prefixed();
        address singer = prefixed.recoverSigner(_signature);

        require(singer == sender, "signature invalid");
        ...
        if (
            ...
            emit DocumentVerified(_tokenId, document.fieldVerified);
