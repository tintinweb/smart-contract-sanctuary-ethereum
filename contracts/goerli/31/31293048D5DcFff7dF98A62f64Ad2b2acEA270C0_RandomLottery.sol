...
contract RandomLottery is VRFConsumerBaseV2 {
    ...
    uint256 public _minBet = 0.001 ether;
    uint256 public _minPick = 0.002 ether;
    uint256 public _expired = 1 minutes;

    uint256 public lotteryRound = 0;
    mapping(uint256 => LotteryData) public lotterys;
    ...
    modifier potChecker(uint256 _lotteryId) {
        ...
        require(lottery.pot >= _minPick || block.timestamp > lottery.expiredAt, "not enough pot.");
        _;
    }

    modifier isActived(uint256 _lotteryId) {
        LotteryData storage lottery = lotterys[_lotteryId];
    ...
    // 1000000000000000
    function entry(uint256 _lotteryId) public payable isActived(_lotteryId) notExpired(_lotteryNumber) {
        LotteryData storage lottery = lotterys[_lotteryId];
        require(findParticipantByLotteryId(_lotteryId).id == address(0), "you have a tickit!");
    ...
    function findParticipantByLotteryId(uint256 _lotteryId) public view returns (Participant memory participant) {
        LotteryData storage lottery = lotterys[_lotteryId];
        for(uint256 i; i < lottery.totalParticipants; i++) {
            if(lottery.participant[i].id == msg.sender) {
                return lottery.participant[i];
            }
        }
    }
    ...
    */
    ...
    function pickWinner(uint256 _lotteryId)
        external
        onlyOwner
        isActived(_lotteryId)
        potChecker(_lotteryId)
        lock {
            LotteryData storage lottery = lotterys[_lotteryId];

            if(lottery.expiredAt < block.timestamp) {
                for(uint256 i; i < lottery.totalParticipants; i++) {
    ...
    function getRandomNumber(uint256 _lotteryId)
        ...
        notExpired(_lotteryId) {
            ...
            require(!lottery.isAlreadyRandom, "already request random");
