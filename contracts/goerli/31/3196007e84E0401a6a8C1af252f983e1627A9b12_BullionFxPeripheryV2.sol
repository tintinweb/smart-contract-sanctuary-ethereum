...
// File: contracts/GasOptimization.sol_flattened.sol_flattened.sol
...
// token0 is gold
// token1 is usdc
// PAIR_ADDRESS = 0xc044b084e3fBF5179A14b1af382c85F6F4C2202d;
...
contract BullionFxPeripheryV2 is Ownable {
    ...
    IMergeRouter public constant MERGE_ROUTER =
        IMergeRouter(0x1057a82CADa0c4064008E8818bB083b9f96e5460);

    /// @notice The address of the USDC contract.
    IUSDC public constant USDC =
        IUSDC(0x4D6F6B49A69fa6efC476D7Bdc149B31Ca5944210);

    /// @notice The address of the BullionFX Gold token contract.
    IGold public constant GOLD =
        IGold(0x4cE6Ab1e12249bC6C2A6AD23bA1eDc8096bDD329);

    /// @notice The address associated with the private key used to sign the message.
    address public signer;

    /// @notice The address of the recipient of treasury fees.
    address public treasuryFeeRecipient;

    //@audit - check if packing reduces gas cost?

    /// @notice Treasury fee percentage, measured in basis points
    ...
    /// @notice Used for treasury fee calculation, measured in basis points
    ...
    //@audit - change event name, see what params to emit and what params to index
    event TradeExecuted(
        address indexed trader,
        uint256 indexed goldMinted,
    ...
    //@audit - change errors namd and consider having params
    ...
    constructor(
        ...
        // @audit - confirm it approval works in constructor
    ...
    /// @param _newTreasuryRecipient Address of the new treasury fee recipient.
    function changeTreasuryRecipient(
        address _newTreasuryRecipient
    ) external onlyOwner {
        if (_newTreasuryRecipient == address(0)) {
            revert ZeroAddress();
        }

        emit TreasuryFeeRecipientChanged(
            treasuryFeeRecipient,
            _newTreasuryRecipient
        );

        // Overwrites the old treasury fee recipient address
        treasuryFeeRecipient = _newTreasuryRecipient;
    }
    ...
    //@audit - change fn an event name, set it so it represents that its used to purchase physical gold
    /// @notice Transfers USDC held by this contract. Can only be called by the owner of this contract.
    ...
    function transferUSDC(
        ...
        //@audit - usdc emits transfer event, do we really need to?
    }
    ...
    function swap(
        ...
        // chain id and contract addr?
        // is nonce needed?
        // kinda constant path somehow?
        // if not constant path then use it in sig?
        ...
        // we dont care if user gets more than min amount, right?
        MERGE_ROUTER.swapExactTokensForTokens(
            _amountIn,
            _amountOutMin,
            _path,
            msg.sender,
        ...
        // uint256 goldToMint = totalGold - amounts[amounts.length - 1];
        uint256 _usdcAvailable = _totalUSDC - _amountIn;

        uint256 _goldToMint = _usdcAvailable / _currentGoldPrice;
        uint256 _mintFee = (_goldToMint * treasuryFeePercentage) /
            PERCENTAGE_CALCULATION;

        GOLD.mint(msg.sender, _goldToMint - _mintFee);
        GOLD.mint(treasuryFeeRecipient, _mintFee);
    ...
    //@audit make it public and use in swap fn
    function goldToMint(
        uint256 _totalUSDC,
        uint256 _amountIn,
        uint256 _currentGoldPrice
    ) external view returns (uint256) {
        // @audit isnt it better to send amountIn and usdc available instead of total usdc?
        uint256 _usdcAvailable = _totalUSDC - _amountIn;
        uint256 _goldToMint = _usdcAvailable / _currentGoldPrice;
        uint256 _mintFee = (_goldToMint * treasuryFeePercentage) /
            PERCENTAGE_CALCULATION;

        return (_goldToMint - _mintFee);
    }

    // Generates message hash. Used to avoid stack too deep error in 'swap' function.
