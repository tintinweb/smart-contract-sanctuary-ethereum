...
library json{
    ...
    function lemons(uint256 rI)internal pure returns(string memory){
        ...
        return Base64.encode(bytes(string(abi.encodePacked(base, Strings.toString(rI), '"}]}'))));
    }
...
contract Raffle is ERC721Enumerable, VRFConsumerBase, ReentrancyGuard, UniRouterData{
    using SafeERC20 for IERC20;
    ...
    function primePot()external payable nonReentrant playable{
        require(msg.value >= 0.1 ether, "Needs to equal or greater than Lemon cost");
        raffleData[raffleIndex][0] += msg.value;
    ...
    function mintLemon(uint8 buyAmount)external payable nonReentrant playable returns (uint256[] memory) {
        require(buyAmount > 0 && buyAmount <11, "Can buy between 1-10 Lemons");
    ...
    function consolationAmount(uint256 tokenId)public view returns(uint256){
        ...
        if(index == raffleIndex || winningToken[index] == tokenId || !_exists(tokenId)){return 0;}
        else{
    ...
    function claimConsolationPrize(uint256 tokenId)external returns(uint256){
        ...
        require(msg.sender == user && amount >0, "Lemon: Nothing To Claim");
