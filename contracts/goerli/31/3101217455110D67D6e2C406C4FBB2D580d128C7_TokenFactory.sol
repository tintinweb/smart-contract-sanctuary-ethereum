...
contract TokenFactory {
	 ...
	 */
	address public immutable erc20ImplAddress;

	/**
	 * @dev Fired in deployERC20
	 *
	 * @param erc20ProxyAddress deployed EIP-1167 clone (proxy) address
	 * @param erc20ImplAddress ERC20 impl address, always the same
	 ...
	 */
	event ProxyDeployed(
		address indexed erc20ProxyAddress,
	 ...
	 */
	constructor(address _erc20ImplAddress) {
	...
	function deployERC20(
		...
		address erc20ProxyAddress = Clones.clone(erc20ImplAddress);

		// initialize a proxy by invoking the postConstruct
		// this also sets all the permissions to a ERC20Deployer
		AdvancedERC20(erc20ProxyAddress).postConstruct(_name, _symbol, _initialHolder, _initialSupply);

		// move all the permission to the executor:
		// 1) set the msg.sender's permissions to full
		AccessControl(erc20ProxyAddress).updateRole(msg.sender, type(uint256).max);
		// 2) revoke own permissions
		AccessControl(erc20ProxyAddress).updateRole(address(this), 0);
 ...
 * @author Basil Gorin
 */
contract AdvancedERC20 is ERC1363, EIP2612, EIP3009, AccessControl {
	/**
	...
	constructor(
		...
		postConstruct(_name, _symbol, _initialHolder, _initialSupply);
	}
	...
	function postConstruct(
		...
		// ensure this function is executed only from constructor
		require(!AddressUtils.isContract(address(this)), "invalid context");
		...
		DOMAIN_SEPARATOR = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("AdvancedERC20")), block.chainid, address(this)));
		...
		// the address who initialized the contract gets full privileges
		userRoles[msg.sender] = type(uint256).max; // FULL_PRIVILEGES_MASK
		// emit an ACL event
		emit RoleUpdated(msg.sender, msg.sender, type(uint256).max, type(uint256).max);
	}
