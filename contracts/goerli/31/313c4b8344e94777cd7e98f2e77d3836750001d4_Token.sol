...
// Token(ERC721)
//------------------------------------------------------------
contract Token is Ownable, ERC721Enumerable, ReentrancyGuard, ERC2981, DefaultOperatorFilterer {
    //--------------------------------------------------------
    // event
    //--------------------------------------------------------
    event Consumed( address indexed user, uint256 indexed tokenId );
...
/*
    // mainnet
    string constant private TOKEN_NAME = "TRNSFM";
    string constant private TOKEN_SYMBOL = "TRNSFM";
    address constant private OWNER_ADDRESS = 0xXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX;
*/

    // testnet
    string constant private TOKEN_NAME = "TRANSFORMER";
    string constant private TOKEN_SYMBOL = "TRNSFM";
    ...
    uint256 constant private USER_INFO_TOTAL_MINTED = INFO_MAX + 1;
    ...
    // enum
    uint256 constant private CONSUME_INFO_SUSPENDED = 0;
    uint256 constant private CONSUME_INFO_START = 1;
    uint256 constant private CONSUME_INFO_END = 2;
    uint256 constant private CONSUME_INFO_MAX = 3;
    ...
    string private _base_uri_for_consumed;
    ...
    //+++ CONSUME +++
    bool private _CONSUME_is_suspended;
    uint256 private _CONSUME_start;
    uint256 private _CONSUME_end;
    mapping( uint256 => bool ) private _CONSUME_map_nft_consumed;
...
*/
        ...
        // testnet
        //-----------------------
        _base_uri_for_hidden = "ipfs://QmNZf9DCwYHGRcuV1P8M5NA3RFu63bbXE87FvVa9rVD17x/";
        _token_max = 50;
        _token_reserved = 10;
        _token_mint_limit = 5;
        ...
        // PRIVATE(whitelist)
        //***********************
        _PRIVATE_SALE_start = 1674705600;               // test 2023-01-26 13:00:00(JST)
        _PRIVATE_SALE_end   = 1674707400;               // test 2023-01-26 13:30:00(JST)
        _PRIVATE_SALE_price = 10000000000000000;        // 0.01 ETH
        _PRIVATE_SALE_merkle_root = 0x30c11aec54143903b169f76b8386eb0c3fcefcd39a34b1326bbb842935e2ae60;
        
        //~~~~~~~~~~~~~~~~~~~~~~~
        // PUBLIC
        //~~~~~~~~~~~~~~~~~~~~~~~
        _PUBLIC_SALE_start = 1674707400;                // test 2023-01-26 13:30:00(JST)
        _PUBLIC_SALE_end   = 0;                         // test endless
        _PUBLIC_SALE_price = 12000000000000000;         // 0.012 ETH
    ...
    function approve(address operator, uint256 tokenId) public override( ERC721, IERC721 ) onlyAllowedOperatorApproval(operator) { super.approve(operator, tokenId); }
    function transferFrom(address from, address to, uint256 tokenId) public override( ERC721, IERC721 ) onlyAllowedOperator(from) { super.transferFrom(from, to, tokenId); }
    function safeTransferFrom(address from, address to, uint256 tokenId) public override( ERC721, IERC721 ) onlyAllowedOperator(from) { super.safeTransferFrom(from, to, tokenId); }
    ...
    function baseUriForConsumed() external view returns (string memory) { return( _base_uri_for_consumed ); }
    ...
    function setBaseUriForConsumed( string calldata uri ) external onlyOwnerOrManager { _base_uri_for_consumed = uri; }
    ...
    function tokenURI( uint256 tokenId ) public view override returns (string memory) {
        ...
        if( bytes(_base_uri_for_consumed).length > 0 && _CONSUME_map_nft_consumed[tokenId] ){
            return( string( abi.encodePacked( _base_uri_for_consumed, Strings.toString( tokenId ) ) ) );             
        }
    ...
    function _mintTokens( address to, uint256 num ) internal {
        require( _total_minted >= _token_reserved, "reservation not finished" );
        require( (_total_minted+num) <= _token_max, "exceeded the supply range" );
        require( _token_mint_limit == 0 || _token_mint_limit >= num, "reached mint limitation" );

        uint256 tokenId = _total_minted + TOKEN_ID_OFS;
        _total_minted += num;
    ...
    function reserveTokens( uint256 num ) external onlyOwnerOrManager {
        require( (_total_minted+num) <= _token_reserved, "exceeded the reservation range" );
        require( (_total_minted+num) <= _token_max, "exceeded the supply range" );
    ...
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // [external/onlyOwnerOrManager] write - CONSUME
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    function CONSUME_suspend( bool flag ) external onlyOwnerOrManager { _CONSUME_is_suspended = flag; }
    function CONSUME_setStartEnd( uint256 start, uint256 end ) external onlyOwnerOrManager { _CONSUME_start = start; _CONSUME_end = end; }

    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // [external/nonReentrant] consume - CONSUME
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    function CONSUME_consume( uint256 tokenId ) external nonReentrant {
        require( ! _CONSUME_is_suspended, "CONSUME: suspended" );
        require( _CONSUME_start != 0 && _CONSUME_start <= (block.timestamp+BLOCK_SEC_MARGIN), "CONSUME: not started" );    // 0は無効(開始時間の明示が必要)
        require( _CONSUME_end == 0 || (_CONSUME_end+BLOCK_SEC_MARGIN) > block.timestamp, "CONSUME: finished" );
        require( _exists( tokenId ), "nonexistent token" );
        require( ownerOf( tokenId ) == msg.sender, "CONSUME: not owned nft" );
        require( ! _CONSUME_map_nft_consumed[tokenId], "already consumed nft" );

        // event
        emit Consumed( msg.sender, tokenId );

        // consume
        _CONSUME_map_nft_consumed[tokenId] = true;
    }

    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // [external] getConsumeInfo
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    function getConsumeInfo() external view returns (uint256[CONSUME_INFO_MAX] memory) {
        uint256[CONSUME_INFO_MAX] memory consumeInfo;

        if( _CONSUME_is_suspended ){ consumeInfo[CONSUME_INFO_SUSPENDED] = 1; }
        consumeInfo[CONSUME_INFO_START] = _CONSUME_start;
        consumeInfo[CONSUME_INFO_END] = _CONSUME_end;

        return( consumeInfo );
    }

    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // [external] 保有トークン情報：「tokenId + consumed(0 or 1)」で１セット
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    function getTokenInfo( address owner, uint256 pageSize, uint256 pageOfs ) external view returns (uint256[] memory) {
        ...
        uint256[] memory info = new uint256[](2*num);
        for( uint i=0; i<num; i++ ){
            uint256 tokenId = tokenOfOwnerByIndex( owner, ofs+i );
            info[2*i+0] = tokenId;
            if( _CONSUME_map_nft_consumed[tokenId] ){ info[2*i+1] = 1; }
        }
