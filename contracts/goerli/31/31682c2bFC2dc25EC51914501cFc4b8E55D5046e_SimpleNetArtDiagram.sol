...
contract SimpleNetArtDiagram is Ownable, IERC721Metadata, ERC721Royalty {
    ...
    /**
     * @dev Contract owner may mint max supply to the contract owner only once
     */
    ...
    /**
     * @dev I can't think of a reason someone would want to burn
    ...
    /**
     * @dev Returns the Uniform Resource Identifier (URI) for `_tokenId` token if it's a valid token id
     */
    function tokenURI(uint256 _tokenId) public view override(ERC721, IERC721Metadata) returns (string memory) {
        ...
        ownerOf(_tokenId);
        string memory edition = string(abi.encodePacked(_tokenIdToString(_tokenId), "/3"));
        string memory metadata = Base64.encode(
            abi.encodePacked(
                ...
                '","edition":"',
                edition,
                '","attributes": [{"trait_type":"Artist","value":"MTAA"},{"trait_type":"License","value":"CC0 1.0 ipfs://QmZcU7ZkmVSNfVZjsxoHSoCtw89Az5hmqufLPowZxCURn8"},{"trait_type":"Edition", "value":"',
                edition,
                '"}]}'
            )
        );
