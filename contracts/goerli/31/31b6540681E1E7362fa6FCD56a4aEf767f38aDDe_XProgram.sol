...
interface IMatrix {
	...
	function SponsorLevelTracking(
		uint _AID
	) external view returns (uint F1SL2_, uint F1SL5_, uint F2SL2_, uint F3SL2_);
}
...
interface ITuktu {
	function Initialize(IXProgram _XProgram, IMatrix _Matrix) external;
...
interface IXProgram {
	...
	function Initialize(IAccount _Account, IBalance _Balance, ITuktu _Tuktu) external;
...
abstract contract Matrix is IMatrix, Ownable2Step {
	...
	function _CFPendingOf(uint _AID, uint _MATRIX) private view returns (uint[3] storage CF_) {
	...
	function InitializeMatrix(uint _Starting) private {
		SLOf[_Starting] = 1;
	}
	...
	function _InitMaxtrixes(uint _AID, uint _SID, uint _UB, uint _UT) external onlyTuktu {
		...
		// Unilevel matrix
		L1ID[_SID][UNILEVEL].push(_AID);
		UID[_AID][UNILEVEL] = _SID;

		// Update sponsor level for upline when node changes from SL1 to SL2
		if (L1ID[_SID][UNILEVEL].length == 3) _UpdateSponsorLevelForUpline(_AID);

		// Binary matrix
		if (_VerifyUplineID(_AID, _SID, _UB, BINARY)) {
			L1ID[_UB][BINARY].push(_AID);
			UID[_AID][BINARY] = _UB;
		} else revert("Verify UID BINARY: fail");

		// Ternary matrix
		if (_VerifyUplineID(_AID, _SID, _UT, TERNARY)) {
			L1ID[_UT][TERNARY].push(_AID);
			UID[_AID][TERNARY] = _UT;
	...
	function _VerifyUplineID(uint _AID, uint _SID, uint _UID, uint _MATRIX) private returns (bool Success_) {
		...
		uint[3] storage cf = _CFPendingOf(_AID, _MATRIX);
		cf[0] = _CFPendingOf(_UID, _MATRIX)[0] + 1; // XR

		if (_SID == _UID) {
			cf[1] = 1; // XS

			if (_UplineOnMatrix(_SID, _MATRIX) == 0 && cf[0] == cf[1]) return (true);
			if (_UplineOnMatrix(_SID, _MATRIX) != 0 && cf[0] != cf[1]) return (true);
...
contract XProgram is IXProgram, Matrix {
	...
	function Initialize(IAccount _Account, IBalance _Balance, ITuktu _Tuktu) public onlyOwner {
		Accounts = _Account;
		Balances = _Balance;
		Tuktu = _Tuktu;
	}
