...
import '@proak-us/acs-erc721a-upgradeable/contracts/ERC721AUpgradeable.sol';
import '@proak-us/acs-erc721a-upgradeable/contracts/extensions/ERC721ABurnableUpgradeable.sol';
...
import "./utils/TreasuryUpgradeable.sol";
import "./interfaces/IERC2981Upgradeable.sol";
import "./interfaces/IERC721AProAKUpgradeable.sol";

/**
 * @title ProAK NFTs implemented using the ERC-721A standard.
 * @dev This top level file holds no data directly to ease future upgrades.
 */
contract ERC721AProAKUpgradeable is
TreasuryUpgradeable,
ERC721AUpgradeable,
IERC2981Upgradeable,
IERC721AProAKUpgradeable,
OwnableUpgradeable,
...
UUPSUpgradeable
{
    address private creatorInternalAddress;
    uint96 private _defaultRoyaltyInfo;
    ...
    function initialize(
        ...
        address payable _treasuryPaymentAddress,
        address payable _creatorPaymentAddress,
        address _creatorInternalAddress,
        uint256 _secondaryTreasuryFeeBasisPoints,
        uint256 _secondaryCreatorFeeBasisPoints
    ) initializerERC721A initializer public {
        __TreasuryNode_init(_treasuryPaymentAddress, _creatorPaymentAddress, _secondaryTreasuryFeeBasisPoints, _secondaryCreatorFeeBasisPoints);
        ...
        _grantRole(ADMIN_ROLE, _treasuryPaymentAddress);
        ...
        creatorInternalAddress = _creatorInternalAddress;
    }
    ...
    function getCategory(
        uint256 category
    ) public view returns (Categories memory) {
        return categories[category];
    }

    function getMintedByAddress(
        address to, 
        uint256 sales
    ...
    function getMintedByCategory(
        uint256 category
    ) public view returns (uint256) {
        return _totalMintedByCategories(category);
    }
    ...
    function safeMint(
        ...
        require(_totalMintedByAddress(to, sales) + amount <= salesPhase[sales].maxPerWallet || salesPhase[sales].maxPerWallet == 0, "maxPerWallet limit.");
        require(msg.value >= getPrice(amount, sales), "Mint price is not correct.");
        if(salesPhase[sales].hasWhitelist) {
            if (ecrecover(keccak256(abi.encode(sales, to)), v, r, s) != creatorInternalAddress) revert InvalidCoupon();
        }
    ...
    function _setTokenURI(
        uint256 tokenId, 
        uint256 _currentCategoriesIndex, 
        uint256 category
    ) internal virtual {
    ...
    function royaltyInfo(
        ...
        return (address(this), (_salePrice * categories[tokenCategory[_tokenId].category].royaltyFraction) / _feeDenominator());
    }
    ...
    view
    virtual
    override(ERC721AUpgradeable, AccessControlUpgradeable, IERC165Upgradeable, IERC721AUpgradeable)
    returns (bool)
    {
     ...
     *
     * If `newOwner` is not zero address.
     *
    ...
    /**
     * @dev Change creator payment address `_to`.
     *
     * If `balance` had been granted admin role, emits a {RoleRevoked} event.
     *
     * @param _to address payable of the new creator payment account
     * Requirements:
     *
     * - the caller must have admin role.
     */
    function changeCreatorPaymentAddress(
        address payable _to
    ) public onlyOwner {
        _revokeRole(ADMIN_ROLE, getCreatorPaymentAddress());
    ...
    ) internal onlyRole(ADMIN_ROLE) override {}
    ...
    function withdraw() external onlyRole(ADMIN_ROLE) {
        (uint256 secondaryTreasuryFeeBasisPoints, uint256 secondaryCreatorFeeBasisPoints) = getRevenueConfig();
        uint256 balance = address(this).balance;
        //Pay to Treasury
        payable(getTreasuryPaymentAddress()).transfer((balance * secondaryTreasuryFeeBasisPoints) / 100);
        //Pay to Creator
        payable(getCreatorPaymentAddress()).transfer((balance * secondaryCreatorFeeBasisPoints) / 100);
    }

    function safeReserveMint(
        address to, 
        uint256 amount, 
        uint256 category
    ) public onlyRole(ADMIN_ROLE) {
        if (_totalMintedByCategories(category) + amount > categories[category].tokenSupply || categories[category].mintReserve <= 0) revert NoCategoryReserveSupplyToMint();
        uint256 _currentIndex = ERC721AStorage.layout()._currentIndex;
        uint256 _currentCategoriesIndex = ERC721AStorage.layout()._currentCategoriesIndex[category];
        _safeMint(to, amount, 0, category);
        _setTokenURI(_currentIndex, _currentCategoriesIndex, category);
        categories[category].mintReserve -= 1;
    }

    function setSalesPhase(
        Sales memory _newSalesPhase, 
        uint256 position
    ) public onlyRole(ADMIN_ROLE) {
        if (position <= 0) revert SalesPhaseNotAllowed();
        salesPhase[position] = _newSalesPhase;
        emit SalesPhase(_newSalesPhase, position);
    }

    // ** -------------------------------- ADMIN - CATEGORY ---------------------------------- ** //

    function setCategory(
        Categories memory _category, 
        uint256 position
    ) public onlyRole(ADMIN_ROLE) {
        if (categories[position].isFrozen) revert CategoryFreezed();
        categories[position] = _category;
    }
    ...
    function updateCategory(
        ...
        if (_totalMintedByCategories(position) + _mintReserve > categories[position].tokenSupply) revert CategoryReserveWillExceed();
        if (_royaltyFraction > _feeDenominator()) revert RoyaltyFeeWillExceed();
        categories[position].mintReserve = _mintReserve;
        categories[position].tokenSupply = _tokenSupply;
        categories[position].royaltyFraction = _royaltyFraction;
    }

    function setCategoryFreeze(
        uint256 position
    ) public onlyRole(ADMIN_ROLE) {
        if (categories[position].isFrozen != false) revert CategoryFreezed();
        categories[position].isFrozen = true;
    }
...
contract ERC721AUpgradeable is ERC721A__Initializable, IERC721AUpgradeable {
     ...
     */
    function _mint(address to, uint256 quantity, uint256 sales, uint256 category) internal virtual {
...
import '@proak-us/acs-erc721a-upgradeable/contracts/IERC721AUpgradeable.sol';
/**
 * @notice A reference to the treasuryPaymentAddress contract.
 */
abstract contract TreasuryUpgradeable is Initializable, IERC721AUpgradeable {
    using AddressUpgradeable for address payable;

    address payable private treasuryPaymentAddress;
    address payable private creatorPaymentAddress;
    uint256 private secondaryTreasuryFeeBasisPoints;
    uint256 private secondaryCreatorFeeBasisPoints;

    /**
     * @dev Called once after the initial deployment to set the treasuryPaymentAddress address.
     */
    function __TreasuryNode_init(address payable _treasuryPaymentAddress, address payable _creatorPaymentAddress, uint256 _secondaryTreasuryFeeBasisPoints, uint256 _secondaryCreatorFeeBasisPoints) internal initializer {
        if (_treasuryPaymentAddress == address(0)) revert TransferToZeroAddress();
        if (_creatorPaymentAddress == address(0)) revert TransferToZeroAddress();

        treasuryPaymentAddress = _treasuryPaymentAddress;
        creatorPaymentAddress = _creatorPaymentAddress;
        secondaryTreasuryFeeBasisPoints = _secondaryTreasuryFeeBasisPoints;
        secondaryCreatorFeeBasisPoints = _secondaryCreatorFeeBasisPoints;
    }

    /**
     * @dev Returns the address of the treasuryPaymentAddress.
     */
    function getTreasuryPaymentAddress() public view returns (address payable) {
        return treasuryPaymentAddress;
    }
     ...
     * @dev Returns the address of the creator.
     */
    function setCreatorPaymentAddress(address payable _to) internal virtual {
        creatorPaymentAddress = _to;
    }

    /**
     * @dev Get Revenue share configuration.
     * Returns the revenue share for Treasury `uint256` and Creator `uint256`.
     */
    function getRevenueConfig() public view
    returns (uint256, uint256) {
        return (secondaryTreasuryFeeBasisPoints,secondaryCreatorFeeBasisPoints);
    }
...
/**
 * @dev Interface for the ERC721AProAK Standard.
 *
 */
interface IERC721AProAKUpgradeable is IERC165Upgradeable {
    /**
     * ERC721AProAK: Category not allowed to update.
     */
    error CategoryFreezed();

    /**
     * ERC721AProAK: No Category reserve left to mint.
     */
    error NoCategoryReserveSupplyToMint();

    /**
     * ERC721AProAK: Category reserve will exceed total supply.
     */
    error CategoryReserveWillExceed();

    /**
     * ERC721AProAK: Invalid coupon for this sales phase.
     */
    error InvalidCoupon();
    ...
    struct Categories {
        ...
        bool isFrozen; // If Category is freezed
        uint256 tokenSupply; // Total Supply of NFTs available to this category
        uint96 royaltyFraction; // The royalty amount
    }
    ...
    /**
     * @dev Emitted when salesPhase at `position` token is changed to `newSalesPhase`.
     */
    event SalesPhase(Sales indexed newSalesPhase, uint256 indexed position);
}
