...
contract FiberRouter is ReentrancyGuardUpgradeable, OwnableUpgradeable {
     ...
     */
    function setOracle(address _token, address _oracleAddress) external onlyOwner {
        priceFeed[_token] = AggregatorV3Interface(_oracleAddress);
    }


    function getFoundryTokenPrice(address _token) public view returns(uint){
         (
            /*uint80 roundID*/,
            int price,
            /*uint startedAt*/,
        ...
        return uint(price)*10**(18-baseDecimals);
        // return uint(price);
    }
    ...
    */
    ...
    function withdrawSignedAndSwapToFoundry(
        ...
        uint bridgeFoundryPrice = getFoundryTokenPrice(bridgeFoundry);
        uint targetFoundryPrice = getFoundryTokenPrice(targetFoundry);
        uint amountOut = amount*bridgeFoundryPrice/targetFoundryPrice;
        FundManager(pool).withdrawSigned(
            targetFoundry,
            payee,
            amountOut,
