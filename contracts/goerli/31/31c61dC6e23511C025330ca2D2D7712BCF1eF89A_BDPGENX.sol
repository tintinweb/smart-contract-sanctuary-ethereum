...
contract TOKEN is ERC721AQueryable, ERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 public constant PUB_PRICE = 0.002 ether;
    ...
    address public royaltyAddress; // 仮入れ
    ...
    function _mintCheck(
        ...
        require(_quantity <= MINT_LIMIT, "Mint quantity over");
        require(msg.value >= _cost, "Not enough funds");
        require(
            claimed[msg.sender] + _quantity <= MINT_LIMIT,
            "Already claimed max"
        );
    }
    ...
    function withdrawRevenueShare() external onlyOwner {
        ...
        address cre = payable(0x8E0b280135F2C90aad79b464f23C01C9751A9f2D); // 仮入れ
        address dev = payable(0x8E0b280135F2C90aad79b464f23C01C9751A9f2D); // 仮入れ
        address mer = payable(0x8E0b280135F2C90aad79b464f23C01C9751A9f2D); // 仮入れ

        bool success;

        (success, ) = cre.call{value: ((sendAmount * 7000) / 10000)}("");
        require(success, "Failed to withdraw Ether");

        (success, ) = dev.call{value: ((sendAmount * 2500) / 10000)}("");
        require(success, "Failed to withdraw Ether");

        (success, ) = mer.call{value: ((sendAmount * 500) / 10000)}("");
        require(success, "Failed to withdraw Ether");
    }
