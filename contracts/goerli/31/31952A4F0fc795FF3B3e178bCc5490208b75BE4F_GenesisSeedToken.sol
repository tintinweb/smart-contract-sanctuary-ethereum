...
contract GenesisSeedToken is ERC721, ERC721Enumerable, AccessControl, Ownable, IGenesisSeedToken {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '", "description": "The most adorable goREEL token ever! This token is a representation of your early seed investment in the goREEL project. It is a non-fungible token (NFT) that can be used to receive airdrops of future goREEL tokens and other valuable prizes. It is also a collectible, and can be traded on the secondary market."',
                        // getTraits(_tokenId),
                        '"image":',
                        _baseImageURI,
                        Strings.toString(_tokenId),
     ...
     */
    function setBaseImageURI(string memory baseImageURI_) external onlyOwner {
        if (metadataFrozen) revert MetadataIsFrozen();

        _baseImageURI = baseImageURI_;
    }

    // needed from ERC721Enumerable
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 tokenId,
