...
contract DAOStaking is IModule, IDAOV1Staking, Initializable, AccessControlUpgradeable {
    ...
    function enforcePrerequisites(address registryAddr, bytes32 orgId) external view {
        ...
        require(address(govToken) == registry.getModuleValue(orgId, Modules.TOKEN_V1, 0), "Prerequisites: govToken address must match registry");
    }

    function enforceSafeRemove(address registryAddr, bytes32 orgId) external view {
        IRegistry registry = IRegistry(registryAddr);

        require(!registry.hasModule(orgId, Modules.DAO_V1_GOVERNANCE), "SafeRemove: module still required");
        require(!registry.hasModule(orgId, Modules.DAO_V1_REWARDS), "SafeRemove: module still required");
    }
