...
interface ICustomerContract {
    function relyRiskScore(uint8 riskScore, bytes32 _txn_hash) external returns (bool status);
    function handle(
        uint32 _origin, 
        uint32 _nonce,
        bytes32 _sender,
...
contract KYTContract is Ownable{
    uint fee = 0 wei;
    string public constant KYT_VERSION = "0.0.2";
    ...
    function executeHandle(bytes32 _txn_hash, address _contract, uint8 _risk_score, 
        ...
        if (ICustomerContract(_contract).relyRiskScore(_risk_score, _txn_hash)) {
