...
interface IERC721Sig {
     ...
     *  @param saleRecipient The recipient of the minted token's primary sales proceeds.
...
/**
 *  @title   Ownable
 *  @notice  Unseen's `Ownable` is a contract extension to be used with any base contract. It exposes functions for setting and reading
 *           who the 'owner' of the inheriting smart contract is, and lets the inheriting contract perform conditional logic that uses
 *           information about who the contract's owner is.
 */

abstract contract Ownable is IOwnable {
    /// @dev Owner of the contract (purpose: Unseen compatibility)
...
abstract contract ERC721Sig is EIP712, IERC721Sig {
    using ECDSA for bytes32;

    bytes32 private constant TYPEHASH =
        keccak256(
            "MintRequest(address to,address saleRecipient,uint256 quantity,uint256 pricePerToken,address currency,uint128 validityStartTimestamp,uint128 validityEndTimestamp,bytes32 uid)"
        );

    /// @dev Mapping from mint request UID => whether the mint request is processed.
    mapping(bytes32 => bool) private minted;

    constructor() EIP712("ERC721Sig", "1") {}
...
/**
 *  Unseen's `Ownable` is a contract extension to be used with any base contract. It exposes functions for setting and reading
 *  who the 'owner' of the inheriting smart contract is, and lets the inheriting contract perform conditional logic that uses
 *  information about who the contract's owner is.
 */
...
contract ERC721Base is ERC4907A, ERC721AQueryable, IContractMetadata, Ownable {
    ...
    modifier hasMinterRole(address _signer) {
        require(_minterRole[_signer], "Unseen: Not authorized");
        _;
    }
    ...
    function saleRecipient() public view returns (address) {
    ...
    function _grantMinterRole(address _signer) external onlyOwner {
        require(
            !_minterRole[_signer],
            "Unseen: Signer already have minter role"
        );
        _minterRole[_signer] = true;
