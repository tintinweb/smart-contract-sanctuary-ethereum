...
contract VLFIV2 is ERC20Upgradeable, ERC20PermitUpgradeable, AccessControlUpgradeable, ERC20VotesUpgradeable {
    ...
    address private lionGamingContract;
...
/// @notice initialize function called by Openzeppelin Hardhat upgradeable plugin
        ...
        onlyRole(HOUSE_POOL_DATA_PROVIDER)
    {
        int256 tempPendingStakes;
            tempPendingStakes += int256(betinformation.amount * 10**precisionDifference);
            betInfoWithId[betinformation.Id].parlay = betinformation
                .parlay;
            betInfoWithId[betinformation.Id].user = betinformation.user;
            betInfoWithId[betinformation.Id].Id = betinformation.Id;
            betInfoWithId[betinformation.Id].amount = betinformation
                .amount;
            betInfoWithId[betinformation.Id].result = betinformation
                .result;
            betInfoWithId[betinformation.Id].payout = betinformation
                .payout;
            betInfoWithId[betinformation.Id].commission = betinformation
                .commission;
            userBets[betinformation.user].push(betinformation.Id);
    ...
    function updateBet(
        ...
        uint256  _payout
            ...
            if (betInfoWithId[_Id].parlay) {
                betInfoWithId[_Id].payout = _payout;
            }  
    }
    ...
    function settleBet(uint256 _Id, uint256 _result)
        external
        onlyRole(HOUSE_POOL_DATA_PROVIDER)
    {
            if (_result == 1) {
                betInfoWithId[_Id].result = _result;
                liquidity -= (betInfoWithId[_Id].payout * 10**precisionDifference +
                    betInfoWithId[_Id].amount * 10**precisionDifference);
                bool success = STAKED_TOKEN.transfer(
                    lionGamingContract,
                    betInfoWithId[_Id].payout
                );
                if (success) {
                    pendingStakes -= int256(betInfoWithId[_Id].amount * 10**precisionDifference);
                    emit BetSettled(_Id, _result);
                    updateAttributes();
                } else {
                    revert();
                }
                ...
                betInfoWithId[_Id].result = _result;
                liquidity += betInfoWithId[_Id].amount * 10**precisionDifference;
                pendingStakes -= int256( betInfoWithId[_Id].amount * 10**precisionDifference);
                updateAttributes();
            }
    }
    ...
    function stake(address onBehalfOf, uint256 amount) public {
        ...
        runningTotalDeposits += amount * 10**precisionDifference;
        uint256 lpTokensToMint = (amount * 10**precisionDifference * 10**MAX_PRECISION) / lpTokenPrice;
    ...
    function unStake(address to, uint256 amount) public {
        ...
        uint256 tokensToBurn = (amount * 10**precisionDifference * 10**MAX_PRECISION) / lpTokenPrice;
        ...
        user.rewardDebt -= int256(
            (
                (((amount * 10**precisionDifference * 10**MAX_PRECISION) * farm.accRewardsPerShare) /
                    lpTokenPrice)
            ) / ACC_REWARD_PRECISION
        );
