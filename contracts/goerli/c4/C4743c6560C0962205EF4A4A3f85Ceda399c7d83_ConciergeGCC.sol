...
contract ConciergeGCC is Ownable, ReentrancyGuard {
    ...
    // Public view reads staking info mapping based on staker address
    function getStakingInfo(address staker) public view returns (StakingInfo memory) {
    ...
    function stakeAndPurchase(uint256 tokenId) public nonReentrant {
        ...
        // check & approve USDC spend
        if(tokenContract.allowance(msg.sender, address(this)) < PRICE) {
            tokenContract.approve(address(this), PRICE);
        }        

        // transferrs USDC payment
