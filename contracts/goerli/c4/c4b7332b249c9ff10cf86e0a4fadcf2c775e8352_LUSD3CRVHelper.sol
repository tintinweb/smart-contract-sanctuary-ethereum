...
contract LUSD3CRVHelper {
    ...
    function testCurveVirtualPrice() public returns (bool){
        uint256 fetchedVirtualPrice = ICurvePool(curve3Pool).get_virtual_price();

        uint256[3] memory balances;
        for (uint256 i=0; i<3; ++i){
            balances[i] = ICurvePool(curve3Pool).balances(i);
        }
        uint256 tokenSupply = tokenAddress.totalSupply();
        
        uint256 blockTimeStamp = block.timestamp;
        uint256 futureATime = ICurvePool(curve3Pool).futureATime();
        uint256 futureA = ICurvePool(curve3Pool).futureA();
        uint256 initialA = ICurvePool(curve3Pool).initialA();
        uint256 initialATime = ICurvePool(curve3Pool).initialATime();

        uint256 computedPrice = _get_virtual_price(balances,tokenSupply,blockTimeStamp, futureATime, futureA, initialA, initialATime);

        emit LogVirtualPrice(fetchedVirtualPrice, computedPrice);
    }

    function _A(uint256 blockTimeStamp, uint256 futureATime, uint256 futureA, uint256 initialA, uint256 initialATime) private returns (uint256){
        ...
        } else { // when t1 == 0 or blockTimeStamp >= t1
            return a1;
        }
    }

    function _xp(uint256[3] memory balances) private pure returns(uint256[3] memory){
        uint256[3] memory result = [uint256(1000000000000000000),uint256(1000000000000000000000000000000),uint256(1000000000000000000000000000000)];
    ...
    function _get_D(uint256[3] memory xp, uint256 amp) private returns (uint256){
        uint256 s = 0;
    ...
    function _get_virtual_price(uint256[3] memory balances,uint256 tokenSupply,  uint256 blockTimeStamp, uint256 futureATime, uint256 futureA, uint256 initialA, uint256 initialATime) private returns(uint256){
        uint256 d = _get_D(_xp(balances),_A(blockTimeStamp,futureATime,futureA,initialA,initialATime));
