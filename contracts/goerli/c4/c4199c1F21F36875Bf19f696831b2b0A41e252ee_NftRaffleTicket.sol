...
contract NftRaffleTicket is Ownable {
    ...
    mapping(uint256 => uint256[]) public raffleTicketsById;
    mapping(uint256 => address) ticketIdByUser;
    mapping(address => mapping(uint256 => uint256)) participantAmount;
    mapping(uint256 => mapping(address => uint256[]))
        public getTicketsByRaffleUser;

    event RaffleTicketCreated(
    ...
    function raffleTicketCreated(
        ...
        IERC721Receiver(address(this)).onERC721Received(
            nftContract,
            _msgSender(),
            nftId,
        ...
        TicketId.increment();
        uint256 raffleId = randomSelection(msg.sender).add(TicketId.current());
    ...
    function enterParticipant(uint256 raffleId, uint256 numberOfTickets)
        external
        payable
    {
        require(
            block.timestamp <= raffleTickets[raffleId].endDate,
            "Raffle competition is Over"
        );
        require(numberOfTickets > 0, "Minimum 1 Ticket should be there");
        require(
            msg.value >=
                (raffleTickets[raffleId].ticketPrice).mul(numberOfTickets),
            "Minimum Amount is Required"
        );
        raffleParticipant[raffleId].push(_msgSender());
        participantAmount[_msgSender()][raffleId] = (
            participantAmount[_msgSender()][raffleId]
        ).add(msg.value);
        for (uint256 i = 0; i < numberOfTickets; i++) {
            TicketId.increment();
            uint256 ticketId = TicketId.current();
            ...
            raffleTickets[raffleId].participants =
                raffleTickets[raffleId].participants +
                1;
            emit EnterParticipant(
                raffleId,
                numberOfTickets,
                _msgSender(),
                ticketId,
    ...
    function winnerSelected(uint256 raffleId) external returns (address) {
        require(
            block.timestamp > raffleTickets[raffleId].endDate,
            "Raffle competition is Not Over yet"
        );
        require(
            raffleTickets[raffleId].owner == msg.sender ||
                owner() == msg.sender,
            "Caller is not Owner or Admin"
        );
        if (
            raffleTickets[raffleId].participants >=
            raffleTickets[raffleId].min_ticketSize
        ) {
            uint256 winnerTicketId = raffleTicketsById[raffleId][
                randomSelection(msg.sender) % raffleTicketsById[raffleId].length
            ];
            address winner = ticketIdByUser[winnerTicketId];
            ERC721(raffleTickets[raffleId].nftContract).safeTransferFrom(
                address(this),
                winner,
                raffleTickets[raffleId].nftId
            );
            uint256 totalAmount = (raffleTickets[raffleId].ticketPrice).mul(
                raffleTickets[raffleId].participants
            );
            uint256 platformAmount = totalAmount.mul(platformFee).div(100);
            (bool success, ) = _msgSender().call{
                value: totalAmount.sub(platformAmount)
            }("");
            require(
                success,
    ...
    function claimUserdraw(
        ...
        uint256 amount = participantAmount[userAddress][raffleId];
        ...
        participantAmount[userAddress][raffleId] = participantAmount[
            userAddress
        ][raffleId].sub(amount);
    }

    function changePlatformFee(uint256 amount) external onlyOwner {
        platformFee = amount;
    }
    ...
    function removeWhiteListNftAddress(uint256 index) public onlyOwner {
    ...
    function withdrawNft(address nftContract, uint256 nftId) public onlyOwner {
        ERC721(nftContract).safeTransferFrom(address(this), owner(), nftId);
    }

    ////////////////////// View Function //////////////////////
    function getRaffleTicketsByRaffleUser(uint256 raffleId, address userAddress)
        public
        view
    ...
    function getRaffleTicketsByRaffle(uint256 id)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getRaffleParticipant(uint256 id)
        public
        view
    ...
    function raffleStatus(uint256 id) public view returns (RaffleStatus) {
        return raffleTickets[id].status;
    }

    function claimBalance(uint256 id, address userAddress)
        public
        view
        returns (uint256)
    {
    ...
    function getRaffle(uint256 id) public view returns (Raffle memory) {
        return raffleTickets[id];
    }

    function getRaffleUniqueId() public view returns (uint256[] memory) {
    ...
    function randomSelection(address userId) internal view returns (uint256) {
            ...
            keccak256(
                abi.encodePacked(
                    block.timestamp +
                        ...
                        block.gaslimit +
                        ((uint256(keccak256(abi.encodePacked(userId)))) /
                            (block.timestamp)) +
                        block.number
                )
            )
        ...
        return (uniqueId -
            ((uniqueId / 1000000000000000000) * 1000000000000000000));
    }
}
