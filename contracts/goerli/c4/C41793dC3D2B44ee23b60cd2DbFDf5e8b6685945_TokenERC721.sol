...
// File contracts/token/LockERC721.sol
    ...
    ITokenERC721
{
    ...
    /// @dev ONEOF Mapping from tokenId  => isLocked boolean
    //  false by default, all NFTs unlocked; can be locked per tokenId
    mapping(uint256 => bool) private nftLockStatus;
    ...
    ///     =====   ONEOF      =====
    ///     =====   Modifiers  =====
    modifier nftUnlocked(uint256 tokenId)  {
        require(!nftLockStatus[tokenId]);
        _;
    }
    ...
    /// @dev ONEOF Lets a module admin lock an NFT for any duration in seconds
    function setNFTLockstatus(uint256 tokenId, bool lock) external onlyRole(DEFAULT_ADMIN_ROLE) {
        nftLockStatus[tokenId]= lock;
        if(lock){
    ...
    function getLockStatus(uint tokenId) external view returns(bool){
        return nftLockStatus[tokenId];
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual whenNotPaused nftUnlocked(tokenId) override(ERC721EnumerableUpgradeable) {
        super._beforeTokenTransfer(from, to, tokenId);
