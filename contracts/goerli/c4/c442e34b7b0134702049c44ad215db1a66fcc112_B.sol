...
contract B {
    ...
    struct user {
        ...
        mapping(string => VOTE) voted; // 문제
    }
    enum VOTE {NONE, AGREE, DISAG}
    ...
    function getUser2(string memory _a) public view returns(VOTE) {
        return users[msg.sender].voted[_a];
    }

    function vote(string memory _title, bool _b) public {
        require(users[msg.sender].voted[_title] == VOTE.NONE,"ALREADY VOTED");
        require(Polls[_title].status == Status.ONGOING,"CANNOT VOTE.");
        ...
        if(keccak256(bytes(Polls[_title].title)) == keccak256(bytes(_title))) { //title과 _title 비교, string은 직접 비교가 아닌 hash값으로
            ...
            if(_b == true) {
                ...
                users[msg.sender].voted[_title] = VOTE.AGREE;  // voted는 string과 bool로 구성(key -value), _title을 key값으로 넣고 _b를 value 값으로 넣음.
            } else {
                Polls[_title].disag++;
                users[msg.sender].voted[_title] = VOTE.DISAG;
            }
            if(Polls[_title].agree + Polls[_title].disag >= 10){
                if(Polls[_title].agree * 3 >= Polls[_title].disag * 7){
