...
/**
 * @title : Block Rights contract
 * 
 * @dev Operation Rights for Block plot 
...
contract BlockRights is ERC721Metadata, Extend {
    ...
    event ExtendRights(uint256 rightsId, uint256 ExtendId, uint256 second, address extendcontract);
    ...
    constructor( address _address, string memory name, string memory symb, address landnft) ERC721(name, symb) 
    {
        ...
        _setBaseURI(string(abi.encodePacked("https://pfp.allimeta.world/lands/blockrights/", symb, "/")));
    }
   ...
   function onERC721Received(address operator, address from, uint256 tokenId, bytes memory data) public returns (bytes4) 
     ...
     * @param tokenId The rightsId to mint.
     * @param sectime  The period what let rights use
     * @param enable turn it on or off     
     */
   function lease_mint(address to, uint256 tokenId, uint256 sectime, bool enable) internal returns (bool) 
    {
        ...
        tiles[tokenId] = Tile(false, 0, 0, exists);             
        if( sectime <= LeaseMax )
        {
            tiles[tokenId].leasetime = sectime;
        }
        ...
        emit LeasePlotMinted(to, tokenId, tiles[tokenId].leasetime, tiles[tokenId].enable );
   ...
   function LeaseExtend( uint256 rightsId, uint256 ExtendId ) external 
    {
        require(
            _isApprovedOrOwner(_msgSender(), rightsId),
            "caller is not token owner nor approved"
        );

        Extendnft ex =  Extendnft(extendContract);
        (uint256 ex_rightsid, uint256 second, uint256 deadline) =  ex.getExtend(ExtendId);  
        
        require( tiles[rightsId].enable, "Rights still not active");
        require( deadline > block.timestamp, "Extend time-out");
        require( ex_rightsid == rightsId , "Invalid Rights Id" );

        tiles[rightsId].expire = tiles[rightsId].expire + second;
        ex.burn(ExtendId);

        emit ExtendRights( rightsId, ExtendId, second, extendContract );
    }
