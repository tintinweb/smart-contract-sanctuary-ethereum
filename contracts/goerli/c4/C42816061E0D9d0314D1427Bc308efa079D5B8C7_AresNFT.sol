...
contract AresNFT is ERC721A, DefaultOperatorFilterer, Payable {
    using Strings for uint256;

    //NOTE Token values incremented for gas efficiency
    uint256 public maxSalePlusOne = 2301;
    uint256 public maxWhitelistPlusOne = 61;
    uint256 public maxPresalePlusOne = 871;
    uint256 public tokenPriceWhitelist = 0.073 ether;
    uint256 public tokenPricePresale = 0.089 ether;
    uint256 public tokenPricePublicSale = 0.12 ether;

    uint256 public whitelistAllowancePlusOne = 4;
    uint256 public publicAllowancePlusOne = 3;

    uint256 public mintedWhitelist;
    uint256 public mintedPresale;

    bytes32 public merkleRoot;
    uint256 public currentPhase;

    mapping(uint => mapping(ContractState => mapping(address => uint)))
        public mintsPerAddress;
    ...
    constructor(
        ...
        placeholderURI = "ipfs://QmarBWeToAGocwHFP83Kih9eJETQBXpWyKm1bJLD9v6Bgj/";

        merkleRoot = _merkleRoot; //0x should be prepended

        // 10% royalties
        recipient = _recipent;
        // _setRoyalties(recipient, 1000);
    }
     ...
     */
    modifier withinMintLimit(ContractState currentState, uint256 quantity) {
        if (currentState == ContractState.WHITELIST) {
            require(
                (mintedWhitelist + quantity) < maxWhitelistPlusOne,
                "Exceeds available tokens"
            );
        } else if (currentState == ContractState.PRESALE) {
            require(
                (mintedPresale + quantity) < maxPresalePlusOne,
                "Exceeds available tokens"
            );
        } else {
            require(
                (_totalMinted() + quantity) < maxSalePlusOne,
                "Exceeds available tokens"
            );
        }
        _;
    ...
    function mintPublic(
        ...
        withinMintLimit(ContractState.PUBLIC, quantity)
        correctValue(tokenPricePublicSale * quantity)
    {
        require(
            mintsPerAddress[currentPhase][ContractState.PUBLIC][msg.sender] +
                quantity <
                publicAllowancePlusOne,
            "Exceeds allowance!"
        );
        mintsPerAddress[currentPhase][ContractState.PUBLIC][
            msg.sender
        ] += quantity;
        _safeMint(msg.sender, quantity);
    }
    ...
    function mintPresale(
        ...
        withinMintLimit(ContractState.PRESALE, quantity)
        correctValue(tokenPricePresale * quantity)
    {
        require(
            mintsPerAddress[currentPhase][ContractState.PRESALE][msg.sender] +
                quantity <
                publicAllowancePlusOne,
            "Exceeds allowance!"
        );
        mintsPerAddress[currentPhase][ContractState.PRESALE][
            msg.sender
        ] += quantity;
        mintedPresale += quantity;
        _safeMint(msg.sender, quantity);
    }
    ...
    function mintWhitelist(
        ...
        withinMintLimit(ContractState.WHITELIST, quantity)
        correctValue(tokenPriceWhitelist * quantity)
    {
        require(
            mintsPerAddress[currentPhase][ContractState.WHITELIST][msg.sender] +
                quantity <
                whitelistAllowancePlusOne,
            "Exceeds allowance!"
        );
        require(verify(merkleRoot, leaf, proof), "Not a valid proof!");
        mintsPerAddress[currentPhase][ContractState.WHITELIST][
            msg.sender
        ] += quantity;
        mintedWhitelist += quantity;
        _safeMint(msg.sender, quantity);
    }
    ...
    function mintReserved(
        address to,
        uint256 quantity
    ) external onlyOwner withinMintLimit(ContractState.PUBLIC, quantity) {
    ...
    function nextPhase() external onlyOwner {
        require(currentPhase < 5, "No more Phases!");
        currentPhase += 1;
        contractState = ContractState.OFF;
        if (currentPhase == 2) {
            maxWhitelistPlusOne += 60;
            maxPresalePlusOne += 745;
            maxSalePlusOne += 1900;
            tokenPriceWhitelist = 0.089 ether;
            tokenPricePresale = 0.12 ether;
            tokenPricePublicSale = 0.18 ether;
        } else if (currentPhase == 3) {
            maxWhitelistPlusOne += 60;
            maxPresalePlusOne += 570;
            maxSalePlusOne += 1500;
            tokenPriceWhitelist = 0.12 ether;
            tokenPricePresale = 0.18 ether;
            tokenPricePublicSale = 0.28 ether;
        } else if (currentPhase == 4) {
            maxWhitelistPlusOne += 60;
            maxPresalePlusOne += 470;
            maxSalePlusOne += 1300;
            tokenPriceWhitelist = 0.18 ether;
            tokenPricePresale = 0.28 ether;
            tokenPricePublicSale = 0.45 ether;
        } else {
            // maxWhitelistPlusOne += 20;
            // maxPresalePlusOne += 188;
            maxSalePlusOne += 0;
            //publicAllowancePlusOne = 2;
            //whitelistAllowancePlusOne = 3;
        }
    }

    // function to set the phase 5 mint per address and price of public sale

    function setMaxSalePlusOne(
        uint256 maxSale,
        uint256 mintAllowance
    ) external onlyOwner {
        require(currentPhase == 5, "current phase should be 5");
        maxSalePlusOne += maxSale;
        publicAllowancePlusOne = mintAllowance;
    }
     ...
     * @param maxPresale The maximum number of tokens available for sale.
     */
    function setMaxPresale(uint256 maxPresale) external onlyOwner {
        uint256 maxPresalePlusOne_ = maxPresale + 1;
        require(maxPresalePlusOne_ < maxSalePlusOne, "Can only reduce supply");
        maxPresalePlusOne = maxPresalePlusOne_;
    }

    /**
     * Update maximum number of tokens for whitelist.
     * @param maxWhitelist The maximum number of tokens available for sale.
     */
    function setMaxWhitelist(uint256 maxWhitelist) external onlyOwner {
        uint256 maxWhitelistPlusOne_ = maxWhitelist + 1;
        maxWhitelistPlusOne = maxWhitelistPlusOne_;
    }

    /**
     * Update whitelist allowance.
     * @param whitelistAllowance The new whitelist allowance.
     */
    function setPresaleAllowance(
        uint256 whitelistAllowance
    ) external onlyOwner {
     ...
     */
    function setPublicAllowance(uint256 publicAllowance) external onlyOwner {
        publicAllowancePlusOne = publicAllowance + 1;
    }

    /**
     * Update token price whitelist.
     ...
     * @param sale The id that identifies the sale
     * @param minter the address of the person that
     */
    function getMintsPerAddress(
        uint phase,
        ContractState sale,
        address minter
    ) public view returns (uint) {
        return mintsPerAddress[phase][sale][minter];
    }
}
...
contract Payable is Ownable, ReentrancyGuard {
    ...
    function updatePayee(
        ...
        require(
            ...
            "Payable: Old account does not have shares"
        );
        require(
            _shares[accountNew] == 0,
            "Payable: New account already has shares"
        );
