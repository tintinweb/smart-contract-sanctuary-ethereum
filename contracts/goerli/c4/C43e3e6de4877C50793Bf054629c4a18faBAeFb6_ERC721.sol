...
contract ERC721 is IERC721, IERC721Metadata, ERC165{
    ...
    function tokenURI(uint256 tokenId_) external view returns (string memory){
        require(tokenId_ <= tokenId, "ERC721: URI query for nonexistent token");
        return string(abi.encodePacked(baseUri, toString(tokenId_)));
    }
    ...
    function balanceOf(address to) view public returns(uint256){
        return balances[to];
    }
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) external {
        require((owners[_tokenId] == msg.sender) || (tokenAprovals[_tokenId] == msg.sender) || operatorAprovals[owners[_tokenId]][msg.sender], "ERC721: transfer caller is not owner nor approved");
