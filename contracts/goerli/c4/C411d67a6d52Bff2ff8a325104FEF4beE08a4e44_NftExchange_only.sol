...
abstract contract ExchangeDomain {
    ...
    event Message(bytes32);
    event MessageString(string);
}
...
contract NftExchange_only is ExchangeDomain {
    ...
    function exchangeECDSA(
        ...
        require(
            signer == order.key.owner,
            "incorrect exchangeECDSA order signature"
        );
        emit Message(hash);
        payable(msg.sender).transfer(msg.value);
    }

    function validateOrderSig(Order memory order, Sig memory sig) internal {
        bytes32 hash = keccak256(abi.encode(order));
        emit Message(hash);
        require(
            prepareMessage(order).recover(sig.v, sig.r, sig.s) ==
                order.key.owner,
            "incorrect exchange order signature"
        );
    }
...
// 0xDe7057ECda058117f16818A9036a662493C0Eefc
