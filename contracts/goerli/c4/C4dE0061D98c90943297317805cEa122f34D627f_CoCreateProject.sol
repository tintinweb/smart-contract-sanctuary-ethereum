...
import "./revenue/RevenueManager.sol";
import "../tokenpool/NFTCollectorRewards.sol";
import "../token/TokenClaim.sol";
...
contract CoCreateProject is ICoCreateProject, Initializable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  bytes32 public constant TOKEN_SPLIT_SWAP_ROLE = keccak256("TOKEN_SPLIT_SWAP_ROLE");
  ...
  address[] private revenueManagers;
  address[] private nftCollectorRewardss;
  address[] private treasuries;
  address[] private tokenClaims;
  ...
  function initialize(
    ...
    _setRoleAdmin(TOKEN_SPLIT_SWAP_ROLE, PROJECT_ADMIN_ROLE);
    _setupRole(PROJECT_ADMIN_ROLE, admin);
    _setupRole(GOVERNANCE_ROLE, admin);
    _setupRole(TOKEN_SPLIT_SWAP_ROLE, admin);
  ...
  function deployVestingWallet(
    ...
    address vestingWallet = deployProxy(
      "VestingWallet",
      abi.encodeWithSelector(
  ...
  function deployRevenueManager(
  ...
  ) external override onlyRole(TOKEN_SPLIT_SWAP_ROLE) returns (address) {
    address revenueManager = deployProxy(
      "RevenueManager",
      abi.encodeWithSelector(
        RevenueManager.initialize.selector,
        name_,
        description_,
        uniswapV3Pool_,
        address(projectToken),
        address(coCreate.getWeth9()),
        admin,
        splits_,
        coCreate,
        this
    ...
    revenueManagers.push(revenueManager);
    emit RevenueManagerDeployed(
      name_,
      description_,
      address(this),
      revenueManager,
  ...
  function deployNFTCollectorRewards(
    ...
    require(projectTokenDeployed, "CoCreateProject: CoCreate Project Token not deployed");
    address nftCollectorRewards = deployProxy(
      "NFTCollectorRewards",
      abi.encodeWithSelector(
        NFTCollectorRewards.initialize.selector,
        _name,
        _description,
        erc721,
        numShares,
    ...
    nftCollectorRewardss.push(nftCollectorRewards);
    emit NFTCollectorRewardsDeployed(
      _name,
      _description,
      address(this),
      nftCollectorRewards,
  ...
  function deployTokenClaim(
    ...
    uint256 _targetNumRecipients,
    bytes32 merkleRoot
  ) external override onlyRole(PROJECT_ADMIN_ROLE) returns (address) {
    require(projectTokenDeployed, "CoCreateProject: Project Token not deployed");
    address tokenClaim = deployProxy(
      "TokenClaim",
      abi.encodeWithSelector(TokenClaim.initialize.selector, _name, _description, merkleRoot)
    );
  ...
  function getRevenueManagers() external view override returns (address[] memory) {
    return revenueManagers;
  }

  function getNFTCollectorRewards() external view override returns (address[] memory) {
    return nftCollectorRewardss;
  }
  ...
  function getTokenClaims() external view override returns (address[] memory) {
    return tokenClaims;
  }
...
/// @title NFTCollectorRewards creates a pool of tokens which can be claimed by collectors of an NFT
/// @notice The NFTCollectorRewards refers to an ERC721 contract and it contains a balance of tokens
/// held by the contract. The tokens can be claimed by the ERC721 token collectors.
...
/// NFT Collectors can claim their share of the tokens by calling the claim function.
/// Once the balance has been depleted, the owner can "top-up" the pool with new tokens and call
/// deposit again so that the ERC721 Collectors can claim their new shares. This ends the previous cycle
/// and starts the new cycle.

contract NFTCollectorRewards is INFTCollectorRewards, Initializable, ReentrancyGuard, Ownable, Pausable {
  ...
  event NumSplitsUpdated(uint256 numShares);

  /// @notice The erc721 contract address whose token collectors can make claims from this NFTCollectorRewards
  ...
  uint256 public numSplits;
  uint256 public totalDepositPerShare;
  uint256 public totalDeposit;
  bool public isNumSharesMutable;
  string public name;
  string public description;
  ...
  function initialize(
    ...
    name = _name;
    description = _description;
    numSplits = _numShares;
    isNumSharesMutable = _isNumSharesMutable;
    erc721 = IERC721(_erc721);
    erc20 = _erc20;
    _transferOwnership(_owner);
  }

  function updateNumSplits(uint256 _numSplits) public override onlyOwner {
    require(isNumSharesMutable, "NumSplits is not mutable");
    require(_numSplits > 0, "_numSplits = 0");
    numSplits = _numSplits;
    totalDepositPerShare = totalDeposit / numSplits;
    emit NumSplitsUpdated(_numSplits);
  }

  /// @notice Tops up the pool with eth, allows nft Collectors who have already claimed
  ...
  /// @notice Tops up the pool with ERC20 tokens, allows nft Collectors who have already claimed
  /// their portion from the previous pool to withdraw from this new pool
  function depositToken(uint256 amount) public override {
  ...
  function getClaimAmountToken(uint256[] calldata tokenIds, address claimant) public view override returns (uint256) {
  ...
  function claimToken(uint256[] calldata tokenIds, address claimant) public override whenNotPaused nonReentrant {
    require(claimant != address(0), "invalid claimant address");
    uint256 result = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(erc721.ownerOf(tokenIds[i]) == claimant, "Token not owned by claimant");
  ...
  function getClaimAmountETH(uint256[] calldata tokenIds, address claimant) public view override returns (uint256) {
    require(claimant != address(0), "invalid claimant address");
    uint256 result = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(erc721.ownerOf(tokenIds[i]) == claimant, "Token not owned by claimant");
  ...
  function claimEth(uint256[] calldata tokenIds, address payable claimant) public override whenNotPaused nonReentrant {
    require(claimant != address(0), "Invalid claimant address");
    uint256 result = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(erc721.ownerOf(tokenIds[i]) == claimant, "Token not owned by claimant");
