...
contract Infynyty is ReentrancyGuard, Ownable {
    ...
    function buy(
        ...
        (address royalty, uint256 royaltyAmount) = royaltyStorage(royaltyAddress).royaltyInfo(sellerInfo.tokenId, msg.value);
        transferETH(sellerInfo.seller, msg.value);
        transferETH(royalty, royaltyAmount);
        transferETH(platformAddress, royaltyStorage(royaltyAddress).platformFeeInfo(msg.value));
        transferERC1155(
            sellerInfo.NFTAddress,
            sellerInfo.seller,
            msg.sender,
    ...
    function changeMarketInfo(
        ...
        if(newRoyaltyAddress != address(0)){
...
contract royaltyStorage is Ownable {
    ...
    function updateNFT(address newaddress) external onlyOwner {
        require(newaddress != address(0), "Address invalid");
        infynytyAddress = newaddress;
    }
    function updateFee(uint256 percentageFee) external {
        require(percentageFee <= _feeDenominator(), "ERC2981: royalty fee will exceed salePrice");
        platformFee = percentageFee;
    }
    function setRoyalty(uint256 tokenId, address firstOwner, uint256 percentageFee) external {
        require(percentageFee <= _feeDenominator(), "ERC2981: royalty fee will exceed salePrice");
        require(firstOwner != address(0), "ERC2981: Invalid parameters");
        require(msg.sender == infynytyAddress, "CALLER_INVALID_PERMISSION");
        _tokenRoyaltyInfo[tokenId] = RoyaltyInfo(firstOwner, percentageFee);
    }
    ...
    function platformFeeInfo(uint256 _salePrice) public view returns (uint256) {
