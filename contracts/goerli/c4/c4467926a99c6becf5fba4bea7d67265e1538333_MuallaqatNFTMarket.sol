...
import "./Tokens/@rarible/royalties/contracts/LibRoyaltiesV2.sol";
import "./Tokens/MuallaqatNFT.sol";


contract MuallaqatNFTMarket is Initializable, ReentrancyGuardUpgradeable ,OwnableUpgradeable, PausableUpgradeable {
  ...
  function setChangeStatusFeesPrice(uint256 _price) public onlyOwner  {
    changeStatusFeesPrice = _price;
  }
  ...
  function initialize() public initializer {
      ...
      changeStatusFeesPrice = 0.0006 ether; 
  ...
  function getFees() public view returns (uint256[] memory) {
    ...
    fees[1] = changeStatusFeesPrice;
    fees[2] = premiumFees;
    fees[3] = premiumDays;
    fees[4] = ownerPercentage;
    return fees;
  }


  function getMarketToken(address tokenContract,uint256 tokenId) public view returns (string memory) {
    return ERC721(tokenContract).tokenURI(tokenId);
  }
  function getMarketTokenByItemId(uint256 itemId) public view returns (string memory) {
    require(idToMarketItem[itemId].itemId !=0 ,"Please chose a vaild item");
 
    return MuallaqatNFT(idToMarketItem[itemId].tokenContract).tokenURI(idToMarketItem[itemId].tokenId);
  }
  function getMarketItem(uint256 marketItemId) public view returns (MarketItem memory) {
    return idToMarketItem[marketItemId];
  }
 
  function setExpiringDateAndPremium(uint256 marketItemId, uint256 expiringDate, bool premium) public onlyOwner {
    require(expiringDate > block.timestamp, "Expiring date must be in the future");
    require(idToMarketItem[marketItemId].status == Status.onSale, "Market item must be on sale");

    MarketItem memory item = idToMarketItem[marketItemId];
    item.expiringDate = expiringDate;
    item.premium = premium;
    idToMarketItem[marketItemId] = item;
  }
  ...
  function createMuallaqatMarketItem(
    ...
    // if less than 100 ether
    if(price < 100 ether)
      require(msg.value >= ListingAndFeesPrice, "Price must be equal to listing price");
    
    uint256 premiumExpiringDate = 0;
    if(premium){
      require(msg.value >= premiumFees, "Price must be equal to premium fees");
  ...
  function createMarketItem(
    ...
    if(royalties>0)
      MuallaqatNFT(nftContract).setRoyalties(tokenId,payable(msg.sender),royalties);
  ...
  function createMarketSale(
    ...
    require(idToMarketItem[itemId].itemId !=0 ,"Please chose a vaild item");
    require(idToMarketItem[itemId].status == Status.onSale,"Item Not on Sale .. contact owner .");
    require(msg.value >= idToMarketItem[itemId].salePrice,"Price must be greater than or equal to the sale price");
    require(
      MuallaqatNFT(idToMarketItem[itemId].tokenContract).supportsInterface(LibRoyaltiesV2._INTERFACE_ID_ROYALTIES) 
      || 
      MuallaqatNFT(idToMarketItem[itemId].tokenContract).supportsInterface(_INTERFACE_ID_ERC2981), 
    "Token must support royalties");
    
    uint tokenId = idToMarketItem[itemId].tokenId;
    uint amount = msg.value ;

    if(amount>100 ether){
      payable(admin).transfer(amount * ownerPercentage/10000); // % to owner
    ...
    idToMarketItem[itemId].soldDate = block.timestamp;
   ...
   emit MarketSale(
      ...
      idToMarketItem[itemId].soldDate,
      msg.sender
    );
    ...
    // change market item status 
  function SetBuyPrice(uint256 itemId, uint newPrice) public payable nonReentrant whenNotPaused {
    require(idToMarketItem[itemId].itemId !=0 ,"Please chose a vaild item");
    // only if on sale
    require(idToMarketItem[itemId].status == Status.onSale,"Item is not on sale");
    // only owner of item 
    require(idToMarketItem[itemId].owner == msg.sender,"You are not the owner of this item");
    require(msg.value >= changeStatusFeesPrice , "Please submit the Fees in order to complete the process");
    payable(admin).transfer(msg.value);
    idToMarketItem[itemId].salePrice = newPrice;
  }
  // change market item status 
  function changeMarketItemStatus(uint256 itemId, uint status) public payable nonReentrant whenNotPaused {
    require(idToMarketItem[itemId].itemId !=0 ,"Please chose a vaild item");
    // only if on sale
    require(idToMarketItem[itemId].status == Status.onSale,"Item is not on sale");
    // only owner of item 
    ...
    idToMarketItem[itemId].status = Status(status);
    if(Status(status)==Status.Canceled){
      MuallaqatNFT(idToMarketItem[itemId].tokenContract).transferFrom(address(this), msg.sender, idToMarketItem[itemId].tokenId);
    }
  }
  // delete itemId
  function deleteItem(uint256 itemId) public   {
    require(idToMarketItem[itemId].itemId !=0 ,"Please chose a vaild item");
    /// if admin or owner of item
  ...
  function fetchMarketItems() public view returns (MarketItem[] memory) {
    ...
    for (uint i = 0; i < itemCount; i++) {
      if (idToMarketItem[i + 1].status == Status.onSale) {
        uint currentId = i + 1;
  ...
  function fetchMarketItemsLimit(uint start, uint limit ) public view returns (MarketItem[] memory) {
    if(limit>_itemIds.current()){
      limit = _itemIds.current();
    }
...
contract MuallaqatNFT is ERC721URIStorageUpgradeable, PullPaymentUpgradeable, OwnableUpgradeable,RoyaltiesV2Impl {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    ...
    function initialize() initializer public {
       __ERC721_init("Muallaqat Digital Asset", "MDA");
       __Ownable_init();
       baseTokenURI = "";
       /*
        {
            "collectionName": "Muallaqat Digital Asset",
            "safelistRequestStatus": "not_requested",
            "imageUrl": "https://ipfs.muallaqat.io/xxx",
            "externalUrl": "https://muallaqat.io",
            "lastIngestedAt": "2023-01-24T00:08:07.000Z"
        }
        */
    ...
    function approveMarket(uint256 newItemId, address marketplaceContractAddress)  public  {
        approve(marketplaceContractAddress, newItemId);
        emit Approved(newItemId,marketplaceContractAddress);
    }
