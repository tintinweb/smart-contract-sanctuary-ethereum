...
contract meta_power is owned {
    ...
    uint public oneDay = 300 seconds; // this can be changed for testing,  like '30 min' , '100' etc
    ...
    function firstInvest(address _referrer, uint _amount) public returns(bool) {
        ...
        for(uint i=0;i<10;i++) {
            ...
            userInfo memory tmp = userInfos[_ref];
            if ( tmp.totalPaid + amt > tmp.investedAmount  * maxPayout / 10000  ) amt = (tmp.investedAmount * maxPayout / 10000 ) - tmp.totalPaid;
        ...
        //if booster
        if(block.timestamp - temp.joinTime <= 30 * oneDay && _amount >= temp.investedAmount && _amount > (10 * (10 ** 19)) && temp.investedAmount > ( 10* ( 10**19 ) )&& temp.returnPercent < 125  ) {
    ...
    function nextRoiWithdraw(uint fromIndex, uint toIndex) public returns(bool) {
        ...
        for(uint i=fromIndex; i<=toIndex; i++) {
            uint day = ( block.timestamp - payTimeRecords[msg.sender][i]) / oneDay;
            if (day > 0) {
                uint invA = investRecords[msg.sender][i];
                uint amt = day * invA * defaultROI / div;
