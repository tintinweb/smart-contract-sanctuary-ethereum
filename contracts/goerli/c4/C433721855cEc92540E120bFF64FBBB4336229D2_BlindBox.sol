...
contract BlindBox is ERC1155, Ownable, Pausable {
    ...
    IERC721 public lostLand = IERC721(0x66e70b761F2dC07033dd08F8856393eDcC5b2E40); // Land ERC721
    ...
    address public signer = address(0xf6fC42Bded3C5c9Cc460558Ad02dF0a89D7a758e); // todo
    ...
    function newBox(uint boxId_, string memory name_, uint256 price_, uint256 totalSupply_) public onlyOwner {
    ...
    function updateBox(string memory name_, uint boxId_, uint256 price_, uint256 totalSupply_, bool status_) public onlyOwner {
    ...
    function registration(address userAddress, address referrerAddress) private {
        require(!isReferrerExists(userAddress), "referrer exists");
        uint32 size;
        assembly {
    ...
    function buyBox(uint boxId_, uint num_, address referrer_) public whenNotPaused {
        require(boxMap[boxId_].id != 0 && boxMap[boxId_].mintNum + num_ <= boxMap[boxId_].totalSupply, "box id or num err");
    ...
    function openBox(uint boxId_, uint256 tokenId_, bytes calldata signature, uint deadline) public ensure(deadline) returns (bool) {
        bytes32 digest = keccak256(abi.encode(_msgSender(), boxId_, tokenId_, deadline));
        require(isSignatureValid(signature, digest, signer), 'offer signature error');
        boxMap[boxId_].openNum += 1;
        _burn(_msgSender(), boxId_, 1);
        lostLand.mint(_msgSender(), tokenId_);
        emit OpenBox(_msgSender(), boxId_, tokenId_);
        return true;
    }
