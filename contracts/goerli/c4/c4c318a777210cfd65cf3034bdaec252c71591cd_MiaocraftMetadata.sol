...
contract MiaocraftMetadata is IMetadata, Initializable, Ownable {
    ...
    function getMetadata(uint256 tokenId) public view returns (string memory) {
...

        {
            ...
            attributes[1] = FormatMetadata.formatTraitString(
                "Spins Decay",
                spinsDecay.toDecimalString(1)
            );
            attributes[2] = FormatMetadata.formatTraitString(
                "Net Spins",
                netSpins.toDecimalString(1)
            );
            attributes[3] = FormatMetadata.formatTraitNumber(
                "Health",
                (netSpins * 100) / spins,
                "boost_percentage"
            );
...

        {
            ...
            attributes[4] = FormatMetadata.formatTraitNumber("x", x, "number");
            attributes[5] = FormatMetadata.formatTraitNumber("y", y, "number");
        }

        attributes[6] = FormatMetadata.formatTraitNumber(
            "Last Service Time",
        ...
        returns (string memory)
    {
        return
            bytes(
                renderer.getSvg(Math.max(spins, 1), maxWidth, maxHeight, seed)
            ).encode();
    }
...
library DecimalString {
    function toDecimalString(
        ...
        return string(abi.encodePacked(integerValue, ".", decimalValue));
    }
    ...
    function toDecimalString(uint256 value, uint256 precision)
        ...
        returns (string memory)
    {
        return toDecimalString(value, 18, precision);
    }
}
