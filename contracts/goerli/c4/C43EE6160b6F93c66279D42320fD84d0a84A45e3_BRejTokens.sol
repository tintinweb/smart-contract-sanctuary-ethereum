...
contract BRejTokens is Context, ERC20Pausable, AccessControlEnumerable {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");

    constructor(
        string memory name_,
        string memory symbol_,
        address pauserAccount,
        address initialAccount,
        uint256 initialBalance
    ) ERC20(name_, symbol_) {
        require(bytes(name_).length > 0, "Token: name is required");
        require(bytes(symbol_).length > 0, "Token: length is required");
        require(initialBalance > 0, "Inital balance is required");

        _grantRole(PAUSER_ROLE, pauserAccount);
