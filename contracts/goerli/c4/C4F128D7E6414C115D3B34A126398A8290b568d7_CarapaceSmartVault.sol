...
contract CarapaceSmartVault is ReentrancyGuard {
    ...
    modifier onlyVaultReady(uint256 _vaultId) {
        // require(!paused(), "CP");    //Preset for ERC721
        require(carapaceStorage.getSmartVaultState(_vaultId) == status.Created, "VNC");
        _;
    }
    ...
    function createSmartVault(
        ...
        (uint256 _vaultRewards, uint256 _protocolRewards) = carapaceDeposit.deposit{value: msg.value}();
    ...
    function editSmartVault(
        ...
        uint256 _trusteeIncentive,
        uint256 _lockDays
    ) external onlyVaultOwner(_vaultId) onlyVaultReady(_vaultId) {
        require(_beneficiaries.length > 0 && _trustees.length > 0, "0BT");
    ...
    function cancelSmartVault(uint256 _vaultId) external onlyVaultOwner(_vaultId) onlyVaultReady(_vaultId) {
        ...
        carapaceDeposit.cancelDeposit(_returnDeposit, _returnRewards, msg.sender);
        emit vaultEvent(_vaultId, msg.sender, uint256(carapaceStorage.getSmartVaultState(_vaultId)));
    }
    ...
    function withdrawFunds(uint256 _vaultId, uint256 _amountToWithdraw) external onlyVaultOwner(_vaultId) onlyVaultReady(_vaultId) nonReentrant() {
        (uint256 beforeBalance, ) = carapaceStorage.getSmartVaultRwrdInfo(_vaultId); //balance em ETH
        // check possible reentrancy
        (uint256 _vaultRewards, uint256 _protocolRewards) = carapaceDeposit.withdrawFunds(beforeBalance, _amountToWithdraw, msg.sender);
        carapaceStorage.subtractBalance(_vaultId, _amountToWithdraw, _vaultRewards, _protocolRewards); //subtrai balance em ETH + rewards
    }

    function requestExecution(uint256 _vaultId) external onlyVaultTrustees(_vaultId) {
    ...
    function executeSmartVault(uint256 _vaultId) external onlyVaultTrustees(_vaultId) nonReentrant() { 
        require(carapaceStorage.getSmartVaultState(_vaultId) == status.Paused, "VP");
        ( , , uint256 _requestExecTS, uint256 _pauseDays) = carapaceStorage.getSmartVaultInfo(_vaultId);
        (uint256 _vaultBalance, ) = carapaceStorage.getSmartVaultRwrdInfo(_vaultId); //balance em ETH
        // require((block.timestamp - _requestExecTS) > _pauseDays * 1 days );
        require((block.timestamp - _requestExecTS) > _pauseDays);  // uses seconds only for testing, remove to mainnet

        (uint256 _totalFees, uint256 _vaultRewards, uint256 _protocolRewards) = carapaceDeposit.executeDeposit(_vaultBalance, msg.sender, carapaceStorage.getSmartVaultTrusteeIncentive(_vaultId));
    ...
    function transferFungibleAssets(uint256 _vaultId) private {
        ...
        // Transfer fungible tokens to Escrow and set map to beneficiaries
    ...
    // VaultIDs array, transfers SUM balance
    function withdrawBeneficiaryBalance(uint256[] memory _vaultId) external nonReentrant() { 
        uint256 beneficiaryFunds;

        for(uint256 i=0;i<_vaultId.length;i++) {
            require(carapaceStorage.getSmartVaultState(_vaultId[i]) == status.Executed, "VNE");
            (uint256 vaultBeneficiaryBalance, uint256 vaultBeneficiayRewards) = carapaceStorage.getSmartVaultBeneficiaryBalance(_vaultId[i], msg.sender);
            beneficiaryFunds += carapaceDeposit.calculateVaultRewards(vaultBeneficiaryBalance, vaultBeneficiayRewards)+vaultBeneficiaryBalance;
    ...
    // Transfers all selected Fungible Assets from multiple vaults
    // It's made one transfer for each token if the array is sorted by token  address (Front-end does)
    function withdrawBeneficiaryFungibleAssets(withdrawFungibleAsset[] memory _fungibleAssets) external nonReentrant() {
        ...
        for (uint256 i=0;i<_fungibleAssets.length-1;i++) {
            require(carapaceStorage.getSmartVaultState(_fungibleAssets[i].vaultid) == status.Executed, "VNE");
        ...
        require(carapaceStorage.getSmartVaultState(_fungibleAssets[_fungibleAssets.length-1].vaultid) == status.Executed, "VNE");
    ...
    // Transfers all NFTs selected from multiple vaults
    function withdrawBeneficiaryNonFungibleAssets(withdrawNonFungibleAsset[] memory _nonfungibleAssets) external nonReentrant() {
        for (uint256 i=0;i<_nonfungibleAssets.length;i++) { 
            require(carapaceStorage.getSmartVaultState(_nonfungibleAssets[i].vaultid) == status.Executed, "VNE");
    ...
    function getVaultRewards(uint256 _vaultId) public view returns (uint256 _vaultTotalRewards) {
        ...
        return carapaceDeposit.calculateVaultRewards(_vaultBalance, _vaultRewards);
    }
    ...
    function getProtocolRewards() public view returns (uint256 _protocolTotalRewards) {
        ...
        return carapaceDeposit.calculateProtocolRewards(carapaceStorage.getTLV(), _protocolRewards);
    }

    // Returns ETH value (Balance + Rewards)
    function getVaultBeneficiaryFunds(uint256 _vaultId, address _beneficiary) external view returns (uint256 _beneficiaryFunds) {
        ...
        return carapaceDeposit.calculateVaultRewards(vaultBeneficiaryBalance, vaultBeneficiayRewards)+vaultBeneficiaryBalance;
    }
...
interface ICarapaceDeposit {
    ...
    function deposit() external payable returns (uint256 _vaultRewards, uint256 _protocolRewards);
    ...
    function cancelDeposit(uint256 _balance, uint256 _rewards, address _owner) external;
    function executeDeposit(uint256 _balance, address _trustee, uint256 _trusteeIncentive) external returns (uint256 _totalFees, uint256 _vaultRewards, uint256 _protocolRewards);
    function withdrawBeneficiaryFunds(uint256 _amount, address _beneficiary) external returns (uint256 _rewardsWithdrawnRETH);
    function withdrawOwnerRewards(address _owner, uint256 _vaultTotalRewards) external returns (uint256 _rewardsWithdrawnRETH);
    function withdrawProtocolRewards(uint256 _protocolRewards) external returns (uint256 _rewardsWithdrawn);
    function calculateVaultRewards(uint256 _vaultBalance, uint256 _vaultRewards) external view returns (uint256 _vaultTotalRewards);
