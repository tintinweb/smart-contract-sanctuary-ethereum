...
contract ProxySale is Ownable, ReentrancyGuard, IProxySale {
    ...
    event Stake(address indexed owner, uint256 indexed tokenId, uint256 indexed stakeType);
    ...
    mapping(uint256 => uint256) public stakeList;
    ...
    constructor(address _singer, address _vault) public {
        singer = _singer;
        vault = _vault;
    }

    function isStake(uint256 _tokenId) external override returns (bool){
        return stakeList[_tokenId] > 0;
    }
    ...
    function publicMint(string calldata salt, bytes calldata _signature)
    external
    payable
    nonReentrant
    eoaOnly
    ...
    function publicMintOfStake(string calldata salt, bytes calldata _signature, uint256 _stakeType)
    external
    payable
    nonReentrant
    eoaOnly
    ...
    function _stake(uint256 _tokenId, uint256 _stakeType) internal {
        require(!(stakeList[_tokenId] > 0), "Token already staked");

        require(erc721.ownerOf(_tokenId) == msg.sender, "Only owner can stake");

        stakeList[_tokenId] = _stakeType;

        emit Stake(msg.sender, _tokenId, _stakeType);
    }

    function stake(string calldata salt, bytes calldata _signature, uint256 _tokenId, uint256 _stakeType) external {

        _stake(_tokenId, _stakeType);
    }

    function unStake(string calldata salt, bytes calldata _signature,  uint256 _tokenId) external {
        require(stakeList[_tokenId] > 0, "Token not staked");

        require(erc721.ownerOf(_tokenId) == msg.sender, "Only owner can unstake");

        stakeList[_tokenId] = 0;
