...
{   
    ...
    event ActionExecuted(bytes32 cairoProgramHash, bytes32[] cairoProgramOutput);
    ...
    function execute_action(bytes32[] memory cairoProgramOutput) external {

        bytes32 cairoProgramOutputHash = keccak256(abi.encodePacked(cairoProgramOutput));
        bytes32 fact = keccak256(abi.encodePacked(cairoProgramHash, keccak256(abi.encodePacked(cairoProgramOutputHash))));

        // Check with SHARP if execution was done
        require(cairoVerifier.isValid(fact));
