...
import "./USNRegistry.sol";
import "../INTERFACES/IUSNLabsContract.sol";
...
/**
 *  @title Unseen Factory
 *  @notice Factory contract to deploy proxies from different implementations types and versions
 *  @author decapinator.eth | Unseen
 **/
contract USNFactory is USNRegistry {
    ...
    /// @dev Emitted when an implementation is added
     ...
     *  @dev Sets the initial owner and roles
     **/
    constructor(address _owner) payable {
        _setInitialOwner(_owner);
        _setupRole(DEFAULT_ADMIN_ROLE, _owner);
        _setupRole(FACTORY_ROLE, _owner);
    }

    /**
     *  @notice Deploys a proxy that points to the latest version of the given contract type
     *  @param _type Contract type { eg: ERC721 - ERC721A - ERC1155 ...}
     *  @param _data Deployment initialization params encoded
     **/
    ...
    /**
     *  @notice Deploys a proxy at a deterministic address by taking in `salt` as a parameter.
     *  @param _type Contract type { eg: ERC721 - ERC721A - ERC1155 ...}
     *  @param _data Deployment initialization params encoded
    ...
    function deployProxyDeterministic(
        ...
        require(_implementation != address(0), "USN: Invalid module");
     ...
     * @param _type Contract type { eg: ERC721 - ERC721A - ERC1155 ...}
     * @param _salt Unique identifier
     * @notice Returns a proxy  at a deterministic address by taking in `salt` as a parameter.
     */
    function getCreate2Address(
        ...
        address _implementation = implementation[_type][currentVersion[_type]];
        require(_implementation != address(0), "invalid module");
        bytes32 salthash = keccak256(abi.encodePacked(_msgSender(), _salt));
        return Clones.predictDeterministicAddress(_implementation, salthash);
    }

    /**
     *  @notice Lets a contract admin set the address of a contract type x version.
     *  @param _implementation Base contract address to be cloned
     **/
    function addImplementation(address _implementation) external {
        require(hasRole(FACTORY_ROLE, _msgSender()), "USN: Not admin");

        IUSNLabsContract module = IUSNLabsContract(_implementation);

        bytes32 ctype = module.contractType();
        require(ctype.length != 0, "invalid module");

        uint version = module.contractVersion();
        uint currentVersionOfType = uint(currentVersion[ctype]);
        require(version > currentVersionOfType, "USN: Wrong module version");
    ...
    /**
     *  @notice Returns the implementation given a contract type and version.
     *  @param _type Contract type { eg: ERC721 - ERC721A - ERC1155 ...}
     *  @param _version Version of the requested contract type
     **/
    ...
    /**
     *  @notice Returns the latest implementation given a contract type.
     *  @param _type Contract type { eg: ERC721 - ERC721A - ERC1155 ...}
     **/
...
/**
 *  @title Unseen
 *  @notice Registry contract holding all proxies data
 *  @author Unseen | decapinator.eth
 **/
contract USNRegistry is Multicall, TwoStepOwnable, AccessControlEnumerable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    /**
     *  @notice Add a new proxy address for a deployer
     *  @param _deployer proxy deployer address
     *  @param _deployment proxy address to point to
     **/
    function add(address _deployer, address _deployment) internal {
    ...
    /**
     *  @notice Get all proxies deployed by deployer address
     *  @param _deployer Address to map over
     **/
    function getAll(
        address _deployer
    ...
    /**
     *  @notice  Count how many proxies are deployed by deployer address
     *  @param _deployer Address to map over
     **/
...
/**
 *  @title IUSNLabsContract
 *  @notice Interface for USN Labs contract.
 */
interface IUSNLabsContract {
    ...
    /// @notice Returns the metadata URI of the contract.
    function contractURI() external view returns (string memory);

    /**
     *  @notice Sets contract URI for the storefront-level metadata of the contract.
     *  @param _uri Uri file storing metadata content
     */
