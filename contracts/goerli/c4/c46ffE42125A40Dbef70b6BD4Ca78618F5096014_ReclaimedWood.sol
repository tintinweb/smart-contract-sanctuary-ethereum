...
contract ReclaimedWood {
    ...
    function redeemProfit(address nftContract, uint256 tokenId) internal {
        TreesInterface treesContract = TreesInterface(nftContract);
    ...
    function redeemProfits(address[] memory nftContracts, uint256[] memory tokenIds) external {

        for(uint i = 0; i < nftContracts.length; i++) {
            redeemProfit(nftContracts[i], tokenIds[i]);
        }
