...
contract BaseCoordinateBind is Ownable {
    ...
    function BindCNS(address metaverse, uint256 tokenId, string memory name) public {
...
contract IPSCoordinateSystem is Ownable {
    ...
    event CoordCreated(address indexed user, uint256 indexed tokenId, address indexed metaverse);
    constructor(address ips_) {
        Ips = ips_;
    }

    function setSigner(address _metaverse, address _signer) public {
        require(Ownable(_metaverse).owner() == msg.sender, "invalid owner");
        metaSigner[_metaverse] = _signer;
    }

    //help external metaverse function
    function ownerSetSigner(address _metaverse, address _signer)
        public
        onlyOwner
    {
        metaSigner[_metaverse] = _signer;
    ...
    function createCoordinate(
        ...
        require(
            metaSigner[coord.metaverse] != address(0),
            "no signer been set"
        );
        require(
            ECDSA.recover(
                prefixed(keccak256(abi.encodePacked(coord.x, coord.y, coord.z, tokenId))),
                signature
            ) == metaSigner[coord.metaverse],
            "invalid signature"
        );
    ...
    function getCoordinate(address meta_, uint256 tokenId) public view returns(Coordinate memory) {
