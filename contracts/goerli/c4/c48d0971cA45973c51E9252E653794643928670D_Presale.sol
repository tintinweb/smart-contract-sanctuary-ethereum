...
contract Presale is Ownable, ReentrancyGuard {
    ...
    constructor(uint _publicPrice, uint _privatePrice, uint _publicMaxDeposit, uint _privateMaxDeposit, address _stable) {
    ...
    function enterPublic(uint _usdAmount) external {
        require(presaleIsOpen == true, "Presale is closed");
        require(_usdAmount > 0, "Wrong value");
        require(deposit[msg.sender] + _usdAmount <= publicMaxDeposit, "Max deposit reached");
    ...
    function enterPrivate(uint _usdAmount, bytes32[] calldata _merkleProof) external {
        require(presaleIsOpen == true, "Presale is closed");
        require(_usdAmount > 0, "Wrong value");
        require(deposit[msg.sender] + _usdAmount <= privateMaxDeposit, "Max alloc reached");
