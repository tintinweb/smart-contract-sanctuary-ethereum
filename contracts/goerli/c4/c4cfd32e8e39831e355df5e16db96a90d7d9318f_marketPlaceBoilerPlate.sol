   ...
   mapping(address => uint256) public whitelistedAddresses;
    ...
    function createMarketItem(
        ...
        uint256 price
            ...
            require (!paused, "Marketplace is Puased!");
        ...
        function Whitelist(address _addressToWhitelist, uint256 number_of_nft) public onlyOwner {
    ...
    //uint256 fromPrice4, uint256 toPrice4, 
     uint256 elsedaystohold4) public onlyOwner {

            if(getLastPrice <= fromPrice1 && getLastPrice <=toPrice1){
                onHold = true;
                endHold = block.timestamp + (daystohold1 * 1 days);
            }
            else if (getLastPrice > fromPrice2 && getLastPrice <= toPrice2){
                onHold = true;
                endHold = block.timestamp + (daystohold2 * 10 days) ;
            }
            else if (getLastPrice > fromPrice3 && getLastPrice <= toPrice3){
                onHold = true;
                endHold = block.timestamp + (daystohold3 * 20 days) ;
            }
            else{
                onHold = true;
                endHold = block.timestamp + (elsedaystohold4 * 30 days) ;
            }

    } 

    function Pause(bool _state) public onlyOwner returns (bool){
        paused = _state;
        return paused;
    }
