...
contract DNSv1 is UUPSUpgradeable, OwnableUpgradeable, ERC721Upgradeable {    
    ...
    //owner to array of tokenId
    mapping(address => uint256[]) private _owners;

    event Shout(uint256 indexed _id);

    function initialize(uint _rate, string memory _baseUri) public initializer {
      __ERC721_init("DNSv1", "DNS");
      __Ownable_init();
    ...
    function _burn(uint256 tokenId) internal virtual override {
        ...
        delete _tokenIds[_records[tokenId].host];
        delete _records[tokenId];
    ...
    function record(uint256 tokenId) public view virtual returns (Record memory) {
    ...
    function token(string memory host) public view returns (uint256) {
        uint256 tokenId = _tokenIds[host];
        return tokenId;
    }
   ...
   function _transfer(address from, address to, uint256 tokenId) internal override {
    ...
    function register(string memory host, uint numOfDays) public payable returns (uint256){
        ...
        uint256 curTokenId = _tokenIds[host];
        if (_exists(curTokenId)){    //exist record
            Record storage curRecord = _records[curTokenId];
            if (block.timestamp/60/60/24 > curRecord.expiredAt){
                //expired 
            ...
            uint16 expiredAt = uint16(block.timestamp/60/60/24 + numOfDays);
            Record memory _record = Record(65, expiredAt, 0, 0, host);   //A = 65
            _records[tokenId] = _record;
            _tokenIds[host] = tokenId;
