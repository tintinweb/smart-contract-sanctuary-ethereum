...
// File: contracts/contract-allow-list/contracts/proxy/interface/IContractAllowListProxy.sol
...
// File: contracts/contract-allow-list/contracts/ERC721AntiScam/restrictApprove/IERC721RestrictApprove.sol
...
// File: contracts/contract-allow-list/contracts/ERC721AntiScam/restrictApprove/ERC721RestrictApprove.sol
...
// File: contracts/contract-allow-list/contracts/ERC721AntiScam/IERC721AntiScam.sol
...
interface IERC721AntiScam is IERC721RestrictApprove {
}
// File: contracts/contract-allow-list/contracts/ERC721AntiScam/ERC721AntiScam.sol
    ...
    Ownable
{
    ...
    function _afterTokenTransfers(
        ...
        uint256 startTokenId,
        uint256 quantity
    ) internal virtual override(ERC721RestrictApprove) {
...
// File: contracts/ShikibuWorld.sol
...
contract ShikibuWorld is
    ...
    uint256 public maxReservedSupply = 1000;
    uint256 public ReservedMinted = 0;
    uint256 public maxTxMintAmount = 1;
    ...
    // ★withdrawAddress更新する
    address public constant withdrawAddress = 0x71F78146DB26F32AA79255d5e1cb2957f15e1DBa;
    address public constant CONTRIBUTOR_ADDRESS_1 = 0x1C31a6F8832aF5310c8472EE68dcae83275185Ba;
//    address public constant CONTRIBUTOR_ADDRESS_2 = 0x71F78146DB26F32AA79255d5e1cb2957f15e1DBa;
//    address public constant CONTRIBUTOR_ADDRESS_3 = 0x71F78146DB26F32AA79255d5e1cb2957f15e1DBa;
//    address public constant CONTRIBUTOR_ADDRESS_4 = 0x71F78146DB26F32AA79255d5e1cb2957f15e1DBa;
//    address public constant CONTRIBUTOR_ADDRESS_5 = 0x71F78146DB26F32AA79255d5e1cb2957f15e1DBa;
    address public constant CONTRIBUTOR_ADDRESS_6 = 0x71F78146DB26F32AA79255d5e1cb2957f15e1DBa;

    struct RewardManage {
        uint256 rewardAmmount;
        uint256 firstUnlockedAmmount;
        uint256 secondUnlockedAmmount;
        uint256 thirdUnlockedAmmount;
        uint256 fourthUnlockedAmmount;
        uint256 fifthUnlockedAmmount;
    }
    mapping(address => RewardManage) public RewardManages;

    address public adminSigner;
    address public admin;

    mapping(address => uint256) public presaleMintCount;
    mapping(address => uint256) public publicsaleMintCount;
    ...
    constructor() ERC721A("Shikibu World", "SKB") {
        admin = 0x71F78146DB26F32AA79255d5e1cb2957f15e1DBa;
        adminSigner = 0x71F78146DB26F32AA79255d5e1cb2957f15e1DBa;
        RewardManages[CONTRIBUTOR_ADDRESS_1] = RewardManage(2,1,0,0,0,0);
//        RewardManages[CONTRIBUTOR_ADDRESS_2] = RewardManage(1,1,0,0,0,0);
//        RewardManages[CONTRIBUTOR_ADDRESS_3] = RewardManage(1,1,0,0,0,0);
//        RewardManages[CONTRIBUTOR_ADDRESS_4] = RewardManage(1,1,0,0,0,0);
//        RewardManages[CONTRIBUTOR_ADDRESS_5] = RewardManage(1,1,0,0,0,0);
        RewardManages[CONTRIBUTOR_ADDRESS_6] = RewardManage(2,10,0,0,0,0);
        _safeMint(withdrawAddress,10);
        _safeMint(CONTRIBUTOR_ADDRESS_1,RewardManages[CONTRIBUTOR_ADDRESS_1].rewardAmmount);
//        _safeMint(CONTRIBUTOR_ADDRESS_2,RewardManages[CONTRIBUTOR_ADDRESS_2].rewardAmmount);
//        _safeMint(CONTRIBUTOR_ADDRESS_3,RewardManages[CONTRIBUTOR_ADDRESS_3].rewardAmmount);
//        _safeMint(CONTRIBUTOR_ADDRESS_4,RewardManages[CONTRIBUTOR_ADDRESS_4].rewardAmmount);
//        _safeMint(CONTRIBUTOR_ADDRESS_5,RewardManages[CONTRIBUTOR_ADDRESS_5].rewardAmmount);
        _safeMint(CONTRIBUTOR_ADDRESS_6,RewardManages[CONTRIBUTOR_ADDRESS_6].rewardAmmount);
    ...
    function presaleMint(
        ...
        // セールフェイズチェック
        require(phase == SalePhase.PreSale, "presale is disabled");
        ...
        // プレセール署名チェック
        require(
            keccak256(abi.encodePacked(phase, currentSaleId, msg.sender, _allocated))
        ...
        // ミント数上限チェック
        require(
            presaleMintCount[msg.sender] + _mintAmount <= _allocated,
        ...
        require(
            _mintAmount + totalSupply() - ReservedMinted <= maxSupply - maxReservedSupply,
    ...
    function reservedMint(
        ...
        // セールフェイズチェック
        require(phase != SalePhase.Locked, "sale is disabled");
    ...
    function publicsaleMint(
        ...
        // セールフェイズチェック
        require(phase == SalePhase.PublicSale, "publicsale is disabled");
        
        // ミント数がゼロでないこと
        require(_mintAmount != 0, "mintAmount is zero");

        // 1Txあたりのミント数上限チェック
        require(_mintAmount <= maxTxMintAmount, "Exceeds the maximum number of mints per Tx");

        // 総ミント数上限チェック
        require(
            publicsaleMintCount[msg.sender] + _mintAmount <= maxMintAmount,
    ...
    function burnMint(
        ...
        // セールフェイズチェック
        require(phase == SalePhase.BurnMint, "presale is disabled");
    ...
    ////////// onlyAdmin functions start //////////
    function setMaxSupply(uint256 _value) public onlyAdmin {
        maxSupply = _value;
    }

    function setMaxReservedSupply(uint256 _value) public onlyAdmin {
        maxReservedSupply = _value;
    }

    function setMintCost(uint256 _value) public onlyAdmin {
        mintCost = _value;
    }

    function setmaxMintAmount(uint256 _value) public onlyAdmin {
        maxMintAmount = _value;
    }

    function setBaseURI(string memory _value) public onlyAdmin {
        baseURI = _value;
    }

    function setBaseExtension(string memory _value) public onlyAdmin {
    ...
    function setCurrentSaleId(uint8 _value) public onlyAdmin {
        currentSaleId = _value;
    }
    ...
    function _beforeTokenTransfers(
        ...
        // Reward Lock-up
        if(block.timestamp > 1686409200) { // UNIXTIME 2023-06-11 00:00
            return; // Lock-up completed
        }
        address[2] memory rewardaddress = [CONTRIBUTOR_ADDRESS_1,
//                                           CONTRIBUTOR_ADDRESS_2,
//                                           CONTRIBUTOR_ADDRESS_3,
//                                           CONTRIBUTOR_ADDRESS_4,
//                                           CONTRIBUTOR_ADDRESS_5,
                                           CONTRIBUTOR_ADDRESS_6];
        if(block.timestamp < 1673362800) { // UNIXTIME 2023-01-11 00:00
            for(uint256 i = 0; i < rewardaddress.length; i++) {
                if(from == rewardaddress[i]) {
                    require(balanceOf(from) <= RewardManages[from].firstUnlockedAmmount,
                    "Transfer is not possible during lockup.");
                }
            }
            return;
        }
        if(block.timestamp < 1676041200) { // UNIXTIME 2023-02-11 00:00
            for(uint256 i = 0; i < rewardaddress.length; i++) {
                if(from == rewardaddress[i]) {
                    require(balanceOf(from) <= RewardManages[from].secondUnlockedAmmount,
                    "Transfer is not possible during lockup.");
                }
            }
            return;
        }
        if(block.timestamp < 1678460400) { // UNIXTIME 2023-03-11 00:00
            for(uint256 i = 0; i < rewardaddress.length; i++) {
                if(from == rewardaddress[i]) {
                    require(balanceOf(from) <= RewardManages[from].thirdUnlockedAmmount,
                    "Transfer is not possible during lockup.");
                }
            }
            return;
        }
        if(block.timestamp < 1681138800) { // UNIXTIME 2023-04-11 00:00
            for(uint256 i = 0; i < rewardaddress.length; i++) {
                if(from == rewardaddress[i]) {
                    require(balanceOf(from) <= RewardManages[from].fourthUnlockedAmmount,
                    "Transfer is not possible during lockup.");
                }
            }
            return;
        }
        if(block.timestamp < 1683730800) { // UNIXTIME 2023-05-11 00:00
            for(uint256 i = 0; i < rewardaddress.length; i++) {
                if(from == rewardaddress[i]) {
                    require(balanceOf(from) <= RewardManages[from].fifthUnlockedAmmount,
                    "Transfer is not possible during lockup.");
                }
            }
            return;
    ...
    function setCALLevel(uint256 level) public override onlyAdmin {
        CALLevel = level;
    }

    function setCAL(address calAddress) public override onlyAdmin {
        _setCAL(calAddress);
    }
    ...
    ////////// public functions start //////////
    function totalReservedMint() public view returns (uint256) {
    ...
    function getPresaleMintCount(address _address)
        external
        view
        returns (uint256)
    {
        return presaleMintCount[_address];
    }
