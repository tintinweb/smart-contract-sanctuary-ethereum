...
contract DidV1 is ERC721EnumerableUpgradeable, DGIssuer {
    ...
    event AddKYC(uint256 tokenId, address KYCProvider, uint256 KYCId, bool status, uint256 updateTime);
    ...
    function getAuthorizedAddrs(string memory did) public view returns (address[] memory) {
        EnumerableSetUpgradeable.AddressSet storage auths = _auths[did2TokenId[did]];
    ...
    function isAddrAuthorized(string memory did, address addr) public view returns (bool) {
        return _auths[did2TokenId[did]].contains(addr);
    }
    ...
    function addKYC(
        ...
        require(_validate(keccak256(abi.encodePacked(tokenId, KYCProvider, KYCId, status, updateTime)), evidence), "invalid evidence");
        tokenId2KYC[tokenId][KYCProvider][KYCId] = KYCInfo({status: status, updateTime: updateTime});
        emit AddKYC(tokenId, KYCProvider, KYCId, status, updateTime);
    }
...
contract DeedGrain is ERC1155Supply {
    ...
    function mint(address to, uint256 tokenId, uint256 amount) public onlyControllers {
        require(balanceOf(to, tokenId) == 0, "already got tokenId");
...
contract DidV1Storage {
    ...
    mapping(uint256 => mapping(address => mapping(uint256 => KYCInfo))) tokenId2KYC;
}
