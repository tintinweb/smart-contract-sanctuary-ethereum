...
contract Compound4626 is ERC20, IERC4626 {
    ...
    uint256 immutable cTokenDecimals;
    ...
    constructor(
        ...
        string memory n,
        string memory s
    ) ERC20(n, s, ICERC20(c).decimals()) {
        cToken = ICERC20(c);
        underlyingDecimals = IERC20(cToken.underlying()).decimals();
        IERC20(cToken.underlying()).approve(c, type(uint256).max);
        cTokenDecimals = IERC20(cToken).decimals();
    }
        ...
        returns (uint256 shares)
    {
        return ((_assets * 1e27) /
            (
                ((cToken.exchangeRateStored() * 1e27) /
    ...
    {
        return ((IERC20(cToken.underlying()).balanceOf(_receiver) * 1e27) /
            (
                ((cToken.exchangeRateStored() * 1e27) /
                    ((1 * 10) ^ (18 + underlyingDecimals - cTokenDecimals)))
            ));
