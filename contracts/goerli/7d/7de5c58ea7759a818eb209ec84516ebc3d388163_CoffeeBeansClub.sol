...
contract CoffeeBeansClub is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 amount_) public payable {
        ...
        require(totalSupply() + amount_ <= MAX_AMOUNT, "Token supply exceeded"); // TODO: To be tested!
        require(
            msg.value >= (_listingPrice * amount_),
    ...
    function claim(bytes32[] calldata proof_) public {
        require(_saleIsActive, "Sale is not open");
        require(totalSupply() + 1 <= MAX_AMOUNT, "Token supply exceeded"); // TODO: To be tested!

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(proof_, _root, leaf),
            "Caller not whitelisted"
        );

        require(!_claimed[msg.sender], "You've already claimed");
