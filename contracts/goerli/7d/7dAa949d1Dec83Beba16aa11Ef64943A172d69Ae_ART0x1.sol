...
contract ART0x1 is ERC721Base {
    ...
    mapping(uint256 => address) public artInstructions;

    constructor() ERC721Base("ART0x1", "A0X1", 0.025 ether, 10000) {}
    ...
    function mint(
        uint256 _numToBeMinted
    )
        external
        payable
        hasExactPayment(_numToBeMinted)
        withinMintLimit(10, _numToBeMinted)
    {
        _mintMany(_msgSender(), _numToBeMinted);
    }

    function setArtInstructions(
        uint256 _tokenId,
        string[11] memory _instructions
    ) public {
        require(_exists(_tokenId), "ART0x1: token does not exist.");
        require(ownerOf(_tokenId) == msg.sender, "ART0x1: not your token.");

        bytes[11] memory bytesInstructions;

        for (uint256 i = 0; i < _instructions.length; ) {
        ...
        bytes memory data = abi.encode(bytesInstructions);
        artInstructions[_tokenId] = SSTORE2.write(data);
    }

    function getArtInstructions(
        uint256 _tokenId
    ) public view returns (string[11] memory) {
        require(_exists(_tokenId), "ART0x1: token does not exist.");

        bytes memory instr = SSTORE2.read(artInstructions[_tokenId]);
        bytes[11] memory byteStrings = abi.decode(instr, (bytes[11]));
        string[11] memory result;
    ...
    function tokenURI(
        ...
        require(_exists(_tokenId), "ART0x1: token does not exist.");
        require(rendererAddress != address(0), "ART0x1: no renderer set.");

        bytes memory instr = SSTORE2.read(artInstructions[_tokenId]);
        bytes[11] memory instructions = abi.decode(instr, (bytes[11]));

        return IRenderer(rendererAddress).tokenURI(_tokenId, instructions);
    }
}
