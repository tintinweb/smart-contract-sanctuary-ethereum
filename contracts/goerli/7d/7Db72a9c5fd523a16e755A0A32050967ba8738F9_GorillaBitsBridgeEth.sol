...
contract GorillaBitsBridgeEth is ERC1155Holder, Ownable, ReentrancyGuard {
  ...
  mapping(address => bool) approvedClaimers;
  ...
  /***********External**************/
  /**
   * @dev claiming a token based on a merkle proof that was precomputed and that is part of the
  ...
  function claim(
    ...
    require(enabled, "Bridging is stopped");
    require(verify(_oldId, _newId, msg.sender, _network, _signature), "Not approved");
    totalBridged++;
    idsBridged.push(_newId);
    oldIdsBridgedBySender[_account].push(_oldId);

    mintOnClaiming(_account, _newId);
  }
   ...
   */
  function setGBContract(address _contract) external onlyOwner {
    require(_contract != address(0), "_contract !address 0");
    gbContract = GorillaBitsI(_contract);
  }
   ...
   */
  function getGBContract() external view returns (address) {
    return address(gbContract);
  }
