...
interface ICrimereports {
    ...
    function burnReport(uint256 _crimeId) external;
    function safeTransferFrom(
        address from,
        address to,
        uint256 tokenId
...
contract Dummies is
    ...
    IERC721Receiver
{
    constructor() ERC721("Dummies", "DUMB") {}
    ...
    bool private deathRowToggle;
    mapping(uint256 => string) BASE_URIS;
    string private BASE_EXTENSION;

    // ------------------ External ------------------ //

    function incarcerate(uint256 _crimeId)
        external
        nonReentrant
    {
     ...
     function deathRow(uint256 _celId, bool _status) external {
        require(ownerOf(_celId)==msg.sender, "You don't own this Celmate");
        if(_status){
            require(!deathRowInfos[_celId].status, "You're already on DeathRow");
            DeathRow memory currDeathRow = DeathRow(block.timestamp,true);
            deathRowInfos[_celId] = currDeathRow;
        }else{
            require(deathRowInfos[_celId].status, "You're not on DeathRow");
    ...
    function evolveCelmate(uint256 _celId, uint256 _newState) public {
        require(authorised[msg.sender], "Not authorised");
        celstates[_celId] = _newState;
    }

    function getCelmateBlock(uint256 _celId) public view returns (uint256){
        return celblocks[_celId];
    }

    function changeCelblock(uint256 _celId, uint256 _targetBlock) public{
        require(authorised[msg.sender], "Not authorized");
    ...
    function setDeathRow(bool _flag) external onlyOwner {
        deathRowToggle = _flag;
    }
