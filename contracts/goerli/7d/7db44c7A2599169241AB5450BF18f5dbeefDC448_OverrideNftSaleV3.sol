...
contract OverrideNftSaleV3 is Ownable, ReentrancyGuard {
    ...
    uint64 public adminFreeMinted; // only if public doesn't complete mint
    ...
    function adminMintFor(uint256 quantity, address who) external onlyOwner {

        uint256 targetMintIndex = currentMintIndex();
        require(targetMintIndex <= DIFFUSION_MAX_SUPPLY_INDEX, "Sold out! Sorry!");
        require(quantity > 0, "Cannot mint 0");

        adminFreeMinted += uint64(quantity);
        ...
        IOverrideNft(diffusionNftContract).mintBatch(who, ids);
