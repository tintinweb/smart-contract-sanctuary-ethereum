...
contract TheShrine is FxBaseRootTunnel, Ownable {
    ...
    mapping(address => bool) whitelistedContract;
    ...
    function stake(
        ...
        locked[collection][tokenId] = lockupDelta;
    ...
    function unstake(uint256 tokenId, address collection) external notPaused {
        ...
        if (
            locked[collection][tokenId] != 0 ||
            locked[collection][tokenId] >= block.timestamp
        )
            revert TokenLocked({
                tokenId: tokenId,
                currentTime: block.timestamp,
                unlockTime: locked[collection][tokenId]
            });
        ...
        _sendMessageToChild(abi.encode(msg.sender, collection, tokenId, false));
    ...
    function stakeMultiple(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            locked[collection][tokenIds[i]] = lockupDelta;
        ...
        notPaused
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (
                locked[collection][tokenIds[i]] != 0 ||
                locked[collection][tokenIds[i]] >= block.timestamp
            )
                revert TokenLocked({
                    tokenId: tokenIds[i],
                    currentTime: block.timestamp,
                    unlockTime: locked[collection][tokenIds[i]]
                });
            ...
            _sendMessageToChild(
                abi.encode(msg.sender, collection, tokenIds[i], false)
            );
