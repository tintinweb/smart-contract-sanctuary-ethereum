...
// store all the used address and token those transfer by admin
struct GetToken {
    address users;
    uint256 tokens;
}
...
contract Community is AccessControl {
    ...
    // store all the used address and token those transfer by admin
    mapping(uint256 => GetToken[]) gettokens;
    ...
    mapping(uint256 => Communitie) public alltypes;
    ...
    constructor(Iblc_token token_) {
        ...
        alltypes[1] = Communitie(
            ...
            0, // Distribution Start time
            0, // Distribution End time
            false, // Is Distribution Start
            true // Is Distribution End
        );
        alltypes[2] = Communitie(
            "Liquidity",
            0, // Cliff month
            4, // Vested Month
            10 * 1e25, // Total token
    ...
    function distributionStart() external onlyOwner {
        for (uint256 commId = 1; commId <= 3; ) {
            require(
                alltypes[commId].isdistStart != true,
                "Distribution Already Started"
            );
            alltypes[commId].startTime = block.timestamp;
            alltypes[commId].isdistStart = true;
            unchecked {
                commId++;
            }
    ...
    function distributionEnd(uint256 commId) external onlyOwner {
        uint256 _cliff = alltypes[commId].cliff;
        uint256 _vested = alltypes[commId].vested;
        uint256 _startTime = alltypes[commId].startTime;

        // check cliff and vested month of the distribution token is over or not
        uint256 completeTime = _startTime + ((_cliff + _vested) * 120);
        require(block.timestamp > completeTime, "Cliff/Vested not Over!");
        alltypes[commId].endTime = block.timestamp;
        alltypes[commId].isdistEnd = false;
    }
    ...
    function transferToken(
        ...
        require(alltypes[commId].unlockToken >= amount, "Insufficient Token!");
    ...
    function _sendToken(
        ...
        alltypes[_commId].unlockToken -= amount;
        token.safeTransfer(_user, amount);
        gettokens[_commId].push(GetToken(_user, amount));
    ...
    function checkCliff(uint256 commId) private {
        ...
        uint256 _vested = alltypes[commId].vested;
        uint256 _startTime = alltypes[commId].startTime;
        bool _commStart = alltypes[commId].isdistStart;
        bool _commEnd = alltypes[commId].isdistEnd;
    ...
    function _unlockToken(
        ...
        uint256 _unlocktoken = alltypes[_commId].unlockToken;
        uint256 _remaintoken = alltypes[_commId].remainToken;
        uint256 _avgtoken = alltypes[_commId].avgToken;
        ...
        alltypes[_commId].unlockToken = _unlocktoken;
        alltypes[_commId].remainToken = _remaintoken;
    }
    ...
    function allusers(uint256 _commId) public view returns (GetToken[] memory) {
        return gettokens[_commId];
    }
}
