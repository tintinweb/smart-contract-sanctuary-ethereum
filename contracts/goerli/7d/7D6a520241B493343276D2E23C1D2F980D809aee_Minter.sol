...
import "./ERC721withVotesAndEIP2981.sol";

/// @title Minter
/// @author future rob <[email protected]>
contract Minter is Context, Ownable, Pausable {
    /// ERRORS ///
    error PoolSupplyDepleted();
    error SendCorrectAmount();

    /*
    @param token - Address of the nft
    @param pool - Token ids to be minted
    @param price - Price to mint the token
    ...
    */
    ...
    /// CONSTRUCTOR ///
    /// @notice Creates a new Minter
    /// @param _token The address of the token contract.
    /// @param _pool The pool of token ids that will be minted.
    /// @param _price The price of each token.
    /// @param _baseURI The base URI for the minted tokens.
    constructor(
        address _token,
        uint256[10] memory _pool,
        uint256 _price,
    ...
    /// MINT FUNCTION ///
    /// @notice Mints an NFT from the minter contract.
    /// @notice requires the sender to pay the price of the token.
    /// @dev generates a random unique id from a given pool.
    /// @dev param _to The address that will receive the NFT.
    /// @dev param _tokenId The ID of the NFT.
    function mintNft() public payable {
        if (pool.length == 1) {
            revert PoolSupplyDepleted();
        }
        if (msg.value != price) {
            revert SendCorrectAmount();
        }

        /// @dev Create token reference
        ERC721withVotesAndEIP2981 Token = ERC721withVotesAndEIP2981(token);

        /// @dev Generate a psudo random TokenId from pool
        uint8 randomTokenId = uint8(
            (block.timestamp + block.difficulty) % (pool.length - 1)
        );


        uint256 tokenId = pool[randomTokenId];
        string memory uriRef = Strings.toString(tokenId);
        string memory uri = string(abi.encodePacked(baseURI, uriRef,'.json'));

        Token.externalMint(msg.sender, tokenId, uri);

        /// @dev Move tokenId to the end of the pool
        pool[randomTokenId] = pool[pool.length - 1];

        /// @dev Remove the last item from pool
        pool.pop();
    }

    // CHANGE PRICE FUNCTION ///
    /// @notice Changes the price of the minted tokens.
    /// @dev param _newPrice The new price of the minted tokens.
    function changePrice(uint256 _newPrice) public onlyOwner {
        price = _newPrice;
    }

    // CHANGE BASE URI FUNCTION ///
    /// @notice Changes the base URI of the minted tokens.
    /// @dev param _newBaseURI The new base URI of the minted tokens.
    function changeBaseURI(string memory _newBaseURI) public onlyOwner {
        baseURI = _newBaseURI;
    }

    // RETURN POOL ARRAY BY ID ///
    /// @notice Returns the pool of token by id.
    /// @dev unable to return pool since it has a dynamiclly sized array.
    function getPoolID(uint256 _id) public view returns (uint256) {
        return pool[_id];
    }
    
    /// ETH WITHDRAWAL ///
    /// @notice Withdraw all ETH from the contract to the owner's addres.
    ...
    EIP2981RoyaltyOverrideCore
{
    ...
    /// @notice Owener can sets token royalty
    ...
    /// @notice Owener can sets contract default royalty
    /// @dev See {IEIP2981RoyaltyOverride-setContractDefaultRoyalty}.
