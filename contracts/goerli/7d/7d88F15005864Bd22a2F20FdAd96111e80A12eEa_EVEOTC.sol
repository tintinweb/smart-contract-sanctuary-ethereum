...
interface IEVEOTCStakes {
    function getSmartDiscount(address user) external view returns (uint8);
    function getOptionDiscount(address user) external view returns (uint8);
}
...
contract EVEOTC is EVEOTCTokens {
    ...
    function buySmartTrade(uint256 _index, address _to, uint256 _amount) external nonReentrant isNotPaused {
        ...
        uint32 _commission_buy = commission_smart_trades_buy * (100 - stakes.getSmartDiscount(msg.sender)) / 100;
    ...
    function payPremium(uint256 _index, address _to, uint256 _amount) external nonReentrant isNotPaused {
        ...
        // calculates the commission at the moment of buying, to consider if the user has an active stake / discount
        uint32 _commission_buy = commission_option_trades_buy * (100 - stakes.getOptionDiscount(msg.sender)) / 100;
