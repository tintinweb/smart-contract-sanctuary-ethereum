...
contract APWineZap is Initializable, ERC1155HolderUpgradeable {
     ...
     * @param _apwRegistry the address of the registry contract.
     */
    function initialize(address _apwRegistry) external initializer {
        require(
            _apwRegistry != address(0),
            "ApwineZap: Cannot set registry to zero address"
        );
     ...
     * @param underlyingAddress The address of the asset(underlying token).
    ...
    function zapInToCurvePoolWithAsset(
        ...
        uint256 assetToDeposit = IFutureVault(ibt).convertToAssets(
        ...
        uint256 ptReceived = IFutureVault(_futureVault).deposit(
            assetToDeposit,
            address(this)
        );
        ...
        uint256 ibtReceived = IFutureVault(ibt).deposit(
            _amount - assetToDeposit,
            address(this)
        );
        ...
        require(
            ...
            "APWineZap: Actual lp token amount is less than the minimum expected value"
        );
    ...
    function zapInToCurvePoolWithIBT(
        ...
        uint256 ptReceived = IFutureVault(_futureVault).depositWithIBT(
            _amount - ibtToDepositInPool,
            address(this)
        );
        address yt = IFutureVault(_futureVault).getYT();
    ...
    function _addLiquidityToCurvePool(
        ...
        **/
        ...
        require(success, "ApwineZap: Failed to add liquidity to curve pool");
    ...
    function _calcExpectedLpTokenAmount(
        ...
        require(
            success,
            "ApwineZap: Could not fetch expected LP token for the curve pool"
        );
     ...
     * @param _futureVault the address of the futureVault associated with the pool.
