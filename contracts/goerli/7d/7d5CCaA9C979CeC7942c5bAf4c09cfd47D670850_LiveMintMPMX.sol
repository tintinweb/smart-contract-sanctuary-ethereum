...
contract LiveMintMPMX is Ownable, ReentrancyGuard
{   
    /*-------------------*/
    /*     MAPPINGS      */
    /*-------------------*/
    ...
    mapping(uint=>string) public _Data;                // Returns [ArtistID][TicketID] => Data
    mapping(address => bool) public Role;                               // Returns [Address] => BRT Minter Role Mapping
    mapping(uint=>uint) public _ArtistIDs;                               // Returns [TicketID] => ArtistID
    ...
    address private constant MPMX = 0xb54C3468deCEF88B00F9952990cB081285e64b13;
    address private constant ArtBlocksMinter = 0x9F19DC02Fe3FdF5C9242812c355226186Cf3b71B;
    ...
    { 
        ...
        IERC20(0x9B067ff0bE00e3c9a7F5511D58C4b564D7793784).approve( // BRT Approval For LiveMinting
            ArtBlocksMinter, 
    ...
    /**
     * @dev Batch Mints Verified Users On The Brightlist NFTs
     */
     ...
     */
    function readData(uint Range) public view returns(string[] memory AllDatas)
    {
        string[] memory AllData = new string[](Range);
        for(uint TicketID; TicketID < Range; TicketID++) { AllData[TicketID] = _Data[TicketID]; }
        return AllData;
    }
    ...
    function readCheckedInTicketIDs(uint[] calldata ArtistIDs, uint Range) public view returns(uint[][] memory TokenIDs)
    {
        ...
        for(uint ArtistID; ArtistID < ArtistIDs.length; ArtistID++)
        {
            ...
            for(uint TokenID; TokenID < Range; TokenID++)
            {
                if(BrightListArtist[TokenID] != address(0)) 
                { 
