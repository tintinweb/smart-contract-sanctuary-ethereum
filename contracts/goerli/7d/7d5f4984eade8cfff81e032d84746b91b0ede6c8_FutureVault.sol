...
interface IFutureVault is IERC20Upgradeable, IERC4626Upgradeable {
    ...
    /**
     * @notice deposit underlying token's of another address  - works like {IERC4626t-deposit} pt and yt will be minted to msg.sender
     * @param assets - umount of underlying asset to deposit
     * @param account - address to deposit from
     **/
    function depositFor(
        uint256 assets,
        address account
    ...
    /**
     * @notice deposit ibt token of another address  - works like {IERC4626t-deposit} pt and yt will be minted to msg.sender
     * @param ibtAmount - amount of ibt to deposit
     * @param account - address to deposit from
     **/
    function depositIBTFor(
        uint256 ibtAmount,
        address account
    ) external returns (uint256 shares);
    ...
    PausableUpgradeable
{
    ...
    address public ibt; // interest bearing token held by the vault.
    address private zapDepositor; //zap depositor
    uint256 private IBT_UNIT; // equal to one unit(10^decimals) of the token (IBT) held by the vault.
    uint256 private ASSET_UNIT; // equal to one unit(10^decimals) of the token (asset).
    mapping(address => uint256) private ibtRateOfUser; // stores the ibt rate against the user when he last updated yield.
    mapping(address => uint256) private ptRateOfUser; // stores the pt rate against the user when he last updated yield.
    ...
    uint256 internal ptRate;
    uint256 internal oldIBTRate;
    uint256 internal ibtRate;

    struct UpdateYieldData {
    ...
    /// @notice A modifier that ensures the caller is the admin.
    modifier onlyAdmin() {
     ...
     * @param _asset Asset which the user deposits in vault(Underlying).
     * @param _ibt The token which vault holds.
    ...
    function initialize(
        ...
        __Pausable_init();
        EXPIRY = _expiry;
        IBT_UNIT = 10 ** IERC20MetadataUpgradeable(_ibt).decimals();
        ASSET_UNIT = 10 ** _asset.decimals();
        admin = _admin;
        ibt = _ibt;
        yt = _deployYT("Yield Token", "YT");
        ibtRate = convertToAssetsOfIBT(IBT_UNIT);
        ptRate = ASSET_UNIT;
    ...
    function deposit(
        uint256 assets,
        address receiver
    ) public override notExpired whenNotPaused returns (uint256 shares) {
    ...
    /** @dev See {IFutureVault-depositWithIBT}.*/
    function depositWithIBT(
        uint256 ibtAmount,
        address receiver
    )
    ...
    function depositFor(
        ...
        shares = _depositIBT(ibtAmount, msg.sender);
    }
        ...
        returns (uint256 assets)
    {
        ...
        uint256 assetsInIBT = convertToSharesOfIBT(assets);
        IERC4626Upgradeable(ibt).withdraw(assetsInIBT, receiver, address(this));
    ...
    {
        transferYield = false;
        uint256 yield = claimYield(owner);
        uint256 maxAssetsToWithdraw = maxWithdraw(owner);
        require(
            maxAssetsToWithdraw >= assets,
        ...
        if (isUserYieldNegative[owner]) {
            shares = convertToAssetsOfIBT(yieldOfUserInIBT[owner]) + assets;
            shares = shares > yt.balanceOf(owner)
                ? yt.balanceOf(owner)
                : shares;
            yieldOfUserInIBT[owner] = 0;
            isUserYieldNegative[owner] = false;
            amountToWithdraw = assetsInIBT;
        } else {
            shares = convertToShares(assets);
            amountToWithdraw = assetsInIBT + yield;
        }
        ibtRateOfUser[owner] = convertToAssetsOfIBT(IBT_UNIT);
        yt.burnWithoutClaim(owner, shares);

        _withdraw(msg.sender, receiver, owner, amountToWithdraw, shares);
    ...
    function previewDeposit(
        ...
        require(block.timestamp <= EXPIRY, "FutureVault: Vault has expired");
        return convertToShares(assets);
    }
    ...
    function totalAssets()
        ...
        view
        virtual
        override(APWineERC4626Upgradeable, IERC4626Upgradeable)
        returns (uint256)
    {
        uint256 ibtBalance = IERC20MetadataUpgradeable(ibt).balanceOf(
            address(this)
        );
    ...
    /** @dev See {IFutureVault-convertToSharesWithRate}. */
    function convertToSharesWithRate(
        uint256 assets,
        uint256 _ptRate
    ) public view returns (uint256 shares) {
        return (assets * ASSET_UNIT) / _ptRate;
    }

    /** @dev See {IFutureVault-convertToSharesOfIBT}. */
    function convertToSharesOfIBT(
        uint256 assets
    ) public view override returns (uint256) {
    ...
    /** @dev See {IFutureVault-convertToAssetsWithRate}. */
    function convertToAssetsWithRate(
        uint256 shares,
        uint256 _ptRate
    ) public view returns (uint256 assets) {
        return (shares * _ptRate) / ASSET_UNIT;
    }

    /** @dev See {IFutureVault-convertToAssetsOfIBT}. */
    function convertToAssetsOfIBT(
        uint256 ibtAmount
    ) public view override returns (uint256) {
    ...
    /** @dev See {IFutureVault-convertIBTToAssetsWithRate}. */
    function convertIBTToAssetsWithRate(
        uint256 ibtAmount,
        uint256 _ibtRate
    ) public view override returns (uint256) {
        return (ibtAmount * (_ibtRate)) / (IBT_UNIT);
    }

    /** @dev See {IFutureVault-convertAssetsToIBTWithRate}. */
    function convertAssetsToIBTWithRate(
        uint256 assets,
        uint256 _ibtRate
    ) public view override returns (uint256) {
        ...
        return (assets * (IBT_UNIT)) / (_ibtRate);
    }

    /** @dev See {IFutureVault-underlying}. */
    ...
    function claimYield(address _user) public override returns (uint256) {
        return claimYieldOfAmount(_user, yt.balanceOf(_user));
    }

    /**
     * @dev Claim yield of a specific amount of YT
     * @param _user The user to claim yield for
     * @param _amount The amount setting a ratio amount/yt balance which indicates the proportion of yield to claim
     * @return The amount of yield claimed in IBT
     */
    function claimYieldOfAmount(
        ...
        uint256 userYTBalance = yt.balanceOf(_user);
        if (
            !isUserYieldNegative[_user] &&
            yieldToTransfer != 0 &&
            (_amount != 0 || userYTBalance == 0)
        ) {
            require(
                _amount <= userYTBalance,
                "FutureVault: amount exceeds user YT balance"
            );
            // get the amount to user yt balance ratio (1 if yt balance is 0)
            uint256 ratio = userYTBalance == 0
                ? IBT_UNIT
                : (_amount * IBT_UNIT) / userYTBalance;
            yieldToTransfer = (yieldToTransfer * ratio) / IBT_UNIT;
            require(
                yieldToTransfer <= yieldOfUserInIBT[_user],
                "FutureVault: yieldToTransfer exceeds user yield"
            );
            ...
            emit YieldTransferred(_user, convertToAssetsOfIBT(yieldToTransfer));
            if (!transferYield) {
                transferYield = true;
        ...
        returns (uint256)
    {
        ...
        uint256 maxBurnableShares = (ytBalance > ptBalance)
            ? ptBalance
            : ytBalance;
        return paused() ? 0 : convertToAssets(maxBurnableShares);
    }
     ...
     */
    function updateZapDepositor() public onlyAdmin {
        zapDepositor = registry.getZapDepositor();
    }
    ...
    /** @dev See {IFutureVault-getIBT}. */
    function getIBT() public view override returns (address) {
        return ibt;
    }

    /** @dev See {IFutureVault-getYT}. */
    function getYT() public view override returns (address) {
        return address(yt);
    }
    ...
    function claimFees() public returns (uint256) {
        ...
        uint256 underlyingAmount = IERC4626Upgradeable(ibt).withdraw(
            _fees,
            feeCollector,
            address(this)
        );
    ...
    function updateYield(address _user) public override returns (uint256) {
        UpdateYieldData memory data;
        data.oldIBTRateUser = ibtRateOfUser[_user];
        data.oldPTRateUser = ptRateOfUser[_user];
        _updatePTRate();
        data.newIBTRate = ibtRate;
        data.newPTRate = ptRate;
        ibtRateOfUser[_user] = data.newIBTRate;
        ptRateOfUser[_user] = data.newPTRate;
        if (data.oldIBTRateUser == 0) {
            return 0;
        }
        ...
        uint256 ibtOfYT = convertAssetsToIBTWithRate(
            convertToAssetsWithRate(userYTBalance, data.oldPTRateUser),
            data.oldIBTRateUser
        );

        if (
            data.oldPTRateUser == data.newPTRate &&
            data.newIBTRate > data.oldIBTRateUser
        ) {
            ...
            uint256 yieldInIBT = convertAssetsToIBTWithRate(
                yieldInUnderlying,
                data.newIBTRate
            );
            ...
            if (data.oldPTRateUser > data.newPTRate) {
                // PT depeg happened
                if (data.newIBTRate >= data.oldIBTRateUser) {
                    ...
                    uint256 expectedNegativeYieldInUnderlying = MathUpgradeable
                        .ceilDiv(
                            ibtOfYT * (data.oldIBTRateUser - data.newIBTRate),
                            IBT_UNIT
                        );
        ...
        emit YieldUpdated(_user, netUserYield);
        return netUserYield;
    }
    ...
    function beforeYtTransfer(
        ...
        if (isUserYieldNegative[_from]) {
            ...
            if (isUserYieldNegative[_to]) {
                ...
                if (yieldOfReceiver >= proportionateYield) {
                    yieldOfUserInIBT[_to] =
                        yieldOfReceiver -
                        proportionateYield;
                } else {
                    yieldOfUserInIBT[_to] =
        ...
        require(
            ...
            "FutureVault: Cannot transfer more than the balance"
        );
    }
    ...
    function _convertToShares(
        ...
        return (assets * ASSET_UNIT) / ptRate;
    }
    ...
    function _convertToAssets(
        ...
        return (shares * ptRate) / ASSET_UNIT;
    }
    ...
    /**
     * @dev Internal function for updating PT rate i.e. depegging it if negative yield happened.
     */
    function _updatePTRate() internal virtual {
        uint256 currentRate = convertToAssetsOfIBT(IBT_UNIT);
        if (currentRate < ibtRate) {
            ptRate = (ptRate * currentRate) / ibtRate;
        }
        if (currentRate != ibtRate) {
            ibtRate = currentRate;
        }
...
contract YT is Initializable, ERC20PermitUpgradeable {
    ...
    function burn(uint256 amount) public {
        futureVault.claimYield(msg.sender);
