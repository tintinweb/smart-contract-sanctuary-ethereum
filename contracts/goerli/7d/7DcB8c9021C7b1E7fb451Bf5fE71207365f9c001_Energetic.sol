...
contract Energetic is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_GIFTED_TOKENS = 100;
    ...
    string constant TITLE = "Test";
    string constant DESCRIPTION = "Testing Description";
    ...
    string constant SVG_RECT_OPENER =
        '<rect width="500" height="500" fill="url(#outer_layer)">';
    string constant SVG_RECT_CLOSER = "</rect>";
    string constant SVG_CIRCLE_OPENER =
        '<ellipse width="500" height="500" cx="250" cy="250" rx="0" fill="url(#inner_layer)">';
    string constant SVG_CIRCLE_CLOSER = "</ellipse>";
    ...
    // ==================================================== //
    /// @notice ACCESS CONTROL / SANITY MODIFIERS
        ...
        maxTokensPerWallet(numberOfTokens)
    {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, createToken());
        }
    }
    ...
    // ==================================================== //
    /// @notice PUBLIC read-only functions
    ...
    // ==================================================== //
    /// @notice OWNER read-only admin functions

    function setIsSaleActive(bool _isSaleActive) external onlyOwner {
        isSaleActive = _isSaleActive;
    }

    function reserveForGifting(uint256 numOfTokensToReserve)
        external
        nonReentrant
        onlyOwner
        canGiftTokens(numOfTokensToReserve)
    {
        numGiftedTokens += numOfTokensToReserve;
        for (uint256 i = 0; i < numOfTokensToReserve; i++) {
            _safeMint(msg.sender, createToken());
        }
    }
        ...
        canGiftTokens(addresses.length)
    {
        ...
        for (uint256 i = 0; i < numToGift; i++) {
            _safeMint(addresses[i], createToken());
        }
    }
    ...
    function generateOuterEnergy() public pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    SVG_RECT_OPENER,
                    '<animate attributeName="rx" values="10;250;10" dur="30s" repeatCount="indefinite" />',
                    SVG_RECT_CLOSER
                )
            );
    }

    function generateInnerEnergy() public pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    SVG_CIRCLE_OPENER,
                    '<animate attributeName="ry" attributeName="rx" values="220;0;220" dur="24s" repeatCount="indefinite"/>',
                    SVG_CIRCLE_CLOSER
                )
            );
    }

    function generateOuterEnergyGradient() public pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '<radialGradient id="outer_layer">'
                    '<stop offset="0" stop-color="hsl(160, 100%, 50%)" />',
                    '<stop offset="50%" stop-color="hsl(48, 100%, 50%)" />',
    ...
    function generateInnerEnergyGradient() public pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '<radialGradient id="inner_layer">'
                    '<stop offset="0%" stop-color="hsl(300, 100%, 50%)" />',
                    '<stop offset="50%" stop-color="hsl(30, 100%, 50%)" />',
                    '<stop offset="100%" stop-color="hsl(187, 100%, 50%)" />',
                    "</radialGradient>"
                )
            );
    }
        ...
        returns (string memory)
    {
        ...
        bytes memory image = abi.encodePacked(SVG_PROTOCOL_URI, generateSVG());
