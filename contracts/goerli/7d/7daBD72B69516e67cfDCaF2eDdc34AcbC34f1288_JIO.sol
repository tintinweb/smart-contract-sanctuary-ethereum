...
import { IERC721, ERC721r } from "./ERC721R.sol";
...
contract JIO is ERC721r, Ownable, ReentrancyGuard, DefaultOperatorFilterer, ERC2981 {

  constructor() ERC721r("JIO", "JIO", 8000) {
    _setDefaultRoyalty(msg.sender, 500);
  }

  event RevealMinted(address indexed from, uint256[] tokenIds);

  address public constant unrevealedContract = 0xa8d11e6c17B4fe6611FcAD65e8a88288f3926C0d;
  address public constant burnAddress = 0x000000000000000000000000000000000000dEaD;

  string private baseTokenURI;
  bool public revealActive = false;
  uint256 public revealLimit = 10;

  function revealNFT(uint256[] calldata tokenIds) external nonReentrant {
    ...
    if (!nft.isApprovedForAll(msg.sender, address(this))) revert UnapprovedForTokenTransfer();
    ...
    uint256[] memory mintedTokenIds = _mintRandom(msg.sender, tokenIds.length);
    emit RevealMinted(msg.sender, mintedTokenIds);
  }
  ...
  function setApprovalForAll(address operator, bool approved) public override (ERC721r) onlyAllowedOperatorApproval(operator) {
    super.setApprovalForAll(operator, approved);
  }

  function approve(address operator, uint256 tokenId) public override (ERC721r) onlyAllowedOperatorApproval(operator) {
...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _mintRandom(address to, uint _numToMint) internal virtual returns (uint256[] memory) {
        ...
        uint256[] memory mintedTokenIds = new uint256[](_numToMint);
