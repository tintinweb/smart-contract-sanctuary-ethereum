...
contract Drealm is Initializable, UUPSUpgradeable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    IERC20Upgradeable public realm;
    ...
    struct UserInfo {
        ...
        uint256 rewardREALMDebt; // Reward debt in REALM.
        //
        // We do some fancy math here. Basically, any point in time, the amount of REALM
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accREALMPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accREALMPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. REALMs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that REALMs distribution occurs.
        uint256 accREALMPerShare; // Accumulated REALMs per share, times 1e12. See below.
        uint256 lastTotalREALMReward; // last total rewards
        uint256 lastREALMRewardBalance; // last REALM rewards tokens
        uint256 totalREALMReward; // total REALM rewards tokens
    }
    ...
    // Bonus muliplier for early REALM makers.
    ...
    // total REALM staked
    uint256 public totalREALMStaked;
    // total REALM used for purchase land
    uint256 public totalRealmUsedForPurchase;
    ...
    function add(uint256 _allocPoint, IERC20Upgradeable _lpToken, bool _withUpdate) public onlyOwner {
        ...
        poolInfo.push(PoolInfo({
            ...
            accREALMPerShare: 0,
            lastTotalREALMReward: 0,
            lastREALMRewardBalance: 0,
            totalREALMReward: 0
        }));
    }

    // Update the given pool's REALM allocation point. Can only be called by the owner.
    ...
    // View function to see pending REALMs on frontend.
    function pendingREALM(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accREALMPerShare = pool.accREALMPerShare;
        uint256 lpSupply = totalREALMStaked;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalREALMStaked.sub(totalRealmUsedForPurchase));
            uint256 _totalReward = rewardBalance.sub(pool.lastREALMRewardBalance);
            accREALMPerShare = accREALMPerShare.add(_totalReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accREALMPerShare).div(1e12).sub(user.rewardREALMDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalREALMStaked.sub(totalRealmUsedForPurchase));
        uint256 _totalReward = pool.totalREALMReward.add(rewardBalance.sub(pool.lastREALMRewardBalance));
        pool.lastREALMRewardBalance = rewardBalance;
        pool.totalREALMReward = _totalReward;
        
        uint256 lpSupply = totalREALMStaked;
        if (lpSupply == 0) {
            pool.lastRewardBlock = block.number;
            pool.accREALMPerShare = 0;
            pool.lastTotalREALMReward = 0;
            user.rewardREALMDebt = 0;
            pool.lastREALMRewardBalance = 0;
            pool.totalREALMReward = 0;
            return;
        }
        
        uint256 reward = _totalReward.sub(pool.lastTotalREALMReward);
        pool.accREALMPerShare = pool.accREALMPerShare.add(reward.mul(1e12).div(lpSupply));
        pool.lastTotalREALMReward = _totalReward;
    }

    /**
    @notice Sorting the highest REALM staker in pool
    @param _pid : pool id
    @param left : left
    @param right : right
    @dev Description :
        It is used for sorting the highest REALM staker in pool. This function definition is marked
    ...
    */
    ...
    @dev Description :
    DAO governance will be performed by the top 50 wallets with the highest amount of staked REALM tokens.
    */
    ...
    /**
    @notice REALM staking track the Highest 50 staked users
    @param _pid : pool id
    ...
    */
    ...
    // Deposit REALM tokens to MasterChef.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            uint256 realmReward = user.amount.mul(pool.accREALMPerShare).div(1e12).sub(user.rewardREALMDebt);
            pool.lpToken.safeTransfer(msg.sender, realmReward);
            pool.lastREALMRewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalREALMStaked.sub(totalRealmUsedForPurchase));
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        totalREALMStaked = totalREALMStaked.add(_amount);
        user.amount = user.amount.add(_amount);
        user.rewardREALMDebt = user.amount.mul(pool.accREALMPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        totalREALMStaked = totalREALMStaked.sub(_amount);
        user.rewardREALMDebt = user.amount.mul(pool.accREALMPerShare).div(1e12);
    ...
    // Earn REALM tokens to MasterChef.
    function claimREALM(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
    ...
    // Safe REALM transfer function to admin.
    function accessREALMTokens(uint256 _pid, address _to, uint256 _amount) public {
        require(msg.sender == adminAddress, "sender must be admin address");
        require(totalREALMStaked.sub(totalRealmUsedForPurchase) >= _amount, "Amount must be less than staked REALM amount");
        PoolInfo storage pool = poolInfo[_pid];
        uint256 RealmBal = pool.lpToken.balanceOf(address(this));
        if (_amount > RealmBal) {
            pool.lpToken.transfer(_to, RealmBal);
            totalRealmUsedForPurchase = totalRealmUsedForPurchase.add(RealmBal);
            emit EmergencyWithdraw(_to, _pid, RealmBal);
        } else {
            pool.lpToken.transfer(_to, _amount);
            totalRealmUsedForPurchase = totalRealmUsedForPurchase.add(_amount);
