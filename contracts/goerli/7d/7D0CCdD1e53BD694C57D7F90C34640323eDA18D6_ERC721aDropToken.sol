...
contract ERC721aMinos is ERC721AUpgradeable, TwoStepOwnable, ISeaDropTokenContractMetadata {
    uint256 _maxSupply;
    ...
    function hide() external virtual onlyOwner {
        revealed = false;
    }

    function tokenURI(uint256 tokenId) public virtual view override returns (string memory) {
        require(_exists(tokenId), "token id does not exist");
        string memory metadataPointerId = !revealed ? 'unrevealed' : string(abi.encodePacked("assets/", Strings.toString(tokenId)));
        string memory result = string(abi.encodePacked(_tokenBaseURI, metadataPointerId, '/metadata.json'));
        return bytes(_tokenBaseURI).length != 0 ? result : '';
    }
}
