...
// These constants are from TickMath.sol
/// @dev The minimum value that can be returned from #getSqrtRatioAtTick. Equivalent to getSqrtRatioAtTick(MIN_TICK)
uint160 constant MIN_SQRT_RATIO = 4295128739;
/// @dev The maximum value that can be returned from #getSqrtRatioAtTick. Equivalent to getSqrtRatioAtTick(MAX_TICK)
uint160 constant MAX_SQRT_RATIO = 1461446703485210103287273052203988822378723970342;
uint32 constant TOTAL_PERCENT = 10000;

contract RevenueManager is IUniswapV3SwapCallback, Initializable, AccessControl, IRevenueManager {
  bytes32 public constant RM_ADMIN_ROLE = keccak256("RM_ADMIN_ROLE");

  IWETH9 public weth;
  address public uniswapV3Pool;
  address public uniswapPoolToken0;
  address public uniswapPoolToken1;
  address public projectToken;
  /// @notice revenueToken is the other token which forms the primary liquidity pair
  // with instanceToken on the liquidity pool
  ...
  DerivedSplitInfo public derivedSplitInfo;
  Split[] public splits;

  ICoCreateLaunch public coCreate;
  ...
  /// @notice This initializes the Revenue Manager with the given params
  /// @param uniswapV3Pool_ The address of the uniswapV3Pool where the swaps happen
  /// @param instanceToken_ The address of the project fungible token
  /// Value can range from 0-10,000. A value of 10,000 represents 100%. Ex: If the RevenueManager has 100WETH
  /// and ftPercentageSwap_ is 9,000, then 90% of the holdings (90 ETH) is swapped for the project FT
  function initialize(
    string memory name_,
    string memory description_,
    address uniswapV3Pool_,
    address instanceToken_,
    address revenueToken_,
    address owner_,
    Split[] calldata splits_,
    ICoCreateLaunch coCreate_,
    ICoCreateProject project_
  ) public initializer {
    require(revenueToken_ != address(0), "Invalid revenueToken address");
    require(address(project_) != address(0), "Invalid createInstance address");
    name = name_;
    description = description_;
    project = project_;
    _grantRole(RM_ADMIN_ROLE, owner_);
    _setRoleAdmin(RM_ADMIN_ROLE, RM_ADMIN_ROLE);
    coCreate = coCreate_;
    projectToken = instanceToken_;
    revenueToken = revenueToken_;
    weth = coCreate.getWeth9();
    // TODO: Add support for case when revenueToken != weth
    // For now only revenueToken == weth is supported
    require(revenueToken == address(weth), "Only revenueToken == weth_ supported");
    _updateSplit(splits_);
    uniswapV3Pool = uniswapV3Pool_;
    if (uniswapV3Pool_ != address(0) && instanceToken_ != address(0)) {
      _initializeUniswapPool(uniswapV3Pool_);
    }
  }

  function _initializeUniswapPool(address uniswapV3Pool_) internal {
    uniswapPoolToken0 = IUniswapV3PoolImmutables(uniswapV3Pool_).token0();
    uniswapPoolToken1 = IUniswapV3PoolImmutables(uniswapV3Pool_).token1();
    if (projectToken == uniswapPoolToken0) {
      require(revenueToken == uniswapPoolToken1, "Invalid pool");
      sqrtPriceLimitX96 = MAX_SQRT_RATIO - 1;
      zeroForOne = false;
    } else if (projectToken == uniswapPoolToken1) {
      require(revenueToken == uniswapPoolToken0, "Invalid pool");
  ...
  function initializeUniswapPool(address uniswapV3Pool_) external onlyRole(RM_ADMIN_ROLE) {
    require(uniswapV3Pool_ != address(0), "Invalid uniswapV3Pool_");
    require(uniswapV3Pool == address(0), "Already initialized");
    uniswapV3Pool = uniswapV3Pool_;
    _initializeUniswapPool(uniswapV3Pool_);
  }

  function updateSplit(Split[] calldata splits_) external onlyRole(RM_ADMIN_ROLE) {
    _updateSplit(splits_);
  }

  function _updateSplit(Split[] calldata splits_) internal {
    uint256 len = splits_.length;
    DerivedSplitInfo memory derivedSplitInfo_ = DerivedSplitInfo({
      instanceTokenSplits: new uint32[](len),
      instanceTokenSplitsTotal: 0,
      revenueTokenSplits: new uint32[](len),
      revenueTokenSplitsTotal: 0
    });
    uint32 cumulativePercentShareOfRevenue_ = 0;
    delete splits;
    for (uint256 i = 0; i < len; i++) {
      address payee = splits_[i].payee;
      uint32 percentShareOfRevenue = splits_[i].percentShareOfRevenue;
      require(payee != address(0), "Invalid payee");
      require(percentShareOfRevenue <= TOTAL_PERCENT, "Invalid percentShareOfRevenue");
      require(splits_[i].percentRevenueInProjectToken <= TOTAL_PERCENT, "Invalid percentRevenueInProjectToken");
      cumulativePercentShareOfRevenue_ += percentShareOfRevenue;
      derivedSplitInfo_.instanceTokenSplits[i] =
        (splits_[i].percentShareOfRevenue * splits_[i].percentRevenueInProjectToken) /
        TOTAL_PERCENT;
      derivedSplitInfo_.instanceTokenSplitsTotal += derivedSplitInfo_.instanceTokenSplits[i];
      derivedSplitInfo_.revenueTokenSplits[i] = percentShareOfRevenue - derivedSplitInfo_.instanceTokenSplits[i];
      derivedSplitInfo_.revenueTokenSplitsTotal += derivedSplitInfo_.revenueTokenSplits[i];
      splits.push(splits_[i]);
    }
    require(cumulativePercentShareOfRevenue_ == TOTAL_PERCENT, "percentShareOfRevenueCumulative does not add up");

    // splits = splits_;
    derivedSplitInfo = derivedSplitInfo_;
  }

  function _distributeInstanceTokens() internal {
    uint256 instanceTokenBalanceAfterSwap = IERC20(projectToken).balanceOf(address(this));
    uint256 len = splits.length;
    uint256 splitsTotal = derivedSplitInfo.instanceTokenSplitsTotal;
    if (instanceTokenBalanceAfterSwap > 0 && splitsTotal > 0) {
      for (uint256 i = 0; i < len; i++) {
        uint256 payoutAmount = (derivedSplitInfo.instanceTokenSplits[i] * instanceTokenBalanceAfterSwap) / splitsTotal;
        if (payoutAmount > 0) {
          address payee = splits[i].payee;
          SafeERC20.safeTransfer(IERC20(projectToken), payee, payoutAmount);
          emit RevenuePayout(projectToken, payee, payoutAmount);
        }
      }
    }
  }

  function _distributeRevenueTokens() internal {
    uint256 revenueTokenBalanceAfterSwap = IERC20(revenueToken).balanceOf(address(this));
    uint256 len = splits.length;
    uint32 revenueTokenSplitsTotal = derivedSplitInfo.revenueTokenSplitsTotal;
    if (revenueTokenBalanceAfterSwap > 0 && revenueTokenSplitsTotal > 0) {
      for (uint256 i = 0; i < len; i++) {
        address payee = splits[i].payee;
        uint256 payoutAmount = (derivedSplitInfo.revenueTokenSplits[i] * revenueTokenBalanceAfterSwap) /
          revenueTokenSplitsTotal;
        if (payoutAmount > 0) {
          SafeERC20.safeTransfer(IERC20(revenueToken), payee, payoutAmount);
          emit RevenuePayout(revenueToken, payee, payoutAmount);
        }
      }
    }
  }

  function distribute() public onlyRole(RM_ADMIN_ROLE) {
    // Convert eth to weth
    weth.deposit{ value: address(this).balance }();

    // Sent protocol fee to protocolFeeRecipient
    uint32 protocolFeePercent = coCreate.getCoCreateFeePercent(address(project));
    address protocolFeeRecipient = coCreate.getProtocolFeeRecipient();
    uint256 protocolFeeAmount = (IERC20(revenueToken).balanceOf(address(this)) * protocolFeePercent) / TOTAL_PERCENT;
    SafeERC20.safeTransfer(IERC20(revenueToken), address(protocolFeeRecipient), protocolFeeAmount);

    uint256 revenueFTBalance = IERC20(revenueToken).balanceOf(address(this));
    uint256 tokensToSwap = ((revenueFTBalance * derivedSplitInfo.instanceTokenSplitsTotal) / TOTAL_PERCENT);
    if (tokensToSwap > 0) {
      require(uniswapV3Pool != address(0), "Uninitialized pool");
      // Swap as many token as possible
      //TODO: Change to limit order and validate result of swap and balances
      IUniswapV3PoolActions(uniswapV3Pool).swap(address(this), zeroForOne, int256(tokensToSwap), sqrtPriceLimitX96, "");
    }
    if (projectToken != address(0)) {
      _distributeInstanceTokens();
    }
    _distributeRevenueTokens();
  }
  ...
  function uniswapV3SwapCallback(
    ...
    require(msg.sender == uniswapV3Pool, "Invalid uniswap callback");
    if (amount0Delta > 0) {
      SafeERC20.safeTransfer(IERC20(uniswapPoolToken0), uniswapV3Pool, uint256(amount0Delta));
    }
    if (amount1Delta > 0) {
      SafeERC20.safeTransfer(IERC20(uniswapPoolToken1), uniswapV3Pool, uint256(amount1Delta));
    }
  }
  ...
  function withdrawOtherERC20(
    ...
    uint256 amount,
    address transferTo
  ) external override onlyRole(RM_ADMIN_ROLE) {
