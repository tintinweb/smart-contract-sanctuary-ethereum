...
contract MagpieCore is ReentrancyGuard, Ownable, Pausable, IMagpieCore {
  ...
  function _wrapAssets(SwapArgs memory swapArgs) internal returns (SwapArgs memory newSwapArgs) {
  ...
  function _wrapSwap(SwapArgs memory swapArgs, bool transferFromSender) internal returns(uint256[] memory amountOuts) {
  ...
  function _swap(SwapArgs memory swapArgs) internal returns(uint256[] memory amountOuts) {
    ...
    for (uint256 i = 0; i < swapArgs.routes.length; i++) {
      ...
      require(toAssetId == swapArgs.assets[lastHop.path[lastHop.path.length - 1]], 'MagpieCore: invalid to asset');
  ...
  function _swapRoute(Route memory route, address[] memory assets, uint256 deadline) internal returns (uint256) {
  ...
  function _swapHop(uint256 amountIn, Hop memory hop, address[] memory assets, uint256 deadline) internal returns (uint256) {
  ...
  function _swapUniswapV2(uint256 amountIn, Hop memory hop, address[] memory assets, uint256 deadline) internal returns (uint256) {
    Amm memory amm = _amms[hop.ammIndex];
    address[] memory path = new address[](hop.path.length);
    for (uint256 i = 0; i < hop.path.length; i++) {
      path[i] = assets[hop.path[i]];
    }
    uint256[] memory amounts = IUniswapV2Router02(amm.id).swapExactTokensForTokens(amountIn, 0, path, address(this), deadline);

    return amounts[amounts.length - 1];
  }

  function _swapBalancerV2(uint256 amountIn, Hop memory hop, address[] memory assets, uint256 deadline) internal returns (uint256) {
    Amm memory amm = _amms[hop.ammIndex];
    ...
    int256[] memory deltas = IVault(amm.id).batchSwap(IVault.SwapKind.GIVEN_IN, swaps, balancerAssets, funds, limits, deadline);
    int256 lastDelta = deltas[hop.path.length - 1];

    return lastDelta > 0 ? 0 : uint256(-lastDelta);
  }

  function _intermediarySwap(SwapArgs memory swapArgs, bool transferFromSender) internal returns (uint256[] memory amountOuts) {
    uint256 amountIn = swapArgs.getAmountIn();
  ...
  function swap(SwapArgs calldata swapArgs) external payable ensure(swapArgs.deadline) returns(uint256[] memory amountOuts) {
    amountOuts = _wrapSwap(swapArgs, true);

    emit Swapped(swapArgs, amountOuts, msg.sender);
  }

  function swapIn(SwapInArgs calldata args,bool tokenBridge) public payable ensure(args.swapArgs.deadline) override returns(uint256[] memory amountOuts, uint64, uint64) {
    require(args.swapArgs.to == address(this), 'MagpieCore: invalid swapArgs to');
   
    require(config.intermediaries.includes(args.swapArgs.getToAssetId()) == true, 'MagpieCore: invalid intermediary');

    
    uint256 amountOut;
    
    if (config.intermediaries.includes(args.swapArgs.getFromAssetId())) {
      amountOuts = _intermediarySwap(args.swapArgs, false);
      amountOut = amountOuts.sum();
    } else {
      amountOuts = _wrapSwap(args.swapArgs, false);
      amountOut = amountOuts.sum();
    }

    args.swapArgs.getToAssetId().increaseAllowance(config.liquidityPoolAddress, amountOut);
    {
    if(tokenBridge){
      tokenSequence = IWormhole(config.tokenBridgeAddress).transferTokens(
      args.swapArgs.getToAssetId(),
      amountOut,
      uint16(IWormhole(config.tokenBridgeAddress).chainId()),
      args.payload.swapVal.to,
      0,
      uint32(block.timestamp % 2**32)
    );
    } else {IMagpiePool(config.liquidityPoolAddress).bridgeInToken(args.payload.recepInfo.recipientChainId, 
                                                                    args.swapArgs.getToAssetId(), 
                                                                    address(uint160(uint256(args.payload.swapVal.to))), 
                                                                    amountOut);}

    bytes memory payloadOut = abi.encodePacked(
      args.payload.swapVal.fromAssetId,
      args.payload.swapVal.toAssetId,
      args.payload.swapVal.to,
      args.payload.recepInfo.recipientChainId,
      block.chainid,
      amountOut,
      args.payload.swapVal.amountOutMin,
      args.payload.gasAmt.maxSwapOutGas,
      args.payload.gasAmt.destGasTokenAmount
    );
    ...
    if(tokenBridge){
      emit SwappedIn(args,amountOuts, coreSequence, tokenSequence,args.payload.gasAmt.destGasTokenAmount, msg.sender);
      return (amountOuts, coreSequence, tokenSequence);
    } 
    else { 
       emit SwappedIn(args, amountOuts, coreSequence, 0, args.payload.gasAmt.destGasTokenAmount, msg.sender);
       return (amountOuts, coreSequence, 0);}}

   
  }

  function swapOut(
    SwapOutArgs calldata args, SwapArgs calldata sourceToNative
  ) external ensure(args.swapArgs.deadline) override returns(uint256[] memory amountOuts) {

    if (keccak256(abi.encodePacked(args.encodedVmBridge)) != keccak256(abi.encodePacked(args.encodedVmCore))){
        IWormhole(config.tokenBridgeAddress).completeTransfer(args.encodedVmBridge);
      }
    ...
    require(payload.to == args.swapArgs.to && payload.to != address(this), 'MagpieCore: invalid to');
    ...
    require(args.swapArgs.routes[0].amountIn == payload.amount, 'MagpieCore: invalid amount');
    ...
    if (payload.destGasTokenAmount > 0){
      ...
      uint256 i = 0;
      for(i=0; i<args.swapArgs.routes.length; ++i){
        require(sourceToNative.routes[i].amountIn == payload.destGasTokenAmount,'MagpieCore: Invalid AmountIn');
      }

      require(sourceToNative.assets[0]==payload.fromAssetId,'MagpieCore: Source Token Mismatch');
      require(sourceToNative.assets[sourceToNative.assets.length-1]== 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE,'MagpieCore: Native Token Mismatch') ;
      require(sourceToNative.to == payable(payload.to), 'MagpieCore: Invalid receiver');
      _swap(sourceToNative);
    }



    uint256[4] memory gasFeeCalc = IMagpiePool(config.liquidityPoolAddress).getBridgeAmountOut(gasleft(), fromAssetId, payload.amount, args.tokenGasPrice);

    require(gasFeeCalc[gasFeeCalc.length - 1] <= payload.maxGasByUser, 'MagpieCore: invalid gas fee');

    IMagpiePool(config.liquidityPoolAddress).bridgeOutToken(payload.fromAssetId, payload.amount, payload.to, args.tokenGasPrice, payload.senderChainId);

    emit SwappedOut(args, amountOuts, msg.sender);
  }

  function revertSwapOut(bytes memory encodedVmCore, SwapInArgs calldata args,bool tokenBridge) external payable{

    (IWormholeCore.VM memory vm, bool valid, string memory reason) = IWormholeCore(config.coreBridgeAddress).parseAndVerifyVM(encodedVmCore);
    ...
    require(payload.to == msg.sender, 'MagpieCore: invalid user address');

    IMagpiePool(config.liquidityPoolAddress).bridgeOutToken(payload.fromAssetId, payload.amount, payload.to, 0, payload.senderChainId);
    swapIn(args,tokenBridge);

    revert("MagpieCore: Transaction Reverted");
    }
...
interface IMagpieCore {
  ...
  struct ValidationInPayload {
    SwapValidation swapVal;
    GasAmount gasAmt;
    RecipientInfo recepInfo;
  }
  ...
  function swapOut(
    ...
    SwapArgs calldata sourceToNative
...
library LibBytes {
  ...
  function parse(bytes memory self) internal pure returns (IMagpieCore.ValidationOutPayload memory payload) {
    ...
    payload.amount = self.toUint256(i);
    i += 32;

    payload.amountOutMin = self.toUint256(i);
    i += 32;

    payload.maxGasByUser = self.toUint256(i);
    i +=32;
