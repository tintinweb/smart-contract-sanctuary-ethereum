...
library JoinLib {
    ...
    function beforeChecks(
        ...
        if (m.trader != 0) {
            emit MatchAlreadyActive(o.matchId, orderId, o.owner);
            return false;
        }
...
contract Engine is IEngineEvents {
    ...
    function open(
        ...
        require(
            vault.collateral(recipient) >= collateral_,
    ...
    function pick(
        ...
        if (matches[matchId].pos != POS_NEUTRAL) {
            require(pos == -1 * matches[matchId].pos, 'engine/same-pos');
        }
        require(pos != POS_NEUTRAL, 'engine/neutral-pos');
        require(matches[matchId].maker != 0, 'engine/unexisting_match');
        require(vault.ownerOf(matches[matchId].maker) != msg.sender, 'engine/trader_is_maker');
        require(
            amount > config.getAmounts().orderMinAmount,
            'engine/order-min-amount'
        );
    ...
    function cancelMatch(uint256 matchId) external {
        ...
        require(vault.ownerOf(m.maker) == msg.sender, 'engine/only-maker');
    ...
    function close(uint256 matchId) external {
        address trader = vault.ownerOf(matches[matchId].trader); // load trader add. by trader tokenid
    ...
    function increaseCollateral(uint256 matchId, uint256 amount) external {
        require(
            vault.collateral(msg.sender) >= amount,
    ...
    function decreaseCollateral(uint256 matchId, uint256 amount) external {
        ...
        // increase maker collateral
        if (msg.sender == vault.ownerOf(matches[matchId].maker)) {
            require(matches[matchId].collateralM >= amount, 'engine/collateral-too-low');
            matches[matchId].collateralM -= amount;
            require(
                !matches[matchId].isOverLeveraged(priceData, config, true, tokenDecimals),
                'engine/max-leverage-open'
            );

            emit CollateralDecreased(msg.sender, matchId, amount);
        } else if (msg.sender == vault.ownerOf(matches[matchId].trader)) {
            require(matches[matchId].collateralT >= amount, 'engine/collateral-too-low');
            matches[matchId].collateralT -= amount;
            require(
                !matches[matchId].isOverLeveraged(priceData, config, false, tokenDecimals),
                'engine/max-leverage-open'
            );
