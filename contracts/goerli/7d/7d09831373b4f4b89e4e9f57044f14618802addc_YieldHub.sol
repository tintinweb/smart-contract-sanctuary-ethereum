...
contract YieldHub is Ownable {
	...
	IERC721 public kongzContract = IERC721(0x57a204AA1042f6E66DD7730813f4024114d74f37);
	...
	// admin //
	///////////
	function setBanana(address _nana) external onlyOwner {
		newNana = _nana;
	}
	function setKongz(address _kongz) external onlyOwner {
		kongzContract = IERC721(_kongz);
	}
	...
	function getTotalClaimable(address _user, address _token) external view returns(uint256) {
		...
		uint256 bal = kongzContract.balanceOf(_user);
		uint256 delta = time.sub(max(data.lastUpdate, yieldToken.start));

		uint256 pending = bal.mul(yieldToken.weeklyRate.mul(delta));//.div(604800);
	...
	function getReward(address _user) public {
		...
		updateUserToken(_user, newNana, balOf);
	...
	function updateUserToken(address _user, address _token, uint256 _balOf) internal {
		...
		if (block.timestamp > yieldToken.start) {
			...
			if (userLastUpdate > 0 && delta > 0) {
				_userData.rewards += _balOf.mul(yieldToken.weeklyRate).mul(delta);//.div(604800);
			}
