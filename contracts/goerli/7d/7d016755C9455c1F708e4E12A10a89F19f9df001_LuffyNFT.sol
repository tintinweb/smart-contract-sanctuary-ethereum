...
contract LuffyMarket is Ownable {
    ...
    constructor(){
        ...
        BUY_SALT = keccak256(bytes("List(uint256 nonce,address nftAddress,uint256 tokenID,address from,string selltype,string price,string divid,uint256 amount,bool burn_Method)"));
    ...
    function getSignedResult (
      ...
      return ECDSA.recover(_hash, approvedSignature);
    }
...
function purchaseWithFixedPrice (
    ...
    address to = msg.sender;
       ...
       bytes32 digest = keccak256(
            abi.encodePacked(uint8(0x19), 
            uint8(0x01), DOMAIN_SEPARATOR, keccak256(abi.encode(BUY_SALT, nonces[tokenId]++, nftAddress, tokenId, from, tokenAddr, price, burn_Method)))
        ); 
    ...
    nftInstance.transferFrom(from, to, tokenId);
      ...
      if(burn_Method == true) {           
            ...
            require(token.allowance(to, address(this)) >= price, "Payable is not allowed by buyer");

            uint256 burnPrice = price * 98 / 100;

            token.transferFrom(to, burnAddr, burnPrice); 
            token.transferFrom(to, from, (price - burnPrice));
...
function purchaseAuction(
  ...
  bytes memory signature 
    ...
    require(saleTime > block.timestamp, "Timeout for this sale");
    require(recoveredAddress == from, "invalid signature");
    require(nftInstance.ownerOf(tokenId) == from, "wrong owner");
    require(nftInstance.getApproved(tokenId)==address(this), "Nft Not Approved By the owner");
  ...
  // function burnMethod (bool burn) external {
  //   burn_Method = burn;
  // }
}
