...
contract TokenSwap is Ownable, Pausable {
    ...
    constructor(
        ...
        require(_minAmount > 0, "Swap: Minimum swap amount cannot be zero");
    ...
    function updateSwapRate(uint256 _swapRate) public onlyOwner {
        swapRate = _swapRate;
    }

    function updateMinimumSwapAmount(uint256 _minAmount) public onlyOwner {
        require(_minAmount > 0, "Swap: Minimum swap amount cannot be zero");
        minAmount = _minAmount;
    }

    function updateNumerator(uint256 _numerator) public onlyOwner {
        numerator = _numerator;
    }
    ...
    function sell(uint256 _tokenAmount) public {
        require(_tokenAmount >= minAmount, "Swap: Too low value for Swap");
        require(
    ...
    function tokensIntoUsdc(uint256 _tokenAmount)
        ...
        returns (uint256)
    {
        uint256 calculatedPrice = ((_tokenAmount * tokenPriceInUsd) *
            _getUsdcToUsdPrice()) / (numerator * 1 ether);
    ...
    function usdcIntoTokens(uint256 _usdcAmount) public view returns (uint256) {
        uint256 calculatedPrice = ((_usdcAmount * _getUsdcToUsdPrice()) *
            tokenPriceInUsd) / (numerator * 1 ether);
    ...
    function withdraw(address _contractAddress) public onlyOwner {
        uint256 balance = IERC20(_contractAddress).balanceOf(address(this));
        require(balance > 0, "Zero balance");
        IERC20(_contractAddress).transfer(msg.sender, balance);
    }
}
