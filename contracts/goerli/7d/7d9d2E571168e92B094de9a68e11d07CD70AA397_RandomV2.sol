...
contract RandomV2 is VRFConsumerBaseV2, Ownable, recovery {
  ...
  function _requestRandomWordsAtGasLimit(uint32 numberOfWords, uint speed, uint32 gasLimit) internal returns (bytes32){
    require(speed < keys.length,"Invalid speed");
    bytes32 keyHash = keys[speed];
    return bytes32(COORDINATOR.requestRandomWords(
      keyHash,
  ...
  // from original random
    ...
    function requestRandomNumberWithCallbackAtGasLimit(uint32 gasLimit ) public onlyAuth returns (bytes32) {
       bytes32 requestId = _requestRandomWordsAtGasLimit(1, 0,gasLimit);
       requestIDs.push(requestId);
       callbacks[requestId] = msg.sender;
       emit Request(requestId);
       return requestId;
