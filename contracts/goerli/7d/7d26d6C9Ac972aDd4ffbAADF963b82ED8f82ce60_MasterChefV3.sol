...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard {
    ...
    event UpdateLiquidity(address indexed from, uint256 indexed pid, uint256 indexed tokenId, int128 liquidity);
    ...
    event Burn(address indexed sender, uint256 indexed pid, uint256 indexed tokenId);
    ...
    function add(uint256 _allocPoint, IUniswapV3Pool _v3Pool, ILMPool _LMPool, bool _withUpdate) external onlyOwner {
    ...
    function withdraw(uint256 _tokenId, address _to) external nonReentrant {
        ...
        if (address(pool.LMPool) != address(0)) {
            ...
            pool.LMPool.updatePosition(positionInfo.tickLower, positionInfo.tickUpper, -int128(positionInfo.liquidity));
        }
    ...
    function updateLiquidity(uint256 _tokenId) external nonReentrant {
        UserPositionInfo storage positionInfo = userPositionInfos[_tokenId];
        if (positionInfo.pid == 0) revert InvalidPid();
        harvestOperation(positionInfo, _tokenId, address(0));
        updateLiquidityOperation(positionInfo, _tokenId);
    }

    function updateLiquidityOperation(UserPositionInfo storage positionInfo, uint256 _tokenId) internal {
        (, , , , , int24 tickLower, int24 tickUpper, uint128 liquidity, , , , ) = nonfungiblePositionManager.positions(
            _tokenId
        );
        int128 liquidityDelta = int128(liquidity) - int128(positionInfo.liquidity);
        if (liquidityDelta != 0) {
            positionInfo.liquidity = liquidity;
            PoolInfo memory pool = poolInfo[positionInfo.pid];
            if (address(pool.LMPool) == address(0)) revert NoLMPool();
            pool.LMPool.updatePosition(tickLower, tickUpper, liquidityDelta);
            emit UpdateLiquidity(msg.sender, positionInfo.pid, _tokenId, liquidityDelta);
        }
    }
    ...
    function burn(uint256 _tokenId) external payable {
        ...
        delete userPositionInfos[_tokenId];
        emit Burn(msg.sender, positionInfo.pid, _tokenId);
    }
    ...
    function unpkeepOperation(bool _withUpdate) internal {
        ...
        if (lastestPeriodEndTime > currentTime) {
