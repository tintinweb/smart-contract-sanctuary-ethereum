...
contract AgencyUsdcAMMRouter is Ownable, AccessControl, ERC2771Recipient {
    ...
    function addLiquidity(
        ...
        SafeERC20.safeTransferFrom(agencyToken, _msgSender(), address(pair), amountA);
    ...
    function removeLiquidity(
        ...
        SafeERC20.safeTransferFrom(IERC20(address(pair)), _msgSender(), address(pair), liquidity);
    ...
    function swapExactFraxForTemple(
        ...
        require(allowed[_msgSender()] || openAccessEnabled, "Router isn't open access and caller isn't in the allowed list");
        ...
        // Swap on AMM
        if (amountInAMM > 0) {
            SafeERC20.safeTransferFrom(usdcToken, _msgSender(), address(pair), amountInAMM);
            pair.swap(amountOutAMM, 0, to, new bytes(0));
        }

        // Mint on protocol
        if (amountInProtocol > 0) {
            SafeERC20.safeTransferFrom(usdcToken, _msgSender(), protocolMintEarningsAccount, amountInProtocol);
    ...
    //     SafeERC20.safeTransferFrom(usdcToken, _msgSender(), pair, amountIn);
    ...
    function swapExactTempleForFrax(
        ...
        if (priceBelowIV) {
            ...
            require(amountOut >= amountOutMin, 'AgencyUsdcAMMRouter: INSUFFICIENT_OUTPUT_AMOUNT');
            SafeERC20.safeTransferFrom(agencyToken, _msgSender(), address(pair), amountIn);
    ...
    //     require(amountIn <= amountInMax, 'TempleFraxAMMRouter: EXCESSIVE_INPUT_AMOUNT');
    //     SafeERC20.safeTransferFrom(agencyToken, _msgSender(), pair, amountIn);
...
contract AgencyERC20Token is ERC20, ERC20Burnable, Ownable, AccessControl, ERC2771Recipient{
