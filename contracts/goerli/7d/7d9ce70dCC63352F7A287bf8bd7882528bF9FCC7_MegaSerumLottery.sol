...
contract MegaSerumLottery is VRFV2WrapperConsumerBase, ConfirmedOwner {
  ...
  function lottery() external onlyOwner returns (uint16[] memory) {
    require(ticketsAmount > 0, "Haven't set participants number");
    require(s_requests[lastRequestId].fulfilled, "Haven't got randomness");
    uint256 random = s_requests[lastRequestId].randomWords[0];

    uint16 result = uint16(SafeMath.mod(random, ticketsAmount));
    results.push(result);
  ...
  function claim(uint256 _phase) external {
    ...
    require(msg.sender == winner[_phase], 'Not the winner');
