...
contract ConverterTest is Converter {
    uint256 public currentTimestamp;
    mapping(address => uint256) public customizedEnergy;
    ...
    constructor(
        ...
        uint256 time,
        uint256 finishTime
    ) Converter(owner, lba, _periods, time, finishTime) {}

    function currentTime() public view virtual override returns (uint256) {
        return currentTimestamp > 0 ? currentTimestamp : block.timestamp;
    }
    ...
    function getEnergy(address addr, uint256 periodId) public view virtual override returns (uint256) {
        uint256 energy = customizedEnergy[addr] - getConsumedEnergy(addr);
        if (energy > 0) return energy;
        return super.getEnergy(addr, periodId);
    }

    function setEnergy(address addr, uint256 _energy) external {
        customizedEnergy[addr] = _energy * 10**36;
    }
}
...
contract Staking is IStaking, Util, PermissionControl, Pausable {
    ...
    function stakeFor(
        ...
        uint256 tokenBalance = _token[tokenId].balanceOf(staker);
        if (amount > tokenBalance) revert InvalidInput(INSUFFICIENT_BALANCE);
