...
contract Yachtify is Context, Ownable, IERC20, ERC20Detailed {
  ...
  using SafeMath for uint256;
    ...
    uint256 public BUYcharityFee = 2;
    uint256 public BUYburnFee = 1;
    uint256 public BUYliquidityFee = 2;
    uint256 public BUYtotalFee = BUYliquidityFee.add(BUYcharityFee).add(BUYburnFee);

    uint256 public SELLcharityFee = 2;
    uint256 public SELLburnFee = 1;
    uint256 public SELLliquidityFee = 3;
    uint256 public SELLtotalFee = SELLliquidityFee.add(SELLcharityFee).add(SELLburnFee);

    address payable public charityaddress = payable(0x78282540167f21A17d80248721261DBd0dD5e8da);
    ...
    constructor () ERC20Detailed("Yachtify", "YCHT", 18) {
      _owner = msg.sender ;
    ...
    function setcharityaddress(address payable wallet) external onlyOwner
    {
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        if (
            ...
            uint256 walletTokens = contractTokenBalance.mul(SELLcharityFee).div(SELLtotalFee);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 marketingShare = newBalance.mul(SELLcharityFee).div((SELLcharityFee));
            //uint256 rewardShare = newBalance.sub(marketingShare);
            payable(charityaddress).transfer(marketingShare);
    ...
    function setSellFee(uint256 _onSellEthBurndFee, uint256 _onSellliuidityFee, uint256 _onSellcharityFee) public onlyOwner {

        SELLcharityFee = _onSellcharityFee;
        SELLburnFee = _onSellEthBurndFee;
        SELLliquidityFee = _onSellliuidityFee;
        uint256  onSelltotalFees;
        onSelltotalFees = SELLcharityFee.add(SELLburnFee).add(SELLliquidityFee);
        require(onSelltotalFees <= 15, "Sell Fee should be 15% or less");
    }

    function setBuyFee(uint256 _onBuyBurndFee, uint256 _onBuyliuidityFee, uint256 _onBuycharityFee) public onlyOwner {

        BUYcharityFee = _onBuycharityFee;
        BUYburnFee = _onBuyBurndFee;
        BUYliquidityFee = _onBuyliuidityFee;
        uint256  onBuytotalFees;
        onBuytotalFees = BUYcharityFee.add(BUYburnFee).add(BUYliquidityFee);
