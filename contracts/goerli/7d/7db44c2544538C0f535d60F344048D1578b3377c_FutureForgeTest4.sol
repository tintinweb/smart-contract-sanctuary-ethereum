...
contract FutureForgeTest4 is AccessControl, ERC1155Holder {
  ...
  uint256 tier1Supply = 0;
  uint256 tier2Supply = 0;
  uint256 tier3Supply = 0;

  uint256 tier1MapLength = 4;
  uint256 tier2MapLength = 6;
  uint256 tier3MapLength = 9;

  uint256 tier1startId = 2;
  uint256 tier2startId = 6;
  uint256 tier3startId = 12;

  uint256 constant TIER_1_MAX_SUPPLY = 47;
  uint256 constant TIER_2_MAX_SUPPLY = 58;
  uint256 constant TIER_3_MAX_SUPPLY = 65;
  ...
  function _initializeArtifacts() internal {
    // Tier 1 Artifacts
    _tier1Artifacts[2] = Artifact(0, 20, 1, 2); // Hover Drone Assistant of Skill
    _tier1Artifacts[3] = Artifact(0, 12, 1, 3); // Power Arm of Skill
    _tier1Artifacts[4] = Artifact(0, 5, 1, 4); // Robot Helper of Skill
    _tier1Artifacts[5] = Artifact(0, 10, 1, 5); // Elite Robotic Arm of Skill
    ...
    // Tier 3 Artifacts
    _tier3Artifacts[12] = Artifact(0, 20, 3, 12); // Mutant Antilope
    _tier3Artifacts[13] = Artifact(0, 12, 3, 13); // Mutant Jelly Processor
    _tier3Artifacts[14] = Artifact(0, 12, 3, 14); // Wind Turbine
    _tier3Artifacts[15] = Artifact(0, 10, 3, 15); // Solar Array
    _tier3Artifacts[16] = Artifact(0, 15, 3, 16); // Smugglers Crate
    _tier3Artifacts[17] = Artifact(0, 5, 3, 17); // Future Horizon eVTOL mk1
    _tier3Artifacts[18] = Artifact(0, 5, 3, 18); // Interplanetary Survey Rig
    _tier3Artifacts[19] = Artifact(0, 5, 3, 19); // Space Mining Refinery Rig
    _tier3Artifacts[20] = Artifact(0, 5, 3, 20); // Astro Chip Upgrade
  }
  ...
  function forge(uint256 _dragonQuantity) public returns (uint256) {
    ...
    if (_dragonQuantity > 8) {
      tier = 1;
    } else if (_dragonQuantity > 5) {
      tier = 2;
    }
