...
/*
  ___ _           ___              _              
 | _ ) |_  _ ___ / __|_ _ ___ __ _| |_ ___ _ _ ___
 | _ \ | || / -_) (__| '_/ -_) _` |  _/ _ \ '_(_-<
 |___/_|\_,_\___|\___|_| \___\__,_|\__\___/_| /__/                             
                                                                
*/

/// @title BlueCreators NFT contract.
contract BlueCreators is Ownable, PaymentSplitter {
    ...
    // Events
    event SubscriptionRegister(address sender, Subscription subscription_registered);
    event CreatorRegister(Creator creator_registered);
    ...
    * @param discord_user_id The discord user's id covered by the subscription
    ...
    **/
    function individualSubscribe(string calldata discord_user_id, Version version, uint8 month, string calldata code) external payable {
        ...
        uint date_of_purchase = _subscribers[discord_user_id].date_of_purchase;
        uint8 month_bought = _subscribers[discord_user_id].month;
        ...
        {
            if(uint(_subscribers[discord_user_id].version) != uint(version)) revert AlreadyActivePlanError();
            _subscribers[discord_user_id].month += month;
        }
        else 
        {
            _subscribers[discord_user_id].discord_id = discord_user_id;
            _subscribers[discord_user_id].date_of_purchase = current_time;
            _subscribers[discord_user_id].month = month;
            _subscribers[discord_user_id].version = version;
            _subscribers[discord_user_id].cashback_code = code;
        }
        emit SubscriptionRegister(msg.sender, _subscribers[discord_user_id]);
    ...
    * @param discord_guild_id The discord guild's id covered by the subscription
    * @param version The version the community will subscribe too
    * @param month The amount of month for the subscription plan
    **/
    function communitySubscribe(string calldata discord_guild_id, Version version, uint8 month) external payable {
        if(tx.origin != msg.sender) revert CallerIsContractError();
        ...
        uint date_of_purchase = _communities[discord_guild_id].date_of_purchase;
        uint month_bought = _communities[discord_guild_id].month;
        uint current_time = block.timestamp;
        if(date_of_purchase + (month_bought * 30 days) > current_time) // Previous subscription is still valid
        {
            if(uint(_communities[discord_guild_id].version) != uint(version)) revert AlreadyActivePlanError();
            _communities[discord_guild_id].month += month;
        }
        else 
        {
            _communities[discord_guild_id].discord_id = discord_guild_id;
            _communities[discord_guild_id].date_of_purchase = current_time;
            _communities[discord_guild_id].month = month;
            _communities[discord_guild_id].version = version;
        }
        emit SubscriptionRegister(msg.sender, _communities[discord_guild_id]);
    }

    /**
    * @notice Insert a new creator
    *
    * @param creatorAddress The address of the creator that will be added
    * @param creatorReductionPercentage The reduction percentage that will be applied for this creator
    * @param creatorCashbackPercentage The cashback percentage that will be applied for this creator
    * @param creatorCode The creator code that will need to be used to retrieve the percentage
    */
    function addNewCreator(address creatorAddress, uint creatorReductionPercentage, uint creatorCashbackPercentage, string calldata creatorCode) external {
        _enforceAdmin();
        ...
        _creators[creatorCode].creator = creatorAddress;
    ...
    *
    * @param creator_code The creator code to search for
    *
    * @return creator The creator associated to the code
    **/
    function getCreator(string memory creator_code) public view returns (Creator memory creator) {
        return _creators[creator_code];
    }

    /**
    * @notice Return a subscriber associated to a discord user id
    *    
    * @param discord_user_id The discord user id to search for
    *
    * @return subscriber The subscriber associated to the discord user id
    **/
    function getSubscriber(string calldata discord_user_id) public view returns (Subscription memory subscriber) {
        return _subscribers[discord_user_id];
    }

    /**
    * @notice Return a community associated to a discord guild id
    *    
    * @param discord_guild_id The discord guild id to search for
    *
    * @return community The community associated to the discord guild id
    **/
    function getCommunity(string calldata discord_guild_id) public view returns (Subscription memory community) {
        return _communities[discord_guild_id];
    }
