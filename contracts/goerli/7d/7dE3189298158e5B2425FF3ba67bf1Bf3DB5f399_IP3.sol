...
contract IP3 {
    ...
    function purchaseByDuration(
       ...
       uint256 latestStartPriceOnRecord = authroizeRecordMap[hashedNft]
        ...
        if (latestStartPriceOnRecord !=0) {
            ...
            latestStartPrice = getCurrentStartingPrice(hashedNft);
        } else { 
            // new NFT to be licenced
            // get the start price from the input
            latestStartPrice = authorizedNFTSubmit.latestStartPrice;
            // input the list start time and list end time
        ...
        // get the latest timestamp, if no record on-chain default is 0
        uint256 latestTimeStamp = authroizeRecordMap[hashedNft].authorizedNFT.lastActive;

        //discountPerTime, example 0.1 ERC20 in 6 decimals / 3600 secs
        //TODO: could be NFT specific feature 
        uint256 discountPerTime = 10**5;
        
        // if new NFT to be licenced, could be 0, otherwise get the current transactional counts
        uint256 currentTxNum = authroizeRecordMap[hashedNft].authorizeRecord.totalAuthorizedCount;
        ...
        authroizeRecordMap[hashedNft].authorizedNFT.latestStartPrice = latestStartPrice;
        authroizeRecordMap[hashedNft].authorizedNFT.lastActive = block.timestamp;

        AuthorizeCertificate memory newAuthorizeCertificate =  AuthorizeCertificate(authroizeRecordMap[hashedNft].authorizedNFT, newTerm, renterAddress, termedPrice, singature);
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    authorizedNFT.latestStartPrice,
        ...
        returns (uint256)
    {
        ...
        return
            SmartPriceEngine.updateStartingPrice(
                currentTxNum,
                latestTimeStamp,
                latestStartPrice,
                discountPerTime
...
library SmartPriceEngine {
    ...
    function computeAuthroizedPriceByCount(
        ...
        uint256 currentStartPrice = updateStartingPrice(currentTxNum, latestTimeStamp, latestStartPrice, discountPerTime);
    ...
    function computeAuthroizedPriceByDuration(
        ...
        uint256 currentStartPrice = updateStartingPrice(currentTxNum, latestTimeStamp, latestStartPrice, discountPerTime);
        // incrementByDuration: for example 0.1 erc20 / 3600 sec
        uint256 authorizedPrice = currentStartPrice + duration * incrementByDuration;
    ...
    function updateStartingPrice(
        ...
        uint256 currentLatestStartPrice = latestStartPrice -
            ((latestTimeStamp - block.timestamp) * discountPerTime) /
            TIME_INTERVAL + INCREMENT_BY_TX*currentTxNum;

        if (latestStartPrice < MIN_THRESHOLD_PRICE) {
            return MIN_THRESHOLD_PRICE;
        }
        
        return currentLatestStartPrice;
    }
}
