...
contract DroolRewards is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    uint public DroolPerSacrifice = 69 ether;
    
    // Staking
    uint256 public immutable DROOL_PER_SECOND_PER_APT = 798611111000000; // 69 drool per day
    ...
    // Burning Functions
    function sacrifice(uint tokenId) external {
        require(apmfer.ownerOf(tokenId) == msg.sender, "You don't own this mfer");
        apmfer.burn(tokenId);
        drool.safeTransfer(msg.sender, DroolPerSacrifice);
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return (block.timestamp - claimed) * DROOL_PER_SECOND_PER_APT * balances[account];
    }
    ...
    modifier updateReward(address account) {
        uint total = (block.timestamp - lastClaim[account]) * DROOL_PER_SECOND_PER_APT * balances[account];
