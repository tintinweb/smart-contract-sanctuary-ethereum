    ...
    ReentrancyGuardUpgradeable
{
    ...
    * @param - Account
    ...
    function whiteListedUser(address account) public onlyOwner {
    ...
    * @param count - number of token user mint
     */
    function tokenMint(uint256 count)
        public
        payable
        userCount(count)
    {
        ...
        for (uint256 i = 1; i <= count; i++) {
            if (countOfUser[msg.sender] == 1) {
                safeMint(msg.sender);
            } else if (countOfUser[msg.sender] == 2 && whiteList[msg.sender]) {
                require(msg.value == whiteListedTokenFee, "Amount should be 0.0035");
                safeMint(msg.sender);
            } else if (countOfUser[msg.sender] == 2 && !whiteList[msg.sender]) {
                require(msg.value == tokenFee, "Amount should be 0.0036");
                safeMint(msg.sender);
            }
        }
        _totalMint += count;
    ...
    * @param count
    ...
    modifier userCount(uint256 count) {
        ...
        require(
            countOfUser[msg.sender] <= 2,
            "User count is less than 3"
        );
        _;
    }
    ...
    * @param updateWhiteListedTokenFee, updateTokenFee
     */
    function updateFee(uint256 updateWhiteListedTokenFee, uint256 updateTokenFee) external onlyOwner{
        whiteListedTokenFee = updateWhiteListedTokenFee;
