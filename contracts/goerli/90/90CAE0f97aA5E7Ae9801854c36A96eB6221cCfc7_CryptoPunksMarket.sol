...
contract CryptoPunksMarket {
    ...
    // Function added for dev punks to pre-assign all punks to dev punks project
    // Dev punks can be requested for use in punk related projects
    function assignDevPunksToOwner() {
        if (msg.sender != owner) throw;
        for (uint256 i = 0; i < totalSupply; i++) {
            punkIndexToAddress[i] = owner;
        }
