...
contract CelestialCastleMainland is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC721ReceiverUpgradeable, ICelestialCastle {
	...
	function travel(
		...
		_travel(freakIds, celestialIds, fbxAmount, msg.sender, false);
	}
	...
	function travelFromHunting(
		...
		_travel(freakIds, celestialIds, fbxAmount, owner, false);
	}
	...
	function travelFromMigration(
		...
		require(isTravelEnabled, "CelestialCastle: travel is disabled");
		require(freakIds.length + celestialIds.length <= assetLimit, "CelestialCastle: Too many assets");
		require(msg.sender == address(freaksNGuilds), "CelestialCastle: caller is not FnG");
		_travel(freakIds, celestialIds, fbxAmount, owner, true);
	}
	...
	function _travel(
		...
		if (freakIds.length > 0) {
			calls[0] = initiateFreaksTravel(freakIds, !isFromMigration);
			callsIndex++;
		}

		if (celestialIds.length > 0) {
			calls[callsIndex] = initiateCelestialsTravel(celestialIds, !isFromMigration);
			callsIndex++;
		}

		if (fbxAmount > 0) {
			if(isFromMigration){
				calls[callsIndex] = initiateFreaksBucksTravelFromMigration(fbxAmount, owner);
			} else {
	...
	function travelFreaks(uint256[] calldata freakIds) external {
		...
		calls[0] = initiateFreaksTravel(freakIds, true);
	...
	function travelCelestials(uint256[] calldata celestialIds) external {
		require(isTravelEnabled, "CelestialCastle: travel is disabled");
		bytes[] memory calls = new bytes[](1);
		calls[0] = initiateCelestialsTravel(celestialIds, true);
		portalMainland.sendMessage(abi.encode(calls));
	}
	...
	function initiateFreaksTravel(uint256[] calldata freakIds, bool isTransferRequired) internal returns (bytes memory) {
		...
		for (uint256 i = 0; i < freakIds.length; i++) {
			...
			if(isTransferRequired){
				...
				require(freaksNGuilds.ownerOf(freakIds[i]) == address(this), "CelestialCastle: freaks must be transfered to contract");
			}
		}
		return abi.encode(CelestialPortalMessages.RETRIEVE_FREAKS, abi.encode(freakIds, freaks));
	}

	function initiateCelestialsTravel(uint256[] calldata celestialIds, bool isTransferRequired) internal returns (bytes memory) {
		...
		for (uint256 i = 0; i < celestialIds.length; i++) {
			...
			if(isTransferRequired){
				...
				require(freaksNGuilds.ownerOf(celestialIds[i]) == address(this), "CelestialCastle: freaks must be transfered to contract");
			}
		}
	...
	function initiateFreaksBucksTravelFromMigration(uint256 fbxAmount, address owner) internal returns (bytes memory) {
		freaksBucks.burn(owner, fbxAmount);
		return abi.encode(CelestialPortalMessages.RETRIEVE_FBX, abi.encode(fbxAmount));
	}
