...
function claimTokens() public nonReentrant icoNotActive returns (bool) {
    ...
    if (claimedTokens[msg.sender] == 0) {
        tokensToClaim = totalTokensPurchasedByUser.mul(50).div(100); // first time claiming, allow to claim 50%
    } else {
        tokensToClaim = totalTokensPurchasedByUser.mul(10).div(100); // subsequent claims, allow to claim 10%
    }
