...
contract ERC20StakingFactory is AccessControl {
  ...
  address private dexToken;
  address private dexVault;
  uint256 private dexStakeFee;
  uint256 private dexUnStakeFee;
 ...
 function addVault(
  ...
  function listVaults() external view returns ( vaultInfo[] memory) {
            return VaultInfo;   
  }

  function addRewards(uint256 index,uint256 _amount) external{
    require(index < VaultInfo.length,"Pool does not exists");
    ITokenStaking tokenContract= ITokenStaking(address(VaultInfo[index].stakingContract));
    tokenContract.addRewards(_amount);
  }

 function updateAPY(uint256 index,uint256 _apy) external onlyRole(OPERATOR_ROLE){
    require(index < VaultInfo.length,"Pool does not exists");
    ITokenStaking tokenContract= ITokenStaking(address(VaultInfo[index].stakingContract));
    tokenContract.updateAPY(_apy);
 }

 function updateClosingIn(uint256 index,uint256 _closingIn) external onlyRole(OPERATOR_ROLE){
    require(index < VaultInfo.length,"Pool does not exists");
    ITokenStaking tokenContract= ITokenStaking(address(VaultInfo[index].stakingContract));
    tokenContract.updateClosingIn(_closingIn);
 }

 function updateLockPeriod(uint256 index,uint256 _lockPeriod) external onlyRole(OPERATOR_ROLE){
    require(index < VaultInfo.length,"Pool does not exists");
    ITokenStaking tokenContract= ITokenStaking(address(VaultInfo[index].stakingContract));
    tokenContract.updateLockPeriod(_lockPeriod);
 }

 function updateMinStakeRequired(uint256 index,uint256 _minStakeRequired) external onlyRole(OPERATOR_ROLE){
    require(index < VaultInfo.length,"Pool does not exists");
    ITokenStaking tokenContract= ITokenStaking(address(VaultInfo[index].stakingContract));
    tokenContract.updateMinStakeRequired(_minStakeRequired);
 }
 ...
 function removeVault(uint256 index) external onlyRole(DEFAULT_ADMIN_ROLE){
  ...
  function updateDexFeeConfiguration(address _dexToken,address _dexVault, uint256 _dexStakeFee, uint256 _dexUnStakeFee)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
      dexToken=_dexToken;
      dexVault=_dexVault;
      dexStakeFee=_dexStakeFee;
      dexUnStakeFee=_dexUnStakeFee;

  }

   function dexFeeConfiguration(address stakedToken)
    external
    view
    returns (address _dexToken, address _dexVault,uint256 _dexStakeFee,uint256 _dexUnStakeFee)
  {
    if(stakedToken==dexToken){
        _dexToken = dexToken;
        _dexVault = dexVault;
        _dexStakeFee = 0;
        _dexUnStakeFee = 0;
        ...
        _dexToken = dexToken;
        _dexVault = dexVault;
        _dexStakeFee = dexStakeFee;
        _dexUnStakeFee = dexUnStakeFee;
    }
...
interface ITokenStaking {
 ...
 function updateClosingIn(uint256 _closingIn) external;
 function updateLockPeriod(uint256 _lockPeriod) external;
 function updateMinStakeRequired(uint256 _minStakeRequired) external;
 ...
 */
contract ERC20Staking is AccessControl, Pausable {
  struct Stake {
  ...
  constructor(address _operator,address _factory,address _stakedToken,address _rewardToken, uint256 _lockPeriod, uint256 _minStakeRequired, uint256 _apy,uint256 _closingIn) {
   ...
   */
  function updateAPY(uint256 _apy) external onlyRole(OPERATOR_ROLE) {
    PoolInfo.apy = _apy;
  }
   ...
   * @param _apy APY value.
   */
  function updateClosingIn(uint256 _closingIn) external onlyRole(OPERATOR_ROLE) {
   ...
   */
  function updateMinStakeRequired(uint256 _minStakeRequired) external onlyRole(OPERATOR_ROLE) {
   ...
   */
  function updateLockPeriod(uint256 _lockPeriod) external onlyRole(OPERATOR_ROLE) {
...
function stake(uint256 _amount) external whenNotPaused {
    ...
    (,address dexVault,uint256 dexStakeFee,)= factory.dexFeeConfiguration(PoolInfo.stakedToken);
    if(dexStakeFee>0){
       uint256 fees=((_amount *dexStakeFee) /10000);
       _amount=_amount-fees;
       // transfer to stakedToken fees vault
       IERC20(PoolInfo.stakedToken).transfer(dexVault, fees);
    }
  ...
  function withdrawAvailableReward() public whenNotPaused {
    ...
    require(availableRewards >= rewards, "Withdraw is paused");

    (,address dexVault,,uint256 dexUnStakeFee)= factory.dexFeeConfiguration(PoolInfo.stakedToken);
    if(dexUnStakeFee>0){
       uint256 fees=((rewards *dexUnStakeFee) /10000);
       rewards=rewards-fees;
       // transfer to rewardToken fees vault
       IERC20(PoolInfo.rewardToken).transfer(dexVault, fees);
  
    }
    // transfer available rewardToken to sender
    IERC20(PoolInfo.rewardToken).transfer(msg.sender, rewards);
    
    stakeOf[msg.sender].timeAt = block.timestamp;
     availableRewards =availableRewards-rewards;
...
function emergencyUnstake() external {
    ...
    uint256 availableTokens=minStakeRequiredOf[msg.sender];
     ...
     if(dexUnStakeFee>0){
       uint256 fees=((availableTokens *dexUnStakeFee) /10000);
       availableTokens=availableTokens-fees;
       // transfer to stakedTokens vault
       IERC20(PoolInfo.stakedToken).transfer(dexVault, fees);
   ...
   */
  function pause(bool _isPause) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
interface IFactory {
 ...
 function dexFeeConfiguration(address stakedToken) external view returns (address dexToken, address dexVault,uint256 dexStakeFee,uint256 dexUnStakeFee);
