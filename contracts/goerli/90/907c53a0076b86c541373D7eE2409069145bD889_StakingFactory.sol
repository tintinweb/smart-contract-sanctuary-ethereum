...
contract StakingFactory is Ownable, IStakingFactory {
    ...
    /**
     * @notice Weight is scaled by 10e18
     */
    ...
    function _createUfoAndLpPools(
        ...
        _createPool(_beacon, ufoPoolIndex, _ufoToken, lockInBlocks, _admin, 612002322, weight);
        _createPool(_beacon, lpPoolIndex, _lpToken, lockInBlocks, _admin, 10e26, weight);
    }
    ...
    function approxNumberOfBlockPerDays(uint256 noOfdays) public pure returns (uint256) {
        return (noOfdays * (1 days) * (86400) * (10)) / (131);
    }
    ...
    function updateTVL(uint256 tvl) external override onlyPool {
        ...
        if (pool.ufoPool) {
            totalUfoWeight = totalUfoWeight - ((pool.tvl * (pool.weight)) / (WEIGHT_SCALE));
            totalUfoWeight = totalUfoWeight + ((tvl * (pool.weight)) / (WEIGHT_SCALE));
        } else {
            totalLpWeight = totalLpWeight - ((pool.tvl * (pool.weight)) / (WEIGHT_SCALE));
            totalLpWeight = totalLpWeight + ((tvl * (pool.weight)) / (WEIGHT_SCALE));
        }
        pool.tvl = tvl;
     ...
     * @param newValue new plasma reward per block per token
     */
    function changeUfoPoolPlasmaPerBlockPerToken(uint256 newValue) external onlyOwner {
        for (uint256 index = 0; index < totalPools; index += 2) {
            address pool = poolNumberToPoolAddress[index];
            IStaking(pool).updatePlasmaPerBlockPerToken(newValue);
        }
    }
     ...
     * @param newValue new plasma reward per block per token
     */
    function changeLpPoolPlasmaPerBlockPerToken(uint256 newValue) external onlyOwner {
        for (uint256 index = 1; index < totalPools; index += 2) {
            address pool = poolNumberToPoolAddress[index];
            IStaking(pool).updatePlasmaPerBlockPerToken(newValue);
        }
    }
