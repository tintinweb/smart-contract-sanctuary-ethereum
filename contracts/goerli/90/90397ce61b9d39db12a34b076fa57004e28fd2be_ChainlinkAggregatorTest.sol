...
contract ChainlinkAggregatorTest is ChainlinkClient, Ownable {
    using Chainlink for Chainlink.Request;
    ...
    constructor(
        ...
        linkToken = IERC20(_chainlinkToken);
     ...
     * @notice Initiatiate a price request via chainlink for eth pair. 
       @param _collectionAddres The NFT contract of the collection the pair trades
     */
    function requestFloorPrice(address _collectionAddres) external onlyOwner returns (bytes32) {
        ...
        string memory requestURL = string(
            abi.encodePacked(
                ...
                Strings.toHexString(address(_collectionAddres))
            )
        );
    ...
    function withdrawLink(address _receiver) external onlyOwner {
        linkToken.transfer(_receiver, linkToken.balanceOf(address(this)));
    }
}
