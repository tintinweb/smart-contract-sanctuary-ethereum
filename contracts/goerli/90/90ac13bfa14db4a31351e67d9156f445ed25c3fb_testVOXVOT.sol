...
//tested contract
contract testVOXVOT is ERC721Enumerable, Ownable, Pausable{
  using Strings for uint256;
  ...
  uint256 public publicMaxMintAmount = 5;
  ...
  mapping (address => bool)[4] public WLMinteList;
  uint256 public wave;
  ...
  function publicMint(uint256 amount) public payable whenNotPaused {
    ...
    require(publicMintCount[msg.sender] + amount <= publicMaxMintAmount, "The maximum number of mints possible has been exceeded");
    ...
    for (uint256 i = 1; i <= amount; i++) {
      ownerdatainterface.setChildPurpose(mintFromAddress,msg.sender,supply + i, msg.sender);
  ...
  function whitelistMint(bytes32[] calldata proof) public payable whenNotPaused {
    ...
    require(!WLMinteList[wave][msg.sender], "Minted.");
    ...
    //fxportal
    ownerdatainterface.setChildPurpose(mintFromAddress,msg.sender,supply + 1, msg.sender);

    _safeMint(msg.sender, supply + 1);
    WLMinteList[wave][msg.sender] = true;
  }
  ...
  function transferFrom(
      ...
      override
      {
          require(_isApprovedOrOwner(_msgSender(), tokenId),"EERC721: transfer caller is not owner nor approved");
  ...
  function setPublicMaxMintAmount (uint256 _newAmount) public onlyOwner {
      publicMaxMintAmount = _newAmount;
  }
