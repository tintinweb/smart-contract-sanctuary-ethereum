...
abstract contract AbstractSilicaV2_1 is ERC20, Initializable, ISilicaV2_1, SilicaV2_1Storage {
    ...
    event BuyerCollectPayout(uint256 rewardTokenPayout, uint256 paymentTokenPayout);
    ...
    function initialize(InitializeData calldata initializeData) external override initializer {
        ...
        //@TODO: add a check on the size of unitPrice and resourceAmount so that the reservedPrice is > 0;
    ...
    /// @notice Set the reword token address, payment token address, oracle Registery address and
    ...
    /// @dev last due day should always after first due day
    ...
    function defaultContract(
        ...
        /// @review Does reward excess need to be set now?
    ...
    function isFinished() public view override returns (bool) {
        ...
        if (numDays == lastDueDayMem + 1 - firstDueDay) {
            return true;
        }
        return false;
    }
    ...
    function finishContract(
        ...
        // Make sure we're not wasting writes
        rewardDelivered = _totalRewardDelivered;
        resourceAmount = totalSupply();
    ...
    function getDaysAndRewardFulfilled(
        ...
        if (_lastDayContractOwesReward < firstDueDay) {
            return (0, 0); //@ATTN: include collateral
        }

        uint256 totalDue;

        uint256 numDaysOwed = _lastDayContractOwesReward + 1 - firstDueDay;
        ...
        return (numDaysOwed, totalDue + getCollateralLocked(_lastDayContractOwesReward));
    }
    ...
    function getRewardDeliveredSoFar() public view override returns (uint256) {
    ...
    /// @notice Function that released collateral
    ...
    /// @notice Function that calculate the dates collateral gets partial release
    function getCollateralUnlockDays(uint256 _firstDueDay) internal view returns (uint256, uint256) {
        uint256 numDeposits = lastDueDay + 1 - _firstDueDay;

        uint256 initCollateralReleaseDay = numDeposits % 4 > 0 ? _firstDueDay + 1 + (numDeposits / 4) : _firstDueDay + (numDeposits / 4);
        uint256 finalCollateralReleaseDay = numDeposits % 2 > 0 ? _firstDueDay + 1 + (numDeposits / 2) : _firstDueDay + (numDeposits / 2);

        if (numDeposits == 2) {
            finalCollateralReleaseDay += 1;
        }

        return (initCollateralReleaseDay, finalCollateralReleaseDay);
    }

    /// @notice Function returns the rewards amount silica needs deliver for next Oracle update
    function getRewardDueNextOracleUpdate() external view override returns (uint256 rewardDueNextOracleUpdate) {
        uint256 lastIndexedDay = getLastIndexedDay();
        uint256 currentLastDay = lastIndexedDay < lastDueDay + 1 ? lastIndexedDay : lastDueDay; /* CHECK + 1 */
        uint256 totalDue;

        for (uint256 day = firstDueDay; day <= currentLastDay; day++) {
            totalDue = totalDue + getRewardDueOnDay(day);
        }

        uint256 rewardBalance = IERC20(rewardToken).balanceOf(address(this)) - getCollateralLocked(currentLastDay);
        if (rewardBalance < totalDue) {
            return (totalDue - rewardBalance);
        } else {
            return (0);
        }
    }
    ...
    function getMintAmount(
        ...
        uint256 mintAmount = (consensusResource * purchaseAmount) / _reservedPrice;
        return mintAmount;
    }
    ...
    function _transferBuyerPayoutOnFinish(address buyerAddress, uint256 buyerBalance) internal returns (uint256 rewardPayout) {
        ...
        emit BuyerCollectPayout(rewardPayout, 0);
        ...
        returns (uint256 rewardPayout, uint256 paymentPayout)
    {
        ...
        emit BuyerCollectPayout(rewardPayout, paymentPayout);
    ...
    /// @notice Gets the storage status
    //@review: this function is needed for storage management with foundry, could be removed for production
    function getStorageStatus() external view override returns (SilicaV2_1Types.Status) {
        return (status);
    }
...
contract SilicaV2_1 is AbstractSilicaV2_1 {
    ...
    function getCommodityType() external view override returns (uint8) {
...
interface ISilicaV2_1 {
    ...
    /// @notice Returns the commodity type the seller is selling with this contract
    /// @return The commodity type the seller is selling with this contract
    function getCommodityType() external view returns (uint8);
