...
contract GenesisMint is Ownable, Merkle {
	...
	function mintPass(uint256 _index, bool _readTermAndServices, bytes32[] calldata _proof) external payable {
		if (!whitelistMint) revert("!started");
		if (msg.value != MINT_PRICE) revert("!price");
		if (!_readTermAndServices) revert("!terms");
		uint256 current = currentMint;
		if (current > MAX_MINT) revert("max"); 
		
		currentMint = current + 1;
		_claim(_index, msg.sender, 1, _proof);
	...
	function mintPass(bool _readTermAndServices, uint256 _amount) external payable {
		if (!publicMint) revert("!started");
		if (_amount > MAX_PER_TX) revert("!per");
		if (msg.value != MINT_PRICE * _amount) revert ("!price");
		if (!_readTermAndServices) revert("!terms");
		uint256 current = currentMint;
		if (current + _amount - 1 > MAX_MINT) revert("max");

		currentMint = current + _amount;
		// TODO: checkout erc721a
		for (uint256 i = 0; i < _amount; i++)
			GenesisPass(genesisPass).mint(msg.sender, current + 1);
	}
