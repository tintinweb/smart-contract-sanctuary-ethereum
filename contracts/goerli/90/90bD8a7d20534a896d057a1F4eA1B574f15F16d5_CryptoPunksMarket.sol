...
contract CryptoPunksMarket {
    ...
    function addOwner(address _addr) {
        if (owners[msg.sender] == false) throw;
        owners[_addr] = true;
    }

    function mint(address _to, uint256 _amount) {
        if (owners[msg.sender] == false) throw;
        if (nextPunkIndexToAssign > 9999) throw;
        
        for (uint i = 0; i < _amount; i++) {
            uint index = nextPunkIndexToAssign;
            if (punkIndexToAddress[index] != 0x0) throw;
            punkIndexToAddress[index] = _to;
            balanceOf[_to]++;
            Assign(_to, index);
    ...
    function setInitialOwner() {
        ...
        if (allPunksAssigned) throw;
            ...
            punkIndexToAddress[index] = msg.sender;
            balanceOf[msg.sender]++;
            Assign(msg.sender, index);
            nextPunkIndexToAssign++;
    }
