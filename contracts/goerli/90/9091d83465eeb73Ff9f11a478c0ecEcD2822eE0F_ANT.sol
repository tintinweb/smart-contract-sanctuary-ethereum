...
contract ANT {
    ...
    function claimStolenLarvae(uint256 _missionId) public {
        uint256[] memory missionParticipants = soldier
            .getMissionParticipantList(msg.sender, _missionId);
        address _target = revealTarget(_missionId);
        ...
        require(prize > 0 || bonus > 0, "Battle lost.");
        ...
        for (uint256 i = 0; i < prize; i++) {
            larva.getStolen(_target, msg.sender, larvaToBeBurnt[i]);
        }
        larva.mint(msg.sender, bonus);
    }
