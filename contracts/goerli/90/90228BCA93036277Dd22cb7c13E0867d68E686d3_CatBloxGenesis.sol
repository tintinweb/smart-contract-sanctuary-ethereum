...
contract CatBloxGenesis is ERC721, IERC2981, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter private tokenCounter;

    string private baseURI = "https://us-central1-catblox-1f4e5.cloudfunctions.net/api/prereveal";
    ...
    modifier milkListActive() {
        require(isMilkListActive, "Milk list minting is not active");
        _;
    }
    
    modifier reserveListActive() {
        require(isReserveListActive, "Reserve list minting is not active");
        _;
    }
    ...
    modifier maxCatsPerWallet(uint256 numberOfTokens) {
        require(
            balanceOf(msg.sender) + numberOfTokens <= MAX_CATS_PER_WALLET,
            "PM: Two cats max per wallet"
        );
        _;
    }
    ...
    constructor(uint256 _maxCats) ERC721("CatBlox Genesis", "CATBLOXGENESIS") {
        maxCats = _maxCats;
    }
    ...
    function publicMint(uint256 numberOfTokens)
        ...
        canMintCats(numberOfTokens)
        maxCatsPerWallet(numberOfTokens)
    {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, nextTokenId());
        }
    ...
    function setVerificationHash(string memory _hash) external onlyOwner {
        verificationHash = _hash;
    }

    // Set prices 

    function setPublicSalePrice(uint256 p) external onlyOwner {
        publicSalePrice = p;
    }

    function setMilkListPrice(uint256 p) external onlyOwner {
        milkListSalePrice = p;
    }

    function setReserveListPrice(uint256 p) external onlyOwner {
        reserveListSalePrice = p;
    }
    ...
    function setMilkListMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        milkListMerkleRoot = merkleRoot;
    }
