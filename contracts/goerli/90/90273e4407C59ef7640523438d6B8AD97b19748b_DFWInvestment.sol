...
contract DFWInvestment {
  ...
  function setupForTest(
    ...
    uint256 timeRate)
  public onlyTest onlyAdmin onlyBefore(startTime) {
    ...
    for (uint8 i = 0; i < stageCnt; i++) {
      ...
      addStage(rate, start, end, voteStart, voteEnd,
        secondVoteStart, secondVoteEnd);
    }
  ...
  function reset() public onlyTest onlyAdmin {
    _curStageId = -1;
    delete investTotal;
  ...
  function curTime() public view returns(uint256) {
  ...
  function nextStage(bool passed) private {
    stages[curStageId()].passed = passed;

    emit NextStage(passed);
