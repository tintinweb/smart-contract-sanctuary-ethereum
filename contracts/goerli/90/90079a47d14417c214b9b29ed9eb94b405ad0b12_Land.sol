...
contract Land is ERC721A, Ownable {
    ...
    uint256 public constant MAX_PRESALE_LANDS = 1000;
    uint256 public constant PRESALE_MINT_PRICE = 0.000001 ether;

    uint256 public allowListR1StartTime;
    uint256 public allowListR2StartTime;
    uint256 public totalAllowListMinted;
    mapping(address => uint256) public allowListMinted;
    uint256 public constant MAX_ALLOW_LIST_LANDS = 4500;
    uint256 public constant ALLOW_LIST_MINT_PRICE = 0.000001 ether;

    uint256 public specialR1StartTime;
    uint256 public specialR2StartTime;
    uint256 public totalSpecialMinted;
    mapping(address => uint256) public specialMinted;
    uint256 public startSpecialTokenId;
    uint256 public endSpecialTokenId;
    uint256 public specialLandsReleaseTime;
    uint256 public constant MAX_SPECIAL_LANDS = 3000;
    uint256 public constant SPECIAL_MINT_PRICE = 0.000001 ether;

    uint256 public claimableStartTime;
    uint256 public totalLandsClaimed;
    mapping(address => uint256) public claimMinted;
    uint256 public constant MAX_CLAIMABLE_LANDS = 500;

    uint256 public mintStartTime;
    uint256 public mintDuration;
    bool private _restrictMint = false;
    uint256 public MINT_START_PRICE = 0.000001 ether;
    uint256 public MINT_END_PRICE = 0.0000005 ether;
    uint256 public MINT_PRICE_FACTOR = 0.00000005 ether;
    ...
    constructor(uint256 _maxLands) ERC721A("Worlds Beyond", "WBITS") {
        MAX_LANDS = _maxLands;
    }
    ...
    function presaleR1MintLands(
        ...
        external
        payable
        inTime(presaleR1StartTime, presaleR2StartTime)
    ...
    function presaleR2MintLands(
        ...
        external
        payable
        inTime(presaleR2StartTime, allowListR1StartTime)
    ...
    function allowListR2MintLands(
        ...
        external
        payable
        inTime(allowListR2StartTime, specialR1StartTime)
    ...
    function specialR1MintLands(
        ...
        external
        payable
        inTime(specialR1StartTime, specialR2StartTime)
        onlyVerified(Round.SpecialR1, maxLands, signature)
        mustMatchPrice(SPECIAL_MINT_PRICE, numLands)
    {
        _specialMintLands(numLands, maxLands);
    }

    function specialR2MintLands(
        uint256 numLands,
        uint256 maxLands,
        bytes calldata signature
    )
        external
        payable
        inTime(specialR2StartTime, claimableStartTime)
        onlyVerified(Round.SpecialR2, maxLands, signature)
        mustMatchPrice(SPECIAL_MINT_PRICE, numLands)
    {
        _specialMintLands(numLands, maxLands);
    }
    ...
    function mintLands(uint256 numLands, bytes calldata signature)
        external
        payable
    {
        ...
        if (_restrictMint) {
            require(
                _verify(keccak256(abi.encodePacked(msg.sender)), signature),
                "Restrict mint"
            );
        } else {
            require(
    ...
    function _getMintPrice() internal view returns (uint256) {
        ...
        uint256 m = (elapsed - 3600) / 1800 + 1;
        if (MINT_START_PRICE < MINT_PRICE_FACTOR * m) {
            return MINT_END_PRICE;
        }
        uint256 price = MINT_START_PRICE - MINT_PRICE_FACTOR * m;
    ...
    function _specialMintLands(uint256 numLands, uint256 maxLands) internal {
        require(
            numLands + specialMinted[msg.sender] <= maxLands,
            "Max lands per address exceeded"
        );
        require(
            totalSpecialMinted + numLands <= MAX_SPECIAL_LANDS,
            "Max special list lands exceeded"
        );

        specialMinted[msg.sender] += numLands;
        totalSpecialMinted += numLands;
        if (startSpecialTokenId == 0) {
            startSpecialTokenId = _nextTokenId();
        }
        _mintLands(msg.sender, numLands);
        if (totalSpecialMinted == MAX_SPECIAL_LANDS) {
            endSpecialTokenId = totalSupply();
        }
    }
    ...
    function _beforeTokenTransfers(
        ...
        if (
            startSpecialTokenId <= startTokenId &&
            startTokenId <= endSpecialTokenId
        ) {
            require(
                block.timestamp >= specialLandsReleaseTime,
    ...
    modifier inTime(uint256 startTime, uint256 endTime) {
        require(
            startTime > 0 &&
    ...
    function setPresaleStartTime(uint256 _r1StartTime, uint256 _r2StartTime)
        external
    /*onlyOwner*/
    {
    ...
    function setAllowListStartTime(uint256 _r1StartTime, uint256 _r2StartTime)
        external
    /*onlyOwner*/
    {
    ...
    function setSpecialStartTime(
        uint256 _r1StartTime,
        uint256 _r2StartTime,
        uint256 _releaseTime /*onlyOwner*/
    ) external {
        specialR1StartTime = _r1StartTime;
        specialR2StartTime = _r2StartTime;
        specialLandsReleaseTime = _releaseTime;
    }

    function setClaimableStartTime(
        uint256 _startTime /*onlyOwner*/
    ) external {
        claimableStartTime = _startTime;
    }

    function setSigner(
        address _signer /*onlyOwner*/
    ) external {
        signer = _signer;
    }

    function setMintStartTime(uint256 _mintStartTime, uint256 _mintDuration)
        external
    /*onlyOwner*/
    {
    ...
    function burnSupply(uint256 _newSupply) external onlyOwner {
        ...
        require(
            ...
            "new supply greater than current max supply"
        );
        require(
    ...
    function setProvenanceHash(
        string memory _provenance /*onlyOwner*/
    ) external {
