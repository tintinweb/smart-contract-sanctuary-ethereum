...
contract SBTCollection is ERC721, ERC721URIStorage, Mintable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    // Onchain player information
    struct Soul {
        uint256 PlayerID;
    ...
    function setSoulScore(uint256 tokenId, uint256 _score) external onlyOwner {
        souls[tokenId].Overallscore = _score;
    }
    ...
    function safeMint(
        ...
        // set soul data for token ID
        Soul memory playerSoul = Soul(_PlayerId, 0, block.timestamp);
        souls[tokenId] = playerSoul;
    }
