...
interface ITaggrNft {
  function initialize(address owner, address distributor, string memory name, string memory symbol, uint256 maxSupply, uint96 royaltiesPct) external;
...
interface INftDistributor {
  ...
  event NftClaimed(uint256 indexed index, address indexed to, address indexed contractAddress, uint256 tokenId);
  ...
  function purchaseNft(address contractAddress, uint256 tokenId) external;
}
