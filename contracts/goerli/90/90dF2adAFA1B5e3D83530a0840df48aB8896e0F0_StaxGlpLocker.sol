...
/// @notice Users purchase stxGLP with whitelisted tokens, ETH, or staked GLP -- 1:1 as if they were purchasing via GMX.io directly.
...
contract StaxGlpLocker is Ownable, ReentrancyGuard {
    ...
    event StaxStakingSet(address indexed staxStaking);
    ...
    /// @notice Set the Stax GLP Manager contract used to buy/sell and stake GLP
    ...
    function whitelistedTokens() external view returns (address[] memory) {
        return staxGlpManager.whitelistedTokens();
    }
     ...
     */
    function buyStxGlpWithTokenQuote(uint256 _amount, address _token) external view returns (
        uint256 gmxFeeBasisPoints, uint256 usdgAmountOut, uint256 glpAmountOut
    ) {
        address tokenIn = (_token == address(0)) ? wrappedNativeToken : _token;
        return staxGlpManager.buyStxGlpQuote(_amount, tokenIn);
    }
    ...
    /** 
      * @notice User buys stxGlp with an amount of (GMX.io whitelisted) tokens. STAX mints stxGLP 1:1 to the amount of GLP bought.
      * @param _amount How much of token to spend to purchase on GMX.io
      * @param _token What token to purchase with. This must be a whitelisted GMX.io asset.
    ...
    function buyStxGlpWithStakedGlp(
        ...
        // This is a special GMX.io contract which unstakes from the user and restakes to the STAX depositor
     ...
     * @param _stxGlpAmount The amount of stxGlp to sell
     * @param _toToken The token to receive when selling
     * Can also use the zero address (0x) for a quote of the native ETH/AVAX.
     * @dev This quote includes any fees GMX.io takes when purchasing GLP, and also STAX withdrawal fees.
     */
    function sellStxGlpToTokenQuote(uint256 _stxGlpAmount, address _toToken) external view returns (
        uint256 staxFeeBasisPoints, uint256 gmxFeeBasisPoints, uint256 tokenAmountOut
    ) {
        address tokenOut = (_toToken == address(0)) ? wrappedNativeToken : _toToken;
        return staxGlpManager.sellStxGlpQuote(_stxGlpAmount, tokenOut); 
    }
    ...
    /** 
      ...
      * @param _toToken What token to receive. This must be a whitelisted GMX.io asset.
      ...
      */
    function sellStxGlpToToken(uint256 _amount, address _toToken, uint256 _minAmountOut, address _recipient) external returns (
        uint256 amountOut
    ) {
        stxGlpToken.safeTransferFrom(msg.sender, address(staxGlpManager), _amount);
        amountOut = staxGlpManager.sellStxGlp(_amount, _toToken, _minAmountOut, _recipient);
    ...
    /** 
      ...
      * @param _recipient The receiving address of the `_toToken`
      */
    function sellStxGlpToNative(uint256 _amount, uint256 _minAmountOut, address payable _recipient) external nonReentrant returns (
        uint256 amountOut
    ) {
        // Sell the stxGLP back to the wrapped native token (eg weth), to this contract.
        stxGlpToken.safeTransferFrom(msg.sender, address(staxGlpManager), _amount);
        amountOut = staxGlpManager.sellStxGlp(_amount, wrappedNativeToken, _minAmountOut, address(this));
    ...
    function sellStxGlpToStakedGlp(uint256 _amount, address _recipient) external returns (uint256 amountOut) {
        stxGlpToken.safeTransferFrom(msg.sender, address(staxGlpManager), _amount);
        amountOut = staxGlpManager.sellStxGlpToStakedGlp(_amount, _recipient);
