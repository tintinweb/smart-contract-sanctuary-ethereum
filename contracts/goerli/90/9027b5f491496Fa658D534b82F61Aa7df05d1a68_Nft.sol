...
contract Nft is ERC721, Auction, ReentrancyGuard{
    ...
    */
    ...
    function safeTransferFrom(address from,address to,uint256 _tokenId) public virtual override {
        ...
        require(to != address(0) && to != from, "Estate protocol: invalid address");
    ...
    function auctionTransfer(uint256 _nftId,address to) external onlyOwner {
        ...
        emit Transfer(_msgSender(),to,_nftId);
       
    }

    function updateUserMintCount(address user) external onlyOwner {
    ...
    */
    function setTokenURI(uint256 nftId,string memory nftURI) public onlyOwner {
...
contract Auction is AccessControl,Ownable {
    ...
    function createNewAuction(uint256 _nftId,uint256 _nftPrice, uint256 maxBids) external onlyOwner {
        uint256 isCreated = nftAuction[_nftId].auctionDuration;
        require(isCreated == 0, "Nft already listed for auction" );

        nftAuction[_nftId].NftId = _nftId;
        nftAuction[_nftId].NftPrice = _nftPrice;

        if(_nftPrice == 0) {
            nftAuction[_nftId].NftPrice = 0.001 ether;
        }
        else {
            nftAuction[_nftId].NftPrice = _nftPrice;
        }
        ...
        if(maxBids == 0) {
            nftAuction[_nftId].maximumBids = 30;
        }
        else {
            nftAuction[_nftId].maximumBids = maxBids;
        }
        
        emit AuctionCreated(_nftId,_nftPrice,maxBids,nftAuction[_nftId].auctionDuration,false);
    }
    ...
    function startAuction(uint256 _nftId,uint256 startInUnixTime,uint256 durationInSecs) external onlyOwner {
        ...
        require(Nft.NftPrice > 0, "Nft not set for auction.");
    ...
    function toggleAuction(uint256 _nftId) external onlyOwner {
        AuctionStruct memory NftStatus = nftAuction[_nftId];

        require(NftStatus.NftPrice > 0, "Nft not listed for auction.");
    ...
    function makeABid(uint256 _nftId, uint256 bidAmount) public {
        addressRequireChecks();

        AuctionStruct memory Nft = nftAuction[_nftId];
        bool isBidded = userBids[_nftId][_msgSender()];

        require(bidAmount * 10 ** 18 >= Nft.NftPrice, "Bid Amount is low.");        
        ...
        nftAuction[_nftId].nftBidders.push(_msgSender());

        userBids[_nftId][_msgSender()] = true;

        nftAuction[_nftId].totalBids = Nft.totalBids + 1;
    ...
    //this function is to check the msgSender not the zero address
