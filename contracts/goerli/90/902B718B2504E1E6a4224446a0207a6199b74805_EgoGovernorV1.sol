...
import "hardhat/console.sol"; // FIXME: remove it
    ...
    CommonChecks
{
    ...
    uint public quorumNumerator; // percentage of total supply to reach quorum
    uint public autonomousQuorumNumerator; // percentage of total supply to reach quorum for autonomous proposal
    uint public denomenator;
    /// @notice Delay betwen proposal creation time and voting start in seconds
    uint32 public votingDelay;
    /// @notice Voting duration in seconds
        ...
        notZeroAddress(_initParams.token)
    {
        require(
            ...
            "EgoGovernor: UNSUPPORTED_INTERFACE"
        );
        ...
        _setQuorumNumerator(_initParams.quorumNumerator);
        _setAutonomousQuorumNumerator(_initParams.autonomousQuorumNumerator);
        _setDenomenator(_initParams.denomenator);
        _setVotingDelay(_initParams.votingDelay);
        _setVotingPeriod(_initParams.votingPeriod);
        _setPreventLateQuorum(_initParams.preventLateQuorum);
        _setProposalThreshold(_initParams.proposalThreshold);
    }

    /**
     * @notice Admin function for setting the quorum numerator
     * @param _newQuorumNumerator new quorum numerator
     */
    function setQuorumNumerator(uint _newQuorumNumerator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _setQuorumNumerator(_newQuorumNumerator);
    }

    /**
     * @notice Admin function for setting the autonomous quorum numerator
     * @param _newAutonomousQuorumNumerator new quorum numerator for autonomous proposals
     */
    function setAutonomousQuorumNumerator(uint _newAutonomousQuorumNumerator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _setAutonomousQuorumNumerator(_newAutonomousQuorumNumerator);
    }

    /**
     * @notice Admin function for setting the quorum denomenator
     * @param _newDenomenator new quorum denomenator
     */
    function setDenomenator(uint _newDenomenator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _setDenomenator(_newDenomenator);
    }
     ...
     */
    function setVotingDelay(uint32 _newVotingDelay)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     */
    function setVotingPeriod(uint32 _newVotingPeriod)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    /**
     * @notice Admin function for setting the prevent late quorum
     * @param _newPreventLateQuorum new quorum deadline
     */
    function setPreventLateQuorum(uint32 _newPreventLateQuorum)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _setPreventLateQuorum(_newPreventLateQuorum);
    }
     ...
     */
    function setProposalThreshold(uint _newProposalThreshold)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        onlyRole(DAO_ADMIN_ROLE)
    {
        ...
        require(
            status != ProposalState.Rejected,
            "EgoGovernor: ALREADY_REJECTED"
        );
    ...
    function cancelProposal(uint _proposalId) external {
        ...
        require(proposal.account == _msgSender(), "EgoGovernor: WRONG_ACCOUNT");

        ProposalState status = state(_proposalId);

        require(
            status == ProposalState.Pending || status == ProposalState.Active,
            "EgoGovernor: NOT_PENDING_OR_ACTIVE"
        );
     ...
     * @param _support 0 - against, 1 - for
     */
    function castVote(uint _proposalId, uint8 _support)
        external
        returns (uint)
    {
        return _castVote(_proposalId, _msgSender(), _support, "");
    }
    ...
    function castVoteWithReason(
        ...
        return _castVote(_proposalId, _msgSender(), _support, _reason);
    }
    ...
    function state(uint _proposalId) public view returns (ProposalState) {
        ...
        require(proposal.voteStart != 0, "EgoGovernor: INVALID_ID");

        if (proposal.rejected) {
            return ProposalState.Rejected;
        } else if (proposal.cancelled) {
            return ProposalState.Cancelled;
        } else if (proposal.voteStart >= block.number) {
            return ProposalState.Pending;
        } else if (_proposalDeadline(proposal) >= block.number) {
            return ProposalState.Active;
        } else if (
            accessRegistry.hasRole(AUTONOMOUS_DAO_ROLE, proposal.account) &&
            autonomousQuorumReached(proposal) &&
            proposal.forVotes > proposal.againstVotes
        ) {
            return ProposalState.Succeeded;
        } else if (
            quorumReached(proposal) && proposal.forVotes > proposal.againstVotes
        ) {
            return ProposalState.Succeeded;
        } else {
     ...
     * Quorum = currentSupply * numerator / denominator
     */
    function quorumReached(Proposal memory _proposal)
        public
        view
        returns (bool)
    {
        return _quorum(_proposal.voteStart) <= _proposal.forVotes;
    }

    /**
     * @dev Amount of votes already cast passes the autonomous threshold limit.
     */
    function autonomousQuorumReached(Proposal memory _proposal)
        public
        view
        returns (bool)
    {
        return
            (token.getPastTotalSupply(_proposal.voteStart) *
    ...
    function propose(
        ...
        require(
            _getVotes(_msgSender(), block.number - 1) >= proposalThreshold ||
                accessRegistry.hasRole(AUTONOMOUS_DAO_ROLE, _msgSender()),
            "EgoGovernor: THRESHOLD"
        );
        ...
        require(proposal.voteStart == 0, "EgoGovernor: EXISTS");

        uint32 snapshot = block.number.toUint32() + votingDelay;
        uint32 deadline = snapshot + votingPeriod;
    ...
    function _castVote(
        ...
        require(
            ...
            "EgoGovernor: NOT_ACTIVE"
        );

        uint weight = _getVotes(_account, proposal.voteStart);
        _countVote(_proposalId, _account, _support, weight.toUint128());

        if (proposal.extendedDeadline == 0 && quorumReached(proposal)) {
        ...
        emit VoteCast(_account, _proposalId, _support, weight, _reason);
    ...
    function _countVote(
        ...
        require(!hasVoted[_proposalId][_account], "EgoGovernor: ALREADY_VOTED");
        ...
        if (_support == uint8(VoteType.Against)) {
            ...
            revert("EgoGovernor: INVALID_SUPPORT");
        }
    }

    function _setQuorumNumerator(uint _newQuorumNumerator) private {
        require(_newQuorumNumerator != 0, "EgoGovernor: INVALID_NUMENATOR");
        quorumNumerator = _newQuorumNumerator;

        emit SetQuorumNumerator(_newQuorumNumerator);
    }

    function _setAutonomousQuorumNumerator(uint _newAutonomousQuorumNumerator)
        private
    {
        require(
            _newAutonomousQuorumNumerator != 0,
            "EgoGovernor: INVALID_AUTO_NUMENATOR"
        );
        autonomousQuorumNumerator = _newAutonomousQuorumNumerator;

        emit SetAutonomousQuorumNumerator(_newAutonomousQuorumNumerator);
    }

    function _setDenomenator(uint _newDenomenator) private {
        require(_newDenomenator != 0, "EgoGovernor: INVALID_DENOMINATOR");
        denomenator = _newDenomenator;

        emit SetDenomenator(_newDenomenator);
    }

    function _setVotingDelay(uint32 _newVotingDelay) private {
        require(_newVotingDelay != 0, "EgoGovernor: INVALID_DELAY");
        votingDelay = _newVotingDelay;

        emit SetVotingDelay(_newVotingDelay);
    }

    function _setVotingPeriod(uint32 _newVotingPeriod) private {
        require(_newVotingPeriod != 0, "EgoGovernor: INVALID_PERIOD");
        votingPeriod = _newVotingPeriod;

        emit SetVotingPeriod(_newVotingPeriod);
    }

    function _setPreventLateQuorum(uint32 _newPreventLateQuorum) private {
        require(_newPreventLateQuorum != 0, "EgoGovernor: INVALID_LATE_QUORUM");
        preventLateQuorum = _newPreventLateQuorum;

        emit SetPreventLateQuorum(_newPreventLateQuorum);
    }

    function _setProposalThreshold(uint _newProposalThreshold) private {
        require(_newProposalThreshold != 0, "EgoGovernor: INVALID_THRESHOLD");
    ...
    function _quorum(uint _blockNumber) private view returns (uint) {
        return
            (token.getPastTotalSupply(_blockNumber) * quorumNumerator) /
            denomenator;
    }

    function _getVotes(address account, uint blockNumber)
        private
        view
        returns (uint)
    {
...
interface IEgoGovernorV1 {
    ...
    struct Proposal {
        ...
        bool rejected; // 136
        bool cancelled; // 144
        uint128 againstVotes; // 128
        uint128 forVotes; // 256
    }
    ...
    event SetDenomenator(uint newDenomenator);
