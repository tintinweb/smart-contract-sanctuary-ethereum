    ...
    IAnimeMetaverseReward
{
    ...
    function mintBatch(
        ...
        for (uint256 index = 0; index < _ids.length; index = _uncheckedInc(index)) {
            uint256 _id = _ids[index];

            if (_amounts[index] > maxMintLimit || _amounts[index] < 1) {
                revert InvalidMintingAmount(_amounts[index]);
            }
            if (_id < 1 || _id > MAX_MINTATBLE_TOKEN_ID) {
                revert InvalidMintableTokenId();
            }
            if (
                tokenCollection[_id].totalSupply + _amounts[index] >
