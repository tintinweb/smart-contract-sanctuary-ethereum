...
contract BlurFarmer is TinyERC721, Ownable, ReentrancyGuard {
    uint256 mintPrice = 0.01e18;

    string svg;

    constructor(string memory _svgString)
    ...
    function mint(uint256 quantity) public payable nonReentrant {
        require(msg.value == quantity * mintPrice, "Send correct value");
    ...
    function burn(uint256[] memory tokenIds) public nonReentrant {
        ...
        bool sent = payable(msg.sender).send(tokenIds.length * mintPrice);
        require(sent);
    }
        ...
        returns (string memory output)
    {
        ...
        bytes memory dataURI = abi.encodePacked(
            '{',
                '"name": BlurFarmer #', toString(tokenId), '",',
                '"description": "BlurFarmer is for BLUR farming. Token can be burnt for 1 ETH.",',
                '"image": "', svg, '"',
            '}'
        );
        return string(
