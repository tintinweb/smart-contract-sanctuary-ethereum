  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function createAndMint(IPropsCreatorConfig.ERC1155Token[] memory _token, IPropsCreatorConfig.Split[] memory _primarySplit, IPropsCreatorConfig.Split[] memory _royaltySplit, IPropsCreatorConfig.CreateMintConfig[] memory _config) external payable nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < _token.length; i++) {
            ...
            string memory tokenCheck = IPropsCreatorConfig(configContract).getTokenCheck(_token[i]);

            string memory primaryCheck = IPropsCreatorConfig(configContract).getPrimaryCheck(_primarySplit[i]);

            string memory royaltyCheck = IPropsCreatorConfig(configContract).getRoyaltyCheck(_royaltySplit[i]);
    ...
    function mint(
        ...
        require(!IPropsCreatorConfig(configContract).isSanctioned(_msgSender()), "S");
        require(IPropsCreatorConfig(configContract).isUniqueArray(_allowlistIds), "D");
