...
import "./types/CryptosAccessControlled.sol";

contract Distributor is IDistributor, CryptosAccessControlled {
    /* ========== DEPENDENCIES ========== */
    ...
    IERC20 private immutable cryptos;
    ...
    constructor(
        ...
        address _staking,
        address _authority
    ) CryptosAccessControlled(ICryptosAuthority(_authority)) {
        require(_treasury != address(0), "Zero address: Treasury");
        treasury = ITreasury(_treasury);
        require(_cryptos != address(0), "Zero address: CRYPTOS");
        cryptos = IERC20(_cryptos);
    ...
    function nextRewardAt(uint256 _rate) public view override returns (uint256) {
        return cryptos.totalSupply().mul(_rate).div(rateDenominator);
    }
...
import "../interfaces/ICryptosAuthority.sol";

abstract contract CryptosAccessControlled {
    /* ========== EVENTS ========== */

    event AuthorityUpdated(ICryptosAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    ICryptosAuthority public authority;

    /* ========== Constructor ========== */

    constructor(ICryptosAuthority _authority) {
    ...
    function setAuthority(ICryptosAuthority _newAuthority) external onlyGovernor {
