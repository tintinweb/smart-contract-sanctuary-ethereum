...
contract XLARSCPrepaymentUsd is Initializable, BaseRSCPrepayment {
     ...
     * @notice Internal function to convert ETH value to ETH value
     */
    function _convertTokenToUsd(address token, uint256 _ethValue) internal view returns (uint256) {
        return (_getTokenUsdPrice(token) * _ethValue) / 1e18;
    }

    /**
     * @notice Internal function to convert USD value to ETH value
     */
    function _convertUsdToToken(address token, uint256 _usdValue) internal view returns (uint256) {
        return (_usdValue * 1e25 / _getTokenUsdPrice(token) * 1e25) / 1e32;
    }
