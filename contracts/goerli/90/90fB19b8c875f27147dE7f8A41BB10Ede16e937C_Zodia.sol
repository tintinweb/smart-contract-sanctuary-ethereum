    ...
    OperatorFilterer
{
    ...
    // Maps a `_questId` to a `zodiaId` to a completion.
    ...
    function abandonQuests(uint256[] calldata tokenIds) external {
        unchecked {
            for (uint256 i; i < tokenIds.length; i++) {
    ...
    function claimQuestReward(uint256 tokenId) external payable nonReentrant {
        ...
        if (completed[journal.questId][tokenId]) revert AlreadyClaimedReward();
        ...
        questRewardClaimBalance[quest.creator] += msg.value;
    ...
    // The reason anyone can call this is because overseeer can de-role quest giver but should not
    // block their access to already claimed funds
    ...
    function _startQuest(
        ...
        // If zodia already has already complete quest then cannot claim again
        if (completed[questId][tokenId]) revert AlreadyClaimedReward();
