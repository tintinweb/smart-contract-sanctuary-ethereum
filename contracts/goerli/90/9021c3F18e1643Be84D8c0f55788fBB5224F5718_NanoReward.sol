...
// File: META NANOs/Reward NFTs/NativeMetaTransaction.sol
...
// File: META NANOs/Reward NFTs/ContextMixin.sol
...
contract NanoReward is ERC721A, ERC2981, NanoBase {

    constructor(address _metaNanoData)
    ERC721A("Meta Nano Rewards", "NANOREWARD")
    NanoBase(_metaNanoData, "NanoReward")
    {}

    function mintFor(address destination, uint256 quantity) external only("adminMinter") {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken();
        return MetaNanoTokenMetaDataI(metananoData.getAddress("tokenMetaDataNanoReward")).tokenURI(tokenId);
    }
}


contract NanoNames is NanoBase {

    constructor(address _metaNanoData)
    NanoBase(_metaNanoData, "NanoNames")
    {}
    ...
    function updateName(uint256 tokenId, string memory newName) external only("nameUpdates") {
