...
contract KonaFinance is KonaOracle, AccessControl {
    ...
    event LoanWidthdrawn(uint256 indexed providerID, uint256 indexed loanID, address indexed lender, uint256 amount);
    ...
    function createLoan(uint256 _providerID, uint256 _loanID, uint256 _amount, address _borrower, uint256 _maturity, uint256 _repayments, uint256 _totalLocked, uint256 _apy) public {
    ...
    function invest(uint256 _providerID, uint256 _loanID, address _payToContract) external {
        ...
        string memory _url = string(abi.encodePacked(string.concat(providers[_providerID].apiURL, Strings.toString(_providerID), '/', Strings.toString(_loanID))));

        callOracle(_providerID, _loanID, _url, this.fulfill.selector);

        emit LoanPreInvested(_providerID, _loanID, msg.sender);
    }

    function claim(uint256 _providerID, uint256 _loanID) public {
        require(providers[_providerID].valid, "Invalid provider");
    ...
    function setLoanAsLockDone(uint256 _providerID, uint256 _loanID, string memory _lockReference) external {
        ...
        emit LoanWidthdrawn(_providerID, _loanID, loan.borrower, loan.amount);    
    }

    function deleteLoan(uint256 _providerID, uint256 _loanID) public {
        require(providers[_providerID].valid, "Invalid provider");
        require(providers[_providerID].creators[msg.sender], "Forbidden access");
    ...
    function repay(uint256 _amount, uint256 _providerID, uint256 _loanID) public {
        ...
        for (uint i = 0; i < providers[_providerID].feeAmounts.length; i++) {
            ...
            feesToCollect[providers[_providerID].feeWallets[i]] += total;
            lenderTotal -= total;
        }
    ...
    function fulfill(bytes32 _requestId, uint256 _oracleValue) public recordChainlinkFulfillment(_requestId) {
        ...
        if (_oracleValue == 1) {
            emit ErrorAPICall(providerID, loanID);
        }
        else if(_oracleValue == 3) {
            // Could not lock receivables, delete loan
            delete providers[providerID].loans[loanID];

            emit LoanDeleted(providerID, loanID, address(0));
        } else {
            Loan memory loan = providers[providerID].loans[loanID];
            ...
            if (IERC20(brzToken).balanceOf(loan.lender) < loan.amount || IERC20(brzToken).allowance(loan.lender, address(this)) < loan.amount){
                providers[providerID].loans[loanID].lender = address(0);
            } else {
                IERC20(brzToken).transferFrom(loan.lender, address(this), loan.amount);
                providers[providerID].loans[loanID].invested = true;

                emit LoanInvested(providerID, loanID, loan.lender);
            }
        }
    ...
    function enableProvider(uint256 _providerID, string memory _name, string memory _apiURL, uint256[] memory _feeAmounts, address[] memory _feeWallets) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setProviderApiURL(uint256 _providerID, string memory _apiURL) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(providers[_providerID].valid, "Invalid provider");

        providers[_providerID].apiURL = _apiURL;
    }

    function setProviderCreator(uint256 _providerID, address _creator, bool _enabled) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(providers[_providerID].valid, "Invalid provider");

        providers[_providerID].creators[_creator] = _enabled;
    }

    function disableProvider(uint256 _providerID) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(providers[_providerID].valid, "Invalid provider");
    ...
    function updateState(uint256 _providerID, uint256 _loanID, address _lender, address _payToContract, bool _invested) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateKonaFees(uint256 _konaFees) public onlyRole(DEFAULT_ADMIN_ROLE) {
        konaFees = _konaFees;
    }

    function updateLoan(uint256 _providerID, uint256 _loanID, uint256 _maturity, uint256 _repayments, uint256 _totalLocked, uint256 _apy) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function getLoan(uint256 _providerID, uint256 _loanID) public view returns (string memory, uint256, uint256, uint256, address, bool, address, address) {
        Loan memory loan = providers[_providerID].loans[_loanID];
        
        if (!providers[_providerID].valid || !loan.valid) {
            return ('', 0, 0, 0, address(0), false, address(0), address(0));
        }

        return (loan.lockReference, loan.amount, loan.lenderToClaim, loan.totalRepaid, loan.borrower, loan.invested, loan.lender, loan.payToContract);
    }

    function getLoanBasic(uint256 _providerID, uint256 _loanID) public view returns (bool, uint256, bool, uint256) {
        Loan memory loan = providers[_providerID].loans[_loanID];
    ...
    function TEST_fullfill(bytes32 _requestId, uint256 _oracleValue) external onlyOwner {
        fulfill(_requestId, _oracleValue);
    }
...
contract KonaOracle is ChainlinkClient, ConfirmedOwner {
    ...
    event ResultContractData(bytes32 indexed requestId, uint256 value, uint256 providerID, uint256 loanID);
    event RequestContractData(string url, uint256 providerID, uint256 loanID);
    event ErrorAPICall(uint256 providerID, uint256 loanID);
    ...
    function callOracle(uint256 _providerID, uint256 _loanID, string memory _url, bytes4 selector) internal returns (bytes32 requestId) {
        ...
        requestId = sendChainlinkRequest(request, oracleFee);

        requests[requestId].providerID = _providerID;
        requests[requestId].loanID = _loanID;

        emit RequestContractData(_url, _providerID, _loanID);
    }
