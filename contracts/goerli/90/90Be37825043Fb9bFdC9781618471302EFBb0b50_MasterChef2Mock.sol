...
abstract contract WithStopLoss {
  ...
  function _runStopLoss(
    ...
    require(stopLoss.path.length > 1 && stopLoss.amountOut > 0, "WithStopLoss::_runStopLoss: stop loss disabled");
    ...
    return stopLoss.path[stopLoss.path.length - 1];
  }
}
...
import "./utils/Uniswap/WithStopLoss.sol";
import "./IMasterChef2.sol";
...
contract MasterChef2LpRestake is Automate, WithStopLoss {
  ...
  function runStopLoss(uint256 gasFee, uint256 _deadline) external bill(gasFee, "PancakeSwapMasterChef2LpStopLoss") {
    ...
    IERC20 exitToken = IERC20(_runStopLoss(liquidityRouter, inTokens, _deadline));
