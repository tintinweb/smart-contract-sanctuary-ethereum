...
abstract contract MultiMerkleRoots {
    ...
    function _setMerkleRoot(uint _allocation, bytes32 _newRoot) internal {
        _merkleRoots[_allocation] = _newRoot;
    }
...
contract FLT is ERC721I, Ownable, Security, CollectionOwnership, MultiMerkleRoots {
    // Constructor
    constructor() payable ERC721I("Funcles Legacy Ticket", "FLT") {}
    ...
    function setMerkleRoot(uint _allocation, bytes32 _newRoot) external onlyOwner {
        _merkleRoots[_allocation] = _newRoot;
    }
    ...
    function claim(uint256[] calldata tokenIds_) external onlySender publicClaimEnabled {
        ...
        // TODO: require that the user is whitelisted for their allocation~

        uint256 _len = tokenIds_.length;
        for (uint256 i = 0; i < _len; i++) {
            uint256 _tokenId = tokenIds_[i];
            require(!_claimedTokenIds[_tokenId], string(abi.encodePacked("Token ", _toString(_tokenId), " already claimed")));
            require(_collectionAddress.ownerOf(_tokenId) == msg.sender, string(abi.encodePacked("Token ", _toString(_tokenId), " does not belong to sender")));
            _claimedTokenIds[tokenIds_[i]] = true;
        }
        _mintMany(msg.sender, tokenIds_);
    }

    // TODO: confirm with the funcles team whether to keep the mapping of tokenIds #1 to tokenId #1 to a bit more gas, or just have them mint a new NFT.
    function claimv2(uint256[] calldata tokenIds_, bytes32[] calldata _merkleProof, uint _allocation) external onlySender publicClaimEnabled {
        require(isAllowlisted(msg.sender, _merkleProof, _allocation), "Invalid proof");
