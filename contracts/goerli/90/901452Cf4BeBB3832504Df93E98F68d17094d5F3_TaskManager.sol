...
contract TaskManager is Validatable, ITaskManager, ReentrancyGuardUpgradeable {
    ...
    event CreatedTask(TaskInfo taskInfo);
    event UpdatedTask(
    ...
    event CancalledTask(uint256 indexed taskId);
    event CompletedMilestone(uint256 indexed taskId);
    ...
    /**
     * Throw an exception if array addresses is not valid
     */
    modifier validAddressArray(address[] memory arrays) {
        require(arrays.length > 0, "Invalid length");
        for (uint256 i = 0; i < arrays.length; i++) {
            require(arrays[i] != address(0), "Invalid address");
        }
        _;
    }
     ...
     */
    modifier onlyProjectOwner(address collection) {
        require(project.getProjectOwnerOf(collection) == _msgSender(), "Caller is not project owner");
        _;
    }
     ...
     * @param project_ Address of project contract
     */
    function initialize(
        IAdmin _admin,
        IProject project_
    ) public initializer notZeroAddress(address(_admin)) notZeroAddress(address(project_)) {
    ...
    function createTask(
        ...
        uint256 endTime,
        uint256 budget
    ) external onlyProjectOwner(collection) notZero(budget) {
        require(startTime > block.timestamp, "Invalid startTime");
        require(startTime < endTime, "Invalid endTime");
        require(
            budget + budgetInUseOf[collection] <= IClaimPool(project.getClaimPoolOf(collection)).getReward(collection),
        ...
        budgetInUseOf[collection] = budgetInUseOf[collection] + budget;
        emit CreatedTask(tasks[_taskCounter.current()]);
    }
    ...
    function updateTask(
        ...
        uint256 endTime,
        uint256 budget
    ) external onlyProjectOwner(collection) validTaskId(taskId) notZero(budget) {
        ...
        require(
            budget + budgetInUseOf[collection] - taskInfo.budget <=
                IClaimPool(project.getClaimPoolOf(collection)).getReward(collection),
            "Over collection budget"
        );
        ...
        budgetInUseOf[collection] = budgetInUseOf[collection] + budget - taskInfo.budget;
        taskInfo.collection = collection;
        taskInfo.budget = budget;
        taskInfo.startTime = startTime;
        taskInfo.endTime = endTime;

        emit UpdatedTask(taskId, collection, budget, startTime, endTime);
    }
     ...
     */
    function cancelTask(uint256 taskId) external onlyProjectOwner(tasks[taskId].collection) {
        TaskInfo storage taskInfo = tasks[taskId];
        require(taskInfo.status == StatusTask.ACTIVE, "Task was done or cancel");
     ...
     *
     * emit {CompletedMilestone} events
     */
    function completeTask(
        uint256 taskId,
        address[] memory users,
        uint256[] memory rewards
    ) external onlyProjectOwner(tasks[taskId].collection) validAddressArray(users) {
        TaskInfo storage taskInfo = tasks[taskId];

        require(taskInfo.status == StatusTask.ACTIVE, "Task was done or cancel");
        require(users.length == rewards.length, "Invalid reward length");

        uint256 spentBudget;
        for (uint256 i = 0; i < users.length; ++i) {
            spentBudget = spentBudget + rewards[i];
        }
        require(spentBudget <= taskInfo.budget, "Overflow budget");
        ...
        IReward(project.rewardAddress()).releaseReward(project.getPaymentTokenOf(taskInfo.collection), users, rewards);

        // Transfer token to reward pool
        IClaimPool(project.getClaimPoolOf(taskInfo.collection)).withdrawCollection(
            taskInfo.collection,
            project.rewardAddress(),
            spentBudget
        );

        emit CompletedMilestone(taskId);
    }
