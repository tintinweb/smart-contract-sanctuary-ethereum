 ...
 * @dev Implementation of 
 ttps://eips.ethereum.org/EIPS/eip-721[ERC721] Non-Fungible Token Standard, including
...
contract NFTMarketPlace is ReentrancyGuard, IERC721Receiver {
    ...
    function fetchMyNFTs(address sender)
        ...
        returns (NftMerketItem[] memory)
        {
            ...
            for (uint256 i = 0; i < totalItemCount; i++) {
                if (idForMarketItem[i].owner == sender) {
                    itemCount += 1;
                }
            }

            NftMerketItem[] memory items = new NftMerketItem[](itemCount);
            for (uint256 i = 0; i < totalItemCount; i++) {
                if (idForMarketItem[i].owner == sender) {
                    uint256 currentId = i;
   ...
   //adding non-replicant users to whitelist one-by-one
   ...
   //Verify user is Whitelisted or not 
    ...
    function remove(address userAddress) public {
        ...
        delete WhiteArray[WhiteArray.length - 1];
        // WhiteArray.length--;
    }
