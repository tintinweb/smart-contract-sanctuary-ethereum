...
import "./GenesisRocket.sol";
import "@openzeppelin/contracts/metatx/ERC2771Context.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract SigTrigger is ERC2771Context, Ownable {
...
// mapping(TokenURIType => mapping(uint256 => string)) private tokenURIs;
...
//        signer = _signer;
        URI1 = "ipfs://QmQf54fHMFpFqZSUc6epgGNbXpNqcBwPnRiohdHvacmpoj/1.json";
        URI2 = "ipfs://QmQf54fHMFpFqZSUc6epgGNbXpNqcBwPnRiohdHvacmpoj/2.json";
        URI3 = "ipfs://QmQf54fHMFpFqZSUc6epgGNbXpNqcBwPnRiohdHvacmpoj/3.json";
    }
    ...
    function  MintTrigger(address userWallet, bytes32 hash, bytes memory signature) external  {
    ...
    //    thistokenID = rocket._nextTokenId();
    ...
    // FactoryLink Trigger
    function upgradeTrigger(uint256 typeOfURI, uint256 tokenId, string calldata newURI) external {
    // check if the msg sender is the owner
        require(rocket.ownerOf(tokenId) == msg.sender, "You don't own the given Box");
        rocket.setNewTokenURI(typeOfURI, tokenId, newURI);
    }

    function upgradeProxy(uint256 tokenId) external {
    rocket.setNewTokenURI(1, tokenId, URI1);
    }

    // function upgradeProxy(uint256 tokenId) external {
    // upgradeTrigger(1, tokenId, URI1);
    // }
...
//   ipfs://QmQf54fHMFpFqZSUc6epgGNbXpNqcBwPnRiohdHvacmpoj/1.json
//   ipfs://QmQf54fHMFpFqZSUc6epgGNbXpNqcBwPnRiohdHvacmpoj/2.json
//   ipfs://QmQf54fHMFpFqZSUc6epgGNbXpNqcBwPnRiohdHvacmpoj/3.json



function  MintTrigger2() external  {
    rocket.minterMint(_msgSender());
    }


//  function upgradeProxy(uint256 tokenId, string calldata newURI) external {
//      Code to be executed if the DNA is 1
//         if (dna == 1) {
//         rocket.setNewTokenURI(1, tokenId, newURI);
...
// }
    ...
    // function mintAndUpgradeTrigger(address userWallet, bytes32 hash, bytes memory signature, TokenURIType typeOfURI) external {
    //     require(mintIsOpen, "Mint is not open right now");
    //     require(recoverSigner(hash, signature) == signer, "incorrect signature or hash");
    //     require(!signatureUsed[signature], "Signature has already been used.");
    //     require(!walletAddressused[userWallet], "User already minted");
    ...
    //     rocket.minterMint(_msgSender());
    //     // Set the token URI
    //     rocket.setNewTokenURI(tokenURIs[typeOfURI][tokenId]);
    //     // Mark signature as redeemed
    //     signatureUsed[signature] = true;    
    //     walletAddressused[userWallet] = true;
    // }
    ...
    // function storeURIs(TokenURIType typeOfURI, uint256 tokenId, string calldata newURI) public {
    //  tokenURIs[typeOfURI][tokenId] = newURI;
    // }
...
contract GenesisRocket is ERC721A, ERC721AQueryable, IERC721ABurnable, Context, OwnableRoles, Pausable, DefaultOperatorFilterer, ERC2981 {
