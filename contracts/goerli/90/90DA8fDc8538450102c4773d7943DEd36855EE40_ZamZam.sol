...
contract ZamZam is ERC20, Ownable {
    ...
    uint256 public swapTripUSDT; //Can keep statement the same, but pass stable coin address with 18 decimals when using Ethereum
    ...
    address private USDTAddress = 0x4c49acC40aC6C32B9Ab271027bE228D5b09d2100; //Make sure to set the address to USDC when deploying to ETH network. USDT only has 6 decimal places.
    ...
    constructor () ERC20("ZamZam", "ZMX2") {
        ...
        setSwapTripUSDT(10); // Initialize with 10 USDT use a higher amount when launching on the ETH network
...
// Function to allow the owner to update the USDT address incase of depegg or other emergency
...
//internal burn 
    ...
    function getTokenPriceInUSDT(uint256 amountOfTokens) public view returns(uint256) {
        address[] memory path = new address[](3);
        path[0] = address(this); // this contract's address
        path[1] = uniswapV2Router.WETH(); // WETH address
        path[2] = USDTAddress; // USDT address

        try uniswapV2Router.getAmountsOut(amountOfTokens, path) returns (uint[] memory amounts) {
            return amounts[2]; // Amount of USDT you would get for the tokens
        } catch {
    ...
    function contractTaxUSDT() public view returns(uint256) {
        uint256 tokenAmount = devFeeBalance + partnershipsBalance + autoBurnBalance + maintenanceBalance;
        return getTokenPriceInUSDT(tokenAmount);
    }

    function setSwapTripUSDT(uint256 newThresholdInWholeNumbers) public onlyOwner {
        swapTripUSDT = newThresholdInWholeNumbers * 10**18; // Account for USDT's 18 decimal places
    }

    //quick verification check before attempting to swap
    function shouldSwapNow() public view returns(bool) {
        ...
        if (contractTaxUSDT() < swapTripUSDT) {
            return false;
        }
            return true;
    }
    ...
    function checkSwapTrip() public onlyOwnerOrContract {
        if (shouldSwapNow()) {
            swapNow();
        }
        else {
            ...
            // Check if token balance is more than 0.001 of the total supply and getTokenPriceInUSDT returns 0
            if ((tokenBalance * 1000) / totalSupply >= 1 && getTokenPriceInUSDT(tokenBalance) == 0) {
                swapNow();
            }
        }
    }
   ...
   function addToTaxBalance(uint256 amount) external onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (!isExcluded) {
            ...
            // Check if it's time to swap after a sell operation
