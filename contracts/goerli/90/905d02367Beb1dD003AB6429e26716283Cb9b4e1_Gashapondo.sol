...
contract Gashapondo is ERC721, ERC721Burnable, Ownable, IGashapondo {
    ...
    bytes32 public constant COLLECTION_ADDED_MINTER = keccak256("ADDED_MINTER");
    bytes32 public constant COLLECTION_REMOVED_MINTER = keccak256("REMOVED_MINTER");
    ...
    mapping(uint256 => mapping(address => bool)) public collectionMinters;
     ...
     */
    modifier onlyCollectionMinter(uint256 collectionId) {
        require(collectionMinters[collectionId][_msgSender()], "Caller is not the minter");
        _;
    }
    ...
    modifier collectionExists(uint256 collectionId) {
        require(collectionId <= _collectionCount, "Collection does not exist");
        _;
    }
    ...
    function mint(address to, uint256 collectionId)
        external
        override
        onlyCollectionMinter(collectionId)
     ...
     * Add a new minter
     */
    function addMinter(uint256 collectionId, address minter)
        external
        override
        onlyAdmin
        collectionExists(collectionId)
    {
        collectionMinters[collectionId][minter] = true;
        emit PlatformUpdated(COLLECTION_ADDED_MINTER);
    }

    /**
     * Remove minter
     */
    function removeMinter(uint256 collectionId, address minter)
        external
        override
        onlyAdmin
        collectionExists(collectionId)
    {
        collectionMinters[collectionId][minter] = false;
        emit PlatformUpdated(COLLECTION_ADDED_MINTER);
    }
    ...
    function addCollection(
        ...
        _collections[collectionId].useRandomTokenId = useRandomTokenId;
    ...
    ) external override onlyAdmin whenNotInState(CollectionState.FROZEN, collectionId) collectionExists(collectionId) {
    ...
    function setCollectionRandomTokenId(uint256 collectionId, bool useRandomTokenId)
        external
        override
        onlyAdmin
        whenNotInState(CollectionState.FROZEN, collectionId)
    ...
    function setCollectionState(uint256 collectionId, CollectionState nextState)
        external
        override
        onlyAdmin
        collectionExists(collectionId)
    ...
    function togglePaused(uint256 collectionId)
        external
        override
        onlyAuthor(collectionId)
        collectionExists(collectionId)
    {
        CollectionState currentState = _collections[collectionId].state;
        require(
    ...
    function _mintToken(address to, uint256 collectionId) internal returns (uint256 tokenId) {
        ...
        if (_collections[collectionId].useRandomTokenId) {
