...
contract Referral is Ownable {
        ...
        payable
    {
        ...
        require(msg.value >= mintRequest.price, "Not enough ETH");
        ...
        (bool referralPaymentSuccess, ) = mintRequest.referralAddress.call{
            value: msg.value.div(100).mul(mintRequest.referralRewardPercent)
        }("");
        require(referralPaymentSuccess, "Failed to send Ether to referral");

        (bool ownerPaymentSuccess, ) = mintRequest.tokensOwnerAddress.call{
            value: msg.value.div(100).mul(
                100 -
                    (
                        mintRequest.referralRewardPercent.add(
                            mintRequest.systemRewardPercent
                        )
                    )
            )
