...
import "@gnus.ai/contracts-upgradeable-diamond/contracts/utils/cryptography/ECDSAUpgradeable.sol";

//  ==========  INTERNAL IMPORTS    ==========

import {Modifiers} from "../guild/GuildStorage.sol";
import {GuildXPStorage} from "./GuildXPStorage.sol";
...
contract GuildXP is Modifiers, Initializable {
    using GuildXPStorage for GuildXPStorage.Layout;
    ...
    event GuildXPClaimed(address indexed user, uint256 indexed amount);
    ...
    function claim(bytes calldata signature, uint256 xpAmount) external onlyMembers {
        if (xpAmount == 0) revert InvalidAmount();
        if (!_verifySignatureXP(msg.sender, signature, xpAmount)) revert InvalidSignature();
        ++GuildXPStorage.layout().nonces[msg.sender];
        emit GuildXPClaimed(msg.sender, xpAmount);
        gs.balances[msg.sender] += xpAmount;
    }
    ...
    function burn(address from, uint256 amount) external onlyManagers {
        emit GuildXPBurnedFrom(from, msg.sender, amount);
        gs.balances[from] -= amount;
    }
    ...
    function setSigner(address signer) external onlyManagers {
        gs.signer = signer;
    }
    ...
    function getNonce(address user) external view returns (uint256) {
        return GuildXPStorage.layout().nonces[user];
    }
    ...
    function _verifySignatureXP(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(address(this), user, xpAmount, GuildXPStorage.layout().nonces[user])
        );
        return gs.signer == messageHash.toEthSignedMessageHash().recover(signature);
    }
}
...
library GuildXPStorage {
    struct Layout {
        mapping(address => uint256) nonces;
    }

    bytes32 internal constant STORAGE_SLOT = keccak256("guild.contracts.storage.guildxp");
