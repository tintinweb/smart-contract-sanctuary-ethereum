...
contract DexOrderBookV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    /// @param _price price of token to place order
    /// @param _amount amount of base token to place order
    /// @param _expiryInd expiry index of order (0-4)
    /// @param _isBuyOrder boolean to indicate if order is a buy order or not
    /// @param _isLimitOrder boolean to indicate if order is a limit order or not
    function placeOrder(
        address _baseToken,
        address _quoteToken,
        uint256 _price,
        ...
        require(
            _expiryInd < expiryDurations.length,
            "Invalid expiry duration."
        );

        // Converting ETH to WETH
        if (_baseToken == USE_ETHEREUM) {
            require(msg.value >= _amount + orderFee, "Insufficient ETH");
            IWETH(wethToken).deposit{value: _amount}();

            _baseToken = wethToken;
        }
        _quoteToken = _quoteToken == USE_ETHEREUM ? wethToken : _quoteToken;
        ...
        require(msg.value >= orderFee, "Insufficient fee");

        if (_baseToken != USE_ETHEREUM) {
            IERC20Upgradeable(_baseToken).transferFrom(
                msg.sender,
                address(this),
                _amount
            );
        ...
        uint256 _expireAt = _expiryInd == 0
            ? 0
            : block.timestamp + expiryDurations[_expiryInd];
    ...
    function cancelOrder(uint256 _orderId) external nonReentrant {
        ...
        require(
            orders[msg.sender].orders[_orderId].baseToken != address(0),
            "Unauthorized Access!"
        );
        require(
            orders[msg.sender].orders[_orderId].status == OrderStatus.PENDING,
            "Order Can not be cancelled"
        );

        (bool isPaid, ) = payable(msg.sender).call{value: orderFee}("");
        require(isPaid, "Fee Transfer Failed!");

        Order storage _currentOrder = orders[msg.sender].orders[_orderId];
        _currentOrder.status = OrderStatus.CANCELLED;
        IERC20Upgradeable(_currentOrder.baseToken).transfer(
            msg.sender,
    ...
    function proceedOrder(address _usr, uint256 _orderId)
        external
        isAuthorized
    {
        ...
        require(
            IERC20Upgradeable(_currentOrder.baseToken).approve(
        ...
        // transfer order fee to the relayer
        (bool isPaid, ) = payable(_relayerPubKey).call{value: orderFee}("");
        require(isPaid, "Fee Transfer Failed!");

        address[] memory path = new address[](2);
        path[0] = _currentOrder.baseToken;
        path[1] = _currentOrder.quoteToken;
        ...
        )
    {
        ...
        return (
            _currentOrder.amountIn,
            _currentOrder.baseToken,
            _currentOrder.quoteToken,
    ...
    function removeStuckETH(address _recipient, uint256 _amount)
        external
        onlyOwner
    {
