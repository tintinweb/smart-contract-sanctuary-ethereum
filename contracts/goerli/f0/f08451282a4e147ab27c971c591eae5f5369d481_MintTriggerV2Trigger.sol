...
// File: contracts/MintTriggerV2.sol
...
abstract contract GenesisRocket
    {
    function minterMint(address to) external virtual;
    function checkMints(address a) external virtual returns(uint256); 
    }



contract MintTriggerV2Trigger is ERC2771Context, Ownable, ReentrancyGuard {
    ...
    function  MintTriggerSig(address userWallet, bytes32 hash, bytes memory signature, bytes32[] calldata _merkleProof) external nonReentrant {
       ...
       // Rocket Mint Trigger Signature edition
    function  MintTrigger() external nonReentrant {
        require(mintIsOpen, "Mint is not open right now");
        require(rocket.checkMints(_msgSender()) < 1 , "You already minted a rocket!");
        // Call Rocket Production contract
        rocket.minterMint(_msgSender());
   ...
   //Set Signer
    ...
    //Set Merkleroot
    function setMerkleRoot(bytes32 newRoot) external onlyOwnerOrAdmin{
        merkleroot = newRoot;
    }
