...
contract NFTStaking is Ownable, IERC721Receiver {
    ...
    constructor(BabyApe _nft, BASRewards _token) {
        nft = _nft;
        token = _token;
    }
    ...
    PaymentSplitter 
{
    ...
    string public notRevealedUri = "https://ipfs.io/ipfs/QmUNNr7yGkZynBpuKTZf3rqQtsEDHJHxeQwrdm5gaNKKY1?filename=notRevealed.json";
    ...
    onlyAccounts
    {
        require(msg.sender == account,"BabyApe: Not allowed");
        require(presaleM,"BabyApe: Presale is OFF");
        require(!paused,"BabyApe: Contract is paused");
        require(_amount <= presaleAmountLimit,"BabyApe: You can't mint so much tokens");
        require(_presaleClaimed[msg.sender] + _amount <= presaleAmountLimit,"BabyApe: You can't mint so much tokens");


        uint current = _tokenIds.current();

        require(current + _amount <= maxSupply,"BabyApe: max supply exceeded"
        );
        require(_price * _amount <= msg.value,"BabyApe: Not enough ethers sent"
        );
