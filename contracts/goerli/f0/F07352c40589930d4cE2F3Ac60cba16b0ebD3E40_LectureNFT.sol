...
contract LectureNFT is ERC721, Ownable{
    ...
    function mintPlanet(uint256 count) external payable{
        ...
        require(msg.value >= 1000000000000000 * count); //0.001 ETH
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenMetadataNo[tokenId].toString())) : "";
    }
