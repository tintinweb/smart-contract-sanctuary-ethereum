...
contract PH is ERC721A, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
    uint256 public MAX_SUPPLY = 5;

    
    uint256 public LAST_SUPPLY = 5;
    
    bool public IS_LAST_ACTIVE = false;

    
    uint256 public LAST_TX_LIMIT = 1;

    
    uint256 public LAST_WALLET_LIMIT = 1;

    
    uint256 public LAST_PRICE = 0.0001 ether;

    
    uint256 public LAST_COUNT = 0;

    

    
    mapping(address => uint) public LAST_Minted;

    bool public _revealed = false;

    string private baseURI = "";
    string private preRevealURI = "https://ipfs.w3bmint.xyz/ipfs/QmczvZdUp2uydtpErtDu6zoim3Ftua88UPsGhF2kmKu5qp";

    mapping(address => uint256) addressBlockBought;

    address public constant RL_ADDRESS = 0xc9b5553910bA47719e0202fF9F617B8BE06b3A09; 

    constructor() ERC721A("PH", "PH") {
        
        
        _safeMint(0x2e955055B81Ab6FC838c2222e5d1e61169ECCd87, 1);
    }
    ...
    function LASTMint(uint256 numberOfTokens) external isSecured(LAST_PRICE * numberOfTokens) payable {
        validateMint(IS_LAST_ACTIVE, LAST_COUNT, LAST_SUPPLY, LAST_Minted[msg.sender], LAST_WALLET_LIMIT, LAST_TX_LIMIT, numberOfTokens, true);
        addressBlockBought[msg.sender] = block.timestamp;
        LAST_Minted[msg.sender] += numberOfTokens;
        LAST_COUNT += numberOfTokens;
    ...
    function toggleLASTMintStatus() external onlyOwner {
        IS_LAST_ACTIVE = !IS_LAST_ACTIVE;
    }

    // SUPPLY CHANGERS
    
    function setLASTSupply(uint256 _supply) external onlyOwner {
        LAST_SUPPLY = _supply;
    }
    ...
    function setLASTPrice(uint256 _price) external onlyOwner {
        LAST_PRICE = _price;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        (bool successFinal, ) = payable(0x2e955055B81Ab6FC838c2222e5d1e61169ECCd87).call{ value: address(this).balance }("");
