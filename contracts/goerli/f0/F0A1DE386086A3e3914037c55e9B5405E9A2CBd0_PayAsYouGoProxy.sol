...
contract PayAsYouGo is ReentrancyGuardUpgradeable, IAnkrProtocol, ITransportLayer {
    ...
    mapping(address => uint64) private _requestNonce;
    uint256 private _collectedFee;

    function initialize(IChainConfig chainConfig, IERC20Upgradeable ankrToken) external initializer {
        __ReentrancyGuard_init();
        __AnkrProtocol_init(chainConfig, ankrToken);
    }

    function __AnkrProtocol_init(IChainConfig chainConfig, IERC20Upgradeable ankrToken) internal {
        _ankrToken = ankrToken;
        _chainConfig = chainConfig;
    }

    modifier onlyFromGovernance() {
        require(msg.sender == address(_chainConfig.getGovernanceAddress()), "AnkrProtocol: only governance");
        _;
    }

    modifier onlyFromConsensus() {
        require(msg.sender == address(_chainConfig.getConsensusAddress()), "AnkrProtocol: only consensus");
        _;
    }

    modifier onlyTreasury() {
        require(msg.sender == address(_chainConfig.getTreasuryAddress()), "AnkrProtocol: only treasury");
        _;
    }
    ...
    function deposit(uint256 amount, uint64 timeout, bytes32 publicKey) external nonReentrant override {
        require(amount % BALANCE_COMPACT_PRECISION == 0, "AnkrProtocol: remainder is not allowed");
    ...
    function _lockDeposit(address sender, uint256 amount, uint64 timeout, bytes32 publicKey) internal {
        if (amount > 0) {
            require(_ankrToken.transferFrom(msg.sender, address(this), amount), "AnkrProtocol: can't transfer");
        }
    ...
    function _createWithdrawal(address sender, uint256 amount) internal {
        ...
        UserBalance memory userDeposit = _userDeposits[sender];
        require(userDeposit.available >= amount80, "AnkrProtocol: insufficient balance");
    ...
    function handleChargeFee(address[] calldata users, uint256[] calldata fees) external onlyFromConsensus override {
    ...
    function handleWithdraw(address[] calldata users, uint256[] calldata amounts, uint256[] calldata fees) external onlyFromConsensus override {
        require(users.length == amounts.length && amounts.length == fees.length, "AnkrProtocol: corrupted data");
        for (uint256 i = 0; i < users.length; i++) {
    ...
    function _doWithdraw(address user, uint256 amount, uint256 fee) internal {
        ...
        require(_ankrToken.transfer(user, amount), "AnkrProtocol: can't transfer");
        // emit event
...
import "./PayAsYouGo.sol";

contract PayAsYouGoProxy is ManageableProxy {

    constructor(IChainConfig chainConfig, IERC20Upgradeable ankrToken) ManageableProxy(
        chainConfig, _deployDefault(),
        abi.encodeWithSelector(PayAsYouGo.initialize.selector, chainConfig, ankrToken)
    ) {
    }

    function _deployDefault() internal returns (address) {
        PayAsYouGo impl = new PayAsYouGo{
        salt : keccak256("PayAsYouGoV0")
        }();
        return address(impl);
    }
}