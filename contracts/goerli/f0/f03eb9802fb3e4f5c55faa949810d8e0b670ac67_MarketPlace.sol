...
contract MarketPlace is Initializable, OwnableUpgradeable {
    ...
    function lazyBuy(SellerDetails calldata sellerDetails) external {
        ...
        // validate seller
        address signer = verifySellerSign(sellerDetails);
        ...
        uint256 remaining_amount = sellerDetails.amount;
        // Transfer royalty to the creator
        if (royaltyAmount > 0) {
            if (sellerDetails.seller != receiver) {
                instanceERC20.transferFrom(msg.sender, receiver, royaltyAmount);
