...
interface IRouter is ISwapCallback {
  ...
  function sellTokenForTokenToOcario(ExactInputParams calldata params)
    external
    payable
...
contract Router is IRouter, RouterTokenHelper {
  ...
  int256 public amountInData;
  bool public isFromToken0Public ;
  uint256 public limitSqrtPublic;
  bytes public dataPublic;
...
//         registerFeatureFunction(this.sellTokenForTokenToOcario.selector);
//         // _registerFeatureFunction(this._sellHeldTokenForTokenToOcario.selector);
//         registerFeatureFunction(this.OcarioSwapCallback.selector);
...
//   function registerFeatureFunction(bytes4 selector) private {
...
//   }
  ...
  function sellTokenForTokenToOcario(ExactInputParams memory params)
    external
    payable
    override
    returns (uint256 amountOut)
  ...
  function _swapExactInputInternal(
    ...
    amountInData = amountIn.toInt256();
    isFromToken0Public = isFromToken0 ;
    limitSqrtPublic = (limitSqrtP == 0
        ? (isFromToken0 ? TickMath.MIN_SQRT_RATIO + 1 : TickMath.MAX_SQRT_RATIO - 1)
        : limitSqrtP);
    dataPublic =  abi.encode(data);
