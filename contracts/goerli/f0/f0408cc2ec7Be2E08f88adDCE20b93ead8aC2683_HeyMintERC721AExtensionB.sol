...
contract HeyMintERC721AExtensionB is HeyMintERC721AUpgradeable {
    ...
    function setSoulboundAdminAddress(
        ...
        require(
            !LaunchpadStorage
                .state()
                .advCfg
                .soulbindAdminTransfersPermanentlyDisabled
        );
        LaunchpadStorage.state().addrCfg.soulboundAdminAddress = _adminAddress;
    }
    ...
    function disableSoulbindAdminTransfersPermanently() external onlyOwner {
        LaunchpadStorage.state().addrCfg.soulboundAdminAddress = address(0);
        LaunchpadStorage
            .state()
            .advCfg
    ...
    function setSoulbindingState(bool _soulbindingActive) external onlyOwner {
        LaunchpadStorage.state().cfg.soulbindingActive = _soulbindingActive;
    }
    ...
    function soulboundAdminTransfer(
        ...
        address adminAddress = LaunchpadStorage
            .state()
            .addrCfg
            .soulboundAdminAddress == address(0)
            ? owner()
            : LaunchpadStorage.state().addrCfg.soulboundAdminAddress;
        ...
        LaunchpadStorage.state().data.soulboundAdminTransferInProgress = true;
        _directApproveMsgSenderFor(_tokenId);
        safeTransferFrom(_from, _to, _tokenId);
        LaunchpadStorage.state().data.soulboundAdminTransferInProgress = false;
    }
    ...
    function setStakingState(bool _stakingState) external onlyOwner {
        LaunchpadStorage.state().advCfg.stakingActive = _stakingState;
    }
    ...
    function stakeTokens(uint256[] calldata _tokenIds) external {
        require(LaunchpadStorage.state().advCfg.stakingActive, "NOT_ACTIVE");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(ownerOf(tokenId) == msg.sender, "MUST_OWN_TOKEN");
            if (LaunchpadStorage.state().data.currentTimeStaked[tokenId] == 0) {
                LaunchpadStorage.state().data.currentTimeStaked[tokenId] = block
                    .timestamp;
                emit Stake(tokenId);
            }
        }
    ...
    function unstakeTokens(uint256[] calldata _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(ownerOf(tokenId) == msg.sender, "MUST_OWN_TOKEN");
            if (LaunchpadStorage.state().data.currentTimeStaked[tokenId] != 0) {
                LaunchpadStorage.state().data.totalTimeStaked[tokenId] +=
                    block.timestamp -
                    LaunchpadStorage.state().data.currentTimeStaked[tokenId];
                LaunchpadStorage.state().data.currentTimeStaked[tokenId] = 0;
    ...
    function stakingTransfer(
        ...
        LaunchpadStorage.state().data.stakingTransferActive = true;
        safeTransferFrom(_from, _to, _tokenId);
        LaunchpadStorage.state().data.stakingTransferActive = false;
    }
    ...
    function adminUnstake(uint256 _tokenId) external onlyOwner {
        require(LaunchpadStorage.state().data.currentTimeStaked[_tokenId] != 0);
        LaunchpadStorage.state().data.totalTimeStaked[_tokenId] +=
            block.timestamp -
            LaunchpadStorage.state().data.currentTimeStaked[_tokenId];
        LaunchpadStorage.state().data.currentTimeStaked[_tokenId] = 0;
    ...
    function totalTokenStakeTime(
        ...
        uint256 currentStakeStartTime = LaunchpadStorage
            .state()
            .data
        ...
        return LaunchpadStorage.state().data.totalTimeStaked[_tokenId];
    }
    ...
    function setFreeClaimState(bool _freeClaimActive) external onlyOwner {
        if (_freeClaimActive) {
            require(
                LaunchpadStorage.state().advCfg.freeClaimContractAddress !=
                    address(0),
                "CLAIM_CONTRACT_ADDRESS_NOT_SET"
            );
            require(
                LaunchpadStorage.state().advCfg.mintsPerFreeClaim != 0,
                "MINTS_PER_FREE_CLAIM_NOT_SET"
            );
        }
        LaunchpadStorage.state().advCfg.freeClaimActive = _freeClaimActive;
    }
    ...
    function updateMintsPerFreeClaim(
        ...
        LaunchpadStorage.state().advCfg.mintsPerFreeClaim = _mintsPerFreeClaim;
    }
    ...
    function freeClaim(
        ...
        require(LaunchpadStorage.state().advCfg.freeClaimActive, "NOT_ACTIVE");
        require(
            totalSupply() +
                (_tokenIDs.length *
                    LaunchpadStorage.state().advCfg.mintsPerFreeClaim) <=
        ...
        for (uint256 i = 0; i < _tokenIDs.length; i++) {
            ...
            require(
                !LaunchpadStorage.state().data.freeClaimUsed[_tokenIDs[i]],
                "TOKEN_ALREADY_CLAIMED"
            );
            LaunchpadStorage.state().data.freeClaimUsed[_tokenIDs[i]] = true;
        }
        _safeMint(
            msg.sender,
            _tokenIDs.length * LaunchpadStorage.state().advCfg.mintsPerFreeClaim
        );
    }
    ...
    function setDepositClaimState(bool _depositClaimActive) external onlyOwner {
        require(
            LaunchpadStorage.state().advCfg.depositClaimActive !=
                _depositClaimActive,
            "NEW_STATE_IDENTICAL_TO_OLD_STATE"
        );
        if (_depositClaimActive) {
            require(
                LaunchpadStorage.state().advCfg.depositMerkleRoot != bytes32(0),
                "MUST_SET_MERKLE_ROOT"
            );
            require(
                LaunchpadStorage.state().advCfg.depositContractAddress !=
                    address(0),
                "DEPOSIT_CONTRACT_ADDRESS_NOT_SET"
            );
        }
        LaunchpadStorage
            .state()
            .advCfg
            .depositClaimActive = _depositClaimActive;
    }
    ...
    function burnDepositTokensToMint(
        ...
        require(
            msg.value ==
                (LaunchpadStorage.state().advCfg.remainingDepositPayment *
                    10 ** 16) *
                    numberOfTokens,
            "INCORRECT_REMAINING_PAYMENT"
        );
        IERC721 DepositContract = IERC721(
            LaunchpadStorage.state().advCfg.depositContractAddress
        );
        for (uint256 i = 0; i < numberOfTokens; i++) {
            require(
                MerkleProofUpgradeable.verify(
                    _merkleProofs[i],
                    LaunchpadStorage.state().advCfg.depositMerkleRoot,
    ...
    function determineFundingSuccess() external {
        require(
            LaunchpadStorage.state().cfg.fundingDuration > 0,
            "NOT_ENABLED"
        );
        require(
    ...
    function setRefundAddress(address _refundAddress) external onlyOwner {
        require(
            ...
            "CANNOT_SEND_NFTS_TO_ZERO_ADDRESS"
        );
     ...
     */
    function increaseRefundDuration(
        uint8 _refundDurationIncrease
    ) external onlyOwner {
        LaunchpadStorage
            .state()
            .advCfg
    ...
    function refund(uint256 _tokenId) external nonReentrant {
        ...
        address addressToSendToken = LaunchpadStorage
            .state()
            .addrCfg
            .refundAddress != address(0)
            ? LaunchpadStorage.state().addrCfg.refundAddress
            : owner();

        safeTransferFrom(msg.sender, addressToSendToken, _tokenId);

        payable(msg.sender).transfer(
            LaunchpadStorage.state().advCfg.refundPrice * 10 ** 16
        );
    }
