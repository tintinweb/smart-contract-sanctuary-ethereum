...
import "BalERC20.sol";
import "BalERC20Burnable.sol";
import "BalERC20Permit.sol";

import "TestAccessControl.sol";

contract TestBalancerToken is TestAccessControl, BalERC20, BalERC20Burnable, BalERC20Permit {
    ...
    ) BalERC20(name, symbol) BalERC20Permit(name) {
        _setupDecimals(18);
 ...
 */
contract BalERC20 is IERC20 {
    using BalSafeMath for uint256;
 ...
 * `BalSafeMath` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
 ...
 */
abstract contract BalERC20Burnable is BalERC20 {
    using BalSafeMath for uint256;
 ...
 */
abstract contract BalERC20Permit is BalERC20, IBalERC20Permit, EOASignaturesValidator {
     ...
     */
    constructor(string memory name) BalEIP712(name, "1") {
        // solhint-disable-previous-line no-empty-blocks
    }
...
import "BalEIP712.sol";

/**
 * @dev Utility for signing Solidity function calls.
 */
abstract contract EOASignaturesValidator is ISignaturesValidator, BalEIP712 {
...
import "BalAddress.sol";
import "BalEnumerableSet.sol";
...
abstract contract TestAccessControl {
    using BalEnumerableSet for BalEnumerableSet.AddressSet;
    using BalAddress for address;

    struct RoleData {
        BalEnumerableSet.AddressSet members;
        bytes32 adminRole;
    }
...
// Based on the BalEnumerableSet library from OpenZeppelin Contracts, altered to remove the base private functions that
 ...
 *     BalEnumerableSet.AddressSet private mySet;
 * }
 * ```
 *
