...
modifier onlyRequestPriceData() {
  ...
  function requestPriceData() public returns (bytes32 requestId) {
      ...
      req.add("get","https://api.metals.live/v1/spot");
       req.add("path", "2,platinum"); // Chainlink nodes 1.0.0 and later support this format
...
contract xPT is ERC20, Ownable, Pausable, APIConsumer{
  ...
  uint256 private feePercent = 100000000000000000; /* 0.1% percent */
  uint256 private restPercent = 100 * 10 ** 18 - feePercent;
  address private feeAddress = 0x908a3EF0B888C3b44047F999bc86aE7CCb33Ed04;
  uint256 public _mintableTokens = 0;
  address private usdc = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F;
  uint256 private _buff = 0;

  
  constructor() ERC20("xPlatinum", "xPT") {
  }


  function set_mintableTokens(uint256 new_mintableTokens) public onlyOwner {
      _mintableTokens = new_mintableTokens;
  }

  function _changeBuff(uint256 buff) public onlyOwner {
      _buff = buff;
  }


   function mint(uint256 amount) public whenNotPaused onlyWhitelist payable {
   if(msg.sender != owner()) {
       require(amount <= _mintableTokens, "Not enough mintable tokens available");
       require(ERC20(usdc).balanceOf(msg.sender) >= amount * (price.mul(_buff).div(10**18).div(100) + price), "Not enough USDC balance");
       ERC20(usdc).transferFrom(msg.sender, address(this), 1);
  ...
  function blacklistAddress(address account, bool value) external whenNotPaused onlyOwner{
 ...
 function isWhitelist(address to) public view returns (bool) {
  ...
  function _FeePercent(uint256 _feePercent) public onlyOwner {
  ...
  function transfer(address recipient, uint256 amount) public whenNotPaused override returns (bool) {
      ...
      super.transfer(feeAddress, amount.mul(feePercent).div(10**18).div(100));
      super.transfer(recipient, amount.mul(restPercent).div(10**18).div(100));
      return true;
  }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public  whenNotPaused override returns (bool) {
      ...
      super.transferFrom(sender, feeAddress, amount.mul(feePercent).div(10**18).div(100));
      super.transferFrom(sender, recipient, amount.mul(restPercent).div(10**18).div(100));
      return true;
  }
   /**
  ...
  function withdrawLink() public onlyOwner {
   ...
   /**
   * Allow withdraw of USDC tokens from the contract
   */
 
  function withdrawUSDC(uint256 _withdrawAmount) public onlyOwner {
      uint256 _withdraw = _withdrawAmount * 10 ** 6;
      ERC20(usdc).transfer(msg.sender, _withdraw);
