...
// File contracts/RAMIcePreBuy.sol


pragma solidity ^0.8.9;

contract RAMIcePreBuy is Ownable {

    address public _RAMIce;
    ...
    uint256 public _preBuyPrice = 0.05 ether;
    ...
    function checkPreBuyStopped() private view {
        require(!_preBuyStopped, "RAMIcePreBuy: pre buy stopped");
    }

    /**
     * @dev This method is used to set RAMIce contract address.
     * @param _ramContract RAMIce contract address
     */
    function configurePreBuy(address _ramContract) external onlyOwner {
        _RAMIce = _ramContract;
    }
     ...
     * This method is only accessible by RAMIce contract.
     * @param _batchNum pre buy batch number
     * @param _user user's address
     */
    function setUserMintStatus(uint8 _batchNum, address _user) external {
        require(msg.sender == _RAMIce, "RAMIcePreBuy: caller is not RAMIce");
    ...
    function registerForPreBuy(uint8 _num) payable external {
        checkPreBuyStopped();
        require(_preBuyUsers[_totalPreBuyBatch-1]._users[msg.sender] + _num <= 3, "RAMIcePreBuy: max 3 nfts allowed");
        require((_preBuyPrice * _num) == msg.value, "RAMIcePreBuy: invalid price");
        if(_preBuyUsers[_totalPreBuyBatch-1]._users[msg.sender] == 0) {
            require(_preBuyUsers[_totalPreBuyBatch-1]._usersCount < _preBuyUsers[_totalPreBuyBatch-1]._batchSize, "RAMIcePreBuy: max users limit reached");
    ...
    function createNewPreBuyBatch(uint8 _batchSize) external onlyOwner {
        ...
        if(_totalPreBuyBatch > 0) {
            require(_preBuyUsers[_totalPreBuyBatch-1]._usersCount == _preBuyUsers[_totalPreBuyBatch-1]._batchSize, "RAMIcePreBuy: previous batch is not ended");
        }
