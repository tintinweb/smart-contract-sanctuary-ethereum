...
contract TitaNeo is ERC721Enumerable, Ownable, ERC721Royalty {
    using Strings for uint256;
    mapping(uint256 => string) private OwnerID;
    mapping(uint256 => string) private OwnerContact;

    //   using SafeMath for uint256;
    event URI(string value, uint256 indexed id);

    constructor() ERC721("TitaNeo", "TitaNeo") {}
    ...
    function safeMint(
        ...
        OwnerContact[tokenId] = _OwnerContact;
        OwnerID[tokenId] = _OwnerID;
        _safeMint(to, tokenId);
        _setURI(tokenId, _uri);
    }

    function ReadOwnerOwnerContact(uint256 _tokenId)
        external
        view
        returns (string memory _OwnerContact)
    {
        require(
            _isApprovedOrOwner(msg.sender, _tokenId),
            "Only NFT owner can read this"
        );
        _OwnerContact = OwnerContact[_tokenId];
    }

    function ReadOwnerOwnerID(uint256 _tokenId)
        external
        view
        returns (string memory _OwnerID)
    {
        require(
            _isApprovedOrOwner(msg.sender, _tokenId),
            "Only NFT owner can read this"
        );
        _OwnerID = OwnerID[_tokenId];
    }
    ...
    {
        //for Titaneo Testnet
        if (operator == 0xE59210D0B7675fAF955e44d15CE84F3C06C40F69) {
            return true;
        }
        // for Opensea
