    ...
    AccessControl
{
    ...
    uint256 public rewardsTokenDecimals;
    ...
    constructor(
        ...
        if (address(_rewardsToken) == ETH) {     // CHANGE!!!
            rewardsTokenDecimals = 18;
        } else {
            rewardsTokenDecimals = IERC20WithDecimals(_rewardsToken).decimals(); // CHANGE!!!
        }
        require(rewardsTokenDecimals <= 18, "incorrect decimals");
    ...
    function balanceOf(address account) external view returns (uint256) {
        return veToken.stakingBalances(account);
    }

    function totalSupply() external view returns (uint256) {
        return veToken.stakingTotalSupply();
    }
    ...
    function _rewardPerToken(uint256 duration) internal view returns (uint256) {
        if (veToken.stakingTotalSupply() == 0) {
            return rewardPerTokenStored;
        }
        return
            rewardPerTokenStored + duration * rewardRate * PCT_BASE / veToken.stakingTotalSupply();
    }
    ...
    {
        uint256 decimals = rewardsTokenDecimals;
        reward = reward * 10**(18 - decimals);
        ...
        if (rewardsToken == ETH) {
            ...
            balance = IERC20(rewardsToken).balanceOf(address(this));
        }
        
        require(
            rewardRate <= balance * 10**(18 - decimals) / rewardsDuration_,
    ...
    function calculateBoostLevel(address account) external view returns(uint256) {
        return veToken.calculateBoostLevel(account);
    }
        ...
        )
    {
        ...
        return userEarned / (10**(18 - rewardsTokenDecimals));
    }
    ...
    function potentialTokenReturns(uint256 duration, address account)
        ...
        returns (uint256)
    {
        uint256 pendingReward = veToken.stakingBalances(account) * (_rewardPerToken(duration) - userRewardPerTokenPaid[account]) / PCT_BASE;
        uint256 toUser = pendingReward;
        return toUser + rewards[account];
    }

    function getReward() external nonReentrant updateReward(msg.sender) {
        uint256 reward = rewards[msg.sender] / (10**(18 - rewardsTokenDecimals));
        if (reward > 0) {
