 ...
 *  @title Trade
 */
contract Liquidation is Events {
    /**
     *  @notice Triggers liquidation for given account
     */
    function _liquidate(address account) public {
        // TODO Implement maintenance margin checks and liquidation
...
contract Synthetic is Fees, Storage, Token {
            ...
            if (previousBalance.sign == updatedBalance.sign || previousBalance.amount.value == 0) {
                ...
                if (Math.isLessThan(updatedBalance.amount, previousBalance.amount)) {
                    ...
                    updatedBalance.cost = Math.mul(
                        previousBalance.cost,
                        Math.add(previousBalance.amount, matchSize)
                    );
   ...
   /**
    ...
    function _calculateUnrealizedPnl(
        ...
        // -previous_cost
        Math.Number memory previousBalanceNeg = Math.create(previousBalance.cost, false);

        // -previous_asset_balance.cost + asset_balance.cost
        Math.Number memory balanceDiff = Math.add(previousBalanceNeg, updatedBalance.cost);

        // trade_amount * fill_price
        Math.Number memory matchValue = Math.mul(matchPrice, matchSize);

        // unrealized_pnl = -previous_asset_balance.cost + asset_balance.cost - (trade_amount * fill_price)
        Math.Number memory unrealizedPnl = Math.sub(balanceDiff, matchValue);
...
contract Fees {
    ...
    Math.Number maxFeePct = makerFeePct;
}
