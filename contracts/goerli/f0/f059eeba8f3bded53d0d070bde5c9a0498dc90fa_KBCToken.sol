...
contract KBCToken is ERC721("KUMABondCloneToken", "KBC"), IKBCToken {
    using Counters for Counters.Counter;

    IKIBTAddressProvider public immutable KIBTAddressProvider;
    ...
    modifier onlyKUMASwap(uint256 parentId) {
        bytes32 riskCategory = IKUMABondToken(KIBTAddressProvider.getKUMABondToken()).getBond(parentId).riskCategory;
        if (msg.sender != KIBTAddressProvider.getKUMASwap(riskCategory)) {
            revert Errors.CALLER_NOT_KUMASWAP();
        }
        _;
    }

    constructor(IKIBTAddressProvider _KIBTAddressProvider) {
