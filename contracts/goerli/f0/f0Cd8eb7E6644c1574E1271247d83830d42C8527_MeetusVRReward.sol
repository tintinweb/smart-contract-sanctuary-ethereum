...
contract MeetusVRReward is AccessControl, ERC721Holder, ERC1155Holder, Rescueable {
    ...
    bytes32 public constant TOKEN_CLAIM_TYPEHASH =
        keccak256(
            "TokenClaim(address user,uint256 nonce,uint8 tokenStandard,address token,uint256 tokenId,uint256 amount,uint8 claimMethod,uint256 signatureTimestamp,uint256 deadline)"
        );
    ...
    modifier notBlacklisted() {
        require(!blacklist[msg.sender], "MeetusVRReward: blacklisted sender");
        _;
    }
    ...
    constructor(uint256 _claimDelay, uint256 _minSigner, uint256 _maxClaim) {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("MeetusVRReward")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
    ...
    function _cancelNonce(uint256 nonce, address account) internal {
        require(!claimed[account][nonce], "MeetusVRReward: Nonce already claimed");
    ...
    function claim(TokenClaimData calldata _claimData) public notBlacklisted {
        require(!emergencyLock, "MeetusVRReward: Contract is locked");
        require(_claimData.signatureTimestamp + claimDelay <= block.timestamp, "MeetusVRReward: Claim too soon!");
        require(_claimData.deadline >= block.timestamp, "MeetusVRReward: Claim expired");
        require(!cancelled[msg.sender][_claimData.nonce], "MeetusVRReward: Nonce cancelled");
        require(!claimed[msg.sender][_claimData.nonce], "MeetusVRReward: Nonce claimed");

        _validateSignatures(_claimData);
        _claimTokens(_claimData);
    ...
    function _claimTokens(TokenClaimData calldata _claimData) internal {
        if (_claimData.claimMethod == ClaimMethod.Mint) {
            if (_claimData.tokenStandard == TokenStandard.ERC721) {
                if (_claimData.token == address(buildingNFT)) buildingNFT.mint(msg.sender, _claimData.tokenId);
                else if (_claimData.token == address(roomNFT)) roomNFT.mint(msg.sender, _claimData.tokenId);
            }
        } else {
            if (TokenStandard(_claimData.tokenStandard) == TokenStandard.ERC20) {
                require(maxClaim >= _claimData.amount, "MeetusVRReward: claim amount too high!");
                IERC20 token = IERC20(_claimData.token);
                token.safeTransfer(msg.sender, _claimData.amount);
            } else if (_claimData.tokenStandard == TokenStandard.ERC721) {
    ...
    function _validateSignatures(TokenClaimData calldata _claimData) internal view {
        ...
        _checkSinger(digest, _claimData.signatures);
    }

    function _checkSinger(bytes32 digest, bytes[] calldata _signatures) private view {
        uint256 validSignerCount = 0;
        address[] memory signers = new address[](_signatures.length);

        for (uint256 x = 0; x < _signatures.length; x++) {
            address signer = digest.recover(_signatures[x]);

            if (hasRole(CLAIM_SIGNER_ROLE, signer)) {
                for (uint256 y = 0; y < validSignerCount; y++) {
                    require(signers[y] != signer, "MeetusVRReward: Duplicate signature");
                }
                signers[validSignerCount++] = signer;
            }
        }
        require(validSignerCount != 0, "MeetusVRReward: Invalid Signatures");
        require(minValidSignersCount <= validSignerCount, "MeetusVRReward: Not enough valid signatures");
    }
    ...
    function updateEmergencyLock(bool status) external {
        require(
            ...
            "MeetusVRReward: You don't have EMERGENCY_LOCK_ROLE"
        );
