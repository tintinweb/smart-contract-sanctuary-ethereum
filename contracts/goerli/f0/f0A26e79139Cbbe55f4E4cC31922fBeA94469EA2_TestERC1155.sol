...
contract TestERC1155 is ERC1155, Ownable {
    ...
    uint256 public MAX_AMOUNT_PER_TX = 2;
        ...
        onlyPaper(keccak256(abi.encode(toAddress, amount)), _nonce, _signature)
    {
        require(
            ...
            "Max token number per transaction exceeded"
        );
    ...
    function checkPreSaleClaimEligibility(uint256 amount)
        external
        view
        returns (string memory)
    {
        if (amount > MAX_AMOUNT_PER_TX) {
            return "Max token number per transaction exceeded";
        }
