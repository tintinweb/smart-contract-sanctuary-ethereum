...
contract AjiraPayFinanceStaking is Ownable, AccessControl, ReentrancyGuard{
    ...
    function withdrawSingleStake(uint256 _stakingId) external canClaimRewards(msg.sender) hasNotClaimedStakeId(msg.sender, _stakingId) nonReentrant{
        ...
        uint256 _totalStakePeriodDiff = _stakeItem.stakedAt.sub(block.timestamp);
        //uint256 timeDiff = _stakeItem.stakeDuration.sub(_stakeItem.stakedAt);
        uint256 amount = _stakeItem.stakingAmount;
        uint256 totalAccumulatedReward = amount.mul(stakeApr).mul(_totalStakePeriodDiff).div(_stakeItem.stakeDuration);
    ...
    function addStakingTier(uint56 _apr, uint256 _deadlineInDays) external onlyRole(MANAGER_ROLE) nonReentrant{
        ...
        stakingTierById[currentTierId] = StakeTier(currentTierId, _apr, deadlineInDays, block.timestamp, msg.sender);
