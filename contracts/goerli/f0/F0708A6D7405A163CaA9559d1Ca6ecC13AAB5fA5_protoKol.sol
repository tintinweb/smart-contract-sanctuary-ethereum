...
contract protoKol is iProtoKol {
    ...
    //IN Matic
    address private UNISWAP_ROUTER_ADDRESS = 0x8954AfA98594b838bda56FE4C12a09D7739D179b;
    // address private constant FACTORY = 0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32;

    // In local
    //IN Matic
    // address private UNISWAP_ROUTER_ADDRESS = 0xa89cbe6F2Bcd58Bbbb577CC269238F13C1a0a8c0;
    ...
    constructor(
        ...
        require(_admin != address(0) && _oracle != address(0) && _usdt != address(0) && _staking != address(0), "ZA");
   ...
   */
    function generateTGE(
        ...
        uint256 presaleDecimals = IERC20(presaleToken).decimals();
        require(presaleToken != address(0x0), "Protokol:update PST first");
        ...
        _safeTransferFrom(presaleToken, msg.sender, _stakingContract, (staking * 10**presaleDecimals) / 10**18);

        _safeTransferFrom(
            presaleToken,
            msg.sender,
            _vault,
            (((markBudge * 10**presaleDecimals) / 10**18) * _platformPer) / 10000
        );
        ...
        _safeTransferFrom(
            ...
            msg.sender,
            address(this),
            (campaigns[_campaignId].tgeDetails.tgeAmount * 10**presaleDecimals) / 10**18
        );
   ...
   */
    function claimPreSaleTokens(
        ...
        if (blackListedKOL[msg.sender][_campaignId] || timePassed) {
   ...
   */
    function claimBackInvestmentByKOL(
        ...
        // require(blackListedKOL[msg.sender][_campaignId] != true, "Protokol:KOL blacklisted"); // #change3
   ...
   */
        ...
        onlyAdmin
    {
        if (!wantToWithdraw) {
            ...
            if (kolToken != address(0)) {
                ...
                UNISWAP_ROUTER_ADDRESS = _setNewAddresses[4];
            }
        } else {
        ...
        emit WithdrawPenaltyAndUpdateAddress(_setNewAddresses, penaltyAmount);
    }
    ...
    function withdawPresaleByCampaignOwner(uint256 _campaignId) external {
        ...
        require(!campaigns[_campaignId].campaignData.isPresaleWithdraw, "Owner Already withraw");

        campaigns[_campaignId].campaignData.isPresaleWithdraw = true;
        ...
        )
    {
        ...
        if (campaigns[_campaignId].vestingData.isVestingInEnabled) {
            ...
            //_vesting represents KOL reward
