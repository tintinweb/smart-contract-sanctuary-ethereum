...
contract GMITokenStorage is StorageBase, IGMITokenStorage {
    ...
    // GMI airdrops lastClaimNonce mappings are stored by airdropId
    mapping(bytes4 => mapping(address => uint256)) private airdropLastClaimNonce;

    mapping(address => uint256) private listingRewardsLastClaimNonce;
    ...
    {
        _lastClaimNonce = listingRewardsLastClaimNonce[_user];
    }
    ...
    function setListingRewardsLastClaimNonce(address _user, uint256 _lastClaimNonce)
        external
        requireOwner
    {
        listingRewardsLastClaimNonce[_user] = _lastClaimNonce;
    }
}
...
contract GMIToken is NonReentrant, GMITokenAutoProxy, IGMIToken {
    ...
    constructor(
        ...
        string memory _symbol,
        uint8 _decimals
    ) public GMITokenAutoProxy(_proxy, this, _owner) {
    ...
    function getAirdropLastClaimNonce(bytes4 airdropId, address _user)
        ...
        returns (uint256 _lastClaimNonce)
    {
        _lastClaimNonce = gmiTokenStorage.getAirdropLastClaimNonce(airdropId, _user);
    }
    ...
    function getListingRewardsLastClaimNonce(address _user)
        ...
        returns (uint256 _lastClaimNonce)
    {
        _lastClaimNonce = gmiTokenStorage.getListingRewardsLastClaimNonce(_user);
    }
    ...
    function mint(address recipient, uint256 amount) external onlyGMITokenOwnerOrFarming {
        ...
        IGMITokenGovernedProxy(proxy).emitTransfer(address(0x0), recipient, amount);
    }

    function burn(address recipient, uint256 amount) external onlyGMITokenOwner {
        _burn(recipient, amount);
        IGMITokenGovernedProxy(proxy).emitTransfer(recipient, address(0x0), amount);
    }
    ...
    function claimListingRewards(
        ...
        require(
            lastClaimNonce == gmiTokenStorage.getListingRewardsLastClaimNonce(recipient),
        ...
        gmiTokenStorage.setListingRewardsLastClaimNonce(recipient, claimNonce);
        // Mint claim amount to recipient
        ...
        // Emit Transfer event
        IGMITokenGovernedProxy(proxy).emitTransfer(address(0x0), recipient, claimAmount);
    }
    ...
    function claimAirdrop(
        ...
        // Make sure claim has not been processed yet
        require(
            lastClaimNonce == gmiTokenStorage.getAirdropLastClaimNonce(airdropId, recipient),
            'GMIToken: invalid lastClaimNonce value'
        );
        ...
        LibClaimAirdrop.validateClaim(
            ...
            claimAmountAirdrop, // Claim amount corresponding to GMI airdrop
            claimAmountReferral1, // Claim amount corresponding to first level of GMI airdrop referral rewards
            claimAmountReferral2, // Claim amount corresponding to second level of GMI airdrop referral rewards
            claimAmountReferral3, // Claim amount corresponding to third level of GMI airdrop referral rewards
            airdropId, // Airdrop campaign Id
            lastClaimNonce, // Recipient's last claim nonce
            claimNonce, // Recipient's current claim nonce
            airdropServiceSignature, // Claim signature from GMI airdrop service
            proxy, // Verifying contract address
            gmiTokenStorage.getAirdropService() // Airdrop service address
        );
        // Update recipient's last claim nonce to current claim nonce
        gmiTokenStorage.setAirdropLastClaimNonce(airdropId, recipient, claimNonce);
    ...
    function mintAirdropClaim(
        ...
        // Emit Transfer event
        IGMITokenGovernedProxy(proxy).emitTransfer(address(0x0), recipient, totalClaimAmount);
    }
}
...
library LibClaimListing {
    bytes32 constant LISTING_CLAIM_TYPEHASH =
        keccak256(
            'GMIClaim(address recipient,uint256 amount,uint256 lastClaimNonce,uint256 claimNonce)'
        );
    ...
    function hashClaim(
        ...
        return
            keccak256(
                abi.encode(LISTING_CLAIM_TYPEHASH, recipient, amount, lastClaimNonce, claimNonce)
            );
    }
...
library LibClaimAirdrop {
    bytes32 constant AIRDROP_CLAIM_TYPEHASH =
        keccak256(
            'GMIClaim(address recipient,uint256 claimAmountAirdrop,uint256 claimAmountReferral1,uint256 claimAmountReferral2,uint256 claimAmountReferral3,bytes4 airdropId,uint256 lastClaimNonce,uint256 claimNonce)'
        );
...
interface IGMITokenStorage {
    ...
    function setListingRewardsLastClaimNonce(address _user, uint256 _lastClaimNonce) external;
}
...
contract GMITokenAutoProxy is GovernedERC20 {
    constructor(address _proxy, IGovernedContract _impl, address _owner) public GovernedERC20(_proxy, _owner) {
        if (_proxy == address(0)) {
