...
contract RtlolPackFunctions is AccessControl, ReentrancyGuard {
    ...
    mapping(uint256 => address) public cardContracts;
    ...
    event PackOpened(address cardsContract, uint256 packTokenId);
    ...
    function openPacks(
        ...
        uint256[][] memory _amount,
        bytes[] memory sig,
        uint256[] memory cardsContractId,
        ...
        for (uint256 i = 0; i < _cardId.length; i++) {
            ...
            bytes memory _hashingString = abi.encode(_salt[i], _packId[i], cardsContractId[i], "|openPacks|");
        ...
        for (uint256 i = 0; i < _cardId.length; i++) {
            ...
            IRainiCards cardsContract = IRainiCards(cardContracts[cardsContractId[i]]);
            for (uint256 j = 0; j < _cardId[i].length; j++) {
                cardsContract.mint(
                    _msgSender(),
                    _cardId[i][j],
            ...
            emit PackOpened(address(cardsContract), _packId[i]);
        }
    }
