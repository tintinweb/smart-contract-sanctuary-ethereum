...
contract Layerr1155 is DefaultOperatorFilterer, Initializable, ERC1155, ERC2981 {
  ...
  function mint(uint _id, uint _amount) public payable {
    require(!isClaim[_id] && block.timestamp >= tokenSaleStarts[_id] && block.timestamp <= tokenSaleEnds[_id], "Sale is not active for this token");
  ...
  function batchMint(uint[] memory _ids, uint[] memory _amounts) public payable {
    ...
    for (uint i = 0; i < _ids.length;) {
      ...
      require(!isClaim[id] && block.timestamp >= tokenSaleStarts[id] && block.timestamp <= tokenSaleEnds[id], "This token is a claim");
