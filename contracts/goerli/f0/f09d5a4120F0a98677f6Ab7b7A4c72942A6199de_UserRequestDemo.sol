...
contract SxTRequest is Admin, String, Initializer, ChainlinkClient {
    ...
    function initialize(
        ...
        require(operator != ZERO_ADDRESS, "SxTRequest: Cannot set to Zero Address");
        require(chainlink != ZERO_ADDRESS, "SxTRequest: Cannot set to Zero Address");
    ...
    function setChainlinkOperator(address newOperator) external onlyAdmin {
        require(newOperator != ZERO_ADDRESS, "SxTRequest: Cannot set to Zero Address");
     ...
     * @param amount - Amount of the LINK tokens to transfer
     */
    function withdrawChainlink(address to, uint256 amount) external onlyAdmin {
    ...
    function requestQuery(address callerContract, string memory query, string memory resourceId)
        ...
        returns (bytes32)
    {
        require(ChainlinkTokenInterface(chainlinkTokenAddress()).transferFrom(msg.sender, address(this), FEE), "SxTRequest: Insufficient LINK tokens");
