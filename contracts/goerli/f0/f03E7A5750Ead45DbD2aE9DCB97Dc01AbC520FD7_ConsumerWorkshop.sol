...
contract ConsumerWorkshop is ChainlinkClient {
    ...
    function requestAfter(
        ...
        Market _market,
        uint40 _timestamp,
        uint256[] calldata _statusIds,
        bytes32[] calldata _gameIds
    ) external {
        Chainlink.Request memory req = buildOperatorRequest(_specId, this.fulfillGames.selector);

        req.addUint("sportId", _sportId);
        req.addUint("market", uint256(_market));
        req.addUint("timestamp", _timestamp);
        _addUintArray(req, "statusIds", _statusIds); // Custom method
        req.addBytes("gameIds", abi.encode(_gameIds));
