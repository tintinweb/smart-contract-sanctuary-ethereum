...
/**
   ...
   @author Vasi 
   @notice Lock up funds for a period of time
...
contract Accountability is ReentrancyGuard {
    ...
    struct LockedFunds {
        uint256 amount; // The amount of funds locked up for
        uint256 time; // The time the funds were locked up for
    ...
    function lockFunds(uint256 time) public payable nonReentrant {
        require(
            lockedFunds[msg.sender].amount == 0,
            "You have already locked up funds"
        );
        lockedFunds[msg.sender] = LockedFunds(msg.value, time, block.timestamp);
    }
    ...
    function withdraw() public nonReentrant {
        ...
        require(
            block.timestamp >=
                lockedFunds[msg.sender].lockedAt + lockedFunds[msg.sender].time,
            "You cannot withdraw yet."
        );

        require(
            accountabilityNFTs.balanceOf(msg.sender) > 0,
...
contract AccountabilityNFTs is ERC721SignatureMint {
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
