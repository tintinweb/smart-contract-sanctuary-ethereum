...
contract RainCollateralController is Ownable {
    ...
    // Hash of user readable name of signing domain
    bytes32 public constant DOMAIN_NAME_HASH =
        keccak256(bytes("Rain Collateral"));

    // Hash of current major version of signing domain
    ...
    ) external isCollateralAdmin(_collateralProxy) {
    ...
    /**
     * @notice Used to verify withdraw signature
     * @dev Uses the ECDSA and check if
     *      signing address = controllerAdmin
     *      and increments its nonce.
     *      Reverts transaction if signature is not valid.
     * @param _collateralProxy targeting RainCollateral proxy address
     * @param _asset asset's contract address
     ...
     * @param _expiresAt timestamp when signature expires as unix seconds
     * @param _salt disambiguating salt for signature
     * @param _signature withdraw signature generated by controllerAdmin
     *
     * NOTE : We use {ethers.utils.solidityKeccak256} with action name "withdraw"
     *       and withdraw function parameters(collateral,asset,amount,nonce,expiresAt)
     *       to generate messageHash,
     *		 that will be signed using {ethers-signMessage} function.
     */
    function _verifyWithdrawalSignature(
        ...
        bytes32 _salt,
        bytes memory _signature
    ) internal activeSignature(_expiresAt) {
    ...
    /**
     * @notice Used to verify payment signature
     * @dev Uses ECDSA and check if signing address = controllerAdmin and increments nonce
     * @param _collateralProxy targeting RainCollateral proxy address
     * @param _assets array of asset's contract addresses
     * @param _amounts array of amounts corresponding to _assets
     * @param _expiresAt timestamp when signature expires as unix seconds
     * @param _salt disambiguating salt for signature
     * @param _signature payment signature generated by controllerAdmin
     *
     * NOTE : We use {ethers.utils.solidityKeccak256} with "pay" function name
     *       and parameters (user,collateral,assets,amounts,nonce)
     *       to generate messageHash,
     *		 that will be signed using {ethers-signMessage} function.
     */
        ...
        bytes32 messageHash = keccak256(
            bytes.concat(
                abi.encode(
                    ...
                    msg.sender,
                    _collateralProxy,
                    keccak256(abi.encodePacked(_assets)),
                    keccak256(abi.encodePacked(_amounts))
                ),
                abi.encode(nonce[_collateralProxy], _expiresAt)
            )
        );
        _verifySignature(_collateralProxy, messageHash, _salt, _signature);
    }
    ...
    function _verifySignature(
        ...
        require(
            hash.recover(_signature) == controllerAdmin,
            "Invalid signature"
        );
