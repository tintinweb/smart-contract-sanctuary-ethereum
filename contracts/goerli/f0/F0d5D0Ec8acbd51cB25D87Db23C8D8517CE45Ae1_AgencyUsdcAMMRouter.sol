...
interface IAgencyTWAP {
  ...
  function consult(uint256 amountIn) external view;
}

interface IAgencyTreasury {
  function intrinsicValueRatio()
    external
    view
    returns (uint256 frax, uint256 temple);
}

contract AgencyUsdcAMMRouter is Ownable, AccessControl, ERC2771Recipient {
  bytes32 public constant CAN_ADD_ALLOWED_USER =
    keccak256("CAN_ADD_ALLOWED_USER");
  ...
  uint256 public priceCrossedBelowDynamicThresholdBlock;

  // Percentage (represented as an int from 0 to 100)
  uint256 public constant DYNAMIC_THRESHOLD_INCREASE_DENOMINATOR = 10000;
  uint256 public dynamicThresholdIncreasePct =
  ...
  // necessary function to override, as `Context.sol` and `ERC2771Recipient.sol` both define it
  function _msgSender()
    internal
    view
  ...
  // necessary functions to override, as `Context.sol` and `ERC2771Recipient.sol` both define it
  function _msgData()
    internal
    view
    override(Context, ERC2771Recipient)
  ...
  function setDynamicThresholdDecayPerBlock(
    uint256 _dynamicThresholdDecayPerBlock
  ) external onlyOwner {
    dynamicThresholdDecayPerBlock = _dynamicThresholdDecayPerBlock;
  }
  ...
  function setDynamicThresholdIncreasePct(uint256 _dynamicThresholdIncreasePct)
    external
    onlyOwner
  {
    require(
      _dynamicThresholdIncreasePct > 0 &&
        _dynamicThresholdIncreasePct <= DYNAMIC_THRESHOLD_INCREASE_DENOMINATOR,
      "Increase is a pct represented as an integer between 0 and 100"
    );
  ...
  // To decide how much we mint on protocol, we linearly interp between these two price points, only when the
  // market is trading above the dynamicThresholdPrice;
  function setInterpolateFromPrice(uint256 frax, uint256 temple)
    external
    onlyOwner
  {
  ...
  function setInterpolateToPrice(uint256 frax, uint256 temple)
    external
    onlyOwner
  {
  ...
  function addAllowedUser(address userAddress) external {
    require(
      hasRole(CAN_ADD_ALLOWED_USER, owner()),
  ...
  function setProtocolMintEarningsAccount(address _protocolMintEarningsAccount)
    external
    onlyOwner
  {
    protocolMintEarningsAccount = _protocolMintEarningsAccount;
    ...
    if (reserveA == 0 && reserveB == 0) {
      ...
      if (amountBOptimal <= amountBDesired) {
        require(
          ...
          "TempleFraxAMMRouter: INSUFFICIENT_FRAX"
        );
        ...
        require(
          ...
          "TempleFraxAMMRouter: INSUFFICIENT_TEMPLE"
        );
    ...
    )
  {
    ...
    SafeERC20.safeTransferFrom(usdcToken, _msgSender(), address(pair), amountB);
  ...
  function swapExactFraxForTemple(
    ...
    address to,
    uint256 deadline
  ) external virtual ensure(deadline) returns (uint256 amountOut) {
    require(
      allowed[_msgSender()] || openAccessEnabled,
      "Router isn't open access and caller isn't in the allowed list"
    );

    // Check temple out is witamountOutAMMhin acceptable user bounds
    (
      uint256 amountInAMM,
      uint256 amountInProtocol,
      uint256 amountOutAMM,
      uint256 amountOutProtocol
    ) = swapExactFraxForTempleQuote(amountIn);
    ...
    if (amountInProtocol > 0) {
      ...
      (uint256 rt, uint256 rf, ) = pair.getReserves();
      uint256 newDynamicThresholdPriceTemple = (rt *
        dynamicThresholdPrice.frax *
        DYNAMIC_THRESHOLD_INCREASE_DENOMINATOR) /
        (rf * dynamicThresholdIncreasePct);

      (
        ,
        uint256 dynamicThresholdPriceTemple
      ) = dynamicThresholdPriceWithDecay();
      if (newDynamicThresholdPriceTemple < dynamicThresholdPriceTemple) {
        // when not decaying, ensure DTP only ever increases
  ...
  function swapExactTempleForFrax(
    ...
    address to,
    uint256 deadline
  ) external virtual ensure(deadline) returns (uint256) {
    require(
      allowed[_msgSender()] || openAccessEnabled,
      "Router isn't open access and caller isn't in the allowed list"
    );
    ...
    (
      bool priceBelowIV,
      bool willCrossDynamicThreshold,
      uint256 amountOut
    ) = swapExactTempleForFraxQuote(amountIn);
    if (priceBelowIV) {
      // if price is bellow IV, agency tokens that are sold are burned and stablec is sent from pair?
      require(
        amountOut >= amountOutMin,
        "AgencyUsdcAMMRouter: INSUFFICIENT_OUTPUT_AMOUNT"
      );
      agencyToken.burnFrom(_msgSender(), amountIn);
      SafeERC20.safeTransfer(usdcToken, to, amountOut);
    } else {
      // only set the threshold price decay if we aren't already in decay mode
      if (
        willCrossDynamicThreshold && priceCrossedBelowDynamicThresholdBlock == 0
      ) {
        priceCrossedBelowDynamicThresholdBlock = block.number;
        emit PriceCrossedBelowDynamicThreshold(
  ...
  function mintRatioAt(uint256 temple, uint256 frax)
    public
    view
    returns (uint256 numerator, uint256 denominator)
  {
    ...
    (uint256 ts, uint256 fs, uint256 t1, uint256 f1, uint256 t2, uint256 f2) = (
      temple,
      frax,
      interpolateFromPrice.temple,
      interpolateFromPrice.frax,
      interpolateToPrice.temple,
      interpolateToPrice.frax
    );
    // temple = reserveTemple
    // frax = reserveFrax
    // current
    // _reserve0   uint112 :  889382990550305723438 ( temple )
    // _reserve1   uint112 :  900000000000000002000 ( frax )
    uint256 n1 = fs * t1; // n1 = reserveFrax * interpolateFromPrice.temple ( current: 900000000000000002000 * 50000 = 4,5×10²⁵ )
    uint256 n2 = f1 * ts; // n2 = interpolateFromPrice.frax * reserveTemple ( current: 100000 * 889382990550305723438 = 8,893829906×10²⁵ )
  ...
  function dynamicThresholdPriceWithDecay()
    public
    view
    returns (uint256 frax, uint256 temple)
  {
    uint256 thresholdDecay = 0;

    if (priceCrossedBelowDynamicThresholdBlock > 0) {
      thresholdDecay =
        (block.number - priceCrossedBelowDynamicThresholdBlock) *
        dynamicThresholdDecayPerBlock;
    }

    return (
      dynamicThresholdPrice.frax,
      dynamicThresholdPrice.temple + thresholdDecay
    );
  }

  function swapExactFraxForTempleQuote(uint256 amountIn)
    public
    view
    returns (
      uint256 amountInAMM,
  ...
  {
    (uint256 reserveTemple, uint256 reserveFrax, ) = pair.getReserves();
    (
      uint256 thresholdPriceFrax,
      uint256 thresholdPriceTemple
    ) = dynamicThresholdPriceWithDecay();

    // if AMM is currently trading above target, route some portion to mint on protocol
    if (
      thresholdPriceTemple * reserveFrax >= thresholdPriceFrax * reserveTemple
    ) {
      (uint256 numerator, uint256 denominator) = mintRatioAt(
        reserveTemple,
        reserveFrax
      );
  ...
  function swapExactTempleForFraxQuote(uint256 amountIn)
    public
    view
    returns (
      bool priceBelowIV,
    ...
    )
  {
    ...
    if (!priceBelowIV) {
      ...
      bool currentPriceIsAboveThreshold = thresholdPriceTemple * reserveFrax >
        thresholdPriceFrax * reserveTemple;
      bool postSellPrieIsBelowThreshold = thresholdPriceTemple *
        (reserveFrax - amountOut) <
        thresholdPriceFrax * (reserveTemple + amountIn);
      willCrossDynamicThreshold =
        currentPriceIsAboveThreshold &&
        postSellPrieIsBelowThreshold;
    }
  }
...
// rename contracts and create separate repository for the ecsa
contract AgencyERC20Token is ERC20, ERC20Burnable, Ownable, AccessControl {
    ...
    constructor() ERC20("Agency", "AGENCY") {
        transferOwnership(address(0xc16F2D51e96B98A88772AF67523B8f836a3152c3));
        _setupRole(DEFAULT_ADMIN_ROLE, owner());
    }
