...
contract HiveManager is Ownable, IERC721Receiver {
    ...
    function FinishHive(uint256 _hid) private {
        ...
        for (uint256 index = 0; index < seasonDays; index++) {
    ...
    function StakeHive(uint256 _hid, uint256[] memory _hornets, bool _isWorker) public {
        ...
        require(today < seasonDays - 1, "HiveManager: The season is ended or last day.");
        ...
        if (_isWorker) {
            ...
            hiveDay.yieldVenom += _hornets.length * 10**18;
        } else {
            ...
            hiveDay.defense += _hornets.length * staticDefensePerGuard;
        }

        hive.yieldVenom += _hornets.length * 10**18;
    ...
    function ActionHive(uint256 _hid, uint256 _hornetId, bool _isHarvest) public {
        ...
        if (_isHarvest) {
            ...
            hiveDay.signedWorkers = hiveDay.signedWorkers + 1;
        } else {
            hiveDay.defense += staticDefensePerGuard;
            hiveDay.signedGuards = hiveDay.signedGuards + 1;
        }
    ...
    function FailedAttack(InvaderInfo storage _invader, bool _hasAVP, AttackInfo storage _attack, uint256 _random, uint256 _attackRate) private {
        if (_random >= 10000 - 5 * (100 - _attackRate)) {
            _invader.status = _hasAVP? 0 : 2;
            _attack.statuses.push(_hasAVP? 3 : 5);
        } else if (_random >= 10000 - 65 * (100 - _attackRate)) {
            _invader.status = 1;
    ...
    function getHiveInfo(uint256 _hid) public view returns (HiveInfo memory) {
        return hiveInfo[_hid];
    }

    function getHornetInfo(uint256 _hornet) public view returns (HornetInfo memory) {
        return hornetInfo[_hornet];
    }

    function getUserInfo(uint256 _hid, address _addr) public view returns (UserInfo memory) {
        return userInfo[_hid][_addr];
    }

    function getHiveDayInfo(uint256 _hid, uint256 _day) public view returns (HiveDayInfo memory) {
        return hiveDayInfo[_hid][_day];
    }

    function getAttackInfo(uint256 _hid, uint256 _attacker) public view returns (AttackInfo memory) {
        return attackInfo[_hid][_attacker];
    }

    function getInvaderInfo(uint256 _invader) public view returns (InvaderInfo memory) {
        return invaderInfo[_invader];
    }

    function getHornetStatus(uint256 _hornet, uint256 _day) public view returns (uint256) {
        HornetInfo storage hornet = hornetInfo[_hornet];
        if (hornet.lastProcessingDay == _day) { return hornet.status; }
        return 0;
    }
}