...
// File: contracts/interfaces/ISweepersToken.sol
...
// File: contracts/interfaces/IDust.sol
...
contract Garage is Ownable, ReentrancyGuard {
    ...
    function getUnclaimedDust(uint16[] calldata _ids) external view returns (uint256) {
        uint16 length = uint16(_ids.length);
        uint256 owed;
        for (uint16 i = 0; i < length; i++) {
            require(StakedAndLocked[_ids[i]], 
