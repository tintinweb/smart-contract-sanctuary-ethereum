...
//To implement next round
// implement palettes
// split into seperate contract
// 32x32 as well
// background like on-chain kevin 
...
contract CCZoo is ERC721A, Ownable {
    ...
    uint256 public constant MINT_PRICE = 0.005 ether; //0.005 ETH SETTTT; 
    ...
    string[NUM_LAYERS] public LAYER_NAMES = ["Palette", "Background", "Hat", "Hand" , "Ground"];
    Palette[] palettes;
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) { //view
        if(tokenId > totalSupply() || tokenId == 0 ) revert tokenDoesntExists();
        ...
        //keep this in this contract? and only run the render throught the other ß
    ...
    function _getTokenSVG(uint256[NUM_LAYERS] memory tokenTraits, uint256 tokenId, bytes memory buffer) public view  { //view
        ...
        //render background
        _renderTrait(traits[1], tokenTraits[1], buffer, 0, 0);

        //set animal to random if 0
        uint8[2][3] memory locations;
        if(tokenAnimal[tokenId] == 0) {
            locations = _renderAnimal(animals[ (uint256(keccak256(abi.encodePacked(tokenId,block.number))) % (animals.length-1)) + 1], buffer);
        } else {
            locations = _renderAnimal(animals[tokenAnimal[tokenId]], buffer);
        }
    ...
    function _renderAnimal(Animal memory _currentAnimal, bytes memory buffer) private view returns(uint8[2][3] memory locations)  { //view
        (, bytes memory _toDecode) = inflateLib.puff(SSTORE2.read(_currentAnimal.data), _currentAnimal.destLen);
        bytes memory _animalData = abi.decode(_toDecode, (bytes));
        (locations, _animalData) = gfx.drawPixelsAnimal(_animalData);
    ...
    function getTokenSVGWithAnimal(uint256 tokenId, string memory name)  public view returns (string memory) {
        ...
        //render animal
        uint8[2][3] memory locations = _renderAnimal(_getAnimal(name), buffer);
    ...
    function getTokenSVGForBytes(bytes memory _animal, uint256[NUM_LAYERS] memory tokenTraits)  public view returns (string memory) {
        ...
        (locations, _animalData) = gfx.drawPixelsAnimal(_animal);
        buffer.appendSafe(_animalData);
    ...
    function suggestAnimal(string memory _name, bytes memory _data) public {
        emit animalSuggested(_name, _data, msg.sender);
    }
...
interface IExquisiteGraphics {
  ...
  function drawPixelsAnimal(bytes memory data)
    external
    view
