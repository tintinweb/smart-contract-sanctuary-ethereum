...
contract ISPSWAT {
  ...
  function safeTransferFrom(address, address, uint256) external payable { require(false, ERROR); }
  function safeTransferFrom(address, address, uint256, bytes calldata) external payable { require(false, ERROR); }
  function transferFrom(address, address, uint256) external payable { require(false, ERROR); }
  function approve(address, uint256) external payable { require(false, ERROR); }
  function setApprovalForAll(address, bool) external payable { require(false, ERROR); }
  ...
  function ownerOf(uint256 _tokenId) external pure returns (address _owner) {
    require(_tokenId < 5, NOT_VALID_NFT);
