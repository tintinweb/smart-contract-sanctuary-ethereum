    ...
    CustomPaymentSplitterUpgradeable
{
   ...
   function initialize(
    ...
    // Black
    setPrice(1, 0.0049 ether);
    updateBalanceLimit(1, 5);

    // Platinum
    setPrice(2, 0.0059 ether);
    updateBalanceLimit(2, 4);
    
    // Gold
    setPrice(3, 0.0069 ether);
    updateBalanceLimit(3, 3);

    // Silver
    setPrice(4, 0.0079 ether);
    updateBalanceLimit(4, 2);

    // Bronze
    setPrice(5, 0.0088 ether);
    updateBalanceLimit(5, 1);

    // Public
    setPrice(6, 0.0088 ether);
    updateBalanceLimit(6, 2);
  ...
  function _startTokenId() pure internal override(ERC721AUpgradeable) returns (uint256) {
    ...
    {
        require(phase == 1, "Phase One not live");
    ...
    function phaseTwoMint(uint256 amount_, bytes32[] calldata proof_, uint8 stage_)
        ...
        onlyWhitelisted(stage_, msg.sender, proof_)
    {
        require(phase == 2, "Phase Two not live");
    ...
    function phaseThreeMint(uint256 amount_, bytes32[] calldata proof_, uint8 stage_)
        external
        payable
        onlyWhitelisted(stage_, msg.sender, proof_)
    {
        require(phase == 3, "Phase Three not live");
        uint8 _stage = uint8(stage_);
        _increaseBalance(_stage, msg.sender, amount_);
        _callMint(msg.sender, amount_);
        _handlePayment(amount_ * price(_stage));
    ...
    function mint(uint256 amount)
        ...
        payable
    {
        require(phase == 4, "Public not live");   
        _increaseBalance(6, msg.sender, amount);
        _callMint(msg.sender, amount);
        _handlePayment(amount * price(6));
    }

    function setPhase(uint8 phase_) external onlyAdmin {
        phase = phase_;
    }
