...
contract Primes is ERC721, Ownable {
    ...
    uint256 private availableSerials;
    ...
    function tokenURI(
        ...
        string memory dataImage = generateBase64SVG(serial);
        uint primesCount = PrimesUtils.getNumberOfPrimes(serial);
        return
            string(
                abi.encodePacked(
    ...
    function getSerialNumber() internal returns (uint) {
        uint updatedNumAvailableTokens = _numAvailableTokens;
        serial = getRandomAvailableTokenId(
            msg.sender,
