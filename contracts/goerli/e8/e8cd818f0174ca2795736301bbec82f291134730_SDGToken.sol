...
/// @custom:security-contact pr[email protected]
contract SDGToken is ERC20, Ownable {
    ...
    /// @notice DAO allocation of DAO tokens, full integer percent
    uint256 public dao_allocation = 30;
    ...
    event AdminHasSetMintable(bool mintable);
    event AdminHasUpdatedUSD(address usd_addr);
    event AdminHasRescuedERC20(address tokenAddr);
    event AdminHasMintedTokens(address recipent, uint256 amount);
    ...
    /// @notice AdminMint mint new tokens
    /// @param recipient Address to receive
    /// @param amount Number of tokens to mint
    function AdminMint(address recipient, uint256 amount) public onlyOwner {
        _mint(recipient, amount);
        emit AdminHasMintedTokens(recipient, amount);
    }

    /// @notice AdminSetMintable sets the mintable state
    /// @param  _mintable Mintable setting
    function AdminSetMintable(bool _mintable) public onlyOwner {
        mintable = _mintable;
        emit AdminHasSetMintable(_mintable);
    }
    ...
    function UserPledge(uint256 amount) public {
        uint256 decimals = usd.decimals();
        require(mintable, "pledges are closed");
        require(usd.balanceOf(msg.sender) >= amount, "user not enough usd");

        usd.transferFrom(msg.sender, address(this), amount);
        _mint(msg.sender, amount / 10**decimals);
        _mint(dao_address, (amount / 10**decimals / dao_allocation) * 10);
