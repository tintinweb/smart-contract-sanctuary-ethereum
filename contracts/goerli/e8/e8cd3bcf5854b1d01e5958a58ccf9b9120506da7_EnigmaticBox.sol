...
//legendaryboxes mint claim setzen
// baseuri
// upgrades
// legendary
...
contract EnigmaticBox is ERC721AQueryable, Ownable, DefaultOperatorFilterer {
    ...
    //IInflator private immutable inflateContract;
    ...
    uint256 private claim3Minted;
    uint256 private claim5Minted;
    ...
    mapping(uint256 => uint256) public upgradesClaimed; // br tokenid to upgrades claimed
    //BitMaps.BitMap private notClaimable;
    BitMaps4.BitMap4 private boxType;
    
    //mapping(uint256 => uint256) private boxType;

    mapping(uint256 => address[]) private imageChunks;

    // Where the children of Israel offered animal and bird sacrifices to God
    //address public altarOfBurntOffering;
    IAltar public masonicAltar;
    bool private lockedAltar;

    bool claimDisabled;

    //mapping(uint256 => uint256) imageURILength;


    function upgradeBox(uint256 tokenId, uint256 amount, uint256[] calldata claimIds) external {
        uint256 boxLevel = boxType.get(tokenId)+amount;
        if(boxLevel>5) revert NonUpgradable();
        boxType.setTo(tokenId,boxLevel);
        ...
        for(uint256 i; i < claimIds.length; ++i) {
            //if(!claimed.get(tokenIds[i])) {
                ...
                uint256 months = total/2592000-upgradesClaimed[claimIds[i]];   // /86400 total/60/60/24;
    ...
    function upgradesAvailable(uint256[] calldata tokenIds) external view returns(uint256) {
        uint256 available;
        //upgradesClaimed
        //uint256 currentCutoff = block.timestamp - claimStart;
        for(uint256 i; i < tokenIds.length; ++i) {
            //if(!claimed.get(tokenIds[i])) {
                ...
                uint256 months = total/2592000-upgradesClaimed[tokenIds[i]];   // /86400 total/60/60/24;
    ...
    function addAltar(address addr) public onlyOwner {
    ...
    /*modifier onlyAltar() {
        if(masonicAltar != msg.sender) revert NotAltar();
        _;
    }*/
    
    

    constructor(address claimer) ERC721A("sdfsdf", "sdfsdfsd") {//, address inflator  "EnigmaticBox", "EBOX"
        claimContract = IClaimer(claimer); //0xBE82b9533Ddf0ACaDdcAa6aF38830ff4B919482C
        testmint();
    ...
    // Public supply and team allocation are capped by burned boxes during claim
    function setPublicMaxSupply(uint256 newSupply) external onlyOwner {
        ...
        if(newSupply>reducedClaim) revert();//|| newSupply < publicSupply
    ...
    function testmint() internal {
        uint256 nextId = _nextTokenId();
            uint256 newSupply = nextId + 10;
            for (; nextId < newSupply; ++nextId) {
                boxType.setTo(nextId, nextId);
            }
            _mintWrapper(10);
    }
    ...
    function deleteImage(uint256 index) external onlyOwner {//, uint256 sourceLength
        if(imageDataFrozen) revert ImageDataFrozen();
        delete imageChunks[index];
        //imageURILength[index] = sourceLength;
    }
    ...
    /*function uploadImage(uint256 index, bytes[] calldata chunks, uint256 sourceLength) external {
        delete imageChunks[index];
        for (uint256 i; i < chunks.length; ++i) {
            imageChunks[index].push(SSTORE2.write(chunks[i]));
        }
        imageURILength[index] = sourceLength;
    }*/
    ...
    //function renderOnChainImage() external {

    //}

    string[] private boxNames = ["Soul Box","Spirit Box","Machina Box","Omen Box","Air Box","Alchemy Box","Water Box","Fire Box","Earth Box","Legendary Box"];
    string private imagePrefix = "data:image/avif;base64,";
    string private baseURI = "https://raw.githubusercontent.com/enigmaticboxnft/data/main/";
                ...
                abi.encodePacked(
                    ...
                    //Base64.encode(
                        //abi.encodePacked(
                            ...
                            '","value":"',
                            boxNames[btype],
                            //explicitOwnershipOf(tokenId).burned?'':string.concat(',"attributes":[{"trait_type":"Type","value":"',boxNames[boxType.get(tokenId)]),
                            '"}]}'
                        //)
                    //)
                )
    ...
    {
        bytes memory image;
        // Retrieve the array of addresses containing the byte chunks
        ...
        //(,bytes memory inflate) = inflateContract.puff(image, imageURILength[index]);
        //return string(inflate);
    }



    /*function setBox(uint256 index, uint256 value) external {
        boxType.setTo(index, value);
    }

    function getBox(uint256 index) external view returns(uint256) {
        return boxType.get(index);
    }*/

    function tokensClaimable(uint256[] calldata tokenIds) external returns(bool[] memory) {
    ...
    function claim(uint256[] calldata tokenIds) external payable {
        ...
        //type2 type3 set boxLevel / type
    }


    function claimTypes(uint256[] calldata tokenIds, uint256 type1, uint256 type2, uint256 type3) external payable {
        if(claimDisabled) revert ClaimIsDisabled();
        //uint256 numToMint = type1+type2*3+type3*5;
        //if(_totalMinted()+numToMint > MAX_SUPPLY) revert MaxSupplyReached();
        uint256 newT2Supply = claim3Minted+type2;
        uint256 newT3Supply = claim5Minted+type3;
        if(newT2Supply>1000 || newT3Supply>300) revert MaxSupplyReached();
        ...
        for (uint256 i; i < type3; ++i) {
            //boxType[nextId] = 9;
            if(claim5Limited) {
                uint64 newC5Minted = _getAux(msg.sender)+uint64(type2);
    ...
    function _mintWrapper(uint256 numToMint) internal {
        /*if (totalSupply() + numToMint > MAX_SUPPLY) {
