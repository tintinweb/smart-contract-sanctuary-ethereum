...
contract AIBF is ERC721A, Ownable {
   
    string private baseURI;    
    uint256 constant public maxTokens = 306;
    uint256 public _reserved = 6;
    ...
    constructor() ERC721A("AI Best Friend", "AIBF") {}
    ...
    modifier mintCompliance(uint256 _numberOfTokens) {
        ...
        require(msg.value >= price * _numberOfTokens, "Not Enough Bones!");
        require(_numberMinted(msg.sender) + _numberOfTokens <= maxPerWallet, "No Dogs Left");
    ...
    function mint(uint256 _numberOfTokens) external payable mintCompliance (_numberOfTokens){
        if (allowlistLive) {
            require(_allowList[msg.sender] >= _numberOfTokens, "Pant Pant");
            _allowList[msg.sender] -= _numberOfTokens;
        } else {
          require(publicSaleLive, "Still in the Dog House");
        }
        _mint(msg.sender, _numberOfTokens);  
    }

    function dogGroomer(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= _reserved, "No more dogs to groom!" );
        _mint(_to, _amount);
        _reserved -= _amount;
    }
