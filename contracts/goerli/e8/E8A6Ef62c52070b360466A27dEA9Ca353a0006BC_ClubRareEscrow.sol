...
contract ClubRareEscrow is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, IEscrow {
    using CountersUpgradeable for CountersUpgradeable.Counter;

    CountersUpgradeable.Counter _orderIds;
    ...
    function initialize(address _marketplace, address tradeAddress, address _validator) external initializer {
        __Context_init();
        __Ownable_init();
    ...
    function createOrder(
        ...
        require(msg.sender == marketplace || msg.sender == trade, "Invalid Caller");
        _orderIds.increment();
        ...
        emit EscrowCreated(id, _tokenId, _seller, _buyer, _amount, block.timestamp);
        return id;
    }

    function releaseEscrow(uint256 _orderId, bool isRevert) external virtual whenNotPaused nonReentrant {
        require(validator.admins(_msgSender()) || owner() == _msgSender(), "Not an admin");
        uint256 id = _orderId;
        Product memory order = orders[id];
        require(order.productStatus == Status.Pending, "Order already released");
        _releaseEscrow(_orderId, isRevert);
        order.productStatus = isRevert ? Status.Refunded : Status.Delivered;
        order.updatedAt = block.timestamp;
    }

    function _releaseEscrow(uint256 _orderId, bool isRevert) internal {
        uint256 id = _orderId;
        Product memory order = orders[id];
        bool isToken = order.erc20Address == address(0) ? false : true; // if native currency or not
        if (!isRevert) {
            Methods sellType = order.sellType;
            //release NFT to buyer
            uint256 sellerEarning = IMarketplacePayment(marketplace).chargeAndSplit(
                order.amount,
                order.buyer,
                order.erc20Address,
                order.royaltyAmt,
            ...
            IMarketplacePayment(marketplace).disburse(
                order.tokenAddress,
                order.erc20Address,
                order.buyer,
                order.tokenId,
            ...
            emit Delivered(id, block.timestamp);
            if (sellType == Methods.Buy) {
                emit Buy(
                    order.buyer,
                    order.seller,
                    order.tokenAddress,
            ...
            if (sellType == Methods.AcceptOffer) {
                emit AcceptOffer(
                    order.buyer,
                    order.seller,
                    order.tokenAddress,
            ...
            emit OrderReturned(id, block.timestamp);
        }
    }
}
