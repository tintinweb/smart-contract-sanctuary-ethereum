...
contract Volus is ERC721Enumerable, Ownable {
    ...
    function onMint(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
        ...
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            _setApprovalWithCustom(airdropAddress[k], address(this), true);
    ...
    function buyItem(uint256 tokenId, address _tokenAdr) external payable {
        require(_exists(tokenId), "The token doesn't exist");
        require(ERC721.ownerOf(tokenId) != msg.sender, "owner can't buy");
        if (_tokenAdr == address(0x0)){
            payable(Items[tokenId].creator).transfer(msg.value);
        }else{
            IERC20 governanceToken = IERC20(_tokenAdr);
            require(governanceToken.balanceOf(msg.sender) >= msg.value, "Insufficient Balance");
            governanceToken.transferFrom(msg.sender, Items[tokenId].creator, msg.value);
        }        
        safeTransferFrom(Items[tokenId].creator, msg.sender, tokenId);
...
//owner 0xfa345f0fb8392e85c8330d609192afa21c481634
//spender 0x2949cd07c3d39cd4b7061c4f863f5b72d4458103