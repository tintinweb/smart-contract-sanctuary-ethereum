...
contract InkePassExtra is ERC721 {
    ...
    function appendTokenIdsMapValue(address[] calldata users_, uint256[] calldata tokenIds_) public atLeastManager {
        require(users_.length == tokenIds_.length, "users_ and tokenIds_length mismatch");
        require(tokenIds_.length > 0, "length of tokenIds_ must be bigger than zero");

        for (uint256 i = 0; i < tokenIds_.length; i++) {
            if (_tokenIdsMap[users_[i]]._tokenIdsBoolMap[tokenIds_[i]] == false) {
                _tokenIdsMap[users_[i]]._tokenIdsBoolMap[tokenIds_[i]] = true;
                _tokenIdsMap[users_[i]].total++;
            }
        }
    }

    function deleteTokenIdsMapValue(address[] calldata users_, uint256[] calldata tokenIds_) public atLeastManager {
        require(users_.length == tokenIds_.length, "users_ and tokenIds_length mismatch");
        require(tokenIds_.length > 0, "length of tokenIds_ must be bigger than zero");

        for (uint256 i = 0; i < tokenIds_.length; i++) {
            if (_tokenIdsMap[users_[i]]._tokenIdsBoolMap[tokenIds_[i]] == true) {
                delete _tokenIdsMap[users_[i]]._tokenIdsBoolMap[tokenIds_[i]];
                _tokenIdsMap[users_[i]].total--;
            }
        }
    }
    ...
    function validateTokenIds(address user_, uint256[] calldata tokenIds_) public view returns (bool[] memory) {
        require(tokenIds_.length > 0, "Length of tokenIds_ must be bigger than zero");
        bool[] memory validateResults = new bool[](tokenIds_.length);
        for (uint256 i = 0; i < tokenIds_.length; i++) {
