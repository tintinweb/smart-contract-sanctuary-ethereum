...
contract HODLBonding is Ownable, ReentrancyGuard {
    ...
    modifier notContract() {
        require(!msg.sender.isContract(), "HODL: Contracts not allowed");
        _;
    }
    ...
    function buyHFD(
        ...
        if (_token == address(0)) {
            ...
            price = isDynamicPriceUsed ? oracle.getPriceInUSD(_amount) : (initialPrice * _amount) / 10**hfd.decimals();
            uint256 discountedPrice = price - ((price * discountPerLock[_lockId]) / 10000);
