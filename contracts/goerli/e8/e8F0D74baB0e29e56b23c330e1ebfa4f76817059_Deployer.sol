...
/// @notice This contract is used to deploy the logic and proxy contracts
/// @notice It also transfers ownership of the proxy to the Gnosis Safe
contract Deployer {
    address public immutable gnosisSafe;
    address public immutable logic;
    address public immutable proxy;
    address public logicImproved;
    ...
    /// @notice Deploy the logic and proxy contracts on construction
    /// @param _gnosisSafe The address of the Gnosis Safe you want to own the proxy
    constructor(address _gnosisSafe) {
        // save the gnosis safe address
        gnosisSafe = _gnosisSafe;

        // deploy the logic and proxy contracts
        logic = address(new Logic());
        proxy = address(new Proxy(logic));

        // attach the proxy to the logic contract
        Logic proxyWithLogic = Logic(proxy);

        // initialise the proxy
        proxyWithLogic.initialize(1);

        // transfer ownership of the proxy to the gnosis safe
        proxyWithLogic.transferOwnership(gnosisSafe);
    }

    /// @notice Used by the Gnosis Safe to upgrade the logic contract
    /// @notice you must first transfer ownership of the proxy to the Deployer contract
    /// @notice then call this function
    function upgradeLogic() public onlyGnosisSafe {
        // deploy the improved logic contract
        logicImproved = address(new LogicImproved());

        // attach the proxy to the logic contract
        Logic proxyWithLogic = Logic(proxy);

        // upgrade the proxy to use the improved logic
        proxyWithLogic.upgrade(logicImproved);

        // attach the proxy to the improved logic contract
        LogicImproved proxyWithLogicImproved = LogicImproved(proxy);

        // transfer ownership of the proxy back to the gnosis safe
        proxyWithLogicImproved.transferOwnership(gnosisSafe);
    }
}
