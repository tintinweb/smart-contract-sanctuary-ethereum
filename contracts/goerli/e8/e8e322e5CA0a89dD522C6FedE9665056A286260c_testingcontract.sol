...
contract testingcontract is ERC721A, Ownable {
    ...
    function mint(uint256 amt) external payable {
        ...
        require(msg.value >= amt * cost, "Insufficient funds");
        require(totalSupply() + amt < maxSupply + 1, "Not enough Memurs left.");

        _safeMint(msg.sender, amt);
    }

    function mint_proof(uint256 amt, bytes32[] memory proof) external payable {
        require(mintEnabled, "Minting is not live yet.");
        require(msg.sender == tx.origin, "No bots, only true Memurs!");
        require(msg.value >= amt * cost, "Insufficient funds");
        require(totalSupply() + amt < maxSupply + 1, "Not enough Memurs left.");
    ...
    function mint_proof_byte(uint256 amt, bytes32 proof) external payable {
        require(mintEnabled, "Minting is not live yet.");
        require(msg.sender == tx.origin, "No bots, only true Memurs!");
        require(msg.value >= amt * cost, "Insufficient funds");
        require(totalSupply() + amt < maxSupply + 1, "Not enough Memurs left.");
