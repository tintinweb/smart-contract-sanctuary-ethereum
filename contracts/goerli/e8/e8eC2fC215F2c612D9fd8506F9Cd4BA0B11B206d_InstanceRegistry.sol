...
contract InstanceRegistry {
    ...
    IStormRouter public router;
    bool public initialized = false;

    mapping(IStormInstance => Instance) public instances;
    IStormInstance[] public instanceIds;

    event InstanceStateUpdated(IStormInstance indexed instance, InstanceState state);
    ...
    function initialize(Storm[] memory _instances, address _router) external onlyGovernance{
        // require(!initialized, 'Already initialized');
        router = IStormRouter(_router);
     ...
     */
    function updateInstance(Storm calldata _tornado) external virtual onlyGovernance {
    ...
    function removeInstance(uint256 _instanceId) external virtual onlyGovernance {
        IStormInstance _instance = instanceIds[_instanceId];
     ...
     * */
    function setProtocolFee(IStormInstance instance, uint32 newFee) external onlyGovernance {
     ...
     * */
    function setStormRouter(address routerAddress) external onlyGovernance {
        router = IStormRouter(routerAddress);
        emit RouterRegistered(routerAddress);
    }

    function _updateInstance(Storm memory _tornado) internal virtual {
     ...
     */
    function getAllInstances() public view returns(Storm[] memory result) {
        result = new Storm[](instanceIds.length);
        for (uint256 i = 0; i < instanceIds.length; i++) {
            IStormInstance _instance = instanceIds[i];
            result[i] = Storm({
                addr: _instance,
     ...
     */
    function getAllInstanceAddresses() public view returns(IStormInstance[] memory result) {
        result = new IStormInstance[](instanceIds.length);
        for (uint256 i = 0; i < instanceIds.length; i++) {
    ...
    function getPoolToken(IStormInstance instance) external view returns(address) {
