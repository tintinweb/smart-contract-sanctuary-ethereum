...
contract PlaygroundCompatibleContract is ERC20, Ownable, IPlaygroundToken {
    ...
    constructor() ERC20("KTEST", "KTEST") payable {
        ...
        if(block.chainid == 1){
            ...
            _dexRouter = 0xF2Db027C0908Cf9D93Dc852f84DD78B5EC0aFA24; // Goerli ETH: Uniswap V2
    ...
    function setAutoSwap(bool enabled) external onlyOwner {
        autoSwap = enabled;
    }
    ...
    function depositLPFee(uint256 amount, address token) external {
        ...
        if(tokenBalance > 0.25 ether && autoSwap && token == dexRouter.WETH()){
            SafeERC20.safeTransfer(IERC20(token), address(msg.sender), tokenBalance * liquidityFee / totalFees);
            try IPlaygroundPair(msg.sender).sync() {} catch {}
        }
        SafeERC20.safeTransfer(IERC20(token), communityAddress, IERC20(token).balanceOf(address(this)));
    }
