...
contract Catbotican is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard, IERC2981 {
    ...
    uint256 public replicatStartDate;
    uint256 public descendicatStartDate;

    mapping(address => bool) public proxyRegistryAddress;

    mapping(uint256 => uint16[]) public descendicatMapping;
    ...
    constructor(
        ...
        uint256 _replicatStartDate,
        uint256 _descendicatStartDate,
        address _cbotContract,
        address _nvContract,
        address _proxyAddress
    ) ERC721('Catbotican', '3DCBOT') {
        baseURI = _baseContractURI;
        tokenSuffixURI = _tokenSuffixURI;
        recipients = _recipients;
        splits = _splits;
        replicatStartDate = _replicatStartDate;
        descendicatStartDate =  _descendicatStartDate;
        cbotContract = IERC721(_cbotContract);
        nvContract = IERC1155(_nvContract);
        proxyRegistryAddress[_proxyAddress] = true;
    }

    function mintRelpicat(uint256 id) public nonReentrant {
    ...
    function mintDescendicat(uint256 id, uint8 mainLock, uint8 additionalLock) external nonReentrant {
        require(cbotContract.ownerOf(id) == msg.sender,'Catbot not owned');
        ...
        descendicatMapping[id].push(uint16(currentTokenId));
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        require(!tokenStaked[tokenId] || from == address(0),'ERC721Pausable: token transfer while paused');
    }

    function stakeToken(uint256 id) external {
        require(msg.sender == ownerOf(id),'unauthorized');
        tokenStaked[id] = !tokenStaked[id];
    }
