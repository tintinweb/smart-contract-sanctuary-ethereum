...
contract SoccerGame is Ownable, ERC1155 {
    ...
    uint256 constant MAX_ID_CNT = 10;
    uint256 constant MAX_SUPPLY_CNT = 320;
    ...
    function whitelistMint(bytes32[] calldata merkleProof, uint256 quantity) external callerIsUser 
        isValidMerkleProof(merkleProof, whitelistMerkleRoot)
    {
        ...
        require(nowSupply + quantity <= MAX_SUPPLY_CNT, "reached max supply");
        nowSupply += quantity;
        mutiMint(quantity);
    }
    ...
    function publicSaleMint(uint256 quantity) external payable callerIsUser {
        SaleConfig memory config = saleConfig;
        uint256 publicPriceWei = uint256(config.publicPriceWei);
    ...
    function mutiMint(uint256 quantity) private {
        uint256 startId = getRandom();
        for(uint256 i=0; i<quantity;) {
            if (idCnt[startId] < MAX_ID_CNT) {
                _mint(msg.sender, startId, 1, "");
                idCnt[startId] += 1;
                i += 1;
            }
            startId = (startId + 1) % (MAX_ID + 1);
        }
    }
