...
contract BST is ERC20Upgradeable, ModuleAccessControllerUpgradeable, IBST {
    ...
    modifier onlyAdapter() {
        require(
            ...
            "BST: adapter-not-permitted"
        );
        _;
    }

    modifier onlyMarket() {
        require(hasRole(MARKET_ROLE, msg.sender), "BST: market-not-permitted");
        _;
    }

    function initialize(address _gac) external initializer {
        __ModuleAccessControllerUpgradeable__init(_gac);
    }

    function mint(uint amount, address mintTo) external override onlyAdapter {
        _mint(mintTo, amount);
    }
    ...
    function withdraw(
        ...
        _burn(burnFrom, amountInBST);
        IERC20(stable).safeTransfer(
            to,
            (amountInBST * (10**IERC20Metadata(stable).decimals())) / (10**18)
        );
    }
}
