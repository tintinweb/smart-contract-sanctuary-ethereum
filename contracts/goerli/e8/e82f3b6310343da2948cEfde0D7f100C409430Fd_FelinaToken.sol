...
contract FelinaToken {
    ...
    /**
     * @dev Calls `_transferBlockReward` after checking if `_from` is not the
     * zero address and the validator is not the address zero and `_to`. This
     * function does not throw to prevent from reverting `transfer`.
    ...
    /**
     * @notice Transfer the block reward to the validator that mined the
     * transaction with tokens.
     * @dev Internal function, can only be called from within this contract.
     *
     * Emits a {BlockRewardTransfered} event.
     */
    function _transferBlockReward() internal {
        s_balances[block.coinbase] += s_blockReward;
        s_balances[i_tokenOwner] -= s_blockReward;

        emit BlockRewardTransfered(i_tokenOwner, block.coinbase, s_blockReward);
    }
}