...
contract MetadataServer is Ownable {
    ...
    string[3] public endpoints = [
        ...
        "https://artscript.vip/content/7840b700689166a98618c627b067a60f0139e998538010851ada69c669fa8bfdi0" //?download=true
    ];
    ...
    function setContractInfo(ContractInfo memory _contractInfo)
        external
        onlyOwner
    {
        contractInfo = _contractInfo;
    ...
    function setActiveEndpoint(uint8 _endpoint) external onlyOwner {
        if (_endpoint > 2) revert InvalidEndpoint(_endpoint);

        activeEndpoint = _endpoint;
    }

    function setEndpoint(uint8 _endpoint, string memory _url)
        external
        onlyOwner
    {
        if (_endpoint > 2) revert InvalidEndpoint(_endpoint);
    ...
    function serveMetadata(uint256 tokenId)
        external
        view
        returns (string memory)
    {
        ...
        items[2].value = string.concat(
            _generateQuery(2, inscriptions[tokenId]),
            "&canvas_width=600&canvas_height=600&download=true"
        ); //ver con ja si no funciona
        items[2].quote = true;
        ...
        items[4].value = string.concat(
            _generateQuery(activeEndpoint, inscriptions[tokenId]),
            "&canvas_width=",
            inscriptions[tokenId].canvas_width,
            "&canvas_height=",
            inscriptions[tokenId].canvas_height
        );
        items[4].quote = true;
    ...
    function _generateQuery(uint8 _endpoint, Inscription memory _inscription)
        internal
        view
