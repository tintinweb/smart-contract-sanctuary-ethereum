...
contract PrimaryIndexTokenLeverage is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    event LeveragedBorrow(address indexed user, address projectToken, address lendingToken, uint addingAmount, uint totalDepositedAmount, uint borrowAmount);
    ...
    function deferLiquidityCheck(address user, address projectToken, address lendingToken, uint lendingTokenAmount) internal view {
        uint pitRemaining = primaryIndexToken.pitRemaining(user, projectToken, lendingToken);
    ...
    function nakedBorrow(address user, address lendingToken, uint lendingTokenAmount, address projectToken) public nonReentrant {
        address _lendingToken = primaryIndexToken.lendingTokenPerCollateral(user, projectToken);
        ...
        primaryIndexToken.calcBorrowPosition(user, projectToken, lendingToken, lendingTokenAmount, _lendingToken);
    ...
    function leveragedBorrow(address projectToken, address lendingToken, uint notionalExposure, uint marginCollateralAmount, uint maxSlippage, bytes memory buyCalldata) public {
        ...
        nakedBorrow(msg.sender, lendingToken, lendingTokenCount, projectToken);
        ...
        (uint totalCollateral, uint addingAmount) = collateraliseLoan(msg.sender, projectToken, amountRecive, marginCollateralAmount);

        deferLiquidityCheck(msg.sender, projectToken, lendingToken, lendingTokenCount);

        if(!isLeveragePosition[msg.sender][projectToken]){
            isLeveragePosition[msg.sender][projectToken] = true;
        }
        emit LeveragedBorrow(msg.sender, projectToken, lendingToken, addingAmount, totalCollateral, amountRecive);
    }
