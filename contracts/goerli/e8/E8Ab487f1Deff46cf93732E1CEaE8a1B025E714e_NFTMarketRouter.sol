...
import "../../../interfaces/internal/routes/INFTCollectionFactory.sol";
import "../../../libraries/AddressLibrary.sol";
...
abstract contract NFTCollectionFactoryRouterAPIs is NFTMarketRouterCore {
  function _createNFTTimedEditionCollection(
    ...
    bytes memory returnData = _routeCallFromMsgSender(
      ...
      abi.encodeWithSelector(
        INFTCollectionFactory.createNFTTimedEditionCollection.selector,
        name,
        symbol,
        tokenURI,
        mintEndTime,
  ...
  function _createNFTTimedEditionCollectionWithPaymentFactory(
    ...
    bytes memory returnData = _routeCallFromMsgSender(
      nftCollectionFactory,
      abi.encodeWithSelector(
        INFTCollectionFactory.createNFTTimedEditionCollectionWithPaymentFactory.selector,
        name,
        symbol,
        tokenURI,
        mintEndTime,
  ...
  NFTDropMarketRouterAPIs
{
   ...
   * The sale will last for 24 hours starting at `saleParams.generalAvailabilityStartTime`.
   * @param collectionParams The parameters for the edition collection creation.
   * @param saleParams  The parameters for the sale creation.
   ...
   */
  function createTimedEditionCollectionAndSale(
    TimedEditionCollectionCreationParams calldata collectionParams,
    FixedPriceSaleParams calldata saleParams,
    uint256 txDeadlineTime
  ) external txDeadlineNotExpired(txDeadlineTime) returns (address collection) {
    uint256 generalAvailabilityStartTime = saleParams.generalAvailabilityStartTime;
    ...
    _createFixedPriceSaleV3(
      collection,
      saleParams.exhibitionId,
      saleParams.price,
      saleParams.limitPerAccount,
      generalAvailabilityStartTime,
      0
    );
  }
   ...
   * @dev The collection will include the `nftDropMarket` as an approved minter.
   */
  function createTimedEditionCollectionAndSaleWithPaymentFactory(
    TimedEditionCollectionCreationParams calldata collectionParams,
    CallWithoutValue calldata paymentAddressFactoryCall,
    FixedPriceSaleParams calldata saleParams,
    uint256 txDeadlineTime
