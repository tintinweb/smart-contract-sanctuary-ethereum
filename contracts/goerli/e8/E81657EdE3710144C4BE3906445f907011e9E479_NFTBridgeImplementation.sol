...
contract NFTBridge is NFTBridgeGovernance {
    ...
    function completeTransfer(bytes memory proofBlob, bytes32[] memory merkleProof) public {
        _completeTransfer(proofBlob,merkleProof);
    }

    // Execute a Transfer message
    function _completeTransfer(bytes memory proofBlob, bytes32[] memory merkleProof) internal {
...
//    }
    ...
    function setZKHole(address h) internal {
        _state.zkhole = payable(h);
    }
...
contract NFTBridgeGovernance is NFTBridgeGetters, NFTBridgeSetters, ERC1967Upgrade {
    ...
    function registerChain(uint16 chainId, bytes32 contracAddress) public onlyOwner {
        setBridgeImplementation(chainId, contracAddress);
    }
...
interface IZKHole is Structs {
    event LogMessagePublished(address indexed sender, uint64 sequence, uint32 nonce, bytes payload);
    ...
    function parseAndVerifyProof(bytes memory proofBlob,bytes32[] memory merkleProof) external view returns (Structs.VM memory vm, bool valid, string memory reason);
...
contract NFTBridgeStorage {
    ...
    struct State {
        address payable zkhole;

        address tokenImplementation;

        address owner;

        address provethVerifier;
...
contract NFTBridgeState {
    ...
    uint256 public constant MIN_LOCK_TIME = 1 minutes;
}
