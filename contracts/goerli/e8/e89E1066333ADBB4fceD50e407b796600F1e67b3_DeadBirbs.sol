...
contract DeadBirbs is ERC721A, Seller, AttributeStore, ReentrancyGuard {
    ...
    constructor(
        ...
        // allowlist sale 
        setAllowlistSale(1675710175, 120960000, 20000000000, 1, 0xe12e7B6DDA6bC392b8441240459f7960E016D7Cb, false);
        
        // public sale
        setPublicSale(1675710175, 120960000, 20000000000, 1);

        setAttributeSigner(0x3c8Ba5Ce97a0eA4CB1E379055C465D52bCF869e1);
    }
    ...
    */
    ...
    function publicMint2(string calldata attributeHash, bytes calldata attributeSignature) external payable nonReentrant {
        verifyPublicSale(1);
        verifyAttributeHash(attributeHash, attributeSignature);
        _safeMint(msg.sender, 1);
    }

    function publicMint3(string calldata attributeHash, bytes calldata attributeSignature) external payable nonReentrant {
        verifyPublicSale(1);
        _safeMint(msg.sender, 1);
    }

    function publicMint4(string calldata attributeHash, bytes calldata attributeSignature) external payable {
        verifyPublicSale(1);
        verifyAttributeHash(attributeHash, attributeSignature);
        sotreAttributeHash(attributeHash, msg.sender, totalSupply());
        _safeMint(msg.sender, 1);
    ...
    */
    ...
    // build custom token URI

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        string memory uri = super.tokenURI(tokenId);

        return bytes(uri).length > 0 ? string(abi.encodePacked(uri, "?attr=", tokenData[tokenId])) : "";
    }
