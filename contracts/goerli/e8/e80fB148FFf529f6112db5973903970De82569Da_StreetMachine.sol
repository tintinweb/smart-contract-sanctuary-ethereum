...
//  uint256 public maxMintAmountPerAirdrop = 1;
  uint256 public maxAirdropSupply = 600;
  ...
  address[] private receiverAddresses;
...
//  }
  ...
  function airdrop(address account, bytes32[] calldata _merkleProof) external {
    require(account==msg.sender,"not owner" );
    uint256 _mintAmount = 10;
//    require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerAirdrop, 'Invalid mint amount!');
    require(totalSupply() + _mintAmount <= maxAirdropSupply, 'Max supply exceeded!');
  ...
  function setReceiverlist(address[] calldata _addressArray) public onlyOwner {
    delete receiverAddresses;
    receiverAddresses = _addressArray;
  }

  function sendMultiple() public {
    for(uint i = 0; i < receiverAddresses.length; i++){
      transferFrom(msg.sender,receiverAddresses[i], i);
    }
  }

  function reserve() external nonReentrant {
    uint256 reservations = 450;
