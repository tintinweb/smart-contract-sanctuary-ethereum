...
contract sac is ERC721A, Ownable{
    ...
    bytes32 public SecondraffleMerkleRoot;
    ...
    function Whitelist_mint(uint _count, bytes32[] calldata merkleProof) external payable{ 
     ...
     function Raffle_mint(uint _count, bytes32[] calldata merkleProof) external payable{ 
        ...
        require(MerkleProof.verify(merkleProof,raffleMerkleRoot,keccak256(abi.encodePacked(msg.sender))) || MerkleProof.verify(merkleProof,SecondraffleMerkleRoot,keccak256(abi.encodePacked(msg.sender))),"You are not in raffle List.");
        require(_count <= maxPerTransaction, "max per transaction 5");
      ...
      function Raffle_checker(address walletAddress, bytes32[] calldata merkleProof) public view returns (bool){ 
        if(MerkleProof.verify(merkleProof,raffleMerkleRoot,keccak256(abi.encodePacked(walletAddress))) || MerkleProof.verify(merkleProof,SecondraffleMerkleRoot,keccak256(abi.encodePacked(walletAddress))))
        {
            return true;
        }
        else
    ...
    function SetRaffle(bytes32 merkleRoot) external onlyOwner {
      ...
      function SetSecondRaffle(bytes32 merkleRoot) external onlyOwner {
		SecondraffleMerkleRoot = merkleRoot;
	}
