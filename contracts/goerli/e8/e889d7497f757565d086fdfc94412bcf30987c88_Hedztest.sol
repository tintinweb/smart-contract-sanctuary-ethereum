...
contract Hedztest is ERC721A, Ownable, Revelator {
    ...
    constructor(
        ...
        ERC721A("Hedz", "HEDZ")
        Revelator(_vrfCoordinator, _keyHash, _subscriptionId)
    {
    ...
    function preMint(
        ...
        if (
            mintPhase != MintPhase.Collectors &&
            mintPhase != MintPhase.Community
        ...
        bytes32 node = keccak256(
            abi.encodePacked(msg.sender, index, amountAllocated)
        );
    ...
    function isClaimed(MintPhase _mintPhase, uint256 index)
        external
        view
        returns (bool)
    {
    ...
    function setMerkleRoot(MintPhase _mintPhase, bytes32 merkleRoot)
        external
        onlyOwner
    {
        if (
            _mintPhase != MintPhase.Collectors &&
            _mintPhase != MintPhase.Community
    ...
    function setMintPhase(MintPhase _mintPhase) external onlyOwner {
        if (
            _mintPhase == MintPhase.Collectors &&
            merkleRoots[MintPhase.Collectors] == 0x0
        ) revert MerkleRootNotSet();
        if (
            _mintPhase == MintPhase.Community &&
            merkleRoots[MintPhase.Community] == 0x0
        ) revert MerkleRootNotSet();
