...
contract sampleProject is ERC20, Ownable {
    ...
    uint256 public _maxHoldings = 2500000 * 1e18; //adjustable
    ...
    function _swap(uint256 amountSwap) internal lockSwap {
		...
		uint256 amountForLiquidity = amountSwap / 3;

        uint256 actualLiquidity = amountForLiquidity / 2;

        uint256 tokensToSwap = amountSwap - actualLiquidity;

        //proceed to liquidate: save liquidity tokens (half of amountForLiquidity)
        ...
        //1 third go to LP but half only added per swap

        uint256 liquidityETH = (actualLiquidity * swappedETH) / tokensToSwap;

		uint256 tokensAdded = _addLiquidity(actualLiquidity, liquidityETH);

        //send eth balance to marketing and admin

        _paymentETH(_marketingWallet, (swappedETH - liquidityETH)/2);

        _paymentETH(_adminWallet, (swappedETH - liquidityETH)/2);

        emit autoLiquidity(tokensAdded, block.timestamp);            

    }

    function _addLiquidity(uint256 tokens, uint256 liquidityETH) private returns(uint256){
