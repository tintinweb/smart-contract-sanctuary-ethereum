...
contract SattvaSoulSuportersSeller is
    AccessControl,
    Ownable,
    Pausable
{
    ...
    uint256 public maxSupply = 48;  //It's test
    uint256 public cost = 100000000000000;  //It's test
    uint256 public mintedCount = 0;
    bytes32 public merkleRoot;
    mapping(address => uint256) public userMintedAmount;

    ISattvaSoulSupporters public sss;
        ...
        whenNotPaused
    {
        ...
        if(_mintAmount + userMintedAmount[msg.sender] > _allowedMintAmount) revert MaxMintPerAddressExceeded();
        if(_mintAmount + mintedCount > maxSupply) revert MaxSupplyExceeded();
        ...
        sss.mint(msg.sender, _mintAmount);
    }

    function airDrop(address[] calldata _airdropAddresses , uint256[] memory _usersMintAmount)
        external
        onlyRole(AIRDROPPER)
    {
        if(_airdropAddresses.length != _usersMintAmount.length)
            revert NotMatchLengthAddressesAndUsers();
        
        uint256 _mintAmount = 0;
        for(uint256 i=0; i<_usersMintAmount.length; i++){
            _mintAmount += _usersMintAmount[i];
        }
        ...
        for(uint256 i=0; i<_airdropAddresses.length; i++){
            ...
            //userMintedAmount[_airdropAddresses[i]] += _usersMintAmount[i];
            sss.mint(_airdropAddresses[i], _usersMintAmount[i]);
        }
    }
    ...
    function setSSS(address value)
        external
        onlyRole(ADMIN)
    {
        sss = ISattvaSoulSupporters(value);
    }
