...
contract Transfer {
  ...
  function transfer(
    ...
    CallParams memory callParams = CallParams({
      ...
      destinationMinOut: 0 // the minimum amount that the user will accept due to slippage from the StableSwap pool (1% here)
    });
    ...
    XCallArgs memory xcallArgs = XCallArgs({
      ...
      originMinOut: 0 // the minimum amount that the user will accept due to slippage from the StableSwap pool (1% here)
    });

    connext.xcall(xcallArgs);

    emit TransferInitiated(asset, msg.sender, recipient);
  }
}
