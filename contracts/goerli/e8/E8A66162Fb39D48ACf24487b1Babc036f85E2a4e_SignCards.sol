...
contract SignCards is Ownable, ERC1155Holder {
    using SafeERC20 for IERC20;

    ERC1155Burnable public parallelAlpha =
        ERC1155Burnable(0x76BE3b62873462d2142405439777e971754E8E77);
    ERC1155Invoke public signedParallelAlpha =
        ERC1155Invoke(0x76BE3b62873462d2142405439777e971754E8E77);
    IERC20 public prime = IERC20(0xb23d80f5FefcDDaa212212F028021B41DEd428CF);
    ...
    event CardsSigned(address from, uint256[] tokenIds, uint256[] quantities);

    constructor(
        address _parallelAlpha,
        address _signedParallelAlpha,
        address _prime,
        ...
        parallelAlpha = ERC1155Burnable(_parallelAlpha);
        signedParallelAlpha = ERC1155Invoke(_signedParallelAlpha);
        prime = IERC20(_prime);
        feeReceiver = _feeReceiver;
        price = _price;

        if (
            _tokenIds.length != _supply.length ||
            _tokenIds.length != _tokenUris.length
        ) revert ParamLengthMissMatch();

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            maxSupply[_tokenIds[i]] = _supply[i];
            tokenUri[_tokenIds[i]] = _tokenUris[i];
        }
    }

    function setParallelAlpha(address _parallelAlpha) external onlyOwner {
        parallelAlpha = ERC1155Burnable(_parallelAlpha);
    }

    function setSignedParallelAlpha(address _signedParallelAlpha)
        external
        onlyOwner
    {
        signedParallelAlpha = ERC1155Invoke(_signedParallelAlpha);
    ...
    function setTokenIdsSupply(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (totalSupply[_tokenIds[i]] > _supply[i])
    ...
    function onERC1155Received(
        ...
        _signCards(operator, _tokenIds, _quantities);
    ...
    function onERC1155BatchReceived(
        ...
        _signCards(operator, ids, values);
        return this.onERC1155BatchReceived.selector;
    }
    ...
    function _signCards(
        ...
        uint256 totalQuantities;
        string[] memory _tokenUri = new string[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (
                maxSupply[_tokenIds[i]] <
                totalSupply[_tokenIds[i]] + _quantities[i]
            ) revert MaxSupplyReached();

            totalQuantities += _quantities[i];
            totalSupply[_tokenIds[i]] += _quantities[i];
            parallelAlpha.burn(address(this), _tokenIds[i], _quantities[i]);
            _tokenUri[i] = tokenUri[_tokenIds[i]];
        }

        signedParallelAlpha.mintBatch(
            _from,
            _tokenIds,
            _quantities,
            _tokenUri,
        ...
        prime.safeTransferFrom(_from, feeReceiver, totalQuantities * price);

        emit CardsSigned(_from, _tokenIds, _quantities);
    }
}