...
// Thrown if zero address is provided as owner
...
contract Diamond {
    ...
    constructor(
        ...
        if (_contractOwner == address(0)) revert ZeroOwnerAddress();
        ...
        // Note that maxUpdateIdToFees is initialized with 0 by default.
        gs.updateIdToFees[0] = LibDIVAStorage.Fees({
            startPoolId: 0,
            protocolFee: 2500000000000000, // 0.25%
            settlementFee: 500000000000000 // 0.05%
        });

        // Initialize settlement related parameters. Note that
        // maxUpdateIdToSettlementPeriods is initialized with 0 by default.
        gs.updateIdToSettlementPeriods[0] = LibDIVAStorage.SettlementPeriods({
            startPoolId: 0,                     // poolId at which the fees become applicable
            submissionPeriod: 7 days,           // days since expiry; max value: 15 days <= 2^24
            challengePeriod: 2 days,            // days since time of submission; max value: 15 days <= 2^24
            reviewPeriod: 5 days,               // days since time of challenge; max value: 15 days <= 2^24
            fallbackSubmissionPeriod: 10 days   // days since the end of the submission period; max value: 15 days <= 2^24
        });
        ...
        // Initializing EIP712 domain separator and store chain id to protect against replay attacks
...
library LibEIP712 {
    ...
    function _fillOfferAddLiquidityLib(
        ...
        LibDIVA._checkAddLiquidityAllowed(_poolParameters, _makerFillAmount + _takerFillAmount);
        ...
        if (_poolParams.collateralToken != address(0)) {
            ...
            if (offerInfo.status != OfferStatus.FILLABLE) {
                ...
                if (_offerRemoveLiquidity.makerIsLong) {
                    _makerPositionTokenBalance = IERC20(_poolParams.longToken)
                        .balanceOf(_offerRemoveLiquidity.maker);
                } else {
                    _makerPositionTokenBalance = IERC20(_poolParams.shortToken)
                        .balanceOf(_offerRemoveLiquidity.maker);
                }
...
// Thrown inside `removeLiquidity` if return collateral is paused
error ReturnCollateralPaused();

// Thrown inside `removeLiquidity` if status of `finalReferenceValue`
// is already "Confirmed"
error FinalValueAlreadyConfirmed();

// Thrown inside `removeLiquidity` if a user's short or long position
// token balance is smaller than the indicated amount 
error InsufficientShortOrLongBalance();

// Thrown if `_amount` provided by user in `removeLiquidity` results
// in zero protocol fee amount; user should increase `_amount` argument
// to avoid this from happening
error ZeroProtocolFee();

// Thrown if `_amount` provided by user in `removeLiquidity` results
// in zero settlement fee amount; user should increase `_amount` argument
// to avoid this from happening
...
library LibDIVA {
    ...
    function _createContingentPoolLib(
    ...
    function _addLiquidityLib(
        ...
        uint256 _collateralAmountIncr = addLiquidityParams.collateralAmountMsgSender + addLiquidityParams.collateralAmountMaker;
    ...
    function _checkAddLiquidityAllowed(LibDIVAStorage.Pool memory _pool, uint256 _collateralAmountIncr) internal view {        
    ...
    function _removeLiquidityLib(
        ...
        // Initialize Pool struct
        LibDIVAStorage.Pool storage _pool = ps.pools[_removeLiquidityParams.poolId];
        ...
        if (
            shortToken.balanceOf(_removeLiquidityParams.shortTokenHolder) < _removeLiquidityParams.amount ||
            longToken.balanceOf(_removeLiquidityParams.longTokenHolder) < _removeLiquidityParams.amount
        ) revert InsufficientShortOrLongBalance();

        // Get fee parameters applicable for given `_poolId`
        LibDIVAStorage.Fees memory _fees = _getFees(_removeLiquidityParams.poolId);
        ...
        shortToken.burn(_removeLiquidityParams.shortTokenHolder, _removeLiquidityParams.amount);
        longToken.burn(_removeLiquidityParams.longTokenHolder, _removeLiquidityParams.amount);
        ...
        // contract and can be claimed via `claimFee` function.
        _allocateFeeClaim(_removeLiquidityParams.poolId, _pool.dataProvider, _settlementFee);

        // Collateral amount to return net of fees
        collateralAmountRemovedNet = _removeLiquidityParams.amount - _protocolFee - _settlementFee;
