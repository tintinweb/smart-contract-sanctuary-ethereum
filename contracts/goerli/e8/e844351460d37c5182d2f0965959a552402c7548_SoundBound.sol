...
//    uint256 private whitelistPurchasePrice = 0 wei;
    ...
    event logNFTCreatedWithSubdomain(uint256 indexed curTokenId, address indexed ownerWalletAddress, string subdomain, uint32 genre);
    event logNFTCreatedWithOwner(uint256 indexed curTokenId, address indexed ownerWalletAddress, string subdomain, uint32 genre);
    ...
    // Subdomain syntax check
    strings.slice invalidSubdomainChar = ".".toSlice();
    ...
    function subdomainCheck(string memory subdomain) internal view returns (bool) {
        strings.slice memory domainSlice = subdomain.toSlice();
        return strings.contains(domainSlice, invalidSubdomainChar);
    }
    ...
    // Purchase music NFT use domain
    function mintNFTFromDomain(string memory name, uint32 genreId) external payable {
    ...
    // Purchase music NFT use subdomain
    // function mintNFTFromSubdomain(string memory subdomain) external payable {
    //     require(!paused, "The contract has been paused");
    //     if (isWhitelistPeriod) {
    //         require(isWhitelisted(msg.sender), "Sorry, you are not being whitelisted, please mint during the public sales period.");
    //     }
    //     require(!subdomainCheck(subdomain), "Please don't include character '.' in the subdomain.");
    ...
    //     domainToTokenId[name] = newItemId;
    //     purchaseStatus[name] = true;
    //     emit logNFTCreatedWithSubdomain(newItemId, msg.sender, name, 0);
    // }

        function mintNFTFromSubdomain(string memory subdomain) external view returns (string memory) {
        require(!paused, "The contract has been paused");
        if (isWhitelistPeriod) {
            require(isWhitelisted(msg.sender), "Sorry, you are not being whitelisted, please mint during the public sales period.");
        }
    ...
    // Purchase music NFT by admin account
    function mintNFTByAdmin(string[] memory domainMap, address[] memory addressMap) external onlyOwner {
        require(!paused, "The contract has been paused");
        for (uint i=0; i<domainMap.length; i++) {
            ...
            domainToTokenId[name] = newItemId;
            purchaseStatus[name] = true;
            emit logNFTCreatedWithOwner(newItemId, msg.sender, name, 0);
        }
    }
    ...
    function getTokenIdByNode(string memory name) external view returns (uint256) {
        require(purchaseStatus[name] == true,  "The NFT hasn't been purchased yet.");
