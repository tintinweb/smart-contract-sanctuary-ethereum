...
contract CimoStaking is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    function withdraw(uint32 _pid, uint256 _amount) public nonReentrant {
        ...
        user.withdrawn = uint112(user.withdrawn + _amount);
    ...
    function addLiquidityNoFeeAndStake(
        ...
        require(
            (lockTime >= 604800 &&
                ...
                (lockTime == 0 && user.lockedUntil >= block.timestamp),
            "addLiquidityNoFeeAndStake : Can't lock tokens for less than 1 week"
        );
        updatePool(0);
        if (user.weightedBalance > 0) {
        ...
        (, uint256 ETHUsed, uint256 numLiquidityAdded) = IUniswapV2Router02(
            params.pair
        ).addLiquidityETH(
            address(this),
    ...
    function reinvestETHRewards(uint256 amountOutMin) public nonReentrant {
        ...
        user.withdrawn += uint112(ETHPending);
        user.claimableETH = 0;
        ...
        if (_amount > 0) {
            require(
                (lockTime >= 604800 &&
                    ...
                    user.lockedUntil <= lockTime + block.timestamp) ||
                    (lockTime == 0 && user.lockedUntil >= block.timestamp),
                "deposit : Can't lock tokens for less than 1 week"
            );
    ...
    function lockTokens(
        ...
        user.lockTimeBoost = uint16(
            (2 * 1000 * (lockTime - 604800)) / 30844800
        ); // 0 - 2000
        updateUserWeightedBalance(_pid, sender);
    ...
    function updateUserWeightedBalance(uint256 _pid, address _user) public {
        ...
        if (user.lockedUntil < block.timestamp) {
            ...
            user.weightedBalance =
                (user.amount *
                    (1000 + user.lockTimeBoost) *
                    (1000 + user.boostPointsBP)) /
                1000000;
        }
