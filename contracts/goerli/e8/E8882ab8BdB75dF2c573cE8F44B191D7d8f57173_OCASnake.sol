...
contract OCASnakeArt is Ownable, ReentrancyGuard {
    ...
    function allYourBaseAreBelongToUs(uint256 tokenId)
        ...
        returns (string memory)
    {
        string[] memory selectedColors = getAppleIndex(tokenId);
    ...
    function retrofy(uint256 tokenId) private view returns (string[] memory) {
        string[] memory parts;
        ...
        for (uint256 i = 0; i < 20; i++) {
            for (uint256 index = 0; colors.length - 1 < 3; index++) {
                generateColors(tokenId, i == 0 ? 420 : 69 + i);
            }
            ...
            parts[1] = funkToString(colors[0], colors[1], colors[2], colors[3]);
        }
