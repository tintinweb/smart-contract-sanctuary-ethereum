...
interface ISwap {
    ...
    function offer(uint256 swapId, ItemNFT[] calldata nfts, ItemCoin[] calldata coins, uint256 currency) external payable;
...
contract Swap is OwnableUpgradeable, ISwap {
    ...
    function offer(
        ...
        require(msg.value == currency, "currency not match");
    ...
    function accept(uint256 offerId) external payable override {
        ...
        payable(idToOwner[offerIdToSwapId[offerId]]).transfer(
            offerIdToCurrency[offerId]
        );
    ...
    function removeOffer(uint256 offerId) external override {
        require(offerIdToUser[offerId] == msg.sender, "You are not the bidder");
        offerOpened[offerId] = false;
        // refund currency
        payable(msg.sender).transfer(offerIdToCurrency[offerId]);
