    ...
    W3JEPayable
{
    ...
    uint256 public createCost = 0.1 ether;
    ...
    modifier enoughEthForCreate() {
        require(msg.value >= createCost, "not enough eth.");
        _;
    }

    function addToken(uint256 cost) external payable enoughEthForCreate {
        tokenOwner[nextId] = msg.sender;
        mintCost[nextId] = cost;
        nextId++;
    }

    function setCreateCost(uint256 value) external onlyAdmin {
        createCost = value;
    }

    function setMintCost(uint256 id, uint256 cost) external {
        require(msg.sender == tokenOwner[id], "you are not token owner.");
        mintCost[id] = cost;
    }
    ...
    modifier enoughEthForMint(uint256 id) {
        require(msg.value >= mintCost[id], "not enough eth.");
        _;
    }

    modifier hasRight(uint256 id, bytes calldata signature) {
        require(
            keccak256(abi.encodePacked(msg.sender, id, nonce[msg.sender]))
                .toEthSignedMessageHash()
                .recover(signature) == getSigner(),
            "invalid proof."
        );
    ...
    modifier existsId(uint256 id) {
        require(id < nextId, "not exist id.");
        _;
    }

    function mint(uint256 id, bytes calldata signature)
        external
        payable
        existsId(id)
        enoughEthForMint(id)
        hasRight(id, signature)
    {
    ...
    function setBaseURI(string memory _value) external override onlyAdmin {
        baseURI = _value;
    }
    ...
    // Payable
    // ==================================================================
    function setWallet(address value) external override onlyAdmin {
        _setWallet(value);
    }
}
