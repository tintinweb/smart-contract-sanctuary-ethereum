...
/// @param _indexer Address to register Graphcast ID to.
...
contract GraphcastRegistryV2 is GraphcastRegistry{
     ...
     * @dev Emitted when `indexer` sets `graphcastID` access.
     */
    event StakingChanged(address indexed staking);

    function setStaking(address _addr) external onlyOwner {
        staking_addr = _addr;
        emit StakingChanged(staking_addr);
    }
     ...
     * @param _indexer Indexer address to authorize Graphcast ID to
     * @param _graphcastID Address to authorize as the Graphcast ID
     */
    function setGraphcastID(address _indexer, address _graphcastID) external {
        if (!_isAuth(_indexer))
