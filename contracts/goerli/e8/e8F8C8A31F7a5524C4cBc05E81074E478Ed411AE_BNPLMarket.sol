    ...
    IBNPLMarket
{
    ...
    event CancelledDiscreteOrder(uint256 bidId);
    ...
    */
    function onUpgrade(
        ...
        require(keccak256(abi.encode(upgradedToVersion)) != keccak256(abi.encode(CONTRACT_VERSION)), "already upgraded");
        upgradedToVersion = CONTRACT_VERSION;
    ...
    function submitDiscreteOrder(
        ...
        discreteOrders[bidId] = DiscreteOrder({
            ...
            expiration: _submitBidArgs.signatureExpiration,
            assetContractAddress: _assetContractAddress,
            assetTokenId: _assetTokenId,
            quantity: _quantity,
            lender: address(0),
            referralAddress: _submitBidArgs.referralAddress,
            principal: _submitBidArgs.principal,
            downPayment: _submitBidArgs.downPayment
        });
    ...
    function acceptDiscreteOrder(
        ...
        require(
            block.timestamp < order.expiration,
        ...
        _collectDownpaymentForDiscreteOrder( order );

        _acceptBidAndPurchaseNFT(
            _basicOrderParameters,
            _bidId,
            order.totalPurchasePrice,
            discreteOrders[_bidId].downPayment,
            order.lender
        );

        _payReferralFee(
            discreteOrders[_bidId].principal,
            discreteOrders[_bidId].referralAddress
        );
    }

      /**
     * @notice Transfer weth downpayment from the borrower into this contract so it can be used to purchase NFT immediately.
     * @param _order Discrete order details 
     */

    function _collectDownpaymentForDiscreteOrder( DiscreteOrder memory _order ) internal {

        IWETH(wethAddress).transferFrom( _order.borrower, address(this), _order.downPayment );

        _unwrapWeth(_order.downPayment);
    ...
    function _acceptBidAndPurchaseNFT(
        ...
        _repayLoanWithExcess(_bidId, _purchasePrice, _downPayment, ethGainedFromAcceptBid); 
     ...
     */
    function _repayLoanWithExcess(uint256 _bidId, uint256 _purchasePrice, uint256 _downPayment, uint256 _ethGainedFromAcceptBid  ) 
    internal {

        uint256 excessAmount =   ( _downPayment + _ethGainedFromAcceptBid) - _purchasePrice;
     ...
     */
    function _unwrapWeth(uint256 amt)
        internal
        virtual
        returns (uint256)
    {
     ...
     * @param amt Amount to unwrap in wei.
     */
    function _wrapWeth(uint256 amt)
        internal
        virtual
        returns (uint256)
    {
