...
contract MockERC20Factory {
    ...
    function createMockERC20(string calldata name_, string calldata symbol_, uint8 decimals_) public {
        MockERC20 _MockERC20 = new MockERC20(name_, symbol_, decimals_);
    ...
    function createMockERC20Batch(string[] calldata names_, string[] calldata symbols_, uint8[] calldata decimals_) external {
        require(names_.length == symbols_.length, "length mismatch");
        uint256 l = names_.length;
        uint256 i; unchecked { do {
            createMockERC20(names_[i], symbols_[i], decimals_[i]);
        } while(++i < l); }
    }
}