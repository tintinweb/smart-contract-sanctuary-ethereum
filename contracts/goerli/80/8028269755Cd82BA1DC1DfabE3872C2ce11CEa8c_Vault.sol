...
contract RemoteClaim is IPlug {
    ...
    constructor(address _socket, address _vaultAddress) {
        socket = _socket;
    ...
    function inbound(
        ...
        require(msg.sender == socket, "Counter: Invalid Socket");
        (address rewardHolder, address recipient, uint256 amount) = abi.decode(
            payload,
        ...
        vault.claimRewards(rewardHolder, recipient, amount);
    }

    function claimRemoteRewards(address recipient, uint256 amount, uint256 targetChain, uint256 msgGasLimit) external payable {
        bytes memory payload = abi.encode(msg.sender, recipient, amount);
        _outbound(targetChain, msgGasLimit, payload);
    }
...
contract Vault {
    ...
    event RewardsClaimed(
        address indexed rewardHolder,
        address indexed recipient,
        uint256 indexed claimedRewardAmount
    );
    ...
    address public remoteClaim;

    mapping(address => uint256) rewardStore;
    ...
    function setRemoteClaim(address _remoteClaim) public onlyOwner {
        remoteClaim = _remoteClaim;
    }

    modifier onlyRemoteClaim() {
        require(
            msg.sender == remoteClaim,
            "can only be called by remoteClaim contract"
        );
        _;
    }
    ...
    function addRewards(
        ...
        rewardStore[claimHolder] += amount;
        emit RewardsAdded(claimHolder, amount);
    }
    ...
    function claimRewards(
        ...
        address recipient,
        uint256 amount
    ) external payable onlyRemoteClaim {
        require(amount >= rewardStore[rewardHolder], "Not enough rewards");
        payable(recipient).transfer(amount);
        emit RewardsClaimed(rewardHolder, recipient, amount);
    }
}
