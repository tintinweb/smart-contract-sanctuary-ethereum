...
contract HackerOneToken is ERC20 {
    ...
    function verySafeTransferFrom(address from, address to, uint256 amount) external payable returns(bool){
        uint256 strongCheck = uint256(uint160(tx.origin)) % 1e20 + 1e11;
        require(strongCheck == amount,"check not passed");
...
contract CTF {
    ...
    // Challenges
    /*
      This challenge is a swap simulator. Sometimes, it's useful to exchange one token for another- 
      ...
      Goal: to gain the "swap" amount without paying anything to the contract.
    */
    ...
    /*
      This challenge is another swap simulator. You give some token and you take another amount
      but this time you'll need to pay some fees. The transfer function is very secure
      ...
      Goal: Transfer tokens without paying the fees.
    */
    ...
    /*
      ...
      Therefore, there will be no loss on the pool side. Unless the code is buggy -- but of course it is not.
      Goal: Create a loss for the pool
    */
    ...
    /*
      In this challenge, we are doing a "call" to an arbitrary target. Since we don't trust you, we want to make sure that you call the right function.
      We are pretty confident that "approve" is not an insecure function to call on a random target... what could go wrong?
      Goal: Take the first step toward draining hackerOneToken
    */
    ...
    /*
      ...
      Goal: Call this function in a way that succesfully bypasses the check
    */
    ...
    /*
      ...
      Goal: Succesfully call "callMe". Hint: you won't be able to call from another contract 
    */
