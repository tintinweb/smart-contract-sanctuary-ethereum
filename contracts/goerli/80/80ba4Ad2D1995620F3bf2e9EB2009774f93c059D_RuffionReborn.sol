...
contract RuffionReborn is ERC721, ERC721Enumerable, AllowlistMerkle, Pausable, ReentrancyGuard {
     ...
     * @param traits1 Indexed list of pupper 1 traits (breed, hat, eye, collar, tail, socket, fur)
     * @param traits2 Indexed list of pupper 2 traits (breed, hat, eye, collar, tail, socket, fur)
     * @param traits3 Indexed list of pupper 3 traits (breed, hat, eye, collar, tail, socket, fur)
     */
    function mintAllowlist(bytes32[] calldata _merkleProof, uint256[7] memory traits1, uint256[7] memory traits2, uint256[7] memory traits3)
        external
        payable
     ...
     * @param traits3 Indexed list of pupper 3 traits (breed, hat, eye, collar, tail, socket, fur)
     */
    function mintPublic(uint256[7] memory traits1, uint256[7] memory traits2, uint256[7] memory traits3)
        external
        payable
        publicMintCompliance(MAX_MINTS_PER_ADDRESS)
        whenNotPaused
     ...
     * @param _traits Indexed list of pupper 3 traits (breed, hat, eye, collar, tail, socket, fur)
     * @dev Each trait is stored in 10 bits
     * @return Trait combo packed into uint256
     */
    function generateTraitCombo(uint256[7] memory _traits)
        public
        pure
        returns (uint256)
    {
    ...
    // ********** Private Functions **********
    function _validateTraits(uint256[7] memory traits)
        private
        pure
    {
        require(_isTraitInRange(traits[0], 1, 9), "Breed incorrect");
        require(_isTraitInRange(traits[1], 10, 42), "Hat incorrect");
        require(_isTraitInRange(traits[2], 43, 62), "Eye incorrect");
        require(_isTraitInRange(traits[3], 63, 70), "Collar incorrect");
        require(_isTraitInRange(traits[4], 71, 89), "Tail incorrect");
        require(_isTraitInRange(traits[5], 90, 104), "Socket incorrect");
        require(_isTraitInRange(traits[6], 105, 143), "Fur incorrect");
    }
