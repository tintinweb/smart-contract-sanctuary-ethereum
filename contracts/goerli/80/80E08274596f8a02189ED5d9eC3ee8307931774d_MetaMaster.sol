...
contract MetaMaster {
    ...
    mapping (address => uint) depositCount1;//
    ...
    address constant public USDTAddress = 0xE78bc727D8d0831A938E697647fEf82D195953E6;
    ...
    function deposit(uint256 _USDTAmount, address _referrer) public {
        ...
        uint refferBonus = _USDTAmount*5/100;      //
        uint creatorpay=_USDTAmount*2/100;
        ...
        if((depositCount1[msg.sender] ==0) && (depositCount1[_referrer]>=1)) {       
           //payable(_referrer).transfer(refferBonus);
            ...
            balances[msg.sender] += _USDTAmount - creatorpay - refferBonus;
            depositCount1[msg.sender] ++;
            
        }
        else { 
            balances[msg.sender] += _USDTAmount - creatorpay;
            depositCount1[msg.sender] ++;    
    ...
    function getUserTotalReturns(address _userAddress) public view returns(uint256){
        ...
        for(uint8 i = 0; i < users[_userAddress].deposits.length; i++) {
            uint256 reward;
            uint256 numberOfDays = block.timestamp.sub(users[_userAddress].deposits[i].depositedTimeStamp).div(10 minutes);
