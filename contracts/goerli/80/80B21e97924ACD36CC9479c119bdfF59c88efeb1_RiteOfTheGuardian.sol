...
contract RiteOfTheGuardian is RitualBindings {    
    ...
    address constant GENESIS_CONTRACT=0x96223b35FCc80E7F05eC58F1f8D5A55f6AFDe74B;

    string constant UNDEAD_KEY = 'undead';
    string constant MORTAL_KEY = 'mortal';
    ...
    function bestowBountyOfCharun(address recipient, uint256 quantity) internal {
        ...
        if (ResidualBrainz(brainzContract).balanceOf(address(this)) >= bounty) {
            ResidualBrainz(brainzContract).transfer(recipient,bounty);
        }
    }

    function elderRitual(uint64 quantity, uint256[] memory elders, bytes calldata sigil) 
    external requiresClaimSig(sigil,msg.sender,elders) {
        for (uint i = 0; i < elders.length; i++) {
            if (hasBeenClaimed(elders[i], GENESIS_CONTRACT)) {
                revert ElderAlreadyClaimed(elders[i]);
            }
            
            claim(elders[i], GENESIS_CONTRACT);
        }
    ...
    function initiateRitual(uint256 quantity, bytes calldata sigil) 
    external payable requiresAllowSig(sigil,msg.sender) {
    ...
    function openRitual(uint256 quantity) external payable {
        Phase memory phased = findPhase(OPEN);
        
        giveCharunHisCoin(phased.cost, quantity);
        
        phasedMint(OPEN, quantity, false);
    ...
    function necroscribe(uint256 tokenId) external payable {

        necrophize(tokenId);

        if (inscriptionExists(NECRO,tokenId)) {
            revert AlreadyInscribed();
        }        
        
        giveCharunHisCoin(inscriptionCost,1);

        script(NECRO,tokenId);    
    }  

    function inscribeUndead(string memory inscription, uint256 tokenId) external onlyOwner {
        inscript(inscription,NECRO,tokenId);
    }
    ...
    function vitascribe(uint256 tokenId) external payable {

        vitalize(tokenId);

        if (inscriptionExists(MORTAL,tokenId)) {
            revert AlreadyInscribed();
        }

        giveCharunHisCoin(inscriptionCost,1);

        script(MORTAL,tokenId);    
    }  

    function inscribeMortal(string memory inscription, uint256 tokenId) external onlyOwner {
        inscript(inscription,MORTAL,tokenId);
    }  
    ...
    function summonedHorde() external view returns (uint256) {
        return getMaxSupply();
    }

    function raiseDead(uint256 newHorde) external onlyOwner {
        setMaxSupply(newHorde);
    }           
    ...
    function giveCharunHisCoin(uint256 cost, uint256 quantity) internal {   
        ...
        CHARUN.transfer(cost*quantity);
    }      
    function giveCharunHisBrainz(uint256 cost, uint256 quantity) internal {   
        ResidualBrainz(msg.sender).transfer(CHARUN,quantity*cost);          
    }        
}
...
import { SetInscribable, InscribableData } from "./SetInscribable.sol";
...
contract RitualBindings is EIP712Allowlisting {  
    ...
    address payable internal CHARUN = payable(defaultPayable);
    uint256 ELDER = 0;
    uint256 INITIATE = 1;
    uint256 OPEN = 2;
    uint256 NECRO = 0;
    uint256 MORTAL = 1;
    address hordeContract = address(0x96223b35FCc80E7F05eC58F1f8D5A55f6AFDe74B);  
    address brainzContract = address(0xe1d4892E63f15d3d24Af4C71393956a588199B1e);    
    mapping(address => uint256) elderMints;
    ...
    constructor(string memory name, string memory symbol) FlexibleMetadata(name,symbol) {
        ...
        Phase[] storage phases = getPhases();
                          //phase       //max per wallet  //max for phase   //cost
        phases.push(Phase("elders",     32,               380,              0));
        phases.push(Phase("initiates",  2,                48,               2500)); // BRAINZ
        phases.push(Phase("acolytes",   3,                888,              .02 ether)); // eth (410 after phases 1 & 2)
        phases.push(Phase("open",       4,                1001,             .04 ether)); // eth (63 after phases 1, 2, 3)
        /** airdrop first 50 to treasury 410 * .02 = 8.2 eth; 63 * .04 = 2.5 ~ 10 eth */
        initialize(phases,1001);
    }
    ...
    function canMint(uint256 phase, uint256 quantity) internal override virtual returns(bool) {
        ...
        uint256 requestedMints = getSenderMints()+quantity-elderMints[msg.sender];
    ...
    function script(uint256 scriptClass, uint256 tokenId) internal {
      inscribable.script(scriptClass,tokenId);
    }    
    ...
    function findInscription(uint256 scriptClass, uint256 tokenId) internal view returns (string memory) {
      return inscribable.findInscription(scriptClass,tokenId);
    }
    function inscriptionExists(uint256 scriptClass, uint256 tokenId) internal view returns (bool) {
      return inscribable.inscriptionExists(scriptClass,tokenId);
    }
}
