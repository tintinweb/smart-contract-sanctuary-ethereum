...
// NightJackalIssue1 Contract v1.0.1
// Creator: Nothing Rhymes With Entertainment

pragma solidity >=0.8.9 <0.9.0;

import "./NRWE/BridgeBlockChainShrine.sol";
import "./NRWE/ComicBookPublication.sol";

contract NightJackalIssue1 is ComicBookPublication, BridgeBlockChainShrine {
    constructor(
            string memory _name, 
            string memory _symbol, 
            uint256 _maxSupply, 
            string memory _series_name,
            string memory _title,
            uint32 _issueNum, 
            uint8 _volume, 
            Issue.BookType _type, 
            uint8 _printing,
            string memory _image_uri,
            string memory _initBaseUri
     ) ComicBookPublication(_name, 
                            _symbol, 
                            _maxSupply, 
                            _series_name, 
                            _title, 
...
// SPDX-License-Identifier: MIT
// ComicBookPublication Contract v2.2.1
// Creator: Nothing Rhymes With Entertainment

pragma solidity >=0.8.9 <0.9.0;

import "./VerifiableERC721AMint.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "./library/Issue.sol";

/**
 * @title ComicBookPublication
 * @author Heath C. Michaels, (@heathcmichaels @wanderingme @wanderingheath) 
 * @dev Comic book publication contract extending Chiru Labs' ERC721A standard
 *
 * @notice Contract creates a scrapable ISSUE object containing: 
 *
 *          Series Name (e.g., "Night Jackal")
 *          Title (i.e., story's title)
 *          Volume (i.e., current run)
 *          Issue Number
 *          Book Type (enum)
 ...
 *          Printing (e.g., first printing)
 *          Image Uri 
 *
 *          Usage: Issue("Night Jackal", "The Prince", 1, 1, 0, 1, "ipfs://CID");
 *
 */
 
contract ComicBookPublication is VerifiableERC721AMint{
    using Issue for Issue.data;
    Issue.data public CURRENT_ISSUE;
     ...
     constructor(
            ...
            CURRENT_ISSUE = Issue.data(_series_name, _title, _volume, _issueNum, _type, _printing, _image_uri);
    }

   function tokenURI(uint256 tokenId) public view virtual override (BaseERC721AMint) returns (string memory){
    return string(abi.encodePacked(_baseURI(),  Strings.toString(CURRENT_ISSUE._issueNum), "/", Strings.toString(tokenId), baseExt));
   }
    
}

// SPDX-License-Identifier: MIT
// BridgeBlockChainShrine Contract v1.0.0
// Creator: Nothing Rhymes With Entertainment

/**

 * @title BridgeBlockChainShrine
 * @author Heath C. Michaels, (@heathcmichaels @wanderingme @wanderingheath)
 *
 * @notice Connects publishing contract to Blockchain Shrine without the bloat
...
import "./library/ShoutOut.sol";
import "./interface/IBlockChainShrine.sol";
import "./library/ExternalContractManager.sol";

contract BridgeBlockChainShrine is Ownable{

    using ExternalContractManager for ExternalContractManager.Data;

    /**
    *   @dev External contract manager used to track contract for IBlockChainShrine
    *           1. _contract = External contract address (IBlockChainShrine type casting necessary)
    */     
    ExternalContractManager.Data private connectedBlockChainShrine;

     /**
    *   @dev sets block chain shrine contract
    */  
    function setBlockChainAddress(address _addy) external onlyOwner {
        connectedBlockChainShrine._contract = address(_addy);
    }
     /**
    *   @dev returns tuple (string, string) using single address
    */  
    function getShoutOutByAddress(address _user) public view returns (ShoutOut.data memory){
        return IBlockChainShrine(connectedBlockChainShrine._contract).getShoutOutByAddress(_user);
    }
    /**
    *   @dev returns tuple (string, string) requiring parsing [name, link, name, link]
    */
    function getAllShoutOuts() public view returns (ShoutOut.data[] memory){
        return IBlockChainShrine(connectedBlockChainShrine._contract).getAllShoutOuts();
    }
}

// SPDX-License-Identifier: MIT
// Library Issue Object v1.0.0
// Creator: Nothing Rhymes With Entertainment
...
library Issue {
    ...
    /**
    *
    *@dev BookType is an enum designating the type of publication. Options are as follows: 
    *
    *        BOOK TYPES
    *        0 Serialized - Most common type when referring to comic books, aka "floppy" comics. These are periodical, serialized excerpts from a larger story, such as Amazing Spider-Man #1, #2, and so on.
    *        1 One Shot - A single issue released for a one-time occassion.
    ...
    */
    ...
    /**
    *  @dev Issue struct is scrapable object of the publication information.
    *
    *       Usage: Issue("Night Jackal", "The Prince", 1, 1, 0, 1, "ipfs://CID");
    *
    *       Volume and Printing max value is 256
    *       Issue Nums max out at 4294967295
    *
     */
  struct data {
     //Issue Series Name
...
// SPDX-License-Identifier: MIT
// IBlockChainShrine Contract v1.0.0
// Creator: Nothing Rhymes With Entertainment
...
interface IBlockChainShrine {
    function getShoutOutByAddress(address _user) external view returns (ShoutOut.data memory);
    function getAllShoutOuts() external view returns (ShoutOut.data[] memory);
}

// SPDX-License-Identifier: MIT
// Library ShoutOut Data Contract v1.0.0
// Creator: Nothing Rhymes With Entertainment
...
library ShoutOut {
    struct data {
     //"@heathcmichaels" or "Heath C. Michaels"
        string _handle;
        //"http://linktr.ee/NightJackal/"
        string _link;
   }
}
