...
contract SumoMarketV4 is Ownable {
    ...
    function MAKEBID(uint256 id_, BIDINFO memory BIDINFO_) external payable onlySender {
        require(BIDINFO_.bidder == msg.sender, "Not sender");
        require(BIDINFO_.priceID > 0, "Invalid price");
        require(msg.value == BIDINFO_.priceID, "Value sent is not correct");

        BID[id_].push(BIDINFO_);
        emit BIDDONE(msg.sender, id_, BIDINFO_.priceID);
    }

    function CANCELBID(uint256 id_) external onlySender {
        require(id_ > 0 && id_ < 334, "Invalid token ID");
    ...
    function ACCEPTBID(uint256 id_, address bidby_) external onlySender {
        require(id_ > 0 && id_ < 334, "Invalid token ID");
        require(ERC721.ownerOf(id_) == msg.sender, "You do not own this token");
