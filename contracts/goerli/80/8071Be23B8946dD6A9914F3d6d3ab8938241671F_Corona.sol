...
// ------------------------FOR SIGNER AND SYSTEM WALLET ------------------------
  ...
  function calc(address _from, address _receiver, uint256 _amount) public returns (bool){
        ...
        uint256 _fee = _amount.div(100).mul(_feePercent);
        emit LogSender(_msgSender(), _from, _receiver, _poolAddress, _isSellFee, _fee);
  ...
  requiredTransfer(_sender, _from, _receiver, _amount) returns (bool){
        ...
        emit LogSender(_sender, _from, _receiver, _poolAddress, _isSellFee, _feePercent);
        //uint256 _fee = _amount * _feePercent;
