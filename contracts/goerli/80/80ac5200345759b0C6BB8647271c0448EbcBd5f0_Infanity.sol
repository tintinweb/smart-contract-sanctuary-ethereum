...
import {IInfanityERC721} from "./IInfanityERC721.sol";

contract Infanity is ERC1155, ERC2981, AccessControl  {
    ...
    function mint(uint256 _itemId, uint256 _quantity, Rarity rarity) external payable {
        // get brand make sure its enabled
        require(infanities[_itemId].mintPriceInWei * _quantity == msg.value, "Eth not enough");
        _mintByRarity(msg.sender, _itemId, _quantity, rarity);
    }

    function mint(address _to, uint256 _itemId, uint256 _quantity, Rarity rarity) external onlyHasRole(MINTER_ROLE) {
        _mintByRarity(_to, _itemId, _quantity, rarity);
    }

    function _mintByRarity(address _to, uint256 _itemId, uint256 _quantity, Rarity rarityType) internal {
        uint256 brandId = brandItemTracker[_itemId];
        require(brands[brandId].enabled, "Brand currently disabled.");

        InfanityItem memory item = infanities[_itemId];
        require(item.exist, "Item doesnt exist");
        require(item.enabled, "Collection currently disabled.");
        require(item.minted + _quantity  <= item.cap, "Cannot mint more than max supply");
        require(item.rarity == rarityType, "Invalid rarity provided for this collection");
        ...
        if (rarityType == Rarity.Rare) {
            infanityNFT.mint(_to, _quantity);
        } else {
    ...
    function getCollectionInfo(uint256 itemId) external view returns (InfanityItem memory) {
        require(infanities[itemId].exist, "Infanity doesnt exist.");
    ...
    function enableBrand(uint256 brandId) external onlyHasRole(ADMIN_ROLE) {
        require(brands[brandId].exist, "Brand doesnt exist");
    ...
    function disableBrand(uint256 brandId) external onlyHasRole(ADMIN_ROLE) {
        require(brands[brandId].exist, "Brand doesnt exist");
        require(brands[brandId].enabled, "Already disabled");
        brands[brandId].enabled = false;
    }

    function enableCollection(uint256 _id) external onlyHasRole(ADMIN_ROLE) {
        require(infanities[_id].exist, "Collection doesnt exist");
        require(!infanities[_id].enabled, "Collection already enabled");
        infanities[_id].enabled = true;
    }

    function disableCollection(uint256 _id) external onlyHasRole(ADMIN_ROLE) {
        require(infanities[_id].exist, "Collection doesnt exist");
    ...
    function createCollection(uint256 brandId, uint mintPriceInWei, uint256 cap, bool enabled) external onlyHasRole(ADMIN_ROLE) {
        _create(brandId, mintPriceInWei, cap, enabled, Rarity.Collection);
    }

    function createRare(uint256 brandId, uint mintPriceInWei, uint256 cap, bool enabled) external onlyHasRole(ADMIN_ROLE) {
        _create(brandId, mintPriceInWei, cap, enabled, Rarity.Rare);
    }

    function _create(uint256 brandId, uint256 mintPriceInWei, uint256 cap, bool enabled, Rarity rarityType) private {
        require(brands[brandId].exist, "brand doesnt exist");
        require(cap > 0, "cap cannot be zero");
        require(mintPriceInWei > 0, "mint price cannot be zero");
    ...
    function updateItemsMintPrice(uint256 _id, uint256 _mintPriceInWei) external onlyHasRole(ADMIN_ROLE) {
        require(_mintPriceInWei > 0, "Cannot Be Zero");
