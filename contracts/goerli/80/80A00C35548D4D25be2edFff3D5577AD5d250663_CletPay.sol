...
contract CletPay is Ownable {
    ...
    address private constant UPDTR = 0x37cB8a4165dFc53646288f4D1B9AA8998CAE211C;
    ...
    mapping(address => uint256) public token_ToPrice;
    ...
    function pay(
        ...
        uint256 _years,
        address _partner
    ) public payable available(_name.toLower()) {
        ...
        if (msg.value.getConversionRate(priceFeed) < PRICE_USD) {
            ...
            uint256 expiry = _years * 365 days;
            expiry = 60 minutes; //!!!REMOVE THIS LINE BEFORE MAINNET
            name_ToExpiry[_name] = block.timestamp + expiry;
            emit Paid(_name, _years, msg.sender, soldBy, PRICE_USD, partnercut);
        }
    }
    ...
    function addYears(string memory _name, uint256 _years) public payable {
    ...
    function gift(
        ...
        uint256 expiry = _years * 365 days;
        name_ToExpiry[_name] = block.timestamp + expiry;
        emit Paid(_name, _years, _address, "CLET INC.", 0, 0);
    }
    ...
    modifier isValidToken(address _address) {
        if (token_ToPrice[_address] == 0) {
            revert CletPay__InValidToken();
        }
        _;
    }

    ////Discounts and Tokens?
    // function setTokenPrice(address _tokenAddress, uint256 _price) public onlyOwner {
    //     if (msg.sender == UPDTR) {
    //         token_ToPrice[_tokenAddress] = _price;
    //     }
    // }

    // function payWithToken(string memory _name, uint256 _years, address _tokenAddress) public available(_name) isValidToken(_tokenAddress) {
    //     uint256 PRICE_USD = getAmountToPay(_name, _years);
    //     IERC20 token = IERC20(_tokenAddress);
    //     uint256 (userTokenBalInUSD, requiredTokenAmount); //= getConversionOf(token.balanceOf(msg.sender);
    //     if (userSKLBalInUSD < PRICE_USD) {
    //         revert CletPay__InsufficientFunds();
    //     } else {
    //             token.transferFrom(msg.sender, CLDGR, requiredTokenAmount);
    //         name_Exists[_name] = true;
    //         name_ToOwner[_name] = msg.sender;
    //         uint256 expiry = _years * 365 days;
    //         expiry = 10 minutes; //!!!REMOVE THIS LINE BEFORE MAINNET
    //         name_ToExpiry[_name] = block.timestamp + expiry;
    //         emit Paid(_name, _years, msg.sender, "CLET INC.", PRICE_USD, 0);
    //     }
    // }
}
