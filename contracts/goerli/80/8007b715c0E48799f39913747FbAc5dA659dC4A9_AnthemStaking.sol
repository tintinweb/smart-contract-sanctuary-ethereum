...
contract AnthemStaking is ReentrancyGuard, AccessControl {
    ...
    function addNFTContracts(address _contractAddress) public onlyRole(ADMIN_ROLE) {
        NFTContracts[_contractAddress] = true;
        contractAddressList.push(_contractAddress);
    }

    function delNFTContracts(address _contractAddress) public onlyRole(ADMIN_ROLE) {
        uint256 index;
        bool isExist = false;
    ...
    function deposit(address _contractAddress, uint256 _tokenId) public nonReentrant {
        ...
        depositNFTsByWalletAddress[msg.sender].push(
            StakingNFT({
    ...
    function withdraw(address _contractAddress, uint256 _tokenId) public nonReentrant {
        ...
        delete depositNFTsByWalletAddress[msg.sender][depositNFTsIndex];
        delete depositUsersByContractAddress[_contractAddress][depositUsersIndex];
    ...
    function forceWithdraw(
        ...
        StakingNFT[] storage depositNFTList = depositNFTsByWalletAddress[receiver];
        uint256 depositNFTsIndex = 0;
        bool depositNFTListExist = false;
        while (depositNFTsIndex < depositNFTList.length) {
            if (
        ...
        while (depositNFTsIndex < depositNFTList.length) {
            if (
                depositUserList[depositUsersIndex].walletAddress == receiver &&
                depositUserList[depositUsersIndex].tokenId == _tokenId
            ) {
                depositUsersListExist = true;
                break;
        ...
        require(depositNFTListExist && depositUsersListExist, "This is not a deposit.");

        IERC721 token = IERC721(_contractAddress);
        token.safeTransferFrom(address(this), receiver, _tokenId);
        delete depositNFTsByWalletAddress[receiver][depositNFTsIndex];
        delete depositUsersByContractAddress[_contractAddress][depositUsersIndex];

        emit Withdraw(receiver, _contractAddress, _tokenId);
    }
    ...
    function getDepositUsersByContractAddressWithPagin(
        address contractAddress,
        uint256 pageSize,
        uint256 pageOfs
