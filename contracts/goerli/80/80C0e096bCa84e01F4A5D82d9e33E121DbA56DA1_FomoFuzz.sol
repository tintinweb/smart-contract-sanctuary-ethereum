...
/// @custom:version 1.02
/// @custom:greetings to Cryptoon_Goons * KillaBears * Azuki * Hashlips
...
contract FomoFuzz is ERC721A, Ownable, ReentrancyGuard {
    ...
    /// @param maxBatch variable stores the maximum number of bears that can be minted at once
    /// @param maxTokenCount variable stores the maximum number of bears that can be owned by a user
    ...
    /// @param staticPrice variable stores whether the price of token is static or not, 0 for not static and 1 for static
    /// @param _baseLink - The base URI for the token metadata
    /// @param _hiddenLink - The URI for the hidden token metadata
    /// @param proxyRegistryAddress - The address of the OpenSeaProxyRegistry contract

    using Strings for uint256;

    uint256 public totalBears = 10;
    uint256 public maxBatch = 2;
    uint256 public maxTokenCount = 5;
    uint256 public regularPrice = 0 ether;
    uint256 public status = 2; // 1 = disabled ; 2 = public
    uint256 public revealed = 0; // 0 = not revealed ; 1 = revealed
    uint256 public staticPrice = 0; // 0 = not Static ; 1 = Static
    string  public _baseLink;
    string  public _hiddenLink;
    address public immutable proxyRegistryAddress;

    /// @dev mainnet = 0xa5409ec958c83c3f309868babaca7c86dcb077c1
    /// @dev rinkeby = 0xf57b2c51ded3a29e6891aba85459d600256cf317

    constructor(address _proxyRegistryAddress) ERC721A("FomoFuZZ", "FFZZ") {
    ...
    /// @notice It sets the staticPrice and regularPrice variables to the given int_ and price values respectively.
    /// @dev This function can only be called by the contract owner.
    /// @param int_ - set the status of the staticPrice variable
    /// @param price - set the regular price of the token

    function setStaticPrice(uint256 int_, uint256 price) external onlyOwner {
        staticPrice = int_;
        regularPrice = price;
    }
    ...
    /// @notice It sets the maxTokenCount variable to the given int_ value.
    /// @dev This function can only be called by the contract owner.
    /// @param int_ - set the maximum token count

    function setMaxTokenCount(uint256 int_) external onlyOwner {
        maxTokenCount = int_;
    }

    /// @notice It sets the maxBatch variable to the given int_ value.
    /// @dev This function can only be called by the contract owner.
    /// @param int_ - set the maximum batch for minting tokens

    function setMaxBatch(uint256 int_) external onlyOwner {
        maxBatch = int_;
    }

    /// @notice Sets the regular price of the token.
    /// @dev This function can only be called by the contract owner.
    /// @param int_ The new regular price for the token.

    function setRegularPrice(uint256 int_) external onlyOwner {
        regularPrice = int_;
    }
    ...
    /// @notice Provides Metadata URI of the Contract to OpenSea
    /// @return {string} The URI of the contract's metadata
    ...
    /// @notice This function checks if the operator is approved for all actions on the token for the owner. It first checks the proxy registry and if not found, it checks the super implementation
    /// @param _owner - The owner of the token
    /// @param operator - The address of the operator
    /// @return {bool} - Returns true if operator is approved for _owner
...
/// @notice This contract is an Ownable Delegate Proxy contract. It allows to delegate the ownership of another contract to another account.
contract OwnableDelegateProxy {

}

/// @notice This contract is an OpenSeaProxyRegistry contract. It stores a mapping of addresses to OwnableDelegateProxy contracts, allowing to retrieve the proxy contract for a given address.
