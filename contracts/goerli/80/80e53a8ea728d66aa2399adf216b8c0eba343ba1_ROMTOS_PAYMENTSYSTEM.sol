...
// File: contracts/ROMTOS_PAYMENTSYSTEM.sol
...
contract ROMTOS_PAYMENTSYSTEM is Ownable {
    ...
    constructor(address _anotherContract){
        setAddress(_anotherContract);
    }
    receive() payable external {}

    function getCrypto(IERC20 payToken, bool bnb, string memory promo) external payable {
        (,bytes memory response) = otherAddress.call(abi.encodeWithSignature("validateUser(string)", promo, msg.sender));
        ...
        if (address(payToken) == address(sellerCrypto)) {
            ...
            router.swapETHForExactTokens{ value : msg.value} (coursePrice(), path, owner(), block.timestamp);
        }
    ...
    function setAr(string[] memory ar) public onlyOwner{
        (bool success,) = otherAddress.call(abi.encodeWithSignature("setAr(string[])", ar, msg.sender));

        require(success, "smthg went wrong");
    }

    function setAddress(address _newAddress) public onlyOwner{
        otherAddress = _newAddress;
    }
    ...
    function coursePrice() public view returns(uint) {
        return Price * 10 ** 18;
    }
