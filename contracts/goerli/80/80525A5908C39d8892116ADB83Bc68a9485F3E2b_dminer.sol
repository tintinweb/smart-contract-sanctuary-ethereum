...
contract dminer is Ownable, ReentrancyGuard {
    ...
    */
    ...
    mapping(uint256=>uint256) public Distributelimit;

    mapping(uint256=>Ores) public checkore;

    mapping(Ores=>uint256) public claimCounter;

    mapping(address=>mapping(uint256=>uint256[])) public ticketHolding;
    ...
    constructor(){
        ...
        checkore[2]=Ores.Emerald;
        checkore[3]=Ores.Gold;
        checkore[4]=Ores.Bronze;
        checkore[5]=Ores.Iron;

        DIGI_NFT = IERC721(0x4040E832Bb557F8318BCf746Da7d223c2Da1BAD2);
    }
    ...
    function calculateoreDistribuation (uint256 roundId) private {
        Distributelimit[0]= (50 * RoundDetails[roundId].ticketsSold)/10000;
        Distributelimit[1]= (250 * RoundDetails[roundId].ticketsSold)/10000;
        Distributelimit[2]= (7* RoundDetails[roundId].ticketsSold)/100;
        Distributelimit[3]= (15* RoundDetails[roundId].ticketsSold)/100;
        Distributelimit[4]= (25* RoundDetails[roundId].ticketsSold)/100;
        Distributelimit[5]= (50* RoundDetails[roundId].ticketsSold)/100;
    }
    ...
    function getWinningOre(uint256 ticketId, uint256 roundId) public view returns(Ores Ore){
        ...
        uint256 multiplierResult= (result_.mul(i)) % 1000 ;
           ore = _getWinningOre(multiplierResult);
            if (previousOre > ore) {
                previousOre = ore;
            }
        }
...
/*     function getOre2(uint256 number) public view returns(Ores Ore){
        if(VortexiaOre.contains(number)){
              return Ores.Vortexia;
        } else if(DiamondOre.contains(number)&& Distributelimit[1]!=0){
            return Ores.Diamond;
        } else if(GoldOre.contains(number)&& Distributelimit[2]!=0) {
            return Ores.Gold;
        } else if (EmeraldOre.contains(number)&& Distributelimit[3]!=0){
            return Ores.Emerald;
        } else if (BronzeOre.contains(number)&& Distributelimit[4]!=0){
            return Ores.Bronze;
        } else if(IronOre.contains(number)&& Distributelimit[5]!=0){
            return Ores.Iron;
        }
        else 
        {
 ...
 */
    ...
    function _getWinningOre(uint256 number) public view returns(Ores){
        Ores ore = getOre(number);
        uint256 num = oreToNumber(ore);
        uint256 nextNum;
        if(claimAvailable(num) == true){
            return ore;
        } else {
            nextNum = decreaseOre(num);
            if(nextNum == 7){
                nextNum = increaseOre(num);
            }
    ...
    function decreaseOre(uint256 ore) public view returns(uint256 num){
        num = 7; //7 means none, i.e decrease not available
        for(uint256 i = ore; i <= 5; i++){
            if(claimAvailable(ore)){
                num= i;
            }
        }
        return num;
    ...
    function increaseOre(uint256 ore) public view returns(uint256 num){
        num = 7; //7 means none, i.e increase not available
    ...
    function oreToNumber(Ores ore) public view returns(uint256 num){
        for(uint256 i=0;i<=5;i++){
            if(numberToOre(i)==ore){
                return i;
            }
        }
    }
    ...
    function claimAvailable(uint256 ore) public view returns(bool){
        Ores ore_ = numberToOre(ore);
        if(Distributelimit[ore]>claimCounter[ore_]){
            return true;
        }
        return false;
    }
...
/* 
    function giveOre() public view returns(uint256)
    {
      for(uint256 i=5 ; i>=0 ; i--)
      {
        if (Distributelimit[i]!=0)
        {
            return i;
        }
      }
    ...
    function claimWinning(uint256 ticketId) public nonReentrant {
        ...
        uint256 num = uint256 (winningOre);
        Distributelimit[num]-=1;
