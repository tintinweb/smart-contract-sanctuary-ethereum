...
contract BottleTypePricePosting is ChainlinkClient, ConfirmedOwner {
  ...
  event RequestFulfilledValue1AndValue2(bytes32 indexed requestId, uint256 indexed bottleTypeID, uint256 timeUnix , uint256 locationID, uint256 price);

  function fulfillValue1AndValue2(bytes32 requestId, uint256 _bottleTypeID, uint256 _timeUnix, uint _locationID, uint _price)
    public
    recordChainlinkFulfillment(requestId)
  {
    emit RequestFulfilledValue1AndValue2(requestId, _bottleTypeID, _timeUnix, _locationID, _price);
