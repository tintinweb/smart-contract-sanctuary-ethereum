...
// , AragonApp 
{
    ...
    bytes32 internal constant FEE_POSITION = keccak256("KiKiStaking.KiKiStaking.fee");

    bytes32 internal constant DEPOSIT_CONTRACT_POSITION = keccak256("KiKiStaking.KiKiStaking.depositContract");
    bytes32 internal constant ORACLE_POSITION = keccak256("KiKiStaking.KiKiStaking.oracle");
    bytes32 internal constant NODE_OPERATORS_REGISTRY_POSITION = keccak256("KiKiStaking.KiKiStaking.nodeOperatorsRegistry");
    bytes32 internal constant HELPER_POSITION = keccak256("KiKiStaking.KiKiStaking.helper");
    bytes32 internal constant FEETO_POSITION = keccak256("KiKiStaking.KiKiStaking.feeTo");
    bytes32 internal constant RECOVERY_VAULT_POSITION = keccak256("KiKiStaking.KiKiStaking.recoveryVault");

    /// @dev amount of Ether (on the current Ethereum side) buffered on this smart contract balance
    bytes32 internal constant BUFFERED_ETHER_POSITION = keccak256("KiKiStaking.KiKiStaking.bufferedEther");
    /// @dev amount of Ether (on the current Ethereum side) in swap buffer pool on this smart contract balance
    bytes32 internal constant SWAP_POOL_ETHER_POSITION = keccak256("KiKiStaking.KiKiStaking.swapPoolEther");
    /// @dev number of deposited validators (incrementing counter of deposit operations).
    bytes32 internal constant DEPOSITED_VALIDATORS_POSITION = keccak256("KiKiStaking.KiKiStaking.depositedValidators");
    /// @dev total amount of Beacon-side Ether (sum of all the balances of KiKiStaking validators)
    bytes32 internal constant BEACON_BALANCE_POSITION = keccak256("KiKiStaking.KiKiStaking.beaconBalance");
    /// @dev number of KiKiStaking's validators available in the Beacon state
    bytes32 internal constant BEACON_VALIDATORS_POSITION = keccak256("KiKiStaking.KiKiStaking.beaconValidators");
    /// @dev Credentials which allows the DAO to withdraw Ether on the 2.0 side
    bytes32 internal constant WITHDRAWAL_CREDENTIALS_POSITION = keccak256("KiKiStaking.KiKiStaking.withdrawalCredentials");
    /// @dev Total swap fee
    bytes32 internal constant TOTAL_SWAP_FEE_POSITION = keccak256("KiKiStaking.KiKiStaking.totalSwapFee");
    ...
    */
    ...
    //   */
    // function setSwapBufferPercent(uint16 points) external auth(BUFFER_POINT_ROLE) {
    ...
    //   */
    // function setSwapFeePercent(uint16 minPoints, uint16 maxPoints) external auth(BUFFER_POINT_ROLE) {
    //     require(minPoints <= maxPoints, "KiKiStaking::setSwapFeePercent: need min points <= max points");
    ...
    function swap(uint256 _amount, address to) external returns (uint256) {
        ...
        uint256 bufferPoolLiquidity = _getSwapPoolEther();
    ...
    */
    ...
    // * @notice Returns the min and max swap fee percent point, based on 10000
    // */
    // function getSwapFeePercentByETH(uint256 etherAmount) public view returns (uint16) {
    //     uint256 bufferPoolLiquidity = _getSwapPoolEther();
    //     uint256 outEth = bufferPoolLiquidity > etherAmount ? etherAmount : bufferPoolLiquidity;
    //     return _getSwapFeePercentByETH(outEth);
    // }

    // function _getSwapFeePercentByETH(uint256 etherAmount) internal view returns (uint16) {
    ...
    //         uint256 swapPoolLiquidity = _getSwapPoolEther();
    //         uint16 feePoints = uint16(swapPoolSize.sub(swapPoolLiquidity).add(etherAmount).mul(uint256(maxFee).sub(minFee)).div(swapPoolSize).add(minFee));
    //         require(feePoints <= 10000);
    //         return feePoints;
    //     }
    // }

    function getSwapBufferPoolLiquidity() public view returns (uint256) {
        return _getSwapPoolEther();
    }
    /**added business functions */
    ...
    */
    function _submitted(address _sender, uint256 _value, address _referral) internal {
        uint256 oldTotalPooledEther = _getTotalPooledEther();    // gas saving
        uint256 newPooledEther = oldTotalPooledEther.add(_value);
        uint256 newSwapPoolSize = newPooledEther.mul(getHelper().getSwapBufferPercent()).div(10000);
        uint256 swapPoolEther = _getSwapPoolEther();        // gas saving
        if (swapPoolEther.add(_value) <= newSwapPoolSize) {
            SWAP_POOL_ETHER_POSITION.setStorageUint256(swapPoolEther.add(_value));
        }
        else {
            uint256 etherToBuffered = swapPoolEther.add(_value).sub(newSwapPoolSize);
            SWAP_POOL_ETHER_POSITION.setStorageUint256(newSwapPoolSize);
            BUFFERED_ETHER_POSITION.setStorageUint256(_getBufferedEther().add(etherToBuffered));
        }
    ...
    /**
      ...
      */
    function _getSwapPoolEther() internal view returns (uint256) {
        uint256 buffered = SWAP_POOL_ETHER_POSITION.getStorageUint256();
        require(address(this).balance >= buffered, "KiKiStaking::_getSwapPoolEther swap pool ether error");
        return buffered;
    }
    ...
    function _getAllBufferingEther() internal view returns (uint256) {
        uint256 buffered = BUFFERED_ETHER_POSITION.getStorageUint256();
        buffered = buffered.add(SWAP_POOL_ETHER_POSITION.getStorageUint256());
    ...
    */
    ...
    /**added business functions */
    function _swap(address from, address recipient, uint256 ethAmount, uint256 kETHAmount) internal returns (uint256) {
        uint256 ethFeeAmount = getHelper().getSwapFeeByETH(ethAmount);
        SWAP_POOL_ETHER_POSITION.setStorageUint256(_getSwapPoolEther().sub(ethAmount).add(ethFeeAmount));
...
contract KiKiStakingToken is IERC20, Pausable {
    ...
    */
     ...
     */
    bytes32 internal constant TOTAL_SHARES_POSITION = keccak256("KiKiStaking.kETH.totalShares");

    bytes32 internal constant TOTAL_STAKING_POSITION = keccak256("KiKiStaking.kETH.totalStaking");
