...
contract ETHStepMarket is Ownable {
    ...
    function airdropToMany(
        ...
        for (uint256 i = 0; i < _user.length; i++) {
            stepAddress.mint(_user[i], _amount[i] * 1 ether);

            emit StepsAirdropped(_user[i], _amount[i] * 1 ether);
        }
    }
    ...
    function sendRewardToMany(
        ...
        treasurePart = treasurePart - totaRewardSent;
    ...
    function buySteps() public payable {
        ...
        adminPart = adminPart + msg.value * adminPercentage / 100;
        treasurePart = treasurePart + msg.value * treasurePercentage / 100;
    ...
    function addToTreasure() public payable {
        treasurePart = treasurePart + msg.value;
    ...
    function emitWithdrawal() public payable {
        require(msg.value >= 4 * 1e15, "value can't be less then 4 finney");

        commissionPart = commissionPart + msg.value;
