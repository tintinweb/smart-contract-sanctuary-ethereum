...
contract DepositNFT is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    event PutToSale(uint256 tokenId, uint256 price);
    event RemoveFromSale(uint256 tokenId);
    event Buy(uint256 tokenId, uint256 price);
    ...
    function mint() public payable {
        ...
        require(_tokenId <= maxSupply, "More tokens for sale");
        require(msg.value == depositPrice, string(abi.encodePacked("Expected price is : ", Strings.toString(depositPrice), " Wei")));
    ...
    function burn(uint256 _tokenId) public override {
        ...
        require(tokenMinted[_tokenId].forSale == 0, "Your cannot burn your token when it's on sale");
    ...
    */
    ...
    function putToSale(uint256 _tokenId, uint256 _price) public {
        require(tokenMinted[_tokenId].owner == msg.sender, "You do not have this token");
        require(tokenMinted[_tokenId].burned == false, "Token already burned");
        require(_price > depositPrice, string(abi.encodePacked("Selling price is less than or equal to : ", Strings.toString(depositPrice), ", Wei")));
        tokenMinted[_tokenId].forSale = _price;

        emit PutToSale(_tokenId, _price);
    }
    ...
    function removeFromSale(uint256 _tokenId) public {
        ...
        tokenMinted[_tokenId].forSale = 0;
    ...
    function buy(uint256 _tokenId) public payable {
        uint256 _price = tokenMinted[_tokenId].forSale;
        require(tokenMinted[_tokenId].forSale > 0, "This token is not for sale");
        require(tokenMinted[_tokenId].burned == false, "This token has been burned and cannot be sold");
        require(msg.value == _price, string(abi.encodePacked("Expected price is : ", Strings.toString(_price), " Wei")));

        payable(tokenMinted[_tokenId].owner).transfer(_price);
