...
contract WenMint1155 is ERC1155, Ownable, PaymentSplitter, ReentrancyGuard, DefaultOperatorFilterer, Referable {
    ...
    function isEligible(
        ...
        if (!_isEligible.claim && !_isEligible.preSale && _token.pubSaleIsActive && (_value == _token.pubSalePrice * _quantity)) {
            if (_token.pubMintType == PublicMintType.PerWallet) {
                _isEligible.pubSale = (_quantity <= (_token.pubMaxMint - _hasMinted));
            } else {
                _isEligible.pubSale = (_quantity <= _token.pubMaxMint);
            }
            if (_isEligible.pubSale && (_token.pubMintType == PublicMintType.PerWallet)) hasMinted[_id][_address] += _quantity;
        }
        return _isEligible;
    }
    ...
    function mint(
        ...
        Referable.payReferral(_address, _referrer, _quantity, msg.value);
    }
    ...
    function airdrop(address[] memory _addresses, uint16[] memory _ids, uint16[] memory _quantities) external onlyOwner {
        ...
        require(_addresses.length == _ids.length, "Invalid");
        for (uint256 i; i < _addresses.length; i++) {
            require(tokens[_ids[i]].totalSupply + _quantities[i] <= tokens[i].maxSupply, "No supply");
            _mint(_addresses[i], _ids[i], _quantities[i], "");
            tokens[_ids[i]].totalSupply += uint16(_quantities[i]);
        }
    }
}
