...
Tiny Winged Turtlez: 0x6fE0b5c1ba36C79D9D7dD43f60a3e53E3699C8C0
...
- The color pallette is inspired by the original Checks piece designed by @jackbutcher
...
contract CheckTurtz is ERC721, Ownable {
    IERC721 public TinyWingedTurtlez;
    IERC721 public Checks;
    IERC721 public ChecksV2; // @dev Just in case

    uint256 public mintEnds;

    // @dev No upper limit on supply
    ...
    mapping(uint256 => CheckTurt) public checkTurtz; // @dev No upper limit on supply

    mapping(address => bool) public hasMinted;

    string[80] private colorPallette = [
        "#DB395E",
        "#602263",
        "#5C83CB",
        "#B1EFC9",
    ...
    // @dev Index 0 goes up in increments of 108 and index 1 goes up in increments of 105
    ...
    constructor(
        ...
        try IERC721(_twtAddress).balanceOf(address(this)) returns (uint256) {
            TinyWingedTurtlez = IERC721(_twtAddress);
        } catch {
            revert BadTWTContract();
        }

        try IERC721(_checksAddress).balanceOf(address(this)) returns (uint256) {
            Checks = IERC721(_checksAddress);
        } catch {
            revert BadChecksContract();
        }
    ...
    function addChecksV2Contract(address _checksV2Address) external onlyOwner {
        ...
        try IERC721(_checksV2Address).balanceOf(address(this)) returns (
            uint256
        ) {
            ChecksV2 = IERC721(_checksV2Address);
        } catch {
            revert BadChecksV2Contract();
        }
    }
    ...
    function mint() external {
        ...
        uint16 colors = 0; // @dev Max supply 5,000 + 16,030 = 24,030

        colors = uint16(
            TinyWingedTurtlez.balanceOf(msg.sender) +
                Checks.balanceOf(msg.sender)
        );

        if (address(ChecksV2) != address(0)) {
            colors += uint16(ChecksV2.balanceOf(msg.sender));
        }
    ...
    function getDefaultTurt() internal pure returns (string memory) {
        return
            "<rect x='0' y='0.75' width='34.2' height='57' fill='#65bc48' /><rect x='34.2' y='12.15' width='11.4' height='34.2' fill='#65bc48' /><rect x='11.4' y='12.15' width='22.8' height='22.8' fill='#ffffff' /><rect x='23.56' y='23.55' width='11.4' height='11.4' fill='#000000' />";
    }

    function getCheckTurt(
        uint256 tokenId,
        uint8 i // @dev Max value of i is 79
    ) internal view returns (string memory) {
        // @dev Max value of colorPalletteIndex is 79
        uint8 colorPalletteIndex = uint8(
        ...
        return
            string(
                abi.encodePacked(
                    "<rect x='0' y='0.75' width='34.2' height='57' fill='",
                    colorPallette[colorPalletteIndex],
                    "' /><rect x='34.2' y='12.15' width='11.4' height='34.2' fill='",
                    colorPallette[colorPalletteIndex],
                    "' /><rect x='11.4' y='12.15' width='22.8' height='22.8' fill='#ffffff' /><rect x='23.56' y='23.55' width='11.4' height='11.4' fill='#000000' />"
                )
            );
    }
