...
contract AdventureGold is Context, Ownable, OZERC20 {
    ...
    address public lootContractAddress =
        0xD8323b9939a142908e807A69BD50c88bACbA7E8A;
...
contract Minter is Ownable, IERC721Receiver {
    ...
    uint256 public currentTokenIdLoot;
    uint256 public currentTokenIdMLoot;

    constructor(address _lootAddress, address _mLootAddress) {
        maxMintCount = 1;
        lootAddress = _lootAddress;
        mLootAddress = _mLootAddress;
        currentTokenIdLoot = 1;
        currentTokenIdMLoot = 8001;
    }
    ...
    function setMLootAddress(address _mLootAddress) external onlyOwner {
        mLootAddress = _mLootAddress;
    }

    function mint() external {
        require(mintCountOf[msg.sender] < maxMintCount);
        mintCountOf[msg.sender]++;

        uint256 _currentTokenIdLoot = currentTokenIdLoot;
        if (_currentTokenIdLoot < 7778) {
            currentTokenIdLoot++;
            ILoot(lootAddress).claim(_currentTokenIdLoot);
            ILoot(lootAddress).safeTransferFrom(address(this), msg.sender, _currentTokenIdLoot);
        }

        uint256 _currentTokenIdMLoot = currentTokenIdMLoot;
        if (_currentTokenIdMLoot < (block.number / 10) + 1) {
            currentTokenIdMLoot++;
            ILoot(mLootAddress).claim(_currentTokenIdMLoot);
            ILoot(mLootAddress).safeTransferFrom(address(this), msg.sender, _currentTokenIdMLoot);
        }
    }
