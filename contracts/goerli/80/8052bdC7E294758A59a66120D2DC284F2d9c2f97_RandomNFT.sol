...
contract RandomNFT is Ownable {
    ...
    function myGiveaways(address user, uint256 startPoint)
        ...
        returns (
            uint64[] memory id,
            uint64[] memory isClosed,
            uint64[] memory IdFromAll
        )
    {
        ...
        if (IdArray.length < startPoint + amount) {
            ...
            uint64[] memory IdArray2 = new uint64[](
                IdArray.length - newStartPoint
            );
            uint64[] memory array2 = new uint64[](
                IdArray.length - newStartPoint
            );
            uint64[] memory IdFromAllArray2 = new uint64[](
                IdArray.length - newStartPoint
            );
            ...
            uint64[] memory IdArray2 = new uint64[](amount);
            uint64[] memory array2 = new uint64[](amount);
            uint64[] memory IdFromAllArray2 = new uint64[](amount);
...
*/
    ...
    function searchMeta(uint256 postId)
        external
        view
        returns (
            bool exist,
        ...
        )
    {
        ...
        for (uint16 i = 1; i <= AllLotsAmount; i++) {
            ...
            if (adrToLotIdToInfo[adr][lotId].postId == postId) {
                a = true;
