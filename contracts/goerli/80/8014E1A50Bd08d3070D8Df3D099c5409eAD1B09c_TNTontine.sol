...
/*
   ...
   When the full supply has been minted, the Tontine begins.
   1. When a token is transferred or sold, it burns and adds to the receiver.
   2. When 3 Tontine remain, the final 3 split the mint.
...
contract TNTontine is ERC721URIStorage, Ownable {
    ...
    function mint() external payable {
        ...
        if (tokenByAddress[msg.sender] == 0) {
            _mint(msg.sender, newItemId);
            tokenByAddress[msg.sender] = newItemId;
            addAddress(msg.sender);
        }

        _balances[msg.sender]++;

        // Create or replace the existing tokenid uri
        string memory tokenUri = _buildTokenURI(
            tokenByAddress[msg.sender],
            _balances[msg.sender]
        );
        _setTokenURI(tokenByAddress[msg.sender], tokenUri);
    }
    ...
    function safeTransferFrom(
        ...
        // remove the from token count
        if (_balances[from] > 0) {
            _balances[from]--;
        }

        // if from token count is zero, remove address from winners list
        if (_balances[from] == 0) {
            removeAddress(to);
        }
    }
            ...
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        ...
                        '<image width="100%" height="100%" href="https://ipfs.io/ipfs/QmSQPLbA8iR5YfaeKZ2dsCnPL9DYR78G9SwPaGKF6twvuX"/>',
                        '<path d="M18.6 3H5.4A2.4 2.4 0 0 0 3 5.4v13.2A2.4 2.4 0 0 0 5.4 21h13.2a2.4 2.4 0 0 0 2.4-2.4V5.4A2.4 2.4 0 0 0 18.6 3Z"',
                        ' fill="#9eb6d0" fill-opacity="0.125" stroke="orange" stroke-width="0.25" stroke-opacity="0.25"stroke-miterlimit="10"/>',
                        '<text x="50%" y="50%" text-anchor="middle" fill="white" font-size="7.5px" dy=".3em">',
                        uintToString(id),
                        "</text>",
                        '<text x="50%" y="95%" text-anchor="middle" fill="white" font-size="1.5px" font-family="Courier New">Sacrificed NFTs: ',
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"Tontine", "image":"',
                                image,
                                unicode'", "description": "The Tontine. Three Winners. Requires stamina and resolve."}'
                            )
                        )
                    )
                )
    ...
    event BalanceCheck(uint256 remainingHolders);
    event BalancePayout(address winner, uint256 amount);

    function splitBalance() private {
        emit BalanceCheck(tokenHolders.length());

        if (tokenHolders.length() > TOTAL_WINNERS) {
            return;
        }
        ...
        for (uint i = 0; i < tokenHolders.length(); i++) {
            ...
            emit BalancePayout(holder, pAmount);
        }
    }
    ...
    function ownerClaim() external onlyOwner {
        ...
        uint256 balance = address(this).balance;
        uint256 claimable = (balance * SPLIT_PERCENT) / 100;
        require(claimable > 0, "No funds available to claim");
