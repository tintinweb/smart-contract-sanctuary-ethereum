...
contract NiftMarketing is ERC721Holder, ERC1155Receiver, ERC1155Holder, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => VotingList)) public collectionVotingList;
    ...
    function vote(uint256 _marketingId, bool _vote) public {
        require(_marketingId > 0 && _marketingId < marketingId, "Contract ID does not exist");
        ...
        require(marketing.proposal.delegate != msg.sender, "Can't vote for yourself");

        if(collectionVotingList[_marketingId][msg.sender].delegate == marketing.proposal.delegate){ 
            if(collectionVotingList[_marketingId][msg.sender].voted){
                marketings[_marketingId].proposal.positiveVote--;   
            }else{
                marketings[_marketingId].proposal.nagetiveVote--;
            }
            marketings[_marketingId].proposal.voteCount--;
        }
        ...
        collectionVotingList[_marketingId][msg.sender] = VotingList({ voted:_vote, delegate:marketing.proposal.delegate});
    ...
    function calculateMarketingFees(uint256 _marketingId, address _ownerAddr) external view returns (uint256) {
        require( _marketingId > 0 && _marketingId < marketingId, "Contarct out of range" );
        Marketing memory marketing = marketings[_marketingId];
