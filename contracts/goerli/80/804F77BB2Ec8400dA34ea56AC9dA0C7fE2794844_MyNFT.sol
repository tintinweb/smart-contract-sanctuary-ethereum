...
contract MyNFT is ERC721, Ownable {
    event NftsBought(address buyer, uint256 price, uint256 amount);
    ...
    uint256 internal totalSharesAmount;
    uint256 internal totalTokenReleased;
    address[] internal payeesAddresses;
    mapping(address => uint256) internal shares;
    mapping(address => uint256) internal tokenReleased;

    string constant dummyUrl =
        "ipfs://QmfQ6F4NWjLBG8oGZPSsS6vdJbDV4XvxUjZGmv3UkyYgJr";
    string baseURIdata;

    constructor(
        uint256 price,
        address[] memory payeesIn,
        uint256[] memory sharesIn,
        uint256 tokenAmount
    ) ERC721("Name", "NN") {
        require(price != 0, "NFT price is zero");
        require(
            payeesIn.length == sharesIn.length,
            "TokenPaymentSplitter: payees and shares length mismatch"
        );
        require(payeesIn.length > 0, "TokenPaymentSplitter: no payees");
        require(tokenAmount != 0, "Mint zero tokens");

        NFTprice = price;

        for (uint256 i = 0; i < payeesIn.length; i++) {
            _addPayee(payeesIn[i], sharesIn[i]);
        }
    ...
    function _mintNFT() private onlyOwner returns (uint256) {
        tokenIds.increment();
    ...
    function buyNFTs(uint256 amount) external payable {
        uint256 price = NFTprice * amount;

        require(
            amount <= tokenIds.current() - lastSoldID.current(),
        ...
        for (uint8 i = 0; i < amount; i++) {
            ...
            _transfer(owner(), msg.sender, lastSoldID.current());
        }

        emit NftsBought(msg.sender, msg.value, amount);
    ...
    function availableNFTs() external view returns (uint256) {
        return tokenIds.current() - lastSoldID.current();
    }
    ...
    function priceNFT() external view returns (uint256) {
        return NFTprice;
    }
    ...
    function _addPayee(address account, uint256 shares_) private onlyOwner {
        ...
        payeesAddresses.push(account);
        shares[account] = shares_;
        totalSharesAmount = totalSharesAmount + shares_;
    }

    function availableAmountToWithdraw(address account)
        public
        view
        returns (uint256)
    {
        ...
        uint256 payment = (tokenTotalReceived * shares[account]) /
            totalSharesAmount -
            tokenReleased[account];
    ...
    function withdraw(address account) external {
        ...
        require(msg.sender == account, "Withdraw can only payee");

        uint256 payment = availableAmountToWithdraw(account);
        ...
        emit Withdrawed(msg.sender, account, payment);
    }
}
