...
contract ArtBaseRedeemedToken is ERC721Enumerable, IERC721Receiver, ReentrancyGuard, Ownable {
  using Strings for uint256;

  uint256 public currentMaxSupply;

  address public targetNFTs;

  constructor() ERC721("ArtBase Redeemed Tokens", "ABRT") {
    targetNFTs = 0x92511429F6c4780eBC8B4708B32D3eb5061bb4cB; //TODO: switch to mainnet contract address
  }

  function redeem(uint256 tokenToRedeem) external {
    ERC721 nftToRedeem = ERC721(targetNFTs);
    require(nftToRedeem.ownerOf(tokenToRedeem) == msg.sender, "You don't own that token");
    nftToRedeem.transferFrom(msg.sender, address(this), tokenToRedeem);
    _safeMint(_msgSender(), tokenToRedeem);
  }

  // For emergency unstuck purposes etc.
  function devMint(uint256 tokenToSend, address _To) external onlyOwner {
    _safeMint(_To, tokenToSend);
  }
  ...
  function changeTargetNFTs(address newTarget) external onlyOwner {
    targetNFTs = newTarget;
  }
  ...
  function _beforeTokenTransfer(address from, address to, uint256 startingTokenId) internal override 
  {
      require(from == address(0) || to == address(0), "Token is soulbound and cannot be transferred");
      super._beforeTokenTransfer(from, to, startingTokenId);
  }
