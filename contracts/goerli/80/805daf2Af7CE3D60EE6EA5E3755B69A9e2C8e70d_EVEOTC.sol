...
contract EVEOTC is EVEOTCTokens {
     ...
     */
    function smart_trades_to_query(uint256 _index) external view returns (address[] memory to) {
        return smart_trades[_index].to;
    }
    ...
    function smart_trades_length() external view returns(uint256 index) { return smart_trades.length; }
    ...
    function option_trades_length() external view returns(uint256 index) { return option_trades.length; }
     ...
     *                              If smart_trade is true, then this is the discount percentage
     *                              If smart_trade is false, this is the premium price
     * @param _time vesting duration in seconds or expiration day if it is 
     ...
     *
     */
    function cancelOffer(uint256 _index) external nonReentrant {
        ...
        require(smart_trades[_index].owner == msg.sender, "OTC: caller is not the owner of the trade");

        // return remaining amount + surplus
        uint256 amount_returned = smart_trades[_index].available - smart_trades[_index].filled + smart_trades[_index].surplus - smart_trades[_index].filled_surplus;
        if (amount_returned > 0) {
            require(IERC20(smart_trades[_index].from).transfer(msg.sender, amount_returned), "OTC: error canceling offer");
        }

        // set offer cancelled
        smart_trades[_index].active = false;
    ...
    /**
     * Validate a smart trade parameters
     *     @param _index: index in the smart_trades array
     *     @param _to: address of the token used for payment, must be previously approved
     *     @param _amount: to buy
     */
    function validateSmartTrade(uint256 _index, address _to, uint256 _amount) internal view {
    ...
    function calculateTotalPayment(uint256 _index, address _to, uint256 _sell_token_price, uint256 _pay_token_price, uint256 _amount) 
         ...
         * 
         * Example, Buy 10 * 10**18 weis paying with BTC
         * 
         ...
         * BTC with 6 decimals:
         *  0,0675
         * Buying 10 x 10**18 Ether: 
         *  0,0675 * 10 * 10 ** 6 / 10**18
         *  0,675 * 10 ** 6 = 67500000 BTC
         */ 

        // uint256 sell_token_discount_price = sell_token_price * (1 - (smart_trades[_index].discount_or_premium / 100));

        uint256 sell_token_discount_price = _sell_token_price - ((_sell_token_price * smart_trades[_index].discount_or_premium) / 100);
        //                                  1500             - ((1500             * 10                                       / 100)) 
        //                                  1500             - ((15000                                                       / 100)) 
        //                                  1500             - 150
        //                                  1350
        ...
        return (_amount   * sell_token_discount_price * 10 ** IERC20(_to).decimals()) / 10 ** IERC20(smart_trades[_index].from).decimals() / _pay_token_price;
        //                       10x10**18 * (135000000000              * 10 ** 6                    ) / 10**18                            / 2000000000000;
        //                       10x10**18 * (135000000000              * 1000000                    ) / 10**18                            / 2000000000000;
        //                       10x10**18 * 135000000000000000                                        / 10**18                            / 2000000000000;
    ...
    function estimateSmartTrade(uint256 _index, address _to, uint256 _amount) external view returns (uint256 total_payment) {
        ...
        // it is not a custom price?
            ...
            sell_token_price = getColdUSDPrice(smart_trades[_index].from);
        }
    ...
    function buySmartTrade(uint256 _index, address _to, uint256 _amount) external nonReentrant {
        ...
        // validate parameters
        validateSmartTrade(_index, _to, _amount);
        ...
        if (_surplus > 0) {
            require(IERC20(smart_trades[_index].from).transfer(getOwner(), _surplus), "OTC: error paying commissions to owner");
            smart_trades[_index].filled_surplus += _surplus;
        }
