...
contract DefiOTCSwap is ReentrancyGuard {
	...
	/// Helper function to create a limit order and insert before specified orderId (position)
	function createLimitOrderAtPositionHelper(
		...
		uint96 prevOrderId = orderAtPosition.prev;
		if (prevOrderId != 0) {
			Order storage orderBeforePosition = orderIdMapping[prevOrderId];
			if ((orderBeforePosition.tokenXAmount * desiredTokenYAmount / orderBeforePosition.desiredTokenYAmount) < tokenXAmount) revert InvalidOrderPosition();
			orderBeforePosition.next = _orderId;
		} else {
			orderBooks[tokenX][tokenY] = _orderId;
		}

		orderAtPosition.prev = _orderId;
	...
	function swapExactETHForTokenX(
		...
		swapExactTokenYForTokenXHelper(tokenX, address(weth), tokenYAmount, minTokenXAmountOut);
	}
