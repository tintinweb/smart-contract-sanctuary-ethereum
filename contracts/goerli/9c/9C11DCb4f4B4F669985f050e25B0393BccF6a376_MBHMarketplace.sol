...
contract MBHMarketplace is Ownable, ReentrancyGuard {
  ...
  modifier validListing(
    ...
    require(block.timestamp < listedItem.deadline, "item not buyable");
    _;
  }
    ...
    validListing(_nftAddress, _tokenId, _owner)
  {
    ...
    require(listedItem.price > 0, "not listed item");

    uint256 feeAmount = listedItem.price.mul(platformFee).div(1e3);

    if(listedItem.payMethod == 0) {
      require(msg.value >= listedItem.price, "Value below price");
