...
contract SFLAB is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    while (idLids.length < numLids) {
      uint256 r = (uint256(
        keccak256(abi.encodePacked((block.difficulty * block.timestamp)))
      ) % _maxSupply) + 1;

      for (uint256 i = 0; i < idLids.length; i++) {
        if (idLids[i] == r) {
        idLids[indexCounter] = r;
        indexCounter++;
        }
      }
    }
    uint256 indexCounter2 = 0;
    while (testLids.length < numLids) {
        testLids.push(indexCounter2);
        indexCounter2++;
    }
  }
