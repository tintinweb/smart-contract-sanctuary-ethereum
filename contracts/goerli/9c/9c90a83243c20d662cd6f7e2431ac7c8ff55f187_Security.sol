...
contract Factory {
    ...
    function getContractsDeployed(address x) external {
         contractsDeployed.push(x);
    }
...
contract Security is ERC20 {
    ...
    constructor(string memory name_, string memory symbol_,address fundAddress) ERC20 ( name_, symbol_ ){
        CreditAgricoleAddress = msg.sender;        
        _name = name_;
        _symbol = symbol_;
    ...
    function _burn(address account, uint256 amount) override internal onlyFundAddress {
...
contract Fund {
    ...
    function swap(uint amount, address issuingOwner)  external  {


        require(contractsDeployed[msg.sender]);
        f = 4;
        uint256 netValue = amount.mul(97).div(100);
        security.mint(issuingOwner, netValue);
