    ...
    IClubrareMarketplace
{
        ...
        isNotBlacklisted(msg.sender)
    {
        ...
        if (order.paymentToken == address(0)) {
            ...
            IERC20 erc20Token = IERC20(_order.paymentToken);
            require(
    ...
    function _splitFee(
        ...
        for (uint256 i; i < len; i++) {
            ...
            if (isToken) {
                if (isClaim) {
                    IERC20(_erc20Token).transfer(payee, commission);
                } else {
                    IERC20(_erc20Token).transferFrom(user, payee, commission);
                }
            } else {
    ...
    function _chargeAndSplit(
        ...
        bool isEth = _checkEth(_erc20Token);
        IERC20 ptoken = IERC20(isEth ? wETHAddress : _erc20Token);
        
        address _token = _erc20Token;
        uint256 amt =validator._getAmount(_token, _amount);
    ...
    function _executeExchange(
        ...
        if (order.orderType != Type.Escrow) {
            ...
            if (isToken) {
                if (isClaim) {
                    IERC20(order.paymentToken).transfer(_seller, _amount);
                } else {
                    IERC20(order.paymentToken).transferFrom(buyer, _seller, _amount);
                }
            } else {
            ...
            if (isToken) {
                IERC20(order.paymentToken).transferFrom(buyer, address(escrow), _amount);
            } else {
