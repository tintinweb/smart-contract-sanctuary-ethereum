...
contract OperatorControllerForRewardsV2 is Ownable {
     ...
     */
    function releaseTokensAndUpdateRewards(address[] memory tokens) external onlyOwner {
        ...
        try feeSharingSetter.convertCurrencyToRewardToken(tokens) {} catch {}
 ...
 * @notice It receives raffle game fees and owns the FeeSharingSystem contract.
...
contract FeeSharingSetter is ReentrancyGuard, AccessControl {
    ...
    event NewFeeSharingSystem(address feeSharingSystem);
    ...
    constructor(
        ...
        hunterGovernanceToken = feeSharingSystem.hunterGovernanceToken();
     ...
     * based on the balance of HTGs in fee-staking addresses that exist in the set.
     */
    function updateRewards() external onlyRole(OPERATOR_ROLE) {
        ...
        if (numberAddressesForFeeStaking > 0) {
            uint256[] memory HTGBalances = new uint256[](numberAddressesForFeeStaking);
            ...
            for (uint256 i = 0; i < numberAddressesForFeeStaking; i++) {
                uint256 HTGBalance = hunterGovernanceToken.balanceOf(_feeStakingAddresses.at(i));
                totalAmountStaked += HTGBalance;
                HTGBalances[i] = HTGBalance;
            }
            ...
            if (totalAmountStaked > 0) {
                ...
                for (uint256 i = 0; i < numberAddressesForFeeStaking; i++) {
                    uint256 amountToTransfer = (HTGBalances[i] * reward) / totalAmountStaked;
     ...
     * @param tokens addresses of the token to sell
     */
    function convertCurrencyToRewardToken(address[] memory tokens)
        external
        nonReentrant
        onlyRole(OPERATOR_ROLE)
    {
        ...
        for (uint256 i = 0; i < tokens.length; ++i) {
            uint256 amountToConvert = IERC20(tokens[i]).balanceOf(address(this));
            if (tokens[i] != address(rewardToken) && amountToConvert > 0) {
                // Adjust allowance for this transaction only
                IERC20(tokens[i]).safeIncreaseAllowance(address(rewardConvertor), amountToConvert);
                // Exchange token to reward token
                uint256 amountReceived = rewardConvertor
                    .convert(tokens[i], address(rewardToken), amountToConvert);
                emit ConversionToRewardToken(tokens[i], amountToConvert, amountReceived);
            }
        }
    }
    ...
    function updateFeeSharingSystem(address _feeSharingSystem) external onlyRole(DEFAULT_ADMIN_ROLE) {
        feeSharingSystem = FeeSharingSystem(_feeSharingSystem);

        emit NewFeeSharingSystem(_feeSharingSystem);
    }
