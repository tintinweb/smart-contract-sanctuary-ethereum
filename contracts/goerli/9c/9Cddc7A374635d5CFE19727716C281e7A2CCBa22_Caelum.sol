    ...
    ERC2981
{
    ...
    error ExceedingTeamReservedError();

    uint256 private MAX_SUPPLY = 4444;
    uint256 public PRICE_PUBLIC_SALE = 0.075 ether;
    uint256 public PRICE_PRESALE = 0.06 ether;
    uint256 private MAX_PER_MINT_PRESALE = 2;
    uint256 private MAX_PER_MINT_PUBLIC = 5;

    uint256 private RESERVED = 68;
    uint256 public TEAM_RESERVED = 150;
    ...
    mapping(address => bool) private addressReservedMintClaim;
    ...
    ) ERC721A("CaelumNFT", "CAELUM") {
        setBaseURI(_initNotRevealedUri);
        setNotRevealedUri(_initNotRevealedUri);
        setMaxSupply(_maxSupply);
        setReserved(_reserved);
        setPresaleMerkleRoot(_presaleMerkleRoot);
        setReservedMerkleRoot(_ReservedMerkleRoot);
        _registerForOperatorFiltering();
        operatorFilteringEnabled = true;
        _setDefaultRoyalty(msg.sender, 500);
    }

    function checkIfReservedMinted(
        address ad1
    ) public view onlyOwner returns (bool) {
        return addressReservedMintClaim[ad1];
    }
    ...
    function setPublicMaxMint(uint256 _newmaxMintAmount) public onlyOwner {
        MAX_PER_MINT_PUBLIC = _newmaxMintAmount;
    }

    function setPresaleMaxMint(uint256 _newmaxMintAmount) public onlyOwner {
        MAX_PER_MINT_PRESALE = _newmaxMintAmount;
    }

    function setTeamReserved(uint256 _teamReserved) public onlyOwner {
        TEAM_RESERVED = _teamReserved;
    }
    ...
    /**
     * Returns how much can be minted
     */
    ...
    /*    function mint(uint256 _mintAmount) public payable {
        if (msg.sender == owner()) {
            ...
            publicMint(_mintAmount);
        }
    }*/

    function teamMint(uint256 _mintAmount) public onlyOwner {
        if (team_minted + _mintAmount > TEAM_RESERVED) {
            revert ExceedingTeamReservedError();
        }
