...
contract Token is ERC20 {
    ...
    function mint(address account, uint256 amount) public onlyOwner returns (bool) {
        require(kycList[account], "E : account must be kyc");

        _mint(account, amount);
        return true;
    }

    function changeKYC(address account, bytes calldata kycdata, bool status) external onlyOwner {
        kycList[account] = status;
    }
    ...
    function sell(uint256 value) public returns (bool) {
        ...
        this.transferFrom(seller, address(this), value);

        uint256 earnedAmount = value * uint256(price);

        earnedAmount.customSend(seller);

        emit Sell(seller, value);
