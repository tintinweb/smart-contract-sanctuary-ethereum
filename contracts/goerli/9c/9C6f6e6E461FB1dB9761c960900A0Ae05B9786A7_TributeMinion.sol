...
contract Baal is CloneFactory, Module {
    ...
    address payable public optionalSusReceiver; /* address to receive optional fee*/
    address public optionalLootReceiver; /* address to receive optional loot share*/
    uint256 public optionalLootCut; /* % optional loot cut*/
    ...
    *****************/
    ...
    function submitProposal(
        ...
        if (getCurrentVotes(msg.sender) >= sponsorThreshold) {
            ...
            (bool _success, ) = target.call{value: proposalOffering}(""); /*Send ETH to sink*/
            require(_success, "could not send");
        }

        // forward remaining funds in msg.value
        if(msg.value >= proposalOffering && msg.value - proposalOffering > 0 && optionalSusReceiver != address(0)) {
            (bool _success, ) = optionalSusReceiver.call{value: msg.value - proposalOffering}(""); /*Send ETH to optionalSusReceiver*/
            // optionalSusReceiver.transfer(msg.value - proposalOffering); /*Send ETH to optionalSusReceiver*
            
            require(_success, "could not send");
            // todo: fallback send funds to target
        }
    ...
    *******************/
    ...
    function _mintLootCut(uint256 amount) 
        internal 
    {
        if(optionalLootCut > 0){
            uint256 cut = (amount / 100) * optionalLootCut;
            _mintLoot(optionalLootReceiver, cut);
        }
    }
        ...
        baalOrGovernorOnly
    {
        (
            ...
            uint256 sponsor,
            uint256 minRetention,
            address payable susReceiver,
            address lootReceiver,
            uint256 lootCut
        ) = abi.decode(
                _governanceConfig,
                (uint32, uint32, uint256, uint256, uint256, uint256, address, address, uint256)
            );
        ...
        optionalSusReceiver = susReceiver; /*set receiver of optional fee */
        optionalLootReceiver = lootReceiver;
        optionalLootCut = lootCut;
...
contract BaalSummoner is ModuleProxyFactory {
    ...
    // address to send any msg.value too
    address payable public immutable optionalSusReceiver;
    ...
    constructor(
        ...
        address payable _optionalSusReceiver
    ) {
        template = _template;
        ...
        optionalSusReceiver = _optionalSusReceiver;
    }
    ...
    function summonBaal(
        ...
        // forward remaining funds in msg.value
        if(msg.value > 0 ) {
            (bool _success, ) = optionalSusReceiver.call{value: msg.value}(""); /*Send ETH to sink*/
            require(_success, "could not send");
            // todo: handle fail
        }
    ...
    function summonBaalAndSafe(
        ...
        // forward remaining funds in msg.value
        if(msg.value > 0 ) {
            (bool _success, ) = optionalSusReceiver.call{value: msg.value}(""); /*Send ETH to sustainability receiver*/
            require(_success, "could not send");
            // todo: handle fail
        }
