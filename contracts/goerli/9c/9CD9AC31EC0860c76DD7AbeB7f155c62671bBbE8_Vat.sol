...
contract Vat {
    ...
    event setParam(bytes32 ilk, bytes32 what, uint data);
    ...
    function setNewLine(uint data) external auth {
        require(live == 1, "Vat/not-live");
        Line = data;
        emit setLine(data);
    }
    function setParamsPerIlk(bytes32 ilk, bytes32 what, uint data) external auth {
        require(live == 1, "Vat/not-live");
    ...
    function addDebt(bytes32 i, address u, uint wad) external {
        // system is live
        require(live == 1, "Vat/not-live");
        urns[i][u].debt = add(urns[i][u].debt, wad);
    }

    function subDebt(bytes32 i, address u, uint wad) external {
        // system is live
        require(live == 1, "Vat/not-live");
        urns[i][u].debt = sub(urns[i][u].debt, wad);
    }
    // --- CDP Fungibility ---
