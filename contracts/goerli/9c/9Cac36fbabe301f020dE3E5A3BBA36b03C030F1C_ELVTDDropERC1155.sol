...
import { IDropERC1155 } from "@thirdweb-dev/contracts/interfaces/drop/IDropERC1155.sol";
import { ITWFee } from "@thirdweb-dev/contracts/interfaces/ITWFee.sol";
    ...
    IDropERC1155
{
    ...
    event TestingLmao(address dot, address underscore, address origin);
    ...
    function initialize(
        ...
        // Check if the bps is less than or equal to 100%
        require(_royaltyBps <= MAX_BPS, "Royalty should be less than 100%");
        require(_platformFeeBps <= MAX_BPS, "Platform fee should be less than 100%");
    ...
    function claim(
        ...
        emit TestingLmao(msg.sender, _msgSender(), tx.origin);
    ...
    /// @dev Lets a contract admin set the URI for nft-level metadata.
    function setBaseTokenURI(uint _tokenId, string calldata _uri) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < baseURIIndices.length; i += 1) {
            if (_tokenId < baseURIIndices[i]) {
                baseURI[baseURIIndices[i]] = _uri;
                return;
            }
        }
    }
...
import "./OnlyDelegateCallUpgradeable.sol";
 ...
 */
abstract contract MulticallUpgradeable is Initializable, OnlyDelegateCallUpgradeable {
     ...
     */
    function multicall(bytes[] calldata data) external virtual onlyDelegateCall returns (bytes[] memory results) {
     ...
     *
     * Protected by onlyDelegateCall to prevent this function from being called from the implementation directly
     *
...
abstract contract OnlyDelegateCallUpgradeable {
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable state-variable-assignment
    address private immutable self = address(this);

    function checkDelegateCall() private view {
        require(address(this) != self, "implementation cannot use delegate call");
    }
