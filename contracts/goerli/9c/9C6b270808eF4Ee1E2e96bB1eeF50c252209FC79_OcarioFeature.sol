...
contract OcarioFeature is IFeature, IOcarioFeature, FixinCommon, FixinTokenSpender {
    ...
    IERC20TokenV06 public intoken;
    IERC20TokenV06 public outtoken;
    ...
    /// @param data Arbitrary data forwarded from swap() caller. An ABI-encoded
    ///        struct of: inputToken, outputToken, fee, payer
    function OcarioSwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external override {
        ...
        address payer;
        {
            ...
            require(
                ...
                "OcarioFeature/INVALID_SWAP_CALLBACK_CALLER"
            );
        }
    ...
    function _swap(
        ...
        if (sellAmount != 0) {
            require(sellAmount <= uint256(type(int256).max), "OcarioFeature/SELL_AMOUNT_OVERFLOW");
            // Perform a swap for each hop in the path.
            bytes memory swapDataCall;
            while (true) {
                bool isPathMultiHop = _isPathMultiHop(encodedPath);
                isPathMultiHopPublic = isPathMultiHop;
                bool zeroForOne;
                IOcarioPool pool;
                {
                    ...
                    ( inputToken,  fee,  outputToken) = _decodeFirstPoolInfoFromPath(
                        encodedPath
                    );
                    intoken = inputToken;
                    ...
                    zeroForOnep = zeroForOne;
                    swapDataCall = _updateSwapCallbackData(inputToken,outputToken,fee,source);
                    // SwapCallbackData memory myStruct = SwapCallbackData({
                    //     path: abi.encodePacked(inputToken, fee, outputToken),
                    //     source: source
                    //     });
                    // swapDataCallTemp = abi.encode(myStruct);      
                    datatemp = swapDataCall;
                }
                ...
                //     require(_buyAmount >= 0, "OcarioFeature/INVALID_BUY_AMOUNT");
                ...
                // source = address(this); // Subsequent hops are paid for by us.
        ...
        // require(minBuyAmount <= buyAmount, "OcarioFeature/UNDERBOUGHT");
    }
    ...
    function _updateSwapCallbackData(
        ...
        assembly {
            ...
            mstore(add(p, 96), and(ADDRESS_MASK, source))
        }
    ...
    function _toPool(
        ...
        uint24 fee,
        IERC20TokenV06 outputToken
    ) private view returns (IOcarioPool pool) {
        // address(keccak256(abi.encodePacked(
        //     hex"ff",
        //     OCR_FACTORY_ADDRESS,
        //     keccak256(abi.encode(inputToken, outputToken, fee)),
        //     OCR_POOL_INIT_CODE_HASH
        // )))
        // bytes32 ffFactoryAddress = OCR_FF_FACTORY_ADDRESS;

        // bytes32 poolInitCodeHash = OCR_POOL_INIT_CODE_HASH;
        // (IERC20TokenV06 token0, IERC20TokenV06 token1) = inputToken < outputToken
        //     ? (inputToken, outputToken)
        //     : (outputToken, inputToken);
        //    pool = IOcarioPool(PoolAddress.computeAddress(Factory,address(token0) , address(token1), fee, poolInitCodeHash));
    ...
    function _decodeFirstPoolInfoFromPath(
        ...
        require(encodedPath.length >= SINGLE_HOP_PATH_SIZE, "OcarioFeature/BAD_PATH_ENCODING");
        assembly {
    ...
    function _shiftHopFromPathInPlace(bytes memory encodedPath) private pure returns (bytes memory shiftedEncodedPath) {
        require(encodedPath.length >= PATH_SKIP_HOP_SIZE, "OcarioFeature/BAD_PATH_ENCODING");
