    ...
    AccessControl
{
    ...
    function lazyMint(
         ...
         */
        uint256 feeAmount = (msg.value * _voucher.feeBps) / 10000;
        sellerAmount -= feeAmount;
        _sendValue(_voucher.feeRecipient, feeAmount);
    ...
    function _burn(
        uint256 tokenId
    ) internal override(ERC721, ERC721URIStorage, ERC2981NSovreign) {
        super._burn(tokenId);
    }
    ...
    function initialize(bytes calldata _data) external {
        ...
        uint24 royaltyBps;
        (name, symbol, owner, royaltyRecipient, royaltyBps) = abi.decode(
            _data,
            (string, string, address, address, uint24)
        );

        _setRoyaltyInfo(royaltyRecipient, royaltyBps);
    ...
    ) ERC721LazyMintableSovreign(_eip712DomainName) {
        _FACTORY = factory_;
    }
}
...
contract ERC2981NSovreign {
    /**
     * @notice contract-wide royalty `bps` and `recipient` set at deployment.
     *      Setter function may be called from child contract.
     */
    ...
    /**
     * @notice constant 10,000 BPS = 100% shares sale price, cannot be reset by child contract implementation.
     */
    uint24 private constant _BPS_DENOMINATOR = 10000;

    CollectionRoyaltyInfo private _royaltyInfo;
    /**
     ...
     *      frontends will be able to detect support for ERC-2981 standard,
     *      however `royaltytInfo` will return 0 royalties amount by default for each NFT for as long as it remains unsold once minted.
     */
    mapping(uint256 => bool) private _secondaryMarketInfo;

    /**
     * @notice Called with the sale price to determine how much royalty
     *      is owed and to whom.
     * @param _salePrice - the sale price of the NFT asset specified by _tokenId
     * @param _tokenId as royalties are set contract wide for self-sovreign collections,
     *      _tokenId MUST exist or else the function will revert
     ...
     * @dev note that this function MAY be called by any contract
     *      This does not represent a security risk; ultimately it is up to marketplaces to decide whether to pay royalties or not
     *      There are arguments both in favor and against this choice:
     *      + not having to check `_isApprovedOrOwner` removes the need to inherit from ERC721 (which would make this contract a 721 extension), making it a standalone ERC2981 implementation
     *      + The calling marketplace (both on-chain and off-chain) may not be the token's owner or authorized operator at the time the sale happens and the `royaltyInfo` function is called
     *      - Any contract may call this function in order to set secondary market info without the owner's consent (or it may be the owner itself),
     *        therefore the only definitive solution would be to use a public whitelist of marketplaces allowed to call `royaltyInfo`
     * @dev marketplaces are responsible to ensure that the _tokenId exists and hasn't been burned
     */
    function royaltyInfo(
        ...
        if (_secondaryMarketInfo[_tokenId])
            royaltyAmount = (_salePrice * _royaltyInfo.bps) / _BPS_DENOMINATOR;
        else if (msg.sender.code.length > 0)
            _secondaryMarketInfo[_tokenId] = true;
     ...
     * @dev access control MUST be added by contracts using this function for security reasons, thus the `internal` visibility modifier
     * @param _recipient contract-wide `_royaltyRecipient` is set at deployment. Setter function may be implemented by child contract implementation.
     *      royaltyRecipient_ MUST be different from the zero address
     * @param _bps contract-wide `_royaltyBps` is set at deployment. Setter function may be implemented by child contract implementation.
     *      royaltyBps_ MUST be lower than or equal to `_BPS_DENOMINATOR`
     */
    function _setRoyaltyInfo(address _recipient, uint24 _bps) internal {
        if (_bps > _BPS_DENOMINATOR) revert(); // royalty fee amount MUST not exceed salePrice
        _royaltyInfo = CollectionRoyaltyInfo(_recipient, _bps);
    }

    function _setRoyalty(uint256 _tokenId) internal {
        _secondaryMarketInfo[_tokenId] = true;
    }

    function _setRoyaltyRecipient(address _recipient) internal {
        _royaltyInfo.recipient = _recipient;
    }

    /**
     * @dev Removes default royalty information.
     */
    function _deleteRoyaltyInfo(uint256) internal {
        delete _royaltyInfo;
    }

    function _burn(uint256 _tokenId) internal virtual {
        delete _secondaryMarketInfo[_tokenId];
    }
}
...
abstract contract ERC721URIStorage is ERC721 {
    ...
    function _burn(uint256 _tokenId) internal virtual override {
        ...
        if (_tokenURIs[_tokenId].length > 0) {
...
abstract contract ERC721LazyMintableSovreign is ERC721 {
    ...
    constructor(string memory _eip712DomainName) {
        ...
        VOUCHER_TYPEHASH = keccak256(
            "Voucher(bytes32 tokenURI,uint256 price,uint256 endTime,uint256 commissionBps,uint256 feeBps,address commissionRecipient,feeRecipient)"
        );
        /**
...
contract ERC721 {
    ...
    function _mintAndTransfer(
        ...
        require(_checkOnERC721Received(_recipient, _minter, _tokenId, _data));
    }
