...
contract AgentRoleUpgradeable is OwnableUpgradeable {
    ...
    function isAgent(address _agent) public virtual view returns (bool) {
...
// File: contracts/token/ERC1404.sol
...
interface ICompliance {
    ...
    /// ADDED FUNCTIONS (1404 compliance)

    /** 
     * @notice public facing function to return any token restrictions from compliance.  Any 
     *         token transfer restrictions that are not native to the token should be defined
     *         and checked for here.
     * 
     * @dev the arguments passed to this function should be in the same form as the ERC transfer 
     *      standard of _from, _to, _value.  Restriction checks should be done for any functionalities
     *      that are added on top of the baseline ERC-20 checks such as allowance, which are done
     *      by default on the token implementation. (see /token/Token.sol)
     * 
     * @param _from transfer "from" address
     * @param _to transfer "to" address
     * @param _value transfer amount
     */
    function checkTransferCompliance(address _from, address _to, uint256 _value) external view returns(uint8 code);

    /** 
     * @notice public facing function to check for any specific restriction messages for restrictions thrown by this compliance.  
     *         If restriction code is not recognized, message will be returned as "ERROR UNKNOWN"
     * 
     * @dev any transfer restriction messages that are not native to the token contract
     *      should be defined on this contract then assigned here to a code returned from
     *      checkTransferCompliance().
     * 
     * @param _restrictionCode the restriction code to get the message for
     */
    function restrictionMessage(uint8 _restrictionCode) external view returns(string memory);
}
...
contract TokenStorage {
    ...
    /// @dev RESTRICTION CODES AND MESSAGES
    /// @notice transfer restriction codes 0-3 are reserved by default implementation and should be inherited the same by children
    uint8 public constant SUCCESS_CODE = 0;
    uint8 public constant FROZEN_ACCOUNT_CODE = 1;
    uint8 public constant PARTIAL_FREEZE_CODE = 2;
    uint8 public constant NON_WHITELIST_CODE = 3;

    string public constant SUCCESS_MESSAGE = "SUCCESS";
    string public constant FROZEN_ACCOUNT_ERROR = "ILLEGAL_TRANSFER_ACCOUNT_FROZEN";
    string public constant PARTIAL_FREEZE_ERROR = "ILLEGAL_TRANSFER_PARTIAL_FREEZE";
    string public constant NON_WHITELIST_ERROR = "RECEIVER_NOT_WHITELISTED";
...
interface IToken is IERC20 {
    ...
    /**
     * @notice only callable by owner of contract or a designated agent
     * 
     * @dev assign disclaimer text data to storage.  
     * 
     * @param _disclaimer the disclaimer string data to assign
     */
    function setDisclaimer(string memory _disclaimer) external;

    /**
     * @notice override IToken and AgentRoleUpgradeable (interfacing purposes)
     * 
     * @dev external facing, check an address for agent role
     */
...
contract Token is IToken, ERC1404, AgentRoleUpgradeable, TokenStorage {
...
bool private initialize;
    ...
    function transfer(address _to, uint256 _amount) public override whenNotPaused returns (bool) {
        uint8 restrictionCode = detectTransferRestriction(msg.sender, _to, _amount);
    ...
    function transferFrom(
        ...
        uint8 restrictionCode = detectTransferRestriction(_from, _to, _amount);
        if (restrictionCode == SUCCESS_CODE) {
     ...
     * @notice only callable by owner of contract or a designated agent
     * 
     * @dev assign disclaimer text data to token.  
     */
    function setDisclaimer(string memory _disclaimer) public override {
        require(msg.sender == owner() || isAgent(msg.sender), "UNAUTHORIZED");
        disclaimer = _disclaimer;
    }

    /**
     * @notice Public facing, (1404 compliant) function that does necessary transfer checks 
     *         (local and on compliance contract) and returns a uint8 numeric code value 
     *         denoting the type of restriction that is enforced on the transfer.  The function
     *         will return a a single code value.  If there are multiple restrictions for a transfer,
     *         this function will return a differnent code once the first one is resolved until there are
     *         no more transfer restrictions (which are defined here and on the compliance contract, see
     *         ICompliance::checkTransferCompliance())
     * 
     * @dev the arguments passed to this function should be in the same form as the ERC transfer 
     *      standard of _from, _to, _value.  Restriction checks should be done for any functionalities
     *      that are added on top of the baseline ERC-20 checks such as allowance, which are done
     *      by default on the token implementation. (see /token/Token.sol)
     * 
     * @return restrictionCode the restriction code that is triggered. SUCCESS will be denoted as code 0
     */
    function detectTransferRestriction (address _from, address _to, uint256 _value) public override(ERC1404, IToken) view returns (uint8 restrictionCode) {
        if (!tokenIdentityRegistry.isVerified(_to)) {
            return NON_WHITELIST_CODE;
        } 
        if (frozen[_from] || frozen[_to]) {
            return FROZEN_ACCOUNT_CODE;
        } 
        if (frozenTokens[_from] > 0 ) {
            uint256 availableTokens = balanceOf(_from) - frozenTokens[_from];
            if(availableTokens < _value) {
                return PARTIAL_FREEZE_CODE;
            }
        }
        return tokenCompliance.checkTransferCompliance(_from, _to, _value);
    }

    /** 
     * @notice  public facing (1404 compliant) function to check any specific restriction messages that may be related to this contract
     *          if a restriction code is not recognized, message will be returned as "ERROR UNKNOWN"
     * 
     * @dev any additional transfer checks or messages should not be added to the token implementation but rather to the compliance contract
     *      where any additional transfer checks will be defined and checked for. (See ICompliance.sol)
     */
    function messageForTransferRestriction (uint8 _restrictionCode) public override(ERC1404, IToken) view returns (string memory message) {
        if(_restrictionCode == SUCCESS_CODE) {
            return SUCCESS_MESSAGE;
        } else {
            if(_restrictionCode == NON_WHITELIST_CODE) {
                return NON_WHITELIST_ERROR;
            }
            if(_restrictionCode == FROZEN_ACCOUNT_CODE) {
                return FROZEN_ACCOUNT_ERROR;
            }
            if(_restrictionCode == PARTIAL_FREEZE_CODE) {
                return PARTIAL_FREEZE_ERROR;
            }
            return tokenCompliance.restrictionMessage(_restrictionCode);
        }
    }

    /**
     * @dev override IToken and AgentRoleUpgradeable (interfacing)
     */
    function isAgent(address _agent) public override (IToken, AgentRoleUpgradeable) view returns (bool) {
        return super.isAgent(_agent);
    }
