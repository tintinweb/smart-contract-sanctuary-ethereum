...
contract ERC1155Openzeppelin is ERC1155, Ownable, Loyalty {
    ...
    string public baseUri = "https://eth-api.bullz.com/api/nfts/item/";
    ...
    function awardItem(uint256 newItemId, uint256 amount, bytes memory data,uint256 loyaltyPercent, uint resaleStatus) public returns (uint256) {
...
abstract contract Loyalty is ILoyalty{
    ...
    mapping(uint256 => uint) public resaleStatuses;

    uint public INIT_RESALE = 0;
    uint public RESALE_ALLOWED = 1;
    uint public RESALE_REJECTED = 2;
    ...
    function isLoyalty() override external pure returns (bool) {
        return true;
    }
    function addLoyalty(
        uint256 assetId,
        ...
        creators[assetId] = right_holder;
        resaleStatuses[assetId] = resaleStatus;
        _addLoyalty(assetId, right_holder, percent);
    }
    ...
    function getLoyalty(
        ...
        return loyalties[assetId][right_holder];
    }
    ...
    function _isInLoyalty(uint256 assetId)
        internal
        view
        returns (bool)
    {
    ...
    function _isResaleAllowed(uint256 assetId, address currentUser)
        ...
        returns (bool)
    {
        return address(creators[assetId]) == currentUser ? true: resaleStatuses[assetId] != RESALE_REJECTED;
    }
    ...
    function _addLoyalty(
        ...
        loyalties[assetId][right_holder] = percent;
        hasLoyalty[assetId] = true;
        emit AddLoyalty(address(this),assetId, right_holder, percent);
    }
