...
contract Fidu is ERC20PresetMinterPauserUpgradeSafe {
  ...
  */
    ...
    function mintTo(address to, uint256 amount) public {
        require(
            canMint(amount),
            "Cannot mint: it would create an asset/liability mismatch"
        );
    ...
    function burnFrom(address from, uint256 amount) public override {
        ...
        require(
            canBurn(amount),
            "Cannot burn: it would create an asset/liability mismatch"
        );
        _burn(from, amount);
    }
    ...
    function canMint(uint256 newAmount) public view returns (bool) {
        ISeniorPool seniorPool = config.getSeniorPool();
        uint256 liabilities = totalSupply()
            .add(newAmount)
            .mul(seniorPool.sharePrice())
            .div(fiduMantissa());
        ...
        if (_assets >= liabilitiesInDollars) {
            ...
            return
                liabilitiesInDollars.sub(_assets) <=
    ...
    function canBurn(uint256 amountToBurn) public view returns (bool) {
        ISeniorPool seniorPool = config.getSeniorPool();
        uint256 liabilities = totalSupply()
            .sub(amountToBurn)
            .mul(seniorPool.sharePrice())
    ...
    function updateGoldfinchConfig() public {
        require(
            hasRole(MINTER_ROLE, _msgSender()),
            "ERC20PresetMinterPauser: Must have minter role to change config"
        );
