...
contract PoolFunctionality is Ownable, IPoolFunctionality {
    ...
    function _doSwapTokens(InternalSwapData memory swapData) internal returns (uint256 amountIn, uint256 amountOut) {
...

        {
            ...
            if (swapData.supportingFee) {
                amounts[0] = IERC20(swapData.path[0]).balanceOf(initialTransferSource) - curBalance;
                require(amounts[0] <= amountIn, "IAI");
            }
        }

        {
            ...
            require(amountOut <= amounts[swapData.path.length - 1], "IAO");
        }
    ...
    function _swapCurve(
        ...
        uint256 currentSwapAmount = amounts[0];
        for (uint256 i; i < path.length - 1; ++i) {
            (address input, address output) = (path[i], path[i + 1]);
            ...
            if (IERC20(input).allowance(address(this), pool) < currentSwapAmount) {
                IERC20(input).safeIncreaseAllowance(pool, type(uint256).max);
            }
            if (is_meta)
                ICurvePool(pool).exchange_underlying(inputInd, outputInd, currentSwapAmount, 0);
            else
                ICurvePool(pool).exchange(inputInd, outputInd, currentSwapAmount, 0);

            if (supportingFee) {
                currentSwapAmount = IERC20(path[i + 1]).balanceOf(address(this)) - curBalance;
                require(currentSwapAmount <= amounts[i + 1], "ICSA");
            } else {
                currentSwapAmount = amounts[i + 1];
            }
        }
    }
