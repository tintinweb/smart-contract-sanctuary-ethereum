...
interface iMachine {
    function register_contract(address _contract) external;
...
contract theMachine is protected, iMachine {
    ...
    function register_contract(address _contract) public override safe {
        iContract Contract;
        Contract = iContract(_contract);
        require(Contract.getOwner() == msg.sender, "not owner");
        registered_contracts[_contract].Contract = Contract;
        registered_contracts[_contract].owner = Contract.getOwner();
        registered_contracts[_contract].is_active = true;
    }
...
contract theContract is protected, iContract {
    ...
    function set_machine(address addy) 
        ...
        machine = addy;
        Machine = iMachine(machine);
        Machine.register_contract(address(this));
    }
    // !SECTION Setters
