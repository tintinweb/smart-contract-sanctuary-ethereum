...
// |                                                                  |
// ====================================================================
// ====================== OrionUnifiedFarmTemplate =====================
// ====================================================================
// Farming contract that accounts for veORN
// Overrideable for UniV3, ERC20s, etc
// New for V2
//      - Multiple reward tokens possible
//      - Can add to existing locked stakes
//      - Contract is aware of proxied veORN
//      - veORN multiplier formula changed
// Apes together strong

// Orion Finance: https://github.com/OrionFinance
...
// Originally inspired by Synthetix.io, but heavily modified by the Orion team
// (Locked, veORN, and UniV3 portions are new)
// https://raw.githubusercontent.com/Synthetixio/synthetix/develop/contracts/StakingRewards.sol

import "@openzeppelin/contracts/utils/math/Math.sol";
import "./IveORN.sol";
import "./interfaces/IOrionGaugeController.sol";
import "./interfaces/IOrionGaugeORNRewardsDistributor.sol";
...
contract OrionUnifiedFarmTemplate is Owned, ReentrancyGuard {

    /* ========== STATE VARIABLES ========== */

    // Instances
    IveORN private immutable veORN;
    ...
    // veORN related
    uint256 public veorn_boost_scale_factor = uint256(4e18); // E18. 4x = 4e18; 100 / scale_factor = % veorn supply needed for max boost
    uint256 public veorn_max_multiplier = uint256(2e18); // E18. 1x = 1e18
    uint256 public veorn_per_frax_for_max_boost = uint256(4e18); // E18. 2e18 means 2 veORN must be held by the staker per 1 FRAX
    mapping(address => uint256) internal _veornMultiplierStored;
    mapping(address => bool) internal valid_veorn_proxies;
    ...
    function rewardRates(uint256 token_idx) public view returns (uint256 rwd_rate) {
        ...
        if (gauge_controller_address != address(0)) {
            rwd_rate = (IOrionGaugeController(gauge_controller_address).global_emission_rate() * last_gauge_relative_weights[token_idx]) / 1e18;
        }
        else {
    ...
    // Takes into account the locked stake time multiplier and veORN multiplier
    ...
    function userStakedOrion(address account) public view returns (uint256) {
        return (fraxPerLPStored * _locked_liquidity[account]) / MULTIPLIER_PRECISION;
    }

    function proxyStakedOrion(address proxy_address) public view returns (uint256) {
        return (fraxPerLPStored * proxy_lp_balances[proxy_address]) / MULTIPLIER_PRECISION;
    }

    // Max LP that can get max veORN boosted for a given address at its current veORN balance
    function maxLPForMaxBoost(address account) external view returns (uint256) {
        return (veORN.balanceOf(account) * MULTIPLIER_PRECISION * MULTIPLIER_PRECISION) / (veorn_per_frax_for_max_boost * fraxPerLPStored);
    }
    ...
    function minVeORNForMaxBoost(address account) public view returns (uint256) {
        return (userStakedOrion(account) * veorn_per_frax_for_max_boost) / MULTIPLIER_PRECISION;
    }

    function minVeORNForMaxBoostProxy(address proxy_address) public view returns (uint256) {
        return (proxyStakedOrion(proxy_address) * veorn_per_frax_for_max_boost) / MULTIPLIER_PRECISION;
    }

    function getProxyFor(address addr) public view returns (address){
        if (valid_veorn_proxies[addr]) {
            // If addr itself is a proxy, return that.
            // If it farms itself directly, it should use the shared LP tally in proxyStakedOrion
            return addr;
        }
        else {
    ...
    function veORNMultiplier(address account) public view returns (uint256 veorn_multiplier) {
        // Use either the user's or their proxy's veORN balance
        uint256 veorn_bal_to_use = 0;
        address the_proxy = getProxyFor(account);
        veorn_bal_to_use = (the_proxy == address(0)) ? veORN.balanceOf(account) : veORN.balanceOf(the_proxy);

        // First option based on fraction of total veORN supply, with an added scale factor
        veorn_multiplier = (veorn_bal_to_use * veorn_max_multiplier * veorn_boost_scale_factor) 
                            / (veORN.totalSupply() * MULTIPLIER_PRECISION);
        
        // Cap the boost to the veorn_max_multiplier
        if (veorn_multiplier > veorn_max_multiplier) veorn_multiplier = veorn_max_multiplier;
    }

    /* =============== MUTATIVE FUNCTIONS =============== */


    // Proxy can allow a staker to use their veORN balance (the staker will have to reciprocally toggle them too)
    // Must come before stakerSetVeORNProxy
    // CALLED BY PROXY
    function proxyToggleStaker(address staker_address) external {
        require(valid_veorn_proxies[msg.sender], "Invalid proxy");
    ...
    // Staker can allow a veORN proxy (the proxy will have to toggle them first)
    // CALLED BY STAKER
    function stakerSetVeORNProxy(address proxy_address) external {
        require(valid_veorn_proxies[proxy_address], "Invalid proxy");
    ...
    function updateRewardAndBalance(address account, bool sync_too) public {
        ...
        if (account != address(0)) {
            ...
            // Update the user's stored veORN multipliers
            _veornMultiplierStored[account] = new_veorn_multiplier;
    ...
    function _syncEarned(address account) internal {
        if (account != address(0)) {
            ...
            // console.log("syncEarned");
            // Update the rewards array
            for (uint256 i = 0; i < earned_arr.length; i++){ 
                rewards[account][i] = earned_arr[i];
                // console.log(earned_arr[i]);
            }
    ...
    function retroCatchUp() internal {
        ...
        for (uint256 i = 0; i < rewardDistributors.length; i++){ 
            ...
            if (reward_distributor_address != address(0)) {
                IOrionGaugeORNRewardsDistributor(reward_distributor_address).distributeReward(address(this));
            }
        }
        ...
        for (uint256 i = 0; i < gaugeControllers.length; i++){ 
            ...
            if (gauge_controller_address != address(0)) {
                if (force_update || (block.timestamp > last_gauge_time_totals[i])){
                    ...
                    last_gauge_relative_weights[i] = IOrionGaugeController(gauge_controller_address).gauge_relative_weight_write(address(this), block.timestamp);
                    last_gauge_time_totals[i] = IOrionGaugeController(gauge_controller_address).time_total();
                }
            }
        }
    }
    ...
    // Adds a valid veORN proxy address
    function toggleValidVeORNProxy(address _proxy_addr) external onlyByOwnGov {
        valid_veorn_proxies[_proxy_addr] = !valid_veorn_proxies[_proxy_addr];
    }
    ...
    function setMiscVariables(
        ...
        // [1] uint256 _veorn_max_multiplier, 
        // [2] uint256 _veorn_per_frax_for_max_boost,
        // [3] uint256 _veorn_boost_scale_factor,
        ...
        veorn_max_multiplier = _misc_vars[1];
        veorn_per_frax_for_max_boost = _misc_vars[2];
        veorn_boost_scale_factor = _misc_vars[3];
...
// |                                                                  |
// ====================================================================
// ======================= OrionUnifiedFarm_ERC20 ======================
// ====================================================================
// For ERC20 Tokens
// Uses OrionUnifiedFarmTemplate.sol

import "./OrionUnifiedFarmTemplate.sol";

contract OrionUnifiedFarm_ERC20 is OrionUnifiedFarmTemplate {
    ...
    OrionUnifiedFarmTemplate(_owner, _rewardTokens, _rewardManagers, _rewardRatesManual, _gaugeControllers, _rewardDistributors, veORN_)
    {
        ...
        )
    {
        ...
        // Get the veORN multipliers
        // For the calculations, use the midpoint (analogous to midpoint Riemann sum)
        new_veorn_multiplier = veORNMultiplier(account);

        uint256 midpoint_veorn_multiplier;
        if (
            (_locked_liquidity[account] == 0 && old_combined_weight == 0) || 
            (new_veorn_multiplier >= _veornMultiplierStored[account])
        ) {
            // This is only called for the first stake to make sure the veORN multiplier is not cut in half
            // Also used if the user increased or maintained their position
            midpoint_veorn_multiplier = new_veorn_multiplier;
        }
        else {
            // Handles natural decay with a non-increased veORN position
            midpoint_veorn_multiplier = (new_veorn_multiplier + _veornMultiplierStored[account]) / 2;
        }
        ...
        for (uint256 i = 0; i < lockedStakes[account].length; i++) {
            ...
            uint256 combined_boosted_amount = liquidity + ((liquidity * (midpoint_lock_multiplier + midpoint_veorn_multiplier)) / MULTIPLIER_PRECISION);
...
interface IOrionGaugeController {
    struct Point {
        uint256 bias;
        uint256 slope;
    }
...
interface IOrionGaugeORNRewardsDistributor {
  function acceptOwnership() external;
