 ...
 * @title KYCRegistry
 * @author Ondo Finance
 * @notice This contract manages KYC status for addresses that interact with Ondo
 */

contract KYCRegistry is AccessControlEnumerable {
  /// @dev Admin role that has permission to add/remove KYC status
  bytes32 public constant REGISTRY_ADMIN = keccak256("REGISTRY_ADMIN");

  /// @dev Mapping from address to boolean that reflects if address is KYC'd
  ...
  /**
   * @notice Add addresses to KYC list
   *
   * @param addresses List of addresses to grant KYC'd status
   */
  function addKYCAddresses(address[] calldata addresses)
    external
    onlyRole(REGISTRY_ADMIN)
  {
    uint256 length = addresses.length;
    for (uint256 i = 0; i < length; i++) {
      isKYCd[addresses[i]] = true;
    }
  }
   ...
   */
  function removeKYCAddresses(address[] calldata addresses)
    external
    onlyRole(REGISTRY_ADMIN)
  {
    uint256 length = addresses.length;
    for (uint256 i = 0; i < length; i++) {
      isKYCd[addresses[i]] = false;
    }
  }
}
