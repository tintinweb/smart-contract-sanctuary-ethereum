...
contract testContract is ERC721A, Ownable {
    ...
    uint256 public priceReferral = 0.005 ether;

    uint256 public maxPerTx = 10;

    uint256 public referralSplit = 2; //Mint Proceeds are split in 2
    ...
    //mapping(uint256 => address) private _leaderboardPosition;

    constructor() ERC721A("testContract", "TEST") {}

    function mint(uint256 count) external payable {
        uint256 cost = price;

        require(msg.value >= count * cost, "Incorrect Eth amount");
    ...
    function mintReferral(
        ...
        require(mintEnabled, "Minting is not live yet");
        require(count < maxPerTx + 1, "Max per TX reached.");
        require(_referralBalance[referral] >= 1, "Invalid referral");
        require(referral != msg.sender, "Cannot refer self");

        if (_referralBalance[msg.sender] < 1) {
            _referralBalance[msg.sender]++;
        }
        _referralBalance[referral] += count;
        referral.transfer(share);
        _safeMint(msg.sender, count);
    ...
    //function referralBalance(address walletAddress) public view returns (uint256){
    //    return _referralBalance[walletAddress];
    //}
