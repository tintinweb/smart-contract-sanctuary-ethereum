...
library PeriodStaking {
    ...
    function updatePeriod(PeriodStakingStorage storage periodStakingStorage) internal {
        ...
        if (walletStakedAmount.stakedBalance > 0 && walletStakedAmount.lastUpdate < periodStakingStorage.currentRewardPeriodId && walletStakedAmount.lastUpdate > 0) {
            ...
            for (; i <= periodStakingStorage.currentRewardPeriodId; i++) {
                RewardPeriod storage rewardPeriod = periodStakingStorage.rewardPeriods[i];
                periodStakingStorage.walletRewardableCapital[i][msg.sender] = walletStakedAmount.stakedBalance * (rewardPeriod.end - rewardPeriod.start);
    ...
    function getWalletRewardPeriodStakingScore(
        ...
        if (walletStakedAmount.lastUpdate > 0 && walletStakedAmount.lastUpdate < period) {
            ...
            return periodStakingStorage.walletRewardableCapital[period][wallet];
        }
    }
    ...
    function stakeRewardPeriod(
        ...
        periodStakingStorage.walletRewardableCapital[periodStakingStorage.currentRewardPeriodId][msg.sender] += (currentRewardPeriod.end - block.number) * amount;
    ...
    function claimRewardPeriod(
        ...
        require(periodStakingStorage.walletRewardableCapital[rewardPeriodId][msg.sender] > 0, "no rewards to claim");
        ...
        uint256 payableRewardAmount = calculatePeriodRewards(
            ...
            periodStakingStorage.walletRewardableCapital[rewardPeriodId][msg.sender]
        );
        periodStakingStorage.walletRewardableCapital[rewardPeriodId][msg.sender] = 0;
    ...
    function calculateWalletRewardsYieldPeriod(
        ...
        uint256 offset = calculateRewardableCapital(rewardPeriod, currentStakedBalance);
        if(offset >= walletRewardableCapital) return 0;
        uint256 currentWalletRewardableCapital = walletRewardableCapital - offset;
        uint256 walletRewards18 = (currentWalletRewardableCapital * yieldPeriod) / 10000 / 100;
    ...
    function calculateRewardableCapital(RewardPeriod memory rewardPeriod, uint256 amount) internal view returns (uint256) {
        ...
        if (block.number > rewardPeriod.end) blockNumber = 0;
        uint256 stakingDuration = (rewardPeriod.end - blockNumber) * 10**18;
