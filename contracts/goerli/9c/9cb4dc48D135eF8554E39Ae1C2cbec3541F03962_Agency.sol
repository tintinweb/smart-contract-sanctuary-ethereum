...
contract Agency is ERC721A, ERC2981, Ownable { // todo: rename contract, constructor, file

  uint constant EARLY_SUPPLY = 5000;
  uint constant MAX_SUPPLY = 7_700;
  uint constant TEAM_MINT_SUPPLY = 200;

  uint public price = 0.0001 ether; // todo: update w final price
  ...
  bool public missionsAreGo = true; // todo: false
  bool public itemsAreUpgradeable = true; // todo: false

  bool public debug = true; // todo: remove
  function setDebug(bool _v) external { debug = _v; } // todo: remove
  ...
  // todo: update these
  constructor() ERC721A("INTERNATIONAL", "INTL") {
    // Hello, Agent 0
    _mint(msg.sender, 1);
  }
  ...
  //     addressHasMinted[msg.sender] = true; // todo: bring back in
  //   }
  ...
  function onERC721Received(address, address from, uint256 tokenId, bytes memory)
    ...
    returns(bytes4)
  {
    if (missionsAreGo == false) revert ProgramNotYetBegun();
