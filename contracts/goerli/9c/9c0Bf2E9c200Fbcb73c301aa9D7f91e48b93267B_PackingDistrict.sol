...
contract PackingDistrict is ERC721, Ownable {
    ...
    function tokenURI(
        ...
        string memory seedStr = uint256(seed).toHexString(32);
        ...
        requests[3].wrapPrefix = '<script>';
        requests[3].wrapSuffix = '</script>';
        requests[3].scriptContent = scriptContent;

        uint256 finalBufferSize = bufferSize + bytes('<script>').length + bytes('</script>').length + scriptContent.length;
        ...
        string memory name = string.concat("Packing #", tokenIdStr);
        string memory image = string.concat(baseURI, "?tokenId=", tokenIdStr, "&seed=", seedStr);
        bytes memory metadata = abi.encodePacked(
            '{"name":"',
                name,
            '", "description":"Just a simple circle packing algorithm using p5js", "image":"',
                image,
            '", "animation_url":"',
