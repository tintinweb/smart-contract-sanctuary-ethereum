...
abstract contract SnowInterface {
    function Claim(uint256 _tokenID, address receiver) public virtual;
}

pragma solidity >=0.8.17 <0.9.0;

contract MintingContract is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
...
// ================== Burn Functions Start ======================= 
 ...
 function openBox(uint256 _tokenID, address receiver) public nonReentrant{
  
  // requirements
  require(msg.sender == ownerOf(_tokenID), 'Not the owner!');
  require(claimToggle, 'Claiming phase not started yet');


  // burning 
   _burn(_tokenID);
   tokenIDBurnt[_tokenID]= true;
   tokensBurned +=1;  

  // minting of the box
  SnowInterface claimContract = SnowInterface(claimingContract);
  claimContract.Claim(_tokenID, receiver);

  // updating of the mappings
...
// burn toggle
  function setclaimToggle() public onlyOwner{
    claimToggle = !claimToggle;
  }  

// set claiming contract
  function setClaimingContract(address _contractAddress) public onlyOwner{
    claimingContract = _contractAddress;
  }   
