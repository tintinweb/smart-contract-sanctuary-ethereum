...
contract CollarFlexStake is Ownable, ReentrancyGuard, Pausable, SafeERC20 {

    address[9] public approvedTokens;
    address public rewardToken;
    uint128 constant public max_staking_amount = 500 * 1e9 * 1e18;
    uint128 public remaining_reward_amount = 200 * 1e6 * 1e18;
    uint128 public lp_staked;
    uint128 public native_staked;
    ...
    function depositTokens(address _tokenAddress, uint256 _tokenAmount) external whenNotPaused nonReentrant {
        if(checkTokenLimit(_tokenAddress, _tokenAmount)) revert stakingAmountReached();

        currentID++ ;

        if(isLPApproved[_tokenAddress]) {
            lp_staked + _tokenAmount;
        } else if(isNativeApproved[_tokenAddress]) {
            native_staked + _tokenAmount;
        }

        uint256 reward = calculateReward(_tokenAmount, (max_staking_days * 86400));
        if(reward <= remaining_reward_amount ) revert rewardAmountExceed();

        remaining_reward_amount -= uint128(reward);
    ...
    function checkTokenLimit(address _tokenAddress, uint256 _tokenAmount) public view returns(bool) {
        ...
        if(isLPApproved[_tokenAddress]) {
            return (lp_staked + _tokenAmount) <= max_staking_amount * 80 / 100 ;
        } else if(isNativeApproved[_tokenAddress]) {
            return (native_staked + _tokenAmount) <= max_staking_amount * 20 / 100 ;
    ...
    function withdraw(uint256 _stakeID) external whenNotPaused nonReentrant {
        ...
        remaining_reward_amount += uint128( user.allocateReward - user.rewardEarned);
