...
contract PanaverseDAO is GovernorPreventLateQuorum, GovernorCountingSimple, GovernorSettings, GovernorVotesQuorumFraction, GovernorTimelockControl {

  uint public constant INVERSE_BASIS_POINT660 = 16;
  uint[] public savedNumbers;
  constructor (string memory name_, uint256  initialVotingDelay, uint256  initialVotingPeriod, uint256  initialProposalThreshold, uint64  initialVoteExtension, uint256  quorumNumeratorValue, IVotes  tokenAddress, TimelockController  timelockAddress)
    Governor(name_)
  ...
  function proposalThreshold() public virtual view  override (Governor, GovernorSettings) returns (uint256  )
  {
    return super.proposalThreshold();
  }
  

  function _castVote(uint256 proposalId, address account, uint8 support, string memory reason, bytes memory params) internal virtual   override (Governor, GovernorPreventLateQuorum) returns (uint256  )
  {
    return super._castVote(proposalId,account,support,reason,params);
  }
  

  function proposalDeadline(uint256 proposalId) public virtual view  override (Governor, GovernorPreventLateQuorum, IGovernor) returns (uint256  )
  {
    return super.proposalDeadline(proposalId);
  }
  

  function _execute(uint256 proposalId, address[] memory targets, uint256[] memory values, bytes[] memory calldatas, bytes32 descriptionHash) internal virtual override (Governor, GovernorTimelockControl)
  {
    return GovernorTimelockControl._execute(0, targets, values, calldatas, descriptionHash);
  }
  ...
  function _executor() internal virtual view override (Governor, GovernorTimelockControl) returns (address )
  {
    return GovernorTimelockControl._executor();
  }
  

  function state(uint256 proposalId) public virtual view override (Governor, GovernorTimelockControl) returns (ProposalState )
  {
    return GovernorTimelockControl.state(proposalId);
  }
  

  function supportsInterface(bytes4 interfaceId) public virtual view  override (Governor, GovernorTimelockControl) returns (bool  )
  {
    return super.supportsInterface(interfaceId);
  }
  

  function getNumber(uint _index_) public returns (uint) { return savedNumbers[_index_];}
  function _addNumber(uint _number) private {savedNumbers.push(_number);}
}
