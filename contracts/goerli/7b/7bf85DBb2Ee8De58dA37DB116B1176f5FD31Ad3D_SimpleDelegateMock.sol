    ...
    IModuleListener
{
    ...
    function onListenRemoved(bytes32 hname)
        public
        virtual
        override
        onlyManager
    ...
    function onUpdate(address oldInstance, address _manager)
        external
        virtual
        override
    {
    ...
    function _onUpdate(address oldInstance, address _manager)
        internal
        virtual
        onlyManager
    {
...
/*
 * System-wide roles manager
 */
contract RoleManager is ModuleUpgradeable {
    mapping(bytes32 => bytes32[]) public roles;
    mapping(bytes32 => mapping(bytes32 => uint256)) public roleIds;
    ...
    function getId() public view override returns (bytes32) {
        return CoreConsts.ROLE_MANAGER_MODULE_ID;
    }

    function getName() public view override returns (string memory) {
        return CoreConsts.ROLE_MANAGER_MODULE_NAME;
    }
    ...
    function grantRole(bytes32 role, address account) public virtual override {
        ...
        bytes32 hname = IModuleManager(manager).getModuleId(account);
        if (hname == bytes32(0)) return;

        roleIds[hname][role] = roles[hname].length;
        roles[hname].push(role);
    }
    ...
    function revokeRole(bytes32 role, address account) public virtual override {
        ...
        delete roles[hname][roleIds[hname][role]];
        delete roleIds[hname][role];
    }

    function transferRoles(
        bytes32 hname,
        address old_instance,
        address new_instance
    ) public virtual onlyManager {
        for (uint i = 0; i < roles[hname].length; i++) {
            _revokeRole(roles[hname][i], old_instance);
            _grantRole(roles[hname][i], new_instance);
        }
    }
}
...
abstract contract ConnectorTXNFTDecoder is Initializable {
    address public TX_NFT_decoder;

    uint256[256]        private __gap;

    event DecoderLink(address _addr);

    function __Decoder_init() internal onlyInitializing {}

    function __Decoder_onListenAdded(
        bytes32 hname,
        address contractInstance,
        bool isNew
    ) internal returns (bool) {
        if (hname == PluginsConsts.TX_NFT_DECODER_ID) {
            _linkDecoder(contractInstance);
            return true;
        } else return false;
    }

    function __Decoder_onListenRemoved(bytes32 hname) internal returns (bool) {
        if (hname == PluginsConsts.TX_NFT_DECODER_ID) {
            TX_NFT_decoder = address(0);
            return true;
        } else return false;
    }

    function _linkDecoder(address addr) internal {
        TX_NFT_decoder = addr;
        emit DecoderLink(addr);
    }
}

// SPDX-License-Identifier: Unlicense
interface IAuthTXPlugin {
    function authorize(
        address _from,
        address _walletAddr,
        address payable _to,
...
import "../../proxywallet/ProxyHelper.sol";

import "./TXNFTDecoder.sol";
import "./RentableTokensStorage.sol";

import "./TXDefaultValidatorPluginBase.sol";
import "./AuthPluginsLib.sol";

import "./IAuthTXPlugin.sol";

//import "hardhat/console.sol";

error TXDefaultValidatorPlugin_PostValidationFailed_ERC721TokenMissplacingAttempt(
    address _from,
    address _walletAddr,
    address payable _to,
    uint256 _value,
...
error TXDefaultValidatorPlugin_ApproveForbidden(
    address _from,
    address _walletAddr,
    address _tokenAddr,
    uint256 _tokenId,
    address _delegateAddr
);

contract TXDefaultValidatorPlugin is
    ReentrancyGuardUpgradeable,
    ProxyHelper,
    TXDefaultValidatorPluginBase,
    IAuthTXPlugin
{
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableSet for EnumerableSet.Bytes32Set;

    address public latestDestination;
    ...
    function initialize() public virtual initializer {
        ...
        TXDefaultValidatorPluginBase.__Base_init();
    }

    function isValidOwnership(bytes32 _tokenId) external view returns (bool) {
        return _isValidOwnership(_tokenId);
    }

    function _isValidOwnership(bytes32 _tokenId) internal view returns (bool) {
        if (!rentable_storage.isRentable(_tokenId)) return true;
        AppConsts.Token memory token = rentable_storage.getTokenFromId(
            _tokenId
        );
        address holder;
        try IERC721(token.tokenAddr).ownerOf(token.tokenId) returns (
            address res
        ) {
            holder = res;
        } catch {
        ...
        address proxyWallet = rentable_storage.getLockedTo(_tokenId);
        if (holder == proxyWallet) return true;
        if (rentable_storage.isTrustedTokenHolder(holder)) return true;
        if (
            rentable_storage.isTrustedProjectTokenHolder(
                token.tokenAddr,
                holder
            )
        ) return true;
        if (rentable_storage.isTrustedIndividualTokenHolder(_tokenId, holder))
            return true;
        if (rentable_storage.isTrustedHolder4ProxyWallet(proxyWallet, holder))
            return true;
        return false;
    }

    function _isValidDelegate(bytes32 _tokenId) internal view returns (bool) {
        if (!rentable_storage.isRentable(_tokenId)) return true;
        AppConsts.Token memory token = rentable_storage.getTokenFromId(
            _tokenId
        );
        address delegate;
        try IERC721(token.tokenAddr).getApproved(token.tokenId) returns (
            address res
        ) {
            delegate = res;
        } catch {
        ...
        if (delegate == address(0)) return true;
        address proxyWallet = rentable_storage.getLockedTo(_tokenId);
        if (delegate == proxyWallet) return true;
        if (rentable_storage.isTrustedTokenDelegate(delegate)) return true;
        if (
            rentable_storage.isTrustedProjectTokenDelegate(
                token.tokenAddr,
                delegate
            )
        ) return true;
        if (
            rentable_storage.isTrustedIndividualTokenDelegate(
                _tokenId,
                delegate
            )
        ) return true;
        if (
            rentable_storage.isTrustedDelegate4ProxyWallet(
                proxyWallet,
                delegate
            )
        ) return true;
    ...
    function authorize(
        ...
        latestDestination = _to;
        latestCaller = _walletAddr;
        latestUser = _from;
        latestValue = _value;
        latestData = _data;
        isAdmin = hasRole(AppConsts.PROXY_WALLET_ADMIN_ROLE, _from);
        isUser = hasRole(AuthPluginsLib.getUserRole(_walletAddr), _from);
        _forwardTx(payable(TX_NFT_decoder), gasleft(), 0, _data);
    }
    ...
    function postValidate(
        ...
        bytes32[] memory tokens = rentable_storage.getLockedTokensFor(
            _walletAddr
        );
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (!_isValidOwnership(tokens[i])) {
                ...
                revert TXDefaultValidatorPlugin_PostValidationFailed_ERC721TokenMissplacingAttempt(
                    _from,
                    _walletAddr,
                    _to,
                    _value,
            ...
            if (!_isValidDelegate(tokens[i])) {
                address token_delegate;
                try
                    IERC721(token.tokenAddr).getApproved(token.tokenId)
                returns (address res) {
                    token_delegate = res;
                } catch {}
                revert TXDefaultValidatorPlugin_ApproveForbidden(
                    _from,
                    _walletAddr,
                    token.tokenAddr,
                    token.tokenId,
...
import "../AppModuleUpgradeable.sol";
import "./PluginsConsts.sol";
import "./ConnectorRentableTokensStorage.sol";
import "./ConnectorTXNFTDecoder.sol";

abstract contract TXDefaultValidatorPluginBase is
    ConnectorRentableTokensStorage,
    ConnectorTXNFTDecoder,
    AppModuleUpgradeable
{
    ...
    function __Base_init() internal onlyInitializing {
        ...
        ConnectorTXNFTDecoder.__Decoder_init();

        AppModuleUpgradeable.__AppModule_init();
    }

    function getId() public view override returns (bytes32) {
        return PluginsConsts.DEFAULT_PLUGIN_ID;
    }

    function getName() public view override returns (string memory) {
        return PluginsConsts.DEFAULT_PLUGIN_NAME;
    }
    ...
    function _onListenAdded(
        ...
        if (
            ...
            if (
                !ConnectorTXNFTDecoder.__Decoder_onListenAdded(
                    hname,
                    contractInstance,
                    isNew
                )
    ...
    function _onListenRemoved(bytes32 hname) internal override {
        if (!ConnectorRentableTokensStorage.__Storage_onListenRemoved(hname))
            if (!ConnectorTXNFTDecoder.__Decoder_onListenRemoved(hname))
                ModuleUpgradeable._onListenRemoved(hname);
    }
}
...
import "../AppConsts.sol";
import "../common/RoleManager.sol";
import "./IRentableTokensStorage.sol";
import "./TXDefaultValidatorPlugin.sol";
import "../../proxywallet/IProxyWallet.sol";
import "hardhat/console.sol";

//error TXNFTDecoder_ApproveForbidden(address _from, address _walletAddr, address _tokenAddr, uint256 _tokenId, address _delegateAddr);
error TXNFTDecoder_Approval4AllForbidden(
    address _from,
    address _walletAddr,
    address _tokenAddr,
    address _delegateAddr,
...
error TXNFTDecoder_TransferForbidden(
    address _from,
    address _walletAddr,
    address _tokenAddr,
    address _source,
...
error TXNFTDecoder_WithdrawForbidden(
    address _from,
    address _walletAddr,
    address _tokenAddr,
    address _source,
...
error TXNFTDecoder_WithdrawNotToVault(
    address _from,
    address _walletAddr,
    address _tokenAddr,
    address _source,
...
error TXNFTDecoder_CallerNotUser(
    address _from,
    address _walletAddr,
    address _to,
    uint256 _value,
...
// TODO modularize this so its upgradeable
contract TXNFTDecoder {
    //    using EnumerableSet for EnumerableSet.AddressSet;
    //    using EnumerableSet for EnumerableSet.Bytes32Set;

    TXDefaultValidatorPlugin immutable auth_plug;
    IRentableTokensStorage immutable rentable_storage;
    RoleManager immutable role_manager;

    modifier authOnly() {
        require(msg.sender == address(auth_plug), "TXNFTDecoder: Wrong caller");
        _;
    }

    constructor(
        address _auth_plug_addr,
        address _rentable_storage_addr,
        address _role_manager_addr
    ) {
        auth_plug = TXDefaultValidatorPlugin(_auth_plug_addr);
        rentable_storage = IRentableTokensStorage(_rentable_storage_addr);
        role_manager = RoleManager(_role_manager_addr);
    }

    fallback() external payable authOnly {
        if (!auth_plug.isUser())
            revert TXNFTDecoder_CallerNotUser(
                auth_plug.latestUser(),
                auth_plug.latestCaller(),
                auth_plug.latestDestination(),
                auth_plug.latestValue(),
                auth_plug.latestData()
            );
    }
    ...
    function setApprovalForAll(
        ...
        if (!auth_plug.isUser())
            revert TXNFTDecoder_Approval4AllForbidden(
                auth_plug.latestUser(),
                auth_plug.latestCaller(),
                auth_plug.latestDestination(),
                operator,
        ...
        if (rentable_storage.isTrustedTokenDelegate(operator)) return;
    ...
    function safeTransferFrom(
        ...
        // if (from == auth_plug.latestCaller()) {
        //     IProxyWallet(auth_plug.latestCaller()).trackWithdrawNFT(
        //         auth_plug.latestDestination(),
        //         tokenId
        //     );
        // }
        _authorizeTransfer(from, to, tokenId);
    }
    ...
    function _authorizeTransfer(
        ...
        if (!auth_plug.isUser() && !auth_plug.isAdmin())
            revert TXNFTDecoder_TransferForbidden(
                auth_plug.latestUser(),
                auth_plug.latestCaller(),
                auth_plug.latestDestination(),
                from,
        ...
        bytes32 tokenUUID = AppConsts.token2Id(
            AppConsts.getTokenStruct(auth_plug.latestDestination(), tokenId)
        );
        if (auth_plug.isAdmin()) {
            if (
                (rentable_storage.getLockedTo(tokenUUID) == address(0)) &&
                (rentable_storage.isRentable(tokenUUID))
            )
                if (role_manager.hasRole(AppConsts.RENT_VAULT_ROLE, to)) return;
                else
                    revert TXNFTDecoder_WithdrawNotToVault(
                        auth_plug.latestUser(),
                        auth_plug.latestCaller(),
                        auth_plug.latestDestination(),
                        from,
            ...
            revert TXNFTDecoder_WithdrawForbidden(
                auth_plug.latestUser(),
                auth_plug.latestCaller(),
                auth_plug.latestDestination(),
                from,
...
/**
 * @title ProxyHelper
 * @notice Utilities for proxy wallet
 */
abstract contract ProxyHelper {
    /**
    ...
    function _forwardTx(
        ...
        (bool success, bytes memory res_data) = _to.call{
            value: _value,
            gas: _gas
        }(_data);
        if (success == false) {
    ...
    /**
     * @notice forwards a static call
     */
    function _forwardStaticCall(address payable _to, bytes memory _data)
        internal
        view
        virtual
        returns (bytes memory)
    {
        (bool success, bytes memory res_data) = _to.staticcall(_data);
        if (success == false) {
            assembly {
                let ptr := mload(0x40)
                let size := returndatasize()
...
contract ERC721Mock is ERC721 {
    constructor() ERC721("MOCK Token", "MT") {}

    function mint(address recipient, uint256 tokenId) public {
        _safeMint(recipient, tokenId, "");
    }

    // Allows to burn token. Normally, should not exists in normal ERC721 implementations
    function brokenTransferFrom(
        address from,
        address to,
        uint256 tokenId
    ) public {
        if (to != address(0)) _transfer(from, to, tokenId);
        else _burn(tokenId);
    }
}
...
import "../modules/txauthorizer/TXDefaultValidatorPlugin.sol";
...
error DelegateMock_NotProxyWallet();

// This is the main building block for smart contracts.
contract SimpleDelegateMock is IERC721Receiver{
    ...
    ERC721Mock	tokenContract;

    function setTokenAddr(address _token_addr) external {
	tokenContract = ERC721Mock(_token_addr);
    }
    ...
    function setApprovalForAll(address operator, bool _approved) external {
	tokenContract.setApprovalForAll(operator, _approved);
    }
