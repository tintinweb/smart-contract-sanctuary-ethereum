...
abstract contract Drop is IDrop, Role {
   ...
   /// @dev The next token ID of the NFT to claim.
    ...
    function claim(
        ...
        _beforeClaim(recipient, amount, _pricePerToken, _allowlistProof, _data);

        verifyClaim(_salesPhaseId, recipient, amount, _pricePerToken, _allowlistProof);

        // Update contract state.
        
        salesPhase.supplyClaimedByWallet[_salesPhaseId][recipient] += amount;
        salesPhase.limitLastClaimTimestamp[_salesPhaseId][recipient] = block.timestamp;

        // If there's a price, collect price.
        _collectPriceOnClaim(address(0), amount, _pricePerToken);

        // Mint the relevant tokens to claimer.
        _transferTokensOnClaim(recipient, _salesPhaseId, amount);

        _afterClaim(recipient, amount, _pricePerToken, _allowlistProof, _data);
    }
