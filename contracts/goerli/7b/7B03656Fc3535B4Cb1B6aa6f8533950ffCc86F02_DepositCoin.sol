...
contract DepositCoin is Adminable,CfoNftTakeable,ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    bytes32 public constant DOMAIN_TYPEHASH = keccak256("EIP712Domain(string name,uint256 chainId,address verifyingContract)");
    bytes32 public constant WITHDRAW_REWARDS_PERMIT_TYPEHASH = keccak256("withdraw(address account,address token,uint256 amount,uint256 rand)");
    // bytes32 public constant EXIT_PERMIT_TYPEHASH = keccak256("exit(address account,uint256[] nftIds,uint256[] hashRates,uint256 rand)");
    bytes32 public immutable DOMAIN_SEPARATOR;

    mapping(uint256 => bool) public isUsedRand;

    event Withdraw(address user, address token,uint256 reward,uint rand);

    constructor() {
        DOMAIN_SEPARATOR = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("DepositCoin")), ChainId.get(), address(this)));
    }

    // withdraw token
    function withdraw(address account, address token, uint amount, uint rand, uint8 v,bytes32 r,bytes32 s) public nonReentrant {
        ...
        require(account == msg.sender || isAdmin[account],"caller must be account");
        require(!isUsedRand[rand],"rand used");
        isUsedRand[rand] = true;
    
        address signatory = recoverWithdrawRewardsSign(account,token,amount,rand,v,r,s);
        require(signatory != address(0), "invalid signature");
        require(isAdmin[signatory], "unauthorized");
        ...
        emit Withdraw(account, token, amount, rand);
    }
    ...
    event DepositETH(address user, uint256 amount);
    ...
    // get accounts tokens balances list
    function accountsTokensBalancesList(address[] memory accounts, address[] memory tokens) public view returns(ata[] memory accoutBalancesList) {
        uint256 accountsLength = accounts.length;
        uint256 tokensLength = tokens.length;
        uint256 totalLength = accountsLength.mul(tokensLength);
        accoutBalancesList = new ata[](totalLength);
        uint256 _count = 0;
        
        for(uint i = 0; i < accountsLength; i++){
             for(uint i2 = 0; i2 < tokensLength; i2++){
                accoutBalancesList[_count++] = ata({
                    account: accounts[i],
                    token: tokens[i2],
                    amount: IERC20(tokens[i2]).balanceOf(accounts[i])
                });
            }   
        }
