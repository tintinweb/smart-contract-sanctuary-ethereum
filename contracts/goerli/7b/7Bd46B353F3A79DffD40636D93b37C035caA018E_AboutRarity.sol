...
contract AboutRarity is ERC721Drop {
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
...
// versione 0.9 revisione colori

    function tokenURI(uint256 tokenId) override public view returns (string memory){
        string[7] memory parts;

        //parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 16px; }</style><style>.footnote { fill: white; font-family: verdana; font-size: 8px; }</style><rect width="100%" height="100%" fill="black" /><text x="25" y="50" class="base">';

        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 16px; }</style><style>.footnote { fill: white; font-family: verdana; font-size: 8px; }</style><rect width="100%" height="100%" fill=';

        parts[2] = ' />';


        
        parts[4] = '<text x="25" y="50" class="base">';


        uint256 rand = random(string(abi.encodePacked(parts[0], toString(tokenId))));
        uint256 rand2 = random(string(abi.encodePacked(parts[0], parts[4], toString(tokenId))));
        ...
        if (greatness > 960) {
            ...
            //uint rColorR = 100;
            uint rColorG = 50 + rand % 150;
            uint rColorB = 55 + rand2 % 200;
            
            //parts[1] = string(abi.encodePacked('"rgb(', toString (rColorR), ',', toString (rColorG), ',', toString (rColorB), ')"'));
            //parts[1] = string(abi.encodePacked('"rgb(100,', toString (rColorG), ',', toString (rColorB), ')"'));

            if (greatness > 995) {
                
                parts[1] = string(abi.encodePacked('"rgb(125,', toString (rColorG), ',', toString (rColorB), ')"'));

                parts[3] = '<pattern id="p" width="50" height="50" patternUnits="userSpaceOnUse"><path d="M14.195 2.37c-3.107.02-6.144 1.23-8.394 3.48-3 3-4.15 7.4-3.1 11.5l.399 1.4 1.4.35c1 .25 2 .4 3 .4 3.15 0 6.201-1.25 8.451-3.5 3-3 4.15-7.4 3.1-11.5l-.35-1.4-1.4-.35a12.187 12.187 0 00-3.106-.38zm21.46 0a12.172 12.172 0 00-3.104.38l-1.4.35-.35 1.4c-1 4.15.15 8.55 3.15 11.5 2.25 2.25 5.35 3.5 8.45 3.5 1 0 2-.15 3-.4l1.4-.35.35-1.4c1.05-4.1-.1-8.5-3.1-11.5-2.25-2.25-5.289-3.46-8.397-3.48zm-28 28.1a12.173 12.173 0 00-3.104.38l-1.4.351L2.8 32.6c-1.05 4.1.1 8.5 3.1 11.5 2.25 2.25 5.349 3.5 8.449 3.5 1 0 2-.149 3-.399l1.4-.351.35-1.399c1.05-4.1-.1-8.5-3.1-11.5-2.213-2.25-5.24-3.46-8.346-3.48zm34.69 0c-3.107.022-6.144 1.231-8.394 3.481-3 3-4.151 7.4-3.101 11.5l.351 1.399 1.399.351c1 .25 2 .399 3 .399 3.15 0 6.2-1.25 8.45-3.5 3-3 4.15-7.4 3.1-11.5l-.3-1.399-1.399-.351a12.184 12.184 0 00-3.105-.38z" stroke-width="1" stroke="none" fill="rgb(20, 20, 20)" fill-opacity="0.1"/><path class="st18" d="M19.85 19.85c-2.4 2.4-5.35 4.15-8.5 5.1 3.15 1 6.1 2.7 8.5 5.1 2.4 2.4 4.15 5.35 5.1 8.55 1-3.15 2.7-6.1 5.1-8.55 2.4-2.4 5.35-4.15 8.5-5.1-3.15-1-6.1-2.7-8.5-5.1a20.27 20.27 0 01-5.1-8.55c-.95 3.2-2.65 6.15-5.1 8.55z" stroke-width="1" stroke="none" fill="rgb(20, 20, 20)" fill-opacity="0.1"/><path d="M22.25 0A2.75 2.75 0 0025 2.75 2.75 2.75 0 0027.75 0zM0 22.25v5.5A2.75 2.75 0 002.75 25 2.75 2.75 0 000 22.25zm50 0A2.75 2.75 0 0047.25 25 2.75 2.75 0 0050 27.75zm-25 25A2.75 2.75 0 0022.25 50h5.5A2.75 2.75 0 0025 47.25z" stroke-width="1" stroke="none" fill="rgb(20, 20, 20)" fill-opacity="0.1"/></pattern><rect fill="url(#p)" width="100%" height="100%"/>';

                parts[5] = "Text 1 leggendario";
                } else {
                
                parts[1] = string(abi.encodePacked('"rgb(100,', toString (rColorG), ',', toString (rColorB), ')"'));

                parts[5] = "Text 1 unico";
            }

            //uint serial = 1000000000 + rand % 100000000;
            //parts[8] = toString (serial);
            } else if (greatness > 730) {
                // rare
                //uint rColorR = 150;
                uint rColorG = 100 + 10 * (rand % 6);
                uint rColorB = 100 + 10 * (rand % 7);
                
                //parts[1] = string(abi.encodePacked('"rgb(', toString (rColorR), ',', toString (rColorG), ',', toString (rColorB), ')"'));
                parts[1] = string(abi.encodePacked('"rgb(150,', toString (rColorG), ',', toString (rColorB), ')"'));
                
                
                parts[5] = "Text 2 raro";
                //parts[8] = serial_2[rand % serial_2.length];
            } else {
                // common

                //uint rColorR = 200;
                uint rColorG = 100 + 15 * (rand % 4);
                uint rColorB = 100 + 15 * (rand % 5);
                
                //parts[1] = string(abi.encodePacked('"rgb(', toString (rColorR), ',', toString (rColorG), ',', toString (rColorB), ')"'));

                parts[1] = string(abi.encodePacked('"rgb(160,', toString (rColorG), ',', toString (rColorB), ')"'));

                parts[5] = "Text 3 normale";
                //parts[8] = serial_3[rand % serial_3.length];
            }

        //parts[6] = '</text><text x="10" y="54" class="base">';
        parts[6] = '</text><text x="260" y="330" class="footnote"> GOERLI_TEST_AboutR </text></svg>';

        //parts[7] = "# ";

        //parts[7] = "GOERLI_TEST_AboutR";
        
        //parts[7] = 'GOERLI_TEST_AboutR</text></svg>';

        string memory output = 
            string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6]));

        //string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "___NOME____: ', toString(tokenId), '", "description": "____DESCRIZIONE_NFT____", "attributes": [{"trait_type": "___trait_type___", "value": "',parts[5], '"}, {"trait_type": "___trait_type2_serial___", "value": "# ',parts[8], '"}], "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));

        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "# ', toString(tokenId), '", "description": "____DESCRIZIONE_NFT____", "attributes": [{"trait_type": "Rarity", "value": "',parts[5], '"}, {"trait_type": "Colour", "value": ',parts[1], '}], "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
