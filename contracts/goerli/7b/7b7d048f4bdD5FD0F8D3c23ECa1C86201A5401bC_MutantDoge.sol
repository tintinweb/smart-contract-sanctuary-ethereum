...
contract MutantDoge is ERC721, ERC1155Holder, Ownable, ReentrancyGuard {
  ...
  bool public m1MintState = false;
  bool public m2MintState = false;
  bool public megaMintState = false;
  ...
  constructor(address _serum) ERC721("Mutant Doge", "MDOG") {
  ...
  function flipMintState(uint8 _serumType) external onlyOwner {
    if (_serumType == serum.M1_SERUM()) m1MintState = !m1MintState;
    if (_serumType == serum.M2_SERUM()) m2MintState = !m2MintState;
    if (_serumType == serum.MEGA_SERUM()) megaMintState = !megaMintState;
  }

  function mintM1(uint256 _tokenId) external payable nonReentrant {
    require(m1MintState, "Mint haven't started");
    ...
    nonReentrant
  {
    ...
    require(
      m1Supply + _tokenIds.length <= m1MaxSupply,
    ...
    serum.burn(msg.sender, serum.M1_SERUM(), _tokenIds.length);
  }

  function mintM2(uint256 _tokenId) external payable nonReentrant {
    require(m2MintState, "Mint haven't started");
    ...
    serum.safeTransferFrom(msg.sender, address(this), serum.M2_SERUM(), 1, "");
  ...
  function mintMega() external payable nonReentrant {
    require(megaMintState, "Mint haven't started");
  ...
  ERC1155Supply
{
  ...
  uint8 private megaCount1 = 8; // 8 mega serum potions distribute to tokenId with 0 ended
  uint8 private megaCount2 = 4; // 4 mega serum potions distrubute to others
  uint256 private zeroSuffixCount = 1000;
  uint256 private nonZeroSuffixCount = 9000;
  ...
  constructor(string memory _baseURI, address _dogc) ERC1155(_baseURI) {
    baseURI = _baseURI;
    validSerumTypes[MEGA_SERUM] = true; // mega serum
    validSerumTypes[M1_SERUM] = true; // m1 serum
    dogc = Dogc(_dogc);
  }
  ...
  function uri(uint256 _typeId) public view override returns (string memory) {
    require(validSerumTypes[_typeId], "URI requested for invalid serum type");
    return
  ...
  function mintBatch() external payable nonReentrant {
    require(mintState, "Public mint haven't start");
    ...
    if (balanceOf(msg.sender, MEGA_SERUM) > 0) {
      return;
    }
    ...
    for (uint256 i = 0; i < validIdsLength; i++) {
      if (validIds[i] % 10 == 0) {
        if (megaCount1 > 0) {
          uint256 n = random(zeroSuffixCount);
          if (n < 8) {
            _mint(msg.sender, MEGA_SERUM, 1, "");
            megaCount1--;
            return;
          }
          zeroSuffixCount--;
        ...
        if (megaCount2 > 0) {
          uint256 n = random(nonZeroSuffixCount);
          if (n < 4) {
            _mint(msg.sender, MEGA_SERUM, 1, "");
            megaCount2--;
            return;
  ...
  function mintM2Batch() external payable nonReentrant {
    require(m2MintState, "Public mint haven't start");
    uint256[] memory ids = dogc.tokensOfOwner(msg.sender);
    ...
    _mint(msg.sender, M2_SERUM, ids.length, "");

    for (uint256 i = 0; i < ids.length; i++) {
      isM2Minted[ids[i]] = true;
    }
  }
  ...
  function flipM2MintState() external onlyOwner {
    m2MintState = !m2MintState;
  }

  function setSerumTypeValidation(uint8 _type, bool _validation)
    external
    onlyOwner
  {
    validSerumTypes[_type] = _validation;
  }
