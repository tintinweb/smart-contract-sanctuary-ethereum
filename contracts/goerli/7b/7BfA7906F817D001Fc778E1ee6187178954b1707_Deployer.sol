...
import "./LogicImproved.sol";
import "./Logic.sol";
import "./Proxy.sol";

/// @title Deployer contract
/// @notice Contract that deploys LogicImproved, Logic, and Proxy as well as sets ownership in a single atomic transaction
contract Deployer {
  LogicImproved public immutable logicImproved;
  Logic public immutable logic;
  Proxy public immutable proxy;

  constructor(address safeMultisig) {
    logicImproved = new LogicImproved();
    logic = new Logic();
    proxy = new Proxy(address(logic));
    (bool initSuccess, ) = address(proxy).call(
      abi.encodeWithSignature("initialize(uint256)", 123)
    );
    require(initSuccess, "Failed to initialize proxy");

    (bool ownershipSetSuccess, ) = address(proxy).call(
      abi.encodeWithSignature("transferOwnership(address)", safeMultisig)
    );
    require(ownershipSetSuccess, "Failed to set ownership of proxy");
  }
}
