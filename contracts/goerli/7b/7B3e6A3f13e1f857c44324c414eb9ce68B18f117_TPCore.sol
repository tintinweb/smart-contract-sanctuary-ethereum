...
contract TPCore is AccessControl {
    ...
    // 48 bit minimum
    uint48 public tpyRewardPerInterval = 1027 * 7 * 4 * (10**8); // per day * 4 week
    
    // 32 (max 1 year)
    uint32 public tpyInterval = 1 days;//4 weeks;
    ...
    function stake(uint104 amount) external {
        ...
        if (0 != amount) {
            require(amount <= stakingcoinAmount, "There is no as much allowance for stakingcoin");
    ...
    function unstake(uint104 amount) external {
        require(0 < amount, "The amount cannot be 0");
        
        uint256 totalReward = strategy.estimateReward(address(this));
        uint104 estimatedReward = estimateGrossReward(_msgSender());

        require(0 != estimatedReward, "Dont be greedy."); 

        // recalculation 
        uint104 stakingAmountPart = amount * userStaking[_msgSender()].amount / estimatedReward;
        uint104 profit = amount - stakingAmountPart;
        uint104 fee = (10000 - ownerPercentage) * profit / 10000;
        uint104 tpyToTransfer = amount - fee;
        uint104 fromYearnAmount = amount * amount / tpyToTransfer;
        // end of recalc
        ...
        takeReward(fromYearnAmount);// yearn => TPCore
    ...
    function totalTPYAvail() external view returns (uint256) { 
    ...
    function takeTPYReward(uint104 unstakeCoinAmount) private {
        ...
        if(tpyRewards == 0) { // No TPY Rewards !!!
        ...
        if(tpy.balanceOf(address(this)) < tpyToTransfer) { // Not enough tpy balance for TPCore !!!
