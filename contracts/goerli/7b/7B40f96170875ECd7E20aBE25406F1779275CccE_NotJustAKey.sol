...
contract NotJustAKey is ERC721A, Ownable {
  ...
  constructor(string memory _baseTokenURI, address _withdrawalAddress)
  ...
  function setMerkleRoots(
    ...
    bytes32 _publicRaffleMerkleRoot
  ) external onlyOwner {
  ...
  function processMint(uint256 _quantity) internal {
    ...
    require(totalSupply() + _quantity <= MAX_KEYS, "MAX CAP OF KEYS EXCEEDED");
  ...
  function remainingSale() external payable callerIsUser {
    require(saleStatus == SaleStatus.REMAINING, "REMAINING SALE IS NOT ACTIVE");
    require(
