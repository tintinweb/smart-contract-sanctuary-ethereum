...
contract Staker {
    ...
    uint256 public withdrawalPeriod = 1 minutes;
    ...
    modifier onlyCompleted() {
        ...
        require(
            completed,
            "Staking period must be over to perform this operation"
        );
        _;
    }

    function _setTimers() internal {
        withdrawalDeadline = block.timestamp + stakingPeriod;
        claimDeadline = withdrawalDeadline + withdrawalPeriod;
    }
        ...
        claimDeadlineReached(false)
    {
        ...
        depositTimestamps[msg.sender] = depositTimestamps[msg.sender] != 0
        ...
        notCompleted
    {
        ...
        uint256 stakingPeriods = stakingSession +
            1 -
            depositTimestamps[msg.sender];
        uint256 balanceRewards = (individualBalance + rewardRatePerBlock) *
            (stakingPeriods)**2;
        balances[msg.sender] = 0;
        depositTimestamps[msg.sender] = 0;

        require(
            address(this).balance > balanceRewards,
    ...
    /// Allow a user to start a new staking period by withdrawing ether from the ExampleExternalContract
