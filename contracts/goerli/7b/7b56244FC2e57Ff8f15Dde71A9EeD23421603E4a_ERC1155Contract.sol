...
contract ERC1155Contract is ERC1155, Ownable, PaymentSplitter, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    function checkClaimEligibility(address _address, uint256 _quantity, uint256 _id) external view returns (string memory) {
    ...
    function lockSupply(uint256 _id) external onlyOwner {
    ...
    function fiatMint(address _address, uint256 _quantity, uint256 _id, uint256 _value) internal {
        uint16 _currentSupply = tokens[_id].totalSupply;
        require(tokens[_id].saleIsActive, "Sale inactive");
        require(uint256(_currentSupply) + _quantity <= uint256(tokens[_id].maxSupply), "Insufficient supply");
        require(price(_id) * _quantity <= _value, "ETH incorrect");
        tokens[_id].totalSupply = _currentSupply + uint16(_quantity);
        _mint(_address, _id, _quantity, "");
    }

    function paper(address _address, uint256 _quantity, uint256 _id, bytes32 _nonce, bytes calldata _signature) external payable 
        onlyPaper(keccak256(abi.encode(_address, _quantity, _id)), _nonce, _signature)
    {
        fiatMint(_address, _quantity, _id, msg.value);
    }

    function claimFree(uint256 _id, uint16 _maxMint, uint16 _quantity, bytes32[] memory _proof) external {
