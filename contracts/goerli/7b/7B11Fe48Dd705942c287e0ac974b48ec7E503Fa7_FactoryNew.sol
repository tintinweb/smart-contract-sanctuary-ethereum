...
contract FactoryNew is OwnableUpgradeable{
    ...
    function initializeProxyInstance(address _proxyInstance, address _ownerWallet, address _stakedToken, 
                        ...
                        uint128 _withdrawalFees, uint128 _rewardRate, uint128 _emergencyFees, uint128 _platformFee, address _treasury, uint128 _maxStakeLimit) external onlyOwner {
        ...
        {
        bytes memory init = returnHash(_ownerWallet, _stakedToken, _stakeType, _stakePeriod, _depositFees, _withdrawalFees, _rewardRate, _emergencyFees, _platformFee, _treasury, _maxStakeLimit);
        if (init.length > 0)
    ...
    function returnHash(address _ownerWallet, address _stakedToken, 
        ...
        data = abi.encodeWithSignature("initialize(address,address,uint128,uint128,uint128,uint128,uint128,uint128,uint128,address,uint128)", _ownerWallet,_stakedToken,_stakeType,_stakePeriod,_depositFees,_withdrawalFees,_rewardRate, _emergencyFees,_platformFee,_treasury, _maxStakeLimit);
    }
