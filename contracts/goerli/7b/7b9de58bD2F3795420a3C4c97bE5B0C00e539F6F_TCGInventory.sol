...
contract TCGInventory is ERC721Enumerable, AccessControl, ReentrancyGuard {
    ...
    Data[] public template;
    mapping(uint256 => Card) private cardData;
    ...
    constructor() ERC721("BSTCG#1", "BSTCG#1"){
    ...
    function mint(uint256 templateID, address to)external onlyRole(MINTER_ROLE) returns(uint256){
        require(templateExists(templateID), "Template DNE");
        uint256 tD = tokenID;
        _mint(to, tokenID);
        tokenID++;
    ...
    function templateExists(uint256 templateID)public view returns(bool){
        return templateID < template.length;
    }
    ...
    function updateCardGameInformation(uint256 addWin, uint256 addPlayed, uint256 tokenId) public onlyRole(CONTRACT_ROLE){
        require(_isApprovedOrOwner(msg.sender, tokenId) && _exists(tokenId), "Not Approved");
    ...
    function burn(uint256 tokenId) external{
        require(_isApprovedOrOwner(msg.sender, tokenId) && _exists(tokenId), "Not Approved or Owner");
        _burn(tokenId);
    ...
    function dataReturn(uint256 tokenId) public view returns(uint8 level, uint8 top, uint8 left, uint8 right, uint8 bottom, uint256 winCount, uint256 playedCount){
    ...
    function addTemplateId(string memory imageURL, string memory description, string memory name, uint8 top, uint8 left, uint8 right, uint8 bottom, uint8 level)external onlyRole(ADMIN_ROLE){
        uint256 id = template.length;
        template.push(Data(imageURL, name, description, "", level, top, left, right, bottom));
        template[id].jsonStorage = _templateIdString(id);
        emit templateAdded(id);
    }

    function tokenURI(uint256 tokenId) public view virtual override returns(string memory){
        Card memory c = cardData[tokenId];
        string memory attributes = _attributes(c.templateId, c.level, c.top, c.left, c.right, c.bottom, c.winCount, c.playedCount);
        string memory _json = Base64.encode(bytes(string(abi.encodePacked(template[c.templateId].jsonStorage,  attributes))));
    ...
    function _templateIdString(uint256 templateId) internal view returns(string memory){
        return string(abi.encodePacked('{"name": "Awesome- ', template[templateId].name, '", "description": "', template[templateId].description,'","image": "', template[templateId].imageURL, '",'));
    }

    function updateImageURL(uint256 position, string memory newURL)external onlyRole(ADMIN_ROLE){
        require(templateExists(position), "Position out of Bounds");
    ...
    function _attributes(uint256 id, uint8 level, uint8 top, uint8 left, uint8 right, uint8 bottom, uint256 winCount, uint256 playedCount)internal view returns(string memory){
        ...
        return string(abi.encodePacked('"attributes": [{ "trait_type": "Name", "value": "', template[id].name, attribute,'" }, { "trait_type": "Win Count", "value": "', _wc, '" }, { "trait_type": "Played Count", "value": "', _pc, '" }]}'));
    }

    function _attributes1(uint8 level, uint8 top, uint8 left, uint8 right, uint8 bottom )internal pure returns(string memory){
