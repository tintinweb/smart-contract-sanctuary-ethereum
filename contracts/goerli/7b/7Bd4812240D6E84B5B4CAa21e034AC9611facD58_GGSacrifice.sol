...
contract GGSacrifice is Ownable, ReentrancyGuard {
   ...
   */
  function sacrifice(bytes calldata encoded, bytes calldata token) external noContract nonReentrant {
    ...
    // fail if wallet does not own one or more token
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      require(wallet == seekers.ownerOf(tokenId), "wallet does not own tokenId");
    }

    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      seekers.safeTransferFrom(msg.sender, deadAddress, tokenId);
    }
