    ...
    IERC721LA
{
    ...
    // Used to count differenciate burnt tokens in the bitmap logic (Null address being used unminted tokens)
    address public constant burnAddress = address(0xDEAD);

    // Used in baseURI {ID}
    bytes4 public constant tokenIDStringTemplate = "{id}";
    ...
    function _baseURIContainsTokenID(
        string memory baseURI
    ) internal view returns (bool) {
        bytes memory strAsBytes = bytes(baseURI);
        bytes4 last4Bytes = bytes4(
            abi.encodePacked(
                strAsBytes[strAsBytes.length - 4],
                strAsBytes[strAsBytes.length - 3],
                strAsBytes[strAsBytes.length - 2],
                strAsBytes[strAsBytes.length - 1]
            )
        );
        return last4Bytes == tokenIDStringTemplate;
    }

    function _replaceTokenIDTemplate(
        string memory baseURI,
        uint256 tokenId
    ) internal pure returns (string memory replaced) {
        bytes memory strBytes = bytes(baseURI);
        bytes memory striped = new bytes(strBytes.length - 4);
        for (uint i = 0; i < strBytes.length - 4; i++) {
            striped[i] = strBytes[i];
        }
        replaced = string(
            abi.encodePacked(striped, bytes("/"), LANFTUtils.toString(tokenId))
        );
    }
    ...
    function tokenURI(
        ...
        ERC721State.ERC721LAState storage state = ERC721State
            ._getERC721LAState();
        (uint256 editionId, ) = parseEditionFromTokenId(tokenId);
        string memory baseURI = state._baseURIByEdition[editionId];
        if (_baseURIContainsTokenID(baseURI)) {
            return _replaceTokenIDTemplate(baseURI, tokenId);
        }
    ...
    function _isWinterWallet() internal view returns (bool) {
        address[2] memory winterMintingWallets = [
            0xd541da4C37e268b9eC4d7D541Df19AdCf564c6A9,
            0xe0CB05cBf3dBeb647394905848d9361Daa99dE28
        ];
        return
            msg.sender == winterMintingWallets[0] ||
            msg.sender == winterMintingWallets[1];
    }
