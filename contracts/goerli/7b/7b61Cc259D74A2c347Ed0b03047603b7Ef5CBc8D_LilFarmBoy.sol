...
contract LilFarmBoy is ILilFarmBoy, ERC721AUpgradeable, OwnableUpgradeable, DefaultOperatorFiltererUpgradeable {
   ...
   modifier mintMod(uint256 _qty, bytes32[] memory _proof) {
      ...
      require((lilFarmBoy.totalSupply + _qty) <= lilFarmBoy.supply, 'LFB :: Not enough supply!');
   ...
   modifier farmerMintMod(bytes32[] memory _proof) {
      bytes32 sender = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProofUpgradeable.verify(_proof, mintPhaseData[currMintPhase].merkleRoot, sender), "LFB :: You're not in the list!");
   ...
   function mint(uint256 _qty, bytes32[] memory _proof) external payable mintMod(_qty, _proof) {
      payable(lilFarmBoy.treasury).transfer(msg.value);
      _mint(msg.sender, _qty);
      emit mintLilFarmBoy(msg.sender, _qty);
   }

   function farmerMint(bytes32[] memory _proof) external farmerMintMod(_proof) {
      uint256 qty = userData[msg.sender].farmerPhaseAllocation;
