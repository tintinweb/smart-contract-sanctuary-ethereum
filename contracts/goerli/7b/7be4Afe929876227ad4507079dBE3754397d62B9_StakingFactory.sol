...
contract StakingFactory is Ownable, IStakingFactory {
    ...
    function getPlasmaPerBlock() external view override returns (uint256) {
        ...
        if (_pool.ufoPool) {
            ...
            return ((plasmaRewardsForUfoPools - claimedPlasmaRewardsForUfoPools) * WEIGHT_SCALE) / totalUfoWeight;
        } else {
            if (totalLpWeight == 0) {
                // to avoid division overflow when tvl is 0
                return 0;
            ...
            return ((plasmaRewardsForLpPools - claimedPlasmaRewardsForLpPools) * WEIGHT_SCALE) / totalLpWeight;
        }
    }
