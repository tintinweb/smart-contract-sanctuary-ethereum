...
contract Unbounded1155V4 is ERC1155, IERC1155Receiver, IERC2981 {
  ...
  /*==================================================== Events =============================================================*/
  event TemplateCreated(uint256 baseId, uint256 amount, uint256 minId, uint256 maxId, string uri);
  ...
  event NFTMinted(uint256 baseId, uint256 count, address to);
  ...
  */
  ...
  function mint(address _to, uint256 _baseId) external onlyAuthorizedContract {
    ...
    unchecked {
      ...
      emit NFTMinted(_baseId, mintId, _to);
    }
  }
    ...
    for (uint256 i; i < _baseIds.length; i++) {
      ...
      for (uint256 j; j < _amounts[i]; j++) {
        unchecked {
          ...
          _mint(_to, mintId, 1, "");
          _item.mintCount++;
          emit NFTMinted(_baseIds[i], mintId, _to);
        }
      }
      unchecked {
    ...
    for (uint256 i; i < _baseIds.length; i++) {
      ...
      for (uint256 j; j < _amounts[i]; j++) {
        unchecked {
          ...
          _item.mintCount++;
          emit NFTMinted(_baseIds[i], mintId, _to);
          emit MetaverseSlotAdded(mintId, _metaverseId, _metaverseMetadataTemplate);
        }
      }
      unchecked {
        templateInfo[_baseIds[i]].mintCount += _amounts[i];
  ...
  function createTemplate(uint256 _amount, string calldata _uri) external onlyAdmin {
    ...
    emit TemplateCreated(baseIDCount, _amount, _minId, _maxId, _uri);
  }
   ...
   */
  function createBatchTemplate(uint256[] calldata _amounts, string[] calldata _uris) external onlyAdmin {
    ...
    for (uint256 i = 0; i < _amounts.length; i++) {
      ...
      emit TemplateCreated(baseIDCount, _amounts[i], _minId, _maxId, _uris[i]);
    }
