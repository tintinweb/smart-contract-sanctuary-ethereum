...
import "./interfaces/ICeresVault.sol";
import "./interfaces/ICeresBank.sol";
...
contract CeresBank is CeresBaseUpgradeable, ICeresBank, UUPSUpgradeable {
    ...
    uint256 public override anchorPrice;
    ...
    address public override currentRedeemCol;

    ICeresCoin public asc;
    ICeresCoin public crs;

    function initialize(address _factory, address _asc, address _crs, uint256 _mintCoolDown, uint256 _bonus) public initializer {

        CeresBaseUpgradeable.__CeresBase_init(_factory);
        configs[address(0)] = CollateralConfig(1e6, 1e6, 0.5e6, 1500, 0);
        anchorPrice = 1e6;
    ...
    function mint(address collateral, uint256 amount) external override onlyCeresMiner returns (MintResult memory result){
        ...
        require(block.timestamp > configs[collateral].nextMintTime, "CeresBank: Please wait for mint cooldown!");
        ...
        // mint asc
        asc.mint(factory.governorTimelock(), result.ascToGovernor);
        asc.mint(factory.ceresMiner(), result.ascTotal - result.ascToGovernor);
        ...
        // update next mint time
        configs[collateral].nextMintTime = block.timestamp + mintCooldown;
    }
    ...
    function _calcMintResult(address collateral, uint256 colAmount, uint256 collateralRatio, uint256 volatileRatio, bool isVol) internal view returns (MintResult memory) {
        ...
        return MintResult(ascTotal, ascToGovernor, ascToCol, ascToCrs, colAmount, crsAmount, minerCRSBonus);
    }
    ...
    function redeem(address collateral, uint256 ascAmount) external override onlyOwnerOrGovernor {
        ...
        address veCrs = factory.getStaking(address(crs));
        ...
        // make the transfer
        ICeresCoin(veCrs).mint(stakingASC, veCrsAmount);
    ...
    function setCurrentRedeemCol(address _currentRedeemCol) external onlyOwnerOrGovernor {
        currentRedeemCol = _currentRedeemCol;
    }
...
interface ICeresBank {
    ...
    function currentRedeemCol() external view returns(address);
...
interface ICeresVault {
    ...
    /* ---------- Functions ---------- */
    function withdraw(address to, address[] calldata tokens, uint256[] calldata amounts) external;
}
