...
contract CosmeticRegistry is Ownable {
   ...
   */
  address[] public cosmeticContracts;
  ...
  /**
   * @notice function returning all of the cosmetics owned by an address.
   * 
   * @dev used on front-end.
   */
  function getOwnedCosmetics(address _address) public view returns (uint256[] memory) {
      uint256[] memory ownedCosmetics;
      for (uint256 i = 0; i < cosmeticContracts.length; i++) {
        uint256 cosmeticId = i + 1;
        ICosmeticERC721A cosmetic = ICosmeticERC721A(cosmeticContractOf[cosmeticId]);
        if (cosmetic.balanceOf(_address) > 0) {
          ownedCosmetics[i] = cosmeticId;
        }
      }
  ...
  /**
   * @notice function returning all of the cosmetics an address is eligible for.
   * 
   * @dev used on front-end.
   */
  function getEligibleCosmetics(address _address) public returns (uint256[] memory) {
      uint256[] memory eligibleCosmetics = new uint256[](cosmeticContracts.length);

      for (uint256 i = 0; i < cosmeticContracts.length; i++) {
        uint256 cosmeticId = i + 1;
        ICosmeticERC721A cosmetic = ICosmeticERC721A(cosmeticContractOf[cosmeticId]);
        if (cosmetic.isEligible(_address)) {
          eligibleCosmetics[i] = cosmeticId;
        } else {
          eligibleCosmetics[i] = 0;
        }
      }

      return eligibleCosmetics;
  }
