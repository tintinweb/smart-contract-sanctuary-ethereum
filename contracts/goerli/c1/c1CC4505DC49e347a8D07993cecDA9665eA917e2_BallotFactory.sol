...
contract Ballot is AccessControl {
    ...
    constructor(
        ...
        uint256 _dateToStart,
        uint256 _dateToEnd,
        uint8 _options,
        uint256 _ballotCount,
        ...
        title = _title;
        dateToStart = _dateToStart;
        dateToEnd = _dateToEnd;
        options = _options;
        ...
        Token(token).addTeamMember(address(this));
    }

    function vote(uint8 option, uint256 amount) public isBallotValid(true) {
        ...
        tokensUsed[msg.sender] += amount;
        optionVotes[option] += amount;

        Token(token).addTokensUsedBallot(msg.sender, amount);
    ...
    function voteRemaining(uint8 option) public isBallotValid(true) {
...
contract Token is ERC20, AccessControl {
    ...
    function _beforeTokenTransfer(
        ...
        if (
            ...
            require(
                tokensUsedBallot[from] <= balanceOf(from),
                "Token: No tokens left"
            );
        }
