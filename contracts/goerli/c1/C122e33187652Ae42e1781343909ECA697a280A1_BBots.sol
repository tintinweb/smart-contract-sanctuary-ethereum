...
contract BBots is ERC721Royalty, AccessControl, Ownable {
    ...
    struct RobotType {
        ...
        uint256 frame;
        uint256 background;
        uint256 sepcialEffects;
    }

    bytes32 constant ROLE_MINTER = keccak256("MINTER");

    mapping(uint256 => RobotType) tokenDesc;
    ...
    function mintFor(
        ...
        /// @dev mintingBlob consists of bytes tokenId,botype,robot,frame,background,sepcialeffects
        uint256[] memory _mintingDesc = Parse.parseBlob(mintingBlob);

        uint256 tokenId = _mintingDesc[0];

        super._safeMint(to, tokenId);

        RobotType memory _robotType = RobotType(
            _mintingDesc[1],
            _mintingDesc[2],
            _mintingDesc[3],
            _mintingDesc[4],
...
library Parse {
    ...
    function parseBlob(bytes calldata _blob) internal pure returns (uint256[] memory) {
        string memory blob = string(_blob);
        int256 index = Bytes._indexOf(blob, ",", 0);
        require(index >= 0, "Separator must exist");
        uint256[] memory mintingBlob = Bytes.split(blob, ",");
        return mintingBlob;
    }
}
...
library Bytes {
     ...
     */
    function split(string memory _base, string memory _value) internal pure returns (uint256[] memory splitArr) {
        bytes memory _baseBytes = bytes(_base);
        ...
        splitArr = new uint256[](_splitsCount);
        ...
        while (_offset < _baseBytes.length) {
            ...
            _offset = uint256(_limit) + 1;
            splitArr[_splitsCount++] = toUint(_tmpBytes);
        }
        return splitArr;
    }
