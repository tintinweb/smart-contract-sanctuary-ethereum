...
contract Otherdeed is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public userPublicMintedAmount;

    // constants
    uint256 public immutable MAX_LANDS;
    uint256 public immutable MAX_ALPHA_NFT_AMOUNT;
    uint256 public immutable MAX_BETA_NFT_AMOUNT;
    ...
    constructor(
        ...
        uint256 _alphaLandAmount,
        uint256 _betaLandAmount
    ) ERC721(name, symbol) {
        alphaContract = _alphaContractAddress;
        betaContract = _betaContractAddress;

        MAX_ALPHA_NFT_AMOUNT = _alphaLandAmount;
        MAX_BETA_NFT_AMOUNT = _betaLandAmount;

        betaNftIdCurrent = _alphaLandAmount; //beta starts after alpha
        mintIndexPublicSale = _alphaLandAmount + _betaLandAmount; //public sale starts after beta

        //MAX_LANDS = _alphaLandAmount + _betaLandAmount;
        MAX_LANDS = 100000;
    }
    ...
    ) external whenClaimableActive nonReentrant {
        require(
            alphaTokenIds.length > 0 || betaTokenIds.length > 0,
            "Should claim at least one land"
        );
    ...
    function alphaClaimLand(uint256[] calldata alphaTokenIds) private {
        for (uint256 i; i < alphaTokenIds.length; ++i) {
            ...
            require(
                ERC721(alphaContract).ownerOf(alphaTokenId) == msg.sender,
                "Must own all of the alpha defined by alphaTokenIds"
            );
    ...
    function betaClaimLand(uint256[] calldata betaTokenIds) private {
        for (uint256 i; i < betaTokenIds.length; ++i) {
            ...
            require(
                ERC721(betaContract).ownerOf(betaTokenId) == msg.sender,
                "Must own all of the beta defined by betaTokenIds"
            );
    ...
    function mintLands(uint256[] calldata landIds) public nonReentrant {
        require(
            totalSupply() + landIds.length <= MAX_LANDS,
            "total supply exceed max lands"
        );

        require(
            userPublicMintedAmount[msg.sender] + landIds.length <= 10,
            "exceed user mint limit"
        );

        userPublicMintedAmount[msg.sender] += landIds.length;

        mintLandsCommon(landIds, msg.sender);
    }

    function mintByAdmin(uint256[] calldata landIds, address recipient)
        public
        nonReentrant
        onlyOwner
    {
        ...
        mintLandsCommon(landIds, recipient);
    }

    function mintLandsCommon(uint256[] calldata landIds, address recipient)
        private
    {
        for (uint256 i; i < landIds.length; ++i) {
            require(landIds[i] >= mintIndexPublicSale, "invalid land id");
            _safeMint(recipient, landIds[i]);
        }
    }
}
