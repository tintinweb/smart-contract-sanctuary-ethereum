...
contract NyxRedeemProposal is Ownable, converter {
    string public constant name = "Nyx Proposal - Redeem";
    uint256 public constant nparams = 4;

    // Enums
    ////////////////////
    enum RedeemAction{START, STOP}
    ...
    function parseProposalParams(bytes[] memory params)
        public pure
        returns (RedeemProposalParams memory)
    {
        require(params.length == nparams, "wrong format");

        return RedeemProposalParams(RedeemAction(converter.bytesToUint(params[0])),
        ...
        returns (RedeemProposalParams memory)
    {
        ...
        return RedeemProposalParams(RedeemAction(converter.bytesToUint(param1)),
        ...
        returns (RedeemProposalParams memory)
    {
        ...
        {
            (uint256 param1, uint256 param2, uint256 param3, string memory param4) = abi.decode(params, (uint256, uint256, uint256, string));
            return RedeemProposalParams(RedeemAction(param1),
                param2,
                param3,
                param4);
        }
        ...
        {
            RedeemProposalParams memory nullProposal;
            return nullProposal;
        }
    }

    function proposalToBytes(RedeemProposal memory proposal)
        public pure
        returns (bytes memory)
    {
        return abi.encode(proposal.action, proposal.nNfts,
            proposal.redeemPrice, proposal.reason);
    }
    ...
    function createProposal(uint256 proposalId, bytes[] memory params)
        ...
        returns (bytes memory)
    {
        RedeemProposalParams memory parsedParams = parseProposalParams(params);
        RedeemProposal memory proposal = RedeemProposal(proposalId, parsedParams.action, parsedParams.nNfts,
            parsedParams.redeemPrice, parsedParams.reason);
    ...
    function getProposal(bytes memory proposalBytes)
        ...
        returns (bytes[] memory)
    {
        RedeemProposalParams memory params = parseProposalParams2(proposalBytes);
        bytes[] memory prop = new bytes[](nparams);
        prop[0] = abi.encode(params.action);
        prop[1] = abi.encode(params.nNfts);
        prop[2] = abi.encode(params.redeemPrice);
        prop[3] = abi.encode(params.reason);
        return prop;
    }
        ...
        returns (bytes[nparams][] memory props)
    {
        ...
        {
            RedeemProposalParams memory params = parseProposalParams2(proposalBytesArr[idx]);
            props[idx] = [abi.encode(params.action), abi.encode(params.nNfts), abi.encode(params.redeemPrice), abi.encode(params.reason)];
        }
    }
}
