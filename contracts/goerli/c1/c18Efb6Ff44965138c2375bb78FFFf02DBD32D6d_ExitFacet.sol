...
contract ExitFacet is IOperations, ReentrancyGuard {
    ...
    function exitExpiredDeposit(
        ...
        LibStatusControl._requireExitEnable();
        require(
        ...
        require(_tokenIds.length == _amounts.length, "Exit:invalid input");
        ...
        for (uint256 i = 0; i < dataLength; i++) {
            ...
            LibWithdrawBalance._releaseAsset(msg.sender, _listingId, tokenIds[i], amounts[i], false);
        }
    ...
    function exitAsset(
        ...
        require(_tokenIds.length == _amounts.length, "Exit:invalid input");
        require(_tokenIds.length > 0 && _tokenIds.length < 21, "Exit:invalid length");
        LibStatusControl._requireExitEnable();
    ...
    /// @notice
    function getValidExpiredDepositRequest(uint256 _requestId) external view returns (bool) {
        return
            s.pendingDepositRecord[_requestId].expirationTimestamp < block.timestamp &&
            s.pendingDepositRecord[_requestId].expirationTimestamp != 0;
    }
...
library LibWithdrawBalance {
    ...
    function _releaseAsset(
        ...
        if (_listingId == 0) {
            ...
            uint8 assetType = s.tokenTypes[_listingId];
            if (assetType == 1) {
                require(
