...
/**
 * @dev Marketplace contract definition
 ...
 * IERC1155ReceiverUpgradeable: Semi-fungible token receiver interface (IERC1155)
 */
    ...
    PlatformFeeUpgradeable
{
    ...
    modifier onlyListingCreator(uint256 _listingId) {
        _isListingCreator(_listingId);
        _;
    }

    /// @dev Checks whether a listing exists.
    modifier onlyExistingListing(uint256 _listingId) {
        _isListingExists(_listingId);
        _;
    }
    ...
    function initialize(
        ...
        __PlatformFee_init_unchained(_platformFeeRecipient, uint128(_platformFeeBps));
    ...
    function createListing(ListingParameters memory _params) external override {
        ...
        TokenType tokenType = getTokenType(_params.assetContract);
        uint256 safeQuantity = getSafeQuantity(tokenType, _params.quantityToList);

        require(safeQuantity != 0, "QUANTITY");
        ...
        validateOwnershipAndApproval(tokenType, tokenOwner, _params.assetContract, _params.tokenId, safeQuantity);
    ...
    function updateListing(
        ...
        if (targetListing.quantity != safeNewQuantity) {
            ...
            if (safeNewQuantity > targetListing.quantity) {
                uint256 newAddedQuantity = safeNewQuantity - targetListing.quantity;
            ...
            if (targetListing.quantity > safeNewQuantity && isAuction) {
                transferListingTokens(
                    targetListing.tokenType,
                    targetListing.assetContract,
                    address(this),
    ...
    function offer(
        ...
        require(targetListing.endTime > block.timestamp && targetListing.startTime < block.timestamp, "!PERIOD");
        ...
        if (targetListing.listingType == ListingType.Auction) {
            ...
            require(newOffer.currency == targetListing.currency, "!CURRENCY");
    ...
    function handleOffer(Listing memory _targetListing, Offer memory _newOffer) internal {
        require(_targetListing.quantity != 0 && _newOffer.quantityWanted <= _targetListing.quantity, "QUANTITY");
    ...
    /// @dev Lets an account close an auction for eit her the (1) winning bidder, or (2) auction creator.
    function closeAuction(uint256 _listingId, address _closeFor) external override nonReentrant {
        ...
        if (toCancel) {
            ...
            require(targetListing.endTime < block.timestamp, "<END_TIME");
    ...
    function _cancelAuction(Listing memory _targetListing) internal {
        require(listings[_targetListing.listingId].tokenOwner == _msgSender(), "!LISTER");
    ...
    function setAuctionBuffers(uint256 _timeBuffer, uint256 _bidBufferBps) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_bidBufferBps < MAX_BPS, "!BPS");

        timeBuffer = uint64(_timeBuffer);
        bidBufferBps = uint16(_bidBufferBps);
    ...
    function payout(
        ...
        try IERC2981Upgradeable(_listing.assetContract).royaltyInfo(_listing.tokenId, _totalPayoutAmount) returns (
            ...
            if (royaltyFeeRecipient != address(0) && royaltyFeeAmount > 0) {
                require(royaltyFeeAmount + platformFeeCut <= _totalPayoutAmount, "<FEE");
    ...
    function validateDirectListingSale(
        ...
        require(_listing.listingType == ListingType.Direct, "!DIRECT");

        // Check whether a valid quantity of listed tokens is being bought.
        require(_listing.quantity > 0 && _quantityToBuy > 0 && _quantityToBuy <= _listing.quantity, "!QUANTITY");

        // Check if sale is made within the listing window.
        require(block.timestamp < _listing.endTime && block.timestamp > _listing.startTime, "!READY");

        // Check: buyer owns and has approved sufficient currency for sale.
        if (_currency == CurrencyTransferLib.NATIVE_TOKEN) {
            require(msg.value == settledTotalPrice, "!DEPOSIT");
        } else {
