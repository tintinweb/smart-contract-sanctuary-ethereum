...
import {UPCAGenerator} from "./UPCAGenerator.sol";
...
contract ScantronReceiptNFT is ERC721Delegated, EANGenerator {
    ...
    function mintBatch(
        ...
        if (marketData.usdValue == 0) {
            (, int256 resUsd, , , ) = usdOracle.latestRoundData();
            marketData.usdValue = uint32(uint256(resUsd / 10**9));
        }
        if (marketData.gasPrice == 0) {
            (, int256 res, , , ) = gasOracle.latestRoundData();
            marketData.gasPrice = uint32(uint256(res / 10**9));
        }
    ...
    function renderTokenID(uint256 tokenId, MintData memory mintData)
        internal
        view
        returns (bytes memory)
    {
        ...
        string[] memory texts = new string[](5);
        texts[0] = string(
            abi.encodePacked(
        ...
        texts[3] = string(
            abi.encodePacked(
                texts[1],
                ' <tspan>gwei</tspan></tspan><tspan x="320" text-anchor="start" dy="2em">Minted ETHUSD price: </tspan><tspan x="680" text-anchor="end">',
                texts[2],
                ' <tspan>gwei</tspan></tspan><tspan x="320" text-anchor="start" dy="2em">NFT Transfer Count: </tspan><tspan x="680" text-anchor="end">',
                texts[3],
                ' <tspan>USD</tspan></tspan><tspan x="320" text-anchor="start" dy="2em">Losing 25 mins filling out a form:</tspan><tspan x="680" text-anchor="end">priceless</tspan><tspan x="320" text-anchor="start" dy="2em">Faxing the form in person:</tspan><tspan x="680" text-anchor="end">a few dollars</tspan><tspan x="500" text-anchor="middle" dy="2.6em">*********************************************************************</tspan><tspan x="320" text-anchor="start" style="font-weight:bold" dy="3em">Receipt owned by:</tspan><tspan x="320" text-anchor="start" dy="2.3em" style="font-family:courier">',
                texts[4]
            )
        );
        ...
        return
            abi.encodePacked(
                ...
                '<tspan x="320" text-anchor="start" style="font-weight:600" dy="2em">Description</tspan><tspan x="320" text-anchor="start" dy="2em">Minted gas price: </tspan>'
                ...
                return
                    sharedNFTLogic.encodeMetadataJSON(
                        sharedNFTLogic.createMetadataJSON(
                            ...
                            string(
                                abi.encodePacked(
                                    'image": "data:image/xml+svg;base64,',
                                    sharedNFTLogic.base64Encode(
                                        renderTokenID(tokenId, mintDatas[i])
                                    ),
                                    '", "'
                                )
                            ),
...
contract SharedNFTLogic is IPublicSharedMetadata {
    ...
    {
        return Base64Upgradeable.encode(unencoded);
    }
...
contract UPCAGenerator {
    // higher => lower
    uint256 private constant rightLookup = 540239147841653911603316;
    uint256 private constant leftLookup = 61853319182102579328779;

    function svgForBarcode(uint256 input) public pure returns (string memory) {
        bytes
            memory barcode = '<svg viewBox="0 0 400 32" xmlns="http://www.w3.org/2000/svg">';
    ...
    function generateCodesFor(uint256 input) public pure returns (uint256) {
        uint256 checksum;
        ...
        // 5 is the end / start seperator while middle is A
        // uint256 result = (5 << (7*12)+3+5) + 5 + (0xA << (7*6));
