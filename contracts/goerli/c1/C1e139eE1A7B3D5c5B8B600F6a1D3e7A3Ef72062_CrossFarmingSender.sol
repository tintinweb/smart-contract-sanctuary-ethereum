...
contract CrossFarmingSender is MessageSenderApp, MessageReceiverApp {
    ...
    // gasFeeFloatRate is a range rate in [0.1,10], for converting gas fee from dest chain to source chain.
    // for example, source chain gas price is 1/2 dest chain gas price, so the user paid txfee in sourcechain
    // should base on 2x gas fee in destchain.
    ...
    mapping(DataTypes.MessageTypes => uint256) public gaslimits;
    ...
    event GasLimitUpdated(DataTypes.MessageTypes msgtype, uint256 gaslimit);
    ...
    function sendFarmMessage(bytes calldata _message) external payable onlyVault {
        ...
        // withdraw/emergencywithdraw ack message fee
        if (request.msgType >= DataTypes.MessageTypes.Withdraw) {
            totalFee += (msgBusFee * exchangeRate) / EXCHANGE_RATE_PRECISION;
        }
    ...
    function setGaslimits(DataTypes.MessageTypes _type, uint256 _gaslimit) external onlyOwner {
    ...
    function estimateDestGaslimit(address _account, DataTypes.MessageTypes _msgType)
        public
        view
        returns (uint256 gaslimit)
    {
        ...
        if (!is1st[_account]) gaslimit += createProxyGasLimit;
    }
    ...
    /// @dev unless stop running current contract, don't call it.
