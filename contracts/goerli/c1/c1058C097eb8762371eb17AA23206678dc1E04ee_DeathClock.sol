...
contract DeathClock is EIP712, ERC721r, Whitelist, Ownable, ReentrancyGuard {
    ...
    //TODO private
    IDeathClockDescriptor public _descriptor;
    string private constant SIGNING_DOMAIN = 'DeathVoucher';
    string private constant SIGNATURE_VERSION = '1';
    uint256 private TRANSFERABLE_CLOCKS;
    address private _deathWishSigner;
    Counters.Counter private _soulboundTokenIds;

    // Minting Stuff
    bool public publicCanMint;
    uint256 private constant MINT_PRICE = 0.0004321 ether;

    mapping(uint256 => uint256) public expDates;
    mapping(uint256 => uint256) public mintDates;
    mapping(uint256 => uint256) public updateDates;
    mapping(uint256 => uint256) public remnants;
    mapping(uint256 => uint256) public resets;
    mapping(uint256 => address) public fds;
    ...
    constructor(
        address deathWishSigner,
        address _descriptorAddress,
        uint256 transferableClocks
    )
        ERC721r('Death Clock', 'DEATH', transferableClocks)
        EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION)
    {
        _deathWishSigner = deathWishSigner;
        TRANSFERABLE_CLOCKS = transferableClocks;
        _descriptor = IDeathClockDescriptor(_descriptorAddress);
    }
    ...
    /// @notice Redeems a DeathWish for an actual NFT, creating it in the process.
    ...
    /// @notice Redeems a DeathWish for a Death Clock NFT only if minter is on active whitelist.
        ...
        public
    {
        ...
        updateDates[deathWish.tokenId] = block.timestamp;
        resets[deathWish.tokenId] += 1;
        fds[deathWish.tokenId] = _msgSender();
    }
        ...
        returns (uint256)
    {
        ...
        _safeMint(to, soulboundTokenId);
        _soulboundTokenIds.increment();
        remnants[deathClockTokenId] += 1;
        fds[soulboundTokenId] = _msgSender();
    ...
    function _mintDeathClock(DeathWish calldata deathWish, bytes memory signature) internal {
        ...
        fds[tokenId] = _msgSender();
        expDates[tokenId] = deathWish.expDate;
        mintDates[tokenId] = block.timestamp * 1000;
        updateDates[tokenId] = block.timestamp * 1000;
    }
    ...
    function _afterTokenTransfer(
        ...
        if (from != address(0)) {
            ...
            if (!_isRemnant(tokenId) && !remnantExists[from][tokenId]) {
                ...
                remnantExists[from][tokenId] = true;
            }
        }
    }

    /// @dev Clocks with tokenId >= TRANSFERABLE_CLOCKS are soulbound duplicates known as 'remnants'
    function _isRemnant(uint256 tokenId) internal view returns (bool) {
...
interface IDeathClockDescriptor {
    ...
    // function setColorNames(bytes32[] memory _colorNames) external;

    // function setImageNames(bytes32[] memory _imageNames) external;

    function setPreviewUrls(string[] memory _previewUrls) external;

    function setTokenParams(TokenParams[] memory _tokenParams) external;
