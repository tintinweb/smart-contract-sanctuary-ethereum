...
contract AppStore {
    ...
    struct AppProperty {
        ...
        address[] buyers;
        mapping(address => bool) isBuyersMap;
    }
    ...
    /**
     * devAccount (address) => soldTokenIds (uint256[])
     */
    mapping (address => uint256[]) sellerMap;

    /**
     * sellerAccount (address) => boughtTokenIds (uint256[])
     */
    ...
    function sell(string memory name, string memory tokenURI, uint price) public returns (uint256)  {
        ...
        sellerMap[msg.sender].push(ntfToken);
    ...
    function buy(uint256 tokenId) public payable returns (bool)  {
        ...
        require(appMap[tokenId].isBuyersMap[msg.sender] != true, "Error: This user has already purchased");

        // transform eth to seller
        appMap[tokenId].seller.transfer(msg.value);

        // add buyers to buyApp.buyers 
        appMap[tokenId].isBuyersMap[msg.sender] = true;
        appMap[tokenId].buyers.push(msg.sender);

        // add tokenId to buyerMap
        buyerMap[msg.sender].push(tokenId);
     ...
     *
     */
    function verify(uint256 tokenId, address buyer) public view returns (bool) {
        require(appMap[tokenId].seller != address(0), "Error: There is no app pointed to by this tokenId");

        bool verified = appMap[tokenId].isBuyersMap[buyer] == true;

        return verified;
    }

    function getTokenIdsBySeller(address seller) public view returns(uint256[] memory tokenIds){
        return sellerMap[seller];
    }

    function getTokenIdsByBuyer(address buyer) public view returns(uint256[] memory tokenIds){
        return buyerMap[buyer];
    }
...
contract AppNFT is ERC721URIStorage {
    ...
    function add(address sender, string memory tokenURI, string memory name) public returns (uint256) {
        ...
        for (uint i = 0; i < newItemId; i++) {
            if (compareString(_appNameMap[i], name)) {
                hasSameName = true;
                break;
            }
            if (compareString(this.tokenURI(i), tokenURI)) {
