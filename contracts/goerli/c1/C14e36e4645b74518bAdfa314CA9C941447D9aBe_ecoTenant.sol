...
contract ecoTenant {
   ...
   struct EcoTenantData {
    ...
    function createNewTenant(
        ...
        //Verifies that a tenant with the auth0 id was not created before
    ...
    //Gets the tenant data with the idAuth0 provided
        ...
        returns (EcoTenantData memory)
    {
        ...
        if(!found){
            ...
            return _tenants[position];
        }
    }
    //Gets the tenant with the autoincrement id
    ...
    //Search if the idAuth0 already exists in the contract 
    ...
    //Gets the number of tenants registered in the contract
