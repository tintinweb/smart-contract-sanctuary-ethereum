    ...
    IPool
{
    ...
    function setNewSettingsByOwner(
        ...
        require(
            ITGE(getGovernanceToken().lastTGE()).state() != ITGE.State.Active,
            ExceptionsLibrary.ACTIVE_TGE_EXISTS
        );

        _setGovernanceSettings(governanceSettings_);

        if (addSecretary.length > 0) {
            changePoolSecretary(addSecretary, removeSecretary);
        }
        if (addExecutor.length > 0) {
            changePoolExecutor(addExecutor, removeExecutor);
        }
    }
    ...
    function isValidExecutor(address account) public view returns (bool) {
        //owner can execute ballot if poolExecutor list is empty
        if (poolExecutor.length() == 0 && account == owner()) return true;

        if (isPoolExecutor(account)) return true;
