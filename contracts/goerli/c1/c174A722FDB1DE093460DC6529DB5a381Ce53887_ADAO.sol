...
contract ADAO is ADTradeToken {
     ...
     */
    constructor() public BEP20('AirdropDao', 'ADAO') {}
...
abstract contract ADTradeToken is ListenTradeToken {
    using SafeMath for uint256;
    ...
    IADTradeDividendDipatcherInterface public TradeDispatcher;
    ...
    function changeDispatcher(address dispatcher) external onlyOwner {
        TradeDispatcher = IADTradeDividendDipatcherInterface(dispatcher);
        whiteList[address(TradeDispatcher)] = true;

        address swaper = TradeDispatcher.getSwapHelerAddress();
        whiteList[swaper] = true;
    }

    function changeOpenListen(bool value) external onlyOwner {
        openHook = value;
    }
        ...
        if (openHook) {
            if (whiteList[sender] || whiteList[recipient]) {
                ...
                if (hookAddress[recipient]) {
                    //sell
                    uint256 dispatchPercent = TradeDispatcher
                        .getSellDividendPercents();
                    uint256 dispatchAmount = amount.mul(dispatchPercent).div(
                        100
                    );
                    ...
                    super._transfer(
                        sender,
                        recipient,
                        amount.sub(dispatchAmount)
                    );
                    super._transfer(
                        sender,
                        address(TradeDispatcher),
                        dispatchAmount
                    );
                    TradeDispatcher.dispatchSellDividend(sender, amount);
                } else if (hookAddress[sender]) {
                    //buy
                    uint256 dispatchPercent = TradeDispatcher
                        .getBuyDividendPercents();
                    uint256 dispatchAmount = amount.mul(dispatchPercent).div(
                        100
                    );
                    ...
                    TradeDispatcher.dispatchBuyDividend(recipient, amount);
                } else {
                    super._transfer(sender, recipient, amount);
                }
            }
    ...
    function _doAddHookAddress(address address_) internal {
        hookAddress[address_] = true;
    }

    function addHookAddress(address address_) public onlyOwner {
        _doAddHookAddress(address_);
    }
}
...
interface IADTradeDividendDipatcherInterface {
    ...
    function getSwapHelerAddress() external view returns (address);
}