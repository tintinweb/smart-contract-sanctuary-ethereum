...
contract Echoes is HasConfigStore, ERC1155Supply {
        ...
        private
    {
        ...
        chapterEchoes[genesisTextId][chapterId][nextEchoId] =
            _getContestFactory().deployNewContest(minimumContestTime, approvedEntryThreshold, configStore);
        emit CreatedEchoContest(genesisTextId, chapterId, nextEchoId, minimumContestTime, approvedEntryThreshold);
        ...
        internal
    {
        ...
        if (!initialEchoContestsHaveWinners(genesisTextId, chapterId)) revert InitialEchoContestsDoNotHaveWinners();
    ...
    function initialEchoContestsClosed(uint256 genesisTextId, uint256 chapterId) public view returns (bool) {
        for (uint256 i = 0; i < GlobalConstants.INITIAL_ECHO_COUNT; i++) {
    ...
    function initialEchoContestsHaveWinners(uint256 genesisTextId, uint256 chapterId) public view returns (bool) {
        for (uint256 i = 0; i < GlobalConstants.INITIAL_ECHO_COUNT; i++) {
            if (
...
contract GenesisText is Echoes {
    ...
    event CreatedNextChapterContest(uint256 indexed genesisTextId, uint256 indexed chapterId);
    ...
    function _createNextChapterContest(uint256 genesisTextId) internal {
        ...
        if (nextChapterId[genesisTextId] == 0) {
            ...
            if (!initialEchoContestsHaveWinners(genesisTextId, 0)) revert GenesisEchoContestsDoNotHaveWinners();
        }
        ...
        // has set a winner.
            ...
            if (!initialEchoContestsHaveWinners(genesisTextId, nextChapterId[genesisTextId] - 1)) {
                revert PreviousEchoContestsDoNotHaveWinners();
            }
        }
        chapterContests[genesisTextId][nextChapterId[genesisTextId]] = _getContestFactory().deployNewContest(
            GlobalConstants.DEFAULT_CONTEST_MINIMUM_TIME,
            GlobalConstants.DEFAULT_CONTEST_MINIMUM_APPROVED_ENTRIES,
            configStore
        );
        emit CreatedNextChapterContest(genesisTextId, nextChapterId[genesisTextId]);
        nextChapterId[genesisTextId]++;
    }
    ...
    function _createNextEchoContests(uint256 genesisTextId) internal {
        ...
        if (nextEchoChapterId[genesisTextId] == 0) {
            ...
            emit CreatedNextEchoContest(genesisTextId, nextEchoChapterId[genesisTextId]);
        } else {
            // The chapter contest must have set a winner before we can open its echoes.
            if (!_hasCreatedChapterContest(genesisTextId, _latestEchoId(genesisTextId))) {
                revert ChapterContestNotCreated();
            }
            if (!IContest(chapterContests[genesisTextId][_latestEchoId(genesisTextId)]).hasWinner()) {
            ...
            _createInitialEchoContests(
                genesisTextId,
                _latestEchoId(genesisTextId) + 1,
                GlobalConstants.DEFAULT_CONTEST_MINIMUM_TIME,
                GlobalConstants.DEFAULT_CONTEST_MINIMUM_APPROVED_ENTRIES
            );
            emit CreatedNextEchoContest(genesisTextId, _latestEchoId(genesisTextId) + 1);
        }
        nextEchoChapterId[genesisTextId]++;
    }
    ...
    function _hasCreatedChapterContest(uint256 genesisTextId, uint256 chapterId) public view returns (bool) {
...
contract Spire is GenesisText, Ownable, ReentrancyGuard, Multicall {
    ...
    constructor(ConfigStore _configStore) GenesisText(_configStore, "GENESIS_TEXT_URI") {
        ...
        for (uint256 i = 0; i < GlobalConstants.GENESIS_TEXT_COUNT; i++) {
            ...
            genesisGovernors[i] =
                _getToggleGovernorFactory().deployNewToggleGovernor(IERC1155Supply(address(this)), nextContestWinnerId);
        ...
        nonReentrant
    {
        ...
        if (genesisTextId == currentEchoFlowId && currentEchoFlowId != previousEchoFlowId) {
            if (!latestEchoContestsHaveWinners(previousEchoFlowId)) revert MustSetPreviousEchoContestWinners();
        }
    ...
    function setChapterContestWinner(uint256 genesisTextId, uint256 winningId) external onlyOwner nonReentrant {
        ...
        if (genesisTextId == currentChapterFlowId && currentChapterFlowId != previousChapterFlowId) {
            if (!latestChapterContestHasWinner(previousChapterFlowId)) revert MustSetPreviousChapterContestWinner();
        }
        ...
        returns (bool canAdvance, uint256 newCurrentFlowId, uint256 newPreviousFlowId)
    {
        ...
        if (flow == Flow.Chapter) {
            previousContestHasWinner = currEqualsPrev || latestChapterContestHasWinner(prevFlowId);
        } else {
            previousContestHasWinner = currEqualsPrev || latestEchoContestsHaveWinners(prevFlowId);
        }
        bool latestContestsHaveClosed =
            flow == Flow.Chapter ? _canCloseLatestChapterContest(currFlowId) : latestEchoContestsClosed(currFlowId);
    ...
    function latestEchoContestsClosed(uint256 genesisTextId) public view returns (bool) {
        return initialEchoContestsClosed(genesisTextId, _latestEchoId(genesisTextId));
    }

    function latestEchoContestsHaveWinners(uint256 genesisTextId) public view returns (bool) {
        return initialEchoContestsHaveWinners(genesisTextId, _latestEchoId(genesisTextId));
    }

    function latestChapterContestHasWinner(uint256 genesisTextId) public view returns (bool) {
