...
contract GachaDraw is Ownable {
    ...
    function updateMaximumRewardSupply(
        ...
        if(_maxRewardSupply.length != activity.rewardTokenIds.length) {
            revert InvalidInputLength();
        }
        ...
        for(uint256 index = 0; index < _maxRewardSupply.length; index++) {
            ...
            totalSupplyInCirculation += activity.totalRewardSupply[index];
            newMaxRewardSupply = newMaxRewardSupply + _maxRewardSupply[index];

            activity.maximumRewardSupply[index] = _maxRewardSupply[index];
        }

        activity.remainingSupply = newMaxRewardSupply - totalSupplyInCirculation;
    }
        ...
        validActivity(_activityId)
    {
        ...
        uint256 remainingSupplyRunning = activity.remainingSupply;

        /// @dev For each tickets burns the tickets and mint a random reward
        for (uint256 i = 0; i < _ticketAmount; i = _uncheckedIncOne(i)) {
            uint256 randomIndex = getRandomNumber(remainingSupplyRunning);

            uint256 selectedTokenId;
            uint256 indexCount;

            /// @dev Find out the choosen reward time and increase it's supply
            for (uint256 j = 0; j < activity.rewardTokenIds.length; j = _uncheckedIncOne(j) ) {
                uint256 tokenAmountYetToBeMinted = activity.maximumRewardSupply[j] - activities[_activityId].totalRewardSupply[j];
                indexCount = _uncheckedIncDelta(indexCount, tokenAmountYetToBeMinted);
                if (tokenAmountYetToBeMinted > 0 && indexCount >= randomIndex) {
                    selectedTokenId = activity.rewardTokenIds[j];
                    activities[_activityId].totalRewardSupply[j] = _uncheckedIncOne(activities[_activityId].totalRewardSupply[j]);
                    break;
                }
            }
            ...
            unchecked {
                remainingSupplyRunning--;
                totalRewardWon++;    
            } 
            
        }

        activities[activity.activityId].remainingSupply = remainingSupplyRunning;
    ...
    /// @return incremented value
    function _uncheckedIncDelta(uint256 val, uint256 delta) internal pure returns (uint256) {
        unchecked {
            return val + delta;
        }
    }
