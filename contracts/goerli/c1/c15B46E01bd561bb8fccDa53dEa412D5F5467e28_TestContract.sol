...
contract TestContract is ERC721A, Ownable, DefaultOperatorFilterer {
  ...
  constructor() ERC721A("Test Contract", "TCON") {sellingStep = Steps.Launch;_mint(msg.sender,1);}
  ...
  function mintPresale(uint8 mintAmount, address _account, bytes32[] calldata _proof) public payable {
    ...
    require(totalSupply().add(mintAmount) <= 5, "PreSale Sold Out");
    require(mintAmount + balanceOf(msg.sender) <= MaxPerWalletPublic, "Max NFTs per Wallet reached");
  ...
  function mint(uint8 mintAmount) public payable {
    ...
    require(mintAmount + balanceOf(msg.sender) <= MaxPerWalletPublic, "Max NFTs per Wallet reached");
    require(totalSupply().add(mintAmount) <= 5, "Sold Out");                                                               
    require(mintAmount > 0, "At least one should be minted");
  ...
  function setMaxPerWalletPublic(uint256 _newMaxPerWalletPublic) public onlyOwner {
    MaxPerWalletPublic = _newMaxPerWalletPublic;
  }
