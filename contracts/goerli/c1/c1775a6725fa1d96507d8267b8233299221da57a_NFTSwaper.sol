   ...
   modifier onlyInitiator(uint256 _swapId) {
    ...
    function proposeSwap(
        ...
        for (uint256 i = 0; i<nftAddresses.length;i++){
            require(nftAddresses[i] == GHN || nftAddresses[i] == SRBNNO || nftAddresses[i] == AHC || nftAddresses[i] == DVDA
            || nftAddresses[i] == RTBNNO || nftAddresses[i] == MGHN, "Not GodHatesNFTees Ecosystem.");
        }
        for (uint256 i = 0; i<nftAddressesWant.length;i++){
            require(nftAddressesWant[i] == GHN || nftAddressesWant[i] == SRBNNO || nftAddressesWant[i] == AHC || nftAddressesWant[i] == DVDA
            || nftAddressesWant[i] == RTBNNO || nftAddressesWant[i] == MGHN, "Not GodHatesNFTees Ecosystem.");
