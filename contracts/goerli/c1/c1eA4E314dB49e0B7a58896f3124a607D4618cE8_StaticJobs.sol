...
import "./interfaces/IStaticJobs.sol";
    ...
    UUPSUpgradeable
{
    ...
    function registerJob(bytes[] calldata _jobInfo,address[] calldata _targetAddresses,string memory _name,string[] memory _ipfsForJobDetails) external{
        ...
        emit JobRegistered(msg.sender,_jobInfo,_targetAddresses,jobHash,totalJobs,_name,_ipfsForJobDetails);
        }
    ...
    function executeJob(address _jobMaker,uint256 _jobId,bytes[] memory _jobInfo,address[] memory _targetAddresses)external returns(bool){
        ...
        if(success)
        emit JobExecuted(thisJob.jobMaker ,thisJob.jobId ,msg.sender,gasUsed);
        else
        emit JobFailed(thisJob.jobMaker ,thisJob.jobId ,msg.sender,gasUsed);
        return success;
}
...
interface IStaticJobs {
    ...
    event JobExecuted(address indexed jobMaker,uint256 jobId,address executor,uint256 gasUsed);
    event JobFailed(address indexed jobMaker,uint256 jobId,address executor,uint256 gasUsed);
    event GasDeposited(address indexed depositer, uint256 value);
    event GasWithrawn(address indexed withdrawer, uint256 value);
    event JobRegistered(address indexed jobMaker,bytes[] jobInfo,address[] targetAddresses,bytes32 jobHash,uint256 jobId,string name,string[] ipfsForJobDetails);
