...
contract YourCollectible is ERC721, Ownable {
  ...
  constructor() public ERC721("Predictoor", "OOR") {
...
function changeTeam(uint8 _newTeam, uint8 _oldTeam, uint256 _amount, uint256 _mintID) public payable {
   console.log("new team ", _newTeam);
   console.log("old team ", _oldTeam);
  ...
  function mintItem(uint256 _country, uint256 _amount) public payable returns (uint256)
  {
      ...
      _mint(msg.sender, id);
          TokenInfo storage tokens = AllowedCrypto[0];
            ...
            require (paytoken.balanceOf(msg.sender)>=_amount, "You don't have enough USDC");            
            paytoken.transferFrom(msg.sender, address(this), _amount);  // transfer money from better to the contract
     ...
     // returns 0 for percentages under .0001%
  ...
  function claim() public payable returns (uint256)
  {
     ...
     paytoken.transfer(msg.sender, payout );
  }
...
function setWinner(uint8 _country) public onlyOwner(){
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
      ...
      string memory description = string(abi.encodePacked('Predictoor: decentralised, gamified sports prediction. ID #', uint2str(id) ,' . Bet Size ',uint2str(betSize[id] / 1e18 ),'!!!'));
...
'<g transform="translate(130,70)"><animateMotion path="M 4 10 V 0 H 20 Z" dur="1s" repeatCount="2"/>',
'<rect width="250" height="140" stroke="green" stroke-width="2" fill="#fff"></rect>',
'<text x="125" y="60" alignment-baseline="middle" font-size="20" stroke-width="0" stroke="#000" text-anchor="middle">',
countryNames[country[id]],
' $',uint2str(betSize[id] / 1e18),
'</text><text x="50" y="40" style="font-size:20px;">',
'⚽ Predictoor ⚽',
'</text><text x="50" y="120" style="font-size:20px;">Contender</text></g></svg>'
      ));
