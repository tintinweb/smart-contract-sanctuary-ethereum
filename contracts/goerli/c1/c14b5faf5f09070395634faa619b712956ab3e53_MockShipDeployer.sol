...
struct SetupReturnVars {
    ...
    address raiseControlModuleAddress;
}
...
interface IDeployerActions {
    function createAndSetup(SetupVars calldata vars)
        external
...
import {Order, OfferItem, ItemType, ConsiderationItem, OrderType} from "seaport/lib/ConsiderationStructs.sol";
...
contract MockListModule is IListEvents, IListActions, Module {
    error ApproveFail();

    ConsiderationInterface immutable seaport;
    ...
    constructor(
        address _avatar, // usually the safe
        address _target, // Contract to forward execTransactionFromModule() call
        address _seaport
    ) {
        seaport = ConsiderationInterface(_seaport);
        bytes memory initializeParams = abi.encode(_avatar, _target);
...
contract MockShipDeployer is IDeployerEvents, IDeployerActions {
    ...
    address public GNOSIS_SAFE_TEMPLATE_ADDRESS =
        0x3E5c63644E683549055b9Be8653de26E0B4CD36E;
    address public GNOSIS_SAFE_FALLBACK_HANDLER =
        0xf48f2B2d2a534e402487b3ee7C18c33Aec0Fe5e4;

    // From https://github.com/gnosis/zodiac/blob/b5fd8064c596a5ccef3330b072159239768052dd/src/deployments.json#L84
    ModuleProxyFactory public immutable GNOSIS_SAFE_MODULE_FACTORY =
    ...
    function createAndSetup(SetupVars calldata s)
        ...
        returns (SetupReturnVars memory rv)
    {
        // 1: create and deploy safe
        rv.safeAddress = address(
            GNOSIS_SAFE_PROXY_FACTORY.createProxyWithNonce(
                GNOSIS_SAFE_TEMPLATE_ADDRESS,
                "",
                uint256(
                    keccak256(abi.encode(s.name, msg.sender, address(this))) // salt
                )
            )
        );

        ships.push(address(rv.safeAddress));
        rv.captainGuardAddress = address(new MockCaptainGuard());
        rv.shipModifierAddress = address(new MockShipModifier());
        rv.buyModuleAddress = address(new MockBuyModule());
        rv.listModuleAddress = address(
            new MockListModule(rv.safeAddress, rv.safeAddress, SEAPORT)
        );

        // 7: initialze safe and add buy module
        ...
        GnosisSafe(payable(rv.safeAddress)).setup(
            ...
            1, // threshold
            address(this), // to
            abi.encodeCall(this.initSafe, (rv.listModuleAddress)), // data
    ...
    function initSafe(address listModule) external {
        ModuleManager(address(this)).enableModule(listModule);
    }
}
