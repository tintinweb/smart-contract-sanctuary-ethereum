...
contract ComptrollerHarnessProp65 is Comptroller {
    address internal compAddress;

    constructor() Comptroller() {}

    function _backPopulate(
        PriceOracle _oracle,
        address _pauseGuardian,
        address _borrowCapGuardian,
        uint _closeFactorMantissa,
        uint _compRate,
        uint _maxAssets,
        address _compAddress
    ) public {
        require(compAddress == address(0)); // only once

        proposal65FixExecuted = true;
        oracle = _oracle;
        pauseGuardian = _pauseGuardian;
        borrowCapGuardian = _borrowCapGuardian;
