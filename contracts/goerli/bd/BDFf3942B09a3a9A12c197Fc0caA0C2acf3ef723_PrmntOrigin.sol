...
contract CollectionDescriptor {
    function getExtended() public pure returns (string memory){
        // return '.pa g g circle{stroke-dasharray: calc(1px * var(--n, 1) * var(--prmnt-progress, 50)) calc(1px * var(--n, 1) * var(--prmnt-intensity, 50)) !important !important;} .pa g g{fill:none;animation: calc(1s * var(--prmnt-duration, 12)) ease-in-out infinite r1 normal;transform-origin:320px 320px;animation-delay: calc(-1s * var(--n));}@keyframes r1{0%{transform:rotate(0)}50%{transform:rotate(180deg)}}'; // 5px
        return '.pa g g circle{stroke-dasharray:calc(1px * var(--n,1) * var(--prmnt-progress,50)) calc(1px * var(--n,1) * var(--prmnt-intensity,50))!important;stroke-width:calc(1px * var(--n,1) * .2 * var(--prmnt-depth,10));r:calc((5px * var(--n,1) * var(--n,1))/ 2)}.pa g g{fill:none;animation:calc(1s * var(--prmnt-duration,12)) ease-in-out infinite r1 normal;transform-origin:320px 320px;animation-delay:calc(-1s * var(--n))}@keyframes r1{0%{transform:rotate(0)}50%{transform:rotate(180deg)}}';
    }
    ...
    function createArtwork( uint256 hue, uint256 progress, uint256 intensity, uint256 depth) public pure returns (string memory){
        ...
        TWStrings.toString(hue),
        ", 60%, 50%);'>",
        generatePaths(8, hue, progress, intensity, depth),
        '</svg>');
        return parts;
    }

    function generatePaths(uint count, uint256 hue, uint256 progress, uint256 intensity, uint256 depth) private pure returns (string memory) {
        ...
        for (uint i = 1; i < count; i++) {
            ...
            "<g style='--n: ", //  class='c'
            TWStrings.toString(i),
            ";' ><circle stroke-width='",
             TWStrings.toString((depth * i) / 5), 
            "' cx='320' cy='320' r='", 
            TWStrings.toString(radius * 2),
            "' stroke-dashoffset='", 
            // Strings.toString((intensity * radius) * i),
            TWStrings.toString(intensity * 1000),
            "' stroke-dasharray='",
                TWStrings.toString(i * progress),
                " ",
                TWStrings.toString(i * intensity),
            "' /></g>");
        ...
        // return string.concat("<g stroke='hsl(", TWStrings.toString(hue), ", 45%, 25%)' fill='none' >", pathItems, "</g>"); // class='cs-c1'
        return string.concat("<g stroke='hsl(", TWStrings.toString(hue), ", 70%, 25%)' fill='none' >", pathItems, "</g>");
    }    
}
...
██ █████ ██ ██ ███ ██ ██▄ ███ ██
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
Oh heyyyyy. This is a little experimental. 
Just exploring ways svg can be manipulated.
Come visit prmnt.art if you have ideas about dynamic on-chain art. 
Made in Hoi An, 2023
*/
...
contract PrmntOrigin is ERC721Base, CollectionDescriptor{
    struct Attributes {
        uint256 hue;
        uint256 duration;
        uint256 intensity;
    ...
    function setAttributes(uint256 tokenId,  uint256 _duration,  uint256 _progress, uint256 _intensity, uint256 _depth  ) external {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return string.concat('data:application/json;base64,', 
            Base64.encode(
                abi.encodePacked(
                    '{"description": "Dynamic on-chain art by PRMNT", "name": "Hue ', 
                    TWStrings.toString(tokenAttributes[tokenId].hue), 
    ...
    /**
     * @dev Returns the image string
     */
    function getImage(uint256 tokenId) public view returns (string memory ){
        return createArtwork( tokenAttributes[tokenId].hue, tokenAttributes[tokenId].progress, tokenAttributes[tokenId].intensity, tokenAttributes[tokenId].depth);
    }
    
    /**
     * @dev Returns the gallery hash info, if available.
     */
    function getGalleryData(uint256 tokenId) public view returns (string memory ){
        return string.concat(galleryDataBase, tokenIdToGallery[tokenId]);
    }
    /**
     * @dev External facing func to set gallery data
     * @dev Should only work if sender is token owner
     */
    function setGalleryData(uint256 tokenId, string calldata _hash ) external  {
        if (msg.sender != ownerOf(tokenId)) revert Unauthorized();
        tokenIdToGallery[tokenId] = _hash;
    }
    /**
     *  Withdraw Funds
     */
