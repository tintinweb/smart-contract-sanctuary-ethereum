 ...
 *
 * @notice Ethereum mainnet implementation with the fixed supply 120,000,000,000 CULT (non-configurable)
 *
 * @author Basil Gorin
 */
...
contract AdvancedERC20 is ERC1363, EIP2612, EIP3009, AccessControl {
	...
	constructor(
		...
		postConstruct(msg.sender, _name, _symbol, _initialHolder, _initialSupply);
	}

	/**
	 * @dev "Constructor replacement" for a smart contract with a delayed initialization (post-deployment initialization)
	 *
	 * @param _owner smart contract owner (has minting/burning and all other permissions)
	 * @param _name token name to set
	 * @param _symbol token symbol to set
	 * @param _initialHolder owner of the initial token supply
	 * @param _initialSupply initial token supply
	...
	function postConstruct(
		...
		// owner address gets full privileges
		userRoles[_owner] = type(uint256).max; // FULL_PRIVILEGES_MASK
		// emit an ACL event
		emit RoleUpdated(msg.sender, _owner, type(uint256).max, type(uint256).max);
	}
