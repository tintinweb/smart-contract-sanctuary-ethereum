...
contract TherundownLPConsumer_v2_1 is ChainlinkClient {
    ...
    //constants
    uint internal minimumBet = 1 ether / 100;
    ...
    // userAddress => (gameId => rewardsAmount)
    mapping(address => mapping(bytes32 => uint256)) public userRewards;

    struct BetPoolInfo {
        uint homeTeamBetsTotal;
        uint awayTeamBetsTotal;
    }

    // sportId => (gameId => (betType => bets[]))
    mapping(uint256 => mapping(bytes32 => mapping(uint256 => Bet[]))) public bets;

    // gameId => bet Pool info
    mapping(bytes32 => BetPoolInfo) public betPoolInfo;


    // userAddress => array of gameIds
    mapping(address => bytes32[]) public joinedGames;
    ...
    function placeBet(uint256 _sportId, bytes32 _gameId, uint8 _betType) public payable {
        require(msg.value >= minimumBet, "Invalid amount");
        require(_betType == 1 || _betType == 2, "Invalid bet type");
        bets[_sportId][_gameId][_betType].push(Bet(msg.sender, msg.value, _gameId, _sportId, _betType, block.timestamp));
        if (_betType == 1) {
            betPoolInfo[_gameId].homeTeamBetsTotal += msg.value;
        } else {
            betPoolInfo[_gameId].awayTeamBetsTotal += msg.value;
        }
        for (uint i=0; i < joinedGames[msg.sender].length; i++) {
            if (joinedGames[msg.sender][i] == _gameId) {
                return;
            }
        }
        joinedGames[msg.sender].push(_gameId);
    }

    function betsLength(uint256 _sportId, bytes32 _gameId) public view returns(uint256) {
        return bets[_sportId][_gameId][1].length + bets[_sportId][_gameId][2].length;
    }

    function getUserBetsHomeTeam(uint256 _sportId, bytes32 _gameId, address player) public view returns(Bet[] memory) {
        uint256 resultCount;
        Bet[] memory gameBets = bets[_sportId][_gameId][1];

        for (uint i = 0; i < gameBets.length; i++) {
            if (gameBets[i].user == player) {
                resultCount++;  // step 1 - determine the result count
            }
        }
        
        Bet[] memory result = new Bet[](resultCount);  // step 2 - create the fixed-length array
        uint256 j;

        for (uint i = 0; i < gameBets.length; i++) {
            if (gameBets[i].user == player) {
                result[j] = gameBets[i];  // step 3 - fill the array
                j++;
            }
        }
    ...
    function getUserBetsAwayTeam(uint256 _sportId, bytes32 _gameId, address player) public view returns(Bet[] memory) {
        uint256 resultCount;
        Bet[] memory gameBets = bets[_sportId][_gameId][2];
    ...
    function resolveAndCalculateRewards(uint256 _sportId, bytes32 _gameId, uint8 _winnerBetType) public {
        uint homeTeamBetsTotal = betPoolInfo[_gameId].homeTeamBetsTotal;
        uint awayTeamBetsTotal = betPoolInfo[_gameId].awayTeamBetsTotal;
        Bet[] memory winnerBets = bets[_sportId][_gameId][_winnerBetType];

        for (uint i = 0; i < winnerBets.length; i++) {
            uint winAmount;
            if (_winnerBetType == 1) {
                winAmount = (winnerBets[i].amount/homeTeamBetsTotal)*awayTeamBetsTotal;
            } else {
                winAmount = (winnerBets[i].amount/awayTeamBetsTotal)*homeTeamBetsTotal;
            }
            userRewards[winnerBets[i].user][_gameId] += winAmount;
        }
    }

    function claimRewardsForGame(bytes32 _gameId) public payable {
        require(userRewards[msg.sender][_gameId] > 0, "Too low amount");
        uint256 amountToSend = userRewards[msg.sender][_gameId];
        address payable _to = payable(msg.sender);
        bool sent = _to.send(amountToSend);
        require(sent, "Failed to send rewards");
        userRewards[msg.sender][_gameId] = 0;
    }

    function claimAllRewards() public payable {
        uint totalRewards;
        for (uint i = 0; i < joinedGames[msg.sender].length; i++) {
            totalRewards += userRewards[msg.sender][joinedGames[msg.sender][i]];
        }
        address payable _to = payable(msg.sender);
        bool sent = _to.send(totalRewards);
        require(sent, "Failed to send rewards");
    }
