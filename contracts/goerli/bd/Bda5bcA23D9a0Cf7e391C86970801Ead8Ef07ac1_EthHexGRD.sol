    ...
    ContextMixin
{
    ...
    constructor(string memory name_, string memory symbol_, address predicate_role)
        public
        ERC721(name_, symbol_)
    {
        _setupContractId("DummyMintableERC721");
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(PREDICATE_ROLE, predicate_role);
    ...
    function mint(address user, uint256 tokenId, bytes calldata metaData) external override only(PREDICATE_ROLE) {
        ...
        uri[tokenId]= string(metaData);
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, AccessControl,ERC721Enumerable) returns (bool) {
        return
