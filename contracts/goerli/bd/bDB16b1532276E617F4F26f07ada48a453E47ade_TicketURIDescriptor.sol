...
contract TicketURIDescriptor  {
    ...
    returns (string memory)
    {
        bytes memory svg = abi.encodePacked(
            ...
            '<image overflow="visible" width="35" height="35" xlink:href="https://ipfs.io/ipfs/bafkreic4uf2clrepfxpfaqmicul7kb57b22qkd7tycmwsysjeujyzc5sfe" x="10" y="10"></image>',
            '<text x="40" y="90" style="fill:white; font-size: 10px;" text-decoration="underline">Data Subscription Ticket</text>',
            '<text x="40" y="110" style="fill:white; font-size: 8px;">Product ID: 10000</text>',
            '<text x="40" y="120" style="fill:white; font-size: 8px;" >Subscription Period: </text>',
            '<text style="fill:white; font-size: 6px;">',
                '<tspan x="50" y="130" >Start Time: 312341431</tspan>',
                '<tspan x="50" y="140" >End Time: 332341431</tspan>',
            '</text>',
            '</svg>'
        );
        return string(
    ...
    function generateTokenURI(uint256 tokenId, uint256 productId, uint256 startTime, uint256 endTime) public view returns (string memory){
        bytes memory dataURI = abi.encodePacked(
            '{',
                ...
                '"image": "', generateImageURI(productId, startTime, endTime), '"',
            '}'
        );
        return string(
            abi.encodePacked(
