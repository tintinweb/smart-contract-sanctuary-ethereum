...
contract TreasuryAdaptor {
    ...
    function addOperator(address newOperator) public {
        ...
        if (isActiveOperator(newOperator)) {
            // Active address
            if (isRecentlyBlockedOperator(newOperator)) {
                // Just remove address from block timelock queue
            ...
            // Non active address
            if (isRecentlyAddedOperator(newOperator)) {
                revert DuplicatedAddress();
            } else {
    ...
    function removeOperator(address oldOperator) public {
        if (msg.sender != admin) revert Unauthorized();
        if (isActiveOperator(oldOperator)) {
            // Active address
            if (isRecentlyBlockedOperator(oldOperator)) {
                revert DuplicatedAddress();
            } else {
            ...
            // Non active address
            if (isRecentlyAddedOperator(oldOperator)) {
                // Just remove from allow list timelock
                operators[oldOperator] = 0;
            } else {
                revert RemoveNonActiveAddress();
            }
        }
    }
    ...
    function addWithdrawalAddress(address recipient) public {
        if (msg.sender != admin) revert Unauthorized();
        if (isActiveWithdrawalAddress(recipient)) {
            // Active address
            if (isRecentlyBlockedWithdrawalAddress(recipient)) {
                // Just remove the address from block timelock
            ...
            // Non active address
            if (isRecentlyAddedWithdrawalAddress(recipient)) {
                revert DuplicatedAddress();
            } else {
    ...
    function removeWithdrawalAddress(address recipient) public {
        ...
        if (isActiveWithdrawalAddress(recipient)) {
            ...
            // Non active address
            if (isRecentlyAddedWithdrawalAddress(recipient)) {
                // Just remove address from allow list timelock
                withdrawalAddresses[recipient] = 0;
            } else {
                revert RemoveNonActiveAddress();
            }
        }
    ...
    /// @dev Check if the operator is recently added
    /// The address is added but not yet passes the timelock
    /// @param addr address to check
    /// @return boolean to indicate if the address is recently added
    function isRecentlyAddedOperator(address addr) public view returns (bool) {
        return operators[addr] != 0 && !completedTimelock(operators[addr]);
    }

    /// @dev Check if the withdrawalAddress is recently added
    /// The address is added but not yet passes the timelock
    /// @param addr address to check
    /// @return boolean to indicate if the address is recently added
    function isRecentlyAddedWithdrawalAddress(address addr)
        public
        view
        returns (bool)
    {
        return
            withdrawalAddresses[addr] != 0 &&
            !completedTimelock(withdrawalAddresses[addr]);
    }

    /// @dev Check if the operator is recently blocked
    /// The address is blocked but not yet passes the timelock
    /// @param addr address to check
    /// @return boolean to indicate if the address is recently blocked
    function isRecentlyBlockedOperator(address addr)
        public
        view
        returns (bool)
    {
        return
            operatorsBlocklist[addr] != 0 &&
            !completedTimelock(operatorsBlocklist[addr]);
    }

    /// @dev Check if the withdrawalAddress is recently blocked
    /// The address is blocked but not yet passes the timelock
    /// @param addr address to check
    /// @return boolean to indicate if the address is recently blocked
    function isRecentlyBlockedWithdrawalAddress(address addr)
        public
        view
        returns (bool)
    {
        return
            withdrawalAddressesBlocklist[addr] != 0 &&
