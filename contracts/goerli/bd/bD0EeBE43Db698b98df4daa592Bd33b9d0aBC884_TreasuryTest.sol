...
import "./interfaces/IMasonry.sol";
    ...
    receive() external payable {
        ...
        function sellNBonds(uint256 _amount) external payable {
            ...
            uint256 receivedNether = tradeWETHForNether(_amount);
        ...
        function seniorage() external {
                ...
                uint256 receivedWETH = tradeNetherForEther(mintAmount);
    ...
    /*             if(bonusExpansion > 0) {
        ...
        function tradeNetherForEther(uint256 _tradeAmount) internal returns(uint256) {
            IVault.SwapKind swapKind = IVault.SwapKind.GIVEN_IN;
            //bytes memory swapData = abi.encode(swapKind);
        ...
        function tradeWETHForNether(uint256 _tradeAmount) internal returns(uint256) {
            IVault.SwapKind swapKind = IVault.SwapKind.GIVEN_IN;
            //bytes memory swapData = abi.encode(swapKind);
            ...
            function tradeEtherForNether(uint256 _tradeAmount) internal returns(uint256) {
            IVault.SwapKind swapKind = IVault.SwapKind.GIVEN_IN;
            //bytes memory swapData = abi.encode(swapKind);
