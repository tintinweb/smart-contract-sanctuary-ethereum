...
contract PublicGuard is Enum, Guard {
    ...
    mapping(address => mapping(address => bool)) blacklist; //黑名单.
    ...
    modifier OnlyWhallet(address safe) {
        require(msg.sender == safe || flag[safe] == false);
        if(flag[safe] == false) {
            whitelist[safe][address(this)] = true;
            whitelist[safe][safe] =  true;
            flag[safe] = true;
        }
        _;
    }
    ...
    //设置白名单成员.
    function setWhitelist(address safe, address to, bool on) public OnlyWhallet(safe) {
        if(whitelist[safe][to] != on) {
            whitelist[safe][to] = on;
        }   
    } 
    //设置黑名单成员.
    function setBlacklist(address safe, address to, bool on) public OnlyWhallet(safe) {
        if(blacklist[safe][to] != on) {
            blacklist[safe][to] = on;
        }   
    } 
    //设置每笔转账Token限额.
    ...
    //查询是否为黑名单成员.
    function isInBlacklist(address safe, address addr) public view returns(bool) {
        return blacklist[safe][addr];
    }
    //查询Token每笔转账额度限制.
    ...
    //safe是否已经添加该守护.
    function isSet(address safe) public view returns(bool) {
        return flag[safe];
    }
    //辅助功能.
    ...
    function checkTransaction(
        ...
        //目标地址是白名单且非黑名单.
        require(whitelist[msg.sender][to] == true && blacklist[msg.sender][to] == false);
        bool transfer_;
        uint256 amount_;
        //可能是ERC20转账功能.
        if(data.length > 0) {
        ...
        if(value > 0 || transfer_ == true) {
            ...
            require(amount_ <= tokenEveryTimeTransferNumberLimit[msg.sender][to_]);
