...
contract ListHash {
    ...
    function getVersion(bytes memory inputData) 
		...
		*/
        ...
        version.permalink = topics.getField(1).toUint();   
        version.relayId = uint128(topics.getField(2).toUint());
        version.roothash = topics.getField(3).toUint();
        version.version = abi.decode(log.getData(), (uint128));
        version.timestamp = payload.getBlockTime();
