...
contract feeNFT is ERC721URIStorage, Ownable {
    string internal s_activatedUri; //token URI of the actual NFT
    string internal s_notActivatedUri; //token URI for inactive "memberships", should reflect the inactive state
    uint256 private s_tokenCounter; //counter of tokens to be minted
    uint256 private immutable i_mintFee; //fee payed per mint
    uint256 private immutable i_subscriptionFee; //fee payed every time interval
    uint256 private immutable i_time; //time interval for which the tokenId is activated

    mapping(uint256 => uint256) s_validUntil; //mapping from tokenId to (block.timestamp + time)
    ...
    constructor(
        uint256 mintFee,
        uint256 monthlyFee, //fee payed every 4 weeks
    ...
    function payFee(uint256 tokenId) public payable {
        if (msg.value < i_subscriptionFee) {
            revert RandomIpfsNft__NeedMoreETHSent();
        }
    ...
    function getIsActivated(uint256 index) public view returns (bool) {
        if (block.timestamp > s_validUntil[index]) {
            return false;
        } else {
            return true;
        }
