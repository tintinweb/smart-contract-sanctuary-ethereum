...
contract Collections is Initializable, UUPSUpgradeable {
    ...
    */
    ...
    function create_collection(string memory name, uint256 supply, string memory uri_, uint256 royaltyValue, string memory contract_Metadata_Uri, address royaltyRecipient) public {
            // require(msg.value >= 0, "Not enough amount for the Fees");
            ERC712Base ERC712 =new ERC712Base(name, name, contract_Metadata_Uri);
            CollectionsRecord[CollectionCount] = CollectionsDetails(ERC712, name, msg.sender);
            collectionOwners[address(msg.sender)].ids.push(CollectionCount);
            ERC712.safeMint(msg.sender, uri_, royaltyRecipient, royaltyValue);
            CollectionCount += 1;
            OwnerRole[address(ERC712)] = address(msg.sender);
            emit CollectionCreateEvent(address(msg.sender), address(ERC712), CollectionCount);
     ...
     */
    function mint(ERC712Base Collection, uint256 supply, string memory uri_, uint256 royaltyValue, address royaltyRecipient) external payable {
        ...
        Collection.safeMint(msg.sender, uri_, royaltyRecipient, royaltyValue);
    }
...
contract ERC712Base is ERC721, ERC721URIStorage, Ownable, ERC2981Base {
    ...
    constructor(string memory _name, string memory _symbol, string memory URI_contract) ERC721(_name, _symbol) {
        metadata_uri = URI_contract;
    }

    function safeMint(address to, string memory uri, address royaltyRecipient, uint royaltyValue) public onlyOwner {
