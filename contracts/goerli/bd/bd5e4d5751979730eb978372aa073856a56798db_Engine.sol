...
library FeesMath {
    ...
    ) public view returns (int256 premium_, uint256 premiumFee) {
        uint256 notional = o.amount.wmul(p.price);
        premium_ = premiumBps.bps(notional);
        int256 fee = config.premiumFeeBps().bps(premium_);
...
uint256 constant EXPIRATION_ORDER_PERIOD = 7200;
...
contract Engine is IEngineEvents {
    ...
    function join(Order storage order, PriceData memory priceData) internal {
        ...
        (int256 premiumT, uint256 premiumFee) = order.premium(
            priceData,
            m.premiumBps,
            config,
    ...
    function unjoin(uint256 matchId, PriceData memory priceData) internal {
        ...
        openInterest -= m.amount; // is this correct here ?
        emit CloseMatch(
            matchId,
            m.maker,
            m.trader,
    ...
    {
        uint256 newAmount = m.amount - o.amount;
        ...
        newMatch.minPremiumFeeDiscountPerc =
            m.minPremiumFeeDiscountPerc -
            uint8((uint256(m.minPremiumFeeDiscountPerc) * o.amount) / m.amount);
    ...
    function liquidate(
        ...
        for (uint256 i = 0; i < matchIds.length; i++) {
            ...
            Match memory m = matches[matchId];
            (int256 pnlM, int256 pnlT, bool isLiq) = m.isLiquidatable(
                tokenId,
                p.price,
                config,
                tokenDecimals
    ...
    function validateOICap(uint256 price, uint256 orderAmount)
        internal
        view
        returns (bool)
    {
        if (vault.capLimit() == 0) return false;
